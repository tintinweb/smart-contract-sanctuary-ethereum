...
contract Trailblaze is ERC721A, IERC2981, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public presaleMintCounts; // used for presale lists
    bytes32 public reserveMerkleRoot;
    bytes32 public launchlistMerkleRoot;
    bytes32 public blazelistMerkleRoot;

    // Sale info
    bool public isPublicSaleActive;
    bool public isPresaleActive;

    uint256 public constant MAX_LAUNCHLIST_SALE_MINTS = 1;
    uint256 public constant LAUNCHLIST_SALE_PRICE = 0.02 ether;

    uint256 public constant MAX_BLAZELIST_SALE_MINTS = 3;
    uint256 public constant BLAZELIST_SALE_PRICE = 0.05 ether;

    uint256 public constant MAX_PUBLIC_SALE_MINTS = 5;
    uint256 public constant PUBLIC_SALE_PRICE = 0.06 ether;

    // dynamic based on current batch
    uint256 public currentBatchMinted = 0;
    uint256 public currentBatchSize = 0;

    address public royaltyReceiverAddress;

    constructor(address _royaltyReceiverAddress, uint256 batchSize)
        ERC721A("Trailblaze Collective", "TRAIL")
    {
    ...
    modifier canMint(uint256 numberOfTokens) {
        ...
        require(
            currentBatchMinted + numberOfTokens <= currentBatchSize,
            "Insufficient tokens in batch"
        );
        _;
    }
    ...
    modifier canGiftTokens(uint256 numberOfTokens) {
        ...
        require(
            _totalMinted() + numberOfTokens <= MAX_TOTAL_SUPPLY,
            "Insufficient tokens remaining"
        );
        require(
            currentBatchMinted + numberOfTokens <= currentBatchSize,
        ...
        maxTokensPerWallet(numberOfTokens)
    {
        ...
        currentBatchMinted += numberOfTokens;
    }

    /**
     * @dev mints 1 token per allowlisted address
     * This is our presale list created for NFT newbies who will be purchasing their first ever NFT with us. Its meant to ease financial barriers to entry.
     * Only active during a presale
     */
    function mintLaunchlist(bytes32[] calldata merkleProof)
        external
        payable
        presaleActive
        nonReentrant
        canMint(MAX_LAUNCHLIST_SALE_MINTS)
        isCorrectPayment(LAUNCHLIST_SALE_PRICE, MAX_LAUNCHLIST_SALE_MINTS)
        isValidMerkleProof(merkleProof, launchlistMerkleRoot)
    {
        uint256 numAlreadyMinted = presaleMintCounts[msg.sender];
        require(
            numAlreadyMinted < MAX_LAUNCHLIST_SALE_MINTS,
            "Address has already claimed"
        );
        presaleMintCounts[msg.sender] += MAX_LAUNCHLIST_SALE_MINTS;
     ...
     * Only active during a presale
     */
    function mintBlazelist(uint8 numberOfTokens, bytes32[] calldata merkleProof)
        external
        payable
        presaleActive
        nonReentrant
        canMint(numberOfTokens)
        isCorrectPayment(BLAZELIST_SALE_PRICE, numberOfTokens)
        isValidMerkleProof(merkleProof, blazelistMerkleRoot)
    {
        uint256 numAlreadyMinted = presaleMintCounts[msg.sender];

        require(
            numAlreadyMinted + numberOfTokens <= MAX_BLAZELIST_SALE_MINTS,
    ...
    function setLaunchlistMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        launchlistMerkleRoot = merkleRoot;
    }

    function setBlazelistMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        blazelistMerkleRoot = merkleRoot;
    }

    /**
     * @dev start a new batch. cancels existing batch and rolls over unused tokens to the next.
     */
    function startNewBatch(uint256 batchSize) external onlyOwner {
        ...
        canGiftTokens(numToReserve)
    {
        ...
        currentBatchMinted += numToReserve;
    }
