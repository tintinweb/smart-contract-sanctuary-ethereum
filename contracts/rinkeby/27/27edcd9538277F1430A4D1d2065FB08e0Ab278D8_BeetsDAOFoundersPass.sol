...
contract BeetsDAOFoundersPass is ERC721A, AccessControl{
    bytes32 public constant DAO_MINTER_ROLE = keccak256("DAO_MINTER_ROLE");
    bytes32 private merkleRoot;

    string private baseURI;
    uint256 public price = 0.2 ether;
    uint256 private privateMint = 100;
    ...
    function beetslistMint(bytes32[] calldata _merkelProof, uint256 quantity) external {
        //checking if user(s) claimed their nft
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkelProof, merkleRoot, leaf), "Not allowed!");
        require(balanceOf(msg.sender) + quantity <= allowList[msg.sender], "Quantity exceeds max mint!");
    ...
    function mint(uint256 quantity) external payable returns(bool){
        ...
        require(balanceOf(msg.sender) + quantity <= 10 + allowList[msg.sender], "Quantity exceeds max mint!");
    ...
    function initBeetslist(Allowed[] memory allowedList) external onlyRole(DEFAULT_ADMIN_ROLE){
        for(uint256 i = 0; i < allowedList.length; i++){
            allowList[allowedList[i].user] = allowedList[i].allowedAmount;
        }
    }

    function setPriceInWei(uint256 newPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
        price = newPrice;
    }
    ...
    function withdraw(address payable receiverAddress, uint _withdrawAmount) external onlyRole(DEFAULT_ADMIN_ROLE){
