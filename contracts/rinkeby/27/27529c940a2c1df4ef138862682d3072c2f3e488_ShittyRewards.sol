...
contract ShittyRewards is Pausable, Ownable, ReentrancyGuard{
    ...
    uint256 public constant MIN_TOKEN_REWARD = 0.0002 ether; //0.2 ether;
    ...
    function adjustIfTokenIdAlreadyRewarded(uint256 inputTokenId, uint256 totalSupply) private view returns(uint){
        if(repeatRewardsPerTokenEnabled || !allotedShittyReward[inputTokenId].rewardClaimed){
            ...
            revert("No tokenId found for reward allocation.");
        }
    }
    
    function claimRewards(uint256[] memory _tokenIds) public whenNotPaused nonReentrant returns(bool){
    ...
    function getAllTokenIdsWithRewards() public view whenNotPaused onlyOwner returns (uint256[] memory) {
        uint256 totalSold =  nftContractAddress.totalSupply();
        TokensWithRewards[] memory tempTokensWithRewards = new TokensWithRewards[](totalSold);
        uint numOfRewardedTokens = 0;
        for(uint i = 0; i < totalSold; i++){
            if (!allotedShittyReward[i].rewardClaimed && allotedShittyReward[i].rewardAmountInWei > 0) 
            {
                tempTokensWithRewards[numOfRewardedTokens] = TokensWithRewards(i, allotedShittyReward[i].rewardAmountInWei, allotedShittyReward[i].rewardClaimed);
                numOfRewardedTokens = numOfRewardedTokens + 1;
            }          
        }
