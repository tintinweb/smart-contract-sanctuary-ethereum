...
contract BrokerDefiPro is ERC721, Ownable, ReentrancyGuard {
    ...
    address public BROKER_DEFI_PARTNER;
    ...
    uint public PARTNER_DISCOUNT;
    uint public PRO_COMMISSION;
    uint public PRO_DISCOUNT;
    ...
    constructor(
        ...
        BROKER_DEFI_PARTNER = brokerDefiPartner;
    ...
    */
    ...
    function publicMintPartner(uint amount, uint code) public payable nonReentrant {
        ...
        if (code > 0) {
            require(IPartner(BROKER_DEFI_PARTNER).partnerCodesVerification(code), "Wrong code");
            ...
            address payable recruiter = payable(IPartner(BROKER_DEFI_PARTNER).ownerOf(tokenId));
    ...
    // mint function for public sale with pro code
    function publicMintPro(uint amount, uint code) public payable nonReentrant {
        require(saleIsActive && nftPrice > 0 && treasury != address(0), "Config not done yet");
        require(msg.value >= (nftPrice * amount), "Not enough balance");
        if (code > 0) {
            require(proCodesVerification[code], "Wrong code");
            proCodesCount[code] += 1;
            uint commission = (msg.value / 100) * PRO_COMMISSION;
            uint discount = (msg.value / 100) * PRO_DISCOUNT;
