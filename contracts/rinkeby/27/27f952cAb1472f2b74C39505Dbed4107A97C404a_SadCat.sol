...
contract SadCat is ERC721A, Ownable {
    ...
    uint256 public currentMintAllowance = 0;
    ...
    function publicMint(uint256 _mintAmount) external payable publicSaleIsLive {
        ...
        require(currentMintAllowance > 0, "no more mints allowed");
        ...
        currentMintAllowance = currentMintAllowance - _mintAmount;
    ...
    function addToWhiteList(address[] calldata _addresses,uint256[] calldata qty) external onlyOwner {
        require(_addresses.length == qty.length, "Arrays not the same size");
