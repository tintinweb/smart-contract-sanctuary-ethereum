...
contract Pomo is ERC721A, Ownable, ReentrancyGuard {
    ...
    function mint(bytes memory signature, uint256 quantity) external payable {
        ...
        require(isValid(mintHashPrefix, msg.sender, address(this), signature, owner()), "Invalid signature.");
    ...
    function isValid(string memory _prefix, address _address, address _contractAddress, bytes memory _signature, address owner) private pure returns (bool) {
        return keccak256(abi.encodePacked(_prefix, _address, _contractAddress)).recover(_signature) == owner;
    }
