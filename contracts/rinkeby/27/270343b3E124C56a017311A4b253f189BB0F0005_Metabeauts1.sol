 ...
 */
contract Metabeauts1 is ERC721, Ownable {
...
// price: 100000000000000
  ...
  constructor(
    ...
    address  _taxWallet,
    uint256 _tokenPrice
  ) ERC721("Metabeauts", "MB") {
    baseURI = _initbaseURI;
    taxWallet = payable(_taxWallet);
    tokenPrice = _tokenPrice;

    MAX_PER_WALLET = 100;
    MAX_SUPPLY = 100; //TODO: is this correct?
  ...
  modifier checkWallet(uint256 _mintAmount) {
     require(_mintAmount + balanceOf(msg.sender) <= MAX_PER_WALLET, "Minting more than max allowed");
     _;
  }

  function presaleMint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable correctValue(_mintAmount) checkWallet(_mintAmount){
  ...
  function publicMint(uint256 _mintAmount) public payable correctValue(_mintAmount)checkWallet(_mintAmount){
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    return string(abi.encodePacked(base_, tokenId, '.json'));
  }
  ...
  function airdrop(address[] memory _users) external onlyOwner {
    ...
    for(uint256 i = 1; i <= _users.length; i++){
       uint256 tokenId = getRandomAvailableTokenId(_users[i-1], updatedNumAvailableTokens);
  ...
  function _mint(uint256 _mintAmount) internal {
    ...
    for(uint256 i = 1; i <= _mintAmount; i++){
        uint256 tokenId = getRandomAvailableTokenId(msg.sender, _numAvailableTokens);
        _mintedSupply.increment();
