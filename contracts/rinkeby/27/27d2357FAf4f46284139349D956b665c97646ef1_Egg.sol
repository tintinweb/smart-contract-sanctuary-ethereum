...
contract Egg is ERC721URIStorage, Ownable {
    ...
    function makeEgg(uint256 _tokenId, uint256 _baseR) private {
        string memory eggURI = svgComponents.eggGen(_tokenId, getColors(_baseR), getChoices(_baseR));
...
library svgComponents {
    ...
    function eggGen(uint256 _tokenId, string[5] memory colors, uint8[5] memory choices) external pure returns (string memory eggURI) {
        ...
        string memory finalSVG = addString('',string('<svg xmlns="http://www.w3.org/2000/svg"version="1.1"height="500px"width="500px"><defs>'),pat,defs);
        delete pat;
        ...
        finalSVG = string(abi.encodePacked('data:image/svg+xml;base64,',Base64.encode(bytes(abi.encodePacked(finalSVG)))));
