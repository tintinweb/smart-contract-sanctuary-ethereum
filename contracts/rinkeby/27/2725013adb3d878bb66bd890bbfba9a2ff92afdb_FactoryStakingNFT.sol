...
interface IStakeERC721 {
    ...
    event AddtotalMintAmount(address indexed user, uint256 pendingtotalMintAmount, uint256 totalMintAmount);
   ...
   function initialize(
    ...
    function setlastRewardTime(address _pair, uint256 _lastRewardTime) external;
    function setDMDPerSecond(uint256 _DMDPerSecond) external;
    function addtotalMintAmount(uint256 _pendingtotalMintAmount) external;
...
contract FactoryStakingNFT is Ownable, Initializable {
    address public logicImplement;

    event StakingCreated(address indexed stakingNFT, address indexed logicImplement);

    event SetLogicImplement(address indexed user, address oldLogicImplement, address newLogicImplement);


       function initialize(
       address _logicImplement 
     ) external  onlyOwner {
    ...
    function createStaking(
        ...
        TransparentUpgradeableProxy proxystakingNFT = new TransparentUpgradeableProxy(logicImplement, proxyAdmin, '');
        IStakeERC721 stakingNFT = IStakeERC721(address(proxystakingNFT));
        stakingNFT.initialize(_token, _startTime, _DMDPerSecond, _totalMintAmount, owner);
        emit StakingCreated(address(stakingNFT), logicImplement);
        return address(stakingNFT);
    }
