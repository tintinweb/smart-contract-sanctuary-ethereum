...
contract TestDAO is ERC721A ,Ownable {
    ...
    address public              infrastructureWalletAddress;

    uint16 public               PERMILLESHARE_COMMUNITY=750;  
    uint16 public               PERMILLESHARE_MARKETING=120;
    uint16 public               PERMILLESHARE_TEAM=60;
    uint16 public               PERMILLESHARE_CHARITY=50;
    uint16 public               PERMILLESHARE_INFRASTRUCTURE=20;
    

    bytes32 public              whitelistMerkleRoot;
    uint256 public              MAX_SUPPLY = 10000;                // Real number actually 1 lower, written this way for gas optimization

    
    uint256 public              MAX_PER_TX             = 11;  // Real number actually 1 lower, written this way for gas optimization
  ...
  constructor(
    ...
    address[] memory _proxyRegistryAddresses, 
    address _communityWalletAddress,
    address _marketingWalletAddress,
    address _teamWalletAddress,
    address _charityWalletAddress,
    address _infrastructureWalletAddress


  ) ERC721A("TestDAO", "TESTDAO") {
    baseURI = _baseURI;
    proxyRegistryAddresses = _proxyRegistryAddresses; 
    ...
    infrastructureWalletAddress = _infrastructureWalletAddress;

  }


    // initialize for upgradeable contract
  ...
  function setProxyRegistryAddresses(address[] calldata _proxyRegistryAddresses) external onlyOwner {
        proxyRegistryAddresses = _proxyRegistryAddresses;
    }
  ...
  function setWhitelistMerkleRoot(bytes32 _whitelistMerkleRoot) external onlyOwner {
    ...
    function reserveMint(uint256 count) public onlyOwner {
        require(ownerMinted+count <= RESERVES , 'Would exceed reserved supply');
        ownerMinted += count;
        _safeMint(_msgSender(),count);
    ...
    */
    ...
    function isApprovedForAll(address _owner, address operator) public view override returns (bool) {
        ...
        for (uint i=0; i<proxyRegistryAddresses.length;i++)
        {
        MarketplaceProxyRegistry proxyRegistry = MarketplaceProxyRegistry(proxyRegistryAddresses[i]);
