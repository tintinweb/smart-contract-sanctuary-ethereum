...
contract YOLONounsAuctionHouse is IYOLONounsAuctionHouse, PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function mintNoun(uint256 deployedNextNounId) external payable override whenNotPaused {    
        require(msg.value == reservePrice, 'Incorrect ETH amount');
        require(lastBlockNumber < block.number, 'Already YOLOed on this block');

    	(uint256 _deployedNounId, , , , , ) = NounsAuctionHouse(deployedAuction).auction();
		require(deployedNextNounId == (_deployedNounId + 1), 'Noun Id does not match');
		        
        lastBlockNumber = block.number;
        YOLONounsToken(nouns).mint(msg.sender, deployedNextNounId);
    }
...
interface IYOLONounsAuctionHouse {
    ...
    function mintNoun(uint256 deployedNextNounId) external payable;
...
interface IYOLONounsToken is IERC721 {
    function mint(address to, uint256 deployedNextNounId) external;
