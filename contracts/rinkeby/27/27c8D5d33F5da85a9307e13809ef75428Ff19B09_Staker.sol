...
contract Staker {
  ...
  function stake() public payable notCompleted{
    require(timeLeft() > 0, "deadline already met, call execute");
  ...
  function execute() external notCompleted{
    ...
    require(!openForWithdraw, "execute already called");
  ...
  function withdraw(address payable _to) external notCompleted{
    ...
    uint amount = balances[_to];
    //require(amount > 0, "Caller has no balance");//removed so test passes
    balances[_to] = 0;//set balances[msg.sender] to zero BEFORE transfer
    _to.transfer(amount);
  }
