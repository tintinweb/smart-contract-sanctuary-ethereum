...
Controllable
{
    ...
    function initialize(address owner) public initializer {
        _addController(owner);
    }

    /// @notice only allow owner of the contract
    modifier onlyOwner() {
        require(_isController(msg.sender), "Only owner can call this function");
        _;
    }
    ...
    /// @notice get the base url for the token
    function getBaseUrl() public view returns (string memory) {
        return _baseUri;
    }

    /// @notice set the base url for the token
    function setBaseUrl(string calldata baseUri) public onlyOwner {
        _baseUri = baseUri;
    }

    function setUri(string calldata __uri) public onlyOwner {
        _uri = __uri;
    }

    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        require(_totalBalances[tokenId] > 0, "Token is not minted yet");
        return bytes(_baseUri).length != 0 ? string(abi.encodePacked(_baseUri, tokenId.toString())) : _uri;
    }
