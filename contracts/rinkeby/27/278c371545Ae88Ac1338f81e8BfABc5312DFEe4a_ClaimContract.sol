    ...
    AccessControlUpgradeable
{
    ...
    function initialize(address _distributionRoleAddress) public initializer {
        __Ownable_init();
        __Pausable_init_unchained();
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(DISTRIBUTION_ROLE, _distributionRoleAddress);
        __EIP712_init("TheCypherverse", "1.0.0", _distributionRoleAddress);
    }
     ...
     * @param wallet Wallet allowed to claim the ERC1155 NFT token
     * @param ids Ids allowed to claim the ERC1155 NFT token
     * @param amounts Amount allowed to claim the ERC1155 NFT token
    ...
    function metaClaimERC1155(
        ...
        require(wallet == _msgSender(), "VoxoDeus: Invalid reception Wallet");
        bytes memory data = "";

        bytes32 message = createMetaClaimERC1155(wallet, token_ids, quantities);
        ...
        token.safeBatchTransferFrom(signer, wallet, ids, amounts, data);
        emit tokenERC1155Claimed(wallet, ids, amounts);
    }
     ...
     * @param signature Signature of the payload
     * @param wallet Wallet allowed to claim the ERC721 NFT token
     * @param ids Ids allowed to claim the ERC721 NFT token
     * @param token_ids TokenIds Hashed allowed to claim the ERC721 NFT token
     * @param deadline timestamp of deadline to claim
     */
    function metaClaimERC721(
        ...
        bytes32 message = createMetaClaimERC721(wallet, token_ids);
        address signer = validateMessageSignature(message, signature);
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            token.safeTransferFrom(signer, wallet, ids[i], data);
        }
        emit tokenERC721Claimed(wallet, ids);
    }
     ...
     * @param wallet Wallet allowed to claim the ERC721 NFT token
     * @param quantities Amount Hashed allowed to claim the ERC721 NFT token
     */
    function createMetaClaimERC721(address wallet, string memory quantities)
        internal
        view
        returns (bytes32)
    {
