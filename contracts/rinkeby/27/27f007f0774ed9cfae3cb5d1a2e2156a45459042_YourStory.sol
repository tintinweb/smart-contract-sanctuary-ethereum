...
contract YourStory is WordTable, AddressShifter, ERC721Royalty, Ownable {
    ...
    function contractURI() external pure returns (string memory) {
        return "https://yourstory-green.vercel.app/curi.json";
    }
    ...
    // function multiPrice(uint256 num) internal view returns (uint256 price) {
    //     uint256 endTokenId = currentTokenId + num;
    ...
    // function mintMulti(address recipient, uint256 numToMint)
    //     public
    //     payable
    //     returns (uint256[] memory)
    // {
    //     if (msg.value != multiPrice(numToMint)) {
    //         revert MintPriceNotPaid();
    //     }

    //     // increment currentTokenId
    //     uint256 startingTokenId = currentTokenId + 1;
    //     uint256 endingTokenId = currentTokenId + numToMint;
    //     currentTokenId = endingTokenId; // update current to reflect
    //     if (currentTokenId > TOTAL_SUPPLY) {
    //         revert MaxSupply();
    //     }

    //     // check minters individual max
    //     uint256 currentMintCount = mintCount[msg.sender] + 1;
    //     uint256 newMintCount = mintCount[msg.sender] + numToMint;
    //     if (newMintCount > MAX_MINTS) {
    //         revert MaxAddressMints();
    //     }
    //     mintCount[msg.sender] = newMintCount;
    ...
    //         } else {
    //             tokenIdToMintSeed[tokenId] = shiftBytes20Address(
    //                 bytes20(msg.sender),
    //                 uint8(currentMintCount - 1)
    //             );
    //         }
    //         ++currentMintCount;
    //         tokenIds[i] = tokenId;
        ...
        returns (string memory)
    {
        ...
        string memory output = sentance[0];
        for (uint256 i = 1; i < 40; i++) {
            output = string.concat(output, " ", sentance[i]);
        }
