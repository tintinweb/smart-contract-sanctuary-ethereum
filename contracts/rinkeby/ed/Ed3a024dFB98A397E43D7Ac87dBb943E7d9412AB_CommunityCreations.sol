    ...
    ReentrancyGuard
{
    ...
    /**
     * @dev accept mint fee and mint a Creation
     * @dev if not public mint, only MEV army holders can mint
    ...
    function mintCreation(uint256 _tokenId) public payable {
        ...
        if (!creation.isPublicMint) {
            require(
                IERC721(mevArmyAddress).balanceOf(msg.sender) > 0,
                "Only MEV Army holders may mint this creation."
            );
        }
        ...
        if (!creation.isFree) {
            ...
            address creator = tokenIdToCreation[_tokenId].creatorAddress;
            creatorBalance[creator] += currentPriceInEth;

            // refund any excess funds to the minter
            if (msg.value > currentPriceInEth) {
    ...
    /**
     * @dev make a new Creation and mint the first token to the creator
    ...
    /**
     * @dev update the tokenURI of a Creation. Only the creator of a Creation can update its tokenURI.
    ...
    /**
     * @dev update the creatorAddress for a given Creation. Only the creator of a Creation can update its creator address.
     * @param _tokenId of the Creation you want to update
    ...
    /**
     * @dev update the isFree parameter for a given Creation. Only the creator of a Creation can update whether a Creation is free or not.
     * @param _tokenId of the Creation you want to update
    ...
    /**
     * @dev update the isPublicMint parameter for a given Creation. Only the creator of a Creation can update whether a creation is public mint or not.
     * @param _tokenId of the Creation you want to update
     * @param _isPublicMint parameter of the Creation
     */
    function updateCreationIsPublicMint(uint256 _tokenId, bool _isPublicMint)
        external
    {
        Creation storage creation = tokenIdToCreation[_tokenId];
        if (msg.sender != creation.creatorAddress) revert NotCreator();
    ...
    // ====================== View Functions ====================== //
    /**
     * @dev get a list of all Creations created by the given creator.
     * @param _creator whose Creations you want
     */
    function getCreations(address _creator)
        public
        view
        returns (Creation[] memory)
    {
        Creation[] memory creations = new Creation[](numCreations);

        for (uint i; i < numCreations; i++) {
