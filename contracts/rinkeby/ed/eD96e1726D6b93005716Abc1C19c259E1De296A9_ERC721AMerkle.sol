...
contract ERC721AMerkle is Ownable, ERC721A, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeMath for uint8;

    uint256 public immutable MAX_SUPPLY = 8;
    uint256 public constant PRICE = 10_000_000_000_000_000; // 0.01 ET
    uint256 public immutable amountForTeam;
    uint256 public maxPublic; //max per wallet, public mint
    uint256 public maxPerPublicTx = 5; // max per tx on public mint
    uint256 public maxPresale = 2;

    bool _isActive = false;
    bool _presaleActive = false;

    // allow list token counter
    mapping(address => uint8) private _preSaleListCounter;

    // allow list token counter
    mapping(address => uint8) private _allowListCounter;

    // on allow list boolean
    ...
    ) ERC721A(name, symbol, maxMint_, amountForTeam_) {
        maxPublic = maxMint_;
        ...
        callerIsUser
    {
        ...
        require(
            _allowListCounter[msg.sender] + quantity <= maxPublic,
    ...
    // For marketing etc.
    function internalMint(uint256 quantity) external onlyOwner {
        require(
    ...
    function mintPreSaleTokens(
        ...
        require(
            _preSaleListCounter[msg.sender].add(numberOfTokens) <= maxPresale,
