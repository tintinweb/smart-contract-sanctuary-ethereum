...
contract SmuppyInu is ERC20, AccessControl {
    ...
    constructor(address router) ERC20("Smuppy Inu", "SMUPPY") {
        _mint(_msgSender(), 10000000 * 10 ** decimals()); //mint total supply.
        maxAmount = totalSupply(); // setting maximum transferable amount.
    ...
    function _transfer(
        ...
        if((!hasRole(DEFAULT_OWNER, from)) && (!hasRole(DEFAULT_OWNER, to))) {
            require(maxAmount >=  amount, "SmuppyInu: maxAmount >= amount");
        }
