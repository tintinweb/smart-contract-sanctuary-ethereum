...
contract TexasPoker2 is Ownable, IERC721Receiver {
    ...
    function pledgeETH(address token, uint256 tokenId, uint256 location) public payable {
        ...
        require(_gameInfo.playerNumber < 2, "The room is full");
    ...
    function redeemETH(address token, uint256 tokenId, uint256 location) public {
        GameInfo storage _gameInfo = gameInfos[token][tokenId];
        require(_gameInfo.playerNumber < 2, "Cannot be redeemed");
    ...
    function settlement(address token, uint256 tokenId, uint8 winnerLocation) public onlyOwner {
        ...
        require(_gameInfo.open, "The token is closed");
        require(_gameInfo.playerNumber == 2, "The player number error");
    ...
    function getTotalAmount(address token, uint256 tokenId, address[2] storage players) internal view returns (uint256) {
        uint256 amount;
    ...
    function getPlayers(address token, uint256 tokenId) public view returns (address[2] memory players) {
