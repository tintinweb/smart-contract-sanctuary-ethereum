...
contract RaregotchiToy is ERC721 {
    event Unveiling();
    event ToyOpen(uint16 indexed toyId, string familyName);
    ...
    bool public unveiled = false;
    bool private isFrozen = false;

    address public signerAddress;

    string private veiledUri = "";
    string private unveiledBaseUri = "";
    string private openBaseUri = "";
    string private packageBaseUri = "";

    mapping(uint8 => mapping(address => uint8)) private mintedPerWalletByStage;
    mapping(uint16 => uint16) private tokenMatrix;
    mapping(uint16 => string) public tokenFamilyName;
    mapping(uint256 => bool) public tokenOpen;
     ...
     */
    function setVeiledUri(string calldata _uri)
        external
        onlyOwner
        contractIsNotFrozen
    {
    ...
    function mint(
        ...
        if (totalSupply + amount > totalTokens) {
            amount = uint8(totalTokens - totalSupply);
        }

        require(
            totalSupply + amount <= totalTokens,
            "Mint amount exceeds the availability"
        );
        require(
            mintedPerWalletByStage[stage][msg.sender] + amount <= maxAmount,
        ...
        mintedPerWalletByStage[stage][msg.sender] += amount;
        totalSupply += amount;
    ...
    function open(
        ...
        require(block.timestamp >= fromTimestamp, "Too early to open");
        require(ownerOf(tokenId) == msg.sender, "Invalid Token ID");
        require(tokenOpen[tokenId] == false, "Token already open");

        tokenOpen[tokenId] = true;
        tokenFamilyName[tokenId] = familyName;

        emit ToyOpen(tokenId, familyName);

        petContract.mint(msg.sender, tokenId, petIds);
    }
        ...
        returns (string memory)
    {
        ...
        if (tokenOpen[tokenId]) {
            return
                string(
                    abi.encodePacked(openBaseUri, Strings.toString(tokenId))
                );
        } else if (unveiled) {
            return
                string(
                    abi.encodePacked(unveiledBaseUri, Strings.toString(tokenId))
                );
        } else {
            return veiledUri;
        }
        ...
        return
            keccak256(
                abi.encodePacked(
                    ...
                    Strings.toString(
                        54 + bytes(_familyName).length + _petIds.length * 32
                    ),
                    _address, // 20 bytes
