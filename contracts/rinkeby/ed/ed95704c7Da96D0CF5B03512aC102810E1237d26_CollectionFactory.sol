 ...
 * @notice Errors list
 */
error Artist_Already_Has_Collection();
error Incorrect_Signature();
error Incorrect_Buyer_Address();
error Zero_Uint_Error();
error Artist_Does_Not_Have_Collection();
 ...
 */
contract CollectionFactory is Withdrawable{
     ...
     * @notice Address of signer for sig verification
     */
    mapping(address => address) private s_artistToCollection;


    /**
     * @title Initial Details about an Collection
     
     * @property auctionID - Id of auction
   ...
   /**
     ...
     * @property name Collection symbol
     */
    ...
    constructor(address _s_payment_token_address){
       s_signer = msg.sender;
     ...
     * @param _collectionDetails Colleciton informatiom
     * @param _auctionDetails Auction information
     * @param _signature Signature
    */
    function createCollectionAndAcceptBid(
      ...
      bytes calldata _signature
        ...
        onlyWithAllowance(_auctionDetails.buyer, _auctionDetails.price);
         ...
         // Verifying signature
         bool isVerified = verifyParams(_auctionDetails, msg.sender, _signature);

         if(!isVerified){
            revert Incorrect_Signature();
         }

         if(s_artistToCollection[msg.sender] != address(0)){
            revert Artist_Already_Has_Collection();
         }
          // Creating new Colleciton
         Collection collection = new Collection(
            _collectionDetails.name,
            _collectionDetails.symbol,
            address(this)
         );

        s_artistToCollection[msg.sender] = address(collection);

        // Minting NFT on created collection
       
        collection.mint(_auctionDetails.auctionID, _auctionDetails.tokenURI, _auctionDetails.buyer);

        collection.transferOwnership(msg.sender);

          // Transfering  price tokens for NFT
        transferTokensWithFee(_auctionDetails.buyer, msg.sender, _auctionDetails.price);
        // IERC20(s_payment_token_address).transferFrom(_auctionDetails.buyer, msg.sender, _auctionDetails.price);
        ...
        emit CollectionCreated(address(collection), msg.sender, block.timestamp);

    }


    /**
     * @notice Mints NFT for collection 
     * @param _auctionDetails Id of voting user wants to vote
     * @param _signature Id of voting user wants to vote
    */
    function acceptBidForCollection(AuctionDetails memory _auctionDetails, bytes calldata _signature) public 
         beforeSigExpiration(_auctionDetails.signatureTimestampExpiration){
        ...
        address collectionAddress = s_artistToCollection[msg.sender];

        if(collectionAddress == address(0)){
          revert Artist_Does_Not_Have_Collection();
        }
         mintNFTForCollection(_auctionDetails, collectionAddress);  
    }

    /**
     * @notice Mints NFT for collection 
     * @param _auctionDetails Id of voting user wants to vote
     * @param _collectionAddress Id of voting user wants to vote
    */
    function mintNFTForCollection(AuctionDetails memory _auctionDetails, address _collectionAddress) internal {
        onlyWithAllowance(_auctionDetails.buyer, _auctionDetails.price);

        //  Perform only if collection already exists
        ICollection(_collectionAddress).mint(_auctionDetails.auctionID, _auctionDetails.tokenURI, _auctionDetails.buyer);
          // Transfering  price tokens for NFT
        transferTokensWithFee(_auctionDetails.buyer, msg.sender, _auctionDetails.price);
        // IERC20(s_payment_token_address).transferFrom(_auctionDetails.buyer, msg.sender, _auctionDetails.price);
     ...
     * @param _auctionBuyer address of auction buyer
     * @param _artist address of artist
     * @param _auctionPrice amount of weth 
    */
    function transferTokensWithFee(address _auctionBuyer, address _artist, uint256 _auctionPrice) internal {
        uint256 marketplaceShare = _auctionPrice * s_marketplace_percentage / FEE_DENOMINATOR;

        uint256 artistShare = _auctionPrice - marketplaceShare;

        // Transfering fee to marketplace
        IERC20(s_payment_token_address).transferFrom(_auctionBuyer, address(this), marketplaceShare);
        // Transfering  artist share
        IERC20(s_payment_token_address).transferFrom(_auctionBuyer, _artist, artistShare);
    }
     ...
     * @param _signature msg.sender
    */
    function verifyParams(AuctionDetails memory _auctionDetails, address _sender, bytes calldata _signature) internal view returns(bool) {
       ...
       string memory concatenatedParams = SignerVerification.concatParams(
         ...
         return SignerVerification.isMessageVerified(s_signer, bytes(_signature), concatenatedParams);     
    }
    ...
    */
    ...
    /**
     * @notice Returns artist's collection address
     * @param _artist auction Details
    */
    function getArtistCollectionAddress(address _artist) external view returns(address) {
        return s_artistToCollection[_artist];
    }
    ...
    */
    ...
    /**
        *@notice Emmited when artist creates collection
        *@param collectionAddress collection address
        *@param artistAddress address of an Artist
    ...
    */
    event CollectionCreated(
      ...
      /**
        ...
        *@param buyer address of owner
        *@param artist address of owner
...
contract Collection is ERC721A, Ownable{
   ...
   using Strings for uint256;
    ...
    /**
     * @notice Address of Collection Factory
     */
    address private s_collectionFactoryAddress;
    ...
    /**
     * @notice Allows to call only for Collection Factory
     */
    modifier onlyCollectionFactory(){
      if(msg.sender != s_collectionFactoryAddress){
         revert Not_Collection_Factory();
      }
      _;
    }

    constructor(string memory _name, string memory _symbol, address _s_collectionFactoryAddress)ERC721A(_name, _symbol){
        s_collectionFactoryAddress = _s_collectionFactoryAddress;
    }
    ...
    */
    function mint(uint256 _auctionId, string memory _tokenURI, address _buyer) external onlyCollectionFactory{
          if(s_auctionToStatus[_auctionId]){
             revert Auction_Completed();
          }
          s_auctionToStatus[_auctionId] = true;

          _mint(_buyer, 1);

          uint256 mintedTokenId = totalSupply() - 1;

          _setTokenURI(mintedTokenId, _tokenURI);
    ...
    */
    ...
    /**
     * @notice Returns collectionFactory address
    */
    function getCollectionFactoryAddress() external view returns(address) {
        return s_collectionFactoryAddress;
    }
