...
contract MarketTrades is ReentrancyGuard {
  ...
  function withdrawTrade(
    ...
    for (uint i; i<itemId.length; i++) {
      ...
      idToNftTrade[tradeId[i]] = Trade(false, 0, 0, 0, 0, address(0x0), payable(0x0), address(0x0));
  ...
  function refundTrade(uint itemId, uint tradeId) public nonReentrant hasContractAdmin returns(bool){
    ...
    idToNftTrade[tradeId] = Trade(false, 0, 0, 0, 0, address(0x0), payable(0x0), address(0x0));
