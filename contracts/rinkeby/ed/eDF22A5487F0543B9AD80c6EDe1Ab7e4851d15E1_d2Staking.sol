...
contract d2Staking is Ownable, IERC721Receiver {
    ...
    constructor(d2NFT _nft, d2Reward _token) {
        nft = _nft;
        token = _token;
    }
    ...
    function claim(uint256[] calldata tokenIds, bytes memory signature, uint256 timestamp, uint256[] calldata levels) external {
        _claim(msg.sender, tokenIds, false, signature, timestamp, levels);
    }

    function claimForAddress(address account, uint256[] calldata tokenIds, bytes memory signature, uint256 timestamp, uint256[] calldata levels) external {
        _claim(account, tokenIds, false, signature, timestamp, levels);
    }

    function unstake(uint256[] calldata tokenIds, bytes memory signature, uint256 timestamp, uint256[] calldata levels) external {
        _claim(msg.sender, tokenIds, true, signature, timestamp, levels);
    }

    function signatureWallet(address wallet, uint256[] memory _tokenIds, uint256[] memory _levels, uint256 _timestamp, bytes memory _signature) public view returns (address){
        return ECDSA.recover(keccak256(abi.encode(wallet, _tokenIds, _levels, _timestamp)), _signature);
    }

    function _claim(address account, uint256[] calldata tokenIds, bool _unstake, bytes memory _signature, uint256 _timestamp, uint256[] calldata levels) internal {
        uint256 tokenId;
        uint256 level;
        uint256 earned = 0;

        address signerOwner = signatureWallet(account, tokenIds, levels, _timestamp, _signature);
        require(signerOwner == owner(), "Not authorized to claim");

        require(tokenIds.length == levels.length, "Invalid data");
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            earned += _calculateReward(staked.timestamp, level);
            vault[tokenId] = Stake({
            owner: account,
            tokenId: uint24(tokenId),
            timestamp: uint48(block.timestamp)
    ...
    function _calculateReward(uint256 stakedAt, uint256 level) internal view returns (uint256) {
        return 100000 ether * (block.timestamp - stakedAt) * level / 1 days;
    }

    function earningInfo(uint256[] calldata tokenIds, uint256[] calldata levels) external view returns (uint256) {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            earned = earned + _calculateReward(stakedAt, level);
        }
    ...
    function onERC721Received(
        ...
        require(from == address(0x0), "Cannot send NFT directly");
...
contract d2NFT is ERC721Enumerable, Ownable {
    ...
    modifier canBuy {
        require(totalTokens() <= MAX_ELEMENTS, "SoldOut!");
        _;
    }
...
contract d2Reward is ERC20, ERC20Burnable, Ownable {

    mapping(address => bool) controllers;

    constructor() ERC20("d2Reward", "D2RW") { }
