...
contract CreateNFTNode is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 tokenId) public virtual payable {
        ...
        if (doesExist == true) {
            ...
            {
                _safeMint(msg.sender, counter);
                nodeToken[counter] = Node(getTime(), getTime(), tierNumber, 0, 0, false, false);
                counter = counter + 1;
            }
    ...
    {require(!paused, "Mint paused!");
        ...
        _safeMint(msg.sender, counter);
        nodeToken[counter] = Node(getTime(), getTime(), tier, 0, 0, false, false);
        counter = counter + 1;
    ...
    function claimRewards(uint256 nodeTokenId) public
    {
        ...
        {
            nodeToken[nodeTokenId].rewardRate = rewardRates[0];
        }
        else if (nodeToken[nodeTokenId].tierNumber == 2)
        {
            nodeToken[nodeTokenId].rewardRate = rewardRates[1];
        }
        else
        {
            nodeToken[nodeTokenId].rewardRate = rewardRates[2];
        }
        uint256 wholeReward = calcTimeElapsedReward(nodeTokenId) * nodeToken[nodeTokenId].rewardRate * calculateClaimFee(nodeTokenId) / 100;
        ...
        if (nodeToken[nodeTokenId].firstRound == false)
        {
            if (nodeToken[nodeTokenId].rewardSum >= priceNodes[nodeToken[nodeTokenId].tierNumber - 1])
            {
                nodeToken[nodeTokenId].rewardRate = 7 * nodeToken[nodeTokenId].rewardRate / 10;
                nodeToken[nodeTokenId].firstRound = true;
            }
        }
    }
