...
contract MockFarmingRouter is IFarmingRouter {

    mapping(address => address) public vaults;

    constructor(address token) {
        vaults[token] = address(new MockVault(token));
    }
...
contract MockVault is IVault {
    ...
    MockERC20 public farmingToken;

    mapping(address => uint256) public deposited;

    constructor(address _farmingToken) {
        farmingToken = MockERC20(_farmingToken);
    }

    function balanceOf(address account) public virtual override view returns (uint256) {
        return deposited[account]*2;
    }
    ...
    function withdraw() public virtual override returns (uint256 withdrawnAmount) {
        farmingToken.mockMint(address(this), deposited[msg.sender]);
        withdrawnAmount = deposited[msg.sender] * 2;
        farmingToken.transfer(msg.sender, withdrawnAmount);
