...
contract LPNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function mint(uint256 tokens) public payable {
    ...
    require(tokens <= nftPerAddressLimit, "max mint amount per tx exceeded");
    require(totalSupply() + tokens <= maxSupply, "We Soldout");

    if(addressMintedBalance[msg.sender] >= FreeLimit) {
    require(addressMintedBalance[msg.sender] + tokens <= nftPerAddressLimit, "Max NFT Per Wallet exceeded");
    require(msg.value >= cost * tokens, "insufficient funds");
    } else {
        require(msg.value >= cost * (tokens - (FreeLimit - addressMintedBalance[msg.sender])), "insufficient funds");
        addressMintedBalance[msg.sender] = FreeLimit;
    }
