...
/// @param claimPeriodStartBlock first block of given epochs claim period
/// @param currentBlockNumber current block number
error EarlyClaim(uint claimPeriodStartBlock, uint currentBlockNumber);
...
contract ManagedVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, IManagedVault {
    ...
    uint256 public performanceFeeClaimFirstBlock;
    uint256 public performanceFeeClaimBlockCooldown;
    uint256 public performanceFeeClaimBlockWindowLength;
    ...
    CountersUpgradeable.Counter public redemptionsCounter;
    uint256 public totalRedemptionAmount;
    IERC20 public redemptionToken;
    int256 public redemptionFeeBPS;
    int256 public redemptionMinimumAmount;
    uint256 public redemptionInitialTimestamp;
    uint256 public redemptionRegistrationWindowLength;
    uint256 public redemptionCooldownWindowLength;
    ...
    function totalSupply() public view virtual override returns (uint256) {
        if (accumulatedPerformanceFee > 0) {
            return super.totalSupply() + uint(accumulatedManagementFee) + uint(accumulatedPerformanceFee);
        } else {
            return super.totalSupply() + uint(accumulatedManagementFee);
        }
    }
    ...
    function setRedemptionFeeBPS(int256 _redemptionFeeBPS) external onlyOwner {
        redemptionFeeBPS = _redemptionFeeBPS;
    }

    function setRedemptionMinimumAmount(int256 _redemptionMinimumAmount) external onlyOwner {
        redemptionMinimumAmount = _redemptionMinimumAmount;
    }
    
    function setRedemptionInitialTimestamp(uint256 _redemptionInitialTimestamp) external onlyOwner {
        redemptionInitialTimestamp = _redemptionInitialTimestamp;
    }
    
    function setRedemptionRegistrationWindowLength(uint256 _redemptionRegistrationWindowLength) external onlyOwner {
        redemptionRegistrationWindowLength = _redemptionRegistrationWindowLength;
    }
    
    function setRedemptionCooldownWindowLength(uint256 _redemptionCooldownWindowLength) external onlyOwner {
        redemptionCooldownWindowLength = _redemptionCooldownWindowLength;
    }
    ...
    function setManagementFeeBPS(int256 _managementFeeBPS) external onlyOwner {
        require(_managementFeeBPS <= MAX_MANAGEMENT_FEE_BPS);
    ...
    function claimPerformanceFee(uint256 epoch, address claimAddress, uint256 claimAmount) external onlyOwner {
        uint256 claimPeriodFirstBlock = performanceFeeClaimFirstBlock + performanceFeeClaimBlockCooldown * epoch;
        if(claimPeriodFirstBlock > block.number) revert EarlyClaim(claimPeriodFirstBlock, block.number);
        if(block.number > claimPeriodFirstBlock + performanceFeeClaimBlockWindowLength) revert LateClaim();
    ...
    function registerRedemption(int256 amount) external returns (uint256 redemptionId) {
        require(redemptionMinimumAmount <= amount && uint256(amount) <= balanceOf(msg.sender) - locks[msg.sender]);
        uint256 feeAmount = uint256(amount) * uint256(redemptionFeeBPS) / uint256(HUNDRED_PERCENT_BPS);
        uint256 redemptionAmount = uint256(amount) - uint256(feeAmount);
        _burn(msg.sender, feeAmount);
        _lock(msg.sender, redemptionAmount);
        totalRedemptionAmount += redemptionAmount;
        redemptionId = redemptionsCounter.current();
        redemptions[redemptionId] = Redemption({
            timestamp: block.timestamp,
            account: msg.sender,
            amount: redemptionAmount
        });
        redemptionsCounter.increment();
    }

    function cancelRedemption(uint256 redemptionId) external {
        uint256 redemptionTimestamp = redemptions[redemptionId].timestamp;
        address redemptionAccount = redemptions[redemptionId].account;
        uint256 redemptionAmount = redemptions[redemptionId].amount;
        
        // Investors can cancel their redemptions anytime.
        // Anyone can cancel a redemption that has not been claimed in the next window.
        // That is for not letting investors have dangling redemptions
        require(redemptionAccount == msg.sender
                || redemptionTimestamp + redemptionRegistrationWindowLength + redemptionCooldownWindowLength <= block.timestamp);
        
        delete redemptions[redemptionId];
        totalRedemptionAmount -= redemptionAmount;
        _unlock(redemptionAccount, redemptionAmount);
    }

    function redeem(uint256 epoch, uint256 redemptionId) external {
        uint256 redemptionTimestamp = redemptions[redemptionId].timestamp;
        address redemptionAccount = redemptions[redemptionId].account;
        uint256 redemptionAmount = redemptions[redemptionId].amount;
        
        // We're in a redeem period
        require(0 < epoch);
        require(redemptionInitialTimestamp + epoch * (redemptionRegistrationWindowLength + redemptionCooldownWindowLength) <= block.timestamp);
        require(block.timestamp <= redemptionInitialTimestamp + epoch * (redemptionRegistrationWindowLength + redemptionCooldownWindowLength) + redemptionRegistrationWindowLength);
        
        // The transaction is created in a previous redeem period
        require(redemptionTimestamp <= block.timestamp - redemptionCooldownWindowLength);

        uint256 redemptionAmountInRedemptionToken = redemptionAmount * vaultPrice / tokenPrices[redemptionToken];

        require(reentrancyGuard == 0, "Reentrant call");
        reentrancyGuard = 1;
        bool success = redemptionToken.transferFrom(owner(), redemptionAccount, redemptionAmountInRedemptionToken);
        if (!success) {
            revert TransferFailed();
        }
    ...
    function _updateManagementFees() internal {
        if (0 < updateManagementFeeTimestamp && updateManagementFeeTimestamp < block.timestamp) {
