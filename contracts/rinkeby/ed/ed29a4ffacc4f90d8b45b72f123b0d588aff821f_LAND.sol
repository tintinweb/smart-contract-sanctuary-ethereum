...
contract VerifySign{
    ...
    function verifySig(uint256[] memory tokenId, uint256 _toalScore, uint8 v, bytes32 r, bytes32 s) public view returns (address) {
...
contract Token is ERC20 {
    constructor() ERC20("p_token", "p_token") {
...
contract NFT_W is ERC721A, Ownable, ReentrancyGuard {
  ...
  string public hiddenMetadataUri = "ipfs://QmUJWzMrmyj1bmLmhN4Jk1PWVZKdZWNhVdJ4c4T9R1XpmJ";
...
contract NFT_P is ERC721A, Ownable, ReentrancyGuard {
  ...
  string public uriPrefix = '';
  string public uriSuffix = '.json';
  string public hiddenMetadataUri = "ipfs://QmTooEePyynjoB5u2X516Bo8pDxdJ4zGUX1ZayDwV6PMUk";
...
// File: contracts/ERC1155_test.sol
...
contract LAND is AbstractERC1155Factory, ReentrancyGuard, VerifySign{
    
    //mapping (uint256 => uint256) public claimTokenId;
    ...
    uint256 public constant lv4 = 4;
    uint256 public constant lv5 = 5;

    string public contractURi = "https://gateway.pinata.cloud/ipfs/QmZn9Zee99g8VwXTAxJtdix7FXbVzMZBzYSyb6ugfbJ6jN";
    string public uriPrefix = "ipfs://QmQy7kNvCSryn2QEMRL52GUcekPKAx2B7xTMGViYF8HVD3/";
    string public uriSuffix = ".json";

    address public piplAddress = 0x7DE203500B48Eab74F07047B63aa51e7a3D05B7c;
    address public weareAddress = 0x21BF876Dfd9AE68080244F81dCBF693D669Fec56;
    address public tokenAddress = 0xA7D5682Bf8197080f454322E8015d1B316Dd9eC1;
    ...
    function claim(uint256[] calldata pipl_tokenId, uint256[] calldata weare_tokenId, uint256 _toalScore, uint8 v, bytes32 r, bytes32 s) public payable whenNotPaused{

        NFT_P pipl = NFT_P(piplAddress);
        NFT_W weare = NFT_W(weareAddress);
        Token token = Token(tokenAddress);

        //verify signature
        require(verifySig(pipl_tokenId, _toalScore, v, r, s) == signerAddress, "Not sign by signer");
        ...
        //require(weare.walletOfOwner(msg.sender).length >= 2, "You must hold at lesat 2 unclaimed WeAre");   

        for (uint i = 0; i < weare_tokenId.length; i++){
            //mark down claimed weare
            require(weare.ownerOf(weare_tokenId[i]) == msg.sender, "You must be owner of the WeAre"); 
            require(claimedWeareId[weare_tokenId[i]] == false, "Please use unclaimed WeAre for claim");                     
            claimedWeareId[weare_tokenId[i]] = true;
        }
        
        for (uint i = 0; i < pipl_tokenId.length; i++){
            //burn the token
            require(pipl.getApproved(pipl_tokenId[i]) == address(this) || pipl.isApprovedForAll(msg.sender, address(this)) == true, "Please approve the token for claim");                     
            pipl.safeTransferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD , pipl_tokenId[i]);
        }

        uint256 landId = 1;
        require(totalSupply(landId) + 1 <= maxSupply, "Max supply reached");         
        
        _mint(msg.sender, landId, 1, "");
        token.transfer(_msgSender(), claimTokenAmount * 10 ** token.decimals());
...
/*    function setClaimTokenId (uint256[][] calldata _tokenId) external onlyOwner {
        for (uint i = 0; i < _tokenId.length; i++) {
            claimTokenId[_tokenId[i][0]] = _tokenId[i][1];
        }
    }
*/
    function setContractURI(string memory _newContractURI) public onlyOwner {
