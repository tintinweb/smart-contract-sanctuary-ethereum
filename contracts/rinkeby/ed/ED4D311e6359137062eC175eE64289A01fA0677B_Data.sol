...
contract Data {
    ...
    function feedData(uint256[] memory IDs)
        public
        view
        returns (feedStruct[] memory feedinfo)
    {
        feedStruct[] memory fd;

        for (uint256 n; n < IDs.length; n++) {
            fd[n]._feeds = IDs[n];
            fd[n]._name = name[IDs[n]];
            fd[n]._apiendpoint = APIendpoints[IDs[n]];
            fd[n]._paths = path[IDs[n]];
            fd[n]._freq = freq[IDs[n]];
            fd[n]._dec = dec[IDs[n]];
            fd[n]._bounty = bountyETH[IDs[n]];
        }
        return (fd);
    }
