...
contract Tweettex is ERC20Burnable {
  ...
  constructor() ERC20("Tweettex Token", "TWTT") {
      _mint(msg.sender, INITIAL_LP_DEPLOYMENT); // mint 100 TIGER for initial pools deployment
  ...
  function distributeReward(address twttPool, address stakingPool) external {
      require(msg.sender == operator, "No operator.");
      require(!isDistributed, "only can distribute once");
      require(stakingPool != address(0), "!No zero address");
      require(twttPool != address(0), "!No zero address");

      isDistributed = true;

      // Lp staking pool allocation: 15%
      uint256 stakingPoolAllocation = MAX_TOTAL_SUPPLY * 15 / 100;
      uint256 twttPoolAllocation = MAX_TOTAL_SUPPLY - stakingPoolAllocation - INITIAL_LP_DEPLOYMENT;

      _mint(twttPool, twttPoolAllocation);
      _mint(stakingPool, stakingPoolAllocation);
  }
}