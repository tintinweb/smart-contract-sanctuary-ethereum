...
contract CaveGoblins is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public totalFreeForWL = 600;
    ...
    function WLMint(bytes32[] calldata _merkleProof, uint256 amt)
        external
        payable
    {
        require(wlEnabled, "Trippilist mint is disabled.");
        require(totalSupply() + amt < maxSupply + 1, "No more cave goblins");
        require(
            totalSupply() + amt < totalFreeForWL + 1,
            "No more cave goblins for WL"
        );

        require(amt < maxPerTxWL + 1, "Max per TX reached.");
        require(
            numberMinted(msg.sender) + amt <= maxPerTxWL,
            "Max per wallet reached."
        );
