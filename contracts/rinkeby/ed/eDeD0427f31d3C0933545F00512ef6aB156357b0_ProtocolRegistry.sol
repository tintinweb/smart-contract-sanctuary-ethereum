...
contract ProtocolRegistry is ProtocolBase  {
    ...
    modifier onlyAddTokenRole(address admin) {
        require(
            IAdminRegistry(govAdminRegistry).isAddTokenRole(admin) ==
                true,
    ...
    modifier onlyEditTokenRole(address admin) {
        require(
            IAdminRegistry(govAdminRegistry).isEditTokenRole(admin) ==
                true,
    ...
    modifier onlyAddSpRole(address admin) {
        require(
            IAdminRegistry(govAdminRegistry).isAddSpAccess(admin) ==
                true,
    ...
    modifier onlyEditSpRole(address admin) {
        require(
            IAdminRegistry(govAdminRegistry).isEditSpAccess(admin),
            "GovProtocolRegistry: No admin right to update or remove Strategic Partner"
        );
        _;
    }

    function initialize(address _govAdminRegistry) external initializer {
        __Ownable_init();
...
abstract contract ProtocolBase is
    OwnableUpgradeable,
    IProtocolRegistry, SuperAdminControl
{
