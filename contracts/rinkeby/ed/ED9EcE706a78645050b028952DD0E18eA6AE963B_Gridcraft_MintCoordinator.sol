...
contract Gridcraft_MintCoordinator is Ownable {
  ...
  address signer = 0xe506dbCA845412C04B6d4F7210D64098F58C5da1;
  ...
  uint256 identityPrice = 0.3 ether;
  uint256 landPrice = 0.3 ether;
  uint256 bundleSave = 0.04 ether;

  uint256[][] allowListMax = [[2,1], [1,1], [3,2]];
  ...
  function llamaverseSale(uint256[] memory _llamaVerseIds, bool _stake) external payable {
    ...
    if (userHasLlamaboost(msg.sender)){
  ...
  function setIdentityPrice(uint256 _newPrice) external onlyOwner {
    identityPrice = _newPrice;
  }

  function setLandPrice(uint256 _newPrice) external onlyOwner {
    landPrice = _newPrice;
  }

  function setBundlePrice(uint256 _newPrice) external onlyOwner {
    bundleSave = identityPrice + landPrice - _newPrice;
  }
