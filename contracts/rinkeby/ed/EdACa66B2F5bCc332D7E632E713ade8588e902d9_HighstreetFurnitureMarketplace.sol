...
/** @title The primary marketplace contract of Highstreet furniture */
contract HighstreetFurnitureMarketplace is Context, Ownable, ReentrancyGuard, Pausable, PriceConverter {
  ...
  /**
    * @notice Add new furniture item to the marketplace
    *
    * @dev this function will also set the maximum supply in HighstreetFurniture contract
  ...
  function addItem(uint256 id_, Item memory item_) external onlyOwner {
    ...
    saleList[id_] = item_;
    furniture.setMaxSupply(id_, uint256(item_.maxSupply));
    idList.push(id_);
    emit Add(id_, item_);
  }
  ...
  function _deal(uint256[] memory ids_, uint256[] memory amounts_, address receiver_) internal virtual {
    ...
    for (uint256 i = 0; i < ids_.length; ++i) {
      uint256 id = ids_[i];
      uint256 amount = amounts_[i];
      furniture.mint( receiver_, id, amount, "");
  ...
  /**
    ...
    *
    * @return An array contains the information of all furniture items
    */
...
/** @title The ERC1155 contract of Highstreet furniture */
