...
contract Staking is Ownable, Flowers {
    ...
    function activateNft(uint tokenId_) public onlySender {
    ...
    function claimPassiveStake(address user_, uint tokenId_) public onlySender {
        ...
        uint _totalRewards = (block.timestamp - _stake.since) * rewardPerHour / 2 hours;
    ...
    function getStake(address user_, uint stakeId_) public view returns (uint[] memory _stake) {
        Staker memory staker = stakers[user_];
        uint[] memory arrResult = new uint[](2);
        arrResult[0] = staker.tokenIds[stakeId_];
        arrResult[1] = staker.since[stakeId_];
        return arrResult;
    }
