...
contract YieldToken is ERC20("EtherIron", "EIT"), ERC20Burnable, Ownable {
    ...
    mapping(address => uint256) public rewardsDOUBLE;
    ...
    function getReward(address _to) external {
        require(msg.sender == address(blasterContract));
    ...
    function burn(address _from, uint256 _amount) external {
        require(
            msg.sender == address(blasterContract) ||
                msg.sender == address(ceoContract) ||
                msg.sender == address(vipContract)
        );
        _burn(_from, _amount);
    }
...
contract Blaster is ERC721, Ownable {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return
            "https://gateway.pinata.cloud/ipfs/QmVioobyVMbWsSvqeLdQP8HHzxsu4uomtyQCELRceNJtsa/";
    }
    ...
    // Minting with invite, inviter collecting invite points to swap them for YieldToken
...
contract CEO is ERC721, Ownable {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return
            "https://gateway.pinata.cloud/ipfs/QmQRtg4iQfkF9rA4gkHNuuTY1jnm8UG75knpVFatN5g1Zu/";
    }
...
contract FurnaceVIP is ERC721, Ownable {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return
            "https://gateway.pinata.cloud/ipfs/QmcmKhY8psmN35vsPeak5VoedLKHKScA8GnYStkPB1WcxG/";
    }
...
contract FurnaceHBI is ERC20, ERC20Burnable, Ownable {
    ...
    function withdraw() public onlyOwner {
        uint256 balance = address(this).balance;
        payable(msg.sender).transfer((balance * 60) / 100);
        payable(vipContract).transfer((balance * 40) / 100);
    }
    ...
    function buyHBI(uint256 amount) public payable {
        require(msg.value >= amount * HBI_PRICE);
