    ...
    OwnableUpgradeable
{
    ...
    */
     ...
     * @dev Only Owner mint token
       Do not check Max ToTAL SUPPLY. Should Put this into Minter wallet
     */
        ...
        returns (string memory)
    {
       string memory bowlImage = bowls[_tokenId] == true ? bowlIPFS : bowlEmptyIPFS;
        string memory json = Base64Upgradeable.encode(
            bytes(
                string(abi.encodePacked("{\"name\": \"Mystery Bowl #", StringsUpgradeable.toString(_tokenId), "\", \"image\":\"", bowlImage, "\", \"attributes\": [{\"trait_type\":  \"Summonable\",\"value\":\"", bowls[_tokenId] == true ? "true" : "false","\"}, {\"trait_type\":  \"Starfish\",\"value\":\"", StringsUpgradeable.toString(starfish(_tokenId)),"\"}]""}"))
                )
        );
