...
//Need to add Start and End function
...
contract BabyPool {
    ...
    function Guess(string memory Your_Name, int Weight_in_grams, int Height_in_cm, string memory Hair_Color_brown_blonde_red, string memory Eye_Color_brown_blue_grey, string memory Gender_male_female, int Birth_Year_UTC, int Birth_Month_UTC, int Birth_Day_UTC) payable public returns (bool success) {
      ...
      return string(abi.encodePacked("Price to enter is: ", Strings.toString(GuessPrice), " Wei"));
    }

    //Admin Functions

      function setGuessPrice(uint _valueInWei) public {
    ...
    function Actual_Details_admin_only(int Weight_in_grams, int Height_in_cm, string memory Hair_Color_brown_blonde_red, string memory Eye_Color_brown_blue_grey, string memory Gender_male_female, int Birth_Year_UTC, int Birth_Month_UTC, int Birth_Day_UTC) public returns (bool success) {
    ...
    function Calculate_Winner() payable public {
     ...
     else {require(winner_address!=0x0000000000000000000000000000000000000000, "Winner address is 0x0000000000000000000000000000000000000000, which is invalid");}
    }

    function deletePlayers() public returns(bool success) {
      require(msg.sender == owner, "You are not the Admin!");
      delete players;
      playerCount=0;
    ...
    function Calculate_Score(int Weight_in_grams, int Height_in_cm, string memory Hair_Color_brown_blonde_red, string memory Eye_Color_brown_blue_grey, string memory Gender_male_female, int Date_of_birth) private view returns (int) {
      int score_weight = abs(Weight_in_grams - admin[0].weight_in_grams)/100*10;
      int score_height = abs(Height_in_cm - admin[0].height_in_cm)*20;
      int score_hair_color = keccak256(abi.encodePacked(_toLower(Hair_Color_brown_blonde_red))) != keccak256(abi.encodePacked(_toLower(admin[0].hairColor))) ? int(100) : int(0);
      int score_eye_color = keccak256(abi.encodePacked(_toLower(Eye_Color_brown_blue_grey))) != keccak256(abi.encodePacked(_toLower(admin[0].eyeColor))) ? int(100) : int(0);
      int score_gender = keccak256(abi.encodePacked(_toLower(Gender_male_female))) != keccak256(abi.encodePacked(_toLower(admin[0].gender))) ? int(200) : int(0);
      int score_date = abs(Date_of_birth - admin[0].dateUnix)/86400*20;
