...
contract NFTFactory is Ownable, INFTFactory {
    ...
    constructor(
        ...
        nftImplementation.initialize(msg.sender, "Template", "Template");
    ...
    function deploy(
        ...
        require(userNftContractsInternal[ NFT(token).owner() ].add(token), 'NFTFactory: BROKEN_STRUCTURE');
    ...
    function trackTokenTransfer(address _owner, address _from, address _to, uint _tokenId) external override {
        // initially deployed nft contract is adding to owner, so, if msg.sender is exist in some list then msg.sender is deployed nft contract
        require(userNftContractsInternal[_owner].contains(msg.sender), 'NFTFactory: AUTH_FAILED');
...
interface INFTFactory {
    ...
    event NFTTransferred(address nftContract, address indexed from, address indexed to, uint tokenId);
...
contract NFT is OwnableUpgradeable, ERC721Upgradeable, INFT {
    ...
    uint16 public totalMintedSharesBasisPoints;

    constructor(INFTFactory _nftFactory) {
        nftFactory = _nftFactory;
    }

    function initialize(address _owner, string memory _name, string memory _symbol) initializer public {
        require(_owner != address(0), "NFT: ZERO_ADDRESS");
        ...
        public onlyOwner returns (uint[] memory mintedIds)
    {
        ...
        require(newRoundMintedSharesBP <= round.maxRoundSharesBasisPoints, "NFT: ROUND_SHARES_MORE_THAN_100_PERCENT");
    ...
    function getTokensInfoExtended(uint256[] memory _tokensIds) public view override returns (
        ...
        _owners = new address[](_tokensIds.length);

        for (uint i=0; i<_tokensIds.length; i++) {
            // for nonexisting tokens zero structs would be returned. For check
            ...
            _owners[i] = _tokensInfo[i].shareBasisPoints > 0 ? ownerOf(_tokensIds[i]) : address(0);
        }
    }
