...
interface IReferrerStorage {
    function referrers(address referee) external view returns (address referrer);
    function referrerRoyalty(address referrer) external view returns (uint256 royalty);
}
...
contract Marketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    event UpdateReferrerStorage(address referrerStorage);
...
//    bytes4 private constant _ERC721_RECEIVED = 0x150b7a02;
    ...
    /// @notice Platform fee receipient
    address payable public voteLockingAddress;

    /// @notice Token registry
    ITokenRegistry public tokenRegistryAddress;

    /// @notice Referrer storage
    IReferrerStorage public referrerStorageAddress;
    ...
    function initialize(
        ...
        address payable _voteLockingAddress, 
        uint24 _platformFee
    ) public initializer {
        platformFee = _platformFee;
        treasuryAddress = _treasuryAddress;
        voteLockingAddress = _voteLockingAddress;
    ...
    function acceptOffer(
        ...
        address referrer = referrerStorageAddress.referrers(_msgSender());
        if( referrer != address(0)){
            referrerFee = (feeAmount.mul(uint256(referrerStorageAddress.referrerRoyalty(referrer)))).div(100000);            
            IERC20(offer.payToken).safeTransferFrom(
                _creator,
                referrer,
                referrerFee
            );
        ...
        offer.payToken.safeTransferFrom(
            _creator,
            treasuryAddress,
            (feeAmount.sub(referrerFee)).div(2)
        );
        offer.payToken.safeTransferFrom(
            _creator,
            voteLockingAddress,
    ...
    /**
     @notice Update ReferrerStorage contract
     @dev Only admin
     */
    function updateReferrerStorage (address _referrerStorageAddress) external onlyOwner {
        referrerStorageAddress = IReferrerStorage(_referrerStorageAddress);
        emit UpdateReferrerStorage(_referrerStorageAddress);
    }
...
*/
    ...
    function _buyItem(Listing memory _listing) private {
        ...
        uint256 feeAmount = price.mul(platformFee).div(100000);
        uint256 referrerFee = 0;
        address referrer = referrerStorageAddress.referrers(_listing.owner);
        if( referrer != address(0)){
            referrerFee = (feeAmount.mul(uint256(referrerStorageAddress.referrerRoyalty(referrer)))).div(100000);            
            IERC20(_listing.payToken).safeTransferFrom(
                _msgSender(),
                referrer,
                referrerFee
            );
...
contract ReferrerStorage is OwnableUpgradeable {

    event ReferrerRoyaltySet(address indexed referrer, uint256 referrerRoyalty);

    event ReferrerSet(address indexed referrer, address referree);

    /// @notice Referee (who was invited by a referrer) -> Referrer (who gets referral comission)
    mapping(address => address) public referrers;

    /// @notice ReferrerAddress -> Royalty (will be saved in the % * 1000 form, e.g. 2.755% = 2755)
    mapping(address => uint256) public referrerRoyalty;
    ...
    function setReferrer(address _referrer, address _referree) external onlyOwner {
        require(_referree != address(0), "invalid address");
        referrers[_referree] = _referrer;
        emit ReferrerSet(_referrer, _referree);
    }

    /// @notice Method for setting royalty for referrer
    /// @param _referrer Referrer address
    /// @param _royaltyPercMulByThousand Royalty !!! (MUST be saved in the % * 1000 form, e.g. 2.755% = 2755)
    function setReferrerRoyalty(address _referrer, uint256 _royaltyPercMulByThousand) external onlyOwner {
        require(_royaltyPercMulByThousand <= 100000, "invalid royalty");
        referrerRoyalty[_referrer] = _royaltyPercMulByThousand;
        emit ReferrerRoyaltySet(_referrer, _royaltyPercMulByThousand);
    }
}
...
contract NFTTradablePrivate is ERC721, Ownable {
    ...
    */
    ...
    function isApprovedForAll(address owner, address operator)
        ...
        returns (bool)
    {
        // Whitelist auction, marketplace contract for easy trading.
        if (
            auction == operator ||
...
contract ArtFactory is Ownable {
    ...
    */
    ...
    /// @param _name Name of ERC1155 contract
    /// @param _symbol Symbol of ERC1155 contract
    function createNFTContract(string memory _name, string memory _symbol)
        external
        payable
        returns (address)
    ...
    /// @param  tokenContractAddress Address of NFT contract
    function registerTokenContract(address tokenContractAddress) external onlyOwner { //!!! Why needed
    ...
    /// @param  tokenContractAddress Address of NFT contract
    function disableTokenContract(address tokenContractAddress) external onlyOwner { //!!! Why needed
