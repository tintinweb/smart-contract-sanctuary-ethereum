...
contract PoSFaucetStaking is Ownable {
  ...
  event StakeDeposit(address indexed staker, uint eventIdx, uint time, uint balance, uint locktime, int price);
  event StakeWithdraw(address indexed staker, uint eventIdx, uint time, uint balance);
  ...
  function depositAndLock(uint lockTime) private {
    ...
    emit StakeDeposit(msg.sender, eventIdx, block.timestamp, stakeAmount, lockTime, price);
  }
  ...
  function withdraw() public {
    ...
    uint eventIdx = _faucetEventIdx++;
    emit StakeWithdraw(msg.sender, eventIdx, block.timestamp, balance);
  }
