...
contract StakeNFT is ERC1155Receiver {
    ...
    function stake(uint256 _tokenId, uint256 _amount) public {
        ...
        if (staker.stakedTokens[_tokenId].amount > 0) {
            ...
            staker.stakedTokens[_tokenId] = TokenInfo(
                _tokenId,
                staker.stakedTokens[_tokenId].amount + _amount,
                block.timestamp
            );
        } else {
            staker.stakedTokens[_tokenId] = TokenInfo(
    ...
    function unstake(uint256 _tokenId, uint256 _amount) public {
        ...
        if (staker.stakedTokens[_tokenId].amount >= _amount) {
            ...
            parentNFT.safeTransferFrom(
                ...
                msg.sender,
                _tokenId,
                staker.stakedTokens[_tokenId].amount,
                "0x00"
            );
            delete staker.stakedTokens[_tokenId];
        }
    ...
    function getStakeValue(address _owner, uint256 _tokenId)
        public
        view
        returns (string memory)
    {
