...
contract BLT is ERC721, ERC721Enumerable, Pausable, ReentrancyGuard, AccessControl {
    ...
    address public treasuryAddress = 0x4ecF44f5439C4C29C2FA3a3c65579fA44279De98;
    ...
    function mintPreSale(address pfpContract, uint16 pfpTokenId, uint8 biltVersion) external payable whenNotPaused nonReentrant {
        ...
        require(ERC721(pfpContract).ownerOf(pfpTokenId) == msg.sender, "Must own the PFP token");
    ...
    function mintPublicSale(address pfpContract, uint16 pfpTokenId, uint8 biltVersion) external payable whenNotPaused nonReentrant {
    ...
    function mintPassSale(address passContract, uint16 passTokenId, address pfpContract, uint16 pfpTokenId, uint8 biltVersion) external payable whenNotPaused nonReentrant {
        ...
        //require(howManyUsesPass[passTokenId] < (perPass - 1), "Pass must be burned to mint again.");
    ...
    function mintPassBurn(address passContract, uint16 passTokenId, address pfpContract, uint16 pfpTokenId, uint8 biltVersion) external payable whenNotPaused nonReentrant {
        require(passSaleIsActive, "Pass Sale not active");
        require(totalSupply() < totalTokens, "Sold out");
        require(passContractList[passContract] == true, "Must be an eligible pass");
