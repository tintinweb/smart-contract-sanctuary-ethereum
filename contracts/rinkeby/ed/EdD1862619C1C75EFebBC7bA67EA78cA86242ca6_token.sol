...
contract token is ERC721 {
  ...
  mapping(bytes32 => bytes32) verifyInfo2;
  ...
  event verifyitem(bytes32 productCodeEmit);
    ...
    returns (uint256)
  {
    ...
    // itemInfo2[tokenID].product_code=productHash;
    emit lognewItem(productHash, 'Item created by supplier', block.timestamp);
    return (uint256(itemInfo[productHash_int].itemState));
  }

  function verify2Info(bytes32 tokenID) public view returns (bytes32) {
    // item2Info[tokenID]= productHash;
    // txnHash = verifyInfo2[tokenID];
    // emit verifyitem(verifyInfo2[tokenID]);
    return (verifyInfo2[tokenID]);
  }
  ...
  function purchasedByManufacturer(
    ...
    verifyInfo2[productHash] = txnhash;
