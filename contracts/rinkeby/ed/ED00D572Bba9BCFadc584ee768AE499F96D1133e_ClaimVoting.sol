...
contract ClaimVoting is IClaimVoting, Initializable, AbstractDependant {
    ...
    mapping(uint256 => VotesUpdatesInfo) internal _voteResults;

    event Voted(uint256 claimIndex, address voter, uint256 suggestedClaimAmount);
    ...
    function _isVotePending(uint256 index) internal view returns (bool) {
        uint256 claimIndex = _allVotesByIndexInst[index].claimIndex;

        return (_allVotesByIndexInst[index].status == VoteStatus.VOTED_PENDING_CALCULATION &&
    ...
    function _isVoteAwaitingReception(uint256 index) internal view returns (bool) {
        ...
        return ((_allVotesByIndexInst[index].status == VoteStatus.MAJORITY ||
            _allVotesByIndexInst[index].status == VoteStatus.MINORITY) &&
            !claimingRegistry.isClaimPending(claimIndex));
    }
    ...
    function canUnstake(address user) external view override returns (bool) {
        ...
        for (uint256 i = 0; i < voteLength; i++) {
            if (
                _isVotePending(_myVotes[user].at(i)) ||
                _isVoteAwaitingCalculation(_myVotes[user].at(i)) ||
                _isVoteAwaitingReception(_myVotes[user].at(i))
            ) {
                return false;
            }
        }
        ...
        returns (uint256 _claimsCount, PublicClaimInfo[] memory _votablesInfo)
    {
        ...
        for (uint256 i = offset; i < to; i++) {
            ...
            if (
                ...
                claimingRegistry.isClaimVotable(index) &&
        ...
        returns (MyVoteInfo[] memory _myVotesInfo, VotesUpdatesInfo[] memory _myVotesUpdatesInfo)
    {
        ...
        _myVotesUpdatesInfo = new VotesUpdatesInfo[](to - offset);
        ...
        for (uint256 i = offset; i < to; i++) {
            ...
            if (
                claimInfo.dateSubmitted.add(claimingRegistry.votingDuration(claimIndex)) >
                block.timestamp
            ) {
            ...
            // _myVotesUpdatesInfo[i - offset]
            if (_myNotReceivedVotes[msg.sender].contains(claimIndex)) {
                _myVotesUpdatesInfo[i - offset].calculated = _voteResults[voteIndex].calculated;

                if (!_isVoteAwaitingReception(voteIndex)) {
                    if (
                        _votings[claimIndex].votedYesPercentage >= PERCENTAGE_50 &&
                        _allVotesByIndexInst[voteIndex].suggestedAmount > 0
                    ) {
                        (uint256 stblAmount, uint256 bmiAmount, ) =
                            _calculateMajorityYesVote(claimIndex, msg.sender, oldReputation);

                        _myVotesUpdatesInfo[i - offset].bmiReward = bmiAmount;
                        _myVotesUpdatesInfo[i - offset].stblReward = stblAmount;
                    } else if (
                        _votings[claimIndex].votedYesPercentage < PERCENTAGE_50 &&
                        _allVotesByIndexInst[voteIndex].suggestedAmount == 0
                    ) {
                        (uint256 bmiAmount, ) =
                            _calculateMajorityNoVote(claimIndex, msg.sender, oldReputation);

                        _myVotesUpdatesInfo[i - offset].bmiReward = bmiAmount;
                    } else {
                        (uint256 bmiPenaltyAmount, ) =
                            _calculateMinorityVote(claimIndex, msg.sender, oldReputation);

                        _myVotesUpdatesInfo[i - offset].stakeChange = int256(bmiPenaltyAmount);
                    }
                } else if (_isVoteAwaitingReception(voteIndex)) {
                    _myVotesUpdatesInfo[i - offset].bmiReward = _voteResults[voteIndex].bmiReward;
                    _myVotesUpdatesInfo[i - offset].stblReward = _voteResults[voteIndex]
                        .stblReward;
                    _myVotesUpdatesInfo[i - offset].reputationChange = int256(
                        uint256(_voteResults[voteIndex].reputationChange).sub(oldReputation)
                    );
                    _myVotesUpdatesInfo[i - offset].stakeChange = _voteResults[voteIndex]
                        .stakeChange;
                }
            }
        }
    ...
    function _addVote(
        ...
        _allVotesByIndexInst[_voteIndex].suggestedAmount = suggestedClaimAmount;
        _allVotesByIndexInst[_voteIndex].stakedBMIAmount = stakedBMI;
        _allVotesByIndexInst[_voteIndex].accept = voteFor;
        _allVotesByIndexInst[_voteIndex].status = VoteStatus.VOTED_PENDING_CALCULATION;
    ...
    function voteBatch(
        ...
        uint256 stkBMIAmount
    ) external override {
        require(canVote(msg.sender), "CV: There are reception awaiting votes");
        require(claimIndexes.length == suggestedClaimAmounts.length, "CV: Length mismatches");

        if (stkBMIAmount > 0) {
            stkBMIStaking.lockStkBMIFor(msg.sender, stkBMIAmount);
        }
        uint256 stakedBMI = stkBMIStaking.stakedStkBMI(msg.sender);
        ...
        for (uint256 i = 0; i < claimIndexes.length; i++) {
            ...
            require(claimingRegistry.isClaimVotable(claimIndex), "CV: voting is over");
            require(
            ...
            require(
                _votings[claimIndex].withdrawalAmount >= suggestedClaimAmount,
            ...
            _addVote(msg.sender, claimIndex, suggestedClaimAmount, stakedBMI, voteFor);

            emit Voted(claimIndex, msg.sender, suggestedClaimAmount);
        }
    }
    ...
    function _calculateVoteResult(uint256 claimIndex) internal {
        for (uint256 i = 0; i < _votings[claimIndex].voteIndexes.length(); i++) {
            ...
            if (
                ...
                _voteResults[voteIndex].stblReward = stblAmount;
                ...
                _voteResults[voteIndex].stakeChange = int256(bmiPenaltyAmount);

                status = VoteStatus.MINORITY;
            }

            _allVotesByIndexInst[voteIndex].status = status;
            _voteResults[voteIndex].reputationChange = int256(newReputation);
            _voteResults[voteIndex].bmiReward = bmiAmount;
            _voteResults[voteIndex].calculated = true;
    ...
    function calculateResult(uint256 claimIndex) external override {
        ...
        if (claimingRegistry.claimStatus(claimIndex) == IClaimingRegistry.ClaimStatus.EXPIRED) {
            _resolveClaim(claimer, claimIndex, true);
        } else {
            ...
            _resolveClaim(claimer, claimIndex, false);
    ...
    function _resolveClaim(
        ...
        if (!isExpired) {
            ...
            uint256 allVotedStakedBMI = _votings[claimIndex].allVotedStakedBMIAmount;

            // if no votes or not an appeal and voted < 10% supply of staked StkBMI
            if (
                allVotedStakedBMI == 0 ||
                ((totalStakedStkBMI == 0 ||
                    totalStakedStkBMI.mul(QUORUM).div(PERCENTAGE_100) > allVotedStakedBMI) &&
                    !claimingRegistry.isClaimAppeal(claimIndex))
            ) {
    ...
    function receiveResult() external override {
        ...
        for (uint256 i = 0; i < notReceivedLength; i++) {
            ...
            if (_isVoteAwaitingReception(voteIndex)) {
                stblAmount = stblAmount.add(_voteResults[voteIndex].stblReward);
                bmiAmount = bmiAmount.add(_voteResults[voteIndex].bmiReward);
                bmiPenaltyAmount += _voteResults[voteIndex].stakeChange;
                if (uint256(_voteResults[voteIndex].reputationChange) > oldReputation) {
                    newReputation = newReputation.add(
                        uint256(_voteResults[voteIndex].reputationChange).sub(oldReputation)
                    );
                } else if (uint256(_voteResults[voteIndex].reputationChange) < oldReputation) {
                    newReputation = newReputation.sub(
                        oldReputation.sub(uint256(_voteResults[voteIndex].reputationChange))
                    );
                }
            }
        }
    ...
    function withdrawLockedBMI(uint256 claimIndex) public {
        address claimer = claimingRegistry.claimOwner(claimIndex);

        require(
            claimingRegistry.claimStatus(claimIndex) == IClaimingRegistry.ClaimStatus.EXPIRED,
            "CV: Claim is not expired"
        );
        require(claimer == msg.sender, "CV: Not the claimer");
        bmiToken.transfer(claimer, _votings[claimIndex].lockedBMIAmount);
    }

    /// @dev return maj and min with 10**5 precision, to get % divide by 10**3
    function getRepartition(uint256 claimIndex) external view returns (uint256 maj, uint256 min) {
        uint256 voteCount = _votings[claimIndex].voteIndexes.length();

        if (voteCount != 0) {
            for (uint256 i = 0; i < voteCount; i++) {
                uint256 voteIndex = _votings[claimIndex].voteIndexes.at(i);
                if (voteStatus(voteIndex) == VoteStatus.MAJORITY) {
                    maj = maj.add(1);
                } else if (voteStatus(voteIndex) == VoteStatus.MINORITY) {
                    min = min.add(1);
                }
            }
...
interface IClaimVoting {
    enum VoteStatus {VOTED_PENDING_CALCULATION, MINORITY, MAJORITY, RECEIVED}
    ...
    /// @param stkBMIAmount staked BMI amount added during vote
    function voteBatch(
        uint256[] calldata claimIndexes,
        uint256[] calldata suggestedClaimAmounts,
        uint256 stkBMIAmount
