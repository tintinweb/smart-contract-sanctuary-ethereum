...
contract MetaSaltAuth is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    // struct typeRequestInfo {
    //     uint256 requesterId;
    //     address requester;
    //     uint256 price;           
    // }

    // mapping (uint256 => typeRequestInfo) private Requests;
    IERC20Upgradeable private token;
    uint256 private price;

    //event EventCreateRequest(uint256 requesterId, address requester, uint256 price);
    ...
    // function CreateRequest(uint256 requesterId) external nonReentrant {
    //     uint256 balance = token.balanceOf(msg.sender);
    //     require(balance >= price, "Not Enough Balance");
    //     token.transferFrom(msg.sender, address(this), price);

    //     Requests[requesterId].requesterId = requesterId;
    //     Requests[requesterId].requester = msg.sender;
    //     Requests[requesterId].price = price;        

    //     emit EventCreateRequest(requesterId, msg.sender, price);
    // }

    // function ReadRequest(uint256 requesterId) external view returns (typeRequestInfo memory) {
    //     return Requests[requesterId];
    // }
