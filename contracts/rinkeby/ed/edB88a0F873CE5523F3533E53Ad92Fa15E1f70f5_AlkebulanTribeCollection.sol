...
contract AlkebulanTribeCollection is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  modifier callerIsUser() {
        require(tx.origin == msg.sender, "AlkebulanTribeCollection.io :: Cannot be called by a contract");
        _;
  }
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) callerIsUser{
    ...
    require(whitelistClaimed[_msgSender()] + _mintAmount <= 5, 'Max number of Tokens allowed Exceeded. Kindly reduce the amount of token to be acquired!');
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) callerIsUser{
    ...
    require(publicClaimed[_msgSender()] + _mintAmount <= 5, 'Max number of Tokens allowed Exceeded. Kindly reduce the amount of tokens to be acquired!');

    publicClaimed[_msgSender()] += _mintAmount;
    _safeMint(_msgSender(), _mintAmount);
  }
