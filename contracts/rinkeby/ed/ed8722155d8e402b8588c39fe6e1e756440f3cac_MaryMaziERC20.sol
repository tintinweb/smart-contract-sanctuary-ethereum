...
// File: contracts/MaryMaziERC20.sol


pragma solidity ^0.8.0;


contract MaryMaziERC20 is IERC20 {
    uint8 public decimals = 18;
    string public symbol = "MMT";
    string public name = "MaryMazi";
    address public contractOwner;

    uint MMTTokenPerEther = 1000;

    uint supply = 1000000 * (10 ** decimals);
    uint public issuedToken = 0;
    ...
    constructor() {
        ...
        balanceList[contractOwner] = supply;
    }
    ...
    function deposit(address tokenReceiver) public payable returns(uint) {
        uint256 numTokens = msg.value/10^18 * MMTTokenPerEther;
        require(issuedToken + numTokens <= totalSupply(), "Insufficient available token to issue");

        // TODO: Clarify which account should receive transferred ether,
        // contract address (address(this)) or contractOwner address

        (bool sent, ) = address(this).call{ value: msg.value }("");
        // (bool sent, ) = contractOwner.call{ value: msg.value }("");
        require(sent, "Failed to send Ether");

        transfer(tokenReceiver, numTokens);

        issuedToken += numTokens;
        return issuedToken;
    }
}