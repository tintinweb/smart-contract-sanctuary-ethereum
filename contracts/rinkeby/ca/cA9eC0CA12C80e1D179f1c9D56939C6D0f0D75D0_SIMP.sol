...
contract SIMP is ERC721A, Ownable {
    ...
    uint256 public constant MAX_FREEMINT_AMOUNT = 20;
    uint256 public constant MAX_SUPPLY = 3333;

    uint256 public constant PRICE_ALLOWLIST_SALE = 0 ether; // xx Available
    uint256 public constant PRICE_PUBLIC_SALE = 0.01 ether; // xx Available
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(
            numberMinted(msg.sender) + quantity <= MAX_PUBLIC_MINT_PER_ADDR,
        ...
        refundIfOver(currentPrice() * quantity);
    ...
    function allowlistMint(uint256 quantity) external payable {
        require(
            ...
            "SIMP: This address is not allowed to mint."
        );
        require(
            status == Status.Started || status == Status.AllowListOnly,
            "SIMP: Allowlist mint is not active."
        );
        require(tx.origin == msg.sender, "SIMP: ?? Not you!");
        require(
        ...
        _safeMint(msg.sender, quantity);
        refundIfOver(PRICE_ALLOWLIST_SALE * quantity);
    ...
    function currentPrice() public view returns (uint256) {
        if (totalSupply() < MAX_FREEMINT_AMOUNT) {
            return PRICE_ALLOWLIST_SALE;
        } else {
    ...
    function seedAllowlist(
        ...
        require(addresses.length == numSlots.length, "SIMP: Wrong address.");
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "SIMP: No enough ETH.");
        if (msg.value > price) {
