...
**/
contract Birthday is IBirthday, ERC721, ERC721Pausable, ERC721URIStorage, PullPayment, Ownable {
    ...
    bytes internal constant zer = '..0000...00..00..00..00..00..00...0000..';
    bytes internal constant one = '.1111......11......11......11....111111.';
    bytes internal constant two = '..2222...22..22.....22.....22....222222.';
    bytes internal constant thr = '..3333...33..33.....333..33..33...3333..';
    bytes internal constant fou = '.44..44..44..44..444444......44......44.';
    bytes internal constant fiv = '.555555..55......55555.......55..55555..';
    bytes internal constant six = '..6666...66......66666...66..66...6666..';
    bytes internal constant sev = '.777777.....77.....77.....77.....77.....';
    bytes internal constant eig = '..8888...88..88...8888...88..88...8888..';
    bytes internal constant nin = '..9999...99..99...99999......99...9999..';
    bytes internal constant sls = '............/....../....../.............';

    bytes[] internal numbers = [zer, one, two, thr, fou, fiv, six, sev, eig, nin, sls];

    uint internal constant prefixLength = 33;
    uint internal constant rows = 26;
    uint internal constant columns = 50;
        ...
        string memory json = Base64.encode(
            bytes(string(
                abi.encodePacked(
                    ...
                    '"external_link": "https://nftdob.com", "seller_fee_basis_points": 250, "fee_recipient": "0x183d0b362Ef14b6CE3cF8b3209E76dF6A32797a5"}'
                )
            ))
        );
    ...
    */
    ...
    function draw(uint id, bool mdy) public view returns (string memory) {
        ...
        bytes memory output = new bytes((((columns + 3) * rows) - 3) + 33);
        uint c;
        ...
        uint[4] memory yearNums = [(year / 1000), (year / 100) % 10, (year / 10) % 10, year % 10];
        uint[5] memory dmNums = mdy ? [(month / 10), (month % 10), 10, (day / 10), (day % 10)] : [(day / 10), (day % 10), 10, (month / 10), (month % 10)];
        
        for(uint i = 0; i < rows; i++) {
            for(uint co = 0; co < columns; co++) {
                output[c] = 0x2E;
                if (i >= 16 && i < 21) {
                    uint k = i - 16;
                    if (co > 8 && co < 39) {
                        uint j = co - 8;
                        uint yp = (j / 8);
                        output[c] = numbers[yearNums[yp]][k * 8 + j % 8];
                    }
                } else if (i >= 5 && i < 10) {
                    uint k = i - 5;
                    if (co > 5 && co < 44) {
                        uint j = co - 5;
                        uint yp = (j / 8);
                        output[c] = numbers[dmNums[yp]][k * 8 + j % 8];
                    }
                }
                if (i == (rows - 1) && co >= (columns - 2) && !mdy) {
                    if (co == (columns -2)) {
                        output[c] = 0x44;
                    } else {
                        output[c] = 0x4D;
    ...
    function getSvg(string memory imageData) private view returns (string memory) {
        string memory svg = "<svg xmlns='http://www.w3.org/2000/svg' version='1.1' height='1080' width='1080' viewBox='0 0 360 436' style='background-color:white' class='diagram' text-anchor='middle' font-family='monospace' font-size='13px'><g class='text'>";

        uint y = 20;

        for (uint i = 0; i < rows; i++) {
            svg = string(abi.encodePacked(
                svg, "<text x='180' y='", uint2str(y), "'>", substring(imageData, (i * (columns + 3)) + prefixLength, ((i * (columns + 3)) + prefixLength) + columns) ,"</text>"
            ));
            y += 16;
        }
...
interface IBirthday {
    function mint(uint tokenId, bool mdy) payable external returns (string memory);
