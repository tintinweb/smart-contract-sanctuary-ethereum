...
contract Shop is ReentrancyGuard {
    ...
    function setFactory(address _factory) external returns (bool) {
        require(
            factory == address(0), "Shop: factory address has already been set"
        );
        ...
        returns (bool)
    {
        ...
        GovToken govToken = new GovToken(_lpName, _lpSymbol, msg.sender);

        lps[address(govToken)] = true;

        emit LpCreated(address(govToken));
    ...
    {
        require(
            IFactory(factory).containsDao(_dao), "Shop: only DAO can sell LPs"
        );
...
contract GovToken is ReentrancyGuard, ERC20, ERC20Permit {
    ...
    function burn(uint256 _amount, address[] memory _tokens, address[] memory _adapters, address[] memory _pools) external nonReentrant returns (bool) 
    {
