    ...
    UUPSUpgradeable
{
    ...
    function enteryLottery() public nonReentrant whenNotPaused {
        ...
        require(_currentTime() <= _lottery.endTime, "Ended");
        // if (s_usdc.balanceOf(_msgSender()) < s_entryFee) {
        //     console.log("Errors occuers in 94");
        //     revert Errors.SaturnaLotteryHub__NotEnoughUSDC();
        // }
        require(
            s_usdc.balanceOf(_msgSender()) >= s_entryFee,
            "You sdont havae enoguht usdc"
        );
        // if (s_hasEntered[s_lotteryId.current()][_msgSender()]) {
        //     console.log("Error occurs in 98");
        //     revert Errors.SaturnaLotteryHub__AlreadyEnteredThisLottery();
        // }

        require(
            s_hasEntered[s_lotteryId.current()][_msgSender()] == false,
            "Already entred draw"
        );

        // if (!(_currentTime() <= _lottery.endTime - s_entryBuffer)) {
        //     console.log("Error occurs in 103");
        //     revert Errors
        //         .SaturnaLotteryHub__CannotEnterLotteryEnteryBufferExpired();
        // }
        require(
            (_currentTime() <= _lottery.endTime - s_entryBuffer),
            "Cannot entery 30 mintues before"
        );

        s_usdc.safeIncreaseAllowance(address(this), s_entryFee);
        s_usdc.transfer(address(this), s_entryFee);
    ...
    // function getBalanceOfSender() public view returns {}
