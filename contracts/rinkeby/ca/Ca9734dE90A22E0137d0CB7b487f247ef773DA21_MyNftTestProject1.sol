...
error MyNftTestProject1__InvalidMintAmount();
error MyNftTestProject1__MaxSupplyExceeded();
error MyNftTestProject1__InsufficientFunds();
error MyNftTestProject1__AllowlistSaleClosed();
error MyNftTestProject1__AddressAlreadyClaimed();
error MyNftTestProject1__InvalidProof();
error MyNftTestProject1__PublicSaleClosed();
error MyNftTestProject1__TransferFailed();
error MyNftTestProject1__NonexistentToken();
...
contract MyNftTestProject1 is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    modifier mintCompliance(uint256 _mintAmount, uint256 _maxMintAmount) {
        if (_mintAmount < 1 || _mintAmount > _maxMintAmount)
            revert MyNftTestProject1__InvalidMintAmount();

        if (totalSupply() + _mintAmount > iMaxSupply)
            revert MyNftTestProject1__MaxSupplyExceeded();
    ...
    modifier mintPriceCompliance(uint256 _mintAmount) {
        if (msg.value < sMintPrice * _mintAmount)
            revert MyNftTestProject1__InsufficientFunds();
        ...
        mintPriceCompliance(_mintAmount)
    {
        if (sSaleState != SaleState.AllowlistOnly)
            revert MyNftTestProject1__AllowlistSaleClosed();

        if (sAllowlistClaimed[_msgSender()])
            revert MyNftTestProject1__AddressAlreadyClaimed();

        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));

        if (!MerkleProof.verify(_merkleProof, sMerkleRoot, leaf))
            revert MyNftTestProject1__InvalidProof();
        ...
        mintPriceCompliance(_mintAmount)
    {
        if (sSaleState != SaleState.PublicOpen)
            revert MyNftTestProject1__PublicSaleClosed();
    ...
    function withdraw() public onlyOwner nonReentrant {
        ...
        if (!success) revert MyNftTestProject1__TransferFailed();
    }
    ...
    {
        if (!_exists(_tokenId)) revert MyNftTestProject1__NonexistentToken();
