 ...
 *
 *
 *  Somewhere in the metaverse a bunch of NFT heroes fights to farm the
 *  $LEGENDZ token, an epic currency that only the bravest will be able
 *  to claim.
 *
 *  Enroll some heroes and start farming the $LEGENDZ tokens
 *  https://www.nullheroes.io
 *
 *  - OpenSea is already approved for transactions to spare gas fees
 *  - NullHeroes and related staking contracts are gas fee optimized, at least
 *    as much as I could :)
 *
...
contract NullHeroes is ERC721Enumerable, Ownable, Pausable {
    ...
    // test value
    // uint32 public constant MAX_TOKENS = 400;

    // max genesis heroes (25% of total heroes)
    uint256 public constant MAX_GENESIS_TOKENS = 10000;
    // test value
    // uint32 public constant MAX_GENESIS_TOKENS = 100;

    // max genesis giveaways (5% of genesis heroes)
    uint256 public constant MAX_GENESIS_TOKENS_GIVEAWAYS = 500;
    // test value
    // uint32 public constant MAX_GENESIS_TOKENS_GIVEAWAYS = 5;

    // mint price in $LEGENDZ
    uint256 public MINT_PRICE = 70000;

    // max per transaction (10)
    uint256 public constant MAX_TOKENS_PER_TX = 11;
    // test value
    // uint32 public constant MAX_TOKENS_PER_TX = 3;

    // genesis token giveaways counter
    uint256 public giveawayGenesisTokens;

    // mint genesis price in ether
    ...
    mapping(uint256 => Hero) private heroes;

    // mapping from proxy address to authorization flag
    ...
    ERC721("NullHeroes","NULLHEROES")
    {
        ...
        traits[0] = [1, 5, 3, 5, 4, 0, 3, 0, 5, 1, 4, 3, 2, 3, 4, 0, 2, 5, 1, 2, 4, 2, 0, 1];

        // races - human: 0 | orc: 1 | elf: 2 | undead: 3 | ape: 4 | human: 5
        traits[1] = [1, 1, 5, 5, 0, 2, 0, 0, 5, 1, 3, 5, 3, 1, 0, 5, 5, 1, 5, 1, 0, 1, 0, 1, 4, 0, 3, 5, 0, 5, 1, 1, 5, 0, 5, 0, 0, 1, 3, 5, 3, 1, 5, 0, 1, 1, 0, 5, 1, 0, 1, 0, 3, 5, 0, 5];
    ...
    function enrollGenesisHeroes(uint256 amount) external payable whenNotPaused {
        ...
        require(totalSupply < MAX_GENESIS_TOKENS, "genesis sale is over");
        require(amount < MAX_TOKENS_PER_TX, "1 to MAX_TOKENS_PER_TX - 1 heroes");

        require(totalSupply + amount <= MAX_GENESIS_TOKENS, "amount exceeds the number of genesis tokens left");
        require(msg.value == MINT_GENESIS_PRICE * amount, "ether value is incorrect");
    ...
    function enrollHeroes(uint256 amount) external whenNotPaused {
        uint256 totalSupply = _owners.length;
        require(totalSupply >= MAX_GENESIS_TOKENS, "genesis sale is not over yet");
        require(amount > 0 && amount < MAX_TOKENS_PER_TX, "1 to MAX_TOKENS_PER_TX - 1 heroes");

        require(totalSupply + amount <= MAX_TOKENS, "amount exceeds the number of tokens left");

        // check $LEGENDZ balance
        uint balance = legendz.balanceOf(_msgSender());
        uint cost = MINT_PRICE * amount;
        require(cost <= balance, 'not enough $LEGENDZ');

        // burn $LEGENDZ
        if (cost > 0) legendz.burn(_msgSender(), cost);
    ...
    function enrollGenesisHeroesForGiveaway(address recipient, uint256 amount) external onlyOwner whenNotPaused {
        ...
        require(giveawayGenesisTokens + amount < MAX_GENESIS_TOKENS_GIVEAWAYS, "amount exceeds the number of genesis tokens giveaways left");
     ...
     */
    function batchSafeTransferFrom(address _from, address _to, uint256[] memory _tokenIds, bytes memory _data) public {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
    ...
    /**
     * updates an attribute of the specified hero
     * restricted to operator contracts
     * @param tokenId the id of the token
     * @param attribute the attribute to update
     * @param value the new attribute value
     */
    function updateAttribute(uint256 tokenId, uint8 attribute, uint8 value) public whenNotPaused {
        require(proxies[_msgSender()], "only authorized operators");
        if (attribute == 0) { // force
            heroes[tokenId].force = value;
        } else if (attribute == 1) { // intelligence
            heroes[tokenId].intelligence = value;
        } else if (attribute == 2) { // agility
            heroes[tokenId].agility = value;
        }
    }
...
contract LEGENDZ is ERC20, Ownable {
    ...
    function setProxy(address proxyAddress, bool authorized) public onlyOwner {
        require(proxyAddress != owner(), "cannot change owner's authorization");
        proxies[proxyAddress] = authorized;
    }
}
