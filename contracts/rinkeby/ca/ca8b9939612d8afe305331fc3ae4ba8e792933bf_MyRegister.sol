...
{    
    ...
    //register ens
    function mekeCommit(string memory name, address owner, bytes32 secret, address resolver, address addr) public {
        bytes32 commitment= controller.makeCommitmentWithConfig(name,owner, secret, resolver,addr) ;
        controller.commit(commitment);

    }
    //register ens
    function register(string memory name, address owner, bytes32 secret, address resolver, address addr,uint duration) public payable{
