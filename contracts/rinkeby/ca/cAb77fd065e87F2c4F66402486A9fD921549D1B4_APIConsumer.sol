...
contract APIConsumer is ChainlinkClient, ConfirmedOwner {
    ...
    event LandGenerated(bytes32 indexed requestId, string plots_gend);
     ...
     *
     */
    constructor(address contractAddress) ConfirmedOwner(msg.sender) {
    ...
    function requestcoords(uint256 num_req, uint256 tokenCount)
        public
        returns (bytes32 requestId)
    {
        require(msg.sender == LandAddress);
        ...
        string
            memory url = "https://land-mint.dev.silks.io/api/v1/getcoords?tokenCount=";
        string memory url2 = "&num_req=";
        string memory tokenCountst = Strings.toString(tokenCount);
        string memory num_reqst = Strings.toString(num_req);
        bytes memory full_url = abi.encodePacked(
            url,
            tokenCountst,
            url2,
            num_reqst
        );
        string memory full_url_st = string(full_url);
    ...
    {
        emit LandGenerated(_requestId, _plots);
        plots_gend = _plots;
    }
