    ...
    ReentrancyGuardUpgradeable
{
    ...
    function userBalance(address _user)
        external
        virtual view returns(Token[] memory TokenDetails)
    {
        ...
        for(uint256 i = 0; i < number; i++){
            if(_userBalance[_user][i] != 0){
                Token memory _data = Token(_userBalance[_user][i],tokenType[_userBalance[_user][i]]);
        ...
        virtual
    {
        ...
        require(status && !signatureCheck[signature], "$GenesisV1: cannot purchase the token");
    ...
    function mintTo(
        ...
        emit crossmintToTokenDetails(to, temp, userId, msg.value, _usd_price, "GENESIS-MEMBER");
