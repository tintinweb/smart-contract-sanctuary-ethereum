...
contract ProofSale is Ownable, Context {
    ...
    function buyTokens(address beneficiary, uint256 tokenweiAmount) public payable {
        ...
        IWETH(WETH).deposit{value: weiAmount}();
        assert(IWETH(WETH).transfer(address(this), weiAmount));
        _token.mint(beneficiary, tokenweiAmount);

        emit TokensPurchased(_msgSender(), beneficiary, weiAmount, tokenweiAmount);
    }


    function withdraw(uint256 amountETH) external onlyOwner {
        require(amountETH > 0, "Contract has no money");
        IWETH(WETH).withdraw(amountETH);
        TransferHelper.safeTransferETH(_wallet, amountETH);
