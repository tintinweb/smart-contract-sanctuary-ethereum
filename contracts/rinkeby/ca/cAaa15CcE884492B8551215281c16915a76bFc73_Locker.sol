...
interface ILocker {
    ...
    // To prevent 'Stack too deep' error in _checkpoint function
...
contract Locker is 
    ...
    uint256 constant MAXTIME = 365 * 86400;
    ...
    mapping(address => uint256) public user_slashed;
    address public treasury;

    event Deposit(address indexed provider, uint256 value, uint256 indexed locktime, int128 vetype, uint256 ts, uint256 totalVeOlive, uint256 deltaVeOlive);
    ...
    function slash(address account, uint256 amount) external onlyOwner {
        ...
        require(_locked.end >= user_slashed[account] + amount, "Cannot slash more than locked");
        user_slashed[account] += amount;
        oliveToken.transfer(treasury, amount);
    ...
    function _checkpoint(address addr, LockedBalance memory old_locked, LockedBalance memory new_locked) internal {
        ...
        if (addr != address(0)) {
            ...
            if (old_locked.end > _blockTimestamp() && old_locked.amount > 0) {
                ...
                u_old.bias = u_old.slope * uint256Toint128(old_locked.end - _blockTimestamp());
            }
            if (new_locked.end > _blockTimestamp() && new_locked.amount > 0) {
                u_new.slope = new_locked.amount / uint256Toint128(MAXTIME);
                u_new.bias = u_new.slope * uint256Toint128(new_locked.end - _blockTimestamp());
            }
        ...
        if (_blockTimestamp() > last_point.ts) {
            block_slope = MULTIPLIER * (block.number - last_point.blk) / (_blockTimestamp() - last_point.ts);
        }
    ...
    */
    function deposit_for(address _addr, uint256 _value) external nonReentrant {
        ...
        require(_locked.end > _blockTimestamp(), "Cannot add to expired lock. Withdraw");
    ...
    */
    function create_lock(uint256 _value, uint256 _unlock_time) external nonReentrant {
        ...
        require(unlock_time > _blockTimestamp(), "Can only lock until time in the future");
        require(unlock_time <=  _blockTimestamp() + MAXTIME, "Voting lock can be 1 year max");
    ...
    */
    function withdraw() external nonReentrant {
        ...
        uint256 value = int128Touint256(_locked.amount) - user_slashed[msg.sender];
        ...
        emit Withdraw(msg.sender, value, _blockTimestamp());
    ...
    */
    function totalSupply() public view override returns (uint256) {
        uint256 _epoch = epoch;
        Point memory last_point = point_history[_epoch];
        return supply_at(last_point, _blockTimestamp());
    }
    ...
    */
    function totalSupplyAt(uint256 _block) external view returns (uint256) {
        ...
        if (target_epoch < _epoch) {
            ...
            if (point.blk != block.number) {
                dt = (_block - point.blk) * (_blockTimestamp() - point.ts) / (block.number - point.blk);
            }
        }
    ...
    function getVeOlive(int128 _value, uint256 _unlock_time) external view returns (int128) {
        int128 slope = _value / uint256Toint128(MAXTIME);
        int128 bias = slope * uint256Toint128(_unlock_time - _blockTimestamp());
        return bias;
    }
}
