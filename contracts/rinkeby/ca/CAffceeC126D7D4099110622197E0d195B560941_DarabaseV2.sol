...
contract DarabaseV2 is NFTContractV2{
    using ECDSA for bytes32;

    enum FeeSide{ BOTH, BUYER, SELLER, NONE }
    ...
    event FiatPayment(uint amount, string currency);
    event RentalRenewed(uint rentalId, uint256 startTime, uint256 endTime);
    
    //new modifier added
    modifier checkModifier(){
        if(royaltyFee == 0) revert("royalty fee not equal 0");
        _;
    }
    ...
    function buyRentalPrimarySale(
        ...
        (uint256 totalPayment) = calculateTotalPayment((sellOrder.payment.amount * numberOfRentals) / sellOrder.numberOfTokens, buyerFee);
    ...
    function distributeRevenue(
        ...
        (uint256 totalPayment ) = calculateTotalPayment((payment.amount * amountPerNFT) / numberOfTokens, distributorFee);

        doPlatformPayment(msg.sender, totalPayment, payment, distributorFee);

        for(uint i=0; i<rentals.length; i++){
            uint rentalId = rentals[i];
            address renter = getRentalOwner(pdrId, rentalId);
            if(searchIfRentalOwnerMatches(accounts, renter)){
                
                fee = amountPerNFT* balanceOfRental(pdrId, rentalId);
    ...
    function renewRental(
        ...
        uint pdrId = sellOrder.pdrId;
        uint rentalId = sellOrder.rentalId;
        uint rwoId = getRwoFromPdr(pdrId);
        require(getRwoOwner(rwoId) == _msgSender(), "Only rwo owner");
        require(startTime<endTime, "end time should be greater than start time");
        Rental memory rental = getRentalDetails(pdrId, rentalId);
        uint256 numberOfRentals = rental.amount;
 
        (uint256 totalPayment ) = calculateTotalPayment((sellOrder.payment.amount * numberOfRentals) / sellOrder.numberOfTokens, fee);
        uint256 platformPayment = fee;

        uint remaining = doPlatformPayment(rental.renter, totalPayment, sellOrder.payment, platformPayment);
        doPaymentTransfer(rental.renter, remaining, sellOrder.maker, sellOrder.payment, true);
    
        rental.startTime = startTime;
        rental.endTime = endTime;

        emit RentalRenewed(rentalId, rental.startTime, rental.endTime);
    }
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                keccak256(abi.encode(
                    ...
                    sellOrder.structVar1,
                    sellOrder.structVar2
                    )
                )
            )
        );
    ...
    function doPlatformPayment(
        ...
        if (payment.paymentClass == MATIC_PAYMENT_CLASS) {
            ...
            emit FiatPayment(totalPayment, "Euro"); // TODO: check from where currency 
        }
    ...
    // updated logic for upgrade function
    function setRoyaltyFee(
        uint256 _newRoyaltyFee
    ) external onlyOwner whenNotPaused{
        // updated logic
        royaltyFee = _newRoyaltyFee + 1;
    }
    ...
    //enum testing
    function setFeeSide(FeeSide _feeSide) external {
        feeSide = _feeSide;
    }

    // function to check new added variables
    ...
    function setNewArray(uint256 _val) external {
        newArray.push(_val);
    }

    // new function added
    function newFunction(uint256 _newRoyaltyFee) external onlyOwner {
        royaltyFee = _newRoyaltyFee + 2;
    }
...
contract NFTContractV2 is ERC1155, Ownable, Pausable{
    ...
    event RentalBurnt(address to, uint id, uint256 amount, uint256 startTime, uint256 endTime);
    event RWOTransferred(address to, uint nftId);
    event RentalTransferred(address to, uint id, uint256 amount, uint256 startTime, uint256 endTime);
    ...
    // rwo id to pdr ids
    mapping (uint => uint[]) public rwoToPdr;
    // pdr id to rental ids
    mapping(uint256 => uint[]) public pdrToRental;
    mapping(uint256 => uint) public pdrToRentalId;
    mapping (uint256 => mapping(uint256 => uint256)) public rentalIndex;
    ...
    function mintRWO(
        ...
        // rwo mapping
        rwo[nftId]= Rwo(uint128(nftId), address(to), string(uri));
    ...
    function mintPDR(
        ...
        //pdr mapping
        pdr[nftId] = Pdr(uint128(nftId), string(uri));
    ...
    function setApprovalForOwnerAndAdmin(address from, bool approved) public {
    ...
    function burnRental(
        ...
        emit RentalBurnt(rental[pdrId][rentalId].renter, rentalId, rental[pdrId][rentalId].amount, rental[pdrId][rentalId].startTime, rental[pdrId][rentalId].endTime);
        delete rental[pdrId][rentalId];
    }
    ...
    function balanceOfRental(uint pdrId, uint256 rentalId) public view returns(uint){
        return rental[pdrId][rentalId].amount;
    }

    // Get nft balance
    function getNFTBalance(uint pdrId, uint rentalId) public view returns(uint){
    ...
    function getPDRDetails(
        uint pdrId
    ) public view whenNotPaused returns(uint, address, uint, string memory){
    ...
    function getPDRFromRWO(
        uint rwoId
    ) public view whenNotPaused returns(uint[] memory) {
