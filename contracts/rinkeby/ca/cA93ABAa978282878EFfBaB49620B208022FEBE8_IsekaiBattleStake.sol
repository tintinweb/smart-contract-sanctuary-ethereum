...
contract IsekaiBattleStake is IERC721Receiver, ReentrancyGuard, Context, AccessControlEnumerable {
    ...
    function getRegionInfosLength() public virtual returns (uint256) {
        return regionInfos.length;
    }

    function stake(StakingRequest[] memory _stakingInfo) public virtual {
        require(ISB.isApprovedForAll(_msgSender(), address(this)));

        if (stakingInfos[_msgSender()].stakingCharactorCounts > 0) {
            claim();
        }

        //TODO: 一度すべて返すのではなく入れ替えた分だけ返すようにしたい
        ...
        for (uint256 i = 0; i < _stakingInfo.length; i++) {
            ...
            require(allTokenCount > MaxStakingCharacters);
            for (uint256 j = 0; j < _stakingInfo[i].tokenIds.length; j++) {
                ISB.safeTransferFrom(_msgSender(), address(this), _stakingInfo[i].tokenIds[i]);
            }
    ...
    function claim() public virtual nonReentrant {
        require(stakingInfos[_msgSender()].stakingRegionIds.length > 0);
        ...
        for (uint256 i = 0; i < stakingInfos[_msgSender()].stakingRegionIds.length; i++) {
            ...
            uint256 wpnBalance = WPN.balanceOfAll(_msgSender());
            uint256 amrBalance = AMR.balanceOfAll(_msgSender());
            uint256 weaponCount = (elapsed / targetRegionInfo.WeaponInterval) *
                targetRegionInfo.WeaponCounts[stakingCharactorCount] *
                tokenIds.length;
            weaponCount = wpnBalance + weaponCount > wpnBagCount ? wpnBagCount - weaponCount : weaponCount;
            uint256 armorCount = (elapsed / targetRegionInfo.ArmorInterval) *
                targetRegionInfo.ArmorCounts[stakingCharactorCount] *
                tokenIds.length;
            armorCount = amrBalance + armorCount > armBagCount ? armBagCount - armorCount : armorCount;
            ...
            if (weaponCount > 0) {
                ...
                for (uint256 j = 0; j < weaponCount; j++) {
                    uint256 randomNumber = _getRandomNumber() % 10000;
                ...
                for (uint256 j = 0; j < detailsCount.length; j++) {
                    AMR.mint(_msgSender(), targetRegionInfo.WeaponDetails[j].tokenId, detailsCount[j], '');
                }
            ...
            for (uint256 j = 0; j < tokenIds.length; j++) {
                uint256 random = _getRandomNumber() % 10000;
                if (random < seedPer) {
                    uint256 seedId = random % length;
                    SED.mint(_msgSender(), seedId, 1, '');
                    emit ClaimSeed(_msgSender(), regionId, seedId);
                }
            }
        }
    ...
    function getEstimate() public view virtual returns (uint256, uint256) {
        ...
        for (uint256 i = 0; i < stakingInfos[_msgSender()].stakingRegionIds.length; i++) {
            ...
            uint256[] memory tokenIds = stakingTokenIds[_msgSender()][stakingInfos[_msgSender()].stakingRegionIds[i]];
            if (!targetRegionInfo.active) continue;
            weaponCount +=
                (elapsed / targetRegionInfo.WeaponInterval) *
                targetRegionInfo.WeaponCounts[stakingCharactorCount] *
                tokenIds.length;
            armorCount +=
                (elapsed / targetRegionInfo.ArmorInterval) *
                targetRegionInfo.ArmorCounts[stakingCharactorCount] *
                tokenIds.length;
        }
...
contract IsekaiBattleWeapon is ERC1155Burnable, AccessControlEnumerable, Ownable, IIsekaiBattleWeapon {
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        bytes memory dataURI = abi.encodePacked(
            ...
            ' Lv',
            info.level.toString(),
            '","description": "A collection of Weapons that can be obtained in the fully on-chain game \\"Isekai Battle\\".  \\nThey will be used for attacking other players.  \\n  \\nIsekai Battle (https://isekai-battle.xyz/)","image": "',
            info.image,
...
contract IsekaiBattleArmor is ERC1155Burnable, AccessControlEnumerable, Ownable, IIsekaiBattleArmor {
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        bytes memory dataURI = abi.encodePacked(
            ...
            ' Lv',
            info.level.toString(),
            '","description": "A collection of Armor that can be obtained in the fully on-chain game \\"Isekai Battle\\".  \\nThey will be used for defending attacks from other players.  \\n  \\nIsekai Battle (https://isekai-battle.xyz/)","image": "',
            info.image,
            '","attributes": [{"trait_type":"Lv","value":',
            info.level.toString(),
            '},{"trait_type":"Type","value":"',
