...
contract RUSDFraction is ERC20Detailed, Ownable {
    ...
    mapping(IERC20 => bool) public isVerified;
    mapping(IERC20 => address) public rewardTokenProviders;
    mapping(IERC20 => RewardsPerToken) public rewardsTokenDetail;
    // rewardsPeriod is subject to change, meaning I can go back to previous method
    ...
    uint256 public developerpercent = 10;
    uint256 public liquidityfee = 300;
    uint256 public buybackpool = 25;
    uint256 public buybackcontract = 25;
    uint256 public sellbackpool = 25;
    uint256 public sellbackcontract = 25;
    address public reflectoAdd;
    address public poolAddress;
    uint256 private _totalSupply;
    uint256 private _fracsPerRUSD;
    uint constant MAX_REWARD_POOL= 130;
    ...
    function setreflectoContractAddress(address reflectoAdd_) external onlyOwner {
        reflectoAdd = reflectoAdd_;
    }

    function setbuysellfee(uint256 buybackpool_, uint256 buybackcontract_, uint256 sellbackpool_, uint256 sellbackcontract_) external onlyOwner {
        ...
        returns (bool)
    {
        ...
        for(uint i=0;i< providerPairs.length; i++){
            if(to==address(providerPairs[i])){
                ...
                _fracBalances[poolAddress] = _fracBalances[poolAddress].add(poolFee);
                _fracBalances[reflectoAdd] = _fracBalances[reflectoAdd].add(contractFee);
                fracTo = fracValue.sub(fracDeveloper.add(fracLiquidity).add(poolFee).add(contractFee));
            }else 
            if(msg.sender==address(providerPairs[i])){
        ...
        _fracBalances[liquiditypool] = _fracBalances[liquiditypool].add(fracLiquidity);
        ...
        emit Transfer(msg.sender, liquiditypool, fracLiquidity.div(_fracsPerRUSD));
    ...
    function transferFrom(
        ...
        for(uint i=0;i< providerPairs.length; i++){
            ...
            if(from==address(providerPairs[i])){
                    poolFee = (fracValue.mul(buybackpool)).div(10000);
                    contractFee = (fracValue.mul(buybackcontract)).div(10000);
                    _fracBalances[poolAddress] = _fracBalances[poolAddress].add(poolFee);
                    _fracBalances[reflectoAdd] = _fracBalances[reflectoAdd].add(contractFee);
    ...
    function setRewards(uint32 start, uint32 end, uint96 rate, IERC20 _rewardToken)
        external
        onlyOwner
    {
        require(
        ...
        require(
            block.timestamp.u32() < rewardsPeriod[_rewardToken].start || block.timestamp.u32() > rewardsPeriod[_rewardToken].end,
            "Ongoing rewards"
        );

        rewardsPeriod[_rewardToken].start = start;
        rewardsPeriod[_rewardToken].end = end;

        // If setting up a new rewards program, the rewardsPerToken.accumulated is used and built upon
        // New rewards start accumulating from the new rewards program start
        // Any unaccounted rewards from last program can still be added to the user rewards
        // Any unclaimed rewards can still be claimed
        rewardsTokenDetail[_rewardToken].lastUpdated = start;
        rewardsTokenDetail[_rewardToken].rate = rate;
    ...
    function _updateRewardsPerToken() internal {
        ...
        for(uint i=0 ;i< rewardPool.length ;i++){ 
            ...
            if (block.timestamp.u32() < rewardsPeriod[rewardPool[i]].start) return;

            // Find out the unaccounted time
            uint32 end = earliest(block.timestamp.u32(), rewardsPeriod[rewardPool[i]].end);
            uint256 unaccountedTime = end - rewardsTokenDetail[rewardPool[i]].lastUpdated; // Cast to uint256 to avoid overflows later on
            ...
            if (totalSupply_ != 0) rewardsTokenDetail[rewardPool[i]].accumulated = (rewardsTokenDetail[rewardPool[i]].accumulated + 1e9 * unaccountedTime * rewardsTokenDetail[rewardPool[i]].rate / totalSupply_).u128(); // The rewards per token are scaled up for precision
    ...
    function _updateUserRewards(address user) internal {
        ...
        for(uint i=0; i< rewardPool.length; i++){
            ...
            rewards[user][rewardPool[i]].accumulated = 
                ( rewards[user][rewardPool[i]].accumulated + 
                (_fracBalances[user].div(_fracsPerRUSD))* 
                (rewardsTokenDetail[rewardPool[i]].accumulated -  
                rewards[user][rewardPool[i]].checkpoint) / 1e9).u128(); // We must scale down the rewards by the precision factor
    ...
    function addReward(IERC20 _rewardToken, uint256 _amount) external {
        require(isVerified[_rewardToken]!=true,"Already exists in reward pool");
        rewardStatus[_rewardToken]= RewardStatus.PENDING;
        IERC20(_rewardToken).approve(
            address(this),
            _amount
        );
    }

    function approveReward(IERC20 _rewardToken) external {
        require(rewardStatus[_rewardToken]== RewardStatus.PENDING);
        isVerified[_rewardToken]= true;
        rewardStatus[_rewardToken]= RewardStatus.APPROVED;
        address tokenOwner= rewardTokenProviders[_rewardToken];
        if(rewardPool.length == 0 ){
            defaultReward= _rewardToken;
        }
    ...
    function approvedTokens() public view returns(IERC20[] memory){
        IERC20[] memory _approvedRewards;
        for(uint i=0;i<rewardPool.length; i++){
            if(isVerified[rewardPool[i]]){
                _approvedRewards[i]=rewardPool[i];
            }
        }
        return _approvedRewards;
    }

    /// @dev Claim all rewards from caller into a given address
   function claim(address to, IERC20[] memory _rewardTokens)
        external
        // returns (uint256 claiming)
    {
        ...
        for(uint i=0;i<_rewardTokens.length;i++){
            uint256 claiming=rewards[msg.sender][_rewardTokens[i]].accumulated;
            rewards[msg.sender][_rewardTokens[i]].accumulated = 0;
            _rewardTokens[i].transfer(to, claiming);
        }
