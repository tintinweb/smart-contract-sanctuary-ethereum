...
contract PotLottery {
    ...
    function EnterPot(
        ...
        calculateWinner();
        for (uint256 index = 0; index < _tokenNames.length; index++) {
            string memory _tokenName = _tokenNames[index];
            uint256 _amount = _amounts[index];
            address _participant = _participants[index];

            uint256 tokenDecimal = tokenWhiteList[_tokenName].tokenDecimal;

            bool hasEntryInCurrentPot = participantsTotalEntryInUsd[
                _participant
            ] == 0
                ? false
                : true;
            ...
            uint256 tokenUsdValue = (tokenLatestPriceFeed[_tokenName] *
                _amount) / 10**tokenDecimal;
