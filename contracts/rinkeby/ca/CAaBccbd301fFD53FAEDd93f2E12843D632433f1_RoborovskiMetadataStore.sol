...
contract RoborovskiMetadataStore is Ownable {
    ...
    mapping(uint16 => bytes32) private ipfsHashBytes;
    mapping(uint16 => bytes13) private traitBytes;
    ...
    function getIpfsHashHex(uint16 tokenId) public view returns (bytes memory) {
        return abi.encodePacked(IPFS_PREFIX, ipfsHashBytes[tokenId]);
    }

    function getIpfsHash(uint16 tokenId) public view returns (string memory) {
        return _toBase58(getIpfsHashHex(tokenId));
    }

    function getTraitBytes(uint16 tokenId) public view returns (bytes memory) {
