...
contract Personas is ERC721A, Ownable, Pausable, ReentrancyGuard {
    ...
    function mint(uint256 _mintAmount) public payable nonReentrant {
        ...
        require(balanceOf(_msgSender()) < 2, 'Address already claimed!');
    ...
    function mint2(uint256 _mintAmount) public payable nonReentrant {
        uint256 s = totalSupply();
        uint256 freetest = balanceOf(_msgSender()) + 1;
        require(_mintAmount > 0, "Cant mint 0");
        require(freetest / (balanceOf(_msgSender())) < 2, 'Address already claimed!');
        require(_mintAmount <= maxMintAmount, "Cant mint more then maxmint" );
        require(s + _mintAmount <= maxSupply, "Cant go over supply");
        require(msg.value >= cost * _mintAmount);
        _safeMint(msg.sender, _mintAmount);
