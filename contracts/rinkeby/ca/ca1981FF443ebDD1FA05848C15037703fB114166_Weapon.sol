...
contract Weapon is ERC721A, BaseTokenURI {
    using Strings for uint256;
    ...
    // Mapping for Weapon allotment
    mapping(uint32 => bool) public _weaponAllotted;

    // Whether staking is currently allowed or not
    bool public _stakingOpen = false;

    // Boolean to control claim weapon
    ...
    // EVENTS
    event StakeWeapon(uint32[] tokenIDs, address indexed owner, uint256 time);

    event UnstakeWeapon(uint32[] tokenIDs, address indexed owner, uint256 time);

    event UpdateWeaponURI(uint32 tokenID, string indexed uri, uint256 time);
    ...
    modifier claimIsOpen {
        require(_isClaimActive, "claim weapon is close");
    ...
    function claimWeapon(uint32[] calldata citizenTokenIds)
        public
        claimIsOpen
        {
        require(totalSupply() + citizenTokenIds.length < _maxSupply, "platform reached limit of minting");
        for(uint i = 0; i < citizenTokenIds.length; i++) {
            require(_citizen.ownerOf(citizenTokenIds[i]) == msg.sender, "caller is not the owner of this tokenID");
            require(!_weaponAllotted[citizenTokenIds[i]], "this NFTs weapon has already claimed");
            _weaponAllotted[citizenTokenIds[i]] = true;
        }
