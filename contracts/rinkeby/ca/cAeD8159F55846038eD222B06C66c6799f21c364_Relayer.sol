...
interface IMessenger {
  function sendRelayedMessage(address _from, uint _srcChainId, uint _tokenId, string memory _content) external;
 }

contract Relayer is NonblockingLzApp, Pausable {
    // event emitted every ping() to indicate new message
    ...
    function relayMessage(
        ...
        address _dstPingAddr, // destination address of Relayer contract
        uint _tokenId, // the number of pings
        ...
        bytes memory payload = abi.encode(_tokenId, _content);
    ...
    function _nonblockingLzReceive(
        ...
        (uint _tokenId, string memory _content) = abi.decode(_payload, (uint, string));

        // send relayed message
        messengerContract.sendRelayedMessage(_from, _srcChainId, _tokenId, _content);
    }
