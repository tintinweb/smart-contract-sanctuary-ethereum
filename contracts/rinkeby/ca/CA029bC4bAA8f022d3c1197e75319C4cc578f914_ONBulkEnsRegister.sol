...
contract ONBulkEnsRegister {
  ...
  function makeCommitment(string memory name, address sender, bytes32 secret) public view returns (bytes32) {
    ETHRegistrarController controller = ETHRegistrarController(_controllerAddress);
    return controller.makeCommitment(name, sender, secret);
  }
