...
abstract contract Soulbound is ERC721A, ERC721AQueryable, Ownable {
  ...
  function transferFrom(address from, address to, uint256 tokenId) public override(ERC721A, IERC721A) isTransferable {
    ERC721A.transferFrom(from, to, tokenId);
  }

  function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public override(ERC721A, IERC721A) isTransferable {
    ERC721A.safeTransferFrom(from, to, tokenId, _data);
  }

  modifier isTransferable() {
    if (!transferrable) revert TokenNotTransferable();
    _;
  }
}
