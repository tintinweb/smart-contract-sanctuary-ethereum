    ...
    GovernorTimelockControl
{
    ...
    ) public onlyMember override(Governor, IGovernor) returns (uint256) {
...
contract UserRoles is AccessControlEnumerable {
  bytes32 public constant MEMBER_ROLE = keccak256("Member");
  bytes32 public constant ADMIN_ROLE = keccak256("Admin");
  bytes32 public constant POST_ROLE = keccak256("Post");

  event GroupAdd(bool _status, bytes32 _name, string _reason);
  event GroupRemove(bool _status, bytes32 _name, string _reason);
  ...
  public
  {
    ...
    _setupRole(MEMBER_ROLE, root);
    _setRoleAdmin(MEMBER_ROLE, ADMIN_ROLE);
    _setRoleAdmin(POST_ROLE, ADMIN_ROLE);
    ...
    bytes32[] public groups_tab;

    function addGroup(bytes32 _name)
    public{
      //bytes32  __name = keccak256(abi.encodePacked(_name));
      for(uint i=0;i<groups_tab.length;i++){
        if(groups_tab[i]==_name)
        {
          emit GroupAdd(false,_name,"Group exist.");
          return;
        }
      }
      groups_tab.push(_name);
      _setRoleAdmin(_name, ADMIN_ROLE);
       emit GroupAdd(true,_name,"Group added.");
    }

    function removeGroup(bytes32 _name)
    public{
      //bytes32  __name = keccak256(abi.encodePacked(_name));
      for(uint i=0;i<groups_tab.length;i++){
        if(groups_tab[i]==_name)
        {
          emit GroupRemove(true,_name,"Group removed.");
          return;
        }
      }
      groups_tab.push(_name);
      _setRoleAdmin(_name, ADMIN_ROLE);
       emit GroupRemove(false,_name,"Group not exist.");
    }
    ...
    function addUserToRole(address account, bytes32 _role)
    public virtual onlyAdmin
    {
      //bytes32  __role = keccak256(abi.encodePacked(role));
      grantRole(_role, account);
    }

    function removeUserFromRole(address account, bytes32 _role)
    public virtual onlyAdmin
    {
      //bytes32  __role = keccak256(abi.encodePacked(role));
    ...
    function isRole(address account,bytes32 _role)
  ...
  modifier onlyPost()
  {
    require(isPost(msg.sender), "Restricted to post admins.");
    _;
  }
  ...
  function isPost(address account)
    public virtual view returns (bool)
  {
    return hasRole(POST_ROLE, account);
  }
  ...
  function addPost(address account)
    public virtual onlyAdmin
  {
    grantRole(POST_ROLE, account);
  }
...
contract Proposals {
    ...
    function updateSingleProposalHash(uint proposalId,string memory _hash)
    public
    payable
    onlyMember {
