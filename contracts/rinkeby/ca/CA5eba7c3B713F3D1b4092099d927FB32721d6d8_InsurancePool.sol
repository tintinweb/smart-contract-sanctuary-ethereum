...
DistributionManager
{
    ...
    modifier onlyLendPool() {
        require(_msgSender() == _getLendPool(), "invalid caller");
        _;
    }

    function _getLendPool() internal view returns (address) {
        return _addressProvider.getLendPool();
    }
    ...
    function handleAction(
        ...
        if (usersUnclaimedRewards[_user] > 0) {
            IERC20Upgradeable(REWARD_TOKEN).safeTransfer(_user, usersUnclaimedRewards[_user]);
            usersUnclaimedRewards[_user] = 0;
        }
    }
    ...
    function claimRewards(
        ...
        ? unclaimedRewards
        : _amount;
        usersUnclaimedRewards[user] = unclaimedRewards - amountToClaim;
...
contract DistributionManager is Initializable, OwnableUpgradeable {
    ...
    function _getAssetIndex(
        ...
        if (
            (_emissionPerSecond == 0 && needWingAmount == 0) ||
            _totalBalance == 0 ||
        ...
        GetAssetIndex memory vars;

        vars.currentTimestamp = block.timestamp > DISTRIBUTION_END
        ? DISTRIBUTION_END
        : block.timestamp;
        vars.timeDelta = vars.currentTimestamp.sub(_lastUpdateTimestamp);
        return
        _emissionPerSecond
        .mul(vars.timeDelta).add(needWingAmount)
...
contract LendPoolLoan is Initializable, ILendPoolLoan, ContextUpgradeable, IERC721ReceiverUpgradeable {
    ...
    function enterAuction(
        ...
        DataTypes.LoanData storage loan = _loans[loanId];
        require(loan.state == DataTypes.LoanState.Active, Errors.LPL_INVALID_LOAN_STATE);
        require(loan.bidStartTimestamp == 0, Errors.LPL_INVALID_LOAN_STATE);
        loan.bidStartTimestamp = block.timestamp;
        loan.state = DataTypes.LoanState.Auction;
        emit EnterAuctioned(loan.loanId, nftAsset, nftTokenId);
    }
    ...
    function exitAuction(
        ...
        emit ExitAuctioned(loan.loanId, nftAsset, nftTokenId);
    }
    ...
    function liquidateLoan(
        ...
        liquidateLoanInner(initiator, loanId, bNftAddress, borrowAmount, borrowIndex, false);
    }

    function liquidateLoanInner(
        address initiator,
        uint256 loanId,
        address bNftAddress,
        uint256 borrowAmount,
        ...
        if (byInsurance) {
            _loans[loanId].bidBorrowAmount = 0;
        } else {
        ...
        if (byInsurance) {
            emit LoanLiquidatedByInsurance(initiator,
                loanId,
                loan.nftAsset,
                loan.nftTokenId,
                loan.reserveAsset,
    ...
    function liquidateLoanByInsurance(
        ...
        liquidateLoanInner(initiator, loanId, bNftAddress, borrowAmount, borrowIndex, true);
    }
...
contract LendPoolAddressesProvider is Ownable, ILendPoolAddressesProvider {
    ...
    bytes32 private constant INSURANCE_POOL = "INSURANCE_POOL";
    ...
    bytes32 private constant WING_ORACLE = "WING_ORACLE";
    ...
    bytes32 private constant WING = "WING";
    ...
    function setInsurancePoolImpl(address pool, bytes calldata encodedCallData) external override onlyOwner {
        _updateImplInner(INSURANCE_POOL, pool, encodedCallData);
        emit InsurancePoolUpdated(pool, encodedCallData);
    }

    function _updateImplInner(bytes32 id, address newAddress, bytes memory encodedCallData) internal {
        address payable proxyAddress = payable(_addresses[id]);

        if (proxyAddress == address(0)) {
            // create proxy, then init proxy & implementation
            BendUpgradeableProxy proxy = new BendUpgradeableProxy(newAddress, address(this), encodedCallData);
    ...
    function getInsurancePool() external view override returns (address) {
        return getAddress(INSURANCE_POOL);
    }
    ...
    function setWingAddress(address newAddress) external override onlyOwner {
        _addresses[WING] = newAddress;
        emit WingUpdated(newAddress);
    }

    function getWingAddress() external view override returns (address) {
        return getAddress(WING);
    }
