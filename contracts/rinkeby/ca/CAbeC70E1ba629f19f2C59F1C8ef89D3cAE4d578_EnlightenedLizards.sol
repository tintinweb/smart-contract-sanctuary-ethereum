...
contract EnlightenedLizards is ERC721AQueryable, QuantegyLabsAccessControl {
    ...
    address private _rightsProtocolAddr; // Address of Rights Protocol Smart Contract
		...
		/// Claimables
    ////////////////////////////////////
		struct TokenClaimables {
			string key; // UUID
			bool claimed; // Has it been claimed or not
			string dateClaimed; // When was it claimed
			address addressClaimed; // Who claimed it
		}
		mapping(uint256 => TokenClaimables[]) public tokenIdClaimables;
    mapping(uint256 => bool) public tokenIdToClaimedWearableItem;
    mapping(uint256 => bool) public tokenIdToClaimedPosterItem;
        ...
        mintPriceCompliance(_mintAmount)
    {
        ...
        tokenIdToClaimedWearableItem[newLizardId] = false;
        tokenIdToClaimedPosterItem[newLizardId] = false;
        ...
        Address.sendValue(_rightsFeeRecipient, feeValue);
				...
				// TODO: any upfront costs from direct minting to pay out?
        ...
        returns (string[] memory tokenRightsUris)
    {
		...
		// TODO: Create claimable contract to handle claiming. Create modifier that onlyClaimer can claim it
		// and have some local storage on this contract for setClaimer/claimer - require(msg.sender == claimer)
		function claimWearableItem(uint256 _tokenId, address _phan) public onlyOwner {
			...
			require(tokenIdToClaimedWearableItem[_tokenId] == false, "Wearable item for this token has already been claimed");
			tokenIdToClaimedWearableItem[_tokenId] = true;
		...
		function claimPosterItem(uint256 _tokenId, address _phan) public onlyOwner {
			...
			require(_phan == ownerOf(_tokenId), "Only the token owner can claim redeemables");
			// Reward can only be claimed once per token
			require(tokenIdToClaimedPosterItem[_tokenId] == false, "Poster item for this token has already been claimed");
			tokenIdToClaimedPosterItem[_tokenId] = true;
    ...
    function setCost(uint256 _cost) public onlyCTO {
        cost = _cost;
    }
        ...
        onlyCTO
    {
		...
		/// Banking
		////////////////////////////////////
    function sendToTreasury() public onlyCEO nonReentrant {
...
contract QuantegyLabsAccessControl is Ownable, ReentrancyGuard {
	...
	constructor() {
		...
		// The creator of the contract also will act as the treasury until re-appointed
	...
	/// @dev Internal call to get the treasury address as the CEO only
	function getTreasury() external view onlyCEO returns (address) {
		return treasury;
	}
