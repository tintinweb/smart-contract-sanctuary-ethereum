    ...
    ILayerZeroUserApplicationConfig
{
    ...
    function setTrustedSource(uint16 _chainId, bytes calldata _sourceAddress)
        external
        onlyOwner
    {
        require(
            trustedSourceLookup[_chainId].length == 0,
            "The source address has already been set for the chainId!"
        );
        trustedSourceLookup[_chainId] = _sourceAddress;
    }

    // set the inbound block confirmations
    function setInboundConfirmations(uint16 sourceChainId, uint16 confirmations)
        external
    {
        layerZeroEndpoint.setConfig(
            layerZeroEndpoint.getSendVersion(address(this)),
