...
contract myfi_testnet is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable
{
    ...
    //once closer to mint set start block to desired block
    uint256 public myfiStartBlock = 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;
    ...
    enum State {Init, Waiting, ConnectionTime, Done}
    ...
    //specifies the block number where
    //you can begin connecting to the MyFi Network
    //AKA mint your MyFi NFTs
    //brings contract to waiting state
    function setMyFiStartBlock(uint256 _newMyFiStartBlock) public onlyOwner initState
    {
        myfiStartBlock = _newMyFiStartBlock;
        state = State.Waiting;
    }
    ...
    {
        state = State.Init;
        myfiStartBlock = 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;
    }

    //check whether the sale has begun
    //function connectionStarted() public view returns (bool)
    function connectionStarted() public returns (bool) 
    {
        //not aware if block past sales block
        if(state != State.ConnectionTime)
        {
            if(block.number >= myfiStartBlock)
            {
    ...
    {
        require(connectionStarted(), 'connection ports opening s00n :)');
