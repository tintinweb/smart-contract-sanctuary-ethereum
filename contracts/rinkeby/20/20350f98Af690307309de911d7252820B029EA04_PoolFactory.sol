...
contract PoolFactory is Initializable, OwnableUpgradeable, IPoolFactory {
    ...
    function createPool(uint256 poolId) external returns (address) {
        ...
        //  require(poolInfo.token.balanceOf(poolInfo.manager) != 0, "sender have no balance");
        ...
        Pool pool = new Pool(
         ...
         // require(poolInfo.token.balanceOf(msg.sender) != 0, "sender have not test USDT");
         
        //   require(poolInfo.token.approve(msg.sender, poolInfo.tokenTarget),"not yet approved");
        //   console.log(poolInfo.token.approve(msg.sender, poolInfo.tokenTarget,'hvugvu');
       ...
       // require(poolInfo.token.balanceOf(msg.sender) != 0, "sender have not test USDT");   
       // poolInfo.token.allowance(msg.sender, address(pool));
        // poolInfo.token.approve(msg.sender, poolInfo.tokenTarget+10); 
        // poolInfo.token.approve(address(pool), poolInfo.tokenTarget+10);   
