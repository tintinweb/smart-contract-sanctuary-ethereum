...
contract Erc721SalesforceV1 is Erc721Basic {
    ...
    event Withdraw(uint256 contractBalance, address withdrawAddress, uint256 withdrawAmount, address royaltyAddress, uint256 royaltyAmount);
    ...
    */
    constructor(Params memory params_) Erc721Basic(params_.name, params_.symbol) {
        ...
        _contractURI = params_.contractURI;
        ...
        if (params_.totalRoyalty > 0) {
            ...
            addPayoutSplitter(payable(address(payoutSplitter)), params_.totalRoyalty);
            payoutSplitterAddress = payable(address(payoutSplitter));
        }

        emit NftDeployed(_msgSender(), address(address(this)), payoutSplitterAddress);       
    }

    /**
        @param payoutSplitter_ The address of the PayoutSplitter that would handle royalty payments.
    */
    function addPayoutSplitter(address payable payoutSplitter_, uint96 totalRoyalty_) public onlyRole(DEPLOYER_ROLE) whenNotPaused {
        require(payoutSplitter_ != address(0), "E011");
        require(totalRoyalty_ <= 10000, "E012");    //  <= 100%

        if (payoutSplitter_ != address(0)) {
            payoutSplittersHistory.push(payoutSplitter_);
            payoutSplittersCount++;
            _setDefaultRoyalty(payoutSplitter_, totalRoyalty_); //  ERC721Royalty / ERC2981
        }
    }
    ...
    */
    ...
    /**
        @dev This function is used to reserve/mint tokens by project owners. It can be called by any account in the MINTER_ROLE.
    ...
    */
    ...
    /**
        @param tokenId_ The tokenId for whose URI is requested
    */
    ...
    */
    function withdraw() external onlyRole(WITHDRAW_ROLE) whenNotPaused {
        ...
        (address royaltyAddress, uint256 royaltyAmount) = this.royaltyInfo(0, balance); // Any TokenId can be passed as param. Defaulting to 0
        uint256 withdrawerAmount = balance - royaltyAmount;

        if (royaltyAddress != address(0)) {
            Address.sendValue(payable(royaltyAddress), royaltyAmount);
        }
        Address.sendValue(payable(_msgSender()), withdrawerAmount);
        emit Withdraw(balance, _msgSender(), withdrawerAmount, royaltyAddress, royaltyAmount);
    }
