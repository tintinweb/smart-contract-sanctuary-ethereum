...
contract RociPayment is ERC20CollateralPayment, IRociPayment{
    ...
    * @param _userId NFCS ID
    * @return total Oustanding balance
    */
    function getUserTotalOustanding(uint _userId) external override view returns(uint){
        address user = IERC721(lookup(Role.NFCS)).ownerOf(_userId);
    ...
    ERC20PaymentStandard
{
     ...
     */
    function claimCollateral(uint256 _id, uint256 _amount) external override {
        ICollateralManager manager = ICollateralManager(
            lookup(Role.collateralManager)
        );

        require(
            msg.sender == _loanLookup[_id].borrower,
        ...
        require(
            int256(_amount) <= int256(infoHolder.availableCollateral),
            "You may not remove this much collateral yet"
        );

        manager.withdrawal(_loanLookup[_id].borrower, _amount, msg.sender);
    }
        ...
        returns (PricedLoanInfo memory)
    {
        ...
        uint16 score = oracle.getScore(
            ln.nfcsID
        ).creditScore;

        uint256 ltv = oracle.LTV(ln.ERC20Address,score);
        uint256 lt = oracle.LT(ln.ERC20Address, score);
        require(ltv > 0, "ERC20CollateralPayment: LTV Cannot be = 0");

        int256 collateralUSD = priceFeedc.getLatestPriceUSD(erc20Contract) *
            int256(collateral);
        int256 outstanding = int256(Loan.getOutstanding(ln));
        ...
        int256 outstandingUSD = priceFeedc.getLatestPriceUSD(ln.ERC20Address) *
            outstanding;
    ...
    * @dev function hook to execute every time a loan is changed
     */
    function _afterLoanChange(Loan.loan memory _ln) internal virtual override{
        if(_ln.issued){
            Tracker.updateCollateral(usersCollateral[_ln.borrower],_ln);
        }
    }
 ...
 */
abstract contract ERC20PaymentStandard is IERC20PaymentStandard, AddressHandler {
    ...
    uint public override MAXIMUM_BORROW_LIMIT;
    ...
    function loanLookup(uint _id) external view override returns(Loan.loan memory){
        ...
        returns (uint256, address)
    {
        ...
        (uint128 dailyLimit,uint128 globalLimit) = nfcs.getLimits(ln.nfcsID);
    ...
    {
        if (!isComplete(_id)) {
            Loan.increaseTotalPaymentsValue(_loanLookup[_id], globalLoanLookup[_loanLookup[_id].borrower], globalLoanLookup[address(0)], _am);
            _afterLoanChange(_loanLookup[_id]);
            return true;
        } else {
            return false;
        }
     ...
     */
    function isDelinquent(uint256 _id) public view virtual override returns (bool) {
        return (_isLate(_id));
    }
    ...
    function configureNew(
        ...
        _afterLoanChange(_loanLookup[id]);
        return id;
    }
    ...
    function withdrawl(
        ...
        uint awaitingCollectionBeforeChange = _loanLookup[_id].awaitingCollection;
        Loan.loan storage ln = _loanLookup[_id];
        Loan.onWithdrawal(ln, globalLoanLookup[_loanLookup[_id].borrower], globalLoanLookup[address(0)], _am);
        _afterLoanChange(ln);
        IERC1155 Bonds = IERC1155(lookup(Role.bonds));
        IERC20 erc20 = IERC20(_loanLookup[_id].ERC20Address);
        require(
            _loanLookup[_id].issued,
    ...
    {
        Loan.loan storage ln = _loanLookup[_id];
        Loan.onPayment(ln, globalLoanLookup[_loanLookup[_id].borrower], globalLoanLookup[address(0)], _erc20Amount);
        _afterLoanChange(ln);
    ...
    function _isLate(uint256 _id) internal view virtual returns (bool) {
        return (Loan.isLate(_loanLookup[_id]) &&
            !isComplete(_id));
    }
...
library Tracker{
  ...
  function toArrays(outstandings storage c) internal view returns(address[] memory tokens, uint[] memory amounts, uint length){
    address tmp = (c.head == address(0)) ? TAIL : c.head;

    while(tmp != TAIL){
      tokens[length] = tmp;
      amounts[length] = c.tokenToLoan[tmp].val;
...
*/
interface IERC20CollateralPayment is IERC20PaymentStandard{
...
library Loan{
    ...
    * @param _global global loan for the whole contract
    ...
    function onPayment(loan storage _ln, globalInfo storage _user, globalInfo storage _global, uint _erc20Amount) internal{
        ...
        _ln.paymentDueDate = block.timestamp + _ln.paymentPeriod; //Reset paymentTimer;
    ...
    function limitCheck(loan storage _ln, globalInfo storage _user, limits memory _limits, uint _totalOutstanding) internal{
        if(_limits.dailyLimit != 0){
            _updateLoanDay(_user);
            require(_user.borrowedToday + _ln.principal <= _limits.dailyLimit, "Exceeds daily borrow limit");
            _user.borrowedToday += uint128(_ln.principal);
        }
        if(_limits.totalLimit != 0){
            require(_totalOutstanding + _ln.principal <= _limits.totalLimit, "Exceeds your total borrow limit");
        }
    }
    ...
    * @param _global global loan for the whole contract
    ...
    function issue(loan storage _ln, globalInfo storage _user, globalInfo storage _global) internal{
        require(
            !_ln.issued,
            "cannot issue a loan that is already issued"
        );
    ...
    function isLate(loan storage _ln) internal view returns (bool) {
        return (block.timestamp >= _ln.paymentDueDate);
    }
...
interface NFCSInterface {
    ...
    function getLimits(uint _userId) external view returns(uint128, uint128);
}
