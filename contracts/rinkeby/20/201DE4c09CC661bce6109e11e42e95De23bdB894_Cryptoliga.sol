...
contract Cryptoliga is ERC2981, ERC721A, Pausable, AccessControl {
    ...
    Stages public saleStage = Stages.CLOSED;
    ...
    error InvalidStage(Stages required, Stages current);
    error SaleNotActive();
    ...
    error AttemptToExceedAllowance(uint requested, uint available);
    ...
    function setWlAllowance(uint64 _allowance)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        wlAllowance = _allowance;
    }

    function wlAllowanceUsed(address _to) external view returns (uint64) {
        return _getAux(_to);
    }

    function wlAllowanceRemaining(address _to) public view returns (uint64) {
        return wlAllowance - _getAux(_to);
    }
    ...
    function setStage(uint _stage) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (Stages(_stage) == saleStage) {
            revert InvalidStage(Stages(_stage), saleStage);
        }
    ...
    function fxMint(
        ...
        if (saleStage != Stages.PUBLICSALE) {
        ...
        if (totalSupply() + numberToMint > maxSupply) {
        ...
        // no need to worry about wlAllowance here, because the public mint is open
    ...
    function whitelistMint(
        ...
        if (saleStage != Stages.PRESALE) {
            revert SaleNotActive();
        }

        if (numberToMint > maxBatchSize) {
            revert AttemptToMintOverMaxBatchSize(numberToMint, maxBatchSize);
        }
        ...
        if (_getAux(to) + numberToMint > wlAllowance) {
            revert AttemptToExceedAllowance(numberToMint, wlAllowance);
        }

        _setAux(to, _getAux(to) + numberToMint);
    ...
    /***
     * @dev Mints a new token and transfers it to the given address.
     * @param _to The address to transfer the token to.
     */
    function safeMint(address to, uint64 numberToMint)
        external
