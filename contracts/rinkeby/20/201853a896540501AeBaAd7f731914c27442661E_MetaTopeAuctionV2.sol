...
contract MetaTopeAuctionV2 is MetaTopeIERC1155Receiver, Ownable {
    ...
    function computeWinners(uint256 _batchSize) external onlyOwner onlyEnded {
            ...
            )
        {
            ...
            if (totalTokenWon > totalTokenAmount) {
                winnersComputed = true;
                emit WinnersComputed();
                break;
            }
            ...
            if (lowestPrice == 0) lowestPrice = _bid.pricePerToken;

            if (_bid.pricePerToken < lowestPrice) lowestPrice = _bid.pricePerToken;
        }
    }
    ...
    function transferTokens(uint256 _batchSize) external onlyOwner onlyEnded {
            ...
            )
        {
            ...
            if (transferredIndex > winnerAddresses.length - 1) {
    ...
    function createSortableBids(uint256 _batchSize) external onlyOwner onlyEnded {
        ...
        for (sortedIndex; sortedIndex < range; ++sortedIndex) {
            if (sortedIndex > (bidderAddresses.length - 1)) {
                isSortable = true;
                sortedIndex = 0;
                break;
            }
     ...
     * @dev Sort bids
     */
    function sortBids(uint256 _batchSize) external onlyOwner onlyEnded {
        require(isSortable, "Sortable bids should be created");
        require(_batchSize >= 1, "BatchSize should be greater than 0");

        uint256 range = sortedIndex + _batchSize;
        for (sortedIndex; sortedIndex < range; ++sortedIndex) {
            if (sortedIndex > (sortableAddresses.length - 1)) {
                bidsSorted = true;
                break;
            }

            unchecked {
                WinAuctionBid memory cBid = sortedBids[sortedIndex];
                uint256 j = sortedIndex - 1;
                while ((int(j) >= 0) && (sortedBids[j].pricePerToken < cBid.pricePerToken)) {
                    sortedBids[j + 1] = sortedBids[j];
                    j--;
                }
                sortedBids[j + 1] = cBid;
            }
