...
contract StakeContract {
    ...
    string public name = "SuperStaking";
    ...
    ///@dev Approve
    function approveTokens(address farmToken) public {
    ...
    function farmTokens(uint _amount, uint locking, address farmToken) public {
    ...
    function unfarmTokens(uint id, address unfarmToken) public safe cooldown {
        token_reward = IERC20(unfarmToken);
        token_reward.transfer(msg.sender, farming_unit[msg.sender][id].balance);

        bool has_revenue = is_unlocked(id, msg.sender);
        if (has_revenue) {  
            uint balance = _calculate_rewards(id, msg.sender, unfarmToken);
    ...
    function issueInterestToken(uint id, address issueToken) public safe cooldown {
        require(is_unlocked(id, msg.sender), "Locking time not finished");
        uint balance = _calculate_rewards(id, msg.sender, issueToken);
        token_reward = IERC20(issueToken);
        token_reward.transfer(msg.sender, balance);
    ...
    function get_pool(address tokenPool) public view returns (uint) {
        require(is_farmable[tokenPool], "Not active");
        return(token_pool[tokenPool].total_balance);
    }

  ///@notice Private functions

  ///@dev Helper to calculate rewards in a quick and lightweight way
    function _calculate_rewards(uint id, address addy, address token) public view returns (uint) {
    // get the users farming balance in token
        ...
        uint updated_reward_rate = reward_rate + lock_multiplier[token][locking_time] + time_allowed[locking_time].multiplier;
    ...
    function set_farming_state(bool status, address token) public authorized {
    ...
    function get_reward_rate_timed(uint time, address token) public view returns (uint) {
        uint reward_timed = reward_rate+lock_multiplier[token][time];
        return reward_timed;
    }
    ...
    function set_multiplier(uint time, uint multiplier, address token) public authorized {
    ...
    function get_multiplier(uint time, address token) public view returns(uint) {
