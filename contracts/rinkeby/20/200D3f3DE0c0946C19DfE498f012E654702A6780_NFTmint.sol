...
contract NFTmint is ERC721, Ownable {
    ...
    ERC1155Burnable public ERC1155BurnableInterface;
    address public LAUNCHPAD;
 
    constructor(string memory name, string memory symbol, uint max_supply, string memory base_url, address _contract) ERC721(name,symbol){
        baseURI = base_url;
        LAUNCH_MAX_SUPPLY = max_supply;
        IERC1155Interface = IERC1155(_contract);
        ERC1155BurnableInterface = ERC1155Burnable(_contract);
        LAUNCHPAD = _contract;
    }

    function mintNFT(uint256 _id) external {
        require(IERC1155Interface.balanceOf(msg.sender,_id) >= 2,"Insufficient balance please buy pass before mint" );
        require(LAUNCH_SUPPLY+1 <= LAUNCH_MAX_SUPPLY, "Max supply reached");
        require(msg.sender != address(0),"Invalid address");
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _safeMint(msg.sender, newItemId);
        ERC1155BurnableInterface.burn(msg.sender,_id,1);
        LAUNCH_SUPPLY++;
    }
