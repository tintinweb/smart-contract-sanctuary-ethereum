...
import "./interface/IArtGrailNFT721.sol";
...
contract ArtGrailAuction721 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function resultAuction(address _nftAddress, uint256 _tokenId) external nonReentrant {
        ...
        if (winningBid > auction.reservePrice) {
            ...
            address[] memory creatorAddress = IArtGrailNFT721(_nftAddress).getCreators(_tokenId);
...
interface IArtGrailNFT721 is IERC721Upgradeable {
    using SafeMathUpgradeable for uint256;
