...
contract Vault {
    ...
    function lockETH(address _ethAddress, address _algoAddress) external payable returns (uint256 _id) {
        ...
        users[_id].withdrawn = false;
        users[_id].deposited = true;
    ...
    function unlockETH(uint256 _id, address _algoAddress, uint256 _amount) external {
        ...
        require(users[_id].deposited, 'ETH is not yet deposited!');
        require(!users[_id].withdrawn, 'ETH is already withdrawn!');
        require(users[_id].amount >= _amount, 'balance is not enought to withdraw!');

        users[_id].withdrawn = true;
    ...
    function getBalance() public view returns (uint256 _id) {
