    ...
    Timestamp
{
    ...
    function _sync() internal virtual {
        ...
        if (factory_.shouldUpdateRatio()) {
            ...
            uint256 upTime=factory_.lastRatioUpdate()+factory_.secondsPerUpdate();
            if(_now256() > lastYieldDistribution && lastYieldDistribution < upTime){
    ...
    function _updateYieldRewardsPerWeight(uint256 _dlpPerSecond)internal virtual {
    ...
    function _additional() internal virtual {
        ...
        IFactory factory_ = _factory;
        // update DLP per second value in factory if required
        if (factory_.shouldUpdateAdditionalRatio() && additionalRemainingTotalAmount > 0) {
              uint256 upTime=factory_.lastAdditionalRatioUpdate()+factory_.secondsPerUpdate();
            if(_now256() > lastAdditionalYieldDistribution && lastAdditionalYieldDistribution < upTime){
    ...
    function _updateAdditionalYieldRewardsPerWeight(uint256 _dlpAdditionalPerSecond)internal virtual{
