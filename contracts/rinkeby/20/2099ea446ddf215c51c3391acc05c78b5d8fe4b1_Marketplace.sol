...
contract Marketplace is Ownable, AccessControl {
    ...
    function buyListing(string memory name, address alice, string memory discordId) public {
        uint256 currentBalance = balanceOf(msg.sender);
        uint256 price = NameToPrice[name];

        require(
            currentBalance - addressToSpent[msg.sender] >= price,
            "Insufficient funds"
        );

        uint256 i = 0;
        uint256 numSpots = 0;
        Spot[] memory list = NameToList[name];
        uint256 len = list.length;
        for (i; i < len; i++) {
            if (list[i].addy == alice) {
                numSpots++;
            }
        }
    ...
    function simpleBuyListing(string memory name, address alice, string memory discordId) public {
        ...
        uint256 len = list.length;
        for (i; i < len; i++) {
            if (NameToList[name][i].addy == alice) {
                numSpots++;
            }
        }
    ...
    function addAddressToList(string memory name, address alice, string memory discordId)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        NameToList[name].push(Spot(alice, discordId));
