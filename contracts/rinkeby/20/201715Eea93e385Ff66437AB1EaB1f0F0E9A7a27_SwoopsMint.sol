...
contract SwoopsMint is Ownable {
    ...
    function mint(uint256 quantity) public payable canMint(quantity) {
    ...
    /// Mint a Swoops token according to if the sender is whitelisted.
    ...
    /// Set the new number of max tokens mintable per drop.
    /// @dev Only callable by the contract owner.
    ...
    /// Set the new merkle tree (whitelist) root.
    /// @dev Only callable by the contract owner.
    ...
    /// Set if the sale of tokens is active.
    /// @dev Only callable by the contract owner.
    /// @param active The new state for if token sales are active.
    function setIsSaleActive(bool active) public onlyOwner {
        isSaleActive = active;
    }

    /// Set if the direct 3rd party minting of tokens is active.
    /// @dev Only callable by the contract owner.
    /// @param enabled The new state for if direct minting is enabled.
    function setDirectMintEnabled(bool enabled) public onlyOwner {
        directMintEnabled = enabled;
    }

    /// Increase the drop id.
    /// @dev Only callable by the contract owner.
    function increaseWhitelistDropId() public onlyOwner {
        whitelistDropId++;
    }

    /// Retrieve the total balance of this contract.
