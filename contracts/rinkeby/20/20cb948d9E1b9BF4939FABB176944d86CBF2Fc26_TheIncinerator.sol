...
contract TheIncinerator is Ownable, ReentrancyGuard {
  ...
  mapping (uint256 => mapping (address => uint256)) burnCountForPhase;
  ...
  constructor(address _partner) { // todo: remove _partner attr. might be able to remove constructor alltogether
    _interface = ERC721A(_partner);
  }

  function burn(uint256[] calldata _ids) external nonReentrant() {
    require(_interface.isApprovedForAll(msg.sender, address(this)), "Not approved to burn Jar Dudes");
    require(currentPhase().slotsRemaining > 0, "Phase is over");
    uint256 length = _ids.length;

    Phase memory _currentPhase = currentPhase();

    // The problem here is that someone can just move pieces to a new address and burn
    ...
    for(uint256 i; i < length;) {
      uint256 id = _ids[i];
       // Flips the bit from 0 to 1 for that ID
      _recordIdBurned(id);
    ...
    // Replace the last element of the array with the changes
    phases[phases.length - 1] = _currentPhase;
    // console.log('burn count', burnCountForPhase[_currentPhase.id][msg.sender]); // I can just console log a lot of
  }
