...
contract EarlyBirds_Staking is Ownable {
    ...
    IEB public EB_Contract = IEB(0x62B2b8cc53772b8Ceeccd2ad917F505822Bcd8cE);
    ...
    function stake(uint256[] calldata tokenIds, uint256 tierId)
        ...
        notContract
    {
        require(tiersRate[tierId] > 0, "TIER_NOT_EXIST");
        ...
        for (uint256 index = 0; index < tokenIds.length; index++) {
            if (
                ...
                EB_Contract.transferFrom(
                 ...
                 stakedTokens[tokenIds[index]].collected = this.claimableWormsForId(tokenIds[index]);
        ...
        returns (uint256)
    {
        ...
        for (uint256 index = 0; index < tokenIds.length; index++) {
            uint256 timePassed = block.timestamp -
                stakedTokens[tokenIds[index]].stakeDate;
            uint256 noBoostAmount = timePassed * tiersRate[stakedTokens[tokenIds[index]].tierId];

            if (stakedTokens[tokenIds[index]].boost == 0) {
                totalClaimable += stakedTokens[tokenIds[index]].collected + noBoostAmount;
            } else {
                totalClaimable += stakedTokens[tokenIds[index]].collected + noBoostAmount + ((noBoostAmount * stakedTokens[tokenIds[index]].boost) / 1000);
            }
        }
        ...
        returns (uint256)
    {
            ...
            uint256 timePassed = block.timestamp -
                stakedTokens[tokenId].stakeDate;
            uint256 noBoostAmount = timePassed * tiersRate[stakedTokens[tokenId].tierId];

            if (stakedTokens[tokenId].boost == 0) {
                return stakedTokens[tokenId].collected + noBoostAmount;
            } else {
                return  stakedTokens[tokenId].collected + noBoostAmount + ((noBoostAmount * stakedTokens[tokenId].boost) / 1000);
            }
