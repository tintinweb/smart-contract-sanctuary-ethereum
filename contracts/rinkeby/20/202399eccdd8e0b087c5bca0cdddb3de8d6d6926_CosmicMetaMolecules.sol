...
// ERC-721 Smart Contract for the Cosmic Meta Molecules NFT Collection - https://cosmicmeta.io
...
contract CosmicMetaMolecules is ERC721, ERC721Enumerable, Pausable, Ownable {
    using Strings for uint256;

    string public baseTokenURI;
    string public contractURI;
    uint256 public constant MAX_TOKENS = 8888; // 8888 Molecules
    uint256 public maxMint = 20; // Max 20 Molecules per transaction
    uint256 public price = 0.05 ether; // Each Molecules cost 0.05 ETH to mint
    ...
    constructor(string memory _baseTokenURI, string memory _contractURI) ERC721("Cosmic Meta Molecules", "CMMO") {
        setBaseURI(_baseTokenURI);
        setContractURI(_contractURI);

        // The first 100 Molecules are minted for the team, giveaways and our community
    ...
    function preMint(address _to, uint256 _quantity) public payable {
        ...
        require(hasPresaleAccess(_to), "You are not whitelisted for the Molecules pre-sale");
        require(!mainSale, "You can't pre-mint Molecules at the moment");
        require(_quantity > 0 && _quantity <= maxMint, "You can only mint 1 to 20 Molecules");
    ...
    function mint(address _to, uint256 _quantity) public payable {
        ...
        if (msg.sender != owner()) {
            require(mainSale, "The Molecules main sale is not open");
            require(_quantity > 0 && _quantity <= maxMint, "You can only mint 1 to 20 Molecules");
