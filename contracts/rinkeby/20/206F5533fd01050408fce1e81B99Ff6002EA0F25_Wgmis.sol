...
contract Wgmis is ERC721, Ownable {
    ...
    mapping(address => bool) merkleWhitelistClaimed;
    mapping(address => uint256) merkleWhitelistEarlyAccessMintCount;
    ...
    function mint(address _recipient, uint96 _quantity) external payable {
        ...
        require((msg.value) == (0.069 ether * _quantity), "incorrect ETH value provided");
    ...
    function mintFoundation(address _recipient, uint96 _quantity) external onlyOwner {
        ...
        _handleFoundationMint(_recipient, _quantity);
    }
    ...
    function mintMerkleWhitelistEarlyAccess(bytes32[] calldata _merkleProof, uint96 _merkleProofAmount, uint96 _mintAmount) external payable {
        require(merkleWhitelistClaimed[msg.sender], 'FIRST_MINT_FREE_UNITS');
        ...
        require((msg.value) == (0.069 ether * _mintAmount), "INCORRECT_ETH_VALUE");
    ...
    function _handleFoundationMint(address _recipient, uint96 _quantity) internal {
        for(uint96 i = 0; i < _quantity; i++) {
            uint256 newTokenId = 8887 - foundationMinted - i;
            _mint(_recipient, newTokenId);
        }
