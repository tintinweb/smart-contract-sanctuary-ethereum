...
// GET SALARIES
    ...
    function teamworkBonus(uint[] memory _jobIds) public view returns (uint, uint) { // returns a percent and total salary
        ...
        uint _combinedSalaries = 0; // TO-DO WHY DO WE HAVE THIS IN HERE?
        uint i;
    ...
    function teamworkBonusTest(uint _totalSalaries, uint _capacity, uint _teamsize) public view returns (uint, uint) { // returns a percent and total salary
        // for every 1% of the company that you own, you get 10% bonus
        uint _bonus = _teamsize * 10 / _capacity;
        uint _grandTotal = _totalSalaries + _totalSalaries * _teamsize * 10 / _capacity; 
        return(_bonus, _grandTotal);
    }
