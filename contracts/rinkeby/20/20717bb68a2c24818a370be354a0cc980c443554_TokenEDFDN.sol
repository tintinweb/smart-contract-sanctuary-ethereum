...
contract TokenEDFDN is ERC721, Ownable {
	using Strings for uint256;
	using Counters for Counters.Counter;

	Counters.Counter private _supply;

	string public baseURI = "ipfs://QmVnaRFymyeaD4n7HrUGftQtj5j7jNmcRfaNxsEkJ8P1q2/";
	string public baseExt = ".json";
	
	string public event_date = "Sunday, December 18, 2022, 18:00";
	...
	uint256 public constant MINT_PRICE = 4.4 ether;

	bool private _locked = false; // for re-entrancy guard

	// Initializes the contract by setting a `name` and a `symbol`
	constructor() ERC721( "8Match number #64. Final. Lusail Stadium (First category)" , "LSFCSF" ) {
		_supply.increment();
        //_mintLoop(msg.sender, MAX_SUPPLY);
	...
	function mint(uint256 _quantity) external payable nonReentrant {
		...
		require(_quantity >= 1, "Invalid mint quantity.");
		require((msg.value >= (MINT_PRICE * _quantity)) || (msg.sender == owner()) , "Not enough ETH.");
	...
	function _mintLoop(address _to, uint256 _quantity) private {
		 ...
		 */
		unchecked {
			require((_quantity + (_supply.current() -1)) <= MAX_SUPPLY, "Max supply exceeded.");
