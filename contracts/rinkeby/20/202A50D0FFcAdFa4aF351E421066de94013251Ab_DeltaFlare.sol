  ...
  //      _isExcludedFromContractBuyingLimit[address(uniswapV2Pair)] = true;
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && to != address(0) && to != address(0xdead) && 
           to != uniswapV2Pair && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
            require(amount <= _maxBuy, "Transfer amount exceeds the maxBuyAmount.");               
        } */

        if(!swapping && from != owner() && to != owner() && to != address(0) && to != address(0xdead) &&
        automatedMarketMakerPairs[from] && to != uniswapV2Pair && 
        !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
            require(amount <= _maxSell, "Transfer amount exceeds the maxBuyAmount.");            
        }
        ...
        if(canSwap && !swapping
        && (automatedMarketMakerPairs[from] || automatedMarketMakerPairs[to]) && from != marketingWallet && to != marketingWallet && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
