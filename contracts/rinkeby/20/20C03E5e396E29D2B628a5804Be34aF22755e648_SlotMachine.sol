...
contract SlotMachine is VRFConsumerBaseV2, ReentrancyGuard, Ownable, Pausable {
    VRFCoordinatorV2Interface COORDINATOR;
    IRewardPool REWARD_POOL;
    ...
    uint256 public slotMachineBalance;
        ...
        nonReentrant
    {
        ...
        if (reward > 0) {
            userBalance[userRequestId[requestId]] += reward;
            slotMachineBalance -= reward;
            REWARD_POOL.awardWinner(reward);
        } else {
            slotMachineBalance -= rollPrice;
            REWARD_POOL.addFunds{value: rollPrice}();
        }
    ...
    /// @notice Deposit funds
    function depositFundsForRewards()
        external
        payable
        nonReentrant
        whenNotPaused
    ...
    function executeRoll() external nonReentrant whenNotPaused {
        ...
        require(
            REWARD_POOL.rewardsBalance() > rollPrice * 30,
...
interface IRewardPool {
    ...
    function removeLiquidity(uint256 liquidity) external returns (uint256 amount);

    function addFunds() external payable;

    function awardWinner(uint256 amount) external;

    function rewardsBalance() view external returns (uint256);
}
