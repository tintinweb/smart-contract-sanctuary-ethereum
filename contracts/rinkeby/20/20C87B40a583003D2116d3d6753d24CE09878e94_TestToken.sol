...
contract TestToken is ERC721, ReentrancyGuard, Ownable {
  ...
  uint256 public PRICE_WL = 30000000000000000;
  ...
  function setPRICE_PUBLIC(uint256 _price) external onlyOwner {
    ...
    function RefillWhitelist(address[] calldata _to, uint[] calldata _value) external onlyOwner{
  ...
  function mintWL(uint256 count) public payable nonReentrant onlyWhitelisted {
    ...
    require(
      msg.value >= PRICE_WL * count - PRICE_WL, "Insufficient payment"
    );
  ...
  function godMint(address to, uint256 count) external onlyOwner {
    require(totalSupply() + count - 1 < MAX_SUPPLY, "Exceeds max supply");
    for (uint256 i = 0; i < count; i++) {
