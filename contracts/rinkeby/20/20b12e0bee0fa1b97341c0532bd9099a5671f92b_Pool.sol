...
// File contracts/OnlyTimelock.sol


pragma solidity ^0.8.0;

contract OnlyTimelock is Ownable {
    
    /* ===== STATE ===== */
    uint256 public TIMELOCK_REPLACEMENT_DELAY = 604800; // 1 week

    address public timelock; 
    address public newTimelock; 
    uint256 public nextTimelockUpdate;


    /* ===== INITIALIZE ===== */
    function initializeOnlyTimelock (address _timelock) public onlyOwner() {
        require(timelock == address(0), "OnlyTimelock::initializeOnlyTimelock: can only call this once");
        timelock = _timelock;
    }

    /* ===== REPLACE TIMELOCK ===== */
    function initiateTimelockChange (address _newTimelock) public onlyOwner() {
        require(_newTimelock != address(0), "OnlyTimelock::initiateTimelockChange: must not be zero address");
        newTimelock = _newTimelock; 
        nextTimelockUpdate = block.timestamp + TIMELOCK_REPLACEMENT_DELAY;
    } 

    function cancellTimelockChange () public onlyOwner () {
        newTimelock = address(0);
        nextTimelockUpdate = 0;
    }

    function setNewTimelock () public onlyOwner () {
        require (nextTimelockUpdate != 0, "OnlyTimelock::setNewTimelock: Must initiateTimelockChange first");
        require (nextTimelockUpdate < block.timestamp, "OnlyTimelock::setNewTimelock: wait untill nextTimelockUpdate");
    ...
    modifier onlyTimelock () {
        if (timelock == address(0)) {
            require(msg.sender == owner(), "OnlyTimelock::onlyTimelock: currently this function is only callable by owner");
            _;
        } else {
            require (msg.sender == timelock, "OnlyTimelock::onlyTimelock: This function is only callable by timelock");
            _;
        }
    }
}
...
library WethUtils {
    ...
    IWETH public constant weth = IWETH(0x82aF49447D8a07e3bd95BD0d56f35241523fBab1); // Arbitrum WETH
...
contract Pool is Ownable, OnlyTimelock, ReentrancyGuard, OnlyAdmin {
    ...
    // Collateral ratio
    uint256 public collateralRatio = 1000000; // 90% (900000)
    ...
    uint256 public constant REDEMPTION_FEE_MAX = 10000; // 1%
    uint256 public mintingFee = 5000; // 6 decimals of precision
    uint256 public constant MINTING_FEE_MAX = 10000; // 1%
    ...
    function getXSupplyAndUnclaimed() public view returns (uint256) {
        return ((xToken.totalSupply()).add(unclaimedXToken));
    }

    function realCollateralRatio () public view returns (uint256) {
        return ((usableCollateralBalance()).mul(1000000)).div(getXSupplyAndUnclaimed());
    }
        ...
        )
    {
        ...
        if (_realCollateralRatio < COLLATERAL_RATIO_MAX) {
            _yTokenOutSpot = (_xTokenIn * (COLLATERAL_RATIO_MAX - _realCollateralRatio) * (PRECISION - redemptionFee) * PRICE_PRECISION) / COLLATERAL_RATIO_MAX / PRECISION / _yTokenPrice;
            _yTokenOutTwap = (_xTokenIn * (COLLATERAL_RATIO_MAX - _realCollateralRatio) * (PRECISION - redemptionFee) * PRICE_PRECISION) / COLLATERAL_RATIO_MAX / PRECISION / _yTokenTWAP;
        }

        if (_realCollateralRatio > 0) {
            _ethOut = (_xTokenIn * _realCollateralRatio * (PRECISION - redemptionFee)) / COLLATERAL_RATIO_MAX / PRECISION;
            _ethFee = (_xTokenIn * _realCollateralRatio * redemptionFee) / COLLATERAL_RATIO_MAX / PRECISION;
        }
    }
    ...
    function calcExcessCollateralBalance() public view returns (uint256 _delta, bool _exceeded) {
        uint256 _requiredCollateralBal = (getXSupplyAndUnclaimed() * collateralRatio) / COLLATERAL_RATIO_MAX;
    ...
    bool flashLoansActive = false;

    function setFlashLoanContract (address _flashLoanContract) public onlyOwner () {
        require (flashLoanContract == address(0), "Pool::setFlashLoanContract: can only set once");
        require (_flashLoanContract != address(0), "Pool::setFlashLoanContract: invalid address");
        flashLoanContract = _flashLoanContract;
    }

    function toggleFlashLoansActive (bool _flashLoansActive) public onlyOwner () {
        require(address(flashLoanContract) != address(0), "Pool::setFlashLoanContract: flashloan contract not set");
        flashLoansActive = _flashLoansActive;
    }

    function sendToFlashLoanContract (uint256 _amount) external {
        require (flashLoansActive, "Pool::sendToFlashLoanConract: flashloans are not active");
        require (msg.sender == flashLoanContract, "Pool::sendToFlashLoanContract: only flash loan contract");
        require (_amount <= WethUtils.weth.balanceOf(address(this)), "Pool::sendToFlashLoanContract: amount exceeds weth balance");
        WethUtils.weth.transfer(flashLoanContract, _amount);
    }
    ...
    address[] xTokenWethPath;
    address[] wethXTokenPath;
    address[] yTokenWethPath;
    ...
    function approveArber (address _arber) public onlyTimelock () {
        arbers[_arber] = true;
    }
    ...
    */
    function arberMint(uint256 _xTokenAmount) external onlyArber {
        ...
        uint256 _wethGained = (WethUtils.weth.balanceOf(address(this))).sub(_startWethBalance);
    ...
    function arberBuybackRedeem(
        ...
        uint256 _projectedEndWeth = (_startWethBalance.sub(_wethBorrow)).add(_wethRecievedFromYSwap);
        uint256 _projectedEndXSupply = (getXSupplyAndUnclaimed()).sub(_xTokenIn);
        uint256 _projectedCR = (_projectedEndWeth.mul(1000000)).div(_projectedEndXSupply);
    ...
    */
    ...
    /// @notice set the collateral ratio used in minting
    /// @param _cr The desired collateral ratio (1000000 == 100%), minimum is 90 %
    function setMintCollateralRatio (uint256 _cr) public onlyOwner {
        require(_cr >= 900000, "Pool::setMintCollateralRatio: Must be > 900000 (90 %)");
        collateralRatio = _cr; 
        emit NewCollateralRatioSet(_cr);
    }


    /// @notice Transfer the excess balance of WETH to treasury
    /// @param _amount amount of WETH to reduce
    function reduceExcessCollateral(uint256 _amount) external onlyAdmin {
        (uint256 _excessWethBal, bool exceeded) = calcExcessCollateralBalance();
        if (exceeded && _excessWethBal > 0) {
    ...
    function setSwapStrategy(ISwapStrategy _swapStrategy) external onlyTimelock {
