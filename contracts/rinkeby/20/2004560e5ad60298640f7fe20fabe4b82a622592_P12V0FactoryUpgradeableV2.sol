...
// File contracts/interfaces/IP12V0FactoryUpgradeable.sol
...
interface IP12V0FactoryUpgradeable {
    ...
    // 查询mintDelay
    function getMintDelay(address gameCoinAddress, uint256 amountGameCoin)
        external
        view
        returns (uint256);
...
// File contracts/interfaces/IP12V0ERC20.sol
...
// File contracts/interfaces/IP12Mine.sol
...
interface IP12Mine {
    ...
    function setGameCoinCreator(address _gameCoinCreator) external;
}
...
// File contracts/P12V0FactoryUpgradeableV2.sol
    ...
    OwnableUpgradeable
{
    ...
    function create(
        ...
        require(
            liquidity > 0,
            "P12V0FactoryUpgradeableV2::liquidity value cant be zero"
        );

        // 将信息更新至p12mine中
        IP12Mine(p12mine).setLiquidityInfos(msg.sender, pair, liquidity);

        IP12Mine(p12mine).setGameCoinCreator(msg.sender);

        //授权至p12mine
        IUniswapV2Pair(pair).approve(p12mine,liquidity);
    ...
    function _create(
        ...
        P12V0ERC20 gameCoin = new P12V0ERC20(
            name_,
            symbol_,
            gameId,
            gameCoinIconUrl,
    ...
    function declareMintCoin(
        ...
        require(
            msg.sender == allGames[gameId],
            "FORBIDDEN: have no permission"
        );
        require(
            compareStrings(allGameCoins[gameCoinAddress], gameId),
            "FORBIDDEN"
        );
        //判断当前时间戳是否大于上次token增发的解锁时间，如果不大于则在解锁时间基础上加上延迟，否则在当前时间的基础上加上延迟
        uint256 time;
        uint256 currentTimestamp = getBlockTimestamp();
        bytes32 _preMintId = preMintIds[gameCoinAddress];
        uint256 lastUnlockTimestamp = coinMintRecords[gameCoinAddress][
            _preMintId
        ].unlockTimestamp;
        ...
        uint256 delayD = FullMath.mulDiv(
            amountGameCoin,
            delayK,
            P12V0ERC20(gameCoinAddress).totalSupply()
        ) + 4 * delayB;

        bytes32 mintId = _hashOperation(
            gameCoinAddress,
            msg.sender,
            amountGameCoin,
            time,
    ...
    /**
        hashOperation方法的可见性从public修改为internal
     */
    function _hashOperation(
        ...
        bytes32 preMintIdNew = keccak256(
            abi.encode(
                gameCoinAddress,
                declarer,
                amount,
        ...
        returns (bool)
    {
        ...
        // 这条 mint 没有被执行
        require(
            coinMintRecords[gameCoinAddress][mintId].executed == false,
            "this mint has been executed"
        );
        ...
        // 不大于0 则认为现在还没法 Mint
        require(
            time > coinMintRecords[gameCoinAddress][mintId].unlockTimestamp,
            "It's not time to Mint"
        );
        ...
        /** 
            先 mint 给合约地址
         */
        P12V0ERC20(gameCoinAddress).mint(
            address(this),
            coinMintRecords[gameCoinAddress][mintId].amount
        );
        ...
        returns (uint256 amountP12)
    {
        ...
        if (p12 < gameCoinAddress) {
            (p12Reserved, gameCoinReserved, ) = IUniswapV2Pair(
                IUniswapV2Factory(uniswapFactory).getPair(gameCoinAddress, p12)
            ).getReserves();
        } else {
        ...
        amountP12 = FullMath.mulDiv(
            p12Reserved,
            amountGameCoin,
            (gameCoinReserved * 100)
        );
        return amountP12;
    }
    ...
    function delayGetLpTokenBack() public virtual {}

    function executeGetLpTokenBack() public virtual {}
}