...
contract SpaceTreasury is SpaceAccessControlled, ITreasury {
	...
	mapping(address => address) private dexTokens;
	mapping(address => uint256) private dexLiquidity;
	...
	function create(
		...
		ownerPricingReserves[tokenId] = 50000 * 10**9; // the most flexible constant
		userPricingReserves[tokenId]  = 50000 * 10**9; // for price vs rebase amount
    ...
    function deposit(
		...
		uint256 ownerPricing = ownerPricingReserves[id_];
		uint256 userPricing = userPricingReserves[id_];

		value = futurePayments(id_);
		uint256 toRemove = getAmountOut(value, ownerPricing, userPricing);

		_rebase(id_, ownerPricing + value, userPricing - toRemove);
    ...
    function withdraw(
		...
		require(userReserves[tokenId_] == 0, "Treasury: active users exist");
		...
		address tokenAddress = alpAddress.assetPair(tokenId_).tokenAddress;

		if (dexTokens[tokenAddress] != address(0)) {
			require(dexTokens[tokenAddress] == token_, "Treasury: wrong token pair");
			
			(uint256 amountA,) = dexRouter.removeLiquidity(
				token_,
				tokenAddress,
				dexLiquidity[tokenAddress],
				0,
				0,
				msg.sender,
				deadline_
		...
		delete allocatedDeposits[tokenId_];
	...
	function buy(
		...
		require(alpAddress.assetPair(tokenId_).initialized, invalidToken);
        if (
			!permissions[STATUS.RESERVETOKEN][token_] && 
			!permissions[STATUS.LIQUIDITYTOKEN][token_]
		) {
		...
		uint256 ownerPricing = ownerPricingReserves[tokenId_];
		uint256 userPricing = userPricingReserves[tokenId_];
		uint256 toSend = getAmountOut(value, userPricing, ownerPricing);

		_rebase(tokenId_, ownerPricing - toSend, userPricing + value);
	...
	function sell(
        ...
        require(!permissions[STATUS.RESERVETOKEN][token_], invalidToken);
		require(
			alpAddress.assetPair(tokenId_).epoch > 10 || userReserves[tokenId_] == 0,
			insufficientReserves
		);
		...
		uint256 amount = getAmountOut(value_, userPricing, ownerPricing);
		...
		_rebase(tokenId_, ownerPricing - toSend, userPricing + value_);
    ...
    function incurDebt(
		...
		uint256 ownerPricing = ownerPricingReserves[tokenId_];
		uint256 userPricing = userPricingReserves[tokenId_];
		uint256 debt = getAmountOut(value, ownerPricing, userPricing);

		_rebase(tokenId_, ownerPricing + value, userPricing - debt);
    ...
    function repayDebt(
		...
		uint256 ownerPricing = ownerPricingReserves[tokenId_];
		uint256 userPricing = userPricingReserves[tokenId_];
		uint256 refund = getAmountOut(value, userPricing, ownerPricing);

		_rebase(tokenId_, ownerPricing - refund, userPricing + value);
	...
	function fullDelta(uint256 id) public view override returns (int256) {
		return int256(futurePayments(id)) + int256(initialDeposits[id]) + int256(ownerReserves[id]) - int256(userReserves[id]);
	}
	...
	function _dexDeploy(
		...
		if (tokenAmount > 0) {
			...
			dexTokens[alp] = token;
			dexLiquidity[alp] = liquidity;
		}
		
	}

	function _rebase(uint256 tokenId, uint256 amount0Out, uint256 amount1Out) internal {
		if (alpAddress.assetPair(tokenId).epoch > 0) {
			address tokenAddress = alpAddress.assetPair(tokenId).tokenAddress;
			uint256 circul = IERC20Rebase(tokenAddress).circulatingSupply();
			uint256 total = IERC20(tokenAddress).totalSupply();

			uint256 target = getAmountIn(circul, amount1Out, amount0Out);
			int256 delta = int256(target * ownerReserves[tokenId] / circul) - int256(total);
			
			alpAddress.rebase(tokenId, delta);
			_update(amount0Out, amount1Out, tokenId);
		} else {
			alpAddress.rebase(tokenId, 0);
		}
	}
