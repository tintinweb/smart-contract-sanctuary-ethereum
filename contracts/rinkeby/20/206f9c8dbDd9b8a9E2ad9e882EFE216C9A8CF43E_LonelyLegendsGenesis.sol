...
// Presale functions
  ...
  function setlegendsList(address[] calldata legendsListers, bool holderRank) external onlyOwner {
    uint256 quantity = holderRank ? 2 : 1;
...
// Public Sale functions
  ...
  function publicSaleMint(uint256 _mintAmount) external nonReentrant {
    require(msg.sender == tx.origin, "You cant mint using a smart contract.");
    require(publicSale, "There is no public sale ongoing right now.");
    require(_mintAmount > 0 && _mintAmount <= 1, "You can only mint 1 legend per transaction.");
    require(userMintedPublicSale[msg.sender] + _mintAmount <= 1, "You can only mint 1 Legend during the public sale.");
