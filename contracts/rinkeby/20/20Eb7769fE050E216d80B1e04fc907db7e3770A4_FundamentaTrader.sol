...
contract FundamentaTrader is SecureContract
{
    ...
    function swapExactInputSingle(uint256 id, uint256 amountIn, uint256 amountOutMin, address swapIn, address swapOut, uint24 poolFee) external returns (uint256 amountOut)
    {
        Dex memory dex = _dexs[id];
        ...
        amountOut = ISwapRouter(dex.router).exactInputSingle(params);
    }

    function swapExactInputMultihop(uint256 id, uint256 amountIn, uint256 amountOutMin, address swapIn, bytes calldata path) external returns (uint256 amountOut)
    {
        Dex memory dex = _dexs[id];
        ...
        ISwapRouter.ExactInputParams memory params =
            ISwapRouter.ExactInputParams({
                path: path,//abi.encodePacked(swapIn, dex.poolFee, swapVia, dex.poolFee, swapOut),
                recipient: msg.sender,
                deadline: block.timestamp,
                amountIn: amountIn - fee,
                amountOutMinimum: amountOutMin
        ...
        amountOut = ISwapRouter(dex.router).exactInput(params);
    }
