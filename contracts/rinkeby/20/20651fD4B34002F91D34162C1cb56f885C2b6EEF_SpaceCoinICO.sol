...
contract SpaceCoinICO {
  ...
  constructor(address treasury) {
    ...
    emit TokenDeployed(spcContract, treasury);
  }
  ...
  modifier checkIcoBalance() {
    if (currentPhase == uint8(Phase.SEED)) {
      if (address(this).balance > 15_000 ether) revert IcoLimit();
    }

    if (address(this).balance > 30_000 ether) revert IcoLimit();
    _;
  }
  ...
  function nextPhase() external onlyOwner {
    if (currentPhase + 1 <= uint8(Phase.ONE)) {
      currentPhase++;
      emit PhaseAdvanced(currentPhase);
    }
  }

  function contribute() public payable checkIcoBalance checkIcoPhase {
  ...
  function redeemToken() public {
    if (currentPhase != uint8(Phase.ONE)) revert NotOnRedeemPhase();
