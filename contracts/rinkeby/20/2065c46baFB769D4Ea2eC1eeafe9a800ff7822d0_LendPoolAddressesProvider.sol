...
import {UbiloanUpgradeableProxy} from "../libraries/proxy/UbiloanUpgradeableProxy.sol";
...
contract LendPoolAddressesProvider is Ownable, ILendPoolAddressesProvider {
  ...
  bytes32 private constant Ubiloan_ORACLE = "Ubiloan_ORACLE";
  ...
  bytes32 private constant Ubiloan_DATA_PROVIDER = "Ubiloan_DATA_PROVIDER";
  ...
  /**
   * @dev Returns the id of the Ubiloan market to which this contracts points to
  ...
  function getUbiloanDataProvider() external view override returns (address) {
    return getAddress(Ubiloan_DATA_PROVIDER);
  }

  function setUbiloanDataProvider(address provider) external override onlyOwner {
    _addresses[Ubiloan_DATA_PROVIDER] = provider;
    emit UbiloanDataProviderUpdated(provider);
  }
  ...
  function getImplementation(address proxyAddress) external view onlyOwner returns (address) {
    UbiloanUpgradeableProxy proxy = UbiloanUpgradeableProxy(payable(proxyAddress));
  ...
  function _updateImpl(bytes32 id, address newAddress) internal {
    ...
    if (proxyAddress == address(0)) {
      ...
      UbiloanUpgradeableProxy proxy = new UbiloanUpgradeableProxy(newAddress, address(this), params);

      _addresses[id] = address(proxy);
      emit ProxyCreated(id, address(proxy));
    } else {
      // upgrade implementation
      UbiloanUpgradeableProxy proxy = UbiloanUpgradeableProxy(proxyAddress);
...
contract UbiloanUpgradeableProxy is TransparentUpgradeableProxy {
  constructor(
    address _logic,
    address admin_,
    bytes memory _data
