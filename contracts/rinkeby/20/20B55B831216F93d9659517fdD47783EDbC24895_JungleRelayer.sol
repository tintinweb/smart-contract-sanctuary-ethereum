...
contract JungleRelayer is Ownable, ERC1155Receiver {

    event FeeRecipientChanged(address _feeRecipient);
    event CashbackChanged(uint256 _cashback);
    event ResaleFeeChanged(uint256 _resaleFee);

    enum HowToCall { Call, DelegateCall }
    enum NFTType { ERC721, ERC1155 }

    address public feeRecipient; // Address in which resaleFee will be deposited
    uint96 public denominator = 10000; // 
    uint96 public resaleFee; // Fee charged for resale
    uint96 public cashback; // Fee or amount which will be trasfer back
    ...
    /**
     * @dev Allows updation of current caller address for efficient NFT transfers on receival.
     */
    modifier callerUpdate() {
    ...
    constructor(address _feeRecipient, uint96 _resaleFee, uint96 _cashback){
     ...
     */
    function execute(Relay calldata relay) external payable callerUpdate {
        ...
        if(relay.paymentToken == address(0)) {
            ...
            uint256 remainingBalance = address(this).balance - contractBalance;
    ...
    {
        return _call(dest, howToCall, data, msg.value);
    }
    ...
    * @param _resaleFee resaleFee amount in uint256
     */
    function setResaleFee(uint96 _resaleFee) external onlyOwner {
    ...
    * @param _cashback cashback amount in uint256
     */
    function setCashback(uint96 _cashback) external onlyOwner {
     ...
     */
    function onERC1155Received(address, address, uint256 _id, uint256 _value, bytes calldata _data) external returns(bytes4) {
     ...
     */
    function onERC1155BatchReceived(address, address, uint256[] calldata _ids, uint256[] calldata _values, bytes calldata _data) external returns(bytes4) {
     ...
     */
    function onERC721Received(address, address, uint256 tokenId, bytes memory data) external returns(bytes4) {
        require(currentCaller != address (0),"onERC1155Received: transfer not accepted");
