...
contract Peace is ERC721URIStorage, Ownable {
    ...
    mapping(address=>bool) public recieved;
    mapping(address=>Counters.Counter) public giver;
    uint constant public TOKEN_LIMIT = 5;
    ...
    function mintNFT(address to) public payable {
        require(recieved[to] == false, "already received one");
        require(to != msg.sender, "can not give yourself");
        require(_tokenIds.current() < TOKEN_LIMIT, "limit reached");
        require(giver[msg.sender].current() < 2, 'already gave 3');

        uint256 newItemId = _tokenIds.current();

        recieved[to] = true; 
        _tokenIds.increment();
        giver[msg.sender].increment();
