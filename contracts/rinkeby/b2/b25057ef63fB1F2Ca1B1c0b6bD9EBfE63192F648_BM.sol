...
contract BM is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  enum TokenType { BlindBoxA, BlindBoxB, Merge }
  mapping(uint256 => TokenType) private tokenTypeMapping;
  ...
  // public
  function mintBlindBoxA(uint256 _mintAmount) public onlyMinter {
    uint256 supply = totalSupply();
    require(!paused);
    require(_mintAmount > 0);
    require(_mintAmount <= maxMintAmount);
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      ...
      tokenTypeMapping[tokenId] = TokenType.BlindBoxA;
    }
  }

  
  function mintBlindBoxB(uint256 _mintAmount) public onlyMinter {
    uint256 supply = totalSupply();
    require(!paused);
    require(_mintAmount > 0);
    require(_mintAmount <= maxMintAmount);
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
        uint256 tokenId = supply + i;
      _safeMint(msg.sender, tokenId);
      tokenTypeMapping[tokenId] = TokenType.BlindBoxB;
    }
  }

  function merge(uint256 blindBoxATokenId,uint256 blindBoxBTokenId) public onlyMinter {
    
    require(tokenTypeMapping[blindBoxATokenId]==TokenType.BlindBoxA,"ERC721: tokenId type mismatch");
    require(tokenTypeMapping[blindBoxBTokenId]==TokenType.BlindBoxB,"ERC721: tokenId type mismatch");

    require(_isApprovedOrOwner(_msgSender(), blindBoxATokenId), "ERC721: merge caller is not owner nor approved");
    require(_isApprovedOrOwner(_msgSender(), blindBoxBTokenId), "ERC721: merge caller is not owner nor approved");

    uint256 supply = totalSupply();
    require(!paused);

    burn(blindBoxATokenId);
    burn(blindBoxBTokenId);

    uint256 tokenId = supply + 1;

    _safeMint(msg.sender, tokenId);
    tokenTypeMapping[tokenId] = TokenType.Merge;
  }

  function burn(uint256 tokenId) private {
      delete tokenTypeMapping[tokenId];
      _burn(tokenId);
  }
