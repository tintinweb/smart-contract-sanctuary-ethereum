...
contract LiquidToken is iNFToken {
  ...
  function transferFrom(
    ...
    bool success = ERCtransferFrom(from, to, amount);
    _NFT_blk_transfer(from, to, amount);
    return (success);
  }
  ...
  // REVIEW Liquidity can't be added on uniswap: due to uniswap fees and no decimals?
  ...
  function ERCtransferFrom(
    ...
    address _to,
    uint256 _value
  ) public override onlyContract returns (bool success) {
    uint256 _allowance = allowed[_from][msg.sender];
    require(
      _balances[_from] >= _value && _allowance >= _value,
  ...
  // REVIEW  Find a way to implement decimals (like 0.01 = 1 NFT)
  // NOTE 3 Decimals means 1 TOKEN = 100 NFT
