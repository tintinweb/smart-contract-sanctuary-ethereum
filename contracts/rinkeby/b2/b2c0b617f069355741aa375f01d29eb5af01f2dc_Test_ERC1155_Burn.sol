...
contract Test_ERC1155_Burn is ERC721A, Ownable {

    ITreasure public immutable treasure;
	address treasureAddress = address(0x77bACCC1058D8a6aD6748E93673E81777b23d213);
    ...
    function random() external view returns (uint) {
        uint randomHash = uint(keccak256(abi.encodePacked(msg.sender))) * uint(keccak256(abi.encodePacked(block.timestamp)));
        return randomHash;
    } 

    function generateRandom() external view returns (uint) {
        uint _rand = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp)));
        return _rand;
    }
    ...
    function burn_ERC1155_for_mint(uint256 amount) external returns (uint256) {
    ...
    function chanceAirdrop(address to, uint count) external onlyOwner {
		require(
			_totalMinted() + count <= maxSupply,
			'Exceeds max supply'
		);
