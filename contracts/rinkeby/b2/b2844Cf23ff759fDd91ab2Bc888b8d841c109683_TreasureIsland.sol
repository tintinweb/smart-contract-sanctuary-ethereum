...
//####################################################################################################isHotel toevoegen
    ...
    function isHotel(uint16 id) external view returns (bool);
    function isStore(uint16 id) external view returns (bool);
    function isPenthouse(uint16 id) external view returns (bool);
...
//####################################################################################################HotelClaimed
    ...
    event HotelClaimed(uint16 tokenId, uint earned, bool unstaked);
    event StoreClaimed(uint16 tokenId, uint earned, bool unstaked);
    event PenthouseClaimed(uint16 tokenId, uint earned, bool unstaked);
...
//####################################################################################################Hotel
    mapping(uint256 => uint256) public hotelIndices;
    mapping(address => Stake[]) public hotelStake;

    mapping(uint256 => uint256) public storeIndices;
    mapping(address => Stake[]) public storeStake;

    mapping(uint256 => uint256) public penthouseIndices;
    mapping(address => Stake[]) public penthouseStake;
    ...
    //####################################################################################################hotelstaked
    uint public totalHotelStaked;
    uint public totalStoreStaked;
    uint public totalPenthouseStaked;
    uint public totalPirateStaked = 0;
    uint public unaccountedRewards = 0;

//####################################################################################################hotel toevoegen. daily gold vervangen voor Daily_miner en Daily_hotel?
    // GoldMiner earn 10000 $GGOLD per day
    uint public constant DAILY_MINER_RATE = 10000 ether;
    uint public constant DAILY_HOTEL_RATE = 30000 ether;
    uint public constant DAILY_STORE_RATE = 20000 ether;
    uint public constant DAILY_PENTHOUSE_RATE = 15000 ether;
...
//####################################################################################################get account hotels
    function getAccountHotels(address user) external view returns (Stake[] memory) {
        return hotelStake[user];
    }

    function getAccountStores(address user) external view returns (Stake[] memory) {
        return storeStake[user];
    }

    function getAccountPenthouses(address user) external view returns (Stake[] memory) {
        return penthouseStake[user];
    }
...
//####################################################################################################zelfde voor hotel?
            ...
            else if (goldHunter.isHotel(tokenIds[i])) {
                _stakeHotels(account, tokenIds[i]);
            }
            else if (goldHunter.isStore(tokenIds[i])) {
                _stakeStores(account, tokenIds[i]);
            }
            else if (goldHunter.isPenthouse(tokenIds[i])) {
                _stakePenthouses(account, tokenIds[i]);
            }
            else {
...
//####################################################################################################stakeHotel toevoegen
    function _stakeHotels(address account, uint16 tokenId) internal whenNotPaused _updateEarnings {
        totalHotelStaked += 1;

        hotelIndices[tokenId] = hotelStake[account].length;
        hotelStake[account].push(Stake({
            owner: account,
    ...
    function _stakeStores(address account, uint16 tokenId) internal whenNotPaused _updateEarnings {
        totalStoreStaked += 1;

        storeIndices[tokenId] = storeStake[account].length;
        storeStake[account].push(Stake({
            owner: account,
            tokenId: uint16(tokenId),
            value: uint80(block.timestamp)
        }));
    ...
    function _stakePenthouses(address account, uint16 tokenId) internal whenNotPaused _updateEarnings {
        totalPenthouseStaked += 1;

        penthouseIndices[tokenId] = penthouseStake[account].length;
        penthouseStake[account].push(Stake({
            owner: account,
            tokenId: uint16(tokenId),
            value: uint80(block.timestamp)
        }));
...
//####################################################################################################claimfromhotel toevoegen zoals miner?
    function claimFromStake(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings {
        uint owed = 0;
        for (uint i = 0; i < tokenIds.length; i++) {
            if (goldHunter.isHotel(tokenIds[i])) {
                owed += _claimFromHotel(tokenIds[i], unstake);
            }
            else if (goldHunter.isStore(tokenIds[i])) {
                owed += _claimFromStore(tokenIds[i], unstake);
            }
            else if (goldHunter.isPenthouse(tokenIds[i])) {
                owed += _claimFromPenthouse(tokenIds[i], unstake);
            }
            else {
...
//####################################################################################################claimfromhotel
    function _claimFromHotel(uint16 tokenId, bool unstake) internal returns (uint owed) {
        Stake memory stake = hotelStake[msg.sender][hotelIndices[tokenId]];
        ...
        if (totalGoldEarned < MAXIMUM_GLOBAL_GOLD) {
            owed = ((block.timestamp - stake.value) * DAILY_HOTEL_RATE) / 1 days;
        } else if (stake.value > lastClaimTimestamp) {
            owed = 0; // $GGOLD production stopped already
        } else {
            owed = ((lastClaimTimestamp - stake.value) * DAILY_HOTEL_RATE) / 1 days; // stop earning additional $GGOLD if it's all been earned
        }
        if (unstake) {
            ...
            Stake memory lastStake = hotelStake[msg.sender][hotelStake[msg.sender].length - 1];
            hotelStake[msg.sender][hotelIndices[tokenId]] = lastStake;
            hotelIndices[lastStake.tokenId] = hotelIndices[tokenId];
            hotelStake[msg.sender].pop();
            delete hotelIndices[tokenId];
            ...
            hotelStake[msg.sender][hotelIndices[tokenId]] = Stake({
                owner: msg.sender,
                tokenId: uint16(tokenId),
                value: timestamp
            }); // reset stake
        ...
        emit HotelClaimed(tokenId, owed, unstake);
    }    

    function _claimFromStore(uint16 tokenId, bool unstake) internal returns (uint owed) {
        Stake memory stake = storeStake[msg.sender][storeIndices[tokenId]];
        require(stake.owner == msg.sender, "This NTF does not belong to address");
        require(!(unstake && block.timestamp - stake.value < MINIMUM_TIME_TO_EXIT), "Need to wait 2 days since last claim");

        if (totalGoldEarned < MAXIMUM_GLOBAL_GOLD) {
            owed = ((block.timestamp - stake.value) * DAILY_STORE_RATE) / 1 days;
        } else if (stake.value > lastClaimTimestamp) {
            owed = 0; // $GGOLD production stopped already
        } else {
            owed = ((lastClaimTimestamp - stake.value) * DAILY_STORE_RATE) / 1 days; // stop earning additional $GGOLD if it's all been earned
        }
        if (unstake) {
            if (getSomeRandomNumber(tokenId, 100) <= 50) {
                _payTax(owed);
            ...
            Stake memory lastStake = storeStake[msg.sender][storeStake[msg.sender].length - 1];
            storeStake[msg.sender][storeIndices[tokenId]] = lastStake;
            storeIndices[lastStake.tokenId] = storeIndices[tokenId];
            storeStake[msg.sender].pop();
            delete storeIndices[tokenId];
            ...
            storeStake[msg.sender][storeIndices[tokenId]] = Stake({
                owner: msg.sender,
                tokenId: uint16(tokenId),
                value: timestamp
            }); // reset stake
        ...
        emit StoreClaimed(tokenId, owed, unstake);
    }  

    function _claimFromPenthouse(uint16 tokenId, bool unstake) internal returns (uint owed) {
        Stake memory stake = penthouseStake[msg.sender][penthouseIndices[tokenId]];
        require(stake.owner == msg.sender, "This NTF does not belong to address");
        require(!(unstake && block.timestamp - stake.value < MINIMUM_TIME_TO_EXIT), "Need to wait 2 days since last claim");

        if (totalGoldEarned < MAXIMUM_GLOBAL_GOLD) {
            owed = ((block.timestamp - stake.value) * DAILY_PENTHOUSE_RATE) / 1 days;
        } else if (stake.value > lastClaimTimestamp) {
            owed = 0; // $GGOLD production stopped already
        } else {
            owed = ((lastClaimTimestamp - stake.value) * DAILY_PENTHOUSE_RATE) / 1 days; // stop earning additional $GGOLD if it's all been earned
        }
        if (unstake) {
            if (getSomeRandomNumber(tokenId, 100) <= 50) {
                _payTax(owed);
            ...
            Stake memory lastStake = penthouseStake[msg.sender][penthouseStake[msg.sender].length - 1];
            penthouseStake[msg.sender][penthouseIndices[tokenId]] = lastStake;
            penthouseIndices[lastStake.tokenId] = penthouseIndices[tokenId];
            penthouseStake[msg.sender].pop();
            delete penthouseIndices[tokenId];
            ...
            penthouseStake[msg.sender][penthouseIndices[tokenId]] = Stake({
                owner: msg.sender,
                tokenId: uint16(tokenId),
                value: timestamp
            }); // reset stake
        ...
        emit PenthouseClaimed(tokenId, owed, unstake);
    }  
    ...
    function _claimFromMiner(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        if (totalGoldEarned < MAXIMUM_GLOBAL_GOLD) {
            owed = ((block.timestamp - stake.value) * DAILY_MINER_RATE) / 1 days;
        } else if (stake.value > lastClaimTimestamp) {
            owed = 0; // $GGOLD production stopped already
        } else {
            owed = ((lastClaimTimestamp - stake.value) * DAILY_MINER_RATE) / 1 days; // stop earning additional $GGOLD if it's all been earned
        }
        if (unstake) {
            if (getSomeRandomNumber(tokenId, 100) <= 50) {
                _payTax(owed);
...
//####################################################################################################hotel toevoegen
    modifier _updateEarnings() {
        if (totalGoldEarned < MAXIMUM_GLOBAL_GOLD) {
            ...
            * (totalHotelStaked * DAILY_HOTEL_RATE) 
            * (totalGoldMinerStaked * DAILY_MINER_RATE)
            * (totalStoreStaked * DAILY_STORE_RATE)
            * (totalPenthouseStaked * DAILY_PENTHOUSE_RATE)
            ) / 1 days;
