...
contract Marketplace is IMarketplace, Marketplace1155, Marketplace721, AccessControl {
    ...
    // TODO: rewrite all functions to be separate: 721 and 1155 (createItem721, createItem1155)
    // function createItem(TokenType tokenType, uint256 amount) external override {
    //     tokenType == TokenType.TOKEN721 ?
    //         IToken1155(token721).mint(_tokenIdTracker.current(), msg.sender, amount) :
    //         IToken721(token1155).mint(_tokenIdTracker.current(), msg.sender, amount);
    //     _tokenIdTracker.increment();
    // }

    // function listItem(uint256 tokenId, uint256 price, TokenType tokenType, uint256 amount) external override {
    //     // TODO:
    //     // check owner
    //     if (!_isOwner(tokenId, msg.sender, tokenType, amount)) revert NotOwner();
    //     // 1. add to listings
    //     _listings[tokenId] = Listing({tokenType: tokenType, owner: msg.sender, price: price});
    //     // 2. transferFrom(user, this)
    //     tokenType == TokenType.TOKEN721 ?
    //         IERC721(token721).transferFrom(msg.sender, address(this), tokenId) :
    //         IERC1155(token1155).safeTransferFrom(msg.sender, address(this), tokenId, amount, "");
    //     emit ItemListed(tokenId, price);
    // }

    // function cancelItem(uint256 tokenId, TokenType tokenType, uint256 amount) external override {
    //     if (_listings[tokenId].owner != msg.sender) revert NotOwner();
    //     tokenType == TokenType.TOKEN721 ?
    //         IERC721(token721).transferFrom(address(this), msg.sender, tokenId) :
    //         IERC1155(token1155).safeTransferFrom(address(this), msg.sender, tokenId, amount, "");
    //     delete _listings[tokenId];
    //     // TODO: emit event
    // }

    // function buylItem(uint256 tokenId, TokenType tokenType, uint256 amount) external override {
    //     if (_listings[tokenId].owner == address(0)) revert DoesNotExist();
    //     uint256 cost = _listings[tokenId].price * amount;
    //     if(IERC20(token20).balanceOf(msg.sender) < cost) revert InsufficientBalance();
        
    //     IERC20(token20).transferFrom(msg.sender, _listings[tokenId].owner, cost);
    //     tokenType == TokenType.TOKEN721 ?
    //         IERC721(token721).transferFrom(address(this), msg.sender, tokenId) :
    //         IERC1155(token1155).safeTransferFrom(address(this), msg.sender, tokenId, amount, "");
    //     delete _listings[tokenId];
    ...
    // function _isOwner(uint256 tokenId, address owner, TokenType tokenType, uint256 amount) internal view returns(bool) {
    //     return tokenType == TokenType.TOKEN721 ?
    //         IERC721(token721).ownerOf(tokenId) == owner :
    //         IERC1155(token1155).balanceOf(owner, tokenId) >= amount;
    // }
}
...
interface IMarketplace {
    ...
    // function createItem(TokenType tokenType, uint256 amount) external;
    // function listItem(uint256 tokenId, uint256 price, TokenType tokenType, uint256 amount) external;
    // function cancelItem(uint256 tokenId, TokenType tokenType, uint256 amount) external;
    // function buylItem(uint256 tokenId, TokenType tokenType, uint256 amount) external;
}
...
contract Marketplace721 is IMarketplace721, MarketplaceBase {
    using SafeERC20 for IERC20;
    ...
    // modifier auctionTime(uint256 tokenId) {
    //     // solhint-disable-next-line not-rely-on-time
    //     if (block.timestamp - _listingsAuction721[tokenId].startTimestamp < auctionDuration) revert WrongPeriod();
    //     _;
    // }


    // TODO: use no-reentrant
    ...
    function buylItem721(uint256 tokenId) external override {
        if (_listings721[tokenId].owner == address(0)) revert DoesNotExist();
        uint256 cost = _listings721[tokenId].price;
        if(IERC20(token20).balanceOf(msg.sender) < cost) revert InsufficientBalance();
        
        IERC20(token20).transferFrom(msg.sender, _listings721[tokenId].owner, cost);
        IERC721(token721).transferFrom(address(this), msg.sender, tokenId);
        delete _listings721[tokenId];
    ...
    function makeBid721(uint256 tokenId, uint256 bid) external override {
        ...
        IERC20(token20).transferFrom(msg.sender, address(this), bid);
        ...
        if (previousBidder != address(0)) {
            IERC20(token20).transfer(previousBidder,  previousBid);
        }
        
        emit Item721BiddedAuction(tokenId, msg.sender, bid);
    }

    function finishAuction721(uint256 tokenId) external override {
        ListingAuction721 memory listing =  _listingsAuction721[tokenId];
        ...
        if (listing.bidsCounter <= 2) {
            IERC721(token721).transferFrom(address(this), listing.owner, tokenId);
            emit Item721FinishedAuction(tokenId, listing.owner, listing.price);
        } else {
            IERC721(token721).transferFrom(address(this), listing.lastBidder, tokenId);
            IERC20(token20).transfer(listing.owner, listing.price);
...
contract Marketplace1155 is ERC1155Receiver, IMarketplace1155, MarketplaceBase {
    ...
    function createItem1155(uint256 tokenId, uint256 amount) external override {
    ...
    function cancelItem1155(uint256 tokenId, uint256 amount) external override {
    ...
    function buylItem1155(uint256 tokenId, uint256 amount) external override {
        if (_listings1155[tokenId].owner == address(0)) revert DoesNotExist();
        uint256 cost = _listings1155[tokenId].price * amount;
        if(ERC20(token20).balanceOf(msg.sender) < cost) revert InsufficientBalance();
        
        IERC20(token20).transferFrom(msg.sender, _listings1155[tokenId].owner, cost);
        IERC1155(token1155).safeTransferFrom(address(this), msg.sender, tokenId, amount, "");
        _listings1155[tokenId].amount -= amount;
    ...
    function makeBid1155(uint256 tokenId, uint256 bid) external override {
    ...
    function finishAuction1155(uint256 tokenId) external override {
        ListingAuction1155 memory listing =  _listingsAuction1155[tokenId];
