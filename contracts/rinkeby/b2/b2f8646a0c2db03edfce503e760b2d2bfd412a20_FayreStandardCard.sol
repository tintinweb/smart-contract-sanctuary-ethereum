...
abstract contract FayreMembershipCard721 is OwnableUpgradeable, ERC721EnumerableUpgradeable, ERC721BurnableUpgradeable {
    ...
    function multichainClaim(address to, uint256 fromTokenId, uint256 fromNetworkId, address fromContractAddress, uint256 destinationNetworkId, address destinationContractAddress, MembershipCardData calldata membershipCardData, uint8 v, bytes32 r, bytes32 s) external returns(uint256) {
        ...
        require(isValidator[ecrecover(generatedHash, v, r, s)], "The message must be signed by a validator");
        require(!_isMultichainHashProcessed[generatedHash], "Message already processed");
        require(destinationContractAddress == address(this), "Multichain destination address must be this contract");
        require(destinationNetworkId == _networkId, "Wrong destination network id");
    ...
    function __FayreMembershipCard721_init(uint256 networkId_, string memory name_, string memory symbol_, uint256 priceUSD_, uint256 startingVolume_, uint256 supplyCap_, uint256 nftPriceCap_, uint256 freeMultiAssetSwapCount_) internal onlyInitializing {
        __Ownable_init();
        ...
        __FayreMembershipCard721_init_unchained(networkId_, priceUSD_, startingVolume_, supplyCap_, nftPriceCap_, freeMultiAssetSwapCount_);
    }

    function __FayreMembershipCard721_init_unchained(uint256 networkId_, uint256 priceUSD_, uint256 startingVolume_, uint256 supplyCap_, uint256 nftPriceCap_, uint256 freeMultiAssetSwapCount_) internal onlyInitializing {
