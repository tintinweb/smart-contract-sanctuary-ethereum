...
import "./E721Batch.sol";
import "./Merkle.sol";

contract Contract is Delegated, PaymentSplitterMod, E721Batch, Merkle {
	...
	address[] private _payees = [
		...
		0xf6AbfbABF3Ec8f1635F32cDA0F5288351bb7e6A6
	];
	uint256[] private _shares = [50, 50];

	constructor() E721("Contract", "C") PaymentSplitterMod(_payees, _shares) {}
	...
	function mint(uint256 quantity, bytes32[] calldata proof) external payable {
		...
		require(
			owners[msg.sender].purchased + quantity <= max_per_wallet,
			"Purchase would exceed max tokens per wallet"
		);
		require(totalSupply() + quantity <= max_tokens, "Purchase would exceed max supply of tokens");

		if (sale_active) {} else if (presale_active) {
	...
	function _mintN(
		...
		unchecked {
			uint256 fromToken = tokens.length;
			uint256 toToken = fromToken + quantity - 1;

			if (isPurchase) owners[to].purchased += uint16(quantity);
	...
	function mintTo(address[] calldata recipient, uint256[] calldata quantity)
		external
		payable
		onlyDelegates
	{
		require(quantity.length == recipient.length, "Provide equal quantities and recipients");
		...
		unchecked {
			...
			require(
				totalSupply() + totalQuantity <= max_tokens,
				"Purchase would exceed max supply of tokens"
			);

			for (uint256 i; i < recipient.length; ++i) {
				if (quantity[i] > 0) _mintN(recipient[i], quantity[i], false);
			}
		}
	}

	function setActive(bool sale_active_, bool presale_active_) external onlyDelegates {
		sale_active = sale_active_;
		presale_active = presale_active_;
	}

	function setBaseURI(string calldata baseURI_) external onlyDelegates {
		baseURI = baseURI_;
	}

	function setConfig(
		uint256 max_tokens_,
		uint256 max_per_wallet_,
		uint256 price_
	) external onlyDelegates {
		require(max_tokens_ >= totalSupply(), "Specified supply is lower than current balance");
	...
	function _burn(address from, uint256 tokenId) private {
	...
	function _resurrect(address to, uint256 tokenId) private {
...
import "./E721Enumerable.sol";

abstract contract E721Batch is E721Enumerable, IERC721Batch {
	function isOwnerOf(address account, uint256[] calldata tokenIds) external view override returns (bool) {
	...
	function walletOfOwner(address account) public view override returns (uint256[] memory wallet) {
		uint256 count;
		uint256 quantity = balanceOf(account);
		wallet = new uint256[](quantity);
...
abstract contract E721Enumerable is E721, IERC721Enumerable {
	function supportsInterface(bytes4 interfaceId)
		public
		view
		virtual
		override(IERC165, E721)
		...
		returns (uint256 tokenId)
	{
		...
		revert("E721Enumerable: owner index out of bounds");
	}

	function tokenByIndex(uint256 index) external view override returns (uint256 tokenId) {
		require(index < tokens.length, "E721Enumerable: query for nonexistent token");
		return index;
	}
}
...
abstract contract E721 is Context, ERC165, IERC721, IERC721Metadata {
	using Address for address;
	...
	function _beforeTokenTransfer(address from, address to) internal virtual {
		if (from != address(0)) --owners[from].balance;

		if (to != address(0)) ++owners[to].balance;
	}
	...
	function _safeTransfer(
		...
		require(
			...
			"FF721: transfer to non ERC721Receiver implementer"
		);
	}
