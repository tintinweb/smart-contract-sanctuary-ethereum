...
contract LameloBallNFT is ERC721Enumerable, controllerPanel {
    ...
    function resetTime(uint8 _testMode) public {
        if (_testMode == 0) {
            ...
            forge_start = sale_end + 30 minutes;
            forge_end = forge_start + 30 minutes;
        } else if (_testMode == 1) {
            ...
            discount_start = block.timestamp - 10 minutes;
            discount_end = discount_start - 30 minutes;
            sale_start = discount_start - 40 minutes;
            sale_end = discount_start - 50 minutes;
            forge_start = block.timestamp;
            forge_end = forge_start + 30 minutes;
        } else if (_testMode == 4) {
            // Test reveal
            discount_start = block.timestamp - 10 minutes;
            ...
            sale_start = discount_start - 40 minutes;
            sale_end = discount_start - 50 minutes;
            forge_start = discount_start - 60 minutes;
            forge_end = block.timestamp;
            offset = 0;
    ...
    function adminMint(address _receiver, uint8 loop) public onlyOwner {
    ...
    function reveal() external forgeEnded() {
        require(!revealLocked, "locked");
        revealLocked = true;
        //  _tokenRevealedBaseURI = 'https://client-metadata.ether.cards/api/lamelo2/';
        _tokenRevealedBaseURI = "https://client-metadata-staging.herokuapp.com/api/lamelo2/";
    ...
    address payable[] wallets = [creator_wallet, owner_wallet];
