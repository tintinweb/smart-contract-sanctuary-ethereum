...
contract Minter is Ownable {
    ...
    uint256 public nextTokenId = 1210;
    uint256 public maxWhitelistAllocation = 10;
    ...
    /// @dev this is a superset of holderMarkleRoot
    ...
    /// @param quantityAllowed number alloted in HoldersOnly phase list. this won't
    /// be used if we're not in the HoldersOnly phase.
    function proxyMint(
        ...
        if (mintPhase == MintPhase.HoldersOnly) {
            // check merkle proof. this time we also check the max quantity allowed
            // to see if the number they gave us matches the merkle root.
            require(checkHolderMerkleProof(merkleProof, msg.sender, quantityAllowed), "Invalid holder proof");

            // make sure user won't have already minted max HolderOnly amount
            require(hasMintedHolderAllocation[msg.sender] + quantity <= quantityAllowed, "Exceeds holder allocation");
        ...
        if (mintPhase == MintPhase.WhitelistOnly) {
            ...
            // make sure user is in whitelist merkle proof
            require(checkWhitelistMerkleProof(merkleProof, msg.sender), "Invalid proof");
    ...
    /// @dev IMPORTANT: make sure all holder addresses are also in this list
    function checkWhitelistMerkleProof(
        bytes32[] calldata merkleProof,
        address _address
    ...
    /// @notice check whether the merkleProof is valid for a given address and quantity
    function checkHolderMerkleProof(
        bytes32[] calldata merkleProof,
        address _address,
        uint256 quantityAllowed
    ) public view returns (bool) {
        bytes32 leaf = keccak256(abi.encodePacked(_address, quantityAllowed));
        return MerkleProof.verify(merkleProof, holderMerkleRoot, leaf);
    }
