...
contract Starbirds is ERC721A, Ownable {
    ...
    function mint(uint256 _amount) external payable {
        ...
        require(
            ...
            "Homegirls - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "Homegirls - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "Homegirls - max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "Homegirls - insufficient ethers"
            );
        }
