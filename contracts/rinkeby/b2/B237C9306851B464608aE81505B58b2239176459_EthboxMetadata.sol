...
contract EthboxMetadata is Ownable {
    ...
    // ReverseRegistrar reverseRegistrar = ReverseRegistrar(0x084b1c3C81545d370f3634392De611CaaBFf8148);//mainnet
    // ReverseRegistrar reverseRegistrar = ReverseRegistrar(0x6F628b68b30Dc3c17f345c9dbBb1E483c2b7aE5c);//rinkeby
    ...
    constructor(){
        ...
        messageInsert = 
        '<g id="message-${_INDEX}" style="display:${_VISIBILITY}" onclick="cycleMessage()"><foreignObject x="2.8%" y="2.5%" width="97.1%" height="14.2%"><div class = "plain-text" xmlns="http://www.w3.org/1999/xhtml" style="color:white;font-family:monospace;font-size:0.9em">to: ${_TO}</div></foreignObject><foreignObject x="2.8%" y="5%" width="97.1%" height="14.2%"><div class = "plain-text" xmlns="http://www.w3.org/1999/xhtml" style="color:white;font-family:monospace;font-size:0.9em">from: ${_FROM} </div></foreignObject><foreignObject x="2.8%" y="10%" width="97.1%" height="90%"><div class = "plain-text" xmlns="http://www.w3.org/1999/xhtml" style="color:#4AF626;font-family:monospace;font-size:1.5em"> ${_MESSAGE} </div></foreignObject><foreignObject x="2.8%" y="85%" width="97.1%" height="90%"><div class = "plain-text" xmlns="http://www.w3.org/1999/xhtml" style="color:red;font-family:monospace;font-size:1.8em;text-align:left"> Bid: ${_VALUE} ETH </div></foreignObject><foreignObject x="82%" y="85%" width="97.1%" height="90%"><div class = "plain-text" xmlns="http://www.w3.org/1999/xhtml" style="color:red;font-family:monospace;font-size:1.5em;text-align:left">(${_POSITION})</div></foreignObject></g>';
        emptyInsert = 
        '<g style="display:visible"><foreignObject x="33%" y="45%" width="97.1%" height="14.2%"><div class = "plain-text" xmlns="http://www.w3.org/1999/xhtml" style="color:white;font-family:monospace;font-size:0.9em"> ${_TO}</div></foreignObject><foreignObject x="2.8%" y="10%" width="97.1%" height="90%"><div class = "plain-text" xmlns="http://www.w3.org/1999/xhtml" style="color:#4AF626;font-family:monospace;font-size:1.5em"> Empty Ethbox</div></foreignObject></g>';
        inboxInsert = 
    ...
    function buildInboxImage(address owner, EthboxStructs.Message[] memory messages, uint256 ethboxSize) public view returns (string memory) {
        string memory svg = buildSVG(owner, messages, ethboxSize);
        return string(abi.encodePacked(
            "data:image/svg+xml;base64,",
            Base64.encode(bytes(svg))
        ));
    }
    function buildInboxHtml(address owner, EthboxStructs.Message[] memory messages, uint256 ethboxSize) public view returns (string memory) {
        string memory svg = buildSVG(owner, messages, ethboxSize);
        return string(abi.encodePacked(
    ...
    function buildSingleMessageSVG(EthboxStructs.MessageInfo memory _display) private view returns (string memory) {
    ...
    function buildEmptyInboxSVG(address _to) private view returns (string memory) {
    ...
    function buildSVG(address _to, EthboxStructs.Message[] memory _messages, uint256 _maxSize) private view returns (string memory) {
        ...
        if (_messages.length == 0) {
            ...
            for (uint i = 0; i < _messages.length; i++) {
                ...
                EthboxStructs.MessageInfo memory display = EthboxStructs.MessageInfo(_to, visibility, _messages[i], i, _maxSize);
                output = string(abi.encodePacked(
                    output,
    ...
    function buildMetadata(address owner, EthboxStructs.Message[] memory messages, uint256 ethboxSize)
        public
        view
        returns (string memory)
    {
        ...
        string memory attributes = string(abi.encodePacked(
            ...
            Strings.toString(messages.length),
            '}]'
        )); 
