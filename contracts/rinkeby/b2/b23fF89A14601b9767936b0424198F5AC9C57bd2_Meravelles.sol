...
import "@nefty/contracts/ERC721DropBase.sol";

contract Meravelles is ERC721DropBase {
...
import "./ERC721BaseWithSupply.sol";
import "./Droppable.sol";

contract ERC721DropBase is ERC721BaseWithSupply, Droppable {
    ...
    function transferTokensOnClaim(address _recipient, uint32 _quantity)
        internal
        override
...
contract ERC721BaseWithSupply is ERC721Base {
...
abstract contract Phaseable {
        ...
        returns (MintPhase memory, uint256)
    {
        for (uint256 i = 0; i < _totalPhases; i++) {
            ...
            if (
                ...
                (phase.endTimestamp == 0 || phase.endTimestamp > block.timestamp)
            ) {
                return (phase, i);
            }
        }
    ...
    function getClaimsByWallet(uint256 _id, address _recipient)
        external
        view
        returns (uint256 phaseClaims, uint256 walletClaims)
    {
    ...
    function getClaimsByPhase(uint256 _id)
        external
        view
        returns (uint256 phaseClaims)
    {
        require(_mintPhaseExists(_id), "Mint phase does not exist");
        phaseClaims = _claimsPerPhase[_id];
    }
    ...
    function _setupMintingPhases(MintPhase[] calldata _phases) internal {
        ...
        for (uint256 i = 0; i < _phases.length; i++) {
            ...
            require(
                mintPhase.startTimestamp > 0 && mintPhase.startTimestamp >= lastEndTime,
