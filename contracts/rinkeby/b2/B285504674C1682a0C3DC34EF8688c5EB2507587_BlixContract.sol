...
import "./BlixProxy.sol";

contract BlixContract is NonblockingLzApp {
    address public endpointAddress;
    BlixProxy[] public proxies;

    constructor(address _lzEndpoint) NonblockingLzApp(_lzEndpoint) {
        endpointAddress = _lzEndpoint;
    }

    event CreatedProxy(uint16 _srcChainId, bytes _srcAddress, address _address);
    event CreatedProxyTest(uint16 _srcChainId);

    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal override {
        BlixProxy proxy = new BlixProxy(endpointAddress, _srcChainId, _srcAddress);
        emit CreatedProxyTest(_srcChainId);
        proxies.push(proxy);
        emit CreatedProxy(_srcChainId, _srcAddress, address(proxy));
    }

    function createProxy(uint16 _dstChainId) public payable {
...
contract BlixProxy is NonblockingLzApp {
    ...
    event SendingMessage(uint16 dstChainId, bytes dstAddress, bytes payload);

    constructor(address _lzEndpoint, uint16 _srcChainId, bytes memory _srcAddress) NonblockingLzApp(_lzEndpoint) {
    ...
    function sendMessage(uint16 _dstChainId, bytes memory _dstAddress, bytes memory _payload) public {
        emit SendingMessage(_dstChainId, _dstAddress, _payload);
        _lzSendFull(_dstChainId, _dstAddress, _payload, payable(msg.sender), address(0x0), bytes(""));
    }
...
abstract contract LzApp is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function _lzSendFull(uint16 _dstChainId, bytes memory _dstAddress, bytes memory _payload, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams) internal {
