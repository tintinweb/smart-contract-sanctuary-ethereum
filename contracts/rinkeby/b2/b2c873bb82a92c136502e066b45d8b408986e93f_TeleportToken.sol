...
contract TeleportToken is ERC20Interface, Owned, Oracled, Verify {
    ...
    // Constructor
    // ------------------------------------------------------------------------
    constructor(string memory symbol_, string memory name_, uint8 decimals_, uint256 totalSupply_, uint8 threshold_, uint8 chainId_) public {
        symbol = symbol_;
        name = name_;
        decimals = decimals_;
        _totalSupply = totalSupply_;
        balances[address(0)] = totalSupply_;
    ...
    function mint(address to, uint256 tokensToMint) public onlyOwner {
        require(tokensToMint <= balances[address(0)], "Too much tokens to mint");
        require(to != address(0), "Minting to zero address is prohibited");

        balances[address(0)] = balances[address(0)].sub(tokensToMint);
        balances[to] = balances[to].add(tokensToMint);
    }
