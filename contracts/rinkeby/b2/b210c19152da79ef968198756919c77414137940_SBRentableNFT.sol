...
error SBRentableNFT_MaxBulkMintQuantityExceeded();
error SBRentableNFT_TokenIdDoesNotExist();

contract SBRentableNFT is ERC4907A, Ownable {
  uint256 public constant MAX_BULK_MINT_QUANTITY_PER_TRANSACTION = 100;
  string public s_nftIconUri;

  constructor(string memory name_, string memory symbol_) ERC721A(name_, symbol_) {
    s_nftIconUri = "https://ipfs.io/ipfs/QmRRM8Cebh1YuzQGzBXrECjME2EFZrEw5Q4Umjqk2Qi5Bv";
  }

  function setNFTIconUri(string memory _iconUri) public onlyOwner {
    s_nftIconUri = _iconUri;
  }

  function mintRentableNFTs(uint256 _quantity) external onlyOwner {
    if (_quantity > MAX_BULK_MINT_QUANTITY_PER_TRANSACTION) revert SBRentableNFT_MaxBulkMintQuantityExceeded();
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    if (tokenId < totalSupply()) 
      ...
      revert SBRentableNFT_TokenIdDoesNotExist();
  }
}
