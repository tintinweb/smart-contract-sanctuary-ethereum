...
contract AlooArtMinter {
    event AlooArtMinted(address sender, uint256 tokenId);
    uint256 constant COST_OF_ART = 0.001 ether;

    address payable public owner;
    FakeERC721 public alooERC721;
    constructor() {
        owner = payable(msg.sender);
        alooERC721 = new FakeERC721("aloo", "ALOO");
    }
    
    function mint() external payable {
        require(msg.value == COST_OF_ART);
        uint256 tokenId = alooERC721.mint(msg.sender);
        emit AlooArtMinted(msg.sender, tokenId);
