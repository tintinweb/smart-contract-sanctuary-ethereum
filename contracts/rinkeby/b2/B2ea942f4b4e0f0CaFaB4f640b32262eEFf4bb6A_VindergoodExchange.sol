...
contract ExchangeMain is ReentrancyGuarded, Ownable {
    ...
    function _validatePartialSellingAmount(
        ...
        if (isMint) {
            assembly {
                ...
                buyAmount := mload(add(buyCallData, add(mul(0x20, 3), 0x04)))
                sellAmount := mload(add(sellCallData, add(mul(0x20, 3), 0x04)))
            }
    ...
    function atomicMatch(
        ...
        if (signature == ERC1155_SIGNATURE || signature == 0x280f4e28) {
            (soldOut, amount) = _validatePartialSellingAmount(
                buy,
                sell,
                signature == 0x280f4e28
            );
        }
        ...
        bytes memory returnData = makeStaticCall(
            sell,
            (signature == 0x449a52f8 || signature == 0x280f4e28)
        );
