...
contract IDNTTS is ERC721SLockablePermittable, Ownable, ERC2981ContractWideRoyalties {  
    ...
    constructor(string memory _myBase, string memory _unrevBase) ERC721S("IDNTTS", "IDS") {
            baseURI = _myBase; 
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (revealState) {
            string memory lockState = (getLocked(tokenId) == address(0)) ? "unlocked/" : "locked/";
            return string(abi.encodePacked(baseURI, lockState, tokenId.toString(), metadataExtension));
        } else {
