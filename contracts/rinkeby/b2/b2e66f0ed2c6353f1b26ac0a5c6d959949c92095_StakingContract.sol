...
contract StakingContract is Ownable {
    ...
    address public nftAddress = 0x0D920ec7219119A96bFf2Fd494245c28Db1B576C;
    ...
    function stake(uint256[] memory tokenIds, uint32 _lockTime) public {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            IERC721(nftAddress).approve(address(this), tokenIds[i]);
            ...
            if(_lockTime == 1)
            info.lockTime = 5 minutes;
            else
            info.lockTime = 10 minutes;
    ...
    function unstake(uint256[] memory tokenIds) public {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(!_isStaked) continue;
            if(block.timestamp >= (_startTime + _lockTime)) continue;
    ...
    function getStakingItemInfo(address _user, uint256 _tokenId) public view returns(bool _isStaked, uint _startTime, uint _lockTime) {
        for(uint256 i = 0; i < userInfo[_user].length; i++) {
            if(userInfo[_user][i].tokenId == _tokenId) {
                ...
                _lockTime = userInfo[_user][i].lockTime;
                break;
            }
        }
    }
