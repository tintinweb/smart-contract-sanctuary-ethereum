...
contract ForeverApesRise is ERC721Enumerable, Ownable {
    ...
    uint256 public preSaleLimit = 50;
...
*/
    function whitelist(address[] memory _users, uint8 numAllowedToMint)
        public
        onlyOwner
    {
...
*/
    ...
    function preSaleMint(uint256 quantity) public payable {
        ...
        require(quantity <= whileList[msg.sender],"Exceeded max available to purchase");
        require(super.totalSupply() + quantity <= preSaleLimit,"Ran our of presale Mints");
    ...
    function whitelistQuanityAvailable(address user)external view returns (uint256){
    ...
    function mint(uint256 quantity) public payable {
        ...
        require(super.totalSupply() + quantity <= maxSupply,"Cannot exceed max supply");
        require(quantity * mintPrice <= msg.value, "Not enough ether sent");
        for (uint256 i = 0; i < quantity; i++) {
            _safeMint(msg.sender, _tokenIDs.current());
            redeemed[_tokenIDs.current()] = false;
        ...
        if ( startingIndexBlock == 0 &&(totalSupply() == maxSupply || block.timestamp >= start_index_time)) {
