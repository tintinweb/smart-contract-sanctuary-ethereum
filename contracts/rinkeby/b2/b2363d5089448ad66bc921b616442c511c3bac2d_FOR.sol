...
contract FOR is ERC721URIStorage, Ownable {
    ...
    bytes32 private _whitelistMerkleRoot = 0x4f0a4dfc85667e30c6f8ad3c1698ece1314a4c26d2b1d1691cafe0c4de21dc2b;

    string private _baseURIextended = "ipfs://QmVNQaYgiC1EgS9LkJX16c2vLoVSqAfcPdcqJRAxEbt2La/"; // This one is used on OpenSea to define the metadata IPFS address
    ...
    function M_I_N_T(bytes32[] memory proof,uint256 amount, uint8 tokenID) public {
        ...
        if(whitelist_mint) {
            ...
            _whitelistMerkleRoot,
             keccak256(abi.encodePacked(msg.sender, amount))), "Markle tree verification failed, not whitelisted");
        }

        require(!mint_paused, "Mint is Paused");

        require(alreadyOwnCan[msg.sender] < 3, "Can only mint one collab can and one community can");

        if(tokenID > 4) {
            require(alreadyOwnCan[msg.sender] != 2, "Can only mint one collab can");

        } else {
            require(alreadyOwnCan[msg.sender] != 1, "Can only mint one community can");
        }

        // TODO somehow allow people to mint


        // in case some of the randon cans sold out, you can still get a can without spending another transaction gas fee
        // but now instead of randon its from 0 to 4
        ...
        // Rekt code sorry guys, no time to do math 
