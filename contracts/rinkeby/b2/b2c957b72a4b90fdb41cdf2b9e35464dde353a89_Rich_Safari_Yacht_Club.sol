...
contract Rich_Safari_Yacht_Club  is ERC721Enumerable, Ownable
{
    ...
    uint public wl_mint_price= 0.5 ether;
    bool public iswlmintPaused = true;

    uint public early_mint_price= 0.5 ether;
    bool public isearlymintPaused = true;
    ...
    constructor() ERC721("Rich Safari Yacht Club", "RSYC")  {
    _uriBeforeRevel = "https://gateway.pinata.cloud/ipfs/QmTkzjbmMj14xAPzA3UeJcPgpbA3DEtqv3gGxRpszbRCyw";
    }
    ...
    function flipwl_mintStatus() public onlyOwner {
        iswlmintPaused = !iswlmintPaused;
    }
    function flippre_mintStatus() public onlyOwner {
        isearlymintPaused = !isearlymintPaused;
    }
 ...
 function changeRevelStatus() external onlyOwner {
    ...
    function Giveaway(uint chosenAmount, address winner_address) public onlyOwner{
                ...
                _safeMint(winner_address, totalsupply());
                tokenId++;
                }

    }

    function Earlyaccess_mint(uint chosenAmount) public payable isSaleOpen{

                require(isearlymintPaused == false,"Quantity must be lesser then MaxSupply");            
                require(totalSupply()+chosenAmount<=_TOTALSUPPLY-reserve,"Quantity must be lesser then MaxSupply");
                require(chosenAmount > 0, "Number of tokens can not be less than or equal to 0");
                require(chosenAmount <= maxQuantity,"Chosen Amount exceeds MaxQuantity");
                require(early_mint_price.mul(chosenAmount) == msg.value, "Sent ether value is incorrect");
    ...
    function whitelist_mint(uint chosenAmount) public payable isSaleOpen{
                ...
                require(iswlmintPaused == false,"Quantity must be lesser then MaxSupply");            
                require(totalSupply()+chosenAmount<=_TOTALSUPPLY-reserve,"Quantity must be lesser then MaxSupply");
                require(chosenAmount > 0, "Number of tokens can not be less than or equal to 0");
                require(chosenAmount <= maxQuantity,"Chosen Amount exceeds MaxQuantity");
     ...
     function mint(uint chosenAmount) public payable isSaleOpen{
                ...
                require(ismintPaused == false,"Quantity must be lesser then MaxSupply");
                require(totalSupply()+chosenAmount<=_TOTALSUPPLY-reserve,"Quantity must be lesser then MaxSupply");
                require(chosenAmount > 0, "Number of tokens can not be less than or equal to 0");
                require(chosenAmount <= maxQuantity,"Chosen Amount exceeds MaxQuantity");
