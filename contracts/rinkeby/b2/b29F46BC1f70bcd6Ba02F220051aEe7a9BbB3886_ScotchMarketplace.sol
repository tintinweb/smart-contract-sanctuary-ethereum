...
contract ScotchMarketplace is ScotchBeneficiary, ScotchLogger, ReentrancyGuard {
  ...
  bool public _opAllowance = true;

  bool public _opSafeTransferFrom = true;

  bool public _opTransfer = true;
  ...
  function setOptions(bool allowance, bool safeTransferFrom, bool transfer) public  {

    _opAllowance = allowance;

    _opSafeTransferFrom = safeTransferFrom;

    _opTransfer = transfer;
  ...
  private returns (uint256) {
    ...
    _log(string.concat("PriceAmount=", Strings.toString(priceAmount), "; FeeAmount=", Strings.toString(feeAmount), "; SellerAmount=", Strings.toString(sellerAmount)));
...

    {
      ...
      if (_opAllowance){
        ...
        _log(string.concat("Allowance=", Strings.toString(priceAmount)));

      }



      if (_opSafeTransferFrom){

      _log("before safeTransferFrom buyer to marketplace");
      ...
      _log("before transfer to seller");
