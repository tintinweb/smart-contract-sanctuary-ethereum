...
// File: contracts/KimiSwapPair.sol
...
contract KimiSwapPair is ERC20, Math {
    ...
    modifier lock() {
        require(!isEntered, "KimiSwap: You cannot Enter in the contract");
        isEntered = true;
        _;
        isEntered = false;
    }
    ...
    constructor() ERC20("KimiSwap Pair", "KIMI-LP", 18) {
        factory = msg.sender;
    ...
    function initialize(address token0_, address token1_) public {
        ...
        require(msg.sender == factory, "KimiSwap: FORBIDDEN"); // sufficient check
    ...
    function mint(address to) public lock returns (uint256 liquidity) {
        ...
        require(liquidity > 0, "KimiSwap: INSUFFICIENT_LIQUIDITY_MINTED");
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "KimiSwap: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        require(
            ...
            "KimiSwap: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        ...
        require(
            ...
            "KimiSwap: INSUFFICIENT_LIQUIDITY"
        );
        ...
        require(
            ...
            "KimiSwap: INSUFFICIENT_INPUT_AMOUNT"
        );
    ...
    function _safeTransfer(
        ...
        require(
            ...
            "KimiSwap: TRANSFER_FAILED"
        );
    }
}

// File: contracts/KimiSwapFactory.sol

pragma solidity >=0.8.10;



contract KimiSwapFactory {

    mapping(address => mapping(address => address)) public pairs;
    address[] public allPairs;

    bytes32 public constant INIT_CODE_HASH =
        keccak256(abi.encodePacked(type(KimiSwapPair).creationCode));
    ...
    {
        require(tokenA != tokenB, "KimiSwap: IDENTICAL_ADDRESSES");
        (address token0, address token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "KimiSwap: ZERO_ADDRESS");
        require(pairs[token0][token1] == address(0), "KimiSwap: PAIR_EXISTS");

        bytes memory bytecode = type(KimiSwapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt) 
        }

        IKimiSwapPair(pair).initialize(token0, token1);
