...
// File: contracts/IDOGETLD.sol


pragma solidity ^0.8.4;


interface IDOGETLD is IERC721 {
...
// File: contracts/DOGEDomains.sol
...
contract DOGEDomains is Ownable, ReentrancyGuard {
  address[] public supportedNfts; // whitelisted dogeverse NFT addresses
  bool public paused = true;

  uint256 public price; // domain price in $wdoge
  uint256 public royaltyFee = 2_000; // share of each domain purchase (in bips) that goes to DOGE Domains
  ...
  // TLD contract (.doge)
  IDOGETLD public immutable tldContract;
  ...
  constructor(
    ...
    address _tldAddress,
    address _wdogeAddress,
    uint256 _price
  ) {
    supportedNfts.push(_nftAddress);
    tldContract = IDOGETLD(_tldAddress);
    wdoge = IERC20(_wdogeAddress);
    price = _price;
  }

  // READ

  /// @notice Returns true or false if address is eligible to mint a .doge domain
  ...
  /// @notice A $wdoge approval transaction is needed to be made before minting
  function mint(
    string memory _domainName,
    address _domainHolder,
    address _referrer
    ...
    wdoge.transferFrom(msg.sender, tldContract.getFactoryOwner(), royaltyPayment);

    // send referral fee
    if (referralFee > 0 && _referrer != address(0)) {
      uint256 referralPayment = (price * referralFee) / MAX_BPS;
      ownerPayment -= referralPayment;
      wdoge.transferFrom(msg.sender, _referrer, referralPayment);
    }

    // send the rest to the owner
    wdoge.transferFrom(msg.sender, owner(), ownerPayment);
  ...
  /// @notice Transfer .doge TLD ownership to another address
