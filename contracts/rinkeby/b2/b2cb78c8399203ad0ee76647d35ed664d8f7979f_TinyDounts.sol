...
contract TinyDounts is ERC721A, Ownable {
	using Strings for uint256;
	enum SaleStatus {
		PAUSED,
		PUBLIC
	...
	constructor(address beneficiary_) ERC721A('TinyDonuts', 'TD') {
        BENEFICIARY = beneficiary_;
    }

	function contractURI() public pure returns (string memory) {
		return
			'ipfs://QmdfZJW5F1uA6cf7YMhaKWp14hpbKtcXqFuTm6VmW76aVQ';
	}
	...
	function mint(uint256 count) external payable {
		require(saleStatus != SaleStatus.PAUSED, 'TinyDonuts: Sales are off');
		require(
			count <= TOKENS_PER_TRAN_LIMIT,
			'TinyDonuts: Number of requested tokens exceeds allowance (10)'
		);
		require(
			_totalMinted() + count <= COLLECTION_SIZE,
			'TinyDonuts: Number of requested tokens will exceed collection size'
		);
		...
		require(
			...
			'TinyDonuts: Ether value sent is not sufficient'
		);
