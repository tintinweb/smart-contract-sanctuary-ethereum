...
contract HarukaToken is Ownable, ERC20Burnable {    
    ...
    uint256 public emissionRate = uint(100 * (10 ** 18)) / 86400;

    // 24 April 2022 @ 12:00:00 GMT
    uint256 public emissionStart = 1650758400;
    ...
    */
    ...
    function collectChibi(uint256 _tokenToKeep, uint256 _tokenToBurn) public noPauseNoContract() {
        require(ERC721A(harukaRonin).ownerOf(_tokenToKeep) == msg.sender, "Must own tokenToKeep");
        require(ERC721A(harukaRonin).ownerOf(_tokenToBurn) == msg.sender, "Must own tokenToBurn");

        ERC721A(harukaRonin).safeTransferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, _tokenToBurn);

        tokens[_tokenToKeep].emissionModifier = tokens[_tokenToBurn].emissionModifier + tokens[_tokenToKeep].emissionModifier;
    }
