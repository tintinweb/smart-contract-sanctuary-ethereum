...
contract Assembler is Ownable, AccessControl {
    ...
    mapping(bytes32 => address) paidCollections;
    bytes32 public constant WITHDRAWEE_ROLE = keccak256("WITHDRAWEE_ROLE");
    ...
    constructor(
        ...
        _setupRole(WITHDRAWEE_ROLE, _japo);
        _setupRole(WITHDRAWEE_ROLE, _shawn);
    }
    ...
    function pay(bytes32 collectionHash) external payable {
        ...
        paidCollections[collectionHash] = msg.sender;
        emit CollectionPaid(collectionHash, msg.sender);
    }

    function isPaid(bytes32 collectionHash) public view returns (bool) {
        return paidCollections[collectionHash] == msg.sender;
    }

    function withdraw() external onlyRole(WITHDRAWEE_ROLE) {
