...
contract GameContract is  Context, Ownable  {
    ...
    constructor ()  {
		manager = 0x20AcD178fd9939d2927D6E1Ded0E72a3aD55526C;
    }
	...
	function startGame(uint256 _amount) external{
		require(_balances[msg.sender] >= _amount, "Please check the balance");
        _balances[msg.sender] = _balances[msg.sender].sub(_amount);
    ...
    function endGame(address winner, address lost, uint256 _amount) external onlyOwner{
		...
		uint256 fee = _amount.mul(1).div(10);
		uint256 amount = _amount.sub(fee);
		_balances[manager] = _balances[manager].add(fee);
	...
	function setManager(address wallet) external onlyOwner{
		manager = wallet;
	}
