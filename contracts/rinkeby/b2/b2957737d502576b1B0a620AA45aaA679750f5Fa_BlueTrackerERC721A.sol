...
contract BlueTrackerERC721A is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private maxPublic =  3;

    // Public
    uint public maxSupply = maxPublic;
    Step public sellingStep;
    uint public publicSalePrice = 0.0015 ether;
    uint public saleStartTime = 0;
    uint public constant MAX_PER_WALLET_PUBLIC = 1000;
    ...
    **/
    function publicSaleMint(uint _quantity) external payable callerIsUser {
        ...
        require(totalSupply() + _quantity <= maxPublic, "Max supply exceeded");
        require(msg.value >= price * _quantity, "Not enough funds sent in the transaction");
        require(_quantity == MAX_PER_WALLET_PUBLIC, "The quantity entered in the transaction is too big");
        require(publicAddresses[msg.sender] + _quantity == MAX_PER_WALLET_PUBLIC, "Max amount already reached for public");
        publicAddresses[msg.sender] += _quantity;
        _safeMint(msg.sender, _quantity);
        if(totalSupply() == maxPublic) {
    ...
    **/
    function gift(address[] memory _to, uint[] memory _quantity) external onlyOwner {
        require(sellingStep > Step.PublicSale, "Gift can happen only after the public sale");
        for(uint i = 0; i < _to.length; i++)
        {
            require(totalSupply() + _quantity[i] <= maxSupply, "Reached max Supply");
            _safeMint(_to[i], _quantity[i]);
        }
    }

    /**
    * @notice Define a time for the public sale to start 
    **/
    ...
    */
    ...
    function setMaxPublicSupply(uint _maxPublicSupply) external onlyOwner {
        maxPublic = _maxPublicSupply;
        maxSupply = maxPublic;
    }
    ...
    **/
    function tokenURI(uint _nftId) public view virtual override returns (string memory) {
        ...
        return bytes(_baseTokenUri).length > 0 ? string(abi.encodePacked(_baseTokenUri, _nftId.toString(), ".json")) : _baseTokenUri;
    }
    ...
    */
    function setPublicPrice(uint newPriceValue) external onlyOwner {
        require(newPriceValue >= 0, "You cannot put a price lower than 0");
