...
contract MetaborgDistributionERC1155 is ERC1155Upgradeable {
    ...
    function batchDistribution(uint _mangaDistributionID, uint _mangaVersion, address[] memory _addressList, uint[] memory _quantityList) public returns(bool){
        ...
        if(_mangaVersion == uint(mangaVersion.ORIGINAL)) {
            ...
            mangaDistribution[_mangaDistributionID].originalAssigned = assigned.add(_addressList.length);
        }
        if(_mangaVersion == uint(mangaVersion.GOLD)) {
            ...
            mangaDistribution[_mangaDistributionID].goldAssigned = assigned.add(_addressList.length);
        }
        if(_mangaVersion == uint(mangaVersion.DIAMOND)) {
            ...
            mangaDistribution[_mangaDistributionID].diamondAssigned = assigned.add(_addressList.length);
        }
        require(_addressList.length <= supply.sub(assigned), "NOT_ENOUGH_MANGA");
        uint allowedMint;
        ...
        for(uint index = uint(0); index < _addressList.length; index++){
            ...
            _mint(_addressList[index], tmpERC1155MangaIDsArray[_mangaVersion], _quantityList[index], bytes("0"));
        }
        return true;
    }
