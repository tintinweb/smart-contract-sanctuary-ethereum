...
contract TradeCoinERC721 is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, RoleControl {
    ...
    event productPending(address indexed creator, uint indexed _tokenId);
    ...
    event productBurned(address indexed productConsumer, uint indexed _tokenId);
    ...
    modifier notAtPendingCreation(State _state, uint tokenId) {
    ...
    function initialTokenization(
        ...
        require(keccak256(abi.encodePacked(rootHash)) != keccak256(abi.encodePacked("")), 
        ...
        // Store data on-chain
        tradeCoin[id] = TradeCoin(commodity, weight, state, _msgSender(), basicTransformation, rootHash);
    ...
    function initiateCommercialTx(
        ...
        emit offering(_tokenId, _msgSender(), _docHash, _docType, _rootHash, payInFiat);
    }
    ...
    function addTransformation(
        ...
        emit transformation(_msgSender(), _tokenId, _docHash, _docType, _rootHash, newWeight, _transformationCode);
    }
    ...
    function changeStateAndHandler(
        ...
        emit handlerAndStateUpdate(_tokenId, _msgSender(), _docHash, _docType, _rootHash, _newState, _newCurrentHandler);
    }
    ...
    ) public onlyNFTOwner(_msgSender(), _tokenId) notAtPendingCreation(State.PendingCreation, _tokenId) {
    ...
    function batchProduct(
        ...
        for(uint tokenId; tokenId < _tokenIds.length; tokenId++){
            ...
            require(hashed == keccak256(abi.encode(
                ...
                tradeCoin[_tokenIds[tokenId]].rootHash
            )), 
    ...
    function mintUniqueProductBatch(
        ...
        _safeMint(_msgSender(), id);
        // Store data on-chain
        tradeCoin[id] = TradeCoin(_commodity, _weight, _state, currentHandler, transformations, rootHash);
    ...
    function updateRootHash(uint _tokenId, string memory _docHash, string memory _rootHash) public onlyInformationHandlerOrAdmin {
        tradeCoin[_tokenId].rootHash = _rootHash;
