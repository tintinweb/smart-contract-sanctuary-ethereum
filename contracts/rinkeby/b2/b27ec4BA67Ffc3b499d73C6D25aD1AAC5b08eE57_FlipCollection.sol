...
contract FlipCollection is BaseCollection {
    
    using SafeMathUpgradeable for uint256;
    using MerkleProofUpgradeable for bytes32[];

    event MerkleRootSet(bytes32 indexed merkleRoot);
    event PresaleSet(bool active);
    event SaleSet(bool active, uint256 price);
    event AuctionSet(bool active, uint256 duration, uint256 start, uint256 end);
    ...
    function initialize(
        ...
        __BaseCollection_init(name_, symbol_, treasury_, trustedForwarder_, baseTokenURI_, contractURI_);
        _collectionType = "flip";
    }
    ...
    {
        require(!_presaleActive, "Presale is currently active. You must be whitelisted to mint, and use the presale form. : 1");
        require(_auctionActive || _saleActive, "Sale not active: 2");

        _purchaseMint(numberOfTokens, _msgSender());
    }

    //Must provide MerkleRoot Proof for whilelist. 
    //Create proof with the address in JS using Merkle proof function.
    function presaleMint(uint256 numberOfTokens, bytes32[] calldata proof)
        public
        payable
        onlyMintable(numberOfTokens)
    {
        require(_presaleActive, "Presale must be active. : 3");
        require(_merkleRoot != "", "Whitelist not active. : 4");
        require(
    ...
    //Create this in JS with an array of addresses
    ...
    function startSale(
        ...
        emit SaleSet(_saleActive, _price);
    }

    //set merkle root first in js with an array of addresses using merkle root function then start presale. 
    //Root can be updated.
    function startPreSale(
        ...
        emit SaleSet(_saleActive, _price);
        emit PresaleSet(_presaleActive);
    }
    ...
    function startAuction(
        ...
        emit AuctionSet(_auctionActive, _duration, newStartPrice, newEndPrice);
        emit PresaleSet(_presaleActive);
    }
    ...
    function stopAllSales() public onlyOwner {
        ...
        emit SaleSet(_saleActive,0);
        emit AuctionSet(_auctionActive,0,0,0);
        emit PresaleSet(_presaleActive);
    }
    ...
    /// @dev Sets the presale bool.
    function setPresale(bool presaleActive_) public onlyOwner{
        _presaleActive = presaleActive_;
        emit PresaleSet(_presaleActive);
    }
...
interface IFlipKit {
     ...
     */
    event CollectionCreated(address indexed collectionAddress, address affiliate, string name, string symbol, string collectionType);
...
import "./interfaces/IFlipKit.sol";
    ...
    IBaseCollection
{
    ...
    IFlipKit internal _FlipKit;
    address internal _treasury;
    uint256 internal _totalRevenue;
    string internal _contractURI;
    string public baseExtension;
    string public _collectionType;
    ...
    function collectionType() public view returns (string memory) {
    ...
    function setBaseTokenURI(string memory _baseTokenURI) public {
        ...
        emit TokenBaseURISet(baseTokenURI);
    }
    ...
    function withdraw() public {
        ...
        uint256 fees = _FlipKit.getFees(address(this));
        AddressUpgradeable.sendValue(payable(_treasury), balance.sub(fees));
        AddressUpgradeable.sendValue(payable(address(_FlipKit)), fees);

        _FlipKit.addFeesClaimed(fees);
    }
