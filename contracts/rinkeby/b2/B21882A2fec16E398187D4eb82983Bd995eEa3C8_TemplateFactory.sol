...
contract TemplateFactory is ITemplateFactory, ReentrancyGuardUpgradeable, NetworkAccessUpgradeable {
    ...
    function uploadTemplate(bytes32 templateId, bytes memory initialPart) external override returns (bool) {
        return uploadTemplate(templateId, initialPart, 1);
    }

    function uploadTemplate(bytes32 templateId, bytes memory initialPart, uint256 totalParts) public override onlyRole(MANAGE_TEMPLATE_ROLE) nonReentrant returns (bool) {
        ...
        emit TemplateVersionCreated(templateId, _version, _msgSender());
    ...
    function predictDeployedAddress(bytes32 templateId, uint256 _version, bytes memory args) external override view returns (address) {
        return predictDeployedAddress(templateId, _version, args, _salt(templateId, _version));
    }

    function predictDeployedAddress(bytes32 templateId, uint256 _version, bytes memory args, bytes32 salt) public override view returns (address) {
