...
contract HIFENFT is ERC1155 {
    ...
    constructor(address _token, address _charity, address _team, address _reward, address _treasury, address _liquidity) ERC1155 (){
    ...
    function mint(uint256 _numofTokensToMint)  public {
        ...
        randomId = isExists(randomId);
        isOverlapRewardClaimed[rateUpdatedTime[tokenTier[randomId]]][randomId] = true;
        nextTimePeriodToPayFee[randomId] = block.timestamp;
        lastClaimedTimestamp[randomId] = block.timestamp;
        _mint(randomId, randomId);
        assignedNFTs += 1;
        }
    }

    function reserveNFTs()  public onlyOwner{
        uint256 tokenId = reservedIds;
        uint256 randomId = getRandomId();
        randomId = isExists(randomId);
        _mint(tokenId, tokenId);
        reservedIds += 1;
        assignedNFTs += 1;
    }
    ...
    function queensTribute(uint id) public payable {
        ...
        //Transfer
        (bool sent,) = payable(adminAddress).call{value: msg.value}("");
