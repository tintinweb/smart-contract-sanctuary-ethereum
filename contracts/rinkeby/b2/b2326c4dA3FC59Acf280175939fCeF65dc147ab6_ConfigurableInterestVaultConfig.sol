...
contract ConfigurableInterestVaultConfig is IVaultConfig, OwnableUpgradeSafe {
  ...
  event SetWorkers(address indexed caller, address worker, bool isWorker);
  ...
  function initialize(
    ...
    setParams(_reservePoolBps, _getWrappedNativeAddr, _getWNativeRelayer);
  }
  ...
  function setWorkers(address[] calldata addrs, bool isWorker)
    external
    onlyOwner
  {
    for (uint256 idx = 0; idx < addrs.length; idx++) {
      workers[addrs[idx]] = isWorker;
      emit SetWorkers(_msgSender(), addrs[idx], isWorker);
    }
  }
