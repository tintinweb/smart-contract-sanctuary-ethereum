...
contract Base721 is Generic721, controllerPanel {
    ...
    function mintTest(address _receiver,uint64 _cardType) external onlyAllowed {
        mint(_receiver, _cardType);
    }
    ...
    function whitelistMint(uint64 _cardType, bytes memory signature) public {
        // Validate.
        require(canMint(_cardType),'!Time');
...
// ! REMOVE EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE
...
abstract contract Generic721 is ERC721Enumerable {
    ...
    uint64 public currentTestSeries = 0;
    ...
    function getCurrentSeries() public view returns(uint64) {
        if(currentTestSeries != 0) {
            return currentTestSeries;
        }
         for (uint64 i = 1; i <= CardTypeCount; i++) {
            if (CardType[i].mintTime <= block.timestamp && block.timestamp <= CardType[i].mintEndTime) {
                return i;
            }
        }
        return 0;
    ...
    function setCurrentTestSeries(uint64 _number) public {
        require(_number!=0,'!0');
