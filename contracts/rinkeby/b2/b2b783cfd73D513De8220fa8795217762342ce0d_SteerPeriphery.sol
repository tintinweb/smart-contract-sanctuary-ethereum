...
import "./interfaces/IUniLiquidityManager.sol";

// import "./UniLiquidityManager.sol";

/// @title Periphery contract to facilitate common actions on the protocol
/// @author Steer Protocol
/// @notice You can use this contract to enumerate strategy and vault details but also create, join, and leave vaults
/// @dev This function should be used when doing protocol integrations
contract SteerPeriphery is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    address private strategyRegistry;
    ...
    /// @notice Get the strategies by creator
        ...
        returns (IStrategyRegistry.RegisteredStrategy[] memory)
    {
        ...
        uint256 balance = IStrategyRegistry(strategyRegistry).balanceOf(
            _address
        );
    ...
    /// @notice Get the vaults using a given strategy
        ...
        returns (IVaultRegistry.VaultData[] memory)
    {
        ...
        uint256 count = IVaultRegistry(vaultRegistry).getVaultCountByStrategyId(
            _strategyId
        );

        // Create an array to hold the vault details
        IVaultRegistry.VaultData[]
            memory strategyVaults = new IVaultRegistry.VaultData[](count);
    ...
    function deposit(
        ...
        IUniLiquidityManager vaultInstance = IUniLiquidityManager(_vaultAddress);
    ...
    function vaultDetailsByAddress(address _vault)
        external view
        returns (IUniLiquidityManager.VaultDetails memory details)
    {
        IUniLiquidityManager vaultInstance = IUniLiquidityManager(_vault);

        return IUniLiquidityManager.VaultDetails(
            ...
            vaultInstance.getBalance0(),
            vaultInstance.getBalance1()
        );
    }

    function vaultBalancesByAddress(address _vault, bool _includeFees)
        external
        returns (IUniLiquidityManager.VaultBalance memory balances)
    {
        IUniLiquidityManager vaultInstance = IUniLiquidityManager(_vault);
        (uint256 bal0, uint256 bal1) = vaultInstance.getTotalAmounts(_includeFees);

        return IUniLiquidityManager.VaultBalance(
            bal0,
            bal1
        );
    }
...
interface IUniLiquidityManager is IERC20 {
     ...
     */
    function getTotalAmounts(bool includeFees)
    external
    view
    returns (uint256 total0, uint256 total1);

    /**
     * @dev Given a prior queued deposit, a user may claim their LP tokens here
     * @return uint256 amount of LP tokens earned by deposit and claimed
     */
    function claimLPTokensFromDeposit() external returns (uint256);

    /**
     * @dev given a prior queued withdrawal, a user may claim their withdrawn tokens here
     */
    function claimWithdrawal() external;

    /**
     * @dev can only be called by orchestrator. Tends, and sets new bins.
     * @param startTick lowest tick to be included in new bins
     * @param newBins new bins to be used
     */
    function update(int24 startTick, uint32[] calldata newBins) external;

    /**
     * @dev queues a withdrawal
     * @param _LPTs is amount of lp tokens they want to withdraw
     */
    function queueWithdrawal(uint256 _LPTs) external;

    function getLPTValue() external view returns (uint256, uint256);

    /**
     * @dev strategist-only function to withdraw funds strategist has earned from vault proceeds
     */
    function strategistPullFees() external;

    /**
     * @dev steer-only function to withdraw funds steer has earned from vault proceeds
     */
    function steerPullFees() external;
    ...
    /**
     * @dev returns TVL in terms of T0.
     */
    function getStrategyTVLinT0() external view returns (uint256);

    /**
     * @dev pulls earned fees from uniswap into the vault. Can be called by anyone. Called as part of tend().
     */
