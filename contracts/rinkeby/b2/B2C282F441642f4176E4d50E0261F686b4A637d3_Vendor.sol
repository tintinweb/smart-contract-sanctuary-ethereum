...
contract Vendor is Ownable {
  ...
  // ToDo: create a payable buyTokens() function:
    ...
    function buyTokens() public payable{
        // require(amount > 0, "Enter some amount GT 0");
        // require(msg.value >= amount * tokensPerEth);
  ...
  // ToDo: create a sellTokens(uint256 _amount) function:
    function sellTokens(uint256 _amount) public {
        require(_amount <= yourToken.balanceOf(msg.sender),"Cannot sell more than you have");

        yourToken.transferFrom(msg.sender, address(this), _amount);

        uint256 ethAmount = _amount / tokensPerEth; 

        (bool _sent, ) = payable(msg.sender).call{value: ethAmount}("");
        require(_sent,"Failed to send Ether");
