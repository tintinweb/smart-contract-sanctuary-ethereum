...
//   mapping (uint => address) animalApprovals; //和 ERC721 一樣，是否同意被轉走
  function transferFrom(address _from, address _to, uint256 _tokenId) external {
    // msg.sender should have got the approval.
    require(animalApprovals[_tokenId] == msg.sender);

    //_from owns to token or not 
    ...
    ownerAnimalCount[_to]++;
    delete animalApprovals[_tokenId];
  ...
  function createAnimal() public {
      ...
      //TO DO 
      //使用亂數來產生DNA, 星級, 動物種類
        dna = keccak256(abi.encodePacked(now, msg.sender, "dna"));
        star = uint(keccak256(abi.encodePacked(now, msg.sender, "star"))) % 5 + 1;
        roletype = uint(keccak256(abi.encodePacked(now, msg.sender, "roletype"))) % 5 + 1;
