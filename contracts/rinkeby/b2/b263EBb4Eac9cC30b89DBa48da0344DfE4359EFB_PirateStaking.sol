...
interface IPirateMetadata {
     function getTribeForPirate(uint8) external view returns (uint8);
}
...
contract PirateStaking is Ownable {
    ...
    IPirateMetadata metadataContract;
    ...
    function addPirates(uint8[] calldata pirateIds) external {
        for (uint i = 0; i < pirateIds.length; i++) {
            addPirate(pirateIds[i]);
        }
        stakedPirateCount += pirateIds.length;
    }

    function addPirate(uint8 pirateId) public {
    ...
    function removePirate(uint8 pirateId) public {
    ...
    function removePiratesForPlayer(uint8[] calldata pirateIds) external { 
        for (uint i = 0; i < pirateIds.length; i++) {
