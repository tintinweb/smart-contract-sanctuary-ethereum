...
contract NeighboursToken is ERC721, Ownable {
    ...
    event TokenUniqueBought(address from, address to, uint tokenId, uint price);
    event TokenPaintingOrdered(uint tokenId, uint price);
    ...
    function mintUnique(address to, uint imageId) external payable ifMintAllowed {
        require(msg.value >= _mintUniquePrice, "NeighboursToken: not enough value");

        uint tokenId = _mintNewToken(to, imageId);
        _imageTokens[imageId] = tokenId;

        emit TokenUniqueBought(_msgSender(), to, tokenId, msg.value);
    }
    ...
    function orderPainting(uint tokenId) external payable ifMintAllowed {
        ...
        emit TokenPaintingOrdered(tokenId, msg.value);
    }
