...
contract UmiverseEarth is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public constant freeMintAllowance = 3;
    ...
    function freeMint(uint256 numberOfTokens, bytes32[] calldata proof) public payable whenFreeMintIsActive {
        require(
            tx.origin == msg.sender,
            "Transaction from another contract not allowed"
        );
      require(
        ...
        require(_verify(proof, freeMintMerkleRoot, keccak256(abi.encodePacked(_msgSender()))), "Not allowed to mint for free");
        require(addressToMinted[_msgSender()] + numberOfTokens <= freeMintAllowance, "Exceeds free mint allowance"); 
