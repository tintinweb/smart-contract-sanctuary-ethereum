...
contract MultiSigOwner is ERC165, MultiSigWallet {
    ...
    function submitInternalAddOwner(address owner) external returns (uint256 transactionId) {
        transactionId = submitTransaction(address(this), 0, abi.encodeWithSelector(MultiSigWallet.addOwner.selector, owner));
    }

    function submitInternalRemoveOwner(address owner) external returns (uint256 transactionId) {
        transactionId = submitTransaction(address(this), 0, abi.encodeWithSelector(MultiSigWallet.removeOwner.selector, owner));
    }

    function submitInternalReplaceOwner(address owner, address newOwner) external returns (uint256 transactionId) {
        transactionId = submitTransaction(address(this), 0, abi.encodeWithSelector(MultiSigWallet.replaceOwner.selector, owner, newOwner));
    }

    function submitInternalChangeRequirement(uint256 _required) external returns (uint256 transactionId) {
        transactionId = submitTransaction(address(this), 0, abi.encodeWithSelector(MultiSigWallet.changeRequirement.selector, _required));
    }
    ...
    function submitProxyChangeAdmins(address[] calldata proxies, address newAdmin) external returns (uint256[] memory transactionIds) {
        ...
        for (uint256 i = 0; i < proxies.length; i++) {
            transactionIds[i] = submitTransaction(proxies[i], 0, abi.encodeWithSelector(OwnedUpgradeableProxy.changeAdmin.selector, newAdmin));
        }
    }
    ...
    function submitProxyUpgradeTo(address proxy, address newImplementation) external returns (uint256 transactionId) {
        transactionId = submitTransaction(proxy, 0, abi.encodeWithSelector(OwnedUpgradeableProxy.upgradeTo.selector, newImplementation));
    }
    ...
    function submitProxyUpgradeBeaconTo(address proxy, address newBeacon) external returns (uint256 transactionId) {
        transactionId = submitTransaction(proxy, 0, abi.encodeWithSelector(OwnedBeaconProxy.upgradeBeaconTo.selector, newBeacon));
    }
    ...
    function submitBeaconUpgradeTo(address beacon, address newImplementation) external returns (uint256 transactionId) {
        transactionId = submitTransaction(beacon, 0, abi.encodeWithSelector(UpgradeableBeacon.upgradeTo.selector, newImplementation));
    }
    ...
    function submitTransferOwnerships(address[] calldata contractAddresses, address newOwner) external returns (uint256[] memory transactionIds) {
        ...
        for (uint256 i = 0; i < contractAddresses.length; i++) {
            transactionIds[i] = submitTransaction(contractAddresses[i], 0, abi.encodeWithSelector(IOwnable.transferOwnership.selector, newOwner));
        }
    }
    ...
    function submitRenounceOwnerships(address[] calldata contractAddresses) external returns (uint256[] memory transactionIds) {
        transactionIds = new uint256[](contractAddresses.length);
        for (uint256 i = 0; i < contractAddresses.length; i++) {
            transactionIds[i] = submitTransaction(contractAddresses[i], 0, abi.encodeWithSelector(IOwnable.renounceOwnership.selector));
        }
    }
    ...
    function submitGrantRoles(
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            transactionIds[i] = submitTransaction(contractAddress, 0, abi.encodeWithSelector(IAccessControl.grantRole.selector, role, accounts[i]));
        }
    }
    ...
    function submitRevokeRoles(
        ...
        transactionIds = new uint256[](accounts.length);
        for (uint256 i = 0; i < accounts.length; i++) {
            transactionIds[i] = submitTransaction(contractAddress, 0, abi.encodeWithSelector(IAccessControl.revokeRole.selector, role, accounts[i]));
        }
    }
    ...
    function submitRenounceRole(address contractAddress, bytes32 role) external returns (uint256 transactionId) {
        transactionId = submitTransaction(contractAddress, 0, abi.encodeWithSelector(IAccessControl.renounceRole.selector, role, address(this)));
    }
