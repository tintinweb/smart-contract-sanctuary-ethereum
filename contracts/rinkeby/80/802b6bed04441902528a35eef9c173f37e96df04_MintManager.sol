...
contract MintManager is Initializable, OwnableUpgradeable {
    ...
    uint public constant PODTOWN_PERCENTAGE = 500;
    ...
    function initialize(address _podtownAddress) public initializer {
        __Ownable_init();
    ...
    function mintByCustomToken(uint amount, uint8 tokenIndex) external {
        ...
        IERC20(supportedToken[tokenIndex].tokenAddress).transferFrom(msg.sender, address(this), amount);
    ...
    function withdraw() public payable onlyOwner {
        ...
        uint podtownBalance = (balance * PODTOWN_PERCENTAGE) / 1000;
        uint basedBalance = balance - podtownBalance;
    ...
    function calculatePrice(uint8 tokenIndex) public view returns (uint) {
        if (tokenIndex == 0) {
            ...
            return ((price * base) * supportedToken[tokenIndex].conversionRate) / 10**3;
        }
    }
    ...
    function calculatePrices(uint quantity, uint8 tokenIndex) public view returns (uint) {
        if (tokenIndex == 0) {
            ...
            return ((price * base) * supportedToken[tokenIndex].conversionRate * quantity) / 10**3;
        }
    }

    function setNftAddress(IGodNFT _nftAddress) public {
