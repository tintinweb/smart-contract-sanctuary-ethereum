...
contract Nft1155 is ERC1155, Ownable  {
    ...
    function uri(uint256 tokenId) public view  override  returns (string memory) {
        ...
        return string(abi.encodePacked(_uri, ipfsHashes[tokenId]));
    }


    function mint(address to, uint256 amount, string memory ipfsHash) public onlyOwner {

        _tokenIdCounter.increment();
        uint256 newToken = _tokenIdCounter.current();
        ipfsHashes[newToken] = ipfsHash;
        _mint(to, newToken, amount, '');
    }
