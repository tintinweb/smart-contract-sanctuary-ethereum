...
contract NiftyERC721Token is ERC721Omnibus, NiftyPermissions, RejectEther, IERC2309, IERC2981 {    
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, NiftyPermissions, IERC165) returns (bool) {
        return 
    ...
    function mint(address to, bool finalizeContract, uint256[] memory amounts) external {
        _requireOnlyValidSender();
        ...
        if(_collectionStatus.defaultOwner != address(0)) {
            require(to == _collectionStatus.defaultOwner, ERROR_CANNOT_CHANGE_DEFAULT_OWNER);
        }
        
        uint72 nextNiftyType = _lastNiftyType.niftyType;
        uint88 nextIdCounter = _lastNiftyType.idLast + 1;
        ...
        uint256 newlyMinted = lastIdCounter - _lastNiftyType.idLast;        
        ...
        _collectionStatus.isContractFinalized = finalizeContract;
        _collectionStatus.amountCreated += uint88(newlyMinted);
        _collectionStatus.defaultOwner = to;        
...
abstract contract ERC721Omnibus is ERC721 {
