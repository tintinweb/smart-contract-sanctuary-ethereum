...
import "./ASMBrainsASTOAirdrop.sol";

contract ASMBrainsASTOAirdropTest is ASMBrainsASTOAirdrop {
    ...
    constructor(
        ...
        IERC721 _brain,
        uint256 _tokenAmountPerBrain
    ) ASMBrainsASTOAirdrop(owner, _token, _brain, _tokenAmountPerBrain) {}

    function setClaimStatus(uint256[] calldata brainIds, bool status) external {
        for (uint256 i = 0; i < brainIds.length; i++) {
            claimed[brainIds[i]] = status;
        }

        totalBrainsClaimed = status ? totalBrainsClaimed + brainIds.length : totalBrainsClaimed - brainIds.length;
    }

    function resetClaimedBrainsAmount() external {
        totalBrainsClaimed = 0;
    }
}