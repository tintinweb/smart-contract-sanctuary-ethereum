...
contract FixedRateMarket is Initializable, ERC20, IFixedRateMarket {
  ...
  function borrow(
    ...
    require(block.timestamp < _maturity, "market expired");
    ...
    uint amountFV = Interest.PVToFV(
                                    ...
                                    block.timestamp,
                                    _maturity,
                                    _qParams.MANTISSA_APR()
                                    );

    _processLoan(amountPV, amountFV, quote);
  }

  /// @notice Execute against Quote as a borrower, using FV rather than PV.
  /// @param amountFV Amount that the borrower wants to execute as FV
  /// @param lender Account of the lender
  /// @param quoteType *Lender's* type preference, 0 for PV+APR, 1 for FV+APR
  /// @param quoteExpiryTime Timestamp after which the quote is no longer valid
  /// @param APR In decimal form scaled by 1e4 (ex. 10.52% = 1052)
  ...
  function borrowFV(
    ...
    // Calculate the equivalent `amountPV`
    uint amountPV = Interest.FVToPV(
                                    APR,
                                    amountFV,
                                    block.timestamp,
                                    _maturity,
  ...
  function repayBorrow(uint amount) external returns(uint){
    ...
    require(_checkApproval(msg.sender, _tokenAddress, amount),
            "insufficient allowance");

    // Check borrower has enough balance
    require(_checkBalance(msg.sender, _tokenAddress, amount),
  ...
  function _processLoan(
    ...
    require(!_voidNonces[quote.quoter][quote.nonce], "invalid nonce");    
    ...
    if(quote.side == 1){
      ...
      _createFixedRateLoan(msg.sender, quote.quoter, amountPV, amountFV);
    }else if (quote.side == 0){
      // If quote.side = 0, the quoter is the borrower
      _createFixedRateLoan(quote.quoter, msg.sender, amountPV, amountFV);
    }else {
  ...
  function _createFixedRateLoan(
    ...
    require(amountPV > 0, "invalid amount");

    // Interest rate needs to be positive
    require(amountPV < amountFV, "invalid APR"); 
    ...
    require(_checkApproval(lender, _tokenAddress, amountPV),
            "lender insufficient allowance");

    // Check lender has enough balance
    require(_checkBalance(lender, _tokenAddress, amountPV),
    ...
    // Check if borrower has sufficient collateral for loan
    IQollateralManager qollateralManager = IQollateralManager(_qollateralManagerAddress);
    uint liquidityRatio = qollateralManager.hypotheticalLiquidityRatio(
                                                                       borrower,
                                                                       address(0),
                                                                       0,
                                                                       address(this),
    ...
    require(
            liquidityRatio >= qollateralManager.minLiquidityRatio(),
