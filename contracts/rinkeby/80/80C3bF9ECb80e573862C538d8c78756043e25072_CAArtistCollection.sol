...
contract CAArtistCollection is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControl {
    ...
    bool private _publicSaleOpen = true;                                                   // public-mint-on
    ...
    uint256 private _nftPrice = 0.0249 ether;                                   // TODO nft price 
    ...
    address private constant GOLD_CONTRACT = 0x1bEEf8EEc88F731daf1f39389618F67231EC19bf; 
    ...
    constructor() ERC721("CAArtistCollection", "CAAC") {
    ...
    function uploadTokenURIs(uint256 _startTokenId, string[] memory _uriArray) public onlyRole(MINTER_ROLE) {
        for (uint i = 0; i < _uriArray.length; i++) {
            _setTokenURI( _startTokenId + i, _uriArray[i] );
        }
    }
    ...
    function earlyMintNFT( uint _passTokenId, uint _quantity, address _to ) public payable {

        // is the nft type available for sale
        require( isAvailableForSale(), "M0: not for sale" );

        // id the owner Platinum or Gold
        require (
            IERC721(GOLD_CONTRACT).ownerOf(_passTokenId) == msg.sender,
            "You need Gold or Platinum to mint"
        );
    ...
    function mintNFT( uint _quantity, address _to ) public payable {

        // is the nft type available for sale
        require( isAvailableForSale(), "M0: not for sale" );

        // is presale or public sale open
        require( isPublicSaleOpen(), "M1: not available for sale" );
    ...
    function setTreasury( address _newCaTreasury, address _newArtistTreasury  )  public onlyRole(DEFAULT_ADMIN_ROLE) {
