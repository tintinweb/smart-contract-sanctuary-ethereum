...
contract MoonlightBirdy is ERC721A, Ownable, ReentrancyGuard {
    ...
    function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
        require(!paused && whitelistMintEnabled, "Moonlight Birdy: Whitelist mint phase not open yet!");

        require(!whitelistClaimed[_msgSender()], "Address already claimed!");
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Invalid proof!");

        require(_numberMinted(msg.sender) + _mintAmount <= 1, "You can only mint 1 NFT on Whitelist phase!");
    ...
    function getClaimedWhitelistValue(address _address) public view returns (bool) {
