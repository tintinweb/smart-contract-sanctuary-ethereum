...
contract OlympusBondDepository is Ownable {
    ...
    struct Bond {
        ...
        uint256 vesting; // timedelta left to vest
    ...
    struct Adjust {
        ...
        uint256 target; // BCV when adjustment INISHED
        uint256 buffer; // minimum length (in ms) between adjustments
        uint256 lastTime; // last time when last adjustment made
    }
    ...
    function setAdjustment(
        ...
        adjustment = Adjust({ add: _addition, rate: _increment, target: _target, buffer: _buffer, lastTime: block.timestamp });
    }
    ...
    function redeem(address _recipient, bool _stake) external returns (uint256) {
        ...
        uint256 percentVested = percentVestedFor(_recipient); // (timedelta since last interaction / vesting term remaining)
    ...
    function percentVestedFor(address _depositor) public view returns (uint256 percentVested_) {
        ...
        uint256 timestampSinceLast = block.timestamp.sub(bond.lastTime);
