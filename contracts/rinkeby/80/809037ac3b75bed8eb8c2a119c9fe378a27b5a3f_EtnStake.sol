...
//        uint nftId;
    ...
    mapping (uint => address[]) public nftCountMap;              //公会成员计数
    ...
    function stakeFor(address _to, uint _nftId,uint _value, uint _level) public returns (uint){
        ...
        // add stake record
        addCount(_nftId, _to);
        userRecordMap[_nftId][_to].push(Record( _value,expire,_level));
        addGroupPower(_nftId,_value,_level);
        emit Stake( _to,_nftId, _value,_level);
        return _value;
    }
    function stake(uint _nftId,uint _value, uint _level) public returns (uint){
        return stakeFor(msg.sender,_nftId,_value,_level);
    }
    ...
    function stakeWithAdviceFor(address _to, uint _nftId,uint _value, uint _level, address _up) public{
        //200天，推荐人奖励质押金额➕15%的推荐奖励算力；
        //700天，推荐人奖励质押金额➕25%的推荐奖励算力；
        uint staked = stakeFor(_to, _nftId,_value,_level);
        if(_up == _to){
            return;
        }
    ...
    function stakeWithAdvice(uint _nftId,uint _value, uint _level, address _up) public{
        return stakeWithAdviceFor(msg.sender,_nftId,_value,_level,_up);
    }
