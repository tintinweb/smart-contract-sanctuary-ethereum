...
contract OGBeansList is Ownable {
     ...
     */
    function _addOgBean(address ownerAddress, uint256 tokenId) external onlyOwner {
        ogBeanTokenIds.push(tokenId);
        ogBeanOwner[tokenId] = ownerAddress;
    }
    ...
    function getOgBeanAddress(uint256 trackerNum) external view returns (address) {
            require(ogBeanTokenIds.length > trackerNum, 'Number does not exist');
            uint256 tokenId = ogBeanTokenIds[trackerNum];
            address ownerAddress = ogBeanOwner[tokenId];
            return ownerAddress;
    }
