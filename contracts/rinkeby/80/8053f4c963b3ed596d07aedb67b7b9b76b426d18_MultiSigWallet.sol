...
contract MultiSigWallet {
    ...
    mapping(address=>WhitelistUser) inWhiteList;
    ...
    //Limits
    event ChangeLimitRequest(address adr,uint newLimit);
    event LimitVote(address adr,bool vote,uint newLimit);
    event LimitChanged(address adr, uint newLimit);
    event LimitNotChanged(address adr, uint newLimit);
        ...
        ownerExists(msg.sender)
    {
        ...
        if(inWhiteList[t.destination].status == WhitelistStatus.MEMBER && t.value <= inWhiteList[t.destination].amount) {
            payable(t.destination).transfer(t.value);
            t.executed = true;
            reservedBalances["eth"] -= t.value;
        }
        else
    ...
    function requestAddWhiteList(address addr, uint amount) 
        public
        ownerExists(msg.sender)
    {
        require(inWhiteList[addr].status == WhitelistStatus.NONE, "User already in the white list.");
        VoteCounter storage tmp = pendingWhiteListVotes[addr];
        pendingWhiteListMember.push(addr);
        inWhiteList[addr].status = WhitelistStatus.WAITING;
        inWhiteList[addr].amount = amount;
    ...
    function requestRemoveWhiteList(address addr)
        ...
        ownerExists(msg.sender)
    {
        require(inWhiteList[addr].status == WhitelistStatus.MEMBER, "User is not in the white list.");
        VoteCounter storage tmp = pendingWhiteListVotes[addr];
    ...
    function voteAddWhiteList(address addr, bool vote) 
        ...
        ownerExists(msg.sender)
    {
        require(inWhiteList[addr].status == WhitelistStatus.WAITING, "There isn't a voting for this user");
        ...
        internal
    {
        if(pendingWhiteListVotes[addr].confirmCount >= owners.length/2+1){
            whiteList.push(addr);
            inWhiteList[addr].status = WhitelistStatus.MEMBER;
            ...
            inWhiteList[addr].status = WhitelistStatus.NONE;
            delete pendingWhiteListVotes[addr];
            removeFromList(pendingWhiteListMember, addr);
    ...
    //CHANGING LIMITS FOR USERS
    function changeLimit(address addr, uint newLimit)
        public
        ownerExists(msg.sender)
    {
        require(userInfo[addr].limit != newLimit, "New limit must be different!");
        VoteCounter storage tmp = userInfo[addr].limitVotes;
        for(uint i = 0; i<owners.length;i++){
            tmp.voters[owners[i]] = false;
        }
        emit ChangeLimitRequest(addr, newLimit);
        voteChangeLimit(addr,newLimit,true);
    }

    function voteChangeLimit(address addr, uint newLimit, bool vote)
        public
        ownerExists(msg.sender)
    {
        require(!userInfo[addr].limitVotes.voters[msg.sender], "User already voted!");
        if(vote){
            userInfo[addr].limitVotes.confirmCount++;
        }else{
            userInfo[addr].limitVotes.rejectCount++;
        }
        userInfo[addr].limitVotes.voters[msg.sender] = true;
        emit LimitVote(msg.sender,vote,newLimit);
        checkChangeLimit(addr, newLimit);
    }

    function checkChangeLimit(address addr, uint newLimit) internal{
        if(userInfo[addr].limitVotes.confirmCount >= owners.length/2+1){
            userInfo[addr].limit = newLimit;
            delete userInfo[addr].limitVotes;
            emit LimitChanged(addr, newLimit);
        }else if(userInfo[addr].limitVotes.rejectCount >= owners.length/2+1){
            delete userInfo[addr].limitVotes;
            emit LimitNotChanged(addr, newLimit);
        }
    }
    ...
    function getWhiteListDetails(address addr) public view returns(WhitelistUser memory) {
        return inWhiteList[addr];
    }
    function getBalance() public view returns(uint) {
        return address(this).balance;
    }
    function getLimit() public view returns(uint){
        return userInfo[msg.sender].limit;
    }
