...
contract DegenApe is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    string public baseURI = "ipfs://QmYd1Pvxt69hvv7mpgKLQTDS94Vs7syh8k6TE7pDU5eTcY/";
    ...
    constructor() ERC721A("Degen Ape NFT", "DA") {}
    ...
    function mint(uint256 _quantity) external payable {
        ...
        bool isFree = !(mintedFree[msg.sender]);

        if(isFree) {
            if(_quantity > 1) {
                require(msg.value >= (cost * _quantity) - cost, "Enter the correct mint amount in ETH");
            } else {
                require(msg.value >= 0 ether, "Enter the correct mint amount in ETH");
            }
            mintedFree[msg.sender] = true;
            totalFreeMinted += 1;
        } else {
            require(msg.value >= cost * _quantity, "Enter the correct mint amount in ETH");
        }
    ...
    function setMaxMintPerWallet(uint256 _newMaxMint) external onlyOwner {
    ...
    function withdrawETH() external onlyOwner nonReentrant {
        require(address(this).balance != 0 ether, "Zero Balance");
