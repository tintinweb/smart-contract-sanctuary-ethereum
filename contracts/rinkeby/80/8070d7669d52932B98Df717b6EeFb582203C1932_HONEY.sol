...
contract HONEY is ERC20, Pausable, Ownable, ReentrancyGuard {
    uint256 public EMISSION_RATE = 57870370370370; // 5 per day

    IERC721 private constant _bearCartel =
        IERC721(0x448fedfe892f14D2800429752dd64e8D4855ec06);
    ...
    constructor() ERC20("Honey", "HONEY") {
        _mint(owner(), 2000000 * 1000000000000000000); // mint 2 million tokens for liquidity pool
        _pause();
    }
    ...
    function burnBears(uint256[] memory tokenIDs)
        ...
        callerIsSender
    {
        uint256 honeyPerToken = 1000000000000000000 * 200; // 200 HONEY per token
        for (uint256 i = 0; i < tokenIDs.length; i++) {
            require(
                _bearCartel.ownerOf(tokenIDs[i]) == msg.sender,
                "NOT_OWNER_OF_TOKEN"
