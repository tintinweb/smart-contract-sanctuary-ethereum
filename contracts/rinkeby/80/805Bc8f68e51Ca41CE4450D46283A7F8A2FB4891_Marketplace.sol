...
import "./NFT.sol"; // для функции mint
...
contract Marketplace {
    ...
    mapping(uint256 => _sListItem) public sListItem;        // Список продаваемых NFT
    ...
    modifier auctionTimeNotEnded(uint256 _tokenID) {
        require(block.timestamp - sListAuction[_tokenID].auctionStartTime <= durationAuction, "Auction ended");
        _;
    } 
    // Только владелец NFT
    modifier onlyOwnerNFT(uint256 _tokenID) {
        require(_NFT.ownerOf(_tokenID) == msg.sender, "Only owner NFT can call this function widt that _tokenID");
        _;
    } 
// Функция createItem() - создание нового предмета, обращается к контракту NFT и вызывает функцию mint.
    function createItem(string memory _tokenURI) public returns(uint256){
        return _NFT.mint(msg.sender, _tokenURI);
    }
// Функция mint(), доступ к которой должен иметь только контракт маркетплейса
// Размещена в контракте _NFT

// Функция listItem() - выставка на продажу предмета.
    function listItem(uint256 _tokenID, uint256 _price) public onlyOwnerNFT(_tokenID) {
        require(_NFT.ownerOf(_tokenID) == msg.sender, "Only owner can sell token");
        // отправляем nft на контракт
        _NFT.transferFrom(msg.sender, address(this), _tokenID);

        // Сохраняем иформацию о выставлении предмета на продажу
        sListItem[_tokenID] = _sListItem({
            vendor: msg.sender,
            price: _price
        });
    }
...
// Функция cancel() - отмена продажи выставленного предмета
    function cancel(uint256 _tokenID) public {
        ...
        require(sListItem[_tokenID].vendor == msg.sender, "Only owner can cancel sell");
        // отправляем nft владельцу
        _NFT.transferFrom(address(this), sListItem[_tokenID].vendor, _tokenID);
        // ЗАТИРАЕМ АДРЕС ПРОДАВЦА
        sListItem[_tokenID].vendor = address(0x0);
}

// Функция listItemOnAuction() - выставка предмета на продажу в аукционе.
    function listItemOnAuction(uint256 _tokenID, uint256 _startbidPrice) public onlyOwnerNFT(_tokenID) {
        // отправляем nft на контракт
        _NFT.transferFrom(msg.sender, address(this), _tokenID);
...
// Функция makeBid() - сделать ставку на предмет аукциона с определенным id.
    function makeBid(uint256 _tokenID, uint256 _price) auctionTimeNotEnded(_tokenID) public {
        // стоимость превышает предыдущую ставку ?
        require(sListAuction[_tokenID].bidPrice < _price,"Your price must be more than the minimum bid");
        // переводим токены на контракт
        _Token.transferFrom(msg.sender, address(this), sListAuction[_tokenID].bidPrice);

        // если это НЕ ПЕРВАЯ ставка то возвращаем токены предыдущему бидеру
        if (sListAuction[_tokenID].lastBuyer != address(0x0)){
            _Token.transferFrom(address(this), sListAuction[_tokenID].lastBuyer, sListAuction[_tokenID].bidPrice);
        }

        sListAuction[_tokenID].auctionStartTime= block.timestamp;
...
// Функция finishAuction() - завершить аукцион и отправить НФТ победителю
    function finishAuction(uint256 _tokenID)  auctionTimeNotEnded(_tokenID)  public {
        ...
        _Token.transferFrom(address(this), currentOwner, sListAuction[_tokenID].bidPrice);
    }

// Функция cancelAuction() - отменить аукцион
    function cancelAuction(uint256 _tokenID) public {
        require(sListAuction[_tokenID].vendor == msg.sender, "Only owner can cancel auction");
        require(sListAuction[_tokenID].auctionStartTime + durationAuction <= block.timestamp, "Auction has not ended yet");

         // отправляем nft и токены
        _NFT.transferFrom(address(this), sListAuction[_tokenID].vendor, _tokenID);
        _Token.transferFrom(address(this), sListAuction[_tokenID].vendor, sListAuction[_tokenID].bidPrice);

        // Обнуляем адрес продавца чтобы понимать что он продан или снят с продажи
        sListAuction[_tokenID].vendor = address(0x0);
    }
