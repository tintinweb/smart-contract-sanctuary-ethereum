...
contract PassageRegistry is Initializable, AccessControlUpgradeable, UUPSUpgradeable, ERC2771Recipient {
    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
    uint256 public passportLatestVersion;
    uint256 public loyaltyLatestVersion;
    string public globalPassportBaseURI;
    string public globalLoyaltyBaseURI;

    // version -> implementation
    mapping(uint256 => address) public _passportImplementations;
    mapping(uint256 => address) public _loyaltyImplementations;
    ...
    modifier onlyFromManagedPassport() {
        require(managedPassports[_msgSender()], "ONLY_PASSPORT");
        _;
    }

    modifier onlyFromManagedLoyaltyLedger() {
        require(managedLoyaltyLedgers[_msgSender()], "ONLY_LOYALTYLEDGER");
        _;
    }
    ...
    function createPassport(bytes memory data) external returns (address passportAddress) {
        ERC1967Proxy proxy = new ERC1967Proxy(_passportImplementations[passportLatestVersion], data);
    ...
    function upgradePassport(uint256 version, address passportAddress) external returns (uint256 newVersion) {
        ...
        require(managedPassports[passportAddress], "no longer managed");
        require(passport.hasRole(passport.UPGRADER_ROLE(), _msgSender()), "not upgrader role");

        uint256 currentVersion = passport.passportVersion();
        require(version == currentVersion + 1, "only +1 increment allowed");
        address upgradeAddress = _passportImplementations[version];
        require(upgradeAddress != address(0), "version not found");
    ...
    function createLoyalty(bytes memory data) external returns (address loyaltyAddress) {
        ERC1967Proxy proxy = new ERC1967Proxy(_loyaltyImplementations[loyaltyLatestVersion], data);
    ...
    function upgradeLoyalty(uint256 version, address loyaltyAddress) external returns (uint256 newVersion) {
        ...
        require(managedLoyaltyLedgers[loyaltyAddress], "no longer managed");
        require(ll.hasRole(ll.UPGRADER_ROLE(), _msgSender()), "not upgrader role");
        uint256 currentVersion = ll.loyaltyLedgerVersion();

        require(version == currentVersion + 1, "only +1 increment allowed");
        address upgradeAddress = _loyaltyImplementations[version];
        require(upgradeAddress != address(0), "version not found");
    ...
    function updatePassportImplementation(uint256 version, address implementation) external onlyRole(MANAGER_ROLE) {
        _passportImplementations[version] = implementation;
        if (passportLatestVersion < version) passportLatestVersion = version;

        emit PassportImplementationUpdated(version, implementation);
    }

    function updateLoyaltyImplementation(uint256 version, address implementation) external onlyRole(MANAGER_ROLE) {
        _loyaltyImplementations[version] = implementation;
        if (loyaltyLatestVersion < version) loyaltyLatestVersion = version;

        emit LoyaltyImplementationUpdated(version, implementation);
    }
...
interface IPassport {
    ...
    function limitPerWallet() external view returns (uint256);
    ...
    function passageRegistry() external view returns (address);
    ...
    function setClaimEnabled(bool _claimEnabled) external;

    function setClaimFee(uint256 _claimFee) external;

    function setLimitPerWallet(uint256 _limitPerWallet) external;
...
interface ILoyaltyLedger {
    ...
    event RoleGranted(bytes32 indexed role, address indexed account, address indexed sender);
    event RoleRevoked(bytes32 indexed role, address indexed account, address indexed sender);
    event TokenCreated(uint256 id, string name, uint256 maxSupply, bool transferEnabled, uint256 limitPerWallet);
    event TransferBatch(
    ...
    function setLimitPerWallet(uint256 _id, uint256 _limitPerWallet) external;

    function setTokenClaimEnabled(uint256 _id, bool _enabled) external;

    function setTokenClaimFee(uint256 _id, uint256 _claimFee) external;
