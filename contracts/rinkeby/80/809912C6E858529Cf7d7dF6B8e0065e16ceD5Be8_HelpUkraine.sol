...
contract HelpUkraine is ERC1155PackedBalance, Ownable, ReentrancyGuard {
    ...
    uint256 public priceBronze = 0.01 ether;
    uint256 public priceSilver = 0.02 ether;
    uint256 public priceGold = 0.03 ether;
    bool public isWarEnd = false;
    uint256[] public count = new uint256[](3);
    ...
    */
    function mintNFT(uint256 mint_type) public payable nonReentrant returns(bool){
        require(isWarEnd == false, "War Ended!! Thanks for your supporting!");
        require((mint_type == 0 && msg.value >= priceBronze) ||
                (mint_type == 1 && msg.value >= priceSilver) ||
                (mint_type == 2 && msg.value >= priceGold), "Please, Help Ukraine!");
        _mint(msg.sender, mint_type, 1, "");
        count[mint_type] ++;
        return true;
    }
    ...
    */
    ...
    /*
     * Function to set status of war
    */
    function setWarStatus(bool status) external onlyOwner {
        isWarEnd = status;
    }
    /*
     * Function to get Count of given token ID
    */
    function getCount(uint256 _tokenId) public view returns (uint256) {
        return count[_tokenId];
    }
    /*
     * Function to withdraw funds to UKraine Government
    */
    function withdrawFunds(address ukraineAddress, uint256 withdrawAmount) external onlyOwner returns (bool) {
        ...
        payable(ukraineAddress).transfer(withdrawAmount);
        return true;
    }
}
