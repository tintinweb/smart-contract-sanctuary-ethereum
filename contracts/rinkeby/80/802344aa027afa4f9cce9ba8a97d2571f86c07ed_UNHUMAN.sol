...
abstract contract ERC721URIStorage is ERC721 {
    ...
    mapping(uint256 => uint256[2][]) private _coords;
    ...
    function drawPoly(uint256 x, uint256 y, uint256 w, uint256 h, uint[3] memory color) public view returns (string memory) {
    ...
    function mixColors(uint colorIndex1, uint colorIndex2, uint ratio) public view returns (uint256[3] memory){
        uint256[3] memory colors;
        string[8] memory colorString;
        uint256[3][] memory colorRGB;
        colorString[0] = "Red";
        colorRGB[0][0] = 255;
        colorRGB[0][1] = 0;
        colorRGB[0][2] = 0;
        colorString[1] = "Magenta";
        colorRGB[1][0] = 255;
        colorRGB[1][1] = 0;
        colorRGB[1][2] = 255;
        colorString[2] = "Lime";
        ...
        colorRGB[5][1] = 100;
        colorRGB[5][2] = 100;
        colorString[6] = "Orangered";
        colorRGB[6][0] = 255;
        colorRGB[6][1] = 69;
        colorRGB[6][2] = 0;
        colorString[7] = "Purple";
        colorRGB[7][0] = 128;
        colorRGB[7][1] = 0;
        colorRGB[7][2] = 128;
        uint256 r1 = colorRGB[colorIndex1][0];
        uint256 g1 = colorRGB[colorIndex1][1];
        uint256 b1 = colorRGB[colorIndex1][2];
        uint256 r2 = colorRGB[colorIndex2][0];
        uint256 g2 = colorRGB[colorIndex2][1];
        uint256 b2 = colorRGB[colorIndex2][2];

        colors[0] = sqrt(((r1*r1+r2*r2)/2)) / ratio;
        colors[1] = sqrt(((g1*g1+g2*g2)/2)) / ratio;
        colors[2] = sqrt(((b1*b1+b2*b2)/2)) / ratio;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        for (uint256 i = 0; i < _noObjects[tokenId]-1; i++) {
            for (step = 0; step < limit; step++){ 
                slot.newColor = mixColors(_colors[tokenId][i], _colors[tokenId][i+1], step);
                slot.newX =  _coords[_noObjects[tokenId]][i*2][0] + (_coords[_noObjects[tokenId]][i * 2 + 2][0] - _coords[_noObjects[tokenId]][i*2][0]) * (step / limit);
                slot.newY =  _coords[_noObjects[tokenId]][i*2][1] + (_coords[_noObjects[tokenId]][i * 2 + 2][1] - _coords[_noObjects[tokenId]][i*2][1]) * (step / limit);
                slot.newW =  _coords[_noObjects[tokenId]][i * 2 + 1][0] + (_coords[_noObjects[tokenId]][i * 2 + 3][0] - _coords[_noObjects[tokenId]][i * 2 + 1][0]) * (step / limit);
                slot.newH =  _coords[_noObjects[tokenId]][i * 2 + 1][1] + (_coords[_noObjects[tokenId]][i * 2 + 3][1] - _coords[_noObjects[tokenId]][i * 2 + 1][1]) * (step / limit);
                slot.poly = drawPoly(slot.newX, slot.newY, slot.newW, slot.newH, slot.newColor);
        ...
        slot.polys = string(abi.encodePacked(slot.polys, "</svg>"));
        ...
        if (bytes(slot.polys).length > 0) {
            return string(abi.encodePacked(_baseURI(), slot.polys));
        }
    ...
    function _setCoords(uint256 tokenId, uint256[2][] memory _coordsPassed) internal virtual {
...
constructor(address initialRoyaltiesReceiver) ERC721("UNHUMAN COMPOSITIONS", "UNHUMAN") {
    ...
    function mint(uint256[2][] memory coords, uint256 noObjects, uint256 noColors, uint[] memory colors) external {
    //add payable
