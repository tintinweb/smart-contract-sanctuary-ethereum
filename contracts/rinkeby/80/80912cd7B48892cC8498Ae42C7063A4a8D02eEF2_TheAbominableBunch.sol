...
contract TheAbominableBunch is IERC721, ERC721Enumerable, EIP712Whitelisting, ReentrancyGuard, ContextMixin, NativeMetaTransaction {
    using Strings for uint256;

    // Constants.
    uint16 public MAX_SUPPLY     = 4400;                 // The total amount that can be minted ever.
    uint128 public WAVE_1_COST   = 0.065 ether;          // (QuickDraw) Wave 1 cost.
    uint128 public WAVE_3_COST   = 0.09 ether;           // (Public) Wave 3 cost.

    // Configuration.
    uint8 public maxPerAddress = 6;                      // The max amount of mints any single address can do.
    ...
    mapping(address => uint16) private mintBalances;     // Store lifetime mints for addresses.
    ...
    /// @param _initBaseURI The base URI to access the NFT metadata.
    /// @param _initNotRevealedUri The URI of the not revealed version of the NFT metadata.
    constructor(string memory _name, string memory _symbol, string memory _initBaseURI, string memory _initNotRevealedUri, address proxyRegistryAddress)
        ERC721(_name, _symbol) 
        EIP712Whitelisting(_name)
    {
        _initBaseURI = _initBaseURI;                   // Set the base URI to the metadata.
        _initNotRevealedUri = _initNotRevealedUri;     // Set the not revealed URI for the placeholder metadata. 
    
        _proxyRegistryAddress = proxyRegistryAddress;
        _initializeEIP712("The Abominable Bunch");
    }
    ...
    function purchase(uint16 _max, bytes calldata _signature, uint16 _mintAmount) external payable 
        nonReentrant 
    {
        require(_mintAmount > 0,  "Mint minimum one");
        ...
        else
        {
            ...
            if (_mintAmount + mintBalances[msg.sender] > maxPerAddress) { revert("Exceeds limit"); }
            ...
            {      
                ...
                require(!(_mintAmount + mintBalances[msg.sender] > _max), "Exceeded allocation");

                // Check the whitelist details match.
                require(checkWhitelist(_max, _signature),  "Invalid signature");
            }

            // Mint!
            _mint(_mintAmount, isWhiteList ? WAVE_1_COST : WAVE_3_COST);
        }
    }
        ...
        returns (string memory)
    {
        ...
        require(_exists(_tokenId), "ERC721Metadata: URI of nonexistent token");
    ...
    // /// @notice Returns the current price based on the wave.
    // /// @return the compiled Uri string to the nft metadata.
    // function currentCost() public view virtual returns (uint128)
    // {
    //     // If we are the owner, return 0 cost.
    //     // In a view the send can be anything. But as this is read only and we check the eth value in the purchase function this wont cause an issue.
    //     // This is purely as a helper for the DAPP instead of hardcoding values.
    //     if (msg.sender == owner())
    //     {
    //         return 0;
    //     }
    //     else
    //     {
    //         // If we are wave one  the respective cost.
    //         if (currentWave == 1) 
    //         {
    //             return WAVE_1_COST;
    //         }
    //         // If we are wave three return the public cost.
    ...
    function _mint(uint16 _quantity, uint128 price) internal {
        ...
        uint256 supply = totalSupply(); // Check the total supply.
        ...
        require(msg.value >= price * _quantity, "Insuffcient Eth");
    ...
    // /// @notice Set the place holder image url before NFTs are relvealed.
    // function setNotRevealedURI(string memory _notRevealedURI) external onlyOwner {
    //     notRevealedUri = _notRevealedURI;
    // }

    // /// @notice Set the base url for the the NFTs.
    ...
    /// @notice Set the wave for the mint.
...
contract EIP712Whitelisting is Ownable {
    ...
    bytes32 public constant MINTER_TYPEHASH = keccak256("Minter(address wallet,int max)");
    ...
    function checkWhitelist(uint16 max, bytes calldata signature) internal view returns (bool) {
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(MINTER_TYPEHASH, msg.sender, max))
            )
        );
    ...
    // modifier requiresWhitelist(uint8 wave, uint8 max, bytes calldata signature) {
    //     require(whitelistSigningKey != address(0), "Whitelist not enabled");
    ...
    //             "\x19\x01",
    //             DOMAIN_SEPARATOR,
    //             keccak256(abi.encode(MINTER_TYPEHASH, msg.sender, wave, max))
    //         )
    //     );
