...
contract BabyWizards is ERC721ABurnable, Ownable, ReentrancyGuard {
 ...
 function generalAdmission(uint adoptCount, uint256 tokenId) validateAdoption(adoptCount, tokenId)  nonReentrant external payable {
 ...
 function childAdmission(uint adoptCount, uint256 tokenId) validateAdoption(adoptCount, tokenId)  nonReentrant external payable {
     require(currentMode == ReleaseMode.OPEN, "It's not go time yet.");
   
        try ERC721A(BabiesAddress).ownerOf(tokenId) returns (
            address wizardOwner
        ) {
            if (wizardOwner == msg.sender) {
