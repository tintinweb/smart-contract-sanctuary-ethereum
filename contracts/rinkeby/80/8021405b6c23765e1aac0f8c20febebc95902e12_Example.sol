...
contract Example is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable {
	using Counters for Counters.Counter;
	using Strings for uint256;
	
	string private uri = "https://example.com";
	...
	constructor() ERC721("Example", "EXA") {
	}
	...
	function whitelistCheck(bytes32[] calldata proof) public view returns (bool){
		bytes32 leaf = keccak256(abi.encodePacked(msg.sender));

        bool valid = false;

        for(uint i = 0; i < merkleRoot.length; i++) {
            if(MerkleProof.verify(proof, merkleRoot[i], leaf)) {
                valid = true;
	...
	function whitelistUpdate(bytes32[] memory root) public onlyOwner {
        merkleRoot = root;
	}
    
    function whitelistGet() public view  returns(bytes32[] memory) {
        return merkleRoot;
	}
