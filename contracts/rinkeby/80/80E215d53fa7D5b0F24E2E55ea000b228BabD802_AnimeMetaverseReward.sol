...
/// @notice Can not airdrop less than 1 item
/// @dev Use this custom error on revert function whenever evaluating Airdrop amount
error InvalidAirdropAmount();

/// @notice Address is not included in Burner list
/// @dev Use this custom error on revert function whenever checking burner address
error InvalidBurner();

/// @notice Address is not included in Minter list
/// @dev Use this custom error on revert function whenever checking minter address
error InvalidMinter();

/// @notice Token ID is not matched with the existing one
/// @dev Use this custom error on revert function whenever checking item token ID
error InvalidTokenId();

/// @notice Token type is not matched with the existing one
/// @dev Use this custom error on revert function whenever checking token type
error InvalidTokenType();

/// @notice Can not airdrop beyond the maximum limit
/// @dev Use this custom error on revert function whenever airdrop amount exceeds
///      the maximum limit
/// @param amount The amount of item that would be airdropped
error MaxAirdropLimitExceeded(uint256 amount);

/// @notice The token supply already reaches its maximum limit
/// @dev Use this custom error on revert function whenever token total supply would
///      exceed the maximum supply if the token is minted
/// @param range  Amount of token that would be minted
error MintAmountForTokenTypeExceeded(uint256 range);

/// @notice Address is not allowed to burn the token
/// @dev Use this custom error on revert function whenever checking address permission
///      to burn tokens
    ...
    IAnimeMetaverseReward
{
    ...
    uint256 public constant DIGITAL_MERCH_TOKEN_TYPE = 2;
    uint256 public constant GIFT_BOX_TOKEN_TYPE = 3;
    uint256 public constant COMPONENT_TOKEN_TYPE = 4;
    uint256 public constant AM_ITEM_TOKEN_TYPE = 5;
        ...
        validAirdropAmount(_amount)
    {
        ...
        for (uint256 j = 0; j < _amount; j++) {
            ...
            activityMap[tokenIdStartIndex + j] = _activityId;
        }
    ...
    /// @dev this internal function is only called when someone burns a merchendise NFT.
    ///      Then this function will mint a digital merchendise NFT to the holder of that merchendise NFT
    ...
    /// @dev Only registered address in burner list can execute this function
