...
contract SickClub is IERC721Receiver, Ownable{
    ...
    struct UserSticks{
        ...
        uint16 [] stickIds;
        //Stick index;
        mapping(uint256 => uint256) stickIndex;
    }
    ...
    mapping(uint16 => StickAttributes) private stick; 
    mapping(uint256 => FightStats) private fightLogs; 
    mapping(address=> UserSticks) private userSticks;

    address public STICK; 
    address public treasury;
    uint256 public fightFee=1*1e18;
    uint256 public collectedFees;
    bool public fightPaused = false;
    uint8 public treasuryPercentage=10;
    uint256 public fightNumber = 0;

    event FightStarted(uint256 indexed fightId, uint16 indexed stickOne, uint16 indexed stickTwo);
    event FightEnded(uint256 indexed fightId, uint16 indexed winnerStick, uint256 indexed prize, uint256 treasuryFee);
    event JoinedClub(uint16 indexed StickId);
    event LeftClub(uint16 indexed StickId);
    ...
    function BeatMyStick(uint8 _stickId) public{
        require(!fightPaused, "Fight is paused");
        require (msg.sender == checkStickOwner(_stickId), "Not your STICK");
        require(!stick[_stickId].inLine, "STICK already hard to fight");    
        ...
        uint256 arrayLength=(userSticks[msg.sender].stickIds).length;
        userSticks[msg.sender].stickIndex[_stickId]=arrayLength-1;
        emit JoinedClub(_stickId);
    }

    //Get NFT out from the queue (Don't want to fight anymore)
    function GetThefOut(uint8 _stickId) public{
        ...
        uint256 indexRemovedStick=userSticks[msg.sender].stickIndex[_stickId];
        uint256 lastIndex=(userSticks[msg.sender].stickIds).length-1;
    ...
    function mySticks(address _userAddress) public view returns (uint256 balance, uint16 [] memory sticks){
        return (userSticks[_userAddress].balance, userSticks[_userAddress].stickIds);
    }
    ...
    function startFight(uint16 _stickOne, uint16 _stickTwo) public onlyOwner returns(uint256) {
    ...
    function fightLog(uint8 _fightId) public view returns(uint16 playerOne,uint16 playerTwo,uint8 winner,uint256 prize){
    ...
    function stickStats(uint8 _stickId) public view returns(bool inFight, bool inLine, uint256 totalWins, uint256 totalLoses){
