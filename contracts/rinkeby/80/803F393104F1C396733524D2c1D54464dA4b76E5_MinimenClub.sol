...
contract MinimenClub is ERC721OwnerEnumerable {
        ...
        returns (bytes32)
    {
        return
            ECDSA.toEthSignedMessageHash(
                keccak256(
                    abi.encode(address(this), block.chainid, owner(), approved)
                )
            );
    }

    function hashToSignForApproval(address approved)
        external
        view
        returns (bytes32)
    {
    ...
    function mint(address approvedWallet, bytes memory signature) external {
        if (totalSupply() >= _tokenLimit()) revert OverSupplyLimit();
        if (_addressData[_msgSender()].numberMinted > 4) revert OverMintLimit();
    ...
    function claim(uint256[] memory tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (tokenId - _minTokenId >= LEGENDARY_COUNT) {
                revert TokenNotClaimable();
            }
            if (_claimedTimestamp[tokenId] != 0) revert TokenAlreadyClaimed();
