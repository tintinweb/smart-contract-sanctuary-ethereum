...
contract Humanians is ERC721AEnumerable, Payable {
    ...
    /**
     * The block.timestamp each of this owners tokens were transferred to the current owner.
     * @param owner The owner to query
     */
    function diamondSinceOfOwner(address owner) public view returns (uint256[] memory) {
        uint256[] memory tokens = tokensOfOwner(owner);
        uint256[] memory diamonds = new uint256[](tokens.length);

        for (uint256 i = 0; i < tokens.length; i++) {
            diamonds[i] = diamondSince(tokens[i]);
        }
