...
contract InternalTest is ERC721A, Ownable {
    ...
    uint256 constant whitelistPrice = 0.00005 ether;

    uint256 public maxSupply = 300; // Maximun mint ACE
    uint256 public maxPreMintSupply = 110; // Maximun mint ACE (1998 + 300)
    ...
    uint256 public maxPartnerMintAmount = 10; //How many partner pre mint in one times
    ...
    constructor() ERC721A("ACEYMKTest 2", "ACE") {}
    ...
    function preMint(uint256 mintAmount) public payable {
        ...
        uint256 ownerMintedCount = addressMintedBalance[msg.sender];
        require(ownerMintedCount + mintAmount <= maxPreMintAmount, "Maximun mint NFT per address exceeded");
    ...
    */
    ...
    function isPreMintSoldOut() external view returns (bool) {
        uint256 supply = totalSupply();

        if(supply >= maxPartnerMintAmount)
            return true;
        else
            return false;
    }
