...
contract HotDougsMinter is Ownable, ReentrancyGuard, ERC721TokenReceiver {
    ...
    /// @notice send a free doug per tokenId to the transaction originator
    function sendFreeDougsById(uint256[] memory freeDougArray) public {
        // check if address has free valid Hot Dougs to claim
        HotDougs _hotDougs = HotDougs(dougContractAddress);
    ...
    function sendFreeDougsNew() external {
        if (checkForFreeDougs(tx.origin) == false) {
            revert noFreeDougsForAddress();
        }
        uint256[] memory freeDougArray = getFreeDougArray(tx.origin);
        sendFreeDougsById(freeDougArray);
    }

    // Get a list of free dougs eligible by an address
        ...
        returns (uint256[] memory)
    {
        ...
        //require(tokenBalance > 0, "No Dougs owned");
    ...
    // /// @notice used to get count of free Dougs available for an address for Gas limit purposes
    // function countOfFreeDougs(address _address) public view returns (uint256) {
    //     HotDougs _hotDougs = HotDougs(dougContractAddress);
    //     uint256 tokenBalance = _hotDougs.balanceOf(_address);
    //     require(tokenBalance > 1, "No Tokens! per countOfFreeDougs");
    //     uint256[] memory addressTokenIds = _getTokenIds(_address);
    //     uint256 length = addressTokenIds.length;
    //     uint256 claimableDougs;
