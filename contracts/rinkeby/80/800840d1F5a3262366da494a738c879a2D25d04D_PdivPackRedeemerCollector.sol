...
contract PdivPackRedeemerCollector is VRFConsumerBaseV2, Ownable {
    ...
    function handleReceive(
        ...
        require(_cardIds[0] == pdIVTokenId, "needs PDIV pack reservation");
    ...
    function initializeAvailablePackIds(uint256 batchNum, uint256 batchSize)
        public
        onlyOwner
    {
        require(
            availablePackIds.length == batchNum * batchSize,
            "cant re-initialize same batch"
        );
        for (
            uint16 i = uint16(availablePackIds.length);
            i < Math.min(((batchNum + 1) * batchSize), numCards);
            i++
        ) {
    ...
    function setPdIVTokenId(uint256 _val) public onlyOwner {
        pdIVTokenId = _val;
    }
