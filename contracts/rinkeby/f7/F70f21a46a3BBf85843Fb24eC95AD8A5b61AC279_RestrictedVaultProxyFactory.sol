...
contract RestrictedVaultProxyFactory is RestrictedProxyFactory {
  ...
  ) internal virtual returns (RestrictedVaultProxy) {
    return new RestrictedVaultProxy(implementation);
  }
  ...
  function createProxy (
    address implementation,
    bytes memory initdata
  ) public returns (RestrictedVaultProxy) {
    require(
      implementation != address(0),
      "address: implementation cannot be 0 address."
    );

    RestrictedVaultProxy proxy = createProxyInstance(implementation, initdata);
    ...
    emit ProxyCreation(address(proxy), implementation);
