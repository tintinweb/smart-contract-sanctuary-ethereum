...
contract Today is IERC721, IERC2981, ERC721Enumerable, ReentrancyGuard, Ownable {
	...
	function tokenURI(uint256 tokenId) public view override returns (string memory) {
		...
		string memory svg = getSVG(storedTime);
	...
	function getSVG(uint256 time) private pure returns (string memory) {
		return
			string(
				abi.encodePacked(
	...
	function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view override returns (address, uint256) {
		...
		return (payable(owner()), (_salePrice / 100) * royaltyPercentage);
	}
}
