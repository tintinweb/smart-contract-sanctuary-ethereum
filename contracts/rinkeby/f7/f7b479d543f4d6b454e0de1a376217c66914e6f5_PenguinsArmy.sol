...
contract PenguinsArmy is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_PENGIUNS_PER_WALLET = 2;
    uint256 public immutable maxPenguins;

    bool public isWhiteListActive;
    bool public isPublicSaleActive;

    uint256 public whiteListSalePrice = 0.05 ether;
    ...
    constructor(
        ...
        uint256 _maxPenguins
    ) ERC721(_name, _symbol) {
        setBaseURI(_initBaseURI);
        maxPenguins = _maxPenguins;
    }

    // ============ ACCESS CONTROL MODIFIERS ============

    modifier whiteListActive() {
        require(isWhiteListActive, "Milk list not active");
        _;
    }
    ...
    modifier totalNotExceeded(uint256 numberOfTokens) {
        require(
            tokenCounter.current() + numberOfTokens <= maxPenguins,
            "Not enough penguins remaining to mint"
        );
        _;
    }
    ...
    // constructor(string memory _baseURI, uint256 _maxPenguins) ERC721("Penguins Army", "PENGUINS") {
    //     baseURI = _baseURI;
    //     maxPenguins = _maxPenguins;
    //     mintToTeam(1, msg.sender);
    // }
    ...
    function mintWhitelistSale(
        ...
        nonReentrant
        whiteListActive
        isCorrectPayment(whiteListSalePrice, numberOfTokens)
        totalNotExceeded(numberOfTokens)
        isValidMerkleProof(merkleProof, whiteListMerkleRoot)
    {
        uint256 numAlreadyMinted = whiteListMintCounts[msg.sender];
        require(numAlreadyMinted + numberOfTokens <= MAX_PENGIUNS_PER_WALLET, "ML: Two cats max per wallet");
        ...
        totalNotExceeded(numberOfTokens)
    {
        ...
        require(numAlreadyMinted + numberOfTokens <= MAX_PENGIUNS_PER_WALLET, "PM: Two cats max per wallet");
    ...
    function setWhitelistListPrice(uint256 _price) external onlyOwner {
