...
contract  Crypcentra  is ERC721Enumerable, Ownable
{
    ...
    uint public price = 0.65 ether;    //0.65 whitelist --- 0.85 during public
    // uint public preSalePrice=0.65 ether;
    uint public status=0;
    uint public whiteListCounter=10;
    bytes32 public merkleRoot = 0x99af027a6b9f08e037959df3f58f6141556c18dfb42f1424563317ea11dafb64;
    uint public whitelistedSpots = 1350;
 
    constructor(string memory baseURI) ERC721("Crypcentra.com", "Crypcentra")  {
        setBaseURI(baseURI);
    ...
    require(status==1, "Presale not active");
    require(totalSupply() + quantity <= _TOTALSUPPLY, "reached max supply");
    require( (balanceOf(msg.sender) + quantity <= whiteListCounter ), "Quantity exceeds allowed Mints During Whitelist" );
    ...
    for (uint i = 0; i < quantity; i++) {
        ...
        require(status == 2, "Sale is not active at the moment");
   ...
   function contractURI() public view returns (string memory) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Crypcentra.com", "description": "Crypcentra Smart Contract", "seller_fee_basis_points": 750, "fee_recipient": "0x92BBe4DC9C1F13121CFD6B32ED7F30d4aaf7e8F3"}'))));
