...
contract NemoLand is Admin, ERC721Upgradeable {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) cTypeToZonePrice;
    ...
    function initialize(
        ...
        for (
            ...
            for (
                uint256 zone = 0;
                zone < zonePricing[currencyType].length;
                zone++
            ) {
                cTypeToZonePrice[currencyType][zone] = zonePricing[
                    currencyType
                ][zone];
            }
        }
        for (uint256 cType = 0; cType < _usdtAddresses.length; cType++) {
            cTypeToAddress[cType] = IERC20(_usdtAddresses[cType]);
        }
    }
    ...
    function mintLand(
        ...
        uint256 amtRequired = cTypeToZonePrice[currencyType][_zoneId] *
            lands.length;
        ...
        if (uint256(CurrencyType.TYPE1) == currencyType) {
            ...
            cTypeToAddress[currencyType].transferFrom(
                msg.sender,
                receivingWallet,
                amtRequired
            );
