...
contract Sample247 is ERC721A, Ownable {
    // @dev Using errors instead of requires with strings saves gas at deploy time
    error SaleNotActive();
    error MaxSupplyReached();
    error MaxMintedForAddress();

    bool saleActive = false;
    uint256 constant MAX_SUPPLY = 10000;
    string public baseUri;

    constructor(string memory baseURI) ERC721A("Sample247", "247Sample") {
        baseUri = baseURI;
    }
    ...
    function publicMint(uint256 amount) public {
        ...
        if (_numberMinted(msg.sender) + amount > 2)
            revert MaxMintedForAddress();
