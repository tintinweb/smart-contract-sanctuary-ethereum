...
contract LotteryDrop is Ownable {
    ...
    uint256[] lotteryPlans = [100, 1_000, 5_000, 10_000, 100_000, 1_000_000];
    uint256 currentPlan = 0;
    uint256 public _totalRaffles = 0;
    ticket[] public tickets;

    event LotteryStateChanged(LotteryState newState);
	event NumberDrawn(uint winningNumber);
    ...
    function buyTicket(uint256 _amount) public isState(LotteryState.Open) {
        ...
        require(allowance >= _amount*110*usdc.decimals()/100, "Check the token allowance"); //allowance should include 10% fee
        usdc.transferFrom(_buyer, address(this), _amount*usdc.decimals()); //pay to LD contract
        usdc.transferFrom(_buyer, owner(), _amount*usdc.decimals()*10/100); //owner's cut
    ...
    function pickJackpotWinner() public onlyOwner isState(LotteryState.Open) {
        uint256 jackpotWinner = _pickJackpotWinner();
        usdc.transfer(tickets[jackpotWinner].owner, tickets.length*usdc.decimals());
        _changeState(LotteryState.Finished);
    }

    function initializeJackpot(uint256 _lotteryPlan) public onlyOwner isState(LotteryState.Finished) {
