...
interface IERC721 is IERC165 {
    ...
    function approve(address to, uint16 tokenId) external;

    function setApprovalForAll(address operator, bool _approved) external;

    function getApproved(uint16 tokenId) external view returns (address operator);
...
abstract contract ERC721URIStorage is ERC721 {
    using Strings for uint256;
        ...
        //require(!token_struct[tokenId].is_staked, "Cannot approve staked token");
    ...
    function _setTokenURI(uint16 tokenId, string memory _tokenURI) internal virtual {
        require(_exists(tokenId), "ERC721URIStorage: URI set of nonexistent token");
        _tokenURIs[tokenId] = _tokenURI;
    }

    function _burn(uint16 tokenId) internal virtual override {
        super._burn(tokenId);
...
contract FOR is ERC721URIStorage, Ownable {
   ...
   /* function setMarketCap (bytes32[] memory proof, uint8 amount, uint64 marketCap, uint8 whitelistID) external {
    ...
    function M_I_N_T(address to) public {
        ...
        totalSupply < 9999 ? totalSupply += 1 : totalSupply = 9999;
    ...
    function level(uint16 tokenId) view public returns (uint) {
        ...
        require(token_struct[tokenId].is_staked, "not staked");
