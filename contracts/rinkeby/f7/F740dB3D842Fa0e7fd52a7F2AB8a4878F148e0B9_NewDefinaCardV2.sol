...
contract NewDefinaCardV2 is ERC721EnumerableUpgradeable, NonblockingReceiverUpgradeable {
    ...
    event MergeSuccess(uint256 indexed tokenIdA, uint256 indexed tokenIdB, address owner, uint _currentRarity, uint _currentHero, uint _currentPoint, bool success, bytes32 randTransactionHash);
    ...
    function setMergerOperator(address newMergerOperator_) external onlyOwner {
        require(newMergerOperator_ != address(0), "The address is null");
        mergerOperator = newMergerOperator_;
    }
    // old rarity => new rarity
    mapping(uint => uint) public rarityRarityMap;
    // token => genesis point
    mapping(uint => uint) public genesisPointMap;
    // old rarity => genesis point
    mapping(uint => uint) public rarityPointMap;
    // Okex heroId => genesis point
    mapping(uint => uint) public okexPointMap;
    // Okex heroId => is Okex card
    mapping(uint => bool) public iSOkexMap;
    // token => heroId
    ...
    uint256 public maxClaimedAmount;
    ...
    //rarity=>scales
    mapping(uint => uint) public rarityScalesMap;
    ...
    IERC20Upgradeable  public mergeToken;
    ...
    function setRarityHeroIds(uint[][] calldata _rarityHeroIds) onlyOwner public {
        require(_rarityHeroIds.length > 0, "array is not empty");
    ...
    function setOldCardSetting(uint[][] calldata _oldCardSetting) onlyOwner public {
        require(_oldCardSetting.length == 4, "array length is 4");
        ...
        require(_rarityRarity.length > 0 && _rarityPoints.length > 0, "array is not empty");
        require(_okexHeroIds.length == _okexPoints.length, "HeroId and Points have the same length");
        //remove
        for (uint i = 0; i < _rarityRarity.length; i++) {
            uint oldRarity = i + 1;
            uint newRarity = _rarityRarity[i];
            delete rarityRarityMap[oldRarity];
    ...
    function startPublicSale(
        ...
        require(!publicSaleActive, "sale has already begun");
        require(_nftPrice > 0, "Price must be greater than 0");
        require(_currToken != IERC20Upgradeable(address(0)), "The address of token is null");
    ...
    function mintMulti(uint amount_) whenPublicSaleActive onlyEOA external {
        ...
        require(mintIndexPublicSale + amount_ <= MAX_MINT, "Mint exceeds supply");
    ...
    function mintMulti(uint amount_,uint rarity_) whenPublicSaleActive onlyEOA external {
        require(amount_ > 0, "missing amount");
        require(mintIndexPublicSale + amount_ <= MAX_MINT, "Mint exceeds supply");
        currToken.transferFrom(_msgSender(), address(this), nftPrice * amount_);
        for (uint i = 0; i < amount_; ++i) {
            uint rarity = rarity_;
            uint tokenId = mintIndexPublicSale++;
            uint heroId = CommonUtils.getHeroByRand(rarityHeroIdsMap[rarity], _msgSender(), i);
    ...
    function changeNFTA(IDefinaCard nftA_) onlyOwner external {
        nftA = nftA_;
    }
    ...
    function nftOwnerClaimCards(uint256[] calldata tokenIds) whenClaimableActive onlyEOA external {
        for (uint256 i; i < tokenIds.length; ++i) {
            ...
            require(nftA.ownerOf(tokenId) == _msgSender(), "Must own all of the nft defined by tokenIds");
            require(tokenId <= maxClaimedAmount, "NFT not allowed to claim");
    ...
    function claimLandByTokenId(uint256 tokenId) private {
        ...
        heroIdMap[tokenId] = CommonUtils.stringToUint(cardURI);
        if (iSOkexMap[heroIdMap[tokenId]] == true) {
            genesisPointMap[tokenId] = okexPointMap[heroIdMap[tokenId]];
        } else {
    ...
    // Merge Methods
    function startMerge(IERC20Upgradeable _mergeToken, uint[] calldata _mergePrices, uint[] calldata _directionalPrices, uint[] calldata rarityScales
    ) onlyOwner external {
        require(!mergeActive, "Merge has already begun");
        require(_mergePrices.length == (maxRarity - 1) && _directionalPrices.length == (maxRarity - 1) && rarityScales.length == (maxRarity - 1), "array length is wrong");
        require(_mergeToken != IERC20Upgradeable(address(0)), "The address of token is null");
    ...
    function addMerge(uint tokenIdA, uint tokenIdB, uint price) payable onlyEOA whenMergerActive external {
        require(_isApprovedOrOwner(_msgSender(), tokenIdA)&&_isApprovedOrOwner(_msgSender(), tokenIdB), "caller is not owner nor approved");
        require(tokenIdA != tokenIdB && rarityMap[tokenIdA] == rarityMap[tokenIdB] && rarityMap[tokenIdA] < maxRarity, "Two cards do not meet the merge rules");
        require(!forMerge[tokenIdA] && !forMerge[tokenIdB], "NFT already for merging");
        if (iSOkexMap[heroIdMap[tokenIdA]] == true) {
            uint directionalPrice = rarityDirectionalPriceMap[rarityMap[tokenIdA]];
            require(price >= directionalPrice, "missing directional price");
        }
        uint basePrice = rarityMergePriceMap[rarityMap[tokenIdA]];
        require(price >= basePrice, "missing amount");
    ...
    function toMerge(address user, uint tokenIdA, uint tokenIdB, bytes32 randomness_, bytes32 transactionHash) onlyOperator external {
        require(tokenIdA != tokenIdB && rarityMap[tokenIdA] == rarityMap[tokenIdB] && rarityMap[tokenIdA] < maxRarity, "Two cards do not meet the merge rules");
        require(mergeMap[tokenIdA].tokenIdB == tokenIdB, "NFTA merge object is not NFTB");
        bool mergeResult = CommonUtils.getMergeResult(rarityScalesMap[rarityMap[tokenIdA]], user, randomness_, transactionHash);
        bool success = false;
        if (mergeResult) {
            success = true;
            uint directionalPrice = rarityDirectionalPriceMap[rarityMap[tokenIdA]];
            ...
            genesisPointMap[tokenIdA] = genesisPointMap[tokenIdA] + genesisPointMap[tokenIdB];

            if (mergeMap[tokenIdA].price < directionalPrice) {
                //ModeL ONE
                uint heroId = CommonUtils.getHeroBySeed(rarityHeroIdsMap[newRarity], user, randomness_, transactionHash);
        ...
        delete forMerge[tokenIdB];
        delete mergeMap[tokenIdA];
        emit MergeSuccess(tokenIdA, tokenIdB, user, rarityMap[tokenIdA], heroIdMap[tokenIdA], genesisPointMap[tokenIdA], success, transactionHash);
    }
    ...
    function traverseChains(uint16 _chainId, uint tokenId) public payable {
        require(_msgSender() == ownerOf(tokenId), "You must own the token to traverse");
        ...
        require(msg.value >= messageFee, "LZNFT: msg.value not enough to cover messageFee. Send gas for message fees");
...
library CommonUtils {
    ...
    function getHeroBySeed(uint[] storage heroIds, address user, bytes32 seed, bytes32 transactionHash) internal view returns (uint) {
        uint mod = heroIds.length;
        uint rand = uint(keccak256(abi.encodePacked(
                block.timestamp,
                block.difficulty,
