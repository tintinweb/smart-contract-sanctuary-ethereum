...
contract ERC721Special is Initializable, ERC721, ContractRoles, Whitelist, FeesDistribution {
    ...
    uint256 public _defaultMintingPrice = 2 gwei;
    uint256 public _whitelistMintingPrice = 1 gwei;
    ...
    {
        uint256 mintingAmount = min(msg.value / _defaultMintingPrice, AIRDROP_LIMIT);
        require(
    ...
    function mintWhitelist(bytes32[] calldata proof) 
        ...
        returns (uint256[AIRDROP_LIMIT] memory _mintedTokensId) 
    {
        uint256 mintingAmount = min(msg.value / _whitelistMintingPrice, WHITELIST_LIMIT - whitelistMintedByUsers[msg.sender]);
        require(
            whitelistNFTLimit - whitelistMinted >= mintingAmount &&
            mintingAmount > 0 &&
            inWhitelist(msg.sender, proof),
            "User cannot mint using whitelist"
        );

        whitelistMinted += mintingAmount;
        whitelistMintedByUsers[msg.sender] += mintingAmount;
