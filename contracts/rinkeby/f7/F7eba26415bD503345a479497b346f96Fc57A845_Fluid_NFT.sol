...
contract Fluid_NFT is IERC721E, protected, FuidAdvancedNft {
    ...
    function set_token(address tkn) public onlyAuth{
        ERC20_Token = tkn;
    }
    ...
    function mint(uint mint_mode) public payable safe returns(uint id) {
        ...
        if(mint_mode == 0) {
            require(is_eth_mintable, "Cannot mint with eth"); 
            require(msg.value==eth_minting_price, "Not enough eth");
             
        }
        // Minting with custom token 
        else if(mint_mode == 1) {
            require(is_token_mintable, "Custom token not enabled"); 
            IERC20 mintable_token = IERC20(token_address);
            uint token_decimals = mintable_token.decimals();
            require(mintable_token.allowance(msg.sender, address(this)) >= token_mint_price*(10**token_decimals), "Allowance");
            mintable_token.transferFrom(msg.sender, address(this), (token_mint_price * (10**token_decimals)));
        ...
        // Using OOO to complete minting
        if(is_ooo_mintable) {
            IERC20 TOKEN = IERC20(ERC20_Token);
            require(TOKEN.allowance(msg.sender,address(this)) >= ooo_mint_price*(10**18), "Allowance on OOO");
            TOKEN.transferFrom(msg.sender, address(this), ooo_mint_price*(10**18));
        }
    ...
    function _mint(address to, uint256 quantity) internal returns(uint _id){
        ...
        IERC20 erc_token = IERC20(ERC20_Token);
