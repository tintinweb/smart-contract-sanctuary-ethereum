...
contract BabyZuki is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public immutable freeAmountForPublic;
    uint256 public whitelistSaleEndTime;
    uint256 internal remainingFreeMintForPublic = 0;
    ...
    function setupSaleInfo(
        ...
        saleConfig = SaleConfig(whitelistPrice, publicPrice, publicSaleKey);
    }
    ...
    function whitelistMint() external payable callerIsUser {
        ...
        require(
            totalSupply() + 1 + (amountForDevs - numberMinted(owner())) <=
                collectionSize,
            "reached max supply"
        );
        ...
        callerIsUser
    {
        ...
        uint256 publicSaleStartTime = uint256(whitelistSaleEndTime);
        require(
        ...
        if (remainingFreeMintForPublic > 0) {
            if (remainingFreeMintForPublic >= quantity) {
                remainingFreeMintForPublic -= quantity;
                quantity = 0;
            } else {
                quantity -= remainingFreeMintForPublic;
                remainingFreeMintForPublic = 0;
            }
        }
    ...
    // For marketing etc.
    function devMint(address _receiver, uint256 quantity) external onlyOwner {
        require(!paused, "Sale has not begun yet");
        require(
        ...
        if (_receiver != address(0) && _receiver != owner()) {
            require(
                numberMinted(msg.sender) + quantity <= maxPerAddressDuringMint,
                "can not mint this many"
            );
        } else if (_receiver == address(0) || _receiver == owner()) {
            require(
        ...
        uint256 endtoken = totalSupply();

        if (_receiver != address(0) && _receiver != owner()) {
            for (uint256 j = starttoken; j < endtoken; j++) {
                safeTransferFrom(msg.sender, _receiver, j);
            }
        }
    }
