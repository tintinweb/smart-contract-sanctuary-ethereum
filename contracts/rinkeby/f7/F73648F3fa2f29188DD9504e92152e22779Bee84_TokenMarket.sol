...
contract TokenMarket is
    TokenMarketBase, PausableImplementation
{
    ...
    */
        ...
        whenNotPaused
    {
        ...
        uint256 response = ITierLevel(TierLevel)
        ...
        require(checkApprovalCollaterals(loanDetails.stakedCollateralTokens, loanDetails.stakedCollateralAmounts, loanDetails.isMintSp, loanDetails.borrower), "Collateral Approval Error");
    ...
    function activateLoan(
        ...
        for (uint256 i = 0; i < loanIds.length; i++) {
            ...
            uint256 platformFee = (loanDetails.loanAmountInBorrowed *
                ...
                (10000);
            // uint256 loanAmountAfterCut = loanDetails
            ...
            );
            {
                ...
                require(checkApprovedTransferCollateralsandMintSynthetic(loanIds[i], loanDetails.stakedCollateralTokens, loanDetails.stakedCollateralAmounts, loanDetails.borrower), "Transfer Collateral Failed");
    ...
    /// @dev check ERC20 collateral token approval
    function checkApprovalCollaterals(address[] memory _collateralTokens, uint256[] memory _collateralAmounts, bool[] memory isMintSp, address borrower) internal returns (bool) {
        ...
        uint256 length = _collateralTokens.length;
         for (uint256 i = 0; i < length; i++) {
            ...
            uint256 allowance = IERC20Upgradeable(
                _collateralTokens[i]
    ...
    function checkApprovedTransferCollateralsandMintSynthetic(
        ...
        uint256 length = collateralAddresses.length;
                for (uint256 k = 0; k < length; k++) {
                    require(
                        IERC20Upgradeable(collateralAddresses[k])
                            .allowance(borrower, address(this)) >=
                            collateralAmounts[k],
        ...
        )
    {
        ...
        uint256 maxLoanAmountValue = ITierLevel(TierLevel)
    ...
    /// caller of this function will be liquidator contract
...
abstract contract PausableImplementation is PausableUpgradeable, OwnableUpgradeable {
