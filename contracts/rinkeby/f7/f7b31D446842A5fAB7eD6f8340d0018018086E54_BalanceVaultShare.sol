...
contract BalanceVaultShare is ERC721AQueryableUpgradeable, OwnableUpgradeable {
    ...
    function getImagePlainText(uint _tokenId) public view returns (string memory) {
        ...
        uint length = /* rect+text for each token amount */ 2 * tokenLength + 12 /* 11 in header + 1 in footer */;
        ...
        // <rect> for others
        output = string(abi.encodePacked(output, parts[4 + tokenLength], parts[5 + tokenLength], parts[6 + tokenLength]));
        // <text> for tokens
        for (uint i = 0; i < tokenLength; i++) {
            output = string(abi.encodePacked(output, parts[7 + tokenLength + i]));
        }
        // <text> for others + </svg>
        output = string(abi.encodePacked(output, parts[8 + 2 * tokenLength], parts[9 + 2 * tokenLength], parts[10 + 2 * tokenLength], parts[10 + 2 * tokenLength]));
