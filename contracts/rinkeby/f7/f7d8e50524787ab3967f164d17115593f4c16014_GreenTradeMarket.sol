...
contract GreenTradeMarket is Ownable, ERC721Holder {
	...
	/** Events */
    event ItemListed(uint256 id, uint256 token_id, uint256 price, address tokenAdr, address creator, address owner, uint256 creatorFee);
    ...
    function list(uint256 _token_id, uint256 _price, address _tokenAdr) OnlyItemOwner(_token_id) public {
    ...
    function delist(uint256 _id) external {        
        ...
        nftCollection.safeTransferFrom(address(this), pairs[_id].owner, pairs[_id].token_id);        
        pairs[_id].bValid = false;
        emit ItemDelisted(_id);        
    }


    function buyToOther(uint256 _id, address _to) external payable {
		...
		if (pair.tokenAdr == address(0x0)) {
			...
			// transfer coin token to creator
		...
		nftCollection.safeTransferFrom(address(this), _to, pair.token_id);
