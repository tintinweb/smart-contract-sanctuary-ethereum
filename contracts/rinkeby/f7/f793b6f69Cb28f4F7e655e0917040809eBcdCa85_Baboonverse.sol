...
contract Baboonverse is ERC721A, Ownable, VRFConsumerBase, ReentrancyGuard {
    ...
    uint256 public constant WHITELIST_MINT_PRICE = 0.005888 ether;
    ...
    function devTeamMint(uint256 quantity_, bytes32[] memory proof_) external isUser isWhitelistMint isSaleActive {
        uint256 maxSupply = currentMaxSupply();
    ...
    function baboonfrensMint(bytes32[] memory proof_) external isUser isSaleActive {
        uint256 maxSupply = currentMaxSupply();
        require(publicMintEnabled && block.timestamp >= publicMintTime, "Baboonfrens not started");
        require(MerkleProof.verify(proof_, merkleRootBaboonfrens, keccak256(abi.encodePacked(msg.sender))), "You're not Baboonfrens");
        require(totalSupply() < maxSupply, "Sold out");
        require(numberMinted(msg.sender) <= 1, "Max per wallet reached.");
        require(maxSupply >= totalSupply() + 1, "Max supply exceeded");
