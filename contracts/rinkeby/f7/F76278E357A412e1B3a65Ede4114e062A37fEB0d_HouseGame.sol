...
contract HouseGame is IHouseGame, HouseGameState, ERC721Enumerable, Ownable, Pausable {
  ...
  uint256 public immutable HOUSE_MAX_TOKENS;
  // number of tokens that can be claimed for free
  uint256 public HOUSE_PAID_TOKENS;
  // number of tokens have been minted so far
  uint16 public houseMinted;
  uint256 public constant housePresalePrice = 0.07 ether;
  uint256 public HOUSE_COST_MINT;

  // Utility building
  uint256 public constant BUILDING_MAX_MINT_PRICE = 3.5 ether;
  uint256 public constant BUILDING_FLOOR_PRICE = 0.7 ether;
  // max number of tokens that can be minted
  uint256 public immutable BUILDING_MAX_TOKENS;
  // number of tokens that can be claimed for free
  uint256 public BUILDING_PAID_TOKENS;
  // number of tokens have been minted so far
  uint16 public buildingMinted;
  uint256 public constant buildingPresalePrice = 0.7 ether;

  uint16 public minted;

  mapping(uint256 => uint256) public imageNumber;
  ...
  //public sale
  uint256 private housePriceDecrementAmt = 0.023334 ether;
  uint256 private buildingPriceDecrementAmt = 0.23334 ether;
  uint256 private timeToDecrementPrice = 5 minutes;
   ...
   */
  constructor(address _cash, address _randomizer, string memory _initBaseURI, uint256 _houseMaxTokens, uint256 _buildingMaxTokens) ERC721("House Game", 'HGAME') { 
    cash = ICASH(_cash);
    randomizer = IRandomizer(_randomizer);

    setBaseURI(_initBaseURI);

    HOUSE_MAX_TOKENS = _houseMaxTokens;
    BUILDING_MAX_TOKENS = _buildingMaxTokens;
    HOUSE_PAID_TOKENS = _houseMaxTokens / 2;
    BUILDING_PAID_TOKENS = _buildingMaxTokens;
   ...
   * mint a token
   */
  function mintHouse(uint256 amount) external payable whenNotPaused {
    require(tx.origin == _msgSender(), "Only EOA");
    require(amount > 0, "Invalid mint amount");
    require(houseMinted + amount <= HOUSE_MAX_TOKENS, "All tokens minted");
    if (houseMinted < HOUSE_PAID_TOKENS) {
      require(houseMinted + amount <= HOUSE_PAID_TOKENS, "All tokens on-sale already sold");
      if(hasPublicSaleStarted) {
        require(msg.value >= amount * currentPriceToMint(true), "Invalid payment amount");
      }
      else {
        require(amount * housePresalePrice == msg.value, "Invalid payment amount");
  ...
  function mintBuilding(uint256 amount) external payable whenNotPaused {
    require(tx.origin == _msgSender(), "Only EOA");
    require(amount > 0, "Invalid mint amount");
    require(buildingMinted + amount <= BUILDING_MAX_TOKENS, "All tokens minted");
    if (buildingMinted < BUILDING_PAID_TOKENS) {
      require(buildingMinted + amount <= BUILDING_PAID_TOKENS, "All tokens on-sale already sold");
      if(hasPublicSaleStarted) {
        require(msg.value >= amount * currentPriceToMint(false), "Invalid payment amount");
      }
      else {
        require(amount * buildingPresalePrice == msg.value, "Invalid payment amount");
        require(_whitelistAddresses[_msgSender()].isWhitelisted, "Not on whitelist");
  ...
  function currentPriceToMint(bool isHouse) view public returns(uint256) {
    ...
    if(maxPrice - decrementAmt <= floorPrice) {
      return floorPrice;
    }
  ...
  function mintCost(uint256 tokenId) public view returns (uint256) {
    if (tokenId <= HOUSE_PAID_TOKENS) return 0;
    return HOUSE_COST_MINT;
  }
  ...
  function selectTraits(uint256 seed, bool isHouse) internal view returns (HouseBuilding memory t) {    
    ...
    uint256 _imageNumber = imageNumber[t.model];

    seed >>= 16;
    t.imageId = (uint8(seed & 0xFFFF) % _imageNumber) + 1;
  }
  ...
  function setContracts(address _randomizer, address _agent) external onlyOwner {
    ...
    require(_agent != address(0), "Invalid contract address");
    randomizer = IRandomizer(_randomizer);
    agent = IAgent(_agent);
  }
  ...
  function setCostMintHouse(uint256 _houseCostMint) external onlyOwner {
    ...
    HOUSE_COST_MINT = _houseCostMint;
  }
  ...
  function setImageNumber(uint256 _model, uint256 _imageNumber) public onlyOwner {
    require(_imageNumber > 0, "Invalid set image number");
    imageNumber[_model] = _imageNumber;
  }
...
interface IHouseGame is IERC721 {
