...
contract TestCollection is ERC721, Ownable, ReentrancyGuard {
    ...
    function memberMint(uint256 tokenId, uint256 luckyNum)
        external
        payable
        nonReentrant
        callerIsUser
    {
        ...
        if (totalSupply.current() == MAX_SUPPLY) revert AllMinted();
        if (tokenId < 0 || tokenId >= MAX_SUPPLY) revert TokenIdOutOfRange();
        if (_exists(tokenId) || ownerOf(luckyNum) != msg.sender)
