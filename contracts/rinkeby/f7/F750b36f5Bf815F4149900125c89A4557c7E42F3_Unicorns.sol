...
contract Unicorns is ERC721AQueryable, ERC721ABurnable, Ownable {
    // ============ State Variables ============
    ...
    uint256 public priceAfterFirstMint = 0.005 ether;
    ...
    function mint(uint256 quantity) external payable onlyWhenNotPaused callerIsUser {
        ...
        require(numberMinted + quantity <= maxPerWallet, "Exceeding max per wallet limit");
        if (numberMinted > 0 || quantity > 1) {
            if (numberMinted > 0) {
                require(msg.value >= priceAfterFirstMint * quantity, "Need to send more ether");
            } else {
                require(msg.value >= priceAfterFirstMint * (quantity - 1), "Need to send more ether");
            }
        }
    ...
    function isFirstTimeMinting(address userAddress) public view returns (bool) {
        if (_numberMinted(userAddress) > 0) {
            return false;
        } else {
            return true;
        }
