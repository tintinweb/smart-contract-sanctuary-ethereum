...
/// TODO: Try to use UnisWap V3 0.8.0
    ...
    IERC721ReceiverUpgradeable
{
    ...
    /** @dev the digital representation of the commission in the Uniswap
    V3 is indicated as: 100 = 0.01%, 500 = 0.05%, 3000 = 0.3%, 10_000 = 1%. */
    struct OrderParams {
        uint24 fee;
        address token0;
        address token1;
    ...
    function initialize(address _nftPositionManager, address _uniswapV3Factory)
        public
        initializer
    {
    ...
    function createOrder(OrderParams memory params) external payable returns(uint256) {
        // Get pool.
        ...
        if (params.orderType == OrderType.TAKE_PROFIT) {
            require(
                params.tickLower <= TickMath.MAX_TICK - tickSpacing,
                "tickLower > MAX_TICK - tickSpacing."
            );
            require(
                params.amountOfToken0 > 0,
                "The amount of token to create an order is less than zero"
            );
            // Transfer tokens from user to us.
            IERC20Upgradeable(params.token0).safeTransferFrom(
                msg.sender,
                address(this),
                params.amountOfToken0
            );
            if (
                IERC20Upgradeable(params.token0).allowance(address(this), 
                address(nftPositionManager)) == 0
            ) {
                IERC20Upgradeable(params.token0).safeApprove(
                    address(nftPositionManager),
                    type(uint256).max
                );
            }
        } else if (params.orderType == OrderType.BUY_LIMIT) {
            require(
                params.tickLower >= TickMath.MIN_TICK,
                "tickLower > MIN_TICK."
            );
            require(
                params.amountOfToken1 > 0,
                "The amount of token to create an order is less than zero"
            );
            // Transfer tokens from user to us.
            IERC20Upgradeable(params.token1).safeTransferFrom(
                msg.sender,
                address(this),
                params.amountOfToken1
            );
            if (
                IERC20Upgradeable(params.token1).allowance(address(this), address(nftPositionManager)) ==
                0
            ) {
                IERC20Upgradeable(params.token1).safeApprove(
                    address(nftPositionManager),
                    type(uint256).max
                );
            }
        ...
        // Prepare data.
        params.tickLower = params.tickLower == 0 ? params.tickUpper - tickSpacing : params.tickLower;
        params.tickUpper = params.tickUpper == 0 ? params.tickLower + tickSpacing : params.tickUpper;
        ...
        (uint256 tokenId, uint128 liquidity, , ) = nftPositionManager.mint(
            INonfungiblePositionManager.MintParams({
                ...
                fee: params.fee,
                amount0Desired: params.amountOfToken0,
                amount1Desired: params.amountOfToken1,
                amount0Min: 0,
                amount1Min: 0,
        ...
        orders[poolAddress][tokenId] = Order({
            ...
            tickUpper: params.tickUpper,
            fee: params.fee,
            amountOfToken0: params.amountOfToken0,
            amountOfToken1: params.amountOfToken1,
            recievedAmount: params.recievedAmountOfToken1 == 0
                ? params.recievedAmountOfToken0
                : 0,
            liquidity: liquidity,
