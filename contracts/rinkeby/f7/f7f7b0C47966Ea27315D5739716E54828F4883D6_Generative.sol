    ...
    Whitelist
{
    ...
    uint256 public constant MAX_PRESALE_SUPPLY = 2500;

    uint256 public salePrice;

    uint256 public presalesMintedQty;

    constructor() ERC721A("GenerativeSound", "GSound") {}
...
contract PublicSalesActivation is Ownable {
        ...
        onlyOwner
    {
        require(
            _endTime >= _startTime,
            "PublicSalesActivation: End time should be later than start time"
        );
...
contract Whitelist is Ownable {
    ...
    address public _whitelistSigner;
    ...
    modifier isSenderWhitelisted(bytes32 hash, bytes calldata sig) {
        ...
        require(
            ECDSA.recover(messageDigest, sig) == _whitelistSigner,
            "Whitelist: Invalid signature"
        );
        require(!_signatureUsed[sig], "Whitelist: Reused Signature");
        _;
    }

    function setWhitelistSigner(address signer) external onlyOwner {
        _whitelistSigner = signer;
    }
}
