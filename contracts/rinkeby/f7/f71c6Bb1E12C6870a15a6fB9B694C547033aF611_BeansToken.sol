...
contract BeansToken is IBeansToken, Ownable, ERC721Checkpointable {
    ...
    string private _contractURIHash = 'QmQP9jRqbkHcwYnmKvgUaN2fMALFbAeHwkuBn263jTZs4X';
    ...
    function mint() public override onlyMinter returns (uint256) {
        ...
        if (_currentBeanId <= 85 && _currentBeanId % 3 == 0) {
    ...
    function _mintTo(address to, uint256 beanId) internal returns (uint256) {
        ...
        string memory dataURI = _dataURI(beanId);
        emit BeanCreated(beanId, dataURI);
...
interface IBeansToken is IERC721 {
    event BeanCreated(uint256 indexed tokenId, string dataURI);
