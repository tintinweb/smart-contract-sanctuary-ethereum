...
// Creator: 0xBasedPixel; Based Pixel Labs/Yeety Labs
...
/*
     ...
     |__/  /      \  ___/   |___   |__/
*/
// Pixel is based. No scarcity mentality nonsense.
// Infinite mint, get 10 pixel grids at a time for 0.003 ETH per pack.
...
contract BasedPixels is ERC721, Ownable {
    ...
    //    mapping(address => bool) private _ownerCheck;
    ...
    string public royaltyAddr = "0x66F3794178997a12779527B36a2F042e001625aF";
    ...
    string private collecImg = "<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' width='100%' height='100%' viewBox='0 0 16 16'><path d='M0,0L16,0,16,16,0,16' fill='black'/><path d='M3,3L3,8,6,8,6,3M4,4L7,4,7,5,4,5M4,6L7,6L7,7L4,7M9,8L9,13,10,13,10,11,12,11,12,8M10,9L13,9,13,10,10,10' fill='white'/></svg>";
    ...
    uint256 private rectStart = 27340891238026048097263813569089141950244693325490254729102276207101788291072;
    uint256 private heightNum = 14658463167467038245011812029062285656083636211927847345906851957315119087616;
    uint256 private inner1 = 57183955277861053027410940421065042940111934717952;
    uint256 private inner2 = 793586231870635386738538214064128;
    ...
    //            _randomRoots[(currentIndex/10)+10+i] = randomNumber((_randomRoots[(currentIndex/10)+9])%256, (currentIndex/10)+i);
    //            _slices[(currentIndex/10)+13+i] = randomNumber((_randomRoots[(currentIndex/10)+12])%256, (currentIndex/10)+i);
    //        }
    //        _safeMint(address(0), msg.sender, amount*10);
    //    }

    function mintTo(uint amount, address receiver) public payable {
        require(!paused, "mint is paused");
        require(isSeeded, "randomness not yet seeded");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        for (uint i = 0; i < 12; i++) {
            slices[i] = _randomRoots[mintBatchRootID+10]^_randomRoots[mintBatchRootID + (tokenId%10)];
            uint256 intermediate;

            //            if (_slices[mintBatchRootID+13]%3 == 0) {
            //                intermediate = _slices[mintBatchRootID+i]^_slices[mintBatchRootID+12];
            //            }
            //            else if (_slices[mintBatchRootID+13]%3 == 1) {
            //                intermediate = _slices[mintBatchRootID+i]&_slices[mintBatchRootID+12];
            //            }
            //            else {
            //                intermediate = _slices[mintBatchRootID+i]|_slices[mintBatchRootID+12];
            ...
            //            slices[i] = slices[i]^intermediate;

            if ((_slices[mintBatchRootID+13]>>128)%3 == 0) {
                slices[i] = slices[i]^intermediate;
            }
            else if ((_slices[mintBatchRootID+13]>>128)%3 == 1) {
                slices[i] = slices[i]&intermediate;
            }
            else {
                slices[i] = slices[i]|intermediate;
        ...
        for (uint256 i = 0; i < 4; i++) {
            for (uint256 j = 0; j < 64; j++) {
                ...
                uint256 hcR = (slices[i*3]>>(j*4))%16;
                uint256 hcG = (slices[i*3+1]>>(j*4))%16;
                uint256 hcB = (slices[i*3+2]>>(j*4))%16;
                ...
                uint256 strNum2 = heightNum+((hexChars[hcR])<<80);
                strNum2 = strNum2+((hexChars[hcG])<<72);
                strNum2 = strNum2+((hexChars[hcB])<<64)+(39<<56)+(47<<48)+(62<<40);
                rectComponents[(i*64 + j)*2] = bytes32(strNum1);
                rectComponents[(i*64 + j)*2 + 1] = bytes32(strNum2);

                uint256 redLevel = colorTracker[hcR];
                if ((redLevel>>(hcG*16+hcB))%2 == 0) {
                    uniqueColors += 1;
                    redLevel += (uint256(1)<<(hcG*16+hcB));
                    colorTracker[hcR] = redLevel;
                }
                rTotal += hcR;
                gTotal += hcG;
                bTotal += hcR;
        ...
        uint256 total = rTotal+gTotal+bTotal;
        if (total == 0) {
            total = 3;
            rTotal = 1;
            gTotal = 1;
        ...
        uint256 rDom = (rTotal*10000)/total;
        uint256 gDom = (gTotal*10000)/total;
        uint256 bDom = (bTotal*10000)/total;
        ...
        return string(
            abi.encodePacked(
                ...
                "\"value\":",Strings.toString(rDom/100),".",Strings.toString(rDom%100),"},",
                "{\"trait_type\":\"Green Dominance\",\"value\":",
                Strings.toString(gDom/100),".",Strings.toString(gDom%100),"},{\"trait_type\":",
                "\"Blue Dominance\",\"value\":",Strings.toString(bDom/100),".",Strings.toString(bDom%100),"}]}"));
    }

    function contractURI() public view returns (string memory) {
        return string(
            abi.encodePacked(
                "data:application/json;utf8,{\"name\":\"Based Pixels Testing 45.5\",",
                ...
                "Find your favorite pixel design. Happy hunting!\",",
