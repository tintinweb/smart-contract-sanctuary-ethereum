...
contract ASMSceneClaim is Ownable, ERC1155Holder {
  ...
  function _verify(
    ...
    return
      keccak256(message).toEthSignedMessageHash().recover(signature) == account;
  }

  function privateClaim(bytes memory encoded, bytes calldata signature)
    external
  {
    require(state == State.PrivateClaim, "PrivateClaim is not active");
    require(!claimed[saleNonce][msg.sender], "user has already claimed");
    require(msg.sender == tx.origin, "contracts cant mint");
    require(_verify(encoded, signature, signer), "invalid signature.");
