...
error RiggedRoll__NotFavorable(uint256);
...
contract RiggedRoll is Ownable {
    ...
    uint256 s_amountToSend = 0.002 ether;
    ...
    function riggedRoll() external {
        ...
        console.log("THE ANTICIPATED ROLL IS ", roll);

        if (roll <= 2) {
            if (address(this).balance >= .002 ether) {
                diceGame.rollTheDice{value: s_amountToSend}();
            } else {
                revert RiggedRoll__NotEnoughETH();
            }
        } else {
            revert RiggedRoll__NotFavorable(roll);
        }
    }
