...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    uint256 private _CHAIN_ID;

    bytes32 public DOMAIN_SEPARATOR = 0xd38471a54d114ee69fbb07d1769a0bbecd4f429ddf5932c7098093908e24bd9d ;
    ...
    event SetUtils(address utils);

    event SetERC20(address ERC20);

    event SetProxyRegistry(address proxyRegistry);

    event SetTokenTransferProxy(address tokenTransferProxy);
     ...
     */
    function _deriveDomainSeparator(address _utils) internal view returns (bytes32) {
        require(address(utils) != address(0), "Utils not set");
        return
        keccak256(
            abi.encode(
                ...
                Utils(_utils).chainId(),
                Utils(_utils).msgSender()
            )
        );
    }
    ...
    /**
     * @dev Change the utils (owner only)
     * @param newUitls New Utils address
     */
    function changeUtils(address newUitls)
    public
    onlyOwner
    {
        utils = Utils(newUitls);
        DOMAIN_SEPARATOR = _deriveDomainSeparator(utils);
        emit SetUtils(address(utils));
    }
    ...
    /**
     * @dev Change the exchangeToken (owner only)
     * @param exchangeTokenAddr New exchangeToken address
     */
    function changeExchangeToken(address exchangeTokenAddr)
    public
    onlyOwner
    {
        exchangeToken = ERC20(exchangeTokenAddr);
        emit SetERC20(address(exchangeToken));
    }

    /**
     * @dev Change the ProxyRegistry (owner only)
     * @param registryAddress New ProxyRegistry address
     */
    function changeProxyRegistry(ProxyRegistry registryAddress)
    public
    onlyOwner
    {
        registry = registryAddress;
        emit SetProxyRegistry(address(registry));
    }
    /**
     * @dev Change the TokenTransferProxy (owner only)
     * @param tokenTransferProxyAddress New TokenTransferProxy address
     */
    function changeTokenTransferProxy(TokenTransferProxy tokenTransferProxyAddress)
    public
    onlyOwner
    {
        tokenTransferProxy = tokenTransferProxyAddress;
        emit SetTokenTransferProxy(address(tokenTransferProxy));
    }

    modifier onlySet_ProxyRegistry_TokenTransferProxy_Utils(){
        require(address(registry) != address(0), "ProxyRegistry not set");
        require(address(tokenTransferProxy) != address(0), "TokenTransferProxy not set");
        require(address(utils) != address(0), "Utils not set");
        _;
    }
    ...
    function transferTokens(
        ...
        address to,
        uint256 amount
    ) internal onlySet_ProxyRegistry_TokenTransferProxy_Utils {
        if (amount > 0) {
    ...
    internal
    view
    onlySet_ProxyRegistry_TokenTransferProxy_Utils
    returns (bytes32)
    {
        return
        keccak256(
    ...
    function atomicMatch(
        ...
        Sig memory sellSig,
        bytes32 metadata
    ) internal reentrancyGuard onlySet_ProxyRegistry_TokenTransferProxy_Utils {
        ...
        if (buy.staticTarget != address(0)) {
            require(
                staticCall(buy.staticTarget, sell.calldatas, buy.staticExtradata)
            );
        }
