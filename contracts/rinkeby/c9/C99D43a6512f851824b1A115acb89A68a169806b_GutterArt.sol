...
contract GutterArt is ERC1155Burnable, Ownable, ReentrancyGuard {
	...
	function migrate(
		...
		uint256 quantity,
		string memory nonce
	) external payable nonReentrant canMint(signature, hash, nftID, quantity, nonce) {
    ...
    function mint(
        ...
        require(!userHasMinted[nftID][msg.sender], "already used");
		require(quantity * ethPrices[nftID] == msg.value, "wrong price");

		userHasMinted[nftID][msg.sender] = true;
		_mint(msg.sender, nftID, quantity, "");
	}
	...
	function mintWithGang(
		...
		uint256 quantity,
		string memory nonce
	) external nonReentrant canMint(signature, hash, nftID, quantity, nonce) {
        require(!userHasMinted[nftID][msg.sender], "already used");
