...
contract Sorce is ERC721A, Ownable, ReentrancyGuard {
  constructor()  ERC721A("Cult1vate Sorce", "SRC") {
  }
  
  string _baseTokenURI = "ipfs://QmUhFDn8L4kEVsC4VY9Bp6qwhUy5DV4VHw76s1S1tEooLn";
  ...
  function migrateToken(uint256 tokenId) public payable  {
      ...
      require(balanceOf(msg.sender) > 0, "Doesn't own the token"); // Check if the user own one of the sorce
      require(ownerOf(tokenId) == msg.sender, "Doesn't own the token");
