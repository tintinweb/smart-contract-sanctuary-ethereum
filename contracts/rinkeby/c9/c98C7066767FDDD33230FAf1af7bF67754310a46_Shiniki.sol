    ...
    ERC2981Upgradeable
{
    ...
    //number mint public
    mapping(address => uint256) private numberPublicMinted;

    //number mint pre
    mapping(address => uint256) private numberPreMinted;
    ...
    function initialize(
        ...
        // currentIndex[ROUND[0]][TYPE_1] = 0;
        // currentIndex[ROUND[0]][TYPE_2] = collectionSize_ / 2;
        // startIndex[ROUND[0]][TYPE_1] = 0;
        // startIndex[ROUND[0]][TYPE_2] = collectionSize_ / 2;
        revealed = false;
    ...
    // round
    // function setRound(bytes32 _round) external onlyOwner {
    //     ROUND[ROUND.length] = _round;
    // }
    ...
    function transferToken(
        ...
        require(
            IERC20Upgradeable(token).balanceOf(WALLET_REWARD) >= amount,
            "Shiniki: Token insufficient balance"
        );
    ...
    IERC721MetadataUpgradeable
{
    ...
    // round
    bytes32[] public ROUND;

    // default URI
    string public defaultURI;

    // curent index by round,type
    mapping(bytes32 => mapping(bytes32 => uint256)) public currentIndex;

    // start index by round,type
    mapping(bytes32 => mapping(bytes32 => uint256)) public startIndex;
    ...
    // round
    function getCurrentRound() public view returns (bytes32) {
        return ROUND[ROUND.length - 1];
    }

    function getRoundAt(uint256 i) public view returns (bytes32) {
        return ROUND[i];
    }

    function getAllRound() public view returns (bytes32[] memory) {
        return ROUND;
    }
    ...
    function typeTokenUri(uint256 tokenId) internal view returns (bytes32) {
        ...
        //     bytes32 round = getRoundAt(i);
        //     if (
        //         tokenId < currentIndex[round][TYPE_1] &&
        //         tokenId >= startIndex[round][TYPE_1]
        //     ) {
        //         return TYPE_1;
        //     }
        // }
    ...
    function _exists(uint256 tokenId) public view returns (bool) {
        for (uint256 i = 0; i < ROUND.length; i++) {
            ...
            if (
                (tokenId < currentIndex[round][TYPE_1] &&
                    tokenId >= startIndex[round][TYPE_1]) ||
                (tokenId < currentIndex[round][TYPE_2] &&
                    tokenId >= startIndex[round][TYPE_2])
            ) {
                return true;
            }
        }
