...
contract BridgeAssist is AccessControl {
    ...
    constructor(IERC20MintableBurnable token_, address owner) {
        _grantRole(DEFAULT_ADMIN_ROLE, owner);
        CHAIN_ID = block.chainid;
        token = token_;
    }

    function send(uint256 amount, uint256 toChainId) external {
    ...
    function fulfill(Transaction memory transaction, bytes memory signature) external {
        require(transaction.toChainId == CHAIN_ID, 'Wrong "to" chain id');
        ...
        token.mint(transaction.user, transaction.amount);
    }
