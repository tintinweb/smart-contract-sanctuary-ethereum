...
contract NFTContract is Ownable, ERC721 {
    using Strings for uint256;
  ...
  ) ERC721("NFT", "NFT", maxBatchSize_, collectionSize_) {
  ...
  function mint(uint256 _mintAmount) external payable  {
    ...
    require( numberMinted(msg.sender) + _mintAmount <= maxPerAddressDuringMint,
  ...
  function adminMint(uint256 _mintAmount, address _receiver) external onlyOwner {
    uint256 totalSupply = totalSupply();
    for(uint i; i < _mintAmount; i++) {
    _safeMint(_receiver , totalSupply + i);
     }
     delete _mintAmount;
     delete _receiver;
     delete totalSupply;
