...
contract InsuranceProvider is Ownable {
    ...
    uint256 constant ORACLE_PAYMENT = 50000000000000000;
    ...
    constructor(address _oracle1, bytes32 _jobId1, address _oracle2, bytes32 _jobId2) payable {
    ...
    function setOracleAndJob1(address _oracle, bytes32 _jobId) public {
        oracles[0] = _oracle;
        jobIds[0] = _jobId;
    }

    function setOracleAndJob2(address _oracle, bytes32 _jobId) public {
        oracles[1] = _oracle;
        jobIds[1] = _jobId;
    }
    ...
    function newContract(
        ...
        InsuranceContract i = (new InsuranceContract){value: (_payoutValue * 1 ether).div(uint256(getLatestPrice()))}(
                _client,
                _duration,
                _premium,
                _payoutValue,
        ...
        link.transfer(
            address(i),
            ((_duration.div(DAY_IN_SECONDS)) + 2) * ORACLE_PAYMENT.mul(2)
        );
    ...
    event WithdrawAll(address, uint);
...
contract InsuranceContract is ChainlinkClient, Ownable {
    ...
    string constant WORLD_WEATHER_ONLINE_KEY = "ccf822804c064ad8a38160801221704";
    ...
    string constant WEATHERBIT_KEY = "2398ba3e4ba84f8db2fd405bca28a803";
    string constant WEATHERBIT_PATH = "data,0,precip";

    uint256 constant DAY_IN_SECONDS = 60;
    uint256 constant DROUGHT_DAYS_THRESHOLD = 3;
    ...
    uint256 currentRainfall;
    uint256[2]  currentRainfallList;
    uint256  currentRainfallDateChecked;
    uint256 dataRequestsSent;
    address[2] oracles;
    bytes32[2] jobIds;
    uint256 oraclePaymentAmount;
    AggregatorV3Interface priceFeed;
    ...
    constructor(
        ...
        require(
            msg.value >= _payoutValue.div(uint256(getLatestPrice())),
        ...
        client = _client;
        startDate = block.timestamp + DAY_IN_SECONDS; //contract will be effective from the next day
        duration = _duration;
        premium = _premium;
        ...
        callFrequencyOncePerDay
    {
        ...
        if (dataRequestsSent > 1) {
            currentRainfall = (
                currentRainfallList[0].add(currentRainfallList[1]).div(2)
            );
    ...
    function checkEndContract() private onContractEnded {
        ...
        if (requestCount >= (duration.div(DAY_IN_SECONDS) - 1)) {
            ...
            //insurer hasn't done the minimum number of data requests, client is eligible to receive his premium back
            client.transfer(premium.div(uint256(getLatestPrice())));
            insurer.transfer(address(this).balance);
        }
    ...
    function getChainlinkToken() public view returns (LinkTokenInterface) {
      return LinkTokenInterface(chainlinkTokenAddress());
    }
    ...
    modifier callFrequencyOncePerDay() {
        // TODO: write modifier to ensure 1-per-day call
        _;
    }

    event contractEnded(uint256 timestamp, uint256 paidOutAmount);
    event contractPaidOut(
        uint256 timestamp,
        uint256 paidOutAmount,
        uint256 rainfallAount
    );
    event dataReceived(uint256 rainfallAmount);
    event rainfallThresholdReset(uint256 rainfallAmount);
    event contractCreated(
        address insurer,
        address client,
        uint256 duration,
