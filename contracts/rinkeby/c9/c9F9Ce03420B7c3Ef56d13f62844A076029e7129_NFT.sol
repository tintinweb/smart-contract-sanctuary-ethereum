...
contract NFT is ERC721, Ownable, ERC2981 {
  ...
  uint256 public constant WLCOST = 0.09 ether;
  uint256 public constant COST = 0.12 ether;
  uint256 public constant MAXMINTAMOUNT = 5;

  Counters.Counter private supply;
  address private t1 = 0x9C08c19d9ba09a4B47440773a21266770DFae16C; //Treasury wallet
  ...
  //Allowlist
  bytes32 public merkleRoot = 0x2f727751776094c84affb0688430a674ada76ec5a6e76f94f5efce19c6a1ce7c;
  mapping(address => bool) public whitelistClaimed;

  constructor() ERC721("10 Alphabetsss", "10ASSS") {
...
//*** PUBLIC FUNCTION ***//
  function alSaleMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable {
    require(!paused);
    require(_periodMintStatus == PeriodMintStatus.WL, "CP: Minting status invalid.");
    ...
    if (msg.sender != owner()) {
      ...
      require(msg.value >= WLCOST * _mintAmount, "Insufficient Eth.");
    }
  ...
  function publicSaleMint(uint256 _mintAmount) public payable {
    require(!paused);
    require(_periodMintStatus == PeriodMintStatus.PUBLIC, "CP: Minting status invalid.");
    require(msg.sender == tx.origin, "CP: We like real users.");
    require(_mintAmount > 0 && _mintAmount <= MAXMINTAMOUNT, "Out of mint amount limit.");
    require(supply.current() + _mintAmount <= MAXSUPPLY, "Out of supply.");
...
//*** ONLY OWNER FUNCTION **** //
  ...
  function setMintStatus(uint256 status) public onlyOwner {
    require(status <= uint256(PeriodMintStatus.PUBLIC), "CP: Out of bounds.");
  ...
  function bulkMint(uint256 _mintAmount) public onlyOwner {
    require(!paused);
    require(supply.current() + _mintAmount <= MAXSUPPLY, "Out of supply.");
  ...
  function bulkAirDrop(address[] calldata _airDropAddresses) public onlyOwner {
    require(!paused);
