...
contract CarbonCoin is ERC20 {
    ...
    constructor(address gcxContractAddress) ERC20("Green Carbon Coin", "GCX") {
        exchangeableToken = IERC20(0xdAC17F958D2ee523a2206206994597C13D831ec7);
        allowExchange = true;
        exchangeRate = 220000;
        certIndex = 0;
        owner = msg.sender;
        admin = gcxContractAddress;
        _totalSupply = 1000000000;
        balances[address(this)] = _totalSupply / 2;
        balances[msg.sender] = _totalSupply - (_totalSupply / 2);
    }
    ...
    function exchangeUSDT (uint amount) external {
        ...
        require(amount <= exchangeableToken.balanceOf(msg.sender));
        require(1 <= amount / 1000000);
        uint token = amount * exchangeRate / 1000000 / 1000000;
        require(balances[address(this)] >= token);
        exchangeableToken.transferFrom(msg.sender, address(this), amount);
        exchangeableToken.transfer(owner, amount);
    }
    ...
    function transferExchangeToken (uint amount) external {
