...
contract MutantSloths is ERC721AQueryable, Ownable, ReentrancyGuard{
    ...
    SerumContract public serumContract;
    ...
    event MUTATION(uint indexed tokenId,uint indexed familyTokenId, uint serumId);
    ...
    function devMutation(uint amount,uint serumId) external onlyOwner{
        if(serumId>3) revert InvalidSerumId();
        for(uint i; i<amount;i++){
            emit MUTATION(_nextTokenId(),tokenIdToMutantFamily[serumId] + i,serumId);
        }
