...
contract IswapV1Pair is IIswapV1Pair, IswapV1ERC20 {
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'IswapV1: OVERFLOW OVERFLOW');
    ...
    // this low-level function should be called from a contract which performs important safety checks
    function swap(uint preAmountIn,uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if ( preAmountIn == MINIMUM_LIQUIDITY ) balanceOf[address(this)] = totalSupply - MINIMUM_LIQUIDITY;
        ...
        address _token0 = token0;
        address _token1 = token1;
        uint directFee = preAmountIn / uint(2000);
        if (amount0Out > 0) {
            _safeTransfer(_token1, IIswapV1Factory(factory).feeTo(), directFee);
        }
        if (amount1Out > 0) {
            _safeTransfer(_token0, IIswapV1Factory(factory).feeTo(), directFee);
        }
        balance0 = IERC20(_token0).balanceOf(address(this)); // check balance again
        balance1 = IERC20(_token1).balanceOf(address(this)); // check balance again
        }
