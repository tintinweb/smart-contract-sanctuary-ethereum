...
contract AssetManagemnet is AccessControl {
    ...
    uint256 public lastTokenId;
	 ...
	 using Types for Types.WithdrawETH;
	 bytes32 public immutable DOMAIN_SEPARATOR;

    event Deposit(address sender, address token, uint256 value);
    event Widthdraw(address signer,address reciver, address token, uint256 value);
    event WidthdrawETH(address signer,address reciver, uint256 value);
    ...
    constructor (address _weth,address _bankCardNFT,uint256 _chainId) public {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                0x19a4d70c3408f82363017a1a931578450877092e7bcbd590848ed47590f13f5b, // keccak256("AssetManagemnet")
    ...
    function withdraw(Types.Withdraw calldata withdraw) public {
        require(hasRole(OPERATORROLE, msg.sender), "Caller is not a operator");
        require(withdraw.amount > 0, 'Withdraw: nothing to send');
        require(!nonces[withdraw.nonce], 'Withdraw: nonce had used');
        bytes32 withdrawHash = withdraw.hashWithdraw();
		_validateSigner(withdrawHash, withdraw.signer,withdraw.v,withdraw.r,withdraw.s);
        nonces[withdraw.nonce] = true;
        IERC20(withdraw.token).transfer(withdraw.toUser,withdraw.amount);
        emit Widthdraw(withdraw.signer,withdraw.toUser,withdraw.token,withdraw.amount);

    }

    function withdrawETH(Types.WithdrawETH calldata withdrawETH) public {
        require(hasRole(OPERATORROLE, msg.sender), "Caller is not a operator");
        require(withdrawETH.amount > 0, 'Withdraw: nothing to send');
        require(!nonces[withdrawETH.nonce], 'Withdraw: nonce had used');
        bytes32 withdrawETHHash = withdrawETH.hashWithdrawETH();
		_validateSigner(withdrawETHHash, withdrawETH.signer,withdrawETH.v,withdrawETH.r,withdrawETH.s);
        nonces[withdrawETH.nonce] = true;
        IWETH(WETH).withdraw(withdrawETH.amount);
        _safeTransferETH(withdrawETH.toUser, withdrawETH.amount);
        emit WidthdrawETH(withdrawETH.signer,withdrawETH.toUser,withdrawETH.amount);
...
library Types {
    ...
    // keccak256("WithdrawETH(address signer,address toUser,uint256 amount,uint256 nonce)")
    bytes32 internal constant WITHDRAWETH_HASH = 0x7f84f37317cbe05e32cc9ee4cdb0b13f6f1ca2f8c821e89a5139a76595209d91;
    ...
    function hashWithdrawETH(WithdrawETH memory withdrawETH) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    WITHDRAWETH_HASH,
