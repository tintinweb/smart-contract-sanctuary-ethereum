...
contract Swissstyle is ERC721, Ownable, PaymentSplitter {
        ...
        returns (string memory)
    {
        ...
        return
            string(abi.encodePacked(base, Strings.toString(tokenId), ".json?nameChanges=",
                Strings.toString(_nameChanges[tokenId]),
                "&nameB64=", Base64.encode(bytes(_names[tokenId]))));
    }

    function changeName(uint256 tokenId, string memory name) payable public owns(tokenId) costs(nameChangePrice(tokenId, name)) {
        _nameChanges[tokenId] += 1;
        _names[tokenId] = name;
    }
    function nameChangePrice(uint256 tokenId, string memory name) public view returns (uint256) {
        return TOKEN_PRICE;
    }
    function supportsInterface(bytes4 interfaceId)
        public
    ...
    function _mint(address to, uint256 tokenId) internal override {
        ...
        _names[tokenId] = string(abi.encodePacked("Untitled Token #", Strings.toString(tokenId)));
    }
}