...
error TeamMintIsNotActivated();
error IncorrectAddress();
error InvalidCoupon();
error PriceIsTooLow();
error NotATokenOwner();

contract TinyAstro is ERC721TA, Ownable, ReentrancyGuard {
  ...
  mapping(address => uint256) internal teamList;

  constructor() ERC721TA("TinyAstro", "TA") {
    teamList[0xd499CcA6E36dcE70Dd7ed6e05dD80a7506861512] = 5;
    teamList[0x8A5C86c9327846d32240be4b7c7611130Bc3459a] = 5;
    teamList[0x12daFc03aF81C0661262A60509b008D54CDa35A2] = 5;
    mintConfig = Config({
      maxMint: 0,
      tierNumber: 0,
      teamActive: false,
  ...
  // ==== MODIFIERS ==== //
  modifier mintGeneralCompliance( uint256 _mintAmount, uint256 _tierNumber ) {
    // validate contract is active
    // isActive is true
    if( !mintConfig.isActive ){ revert MintingNotStarted(); }

    // validate user tier number
    // tier should be equal to current config tier
    if(
        _tierNumber != mintConfig.tierNumber
    ...
    //we allow to mint only once
    //uint256 mintedAmt = numberMinted(msg.sender);
  ...
  // ==== MAIN FUNCTIONS ==== //
  // Mint Functions //
  function teamMint(uint256 _mintAmount)
  external
  nonReentrant
  {

    //active team minting
    if( !mintConfig.teamActive ){ revert TeamMintIsNotActivated(); }

    //validate msg sender
    //should be in teamList
    uint256 teamMaxMint = teamList[msg.sender];
    if( teamMaxMint == 0){ revert IncorrectTierAttempt(); }

    // validate mintAmount
    // should be > 0
    // with already minted qty should not exceed allowed config team max mint amount
    if( _mintAmount == 0 || ( numberMinted(msg.sender) + _mintAmount ) > teamMaxMint ){
      revert InvalidMintAmount();
    }
  ...
  function mintTier( uint256 _mintAmount, bytes32 _r, bytes32 _s, uint256 _v, uint256 _tierNumber )
  external
  mintGeneralCompliance(_mintAmount, _tierNumber)
  payable
  nonReentrant {
    // validate msg.value
    // should be == publicSalePrice by quantity of minting tokens
    if( msg.value < ( price * _mintAmount ) ){ revert IncorrectFundsAmount(); }


    //validate couponCode with assigned tier number,
    bytes32 digest = keccak256( abi.encode(_tierNumber, msg.sender) );
  ...
  external
  mintGeneralCompliance(_mintAmount, 4)
  payable
  nonReentrant {
    // validate msg.value
    // should be == publicSalePrice by quantity of minting tokens
    if( msg.value < ( publicSalePrice * _mintAmount ) ){ revert IncorrectFundsAmount(); }

    //validate config tierNumber
    //should be == 4 for public sale
    if(mintConfig.tierNumber != 4){ revert PublicMintIsNotActivated(); }
  ...
  nonReentrant {
    ...
    require(ownership.addr == msg.sender, "Not token owner");
    _burn(_tokenId);
  }

  // Public View Functions //
  //walletOfOwner - do we need this ? 
  ...
  function getMintData(address _owner) public view returns( uint256, uint256, uint256 ){
    return (
      numberMinted(_owner),
      totalSupply(),
      mintConfig.tierNumber
    );
  }
  ...
  function setPrice(uint256 _price) external onlyOwner {
    ...
    if( _price < 10000000000000000 ){ revert PriceIsTooLow(); }
    price = _price;
  }
  ...
  function pauseContract() external onlyOwner {
    ...
    mintConfig.teamActive = false;
    mintConfig.isActive = false;
  }

  function setActiveTeamMint() external onlyOwner {
    mintConfig.tierNumber = 0;
    mintConfig.maxMint = 0;
    mintConfig.teamActive = true;
    mintConfig.isActive = false;
  }
  ...
  function addToTeamList(address _teamMember, uint256 _amt) external onlyOwner {
    teamList[_teamMember] = _amt;
  }

  function removeFromTeamList(address _teamMember) external onlyOwner {
    teamList[_teamMember] = 0;
  }
  ...
  function withdraw() external onlyOwner {
    //payment split if needed?
    //(bool hs, ) = payable(0x00).call{value: address(this).balance * 5 / 100}("");
    //require(hs);
  ...
  function _isCouponValid( bytes32 digest, bytes32 _r, bytes32 _s, uint256 _v )
  internal
  view
  returns (bool) {
    address signer = ecrecover(digest, uint8(_v), _r, _s);
    if( signer == address(0) ){ revert IncorrectAddress(); }
 ...
 */
contract ERC721TA is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner = ERC721TA.ownerOf(tokenId);
