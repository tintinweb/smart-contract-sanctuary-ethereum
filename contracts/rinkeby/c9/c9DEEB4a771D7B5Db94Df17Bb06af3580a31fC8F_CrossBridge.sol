...
contract CrossBridge is AccessControl {
    ...
    function redeem(uint _amount, uint64 _chainFrom, uint64 _chainTo, uint _nonce, string memory _symbol, uint8 v, bytes32 r, bytes32 s) public {
        ...
        bool isValid = checkSign(hashed, v, r, s);

        require(isValid, "CrossBridge::redeem:signature not valid");
        hashWasUsed[hashed] = true;
        IERC20MintBurn(addressBySymbol[_symbol]).mint(msg.sender, _amount);
    }

    function checkSign(bytes32 hashed, uint8 v, bytes32 r, bytes32 s) private view returns(bool) {
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        bytes32 ethSignedMessageHash = keccak256(abi.encodePacked(prefix, hashed));
    ...
    function getMessageHash(address _to, uint _amount, uint64 _chainFrom, uint64 _chainTo, uint _nonce, string memory _symbol) private pure returns(bytes32) {
