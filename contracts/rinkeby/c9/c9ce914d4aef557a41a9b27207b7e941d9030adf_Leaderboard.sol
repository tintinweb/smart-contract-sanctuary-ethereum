...
// File: contracts/test_alex_rank.sol
...
contract Leaderboard {
  ...
  event Change_token(address indexed oldtoken, address indexed token);
  ...
  function changeToken(address new_token) public onlyOwner() returns (bool){
  ...
  function see_rank_address(uint256 position)public view returns(address){
    return leaderboard[position]._address;
  }

  function see_rank_score(uint256 position)public view returns(uint256){
    return leaderboard[position].score;
  }

  function see_rank_name(uint256 position) public view returns (string memory){
    return leaderboard[position].user;
  }
