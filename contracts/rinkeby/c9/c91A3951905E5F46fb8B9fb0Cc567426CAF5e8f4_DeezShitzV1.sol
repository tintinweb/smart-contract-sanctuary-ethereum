...
contract DeezShitzV1 is ERC721A, Ownable, ReentrancyGuard {
    bool public flushToilet = false;  // end pooping
    uint256 public MAX_SHITZ_SUPPLY = 2000;  // total maximum shitz
    ...
    constructor() ERC721A("DeezShitzV1", "DEEZSHITZV1") {}


    function pooping(uint256 _shitzQuantity) external payable nonReentrant {
        require(flushToilet);
        require(_shitzQuantity + _numberMinted(msg.sender) <= maxShitzMint, "Don't be greedy, share the shitz");
        require(totalSupply() + _shitzQuantity <= MAX_SHITZ_SUPPLY, "No more shitz");
        require(msg.sender == tx.origin);

        // temporary
        require(msg.value >= (mintRate * _shitzQuantity), "Not enough ether sent");
