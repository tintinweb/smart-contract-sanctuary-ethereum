...
contract MogiesDutchAuction is Ownable, ERC721A, ReentrancyGuard {
    ...
    function mintRemaining() external callerIsUser auctionAndSalesEnded {
        ...
        if (usersBonusNotMinted + usersBonusMinted > totalRemainingMintAmount) {
            quantity = 0;
        } else if (
            usersBonusNotMinted + usersBonusMinted == totalRemainingMintAmount
        ) {
            quantity = 1;
        } else {
            quantity =
                totalRemainingMintAmount /
                (usersBonusNotMinted + usersBonusMinted);
        }
        ...
        if (
            ...
            totalRemainingMintAmount % (usersBonusNotMinted + usersBonusMinted)
        ) {
            quantity++;
        }
