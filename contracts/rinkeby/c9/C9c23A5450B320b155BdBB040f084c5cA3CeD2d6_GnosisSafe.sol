    ...
    GuardManager
{
   ...
   function execTransaction(
        ...
        require(gasleft() >= ((safeTxGas * 64) / 63).max(safeTxGas + 2500) + 2500);
        // Use scope here to limit variable lifetime and prevent `stack too deep` errors
    ...
    function checkNSignatures(
        ...
        require(signatures.length >= requiredSignatures.mul(65));
        ...
        for (i = 0; i < requiredSignatures; i++) {
            ...
            if (v == 0) {
                ...
                require(uint256(s) >= requiredSignatures.mul(65));

                // Check that signature data pointer (s) is in bounds (points to the length of data -> 32 bytes)
                require(uint256(s).add(32) <= signatures.length);
                ...
                require(uint256(s).add(32).add(contractSignatureLen) <= signatures.length);
                ...
                require(ISignatureValidator(currentOwner).isValidSignature(data, contractSignature) == EIP1271_MAGIC_VALUE);
            } else if (v == 1) {
