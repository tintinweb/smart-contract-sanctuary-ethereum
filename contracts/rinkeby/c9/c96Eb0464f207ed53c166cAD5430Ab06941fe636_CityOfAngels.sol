...
contract CityOfAngels is Ownable, ERC721A, ReentrancyGuard {

  // Contract Constants
  uint256 constant public cost = 0.05 ether;
  uint256 constant public maxSupply = 5678;
  ...
  uint256 public teamClaimedAmount = 0;
  ...
  constructor() ERC721A("CityOfAngels", "CoA",5678) {}
    ...
    callerIsUser
  {
    ...
    require(quantity <= maxMintAmount, "Exceeds 3, the max qty per mint");
  ...
  function teamMint(uint256 quantity) external onlyOwner {
    require(
      ...
      "Team claim exceeds total supply!" 
    );
    require(
      teamClaimedAmount + quantity <= teamClaimAmount,
      "You have already minted max team amount!"
    );
  ...
  function getIsWhitelisted(address _addr, bytes32[] calldata _merkleProof)
    external
    view
    returns (bool)
  {
