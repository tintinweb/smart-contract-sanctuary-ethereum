...
contract MasterGame is Game, TeamManager {

    uint constant VERSION = 10;
    ...
    mapping (uint => mapping (uint => capcha)) capchas; // [team][round]

    // Payments + Points
    uint paymentAmount = 1;
    uint paymentDecimals = 18;
    uint totalPayments = 0;
    mapping (address => uint) public userBalances;
    mapping (uint => uint) public teamBalances;
    ...
    function pressButton(string memory _answer, string memory _newQuestion, string memory _newAnswer) public payable {
        uint _teamId = getTeamByAddress(msg.sender);
        uint _round = currentRound();
        require(msg.value >= paymentAmount * 10**paymentDecimals, "Payment not enough.");
        require(buttonReady(), "Button not ready.");
        require(getTurnAddress(_teamId) == msg.sender, "Not your turn"); 
        require(teamAlive(_teamId) == true, "Your team already lost.");
        require(capchas[_teamId][_round].solved == false, "Capcha already solved.");
        require(capchas[_teamId][_round-1].answer == keccak256(abi.encodePacked(_answer)), "Wrong answer. Ask for help if you need to.");
        capchas[_teamId][_round].solved == true;
        ...
        capchas[(_teamId + 1) % numTeams()][_round+1] = _capcha;
         // increment turn for team
        rotateTurn(_teamId);
        // log payments + points
        userBalances[msg.sender] += msg.value;
        teamBalances[_teamId] += msg.value;
        totalPayments += paymentAmount * 10**paymentDecimals;
    }

    function createTeam(address[] memory _members, string memory _name) public override {
        require(startTime == 0, "Game already started");
        capchas[numTeams()][0] = capcha({
        ...
        super.createTeam(_members,_name);
    }

    function createDummyTeam() public {
        address[] memory a = new address[](1);
        a[0] = 0xb4005DB54aDecf669BaBC3efb19B9B7E3978ebc2;
        createTeam(a,"McDonalds");
    }

// Payments

    function emergencyWithdrawal() public payable {
        require(emergency, "Not an emergency");
    ...
    function withdrawWinnings() public payable {
        require (inTeam(msg.sender, winningTeam()), "You are not on winning team");
        uint amount = totalPayments * userBalances[msg.sender] / teamBalances[winningTeam()];
        userBalances[msg.sender] = 0;
        payable(msg.sender).transfer(amount);
    }
    ...
    function setEmergency(bool _emergency) public {
        require(false, "TO DO");
...
// View Functions
    ...
    function getCapcha(uint _teamId, uint _round) public view returns (capcha memory){
        return capchas[_teamId][_round];
    }

    function teamAlive(uint _teamId) public view returns (bool) {
        for (uint r = 1; r < currentRound(); r++){
                if (!capchas[_teamId][r].solved)
                    return false;
            }  
        return true;
    }

    function aliveStatuses() public view returns (bool[] memory){ // TO DO -- ACCOUNT FOR ROUND 1
        uint _round = currentRound();
        // for each team, set '_aliveStatus' as true 
        bool[] memory _aliveStatus = new bool[](numTeams());
        for (uint t = 0; t < numTeams(); t++){
            _aliveStatus[t] = true;
        }
        // check previous rounds for unsolved capcha
        for (uint r = 0; r < _round; r++){
            for (uint t = 0; t < numTeams(); t++){
                if (!capchas[t][r].solved)
                    _aliveStatus[t] = false;
            }
        }
        // if we are past the deadline + graceperiod, test the current round
        if (block.timestamp >= deadline() + gracePeriod){
            for (uint t = 0; t < numTeams(); t++){
                if (!capchas[t][_round].solved)
                    _aliveStatus[t] = false;
            }
        }
        return _aliveStatus;
    ...
    function aliveTeamCount() public view returns (uint){
        // count teams that are still alive
        bool[] memory _aliveStatus = aliveStatuses();
        uint _results = 0;
        for (uint i = 0; i < _aliveStatus.length;i++){
            if (_aliveStatus[i] == true)
                _results++;
        }
        return _results;
    }

    function winningTeam() public view returns (uint) {
        require (aliveTeamCount() == 1, "Only one team can be alive for a winner to exist.");
        bool[] memory _aliveStatus = aliveStatuses();
        for (uint i = 0; i < _aliveStatus.length;i++){
            if (_aliveStatus[i] == true)
                return i;
...
contract Game  { //is Ownable
    ...
    constructor(){
        ...
        intervalDecay = 5; // percent rate decay
...
// View Functions
    ...
    function roundByTime(uint _timestamp) public view returns (uint) {
        uint _round = (_timestamp - startTime) /  interval;
        return (_round + 1);
    } 

    function deadlineByRound(uint _round) public view returns (uint) {
        uint _timestamp = ((_round - 1) *  interval) + startTime;
        return (_timestamp);
    } 

    function buttonReady() public view returns (bool) {
        uint _now = block.timestamp;
        uint _round = roundByTime(_now);
        uint _deadline = deadlineByRound(_round);
        return (_now >= _deadline && _now <= _deadline + gracePeriod);
    }

    function currentRound() public view returns (uint){
        return roundByTime(block.timestamp);
    }

    function deadline() public view returns (uint){
        uint _round = roundByTime(block.timestamp);
        return deadlineByRound(_round);
    }

    // function decayed(uint256 t) public view returns (uint256 price) {
    //     uint value = interval;
    //     uint halfLife = 50;
    //     value >>= (t / halfLife);
    //     t %= halfLife;
...
// Team Management
    ...
    function createTeam(address[] memory _members, string memory _name) public virtual {
    ...
    function nominateMember(uint _teamId, address _newAddress) public { // to-do: require you are on team
...
// View functions
    ...
    function getTurnIndex(uint _teamId) public view returns (uint) {
        return teams[_teamId].turn(0);
    }

    function getTurnAddress(uint _offsetIndex, uint _teamId) public view returns (address) {
        uint _turn = teams[_teamId].turn(_offsetIndex);
        return teams[_teamId].members(_turn);
    }

    function getTurnAddress(uint _teamId) public view returns (address) {
        return getTurnAddress(0,_teamId);
    }
...
contract Team {
    ...
    mapping (address => mapping (address => bool)) private nomineeApprovals;
