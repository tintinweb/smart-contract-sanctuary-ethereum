...
contract Shrumies is ERC721F {
    ...
    address public mintpass = 0x05269d482d4D5a81e338a9E09ca2965Bf70154c6;  
    address private constant FRANK = 0xF40Fd88ac59A206D009A07F8c09828a01e2ACC0d;
    address private constant SONCHO = 0x55978D59448ff192a8409155AD210E3869f21755;
    ...
    address private constant MUSAL = 0x045474f0d29cc3D2C3c1ff6dd262f242A64D1B3B;

    mapping(address => uint256) private mintAmount;
    
    event priceChange(address _by, uint256 price);

    constructor() ERC721F("Shrumies", "SHR") {
    ...
    */
    function preSalemint(uint256 numberOfTokens) external payable {
        ...
        if (IERC1155(mintpass).balanceOf(msg.sender,1)>0){  // OG = 3 max mint
            require(mintAmount[msg.sender]+numberOfTokens<4,"Purchase would exceed max mint for walet");
        } else if (IERC1155(mintpass).balanceOf(msg.sender,2)>0){  // WL = 2 max mint
            require(mintAmount[msg.sender]+numberOfTokens<3,"Purchase would exceed max mint for walet");
        } 
    ...
    */
    function hasMintPass(address sender) public view returns (bool){
        if(sender==address(0)){
            return false;
        } else if (IERC1155(mintpass).balanceOf(sender,1)>0){  // OG = 3 max mint
            return true;
        } else if (IERC1155(mintpass).balanceOf(sender,2)>0){  // WL = 2 max mint
            return true;
        } 
        return false;
    }
    ...
    function withdraw() public onlyOwner {
        ...
        _withdraw(SONCHO,(balance * 30) / 100);
        _withdraw(FRANK,(balance * 20) / 100);
        _withdraw(DAVE,(balance * 15) / 100);
        _withdraw(MUSAL,(balance * 10) / 100);
        _withdraw(JLOW,(balance * 10) / 100);
