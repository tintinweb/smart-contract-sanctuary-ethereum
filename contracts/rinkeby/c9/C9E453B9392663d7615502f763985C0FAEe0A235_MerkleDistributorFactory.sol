...
contract MerkleDistributorFactory {
  ...
  constructor() {
    MERKLE_DISTRIBUTOR_MASTER_COPY = address(
      new MerkleDistributor(address(1), address(1), bytes32(""), 0)
    );
  }
  ...
  function saltDistributor(
    ...
    return
      keccak256(
        abi.encodePacked(albumSafeAddress, token, merkleRoot, claimDuration)
      );
  }
  ...
  function predictMerkleDistributorAddress(
    ...
    predicted = Clones.predictDeterministicAddress(
      ...
      saltDistributor(albumSafeAddress, token, merkleRoot, claimDuration)
    );
  }
...
contract MerkleDistributor is IMerkleDistributor {
  ...
  constructor(
    ...
    initialize(albumSafe, token, merkleRoot, claimDuration);
  }
