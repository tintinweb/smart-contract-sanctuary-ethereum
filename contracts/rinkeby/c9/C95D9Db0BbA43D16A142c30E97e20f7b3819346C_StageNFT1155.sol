...
// File: StageNFT1155Auctions.sol
//SPDX-License-Identifier: UNLICENSED

pragma solidity ^0.8.0;
contract StageNFT1155Auction {  //is SpaceERC20 
    event availableForBids(uint, string) ;
    event removeFormSale (uint, string );
    enum status {NotOnSale ,onAuction, onBidding, OnfixedPrice, notOnfixedPrice}
    ...
    modifier notOnFixedPrice (address owner, uint nftId){
        require( Nft[owner][nftId].salestatus == status.notOnfixedPrice, "NFT is Already Available for Fixed Price");
        _;
    }
    modifier onFixedPrice (address owner, uint nftId){
        require( Nft[owner][nftId].salestatus == status.OnfixedPrice, "NFT is Not on Fixed Price");
        _;
    }
//    
...
//     // }
    ...
    function _placeNftForFixedPrice(address owner ,uint nftId, uint Fixedamount )  NftExist(owner , nftId) notOnFixedPrice(owner,nftId) internal{ 
        //CurrentStatus = status(3);
        //Nft[owner][nftId].salestatus = CurrentStatus;
        Nft[owner][nftId].salestatus = status.OnfixedPrice;
        Nft[owner][nftId].minimumPrice = Fixedamount;
    }

    function _removeFromFixedPrice(address ownerAddress, uint nftId) NftExist(ownerAddress,nftId) onFixedPrice(ownerAddress,nftId) internal {
        // check Already on Sale 
        CurrentStatus = status(0);
        Nft[ownerAddress][nftId].salestatus = status.notOnfixedPrice;
...
// File: StageNFT1155Royalties.sol
...
contract StageNFTErc20 {
    ...
    function _royaltyAndStageNFTFee (uint _NftPrice, uint percentage, address payable minterAddress, address payable NftSeller) internal  {
        ...
        uint _StageNFTFee = _calculateStageNFTFee(_NftPrice);
    ...
    function _calculateStageNFTFee(uint Price) internal pure returns(uint) {
...
abstract contract ERC1155Burnable is ERC1155 {
    function burn(
        ...
        _burn(account, id, numOfCopies);
    }

    function burnBatch(
        address account,
        uint256[] memory ids,
        uint256[] memory numOfCopies
    ) public virtual {
        require(
            account == _msgSender() || isApprovedForAll(account, _msgSender()),
            "ERC1155: caller is not owner nor approved"
        ...
        _burnBatch(account, ids, numOfCopies);
    }
}
// File: StageNFT1155.sol
...
contract StageNFT1155 is ERC1155, ERC1155Burnable, StageNFTErc20 ,StageNFT1155Auction { 
    ...
    */
    ...
    function mintLazyMintedNfts (address to, uint tokenID, uint numOfCopies, bytes memory data, string memory tokenURI, uint NftPrice, address payable MinterAddress, uint RoyaltyValueOfMinter) contractIsNotPaused public payable{
        ...
        Nft[_msgSender()][tokenID].salestatus = status.notOnfixedPrice;
        //Send Amount to Local Minter
        // Deduct Royalties
        _royaltyAndStageNFTFee(NftPrice*numOfCopies, RoyaltyValueOfMinter, MinterAddress, MinterAddress );
    ...
    */
    function StageNFTsafeTransferFrom(address from, address to, uint id, uint numOfCopies, bytes memory data ) internal {
        _royaltyAndStageNFTFee( ((NFT_Price[from][id])*numOfCopies), _royalties[id].percentage, _royalties[id].recipient, payable(from));
