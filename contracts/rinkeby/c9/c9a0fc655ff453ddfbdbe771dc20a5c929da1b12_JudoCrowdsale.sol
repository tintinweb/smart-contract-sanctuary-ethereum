...
contract JudoCrowdsale is Ownable {
    using SafeMath for uint256;
    ...
    // Start time 02/08/2022 @ 1:30pm (UTC) //
    uint256 public constant CROWDSALE_START_TIME = 1644327000;
    ...
    // Pointer to the JudoToken
    IERC20 public judoToken;

    // How many judos do we send per BNB contributed.
    uint256 public JudosPerBnb;
    // How many judos do we send per BUSD contributed.
    uint256 public JudosPerBusd;

    IERC20 public busdToken = IERC20(0xeb8f08a975Ab53E34D8a0330E0D34de942C95926);

    
    //===============================================//
    //                 Constructor                   //
    //===============================================//
    constructor(uint256 _judosPerBnb, uint256 _judoPerBusd) {
        JudosPerBnb = _judosPerBnb;
        JudosPerBusd = _judoPerBusd;
    }
    ...
    function purchaseFINOTokensInBNB() external payable {
          // Validations.
            require(
                msg.sender != address(0),
                "JudosCrowdsale: beneficiary is the zero address"
            );
            ...
            require(msg.value >= MIN_CONTRIBUTION_BNB, "FINOCrowdsale: minimum contribution is 0.05 BNB");
            require(contributionsInBNB[msg.sender] + msg.value <= MAX_CONTRIBUTION_BNB, "FINOCrowdsale: maximum contribution is 50 BNB");
            
            // If we've passed validations, let's get them $FINOs
            _buyTokensInBNB(msg.sender);
      
    }

    // Main entry point for buying into the Pre-Sale. Contract Receives $BUSD
    function purchaseFINOTokensInBUSD(uint256 _busdAmount) public  {
          // Validations.
            require(
                msg.sender != address(0),
                "JudosCrowdsale: beneficiary is the zero address"
            ...
            require(_busdAmount >= MIN_CONTRIBUTION_BUSD, "FINOCrowdsale: minimum contribution is 20 BUSD");
            require(contributionsInBUSD[msg.sender] + _busdAmount <= MAX_CONTRIBUTION_BUSD, "FINOCrowdsale: maximum contribution is 20000 BUSD");
           
            // If we've passed validations, let's get them $FINOs
            _buyTokensInBUSD(msg.sender, _busdAmount);
      
    }

    /**
     * Function to calculate how judo `weiAmount` can the sender purchase
     * based on total available cap for this round, and how judo bnb they've contributed.
     *
     * At the end of the function we refund the remaining BNB not used for purchase.
     */
    function _buyTokensInBNB(address beneficiary) internal {
        
        _buyTokensInBNB(beneficiary, msg.value);

    }

    /**
     * Function that perform the actual transfer of $FINOs
     */
    function _buyTokensInBNB(address beneficiary, uint256 _amount) internal {
    ...
    function _buyTokensInBUSD(address beneficiary, uint256 _busdAmount) internal {
    ...
    function claimTokens() public {
        ...
        // Calculate how many $FINOs can be bought with contributed amount
        ...
        // Transfer the $FINOs to the beneficiary
        judoToken.safeTransfer(msg.sender, finalTokenAmount);
    ...
    // Calculate how many $FINOs do they get given the amount of wei
    function _getTokenAmountBNB(uint256 weiAmount) internal view returns (uint256)
    {
        return weiAmount.mul(JudosPerBnb);
    }

    // Calculate how many $FINOs do they get given the amount of BUSD
    function _getTokenAmountBUSD(uint256 _amount) internal view returns (uint256)
    {
        return _amount.mul(JudosPerBusd);
    }
    ...
    function changeJudoBnbRate(uint256 _newRate) public onlyOwner returns(bool) {
        require(_newRate != 0, "New Rate can't be 0");
        JudosPerBnb = _newRate;
        return true;
    }
    ...
    function addTokenAddress(IERC20 _judoToken)public onlyOwner{
        judoToken = _judoToken;
    }
    
    function takeOutRemainingTokens() public onlyOwner {
        judoToken.safeTransfer(msg.sender, judoToken.balanceOf(address(this)));
    }
