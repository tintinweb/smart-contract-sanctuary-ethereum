     ...
     RoyaltiesV2Impl 
{
    ...
    function mint(
        ...
        uint256 _quantity,
        bytes memory _data
    ) public virtual onlyOwnerOrProxy  returns (uint256){
...
contract AssetContract is ERC1155Tradable{
    ...
    function safeTransferFromV2(
        ...
        if (mintedBalance < _amount) {
            ...
            mint(_to,_royalty,_id, _amount - mintedBalance, _data);
            if (mintedBalance > 0) {
    ...
    function safeBatchTransferFromV2(
        address _from,
        address _to,
        LibPart.Part memory _royalty,
        uint256[] memory _ids,
        ...
        require(
            _ids.length == _amounts.length,
            "safeBatchTransferFrom: INVALID_ARRAYS_LENGTH"
        );
        for (uint256 i = 0; i < _ids.length; i++) {
            safeTransferFromV2(_from, _to, _royalty, _ids[i], _amounts[i], _data);
        }
 ...
 */
contract LasquareOpenseaStore is
    AssetContract,
    ReentrancyGuard
{
    ...
    function mint(
        ...
        uint256 _quantity,
        bytes memory _data
    ) public override nonReentrant creatorOnly(_id) returns (uint256) {
        _mint(_to, _id, _quantity, _data);
        _saveRoyalties(_id, _royalty);
        return _id;
    }
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            require(
                ...
                "AssetContract#_batchMint: ONLY_CREATOR_ALLOWED"
            );
...
abstract contract AbstractRoyalties {
        ...
        internal
    {
        ...
        if (royalties[_id].account == address(0x0)) {
