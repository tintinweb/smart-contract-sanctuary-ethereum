...
contract NFTBadge is Initializable{
    ...
    function userTransfer(uint256 id, address from, address to) private {
        ...
        InterfaceNFTToken(NFTTokenAddress).removeBadgeList(from, InterfaceNFTToken(NFTTokenAddress).findBadgeIndex(from, id), msg.sender, mapTokenRole[id]);
        if (InterfaceNFTToken(NFTTokenAddress).findBadgeIndex(to, id) < int(0)) {
            InterfaceNFTToken(NFTTokenAddress).addUserBadge(to, id,  msg.sender);
        }
    }
    ...
    function issueBadge(address _receiverAddress, Badge memory _badge, string memory _uri, Attribute [] memory _attributes) public {
        ...
        InterfaceNFTToken(NFTTokenAddress).addUserBadge(_receiverAddress,_badge.tokenId,  msg.sender);
    ...
    function voidBadge(address _target, string memory _id, uint256 _amount, string memory _reason) public {
        ...
        InterfaceNFTToken(NFTTokenAddress).removeBadgeList(_target, InterfaceNFTToken(NFTTokenAddress).findBadgeIndex(_target, mapId[_id]), msg.sender, mapTokenRole[mapId[_id]]);
