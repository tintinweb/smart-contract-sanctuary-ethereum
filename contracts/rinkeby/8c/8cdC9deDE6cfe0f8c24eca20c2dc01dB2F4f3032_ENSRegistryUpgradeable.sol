 ...
 */
contract ENSRegistryUpgradeable is ENS, Initializable, UUPSProxy {
    struct Record {
        address owner;
        address resolver;
        uint64 ttl;
    ...
    modifier authorised(bytes32 _node) {
        address recordOwner = records[_node].owner;
        require(
            recordOwner == msg.sender || operators[recordOwner][msg.sender],
            "ENSRegistry: not authorized"
        );
        _;
    }
     ...
     * @param _recordOwner The address of the new owner.
    ...
    function setSubnodeRecord(
        ...
        bytes32 subnode = setSubnodeOwner(_node, _label, _recordOwner);
     ...
     * @param _recordOwner The address of the new owner.
     */
    function setOwner(bytes32 _node, address _recordOwner)
        public
        virtual
        override
        authorised(_node)
    {
        _setOwner(_node, _recordOwner);
        emit Transfer(_node, _recordOwner);
    }
    ...
    function setSubnodeOwner(
        ...
        _setOwner(subnode, _recordOwner);
        emit NewOwner(_node, _label, _recordOwner);
        return subnode;
    }
     ...
     * @param _recordOwner The address that owns the records.
     * @param _operator The address that acts on behalf of the owner.
     * @return True if `operator` is an approved operator for `owner`, false otherwise.
     */
    function isApprovedForAll(address _recordOwner, address _operator)
        external
        view
        virtual
        override
        returns (bool)
    {
        return operators[_recordOwner][_operator];
    }

    function _setOwner(bytes32 _node, address _recordOwner) internal virtual {
        // Includes fix from new ENSRegistry
        // https://github.com/ensdomains/ens-contracts/blob/v0.0.8/contracts/registry/ENSRegistryWithFallback.sol#L59-L67
        address addr = _recordOwner;
        if (addr == address(0x0)) {
            addr = address(this);
        }
        records[_node].owner = addr;
    }
