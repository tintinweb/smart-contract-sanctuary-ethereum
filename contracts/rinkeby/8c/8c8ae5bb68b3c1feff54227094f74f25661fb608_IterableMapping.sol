...
contract NMCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("NMC_Dividend_Tracker", "NMC_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "NMC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "NMC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NMC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "NMC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "NMC_Dividend_Tracker: Cannot update claimWait to same value");
...
/**
•	Tax 11.5% automatically charge
o	5% tax divided among holders
o	5% added back to liquidity
o	1.5% Sadaqah Wallet
1 Quadrillion Tokens 1,000,000,000,000,000

Token Name: Nifty Muslim Coin
Token Symbol: NMC
*/
contract NiftyMuslimCoin is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    NMCDividendTracker public dividendTracker;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _sadaqahFee, uint256 _burnFee) public onlyOwner {
        ETHRewardFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        sadaqahFee = _sadaqahFee;
        burnFee = _burnFee;

        totalFees = ETHRewardFee.add(liquidityFee).add(sadaqahFee).add(burnFee); // total fee transfer and buy
    }

    function setSellFee(uint256 _onSellbnbRewardFee, uint256 _onSellliuidityFee, uint256 _onSellsadaqahFee, uint256 _onSellBurnFee) public onlyOwner {
        onSelltotalFees = _onSellbnbRewardFee;
        onSellliquidityFee = _onSellliuidityFee;
        onSellsadaqahFee = _onSellsadaqahFee;
        onSellburnFee = _onSellBurnFee;

        onSelltotalFees = onSellETHRewardFee.add(onSellliquidityFee).add(onSellsadaqahFee).add(onSellburnFee);
    }
    ...
    function setSadqahWallet(address payable _newSadqahWallet) public onlyOwner {
        SadqahWallet = _newSadqahWallet;
    }

    constructor() ERC20("Nifty Muslim Coin", "NMC-1") { // please check this name and symbol 
        ETHRewardFee = 5;
        liquidityFee = 5;
        sadaqahFee = 2;
        burnFee = 0;

        onSellETHRewardFee = 5;
        onSellliquidityFee = 5;
        onSellsadaqahFee = 2;
        onSellburnFee = 0;

        SadqahWallet = payable(0x7015fB49af16A9B38D408CB044570F3237b103FE);

        totalFees = ETHRewardFee.add(liquidityFee).add(sadaqahFee).add(burnFee); // total fee transfer and buy

        onSelltotalFees = onSellETHRewardFee.add(onSellliquidityFee).add(onSellsadaqahFee).add(onSellburnFee);

    	dividendTracker = new NMCDividendTracker();
        ...
        excludeFromFees(SadqahWallet, true);
        excludeFromFees(address(this), true);
        
        // exclude from max tx
        _isExcludedFromMaxTx[owner()] = true;
        _isExcludedFromMaxTx[address(this)] = true;
        _isExcludedFromMaxTx[SadqahWallet] = true;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "NMC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "NMC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "NMC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "NMC: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "NMC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "NMC: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(onSelltotalFees.sub(onSellliquidityFee)).mul(onSellsadaqahFee);
        SadqahWallet.transfer(marketingAmount);
    ...
    function swapAndSendBNBToMarketing(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, SadqahWallet);
    }
