    ...
    MerkleLeaves
{
    ...
    ) ERC721A(__name, __symbol, MAX_BATCH_SIZE) PaymentSplitter(__addresses, __splits) {
        baseURI = __baseURI;
    ...
    function reservePCTokens(address[] memory friends, uint256 count) external onlyOwner {
        require(0 < count && count <= MAX_BATCH_SIZE, 'Invalid count');
    ...
    function claimPCTokens(bytes32[] calldata proof, uint256 count) external payable nonReentrant {
        require(mintControlFlags.getBoolean(CLAIMING_PHASE), 'Claiming stopped');
        require(0 < count && count <= MAX_BATCH_SIZE, 'Invalid count'); // TODO dont need to check this, checked within erc721a.
        require(msg.value >= claimPricePerNft * count, 'Invalid price');

        _claimPCTokens(_msgSender(), proof, count);
    }

    function presalePCTokens(bytes32[] calldata proof, uint256 count)
        external
        payable
        nonReentrant
    {
        ...
        _presalePCTokens(_msgSender(), proof, count);
    }

    /**
     * Purchase bound by terms & conditions of project.
     */
    function mintPCTokens(uint256 count) external payable nonReentrant onlyUsers {
    ...
    function _internalMintTokens(address minter, uint256 count) internal {
        require(totalSupply() + count <= maxNftsForSale, 'Limit exceeded');
 ...
 * @dev Helper contract to protect against contract based mint spamming attacks.
 */
...
/**
 * @title Merkle Tree Leaf Functionality
 * @author @NiftyMike, NFT Culture
 * @dev This is a companian abstract contract to NFTC Labs' MerkleClaimList.sol library.
...
abstract contract MerkleLeaves {
    ...
    function getIndexedLeafFor(address wallet, uint256 index) external pure returns (bytes32) {
     ...
     */
    function _generateIndexedLeaf(address wallet, uint256 index) internal pure returns (bytes32) {
...
/**
 * @title NFTC Merkle Claimlist Implementation
 * @author @NiftyMike, NFT Culture
 * @dev "Claimlist" - an approach for an allowlist backed by a Merkle Tree.
 ...
 *
 * Small helper library, in case you need a map of count tracked by wallet.
 *
 * Please report bugs or security issues to @author
 * Please credit @author if you re-use this code
 */
