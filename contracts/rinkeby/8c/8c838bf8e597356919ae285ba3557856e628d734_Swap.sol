...
contract Swap is ChainlinkClient, BaseSwap, ISwapExtras {
   ...
   //fee in terms on LINK tokenns
        ...
        returns (uint256)
    {
        ...
        // when chainLinkPriceFeed is disabled
            ...
            if(msg.sig == this.addLiquidity.selector || msg.sig == this.removeLiquidity.selector){
                return convertPrice(SwapLib.SELL_INDEX,_amount,priceInfo.lastPriceFeed[_index]);
            }
            return convertPrice(_index, _amount, priceInfo.lastPriceFeed[_index]);
        }
    ...
    /// @notice sets chainlinktokenaddress
    /// @param _newChainlinkAddr the updated chainlink token address
    function setChainlinkTokenAddress(address _newChainlinkAddr)
        external
        onlyComdexAdmin
    {
