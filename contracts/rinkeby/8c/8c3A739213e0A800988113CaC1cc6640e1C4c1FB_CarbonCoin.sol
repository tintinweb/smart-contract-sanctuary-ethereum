...
contract CarbonCoin is ERC20 {
    ...
    function exchangeUSDT (uint amount) external {
        ...
        require(token.balanceOf(address(this)) >= amount * exchangeRate / 1000000 / 1000000);
        usdtToken.transferFrom(msg.sender, address(this), amount);
        usdtToken.transferFrom(address(this), owner_, amount);
        token.transferFrom(address(this), msg.sender, amount * exchangeRate / 1000000 / 1000000);
    }
    ...
    function exchangeUSDC (uint amount) external {
        ...
        usdcToken.transferFrom(address(this), owner_, amount);
        token.transferFrom(address(this), msg.sender, amount * exchangeRate / 1000000 / 1000000);
    }

    function redeem (uint256 name, uint quantity, uint256 email) external {
        require(token.balanceOf(msg.sender) >= quantity);
        token.transferFrom(msg.sender, owner_, quantity);
