...
contract ChronixStaking is Initializable, ReentrancyGuardUpgradeable, ContextUpgradeable {
    ...
    function registerDeposit(address ownerOfToken, address contractAddress, uint256[] memory tokenIds) public authorised {
      ...
      if (contractType == ContractTypes.CRG) {  newYield +=traitsForContract[contractAddress]; user.stakedKits.push(tokenIds[i]); }
         if (contractType == ContractTypes.CRP) { 
            user.stakedPushers.push(tokenIds[i]);    
            user.accumulatedAmount +=getPotTaxOf(user.stakedPushers.length,_msgSender());
            user.ownPot=pot;
      ...
      emit AutoDeposit(contractAddress, tokenIds.length, _msgSender());
    }
