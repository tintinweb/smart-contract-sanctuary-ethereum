...
contract WePiggySimplePriceOracle is OwnableUpgradeable, PriceOracle{
    ...
    function getPrice(address underlying) public view returns (uint price) {
        ...
        for(uint i = 0 ; i < tokenConfig.sources.length; i++){
            price = getSourcePrice(underlying, i);
            if(price > 0){
                return price;
            }
        }
    ...
    function getMarketSources(MarketTokenInterface marketToken) public view returns(address[] memory){
        address underlying = marketToken.underlying();
    ...
    function _setTokenConfigs(TokenConfig[] memory configs) public onlyOwner{
        for(uint i = 0; i < configs.length; i++){
           ...
           _setConfig(tokenConfig.underlying, tokenConfig.sources, tokenConfig.baseUnit, tokenConfig.fixedUsd);
       }
    }

    function _setConfig(address underlying, address[] memory sources, uint256 baseUnit, bool fixedUsd) public onlyOwner{
