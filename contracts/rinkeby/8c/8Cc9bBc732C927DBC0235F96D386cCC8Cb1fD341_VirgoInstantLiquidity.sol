...
contract VirgoInstantLiquidity is Ownable, Pausable, ReentrancyGuard {
    ...
    // keccak256("sellNFTsForETH(address _userAddr,uint256 _totalPrice,uint256 _nonce,uint256 _deadline)")
    bytes32 private constant SELLNFTSFORETH_TYPEHASH = 0x62ab4af8eeeb009484e43193d0fcc8da06973e8df64b4bf81c46b7cc719b6936;
     // keccak256("buyNFTsForETH(address _userAddr,uint256 _totalPrice,uint256 _nonce,uint256 _deadline)")
    bytes32 private constant BUYNFTSFORETH_TYPEHASH = 0x04092b8dbb765104e7187b7011c4bcd8bda95b887d9ae0ddc466f6020b97b854;
   ...
   //Get the balance of contract
    ...
    function verify(        
        ...
        bytes32 messageHash = getMessageHash(_typeHash, _userAddr, _totalPrice, _nonce, _deadline);
    ...
    function sellNFTsForETH( 
        ...
        require(_userAddr == _msgSender(), 'user address must be trade owner');
        ...
        for (uint256 i = 0; i < _erc721Details.length; i++) {
            for (uint256 j = 0; j < _erc721Details[i].ids.length; j++) {
                  ...
                  if (IERC721(_erc721Details[i].tokenAddr).ownerOf(_tokenId) == _msgSender()) {                        
                    ...
                    emit SellERC721Fail(_userAddr, _erc721Details[i].tokenAddr, _erc721Details[i].ids[j]);                  
            }
        }
        ...
        for (uint256 i = 0; i < _erc1155Details.length; i++) {
            for (uint256 j = 0; j < _erc1155Details[i].ids.length; j++) {  
                ...
                if (_balanceOf >= _amounts) {
                    ...
                    emit SellERC1155Fail(_userAddr, _erc1155Details[i].tokenAddr, _tokenId, _amounts.sub(_balanceOf));
                }
                else 
                    emit SellERC1155Fail(_userAddr, _erc1155Details[i].tokenAddr, _tokenId, _amounts);
            }
        }
        ...
        for (uint256 i = 0; i < _erc721Details.length; i++) {
            for (uint256 j = 0; j < _erc721Details[i].ids.length; j++) {
                  ...
                  if (IERC721(_erc721Details[i].tokenAddr).ownerOf(_tokenId) == address(this)) {
                    ...
                    emit BuyERC721Fail(_userAddr, _erc721Details[i].tokenAddr, _tokenId);                  
            }
        }
        ...
        for (uint256 i = 0; i < _erc1155Details.length; i++) {
            for (uint256 j = 0; j < _erc1155Details[i].ids.length; j++) { 
                ...
                if (_balanceOf >= _amounts) {
                    ...
                    emit BuyERC1155Fail(_userAddr, _erc1155Details[i].tokenAddr, _tokenId, 
