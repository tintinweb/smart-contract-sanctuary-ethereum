...
contract Vesting is Ownable {
    ...
    /// @notice AZURO token contract
    IERC20Votes public immutable azuro;
     ...
     * @param azuro_ Address of the AZURO contract
     * @param vestingBegin_ Timestamp of the vesting begin time
     * @param vestingEnd_ Timestamp of the vesting end time
     */
    constructor(IERC20Votes azuro_, uint256 vestingBegin_, uint256 vestingEnd_) Ownable() {
    ...
    function claim(address account) external {
        ...
        require(azuro.transfer(account, totalAmount), "Vesting::claim: transfer error");
    }
    ...
    function holdTokens(HoldParams[] memory params) external onlyOwner {
        ...
        require(azuro.transferFrom(msg.sender, address(this), totalAmount), "Vesting::holdTokens: transfer failed");
    ...
    /**
     * @notice Function gets total amount tokens for claim in future
     * @param account Account to calculate amount for
     * @return amount Total amount of tokens
     */
    function getBalanceOf(address account) public view returns (uint256 amount) {
        for (uint8 i = 0; i < vestingIds[account].length; i++) {
            VestingParams memory vestParams = vestings[i];
    ...
    function _holdTokens(HoldParams memory params) private {
        ...
        if (params.unlocked > 0) {
            azuro.transfer(params.recipient, params.unlocked);
        }
    ...
    /**
     * @notice Function gets voting power for compound delegated votes with vesting allocation
     * @param account Address of account
     * @return Amount of votes
     */
    function getVotesWithVested(address account) external view returns (uint256) {
        return azuro.getVotes(account) + getBalanceOf(account);
    }
}
