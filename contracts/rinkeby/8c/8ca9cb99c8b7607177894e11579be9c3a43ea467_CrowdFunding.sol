...
// File: contracts/StructCrowdFunding.sol


pragma solidity ^0.8.10;

contract CrowdFunding {
    event Log(address funderAddress , uint256 amount);
    struct Funder {
    ...
    */
    ...
    function checkFunders(uint256 campaignID, uint256 funderID)  public payable
        returns (address funderAddress , uint256 amount)
    {
        Campaign storage c = campaigns[campaignID];
        emit Log(msg.sender,msg.value);
        return(c.funders[funderID].addr,c.funders[funderID].amount);
