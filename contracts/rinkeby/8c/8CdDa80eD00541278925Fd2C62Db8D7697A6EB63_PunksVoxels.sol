...
contract ApeVoxels is ERC721, ReentrancyGuard, Ownable {
    ...
    function update(uint count, address whoToMintTo) external {
        require(msg.sender == whoCanMint, "Sorry you cant mint");
        unchecked {
...
contract PunksVoxels is ERC721, ReentrancyGuard, IERC721Receiver, IERC1155Receiver, Ownable {
    ...
    function wrapMany(uint[] calldata tokenIds) nonReentrant external {
        ...
        uint countToUpdate = 0;
        for(uint i = 0; i < count;) {
            if(_exists(tokenIds[i]) == false) {
                // Mint the PunkVoxel
                ...
                countToUpdate = countToUpdate + 1;
            } else {
                require(false, "Bruh.. we only allow wrapping.");
            }
        ...
        if(countToUpdate > 0) {
            _balances[msg.sender] += countToUpdate;
            punkApes.update(countToUpdate, msg.sender);
        }
    }
