...
contract SpaceMarket is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    struct ItemInfo {
        IERC721 nftContract;
        uint256 nftTokenId;
        address paymentContract; // $ANT = 0x1234.. or MATIC = 0x0
        uint256 price;
        address owner;
        address recipient;
        uint32 at;
    ...
    mapping(address=>bool) public isNFTAllowed;
    mapping(uint256=>uint256) public itemIdToIndex;
    mapping(address=>mapping(uint256=>uint256)) public itemIdToIndexBy;
    ...
    public
    {
        ...
        _setupRole(PARTNER_ROLE, _msgSender());
        // TODO: test
    ...
    function create(IERC721 nftContract, uint256 tokenId, address recipient, address paymentContract, uint256 price)
    public
    nonReentrant
    returns (uint256 itemId)
    {
        ...
        require(hasRole(PARTNER_ROLE, sender) || isNFTAllowed[address(nftContract)],
                "ERR_NOT_PERMITTED");
        require(address(tokenContract) == paymentContract || paymentContract == address(0x0),
                "ERR_PAYMENT_METHOD");
        ...
        require(
            nftContract.getApproved(tokenId) == address(this) || nftContract.isApprovedForAll(sender, address(this)),
            "ERR_APPROVE"
        );
        require(
        ...
        ItemInfo memory item = ItemInfo({
            ...
            price: price,
            owner: sender,
            paymentContract: address(tokenContract),
            recipient: recipient,
        ...
        itemIdToIndex[itemId] = itemsIds.length - 1;
        itemIdToIndexBy[sender][itemId] = itemsBy[sender].length - 1;
        nextItemId += 1;
        emit Create(item.owner, address(item.nftContract), item.nftTokenId, item.price); 
    }
    ...
    nonReentrant
    {
        ...
        require(
            nftContract.getApproved(tokenId) == address(this) || nftContract.isApprovedForAll(itemInfo.owner, address(this)),
            "ERR_APPROVE"
        );
        ...
        // TODO: test
        // @dev handle ETH price
        if(itemInfo.paymentContract == address(0x0)) {
            require(msg.value >= itemInfo.price, "ERR_NOT_ENOUGH");
            uint256 refund = msg.value - itemInfo.price;
            if(refund > 0) {
                // TODO: test
                payable(sender).transfer(refund);
            }
            payable(itemInfo.recipient).transfer(itemInfo.price);
        // @dev handle ANT price
        } else {
            tokenContract.transfer(itemInfo.recipient, itemInfo.price);
            itemInfo.nftContract.safeTransferFrom(itemInfo.owner, sender, itemInfo.nftTokenId);
        }

        _removeItem(itemInfo.owner, itemId, itemInfo, ItemStatus.Executed);

        emit Execute(sender, address(itemInfo.nftContract), itemInfo.nftTokenId, itemInfo.price); 
    }

    function _removeItem(address sender, uint256 itemId, ItemInfo memory itemInfo, ItemStatus status)
    internal
    {
        itemInfo.status = status;
        itemsInfo[itemId] = itemInfo;
        itemsIds[itemIdToIndex[itemId]] = itemsIds[itemsIds.length - 1];
        itemsIds.pop();
        itemsBy[sender][itemIdToIndexBy[sender][itemId]] = itemsBy[sender][itemsBy[sender].length - 1];
        itemsBy[sender].pop();
        delete itemIdToIndex[itemId];
        delete itemIdToIndexBy[sender][itemId];
        delete nftTokensToItemId[itemInfo.nftContract][itemInfo.nftTokenId];
    }
    ...
    nonReentrant
    {
        ...
        _removeItem(sender, itemId, itemInfo, ItemStatus.Canceled);
        emit Cancel(itemInfo.owner, address(itemInfo.nftContract), itemInfo.nftTokenId, itemInfo.price); 
    }
    ...
    returns( ItemInfo[] memory items, uint256 left)
    {
        ...
        for(uint256 idx=0; idx < limit; idx++) {
            items[idx] = itemsInfo[lookupItemsIds[offset + idx]];
        }

        if(lookupItemsIds.length > offset + limit) {
            // TODO: test
    ...
    function removeTokenId(address owner, uint256 itemId) 
    internal
    {
    ...
    function sudoNFTAllow(address nftContract, bool allow)
    public
    onlyRole(WHITELISTER_ROLE)
    {
        isNFTAllowed[nftContract] = allow;
    }
...
interface IAntimatter is IERC20, IAccessControlEnumerableUpgradeable {
}
