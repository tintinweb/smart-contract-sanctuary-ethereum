...
contract CryptoMarket is ReentrancyGuard {
  ...
  struct MarketToken {
     ...
     uint time;//this says the time for auction
  ...
  function create_market_items_erc721(uint256 tokenId ,string memory URI,string memory title,string memory description, address nftContract,uint royalities) public  {
    ...
    markettokens[n] = MarketToken(n,tokenId,URI,title,description,nftContract,payable(msg.sender),payable(msg.sender),0,royalities,0,payable(0x00),false,false,false,false);
  ...
  function createMarketSale_erc721(uint256 itemId) public payable nonReentrant{
    ...
    if(markettokens[itemId].is_fixed==true){
      ...
      markettokens[itemId].owner.transfer(msg.value);

      markettokens[itemId].owner = payable(msg.sender);

      markettokens[itemId].sell = false;

      IERC721(markettokens[itemId].nftContract).safeTransferFrom(markettokens[itemId].owner,msg.sender, markettokens[itemId].tokenId);
  ...
  function keepForSale(uint256 price,uint256 itemId) public {
  ...
  function createMarketSale_erc1155(uint256 itemId) public payable nonReentrant{
    ...
    if(markettokens[itemId].is_fixed==true){
      ...
      IERC1155(markettokens[itemId].nftContract).safeTransferFrom(markettokens[itemId].owner,msg.sender, markettokens[itemId].tokenId,1,'0X00');
  ...
  function keep_for_timed_auction(uint256 itemId,uint time,uint price) public{
  ...
  function burn(uint256 itemId) public{
      ...
      require(markettokens[itemId].is_single==true,'this not erc721 token');

      markettokens[itemId].owner = payable(address(this));

      markettokens[itemId].creater = payable(address(this));

      markettokens[itemId].bidder = payable(address(this));
