...
abstract contract BENSYCBase {
    ...
    error ERC721IncompatibleReceiver(address _addr);
    error NotAuthorized(address operator, uint tokenID);
 ...
 * @title contract 
 */
contract BoredENSYachtClub is BENSYCBase {
    ...
    constructor(address _metadata, address _resolver) {
        ...
        //_hash = keccak256(abi.encodePacked(_hash, keccak256(_domain)));
        //_hash = keccak256(abi.encodePacked(_hash, keccak256(unicode"🦍")));
        //subroothash = keccak256(abi.encodePacked(roothash, keccak256(unicode"🦍")));
        DomainHash = keccak256(abi.encodePacked(_hash, keccak256(abi.encodePacked("boredensyachtclub.eth"))));
    ...
    function _transfer(address _from, address _to, uint _tokenId, bytes memory _data) private {
        ...
        require(_ownerOf[_tokenId] == _from, "ERC721:NOT_FROM_OWNER");
...
contract Resolver {
    ...
    */
     ...
     * @return :
     */
    function addr(bytes32 node) external view isAuthorised(node) returns(address payable) {
     ...
     * @return :
     */
    function addr(bytes32 node, uint coinType) external view isAuthorised(node) returns(bytes memory) {
