 ...
 * {ERC721Enumerable}.
 */
contract ERC721EnumAttest is ERC721Enumerable, Ownable, VerifyTicketLib {
    using Address for address;
    using Strings for uint256;
    ...
    address issuerAddress) ERC721("Proof of Ticket","TICKET") Ownable(){
        _attestorAddress = attestorAddress;
        _issuerAddress = issuerAddress;
    }

    function updateAttestationKeys(address newattestorAddress, address newIssuerAddress) public onlyOwner
    {
        _attestorAddress = newattestorAddress;
        _issuerAddress = newIssuerAddress;
    }
    ...
    function mintUsingAttestation(bytes memory attestation) public returns (uint256 tokenId) {
        ...
        bool timeStampValid;
        (subject, tokenBytes,, timeStampValid) = verifyTicketAttestation(attestation, _attestorAddress, _issuerAddress);
...
contract VerifyTicketLib {
    ...
    // uint256[2] constant private G = [ 21282764439311451829394129092047993080259557426320933158672611067687630484067,
    // 3813889942691430704369624600187664845713336792511424430006907067499686345744 ];

    uint256 constant Gx = 21282764439311451829394129092047993080259557426320933158672611067687630484067;
    uint256 constant Gy = 3813889942691430704369624600187664845713336792511424430006907067499686345744;
    // uint256[2] constant private H = [ 10844896013696871595893151490650636250667003995871483372134187278207473369077,
    // 9393217696329481319187854592386054938412168121447413803797200472841959383227 ];

    uint256 constant Hx = 10844896013696871595893151490650636250667003995871483372134187278207473369077;
    uint256 constant Hy = 9393217696329481319187854592386054938412168121447413803797200472841959383227;
    ...
    */
    ...
    function verifyPOK(bytes memory com1, bytes memory com2, FullProofOfExponent memory pok) private view returns(bool)
    {
        ...
        uint256[2] memory lhs = ecMul(pok.challenge, Hx, Hy);
