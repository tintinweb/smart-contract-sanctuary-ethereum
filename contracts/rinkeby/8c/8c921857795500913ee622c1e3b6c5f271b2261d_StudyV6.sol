...
contract StudyV6 is ERC721Enumerable,ERC2981,Ownable,Pausable,ReentrancyGuard{
  using Strings for uint256;
  string baseURI = "";

  bool public mintStart = false;
  bool public stakeStart = true;
  uint256 constant maxSupply = 20;
  uint256 constant mintLimit = 5;
  uint256 stakeMintCost = 1000;

  address public royaltyAddress = 0x28e2F1E9ECC957AdEbeC934fdD716Fbfb502e371;
  ...
  constructor() ERC721("StudyV6", "SV6") {
  ...
  function stakeMint(uint256 quantity) public nonReentrant {
    ...
    require((quantity + supply) <= (maxSupply), "Sorry. No more NFTs");
  ...
  function ownerMint(address _address, uint256 count) public onlyOwner {
    uint256 supply = totalSupply();
    require(supply + count <= maxSupply,"Total supply cannot exceed maxSupply");
  ...
  Asagi at_Contract = Asagi(0x1CbB4FD9f5E0EE2f2839CA47b356143199D7bE11);
  ...
  function stake() public{
    ...
    for (uint256 i; i < at_Contract.walletOfOwner(msg.sender).length; i++) {
      timeStampMap[at_Contract.walletOfOwner(msg.sender)[i]]=block.timestamp;
      stampOwnerMap[at_Contract.walletOfOwner(msg.sender)[i]]=msg.sender;
    }
  }
