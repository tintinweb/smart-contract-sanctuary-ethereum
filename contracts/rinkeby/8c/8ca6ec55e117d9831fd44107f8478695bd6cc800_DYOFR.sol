...
contract DYOFR is ERC721AWithRoyalties {
    ...
    function mint(
        ...
        if (mintInternal(quantity,0) && !burned) emitFauxTransfers(quantity);
    }
    ...
    function iHavePaperHands(
        ...
        if ( _ownerOf(tokenId) != msg.sender ) revert YouTried();
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    '{"name": "DYOFR Pass #',
                                    _toString(tokenId),
                                    '", "description": "DYOFR. An acronym to live by. Reveal Jun 7 2022. Tokens serve as a full-access-pass towards the next generation of tools for dominating the blockchain(s).", "image": "',imageURI,'"}'
                                )
                            )
                        )
                    )
