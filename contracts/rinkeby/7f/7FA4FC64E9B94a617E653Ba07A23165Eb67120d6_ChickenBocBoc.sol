...
contract ChickenBocBoc is Ownable, ERC721A, IERC2981, ReentrancyGuard {
    /* ========== DEPENDENCIES ========== */
    ...
    uint64 public constant MAX_SUPPLY = 3333;
    uint64 public constant MAX_MINT_QUANTITY = 10;
    uint64 public constant MAX_PROMOTIONAL_NFT_COUNT = 333;
    uint64 public constant PUBLIC_SALE_PRICE = 0.033 ether;

    address payable private constant _owner1 = payable(0x5181755079feaB445f9EA00A5c47e2F596C70767);
    address payable private constant _owner2 = payable(0xbEa43E4bbD33175a62287F483895f8d7414FaFa6);
    address payable private constant _owner4 = payable(0x9D0D087528A5Ab2370b5e5C8187d2b3Be0c0ceF0);
    address payable private constant _owner3 = payable(0x5f76b84F3B8DA62dA9368786310A8530Fe6B2dCa);

    /* ====== VARIABLES ====== */

    string private _customBaseURI = "ipfs://QmeBpWUj1Rt7MjFc8Zt1NWwdRFLfxbXto15aQyQVekC8ia";

//    address private _openSeaProxyRegistryAddress = 0x58807baD0B376efc12F5AD86aAc70E78ed67deaE;
...
//    bool private _isOpenSeaProxyActive = true;
    ...
    modifier tokenExists(uint256 tokenId_) {
        require(_exists(tokenId_), "BocBoc: !exist");
        _;
    }

    modifier publicSaleActive() {
        require(isPublicSaleActive, "BocBoc: !active");
        _;
    }

    modifier maxMintsPerTX(uint256 quantity_) {
        require(quantity_ <= MAX_MINT_QUANTITY, "BocBoc: max mint exceeded");
        _;
    }

    modifier canMintNFTs(uint256 quantity_) {
        require(totalSupply() + quantity_ <= MAX_SUPPLY, "BocBoc: quantity exceeded totalSupply()");
        _;
    }

    modifier isCorrectPayment(uint256 quantity_) {
        require((PUBLIC_SALE_PRICE * quantity_) == msg.value, "BocBoc: !enough eth, still hungry");
        _;
    }

    /* ====== CONSTRUCTOR ====== */

    constructor() ERC721A("Chicken Boc Boc", "BocBoc") {}
    ...
    {
        // Check free chickens left & check owner AUX
        if (_currentPromotionalIndex < MAX_PROMOTIONAL_NFT_COUNT && _getAux(msg.sender) == 0) {
            unchecked {
            ...
            // Add one free chicken
            quantity_ += 1;
            assert(totalSupply() + quantity_ <= MAX_SUPPLY);
        }
...
//    }
    ...
    function withdraw() public {
        uint256 balance_ = address(this).balance / 4;

        (bool success1_, ) = _owner1.call{value: balance_}("");
        require(success1_, "Transfer failed.");

        (bool success2_, ) = _owner2.call{value: balance_}("");
        require(success2_, "Transfer failed.");

        (bool success3_, ) = _owner3.call{value: balance_}("");
        require(success3_, "Transfer failed.");

        (bool success4_, ) = _owner4.call{value: balance_}("");
        require(success4_, "Transfer failed.");
    }

    function withdrawTokens(IERC20 token) public {
        uint256 balance = token.balanceOf(address(this)) / 4;
        token.safeTransfer(_owner1, balance);
        token.safeTransfer(_owner2, balance);
        token.safeTransfer(_owner3, balance);
        token.safeTransfer(_owner4, balance);
    }
...
//    }
    ...
    {
        return (address(this), (salePrice_ * 5) / 100);
    }
}
