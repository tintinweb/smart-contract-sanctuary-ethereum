...
// File: contracts/SaveTheDate.sol
 ...
 */
contract SaveTheDate is ERC721, Ownable, ReentrancyGuard {
    ...
    address private _donorWallet;
    
    mapping(uint256 => DropTier) private _dropTiers;
    mapping(uint256 => uint256) private _seriesTokenCounter;
    ...
    function setTokenSupply(uint256 maxSupply) public onlyOwner {
        require(maxSupply > tokenCount(), "Invalid max supply");
    ...
    function setEligibleCommunities(CommunityClaim[] calldata claimers) external onlyOwner {
        ...
        for (uint256 i = 0; i < claimers.length; i++) {
            _communities.push(CommunityClaim(claimers[i].tokenContract, claimers[i].eligibleTokens, claimers[i].isExempt));
        }   
    }
    ...
    function checkDateAvailability(DateToMint calldata date) public view returns (bool) {
        ...
        DateToken memory token = mintedDates[_tokenDate];
        return (!(token.tokenId > 0 || (token.booker != address(0) && token.isReserved)));
    }
    ...
    function mintDate(DateToMint calldata tokenDate, bytes32[] calldata merkleProof) external payable ensureAvailability nonReentrant returns (uint256) {
        ...
        (bool _eligibleCommunity, bool _communityExempt, uint256 _communityIndex, uint256 _redeemIndex) = _verifyCommunityClaim(_msgSender());

        uint256 tokenPrice = (_communityExempt ? 0 : _dropTiers[dropSeries].regularPrice);
        string memory _tokenDate = _dateToString(tokenDate);
        DateToken memory token = mintedDates[_tokenDate];

        if (tokenDate.isRandom && !_communityExempt && _validateDiscount(_tokenDate, tokenDate.pass)) {
        ...
        require(token.tokenId == 0, "Date already minted");

        // Donated date can't be minted
        require(!token.isReserved || (token.isReserved && token.booker != _donorWallet), "Donated token claim");

        if (salesPhase == SalesPhase.Presale && checkReservationUpheld()) {
            require(!token.isReserved || (token.isReserved && (token.booker == address(0) || token.booker == _msgSender())), "Token reservation upheld");
        }
        ...
        _seriesTokenCounter[dropSeries]++;

        if (_seriesTokenCounter[dropSeries] == _dropTiers[dropSeries].pieces) {
    ...
    function mintDonatedDate(DateToMint calldata tokenDate, address recipient) public ensureAvailability onlyOwner {
        ...
        if (token.isReserved && token.booker != address(0)) {
            require(token.booker == recipient || token.booker == _donorWallet, "Invalid recipient");
        }
    ...
    function mintDonatedDates(DateToMint[] calldata tokenDates, address[] calldata recipients) external ensureAvailability onlyOwner {
        ...
        for (uint i = 0; i < recipients.length; i++) {
            string memory _tokenDate = _dateToString(tokenDates[i]);
            DateToken memory token = mintedDates[_tokenDate];
            require(token.tokenId == 0 , "Date already minted");

            if (token.isReserved && token.booker != address(0)) {
                require(token.booker == recipients[i] || token.booker == _donorWallet, "Invalid recipient");
            }
     ...
     */
    function reserveDates(DateToMint[] calldata bookedDates, address[] calldata bookers) external view onlyOwner {
        require(bookedDates.length == bookers.length, "Invalid call data");

        for (uint i = 0; i < bookedDates.length; i++) {
            require(checkDateAvailability(bookedDates[i]), "Date is unavailable");

            string memory _tokenDate = _dateToString(bookedDates[i]);
            DateToken memory token = mintedDates[_tokenDate];
            
            token.booker = bookers[i];
     ...
     */
    function releaseDates(DateToMint[] calldata bookedDates, address[] calldata bookers) external view onlyOwner {
        require(bookedDates.length == bookers.length, "Invalid call data");
        ...
        for (uint i = 0; i < bookedDates.length; i++) {
            ...
            if (token.tokenId == 0 && (token.isReserved && (token.booker == address(0) || token.booker == bookers[i]))) {
                delete token;
            } 
        }
    }
    ...
    function _verifyCommunityClaim(address claimer) internal view returns (bool eligible, bool exempt, uint256 communityIndex, uint256 redeemIndex) {
        for (uint i = 0; i < _communities.length; i++) {
            ...
            uint256 balance = communityToken.balanceOf(claimer);
            if (balance > 0) {
                communityIndex = i;
                ...
                if (_communities[i].isExempt) {
                    ...
                    for (uint j = 0; j < _eligibleTokens.length; j++) {
                        exempt = (communityToken.ownerOf(_eligibleTokens[j]) == claimer);
                        if (exempt) {
                            redeemIndex = j;
                            break;
                        }
     ...
     */
    function _validateDiscount(string memory _date, bytes memory pass) internal view returns (bool) {
        if (pass.length > 0) {
            bytes32 hash = ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(_date)));
            return (owner() == ECDSA.recover(hash, pass));
        } else {
            return false;
        }
    }
    ...
    function withdraw() public onlyOwner nonReentrant {
        ...
        Address.sendValue(payable(_donorWallet), balance);
    }
    ...
    function _dateToString(DateToMint calldata date) internal pure returns (string memory tokenDate) {
        ...
        tokenDate = string(abi.encodePacked(date.year, monthSuffix, date.month, daySuffix, date.day));
    }
    ...
    function _verifyDate(uint256 year, uint256 month, uint256 day) internal pure returns (bool valid) {
        if ((year >= DEFAULT_EXCLUSIVE_YEAR && year <= DEFAULT_INCLUSIVE_YEAR) && month > 0 && month <= 12) {
     ...
     */
    function _redeemExemptToken(uint256 _communityIndex, uint256 _tokenIndex) internal {
