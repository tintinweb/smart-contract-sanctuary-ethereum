...
contract NFT is ERC721Enumerable, Ownable, Pausable{
    ...
    uint256 private maxSupply = 4500;

    string private notRevealURI;

    uint256 private revealTime = 60;
    ...
    mapping(uint256 => uint8) public saleTokenId;
    ...
    function preSaleMint(address _to, uint256 _count) public payable whenNotPaused{
        require(preSaleStartDate <= block.timestamp && preSaleEndDate > block.timestamp, "Presale ended or not started yet");
        require(msg.value >= _count * preSalePrice, "Less Amount");
        require(isWhitelisted(_to), "User is not whitelisted");     
        require(totalSupply() + _count <= maxSupply, "Supply limit reached!"); 
        for (uint256 i = 1; i <= _count; i++) {
            tokenID++;
            saleTokenId[tokenID] = 1;
            _safeMint(_to, tokenID);

        }       
        emit PreSaleMint(_to, _count, msg.value, block.timestamp);
    }

    function publicSaleMint(address _to, uint256 _count) public payable whenNotPaused{
        require(publicSaleStartDate <= block.timestamp && publicSaleEndDate > block.timestamp, "Publicsale ended or not started yet");
        require(msg.value >= _count * publicSalePrice, "Less Amount");     
        require(totalSupply() + _count <= maxSupply, "Supply limit reached!"); 
        for (uint256 i = 1; i <= _count; i++) {
            tokenID++;
            saleTokenId[tokenID] = 2;
            _safeMint(_to, tokenID);
        }       
        emit PublicSaleMint(_to, _count, msg.value, block.timestamp);
    }
    ...
    function whitelist(address _account) public onlyOwner {
        require(! whitelistUsers[_account], "User already whitelisted");        
    ...
    function tokenURI(uint256 _tokenId) public view  override returns (string memory)
    {
        ...
        if(saleTokenId[_tokenId] == 1 && block.timestamp > preSaleEndDate + revealTime){
            return super.tokenURI(_tokenId);
        }else if(saleTokenId[_tokenId] == 2 && block.timestamp > publicSaleEndDate + revealTime){
            return super.tokenURI(_tokenId);
        }else{
            return notRevealURI;
        }
