...
contract FFVaultGenerator is AccessControl, Pausable, ReentrancyGuard {
    ...
    function claimSingle(
        ...
        require(
            !(claims[vault_][msg.sender] > _start &&
                claims[vault_][msg.sender] < _end),
            "already claimed"
        );
        require(
    ...
    function isValidClaim(
        ...
        // bytes32 _rootHash = 0xca0b5f7090b9ae4e541b3ca50ec45798910dfa366b6199a4ee8a776ffeb48479;
        bytes32 _keca = keccak256(abi.encodePacked(msg.sender, amount_));
        require(
            MerkleProof.verify(merkleProof_, _rootHash, _keca),
            "Incorrect proof"
        );
        return true;
    }
    ...
    function initiateVault(
        ...
        // #TODO test if no current has works
        require(_start < start_, "old start must be < new");
        require(_end < end_, "old end must be < old");
        /*
        # this requirement does not allow for a simple time adjustment, no nee for it
        require(
            keccak256(abi.encodePacked(vaults[vault_].currentHash)) !=
                keccak256(abi.encodePacked(currentHash_)),
            "new hash must be != old"
        ); */
    ...
    function claimable(
        ...
        require(isValidClaim(vault_, amount_, merkleProof_));
