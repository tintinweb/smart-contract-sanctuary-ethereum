...
// File: contracts/Tasty_People.sol
...
contract Tasty_People is ERC721A, Ownable {
  using Strings for uint256;
  ...
  function mint(uint256 tokens) public payable {
    require(!paused, "TPX: oops contract is paused");
    uint256 supply = totalSupply();
    uint256 ownerTokenCount = balanceOf(_msgSender());
    require(tokens > 0, "TPX: need to mint at least 1 NFT");
    require(tokens <= maxpertx, "TPX: max mint amount per tx exceeded");
    require(supply + tokens <= maxSupply, "TPX: We Soldout");
    require(ownerTokenCount + tokens <= MaxperWallet, "TMX: Max NFT Per Wallet exceeded");
    require(msg.value >= cost * tokens, "TPX: insufficient funds");
    ...
    require(tokens > 0, "TPX: need to mint at least 1 NFT");
    require(tokens <= 3, "TPX: max mint per Tx exceeded");
    require(supply + tokens <= freeSupply, "TPX: Whitelist MaxSupply exceeded");
