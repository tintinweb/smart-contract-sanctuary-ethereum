...
contract Distributor is Context, AccessControlEnumerable, Pausable {
    ...
    event NewDickMinted(address minter, uint256 quantity);
    ...
    modifier onlyFreeMintAccess() {
        ...
        uint256 freeMintQuantity = freeMint[_msgSender()];

        require(freeMintQuantity > 0, "Distributor: No Free Mint to claim");

        _;

        whitelist[_msgSender()] = whitelist[_msgSender()].sub(
            freeMint[_msgSender()]
        );
        freeMint[_msgSender()] = 0;
    }
        ...
        onlyAccess(quantity)
    {
        ...
        emit NewDickMinted(_msgSender(), quantity);

        liquidityCollector.transfer(priceToPay);
    }

    function claimFreeMint() external onlyFreeMintAccess {
