...
contract animalMain is ERC721, Ownable {
  ...
  function transfer(address _to, uint256 _tokenId) public {
    //TO DO 請使用 require 判斷要轉的動物id是不是轉移者的
  ...
  function createAnimal() payable public checkValue{
    ...
    // TO DO 
    // 使用亂數來產生DNA, 星級, 動物種類
    dna = keccak256(abi.encodePacked(block.timestamp, msg.sender, "dna"));
    star = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, "star"))) % 5 + 1;
    roletype = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, "roletype"))) % 5 + 1;
    // 動手玩創意，可以限制每次建立動物需要花費多少ETH
