...
contract WerdWorldNFT is ERC721, ERC2981Royalties, AccessControl {
    ...
    uint256 public maxSupply = 1003;

    uint8 private constant MAX_PUBLIC_MINT = 3;
    ...
    event SaleStatus(string status);
    ...
    constructor(string memory _baseUri) ERC721 ("TheWerdWorld", "WERD") {
    ...
    function mint(uint8 _amount) external payable {
        ...
        if (totalSupply() == maxSupply) {
            emit SaleStatus("SOLD OUT!");
        }
    }

    function giftMint(address to) external onlyOwner {
        require(totalSupply() < maxSupply, "None left");
    ...
    function activatePreSale() public onlyRole(ADMIN_ROLE) {
        isPreSaleActive = true;
        emit SaleStatus("Pre-Sale Active");
    }

    function activatePublicSale() public onlyRole(ADMIN_ROLE) {
        isPreSaleActive = false;
        isPublicSaleActive = true;
        emit SaleStatus("Public Sale Active");
    }

    function reveal(string memory _newURI) public onlyRole(ADMIN_ROLE) {
      ...
      returns (string memory)
    {
      ...
      if (_revealed == false) {
        return string(abi.encodePacked(baseTokenURI, "pre-reveal", META_SUFFIX));
      }

      return bytes(baseTokenURI).length > 0
        ? string(abi.encodePacked(baseTokenURI, _tokenId.toString(), META_SUFFIX))
        : "";
    }
