...
contract SWFNFT is Ownable, ERC721A, ReentrancyGuard {
    ...
    //marketplace
    function setAllowApproveAll(bool enable) public onlyOwner {
        allowApproveAll = enable;
    }

    function setApprovalForAll(address operator, bool approved) public  virtual override(ERC721A) {
        require(allowApproveAll, 'Approvals Currently Disabled');
...
function transferFrom(
    ...
    function _payRoyality(uint256 _royalityFee) internal {
        (bool success1, ) = payable(creator).call{value: _royalityFee}("");
        require(success1);
    }
