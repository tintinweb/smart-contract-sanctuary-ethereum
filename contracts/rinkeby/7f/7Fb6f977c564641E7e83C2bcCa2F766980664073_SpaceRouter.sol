...
contract SpaceRouter {
    ...
    constructor(address _liquidityPool, address _spaceCoin) {
        ...
        /* set max allowance to liquidityPool */
    }
     ...
     *
     * @param spcTargetAmount amount of SPC to deposit
     * @param spcMinAmount amount of SPC considering slippage
     * @param ethMinAmount amount of ETH considering slippage
     *
     * @return amount of ETH and SPC deposited, liquidity received
     */
    function deposit(
        uint256 spcTargetAmount,
    ...
    {
        (uint256 ethReserve, uint256 spcReserve) = liquidityPool.getReserves();
        uint256 ethTargetAmount = msg.value;
        uint256 _amountEth = 0;
        uint256 _amountSpc = 0;

        if (ethReserve == 0 && spcReserve == 0) {
            _amountEth = ethTargetAmount;
            _amountSpc = spcTargetAmount;
        } else {
            uint256 optimalSpcAmount = calcOptimalAmount(ethTargetAmount, ethReserve, spcReserve);
            if (optimalSpcAmount <= spcTargetAmount) {
                if (optimalSpcAmount < spcMinAmount) revert WrongSpcAmount(optimalSpcAmount, spcMinAmount);
                _amountEth = ethTargetAmount;
                _amountSpc = optimalSpcAmount;
            } else {
                uint256 optimalEthAmount = calcOptimalAmount(spcTargetAmount, spcReserve, ethReserve);
                if (optimalEthAmount < ethMinAmount) revert WrongEthAmount(optimalEthAmount, ethMinAmount);
                _amountEth = optimalEthAmount;
                _amountSpc = spcTargetAmount;
            }
        }
     ...
     *
     * @param spcMinAmount min amount of SPC to receive considering slippage
     * @param ethMinAmount min amount of ETH to receive considering slippage
    ...
    function withdraw(
        ...
        if (eth < ethMinAmount || spc < spcMinAmount) revert InsufficientAmount(eth, ethMinAmount, spc, spcMinAmount);
    ...
    function swapEthForSpc(uint256 minSpcOut) external payable returns (uint256, uint256) {
        ...
        uint256 spcOut = liquidityPool.swap{value: amountEthIn}(0, msg.sender);
     ...
     * @notice allow user to swap ETH for SPC
     *
     * @dev SPC increaseAllowance is called on client. If tax is active on spc, need to calc the value without the tax that will be sent to the liquidity pool
     *
    ...
    function swapSpcForEth(uint256 amountIn, uint256 minEthOut) external returns (uint256, uint256) {
        ...
        /* this check is needed because, if the tax is appied, it will be applied to every transfer.
         * This includes the transferFrom user to liquidity pool in order to perfrom the swap from SPC to ETH
         * So, the amountIn won't be the same the user will insert because it needs to be updated with the tax applied.
         * This could be generic with TAX_RATE defined on spaceCoin (in case it could be updated, but it's out of scope in this project)
         */
        if (spaceCoin.isTaxActive()) {
            amountSpcIn -= ((amountSpcIn * 200) / 10_000);
        }

        uint256 ethOut = liquidityPool.swap(amountSpcIn, msg.sender);
    ...
    function getAmountReceived(
        ...
        /*
         * Leaving here, is the "classic" formula, but I think the one used it's just more clear (especially after AbhiG's discord explanation)
         * https://discord.com/channels/870313767873962014/1005164980410269796/1006678906193064050
         *
         * y = k / (x + x_in - (x_in * fee))
         * (uint256 ethReserve, uint256 spcReserve) = liquidityPool.getReserves()
         * uint256 amountOut = (ethReserve * spcReserve) / ((reserveIn + amountIn) * 100 - (amountIn * 99));
         */
    }
...
contract LiquidityPool is ERC20 {
     ...
     *
     * @dev needed because actions are performed after .call function, not using checks-effect-interaction since, in this case.
     * I believe this approach is more clear in this case compared to checks-effect-interactions.
     *
     */
    modifier lock() {
        if (isUnlocked == 2) {
    ...
    function burn(uint256 liquidity, address to) external lock returns (uint256, uint256) {
        ...
        if (ethOut <= 0 || spcOut <= 0) {
            revert InsuffiecientLiquidityBurned(ethOut, spcOut);
        }
    ...
    /* Payable because it can receive ETH and not an ERC20 like WETH */
    function swap(uint256 spcIn, address to) external payable lock returns (uint256) {
        uint256 _ethIn = msg.value;
        uint256 _spcIn = spcIn;
        ...
        emit Swapped(msg.sender, _ethIn > 0 ? _ethIn : _spcIn, amountOut);
        return amountOut;
    }
    ...
    error InsuffiecientLiquidityBurned(uint256, uint256);
    error BurnTransferEthToError(address);
    error InsufficientInAmount(uint256, uint256);
    error InsufficientLiquidity(uint256, uint256);
    error WrongKValueAfterSwap(uint256, uint256);
    error SwapTransferEthToError(address, uint256);
    error ReservesAmountInvalid(uint256, uint256);
}
