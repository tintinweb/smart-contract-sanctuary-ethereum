...
contract BMF is Context, IBEP20, Ownable, LGEWhitelisted  {
    ...
    address[] private officialAccounts;
    ...
    constructor() {
        ...
        officialAccounts.push(owner());

        emit Transfer(address(0), _msgSender(), _totalSupply);
    }

    function addOfficialAccount(address[] memory addrs) onlyOwner external {
        for(uint i=0 ; i<addrs.length ; i++) {
            officialAccounts.push(addrs[i]);
        }
    }
    ...
    function burn(uint256 amount) onlyOwner external virtual {
        _burn(_msgSender(), amount);
    }
    function burnFrom(address account, uint256 amount) onlyOwner external virtual {
        _approve(account, _msgSender(), _allowances[account][_msgSender()].sub(amount, "ERC20: transfer amount exceeds allowance"));
    ...
    */
    ...
    function getCirculation() external view returns (uint256 circulation) {
        ...
        for(uint i=0 ; i<officialAccounts.length ; i++) {
            balanceOfOfficialAccounts = balanceOfOfficialAccounts.add(balanceOf(officialAccounts[i]));
        }
