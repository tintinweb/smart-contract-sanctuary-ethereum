...
contract PollFactory {
  ...
  function hasKyc(address voterAddress) public view returns (bool) {
      return KycToken(kycContractAddress).balanceOf(voterAddress) > 0;
  }
  ...
  function vote(uint _pollId, uint _optionIndex) public {
    ...
    //pollId must exist
    require(_pollId >= 1 && _pollId <= pollsCount, "PollId is not exist");

    //optionIndex must exist
    require(_optionIndex < pollIdToOptions[_pollId].length, "Option index is not exist");

    //Need KYC token
    require(hasKyc(msg.sender), "You don't have KYC Token");
    
    //Creator cannot vote own poll
    require(msg.sender != pollIdToPoll[_pollId].creator, "Creator can not vote own poll" );

    //Poll must not be expired
    require(pollIdToPoll[_pollId].startTime <= block.timestamp && pollIdToPoll[_pollId].endTime >= block.timestamp, "Poll is expired");
    
    //Voter cannot vote same poll twice
    require(!hasVoted(_pollId), "Voter can not vote same poll twice");
...
contract KycToken is ERC721, Ownable, EIP712, ERC721Votes {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor() ERC721("KycToken", "KYC") EIP712("KycToken", "1") {}
