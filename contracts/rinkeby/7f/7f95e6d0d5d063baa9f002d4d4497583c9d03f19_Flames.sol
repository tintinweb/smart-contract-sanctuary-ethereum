...
contract Flames is ERC721Enumerable, Ownable {using Strings for uint256;
    uint256 public MAX_FLAMES = 9999; 
    uint256 public PRICE = 0.069 ether; 
    uint256 public MAX_FLAMES_MINT = 24;
    uint256 public MAX_PER_TX = 8;
    uint256 public constant PRESALE_MAX_MINT = 1;
    uint256 public constant RESERVED_FLAMES = 125; 
    address public constant founderAddress = 0xDEcF7554fd6bA6DeA809bdDf56f8685C6c96F76C;    
    address private constant launchpadAddress = 0x3351aCC36f7E780Db971FE1F9d8D7f58493B8162; 
    address private constant donationAddress = 0xDEcF7554fd6bA6DeA809bdDf56f8685C6c96F76C; 
    uint256 public reservedClaimed;
    uint256 public numFlamesMinted;
    ...
    event HandpickedMint(address minter, uint256 amountOfFlames);
    event PresaleMint(address minter, uint256 amountOfFlames);
    event PublicSaleMint(address minter, uint256 amountOfFlames);
    ...
    constructor(string memory baseURI) ERC721("MMNFT Flames", "FLAME") {
        baseTokenURI = baseURI;
    }

    function claimReserved(address recipient, uint256 amount) external onlyOwner {
        require(reservedClaimed + amount <= RESERVED_FLAMES, "Minting would exceed max reserved FLAMES");
        require(recipient != address(0), "Cannot add null address");
        require(totalSupply() + amount <= MAX_FLAMES, "Minting would exceed max supply");

        uint256 _nextTokenId = numFlamesMinted + 1;

        for (uint256 i = 0; i < amount; i++) {
            _safeMint(recipient, _nextTokenId + i);
        }
        numFlamesMinted += amount;
    ...
    function mintPartnershipPresale() external payable whenPresaleStarted {
        ...
        require(totalSupply() + 1 <= MAX_FLAMES - (RESERVED_FLAMES - reservedClaimed), "Minting would exceed max supply");
        ...
        uint256 tokenId = numFlamesMinted + 1;
    ...
    function mint(uint256 amountOfFlames) external payable whenPublicSaleStarted {
        require(totalSupply() + amountOfFlames <= MAX_FLAMES - (RESERVED_FLAMES - reservedClaimed), "Minting would exceed max supply");
        require(totalClaimed[msg.sender] + amountOfFlames <= MAX_FLAMES_MINT, "Purchase exceeds max allowed per address");
        require(amountOfFlames > 0, "Must mint at least one Flame");
        require(amountOfFlames <= MAX_PER_TX, "Amount over max per transaction. ");
        require(PRICE * amountOfFlames <= msg.value, "ETH amount is incorrect");

        for (uint256 i = 0; i < amountOfFlames; i++) {
            uint256 tokenId = numFlamesMinted + 1;
        ...
        emit PublicSaleMint(msg.sender, amountOfFlames);
    }

    function bulkPurchase(uint256 amountOfFlames) external payable {
        require(totalSupply() + amountOfFlames <= MAX_FLAMES - (RESERVED_FLAMES - reservedClaimed), "Minting would exceed max supply");
        require(amountOfFlames > 0, "Must mint at least one Flame");
        require(msg.sender == launchpadAddress || msg.sender == donationAddress, "Must be launchpad or donation wallet address");
        require(msg.sender == donationAddress || PRICE * amountOfFlames <= msg.value, "ETH amount is incorrect");
        require(amountOfFlames <= 100, "Cannot mint over 200");
