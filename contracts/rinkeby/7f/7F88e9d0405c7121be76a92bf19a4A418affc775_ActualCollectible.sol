...
contract ActualCollectible is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Royalty, VRFConsumerBase, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    enum Collectibles{banner, lifer, nfta, nftb, nftc}

    event ReturnedCollectible(bytes32 indexed requestID, uint256 randomNumber, uint256 tokenId);
    event RequestedCollectible(bytes32 indexed requestID, address requester, uint256 tokenId); 
    event RequestedRandomness(bytes32 requestID);

    mapping(address => mapping(uint256 => uint256)) private _ownedTokens;
    mapping(uint256 => Collectibles) public tokenIdToCollect;
    ...
    constructor(address _VRFCoordinator, address _LinkToken, uint256 _linkFee, bytes32 _keyHash) public 
    ...
    function createCollectible(string memory uri) public returns (bytes32) {
        ...
        emit RequestedCollectible(requestID, msg.sender, tokenId);
    }
    function fulfillRandomness(bytes32 requestID, uint256 randomNumber) internal override {
        require(PRICE_PER_TOKEN <= msg.value, "Ether value sent is not correct");
        require(saleIsActive, "Sale must be active to mint tokens");
        require(randomNumber > 0, "random not found yet");
        string memory newtokenURI = requestIdToTokenURI[requestID];
        address collectibleOwner = requestIdToSender[requestID];
        uint256 tokenId = _tokenIdCounter.current();
        _setTokenURI(tokenId, string(abi.encodePacked(
            _tempURI, uintToString(tokenId))));
        _safeMint(collectibleOwner, tokenId);
        
        Collectibles collectible = Collectibles(randomNumber % 5);
        tokenIdToCollect[tokenId] = collectible;
        requestIdToTokenId[requestID] = tokenId;

        emit ReturnedCollectible(requestID, randomNumber, tokenId);
    ...
    function reserve(uint amount) public onlyOwner {
      ...
      require(supply + amount <= MAX_SUPPLY, 
      "Reserve amount would exceed max tokens");
      require(supply + amount == 0, 
      "Reserve amount needs to be larger than 0");
      for (i = 0; i < amount; i++) {
