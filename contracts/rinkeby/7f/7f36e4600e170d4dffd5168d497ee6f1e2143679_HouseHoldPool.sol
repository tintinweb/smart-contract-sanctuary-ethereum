...
contract HouseHoldPool is UpgradeablePausable, IHouseHoldPool, AERC20Transfer {
  ...
  event DepositMade(address indexed origin, address to, uint256 indexed tokenId, uint256 amount);
  event DrawdownMade(address indexed owner, uint256 amount);
    ...
    returns (uint256 tokenId)
  {
    ...
    return _receiveDeposit(amount, period);
  }

  function _receiveDeposit(uint256 amount, uint256 period) internal returns (uint256 tokenId) {
    ...
    require(amount > 100, "llega");
    tokenId = hhConfig.getHHPoolToken().mintToken(params, _msgSender(), "");
    require(amount > 1000, "intenta el minteo");
    safeERC20TransferFrom(hhConfig.getUSDCContract(), _msgSender(), address(this), amount);
    require(amount > 2000, "termina la transferencia");
    emit DepositMade(_msgSender(), address(this), tokenId, amount);
    return tokenId;
  }

  function drawdown(uint256 amount, address borrower) external override onlyAdmin whenNotPaused {
    IERC20 usdc = hhConfig.getUSDCContract();
    ...
    safeERC20TransferFrom(hhConfig.getUSDCContract(), address(this), borrower, amount);
