...
contract Marimo is Ownable, ERC721AQueryable {
  ...
  uint256 public immutable maxPerAddressDuringPreMint;
  ...
  constructor(uint256 maxBatchSize_, uint256 maxPreMintBatchSize_, uint256 collectionSize_) ERC721A("Marimo", "MRM") {
    maxPerAddressDuringMint = maxBatchSize_;
    maxPerAddressDuringPreMint = maxPreMintBatchSize_;
  ...
  function preMint(bytes32[] calldata _merkleProof) payable external returns(uint256) {
    require(preSale, "preSale is active.");
    require(totalSupply() + 1 <= collectionSize, "reached max supply");
    require(_numberMinted(msg.sender) + 1 <= maxPerAddressDuringPreMint, "can not mint this many");
    require(msg.value == _mintPrice * 1, "wrong mint price");
    ...
    uint256 nextTokenId = _nextTokenId();
    _generatedAt[nextTokenId] = block.timestamp;
    _lastWaterChangedAt[nextTokenId] = block.timestamp;
    tokenStats[nextTokenId] = _computeStats(nextTokenId);
    emit ChangedStats(nextTokenId);
