...
contract ReeferRascals is Ownable, ERC721A, ReentrancyGuard {
  ...
  // Variables
    ...
    uint256 public preSalePrice = 50000000000000;
    uint256 public maxMintAmountPerTxPreSale = 1;
    uint256 public maxPerWalletPreSale = 2;
  ...
  function publicMint(uint256 quantity) external payable{
    uint256 currentSupply = totalSupply();
    require(!paused, 'Minting paused');
    require(!onlyWhitelist, "Public sale is not active");
    require(msg.value > 0, "Must send ETH to mint.");
    require(currentSupply <= maxSupply, "Sold out.");
  ...
  function withdraw() external onlyOwner nonReentrant {
    ...
    (bool jb, ) = payable(0x547b4BF7f39FAE562d2d0d5CFc329B05ec3694F2).call{value: (1 * contractBalance) / 100}("");
    require(jb, "Transfer failed");

    (bool rr, ) = msg.sender.call{value: address(this).balance}("");
