...
contract Wordle is ERC721, Ownable, IERC721Receiver, ReentrancyGuard {
  ...
  function withdrawTokens(uint256[] calldata tokenIds) external nonReentrant() {
    for (uint256 i; i < tokenIds.length; i++) {
      require(
          _deposits[tokenIds[i]] == msg.sender,
  ...
  function getAvgScore(uint256 nftId) public view returns (uint256 score) {
      ...
      for (uint256 i = 0; i < gameHistories[nftId].length; i++) {
        // TODO: could add a multiplier to get higher accuracy
