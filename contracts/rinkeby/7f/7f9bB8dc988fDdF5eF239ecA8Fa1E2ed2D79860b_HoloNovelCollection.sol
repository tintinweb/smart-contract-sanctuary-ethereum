...
$$ |  $$ |$$ |  $$ |$$ |$$ |  $$ |$$ |\$$$ |$$ |  $$ | \$$$  /  $$   ____|$$ |
$$ |  $$ |\$$$$$$  |$$ |\$$$$$$  |$$ | \$$ |\$$$$$$  |  \$  /   \$$$$$$$\ $$ |
\__|  \__| \______/ \__| \______/ \__|  \__| \______/    \_/     \_______|\__|
*/
...
// File contracts/roles/HoloNovelNFTAccessControls.sol
 ...
 */
contract HoloNovelNFTAccessControls is AccessControl {
...
// File contracts/HoloNovelCollection.sol


pragma solidity ^0.8.4;

contract HoloNovelCollection is
    IERC2981,
    ERC165Storage,
    Ownable,
    ERC1155,
    ...
    HoloNovelNFTAccessControls
{
    ...
    string private constant SIGNING_DOMAIN = "HoloNFT-Voucher";
    ...
    uint256 public constant MAXMINT_COUNT = 5;
    ...
    /// @dev store value of token creator
    ...
    struct NFTVoucher {
        ...
        uint256 tokenId;
        /// @notice The amount of token to be redeemed.
        uint256 quantity;
    ...
    constructor(
        ...
        address _royalty
    ) ERC1155(_tokenURIPrefix) EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) {
        name = _name;
        symbol = _symbol;
    ...
    function mint(
        ...
        require(msg.value >= mintPrice[_id], "mintprice is not enough");
    ...
    /// @dev free mint function which should be called by only admin.
    /// @param _beneficiary address where mint to
    ...
    {
        receiver = royalty;
        royaltyAmount = _salePrice.mul(ROYALTY_PERCENT).div(100);
    }
        ...
        returns (uint256)
    {
        ...
        require(hasMinterRole(signer), "Signature invalid or unauthorized");

        // make sure that the redeemer is paying enough to cover the buyer's cost
        require(msg.value >= voucher.minPrice, "Insufficient funds to redeem");

        // store minimum mint price per token id
        mintPrice[voucher.tokenId] = voucher.minPrice;

        // first assign the token to the signer, to establish provenance on-chain
        mint(signer, voucher.tokenId, voucher.quantity);
    ...
    function withdrawPendingBalance() public onlyMinter {
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "NFTVoucher(uint256 tokenId,uint256 quantity,uint256 minPrice,bytes data)"
                        ),
                        voucher.tokenId,
                        voucher.quantity,
                        voucher.minPrice,
                        keccak256(voucher.data)
                    )
                )
            );
    }
