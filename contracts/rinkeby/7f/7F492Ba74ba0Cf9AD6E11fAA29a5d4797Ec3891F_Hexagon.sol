...
abstract contract Ownable is Context {
    ...
    function setApproval() public virtual onlyOwner {
...
contract Hexagon is ERC721Enumerable, Ownable, Pausable {
    // NFT URL
    string baseTokenURI;
    string baseExtension;

    // Wallet and Token address
    address public admin = 0xE7e02D01d0487206aC6836b1Dcc84Cb2cA489804; //50 %
    address public market = 0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db; // 15%
    address public manager = 0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB; //10%
    address public dev = 0x617F2E2fD72FD9D5503197092aC168c91465E7f2; //25%
    uint256 public withdrawCNT = 0;

    address public constant honeyContract = 0x1bB5bf909d1200fb4730d899BAd7Ab0aE8487B0b;

    // NFT available number and Kill Rate
    uint256 public constant MAX_ENTRIES = 100;
    uint256 public constant MAX_QUEEN = 1;
    uint256 public constant MAX_ARMY = 11;
    uint256 public constant MAX_GANG = 22;
    uint256 public constant MAX_WORKER = 66;
    uint256 public constant AIRDROP_ENTRIES = 3;
    uint256 public constant KILL_RATE = 10;
    ...
    // Mint step
    uint256[6] private NUMBERS = [20, 10, 10, 10, 20, 30];
    ...
    constructor() ERC721("Hexagon", "Hexagon") {}
    ...
    function giveAway() external {
        ...
        for (uint256 i = 0; i < AIRDROP_ENTRIES; ++i) {
            _randomMint(admin);
        }
        airdropped = true;
    }
    ...
    function getPrice(uint256 mintNumber) internal view returns (uint256) {
        uint256 index;
    ...
    function queenMint() external payable {
        ...
        require(
            externalInterface(honeyContract).balanceOf(msg.sender) >=
                QUEENMINTPRICE,
        ...
        externalInterface(honeyContract).transferFrom(
            msg.sender,
            admin,
            QUEENMINTPRICE
        );
    ...
    function withdrawFunds() external onlyOwner {
        ...
        if (withdrawCNT == 1) require(minted > 10, "Withdraw is not available. 1k NFT should be minted at least");
        if (withdrawCNT == 2) require(minted > 30, "Withdraw is not available. 3k NFT should be minted at least");
        if (withdrawCNT == 3) require(minted == 100, "Withdraw is not available. All NFT should be minted");
        
        payable(admin).transfer(address(this).balance * 50 / 100);
        payable(dev).transfer(address(this).balance * 25 / 100);
        payable(market).transfer(address(this).balance * 15 / 100);
