...
contract CoinDeedFactory is ICoinDeedFactory, AccessControl {
    bytes32 public constant DEED_MANAGER_ROLE = keccak256("DEED_MANAGER_ROLE");
    ...
    ) external override onlyNotProcessing onlyDeedManager returns (address) {
    ...
    modifier onlyDeedManager() {
        require(
            hasRole(DEED_MANAGER_ROLE, msg.sender),
            "CoindeedFactory: Caller doesn't have permission to create a deed"
        );
        _;
    }
    ...
    function permitDeedManagers(address[] calldata _managers)
    public
    override
    onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            grantRole(DEED_MANAGER_ROLE, _managers[i]);
        }
    }

    function unpermitDeedManagers(address[] calldata _managers)
    public
    override
    onlyRole(DEFAULT_ADMIN_ROLE)
    {
        uint256 length = _managers.length;
        for (uint256 i = 0; i < length; i++) {
            revokeRole(DEED_MANAGER_ROLE, _managers[i]);
        }
    }
    ...
    function emitBuySucceed(uint256 totalSupply, uint256 purchasedTokenPrice, uint256 totalStake) external override {
        emit BuySucceed(msg.sender, totalSupply, purchasedTokenPrice, totalStake);
    }
