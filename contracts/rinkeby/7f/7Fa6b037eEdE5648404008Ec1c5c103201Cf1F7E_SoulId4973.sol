...
contract SoulId4973 is Ownable, ERC4973SoulId, EIP712Whitelist, ReentrancyGuard {
    ...
    string baseURI = "ipfs://QmPtKkoMNJ6ZAQfvpwNTXzJtP7zq7vD2n9A3Sqo4i6qw4a/";
    
    constructor(string memory name, string memory symbol) 
        ERC4973SoulId(name, symbol) 
        EIP712Whitelist() 
    {}
    ...
    modifier callerIsClaimContract() {
        address addr = msg.sender;
        require(claimContract != address(0), "Linked claim contract is not set");
        require(claimContract == addr, "Caller is not claim contract");
        _;
    }
    ...
    function claim(address _to) external nonReentrant callerIsClaimContract() {
        _runMint(_to);
    }
    ...
    function whitelistMint(bytes calldata _signature) external callerIsUser checkWhitelist(_signature) {
        _addressData[msg.sender].whitelistUsed++;
        _runMint(msg.sender);
    }
...
contract ERC4973SoulId is Ownable, ERC165, IERC4973, IERC4973SoulId, IERC721Metadata {
    ...
    /**
     * @dev Returns whether the address is either the owner or issuer
     *
     */
    function _isOwnerOrIssuer(address addr, uint256 tokenId) internal view returns (bool) {
        return (addr == ownerOf(tokenId) || addr == owner());
    }
    ...
    /**
     * @dev Returns whether the address is either the owner or guardian or issuer
     *
     */
    function _isOwnerOrGuardianOrIssuer(address addr, uint256 tokenId) internal view returns (bool) {
        return (addr == ownerOf(tokenId) || isGuardian(addr, tokenId) || addr == owner());
    }

    /**
     * @dev Returns whether address is guardian of `tokenId`. See {IERC4973SoulId-isGuardian}.
     */
    function isGuardian(address addr, uint256 tokenId) public view override returns (bool) {
     ...
     * - `tokenId` owner
     */
    function setGuardian(address guardian, bool approved, uint256 tokenId) public override {
     ...
     */
    function isApprovedForRenew(uint256 tokenId) public view override returns (bool) {
        uint256 expiry = uint256(_renewalRequest[tokenId].expireTimestamp);
        require(expiry < block.timestamp || expiry == 0, "ERC4973SoulId: renewal request expired");
        return _renewalRequest[tokenId].approved;
    }
     ...
     */
    function requestRenew(uint256 expire, uint256 tokenId) public override {
        ...
        _renewalRequest[tokenId].expireTimestamp = uint64(expire);
        _renewalRequest[tokenId].createTimestamp = uint64(block.timestamp);
     ...
     */
    function approveRenew(uint256 tokenId, bool approved) public override {
        require(owner() == _msgSender(), "ERC4973SoulId: approve renewal request from non-contract owner");
        require(!isApprovedForRenew(tokenId), "ERC4973SoulId: approve renewal request already sent");
        _renewalRequest[tokenId].approved = approved;
     ...
     * Emits a {Renew} event.
     */
    function renew(address to, uint256 tokenId) public override {
        require(_isOwnerOrGuardian(_msgSender(), tokenId), "ERC4973SoulId: unauthorized access");
        require(isApprovedForRenew(tokenId), "ERC4973SoulId: unapproved renew request");

        address owner = ownerOf(tokenId);

        unchecked {
            // reset renewal request
            _renewalRequest[tokenId].approved = false;
...
interface IERC4973SoulId {
  ...
  function isApprovedForRenew(uint256 tokenId) external view returns (bool);
