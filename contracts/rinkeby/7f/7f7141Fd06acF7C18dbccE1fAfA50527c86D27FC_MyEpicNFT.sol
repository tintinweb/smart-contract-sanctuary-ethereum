...
contract MyEpicNFT is ERC721URIStorage{
    ...
    string[] firstWords = ["Happy", "Sad", "Angry", "Love",
    "Anxiety", "Positive"];
    string[] secondWords = ["Food", "Book", "Movie", "Music",
    "Sport", "Picture"];
    string[] thirdWords = ["Dog", "Rabbit", "Cat", "Horse",
    "Penguin", "Lion"];

    event NewEpicNFTMinted(address sender, uint256 tokenId);
    event NftMintLimit(uint256 limit);
    ...
    function pickRandomFirstWord(uint256 tokenId) public view returns (string memory) {
        ...
        uint256 rand = random(string(abi.encodePacked("FIRST_WORD", Strings
    .toString(tokenId))));
    ...
    function makeAnEpicNFT() public {
        if (limit != 0) {
            ...
            // 次のNFTがMintされる時のカウンターをインクリメント
            _tokenIds.increment();
            limit -= 1;

            emit NewEpicNFTMinted(msg.sender, newItemId);
        } else {
            console.log("you cannot mint anymore!");
        }
    }
}
