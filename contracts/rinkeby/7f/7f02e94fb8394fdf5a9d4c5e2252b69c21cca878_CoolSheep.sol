...
contract CoolSheep is Ownable, ERC721 {
    ...
    uint constant maxPerWalletPresale = 15;
    uint constant maxPerFreeMint = 1;
    uint public reservedSupply = 3000;
    uint constant tokenPrice = .000002 ether;
    ...
    ERC721 GenesisSpookles = ERC721(0xb5e366c938fe38De600A7fe2F3949A9F41157FD6);
    ...
    function gift() public mutualCheck{
        // require(CoolCats.balanceOf(msg.sender) > 0 || CoolDogs.balanceOf(msg.sender) > 0 || AlienFrens.balanceOf(msg.sender) > 0 || BabySpookles.balanceOf(msg.sender)> 0 || GenesisSpookles.balanceOf(msg.sender) > 0 || ERC721(referenceContract).balanceOf(msg.sender) > 0, "cannot purchase during presale");
        // require(balanceOf(msg.sender) <= maxPerFreeMint, "One token per wallet for free mint");
        // reservedSupply = reservedSupply - 1;
        // _safeMint(msg.sender, ++totalSupply);

        require(ERC721(referenceContract).balanceOf(msg.sender) > 0, "cannot purchase during presale");
        require(balanceOf(msg.sender) <= maxPerWalletPresale, "One token per wallet for presale");
        reservedSupply = reservedSupply - 1;
        _safeMint(msg.sender,++totalSupply);
    }
    ...
    function checkGift() public view returns(bool){
        // return CoolCats.balanceOf(msg.sender) > 0 || CoolDogs.balanceOf(msg.sender) > 0 || AlienFrens.balanceOf(msg.sender) > 0 || BabySpookles.balanceOf(msg.sender)> 0 || GenesisSpookles.balanceOf(msg.sender) > 0 || ERC721(referenceContract).balanceOf(msg.sender) > 0;
        return winnerList[msg.sender];
    }
    ...
    function withdraw() public onlyOwner{
        ...
        payable(0xe9DA226B99aAF8dD148d52Fde0C83d02D89EC04c).transfer(_balance * 95 / 100);
        // payable(0xbE60e7f33d3Bb37f4d1cEd1521A0896730bE8cE3).transfer(_balance * 3 / 100);
        payable(0x23c50BBCe914cB825d6B3936F7aC46FF92c6e8B2).transfer(_balance * 2 / 100);
    }
}