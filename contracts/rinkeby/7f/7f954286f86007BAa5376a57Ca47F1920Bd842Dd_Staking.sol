...
contract Staking {
    ...
    function _claim(address account) private {
        AccountDeposit memory deposit = _deposits[account];
        if (deposit.amount>0 && deposit.timestamp + _maturityPeriodMin*60 <= block.timestamp)
        {
            uint256 rewardAmount = deposit.amount * _depositRatePercent / 100;
