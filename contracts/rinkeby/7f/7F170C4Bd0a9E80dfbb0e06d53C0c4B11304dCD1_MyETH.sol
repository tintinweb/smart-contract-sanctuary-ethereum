...
contract MyETH is ERC721A,Ownable {
    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public constant tokenPrice = 0.01 ether; // 0.0019 ETH // 0.01 ether price;
    uint256 public constant maxTokenPurchase = 3;
    uint256 public constant MAX_TOKENS = 10000;// kaç tane token var
    ...
    uint256 public presaleMaxMint = 2; // Ön satışta maksimum kaç tane mintlenir
    uint256 public devReserve = 64; // developerlar için ayrılmış.

    event EmojiMinted(uint256 tokenId, address owner);
    
    
  constructor() ERC721A("MyETH", "MyETH") {}
  ...
  function setBaseURI(string memory _newBaseURI) public onlyOwner {
   ...
   function withdraw() public payable onlyOwner { // içerideki tüm parayı al ve gönder
        (bool os, ) = payable(owner()).call{ value: address(this).balance }("");
        require(os);
    }
    function reveal() public onlyOwner { // tokenUri değiştirmek için
        isRevealed = true;
        }

    function reserveTokens(address _to, uint256 _reserveAmount) // dev için 65 tane otamatik mintliyor
    external
    onlyOwner
    {
    require(
    ...
    devReserve = devReserve.sub(_reserveAmount);
     ...
     function toggleSaleState() external onlyOwner { // Satış Aktif et
        saleIsActive = !saleIsActive;
      }
    
      function togglePresaleState() external onlyOwner { // ÖnSatış Akttif et veya durdur 
        presaleIsActive = !presaleIsActive;
      }
    
      function tokensOfOwner(address _owner) // 1 3 4 id' li tokenlar owner'a ait
        external
        view
      ...
      function mintEmoji(uint256 numberOfTokens) external payable { // satışta kullanılack
      ...
      function presaleEmoji(uint256 numberOfTokens) external payable { // ÖnSatışta kullanılacak
