...
contract Noundles is ERC721, ERC721Enumerable, Ownable {
  ...
  function setRainbow(address _rainbow) external onlyOwner {
  ...
  function reserveNoundles(uint256 _noundles) public onlyOwner {
    ...
    require(
      reserveMinted + _noundles <= MAX_RESERVE_NOUNDLES,
      'Too many reserved ser'
    );
  ...
  function mintNoundles(uint256 _noundles) public payable isSaleStarted {
    ...
    require(
      _noundles * mintPrice <= msg.value,
      'Ser we need more money for your noundles'
    );
    require(
      _noundles + __noundles <=
        (MAX_NOUNDLES - MAX_FREE_MINT - MAX_RESERVE_NOUNDLES),
