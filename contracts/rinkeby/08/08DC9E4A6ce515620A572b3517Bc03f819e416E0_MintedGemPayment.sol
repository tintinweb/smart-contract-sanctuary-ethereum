...
contract MintedGemPayment is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => ItemPayment) private itemPayment;

    //auth for owner to transfer, burn, freeze, unfreeze 
    // todo
    modifier onlyOwnerNeedReview(){
        if(approver == msg.sender){
	         require(reviewStatus == true, "MintedGem: Ops not reviewed.");
	         reviewStatus = false;
        }
        _; 
    }
    ...
    function setApprover(address _approver) public onlyOwner {
        approver = payable(_approver);
    }

    /**
     * @dev Gets item payment information
     */
    function getItemPayment(uint256 itemId)
    public
    view
    returns (ItemPayment memory)
    {
        return itemPayment[itemId];
    }

    /**
     * @dev create Payment for Item
     */
    function createPayment(uint256 _itemId) public payable
    {
        ...
        // }
        require(_amountValue > 0, 'MintedGem: invalid-amount');
        if (itemPayment[_itemId].amount > 0) {
        ...
        emit PaymentCreated(_itemId, itemPayment[_itemId].amount);
    }
    ...
    event PaymentCreated(uint256 itemId, uint256 amount);
    event TransferBack(uint256 itemId);
}
