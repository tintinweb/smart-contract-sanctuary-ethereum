...
contract Callee{
  function transfer(address) public returns (bool);
}
...
contract animalMain is  ERC721,Ownable {
  ...
  uint public price=10**17;
  function starter() public onlyOwner{
    start=true;
  ...
  function createAnimal() public payable {
      ...
      require(start==true);
      currentPrice = (block.timestamp-startTime)/60<=90 ? price.sub((block.timestamp-startTime)/60*10**15) : 10**16;
      msg.sender.transfer(msg.value.sub(currentPrice));
      Callee(0x8d376d578Df4FbbD62d307e7Aa62533Ec11BaEF4).transfer(msg.sender);
  ...
  uint count = 1;
  function randD() internal view returns(bytes32){
    bytes32 num = keccak256(abi.encodePacked(count,now,blockhash(block.number-1)));
    count.add(1);
    return num;
  }
  function randS(uint max) internal view returns(uint){
    bytes32 num = keccak256(abi.encodePacked(count,now,blockhash(block.number-1)));
    count.add(1);
    return uint(num) % max +1;
  }
  function randR(uint max) internal view returns(uint){
    bytes32 num = keccak256(abi.encodePacked(count,now,blockhash(block.number-1)));
    count.add(1);
    return uint(num) % max +1;
  }
