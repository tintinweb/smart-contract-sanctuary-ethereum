...
contract Erc721ClosedSaleV3 is Erc721BaseV3 {
  ...
  /**
    ...
    */
  constructor(ClosedSaleParams memory params_) Erc721BaseV3(params_.name, params_.symbol) {
    ...
    for (uint256 i = 0; i < params_.roles.length; i++) {
  ...
  /**
    ...
    */
  function addPayoutSplitter(
    uint96 totalRoyalty_,
    address[] memory payees_,
    uint256[] memory shares_
  ) public onlyRole(CONTRACT_ADMIN_ROLE) whenNotPaused returns (address payable) {
    ...
    PayoutSplitterV3 payoutSplitter = new PayoutSplitterV3(payees_, shares_);
  ...
  /**
    ...
    */
  function toggleSaleState() external onlyRole(CONTRACT_ADMIN_ROLE) whenNotPaused {
  ...
  /**
    ...
    */
  function togglePauseState() external onlyRole(CONTRACT_ADMIN_ROLE) {
    if (paused()) {
      _unpause();
    } else {
      _pause();
  ...
  /**
    ...
    */
  function revealSale(string memory baseURI_) external onlyRole(CONTRACT_ADMIN_ROLE) whenNotPaused {
  ...
  /**
    ...
    */
  function withdraw() external onlyRole(FINANCE_ADMIN_ROLE) whenNotPaused {
  ...
  /**
    ...
    */
  function setPricePerToken(uint256 newPricePerToken_) external onlyRole(CONTRACT_ADMIN_ROLE) whenNotPaused {
...
abstract contract RoleV3 is Ownable, AccessControl {
  /**
...
contract PayoutSplitterV3 is PaymentSplitter {
    /**
...
abstract contract Erc721BaseV3 is ERC721, ERC721Enumerable, ERC721Royalty, ERC721Pausable, RoleV3 {
  /**
  ...
  function supportsInterface(bytes4 interfaceId)
    public
    view
    override(ERC721, ERC721Enumerable, ERC721Royalty, RoleV3)
    returns (bool)
  {
    return super.supportsInterface(interfaceId);
  }
...
struct OpenSaleParams {
  ...
  bool requiresVoucher; /// This indicates that all Mint functions will need a Voucher to be passed in.
  bool requiresMultiSigner; /// This indicates if all adminTasks requires Multiple Signers.
  bytes32[] roles; /// The Access roles for the Nft Contracts.
  address[] roleAddresses; /// The addresses for the access roles.
  uint256[] roleMultiSigThresholds; /// The Multisig threholds for a Role.
...
/// @notice A struct representing the contents of a MultiSigner packet.
struct MultiSignerParams {
  uint256 multiSigTransactionId; /// A unique ID that represents an instance of a Transaction. This is used to prevent replay attacks using the same MultiSigParam.
  bytes32 roleUsed; /// This is used to obtain the MultiSig threshold for a signing Role (see OpenSaleParams.roleThresholds).
  bytes32[] signatures; /// The multiple signatures required to proceed with the transaction.
}

/// @notice A struct representing the contents of a Voucher packet.
struct VoucherParams {
  address wallet; /// An address indicating to who the Voucher has been issued. This is verified against msg.sender.
  string tokenPrice; /// The Token's Price. This is verified against msg.value.
  uint256 nonce; /// A unique ID that represents this Voucher. This is used to prevent replay attacks using the same Voucher.
}
