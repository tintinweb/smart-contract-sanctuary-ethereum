...
contract NjdaoToken is ERC721, ERC721URIStorage, Ownable {
  ...
  function activateToken(uint256 tokenId, bytes32 _hash, address _tokenOwner) public {
    require(whitelist[msg.sender] || msg.sender == owner(), "You are not authorised to verify hash");
    tokenActive[tokenId] = addressVerifyHash[tokenId][_tokenOwner] == _hash;
  }

  function verifyHash(uint256 tokenId, bytes32 _hash, address _tokenOwner) public view returns (bool) {
    require(whitelist[msg.sender] || msg.sender == owner(), "You are not authorised to verify hash");
    return addressVerifyHash[tokenId][_tokenOwner] == _hash;
  }

  function submitVerifyHash(uint256 tokenId, bytes32 _hash) public {
    require(whitelist[msg.sender], "You are not authorised to submit verify hash");
  ...
  function isAddressInWhitelist(address _whitelistAddress) view public onlyOwner returns (bool status) {
    status = whitelist[_whitelistAddress];
  }
  ...
  function transferFrom( address from, address to, uint256 tokenId)
    ...
    public 
  {
    require(tokenActive[tokenId], "token not activated, can not transfer");
    require(whitelist[to], "Address to is not authorised to own token");
