...
contract animalMain is  ERC721,Ownable {
  ...
  function createAnimal() public payable{
      ...
      //TO DO 
      //使用亂數來產生DNA, 星級, 動物種類
      bytes32 result = keccak256(abi.encodePacked(msg.sender ,block.coinbase ,blockhash(block.number-1)));
      dna = result;
      star = uint(result) % 5 + 1; //星等1-5
      roletype = uint(result) % 4 + 1;//4種

      //動手玩創意，可以限制每次建立動物需要花費多少ETH
      require(msg.value >= 1000 wei, "Please spend at least 1000 wei to get your animal");
