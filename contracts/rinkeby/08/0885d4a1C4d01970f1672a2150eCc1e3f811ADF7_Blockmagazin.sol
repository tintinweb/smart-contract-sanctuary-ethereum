...
contract Blockmagazin is ERC1155, Ownable, Pausable {
    ...
    */
    function addEdition(uint256 _tokenId, string memory _editionName, uint256 _maxSupply, address[] memory _partnerAddresses, uint256 _price) public onlyOwner {
        require(_tokenId > 0, "Invalid token id provided.");
        require(editions[_tokenId].tokenId == 0, "Edition with tokenId already created.");
        editions[_tokenId] = Edition(_tokenId, _editionName, _maxSupply, 0, _partnerAddresses, _price, false);
    }
    ...
    */
     ...
     */
    function addToEditionPartnerAddresses(uint256 _tokenId, address _address) public onlyOwner {
        require(editions[_tokenId].tokenId != 0, "Edition not found.");
        editions[_tokenId].partnerAddresses.push(_address);
    }
    ...
    function mint(uint256 _tokenId) public payable {
        ...
        bool partnerAddresses = checkPartnerAddress(editions[_tokenId].tokenId, msg.sender);
        if (!partnerAddresses) {
            require(editions[_tokenId].price <= msg.value, "The sent value is not enough.");
        }
        require(balanceOf(msg.sender, editions[_tokenId].tokenId) < 1, "Sender already own a token.");
    ...
    function checkPartnerAddress(uint256 _tokenId, address _sender) internal returns (bool) {
        bool listed = false;
        for (uint i = 0; i < editions[_tokenId].partnerAddresses.length; i++) {
            if (_sender == editions[_tokenId].partnerAddresses[i]) {
                listed = true;
                for (uint j = i; j < editions[_tokenId].partnerAddresses.length - 1; j++) {
    ...
    * - `_tokenId` edition tokeId.
    */
    function getEditionByTokenId(uint _tokenId) public view returns (Edition memory _edition) {
