...
contract fart is ERC721A, Ownable {
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "Fart - The Contract is Paused");
        require(
            tokenCount + _amount <= maxSupply,
            "Fart - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "Fart - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "Fart - max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "Fart - insufficient ethers"
            );
        }
