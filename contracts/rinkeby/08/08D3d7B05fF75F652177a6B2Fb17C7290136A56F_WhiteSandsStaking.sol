...
contract WhiteSandsStaking is IERC721Receiver, Ownable {
    ...
    mapping(address => uint256[]) public _stakedByOwners;
    TokenInfo[] public _staked;
    IERC721[] public _collections;
    PermittedOperations public _permittedOperation = PermittedOperations.ALL;
    ...
    function setPermittedOperations(uint256 permittedOperation)
        external
        onlyOwner
    {
        _permittedOperation = PermittedOperations(permittedOperation);
    }

    function acceptCollection(address collection_) external onlyOwner {
    ...
    {
        require(
            (PermittedOperations.STAKE == _permittedOperation) ||
                (PermittedOperations.ALL == _permittedOperation),
            "!op"
        );
        ...
        for (uint256 i = 0; i < collections.length; i++) {
            ...
            _indexOfTokens[collection][id][msg.sender] = _staked.length;
            _stakedByOwners[msg.sender].push(_staked.length);
            _indexOfTokensByOwners[collection][id][
                msg.sender
    ...
    {
        require(
            (PermittedOperations.UNSTAKE == _permittedOperation) ||
                (PermittedOperations.ALL == _permittedOperation),
            "!op"
        );
        require(collections.length == ids.length, "!params");
        for (uint256 i = 0; i < collections.length; i++) {
            ...
            uint256 id = ids[i];
            require(
                _indexOfTokens[collection][id][msg.sender] > 0,
                "!notfound"
            );
            if (_stakedByOwners[msg.sender].length > 1) {
                ...
                uint256 indexOfLast = _stakedByOwners[msg.sender][
                    _stakedByOwners[msg.sender].length - 1
                ];
                _stakedByOwners[msg.sender][index - 1] = indexOfLast;
                TokenInfo memory last = _staked[indexOfLast - 1];
            ...
            if (_staked.length > 1) {
                ...
                _staked[index - 1] = last;
                _indexOfTokens[_collections[last.collectionId]][last.id][
                    last.owner
                ] = index;
                uint256 indexOfLast = _indexOfTokensByOwners[
                    _collections[last.collectionId]
                ][last.id][last.owner];
                _stakedByOwners[last.owner][indexOfLast - 1] = index;
            }
            _staked.pop();
        ...
        )
    {
        ...
        for (uint256 i = 0; i < count; i++) {
            TokenInfo memory ti = _staked[_stakedByOwners[owner][i] - 1];
        ...
        )
    {
        ...
        for (uint256 i = from; i < count; i++) {
            collections[i] = address(_collections[_staked[i].collectionId]);
            ids[i] = _staked[i].id;
            owners[i] = _staked[i].owner;
            timestamps[i] = _staked[i].timestamp;
        }
