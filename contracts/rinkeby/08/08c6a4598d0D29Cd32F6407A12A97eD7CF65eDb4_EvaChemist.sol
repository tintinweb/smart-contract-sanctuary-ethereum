...
contract EvaChemist is ERC721Enumerable, Ownable, Pausable {
    ...
    function generate(uint256 tokenId) internal {
        Chemist memory newChemist = Chemist(
            ...
            randomNum(3, block.difficulty, tokenId) * 10 + 10, //possible boost values [10,20,30]
            randomNum(2, block.difficulty, tokenId) == 0 ? "+" : "-",
            randomNum(6, block.difficulty, tokenId) + 1 //utility/utility ranges from 1-6
        );
            ...
            string(
                abi.encodePacked(
                    ...
                    abi.encodePacked(
                        '{"name":"',
                        token.name,
                        '","description":"A chemist is your utility savor',
                        '","image":"',
    ...
    function getChemistOperator(uint256 tokenId)
        external
        view
        returns (string memory operator)
    {
        return Chemists[tokenId].operator;
    }

    /*
    @dev
    +Reduces the utility of a given chemist
    +callable only by lab

    */
    function decreaseUtility(uint256 tokenId) external {
        require(msg.sender == address(lab), "only lab");

        Chemists[tokenId].utility -= 1;
    }
  ...
  gets the utility of chemist
    ...
    */
    function getChemistUtility(uint256 tokenId)
        external
        view
        returns (uint256 utility)
    {
        return Chemists[tokenId].utility;
    }

    function getChemistBoostValue(uint256 tokenId)
        external
        view
        returns (uint256 boost)
    {
        return Chemists[tokenId].boostValue;
    }
...
contract Lab is Ownable, IERC721Receiver, Pausable {
    ...
    uint256 private MAX_MINT_STORE = 500; //1% chance to get a store during generation mint
    uint256 public GENESIS_PERFUMES = 1000;
    uint256 public GENESIS_FLOWERS = 9000;
    uint256 private MAX_MANUFACTURED_PERFUMES = 5000; //Perfumes minted after burning a collection of flowers with the exact total of 100cc
    uint256 public MINTED_TOKENS;
    uint256 public MINTED_FLOWERS;
    uint256 private MINTED_STORES;
    uint256 public MINTED_PERFUMES;
    uint256 public MINTED_CHEMISTS;
    uint256 private MANUFACTURED_PERFUMES;
    ...
    uint256 public florDueToPerfumeHolders;
    ...
    function _mintGenesisTokens(uint256 _amount, bool _stake) internal {
        for (uint256 i = 0; i < _amount; i++) {
            ...
            if (
                (luckyNumber == 1 && MINTED_PERFUMES < GENESIS_PERFUMES) ||
                (MINTED_FLOWERS == GENESIS_FLOWERS &&
                    MINTED_PERFUMES < GENESIS_PERFUMES)
            ) {
                if (_stake) {
                    perfume.mint(address(this), false, 0);
            ...
            if (
                (luckyNumber != 1 && MINTED_FLOWERS < GENESIS_FLOWERS) ||
                (MINTED_PERFUMES == GENESIS_PERFUMES &&
                    MINTED_FLOWERS < GENESIS_FLOWERS)
            ) {
                if (_stake) {
    ...
    */
    ...
    @Dev 
    *Calculates the mint cost depending on the token's generation 
    ...
    */
    ...
    @args Array of token ids
    */
    function makePerfumeWithChemistAssurance(
        ...
        require(
            chemist.ownerOf(chemistId) == msg.sender,
            "Not owner of chemist"
        );

        require(tokenIds.length >= 2, "Not enough flowers"); //requires a minimum of 2 flowers to make a perfume
        uint256 totalCC;
        uint256[] memory flowerTraits = new uint256[](tokenIds.length);
        uint256 chemist_utility = chemist.getChemistUtility(chemistId);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
        ...
        uint256 traitIndex = getTraitIndex(flowerTraits); //If the input is (5 boisés, 3 Floral) the returned trait is: boisés.
        ...
        if (totalCC != 100 && chemist_utility > 0) {
            string memory operator = chemist.getChemistOperator(chemistId); //gets the chemist operator +/-
            uint256 boostedCC = totalCC +
                chemist.getChemistBoostValue(chemistId);
            if (
                (boostedCC >= 100 &&
                ...
                chemist.decreaseUtility(chemistId); //decreases utility of chemist
            }
        }
        if (totalCC != 100 && chemist_utility == 0) {
    ...
    function makePerfumeWithoutChemistAssurance(uint256[] calldata tokenIds)
        public
    {
        require(
            MANUFACTURED_PERFUMES < MAX_MANUFACTURED_PERFUMES,
    ...
    function claimFlowersFlor(uint256[] calldata tokenIds) public {
        ...
        florDueToPerfumeHolders += (claimAmount * 2) / 10; //  20% tax on total Flor claimed goes to perfume holders
        flor.mint(_msgSender(), (claimAmount * 8) / 10);
    }
    ...
    function unstakeFlowers(uint256[] calldata tokenIds) public {
        ...
        if (randomNum(2, dueFlorAmount) == 1) {
            ...
            florDueToPerfumeHolders += (dueFlorAmount * 2) / 10; //Credits perfume holders due amount with 20% tax
        } else {
            florDueToPerfumeHolders += dueFlorAmount; //sends all Flor to perfume holders
        }
    }

    /*
    @Dev When unstaking perfumes player gets his perfumes and the due amount of Flor collected from taxed flowers' owners
    ...
    */
    ...
    function unstakePerfumes(uint256[] calldata tokenIds) public {
        ...
        uint256 dueFlorAmount = (florDueToPerfumeHolders * tokenIds.length) /
            STAKED_PERFUMES;
        if (dueFlorAmount > 0) {
            florDueToPerfumeHolders -= dueFlorAmount;
        }

        STAKED_PERFUMES -= tokenIds.length;
...
contract FLOR is ERC20, Ownable {
    ...
    /**
     * mints $FLOR to a recipient
     * @param to the recipient of the $FLOR
     * @param amount the amount of $FLOR to mint
     */
    ...
    /**
     * burns $FLOR from a holder
     * @param from the holder of the $FLOR
     * @param amount the amount of $FLOR to burn
     */
