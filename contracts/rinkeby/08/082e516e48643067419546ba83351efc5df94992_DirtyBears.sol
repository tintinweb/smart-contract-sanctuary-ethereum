...
contract DirtyBears is ERC721A, Ownable {
    ...
    event NewDirtyBearMinted(address sender, uint256 tokenId);

    /// All allowlist set on Merkle Tree
    bytes32 public MERKLEROOT = 0xb2962b3ac6a72a6a9878e92a2ea7a65135ba9602e959afddfeb56bec2be1c15f;
    ...
    /// Minting Variables
    string public _preRevealURI = "ipfs://QmWJqDyauRwAqbKUZiouMcr5ErVw7opNPtwifFn2sfdETg/prerevealmeta.json";
    ...
    function allowlistMint(bytes32[] calldata _proof) external {
        require(state == Status.ALLOWLIST, "Dirty Bears: Claim Is Not Live");
        require(MerkleProof.verify(_proof,MERKLEROOT,keccak256(abi.encodePacked(msg.sender))),"Invalid proof.");
        require(_numberMinted(msg.sender) < 1, "Dirty Bears: Already Minted from Address");
        require(totalSupply() + MAX_PER_ADDRESS <= MAX_SUPPLY, "Dirty Bears: Mint Supply Exceeded");
        emit NewDirtyBearMinted(msg.sender, totalSupply() + MAX_PER_ADDRESS);
        _safeMint(msg.sender, MAX_PER_ADDRESS);
    }
    
    function publicMint() external {
        require(state == Status.PUBLIC, "Dirty Bears: Public not live");
        require(_numberMinted(msg.sender) < 1, "Dirty Bears: Exceeds Max Per Wallet");
        require(totalSupply() + MAX_PER_ADDRESS <= MAX_SUPPLY, "Dirty Bears: Mint Supply Exceeded");
        emit NewDirtyBearMinted(msg.sender, totalSupply() + MAX_PER_ADDRESS);
        _safeMint(msg.sender, MAX_PER_ADDRESS);
    }

    function checkAllowlist(bytes32[] memory proof, bytes32 leaf) external view returns (bool) {
    ...
    function airdrop(address _address, uint256 _quantity) external onlyOwner {
        ...
        for (uint256 i = 1; i <= _quantity; i++) {
            emit NewDirtyBearMinted(_address, totalSupply() + i);
        }
