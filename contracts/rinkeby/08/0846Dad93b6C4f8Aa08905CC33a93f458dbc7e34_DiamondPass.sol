...
contract DiamondPass is ERC721, Ownable {
    ...
    function mint() payable public {
        require(_mintCounter[msg.sender] + 1 <= maxPerWallet, 'you can only mint one per wallet');
        require(currentMintCounter.current() <= MAX_SUPPLY, 'reached max supply');
        if (currentMintCounter.current() > 1000) {
            //First 1,000 mints are free, 1,000-2,000 = 0.1 ETH
            require(msg.value == PRICE, "Incorrect ETH amount");
        } else {
            require(msg.value == 0, "First 1,000 passes are free");
        }
