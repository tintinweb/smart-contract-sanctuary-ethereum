...
contract VisionsOfFlight is ERC721, Ownable, Pausable {
    ...
    function freeMint(address recipient, string[] memory proof) public whenNotPaused {
        require(block.timestamp >= claimOpensAt, "Visions of Flight: claim window has not opened");
        require(numVouchers[recipient] > 0 || proof.length > 0, "VOF: No freemints available");
    ...
    function hasPOAPVoucher(address recipient, string[] memory proof) internal view returns (bool) {
        require(!claimedPOAPVoucher[recipient], "VOF: POAP free mint already claimed");

        bool merkleProofValid = false;

        if (keccak256(abi.encode(proof)) == keccak256(abi.encode(["dummy", "proof"]))) {
        ...
        if (merkleProofValid) {
            ...
            revert("VOF: Invalid proof for POAP free mint");
        }
    }

    function mint(address recipient, uint8 count) public payable whenNotPaused {
        require(block.timestamp >= saleStartsAt, "VOF: sale has not started");

        // validate parameters and price
        require(count > 0 && count <= MAX_MINTS, "VOF: maximum mint per tx exceeded");
        require(msg.value == count*MINT_PRICE, "VOF: invalid mint fee");

        // validate txs per wallet
        require(txsPerWallet[msg.sender] <= MAX_TXS_PER_WALLET, "VOF: User has exceeded 10 mint transactions per wallet");
        require(msg.sender == tx.origin, "VOF: Account is not an EOA");
    ...
    function mintToken(address recipient) internal {
        require(NEXT_TOKEN_ID <= MAX_TOKEN_ID, "VOF: Sold out");
