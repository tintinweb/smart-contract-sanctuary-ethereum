...
contract Erc721OpenSaleV3 is Erc721SaleV3 {
  ...
  {
    emit NftDeployed(_msgSender(), address(this), address(0));
  }
}
...
abstract contract Erc721SaleV3 is Erc721BaseV3 {
  ...
  ) Erc721BaseV3(name_, symbol_) {
    _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
    _grantRole(CONTRACT_ADMIN_ROLE, _msgSender());

    require(bytes(name_).length > 0, "E001");
    require(bytes(baseURI_).length > 0, "E002");
    require(maxSupply_ > 0, "E004");
    require(maxTokensPerWallet_ > 0 && maxTokensPerWallet_ < maxSupply_, "E006");
    require(roles_.length == roleAddresses_.length, "E008");
    require(roles_.length > 0, "E009");

    _contractURI = contractURI_; //  If N/A, then any non-zero length string can be passed in. e.g: `none`
