...
contract PlaylistDAO is ERC721Enumerable, ReentrancyGuard, AccessControl, EIP712Whitelisting {
    ...
    uint256 public MAX_MULTIMINT_PREMIUM;
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    bytes32 public constant STAFF_MINTER_ROLE = keccak256("STAFF_MINTER_ROLE");
    ...
    constructor (
    ...
    ) ERC721(tokenName, tokenSymbol) EIP712Whitelisting(tokenName, version) {
        _setupRole(ADMIN_ROLE, msg.sender);
        _setRoleAdmin(ADMIN_ROLE, ADMIN_ROLE);
        _setRoleAdmin(STAFF_MINTER_ROLE, ADMIN_ROLE);
        ...
        MAX_MULTIMINT_PREMIUM = 1;
        MAX_MULTIMINT = 3;

        MAX_STANDARD_SUPPLY = 7500;
        MAX_PREMIUM_SUPPLY = 2500;
        MAX_STANDARD_RESERVED_SUPPLY = 400;
        MAX_PREMIUM_RESERVED_SUPPLY = 100;
    ...
    function setSaleState(bool state) external onlyRole(ADMIN_ROLE) {
        if (state) {
            require(!whitelistSaleIsActive, "Whitelist sale should not be active");
        }
        saleIsActive = state;
    }

    function setWhitelistSaleState(bool state) external onlyRole(ADMIN_ROLE) {
        if (state) {
            require(!saleIsActive, "Sale should not be active");
        }
        whitelistSaleIsActive = state;
    }

    function setStandardPrice(uint256 price) external onlyRole(ADMIN_ROLE) {
        STANDARD_PRICE = price;
    }

    function setPremiumPrice(uint256 price) external onlyRole(ADMIN_ROLE) {
        PREMIUM_PRICE = price;
    }

    function setPremiumLimitPerWallet(uint256 maxPerWallet) external onlyRole(ADMIN_ROLE) {
        MAX_PREMIUM_PER_WALLET = maxPerWallet;
    }

    function setStandardLimitPerWallet(uint256 maxPerWallet) external onlyRole(ADMIN_ROLE) {
        MAX_STANDARD_PER_WALLET = maxPerWallet;
    }

    function setMultiMint(uint256 maxMultiMint) external onlyRole(ADMIN_ROLE) {
        MAX_MULTIMINT = maxMultiMint;
    }

    function setMultiMintPremium(uint256 maxMultiMintPremium) external onlyRole(ADMIN_ROLE) {
        MAX_MULTIMINT_PREMIUM = maxMultiMintPremium;
    }
    ...
    function mintPremium(uint256 count) public payable nonReentrant {
        ...
        require(count - 1 < MAX_MULTIMINT_PREMIUM, "Trying to mint too many at a time");
    ...
    function mintStandardReserved(uint256 count) external onlyRole(ADMIN_ROLE) {
    ...
    function mintStandardReservedToAddress(uint256 count, address account) external onlyRole(ADMIN_ROLE) {
        require(totalStandardReservedSupply() + count - 1 < MAX_STANDARD_RESERVED_SUPPLY, "Exceeds max supply");
    ...
    function mintPremiumReserved(uint256 count) external onlyRole(ADMIN_ROLE) {
    ...
    function mintPremiumReservedToAddress(uint256 count, address account) external onlyRole(ADMIN_ROLE) {
        require(totalPremiumReservedSupply() + count - 1 < MAX_PREMIUM_RESERVED_SUPPLY, "Exceeds max supply");
    ...
    function mintStaffStandard(address account, uint256 count) public nonReentrant onlyRole(STAFF_MINTER_ROLE) {
        require(totalStandardSupply() + count - 1 < MAX_STANDARD_SUPPLY - MAX_STANDARD_RESERVED_SUPPLY, "Exceeds max supply");

        for (uint256 i = 0; i < count; i++) {
            standardSupplyCounter.increment();
            _safeMint(account, MAX_STANDARD_RESERVED_SUPPLY + totalStandardSupply());
        }
    }

    function mintStaffPremium(address account, uint256 count) public nonReentrant onlyRole(STAFF_MINTER_ROLE) {
        require(totalPremiumSupply() + count - 1 < MAX_PREMIUM_SUPPLY - MAX_PREMIUM_RESERVED_SUPPLY, "Exceeds max supply");

        for (uint256 i = 0; i < count; i++) {
            premiumSupplyCounter.increment();
            _safeMint(account, MAX_STANDARD_SUPPLY + MAX_PREMIUM_RESERVED_SUPPLY + totalPremiumSupply());
        }
    }

    function mintStaffStandardReserved(address account, uint256 count) public nonReentrant onlyRole(STAFF_MINTER_ROLE) {
        require(totalStandardReservedSupply() + count - 1 < MAX_STANDARD_RESERVED_SUPPLY, "Exceeds max supply");
    ...
    function mintStaffPremiumReserved(address account, uint256 count) public nonReentrant onlyRole(STAFF_MINTER_ROLE) {
        require(totalPremiumReservedSupply() + count - 1 < MAX_PREMIUM_RESERVED_SUPPLY, "Exceeds max supply");
    ...
    function setWhitelistSigningAddress(address newSigningKey) public onlyRole(ADMIN_ROLE) {
        _setWhitelistSigningAddress(newSigningKey);
    }
    ...
    function setBaseURI(string memory customBaseURI_) external onlyRole(ADMIN_ROLE) {
        customBaseURI = customBaseURI_;
    }
    ...
    function withdrawERC20(address tokenAddress) public onlyRole(ADMIN_ROLE) {
        IERC20(tokenAddress).transfer(msg.sender, IERC20(tokenAddress).balanceOf(address(this)));
    }

    /** PAYEES **/
    function changePayees(address[] memory payees, uint256[] memory shares) external onlyRole(ADMIN_ROLE) {
        _splitter = new PaymentSplitter(payees, shares);
    }
