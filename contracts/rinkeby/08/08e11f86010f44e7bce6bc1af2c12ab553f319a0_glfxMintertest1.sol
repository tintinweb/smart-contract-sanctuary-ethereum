...
interface IManifoldERC721Edition {
     ...
     */
    function mint(address creator, uint256 series1, uint256 series2, uint256 series3, uint256 series4, uint256 series5, uint256 series6, uint256 series7, address recipient) external payable;
...
// do i really need this?
     ...
     */
    function mint(address creator, uint256 series1, uint256 series2, uint256 series3, uint256 series4, uint256 series5, uint256 series6, uint256 series7, address recipient) external whenNotPaused payable override nonReentrant {
        require(msg.value >= mintRate, "Sorry, not enough ether.");
        uint16 statusCheck = 0;
        for (uint i = 1; i < 8; i++) { if(_totalSupply[creator][i] == _maxSupply[creator][i]){ statusCheck++; }}
        require(statusCheck < 7, "Fully minted");
        uint256 selectedSeries = tokenCheck(series1, series2, series3, series4, series5, series6, series7);
    ...
    function random() internal view returns (uint){
        ...
        uint randomSeed = uint(keccak256(abi.encodePacked(tough, time, msg.sender))) % 128;
        return (randomSeed);
    }
