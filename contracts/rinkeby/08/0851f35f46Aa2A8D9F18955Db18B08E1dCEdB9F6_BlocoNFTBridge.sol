...
import "./anyCall/anycall_app/ERC721Gateway_MintBurn.sol";
import "./IBlocoMintBurn721.sol";

contract BlocoNFTBridge is ERC721Gateway_MintBurn {

    constructor (address anyCallProxy, uint256 flag, address _token) ERC721Gateway_MintBurn(anyCallProxy, flag, _token) {}

    function Swapout(uint256 tokenId, address receiver, uint256 destChainID) external payable override returns (uint256) {	
        uint256 num_tokens = IBlocoMintBurn721(token).numTokens(tokenId);	
        (bool ok, bytes memory extraMsg) = _swapout(tokenId);	
        require(ok);	
        swapoutSeq++;	
        bytes memory data = abi.encode(num_tokens, msg.sender, receiver, swapoutSeq, extraMsg);	
    ...
    function Swapout_no_fallback(uint256 tokenId, address receiver, uint256 destChainID) external payable override returns (uint256) {	
        uint256 num_tokens = IBlocoMintBurn721(token).numTokens(tokenId);	
        (bool ok, bytes memory extraMsg) = _swapout(tokenId);	
        require(ok);	
        swapoutSeq++;	
...
pragma solidity ^0.8.12;
import "./anyCall/interfaces/IMintBurn721.sol";
interface IBlocoMintBurn721 is IMintBurn721{
    function mint(address account, uint256 numTokens) external;
    function numTokens(uint256 tokenId) external view returns (uint256 numTokens);
}
...
abstract contract ERC721Gateway is IERC721Gateway, AnyCallApp {
    ...
    function Swapout(uint256 tokenId, address receiver, uint256 destChainID) external payable virtual returns (uint256) {
        (bool ok, bytes memory extraMsg) = _swapout(tokenId);
        require(ok);
        swapoutSeq++;
    ...
    function Swapout_no_fallback(uint256 tokenId, address receiver, uint256 destChainID) external payable virtual returns (uint256) {
        (bool ok, bytes memory extraMsg) = _swapout(tokenId);
        require(ok);
        swapoutSeq++;
        bytes memory data = abi.encode(tokenId, msg.sender, receiver, swapoutSeq, extraMsg);
