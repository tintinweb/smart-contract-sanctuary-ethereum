...
contract MoneyBears is ERC721A, Ownable, ReentrancyGuard{
    ...
    uint256 public maxRoundOneSupply = 1665;
    uint public maxRoundTwoSupply = 8999;
    uint public roundOneMintPrice = .001 ether;
    uint public roundTwoMintPrice = .001 ether;
    ...
    mapping(address => uint256) public roundOneMints;
    mapping(address => uint256) public roundTwoMints;


    address wlAuthorizerRoundOne = 0x29e7844BD4b7478497dfbFb7610c51AFA3FdfF0c;
    address wlAuthorizerRoundTwo;

    
    bool public revealed;
    //False on mainnet
    bool public presaleRoundOneLive = true;
    bool public publicRoundOneLive = true;
   ...
   function roundOneWhitelistMint(uint amount, uint max, bytes memory signature) external payable{
       ...
       if(roundOneMints[msg.sender] + amount > max) revert MintingTooMany();
       if(totalSupply() + amount > maxRoundTwoSupply) revert SoldOut(); 

       roundOneMints[msg.sender] += amount;
       _mint(msg.sender,amount);
   }
   ...
   function roundTwoWhitelistMint(uint amount, uint max, bytes memory signature) external payable{
    ...
    if(roundTwoMints[msg.sender] + amount > max) revert MintingTooMany();
    if(totalSupply() + amount > maxRoundTwoSupply) revert SoldOut(); 

    roundTwoMints[msg.sender] += amount;
    _mint(msg.sender,amount);
}
