...
contract Bank{
	...
	mapping(address => mapping(string => uint256)) public ERC20accountBalances;
	...
	function depositTokens(uint256 amount, string memory symbol) external {
		ERC20accountBalances[msg.sender][symbol] += amount;
	...
	function withdrawTokens(uint256 amount, string memory symbol) external {
		require(ERC20accountBalances[msg.sender][symbol] >= amount, 'Insufficent funds');

		ERC20accountBalances[msg.sender][symbol] -= amount;
		ERC20(whitelistedTokens[symbol]).transfer(msg.sender, amount);
	}
	
	function ETHreceiveMoney() payable public {ETHBalanceOf[msg.sender]+=msg.value;}
	
	function ETHgetBalance(address addr) public view returns(uint) {
        return ETHBalanceOf[addr];
    }
	
	function ETHgetBalanceOfMe() public view returns(uint) {
        return ETHBalanceOf[msg.sender];
    }

	function ETHwithdrawMoney(uint amount) public {
		require(amount<=ETHBalanceOf[msg.sender]);
        address payable to = payable(msg.sender);
        ETHBalanceOf[to]-=amount;
        to.transfer(amount);
    }

	function ETHwithdrawMoneyTo(address payable _to,uint amount) public {
        require(amount<=ETHBalanceOf[msg.sender]);
		_to.transfer(amount);
		ETHBalanceOf[msg.sender]-=amount;
		ETHBalanceOf[_to]+=amount;
    }
}
