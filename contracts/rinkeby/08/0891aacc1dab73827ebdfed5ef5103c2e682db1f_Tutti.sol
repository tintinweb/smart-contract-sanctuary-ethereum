...
contract Tutti is ERC721AQueryable, Ownable, ReentrancyGuard, SignedTokenVerifier, Pausable {
  ...
  constructor(string memory baseURI, address _signer) ERC721A("Tutti", "TF") {
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) whenNotPaused {
    _safeMint(_msgSender(), _mintAmount);
  }

  function freeMint(uint256 _amount, bytes calldata _token, string calldata _email, string calldata _salt) public whenNotPaused nonReentrant {
