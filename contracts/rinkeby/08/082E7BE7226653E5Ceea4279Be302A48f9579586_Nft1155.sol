...
import "./rarible/RaribleUserToken.sol";
contract Nft1155 is RaribleUserToken {
    address tempSinger = 0x62862bE76377c764D5396d5aA5B9B6A9b78e76b8;
    constructor() RaribleUserToken("parallel", "LL", "https://parallel.life/contract-data/1155", "https:/", tempSinger)public{}

    function simpleMint(uint256 id, Fee[] memory fees, uint256 supply, string memory uri) public onlyOwner {
        _mint(id, fees, supply, uri);
    }

    function batchMint(uint256[] memory _ids, uint256[] memory _supplies, Fee[] memory fees) public onlyOwner {
        for(uint i=0; i<_ids.length; i++) {
            simpleMint(_ids[i], fees, _supplies[i], "https://paralleltest.com");
        }
    }
}
