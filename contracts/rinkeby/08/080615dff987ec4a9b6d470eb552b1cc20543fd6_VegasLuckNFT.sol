...
contract VegasLuckNFT is ERC721Enumerable, Ownable {
    ...
    uint256 private TIER1_COUNTER;
    uint256 private TIER2_COUNTER;
    uint256 private TIER3_COUNTER;
    ...
    function mintTier1(uint256 numberOfTokens, uint256 referralCode) external payable {
        ...
        require(TIER1_COST * numberOfTokens <= msg.value,"ETH amount is not sufficient");

        if (MAP_ADDRESS_FROM_REFERRAL[referralCode] != address(0)) {
            payable(MAP_ADDRESS_FROM_REFERRAL[referralCode]).transfer(msg.value * 15 / 100);
        }
    ...
    function mintTier2(uint256 numberOfTokens, uint256 referralCode) external payable {
        ...
        require(TIER2_COST * numberOfTokens <= msg.value,"ETH amount is not sufficient");
    ...
    function mintTier3(uint256 numberOfTokens, uint256 referralCode) external payable {
        ...
        require(TIER3_COST * numberOfTokens <= msg.value,"ETH amount is not sufficient");
