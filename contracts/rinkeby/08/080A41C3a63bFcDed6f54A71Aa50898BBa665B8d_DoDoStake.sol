...
contract DoDoStake is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 private constant RATE_NORMAL = (5 * 60); // 1 token per 5 minutes
    uint256 private constant RATE_SLOW = (30 * 60); // 1 token per 30 minutes

    constructor(address dodoContractAddress, address faithContractAddress) {
        _pause();
        __deployTime = block.timestamp;
        __slowRewardsTimeStamp = __deployTime + (27 * 24 * 60 * 60); // 27 days later
    ...
    function claim(uint256 tokenId) internal {
        ...
        uint256 rewardQuntity = calculateRewardQuantity(stakedDoDo);
    ...
    function getClaimableFaith(uint256 tokenId) public view returns (uint256) {
        StakedDoDo memory stakedDoDo = __stakedDoDos[tokenId];
        return calculateRewardQuantity(stakedDoDo);
    }

    function calculateRewardQuantity(StakedDoDo memory stakedDoDo) internal view returns (uint256) {
        if (stakedDoDo.start > __slowRewardsTimeStamp) {
            return ((block.timestamp - stakedDoDo.start) / RATE_SLOW) * (10**18);
        }
        if (block.timestamp > __slowRewardsTimeStamp) {
            uint256 durationSlow = block.timestamp - __slowRewardsTimeStamp;
            uint256 durationNormal = __slowRewardsTimeStamp - stakedDoDo.start;
            return ((durationSlow / RATE_SLOW) + (durationNormal / RATE_NORMAL)) * (10**18);
        }
        return ((block.timestamp - stakedDoDo.start) / RATE_NORMAL) * (10**18);
    }
    ...
    function setSlowRewardsTimeDiff(uint256 _timeDiff) public onlyOwner {
        __slowRewardsTimeStamp = __deployTime + _timeDiff;
    }
}
