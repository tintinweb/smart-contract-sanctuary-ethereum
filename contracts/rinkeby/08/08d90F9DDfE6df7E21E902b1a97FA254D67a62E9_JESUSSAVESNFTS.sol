...
contract JESUSSAVESNFTS is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    // string public maxFreeMintsPerTx = 1;
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        ...
        require(
            mintedFreeAmount[msg.sender] <= maxFreeMintsPerTx,
            "Exceeds free max per tx!"
        );
        require(
            _numberMinted(msg.sender) + _mintAmount <= maxPerWallet,
            "Exceeds max wallet limits!"
        );
        require(
        ...
        mintPriceCompliance(_mintAmount)
    {
        ...
        require(balanceOf(msg.sender) <= 1, "Only 1 NFT per");
