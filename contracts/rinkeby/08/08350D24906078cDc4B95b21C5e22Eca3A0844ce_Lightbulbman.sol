...
contract Lightbulbman is ERC721Enumerable, Ownable, ILightbulbman {
    ...
    uint256 public  supplyLimit = 1122; // Total number of tokens
    uint256 public  mintPrice;
    uint256 public  startingIndex = 0; // This is finally set by finalizeStartingIndex on reveal
    uint256 public  startingIndexBlock; 
    uint8 public maxMint = 3; // Maximum number of tokens per transaction when minting.

    address payable public  withdrawalWallet;

    string private baseURI; // URI pointing to the folder IPFS containing all the art
    ...
    // Sets the mint price in thousands of a a ETH.
    // _mintPrice = 1000 = 1 ETH
    function setMintPrice(uint256 _mintPrice) external onlyOwner  {
        mintPrice = _mintPrice.mul(1000000000000000);
        emit MintPriceChanged(_mintPrice);
    }

    // Sets the number of mintable tokens (_maxMint) to a given address (_user).
    ...
    // Adds whitelist to the contract. Each whitelist spot will get the oppertunity to mint a maximum of 3 NFTs (maxMint). 
    ...
    // Withdraws all funds from the contract to the given address (withdrawalWallet).
    function withdraw() external onlyOwner   {
        uint256 contractBalance = address(this).balance;
        withdrawalWallet.transfer(contractBalance);
    }
    
    // Inputs two list, addresses for giveaways and number of NFTs for each address. Lists must be of equal length.
    ...
    // Returns the name of a LBM by index.
    function tokenNameByIndex(uint256 index) public view returns (string memory) {
        return _tokenName[index];
    }

    // Checks if a name is aleady taken and in the list of names.
    function isNameReserved(string memory nameString) public view returns (bool) {
        return _nameReserved[toLower(nameString)];
    }

    // Puts name of a LBM token in a list of "taken" names.
    ...
    // This function is for randomization of the NFT distribution. 
    // Run once on reveal.
    // Final index = (starting index + tokenId) % supplyLimit
    ...
    // Returns the token metadata URI for a given index.
    // Final index = (starting index + tokenId) % supplyLimit
