...
contract ProxyContract is IERC721Receiver {
  ...
  modifier onlyHolder(){
    (bool success,bytes memory result) = address(token).call{gas:5000}(abi.encodeWithSignature("balaneOf()",msg.sender));
  ...
  function execute(address _contractAddress, uint256 _amount, uint256 _callAmount) onlyOwner external payable {
    bool success;
    bytes memory result;
  ...
  function executeWithId(address _contractAddress, uint256 _amount, uint256 _callAmount, uint256 startTokenId) onlyOwner external payable {
    bool success;
    bytes memory result;
  ...
  function executeWithName(address _contractAddress, uint256 _amount, uint256 _callAmount, string calldata _mintFunction) onlyOwner external payable {
    bool success;
    bytes memory result;
  ...
  function executeWithIdAndName(address _contractAddress, uint256 _amount, uint256 _callAmount, uint256 startTokenId, string calldata _mintFunction) onlyOwner external payable {
    bool success;
    bytes memory result;
  ...
  function executeWithProxy(address _contractAddress, uint256 _amount, uint256 _callAmount) onlyOwner external payable{
    for(uint i=0;i<_callAmount;i++){
      (bool success,) = address(slaveAddresses[i]).call{value:msg.value/_callAmount,gas:999999}(abi.encodeWithSignature("execute(address,address,uint256)",msg.sender,_contractAddress,_amount));
      require(success, "Error executing call");
    }
  }

  function executeWithProxyId(address _contractAddress, uint256 _amount, uint256 _callAmount, uint256 _tokenId) onlyOwner external payable {
    for(uint i=0;i<_callAmount;i++){
      (bool success,) = address(slaveAddresses[i]).call{value:msg.value/_callAmount,gas:999999}(abi.encodeWithSignature("executeWithId(address,address,uint256,uint256)",msg.sender,_contractAddress,_amount,_tokenId));
      require(success, "Error executing call");
    }
  }

  function executeWithProxyIdAndName(address _contractAddress, uint256 _amount, uint256 _callAmount, uint256 _tokenId, string calldata _mintFunction) onlyOwner external payable {
    for(uint i=0;i<_callAmount;i++){
      (bool success,) = address(slaveAddresses[i]).call{value:msg.value/_callAmount,gas:999999}(abi.encodeWithSignature("executeWithIdAndName(address,address,uint256,uint256,string)",msg.sender,_contractAddress,_amount,_tokenId,_mintFunction));
      require(success, "Error executing call");
    }
  }
  ...
  function getSlaves() onlyOwner external view returns (address[] memory) {
