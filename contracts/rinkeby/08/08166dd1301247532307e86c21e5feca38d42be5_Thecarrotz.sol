...
contract Thecarrotz is ERC721Enumerable, Ownable, PaymentSplitter {
  using Strings for uint256;
  ...
  uint public wlMaxPerWallet = 0;
  uint public publicMaxPerTx = 5;
  uint public publicMaxPerWallet = 0;
  ...
  function whitelistMint(uint _mintAmount, bytes32[] calldata _proof) public payable {
    ...
    if (msg.sender != owner()) {
        if(totalFree + _mintAmount <= maxFree) {
            ...
            if((totalFree + _mintAmount) - maxFree <= wlMaxPerTx) {
                if (!claimedMint[msg.sender]) {
                    require(msg.value >= cost * _mintAmount - cost, "Sorry you did not send enough ether");
                    claimedMint[msg.sender] = true;
		            totalFree += 1;
