...
//Mint
    ...
    if (currentID == 1){
        if (balanceOf(refadd, 1) > 0) {ref = true;} 
       // if (balanceOf(msg.sender, 1) > 0) {newal = true;} 
         if (user.date.length > 0) {newal = true;}  
       //FOR TEST
...
function turbo() external payable {
    ...
    require(user.date.length > 0, "Alien NFT detected. The first Aura NFT Power 50 must be minted from your wallet!");
    user.qalification = true;
    user.date.push(block.timestamp);
    user._sum.push(0);
    user._dynrew.push(user.balance);
