...
contract HoloNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    XERC721 public xoid; 
    address signerAddress = 0xB6d56a1313bBb18923826E2cDb171de784317e4d;
    string uri;
    uint price;
    bool publicMint = false;

    uint public MAX_TOTAL_SUPPLY = 33333;
    uint public capSupply = 8888;

    mapping(uint => int) public redeemed_xoids;
    mapping(address => int) public redeemed_frens;

    constructor(address _xoid, string memory _uri, address sbAddress) ERC721A("HOLOGENX", "HOLOG") {
        xoid = XERC721(_xoid);
        uri = _uri;

        _mint(0x1B74C5B5F3698EF9f7227C0137cb9C917B94ee8c, 444); // vault
        _mint(sbAddress, 88);
    }

    function contractURI() public view returns (string memory) {
        return string(abi.encodePacked(_baseURI(), abi.encodePacked("contract")));
    }
