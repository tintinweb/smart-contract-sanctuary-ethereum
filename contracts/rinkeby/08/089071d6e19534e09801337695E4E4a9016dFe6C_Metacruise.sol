...
contract Metacruise is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  address private assistance;
  address private voxlead;
  address private treasury1;
  address private treasury2;


  uint256 private shares_dev1;
  uint256 private shares_dev2;
  uint256 private shares_assistance;
  uint256 private shares_voxlead;
  uint256 private shares_treasury1;
  uint256 private shares_treasury2;

  // Events
  event Mint(address recipient, uint256 amount);
  event WhitelistToggle(bool whitelistState);
  event SaleToggle(bool saleState);
  ...
  constructor() ERC721("Metacruise", "CRUISER") {
    ...
    // Set shareholders
    dev1 = 0xa22ca176041574ac4499CB3bab1CA36540ecCe1c ;
    dev2 = 0xD3Ad4eeb4Da848C8Ff7Bff65A420C580Dd63821d ;

    treasury1 = 0xdd0c5971813EBFC7835E54580A0128811b6f4e00 ;
    treasury2 = 0x9C517399217627e06305fA5295aB23cE1a0C0D47 ;

    voxlead = 0xa22ca176041574ac4499CB3bab1CA36540ecCe1c ;
    assistance = 0x48383D22742BDE87A2947CD5318b6dC199AB2188 ;
    ...
    shares_treasury1 = 2500;
    shares_treasury2 = 2500;

    shares_voxlead = 700;
    shares_assistance = 300;

    // Set addresses of admins and whitelist them
    admins[dev1] = true;
    admins[dev2] = true;

    whitelistedAddresses[dev1] = true;
    whitelistedAddresses[dev2] = true;
  }
  ...
  function acquire(address recipient, uint256 amount) public payable nonReentrant {
    ...
    if(isWhitelistActive){
      ...
      require(tokenIndex < 1000, "only 1000 avalable during whitelist");
  ...
  // Whitelists a batch of addresses
  function addWhitelisted(address[] memory accounts) public onlyAdmin
  {
      for (uint256 account = 0; account < accounts.length; account++) {
          whitelistedAddresses[accounts[account]] = true;
      }
  }

  // Removes a batch of addresses from the whitelist
  function removeWhitelisted(address[] memory accounts) public onlyAdmin
  {
      for (uint256 account = 0; account < accounts.length; account++) {
          whitelistedAddresses[accounts[account]] = false;
      }
  }

  // Whitelist switch
  function stopWhitelist() public onlyAdmin{
    require(isWhitelistActive == true);
    isWhitelistActive = false;
    emit WhitelistToggle(isWhitelistActive);
  }

  function startWhitelist() external onlyAdmin{
    require(isWhitelistActive == false);
  ...
  function withdrawEarnings() public onlyAdmin {
      ...
      // Send calculated cut to every shareholder
      Address.sendValue(payable(dev1), balance * shares_dev1 / totalShares);
      Address.sendValue(payable(dev2), balance * shares_dev2 / totalShares);
      Address.sendValue(payable(treasury1), balance * shares_treasury1 / totalShares);
      Address.sendValue(payable(treasury2), balance * shares_treasury2 / totalShares);
      Address.sendValue(payable(voxlead), balance * shares_voxlead / totalShares);
      Address.sendValue(payable(assistance), balance * shares_assistance / totalShares);
  }
}
