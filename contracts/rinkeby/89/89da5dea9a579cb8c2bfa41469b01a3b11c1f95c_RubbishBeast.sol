...
contract RubbishBeast is ERC2981, ERC721AQueryable, Ownable {
   ...
   // IFlyCoin public immutable _flyCoin;
   // uint256 public immutable _flyCoinPerPair;
    ...
    constructor(
        ...
        uint32 teamSupply,
        uint32 walletLimit
    ) ERC721A("TestFLYBeast", "TTTTTTTTTTTTTTTB") {
        require(fly != address(0));
        // require(flyCoin != address(0));
        require(maxSupply >= holderSupply + teamSupply);

        _fly = IERC721(fly);
        // _flyCoin = IFlyCoin(flyCoin);
        // _flyCoinPerPair = flyCoinPerPair;
        _price = price;
    ...
    function rubbishClaim(uint256[] memory tokenIds) external {
        ...
        // _flyCoin.holderClaim(msg.sender, pairs * _flyCoinPerPair);
    ...
    function _status(address minter) external view returns (Status memory) {
        ...
        return Status({
            ...
            // flyCoinPerPair: _flyCoinPerPair,
