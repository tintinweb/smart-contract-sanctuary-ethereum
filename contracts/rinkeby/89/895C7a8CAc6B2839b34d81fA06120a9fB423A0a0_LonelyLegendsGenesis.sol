...
// Presale functions
  ...
  function holderMint(uint256 _mintAmount) external nonReentrant {
    ...
    require(_mintAmount > 0 && _mintAmount <= 2, "You can only mint 2 legends per transaction.");
    require(userMintedHolderSale[msg.sender] + _mintAmount <= 2, "You can only mint 2 Legends during the holder sale.");
  ...
  function preSaleMint (uint256 _mintAmount) external nonReentrant {
    ...
    require(preSaleSold + _mintAmount <= maxSupply, "The presale is finished!");
  ...
  function setlegendsList(address[] calldata legendsListers, bool holderOGRank) external onlyOwner {
    uint256 quantity = holderOGRank ? 2 : 1;
