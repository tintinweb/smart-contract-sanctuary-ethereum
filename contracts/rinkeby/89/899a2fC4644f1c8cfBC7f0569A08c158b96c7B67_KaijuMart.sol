...
contract KaijuMart is AccessControl, ReentrancyGuard {
    ...
    function _create(
        ...
        internal
    {
        if (!lot.redeemer.supportsInterface(type(IKaijuMartRedeemable).interfaceId))
    ...
    /**
     * @notice Increase the sender's current bid on an auction lot
     * @dev Auctions cannot accept `EITHER` PaymentType so we can just assume the token type from the auction details
     * @param auctionId Lot identifier for the auction
     * @param value Amount to increase the current bid by
     */
    function increaseBid(
        ...
        onlyKingz
    {
        ( uint104 increase, uint104 newBid ) = managerContracts.auction.increaseBid(auctionId, value, _msgSender());
...
interface IAuctionManager {
    ...
    function increaseBid(uint256 id, uint104 value, address sender) external returns (uint104, uint104);
