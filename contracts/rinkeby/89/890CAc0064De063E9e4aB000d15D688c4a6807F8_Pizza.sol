...
interface A {
    ...
    function traitTypes(uint256 i, uint256 j) external view returns (
...
A a = A(0x175859851690ad76009cE1f8048Da982A6fd389A);
    ...
    constructor() ERC721("Pizza", "WTF") {
        _owner = msg.sender;    
        
        //bg colors
        TIERS[0] = [364, 20, 954, 2007, 45, 499, 990, 333, 2447, 118];
        
        
        //basis colors
        TIERS[1] = [1671, 20, 929, 2323, 2343, 491];
        
        //Thickness
        TIERS[2] = [4277, 3500];
        
        //Crust
        TIERS[3] = [3211, 2165, 473, 20, 256, 630, 1022];
        
        //Cheese
        TIERS[4] = [204, 2451, 772, 1239, 3111];
        
        //Topping
        TIERS[5] = [340, 792, 507, 1780, 12, 555, 2732, 60, 999];
        
        //Vegetable
        TIERS[6] = [280, 327, 424, 2420, 1793, 1669, 864];
        
        //Spice
        TIERS[7] = [359, 976, 2968, 1252, 2210, 12];

        //Size
        TIERS[8] = [3992, 444, 3325, 16];        

    }

  
    /**
     * @dev Converts a digit from 0 - 7777 into its corresponding rarity based on the given rarity tier.
     * @param _randinput The input from 0 - 7777 to use for rarity gen.
    ...
    /**
     * @dev Mints new tokens. require(balanceOf(_msgSender()) <= 2,"Max mints per wallet = 1");
     */
    function bakePizza(uint256 _quantity) public {
...
*/
    ...
    function getTraits(uint256 i, uint256 j) internal view returns(Trait memory) { 
       try a.traitTypes(i,j) returns(
             string memory traitName, 
             string memory traitType, 
             string memory pixels,
             uint256 pixelCount
         ) {
             return Trait(traitName, traitType, pixels, pixelCount);
         } catch (bytes memory){
        ...
        returns (string memory)
    {
        ...
        bgRect = string(
                    abi.encodePacked(
                        '<rect class="bg" fill="#',
                        getTraits(0,Oven.parseInt(Oven.substring(_hash, 0, 1))).pixels,
                        '"/>'
                    )
                );
        ...
        basisRect = string(
                    abi.encodePacked(
                        '<circle fill="#',
                        getTraits(1,Oven.parseInt(Oven.substring(_hash, 1, 2))).pixels,
                        '" cx="12" cy="12" r="9.9" />'
                    )
                );
        ...
        if(Oven.compareStrings(getTraits(2,Oven.parseInt(Oven.substring(_hash, 2, 3))).traitName, "Thick")){  
             crustColor = string(
                    abi.encodePacked('c',
                    Oven.substring(getTraits(3,Oven.parseInt(Oven.substring(_hash, 3, 4))).pixels, 14, 16)
                    )
                );
        ...
        for (uint8 i = 3; i < 8; i++) {
            ...
            for (
                uint16 j = 0;
                j < getTraits(i,thisTraitIndex).pixelCount;
                j++
            ) {
                
                string memory thisPixel;
                
                if(j <= getTraits(i,thisTraitIndex).pixelCount - 1){
                 thisPixel = Oven.substring(
                    getTraits(i,thisTraitIndex).pixels,
                    j * 4,
                    j * 4 + 4
                );
        ...
        for (uint8 i = 0; i < 9; i++) {
            ...
            metadataString = string(
                abi.encodePacked(
                    metadataString,
                    '{"trait_type":"',
                    getTraits(i,thisTraitIndex).traitType,
                    '","value":"',
                    getTraits(i,thisTraitIndex).traitName,
                    '"}'
                )
            );
