...
contract NFTDistributor is AccessControl {
    ...
    uint256 public tokenPrice =  0.01 ether;  //uint256(1225 * 10**16);  
    ...
    INFT public nft;
    uint256 public nft_lastTokenId;
    uint256 public nft_maxTokenId;
    ...
    constructor(INFT _nft, uint256 _lastTokenId, uint256 _maxTokenId) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);

        withdrawalAddress = payable(msg.sender);

        nft = _nft;
        nft_lastTokenId = _lastTokenId;
        nft_maxTokenId = _maxTokenId;
    ...
    function setConfig_token(INFT _nft)
    ...
    function mint(uint256 _num) payable public returns (bool result) {
        ...
        require(nft_lastTokenId + _num <= nft_maxTokenId, "Maximum cap reached");

        for(uint256 i=0; i<_num; i++){
            nft_lastTokenId++;
            totalMints++;
            nft.mint(msg.sender, nft_lastTokenId);
        }
