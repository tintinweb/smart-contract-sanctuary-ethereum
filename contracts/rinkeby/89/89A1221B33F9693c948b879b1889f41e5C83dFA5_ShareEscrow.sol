...
contract ShareEscrow is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    error InvalidData();
    ...
    modifier noChange(address _old, address _new) {
    ...
    /// @notice create multiple tasks with same dao and token.
    function createTasks(
        ...
        emit CreateTasks(_taskId, _sender, _token, _amountList, _offchainDetailsList);

        // --- NO INTERNAL STATE CHANGE MUST HAPPEN AFTER THIS ---

        // Transfer escrow amount and fee to this contract
        // _sender (dao) must have approved escrow amount + fee to this contract.
        IERC20(_token).safeTransferFrom(_sender, address(this), _totalAmountWithFee);
    }
    ...
    function assignTask(bytes calldata _data, bytes calldata _signature) external {
        ...
        ) = abi.decode(_data, (SignatureFunction, uint256, address, uint256, address, bytes));
        ...
        // Check dao signature
        _checkUserSignature(_task.dao, _hash, _signature, 0);

        // Check new contributor signature
        _checkUserSignature(_contributor, _hash, _signature, 1);

        // delete approved hashes
        _deleteApprovedHash(_task.dao, _hash);
        _deleteApprovedHash(_contributor, _hash);

        // Set contributor
        tasks[_taskId].contributor = _contributor;

        // Set task as Active
        tasks[_taskId].status = Status.Active;
    ...
    function completeTasks(bytes calldata _data, bytes calldata _signature) external {
        ...
        ) = abi.decode(_data, (SignatureFunction, uint256[], address, uint256, bytes[]));

        _validateData(SignatureFunction.CompleteTasks, _decodedFunction, _contractAddress, _chainId);

        _completeTasksDivided(_data, _signature, _taskIdList, _offchainDetailsList);
    }
    ...
    function cancelTask(bytes calldata _data, bytes calldata _signature) external {
        ...
        // delete approved hashes
        _deleteApprovedHash(_task.dao, _hash);
        _deleteApprovedHash(_task.contributor, _hash);
    ...
    function raiseDispute(uint256 _taskId, bytes calldata _offchainDetails) external {
        ...
        // Only task's dao or contributor can raise dispute
        if (_sender != _task.dao && _sender != _task.contributor) revert OnlyDaoOrContributor();

        // Revert if task is not Active
        if (_task.status != Status.Active) revert InvalidStatus(_task.status, Status.Active);

        // Set task status as InDispute
        tasks[_taskId].status = Status.InDispute;

        emit RaiseDispute(_taskId, _sender, _offchainDetails);
    }
    ...
    function resolveDispute(
        ...
        if (_decision == 0) {
            ...
            tasks[_taskId].status = Status.Complete;
    ...
    function changeTreasury(address _newTreasury) external onlyOwner noChange(treasury, _newTreasury) {
    ...
    function changeArbitrator(address _newArbitrator) external onlyOwner noChange(arbitrator, _newArbitrator) {
    ...
    function _cancelTask(uint256 _taskId, bytes memory _offchainDetails) internal {
        ...
        tasks[_taskId].status = Status.Cancel;

        emit CancelTask(_taskId, _offchainDetails);

        // --- NO INTERNAL STATE CHANGE MUST HAPPEN AFTER THIS ---

        // Transfer tokens back to dao
        IERC20(_task.token).safeTransfer(_task.dao, _task.amount + _task.fee);
    }

    function _completeTasksDivided(
        ...
        Task memory _firstTask = tasks[_taskIdList[0]];
        address _token = _firstTask.token;
        address _dao = _firstTask.dao;
        address _contributor = _firstTask.contributor;

        // Check dao signature
        _checkUserSignature(_dao, _hash, _signature, 0);

        // Check new contributor signature
        _checkUserSignature(_contributor, _hash, _signature, 1);

        // delete approved hashes
        _deleteApprovedHash(_dao, _hash);
        _deleteApprovedHash(_contributor, _hash);
        ...
        for (uint256 i; i < _loopLength; ) {
            Task memory _task = tasks[_taskIdList[i]];

            // Make sure task token, dao and contributor are similar to decoded.
            if (_task.token != _token || _task.dao != _dao || _task.contributor != _contributor) {
                revert InvalidData();
            }
            ...
            // Set task status as complete
            tasks[_taskIdList[i]].status = Status.Complete;
    ...
    function _deleteApprovedHash(address _address, bytes32 _hash) internal {
