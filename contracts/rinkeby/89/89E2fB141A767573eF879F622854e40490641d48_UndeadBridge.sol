...
contract UndeadBridge is EIP712Upgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
  ...
  mapping(uint256 => Record) public swapRecord;
  ...
  event Swap(address _sender, uint256 _swapId, address _token, uint256 _amount, uint256 _sigTime, uint256 _time);
  ...
  function setFeeConfig(uint256 _swapFee, address _feeHolder) external onlyRole(MODIFICATION_ROLE) {
    swapFee = _swapFee;
  ...
  function setSwapEnabled(bool _status) external onlyRole(MODIFICATION_ROLE) {
    swapEnabled = _status;
  }
  ...
  function swap(
    ...
    require(swapRecord[_swapId].amount == 0, "already executed");
    require(block.timestamp <= _sigTime + receiveTime, "sig expired");

    // Authentication
    _verify(sender, _swapId, _token, _amount, _sigTime, _signatures);

    uint256 feeAmount = (_amount * swapFee) / 100e2;
    // Log
    swapRecord[_swapId] = Record(_swapId, sender, _token, _amount, feeAmount, block.timestamp);
    ...
    emit Swap(sender, _swapId, _token, _amount, _sigTime, block.timestamp);
  }
