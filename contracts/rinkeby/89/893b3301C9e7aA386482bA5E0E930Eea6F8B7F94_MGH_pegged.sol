...
interface ApprovalReceiver {
    function approvalGiven(address owner, uint256 amount, bytes memory data) external returns(bool);
}

contract MGH_pegged is MintSwapCanonicalToken {
    ...
    function burnPeggedToken(address account, uint256 amount) public {
    ...
    function setDefaultApproved(address account, bool approved) external onlyOwner {
        require(isContract(account), "EOA cannot be default approved");
        require(defaultApproved[account] != approved, "already set");
        defaultApproved[account] = approved;
    }

    function approveAndCall(address spender, uint256 amount, bytes memory data) external {
        require(isContract(spender), "spender is not a contract");
        require(approve(spender, amount), "approval failed");
        require(
            ApprovalReceiver(spender)
                .approvalGiven(_msgSender(), amount, data),
            "callback falied"
        );
    }

    function allowance(address owner, address spender) public view virtual override returns (uint256) {
        return defaultApproved[spender]
            ? type(uint256).max
