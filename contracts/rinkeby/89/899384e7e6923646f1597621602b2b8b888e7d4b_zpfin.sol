...
contract zpfin is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    ...
    string memory baseTokenURI_
  ) ERC721A("zpfin", "zpf", maxBatchSize_, collectionSize_, collectionSizeFree_, baseTokenURI_) {
  ...
  function mint(uint256 quantity) external payable {address _caller = _msgSender();
        ...
        if(collectionSizeFree < totalSupply() + quantity || freemintAmount < quantity)
