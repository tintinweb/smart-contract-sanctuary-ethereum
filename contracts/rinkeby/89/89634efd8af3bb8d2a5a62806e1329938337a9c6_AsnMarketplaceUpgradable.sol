    ...
    IERC1155ReceiverUpgradeable
{
    ...
    {
        // require(
        //     tokenSaleData[collectableId].nTokens >= 1 &&
        ...
        tokenSaleData[collectableId].nTokens -= 1;
        if (tokenSaleData[collectableId].nTokens <= 0) {
            delete sale[collectableId];
        }
    }
    ...
    function buyBatch(
        ...
        for (uint256 i = 0; i < collectableIds.length; i++) {
            // require(
            //     tokenSaleData[collectableIds[i]].nTokens >= 1 &&
        ...
        for (uint256 i = 0; i < collectableIds.length; i++) {
            tokenSaleData[collectableIds[i]].nTokens -= 1;
            if (tokenSaleData[collectableIds[i]].nTokens <= 0) {
