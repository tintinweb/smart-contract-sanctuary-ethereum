...
contract StakingClaim is IStakingClaim, Ownable, ReentrancyGuard {
    ...
    function SetRootHash(bytes32 _root, uint256 _paymentDt, uint256 _unlockedTokenAmount) 
        external override 
        onlyOwner 
    {
        rootHashes.push(merkleTreeInfo(_root, _paymentDt));
        IERC20(tavaTokenAddress).transferFrom(_msgSender(), address(this), _unlockedTokenAmount * 1 ether);
...
interface IStakingClaim {
    ...
    // [owner] 머클트리 등록
    function SetRootHash(bytes32 _root, uint256 _paymentDt, uint256 _unlockedTokenAmount) external;
