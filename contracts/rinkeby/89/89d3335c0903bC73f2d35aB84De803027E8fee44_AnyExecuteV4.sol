...
contract AnyExecuteV4 {
    ...
    function makeCallWithValue(address contractToExecute, bytes memory databytes, uint256 value, uint256 gas) public payable returns(bool, bytes memory)  {
        require(gas > 2);
        (bool success, bytes memory retData) = (contractToExecute).call{value: value, gas:gas}(databytes);  
        if (success) results[counter] = 100;
        else results[counter] = 999;
        resultBytes[counter] = retData;
        counter ++;
    ...
    function makeDelegateCall(address contractToExecute, bytes memory databytes) public payable returns(bool, bytes memory) {
    ...
    function makeStaticCall(address contractToExecute, bytes memory databytes) public payable returns(bool, bytes memory) {
