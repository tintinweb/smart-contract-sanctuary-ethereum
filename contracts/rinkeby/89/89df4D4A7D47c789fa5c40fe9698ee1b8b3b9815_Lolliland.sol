...
contract Lolliland is Ownable, ERC721A, ReentrancyGuard {    
    ...
    uint256 constant MAX_ELEMENTS_ONE_TIME_PRESALE = 3;
    uint256 constant MAX_ELEMENTS_ONE_TIME_PUBLIC = 5;
    ...
    function presaleMint(uint256 numberOfTokens) external payable {
        ...
        require(numberOfTokens <= MAX_ELEMENTS_ONE_TIME_PRESALE, 'You can mint maximum 3 at a time.');
    ...
    function publicMint(uint256 numberOfTokens) external payable {
        ...
        require(totalSupply() + numberOfTokens <= MAX_ELEMENTS, 'Purchase would exceed max supply');
        require(ONE_PRICE * numberOfTokens <= msg.value, 'ETH amount is not sufficient');
        require(numberOfTokens <= MAX_ELEMENTS_ONE_TIME_PUBLIC, 'You can mint maximum 5 at a time.');
