...
contract ZCRStakingContract is ReentrancyGuard, Ownable {
    ...
    uint256 public constant DURATION = 10 minutes; // 10 blocks
    uint256 private rewardsPerHour = 1369863014000000; // 1 month
   ...
   function start() external onlyOwner()
    ...
    function stake(uint256 _tokenId) external nonReentrant {
        ...
        require(stakers[msg.sender].amountStaked == 0, "Only 1 NFT per wallet!");
        require(started, "Contract paused!");
    ...
    function claimRewards() external {
        ...
        // require(rewards > address(this).balance, "No balance to send!");
        stakers[msg.sender].timeOfLastUpdate = block.timestamp;
    ...
    function getStakedTokensID(address _user) public view returns (uint256) {
        if (stakers[_user].amountStaked > 0) {
            uint256 _id = 0;
            for (uint256 j = 0; j < stakers[_user].stakedTokens.length; j++) {
                uint256 _tokenId = stakers[_user].stakedTokens[j].tokenId;
                address _staker = stakers[_user].stakedTokens[j].staker;

                if (_staker == _user && stakerAddress[_tokenId] == _user) {
                    _id = _tokenId;
                }
            }
            return _id;            
        ...
        returns (uint256 _rewards)
    {
        ...
        return (((((periodFinish - stakers[_staker].timeOfLastUpdate) * stakers[_staker].amountStaked)) * rewardsPerHour) / 3600);            
        }else{
