...
import { BondsRegistry } from "../BondsRegistry.sol";
import { Constants } from "../libraries/Constants.sol";

/// @title MockBonds Registry
/// @dev Mock contract that accepts lower period discounts
contract BondsRegistryMock is BondsRegistry {
    /// @dev Simply removes the lockDuration check
    function setDiscounts(DiscountOption[] calldata _discountOptions) external virtual override onlyOwner {
        for (uint256 i = 0; i < _discountOptions.length; i++) {
            require(
                _discountOptions[i].discount < Constants.DISCOUNT_BASE && _discountOptions[i].discount > 0,
    ...
    PausableUpgradeable
{
    ...
    function resign(uint256 _offerId, uint256 _value) external virtual whenNotPaused updateOffer(_offerId) {
        ...
        if (_value > currentUnderlyingSupplied) {
            valueToRemove = currentUnderlyingSupplied;
        } else {
            valueToRemove = _value;
        }
        // we assume _value is valid if offer.balance didn't underflow
        unchecked {
            underlyingSupplied = currentUnderlyingSupplied - valueToRemove;
        }
        vesting.afterOfferResignation(msg.sender, _offerId, valueToRemove);

        underlying.safeTransfer(address(vesting), valueToRemove);
        emit LogResign(msg.sender, _offerId, valueToRemove);
    }
