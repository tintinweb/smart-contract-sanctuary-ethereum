...
contract CryptoDefenders is ERC721A, Pausable, Ownable, IERC2981 {
    ...
    uint256 public constant changePrice = 0.01 ether;
    ...
    function _getRandomNumber() internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), block.difficulty, block.timestamp, msg.sender)));
    }
    ...
    function tryUpgrade(uint256 tokenId, Traits trait) payable external whenNotPaused {
        ...
        uint256 rndWeight = _getRandomNumber() % 100;
        uint256 newAttr;
    ...
    function getTokenInfo(uint256 tokenId) public view returns (TokenInfo memory info) {
        uint24 extraData = _ownershipAt(tokenId).extraData;
