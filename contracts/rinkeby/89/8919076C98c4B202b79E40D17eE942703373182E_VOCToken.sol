...
contract NODERewardManagement {
    ...
    uint256 public dailyDistributeTime;

    // real : 60 * 60 * 24, test : 60 * 3
    uint256 private DetaforDay = 60 * 30;
    // real : 60 * 60 * 24 * 365, test : 60 * 5 * 3
    uint256 private DetaforYear = 60 * 30 * 10;
    ...
    uint256[] private earlyBirdDayBonus = [
        7500 * (10**18),
        5000 * (10**18),
        2500 * (10**18)
    ];
    uint256[] private earlyBirdYearBonus = [
        270000 * (10**18),
        180000 * (10**18),
        90000 * (10**18)
    ];
    ...
    modifier onlyGenesis() {
        require(msg.sender == genesis || msg.sender == gateKeeper, "Not Genesis");
        _;
    }
    ...
    function _getProcessedTime() public view returns (uint256) {
        return (dailyDistributeTime - startTime);
    }
        ...
        onlyStarted
    {
        ...
        require(dailyDistributeTime + DetaforDay > cur_time, "Not Finish to distribute,Please wait some minutes");

        require(!createNodeFlag, "Creating NODE by other, Please try again after.");
    ...
    function removeNode(address account, uint256 _creationTime)
        external
        onlySentry
        onlyStarted
    {
        ...
        nodes[validIndex] = nodes[numberOfNodes - 1];
        nodes.pop();
        nodeOwners.set(account, nodes.length);
        totalNodesCreated--;
        removeNodePenaltyAmount++;
        if (autoDistri && !distribution && !distributionAnnual && totalNodesCreated > 0 ) {
    ...
    function removeNodeAll(address account, uint256 nodeCount)
        external
        onlySentry
        onlyStarted
    {
        ...
        removeNodePenaltyAmount += nodeCount;
        if (autoDistri && !distribution && !distributionAnnual && totalNodesCreated > 0) {
            distributeRewardsDaily();
            distributeRewards();
        }
    ...
    function distributeRewardsDaily() private {
        uint256 cur_time = block.timestamp;

        if (dailyDistributeTime + DetaforDay < cur_time) {
            distribution = true;
            uint256 numberOfnodeOwners = nodeOwners.keys.length;
            if (numberOfnodeOwners > 0) {
                uint256 totalRewardAmount = 0;
                uint256 mintTokenAmount = 0;
                if (lastIndexProcessedDaily == 0) {
                    (totalRewardAmount, mintTokenAmount) = calcRewardNode();
                    rewardNodeDaily = totalRewardAmount.div(totalNodesCreated);
                ...
                uint256 localLastIndex = lastIndexProcessedDaily;
                uint256 nodesCount;
                NodeEntity[] storage nodes;
                NodeEntity storage _node;

                while (
                    gasUsed < gasForDistribution 
                ) {
                    if (localLastIndex >= nodeOwners.keys.length) {
                        localLastIndex = 0;
                        dailyDistributeTime += DetaforDay;
                        ...
                        if (dailyDistributeTime + DetaforDay < cur_time) {
                            ...
                            rewardNodeDaily = totalRewardAmount.div(
                                totalNodesCreated
                            );
                            IVOCToken(token).mint(token, mintTokenAmount);
                        } else {
                    ...
                    for (uint256 i = 0; i < nodesCount; i++) {
                        _node = nodes[i];
                        _node.rewardAvailable += rewardNodeDaily;
                    }
                    newGasLeft = gasleft();
                ...
                lastIndexProcessedDaily = localLastIndex;
                distribution = false;
            }
        }
    }
    ...
    function distributeRewards() private {
        ...
        if (numberOfnodeOwners > 0) {
            ...
            if(totalAnnualBonus > 0) {
                IVOCToken(token).mint(token, totalAnnualBonus);
            }
    ...
    function calcRewardNode() private returns (uint256, uint256) {
        ...
        uint256 totalRewardAmount = increaseTokenAmount +
            removeNodePenaltyAmount.mul(removeNodeBonus);
        removeNodePenaltyAmount = 0;
        if (isEarlyBirdTime) {
            uint256 earlyBirdReward = calcEarlyBirdReward();
            totalRewardAmount += earlyBirdReward;
            increaseTokenAmount += earlyBirdReward;
        }
    ...
    function calcEarlyBirdReward() private returns (uint256) {
        ...
        if (deltaTime < (DetaforYear - DetaforDay)) {
            mintTokenAmount = earlyBirdDayBonus[0];
        } else if (deltaTime < DetaforYear) {
            mintTokenAmount = earlyBirdYearBonus[0];
        } else if (deltaTime < (2 * DetaforYear - DetaforDay)) {
            mintTokenAmount = earlyBirdDayBonus[1];
        } else if (deltaTime < (2 * DetaforYear)) {
            mintTokenAmount = earlyBirdYearBonus[1];
        } else if (deltaTime < (3 * DetaforYear - DetaforDay)) {
            mintTokenAmount = earlyBirdDayBonus[2];
        } else {
            mintTokenAmount = earlyBirdYearBonus[2];
    ...
    function _distributeRewards() external onlySentry onlyStarted {
        distributeRewardsDaily();
        distributeRewards();
    }
    ...
    function start() external onlyGenesis {
        ...
        dailyDistributeTime = block.timestamp;
        isStart = true;
    }
}
...
contract VOCToken is ERC20, Ownable {
  ...
  uint256 private removeNodePrice = 99 * (10**18);

  uint256 public fee = 0;
  uint256 private rewardAmountFee = 0;
  ...
  constructor(address uniV2Router, address _usdcAddres) ERC20("VOCYield.io", "VOC") {
    require(uniV2Router != address(0), "ROUTER CANNOT BE ZERO");
    require(_usdcAddres != address(0), "USDC CANNOT BE ZERO");
    ...
    address _uniswapV2Pair = IUniswapFactory(_uniswapV2Router.factory())
      .createPair(address(this), _usdcAddres);

    uniswapV2Router = _uniswapV2Router;
    uniswapV2Pair = _uniswapV2Pair;
    usdcAddress = _usdcAddres;
    feeDestination = address(0);
  ...
  modifier onlyMintable() {
    require(
      genesisAddress == _msgSender() ||
        _msgSender() == address(nodeRewardManager) ||
        owner() == _msgSender(),
  ...
  function setFee(uint256 _fee) external onlyOwner {
    require(_fee <= 200, "Fee is too big");
    fee = _fee;
  }

  function setDestination(address _destination) external onlyOwner {
    require(
      _destination != address(0) && _destination != feeDestination,
      "Destination CANNOT BE ZERO"
    );
    feeDestination = _destination;
  }

  function setGenesis(address _genesisAddress) external onlyOwner {
    require(_genesisAddress != address(0), "Genesis CANNOT BE ZERO");
  ...
  function setRemoveNodePrice(uint256 _price) external onlyOwner {
    removeNodePrice = _price;
  }
  ...
  function swapTokensForUSDC(address destination, uint256 tokenAmount) private {
  ...
  function cashoutReward(uint256 blocktime) public {
    ...
    if (fee > 0) {
      uint256 _rewardAmountFee = rewardAmount.mul(fee).div(10000);
      rewardAmountFee += _rewardAmountFee;
      rewardAmount = rewardAmount.sub(_rewardAmountFee);
    }
  ...
  function removeNodeAll() public {
    ...
    uint256 totalTokenAmount = removeNodePrice.mul(nodeCount);
    super._transfer(address(this), sender, totalTokenAmount);
  }
  ...
  function getNodesNamesOf(address account)
    public
    view
    returns (string memory)
  {
    require(account != address(0), "SENDER CAN'T BE ZERO");
    require(nodeRewardManager._isNodeOwner(account), "NO NODE OWNER");
    return nodeRewardManager._getNodesNames(account);
  }

  function getNodesCreatimeOf(address account)
    public
    view
    returns (string memory)
  {
    require(account != address(0), "SENDER CAN'T BE ZERO");
    require(nodeRewardManager._isNodeOwner(account), "NO NODE OWNER");
    return nodeRewardManager._getNodesCreationTime(account);
  }

  function getNodesRewardsOf(address account)
    public
    view
    returns (string memory)
  {
    require(account != address(0), "SENDER CAN'T BE ZERO");
    require(nodeRewardManager._isNodeOwner(account), "NO NODE OWNER");
    return nodeRewardManager._getNodesRewardAvailable(account);
  }

  function getRemoveNodePrice() public view returns (uint256) {
    return removeNodePrice;
  }
  ...
  function getProcessedTime() public view returns (uint256) {
    return nodeRewardManager._getProcessedTime();
  }
  ...
  function release() public onlyOwner {
    require(rewardAmountFee > 0, "don't have tokens");
    require(feeDestination != address(0), "don't have tokens");
    swapTokensForUSDC(feeDestination, rewardAmountFee);
    rewardAmountFee = 0;
  }
}