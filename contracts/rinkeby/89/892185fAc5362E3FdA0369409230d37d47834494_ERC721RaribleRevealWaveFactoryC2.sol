...
import "./ERC721RaribleMinimalRevealWave.sol";
import "./BeaconProxy.sol";
import "./Ownable.sol";

/**
 * @dev This contract is for creating proxy to access ERC721RaribleMinimalRevealWave token.
 *
 * The beacon should be initialized before call ERC721RaribleMinimalRevealWaveFactoryC2 constructor.
 *
 */
contract ERC721RaribleRevealWaveFactoryC2 is Ownable {
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, uint salt) external {
        ...
        ERC721RaribleMinimalRevealWave token = ERC721RaribleMinimalRevealWave(address(beaconProxy));
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI) view internal returns(bytes memory){
        return abi.encodeWithSelector(ERC721RaribleMinimalRevealWave(0).__ERC721Rarible_init.selector, _name, _symbol, baseURI, contractURI, transferProxy, lazyTransferProxy);
    }
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) view internal returns(bytes memory){
        return abi.encodeWithSelector(ERC721RaribleMinimalRevealWave(0).__ERC721RaribleUser_init.selector, _name, _symbol, baseURI, contractURI, operators, transferProxy, lazyTransferProxy);
    }
}