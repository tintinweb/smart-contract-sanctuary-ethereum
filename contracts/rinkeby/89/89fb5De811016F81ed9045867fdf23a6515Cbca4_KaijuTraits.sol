...
error KaijuTraits_InvalidSignature();
error KaijuTraits_NonceAlreadyUsed();
error KaijuTraits_SupplyLocked();

contract KaijuTraits is ERC1155, Ownable, AccessControl {
     ...
     */
    function lockSupply(uint256 id) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function kmartMint(
    ...
    ) public onlyRole(KMART_ROLE) {
        if (nonceUsed[nonce]) revert KaijuTraits_NonceAlreadyUsed();
        ...
        if (signer != ECDSA.recover(
            ...
            signature
        )) revert KaijuTraits_InvalidSignature();
    ...
    function kmartMintBatch(
        ...
        if (signer != ECDSA.recover(
            ECDSA.toEthSignedMessageHash(keccak256(
                abi.encodePacked(tx.origin, ids, amounts, nonce)
            )),
            signature
        )) revert KaijuTraits_InvalidSignature();
