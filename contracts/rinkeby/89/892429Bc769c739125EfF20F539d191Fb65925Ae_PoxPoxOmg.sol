...
contract PoxPoxOmg is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  bool public freeMintLimitEnabled = true;
  ...
  modifier freeMintCompliance(address _address, uint256 _mintAmount) {
    if(freeMintLimitEnabled){
      require(!freeMintClaimed[_msgSender()], 'Address already Minted!');
      require(balanceOf(_address) + _mintAmount <= freeMintLimitBalance, string.concat('Token amount exceeded! Only allowed to ', Strings.toString(freeMintLimitBalance), ' Tokens per wallet at this state.'));
    }      
    _;
  }
...
*/
  ...
  function setFreeMintLimitBalance(uint256 _freeMintLimitBalance) public onlyOwner {
    freeMintLimitBalance = _freeMintLimitBalance;
  }
  ...
  function setFreeMintLimitEnabled(bool _state) public onlyOwner {
    freeMintLimitEnabled = _state;
  }
