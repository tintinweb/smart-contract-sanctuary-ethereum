...
contract ShintaroToken is ERC1155Burnable, ERC1155Supply, Ownable {
  struct TokenInfo {
    ...
    bool isPublic;
    uint64 maxPerTxn;
    uint64[] counterIds; // if own counter id => can not mint this token
    uint64 editions; // MAX_SUPPLY
    uint64 minted;
    uint256 price;
    uint256 start;
    uint256 end;
  ...
  constructor() ERC1155("") {
    // counter tokens: 8, [9] and 9, [8] only
    // tokenid 8-9: start=1656021600 end=1656108000
    // tokenid 0-7: set mint window after draw results, date TBD
    init(0, type(uint64).max, new uint64[](0), 100, 0.1 ether, 0, 10000000000, "ipfs://QmXda3TupJdM5pUqKMchuDCJyXsBtsbpRcmFD139ZZrMcw"); // change start time, end time and uri
    init(1, type(uint64).max, new uint64[](0), 100, 0.1 ether, 0, 10000000000, "ipfs://QmXygnK49TfgjHYTZqR7QtnzXtuLfEK1YhjmtDza6j85iY"); // change start time, end time and uri
    init(2, type(uint64).max, new uint64[](0), 50, 0.2 ether, 0, 10000000000, "ipfs://QmPyk2b8FPmBKueyMJ6KWGaibB5mim9G17eHFwfcRyjB5c"); // change start time, end time and uri
    init(3, type(uint64).max, new uint64[](0), 50, 0.2 ether, 0, 10000000000, "ipfs://QmemAMyoewjbNxfKWvVVNFoHHhkkRdgics7yoDjuSUCGPy"); // change start time, end time and uri
    init(4, type(uint64).max, new uint64[](0), 25, 0.7 ether, 0, 10000000000, "ipfs://QmUa9fUCi7fgqQ712HTZb23FqdN3KmDL4a6McyjtpaLpLx"); // change start time, end time and uri
    init(5, type(uint64).max, new uint64[](0), 25, 0.7 ether, 0, 10000000000, "ipfs://QmQWWsNrmGMCpDgZ1MM4xYjgGZGXSD39bNywgwNyqb6wES"); // change start time, end time and uri
    init(6, type(uint64).max, new uint64[](0), 10, 1.25 ether, 0, 10000000000, "ipfs://QmZqbQDjDZTBEuTqDqRCWbtz2TmxRwFczhbTT5H796xRZ1"); // change start time, end time and uri
    init(7, type(uint64).max, new uint64[](0), 10, 1.25 ether, 0, 10000000000, "ipfs://QmWGf2RLimHmPpkKjU5hwiAWsu2E2VdM54sfvGBXARVP4q"); // change start time, end time and uri

    uint64[] memory counters = new uint64[](1);
    counters[0] = 9;
    init(8, type(uint64).max, counters, type(uint64).max, 0 ether, 0, 10000000000, "ipfs://QmZqbQDjDZTBEuTqDqRCWbtz2TmxRwFczhbTT5H796xRZ1"); // change start time, end time and uri

    counters[0] = 8;
    init(9, type(uint64).max, counters, type(uint64).max, 0 ether, 0, 10000000000, "ipfs://QmR714TzFBpfQgmMmJSsYY1VZ1hftG6wAj2QnoQ96TLKU3"); // change start time, end time and uri
  }
  ...
  function mint(
    ...
    require(tx.origin == msg.sender, "Not allowed"); // not allow contract
    require(token.start <= block.timestamp && block.timestamp <= token.end, "Sale not active"); // active time
    require(_amount <= token.maxPerTxn, "Exceed txn");
    require(token.isPublic || eligibleClaim(_tokenId, _maxAmount, msg.sender, _signature, _amount), "Not eligible"); // require public or whitelist
    require(token.minted + _amount <= token.editions, "Exceed max"); // require not sold out
    require(msg.value >= token.price * _amount, "Ether value incorrect"); // require correct price
  ...
  // team claim: 15 amount of tokenId 0, 1 (whitelist 85 only from these selections)
  function teamClaim(
