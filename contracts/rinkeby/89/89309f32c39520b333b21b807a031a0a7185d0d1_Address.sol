...
contract RUGGED is ERC721Optimized, Ownable, ReentrancyGuard {
    using Strings for uint256;

  uint256 public constant MAX_Supply = 3600;
    uint256 public maxRUGPurchase = 10;
    uint256 public maxFreePerWallet = 2;
    uint256 public totalFree = 600;
    uint256 public RUGPrice = 0.021 ether;
    string public _baseRUGURI;
    
    event RUGMinted(address indexed mintAddress, uint256 indexed tokenId);
    event PermanentURI(string _value, uint256 indexed _id);

    constructor(string memory baseURI) ERC721Optimized("RUGGED", "RUGGED") {
        _baseRUGURI = baseURI;
    }
    ...
    function setMintPrice(uint256 newPrice) public onlyOwner {
        require(newPrice >= 0, "RUG price must be greater than zero");
       RUGPrice = newPrice;
    }

    function mintRUG(uint256 numberOfTokens) public payable nonReentrant {
        require((RUGPrice * numberOfTokens) <= msg.value, "Ether value sent is not correct");
        require(numberOfTokens <= maxRUGPurchase, "You can mint max 10 RUGGED per transaction");
        require((totalSupply() + numberOfTokens) <= MAX_Supply, "Purchase would exceed max supply of MBAIs");
    ...
    function createCollectible(address mintAddress) private {
        ...
        if (mintIndex < MAX_Supply) {
            ...
            emit RUGMinted(mintAddress, mintIndex);
        }
    }
