...
contract PayBNftPool is IPayBNftPool, Ownable, ReentrancyGuard{
    IPayBNftMarketplace public PayBMKT;
    using Address for address;
    using SafeMath for uint256;

    mapping(address => mapping(address => uint256)) amountWithdrawn;

    constructor(address _PayBNftMarketplace, address _owner) {
        PayBMKT = IPayBNftMarketplace(_PayBNftMarketplace);
    ...
    function getAmountEarn(address _user, address _tokenAddress) public view override returns(uint256){
        return PayBMKT.getAmountEarn(_user, _tokenAddress);
    }
    ...
    function withdraw(address _tokenAddress) external override nonReentrant {
        uint256 subOwnerFee = PayBMKT.getAmountEarn(msg.sender, _tokenAddress);
        if(_tokenAddress == address(0)){
            ...
            PayBMKT.setDefaultAmountEarn(msg.sender, _tokenAddress);
        }else{
            require(subOwnerFee > 0, "Earn = 0");
    ...
    function setMarketplaceAddress(address marketPlaceAddress) external override onlyOwner() {
        PayBMKT = IPayBNftMarketplace(marketPlaceAddress);
    }
