...
contract Habit is ERC721 {
    ...
    // function tokenURI(uint256 habitId) external view returns (string memory) {
    //     HabitData memory habit = habits[habitId];
    ...
    // function _generateSVGImage(HabitData memory habit)
    //     private
    //     pure
    ...
    //             abi.encodePacked(
    //                 fixedPointToDecimalString(
    //                     _params.swapsExecuted > 0
    //                         ? _params.swapped / _params.swapsExecuted
    //                         : 0,
    //                     _params.toDecimals
    //                 ),
    //                 " ",
    ...
    {
        if (_habitTimeframe == 1 days) return "Every day";
        if (_habitTimeframe == 1 weeks) return "Every week";
        if (_habitTimeframe == 30 days) return "Every month";
        if (_habitTimeframe == 365 days) return "Every year";
    ...
    function commit(
        ...
        timeframeToDescription(timeframe);
