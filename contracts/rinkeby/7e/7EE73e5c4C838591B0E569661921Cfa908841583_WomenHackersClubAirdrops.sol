...
contract WomenHackersClubAirdrops is Ownable, ONFT721 {
    ...
    constructor(address _layerZeroEndpoint, uint startMintId, uint endMintId) ONFT721("Women Hackers Club Airdrops", "WHC Airdrops", _layerZeroEndpoint) {
        nextMintId = Counters.Counter(startMintId);
        maxMintId = endMintId;
    }


   function ownerMint(address addr, uint256 mintCount) external onlyOwner {
        mint(addr,mintCount);
    }

    function idMint(address addr, uint256 id) external onlyOwner {
        _safeMint(addr,id);
    }
