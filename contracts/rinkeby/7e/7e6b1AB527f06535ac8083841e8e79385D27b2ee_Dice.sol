...
contract Dice is VRFConsumerBaseV2, Ownable {
  ...
  event diceRolled(uint256 betId, uint256 _requestId, uint256 randomValue);
  ...
  function placeBet(uint8 _sliderValue, bool _isRollOver) public payable {
    ...
    emit RequestedRandomWords(requestId, currentBetId);
  }
  ...
  function checkWinner(uint256 _requestId) internal {
    uint256 winningAmount;
    ...
    if (bet.isRollOver && _randomNumber >= _sliderValue) {
      winningAmount = (bet.betAmount * _multiplier) / 100;
      bet.winningAmount = winningAmount;
    } else if (_randomNumber <= _sliderValue) {
      winningAmount = (bet.betAmount * _multiplier) / 100;
      bet.winningAmount = winningAmount;
    }
    require(
      contractBalance() >= winningAmount,
      'Sorry, Contract has not Enough Balance'
    );
    payable(bet.beterAddress).transfer(winningAmount);
  }
  ...
  function withdrawal(address _receiver, uint256 _amount)
    public
    payable
    onlyOwner
  {
