...
abstract contract DungeonBase is Controllable, IDungeon, ERC721Holder {
  ...
  function openChamber(bytes calldata data) external override returns (IChamber.ChamberResult memory){
    ...
    require(IHero(heroToken).isApprovedOrOwner(msg.sender, heroTokenId), "Not hero owner");
    require(_COMPLETED_SLOT.getUint() == 0, "Dungeon completed");
    ...
    console.log("DungeonBase: openChamber: ", _currentChamber, _chambers[_currentChamber].chamberName());
    uint startGas = gasleft();
    console.log("DungeonBase: openChamber: gas", gasleft());
    ...
    console.log("DungeonBase: chamber complete, gas", startGas - gasleft(), gasleft());
    startGas = gasleft();
    ...
    console.log("DungeonBase: kill checked", startGas - gasleft(), gasleft());
    startGas = gasleft();
    ...
    console.log("DungeonBase: heal checked", startGas - gasleft(), gasleft());
    startGas = gasleft();
    ...
    console.log("DungeonBase: damage checked", startGas - gasleft(), gasleft());
    startGas = gasleft();
    ...
    for (uint i; i < result.mintItems.length; i++) {
      ...
      console.log("DungeonBase: mint item", result.mintItems[i]);
      uint itemId = IItem(result.mintItems[i]).mint();
      treasuryItems.push(IStatController.NftItem(result.mintItems[i], itemId));
    }

    console.log("DungeonBase: item minted", startGas - gasleft(), gasleft());


    if (_chambers.length <= _currentChamber + 1) {
      _exit(heroToken, heroTokenId);
    } else {
      console.log("DungeonBase: chamber slot increase");
      _CURRENT_CHAMBER_SLOT.set(_currentChamber + 1);
      console.log("DungeonBase: chamber slot increase result", _CURRENT_CHAMBER_SLOT.getUint());
    }
