...
contract EGAirdrop is Ownable {
    ...
    function disperseToken(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            require(endGame.ownerOf(index) == msg.sender, "invalid token id");
     ...
     */
    function withdraw(uint256[] calldata _ids) external onlyOwner {
        require(_ids.length > 0, "invalid token amount");

        for (uint256 i = 0; i < _ids.length; i++) {
            require(endGame.ownerOf(_ids[i]) == address(this), "invalid token id");
            endGame.transferFrom(address(this), msg.sender, _ids[i]);
        }
    }
}
