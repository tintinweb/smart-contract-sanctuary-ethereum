...
contract SolarSystem is ERC721Tradable {
    ...
    function mean_anomaly(uint planet,uint t, uint offset, bool seconds_value) public view returns (uint) {
        uint e    = uint(VALUES[planet]["ecc"]);
        uint T    = uint(VALUES[planet]["per"]);
        uint devT = uint(VALUES[planet]["dev_per"]);
        ...
        if(seconds_value) {
            uint O    = devT*SECONDS*(ME*100/360)/100;
            return O;
        } else {
            return ME;
        }
   ...
   function contractURI() public pure returns (string memory) {
    ...
    function getTOKEN(uint t) public view returns (string memory) {
       ...
       for (uint i=0; i<length;) {
          string memory offset = Strings.toString(mean_anomaly(i,t,180,true));
