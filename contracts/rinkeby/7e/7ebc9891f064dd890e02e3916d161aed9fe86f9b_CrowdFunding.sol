...
contract CrowdFunding {
    ...
    */
    ...
    event funderData(address funder, uint256 amount);
    function checkFunder(uint256 campaignID, uint256 funderID) public returns (address funder, uint256 amount) {
        Campaign storage c = campaigns[campaignID];
        funder = c.funders[funderID].addr;
        amount = c.funders[funderID].amount;
        emit funderData(funder, amount);
    }
}