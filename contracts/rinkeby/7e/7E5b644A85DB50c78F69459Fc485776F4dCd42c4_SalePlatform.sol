...
import "./interfaces/IQuantumKeyUnlocks.sol";
...
contract SalePlatform is ContinuousDutchAuction, ReentrancyGuard, Auth {
    ...
    IQuantumKeyUnlocks public keyUnlocks;
    ...
    constructor(
        ...
        keyUnlocks = IQuantumKeyUnlocks(deployedUnlocks);
    ...
    function unlockWithKey(uint256 keyId, uint128 dropId, uint256 variant) nonReentrant checkCaller payable public {
        ...
        if (sale.enabledKeyRanges.length > 0) {
            if ((keyId >= (dropId * SEPARATOR)) && (keyId < (((dropId+1) * SEPARATOR)-1))) inRange = true;
        } 
        else inRange = true;
    ...
    function createUnlockSale(uint128 price, uint64 start, uint64 period, address artist, uint256[] calldata enabledKeyRanges) requiresAuth public {
        ...
        keySales[_nextUnlockDropId++] = UnlockSale(price, start, period, artist, blankRanges);
...
interface IQuantumKeyUnlocks {
    function mint(address to, uint256 dropId, uint256 variant) external returns (uint256);
}
