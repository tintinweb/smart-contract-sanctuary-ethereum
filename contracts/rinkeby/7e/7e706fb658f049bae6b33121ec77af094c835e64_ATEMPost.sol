...
contract ATEMPost is ERC1155Pausable, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        address initialPlatformFeeAccount) ERC1155(_uri) {
            name = _name;
            symbol = _symbol;
    ...
    {  
        ...
        if (_supply[id] == 0) {
            ...
            require(
                _verify(
                    keccak256(abi.encodePacked(id, contentURI_)), 
                    signature_,
                    _platformAuthorizeAccount),
                'ATEMPost#mint: invalid signature of platform authorizer'
            );
    ...
    ) public nonReentrant() onlyCreatorBatch(ids)  
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (_supply[ids[i]] == 0) {
                ...
                require(
                    _verify(
                        keccak256(abi.encodePacked(ids[i], contentURIs[i])), 
                        signatures[i],
                        _platformAuthorizeAccount),
                    'ATEMPost#mintBatch: invalid signature of platform authorizer'
                );
    ...
    ) public payable nonReentrant()
    {
        ...
        address creator_ = creator(id);
        _withdraw(creator_, totalPriceDeductPlatformFee);
    ...
    ) public payable nonReentrant()
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            totalPriceSingle = _price[ids[i]] * amounts[i];
            ...
            _withdraw(creator(ids[i]), totalPriceSingle - platformFeeSingle);
