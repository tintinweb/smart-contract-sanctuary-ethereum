...
contract VinciStakingV1 is Ownable {
    ...
    mapping(uint => uint) totalStakedPerTier;
    ...
    */
    ...
    Requirements:
        user scheduleUnstaking balance higher thant _amount
        _poolId exists (non zero address as owner)
    ...
    */
    function cancelScheduledUnstaking(
        ...
        updateUserState(_msgSender());
        require(_amount <= pools[_poolId].scheduledUnstaking[_msgSender()], 'not enough scheduled for unstake');
    ...
    */
    function moveFunds(
        ...
        pools[_originPoolId].movementLock[_msgSender()] = block.timestamp + epochDuration;
    ...
    */
    ...
    /**
    @notice counts the number of epochs since the genesis timestamp
    */
    function epocsSinceGenesis() public view returns (uint) {
        return uint(block.timestamp - genesisSundayTimestamp) / uint(epochDuration);
    }
    ...
    */
    ...
    /**
    @notice estimates the potential tier of a user if he would relock() right now
    */
    function potentialTier(address _user) external view returns (uint) {
