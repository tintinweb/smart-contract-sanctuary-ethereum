...
contract Contract is ERC721A, ReentrancyGuard, Ownable {
    ...
    function mint(uint16 amount) external nonReentrant {
        require(_state == State.Open, "claim has not started yet");
        require(totalSupply() < 10000, "claim has reached max supply");
        require(msg.sender == tx.origin, "contracts cant mint");
        require(!claimed[msg.sender], "already minted");
        require(amount < 10, "max minting is 10 tokens");
