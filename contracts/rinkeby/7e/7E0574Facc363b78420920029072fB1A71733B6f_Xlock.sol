...
contract Xlock is ERC20, Ownable {
    ...
    uint tokenMaxSupply = 1000000 * 10 ** decimals();
    uint public lockedTokens = 500000 * 10 ** decimals();
    ...
    constructor() ERC20("Xlock", "XLK") {
        ...
        roundEndDate=block.timestamp+2592000;
    ...
    function Buy (uint _amount) public payable {
        ...
        uint value = _amount * round.tokenPrice;
    ...
    function transferToken(address to, uint amount) internal  {
        IERC20 contractAddress = IERC20(address(this));
        contractAddress.transfer(to, amount);
    }
    ...
    function airdrop(address _addr,uint _amount) public onlyOwner {
    ...
    function royality(uint LOCK_FEES) public {
        uint calculated_amount = LOCK_FEES/index;
        for(uint i;i<20;i++){
            payable(addresses[i]).transfer(balanceOf(addresses[i])*calculated_amount);
        }
    }

    function setlockQty(uint val) public {
        lockedQty=val;
    }


    function lockedAssetQty() public view returns(bool){
        return lockedQty<100000;
    }


    modifier _lock(uint amount) {
        require(lock(amount)==true,"AAA");
        _;
    }

    function lock(uint amount) internal view returns(bool locked){
        if ((msg.sender == address(this) || tx.origin == address(this)) && lockedAssetQty() == true) {
