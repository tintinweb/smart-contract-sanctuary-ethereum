...
contract RewardStakingV3 is Initializable, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    using SafeMath for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    string public name;
    IPurseTokenV3 public purseToken;
    uint256 private _totalReceiptSupply;
       
    struct UserInfo {
        uint256 receiptToken;   
        uint256 stakedToken; //Unused
        uint256 withdrawnToken; //Unused
    }

    mapping (address => UserInfo) public userInfo;

    function initialize(IPurseTokenV3 _purseToken) public initializer {
    ...
    function getPurseAmount(uint256 receiptAmount) external view returns (uint256){  
        uint256 totalXPurse = totalReceiptSupply();
        uint256 purseAmount = receiptAmount.mul(purseToken.balanceOf(address(this))).div(totalXPurse);
        return purseAmount;
    }  
