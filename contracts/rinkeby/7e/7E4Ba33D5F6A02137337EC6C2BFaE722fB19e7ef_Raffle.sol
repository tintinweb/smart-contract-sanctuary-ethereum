...
error Raffle__NotEnoughEtherToEnter();
...
contract Raffle is VRFConsumerBaseV2, KeeperCompatibleInterface {
	...
	function enterRaffle() public payable {
		if (msg.value < i_entranceFee) {
			revert Raffle__NotEnoughEtherToEnter();
		}
	...
	/**
	 * @dev This function is the one that chainlink Keeper nodes call
	...
	function checkUpkeep(
		...
		upkeepNeeded = (isRaffleOpen && isTimePassed && hasPlayers && hasBalance);
	}

	function performUpkeep(bytes calldata) external override {
		// Request the random number then use it
