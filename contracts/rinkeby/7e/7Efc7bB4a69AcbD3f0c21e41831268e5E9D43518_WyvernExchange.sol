...
contract ExchangeCore is ReentrancyGuarded, Ownable {
        ...
        returns (bytes32)
    {
        ...
        require(validateOrder(hash, order, sig), "Failed at requireValidOrder");
        return hash;
    }
        ...
        reentrancyGuard
    {
        ...
        if (sell.maker == msg.sender) {
            require(validateOrderParameters(sell), "Failed at atomicMatch.validateOrderParameters(sell)");
        } else {
            sellHash = requireValidOrder(sell, sellSig);
        }
        
        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), "Failed at atomicMatch.ordersCanMatch");
        ...
        require(ArrayUtils.arrayEq(buy.calldata, sell.calldata), "Failed at atomicMatch.ArrayUtils.arrayEq");
        ...
        require(proxy.proxy(sell.target, sell.howToCall, sell.calldata), "Failed at atomicMatch.proxy.proxy");

        /* Static calls are intentionally done after the effectful call so they can check resulting state. */

        /* Handle buy-side static call if specified. */
        if (buy.staticTarget != address(0)) {
            require(staticCall(buy.staticTarget, sell.calldata, buy.staticExtradata), "Failed at atomicMatch.staticCall(buy.staticTarget...)");
        }

        /* Handle sell-side static call if specified. */
        if (sell.staticTarget != address(0)) {
            require(staticCall(sell.staticTarget, sell.calldata, sell.staticExtradata), "Failed at atomicMatch.staticCall(sell.staticTarget...)");
        }
