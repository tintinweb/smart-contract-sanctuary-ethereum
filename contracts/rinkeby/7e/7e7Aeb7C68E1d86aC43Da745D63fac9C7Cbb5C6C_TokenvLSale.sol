...
contract TokenvLSale {
  ...
  function buy(uint tokens) public payable {
    require(msg.value/phase(currentPhaseIndex).price == tokens, 'Error value does not match');
  ...
  function __unAmmount(uint _amount, uint decimals) private pure returns(uint){
    return _amount/(10**decimals);
  }

  function __tokens() public view returns(uint){
    return __unAmmount(token.balanceOf(msg.sender), 18);
  }
  ...
  function totalTokens() public view returns(uint) {
    return __unAmmount(token.totalSupply(), 18);
  }
