...
import "./HumblGalleryUpgradeable.sol";
import "./IHumbleNFTERC721Upgradeable.sol";


contract AuctionUpgradeable is OwnableUpgradeable {
    HumblGalleryUpgradeable public humbleGallery;
    using ECDSAUpgradeable for bytes32;
    address public admin;
    function initialize(HumblGalleryUpgradeable _humbleGallery, address _admin) initializer public {
        __Ownable_init();
        setERC1155(_humbleGallery);
        admin = _admin;
    }
    function setERC1155(HumblGalleryUpgradeable _humbleGallery) public onlyOwner {
        humbleGallery = _humbleGallery;
    ...
    function getToken(uint8 index) public view returns(address)  {
        return humbleGallery.getPaymentToken(index);
    }
    ...
    function hashOfAuction(AuctionPayload memory auction) public pure returns(bytes32) {
    ...
    function bid(AuctionPayload memory _bid) public pure returns(bytes32) {
        return hashOfAuction(_bid);
    }
    function addOnAuction(AuctionPayload memory auction) public pure returns(bytes32){
        return hashOfAuction(auction);
    }
    function validateSignature(address seller, bytes32 hash, Sig memory sig) public pure returns(bool){
        bytes32 _hash = hash.toEthSignedMessageHash();
        return _hash.recover(sig.v, sig.r, sig.s) == seller;
    }
    /* Accept bid by seller */
    function safeTransferNFT(AuctionPayload memory sellerPayload,AuctionPayload memory buyerPayload, Sig memory sellerSig, Sig memory buyerSig, uint256 tokenId) public payable {
        bytes32 sellerHash = hashOfAuction(sellerPayload);
        bytes32 buyerHash = hashOfAuction(buyerPayload);
        require(validateSignature(sellerPayload.seller, sellerHash, sellerSig), "H154");
        require(validateSignature(buyerPayload.bidder, buyerHash, buyerSig), "H155");
        require(_msgSender() == sellerPayload.seller || admin == _msgSender(),"H156");
        require(buyerPayload.auctionId==sellerPayload.auctionId,"H157");
        require(buyerPayload.paymentToken==sellerPayload.paymentToken,"H158");
        if(buyerPayload.tokenId==0){
            buyerPayload.tokenId=tokenId;
        }
        address _token = humbleGallery.getPaymentToken(sellerPayload.paymentToken);
        humbleGallery.transferNFT(sellerPayload.seller, buyerPayload.bidder,buyerPayload.tokenId,buyerPayload.bidPrice,1,_token);
    }
    function safeTransferNFTERC721(IHumbleNFTERC721Upgradeable ERC721humbleNFT, AuctionPayload memory sellerPayload,AuctionPayload memory buyerPayload, Sig memory sellerSig, Sig memory buyerSig, uint256 tokenId) public payable {
        bytes32 sellerHash = hashOfAuction(sellerPayload);
        bytes32 buyerHash = hashOfAuction(buyerPayload);
        require(validateSignature(sellerPayload.seller, sellerHash, sellerSig), "H154");
        require(validateSignature(buyerPayload.bidder, buyerHash, buyerSig), "H155");
        ...
        address _token = humbleGallery.getPaymentToken(sellerPayload.paymentToken);
        humbleGallery.safeBuyNFTERC721(ERC721humbleNFT, sellerPayload.seller, buyerPayload.bidder,buyerPayload.tokenId,buyerPayload.bidPrice,_token);
    }
 ...
 */
interface IHumbleNFTERC721Upgradeable  {
    /**
...
// ERC1155RoyaltyUpgradeable

contract HumblGalleryUpgradeable is Initializable, ERC1155Upgradeable, OwnableUpgradeable, PausableUpgradeable,ERC1155SupplyUpgradeable,CommissionUpgradeable, ReentrancyGuardUpgradeable {
   
/* Execute once on contract deployment with commission fee, commission fee receiver, royaltyFraction and royalty receiver */
function initialize(uint96 commissionPercentage, address commissionReceiver) initializer public {
        __ERC1155_init("");
        __Ownable_init();
        __Pausable_init();
        ...
        require(commissionPercentage<=10000, "H102");
    ...
    CountersUpgradeable.Counter private NFTId;
    ...
    function setProxy(address proxy,bool value) public virtual onlyOwner {
        proxies[proxy] = value;
    }
    ...
    function mint(uint256[6] memory intData, string calldata tokenURI, uint8 ERC20TokenIndex,address seller, address[] memory royaltyReceiver,uint96[] memory royaltyPercentage) public whenNotPaused returns(uint256) {
        require(royaltyPercentage.length == royaltyReceiver.length, "H105");
        return _mint(Payload(intData[0],intData[1],intData[2],intData[3],intData[4],intData[5],tokenURI,ERC20TokenIndex,seller,royaltyReceiver,royaltyPercentage));
    }
    
    function lazyMint(uint256[6] memory intData, string calldata tokenURI, uint8 ERC20TokenIndex,address seller,address[] memory royaltyReceiver,uint96[] memory royaltyPercentage, uint256 quantity, Signature memory sig) public payable whenNotPaused returns(uint256 tokenId)  {
        Payload memory payload = Payload(intData[0],intData[1],intData[2],intData[3],intData[4],intData[5],tokenURI,ERC20TokenIndex,seller,royaltyReceiver,royaltyPercentage);
        bytes32 hash = _Hash(payload);
        require(!executed[hash],"H106");
    ...
    function transferWithMint(uint256[6] memory intData, string calldata tokenURI, uint8 ERC20TokenIndex,address seller,address[] memory royaltyReceiver,uint96[] memory royaltyPercentage, uint256 quantity,address to) public whenNotPaused returns(uint256 tokenId)  {
        Payload memory payload = Payload(intData[0],intData[1],intData[2],intData[3],intData[4],intData[5],tokenURI,ERC20TokenIndex,seller,royaltyReceiver,royaltyPercentage);
        payload.tokenId = _mint(payload);
    ...
    function safeBuyNFT(Payload memory payload, Signature memory sig, uint256 quantity,uint256 tokenId) public payable whenNotPaused nonReentrant {
        bytes32 hash = _Hash(payload);
        address buyer = _msgSender();
        ...
        if(!executed[hash]){
            addOnSale(payload.tokenId,payload.seller,payload.unitPrice,payload.saleQuantity);
            executed[hash] = true;
        }
        _safeTransferNFT(payload , buyer, quantity);
    }
    function addOnSale(uint256 tokenId,address account, uint256 unitPrice, uint256 saleQuantity) public whenNotPaused {   
    ...
    function updateTokenPrice(address account, uint256 tokenId, uint256 unitPrice) public whenNotPaused {   
        require(account == _msgSender(), "H112");
        require(_exists(tokenId), "H113");
        require(unitPrice >=1, "H114");
        require(balanceOf(account, tokenId)>0, "H115");
        HumblNFT storage nft = HumblNFTs[tokenId];
        nft.salePrice[account] = unitPrice;
    }
    function validateSignature(address seller, bytes32 hash, Signature memory sig) public pure returns(bool){
        bytes32 _hash = hash.toEthSignedMessageHash();
        return _hash.recover(sig.v, sig.r, sig.s) == seller;
    }
    function _Hash(Payload memory payload) public pure returns(bytes32) {
    ...
    function _mint(Payload memory payload) internal returns(uint256 tokenId) { 
        ...
        require(payload.buyLimit <= payload.saleQuantity , "H119");
        _mint(payload.seller, tokenId, payload.quantitities, "");
        if (bytes(payload.tokenURI).length > 0){
           emit URI(payload.tokenURI, tokenId);
        }
        HumblNFT storage newNFT = HumblNFTs[tokenId];
    ...
    function setCommission(uint256 toeknId, uint96 commissionPercentage) public virtual onlyOwner  {
        require(_exists(toeknId), "H120");
        require(commissionPercentage>=0, "H121");
        require(commissionPercentage<=10000, "H122");
        HumblNFT storage nft = HumblNFTs[toeknId];
    ...
    function setDefaultCommission (uint96 commissionPercentage) public virtual onlyOwner {
        ...
        require(commissionPercentage<=10000, "H124");
        _setDefaultCommission(commissionPercentage);
    }
    ...
    function addOnSupply(address account, uint256 tokenId, uint256 quantity) public tokenMinterOnly(tokenId) {   
    ...
    function burnSupply(address account, uint256 tokenId, uint256 quantity) public virtual tokenMinterOnly(tokenId) {   
    ...
    function getQunatityFromSale(uint256 tokenId, address owner) public view returns(uint256) {
        require(owner==_msgSender(), "H130");
        require(_exists(tokenId), "H131");
        HumblNFT storage nft = HumblNFTs[tokenId];
        return nft.onSale[owner];
    }
    /* External contracts */
    
    /* Execute fund transfer ERC721 - royalty, commission and NFT price */
    function safeBuyNFTERC721(IHumbleNFTERC721Upgradeable ERC721humbleNFT, Payload memory payload, Signature memory sig) public payable whenNotPaused {
        bytes32 hash = _Hash(payload);
        require(!executed[hash],"H132");
        require(validateSignature(payload.seller, hash, sig), "H133");
        address ERC20Token = paymentTokens[payload.ERC20TokenIndex];
        _executeTransferERC721(ERC721humbleNFT,payload.seller,_msgSender(), payload.tokenId, payload.unitPrice,ERC20Token);
        executed[hash]=true;
    }
    function safeBuyNFTERC721(IHumbleNFTERC721Upgradeable ERC721humbleNFT, address seller, address buyer, uint256 tokenId,uint256 unitPrice, address ERC20Token) public payable whenNotPaused {
        require(proxies[_msgSender()], "H149");
        _executeTransferERC721(ERC721humbleNFT,seller,buyer,tokenId,unitPrice,ERC20Token);
    }
    function _executeTransferERC721(IHumbleNFTERC721Upgradeable ERC721humbleNFT, address seller, address buyer, uint256 tokenId,uint256 unitPrice, address ERC20Token) internal {
        require(seller != address(0), "H150");
        require(ERC721humbleNFT.balanceOf(seller) >= 1, "H151");
        if(ERC20Token == address(0)){
            require(msg.value >= unitPrice, "H152");
        }else{
            require(IERC20Upgradeable(ERC20Token).balanceOf(buyer) >= unitPrice, "H153");
        }
        ...
        if(royaltyReceiver[0]!=seller){
            royaltyAmount[0] = (unitPrice * royaltyPercentage) / 100;
        }else{
            royaltyAmount[0] = 0;
        ...
        uint256 commissionAmount;
        if(commission > 0){
            (commissionReceiver,commissionAmount) = _getCommission(commission, unitPrice);
        }
        NFT memory _data = NFT(tokenId, seller, buyer, 1, 
        unitPrice, commissionReceiver, commissionAmount, royaltyReceiver, royaltyAmount, ERC20Token);
        _executeFundTransfer(_data);
        ERC721humbleNFT.safeTransferFrom(seller, buyer, tokenId, '');
    }
    
    function _safeTransferNFT(Payload memory payload, address to, uint256 quantity) internal  {
        ...
        _executeTransferNFT(payload.seller, to, payload.tokenId, payload.unitPrice, quantity, ERC20Token);
        nft.onSale[payload.seller] = (nft.onSale[payload.seller]).sub(quantity);
    }
   
    function transferNFT(address seller, address buyer, uint256 tokenId,uint256 unitPrice, uint256 quantity, address ERC20Token) public payable {
        require(proxies[_msgSender()], "H142");
        _executeTransferNFT(seller, buyer, tokenId, unitPrice, quantity, ERC20Token);
    }
     
    function _executeTransferNFT(address seller, address buyer, uint256 tokenId,uint256 unitPrice, uint256 quantity, address ERC20Token) internal {
        ...
        if(nft.isCommissionChanged){
            ...
            (commissionReceiver, commissionAmount) = _getCommission(_getDefaultCommission(), saleAmount);
        }
        NFT memory _data = NFT(tokenId, seller, buyer, quantity, 
        saleAmount, commissionReceiver, commissionAmount, nft.royaltyReceiver, royaltyAmount, ERC20Token);
        _executeFundTransfer(_data);
    ...
    function _executeFundTransfer(NFT memory _data) internal {
        ...
        if(_data.commissionAmount > 0){ 
            if(_data.ERC20Token == address(0)){
                ...
                _transferWithTokens(_data.ERC20Token, _data.buyer, _data.commissionReceiver, _data.commissionAmount);
            }
        }
        ...
        if(_data.royaltyAmount.length > 0){ 
            if(_data.ERC20Token == address(0)){
                ...
                for (uint256 i = 0; i < _data.royaltyAmount.length; i++) {
                     totalRoyaltyAmount = totalRoyaltyAmount.add(_data.royaltyAmount[i]);
                    _transferWithTokens(_data.ERC20Token, _data.buyer, _data.royaltyReceiver[i], _data.royaltyAmount[i]);
                    emit royaltyEvent(_data.royaltyReceiver[i], _data.royaltyAmount[i]);
                }
            }   
        }
        uint256 _transferAbleAmount = (_data.saleAmount).sub((totalRoyaltyAmount).add(_data.commissionAmount),'H148');
        if(_data.ERC20Token == address(0)){
        ...
        emit safeSaleNFT(_data.seller, _data.buyer , _data.tokenId, _data.quantity, _transferAbleAmount, _data.commissionAmount, _data.commissionReceiver);
    ...
    {
        if (price > 0) {
            require(IERC20Upgradeable(ERC20Token).transferFrom(from, to, price));
        }
    }
