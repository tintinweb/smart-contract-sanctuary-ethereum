...
contract Vault {
    ...
    function decreasePositionLeverageConstant(
        ...
        uint256 _collateralDelta = _sizeDelta.mul(DEFAULT_DECIMALS).div(position.leverage);
    ...
    // Decrease the position, but keep the collateral constant; i.e decrease the leverage
    function decreasePositionCollateralConstant(
        uint256 _sizeDelta,
        uint256 commodityPrice // in USD
        ...
        uint256 usdOut = _reduceCollateral(msg.sender, 0, _sizeDelta, commodityPrice);

        if (position.size != _sizeDelta) {
            position.size = position.size.sub(_sizeDelta);
            position.leverage = position.size.mul(DEFAULT_DECIMALS).div(position.collateral);
            _validate(position.collateral == originalCollateral, 3);
        } else {
            delete positions[key];
        }
    ...
    function liquidatePosition(
        ...
        (bool hasProfit, uint256 delta) = getDelta(commodityPrice, position.size, position.averagePrice);

        if (!hasProfit && position.collateral < delta) {
            // TODO should update the pool, reserve etc
            delete positions[key];
            return;
        }
        ...
        emit Liquidation(remainingCollateral.mul(MAX_LEVERAGE), position.size);
        _validate(remainingCollateral.mul(MAX_LEVERAGE) < position.size, 5);

        // Decrease by the entire size
        decreasePositionCollateralConstant(position.size, commodityPrice);
    }
    ...
    function getPositionKey(address _account) public pure returns (bytes32) {
        return keccak256(abi.encode(_account));
    }
