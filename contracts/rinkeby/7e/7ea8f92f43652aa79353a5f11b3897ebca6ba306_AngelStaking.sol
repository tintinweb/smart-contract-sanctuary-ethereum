...
/** 
 *  SourceUnit: f:\projects\USA\blockchain\angel_nft_contract\contracts\AngelStaking.sol
*/
...
interface IAngel is IERC721Enumerable {
...
contract Angel is IAngel, ERC721Enumerable, Ownable {
  using Strings for uint256;
...
contract AngelStaking is Ownable {
  using SafeMath for uint256;
  ...
  event RewardPaid(address indexed user, uint256 reward);
    ...
    //@notice event emitted when contract receive stable coin
  event TransferReceived(address _form, uint _amount);
  ...
  constructor(address chicksAddress, address eggAddress) {
    setEggContract(eggAddress);
    setChicksContract(chicksAddress);    
  }
  ...
  function setChicksContract(address chicksAddress) public onlyOwner{
    ...
    _angelContract = IAngel(chicksAddress);
  }
  ...
  function stakeArray(
    ...
    require(_angelContract.ownerOf((_tokenIds[i])) == msg.sender || msg.sender == owner(), "Sender is not owner of current NFT token");
  ...
  function stake(
    ...
    console.log("abcddfefsef", _angelContract.ownerOf((_tokenId)));
    // console.log("getNFTUser", _angelContract.getNFTUser(_tokenId));
    require(_angelContract.ownerOf((_tokenId)) == msg.sender || msg.sender == owner(), "Sender is not owner of current NFT token");
  ...
  function _stake(
    ...
    _angelContract.transferFrom(user, address(this), _tokenId);
    addStakeUsers(user);
  ...
  function _unstake(
    ...
    _angelContract.transferFrom(address(this), user, _tokenId);
