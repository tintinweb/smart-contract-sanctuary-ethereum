...
contract JellyPool is IJellyPool, JellyPoolNFT, Documents {
    ...
    // bytes32 public constant override TEMPLATE_ID = keccak256("JELLY_POOL");
        ...
        if (userBalance == 0) {
            ...
            for(uint i = 0; i < rewardTokens.length; i++) {
                ...
                if(tokenRewards[_tokenId][rewardToken].lastRewardPoints == 0) {
                    tokenRewards[_tokenId][rewardToken].lastRewardPoints = uint256(poolRewards[rewardToken].rewardsPerTokenPoints);
                }
            }
        }
        ...
        public
    {
        ...
        rewarder.updateRewards(address(this));
        ...
        for(uint i = 0; i < rewardTokens.length; i++) {
            ...
            if (_exists(_tokenId) && sTotal > 0) {
                uint256 currentRewards = rewarder.poolRewards(address(this), rewardToken, uint256(rInfo.lastUpdateTime), block.timestamp);
                currentRewardPoints = currentRewardPoints + (currentRewards * 1e18 * pointMultiplier / sTotal);
            }
            poolRewards[rewardToken] = RewardInfo(BoringMath.to48(block.timestamp), BoringMath.to208(currentRewardPoints)) ;

            TokenRewards memory tRewards = tokenRewards[_tokenId][rewardToken];
            uint256 rewards = tokenInfo[_tokenId].staked * (currentRewardPoints - tRewards.lastRewardPoints)
                                                                / 1e18
                                                                / pointMultiplier;
