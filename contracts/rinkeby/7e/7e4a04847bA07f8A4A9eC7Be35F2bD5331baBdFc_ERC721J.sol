    ...
    Ownable
{
    ...
    function ownerMintsCopy(uint256 tokenId, address to) public onlyOwner {
        ...
        _safeMintCopy( to, id);
        emit Transfer(ownerOf(tokenId), to, id);
    ...
    function mintCopy(uint256 tokenId) public payable {
        ...
        _safeMintCopy(msg.sender, id);
         emit Transfer(ownerOf(tokenId), msg.sender, id);
    ...
    function _mintCopy(address to, uint256 tokenId) internal virtual {
        if (to == address(0)) revert MintToZeroAddress();
        if (_exists(tokenId)) revert TokenAlreadyMinted();
