    ...
    Withdrawable
{
    ...
    uint256 public EACH_MAX_QTY = 200;
    uint256 public PUBLIC_MAX_QTY_PER_MINTER = 3;

    mapping(address => uint256) public publicSalesMinterToTokenQty;
    mapping(uint256 => uint256) public typesQuantity;
    mapping(uint256 => uint256) public idToNFTType;

    uint256 public publicSalesMintedQty = 0;

    constructor()
        ERC721("Sidekast Memento", "SK-MEMENTO")
        Whitelist("Sidekast Memento", "1")
    {}
    ...
    {
        require(publicSalesMintedQty < TOTAL_SUPPLY, "Exceed sales max limit");
        require(_nftType > 0 && _nftType < 7, "Error NFT type");
        require(
            typesQuantity[_nftType] + 1 <= EACH_MAX_QTY,
            "Exceed max mint for this type of NFT"
        );
        require(
        ...
        typesQuantity[_nftType] += 1;
        publicSalesMinterToTokenQty[msg.sender] += 1;
        publicSalesMintedQty += 1;
        idToNFTType[publicSalesMintedQty] = _nftType;
        _safeMint(msg.sender, publicSalesMintedQty);
    }
    ...
    function changeEachMaxQty(uint256 eachMaxQty) external onlyOwner {
        EACH_MAX_QTY = eachMaxQty;
    }

    function changeMaxQuantityPerMinter(uint256 maxQtyPerMinter)
        external
        onlyOwner
    {
        PUBLIC_MAX_QTY_PER_MINTER = maxQtyPerMinter;
    }
}
