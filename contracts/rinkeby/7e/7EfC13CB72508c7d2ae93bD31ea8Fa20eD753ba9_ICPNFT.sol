...
contract ICPNFT is ERC721, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxPresaleMintAmountPerTx = 1;
  ...
  constructor() ERC721("ICPNFT", "ICPNFT") {
    setUriPrefix("");
  ...
  function getAddressTokenIds(address _owner) public view returns(uint256[] memory) {
      return addressTokenIds[_owner];
  }
  ...
  function _mintLoop(address _receiver, uint256 _mintAmount, uint256[] memory ids) internal {
    ...
    for (uint256 i = 0; i < _mintAmount; i++) {
      ...
      addressTokenIds[_receiver].push(ids[i]);
