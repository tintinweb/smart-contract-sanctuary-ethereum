...
import "./IFailedExperiments.sol";
import "./IScales.sol";

contract Mutants is ERC721, Ownable, ReentrancyGuard, VRFConsumerBase {
  ...
  uint256 public completedExperiments;
  ...
  function claim(uint256[] calldata tokenIds) public nonReentrant {
    ...
    completedExperiments += tokenIds.length;
  }

  /**
   * @notice Mint a mutant using a signature
   * @param amount of mutants to mint
   * @param free indicates if the mint costs anything
   * @param signature created by signer account
   */
  function mint(uint256 amount, bool free, bytes memory signature) public payable nonReentrant {
    require(!signatureUsed[signature], "Signature already used");

    address _signer = ECDSA.recover(
      ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(_msgSender(), amount, free))),
      signature
    );
    require(signer == _signer, "Invalid signature");

    require(msg.value == MINT_PRICE * (free ? 0 : amount) , "Invalid Ether amount sent");
    require(
      totalSupply - completedExperiments + amount <= MAX_MINTABLE,
      "Insufficient supply"
    );
...
interface IScales is IERC20 {
  function getClaimable(address) external view returns (uint256);
