...
contract ETH_TO_FIAT4 {
    ...
    using RoomStruct for RoomStruct.RoomNumber;
    ...
    mapping(uint256 => RoomStruct.RoomNumber) roomNumberMap;
    ...
    function makeRoomEth(
        ...
        require(_timeForApproveFromSender <= maxTimeForApproveFromSender &&
                ...
                _timeForExecutor % multiplicityOfTime == 0, "Incorrect time");
        require(_maxLimit > _lowLimit && msg.value > _lowLimit && _maxLimit > msg.value, "Incorrect limits");
    ...
    function completeDeal (
        ...
        roomNumberMap[_roomNumber].executor[roomNumberMap[_roomNumber].counter] = RoomStruct.Executor({
    ...
    function getExecutor(uint256 _roomNumber, uint256 _counter) public view returns(RoomStruct.Executor memory) {
...
library RoomStruct {
    ...
    function _getRoomDynamic(RoomNumber storage self) internal view returns(
        bool,
        bool,
        bool,
        uint256,
    ...
    function _getRoomStatic(RoomNumber storage self) internal view returns(
        address,
        uint256,
        uint64,
        uint256,
        ...
        return(
            self.sender,
            self.timestamp,
            self.timeForApproveFromSender,
            self.maxLimit,
            self.lowLimit,
            self.timeForExecutor
        );
    }
