    ...
    ReentrancyGuard
{
    ...
    function _beforeTokenTransfer(
        ...
        if ((_from != address(0)) && (_to != address(0))) {
            if (
                currentStatus == Status.forFractionSale &&
                userReservePrice[_from] != userReservePrice[_to]
            ) {
                if (userReservePrice[_to] != 0) {
                    //_to have voted
                    totalVoted += _amount;
                    reservePriceTotal +=
                        (_amount * userReservePrice[_to] * totalFNFT) /
                        totalSupply();
                    auctionLengthTotal +=
                        (_amount * userAuctionLength[_to]) /
                        totalSupply();
                    bidIncrementTotal +=
                        (_amount * userBidIncrement[_to]) /
                        totalSupply();
                }
                if (userReservePrice[_from] != 0) {
                    ...
                    totalVoted -= _amount;
                    reservePriceTotal -=
                        (_amount * userReservePrice[_from] * totalFNFT) /
                        totalSupply();
                    auctionLengthTotal -=
                        (_amount * userAuctionLength[_from]) /
                        totalSupply();
                    bidIncrementTotal -=
                        (_amount * userBidIncrement[_from]) /
                        totalSupply();
                }
            }
