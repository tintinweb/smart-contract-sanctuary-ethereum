...
contract RottenBananos is ERC1155, Ownable {
    ...
    uint256 public constant MAX_MINTS_WL = 1;
    ...
    function MintWhitelist(uint256 DVDA_ID) public payable {
        require(!WhitelistPaused, "Whitelist sale is paused"); //whitelist paused or not
        require(MAX_MINTS_WL + _minted <= MAX_WL_SUPPLY, "Bananos: Exceed WL max supply"); //max supply
        address _owner = DVDA.ownerOf(DVDA_ID); //owner of DVDA token ID
        require(_owner == msg.sender, "Must be Owner of DVDA TOKEN ID to mint"); //requires to own at least 1 dvda
        require(msg.value == MAX_MINTS_WL * BANANA_PRICE_WHITELIST, "Invalid funds provided"); //price: free
        require(!dvdaTokenIDBlacklisted[DVDA_ID], "DVDA Token ID Blacklisted"); //requires not blacklisted
        require(!AlreadyClaimedDVDA[DVDA_ID], "Banano already claimed with this DVDA Token ID.");
        _minted += MAX_MINTS_WL;
        AlreadyClaimedDVDA[DVDA_ID] = true; 
        _mint(msg.sender, 0, MAX_MINTS_WL, "");
        delete _owner;
    }
    ...
    function MintPublic(uint256 amount) public payable {
        ...
        require(amount <= MAX_MINTS_PUBLIC, "Max NFT per address exceeded");
