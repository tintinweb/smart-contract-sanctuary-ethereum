...
contract SpaceCoinLP is ERC20 {
    ...
    function mint(address _recipient) external returns (uint lpTokensToMint) {
        SpaceCoin spaceCoin = SpaceCoin(tokenAddress);
        ...
        if (_totalSupply == 0) {
            ethTotal = ethAdded;
            spcTotal = spcAdded;
            k = ethAdded * spcAdded / 1 ether;   //k value = totalSupply * k 

            _mint(_recipient, 1 ether); //we're doing it this way. 1 LP token = 100% of the pool at inception. 
            return 1 ether;
        } else {
    ...
    function burn(address _recipient) external returns (uint256 ethToSend, uint256 spcToSend) {
        ...
        uint256 ethBalance = address(this).balance;
        uint256 spcBalance = IERC20(tokenAddress).balanceOf(address(this));
        ...
        bool success = IERC20(tokenAddress).transfer(_recipient, spcToSend);
    ...
    function skim(address _recipient) external {
        (bool sent, ) = _recipient.call{value: address(this).balance - ethTotal}("");
        require(sent, "Failed to send Ether");

        bool success = IERC20(tokenAddress).transfer(_recipient, IERC20(tokenAddress).balanceOf(address(this)) - spcTotal);
        require (success, "SPC transfer failed");           
    }
}
...
contract SpaceRouter {
    address public tokenAddress;
    address payable public lpTokenAddress;
    ...
    function addLiquidity(uint256 amountSpcIn, uint256 amountSpcMin, uint256 amountEthMin) external payable {
        SpaceCoinLP lpContract = SpaceCoinLP(lpTokenAddress);
        ...
        if (lpContract.totalSupply() != 0) {
            //check the quote
            //for true: amountIn = spcIn, amountOut = ethInBalanced
            uint256 amountEthBalanced = lpContract.idealQuote(amountSpcIn, true);
        ...
        //send the coins 
        bool success = IERC20(tokenAddress).transferFrom(msg.sender, lpTokenAddress, amountSpcToAdd);
    ...
    function swapExactTokenForEth(uint256 amountIn, uint256 amountOutMin) external {
        SpaceCoinLP lpContract = SpaceCoinLP(lpTokenAddress);
        bool success = IERC20(tokenAddress).transferFrom(msg.sender, lpTokenAddress, amountIn);
        require (success, "could not transfer SPC"); 
