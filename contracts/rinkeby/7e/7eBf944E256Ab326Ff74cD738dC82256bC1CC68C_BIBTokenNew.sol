...
contract BIBTokenNew is Initializable, ERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    uint64 private constant _startingTime = 1660492800;

    VestingWallet public teamVestingWallet;
    VestingWallet public privateSaleVestingWallet;
...
transfer(liquidityAddr,(initialSupply.mul(3).div(100))); // Send 5% of tokens to ido wallet 5,000,000,000 bibtoken
transfer(marketingAddr,(initialSupply.mul(8).div(100)));// Send 3% of tokens to tournaments wallet 35,000,000,000 bibtoken

transfer(nftminingAddr,(initialSupply.mul(16).div(100))); // Send 8% of tokens to liquidity wallet 6,000,000,000 bibtoken
transfer(businessAddr,(initialSupply.mul(15).div(100)));// Send 16% of tokens to nftmining wallet 4,000,000,000 bibtoken

}


function linearRelease(address token) public onlyOwner {
    //to  team reserve, lock one year
    teamVestingWallet = new VestingWallet(devAddr, _startingTime, 365 days);
    //to  privateSale, lock one year,release one year ,合约上线365天后，再分365天线性解锁。
    privateSaleVestingWallet = new VestingWallet(privateSaleAddr, _startingTime + 365 days, 365 days);
    //to nft, lock to 4 years
    nftVestingWallet = new VestingWallet(nftminingAddr, _startingTime, 4 * 365 days);

    teamVestingWallet.release(token);
    privateSaleVestingWallet.release(token);
    nftVestingWallet.release(token);
