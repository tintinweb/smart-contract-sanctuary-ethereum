...
contract Land is ERC721Enumerable, Ownable, ReentrancyGuard, VRFConsumerBase {
    ...
    uint256 immutable public MAX_PUBLIC_SALE_AND_CONTRIBUTORS_AMOUNT;
    uint256 immutable public RESERVED_CONTRIBUTORS_AMOUNT;
    uint256 immutable public MAX_FUTURE_LANDS;
    uint256 constant public MAX_MINTABLE_AMOUNT = 150;
    uint256 constant public MAX_METADATA_LOADING_SIZE = 300;

    // structs
    struct LandAmount {
        uint256 alpha;
        uint256 beta;
        uint256 publicSaleAndContributors;
        uint256 future;
    }
    ...
    modifier checkMetadataRange(Metadata memory _landMetadata){
        require(_landMetadata.range[1] < MAX_LANDS_WITH_FUTURE, "Range upper bound cannot exceed MAX_LANDS_WITH_FUTURE - 1");
        _;
    }
    ...
    modifier mintLandsRequires(uint256 numLands) {
        require(numLands > 0, "Must mint at least one beta");   
        require(currentNumLandsMintedPublicSale + numLands <= MAX_PUBLIC_SALE_AND_CONTRIBUTORS_AMOUNT - RESERVED_CONTRIBUTORS_AMOUNT, "Minting would exceed max supply");
        require(numLands <= MAX_MINTABLE_AMOUNT, "numLands should not exceed MAX_MINTABLE_AMOUNT");
        _;
    }
    ...
    constructor(string memory name, string memory symbol,
        ...
        MAX_PUBLIC_SALE_AND_CONTRIBUTORS_AMOUNT = amount.publicSaleAndContributors;
        MAX_FUTURE_LANDS = amount.future;
        MAX_LANDS = amount.alpha + amount.beta + amount.publicSaleAndContributors;
    ...
    function getRemainingSaleTime() external view returns (uint256) {
        ...
        if (getElapsedSaleTime() >= publicSalePriceLoweringDuration) {
            return 0;
        }

        return (publicSaleStartTime + publicSalePriceLoweringDuration) - block.timestamp;
    }
    ...
    function getMintPrice() public view whenPublicSaleActive returns (uint256) {
        ...
        if (elapsed >= publicSalePriceLoweringDuration) {
            ...
            uint256 currentPrice = ((publicSalePriceLoweringDuration - elapsed) *
                publicSaleStartPrice) / publicSalePriceLoweringDuration;
            price = 
                currentPrice > publicSaleEndingPrice
                    ? currentPrice
    ...
    function mintLands(uint256 numLands) external whenPublicSaleActive nonReentrant mintLandsRequires(numLands) {
        require(msg.sender == tx.origin, "Address originating transaction should be the same as caller");
        uint256 costToMint = getMintPrice() * numLands;
        
        IERC20(tokenContract).safeTransferFrom(msg.sender, address(this), costToMint);
    ...
    function nftOwnerClaimLand(uint256[] calldata alphaTokenIds, uint256[] calldata betaTokenIds) external whenClaimableActive {
        ...
        require(alphaTokenIds.length + betaTokenIds.length <= MAX_MINTABLE_AMOUNT, "Input length should be <= MAX_MINTABLE_AMOUNT");
    ...
    function contributorsClaimLand(uint256 amount, address recipient) external onlyContributors(msg.sender) whenContributorsClaimActive {
        ...
        require(amount <= MAX_MINTABLE_AMOUNT, "amount should not exceed MAX_MINTABLE_AMOUNT");
    ...
    function claimUnclaimedAndUnsoldLands(address recipient) external onlyOwner {
        claimUnclaimedAndUnsoldLandsWithAmount(recipient, MAX_MINTABLE_AMOUNT);
    }

    function claimUnclaimedAndUnsoldLandsWithAmount(address recipient, uint256 maxAmount) public onlyOwner {
        require(publicSaleStartTime > 0 && !claimableActive && !publicSaleActive,
            "cannot claim the unclaimed if claimable or public sale are active");
        require(maxAmount <= MAX_MINTABLE_AMOUNT, "maxAmount cannot exceed MAX_MINTABLE_AMOUNT");
    ...
    function claimFutureLands(address recipient) external onlyOwner {
        claimFutureLandsWithAmount(recipient, MAX_MINTABLE_AMOUNT);
    }
    ...
    // metadata
    function loadLandMetadata(Metadata memory _landMetadata) external onlyOperator checkMetadataRange(_landMetadata) {
        if(metadataHashes.length == 0){
            require(_landMetadata.range[1] == MAX_LANDS - 1, "For first metadata range upper bound should be MAX_LANDS - 1");
        }
        metadataHashes.push(_landMetadata);
    } 

    function putLandMetadataAtIndex(uint256 index, Metadata memory _landMetadata) external onlyOperator checkMetadataRange(_landMetadata) {
        if(index == 0){
            require(_landMetadata.range[1] == MAX_LANDS - 1, "For first metadata range upper bound should be MAX_LANDS - 1");
        }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        if(isRandomRequestForPublicSaleAndContributors[requestId]){
            publicSaleAndContributorsOffset = (randomness % MAX_PUBLIC_SALE_AND_CONTRIBUTORS_AMOUNT);
