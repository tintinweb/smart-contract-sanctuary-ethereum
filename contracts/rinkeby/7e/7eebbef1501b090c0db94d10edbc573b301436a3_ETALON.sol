...
abstract contract FastSellLimit {
    ...
    function getTimeInterval(uint256 balance, uint256 onePercent)
        ...
        returns (uint256)
    {
        if (balance >= onePercent) return 2 minutes; // if more 1% of total then 24 hours
        return (2 minutes * balance) / onePercent;
    }
...
abstract contract TradableErc20 is RewardableErc20, FastSellLimit, Ownable {
        ...
        returns (uint256)
    {
        ...
        uint256 c = getWalletSizeFee(_balances[account] + amount, amount);
        return a + b + c;
    }
    ...
    function getFeeTransfer(address account, uint256 amount)
        ...
        returns (uint256)
    {
        return getWalletSizeFee(_balances[account] + amount, amount);
    }
    ...
    function _getFeeBuy(address from, uint256 amount)
        ...
        returns (uint256)
    {
        return _arrangeFee(from, amount, getFeeBuy(from, amount));
    }
