...
contract OSBFactory is ContextUpgradeable {
    ...
    function setLibraryAddress(address _library721Address, address _library1155Address) external {
    ...
    function create(bool _isSingle, string memory _baseUri, string memory _name, string memory _symbol, uint256 _maxBatch, address _royaltyReceiver, uint96 _royaltyFeeNumerator) external returns(TokenInfo memory) {
        ...
        if(_isSingle) {
            ...
            _osb721.initialize(_msgSender(), _baseUri, _name, _symbol, _maxBatch, _royaltyReceiver, _royaltyFeeNumerator); 
            deployedContract = address(_osb721);
        } else {
            OSB1155 _osb1155 = OSB1155(ClonesUpgradeable.cloneDeterministic(library1155Address, salt));
            _osb1155.initialize(_msgSender(), _baseUri, _name, _symbol, _maxBatch, _royaltyReceiver, _royaltyFeeNumerator); 
...
contract $OSBFactory is OSBFactory {
    ...
    function $setting() external view returns (ISetting) {
        return setting;
    }
