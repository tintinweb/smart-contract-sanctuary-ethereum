   ...
   function stake(uint256 tokenId, uint256 cargo) external {
    ...
    function unstake(uint256 tokenId, uint256 cargo) external {
        ...
        if(cargo == 1){
            ...
            uint256 taxafinal = calculateTokens(tokenId).mul(taxa).div(10**2);
            _transfer(address(this), msg.sender, calculateTokens(tokenId).sub(taxafinal)); // Minting the tokens for staking
            _transfer(address(this), owner(), taxafinal); // transfer fees to owner
    ...
    function stakefees(uint256 fees) external onlyOwner {
	    require(fees >= 0 && fees <= 100, "Value out of range: values between 0 and 100");
