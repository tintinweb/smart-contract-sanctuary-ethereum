...
contract Bear is ERC721AQueryable, BaseTokenURI {
  using Strings for uint256;
  using ECDSA for bytes32;

  address public signer;
  uint256 public maxAllowance = 1;

  mapping(uint256 => bool) public isPhaseRunning;
  mapping(uint256 => uint256) public phaseCost;
  ...
  constructor(
    ...
    phaseCost[1] = _phase1Cost;
    phaseCost[2] = _phase2Cost;
  }
  ...
  function mint(
    ...
    require(isPhaseRunning[_phase], "Minting is paused");
    require(isSignatureValid(msg.sender, _phase, mintNonce[msg.sender], _signature), "Bear: invalid signature");
    require(balanceOf(msg.sender) + _mintAmount <= maxAllowance, "Purchase would exceed max allowance");

    mintX(msg.sender, _mintAmount, _phase);
    mintNonce[msg.sender] += 1;
  }
  ...
  function mintX(
    ...
    if (to != owner() && msg.sender != owner()) {
      require(msg.value >= phaseCost[_phase] * _mintAmount, "Amount sent less than the cost of minting NFT(s)");
      if (msg.value > phaseCost[_phase] * _mintAmount) {
        (bool success, ) = address(msg.sender).call{value: msg.value - (phaseCost[_phase] * _mintAmount)}("");
  ...
  function isSignatureValid(
    ...
    bytes32 hash = keccak256(abi.encodePacked(address(this), _whitelistAddress, _phase, _nonce));
  ...
  function setIsPhaseRunning(uint256 _phase, bool _isRunning) external onlyOwner {
    isPhaseRunning[_phase] = _isRunning;
  }

  function setPhaseCost(uint256 _phase, uint256 _cost) external onlyOwner {
    phaseCost[_phase] = _cost;
  }
  ...
  function configure(uint256 _maxAllowance, address _signer) external onlyOwner {
    maxAllowance = _maxAllowance;
    signer = _signer;
  }
