...
contract NFTSales is Ownable, ERC721Min, ReentrancyGuard {
    ...
    address private treasuryAddress = 0xA791EEFD0ffa00337Bdd22ee58B8cE2C42C3499A; 
    ...
    constructor() ERC721Min("BAHAMAS EXPERIENCE NFT", "BXP") {
    ...
    function buyOne(uint256 nftTypeId) external payable {
        ...
        // update pending BXP
        UserInfos[_msgSender()].pendingBXP += UserInfos[_msgSender()].bxpPerBlock * (block.number - (UserInfos[_msgSender()].lastCalculatedBlock));
        UserInfos[_msgSender()].lastCalculatedBlock = block.number;
        UserInfos[_msgSender()].bxpPerBlock += NftTypes[nftTypeId].bxpPerBlock;
    }     
    ...
    function buyThree(uint256 nftTypeId) external payable {
        ...
        // update pending BXP
        UserInfos[_msgSender()].pendingBXP += UserInfos[_msgSender()].bxpPerBlock * (block.number - (UserInfos[_msgSender()].lastCalculatedBlock)) * 3;
        UserInfos[_msgSender()].lastCalculatedBlock = block.number;
        UserInfos[_msgSender()].bxpPerBlock += NftTypes[nftTypeId].bxpPerBlockForThree;
    }
    ...
    function buy(uint256 nftTypeId, uint16 amount) external payable {
        ...
        // update pending BXP
        UserInfos[_msgSender()].pendingBXP += UserInfos[_msgSender()].bxpPerBlock * (block.number - (UserInfos[_msgSender()].lastCalculatedBlock)) * amount;
        UserInfos[_msgSender()].lastCalculatedBlock = block.number;
        UserInfos[_msgSender()].bxpPerBlock += NftTypes[nftTypeId].bxpPerBlock * amount;
    }
    ...
    function mintOne(address receiver, uint256 nftTypeId) external onlyProxy {
        ...
        _mintMin2(receiver);
        // update pending BXP
        UserInfos[receiver].pendingBXP += UserInfos[receiver].bxpPerBlock * (block.number - (UserInfos[receiver].lastCalculatedBlock));
        UserInfos[receiver].lastCalculatedBlock = block.number;
        UserInfos[receiver].bxpPerBlock += NftTypes[nftTypeId].bxpPerBlock;
    }
    ...
    function mintThree(address receiver, uint256 nftTypeId) external onlyProxy {
        ...
        _mintMin2(receiver);
        // update pending BXP
        UserInfos[receiver].pendingBXP += UserInfos[receiver].bxpPerBlock * (block.number - (UserInfos[receiver].lastCalculatedBlock)) * 3;
        UserInfos[receiver].lastCalculatedBlock = block.number;
        UserInfos[receiver].bxpPerBlock += NftTypes[nftTypeId].bxpPerBlockForThree;
    }   
    ...
    function mint(address receiver, uint256 nftTypeId, uint256 tokenQuantity) external onlyProxy {
        ...
        // update pending BXP
        UserInfos[receiver].pendingBXP += UserInfos[receiver].bxpPerBlock * (block.number - (UserInfos[receiver].lastCalculatedBlock)) * tokenQuantity;
        UserInfos[receiver].lastCalculatedBlock = block.number;
        UserInfos[receiver].bxpPerBlock += NftTypes[nftTypeId].bxpPerBlock * tokenQuantity;
    }    
    ...
    // used by external contract to interact with user BXP
    function redeemBXP(address user, uint256 amount) external onlyProxy {
        require(UserInfos[user].pendingBXP >= amount, "INSUFFICIENT_BXP");
        UserInfos[user].pendingBXP -= amount;
    }

    function pendingBXP(address user) external view returns(uint256) {
        return UserInfos[user].pendingBXP + UserInfos[user].bxpPerBlock * 
            (block.number - (UserInfos[user].lastCalculatedBlock));
    }
    ...
    function gift(uint256 nftTypeId, address[] calldata receivers, uint256[] memory amounts) external onlyOwner {
        uint256 bxpPerBlock = NftTypes[nftTypeId].bxpPerBlock;
        for (uint256 x = 0; x < receivers.length; x++) {
            ...
            // update pending BXP
            UserInfos[receivers[x]].pendingBXP += UserInfos[receivers[x]].bxpPerBlock * (block.number - (UserInfos[receivers[x]].lastCalculatedBlock)) * amounts[x];
            UserInfos[receivers[x]].lastCalculatedBlock = block.number;
            UserInfos[receivers[x]].bxpPerBlock += bxpPerBlock * amounts[x];
        }
    }

    // ** - NFT Types - ** //
    function addNftType(uint16 _maxMint, uint256 _price, bool _saleActive, 
        string calldata _uri, uint256 _bxpPerBlock) external onlyOwner
    {
        nftTypeCount++;
        ...
        NftTypes[nftTypeCount].bxpPerBlock = _bxpPerBlock;
        NftTypes[nftTypeCount].bxpPerBlockForThree = _bxpPerBlock * 3;
    }
    ...
    // transfers are gonna suck, but it is what it is
    function batchSafeTransferFrom(address _from, address _to, uint256[] memory _tokenIds, bytes memory data_) public override {
        ...
        uint256 totalBXPPerBlock;
        for(uint256 x; x < _tokenIds.length; x++) {
            totalBXPPerBlock += NftTypes[NftIdToNftType[_tokenIds[x]]].bxpPerBlock;
        }     

        if (totalBXPPerBlock == 0) super.batchSafeTransferFrom(_from, _to, _tokenIds, data_);
        // update pending BXP for _from
        UserInfos[_from].pendingBXP += UserInfos[_from].bxpPerBlock * (block.number - (UserInfos[_from].lastCalculatedBlock));
        UserInfos[_from].lastCalculatedBlock = block.number;
        UserInfos[_from].bxpPerBlock -= totalBXPPerBlock;
        // update pending BXP for _to
        UserInfos[_to].pendingBXP += UserInfos[_to].bxpPerBlock * (block.number - (UserInfos[_to].lastCalculatedBlock));
        UserInfos[_to].lastCalculatedBlock = block.number;
        UserInfos[_to].bxpPerBlock += totalBXPPerBlock;
    ...
    function batchTransferFrom(address _from, address _to, uint256[] memory _tokenIds) public override {
        ...
        if (totalBXPPerBlock == 0) super.batchTransferFrom(_from, _to, _tokenIds);
        // update pending BXP for _from
        UserInfos[_from].pendingBXP += UserInfos[_from].bxpPerBlock * (block.number - (UserInfos[_from].lastCalculatedBlock));
        UserInfos[_from].lastCalculatedBlock = block.number;
        UserInfos[_from].bxpPerBlock -= totalBXPPerBlock;
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public override {
        require(transferEnabled || _msgSender() == owner() || proxyToApproved[_msgSender()], "TRANSFER_DISABLED");
        if (NftTypes[NftIdToNftType[tokenId]].bxpPerBlock == 0) super.safeTransferFrom(from, to, tokenId, _data);

        // update pending BXP for _from
        UserInfos[from].pendingBXP += UserInfos[from].bxpPerBlock * (block.number - (UserInfos[from].lastCalculatedBlock));
        UserInfos[from].lastCalculatedBlock = block.number;
        UserInfos[from].bxpPerBlock -= NftTypes[NftIdToNftType[tokenId]].bxpPerBlock;
        // update pending BXP for _to
        UserInfos[to].pendingBXP += UserInfos[to].bxpPerBlock * (block.number - (UserInfos[to].lastCalculatedBlock));
        UserInfos[to].lastCalculatedBlock = block.number;
        UserInfos[to].bxpPerBlock += NftTypes[NftIdToNftType[tokenId]].bxpPerBlock;
    ...
    function transferFrom(address from, address to, uint256 tokenId) public override {
        require(transferEnabled || _msgSender() == owner() || proxyToApproved[_msgSender()], "TRANSFER_DISABLED");
        if (NftTypes[NftIdToNftType[tokenId]].bxpPerBlock == 0) super.transferFrom(from, to, tokenId);
