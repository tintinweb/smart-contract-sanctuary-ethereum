...
// SPDX-License-Identifier: NOLICENSE
 //Airdrop single and multiple working
 //walletsize working both presale and fairlaunch
 //max transaction working for fairlaunch
 //swap enabled and trading enabled
 //old with verification phase until buy. sell is working but needs to workout properly
...
interface IERC20 {
    ...
    event SwapAndLiquifyEvent(address indexed from, address indexed to, uint value);
    event AddLiquidityEvent(address indexed from, address indexed to, uint value1, uint value2);
    event SwapNarrativeEvent(uint denominator,uint tokensToAddLiquidityWith,uint toSwap,uint initialBalance,uint deltaBalance,uint unitBalance,uint ethToAddLiquidityWith);
}
...
contract SmartContract is Context, IERC20, Ownable {
    ...
    uint256 public maxWalletSize = 10000000000000001 * 10**9; //10quad
    ...
    address public marketingAddress = 0x3a2775eED458cef0f2C6cFa169Ff6f4E1ed17885;  //MAC0xDC41032b5CaA4D20bc45C3d79225834D94586b11 

    address private routerAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    string private constant _name = "SmartContract";
    string private constant _symbol = "FTF5";
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        emit SwapNarrativeEvent(denominator, tokensToAddLiquidityWith, toSwap, initialBalance, deltaBalance, unitBalance, ethToAddLiquidityWith);

        if(ethToAddLiquidityWith > 0){
            addLiquidity(tokensToAddLiquidityWith, ethToAddLiquidityWith);
            emit AddLiquidityEvent(_msgSender(), owner(), tokensToAddLiquidityWith, ethToAddLiquidityWith);
        }

        uint256 devAmt = unitBalance * 2 * taxes.dev;
        if(devAmt > 0){
            payable(devAddress).transfer(devAmt);
            emit SwapAndLiquifyEvent(address(this), devAddress, devAmt);
        }
        
        uint256 marketingAmt = unitBalance * 2 * taxes.marketing;
        if(marketingAmt > 0){
            payable(marketingAddress).transfer(marketingAmt);
            emit SwapAndLiquifyEvent(address(this), marketingAddress, marketingAmt);
        }
