...
contract FixedRateMarket is Initializable, ERC20, IFixedRateMarket {
  ...
  /// @param collateralAddress Liquidator's choice of which currency to be paid in
  function liquidateBorrow(
    ...
    require(
            _qollateralManager.liquidityRatio(borrower) < _qParams.minLiquidityRatio() ||
            block.timestamp > _maturity,
            "account ineligible for liquidation"
            );    
    ...
    if(block.timestamp > _maturity){
      closeFactor = _qParams.MANTISSA_FACTORS();
    }
        
    // Liquidator cannot repay more than the percentage of the full loan balance
    // determined by `closeFactor`
    uint maxRepayment = _accountBorrows[borrower] * closeFactor / _qParams.MANTISSA_FACTORS();
    amount = Math.min(amount, maxRepayment);

    // Amount must be positive
    require(amount > 0, "liquidation/owed amount must be positive");
    ...
    uint rewardUSD = amountUSD * _qParams.liquidationIncentive() / _qParams.MANTISSA_FACTORS();

    // Get the local amount of collateral to reward liquidator
    uint rewardLocal = _qollateralManager.USDToLocal(collateralAddress, rewardUSD);

    // Ensure the borrower has enough collateral balance to pay the liquidator
    uint balance = _qollateralManager.collateralBalance(borrower, collateralAddress);
    require(rewardLocal <= balance, "borrower account balance too low");
...
contract QollateralManager is Initializable {
  ...
  function _addAccountMarket(address account) external {
    ...
    require(_qontroller.isMarketEnabled(msg.sender), "Unauthorized");
  ...
  function _transferCollateral(
    ...
    require(amount <= _collateralBalances[from][tokenAddress], "`from` balance too low");
  ...
  function _localToUSD(
    ...
    QTypes.Asset memory asset = _qontroller.assets(tokenAddress);
    require(asset.isEnabled, "token not supported");
...
contract QParams is Initializable {
  ...
  function _setCloseFactor(uint closeFactor_) external {
    ...
    require(closeFactor_ <= MANTISSA_FACTORS(), "must be between 0 and 1");
  ...
  function MANTISSA_APR() public pure returns(uint){
    return 1e4;
  }
