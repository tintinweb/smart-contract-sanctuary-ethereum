...
contract AffiliateMint is ERC1155, Ownable, ReentrancyGuard {
    ...
    uint256 public constant EARLY_SALE_PRICE = 0.01 ether; //0.45 ether;
    uint256 public constant SALE_PRICE = 0.02 ether; //0.68 ether;
    ...
    /// @dev requires EARLY_SALE_PRICE to be sent if supply < 1251, or SALE_PRICE aftert that condition has been met.
    modifier isCorrectPayment(uint256 quantity) {
        if (totalSupplyMember + quantity < 1251) {
            require(EARLY_SALE_PRICE * quantity == msg.value, "Incorrect ETH value sent");
        } else if (totalSupplyMember > 1250) {
            require(SALE_PRICE * quantity == msg.value, "Incorrect ETH value sent");
        } else {
            uint256 num = quantity - (1250 - totalSupplyMember);
            uint256 earlyNum = quantity - num;
            require(
                ((SALE_PRICE * num) + (earlyNum * EARLY_SALE_PRICE)) == msg.value,
    ...
    function withdraw() external onlyOwner withdrawUnlocked {
