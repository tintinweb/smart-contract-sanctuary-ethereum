...
contract NounletToken is Clone, ERC1155B, INounletToken {
    ...
    /// @notice Mapping to track delegate votes
    mapping(address => mapping(uint256 => address)) private _delegates;
    /// @notice Mapping to track number of delegate checkpoints
    mapping(address => mapping(uint256 => uint256)) public numCheckpoints;
    /// @notice Mapping to track delegate checkpoints
    ...
    function safeTransferFrom(
        ...
        uint256 _amount,
        bytes memory _data
    ) public override(ERC1155B, INounletToken) {
        require(
            msg.sender == _from ||
        ...
        require(_from == ownerOf[_id], "WRONG_FROM"); // Can only transfer from the owner.
        ...
        } else require(_to != address(0), "INVALID_RECIPIENT");
    }

    /// @notice Getter for delegates of address
    /// @param account Address of delegating account
    ...
    /// @notice Getter for current votes
    /// @param account Address of delegating account
    /// @param id ID of the token type
    ...
    /// @notice Getter for previous votes
    /// @param account Address of delegating account
    /// @param id ID of the token type
    /// @param timestamp Timestamp of checkpoint
    ...
    function batchDelegate(address account, uint256[] calldata ids)
        external
        payable
    {
    ...
    /// @dev Move delgate votes between addresses
    /// @param srcRep Address of original delegate
    /// @param dstRep Address of new delegate
    /// @param id ID of the token type
    ...
    /// @dev Write checkpoints of vote counts
    ...
    function _writeCheckpoint(
        ...
        unchecked {
            ...
            if (
                ...
                checkpoints[delegatee][nCheckpoints - 1][id].votes = newVotes
                    .safeCastTo192();
            } else {
