...
contract Staking {
    ...
    function __Staking_init() public {
		require(dpos == address(0));
        dpos = msg.sender;
...
contract DPOS is Configurable {
    ...
    function __DPOS_init(DPosMine dposMine, Staking staking_, uint maxValidators_) public virtual initializer {
        __Governable_init_unchained(msg.sender);
        __DPOS_init_unchained(dposMine, staking_, maxValidators_);
    }
    ...
    function _forward(address delegate) internal {
        ...
        if(v && (N < maxValidators) && (p == address(0) || staking.isValidator(p)))
            staking.addValidator(delegate);
        if(p == dd.next)
            return;
