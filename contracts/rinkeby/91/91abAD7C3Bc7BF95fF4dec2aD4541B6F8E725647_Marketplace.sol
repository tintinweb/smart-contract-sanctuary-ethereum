...
contract Marketplace is ReentrancyGuard,ERC1155Holder {
    ...
    uint256 private listingFee = 0.000045 ether;
    ...
    function createMarketItem(
        ...
        require(price > 0,"price must be great thant 1");
        require(msg.value == listingFee, "Insufficient ListingFee");
        require(
            IERC1155(nftContractAddress).balanceOf(
                msg.sender,
                tokenId
            ) >= amount,
        ...
        require(
            ...
            "ERC1155 token must be approved to marketplace contract"
        );

        _marketItemIds.increment();
        uint256 marketItemId = _marketItemIds.current();

        address creator = NFTMint(nftContractAddress).getTokenCreatorById(tokenId);
        ...
        IERC1155(nftContractAddress).safeTransferFrom(msg.sender, address(this), tokenId, amount, "");
     ...
     */
    function createMarketSale(address nftContractAddress, uint256 marketItemId, uint256 amount) public payable nonReentrant {
        uint256 price = marketItemIdToMarketItem[marketItemId].price;
        uint256 tokenId = marketItemIdToMarketItem[marketItemId].tokenId;
        require(msg.value == price * amount, "Please submit the asking price in order to continue");
        require(amount <= marketItemIdToMarketItem[marketItemId].tokenAmount, "Amount Exceeds Balance");
        ...
        IERC1155(nftContractAddress).safeTransferFrom(address(this), msg.sender, tokenId, amount, "");
        if(marketItemIdToMarketItem[marketItemId].tokenAmount - amount == 0)
        {
            marketItemIdToMarketItem[marketItemId].sold = true;
            _tokensSold.increment();
        }
        else
            marketItemIdToMarketItem[marketItemId].tokenAmount -= amount;
        ...
        emit MarketItemSale(
            ...
            tokenId,
            amount,
            marketItemIdToMarketItem[marketItemId].price * amount,
    ...
    function cancelMarketItem(address nftContractAddress, uint256 marketItemId) public nonReentrant {
        ...
        IERC1155(nftContractAddress).safeTransferFrom(address(this), msg.sender, tokenId, marketItemIdToMarketItem[marketItemId].tokenAmount, "");
    ...
    */
    ...
    function getMarketItemsByOwnerAddress(address ownerAddress) public view returns (MarketItem[] memory) {
        uint256 itemsCount = _marketItemIds.current();
        uint256 soldItemsCount = _tokensSold.current();
        uint256 canceledItemsCount = _tokensCanceled.current();
        uint256 availableItemsCount = itemsCount - soldItemsCount - canceledItemsCount;
        ...
        for (uint256 i = 0; i < itemsCount; i++) {
            ...
            MarketItem memory item = marketItemIdToMarketItem[i + 1];
            if (item.owner != ownerAddress) continue;
            marketItems[currentIndex] = item;
            currentIndex += 1;
        }
...
contract NFTMint is ERC1155, Ownable {
    ...
    // function initialize(string memory _name, string memory _symbol, address ownerAddress) public {
    //     Owner = ownerAddress;
    //     name = _name;
    //     symbol = _symbol;
    // }
    ...
    modifier isOpenForSwap() {
        require(openMint, "Mint is not allowed");
        _;
    }

    

    function getTokenByID(uint256 tokenId) public view returns (string memory) {
        return _tokenURIs[tokenId];
    }
    function getTokenByAddress(address tokenOwner) public view returns (NFT[] memory) {
        uint256 tokenCount = 0;
        for(uint256 i = 0 ; i < _tokenIds.current() ; i++) {
            uint256 tokenId = i + 1;
            if(_creators[tokenId] == tokenOwner)
                tokenCount++;
        }

        NFT [] memory ownNFT = new NFT[](tokenCount);
        uint256 currentIndex = 0;
        for(uint256 i = 0 ; i < _tokenIds.current(); i++) {
            uint256 tokenId = i + 1;
            if(_creators[tokenId] != tokenOwner) continue;
                ownNFT[currentIndex].id = tokenId;
                ownNFT[currentIndex].uri = _tokenURIs[tokenId];
                ownNFT[currentIndex].tokenAmount = balanceOf(tokenOwner, tokenId);
                currentIndex += 1;
        }
        return ownNFT;
    }

    function setMintState(bool _new) external onlyOwner {
        openMint = _new;
    }
    ...
    //    Owner = payable(ContractOwner);
    // }

    function setMarketplaceAddress(address mAddress) public onlyOwner {
        marketplaceAddress = mAddress;
    }
    ...
    function mintNFT(uint256 mintAmount, string memory tokenURI) public returns(uint256) {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _mint(msg.sender,newItemId,mintAmount,"");
