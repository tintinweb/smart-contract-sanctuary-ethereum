...
contract Squirrely is ERC721, Ownable {
    ...
    function mint(uint256 amt ) external payable {  
        ...
        require(msg.value >= MINT_PRICE * amt, "not enough ETH sent, check price");
        require(amt < 11, "You can mint a maximum of 10");
        unchecked {
            if (totalSupply + amt > MAX_SUPPLY) revert SupplyReached();
            totalSupply += amt;
            for (uint256 i; i < amt; i++) _safeMint(msg.sender, totalSupply + i);
        }
    }
