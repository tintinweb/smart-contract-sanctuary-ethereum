...
import "./SomaSwapPair.sol";

contract SomaSwapFactory is ISomaSwapFactory, Accessible {

    string public constant name = 'SomaSwapFactory';

    bytes32 public constant override FEE_SETTER_ROLE = keccak256('SomaSwapFactory.FEE_SETTER_ROLE');
    bytes32 public constant override CREATE_PAIR_ROLE = keccak256('SomaSwapFactory.CREATE_PAIR_ROLE');
    bytes32 public constant override MANAGE_ROUTER_ROLE = keccak256('SomaSwapFactory.MANAGE_ROUTER_ROLE');
    ...
    SomaContract(somaAddress) {
        INIT_CODE_HASH = keccak256(_pairBytecode());
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(ISomaSwapFactory).interfaceId ||
    ...
    function addRouter(address router) external onlyRole(MANAGE_ROUTER_ROLE) override {
        isRouter[router] = true;
        emit RouterAdded(router, _msgSender());
    }

    function removeRouter(address router) external onlyRole(MANAGE_ROUTER_ROLE) override {
        isRouter[router] = false;
        emit RouterRemoved(router, _msgSender());
    }

    function createPair(address tokenA, address tokenB) external whenNotPaused onlyRole(CREATE_PAIR_ROLE) override returns (address pair) {
        require(tokenA != tokenB, 'SomaSwapFactory: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'SomaSwapFactory: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'SomaSwapFactory: PAIR_EXISTS'); // single check is sufficient

        pair = Deployer.deploy(
            _pairBytecode(),
            keccak256(abi.encodePacked(token0, token1))
        );

        ISomaSwapPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external override onlyRole(FEE_SETTER_ROLE) whenNotPaused {
        emit FeeToUpdated(feeTo, _feeTo, _msgSender());
        feeTo = _feeTo;
    }

    function _pairBytecode() private view returns (bytes memory) {
        return abi.encodePacked(
            type(SomaSwapPair).creationCode,
            abi.encode(address(SOMA))
        );
    }
}
...
import "../../SecurityTokens/ERC20/SomaERC20Permit.sol";
import "../../SecurityTokens/ERC20/utils/ERC20Helper.sol";

contract SomaSwapPair is ISomaSwapPair, SomaERC20Permit {
    ...
    function initialize(address _token0, address _token1) external override {
        ...
        string memory _symbol = "SOMA";

        {
            string memory token0Name = token0.tryName();
            string memory token1Name = token1.tryName();
        ...
        {
            string memory token0Symbol = token0.trySymbol();
            string memory token1Symbol = token1.trySymbol();
        ...
        initialize(_name, _symbol, 'SomaSwap');
    }
    ...
    function requiredAccess() public view virtual override returns (bytes32) {
        bytes32 token0Access = GuardHelper.requiredAccess(token0);
        bytes32 token1Access = GuardHelper.requiredAccess(token1);
        return GuardHelper.mergeAccess(token0Access, token1Access, super.requiredAccess());
    }
    ...
    function swap(
        ...
        address to,
        bytes calldata data
    ) external lock onlyAccessApproved(_forwardSender()) override {
...
library GuardHelper {
    ...
    function check(bytes32 access, bytes32 query) internal pure returns (bool) {
...
contract SomaERC20Permit is ERC20PermitUpgradeable, SecurityToken {
...
abstract contract GuardableUpgradeable is IGuardable, AccessibleUpgradeable {
    ...
    modifier onlyAccessApproved(address sender) {
        require(canAccess(sender), 'required access not met');
        _;
    }
...
contract SomaContractUpgradeable is ISomaContract, ContextUpgradeable, ERC165Upgradeable {
