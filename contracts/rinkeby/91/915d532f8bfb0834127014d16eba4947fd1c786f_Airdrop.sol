...
contract Airdrop is ERC1155Supply, Ownable {
    ...
    constructor(address _minter, string memory _name, string memory _symbol, string memory _uri) ERC1155(_uri) {
        name = _name;
        symbol = _symbol;
        minter = _minter;
    }
    ...
    function burn(address from, uint256 id, uint256 amount) external {
        require(
            from == _msgSender() || isApprovedForAll(from, _msgSender()) || _msgSender() == minter,
            "ERC1155: caller is not owner nor approved"
        );
