...
contract LandManager is Ownable {
    ...
    constructor (IERC721 _landsContract) {
    ...
    ) public onlyOwnerOrLandsContract 
    {
        ...
        require(numOfSold == tokenIds.length, "LandManager: Invalid Arguments");
        LandTierInfo memory _tierInfo = landTierInfo[_landTierName];
        require(getRemainingCap(_landTierName) > numOfSold, "LandManager: Cap out of bound");

        uint256 updatedSold = _tierInfo.sold + numOfSold;
        _pushTokenIdsInTier(_landTierName, tokenIds, updatedSold);
