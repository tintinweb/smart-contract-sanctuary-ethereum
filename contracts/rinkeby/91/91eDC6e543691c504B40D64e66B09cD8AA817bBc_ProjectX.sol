...
contract ProjectX is ERC721A, Ownable {
    ...
    string public hiddenMetadataUri = "ipfs://bafkreig3mztky63vv4bi732ds4hh3fmqgo7pmrouowxs6wz7wuasl5m2rq/";
    address public beneficiary;
    uint256 public publicPrice = 0.07 ether;
    uint256 public allowListPrice = 0.05 ether;
    uint256 public maxSupply = 7777;
    uint256 public teamSupply = 777;
    ...
    constructor(address _beneficiary) ERC721A("Project X", "PX") {
    ...
    modifier mintChecks(uint256 _mintAmount) {
        uint256 price = isAllowListActive ? allowListPrice : publicPrice;
    ...
    function _verify(bytes32 leaf, bytes32[] memory proof, bytes32 _merkleRoot)
        internal
        pure
        returns (bool)
    {
    ...
    function setMerkleRoots(bytes32 _allowListMerkleRoot, bytes32 _complimentaryMintListMerkleRoot)
        external
        onlyOwner
    {
    ...
    function mint(uint8 _mintAmount) public payable mintChecks(_mintAmount) {
        require(!isAllowListActive, "Allow List is still active");
    ...
    {
        require(isAllowListActive, "Allow List is paused");
        require(_verify(_leaf(msg.sender), proof, allowListMerkleRoot), "Invalid proof");

        _safeMint(msg.sender, _mintAmount);
    }

    function mintComplimentary(bytes32[] calldata proof)
        public
        payable
    {
        require(!paused, "The contract is paused");
        require(_complimentaryMintClaimed[msg.sender] == false, "Complimentary mint already claimed");
        require(_verify(_leaf(msg.sender), proof, complimentaryMintListMerkleRoot), "Invalid proof");
        require(_totalMinted() + 1 <= maxSupply, "Supply exhausted");
        _safeMint(msg.sender, 1);
        _complimentaryMintClaimed[msg.sender] = true;
    }

    function mintComplimentaryClaimed()
        public
        view
        returns (bool)
    {
    ...
    function mintWithPartner(uint8 _mintAmount, address partner)
        public
        payable
        mintChecks(_mintAmount)
    {
        ...
        _safeMint(msg.sender, _mintAmount);
        //Pay out partner commission
        (bool os, ) = payable(partner).call{
            value: (publicPrice * _mintAmount * 15) / 100
        }("");
        require(os);
    }
