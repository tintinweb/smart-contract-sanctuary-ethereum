...
interface OpenerMintInterface is IERC721 {
    ...
    function setMarketplaceDisForNFT(uint256 _tokenId, uint256[] memory _marketplaceDistributionAmounts, 
...
contract NFTid is Ownable, ERC721URIStorage , OpenerMintInterface, NFTidInterface {
    ...
    function setMarketplaceDisForNFT(uint256 _tokenId, uint256[] memory _marketplaceDistributionAmounts, 
    address[] memory _marketplaceDistributionAddresses) external override {
        require(msg.sender == _admin || tx.origin == _admin, "Not authorized"); // can be called from this contract or from the opener contract
    ...
    function setLastNFTID(uint256 newId) external override onlyOpener() {
        require(newId > lastNFTID, "Wrong value");
        lastNFTID = newId;
    }
