		...
		IERC1155ReceiverUpgradeable
{
    ...
    ///         deducting royalties
    function _deduceRoyalties721(uint256 tokenId, uint256 grossSaleValue)
        internal
        returns (uint256 netSaleAmount)
    {
        ...
        if (royaltiesAmount > 0) {
					(bool royaltySent,)=royaltiesReceiver.call{value:royaltiesAmount}("");
					require(royaltySent,"Royalty Sending Failed");
        }
		...
		function _deduceRoyalties1155(uint256 tokenId, uint256 grossSaleValue)
        internal
        returns (uint256 netSaleAmount)
    {
        // Get amount of royalties to pays and recipient
		...
		function finaliseERC721Auction(uint256 tokenId,address buyer,address seller,uint256 bidAmount) public payable OnlyAdmin returns(bool){
			uint256 feeAmt = (bidAmount * feePercent) /
            (10000 + feePercent);
        uint256 netBidAmount = bidAmount - feeAmt;

        uint256 netSalePrice = _deduceRoyalties721(
            tokenId,
            netBidAmount
        );
				(bool feesSent,) = feeAddress.call{value:feeAmt}("");
				require(feesSent,"Fee Transfer Failed");

				//Transfer amount to seller after deducting loyalties
				(bool sellerSent,)=seller.call{value:netSalePrice}("");
        ...
        tokenContract721.safeTransferFrom(
            ...
            tokenId
				...
				emit LogOrderFinalised721(tokenId,buyer,seller);
				return true;
		}

		function finaliseERC1155Auction(uint256 tokenId,address buyer,address seller,uint256 bidAmount,uint256 quantity) public payable OnlyAdmin returns(bool){
			uint256 feeAmt = (bidAmount * feePercent) /
            (10000 + feePercent);
        uint256 netBidAmount = bidAmount - feeAmt;
        ...
        uint256 netSalePrice = _deduceRoyalties721(
            ...
            netBidAmount
				...
				string memory uri = tokenContract1155.uri(tokenId);
        ...
        tokenContract1155.safeTransferFrom(
            ...
            tokenId,
				...
				emit LogOrderFinalised1155(tokenId,buyer,seller,quantity);
				return true;
		}
    ...
    function onERC721Received(
		...
		function supportsInterface(bytes4 interfaceId) public override(AccessControlUpgradeable,IERC165Upgradeable) view returns (bool){
        ...
        returns (uint256 tokenId)
    {
		...
		function distributeTokens(address[] memory transferAddr,uint256[] memory ids,uint256[] memory quantity) public returns(bool){
			...
			for(uint i =0; i<transferAddr.length;i++){
				...
				require(balanceOf(address(this), ids[i])>=quantity[i],"Insufficient balance");
				string memory tokenuri = uri(ids[i]);
				safeTransferFrom(address(this), transferAddr[i], ids[i], quantity[i], bytes(tokenuri));
			}
			return true;
		}
}
    ...
    GenericAccessControl
{
    ...
    function redeem(address redeemer, NFTVoucher calldata voucher,uint256 tokenId)
        internal
        returns (uint256)
    {
