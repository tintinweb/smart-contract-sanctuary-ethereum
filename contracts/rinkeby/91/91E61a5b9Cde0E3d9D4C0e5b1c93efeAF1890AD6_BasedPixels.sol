...
contract BasedPixels is ERC721, Ownable {
    ...
    function getSlices(uint256 tokenId) public view returns (uint256[12] memory) {
        ...
        uint256[] memory slices = new uint256[](36);
        for (uint i = 0; i < 12; i++) {
            ...
            slices[i+24] = (_randomRoots[mintBatchRootID+(tokenId%10)+2]^_slices[mintBatchRootID+i+2])^(_randomRoots[mintBatchRootID+12]^_slices[mintBatchRootID+14]);
            if ((_slices[mintBatchRootID+15]>>((tokenId%10)*25))%3 == 0) {
                slices[i] = slices[i]^slices[i+12];
            }
            else if ((_slices[mintBatchRootID+15]>>((tokenId%10)*25))%3 == 1) {
                slices[i] = slices[i]&slices[i+12];
            }
            else {
                slices[i] = slices[i]|slices[i+12];
            }
            if ((_randomRoots[mintBatchRootID+13]>>((tokenId%10)*25))%3 == 0) {
                slices[i] = slices[i]^slices[i+24];
            }
            else if ((_randomRoots[mintBatchRootID+13]>>((tokenId%10)*25))%3 == 1) {
                slices[i] = slices[i]&slices[i+24];
            }
            else {
                slices[i] = slices[i]|slices[i+24];
    ...
    function contractURI() public view returns (string memory) {
        return string(
            abi.encodePacked(
                "data:application/json;utf8,{\"name\":\"Based Pixels Testing 64\",",
