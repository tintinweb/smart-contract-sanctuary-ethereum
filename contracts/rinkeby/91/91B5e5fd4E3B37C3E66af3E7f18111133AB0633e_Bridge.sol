...
contract Bridge is ReentrancyGuard, AccessControl {
    ...
    event TokensUpdated(address indexed token, bool status);
    ...
    function swap(
        ...
        require(
            supportedChains[chainFrom] == true &&
    ...
    function redeem(
        ...
        bytes32 signedDataHash = keccak256(
            abi.encodePacked(receiver, token, amount, nonce, time)
        );
    ...
    function updateTokens(address token, bool status) external adminControl {
        supportedTokens[token] = status;
        emit TokensUpdated(token, status);
    }
