...
abstract contract CTokenInterface is CTokenStorage {
    ...
    function getAccountSnapshot(address account) virtual external  returns (uint, uint, uint, uint);
...
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter {
    ...
    function getAccountSnapshot(address account) override external returns (uint, uint, uint, uint) {
        return (
            NO_ERROR,
    ...
    function exchangeRateStoredInternal() virtual internal view returns (uint) {
        ...
        if (_totalSupply == 0) {
            ...
            // emit Event(totalCash, cashPlusBorrowsMinusReserves);
