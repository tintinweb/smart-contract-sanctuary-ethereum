...
import {NounletMinter} from "./NounletMinter.sol";
import {NFTReceiver} from "../utils/NFTReceiver.sol";
import {ReentrancyGuard} from "@rari-capital/solmate/src/utils/ReentrancyGuard.sol";
import {SafeSend} from "../utils/SafeSend.sol";

import {INounletAuction, Auction, Vault} from "./interfaces/INounletAuction.sol";
    ...
    SafeSend
{
    ...
    constructor(address _registry, address _supply) NounletMinter(_supply) {
        registry = _registry;
    }
    ...
    function createAuction(
        ...
        if (vaultInfo[_vault].currentId == 0) {
            _create(_vault, _mintProof);
        }
    }
    ...
    {
        _settle(_vault);
        if (vaultInfo[_vault].currentId < TOTAL_SUPPLY) {
            _create(_vault, _mintProof);
        }
    }
    ...
    function bid(address _vault) external payable nonReentrant {
        ...
        if (msg.value < ethAmount + ((ethAmount * MIN_INCREASE) / 100))
            revert InvalidBidIncrease();

        address payable previousBidder = payable(auction.bidder);
        if (previousBidder != address(0)) {
            _sendEthOrWeth(previousBidder, ethAmount);
        }
        ...
        if (endTime < extendedTime) {
            auction.endTime = uint32(extendedTime);
        }

        auction.amount = uint64(msg.value);
        auction.bidder = msg.sender;

        emit Bid(_vault, token, id, msg.sender, msg.value);
    }

    function withdraw(address _vault, uint256 _id) external {
        address winner = auctionInfo[_vault][_id].bidder;
        uint32 endTime = auctionInfo[_vault][_id].endTime;
        if (msg.sender != winner || uint256(endTime) < block.timestamp)
            revert NotWinner();

        address token = INounletRegistry(registry).vaultToToken(_vault);
        INounletToken(token).safeTransferFrom(
            address(this),
            msg.sender,
            _id,
            1,
    ...
    /// @param _vault Address of the vault
    function _create(address _vault, bytes32[] calldata _mintProof) internal {
        uint256 id = vaultInfo[_vault].currentId;
        _mintFraction(_vault, address(this), id, _mintProof);

        address token = INounletRegistry(registry).vaultToToken(_vault);
        uint256 endTime = block.timestamp + DURATION;
        auctionInfo[_vault][id].endTime = uint32(endTime);
        auctionInfo[_vault][id].bidder = msg.sender;
        vaultInfo[_vault].curator = msg.sender;

        emit Created(_vault, token, id, msg.sender, endTime);
    }

    /// @notice Settles an ended auction
    /// @param _vault Address of the vault
    function _settle(address _vault) internal {
        uint256 id = vaultInfo[_vault].currentId;
        Auction storage auction = auctionInfo[_vault][id];

        require(auction.endTime != 0, "Auction hasnt begun");
        // require(!auction.settled, "Auction has already been settled");
        require(block.timestamp >= auction.endTime, "Auction hasnt completed");

        address token = INounletRegistry(registry).vaultToToken(_vault);

        if (auction.bidder.code.length == 0) {
            INounletToken(token).safeTransferFrom(
                address(this),
                auction.bidder,
                id,
        ...
        (address royaltyAddress, uint256 royaltyAmount) = INounletToken(token)
            .royaltyInfo(id, auction.amount);

        _sendEthOrWeth(royaltyAddress, royaltyAmount);

        ++vaultInfo[_vault].currentId;

        emit Settled(_vault, token, id, auction.bidder, auction.amount);
    }
}
...
import {INounletMinter} from "./interfaces/INounletMinter.sol";

/// @title NounMod
/// @author Fractional Art
/// @notice Module contract for setting permissions need by NounVault
contract NounletMinter is INounletMinter {
