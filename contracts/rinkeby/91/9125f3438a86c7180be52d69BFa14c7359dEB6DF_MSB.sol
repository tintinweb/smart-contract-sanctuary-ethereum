...
contract MSB is ERC721A, Ownable  {
    ...
    uint256 constant public WL_PRICE = 0.009 ether;
    uint256 constant public PRICE = 0.011 ether;
    uint256 constant public NAME_CHANGE_PRICE = 0.001 ether;
    uint256 public giveawayLimit = 3;
    ...
    function whitelistMint(uint256 numberOfTokens, bytes32[] calldata merkleProof ) payable external callerIsUser {
        ...
        require(maxMint[msg.sender].add(numberOfTokens) <= 10,"Max 10 mints allowed per whitelisted wallet");
    ...
    function mint(uint256 numberOfTokens) external payable callerIsUser {
        ...
        require(maxMint[msg.sender].add(numberOfTokens) <= 10,"Max 10 mints allowed per wallet");
    ...
    function setName(uint256 tokenId, string memory newName) public {
        ...
        require(bytes(newName).length > 0 && bytes(newName).length <= 50,"Name exceeds allowed characer limit");
    ...
    function upadateOwnerWallet(address _wallet) external onlyOwner {
        ownerWallet = _wallet;
    }
