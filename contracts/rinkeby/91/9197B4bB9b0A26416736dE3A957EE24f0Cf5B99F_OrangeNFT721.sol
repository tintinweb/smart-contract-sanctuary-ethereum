...
// File: OrangeNFT721.sol

contract OrangeNFT721 is ERC721URIStorage, Ownable {

    event SetManager(address oldManager, address newManager);
    event SetSetter(address oldSetter, address newSetter);
    ...
    modifier onlySetter() {
        require(msg.sender == setter || msg.sender == owner(), "must be setter address");
        _;
    }

    constructor (string memory name_, string memory symbol_, address manager_, address setter_) ERC721(name_, symbol_) {
        manager = manager_;
        setter = setter_;
    ...
    function setTokenURI(uint256 tokenId, string memory uri) public onlySetter {
