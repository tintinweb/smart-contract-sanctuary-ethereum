...
contract NFTToken is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function changeTokenUri(uint256 tokenId, bytes memory newUri) public onlyOwner {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");

        metaHash[tokenId] = newUri;
    }
    ...
    function _baseURI(uint256 tokenId) internal view virtual returns (string memory) {
        return string(abi.encodePacked(metaHash[tokenId]));
    }
...
abstract contract ERC721Enumerable is NFTToken, IERC721Enumerable {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256) {
        require(index < NFTToken.balanceOf(owner), "ERC721Enumerable: owner index out of bounds");
