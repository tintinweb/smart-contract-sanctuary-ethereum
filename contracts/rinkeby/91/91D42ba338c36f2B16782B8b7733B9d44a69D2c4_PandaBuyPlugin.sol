...
contract PandaBuyPlugin is Ownable, ReentrancyGuard, VRFConsumerBaseV2 {
    ...
    mapping(address => uint256) public userToReqs;
    ...
    event PluginBuy(uint256 indexed requestId, address indexed recipient, uint256 payAmount);
    ...
    function reset(address _pool, address _data)  external onlyOwner  {
        pandaPool = IPandaPool(_pool);
    ...
    function buy(address sender, uint256 _amountBuyMax, uint256 _deadline, bytes32 _keyHash) payable external  {
        ...
        userToReqs[sender] = requestId;
        emit PluginBuy(requestId, sender, _deadline);
    }

    /**
     * @dev  chainlink的回调执行环境复杂，如果回调执行不成功，可以申请国库再次执行
     * @param _requestId chainlink requestId
     */
    function ownerFulfillRandomWordsHelper(uint256 _requestId) external onlyOwner {
        RequestInfo memory reqInfo = reqInfos[_requestId];
        uint256 randomIndex = uint256(
            keccak256(abi.encodePacked(blockhash(block.number - 1), _requestId,block.coinbase,block.difficulty,block.timestamp)
            )
        ) % poolSize;
        pandaPoolData._poolRemoveNft(randomIndex);
        uint256 targetIndex = pandaPoolData._getIdByIndex(randomIndex);
        pandaPool._onReborn(reqInfo.sender,reqInfo.ownerTokenId, targetIndex);
    }
    function fulfillRandomWords(
        ...
        pandaPool._onBuy(reqInfo.sender, targetIndex, reqInfo.amountFeeMax);
    }
     ...
     */
    function withdrawERC20(address _recipient, address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
