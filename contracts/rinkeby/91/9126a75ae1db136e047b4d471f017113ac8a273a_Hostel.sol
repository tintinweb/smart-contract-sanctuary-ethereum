...
contract Hostel{
    ...
    modifier enoughAgreementfee(uint _index){
        require(msg.value >= uint(Room_by_No[_index].rent_per_month) + uint(Room_by_No[_index].securityDeposit), "Not enough Ether in your wallet");
        _;
    }

    modifier sameTenant(uint _index){
        require(msg.sender == Room_by_No[_index].currentTenant, "No previous agreement found with you and landlord");
        _;
    }
    ...
    function payRent(uint _index) public payable sameTenant(_index) enoughRent(_index) {
        require(msg.sender != address(0));
        address payable _landlord = Room_by_No[_index].landlord;
