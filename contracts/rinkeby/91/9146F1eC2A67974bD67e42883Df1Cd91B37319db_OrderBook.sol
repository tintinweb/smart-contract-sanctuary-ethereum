...
contract OrderBook {
    ...
    mapping(address => address[]) public tradePartners; //Stores active dataStreams between buyer and seller, so that no multiple streams are possible
    ...
    event sendData(
        ...
        address buyer,
        uint256 time
    ); //event are hashed information which can be read outside of the blockchain
    ...
    function InternalremoveSellOder(uint256 _index) internal {
        ...
        require(copy.addressOf != address(0), "no such order"); // not exists
    ...
    function priceMatching(uint256 _limitPrice, uint256 _amount)
        internal
        returns (uint256)
    {
        uint256 units = 0;
    ...
    function addBuyOrder(uint256 _amount, uint256 _limitPrice) public {
        ...
        if (sellList.length == 0) {
            ...
            rest = priceMatching(newOrder.limitPrice, newOrder.amount);
        }
        if (rest <= 0) {
    ...
    function removeBuyOder(uint256 _index) public {
        ...
        require(
            ...
            "place and order before you can remove it"
        );
    ...
    function getBook() public view returns (Order[] memory) {
        return sellList;
    }
}