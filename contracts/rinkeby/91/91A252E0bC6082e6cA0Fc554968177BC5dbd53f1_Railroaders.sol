...
/**
 * @title Railroaders
 * @notice Some cool Railroaders.
 */
contract Railroaders is ERC721Royalty, Ownable {
  ...
  constructor(
    ...
    require(_nftPrice > 0, "RRNFT/price-gt-zero");
    require(_maxNFT > 0, "RRNFT/max-nft-gt-zero");
    require(_maxMint > 0, "RRNFT/max-mint-gt-zero");
  ...
  /**
   * @notice Set some Railroaders aside
   */
  ...
  function mintNFT(uint256 _numberOfTokens) external payable {
    ...
    require(_saleIsActive,"RRNFT/sale-inactive");

    uint256 _totalSupply = totalSupply;

    require(_totalSupply + _numberOfTokens <= maxNFT, "RRNFT/nfts-sold-out");
    require(_numberOfTokens <= maxMint, "RRNFT/exceeds-max-mint");

    uint256 _amount = _numberOfTokens * nftPrice;
    require(_amount == msg.value, "RRNFT/insufficient-funds");
  ...
  function withdraw(uint256 _amount) external onlyOwner {
    require(_amount > 0, "RRNFT/withdraw-amount-gt-zero");

    (bool _success, ) = msg.sender.call{ value: _amount }("");

    require(_success, "RRNFT/failed-to-withdraw-eth");
  }
