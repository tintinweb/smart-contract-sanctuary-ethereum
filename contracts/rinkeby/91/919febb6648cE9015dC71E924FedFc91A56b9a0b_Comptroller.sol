...
abstract contract CNFT1155Interface is CNFT1155Storage {
    ...
    function liquidateBorrow(address borrower, uint repayAmount, CNFTInterface cTokenCollateral) virtual  external returns (uint);
...
contract CNFTDelegationStorage {
    /**
...
abstract contract CNFTDelegatorInterface is CNFTDelegationStorage {
    /**
...
abstract contract CNFTDelegateInterface is CNFTDelegationStorage {
    /**
...
interface ICNFT {
    struct NFTData {
        address owner;
        bool isCollateralized; //true if surrendered to platform
        uint256 blocknumber; //surrendered or redeemed timestamp
        uint256 count; //for erc1155 contracts.
    }
    struct NftIdCount {
        uint tokenId;
        uint tokenCount;
    ...
    function getOwnersNftIdCount(address _account) external view returns (NftIdCount[] memory);
    function isValidTokenToMint(address _account, uint _tokenId) external view returns (bool);
    function mint(uint256 _tokenId, uint _count) external;
    function redeemUnderlyingNFT(address _account, uint256 _tokenId, uint256 _count) external;
...
abstract contract Comptroller_new {

}

abstract contract InterestRateModel_new {
...
abstract contract CToken  {
     ...
     * @notice Contract which oversees inter-cToken operations
     */
    Comptroller_new public comptroller;

    /**
     * @notice Model which tells what the current interest rate should be
     */
    InterestRateModel_new public interestRateModel;
...
// import "./EIP1155Interface.sol";
import "./SwapDaoMainInterface.sol";
// import "./NFTInterfaces.sol";
...
contract Comptroller is ComptrollerNFTStorage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when a collateral factor is changed by admin
    event NewNFTCollateralFactor(address collectionAddress, uint oldCollateralFactorMantissa, uint newCollateralFactorMantissa);
    ...
    SwapDaoMainInterface public sdm;

    //TODO: while creating the comptroller, pass address of mainswapdao contract
    //@notice: collectionType can be "ERC721" or "ERC1155"
    constructor(
        address swapDao, address _underlyingNftAddress,
        string memory _collectionName, string memory _collectionSymbol,
        string memory _collectionType, uint _nftCollateralFactor,
        address _cNFTAddress) {
            admin = msg.sender;
            sdm = SwapDaoMainInterface(swapDao);
            collectionAddress = _underlyingNftAddress;
            collectionName = _collectionName;
            collectionSymbol = _collectionSymbol;
            cNFT = ICNFT(_cNFTAddress);
            collectionType = _collectionType;
            nftCollateralFactorMantissa = _nftCollateralFactor;
    }
    ...
    function addToMarketInternal(CToken cToken, address borrower) internal returns (Error) {
        ...
        //TODOO: Add nft contract detials in comptroller. ie. address, name, symbol.
        //TODO: check for the nft contract details, it should match with that of
        //given in comptroller
    ...
    function redeemNFTAllowed(uint256 _tokenId, address redeemer, uint256 _count) external override returns (uint) {
        /* Otherwise, perform a hypothetical liquidity check to guard against shortfall */
        (Error err, , uint shortfall) = getHypotheticalAccountLiquidityInternal_nft(redeemer, _tokenId, _count);
     ...
     * @param borrowerAmount amount to borrow
    ...
    function borrowAllowed(address cToken, address borrower, uint borrowerAmount)
        ...
        //console.log('Testcount comptroller:2', testCount++);
        ...
        //console.log('Testcount comptroller:31', testCount++);

        //TODO: get the nft token price from oracle.
        //oracle.getUnderlyingPriceOfNftToken(tokenId);

        //console.log('=> underlyingPrice from oracle :', oracle.getUnderlyingPrice(CToken(cToken)));
        if (oracle.getUnderlyingPrice(CToken(cToken)) == 0) {
            return uint(Error.PRICE_ERROR);
        }

        //console.log('Testcount comptroller:41', testCount++);

        uint borrowCap = borrowCaps[cToken];
        // Borrow cap of 0 corresponds to unlimited borrowing
        if (borrowCap != 0) {
            uint totalBorrows = CToken(cToken).totalBorrows();
            uint nextTotalBorrows = add_(totalBorrows, borrowerAmount);
            require(nextTotalBorrows < borrowCap, "market borrow cap reached");
        }
        //console.log('Testcount comptroller:51', testCount++);

        (Error err, , uint shortfall) = getHypotheticalAccountLiquidityInternal(borrower, CToken(cToken), 0, borrowerAmount);
        //console.log('Testcount comptroller:61', testCount++, shortfall);
        ...
        //console.log('Testcount comptroller:71', testCount++);
        ...
        //console.log('Testcount comptroller:81', testCount++);
    ...
    //specific for nfts
    function getHypotheticalAccountLiquidityInternal_nft(
            address account,
            uint256 _tokenId,
            uint256 _count) internal view returns (Error, uint, uint){
        ...
        // console.log('=> nft_Final - Comptroller:getHypotheticalAccountLiquidityInternal:vars.sumCollateral - before adding nftprice',vars.sumCollateral);
        // console.log('User tokenIds with platform :', borrowerTokenIds[account].length);
        //find all the tokenIds collateralized on the platform for a particular borrower
        ICNFT.NftIdCount[] memory borrowerTokenIdCounts = cNFT.getOwnersNftIdCount(account);
        for (uint256 index = 0; index < borrowerTokenIdCounts.length; index++) {
            // Get the normalized price of the nft token
            uint256 tokenId = borrowerTokenIdCounts[index].tokenId;
            uint256 tokenCount = borrowerTokenIdCounts[index].tokenCount;
                vars.oracleNftPriceMantissa = oracle.getUnderlyingPriceOfNftToken(tokenId);
                vars.oracleNftPrice = Exp({mantissa: vars.oracleNftPriceMantissa});
            if(tokenId != _tokenId){
                //TODO: test once more
                vars.sumCollateral = mul_ScalarTruncateAddUInt(
                        mul_(vars.oracleNftPrice, tokenCount),
                        nftCollateralFactorMantissa,
                        vars.sumCollateral);
            } else {
                if(tokenCount >= _count){
                    vars.sumCollateral = mul_ScalarTruncateAddUInt(
                        mul_(vars.oracleNftPrice, (tokenCount-_count)),
                        nftCollateralFactorMantissa,
                        vars.sumCollateral);
                }
            }
        ...
        //log('=> nft_Final - Comptroller:getHypotheticalAccountLiquidityInternal:vars.sumCollateral - after adding nftPrice',
        //    vars.sumCollateral);
        //console.log('=> nft_Final - Comptroller:getHypotheticalAccountLiquidityInternal:vars.sumBorrowPlusEffects', vars.sumBorrowPlusEffects);
    ...
    function getHypotheticalAccountLiquidityInternal(
       ...
       AccountLiquidityLocalVars memory vars; // Holds all our calculation results
        ...
        // For each asset the account is in
        CToken[] memory assets = accountAssets[account];
        //console.log('======> CToken assets :', assets.length);
        //console.log(' ===> cTokenModify:', cTokenModify.name());
        for (uint i = 0; i < assets.length; i++) {
            CToken asset = assets[i];
            ...
            //console.log('=> comptroller:collateralFactor:', markets[address(asset)].collateralFactorMantissa);
            //console.log('=> comptroller:exchangeRateMantissa:', vars.exchangeRateMantissa);
            //console.log('=> comptroller:oraclePriceMantissa:', vars.oraclePriceMantissa);
            // //console.log('=> comptroller:tokensToDenom:', vars.tokensToDenom);
            //console.log('=> comptroller:sumCollateral:', vars.sumCollateral);
            //console.log('=> comptroller:sumBorrowPlusEffects:', vars.sumBorrowPlusEffects);
        ...
        //console.log('=> Final - Comptroller:getHypotheticalAccountLiquidityInternal:vars.sumCollateral - before adding nftprice',
        //    vars.sumCollateral);
        // console.log('User tokenIds with platform :', borrowerTokenIds[account].length);
        //find all the tokenIds collateralized on the platform for a particular borrower
        ICNFT.NftIdCount[] memory borrowerTokenIdCounts = cNFT.getOwnersNftIdCount(account);
        ...
        // These are safe, as the underflow condition is checked first
        //console.log('=> Final - Comptroller:getHypotheticalAccountLiquidityInternal:vars.sumCollateral - after adding nftPrice',
        //    vars.sumCollateral);
        //console.log('=> Final - Comptroller:getHypotheticalAccountLiquidityInternal:vars.sumBorrowPlusEffects', vars.sumBorrowPlusEffects);
    ...
    function _setNftCollateralFactor(uint newCollateralFactorMantissa) external returns(uint){
        ...
        uint oldCollateralFactorMantissa = nftCollateralFactorMantissa;
        nftCollateralFactorMantissa = newCollateralFactorMantissa;

        // Emit event with asset, old collateral factor, and new collateral factor
        emit NewNFTCollateralFactor(collectionAddress, oldCollateralFactorMantissa, newCollateralFactorMantissa);
    ...
    function _supportMarket(CToken cToken) external returns (uint) {
        ...
        Market storage mkt = markets[address(cToken)];
        mkt.isListed = true;
        mkt.isComped = false;
        mkt.collateralFactorMantissa = 0;
...
abstract contract ComptrollerNFTStorage is ComptrollerV7Storage, CErc1155Storage {

    struct NFTData {
        address owner;
        bool isCollateralized; //true if surrendered to platform
        uint256 blocknumber; //surrendered or redeemed timestamp
        uint256 value; //for erc1155 contracts.
    }
    /**
    @notice update the tokenId being collateralized in platform
    */
    mapping(uint256 => NFTData) public nftTokenIds;

    //list of tokenIds of each user
    mapping(address => uint256[]) public borrowerTokenIds;

    //collateral factor of complete collection
    uint public nftCollateralFactorMantissa; //scaled to 1e18

    //collection type "ERC721" or "ERC1155"
    string public collectionType;

    //address of cNFT token.
    ICNFT public cNFT;
}
...
contract ComptrollerErrorReporter {
    enum Error {
        ...
        TOO_MANY_ASSETS,
        TOO_MUCH_REPAY,
        NFT_TOKEN_ALREADY_COLLATERALIZED,
        NFT_PRICE_ERROR,
        NFT_TOKEN_NOT_VALID
    }
...
contract TokenErrorReporter {
    ...
    enum FailureInfo {
        ...
        NFT_TOKEN_MUST_SUPPLIED_FOR_BORROW,
...
// interface ICNFT1155 is ERC1155 {
//     function mint(address _account, uint256 _tokenId, uint _count) external;
//     function burn(uint256 id,uint256 amount) external;
...
import "./CTokenInterfaces_used.sol";
...
abstract contract PriceOracle {
    ...
    function getUnderlyingPriceOfNftToken(uint _tokenId) public virtual view returns (uint);
}
...
interface SwapDaoMainInterface {

    /// @notice owner can deactives any nft market.
    function removeNftMarket(address collectionAddress_) external;
