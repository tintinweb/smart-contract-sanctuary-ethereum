...
contract ChainlinkSwap is BaseSwap {
    ...
    /// @param _amountIn Amount of tokens user want to give for swap
    ...
    function swap(
        ...
        if (_from == dexData.tokenA) {
            ...
            uint256 amountB = (normalizeWhenA(amountA) * getChainLinkFeedPrice()) / (10**8);
            ...
            uint256 amountA = (normalizeAmount(amountB, _from, _to) * (10**8)) / getChainLinkFeedPrice();
    ...
    {
        require(
            (normalizeWhenA(amountA) * getChainLinkFeedPrice()) / (10**8)  == amountB,
...
abstract contract BaseSwap is Ownable, ISwap, Pausable {
    ...
    function normalizeWhenA(uint _amountA) internal view returns(uint256) {
        return normalizeAmount(_amountA, dexData.tokenA, dexData.tokenB);
    }
