...
contract CocoMarketplaceV2 is Ownable {
    ...
    event PurchaseWL (uint256 indexed _id, address indexed _address);
    event PurchaseWithNameWL (uint256 indexed _id, address indexed _address, string _name);
    event EnterRaffle (uint256 indexed _id, address indexed _address, uint256 _amount);
    event EnterRaffleWithName (uint256 indexed _id, address indexed _address, uint256 _amount, string _name);

    constructor() { 
        // cocoAddress = 0x133B7c4A6B3FDb1392411d8AADd5b8B006ad69a4; // mainnet
        cocoAddress = 0x562DEa9BE18FbfABfFA28Ac7Ea2Be511C2A2ED9B; // rinkeby
        whitelistCounter = 27;
        whitelistTimer = 60 * 60 * 24;
        raffleTimer = whitelistTimer;
    }

    function enterRaffle(uint256 _id, uint256 _amount) public {
        require(
        ...
        emit EnterRaffle(_id, msg.sender, _amount);
    }

    function enterRaffleWithName(uint256 _id, uint256 _amount, string memory _name) public {
        require(
            block.timestamp < raffles[_id].endTime,
            "Raffle ended."
        );
        ...
        ICoco(cocoAddress).burnFrom(msg.sender, raffles[_id].price * _amount);
        emit EnterRaffleWithName(_id, msg.sender, _amount, _name);
    }
    ...
    function purchase(uint256 _id) public {
       ...
       require(
        ...
        emit PurchaseWL(_id, msg.sender);
    }

    function purchaseWithName(uint256 _id, string memory _name) public {
        require(
            block.timestamp > whitelists[_id].timestamp,
            "Not live yet."
        );
       ...
       require(
        ...
        emit PurchaseWithNameWL(_id, msg.sender, _name);
    }
    ...
    function editWhitelist(uint256 _id, uint256 _amount, uint256 _price) external onlyOwner {
        whitelists[_id].amount = _amount;
        whitelists[_id].price = _price * 10 ** 18;
    }

    function editRaffle(uint256 _id, uint256 _price, bool _capped) external onlyOwner {
