...
/**
 * @title TheTrade's TErc20 Contract
...
contract TErc20 is TToken, TErc20Interface {
    ...
    function borrow(uint256 borrowAmount, uint256 positionId)
        external
        override(TErc20Interface, ITToken)
        onlyRouter(_msgSender())
        returns (uint256)
    {
        borrowInternal(borrowAmount, positionId);
        return NO_ERROR;
    }
    ...
    function repayBorrow(uint256 repayAmount, uint256 positionId)
        ...
        returns (uint256)
    {
        repayBorrowInternal(positionId);
        return NO_ERROR;
    }
    ...
    function repayBorrowBehalf(
        ...
        uint256 repayAmount,
        uint256 positionId
    ) external override onlyRouter(_msgSender()) returns (uint256) {
        repayBorrowBehalfInternal(borrower, positionId, repayAmount);
        return NO_ERROR;
    }
     ...
     */
    function sweepToken(EIP20NonStandardInterface token)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            ...
            "TErc20::sweepToken: can not sweep underlying token"
        );
