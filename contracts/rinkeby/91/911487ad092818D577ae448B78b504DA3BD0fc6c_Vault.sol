  ...
  OwnableUpgradeSafe
{
  ...
  function initialize(IVaultConfig _config, address _token)
    external
    initializer
  {
    ...
    nonReentrant
  {
    ...
    if (id == 0) {
      ...
      healthBefore = IWorker(worker).health(id);
    ...
    uint256 debt = 0;
    // 2. Perform the actual work, using a new scope to avoid stack-too-deep errors.
    uint256 back;
    {
      uint256 sendERC20 = principalAmount;
      require(
    ...
    // 3. Release execution scope
    POSITION_ID = _NO_ID;
    STRATEGY = _NO_ADDRESS;
    // 4. Return excess token back.
    if (back > 0) {
      _safeUnwrap(msg.sender, back);
    }
    emit Work(id);
  }
  ...
  /// @dev Fallback function to accept Native Token.
