    ...
    IERC721Receiver
{
    ...
    constructor(
        ...
        WyvernEscrowBuyer(_nftExchangeAddress)
    {
        marketId = _marketId;
    ...
    function executeAsBytes(
        ...
        AtomicMatchInputs memory _atomicMatchInputs,
        bytes memory _signature) = abi.decode(_encodedOrderInput, (SubmitBidArgs, AtomicMatchInputs, bytes));

        _execute(_submitBidArgs, _atomicMatchInputs, _signature);
    ...
    */
    ...
    function _execute( 
        ...
        require(
         ...
         address paymentToken = _atomicMatchInputs.addrs[6];
        require(paymentToken == ETH_ADDRESS, "Payment token must be ETH");
      

        uint256 bidId = _submitAssetPurchaseUsingLoan( _submitBidArgs, _atomicMatchInputs  );

      
        TokenType tokenType = fetchTokenTypeFromCalldataMethodIdentifier( bytes4(_atomicMatchInputs.calldataSell) );

        (
            address assetContractAddress,
            uint256 assetTokenId,
            uint256 quantity
        ) = fetchTokenInfoFromCalldata(_atomicMatchInputs,tokenType);
        ...
        _verifySignature(
            getTypeHash(
                ...
                assetTokenId,
                quantity,
                _atomicMatchInputs.uints[4], //base price
                paymentToken), 
            _signature);
    ...
    function _submitAssetPurchaseUsingLoan(
        ...
        uint256 ethUsedForPurchase = _purchaseNFT(_atomicMatchInputs);
    ...
    */
    ...
    function _purchaseNFT(AtomicMatchInputs memory _atomicMatchInputs)
        internal
        virtual
        returns (uint256 ethUsedForPurchase)
    {
        ...
        uint256 basePrice = _atomicMatchInputs.uints[4];
        ...
        atomicMatchThrough_(_atomicMatchInputs, basePrice);
...
contract WyvernEscrowBuyer {
    ...
    ///try to change from memory to calldata

    function atomicMatchThrough_(
        AtomicMatchInputs memory inputs,
        uint256 basePrice
    ) internal virtual {
        return
            IExchangeCore(exchange).atomicMatch_{ value: basePrice }(
                inputs.addrs,
                inputs.uints,
                inputs.feeMethodsSidesKindsHowToCalls,
                inputs.calldataBuy,
                inputs.calldataSell,
                inputs.replacementPatternBuy,
                inputs.replacementPatternSell,
                new bytes(0),
                new bytes(0),
                inputs.vs,
                inputs.rssMetadata
