...
contract Fibswap is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, RouterPermissionsManager, IFibswap {
  ...
  // local assetId => Swap router address
  mapping(address => address) public swapRouters;
   ...
   */
  function addSwapRouter(address[] memory assets, address[] memory routers) external onlyOwner {
    if (assets.length == 0 || assets.length != routers.length) revert Fibswap__addSwapRouter_invalidArgs();
    for (uint256 i = 0; i < assets.length; i++) {
      _addSwapRouter(assets[i], routers[i]);
    }
  }

  /**
   * @notice Used to add supported assets. This is an admin only function
   * @param localAsset - The asset to add
   * @param swapRouter - Address of the pool to swap
   */
  function setupAsset(address localAsset, address swapRouter) external override onlyOwner {
    // Add the asset
    _addAssetId(localAsset);

    // Add the swap pool
    _addSwapRouter(localAsset, swapRouter);
  }
  ...
  function xcall(XCallArgs calldata _args) external payable override returns (bytes32) {
    ...
    if (localAsset != _transactingAssetId) {
      if (swapRouters[localAsset] == address(0) || swapRouters[localAsset] != _args.params.orgParam.to) {
  ...
  function _addLiquidityForRouter(
    ...
    // Asset is approved
    if (!approvedAssets[_local]) revert Fibswap__addLiquidityForRouter_badAsset();
   ...
   */
  function _addSwapRouter(address _asset, address _swapRouter) internal {
    ...
    swapRouters[_asset] = _swapRouter;

    emit SwapRouterUpdated(_asset, _swapRouter, msg.sender);
  }
...
interface IFibswap {
   ...
   */
  event SwapRouterUpdated(address asset, address swapRouter, address caller);
  ...
  function setupAsset(address localAsset, address swapRouter) external;
