...
abstract contract OdysseyDatabase {
    ...
    address treasury; // slot 3
    address admin; //slot 4
    address xp; //slot 5
    ...
    function writeAddress(uint256 slot, address value) internal {
        assembly {
            sstore(slot, value)
        }
    }

    function writeUint256(uint256 slot, uint256 value) internal {
        assembly {
            sstore(slot, value)
        }
    }
...
contract OdysseyRouter is OdysseyDatabase, ReentrancyGuard {
    ...
    function Xp() public view returns (OdysseyXp) {
        return OdysseyXp(readSlotAsAddress(5));
    }
