...
contract NFTMarketplace is Pausable, FeeManager, ERC721Holder {
    ...
    function tokensOfOwner(address _owner)public view returns (uint256[]memory AuctionId,uint[]memory OrderId,uint[] memory TokensOfOwner)
    {
        uint256 count = nftToken.balanceOf(_owner);
        uint256[] memory resultAuction=new uint256[](auctionnID.length);
        uint256[] memory resultOrder=new uint256[](orderrID.length);
        ...
        return (resultAuction,resultOrder,resultTokensOfOwner);
        
    }
    // function orderTokensOfOwner(address _owner)public view returns (uint256[]memory  re)
    // {
