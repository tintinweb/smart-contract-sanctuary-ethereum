...
contract QueenTraits is BaseContractController, IQueenTraits {
    ...
    constructor(IQueenStaff _queenStaff) {
        ...
        traits.push(
            RoyalLibrary.sTRAIT({id: 2, traitName: "ROSTO", enabled: 1})
        );

        traits.push(
            RoyalLibrary.sTRAIT({id: 3, traitName: "CLOTHES", enabled: 1})
        );

        traits.push(
            RoyalLibrary.sTRAIT({id: 4, traitName: "COROA/CHAPEU", enabled: 1})
        );

        traits.push(
            RoyalLibrary.sTRAIT({id: 5, traitName: "COLAR/BRINCO", enabled: 1})
        );

        traits.push(
            RoyalLibrary.sTRAIT({id: 6, traitName: "MOLDURA", enabled: 1})
        );

        //RARITIES
        rarities.push(
            RoyalLibrary.sRARITY({id: 1, rarityName: "COMUM", percentage: 85})
        );
    ...
    function SetArt(RoyalLibrary.sART[] memory _artUri)
        ...
        whenNotPaused
    {
        require(_artUri.length > 0, "Art uri must have at least one value!");

        for (uint256 index = 0; index < _artUri.length; index++) {
            string memory _uri = _artUri[index].uri;
            uint256 _traitId = _artUri[index].traitId;
            uint256 _rarityId = _artUri[index].rarityId;
            ...
            (exists, artIdx) = CheckIfArtInRemoved(_traitId, _rarityId, _uri);
            ...
            emit ArtCreated(_traitId, _rarityId, _uri);
        }
    }
        ...
        returns (bool result)
    {
        ...
        if (!_purge) emit ArtRemoved(_traitId, _rarityId, _artUri);
        else emit ArtPurged(_traitId, _rarityId, _artUri);
