...
contract FancyTraitSale is AccessControlEnumerable {
    ...
    function updatePrice(uint256 _tokenId, uint256 _price) public onlyRole(SALE_MANAGER_ROLE) {
        tokenSaleData[_tokenId].price = _price;
        emit PriceUpdated(_tokenId, _price);
    }

    function clearCounter(uint256 _tokenId) public onlyRole(SALE_MANAGER_ROLE) {
        tokenSaleData[_tokenId].counter = 0;
        emit CounterCleared(_tokenId);
    }

    function updateMaxSupply(uint256 _tokenId, uint256 _maxSupply) public onlyRole(SALE_MANAGER_ROLE) {
        require(
            _maxSupply >= tokenSaleData[_tokenId].counter, 
            "updateMaxSupply: cannot set max supply below item counter"
        );

        tokenSaleData[_tokenId].maxSupply = _maxSupply;
        emit MaxSupplyUpdated(_tokenId, _maxSupply);
    }

    function toggleSale(uint256 _tokenId) public onlyRole(SALE_MANAGER_ROLE) {
        tokenSaleData[_tokenId].saleActive = !tokenSaleData[_tokenId].saleActive;
        emit SaleToggled(_tokenId, tokenSaleData[_tokenId].saleActive);
    }

    function deleteSaleData(uint256 _tokenId) public onlyRole(SALE_MANAGER_ROLE) {
        delete(tokenSaleData[_tokenId]);
        emit SaleDataDeleted(_tokenId); 
    }
