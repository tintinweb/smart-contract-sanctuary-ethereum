...
///////////////////////////////////////////////////////////////
    ...
    // uint256 public rewPerMinutForFreeMint = 1377777777777778;      // 20 token/day
    // uint256 public rewPerMinutForFreeMint = 2857142857142857000;
    uint256 public rewardFreeMint = 20;
    uint256 public rewardDutchMint = 100;

    // uint256 public rewPerMinutForDutchMint = 69444444444444444;     // 100 token/day
    // uint256 public rewPerMinutForDutchMint = 14285714285714290000;
    ...
    uint256 public maxNoOfdaysForFreeMint = 7;

    // uint256 public maxNoOfdaysForDutchMint = 144000;     // 100 days
    uint256 public maxNoOfdaysForDutchMint = 7;
    ...
    function lockedStaking(address _user, uint256 tokenId) external onlyMinter {
    ...
    function lockedReward(uint256 tokenId) public view returns(uint256){
        ...
        if(NFT.getIdType(tokenId) == 1){
            reward += (((noOfDays).mul(rewardFreeMint).mul(1 ether))).div(maxNoOfdaysForFreeMint);
        }
        else if(NFT.getIdType(tokenId) == 2){
            reward += (((noOfDays).mul(rewardDutchMint)).mul(1 ether)).div(maxNoOfdaysForDutchMint);
        }
