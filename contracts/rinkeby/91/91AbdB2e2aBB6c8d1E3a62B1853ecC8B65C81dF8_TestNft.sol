...
contract TestNft is ERC721, Ownable {
    ...
    constructor(string memory _initBaseURI) ERC721("TestNFT", "TNF") {
        setBaseURI(_initBaseURI);
    }

    function mint(uint256 _mintAmount) public payable {
        require(_mintAmount > 0, 'NFT::Cannot mint 0');
        require(_tokenIds.current() + _mintAmount <= MAX_SUPPLY, 'NFT::Mint amount should exceed total');
        require(msg.value >= price * _mintAmount, 'NFT::ETH value sent too low');

        for (uint256 i = 1; i <= _mintAmount; i = i + 1) {
            _tokenIds.increment();
