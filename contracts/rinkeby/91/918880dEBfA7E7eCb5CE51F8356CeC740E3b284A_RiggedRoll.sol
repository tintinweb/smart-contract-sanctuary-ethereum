...
contract RiggedRoll is Ownable {
    ...
    function riggedRoll() public {
        require(address(this).balance >= 0.002 ether, "NOT_ENOUGH");
        uint256 riggedNone = diceGame.nonce();
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(diceGame), riggedNone));
        uint256 roll = uint256(hash) % 16;

        console.log("RIGGED BLOCK NUMBER", block.number );
        console.logBytes32(prevHash);
        console.log("RIGGED ADDRESS", address(diceGame));
        console.log("RIGGED NONCE", riggedNone);
        console.logBytes32(hash);
