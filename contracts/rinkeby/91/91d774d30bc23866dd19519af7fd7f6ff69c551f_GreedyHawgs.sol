...
// File: contracts/GreedyHawgs.sol
    ...
    ReentrancyGuard
{
    ...
    function giftMint(address[] calldata _addresses) external onlyOwner {
        require(
            ...
            "Greedy Hawgsy: max total supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "Greedy Hawgs: recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function presaleMint(uint256 _amount) external payable nonReentrant{
        require(presaleM, "Greedy Hawgs: Presale is OFF");
        require(!paused, "Greedy Hawgs: contract is paused");
        require(
            _amount <= presaleAmountLimit,
            "Greedy Hawgs: You can't mint so much tokens"
        );
        require(
        ...
        require(
            ...
            "Greedy Hawgs: max supply exceeded"
        );
        require(
            _price * _amount <= msg.value,
            "Greedy Hawgs: Ether value sent is not correct"
        );
        uint256 _newItemId;
    ...
    function publicMint(uint256 _amount) external payable nonReentrant{
        require(publicM, "Greedy Hawgs: Public is OFF");
        require(_amount > 0, "Greedy Hawgs: zero amount");
        require(
            totalMint + _amount <= maxSupply - giftSupply,
            "Greedy Hawgs: max supply exceeded"
        );
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Greedy Hawgs: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Greedy Hawgs: burn caller is not owner nor approved"
        );
        _burn(tokenId);
        totalNFT = totalNFT - 1;
    }
