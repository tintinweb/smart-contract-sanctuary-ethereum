...
contract AAERC721A is Ownable, ERC721A, ReentrancyGuard {
    ...
    function freeMint(uint256 quantity) external payable callerIsUser {
        ...
        require(totalSupply() + quantity <= amountForDevs + amountForFree, "All free mints have been used");
        _safeMint(msg.sender, quantity);
    }
    ...
    function canPublicSaleStart()
        ...
        returns (bool)
    {
        return (price != 0 && ((totalSupply() >= amountForDevs + amountForFree) || block.timestamp - freeMintStartTime >= 24*60*60));
    }     
    ...
    function isWhiteListSaleOn()
        ...
        returns (bool)
    {
        return !isPublicMintActive && wlRoot != ""  && price != 0 && (totalSupply() >= amountForDevs + amountForFree + amountForSale || block.timestamp - publicMintStartTime >= 24*60*60); // ensure that the whitelist mint starts
    }                                                                             
