...
library $C {
    ...
    bytes32 internal constant VERIFY_TYPEHASH   = keccak256("Verify(address sender,bytes32 tweetId,Twitter[] twitters,address signatory)");
...
contract Woofer is IERC20, Extendable {
    ...
    function APY() external view viewExtend returns(uint) {
        return $.APY();
    }
    ...
    //function calcIn(address[] calldata path) external view viewExtend returns(uint) {
    //    return _calcIn(path);
    //}
    //function _calcIn(address[] calldata path) internal view returns(uint r) {
    //    uint r1 = _calcRatio1();
    //    if(r1 == 0)
    //        return uint(-1);
    //    r = _calcIn1().mul(1e18).div(r1);
    //
    //    mapping (bytes32 => address) storage configA = Config.configA();
    //    address currency = configA[$C._currency_];
    ...
    function cowoof(bytes32 id, bytes32 twitterId, bytes32 tweetId, uint amount, uint value, PermitSign calldata ps, address[] calldata path, Signature[] calldata signatures) external extend updateWoof(tweetId) {
        address sender = $M.msgSender();
    ...
    function rewoof(bytes32 id, bytes32 tweetId, uint amount, uint value, PermitSign calldata ps, address[] calldata path, Signature[] calldata signatures) external extend updateWoof(tweetId) {
        address sender = $M.msgSender();
        _verify(sender, tweetId, new Twitter[](0), signatures);
        amount = Math.min(amount, $.unlockedOf(sender));
...
library WooferLib {
    ...
    function APY(WooferStru storage $) external view returns(uint y) {
        (, uint r, uint period) = calcRebaseProfit($, address(0));
        r = r.add(1e18);
        y = 1e18;
