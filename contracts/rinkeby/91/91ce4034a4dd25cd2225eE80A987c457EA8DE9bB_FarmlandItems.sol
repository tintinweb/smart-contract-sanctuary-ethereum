...
// VIEW FUNCTIONS
    ...
    {
        return itemWhitelists;                                              // Return the array
    }
}
...
// EVENTS
    ...
    event ItemAdded(address addedBy, uint256 itemID, string name, string imageUrl, uint256 price, uint256 totalSupply, ItemType itemType, IERC20 paymentContract, bool mintingActive);
...
// USER FUNCTIONS
        ...
        onlyWhenMintingActive(itemID)
    {
        ...
        require(totalSupply(itemID) + amount <= item.maxSupply,                                 "Not enough supply");
        _mint(recipient, itemID, amount, "");                                                   // Mint item
        if (item.itemType == ItemType.Payable) {
...
// ADMIN FUNCTIONS
    ...
    /// @param itemName name of the NFT asset
        ...
        onlyOwner 
    {
        ...
        require(ItemType(itemType) == ItemType.Payable || 
                ItemType(itemType) == ItemType.Claimable,                                              "Invalid tem type");
        if ( itemType == ItemType.Payable ) {
        ...
        ItemDetails memory item = ItemDetails(price, maxSupply, itemType, itemName, imageUrl,
                                              animationUrl, IERC20(paymentAddress), true);            // Create new item
        ...
        emit ItemAdded(_msgSender(), newItemId, itemName, imageUrl, price, maxSupply, 
                       itemType, IERC20(paymentAddress), true);                                       // Write event
    }
        ...
        onlyOwner 
    {
        ...
        for (uint256 itemID = 0; itemID < total; itemID++) {                            // Loop through all items
            if ( items[itemID].itemType == ItemType.Payable ) {                         // Checks if the item is payable
