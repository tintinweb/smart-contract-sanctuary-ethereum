...
contract Treasury is Ownable, AccessControl, ReentrancyGuard, IERC721Receiver {

    bytes32 constant BIG_ADMIN_ROLE = keccak256("BIG_ADMIN_ROLE");
    ...
    function withdraw(address _token, address _to, uint256 _amount) external  onlyRole(ADMIN_ROLE) nonReentrant {
    ...
    function depositNFT(address _token, uint256 _id) external {
        require(_token != address(0), "INVALID_TOKEN_ADDRESS");
        require(IERC721(_token).getApproved(_id) == address(this), "NFT_ID_NOT_APPROVE");

        IERC721(_token).safeTransferFrom(address(msg.sender), address(this), _id, "0x");

        emit DepositNFT(_token, address(msg.sender), address(this), _id);
    }

    function withdrawNFT(address _token, address _to, uint256 _id) external onlyRole(ADMIN_ROLE) { 
        require(_token != address(0), "INVALID_TOKEN_ADDRESS");
        require(IERC721(_token).ownerOf(_id) == address(this), "TREASURY_DOESNT_HAVE_THIS_NFT");

        IERC721(_token).safeTransferFrom(address(this), _to, _id, "0x");
