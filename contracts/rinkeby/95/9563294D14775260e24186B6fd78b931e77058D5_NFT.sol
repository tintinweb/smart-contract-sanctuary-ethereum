...
contract NFT is ERC721Enumerable, Ownable {
    ...
    string private notRevealedUri="https://ipfs.io/ipfs/QmQUzE6nNTQWuWrg6eEL2Pgrge5TL7JV9RLENBEgAYjzks";
    string private revealedUri="https://ipfs.io/ipfs/QmZ4ifASJXC6PfjBePqk34TvbZPScLknvjgwVcHVNQNDh2";
	string public projectURI = "https://gateway.pinata.cloud/ipfs/QmPtoRNeLWctE468t1yxJ66z3YZ5r2pES9t5Dz7aNzSMLf";
    ...
    constructor(
        ...
        setReveal(false);
        payments = payable(0xa3F896b4E0D7930DF04A17E6e7b547a893455c42);
    }
    ...
    function freemint() public payable isFreeMinter(msg.sender) {
        require(!paused, 'Contract is paused');
        uint256 supply = totalSupply();
        require(totalSupply() + 1 <= maxSupply, 'Supply minted');

        uint256 senderMintCount = addressMintedBalance[msg.sender];
        require(senderMintCount + 1 <= nftPerAddressLimit, "already minted 3");
        
        addressMintedBalance[msg.sender]++;
        _safeMint(msg.sender, supply + 1);


        // We get the address out of the 
    ...
    // Free Minters
    modifier isFreeMinter(address _sender) {
        if(msg.sender != owner()){
            require(freeMinters[_sender] == true, "Address is not free minter, or already minted 1");
        }
        _;
    }
    ...
    function setReveal(bool _reveal) public onlyOwner {
        if (_reveal == false){
            ...
            setBaseURI(revealedUri); // Small surprise incoming :)
        }
    }
    ...
    function setFreeMint(address _user, bool enable) external onlyOwner {
        freeMinters[_user] = enable;
    }
