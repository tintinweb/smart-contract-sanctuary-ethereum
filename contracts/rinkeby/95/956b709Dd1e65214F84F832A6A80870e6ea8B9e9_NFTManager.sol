    ...
    ResourceImplementationPointer
{
    ...
    function createNFT(uint256 nftNumber) external payable nonReentrant whenNotPaused resourceSet {
        address sender = _msgSender();
    ...
    function cashoutRewardFromSupports(uint256 _nftId, bool swapping) external nonReentrant whenNotPaused resourceSet checkPermissions(_nftId) {
        uint256 reward = _getNFTCashoutRewardsFromSupports(_nftId);
        _cashoutReward(reward, swapping);
    }
    ...
    function compoundAll() external nonReentrant whenNotPaused resourceSet {
        ...
        uint256[] memory nftsSupported = getSupportNFTIdsOf(account);

        for (uint256 i = 0; i < nftsOwned.length; i++) {
            (uint256 amountToCompound, uint256 feeAmount) = _getNFTCompoundRewards(nftsOwned[i], account);
        ...
        for (uint256 i = 0; i < nftsSupported.length; i++) {
            if (nftsSupported[i] == 0) {
                continue;
            }
            (uint256 amountToCompound, uint256 feeAmount) = _getNFTCompoundRewards(nftsSupported[i], account);
            if (amountToCompound > 0) {
                feesAmount += feeAmount;
                amountsToCompound += amountToCompound;
            }
    ...
    function _getNFTCashoutRewardsFromSupports(uint256 _nftId) private returns (uint256) {
        NFTEntity storage nft = _nfts[_nftId];
        ...
        uint256 reward = _calculateRewardsFromValue(nft.supportValue, compoundDelay, rewardPerDay);
    ...
    function getSupportNFTIdsOf(address account) public view returns (uint256[] memory) {
        uint256[] memory nftIds = nftsOfUser[account];
        for (uint256 i = 0; i < nftIds.length; i++) {
            if (ownerOf(nftIds[i]) == account) {
                nftIds[i] = 0;
            }
        }
        return nftIds;
