...
contract TestSale is Ownable {
    ...
    function setSetSaleSupply(uint _additionalSupply) external onlyOwner {
        maxSupplySale = ITestNFT(oxyalandsNFTAddress).totalSupply() +  _additionalSupply;
    }
    /*
     * Time mecanism
    ...
    function initialize() external onlyOwner {
        ...
        for (uint256 i = 1; i < DUTCH_AUCTION_MAX_ROUND + 1; i++) {
    ...
    function mintByOwner(address _to) public onlyOwner {
        require(
            ITestNFT(oxyalandsNFTAddress).totalSupply() + 1 <= maxSupplySale,
     ...
     */
    function mintMultipleByOwner(address[] memory _to, uint[] memory _amount) public onlyOwner {
        for (uint256 i = 0; i < _to.length; i++) {
            require(
            ITestNFT(oxyalandsNFTAddress).totalSupply() + _amount[i] <= maxSupplySale,
            "Tokens number to mint cannot exceed number of MAX tokens"
            );
            ITestNFT(oxyalandsNFTAddress).mintBySaleContract(_to[i], _amount[i]);
        }
    }
    /*
     * Function to mint freeMints
     */
    function freeMint(uint256 _quantity, uint256 count, bytes32[] calldata proof) external isInitialized  {
    ...
    function whitelistMint(uint256 _quantity, bytes32[] calldata proof) public payable isInitialized  {
        uint price = getPrice();
        ...
        require(ITestNFT(oxyalandsNFTAddress).totalSupply() + _quantity <= maxSupplySale, "Max supply for this sale reached");
    ...
    function whitelistMintStake(uint256 _quantity, bytes32[] calldata proof) external payable isInitialized  {
        
        this.whitelistMint(_quantity, proof);
        ...
        for(uint i=0;i<_quantity;i++){
            ITestNFT(oxyalandsNFTAddress).stakeNFT(totalSupply+i);
        }         
    }

    function publicMint(uint _quantity) public payable isInitialized isPublicSaleActive {
        uint price = getPrice();
     ...
     function publicMintStake(uint256 _quantity) external payable isInitialized isPublicSaleActive {
        
        this.publicMint(_quantity);
        uint totalSupply=ITestNFT(oxyalandsNFTAddress).totalSupply();
            ...
            for(uint i = 0; i < 4; i++) {
                ...
                if(count != 0){
                    for(uint j = 0; j < count ; j++) {
                        //should call VRF
                        uint randomNumberFinal = uint256(keccak256(abi.encodePacked(randomNumber,i, j))) % 100;
    ...
    function stake(uint256 _tokenId) external {
        require(ITestNFT(oxyalandsNFTAddress).ownerOf(_tokenId)==msg.sender,"not your nft");
    ...
    function unStake(uint256 _tokenId) external {
        require(ITestNFT(oxyalandsNFTAddress).ownerOf(_tokenId)==msg.sender,"not your nft");
        ITestNFT(oxyalandsNFTAddress).unstakeNFT(_tokenId);
         for(uint i = 0; i < 4; i++) {
                uint count = chanceToWinPerAddress[msg.sender][i];
