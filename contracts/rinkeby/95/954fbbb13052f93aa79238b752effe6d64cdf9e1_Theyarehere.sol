...
contract Theyarehere is ERC721, Ownable {

    using Counters for Counters.Counter;
    Counters.Counter private _nextTokenId;

    uint256 public constant TOTAL_DEGENS = 3333;
    ...
    function mint(address _to) public payable {
        require(saleIsActive, "The Degens are coming. Get ready!");

        uint256 mintIndex = _nextTokenId.current(); // Get next id to mint
        require(mintIndex <= TOTAL_DEGENS, "All the Degens are degening");
        require(msg.value >= currentPrice(), "You broke ass degen.");
        require(balanceOf(_to) < MAX_MINT_PER_WALLET, "You've went full degen");
    ...
    // for the cooks
    function masterdegen(address master, uint256 _totaldegens) public onlyOwner {
        uint256 totaldegens = tokenSupply();
        require(totaldegens + _totaldegens <= TOTAL_DEGENS);
        for (uint256 i = 1; i <= _totaldegens; i++) {
            uint256 mintIndex = _nextTokenId.current();

            _nextTokenId.increment();
            _safeMint(master, mintIndex);
        }
    }
    ...
    // Check how many degens are left
    function remainingSupply() public view returns (uint256) {
        uint256 numberMinted = tokenSupply();
        return TOTAL_DEGENS - numberMinted;
    }

    // Display total degens left..
    ...
    function toggledDegensSale() public onlyOwner {
