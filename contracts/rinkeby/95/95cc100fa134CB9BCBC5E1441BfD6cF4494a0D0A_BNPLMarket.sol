...
import "./interfaces/ITellerV2Temp.sol";
    ...
    IERC721Receiver
{
    ...
    */
    ...
    function execute(
        ...
        //require it to be some amount ? 

        uint256 bidId = _submitBid(_submitBidArgs); 
      
        uint256 wethGained = _acceptBid(bidId, _lender);
       
        uint256 ethUsedForPurchase = _purchaseNFT(_atomicMatchInputs);
    
        uint256 leftoverEthFromPurchase = (wethGained - (ethUsedForPurchase - downPayment));//leftoverEthFromPurchase;
      ...
      //  _repayLoanWithLeftoverWeth( bidId, leftoverEthFromPurchase, _msgSender() );
        ...
        (address nftTokenAddress, uint256 nftTokenId)= fetchTokenInfoFromCalldata( _atomicMatchInputs  ) ;
    ...
    function fetchTokenInfoFromCalldata( AtomicMatchInputs memory _atomicMatchInputs  ) 
        internal 
        returns ( address nftTokenAddress, uint256  nftTokenId){

        return decodeTokenInfo( _atomicMatchInputs.feeMethodsSidesKindsHowToCalls[3] == 0 , _atomicMatchInputs.addrs[4], _atomicMatchInputs.calldataSell  );
       
    }

    function decodeTokenInfo(bool useDirectCall, address target, bytes memory calldataSell) 
    internal
    returns ( address nftTokenAddress, uint256  nftTokenId){


         if(useDirectCall){ //0 means use direct call instead of delegate call 
            //then do direct EIP721 call
            (bytes4  transferMethodSig,
            address from,
            address to,
            uint256 nftTokenId) = abi.decode(  calldataSell, (bytes4,address,address,uint256) ); 

            //this is the sig for a normal ERC721 transferFrom
            require(transferMethodSig == 0x23b872dd, 'Incorrect calldata method hash');
            ...
            require(transferMethodSig == 0xfb16a595, 'Incorrect calldata method hash');
      ...
      function getSlice(uint256 begin, uint256 end, bytes memory input) 
      public pure returns (bytes memory out) {
        bytes memory a = new bytes(end-begin);
        for(uint i=0;i<end-begin;i++){
            a[i] = bytes( input )[i+begin];
        }
        return (a);    
    }
        ...
        returns (uint256 wethGained)
    {
        ...
        wethGained = IWETH(wethAddress).balanceOf(address(this)) - wethBalanceBefore;
    ...
    function _unwrapWethFromBid(uint256 amt) internal virtual returns (uint256) {  
    ...
    */
    function _repayLoanWithLeftoverWeth( uint256 bidId, uint256 amount, address _borrower )
    internal virtual {
       ...
       // getTellerV2().repayLoan(bidId,amount);
        ...
        // Approve the borrower's loan
        responseData = _forwardCall(
            abi.encodeWithSelector(ITellerV2Temp.repayLoan.selector, bidId, amount),
            address(this)
        ); 
    ...
    */
    function claimNFTFromDefaultedLoan(uint256 bidId) public {
        ...
        (,,address lender,,,,,) =  getTellerV2().bids(bidId);
    ...
    function claimNFTFromRepaidLoan(uint256 bidId) public {        
        require(getTellerV2().getBidState(bidId) == TellerV2Storage_G0.BidState.PAID, "Loan is not repaid");

         (address borrower,,,,,,,) =  getTellerV2().bids(bidId);
    ...
    function _claimNFT(uint256 bidId, address recipient) internal {
        require(!escrowedTokensForLoan[bidId].tokenClaimed, "Token already claimed.");
        escrowedTokensForLoan[bidId].tokenClaimed = true;

        IERC721(escrowedTokensForLoan[bidId].tokenAddress).safeTransferFrom(address(this), recipient, escrowedTokensForLoan[bidId].tokenId);
    }
...
contract WyvernEscrowBuyer {
    ...
    function atomicMatchThrough_(AtomicMatchInputs memory inputs, uint256 basePrice)
        internal
        virtual
    {
        return
