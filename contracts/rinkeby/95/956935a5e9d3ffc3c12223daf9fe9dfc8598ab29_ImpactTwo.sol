...
contract ImpactTwo is Ownable, AdminControl {
    ...
    uint256 public startBlockTimestamp = type(uint256).max;
    uint256 public endBlockTimestamp = type(uint256).max;

    constructor(address _creator, uint8 _tokenId) {
        require(IERC1155CreatorCore(_creator).totalSupply(_tokenId) == 0, "Token ID already initialised");
    ...
    function setPrice(uint256 _price) external adminRequired {
        price = _price;
    }

    function setStartBlockTimestamp(uint256 _blockTimestamp) external adminRequired {
        require(_blockTimestamp <= endBlockTimestamp, "Can't be bigger than ending block timestamp");
        startBlockTimestamp = _blockTimestamp;
    }

    function setEndBlockNumber(uint256 _blockTimestamp) external adminRequired {
        require(_blockTimestamp >= startBlockTimestamp, "Can't be bigger than starting block timestamp");
        endBlockTimestamp = _blockTimestamp;
    }
    ...
    function mint() external payable {
        ...
        require(
            block.timestamp >= startBlockTimestamp,
            "Minting has not started yet"
        );
        require(
            block.timestamp <= endBlockTimestamp,
        ...
        tokenIdsForMint[0] = tokenId;
        amountsForMint[0] = 1;

        IERC1155CreatorCore(creator).mintExtensionExisting(callerAddresses, tokenIdsForMint, amountsForMint);
    }
    ...
    function mintNewTokenAsAdmin() external adminRequired {
        ...
        IERC1155CreatorCore(creator).mintExtensionNew(callerAddresses, amountsForMint, uris);
    }

    function setBaseUri(string calldata uri) external adminRequired {
        IERC1155CreatorCore(creator).setBaseTokenURIExtension(uri, true);
    }

    function setTokenId(uint8 _tokenId) external adminRequired {
        require(IERC1155CreatorCore(creator).totalSupply(_tokenId) == 0, "Token ID already initialised");
        tokenId = _tokenId;
    }
}
