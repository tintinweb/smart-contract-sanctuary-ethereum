...
contract CollectionsRepository is ICollectionsRepository {
    ...
    constructor(address _tokenFactoryAddress, address collectionFactoryAddress) {
        _tokenFactory = _tokenFactoryAddress;
        _collectionFactory = collectionFactoryAddress;
    }
    ...
    function addCollection(
        ...
        require(msg.sender == _collectionFactory);
        OmniERC721 omniNft = new OmniERC721(_name, _symbol, _creator, _maxSupply, _metadataUrls, _collectionURI, _mintPrice, _dropFrom, _dropTo, _tokenFactory);
        address collectionAddress = omniNft.collectionAddress();
        _collectionIds++;
        collections.push(collectionAddress);
        userCollections[_creator].push(collectionAddress);
...
contract OmniERC721 is IOmniERC721, ERC721URIStorage {

    uint256 public createdAt;
    uint256 public maxSupply;
    address public creator;
    address public collectionAddress;
    uint256 public dropFrom;
    uint256 public dropTo;
    string public collectionURI;
    uint256 public mintPrice;
    uint256 public tokenIds;
    bool public claimed;
    string[] private metadataUrls;
    ...
    constructor(
        ...
        if (_mintPrice != 0) {
            require(_mintPrice > 0 && _maxSupply > 0 && _metadataUrls.length > 0);
        }

        if (_metadataUrls.length > 0) {
            require(_metadataUrls.length == _maxSupply);
        }
        ...
        dropTo = _dropTo;
        if (dropFrom > 0) {
            require(dropFrom > 0 && dropTo > dropFrom && dropFrom >= (block.timestamp - 1 days) && dropFrom <= (block.timestamp + 92 days + 1 hours) && dropTo <= (dropFrom + 8 days));
        }
    }

    function mint(address owner, string memory tokenURI) override external {
        require(msg.sender == _tokenFactory);
        if (dropFrom > 0) {
            require(block.timestamp >= dropFrom && block.timestamp <= dropTo, "Drop isn't happening now");
        }
        tokenIds++;

        if (maxSupply > 0) {
            require(tokenIds <= maxSupply);
        }
        _safeMint(owner, tokenIds);
        _setTokenURI(tokenIds, metadataUrls.length > 0 ? metadataUrls[tokenIds - 1] : tokenURI);
    }

    function getMintPrice() override external view returns (uint256) {
        return mintPrice;
    }

    function getCreator() override external view returns (address) {
        return creator;
    }

    function getDetails() override public view returns (string memory, address, uint256, uint256, uint256) {
        return (collectionURI, collectionAddress, dropFrom, dropTo, mintPrice);
    }

    function isClaimable() external override view returns (bool) {
        if (claimed == true) {
            return false;
        }
        if (dropTo > 0) {
            return dropTo <= (dropTo + 7 days);
        }
        return tokenIds == maxSupply && createdAt <= (createdAt + 7 days);
    }

    function setClaimed() external override {
        require(msg.sender == _tokenFactory);
        claimed = true;
    }
}
...
contract TokenFactory is NonblockingLzApp {
    ...
    function mintToken(
        ...
        address tokenOwner = rec != address(0) ? rec : msg.sender;
        ...
        if (_chToId[chSym] == _contractChainId) {
            ...
            if (price > 0) {
                payOnMint(price, msg.sender, asset, omniNft.getCreator());
            }
            omniNft.mint(tokenOwner, URI);
            emit Minted(coll, rec);
            return;
        }
        if (mintPrice > 0) {
            payOnMint(mintPrice, msg.sender, asset, address(this));
        }
        bytes memory payload = _getPayload(1, coll, URI, tokenOwner, mintPrice, creator, asset);
        _lzAction(payload, chId, gas, msg.sender, false);
    }
    ...
    function _nonblockingLzReceive(
        ...
        (uint256 act, address coll, string memory URI, address rec, uint256 paid, address cre, address asset) = abi.decode(_payload, (uint256, address, string, address, uint256, address, address));
        bool isReq = act == 1;

        if (isReq == false && paid > 0) {
            pay(rec, paid, asset);
            return;
        }
        ...
        if (isReq) {
            ...
            require(cre == nft.getCreator());

            try nft.mint(rec, URI) {
                emit Minted(coll, rec);
                _resAction(
                    _getPayload(2, coll, "", cre, price, cre, asset),
                    srcId
                );
            } catch {
                emit NewRefund(coll, rec);
                _resAction(
                    _getPayload(2, coll, "", rec, price, cre, asset),
                    srcId
                );
            }
        }
    ...
    function _lzAction(bytes memory pload, uint16 chId, uint256 gas, address gasTo, bool estFee) private {
        uint msgFee;
        if (estFee) {
            (uint fee,) = lzEndpoint.estimateFees(chId, address(this), pload, false, bytes(""));
            msgFee = fee;
        }

        bytes memory remote = this.getTrustedRemote(chId);
        lzEndpoint.send{value: (estFee ? msgFee : msg.value)}(
            chId,
            remote,
            pload,
            payable(gasTo),
            address(0x0),
            _getAdapter(gas, remote)
        );
    }

    function _resAction(bytes memory payload, uint16 chId) private {
        _lzAction(payload, chId, 0, address(this), true);
    }

    function payOnMint(uint256 price, address spender, address asset, address rec) internal {
        bool isSupported;
    ...
    function pay(address rec, uint256 price, address asset) private {
        IERC20 erc = IERC20(asset);
        erc.transfer(rec, price);
        emit Paid(rec);
    }

    function estimateFees(string memory chSym, address coll, string memory URI, address rec, uint256 price, address cre, uint gas, address asset) external view returns (uint) {
        bytes memory payload = _getPayload(1, coll, URI, rec, price, cre, asset);
        uint16 chId = uint16(_chToId[chSym]);
        bytes memory remote = this.getTrustedRemote(chId);
        bytes memory adapter = _getAdapter(gas, remote);
        (uint fee,) = lzEndpoint.estimateFees(chId, address(this), payload, false, adapter);

        return fee;
    }

    function _getPayload(uint256 act, address coll, string memory URI, address rec, uint256 price, address cre, address asset) private pure returns (bytes memory) {
        return abi.encode(act, coll, URI, rec, price, cre, asset);
    }

    function _getAdapter(uint gas, bytes memory remote) private pure returns (bytes memory) {
        if (gas == 0) {
            return bytes("");
        }
        uint16 v = 1;
        uint256 nativeGas = 200000000000000000;
        return abi.encodePacked(v, gas, nativeGas, remote);
    }
