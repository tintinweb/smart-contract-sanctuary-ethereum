...
contract AlphaWhaleNFT is ERC721Enumerable, Ownable, Pausable {
    ...
    function whitelistMint(
        ...
        for (uint256 i = 1; i <= _numberOfTokenToMint; i++) {
    ...
    function withdraw() public onlyOwner {
        ...
        // this will split the payment to 33.33%
        uint256 fundPerFounder = (funderWalletFund * 3333) / 10000;

        payable(alphaWhaleFundWallet).transfer(alphaWhaleFund);

        payable(ctoWallet).transfer(fundPerFounder);
        payable(cooWallet).transfer(fundPerFounder);
        // sent all the remaining eth balance to ceoWallet
        // this will make sure that no eth will remain on this contract
        payable(ceoWallet).transfer(address(this).balance);
    }
