...
contract Community is ICommunity {
    ...
    function unpublishProject(uint256 _communityID, address _project)
        external
        override
        isPublishToCommunity(_communityID, _project)
        ...
        isPublishToCommunity(_communityID, _project)
    {
        ...
        uint256 _lenderFee = (_lending * _projectInstance.lenderFee()) /
            (_projectInstance.lenderFee() + 1000);
        uint256 _amountLent = _lending - _lenderFee;
        require(
            _amountLent <=
        ...
        _currency.safeTransferFrom(_msgSender(), _treasury, _lenderFee);
        _currency.safeTransferFrom(_msgSender(), _project, _amountLent);

        _projectInstance.lendToProject(_amountLent);

        IToken20 _wrappedToken = IToken20(
            homeFiInstance.wrappedToken(address(_currency))
        );
        _wrappedToken.mint(_msgSender(), _lending);
        communities[_communityID]
            .projectDetails[_project]
            .totalLent += _amountLent;

        if (
            communities[_communityID]
                .projectDetails[_project]
                .lendingDetails[_msgSender()]
                .lentAmount > 0
        ) {
        ...
        communities[_communityID]
            .projectDetails[_project]
            .lendingDetails[_msgSender()]
            .lendingTimestamp = block.timestamp;
        ...
        onlyProjectBuilder(_project)
    {
        ...
        claimInterest(_communityID, _project, _lender, _wrappedToken);

        uint256 _amountToReturn = _communityProject
            .lendingDetails[_lender]
            .lentAmount;

        require(_repayAmount <= _amountToReturn, "Community::!Liquid");

        _communityProject.lendingDetails[_lender].lentAmount =
            _amountToReturn -
            _repayAmount;
        _wrappedToken.burn(_lender, _repayAmount);
        _currency.safeTransferFrom(_msgSender(), _lender, _repayAmount);
        ...
        override
    {
        ...
        // update debt
        _reduceDebt(_communityID, _project, _repayAmount, _details, _lender);
    }
    ...
    function transferDebt(
        ...
        returns (bool)
    {
        require(!transferDebtLock, "Community::locked");
        CommunityStruct storage _community = communities[_communityID];
        ProjectDetails storage _communityProject = _community.projectDetails[
            _project
        ];
        ...
        claimInterest(_communityID, _project, _msgSender(), _wrappedToken);
        uint256 _userLentAmount = _communityProject
            .lendingDetails[_msgSender()]
            .lentAmount;

        require(
            _amount > 0 && _amount <= _userLentAmount,
            "Community::!Balance"
        );

        if (_communityProject.lendingDetails[_to].lentAmount > 0) {
            claimInterest(_communityID, _project, _to, _wrappedToken);
        }

        _communityProject.lendingDetails[_msgSender()].lentAmount =
            _userLentAmount -
            _amount;
        _communityProject.lendingDetails[_to].lentAmount += _amount;
    ...
    function unlockTransferDebt() external override onlyCommunityAdmin {
        transferDebtLock = false;
        eventsInstance.transferDebtLock(false);
    }

    function lockTransferDebt() external override onlyCommunityAdmin {
        transferDebtLock = true;
        eventsInstance.transferDebtLock(true);
    }
    ...
    function returnToLender(
        uint256 _communityID,
        address _project,
        address _lender
    ) public view override returns (uint256 _return, uint256 _lent) {
        LendingDetails storage _lending = communities[_communityID]
            .projectDetails[_project]
            .lendingDetails[_lender];
        uint256 _apr = communities[_communityID].projectDetails[_project].apr;
        uint256 _dateOfLending = _lending.lendingTimestamp;
        uint256 _lentAmount = _lending.lentAmount;

        uint256 _noOfDays = (block.timestamp - _dateOfLending) / 86400; // 24*60*60

        uint256 _amountToReturn = _lentAmount +
            ((_lentAmount * _apr * _noOfDays) / 365000);
        return (_amountToReturn, _lentAmount);
    }
    ...
    function claimInterest(
        ...
        (uint256 _amountToReturn, uint256 _lent) = returnToLender(
            _communityID,
            _project,
            _user
        );
        ...
        require(_lent > 0, "Community::Claim||!lent");
        if (_amountToReturn > _lent) {
            _communityProject
                .lendingDetails[_user]
                .lentAmount = _amountToReturn;
            _communityProject.lendingDetails[_user].lendingTimestamp = block
                .timestamp;
            uint256 _interestEarned = _amountToReturn - _lent;
    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier isPublishToCommunity(uint256 _communityID, address _project) {
        require(
            projectPublished[_project] == _communityID,
            "Community::!published"
        );
    ...
    struct LendingDetails {
        // object storing data for individual lending event
        uint256 lentAmount;
        uint256 lendingTimestamp;
        address lender; // we don't need that as we already know it from the mapping
    }
    ...
    struct ProjectDetails {
        ...
        uint256 publishFee;
        bool publishFeePaid;
        mapping(address => LendingDetails) lendingDetails; // lender address => LendingDetails
    }
     ...
     * lock transfer debt.
     */
    function lockTransferDebt() external virtual;

    /**
     * unlock transfer debt.
     */
    function unlockTransferDebt() external virtual;
     ...
     * @param _communityID uint256 - the the uuid of the community
     * @param _project address - the address of the deployed project contract
     * @param _lender address - the address of the lender to repay + interest
    ...
    /**
     * Transfer ownership of lendings from one user to another
     * @dev modifier nonReentrant
     * @dev modifier nonZero(_to)
     * @dev transferDebtLock must be false
     * @notice in practice transfer debt erc20 tokens + update references in custom nft logic
     *
     * @param _communityID uint256 - the the uuid (serial) of the community
     * @param _project address - the address of the deployed project contract
     * @param _to address - the address receiving ownership of the specified lendings
     ...
     * @param _project address - the address of the deployed project contract
     * @param _lender address - the address of the lender to repay + interest
     * @return _return uint256 - the amount lent by _address + interest paid (the amount of tokens reclaimed)
     * @return _lent uint256 - the amount _address lent initially
     */
    function returnToLender(
        uint256 _communityID,
        address _project,
        address _lender
    ) public view virtual returns (uint256 _return, uint256 _lent);
     ...
     * @param _user address - address of lender claiming interest
     ...
     * @param _repayAmount uint256 - the amount of funds repaid to the lender, in the project currency
     * @param _details bytes - some details on why debt is reduced (off chain documents or images)
     * @param _lender address - lender address
     */
    function _reduceDebt(
        uint256 _communityID,
        address _project,
