    ...
    Pausable
{
    ...
    // 钱包地址：第一轮mintlist，数量可以和nft的供给一致
    // The allowClaims merkletree root.
    bytes32 public allowlistRoot;

    // 钱包地址：第二轮mintlist，数量任意，nft供给的15%～30%，太高会导致gas太高
    ...
    //     if (mintPhase != MintPhase.TEAM) revert WrongMintingPhase();

    //     _verifyMint(quantity, 0, 0, proof);
    // }

    /**
     * @dev See {IPlugman-mint}.
     */
    ...
    // 设置版税的接收者
    function setRoyaltyReceiver(address receiver) external override onlyOwner {
...
/**
 * @dev Interface of a Plugman compliant contract.
 */
interface IPlugman {
    ...
    // Thrown when minting when there are no more Plugman.
    error SoldOut();
     ...
     * @dev Mints Plugman.
     *
     * quantity - the number of Plugman to mint
     *
     * Requirements:
     *
    ...
    /**
     * @dev Mints Plugman from a whitelist.
     *
     * quantity - the number of Plugman to mint
     * freeAllowance - the number of free Plugman allocated to `msg.sender`
     * paidAllowance - the number of paid Plugman allocated to `msg.sender`
     * proof - merkle proof proving `msg.sender` has said `freeAllowance` and `paidAllowance`
     *
     * Requirements:
     *
