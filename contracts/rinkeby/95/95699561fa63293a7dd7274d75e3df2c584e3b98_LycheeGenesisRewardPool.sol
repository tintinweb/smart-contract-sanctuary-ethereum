...
// File contracts/distribution/LycheeGenesisRewardPool.sol
...
contract LycheeGenesisRewardPool {
    using SafeMath for uint256;
    ...
    uint256 public poolStartTime; // 1649649600

    // The time when TOMB mining ends.
    uint256 public poolEndTime;

    // MAINNET
    uint256 public tombPerSecond = 0.0578703 ether;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if (_pending > 0) {
                _updateVested(_sender, _pending);
    ...
    function _updateVested(address account, uint256 amount) internal {
        _claimVested(account);
        VestingSchedule storage schedule = vesting[account];
        schedule.amount = schedule.amount.sub(schedule.claimed).add(amount);
        schedule.period = 7 days;
        schedule.end = block.timestamp.add(7 days);
        schedule.claimed = 0;
    ...
    function _claimVested(address account) internal {
        ...
        uint256 duration = (block.timestamp > schedule.end ? schedule.end : block.timestamp).sub(schedule.lastClaimed);
        uint256 claimable = schedule.amount.mul(duration).div(schedule.period);
        ...
        safeTombTransfer(account, claimable);
    }
}