 ...
 */
contract ChibiDB is ERC721Whitelist {
    ...
    uint256  public  maxSupply = 1000;  // current release limit - can change to allow second mint, or to allow whitelist limits
    ...
    uint256 public whitelistPrice = 10000000000000000; //0.010 ETH
    uint constant maxTokenPurchase = 10;   // what's the max someone can buy
    uint constant maxWhitelistPurchase = 2;   // what's the
    ...
    constructor()
        ERC721Whitelist("ChibiDBs", "CHIBIDBS")
    {
    }
    ...
    function whitelistMint(uint256 amount, bytes32[] calldata proof) public payable {
        string memory payload = string(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(proof, whitelistRoot, keccak256(abi.encodePacked(payload))), "BAD_MERKLE_PROOF");

        require(whitelistSaleActive, "SALE_CLOSED");
        require(amount <= maxTokenPurchase, "AMOUNT_EXCEEDS_MAX_PER_CALL");
        require(amount * whitelistPrice <= msg.value, "WRONG_ETH_AMOUNT");
        require(whitelistMinted[msg.sender] + amount <= maxWhitelistPurchase, "EXCEEDS_ALLOWANCE"); 
...
/**
 * @title ERC721Whitelist
 * ERC721Whitelist - ERC721 contract that has a Merkle based whitelist
 */
abstract contract ERC721Whitelist is ERC721, Ownable {
   ...
   // The following functions are overrides required by Solidity.
    ...
    // get and set the whitelist sale state
    function setWhitelistSale(bool _setSaleState) public onlyOwner{
        whitelistSaleActive = _setSaleState;
    }    

    // set the whitelist root
