...
contract HexArt is ERC721URIStorage, ERC2981, Ownable {
    ...
    modifier isListedOnSale(uint256 _tokenId) {
        require(
    ...
    modifier isLocked(uint256 _nftTokenId) {
        require(
    ...
    function mint(
        ...
        assetLockPeriod[tokenId] = block.timestamp + (600 * _lockperiodInDays);
        emit nftMinted(tokenId, _feeNumerator, _lockperiodInDays);
    }
    ...
    /*
     * @dev Attach NFT(HSI) assets to a given hexart.
    ...
    /*
     * @dev  Remove NFT(HSI) assets to a given hexart.
     * @param _assetContract , NFT asset contract address.
    ...
    function removeNFTAsset(
        ...
        isLocked(_nftTokenId)
        isListedOnSale(_nftTokenId)
    {
       
        require(isLastOwner(_nftTokenId, msg.sender));
    ...
    function removeNFT(
        ...
        for (uint256 i = 0; i < _assetId.length; i++) {
            if (isAssetAttached(_nftTokenId, _assetId[i], _assetContract)) {
                require(
                    transferAsset(_assetContract, _assetId[i]),
                    "Asset not removed"
                );
                removeAssetToken(findIndexOfAsset(_assetId[i],_nftTokenId), _nftTokenId);
            }
        }
    }


    function findIndexOfAsset(uint256 _assetId,uint256 _nftTokenId) internal view returns(uint256){
        uint256 index;
        ...
        isListedOnSale(_tokenId)
    {
        ...
        require(
            ERC20(_assetContract).transferFrom(
                _msgSender(),
                address(this),
                _amount
            ),
    ...
    function isLastOwner(uint256 _tokenId, address _caller)
        internal
        view
        returns (bool)
    {
        if (
            (marketplace.getLastOwnerOfNft(_tokenId) == _caller ||
                initialOwner[_tokenId] == _caller)
        ) {
            return true;
        } else {
            return false;
