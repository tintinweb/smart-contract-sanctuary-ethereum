...
/**
 * @title The ERC721 contract for the Warped collection.
 *
 ...
 *   - Phase 3 - Public minting, any address can mint, as long as a result of
 *               that the address balance doesn't exceed MAX_PUBLIC_MINT. This
 *               is more like a soft rule to slow down any hogging of tokens, as
 *               this doesn't prevent someone from minting more than
 *               MAX_PUBLIC_MINT, by using multiple addresses, for example. 
 ...
 * PROVENANCE hash is published here. This will contain the hash of all the
 * bytes of the animation files put together in order - anyone can download all
 * these files and run a SHA256 hash on all the files to verify that they have
 * the expected content. Thus, any change to the images will be forever 
 ...
 * simply pause the minting.
 *
 */ 
contract WarpedCollection is ERC721, Ownable, Pausable {
    ...
    uint8 public constant MAX_OWNER_MINT = 100;

    // Keeps track of the number of tokens already minted by the owner.
    uint8 public ownerMintedCount;
    
    // Keeps track of how many tokens minted by each whitelisted address.
    mapping (address => uint8) public wlMintCount;
    ...
    // The address that will receive the funds and the owner minted tokens.
    address payable private _fundingAddr;
        ...
        ERC721("Warped", "WARP")
    {
        ...
        // Init the funding address with the owner address.
        _fundingAddr = payable(owner());
    ...
    /**
     * @dev Present to make sure this doesn't run when not paused.
     */
    ...
    /**
     * @notice Sets the address that receives the eth from the minting.
     * @param fundingAddr the funding address.
     */
    function setFundingAddress(address payable fundingAddr)
        public onlyOwner
    {
        _fundingAddr = fundingAddr;
    }

    /**
     * @notice Transfers all the minting funds to the funding address. 
     */
    function withdrawFunds() public onlyOwner {
        (bool success, ) = _fundingAddr.call{value: address(this).balance}("");
        require(success);
    }
     ...
     * @dev This matches a SHA256 hash of all the token image files put together.
     */
     ...
     *         all the token metadata JSONS. The full URI will be this base
     *         followed by the token id number.
    ...
    /**
     * @return true if whitelisted addresses can mint only 1 token in this phase.
     */
     ...
     *         correct amount of eth has to be sent with the transaction that
     *         calls this. This eth amount should be = MINT_PRICE * mintNum.
        ...
        public payable whenNotPaused
    {
        ...
        if (isWhitelistActive()) {
            ...
            // to in this phase.
            require(wlMintCount[_msgSender()] + mintNum <= maxAllowed,
                "EXCEEDS_WL_ALLOWED");

            // Update the counter of minted tokens by this whitelisted address.
            wlMintCount[_msgSender()] += mintNum;
        
        } else {
            // Public Phase

            // Make sure that the sender will not end up holding more tokens
            // than the ones allowed during the public minting phase.
            // NOTE: This will not stop an individual from minting more than
            //       that amount - this is more like a soft rule.
            require(balanceOf(_msgSender()) + mintNum <= MAX_PUBLIC_MINT,
     ...
     *         resulting tokens will be held by the funding account. This should
     *         fail during phases 0 and 4, when minting is deactivated for
     *         everyone.
     * @param mintNum the number of tokens to be minted.
     */
    function ownerMint(uint8 mintNum) public onlyOwner whenNotPaused {
        ...
        for (uint32 i = 0; i < mintNum; i++) {
            ...
            // Mint the token into the funding address.
            _safeMint(_fundingAddr, newTokenId);
        }
