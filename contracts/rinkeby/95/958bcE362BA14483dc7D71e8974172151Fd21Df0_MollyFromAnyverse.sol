...
contract MollyFromAnyverse is ERC721Enumerable, Ownable, Creators, RG, Signable {
    ...
    bool private NFTsReserved = false;
    ...
    uint256 public constant tokenPerAddress = 5;

    mapping(uint256 => address) public creatorOf;
    mapping(address => uint256) public chargeOf;
    ...
    function WLMint(uint256 amount, bytes calldata signature) public payable costs(mintPrice * amount) {
        _mint(amount, signature);
    }

    function _mint(uint256 amount, bytes calldata signature) private lock {
    ...
    function withdrawAll() public onlyCreators {
        ...
        _widthdraw(evolution, balance.mul(50).div(100));
        _widthdraw(devs, balance.mul(20).div(100));
        _widthdraw(artist, balance.mul(12).div(100));
        _widthdraw(storyTeller, balance.mul(10).div(100));
        _widthdraw(ceo, balance.mul(5).div(100));
        _widthdraw(social, balance.mul(3).div(100));
        _widthdraw(payback, address(this).balance);
    }
