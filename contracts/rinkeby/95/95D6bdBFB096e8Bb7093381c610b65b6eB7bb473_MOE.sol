  ...
  IERC721Enumerable
{
  ...
  function _safeMint(
    ...
    // We know if the first token in the batch doesnt exist, the other ones dont as well, because of serial ordering.
...
contract MOE is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public allowlistprice = 0.0000019 ether;
    uint256 public allowlistmaxmint = 10000;
    uint256 public waitlistprice = 0.0000019 ether;
    uint256 public waitlistmaxmint = 10000;
    uint256 public publicprice = 0.0000025 ether;
    uint256 public publicmaxmint = 10000;
    uint256 public collectionsize = 10000;
    uint256 public reservedsize = 100;

//minteléskor mintpause=1 !!!

    uint256 public mintpause;

//minteléskor a lenti =1 kivenni!!!
    uint256 public allowlistmintopen=1;
    uint256 public waitlistmintopen;
    uint256 public publicmintopen=1;
    uint256 public stakingpoolopen=1;
...
// StakeLog struct and mapping

//status -> 1 == staked; 2 == unstaked
...
constructor() ERC721A("Mosaique6", "MOE",10000,10000){}
...
//	safeTransferFrom(address(this), address(msg.sender), _id );
...
/*
//Ha a SYMBA-t bevezetjük, akkor!

    function getcoinamount(address addr,address coincontract) public view virtual returns (uint256) 
    {
        return  IERC20(coincontract).balanceOf(address(addr));
    }
*/

    function getholdertokens(address _owner) public view returns (uint[] memory) 
    {
        uint[] memory _tokensOfOwner = new uint[](ERC721A.balanceOf(_owner));
        uint i;

        for (i=0;i<ERC721A.balanceOf(_owner);i++){
            _tokensOfOwner[i] = ERC721A.tokenOfOwnerByIndex(_owner, i);
        }
