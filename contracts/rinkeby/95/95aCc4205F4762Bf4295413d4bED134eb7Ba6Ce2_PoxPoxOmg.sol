...
contract PoxPoxOmg is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  //uint256 public freeMintLimitBalance;

  bool public paused = true;
  bool public revealed = false;
  bool public freeMintComplianceEnabled = true;
  ...
  constructor(
    ...
    //uint256 _price,
    //uint256 _freeMintLimitBalance
  ) ERC721A("POXPOX OMG", "POX") {
    //maxSupply = _maxSupply;
    //setPrice(PRICE);
    //setMaxMintAmountPerTx(_maxMintAmountPerTx);
    setHiddenMetadataUri(_hiddenMetadataUri);
    //setFreeMintLimitBalance(_freeMintLimitBalance);
  ...
  modifier freeMintCompliance(address _address, uint256 _mintAmount) {
    if(freeMintComplianceEnabled){
      require(!freeMintClaimed[_msgSender()], 'Address already Minted!');
