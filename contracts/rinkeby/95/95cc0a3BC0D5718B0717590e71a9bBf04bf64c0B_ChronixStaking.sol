...
contract ChronixStaking is Initializable, ReentrancyGuardUpgradeable, ContextUpgradeable {
    ...
    function getPotTax(address staker) public view returns(uint amountEarned) {
        ...
        if(address(PushersNFT) != address(0)){
          if(ERC721A(PushersNFT).balanceOf(staker) !=0 && user.stakedChronix.length !=0 || ERC721A(ChronixNFT).balanceOf(staker) !=0){
           return (pot)*((ERC721A(PushersNFT).balanceOf(staker) * 10000) / ERC721A(PushersNFT).totalSupply()) /10000;
        }
        else {
          return 0;
        }
    ...
    function getPotTaxOf(uint amount,address staker) public view returns(uint) {
        ...
        if(user.stakedChronix.length !=0 && user.stakedPushers.length !=0){
           return (pot- user.ownPot)*((amount * 10000) / ERC721A(PushersNFT).totalSupply()) /10000;
        }
        else {
          return 0;
        }
