...
contract ERC1155Contract is ERC1155, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    modifier onlyBurner(uint256 _id) {
        require(msg.sender == tokenTypes[_id].burnerContract, "Not authorized to perform this action");
        _;
    }
    ...
    function setBurnerAddress(uint256 _id, address _address) external onlyOwner {
        tokenTypes[_id].burnerContract = _address;
    }

    function burnForAddress(uint256 _id, uint256 _quantity, address _address) external onlyBurner(_id) {
        require(tokenTypes[_id].totalSupply - _quantity >= 0, "Insufficient supply");
        tokenTypes[_id].totalSupply = tokenTypes[_id].totalSupply - _quantity;
    ...
    function maxSupply(uint256 _id) public view returns (uint24) {
        return tokenTypes[_id].maxSupply;
    }

    function preSalePrice(uint256 _id) public view returns (uint72) {
        return tokenTypes[_id].preSalePrice;
    }

    function pubSalePrice(uint256 _id) public view returns (uint72) {
        return tokenTypes[_id].pubSalePrice;
    }

    function maxPerWallet(uint256 _id) public view returns (uint24) {
        return tokenTypes[_id].maxPerWallet;
    }

    function maxPerTransaction(uint256 _id) public view returns (uint24) {
        return tokenTypes[_id].maxPerTransaction;
    }

    function preSaleIsActive(uint256 _id) public view returns (bool) {
        return tokenTypes[_id].preSaleIsActive;
    }

    function saleIsActive(uint256 _id) public view returns (bool) {
    ...
    function setClaimState(uint256 _id, bool _isActive) public onlyOwner {
        tokenTypes[_id].claimIsActive = _isActive;
    }

    function price(uint256 _id) public view returns (uint256) {
        return tokenTypes[_id].preSaleIsActive ? tokenTypes[_id].preSalePrice : tokenTypes[_id].pubSalePrice;
    }
    ...
    function claimFree(uint256 _id, bytes memory _data, uint256 _maxMint, uint256 _quantity, bytes32[] memory _proof) public {
        require(tokenTypes[_id].claimIsActive, "Free claim is not active.");
        uint256 _currentSupply = tokenTypes[_id].totalSupply;
        require(_currentSupply + _quantity <= tokenTypes[_id].maxSupply, "Requested quantity would exceed total supply.");
