...
contract Gloomz  is ERC721A , Ownable{
  ...
  uint256 public constant SUPPLY_CAP = 50;
  uint256 public constant AMOUNT_FREE_MINTS = 20;
  uint256 public  DEV_MINT_RESERVE = 25;
  ...
  function mintGloom(uint256 amount) external payable  {
    require(saleOpen== true, "Not open");
    uint256 ts = totalSupply();
    uint256 reserve = DEV_MINT_RESERVE;
    uint256 supplyCAp = SUPPLY_CAP-reserve-2;
    require(ts<=supplyCAp,"SUPPLY CAP REACHED");
    if(ts>= AMOUNT_FREE_MINTS ){
      require(amount<=MAX_PER_TX_FREE,"TO MANY MINTS ON FREE");
      require(msg.value >= amount* MINT_PRICE, "NOT ENUFF FUNDS SEND");
    }else{
      require(amount<=MAX_PER_TX_PUBLIC, "TO MANY NFTS ");
    }
  ...
  function _baseURI() internal view override returns(string memory ){
    return 'http://gloomz.io/metadata/';
  }
