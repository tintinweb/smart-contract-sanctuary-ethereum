...
contract RaffleMarketPlace is Ownable, ERC721Holder {
    using SafeMath for uint256;
    
    /* Fee Variables */
    uint256 public constant MAX_FEE_BY_MILLION = 300000; // 3%
    uint256 public s_registerFeeByMillion;
    ...
    mapping(uint256 => RaffleData) private s_raffles;
    mapping(uint256 => bool) private s_isActiveRaffle;

    uint256 public s_totalRaffleCounter;

    /* Events */
    event ChangedRegisterFee(uint256 registerFeeByMillion);
    event RaffleRegister(
        uint256 indexed totalRaffleCounter, 
    ...
    constructor() {
        ...
        s_registerFeeByMillion = 30000; // 3%
    }

    function setRegisterFeeByMillion(uint256 registerFeeByMillion) external onlyOwner {
        if (registerFeeByMillion > MAX_FEE_BY_MILLION) {
            revert Goobig__InvalidFee();
        }
        s_registerFeeByMillion = registerFeeByMillion;
        emit ChangedRegisterFee(registerFeeByMillion);
    }
    ...
    function registerRaffle(
        ...
        // if (IERC165(nftAddress).supportsInterface(_INTERFACE_ID_ERC721)) {
        //     revert Goobig__InvalidNFTAddress();            
        // }
        ...
        emit RaffleRegister(totalRaffleCounter, msg.sender, nftAddress, tokenId, ticketPrice, totalTickets, duration, block.timestamp);
    }
}
