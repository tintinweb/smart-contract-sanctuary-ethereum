...
contract PNFTTest1 is ERC721A , Ownable{  
    constructor() ERC721A("PNFTTest1", "PNFT1") {}  
	...
	function giveFeed(uint256 tokenId, uint256 feedAmt, address tokenOwner) public {
        ...
        feeds[tokenId] = Feed(block.timestamp, feedAmt, tokenOwner, 0, 0);
	...
	function takeFeed(uint256 tokenId, uint256 feedAmt) public {
		require(msg.sender == _stakeContract, "Only callable from Staking Contract");
        require(_exists(tokenId), "Token does not exist");
		require(feeds[tokenId].amt >= feedAmt, "Not enough Feed to remove");
		
		uint256 curAge = calcAge(tokenId);
			
		Feed memory tokenFeed = feeds[tokenId];
		tokenFeed.amt = tokenFeed.amt - feedAmt;
    ...
    function calcAge(uint256 tokenId) public view returns(uint256){
		...
		if (feeds[tokenId].ownerAddress == address(0x0000000000000000)){
			return curAge;
		}
		...
		uint256 tokenFactor = (tokenFeed.amt > TOKEN_FACTOR) ? TOKEN_FACTOR : tokenFeed.amt;
	...
	function timeUntilNextAge(uint256 tokenId) public view returns(uint256){
		...
		require(feeds[tokenId].amt > 0 , "NFT is not being Fed!");
		
		Feed memory tokenFeed = feeds[tokenId];
		uint256 totalFeedAmt = (TOKEN_FACTOR / tokenFeed.amt) * TIME_FACTOR;
