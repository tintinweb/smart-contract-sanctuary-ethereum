...
contract Marketplace is ERC721Enumerable, Ownable{
    ...
    uint256 public productTransactionCount = 0;
    uint256 public loyaltyValue = 10;
    ...
    constructor() ERC721() {
        nameContract = "NFT Buruan";
        ownerContract = payable(msg.sender);
    }

    function Mint(string memory _id, string memory _name, uint256 _price, string memory _detailProductParam) public payable{
        // Require a id
        require(bytes(_id).length > 0);
        // Require product not exists
        ...
        // Create the product
        products[_id] = Product(_id, _name, _price, _detailProductParam, payable(msg.sender), false, true);
        // Transfer to owner
        ownerContract.transfer(msg.value);
        // Trigger an event
        emit ProductCreated(_id, _name, _price, _detailProductParam, payable(msg.sender),  true, true);
    }

    function updateStatusPrice(string memory _id, uint256 _price, bool _onSale) public {
        // Fetch the product
    ...
    function buyItem(string memory _id) public payable {
        // Fetch the product
        Product memory _product = products[_id];
        // Fetch the owner
        ...
        products[_id] = _product;
        // Pay the seller 
        _seller.transfer(msg.value * (100 - loyaltyValue) / 100);
        // Pay the loyalty to owner
        ownerContract.transfer(msg.value * loyaltyValue / 100);
        // Increment Product Transaction
        productTransactionCount ++;
        // Trigger an event
        emit ProductResell(_id, _product.name, _product.price, payable(msg.sender), false);
    }
}