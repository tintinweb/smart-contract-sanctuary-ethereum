...
abstract contract TokenMintingPool is VRFConsumerBase {
    ...
    function fulfillRandomness(bytes32, uint256 randomness) internal override {
        if (!tokenSupplyLocked()) {
            _randomSeeds = _expandRandomness(randomness);
        }
    }

    /**
     * @dev From a single VRF Coordinator Random Value, we generate 6 different seeds
     */
    function _expandRandomness(uint256 randomValue) internal pure returns (uint256[] memory expandedSeeds) {
    expandedSeeds = new uint256[](6);
    for (uint256 i = 0; i < 6; i++) {
        expandedSeeds[i] = uint256(keccak256(abi.encode(randomValue, i)));
    }
    ...
    function tokenSupplyLocked() public view returns (bool) {
        uint256[] memory nullArray = new uint256[](6);
        return (keccak256(abi.encode(_randomSeeds)) != keccak256(abi.encode(nullArray)));
    }
