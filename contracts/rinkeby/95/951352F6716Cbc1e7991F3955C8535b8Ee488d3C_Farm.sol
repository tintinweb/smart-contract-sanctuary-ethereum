...
abstract contract Horser {
    ...
    function checkStable(uint256 horseID)
        public
        view
        virtual
        returns (uint256[] memory);

    function setStable(uint256 horseID, uint256 farmID) external virtual;
}
...
contract Farm is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    mapping(uint256 => uint256[]) private horses;
    //maps farm id to horse ids
    mapping(uint256 => uint256[]) private stableparams;
    mapping(uint256 => StableRequest[]) private stablerequests;
    ...
    function setHorseGovAddress(address contractAddress) public onlyOwner {
        HorseGovAddress = contractAddress;
    }
    ...
    function wrapLandtoFarmReq(uint256[] memory ids, string memory farmName)
        external
    {
    ...
    function stableHorse(uint256 horseID, uint256 farmID) public payable {
        require(farms[farmID].length > 0);
        require(
            (horses[farmID].length - farms[farmID].length) > 0,
        ...
        if (
            ...
            uint256[] memory stable = HorseContract.checkStable(horseID);
            if (stable[0] == 0) {
                horses[farmID].push(horseID);
            } else {
                if (block.timestamp < stable[1]) {
                    require(
                        msg.value >= stableparams[farmID][0],
                        "MUST SEND >= ETH AMOUNT TO BREAK CONTRACT"
                    );
                    horses[farmID].push(horseID);
                    HorseContract.setStable(horseID, farmID);
                } else {
                    horses[farmID].push(horseID);
                    HorseContract.setStable(horseID, farmID);
                }
            ...
            uint256[] memory stable = HorseContract.checkStable(horseID);
            if (stable[0] == 0) {
                stablerequests[farmID].push(StableRequest(horseID, msg.sender));
            } else {
                if (block.timestamp < stable[1]) {
                    require(
                        msg.value >= stableparams[farmID][0],
                    ...
                    stablerequests[farmID].push(
                        StableRequest(horseID, msg.sender)
                    );
                } else {
                    stablerequests[farmID].push(
                        StableRequest(horseID, msg.sender)
