    ...
    PaymentSplitter 
{
    ...
    uint256 public totalMaxSupply = 300; //maxSupplay  ---> totalMaxSupply
    ...
    bool public presale = false;   //presaleM ---> presale
    bool public mint = false;    //publicM  ---> mint

    uint256 presaleAmount = 3;  //presaleAmountLimit --->  presaleAmount
    ...
    onlyAccounts
    {
        ...
        require(presale,                       " Presale is OFF");
        require(!paused,                        "Contract is paused");
        require(
            _amount <= presaleAmount,      "You can't mint so much tokens");
        require(
            presaleClaimed[msg.sender] + _amount <= presaleAmount,  " You can't mint so much tokens");


        uint current = _tokenIds.current();

        require(
            current + _amount <= totalMaxSupply,
            "max supply exceeded"
        );
        require(
    ...
    onlyAccounts
    {
        require(mint,                        "PublicSale is OFF");
        require(!paused, "Contract is paused");
    ...
    function refundGuaranteeActive() public view returns (bool) {
        return (block.timestamp < (saleEndTime + 30 days));
    }
