...
// File: contracts/interfaces/IAvnValidatorsManagerV2.sol


pragma solidity 0.8.11;

interface IAvnValidatorsManagerV2 {
  event LogValidatorDepositUpdated(uint256 validatorDeposit);
  event LogValidatorChallengeWindowUpdated(uint256 challengeWindowInSeconds);
  event LogQuorumUpdated(uint256[2] quorum);
  event LogValidatorRegistered(bytes32 indexed t1PublicKeyLHS, bytes32 indexed t1PublicKeyRHS, bytes32 indexed t2PublicKey,
  ...
  function setValidatorChallengeWindow(uint256 challengeWindowInSeconds) external;
  function setQuorum(uint256[2] calldata quorum) external;
  function upgradeAvn(address avnValidatorsManagerV1) external;
  function registerValidator(bytes calldata t1PublicKey, bytes32 t2PublicKey) external;
  function activateValidator(bytes32 targetT2PublicKey, uint256 t2TransactionId, bytes calldata confirmations) external;
  function deregisterValidator(bytes32 targetT2PublicKey, uint256 t2TransactionId, bytes calldata confirmations) external;
  function claimValidatorDeposit(bytes32 t2PublicKey) external;
...
// File: ../contracts/AvnValidatorsManagerV2.sol
...
contract AvnValidatorsManagerV2 is IAvnValidatorsManagerV2, Owned {
  ...
  constructor(IERC20 _avt, IAvnStorage _avnStorage, IAvnFTTreasuryV2 _avnFTTreasuryV2)
  {
    avt = _avt;
    avnStorage = _avnStorage;
  ...
  modifier onlyWhenValidatorFunctionsEnabled() {
    require(validatorFunctionsDisabled == false && avnUpgraded, "Function currently disabled");
    _;
  }
  ...
  function upgradeAvn(address _avnValidatorsManagerV1)
    onlyOwner
    external
  {
    require(avnUpgraded == false, "AVN already upgraded");
    AvnValidatorsManager v1 = AvnValidatorsManager(_avnValidatorsManagerV1);
    validatorDeposit = v1.validatorDeposit();
    validatorChallengeWindowInSeconds = v1.validatorChallengeWindowInSeconds();
    uint256[2] memory v1Quorum = [v1.quorum(0), v1.quorum(1)];
    setQuorum(v1Quorum);
    validatorIdNum = v1.validatorIdNum();
    numActiveValidators = v1.numActiveValidators();

    for (uint256 i = 1; i < validatorIdNum; i++) {
      t1Address[i] = v1.t1Address(i);
      t2PublicKey[i] = v1.t2PublicKey(i);
      if (v1.deposit(i) > 0) deposit[i] = v1.deposit(i);
      if (v1.isRegistered(i)) isRegistered[i] = true;
      if (v1.isActive(i)) isActive[i] = true;
      if (v1.isDeregistered(i)) isDeregistered[i] = true;
      if (v1.challengeEnd(i) > 0) challengeEnd[i] = v1.challengeEnd(i);
      idByT1Address[t1Address[i]] = v1.idByT1Address(t1Address[i]);
      idByT2PublicKey[t2PublicKey[i]] = v1.idByT2PublicKey(t2PublicKey[i]);
    }
  ...
  {
    uint256 targetId = idByT2PublicKey[_targetT2PublicKey];
    require(isActive[targetId] == false, "Target already active");
    require(isRegistered[targetId], "Target must be registered");
    verifyConfirmations(toConfirmationHash(_targetT2PublicKey, _t2TransactionId), _confirmations);
    isActive[targetId] = true;
    numActiveValidators++;
    ...
    external
  {
    ...
    deposit[id] = 0;
    challengeEnd[id] = 0;
    unlockAVTFromTreasury(t1Address[id], lockedDeposit);
    emit LogValidatorDepositClaimed(t1Address[id]);
  }
    ...
    external
  {
    ...
    deposit[targetId] = 0;
    challengeEnd[targetId] = 0;
    unlockAVTFromTreasury(owner, lockedDeposit);
    emit LogValidatorSlashed(t1Address[targetId], _targetT2PublicKey, _t2TransactionId, lockedDeposit);
  }
  ...
  {
    uint256 id = idByT1Address[_t1Address];
    require(isRegistered[id] == false, "Already registered");
    checkT1PublicKey(_t1Address, _t1PublicKey);
    ...
    if (isDeregistered[id]) {
      ...
      challengeEnd[id] = 0;
      if (existingDeposit > validatorDeposit) {
        unlockAVTFromTreasury(_t1Address, existingDeposit - validatorDeposit);
      } else if (existingDeposit < validatorDeposit) {
        lockAVTInTreasury(_t1Address, validatorDeposit - existingDeposit);
      }
    } else {
  ...
  function unlockAVTFromTreasury(address _recipient, uint256 _amount)
    private
  {
    avnFTTreasuryV2.unlockERC20Tokens(address(avt), _recipient, _amount);
  }
    ...
    private
  {
    require(avt.balanceOf(_t1Address) >= _amount, 'Insufficient AVT funds');
    require(avt.allowance(_t1Address, address(this)) >= _amount, 'AVT amount requires approval');
    assert(avt.transferFrom(_t1Address, address(avnFTTreasuryV2), _amount));
  }
    ...
    view
  {
    ...
    for (uint256 i; i < numConfirmations; i++) {
      ...
      if (v != 27 && v != 28 || uint256(s) > 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0) {
        ...
        id = idByT1Address[ecrecover(ethSignedPrefixMsgHash, v, r, s)];
        if (isActive[id] && confirmed[id] == false) {
          if (++validConfirmations == requiredConfirmations) break;
          confirmed[id] = true;
        }
      }
