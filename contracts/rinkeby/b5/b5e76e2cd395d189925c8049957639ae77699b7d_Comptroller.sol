...
import "./Governance/Safelend.sol";

/**
 * @title Safelend's Comptroller Contract
 * @author Safelend
 */
contract Comptroller is ComptrollerV3Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    event MarketComped(SToken sToken, bool isComped);

    /// @notice Emitted when SFL rate is changed
    event NewCompRate(uint oldCompRate, uint newCompRate);

    /// @notice Emitted when a new SFL speed is calculated for a market
    event CompSpeedUpdated(SToken indexed sToken, uint newSpeed);

    /// @notice Emitted when SFL is distributed to a supplier
    event DistributedSupplierComp(SToken indexed sToken, address indexed supplier, uint compDelta, uint compSupplyIndex);

    /// @notice Emitted when SFL is distributed to a borrower
    event DistributedBorrowerComp(SToken indexed sToken, address indexed borrower, uint compDelta, uint compBorrowIndex);

    /// @notice The threshold above which the flywheel transfers SFL, in wei
    uint public constant compClaimThreshold = 0.001e18;

    /// @notice The initial SFL index for a market
    ...
    function exitMarket(address cTokenAddress) external returns (uint) {
        SToken sToken = SToken(cTokenAddress);
    ...
    function mintAllowed(address sToken, address minter, uint mintAmount) external returns (uint) {
        ...
        updateCompSupplyIndex(sToken);
        distributeSupplierComp(sToken, minter, false);
    ...
    function redeemAllowed(address sToken, address redeemer, uint redeemTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateCompSupplyIndex(sToken);
        distributeSupplierComp(sToken, redeemer, false);
    ...
    function borrowAllowed(address sToken, address borrower, uint borrowAmount) external returns (uint) {
        ...
        updateCompBorrowIndex(sToken, borrowIndex);
        distributeBorrowerComp(sToken, borrower, borrowIndex, false);
    ...
    function liquidateBorrowAllowed(
        ...
        uint borrowBalance = SToken(cTokenBorrowed).borrowBalanceStored(borrower);
    ...
    function seizeAllowed(
        ...
        if (SToken(cTokenCollateral).comptroller() != SToken(cTokenBorrowed).comptroller()) {
    ...
    function transferAllowed(address sToken, address src, address dst, uint transferTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateCompSupplyIndex(sToken);
        distributeSupplierComp(sToken, src, false);
        distributeSupplierComp(sToken, dst, false);
     ...
     *  Note that `cTokenBalance` is the number of sTokens the account owns in the market,
    ...
    function getHypotheticalAccountLiquidity(
        ...
        (Error err, uint liquidity, uint shortfall) = getHypotheticalAccountLiquidityInternal(account, SToken(cTokenModify), redeemTokens, borrowAmount);
        return (uint(err), liquidity, shortfall);
    }
     ...
     * @param cTokenBorrowed The address of the borrowed sToken
     * @param cTokenCollateral The address of the collateral sToken
     * @param actualRepayAmount The amount of cTokenBorrowed underlying to convert into cTokenCollateral tokens
     * @return (errorCode, number of cTokenCollateral tokens to be seized in a liquidation)
     */
    function liquidateCalculateSeizeTokens(address cTokenBorrowed, address cTokenCollateral, uint actualRepayAmount) external view returns (uint, uint) {
        /* Read oracle prices for borrowed and collateral markets */
        uint priceBorrowedMantissa = oracle.getUnderlyingPrice(SToken(cTokenBorrowed));
        uint priceCollateralMantissa = oracle.getUnderlyingPrice(SToken(cTokenCollateral));
         ...
         */
        uint exchangeRateMantissa = SToken(cTokenCollateral).exchangeRateStored(); // Note: reverts on error
        uint seizeTokens;
    ...
    function _supportMarket(SToken sToken) external returns (uint) {
        ...
        markets[address(sToken)] = Market({isListed: true, isComped: false, collateralFactorMantissa: 0});
    ...
    /**
     * @notice Recalculate and update SFL speeds for all SFL markets
     */
    ...
    function refreshCompSpeedsInternal() internal {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            updateCompSupplyIndex(address(sToken));
            updateCompBorrowIndex(address(sToken), borrowIndex);
        }
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            SToken sToken = allMarkets_[i];
            if (markets[address(sToken)].isComped) {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            uint newSpeed = totalUtility.mantissa > 0 ? mul_(sflRate, div_(utilities[i], totalUtility)) : 0;
            sflSpeeds[address(sToken)] = newSpeed;
            emit CompSpeedUpdated(sToken, newSpeed);
        }
    }

    /**
     * @notice Accrue SFL to the market by updating the supply index
     * @param sToken The market whose supply index to update
     */
    function updateCompSupplyIndex(address sToken) internal {
        SflMarketState storage supplyState = sflSupplyState[sToken];
        uint supplySpeed = sflSpeeds[sToken];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, uint(supplyState.block));
        if (deltaBlocks > 0 && supplySpeed > 0) {
            uint supplyTokens = SToken(sToken).totalSupply();
            uint sflAccrued = mul_(deltaBlocks, supplySpeed);
            Double memory ratio = supplyTokens > 0 ? fraction(sflAccrued, supplyTokens) : Double({mantissa: 0});
            Double memory index = add_(Double({mantissa: supplyState.index}), ratio);
            sflSupplyState[sToken] = SflMarketState({
    ...
    /**
     * @notice Accrue SFL to the market by updating the borrow index
     * @param sToken The market whose borrow index to update
     */
    function updateCompBorrowIndex(address sToken, Exp memory marketBorrowIndex) internal {
        SflMarketState storage borrowState = sflBorrowState[sToken];
        uint borrowSpeed = sflSpeeds[sToken];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, uint(borrowState.block));
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint borrowAmount = div_(SToken(sToken).totalBorrows(), marketBorrowIndex);
            uint sflAccrued = mul_(deltaBlocks, borrowSpeed);
            Double memory ratio = borrowAmount > 0 ? fraction(sflAccrued, borrowAmount) : Double({mantissa: 0});
            Double memory index = add_(Double({mantissa: borrowState.index}), ratio);
            sflBorrowState[sToken] = SflMarketState({
                index: safe224(index.mantissa, "new index exceeds 224 bits"),
                block: safe32(blockNumber, "block number exceeds 32 bits")
            });
        } else if (deltaBlocks > 0) {
    ...
    /**
     * @notice Calculate SFL accrued by a supplier and possibly transfer it to them
     * @param sToken The market in which the supplier is interacting
     * @param supplier The address of the supplier to distribute SFL to
     */
    function distributeSupplierComp(address sToken, address supplier, bool distributeAll) internal {
        SflMarketState storage supplyState = sflSupplyState[sToken];
        Double memory supplyIndex = Double({mantissa: supplyState.index});
        Double memory supplierIndex = Double({mantissa: sflSupplierIndex[sToken][supplier]});
        sflSupplierIndex[sToken][supplier] = supplyIndex.mantissa;
        ...
        uint supplierAccrued = add_(sflAccrued[supplier], supplierDelta);
        sflAccrued[supplier] = transferComp(supplier, supplierAccrued, distributeAll ? 0 : compClaimThreshold);
        emit DistributedSupplierComp(SToken(sToken), supplier, supplierDelta, supplyIndex.mantissa);
    }

    /**
     * @notice Calculate SFL accrued by a borrower and possibly transfer it to them
     * @dev Borrowers will not begin to accrue until after the first interaction with the protocol.
     * @param sToken The market in which the borrower is interacting
     * @param borrower The address of the borrower to distribute SFL to
     */
    function distributeBorrowerComp(address sToken, address borrower, Exp memory marketBorrowIndex, bool distributeAll) internal {
        SflMarketState storage borrowState = sflBorrowState[sToken];
        Double memory borrowIndex = Double({mantissa: borrowState.index});
        Double memory borrowerIndex = Double({mantissa: sflBorrowerIndex[sToken][borrower]});
        sflBorrowerIndex[sToken][borrower] = borrowIndex.mantissa;

        if (borrowerIndex.mantissa > 0) {
            Double memory deltaIndex = sub_(borrowIndex, borrowerIndex);
            uint borrowerAmount = div_(SToken(sToken).borrowBalanceStored(borrower), marketBorrowIndex);
            uint borrowerDelta = mul_(borrowerAmount, deltaIndex);
            uint borrowerAccrued = add_(sflAccrued[borrower], borrowerDelta);
            sflAccrued[borrower] = transferComp(borrower, borrowerAccrued, distributeAll ? 0 : compClaimThreshold);
            emit DistributedBorrowerComp(SToken(sToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }

    /**
     * @notice Transfer SFL to the user, if they are above the threshold
     * @dev Note: If there is not enough SFL, we do not perform the transfer all.
     * @param user The address of the user to transfer SFL to
     * @param userAccrued The amount of SFL to (possibly) transfer
     * @return The amount of SFL which was NOT transferred to the user
     */
    function transferComp(address user, uint userAccrued, uint threshold) internal returns (uint) {
        if (userAccrued >= threshold && userAccrued > 0) {
            Safelend comp = Safelend(getSflAddress());
     ...
     * @param holder The address to claim SFL for
     */
     ...
     * @param holder The address to claim SFL for
     * @param sTokens The list of markets to claim SFL in
     */
    function claimComp(address holder, SToken[] memory sTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimComp(holders, sTokens, true, true);
    }

    /**
     * @notice Claim all comp accrued by the holders
     * @param holders The addresses to claim SFL for
     * @param sTokens The list of markets to claim SFL in
     * @param borrowers Whether or not to claim SFL earned by borrowing
     * @param suppliers Whether or not to claim SFL earned by supplying
     */
    function claimComp(address[] memory holders, SToken[] memory sTokens, bool borrowers, bool suppliers) public {
        for (uint i = 0; i < sTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint j = 0; j < holders.length; j++) {
                    distributeBorrowerComp(address(sToken), holders[j], borrowIndex, true);
                }
            }
            if (suppliers == true) {
                updateCompSupplyIndex(address(sToken));
                for (uint j = 0; j < holders.length; j++) {
                    distributeSupplierComp(address(sToken), holders[j], true);
                }
            }
        }
    }
    ...
    /**
     * @notice Set the amount of SFL distributed per block
     * @param compRate_ The amount of SFL wei per block to distribute
     */
    function _setCompRate(uint compRate_) public {
        require(adminOrInitializing(), "only admin can change comp rate");

        uint oldRate = sflRate;
        sflRate = compRate_;
    ...
    /**
     * @notice Add markets to compMarkets, allowing them to earn SFL in the flywheel
     * @param sTokens The addresses of the markets to add
     */
    function _addCompMarkets(address[] memory sTokens) public {
        require(adminOrInitializing(), "only admin can add comp market");

        for (uint i = 0; i < sTokens.length; i++) {
            _addCompMarketInternal(sTokens[i]);
        }

        refreshCompSpeedsInternal();
    }

    function _addCompMarketInternal(address sToken) internal {
        ...
        emit MarketComped(SToken(sToken), true);

        if (sflSupplyState[sToken].index == 0 && sflSupplyState[sToken].block == 0) {
            sflSupplyState[sToken] = SflMarketState({
        ...
        if (sflBorrowState[sToken].index == 0 && sflBorrowState[sToken].block == 0) {
            sflBorrowState[sToken] = SflMarketState({
                index: compInitialIndex,
                block: safe32(getBlockNumber(), "block number exceeds 32 bits")
            });
    ...
    /**
     * @notice Remove a market from compMarkets, preventing it from earning SFL in the flywheel
     * @param sToken The address of the market to drop
     */
    function _dropCompMarket(address sToken) public {
        ...
        emit MarketComped(SToken(sToken), false);
    ...
    /**
     * @notice Return the address of the SFL token
     * @return The address of SFL
     */
    function getSflAddress() public view returns (address) {
        return 0xA64bF6ee9E986200cDad3C47155a494185f7c0D9;
    }
}
...
contract Safelend {
    ...
    string public constant name = "Safelend";
    ...
    uint public constant totalSupply = 10000000e18; // 10 million SFL
...
contract ComptrollerV2Storage is ComptrollerV1Storage {
    struct Market {
        ...
        /// @notice Whether or not this market receives SFL
        bool isComped;
    }
...
contract ComptrollerV3Storage is ComptrollerV2Storage {
    struct SflMarketState {
        /// @notice The market's last updated sflBorrowIndex or sflSupplyIndex
        uint224 index;
    ...
    /// @notice The rate at which the flywheel distributes SFL, per block
    uint public sflRate;

    /// @notice The portion of sflRate that each market currently receives
    mapping(address => uint) public sflSpeeds;

    /// @notice The SFL market supply state for each market
    mapping(address => SflMarketState) public sflSupplyState;

    /// @notice The SFL market borrow state for each market
    mapping(address => SflMarketState) public sflBorrowState;

    /// @notice The SFL borrow index for each market for each supplier as of the last time they accrued SFL
    mapping(address => mapping(address => uint)) public sflSupplierIndex;

    /// @notice The SFL borrow index for each market for each borrower as of the last time they accrued SFL
    mapping(address => mapping(address => uint)) public sflBorrowerIndex;

    /// @notice The SFL accrued but not yet transferred to each user
    mapping(address => uint) public sflAccrued;
}
