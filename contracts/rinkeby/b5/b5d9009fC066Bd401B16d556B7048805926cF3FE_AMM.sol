...
import "contracts/AMM/interfaces/IAMM.sol";
import "contracts/AMM/interfaces/IERC20.sol";
import "contracts/AMM/interfaces/ILPToken.sol";
import "contracts/AMM/interfaces/IFutureVault.sol";
import "contracts/AMM/interfaces/IFutureWallet.sol";
import "contracts/AMM/interfaces/IController.sol";
import "./library/AMMMaths.sol";
import "contracts/AMM/RoleCheckable.sol";
...
contract AMM is IAMM, RoleCheckable {
    ...
    bytes32 internal constant ROUTER_ROLE =
    ...
    uint256 private constant MIN_LIQUIDITY_AMOUNT = 10**4;

    uint256 private _status;

    mapping(uint256 => mapping(uint256 => uint256))
        private poolToUnderlyingAtPeriod;
    ...
    event LiquidityCreated(address indexed _user, uint256 indexed _pairID);
    event PoolJoined(
    ...
    event WeightUpdated(address indexed _token, uint256[2] _newWeights);
    event ExpiredTokensWithdrawn(address indexed _user, uint256 _amount);
    ...
    function initialize(
        ...
        require(
            _poolTokens.supportsInterface(ERC1155_ERC165),
            "AMM: Interface not supported"
        );
        require(
            _underlyingOfIBTAddress != address(0),
            "AMM: Invalid underlying address"
        );
        require(_futureVault != address(0), "AMM: Invalid future address");
        require(_admin != address(0), "AMM: Invalid admin address");
        require(
            _feesRecipient != address(0),
            "AMM: Invalid fees recipient address"
        );
    ...
    function togglePauseAmm() external override isAdmin {
        ...
        state = state == AMMGlobalState.Activated
            ? AMMGlobalState.Paused
            : AMMGlobalState.Activated;
    ...
    function switchPeriod() external override {
        ammIsActive();
        require(
            futureVault.getCurrentPeriodIndex() > currentPeriodIndex,
            "AMM: Invalid period index"
        );
        if (futureVault.isTerminated()) {
            state = AMMGlobalState.Terminated;
            emit AMMStateChanged(state);
            underlyingSavedPerPeriod[currentPeriodIndex] = pairs[0].balances[1];
            uint256 oldIBTBalance = ibt.balanceOf(address(this));
            uint256 ptBalancePool0 = pairs[0].balances[0];
            uint256 ptBalancePool1 = pairs[1].balances[0];
            require(
                ptBalancePool0 + ptBalancePool1 > 0,
                "AMM: Pools have 0 PT balance"
            );
            uint256 ratioPTPool0 = ptBalancePool0.mul(IBT_UNIT).div(
                ptBalancePool0 + ptBalancePool1
            );
            IFutureWallet(futureVault.getFutureWalletAddress()).redeemYield(
                currentPeriodIndex
            ); // redeem ibt from expired ibt
            uint256 IBTBalance = ibt.balanceOf(address(this));
            uint256 diffIBTBalance = IBTBalance.sub(oldIBTBalance);
            IController(futureVault.getControllerAddress())
                .exitTerminatedFuture(futureVault);
            uint256 newIBTBalance = ibt.balanceOf(address(this));
            uint256 newDiffIBTBalance = newIBTBalance.sub(IBTBalance);
            _saveExpiredIBTs(
                0,
                (ratioPTPool0 * newDiffIBTBalance).div(IBT_UNIT),
                currentPeriodIndex
            );
            _saveExpiredIBTs(
                1,
                ((IBT_UNIT - ratioPTPool0) * newDiffIBTBalance).div(IBT_UNIT) +
                    diffIBTBalance,
                currentPeriodIndex
            );
            delete pairs[0];
        ...
        generatedYieldAtPeriod[currentPeriodIndex] = futureVault
            .getYieldOfPeriod(currentPeriodIndex);
        currentPeriodIndex = futureVault.getCurrentPeriodIndex();
    ...
    function _renewUnderlyingPool() internal {
        ...
        if (ptBalance != 0) {
            IController(futureVault.getControllerAddress()).withdraw(
                futureVault,
                ptBalance
            );
        }
        _saveExpiredIBTs(
            0,
            ibt.balanceOf(address(this)).sub(oldIBTBalance),
            currentPeriodIndex
        );
        _resetPair(0);
    }
    ...
    function _renewFYTPool() internal {
        address fytAddress = futureVault.getFYTofPeriod(
            futureVault.getCurrentPeriodIndex()
        );
        ...
        if (ptBalance != 0) {
            ...
            IController(futureVault.getControllerAddress()).withdraw(
                futureVault,
                ptBalance
            ); // withdraw current pt and generated fyt
        }
        _saveExpiredIBTs(
            1,
            ibt.balanceOf(address(this)).sub(oldIBTBalance),
    ...
    function _saveExpiredIBTs(
        ...
        poolToUnderlyingAtPeriod[_pairID][_periodID] = futureVault
            .convertIBTToUnderlying(_ibtGenerated);
    }
    ...
    function _getUpdatedUnderlyingWeightAndYield()
        internal
        view
        returns (uint256, uint256)
    {
        uint256 inverseSpotPrice = (AMMMaths.SQUARED_UNIT).div(
            getSpotPrice(0, 1, 0)
        );
        uint256 yieldRecorded = futureVault.convertIBTToUnderlying(
            futureVault.getUnrealisedYieldPerPT()
        );
        if (
            lastBlockYieldRecorded != block.number &&
            lastYieldRecorded != yieldRecorded
            ...
            uint256 newSpotPrice = (
                (AMMMaths.UNIT + yieldRecorded).mul(AMMMaths.SQUARED_UNIT)
            ).div(((AMMMaths.UNIT + lastYieldRecorded).mul(inverseSpotPrice)));
            if (newSpotPrice < AMMMaths.UNIT) {
                uint256[2] memory balances = pairs[0].balances;
                uint256 newUnderlyingWeight = balances[1]
                    .mul(AMMMaths.UNIT)
                    .div(
                        balances[1].add(
                            balances[0].mul(newSpotPrice).div(AMMMaths.UNIT)
                        )
                    );
     ...
     */
    function withdrawExpiredToken(address _user, uint256 _lpTokenId)
        external
        override
    {
        nonReentrant();
    ...
    function _withdrawExpiredToken(address _user, uint256 _lpTokenId) internal {
        (
            ...
            uint256 pairId
        ) = getExpiredTokensInfo(_user, _lpTokenId);
        ...
        if (pairId == 0) {
            uint256 userUnderlyingAmount = underlyingSavedPerPeriod[
                lastPeriodId
            ].mul(userTotal).div(tokenSupply);
    ...
    {
        require(
            poolTokens.getAMMId(_lpTokenId) == ammId,
            "AMM: invalid amm id"
        );
        ...
        if (periodIndex == 0 || periodIndex == currentPeriodIndex)
            return (0, periodIndex, pairID);
        ...
        for (uint256 i = periodIndex.add(1); i < currentPeriodIndex; i++) {
            redeemable = redeemable
                .mul(
                    AMMMaths.UNIT.add(
                        futureVault.convertIBTToUnderlying(
                            generatedYieldAtPeriod[i]
                        )
                    )
                )
                .div(AMMMaths.UNIT);
        ...
        returns (uint256 tokenAmountOut, uint256 spotPriceAfter)
    {
        ...
        require(
            weights[_tokenIn] > 0 && weights[_tokenOut] > 0,
            "AMM: Invalid token address"
        );
        ...
        returns (uint256 tokenAmountIn, uint256 spotPriceAfter)
    {
        ...
        (tokenAmountIn, spotPriceAfter) = calcInAndSpotGivenOut(
            _pairID,
            _tokenIn,
            _maxAmountIn,
            _tokenOut,
        ...
        returns (uint256 tokenAmountIn, uint256 spotPriceAfter)
    {
        ...
        require(
            tokenWeightIn > 0 && tokenWeightOut > 0,
            "AMM: Invalid token address"
        );
    ...
    function joinSwapExternAmountIn(
        ...
        // require(
        //     _tokenAmountIn <=
        //         inTokenBalance.mul(AMMMaths.MAX_IN_RATIO) / AMMMaths.UNIT,
        //     "AMM: Max in ratio reached"
        // );
    ...
    function exitSwapPoolAmountIn(
        ...
        // require(
        //     tokenAmountOut <=
        //         outTokenBalance.mul(AMMMaths.MAX_OUT_RATIO) / AMMMaths.UNIT,
        ...
        override
    {
        ...
        // ammIsActive();
        // require(
        //     !pairs[_pairID].liquidityIsInitialized,
        //     "AMM: Liquidity already present"
        // );
        // require(
        //     _tokenAmounts[0] > MIN_LIQUIDITY_AMOUNT &&
        //         _tokenAmounts[1] > MIN_LIQUIDITY_AMOUNT,
        //     "AMM: Insufficient token amount"
        // );
    ...
    function _pullToken(
        ...
        // address _tokenIn = _tokenID == 0
        //     ? address(pt)
        //     : pairs[_pairID].tokenAddress;
        // pairs[_pairID].balances[_tokenID] = pairs[_pairID]
        //     .balances[_tokenID]
        //     .add(_amount);
    ...
    function addLiquidity(
        ...
        // for (uint256 i; i < 2; i++) {
        //     uint256 amountIn = _computeAmountWithShares(
        //         pairs[_pairID].balances[i],
        //         _poolAmountOut,
        //         poolTotal
        //     );
    ...
    function removeLiquidity(
        ...
        // uint256 poolTotal = totalLPSupply[_pairID][currentPeriodIndex];
        // require(
        //     poolTotal >= _poolAmountIn + MIN_LIQUIDITY_AMOUNT,
        //     "AMM: Cannot remove all liquidity"
        // );

        // for (uint256 i; i < 2; i++) {
        //     uint256 amountOut = _computeAmountWithShares(
        //         pairs[_pairID].balances[i],
        //         _poolAmountIn,
        //         poolTotal
        //     );
        ...
        //         _pairID,
        //         i,
        //         amountOut.mul(AMMMaths.UNIT.sub(AMMMaths.EXIT_FEE)).div(
        //             AMMMaths.UNIT
        //         )
        //     );
        // }
    ...
    function _joinPool(
        ...
        //     _user,
        //     ammId,
        //     uint64(currentPeriodIndex),
        //     uint32(_pairID),
        //     _amount,
        //     bytes("")
        // );
        // totalLPSupply[_pairID][currentPeriodIndex] = totalLPSupply[_pairID][
        //     currentPeriodIndex
        // ].add(_amount);
    ...
    function rescueFunds(IERC20 _token, address _recipient) external isAdmin {
        require(
            _token != ibt &&
                _token != pt &&
                _token != underlyingOfIBT &&
                _token != fyt,
            "AMM: Invalid token rescued"
        );
        ...
        returns (uint256)
    {
        if (tokenToPairID[_tokenAddress] == 0)
            require(
                pairs[0].tokenAddress == _tokenAddress ||
                    _tokenAddress == address(pt),
                "AMM: invalid token address"
            );
     ...
     */
    function tokenIdsAreValid(uint256 _tokenIdInd, uint256 _tokenIdOut)
        private
        pure
    {
        require(
            _tokenIdInd < 2 && _tokenIdOut < 2 && _tokenIdInd != _tokenIdOut,
            "AMM: Invalid Token ID"
        );
    }
}
...
import "contracts/AMM/interfaces/IPT.sol";
import "contracts/AMM/interfaces/IRegistry.sol";
import "contracts/AMM/interfaces/IFutureWallet.sol";
...
library AMMMaths {
    ...
    function calcOutGivenIn(
        ...
        uint256 weightRatio = slippageBase.mul(_tokenWeightIn).div(
            _tokenWeightOut
        );
        uint256 adjustedIn = _tokenAmountIn.mul(UNIT.sub(_swapFee)).div(UNIT);
        uint256 y = UNIT.mul(_tokenBalanceIn).div(
            _tokenBalanceIn.add(adjustedIn)
        );
    ...
    function calcInGivenOut(
        ...
        uint256 slippageBase = UNIT.mul(UNIT).div(_slippageFactor);
        uint256 weightRatio = slippageBase.mul(_tokenWeightOut).div(
            _tokenWeightIn
        );
        uint256 y = UNIT.mul(_tokenBalanceOut).div(
            _tokenBalanceOut.sub(_tokenAmountOut)
        );
    ...
    function calcPoolOutGivenSingleIn(
        ...
        uint256 tokenAmountInAfterFee = _tokenAmountIn.mul(UNIT.sub(zaz)).div(
            UNIT
        );
    ...
    function calcSingleOutGivenPoolIn(
        ...
        uint256 poolAmountInAfterExitFee = _poolAmountIn
            .mul(UNIT.sub(EXIT_FEE))
            .div(UNIT);
        ...
        uint256 tokenOutRatio = AMMMathsUtils.pow(
            poolRatio,
            UNIT.mul(UNIT).div(normalizedWeight)
        );
        uint256 newTokenBalanceOut = tokenOutRatio.mul(_tokenBalanceOut).div(
            UNIT
        );

        uint256 tokenAmountOutBeforeSwapFee = _tokenBalanceOut.sub(
            newTokenBalanceOut
        );
    ...
    function calcPoolInGivenSingleOut(
        ...
        uint256 tokenAmountOutBeforeSwapFee = UNIT.mul(_tokenAmountOut).div(
            UNIT.sub(zar)
        );

        uint256 newTokenBalanceOut = _tokenBalanceOut.sub(
            tokenAmountOutBeforeSwapFee
        );
        uint256 tokenOutRatio = UNIT.mul(newTokenBalanceOut).div(
            _tokenBalanceOut
        );
...
import "contracts/AMM/interfaces/IRoleCheckable.sol";
