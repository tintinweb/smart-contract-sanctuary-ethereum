...
/// @title Hyperfy Spaces Staking Contract
/// TODO choose which event variables to index if any
...
contract Stake is ReentrancyGuard, Ownable {
    ...
    uint256 public totalHarvestedPeriod;
    ...
    event periodCreated(uint256 _blockStart, uint256 _blockEnd, uint256 _tokenPerBlock, uint256 maxEmission);
    ...
    function createPeriod(uint256 _blockStart, uint256 _blockEnd, uint256 _maxEmission) public onlyOwner {
        ...
        emit periodCreated(blockStart, blockEnd, tokenPerBlock, maxEmission);
    }
    ...
    event newHarvest(uint256 _lastHarvest, uint256 harvestTotal, uint256 owedPerToken, uint256 _totalHarvestedPeriod);

    function harvest() public {
        require(block.number >= blockStart && block.number <= blockEnd, "Harvest is not within the period");

        uint256 totalStaked = land.balanceOf(address(this));
        uint256 harvestTotal = (block.number - lastHarvest) * tokenPerBlock;
        uint256 owedPerToken = harvestTotal / totalStaked;
    ...
    function withdraw(uint256 tokenId) public nonReentrant {
        require(owner[tokenId] == msg.sender, "You are not the owner of this token");
    ...
    function getStakingTime(uint256 tokenId) public view returns(uint256) {
        if(user[msg.sender].stakeStart[tokenId] == 0) {
            return 0;
        } else {
            return block.timestamp - user[msg.sender].stakeStart[tokenId];
        }
    }
