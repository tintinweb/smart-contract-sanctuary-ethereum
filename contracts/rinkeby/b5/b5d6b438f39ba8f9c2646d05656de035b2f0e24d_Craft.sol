...
contract Craft is ERC721, Ownable {
    ...
    constructor(string memory baseURI_, uint256 _preSaleStartTimestamp, uint256 _generalSaleStartTimestamp) ERC721("Craft", "Craft") {
        baseURI = baseURI_;
    ...
    function airDrop(address[] memory _accounts, uint256 _amount) external onlyOwner {
        require(getAirDropSupply().add((_accounts.length * _amount)) <= reserve + remainingToken, "Can no longer airdrop.");
