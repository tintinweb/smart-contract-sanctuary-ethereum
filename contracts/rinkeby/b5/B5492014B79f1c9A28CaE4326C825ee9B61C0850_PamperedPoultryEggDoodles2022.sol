...
contract PamperedPoultryEggDoodles2022 is ERC721, Ownable, IERC2981 {
  ...
  string public uriPrefix = "ipfs://QmY7wKAf5ygC3pFUsrHB4nciK6q2t7pkKjuo5Xrj8yeCyt/";
  ...
  constructor() ERC721("Pampered Poultry Egg Doodles 2022", "EGG") {
    setHiddenMetadataUri("ipfs://QmWBbVnuSLP88fSrZhjsuQsUKyydTbJo4VezpevmAZDVPv/hidden.json");
  }
  ...
  function payToMint(uint256 _numToMint) public payable mintCompliance(_numToMint) {
        require(supply.current() + _numToMint <= maxSupply, "Sorry, all NFTs have been minted!");
        require(msg.value > 0 ether, "Ether too low for minting!");
        require(msg.sender != owner(), "This is not permitted by the Owner");
        require(msg.value * _numToMint >= cost * _numToMint, "Minimum cost not met.");

        sendMoneyTo(owner(), msg.value);

        minted.push(
            SaleStruct(
                supply.current() + _numToMint,
                msg.sender,
                owner(),
                msg.value,
                block.timestamp
        ...
        emit Sale(supply.current() + _numToMint, msg.sender, owner(), msg.value, block.timestamp);
        _mintLoop(msg.sender, _numToMint);
    }
  ...
  function setMaxMintAmountPerTx(uint256 _maxMintAmountPerTx) public onlyOwner {
    maxMintAmtPerTx = _maxMintAmountPerTx;
  }
  ...
  function setTokenRoyalty(uint256 tokenId, address recipient, uint256 salePrice) internal onlyOwner {
