...
contract BuildAFren is ERC721, Ownable {
    using Strings for uint256;
    ...
    function maxMintAmountPerWallet() internal view returns (uint256 _cost){
        if(totalSupply() < freeMint){
            return maxFreeMintPerWallet;
        }
        else{return maxSupply;}
    }
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(addressMinted[msg.sender] + _mintAmount <= maxMintAmountPerWallet(), "Max mint per address exceeded for the free mint.");
