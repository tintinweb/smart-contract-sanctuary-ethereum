...
contract Mortgage is UpgradeablePausable, IMortgage {
  ...
  MortgageTerms public mortgageTerms;
  MortgageStatus public status;

  event MortgageCreated(uint256 clientId, address);
  ...
  function initializeContract(
    ...
    require(_msgSender() == hhConfig.getHouseHoldAddress(), "Just Househould can do this");
    ...
    mortgageTerms = MortgageTerms({
      ...
      lateFee: lateFee,
      mortgageDefault: mortgageDefault
    });
    emit MortgageCreated(clientId, address(this));
    status = MortgageStatus.CREATED;
    return uint(MortgageStatus.CREATED); 
  }

  function onDrawdown(uint256 amount) external override onlyHouseHoldPool whenNotPaused returns (uint256) {
    accounting.balance = amount;
    status = MortgageStatus.ACTIVE;
    return uint(MortgageStatus.ACTIVE);
  }

  function onRepayment(uint256 amount) external override onlyHouseHoldPool whenNotPaused {
    accounting.balance = accounting.balance.sub(amount);
    emit Repayment(amount);
  }

  function closeMortgage() external override onlyHouseHoldPool whenNotPaused returns (uint256) {
    status = MortgageStatus.CLOSED;
    return uint(MortgageStatus.CLOSED);
  }

  function completeMortgage() external override onlyHouseHoldPool whenNotPaused returns (uint256) {
    status = MortgageStatus.COMPLETED;
    return uint(MortgageStatus.COMPLETED);
  }

  function getMortgageTerms() external view override returns (MortgageTerms memory) {
  ...
  function getAccounting() external view override returns (Accounting memory) {
    return accounting;
  }
