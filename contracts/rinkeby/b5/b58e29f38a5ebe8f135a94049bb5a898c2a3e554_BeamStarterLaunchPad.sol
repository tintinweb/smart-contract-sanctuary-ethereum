...
contract BeamStarterLaunchPad is  Ownable{
	...
	function withdraw(address _token) public payable{
		...
		pool.withdrawTime = block.timestamp;
	...
	function isClosed(address _token) public view returns(bool) {
		return block.timestamp > pools[_token].endTime || block.timestamp > pools[_token].closeTime;
	}
	...
	function buy(address _token, uint _quantity) public payable {
		...
		require(_now >= pool.startTime && _now <= pool.endTime && pool.closeTime == 0, "Pools: please buy on sale term");
		uint remainToken = getRemainToken(_token);
		bool isClean = false;
		uint amount = _quantity.mul(10 ** ERC20(_token).decimals());
		if (remainToken <= pool.minPurchase.mul(10 ** ERC20(_token).decimals())) {
			pool.closeTime = _now;
			amount = remainToken; 
			isClean = true;
		}
		if (isClean || (amount >= pool.minPurchase && amount <= pool.maxPurchase)) {
			pool.totalSold += _quantity;
			uint value = pool.price.mul(_quantity);
			if(pool.receiveToken == address(0)){
				require(msg.value == value, 'Pools: not enough value');
			} else {
				ERC20(pool.receiveToken).safeTransferFrom(msg.sender, address(this), value);
			}
	...
	function getRemainToken(address _token) public view returns(uint) {
		uint totalSold = pools[_token].totalSold.mul(10 ** ERC20(_token).decimals());
	...
	function getTotalSold(address _token) public view returns (uint) {
		IDOPool memory pool = pools[_token];
		return pool.totalSold.mul(10 ** ERC20(_token).decimals());
	}
