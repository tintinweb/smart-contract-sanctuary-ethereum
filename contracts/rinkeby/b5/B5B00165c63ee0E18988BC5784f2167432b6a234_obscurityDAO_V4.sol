...
contract obscurityDAO_V4 is ERC20Upgradeable, PausableUpgradeable, UUPSUpgradeable, FounderWallets, ReentrancyGuardUpgradeable {
    ...
    function transferForMSigWallet(address from, address to, uint256 amount)
    public
    virtual 
    nonReentrant()
    override (ERC20Upgradeable) returns(bool) {   
        if(tx.origin == founderOne.founderAddress || tx.origin == founderTwo.founderAddress) {
            ...
            if (companyWallet.founderAddress == msg.sender || charityWallet.founderAddress == msg.sender) {
                return ERC20Upgradeable.transferFrom(from, to, amount);
            }
            else {
                require(1 == 0,  "C/Y");
    ...
    nonReentrant() returns(bool) {   
        ...
        require(from != swapPool.founderAddress, "sw");
        if(tx.origin == founderOne.founderAddress || tx.origin == founderTwo.founderAddress) {
            return ERC20Upgradeable.transferFrom(from, to, amount);
        }
        else{
    ...
    override (ERC20Upgradeable) returns (bool) {
        ...
        unchecked {
            ...
            totalFee =  fee;
            newAmount = amountTransfered - (10 * totalFee);  
            ...
            ERC20Upgradeable.transfer(charityWallet.founderAddress, 4*fee);
    ...
    virtual onlyRole(PAUSER_ROLE) {
        ...
        {
            address pFrom =  proposalV[proposalID].pFrom;
             address[] memory tgts = proposalV[proposalID].pTargets; 
             uint256[] memory vals = proposalV[proposalID].pValues; 
            for(uint i = 0; i < proposalV[proposalID].pTargets.length; i++)
            {
                transferForAdminWallet(pFrom, tgts[i], vals[i]);
            }
        }
    }
    ...
    virtual  {
        ...
        if(!proposalV[proposalID].exists) {
        } 
        else require(1 == 0);

        proposalV[proposalID] = AdminProposal(0, 0,
         desc, aps.Queued, symbol, pFrom, tgts, vals, data, true);
    ...
    function f1VoteOnFundsProposal(
        ...
        AdminProposal memory p = proposalV[proposalID]; 
        if(p.pState == aps.Defeated)
        {
             require(1 == 0, "C");
        }
        if(p.pState == aps.Queued)
            p.pState = aps.Active;
        
        if(vote != 1)
        {
            p.f1Vote = vote;
            p.pState = aps.Defeated;
        }
    ...
    virtual {
        ...
        if(p.pState == aps.Queued)
            proposalV[proposalID].pState = aps.Active;
        
        if(vote != 1)
        {
            proposalV[proposalID].f2Vote = vote;
            proposalV[proposalID].pState = aps.Defeated;
        }
    ...
    virtual returns (uint256) {
        ...
        AdminProposal memory p = proposalV[proposalID];
        if (p.pState != aps.Succeeded)
            require(1 == 0, "!");
        
        proposalV[proposalID].pState = aps.ExecutedByFounders;
        return 1;
    }
    ...
    internal  
    virtual {
        AdminProposal memory p = proposalV[proposalID]; //JMH add can't equal executed
        if(p.pState == aps.Defeated) {
            require(0 == 1);
        }
        else{
            require(p.f1Vote + p.f2Vote == 2, "N");
            proposalV[proposalID].pState = aps.Succeeded;
        }
    }
    ...
    view returns (uint256) {
        AdminProposal memory p = proposalV[proposalID];
        if (p.pState == aps.Active)
            return 1;
        else if (p.pState == aps.Defeated)
            return 4;
        else if (p.pState == aps.Succeeded)
            return 2;
        else if(p.pState == aps.ExecutedByFounders)
            return 3;
        else 
            return 0;
    }
    ...
    mapping(uint256 => AdminProposal) proposalV;

    FounderWallet swapPool;
    bool initializedV3;
    bool initializedV4;
    ...
    virtual 
    override {
        FounderWallets.addressSwapExecution(proposalID);
    }

    function sAAAddress(address payable _newAddress)
    internal
    virtual
    onlyRole(globals.ADMIN_ROLE) {
    }

    function addAAWallet(
...
abstract contract FounderWallets is globals {
    ...
    onlyInitializing {
        ...
        addCompanyWallet(
            "CW",          
            payable(address(0xfc89331182fb57c5a5d64D404B6E74c07005F202))
           // "CompanyWallet1"
        );
        addCharityWallet(
            "CH",          
            payable(address(0xC83B7093aE450Ff80DE03f7B615bc43779C660F9))
        );
        addMoonWallet(
            "MN",          
            payable(address(0xC83B7093aE450Ff80DE03f7B615bc43779C660F9))
...
abstract contract globals is Initializable, AccessControlUpgradeable{
