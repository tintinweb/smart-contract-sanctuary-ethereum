...
contract ERC721Sale is ReentrancyGuard, Ownable {
    ...
    function distributePayment(
        ...
        if (_token.supportsInterface(_INTERFACE_ID_HAS_SECONDARY_SALE)) {
            ...
            if(isSecondarySale) {
                ...
                if ( _currencyId == 0 ) {
                    ...
                    require(royaltySent, "ERC721Sale.distribute: Royalty transfer failed");
                } else {
        ...
        if ( _currencyId == 0 ) {
            ...
            require(serviceFeeSent, "ERC721Sale.distribute: ServiceFee transfer failed");
            (bool ownerReceivingAmountSent, ) = payable(_owner).call{value: ownerReceivingAmount}("");
            require(ownerReceivingAmountSent, "ERC721Sale.distribute: ownerReceivingAmount transfer failed");
        } else {
    ...
    */
    ...
    function getCurrencyName(uint8 index) external view returns (string memory) {
        require(isCurrencyValid(index), "ERC721Sale.sell: Currency is not supported");
        return supportCurrencyName[index];
    }
    ...
    function isCurrencyValid(uint8 index) public view returns (bool) {
        return (index == 0 || supportCurrencies[index] != address(0x0)) ? true : false;
    }
}
