    ...
    TokenRescuer
{
    ...
    /// The maximum number of mints per address in the whitelist sale state.
    uint256 public constant MAX_WHITELIST_MINT = 3;

    /// The maximum number of mints per address in the public sale state.
    ...
    /// The price per token mint (whitelist sale state).
    uint256 public priceWhitelist;

    /// The price per token mint (public sale state).
    ...
    /**
     * @notice Tracks revocations of the automatic OpenSea proxy approval.
     */
    mapping(address => bool) public userRevokedRegistryApproval;

    /**
     * @notice The total tokens minted by an address in whitelist sale state.
     */
    mapping(address => uint256) public whitelistMinted;

    /**
     * @notice The total tokens minted by an address in public sale state.
     */
    ...
    /**
     * @notice Reverts if the correct ether value was not sent.
     * @param _mintAmount The number of tokens being minted.
     * @param _price The correct token price for the current sale state.
     */
     ...
     * @param _saleState The sale state to be validated.
     */
        ...
        onlyWithCorrectPayment(_mintAmount, pricePublic)
    {
        ...
        if (publicMinted[_msgSender()] > MAX_PUBLIC_MINT)
            revert ExceedsMintAllocation();
        ...
        onlyOwner
    {
        ...
        emit WhitelistPriceChanged(_newPrice);
    }
        ...
        onlyOwner
    {
        ...
        emit PublicPriceChanged(_newPrice);
    }
...
interface IPotadlesNFT {
    ...
    event WhitelistPriceChanged(
        uint256 newWhitelistPrice
    );

    event PublicPriceChanged(
