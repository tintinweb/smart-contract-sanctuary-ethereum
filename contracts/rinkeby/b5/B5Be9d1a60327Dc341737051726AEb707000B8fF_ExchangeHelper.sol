...
library LibOrder {
    ...
    function hash(bytes32 orderKeyHash, uint256 matchBeforeTimestamp)
        internal
        pure
        returns (bytes32)
    {
...
library LibExchange {
    ...
    function validateMatch(
        ...
        if (_orderLeft.salt > 0) {
            // Make sure current block`s timestamp is below matchLeftBeforeTimestamp
            require(
                _matchLeftBeforeTimestamp > block.timestamp,
                'LibExchange: current block`s timestamp is higher than matchLeftBeforeTimestamp'
            );
        ...
        if (_orderRight.salt > 0) {
            // Make sure current block`s timestamp is below matchRightBeforeTimestamp
            require(
                _matchRightBeforeTimestamp > block.timestamp,
                'LibExchange: current block`s timestamp is higher than matchRightBeforeTimestamp'
            );
            // OrderBook must be the signer
            if (
                recoverMatchAllowanceSigner(
