...
error ExceedMaxOgGatorMint(); /// @notice Thrown when user attempts to exceed the mint more than they hold of og gators.
...
error NotOgGatorOwner(); /// @notice Thrown if the user is not a OG gator holder

contract Scribblegators is Ownable, ERC721A, MerkleWhitelist {
    uint public buyPrice = 0.01 ether;
    uint public nameChangePrice = 0.03 ether; // Price to change the name of the NFT
    uint public mxSupply = 2500; //first index is 0
    uint public preSaleStartTime;
    uint public saleStartTime;
    uint public revealStartTime;
    uint public refundStartTime;
    uint public refundAtPercentageMinted;
    uint public wdrawStartTime;
    uint public wdrawAtPercentageMinted;
    uint16 public publicWlMinted; //How many publicWL have been minted
    uint256 public publicWLStartTime = 1648166400;
    
    bool public initialFundsWithdraw;
    bool public remainingFundsWithdraw;
    bool public revealed;

    address public ogGatorContractAddy =
        0x3e99Cdf32D27526E06Fd4070619e21D9852281E7;

    address public bradAddy =
        0x3e99Cdf32D27526E06Fd4070619e21D9852281E7;

    address public mootAddy =
        0x3e99Cdf32D27526E06Fd4070619e21D9852281E7;

    address public influencerAddy =
        0x3e99Cdf32D27526E06Fd4070619e21D9852281E7;

    string public contractUri = "ipfs://QmWQMuChCoTc2jKoDqCVHBxYe6pp1E2KMyW4HG8U7AA6wF";
    string public baseUri;
    string public unrevealedUri = "ipfs://QmWQMuChCoTc2jKoDqCVHBxYe6pp1E2KMyW4HG8U7AA6wF";
    ...
    constructor() ERC721A("Scribblegators", "SCRBL") {}
    ...
    function mintPreSale(uint tokenAmt, bytes memory signature) external payable {
        ...
        if(msg.value < tokenAmt * buyPrice) revert ValueTooLow();
        ...
        if(totalSupply() + tokenAmt > mxSupply) revert ExceedMaxSupply();

        _safeMint(msg.sender, tokenAmt);
    }

    function ogGatorMint(uint tokenAmt, bytes memory proof) external payable {
        if(msg.sender != tx.origin) revert AntiBot(); // Anti-bot measure
        
       /*  for (uint i=1; i < IERC721(ogGatorContractAddy).totalSuppy(); i++) {
             if(IERC721(ogGatorContractAddy).ownerOf(i) == msg.sender) {

			 }
        } */


        if(IERC721(ogGatorContractAddy).balanceOf(msg.sender) > 0) revert NotOgGatorOwner();

        uint256 currentTime = block.timestamp;
        if(currentTime < preSaleStartTime || currentTime > saleStartTime) revert SaleNotStartedOrEnded();


        if(_numberMinted(msg.sender) + tokenAmt > IERC721(ogGatorContractAddy).balanceOf(msg.sender)) revert ExceedMaxOgGatorMint();
    ...
    function teamMint(uint256 quantity, address receiver) public onlyOwner {
        //Max supply
        require(
            totalSupply() + quantity <= 2500,
            "Max supply of 2500 total!"
        );
    ...
    function refundExtraETH() public {
        uint256 currentTime = block.timestamp;
        require(currentTime > refundStartTime && refundAtPercentageMinted < totalSupply(), "Refund not possible.");

        uint256 totalRefund;

        //What they paid - what they should have paid = refund.
        uint256 refund = 0.05 ether;
    ...
    function withdrawFunds() public onlyOwner {
        // If time is greater than the time we set for withdrawing and if total minted is greater than or equal to the withdraw percentage variable 
        require(block.timestamp >= wdrawStartTime && 2500 * wdrawAtPercentageMinted >= totalSupply());

        uint256 finalFunds = address(this).balance;

        (bool succ, ) = payable(bradAddy).call{
            value: (finalFunds * 50) / 100
        }("");
        require(succ, "transfer failed");

        (succ, ) = payable(mootAddy).call{
            value: (finalFunds * 50) / 100
        }("");
        require(succ, "transfer failed");

        (succ, ) = payable(influencerAddy).call{
            value: (finalFunds * 10) / 100
        }("");
        require(succ, "transfer failed");
    }
...
interface IScribblegators {
	function balanceOG(address _user) external view returns(uint256);
}

contract GatrToken is ERC20("Gator", "GATR") {
	using SafeMath for uint256;

	uint256 constant public BASE_RATE = 10 ether; 
	uint256 constant public INITIAL_ISSUANCE = 300 ether;
	// Fri, 09 Nov 2040 08:45:25 +0000
	uint256 constant public END = 2236063525;

	mapping(address => uint256) public rewards;
	mapping(address => uint256) public lastUpdate;

	IScribblegators public scribblegatorsContract;

	event RewardPaid(address indexed user, uint256 reward);

	constructor(address _scribblegators) public {
		scribblegatorsContract = IScribblegators(_scribblegators);
	}
	...
	function updateRewardOnMint(address _user, uint256 _amount) external {
		require(msg.sender == address(scribblegatorsContract), "Can't call this");
		uint256 time = min(block.timestamp, END);
		uint256 timerUser = lastUpdate[_user];
		if (timerUser > 0)
			rewards[_user] = rewards[_user].add(scribblegatorsContract.balanceOG(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(86400)
	...
	function updateReward(address _from, address _to, uint256 _tokenId) external {
		require(msg.sender == address(scribblegatorsContract));
		if (_tokenId < 1001) {
			uint256 time = min(block.timestamp, END);
			uint256 timerFrom = lastUpdate[_from];
			if (timerFrom > 0)
				rewards[_from] += scribblegatorsContract.balanceOG(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(86400);
			if (timerFrom != END)
				lastUpdate[_from] = time;
			if (_to != address(0)) {
				uint256 timerTo = lastUpdate[_to];
				if (timerTo > 0)
					rewards[_to] += scribblegatorsContract.balanceOG(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(86400);
				if (timerTo != END)
					lastUpdate[_to] = time;
			}
		}
	...
	function getTotalClaimable(address _user) external view returns(uint256) {
		uint256 time = min(block.timestamp, END);
		uint256 pending = scribblegatorsContract.balanceOG(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(86400);
...
contract MerkleWhitelist is Ownable {
  ...
  bytes32 public ogGatorListtMerkleRoot;
  ...
  */
  ...
  function verifyOgGatorSender(address userAddress, bytes32[] memory proof) public view returns (bool) {
    return _verify(proof, _hash(userAddress), ogGatorListtMerkleRoot);
  }
  ...
  function _verifyOgGatorSender(bytes32[] memory proof) internal view returns (bool) {
    return _verify(proof, _hash(msg.sender), mouseWhitelistMerkleRoot);
  }
