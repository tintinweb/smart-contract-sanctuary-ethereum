...
contract IslandToken is ERC20, ERC20Burnable, Pausable, Ownable, AccessControl, IIslandToken {
    // roles
     ...
     */
    function setStaking(address stakingContract) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function setTaxes(uint256[] calldata _newTaxes) external onlyRole(DEFAULT_ADMIN_ROLE) {
      require(_newTaxes.length == 5, "I1");
      for (uint256 index = 0; index < 5; index++) {
        require(_newTaxes[index] <= 30, "I2");
      }
      taxes = _newTaxes;
    }
