    ...
    SuperAdminControl
{
        ...
        onlyLiquidatorRole(msg.sender)
    {
        ...
        NftLoanData.LoanDetailsNFT memory loanDetailsNFT = loanOffersNFT[_loanId];
        NftLoanData.LenderDetailsNFT
            memory lenderDetails = activatedNFTLoanOffers[_loanId];

        require(
            lenderDetails.activationLoanTimeStamp != 0,
            "GLM: loan not activated"
        );
        ...
        IERC20Upgradeable(loanDetailsNFT.borrowStableCoin).safeTransfer(
            activatedNFTLoanOffers[_loanId].lender,
            earnedAPY
        );
