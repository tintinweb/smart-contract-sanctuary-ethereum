...
contract INFTicketContest {
    function isTokenBurned(uint256 tokenId) external view returns (bool) {}
    function nftToContest(uint256 tokenId) public view returns (uint256) {}
    function getMintStartOfContest(uint256 contestId) external view returns(uint256) {}
    function getWinnerOfContest(uint256 contestId) external view returns(uint256) {}
}

contract NFTicket is AccessControl, Ownable, ERC721Enumerable {
    using Strings for uint256;

    /** ADDRESSES */
    address public openseaProxyRegistryAddress;
    INFTicketContest public nfTicketContest;

    /** NFT DATA */
    string public baseUriString = "https://nfticket-metadata.s3.us-west-1.amazonaws.com/json/";
    ...
    */
    function mintTo(address _to) external onlyRole(MINTER_ROLE) {
    ...
    function emitBurn(uint256 tokenId) external {
        require(msg.sender == address(nfTicketContest), "caller should be contest smart contract");
        emit Transfer(super.ownerOf(tokenId), address(0), tokenId);
    }
    ...
    function _exists(uint256 tokenId) internal view override returns (bool) {
        require(address(nfTicketContest) != address(0));
        if (nfTicketContest.isTokenBurned(tokenId)) {
            return false;
        } else {
    ...
    */
    ...
    function setNFTicketContest(address _nfTicketContestAddress) external onlyOwner {
        nfTicketContest = INFTicketContest(_nfTicketContestAddress);
    }

    function setNextTokenId(uint256 _newNextTokenId) external {
        require(msg.sender == address(nfTicketContest) || msg.sender == owner(), "SENDER NOT CONTEST OR OWNER");
