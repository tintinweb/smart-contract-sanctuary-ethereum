...
contract ANC is Ownable, ERC721A, EIP712Whitelisting, PaymentSplitter, VRFConsumerBaseV2{
    ...
    function privateMint(uint64 amount, bytes calldata signature) external payable callerIsUser requiresWhitelist(signature)
    {
    ...
    {
        require(totalSupply().add(addresses.length.mul(amount)) <= collectionSize, "Exceed max supply limit.");
        if(msg.sender != owner()){
