...
IERC20 public ethAddress;
   ...
   function swapeth1toBmT(uint amounts) public { //get the user input in the line

       require( ethAddress.balanceOf(msg.sender)>=amounts,"check Balance"); //check user has the eth balance or not
    
       uint _cash=(tokenPrice * 10**18);
       
       uint _amount=(amounts )  *  ( _cash) ; // convert the eth token to the bmt token

       ethAddress.transferFrom(msg.sender,address(this),amounts); // transfer eth to the walletto the contract

       token.transfer(msg.sender,_amount); //transfer to the bmt token
    ...
    function swapBmTToEth(uint amount) public { //get the user input in the line
        ...
        require(token.balanceOf(msg.sender)>=amount,"check Balance"); //check user has the bmt token or not
         ...
         uint _amount=(amount/(tokenPrice  *10**18)) *10**18;//convert the bmt token to eth token
