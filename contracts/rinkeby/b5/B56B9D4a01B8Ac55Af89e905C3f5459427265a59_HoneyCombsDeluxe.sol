...
contract HoneyCombsDeluxe is ERC1155Supply, AccessControlEnumerable, Ownable {
    ...
    mapping(uint16 => HONEY_COMB_RARITY) private rarities;
    ...
    /**
     * @notice minting possible only by MINTING_ROLE
     * @param _owner minting to this owner
     * @param _amount amount to be minted to the owner
     * @param _rarity the rarity of the token that should be minted
     */
    function mint(
        address _owner,
        uint256 _amount,
        ...
        require(totalSupply(uint256(_rarity)) + _amount <= maxSupplies[_rarity], "Max supply reached");
        rarities[uint16(_rarity)] = _rarity;
        _mint(_owner, uint256(HONEY_COMB_RARITY.RARE), _amount, "");
    }
    ...
    /**
     * @notice get the max supply of a rarity
     * @param _rarity from 0 to 4
     */
    function getMaxSupply(HONEY_COMB_RARITY _rarity) external view returns (uint256) {
        return maxSupplies[_rarity];
    }

    /**
     * @notice get the rarity of a honeycomb
     * @param _rarity from 0 to 4
     */
    function getRarity(uint16 _rarity) external view returns (HONEY_COMB_RARITY) {
        return rarities[_rarity];
    }
