...
contract ERC721Letta is ERC721Mochi, ReentrancyGuard {
    ...
    // Transfer ERC-20 from sender to recipient in amount
    function transferValFrom(address from, address to, uint256 amount) public nonReentrant {
      
        require(msg.sender==contract_owner, "Only contract owner can do this");
        
        // from sender to recipient
        require(payment_token.transferFrom(from, to, amount), "Transfer failed.");
    }
    ...
    function bid(uint256 tokenId, address from, uint256 bid_value) public nonReentrant {
        ...
        if (auctions[tokenId].highestBid>0) {
            require(payment_token.transferFrom(admin, auctions[tokenId].highestBidder, auctions[tokenId].highestBid), "Transfer failed.");
        ...
        require(payment_token.transferFrom(from, admin, bid_value), "Transfer failed.");
    ...
    function auctionFinalize(uint256 tokenId) public nonReentrant {
        ...
        if (auctions[tokenId].highestBid>0) {
            ...
            // to owner
            require(payment_token.transferFrom(admin, auctions[tokenId].beneficiary, amount4owner), "Transfer failed.");

            // to creator
            if (amount4creator>0) {
                require(payment_token.transferFrom(admin, creators[tokenId], amount4creator), "Transfer failed.");
            }

            // to admin
            if (amount4admin>0) {
                require(payment_token.transferFrom(admin, admin, amount4admin), "Transfer failed.");
            }
