...
import "./GnarDonksNFT.sol";

/**
 * @title Wisefy
 * @custom:a Luciano Greiner / Alfredo Lopez
 */
contract GnarDonksNFTMock is GnarDonksNFT {
    using Address for address;
    // Immutable Address for Testing
    address immutable USDC_ADDRESS_TESTNET;
    ...
    {
        USDC_ADDRESS_TESTNET = _usdcAddress;
    }

    function ethToUsd(
        uint256 amount // Wei?
    ) public view returns (uint256) {
        return _ethToUsd(amount);
    }

    function usdToEth(
        uint256 amount // Wei?
    ) public view returns (uint256) {
        return _usdToEth(amount);
    }
        ...
        whenNotPaused
    {
        ...
        require(
            _stableCoin == USDC_ADDRESS || _stableCoin == USDC_ADDRESS_TESTNET,
            "ERC721: Token Stable Coin not permitted"
        );
        // Pre Check of tier before to mint
        beforeMintOfTier(tierOf(tokenId));
        // Get the price of the token
        uint256 price = tierPriceInUSD(tierOf(tokenId));
        ...
        require(
            _stableToken.balanceOf(_msgSender()) >=
                (price * (10**_stableToken.decimals())),
        ...
        // Call the mint method from ERC721Wisefy
    ...
    PaymentSplitter
{
    ...
    /** @dev USDT Token for Ethereum (TETHER), 6 decimals */
    address public constant USDT_ADDRESS =
        address(0xdAC17F958D2ee523a2206206994597C13D831ec7);
    /** @dev USDC Token for Ethereum, 6 decimals */
    address public constant USDC_ADDRESS =
        address(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48);

    // TODO: Alfredo: check this on mint/pre-mint
    uint256 internal launchTs;

    //TODO: Alfredo: you can manipulate this on Mock contract
    ...
    constructor(
        ...
        ERC721Wisefy(
            _name,
            _symbol,
            "http://gnardonks.s3-website-us-east-1.amazonaws.com/",
            "http://gnardonks.s3-website-us-east-1.amazonaws.com",
            _totalSupply
        )
    {
        ...
        tiers[1] = 5000;
        tiers[2] = 2500 | (0 << 16) | (31560000 << 32) | (250 << 64); // 250 per yer
        tiers[3] = 1000 | (0 << 16) | (31560000 << 32) | (100 << 64); // 100 per yer
        tiers[4] = 500 | (0 << 16) | (31560000 << 32) | (50 << 64); // 50 per yer
        tiers[5] = 75 | (0 << 16) | (63120000 << 32) | (15 << 64); // every 2 years
        tiers[6] = 10;
    }
        ...
        nonReentrant
    {
        ...
        beforeMintOfTier(tierOf(tokenId));
        // Get the price of the token
        uint256 price = tierPriceInETH(tierOf(tokenId));
        // Get the balance of the sender
        uint256 balance = msg.value;
        // Check if the balance is enough to pay the price
        require(balance >= price, "Insufficient funds to pay the price");
        // Call the mint method from ERC721Wisefy
        _safeMint(to, tokenId);
        // Post check of tier after to mint
        afterMintOfTier(tierOf(tokenId));
        ...
        whenNotPaused
    {
        require(_stableCoin.isContract(), "Invalid stable coin address");
        require(
            _stableCoin == USDT_ADDRESS || _stableCoin == USDC_ADDRESS,
            "ERC721: Token Stable Coin not permitted"
        );
        // Pre Check of tier before to mint
        beforeMintOfTier(tierOf(tokenId));
     ...
     * @param tokenId tokenId of the token to be minted, with this define the tier
     */
    function mintPerWallet(address to, uint256 tokenId)
        external
    ...
    function tierPriceInETH(uint256 tier) public view returns (uint256) {
        if (tier == 1) {
            return 1 ether; // the price is 1 ether, and is native in solididty
        } else if (tier == 2) {
            return _usdToEth(tierPriceInUSD(2));
        } else if (tier == 3) {
            return _usdToEth(tierPriceInUSD(3));
        } else if (tier == 4) {
            return _usdToEth(tierPriceInUSD(4));
        } else if (tier == 5) {
            return _usdToEth(tierPriceInUSD(5));
        } else {
            return _usdToEth(tierPriceInUSD(6));
        }
    }

    function tierPriceInUSD(uint256 tier) public view returns (uint256) {
        if (tier == 1) {
            return _ethToUsd(tierPriceInETH(1)) / 1 ether; // the price is 1 ether, and is native in solididty
        } else if (tier == 2) {
            return 5000;
        } else if (tier == 3) {
            return 10000;
    ...
    function _ethToUsd(
        ...
        (int256 _price, uint8 _decimals) = getLatestPrice();
        uint256 price = mulDiv(uint256(_price), 102 * (10**16), 10**18); // add 2% to the market price
    ...
    function _usdToEth(
        ...
        (int256 price, uint8 _decimals) = getLatestPrice();
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(AccessControl, ERC721Wisefy, ERC2981)
        returns (bool)
    {
        return
     ...
     * @param tier uint256
     */
    function beforeMintOfTier(uint256 tier) internal virtual {
        require(
     ...
     * @param tier uint256
     */
    function afterMintOfTier(uint256 tier) internal virtual {}

    function decodeTier(uint256 tier)
        private
        returns (
            uint256 supply,
    ...
    {
        //[ supply 16, supply per period 16, 16 period_time, total_sales 16, sales period 1 16, sales period 2 16, ... sales period 10 16].
        //tiers[1] = 5000 | 5000 << 16;
        //tiers[2] = 2500 | 250 << 16 | 31560000 << 32; // 250 per yer
    }
}
...
contract Claimable is Ownable {
    ...
    modifier validAddress(address _to) {
        require(_to != address(0), "ERC721: Not Add Zero Address");
        /* solcov ignore next */
        _;
    }
        ...
        onlyOwner
    {
        ...
        token.safeTransferFrom(_msgSender(), _to, balance);
    }
    ...
    function claimErc1155Tokens(
        ...
        token.safeTransferFrom(_msgSender(), _to, _id, balance, "");
    }
}
