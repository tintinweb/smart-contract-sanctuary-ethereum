...
contract ConcreteJungle is ERC721, Ownable {
    ...
    function mint(uint amount_) public payable {
        require(amount_ > 0, "cannot be 0");
        require(isPublicMintEnabled, "Minting not enabled yet.");
        require(msg.value == amount_ * mintPrice, "Wrong mint value");
        require(totalSupply + amount_ <= maxSupply, "Sold out");
        require(
        ...
        for (uint i = 0; i < amount_; i++) {
            ...
            mintedNFTs[newTokenId] = newTokenId;
            addTokenToAddress(newTokenId);
            _safeMint(msg.sender, newTokenId);
        }
    }

    function addTokenToAddress(uint256 tokenId_) public {
        require(
            walletMints[msg.sender].length < 5,
            "You have reached to maximum for this address"
        );
        walletMints[msg.sender].push(tokenId_);
    }

    function getAllTokensOfAddress() public view returns (uint[] memory) {
    ...
    fallback() external payable {
        require(msg.data.length == 0, "sorry");
        withdrawWallet.transfer(msg.value);
    }
}
