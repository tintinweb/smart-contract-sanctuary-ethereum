...
contract PricePrediction is SmolGame {
  ...
  struct Prediction {
    ...
    uint16 startRoundPhaseId;
    uint80 startRoundId;
    uint16 endRoundPhaseId; // not set until prediction is settled
   ...
   */
  function getRoundIdAndPriceUSD(address _proxy)
    public
    view
    returns (
      uint16,
    ...
    )
  {
    ...
    return (phaseId, roundId, uint256(price) * (10**18 / 10**decimals));
  }
  ...
  function predict(
    ...
    ) = getRoundIdAndPriceUSD(_priceFeedProxy);
  ...
  function settlePrediction(
    ...
    _openPrediction.endRoundPhaseId = _answeredPhaseId;
