...
UUPSUpgradeable
{
    ...
    function _tokenIdCounter() public view virtual returns (uint256) {
    ...
    function safeMint(uint256 amount) public payable {
        require(_currentIndex + amount <= _totalSupply() - mintReserve, "There's no token to mint.");
        require(_numberMinted(msg.sender) + amount <= maxMintPerAddress, "One address can mint 10 tickets.");
        require(mintPrice == 0, "Mint price is not correct.");
        // uint256 decreases = MathUpgradeable.min(
        //     (block.timestamp - auction.startDate) / auction.decreaseInterval,
        //         auction.numDecreases
        // );
		// bytes32 digest = keccak256(abi.encode(decreases, msg.sender));
    ...
    function safeReserveMint(address to, uint256 amount) public onlyOwner {
        require(_currentIndex + amount < _totalSupply(), "There's no token to mint.");
        _safeMint(msg.sender, amount);
    }
