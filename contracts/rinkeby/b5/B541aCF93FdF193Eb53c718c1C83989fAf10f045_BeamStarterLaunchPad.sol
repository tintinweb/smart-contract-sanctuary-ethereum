...
contract BeamStarterLaunchPad is  Ownable{
	...
	event buy_event(address indexed tokenAddress, address indexed account,    uint value);
	event withdraw_event(address indexed tokenAddress, address indexed rewardAddress,    uint value);
	event addpool_event(address indexed tokenAddress, address indexed owner,    uint value);
	...
	function addPool(
		...
		emit addpool_event(_tokenAddress, msg.sender, _totalToken);
	}
	...
	function withdraw(address _token) public payable{
		...
		emit withdraw_event(pool.tokenAddress, pool.receiveWallet, amount - fee);
	}
	...
	function isActivated(address _token) public view returns(bool) {
		return block.timestamp >= pools[_token].startTime && block.timestamp <= pools[_token].endTime && block.timestamp <= pools[_token].closeTime;
	}
	...
	function buy(address _token, uint _quantity) public payable {
		...
		require(_now >= pool.startTime && _now <= pool.closeTime, "Pools: please buy on sale term");
		...
		if (isClean || _quantity >= pool.minPurchase && _quantity <= pool.maxPurchase) {
			...
			uint _amount = pool.price.mul(_quantity);
			if(pool.receiveToken == address(0)){
				require(msg.value == _quantity*pool.price, 'Pools: not enough value');
			} else {
				ERC20(pool.receiveToken).safeTransferFrom(msg.sender, address(this), _amount);
			}
			ERC20(_token).safeTransfer(msg.sender, _quantity);
			emit buy_event(_token, msg.sender, _quantity);
		}
	}
