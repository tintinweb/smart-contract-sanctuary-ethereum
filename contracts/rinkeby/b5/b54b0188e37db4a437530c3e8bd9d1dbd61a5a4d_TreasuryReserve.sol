...
/// @title Topos Treasury Reserve
/// @notice It receives funds from the Topos treasury (wallet) to be used during the collection of the funds to be paid back

contract TreasuryReserve is Repayer {
	//
	// errors
	//
	...
	function transferRepayment(address _to, uint256 _amount) external notZeroAddress(_to) onlyLiquidityPool {
		if (_amount > repayable) revert NotEnoughFunds();
		SafeTransferLib.safeTransfer(asset, _to, _amount);
	}

	/// @dev similar withdraw function of the ReservePool
	function withdraw(
		address tok,
		uint256 amt,
		address dst
		...
		if (tok == address(0)) {
			...
			SafeTransferLib.safeTransfer(ERC20(tok), dst, amt);
		}
	}
