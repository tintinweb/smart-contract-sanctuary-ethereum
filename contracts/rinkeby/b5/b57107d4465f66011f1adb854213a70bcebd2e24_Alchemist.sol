...
contract Alchemist is ReentrancyGuard, Initializable {
    ...
    uint8 public ratioOfSteady;
    uint256 public forgePrice;
    uint256 public alchemistId;

    event Split(address indexed source, uint256 splitAmount, uint8 ratioOfSteady);
    event Merge(address indexed source, uint256 mergedAmount, uint8 ratioOfSteady);
    ...
    */    
        ...
        returns (bool) 
    {
        ...
        (address oracleAddress,uint fees,,uint8 decimals,uint durationForMaturity) = IAcademy(address(academy)).getChymeInfo(chyme);
        uint divisor = 10 ** decimals;
        forgePrice = uint256(IAcademy(address(academy)).priceFromOracle(oracleAddress));
        ...
        ISteadyDAORewards(steadyDAORewards).performAction(ISteadyDAORewards.actionType.SPLIT, sChymeAmt, fees, chyme);
        steady.mint(msg.sender, sChymeAmt);
        elixir.safeMint(msg.sender, chyme, ratioOfSteady, uint256(forgePrice), amount, block.timestamp + durationForMaturity);
        emit Split(msg.sender, amount, ratioOfSteady);
        return true;
    }
        ...
        returns (bool) 
    {
        ...
        ISteadyDAORewards(steadyDAORewards).performAction(ISteadyDAORewards.actionType.MERGE, _steady.amount, fees, chyme);
        steady.burnFrom(msg.sender, _steady.amount);
        elixir.burn(tokenId);
        emit Merge(msg.sender, chymeAmountToMerge, ratioOfSteady);
        return true;
    }
...
interface IElixir is IERC721Upgradeable {
    ...
    function getSteadyRequired(uint256 tokenId) external view returns(uint256, uint256, uint256); 
    function safeMint( address _to, address _chyme, uint8 ratio, uint256 _forgePrice, uint256 _amount, uint256 _timeToMaturity ) external;
}
