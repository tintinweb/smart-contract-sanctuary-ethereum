...
contract OptionContract {
    ...
    function buyCallOption(uint amount, uint  currentPrice, uint strikePrice, uint expirationBlock) external payable returns (bool) {
        ...
        require(deltaT <= 31536000, "Max expiration time is 1 year");

        uint CallOptionPrice;
        CallOptionPrice = BlackScholes(currentPrice, strikePrice, expirationBlock);
        ...
        CallOptions[msg.sender][ID].expirationBlock = expirationBlock;
    ...
    function buyAtStrike(uint ID) public payable returns (uint) {
        ...
        uint expirationBlock;
        expirationBlock = CallOptions[msg.sender][ID].expirationBlock;

        require(expirationBlock < block.timestamp, "Can only execute at expiration time i.e. European Option");
