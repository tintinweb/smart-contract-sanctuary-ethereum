...
contract ERC20NonTransferable is ERC20Upgradeable {
    ...
    address public daoOwnerAddress;
    ...
    modifier onlyOwner() {
        require(daoOwnerAddress == msg.sender, "Only Owner");
        _;
    }
    ...
    function updateOwnerFee(uint256 _ownerFeePerDeposit) external onlyOwner {
    ...
    function checkDeposit() public view returns(bool) {
        if(block.timestamp < depositCloseTime)
            return true;        
        else
            return false;
    }
    ...
    /// @param _totalRaiseAmount new raise amount
    function startDeposit(uint256 _days, uint256 _totalRaiseAmount)
        external
        onlyOwner
    {
        require(
            _days > 0,
            "Days should be grater than 0"
        );

        require(
            _totalRaiseAmount > 0,
            "Total raise amount should be greater than 0"
        ); //Total deposit check
        require(
            totalRaiseAmount <= _totalRaiseAmount,
            "Total raise amount should be grater than previous"
        );
    ...
    function airDropToken(
        ...
        require(
            _ownersAirdropFees < 100,
            "Owner fees should be less than 100"
        );
        require(
            _ownersAirdropFees >= 0,
            "Owner cannot be less than 0"
        );
    ...
    /// @param _ownerAddress reflect the symbol of Governance Token
    ...
    function initializeERC20(
        ...
        require(
            _ownerFee >= 0,
            "Owner fee cannot be less than 0"
        );
        require(_ownerFee < 100, "Owner fee cannot exceed 100");

        require(_ownerAddress != address(0), "Owner cannot be null");
...
contract Emitter is Initializable,AccessControl {
    ...
    //FACTORY EVENTS       
    event ChangeImplementationAddress(address indexed factory,address indexed newImplementation);
    event ChangedUSDCAddress(address indexed factory,address indexed USDC);
    event CreateDao(address indexed ownerAddress,address indexed proxy,string name, string symbol, uint256 totalDeposit,uint256 minDeposit, uint256 maxDeposit, uint256 ownerFee, uint256 _days, bool feeUSDC,uint256 quorum,uint256 threshold,address emitter);   
    event FactoryCreated(address indexed _implementation,address indexed _USDC,address indexed _factory, address _emitter);

    address private _factoryAddress;
    bytes32 public constant EMITTER = keccak256("EMITTER");

    function Initialize(address _implementation,address _USDC,address _factory) initializer public {
    ...
    function createDao(address _ownerAddress,address _proxy,string memory _name, string memory _symbol, uint256 _totalDeposit,uint256 _minDeposit, uint256 _maxDeposit, uint256 _ownerFee, uint256 _totalDays,bool _feeUSDC,uint256 _quorum,uint256 _threshold,address _emitter) public onlyOwner{
        _grantRole(EMITTER, _proxy);
        emit CreateDao(_ownerAddress,_proxy,_name,_symbol,_totalDeposit,_minDeposit,_maxDeposit,_ownerFee,_totalDays,_feeUSDC,_quorum,_threshold,_emitter);
    }

    //PROXY EVENTS
    event Deposited(address indexed _proxy, address indexed _depositor, address indexed _tokenAddress, uint256 _amount);
    event StartDeposit(address indexed _proxy, uint256 startTime, uint256 closeTime);
    event CloseDeposit(address indexed _proxy, uint256 closeTime);
    event UpdateMinMaxDeposit(address indexed _proxy, uint256 _minDeposit, uint256 _maxDeposit);
    event UpdateOwnerFee(address indexed _proxy, uint256 _ownerFee);
    event UpdateUSDCTokenAddress(address indexed _proxy,address indexed _USDCTokenAddress);
    event AirDropToken(address indexed _proxy,address _token, uint256 _amount,uint256 _ownersAirdropFees);
    event MintGTToAddress(address indexed _proxy, uint256[] _amount, address[] _userAddress);
    event UpdateGovernanceSettings(address indexed _proxy, uint256 _quorum, uint256 _threshold);
    event UpdateRaiseAmount(address indexed _proxy, uint256 _amount);    
    event SendCustomToken(address indexed _proxy, address _token, uint256[] _amount,address[] _addresses);
    event SendEth(address indexed _proxy,uint256[] _amount, address[] _addresses);

    function deposited(address _proxy, address _depositor, address _tokenAddress, uint256 _amount) public onlyRole(EMITTER) {
        emit Deposited(_proxy,_depositor,_tokenAddress,_amount);
    }

    function startDeposit(address _proxy, uint256 _startTime,uint256 _closeTime) public onlyRole(EMITTER) {
        emit StartDeposit(_proxy, _startTime, _closeTime);
    }

    function closeDeposit(address _proxy, uint256 _closeTime) public onlyRole(EMITTER) {
        emit CloseDeposit(_proxy, _closeTime);
    }

    function updateMinMaxDeposit(address _proxy, uint256 _minDeposit,uint256 _maxDeposit) public onlyRole(EMITTER) {
        emit StartDeposit(_proxy, _minDeposit, _maxDeposit);
    }

    function updateOwnerFee(address _proxy, uint256 _ownerFee) public onlyRole(EMITTER) {
        emit UpdateOwnerFee(_proxy,  _ownerFee);
    }

    function updateUSDCTokenAddress(address _proxy,address _USDCTokenAddress)  public onlyRole(EMITTER) {
        emit UpdateUSDCTokenAddress(_proxy,_USDCTokenAddress);
    }

    function airDropToken(address _proxy,address _token, uint256 _amount,uint256 _ownersAirdropFees) public onlyRole(EMITTER) {
        emit AirDropToken(_proxy,_token,_amount,_ownersAirdropFees);
    }

    function mintGTToAddress(address _proxy, uint256[] memory _amount, address[] memory _userAddress) public onlyRole(EMITTER) {
        emit MintGTToAddress(_proxy,_amount, _userAddress);
    }

    function updateGovernanceSettings(address _proxy, uint256 _quorum, uint256 _threshold) public onlyRole(EMITTER) {
        emit UpdateGovernanceSettings(_proxy, _quorum, _threshold);
    }

    function updateRaiseAmount(address _proxy, uint256 _amount) public onlyRole(EMITTER) {
        emit UpdateRaiseAmount(_proxy, _amount);
    }

    function sendCustomToken(address _proxy, address _token, uint256[] memory _amount,address[] memory _addresses) public onlyRole(EMITTER) {
        emit SendCustomToken(_proxy, _token,_amount,_addresses);
    }

    function sendEth(address _proxy,uint256[] memory _amount, address[] memory _addresses) public onlyRole(EMITTER) {
