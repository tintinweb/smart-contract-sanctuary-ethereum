    ...
    IClubrareMarketplace
{
    ...
    //treasuryWallet to manage admin royalties and sell fee
    ...
    function buy(
        ...
        uint256 earning = settlement(nftContract, _tokenId, paid, msg.sender, _order,false);
        ...
        isNotBlacklisted(msg.sender)
    {
        ...
        settlement(nftContract, _tokenId, amt, taker, _order,false);
    ...
    function claim(Order calldata order) external whenNotPaused nonReentrant isNotBlacklisted(msg.sender) {
        ...
        settlement(nftContract, _tokenId, auction.currentBid, auction.highestBidder, _order,true);
        emit Claimed(
            _order.contractAddress,
            _tokenId,
    ...
    function _chargeAndSplit(
        ...
        uint256 stakingSplit = (stakeFee * _amount) / FEE_DENOMINATOR;
        uint256 lpSplit = (lpStakeFee * _amount) / FEE_DENOMINATOR;
        IERC20Upgradeable ptoken = IERC20Upgradeable(_erc20Token == address(0) ? wETHAddress : _erc20Token);
        ...
        if (_erc20Token == address(0)) {
            ...
            ptoken.transferFrom(user,treasuryWallet,(lpSplit + stakingSplit));
        ...
        return _amount - (stakingSplit + lpSplit + platformFee + royaltyValue);
    }
    ...
    function _executeExchange(
        ...
        _invalidateSignedOrder(order.contractAddress, _tokenId, order,isClaim);
        bool isToken = order.paymentToken == address(0) ? false : true;
    ...
    function _invalidateSignedOrder(
        ...
        bool isAdmin = validator.admins(signer);
        address owner = (isClaim || isAdmin) ? address(this) : signer;
       require(!isMinted || owner == IERC721Upgradeable(contractAddress).ownerOf(_tokenId),"not minted or not owner");
    ...
    EIP712Upgradeable
{
    ...
    function onAuction(uint _time) external pure returns(bool){
        return _time >0;
    }
    /**
...
interface IMarketplacePayment is IClubrareMarketplace {
    function _setFeeSplit(FeeSplit[] calldata _feeSplits, bool isUpdate) external;
