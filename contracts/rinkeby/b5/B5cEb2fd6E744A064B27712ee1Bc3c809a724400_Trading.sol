...
contract Data is IData,ERC1155Holder{
    ...
    Offer[] public offers;
    mapping(uint256 => uint256[]) public offerInfo;
    ...
    constructor(){
        manager = msg.sender;
        Offer memory offer = Offer(address(0),0,block.timestamp,PriceState.invalid);
        offers.push(offer);
    }
    ...
    function createOrder(TradingType tType,address token,address creator,uint256 id,uint256 amount,uint256 single,uint256 startTime,
        ...
        if(tType == TradingType.trading){
            ...
            FixedOption memory option = FixedOption(token,creator,id,amount,single,startTime,endTime,State.solding);
        ...
        if(tType == TradingType.auction){
            ...
            AuctionOption memory auction = AuctionOption(token,creator,id,amount,single,amount.mul(expect),startTime,endTime,address(0),amount.mul(single),State.solding);
    ...
    function joinBidding(uint256 orderId,address customer,uint256 price) public OnlySync returns(uint256 offerId){
        ...
        Offer memory offer = Offer(customer,price,block.timestamp,PriceState.active);
        offers.push(offer);
    ...
    function cancelBidding(uint256 orderId,uint256 offerId) public{
    ...
    function deliveryOrder(uint256 orderId,address customer,uint256 price) public {
        AuctionOption storage auction = auctionOptions[orderId];
...
interface ISynchron is IEnum{
    ...
    function auctionOptions(uint256 orderId) external view returns(address token,address creator,uint256 id,uint256 amount,uint256 single,
        uint256 expect,uint256 startTime,uint256 endTime,address purchaser,uint256 purchasPrice,State state);
    ...
    function fixedOptions(uint256 orderId) external view returns(address token,address creator,uint256 id,uint256 amount,uint256 single,
        uint256 startTime,uint256 endTime,State state);
    //bidder出价者地址，price出价价格，time出价时间，pState出价状态 0/1 有效/无效
    function offers(uint256 offerId) external view returns(address bidder,uint256 price,uint256 time,PriceState pState);
    //更具订单编号获取该订单的所有出价编号
    function offerInfo(uint256 orderId) external view returns(uint256[] memory);
...
interface ITrading is IEnum{
    ...
    //tradingType 0/1 固定价格/拍卖订单，订单类型；orderId对应订单类型的订单编号
...
contract Trading is IEnum,ERC1155Holder{
    ...
    function cancelOffer(uint256 orderId,uint256 offerId) public {
        ...
        require(block.timestamp < endTime && state == State.solding,"Trading:No permit");
        ISynchron(data).cancelBidding(orderId, offerId);
        emit Bidding(msg.sender, orderId, offerId,0, block.timestamp);
    }

    //获取可交割的出价id
    function getActiveOffers(uint256 orderId) public view returns(uint256){
        uint256[] memory offers = ISynchron(data).offerInfo(orderId);
        if(offers.length == 0){
            return 0;
        }
        uint256 payId;
        uint i = offers.length -1 ;
        while(offers[i] >0 ){
            (address bidder,uint256 price,,) = ISynchron(data).offers(offers[i]);
            uint256 balanceBidder = IERC20(weth).balanceOf(bidder);
            if(balanceBidder >= price){
                payId = offers[i];
                break;
            }
            i--;
    ...
    function modify(TradingType tradingType,uint256 orderId,uint256 price,uint256 start,uint256 end) public{
