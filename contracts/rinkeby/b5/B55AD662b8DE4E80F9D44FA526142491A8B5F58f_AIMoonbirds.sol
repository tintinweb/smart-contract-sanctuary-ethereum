...
contract AIMoonbirds is ERC721Optimized, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_MBAI = 5000;
    uint256 public maxMBAIPurchase = 10;
    uint256 public MBAIPrice = 0.1 ether;
    ...
    function mintMBAI(uint256 numberOfTokens) public payable nonReentrant {
        require(
            (MBAIPrice * numberOfTokens) <= msg.value,
            "Ether value sent is not correct"
        );
        require(
            numberOfTokens <= maxMBAIPurchase,
            "You can mint max 10 MBAIs per transaction"
        );
        require(
            (totalSupply() + numberOfTokens) <= MAX_MBAI,
            "Purchase would exceed max supply of MBAIs"
        );
