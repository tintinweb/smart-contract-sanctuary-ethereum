...
abstract contract Mixup is MerkleTreeHistory, ReentrancyGuard {
  ...
  */
  ...
  constructor(IVerifier _verifier, address _hasher, uint256 _denomination, uint32 _merkleTreeHeight, uint256 _commission) MerkleTreeHistory(_merkleTreeHeight, _hasher) {
...
contract ETHMixup is Mixup, ERC2771Context {
    constructor(MinimalForwarder forwarder, IVerifier _verifier, address _hasher, uint256 _denomination, uint32 _merkleTreeHeight, uint256 _commission) Mixup(_verifier, _hasher, _denomination, _merkleTreeHeight, _commission) ERC2771Context(address(forwarder)) {}

    function _processDeposit() internal override {
        require(msg.value == denomination+commission, "Mixup: Please send `mixDenomination` ETH along with transaction");
    }
