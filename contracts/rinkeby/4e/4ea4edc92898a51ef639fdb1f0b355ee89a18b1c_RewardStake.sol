...
contract RewardStake is Ownable {
 ...
 /* returns length for a user allocation */
      function UserAloLen(address beneficiary) public view returns (uint256){
            return userAllocations[beneficiary].length;
      }
   ...
   //change vesting duration. Can only be called by owner.
    ...
    function releaseMultiple(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            release(_beneficiary, _ids[i], _pid);
        }
    }
