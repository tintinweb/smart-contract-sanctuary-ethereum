...
contract LandBoxer is Context, Ownable {
        ...
        onlyOwner
    {
        ...
        for (uint8 i=0; i<_zoneIds.length; i++) {
            _metaTrade(to, _getZoneId(_zoneIds[i]), _scales[i], _amounts[i]);
        }
    }
    ...
    function _metaTrade(bytes32 _zoneId, uint256 _scale, PriceInfo memory _priceInfo, uint256 _amount) private {
        ...
        (tokenIds, seeds) = _metaTradeBasic(_msgSender(), _zoneId, _scale, _amount);
    ...
    function _metaTrade(address to, bytes32 _zoneId, uint256 _scale, uint256 _amount) private {
        uint256[] memory tokenIds;
        uint256[] memory seeds;
        (tokenIds, seeds) = _metaTradeBasic(to, _zoneId, _scale, _amount);
    ...
    function _metaTradeBasic(address to, bytes32 _zoneId, uint256 _scale, uint256 _amount) private returns (uint256[] memory, uint256[] memory) {
        ...
        for (uint256 i=0; i<_amount; i++) {
            ...
            IERC721WithAutoId(landAddress).mint(to);
