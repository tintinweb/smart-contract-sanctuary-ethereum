...
interface IStRSRVotes is IVotesUpgradeable, IStRSR {
...
/*
 * @title StRSRVotes
 * @notice StRSRVotes is an extension of StRSRP1 that makes it IVotesUpgradeable.
 *   It is heavily based on OZ's ERC20VotesUpgradeable
 */
contract StRSRVotes is StRSRP1, IVotesUpgradeable {
    struct Checkpoint {
        uint32 fromBlock;
        uint224 val;
    }
...
abstract contract StRSRP1 is Initializable, ContextUpgradeable, IStRSR, EIP712Upgradeable {
    ...
    string public override name; // mutable
    string public override symbol; // mutable
    ...
    /// @custom:refresher
    function payoutRewards() external override {
        _payoutRewards();
    }
    ...
    function withdraw(address account, uint256 endId) external override {
        // == Refresh ==
    ...
    /// @custom:protected
    function seizeRSR(uint256 rsrAmount) external override {
    ...
    /// @return {qStRSR/qRSR} The exchange rate between StRSR and RSR
    function exchangeRate() public view override returns (uint192) {
        return stakeRate;
    }
    ...
    ///     than linear search?
    function endIdForWithdraw(address account) external view override returns (uint256) {
    ...
    function setUnstakingDelay(uint32 val) public {
    ...
    function setRewardPeriod(uint32 val) public {
...
/* @dev Solidity 0.8.x only allows you to change one of type or size per type conversion.
   ...
   See: https://docs.soliditylang.org/en/v0.8.4/080-breaking-changes.html#new-restrictions
 */
