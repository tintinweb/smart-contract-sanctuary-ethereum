...
contract Skater is ERC721A,ERC721AQueryable, Ownable {
    
    uint256 public MINT_RATE = 0.07 ether;
    uint256 public MAX_SUPPLY = 5000;
    uint256 public MAX_MINTS = 3;
    uint256 public LIMIT_PER_TIME_MINT = 2;
    uint256 public DELAY_MINT = 1 minutes;
    uint256 public MAX_OG = 2;
    uint256 public MAX_WL = 1;
    ...
    function publicMint(uint256 _mintAmount) external payable {
        ...
        require(_mintAmount <= LIMIT_PER_TIME_MINT,"limit per time mint");
    ...
    function privateMint(uint256 _mintAmount,bytes32[] calldata _merkleProof_og,bytes32[] calldata _merkleProof_wl) external payable {
        ...
        require(_mintAmount + _numberMinted(msg.sender) <= max_mint,"Max mint per whitelisted"); 
        _safeMint(msg.sender, _mintAmount);
    }
    ...
    function screening(uint256 _mintAmount,uint256 _value) private view returns (bool){
        ...
        require(_mintAmount + _numberMinted(msg.sender) <= MAX_MINTS,"Max mint amount per session exceeded");
        return true;
    }
