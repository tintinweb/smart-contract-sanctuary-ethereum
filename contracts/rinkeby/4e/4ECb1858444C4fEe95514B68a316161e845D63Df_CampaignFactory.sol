...
contract CampaignFactory is Ownable {
    ...
    mapping(uint256 => _Campaign) private s_campaigns;
    ...
    function createCampaign(
        ...
        // Added factory clone pattern to reduce gas costs
        // Deploys one instance of campaign contract and all other instances will behave as proxies
        // Each instance will delegate calls to the first instance and allow functions to run in the
        // context of the proxy contracts
        address clone = Clones.clone(campaignImplementation);
        Campaign(clone).initialize(_minimum, msg.sender);

        s_deployedCampaigns.push(payable(clone));

        s_numberOfCampaigns++;
        _Campaign storage _campaign = s_campaigns[s_numberOfCampaigns];
        _campaign.id = s_numberOfCampaigns;
        _campaign.campaign = clone;
    ...
    {
        _Campaign storage _campaign = s_campaigns[_id];
        return (
            _campaign.id,
            _campaign.campaign,
            _campaign.title,
...
error Campaign__RequestAlreadyFinalized();
error Campaign__TransferFailed();
error Campaign__RequestDoesNotExist();
error Campaign__AddressIsZero();
...
contract Campaign {
    ...
    uint256 private i_minimumContribution;
    ...
    function initialize(
        ...
        if (_creator == address(0x0)) revert Campaign__AddressIsZero();
        i_manager = _creator;
        i_minimumContribution = _minimum;
    }
    ...
    function createRequest(
        ...
        if (_value <= 0) revert Campaign__ReqAmountNotGreaterThanZero();
    ...
    function approveRequest(uint256 _id) public payable {
        Request storage request = s_requests[_id];

        // check to make sure request exists before finalizing request
        if (request.recipient == address(0x0) && _id != request.id)
    ...
    function finalizeRequest(uint256 _id) public onlyManager {
        Request storage request = s_requests[_id];
        ...
        if (request.complete) revert Campaign__RequestAlreadyFinalized();
