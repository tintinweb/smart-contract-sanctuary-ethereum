...
{   
    ...
    mapping(uint => mapping(uint => uint)) public UserSelectedSlot;
     ...
     **/
    event LiveMintComplete(address Redeemer, uint TicketTokenID, uint CitizenTokenID);

    /**
     * @dev Emitted When `Redeemer` IRL-mints Artist Corresponding To `ArtistID`
     */
    event LiveMintArtistComplete(address Redeemer, uint TicketTokenID, uint ArtistID);

    /**
     * @dev Emitted When `Redeemer` Redeems Golden Token Corresponding To `TicketTokenID` 
     **/
    event GoldenTokenRedeemed(address Redeemer, uint TicketTokenID, uint slotClaimed);

    /**
     * @dev Emitted When `Redeemer` Redeems Artist Mint Pass Corresponding To `TicketTokenID`
     */
    event ArtistMintPassRedeemed(address Redeemer, uint  ArtistID, uint TicketTokenID, uint Slot);
     ...
     **/
    function RedeemGT(uint TicketTokenID, uint TimeSlot) public nonReentrant whenNotPaused
    {
        require(IERC721(_GoldenTokenAddress).ownerOf(TicketTokenID) == msg.sender, "Sender Does Not Own Token With The Input Token ID");
        require(UserSelectedSlot[_CurrentCityIndex][TimeSlot] > 0, "Timeslot Full");
        UserSelectedSlot[_CurrentCityIndex][TimeSlot]--;
        ...
        emit GoldenTokenRedeemed(msg.sender, TicketTokenID, TimeSlot);
    }

    /**
     * @dev Redeems Artist Mint Pass & BrightLists Address To Receive CryptoCitizen
     **/
    function RedeemArtistPass(uint ArtistID, uint TicketTokenID, uint TimeSlot) public nonReentrant whenNotPaused
    {
        ...
        emit ArtistMintPassRedeemed(msg.sender, ArtistID, TicketTokenID, TimeSlot);
    }
    ...
    function _LiveMint(uint TicketTokenID) public onlyMinter whenNotPaused 
    {
        ...
        require(!mintedCitizen[_CurrentCityIndex][TicketTokenID], "LiveMint: Golden Token Already Minted");
        mintingBrightList[_CurrentCityIndex][TicketTokenID] = address(0);
        mintedCitizen[_CurrentCityIndex][TicketTokenID] = true;
    ...
    function _LiveMintArtist(uint ArtistID, uint TicketTokenID) public onlyMinter whenNotPaused 
    {
        ...
        require(to != address(0), "LiveMint: Mint Pass Entered Is Not Brightlisted");
        require(!mintedArtist[ArtistID][TicketTokenID], "LiveMint: Artist Mint Pass Already Minted");
        ...
        if(ArtBlocksMint)
        {
            ...
            IERC721(_ArtBlocksCoreContractAddress).transferFrom(address(this), to, tokenID);
        }
        else { IRandomEdition(ArtistContractAddress)._Mint(to, 1); }
     ...
     * note: Slot is denoted in YYYY-DD-MM with no -'s
     */
     ...
     * @dev note: MintingAddresses = address[] (this is the NFTs to be mintedCitizen)
    ...
    /**
     * @dev Returns Artist ArtBlocks Mint
     */
    function resolveArtistMint(uint ArtistID) public view returns(bool) { return MintType[_CurrentCityIndex][ArtistID]; }

    /**
     * @dev Returns Booked Timeslot For Corresponding `TicketID`
     */
    function readTimeSlot(uint TicketID) public view returns(uint) { return UserSelectedSlot[_CurrentCityIndex][TicketID]; }
