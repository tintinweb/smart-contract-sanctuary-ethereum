    ...
    ReentrancyGuard
{
    ...
    uint256 public mintRate = 0.20 ether; //0.25 ether;
    uint256 public whitelistMintrate = 0.10 ether; //price for whitelisted buyers
    uint256 public maxMintAmountWhiteList = 10;
  ...
  struct SaleConfig {
    ...
    uint256 auctionDropRate;
    bool auctionActive;
  }

  SaleConfig public saleConfig;
*/
    // event logMint(address mintWallet, uint256 quantity);
    ...
    /*
  function activateAuction(bool _state) public onlyOwner {
    saleConfig.auctionActive = _state;
  }
  ...
  function setSaleConfig(
    ...
    uint256 _auctionDropRate
  ) external onlyOwner {
    saleConfig.auctionStartPrice = _auctionStartPrice * (10**16); //To configure 0.3 ether enter 30
    saleConfig.auctionStartTime = _auctionStartTime;
    saleConfig.auctionEndTime = _auctionEndTime;
    saleConfig.auctionEndPrice = _auctionEndPrice * (10**16); //To configure 0.3 ether enter 30
    saleConfig.auctionDropRate = _auctionDropRate * (10**16); //To configure 0.3 ether enter 30
  }



  function getAuctionPrice() public view returns (uint256) {
    uint256 minutesElapsed = (block.timestamp - saleConfig.auctionStartTime) /
      (1200); // Drop the price every 20 minutes
    console.log("Minutes Elapsed: ", minutesElapsed);
    console.log(
      "Price Part2 Calculation ",
      saleConfig.auctionStartPrice -
        (minutesElapsed * saleConfig.auctionDropRate)
    );
    return
      saleConfig.auctionStartPrice -
      (minutesElapsed * saleConfig.auctionDropRate);
  ...
  function auctionMint(uint256 _mintAmount)
    ...
    auctionCompliance(_mintAmount)
  {
    console.log(block.timestamp);
    console.log(saleConfig.auctionStartTime);
    _safeMint(msg.sender, _mintAmount);
  }

  modifier auctionCompliance(uint256 _mintAmount) {
    require(saleConfig.auctionActive, "Auction is not active!");
    require(
      block.timestamp > (saleConfig.auctionStartTime),
      "Auction has not begun yet!"
    );
    require(block.timestamp < (saleConfig.auctionEndTime), "Auction Ended!");
    require(
      msg.value >= getAuctionPrice() * _mintAmount,
  ...
  */
    ...
    function teamMint(uint256 _mintAmount) public onlyOwner {
        require(
            numberMinted(tx.origin) + _mintAmount <= maxTeamMint,
    ...
    function mint(uint256 _mintAmount) public payable onlyAccounts {
    ...
    {
        // Verify whitelist requirements
        require(whitelistMintEnabled, "FreeMint is not enabled!");
        //   require(!whitelistClaimed[_msgSender()], "Address already claimed!");
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(
            MerkleProof.verify(_merkleProof, merkleRoot, leaf),
    ...
    function setMaxMintAmountWhiteList(uint256 _limit) public onlyOwner {
    ...
    function setWhitelistMintRate(uint256 _whitelistMintrate) public onlyOwner {
