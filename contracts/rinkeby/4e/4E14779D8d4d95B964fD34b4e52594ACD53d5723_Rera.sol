...
contract Rera is ERC721URIStorage, Ownable{
 
    event MintROR (address indexed minter, uint256 startWith, uint256 times);
    
    
    uint256 public totalROR;
    uint256 public totalCount = 5555; //bruhTotal
    uint256 public maxBatch = 20; // bruhBatch
    uint256 public price = 100000000000000; // 0.01 eth
    ...
    function mintROR(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalROR + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit MintROR(_msgSender(), totalROR+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalROR++);
        }
    } 
    ...
    function whitelistMint(uint _times) payable public canWhitelistMint {
        ...
        require(totalROR + _times <= totalCount, "Mint amount will exceed total collection amount.");
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalROR + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit MintROR(_addr, totalROR+1, 1);
        _mint(_addr, 1 + totalROR++);
    } 
