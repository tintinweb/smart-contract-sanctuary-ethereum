...
contract NupayICO is INupayICO, Ownable, ReentrancyGuard, Pausable {
    ...
    event AggregatorPriceFeedSetted(address aggregator);
    event Deposited(address indexed account, bytes32 heloAddress, uint256 amount, uint256 price);
    ...
    function deposit(bytes32 heloAddress) external payable whenNotPaused whenNotMigrated returns (bool) {
        ...
        emit Deposited(caller, heloAddress, value, price);
        return true;
    }
    ...
    function setAggregatorPriceFeed(address aggregator) external onlyOwner returns (bool) {
        _aggregatorPriceFeed = AggregatorV3Interface(aggregator);
        emit AggregatorPriceFeedSetted(aggregator);
        return true;
    }
