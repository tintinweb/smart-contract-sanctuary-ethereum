...
contract WTEST100 is ERC721, Ownable {using Strings for uint256; using Counters for Counters.Counter; Counters.Counter private supply;
  ...
  constructor() ERC721("WTEST100", "WT100") {setHiddenMetadataUri("ipfs://QmWrX4cCtnQZPbDDVH1WMibnLYQNyW68WrEVHeAPF7u8VP/hidden.json");}
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {require(!paused, "The contract is paused!"); 
        ...
        if (supply.current() < maxFree) {
            uint256 remainingFreeMint = maxFreeMintPerWallet - freeMintCountMap[msg.sender];
