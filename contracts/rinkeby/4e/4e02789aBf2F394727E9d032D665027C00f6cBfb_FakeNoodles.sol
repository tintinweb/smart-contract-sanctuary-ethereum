...
contract FakeNoodles is ERC721Enumerable, Ownable {
    using Strings for uint256;

    string  private baseURI;
    uint256 private maxTokens = 69;
    ...
    address private t1 = 0x7f88F2829e36BC7eC198bAC592729a8195C61f6a;
    ...
    constructor(string memory __baseURI) ERC721("Fake Noodles", "FNOODS") {
        setBaseURI(__baseURI);
    }

    function merkleMint(bytes32[] calldata _merkleProof) public payable {
        uint256 supply = totalSupply();

        require(publicMintStatus == EPublicMintStatus.ALLOW_LIST, "Allow list closed.");
        require(!allowListClaimed[msg.sender],                    "Already claimed allow list.");
        require(!paused,             "Minting is paused.");
        require(supply <= maxTokens, "Exceeds maximum token supply.");
