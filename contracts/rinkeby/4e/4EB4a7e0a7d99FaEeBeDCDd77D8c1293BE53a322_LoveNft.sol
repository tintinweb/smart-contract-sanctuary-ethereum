...
contract LoveNft is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public maxSupplyTotal = 10000;
    uint256 public maxSupplyPrivate = 8000;
    ...
    mapping(address => uint256) private mintedWallets; // address to number of nft bought
    mapping(address => uint256) private walletsPayed; // address to eth spend
    ...
    uint256 public discountRate = 600; // 10 min
    uint256 public discountPercentage = 17; // 17%
    uint256 public duration = 12000; // 200 minutes
    uint256 public startAt;
    uint256 public expiresAt;
    uint256 public lastMintedPrice;
        ...
        mintCompliance(_mintAmount, maxSupplyTotal)
    {
        ...
        require(
            msg.value >= reservePrice * _mintAmount,
        ...
        mintCompliance(_mintAmount, maxSupplyTotal)
    {
        ...
        uint256 _refund = msg.value - (_price * _mintAmount);
        uint256 pricePayed = msg.value - _refund;
        _safeMint(_msgSender(), _mintAmount);
        walletsPayed[_msgSender()] += pricePayed;
        lastMintedPrice = pricePayed;
        if (_refund > 0) {
            payable(_msgSender()).transfer(_refund);
        }
    }

    function refund() public {
        require(block.timestamp > expiresAt, "auction not ended");
        uint256 bal = walletsPayed[_msgSender()];
        uint256 refundAmount = bal - lastMintedPrice;
        if (refundAmount > 0) {
    ...
    {
        require(_exists(tokenId), "No data for tokenId");
    ...
    function updateMaxSupplyPrivate(uint256 _number) public onlyOwner {
        require(
            ...
            "Private supply can't exceed total supply"
        );
