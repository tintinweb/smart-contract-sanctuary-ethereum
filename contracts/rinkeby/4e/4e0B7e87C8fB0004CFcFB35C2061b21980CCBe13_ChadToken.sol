...
contract ChadToken is Context, IERC20Upgradeable {
    ...
    mapping (address => bool) private _isExcludedFromPrize; // added by codecookerpro
    ...
    uint256 public PrizeShare = 0;  // added by codecookerpro
    ...
    uint64 public subscriptionId;  // added by codecookerpro
    ...
    function intializeContract(address payable setMarketWallet, address payable setDevWallet, string memory _tokenname, string memory _tokensymbol, uint64 _subscriptionId) external onlyOwner { // _subscriptionId added by codecookerpro
        ...
        subscriptionId = _subscriptionId; // added by codecookerpro
        ...
        startingSupply = 1_000_000_000; // modified by codecookerpro from 1_000_000_000_000 to 1_000_000_000
...
//===============================================================================================================
    ...
    function isExcludedFromPrize(address account) public view returns(bool) {
        return _isExcludedFromPrize[account];
    }
    // end
    ...
    function setValues(uint256 ms, uint256 ds, uint256 ps, uint256 vd) external onlyOwner {
        MarketShare = ms;
        DevShare = ds;
        PrizeShare = ps;
        ValueDivisor = vd;
    ...
    function setSubscriptionId(uint64 value) public onlyOwner {
    ...
    function getLotteryEntry(address account) public view returns (uint256) {
       return lottery.balances(account); 
    }

    function getWinnerIndex() public view returns (uint256) {
        return lottery.winnerIndex();
    }

    function getRandomSum() public view returns (uint256) {
        return lottery.randomSum();
    }

    function getRequestedId() public view returns (uint256) {
        return lottery.s_requestId();
    }
    // end
    ...
    function swapAndLiquify(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        if (contractTokenBalance - toLiquify > 0) {
            ...
            uint256 prizefeeshare = OperationsFee/(ValueDivisor)*(PrizeShare);  // added by codecookerpro
            _marketWallet.transfer(marketFee);
            _devWallet.transfer(devfeeshare);   
            payable(address(lottery)).transfer(prizefeeshare);     // added by codecookerpro    
