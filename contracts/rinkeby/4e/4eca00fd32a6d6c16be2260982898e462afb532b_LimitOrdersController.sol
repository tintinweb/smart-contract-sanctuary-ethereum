...
interface IPositionRouter {
    ...
    function executionComplite(uint256 orderId) external;
}
...
// File: contracts\LimitOrdersController.sol
...
/// @dev Contract use UUPS proxy.
contract LimitOrdersController is
    Initializable,
    UUPSUpgradeable,
    ...
    PausableUpgradeable
{
    ...
    function setPositionRouter(address _positionRouter)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        positionRouter = IPositionRouter(_positionRouter);
        ...
        allowedOrder(_tokenId, _poolAddress)
    {
        ...
        if (order.orderType == OrderType.BUY_LIMIT) {
            require(transferedToken0 >= order.receivedAmount, "EXEO5");
        } else if (order.orderType == OrderType.TAKE_PROFIT) {
            require(transferedToken1 >= order.receivedAmount, "EXEO5");
        }
        if (order.orderType == OrderType.BUY_LIMIT) {
        ...
        if (
            address(positionRouter) != address(0) &&
            address(positionRouter) == order.owner
        ) {
            positionRouter.executionComplite(_tokenId);
        }
    }
        ...
        if (_editParams.amountOfToken == 0) {
            ...
            if (order.orderType == OrderType.TAKE_PROFIT) {
                if (_editParams.amountOfToken < order.amountOfToken0) {
                    ...
                    order.receivedAmount -= LiquidityAmounts
