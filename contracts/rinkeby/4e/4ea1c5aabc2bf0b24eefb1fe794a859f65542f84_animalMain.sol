...
contract animalMain is  ERC721,Ownable {
  ...
  event Create(uint _tokenId, uint8 dna,uint8 star, uint8 roletype);
  event CreateInfo( uint8 dna,uint8 star, uint8 roletype);
  ...
  function seeAnimalDna(uint256 _tokenId) public view returns (uint8 dna) {
  ...
  function seeAnimalRole(uint256 _tokenId) public view returns (uint8 roletype) {
    return animals[_tokenId].roletype;
  }

   function seeAnimalRoleName(uint256 _tokenId) public view returns (string Name) {
    
    string memory RoleName;
    
    if(animals[_tokenId].roletype==0)
    {
        RoleName="老虎";
    }

     if(animals[_tokenId].roletype==1)
    {
        RoleName="獅子";
    }

     if(animals[_tokenId].roletype==2)
    {
        RoleName="狗";
    }
  ...
  function createAnimal() public {
     ...
     // emit CreateInfo(uint8(dna),uint8(star),uint8(roletype));
      uint id = animals.push(animal(uint8(dna), uint8(star), uint8(roletype))) - 1;
       emit Create(id,uint8(dna),uint8(star),uint8(roletype));
    ...
    // a random number
    function randMod(uint _modulus) view public returns(uint)
    {
    // increase nonce
   
    return uint(keccak256(abi.encodePacked(now, msg.sender,uint8(0)))) % _modulus;
    }
