...
library OnChainFantasyRenderer {
    ...
    function getProperties(
        ...
        trait.qb1 = teams[shuffledTeams[((qbIndex % teams.length))]];
        trait.qb2 = teams[shuffledTeams[(((qbIndex + 1) % teams.length))]];

        uint256 rbIndex = (uint256(
            keccak256(abi.encode(seed, keccak256("runningback")))
        ) % 1000000000) / 10000000;

        trait.rb1 = teams[shuffledTeams[((rbIndex % teams.length))]];
        trait.rb2 = teams[shuffledTeams[(((rbIndex + 1) % teams.length))]];
        trait.rb3 = teams[shuffledTeams[(((rbIndex + 2) % teams.length))]];
        trait.rb4 = teams[shuffledTeams[(((rbIndex + 3) % teams.length))]];

        uint256 wrIndex = (uint256(
            keccak256(abi.encode(seed, keccak256("widereceiver")))
        ) % 1000000000) / 10000000;

        trait.wr1 = teams[shuffledTeams[((wrIndex % teams.length))]];
        trait.wr2 = teams[shuffledTeams[(((wrIndex + 1) % teams.length))]];
        trait.wr3 = teams[shuffledTeams[(((wrIndex + 2) % teams.length))]];
        trait.wr4 = teams[shuffledTeams[(((wrIndex + 3) % teams.length))]];
        trait.wr5 = teams[shuffledTeams[(((wrIndex + 4) % teams.length))]];

        uint256 teIndex = (uint256(
            keccak256(abi.encode(seed, keccak256("tightend")))
        ) % 1000000000) / 10000000;

        trait.te1 = teams[shuffledTeams[((teIndex % teams.length))]];
        trait.te2 = teams[shuffledTeams[(((teIndex + 1) % teams.length))]];

        uint256 defIndex = (uint256(
            keccak256(abi.encode(seed, keccak256("defense")))
        ) % 1000000000) / 10000000;

        trait.def1 = teams[shuffledTeams[((defIndex % teams.length))]];
        trait.def2 = teams[shuffledTeams[(((defIndex + 1) % teams.length))]];
