...
contract Multisend is Ownable {
    ...
    event AirdropEvent(string indexed eventName, uint256 total, uint time);
    
    constructor() public {
    }

    function multisendAirdrop(address _token, address[] memory _sender, uint256 _amount, string memory _note) external onlyAdmin{
        address[] memory _temp = _sender;
        
        for(uint j=0;j<_temp.length;j++){
            require(_transferAirdrop(_token, _temp[j], _amount), "Failed");
        }
        emit AirdropEvent(_note, _amount.mul(_temp.length), block.timestamp);
    }
    
    function sendAirdrop(address _token, address _account, uint256 _amount) external onlyAdmin{
        require(_transferAirdrop(_token, _account, _amount), "Failed");
    }

    function _transferAirdrop(address _token, address _to, uint256 _amount) private returns(bool){
        require(_to != address(0), "Zero Address");
        require(_amount > 0, "Zero Amount");

        emit SendAirdrop(_to, _amount);
