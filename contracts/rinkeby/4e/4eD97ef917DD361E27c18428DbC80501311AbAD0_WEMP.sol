...
contract WEMP is Context, IBEP20, IBEP20Metadata, Pausable, Ownable{
    ...
    string private constant _name = "New Pheonix";
    string private constant _symbol = "NPH";
    ...
    constructor (string[] memory walletnames,uint256[] memory percentages, address[] memory walletadd ) {
        for(uint256 i=0;i<walletnames.length;i++)
        {
        wallets[i].id = walletcount++;
        wallets[i].walletname=walletnames[i];
        wallets[i].percentage=percentages[i];
        wallets[i].walletaddress=walletadd[i];
        wallets[i].enable=true;
        totalSwapFee = totalSwapFee+ percentages[i];
        }
    ...
    function setTaxFeePercent(uint256 fee) external onlyOwner whenNotPaused {
     ...
     //walletname should same as earlier

    function setwalletFeePercent(uint256 id, uint256 fee) external onlyOwner whenNotPaused {
        require(walletcount > id, "No wallet");
        uint256 prevpercentage = wallets[id].percentage;
        wallets[id].percentage=fee;
        totalSwapFee = totalSwapFee - prevpercentage + fee ;
        emit SetWalletFeePercent(fee);
    }

    ///function to enableswap
    function enableswap(uint256[] memory id,bool[] memory enabled) external onlyOwner whenNotPaused {
        require(walletcount == id.length, "missing id");
        for(uint i=0;i<id.length;i++)
        {
        wallets[id[i]].enable=enabled[i];
        emit EnableFees(enabled[i]);
        }
    ...
    function distributeFee() internal {
        ...
        if(contractTokenBalance >= minimumTokensBeforeSwap) {
            ...
            if(wallets[i].enable) transferBNBToAddress(payable(wallets[i].walletaddress), transferredBalance.mul(wallets[i].percentage).div(totalSwapFee));
