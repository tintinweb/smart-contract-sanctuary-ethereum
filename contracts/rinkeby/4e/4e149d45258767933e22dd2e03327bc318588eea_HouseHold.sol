...
import "./library/ProxyFactoryUtil.sol";
import "./library/ConfigsGetters.sol";


contract HouseHold is UpgradeablePausable, IHouseHold {
  using ConfigsGetters for HouseHoldConfigs;
  HouseHoldConfigs public hhConfig;

  struct MortgagesAddress {
    bool init;
    mapping(address => bool) mortgages;
    address[] addresses;
  }
  mapping(address => MortgagesAddress) public mortgagesBorrowers;
  ...
  function createMortgage(
    ...
    address mortgageAddress = hhConfig.getMortgageAddress();
    address mortgage = ProxyFactoryUtil.deployMinimal(mortgageAddress);
    IMortgage(mortgage).initializeContract(
      address(hhConfig),
      borrower,
      name,
      clientId,
    ...
    _addMortgage(borrower,mortgage);
    return mortgage;
  }
  
  function _addMortgage(address borrower, address mortgage) internal {
    if (!mortgagesBorrowers[borrower].init) {
      mortgagesBorrowers[borrower].init = true;
      mortgagesBorrowers[borrower].addresses = [mortgage];
      borrowers.push(borrower);
    } else{
      mortgagesBorrowers[borrower].addresses.push(mortgage);
    }
    mortgagesBorrowers[borrower].mortgages[mortgage] = true;
  }

  function getUserMortgages(address borrower) public view override returns (address[] memory) {
    return mortgagesBorrowers[borrower].addresses;
  }

  function getMortgageStatus(address borrower, address mortgage) public view override returns (bool) {
    return mortgagesBorrowers[borrower].mortgages[mortgage];
  }

  function getMortgagesBorrowers() public view override returns (address[] memory) {
    return borrowers;
  }
...
interface IHouseHold {
  ...
  function getUserMortgages(address borrower) external returns (address[] memory);

  function getMortgageStatus(address borrower, address mortgage) external view returns (bool);

  function getMortgagesBorrowers() external view returns (address[] memory);
}
...
import "../../interfaces/IHouseHold.sol";
import "../../interfaces/IMortgagePoolTokens.sol";
...
library ConfigsGetters {
  ...
  function getMortgagePoolAddress(HouseHoldConfigs config) internal view returns (address) {
    return config.getAddress(uint256(ConfigsEnums.ContractAddresses.MortgagePool));
  }

  function getMortgagePoolToken(HouseHoldConfigs config) internal view returns (IMortgagePoolTokens) {
    return IMortgagePoolTokens(getMortgagePoolTokenAddress(config));
  }

  function getMortgagePoolTokenAddress(HouseHoldConfigs config) internal view returns (address) {
    return config.getAddress(uint256(ConfigsEnums.ContractAddresses.MortgagePoolToken));
  }
...
interface IMortgagePoolTokens is IERC721Upgradeable {
  struct TokenInfoInput {
    uint256 capital;
    uint256 capitalRedeemed;
  ...
  function onCreated(address poolCreated) external;

  function getMortgagePoolToken(uint256 tokenId) external view returns (TokenInfo memory);

  function isValidPool(address sender) external view returns (bool);

  function mintToken(TokenInfoInput calldata params, address ownerTo, string calldata tokenURI) external returns (uint256 tokenId);
...
interface IHouseHoldPool {
  ...
  function drawdownFromFiat(uint256 user, uint256 date, address mortgage) external;
