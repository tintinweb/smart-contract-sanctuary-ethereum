...
contract StorageV2 is LogicUpgradeable, PausableUpgradeable {
        ...
        whenNotPaused
    {
        ...
        userBoost.rewardDebt = amount / totalSupplyBLID * accBlidPerShare;
        ...
        public
    {
        ...
        if (userBoost.blidDeposit > 0) {
            claimAmount = (userBoost.blidDeposit * accBlidPerShare / totalSupplyBLID) - userBoost.rewardDebt;
            if (claimAmount > 0) {
    ...
    function updateAccBlidperShare() public {
        ...
        accBlidPerShare = accBlidPerShare + (passedblockcount * blidPerBlock / 1e18 * blidSupply);
        lastRewardBlock = block.number;
    }
    ...
    function getBoostingClaimableBLID(address user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && blidSupply != 0) {
            ...
            _accBLIDpershare = accBlidPerShare + 
                (passedblockcount * blidPerBlock / 1e18 * blidSupply);
        }
        return (userBoost.blidDeposit / totalSupplyBLID * _accBLIDpershare) - userBoost.rewardDebt;
    }    
