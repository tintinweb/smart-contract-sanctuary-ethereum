...
contract WhalesTest is ERC721Enumerable, Ownable, RoyaltiesV2Impl, ReentrancyGuard  {
  ...
  constructor( string memory _name, string memory _symbol, string memory _theBaseURI, string memory _notRevealedURI, uint96 _royaltyFeesInBips, bytes32 _merkleRoot) ERC721(_name, _symbol) {
  ...
  function presaleMint(address _account, bytes32[] calldata _proof) external payable nonReentrant {
    ...
    setRoyalties(_nftIdCounter.current());
