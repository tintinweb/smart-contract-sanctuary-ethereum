...
contract PYTest is ERC721AQueryable, Ownable, ReentrancyGuard {

  using Strings for uint256;

// Enable mapping for whitelisting
...
//  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    bool isFree = ((totalFreeMinted + _mintAmount < totalFree + 1) &&
          (_mintedFreeAmount[msg.sender] < maxFreePer));
    

    if (isFree) {
        require(!paused, 'The contract is paused!');
        if(_mintAmount >= (maxFreePer - _mintedFreeAmount[msg.sender]))
        {
          require(msg.value >= (_mintAmount * cost) - ((maxFreePer - _mintedFreeAmount[msg.sender]) * cost), 'Insufficient Funds!');
          _mintedFreeAmount[msg.sender] = maxFreePer;
          totalFreeMinted += maxFreePer;
        }
        else if (_mintAmount < (maxFreePer - _mintedFreeAmount[msg.sender]))
        {
