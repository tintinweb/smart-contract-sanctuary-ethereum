    ...
    ReentrancyGuard
{
    ...
    function initialize(
        ...
        require(
            (_requireVoterTurnoutRate >=
                globalGovernanceSettings.minRequireVoterTurnoutRate()) &&
                (_requireVoterTurnoutRate <=
                    globalGovernanceSettings.maxRequireVoterTurnoutRate()),
            "Vote percentage threshold too high or too low"
        );
        require(
            globalGovernanceSettings.currencyToAcceptableForTrading(
                _currency
            ) == true,
            "The currency is not accepted now"
        ); //check trading ERC20 is enabled
    ...
    function setOwnerTradingFee(uint256 _ownerTradingFee) public {
        require(
            _ownerTradingFee <=
                globalGovernanceSettings.originalOwnerTradingFeeUpperBound(),
            "Too high owner fee"
        );
        require(
            currentState == State.normal,
            "Current state must not be in auction"
        );
    ...
    function setOwnerBuyoutFee(uint256 _ownerBuyoutFee) public {
        require(
            _ownerBuyoutFee <=
                globalGovernanceSettings.originalOwnerBuyoutFeeUpperBound(),
            "Too high owner fee"
        );
        require(
            currentState == State.normal,
    ...
    function _claimFees() internal {
        ...
        uint256 governanceFee = (globalGovernanceSettings.governanceFee() *
            totalSupply() *
            (block.timestamp - lastClaimed)) / 31536000000;
        uint256 originalOwnerBuyoutFee = (ownerBuyoutFee *
            totalSupply() *
            (block.timestamp - lastClaimed)) / 31536000000;
        lastClaimed = block.timestamp;
    ...
    function _updateReservePrice(uint256 _reservePrice, uint256 weight)
        internal
    {
    ...
    function startAuction(uint256 _price) public payable nonReentrant {
        ...
        require(
            totalVoted != 0,
            "Nobody has voted for auction length and bid increment"
        );
        require(
            balanceOf(msg.sender) >=
                (totalSupply() * requireShareholdingRatio) / 10000,
            "Your balance is not enough to buy"
        );
        require(
            _price >= reservePriceTotal,
            "Price lower than reserve price for all supply"
        );
        require(
            (totalVoted * 10000) / totalSupply() >= requireVoterTurnoutRate,
            "Not enough FNFT holders accept buyout"
        );
        if (currency == eth) {
            require(
                msg.value == _price,
                "Please send exact amount of ETH you specified to start an auction"
            );
        } else {
            require(
    ...
    function bid(uint256 _price) public payable nonReentrant {
        require(
            currentState == State.auctioning,
            "Current state not be in auction"
        );
        require(
            balanceOf(msg.sender) >=
                (totalSupply() * requireShareholdingRatio) / 10000,
            "Balance is not enough to bid"
        );
        require(
            _price >=
                (reservePriceTotal * bidIncrementTotal) /
                    10000 +
                    highestBidPrice,
            "Price too low"
        );
        ...
        if (currency == eth) {
            require(
                msg.value == _price,
                "Please send exact amount of ETH you specified to bid"
            );
            payable(highestBidder).transfer(highestBidPrice); //pay back money to the previous highest bidder
        } else {
            require(
                IERC20(currency).transfer(highestBidder, highestBidPrice),
                "No enough tokens"
            );
            require(
                IERC20(currency).transferFrom(
    ...
    function makeBuyoutOffer(uint256 _price) public payable nonReentrant {
        require(
            currentState == State.normal,
            "The NFT has been sold or auction is on-going"
        );
        require(
            allowDirectBuyout && directBuyoutEnabled,
            "Direct buyout is not allowed"
        );
        require(_price > 0);
        ...
        for (uint256 i = 0; i < owners.length; i++) {
            ...
            if (currency == eth) {
                require(
                    ...
                    "address's balance is not enough to pay"
                );
    ...
    function acceptBuyoutOffer(address buyerAddress) public nonReentrant {
        ...
        require(
            balanceOf(msg.sender) * 2 >= totalSupply(),
            "You don't have right to accept buyout offer"
        );
        currentState = State.directlySold;
        _claimFees();
    ...
    function claim() public nonReentrant {
        ...
        require(
            balanceOf(msg.sender) > 0 || msg.sender == highestBidder,
