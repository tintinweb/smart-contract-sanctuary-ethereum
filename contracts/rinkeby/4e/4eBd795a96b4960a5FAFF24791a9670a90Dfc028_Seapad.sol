...
contract Seapad is VRFConsumerBaseV2, ChainlinkClient {
        ...
        returns (uint256)
    {
        ...
        require(
            bal >= cost,
            string(
                abi.encodePacked("Top up required: reserved link balance required: ", SeapadHelper.uint2str(cost), " , you have: ", SeapadHelper.uint2str(bal))
            )
        ); //notenoughbalance
    ...
    function createTokenPart2(
        ...
        project.packAddress = minter.mintPacks(
            ...
            projects2.name,
            string(
                abi.encodePacked("SeaPad ProjectID: #", SeapadHelper.uint2str(pid))
            )
        );
    ...
    function makeChainLinkRequest(
        ...
        request.add("itemsPerPack", SeapadHelper.uint2str(amountToPayOutPerPack));
