...
// File: contracts/SimejiSeller.sol
...
abstract contract SimejiSeller is OwnerPausable, ReentrancyGuard {
    ...
    function cost(uint256 n) public virtual returns (uint256);
    ...
    whenNotPaused
    {
        ...
        _handlePurchase(_msgSender(), remain, true);
...
// File: contracts/OpenSeaGasFreeListing.sol
...
// File: contracts/ERC721Common.sol
...
contract SimejiNFT is ERC721Common, SimejiSeller {
    using Strings for uint256;
    uint256 PUBLIC_PRICE = 0.1 ether;
    uint256 PRE_PRICE = 0.08 ether;
    ...
    function publicBuy(uint256 requested) external payable {
        require(publicSaleEnable, "public sale has not enabled");
        SimejiSeller._purchase(msg.sender, requested);
    }

    function whitelistBuy(uint256 requested, bytes32[] calldata signature_) external payable {
    ...
    function cost(uint256 n) public view override returns (uint256) {
        if (preSaleEnable) {
            ...
            return n * PUBLIC_PRICE;
        }
    }

    function setPublicSaleEnable(bool enable) public onlyOwner {
        publicSaleEnable = enable;
    }

    function setPreSaleEnable(bool enable) public onlyOwner {
        preSaleEnable = enable;
    }
}