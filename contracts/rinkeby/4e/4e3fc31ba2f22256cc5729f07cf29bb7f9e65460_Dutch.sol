...
contract Dutch is ERC721A, Ownable {
    using Strings for uint256;
    using SafeMath for uint256;

    //Dutch auction logic
    uint256 public startingPrice = 20000000000000000; // 0.02eth
    uint256 public decreaseInterval = 300; // In seconds
    uint256 public decreasePerInterval = 5000000000000000; // 0.005 eth
    uint256 public endingPrice = 5000000000000000; // 0.005 eth
    uint256 public startingBlock;
    uint256 public totalInterval =
        (startingPrice - endingPrice) / decreasePerInterval;
    //Allowlist
    uint256 public allowlistPrice = 10000000000000000; // 0.01 eth
    ...
    constructor() ERC721A("Dutch", "Dutch") {}

    function setDutchStartingPrice(uint256 _startingPrice) public onlyOwner {
        startingPrice = _startingPrice;
    }

    function setDecreaseInterval(uint256 _decreaseInterval) public onlyOwner {
        decreaseInterval = _decreaseInterval;
    }

    function setDecreasePerInterval(uint256 _decreasePerInterval)
        public
        onlyOwner
    {
    ...
    function setDutchEndingPrice(uint256 _endingPrice) public onlyOwner {
    ...
    function allowlistMint(bytes memory sig, uint256 _mintAmount)
        ...
        payable
    {
        require(!_paused, "Dutch:: Contract is paused.");
        require(!_allowlistPaused, "Dutch:: Allowlist is paused.");
        require(isValidUser(msg.sender, sig), "Dutch:: Invalid signature");
        uint256 supply = currentSupply;
        require(
            supply + _mintAmount <= maxSupply,
            "Dutch:: Unable to mint amount, will exceed max supply. Try decreasing the quantity."
        );
        require(
            _mintAmount <= allowlistMaxMintAmount,
            "Dutch:: Unable to mint amount, will exceed allowlist limit."
        );
        require(
            _allowlistClaimed[msg.sender] < 4,
            "Dutch:: Unable to mint amount, will exceed allowlist limit."
        );
        require(
            msg.value >= allowlistPrice * _mintAmount,
            "Dutch:: Insufficient funds."
        );
    ...
    function mint(uint256 _mintAmount) public payable {
        uint256 supply = currentSupply;
        require(_mintAmount > 0, "Dutch:: Amount must be at least 1");
        require(
            supply + _mintAmount <= maxSupply,
            "Dutch:: Unable to mint amount, will exceed max supply. Try decreasing the quantity."
        );
        require(
            ...
            "Dutch:: Unable to mint amount, will exceed minting limit."
        );
        if (msg.sender != owner()) {
            require(!_paused, "Dutch:: Contract is paused.");
            require(_allowlistPaused, "Dutch:: Allowlist sale is ongoing.");
            require(startingBlock > 0, "Dutch:: Dutch has not started.");
            require(
