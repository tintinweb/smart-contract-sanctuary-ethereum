...
contract BrokenClub is ERC721A, Ownable, ReentrancyGuard {
    ...
    // START - Minting
    function whitelisMint(uint256 _count, bytes32[] calldata proof, bytes32 merkleRoot) external {
        ...
        // TODO: (IF REQUIRED) Any limit on whitelist sale can be put here
        // Basically if we want any whitelist member to have only "n" number of tokens
        require(_verify(_leaf(msg.sender), proof, merkleRoot), "Not a whitelisted member!");
        _safeMint(msg.sender, _count, "");
    }

    function mint(uint256 _count) external payable {
        require(!whitelistEnabled, "Whitelist sale is still active!");
