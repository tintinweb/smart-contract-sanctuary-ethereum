...
abstract contract MSB is ERC721Enumerable, Ownable {}
...
contract StakeMSB is Ownable{
    ...
    function claimTokenRewards(uint256 tokenId) external {
        ...
        claimAmount[tokenId] = calculateRewards(tokenId);
        isStaked[tokenId] = false;
        claimed[tokenId] = true;
    }

    function calculateRewards(uint256 tokenId) private view returns(uint256) {
        uint256 maxRewards;
        //rewards calculated at normal daily rate
        uint256 claims = (block.timestamp - stakingTimestamp[tokenId]).div(timeBlock).mul(dailyRewards);

        //reward multipler 2x if staked before stakingThresholdTime1
        if(stakingTimestamp[tokenId] <= stakingThresholdTime1) {
            claims = claims.mul(2); 
            maxRewards = maxNormalRewards.mul(2);
            if (claims > maxRewards) claims = maxRewards;
        }

        //reward multipler 1.5x if staked between stakingThresholdTime1 and stakingThresholdTime1
        else  if(stakingTimestamp[tokenId] > stakingThresholdTime1 && stakingTimestamp[tokenId] <= stakingThresholdTime2) {
            claims = claims.mul(3).div(2);
            maxRewards = maxNormalRewards.mul(3).div(2);
            if (claims > maxRewards) claims = maxRewards;
        }

        else {
            if (claims > maxNormalRewards) claims = maxNormalRewards;
        }

        return claims;
    }

    function getCurrentrewards(uint256  tokenId) external view returns(uint256) {
        bool status = ((isStaked[tokenId]) && msb.ownerOf(tokenId) == stakingAddress[tokenId]);
        if(status == false) return 0;
        else {
            uint256 currentRewards = calculateRewards(tokenId);
