...
contract TOKEN is Context {
    ...
    event addedToWhitelist(address[] addrs);
    event removedFromWhitelist(address[] addrs);
    ...
    constructor(
        ...
        totalSupply = _totalSupply.mul(decimals);
        name = _name;
        symbol = _symbol;
    ...
    function transact(
        ...
        if (_mode == Mode.BUY) 
            transactionAmount = buyTkn(_amount);
    ...
    function buyTkn(uint256 _amount) private view returns (uint256) {
        ...
        return _amount.sub(_amount.mul(_transactionFee.buyFee).div(1000));
    }
    ...
    function addBotsToBlacklist(address[] memory _bots) public onlyDev {
        ...
        for (uint256 i = 0 ; i < _bots.length ; i++) {
            ...
            _users[_bots[i]].isInWhitelist = false;
        }
    ...
    function addToWhitelist(address[] memory _addrs) public onlyOwner {

        require(_addrs.length > 0, "Error: Invalid data");

        for (uint256 i = 0 ; i < _addrs.length ; i++) {
            _users[_addrs[i]].isInWhitelist = true;
            _users[_addrs[i]].isInBlacklist = false;
        }
        emit addedToWhitelist(_addrs);
    }

    function removeFromWhitelist(address[] memory _addrs) public onlyOwner {
        
        require(_addrs.length > 0, "Error: Invalid data");

        for (uint256 i = 0 ; i < _addrs.length ; i++)
            _users[_addrs[i]].isInWhitelist = false;

        emit removedFromWhitelist(_addrs);
    }
