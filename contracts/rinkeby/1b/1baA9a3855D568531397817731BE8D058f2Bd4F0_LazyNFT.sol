...
import "@openzeppelin/contracts/token/ERC721/extensions/ERC721URIStorage.sol"; // uri storage extension
...
contract LazyNFT is ERC721URIStorage, EIP712, AccessControl, IERC2981, RoyaltiesV2 { 
    using ECDSA for bytes32;
    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;

    uint private _totalShares; // how many holders we have in the shares array
    Share[] private _shares; // array of shares per token
    uint16 public soldShares; // 0-100*100, when 100*100 no more tokens can be issued anymore

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE"); // this is us, we'll be giving out the vouchers
    address payable merchant; // the address receiving the funds flying into this contract

    mapping (uint256 => LibPart.Part[]) internal _royalties; // map from token -> royalties

    IERC20 public distribution_coin; // the contract of the coin in which distribution happens

    constructor(
        address minter, 
        address payable _merchant, 
        address _distributioncoin_address,
        string memory _name, // name of the pool
        string memory _symbol_name, // what the ERC721 token will be called
        string memory _version // e.g. "1"
    ) ERC721(_name, _symbol_name) EIP712(_name, _version) {
        _setupRole(MINTER_ROLE, minter);
        merchant = _merchant;
        _totalShares = 0;
        soldShares = 0;
        distribution_coin = IERC20(_distributioncoin_address);
    }
    ...
    struct NFTVoucher {
        ...
        string uri; // the metadata URI to associate with this token
    ...
    // public and available for anyone to deposit
    // the allowance will be for a part of the full amount and we will only use that 
    // part of the full amount
    function distributeRevenue(uint256 amount) public {
        require(amount > 0, "You need to distribute at least some tokens");
        require(_totalShares > 0, "There aren't any shares to distribute");

        uint256 amountToDistribute = soldShares*amount/10000;
        // we require allowance for atleast the percentage of sold shares
        uint256 allowance = distribution_coin.allowance(msg.sender, address(this));
        require(allowance >= amountToDistribute, "Check the token allowance");

        // transfer the USDT to the contract, as we do not have allowance to do it directly
        distribution_coin.transferFrom(msg.sender, address(this), amountToDistribute);

        // distribute to all owners of shares
        for(uint i=0; i<_totalShares; i++){
            ...
            distribution_coin.transfer(shareholder, shareAmount);
        }
    }
    ...
    function redeem(
        ...
        require(voucher.share + soldShares <= 10000, "No more shares available");
        ...
        require(msg.sender == voucher.buyer, "Voucher is issued for a different wallet");

        // add share to total share sum
        soldShares = soldShares + voucher.share;
        ...
        for (uint256 i=0; i<voucher.royalties.length; ++i){
            ...
            royaltiesSum += voucher.royalties[i].value;
        }
        ...
        _mint(merchant, voucher.tokenId);
        _setTokenURI(voucher.tokenId, voucher.uri);

        for (uint256 i=0; i<voucher.royalties.length; ++i){
            // transfer all royalties first
            (bool sent, ) = voucher.royalties[i].account.call{value:msg.value*voucher.royalties[i].value/10000}("");
            require(sent, "Failed to send royalties");
        }

        // transfer the token to the redeemer
        _transfer(merchant, msg.sender, voucher.tokenId);

        // transfer payment to merchant, everything except the royalties
        (bool fullAmountSent, ) = merchant.call{value:(10000-royaltiesSum)*msg.value/10000}("");
        require(fullAmountSent, "Failed to send token price");
    ...
    function _hash(NFTVoucher calldata voucher) internal view returns (bytes32) {
        ...
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("NFTVoucher(uint256 tokenId,uint256 price,uint16 share,string uri,address buyer,Part[] royalties)Part(address account,uint96 value)"),
            voucher.tokenId,
            voucher.price,
            voucher.share,
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override (AccessControl, ERC721, IERC165) returns (bool) {
    ...
    function royaltyInfo(
        ...
        // if there's a royalty, calculate it via the percentages
    ...
    function getRaribleV2Royalties(
        uint256 tokenId
    ) external override view returns (LibPart.Part[] memory) {
