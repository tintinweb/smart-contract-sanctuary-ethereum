...
contract Safezone is ISafezone, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
  ...
  uint256 public constant DAILY_SAFE_RATE = 25000 ether;
  ...
  function ownerInitToken(address buyer) external view returns (string[] memory) {

    string[] memory arr = new string[](21); // 21 empty items
    arr[0] = "";
    arr[1] = "";
    arr[2] = "";
    arr[3] = "";
    ...
    uint cpt = 0;
    for (uint i = 0; i < numHumansStaked; i++) {
          Stake memory stake = safezone[i];
          if (buyer == stake.owner){
            arr[cpt] = Strings.toString(stake.tokenId);
            cpt += 1;
          }
    }
    return arr;
