...
mapping(address => userInfo) public data;
   ...
   constructor(){
     ...
     function showrewards() view public returns(uint){
         uint reward = data[user].total_reward;
         return reward;
     }

      function claimReward(uint reward) public returns ( string memory msgs ){
         reward = data[user].total_reward;
         require(reward != 0,"no reward yet");
           ESTtoken.transfer(user,reward);
        data[user].total_reward=0;
