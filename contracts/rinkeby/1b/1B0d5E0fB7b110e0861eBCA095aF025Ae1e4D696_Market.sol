...
contract Market is MarkerModifier, IMarket {
    ...
    function biderOfToken(uint256 _tokenId) public view returns (Bider memory) {
        return _tokenIdToBider[_tokenId];
    }
    ...
    /*
        call Approve on the NFT contract to allow the market to transfer the token
     */
    function acceptBid(uint256 _tokenId) external onlyOwner(_tokenId) {
        require(address(0) != msg.sender, "Zero address is not allowed");
        uint256 _price = _tokenIdToBider[_tokenId].price;
        require(_price > 0, "Price must be greater than 0");
        address bider = _tokenIdToBider[_tokenId].bider;
        _nft.transferFrom(msg.sender, bider, _tokenId);
        _wrapETH.transferFrom(bider, msg.sender, _price);
        emit Bought(msg.sender, bider, _tokenId, _price);
        _tokenIdToBider[_tokenId] = Bider(address(0), 0, 0);
    }
    ...
    function buy(uint256 _tokenId) external onlyBuy(_tokenId) {
        ...
        _wrapETH.transferFrom(msg.sender, _seller, _tokenIdToPrice[_tokenId]);
        _tokenIdToPrice[_tokenId] = 0;
    ...
    function bid(uint256 _tokenId, uint256 _price) external notOwner(_tokenId) {
        ...
        require(_wrapETH.balanceOf(msg.sender) >= _price, "Not enough WETH");
        _tokenIdToBider[_tokenId] = Bider(msg.sender, _price, _tokenId);
        emit Bid(msg.sender, _tokenId, _price);
    }

    function cancelBid(uint256 _tokenId) external notOwner(_tokenId) {
        require(_tokenIdToPrice[_tokenId] == 0, "Token price must not sell");
        require(_tokenIdToBider[_tokenId].bider == msg.sender, "");
        _tokenIdToBider[_tokenId] = Bider(address(0), 0, 0);
