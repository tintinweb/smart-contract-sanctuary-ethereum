...
contract Doodle_Genesis2 is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function addALLPresaleUsers(address[] memory _users, uint addresslistno) public onlyOwner {
        for (uint i = 0; i < addresslistno; i++) {
            presaleWallets[_users[i]] = true;
        }
    }
   
    function removeAllPresaleUsers(address[] memory _users, uint addresslistno) public onlyOwner {
        for (uint i = 0; i < addresslistno; i++) {
