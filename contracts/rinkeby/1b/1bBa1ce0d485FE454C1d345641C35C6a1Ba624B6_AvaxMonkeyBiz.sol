...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    pragma solidity ^0.8.7;
    contract AvaxMonkeyBiz is ERC721Enumerable,  Ownable {
    using Strings for uint256;


   string private uriPrefix = "ipfs://Qma7aDfZ4MwZZpbA4i6whz85bfNTeXF4sWzZnvqkEGDnH2/" ;
  string private uriSuffix = ".json";
  string public revealURL = "ipfs://QmYA9Rx3HJ5UVrQX68wqKfCa7QLjJ1PDPRvrmEybZrh2m9/1.json";
 ...
 uint [] private tokens;
  ...
  mapping (address => uint256) txPerWLAddress;
  mapping(address => bool) public projectProxy;
  mapping (uint => uint) public bizlevel;
  ...
  constructor() ERC721("AvaxMonkeyBiz", "AMB") {
  ...
  function mint(uint256 _mintAmount) external payable  {
    ...
    bizlevel[totalSupply + i] = 1 ;
     }
     delete totalSupply;
     delete _mintAmount;
  }

  function updatebizlevel (uint tokenID) external
  {
     bizlevel[tokenID]++;
  }
 ...
 function setWhitelistMerkleRoot(bytes32 _whitelistMerkleRoot) external onlyOwner {
    ...
    function whitelistMint(uint256 _mintAmount, bytes32[] calldata merkleProof) external payable {
       ...
       require (_txPerAddress < maxWLMintAmountPerTx, "Exceeds max tx per address");
      require(_mintAmount <= maxWLMintAmountPerTx, "Exceeds max per transaction.");
  ...
  function gettokenss() external view returns (uint [] memory)
  {
    ...
    arr[i] = tokenOfOwnerByIndex( msg.sender, i);
