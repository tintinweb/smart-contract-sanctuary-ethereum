...
contract PlantContract is ERC721URIStorage {
  ...
  function water(uint256 tokenId) public {
    ...
    // If it's been too long since you watered the plant, the plant dies :( 
    if(block.timestamp - p.lastWaterTime > 8) {
      plantData[tokenId] = Plant(block.timestamp, PlantStage.SEED, 0, 0, block.timestamp);
