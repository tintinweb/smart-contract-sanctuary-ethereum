...
/**we c est vrai, apres k finis la minting dapp, j vais optimizer ce contract, enlever ERC721Enumerable parsque counter peut faire la mm chose et enlever d autre module si j ai le mm problem  */
    ...
    constructor(string memory _theBaseURI, string memory _notRevealedURI, bytes32 _merkleRoot) ERC721("Blankos", "BLK") PaymentSplitter(_team, _teamShares) {
    ...
    **/
    ...
    function presaleMint(address _account, bytes32[] calldata _proof, uint256 _ammount) external payable  {
        ...
        require(nftsPerWallet[_account] + _ammount <= max_mint_allowed, "You can only get 3 NFT on the Presale");
    ...
    **/
    function saleMint(uint256 _ammount) external payable  {
        ...
        require(nftsPerWallet[msg.sender] + _ammount <= max_mint_allowed, "You can only get 3 NFT on the Presale");

        //The user can only mint max 3 NFTs
        require(_ammount <= max_mint_allowed, "You can't mint more than max_mint_allowed tokens");
