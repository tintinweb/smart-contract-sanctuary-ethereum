...
/*
 9999 max supply
 mint from contract 
 uses jongold.eth's Epochs for some magic 
*/
...
contract Nebulaeth is ERC721, IERC2981, Ownable {
    ...
    uint256 internal constant maxTokens = 9999;
    ...
    function mint() public {
        require(tokenCount < maxTokens, "No more Nebulaeth to discover.");
        // uint256[12] memory currentEpoch = getCurrentEpoch();
        // string[12] memory epochLabels = getEpochLabels();
        metadataJson[tokenCount] = renderer.render(tokenCount);
...
contract Renderer {
    string add = utils.addressToString(msg.sender);
    ...
    string[] private colors = [
        string(abi.encodePacked("ff", utils.getSlice(3, 6, add))),
        utils.getSlice(7, 12, add),
        utils.getSlice(13, 18, add),
        utils.getSlice(19, 24, add),
        utils.getSlice(25, 30, add),
        utils.getSlice(31, 36, add),
        utils.getSlice(37, 42, add)
    ];

    /// ============ get epochs ============

    Epochs public epochs;

    function getEpochLabels() public view returns (string[12] memory) {
        return epochs.getEpochLabels();
    }

    function getCurrentEpoch() public view returns (uint256[12] memory) {
        return epochs.currentEpochs();
    }

    function getEpochs() public view returns (uint256[12] memory) {
        return epochs.getEpochs(block.number);
    }

    uint256[12] currentEpoch = getCurrentEpoch();
    string[12] epochLabels = getEpochLabels();

    function getNebulaeth(uint256 tokenCount, string memory _imageData)
        public
        view
        returns (string memory)
    {
        return
            string.concat(
                string.concat(
                    '{"name": "nebulaeth #',
                    utils.numberToString(tokenCount),
                    '", "background_color": "000000", "image": "data:image/svg+xml;base64,',
                    _imageData,
                    '", "description": "A nebulaeth discovered  in ',
                    epochLabels[6],
                    " ",
                    utils.numberToString(currentEpoch[6]),
                    " ",
                    epochLabels[5],
                    " ",
                    utils.numberToString(currentEpoch[5]),
                    " ",
                    epochLabels[4],
                    " ",
                    utils.numberToString(currentEpoch[4]),
                    " by ",
                    add,
                    ".\\n\\nThe epoch of discovery is the root of the celestial objects clarity, and determines its astrological alignment. Its spectrum derives from the aura of its progenitor's eth.  \\n\\nhttps://epochs.cosmiccomputation.org/"
                ),
                string.concat(
                    string.concat(
                        ...
                        epochLabels[3],
                        '","value": "',
                        utils.numberToString(currentEpoch[3]),
                        '"},'
                    ),
                    string.concat(
                        '{"trait_type": "',
                        epochLabels[2],
                        '","value": "',
                        utils.numberToString(currentEpoch[2]),
                        '"},{"trait_type": "',
                        epochLabels[1],
                        '","value": "',
                        utils.numberToString(currentEpoch[1]),
                        '"},{"trait_type": "',
                        epochLabels[0],
                        '","value": "',
                        utils.numberToString(currentEpoch[0]),
                        '"}]}"'
                    )
                ),
                "}"
    ...
    function render(uint256 tokenCount) public view returns (string memory) {
        return
            string.concat(
                    ...
                    bytes(
                        getNebulaeth(
                            ...
                            string.concat(
                                ...
                                string.concat(
                                    '" seed="',
                                    utils.numberToString(block.number),
...
library utils {
    ...
    function addressToString(address add) internal pure returns (string memory) {
        return Strings.toHexString(uint256(uint160(add)));
    }
    ...
    function bytesToString(bytes memory value)
        internal
        pure
        returns (string memory)
    {
        string memory converted = string(value);
        return converted;
    }
