...
contract HatTest is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function randomHat(uint256 tokenId) internal view returns (hatObject memory) {
    ...
    function random(string memory input) internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(input, msg.sender)));
    }

    function getBaseColor(uint256 tokenId) internal view returns (uint256) {
    ...
    function getLayerThree(uint256 tokenId) internal view returns (uint256) {
        ...
        function getLayerFour(uint256 tokenId) internal view returns (uint256) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        if(tokenId > numTokensMinted){
        return "not yet avalible..";    
        }
