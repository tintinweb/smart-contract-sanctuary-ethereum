...
contract Doadz is Ownable, ERC721Enumerable {
    uint256 public mintPrice = 0.04 ether;
    uint256 public constant mintLimit = 20;
    uint256 public claimMintPrice = 0.01 ether;
    ...
    bool[10001] private claimedBMBs;
    mapping(address => bool) private claimedAddresses;

    constructor(string memory _initBaseURI) ERC721("Doadz by Hamm", "DOADZ") {
    ...
    function setClaimMintPrice(uint256 newClaimMintPrice) external onlyOwner {
        claimMintPrice = newClaimMintPrice;
    }

    function mintDoadz(uint256 numberOfTokens) external payable {
        ...
        _mintDoadz(msg.sender, numberOfTokens);
    }

    function _mintDoadz(address to, uint256 numberOfTokens) private {
        require(getSaleActive(), "Sale is not active");
        require(
    ...
    function claimDoadz() external payable {
        IERC721Enumerable bmb = IERC721Enumerable(address(0x282620f5fBa7609B8293230f07bcD73558E2617A));

        uint256 tokenCount = bmb.balanceOf(msg.sender);
        ...
        for (uint256 i = 0; i < tokenCount; i++) {
            tokenId = bmb.tokenOfOwnerByIndex(msg.sender, i);
            require(
                claimedBMBs[tokenId] == false,
                "Claim prerequisite not satisfied"
            );
            claimedBMBs[tokenId] = true;
        }

        require(
            msg.value >= claimMintPrice * 1,
            "Insufficient payment"
        );

        _mintDoadz(msg.sender, 1);
    }

    function bmbEligibleForClaim(uint256 tokenId) external view returns (bool) {
        return !claimedBMBs[tokenId];
    }
    ...
    function withdraw() external onlyOwner {
        ...
        require(payable(0x78f8445E203D15482EE7A9C4D5cFB05313D10668).send((contractBalance * 100) / 100), "Withdraw failed");
    }
}