...
contract RickHeadTicket is ERC721A, Pausable, Ownable {
    ...
    string private baseURI = "https://nft.dopedudes.xyz/metadata/";
    ...
    function clamTicket(uint64[] memory idsRickHead)
    external
    payable
    whenNotPaused
     {
        for (uint i = 0; i < idsRickHead.length; i++) {
            ...
            require(idsRickHead[i] != 0, "error: 002");
            require(RICKHEAD(rickHead_smartContract).ownerOf(idsRickHead[i]) == msg.sender, "error: 003");
            nftClaimed[idsRickHead[i]] = i;
        }
        _mint(msg.sender, idsRickHead.length - 1);
    }
    ...
    */
    modifier verifyCoupon(Coupon memory coupon, CouponType couponType) {
        if(phase == SalePhase.Phase_Raffle){
            require(couponType == CouponType.PrivateWL, "001");
        }else if(phase == SalePhase.Phase_Public){
            require(couponType == CouponType.PublicWL, "001");
        }
        bytes32 digest = keccak256(
            abi.encode(couponType, msg.sender)
        );
        require(isVerifiedCoupon(digest, coupon), "001");
        _;
    }
