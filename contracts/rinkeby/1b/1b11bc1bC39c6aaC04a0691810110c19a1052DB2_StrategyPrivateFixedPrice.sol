...
contract StrategyPrivateFixedPrice is ISynchronousStrategy {
    ...
    function canExecuteTakerAsk(Orders.TakerOrder calldata, Orders.MakerOrder calldata)
        external
        pure
        override
        returns (
    ...
    {
        (uint256 askPrice, address designatedTaker) = abi.decode(makerAsk.strategyParams, (uint256, address));
        uint256 bidPrice = abi.decode(takerBid.strategyParams, (uint256));
        return (
            (takerBid.taker == designatedTaker &&
                askPrice == bidPrice &&
