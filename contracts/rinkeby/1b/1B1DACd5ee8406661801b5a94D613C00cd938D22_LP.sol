...
contract LP is ERC20Upgradeable, OwnableUpgradeable, ILP {
    uint256 public totalReserve;
    uint256 public lockedLiquidity; // pure reserve
    address public token;
    ICore public core;
    IAzuroBet public azuroBet;
    uint128 public bettingLiquidity; // reserve amounts + bets
    uint128 public reinforcementAbility; // should be 50%
    uint64 public oddsDecimals;
    uint128 public totalFrontRewards;
    uint128 public totalBetsAmount;
    uint128 public frontFee; // in decimals 10^9

    // LP period length 1 week
    uint64 public periodLen;
    /**
     * @dev init timestamp to work by 7 day intervals
     * LP modes: 1 - in/out | 2 - core
     * LP starts from mode 1
     */
    uint64 public initStartDate;

    /*periods before last change*/
    uint64 public savedPeriods;
    ...
    function changeCore(address addr) external override onlyOwner {
        _require(
            address(core) == address(0) || core.getLockedPayout() == 0,
            Errors.NO_LOCKED_PAYMENT
        );
        core = ICore(addr);
    }

    function changeFrontReward(uint128 newFrontFee) external onlyOwner {
        frontFee = newFrontFee;
    }
    ...
    function initialize(
        ...
        frontFee = 0; // 0%
        oracleFee = 10000000; // 1%
        daoFee = 90000000; // 9%
        reinforcementAbility = oddsDecimals / 2; // 50%
        initStartDate = uint64(block.timestamp);
     ...
     */
    function withdrawLiquidity(uint256 amountLP) external override {
        _require(
    ...
    {
        return (ICore(azuroBet.getCoreByTokenId(tokenId)).viewPayout(tokenId));
    }
        ...
        returns (uint256 reward)
    {
        ...
        uint256 toClaim = (totalFrontRewards *
    ...
    function claimReward() external {
        ...
        uint128 toClaim = (totalFrontRewards *
            ((affiliate.amount * oddsDecimals) / totalBetsAmount)) /
            oddsDecimals;
    ...
    function withdrawPayout(uint256 tokenId) external override {
        _require(
            ...
            Errors.ONLY_BET_OWNER
        );
        (bool success, uint128 amount) = ICore(
            azuroBet.getCoreByTokenId(tokenId)
        ).resolvePayout(tokenId);
        _require(success, Errors.NO_WIN_NO_PRIZE);
        bettingLiquidity -= amount;
        TransferHelper.safeTransfer(token, msg.sender, amount);
        emit BetterWin(msg.sender, tokenId, amount);
    }

    function claimDAOReward() external override {
        // transfer rewards to LP's owner - to DAO
        TransferHelper.safeTransfer(token, owner(), totalDAORewards);
        totalDAORewards = 0;
    }

    function sendOracleReward(address _oracle, uint256 _amount)
        external
        override
        onlyCore
    {
        TransferHelper.safeTransfer(token, _oracle, _amount);
    }
    ...
    function addReserve(uint128 initReserve, uint128 profitReserve)
        external
        override
        onlyCore
    {
        if (profitReserve >= initReserve) {
            ...
            // calc front rewards
            uint128 frontRewards = (profit * frontFee) / oddsDecimals;
            totalFrontRewards += frontRewards;

            // calc oracle rewards
            uint256 oracleRewards = (profit * oracleFee) / oddsDecimals;

            // calc DAO rewards
            uint256 daoRewards = (profit * daoFee) / oddsDecimals;
            totalDAORewards += daoRewards;

            totalReserve =
                totalReserve +
                profit -
                (frontRewards + oracleRewards + daoRewards) +
                pendingReward(address(this));
        } else {
            // pool lose
            totalReserve = totalReserve - (initReserve - profitReserve);
        }
        bettingLiquidity = bettingLiquidity - profitReserve;
    ...
    function changePeriod(uint64 newPeriod) public onlyOwner {
        _require(
            initStartDate > 0 &&
        ...
        uint64 newinitStartDate = initStartDate +
            (_getCurPeriod() - savedPeriods + 1) *
            _getPeriodLen();
        uint64 newsavedPeriods = _getCurPeriod();
     ...
     * @dev period length
     */
    function _getPeriodLen() internal view returns (uint64) {
        return periodLen;
    }
     ...
     */
    function _getCurPeriod() internal view returns (uint64) {
        return (
            block.timestamp <= initStartDate
                ? savedPeriods
                : (uint64(block.timestamp) - initStartDate) /
                    _getPeriodLen() +
                    savedPeriods +
                    (savedPeriods > 0 ? 1 : 0)
        );
    ...
    function phase2end() external view override returns (uint256) {
        return
            initStartDate +
    ...
    function getOracleFee() external view override returns (uint256 fee) {
        return oracleFee;
    }

    function getFeeDecimals()
        external
        view
        override
        returns (uint256 feeDecimals)
    {
        return oddsDecimals;
    }
}
