...
contract AdventTokens is ERC20, ERC20Burnable, Ownable {

  mapping(address => bool) controllers;

  uint256 MAX_SUPPLY; 
  
  constructor(uint256 _maxSupply) ERC20("Advent", "ADV") {
    MAX_SUPPLY = _maxSupply;
  }

  function mint(address to, uint256 amount) external {
    require(amount<=MAX_SUPPLY-totalSupply(),"you can't mint more than max supply");
    require(controllers[msg.sender], "Only controllers can mint");
    _mint(to, amount);
  }

  function MaxSupply() view public returns(uint){
    uint supply = MAX_SUPPLY / 1 ether;
    return supply;
  }
...
contract AdventNFT is ERC721A, Ownable {
    ...
    uint256 public WL_Max_Per_Mint; // max per mint in wl
    uint256 price;  //price

    uint256 wl_price; // wl price
    ...
    uint256 _wlPrice,
        uint256 _maxPerMint,
        uint256 _maxwlperwallet,
        string memory _uri,
        address payable _withdrawWallet,
        uint256 max_supply) ERC721A("Advent NFT ", "ADV") {
            price = _startPrice;
            wl_price = _wlPrice;
            Max_Per_Mint = _maxPerMint;
            WL_Max_Per_Mint = _maxwlperwallet;
            baseURI = _uri;
            withdrawWallet = _withdrawWallet;
            MAX_SUPPLY = max_supply;
        }

    function setwithdrawWallet(address payable _withdrawWallet) public virtual onlyOwner {
        withdrawWallet = _withdrawWallet;
    }
    ...
    function setMaxWLMint(uint256 Value) public onlyOwner {
        WL_Max_Per_Mint = Value;
    }
    ...
    function getWlPrice() public view virtual returns (uint256){
        return wl_price;
    }
    ...
    function whitelistMint(uint256 quantity) whenPresaleStarted external payable {
        ...
        require(totalSupply() + quantity <= MAX_SUPPLY, "Not enough tokens left");
        require(reservedAmount > 0, "No reserved tokens for wallet");
        require(quantity <= reservedAmount, string(abi.encodePacked("Can't mint more than reserved presale amount (", reservedAmount.toString(), ")")));
        require(msg.value >= wl_price * quantity, "Insufficient ether to sent");
        presaleList[msg.sender] = reservedAmount - quantity;
        require(quantity <= WL_Max_Per_Mint, "Maximum mint per transaction limit reached");
        _safeMint(msg.sender, quantity);
    }
    ...
    function SaleStarted() external onlyOwner {
        require(withdrawWallet != address(0), "withdrawWallet not set");
...
contract AdventStaking is Ownable, IERC721Receiver {

  uint256 public totalStaked;
  address Myaddress;
  mapping(uint256 => bool) public LegendaryId;
  mapping(uint256 => bool) public RareId;
  uint256 public Original = 25;
  uint256 public Rare = 50;
  uint256 public Legendary = 100;
  ...
  mapping(uint256 => Stake) public vault; 
   ...
   constructor(AdventNFT _nft, AdventTokens _token) { 
    nft = _nft;
    token = _token;
  }

  function SetOrginalRate(uint256 Value) public onlyOwner {
        Original = Value;
    }

  function SetRareRate(uint256 Value) public onlyOwner {
        Rare = Value;
    }

  function SetLegendaryRate(uint256 Value) public onlyOwner {
        Legendary = Value;
    }
  ...
  function addLegendaryIds(uint256[] memory tokenIds, bool a) public onlyOwner {
    for (uint256 i; i < tokenIds.length; i++) {
      LegendaryId[tokenIds[i]] = a;
      }
    }

    function addRareIds(uint256[] memory tokenIds, bool a) public onlyOwner {
    for (uint256 i; i < tokenIds.length; i++) {
      RareId[tokenIds[i]] = a;
      }
    }
  ...
  function setMyAddress(address payable _myAddress) public virtual onlyOwner {
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    if (earned > 0) {
      earned = earned / 10;
      if (LegendaryId[tokenId]){
      reward = earned*Legendary;
      uint256 totalTokens = token.balanceOf(Myaddress);
      require(totalTokens>=reward,"Not Enogh Balance to transfer");
      token.approve(Myaddress,reward);
      token.transferFrom(Myaddress, account, reward);
    } else if(RareId[tokenId]){
      reward = earned*Rare;
      uint256 totalTokens = token.balanceOf(Myaddress);
      require(totalTokens>=reward,"Not Enogh Balance to transfer");
      token.approve(Myaddress,reward);
