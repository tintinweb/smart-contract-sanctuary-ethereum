...
contract cellNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    function _breed_eukaryotes(uint256 tokenId1, uint256 tokenId2) external payable {
        ...
        (uint256 dead_cell, uint256 new_type) = SVGManip.breedEukaryotes(tokenId1, tokenId2);
...
contract svgManip {
    ...
    function _get_num_octaves(uint64 eukaryote, uint64 generation) internal pure returns (string memory) {
        // Make old prokaryotes increasingly wavy
        return (eukaryote == 1) ? "1" : Strings.toString(1 + (generation / 5));
    }
    ...
    function _add_filters(string memory tokenId, bytes memory output, uint64 eukaryote, uint64 _type, uint64 generation, bool blur) view internal returns (bytes memory) {
        ...
        output = abi.encodePacked(output, '\n<filter id="', _get_blur_data(blur)[0], '">\n<feTurbulence result="turbulence" numOctaves="', _get_num_octaves(eukaryote, generation), '" type="turbulence" baseFrequency="0">');
        if (_type == 3) {
            output = abi.encodePacked(
                output, 
    ...
    function breedProkaryote(uint256 tokenId, uint256 cell_generation) external view returns (uint256) {
        ...
        uint256 rand = _random(abi.encodePacked(Strings.toString(cell_generation), "BREED CELLS", _seed_phrase, Strings.toString(tokenId))) % 100;
    ...
    function breedEukaryotes(uint256 tokenId1, uint256 tokenId2) external view returns (uint256, uint256) {
        ...
        if (rand < 1) { // death - rand == 0, 10% chance
            rand = _random(abi.encodePacked("CELL DAEATH", _seed_phrase, Strings.toString(tokenId1),Strings.toString(tokenId2))) % 2;
            if (rand == 0) {
                // cell 1 dies
                return (1, 0);
            } else {
