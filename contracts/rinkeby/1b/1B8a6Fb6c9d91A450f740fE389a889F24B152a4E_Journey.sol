...
contract Journey is ERC721Holder, Ownable, ReentrancyGuard, Pausable {
    ...
    /**
     * @notice Sets Lords Issuance in gwei per staked realm
     */
    ...
    function setHalvingAmount(uint256 _halvingAmount) external onlyOwner {
        halvingAmount = _halvingAmount * 10**18;
    }
        ...
        nonReentrant
    {
        ...
        realmsStaked[msg.sender][0] += uint256(_tokenIds.length);
    ...
    function _epochNum() internal view returns (uint256) {
        if (finalAge != 0) {
            ...
            return (block.timestamp - genesis) / (epoch * 3600) + epoch; // hours
        }
        // return 10;
    }
        ...
        returns (uint256 lords, uint256 totalRealms)
    {
        ...
        for (uint256 i = epochClaimed[_player]; i < _epochNum(); i++) {
            ...
            if (_epochNum() <= halvingAge) {
                preHalvingRealms +=
                    realmsStaked[_player][i] *
                    ((_epochNum() - 1) - i);
            } else {
                preHalvingRealms += realmsStaked[_player][i] * halvingAge;
            }
        }

        if (_epochNum() > halvingAge) {
            for (
                uint256 i = epochClaimed[_player];
                i < _epochNum() - halvingAge;
                i++
            ) {
                postHalvingRealms +=
                    realmsStaked[_player][i] *
                    ((_epochNum() - halvingAge) - i);
            }
        }
