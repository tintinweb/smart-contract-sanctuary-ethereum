...
interface IWineToken {
    ...
    function registerWine(uint256 tokenId_) external;
...
// 
//import "./IWineEnums.sol";
contract WineBrand is ERC1155Holder, Ownable {
    ...
    // Token ID => Wine Data
    mapping(uint256 => string) private _wineData;

    // Registered cellars
    mapping(address => bool) private _cellars;

    event TOKENID(uint256 tokenId_);
    ...
    function registerWine(string memory wine_) external onlyOwner {
        uint256 tokenId = IWineToken(_wineToken).getCurrentTokenId();
        IWineToken(_wineToken).registerWine(tokenId);
        _wineData[tokenId] = wine_;
        emit TOKENID(tokenId);
    }

    function updateWine(string memory wine_, uint256 tokenId_) external onlyOwner {
        require(bytes(_wineData[tokenId_]).length > 0, "WineBrand: this wine is not registered");

        _wineData[tokenId_] = wine_;
    }

    function getWine(uint256 tokenId_) external returns (string memory){
        return _wineData[tokenId_];
    }

    function unregisterWine(uint256 tokenId_) external onlyOwner {
        if (bytes(_wineData[tokenId_]).length > 0) {
    ...
    function mint(address cellar_, uint256 tokenId_, uint256 amount_, bytes memory data_) public onlyOwner {
        require(bytes(_wineData[tokenId_]).length > 0, "WineBrand: this token must be registered by this brand");
