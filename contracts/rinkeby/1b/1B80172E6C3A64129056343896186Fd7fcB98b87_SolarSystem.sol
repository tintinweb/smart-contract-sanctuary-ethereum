...
contract SolarSystem is ERC721Tradable {
    ...
    function decompress() public view returns (string memory) {
        //SPLIT EVERY REPLACE
        ...
        for(uint i = 0; i < count_delim;) {
            ...
            //stringToUint(count.toString())
            for (uint it=0; it < 3;) {
                HTML = findReplace(find.toString(),replace.toString(),HTML);
                unchecked { it++; }
            }
            unchecked { i++; }
        }
