...
contract HappyRobotSale is Ownable {
    ...
    uint8 private maxWLTokenPerWallet = 3;
    ...
    */
    function unlockFloor(uint256 _id) public {
        ...
        require(availableNeurons >= reqNeurons, "Happy Robot NFT: Your neurons are not enough to unlock next level");
    ...
    * get whitelist
    */
    function getWhitelist() external view onlyMaster returns (address[] memory) {
        return whitelist;
    }
    ...
    */
    function getWLTokenMinters() external view onlyMaster returns (address[] memory) {
    ...
    */
    function canMintWLTokenForAccount(address _account, uint16 _quantity) internal view returns (bool) {
        ...
        unchecked {
            return HappyRobotWhitelistToken(wlTokenAddress).balanceOf(_account) + _quantity <= maxWLTokenPerWallet;
        }
    }
    ...
    */
    function exchangeNFTWithWhitelistToken(uint16 _quantity) external onlySaleStatus {
        ...
        // trigger nft token minted event
        emit ExchangedNFT(msg.sender, _quantity, nft.totalSupply(), whitelistToken.totalSupply());
    }
    ...
    */
    function mintWLToken(uint16 _quantity) external payable onlySaleStatus {
        ...
        emit MintedWLToken(msg.sender, _quantity, token.totalSupply());
    }
    ...
    */
    function reset() external onlyTesting {
        HappyRobotWhitelistToken wlToken = HappyRobotWhitelistToken(wlTokenAddress);
        uint256 wlTokenQty = wlToken.balanceOf(msg.sender);

        HappyRobotNFT nft = HappyRobotNFT(nftAddress);
        uint256 nftQty = nft.balanceOf(msg.sender);

        uint256 txFees = 0;
        unchecked {
            txFees = wlTokenQty * wlTokenMintFee + nftQty * nftMintFee;
        }

        if (txFees == 0)    return;

        // burn wl tokens
        wlToken.burn(msg.sender, uint16(wlTokenQty));

        // burn nft tokens
        uint256[] memory nftIds = nft.getTokenIdsOfOwner(msg.sender);
        for (uint16 i = 0; i < nftIds.length; i++) {
            nft.safeTransferFrom(msg.sender, address(0), nftIds[i], "");
        }
...
contract HappyRobotNFT is ERC721A, Ownable {
  ...
  uint8 constant START_TIME_ERR_LIMIT = 60;
  ...
  */
  function passedFloor(uint256 _id, uint16 _passTime) external {
    ...
    if (!isScammer) {
      ...
      if (START_TIME_ERR_LIMIT < diffTime)
        isScammer = true;
    }
