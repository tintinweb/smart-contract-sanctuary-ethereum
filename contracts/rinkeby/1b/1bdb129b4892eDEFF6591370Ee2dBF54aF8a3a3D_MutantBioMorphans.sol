...
contract MutantBioMorphans is ERC721Enumerable, Ownable, Pausable, ReentrancyGuard, IERC2981 {
  ...
  uint8 public constant RESERVED_GIVEAWAYS = 1; // Tokens reserved for giveaway list
  uint16 public constant MAX_TOKENS = 9; // Max number of token sold in sale
  ...
  uint256 public constant SALE_PRICE = 650000000000000; // 0.065 ETH
  ...
  constructor(
    ...
    uint32 _privateSaleEndsAt,
    uint32 _publicSaleStartsAt,
  ...
  function mintNFT(address recipient, uint8 numTokens) external onlyOwner {
    require(block.timestamp >= uint256(publicsaleStartsAt), 'Sale not active');
  ...
  function mintGiveawayNFT(bytes32[] memory proof, uint8 numTokens) external {
    require(block.timestamp >= uint256(saleStartsAt), 'Sale not active');
  ...
  function mintPrivateSale(uint8 numTokens, bytes32[] memory proof) external payable {
    ...
    require(saleActive && block.timestamp >= uint256(saleStartsAt), 'Sale not active');
    uint256 time = (block.timestamp);
    require(time > uint256(privatesaleStartsAt) && time < uint256(privatesaleEndsAt), 'Private sale over');
  ...
  function mintPublicSale(uint8 numTokens) external payable {
    ...
    require(saleActive && block.timestamp >= uint256(publicsaleStartsAt), 'Sale not active');
