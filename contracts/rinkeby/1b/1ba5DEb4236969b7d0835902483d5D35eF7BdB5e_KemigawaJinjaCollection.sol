...
contract KemigawaJinjaCollection is ERC721A, Ownable, Pausable {
    ...
    string public baseURI = 'https://kemigawa.nft-kojiki-project.com/json/';
    ...
    constructor() ERC721A("KemigawaJinjaCollection", "KJC") {
        uint64 forOwnerCountPerAmuletType = 30;

        // 300件(10種類×30ずつ)を運営保有分として事前Mint
        // 配列の宣言と追加がこれであっているか不安
        Claim[] memory claims = new Claim[](amuletTypeNum);
        for (uint64 amuletType = 1; amuletType <= amuletTypeNum; amuletType++) {
            claims[amuletType - 1] = Claim(amuletType, forOwnerCountPerAmuletType);
        }
    ...
    // Mint共通処理
    // お金の支払が不要なMintメイン処理
    function mintCommon(Claim[] memory claims) private
        correctAmuletTypes(claims)
    {
        uint64 totalClaimCount = getTotalClaimCount(claims);

        // Mint処理
        _safeMint(msg.sender, totalClaimCount);

        // TokenIdに対するお守りタイプを格納
        uint64 sum = 0;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (block.timestamp > mintTimestamp + blessingTimeSec) {
            // 有効期限切れだったらExpiredを参照
            return string(abi.encodePacked(baseURI, _toString(amuletType), '_expired', baseExtension));
        } else {
            // 有効期限内だったら通常のメタデータを参照
            return string(abi.encodePacked(baseURI, _toString(amuletType), baseExtension));
        }
    }
