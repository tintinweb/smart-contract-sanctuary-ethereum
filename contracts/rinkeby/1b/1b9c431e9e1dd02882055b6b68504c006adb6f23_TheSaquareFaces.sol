...
contract TheSaquareFaces is ERC721A, Ownable {
    ...
    bool public Sale_Is_Enabled;
    uint256 public Price;
    string public metadataBaseURL;
    uint256 public Max_Mint_TX = 10;
    uint256 public constant Max_Token_Supply = 3210;
    mapping(uint256 => string) private thoughMemory;

    constructor(string memory baseURL) ERC721A ("TheSquareFaces", "TSF", 50) 
        { 
            Sale_Is_Enabled = false;
            Price = 0.001 ether;
            metadataBaseURL = baseURL;
        }


    function thoughtUpdate(string memory _newThought, uint256 _tokenID) external{
    thoughMemory[_tokenID] = _newThought;
    }

    function thoughtReader(uint256 _tokenID) external view returns(string memory){
        return thoughMemory[_tokenID];
    }
    ...
    function toggleSaleStatus() external onlyOwner {
        Sale_Is_Enabled = !Sale_Is_Enabled;
    }
/*
...
*/
    function airdrop(uint256 num, address addr_) external onlyOwner {
        require((totalSupply() + num) <= Max_Token_Supply, "Exceed max supply");
        _safeMint(addr_, num);
    }

    function mint(uint256 numOfTokens) external payable {
        require(Sale_Is_Enabled, "Sale must be active.");
        require(totalSupply() + numOfTokens <= Max_Token_Supply, "Exceed max supply");
        require(numOfTokens > 0, "Must mint at least 1 token");
        require(numOfTokens <= Max_Mint_TX, "Cant mint more than 10");
