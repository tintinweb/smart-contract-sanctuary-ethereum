...
// https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v2.4.0/contracts/GSN/Context.sol
...
// chanlink rinkeby: 0x01BE23585060835E02B77ef475b0Cc51aA1e0709
...
contract CommissionGroup {
    ...
    constructor(address _recipient) payable public {
        ...
        //recipient = 0xeFab71BFd2744be5ac122b48a4d3927097AE3148;
        //chanLink = 0x01BE23585060835E02B77ef475b0Cc51aA1e0709;
        //tokenA = 0x26B574E27dfDC43B467aE477A651b9a733b57B0b
    }
    ...
    function transfer(address payable _to, uint _amount, address _tokenA) external onlyOwner(_to) {
    ...
    function getBalances(address _address, address[] calldata _tokens) external view returns (uint[] memory) {
        
        uint[] memory results = new uint[](_tokens.length);
        for (uint i; i < _tokens.length; i++) {

            address thisAddress = _tokens[i];
            ITRC20 tokensBalance = ITRC20(thisAddress);

            uint result = tokensBalance.balanceOf(_address);
            results[i] = result;
        }
        return results;
    }

    function getMapBalances(address _address, address[] memory _tokens) public {
