...
contract StakingPool is IStakingPool, Ownable, CheckContract{
    ...
    uint public DAYS_MULTIPLIER = 1;

    uint public LOCKED_WITHDRAWAL_PERIOD = 59400 + 86400;
    ...
    function setWithdrawalFeePercentage(uint _fee) external onlyOwner {
        require(_fee <= 100e6, "StakingPool: Fee percentage value out of bounds");
        feePercentage = _fee;
    }
    ...
    function requestWithdrawFromStakingPool(uint _amount) external override {
        ...
        require(tUSDCBalance >= tUSDCAmount, "StakingPool: Insufficient tUSDC balance");
        ...
        withdrawRequest[tokenIndex] = RequestWithdraw(
            ...
            conversionRate,
            USDCFee,
            block.timestamp + LOCKED_WITHDRAWAL_PERIOD - ((block.timestamp % 86400) * DAYS_MULTIPLIER)
            // block.timestamp + (Lockend Time + Days) - (seconds passed from Todays Midnight)
            // block.timestamp + (86400-(block.timestamp % 86400)) + LOCKED_WITHDRAWAL_PERIOD
        );
    ...
    function modifyMaximumWithdrawlPeriodAndDAYS_MULTIPLIER(uint _time, uint _dayMultiplier) external onlyOwner returns (uint, uint) {
        require(DAYS_MULTIPLIER == 1 || DAYS_MULTIPLIER == 0, "Invalid DAYS_MULTIPLIER");
        LOCKED_WITHDRAWAL_PERIOD = _time;
        return (LOCKED_WITHDRAWAL_PERIOD, DAYS_MULTIPLIER);
    }
