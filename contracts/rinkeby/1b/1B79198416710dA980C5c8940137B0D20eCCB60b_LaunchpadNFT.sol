...
// File: contracts/binance/IBooster.sol

pragma solidity ^0.8.3;

abstract contract IBooster  is ERC721BurnableUpgradeable{
    struct BasicAttr {
        uint8 typeId;
        uint8 level;
        string artUri;
    ...
    function batchMintCount(Attr calldata _attr, uint256 count) external virtual;
    ...
    function use(address account,uint256 _id) external virtual;

    function useLevel(Attr memory _attr,address account) external virtual returns (bool result);

    function viewAddressTokens(Attr calldata _attr,address account) public view virtual returns (uint256[] memory);

    function attrs(uint256 token_id) public view virtual returns (Attr memory _attr);

    function batchTransfer(Attr calldata _attr, uint256[] calldata count,address[] calldata to) external virtual;
    
    function batchTransferFrom(Attr calldata _attr, uint256 count,address from,address to) external virtual;

    // function getBasicAttrs(Attr calldata _attr) external view virtual returns (BasicAttr calldata result);
}
...
// File: contracts/binance/LaunchpadNFT.sol
...
// this a demon NFT
contract LaunchpadNFT is Ownable {
    ...
    IBooster public booster = IBooster(0x851C8341ddB5D4a2AAdcf97F2A27Fa4aAa2D8423);
    ...
    constructor(string memory baseURI_, string memory suffix_,address launchpad, uint256 maxSupply,uint8 _level)  {
        baseURI = baseURI_;
        suffix = suffix_;
        LAUNCHPAD = launchpad;
    ...
    function tokenURI(uint256 tokenId) public view virtual  returns (string memory)  {
        uint8 level = booster.attrs(tokenId).level;
        return string(abi.encodePacked(baseURI, level, suffix));
    }
    ...
    function mintTo(address to, uint size) external onlyLaunchpad {
        ...
        IBooster.Attr memory _attr;
        _attr.typeId = 0;
        _attr.level = LEVEL;

        
        LAUNCH_SUPPLY += size;
        booster.batchMintTo(_attr,size,to);
    }
}