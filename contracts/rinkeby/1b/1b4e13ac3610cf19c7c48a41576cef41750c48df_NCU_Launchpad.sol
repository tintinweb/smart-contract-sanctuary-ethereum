...
//     event pre(string name,string symbol,string baseUri,uint256 maxSupply,uint256 presaleSupply,uint256 maxPertrans,uint256 reserve,uint256 price,uint256 presaleprice,uint256 maxperWallet,uint256 tax,address token);
//     event pub(string name,string symbol,string baseuri,uint256 maxSupply,uint256 maxperTrans,uint256 reserve,uint256 price,uint256 tax,address token);
...
//     mapping(address => mapping (string  => otherDetails  ))            public rankingUser_data;
    ...
    constructor(){
        ...
        TOKEN(token).initialize(msg.sender);
        ...
        // // emit TokenDeployed(token);
        // counter++;
        // emit pub(_name,_symbol,_baseuri,_maxsupply,_maxPerTrans,_reserve,_price,msg.value,token);
        }
...
//         // emit TokenDeployed(token);
//         counter++;
//         emit  pre(_name,_symbol,_baseuri,_maxsupply,_preSaleSupply,_maxPerTrans,_reserve,_price,_presalePrice,_maxPerWallet,msg.value,token);
