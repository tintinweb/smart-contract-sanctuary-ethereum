...
contract Verify {
    ...
    constructor() {
        token = IERC20(0x527Dc603ECB9108A2c16c807b83c0e3De7e835Be);
...
contract RadioactiveWaste is ERC20, Ownable {
    ...
    function burn(address user, uint256 amount) external {
        require(
            allowedAddresses[msg.sender] || msg.sender == address(KaijuKingz),
    ...
    function getPendingReward(address user) internal view returns (uint256) {
        return
            (KaijuKingz.balanceGenesis(user) *
                BASE_RATE *
                (block.timestamp -
