...
contract GoBlockchain is ERC1155, Ownable, Pausable, ReentrancyGuard, ERC1155Supply {
    ...
    function buyToken(uint256 item, uint256 amount) public payable whenNotPaused nonReentrant returns (SellOffer memory) {
        ...
        uint256 seller_ammount;
        if(address(offer.seller) != owner()){
            uint256 fee_tax = (msg.value / 100) * FEE_PERCENT;
            seller_ammount = msg.value - fee_tax;
        } else {
            seller_ammount = msg.value;
        }
        this.safeTransferFrom(address(this), address(msg.sender), offer.IDtoken, amount, "0x");
        ...
        uint256 rest = offer.amount - amount;
        if(rest > 0){
