...
contract Kilzuki is ERC721A, Ownable, ReentrancyGuard {
    ...
    function freeMint(uint256 _mintAmount) public mintCompliance(_mintAmount) {
        require(!paused && isFreeMintOpen, "Kilzuki: Free mint phase not open yet!");

        require(totalSupply() + _mintAmount <= maxFreeMintSupply, "Kilzuki: Exceeds max free mint supply!");
        require(_numberMinted(msg.sender) + _mintAmount <= maxFreeMintAmountPerAddr, "Kilzuki: Exceeds max free mint per wallet!");
        _safeMint(_msgSender(), _mintAmount);
    }

    function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
        require(_numberMinted(msg.sender) + _mintAmount <= 20, "Kilzuki: Exceeds max mint per wallet!");
