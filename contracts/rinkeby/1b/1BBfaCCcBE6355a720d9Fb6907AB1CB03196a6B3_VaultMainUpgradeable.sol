...
contract RouterUpgradeable is Initializable, OnlyAddress {
    ...
    function setDeployer(address deployer_) external only(superAdmin) {
        deployer = deployer_;
    }
    ...
    function deployVaults(
        ...
        address baseToken,
        address quoteToken
    ) external only(deployer) {
        uint256 length = _storage.getProxyRoutersLength();
        _storage.setBaseToken(profileId, baseToken);
        _storage.setQuoteToken(profileId, quoteToken);

        for (uint256 i; i < length; i++) {
            address vault = factory.deployProxy2(
                _storage.getProxyRouters(i),
                abi.encodeWithSignature(
                    'init(address,address)',
                    address(this),
                    superAdmin
                ),
                keccak256(abi.encode(profileId))
            );
            _storage.setIsProfileVault(profileId, vault, true);
            _storage.setProfile(vault, profileId);
        }
    }

    function setProxyRouters(address[] memory proxyRouters) external only(superAdmin) {
        _storage.setProxyRouters(proxyRouters);
    }
...
import {IProxyRouter} from './ProxyRouter.sol';
...
contract ProxyRouter is IProxyRouter, Ownable {
    ...
    function updateLogic(address logic_) external onlyOwner {
...
contract HasRouterUpgradeable is HasRouter(address(0), address(0)), Initializable {
...
contract VaultMainUpgradeable is VaultUpgradeable {
    function init(address router_, address superAdmin_) external initializer {
        VAULT_NAME = keccak256('MAIN_VAULT');
        __Vault_init(router_, superAdmin_);
    }
}
...
abstract contract VaultUpgradeable is HasRouterUpgradeable, SafeContract {
    ...
    function __Vault_init(address router_, address superAdmin_)
        internal
        onlyInitializing
    {
        __HasRouter_init(router_, superAdmin_);
    ...
    function transferBaseToken(address to, uint256 amount) external onlyRouter {
        IStorage storage_ = getStorage();
        uint256 profileId = storage_.getProfile(address(this));
        storage_.getQuoteToken(profileId);

        require(address(this) != to, 'Same address');
        require(storage_.isProfileVault(profileId, to), 'Target address is not vault');
        IERC20(storage_.getBaseToken(profileId)).safeTransfer(to, amount);
    }

    function transferQuoteToken(address to, uint256 amount) external onlyRouter {
        IStorage storage_ = getStorage();
        uint256 profileId = storage_.getProfile(address(this));

        require(address(this) != to, 'Same address');
        require(storage_.isProfileVault(profileId, to), 'Target address is not vault');
        IERC20(storage_.getQuoteToken(profileId)).safeTransfer(to, amount);
    }
