...
contract LoanServiceOffer is Ownable {
    ...
    function offers(uint256 _offerId) public view returns(LoanCommon.Offer memory) {
        address loanStorageOfferAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageOfferAddress();
        return LoanStorageOffer_I(loanStorageOfferAddress).offers(_offerId);
    }
    ...
    function updateAcceptedAmount(uint256 _offerId, uint256 _acceptedAmount) public onlyServiceContractsOrOwner {
        address loanStorageOfferAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageOfferAddress();
        LoanStorageOffer_I(loanStorageOfferAddress).updateAcceptedAmount(_offerId, _acceptedAmount);
    }

    /// @notice Update the loan status
    /// @param _offerId The Offer ID
    /// @param _status The new status value
    function updateStatus(uint256 _offerId, LoanCommon.OfferStatus _status) public onlyServiceContractsOrOwner {
        address loanStorageOfferAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageOfferAddress();
        LoanStorageOffer_I(loanStorageOfferAddress).updateStatus(_offerId, _status);
    }
    ...
    function offersByLoanId(uint256 _loanId) public view returns(uint256[] memory) {
        address loanStorageOfferAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageOfferAddress();
        return LoanStorageOffer_I(loanStorageOfferAddress).offersByLoanId(_loanId);
    }
    ...
    function create(uint256 _loanId, uint256 _offeredAmount, uint256 _interest) public payable {
        address loanServiceAddress = LoanOracle_I(getLoanOracleAddress()).getLoanServiceAddress();
        LoanCommon.Loan memory loan = LoanService_I(loanServiceAddress).loans(_loanId);
        require(loan.loanId != 0, "loan must exists");
        require (loan.status == LoanCommon.LoanStatus.RECRUITING, "loan must be in recruiting phase");

        // Make payable value should update treasury
        if (msg.value > 0) {
            ...
            LoanServiceTreasury_I(loanServiceTreasuryAddress).addFunds(msg.sender, msg.value);
        }

        address loanStorageOfferAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageOfferAddress();
        LoanStorageOffer_I(loanStorageOfferAddress).create(msg.sender, _loanId, _offeredAmount, _interest);
    }

    /// @notice convenient method to get a list of offers
    /// @param _start The starting index
    /// @param _end The ending index
    function list(uint256 _start, uint256 _end) public view returns(LoanCommon.Offer[] memory) {
        require(_end >= _start, "The end index must be greater than or equal to the start index");
        LoanCommon.Offer[] memory offerList = new LoanCommon.Offer[](_end.sub(_start).add(1));
    ...
    function listByLoanId(uint256 _loanId) public view returns(LoanCommon.Offer[] memory) {
        uint256[] memory offerIds = offersByLoanId(_loanId);
        LoanCommon.Offer[] memory offerList = new LoanCommon.Offer[](offerIds.length);

        for (uint256 i = 0; i < offerIds.length; i++) {
            offerList[i] = offers(offerIds[i]);
        }
    ...
    function updateOffer(uint256 _offerId, uint256 _offeredAmount, uint256 _interest) public {
        address loanServiceAddress = LoanOracle_I(getLoanOracleAddress()).getLoanServiceAddress();
        address loanStorageOfferAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageOfferAddress();

        LoanCommon.Offer memory offer = offers(_offerId);
        LoanCommon.Loan memory loan = LoanService_I(loanServiceAddress).loans(offer.loanId);

        require(loan.status == LoanCommon.LoanStatus.RECRUITING, "loan must be in a recruiting status");
        require(msg.sender == offer.lender, "sender must be lender of the offer");

        LoanStorageOffer_I(loanStorageOfferAddress).updateOffer(_offerId, _offeredAmount, _interest);
    }
}
