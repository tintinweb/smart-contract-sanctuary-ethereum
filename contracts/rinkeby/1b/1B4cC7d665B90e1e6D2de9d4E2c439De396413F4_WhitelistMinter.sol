 ...
 * @author ehrab
 * @notice Contract is called by GenesisClaim.sol with AccessControl
 */
contract WhitelistMinter is ERC721, ERC721URIStorage, ERC721Enumerable, ERC721Burnable, ERC721Royalty, Pausable, AccessControl {
	// ========== DEPENDENCIES ========== //
	...
	) ERC721('Cannafox.io', 'aCFOX') {
		_grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
		_grantRole(PAUSER_ROLE, msg.sender);
		_grantRole(MINTER_ROLE, msg.sender);
		_grantRole(BURNER_ROLE, msg.sender);
		_setDefaultRoyalty(_royaltyWallet, 5);
	}
	...
	*/
	function setMinterRole(address _address) public onlyRole(DEFAULT_ADMIN_ROLE) returns (address) {
		_grantRole(MINTER_ROLE, _address);
		return _address;
	}

	function setPauserRole(address _address) public onlyRole(DEFAULT_ADMIN_ROLE) returns (address) {
		_grantRole(PAUSER_ROLE, _address);
		return _address;
	}

	function setBurnerRole(address _address) public onlyRole(DEFAULT_ADMIN_ROLE) returns (address) {
		_grantRole(BURNER_ROLE, _address);
		return _address;
	}

	/**
	 * @notice Call internal function to burn alpha NFT's
