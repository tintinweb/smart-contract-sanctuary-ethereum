...
contract NFTTransferProxy is INftTransferProxy, Initializable, Operator {
    address private ERC721_CONTROLLER;
    address private ERC1155_CONTROLLER;
    ...
    function mintAndTransfer(LibAsset.Asset memory asset, address to)
        external
        override
        onlyOperator
    {
        if (asset.assetType.assetClass == bytes4(keccak256("ERC721_LAZY"))) {
            ...
            IERC721LazyMint(ERC721_CONTROLLER).mintAndTransfer(token, data, to);
        } else if (
            ...
            IERC1155LazyMint(ERC1155_CONTROLLER).mintAndTransfer(
                token,
                data,
                to,
                asset.value
    ...
    function setAddressERC721Controller(address _newAddress) external {
        ERC721_CONTROLLER = _newAddress;
    }

    function setAddressERC1155Controller(address _newAddress) external {
        ERC1155_CONTROLLER = _newAddress;
    }
}
