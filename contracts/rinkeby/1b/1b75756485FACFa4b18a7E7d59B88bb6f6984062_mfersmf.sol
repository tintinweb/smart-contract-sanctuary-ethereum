...
contract mfersmf is ERC721A, Ownable, ReentrancyGuard {
  ...
  // ERC721 params
  string private _name = "mfersmf";
  string private _symbol = "mfersmf";
  string private _baseTokenURI = "";

  // Collection params
  uint256 public TOTAL_SUPPLY = 8888;
  uint256 public public_supply = TOTAL_SUPPLY;
  uint256 public presale_minted = 0;
  uint256 private price = 0.0420 ether;
  uint256[3] private MAX_PER_STATUS = [0, 1, 10];

  // mfers
  ERC721 public mfers = ERC721(0x79FCDEF22feeD20eDDacbB2587640e45491b757f);
  ...
  // Constructor
  constructor()
    ERC721A(_name, _symbol, MAX_PER_STATUS[2], TOTAL_SUPPLY)
  {
  ...
  function publicMint(uint256 _qty)
    ...
    qtyValidation(_qty)
  {
    if(!isMfersHolder(msg.sender) || (isMfersHolder(msg.sender) &&  _amountMinted[msg.sender][uint256(status)] > 0)){
  ...
  // Getters
  function isMfersHolder(address _address) public view returns (bool isHolder){
    return mfers.balanceOf(_address) > 0;
  }
    ...
    )
  {
    ...
    if(isMfersHolder(msg.sender) &&  _amountMinted[msg.sender][uint256(status)] == 0 ) {
      _price = 0;
      maxStatus = 1;
    }
    ...
    if (status == MintStatus.PUBLIC) {
      return ("PUBLIC", maxStatus, _price, public_supply);
  ...
  function setMfers(address _address) external onlyOwner {
    mfers = ERC721(_address);
  }
