...
contract CryptoSpace{
    ...
    mapping(uint => uint) numOfFrzdRef;
    mapping(uint => uint) numOfFrzdRefSave;
    ...
    function buyNewLvl(uint lvl) public payable levelIsOpen(lvl){
        if (!lvlIsAv[lvl]){
            _openLevel(lvl);
        }
     ...
     function buyNewLvlWI(uint lvl) external payable levelIsOpen(lvl){
    ...
    function countFrzdRefRew(uint lvl) external returns(uint NumOfRefRew){
        require(block.timestamp >= refRewardsSendTime[lvl] && refRewardsSendTime[lvl]>0, "not available"); 
        if (numOfFrzdRefSave[lvl]==0){
            for (uint i=1; i<=nOfPA[lvl]; i++){ 
                ...
                while (j<=7){
                    ...
                    if (users[_ref].lvlWasAct[lvl]){
                        ...
                        if (wRR[lvl][_ref]==0){
                            numOfFrzdRef[lvl]++;
                            wRRId[lvl][numOfFrzdRef[lvl]]+=users[_ref].id;
                        }
    ...
    function availableLevels() external onlyOwner view returns(bool[16] memory){
        return(lvlIsAv);
    }

    function getlevelPrices() external onlyOwner view returns(uint[16] memory levelPrices){
    ...
    function userSiteDataes(address user) external onlyOwner view returns(uint extraBR, uint[16] memory lostMoney, bool[16] memory levelWasActivated, bool[16] memory levelIsActive, 
    ...
    function userSiteDatesTwo(address user) external onlyOwner view returns(uint[16] memory freezedReferralReward){
    ...
    function levelStartTimeDates() external onlyOwner view returns(uint[16] memory levelsStartTime){
    ...
    function paymentProgressDates(address user) external onlyOwner view returns(uint[16] memory userAbsolutePlace, uint[16] memory reinvest){
    ...
    function userAtLevelDates() external onlyOwner view returns(uint[16] memory usersAtTheLevel) {
