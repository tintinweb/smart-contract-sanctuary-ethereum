...
contract NFT is ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable, ReentrancyGuard {
    ...
    // Private //
    /////////////
    uint256 private maxTokenId = 7000;
    ...
    string public networkProvenance = "";
    string public notRevealedUri;

    uint256 public maxSupply;
    uint256 public maxPurchasePerWallet;

    uint256 public presaleBasePrice;
    uint256 public publicsaleBasePrice;
    ...
    modifier mintActive(uint amount, uint offset) {
        uint256 time = block.timestamp;
        uint256 startTime = publicsaleStartTime;
        require(startTime != 0, "Sale start time is not setup!");
        require(startTime + offset <= time && time <= startTime + 2 weeks, "Sale is not active!");
        _;
    }
    ...
    constructor(
        ...
        uint256 _presaleBasePrice,
        uint256 _publicsaleBasePrice
    ) ERC721(name, symbol) ReentrancyGuard() {
        maxSupply = _maxSupply;
        presaleBasePrice = _presaleBasePrice;
        publicsaleBasePrice = _publicsaleBasePrice;
    ...
    function preSaleMint(uint256 _amount) external payable nonReentrant {
        ...
        require(isBlacklisted[msg.sender], "NFT: Sender is not whitelisted");
        mint(_amount, true);
    }
    ...
    function mint(uint256 _amount, bool _state) internal {
        ...
        require(balanceOf(msg.sender).add(_amount) <= maxPurchasePerWallet, "NFT: You can't mint so much tokens");
        if (_state) {
            require(presaleBasePrice.mul(_amount) <= msg.value, "NFT: Ether value sent for presale mint is not correct");
        } else {
            require(publicsaleBasePrice.mul(_amount) <= msg.value, "NFT: Ether value sent for public mint is not correct");
        }
    ...
    function setpresaleBasePrice(uint256 _presaleBasePrice) external onlyOwner {
        presaleBasePrice = _presaleBasePrice;
    }
    ...
    function setpublicsaleBasePrice(uint256 _publicsaleBasePrice) external onlyOwner {
        publicsaleBasePrice = _publicsaleBasePrice;
    }
    ...
    function airDrop(address[] memory _address) external onlyOwner {
        ...
        for (uint256 i = 0; i < _address.length; i++) {
            require(balanceOf(_address[i]).add(1) <= maxPurchasePerWallet, "NFT: max purchase reached");
