...
contract SDPResolver {
	...
	function claimSubdomain(uint256 tokenId) public authorised(tokenId) {
	...
	function deleteSubdomain(uint256 tokenId) public authorised(tokenId) {
		bytes32 label = keccak256(abi.encodePacked(tokenId.toString()));
		bytes32 labelNode = keccak256(abi.encodePacked(parentNode(), label));
		delete nodeToId[labelNode];
	}
