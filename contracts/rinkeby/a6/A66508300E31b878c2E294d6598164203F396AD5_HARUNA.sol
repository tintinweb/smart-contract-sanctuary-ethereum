...
// File: contracts/extension/ERC721PsiBurnable.sol
...
contract HARUNA is ERC721PsiBurnable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public maxBalance = 5555;
    uint256 public maxMint = 10;

    uint256 public WL_Mint_StartTime = 100000000000;
    ...
    function mintHARUNA(uint256 tokenQuantity) public payable nonReentrant{
        require(
    ...
    function mintHARUNA_Owner(uint256 tokenQuantity) public payable onlyOwner{

        uint256 req = tokenQuantity;

        require(
            totalSupply() + req <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
...
//////////////WLLLLLLLLLLLLLLL//////////////////
    ...
    function mintHARUNA_WL(uint256 tokenQuantity, bytes memory signature) public payable nonReentrant{
        require(
            totalSupply() + tokenQuantity <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
        ...
        require(tokenQuantity <= maxMint, "Can only mint 1 tokens at a time");
        require(block.timestamp > WL_Mint_StartTime,"not in mint time");
        //require(whitelist_counter[msg.sender] < 2,"");
    ...
    function withdraw() public onlyOwner {
        uint256 balance_core = address(this).balance * 2688 / 10000;
        uint256 balance_fund = address(this).balance * 7312 / 10000;
    ...
    function set_WL_Mint_Starttime(uint256 _time) public onlyOwner{
        WL_Mint_StartTime = _time;
    }
