  ...
  AccessControlEnumerable
{
  ...
  // keep track of the next tokenId to be created, also acts as total of token created (unless some were removed)
  ...
  struct Session {
    mapping(address => bool) creatorAllowed;
    address[] creators;
    uint256 commission;
    uint256 start;
    uint256 end;
    ...
    uint16 whitelistDiscount;
    uint256 discountCounts;
  }

  event ArtistUpdated(address account, bool status);
  ...
  function initialize(
    ...
    require(
      ...
      "OpenEdition: can not initialize"
    ); // ensure not init'd already.
    _name = contractName;
    _symbol = contractSymbol;
    _tokenURIPrefix = baseURI;
    _commissionReceiver = feeReceiver;

    _setupRole(OWNER, newOwner);
    _setupRole(ADMIN, newOwner);
    _setRoleAdmin(ADMIN, OWNER);
  }
  ...
  /**
   * @dev Gets the total number of token (and next tokenId to be created)
   */
  function tokenCounts() public view returns (uint256) {
   ...
   */
  function setTokenURIPrefix(string memory uriPrefix) public onlyRole(OWNER) {
  ...
  /**
   * @dev get metadata for a list of token Ids
   */
    ...
    )
  {
    ...
    address[] memory _tokensCreators = new address[](tokenIds.length);
    uint256[] memory _tokensTotalSupplies = new uint256[](tokenIds.length);
    uint256[] memory _tokensMaxSupplies = new uint256[](tokenIds.length);

    for (uint256 i = 0; i < tokenIds.length; ++i) {
      uint256 id = tokenIds[i];
      _tokensUris[i] = uri(id);
      _tokensCreators[i] = creator(id);
      _tokensTotalSupplies[i] = totalSupply(id);
      _tokensMaxSupplies[i] = maxSupply(id);
    }

    return (
      _tokensUris,
      _tokensCreators,
      _tokensTotalSupplies,
      _tokensMaxSupplies
    );
  }
  ...
  function mint(
    ...
    require(
      _maxSupply[id] == 0 || _totalSupply[id] + amount <= _maxSupply[id],
  ...
  function createSession(
    ...
    for (uint256 i = 0; i < whitelistCollections.length; i++) {
      ...
      _sessions[sessionId].collectionWhitelisted[collection] = true;
    }

    for (uint256 i = 0; i < accounts.length; i++) {
      address tokenCreator = accounts[i];
      require(
        tokenCreator != address(0) && !tokenCreator.isContract(),
        "OE: can not create Session"
      );

      _sessions[sessionId].creatorAllowed[tokenCreator] = true;
    }

    require(
      commission <= _ONE_HUNDRED_PERCENT,
      "OpenEdition: params is invalid"
    );

    require(
      whitelistDiscount <= _ONE_HUNDRED_PERCENT,
      "OpenEdition: invalid discount"
    );
    ...
    _sessions[sessionId].whitelistDiscount = whitelistDiscount;
    _sessions[sessionId].discountCounts = discountCounts;
  ...
  /**
   * @dev Update the session's allowed artists
   */
  function updateWhitelistedArtistForSession(
    uint256 sessionId,
    address[] memory artists
  ) public onlyRole(ADMIN) {
    require(
      _sessions[sessionId].start > 0 &&
        block.timestamp < _sessions[sessionId].start,
      "OpenEdition: session on-going"
    );

    _sessions[sessionId].creators = artists;
  }

  /**
   * @dev Removes a session
   */
  function removeSession(uint256[] memory sessionIds) public onlyRole(ADMIN) {
    uint256 cnt = 0;
    ...
    for (; cnt < sessionIds.length; cnt++) {
      ...
      require(
        _sessions[id].start > 0 && block.timestamp < _sessions[id].start,
        "OpenEdition: session on-going"
      );

      if (_sessions[id].tokenIds.length > 0) {
        for (uint256 index; index < _sessions[id].tokenIds.length; index++) {
          require(
            _totalSupply[_sessions[id].tokenIds[index]] == 0,
            "OE: some tokens already minted"
          );

          delete _tokenURIs[_sessions[id].tokenIds[index]];
          delete _tokenToSession[_sessions[id].tokenIds[index]];
          delete _prices[_sessions[id].tokenIds[index]];
          delete _creators[_sessions[id].tokenIds[index]];
          delete _maxSupply[_sessions[id].tokenIds[index]];
        }
      }
  ...
  // Should not allow artist to change the commission
  /**
  ...
  //     _sessions[sessionId].start > 0 &&
  //       block.timestamp < _sessions[sessionId].start,
  //     "OpenEdition: can not change"
  //   );

  //   require(
  //     commission <= _ONE_HUNDRED_PERCENT,
  //     "OpenEdition: invalid commission"
  //   );
  ...
  {
    return (
      _sessions[sessionId].creators,
      _sessions[sessionId].commission,
      _sessions[sessionId].tokenIds,
      _sessions[sessionId].start,
      _sessions[sessionId].end,
      _sessions[sessionId].whitelistDiscount,
      _sessions[sessionId].discountCounts
    );
  }

  // Should not allow owner to change commission receiver
  // /**
  //  * @dev Function to set the commission receiver
  //  */
  // function changeCommissionReceiver(address payable account) public onlyOwner {
  ...
  function _updateTokenMetadata(
    ...
    require(
      _creator != address(0) && bytes(_uri).length > 0,
      "ERC1155: params is invalid"
    );
    ...
    if (tokenMaxSupply != _maxSupply[id]) {
  ...
  function updateTokenMetadata(
    ...
    require(_tokenToSession[tokenId] == sessionId, "OE: token not in session");
    require(
      _sessions[sessionId].creatorAllowed[tokenCreator] == true &&
        block.timestamp < _sessions[sessionId].start,
      "OpenEdition: cannot update token"
    );
    require(_creators[tokenId] == tokenCreator, "OE: you are not the artist");
    ...
    nonReentrant
  {
    ...
    require(_tokenToSession[tokenId] == sessionId, "OE: token not in session");
    require(
      _sessions[sessionId].start < block.timestamp &&
        block.timestamp < _sessions[sessionId].end &&
        price == msg.value,
    ...
    if (price > 0 && price > commission) {
      payable(creator(tokenId)).transfer(price - commission);
    }

    mint(msgSender, tokenId, 1, "");

    emit TokenBought(sessionId, tokenId, msgSender, price, percentage);
  }

  /**
   * @dev Buys token using token from a whitelist collection
   */
  function buyTokenWithWithelist(
    uint256 sessionId,
    uint256 tokenId,
    address whitelistCollection,
    uint256[] calldata whitelistCollectionTokenId
  ) public payable nonReentrant {
    Session storage currentSession = _sessions[sessionId];

    uint256 price = (_prices[tokenId] *
      (_ONE_HUNDRED_PERCENT - currentSession.whitelistDiscount)) /
      _ONE_HUNDRED_PERCENT;

    require(
      currentSession.collectionWhitelisted[whitelistCollection] == true,
      "OE: not in whitelist"
    );
    require(whitelistCollectionTokenId.length == 1, "OE: can only redeem one");
    ...
    require(_tokenToSession[tokenId] == sessionId, "OE: token not in session");
    require(
      currentSession.start < block.timestamp &&
        block.timestamp < currentSession.end &&
        price == msg.value,
      "OpenEdition: can not buy token"
    );
    ...
    uint256 commission = (price * currentSession.commission) /
      _ONE_HUNDRED_PERCENT;
    ...
    ERC721Redeemer.Claims storage redeemedWhitelistTokens = currentSession
      .redeemedWhitelistTokens[whitelistCollection];

    uint256 n = redeemedWhitelistTokens.redeem(
      currentSession.discountCounts,
      msgSender,
      collection,
      whitelistCollectionTokenId
    );

    require(n == 1, "OE: cannot redeem with whitelist");

    mint(msgSender, tokenId, 1, "");

    emit TokenBought(sessionId, tokenId, msgSender, price, commission);
  }
}
