...
contract BNPL is ReentrancyGuard, Ownable, IERC721Receiver {
    ...
    enum OrderState{
        ...
        Created,
        // The Order has already paid completely
        Completed,
        // The Order is on sale
        OnSale
    }
    ...
    function setWallet(address _userWallet, address _custodioWallet) external onlyOwner {
    ...
    function getOrders(uint256[] calldata _orderId) view external returns(Order[] memory){
        uint256 l = _orderId.length;
        Order[] memory result = new Order[](l);
        for (uint i = 0; i < l; i++) {
        result[i] = orders[_orderId[i]];
    }
        return result;
    }

    function getTransaction(bytes32 _transactionId) view external returns(Transaction memory){
        return transactions[_transactionId];
    }

    function getTransactions(bytes32[] calldata _transactionId) view external returns(Transaction[] memory){
        uint256 l = _transactionId.length;
        Transaction[] memory result = new Transaction[](l);
        for (uint i = 0; i < l; i++) {
        result[i] = transactions[_transactionId[i]];
    }
        return result;
    }
    ...
    /* @dev buy now
       List method TBD
    */ 
     function buyNow(uint256 _itemId, uint256 _orderId, uint256 _nftPrice, uint256 _total, uint256 _downPayment, address _custodioWallet, bytes32 _txId, bytes32 _sign) external payable nonReentrant{
        ...
        // total amount need to greater than ntf price
        require(_total>_nftPrice, "Invalid Price");
    ...
    function repay(uint256 _orderId, uint256 _amount, bytes32 _txId, bytes32 _sign) external payable nonReentrant{
        ...
        require(order.state != OrderState.OnSale, "This order cannot repay due to listing");
        require(order.state == OrderState.Created, "Invalid order");
        
        
        if(_amount >= order.rest){
            order.rest = 0;
    ...
    function orderList(uint256 _orderId) external onlyOwner{
        Order memory order = orders[_orderId];
        order.state = OrderState.OnSale;
        orders[_orderId] = order;
    }
