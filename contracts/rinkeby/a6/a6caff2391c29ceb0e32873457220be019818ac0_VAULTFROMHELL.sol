...
contract VAULTFROMHELL is Ownable, IERC721Receiver {
    struct vaultInfo {
        IERC721 nft;
        // IERC20 token;
    ...
    mapping(address => uint256) private addrTotalStake;

    mapping(address => uint256) private addrTotalTowers;

    mapping(address => uint256) private addrTotalFamiliars;
    ...
    // function addCurrency(IERC20 _token) public onlyOwner {

    //     Styx.push(coins({coin: _token}));
    ...
    function _stakeBGF(uint256[] calldata tokenIds) internal {
        ...
        uint256 userTowers = addrTotalTowers[msg.sender]; // balanceOf(msg.sender, towerVault);

        uint256 userStaked = addrTotalStake[msg.sender]; // balanceOf(msg.sender, bgfVault);
    ...
    function _claim(uint256[] calldata tokenIds, bool _unstake) internal {
        ...
        uint256 userTokens = addrTotalStake[msg.sender]; //balanceOf(msg.sender, bgfVault);
        ...
        // vaultid.token.mint(msg.sender, earned);
        ...
        returns (uint256)
    {
        ...
        if (_pid == towerVault) {
            ...
            for (uint256 i = 1; i <= supply; i++) {
                if (activeTowers[i].owner == account) {
                    balance += 1;
                }
            }
        }
    ...
    function unlockLegendary(uint256 _tokenId) public payable {
        uint256 price = 0;

        // coins storage tokens = Styx[0];
    ...
    function unlockLegendarySafe(uint256 _tokenId) public onlyOwner {
        ...
        // Styx.transferFrom(msg.sender, address(this), price);
