...
contract TokenSale is ITokenSale, Ownable, ReentrancyGuard {
    ...
    /// @dev Set this value if token sale should be without tiers
    uint64 public baseTier;
    /// @dev END TIER INFO
    ...
    /// @notice Set base tier after `setTiers`
    /// @param _baseTier Base tier ID.
    function setBaseTier(uint64 _baseTier) external override onlyOwner beforeSales {
        require(_baseTier == 0 || tierIdToBuyStartTimestamp[_baseTier] != 0,
            "TokenSale: base tier isn't added to token sale.");
    ...
    function setBuyEndTimestamp(uint64 _buyEndTimestamp) external override onlyOwner beforeSales {
    ...
    /// @param _limits Wei.
    function setAddressLimits(uint256[] calldata _limits) external override onlyOwner beforeSales {
    ...
    function _getTierId() private view returns (uint64) {
        uint64 _baseTier = baseTier;
        if (_baseTier != 0) return _baseTier;
...
interface ITokenSale {
    ...
    /// @notice Set base tier after `setTiers`
    /// @param _baseTier Base tier ID.
    function setBaseTier(uint64 _baseTier) external;
