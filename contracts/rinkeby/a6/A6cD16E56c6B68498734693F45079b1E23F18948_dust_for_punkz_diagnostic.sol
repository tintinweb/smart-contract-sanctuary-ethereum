...
contract dust_for_punkz_diagnostic is Ownable,  dust_redeemer, IRNGrequestor,IERC777Recipient, ReentrancyGuard {
    ...
    event UnauthorisedAccess(address sender,address expected);
    ...
    function process(uint256 rand, bytes32 requestId) external override {
        if (msg.sender != rngAddress()) {
            emit UnauthorisedAccess(msg.sender,rngAddress());
            return;
        }
    ...
    function getDustAddress() external view returns (address) {
        return dustAddress();
    }
