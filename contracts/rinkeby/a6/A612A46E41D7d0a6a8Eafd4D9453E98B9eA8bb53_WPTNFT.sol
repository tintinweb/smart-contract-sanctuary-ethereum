...
contract WPTNFT is ERC721, IWPTNFT, Ownable {
    ...
    address public feeToken = 0xC7c135124AFEddfdC7868bEa9A7Df728D2B6CAC0;
    uint256 public feeAmount = 358 * 10**6;
    uint256 public performanceFee = 89 * 10**14;
    uint256 public maxSupply = 58;
    uint256 public maxLimit = 5;
    address public feeWallet = 0xC2a5ea1d4406EC5fdd5eDFE0E13F59124C7e9803;
    address private teamWallet = 0xC2a5ea1d4406EC5fdd5eDFE0E13F59124C7e9803;
    // Star id to cid.
    ...
    function _transferFee(uint256 amount) internal {
        ...
        if (!whitelists[msg.sender]) {
            ...
            IERC20(feeToken).transfer(feeWallet, amount);
        }
    }
    ...
    {
        require(
            IERC20(feeToken).balanceOf(msg.sender) >= feeAmount.mul(amount),
            "Not Enough Fee"
        );
    ...
    function getRandomMintNum() public view returns (uint256) {
        uint256 r = random() % maxSupply;
        if (mintedNums[r]) {
            for (uint256 i = r + 1; i < r + maxSupply; i++)
                if (!mintedNums[i % maxSupply]) return i;
        } else return r;
    }
}
