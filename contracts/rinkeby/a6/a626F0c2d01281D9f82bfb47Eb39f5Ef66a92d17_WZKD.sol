...
contract WZKD is ERC721Base, PaymentSplitter, Ownable, AccessControl, Pausable, ReentrancyGuard, MerkleTree {
    uint private maxPerTx = 6;
    uint private maxWlMint = 10;
    ...
    function privateMint(uint amount, bytes32[] calldata _proof) 
        ...
        require(claimed[msg.sender] + amount <= maxWlMint, "amount exceeded maximum");
    ...
    function publicMint(uint amount) external payable whenNotPaused nonReentrant {
        require(amount <= maxPerTx, "amount exceeded maximum");
    ...
    function setMaxWlMint(uint amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        maxWlMint = amount;
    }
