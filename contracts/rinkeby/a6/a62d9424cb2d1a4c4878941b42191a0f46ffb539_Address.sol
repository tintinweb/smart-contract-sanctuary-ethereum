    ...
    PaymentSplitter
{
    ...
    function giftMint(address[] calldata _addresses) external onlyOwner {
        require(
            ...
            "MyNFT: max total supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "MyNFT: recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function freeMint(uint256 _amount, bytes32[] memory proof) external payable {
        require(freeMintState, "MyNFT: FreeMint is OFF");
        ...
        require(verifyFreeMint(msg.sender, proof), "MyNFT: You are not selected for the free mint");

        require(
            totalNFT + _amount <= maxSupplyWithFreeMint,
            "MyNFT: max supply exceeded"
        );
        uint256 _newItemId;
    ...
    function presaleMint(uint256 _amount) external payable {
        require(presaleState, "MyNFT: Presale is OFF");
        require(!paused, "MyNFT: contract is paused");
        ...
        require( _nftMinted[msg.sender] + _amount <= 4, "MyNFT: You can't mint so much tokens");
        ...
        require(
            ...
            "MyNFT: Ether value sent is not correct"
        );
        uint256 _newItemId;
        for (uint256 ind = 0; ind < _amount; ind++) {
            _tokenIds.increment();
    ...
    function publicMint(uint256 _amount) external payable {
        require(publicState, "MyNFT: Public is OFF");
        require(_amount > 0, "MyNFT: zero amount");
        require(
            totalNFT + _amount <= maxSupply,
            "MyNFT: max supply exceeded"
        );
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "MyNFT : burning disabled");
        require(
