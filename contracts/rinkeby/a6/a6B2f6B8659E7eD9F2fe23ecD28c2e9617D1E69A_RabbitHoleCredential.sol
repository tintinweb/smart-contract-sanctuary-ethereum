    ...
    UUPSUpgradeable
{
    ...
    event Mint(address addr, uint256 tokenId, uint256 blockNumber);
        ...
        returns (string memory)
    {
        string memory image = Base64.encode(
            bytes(NFTDAO.generateSVG(_tokenId, tokenIdToBlockNumber[_tokenId]))
        );
...
library NFTDAO {
    function generateSVG(uint256 _tokenId, uint256 _blockNumber)
        internal
        pure
