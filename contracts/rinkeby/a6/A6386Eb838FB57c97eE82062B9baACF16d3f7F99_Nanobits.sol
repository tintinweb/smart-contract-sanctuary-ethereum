...
contract Nanobits is ERC20Burnable, Ownable {
    ...
    uint256 public constant legendaryRatePerDay = 69444444444444 * 2 * 1440; // 6 $NANOBITS per day for 1/1s
    uint256 public constant commonRatePerDay = 34722222222222 * 2 * 1440; // 3 $NANOBITS per day for common
    ...
    // STAKING FUNCTIONS
    function stake(uint256[] memory _tokenIds, uint64 expireTime, bytes memory sig, uint256[] calldata rarity) external isSecured(1) {
    ...
    // UNSTAKE FUNCTIONS 1659559406

    function getTime(uint256 timestamp) public view returns(uint256) {
        // uint256 numOfDays = ((block.timestamp - timestamp) / 1 days) * 1e18;
        uint256 reward = ((block.timestamp - timestamp) * commonRatePerDay); // days reward
        uint256 multiplier = 1e18 + (4e18 * 14 / 1000000);
        uint256 totalRewards = (reward * multiplier) / 1e18;
        return totalRewards;
    }
    ...
    function claim(uint256 id) internal view returns(uint256) {
        ...
        uint256 numOfDays = ((block.timestamp - stakedData[id].timeStaked) / 1 minutes) * 1e18; // number of days
        if(numOfDays > 7) {
    ...
    //         ratePerday = commonRatePerDay;
    //     }
    //     uint256 numOfDays = ((block.timestamp - stakedData[id].timeStaked) / 1 days) * 1e18; // number of days
    //     if(numOfDays > 14) {
    //         uint256 reward = ((block.timestamp - stakedData[id].timeStaked) * ratePerday); // days reward
    //         uint256 multiplier = 1e18 + (numOfDays * 14 / 1000000);
    //         totalRewards = (reward * multiplier) / 1e18;
    ...
    function testMultiplier(uint256 timestamp) external view returns(uint256) {
        // uint256 totalRewards = 0;
        uint256 numOfDays = ((block.timestamp - timestamp) / 1 minutes) * 1e18;
        // if(numOfDays > 14) {
        //     uint256 reward = ((block.timestamp - timestamp) * commonRatePerDay); // days reward
        //     uint256 multiplier = 1e18 + (numOfDays * 14 / 1000000);
        //     totalRewards = (reward * multiplier) / 1e18;
    ...
    function testClaim(uint256 id, uint256 timestamp) external view returns(uint256) {
        uint256 totalRewards = 0;
        uint256 ratePerday = 0;
        ...
        uint256 numOfDays = ((block.timestamp - timestamp) / 1 minutes) * 1e18; // number of days
        if(numOfDays > 7) {
            uint256 reward = ((block.timestamp - timestamp) * ratePerday); // days reward
            uint256 multiplier = 1e18 + (numOfDays * 14 / 1000000);
            totalRewards = (reward * multiplier) / 1e18;
        }
        ...
        if(numOfDays <= 14) {
            totalRewards += ((block.timestamp - timestamp) * ratePerday);
        }
    ...
    function yagiPupContract(address yagipContractAddress) external onlyOwner{
        yagiContract = ERC721A(yagipContractAddress);
    }
