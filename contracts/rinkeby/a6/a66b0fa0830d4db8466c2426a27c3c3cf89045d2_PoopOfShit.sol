...
contract PoopOfShit is ERC2981, ERC721AQueryable, Ownable {
    ...
    function mint(uint32 amount) external payable {
        require(_started, "poopofshit : Sale is not started");

        uint32 publicMinted = _publicMinted();
        uint32 publicSupply = _publicSupply();
        require(amount + publicMinted <= _publicSupply(), "poopofshit : Exceed max supply");

        uint32 minted = uint32(_numberMinted(msg.sender));
        require(amount + minted <= _walletLimit, "poopofshit : Exceed wallet limit");
        ...
        require(msg.value >= requiredValue, "poopofshit : Insufficient fund");
    ...
    function devMint(address to, uint32 amount) external onlyOwner {
        _teamMinted += amount;
        require(_teamMinted <= _teamSupply, "poopofshit : Exceed max supply");
        _safeMint(to, amount);
    }
