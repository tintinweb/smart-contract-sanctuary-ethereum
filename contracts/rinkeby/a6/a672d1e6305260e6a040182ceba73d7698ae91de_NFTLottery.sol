...
contract NFTLotteryFactory is INFTLotteryFactory, Initializable, Ownable {
    ...
    function createNFTLottery(
        ...
        lottery.initialize(this, operator, treasuryFee, info);
    ...
    IERC721Receiver
{
    ...
    mapping(address => uint256[]) userTickets; // odd: index, even: count
    ...
    event BuyTickets(address buyer, uint256 ticketStartIdx, uint256 count);
    event LotteryOpened(uint256 luckyTicket, address luckyAddr, uint256 value);
    ...
    function buyTickets(uint256 count) external payable {
        ...
        require(
            lottery.maxBuy == 0 || count <= lottery.maxBuy,
            "Exceed maxBuy"
        );
        require(
        ...
        userTickets[msg.sender].push(lottery.soldCount);
        userTickets[msg.sender].push(count);
        emit BuyTickets(msg.sender, lottery.soldCount, count);

        total += count;
        lottery.soldCount += count - give;
    ...
    function claimNft() external {
        ...
        require(msg.sender == ticketBook[lottery.luckyTicket], "Not winner");
        transferNftTo(msg.sender);
    }

    function claimPayment() external onlyOwner nonReentrant {
        require(lottery.status == Status.Claimable, "Not claimable");
    ...
    function receiveRandomNumber(uint256 _randomNumber) external override {
        ...
        lottery.luckyTicket = randomNumber % total;
        setLotteryStatus(Status.Claimable);

        address luckyAddr = ticketBook[lottery.luckyTicket];
        emit LotteryOpened(
            lottery.luckyTicket,
            luckyAddr,
    ...
    function transferNftTo(address user) private {
    ...
    function getUserTickets(address a) public view returns (uint256[] memory) {
        return userTickets[a];
    }
