...
contract PoolManager is OwnableUpgradeable, IPoolManager {
    ...
    modifier onlyAmaAddress() {
        emit sender(msg.sender);
        require(
            amaPartnerAddress.contains(msg.sender) || owner() == msg.sender,
            "Not a whitelisted factory"
        );
        _;
    }
    ...
    function setInsurancePrice(uint256 _price) public onlyOwner {
    ...
    function registerAma(
        ...
        require(
            !amaPartnerAddress.contains(_ownerAddress),
            "Already Register !!!"
        );
        amapartner[_ownerAddress] = AmaPartner(
            _name,
            _username,
            _ownerAddress,
            _email,
    ...
    function updatePartnerAddress(address _address, address _oldAddress)
        public
        onlyOwner
    {
        require(
            amaPartnerAddress.contains(_oldAddress),
            "No Partner Avalible with Id"
        );
        require(
            amapartner[_oldAddress].ownerAddress != address(0),
            "No Partner Avalible with Id"
        );
        amapartner[_address] = AmaPartner(
    ...
    function updatePartnerInfo(
    ...
    ) public onlyAmaAddress {
        require(
            amaPartnerAddress.contains(msg.sender),
            "No Partner Avalible with Id"
        );
        ...
        if (_updatedAddress == address(0)) {
            ...
            amapartner[_updatedAddress].details = amapartner[msg.sender]
                .details;
    ...
    function partnerUpdateDetails(uint256 _fees, string memory _details)
        public
        onlyAmaAddress
    {
        AmaPartner storage ama = amapartner[msg.sender];
    ...
    function partnerHistory(address _address)
        public
        view
        returns (AmaPartnerHistory[] memory)
    {
        return amapartnerhistory[_address];
    }

    function poolPartnerList(address _address)
        public
        view
        returns (poolPartnerDetails[] memory)
    {
    ...
    function payAmaPartner(
        address[] memory _partnerAddress,
        address _poolAddress
    ) public payable override onlyAllowedFactory {
        ...
        for (uint256 i = 0; i < _partnerAddress.length; i++) {
            ...
            require(
                amaPartnerAddress.contains(_partnerAddress[i]),
                "Partner address doesn't exist"
            );
            require(
                amapartner[_partnerAddress[i]].status,
                "Currently Partner service not avalible !!"
            );
            uint256 fees = amapartner[_partnerAddress[i]].fees.mul(feesPer).div(
                10000
            );
            uint256 totalPay = amapartner[_partnerAddress[i]].fees.sub(fees);
            (bool success, ) = payable(_partnerAddress[i]).call{
                value: totalPay
            }("");
    ...
    function getInsurancePrice() public view override returns (uint256) {
    ...
    function buyInsurance(
        ...
        userinsurance[_userAddress].push(
            userInsurance(_poolAddress, _amount, block.timestamp)
        );
    }

    function countTotalPay(address[] memory _partnerAddress)
        public
        view
        override
        returns (uint256)
    ...
    function getUSDRate() public view override returns (uint256, uint32) {
        require(USDPAIR != address(0), "USD pair Address not set!!");
        (
            uint256 reserve0,
            uint256 reserve1,
            uint32 blockTimestampLast
        ) = IUniswapV2Pair(USDPAIR).getReserves();

        if (reverseCheck) {
            uint256 rate = reserve1.mul(10**18).div(reserve0).mul(
                10**USDPAIRDECIMALS
            );
            return (rate, blockTimestampLast);
        } else {
            uint256 rate = reserve0.mul(10**18).div(reserve1).mul(
                10**USDPAIRDECIMALS
            );
            return (rate, blockTimestampLast);
        }
