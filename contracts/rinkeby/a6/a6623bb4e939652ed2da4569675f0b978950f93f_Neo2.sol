...
interface iHope {
    function updateReward(address from_, address to_, uint256 tokenId) external;
}
...
contract Neo2 is ERC721, Ownable, MerkleWhitelist {

    address public hopeAddress;
    ...
    function whitelistMint(bytes32[] memory proof_) external payable {
        require(whitelistSaleEnabled, "whitelistMint Paused");
        require(isWhitelisted(msg.sender, proof_), "You are not whitelisted");
        require(addressToWhitelistMinted[msg.sender] < 3, "You have no whitelist mints left");
        require(msg.value == wlmintPrice, "Invalid Value Sent");
        require((1 + tokenCount) <= _totalSupply, "No more NFTs");
    ...
    function publicMint() public payable {
        ...
        require(addressToPublicMinted[msg.sender] < 5, "You have no whitelist mints left");
        require((1 + tokenCount) <= _totalSupply, "No more NFTs");
    ...
    function setHope(address address_) external onlyOwner { 
        hopeAddress = address_; Hope = iHope(address_); 
    }

    function transferFrom(address from_, address to_, uint256 tokenId) public override {
        if ( Hope != iHope(address(0x0)) ) {
            Hope.updateReward(from_, to_, tokenId);
        }
        ERC721.transferFrom(from_, to_, tokenId);
    }

    function safeTransferFrom(address from_, address to_, uint256 tokenId, bytes memory data_) public override {
        if ( Hope != iHope(address(0x0)) ) {
            Hope.updateReward(from_, to_, tokenId);
        }
        ERC721.safeTransferFrom(from_, to_, tokenId, data_);
    }
