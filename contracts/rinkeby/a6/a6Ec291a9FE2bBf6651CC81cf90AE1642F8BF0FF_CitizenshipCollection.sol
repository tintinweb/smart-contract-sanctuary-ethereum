...
contract CitizenshipCollection is
    ERC721,
    IRevocable,
    ReentrancyGuard,
    Coupon,
    ...
    VRFConsumerBaseV2
{
    ...
    mapping(address => PrimaryIdentity) public primaryIdentities;
    ...
    function setPrimary(uint256 tokenId) external returns (bool successful) {
        ...
        primaryIdentities[msg.sender].tokenId = tokenId;
    ...
    function revokePrimary() external returns (bool successful) {
        require(primaryIdentities[msg.sender].unlockedAt < block.timestamp, 
        "must wait for primary lock to expire");

        delete primaryIdentities[msg.sender];
    ...
    /// @return successful -> confirmation of activity
    function lockPrimary(uint256 durationInSeconds) external returns (bool successful) {
        uint256 lockDuration = block.timestamp + durationInSeconds;
        
        require(lockDuration > primaryIdentities[msg.sender].unlockedAt, 
        "must not shorten existing lock duration");

        primaryIdentities[msg.sender].unlockedAt = lockDuration;
        emit PrimaryIdentityRevoked(msg.sender);
        successful = true;
    }
    ...
    function _transfer(
        ...
        require(
            ...
            "ANS: must revoke token id from primary identity"
        );
        ...
        if (_isANSConfigured()) {
            if (IANS(ANS).isAuthEnabled(msg.sender)) {
                ...
                require(
                    successful && index > 0,
                    "ANS: token transfer request must be approved by trustees"
                );
                // There's no Reentrancy risk because IANS is a contract owned by the same project
                IANS(ANS).clearRequest(msg.sender, index);
            }
        }
    ...
    function _isPrimaryIdentity(address account, uint256 tokenId)
        ...
        returns (bool successful)
    {
        successful = primaryIdentities[account].tokenId == tokenId;
    }
}