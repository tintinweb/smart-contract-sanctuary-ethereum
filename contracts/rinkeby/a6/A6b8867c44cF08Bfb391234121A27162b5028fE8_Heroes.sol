...
contract Heroes is ERC721ABurnable, Ownable, ReentrancyGuard {
  ...
  uint256 public freeFee = 10000000000000000;
 ...
 function theMinter(bytes32[] memory _merkleProof, uint mintCount) validateMinting(mintCount) nonReentrant external payable {
     bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
     uint mintingFeesToCharge = mintCount;
     if(MerkleProof.verify(_merkleProof, _freeMintMerkleRoot, leaf)){
       if(_freeMintRedeemed[msg.sender] != 1){ // if the baby is NOT already redeemed, mark it as used and let us use the free one.
        if(mintCount != 0){
         mintingFeesToCharge = 0;
         _freeMintRedeemed[msg.sender] = 1;
       } else {
        mintingFeesToCharge = mintingFeesToCharge - 1;
         _freeMintRedeemed[msg.sender] = 1;
       }
      } 
     }
