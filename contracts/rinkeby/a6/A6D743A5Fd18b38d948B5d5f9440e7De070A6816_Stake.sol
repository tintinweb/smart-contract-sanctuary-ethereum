...
contract Stake {
    ...
    uint[9] public periods = [0, 7 minutes, 7 minutes, 14 minutes, 14 minutes, 30 minutes, 30 minutes, 60 minutes, 90 minutes];
    uint[9] public rates = [0, 333, 250, 200, 166, 143, 125, 111, 100];
    uint[4] public cms = [6, 3, 1, 20];

    IERC20 token = IERC20(0x1627DF00d54675FDb90c47EFf941D55528A527F7);
    uint decimals = 18;
    ...
    function reward() public view returns(uint rw){
        uint _bal = vault[msg.sender].balances;
        uint _days = (block.timestamp - vault[msg.sender].hvtime) / 1 minutes; 
        if (level(_bal) >= 1 && vault[msg.sender].hvtime > 0){
            rw = fracExp(_bal, rates[level(_bal)], _days, 16)  + vault[msg.sender].hvaccum - _bal;
        }
    }
    ...
    function stake(uint256 _value, address _parent) public returns(bool) {
        ...
        if (vault[msg.sender].hvtime > 0 && level(vault[msg.sender].balances) >= 1){
            vault[msg.sender].hvaccum += reward();
        }
    ...
    function unstake() public {
        require(vault[msg.sender].balances > 0, ""); // && start[msg.sender] > 0
        require(block.timestamp - vault[msg.sender].start >= periods[level(vault[msg.sender].balances)], "");
    ...
    function harvest() public {
        require(level(vault[msg.sender].balances) >= 1, "");
        uint rw = reward();
        require(rw > 0, "");
