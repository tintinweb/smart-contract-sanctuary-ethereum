...
contract VeDefed is IVeDefed, Ownable, ReentrancyGuard{
    ...
    constructor(address _tokenAddr, string memory _name, string memory _symbol) {
        token = _tokenAddr;
    ...
    function getLocked(address _addr) external view override returns (LockedBalance memory) {
        return locked[_addr];
    }

    function getUserPointEpoch(address _userAddress) external view override returns (uint256) {
    ...
    function increaseAmountFor(address _beneficiary, uint256 _value) external override {
     ...
     */
    function _increaseAmount(address _beneficiary, uint256 _value) internal nonReentrant {
        LockedBalance memory _locked = locked[_beneficiary];
     ...
     */
    function findBlockEpoch(uint256 _block, uint256 _max_epoch) internal view returns (uint256) {
        // Binary search
        uint256 _min = 0;
