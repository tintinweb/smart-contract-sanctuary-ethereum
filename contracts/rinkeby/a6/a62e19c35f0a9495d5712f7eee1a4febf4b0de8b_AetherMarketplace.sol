...
import "./interfaces/IAetherERC1155.sol";

contract AetherMarketplace is ReentrancyGuard, IBaseExchange, ERC1155Holder {
    ...
    constructor(
        ...
        _DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256("AetherMarketplace"),
                keccak256(bytes("1")),
                chainId,
                address(this)
            )
        );
        aetherERC1155 = _aetherERC1155;
    ...
    function _transfer(
        ...
        IERC1155(aetherERC1155).safeTransferFrom(from, to, tokenId, amount, "");
    }

    function offerCancel(Orders.Offer memory offer) external nonReentrant {
        require(!isPaused[PauseType.OfferCancel], "AETHER: PAUSED");
        require(offer.signer == msg.sender, "AETHER: FORBIDDEN");
    ...
    function primaryCancel(Orders.MintAsk memory askOrder)
        ...
        nonReentrant
    {
        require(!isPaused[PauseType.PrimaryCancel], "AETHER: PAUSED");
        require(askOrder.signer == msg.sender, "AETHER: FORBIDDEN");
        bytes32 hash = askOrder.hash();
        require(!isCancelled[hash], "AETHER: CANCELLED");
        isCancelled[hash] = true;
        if (mintedHash[hash] > 0) {
            // Burn all remaining tokens
            IERC1155(aetherERC1155).burn(
                mintedHash[hash],
                IERC1155(aetherERC1155).balanceOf(
                    address(this),
                    mintedHash[hash]
                )
            );
    ...
    function secondaryCancel(Orders.Ask memory askOrder) external nonReentrant {
        require(!isPaused[PauseType.SecondaryCancel], "AETHER: PAUSED");
        require(askOrder.signer == msg.sender, "AETHER: FORBIDDEN");
        bytes32 hash = askOrder.hash();
        require(!isCancelled[hash], "AETHER: CANCELLED");
        isCancelled[hash] = true;
    ...
    function askUpdate(
        ...
        require(!isPaused[PauseType.AskUpdate], "AETHER: PAUSED");
        bytes32 prevHash = prevAskOrder.hash();
        bytes32 hash = askOrder.hash();
        require(askOrder.signer == msg.sender, "AETHER: INVALID_CALLER");
        require(askOrder.signer == prevAskOrder.signer, "AETHER: SIGNER");
        require(askOrder.askType == prevAskOrder.askType, "AETHER: TYPE");
        require(askOrder.nonce != prevAskOrder.nonce, "AETHER: NONCE");
        require(!isCancelled[prevHash], "AETHER: CANCELLED");
        require(mintedHash[hash] == 0, "AETHER: ALREADY_MINTED");
        require(askOrder.amount > amountFilled[prevHash], "AETHER: AMOUNT_LOW");
    ...
    function primaryBuy(
        ...
        require(!isPaused[PauseType.PrimaryBuy], "AETHER: PAUSED");
        bytes32 askHash = askOrder.hash();
        uint256 _amountFilled = amountFilled[askHash];
        require(amount > 0, "AETHER: INVALID_AMOUNT");
        require(_amountFilled + amount <= askOrder.amount, "AETHER: SOLD_OUT");
        require(askHash == commission.askHash, "AETHER: UNMATCHED_HASH");
        require(askHash == token.askHash, "AETHER: UNMATCHED_HASH");
        require(
        ...
        if (tokenId == 0) {
            tokenId = IERC1155(aetherERC1155).whitelistMint(
                askOrder.amount,
                askOrder.royaltyInfo.value,
                askOrder.royaltyInfo.recipient,
                token.uri,
                ""
            );
    ...
    function primaryClaim(
        ...
        require(!isPaused[PauseType.PrimaryClaim], "AETHER: PAUSED");
        bytes32 askHash = askOrder.hash();
        bytes32 bidHash = bidOrder.hash();
        require(amountFilled[askHash] == 0, "AETHER: SOLD_OUT");
        require(askHash == bidOrder.askHash, "AETHER: UNMATCHED_HASH");
        require(askHash == commission.askHash, "AETHER: UNMATCHED_HASH");
        require(askHash == token.askHash, "AETHER: UNMATCHED_HASH");
        require(askOrder.signer == msg.sender, "AETHER: CALLER");
        require(bidOrder.price >= askOrder.price, "AETHER: PRICE");
        require(
    ...
    function secondaryBuy(
        ...
        require(!isPaused[PauseType.SecondaryBuy], "AETHER: PAUSED");
        bytes32 askHash = askOrder.hash();
        uint256 _amountFilled = amountFilled[askHash];
        require(amount > 0, "AETHER: AMOUNT");
        require(_amountFilled + amount <= askOrder.amount, "AETHER: SOLD_OUT");
        require(askHash == commission.askHash, "AETHER: UNMATCHED_HASH");
        require(
            askOrder.askType == Orders.AskType.SecondarySale,
            "AETHER: ASKTYPE"
        );
        _validateSecondary(askOrder, askHash);
        _verify(askHash, askOrder.signer, askOrder.v, askOrder.r, askOrder.s);
        _verify(
            commission.hash(),
            operator,
    ...
    function secondaryClaim(
        ...
        require(!isPaused[PauseType.SecondaryClaim], "AETHER: PAUSED");
        bytes32 askHash = askOrder.hash();
        bytes32 bidHash = bidOrder.hash();
        require(amountFilled[askHash] == 0, "AETHER: SOLD_OUT");
        require(askHash == bidOrder.askHash, "AETHER: UNMATCHED_HASH");
        ...
        require(bidOrder.price >= askOrder.price, "AETHER: PRICE");
        require(
            askOrder.askType == Orders.AskType.SecondaryAuction,
            "AETHER: ASKTYPE"
        );
        _validateSecondary(askOrder, askHash);
        _verify(askHash, askOrder.signer, askOrder.v, askOrder.r, askOrder.s);
    ...
    function offerClaim(
        ...
        require(!isPaused[PauseType.OfferClaim], "AETHER: PAUSED");
        bytes32 offerHash = offer.hash();
        require(offerHash == commission.askHash, "AETHER: UNMATCHED_HASH");
    ...
    function _validateOffer(Orders.Offer memory offer, bytes32 offerHash)
        internal
        view
    {
        require(!isCancelled[offerHash], "AETHER: OFFER_CANCELLED");
        require(offer.signer != address(0), "AETHER: MAKER");
        require(offer.amount > 0, "AETHER: AMOUNT");
        require(supportedTokens[offer.currency], "AETHER: CURRENCY");
    }

    function _validateSecondary(Orders.Ask memory askOrder, bytes32 askHash)
        internal
        view
    {
        require(!isCancelled[askHash], "AETHER: CANCELLED");
        require(askOrder.signer != address(0), "AETHER: MAKER");
        require(askOrder.amount > 0, "AETHER: AMOUNT");
        require(supportedTokens[askOrder.currency], "AETHER: CURRENCY");
    }
    ...
    function _transferFeesAndFunds(
        ...
        if (isEth) {
            require(msg.value == value, "AETHER: PAYMENT");
        } else {
            require(msg.value == 0, "AETHER: PAYMENT");
        }
        ...
        (address receiver, uint256 royaltyAmount) = IERC1155(aetherERC1155)
        ...
        if (receiver != to) {
            if (isEth) {
                ...
                IERC20(currency).transferFrom(from, receiver, royaltyAmount);
            }
    ...
    function setSupportedTokens(address[] memory _supportTokens, bool _set)
        external
    {
        require(admin == msg.sender || owners[msg.sender], "AETHER: CALLER");
        for (uint8 i = 0; i < _supportTokens.length; i++) {
    ...
    function setOwners(address[] memory _owners, bool _set) external {
        require(admin == msg.sender, "AETHER: CALLER");
...
library Orders {
    ...
    // keccak256("RoyaltyInfo(address recipient,uint16 value)")
    bytes32 internal constant ROYALTY_INFO_TYPEHASH =
        0xc6090a2d0dc6f0e9633ab40b3b4e2195512e08919c9239d71aabe54618353d38;
    // keccak256("MintAsk(uint8 askType,address signer,address currency,uint256 amount,uint256 price,uint256 nonce,RoyaltyInfo royaltyInfo)RoyaltyInfo(address recipient,uint16 value)")
    ...
    function hashRoyaltyInfo(RoyaltyInfo memory royaltyInfo)
        internal
        pure
        returns (bytes32)
    {
        ...
        return
            keccak256(
                abi.encode(
                    ...
                    mintAsk.price,
                    mintAsk.nonce,
                    hashRoyaltyInfo(mintAsk.royaltyInfo)
                )
            );
    }
