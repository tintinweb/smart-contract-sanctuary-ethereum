...
//          ######### ########  ###       ### ######### ########### ########## ########           //
//                                                                                                //
//                             b a c k  f o r  s o m e  f r e s h  m e a t                        //
////////////////////////////////////////////////////////////////////////////////////////////////////
...
contract KVLT_ZOMBIES_GENESIS is ERC721, Ownable {
    using Counters for Counters.Counter;   
    Counters.Counter private _tokenIdCounter;
    uint256 _price = 0.025 ether;
    uint256 _allowlistprice = 0.02 ether;
    uint256 _mintpassprice = 0.01 ether;
    ...
    constructor(address dependentContractAddress) ERC721("KVLT_ZOMBIES_GENESIS", "KVLTS") {
        setBaseURI("");
    ...
    function allowlistMint(uint256 mintCount,uint8 v, bytes32 r,bytes32 s,uint256 mint_allowed,uint256 free) external payable {
        require(PresaleIsActive, "Zombies Presale not active");
        ...
        require(ownerMintedCount + mintCount <= mint_allowed, "Individual minting limit for Zombies on allowlist exceeded!");
    ...
    function mintpassMint(uint256 mintCount) external payable {
        ...
        require(balance > 0, "Must hold at least one Mintpass to mint your Zombies with this function");

        require(mintCount <= 10, "Zombies quantity must be 10 or less");

        uint256 ownerMintedCount = addressMintedBalance[msg.sender];
        require(ownerMintedCount + mintCount <= _perWalletLimit, "Maximum amount of Zombies mints for this wallet exceeded!");

        mint(msg.sender,mintCount);
    }
    
    function zombieMint(uint256 mintCount,uint8 v, bytes32 r,bytes32 s,uint256 mint_allowed,uint256 free) external payable {
        require(PresaleIsActive, "Zombies Presale not active");

        require(verifySignature(v,r,s,mint_allowed,free), "Invalid signature - please mint only on our website and make sure you are on the allowlist.");
        require(free == 1, "You are not allowed to claim a free Cyclop");
        uint256 ownerMintedCount = addressMintedBalance[msg.sender];
        require(ownerMintedCount + mintCount <= mint_allowed, "This exceeds your allowed free Zombies");
    ...
    function publicMint(uint256 mintCount) external payable {
        ...
        require(mintCount <= 5, "Zombies quantity must be 5 or less");
    ...
    function mint(address addr, uint256 mintCount) private {
        require((_tokenIdCounter.current() + mintCount) <= tokenSupply, "Sold out! No more Zombies are available");
