...
contract DreamEyeGenV1 is ERC721AQueryable, ERC721ABurnable, Ownable, ReentrancyGuard, ERC2981 {
  ...
  */
  ...
  function tokenURIForGen(uint256 _tokenId, uint256 gen) public view returns (string memory) {
    require(_exists(_tokenId), 'ERC721Metadata: URI query for nonexistent token');
    ...
    string memory currentBaseURI = genURIPrefix[gen];
    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI, _tokenId.toString(), uriSuffix))
        : '';
  }
  ...
  function setGenUriPrefix(uint256 gen, string memory _uriPrefix) public onlyOwner {
    genURIPrefix[gen] = _uriPrefix;
  }
  ...
  function evolve(uint256 _tokenId, uint256 gen) public {
    require(_msgSender() == ownerOf(_tokenId), "User is not the token owner");
