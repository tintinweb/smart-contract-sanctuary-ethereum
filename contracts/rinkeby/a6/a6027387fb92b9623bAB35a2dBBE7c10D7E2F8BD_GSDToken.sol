...
contract GSDToken is ERC721Enumerable, Ownable, Pausable {
    ...
    // uint256 private constant PRESALE_TIMESTAMP = 1645056000;
    uint256 private constant PUBLIC_TIMESTAMP = 1645142400;
    uint256 private constant PRESALE_TIMESTAMP = 1644942569;
    // uint256 private constant PUBLIC_TIMESTAMP = 1644942569;

    uint256 private constant PRESALE_PRICE = 0.05 ether;
    uint256 private constant PUBLIC_PRICE_INIT = 0.07 ether;
    uint256 private constant PUBLIC_PRICE_MAIN = 0.09 ether;
    ...
    function mint(uint256 _amount) private {
        ...
        for (uint256 i = 0; i < _amount; i++) {
            ...
            emit GSDMinted(msg.sender, tokenId);
        }
    }
        ...
        notPaused
    {
        ...
        require(block.timestamp < PUBLIC_TIMESTAMP, "Presale is ended");
        require(msg.value >= _amount * PRESALE_PRICE, "Value is not enough");

        bytes32 merkleTreeRoot = 0x3dfdbbcefa4df9de540dd9d00ad0597b0b1def432f691e771e03c2637c64ad67;
    ...
    function PublicSale(uint256 _amount) external payable notPaused {
        require(
            ...
            "PublicSale is not started yet"
        );
        if (totalSupply() < 500)
            require(
                msg.value >= _amount * PUBLIC_PRICE_INIT,
                "Value is not enough (init)"
            );
        else
            require(
                msg.value >= _amount * PUBLIC_PRICE_MAIN,
