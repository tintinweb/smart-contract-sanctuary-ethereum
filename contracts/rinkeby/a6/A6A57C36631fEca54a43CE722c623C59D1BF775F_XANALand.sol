...
abstract contract Storage {
    ...
    event _mintCommon(address userAddress, string rarity, uint size, uint amount, uint price, bool isDiscount);
    event _mintLand(address userAddress, string rarity, uint size, uint amount, uint price, bool isDiscount);
...
contract XANALand is ProxyBaseStorage, ERC721AQueryable, Ownable, ReentrancyGuard, Storage {
 using Strings for uint256;
    ...
    function mintNFT(uint256 _mintAmount, string memory rarity, uint256 _size) internal {
    ...
    function calculateDiscount(uint256 _size, string memory rarity) internal view  returns(uint256) {
    ...
    //Discounted Mint Function
     function mintDiscountCommon(uint256 _mintAmount, uint256 _size, bytes32[] calldata proof, bool isLimit) checkSupply(_mintAmount,_size, "Common") external payable nonReentrant{
        string memory rarity = "Common";
        uint256 balance = SafeMath.mul(msg.value,10 ** 18);

         //Check user buy limit merkle tree
        uint256 userBuyCount = isLimit ?  _userBought[3][msg.sender] + _mintAmount : 0;
        require(isWhitelisted(msg.sender, proof,  userBuyCount, 3), "XanaLand: Not whitelisted");
        
        _userBought[3][msg.sender] += _mintAmount;
        //check user purchase amount
        require(balance >= SafeMath.mul(calculateDiscount(_size,rarity),_mintAmount));
        mintNFT(_mintAmount, rarity, _size);
        emit _mintLand(msg.sender, rarity, _size, _mintAmount,balance,true);
    }
   


    //Mint Common Category
    function mintLand(string memory rarity,uint256 _mintAmount, uint256 _size, bytes32[] calldata proof, bool isLimit, uint256 whitelistType) checkSupply(_mintAmount,_size, rarity) external payable nonReentrant{
        require(rates[_size][rarity].valid, "XanaLand: Plot size not valid" );
        uint256 balance = SafeMath.mul(msg.value,10 ** 18);
        ...
        if(status == 1) require(isWhitelisted(msg.sender, proof,  userBuyCount, whitelistType), "XanaLand: Not whitelisted");
        ...
        //Minting Land NFTs as per sent amount above
        mintNFT(_mintAmount, rarity, _size);
        emit _mintLand(msg.sender, rarity, _size, _mintAmount,balance,false);
    }
    ...
    function freeMint(uint256 _mintAmount,  bytes32[] calldata proof) checkSupply(_mintAmount, 1, "Common") external{
        ...
        uint256 userBuyCount =  _userBought[4][msg.sender] + _mintAmount;
        require(isWhitelisted(msg.sender, proof,  userBuyCount, 4), "XanaLand: Not whitelisted");
        _userBought[4][msg.sender] += _mintAmount;
        
        //Minting Land NFTs as per sent amount above
        mintNFT(_mintAmount, rarity, _size);
        emit _mintCommon(msg.sender, rarity, _size, _mintAmount,0,false);
    }
    // internal
    ...
    function isWhitelisted(address account, bytes32[] calldata proof, uint256 quantity, uint256 typeWhitelist) public view returns (bool) {
        return _verify(_leaf(account, quantity), proof, whitelistRoot[typeWhitelist]);
    }
    ...
    //only owner methods
    //set whitelist
    function setWhitelistRoot(bytes32 newWhitelistroot, uint256 typeWhitelist) public onlyOwner {
        whitelistRoot[typeWhitelist] = newWhitelistroot;
    }
