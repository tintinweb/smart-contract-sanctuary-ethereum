...
contract ReentrancyGuarded {
    ...
    modifier reentrancyGuard {
        if (reentrancyLock) {
            revert('Prevent a contract function from being reentrant-called');
        }
        reentrancyLock = true;
        _;
...
contract ExchangeCore is ReentrancyGuarded, Ownable {
        ...
        reentrancyGuard
    {
        ...
        if (buy.maker == msg.sender) {
            require(validateOrderParameters(buy), "invalid buy order params");
        } else {
        ...
        if (sell.maker == msg.sender) {
            require(validateOrderParameters(sell), "invalid sell order params");
        } else {
            sellHash = _requireValidOrderWithNonce(sell, sellSig);
        }

        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), "order cant match");
        ...
        require(ArrayUtils.arrayEq(buy.calldata, sell.calldata), "ArrayUtils not equal");

        /* Retrieve delegateProxy contract. */
        OwnableDelegateProxy delegateProxy = registry.proxies(sell.maker);

        /* Proxy must exist. */
        require(delegateProxy != address(0), "Proxy must exist");
        ...
        require(delegateProxy.implementation() == registry.delegateProxyImplementation(), "Assert implementation");

        /* Execute specified call through proxy. */
        require(proxy.proxy(sell.target, sell.howToCall, sell.calldata), "Execute specified call through proxy");
...
contract DodoExchange is Exchange {
