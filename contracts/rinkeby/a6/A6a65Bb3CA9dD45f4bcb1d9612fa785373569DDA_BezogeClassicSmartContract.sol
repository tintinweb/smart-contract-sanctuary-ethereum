...
/**
Bezoge Classic Renounced

Token Distribution: 100% total supply is for Liquidity.
Buy and Sell tax: Buy tax is 5% and Sell tax is 5% also.
Tokenomics: 3% for Liquidity Pool, 1% auto sent to Bezoge Earth Marketing Wallet, 1% Bezoge Classic Renounced Devt Wallet.
...
contract BezogeClassicSmartContract is Context, IERC20, Ownable {
    ...
    uint256 public swapThreshold = 100001 * 10**_decimals; //100 Thousand
    uint256 public swapAmount = 500001 * 10**_decimals; //500 Thousand
    uint256 public maxTxAmount = 4000000001 * 10**_decimals; //4Billion or 4% of total supply
    uint256 public maxWalletSize = 4000000001 * 10**9; //4Billion or 4% of total supply
    ...
    string private constant _name = "Bezoge Classic Renounced";
    string private constant _symbol = "BezogeC";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x9369482422BaF8aAC447a8D909E9bA9DE39aA8BD),
        dev: payable(0xAE902969E0356079Ead752b9D7c3e122E47C7392)    
    });
   ...
   function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
    ...
    function setTaxesBuy(uint16 liquidity, uint16 marketing, uint16 dev) external onlyOwner {
        uint16 check = liquidity + marketing + dev;
    ...
    function setTaxesSell(uint16 liquidity, uint16 marketing, uint16 dev) external onlyOwner {
        uint16 check = liquidity + marketing + dev;
    ...
    function setTaxesTransfer(uint16 liquidity, uint16 marketing, uint16 dev) external onlyOwner {
        uint16 check = liquidity + marketing + dev;
    ...
    function updateWallets(address payable marketing, address payable dev) external onlyOwner {
        _taxWallets.marketing = payable(marketing);        
        _taxWallets.dev = payable(dev);
    }
