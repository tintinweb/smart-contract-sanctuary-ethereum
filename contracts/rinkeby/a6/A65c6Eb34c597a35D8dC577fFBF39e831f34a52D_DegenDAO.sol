...
contract DegenDAO is ERC20, Ownable {
  ...
  function _swap(uint256 _amountToSwap) private lockSwap {
    ...
    _handlePotentialBalanceTaxMismatch();
  }
  ...
  function _handlePotentialBalanceTaxMismatch() internal {
    uint256 _latestBalance = balanceOf(address(this));
    uint256 _latestDesiredBal = tokensForGambling + tokensForLiquidity;
    if (_latestDesiredBal != _latestBalance) {
      if (_latestDesiredBal > _latestBalance) {
        tokensForGambling = tokensForGambling > _latestBalance
          ? 0
          : tokensForGambling;
        tokensForLiquidity = _latestBalance - tokensForGambling;
      } else {
        tokensForLiquidity = _latestBalance - tokensForGambling;
      }
    }
