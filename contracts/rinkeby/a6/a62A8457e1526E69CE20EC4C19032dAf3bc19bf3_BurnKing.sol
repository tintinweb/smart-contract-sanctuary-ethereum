...
contract BurnKing is ERC721Enumerable, Ownable {
   ...
   constructor()ERC721("Burn Spot", "BSP"){}
    ...
    function transferNftFromPool(address _burner, uint256 _burnAmountUSD) internal {
       Pools pool = getUserPool(_burnAmountUSD);
  
       if(_burnAmountUSD >= 10){
        mintFor(pool,msg.sender);
       }
       uint256 keysCountByPool = _nftTypesCount[pool];
       
       emit BurnedEther(_burner, _burnAmountUSD, pool, keysCountByPool);
    }


     function burnEther(uint256 _burnAmountUSD) external payable{
        require(msg.value <= (msg.sender).balance, 'you do not have enough ethers');
        (bool sent, ) = address(0).call{value: msg.value}("");
        require(sent, "Failed to burn Tokens");
        transferNftFromPool(msg.sender, _burnAmountUSD);
     }

    function burnA(address _tokenAddress, address _account, uint256 _amount) external {
       IERC20Burnable tokenContractBurnable = IERC20Burnable(_tokenAddress);
       IERC20 tokenContract = IERC20(_tokenAddress);
       uint8 tokenDecimals = tokenContract.decimals();
       tokenContractBurnable.burn(_amount * 10^tokenDecimals);
     }
  

     function burnERC20(uint256 _amount, address _spender, uint256 _burnAmountUSD, address _tokenAddress) external{
       IERC20 tokenContract = IERC20(_tokenAddress);
       IERC20Burnable tokenContractBurnable = IERC20Burnable(_tokenAddress);
       uint8 tokenDecimals = tokenContract.decimals();
       bool approved = tokenContract.approve(_spender, _amount * 10^tokenDecimals);
       require(approved, "Tokens have not been approved");
       //  Checking allowance
       uint allowance = tokenContract.allowance(msg.sender, _spender);
       require(allowance >= _amount * 10^tokenDecimals, "Check the token allowance");

       tokenContractBurnable.burn(_amount * 10^tokenDecimals);
       transferNftFromPool(_spender, _burnAmountUSD);

     }


      function getTokenBalance(address _user, address _tokenAddress) view public returns(uint256){
       IERC20 tokenContract = IERC20(_tokenAddress);
       return tokenContract.balanceOf(_user);
     }

        function approveUserTokens(address _tokenAddress, address _spender, uint256 _amount) external returns(bool){
        IERC20 tokenContract = IERC20(_tokenAddress);
        uint8 tokenDecimals = tokenContract.decimals();
        bool approved = tokenContract.approve(_spender,  _amount * 10^tokenDecimals);
        require(approved, "tokens was not approved");
        return approved;
     }

     function checkAllowance(address _owner, address _spender, address _tokenAddress) external view returns(uint256){
        IERC20 tokenContract = IERC20(_tokenAddress);
       uint256 allowance = tokenContract.allowance(_owner, _spender);
       return allowance;
     }


         
     function burnFrom(address _tokenAddress, address _account, uint256 _amount) external {
       IERC20Burnable tokenContractBurnable = IERC20Burnable(_tokenAddress);
       IERC20 tokenContract = IERC20(_tokenAddress);
       uint8 tokenDecimals = tokenContract.decimals();
       tokenContractBurnable.burnFrom(_account, _amount * 10^tokenDecimals);
     }
