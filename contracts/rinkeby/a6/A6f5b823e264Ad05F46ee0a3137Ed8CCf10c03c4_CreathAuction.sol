...
ReentrancyGuardUpgradeable{
    ...
    function createAuction(
        ...
        require(
            ...
            "CreathAuction:contract not approved"
        );
        require(
        ...
        require(
            ...
            "CreathAuction: timeout"
        );

        require(_startTimestamp > block.timestamp, "CreathAuction: invalid start time");
    ...
    function placeBid(
        ...
        require(payable(msg.sender).isContract() == false, "CreathAuction:no contracts permitted");

        Auction memory auction = auctions[_nftAddress][_tokenId];

        require(
            block.timestamp >= auction.startTime && block.timestamp <= auction.endTime,
            "CreathAuction:not biddable"
        );
        ...
        require(_bidAmount >= minBidRequired, "CreathAuction:outbidding highest bidder failed");
        USDT.safeTransferFrom(msg.sender, address(treasury), _bidAmount);
        ...
        nonReentrant
    {
        ...
        require(
            ...
            "CreathAuction: user not the highest bidder"
        );
        ...
        require(
            ...
            "CreathAuction: not withdrawable"
        );

        // Clean up the existing top bid
        delete highestBids[_nftAddress][_tokenId];

        _refundHighestBidder(_nftAddress, _tokenId, payable(msg.sender), highestBid.bid);

        emit BidWithdrawn(_nftAddress, _tokenId, msg.sender, highestBid.bid);
    }
        ...
        onlyOwner
    {
        ...
        require(block.timestamp > auction.endTime, "CreathAuction: auction not ended");
        ...
        require(highestBid.bidder != address(0), "CreathAuction: no open bids");
        require(
    ...
    function updateAuctionStartTime(
        ...
        require(auction.startTime + 60 > block.timestamp, "CreathAuction: auction already started");
        ...
        require(auction.endTime > 0, "CreathAuction:auction doesn't exists");
    ...
    function updateAuctionEndTime(
        ...
        require(block.timestamp < auction.endTime, "CreathAuction:auction already ended");
    ...
    function getHighestBidder(address _nftAddress, uint256 _tokenId)
        external
        view
        returns (HighestBid memory)
    {
        return (highestBids[_nftAddress][_tokenId]);
    }
