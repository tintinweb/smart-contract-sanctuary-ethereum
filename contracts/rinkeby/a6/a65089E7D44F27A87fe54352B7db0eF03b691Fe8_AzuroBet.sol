    ...
    IAzuroBet
{
    ...
    mapping(uint256 => address) private idToCore;
    string public baseURI;

    uint256 public lastBetID; //start from 1, incremented on every mint
    ...
    function mint(address account, address core) external override OnlyLP {
        lastBetID++;
        idToCore[lastBetID] = core;
        super._mint(account, lastBetID);
    }
    ...
    function getLastTokenId() external view override returns (uint256 tokenId) {
        return lastBetID;
    }

    function getCoreByTokenId(uint256 tokenId)
        external
        view
        override
        returns (address core)
    {
        return idToCore[tokenId];
    }
}
