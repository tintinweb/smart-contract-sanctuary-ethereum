...
contract MergeMana is Ownable {
  ...
  function getChanneledMana(uint256 id) public view returns (uint256) {
    uint totalPenalties = getTotalManaPenalties(id);
  ...
  function getBoostedMana(uint256 id) public view returns (uint256) {
    ...
    if (totalPenalties > channeledMana[id]) {
      uint leftOverManaPenalty = totalPenalties - channeledMana[id]; 
  ...
  function getInherentMana(uint256 id) public view returns (uint256) {
    ...
    uint lowerMana = boostedMana[id] + channeledMana[id]; 
    uint totalPenalties = getTotalManaPenalties(id);
    uint inherentManaWithNoPenalties = uint256(uint8(baseManaStorage.indexToData(id)[0])) *
      baseManaMultiplier;

    if (totalPenalties > lowerMana) {
      uint leftOverManaPenalty = totalPenalties - lowerMana; 
  ...
  function _channelMana(uint256 from, uint256 to) internal {
    uint256 inherentManaAndBoostedMana = getInherentMana(from) + getBoostedMana(from);
  ...
  function getBoostManaRitualAmount(address booster) public view returns (uint256) {
