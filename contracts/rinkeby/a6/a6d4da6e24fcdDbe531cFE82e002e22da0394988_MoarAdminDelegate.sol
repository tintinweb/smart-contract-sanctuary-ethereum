...
import "../../submodules/moar/contracts/moar/Moar.sol";

contract MoarAdminDelegate is Ownable {
    Moar public immutable moar;
    ...
    function authorize(address authorization, bool valid) external onlyOwner {
        authorizations[authorization] = valid;
    }
     ...
     */
    function lockTransfers (uint256[] memory tokenIds, bool[] memory locks) external onlyAuthorized {
        moar.lockTransfers(tokenIds, locks);
    }

    function ownerOf(uint256 tokenId) external view returns (address) {
        return moar.ownerOf(tokenId);
    }
}
