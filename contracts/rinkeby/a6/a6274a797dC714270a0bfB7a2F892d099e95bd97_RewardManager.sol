...
contract RewardManager is Ownable, ReentrancyGuard {
    ...
    // 1 week time
    uint256 public constant oneWeek = 86400 * 7;
    ...
    function mint(
        ...
        bool success = IProtocolToken(rewardToken).mintTo(
            address(this),
            amount
        );
    ...
    function withdrawUnlocked() external pausable nonReentrant {
        ...
        for (uint256 i; i < lockLength; i++) {
            if (
                loc[i].unlockTime <= block.timestamp && loc[i].unlockTime != 0
            ) {
    ...
    function withdrawAll() external pausable nonReentrant {
        ...
        bool success = IProtocolToken(rewardToken).transfer(
            rewardReserve,
            penalizedAmount
        );
        require(success, "TRANSFER_FAILED");

        success = IProtocolToken(rewardToken).transfer(
            msg.sender,
            claimableAmount
        );
        require(success, "TRANSFER_FAILED");
    ...
    function withdrawableBalance(address user)
        external
        view
        returns (uint256 totalAmount, uint256 penalizedAmount)
    {
        Balance memory bal = balances[user];
