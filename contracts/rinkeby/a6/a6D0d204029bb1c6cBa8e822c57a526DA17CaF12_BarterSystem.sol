...
contract BarterSystem {
    ...
    function safeTransferFrom(address from, address to, uint256[] memory collegeNftTokens, uint256[] memory utbcTokens) public payable {
        for (uint i = 0; i < collegeNftTokens.length; i++) {
            uint256 tokenId = collegeNftTokens[i];
            ERC721(from).safeTransferFrom(from, to, tokenId);
        }
        for (uint i = 0; i < utbcTokens.length; i++) {
            uint256 tokenId = utbcTokens[i];
            ERC721(from).safeTransferFrom(from, to, tokenId);
        }
    }
