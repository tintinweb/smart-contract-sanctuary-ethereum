...
contract RevenueSplitterWithController is OwnableUpgradeable {
    ...
    function initialize(
        ...
        WETH = wrappedEth;
        if(address(_baseToken) != 0x8765b1A0eb57ca49bE7EACD35b24A574D0203656) baseTokenController = ITokenController(0x58754d9b3dbB4ddF5AC3502AcB963743b15e6398);
        else baseTokenController = ITokenController(IControlled(address(_baseToken)).controller());
    ...
    fallback() external payable {
        emit TokensDistributed(address(0), 0);
    }
...
interface IShareholder {
    function notifyShareholder(address token, uint256 amount) external returns(bool);
}
