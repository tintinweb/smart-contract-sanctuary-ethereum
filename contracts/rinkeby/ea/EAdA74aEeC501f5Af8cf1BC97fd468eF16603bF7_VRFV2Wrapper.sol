...
contract VRFV2Wrapper is ConfirmedOwner, TypeAndVersionInterface, VRFConsumerBaseV2, VRFV2WrapperInterface {
  ...
  function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
    ...
    // Give the user change
    uint256 change = calculateRequestPriceInternal(
      callback.callbackGasLimit,
      callback.requestGasPrice,
      callback.requestWeiPerUnitLink
    ) - calculateRequestPriceInternal(gasUsed, callback.requestGasPrice, callback.requestWeiPerUnitLink);
    if (change > 0) {
      LINK.transfer(callback.callbackAddress, change);
    }
  }
