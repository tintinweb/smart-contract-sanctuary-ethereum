...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accGrayPerShare; // Accumulated CAKEs per share, times 1e12. See below.
    }
    ...
    // GRAY tokens created per block.
    uint256 public grayPerBlock;
    ...
    // The block number when GRAY mining starts.
    ...
    function add(
        ...
        poolInfo.push(
            PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accGrayPerShare: 0})
        );
    }
    ...
    function pendingGray(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accGrayPerShare = pool.accGrayPerShare;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount * pool.accGrayPerShare / 1e12 - user.rewardDebt;
...
contract GrayToken is ERC20("GrayToken", "GRAY"), Ownable {
