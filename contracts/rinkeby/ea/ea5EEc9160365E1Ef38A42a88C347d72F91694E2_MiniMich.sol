    ...
    NativeMetaTransaction
{
    ...
    uint32 private constant MAX_RESERVE = 199;
    uint32 public RESERVE_COUNT = 0;
    ...
    modifier allocTokens(uint32 numToMint) {
        require(
            _tokenIdCounter.current() + numToMint <=
                (MAX_NFT - (MAX_RESERVE - RESERVE_COUNT)),
    ...
    // ** ADMIN ** //
    // *********** //
    function reserve(address[] calldata receivers) public onlyOwner {
        require(
            _tokenIdCounter.current() + receivers.length < MAX_NFT,
            "not enough."
        );
        require(
            RESERVE_COUNT + receivers.length <= MAX_RESERVE,
            "no more reserves."
        );
