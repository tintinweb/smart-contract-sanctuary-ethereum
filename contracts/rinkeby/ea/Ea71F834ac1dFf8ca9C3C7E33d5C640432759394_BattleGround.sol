...
contract BattleGround is ERC721URIStorage, VRFConsumerBase{
    ...
    struct Request{
        bytes32 requestIdToCharacter;
        bytes32 requestIdToAddon;
        uint tokenID;
        BoxType boxType;
        uint date;
        GeneratedNumbers generatedNumbers;
        uint randomNumberToCharacter;
        uint randomNumberToAddon;
    }
    ...
    mapping (uint => string[]) public listofCommonChars;
    mapping (uint => string[]) public listofEpicChars;
    mapping (uint => string[]) public listofLegendaryChars;

    BoxType private memoryBox;
    NumberAddons private memoryAddon; 
    string private memoryJson;
    ...
    event NftPurchase(uint token_0, uint addon_token_0, uint token_1, uint addon_token_1, uint token_2, uint addon_token_2, address buyer);
    event ChangedProbabilityBox(uint box, uint[4] probability);
    event RequestNumberToToken(uint token, bytes32 requestID_1, bytes32 requestID_2);
    event ReceivedRandomNumber(bytes32 requestID, uint randomNumber);
    event ExecutedRequest(Request request);
    event EmittedToken(uint token, uint randomNumberToCharacter, uint indexChar, uint randomNumberToAddon, uint scaledNumber, uint addon);


    constructor() payable ERC721("BattleGround","BTGE") VRFConsumerBase(vrfCoordinator,link) {
    ...
    function buyBox(BoxType boxType) external payable boxCost(boxType) returns (uint[3] memory acquiredTokens ) { 
        ...
        for(uint i = 0; i < 3;i++){
            ...
            Request memory request = Request( requestRandomness(keyhash, fee), requestRandomness(keyhash, fee),
                tokenCounter + i, boxType, block.timestamp,GeneratedNumbers.None, 0, 0);  
            requests.push(request);
            emit RequestNumberToToken(tokenCounter + i, request.requestIdToCharacter, request.requestIdToAddon);
    ...
    function generateAddons(BoxType box) internal view returns (uint[3] memory addonsGenerated, uint[3] memory numbersGenerated){
        
        uint[3] memory randomNumbers = generateThreeRandomNumbers();
        uint[3] memory aux = [randomNumbers[0],randomNumbers[1],randomNumbers[2]];
    ...
    function setTokenuriToTest(string memory tokenURI, BoxType box, NumberAddons numberAddon ) external onlyOwner {
        require(uint8(box) <= 2);
        require(uint8(numberAddon) <= 4);
        
        memoryJson = tokenURI;
        memoryBox = box;
        memoryAddon = numberAddon; 
        
        //modifies token 0 of test
        _setTokenURI(0, tokenURI);
    }


    function getMemoryInfoToken0() external view onlyOwner returns(string memory tokenURI, BoxType box, NumberAddons numberAddon)  {

        return(memoryJson, memoryBox, memoryAddon); 
    }

    function setMemoryInfoToSpecArray() external onlyOwner {
        
        string memory empty = "";
        require(!(compareTwoStrings(memoryJson,empty)),"TokenURI cannot be empty");  
        
        if(uint8(memoryBox)==0) 
            listofCommonChars[uint8(memoryAddon)].push(memoryJson);
        else if(uint8(memoryBox)==1)
            listofEpicChars[uint8(memoryAddon)].push(memoryJson); 
        else if(uint8(memoryBox)==2)
            listofLegendaryChars[uint8(memoryAddon)].push(memoryJson);

        memoryJson = ""; 
        memoryBox = BoxType.Common; 
        memoryAddon = NumberAddons.Zero; 
    }

    function compareTwoStrings(string memory a, string memory b) internal pure returns(bool equal){
        ...
        override
    {
        emit ReceivedRandomNumber(_requestId, _randomness);

        uint index;
        bool generatedAllNumbers =  false;

        for(uint i=0; i<requests.length; i++){
            if(requests[i].requestIdToAddon == _requestId || requests[i].requestIdToCharacter == _requestId){

                if(requests[i].generatedNumbers == GeneratedNumbers.None){
                    requests[i].generatedNumbers = GeneratedNumbers.One;
                    
                    if(_requestId == requests[i].requestIdToCharacter)
                        requests[i].randomNumberToCharacter = _randomness;
                    else
                        requests[i].randomNumberToAddon =  _randomness;
                    
                    break;
                }
                else if(requests[i].generatedNumbers == GeneratedNumbers.One){ 
                    requests[i].generatedNumbers = GeneratedNumbers.Two;
        ...
        if(generatedAllNumbers == true)
        {
            emit ExecutedRequest(requests[index]);

            BoxType boxType = requests[index].boxType;
            
            uint aux = requests[index].randomNumberToAddon;
            requests[index].randomNumberToAddon %= 200;
            uint[4] memory probability = boxToProbabilityArray(boxType);
            uint addons = numberToAddon(requests[index].randomNumberToAddon, probability);
            uint index_char;

            if(boxType==BoxType.Common){
                index_char = requests[index].randomNumberToCharacter%listofCommonChars[addons].length;
                _setTokenURI(requests[index].tokenID, listofCommonChars[addons][index_char]);
            }
            else if(boxType==BoxType.Epic){
                index_char = requests[index].randomNumberToCharacter%listofEpicChars[addons].length;
                _setTokenURI(requests[index].tokenID, listofEpicChars[addons][index_char]); 
            }
            else if(boxType==BoxType.Legendary){
                index_char = requests[index].randomNumberToCharacter%listofLegendaryChars[addons].length;
                _setTokenURI(requests[index].tokenID, listofLegendaryChars[addons][index_char]);
            }

            emit EmittedToken(requests[index].tokenID, requests[index].randomNumberToCharacter, index_char,
                aux, requests[index].randomNumberToAddon, addons);
