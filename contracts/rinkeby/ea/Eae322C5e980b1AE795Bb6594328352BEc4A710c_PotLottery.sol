...
contract PotLottery is Initializable, ReentrancyGuardUpgradeable {
    ...
    event TokenSwapError(string tokenName);
    ...
    function updateTokenUsdValues(string[] memory _tokenNames, uint256[] memory _valuesInUsd) public onlyAdmin {
        require(_tokenNames.length == _valuesInUsd.length, 'No of address is not equal to number of usd values');
    ...
    function setMinimumUsdEntranceFee(uint256 _minimumUsdEntrance) public onlyAdmin {
    ...
    function setAddressToRecieveFee(address _addressToRecieveFee) public onlyAdmin {
    ...
    function EnterPot(
        ...
        if ((potLiveTime + potDuration) <= block.timestamp && (participants.length > 1)) {
            calculateWinner();
        }
    ...
    function calculateWinner() public {
        if ((potLiveTime + potDuration) <= block.timestamp && (participants.length > 1)) {
            ...
            for (uint256 index = 0; index < tokensInPotNames.length; index++) {
                payAccount(tokensInPotNames[index], pot_winner, tokenTotalEntry[tokensInPotNames[index]]);
    ...
    function getRefund() public {
        if (timeBeforeRefund + potStartTime < block.timestamp && participants.length == 1) {
            if (potStartTime == 0) return;
            string memory tokenWithThehighestUsdValue = getPotTokenWithHighestValue();
            ...
            tokenTotalEntry[tokenWithThehighestUsdValue] =
                ...
                amountToPayAsFees;
            for (uint256 index = 0; index < tokensInPotNames.length; index++) {
                payAccount(tokensInPotNames[index], participants[0], tokenTotalEntry[tokensInPotNames[index]]);
            }
            startNewPot();
        }
    }

    function deductAmountToPayAsFees(string memory _tokenName, uint256 _value) private {
    ...
    function removeAccumulatedFees() public onlyAdmin {
        for (uint256 index = 0; index < adminFeeToken.length; index++) {
            payAccount(adminFeeToken[index], addressToRecieveFee, adminFeeTokenValues[adminFeeToken[index]]);
            delete adminFeeTokenValues[adminFeeToken[index]];
        }
        delete adminFeeToken;
    }

    function getAmountToPayAsFees(address _address) private view returns (uint256) {
        string memory tokenWithThehighestUsdValue = getPotTokenWithHighestValue();
        ...
        return ((userBalance / PLSP_Standard) >= 1) ? 0 : (baseFee - ((userBalance / PLSP_Standard) * baseFee));
    }

    function getPotTokenWithHighestValue() private view returns (string memory) {
    ...
    /**
     * @dev Swaps accumulated fee into PLS first, and then to PLSP
     * FIXME currently converts all the balance to PLSP - this should be accumulated fees
     */
    function swapAccumulatedFees() external validPLSP nonReentrant {
        ...
        for (uint256 i = 0; i < adminFeeToken.length; i++) {
            // This is for real-running lottery
     ...
     */
    function burnAccumulatedPSLP() external validPLSP {
        PLSP PLSPToken = PLSP(PLSP_Address);
        uint256 PLSP_Balance = PLSPToken.balanceOf(address(this));
        uint256 PLSP_Fee = adminFeeTokenValues['PLSP'];

        require(PLSP_Balance > 0, 'No PLSP balance');
        require(PLSP_Fee <= PLSP_Balance, 'Wrong PLSP Fee Value');

        PLSPToken.burn(PLSP_Fee);
        adminFeeTokenValues['PLSP'] = 0;
    }
