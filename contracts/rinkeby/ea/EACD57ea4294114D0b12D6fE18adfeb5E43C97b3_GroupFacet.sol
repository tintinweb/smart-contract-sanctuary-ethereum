...
library LibGroup {
    ...
    function _join(
        ...
        LibOwnership._addOwner(member);

        emit Joined(member);

        _tryEnactFormingProposition(proposition, signatures);
    }

    /// @dev Decodes `data` and passes it to `_acquireMore`
    /// `data` must be encoded `JoinData` struct
    function _acquireMoreDecode(bytes memory data) internal {
        JoinData memory joinData = abi.decode(data, (JoinData));
    ...
    function _acquireMore(
        ...
        LibOwnership._acquireMoreOwnershipUnits(member);

        emit AcquiredMore(member);
    ...
    function _tryEnactFormingProposition(
        IWallet.Proposition memory proposition,
        bytes[] memory signatures
    ) internal {
        ...
        if (LibOwnership._isCompletelyOwned()) {
            ...
            bool successful = LibWallet._enactProposition(
                proposition,
                signatures
            );
            require(successful, "Group: forming proposition reverted");
        }
    }

    /// @notice Internal member leaves
    /// @dev `member` will be refunded with his join deposit
    /// Emits `Left` event
...
library LibWallet {
    ...
    function _enactProposition(
        ...
        return _executeTransaction(proposition.tx);
    }
}
