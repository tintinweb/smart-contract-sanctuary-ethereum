...
contract Reel8NFT is Ownable, ERC721 {
     ...
     */
    function buy(uint256 tokenQuantity,  uint256 valueSent, uint256 mintId, bytes memory signature) external payable {
        require(_state != State.PAUSE, "Sale is not active");

        // validate the transaction with the backend
        address recovered = ECDSA.recover(keccak256(abi.encodePacked(tokenQuantity, valueSent, mintId, msg.sender)), signature);
