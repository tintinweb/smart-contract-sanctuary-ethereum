...
contract IGO is Ownable {
        ...
        external
    {
        ...
        require(minterRole[msg.sender] || owner()==msg.sender, "not minter role or owner");
        ...
        box.whitelist_address=whitelist_address;

        //Save list item
        require(nft_id_list.length > 0, "empty nft list");
        require(_checkOwnership(nft_id_list, nft_address), "now owner");
        ...
        {
            IERC721(nft_address).transferFrom(msg.sender, address(this), nft_id_list[i]);
        }
        ...
        external
    {
        ...
        //Get properties box 
        address nft_address = box.nft_address;
        uint256[] memory nft_id_list = box.nft_id_list;

        //Transfer nft box to creator
        for (uint256 i = 0; i < nft_id_list.length; i++) {
            address nft_owner = IERC721(nft_address).ownerOf(nft_id_list[i]);
            require(address(this) == nft_owner, "not nft owner");
            IERC721(nft_address).transferFrom(address(this), msg.sender,nft_id_list[i]);
        }
        //set box
        box.canceled = true;
        ...
        payable
    {
        ...
        uint256 total;
        //Check wihtelist
        if(box.whitelist_address != address(0))
        {
            require(IMember(box.whitelist_address).listMember(msg.sender),"Not member");
        } 
        //Check amount, if amount > total will set amount equal total
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            require(address(this) == IERC721(nft_address).ownerOf(token_id));
            //Transfer NFT
            IERC721(nft_address).transferFrom(address(this), msg.sender, token_id);
            box.purchased_nft[msg.sender].push(token_id);
            nft_transfered++;
            //Remove this NFT id from the list
            _removeNftId(box, token_index);
        ...
        
        {
            ...
            if (payment_token_address == address(0)) {
                ...
                if (eth_to_refund > 0) {
                     payable(msg.sender).transfer(eth_to_refund);
                }
            }
            else 
            {
                IERC20(payment_token_address).transferFrom(msg.sender, address(this), total_payment);
            }
        ...
        //========================================================//
        //=========-  Claim payment and withdraw item -===========//
        //========================================================//
    function claimPaymentAndWithdrawNFT(uint256[] calldata box_ids) external {
        for (uint256 asset_index = 0; asset_index < box_ids.length; asset_index++) {
            ...
            uint256 total = box.nft_id_list.length;
            address nft_address = box.nft_address;
            uint256[] memory nft_id_list = box.nft_id_list;
            ...
            //Withdraw nft 
            if(total != 0){
                for(uint256 nft_index=0; nft_index< total; nft_index++)
                {
                    address nft_owner = IERC721(nft_address).ownerOf(nft_id_list[nft_index]);
                    require(address(this) == nft_owner, "not nft owner");
                    IERC721(nft_address).transferFrom(address(this), msg.sender,nft_id_list[nft_index]);
                }
                box.nft_id_list=new uint256[](0); //clear list nft
                emit WithdrawNFT(msg.sender,box_ids[asset_index],nft_address,block.timestamp);
            }
            ...
            for (uint256 token_index = 0; token_index < box.payment.length; token_index++) {
                ...
                if (amount == 0)  continue; //skip the remaining block of code
        ...
        )
    {
        ...
        whitelist_address= box.whitelist_address;
    }
        //========================================================//
        //==================-  Get box status -===================//
        //========================================================//
        ...
        returns(uint256[] memory nft_id_list)
    {
        ...
        //=================-  Set up minter -=====================//
        //========================================================//
    function addMinterRole(address[] memory addrs) onlyOwner external {
        for (uint256 i = 0; i < addrs.length; i++) {
            minterRole[addrs[i]] = true;
        }
    }

    function removeMinterRole(address[] memory addrs) onlyOwner external {
        for (uint256 i = 0; i < addrs.length; i++) {
            minterRole[addrs[i]] = false;
        }
    }
    ...
    function _checkOwnership(uint256[] memory nft_token_id_list, address nft_address)
        internal
        view
        returns(bool)
    {
    ...
    function _removeNftId(Box storage box, uint256 index) private {
...
interface IMember{
    function listMember(address _addMember) external returns(bool);
