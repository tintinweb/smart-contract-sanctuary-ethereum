...
error TokenOutCannotBeZeroAddr();
error RequirementLengthMismatch();
error EmptyRequirements();
error AlreadyRedeemed();
error SweepingTransferFailed();
error CriteriaNotMet();
error CriteriaLengthMismatch();

/**
  @title A contract for redeeming NFTs depending on ownership of tokens listed
  in a mapping of collection addresses
  @author 0xthrpw
  @author Tim Clancy

  This contract allows for multiple configurations of redemption 'rounds' that
  require holding one or more tokens that are store in the mapping
  'redemptionCriteria'.  A round can be configured to allow qualifying users to
  mint one or more tokens ('amountOut') from a collection ('tokenOut') for a given amount
  ('price') of a ERC20 token ('payingToken').

  @dev the collection specified as 'tokenOut' must specify this contract as a
  'minter' or allow similar privileges

  @dev users must sign an approval allowing this contract to spend the specified
  ERC20 token ('payingToken') on their behalf before performing a redemption

  March 10th, 2022.
*/

contract ImpostorsRedeemer721 is Ownable, ReentrancyGuard {
  using SafeERC20 for IERC20;

  /// A configuration for a token redemption.
  ...
  /// collection out => round => config
  mapping(uint256 => RedemptionConfig) public redemptionConfigs;

  /// round => token address => token id of redeemed token
  mapping (uint256 => mapping (address => mapping (uint256 => bool))) public redeemed;

  event TokenRedemption(
    uint256 indexed round,
    uint256 tokenId,
    address indexed user,
    address indexed collectionIn
  );

  event ConfigUpdate(
    uint256 indexed round,
    uint256 price,
    address indexed payingToken,
    address indexed tokenOut
  );

  /**
    Redeem a token for n number of tokens in return.  This function parses the
    tokens round id, determines the appropriate exchange token and amount,  and
    mints the receipt token(s)

    @param _round The round of the token being redeemed
  ...
  */
  function redeem(
    ...
    if(_tokenIds.length != criteria.length){
      revert CriteriaLengthMismatch();
    }
    //check balance
    for(uint256 i; i < criteria.length; i++){
      if(redeemed[_round][criteria[i]][_tokenIds[i]]){
        revert AlreadyRedeemed();
      }

      if(ITiny721(criteria[i]).ownerOf(_tokenIds[i]) != msg.sender){
        revert CriteriaNotMet();
      }
      //set token id as redeemed for this token and round
      redeemed[_round][criteria[i]][_tokenIds[i]] = true;
      emit TokenRedemption(
        _round, _tokenIds[i], msg.sender, criteria[i]);
    }
    ...
    //mint
    ITiny721(config.tokenOut).mint_Qgo(msg.sender, config.amountOut);

  }

  /**
    Configure redemption amounts for each round.  a balance of at least one of
    each tokensIn results in amountOut number of _tokenOut tokens

    @param _round The round ID of the token being received
  ...
  */
  ...
  function setConfig(
    ...
    if(_data.tokenOut == address(0)){
      revert TokenOutCannotBeZeroAddr();
    }

    if(_criteria.length == 0){
      revert EmptyRequirements();
    }

    redemptionConfigs[_round] = _data;
    //address[] memory tins = new address[](_data.tokensIn.length);
    for(uint256 i = 0; i < _criteria.length; i++){
      redemptionCriteria[_round].push(_criteria[i]);
    }

    emit ConfigUpdate(
      _round, _data.price, _data.payingToken, _data.tokenOut);
  }
  ...
  */
  ...
  /**
    Allow the owner to burn a particular ERC-20 token from the contract.

    @param _token The token to burn
    @param _amount The amount of token to sweep. If amount is zero, then entire
      contract balance is burned.
  */
  function burnToken (
    address _token,
    uint256 _amount
  ) external onlyOwner {
    uint256 amount = (_amount > 0) ? _amount : IERC20(_token).balanceOf(address(this));
    IBlood(_token).burn(amount);
  }
}
