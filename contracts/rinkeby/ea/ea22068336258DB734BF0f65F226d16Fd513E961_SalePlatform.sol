...
contract SalePlatform is ContinuousDutchAuction, ReentrancyGuard, Auth {
    ...
    constructor(
        ...
        address authority,
        address authorizer
    ) Auth(admin, Authority(authority)) {
    ...
    function authorizedUnlockWithKey(
        ...
        if (signer != _authorizer) revert("PURCHASE:INVALID SIGNATURE");
        ...
        UnlockSale memory sale = keySales[mintAuth.dropId];
        if (msg.value != sale.price) revert("PURCHASE:INCORRECT MSG.VALUE");
        _unlockWithKey(mintAuth.keyId, mintAuth.dropId, variant, sale);
    }
    ...
    function _unlockWithKey(
        ...
        require(
            keyUnlocks.dropSupply(dropId) < sale.maxDropSupply,
            "PURCHASE:NO MORE AVAILABLE"
        );
        require(
            sale.numOfVariants > 0 &&
                variant > 0 &&
                variant < sale.numOfVariants + 1,
    ...
    function unlockWithKey(
        ...
        _unlockWithKey(keyId, dropId, variant, sale);
    }
    ...
    function createUnlockSale(
        ...
        keySales[_nextUnlockDropId++] = UnlockSale(
            price,
            start,
            period,
            artist,
        ...
        for (uint256 i = 0; i < enabledKeyRanges.length; i++)
            keySales[_nextUnlockDropId - 1].enabledKeyRanges.push(
                enabledKeyRanges[i]
            );
    }
