...
contract Akamir is Ownable, ERC721A, ReentrancyGuard {
  ...
  function auctionMint(uint256 quantity, uint256 lockUntil) external payable callerIsUser {
    ...
    require(_saleLimit!=0 && _auctionSaleLimit !=0 &&_saleStartTime != 0 &&_saleStartPrice != 0  && block.timestamp >= _saleStartTime, "auction sale has not begun yet");
    require(
  ...
  function allowlistMint(uint256 quantity, uint256 lockUntil) external payable callerIsUser {
    ...
    require(saleLimit != 0 && price != 0 && startTime!=0 && block.timestamp > startTime, "allowlist sale has not begun yet");
  ...
  function isPublicSaleOn(
    ...
    return publicPriceWei != 0 && publicSaleKey_ != 0 && saleLimit != 0 && publicSaleStartTime !=0 && block.timestamp >= publicSaleStartTime;
  }
  ...
  function endAuctionAndSetupMintlistSaleInfo(
    uint256 mintlistPriceWei,
    uint32 mintlistStartTime
  ) external onlyOwner {
    saleConfig.auctionSaleStartTime = 0;
  ...
  function endMintlistAndSetupPublicSaleInfo(
    uint256 publicPriceWei,
    uint32 publicSaleStartTime
  ) external onlyOwner {
