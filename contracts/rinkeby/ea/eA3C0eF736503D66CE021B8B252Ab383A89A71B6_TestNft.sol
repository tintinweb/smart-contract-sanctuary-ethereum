...
contract TestNft is ERC721 {
    ...
    mapping(uint => string) public animationUrls;
    ...
    function mint(uint256 tokenId, string memory animationUrl) public onlyAdmin {
        animationUrls[tokenId] = animationUrl;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked('{"name":"', name(), '", "animation_url":"', animationUrls[tokenId], '"}')
                    )
                )
            )
        );
