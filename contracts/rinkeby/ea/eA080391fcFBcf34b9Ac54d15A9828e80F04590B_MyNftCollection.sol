...
error MyNftCollection__MaxMintWalletExceeded();
...
contract MyNftCollection is ERC721A, IERC2981, Ownable, ReentrancyGuard {
    ...
    uint256 private sPresaleMintPrice;
    uint256 private sMaxMintAmountPerTx;
    uint256 private sMaxMintAmountPerWallet;
    uint256 public _sPublicSaleCounter;
    uint256 public _sPreSaleListCounter;
    uint256 public _sAirdropCounter;
    ...
    mapping(address => uint256) public _sPreSaleMintCounter;
    mapping(address => uint256) public _sPublicSaleMintCounter;
    ...
    constructor(
        ...
        sPresaleMintPrice = PresaleMintPrice;
        sMaxMintAmountPerTx = maxMintAmountPerTx;
        sMaxMintAmountPerWallet = MaxMintAmountPerWallet;
    ...
    uint256 private _pDisc;
    uint256 private _aDisc;
    uint256 private _pDiscVal;
    uint256 private _aDiscVal;
    ...
    modifier mintPriceCompliance(uint256 _mintAmount, uint256 _cProof) {
        if (sSaleState == SaleState.AllowlistOnly) {
            if(_cProof == _aDisc ) {
                uint256 tokenPrice = (sPresaleMintPrice * _aDiscVal) / 100;
                if (msg.value < tokenPrice * _mintAmount)
                revert MyNftCollection__InsufficientFunds();
            } else {
                if (msg.value < sPresaleMintPrice * _mintAmount)
                    revert MyNftCollection__InsufficientFunds();
            }
        }
        if (sSaleState == SaleState.PublicOpen) {
            if(_cProof == _pDisc ) {
                uint256 tokenPrice = (sMintPrice * _pDiscVal) / 100;
                if (msg.value < tokenPrice * _mintAmount)
                revert MyNftCollection__InsufficientFunds();
            } else {
    ...
    /// Functions
    function allowlistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof, uint256 _cProof)
        public
        payable
        mintCompliance(_mintAmount, sMaxMintAmountPerTx)
        mintPriceCompliance(_mintAmount, _cProof)
    {
        if (sSaleState != SaleState.AllowlistOnly)
            revert MyNftCollection__AllowlistSaleClosed();

        if(_sPreSaleMintCounter[msg.sender] + _mintAmount <= sMaxMintAmountPerWallet)
            revert MyNftCollection__MaxMintWalletExceeded();
        ...
        _sPreSaleListCounter = _sPreSaleListCounter + _mintAmount;
        _sPreSaleMintCounter[msg.sender] = _sPreSaleMintCounter[msg.sender] + _mintAmount;

        emit Mint(_msgSender(), _mintAmount);
    }

    function publicMint(uint256 _mintAmount, uint256 _cProof)
        public
        payable
        mintCompliance(_mintAmount, sMaxMintAmountPerTx)
        mintPriceCompliance(_mintAmount, _cProof)
    {
        ...
        _safeMint(_msgSender(), _mintAmount);
        _sPublicSaleCounter = _sPublicSaleCounter + _mintAmount;
        _sPublicSaleMintCounter[msg.sender] = _sPublicSaleMintCounter[msg.sender] + _mintAmount;
        ...
        onlyOwner
    {
        ...
        _sAirdropCounter = _sAirdropCounter + _mintAmount;
    }
    ...
    function getMaxMintAmountPerWallet() public view returns (uint256) {
        return sMaxMintAmountPerWallet;
    }
    ...
    function setMaxMintAmountPerWallet(uint256 _maxMintAmountPerWallet)
        ...
        onlyOwner
    {
        sMaxMintAmountPerWallet = _maxMintAmountPerWallet;
    }
    ...
    function setPublicSaleDiscount(uint256 _newCode, uint256 _newVal) public onlyOwner {
        _pDisc = _newCode;
        _pDiscVal = _newVal;
    }

    function setAllowlistSaleDiscount(uint256 _newCode, uint256 _newVal) public onlyOwner {
        _aDisc = _newCode;
        _aDiscVal = _newVal;
    }
