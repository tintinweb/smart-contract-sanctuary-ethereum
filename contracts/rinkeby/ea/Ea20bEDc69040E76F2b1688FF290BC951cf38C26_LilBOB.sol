...
contract LilBOB is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    Counters.Counter public _tokenId;
    Counters.Counter public _tokenIdVIP;
    Counters.Counter public _giveaways;

    uint256 public constant MAX_NFT = 6; //This is full collection should be 1111
    uint256 public constant MAX_NFT_WL = 2;
    uint256 public MAX_NFT_VIP = 2; // This is for VIP should be 68
    uint256 public constant MAX_NFT_GIVEAWAY = 2; // This is for giveaways should be 143


    uint256 public price = 0.1 ether; //0.1 Ether
    uint256 public WL_price = 0.088 ether; //0.088 Ether

    uint256 public MAX_SALE_MINTS_PER_ADDRESS = 10;
    uint256 public MAX_PRESALE_MINTS_PER_ADDRESS = 10;
    ...
    constructor(address adminSigner, string memory baseURI) ERC721("Lil BOB", "LilBOB") {
        _adminSigner = adminSigner;
        setBaseURI(baseURI);
        _tokenId.set(MAX_NFT_VIP); // Starts at 68 since there are 68 VIP NFTs 
    }
    ...
    function setFixedURI(uint256 newFixedURI) external onlyOwner{
        MAX_NFT_VIP = newFixedURI;
    }
    ...
    function mintNFTTokens(uint256 _count) public payable {
        ...
        require(
            _tokenId.current() + MAX_NFT_GIVEAWAY - _giveaways.current() + _count <= MAX_NFT,
            "Transaction will exceed maximum supply of Lil BOB"
        );
        require(
    ...
    //mint NFTTokens
    function mintNFTTokensGiveaway(uint256 _count) public onlyOwner {
        require(
            _count > 0,
            "Min 1 token can be minted"
        );
        require(
            _giveaways.current() + _count <= MAX_NFT_GIVEAWAY,
            "Transaction will exceed maximum supply of Lil BOB giveaways"
        );

        address _to = msg.sender;

        for (uint256 i = 0; i < _count; i++) {
            _giveaways.increment();
            _mint(_to);
        }
    }

    function mintNFTTokensVIP(uint256 _count) public onlyOwner {
        require(
            _count > 0,
            "Min 1 token can be minted"
        );
        require(
            _tokenIdVIP.current() + _count <= MAX_NFT_VIP,
            "Transaction will exceed maximum VIP supply of Lil BOB"
        );
    ...
    function _mintVIP(address _to) private {
        _tokenIdVIP.increment();
        uint256 tokenId = _tokenIdVIP.current();
    ...
    function random(uint256 timestamp, uint256 tokenId) public view returns (uint256) {
        // uint256 newTokenId = uint256(keccak256(abi.encodePacked(x))) % (MAX_NFT - MAX_NFT_VIP);
        uint256 newTokenId = (uint256(keccak256(abi.encodePacked(timestamp))) + tokenId) % (MAX_NFT - MAX_NFT_VIP) + MAX_NFT_VIP + 1;
        return newTokenId;
    }
        ...
        returns (string memory)
    {
        ...
        if (bytes(baseURI).length > 0) {
            if(tokenId > MAX_NFT_VIP) {
                uint256 newTokenId = random(REVEAL_TIMESTAMP, tokenId);
                // uint256 newTokenId = (uint256(keccak256(abi.encodePacked(REVEAL_TIMESTAMP))) + tokenId) % (MAX_NFT - MAX_NFT_VIP);
                _tokenURI = string(abi.encodePacked(baseURI, newTokenId.toString()));
            } else {
    ...
    function mintPresale(uint256 _count, Coupon memory _coupon) external payable {
        ...
        require(
            _tokenId.current() + _count <= MAX_NFT_WL + MAX_NFT_VIP,
