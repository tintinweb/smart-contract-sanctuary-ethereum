...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    function swapMultiple(
        ...
        require(
            msg.value > 0
                ? tokensToBeSwappedLen == _amountsOutMin.length - 1
        ...
        if (msg.value > 0) {
            require(
                msg.value >= _currentEtherToSwap, //TODO: should it be == or >= and then return excesse ?
        ...
        for (uint256 i = 0; i < tokensToBeSwappedLen; ++i) {
            require(totalTokensAttributed[_tokensIn[i]] >= 0, "Not enough amount attributed");
            _swap(
                _tokensIn[i],
                totalTokensAttributed[_tokensIn[i]],
