...
contract Demo is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    using MerkleProof for bytes32[];
    using ECDSA for bytes32;

    //CONSTANTS
    uint256 public constant PRE_SALE_PRICE = 0.006 ether;
    uint256 public constant PUBLIC_SALE_PRICE = 0.008 ether;
        ...
        nonReentrant
    {
        ...
        require(
            block.timestamp > freeClaimStartTime,
            "free claim hasn't started"
        );
    ...
    function mintPreSale(bytes32[] memory proof, uint256 amount)
        external
        payable
        nonReentrant
    {
        ...
        require(!disabled, "the contract is disabled");
        require(
            proof.verify(_merkleRoot, keccak256(abi.encodePacked(msg.sender))),
            "failed to verify merkle root"
        );
        require(
            amount <= MAX_PURCHASE,
        ...
        require(
            PRE_SALE_PRICE * amount == msg.value,
    ...
    function buy(string memory nonce, uint256 amount)
        external
        payable
        nonReentrant
    {
    ...
    function setFreeClaimStartTime(uint256 _freeClaimStartTime)
        external
        onlyOwner
    {
