...
contract NemoLand is Admin, ERC721Upgradeable {
    ...
    function mintLand(
        ...
        if (state > 0) {
            ...
            _safeMint(msg.sender, state);
            for (counter = 0; counter < lands.length; counter++) {
                landToState[lands[counter]] = state;
            }
        } else {
            for (counter = 0; counter < lands.length; counter++) {
                _safeMint(msg.sender, lands[counter]);
            }
        }
        emit LandMint(state, lands, referral);
    }
    ...
    function isValidRequest(
        ...
        bool isValid = _validSignature(
            _signature,
            keccak256(abi.encode(_state, _id, _zoneId, msg.sender, referral))
        );
        return isValid;
    }
