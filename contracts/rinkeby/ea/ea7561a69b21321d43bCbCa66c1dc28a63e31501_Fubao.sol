...
contract Fubao is ERC721A, Ownable {
    // metadata
    string public baseURI;

    // collection info
    uint256 public immutable collectionSize;
    uint256 public immutable perAddressMaxMintAmount;

    // reserved for the team, for marketing etc.
    ...
    constructor(
        ...
        require(
            reservedAmount_ <= collectionSize_,
            "reserved amount must be smaller than collection size"
        );
        require(
            publicAvailableAmount_ <= collectionSize_ - reservedAmount_,
            "can not set this public available amount"
        );
        baseURI = baseURI_;
    ...
    function setPublicConfig(
        ...
        require(
            ...
            "can not set this reserved amount"
        );
        require(
    ...
    function mint(
        ...
        require(
            ...
            "not enough to support desired mint amount"
        );
        require(
            _numberMinted(_msgSender()) + amount <= perAddressMaxMintAmount,
            "can not mint this many"
        );
        uint256 whiteListRemainAmount = getWhiteListRemainAmount(
            _msgSender(),
        ...
        if (whiteListRemainAmount == 0) {
            ...
            require(
                ...
                "white list mint has not started yet"
            );
    ...
    function getWhiteListRemainAmount(
        ...
        uint256 mintedAmount = whiteListMintedMap[user];
        require(
            totalAmount >= mintedAmount,
            "white list total amount must be larger than minted"
        );
        require(
            MerkleProof.verify(
                merkleProof,
            ...
            "white list merkle proof verify fail"
        );
    ...
    function _publicMint(uint256 amount) private {
        ...
        _setRefundData(amount, publicPrice);
        _safeMint(_msgSender(), amount);
        emit PublicMint(_msgSender(), amount, publicPrice);
    }

    function _whiteListMint(uint256 amount) private {
        publicMintedAmount += amount;
        whiteListMintedAmount += amount;
        whiteListMintedMap[_msgSender()] += amount;
        _setRefundData(amount, whiteListPrice);
        _safeMint(_msgSender(), amount);
        emit WhiteListMint(_msgSender(), amount, whiteListPrice);
    }

    function _setRefundData(uint256 amount, uint256 price) private {
    ...
    function airdrop(address user, uint256 amount) public onlyOwner {
        require(
            ...
            "not enough to support desired airdrop amount"
        );
        reservedMintedAmount += amount;
        _safeMint(user, amount);
        emit Airdrop(_msgSender(), amount);
    }

    function airdropList(address[] memory userList, uint256[] memory amountList)
        public
        onlyOwner
    {
        require(
            userList.length == amountList.length,
            "user list does not match amount list length"
        );
    ...
    function refund(uint256[] calldata tokenIds) public callerIsUser {
        uint256 refundValue;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            transferFrom(_msgSender(), refundAddress, tokenId);
        }
        refundedAmount += tokenIds.length;
        payable(_msgSender()).transfer(refundValue);
        emit Refund(_msgSender(), refundValue, tokenIds);
    }
    ...
    function withdraw() public onlyOwner {
        require(block.timestamp >= refundLastEndTime, "can not widthdraw");
