...
contract YourNftToken is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  modifier mintCompliance(uint256 _quantity) {
    require(_quantity > 0 && _quantity <= maxMintAmountPerTx, 'Invalid mint amount!');
  ...
  function mint(uint256 _quantity) public payable mintCompliance(_quantity) mintPriceCompliance(_quantity) {
    require(!paused, 'The contract is paused!');

    _safeMint(_msgSender(), _quantity);
  }
  
  function mintForAddress(uint256 _quantity, address _receiver) public mintCompliance(_quantity) onlyOwner {
