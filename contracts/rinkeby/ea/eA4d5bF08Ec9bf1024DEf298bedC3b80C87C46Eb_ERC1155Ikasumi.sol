...
contract ERC1155Marketplace is ERC1155YuzuInternal, ReentrancyGuard {
        ...
        ERC1155YuzuInternal(_owner, uri, name, symbol, _anyoneCanMint)
    {
        ...
        admin = _admin;
        require(_commissionRate<=100, "ERC1155Ikasumi: Commission rate has to be between 0 and 100");
    ...
    function buy(uint256 tokenId, uint256 index, uint256 qty) public nonReentrant {

        // check for a valid index position
        require(index < openSales[tokenId].length, "ERC1155Ikasumi: wrong index");

        // Cannot buy more items than available in the offer
        require(openSales[tokenId][index].qty >= qty, "ERC1155Ikasumi: not enough items for sale");

        // Cannot buy more items than the amount the owner have
        require(balanceOf(openSales[tokenId][index].user, tokenId) >= qty, "ERC1155Ikasumi: the seller doesn't have enough items for sale");
    ...
    function cancelSale(uint256 tokenId, uint256 index) external {
        ...
        require(openSales[tokenId][index].user == _msgSender(), "ERC1155Ikasumi: only the original seller can cancel his sales");
    ...
    function cancelBid(uint256 tokenId, uint256 index) external nonReentrant {
        ...
        // check for a valid index position
        require(index < openOffers[tokenId].length, "ERC1155Ikasumi: wrong index");

        // Only the original bidder can cancel his bids
        require(openOffers[tokenId][index].user == _msgSender(), "ERC1155Ikasumi: only the original bidder can cancel his bids");
    ...
    function acceptBid(uint256 tokenId, uint256 index) external nonReentrant {
        ...
        require(balanceOf(_msgSender(), tokenId) >= openOffers[tokenId][index].qty, "Ikasumi: you do not have enough tokens to sell");
         ...
         */ 
        distributeFunds(openOffers[tokenId][index].qty * openOffers[tokenId][index].price, openOffers[tokenId][index].user, _msgSender(), tokenId, false);
...
contract ERC1155Ikasumi is ERC1155Marketplace, EIP712 {
    ...
    function redeem(
        ...
        require(chainId == getChainId(), "ERC1155Ikasumi: Wrong chain");
        require(vouchersMintedQty[id] + qtyToMint <= qty, "ERC1155Ikasumi: the qty exceeds the amount available for minting");
    ...
    function verify(
        ...
        require(recoveredSigner == signer, "ERC1155Ikasumi: wrong signature");
    }
