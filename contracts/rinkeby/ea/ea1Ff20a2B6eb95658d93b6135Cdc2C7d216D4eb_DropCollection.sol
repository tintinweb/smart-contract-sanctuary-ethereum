...
IBaseCollection
{
    ...
    IClassicKit internal _classicKit;
    ...
    function __BaseCollection_init(
        ...
        _classicKit = IClassicKit(_msgSender());
        _treasury = treasury_;
    ...
    function withdraw() external {
        ...
        uint256 fees = _classicKit.getFees(address(this));
        if (fees > 0) {
            _classicKit.addFeesClaimed(fees);
            AddressUpgradeable.sendValue(payable(address(_classicKit)), fees);
        }
        uint256 remaining = balance.sub(fees);
        if (remaining > 0) {
