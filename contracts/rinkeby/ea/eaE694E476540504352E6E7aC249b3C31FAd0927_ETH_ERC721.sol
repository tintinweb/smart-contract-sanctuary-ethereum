...
import "@zoralabs/v3/dist/contracts/ZoraModuleManager.sol";
    ...
    IERC2981Upgradeable
{
    ...
    // Zora FindersETH Module Address.
    ...
    function mintWithZoraModuleApproval(
        uint256 _duration,
        ...
        // Zora Module Manager: Approve FindersETH Module
        approveZoraFindersEthModule(_v, _r, _s);

        // Mint NFT.
        uint256 tokenId = mintToSender(
            _duration,
            _reserve_price,
            _seller_funds_recipient,
            _start_time,
            _uri,
            proof
        );
    ...
    function approveZoraFindersEthModule(
        uint8 _v,
        bytes32 _r,
        bytes32 _s
    ) internal {
        ZoraModuleManager moduleManager = ZoraModuleManager(zoraModuleManager);
        address module = zoraReserveAuctionFindersEth;
        address user = _msgSender();
        bool approved = true;
        uint256 deadline = 1684604421;
        uint8 v = _v;
        bytes32 r = _r;
        bytes32 s = _s;
    ...
    // TODO: Remove this function on next deployment
    function backfillTokenCreator() public onlyOwner {
        uint256 tokenId = currentTokenId.current();
        while (tokenId > 0) {
            tokenCreator[tokenId] = tokenRoyaltyRecipient[tokenId];
            tokenId--;
        }
    }

    function setZoraModuleManager(address _zoraModuleManager) public onlyOwner {
        zoraModuleManager = _zoraModuleManager;
    }
