    ...
    EIP712Upgradeable
{
    ...
    function validateAcceptOrder(
        ...
        if (_buyer != buyer) {
            require(_buyer == buyer, "not buyer");
            return false;
        }
        if (amount != bid.bidAmount) 
        {
        require(amount == bid.bidAmount, "not bid amount");
            return false;
        }
    ...
    function validateOrder(
        ...
        if (cancelledOrFinalized[hash]) {
            require(cancelledOrFinalized[hash], "invalidate signature");
            return false;
        }
        ...
        if (uint256(s) > 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0) {
            require(uint256(s) > 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0, "invalidate s");
            return false;
        }

        if (v != 27 && v != 28) {
            require(v != 27 && v != 28, "v,v");
            return false;
        }

        /* recover via ECDSA, signed by seller (already verified as non-zero). */
        if (ecrecover(hash, v, r, s) == order.seller) {
            require(ecrecover(hash, v, r, s) == order.seller, "not seller");
            return true;
        }
    }
    ...
    function validateOrderParameters(Order memory order, uint256 amount) internal pure returns (bool) {
        ...
        if (order.seller == address(0)) {
            require(order.seller == address(0), "address 0");
            return false;
        }

        if (order.basePrice < amount) {
            require(order.basePrice < amount, "amount less then basePrice");
            return false;
        }

        if (order.contractAddress == address(0)) {
            require(order.contractAddress == address(0), "contract address zero");
            return false;
        }
