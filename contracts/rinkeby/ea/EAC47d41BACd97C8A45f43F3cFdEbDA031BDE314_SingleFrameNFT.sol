...
contract SingleFrameNFT is ERC721Upgradeable {
    ...
    event SingleItemCreated(uint256 id, address creator, string uri, uint256 royalty);
    ...
    function initialize(
        ...
        __ERC721_init("SingleFrameNFT", "SFN");
        factory = _msgSender();
        owner = creator;
	 ...
	 */
    function addItem(string memory _tokenURI, uint256 royalty, uint256 mintFee) external payable returns (uint256){
        require(owner == msg.sender, "Only minter can add item");

        require(msg.value >= mintFee, "insufficient mint fee");
        if (mintFee > 0)payable(feeAddress).transfer(mintFee);
        currentID = currentID.add(1);        
        _safeMint(msg.sender, currentID);
        Items[currentID] = Item(currentID, msg.sender, _tokenURI, royalty);
        emit SingleItemCreated(currentID, msg.sender, _tokenURI, royalty);
        return currentID;
    }
