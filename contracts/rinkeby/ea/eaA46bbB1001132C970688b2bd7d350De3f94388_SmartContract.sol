...
contract SmartContract is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "Rv2";
    ...
    constructor () {
        ...
        rewardWalletMaxToken = 1000;
        qualifiedTokensForReward = 100;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        // //Check if recipient is qualified for rewards
        if(_tOwned[recipient]>=qualifiedTokensForReward
            && recipient != feeAddrWallet 
            && recipient != rewardWallet
        ){
            insertToQualifiedAddresses(recipient);
        }

        // //Check if rewards wallet is ready for reflection
        uint256 countQualifiedAddress = 0;
        ...
        if(_tOwned[rewardWallet] >= rewardWalletMaxToken){
            ...
            uint256 shareToken = _tOwned[rewardWallet]/countQualifiedAddress;
    ...
    function insertToQualifiedAddresses(address Address) internal returns(bool){
        if(Address == feeAddrWallet)return false;
        if(Address == rewardWallet)return false;

        for(uint i = 0; i<qualifiedAddresses.length;i++){
            if(Address == qualifiedAddresses[i]){
                return false;
            }
        }
        qualifiedAddresses.push(Address);       
        return true;
    }
