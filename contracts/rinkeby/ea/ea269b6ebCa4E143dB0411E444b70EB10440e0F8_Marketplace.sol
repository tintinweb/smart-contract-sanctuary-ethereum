...
import "./erc721/EhisabERC721.sol";
import "./erc1155/EhisabERC1155.sol";
    ...
    EIP712Upgradeable
{
    ...
    function _deduceRoyalties1155(
        uint256 tokenId,
        uint256 grossSaleValue,
        address buyer,
        EhisabERC1155 tokenContract1155
    ) internal returns (uint256 netSaleAmount) {
        // Get amount of royalties to pays and recipient
        (address royaltiesReceiver, uint256 royaltiesAmount) = tokenContract1155
            .royaltyInfo(tokenId, grossSaleValue);
        // Deduce royalties from sale value
        uint256 netSaleValue = grossSaleValue - royaltiesAmount;
        // Transfer royalties to rightholder if not zero
    ...
    function finaliseFixedPriceAuction721(
        ...
        EhisabERC721 tokenContract721 = EhisabERC721(tokenContract);
        weth = IERC20(_wethAddr);

        uint256 feeAmt = (otherParams[1] * feePercent) / 10000;
        ...
        returns (bool)
    {
        ...
        uint256 tokenId = otherParams[0];

        uint256 feeAmt = (bidAmount * feePercent) / 10000;
        ...
        tokenContract721.safeTransferFrom(seller, buyer, tokenId);
        emit LogOrderFinalised721(otherParams[2], tokenId, buyer, seller);
        return true;
    }

    function finaliseFixedPriceAuction1155(
        address _owner,
        /**
				0:_tokenId,1:_minPrice,2:_auctionType,3:_quantity,4:endTime,5:salt
				 */
        ...
        EhisabERC1155 tokenContract1155 = EhisabERC1155(tokenContract);
        weth = IERC20(_wethAddr);

        uint256 totalAmount = otherParams[1] * quantity;
        uint256 tokenId = otherParams[0];

        uint256 feeAmt = (totalAmount * feePercent) / 10000;
        uint256 netSalePrice = _deduceRoyalties1155(
            otherParams[0],
            totalAmount,
            _msgSender(),
            tokenContract1155
        ...
        tokenContract1155.safeTransferFrom(
            seller,
            _msgSender(),
            otherParams[0],
            quantity,
            bytes(tokenContract1155.uri(tokenId))
        );
        emit LogOrderFinalised1155(
            otherParams[2],
            otherParams[0],
            _msgSender(),
            seller,
    ...
    function finaliseERC1155Auction(
        address _owner,
        /**
				0:_tokenId,1:_minPrice,2:_auctionType,3:_quantity,4:endTime,5:salt
				 */
        ...
        address tokenContract,
        address _wethAddr
    ) public OnlySeller(_owner) AuctionTimeIsOver(otherParams[4]) {
        uint256 auctionId = otherParams[2];
        address buyer = _buyer;
        uint256 quantity = _quantity;
        require(
            auctionId == 2 || auctionId == 3,
            "Only Unlimited and Timed Auctions can be Finalised"
        );
        uint256 tokenId = otherParams[0];
        address seller = verifySignatureAndQuantityAvailable(
        ...
        uint256 totalAmount = bidAmount * quantity;
        uint256 feeAmt = (totalAmount * feePercent) / 10000;
        ...
        emit LogOrderFinalised1155(auctionId, tokenId, buyer, seller, quantity);
    }
}
...
contract GenericAccessControl is AccessControlUpgradeable {
    ...
    modifier OnlyAdmin() {
		...
		modifier OnlyAllowedContract(address contractAddr){
			require(msg.sender == contractAddr,"Only allowed contract can call this function");
			_;
		}
...
import "./ERC721AccessControl.sol";
import "../../libraries/ERC2981PerTokenRoyalties.sol";

contract EhisabERC721 is
    Initializable,
    LazyNFTERC721,
    ERC2981PerTokenRoyalties
{
    ...
    function mint(string memory _currTokenURI, uint256 royaltyValue)
        public
        returns (uint256)
    {
    ...
    function batchMint(
        string[] memory _currTokenURI,
        uint256[] memory royaltyValue
    ) public returns (uint256) {
        for (uint8 i = 0; i < _currTokenURI.length; i++) {
            require(
                bytes(_currTokenURI[i]).length != 0,
                "Token URI cannot be empty"
            );
            _tokenIds.increment();
            _mint(_msgSender(), _tokenIds.current());
            _setTokenURI(_tokenIds.current(), _currTokenURI[i]);
            _setTokenRoyalty(
                _tokenIds.current(),
                _msgSender(),
                royaltyValue[i]
...
import "./ERC1155AccessControl.sol";
import "../../libraries/ERC2981PerTokenRoyalties.sol";

contract EhisabERC1155 is
    Initializable,
    LazyNFTERC1155,
    ERC2981PerTokenRoyalties
{
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(LazyNFTERC1155, ERC2981PerTokenRoyalties)
        returns (bool)
    {
        return
            interfaceId == type(IERC2981).interfaceId ||
    ...
    function createToken(
        ...
        _setTokenURI(_tokenIds.current(), string(_tokenURI));
        _setTokenRoyalty(_tokenIds.current(), msg.sender, royalty);
        emit TokenCreated(
            _tokenIds.current(),
            bytes(_tokenURI),
            _initialSupply,
    ...
    function distributeTokens(
        address[] memory transferAddr,
        uint256[] memory ids,
        uint256[] memory quantity
    ) public returns (bool) {
        require(
            transferAddr.length == ids.length,
            "Addresses and tokenIds length mismatch"
        );
        require(
            transferAddr.length == quantity.length,
            "Addresses and quntity length mismatch"
        );
        for (uint i = 0; i < transferAddr.length; i++) {
            require(
                transferAddr[i] != address(0),
                "Cannot transfer to a zero address"
            );
            require(
                balanceOf(msg.sender, ids[i]) >= quantity[i],
                "Insufficient balance"
            );
            string memory tokenuri = uri(ids[i]);
            safeTransferFrom(
                msg.sender,
                transferAddr[i],
                ids[i],
...
contract ERC2981PerTokenRoyalties is ERC165Upgradeable, IERC2981 {
    ...
    function _setTokenRoyalty(
        ...
        require(value <= 3000, "ERC2981Royalties: Too high");
...
import "../AccessControl.sol";
import "../../libraries/ERC1155URIStorageUpgradeable.sol";

contract LazyNFTERC1155 is
    Initializable,
    ...
    GenericAccessControl
{
    ...
    function __LazyMintERC1155_init(
        string memory _newBaseURI,
        string memory signDomain,
        string memory signVersion,
        address payable minter,
    ...
    struct NFTVoucher {
        //The minimum price (in wei) that the NFT creator is willing to accept for the initial sale of this NFT.
        uint256 minPrice;
        //The quantity of NFT to be transferred
        uint256 quantity;
        //The metadata URI to associate with this token.
        string uri;
				//Royalty for the minted token
				uint256 royalty;
        //the EIP-712 signature of all other fields in the NFTVoucher struct. For a voucher to be valid, it must be signed by an account with the MINTER_ROLE.
        bytes signature;
    }
    ...
    /// @param redeemer The address of the account which will receive the NFT upon success.
    /// @param voucher A signed NFTVoucher that describes the NFT to be redeemed.
    function redeem(address redeemer, NFTVoucher memory voucher,uint256 tokenId)
        internal
        returns (uint256)
    {
        // make sure signature is valid and get the address of the signer
        ...
        require(msg.value >= voucher.minPrice, "Insufficient funds to redeem");
        // first assign the token to the signer, to establish provenance on-chain
        _mint(signer, tokenId, voucher.quantity, bytes(voucher.uri));
        _setTokenURI(tokenId, voucher.uri);
    ...
    /// @notice Returns a hash of the given NFTVoucher, prepared using EIP712 typed data hashing rules.
    /// @param voucher An NFTVoucher to hash.
    function _hash(NFTVoucher memory voucher)
        internal
        view
        returns (bytes32)
    {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "NFTVoucher(uint256 minPrice,uint256 quantity,string uri,uint256 royalty)"
                        ),
                        voucher.minPrice,
												voucher.quantity,
                        keccak256(bytes(voucher.uri)),
 ...
 */
abstract contract ERC1155URIStorageUpgradeable is Initializable,ERC1155SupplyUpgradeable {
    function __ERC1155URIStorage_init() internal initializer {
        __Context_init_unchained();
        __ERC165_init_unchained();
        __ERC1155URIStorage_init_unchained();
    }

    function __ERC1155URIStorage_init_unchained() internal initializer {
    }
    using StringsUpgradeable for uint256;
