...
contract SmartContract is ERC721A, ReentrancyGuard, Ownable {
    ...
    uint256 private ATMOS_GENESIS_MAX_SUPPLY = 6942;
    IERC721Enumerable public immutable ATMOS_GENESIS;
    ...
    constructor(string memory _initBaseURI, string memory _initNotRevealedUri, address _atmosGenesis ) ERC721A ("ProjectAtmos", "Atmos") {

        setBaseURI(_initBaseURI);
        setNotRevealedURI(_initNotRevealedUri);

        ATMOS_GENESIS = IERC721Enumerable(_atmosGenesis);
    ...
    // Claim Function ====================================================================
    function tokensOfOwner(address owner, uint256 max_loop) public view returns(uint256){
        uint256 tokenCount = ATMOS_GENESIS.balanceOf(owner);
        if (tokenCount == 0){
            ...
            for (tid; tid <= max_loop; tid++){
                if (ATMOS_GENESIS.ownerOf(tid) == owner){
    ...
    function isClaimQualified(address _userAddr) public view returns (bool){
        uint256 balanceOfOwner = ATMOS_GENESIS.balanceOf(_userAddr);
        if (balanceOfOwner != 0){
            return true;
        }
        return false;
    ...
    function claim() external payable{
        ...
        require(isClaimQualified(t4), "Claimer is not an atmos holder.");

        uint256 numTokensClaimed;
        for (uint256 i; i < ATMOS_GENESIS.balanceOf(t4); i++){
            // uint256 tokenId = ATMOS_GENESIS.tokenOfOwnerByIndex(t4, i);
            numTokensClaimed++;
        }

        _safeMint(t4, ATMOS_GENESIS.balanceOf(t4));
