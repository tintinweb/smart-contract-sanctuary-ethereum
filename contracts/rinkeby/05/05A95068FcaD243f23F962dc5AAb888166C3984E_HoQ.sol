...
contract HoQ is ContextMixin, ERC721A, NativeMetaTransaction, Ownable {
    using SafeMath for uint256;

    string public contractURI;

    uint256 private MAX_SUPPLY = 120;
    uint256 private MAX_PUBLIC_SUPPLY = 100;
    ...
    uint private DEVELOPER_PERCENTAGE = 2;

    uint256 public mintPricePublic = 0.06 ether;

    bool private developerWithdrawlInProgress = false;
    ...
    string private _name = "Test Collection";
    ...
    modifier onlyDeveloper {
        require(
            ...
            "Ownable: caller is not the dev"
        );
        _;
    }

    constructor(string memory _ghostTokenURI, address _developer) ERC721A(_name, _symbol) {
    ...
    function mint(uint256 count) public payable mintHasStarted {
        require(msg.value == (count * mintPricePublic), "Wrong amount");
        require(count > 0 && count <= 8, "Wrong amount");
        buyAmount(count);
        developerBalance += msg.value * DEVELOPER_PERCENTAGE / 100;
    }
    ...
    function withdrawDeveloper() external onlyDeveloper {
        require(!developerWithdrawlInProgress, "Dev Withdraw already in progress");
        
        developerWithdrawlInProgress = true;
        
        payable(developer).transfer(developerBalance);
        developerBalance = 0 ether;

        developerWithdrawlInProgress = false;
    }
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    baseTokenURI,
                    Strings.toString(_tokenId + 1), // TODO: awaiting https://github.com/chiru-labs/ERC721A/pull/66
                    ".json"
                )
            );
    }
