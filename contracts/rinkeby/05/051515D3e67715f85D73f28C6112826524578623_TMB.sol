...
contract TMB is ERC721A, Ownable {
    enum Status {
        Pending,
        EarlyBirdSale, 
        PreSale,
    ...
    uint8 public maxEvolutionLevel = 0;
    uint8 public maxBranchNumber = 0;
    ...
    function setTokenStatus(uint256 tokenId, uint8 level, uint8 branch) external callerIsUser{
    ...
    */
    ...
    function setBaseURIs(uint8 level, uint8 branch, string calldata newBaseURIs) external onlyOwner {
        baseURIs[level][branch] = newBaseURIs;
        emit BaseURIsChanged(level, branch, newBaseURIs);
    }
