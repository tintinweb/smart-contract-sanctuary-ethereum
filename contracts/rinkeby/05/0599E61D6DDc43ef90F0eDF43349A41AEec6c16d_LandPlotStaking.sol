...
contract LandPlotStaking is Ownable,ERC1155Receiver {
    ...
    function claimRewards(uint256[] memory collectionIds) public
    {
        ...
        rewardToken.transfer(msg.sender,royaltyAmount);

    }


     function checkclaimRewards(uint256[] memory collectionIds,address account) public view returns (uint256[] memory amounts)
    {
        uint256 totalClaimableAmount;
        uint256[] memory amounts =  new uint[](collectionIds.length);
        for(uint256 i =0;i<collectionIds.length;i++)
