...
contract ClearingHouseViewer is IClearingHouseViewer {
    ...
    function marginRatio(address account) public view override returns (int256) {
        ...
        for (uint256 i = 0; i < clearingHouse.getNumMarkets(); i++) {
            (int256 traderUnrealizedPositionPnl, int256 traderOpenNotional) = clearingHouse
                .perpetuals(i)
                .getTraderPositionHealth(account);

            (int256 lpUnrealizedPositionPnl, int256 lpOpenNotional) = clearingHouse.perpetuals(i).getLpPositionHealth(
                account
            );

            fundingPayments += getTraderFundingPayments(i, account) + getLpFundingPayments(i, account);
        ...
        int256 collateral = getReserveValue(account);
    ...
    function getReserveValue(address account) public view override returns (int256) {
        return clearingHouse.vault().getReserveValue(account);
    }
