...
contract InternetMade is ERC721, Ownable, ReentrancyGuard {
    ...
    function mintWhitelistedFF(
        ...
        require(
            ...
            "You are not FF whitelisted"
        );
        require(
            balanceOf(msg.sender) + _mintAmount <= maxMintAmountWhitelistFF,
            "Max allowed mint amount exceeded for FF whitelist"
        );
        require(
            msg.value >= costWhitelistedFF * _mintAmount,
    ...
    function mintWhitelisted(uint8 _mintAmount, bytes32[] calldata _merkleProof)
        external
        payable
    {
        ...
        require(
            balanceOf(msg.sender) + _mintAmount <= maxMintAmountWhitelist,
    ...
    function mintPublicSale(uint8 _mintAmount) external payable {
        ...
        require(
            ...
            "Max allowed mint amount exceeded for PublicSale"
        );
        require(
    ...
    function _mint(uint8 _mintAmount) internal {
        ...
        require(
            totalSupply + _mintAmount <= phaseMaxSupply(),
            "Max allowed supply exceeded"
        );
        require(
            totalSupply + _mintAmount <= maxSupply,
            "Not enough NFTs left to mint that many!"
        );
    ...
    function getPhaseForTokenId(uint256 tokenId) public view returns(uint8){
        if(tokenId <= phaseMaxSupply0)
            return 0;
        if(tokenId <= phaseMaxSupply0 + phaseMaxSupply1) 
            return 1;
        return 2;
    }
    ...
    function setPhase(uint8 _phase) public onlyOwner validPhase(_phase) {
        ...
        if(phase == 0) {
            maxMintAmount = 2;
            maxMintAmountWhitelist = 2;            
        } else if(phase == 1){
            maxMintAmount = 4;
            maxMintAmountWhitelist = 4;
        } else {
            maxMintAmount = 6;
            maxMintAmountWhitelist = 6;
        }
    }
    ...
    //SETTERS FOR COSTS
    function setCostWhitelistedFF(uint256 _newCostWhitelistedFF)
        public
        onlyOwner
    {
    ...
    function setMaxMintAmountWhitelistFF(uint16 _limitFF) public onlyOwner {
        maxMintAmountWhitelistFF = _limitFF;
    }

    function setMaxMintAmountWhitelist(uint16 _limit) public onlyOwner {
    ...
    function phaseMaxSupply() public view returns(uint256) {
    ...
    function withdraw() public payable onlyOwner nonReentrant {
        ...
        ).call{value: (address(this).balance * 6) / 100}("");
        require(phunsuccess);
        // ====================================================================

        // ================This will pay remaining 94%=========================
        (bool coldsuccess, ) = payable(
            0xa68e3E7861596646A8Dac4406D39677dEE099387 //ADDRESS OF COLD WALLET
