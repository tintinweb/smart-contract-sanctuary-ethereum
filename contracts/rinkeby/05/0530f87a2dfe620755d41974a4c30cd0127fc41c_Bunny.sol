...
contract Bunny is ERC721Enumerable, RedeemerWhitelist {
    ...
    function redeemMint() external onlyWhitelisted {
        require(redeemOpened, "redeem is not active");
        require(!hasMintedRedeemToken(_msgSender()), "redeem token is already minted");

        _safeMint(_msgSender(), tokenId);
        tokenId++;
        setRedeemTokenMinted(_msgSender());
    }
    ...
    function mint(uint256 _count) external payable {
        ...
        require(totalSupply() + _count <= 9750, "reached max supply");
...
contract RedeemerWhitelist is Ownable {
    ...
    uint8 redeemersMaxCount = 250;
    uint8 redeemersCount;
    mapping(address => uint8) public redeemersList;
    ...
    function addRedeemer(address _address) public onlyOwner {
        require(redeemersCount < redeemersMaxCount, "redeemers count reached");
        redeemersList[_address] = 1;
        redeemersCount++;
    }

    function removeRedeemer(address _address) public onlyOwner {
        redeemersList[_address] = 0;
        redeemersCount--;
    }

    function bulkAddRedeemers(address[] calldata _addresses) external onlyOwner {
        require(_addresses.length <= redeemersMaxCount, "addresses count exceeds limit");
        require(redeemersCount + _addresses.length <= redeemersMaxCount);
        for (uint256 i = 0; i < _addresses.length; i++) {
            redeemersList[_addresses[i]] = 1;
        }
        redeemersCount = redeemersCount + uint8(_addresses.length);
    }
    ...
    function bulkRemoveRedeemers(address[] calldata _addresses) external onlyOwner {
        ...
        for (uint256 i = 0; i < _addresses.length; i++) {
            redeemersList[_addresses[i]] = 0;
        }
        redeemersCount = redeemersCount - uint8(_addresses.length);
    }

    function isWhitelisted(address _address) public view returns (bool) {
        return redeemersList[_address] == 1;
    }

    function hasMintedRedeemToken(address _address) public view returns (bool) {
        return redeemersList[_address] == 2;
    }

    function setRedeemTokenMinted(address _address) internal {
        require(redeemersList[_address] > 0, "not whitelisted");
        redeemersList[_address]++;
    }
}
