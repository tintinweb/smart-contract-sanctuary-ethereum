...
contract TheParrotBossX1 is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    mapping(address => bool) public glist;

    constructor() ERC721("TheParrotBossX1", "TPBX1") {}
    ...
    function mint(uint256 quantity) external payable {
        ...
        require((totalSupply() + quantity) <= supply, "The quantity of tokens requested is not available");
        require((price * quantity) <= msg.value, "Ether amount sent is incorrect");
        _mintTokens(quantity);
    }

    function gMint() external {
        require(glist[(msg.sender)] == true, "Wallet address not found in the giveaway list.");
        require(isGiveaway, "The Parrot Boss giveaway is not active");
        require(totalSupply() < supply, "The quantity of tokens requested is not available");
        _mintTokens(1);
    }
    ...
    function triggerGiveaway() public onlyOwner {
    ...
    function addToGlist(address[] memory addresses) public onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++){
            glist[addresses[i]] = true;
        }
    }
    
    function removeFromGlist(address[] memory addresses) public onlyOwner {
        for (uint256 i = 0; i < addresses.length ; i++){
            glist[addresses[i]] = false;
        }
    }
