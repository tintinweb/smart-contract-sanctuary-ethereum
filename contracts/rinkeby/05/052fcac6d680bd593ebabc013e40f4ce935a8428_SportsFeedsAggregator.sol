...
contract SportsFeedsAggregator is ChainlinkClient {
    ...
    function updateRoundAnswer(uint32 _roundId, GameCreate[] memory games)
        internal
        returns (bool, GameCreate[] memory)
    {
        // figure out aggregation
        // int256 newAnswer = Median.calculateInplace(details[_roundId].submissions);

        rounds[_roundId].answeredInRound = _roundId;
        for (uint256 x = 0; x < games.length; x++) {
            GameCreate memory game;
            game = games[x];
            rounds[_roundId].answer.push(game);
        }
    ...
    function getGamesCreated(bytes32 _requestId)
        public
        view
        returns (GameCreate[] memory)
    {
        uint256 count;
        count = requestIdGames[_requestId].length;
        GameCreate[] memory games = new GameCreate[](count);
        for (uint256 i = 0; i < requestIdGames[_requestId].length; i++) {
    ...
    function triggerRound(bytes32 _requestId) internal {
        latestRoundId++;
        updateRoundAnswer(latestRoundId, getGamesCreated(_requestId));
    }
