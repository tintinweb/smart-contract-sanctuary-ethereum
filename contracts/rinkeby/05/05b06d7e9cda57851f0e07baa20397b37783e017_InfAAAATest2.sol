...
contract InfAAAATest2 is ERC721A, Ownable {
    ...
    function setMerkleRoot(bytes32 root) external onlyOwner {
     ...
     function pakpak(bytes32 token) external pure returns(bytes32) {
    ...
    function isWhitelistedToken(bytes32 token, bytes32[] calldata _proof) external view returns(bool) {
