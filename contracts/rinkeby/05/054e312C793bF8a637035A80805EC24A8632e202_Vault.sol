...
contract Vault is BaseContract
{
    ...
    mapping(address => uint256) public _initialDeposit;
    mapping(address => uint256) public _totalDeposit;
    mapping(address => uint256) public _totalClaim;
    mapping(address => uint256[]) public _claims;
    mapping(address => bool) public _negative;
    mapping(address => bool) public _penalized;
    mapping(address => uint256) public _lastAction;
    mapping(address => uint256) public _lastDeposit;
    mapping(address => uint256) public _lastClaim;
    mapping(address => bool) public _maxed;
    mapping(address => uint256) public _maxedPercent;
    ...
    function _deposit(address player_, uint256 quantity_) internal returns (bool)
    {
        ...
        if(_startTime[player_] == 0) {
            _startTime[player_] = block.timestamp;
            _initialDeposit[player_] = quantity_;
        }
        if(referrer[player_] == address(0)) {
            referrer[player_] = addressBook.get("safe");
        }
        _lastAction[player_] = block.timestamp;
        _lastDeposit[player_] = block.timestamp;
        uint256 _refundAmount_ = 0;
        if(_totalDeposit[player_] + quantity_ > maxThreshold()) {
            _refundAmount_ = _totalDeposit[player_] + quantity_ - maxThreshold();
        }
        _totalDeposit[player_] += quantity_ - _refundAmount_;
        if(_refundAmount_ > 0) {
        ...
        if(_totalDeposit[player_] >= maxThreshold()) {
            _maxed[player_] = true;
            _maxedPercent[player_] = rewardPercent(player_);
        }
        return true;
    }
    ...
    {
        if(_maxed[player_]) {
            return _maxedPercent[player_];
        }
        if(_penalized[player_]) {
            return penaltyRate;
        }
    ...
    function effectiveClaims(address player_) public returns (uint256)
    {
        ...
        uint256[] memory _claims_ = _claims[player_];
        ...
        if(_negative[player_] && _periodClaims_ < negativeClaims) {
        ...
        if(_periodClaims_ >= negativeClaims) {
            _negative[player_] = true;
        }
        if(_penaltyClaims_ >= penaltyClaims) {
            _penalized[player_] = true;
            _periodClaims_ = 28;
        }
        if(_startTime[player_] < lookback && _periodClaims_ < neutralClaims) {
    ...
    {
        uint256 _period_ = (block.timestamp - _lastAction[player_]) / period;
        uint256 _available_ = _period_ * rewardPercent(player_) * _totalDeposit[player_];
        if(_available_ + _totalClaim[player_] > maxPayout) {
            _available_ = maxPayout - _totalClaim[player_];
        }
        return _available_;
    }
    ...
    function _claim(address player_) internal returns (bool)
    {
        ...
        require(_lastClaim[player_] <= block.timestamp - claimCooldown, "Cooldown period in effect");
        _lastAction[player_] = block.timestamp;
        _lastClaim[player_] = block.timestamp;
        _totalClaim[player_] += _available_;
        IToken _token_ = _token();
        uint256 _balance_ = _token_.balanceOf(address(this));
