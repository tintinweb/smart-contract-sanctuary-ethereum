...
//    using ECDSA for bytes32;
    ...
    function _safeMint(address from, address to, uint256 quantity) internal {
        storageLayer._safeMint(from, to, quantity);
    }
    ...
    function cancelOffer(uint256 tokenId) public {
        ...
        uint256 l = offers[tokenId].length;
        uint256 offerAmount = offers[tokenId].map[pos-1].offer;
        offers[tokenId].map[pos-1] = offers[tokenId].map[l - 1];
        delete offers[tokenId].map[l - 1];
        offers[tokenId].length -= 1;
        offerPositions[tokenId][msg.sender] = 0;
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public override {
        storageLayer.safeTransferFrom(from, to, tokenId, _data);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId) public override {
        storageLayer.safeTransferFrom(from, to, tokenId);
    }

    function transferFrom(address from, address to, uint256 tokenId) public override {
        storageLayer.transferFrom(from, to, tokenId);
    }

    function approve(address to, uint256 tokenId) public override {
        storageLayer.approve(to, tokenId);
    }
...
contract ERC721Storage is Ownable {
    ...
    function _safeMint(address from, address to, uint256 quantity) public onlyOwner {
