...
contract doge is ERC721A, Ownable, ReentrancyGuard {
   ...
   // uint256 public Price = 0.0066 ether;
    uint256 constant public doges = 3000;
    // uint256 public freeSupply = 1789;
    // uint256 public team = 289;
    // uint256 public maxFree = 2;
    uint256 public maxBuy = 10; 

    bool private saleIsActive = false;
    bool private revealed = false;
   // mapping(address => uint256) public howmanyfreedoges;
    mapping(address => uint256) public howmanybuydoges;



    constructor() ERC721A("doge", "doge") {
    ...
    function mint(uint256 _amount) external  nonReentrant mintCompliance{
        require(totalSupply() + _amount <= doges, "over  supply");
        require(_amount <= maxBuy);
       // require(msg.value == _amount*Price);
        require(howmanybuydoges[msg.sender] < maxBuy);
         _safeMint(msg.sender, _amount);
        howmanybuydoges[msg.sender] += _amount;
    ...
    //     require(howmanyfreedoges[msg.sender] < maxFree);

    //     _safeMint(msg.sender, _mintAmount);
    //     howmanyfreedoges[msg.sender] += _mintAmount;

    // }


    // function setTeam(uint256 _team) external onlyOwner {
    //     team = _team;
    // }
...
//         require((totalSupply() + _amount) <= doges, "mint out!");
//         require(howmanyteam[msg.sender]+_amount <= team);
//          _safeMint(_to, _amount);
//         howmanyteam[msg.sender] += _amount;
//     }
