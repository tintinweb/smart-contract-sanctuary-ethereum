...
contract PreMintPasses is ERC1155MintableTradeable {

    bool private transferFrozen = true;

    // Mapping for burners
    mapping(address => bool) private _burners;

    // Mapping for transferors, they can transfer tokens even if transfer is frozen 
    mapping(address => bool) private _transferors;
    ...
    constructor(
        ...
        address royaltyReceiver,
        address executor
    ) ERC1155MintableTradeable(commonURI, contractURIValue, royaltyPercent, royaltyReceiver, executor) {}


    // throws if called by any account other than the burner.
    modifier onlyBurner() virtual {
        require(_burners[_msgSender()], "PreMintPasses: caller is not burner");
        _;
    }
    ...
    function setBurner(address burner, bool canBurn) public virtual onlyOwner {

        _burners[burner] = canBurn;
    }


    function isTransferor(address transferor) public view virtual returns (bool) {

        return _transferors[transferor];
    }


    function setTransferor(address transferor, bool canTransfer) public virtual onlyOwner {

        _transferors[transferor] = canTransfer;
    }


    // prevent token transfer if transferFrozen flag is false
    function safeTransferFrom(
        address from,
        address to,
        uint256 id,
        ...
        require(_transferors[_msgSender()] || transferFrozen == false, 'PreMintPasses: Pass transfer/trading is frozen');

        super.safeTransferFrom(from, to, id, amount, data);
    }


    // prevent token transfer in batch if transferFrozen flag is false
    function safeBatchTransferFrom(
        address from,
        address to,
        uint256[] memory ids,
    ...
    // only contract owner can burn their token
    function burn(address account, uint256 id, uint256 value) public override onlyBurner {

        super.burn(account, id, value);
    }


    // only contract owner can burn their tokens in batch
    function burnBatch(address account, uint256[] memory ids, uint256[] memory values) public virtual onlyBurner {

        require(account == _msgSender() || isApprovedForAll(account, _msgSender()),
            "PreMintPasses: caller is not owner nor approved");
