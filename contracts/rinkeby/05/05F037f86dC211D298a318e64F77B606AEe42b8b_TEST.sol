...
contract TEST is Ownable, ERC721A, ReentrancyGuard, EIP712Whitelisting {
    using Address for address;
    using SafeMath for uint256;

    uint256 public immutable collectionSize;
    uint256 public immutable maxAirdrop;
    uint256 public immutable publicMintPerWallet;
    ...
    mapping(address => uint256) private _whitelistSaleClaimed;
    ...
    constructor(
        ...
        uint256 maxAirdrop_,
        uint256 publicMintPerWallet_,
        uint256 whitelistMintPerWallet_,
        revenueShareParams memory revenueShare
    ) ERC721A("TEST", "TEST club"){
        collectionSize = collectionSize_;
        maxAirdrop = maxAirdrop_;
        publicMintPerWallet = publicMintPerWallet_;
        whitelistMintPerWallet = whitelistMintPerWallet_;
        _splitter = new PaymentSplitter(
            revenueShare.payees,
            revenueShare.shares
        );
        require(maxAirdrop_ <= collectionSize_, "larger collection size needed");
    }
    ...
    function publicMint(uint256 amount) external payable nonReentrant callerIsUser {
        uint256 price = amount.mul(publicPrice);
        require(price > 0, "public sale has not begun yet");
        require(
            amount <= publicMintPerWallet,
    ...
    {
        uint256 price = amount.mul(whitelistPrice);
        require(price > 0, "whitelist sale has not begun yet");
        require(
            amount <= whitelistMintPerWallet,
            "Mint exceed transaction limits"
        );
        require(
            _whitelistSaleClaimed[msg.sender] + amount <= whitelistMintPerWallet,
            "Mint limit per wallet exceeded."
        );
        require(totalSupply() + 1 <= collectionSize, "reached max supply");
        require(msg.value >= price, "Insufficient funds.");
        _whitelistSaleClaimed[msg.sender] = _whitelistSaleClaimed[msg.sender] + amount;
        _safeMint(msg.sender, amount);
        refundIfOver(price);
    }

    function setMaxWhitelistMintPerWallet(uint256 maxWhitelistMintPerWallet) external onlyOwner{
        whitelistMintPerWallet = maxWhitelistMintPerWallet;
    }
    ...
    {
        require(
            totalSupply().add(addresses.length.mul(amount)) <= collectionSize,
            "Exceed max supply limit."
        );
        require(
            airdropped.add(addresses.length.mul(amount)) <= maxAirdrop,
            "Insufficient reserve."
        );
        for (uint256 i = 0; i < addresses.length; i++) {
            _safeMint(addresses[i], amount);
        }
        airdropped = airdropped.add(addresses.length.mul(amount));
    }
    ...
    function setWhitelistPrice(uint256 whitelistPrice_) external onlyOwner {
