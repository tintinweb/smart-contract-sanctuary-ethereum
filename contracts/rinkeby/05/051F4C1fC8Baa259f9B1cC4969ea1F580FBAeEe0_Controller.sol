    ...
    PausableUpgradeable
{
    ...
    function setExecutionFee(uint256 _executionFee)
        external
    ...
    function cancelOrder(uint256 _tokenId, address _poolAddress)
        public
        allowedOrder(_tokenId, _poolAddress)
    {
        ...
        (uint256 transferedToken0, uint256 transferedToken1) = _closeOrder(
            _tokenId,
            order
        );
        emit CancelOrder(
    ...
    /// @param _tokenId tokenId of uniswapV3 nft
    /// @param _poolAddress address of uniswapV3 pool
    function executeOrder(uint256 _tokenId, address _poolAddress)
        public
        allowedOrder(_tokenId, _poolAddress)
    {
    ...
    function editOrder(EditOrderParams memory _editParams)
        public
        allowedOrder(_editParams.tokenId, _editParams.poolAddress)
        whenNotPaused
        returns (uint256 newAmountOfToken0, uint256 newAmountOfToken1)
    {
        ...
        Order storage order = orders[_editParams.poolAddress][
            _editParams.tokenId
        ];
        uint128 liquidity;
        (, , , uint256 quoteAmount, , ) = checkOrder(
            _editParams.tokenId,
            _editParams.poolAddress
        );
        require(quoteAmount == 0, "EDIT3");

        require(order.owner == msg.sender, "EDIT4");
        if (_editParams.amountOfToken == 0) {
            ...
            if (
                ...
                IERC20Upgradeable(order.token0).safeIncreaseAllowance(
                    address(nftPositionManager),
                    _editParams.amountOfToken - order.amountOfToken0
                );
            } else if (
                ...
                IERC20Upgradeable(order.token1).safeIncreaseAllowance(
                    address(nftPositionManager),
                    _editParams.amountOfToken - order.amountOfToken1
                );
            }
            ...
            if (order.orderType == OrderType.TAKE_PROFIT) {
                if (_editParams.amountOfToken < order.amountOfToken0) {
                                ...
                                liquidity: LiquidityAmounts
                                    .getLiquidityForAmount0(
                                        ...
                                        order.amountOfToken0 -
                                            _editParams.amountOfToken
                                    ),
                                amount0Min: ((order.amountOfToken0 -
                                    _editParams.amountOfToken) * SLIPPAGE) /
                                    SLIPPAGE_ACCURACY,
                                amount1Min: 0,
                                deadline: block.timestamp
                            });
                    (newAmountOfToken0, newAmountOfToken1) = nftPositionManager
                    ...
                    IERC20Upgradeable(order.token0).safeTransfer(
                        _msgSender(),
                        newAmountOfToken0
                    );
                    order.recievedAmount -= LiquidityAmounts
                    ...
                    order.liquidity -= LiquidityAmounts.getLiquidityForAmount0(
                        TickMath.getSqrtRatioAtTick(order.tickLower),
                        TickMath.getSqrtRatioAtTick(order.tickUpper),
                        order.amountOfToken0 - _editParams.amountOfToken
                    );
                    order.amountOfToken0 -= newAmountOfToken0;
                ...
                if (_editParams.amountOfToken < order.amountOfToken1) {
                    INonfungiblePositionManager.DecreaseLiquidityParams
                        memory params = INonfungiblePositionManager
                            .DecreaseLiquidityParams({
                                ...
                                amount0Min: 0,
                                amount1Min: ((order.amountOfToken1 -
                                    _editParams.amountOfToken) * SLIPPAGE) /
                                    SLIPPAGE_ACCURACY,
                                deadline: block.timestamp
                            });
                    ...
                    // Send decreased tokens to user.
                    IERC20Upgradeable(order.token1).safeTransfer(
                        _msgSender(),
                        newAmountOfToken1
                    );
                    order.recievedAmount -= LiquidityAmounts
                    ...
                    order.liquidity -= LiquidityAmounts.getLiquidityForAmount1(
                        TickMath.getSqrtRatioAtTick(order.tickLower),
                        TickMath.getSqrtRatioAtTick(order.tickUpper),
                        order.amountOfToken1 - _editParams.amountOfToken
                    );
                    order.amountOfToken1 -= newAmountOfToken1;
    ...
    function _closeOrder(uint256 _tokenId, Order storage _order)
        internal
