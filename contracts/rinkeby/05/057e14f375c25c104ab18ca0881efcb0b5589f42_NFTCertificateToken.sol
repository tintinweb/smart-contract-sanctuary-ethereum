...
contract NFTCertificateToken is Initializable{
    ...
    function userTransfer(uint256 id, address from, address to) private {
        removeCertificateList(from, findCertificateIndex(from, id));
        if (findCertificateIndex(to, id) < int(0)) {
            mapUserCertificate[to].push(id);
        }
    ...
    function issueCertificate(address _receiverAddress, Certificate memory _certificate, string memory _uri) public  {
        ...
        InterfaceNFTToken(NFTTokenAddress).mintToken(_receiverAddress, _certificate.tokenId, 1, "", msg.sender, address(this), certificateType);
        mapUserCertificate[_receiverAddress].push(_certificate.tokenId);
    ...
    function voidToken(address _target, string memory _id, uint256 _amount, string memory _reason) public  {
        ...
        removeCertificateList(_target, findCertificateIndex(_target, mapId[_id]));
    ...
    function findCertificateIndex(address _receiver, uint256 _tokenId) private view returns (int) {
        if (keccak256(abi.encode(mapTokenType[_tokenId])) == keccak256(abi.encode(certificateType))) {
    ...
    function removeCertificateList(address _receiver, int _index) private {
