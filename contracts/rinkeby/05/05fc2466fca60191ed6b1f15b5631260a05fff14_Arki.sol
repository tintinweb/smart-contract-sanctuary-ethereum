...
interface ArkipelPassInterface{
  function balanceOf(address account, uint256 id) external view returns (uint256);
}

contract Arki is Ownable, ERC721A, ReentrancyGuard {
  address public constant ARKIPELPASS_CONTRACT = 0x6fc41B88FabF5c60beBc9611C3Aee27a1b09C384;
  ArkipelPassInterface ArkipelPassContract = ArkipelPassInterface(ARKIPELPASS_CONTRACT);

  /**
   * String Url
   *
   */
  string private _baseTokenURI = "ipfs://QmP2We2mbVPn6ygTzqVGdWL5EDfhMsK8JKd8pEyNzhwf5j/";
  string private _unrevealedURI = "";

  /**
   * Whitelist setting
   * _rootHash = Root hash from generated merkle tree
   * maxWhitelist = Maximum quantity user in whitelist can mint 
   *
   */
  bytes32 private _rootHash = 0xd8358172f847db0d82166be658a7f1621a1b0b1ef4fca1382a4fce7593d2ef09;
  uint256 public maxWhitelist = 10;
  uint256 public maxPublicSize = 10;
  uint256 public totalCollectionSize = 50;
  ...
  /**
   * Constructor
   * Change name, code, and pricing here
   *
   */
  constructor() ERC721A("ARKIPELAGO TEST", "ARKITEST", maxPublicSize, totalCollectionSize) {
    saleConfig.presalePrice = 0 ether;
    saleConfig.publicPrice = 0.00059 ether;
  }
  ...
  function arkipelPassBalance(address owner) external view returns (uint256) {
    return ArkipelPassContract.balanceOf(owner, 1);
  }
  ...
  function airdrop(uint256 quantity, address destination) public onlyOwner  {
    require(quantity > 0, "Mint must be more tha 0");
  ...
  function openForPublic(bool isOpenForPublic) external onlyOwner {
    _publicMintOpen = isOpenForPublic;
  }
