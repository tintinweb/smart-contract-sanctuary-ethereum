...
contract DataBaseForMarketTest {
    ...
    function getNumberOfAdvertismets() external returns(uint256) {
    ...
    function getZapis(address _address, uint256 numberOfAdvertisment) external returns(advertisments memory){  
        // advertisments[] memory arr = numberOfAdvertismentsOfUser[msg.sender];
        // advertisments memory res = arr[numberOfAdvertisment];
        // return(res);

        return(numberOfAdvertismentsOfUser[msg.sender][numberOfAdvertisment]);
    }
