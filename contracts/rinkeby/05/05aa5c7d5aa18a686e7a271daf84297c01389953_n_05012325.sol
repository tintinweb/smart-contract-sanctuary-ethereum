...
// File: n_20220423/IERC721Receiver.sol
...
// File: n_20220423/IERC721Metadata.sol
...
contract n_05012325 is ERC721{
    ...
    string baseURI = "https://ipfs.io/ipfs/QmS33BcJ65nWPnrQrag7fBRZyyTDU48nLcsMV7iWLmsuDi/";
    ...
    mapping(uint => uint)public periodOfContinuedPossession;
    ...
    constructor() ERC721("n_04292325" , "n_04292325" ) {
    ...
    function devMint(uint256 _tokenID,address _receiver) external callerIsUser onlyOwner{
        _devMint(_tokenID,_receiver);
    }

    // function gift(uint256 _tokenID , address _receiver)external callerIsUser onlyOwner{
    //     require(msg.sender != _receiver);
    //     _devMint(_tokenID,_receiver);
    // }

    function _devMint(uint256 _tokenID,address to)private onlyOwner{
    ...
    function setTokenURI( uint256 targetnftid ,string memory _uri ) public onlyOwner callerIsUser{
    ...
    function setTokenIDRange_max_min(uint _max,uint _min)public onlyOwner callerIsUser{
        max = _max;
        min = _min;
    }
    ...
    function allowlistMint(uint256 _tokenID)public payable callerIsUser{
    ...
    function allowlistMint_public(uint256 _tokenID)public payable callerIsUser{
    ...
    function _inMint(uint256 _tokenID)private{
         require(minted[_tokenID] == false);
        require(_tokenID < max);
        require(_tokenID >= min);
    ...
    function getPeriodOfContinuedPossession(uint _tokenId)public view returns(uint){
        return periodOfContinuedPossession[_tokenId];
    }

    function getNowMinusPeriodOfContinuedPossession(uint _tokenId)public view returns(uint){
        return block.timestamp - periodOfContinuedPossession[_tokenId];
    }
    ...
    function safeTransferFrom(
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        _safeTransfer(from, to, tokenId, _data);
        periodOfContinuedPossession[tokenId] = block.timestamp;
    }
