 ...
 *
 */
contract LL420BudStaking is Ownable, ReentrancyGuard, Initializable {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.UintSet;

    IERC721 public BUD_CONTRACT;
    IERC721 public GAME_KEY_CONTRACT;
    ...
    uint32[6] public thcInfo = [420, 520, 620, 720, 1020, 1420];
    ...
    function initialize(address _budContract, address _gameKeyContract) external initializer {
        ...
        GAME_KEY_CONTRACT = IERC721(_gameKeyContract);
    }
     ...
     */
    function deposit(uint256[] memory _ids) external nonReentrant {
        require(stakingLaunched, "LL420BudStaking: Staking is not launched yet");
     ...
     */
    function depositGameKey(uint256[] calldata _ids) external nonReentrant {
     ...
     * @param _ids The BUD NFT ids to deposit.
     */
    function _withdraw(uint256[] memory _ids) internal {
        require(stakingLaunched, "LL420BudStaking: Staking is not launched yet");
        ...
        for (uint256 i; i < _ids.length; i++) {
            ...
            BudInfo storage bud = _budInfo[_ids[i]];
            require(block.timestamp - bud.timestamp > LOCK_PERIOD, "LL420BudStaking: Cant withdraw in lock period");
