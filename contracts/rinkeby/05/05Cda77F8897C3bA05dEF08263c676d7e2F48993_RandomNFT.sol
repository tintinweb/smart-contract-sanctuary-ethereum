...
contract RandomNFT is Ownable {
    ...
    mapping(address => mapping(uint256 => address[])) adrToLotIdToPart;
    ...
    function makeLotery(
        ...
        require(maxParticipant <= maxAmountPart, "maxParticipant is too big");
        ...
        mainToken.transferFrom(msg.sender, address(this), id);
        lotOwners.push(msg.sender);
    }

    function participate(address adr, uint256 lotId) external {
        ...
        require(
            !isInArray(adrToLotIdToPart[adr][lotId], msg.sender),
            "Already participating in this lot"
        );
        ...
        adrToLotIdToPart[adr][lotId].push(msg.sender);
    }
    ...
    function closeLottery(address adr, uint256 lotId) external {
        require(
            block.timestamp >= adrToLotIdToInfo[adr][lotId].endDate,
        ...
        require(adrToLotIdToInfo[adr][lotId].done == false, "Already done");
        uint256 ran = random(adrToLotIdToPart[adr][lotId].length + 1) - 1;
        adrToLotIdToInfo[adr][lotId].winner = adrToLotIdToPart[adr][lotId][ran];
        adrToLotIdToInfo[adr][lotId].claimStart = block.timestamp;
    }
