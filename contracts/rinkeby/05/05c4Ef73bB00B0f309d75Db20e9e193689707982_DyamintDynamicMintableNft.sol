// contracts/DyamintDynamicMintableNft.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
import "@openzeppelin/contracts/access/Ownable.sol";
import "./DyamineDynamicNft.sol";
...
// future items in the collection, along with
// minting items at contract-level
//

contract DyamintDynamicMintableNft is DyamineDynamicNft {
    uint256 public _mintPriceWei;
    bool public _allowMinting = true;
    ...
    {
        _mintPriceWei = mintPriceWei;
    }

    function setMintPrice(uint256 priceWei)
        public onlyOwner
    {
        _mintPriceWei = priceWei;
    }

    function setAllowMinting(bool value)
        public onlyOwner
    {
        _allowMinting = value;
    }

    function mintItems(uint256[] memory tokenIds)
        public payable
    {
        require(_allowMinting == true, "Minting has been disabled");
        require(msg.value >= tokenIds.length * _mintPriceWei, "You didn't include enough ETH with your transaction");
...
contract DyamineStandardNft is ERC721, Ownable {
    ...
    function approve(address to, uint256 tokenId) public override {
        if (!_exists(tokenId)) {
            require(
                _checkCallerApproved(msg.sender, tokenId),
    ...
    function _checkCallerApproved(address spender, uint256 tokenId) internal view returns (bool) {
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
        ...
        returns (bool)
    {
        ...
        return _isApprovedOrOwnerAtAll(spender, tokenId);
    }

    /**
     * @dev Returns whether `spender` is allowed to manage `tokenId`.
     */
    function _isApprovedOrOwnerAtAll(address spender, uint256 tokenId)
        internal
        view
        virtual
        returns (bool)
