...
contract Ownable is OwnableData {
    ...
    function setMSTokenAddress(address _msToken) public {
    ...
    /// @notice Only allows the `owner` to execute the function.
    modifier onlyMSToken() {
        require(msg.sender == MSToken, "Ownable: caller is not the msToken");
        _;
    }
...
contract rewardPool is Ownable {
    ...
    constructor(address _mstAddress) {     
    ...
    function updateRewardRates(uint256 _rewardRates) external onlyOwner {
        // Reward rate per day for each Minisquatch (1e9 = 1 MSToken)
    ...
    function pendingRewardFor(uint _nftId) public view returns (uint256 _reward) {
        ...
        _reward = (_daysSinceLastClaim * rewardRates).div(1e9);
        return _reward;
    }
    ...
    function claimReward(uint _nftId) public returns (bool success) {
        ...
        nftInfo[_nftId].lastClaim = block.timestamp;
        IERC20(MSToken).transfer(nftInfo[_nftId].owner, _reward);
        return true;
    }
