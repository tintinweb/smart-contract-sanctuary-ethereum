    ...
    function takeFees(
        ...
        if (automatedMarketMakerPairs[_from]) {
            ...
            if (block.number == launchBlock) {
                ...
                totalSellTax = baseSellTax.add(sellRewards).add(_greenWaveJeetTax);
            ...
            // todo moved this from the bottom to test sell fee process
            emit SellFees(_from, address(this), fees);

            if(_greenWaveJeetTax > 0) {
                uint256 greenWaveJeetRewards = _amount.mul(_greenWaveJeetTax).div(100);
...
contract BuyWaveMapping is Ownable {
    ...
    function excludeToGreenWaveMap(address account) external onlyOwner {
        ...
        address[] memory updatedHolders;
        uint256 position = 0;
        for (uint256 i = 0; i < holders.length; i++) {
            address holder = holders[i];
            if (holder != account) {
                updatedHolders[position] = holder;
                position++;
            }
        }
