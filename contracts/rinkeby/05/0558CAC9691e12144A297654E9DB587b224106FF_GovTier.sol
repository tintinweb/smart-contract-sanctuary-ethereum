...
contract GovTier is IGovTier, OwnableUpgradeable, SuperAdminControl {
    ...
    event WalletTier(address _userAddress, bytes32 _tierLevel);
    event RemoveWalletTier(address _userAddress);
    ...
    function initialize(
        ...
        _addTierLevel(
            _bronze,
            TierData(15000e18, 30, false, false, false, false, false, false)
        );
        _addTierLevel(
            _silver,
    ...
    function addEditWalletTierLevel(
        ...
        )
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            emit WalletTier(user, _tierLevel[i]);
        }
    }

    function RemoveWalletTierLevel(
        address[] memory _userAddress
    )
        external
        onlySuperAdmin(
