...
abstract contract NFTMTokenMarketFee is NFTM {
    ...
    function buyNFTWithLox(
        address tokenAddr,
        uint256 tokenId
    ) public payable nonReentrant onlySellingToken(tokenAddr,tokenId) onlyApprovedToken(tokenAddr, tokenId){
    ...
    function checkNFTPriceWithLox(
        address tokenAddr,
        uint256 tokenId
    ) public view onlySellingToken(tokenAddr,tokenId) onlyApprovedToken(tokenAddr, tokenId) returns(uint256){
...
abstract contract NFTMPromotion is NFTM {
    ...
    function createSaleItemWithPromoCode(
        ...
        //TODO: required ERC721

        //require(tokenAddr != address(0), "NFTM: Provided invalid token address");
...
abstract contract NFTM is ReentrancyGuard, Ownable {
    ...
    CurrencyConverterInterface _currencyConverter;
    ...
    constructor(address feeCollector, uint256 marketFeeCENT) {
        ...
        _marketFeeCENT = marketFeeCENT;
         ...
         _currencyConverter = new RealTimeCurrencyConverter(); // TODO: For production deployment
        //_currencyConverter = new DummyCurrencyConverter(); // TODO: For testing deployment
    }
    ...
    function _createSaleItemWithCreatorFee(
        ...
        address payable creator,
        uint256 creatorFee
    ) internal notNullAddress(tokenAddr) onlyApprovedToken(tokenAddr, tokenId){
    ...
    function _executeSaleItem(
        ...
        uint256 creatorFee,
        uint256 marketFee
    ) internal onlySellingToken(tokenAddr,tokenId){
    ...
    function _isTokenApproved(address tokenAddr, uint256 tokenId) internal view returns (bool){
        IERC721 nftContract = IERC721(tokenAddr);
    ...
    modifier onlyApprovedToken(address tokenAddr, uint256 tokenId) {
        require (_isTokenApproved(tokenAddr, tokenId),"NFTM: Market is not token owner nor approved");
        _;
    }
