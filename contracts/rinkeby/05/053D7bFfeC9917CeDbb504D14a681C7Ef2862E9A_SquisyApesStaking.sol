    ...
    Pausable
{
    ...
    function claimAll(address tokenOwner) external override {
        //require(msg.sender == globalDistributer);
        uint256 reward;
        uint256[] memory tokenIds = depositsOf(tokenOwner);
        for (uint256 i; i < tokenIds.length; i++) {
            reward += calculateReward(msg.sender, tokenIds[i]);
        ...
        if (reward > 0) {
            rewardCollected[tokenOwner] += reward;
            IERC20(erc20Address).transfer(tokenOwner, reward);
        }
    }
...
interface GlobalClaimer {
    function claimAll(address tokenOwner) external;
