...
contract PayRueERC721 is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address public commissionReceiverAddress;
    ...
    function resellToken(uint256 _tokenId, uint256 _price) public payable nonReentrant{
        require(_exists(_tokenId), "ERC721Metadata: Sale set of nonexistent token");
        require(_price > 0);
        require(ownerOf(_tokenId) == _msgSender());
        require(msg.value == _price/100,"Invalid commission value");
    ...
    function resellTokenByPropel(uint256 _tokenId, uint256 _price) public nonReentrant{
        require(_exists(_tokenId), "ERC721Metadata: Sale set of nonexistent token");
        require(_price > 0);
        require(ownerOf(_tokenId) == _msgSender());
        require(
            propelToken.transferFrom(msg.sender, payable(commissionReceiverAddress), _price/200 ),
            "Cannot Transfer Reselling Commission As Propel"
        );
        _tokenMeta[_tokenId].sale = true;
        setTokenPrice(_tokenId, _price);
    }
    ...
    function purchaseToken(uint256 _tokenId) public payable nonReentrant {
        require(_tokenMeta[_tokenId].priceInPropel == false);
        require(msg.sender != address(0) && msg.sender != ownerOf(_tokenId));
        require(msg.value == (_tokenMeta[_tokenId].price + (_tokenMeta[_tokenId].price/200)));
        address tokenSeller = ownerOf(_tokenId);

        AddressUpgradeable.sendValue(payable(tokenSeller),msg.value - (msg.value/200) );
        AddressUpgradeable.sendValue(payable(commissionReceiverAddress),msg.value/200 );
        _transfer(tokenSeller, msg.sender, _tokenId);
        _tokenMeta[_tokenId].sale = false;
    }

    function purchaseTokenByPropel(uint256 _tokenId) public nonReentrant {
        require(_tokenMeta[_tokenId].priceInPropel == true);
        require(msg.sender != address(0) && msg.sender != ownerOf(_tokenId));
        address tokenSeller = ownerOf(_tokenId);

        require(
            propelToken.transferFrom(msg.sender, payable(tokenSeller),
            _tokenMeta[_tokenId].price - (_tokenMeta[_tokenId].price / 400) ),
            "Cannot Transfer Nft Price As Propel"
        );

        require(
            propelToken.transferFrom(msg.sender,
            payable(commissionReceiverAddress), _tokenMeta[_tokenId].price / 400 ),
            "Cannot Transfer Buying Commission As Propel"
        );
        _transfer(tokenSeller, msg.sender, _tokenId);
        _tokenMeta[_tokenId].sale = false;
    }
        ...
        returns (uint256)
    {
        ...
        TokenMeta memory meta = TokenMeta(newItemId, _price, _name, _tokenURI, _sale, false);
        _setTokenMeta(newItemId, meta);

        return newItemId;
    }

    function mintCollectableByPropel(
        address _owner,
        string memory _tokenURI,
        string memory _name,
        uint256 _price,
    ...
    returns (uint256)
    {
        ...
        require(
            propelToken.transferFrom(msg.sender, payable(commissionReceiverAddress), _price/200 ),
            "Cannot Transfer Minting Commission As Propel"
        );
        _tokenIds.increment();
        ...
        TokenMeta memory meta = TokenMeta(newItemId, _price, _name, _tokenURI, _sale, true);
        _setTokenMeta(newItemId, meta);
