...
contract OasisX1155 is ERC1155, ERC1155Supply, Pausable {
    ...
    {
        require(exists(id), "OasisX1155: Nonexistent token");
    ...
    function adminMint(
        ...
        require(to != address(0), "OasisX1155: Address cannot be 0");
    ...
    function mint(
        ...
        require(to != address(0), "OasisX1155: Address cannot be 0");
        require(
            received == mintPrice[id].mul(amount),
     ...
     * @param tokenIds_ list of token ids to be added
     * @param mintCostPerTokenId_ price per token Id
     **/
    function addTokensAndChangeMintPrice(
        uint256[] memory tokenIds_,
        uint256[] memory mintCostPerTokenId_
    ) public onlyOwner {
        require(
            tokenIds_.length == mintCostPerTokenId_.length,
            "OasisX1155: IDs/Supply arity mismatch"
        );
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            require(
                !exists(tokenIds_[i]),
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                minted[to] += minted[to].add(amounts[i]);
            }
        }
    }
}
