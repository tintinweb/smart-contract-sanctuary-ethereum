...
contract MoonwalkersA is ERC721A, Pausable, Ownable, ReentrancyGuard {
  ...
  /// @dev Mapping of the amount of NFT minted by a whitelisted address during the presale.
  mapping(address => uint256) private amountMintedWhitelist;

  /// @dev Mapping of the amount of NFT minted by an alpha address during the presale.
  mapping(address => uint256) private amountMintedAlpha;
  ...
  /// @dev Starting block index.
  uint256 public startingIndexBlock;
  
  /// @dev Starting Index in the block.
  ...
  constructor(uint256 _maxSupply) ERC721A("Moonwalkers", "MW") {
    ...
    unrevealedTokenURI = "";
    saleConfig.publicSaleMintPrice = 0.001 ether;
    saleConfig.presaleMintPrice = 0.0008 ether;
    saleConfig.presaleStartTime = 1650540387;
    saleConfig.publicSaleStartTime = 1650540387;
    saleConfig.maxWhitelistMintPerWallet = 5;
    saleConfig.maxAlphaMintPerWallet = 7;
    saleConfig.maxPublicMintPerWallet = 10;
  ...
  function publicSaleMint(uint256 _mintAmount) external payable whenNotLocked callerIsUser {
    ...
    require(block.timestamp > config.publicSaleStartTime, "Public sale has not stared yet."); // Need to take 1 second less cause it's a strict superior
    require(_totalMinted().add(_mintAmount) < maxSupply.add(1), "Max supply reached.");
    require(_mintAmount > 0 && _mintAmount < config.maxPublicMintPerWallet.add(1), "Max mint per tr. exceeded");
    require(msg.value == config.publicSaleMintPrice.mul(_mintAmount), "Tr. value did not equal the mint price");
    require(amountMintedPublic[msg.sender].add(_mintAmount) < config.maxPublicMintPerWallet.add(1), "Max public sale mint per address exceeded");
  ...
  function allowlistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable whenNotLocked callerIsUser {
    ...
    require(_totalMinted().add(_mintAmount) < maxSupply.add(1), "Max supply reached.");
    require(_mintAmount > 0 && _mintAmount < config.maxWhitelistMintPerWallet.add(1), "Max mint per tr. exceeded.");
    require(msg.value == config.presaleMintPrice.mul(_mintAmount), "Tr. value did not equal the mint price.");
    require(amountMintedWhitelist[msg.sender].add(_mintAmount) < config.maxWhitelistMintPerWallet.add(1), "Max presale mint per address exceeded.");
  ...
  function alphaMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable whenNotLocked callerIsUser {
    ...
    require(block.timestamp > config.presaleStartTime && block.timestamp < config.publicSaleStartTime, "Presale is closed.");
    require(_totalMinted().add(_mintAmount) < maxSupply.add(1), "Max supply reached.");
    require(_mintAmount > 0 && _mintAmount < config.maxAlphaMintPerWallet.add(1), "Max mint per tr. exceeded.");
    require(msg.value == config.presaleMintPrice.mul(_mintAmount), "Tr. value did not equal the mint price.");
