...
contract KculpMarket is Ownable {
  address public royaltyAccount;
  address nftContract;

  event ItemSold(uint256 indexed tokenId, address buyer);

  event RoyaltyPaid(uint256 indexed tokenId, address account, uint256 value);

  function testEmit() public {
    emit RoyaltyPaid(1, address(0x1aC20Be7c1B444a284Cd7A6820F2ca03625b90E0), 133700000);
  }

  function setRoyaltyContract(address _account) external onlyOwner {
  ...
  function buy(
    ...
    require(_seller == IERC721(_nftContract).ownerOf(_tokenId), "MarketBuy: Seller is not seller");
    ...
    address[2] memory _accounts = IKculp4(_nftContract).getRoyaltyAccounts(_tokenId);
    ...
    if (_accounts[0] == _accounts[1]) {
      ...
      payable(_accounts[0]).transfer((msg.value * _roy1) / 1000);
    }
    if (_accounts[1] == royaltyAccount) {
      ...
      payable(_accounts[1]).transfer((msg.value * _roy2) / 1000);
    }
    if (royaltyAccount == _seller) {
      _tot = _tot + _roy3;
    } else {
      payable(royaltyAccount).transfer((msg.value * _roy3) / 1000);
    }

    payable(_seller).transfer((msg.value * _tot) / 1000);

    emit RoyaltyPaid(_tokenId, _accounts[0], (msg.value * 25) / 1000);
    emit RoyaltyPaid(_tokenId, _accounts[1], (msg.value * 25) / 1000);

    ERC721(_nftContract).safeTransferFrom(_seller, msg.sender, _tokenId);
