...
contract MarketplaceV3 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     * @param lotId - ID of the lot
     * @param duration - New lot duration
     */
    function renew(uint256 lotId, uint256 duration) external lotIsActive(lotId) notBanned {
        ...
        _lot.auctionStart = block.timestamp;
        _lot.auctionEnd = block.timestamp + duration;
    ...
    /**
     * @notice In case if failed tx happens (from FailedTx event only)
     * @param _amount - Amount of ethers to recover
     * @return Is successfully recovered
     */
    function recoverEther(uint256 _amount) external onlyOwner returns (bool) {
        (bool success, ) = payable(msg.sender).call{value: _amount}("");
        emit EthersRecovered(_amount);
        return success;
    }
