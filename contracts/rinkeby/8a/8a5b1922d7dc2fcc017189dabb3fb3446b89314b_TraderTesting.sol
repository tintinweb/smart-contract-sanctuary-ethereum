...
contract Whitelisting is Ownable{
    ...
    bool registerUserEnabled = false;
    ...
    function registerUser(address addr, uint256 id) external moduleAccess(id){
        require(registerUserEnabled == true, "Can't add");
 ...
 contract TraderTesting is ERC721, ERC721Holder {

    Whitelisting whitelistMethod; 
    ...
    constructor(address whitelistContractAddress) ERC721("TraderSpirits", "TRSP"){
        whitelistMethod = Whitelisting(whitelistContractAddress);
        custodialWallet = msg.sender;
    }


    function mintByUser(uint256 quantity, bytes32[] memory proof) public isPause isHalt payable{
        
        uint256 identifier = whitelistMethod.identifier();

        bool done = false;  //false
        if(privateSaleIsActive == true){
          
        if(identifier == 1){
           require (whitelistMethod.VerifyAddressForWhitelisting(msg.sender) == true, "User not whitelisted");

        } else if (identifier == 2){

            bytes32 leaf = keccak256(abi.encodePacked(msg.sender));

            bytes32[] memory rootHashes = whitelistMethod.getRootHashesForVerifyUsingConract();
           
            uint256 rootHashesLength = rootHashes.length;
    
            bool verified; //for testing in loop
           
            for(uint i = 0; i < rootHashesLength; i++){
                verified = MerkleProof.verify(proof,rootHashes[i], leaf);
               ...
               require(done == true, "User not whitelisted.");
            }
            ...
            uint256 rootHashesLength = rootHashes.length;
             ...
             bool verified; //for testing in loop
            // require(MerkleProof.verify(proof,rootHash, leaf), "User is not whitelisted");
            for(uint i = 0; i < rootHashesLength; i++){
    ...
    function mintThroughStripe(bytes memory UUID, bytes32 paymentID, uint256 quantity) public isHalt isPause onlyCustodialWallet {
        require(publicSaleIsActive == true, "Sale has not been started yet.");
        require(quantity > 0 && quantity <= perTxQuantity, "Invalid Mint Quantity");
        require(UUIDMinted[UUID] + quantity <= perWalletQuantity, "Max NFT per wallet exceeded.");
        ...
        while(count > 0) {
            if(!_exists(publicMintId)){
                totalMinted++;
                publicMinted++;
                _safeMint(address(this), publicMintId);
    ...
    function getUUID_Data(bytes memory UUID, bytes32 paymentId) public onlyOwner view returns(uint256[] memory) {
