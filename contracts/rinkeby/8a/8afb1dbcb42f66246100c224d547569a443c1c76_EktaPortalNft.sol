    ...
    ReentrancyGuard
{
    ...
    address public freeAccessGranter = address(0);
    uint256 public maxMintPerAddress = 0;
    uint256 public price = 0;
    bool public isMintingPaused = true;
    uint256 public publicMintingStartTimestamp = 0;
        ...
        nonReentrant
    {
        ...
        // reserving seat for owner of freeAccessGranterNft before public minting
        if (msg.sender == owner()) {
            require(
        ...
        _batchMint(totalMint + _granterTokenIds.length, msg.sender);
    }

    /// @notice Claims and mints NFT in batch
    /// @dev implicit roles: Contract Owner, Granter Nft Owner, Normal User
    /// Contract Owner: can mint any quantity to fulfill max supply after minting period,
    /// capped by max supply
    /// Grater Nft Owner: has granter Nft(s) which will grant the user free Nft claims
    /// automatically executed along with mint operation. Has extra `maxMintPerAddress` payable mint right
    /// Normal User: needs to be whitelisted. Mint is capped by `maxMintPerAddress`.
    /// @param _claimQty number of free Nfts to claim
    /// @param _mintQty number of (payable) Nfts to mint
    // function batchMint(uint256 _claimQty, uint256 _mintQty)
    //     external
    //     payable
    //     nonReentrant
    // {
    ...
    //     uint256[] memory granterTokenIds = _getGranterTokenIds(msg.sender);
    //     require(
    //         granterTokenIds.length >= _claimQty,
    //         "Can not claim more than you can"
    //     );

    //     uint256 totalMint = _claimQty + _mintQty;

    //     // reserving seat for owner of freeAccessGranterNft before public minting
    //     bool isPublicMintingPeriod = publicMintingStartTimestamp <=
    //         block.timestamp;
    //     if (isPublicMintingPeriod || _claimQty > 0) {
    //         require(
    //             _currentTokenId() + totalMint <= maxSupply,
    //             "Exceeds max supply"
    //         );
    ...
    //     if (_claimQty > 0) {
    //         reservedFreeAccessMints -= _claimQty;
    //         addressClaimCount[msg.sender] += _claimQty;
    //         for (uint256 i; i < _claimQty; i++) {
    //             freeAccessGranterUsed[granterTokenIds[i]] = true;
    //         }
    //     }
    ...
    //     addressMintCount[msg.sender] += _mintQty;
    //     _batchMint(totalMint + granterTokenIds.length, msg.sender);
    // }
