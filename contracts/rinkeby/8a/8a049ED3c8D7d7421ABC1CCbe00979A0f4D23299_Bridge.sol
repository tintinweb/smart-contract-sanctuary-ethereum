...
contract Bridge {
    ...
    constructor(address _token, address validator){
        token = _token;
        _validator = validator;
    }
    
    function swap(uint256 nonce, uint256 amount) public {
        require(IERC20MintBurn(token).balanceOf(msg.sender)>=amount, "balance less than amount");
        
        bytes32 swapId = keccak256(abi.encodePacked(msg.sender, nonce));
        require(swaps[swapId] == false, "duplicate transaction nonce");

        swaps[swapId] = true;
        IERC20MintBurn(token).burn(msg.sender, amount);

        emit SwapInitialized(swapId, msg.sender, amount, nonce);
    }
    
    function redeem(
        bytes32 swapId,
        uint256 amount,
        bytes memory sign) public {

        bytes32 msgHash = keccak256(abi.encodePacked(swapId, amount));
        require(_validSignature(sign, msgHash), "invalid signature");
        
        require(swaps[swapId] == false, "duplicate redeem");

        swaps[swapId] = true;
        IERC20MintBurn(token).mint(msg.sender, amount);
