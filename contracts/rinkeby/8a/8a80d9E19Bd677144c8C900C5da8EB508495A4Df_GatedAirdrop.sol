...
contract GatedAirdrop is IGatedAirdrop, RequestGuildRole, Ownable {
    ...
    constructor(address token_, uint256 amount_, uint256 distributionDuration) {
    ...
    /// @inheritdoc IGatedAirdrop
    /// @dev TODO when we have a more suitable Guild endpoint: remove guildIndex paramters
    function claim(uint256 guildIndex) external {
        ...
        if (IERC20(token).balanceOf(msg.sender) < amount) revert OutOfTokens();
