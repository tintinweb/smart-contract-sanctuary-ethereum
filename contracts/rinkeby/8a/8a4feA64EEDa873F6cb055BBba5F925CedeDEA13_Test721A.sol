...
contract Test721A is Ownable, ERC721A, ReentrancyGuard {
    ...
    {
        if (roundIndex == 0) {
            return foundingMinterAllocate[minter];
        } else if (roundIndex == 1) {
            return favourMinterAllocate[minter];
        } else if (roundIndex == 2) {
            return whitelistMinterAllocate[minter];
        } else if (roundIndex == 3) {
            return publicMinterAllocate[minter];
        }
    }
    ...
    function whitelistMint(
        ...
        require(
            totalSupply() + quantity <= maxPerDurringRound,
        ...
        require(
            minterAllocate <= maxPerAddressDuringMint - minterAllocate,
    ...
    function publicMint(
        ...
        require(
            roundSaleStartTime >= block.timestamp,
        ...
        uint256 minterAllocate = getMinterAllocate(roundIndex, msg.sender);
        require(
            minterAllocate + quantity <=
                maxPerAddressDuringMint - minterAllocate,
            "Can not mint this many"
        );
        setMinterAllocate(roundIndex, quantity, msg.sender);
        _safeMint(msg.sender, quantity);
