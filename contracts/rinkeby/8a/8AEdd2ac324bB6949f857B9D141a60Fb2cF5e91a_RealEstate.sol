...
contract RealEstate is Pausable, Ownable {
    ...
    function addProperty(
        ...
        properties[propertyId] = _property(
            propertyId, _price, _folioNumber, _isAgency,
    ...
    function updateProperty(
        ...
        properties[_propertyId] = _property(
            _propertyId, _price, properties[_propertyId].folioNumber, _isAgency,
    ...
    function buyPropertyToken( uint256 _propertyId, string memory fullName, string memory phoneNumber, uint256 amount_,Tokens token_) public
     {
        ...
        if (token_ == Tokens.USDT) {
            ...
            uint256 percent2 = (properties[_propertyId].price * 2) / 100;
            uint256 totalUSDT = properties[_propertyId].price + percent2;
            require(totalUSDT == amount_, "not valid usdt");

            USDT.transferFrom(_msgSender(), agency(), percent2);
        ...
        if (token_ == Tokens.NNT) {
            ...
            uint256 percent2 = (PriceExchange_ * 2) / 100;
            uint256 totalNNT = PriceExchange_ + percent2;
            require(totalNNT == amount_, "not valid NNT");

            NNT.transferFrom(_msgSender(), agency(), percent2);
    ...
    function acceptingDeal(uint256 _propertyId) public onlyAgency {
        ...
        if (properties[_propertyId].isAgency == true) {
            ...
            if (paidIn[_propertyId] == Tokens.USDT) {
                ...
                uint256 _fee = _propertyPrice.mul(sellerAgencyFee).div(100);
            ...
            if (paidIn[_propertyId] == Tokens.NNT) {
                ...
                uint256 sellerAgencyFeeExchangedToNNT_ = ExchangeHandler.priceCalculatorUSDTtoNNT(sellerAgencyFee);
                uint256 _fee = _propertyPrice.mul(sellerAgencyFeeExchangedToNNT_).div(100);
                uint256 _transferPropertyOwner = _propertyPrice.sub(_fee);
        ...
        if (properties[_propertyId].isAgency == false) {
           ...
           if (paidIn[_propertyId] == Tokens.USDT) {
                ...
                uint256 _fee = _propertyPrice.mul(sellerIndividualFee).div(100);
