...
contract YDF is IYDF, ERC20, Ownable {
  ...
  uint256 public taxRewardsPerc = 100; // 1%
  uint256 public taxEarlyPerc = 1100; // 11%
  ...
  function _transfer(
    ...
    if (_isSell && !(isTaxExcluded[sender] || isTaxExcluded[recipient])) {
      ...
      if (
        ...
        taxMarketingPerc = 75; // 0.75%
        taxRewardsPerc = 25; // 0.25%
        taxEarlyPerc = 1400; // 14%
      }
      ...
      if (tax > 0) {
        ...
        if (_taxEarlyPerc > 0) {
          _burn(sender, tax - _taxAmountETH);
        }
      }
