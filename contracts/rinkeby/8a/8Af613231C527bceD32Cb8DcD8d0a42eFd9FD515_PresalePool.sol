...
contract PresalePool is OwnableUpgradeable, IPool , ReEntrancyGuard {
    ...
    function initialize(
        ...
        audit = _saleInfo[10] == 1 || audit_kyc[0] ? true : false;
        kyc =   _saleInfo[11] == 1 || audit_kyc[1] ? true : false;
    ...
    function getPoolInfo() external override view returns (address, uint8[] memory , uint256[] memory , string memory , string memory , string memory){
       ...
       return (token , state , info , IERC20(token).name() , IERC20(token).symbol() , poolDetails);
    }
    ...
    function withdrawContribution() external noReentrant {
        ...
        uint256 volume = PoolLibrary.convertCurrencyToToken(refundAmount, rate);
        totalVolumePurchased = totalVolumePurchased.sub(volume);
