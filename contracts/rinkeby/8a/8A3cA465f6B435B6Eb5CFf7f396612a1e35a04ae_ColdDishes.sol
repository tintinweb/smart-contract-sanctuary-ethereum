...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory ipfs = ownershipOf(tokenId).ipfs;
    ...
    function _mint(
        ...
        if (to == address(0)) revert MintToZeroAddress();
        if (quantity == 0) revert MintZeroQuantity();
        require(_metadata.length == quantity, "Metadata is not matched");
        ...
        unchecked {
            ...
            for (uint256 i=0; i < quantity; i++) {
                _ownerships[startTokenId].addr = to;
                _ownerships[startTokenId].startTimestamp = uint64(block.timestamp);
                _ownerships[updatedIndex].ipfs = _metadata[i];
                emit Transfer(address(0), to, updatedIndex);
                if (safe && !_checkOnERC721Received(address(0), to, updatedIndex, _data)) {
                    revert TransferToNonERC721ReceiverImplementer();
                }                
...
contract ColdDishes is ERC721A, Ownable {
    ...
    mapping (string => uint256) public mintedAmountOfTribe;
    ...
    constructor () ERC721A("3 Cold Dishes Test 02", "3CDSTEST02") {
    ...
    function mint(string memory tribe, uint256 quantity, string[] memory metadata) external payable {
        ...
        _mint(msg.sender, quantity, metadata, '', true);
    }
}