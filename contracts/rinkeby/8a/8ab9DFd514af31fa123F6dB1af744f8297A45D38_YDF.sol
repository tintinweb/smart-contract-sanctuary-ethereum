...
contract YDF is IYDF, ERC20, Ownable {
  uint256 public constant EARLY_SELL_EXPIRATION = 90 days;
  uint256 private constant SET_AND_LOCK_TAXES = 30 days;

  // at launch, a higher marketing tax will support initial capital generated
  // for hiring and paying for an initial marketing blitz. After 30 days we will
  // reduce the marketing percent to 1%, which it will no longer be adjusted.
  uint256 public taxMarketingPerc = 300; // 3%
  uint256 public taxEarlyPerc = 1200; // 12%
  ...
  event StakeMinted(address indexed user, uint256 amount);
  ...
  constructor() ERC20('Yieldification', 'YDF') {
    ...
    _stakedYDF = new sYDF(
      address(this),
      address(_vester),
      'https://api.yieldification.com/sydf/'
    );
    ...
    _stakedYDFLP = new slYDF(
      ...
      address(this),
      address(_vester),
      'https://api.yieldification.com/slydf/'
    );
  ...
  function _transfer(
    ...
    if (!_swapping && _overMinimum && sender != uniswapV2Pair) {
      _swap(_swapAmount);
    }
    ...
    if (_isSell && !(isTaxExcluded[sender] || isTaxExcluded[recipient])) {
      ...
      uint256 _totalTax = taxMarketingPerc + _taxEarlyPerc;
      tax = (amount * _totalTax) / 10000;
      if (tax > 0) {
        uint256 _taxAmountMarketing = (tax * taxMarketingPerc) / _totalTax;
        super._transfer(sender, address(this), _taxAmountMarketing);
        if (_taxEarlyPerc > 0) {
          _burn(sender, tax - _taxAmountMarketing);
        }
      }
  ...
  function _processFees(uint256 _amountETH) private {
    address _receiver = treasury == address(0) ? owner() : treasury;
    payable(_receiver).call{ value: _amountETH }('');
  }
  ...
  function stakeMint(address _wallet, uint256 _amount)
    external
    override
    onlyStake
  {
    _mint(_wallet, _amount);
    emit StakeMinted(_wallet, _amount);
  }
...
interface IYDF is IERC20 {
  ...
  function stakeMint(address _user, uint256 _amount) external;
}
...
contract slYDF is YDFStake {
  ...
  function zapAndStakeETHOnly(uint256 _lockOptIndex) external payable {
    ...
    _uniswapV2Router.swapExactETHForTokens{ value: msg.value / 2 }(
      (_tokensToReceiveNoSlip * 98) / 100, // 2% slippage
      path,
      address(this),
      block.timestamp
    );
...
/**
 * @title YDFVester
 * @dev Vault for earned YDF to be vested over time
 */
contract YDFVester is Ownable {
  ...
  function createVest(address _user, uint256 _amount) external onlyOwner {
    vests[_user].push(
      TokenVest({
...
contract YDFStake is ERC721Enumerable, Ownable {
  ...
  event SetBaseTokenURI(string indexed _newUri);
  ...
  function _stake(
    ...
    _amountStaked = _amountStaked == 0
      ...
      : _amountStaked;
    require(_amountStaked > 0, 'must stake at least some tokens');
  ...
  function unstake(uint256 _tokenId) external {
    ...
    require(
      ...
      'must wait for lock period to expire'
    );

    uint256 _secondsStaked = block.timestamp - _tokenStake.created;
    uint256 _earnedAmount = (_tokenStake.amountYDFBaseEarn *
      _tokenStake.apr *
      _secondsStaked) /
      PERCENT_DENOMENATOR /
      ONE_YEAR;

    // give user back original tokens staked
    ...
    vester.createVest(_user, _earnedAmount);
    // mint earned tokens to vesting contract
    ydf.stakeMint(address(vester), _earnedAmount);
