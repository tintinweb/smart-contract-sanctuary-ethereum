...
import "../metatx/SmartRightsContext.sol";

contract SmartRightsNFT is SmartRightsContext, ERC165, IERC721, IERC721Metadata {
    ...
    constructor(
        ...
        string memory _uri,
        uint256 _royaltyFeeInBips, 
        address _wallet, 
        address _minter
    ) SmartRightsContext(_forwarder) {      
        name = _name;
        symbol = _symbol;
        uri = _uri;
        royaltyFeeInBips = _royaltyFeeInBips;
        royaltiesWallet = _wallet;
        minter = _minter;
        owner = _msgSender();
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        return string(abi.encodePacked(uri, uint256(_tokenId).toString(), ".json"));
    }
    ...
    function royaltyInfo(uint256 /*_tokenId*/, uint256 _salePrice) external view virtual returns (address, uint256) {
        return (royaltiesWallet, (_salePrice * royaltyFeeInBips) / 10000);
    }
    ...
    function safeMint(address _to, uint256 _tokenId) external virtual {
        require(_msgSender() == minter, "OnlyMinter");
...
contract SmartRightsContext is ERC2771Context {
