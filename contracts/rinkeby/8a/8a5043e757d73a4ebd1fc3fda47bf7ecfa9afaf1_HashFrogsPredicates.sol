...
contract HashFrogsPredicates is Ownable, ICollectionNFTEligibilityPredicate, ICollectionNFTMintFeePredicate, ICollectionNFTTokenURIPredicate {
    ...
    /**
     * @notice This function adds an implementation address.
     * @param variablesExist The ecosystem which this implementation address will reference.
     * @param tokenSymbol The address of the Collection contract.
     * @param baseTokenURI Whether this implementation address is cloneable.
     */
    function buildCustomInitializationDataV1(
        bool[8] memory variablesExist,
       /* bool tokenSymbolExists,
        bool baseTokenURIExists,
        bool capExists,
        bool mintEligibilityPredicateContractExists,
        bool mintFeePredicateContractExists,
        bool royaltyBpsExists,
        bool signatureBlockAddressExists, */
        string memory tokenName,
        string memory tokenSymbol,
        string memory baseTokenURI,
        uint256 cap,
        ...
        if (variablesExist[0] == true) {
            _initializerSettings.tokenName = tokenName;
        }
        if (variablesExist[1] == true) {
            _initializerSettings.tokenSymbol = tokenSymbol;
        }
        if (variablesExist[2] == true) {
            _initializerSettings.baseTokenURI = baseTokenURI;
        } 
        if (variablesExist[3] == true) {
            _initializerSettings.cap = cap;
        }
        if (variablesExist[4] == true) {
           _initializerSettings.mintEligibilityPredicateContract = mintEligibilityPredicateContract;
        }
        if (variablesExist[5] == true) {
            _initializerSettings.mintFeePredicateContract = mintFeePredicateContract;
        }
        if (variablesExist[6] == true) {
           _initializerSettings.royaltyBps = royaltyBps;
        }
        if (variablesExist[7] == true) {
            _initializerSettings.signatureBlockAddress = signatureBlockAddress;
        } 
    ...
    function buildCustomInitializationDataV2(
        string memory tokenName,
        string memory  tokenSymbol,
        address tokenURIPredicateContract,
        uint256 cap,
