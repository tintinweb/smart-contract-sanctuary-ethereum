...
contract atomverse_staking is Ownable, IERC721Receiver {
    ...
    function balanceOf(address account) public view returns (uint256) {
        ...
        for (uint256 i = 1; i <= supply; i++) {
            if (cyroChambers[i].owner == account) {
                balance += 1;
            }
        }
        return balance;
        ...
        returns (uint256[] memory ownerTokens)
    {
        ...
        for (uint256 tokenId = 1; tokenId <= 4444; tokenId++) {
            if (cyroChambers[tokenId].owner == account) {
                tmp[index] = cyroChambers[tokenId].tokenId;
                index += 1;
            }
        }
    ...
    function unStakeAll() external onlyOwner{
        ...
        for (uint256 tokenId = 1; tokenId <= 4444; tokenId++) {
            ...
            if (staked.tokenId!=0){
                ...
                atomverse_nft.transferFrom(address(this), staked.owner, tokenId);
            }
