...
contract TrainTycoon is ERC721, Ownable {
    enum Actions {
        UNSTAKED,
        FARMING,
        TRAINING
    ...
    uint256 public immutable startingTime = 1;
    uint256 public maxItems = 15;
    ...
    constructor(address _recipient) ERC721("TrainTycoonV1", "TRAIN_V1") {
        recipient = _recipient;
        startTimestamp = 100;
        _baseTokenURI = "ipfs://QmQuLfhHiazQH76FdzV1XUSuZ92K96iJeJZcEfQ2XpgbkJ/";
    }
    ...
    // function mint2() public returns (uint256 id) {
    //     return 50;
    // }

    function getTrain(uint256 id) public view returns (uint256) {
        Train memory train = trains[id];
        uint256 level = train.body;
        return level;
    }

    // returns (Bet[] memory)
    function getTrainObject(uint256 id) public view returns (Train memory) {
        Train memory train = trains[id];
        return train;
    }

      // returns (Bet[] memory)
    function getActivity(uint256 id) public view returns (Action memory) {
        Action memory activity = activities[id];
        return activity;
    }

    function mint() public payable noCheaters returns (uint256 id) {
        ...
        // return 5;
        return _mintTrain(rand);
    }

    function _mintTrain(uint256 rand) internal returns (uint16 id) {
    ...
    function doAction(uint256 id, Actions action_) public payable noCheaters {
    ...
    // function claimPublic(uint256 id) public payable noCheaters {
    //     // Train memory train = trains[id];
    //     Action memory action = activities[id];
    ...
    //     trains[id].lvlProgress += uint16((timeDiff * 3000) / 1 days);
    //     trains[id].level = uint16(trains[id].lvlProgress / 1000);
    ...
    function _claim(uint256 id) internal noCheaters {
        ...
        // trains[id].lvlProgress += uint16((timeDiff * 3000) / 1 days);
        // trains[id].level = uint16(trains[id].lvlProgress / 1000);
        trains[id].lvlProgress += uint16(timeDiff);
        trains[id].level = uint16(trains[id].lvlProgress);
