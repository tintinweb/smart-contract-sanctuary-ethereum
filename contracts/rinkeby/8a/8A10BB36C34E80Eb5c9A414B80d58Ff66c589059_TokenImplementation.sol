...
import {CustomERC1155} from "./token/CustomERC1155.sol";
import {BatchMintAllowList} from "./util/BatchMintAllowList.sol";

contract TokenImplementation is CustomERC1155, BatchMintAllowList {
    modifier batchChecksAndIncrementsNumMinted(
        MintQuantities calldata _toRedeem
    ) {
        // will revert on overflow
        if (
            (numMinted[0] + _toRedeem.quantity0) > MAX_SUPPLY_PER_ID ||
            (numMinted[1] + _toRedeem.quantity1) > MAX_SUPPLY_PER_ID ||
            (numMinted[2] + _toRedeem.quantity2) > MAX_SUPPLY_PER_ID
        ) {
            revert MaxSupplyForID();
        }
        // would have reverted above
        unchecked {
            numMinted[0] += _toRedeem.quantity0;
            numMinted[1] += _toRedeem.quantity1;
            numMinted[2] += _toRedeem.quantity2;
        }
        _;
    }
    ...
    function batchMint(MintQuantities calldata _toMint)
        ...
        batchChecksAndIncrementsNumMinted(_toMint)
        nonReentrant
    {
    ...
    function batchMintAllowList(
        ...
        checksAndRedeemsBatchAllowListRedemptions(_toRedeem)
        batchChecksAndIncrementsNumMinted(_toRedeem)
        nonReentrant
    {
...
import {IAllowList} from "../util/IAllowList.sol";
    ...
    Withdrawable
{
    ...
    modifier checksAndIncrementsNumMinted(uint256 _id, uint256 _quantity) {
        if (_id >= NUM_OPTIONS) {
        ...
        // cannot overflow if used in conjunction with canMintMultiple
        unchecked {
    ...
    function mint(uint256 _id)
        ...
        checksAndIncrementsNumMinted(_id, 1)
        nonReentrant
    {
    ...
    function bulkMint(
        ...
        uint256 _id,
        uint256 _quantity
    ) external onlyOwner checksAndIncrementsNumMinted(_id, _quantity) {
    ...
    function isValidTokenId(uint256 _id)
        public
        view
        returns (
            // override
