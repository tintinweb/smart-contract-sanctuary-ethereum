...
contract MetroMiniBlock is ERC721ExtraStorage, Controllable, IMetroBlockInfo {
    ...
    function metMint(uint256 amount) public {
        ...
        if (metMinted > MAX_MET_BLOCKS) {
            uint256 tradeMintRemainder = MAX_TRADE_BLOCKS - tradeMinted;
            require(metMinted <= MAX_MET_BLOCKS + tradeMintRemainder, "Exceeded max MET block count");
            ++tradeMinted; // Consume trade mint slot
        }
    ...
    function ethMint(bytes32 signatureR, bytes32 signatureVS, uint256 amount) public payable {
        ...
        _mintBlock(amount, uint224(whitelistTotal));
    }
