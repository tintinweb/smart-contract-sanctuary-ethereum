...
contract LoopopGoldenArtistCardTest is LoopopCardBase {

    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor(address burner_) LoopopCardBase(burner_, "Loopop Golden Artist Card test", "LCACt") {
    }
...
contract LoopopCardBase is LoopopTokenBase {
    ...
    function safeBatchUsed(address from, uint256[] memory tokenIds) public onlyRole(USE_ROLE) {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(_isApprovedOrOwner(from, tokenIds[i]), "caller is not owner nor approved");
            require(!_isUsed[tokenIds[i]], "token had already used");
            _isUsed[tokenIds[i]] = true;
        }
    }

    function safeBatchRecycle(address from, uint256[] memory tokenIds) public onlyRole(USE_ROLE) {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(_isApprovedOrOwner(from, tokenIds[i]), "caller is not owner nor approved");
            require(!_isUsed[tokenIds[i]], "card is used");
            _safeTransfer(from, burner, tokenIds[i], "");
        }
    }
