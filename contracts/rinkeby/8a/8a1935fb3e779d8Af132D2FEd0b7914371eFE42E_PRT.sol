...
contract PRT is Ownable, ReentrancyGuard {
    ...
    function raffle1stStage(uint256 number, uint i) public payable returns(uint256){
        
        //gold supply = ID 1-200, silver supply = 201-2000, bronze = 2001-20000
        //uint8 randnum = uint8(random(255));
        uint256 v = (number>>(i*5));
        uint8 randnum = uint8(v%255);
        //uint8 randval = uint8(random(MAX_SUPPLY_FOR_TOKEN/NUM_TOTAL)); //0 to 19
        uint8 randval = uint8(v%(MAX_SUPPLY_FOR_TOKEN/NUM_TOTAL)); //0 to 19

        if (randval == 0) {
            if (uint8(randnum % 9) == 1) {
                if (intArr[randval] == 96) {
                    randval = uint8(randnum%(MAX_SUPPLY_FOR_TOKEN/NUM_TOTAL-1)+1);
                }
            } else  {
                randval = uint8(randnum%(MAX_SUPPLY_FOR_TOKEN/NUM_TOTAL-1)+1);
            }
...
function asciiToInteger(bytes32 x) public pure returns (uint256) {
    ...
    function sendMP() public onlyAccounts onlyOwner presalePRTIsDone {
        uint xrand = createXRAND(17);
        uint256 randnum;
         for (uint i = 0; i < 10000; i++) {

            if (i%51 == 0) {
                randnum = (uint256(blockhash(block.number-(i+1))));
            }

            uint24 _winnerTokenPRTID = uint24(PRTID + 1 + xrand + uint32(168888*i)/10000); 
            address winneraddr = getAddrFromPRTID(_winnerTokenPRTID);
            uint winTokenID = raffle1stStage(randnum,i);
            //if (winneraddr != address(0)) {
                //emit RTWinnerAddress(winneraddr, winTokenID); //this needs to be 10000+i <- and i needs to be random also, 1st stage sale
                //emit RTWinnerAddress(address(uint160(randnum)), winTokenID); //this needs to be 10000+i <- and i needs to be random also, 1st stage sale
            //}
            //emit RTWinnerTokenID(20000-raffle1stStage(), _winnerTokenPRTID);//in case to track all winer tickets in logs         
            emit RTWinnerTokenID(winTokenID, _winnerTokenPRTID);//in case to track all winer tickets in logs         
        }
    }
