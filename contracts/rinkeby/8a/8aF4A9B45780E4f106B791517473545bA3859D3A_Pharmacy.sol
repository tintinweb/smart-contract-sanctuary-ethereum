...
import "./DickEnlargeningPill.sol";

contract Pharmacy is Context, AccessControlEnumerable, Pausable {
    ...
    event PillPurchase(address by, uint256 quantity);
    event LiquidityCollectorUpdate(
        address collectorBefore,
        address collectorAfter,
        address initiator
    );
    ...
    constructor(
        ...
        require(
            _supplyCap > 0 && _price > 0,
            "Distributor: Need to define total pill stock on deploy"
        );
    ...
    function purchasePills(uint256 quantity) external payable whenNotPaused {
        uint256 priceToPay = price * quantity;

        require(msg.value >= priceToPay, "Pharmacy: Insufficient amount paid");
        require(
            quantity <= MAX_PER_TX,
            "Pharmacy: Quantity exceeding max quantity per transaction"
        );
        require(
        ...
        emit PillPurchase(_msgSender(), quantity);

        liquidityCollector.transfer(priceToPay);
    }

    function updateLiquidityCollector(address payable newCollector)
        external
        whenNotPaused
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        emit LiquidityCollectorUpdate(
            liquidityCollector,
            newCollector,
            _msgSender()
        );
    ...
    Ownable
{
    ...
    event URIUpdate(string URIBefore, string URIAfter, address initiator);
    event RoyaltyUpdate(
        address receiverBefore,
        uint256 royaltyBefore,
        address receiverAfter,
        uint256 royaltyAfter,
        address initiator
    );
    ...
    function use(uint256 pillId, uint256 targetId) external {
        require(
            ownerOf(pillId) == _msgSender(),
            "DickEnlargeningPill: Pill use failed: Not the owner of the pill"
        );

        require(
            collection.ownerOf(targetId) == _msgSender(),
            "DickEnlargeningPill: Pill use failed, not the owner of the target Id"
        );
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "DickEnlargeningPill: URI query for nonexistent token"
        );
    ...
    function updateURI(string calldata _metaURI)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        emit URIUpdate(metaURI, _metaURI, _msgSender());
        ...
        returns (LibPart.Part[] memory)
    {
        require(
            _exists(_tokenId),
            "Enlargement Pill: Cannot get royalties for non-existing token"
        );
    ...
    function _updateRoyalty(
        ...
        require(
            ...
            "Enlargement Pill: Cannot set royalties above 10%"
        );
...
interface IDickManiac {
    ...
    function emergencySwitch() external;

    function releaseEmergencySwitch() external;
    ...
    Ownable
{
    ...
    function mint(address to) external whenNotPaused onlyRole(MINTER_ROLE) {
        sizeChart[tokenTracker.current()] = 1;
        _mint(to, tokenTracker.current());
        tokenTracker.increment();
    }

    function grow(uint256 tokenId)
        external
        whenNotPaused
        onlyRole(ENLARGEMENT_ROLE)
    {
        require(
            _exists(tokenId),
            "DickManiac: Can not apply enlargement: token Id not exisiting"
        );

        require(
            sizeChart[tokenId] < MAX_SIZE,
            "DickManiac: Can not apply enlargement: maximum size already reached"
        );
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "DickManiac: URI query for nonexistent token"
        );
    ...
    function getSize(uint256 tokenId) public view returns (uint256) {
        require(
            tokenId < tokenTracker.current(),
            "DickManiac: TokenId not existing"
        );
    ...
    // Return same rarible royalty for all
    function getRaribleV2Royalties(uint256 _tokenId)
        external
        view
        returns (LibPart.Part[] memory)
    {
        require(
            _exists(_tokenId),
            "Dick Maniac: Cannot get royalties for non-existing token"
        );
    ...
    function _updateRoyalty(
        ...
        require(
            _royaltyBasisPoints <= 1000,
            "Dick Maniac: Cannot set royalties above 10%"
        );
