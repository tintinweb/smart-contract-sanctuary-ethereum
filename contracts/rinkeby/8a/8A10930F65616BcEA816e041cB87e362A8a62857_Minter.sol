...
contract Minter is Ownable, ReentrancyGuard {
    ...
    function claimById(address uAssetAddress, bytes memory id) public {
    ...
    function liquidation(address userAddress, address uAssetAddress, uint256 uAssetAmount, bytes memory id) external nonReentrant {
        ...
        if (discountedCollateralValue <= collateralBalances[id]) {
            ...
            uint256 refundUAssetAmount = redundant * (10**calculationDecimal - discountRate) / ((10**calculationDecimal) * targetPrice) * ERC20(uAssetAddr).decimals();
            ...
            emit Liquidation(msg.sender, user, id_, sub1, sub2, block.timestamp);
        }
    }
