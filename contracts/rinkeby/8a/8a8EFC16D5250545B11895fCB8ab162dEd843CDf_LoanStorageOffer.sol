...
contract LoanStorageOffer is Ownable {
    ...
    /// @dev offer[paymentId] => LoanPayment
    mapping(uint256 => LoanCommon.Offer) public offerMapping;

    /// @notice Offer tracking ID
    uint256 public offerIdTracker = 0;

    /// @dev loanOfferMapping[loanId] => unit256[] list of offers for a particular loan
    mapping(uint256 => uint256[]) public loanOfferMapping;

    /// @notice get a list of offer IDs by loan ID
    /// @param _loanId The Loan ID
    function offersByLoanId(uint256 _loanId) public view returns(uint256[] memory) {
        return loanOfferMapping[_loanId];
    }

    /// @notice loan offer activities
    enum OfferEvent {
        CREATED,                    // 0
        ACCEPTED_AMOUNT_UPDATED,    // 1
        STATUS_UPDATED,             // 2
        OFFER_UPDATED               // 3
    }

    /// @notice loan offer event
    event OfferActivity(uint256 indexed offerId, uint256 indexed loanId, OfferEvent activity, uint256 timestamp);
    ...
    modifier onlyServiceContractOrOwner() {
        ...
        require(
            msg.sender == loanServiceOfferAddress || msg.sender == owner(),
            "caller must be service contract or owner"
        );
        _;
    }

    /// @notice get an offer object by offer ID
    /// @param _offerId The Offer ID
    function offers(uint256 _offerId) public view returns(LoanCommon.Offer memory) {
        return offerMapping[_offerId];
    }

    /// @notice Make payment on a loan
    /// @param _lender The wallet address of the lender
    /// @param _loanId The ID of the loan
    /// @param _offeredAmount The amount to be applied to the loan
    /// @param _interest The interest for this offer
    function create(address _lender, uint256 _loanId, uint256 _offeredAmount, uint256 _interest) public onlyServiceContractOrOwner {
        offerIdTracker++;
        offerMapping[offerIdTracker] = LoanCommon.Offer({
            offerId: offerIdTracker,
            loanId : _loanId,
            lender : _lender,
            offeredAmount : _offeredAmount,
            interest : _interest,
            acceptedAmount: 0,
            status: LoanCommon.OfferStatus.PENDING,
            createdDate : block.timestamp
        });
        loanOfferMapping[_loanId].push(offerIdTracker);

        // Emit offer created event
        emit OfferActivity(offerIdTracker, _loanId, OfferEvent.CREATED, block.timestamp);
    }

    /// @notice Update the accepted amount
    /// @dev when a loan is activated the expectation is the accepted amount is updated
    /// @param _offerId The offer id
    /// @param _acceptedAmount The accepted amount
    function updateAcceptedAmount(uint256 _offerId, uint256 _acceptedAmount) public onlyServiceContractOrOwner {
        LoanCommon.Offer storage loanOffer = offerMapping[_offerId];

        loanOffer.acceptedAmount = _acceptedAmount;

        // Emit offer updated event
        emit OfferActivity(offerIdTracker, loanOffer.loanId, OfferEvent.ACCEPTED_AMOUNT_UPDATED, block.timestamp);
    }

    /// @notice Update the loan status
    /// @param _offerId The Offer ID
    /// @param _status The new status value
    function updateStatus(uint256 _offerId, LoanCommon.OfferStatus _status) public onlyServiceContractOrOwner {
        LoanCommon.Offer storage loanOffer = offerMapping[_offerId];
        loanOffer.status = _status;

        // Emit offer updated event
        emit OfferActivity(offerIdTracker, loanOffer.loanId, OfferEvent.STATUS_UPDATED, block.timestamp);
    }

    /// @notice update the offered and interest amount for an offer
    /// @param _offerId the offer id
    /// @param _offeredAmount the offered amount of the bid
    /// @param _interest the interest amount of the bid
    function updateOffer(uint256 _offerId, uint256 _offeredAmount, uint256 _interest) public onlyServiceContractOrOwner {
        LoanCommon.Offer storage offer = offerMapping[_offerId];
        offer.offeredAmount = _offeredAmount;
        offer.interest = _interest;
        emit OfferActivity(_offerId, offer.loanId, OfferEvent.OFFER_UPDATED, block.timestamp);
    }
}
