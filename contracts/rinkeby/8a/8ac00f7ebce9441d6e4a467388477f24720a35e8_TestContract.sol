...
contract TestContract is ERC721, Ownable {
    ...
    string internal baseURI; // internal?
    bytes32 private raffleRoot; // private? / initialize
    bytes32 private whitelistRoot; // private? / initialize
    bytes32 private specialWhitelistRoot; // private? / initialize
    ...
    constructor() ERC721("TestContract", "TC") {
        ...
        baseURI = "ipfs://bafybeifejh4t6hguys6vnsuou4pg35iwyzuro2raofsf55t5oa26gr2ppe"; // initialize
        raffleRoot = 0xd4a9c314054fa3ccf75d9dde53317b6316f957c335b9dbbd75c4db3d8725e4df; // initialize / type?
        whitelistRoot = 0xe799cf687d10d9c87dabbae3953645f28d1a799c62167a56ef2f0c0c7a2f8fec; // initialize / type?
        specialWhitelistRoot = 0x9cb405fc04050ce34935950dd509aa037def27069f9a9866a6c39e45b8c20096; // initialize / type?
    }

    function isOnAddressList(bytes32[] memory proof, bytes32 leaf, bytes32 root) public view returns (bool) {
    ...
    modifier mintRequirements(bytes32[] memory proof, uint256 mintCount) {
        ...
        require(tokenCounter < (maxSupply - vault), "All of YommsFrens have been sold out!");
        require(mintCount + tokenCounter <= (maxSupply - vault), "There is only 1 more YommsFrens NFT left. Please cancel and mint 1 YommsFrens NFT only.");

        address msgSender = msg.sender;

        if (mintState == MintState.Raffle) {
            require(mintState == MintState.Raffle, "There is no minting at this time.");
            require(mintCount + tokenCounter <= raffleMaxSupply, "There is only 1 more YommsFrens NFT left for Raffle Mint. Please cancel and mint 1 YommsFrens NFT only.");

            require(isOnAddressList(proof, keccak256(abi.encodePacked(msgSender)), raffleRoot), "Your address is not on the raffle list!");

            require((balanceOf(msgSender) < raffleMintPerWallet) && (mintCount <= raffleMintPerWallet), "You have no more mints left or you are going over your minting limits!");
            ...
            require(mintState == MintState.Whitelist, "There is no minting at this time.");
            require(mintCount + tokenCounter <= whitelistMaxSupply, "There is only 1 more YommsFrens NFT left for Whitelist Mint. Please cancel and mint 1 YommsFrens NFT only.");
            ...
            if (isOnAddressList(proof, keccak256(abi.encodePacked(msgSender)), whitelistRoot)) {
                require((balanceOf(msgSender) < whitelistMintPerWallet) && (mintCount <= whitelistMintPerWallet), "You have no more mints left or you are going over your minting limits!");
            } else if (isOnAddressList(proof, keccak256(abi.encodePacked(msgSender)), specialWhitelistRoot)) {
                require((balanceOf(msgSender) < 2)  && (mintCount <= 2), "You have no more mints left or you are going over your minting limits!");
            }
            ...
            require(mintState == MintState.Public, "There is no minting at this time.");

            require((balanceOf(msgSender) < publicMintPerWallet)  && (mintCount <= publicMintPerWallet), "You have no more mints left or you are going over your minting limits!");
