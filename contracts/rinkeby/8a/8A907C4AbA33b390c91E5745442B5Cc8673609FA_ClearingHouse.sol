...
contract ClearingHouse is IClearingHouse, IncreOwnable, Pausable, ReentrancyGuard {
    ...
    function withdraw(uint256 amount, IERC20Metadata token) external override nonReentrant whenNotPaused {
        ...
        if (!_isMarginValid(msg.sender, minMarginAtCreation)) revert ClearingHouse_WithdrawInsufficientMargin();
    }
    ...
    /// @param isReducingPosition Is reducing the position
    ...
    function changePosition(
        ...
        _changePosition(idx, amount, minAmount, direction, isReducingPosition);
    }
    ...
    function extendPositionWithCollateral(
        ...
        _changePosition(idx, positionAmount, minAmount, direction, false);
    }
    ...
    function closePositionWithdrawCollateral(
        ...
        _changePosition(idx, proposedAmount, minAmount, closeDirection, true);
        ...
        if (!_isMarginValid(msg.sender, minMargin)) revert ClearingHouse_WithdrawInsufficientMargin();
    }
    ...
    function liquidate(
        ...
        if (isTrader) {
            (, positiveOpenNotional) = perpetuals[idx].getTraderPositionHealth(liquidatee);
            ...
            positiveOpenNotional = perpetuals[idx].getLpOpenNotional(liquidatee);
        }

        // update funding rate, so that the marginRatio is correct
        int256 fundingPayments = perpetuals[idx].settleTrader(liquidatee) + perpetuals[idx].settleLp(liquidatee);
        vault.settlePnL(liquidatee, fundingPayments);

        if (_isMarginValid(liquidatee, minMargin)) revert ClearingHouse_LiquidateValidMargin();
        ...
        vault.settlePnL(liquidator, liquidationRewardAmount.toInt256());
    ...
    function seizeCollateral(address liquidatee) external override nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < perpetuals.length; i++) {
            if (perpetuals[i].isTraderPositionOpen(liquidatee)) revert ClearingHouse_seizeCollateralStillOpen();
            if (perpetuals[i].isLpPositionOpen(liquidatee)) revert ClearingHouse_seizeCollateralStillOpen();
        }
    ...
    function provideLiquidity(
        ...
        _provideLiquidity(idx, amount, minLpAmount);
    }
    ...
    function removeLiquidity(
        ...
        vault.settlePnL(msg.sender, profit + fundingPayments);
    ...
    /// @notice Governance doesn't pay trading fees
    /// @param idx Index of the perpetual market
    /// @param proposedAmount Amount of tokens to be sold, in vBase if LONG, in vQuote if SHORT. 18 decimals
    /// @param minAmount Minimum amount that the user is willing to accept, in vQuote if LONG, in vBase if SHORT. 18 decimals
    function sellDust(
        ...
        IERC20Metadata(vault.UA()).safeTransfer(msg.sender, IERC20Metadata(vault.UA()).balanceOf(address(this)));
    ...
    /// @notice Calculate free collateral of a liquidity provider
    /// @param account Trader to get the funding payments
    /// @param isDiscounted Whether the free collateral should be discounted or not
    /// @return freeCollateral in USD. 18 decimals
    function getFreeCollateralLiquidity(address account, bool isDiscounted) public view override returns (int256) {
        /*
            collateral - freeCollateral + fundingPayments + unrealizedPnl / | notional | = minMarginAtCreation
        =>  freeCollateral = collateral + fundingPayments + unrealizedPnl - | notional | * minMarginAtCreation
        =>  freeCollateral = collateral + fundingPayments + unrealizedPnl - | notional |

        Note: The formula used is a bit different than the one for margin ratio in that:
              1. we don't multiply by the minMarginAtCreation
              2. isDiscounted allows to get a discounted value of the user's collaterals
        */

        (int256 absOpenNotional, int256 unrealizedPositionPnl, int256 fundingPayments) = getPositionAcrossMarkets(
            account
        );

        int256 collateral = _getReserveValue(account, isDiscounted);

        // if no trading or LP position open on any market, free collateral is equal to collateral
        if (absOpenNotional == 0) {
            return collateral;
        }

        return collateral + fundingPayments + unrealizedPositionPnl - absOpenNotional;
    }
        ...
        )
    {
        for (uint256 i = 0; i < getNumMarkets(); i++) {
            (int256 traderUnrealizedPositionPnl, int256 traderAbsOpenNotional) = perpetuals[i].getTraderPositionHealth(
                account
            );

            (int256 lpUnrealizedPositionPnl, int256 lpAbsOpenNotional) = perpetuals[i].getLpPositionHealth(account);

            absOpenNotional += traderAbsOpenNotional + lpAbsOpenNotional;
            unrealizedPositionPnl += traderUnrealizedPositionPnl + lpUnrealizedPositionPnl;

            fundingPayments +=
                perpetuals[i].getTraderFundingPayments(account) +
                perpetuals[i].getLpFundingPayments(account);
        }
    }
    ...
    function _liquidateLp(
        ...
        /* check is already performed in the Perpetual Contract */
        return pnL;
    }
    ...
    function _changePosition(
        ...
        (int256 addedOpenNotional, int256 addedPositionSize, int256 profit, bool isPositionIncreased) = perpetuals[idx]
            .changePosition(msg.sender, amount, minAmount, direction, isReducingPosition);
        ...
        if (!_isMarginValid(msg.sender, minMarginAtCreation)) revert ClearingHouse_ExtendPositionInsufficientMargin();
    ...
    /// @param idx Index of the perpetual market
    /// @param amount Collateral amount to add to the vault in vQuote (i.e. USD). 18 decimals
    /// @param minLpAmount Minimum amount of Lp tokens minted. 18 decimals
    function _provideLiquidity(
        ...
        int256 freeCollateralUSD = getFreeCollateralLiquidity(msg.sender, true);
        if (amount.toInt256() > freeCollateralUSD) revert ClearingHouse__AmountProvidedTooLarge();

        int256 fundingPayments = perpetuals[idx].settleLp(msg.sender);
        int256 tradingFees = perpetuals[idx].provideLiquidity(msg.sender, amount, minLpAmount);

        if (fundingPayments != 0 || tradingFees != 0) {
            vault.settlePnL(msg.sender, fundingPayments + tradingFees);
        }

        _isOpenNotionalRequirementValid(idx, msg.sender, false);

        emit LiquidityProvided(idx, msg.sender, amount);
    }
    ...
    function _isMarginValid(address account, int256 ratio) internal view returns (bool) {
        return _marginRatio(account) >= ratio;
    }
    ...
    function _marginRatio(address account) internal view returns (int256) {
        ...
        int256 absOpenNotional;
        int256 unrealizedPositionPnl;
        uint256 perpLength = perpetuals.length;
        for (uint256 i = 0; i < perpLength; i++) {
            (int256 traderUnrealizedPositionPnl, int256 traderAbsOpenNotional) = perpetuals[i].getTraderPositionHealth(
                account
            );
        ...
        int256 collateral = _getReserveValue(account, false);

        return _computeMarginRatio(collateral, unrealizedPositionPnl, absOpenNotional);
    }
    ...
    function _computeMarginRatio(
        ...
        return (collateral + unrealizedPositionPnl).wadDiv(absOpenNotional);
    }
}
