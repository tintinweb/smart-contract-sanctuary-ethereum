...
contract CollateralManager is ICollateralManager, Ownable {
    ...
    mapping(address => mapping(address => collateral))
        internal collateralLookup;
     ...
     * @param _toAdd is the collaterall to add
     */
    function addAcceptedCollateral(address[] memory _toAdd) external onlyOwner {
     ...
     * @param _toRemove is the collaterall to remove
     */
    function removeAcceptedCollateral(address[] memory _toRemove)
        external
        onlyOwner
    {
    ...
    * @param _from who MUST BE APPROVED
    * @param _erc20 address of token to send
    * @param _amount to deposit
    ...
    function deposit(
        ...
        if (collateralLookup[msg.sender][_from].amount == 0) {
            collateralLookup[msg.sender][_from] = collateral(
                block.timestamp,
                _erc20,
                _amount
            );
    ...
    * @param _receiver who receives the withdrawln collateral
    ...
    function withdrawal(address _user, uint256 _amount, address _receiver) external override{
        ...
        collateral storage c = collateralLookup[msg.sender][_user];
...
interface ICollateralManager{
    function getCollateralLookup(address, address) external view returns(address, uint);
    function deposit(address, address, uint) external;
    function withdrawal(address, uint256, address) external;
}
