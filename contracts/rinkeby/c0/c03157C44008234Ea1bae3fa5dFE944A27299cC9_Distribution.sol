...
contract Distribution is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, ContextUpgradeable {
    ...
    function claim(uint256 _saleId) external nonReentrant { 
        SaleInfo memory _saleInfo = ISale(sale).getSale(_saleId); 
        require(_saleInfo.id > 0, "Invalid sale id"); 
 
        Buyer storage buyer = buyers[_saleId][_msgSender()]; 
 
        ProjectInfo memory _projectInfo = IProject(project).getProject(_saleInfo.projectId); 
        require(block.timestamp >= _projectInfo.distributionStart, "claim: distribution is not start"); 
        require(buyer.account == _msgSender(), "claim: caller is not the buyer"); 
        if (_projectInfo.isSingle) { 
            IOSB721(_projectInfo.token).safeTransferFrom(address(this), _msgSender(), _saleInfo.tokenId); 
        } else { 
            uint256 amount = buyer.amount; 
            buyer.amount = 0; 
            IOSB1155(_projectInfo.token).safeTransferFrom(address(this), _msgSender(), _saleInfo.tokenId, amount, ""); 
        } 
    }
