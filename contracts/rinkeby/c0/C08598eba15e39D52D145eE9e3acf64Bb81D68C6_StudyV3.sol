...
contract StudyV3 is ERC721Enumerable,ERC2981,Ownable,Pausable,ReentrancyGuard{
  ...
  constructor() ERC721("StudyV3", "SV3") {
  ...
  function stakeMint(uint256 quantity) public nonReentrant {
    ...
    require(dailyPointmap[msg.sender] >= quantity*stakeMintPoint);
    decreasePoint(quantity*stakeMintPoint);
  ...
  function stake() public{
    dailyPointmap[msg.sender]=dailyPointmap[msg.sender]+checkSrakePont(msg.sender);
  ...
  function checkSrakePont(address _address) public view returns (uint256) {
      uint256 point = 1;
      for (uint256 i; i < at_Contract.walletOfOwner(_address).length; i++) {
        if (stampOwnerMap[at_Contract.walletOfOwner(msg.sender)[i]] == _address){
          point=point+(block.timestamp-timeStampMap[at_Contract.walletOfOwner(msg.sender)[i]]);
        }
      }
    return point/60;
  }
  function setStakeMintPoint(uint256 _newPoint) public onlyOwner {
        stakeMintPoint = _newPoint;
  }
