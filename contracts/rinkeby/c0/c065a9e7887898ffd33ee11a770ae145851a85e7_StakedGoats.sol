...
interface IGoatGauds is IERC721 {
...
contract StakedGoats is Delegated, ERC721EnumerableLite {
    using Strings for uint;

    uint256 constant token = 1e18;
    uint256 constant stakingTime = 0 seconds;
    ...
    address contractGoatGauds = 0xD1e0e17FeCB97a1A21950B83e8eADd2Da95E1Eb7;
    // GG Flowers Token Contract Address
    address contractFlowers = 0x97919eFbCDE03FCAe039959981bB7989C21Fe5A4;

    constructor()
    Delegated()
    ERC721B("Staked Goat Gauds", "SGG", 0) {
    }
    ...
    /// @notice mapping of a tokenId to its stake
    ...
    /// @notice event emitted when a users NFT has been migrated from old staking contract
    event Migrated(address owner, uint256 token);
    ...
    function setContractAddr(address contractGoatGauds_, address contractFlowers_) public onlyOwner {
        contractGoatGauds = contractGoatGauds_;
        contractFlowers = contractFlowers_;
    }

    function setRewards(uint stakingReward_) public onlyOwner {
    ...
    function migrate(uint[] memory tokenIds_, address[] memory addresses_) public onlyOwner {
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            address _user = addresses_[i];
            ...
            _staker.stakeCount++;
            emit Migrated(_user, tokenIds_[i]);
        }
        stakedTotal = stakedTotal + tokenIds_.length;
    }

    function stakeNFT(uint[] memory tokenIds_) public onlySender {
        address _user = msg.sender;
        uint _numTokens = tokenIds_.length;
        require(
            initialized,
        ...
        for (uint256 i = 0; i < _numTokens; i++) {
            require(
                IGoatGauds(contractGoatGauds).ownerOf(tokenIds_[i]) == _user,
        ...
        _staker.stakeCount = _staker.stakeCount + _numTokens;
        stakedTotal = stakedTotal + _numTokens;
        IGoatGauds(contractGoatGauds).burnFrom(_user, tokenIds_);
        for (uint256 i = 0; i < _numTokens; i++) {
            _mint(_user, tokenIds_[i]);
        }
    ...
    function claimStake(uint[] memory tokenIds_) public onlySender {
        address _user;
        uint _totalClaim;
        for (uint i = 0; i < tokenIds_.length; i++) {
            _user = tokenOwner[tokenIds_[i]];
            _totalClaim += getPendingStakeRewards(tokenIds_[i]);
            Stake storage _stake = tokenIdToStake[tokenIds_[i]];
            _stake.since = block.timestamp;
        }
        require(_totalClaim > 0, "No rewards pending!");
        IFlowers(contractFlowers).mint(_user, _totalClaim);
    ...
    function getIndex(uint element, uint[] memory arr) pure internal returns(uint index) {
        ...
        for (uint i = 0 ; i < arr.length; i++) {
            if (encodedElement == keccak256(abi.encode(arr[i]))) {
                return i;
            }
        }
    }
    ...
    function withdrawNFT(uint[] memory tokenIds_) public onlySender {
        ...
        claimStake(tokenIds_);
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            Staker storage _staker = stakers[_user];
            uint[] storage _stakedTokens = _staker.stakedTokens;
            _stakedTokens[getIndex(tokenIds_[i], _stakedTokens)] = _stakedTokens[_stakedTokens.length - 1];
            _stakedTokens.pop();
            _staker.stakedTokens = _stakedTokens;
            _staker.stakeCount--;
            delete tokenIdToStake[tokenIds_[i]];
            delete tokenOwner[tokenIds_[i]];
        }
        stakedTotal = stakedTotal - tokenIds_.length;
        address[] memory _userToArr = new address[](tokenIds_.length);
        for (uint i = 0; i < tokenIds_.length; i++) {
            _userToArr[i] = _user;
        }
        for (uint256 i = 0; i < tokenIds_.length; i++) {
        ...
        IGoatGauds(contractGoatGauds).resurrect(tokenIds_, _userToArr);
