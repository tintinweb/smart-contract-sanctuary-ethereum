...
contract Bank is Ownable, Pausable {
    ...
    function withdraw(uint _amountInWei) external whenNotPaused {
        require(balances[msg.sender] >= _amountInWei, "Not enough");
        balances[msg.sender] -= _amountInWei;
        totalBalance -= _amountInWei;

        payable(msg.sender).transfer(_amountInWei);

        emit Withdraw(msg.sender, address(this), _amountInWei);
    }
