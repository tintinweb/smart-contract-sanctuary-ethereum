...
contract TinyPlanet is Ownable, ERC721A, ReentrancyGuard
{
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory)
    {
        ...
        if(_isRevealed)
        {
            string memory image = string(abi.encodePacked(_baseTokenURI, '/', uint2str(_tokenId + 1), _tokenId > (_animatedMintCount - 1) ? '.png' : '.mp4'));
            bytes memory data1 = abi.encodePacked('{ "description": "', planetsData[_tokenId].desc, '",', 
                                            ' "image": "', image, '",',
                                            ' "name": "', planetsData[_tokenId].name, '", "attributes": [',
                                            '{ "trait_type": "Life Conditions", "value": "', uint2str(getPercentage(planetsData[_tokenId].seed % 100)) ,'" }, ',
                                            '{ "trait_type": "Metal Resources", "value": "', uint2str(getPercentage((planetsData[_tokenId].seed / 100) % 100)) ,'" }, ',
                                            '{ "trait_type": "Gas Resources", "value": "', uint2str(getPercentage((planetsData[_tokenId].seed / 10000) % 100)) ,'" }, ');
            bytes memory data2 = abi.encodePacked(
                                            '{ "trait_type": "Organic Resources", "value": "', uint2str(getPercentage((planetsData[_tokenId].seed / 1000000) % 100)) ,'" }, ',
                                            '{ "trait_type": "Building Difficulty", "value": "', uint2str(getPercentage((planetsData[_tokenId].seed / 100000000) % 100)) ,'" }, ',
