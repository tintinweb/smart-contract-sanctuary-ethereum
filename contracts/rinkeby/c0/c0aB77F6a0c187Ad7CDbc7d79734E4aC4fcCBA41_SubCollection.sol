...
contract SubCollection is ERC721Enumerable, Ownable {
    ...
    uint256 public currentWhiteListVersion = 0;
    ...
    function setRandom(bool _flag) onlyContract public {
        randomFlag = _flag;
    }
    ...
    function mint(address _to) public onlyContract returns (uint256)  {
        ...
        require(randomFlag, "mint is only available on random starting.");
    ...
    function mintId(address _to, uint256 tokenIdToBe) public onlyContract {
        require(block.timestamp >= blockStart, "Exception: Mint has not started yet so you can't get a price yet.");
        require(mintedAmount < MAX_SUPPLY, "Exception: All was minted.");
        require(mintable, "Exception: This is not mintable.");
        require(mintedPerAddress[_to] < maxMintAmount, "Exception: Reached the limit for each user. You can't mint no more");
    ...
    function addWhitelisted(address _address) internal {
        isWhitelisted[currentWhiteListVersion][_address] = true;
    }

    function addBatchWhitelisted(address[] calldata mintWhitelist) external onlyContract {
        for(uint256 i=0; i<mintWhitelist.length; i++) {
            addWhitelisted(mintWhitelist[i]);
        }
    }

    function upgradeWhitelistVersion() external onlyContract {
        currentWhiteListVersion++;
    }

    function checkWhitelisted(address _address) external view returns (bool) {
        return isWhitelisted[currentWhiteListVersion][_address];
    } 

    function removeWhitelisted(address _address) external onlyContract {
        isWhitelisted[currentWhiteListVersion][_address] = false;
    }

    function setCost(bool isPublic, uint256 _newCost) external onlyContract {
        if(isPublic) {
