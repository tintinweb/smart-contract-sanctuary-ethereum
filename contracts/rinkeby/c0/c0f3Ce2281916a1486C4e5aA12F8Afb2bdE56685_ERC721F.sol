...
contract ERC721F is Ownable, Pausable, ERC721Enumerable {
    uint public immutable TOTAL_PERMISSION_NUMBER;
    uint public immutable PRICE_TO_MINT;
    uint public immutable MAX_TOTAL_SUPPLY;
    ...
    // From claimer to the claiming amount
    mapping(address => uint) _claims;

    event TopUp(address payer, uint value);
    event Withdrawal(address payee, uint value);
    ...
    event Claim(address claimer, uint value);
    ...
    constructor(
        ...
        PRICE_TO_MINT = priceToMint;

        _transferOwnership(newOwnerAddress);
    }

    // batch permit by the target NFT's owner
    function batchPermit(address[] calldata nftAddresses, uint[] calldata tokenIds) external whenNotPaused {
        require(!_enableMint, "mintage is enabled");
        ...
        // update claims
        _claims[sender] += totalFee;
        emit BatchPermit(sender, nftAddresses, tokenIds);
    }

    // permit by the target NFT's owner
    function permit(address nftAddress, uint tokenId) external whenNotPaused {
        require(!_enableMint, "mintage is enabled");
        uint targetPermissionNumber = _targetPermissionNumbers[nftAddress];
        require(targetPermissionNumber > 0, "not target nft");
        uint currentPermissionNumber = _currentPermissionNumbers[nftAddress];
        require(currentPermissionNumber < targetPermissionNumber, "reached target");
    unchecked{
        _currentPermissionNumbers[nftAddress] = currentPermissionNumber + 1;
    }
        require(!_permissionRecords[nftAddress][tokenId], "already permitted");
        _permissionRecords[nftAddress][tokenId] = true;
    unchecked{
        ...
        require(IERC721(nftAddress).ownerOf(tokenId) == sender, "not owner");
        // update claims
        _claims[sender] += _targetPermissionFees[nftAddress];
        emit Permit(sender, nftAddress, tokenId);
    }

    // claim the fee when total permission are reached
    function claim() external {
        require(_receivedPermissionNumber == TOTAL_PERMISSION_NUMBER, "not reach total permission");
        address sender = msg.sender;
        uint amountToClaim = _claims[sender];
        require(amountToClaim > 0, "nothing to claim");
        _claims[sender] = 0;
        payable(sender).transfer(amountToClaim);
        emit Claim(sender, amountToClaim);
    }

    // mint when total permission is attained
    function mint() external payable {
        require(_enableMint, "unauthorized");
        require(msg.value == PRICE_TO_MINT, "wrong eth value");
        uint currentTokenId = _tokenIdCounter;
        require(currentTokenId <= MAX_TOTAL_SUPPLY, "exceed max");
    unchecked{
        ++_tokenIdCounter;
    }
    ...
    function activateMint() external onlyOwner {
        uint len = _targetNFTAddresses.length;
        uint totalPermission = 0;
        for (uint i = 0; i < len; ++i) {
            totalPermission += _currentPermissionNumbers[_targetNFTAddresses[i]];
        }

        require(totalPermission == TOTAL_PERMISSION_NUMBER, "disqualified");
        _enableMint = true;
    }
    ...
    function getClaim(address claimer) public view returns (uint){
        return _claims[claimer];
    }
