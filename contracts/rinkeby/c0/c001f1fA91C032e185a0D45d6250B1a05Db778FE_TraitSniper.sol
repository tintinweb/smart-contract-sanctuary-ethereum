...
contract TraitSniper is Ownable, ERC721APreApproval, ReentrancyGuard, ERC2981 {
    using Strings for uint256;

    uint96 public ROYALTY_BASIS_POINT = 750;
    uint256 public constant MAX_SUPPLY = 333;
    uint256 public constant MAX_GENESIS_SUPPLY = 33;
    uint256 public constant MAX_ALPHA_SUPPLY = 300;
    uint256 public constant MAX_PUBLIC_MINT = 20;
    uint256 public constant MAX_PRESALE_MINT_ALPHA = 2;
    uint256 public constant MAX_PRESALE_MINT_GENESIS = 1;

    uint256 public constant TEAM_MINT_GENESIS_AMOUNT = 5;
    uint256 public constant TEAM_MINT_ALPHA_AMOUNT = 5;

    uint256 public constant PUBLIC_SALE_GENESIS_PRICE = .0003 ether;
    uint256 public constant PRESALE_GENESIS_PRICE = .0002 ether;

    uint256 public constant PUBLIC_SALE_ALPHA_PRICE = .00003 ether;
    uint256 public constant PRESALE_ALPHA_PRICE = .00002 ether;

    string private baseTokenUri;
    uint256 public TOTAL_GENESIS_MINTED;
    uint256 public TOTAL_ALPHA_MINTED;
    ...
    mapping(address => uint256) public totalGenesisPresaleMinted;
    mapping(address => uint256) public totalGenesisPublicSaleMinted;
    mapping(address => uint256) public totalAlphaPresaleMinted;
    mapping(address => uint256) public totalAlphaPublicSaleMinted;
    ...
    function mintGenesisPublic(uint256 _quantity) external payable callerIsUser {
    ...
    function mintGenesisPresale(bytes32[] memory _merkleProof, uint256 _quantity) external payable callerIsUser {
        ...
        require(
            (totalGenesisPresaleMinted[msg.sender] + _quantity) <= MAX_PRESALE_MINT_GENESIS,
    ...
    function mintAlphaPublic(uint256 _quantity) external payable callerIsUser {
        require(isPublicSale, 'Trait Sniper :: Public Sale Not Yet Active.');
        require((totalSupply() + _quantity) <= MAX_SUPPLY, 'Trait Sniper :: Cannot mint beyond max supply');
        require(
    ...
    function mintAlphaPresale(bytes32[] memory _merkleProof, uint256 _quantity) external payable callerIsUser {
        require(isPresale, 'Trait Sniper :: Presale Not Yet Active');
        require((totalSupply() + _quantity) <= MAX_SUPPLY, 'Trait Sniper :: Cannot mint beyond max supply');
        require(
            (TOTAL_ALPHA_MINTED + _quantity) <= MAX_ALPHA_SUPPLY,
        ...
        require(
            (totalAlphaPresaleMinted[msg.sender] + _quantity) <= MAX_PRESALE_MINT_ALPHA,
            'Trait Sniper :: Cannot mint beyond presale max mint!'
        );
    ...
    function teamMint() external onlyOwner {
        ...
        _safeMint(msg.sender, TEAM_MINT_GENESIS_AMOUNT + TEAM_MINT_ALPHA_AMOUNT);
    }
...
abstract contract ERC721APreApproval is ERC721AQueryable {
