...
/// @title Parallel Elyx collection smart contract
...
contract Parallel is ERC1155, Ownable, ERC2981ContractWideRoyalties, Pausable {
  ...
  /// @dev Should be in upper case, but not sure Etherscan recognizes it...
  ...
  Counters.Counter private greenlistTokenIds;
  ...
  /*** SUPPLY - REMAINING SUPPLIES FUNCTIONS ***/
  ...
  function greenlistTokenLeft() public view returns (uint) {
    return GREENLIST_SUPPLY - greenlistTokenIds.current();
  }
  ...
  /// @notice 10% for ELYXyak.eth (artist)
  ...
  constructor() ERC1155("ipfs://QmZPg5mnmM51UdMSJMUMSvkE485KY6n8jEdK1QX3fPbZit/{id}.json") {
    _setRoyalties(ELYXYAK_WALLET, ELYXYAK_ROYALTIES);

    // TODO: ADD Daoz wallets for founders initial drop
  }
  ...
  /// @notice This smart contract has 4 mint functions based on the 4 token subsets previously mentionned.
  /// During the exhibition, visitors can mint for free (pre sales)

  /// @notice Mint function for founders.
  function founderMint(uint _amount, address _to) external onlyOwner {
    require(founderTokenLeft() > 0, "Sorry bro': there is no more founder token left.");
    require(founderTokenLeft() - _amount >= 0, "Not enough founder token left");

    for (uint256 index = 0; index < _amount; index++) {
      founderTokenIds.increment();
      uint _tokenId = founderTokenIds.current();

      _mint(_to, _tokenId, 1, "");
    }
  }

  /// @notice Mint function for phygital collector (physical painting + nft). The NFT must match the painting.
  /// @param _tokenId converted from 4 digits string to uint by frontend
  function phygitalMint(uint _tokenId, bytes memory _signature) external whenNotPaused {
    require(validPhygitalTokenId(_tokenId), "Invalid phygital token ID");
    require(phygitalTokenIds[_tokenId] == address(0), "NFT already minted.");
    require(signatureIsValid(_tokenId, _signature), "Invalid signature: please check address and tokenId");

    phygitalTokenIds[_tokenId] = msg.sender;
  ...
  function greenlistMint(uint _amount, bytes memory _signature) external payable whenNotPaused {
    require(signatureIsValid(_signature), "Invalid signature: are you sure this address is greenlisted?");
    require(greenlistTokenLeft() > 0, "No more greenlist token left.");
    require(greenlistTokenLeft() - _amount >= 0, "Not enough token left");
    require(_amount <= maxAmountPerTransaction, "Amount of token exceeds the max amount of tokens per transaction limit");
    require(msg.value == price * _amount, "Oops! An incorrect quantity of ETH was sent! Please check the price of this NFT. :)");

    for (uint256 index = 0; index < _amount; index++) {
      greenlistTokenIds.increment();
      uint _tokenId = greenlistTokenIds.current();
  ...
  function publicMint(uint _amount) external payable whenNotPaused {
    require(publicSaleTokenLeft() > 0, "No more token available: sorry.");
    ...
    for (uint256 index = 0; index < _amount; index++) {
      ...
      uint _tokenId = publicSaleTokenIds.current();
...
/// @author https://github.com/dievardump/EIP2981-implementation
