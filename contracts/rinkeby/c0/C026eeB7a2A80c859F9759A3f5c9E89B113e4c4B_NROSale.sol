...
contract NROSale is Ownable, ReentrancyGuard {
    ...
    uint64 public immutable withdrawTime = 1647514250;
    ...
    {
        if (uint256(withdrawTime) > block.timestamp) {
            return claimableAmout = 0;
        }
        uint256 base = (tokenBought[sender] * 30) / 100;
        uint256 ts = (block.timestamp - uint256(withdrawTime)) / (30 days);
        if (ts >= 10) {
