  ...
  Ownable
{
    ...
    uint256 public limitNFT = 0;
    uint256 public limitDifferentToken = 0;
    ...
    enum State {Initialized, Waiting_for_offers, Ready_to_offer, withdrawn }
    ...
    function destroyAllSwapBox() external onlyOwner {
        ...
        for (uint256 i = 1; i <= total; i++) {
            swapBoxes[i].state = State.withdrawn;
    ...
    function setLimitNFT(uint256 amount) external onlyOwner {
        limitNFT = amount;
    }

    function setLimitToken(uint256 amount) external onlyOwner {
        limitDifferentToken = amount;
    }
    ...
    function _checkAssets(
        ...
        require(erc721Details.length > limitNFT, "Limit for number of NFTs");
        require(erc20Details.tokenAddrs.length > limitDifferentToken, "Limit for number of Tokens");
    ...
    function toWaitingForOffers(
        ...
        require(swapBoxes[boxID].state == State.Initialized, "Not Allowed Operation");
    ...
    function toOffer(
        ...
        swapBoxes[boxID].state = State.Ready_to_offer;

        emit SwapBoxState(
            boxID,
            msg.sender,
            State.Ready_to_offer,
            swapBoxes[boxID].createdTime,
            block.timestamp,
            swapBoxes[boxID].erc721Tokens,
            swapBoxes[boxID].erc20Tokens
    ...
    function destroyBox(
        ...
        swapBoxes[boxID].state = State.withdrawn;
    ...
    function linkBox(
        ...
        require(swapBoxes[offerBoxID].state == State.Ready_to_offer, "Not Allowed Operation");
    ...
    function swapBox(
        ...
        require(swapBoxes[offerBoxID].state == State.Ready_to_offer, "Not offered Swap Box");
        ...
        swapBoxes[listBoxID].state = State.Initialized;
        delete swapBoxes[listBoxID].offers;
        
        swapBoxes[offerBoxID].owner = msg.sender;
        swapBoxes[offerBoxID].state = State.Initialized;
    ...
    function getWaitingSwapBoxes(
        ...
        if (swapBoxes[offerBoxID].state == State.Ready_to_offer) {
            for (uint256 i = 0; i < swapBoxes[offerBoxID].offers.length; i++) {
                if (swapBoxes[offerBoxID].offers[i].active && _existBoxID(swapBoxes[swapBoxes[offerBoxID].offers[i].boxID], offerBoxID, true)) {
                    itemCount++;
                }
