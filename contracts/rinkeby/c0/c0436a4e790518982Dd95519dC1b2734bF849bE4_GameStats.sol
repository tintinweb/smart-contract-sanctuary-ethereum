...
contract GameStats is Ownable, VRFConsumerBase, Pausable {
    ...
    uint256 houseUpgradeCost = 100000000 ether;
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            ...
            if (tokenData.isElite) {
                if (tokenData.faction) {
                    ...
                    blueTotalLevel += tokenData.level;
                } else {
                    redElites.add(tokenKey);
                    redTotalLevel += tokenData.level;
                }
            }
    ...
    function getLevelBoosts(address collection_, uint256[] calldata tokenIds_) external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < tokenIds_.length; ++i) {
            for (uint256 j = 0; j < levelMilestones.length; ++j) {
                ...
                if (j == levelMilestones.length) {
                    levelBoosts[i] = levelImpacts[levelMilestones[j - 1]].boost;
                }
            }
        }
        return levelBoosts;
    ...
    function getLevelBoost(address collection, uint256 tokenId) external view returns (uint256) {
        ...
        for (uint256 j = 0; j < levelMilestones.length; ++j) {
            ...
            if (j == levelMilestones.length) {
                levelBoost = levelImpacts[levelMilestones[j - 1]].boost;
            }
        }
    ...
    function claimLevel(TokenSelection[] calldata tokensSelection_) external {
    ...
    function _claimLevelForToken(address collection_, uint256 tokenId_) internal {
        require(tokenDataEncoded[_getTokenKey(collection_, tokenId_)].owner == msg.sender);
        require(
            tokenDataEncoded[_getTokenKey(collection_, tokenId_)].stakeType != TREE_HOUSE_STAKE
            && tokenDataEncoded[_getTokenKey(collection_, tokenId_)].stakeType != HOUSE_STAKE
        );
        ...
        levelYield = levelYield > LEVEL_CAP ? LEVEL_CAP: levelYield;
    ...
    function isUnStakeClaimSuccessful(
        ...
        for (uint256 j = 0; j < levelMilestones.length; ++j) {
            ...
            if (j == levelMilestones.length) {
                risk -= levelImpacts[levelMilestones[j - 1]].riskReduction;
            }
        }
    ...
    function upgradeHouseSize(uint256 tokenId_, uint256 upgrade_) external {
        ...
        require(tokenDataEncoded[_getTokenKey(contractsAddressesMap["house"], tokenId_)].level + upgrade_ <= HOUSE_CAP);
        require(tokenDataEncoded[_getTokenKey(contractsAddressesMap["house"], tokenId_)].owner == msg.sender);
        require(IToken(contractsAddressesMap["token"]).getWalletBalance(msg.sender) >= houseUpgradeCost * upgrade_);
        // spend

        tokenDataEncoded[_getTokenKey(contractsAddressesMap["house"], tokenId_)].level += upgrade_;
    }
    ...
    function setEliteStatus(address collection_, uint256 tokenId_, bool isElite) external authorised {
        bytes32 tokenKey = _getTokenKey(collection_, tokenId_);
        tokenDataEncoded[tokenKey].isElite = isElite;

        if (isElite) {
            if (tokenDataEncoded[tokenKey].faction) {
                blueElites.add(tokenKey);
                blueTotalLevel += tokenDataEncoded[tokenKey].level;
            } else {
                redElites.add(tokenKey);
                redTotalLevel += tokenDataEncoded[tokenKey].level;
            }
        }
    }
    ...
    function setHousesLevels(uint256[] calldata tokenIds_, uint256[] calldata levels_) external authorised {
        for (uint256 i = 0; i < tokenIds_.length; ++i) {
            tokenDataEncoded[_getTokenKey(msg.sender, tokenIds_[i])].level = levels_[i];
        }
    }
