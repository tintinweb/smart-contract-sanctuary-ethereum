...
contract VaultsController is OwnableUpgradeable {
    ...
    /**
     * @notice Get unexpired vaults
     */
    function getUnExpiredVaults() external view returns (address[] memory) {
        uint256 currentIndex = IVaultsRegistry(registry).index();
        address[] memory unExpiredVaults = new address[](currentIndex);
        uint256 vaultIndex;
        for (uint256 key = 0; key < currentIndex; key++) {
            address vault = IVaultsRegistry(registry).getVaults(key);
            (bool isExpired, ) = isVaultExpired(IVolmexVault(vault));
            if (!isExpired) {
                unExpiredVaults[vaultIndex] = vault;
                vaultIndex++;
            }
        }
        return unExpiredVaults;
    }

    /**
     * @notice Used to check the Vault is expired
     */
    function isVaultExpired(IVolmexVault _vault) public view returns (bool, uint256) {
        return (block.timestamp > _vault.expiryTimestamp(), _vault.expiryTimestamp());
    }
}