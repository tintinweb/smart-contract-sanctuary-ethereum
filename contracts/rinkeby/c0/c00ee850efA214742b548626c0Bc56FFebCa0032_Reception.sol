...
contract Reception is Ownable {
  ...
  function userDeposit(uint256 _roomId) public {
    IRoom _room = IRoom(rooms.at(_roomId - 1));
    if(_room.getWinner() != address(0)) {
      _room.formatRoom();
    }
  ...
  function roomAtIndex (uint256 _index) external view returns (address) {
    return rooms.at(_index - 1);
  }
  // check created Room
  function getCreatedRoom (address player0, address player1) public view returns (uint256) {
    bytes memory addressPair1 = abi.encode(player0, player1);
    bytes memory addressPair2 = abi.encode(player1, player0);
    uint256 id1 = createdRooms[addressPair1];
    uint256 id2 = createdRooms[addressPair2];
    if( id1 > 0 ) {
      return id1;
    } else {
      return id2;
  ...
  function createRoom (address player0, address player1, uint256 baseAmount) public {
    ...
    bytes memory addressPair = abi.encode(player0, player1);
    createdRooms[addressPair] = roomId;
    // init room
    newRoom.initRoom(
      player0, 
      player1, 
  ...
  function decideWinner (uint256 _index, address _winner) public onlyOwner {
    address _room = rooms.at(_index - 1);
...
contract Room is ReentrancyGuard {
  ...
  function formatRoom () external {
    require(msg.sender == receptionist, "Not Receptionist!");
    require(withdrawn[info.player0], "Not withdraw yet!");
    require(withdrawn[info.player1], "Not withdraw yet!");
    end = false;
    winner = address(0);
