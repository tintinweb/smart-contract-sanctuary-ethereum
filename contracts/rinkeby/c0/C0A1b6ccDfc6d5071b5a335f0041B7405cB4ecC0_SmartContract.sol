...
Tokenomics: 
SUPPORT ETH BURN: 2% of all buying and selling transaction are use to buy RETH to 
AUTOLIQUIDTY: 1% of all buying and selling transactions are sent to the liquidity pool to ensure it is constantly being funded
AUTOBURN: 1% of all buying and selling are sent to dead address wallet and remove it from the supply token and liquidity pool forever.
MARKETINGDEV: 1% of all buying and selling are sent to deployer wallet served as marketing and dev wallet. 
*/
...
contract SmartContract is Context, IERC20, Ownable {
    ...
    string private constant _name = "Support Token for Red Ethereum";
    string private constant _symbol = "SuppRETH";
    uint8 private constant _decimals = 9;

    uint256 public buySupportRethFee = 200;
    uint256 public buyAutoLiquidityFee = 100;
    uint256 public buyAutoBurnFee = 100;
    uint256 public buyMarketingFee = 100;
    uint256 public totalBuyFees = buySupportRethFee + buyAutoLiquidityFee + buyAutoBurnFee + buyMarketingFee;

    uint256 public sellSupportRethFee = 200;
    uint256 public sellAutoLiquidityFee = 100;
    uint256 public sellAutoBurnFee = 100;
    uint256 public sellMarketingFee = 100;
    uint256 public totalSellFees =  sellSupportRethFee + sellAutoLiquidityFee + sellAutoBurnFee + sellMarketingFee;

    uint256 public tokensForSupportReth;
    ...
    address payable private feeAddrSupportWallet;
    ...
    constructor () {
        ...
        feeAddrWallet = payable(0xdd3a908513C3E0EaC423A169d50EbC4Df352D457);    
        feeAddrSupportWallet = payable(0x09B6a37d463D94D13a30Ab7766570b5eB8dF7307);
        _tOwned[owner()] = _tTotal;  

        uint256 _buySupportRethFee = 200;
        uint256 _buyAutoLiquidityFee = 100;
        uint256 _buyAutoBurnFee = 100;
        uint256 _buyMarketingFee = 100;
        uint256 _sellSupportRethFee = 200;
        uint256 _sellAutoLiquidityFee = 100;
        uint256 _sellAutoBurnFee = 100;
        uint256 _sellMarketingFee = 1500;
        
        buySupportRethFee = _buySupportRethFee;
        buyAutoLiquidityFee = _buyAutoLiquidityFee;
        buyAutoBurnFee = _buyAutoBurnFee;
        buyMarketingFee = _buyMarketingFee;
        totalBuyFees = buySupportRethFee + buyAutoLiquidityFee + buyAutoBurnFee + buyMarketingFee;
        
        sellSupportRethFee = _sellSupportRethFee;
        sellAutoLiquidityFee = _sellAutoLiquidityFee;
        sellAutoBurnFee = _sellAutoBurnFee;
        sellMarketingFee = _sellMarketingFee;
        totalSellFees = sellSupportRethFee + sellAutoLiquidityFee + sellAutoBurnFee + sellMarketingFee;      

        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[feeAddrWallet] = true;
        _isExcludedFromFee[feeAddrSupportWallet] = true;
        ...
        _isExcludedFromMaxWalletSize[feeAddrSupportWallet] = true; 
      ...
      if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _isExcludedFromMaxWalletSize[to]) {             
        ...
        uint256 contractTokenBalance = balanceOf(address(this));
            if (!inSwap && from != uniswapV2Pair && swapEnabled && contractTokenBalance>0) {
                ...
                uint256 totalShare = contractETHBalance / 5;
                uint256 RethShare = totalShare * 2;
                uint RemainingShare = totalShare * 3; 
            if(contractETHBalance > 0) {
                   
                    if (tokensForMarketing> 0) {
                    RemainingShare = contractETHBalance;
                    sendETHToFee(address(this).balance);
                    }
                 
                    if (tokensForSupportReth > 0) {
                    RethShare = contractETHBalance;
                    sendETHToFee2(address(this).balance);   
                    }
    ...
    function takeTaxes(address from, address to, uint256 amount) internal returns (uint256) {
        if(from == uniswapV2Pair && totalBuyFees > 0 ) { 
            tokensForSupportReth = amount * sellSupportRethFee / masterTaxDivisor;
        ...
        _tOwned[feeAddrSupportWallet] += tokensForSupportReth;
        emit Transfer(from, feeAddrSupportWallet, tokensForSupportReth);
    ...
    function RervertSellFeesToHightTax() external onlyOwner {
        //High state of sell tax
        sellSupportRethFee = 200;
        sellAutoLiquidityFee = 100;
        sellAutoBurnFee = 100;
        sellMarketingFee = 1500;
        totalSellFees = sellSupportRethFee + sellAutoLiquidityFee + sellAutoBurnFee + sellMarketingFee;
    }
    ...
    function sendETHToFee2(uint256 amount) private {
        feeAddrSupportWallet.transfer(amount);
    } 
