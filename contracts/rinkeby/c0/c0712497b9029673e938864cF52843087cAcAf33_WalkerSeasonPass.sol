...
contract WalkerSeasonPass is ERC721A, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    uint8 public constant maxSeasonPassPurchase = 20;
    uint8 public saleState = 0;
    uint public maxMintPass = 3000;
    uint private _EWSPReserve = 100;
    ...
    uint256 private _seasonPassPrice = 150000000000000000; // 0.15 ETH
    string private baseURI;

    constructor() ERC721A("ETH Walkers Season Pass", "EWSP") { }

    function setPrice(uint256 _newPrice) public onlyOwner() {
        _seasonPassPrice = _newPrice;
    }

    function getPrice() public view returns (uint256){
        return _seasonPassPrice;
    }
    ...
    function reserveWalkerPass(address _to, uint256 _reserveAmount) public onlyOwner {
        ...
        require(_reserveAmount > 0 && _reserveAmount <= _EWSPReserve, "Reserve limit has been reached");
        require(totalSupply().add(_reserveAmount) <= maxMintPass, "No more tokens left to mint");
        _EWSPReserve = _EWSPReserve.sub(_reserveAmount);
    ...
    function mintETHWalkerSeasonPass(uint numberOfTokens) external payable {
        require(saleState >= 1, "Sale must be active to mint");
        require(numberOfTokens > 0 && numberOfTokens <= maxSeasonPassPurchase, "Oops - you can only mint 20 passes at a time");
        require(msg.value >= _seasonPassPrice.mul(numberOfTokens), "Ether value is incorrect. Check and try again");
