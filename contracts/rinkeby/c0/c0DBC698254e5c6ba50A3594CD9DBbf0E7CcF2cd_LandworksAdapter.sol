...
contract LandworksAdapter is BaseRentAdapter {
    ...
    function listNFTforRenting(
        ...
        // Validate Listing
        DataTypes.NFTItem memory nftItem = ITribeOne(TRIBE_ONE_ADDRESS).getLoans(_loanId).nftItem;
    ...
    function _delistNFTFromRenting(uint256 _loanId, bool isForce) private {
        ...
        require(_loan.borrower == msg.sender || isForce, "Only loan borrower or T1 can delist.");
        ...
        if (IERC721(_loan.nftItem.nftAddress).ownerOf(_loan.nftItem.nftId) == address(this)) {
            // NFT was withdrawn
            IERC721(_loan.nftItem.nftAddress).approve(TRIBE_ONE_ADDRESS, _loan.nftItem.nftId);
    ...
    function _withdraw(uint256 _loanId, bool isForce) private {
        ...
        require(_loan.borrower == msg.sender || isForce, "Only loan borrower or T1 can withdraw");
        ...
        require(
            IERC721(_loan.nftItem.nftAddress).ownerOf(_loan.nftItem.nftId) == address(this),
            "LandworksAdapter: Withdraw was failed"
        );
    ...
    function claimRentFee(uint256 loanId) external nonReentrant {
        ...
        require(_loan.borrower == msg.sender, "Only loan borrower can claim rent fee");
    ...
    function adjustRentFee(uint256 loanId) external nonReentrant {
        DataTypes.Loan memory _loan = ITribeOne(TRIBE_ONE_ADDRESS).getLoans(loanId);
        require(_loan.borrower == msg.sender, "Only loan borrower can withdraw");

        uint256 assetId = rentAssetIdMap[loanId];
        address paymentToken = ILandworks(LAND_WORKS_ADDRESS).assetAt(assetId).paymentToken;

        address loanCurrency = _loan.loanAsset.currency;
        ...
        if (feeAmount > 0) {
            ...
            if (loanCurrency == TRIBEONE_ETHEREUM_PAYMENT_TOKEN) {
