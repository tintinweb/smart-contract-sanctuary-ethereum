...
contract vestingClaims is Ownable, ReentrancyGuard {
    ...
    */
    ...
    function claim(uint _nftId) public{
        ...
        address nftOwner = nftAddress.ownerOf(_nftId);
        require(nftOwner == _msgSender(),"caller is not the owner "); 
        // require(_totalAmount>0,"Claim amount should be greater than 0");
        _token.safeTransfer(nftOwner, _totalAmount);
    ...
    function getunreleasedAmounts(uint _nftId,uint8 _matchId) internal view returns(uint256 _claimAmount){
    ...
    function getClaimableAmount(uint _nftId,uint8 _matchId) internal view returns(uint _claimAmount) {
        ...
        if(!nftData[_nftId][_matchId].initialized){
            ...
            // nftData[_nftId][_matchId].totalClaimed.add(claimAmount/_decimal);

            if(temp > totalEligible){
                _amount = totalEligible.sub(_lastReleaseAmount);
                return (_amount);
                // return (totalEligible,_amount.mul(_decimal),nftData[_nftId][_matchId].totalClaimed);
            }
                return (_amount);


            // return (totalEligible,claimAmount,nftData[_nftId][_matchId].totalClaimed);
    ...
    function getVestingInfo(uint _nftId,uint8 _matchId) public view returns(uint _totalEligible,uint _claimAmount,uint _totalClaimed,uint _remainingBalTokens,uint _lastClaimedAt){
        if(!nftData[_nftId][_matchId].initialized){
            return (nftData[_nftId][_matchId].totalEligible,getClaimableAmount(_nftId,_matchId),0,nftData[_nftId][_matchId].totalEligible,0);
        }else{
