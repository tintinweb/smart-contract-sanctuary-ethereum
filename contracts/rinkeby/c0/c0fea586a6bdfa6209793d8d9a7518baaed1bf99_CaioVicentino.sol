...
// File: contracts/CaioVicentino.sol
...
//contract CaioVicentino is Context, ERC721Enumerable, Ownable {
contract CaioVicentino is Context, ERC721URIStorage, ERC721Enumerable, Ownable {
  ...
  uint256 private FreeMintPerAddressLimit = 2;
  ...
  uint256 private _deadLine = 1656633600;
...
function getRandomAvailableTokenId(address payable to, uint256 updatedNumAvailableTokens)
    ...
    returns (string memory)
  {
    require(
      _exists(tokenId),
      "ERC721Metadata: URI apontando para token inexistente"
    );
  ...
  // function setRecommendationPercentage(uint256 percentage) public onlyOwner {
  //   _recommendationPercentage = percentage;
  // }
  ...
  // function setIsFreeMint(bool state, uint qtd) public onlyOwner {
  //     maxMintAmount = qtd;
  //     FreeMintPerAddressLimit = qtd;
  //     _isFreeMint = state;
  // }
  ...
  // function getRecommendationProgramPercentage() public view returns (uint256) {
  //     return _recommendationPercentage;
  // }

  // function getAffiliateProgramPercentage() public view returns (uint256) {
  //     return _affiliateProgramPercentage;
  // }
  ...
  function getJsonTokenURI(uint256 tokenId) public view returns (string memory) {
...
// ####################################################
  ...
  function mint(
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      ...
      //_safeMint(endUser, tokenId, bytes(string(abi.encodePacked(baseURI, Strings.toString(tokenId), baseExtension))) );
      _safeMint(endUser, tokenId);
      //_setTokenURI(tokenId, bytes(string(abi.encodePacked(baseURI, Strings.toString(tokenId), baseExtension))));
 ...
 function split(uint256 _mintAmount, address payable _recommendedBy, uint256 _indicationType ) public payable{
    ...
    if(_indicationType > 1){
      ...
      uint amount = msg.value / _splitPercentage;
      ownerAmount = msg.value - amount; //ownerAmount = msg.value / (100 - _splitPercentage); 
