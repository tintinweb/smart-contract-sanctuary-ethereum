...
contract bee is ERC20Burnable, Ownable {
    ...
    address public ALPHA_ADDRESS = 0x1f3ee689cFc036681cecE52F2eE5cd1F0504AC79;
    ...
    function getStakedDuck(address staker)
        public
        view
    ...
    function getStakedDuckling(address staker)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function getDucklingStakedAmount(address staker)
        public
        view
        returns (uint256)
    {
    ...
    function getStakedAlpha(address staker)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function getAlphaStakedAmount(address staker)
        public
        view
        returns (uint256)
    {
    ...
    function stakeDuckById(uint256[] calldata tokenIds)
        external
        stakingEnabled
    {
    ...
    function stakeDucklingsById(uint256[] calldata tokenIds)
        external
        stakingEnabled
    {
        for (uint256 i = 0; i < tokenIds.length; i++) {
    ...
    function stakeAlphaById(uint256[] calldata tokenIds)
        external
        stakingEnabled
    {
        for (uint256 i = 0; i < tokenIds.length; i++) {
    ...
    function unstakeAll() external {
        require(
            getStakedAmount(msg.sender) > 0 ||
                getDucklingStakedAmount(msg.sender) > 0 ||
                getAlphaStakedAmount(msg.sender) > 0,
            "None Staked"
        );
        uint256 totalRewards = 0;
    ...
    function burn(address from, uint256 amount) external {
        require(msg.sender == DUCK_ADDRESS, "NOT_AUTHORIZED");
    ...
    function getReward(uint256 tokenId) internal view returns (uint256) {
        return
            (((block.timestamp - duckTimeStaked[tokenId]) *
                DUCK_EMISSION_RATE) /
                1 /
                60) * 1 ether;
    }

    function getDucklingReward(uint256 tokenId)
        internal
        view
        returns (uint256)
    {
        return
            (((block.timestamp - ducklingTimeStaked[tokenId]) *
                DUCKLING_EMISSION_RATE) /
                1 /
                60) * 1 ether;
    }
    ...
    function getAlphaReward(uint256 tokenId) internal view returns (uint256) {
        return
            (((block.timestamp - alphaTimeStaked[tokenId]) *
                ALPHA_EMISSION_RATE) /
                1 /
                60) * 1 ether;
    }
