...
contract NFTYacht is ERC721, Ownable {
    ...
    function buyOwnership (
        ...
        require ( totalSupply >= (OwnershipToken.current().add(_tokenURI.length)), "!Total Supply");
    ...
    function bookDate (
        ...
        uint year, uint month, uint day, uint hour, uint minute, uint second, uint _id

        ) public {

        require ( DateTimeLibrary.isValidDateTime(year, month, day, hour, minute, second), "inValid Date");
        uint newDAte = DateTimeLibrary.timestampFromDateTime(year, month, day, hour, minute, second);

        if (booking.isInserted(_id)) {
            (,, uint _newYear) = booking.getTime(_id);
            require ( newDAte > _newYear, "Already Booked");
        }
