...
contract FXBToken is ERC721, ERC721Enumerable, ERC721URIStorage,Pausable, Ownable, ERC721Burnable, ERC721Royalty {
    ...
    function safeMint(address to, string memory uri,uint96  feeNumerator,uint256 nftId,SigMessage calldata sigMessage) public  payable  returns (uint256){

        require(verify(owner(),getSafeMintMessageHash(to, uri,feeNumerator, nftId,msg.value),sigMessage.r,sigMessage.s,sigMessage.v),"verify fail");
    ...
    function exchangeNft(address  from, address to, uint256 tokenId, uint256  amount,uint96  payfee,uint256  nftId,uint256  orderId,SigMessage calldata sigMessage) public  payable returns (bool){
        ...
        uint256 payfeeAmount=(amount * payfee) /10000;
        require(msg.value==amount+payfeeAmount,"pay amount is not right");
        require(amount>royaltyamount,"amount must greater than royaltyamount");

        //pay amount
        address payable fromAddress = payable(from);
        fromAddress.transfer(amount-royaltyamount);
        //pay royaltyamount
        ...
        //  address(this).transferFrom(from,to,tokenId);
   ...
   */
    ...
    function getSafeMintMessageHash(address  to, string memory uri,uint96  feeNumerator,uint256  nftId,uint256  value) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(to,uri,feeNumerator,nftId,value));
    }
