...
contract AtomicSwapERC20ToERC20 {
  ...
  function open(uint256 _swapID, uint256 _openValue, address _openTrader, address _openContractAddress, uint256 _closeValue, address _closeTrader, address _closeContractAddress) public onlyInvalidSwaps(_swapID) {
    ...
    require(_openValue <= openERC20Contract.allowance(_openTrader, _closeTrader));
  ...
  function thisContractAddress(address _openTrader, address _openContractAddress, address _closeTrader) public view returns (uint256 openValue) {
    ERC20 openERC20Contract = ERC20(_openContractAddress);
    return openERC20Contract.allowance(_openTrader, _closeTrader);
  }
}