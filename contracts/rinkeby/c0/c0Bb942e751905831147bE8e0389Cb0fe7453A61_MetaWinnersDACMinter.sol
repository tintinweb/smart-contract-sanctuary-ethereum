...
contract MetawinNFTMinter is MintingPhases, Ownable, Pausable, ReentrancyGuard {
    ...
    // CONTRACT VARIABLES //
    uint256 public price = 1 ether;               // Default price 1 ether
    ...
    * @param _address Address to check
    */
    function freemint_hasClaimed(address _address) external view returns(bool) {
    ...
    * @param _merkleProof Merkle proof
    */
    function mintFree(uint8 _tickets, bytes32[] calldata _merkleProof) external whenNotPaused {
        require(phase_current()>MintingPhase(0) && phase_current()<MintingPhase.end);
        ...
        emit FreemintClaimed(msg.sender, _tickets);
    }
    ...
    */
    ...
    /**
    * @dev [Tx][External][PAYABLE] Paid minting
