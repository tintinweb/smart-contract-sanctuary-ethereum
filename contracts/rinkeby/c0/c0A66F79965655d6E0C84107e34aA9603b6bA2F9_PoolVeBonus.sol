...
contract PoolVeBonus is IJellyContract {    
    ...
    /// @notice Mapping of TokenID to amount of tokens locked. 
    mapping(uint256 => uint256) public rewardsLocked;
    ...
    //     require(
    //         accessControls.hasAdminRole(msg.sender),
    //         "setPoolHelper: Sender must be admin"
    //     );
    ...
    // function claimAndLock(uint _tokenId) external returns (uint) {
    //     require(IERC721(poolAddress).ownerOf(_tokenId) == msg.sender, "Must own tokenId");
    //     uint256 rewardAmount = IJellyPoolHelper(poolHelper).unclaimedRewards(poolAddress, poolToken, _tokenId);
    //     IJellyPoolWrapper(poolAddress).fancyClaim(_tokenId);

    //     uint256 tokensAdded = rewardAmount * (bonusPercentage + PERCENTAGE_PRECISION) / uint256( PERCENTAGE_PRECISION);
    ...
    //     return IVeToken(veToken).create_lock_for(tokensAdded, lockDuration, msg.sender);
    // }
    ...
    function claimAndLock(uint _tokenId) external returns (uint) {
        ...
        IJellyPoolWrapper(poolAddress).fancyClaim(_tokenId);
        uint256 rewardAmount = getTokenRewardsClaimed(poolAddress, poolToken, _tokenId);
        uint256 tokensAdded = rewardAmount * (bonusPercentage + PERCENTAGE_PRECISION) / uint256( PERCENTAGE_PRECISION);
    ...
    function lockWithBonus(uint _tokenId, uint256 _amount) external returns (uint) {
        require(IERC721(poolAddress).ownerOf(_tokenId) == msg.sender, "Must own tokenId");

        uint256 rewardsClaimed = getTokenRewardsClaimed(poolAddress, poolToken, _tokenId);

        rewardsLocked[_tokenId] += _amount;
        require(rewardsLocked[_tokenId] <= rewardsClaimed, "Locking more tokens than has been earnt.");
        uint256 tokensAdded = _amount * (bonusPercentage + PERCENTAGE_PRECISION) / uint256( PERCENTAGE_PRECISION);
     ...
     * @param _poolAddress Address of the rewards token.
    ...
    ) public 
    {
        ...
        require(_poolAddress != address(0), "Access controls not set");
        ...
        poolToken = IJellyPool(_poolAddress).poolToken();
