...
contract TastyBones is Ownable, ERC721A {
    ...
    function mintFree(
        ...
        require(mintedTBforFreeMintAddress[msg.sender] <= maxFreeMint,"ALREADY_MINTED_MAX_2");
        ...
        if(bonesOfOwner) {
            ...
            require(mintedTBforFreeMint[boneTokenId] < 1, "BONE_ALREADY_USED_FOR_MINTING");
            canMint += maxFreeBoneMint;
        }

        // For role
        if(MerkleProof.verify(freeMintProof, freeMintListMerkleRoot, keccak256(abi.encodePacked(msg.sender, maxMint)))) {
        ...
        require(numToMint <= canMint,"CANNOT_MINT_MORE_THAN_ALLOWED");
        ...
        if(bonesOfOwner) {
            mintedTBforFreeMint[boneTokenId] = mintedTBforFreeMint[boneTokenId].add(1);
        }
    }
