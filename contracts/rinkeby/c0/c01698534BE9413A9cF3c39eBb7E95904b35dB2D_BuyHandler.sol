...
contract BuyHandler is ReentrancyGuard, BuySignatureUtils {
    ...
    mapping(address => uint256) public addressMintAmountInPrivateTime;
    ...
    constructor(address proxy, uint256 _privatePrice, uint256 _privateStartTime, uint256 _privateEndTime) {
        require(_privateEndTime > _privateStartTime, "BuyHandler: the public time need to be bigger than the private time");
        PROXY = proxy;
    ...
    */
     ...
     * @param _privateStartTime the time that the owner wants to set is the private start time 
    */
    function setPrivateStartTime(uint256 _privateStartTime) public {
        ...
        require(
            ...
            "BuyHandler: the public time need to be bigger than the private time"
    ...
    /**
     @dev Set price
     @param _privatePrice the price that the owner want to set is the nft private price
    */
    ...
    /**
     * @dev Set start time for mint NFT
     * @param _privateEndTime the time that the owner wants to set is the public start time 
    */
    ...
    */
    function buy(
        ...
        require(block.timestamp >= privateStartTime && block.timestamp <= privateEndTime, "BuyHandler: you can't buy nft in this time");
        require(verifyBuySignature(signer,_uuid, _to, _quantity, msg.sender, _signature), "BuyHandler: wrong signature");
        require(msg.sender == _to, "BuyHandler: the sender should be address that will get NFT");
        require(existedUuid[_uuid] != true, "BuyHandler: the uuid was existed");
        require(_quantity <= limitBuy, "BuyHandler: the quantity exceed the limit buy");
        require(currentIndex < IAviumNFT(PROXY).getTotalMint(), "BuyHandler: total mint has been exeeded");
        require(addressMintAmountInPrivateTime[msg.sender] < 2, "BuyHandler: you can only buy up to 2 NFTs");
        addressMintAmountInPrivateTime[msg.sender]+=_quantity;
    ...
    function buyRemainNFTs(
        ...
        uint256 currentIndex = IAviumNFT(PROXY).getCurrentIndex();
        require(currentIndex < IAviumNFT(PROXY).getTotalMint(), "BuyHandler: total mint has been exeeded");
        require(block.timestamp > privateEndTime, "BuyHandler: you can't buy nft in this time");
        uint256 quantity = IAviumNFT(PROXY).getTotalMint() - currentIndex;
        IAviumNFT(PROXY).mint(_to, quantity, _data);
        emit BuyRemainNFTs(_to, quantity, currentIndex);
    }
}
...
contract BuySignatureUtils {
    function getBuyMessageHash(
        ...
        return keccak256(abi.encodePacked(_uuid, _to, _quantity, _payer));
    }
    ...
    function verifyBuySignature(
        ...
        bytes32 messageHash = getBuyMessageHash(_uuid, _to, _quantity, _payer);
