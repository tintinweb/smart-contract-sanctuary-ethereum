...
import "./NFTMarketplaceUpgradableV2.sol";
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";

    contract CreatorFactory is OwnableUpgradeable{

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    mapping(address=>address) public DeployerInstance;
    mapping (address=>bool) public isDeployer;
    event instanceCreated(address creator,address deployerContract);
    

    //initialize contract owner
    function initializeOwner() initializer public{
        __Ownable_init_unchained();
        addDeployerRole(msg.sender);
    }

    function addDeployerRole(address _deployer) public onlyOwner returns (bool){

        require(_deployer != address(0) && isDeployer[_deployer] == false,
        "creator exist");
    ...
    //add new deployer only call by a deployer
    function addCollection(address _creator,
    string memory _name,
    string memory _symbol, 
    string memory _baseTokenURI,
    address _contractHolder) public returns(address){

         onlyDeployer();
         NFTMarketplaceUpgradableV2 ERC721Instance = new NFTMarketplaceUpgradableV2();
         
        address ERC721MinterAdr = address(ERC721Instance);
        ERC721Instance.initialize(_name,_symbol,_baseTokenURI,_contractHolder);
        ERC721Instance.GrantRole(MINTER_ROLE,_creator);
        ERC721Instance.GrantRole(MINTER_ROLE,msg.sender);
        ERC721Instance.transferOwnership(_contractHolder);
        DeployerInstance[_creator] = ERC721MinterAdr;

        emit instanceCreated(_creator,ERC721MinterAdr);
 ...
 */
contract NFTMarketplaceUpgradableV2 is ERC721Upgradeable,
    ...
    function initialize(
        ...
        _setupRole(ADMIN_ROLE, _newSuperAdmin);
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721Upgradeable,AccessControlEnumerableUpgradeable,ERC721EnumerableUpgradeable)
        returns (bool)
    {
   ...
   function GetRoleMemberCount(bytes32 role) public view returns (uint256) {
    ...
    function onlyMinter() internal view {
        require(
            hasRole(ADMIN_ROLE, _msgSender()) || 
            hasRole(MINTER_ROLE, _msgSender()) || 
            owner() == _msgSender(), "Only Minter"
            );
    }

    function onlyPauser() internal view {
        require(
            hasRole(ADMIN_ROLE, _msgSender()) || 
