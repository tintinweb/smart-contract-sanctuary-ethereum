...
interface IAVN {
  ...
  function addValidator(bytes memory t1PublicKey, bytes32 t2PublicKey, uint256 t2TransactionId, bytes calldata confirmations)
      external;
  function registerValidator(address t1Address, bytes calldata t1PublicKey, bytes32 t2PublicKey) external;
  ...
  function liftLegacyStakes(bytes32 t2PublicKey, uint256 amount) external;
  ...
  function lift(address erc20Address, bytes32 t2PublicKey, uint256 amount) external;
  function proxyLift(address erc20Address, bytes32 t2PublicKey, uint256 amount, address approver, uint256 proofNonce,
...
contract AVN is IAVN, Owned {
  ...
  constructor(address avtAddress, IAvnFTTreasury avnFTTreasury)
  {
  ...
  function transferValidators(address avnValidatorsManager)
    onlyOwner
    external
  {
    require(validatorsTransferred == false, "Validators already transferred");

    LegacyValidatorsManager lvm = LegacyValidatorsManager(avnValidatorsManager);
    numActiveValidators = lvm.numActiveValidators();
    nextValidatorId = lvm.validatorIdNum();

    for (uint256 id = 1; id < nextValidatorId; id++) {
      idToT1Address[id] = lvm.t1Address(id);
      idToT2PublicKey[id] = lvm.t2PublicKey(id);
  ...
  function registerValidator(address t1Address, bytes memory t1PublicKey, bytes32 t2PublicKey)
    onlyOwner
    external
  {
    require(t1PublicKey.length == 64, "T1 public key must be 64 bytes");
    require(address(bytes20(uint160(uint256(keccak256(abi.encodePacked(t1PublicKey)))))) == t1Address, "Bad T1 public key");
    uint256 id = t1AddressToId[t1Address];
    require(id == 0, "Validator is already registered");
    ...
    emit LogValidatorRegistered(t1PublicKeyLHS, t1PublicKeyRHS, t2PublicKey, id);
  }
  ...
  function liftLegacyStakes(bytes32 t2PublicKey, uint256 amount)
    onlyOwner
    external
  {
    require(amount <= unliftedLegacyStakes, "Not enough stake remaining");
    unliftedLegacyStakes = unliftedLegacyStakes - amount;
    emit LogLifted(AVT_ADDRESS, address(this), t2PublicKey, amount);
  }
    ...
    external
  {
    ...
    bytes32 validatorHash = keccak256(abi.encode(t1PublicKey, t2PublicKey));
  ...
  function lift(address erc20Address, bytes32 t2PublicKey, uint256 amount)
    ...
    external
  {
    if (msg.sender != approver) {
      checkLiftProof(erc20Address, t2PublicKey, amount, approver, proofNonce, proof);
    }
    doLift(erc20Address, approver, t2PublicKey, amount);
  }

  function liftETH(bytes32 t2PublicKey)
    payable
    onlyWhenLiftingEnabled
    external
  {
    ...
    emit LogLifted(PSEUDO_ETH_ADDRESS, msg.sender, t2PublicKey, msg.value);
  }
    ...
    external
  {
    ...
    require(data.length == 32, "Bad T2 public key");
    ...
    emit LogLifted(msg.sender, from, abi.decode(data, (bytes32)), amount); // TODO - do we need to decode?
  }
  ...
  function doLift(address erc20Address, address approver, bytes32 t2PublicKey, uint256 amount)
    private
  {
    ...
    emit LogLifted(erc20Address, approver, t2PublicKey, newBalance - currentBalance);
  }

  function checkLiftProof(address erc20Address, bytes32 t2PublicKey, uint256 amount, address approver, uint256 proofNonce,
      bytes memory proof)
    private
  {
