...
contract CryptoBalloonerz is ERC721F, RoyaltiesV2Impl {
  ...
  uint256 public tokenPrice = 0.0314 ether; 
  ...
  */
  function setBaseUriToAr() public onlyOwner {
    setBaseTokenURI("ar://D9Qlf21INzNvxRwz4fsQVblzcsLE-z3NQo52B_OKWtE/");
  }
  ...
  */
  function setBaseUriToArweaveNet() public onlyOwner {
    setBaseTokenURI("https://www.arweave.net/D9Qlf21INzNvxRwz4fsQVblzcsLE-z3NQo52B_OKWtE/");
  }
  ...
  */   
  function reserveTokens() external onlyOwner {    
    mintTokensToWallet(ONE, MAX_RESERVE);
    mintTokensToWallet(TWO, MAX_RESERVE);
    mintTokensToWallet(THREE, MAX_RESERVE);
  }
  ...
  */
  function mint(uint numberOfTokens) external payable{
    ...
    if (!isWhitelisted(msg.sender)){
      ...
      if (numOfFreeMints(msg.sender) > 0){
        require(numberOfTokens <= numOfFreeMints(msg.sender), "Number of free mints not greater or equal to the requested amount");
      } else {
        require(tokenPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
      }
    }
