...
abstract contract TeamMembers is Ownable {
  ...
  function addTeamMember(address _address) public onlyOwner {
    ...
    require(!isTeamMember(_address), "This address is already in your team.");
    EnumerableSet.add(members, _address);
  }

  function removeTeamMember(address _address) public onlyOwner {
    EnumerableSet.remove(members, _address);
  }

  function isTeamMember(address _address) public view returns (bool) {
    return EnumerableSet.contains(members, _address);
  }

  function getTeamMembers() public view {}

  modifier onlyTeamOrOwner() {
    bool _isOwner = owner() == _msgSender();
    bool _isTeam = isTeamMember(_msgSender());
    require(
      _isOwner || _isTeam,
      "Team: caller is not the owner or a team member"
    );
    _;
  }
}
