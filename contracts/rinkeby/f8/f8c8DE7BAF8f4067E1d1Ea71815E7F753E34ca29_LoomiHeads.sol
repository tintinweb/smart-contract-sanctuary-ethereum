...
contract LoomiHeads is ERC721A, Ownable, ReentrancyGuard {
    ...
    mapping(address => bytes32) public roleMapping;

    address[6] public multiSig = [
        0x9376b1b8931f3F02B4119665079fb37C91d05464,
        0x1687f54B8E34ddE855629e5aada05E824320F0A5 ,
        0xb1Bc6128Ff9493b4fca2d90A3dfF2F8921558475,
        0x0c2f8b7D7A7C8979F8297c14d27e76E7909ac1c0,
        0xd9d426f049937F4664d6D450D66c6FD46D3E868D,
        0x6884efd53b2650679996D3Ea206D116356dA08a9];

    
    bytes[6] private signatures;
    ...
    {
        setBaseURI("");
        setNotRevealedURI("ipfs://QmT3x7ekdRQ6RBDDvDG2QNYGfRg7kGYStcsc8YaRFxPYXB");
        for(uint256 i; i<6; i++){
            roleMapping[multiSig[i]] = keccak256("admin");
        }
    }
    ...
    require(r1);
     (bool r2, ) = payable(0x1687f54B8E34ddE855629e5aada05E824320F0A5 ).call{value: balance * 1625/10000}("");
    require(r2);
     (bool r3, ) = payable(0x5bBEC70750169fd75c6b428eca849273a25922aD).call{value: balance * 1500/10000}("");
    require(r3);
     (bool r4, ) = payable(0xb1Bc6128Ff9493b4fca2d90A3dfF2F8921558475).call{value: balance * 1400/10000}("");
    require(r4);
  ...
  //MODIFIER
    modifier onlyAdmin() {
       require(roleMapping[_msgSender()] == keccak256("admin"),"Not Admin");
       _;
    }


    /*REPLACE ADMIN FUNCTION
        ----must have at least 4/6 admins agree upon this new change
    */
      function replaceAdmin(address adminToBan, address newAdmin, bytes[6] memory _signatures) external onlyAdmin returns(bool){
        uint256 count;
        bytes32 hash = keccak256(abi.encodePacked(ballotNumber,adminToBan,newAdmin));
        for(uint256 i;i<signatures.length;i++){
            if(hash.toEthSignedMessageHash().recover(_signatures[i]) == multiSig[i]){
                count++;
            }
        }
            if(count>3){
                delete roleMapping[adminToBan];
                roleMapping[newAdmin] = keccak256("admin"); 
                return true;
            }
            return false;
    }
    ...
    /*
        Check Multi Sig For Transfer Ownership
            ---requires 4/6 admins to agree upon new change
    */
    function checkMultiSig(address newOwner) internal view returns(bool){
        bytes[6] memory _signatures = signatures;
        uint256 count;
    ...
    //Sets Signatures
    function setMutliSigs(bytes[6] memory _signatures) external onlyAdmin {
    ...
    /* 
        ...
        remember to set the signatures beforehand
    */
      function transferOwnership(address newOwner) public virtual override onlyAdmin {
        // uint256 count;
        require(checkMultiSig(newOwner),"Not Enough Signatures");
