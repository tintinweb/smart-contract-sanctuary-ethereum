...
contract DBCNFT is ERC721Enumerable, Ownable {
    using Strings for uint256;  

    uint256 public MAX_DBC = 8888; 
    uint256 public PRICE = 0.001 ether; 
    uint256 public MAX_DBC_MINT = 24;
    uint256 public MAX_PER_TX = 8;

    uint256 public constant HANDPICKED_MAX_MINT = 2;
    uint256 public constant PRESALE_MAX_MINT = 1;
    uint256 public constant RESERVED_DBC = 125; 
    address public constant founderAddress = 0x01e1E5d40889EB4d5CaC188E2B7138A16Ea0Bf46; 

    address private constant launchpadAddress = 0xb2513c8ccCCb91CB94AA00E4E261eFDbB20508e7; 
    address private constant donationAddress = 0xDfd143aE8592e8E3C13aa3E401f72E1ca7deAED0; 

    uint256 public reservedClaimed;

    uint256 public numDbcMinted;
    ...
    event HandpickedMint(address minter, uint256 amountOfDBC);
    event PresaleMint(address minter, uint256 amountOfDBC);
    event PublicSaleMint(address minter, uint256 amountOfDBC);
    ...
    constructor() ERC721("Draconian Billionaire Club", "DBC") {

    }

    function claimReserved(address recipient, uint256 amount) external onlyOwner {
        require(reservedClaimed + amount <= RESERVED_DBC, "Minting would exceed max reserved dbc");
        require(recipient != address(0), "Cannot add null address");
        // totalSupply();
        require(totalSupply() + amount <= MAX_DBC, "Minting would exceed max supply");

        uint256 _nextTokenId = numDbcMinted + 1;
    ...
    function mintPartnershipPresale() external payable whenPresaleStarted {
        ...
        require(totalSupply() + 1 <= MAX_DBC - (RESERVED_DBC - reservedClaimed), "Minting would exceed max supply");
        ...
        uint256 tokenId = numDbcMinted + 1;
    ...
    function mintHandpickedPresale(uint256 amountOfDBC) external payable whenPresaleStarted {
        require(_handpickedEligible[msg.sender], "You are not eligible for the handpicked presale");
        require(totalSupply() + amountOfDBC <= MAX_DBC - (RESERVED_DBC - reservedClaimed), "Minting would exceed max supply");
        require(_totalClaimedHandpicked[msg.sender] + amountOfDBC <= HANDPICKED_MAX_MINT, "Purchase exceeds max allowed");
        require(amountOfDBC > 0, "Must mint at least one dbc");
        require(PRICE * amountOfDBC <= msg.value, "ETH amount is incorrect");

        for (uint256 i = 0; i < amountOfDBC; i++) {
            uint256 tokenId = numDbcMinted + 1;
        ...
        emit HandpickedMint(msg.sender, amountOfDBC);
    }

    function mint(uint256 amountOfDBC) external payable whenPublicSaleStarted {
        require(totalSupply() + amountOfDBC <= MAX_DBC - (RESERVED_DBC - reservedClaimed), "Minting would exceed max supply");
        require(totalClaimed[msg.sender] + amountOfDBC <= MAX_DBC_MINT, "Purchase exceeds max allowed per address");
        require(amountOfDBC > 0, "Must mint at least one dbc");
        require(amountOfDBC <= MAX_PER_TX, "Amount over max per transaction. ");
        require(PRICE * amountOfDBC <= msg.value, "ETH amount is incorrect");
        ...
        emit PublicSaleMint(msg.sender, amountOfDBC);
    }

    function bulkPurchase(uint256 amountOfDBC) external payable {
        require(totalSupply() + amountOfDBC <= MAX_DBC - (RESERVED_DBC - reservedClaimed), "Minting would exceed max supply");
        require(amountOfDBC > 0, "Must mint at least one dbc");
        require(msg.sender == launchpadAddress || msg.sender == donationAddress, "Must be launchpad or donation wallet address");
        require(msg.sender == donationAddress || PRICE * amountOfDBC <= msg.value, "ETH amount is incorrect");
        require(amountOfDBC <= 100, "Cannot mint over 200");
