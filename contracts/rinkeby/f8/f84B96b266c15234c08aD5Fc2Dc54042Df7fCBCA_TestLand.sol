...
contract TestLand is Ownable, ERC721BStaticTokenURI {
    ...
    mapping(string => int256[]) private direcs;
    string public base_uri =
        "ipfs://bafybeifd52y3huj6ijsxqfi6s4aottkx66k4jmdspykjzpf5zafsyi5gpm/";

    constructor() ERC721BStaticTokenURI() {
        skyFallsPyramidAddress = 0x1069B160b341b56A411F55D552D1C60Ee6E2ab58;
        direcs["up"] = [int256(0), int256(10)];
        direcs["down"] = [int256(0), int256(-10)];
        direcs["left"] = [int256(-10), int256(0)];
        direcs["right"] = [int256(10), int256(0)];
        for (uint256 x = 0; x < 250; x += 10) {
            for (uint256 y = 0; y < 100; y += 10) {
                land_gend.push(x);
                land_gend.push(y);
            }
        }
    ...
    function mintTransferTest(address to, uint256 amount_req) public onlyOwner {
        ...
        if (amount_req == 250) {
            for (uint256 x = 0; x < 25; x++) {
                for (uint256 y = 0; y < 10; y++) {
                    string memory uri;
                    uri = string(abi.encodePacked(base_uri, x, "_", y));
            ...
            for (uint256 j = 0; j < amount_req; j++) {
                string memory uri;
                uri = string(abi.encodePacked(base_uri, "1", "_", "0"));
                uint256 tokenId = _tokenIdCounter.current();
                setTokenURI(tokenId, uri);
                _tokenIdCounter.increment();
            }
    ...
    function OverrideTokenURI(uint256 tokenId, string memory uri)
        external
        onlyOwner
    {
