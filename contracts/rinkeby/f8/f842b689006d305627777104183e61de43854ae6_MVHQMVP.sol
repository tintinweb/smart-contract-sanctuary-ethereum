    ...
    ERC721RoyaltyUpgradeable
{
    ...
    /// @param baseURI_ The URI base for metadata
    function initialize(string memory baseURI_, string memory contractURI_) external initializer {
        ...
        _contractURI = bytes(contractURI_).length > 0
            ? contractURI_
    ...
    /// @dev Only callable by an address with DEFAULT_ADMIN_ROLE
    /// @param contractURI_ The base URI
    function setContractURI(string memory contractURI_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(bytes(contractURI_).length > 0, "MVHQMVP: invalid URI");
