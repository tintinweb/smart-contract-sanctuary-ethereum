...
contract MarketTaker is IMarketTaker, BlockContext, ClearingHouseCallee, MarketTakerStorageV1 {
    ...
    function swap(SwapParams memory params) external override returns (SwapResponse memory) {
        ...
        bool isReducingPosition = takerPositionSize == 0
            ? false
            : takerPositionSize > 0 ==
                ((params.isBaseToQuote && params.isExactInput) || (!params.isBaseToQuote && !params.isExactInput));
    ...
    function _swap(SwapParams memory params) internal returns (InternalSwapResponse memory) {
        ...
        if (params.isBaseToQuote) {
            if (params.isExactInput) {
                uint256 quoteAmount = IAmm(amm).swap(1, 0, params.amount, params.amountLimit, params.isExactInput);
            ...
            uint256 limit0 = IAmm(amm).getDy(0, 1, params.amount, params.isExactInput);
            uint256 limit1;
            if (params.isExactInput) {
                swapAstp.exchangeFee = params.amount.mulRatio(swapAstp.exchangeFeeRatio);
                limit1 = IAmm(amm).getDy(0, 1, (params.amount - swapAstp.exchangeFee), params.isExactInput);
    ...
    function _isOverPriceLimitBySimulatingClosingPosition(
        ...
        uint256 priceAfter;
        if (isBaseToQuote) {
            (priceAfter, , ) = IAmm(amm).simulatedSwap(1, 0, amount, isExactInput);
        } else {
            (priceAfter, , ) = IAmm(amm).simulatedSwap(0, 1, amount, isExactInput);
        }
