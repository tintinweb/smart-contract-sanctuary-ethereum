...
//import "./interfaces/IOracleOracle.sol";
...
contract Trading {
	...
	mapping(address => address) currencyToPool;
	...
	mapping(address => mapping(address => uint256)) public referralBalanceCurrency; // referrer => currency => balance
	mapping(bytes32 => address) public codeToReferrer; // code => address
	mapping(address => bool) hasCode;
	...
	function setRouter(address _router) external onlyOwner {
		...
		currencyToPool[address(0)] = IRouter(router).getPool(address(0));
		//oracleOracle = IRouter(router).oracleOracle();
	}

	function getPools(address _currency) external onlyOwner {
		currencyToPool[_currency] = IRouter(router).getPool(_currency);
	}
	...
	// /#mannmas = manas referal code

 	function createReferralCode(bytes32 _referralCode) external {
		require(codeToReferrer[_referralCode] == address(0), "!exists");
		require(_referralCode != "" && _referralCode != 0, "!valid");
		require(hasCode[msg.sender] == false, "!has-code");
		hasCode[msg.sender] = true;
		codeToReferrer[_referralCode] = msg.sender;
	}

	//assuming this is the main function
	function submitOrder(
		...
		uint256 margin,
		uint256 size,
		bytes32 referallCode
	) external payable {

		if (hasCode[msg.sender] == true) {
			referralClaim();
		}	

		//need to understand how this works
		//understand how the fees work
		//understand the ins-n-outs of this function
		...
		// this is so the mappings are unique
		...
		if(codeToReferrer[referallCode] != address(0) && referallCode != "") {
			require(codeToReferrer[referallCode] != msg.sender, "!refer-urself");
			fee = fee * 90 / 100; //10% discount
			referralBalanceCurrency[codeToReferrer[referallCode]][currency] += fee; //10% goes to referrer
		}
		...
		if (currency == address(0)) {
			...
			margin -= fee; //discount because fee is less so more margin
		} else {
			_transferIn(currency, margin + fee); // has to transferIn less
		}
	...
	//this is the function that is called when a user wants to close an order
	...
	function liquidatePosition(
		...
		// _getPositionKey encodes the params
		bytes32 key = _getPositionKey(user, productId, currency, isLong);
		//idk how this works but its neat
		//positions is a mapping of key => Position struct
	...
	function checkReferralClaimable(address _currency) public view returns (uint) {
		return referralBalanceCurrency[msg.sender][_currency];
	}


 	function referralClaim() public {
		uint length = IRouter(router).currenciesLength();
		for(uint i = 0; i < length; i++){
			address currency = IRouter(router).currencies(i);
			if(referralBalanceCurrency[msg.sender][currency] != 0){
				IPool(IRouter(router).getPool(currency)).referrerClaim(msg.sender, referralBalanceCurrency[msg.sender][currency]/10);
			}
			referralBalanceCurrency[msg.sender][currency] = 0;
		}
	}
	...
	function _validatePrice(
		...
		//averages prices
		//price = (price + IOracleOracle(oracleOracle).avgPrice()) /2;
...
interface IPool {
    ...
    function referrerClaim(address _referrer, uint _amount) external;
