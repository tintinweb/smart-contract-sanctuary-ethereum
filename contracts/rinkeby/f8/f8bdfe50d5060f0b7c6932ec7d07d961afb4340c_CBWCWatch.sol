...
import './interfaces/ICBWCPieces.sol';
...
contract CBWCWatch is CBWCBase {
    ...
    event CBWCPiecesChanged(address indexed sender, address previousCBWCPieces, address cbwcPieces);
    ...
    event WatchForged(address indexed sender, uint256 indexed tokenId, uint256[] cbwcPiecesTokenIds);
    ...
    function initialize() external initializer {
        ...
        __CBWCBase_init(
            'CBWCWatch',
            'CBWCW',
            'ipfs://QmTWFyGvWTgfQ3wNzsF7XETLLtYD4Yo47UAjn7jUAUw6rn',
            'ipfs://QmTzGvU343VCCh2bXKVrqHoJGQFedLnpzmbtNdu5ZRNqiN/'
        );
    ...
    function forge(
        ...
        emit WatchForged(msg.sender, _currentTokenId, cbwcPiecesTokenIds_);
    }
    ...
    AccessControlUpgradeable
{
    ...
    function __CBWCBase_init(
        ...
        baseURI = baseURI_;
        owner = msg.sender;
        _setDefaultRoyalty(0x64777d628c1f01c01684eb23655A09Afae0a5AFE, 750);
    }
...
interface ICBWCPieces {
    ...
    function burnPieces(uint256[] calldata tokenIds_, address owner_) external returns (bool);
...
abstract contract ERC721KFNCUUPSUpgradeable is IERC721KFNC, UUPSUpgradeable {
    ...
    function tokenURI(uint256 _tokenId) external view virtual returns (string memory) {
        if (_tokenId < _startingTokenId || _tokenId > _currentTokenId) revert QueryNonExistentToken();
    ...
    function totalSupply() external view returns (uint256) {
        return _currentTokenId > _startingTokenId ? _currentTokenId - _startingTokenId - _burnCounter: 0;
    }
    ...
    function approve(address _approved, uint256 _tokenId) external {
        ...
        if (owner != msg.sender && !_operators[msg.sender][_approved] && _tokenOperators[_tokenId] != msg.sender)
    ...
    /// @param name_ Name of the contract
    /// @param symbol_ An abbreviated name for NFTs in this contract
    function __ERC721_init_unchained(string memory name_, string memory symbol_) public {
        _name = name_;
        _symbol = symbol_;
        _startingTokenId = startingTokenId();
    }
    ...
    function safeMint(address _to, uint256 _quantity, bytes memory _data) internal {
        _mint(_to, _quantity);
        unchecked {
            if (_to.code.length != 0) {
                uint256 tokenId = _currentTokenId - _quantity;
                do {
                    if (!_checkERC721Received(address(0), _to, ++tokenId, _data)) {
                        revert TransferToNonERC721ReceiverImplementer();
                    }
                } while (tokenId < _currentTokenId);
            }
        }
    }
    ...
    function _mint(address _to, uint256 _quantity) internal {
        unchecked {
            ...
            uint256 newTotal = _currentTokenId + _quantity + 1;

            for (uint256 i = _currentTokenId + 1; i < newTotal; i++) {
                emit Transfer(address(0), _to, i);
            }

            _tokenOwnersOrdered[_currentTokenId + 1] = _to;
            _currentTokenId = newTotal;
        }
    }
    ...
    function _exists(uint256 _tokenId) internal view returns (bool) {
        return _tokenId >= _startingTokenId && _tokenId <= _currentTokenId && !_burnedTokens[_tokenId];
    }
