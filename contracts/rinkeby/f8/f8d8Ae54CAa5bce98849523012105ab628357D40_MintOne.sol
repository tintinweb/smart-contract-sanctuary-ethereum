...
contract MintOne is Ownable {
    ...
    uint256 public maxSupply = 100; // this should change
    bool public paused = false;

    YfuNFT public yfuNFT;

    address public beesEscrow;

    mapping(address => uint256) public whitelisted;

    constructor(address _yfuNFT, uint256 _cost) {
        yfuNFT = YfuNFT(_yfuNFT);
        cost = _cost;
    }

    // public
    function mint(address _to) public payable {
        uint256 supply = yfuNFT.totalSupply();
        require(!paused, "Paused");
        require(msg.value >= cost, "!cost min amount");

        nftIdCounter.increment();
        yfuNFT.mint(_to, nftIdCounter.current());
    }
    ...
    function whitelistUsers(
        ...
        require(
            users.length == quantity.length,
            "Must provide equal quantities and users"
        );
