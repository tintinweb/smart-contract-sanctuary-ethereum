...
contract FancyTraitV2Sale is AccessControlEnumerable, ReentrancyGuard {
    ...
    event Purchase(address indexed _sender, uint256[] _traitIds, uint256[] _quantities);
    event SaleDataUpdated(uint256 indexed _tokenId);
    event HoneyPriceUpdated(uint256 indexed _tokenId, uint256 _price);
    ...
    event WithdrawHoney(address _destination, uint256 _amount);
    ...
    function purchaseTraitsWithHoney(PurchaseData calldata purchaseData)
        public
    {
        require(
        ...
        for (uint256 i = 0; i < purchaseData.traitTokenIds.length;) {
            ...
            totalHoneyRequired += (tokenSaleData[purchaseData.traitTokenIds[i]])
                .honeyPrice
        ...
        if (
            ...
            honeyVestingContract.spendHoney(
                purchaseData.amountToSpendFromBear[0] > 0 ? purchaseData.fancyBear : new uint256[](0),
                purchaseData.amountToSpendFromBear[0] > 0 ? purchaseData.amountToSpendFromBear : new uint256[](0),
                purchaseData.honeyJars.length > 0 ? purchaseData.honeyJars : new uint256[](0),
                purchaseData.honeyJars.length > 0 ? purchaseData.amountToSpendFromHoneyJars : new uint256[](0)
            );
        }
        ...
        if(purchaseData.fancyBear.length > 0){
            ...
            fancyBearHoneyConsumptionContract.consumeHoney(
                purchaseData.fancyBear[0],
                totalHoneyRequired
            );
        ...
        emit Purchase(msg.sender, purchaseData.traitTokenIds, purchaseData.amountPerTrait);
    }
    ...
    function purchaseTraitsWithETH(
        ...
        for (uint256 i = 0; i < _traitTokenIds.length; i++) {
            require(
    ...
    function updateHoneyPrice(
        ...
        require(
            ...
            "updateHoneyPrice: token Ids and prices array must match in length"
        );
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            tokenSaleData[_tokenIds[i]].honeyPrice = _prices[i];
            emit HoneyPriceUpdated(_tokenIds[i], _prices[i]);
        }
    }
    ...
    function deleteSaleData(uint256[] calldata _tokenIds)
        public
        onlyRole(MANAGER_ROLE)
    {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
    ...
    function withdrawHoney(address _beneficiary, uint256 _amount)
        public
        onlyRole(WITHDRAW_ROLE)
    {
        honeyTokenContract.safeTransfer(_beneficiary, _amount);
        emit WithdrawHoney(_beneficiary, _amount);
    }
