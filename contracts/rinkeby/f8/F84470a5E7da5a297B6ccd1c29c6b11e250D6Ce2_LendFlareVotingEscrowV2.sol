...
contract LendFlareVotingEscrowV2 is
    Initializable,
    ReentrancyGuard,
    ILendFlareVotingEscrow
{
    ...
    function _checkpoint(
        ...
        uint256 timeStamp = formatWeekTs(lastCheckpoint);
        ...
        for (uint256 i; i < 255; i++) {
            ...
            lastPoint.bias -=
                lastPoint.slope *
                int256(timeStamp - lastCheckpoint);
    ...
    //         point.slope = int256(_newLocked.amount.div(MAXTIME));
    //         point.bias =
    //             point.slope *
    //             int256(_newLocked.end.sub(block.timestamp));
    //     }
    // }
        ...
        returns (uint256)
    {
        ...
        return
            uint256(
                point.bias -
                    (point.slope * int256(block.timestamp.sub(point.timestamp)))
            );
    }
    ...
    function supplyAt(Point memory point, uint256 timeStamp)
        internal
        view
        returns (uint256)
    {
        uint256 _timeStamp = formatWeekTs(point.timestamp);
