...
contract Plus1Factory {
    ...
    function createPlus1(
        ...
        _plus1s[address(original)].push(p1);
        emit CreatePlus1(address(p1));
    }

    function numPlus1s(address original) external view returns (uint256) {
        return _plus1s[original].length;
    }

    function getPlus1AtIndex(address original, uint256 index)
        external
        view
        returns (Plus1)
    {
    ...
    function getPlus1s(address original)
        external
        view
        returns (Plus1[] memory)
    {
        return _plus1s[original];
    }
}
