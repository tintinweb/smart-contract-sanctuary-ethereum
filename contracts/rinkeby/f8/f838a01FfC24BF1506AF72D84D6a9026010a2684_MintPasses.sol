...
contract MintPasses is Context, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    mapping (address => uint256) public bidIdOfAddress;
    ...
    function setPause(bool _active) external onlyOwner {
        active = _active;
    }
    ...
    function claimPass() external onlyWhenFinished {
        require(bidIdOfAddress[msg.sender] != 0, "User didn't win");

        Bid storage bid = bids[bidIdOfAddress[msg.sender]];
        bidIdOfAddress[msg.sender] = 0;

        _safeMint(msg.sender, _tokenIdTracker.current());
        _tokenIdTracker.increment();

        emit PassClaimed(msg.sender, _tokenIdTracker.current() - 1, bidIdOfAddress[msg.sender], block.timestamp);
    }

    function bid() external payable onlyActive nonReentrant {
        require(msg.value >= minimumBidAmount, "There is not enough funds to make a bid");
        require(bidIdOfAddress[msg.sender] == 0, "User already have an active bid");
        
        bids[latestBidId] = Bid(msg.value, msg.sender);
        bidIdOfAddress[msg.sender] = latestBidId;
        ...
        if(latestBidId > totalBidsLimit) {
            ...
            payable(bid.bidder).transfer(amount);
            
            emit Refund(msg.sender, msg.value, latestBidId - totalBidsLimit,  block.timestamp);
        }
