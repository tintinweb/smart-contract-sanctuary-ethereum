...
contract CRFTDStakingToken is FxERC721sRootTunnelUDS, ERC20RewardUDS, OwnableUDS, UUPSUpgrade {
    ...
    bytes32 constant MINT_TOKENS_SIG = keccak256("mintERC20Tokens(address,uint256)");
    uint256 constant MIGRATION_START_DATE = 1 << 42;
    ...
    function migrationStarted() public view returns (bool) {
        return s().rewardEndDate == MIGRATION_START_DATE;
    }
    ...
    function safeMigrate(address[] calldata collections, uint256[][] calldata tokenIds) external {
        ...
        for (uint256 i; i < collections.length; ) {
            ...
            // duplicates are caught in child registry and would only hurt the user
            _registerERC721IdsWithChild(collections[i], msg.sender, ids);
        ...
        if (_getRewardMultiplier(msg.sender) != rewardMultiplier) revert MigrationIncomplete();
        ...
        // send remaining user balance to L2
        _sendMessageToChild(abi.encode(MINT_TOKENS_SIG, abi.encode(msg.sender, balance)));
    }
    ...
    function unstake(address collection, uint256[] calldata tokenIds) external {
        ...
        // check for multiplier and optionally require migration?
        if (migrationStarted()) {
...
bytes32 constant REGISTER_SIG = keccak256("registerERC721IdsWithChild(address,address,uint256[])");
bytes32 constant DEREGISTER_SIG = keccak256("deregisterERC721IdsWithChild(address,uint256[])");
