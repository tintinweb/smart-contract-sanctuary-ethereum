...
// Ags token with Governance.
contract AgsToken is BEP20 {
    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private _minters;
    
    constructor(
        uint256 __totalSupply
    ) public BEP20("Alter Grimace Society Token", "AGS") {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "AGS::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "AGS::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "AGS::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "AGS::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying AGSs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "AGS::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    function addMinter(address _addMinter) public onlyOwner returns (bool) {
        require(
            ...
            "AGS: _addMinter is the zero address"
        );
    ...
    function delMinter(address _delMinter) public onlyOwner returns (bool) {
        require(
            ...
            "AGS: _delMinter is the zero address"
        );
    ...
    function getMinter(uint256 _index) public view onlyOwner returns (address) {
        require(_index <= getMinterLength() - 1, "AGS: index out of bounds");
...
contract MasterChef is Ownable, ReentrancyGuard, Whitelist {
    ...
    struct PoolInfo {
        ...
        uint256 accAgsPerShare;   // Accumulated AGS per share, times 1e18. See below.
    }
    ...
    function pendingAgs(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accAgsPerShare = accAgsPerShare.add(agsReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accAgsPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accAgsPerShare = pool.accAgsPerShare.add(agsReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for AGS allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant nonContract {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accAgsPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accAgsPerShare).div(1e18);
    ...
    function safeAgsTransfer(address _to, uint256 _amount, uint256 _pid) internal {
        ...
        if (enableNFTBoost) {
            ...
            if (boost > 0) agsToken.mint(_to, boost);
        }
    }
...
contract SyrupBar is BEP20("SyrupBar Token", "SYRUP") {
    ...
    // Safe ags transfer function, just in case if rounding error causes pool to not have enough AGSs.
    function safeAgsTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 agsBal = ags.balanceOf(address(this));
        if (_amount > agsBal) {
            ags.transfer(_to, agsBal);
        } else {
            ags.transfer(_to, _amount);
        }
    }
    ...
    */
        ...
        external
    {
        ...
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "AGS::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "AGS::delegateBySig: invalid nonce");
        require(now <= expiry, "AGS::delegateBySig: signature expired");
        return _delegate(signatory, delegatee);
    }
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "AGS::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "AGS::_writeCheckpoint: block number exceeds 32 bits");
