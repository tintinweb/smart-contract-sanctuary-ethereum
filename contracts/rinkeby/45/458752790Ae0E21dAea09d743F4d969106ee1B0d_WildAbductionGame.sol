...
import "./interfaces/IWildAbductionGame.sol";
import "./interfaces/IBank.sol";
import "./interfaces/ITraits.sol";
import "./interfaces/ILAND.sol";
import "./interfaces/ISeed.sol";

contract WildAbductionGame is IWildAbductionGame, ERC721, Ownable, Pausable {
    ...
    // mapping from address to bool for free mint addresses
    mapping(address => bool) public freeMintAddresses;
    // mapping from address to bool for whitelisted addresses
    mapping(address => bool) public allowList;

    // whitelist toggle
    bool private public_mint;
    // reference to the Bank for choosing random Alien thieves
    IBank public bank;
    // reference to $LAND for burning on mint
    ILAND public land;
    // reference to Traits
    ...
    // can't have a constructor with an upgradeable contract

    /**
     * instantiates contract and rarity tables
     */
    constructor(ILAND _land, uint256 _maxTokens) ERC721("WAG Game", 'WGAME') {
        land = _land;
    ...
    function mint(uint256 amount, bool stake) external payable nonReentrant whenNotPaused {
        ...
        require(public_mint || minted < 3000);
        ...
        if (minted < PAID_TOKENS) {
            require(minted + amount <= PAID_TOKENS, "oos");
            if (freeMintAddresses[msg.sender] == true && amount == 1) {
                require(msg.value == 0);
        ...
        if (totalLandCost > 0) land.burn(_msgSender(), totalLandCost);
   ...
   */
    function mintCost(uint256 tokenId) public view returns (uint256) {
        ...
        if (tokenId <= MAX_TOKENS * 1 / 2) return 20000 ether;
   ...
   */
    function selectRecipient(uint256 seed) internal view returns (address) {
        ...
        address thief = bank.randomAlienOwner(seed >> 144);
   ...
   */
    function selectTraits(uint256 seed) internal view returns (CowboyAlien memory t) {
        ...
        seed >>= 16;
        t.accessory = selectTrait(uint16(seed & 0xFFFF), 0 + shift);

        seed >>= 16;
        t.hat = selectTrait(uint16(seed & 0xFFFF), 1 + shift);

        seed >>= 16;
        t.top = selectTrait(uint16(seed & 0xFFFF), 2 + shift);

        seed >>= 16;
        t.pants = selectTrait(uint16(seed & 0xFFFF), 3 + shift);

        seed >>= 16;
        t.base = selectTrait(uint16(seed & 0xFFFF), 4 + shift);
   ...
   */
    ...
    /**
     * called after deployment so that the contract can get random alien thieves
   ...
   */
    ...
    function setPaused(bool _paused) external onlyOwner {
     ...
     function addFreeMint(address[] memory addresses, bool freeMint) external onlyOwner {
        require(freeMintCount <= 200);
        if (freeMint) {
...
interface IBank {
    ...
    function pack(uint256, uint256) external view returns(Bank3.Stake memory);
...
contract Bank3 is Ownable, IERC721Receiver, Pausable {
    ...
    event CowboyClaimed(uint256 tokenId, uint256 earned, bool unstaked);
    event AlienClaimed(uint256 tokenId, uint256 earned, bool unstaked);
    event MutantClaimed(uint256 tokenId, uint256 earned, bool unstaked);

    ISeed public randomSource;

    // reference to the WAG NFT contract
    WildAbductionGame game;
   ...
   */
    constructor(WildAbductionGame _game, LAND _land, IBank _oldBank, address _swapper) {
        game = _game;
        land = _land;
    ...
    function addManyToBankAndPack(address account, uint16[] calldata tokenIds) external nonReentrant {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (isMutant(tokenIds[i]))
                _addMutantToBank(account, tokenIds[i]);
   ...
   */
    ...
    /**
     * adds a single Mutant to the Bank
     * @param account the address of the staker
   * @param tokenId the ID of the Mutant to add to the Bank
   */
    function _addMutantToBank(address account, uint256 tokenId) internal whenNotPaused _updateEarnings {
        bank[tokenId] = Stake({
        owner : account,
        tokenId : uint16(tokenId),
        value : uint80(block.timestamp)
   ...
   */
    function claimManyFromBankAndPack(uint16[] calldata tokenIds, bool unstake) external nonReentrant whenNotPaused _updateEarnings {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (isMutant(tokenIds[i]))
                owed += _claimCowboyOrMutantFromBank(tokenIds[i], unstake, false);
            else if (isCowboy(tokenIds[i]))
                owed += _claimCowboyOrMutantFromBank(tokenIds[i],unstake, true);
            else
    ...
    /**
     * realize $LAND earnings for a single Cowboy and optionally unstake it
   ...
   */
    function _claimCowboyOrMutantFromBank(uint256 tokenId, bool unstake, bool cowboy) internal returns (uint256 owed) {
        ...
        if (totalLANDEarned < MAXIMUM_GLOBAL_LAND) {
            ...
            if (randomNo % 3 == 0) {
                extraHalf = false;
                stakeRate = 2;
            } else if (randomNo % 7 == 0) {
                extraHalf = false;
                stakeRate = 3;
            } else if (randomNo % 9 == 0) {
                extraHalf = false;
                stakeRate = 4;
            }
             owed = (block.timestamp - stake.value) * stakeRate / 1 days;
            if (extraHalf) {
                owed += owed / 2;
            }
            }
        ...
        if (unstake) {
            if (cowboy) {
            if (random(tokenId) & 1 == 1) {// 50% chance of all $LAND stolen
                _payAlienTax(owed);
                owed = 0;
            }
        ...
        if (cowboy){
        emit CowboyClaimed(tokenId, owed, unstake);
        } else {
   ...
   */
    function _claimAlienFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
        ...
        emit AlienClaimed(tokenId, owed, unstake);
    }
   ...
   */
    function rescue(uint256[] calldata tokenIds) external nonReentrant {
        ...
        (tokenId, 0, true);
                ...
                delete packIndices[tokenId];
                // Delete old mapping
                emit AlienClaimed(tokenId, 0, true);
            }
        }
    }
   ...
   */
    ...
    /**
     * checks if a token is a Mutant
     * @param tokenId the ID of the token to check
   * @return Mutant - whether or not a token is a Mutant
   */
   ...
   */
    ...
    function setGame(WildAbductionGame _nGame) public onlyOwner {
        game = _nGame;
    }
}
