...
import "./ERC721S.sol";
import "@nervous-net/contract-kit/src/WalletMintLimit.sol";
import "@nervous-net/contract-kit/src/MultiTimedMerkleRoot.sol";
import "@nervous-net/contract-kit/src/MultiTimedSignedPasses.sol";
    ...
    MultiTimedSignedPasses(3)
{
    ...
    string public constant WE_ARE_NERVOUS =
    ...
    uint64 public endMintDate = type(uint64).max;
    ...
    function merkleProofPresale(uint256 index)
        external
        view
    ...
    function setPublicMintDate(uint256 _startPublicMintDate)
        ...
        onlyOwner
    {
        startPublicMintDate = uint64(_startPublicMintDate);
    }

    function setEndMintDate(uint256 _endMintDate) external onlyOwner {
        endMintDate = uint64(_endMintDate);
    }
    ...
    function setMerkleRoot(
        ...
        _setTimedMerkleRoot(index, root, startTimestamp);
    }
    ...
    modifier requireValidMerkleProof(address to, bytes32[] memory merkleProof) {
        if (block.timestamp < startPublicMintDate) {
            if (merkleProof.length == 0) {
                revert("Public sale hasn't started");
            }
            if (!_checkTimedMerkleRoots(to, merkleProof)) {
