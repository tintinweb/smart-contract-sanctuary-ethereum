...
abstract contract ERC721TradableHTML is ContextMixin, ERC721Enumerable, NativeMetaTransaction, Ownable {
    ...
    string private _theURI = "data:text/html;base64,PGh0bWw+PGJvZHk+DQo8aDE+QWJyYW1vIG11c288L2gxPg0KPC9ib2R5PjwvaHRtbD4=";
    ...
    function setTheURI( string memory _newURI) public {
        _theURI = _newURI;
    }
        ...
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            '{"name":"',"Ser?"'", "animation_url":"',imageURI,'"}'
                        )
                    )
                )
            )
...
/// @author: HTMLNFT
//  https://www.HTMLnft.com/
//  HTMLNFT - Roman Empire Set

import "./ERC721TradableHTML.sol";

/**
 * @title HTML
 * HTMLNFT: Roman Empire Collection
 */
...
contract HTML is ERC721TradableHTML {
    using SafeMath for uint256;
    constructor(address _proxyRegistryAddress) ERC721TradableHTML("HTMLNFT Roman Empire Collection", "ROMA", _proxyRegistryAddress) {}
    ...
    function setTokenData(uint256 _tokenId, address _address) public onlyHTML {
    ...
    function tokenURI(uint256 _tokenId) override public view returns (string memory) {
         ...
         return formatTokenURI(SubContract.getTOKEN(1)); 
    }
    // Sale
    ...
    modifier onlyHTML() {
        require((owner() == _msgSender()), "You are not HTML!");
        _;
    }

    function setBaseMetadataURI( string memory _newBaseMetadataURI) public onlyHTML {
        _theBaseURI = _newBaseMetadataURI;
    }

    function activate() external onlyHTML {
        require(!_active, "Already active");
        _active = true;
    }

    function pause() external onlyHTML {
        require(_active, "Already paused");
        _active = false;
    }

    function premine(uint256 coins) external onlyHTML {
        require(!_active, "Already active");
        for (uint i = 0; i < coins; i++) {
            mintTo(msg.sender);
        }
    ...
    function withdraw(address payable recipient, uint256 amount) external onlyHTML {
