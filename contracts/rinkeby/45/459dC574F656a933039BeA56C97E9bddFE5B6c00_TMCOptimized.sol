...
/**
 * @title TMCTest - Membership Card
 * @dev struct allows us to optimize gas for different configs for each Card
 ...
 * Defined 2 methods for each card which allows OG and Public mint
 * @author heet_v
 */
contract TMCOptimized is AbstractERC1155Factory {
    ...
    function ogStandardMint(
        ...
        require(msg.value == numberOfTokens * (standardConfig.tokenPrice), "Sent price is not correct");
        
        // check max per wallet is not exceeded
        uint256 maxPerWallet = standardConfig.maxPerWallet;
        ...
        require(totalSupply(TOKEN_ID_STANDARD) + numberOfTokens <= (standardConfig.maxTokens), "Supply reached max tokens");
    ...
    function ogEpicMint(
        ...
        // Check correct price is sent
        require(msg.value == numberOfTokens * (epicConfig.tokenPrice), "Sent price is not correct");
        
        // check max per wallet is not exceeded
        uint256 maxPerWallet = epicConfig.maxPerWallet;
        require(numberOfTokens > 0 && numberOfTokens <= maxPerWallet, "Purchase amount prohibited");
        require(purchaseTxsEpic[msg.sender] + numberOfTokens <= maxPerWallet , "Minting amount exceeds max allowed per wallet");

        // Check if max supply reached
        require(totalSupply(TOKEN_ID_EPIC) + numberOfTokens <= (epicConfig.maxTokens), "Supply reached max tokens");
    ...
    function ogLegendaryMint(
        ...
        // Check correct price is sent
        require(msg.value == numberOfTokens * (legendaryConfig.tokenPrice), "Sent price is not correct");
        
        // check max per wallet is not exceeded
        uint256 maxPerWallet = legendaryConfig.maxPerWallet;
        require(numberOfTokens > 0 && numberOfTokens <= maxPerWallet, "Purchase amount prohibited");
        require(purchaseTxsLegendary[msg.sender] + numberOfTokens <= maxPerWallet , "Minting amount exceeds max allowed per wallet");

        // Check if max supply reached
        require(totalSupply(TOKEN_ID_LEGENDARY) + numberOfTokens <= (legendaryConfig.maxTokens), "Supply reached max tokens");
    ...
    /**
      ...
      */
    function futureMint(address account, uint256 _tokenId, uint256 numberOfTokens) public onlyOwner
    {
