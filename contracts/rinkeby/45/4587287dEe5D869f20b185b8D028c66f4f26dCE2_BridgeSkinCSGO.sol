    ...
    Ownable
{
    ...
    function safeMint(string memory uri, uint256 tokenID, bytes memory sign) public {
        require(mintedTokens[tokenID] == 0, "TOKEN ALREADY MINTED");
        require(verifyMessage(append(uri, Strings.toString(tokenID)), sign) == true, "BAD SIGN");
        ...
        returns (bool)
    {
        ...
        address signeraddress = m.recover(sign);
        return owner() == signeraddress;
    }

    event Withdraw(address indexed _from, uint256 indexed _tokenID);

    function withdraw(uint256 tokenID) public whenNotPaused {
        address tokenOwner = ownerOf(tokenID);
        require(msg.sender == tokenOwner, "YOU NOT OWNER");
