...
contract Validator {
    address constant randomAddress1 =
        0xa000000000000000000000000000000000000000;
    address constant randomAddress2 =
        0xB000000000000000000000000000000000000000;
    address constant randomAddress3 =
    ...
    function _validateMetadata(
        ...
        require(
            keccak256(bytes(IERC20Metadata(simple_).name())) ==
                keccak256(bytes("SimpleToken")),
            "Validator: wrong SimpleToken name"
        );
        require(
            keccak256(bytes(IERC20Metadata(simple_).symbol())) ==
                keccak256(bytes("ST")),
            "Validator: wrong SimpleToken symbol"
        );
        require(
            IERC20Metadata(simple_).decimals() == 18,
            "SimpleToke: wrong decimals"
        );
        ...
        require(
            keccak256(bytes(IERC20Metadata(contractsHater_).symbol())) ==
                keccak256(bytes("CHT")),
        ...
        require(
            keccak256(bytes(IERC20Metadata(mrGreedy_).symbol())) ==
                keccak256(bytes("MRG")),
            "Validator: wrong mrGreedy symbol"
        );
        require(
            IERC20Metadata(mrGreedy_).decimals() == 6,
            "Validator: wrong mrGreedy decimals"
        );
    }
