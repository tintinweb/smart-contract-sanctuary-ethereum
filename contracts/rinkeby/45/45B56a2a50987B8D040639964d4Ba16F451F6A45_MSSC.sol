...
contract MSSC is SSCVault {
    ...
    function registerSettlementCycle(
        ...
        for (uint256 i = 0; i < totalInstructions; ) {
            ...
            newInstructions.push(instruction.id);
            _instructions[instruction.id] = instruction;
    ...
    function deposit(bytes32 instructionId) external payable {
        ...
        _deposit(depositItem, instruction.amount);
    }
    ...
    /**
     * @notice View function to get deposited funds to a instruction by an account.
     *
...
contract SSCVault is Assertions {
     ...
     *
     * @param instructionId  The instruction identifier of the attempted operation.
     */
    error NoDeposits(bytes32 instructionId);
     ...
     * @param instructionId  The instruction identifier of the attempted operation.
     */
    error AlreadyDeposited(bytes32 instructionId);

    // Events
    event Deposit(address indexed account, bytes32 instructionId);
    event Withdraw(address indexed account, bytes32 instructionId);

    // Track allocated funds to various instructions.
    mapping(bytes32 => uint256) internal _deposits;

    // Track sender of instruction
    mapping(bytes32 => address) private _senderOf;
    ...
    function _deposit(DepositItem memory item, uint256 instructionAmount)
        internal
    {
        _assertNonZeroAmount(instructionAmount);
...
contract Assertions is MSSCErrors {
    ...
    function _assertValidInstruction(Instruction memory instruction)
        ...
        view
    {
        _assertNonZeroAmount(instruction.amount);
...
string constant ERR_MSG_NOT_ENOUGH_ALLOWANCE = "not enough approvals";
...
interface MSSCErrors {
    /**
     * @dev Revert with an error when attempting to give an amount of zero where
     ...
     * @param instructionId  The instruction identifier of the attempted operation.
     */
    error InvalidSuppliedETHAmount(bytes32 instructionId);
     ...
     * @param instructionId  The instruction identifier of the attempted operation.
     */
    error NotASender(bytes32 instructionId);
}