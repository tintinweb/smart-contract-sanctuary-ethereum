...
contract GPC2 is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint96 royaltyFeesInBips = 999; //royalty fee in bases points (100 = 1% 999 = 9.99%)
    address royaltyReceiver; //address to deposit royalties
    ...
    uint256 public cost = 0.01 ether; //mint price
        ...
        nonReentrant
    {
        ...
        _safeMint(
            msg.sender,
            quantity,
            (currentMaxSupply + mutantAddIndex + mutantsSupply[_osTokenId])
        );
    ...
    function getSerumBalance(uint256 _osTokenId, address _testAddress)
        public
        view
        virtual
        returns (uint256)
    {
        return GPC_CHEMISTRY_CONTRACT.balanceOf(_testAddress, _osTokenId);
    }
