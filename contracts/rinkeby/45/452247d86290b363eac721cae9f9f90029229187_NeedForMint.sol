...
contract NeedForMint {
    ...
    uint public subscriptionMonthlyPrice = 0.01 ether;
    ...
    function storeUser(address _address, uint _subscriptionExpires, uint _role, uint _ref) public {
    ...
    function modifyUser(address _address, uint _role, uint _ref) external onlyAdmin {
        User memory user = users[_address];
        if (user.role == 0) {
            storeUser(_address, 0, _role, _ref);
        } else {
