...
contract SampleNFT is ERC721, Ownable, ReentrancyGuard {

    enum MintType{PRESALE, WHITELIST, PUBLIC}

    struct MintSettings {
        uint256 maxPresaleMintId;
        uint256 presaleMintId;

        uint256 maxWhitelistId;
        uint256 whitelistId;
        uint256 whitelistSalePrice;

        uint256 maxPublicMint;
        uint256 publicMintId;
        uint256 PublicSalePrice;

        uint8 maxPresaleMintPerWallet;
        uint8 maxPrivateMintPerWallet;

        bool publicMint;
    }

    function setMintSettings(
        uint256 _maxPresaleMintId,
        uint256 _presaleMintId,
        ...
        uint256 _publicMintId,
        uint256 _PublicSalePrice,

        uint8 _maxPresaleMintPerWallet,
        uint8 _maxPrivateMintPerWallet,

        bool _publicMint
    ) internal virtual onlyOwner {
        require(!freezeSettings,"settings frozen");
        mintSettings.maxPresaleMintId = _maxPresaleMintId;
        mintSettings.presaleMintId = _presaleMintId;

        mintSettings.maxWhitelistId = _maxWhitelistId;
        mintSettings.whitelistId =  _whitelistId;
        mintSettings.whitelistSalePrice =  _whitelistSalePrice;

        mintSettings.maxPublicMint = _maxPublicMint;
        mintSettings.publicMintId = _publicMintId;
    ...
    **/
    ...
    constructor(string memory _baseURI, string memory collectionURI) ERC721("Sample NFT", "NFT") {
        ...
        mintSettings.maxPresaleMintId = 499;

        mintSettings.presaleMintId = 0;

        mintSettings.maxWhitelistId = 2000;
        mintSettings.whitelistId = 6;
        mintSettings.whitelistSalePrice = 0.3 ether;

        mintSettings.publicMintId = 2000;
        mintSettings.maxPublicMint = 3000;
        mintSettings.PublicSalePrice = 0.3 ether;
     ...
     */
    modifier isValidMerkleProof(bytes32[] calldata merkleProof, bytes32 root, uint8 amount) {
        require(
            MerkleProof.verify(
                merkleProof,
                root,
    ...
    modifier canMint(uint256 numberOfTokens, MintType mintType) {
        ...
        if (mintType == MintType.WHITELIST)
            require(
                mintSettings.whitelistId + numberOfTokens <= mintSettings.maxWhitelistId,
                "Not enough tokens remaining to mint"
            );
        if (mintType == MintType.PUBLIC)
            require(
                mintSettings.publicMintId + numberOfTokens <= mintSettings.maxPublicMint,
                "Not enough tokens remaining to mint"
            );
        _;
    }
    ...
    /**
    * @dev mints # token per whitelisted presale address, does not charge a fee
    */
    ...
    public
    isValidMerkleProof(merkleProof, presaleMerkleRoot, amount)
    canMint(amount, MintType.PRESALE)
    nonReentrant
    {
        require(!claimedPre[msg.sender], "NFT is already claimed by this wallet");
        claimedPre[msg.sender] = true;
        for (uint256 i = 0; i < amount; i++) {
            _mint(msg.sender, mintSettings.presaleMintId);
            mintSettings.presaleMintId++;
        }
    }

    /**
    * @dev mints # token per whitelisted address, charges a fee
    */
    ...
    */
    ...
    {
        for (uint256 i = 0; i < amount; i++) {
            _mint(msg.sender, mintSettings.publicMintId);
            mintSettings.publicMintId++;
        }
    }
    ...
    */
    ...
    function freezeMintSettings() external onlyOwner {
        require(!freezeSettings, "already frozen");
        freezeSettings = true;
    }
