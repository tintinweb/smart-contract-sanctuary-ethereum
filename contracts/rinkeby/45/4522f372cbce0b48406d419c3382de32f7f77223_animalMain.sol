...
contract animalMain is  ERC721,Ownable {
  ...
  uint256 public totalMintValue = 0; 
  ...
  function createAnimal() public payable{
      ...
      dna = keccak256(abi.encodePacked(now, msg.sender, uint8(0)));
      star = randMod(5);
      roletype = randMod(3);
       
      uint id = animals.push(animal(dna, uint8(star), uint16(roletype))) - 1;
  ...
  function sendMoneyToOwner(uint money) public onlyOwner returns(bool){
