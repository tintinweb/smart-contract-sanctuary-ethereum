...
contract NFT is 
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721A) returns (string memory) {
        ...
        return bytes(NFTSettings.baseURI).length > 0 ? string(abi.encodePacked(NFTSettings.baseURI, tokenId.toString(), '.json')) : "";
    }
    ...
    function airdropPets(
        ...
        if (whitelist.length == 0 || whitelist_quanityPerUser.length != whitelist.length) {
            revert NFTErrorCodes.ArrayMissmatch();
        }
        ...
        for (uint256 i = 0; i < whitelist.length; i++) {
            ...
            emit NFTEvents.NFTAirdropped(targetAddress, targetQuantity);
        }
    }
    ...
    public payable whenNotPaused
    {
        ...
        if (NFTSettings.whitelistMintActive == false) { 
                revert NFTErrorCodes.WhitelistMintNotActive();
            }
        ...
        if (msg.sender != owner()) 
        {
            if(isWhitelisted(msg.sender) == false) {
                revert NFTErrorCodes.NotWhitelisted();
            }
            uint256 cost = NFTSettings.whitelistMintCost * _mintAmount;
            if(msg.value < cost){
                revert NFTErrorCodes.InsufficientFunds(cost, msg.value);
            }

            if(balanceOf(msg.sender) + _mintAmount > NFTSettings.maxMintPerWallet){
                revert NFTErrorCodes.MaxAmountMinted(NFTSettings.maxMintPerWallet, balanceOf(msg.sender) + _mintAmount);
            }
            //TODO: add an error code or remove this check
            require(_mintAmount <= NFTSettings.maxMintPerTransaction, "Max mint amount per session exceeded");
            }
        _safeMint(msg.sender, _mintAmount);
       
  }

  function mintPublic(uint256 _mintAmount) public payable whenNotPaused
  {
    uint256 supply = totalSupply();
    
    require(_mintAmount > 0, "Need to mint at least 1 NFT");
    if (NFTSettings.publicMintActive == false) { 
            revert NFTErrorCodes.PublicMintNotActive();
		}
    if(supply + _mintAmount > NFTSettings.maxSupply) {
        revert NFTErrorCodes.SoldOut();
    }
  ...
  function unWhitelistAdress(address[] calldata usersToAdd) public onlyOwner
    ...
    ///@dev This function is used to check if the user is whitelisted or not
...
library NFTEvents {
    event NFTAirdropped(address indexed _to, uint256 indexed _quantity);
...
contract NFTSettings is AccessControl, Roles {
    ...
    // If the public mint is active. This needs to be toggled if wanting to mint public
    bool public publicMintActive = false;
    // If the whitelist mint is active. This needs to be toggled if wanting to mint from the whitelist
    ...
    /// @dev Sets the new cost of minting a token
    function setWhitelistMintCost(uint256 newCostInWei)
        public
        onlyRole(Roles.ROLE_SERVER)
    {
        whitelistMintCost = newCostInWei;
    }
    ...
    // *                        SERVER
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // @dev Sets the public mint function to reflect the param passed in.
    // 	this will allow users to pay to mint tokens
    ...
    // @dev Sets the public mint function to reflect the param passed in.
    // 	this will allow users to pay to mint tokens
    function setWhitelistMintActive(bool active)
        public
        onlyRole(Roles.ROLE_SERVER)
    {
