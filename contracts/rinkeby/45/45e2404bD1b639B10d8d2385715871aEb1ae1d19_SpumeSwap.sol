...
import "./SpumeRewardPool.sol"; 
import { ISpumeRewardPool } from "./interfaces/ISpumeRewardPool.sol";
import { ICurrencyManager } from "./interfaces/ICurrencyManager.sol";

contract SpumeSwap is ReentrancyGuard, Ownable, Pausable, AccessControl  {
    //Variables 
    using SafeERC20 for IERC20;
    ICurrencyManager public currencyManager;
    ISpumeRewardPool public spumeRewardPool;
    ISwapRouter public constant swapRouter =
        ISwapRouter(0x1f98407aaB862CdDeF78Ed252D6f557aA5b0f00d);
    bytes32 public constant SWAP_ROLE = keccak256("SWAP_ROLE");
    address public constant WETH = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
    //Events 
    event SwapExecuted(uint256 indexed amount); 
    event Deposited(uint256 indexed amount); 
    event SwapPaused(address account); 
    event SwapUnpasued(address account); 
    
    constructor(address _currencyManager, address _spumeRewardPool) {
    ...
    /*
     * @dev Swap Whitelisted ERC20 for WETH
     */  
     function swapExactInputSingle(address token, uint amountIn) 
     external 
     onlyRole(DEFAULT_ADMIN_ROLE) 
     onlyRole(SWAP_ROLE) whenNotPaused() 
     returns (uint amountOut) 
     {
        require(currencyManager.isCurrencyWhitelisted(token), "Currency: Not whitelisted");
        TransferHelper.safeApprove(address(token), address(swapRouter), amountIn);
     ...
     */  
    function depositToRewardPool(uint256 amount) 
    external 
    whenNotPaused() 
    returns (uint256) 
    {
     ...
     */  
    function pauseSpumeSwap() private onlyOwner whenNotPaused() {
        _pause(); 
    }
     ...
     */
    function unPauseSpumeSwap() private onlyOwner whenPaused() {
        _unpause(); 
    }
     ...
     */  
    function getBalances(address token) internal view onlyOwner whenNotPaused() {
...
interface ISpumeRewardPool {
    ...
    function transferRewardTokensToStakingContract(address stakingContract, uint256 amount) external; 

    function rewardClaimAndSwap(uint256 _rewardTokenAmount) external returns(uint256); 
}
