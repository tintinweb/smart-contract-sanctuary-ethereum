...
contract tesNFTName is Ownable, ERC721A, ReentrancyGuard{
    ...
    constructor() public ERC721A("testNFT", "testNFT") {
    }
   ...
   function freemint(uint256 numberOfTokens) external onlyActive nonReentrant(){
        require(numberOfTokens > 0, "zero count");//保证铸造大于0个
        require(numberOfTokens <= MAX_SUPPLY.sub(totalSupply()), "not enough nfts");//保证有足够的余币可铸
        require(numberOfTokens.add(freeMinted[msg.sender]) <= 1, "already max free minted");//铸币之前，要铸币数量+免费铸币表的数量要小于等于1个
        require(freeMintAmount<=1,"free mint is over");//保证只能免费100个

        freeMinted[msg.sender] = freeMinted[msg.sender].add(numberOfTokens);//更新freeMinted表中该发送者的键所对应的值
        _safeMint(msg.sender, numberOfTokens);//铸币
        freeMintAmount++;//免费铸造数量加一个
        //如果一个地址调用了免费铸币函数，那么首先通过12 require
        //然后如果没有铸币过，免费铸币表就是0，那么1+0小于等于1，满足条件，如果铸币过就是免费铸币表就是1，那么1+1=2大于1，不能再执行
        //没有铸币过的话，先让免费铸币表加上1，然后铸币，最后免费铸币数量+1
        //按理来说没有对公售函数造成影响
    }
