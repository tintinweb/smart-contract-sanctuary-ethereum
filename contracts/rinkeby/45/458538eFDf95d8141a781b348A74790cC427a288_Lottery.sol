...
contract Lottery is Ownable, VRFConsumerBaseV2 {
    ...
    function enter() public payable {
        ...
        require(msg.value >= getEntryFee(), "You gotta send more ETH!");
        players.push(payable(msg.sender));
    }

    function getEntryFee() public view returns (uint256) {
        (, int256 price, , , ) = ethUsdPriceFeed.latestRoundData();
        // We are adjusting the price as the price feed which we are using gives only 8 decimals
        uint256 adjustedPrice = uint256(price) * (10**10);
        uint256 entryFee = (entryFeeInUSD * (10**18)) / adjustedPrice;
        return entryFee;
    }
    ...
    function startLottery() public onlyOwner {
        require(
            lottery_state == LOTTERY_STATE.CLOSED,
            "You can not start a new lottery"
        );
        lottery_state = LOTTERY_STATE.OPEN;
    }
    ...
    function endLottery() public onlyOwner {
        require(
            lottery_state == LOTTERY_STATE.OPEN,
            " You can only end an open lotter"
        );
