...
contract XpaceUpgradeableV3 is ERC721EnumerableUpgradeable, OwnableUpgradeable, PausableUpgradeable, RevealableUpgradeable, WhitelistUpgradeable {
  ...
  uint private _priceDuration;
  uint[] private _mintPrices;

  // refund
  uint private _refundStartAt;
  uint private _refundDuration;
  EnumerableSetUpgradeable.UintSet private _refundMintedTokenIds;
  mapping(uint => uint) private _refundTokenPrice;
  mapping(uint => bool) private _refunded;
  ...
  modifier onlyRefundActive() {
    require(refundActive(), "Refund expired");
    _;
  }

  modifier onlyRefundInactive() {
    require(!refundActive(), "Refund period not over");
    _;
  }
  ...
  function mint(uint quantity) public payable whenNotPaused {
    ...
    require(sellWhitelist() || block.timestamp >= _startAt, "NFT: has not yet started");
    require(quantity > 0, "NFT: quantity cannot be zero");
    require(mintPrice() > 0, "NFT: not set mint price");

    uint amount = mintPrice() * quantity;
    require(amount <= msg.value, "NFT: not enough ether sent");
    ...
    if (refund > 0) AddressUpgradeable.sendValue(payable(_msgSender()), refund);
  }
  ...
  function setMintPrice(uint price) public onlyOwner onlyRefundInactive {
    _mintPrice = price;
    emit MintPrice(price);
  }

  function setRefund(uint startAt, uint duration) public onlyOwner onlyRefundInactive {
    _refundStartAt = startAt;
    _refundDuration = duration;
  }
  ...
  function withdraw() public onlyOwner onlyRefundInactive {
    _resetRefundMinted();

    uint amount = address(this).balance;
    if (amount > 0) AddressUpgradeable.sendValue(payable(_recipient), amount);
  }

  function setPublicAuction(uint startAt, uint priceDuration, uint[] calldata mintPrices) public onlyOwner {
    _startAt = startAt;
    _priceDuration = priceDuration;
    _mintPrices = mintPrices;
  }
  ...
  function getStartAt() public view returns (uint) {
    return _startAt;
  }

  function getPriceDuration() public view returns (uint) {
    return _priceDuration;
  }

  function refundEndTime() public view returns (uint curTime, uint endTime) {
    curTime = block.timestamp;
    endTime = _refundEndTime();
  }
  ...
  function canMintCount(address account) public view returns (uint) {
    ...
    if (sellWhitelist()) {
      ...
      count = balance >= _maxMint ? 0 : _maxMint.sub(balance);
    }
  ...
  function refundToken(uint[] calldata tokenIds) public onlyRefundActive {
    uint refund;

    for (uint i = 0; i < tokenIds.length; i++) {
      uint tokenId = tokenIds[i];
      require(_exists(tokenId), "Nonexistent token");
      require(ownerOf(tokenId) == _msgSender(), "Refund from incorrect owner");
      require(_canRefund(tokenId), "Non-refundable or already refundable");

      _refunded[tokenId] = true;
      refund += _refundTokenPrice[tokenId];
      transferFrom(_msgSender(), _recipient, tokenId);
    }

    AddressUpgradeable.sendValue(payable(_msgSender()), refund);
  }

  function canRefundTokenList(address account) public view returns (uint count, uint[] memory tokenIds, uint[] memory tokenPrices) {
    count = _canRefundTokenCount(account);
    tokenIds = new uint[](count);
    tokenPrices = new uint[](count);
    ...
    for (uint i = 0; i < balanceOf(account); i++) {
      ...
      if (_canRefund(tokenId)) {
        tokenIds[tokenIndex] = tokenId;
        tokenPrices[tokenIndex] = _refundTokenPrice[tokenId];
        tokenIndex += 1;
      }
    }
  }
  ...
  function refundActive() public view returns (bool) {
    ...
    return ts > _refundStartAt && ts < _refundEndTime();
  }

  function _refundEndTime() private view returns (uint) {
    return _refundStartAt + _refundDuration;
  }

  function _canRefund(uint tokenId) private view returns (bool) {
    return _refundMintedTokenIds.contains(tokenId) && !_refunded[tokenId];
  }

  function _canRefundTokenCount(address account) private view returns (uint count) {
    for (uint i = 0; i < balanceOf(account); i++) {
      uint tokenId = tokenOfOwnerByIndex(account, i);
      if (_canRefund(tokenId)) count += 1;
    }
  }
  ...
  function _mint(address to) internal {
    ...
    if (refundActive()) {
      _refundMintedTokenIds.add(tokenId);
      _refundTokenPrice[tokenId] = mintPrice();
    }
  ...
  function _resetRefundMinted() internal {
    for (uint i = 0; i < _refundMintedTokenIds.length(); i++) {
      uint tokenId = _refundMintedTokenIds.at(i);
      _refundMintedTokenIds.remove(tokenId);
    }
  }
}
