...
contract VinciStakingV1 is Ownable, Pausable {
    ...
    // used to store changes in penalty pot during the penalty pot claimable time window:
    // 1. All claims of the penalty pot will be subtractions buffered in this variable.
    // 2. All usual penalties from emerg. unstakes during this time, will also be buffered up as positive in this
    // variable.
    // Only when the time window for claiming penalty pot ends, the buffer cleared into the normal penalty pot.
    //    uint penaltyPotBuffer;
    ...
    event AirdroppedFromPenaltyPot(uint amount, uint nUsers);
    ...
    event SuperstakerGranted(address _user);
    event SuperstakerRemoved(address _user);
    ...
    modifier checkpointUpdated(address _user) {
        if (checkpoint[_user] != 0 && (block.timestamp > checkpoint[_user])) {
    ...
    */
    ...
    function updateUserBalances(address _user) public {
        // only evalyate every hour, to save gas fees to the user
        if (block.timestamp > lastBalancesUpdate[_user]) {
            ...
            if (baseRewards > 0) {// avoid update state variables if there is no rewards
                ...
                if (baseRewards > stakingRewardsFunds) {
                    emit NotEnoughFundsToGiveRewards(baseRewards, stakingRewardsFunds);
                } else {
    ...
    function estimateStakingRewards(address _user) external view returns (uint) {
        return _calculateUserStakingRewards(_user);
    }

    function unclaimableBalance(address _user) public view returns (uint) {
        uint userRewards = _calculateUserStakingRewards(_user);
    ...
    function readTotalActiveStaking(bool _requireSuperstaker) public view returns (uint){
        uint totalStaked = 0;
        for (uint i = 0; i < stakeholders.length; i++) {
    ...
    */
    function airdropFromPenaltyPot(address[] calldata _users, uint[] calldata _amounts) external onlyOwner {
        ...
        require(_totalAmount <= penaltyPot, 'not enough funds in penalty pot');
    ...
    function withdrawCollectedFees(uint _amount, address _to) external onlyOwner {
        require(_amount <= ownersPot, 'amount must be lower than owners pot');
        vinciERC20.transferFrom(address(this), _to, _amount);
    ...
    */
    function setUserPledgeShare(uint _percentage, address _user) external updateBalances(_user) onlyOwner {
    ...
    function _unstake(
        ...
        if (_amount == totalStaked) {
            _finishStakeholder(_user, unstakingDuration);
        } else {
    ...
    */
    ...
    function _calculateUserStakingRewards(address _user) internal view returns (uint) {
        uint baseRewards = _calculateBaseStakingRewards(_user);
    ...
    function _finishStakeholder(address _user, uint _deadline) internal {
    ...
    function _crossCheckpoint(address _user) internal {
        ...
        // evaluate staking balances and finish superstaker if it is 0, with no cancel posibility
        uint userStaking = stakingBalance(_user);

        if (userStaking == 0) {
            _finishStakeholder(_user, 0);
        } else {
