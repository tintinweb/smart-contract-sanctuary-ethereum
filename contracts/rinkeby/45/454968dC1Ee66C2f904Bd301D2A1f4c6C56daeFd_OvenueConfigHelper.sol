...
import {IJuniorRewards} from "./interfaces/IJuniorRewards.sol";
...
import {OvenueJuniorPoolStorage} from "./OvenueJuniorPoolStorage.sol";
import {TranchingLogic} from "./TranchingLogic.sol";
import {OvenueJuniorPoolLogic} from "./OvenueJuniorPoolLogic.sol";

contract OvenueJuniorPool is
    BaseUpgradeablePausable,
    IRequiresUID,
    OvenueJuniorPoolStorage
{
    ...
    function initialize(
        ...
        _initializeNextSlice(_days[3]);

        creditLine = IV2OvenueCreditLine(
    ...
    // {
    //     /// @dev NL: not locked
    //     require(OvenueJuniorPoolLogic.locked(
    //         _poolSlices,
    //         numSlices
    //     ), "NL");
    //     /// @dev LP: late payment
    ...
    // // //     IV2OvenueCreditLine originalCl = creditLine;
    ...
    // // //     if (clBalance > 0) {
    // // //       config.getUSDC().safeTransferFrom(address(originalCl), address(creditLine), clBalance);
    // // //     }
        ...
        returns (uint256, uint256)
    {
        ...
        return
            OvenueJuniorPoolLogic.availableToWithdraw(
                tokenInfo,
                _getTrancheInfo(tokenInfo.tranche)
            );
    }
    ...
    function _lockJuniorCapital(uint256 sliceId) internal {
        ...
        require(
            !OvenueJuniorPoolLogic.locked(_poolSlices, numSlices) &&
    ...
    function _getTrancheInfo(uint256 trancheId)
        internal
        view
        returns (TrancheInfo storage)
    {
        return OvenueJuniorPoolLogic._getTrancheInfo(
            _poolSlices,
            numSlices,
            trancheId
        );
...
interface IOvenueJuniorLP is IERC721 {
  event TokenMinted(
...
import {OvenueConfigOptions} from "./OvenueConfigOptions.sol";
import {IOvenueConfig} from "./interfaces/IOvenueConfig.sol";
...
//   }
  ...
  function getJuniorRewards(IOvenueConfig config) internal view returns (IJuniorRewards) {
...
//   function getTranchedPoolImplementationRepository(OvenueConfig config)
//     internal
//     view
...
//         config.getAddress(uint256(OvenueConfigOptions.Addresses.TranchedPoolImplementationRepository))
//       );
//   }
...
//   function creditLineImplementationAddress(OvenueConfig config) internal view returns (address) {
//     return config.getAddress(uint256(OvenueOvenueConfigOptions.Addresses.CreditLineImplementation));
//   }
...
//   function tranchedPoolAddress(OvenueConfig config) internal view returns (address) {
...
//   function borrowerImplementationAddress(OvenueConfig config) internal view returns (address) {
...
abstract contract OvenueJuniorPoolStorage is IOvenueJuniorPool {
    // event DepositMade(
    //     address indexed owner,
...
library TranchingLogic {
    ...
    function scaleByFraction(
        ...
        return fraction.wadDiv(total).wadMul(amount) / WadRayMath.WAD;
    }
    ...
    function isSeniorTrancheId(uint256 trancheId) external pure returns (bool) {
        ...
        uint numberOfTranchesPerSlice = TranchingLogic.NUM_TRANCHES_PER_SLICE;
...
library OvenueJuniorPoolLogic {
    using TranchingLogic for IOvenueJuniorPool.PoolSlice;
    using TranchingLogic for IOvenueJuniorPool.TrancheInfo;
    using OvenueConfigHelper for IOvenueConfig;
    ...
    function withdraw(
        ...
        // // / @dev NA: not authorized
        // require(config.getJuniorLP().isApprovedOrOwner(msg.sender, tokenId) && hasAllowedUID(msg.sender), "NA");
        // /// @dev IA: invalid amount. Cannot withdraw 0
        // require(amount > 0, "IA");
        (
            uint256 interestRedeemable,
            uint256 principalRedeemable
        ) = TranchingLogic.redeemableInterestAndPrincipal(
                trancheInfo,
                tokenInfo
            );
    ...
    function _calcJuniorRewards(IOvenueConfig config, uint256 numSlices)
        internal
    {
        IJuniorRewards juniorRewards = IJuniorRewards(
            config.juniorRewardsAddress()
        );
        juniorRewards.onTranchedPoolDrawdown(numSlices);
    }
    ...
    {
        return
            TranchingLogic.applyToAllSlices(
                _poolSlices,
                uints[0],
                uints[1],
                uints[2],
                uint256(100) /
                    (IOvenueConfig(addresses[1]).getReserveDenominator()), // Convert the denonminator to percent
                uints[3],
    ...
    function _getTrancheInfo(
        ...
        require(
            trancheId > 0 &&
                trancheId <= numSlices * TranchingLogic.NUM_TRANCHES_PER_SLICE,
            "invalid tranche"
        );
        ...
        IOvenueJuniorPool.TrancheInfo storage trancheInfo = TranchingLogic
...
library Accountant {
  ...
  function calculateWritedownForPrincipal(
    ...
    if (daysLate <= gracePeriodInDays) {
      ...
      writedownPercent = Math.min(WadRayMath.WAD, (daysLate - gracePeriodInDays).wadDiv(maxDaysLate));
    }

    uint256 writedownAmount = writedownPercent.wadMul(principal) / WadRayMath.WAD;
    // This will return a number between 0-100 representing the write down percent with no decimals
    uint256 unscaledWritedownPercent = writedownPercent.wadMul(100) / WadRayMath.WAD;
  ...
  function calculateInterestAccruedOverPeriod(
    ...
    uint256 totalInterestPerYear = balance.wadMul(cl.interestApr()).wadDiv(INTEREST_DECIMALS);
    interestOwed = totalInterestPerYear.wadMul(secondsElapsed).wadDiv(SECONDS_PER_YEAR);
    if (lateFeeApplicable(cl, endTime, lateFeeGracePeriodInDays)) {
      uint256 lateFeeInterestPerYear = balance.wadMul(cl.lateFeeApr()).wadDiv(INTEREST_DECIMALS);
      uint256 additionalLateFeeInterest = lateFeeInterestPerYear.wadMul(secondsElapsed).wadDiv(SECONDS_PER_YEAR);
...
interface IOvenueBackerRewards {
  function allocateRewards(uint256 _interestPaymentAmount) external;
...
// interface IOvenueJuniorPoolLogic {
//     function deposit(
//         IOvenueJuniorPool.TrancheInfo storage trancheInfo,
//         IOvenueConfig config,
//         uint256 amount
...
//  * @author Goldfinch
//  */
// contract SeniorPool is BaseUpgradeablePausable, IOvenueSeniorPool {
