...
contract GameOfDragons is ERC20, Ownable {
    ...
    event FeeSharesUpdated(uint256 liquidityShare, uint256 marketingShare, uint256 devShare);
    ...
    function updateFeeShares(uint256 _marketingFeeShare, uint256 _liquidityFeeShare, uint256 _devShare) external onlyOwner {
        require(_marketingFeeShare + _liquidityFeeShare + _devShare == 100, "Fee shares must add up to 100");
        marketingShare = _marketingFeeShare;
        liquidityShare = _liquidityFeeShare;
        devShare = _devShare;
        emit FeeSharesUpdated(marketingShare, liquidityShare, devShare);
    }
    ...
    function changeDevWallet(address _devWallet) external onlyOwner {
        ...
        emit MarketingWalletChanged(devWallet);
    }
...
////////////            
            ...
            uint256 tokensToSwap  = liquidityTokensForETH + marketingTokens + devTokens;
...
/////////////
            ...
            if(marketingShare > 0) {
                uint256 marketingETH = newBalance * marketingTokens / tokensToSwap;
                sendETH(payable(marketingWallet), marketingETH);

            }

            if(devShare > 0) {
                uint256 devETH = newBalance * devTokens / tokensToSwap;
                sendETH(payable(devWallet), devETH);
