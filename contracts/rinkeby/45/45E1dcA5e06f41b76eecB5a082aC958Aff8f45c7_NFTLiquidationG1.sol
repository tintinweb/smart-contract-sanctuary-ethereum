...
contract NFTLiquidationG1 is NFTLiquidationV1Storage, NFTLiquidationInterface {
    ...
    function liquidateWithSingleRepayFresh(address payable borrower, address cTokenCollateral, address cTokenRepay, uint256 repayAmount) internal {
        ...
        if (cTokenRepay != cEther) {
            ...
            IERC20(underlying).transferFrom(msg.sender, address(this), remained);
    ...
    function liquidateCalculateSeizeTokensEx(address cTokenRepay, uint256 repayAmount, address cTokenCollateral) public view returns(uint256) {
        (, , uint256 possibleRepayAmount) = IComptroller(comptroller).liquidateCalculateSeizeTokensEx(cTokenRepay, cTokenCollateral, repayAmount);
        return possibleRepayAmount;
    }
