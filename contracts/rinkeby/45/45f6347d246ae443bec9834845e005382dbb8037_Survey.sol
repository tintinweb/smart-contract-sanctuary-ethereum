...
contract Survey is Ownable {
    ...
    mapping(uint256 => mapping(address => uint256)) public proposalAddressVote; // Voting Option status of a given address
    ...
    function voteInProposal(uint256 _proposalId, uint256 _voteOption) public {
        ...
        proposalAddressVote[_proposalId][msg.sender] = _voteOption; // Record what the person voted for
    }
    ...
    // // View Functions
    function viewProposalDetails(uint256 _proposalId) public view returns(Proposal memory, string[] memory, uint256[] memory, uint256[] memory, uint256[] memory) {
        uint256 noOfOptions = proposals[_proposalId].numOfOptions;
        string[] memory returnOptionsName;
        uint256[] memory optionsVoteCount;
        for (uint256 i=0; i<noOfOptions; i++) {
            returnOptionsName[i] = proposalOptions[_proposalId][i];
            optionsVoteCount[i] = proposalCount[_proposalId][i];
        }

        address[] memory voterList = proposals[_proposalId].voters;
        uint256[] memory returnVoteChoice;
        uint256[] memory returnVotePower;
        uint256 noOfVoters = voterList.length;
        for (uint256 i=0; i<noOfVoters; i++) {
            // mapping(uint256 => mapping(address => uint256)) public proposalAddressVote; // Voting Option status of a given address
            returnVoteChoice[i] = proposalAddressVote[_proposalId][voterList[i]];
            // mapping(uint256 => mapping(address => uint256)) public proposalPower; // Voting power used by a given address
            returnVotePower[i] = proposalPower[_proposalId][voterList[i]];
        }

        return (proposals[_proposalId], returnOptionsName, optionsVoteCount, returnVoteChoice, returnVotePower);
    }
