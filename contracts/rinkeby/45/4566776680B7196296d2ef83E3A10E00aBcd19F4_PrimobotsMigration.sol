...
contract PrimobotsMigration is Ownable, Pausable, IERC721Receiver {
    ...
    // _tokens array should contain token IDs in ascending order
    function airdrop(address[] memory _receivers, uint256[] memory _tokens)
        external
        onlyOwner
    {
        require(
            _receivers.length == _tokens.length,
            "Arrays length donot match"
        );
        startingId = _tokens[_tokens.length - 1] + 1;
        uint256 i;
        uint256 j;
        if (_tokens[0] % 2 == 0) {
            i = 1;
        ...
        // transferring tokens with odd IDs first
        for (i; i < _receivers.length; i = i + 2) {
        ...
        for (j; j < _receivers.length; j = j + 2) {
            primobots.safeTransferFrom(
                address(this),
                _receivers[j],
                _tokens[j]
