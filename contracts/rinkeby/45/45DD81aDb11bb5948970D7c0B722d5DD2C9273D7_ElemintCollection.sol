  ...
  ERC2981Upgradeable
{
  ...
  function initialize(
    ...
    bytes memory initializeParams = abi.encode(name_, symbol_, marketWallet, baseUriBacktrackLength);
  ...
  function setUp(bytes memory initializeParams) public initializer {
    __Ownable_init();
    (string memory name_, string memory symbol_, address marketWallet, uint16 baseUriBacktrackLength) = abi.decode(
      initializeParams,
      (string, string, address, uint16)
    );

    __ERC721ACM_init(name_, symbol_, marketWallet, baseUriBacktrackLength);
  }
  ...
  IERC721ConsecutiveTransfer
{
  ...
  function tokenOfOwnerByIndex(address owner, uint256 index) public view override returns (uint256) {
    if (owner != _defaultOwner) {
      ...
      unchecked {
        ...
        for (uint256 tokenId = 0; tokenId < _currentIndex; tokenId++) {
   ...
   */
  function tokenOfDefaultOwnerByIndex(uint256 startSearchIndex) public view returns (uint256) {
    if (startSearchIndex >= totalSupply()) revert TokenIndexOutOfBounds();
    unchecked {
      for (uint256 tokenId = startSearchIndex; tokenId < _currentIndex; tokenId++) {
        if ((_ownerships[tokenId].addr == address(0) || _ownerships[tokenId].addr == _defaultOwner)) {
          return tokenId;
        }
      }
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    if (uri.length == 0) {
      ...
      unchecked {
        bytes memory uriTmp;
        for (uint256 i = tokenId - 1; true; i--) {
          ...
          // manual check break because i alway positive
          if (i == 0) {
            break;
          }
        }
  ...
  function _transfer(
    ...
    unchecked {
      if (from == _defaultOwner) {
        ...
        uint64 currIndex = _ownerships[tokenId].indexInOwner;
        uint64 lastTokenOfFrom = _tokensOfOwner[from][_tokensOfOwner[from].length - 1];
        _tokensOfOwner[from][currIndex] = lastTokenOfFrom;
        _ownerships[lastTokenOfFrom].indexInOwner = currIndex;
