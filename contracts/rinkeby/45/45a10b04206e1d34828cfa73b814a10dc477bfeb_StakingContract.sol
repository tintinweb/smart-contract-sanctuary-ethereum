    ...
    function LeaveBattle(uint256 count) public {
       ...
       Battle storage battle = battles[count];
        ...
        if(!battle.joined){
            ...
            require(block.timestamp > (players[msg.sender].battleRecord[players[msg.sender].battleCount].playerstartingTime + 1 minutes),"You haveto wait atleast 3 minutes to leave battle if no one join the battle.");
            ...
            require(block.timestamp > (battle.startingTime + 1 minutes),"You canot leave battle before 3 minutes.");
