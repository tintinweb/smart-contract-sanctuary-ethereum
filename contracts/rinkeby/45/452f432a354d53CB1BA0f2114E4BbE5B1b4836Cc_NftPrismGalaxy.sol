...
contract NftPrismGalaxy is ERC721URIStorage, Ownable {
        ...
        returns (string memory)
    {
        ...
        string memory svg = "<svg width=\"400\" height=\"110\"><rect width=\"300\" height=\"100\" style=\"fill:rgb(0,0,255);stroke-width:3;stroke:rgb(0,0,0)\" /></svg>";

        //string memory image = string(abi.encodePacked(svg[0], svg[1], svg[2]));

        string memory image = string(abi.encodePacked(svg));
        string memory encodedImage = Base64.encode(bytes(image));
