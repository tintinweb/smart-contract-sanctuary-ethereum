...
contract ApolloDAO is Context {
    ...
    event CycleWinnerSelected(address winner, uint256 reward);
    ...
    /// @notice Check if the provided address has been nominated in this cycle. Also returns
    /// the index in the array of nomination data for the cycle.
    /// @param _address The address to check
    function isAlreadyNominated(address _address) public view returns (bool, uint256) {
        VoteData[] memory cycleData = votingData[currentVotingCycleEnd];
        for (uint256 index = 0; index < cycleData.length; index += 1){
            if (_address == cycleData[index].candidate) {
                return (true, index);
            }
        }
        return (false, 0);
    ...
    function vote(address candidate) external {
        ...
        (bool _isNominated, uint256 s) = isAlreadyNominated(candidate);
        ...
        if(_isNominated == true){
            votingData[currentVotingCycleEnd][s].voteCount += 1;
        } else {
            votingData[currentVotingCycleEnd].push(VoteData(candidate, 1, currentVotingCycleEnd)); 
        }
    }

    function completeCycle(address _candidate,uint256 _voteCount) public  onlyOwner{
        ...
        if (minContractBalance < votesToAward) {
            ...
            emit CycleWinnerSelected(leadVoteRecipient.candidate, minContractBalance - daoOwnerTake);
        } else {
            daoOwnerTake = votesToAward * daoOwnerTax / 100;
            _winnings[leadVoteRecipient.candidate] += (votesToAward - daoOwnerTake);
            emit CycleWinnerSelected(leadVoteRecipient.candidate, votesToAward - daoOwnerTake);
        }
    ...
    function isNominated(address nominee) internal view returns (bool) {
