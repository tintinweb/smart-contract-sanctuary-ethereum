...
contract Lottery is SafeTransfer, Ownable, Pausable {
    ...
    event Finalized(uint8 winNumber, address[] winner, uint256 totalAmount);
    ...
    function gameResult() public view returns(uint8 _winNumber, address[] memory _winners) {
        require(gameInfo.isEnded,"Lottery: game is not over");
        _winNumber = gameInfo.winNumber;
    ...
    function pickWinnerAndAwarding() public whenNotEnded onlyOwner {
        uint8 winNumber = uint8(block.timestamp % 100);
        uint256 totalWinner = gameInfo.betNumberToPlayers[winNumber].length();
       ...
       if (totalWinner == 0 ) {
            ...
            for(uint256 i = 0; i < totalWinner; i++) {
                address winner = gameInfo.betNumberToPlayers[winNumber].at(i);
                gameInfo.winners.push(winner);
                safeTokenTransfer(gameInfo.token, payable(winner), rewardAmount);
            }
        }
        gameInfo.winNumber= winNumber;
        gameInfo.isEnded=true;
        emit Finalized(winNumber, gameInfo.winners, totalAmount);
    }
