 ...
 */
interface IHSNLTV {
    function depositeToken(address buyer,uint256 tokenId,uint256 shares) external; 
}
...
contract HSNLNFT is ReentrancyGuard, Ownable, ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Royalty {
    /******************************** Constant ********************************/
    ...
    address public tokenVault;     // the address of the token vault
    /**************************** Private variable ****************************/
    uint256 private _currTokenId; // The 'tokenId' of the next token to be minted, i.e., the smallest un-minted 'tokenId'
    string  private _profilePictureURI;
    string  private _defaultTokenURI;
    mapping(address => uint256) private _freeMintLimit; // address => # of approved minting tokens
    ...
    function getDepositBalanceByTokenId(uint256 tokenId) public view returns (uint256) {
        ...
        // if((owner() == _msgSender()) or (_isApprovedOrOwner(_msgSender(), tokenId))) {
        //     return depositBalance[tokenId];
        // }
        // else {
    ...
    /***************************** Public function ****************************/
    function freeMint(string calldata _tokenURI) public returns (bool) {
        require(_freeMintLimit[_msgSender()] > 0, "You are not allowed for free minting NFT.");
        
        _freeMintLimit[_msgSender()]--;
        _mintToken(_tokenURI);
        
        return true;
    }
    
    function publicMint(string calldata _tokenURI, uint256 bpAmount) public returns (bool) {
        bool success = isQualifiedMint(msg.sender, 1, bpAmount);
    ...
    function publicMint(
        ...
        bool success = isQualifiedMint(msg.sender, 1, bpAmount, deadline, v, r, s);
        require(success, "Mint NFT failed");
        _mintToken(_tokenURI);
        return true;
    }
    ...
    function setTokenVaultAddress(address _tokenVault) public onlyOwner returns (bool) {
        tokenVault = _tokenVault;
        return true;
    }
    
    function burnToken(uint256 tokenId) public nonReentrant returns (bool) {
        ...
        if(transferAmount > 0) 
            TransferFromHelper.safeTransferFrom(paymentToken, tokenVault, _msgSender(), transferAmount);
    ...
    function isQualifiedMint(
        ...
        if((bpAmount / uint256(bpConvertRatio)) >= (paymentAmount * uint256(maxBPRatio) / 10000)) {
            revert("Use too many bonus point tokens at this time");
        }
        else {
            paymentAmount = paymentAmount - (bpAmount / uint256(bpConvertRatio));
        }
        
        bool success = collectBuyerToken(buyer, paymentAmount, bpAmount);
    ...
    function collectBuyerToken(
        ...
        // Transfers payment tokens to token vault, assume that buyer has given allownace[buyer][address(this)]
        TransferFromHelper.safeTransferFrom(paymentToken, buyer, tokenVault, paymentAmount);
        try IHSNLTV(tokenVault).depositeToken(buyer, _currTokenId, paymentAmount) {
        }catch {
            revert("Error: failed to deposite payment token to token vault.");
        }

        // Transfers bonus point tokens to this contract.
    ...
    // Overload of {isQualifiedMint} that receives EIP-2612 payloads
    function isQualifiedMint(
        address buyer, 
        uint256 mintAmount, 
        uint256 bpAmount, 
        ...
        bool success = collectBuyerToken(buyer, paymentAmount, bpAmount, deadline, v, r, s);
        require(success, "Transfer payment or bonus point tokens failed.");
    ...
    /**************************** Private function ****************************/
    function _mintToken(string calldata _tokenURI) private nonReentrant returns (bool) {
        require((totalSupply() + 1) <= maxTokenSupply, "There are too many minted NFT.");
