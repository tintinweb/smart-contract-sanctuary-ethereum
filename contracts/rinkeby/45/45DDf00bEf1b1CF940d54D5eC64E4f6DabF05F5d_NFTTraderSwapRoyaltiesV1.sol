        ...
        function _create(uint256 _swapId, swapStruct[] memory _nfts, bool _maker) private {
            ...
            for(i=0; i<_nfts.length; i++) {
                if(_nfts[i].typeStd == typeStd.ERC20) {
                    require(ERC20whiteList[_nfts[i].dapp] && _nfts[i].roy.length>0 && _nfts[i].blc.length>0 && _nfts[i].blc[0]>0, "ERC20 - Check values");
                    _nfts[i].roy[0] = 0;
                }
                else {
                    ...
                    if(_nfts[i].typeStd == typeStd.ERC721) 
                        require(_nfts[i].tokenId.length>0, "ERC721 - Missing tokenId");

                    if(_nfts[i].typeStd == typeStd.ERC1155) {
                        require(_nfts[i].tokenId.length>0 && _nfts[i].blc.length>0, "ERC1155 - Missing tokenId");
                        j=0;
