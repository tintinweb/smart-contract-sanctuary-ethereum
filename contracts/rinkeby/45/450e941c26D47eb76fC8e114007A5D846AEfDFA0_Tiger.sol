    ...
    ReentrancyGuard
{
    ...
    function claim(uint256[] calldata azukiTokenIds) external {
        ...
        if (
            curTime < uint256(window.startTime) ||
            curTime > uint256(window.endTime)
        ) {
        ...
        for (uint256 i; i < azukiTokenIds.length; ) {
            ...
            // Prevent flash loans
            if (
                block.timestamp - ownershipStart < MIN_OWNERSHIP_TIME_FOR_CLAIM
            ) {
    ...
    function redeemJacketToken(uint256 numTokens) external nonReentrant {
        ...
        if (balanceOf(msg.sender, BLUE) < numTokens)
