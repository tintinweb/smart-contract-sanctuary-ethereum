...
// Pairs on rinkeby: 0x4f781C0EffA536ED102757fF990B933D8433149A
...
contract clearingHouseFlattened {
    ...
    function _checkPair(OrderInfo memory o) internal view returns(address) { // (base, quote)
        ...
        (address baseContract, address quoteContract) = dexUtils.checkQuoteAndBaseContracts(o.makerCoinContract, o.takerCoinContract, _quoteContracts, _baseContracts);
        require(quoteContract != address(0), "checkPair: Invalid quote contract");
        
        // THINK ABOUT any other way using which you can check. Nahi toh yahi theek hai, baaki frontend smmbhaal lega. 

        address _baseContract; // Presumed base contract, that is not in the whitelist. Hence is being checked if it has master contract.
        
        if(o.makerCoinContract == quoteContract) {
            _baseContract = o.takerCoinContract;
        } else {
            _baseContract = o.makerCoinContract;
        }

        // Reverts if _baseContract is somehow 0x0. Otherwise validates the base contract.
        if(baseContract == address(0)) require(address(this) == wrappedCoin(_baseContract).getMasterContract(), "checkPair: Invalid base contract");
    ...
    function _executeTxMakerIsQuote(OrderInfo memory i, uint256 aex_taker_balance, uint256 aex_maker_balance, uint256 aex_maker_fees, uint256 aex_taker_fees, uint256 makerFees, uint256 takerFees) internal {
        ...
        emit TradeExecuted(i.messageHash, i.taker, i.toBeSentByTaker, makerFees, takerFees, aex_maker_fees, aex_taker_fees);
    }
