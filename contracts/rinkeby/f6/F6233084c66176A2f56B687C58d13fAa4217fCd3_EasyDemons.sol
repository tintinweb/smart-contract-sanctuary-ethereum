...
contract EasyDemons is ERC721EnumerableM, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_PRESALE_SUPPLY = 3667;
    ...
    // uint256 public constant MAX_DEVMINT_SUPPLY = 5;
    ...
    function devMintQtyRemaining() external view returns (uint256) {
    ...
    function raffleSaleQtyRemaining() external view returns (uint256) {
    ...
    function presaleMint(uint256 tokenQuantity, uint256 allowance, bytes32[] calldata proof) external payable onlySender nonReentrant {
        ...
        require(presaleQtyMinted + tokenQuantity <= MAX_PRESALE_SUPPLY, "exceed presale alloc");
    ...
    function claimRefund() external nonReentrant() {
        ...
        require(address(this).balance >= claimableAmount(msg.sender), "not enough balance");
        
        _removeAuctionPurchaseHistory(msg.sender);
        Address.sendValue(payable(msg.sender), claimableAmount(msg.sender));
    }
