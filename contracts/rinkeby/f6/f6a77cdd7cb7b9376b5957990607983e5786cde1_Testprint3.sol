...
contract Testprint3 is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721A("Testprint3", "TESTPRNT3"){
    setCost(cost);
...
//Mint NFT
  function mint(uint256 _count)  external payable {
    ...
    if(totalSupply() < maxFreeSupply){
        require(_count > 0 && _count <= maxFreePerTransaction, 'Invalid free mint amount!');
    } else {
        uint256 pay = cost * _count;
        require(msg.value >= pay, 'Free mint done already! Insufficient funds!');
        require(_count > 0 && _count <= maxPerTransaction, 'Invalid mint amount!');
    }
