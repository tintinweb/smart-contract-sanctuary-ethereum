...
contract EthGamble is ChainlinkClient {
    using Chainlink for Chainlink.Request;

    uint256 constant START_TIME = 1;
    uint256 constant END_TIME = 2;
    ...
    uint16 public intervalInMinutes;

    mapping(address => Bet) private bets;
    mapping(uint256 => mapping(bool => uint256)) public groupToTotal;
    mapping(bytes32 => uint256) public requestIdToTimestamp;
    mapping(uint256 => uint256) public timestampToPrice;
    ...
    function getPriceData(uint256 timestamp) external {
        bytes32 requestId = requestPriceData(timestamp);
        requestIdToTimestamp[requestId] = timestamp;
    }

    function requestPriceData(uint256 timestamp)
        internal
        returns (bytes32 requestId)
    {
        ...
        string memory url = string(
            bytes.concat(
                ...
                bytes(Strings.toString(timestamp))
            )
        );
        ...
        recordChainlinkFulfillment(_requestId)
    {
        ...
        timestampToPrice[timestamp] = _price;
    }

    function timestampToGroup(uint256 timestamp)
        internal
        view
        returns (uint256)
    {
        uint256 age = timestamp - timeOfCreation;
        return age / (intervalInMinutes * 1 minutes);
    }

    function groupToTimestamp(uint256 groupIndex)
        internal
        view
        returns (uint256)
    {
        uint256 timestamp = timeOfCreation +
            intervalInMinutes *
            1 minutes *
            groupIndex;
        return timestamp;
    ...
    function placeBet(bool _priceUp) external payable {
        require(
            bets[msg.sender].amount == 0,
            "You have already gambled this round!"
        );
        uint256 amount = msg.value;
        uint256 group = timestampToGroup(block.timestamp);
        bets[msg.sender] = Bet(amount, _priceUp, group);
        groupToTotal[group][_priceUp] += amount;
    }

    function viewBet() external view returns (Bet memory) {
        return bets[msg.sender];
    }

    function viewStartingTime() external view returns (uint256) {
        return groupToTimestamp(bets[msg.sender].group + START_TIME);
    }

    function viewEndingTime() external view returns (uint256) {
        return groupToTimestamp(bets[msg.sender].group + END_TIME);
    }

    function getPayoutAmount(bool _priceUp) internal view returns (uint256) {
        uint256 result;
        uint256 amount = bets[msg.sender].amount;
        bool priceUp = bets[msg.sender].priceUp;
        uint256 group = bets[msg.sender].group;
        if (priceUp == _priceUp) {
            uint256 totalCorrect = groupToTotal[group][priceUp];
            uint256 totalIncorrect = groupToTotal[group][!priceUp];
            uint256 total = totalCorrect + totalIncorrect;
            uint256 share = (total * amount * 995) / (totalCorrect * 1000);
            result = share;
        } else {
            result = 0;
        }
    ...
    function prepareClaim() external {
        uint256 group = bets[msg.sender].group;
        uint256 startingTime = groupToTimestamp(group + START_TIME);
        uint256 endingTime = groupToTimestamp(group + END_TIME);
        require(
            block.timestamp > endingTime,
            "It's too early to prepare the claim!"
        );
        requestPriceData(startingTime);
        requestPriceData(endingTime);
    }

    function claim() external {
        uint256 amount = bets[msg.sender].amount;
        require(amount > 0, "You have not yet placed a bet!");

        uint256 group = bets[msg.sender].group;
        uint256 startingTime = groupToTimestamp(group + START_TIME);
        uint256 endingTime = groupToTimestamp(group + END_TIME);
        require(block.timestamp > endingTime, "It's too early to claim!");

        uint256 startingPrice = timestampToPrice[startingTime];
        uint256 endingPrice = timestampToPrice[endingTime];
        require(
            startingPrice > 0,
            "Price data not correct! Try to call prepareClaim() first!"
        );
        require(
            endingPrice > 0,
            "Price data not correct! Try to call prepareClaim() first!"
        ...
        bool priceUp = endingPrice > startingPrice;

        require(
            bets[msg.sender].priceUp == priceUp,
            "Sadly you did not predict correcly. Better luck next time!"
        );

        uint256 payoutAmount = getPayoutAmount(priceUp);

        if (!payable(msg.sender).send(payoutAmount)) {
            bets[msg.sender].amount = amount;
        }
    }
}
