 ...
 * @title Cryptoliga
 * @author Peter Smith
 * @dev Cryptoliga is a smart contract that allows users to purchase tokens in any ERC20 coin.
 **/
...
contract Cryptoliga is ERC2981, ERC721, Pausable, AccessControl {
    ...
    mapping(address => uint[]) userMints;

    string[] private coins;

    error CoinAlreadyExists(string _symbol);
    error CoinDoesNotExist();
    error CoinNotActive();

    error WrongStage(Stages required, Stages current);

    error InsufficientBalance(uint required, uint available);
    error InsufficientAllowance(uint required, uint available);
    error OverMintQuota(uint required, uint available, uint used);

    error NotEnoughTokensRemaining();
    error AttemptToMintOverMaxBatchSize(uint requested, uint max);
    ...
    modifier onlyStage(Stages _stage) {
        if (saleStage != _stage) {
            revert WrongStage(_stage, saleStage);
        }
        _;
    }

    modifier atLeastStage(Stages _stage) {
        if (saleStage < _stage) {
            revert WrongStage(_stage, saleStage);
        }
        _;
    ...
    modifier okToMint(address _to, uint256 _numberToMint) {
        if (maxBatchSize > 0 && _numberToMint > maxBatchSize) {
            revert AttemptToMintOverMaxBatchSize(_numberToMint, maxBatchSize);
        }

        if (totalSupply() + _numberToMint > maxSupply) {
            revert NotEnoughTokensRemaining();
        }
        ...
        if (
            ...
            revert OverMintQuota(
                _numberToMint,
                wlAllowances[uint(saleStage)],
                userMints[_to][uint(saleStage)]
            );
        }
    ...
    function addCoin(
        ...
        if (coinActive[_symbol]) {
            revert CoinAlreadyExists(_symbol);
        }
    ...
    function getWlAllowance(uint _stage) external view returns (uint256) {
        return wlAllowances[_stage];
    }

    /***
     * @dev Returns the number of tokens remainnig in the current stage.
     * @param _to address The address to check
     */
    function wlAllowancesLeft(address _to) external view returns (uint256) {
        if (userMints[_to].length == 0) {
            return wlAllowances[uint(saleStage)];
        }
        return wlAllowances[uint(saleStage)] - userMints[_to][uint(saleStage)];
    }

    function setCoinPrice(string memory _coinSymbol, uint256 _price)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function getCurrentPrice() external view returns (uint256) {
        if (saleStage == Stages.CLOSED) {
            return 0;
        }
        return saleStage == Stages.PUBLIC ? mintPrice : presalePrice;
    }
    ...
    function removeCoin(string memory _coinSymbol)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        if (!coinActive[_coinSymbol]) {
            revert CoinNotActive();
        }
    ...
    function setStage(uint _stage) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (Stages(_stage) == saleStage) {
            revert WrongStage(Stages(_stage), saleStage);
        }
    ...
    function fxMint(
        ...
        external
        whenNotPaused
        onlyStage(Stages.PUBLIC)
        okToMint(to, numberToMint)
    {
        address _address = coinAddress[_coinSymbol];
        uint256 _price = coinPrice[_coinSymbol];
        uint256 _totalPrice;

        if (_price == 0) {
            revert CoinDoesNotExist();
        }

        IERC20 paymentToken = IERC20(_address);

        _totalPrice = numberToMint * _price;

        if (paymentToken.balanceOf(msg.sender) < _totalPrice) {
        ...
        if (paymentToken.allowance(msg.sender, address(this)) < _totalPrice) {
        ...
        okToMint(to, numberToMint)
    {
        ...
        uint256 _totalPrice = numberToMint * presalePrice;

        if (msg.value < _totalPrice) {
            revert InsufficientBalance(_totalPrice, msg.value);
        }

        _safeMint(to, numberToMint);
    }

    function nativeMint(address to, uint64 numberToMint)
        external
        payable
        whenNotPaused
        onlyStage(Stages.PUBLIC)
        okToMint(to, numberToMint)
    {
        uint256 _totalPrice = numberToMint * mintPrice;
    ...
    function withdrawAll() external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint i = 0; i < coins.length; i++) {
            ...
            address _address = coinAddress[_token];
        ...
        returns (bool)
    {
        ...
        if (!MerkleProof.verify(_merkleProof, root, leaf)) {
            revert IncorrectProof(_merkleProof, root, leaf);
        }

        return true;
    }

    function safeMint(address to, uint256 quantity) external onlyRole(MINTER_ROLE) {
