    ...
    RoyaltiesV2Impl
{
    ...
    function _singleMint(address _account, uint96 _feeBps) internal onlyOwner {
        uint256 tokenId = totalSupply();
        super._mint(_account, tokenId);
        LibPart.Part[] memory royalties = new LibPart.Part[](1);
        royalties[0].account = payable(_account);
        royalties[0].value = _feeBps;
        super._saveRoyalties(tokenId, royalties);
    }

    function mint(address _account, uint96 _feeBps) external onlyOwner {
        _singleMint(_account, _feeBps);
    }
