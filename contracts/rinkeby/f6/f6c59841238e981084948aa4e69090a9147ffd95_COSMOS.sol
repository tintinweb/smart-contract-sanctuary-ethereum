...
contract COSMOS is ERC721Enum, Ownable, ReentrancyGuard {
    using Strings for uint256;
	
    uint256 public MAX_NFT = 9999;
	uint256 public MAX_MINT_PRESALE = 2;
	uint256 public MAX_MINT_RAFFLES = 9999;
	
	uint256 public MAX_BY_MINT_IN_TRANSACTION_PRESALE = 2;
	uint256 public MAX_BY_MINT_IN_TRANSACTION_RAFFLES = 9999;
	
	uint256 public PRESALE_MINTED;
	uint256 public RAFFLES_MINTED;
	uint256 public GIVEAWAY_MINTED;
	
	uint256 public PRESALE_PRICE = 12 * 10**16;
	uint256 public RAFFLES_PRICE = 12 * 10**16;
	
    bool public presaleEnable = false;
	bool public rafflesEnable = false;
	bool public rafflesMerkleRootStatus = true;
	
    string private baseURI;
	bytes32 public merkleRootPreSale;
	bytes32 public merkleRootRaffles;
    ...
    constructor() ERC721P('Cosmos Kidz NFT', 'COSMOS') {}
	...
	function mintRafflesNFT(uint256 _count, bytes32[] calldata merkleProof) public payable{
		uint256 totalSupply = totalSupply();
		bytes32 node = keccak256(abi.encodePacked(msg.sender));
		require(
			rafflesEnable, 
			"Raffles is not enable"
		);
        require(
			totalSupply + _count <= MAX_NFT, 
			"Exceeds max limit"
		);
		if(rafflesMerkleRootStatus)
		{
		   require(
				MerkleProof.verify(merkleProof, merkleRootRaffles, node), 
				"MerkleDistributor: Invalid proof."
		   );
		}
		require(
			users[msg.sender].rafflesmint + _count <= MAX_MINT_RAFFLES,
			"Exceeds max mint limit per wallet"
		);
		require(
			_count <= MAX_BY_MINT_IN_TRANSACTION_RAFFLES,
			"Exceeds max mint limit per tnx"
		);
		require(
			msg.value >= RAFFLES_PRICE * _count,
			"Value below price"
		);
		for (uint256 i = 0; i < _count; i++) {
            _safeMint(msg.sender, totalSupply + i);
		...
		users[msg.sender].rafflesmint = users[msg.sender].rafflesmint + _count;
    }
	...
	function updateRafflesPrice(uint256 newPrice) external onlyOwner {
        RAFFLES_PRICE = newPrice;
    }
	...
	function setRafflesStatus(bool status) public onlyOwner {
        require(rafflesEnable != status);
		rafflesEnable = status;
    }
	
	function setRafflesMerkleRootStatus(bool status) public onlyOwner {
        require(rafflesMerkleRootStatus != status);
		rafflesMerkleRootStatus = status;
    }
	...
	function updateRafflesMintLimit(uint256 newLimit) external onlyOwner {
	    require(MAX_NFT >= newLimit, "Incorrect value");
        MAX_MINT_RAFFLES = newLimit;
    }
	...
	function updateMintLimitPerTransectionRaffles(uint256 newLimit) external onlyOwner {
	    require(MAX_NFT >= newLimit, "Incorrect value");
        MAX_BY_MINT_IN_TRANSACTION_RAFFLES = newLimit;
    }
	
	function updateRafflesMerkleRoot(bytes32 newRoot) external onlyOwner {
	   merkleRootRaffles = newRoot;
	}
