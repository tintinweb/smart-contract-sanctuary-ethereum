...
contract Campaign {
	...
	function donate(uint128 _amount, uint128 _idr) external isApproved returns (bool){
		...
		string memory invoiceId = string(abi.encodePacked(prefix, uint2str(uint64(invoices.length) + 1))); 
		
		invoices.push(invoiceId);
		uint64 donaturIndex = setDonatur(msg.sender);
		invoiceToDetail[invoiceId] = Detail(uint64(invoices.length) - 1, donaturIndex, uint128(block.timestamp), _idr, _amount);
