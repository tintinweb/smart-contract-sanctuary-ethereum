...
/**
* @notice PunksVoxels ERC721 wrapper contract
...
contract ApeVoxels is ERC721, ReentrancyGuard {
    ...
    function mint(uint id, address user) external {
...
contract PunksVoxels is ERC721, ReentrancyGuard, IERC721Receiver, IERC1155Receiver {
    address private owner;
    ...
    function wrapMany(uint[] calldata tokenIds) nonReentrant external {
        ...
        unchecked {
            ...
            for(uint i = 0; i < count; i++) {
                if(_exists(toPunkId(tokenIds[i])) == false) {
                    uint newId = toPunkId(tokenIds[i]);
                    // Mint the punk ape
                    punkApes.mint(newId, msg.sender);
                    // Mint the PunkVoxel
    ...
    {
        return string(abi.encodePacked(baseURIForOGPunksVoxels, Strings.toString(tokenId), baseExt));
    }
