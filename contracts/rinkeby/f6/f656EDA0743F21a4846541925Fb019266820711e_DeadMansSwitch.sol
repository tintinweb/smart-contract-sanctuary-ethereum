...
contract DeadMansSwitch {
    ...
    struct Switch {
        ...
        uint256 downCadence;
        // next block where heartbeat is possible
        uint256 startWindowBlock;
        // last block where heartbeat is possible
        uint256 endWindowBlock;
    }
    ...
    event SwitchMinted(uint256 switchId, uint256 startWindowBlock, uint256 endWindowBlock);
    // transmit switchId and new blockwindows after hearbeat
    event HearBeatReceived(uint256 switchId, uint256 cadence, uint256 downCadence);
    ...
    function mint(
        ...
        emit SwitchMinted(switchId, _cadence, _downCadence);
        return switchId;
    }
    ...
    function isAlive(uint256 _switchId) public view returns (bool) {
        bool alive = true;
        uint256 blockNo = block.number;
        if (blockNo > switches[_switchId].endWindowBlock) alive = false;
        return alive;
    }
