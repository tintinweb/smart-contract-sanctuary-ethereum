...
contract Payable is Ownable {
    ...
    address private constant ADDR1 = 0x0000000000000000000000000000000000000001;
    address private constant ADDR2 = 0x0000000000000000000000000000000000000002;
    address private constant ADDR3 = 0x0000000000000000000000000000000000000003;
    address private constant ADDR4 = 0x0000000000000000000000000000000000000004;
    address private constant ADDR5 = 0x0000000000000000000000000000000000000005;
  ...
  function withdraw() external onlyOwner {
    ...
    payable(ADDR1).transfer(currentBalance * 42 / 100); // 42%
    payable(ADDR2).transfer(currentBalance * 23 / 100); // 23%
    payable(ADDR3).transfer(currentBalance * 18 / 100); // 18%
    payable(ADDR4).transfer(currentBalance * 9 / 100); // 9%
    payable(ADDR5).transfer(currentBalance * 8 / 100); // 8%
  }
}
// File: RebelMuzik.sol
...
contract RebelMuzik is Ownable, ERC721A {
  ...
  uint256 private privateQty = 1;
  uint256 private VIPQty = 2;
  uint256 private publicQty = 10;

  // === Reserved Mint Amounts ===
  uint256 private amountForPrivate = 100;
  uint256 private amountForVIPList = 800;

// === To Check Private and VIP total Supplies ===
  uint256 public reservedPrivateMinted = 0;
  uint256 public reservedVIPMinted = 0;
  ...
  function mintPrivateSale(bytes32[] calldata merkleProof) external payable
    {
      ...
      require(
        ...
        require(
      reservedPrivateMinted + privateQty <= amountForPrivate, 
      "Reached Private Sale Max Supply."
      );
      _safeMint(msg.sender, privateQty);
      privateListMintTracker[msg.sender] = true; 
      reservedPrivateMinted = reservedPrivateMinted + privateQty; 
  }
  ...
  */
  ...
  function mintVIPList(bytes32[] calldata merkleProof, uint256 quantity) external payable 
  {
    ...
    reservedVIPMinted = reservedVIPMinted + quantity;
  }
  // ===============================================================================
 /*
  ...
  function mintPublicSale(uint256 quantity) external payable 
    ...
    function securePublicMint(uint256 quantity) internal {
      ...
      function secureVIPMint(uint256 quantity) internal {
        ...
        require(
          reservedVIPMinted + quantity <= amountForVIPList, 
