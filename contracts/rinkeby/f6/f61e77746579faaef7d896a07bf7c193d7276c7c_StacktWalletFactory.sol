...
contract StacktWalletFactory {
    ...
    constructor(address _proxyFactoryAddress, address _stacktWalletSingletonAddress, address _fallbackAddress) {
    ...
    function createWallet(Setup calldata setup) public payable returns (GnosisSafeProxy proxy) {

        // controller guard
        address controllerGuardAddr = address(0);
        if (setup.controllers.length > 0) {
            StacktControllerGuard controllerGuard = new StacktControllerGuard(address(0), setup.controllers);
        ...
        // stackt wallet
        StacktWalletProxyFactory proxyFactory = StacktWalletProxyFactory(proxyFactoryAddress);
    ...
    StacktGuardManager
{
    ...
    event RefundCalculation(uint256 indexed gas, uint256 gasprice); ///
    ...
    function execTransaction(
        ...
        /* -------------------------------- execution ------------------------------- */
        {
            ...
            if (refundGas) {
                ...
                emit RefundCalculation(gasUsed + BASE_TX_GAS_COSTS + PAYMENT_GAS_COSTS + msg.data.length * DATA_BYTE_GAS_COSTS, tx.gasprice);
