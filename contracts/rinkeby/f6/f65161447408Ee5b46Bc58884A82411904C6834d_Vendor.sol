...
contract Vendor is Ownable {
    ...
    function sellTokens(uint256 _amount) public payable {
        require(_amount > 0, "need tokens to sell");
        uint256 allowance = yourToken.allowance(msg.sender, address(this));
        require(allowance >= _amount, "you dont have enough tokens");
        ...
        if (!success) {
            revert Vendor__BuybackFailed();
        } else {
            emit SellTokens(msg.sender, ethToSend, _amount);
        }
    }
}
