...
contract WorkspaceV2 is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable, WorkspaceStorageV1{
    ...
    event CheckedOut(address indexed _who, uint256 _spaceId, uint256 _returnAmount);
    ...
    function checkIn(uint256 _spaceID) external returns(bool){
        Space storage space = spaces[_spaceID]; 
        userInfo storage user = users[msg.sender];
        require(space.available);
        require(user.spaceID == 0);
        require(user.hold == false,"Plz cancel your hold");
        // we need to send tokens worth 8 hrs to owner of space
        //require(msg.value == ((space.hourlyRate)*8), "Please send accurate Token");
        require(balanceOf(msg.sender) >= ((space.hourlyRate)*8),"Insufficient Tokens" );
        transfer(space.owner, (space.hourlyRate)*8);
        user.spaceID = _spaceID;
        user.entryTime = block.timestamp;
        user.depositToken = (space.hourlyRate)*8;//change name of depositToken
        user.numGuest = 0;
    ...
    function updateSpace(address sender, uint256 _spaceID) internal {
        Space storage space = spaces[_spaceID]; 
        userInfo storage user = users[sender];
        space.numActiveCheckIns = space.numActiveCheckIns-(user.numGuest + 1);
        uint index = space.userIndex[sender];
    ...
    function checkOut(uint256 _spaceID,address _user) external returns(bool){
        userInfo storage user = users[msg.sender];
        Space storage space = spaces[_spaceID];
        //require(user.spaceID == space.spaceID);
        require(space.owner == msg.sender);
        uint256 spaceUsedTime = spaceUsedTimeCalculator(user.entryTime);
        if(spaceUsedTime > (28800)){
            spaceUsedTime = 28800;
        }
        ...
        spaceUsedTime = spaceUsedTime *((space.hourlyRate)/3600);
        uint256 returnValue = (user.depositToken - spaceUsedTime);
        updateSpace(_user, _spaceID);
        transfer(_user, returnValue);
        emit CheckedOut(msg.sender , _spaceID, returnValue); 
        return true;
    }
