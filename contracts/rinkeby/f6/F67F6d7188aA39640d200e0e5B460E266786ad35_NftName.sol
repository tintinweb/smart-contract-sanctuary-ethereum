...
contract NftName is Ownable, ERC721, SutterTreasury {
    ...
    uint256 public constant NftSymbolSupply = 10000;
    uint256 public constant mintAmountLimit = 8;
    ...
    address[] private mintPayees = [
        0xF9935392ED7F815b3323Cd254E72A20154e3D6A4, // Angel Investor
        0x2AC1F229d922a9427f03bEf69261729e7c3c5a25 // Dev
    ];
    uint256[] private mintShares = [67, 33];
        ...
        SutterTreasury(mintPayees, mintShares)
    {
        ...
        presaleConfig = PresaleConfig({
            startTime: 1644516000, // Thu Feb 10 2022 18:00:00 GMT+0000
            endTime: 1644602400, // Fri Feb 11 2022 18:00:00 GMT+0000
            price: 0.088 ether
        });
        ...
        publicConfig = PublicConfig({
            ...
            startTime: 1644516000, // Thu Feb 10 2022 18:00:00 GMT+0000
            dutchEndTime: 1644602400, // Fri Feb 11 2022 18:00:00 GMT+0000
            stepInterval: 600, // 10 min
            price: 0.1 ether,
            dutchStartPrice: 1.5 ether,
    ...
    function buyPresale(uint256 mintAmount) external payable {
        ...
        require(
            ownerTokenCount < mintAmountLimit,
            "You have reached the limit"
        );
        require(mintAmount > 0, "Not enough Mint Amount");
        require(mintAmount <= mintAmountLimit, "Mint Amount is too high");
        require(
    ...
    function buyPublic(uint256 mintAmount) external payable {
        ...
        require(
            (totalSupply + mintAmount) <= NftSymbolSupply,
            "Not enough NftSymbol remaining"
        );
        require(
            ownerTokenCount < mintAmountLimit,
            "You have reached the limit"
        ...
        returns (uint256 currentPrice)
    {
        ...
        } else if (timestamp >= _config.dutchEndTime) {
            currentPrice = _config.dutchEndPrice;
        } else {
            uint256 elapsedIntervals = (timestamp - _config.startTime) /
                _config.stepInterval;
            currentPrice =
                _config.dutchStartPrice -
    ...
    function setDutchAuctionConfig(
        ...
        uint32 _startTime = startTime.toUint32();
        uint32 _dutchEndTime = dutchEndTime.toUint32();
        uint32 _stepInterval = stepInterval.toUint32();

        require(0 < _startTime, "Invalid time");
        require(_startTime < _dutchEndTime, "Invalid time");
        require(0 < stepInterval, "0 step interval");
        require(dutchEndPrice < dutchStartPrice, "Invalid dutch start price");
        require(0 < dutchEndPrice, "Invalid dutch bottom price");
        require(
    ...
    function addWhitelist(address[] calldata addAddress) external onlyOwner {
        for (uint256 i = 0; i < addAddress.length; i++) {
            whitelisted[addAddress[i]] = true;
        }
    }

    function delWhitelist(address[] calldata removeAddress) external onlyOwner {
        for (uint256 i = 0; i < removeAddress.length; i++) {
            whitelisted[removeAddress[i]] = false;
        }
    }
}
