...
contract ELM is ERC721A, Ownable {
  ...
  constructor() ERC721A("ELM Unofficial NFT", "ELM") {
    //   _baseURIextended = "ipfs://QmNMAJRnfJuXEaXqraiUPr4r8q83c63dznBzNkvK5M2GLk/";
  }
  ...
  function mint(uint256 nMints) external payable {
    ...
    if (nMints>1){
        for(uint i=nMints;i>0;i--){
            timestampID[totalSupply()-i] = block.timestamp;
        }
    }else
    {
  ...
  function setMaxmint(uint256 mMint) external onlyOwner {
      MAX_MINTS = mMint;
  }

  function setMaxsupply(uint256 mSupply) external onlyOwner {
      MAX_SUPPLY = mSupply;
  }
  ...
  function refund(uint256 tokenId) public {
      require(tx.origin == msg.sender, "Can't call through contract");
      require(block.timestamp - timestampID[tokenId] < 7 days,"You can't request for refund after 7 days or you don't own that token.");
      transferFrom(msg.sender,0xD406193d51585b2B825c2A7007b71406605248d4,tokenId);
      payable(msg.sender).transfer(0.0066 ether);
  }
