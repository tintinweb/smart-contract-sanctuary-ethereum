...
error NoClaimableKeys();
error NoLegacyKeysToTransfer();
    ...
    IERC1155Receiver
{
    ...
    function claimKeys() external {
        if (!claimActive) revert ClaimingNotActive();
        if (!OSSF.isApprovedForAll(msg.sender, address(this))) revert ApprovalRequired();

        uint256 claimable = OSSF.balanceOf(msg.sender, OSMVHQ_TOKENID);
        if (claimable == 0) revert NoClaimableKeys();
    ...
    */
    ...
    function transferLegacyKeys(address to_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (balance == 0) revert NoLegacyKeysToTransfer();
    ...
    function _beforeTokenTransfers(
        ...
        if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
            ...
            for (uint256 i = startTokenId; i < startTokenId + quantity; i++) {
                if (isKeyFlagged(i)) revert KeyIsFlagged();
            }
        }
