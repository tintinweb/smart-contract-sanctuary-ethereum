...
contract Paradigm is Ownable, ERC721A, ERC2981, PaymentSplitter {
    ...
    uint256 constant public WHITELIST_PRICE = 0.25 ether;
    uint256 constant public PUBLIC_PRICE = 0.35 ether;
    uint256 constant public PUBLIC_SPOTS = 2980;
    uint256 constant public MAX_WORLDS = 5000;
    uint256 constant public MAX_GENESIS = 700;
    ...
    mapping(address => bool) public freeGenesisMinted;
    ...
    constructor(bytes32 root, address[] memory _devs, uint256[] memory _shares) ERC721A("Forgotten Ethereal Worlds Paradigm", "PRDM") PaymentSplitter(_devs, _shares) {
        _merkleRoot = root;
    ...
    // for this version _mints includes the _freeMints you wish to mint
    function genesisMint(uint256 _proofMints, uint256 _freeMints, uint256 _mints, bytes32[] calldata _proof) external payable {
        ...
        if(genesisMints[msg.sender] < _freeMints) {
            require(msg.value >= (_totalMints - _freeMints) * WHITELIST_PRICE, "Paradigm: Insufficient funds");
        }
        else {
    ...
    // For this version _mints does not include your freeMints
    // function genesisMint(uint256 _proofMints, uint256 _freeMints, uint256 _mints, bytes32[] calldata _proof) external payable {
    //     require(saleStatus == SaleStatus.Genesis, "Paradigm: Genesis mint isn't active");
    ...
    //     if(freeGenesisMinted[msg.sender]) {
    //         genesisMints[msg.sender] += _mints;
    //         _mint(msg.sender, _mints);
    //     }
    //     else {
    //         freeGenesisMinted[msg.sender] = true;
    //         genesisMints[msg.sender] += _mints;
    //         _mint(msg.sender, _mints + _freeMints);
    //     }
    // }
