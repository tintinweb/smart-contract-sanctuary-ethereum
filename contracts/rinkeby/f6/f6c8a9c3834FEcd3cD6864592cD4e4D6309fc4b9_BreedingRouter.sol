...
contract BreedingRouter is Ownable, Pausable, ERC721Holder {
    ...
    function rent(uint256 tokenId, uint256 durationInDay) external {
        ...
        LeaseMarket storage leaseMarket = tokenIdLeaseMarket[tokenId];
        require(
            ERC20(leaseMarket.paymentToken).balanceOf(msg.sender) >= leaseMarket.price.mul(durationInDay),
            "Not enougn token for rent"
        );
        require(
            ERC20(pillContractAddress).allowance(msg.sender, address(this)) >= leaseMarket.price.mul(durationInDay),
            "Please approval for contract can use Pill token"
        );
        require(leaseMarket.renter == address(0), "Bunny not available for rent");

        ERC20(leaseMarket.paymentToken).safeTransferFrom(
            msg.sender,
            address(this),
            leaseMarket.price.mul(durationInDay)
        );
    ...
    function rentExtension(uint256 tokenId, uint256 durationInDay) external {
        ...
        ERC20(leaseMarket.paymentToken).safeTransferFrom(
            address(this),
            rentFeeRecient,
            leaseMarket.price.mul(durationInDay).mul(rentFee) / 10000
        );
        // transfer rent fee for owner;
        ERC20(leaseMarket.paymentToken).safeTransferFrom(
            address(this),
            leaseMarket.owner,
            leaseMarket.price.mul(durationInDay).sub(leaseMarket.price.mul(durationInDay).mul(rentFee) / 10000)
        );
    ...
    function _breedingWithRentedBunny(
        ...
        if (useNyanKee) {
            require(
                ERC20(pillContractAddress).allowance(msg.sender, address(this)) >= nyanKeeCost,
                "Please approval for contract can take Pill token"
            );
            require(ERC20(pillContractAddress).balanceOf(msg.sender) >= nyanKeeCost, "Not enough token for NyanKee");
            ERC20(pillContractAddress).safeTransferFrom(msg.sender, rentFeeRecient, nyanKeeCost);
        }
        ...
        require(
            ERC20(pillContractAddress).allowance(msg.sender, address(this)) >= pillCost,
            "Please approval for contract can take Pill token"
        );

        require(ERC20(pillContractAddress).balanceOf(msg.sender) >= pillCost, "not enough pill");
        ERC20(pillContractAddress).safeTransferFrom(msg.sender, rentFeeRecient, pillCost);
