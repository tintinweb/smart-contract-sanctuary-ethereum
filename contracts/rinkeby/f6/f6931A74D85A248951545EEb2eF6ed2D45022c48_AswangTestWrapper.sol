...
contract AswangTestWrapper is Aswang {
    constructor(
        address signer,
    ...
    function _fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) public {
        ERC721ABase.fulfillRandomWords(requestId, randomWords);
    }
}
...
error Aswang_InsufficientReserveSupply();
error Aswang_PrayingDisabled();
error Aswang_SenderNotTokenOwner();
error Aswang_TokenPraying();
 ...
 */
contract Aswang is ERC721ABase {
    uint256 public constant MAX_RESERVED = 100;

    bool public prayingDisabled = true;
    uint256 public reserved;
    mapping(uint256 => uint256) public prayingStartedAt;
    ...
    /**
     * @notice Set the !praying status of a specified set of tokens
     * @param tokenIds Tokens for which to set the !praying status
     * @param pray Flag indicating if token should start or stop !praying
     */
    function setPraying(uint256[] calldata tokenIds, bool pray) external {
        if (pray && prayingDisabled) revert Aswang_PrayingDisabled();

        uint256 timestamp = pray ? block.timestamp : 0;
        uint256 tokensLength = tokenIds.length;

        for (uint256 i; i < tokensLength;) {
            if (ownerOf(tokenIds[i]) != _msgSender()) revert Aswang_SenderNotTokenOwner();
            if (prayingStartedAt[tokenIds[i]] != 0) revert Aswang_TokenPraying();

            prayingStartedAt[tokenIds[i]] = timestamp;
     ...
     * @param receiver Receiver of reserved tokens
     */
    function reserve(uint256 amount, address receiver) external onlyOwner {
        ...
        if (reserved + amount > MAX_RESERVED) revert Aswang_InsufficientReserveSupply();
    ...
    function transferFrom(address from, address to, uint256 tokenId)
        ...
        override(ERC721A)
    {
        if (prayingStartedAt[tokenId] != 0) revert Aswang_TokenPraying();
...
contract ERC721ABase is ERC721AQueryable, Ownable, VRFConsumerBaseV2 {
    ...
    function mint(uint256 amount, bytes memory signature) public virtual payable {
        ...
        if (addressMinted[_msgSender()] + amount > mintConfig.maxPerAddress)
