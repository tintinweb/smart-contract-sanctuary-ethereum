...
contract MockNft is CreateNFT {
    constructor(string memory _initNotRevealedUri, string memory _name,
        string memory _symbol,
        string memory _initBaseURI, string memory _initContractURI, uint256[] memory _initWlCosts, uint256[] memory _initWlPlaces, uint256 _initMaxMintAmount, uint256 _initPublicMintPrice) public CreateNFT(_initNotRevealedUri, _name,
        _symbol,
        _initBaseURI, _initContractURI, _initWlCosts, _initWlPlaces, _initMaxMintAmount, _initPublicMintPrice) {

    }

    function _mock_setTokenId(uint256 value) public {
        nftCounter = value;
    }
...
contract CreateNFT is ERC721Enumerable, Ownable {
    ...
    //TODO:
    function mintLegendary(uint256[] memory tokenIds) public onlyOwner
    {
    ...
    function checkForPause(uint256 counter) private
    {
        if ((counter == wlPlaces[0]) || (counter == wlPlaces[1]))
        {
            pause(true);
        }
    }
