...
contract HDRegistry is Ownable {
    ...
    mapping(address => HDNFT[]) public HDNFTsByOwner;
    ...
    function getNFTByOwner(address owner, uint256 id)
        external
        view
        returns (HDNFT memory)
    {
        return HDNFTsByOwner[owner][id];
    }
    ...
    function addNFT(
        ...
        HDNFTsByOwner[owner].push(
            HDNFT(owner, contractAddr, hashedData, tokenId, rights, status)
        );
        emit registryAdd(
            owner,
            contractAddr,
    ...
    function editRights(
        ...
        HDNFTsByOwner[owner][id].rights = rights;
    }

    function editHashedData(
        address owner,
        uint256 id,
        string memory hashedData
    ) external onlyOwner {
        HDNFTsByOwner[owner][id].hashedData = hashedData;
    }
    ...
    function editStatus(
        ...
        HDNFTsByOwner[owner][id].status = status;
    }
}
