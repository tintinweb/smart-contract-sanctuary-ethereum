...
contract AiamondTestNft4 is ERC1155, IERC2981, Ownable, ContextMixin, NativeMetaTransaction, ReentrancyGuard {    
    using SafeMath for uint256;
    ...
    string internal _uriBase; // will be with slash
    string public url = "ipfs://QmSATBkgL3asJC64nM1UNmrSmHBUULHR7geD8WdFURcFva"; // without slash
    ...
    constructor(
        ...
        _uriBase = string(abi.encodePacked(url, "/"));
    ...
    function buy(uint256 id, uint256 amount)
        ...
        nonReentrant
    {
        require(!_exists(id) || (tokenSupplies[id]+amount) < maxSupplies[id], "You cannot buy and mint existing tokens if the supply is empty");
        ...
        returns (uint256)
    {
        if(id < 5) { // Founder
            return 1000 ether;
        } else if(id > 4 && id < 15) { // Gold
            return 50 ether;
        } else if(id > 14 && id < 5000) { // Silver
            return 1 ether;
        } else if(id == 5000) { // Bronze
            return 0.1 ether;
        }

        require(prices[id] > 0, "Token has no price");
        ...
        onlyOwner
    {
        ...
        tokenSupplies[id] = tokenSupplies[id].add(amount);
    ...
    function _checkMintReq(uint256 _id, uint256 _amount) private view
    {
        ...
        require(_id > 0, "Token id is 0");
        require(tokenSupplies[_id] == 0 || tokenSupplies[_id] + _amount <= maxSupplies[_id], "not enought supply left");
    }
    ...
    */
    ...
    function uri(uint256 _id) override public view returns (string memory) {
        ...
        //return string(abi.encodePacked(url, Strings.toHexString(_id, 32), ".json"));
        return string.concat(_uriBase, Strings.toString(_id), ".json");
    }
    ...
    */
    ...
    function create(
        ...
        require(!_exists(_id), string.concat(
                "token ",
                Strings.toString(_id),
                " already exists"
        ));
        ...
        tokenSupplies[_id] = _initialSupply;
    ...
    */
    ...
    function contractURI() public view returns (string memory) {
        return _uriBase; // Contract-level metadata
    }
}