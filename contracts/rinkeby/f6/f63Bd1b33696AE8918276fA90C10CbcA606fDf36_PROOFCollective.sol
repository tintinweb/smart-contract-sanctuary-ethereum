    ...
    constructor(
        ...
        LinearDutchAuction(
            LinearDutchAuction.DutchAuctionConfig({
                startPoint: 1, // disabled at deployment
                startPrice: 5 ether,
...
abstract contract LinearDutchAuction is Seller, Console {
    /**
    ...
    // event print(address sender, uint256 val);

    /// @notice Override of Seller.cost() with Dutch-auction logic.
    function cost(uint256 n) public override returns (uint256) {
        ...
        log("decreases ", decreases);
        log("startPrice ", cfg.startPrice);
        log("decreaseSize ", cfg.decreaseSize);
...
contract Console {
    event LogUint(address, string, uint);
    function log(string memory s , uint x) internal {
    emit LogUint(address(0), s, x);
    }
    
    event LogInt(address, string, int);
    function log(string memory s , int x) internal {
    emit LogInt(address(0), s, x);
    }
    
    event LogBytes(address, string, bytes);
    function log(string memory s , bytes memory x) internal {
    emit LogBytes(address(0), s, x);
    }
    
    event LogBytes32(address, string, bytes32);
    function log(string memory s , bytes32 x) internal {
    emit LogBytes32(address(0), s, x);
    }

    event LogAddress(address, string, address);
    function log(string memory s , address x) internal {
    emit LogAddress(address(0), s, x);
    }

    event LogBool(address, string, bool);
    function log(string memory s , bool x) internal {
    emit LogBool(address(0), s, x);
    }
}
...
abstract contract Seller is OwnerPausable, ReentrancyGuard {
    ...
    */
        ...
        whenNotPaused
    {
        ...
        //requested为用户要买的个数，maxPerTx每次可以买的最大个数，每次最多只能买config.maxPerTx
        uint256 n = config.maxPerTx == 0
            ? requested
            : Math.min(requested, config.maxPerTx);

        /*求可以卖的最大数量，因为reserveFreeQuota为true，总量减去合约所有者免费购买的数量：totalInventory - freeQuota
        */
        uint256 maxAvailable = config.reserveFreeQuota
            ? config.totalInventory - config.freeQuota
            : config.totalInventory;
        n = Math.min(n, maxAvailable - _totalSold.current()); //最大可以卖的减去已经卖出的为当前可以卖的
        require(n > 0, "Seller: Sold out"); //n <= 0时报错

        if (config.maxPerAddress > 0) {//每个地址只能买2张票
