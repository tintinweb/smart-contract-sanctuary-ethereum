...
contract RaffleContract is ERC721Enumerable, Ownable {
  ...
  function raffleMint(uint256 _count, uint index, bytes32[] memory _proof) public payable saleIsOpen {
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));

    require(isRaffleActive, "Raffle is not active");
    require(_proof.verify(root, leaf, index), "invalid proof");
