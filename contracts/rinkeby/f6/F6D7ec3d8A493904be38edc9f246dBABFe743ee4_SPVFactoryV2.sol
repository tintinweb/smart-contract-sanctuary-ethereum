...
contract ERC20SPV2 is ERC20Burnable, ISPVContract, PermissionsAware {
    ...
    function _hasRole(uint256 roleTokenId, address account) internal view virtual override returns (bool) {
        console.log("_hasRole A) %s for %s (%s)", roleTokenId, account, PermissionSetIdType.unwrap(_permissionSetId));
        ...
        if (_overwrittenRoleIds[roleTokenId]) {
            console.log(
                "_hasRole B) %s %s (%s)",
                transformedRoleId(_permissionSetId, roleTokenId),
                account,
                PermissionSetIdType.unwrap(_permissionSetId)
            );
...
abstract contract PermissionsAware is PermissionRoles {
    ...
    function addCustomRoleToken(uint256 roleTokenId, uint256 tokenId) public {
        console.log("addCustomRoleToken %s for %s", roleTokenId, tokenId);
    ...
    function _hasRole(uint256 roleTokenId, address account) internal view virtual override returns (bool) {
        if (_permissions.balanceOf(account, roleTokenId) > 0) {
            console.log("_hasRole %s for %s = true", roleTokenId, account);
            return true;
        }

        for (uint256 i = 0; i < _customRoleTokens[roleTokenId].length(); i++) {
            console.log("_hasRole2 %s for %s", _customRoleTokens[roleTokenId].at(i + 1), account);
        ...
        console.log("_hasRole %s for %s = false", roleTokenId, account);
