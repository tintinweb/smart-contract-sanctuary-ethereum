...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    bytes32 public constant DOMAIN_SEPARATOR = 0x3dbbebdc924fd5bd0069e939c792a446a887bf0f172099cd4dd1210cb464e66e;
    ...
    event Log2 (
        bool validateOrderParametersBuy
    );
    ...
    event Log4 (
        bool validateOrderParametersSell
    );
    ...
    event Log8 (
        uint buyReplacementPattern,
        uint sellReplacementPattern,
        bool arrayEq
    );
        ...
        reentrancyGuard
    {
        ...
        if (buy.maker == msg.sender) {
            emit Log2(validateOrderParameters(buy));
            require(validateOrderParameters(buy));
        } else {
            emit Log3(_requireValidOrderWithNonce(buy, buySig));
        ...
        if (sell.maker == msg.sender) {
            emit Log4(validateOrderParameters(sell));
            require(validateOrderParameters(sell));
        } else {
            emit Log5(_requireValidOrderWithNonce(sell, sellSig));
            sellHash = _requireValidOrderWithNonce(sell, sellSig);
        }

        /* Must be matchable. */
        emit Log6(ordersCanMatch(buy, sell));
        ...
        emit Log8(
            buy.replacementPattern.length,
            sell.replacementPattern.length,
