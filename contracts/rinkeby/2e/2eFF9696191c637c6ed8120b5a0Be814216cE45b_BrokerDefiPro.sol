...
interface IPartner  {
    function partnerCodesVerification(uint16) external view returns(bool);
}
...
contract BrokerDefiPro is ERC721, Ownable, ReentrancyGuard {
    ...
    */
    ...
    function setCode(uint tokenId) private {
        uint8 code = uint8(block.timestamp);
    ...
    function escrow(uint tokenId) public {
        ...
        require(escrowPeriod[tokenId] == 0, "Already in escrow");
        escrowPeriod[tokenId] = block.timestamp;
    }

    function deEscrow(uint tokenId) public {
        require(escrowPeriod[tokenId] != 0, "Not in escrow yet");
        require(ERC721._exists(tokenId), "escrowing non existent token");
        require(ERC721.ownerOf(tokenId) == msg.sender, "Not your token");
        escrowPeriod[tokenId] = 0;
    }

    // mint function for public sale with partner code
    function publicMintPartner(uint amount, uint16 code) public payable nonReentrant {
        ...
        for (uint index = 0; index < amount; index++) {
            ...
            escrowPeriod[TOKEN_ID] = block.timestamp;
            setCode(TOKEN_ID);
        }
    }

    // mint function for public sale with pro code
    function publicMintPro(uint amount, uint16 code) public payable nonReentrant {
        require(saleIsActive && nftPrice > 0 && treasury != address(0), "Config not done yet");
        require(msg.value >= (nftPrice * amount), "Not enough balance");
        if (code > 0) {
    ...
    function _beforeTokenTransfer(
        ...
        require(false, "BrokerDefi: transfer of Pro tokens is not allowed");
    }
