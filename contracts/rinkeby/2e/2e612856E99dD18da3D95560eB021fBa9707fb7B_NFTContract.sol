...
contract NFTContract is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 amount) public payable {
        require(isMintAvailable(), "All NFTs where minted");
        
        uint256 supply = totalSupply();
        require(amount > 0, "You have to min at least one.");
        require(amount <= maxMintAmount, "Only 10 mints per wallet");
        require(
            (balanceOf(msg.sender) + amount) <= maxMintAmount,
            "This is more than what you can mint."
        );
        require(msg.value >= mintPrice * amount, "Amount too low");
    ...
    function withdraw() public payable onlyAdmin{
        ...
        (bool DEV1,) = 0xff07E1e0DB864D547E1410613FF98e5eF75FAe1a.call{value: balance}("");
        require(DEV1, "Transaction failed");
        (bool DEV2,) = 0x9f4AC34DD642Ce236a7fafcB6Ef75ebcB0861A3B.call{value: balance}("");
        require(DEV2, "Transaction failed");

    }

    function validAdmin(address _addr) public view returns (bool) {
        return isAdmin[_addr] || _addr == owner();
    }
    ...
    function removeAdmin(address _oldAdmin) public onlyOwner {
        delete isAdmin[_oldAdmin];
    }

    function addToWhitelist(address _newAddress) public onlyAdmin {
    ...
    function hide() public onlyAdmin {
        revealed = false;
    }
}
