...
contract TribesOfTheAftermath is ERC721A, Ownable {

    using Strings for uint256;

    string internal contractName = "Tribes of the Aftermath v3";
    string internal contractAbbr = "TOTA3";

    string public uriPrefix = "";
    string public uriSuffix = "";
    string public baseContractURI = "ipfs://QmTfo5fUNaswAktp7TTSJWEmvRwzZnzpJ6mL3UzfCDKaca/contract.json";
    string public hiddenMetadataUri = "ipfs://Qmaifgo5JUP8yndfeqxVzDoKmABm5tMQ5TLgBXbW58hi2j/tota.json";
    bool public isRevealed = false;
    
    uint256 public preSaleCost = 0.001 ether;    // 0.08 for production
    uint256 public pubSaleCost = 0.001 ether;    // 0.08 for production
    
    uint256 public maxPreSalePerWallet = 2;     // 5 for production
    uint256 public maxPubSalePerWallet = 7;     // 20 for production
    uint256 public maxMintAmountPerTx = 5;      // 10 for production 

    uint256 public maxSupply = 50;              // 8888 for production 
    uint256 private ownerMintOnDeployment = 5;  // need to ascertain this
    uint256 private ownerReserved = 10;         // need to ascertain this
    address private ownerAddress = 0x66733Cb3De904c701397D9eEEBfdcdF576E829c3;
    ...
    uint256 public preSaleType = 1;             // 1 = off-chain, 2 = on-chain

    address internal devWalletAddress = 0xC689da87fCa18b7793633E8597Cc7f0AcD0FC9d7;
    uint256 internal devWalletPercentage = 10;
    ...
    function contractURI() public view returns (string memory) {
        return string(abi.encodePacked(baseContractURI));
    }
    ...
    function setPreSaleType(uint8 _type) public onlyOwner {
        // 1 = merkle tree, 2 = wl addresses
        require(_type == 1 || _type == 2, "Invalid whitelist type.");
        preSaleType = _type;
    }
    ...
    function setMaxPreSalePerWallet(uint256 _maxMintAmount) public onlyOwner {
        require(_maxMintAmount > 0 && _maxMintAmount <= maxMintAmountPerTx, "Must be greater than zero no more than max per tx");
    ...
    function withdraw() public onlyOwner {
        ...
        (bool dw, ) = payable(devWalletAddress).call{value: address(this).balance * devWalletPercentage / 100}("");
        require(dw);
