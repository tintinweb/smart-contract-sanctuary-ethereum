...
contract PiSpace is ERC721, ReentrancyGuard, Ownable
{
	...
	uint public sale_price = 0.00001 ether;
	 ...
	 */
	constructor() ERC721("PiSpace", "PISPA")
	{
	...
	*/
	...
	{
		require(_exists(_pie), "Pi Space doesn't exist.");

		string memory ingredients = string(abi.encodePacked(getIngredients(_pie), bytes(_pie.toString())));
	...
	{
		string[] memory times = new string[](6);
		times[0] = (17 + rand(_ingredients, 1, 5)).toString();
		times[1] = rand(_ingredients, 2, 5).toString();
		times[2] = (23 + rand(_ingredients, 3, 5)).toString();
		times[3] = rand(_ingredients, 4, 3).toString();
		times[4] = (29 + rand(_ingredients, 5, 5)).toString();
		times[5] = (8 + rand(_ingredients, 6, 4)).toString();
	...
	function generateSVGCircles(string memory _ingredients, uint _num_circles, string[] memory _colours, uint _nonce) internal view returns (bytes memory)
	{
		...
		{
			cx = bytes(rand(_ingredients, (++_nonce), 1200).toString());
			cy = bytes(rand(_ingredients, (++_nonce), 1200).toString());
			r = bytes(rand(_ingredients, (++_nonce), 800).toString());
			o = bytes((rand(_ingredients, (++_nonce), 65) + 10).toString());
			bm = rand(_ingredients, (++_nonce), blend_modes.length);
			c = rand(_ingredients, (++_nonce), _colours.length);
			...
			svg = abi.encodePacked(
				...
				blend_modes[bm], // blend mode — overlay, lighten, multiply
				'; opacity: 0.',
				o, // opacity 0–1
				';"></circle>'
			);
