  ...
  Pausable
{
  ...
  event RewardLevelChanged(
    address causedBy,
    uint256 oldRewardLevel,
  ...
  constructor(
    ...
    setSupplyDenominator(_supplyDenominator);
    setRewardLevelInterval(_rewardLevelInterval);
    _pause();
  }
   ...
   *         PogPunks staked is calculated by:
   *            totalPogPunksStaked/supplyDenominator.
   * @param _supplyDenominator The denominator used to scale additionalRate for
   *                           calculating staking rewards.
   */
  function setSupplyDenominator(uint256 _supplyDenominator) public onlyOwner {
  ...
  /**
   * @notice Calculate rewards for a stakeholder's token.
   * @param account The stakeholder to calculate rewards for.
  ...
  /**
   * @notice Claim rewards for a stakeholder's tokens.
  ...
  function rebalance() external nonReentrant whenNotPaused {
    ...
    while (currentAccount != accounts.HEAD()) {
      ...
      EnumerableSet.UintSet storage tokens = _deposits[currentAccount];
      for (uint256 i; i < tokens.length(); i++) {
        uint256 depositBlock = _depositBlocks[currentAccount][tokens.at(i)];

        if (currentBlock > depositBlock) {
          reward += calculateTotalRate() * (currentBlock - depositBlock);
        }

        _depositBlocks[currentAccount][tokens.at(i)] = currentBlock;
      }
   ...
   *         staking.
   */
  function handleNewRewardLevel(address causedBy) private whenNotPaused {
    uint256 percentageDeposited = (totalTokensDeposited * 1e2) /
      supplyDenominator;
    ...
    if (rewardLevel != newRewardLevel) {
      emit RewardLevelChanged(causedBy, rewardLevel, newRewardLevel);
    }
  }

  /**
   * @notice Deposit tokens for staking. Claims rewards for tokens before
   *         depositing new ones.
   * @param tokenIds The ids of the tokens to deposit for staking.
   * @dev emit event when reward level is hit
   */
  function deposit(uint256[] calldata tokenIds) external whenNotPaused {
    ...
    handleNewRewardLevel(msg.sender);
  }

  /**
   * @notice Withdraw tokens from staking. Claims all rewards for tokens before
   *         withdrawing them.
