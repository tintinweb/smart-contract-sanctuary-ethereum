...
contract Citizen is ERC721A, BaseTokenURI {
    using Strings for uint256;

    // Hash of whitelisted wallets using makle tree
    ...
    // Boolean to control presale for whitelisted wallets
    ...
    // Uint for minting Price
    uint256 public _mintPrice = 100000000000000;

    // Max minting limit per wallet
    uint32 internal _minitingLimit = 6;
    ...
    constructor(address platformAddress, string memory notRevealURI, uint32 maxSupply_)
        ERC721A("Citizen", "CITIZEN") 
        BaseTokenURI(" ") 
        {
    ...
    // EVENTS
    event StakeNFT(uint32[] tokenIDs, address indexed owner, uint256 time);

    event UnstakeNFT(uint32[] tokenIDs, address indexed owner, uint256 time);

    event UpdateTokenURI(uint32 tokenId, string uri, uint256 time);
    ...
    event WithdrawFund(uint256 amount, uint256 time);
    ...
    function updateTokenURI(uint32 tokenID, string calldata uri, bytes calldata sig) 
        public
        {
    ...
    function withdrawFund()
        ...
        emit WithdrawFund(address(this).balance, block.timestamp);
    }
    ...
    function safeMint(uint256 quantity)
        ...
        require(_numberMinted(msg.sender) + quantity < _minitingLimit, "you can't mint more then 5 NFT");
        require(msg.value >= _mintPrice * quantity, "value is less then minting price");
        _safeMint(msg.sender, quantity);
        emit TokenCount(msg.sender, _numberMinted(msg.sender), totalSupply());
    }

    function preSaleSafeMint(uint256 quantity, bytes32[] calldata merkleProof)
        public
        payable
        preSaleIsOpen
        {
