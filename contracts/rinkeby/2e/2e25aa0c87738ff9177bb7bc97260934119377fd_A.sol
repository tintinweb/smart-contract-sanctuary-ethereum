...
contract A is ReentrancyGuard, AdminControl, ERC721, IA {
    ...
    constructor(address creditsAddress) ERC721("A", "A") {
    ...
    /**
     * @dev See {IA-getMetadataContracts}.
     */
    ...
    /**
     * @dev See {IA-setMetadataContracts}.
     */
    ...
    function _onERC1155Received(address from, uint256 id, uint256 value, bytes calldata data) private {
        ...
        if (data.length == 64) {
            ...
            require(level > 0 && _levelCredits.length >= level, "Invalid request");
            uint8 levelIndex = level-1;
            require(_levelCredits[levelIndex] > 0 && (_levelCredits[levelIndex] - _levelCredits[levelIndex-1]) == value, "Invalid request");
        } else {
            revert("Invalid data");
        }

        // Burn it
        try IACredits(msg.sender).burn(address(this), uint16(value)) {
        } catch (bytes memory) {
            revert("Burn failure");
        }
 ...
 * A Interface
 */
interface IA is IERC1155Receiver {
