...
contract COSMIQS is ERC721A, Ownable, PaymentSplitter  {
    ...
    mapping(address => bool) public incubatorList;
    ...
    function mintCosmiq(uint256 _tokenAmount) external payable {
        ...
        if (msg.sender != owner()) {
            ...
            if(activeIncubator == true) {
                require(incubatorList[msg.sender] == true, "Not on Incubator List");
            ...
            if(activeWhitelist == true && activeIncubator == false) {
                require(onWhitelist[msg.sender] == true, "Not on Whitelist!");
    ...
    //WL+PreSale setting
    function whitelistUsers(address[] calldata _addresses) external onlyOwner {
        for (uint256 i; i < _addresses.length; i++) {
            onWhitelist[_addresses[i]] = true;
        }
    }

    function addIncubatorList(address[] calldata _addresses) external onlyOwner {
        for (uint256 i; i < _addresses.length; i++) {
            incubatorList[_addresses[i]] = true;
        }
    }
