...
contract Request is ReentrancyGuard,Initializable{
    ...
    function completed() external nonReentrant{
        require((msg.sender == owners || msg.sender == market), "!Owner|Market");
        isComplete = true;
    }
...
contract Offer is EscowVault, Fee, ReentrancyGuard{
    ...
    //     //to resell owner need to setApprovalForAll to marketplaceAddress
     ...
     function reTokentoMarket(uint _tokenId, address _tokenAddress, uint _price)public returns(uint) {
...
contract AuctionV1 is ReentrancyGuard{
    ...
    function auctionEnd() public returns(bool){
        ...
        require(msg.sender == beneficial || msg.sender == market , "!allow");
        require(!ended,"Ended");
...
contract MarketAuction is ReentrancyGuard, Offer{
    ...
    function end(address _tokenAddress, uint _tokenId) public{
        require(activeSell[_tokenAddress][_tokenId] == false,"!avai");
        require(AuctionV1(nft2Auction[_tokenAddress][_tokenId]).isEnd()==true, "!end");
        _sendToken(_tokenAddress,_tokenId,AuctionV1(nft2Auction[_tokenAddress][_tokenId]).getHighestBidder());
        transferEscowOnSell(_tokenAddress,_tokenId);
