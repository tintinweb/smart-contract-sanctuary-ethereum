...
contract Passport is ERC721Pausable, AccessControl
{
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    string private _baseTokenURI;
    address private _defaultAdmin;
    event Mint(uint256 indexed tokenId, address indexed wallet);
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");

// "Orange Fund LP Passport", "OFLP"
    constructor(
        string memory name,
    ...
    function mintTo(address wallet)
        external
        whenNotPaused
    {
                // save tokenId in case of burn
        // Transfer(address from, address to, uint256 tokenId)
        require(balanceOf(wallet) == 0, "Balance before mint must be 0");
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(wallet, tokenId);

        emit Mint(tokenId, wallet);
    }
