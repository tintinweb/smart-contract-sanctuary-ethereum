...
contract LockdropFactory is ILockdropFactory, Accessible {
    ...
    function create(
        ...
        uint256 id = _lockdrops.length;
        bytes memory args = abi.encode(address(SOMA));
        bytes32 salt = keccak256(abi.encodePacked(id, asset, address(this)));

        address instance = SOMA.templateFactory().predictDeployAddress(
            TEMPLATE,
            TEMPLATE_VERSION,
            args,
            salt
        ...
        _lockdrops.push(instance);

        SOMA.templateFactory().deployTemplate(
            TEMPLATE,
            TEMPLATE_VERSION,
            args,
            salt
        ...
        ILockdrop(instance).initialize(id, asset, withdrawTo, startsAt, endsAt);

        emit LockdropCreated(id, asset, instance);
    }
}
