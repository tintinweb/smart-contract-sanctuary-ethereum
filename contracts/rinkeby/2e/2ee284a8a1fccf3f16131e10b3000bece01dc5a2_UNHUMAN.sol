...
abstract contract ERC721URIStorage is ERC721 {
    ...
    mapping(uint256 => string) private _gradients;
    ...
    mapping(uint256 => bytes32[]) private _colors;
    ...
    function drawPoly(uint256 point0x, uint256 point0y, uint256 point1x, uint256 point1y, uint256 point2x, uint256 point2y,  uint256 point3x, uint256 point3y, string memory color) public view returns (string memory) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        uint256 gradientCounter = 0;
        polys = '<svg width="500" height="500" xmlns="http://www.w3.org/2000/svg"><polygon fill="black" points="0,0 500,0 500,500 0,500" />';
        polys = string(abi.encodePacked(polys, _gradients[tokenId]));

        for (i = 0; i < (_noObjects[tokenId]*2)-1; i++) {
            points[0] = _coords[tokenId][4 * i][0];
            points[1] = _coords[tokenId][4 * i][1];
            points[2] = _coords[tokenId][4 * i+1][0];
            points[3] = _coords[tokenId][4 * i+1][1];
            points[4] = _coords[tokenId][4 * i+2][0];
            points[5] = _coords[tokenId][4 * i+2][1];
            points[6] = _coords[tokenId][4 * i+3][0];
            points[7] = _coords[tokenId][4 * i+3][1];

            if (i % 2 == 0){
                poly = drawPoly(points[0], points[1], points[2], points[3], points[4], points[5], points[6], points[7], "blue");
                colorCounter = colorCounter + 1;
            }
            else {
                poly = drawPoly(points[0], points[1], points[2], points[3], points[4], points[5], points[6], points[7], string(abi.encodePacked("g", uint2str(gradientCounter))));
                gradientCounter = gradientCounter + 1;
            }
            
            polys = string(abi.encodePacked(polys, poly));
            
        }

        polys = string(abi.encodePacked(polys, "</svg>"));
        ...
        if (bytes(polys).length > 0) {
            return string(abi.encodePacked(_baseURI(), polys));
        }
     ...
     * - `tokenId` must exist.
     */
    function _setGradients(uint256 tokenId, string memory _gradientsPassed) internal virtual {
        require(_exists(tokenId), "ERC721URIStorage: URI set of nonexistent token");
        _gradients[tokenId] = _gradientsPassed;
    }
    ...
    function _setColors(uint256 tokenId, bytes32[] memory _colorsPassed) internal virtual {
        require(_exists(tokenId), "ERC721URIStorage: URI set of nonexistent token");
...
constructor(address initialRoyaltiesReceiver) ERC721("UNHUMAN COMPOSITIONS", "UNHUMAN") {
    ...
    function mint(uint256[2][] memory coords, string calldata gradients, uint256 noObjects, uint256 noColors, bytes32[] memory colors) external payable {
    ...
    _setGradients(MAX_SUPPLY-artBalance[address(this)]+1, gradients);
