...
contract DerangedApes is ERC721A, Ownable, ReentrancyGuard, IERC1155Receiver {
    ...
    event Derangement(uint indexed mutantID, uint indexed sodaID, uint[] indexed flavors);
    ...
    function mutateSODA(address from, uint id, uint[] calldata flavors) internal nonReentrant {
        if(soda.ownerOf(id) != from) { revert NotOwnerOfSODA(); }
        if(flavors.length > 5) { revert FlavorOverflow(); }
    ...
    function getBit(uint32 _packedBools, uint32 _boolNumber) private view returns(uint32) {
        return (_packedBools >> _boolNumber) & 1;
    }

    function setBit(uint32 _packedBools, uint32 _boolNumber) private view returns(uint32) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return
            baseURI.length > 0
                ? string(
                    bytes.concat(baseURI, bytes(tokenId.toString()), bytes(".json"))
                )
                : "";
    }
    ...
    function onERC1155BatchReceived(address operator, address from, uint[] calldata ids, uint[] calldata values, bytes calldata data) external returns (bytes4) {
        uint val = toUint256(data);
        mutateSODA(from, val, ids);
