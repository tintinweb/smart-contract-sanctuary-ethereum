...
contract TollPool is IfnxFiOwnableUpgrade, DecimalERC20 {
    using Decimal for Decimal.decimal;
...
abstract contract BaseBridge is IfnxFiOwnableUpgrade, IBaseBridge, DecimalERC20 {
    using Decimal for Decimal.decimal;
    ...
    // PUBLIC
    //
    function __BaseBridge_init(IAMB _ambBridge, IMultiTokenMediator _multiTokenMediator)
        internal
        initializer
    {
        __Ownable_init();
    ...
    function approveToMediator(IERC20 _token) private {
        if (
            _allowance(_token, address(this), address(multiTokenMediator)).toUint() != uint256(-1)
        ) {
...
contract MetaTxGateway is IfnxFiOwnableUpgrade, LowLevelErrorMessage {
    using SafeMath for uint256;
