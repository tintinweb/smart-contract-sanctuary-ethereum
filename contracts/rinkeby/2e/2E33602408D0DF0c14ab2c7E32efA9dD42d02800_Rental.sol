...
contract Rental is Ownable, IERC721Receiver {
    ...
    // Constructor
    constructor(
        ScratchNFT _nftAddress
    ) {

        rentalPayment = 0.01 ether;
        collateral = 0.03 ether;    
        nft = _nftAddress;
    }
   ...
   struct Lease {
        ...
        uint256  nullificationTime;      //The contract deployoor specifies a period by which the assets must be deposited else the contract is voided
    ...
    function depositNft(
        ...
        if (block.timestamp >= _nullificationTime) revert InvalidState();
        ...
        nft.transferFrom(msg.sender, address(this), tokenId);     
         ...
         book[tokenId].nullificationTime       = _nullificationTime;
    ...
    function depositEth(uint256 _tokenId) external payable {
        ...
        if (block.timestamp >= book[tokenId].nullificationTime) revert NotAvailableNow();
    ...
    function withdrawNft(uint256 _tokenId) external  {
        ...
        book[tokenId].nullificationTime       = 0; 
    ...
    function returnNft(uint256 _tokenId) external {
        ...
        if ( block.timestamp < (book[tokenId].dueDate + book[tokenId].collateralPayoutPeriod) ){
    ...
    function withdrawCollateral(uint256 _tokenId) public {
        ...
        if (tardiness >= book[tokenId].collateralPayoutPeriod) {
            ...
            payableAmount = (tardiness * collateral) / book[tokenId].collateralPayoutPeriod;
        }
        ...
        if(book[tokenId].currentlyOnLease) {
            ...
            // The Borrower transfered the NFT, Collateral gets divided among lender and borrower
            payable(book[tokenId].lenderAddress).transfer(payableAmount);
            payable(book[tokenId].borrowerAddress).transfer(collateral- book[tokenId].collectedCollateral);



        if ( block.timestamp >(book[tokenId].dueDate + book[tokenId].collateralPayoutPeriod) ){
            book[tokenId].lenderAddress           = book[tokenId].borrowerAddress;
        }
            book[tokenId].ethIsDeposited          = false;
            book[tokenId].nftIsDeposited          = false;
            book[tokenId].dueDate                 = 0;
    ...
    function tokensOfOwner(address account) public view returns (uint256[] memory ownersTokens) {
        ...
        for(uint tokenId = 1; tokenId <= supply; tokenId++) {
            if ( book[tokenId].lenderAddress == account && book[tokenId].currentlyOnLease ) {
    ...
    function tokensOnLease( ) public view returns (uint256[] memory tokensCurrentlyOnLease) {
    ...
    function tokensAvailableForLease( ) public view returns (uint256[] memory tokensAvailableForLease) {
        ...
        uint256 index = 0;
        for(uint tokenId = 1; tokenId <= supply; tokenId++) {
            if ( book[tokenId].nftIsDeposited ) {
                tmp[index] = book[tokenId].nftId;
                index +=1;
            }
        }
...
contract ScratchNFT is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Scratch NFT Collection", "SNC") {
        ...
        return "https://gateway.pinata.cloud/ipfs/QmdNQXVHJrzfvEGjvZj9Y4ujnqntzYAWta9pyKUnypkRv7/0";
    }
        // public
