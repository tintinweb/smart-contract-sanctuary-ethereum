...
contract PredictionRecorder {
     ...
     * My address (integer form) is 1520779317728643809679224833904517355724024437910
     *                                | |  |    |   |    |    |
     *                  which means   2 7  3    8   8    9    3 is a valid watermark.
...
contract InvitationalBet {
     ...
     * They can also pre-check their payout factor, which is a function of predicted value & true value.
     * If they did not predict or did not meet watermark requirement, the factor is set to 1.
     *
     * At a later time, payouts to participants will open and anyone can trigger them.
     * It is assumed that all participants have decrypted their predictions.
     * The reward pool will be distributed proportionally to the payout coeffients:
     *
     * Payout coefficient = payout factor * bet value of participant
     */
    ...
    constructor(
        ...
        require(block.timestamp < _targetTime, "Cannot target the past.");
        require(_creationCloseTime < _targetTime, "Predictions must close before the target time.");
        require(_targetTime < _payOpenTime, "Payouts can only happen after the target time.");
    ...
    /**
     * An example of calculating payout factors: staircase exponential decay, bounded below by 1.
    ...
    /**
     * Calculate payout coefficients for every participant.
     * Payouts will be delivered proportionally to these coefficients.
     */
    function computeAllPayCoefficients() public view afterPayOpen returns (uint[] memory) {
        ...
        /* Calculate payout coefficients proportional to bet value */
    ...
    /**
     * Trigger payouts. This completes the bet.
     */
