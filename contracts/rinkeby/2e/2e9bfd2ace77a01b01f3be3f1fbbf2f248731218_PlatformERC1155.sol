...
contract PlatformERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI, IERC2981, Ownable {
  ...
  * @dev Transfers `amount` tokens of token type `id` from `from` to `to`.
  ...
  function batchMint(address to, uint256[] memory tokenIds, uint256[] memory amounts, string[] memory uris) public onlyOwner {
    _mintBatch(to, tokenIds, amounts, "");
    for (uint i = 0; i < tokenIds.length; i++) {
      require(balanceOf(to, tokenIds[i]) == 1, "batchMintToOneOwner: token not minted");
