...
// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@//
////////////////////////////////////////////////////////////////////////////////////////////////////////
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    //apple string

    bool public midnightReleaseIsOn = false;
    bool public arcadeIsOpen = true;
    ...
    address internal arcadeMaintenanceDude =
        0x961f67EFDacfcD05dFa35Ec63F050396F8AEdB90;

    string[2] private metadata = [
        '{"name": "OCA-Snake", "description": "On Chain Arcade Snake game.", "animation_url": "',
        '"}'
    ];
    struct UserConfig {
        uint256 inventory;
        uint256 extraLives;
        uint256 maxEarlyMint;
        address readyPlayerOne;
        address readyPlayerTwo;
        string apple;
    }

    mapping(address => UserConfig) public userConfigList;

    constructor() ERC721A("On Chain Arcade ", "OCA_SNAKE", 1, 1997) {
        //Set all static variables on initialization.

        arcadeOwner = msg.sender;
        _setDefaultRoyalty(arcadeMaintenanceDude, 750);
    ...
    modifier callerIsNeo() {
        bool isTrue = arcadeMaintenanceDude == msg.sender ||
    ...
    // modifier setNeo() {
    //     Neo = arcadeMaintenanceDude == msg.sender;
    //     _;
    // }
        ...
        returns (string[10] memory)
    {
        ...
        for (uint256 index = 0; index < 10; index++) {
            adjectives[index] = wackyAdjectives[
                generateAppleSeed(tokenId, index) % wackyAdjectives.length
            ];
        }
        return adjectives;
    }

    function findNameThatDoesNotExist(uint16 tokenId, string memory snakename)
        private
        view
        returns (string memory)
    {
        string[10] memory adjectives = generateAdjectives(tokenId);
        string memory name;
        for (uint256 index = 0; index < 10; index++) {
            string memory checkName = string(
                abi.encodePacked(adjectives[index], "_", snakename)
            );
            if (!snakenames[checkName]) {
                name = checkName;
                break;
            }
        }
    ...
    struct ni {
        uint8 snakeNameIndex;
        uint8 adjectiveIndex;
        uint8 supremeIndex;
        uint16 combineIndex;
    }

    function snakeNameSave(uint16 tokenId) public view returns (ni memory) {
        uint8 snakeNameIndex = uint8(
            generateAppleSeed(tokenId, 0) % snakes.length
        ) + 1;
        uint8 adjectiveIndex = uint8(
            generateAppleSeed(tokenId, 69) % wackyAdjectives.length
        ) + 2;
        uint8 supremeIndex = uint8(
            generateAppleSeed(tokenId, 420) % supremeAdjectives.length
        ) + 1;
        uint16 combineIndex = uint16(snakeNameIndex * adjectiveIndex);

        // if (snakeNameIndexes[combineIndex]) {
        //     return snakeNameSave(tokenId, seed + 1);
        // } else return combineIndex;

        // snakeNameIndexes[0] = NameInformation(
        //     snakeNameIndex,
        //     adjectiveIndex,
        //     supremeIndex
        // );
        ...
        //     abi.encodePacked(
        //         wackyAdjectives[adjectiveIndex],
        //         "_",
        //         snakes[snakeNameIndex]
        //     )
        // );
        ...
        //         );
        // } else {
        //     snakenames[userName] = true;
        //     return userName;

        // snakenames[userName] = true;

        return ni(snakeNameIndex, adjectiveIndex, supremeIndex, combineIndex);
    }

    function metaDataCreator(string[2] memory vars)
        private
        view
        returns (bytes memory)
    {
        bytes memory metaBytes;
        for (uint8 i = 0; i < vars.length; i++) {
            metaBytes = abi.encodePacked(metaBytes, metadata[i], vars[i]);
        }
        return abi.encodePacked(metaBytes, metadata[1]);
    }
    ...
    function coinRemoval() public {
        (bool success, ) = arcadeMaintenanceDude.call{
        ...
        callerIsNeo
    {
        ...
        //     partyList[addresses[i]] = extraLives;
        // }
    ...
    function checkCoinInflation(uint256 amount) private view {
        ...
        for (uint256 i = 0; i < amount; i++) {
            if (
                midnightReleaseIsOn &&
                userConfigList[msg.sender].inventory == 0 &&
                i == 0
            ) {
                totalCoins += midnightReleasePrice;
            } else {
                totalCoins += arcadePrice;
            }
        }
        require(msg.value >= totalCoins, "Did you ask your mom for quarters?");
    }

    function insertQuarter(uint256 amount) public payable callerIsSender {
        if (
            !midnightReleaseIsOn &&
            arcadeIsOpen &&
            userConfigList[msg.sender].readyPlayerOne == address(0)
        )
            userConfigList[msg.sender] = UserConfig(
                0,
                extraLives,
                0,
                msg.sender,
        ...
        require(amount <= maxLives, "Coinslot is full.");
        bool Neo = arcadeMaintenanceDude == msg.sender ||
            msg.sender == arcadeOwner;
        if (!Neo) {
            ...
            require(amount < maxLives);

            if (midnightReleaseIsOn) {
                // require(userConfigList[msg.sender].extra - amount >= 0);
                require(
                    userConfigList[msg.sender].inventory + amount <=
                        userConfigList[msg.sender].maxEarlyMint
                );
            }

            require(userConfigList[msg.sender].extraLives - amount >= 0);
    ...
    function _safeMint(
        ...
        uint16 startTokenId = currentIndex;
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            tokenInfoList[startTokenId] = TokenInfo(block.timestamp, 0);
    ...
    function snakeIndexToName(uint16 id) private view returns (string memory) {
        NameInformation memory nameInfo = snakeNameIndexes[id];
        string memory wackyAdjective = wackyAdjectives[nameInfo.wackyAdjective];
        string memory snakeName = snakes[nameInfo.snakeName];
        string memory supremeAdjectives = supremeAdjectives[
            nameInfo.supremeAdjective
        ];
        string memory name = string(
            abi.encodePacked(wackyAdjective, snakeName, supremeAdjectives)
        );
        return name;
    }

    function metaDataCreator(uint16 tokenId)
        internal
        view
        returns (bytes memory)
    {
        return
            abi.encodePacked(
                '{"name": "',
                snakeIndexToName(tokenId),
                '", "description": "On Chain Arcade Snake game.", "animation_url": "',
                image_funk(tokenId),
                '"}'
            );
    }
    ...
    IERC721Enumerable
{
    ...
    function _numberMinted(address owner) private view returns (uint256) {
        require(
            owner != address(0),
...
//TODO: Need to create different colors for snake so the chances of same background is minimized (Right now 1/20 could be the same)
//TODO: Need to create website build funk function.

contract OCASnakeArt is Ownable, ReentrancyGuard, username_list {
    bool public turnOnThePower = true;

    string internal apple = "NFT";
    ...
    // constructor(
    //     bool _turnOnThePower,
    //     string memory _apple,
    //     string[8] memory _opacity
    // ) {
    //     opacity = _opacity;
    //     apple = _apple;
    //     turnOnThePower = _turnOnThePower;
    // }
    ...
    function funkCreator(
        ...
        return abi.encodePacked("rgba(", r, ",", g, ",", b, ",", a, ")");
    }
    ...
    function funkToString(
        string memory c1,
        string memory c2,
        string memory c3,
        string memory opacityString
    ) private pure returns (string memory) {
        return string(abi.encodePacked(funkCreator(c1, c2, c3, opacityString)));
    }

    function generateAppleSeed(uint256 tokenId, uint256 salt)
        internal
        view
        returns (uint256)
    {
        uint256 seed = uint256(
            keccak256(abi.encodePacked(apple, tokenId, salt))
        );
        return seed;
    }

    function selectDifficulty() private pure returns (string memory) {
        return Strings.toString(randomNumber(1, 3));
    }

    function randomNumber(uint256 min, uint256 max)
        internal
        pure
        returns (uint256)
    {
    ...
    function allYourBaseAreBelongToUs(uint256 tokenId)
        private
        view
        returns (string memory)
    {
        string[20] memory selectedColors = getAppleIndex(tokenId);

        string memory base64 = Base64.encode(
            // abi.encodePacked(
            //     "<html><script src=https://gateway.pinata.cloud/ipfs/QmZVp7zVurDTromjmhPP3z2JBoEAq1wSg7quVcmSzkRcWB></script><body class='float' style='margin:0;padding:0;box-sizing:border-box;border:solid ",
            //     selectedColors[
            //         generateAppleSeed(tokenId, 786) % uint256(19)
            //     ],
            //     " 4px;background:",
            //     selectedColors[
            //         generateAppleSeed(tokenId, 69) % uint256(19)
            //     ],
            //     "'onload=\"df=",
            //     selectDifficulty(),
            ...
            //     "',z)(0);G('",
            //     selectedColors[
            //         generateAppleSeed(tokenId, 1337) % uint256(19)
            //     ],
            //     "')(a);s.map(G('",
            //     selectedColors[
            //         generateAppleSeed(tokenId, 420) % uint256(19)
            //     ],
            ...
            //     selectedColors[
            //         generateAppleSeed(tokenId, 143) % uint256(19)
            //     ],
            //     "'/></div><span id=sc class=oca_font></span><span id=sp class=oca_font> </span><div html=https://gateway.pinata.cloud/ipfs/QmSqzQFoavagaDBoQZEQADXvrCLWB9t1Ze4d1XhVd6JV4b/><audio id='e'/></html>"
            // )
            funkSnakeCreator(
                [
                    ...
                    selectDifficulty(),
                    selectedColors[
                        generateAppleSeed(tokenId, 23) % uint256(19)
                    ],
                    selectedColors[0],
                    selectedColors[
                        generateAppleSeed(tokenId, 1337) % uint256(19)
    ...
    function image_funk(uint256 tokenId) internal view returns (bytes memory) {
        bytes memory svgData = cloneSnake(tokenId);
        return
            turnOnThePower
                ? (
                    abi.encodePacked(
                        allYourBaseAreBelongToUs(tokenId),
                        '","image_data": "',
                        svgData
                    )
                )
        ...
        returns (string memory)
    {
        return
            Strings.toString(
                uint256(generateAppleSeed(tokenId, number)) % uint256(255)
            );
    }

    function retrofy(uint256 tokenId) private view returns (string[20] memory) {
        string[20] memory parts;

        for (uint256 i = 0; i < parts.length; i++) {
            string memory c1 = i == 1
                ? generateColors(tokenId, 128)
                : generateColors(tokenId, 128 + i);
            string memory c2 = i == 1
                ? generateColors(tokenId, 0)
                : generateColors(tokenId, i);
            string memory c3 = i == 1
                ? generateColors(tokenId, 69)
                : generateColors(tokenId, 69 + i);
            uint256 o = uint256(generateAppleSeed(tokenId, i) + i);

            if (i == 1) parts[1] = funkToString(c1, c2, c3, opacity[0]);
            else
                parts[i] = funkToString(
                    c1,
                    c2,
                    c3,
    ...
    function cloneSnake(uint256 tokenId) private view returns (bytes memory) {
        string[20] memory selectedColors = getAppleIndex(tokenId);
        ...
        return
            funkPictureCreator(
                [
                    ...
                    selectedColors[generateAppleSeed(tokenId, 7) % uint256(19)],
                    selectedColors[1],
                    selectedColors[
                        generateAppleSeed(tokenId, 143) % uint256(19)
                    ],
                    selectedColors[
                        uint256(generateAppleSeed(tokenId, 1337)) % uint256(19)
                    ],
                    string(svgData),
...
contract username_list is Ownable, ReentrancyGuard {
    struct HighScores {
        uint16 score;
        uint128 time;
    }
    ...
    struct NameInformation {
        uint8 snakeName;
        uint8 wackyAdjective;
        uint8 supremeAdjective;
    }
    ...
    mapping(uint16 => TokenInfo) public tokenInfoList;

    mapping(string => bool) public snakenames;
    mapping(uint16 => NameInformation) public snakeNameIndexes;
    string[250] internal wackyAdjectives = [
        "stoic",
        "Evil",
        "Center",
        "Stiff",
    ...
    string[11] internal supremeAdjectives = [
        "",
        "Degen",
        "NGMI",
        "GMI",
    ...
    string[17] internal snakes = [
        ...
        "$|\\\\|a|<3",
        ".../-./.-/-.-/.'////",
        "0101001101101110011000010110101101100101",
        "U25ha2U=",
        "83 110 97 107 101",
        "536e616b65"
    ];
