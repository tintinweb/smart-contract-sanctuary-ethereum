...
contract TestNft is Context, AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage {
    ...
    constructor(
        ...
        uint256 priceValidUntil, uint256 max, address admin, address multiD, address multiN, address signer
    ) ERC721(name, symbol) {
    ...
    function setPrice(uint256 mintPrice, uint256 priceValidUntil) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _price = mintPrice;
        _priceValidUntil = priceValidUntil;
    ...
    function mint(uint256 amount, uint256 slotId, bytes memory signature) external payable {
        require(_canMint(_msgSender(), amount, slotId, signature), "TestNft: must have valid signing");
