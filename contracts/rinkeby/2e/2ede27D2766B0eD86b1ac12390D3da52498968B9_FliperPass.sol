...
contract FliperPass is ERC721Enumerable, Ownable {
    ...
    function setTokenLevel(uint256 tokenId, string memory level) public onlyOwner {
        ...
        require((keccak256(abi.encodePacked(level))) == "silver" || (keccak256(abi.encodePacked(level))) == "gold" || (keccak256(abi.encodePacked(level))) == "platinum", "level must be silver, gold or platinum");
        Pass memory pass = _tokenTraits[tokenId];
        pass.level = level;
        pass.imagePath = keccak256(abi.encodePacked((level))) == "silver" ? silver_image : keccak256(abi.encodePacked((level))) == "gold" ? gold_image : platinum_image;
