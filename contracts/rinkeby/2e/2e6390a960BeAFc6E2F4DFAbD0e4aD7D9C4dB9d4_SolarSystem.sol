...
contract SolarSystem is ERC721Tradable {
    ...
    string private MASK = 'stop-color="#,?A,148|radialGradient,?B,30|gradientTransform="translate(,?C,12|fill="url(#,?D,15|transform="translate(,?E,29|stroke-width="0.5669",?F,8|spreadMethod="pad",?G,15|repeatCount="indefinite"/>,?H,11|keyTimes="0;1" calcMode="linear",?I,1|<ellipse rx=",?4,11|stop id=",?L,154|offset=",?M,154|animateMotion keyPoints="1;0",?N,11|stroke="#808080",?O,8|path=",?P,11|animation-delay:,?Q,13|transform,?R,19|<circle,?S,12|000,?T,30|linear infinite reverse,?U,13|scale(,?V,16|animation:,?Z,13|mask,?X,12|<g id=",?Y,22|</g>,?W,34|s" begin=",??,11|fill=",?0,13|stop-color="rgba(,?1,6';
    ...
    function decompress(string memory text) public view returns (string memory) {
        ...
        for(uint i = 0; i < count_delim;) {
            ...
            for (uint it=0; it < st2num(count.toString());) {
