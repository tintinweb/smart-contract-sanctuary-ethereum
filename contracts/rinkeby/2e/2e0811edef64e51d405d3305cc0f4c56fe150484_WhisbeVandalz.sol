 ...
 * @author
 */
contract WhisbeVandalz is ERC721, Ownable, ERC721RandomlyAssignVandalzTier, ERC721Holder, ERC721Burnable {
    //==Type declarations==//
        ...
        for (uint256 _j; _j < _collectionsLen; _j++) {
            ...
            for (uint256 _i; _i < _tokenIdsLen; _i++) {
                ...
                if (_collections[_j] == Constants.EXTINCTION_OPEN_EDITION_BY_WHISBE) {
                    ...
                    GoodKarmaToken(_collections[_j]).burnTokenForVandal(msg.sender);
                } else {
     ...
     * @dev
     */
    function creatorMint(address[] memory _tos, uint256[] memory _tokenIds) external onlyOwner {
        uint256 _toLen = _tos.length;
        uint256 _tokenIdsLen = _tokenIds.length;
        require(_toLen == _tokenIdsLen, "");
        for (uint256 _i; _i < _toLen; _i++) {
            _internalCreatorMint(_tos[_i], _tokenIds[_i]);
        }
    }
...
contract A721 is ERC721Enumerable, ERC721Burnable {
	constructor(
        string memory _name,
        string memory _symbol
...
library Constants {
    ...
    address public constant THE_HORNED_KARMA_CHAMELEON_BURN_REDEMPTION_BY_WHISBE =
        0x079800e1cf2076412D1792A199f3EaD223F96A8e;
