 ...
 * @notice It distributes hunter governance token tokens with rolling Merkle airdrops.
 */
contract TradingRewardsDistributor is Pausable, ReentrancyGuard, Ownable {
    ...
    // Total amount claimed by user (in HTGs)
    ...
    /**
     * @notice Constructor
     * @param hunterGovernanceToken_ address of the hunter governanceToken token
     */
    constructor(address hunterGovernanceToken_) {
        hunterGovernanceToken = IERC20(hunterGovernanceToken_);
        _pause();
    }
    ...
    function claim(uint256 amount, bytes32[] calldata merkleProof) external whenNotPaused nonReentrant {
        ...
        hunterGovernanceToken.safeTransfer(msg.sender, adjustedAmount);
    ...
    /**
     * @notice Transfer HTGs tokens back to owner
    ...
    function withdrawTokenRewards(uint256 amount) external onlyOwner whenPaused {
        ...
        hunterGovernanceToken.safeTransfer(msg.sender, amount);
