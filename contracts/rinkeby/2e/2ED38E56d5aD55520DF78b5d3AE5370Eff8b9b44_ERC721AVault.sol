...
contract ERC721AVault is Ownable, IERC721Receiver, PaymentSplitter {
    ...
    function updateConfig(uint256 _startId, uint256 _endId, uint256 _nextId) public onlyOwner {
        startId = _startId;
        endId = _endId;
        nextId = _nextId;
    }

    function buy(uint16 _quantity) public payable {
        require(saleIsActive, "Sale inactive");
        require(price * _quantity <= msg.value, "ETH incorrect");
        require(nextId + _quantity <= endId, "Insufficient supply");
