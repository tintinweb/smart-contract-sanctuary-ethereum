...
contract POCWoodtrack{
    ...
    event LogRegister(bytes32 indexed  shshhk);
    ...
    function AddSkshhk(Woodtrack calldata wt ) external onlyOwner returns(uint index) {
        ...
        require(isCertificateType(wt.Certificate),"Certificate not  Exist !") ;
        require(isDebarkType(wt.Debark),"Debark not  Exist !") ;
        require(isTranType(wt.Tran),"Tran not  Exist !") ;
