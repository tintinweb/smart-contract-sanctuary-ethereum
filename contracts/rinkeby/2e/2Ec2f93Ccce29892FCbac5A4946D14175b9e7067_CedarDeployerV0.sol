...
contract CedarDeployerV0 is Initializable, UUPSUpgradeable, OwnableUpgradeable, BaseCedarDeployerV0 {
    ...
    function initialize(CedarERC721PremintV0Factory _premintV0Factory) public initializer {
    ...
    function deployCedarERC721PremintV0(
        ...
        emit CedarDeployed(address(newContract), major, minor, patch, "ICedarERC721PremintV0");
        return newContract;
    }

    // Concrete implementation semantic version - provided for completeness but not designed to be the point of dispatch
    function minorVersion() external pure override returns (uint256 minor, uint256 patch) {
        minor = 0;
        patch = 0;
    }
}
...
abstract contract BaseCedarDeployerV0 is ICedarDeployerV0 {
    function implementationVersion() override external view returns (uint256 major, uint256 minor, uint256 patch) {
        major = 0;
        (minor, patch) = this.minorVersion();
    }
...
contract CedarERC721PremintV0Factory is Ownable, ICedarDeployerEventsV0 {
    ...
    constructor(address _greenlistManagerAddress) {
        ...
        CedarERC721PremintV0 implementation = new CedarERC721PremintV0();
    ...
    function deploy(
        ...
        newClone = CedarERC721PremintV0(Clones.clone(implementationAddress));
...
abstract contract Agreement is OwnableUpgradeable, ICedarAgreementV0 {
    string _userAgreement;
    bool _termsActivated;
    ...
    function __Agreement_init(string memory userAgreement_, address signatureVerifier_) internal {
        _userAgreement = userAgreement_;
        verifier = SignatureVerifier(signatureVerifier_);
    }
    ...
    function setTermsStatus(bool _status) external virtual override onlyOwner {
        _termsActivated = _status;
        emit TermsActive(_status);
    }

    function userAgreement() external view override returns (string memory) {
        return _userAgreement;
    }

    function termsActivated() external view override returns (bool) {
        return _termsActivated;
    }
    ...
    function acceptTerms() external virtual override {
        require(_termsActivated, "ERC721Cedar: terms not activated");
        termsAccepted[_msgSender()] = true;
        emit AcceptTerms(_userAgreement, _msgSender());
    }
    ...
    function storeTermsAccepted(address _acceptor, bytes calldata _signature) external virtual override onlyOwner {
        require(_termsActivated, "ERC721Cedar: terms not activated");
        ...
        emit AcceptTerms(_userAgreement, _acceptor);
    }
...
contract Greenlist is OwnableUpgradeable {
    using Address for address;
...
abstract contract BaseCedarERC721PremintV0 is ICedarERC721PremintV0 {
    function supportedFeatures() override external pure returns (string[] memory features) {
    ...
    function isICedarFeaturesV0() override external pure returns (bool) {
        return true;
    }
}
