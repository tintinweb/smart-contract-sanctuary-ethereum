...
contract Liquidator is LiquidatorBase, SuperAdminControl {
    ...
    function initialize(address _liquidator1, address _liquidator2)
        external
        initializer
    {
        __Ownable_init();
    ...
    function _liquidateCollateralAutoSellOn(
        ...
        for (uint256 i = 0; i < lengthCollaterals; i++) {
            ...
            (uint256 amountIn, ) = govPriceConsumer.getSwapData(
                path[0],
                loanDetails.stakedCollateralAmounts[i],
                path[1]
            );
    ...
    function _liquidateCollateralAutoSellOff(uint256 _loanId) internal {
        ...
        for (
            ...
            if (govClaimToken.isClaimToken(claimToken)) {
                //get price
                uint256 priceofSunCollateral = govPriceConsumer
                    .getSUNTokenPrice(
                        claimToken,
                        loanDetails.borrowStableCoin,
                        loanDetails.stakedCollateralTokens[i],
                ...
                uint256 priceofCollateral = govPriceConsumer
            ...
            if (
                ...
                if (govClaimToken.isClaimToken(claimToken)) {
                    ...
                    exceedAltcoinValue = govPriceConsumer
                        .getNumberofSunTokenFromStable(
                            loanDetails.borrowStableCoin,
                            claimToken,
                            loanDetails.stakedCollateralTokens[i],
                            collateralAmountinStable -
    ...
    */
    ...
    function liquidateLoan(uint256 _loanId, bytes[] calldata _swapData)
        external
        override
        onlyLiquidatorRole(msg.sender)
    {
        ...
        if (lenderDetails.autoSell) {
            ...
            _liquidateCollateralAutoSellOff(_loanId);
        }
    }
