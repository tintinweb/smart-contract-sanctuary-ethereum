...
contract nftescrow is IERC721Receiver {
    ...
    event LoanCreated(uint256 _loanId, address _nftAddress, uint256 _tokenId);
    event LoanStarted(uint256 _loanId, address _lenderAddress, uint256 _loanAmount);
        ...
        returns (Loan[] memory)
    {
        ...
        for (uint i = 0; i < _loanIds.current(); i++) {
            if (loans[i+1].borrowerAddress == _borrowerAddress) {
        ...
        returns (Loan[] memory)
    {
        ...
        uint count = 0;
        for (uint i = 0; i < _loanIds.current(); i++) {
            if (loans[i+1].lenderAddress == _lenderAddress) {
    ...
    function depositETH(uint256 _loanId) external payable {
        ...
        emit LoanStarted(_loanId, msg.sender, msg.value);
    }
    ...
    function _depositNFT(uint256 _loanId) private {
        ...
        emit LoanCreated(_loanId, nftAddress, tokenId);
    }
