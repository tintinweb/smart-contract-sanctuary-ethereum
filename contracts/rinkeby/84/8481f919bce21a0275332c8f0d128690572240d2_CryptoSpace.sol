...
contract CryptoSpace {
    ...
    mapping(uint => mapping (address => uint)) public waitingReferralReward;
    mapping(uint => mapping (uint => uint)) public waitingReferralRewardId;
    mapping(uint => uint) lastWaitingReferralRewardUserId;
    ...
    function _openLevel(uint level) internal {
        ...
        referralRewardsSendTime[level] = block.timestamp + 30;
    }

    function sendFreezedReferralRewards(uint level) external payable {
        ...
        if (numberOfPendingAddresses[level] > 0) {
            for (uint i = 1; i <= numberOfPendingAddresses[level]; i++) {
                countReferralRewards(pendingAddresses[level][i], level);
            }
            for (uint j = 1; j <= lastWaitingReferralRewardUserId[level]; j++) {
                address payable receiver = payable (idToAddress[waitingReferralRewardId[level][j]]);
    ...
    function countReferralRewards(address userAddress, uint level) private {
        address nextReferrer = userAddress;
        uint j = 1;
        while (j<= 7) {
        address payable _referrer = payable(users[nextReferrer].referrer);
        if (users[_referrer].levelWasActivated[level]) {
            ...
            if (waitingReferralReward[level][_referrer] == 0) {
                lastWaitingReferralRewardUserId[level]++;
                waitingReferralRewardId[level][lastWaitingReferralRewardUserId[level]] = users[_referrer].id;
            }
