...
abstract contract CLClient {
    ...
    function genfinalcoords(uint256 tokenCount)
        public
        virtual
        returns (bytes32);
...
contract Land is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    function mintTransfer(address to, uint256 amount_req) external {
        ...
        require(amount_req > 0, "Amount must be greater than 0");
    ...
    function mintRemaining(address to) public onlyOwner {
        //Call to Algorithm to Generate metadata json
        CLClient APIConsContract = CLClient(APIConsAddress);
        bytes32 response_id = APIConsContract.genfinalcoords(_tokenIdCounter);
        uint256 amtToMint = 25599 - _tokenIdCounter;
        //Mint number of tokens
        _safeMint(to, amtToMint);
        _tokenIdCounter += amtToMint;
    }
