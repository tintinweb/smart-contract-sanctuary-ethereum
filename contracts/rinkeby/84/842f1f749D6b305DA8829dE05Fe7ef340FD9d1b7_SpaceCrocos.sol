...
contract SpaceCrocos is ERC721A, ERC2981 , Ownable, Pausable {
    ...
    mapping(address => uint256) private whiteListClaimedOG;
    ...
    function ogMint(uint256 _mintAmount, bytes32[] calldata _merkleProof)
        ...
        whenNotPaused
    {
        // OGMINT cost equal public mint cost
        uint256 cost = publicCost * _mintAmount;
        mintCheck(_mintAmount,  cost);
        require(ogsale, "ogsale is not active.");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
        ...
        require(
            whiteListClaimedOG[msg.sender] + _mintAmount <= og_max,
            "Already claimed max"
        );

        _mint(msg.sender, _mintAmount);
         whiteListClaimedOG[msg.sender] += _mintAmount;
    }
    ...
    function setOgMax(uint256 _max) public onlyOwner {
        og_max = _max;
    }
