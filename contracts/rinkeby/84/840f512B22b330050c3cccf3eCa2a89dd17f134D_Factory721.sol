...
contract Vault721 is ERC721, Pausable {
    ...
    ) ERC721(string(abi.encodePacked("RentaFi Ownership NFT ", _name)), string(abi.encodePacked("RentaFi-ON-", _symbol))) {
    ...
    function redeem(uint256 _lockId) external virtual onlyMarket whenNotPaused {
        ...
        IERC721(originalCollection).transferFrom(address(this), _lend.lender, _lend.tokenId);
    }
    ...
    function burn(uint256 _lockId) external onlyMarket whenNotPaused {
        _burn(_lockId);
    }
