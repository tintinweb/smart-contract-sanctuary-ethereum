...
contract FayreNFTSwapper is OwnableUpgradeable, IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable {
    ...
    */
    ...
    mapping(address => uint256) public cardsMultiAssetSwapFee;
    ...
    function setCardMultiAssetSwapFee(address membershipCardsAddress, uint256 newCardMultiAssetSwapFee) external onlyOwner {
        cardsMultiAssetSwapFee[membershipCardsAddress] = newCardMultiAssetSwapFee;
    }
    ...
    function createSwap(SwapRequest calldata swapRequest) external payable {
        ...
        _processFee(msg.sender, swapRequest.sellerAssetData.length > 1 || swapRequest.bidderAssetData.length > 1);
    ...
    function finalizeSwap(uint256 swapId, bool rejectSwap) external payable {
        ...
        require(swapData.swapRequest.bidder == msg.sender, "E#12");

        _processFee(msg.sender, swapData.swapRequest.sellerAssetData.length > 1 || swapData.swapRequest.bidderAssetData.length > 1);
    ...
    function _processMembershipCards(address owner, bool isMultiAssetSwap, uint256 fee) private returns(uint256) {
        ...
        if (fee > 0) {
            for (uint256 i = 0; i < membershipCardsAddresses.length; i++) {
                uint256 cardMultiAssetSwapFee = cardsMultiAssetSwapFee[membershipCardsAddresses[i]];
