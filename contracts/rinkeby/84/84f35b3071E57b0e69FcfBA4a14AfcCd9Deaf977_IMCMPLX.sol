...
{
    // Specification
    uint256 public totalMaxQty = 1000;
    uint256 public preMintMaxQty = 10;
    uint256 public salesMaxQty = totalMaxQty - preMintMaxQty;
    uint256 public maxQtyPerMinter = 2;
    ...
    uint256 public preMintedQty = 0;

    constructor(
        string memory tokenUri_,
        string memory erc721Name_,
        string memory erc721Symbol_,
        uint256 totalMaxQty_,
        uint256 totalPreMintMaxQty_,
        uint256 maxQtyPerMinter_
        // uint256 price_
    ) ERC721(erc721Name_, erc721Symbol_) {
        totalMaxQty = totalMaxQty_;
        preMintMaxQty = totalPreMintMaxQty_;
        maxQtyPerMinter = maxQtyPerMinter_;
        // price = ethers.utils.parseEther(price_);
        _tokenBaseURI = tokenUri_;
    }

    function setTotalMaxQty(uint256 totalMaxQty_) external onlyOwner {
        totalMaxQty = totalMaxQty_;
    }

    function setPreMinMaxQty(uint256 totalPreMintMaxQty_) external onlyOwner {
        preMintMaxQty = totalPreMintMaxQty_;
    }

    function setMaxQtyPerMinter(uint256 maxQtyPerMinter_) external onlyOwner {
        maxQtyPerMinter = maxQtyPerMinter_;
    }
    ...
    function preMint() public onlyOwner {
        require(preMintedQty < preMintMaxQty, "Exceed sales max limit");
        for (uint256 i = 0; i < preMintMaxQty; i++) {
            _mintSingleNFT();
        }
    }

    function publicSalesMint() external payable isPublicSalesActive {
        require(publicSalesMintedQty < salesMaxQty, "Exceed sales max limit");
        require(
            publicSalesMinterToTokenQty[msg.sender] < maxQtyPerMinter,
