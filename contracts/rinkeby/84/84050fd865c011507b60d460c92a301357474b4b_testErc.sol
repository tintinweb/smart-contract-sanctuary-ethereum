...
contract testErc is ERC20("TestErc", "ERC"), Ownable {
    ...
    uint256 public rewardPercentageForThreeMonths;
    uint256 public rewardPercentageForSixMonths;
    uint256 public rewardPercentageForTwelveMonths;
    ...
    function claimReward() public returns (uint256) {
        ...
        require (stake[msg.sender].returned == false, "Already claimed");
        uint256 unclaimedReward = computeUnclaimedReward(msg.sender);

        if(stake[msg.sender].otherUserReferral != 0){
            uint256 referralUserPercentage = (refeeRewardPercentage * unclaimedReward) / 100;
            _mint(referralUser[stake[msg.sender].otherUserReferral], referralUserPercentage * 10 ** decimals());
        }
        stake[msg.sender].returned = true;
        // mint the tokens and distribute to msg.sender
        _mint(msg.sender, unclaimedReward * 10 ** decimals());
        emit RewardPaid(msg.sender, unclaimedReward * 10 ** decimals());

        return unclaimedReward * 10 ** decimals();
    }
    ...
    function getUnclaimedRewardAmount(address user) public view returns (uint256) {
        return computeUnclaimedReward(user);
    }
