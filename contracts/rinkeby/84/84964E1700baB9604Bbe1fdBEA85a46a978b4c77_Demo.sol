...
contract Demo is ERC721, IERC2981, Ownable, ReentrancyGuard {
    ...
    function withdraw() public onlyOwner {
        ...
        tokensForPrimary = calculateFee(balance, 85);
        remainder -= tokensForPrimary;

        tokensForSecondary = calculateFee(balance, 13);
        remainder -= tokensForSecondary;

        payable(primaryAddress).transfer(tokensForPrimary);
        payable(secondaryAddress).transfer(tokensForSecondary);
        payable(tertiaryAddress).transfer(remainder);
    }
