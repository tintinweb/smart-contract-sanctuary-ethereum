...
contract Parallel is ERC1155, Ownable, ERC2981ContractWideRoyalties, Pausable {
  ...
  uint private constant GREENLIST_SUPPLY = 111;         /// @notice Registration through offline signatures
  uint private constant PUBLIC_SALE_SUPPLY = 778;       /// @notice For everyone at drop
  ...
  uint private constant FIRST_PUBLIC_SALE_ID = 334;
  ...
  function founderMint(address _to) external onlyOwner {
  ...
  function greenlistMint(bytes memory _signature) external whenNotPaused {
  ...
  /// @notice Grant the owner to mint any token and transfer it to any address. In case of something goes wrong with other mint methods.
  function emergencyMint(address _to, uint _tokenId) external onlyOwner {
    _mint(_to, _tokenId, 1, "");
  }

  /// @notice Grant the owner the ability to burn any token from any wallet.
  function emergencyBurn(address _target, uint _tokenId) external onlyOwner {
      _burn(_target, _tokenId, 1);
  }
