    ...
    Ownable
{
    ...
    uint256 public _mintcount = 4680;

    mapping(address => uint256[]) private _ownerToTokenIds;

    address public withdrawable;
    ...
    function uintToString(uint256 v)
        internal
        pure
        returns (string memory test)
    {
        return Strings.toString(v);
    }

    function getTokenIdsByAddress(address _addr)
        public
        view
        returns (uint256[] memory)
    {
        return _ownerToTokenIds[_addr];
    }

    constructor() ERC721("Winged Hat Cosmo Friends", "WHCF") {
    }
        ...
        public
    {
        for (uint256 i = 0; i < _mintcount; i++) {
            ...
            string memory _tokenURI = string(abi.encodePacked("ipfs://QmdC8xZDnWTbGNqXjzNfdAkAJPG4wKB9cd5yjZDKzHKzQh/",uintToString(newItemId), ".json"));
