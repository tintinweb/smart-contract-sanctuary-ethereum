...
contract TAVestingMock is KeeperCompatibleInterface, ReentrancyGuard {
  /*==================================================== State Variables ====================================================*/
  VestingMock vesting;
  ...
  function checkUpkeep(
    ...
    address[] memory _users = vesting.getUsers(0);
    bool outBreak = false;
    for (uint256 i = 0; i < _users.length; i++) {
      uint256[] memory _indexes = vesting.getInvestorIndexes(_users[i], 0);
      for (uint256 k = 0; k < _indexes.length; k++) {
        uint256 withdrawableAmount = vesting.withdrawableTokens(_users[i],vesting.intToEnum(0),_indexes[k]);
  ...
  function performUpkeep(bytes calldata performData) external override nonReentrant {
    ...
    vesting.withdrawTokensAutomatically(_user, vesting.intToEnum(0), _index);
  }
}
