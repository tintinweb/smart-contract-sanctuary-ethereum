...
contract Hacktivists is ERC721A, Ownable{
    ...
    uint256 public constant maxTeamTotalSupply = 2;
    ...
    function teamMint(bytes32[] memory _merkleProof, uint256 _quantity) external payable callerIsUser{
        ...
        require((totalTeamMint[msg.sender] + _quantity)  <= MAX_TEAM_MINT , "Cannot mint beyond whitelist max mint!");
        require(msg.value >= (TEAM_SALE_PRICE * _quantity), "Payment is below the price!");
        require(curTeamTotalSupply + _quantity <= maxTeamTotalSupply, "Not enough whitelist mints available!");
        //Check Merkle Proof
        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, sender), "You are not whitelisted!");
