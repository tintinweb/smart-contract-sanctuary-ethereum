...
contract MagicDOGE is ERC20, Ownable {
    ...
    mapping(address => uint256) public numberOfTokenOnStake;    
    ...
    uint256 public swapTokensAtAmount = 1 * 1 ** 9 * (10**9);
    ...
    function setNftBoosterAddress(address _addr) public onlyOwner{
	    iNftBooster = NftBooster(_addr);
	}

    function stakeToken(address user, uint256 amount) external {
        require(msg.sender == address(iNftBooster));
        numberOfTokenOnStake[user] = amount;
    }
    ...
    function _transfer(
        ...
        uint256 allowedTokenToTransfer = balanceOf(msg.sender).sub(numberOfTokenOnStake[msg.sender]);
        require(amount <= allowedTokenToTransfer, "Token are on stake. you can't have enough to transfer");
...
contract MagicDOGEDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("MagicDOGE_Dividend_Tracker", "MagicDOGE_Dividend_Tracker", 9) {
        ...
        minimumTokenBalanceForDividends = 1 * 1 ** 9 * (10**9); //must hold 10,000,000,000+ tokens to get divs
    }
