...
library svgComponents {
    ...
    function eggGen(uint256 _tokenId, string[5] memory colors, uint8[5] memory choices) external pure returns (string memory eggURI) {
        ...
        eggURI = getJson(finalSVG, _tokenId, colors, atrs);
        delete atrs;
    }
    ...
    function getJson(string memory _eggURI, uint256 _tokenId, string[5] memory colors, bytes32[5] memory _atts) private pure returns (string memory) { 
        string memory cols = string(abi.encodePacked(colors[0],colors[1],colors[2],colors[3],colors[4]));
