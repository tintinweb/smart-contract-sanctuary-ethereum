...
contract AndThatsOkay is ERC721A, Ownable, ReentrancyGuard {
    string public baseURI;
    string public baseExtension = "";
    string public notRevealedUri;
    uint256 public cost = 0.0042 ether;
    uint256 public maxSupply = 4200;
    uint256 public wlSupply = 400;
    ...
    function mint(uint256 tokens) public payable nonReentrant {
        ...
        require(publicSale, "ATO: Sale hasnt started yet");
        require(tokens <= maxPerWallet, "ATO: max mint amount per tx exceeded");
        require(
            totalSupply() + Totalburned() + tokens <= maxSupply,
            "ATO: We Soldout"
        );
        // require(
        //     _numberMinted(_msgSenderERC721A()) + tokens <= maxPerWallet,
        //     "ATO: Max NFT Per Wallet exceeded"
        // );
        require(msg.value >= cost * tokens, "ATO: insufficient funds");
    ...
    {
        require(!paused, "ATO: oops contract is paused");
        require(preSale, "ATO: Presale HATO't started yet");
        require(
            MerkleProof.verify(
                merkleProof,
                merkleRoot,
            ...
            "ATO: You are not Whitelisted"
        );
        require(
            _numberMinted(_msgSenderERC721A()) + tokens <= maxPerWalletWl,
            "ATO: Max NFT Per Wallet exceeded"
        );
        require(tokens <= maxPerWalletWl, "ATO: max mint per Tx exceeded");
        require(
            totalSupply() + Totalburned() + tokens <= wlSupply,
            "ATO: Whitelist MaxSupply exceeded"
        );
    ...
    {
        require(
            totalSupply() + Totalburned() + _mintAmount <= maxSupply,
    ...
    /// @notice return the number burned by an address
    function numberBurned(address owner) public view returns (uint256) {
        return _numberBurned(owner);
    }

    /// @notice return the total number burned
