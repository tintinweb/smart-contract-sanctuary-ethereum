...
import "./SlotTraits.sol";

contract Slots is ERC721Enumerable, Ownable, ReentrancyGuard {
  using ECDSA for bytes32;

  uint256 PRICE_PER_TOKEN = 0.05 ether;
  ...
  constructor(
    ...
    BASE_URI = baseUri;
    traitStorage = SlotTraits(traits);
  }
  ...
  function genTraits(uint256 tokenId) public view returns (Traits memory) {
    ...
    Traits memory traits = Traits({
      slotOne: uint256(keccak256(abi.encode(seed, 1))),
      slotTwo: uint256(keccak256(abi.encode(seed, 2))),
      slotThree: uint256(keccak256(abi.encode(seed, 3)))
    });

    return
      Traits({
        slotOne: traits.slotOne % traitStorage.getSlotsLength(),
        slotTwo: traits.slotTwo % traitStorage.getSlotsLength(),
        slotThree: traits.slotThree % traitStorage.getSlotsLength()
      });
  }
  ...
  function genSvg(uint256 tokenId) public view returns (string memory) {
    ...
    parts[0] = string(
      abi.encodePacked(
        ...
        _baseURI(),
        traitStorage.getSlots()[traits.slotOne][1],
        '" />'
      )
    );
    ...
    parts[1] = string(
      abi.encodePacked(
        '<g transform=""><image width="100%" height="100%" href="',
        _baseURI(),
        traitStorage.getSlots()[traits.slotTwo][1],
        '" />'
      )
    );
    ...
    parts[2] = string(
      abi.encodePacked(
        '<g transform=""><image width="100%" height="100%" href="',
        _baseURI(),
        traitStorage.getSlots()[traits.slotThree][1],
        '" />'
      )
    );
    ...
    returns (string memory)
  {
    ...
    return
      string(
        abi.encodePacked(
          '"attributes": [',
          '{"trait_type":"Slots0","value":"',
          traitStorage.getSlots()[traits.slotOne][0],
          '"},',
          '{"trait_type":"slotOne","value":"',
          traitStorage.getSlots()[traits.slotTwo][0],
          '"},',
          '{"trait_type":"slotTwo","value":"',
          traitStorage.getSlots()[traits.slotThree][0],
          '"}'
        )
      );
  }
