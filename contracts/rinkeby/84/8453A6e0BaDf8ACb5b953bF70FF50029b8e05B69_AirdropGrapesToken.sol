...
contract AirdropGrapesToken is Ownable, Pausable {
    ...
    constructor(
        ...
        require(
            _grapesTokenAddress != address(0),
            "The Grapes token address can't be 0"
        );
        require(
            _alphaContractAddress != address(0),
            "The Alpha contract address can't be 0"
        );
        require(
        ...
        require(
            _gammaContractAddress != address(0),
            "The Gamma contract address can't be 0"
        );
    ...
    function startClaimablePeriod(uint256 _claimDuration)
        external
        onlyOwner
        whenPaused
    {
    ...
    function claimTokens() external whenNotPaused {
        require(
            ...
            "Claimable period is finished"
        );
        require(
            (beta.balanceOf(msg.sender) > 0 || alpha.balanceOf(msg.sender) > 0),
            "Nothing to claim"
        );
        ...
        (
            tokensToClaim,
            gammaToBeClaim
        ) = getClaimableTokenAmountAndGammaToClaim(msg.sender);
        ...
        for (uint256 i; i < gamma.balanceOf(msg.sender); ++i) {
            ...
            if (
                !gammaClaimed[tokenId] && currentGammaClaimed < gammaToBeClaim
            ) {
    ...
    function getClaimableTokenAmountAndGammaToClaim(address _account)
        private
        view
        returns (uint256, uint256)
    {
        ...
        uint256 gammaToBeClaim = min(
            unclaimedAlphaBalance + unclaimedBetaBalance,
            unclaimedGamaBalance
        );
        uint256 tokensAmount = (unclaimedAlphaBalance *
            ALPHA_DISTRIBUTION_AMOUNT) +
            (unclaimedBetaBalance * BETA_DISTRIBUTION_AMOUNT) +
            (gammaToBeClaim * GAMMA_DISTRIBUTION_AMOUNT);
    ...
    function claimUnclaimedTokens() external onlyOwner {
        require(
            block.timestamp > claimStartTime + claimDuration,
            "Claimable period is not finished yet"
        );
