    ...
    IERC721ReceiverUpgradeable
{
    ...
    // NOTE: these hashes are derived with keccak256 hashing and verified in the constructor.
    //Contract Name hash
    ...
    //objId => auction Mapping
    mapping(string => Auction) auctions;

    //Bid increment time in seconds
    uint256 updateClosingTime;
    ...
    function bidding(Order calldata order, uint256 amount)
        ...
        onlyApproved(order.contractAddress, order.tokenId, order.seller)
    {
        require(order.expirationTime > block.timestamp, "auction ended");
        (, uint256 amt) = _validate(order, amount);

        IToken token = IToken(order.contractAddress);
        Auction memory _auction = auctions[order.objId];
        Order calldata _order = order;
        ...
        auctions[_order.objId] = _auction;
    ...
    function claim(Order calldata order) external whenNotPaused nonReentrant isNotBlacklisted(msg.sender) {
        ...
        Auction memory auction = auctions[order.objId];
        require(verifySeller(order) || msg.sender == auction.highestBidder, "not a valid caller");
        ...
        settlement(nftContract, _tokenId, auction.currentBid, auction.highestBidder, _order);
        emit Claimed(
            _order.contractAddress,
            _tokenId,
    ...
    function _validate(Order calldata order, uint256 amount) internal returns (address, uint256) {
        ...
        require(validateOrder(order, paid), "Invalid Order");
        return (signer, paid);
    }
    ...
    function _chargeAndSplit(
        ...
        if (_erc20Token == address(0)) {
            ...
            require(weth.allowance(user, address(this)) >= _amount, "charge:insufficient spent allowance");
     ...
     */
    function validateOrder(Order calldata order, uint256 amount) internal view returns (bool validated) {
        ...
        if (cancelledOrFinalized[digest]) {
            return false;
        }
