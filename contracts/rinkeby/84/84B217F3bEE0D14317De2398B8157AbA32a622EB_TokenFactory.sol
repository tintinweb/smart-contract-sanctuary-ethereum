...
contract TokenFactory is AccessControl{
    ...
    event NewToken(address tokenAddress, address owner, string symbol);
    ...
    function createFor(address owner, string memory _name, string memory _symbol) public returns(address created){
        if (owners[owner] != address(0)) {
            return owners[owner];
        }
        address ret = address(new TestErc20(
            owner,
            _name,
            _symbol
        ));
        owners[owner] = ret;
        emit NewToken(ret, owner, _symbol);
        return ret;
    }
