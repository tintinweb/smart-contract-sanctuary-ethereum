...
contract TestNFT is ERC721Enumerable, Ownable {
    ...
    // external
    function mint(uint256 count) external payable checkLimit(count) checkPrice(count) whenMintActive {
        uint256 _totalSupply = _owners.length;
        for(uint i; i < count; i++) { 
            _mint(msg.sender, _totalSupply + i + 1);
        }
    }
    ...
    function gift(uint[] calldata quantity, address[] calldata recipient) external onlyOwner{
        ...
        require(_totalSupply + totalQuantity + 1 <= maxSupply, "Not enough supply!" );
        for(uint i = 0; i < recipient.length; ++i){
            for(uint j = 0; j < quantity[i]; ++j){
                _mint(recipient[i], _totalSupply + 1);
                _totalSupply++;
            }
        }
    }
