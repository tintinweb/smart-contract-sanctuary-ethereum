...
contract Lottery is ERC1155, Ownable {
  ...
  function mintRandom(address _to,  uint _amount) public {
      (uint zeros,uint ones) = randModulus(_amount);
        _mint(_to,1,zeros,"");
        _mint(_to,2,ones,"");
      
  }

  function randModulus(uint times) public view returns( uint ,uint ){
        uint zeros = 0;
        uint ones = 0;
        for(uint i = 0; i < times; i++){
        if ((uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,msg.sender,i))) % 2 )==0){
            zeros++;
            }
        else {
            ones++;
