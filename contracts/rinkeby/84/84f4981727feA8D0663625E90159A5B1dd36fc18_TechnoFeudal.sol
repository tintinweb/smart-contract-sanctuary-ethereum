...
contract TechnoFeudal is ERC721, IERC2981, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MINT_LIST_PRICE = 0 ether;
    ...
    constructor(address _royaltyReceiverAddress)
        ERC721("Techno-Feudal Citizens-v", "TFCv")
    {
    ...
    modifier isValidMintListAddress(bytes32[] calldata merkleProof) {
        require(
            MerkleProof.verify(
                merkleProof,
                preSaleListMerkleRoot,
    ...
    function mintMintList(uint8 numberOfTokens, bytes32[] calldata merkleProof)
        ...
        isCorrectPayment(PRE_SALE_PRICE, numberOfTokens)
        canMint(numberOfTokens)
        isValidMintListAddress(merkleProof)
    {
    ...
    function setMintListMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        mintListMerkleRoot = merkleRoot;
    }
    /**
