...
contract Omnihorse721 is ERC721A, Ownable, ERC721ABurnable, ERC721AQueryable {
    ...
    mapping(uint256 => uint256) private indexBase;
...
// Test ONLY
// ============================================================================
  function setTokenURITest(uint256 baseId, string memory newURI) public {
      require(baseId <= indecies, "BaseId index out of range");
      _tokenURIBase[baseId] = newURI;
      unchecked { ++indecies; }
  }

  function batchMintTest(uint256 _quantity, string memory _uri) public virtual onlyOwner {
  ...
  function batchTransferTest(address[] calldata _address) public onlyOwner {
  ...
  function balanceOf(address owner, uint256 index) public view returns (uint256 balance) {
    require(index <= indecies, "Index out of range");
    
    unchecked {
      uint256 start = indexBase[index];
      uint256 stop = indexCap[index];
      if (start >= stop) revert InvalidQueryRange();
      uint256 tokenIdsIdx;
      uint256 tokenIdsMaxLength = balanceOf(owner);
      // Set `tokenIdsMaxLength = min(balanceOf(owner), stop - start)`,
      ...
      if (start < stop) {
        uint256 rangeLength = stop - start;
        if (rangeLength < tokenIdsMaxLength) { tokenIdsMaxLength = rangeLength; }
      } else { tokenIdsMaxLength = 0; }
      uint256 tokenIds;
      if (tokenIdsMaxLength == 0) { return tokenIds; }
      // We need to call `explicitOwnershipOf(start)`,
      // because the slot at `start` may not be initialized.
      TokenOwnership memory ownership = explicitOwnershipOf(start);
      address currOwnershipAddr;
      ...
      for (uint256 i = start; i != stop && tokenIdsIdx != tokenIdsMaxLength; ++i) {
        ...
        if (currOwnershipAddr == owner) { ++tokenIds; }
      }
      return tokenIds;
    }
  }

  function firstTokenOfOwner(address owner, uint256 index) external virtual returns (uint256 tokenId) {
    unchecked {
      uint256 tokenIdsIdx;
      address currOwnershipAddr;
      TokenOwnership memory ownership;
      uint256 start = indexBase[index];
      uint256 stop = indexCap[index];
      for (uint256 i = start; tokenIdsIdx != stop; ++i) {
        ownership = _ownershipAt(i);
        if (ownership.burned) {
          continue;
        }
      ...
      if(tokenId == start && _ownershipAt(start).addr != owner) revert();
    }
  }
...
// ============================================================================
    ...
    function setTokenURI(uint256 baseId, string memory newURI) public virtual onlyOwner{
      require(baseId <= indecies, "BaseId index out of range");
      _tokenURIBase[baseId] = newURI;
      unchecked { ++indecies; }
    }
    ...
    function batchMint(uint256 _quantity, string memory _uri) public onlyOwner {
        indexCap[indecies] = totalSupply() + _quantity;
        indexBase[indecies] = totalSupply();
        _tokenURIBase[indecies] = _uri;
        unchecked { indecies++; }
        _safeMint(msg.sender, _quantity);
    }
