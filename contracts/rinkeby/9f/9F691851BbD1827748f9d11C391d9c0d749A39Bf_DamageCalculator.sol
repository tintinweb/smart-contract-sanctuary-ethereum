...
contract DamageCalculator is IDamageCalculator {
    ...
    function getDamageComponents(
        ...
        idx = _getEquipmentComponents(components, equipmentIds, idx);
    ...
    // Returns a fighter damage component
    function getEquipmentDamageComponent(uint256 equipment)
        external
        view
        override
        returns (Damage.DamageComponent memory)
    {
        return _getEquipmentDamageComponent(equipment);
    }

    // Computes damage per block given elements of a party, intended for off-chain reads
    function getDamage(
        uint256 hero,
        uint256[] calldata fighters,
        uint256[] calldata equipment
    ) external view override returns (uint32) {
        if (hero == 0 && _hero.ownerOf(hero) != address(0)) {
            return 0;
        }
        ...
        Stats.HeroStats memory hStats = IHeroURIHandler(_hero.getHandler())
            .getStats(hero);
        // Load base hero damage multiplier
        m = hStats.dmgMultiplier;
        // Apply enhancements to damage, multiply by 10 to match base multi precision
        m += hStats.enhancement * 2;

        // Apply fighter damage
        for (uint256 i = 0; i < hStats.partySize; i++) {
            if (fighters[i] != 0) {
                ...
                f +=
                    (fStats.dmg *
                        (PRECISION + ((fStats.enhancement * PRECISION) / 5))) /
                    PRECISION;
            }
        }

        // Apply equipment damage and multi
        for (uint256 i = 0; i < equipment.length; i++) {
            if (equipment[i] != 0) {
                Stats.EquipmentStats memory eStats = IEquipmentURIHandler(
                    _equipment.getHandler()
                ).getStats(equipment[i]);
                d += eStats.dmg;
                m += eStats.dmgMultiplier;
            }
        }

        return uint32(m * (2500 + f + d));
    }
    ...
    function _getEquipmentDamageComponent(uint256 id)
        internal
        view
        returns (Damage.DamageComponent memory)
    {
        ...
        return Damage.DamageComponent(eStats.dmgMultiplier, eStats.dmg);
    }
    ...
    // Helper to reduce stack depth
    function _getLength(
        uint256 heroes,
        uint256 fighters,
        uint256 equipment
    ) internal pure returns (uint256) {
        return heroes + fighters + equipment;
    }
    ...
    function _getEquipmentComponents(
        Damage.DamageComponent[] memory components,
        uint256[] memory equipment,
        uint256 idx
    ) internal view returns (uint256) {
        for (uint256 i = 0; i < equipment.length; i++) {
            components[idx] = _getEquipmentDamageComponent(equipment[i]);
            idx += 1;
        }
        ...
        returns (uint32)
    {
        if (enhancement == 0) {
            ...
            return (111 * damage) / 100;
        } else if (enhancement == 2) {
            return (176 * damage) / 100;
        } else if (enhancement == 3) {
            return (222 * damage) / 100;
        } else if (enhancement == 4) {
            return (258 * damage) / 100;
        } else if (enhancement == 5) {
            return (287 * damage) / 100;
        } else if (enhancement == 6) {
            return (311 * damage) / 100;
        } else if (enhancement == 7) {
            return (333 * damage) / 100;
        } else if (enhancement == 8) {
            return (352 * damage) / 100;
        } else if (enhancement == 9) {
            return (368 * damage) / 100;
        } else if (enhancement == 10) {
            return (384 * damage) / 100;
        } else if (enhancement == 11) {
            return (398 * damage) / 100;
        } else if (enhancement == 12) {
            return (410 * damage) / 100;
        } else if (enhancement == 13) {
            return (422 * damage) / 100;
        } else if (enhancement == 14) {
            return (433 * damage) / 100;
        }
...
interface IParty {
    ...
    function act(
        ...
        Action[] calldata equipmentActions
    ) external;
    ...
    function getUserEquipment(address user)
        external
        view
        returns (uint256[] memory);
