...
contract Staker is Ownable, IERC721Receiver {
    ...
    mapping(address => uint256[]) private _qty;
    bool public paused = false;
    uint256 nullToken = 1 ether;
    uint256 tokensHour = 1;
    IERC721 public NFT;
    FXY public TOKEN;
    ...
    function setTokenshour(uint256 new_) external onlyOwner {
        tokensHour = new_;
    }
    ...
    function getAssetsByHolder(address holder)
        public
        view
    ...
    function getProfits(uint256 tokenId) public view returns (uint256) {
        if (stakeTime[tokenId] == 0 || tokenId == nullToken) {
            return 0;
        }
        uint256 lWithdraw = stakeTime[tokenId] > lastWithdraw[_msgSender()]
            ? stakeTime[tokenId]
            : lastWithdraw[_msgSender()];
        uint256 sTime = (block.timestamp - lWithdraw) / 1 hours;
        return sTime * tokensHour;
    }

    function getAllProfits() public view returns (uint256) {
        uint256[] memory tokenIds = getAssetsByHolder(_msgSender());
        uint256 profits = 0;
    ...
    function stake(uint256[] calldata tokenIds) public notPaused {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
    ...
    function recover(uint256 tokenId) internal {
    ...
    function removeToken(uint256 tokenId) internal {
        for (uint256 i = 0; i < _qty[_msgSender()].length; i++) {
...
contract FXY is ERC20, Administration {
    ...
    function mintTokens(uint256 amount) public onlyAdmin {
        _mint(owner(), amount);
    }
    ...
    function buy(address from, uint256 amount) external onlyAdmin {
        _burn(from, amount);
    }
