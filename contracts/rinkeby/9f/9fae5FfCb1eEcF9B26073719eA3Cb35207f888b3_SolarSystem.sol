...
contract SolarSystem is ERC721Tradable {
   ...
   function decompress(strings.slice memory text) public view returns (string memory) {
       string memory HTML = "";
       uint count = text.count("|".toSlice());
       for(uint i = 0; i < count;i++) {
            strings.slice memory mapping_tot = text.split("|".toSlice());
            HTML = parts[mapping_tot.toString()].toSlice().len() > 0 ? HTML.toSlice().concat(parts[mapping_tot.toString()].toSlice()) : HTML.toSlice().concat(mapping_tot);
       }
       return HTML;
   }
