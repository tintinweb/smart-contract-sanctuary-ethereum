...
import "../../../common/TransferToken.sol";
import "../../../common/CommonErrors.sol";

/**
  * A Tranch Registry which can create new tranches, and perform some operations on the tranches
  * it has created and initialized.
  * 
  * Each new tranche is cloned from a template implementation which is statically deployed
...
contract TrancheRegistry is ITrancheRegistry, Ownable, Operators {
    ...
    ImplementationDetails[] private _implDetails;

    mapping(uint256 => address[]) private _trancheList; // implId -> tranche array
        
    mapping(address => bool) private _allTranches; // tranche address -> was created from this registry
    ...
    function trancheList(uint256 _implId) external view  onlyValidImplId(_implId) returns (address[] memory tranches) {
        return _trancheList[_implId];
    }

    function createdTranche(address trancheAddress) external  view returns (bool) {
        return _allTranches[trancheAddress];
    }

    // Get number of implementations
    function implDetailsLength() external view returns (uint256) {
        return _implDetails.length;
    }

    function disableImpl(uint256 _implId, bool _value) external  onlyOperators onlyValidImplId(_implId) {
        _implDetails[_implId].disabled = _value;
        emit ImplementationDisabled(_implId, _value);
    }

    function disableTranche(address _tranche, bool _isDisabled) external onlyOperators {
    ...
    function addTrancheImpl(address _implementation) external  onlyOperators {
        if (_implementation == address(0)) revert CommonErrors.InvalidAddress(_implementation);
        ...
        emit TrancheImplCreated(_implDetails.length-1, _implementation);
    }
    ...
    function createTranche(uint256 _implId) external override onlyOperators onlyValidImplId(_implId) returns (address tranche, address stakingAddress, address stakingToken)  {       
        ...
        _trancheList[_implId].push(tranche);
        _allTranches[tranche] = true;
    ...
    // In case of registry upgrade - owner of the new registry can add existing tranches
    ...
    function addExistingTranche(uint256 _implId, address _tranche) external onlyOwner onlyValidImplId(_implId) {
        if (_tranche == address(0)) revert CommonErrors.InvalidAddress(_tranche);
        if (_allTranches[_tranche]) revert TrancheAlreadyExists(_tranche);
        if (ITranche(_tranche).disabled()) revert ITranche.InactiveTranche(_tranche);

        _trancheList[_implId].push(_tranche);
        _allTranches[_tranche] = true;
    ...
    function recoverToken(address _token, address _to, uint256 _amount) external onlyOwner {
        TransferToken.transferTo(IERC20(_token), _to, _amount);
    }
    ...
    /**
      ...
      *
      * Care needs to be taken on new tranche implementations such that it doesn't abuse this access.
      * However these are all STAX contracts only.
      */
    function execute(
        address _to,
        uint256 _value,
        ...
        if (msg.sender != owner() && !operators[msg.sender] && !_allTranches[msg.sender])
    ...
    modifier onlyValidImplId(uint256 _implId) {
        if (_implId >= _implDetails.length || _implDetails[_implId].implementation == address(0)) revert InvalidTrancheImpl(_implId);
        _;
    }
}
...
abstract contract Operators {
    /// @dev A set of addresses which are approved to run operations.
...
library Executable {
    function execute(
        ...
        // On failure, bubble up any underlying revert message and re-revert
        if (!success) {
            assembly {
                let ptr := mload(0x40)
...
library TransferToken {
    ...
    function transferTo(IERC20 _token, address _to, uint256 _amount) internal {
        uint256 balance = _token.balanceOf(address(this));
        if (balance < _amount) revert CommonErrors.InsufficientTokens(address(_token), _amount, balance);

        _token.safeTransfer(_to, _amount);
        emit TokenTransferred(address(this), _to, address(_token), _amount);
    }
}

pragma solidity ^0.8.4;
// SPDX-License-Identifier: GPL-3.0-or-later
// STAX (common/CommonErrors.sol)
