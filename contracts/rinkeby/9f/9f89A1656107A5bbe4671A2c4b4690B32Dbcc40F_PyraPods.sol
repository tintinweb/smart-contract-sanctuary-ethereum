    ...
    AccessControlEnumerable
{
    ...
    IERC721 public immutable hooks;
    ...
    uint256 public constant MAX_PYRA_PODS = 10001;
    ...
    constructor(
        ...
        FixedPriceSeller(
            ...
            Seller.SellerConfig({
                totalInventory: MAX_PYRA_PODS - hooksPoolRemaining,
    ...
    function _handlePurchase(
        ...
        // We're using two separate pools (one from Seller, and one for HOOKS
    ...
    function mintWhitelist(
        ...
        require(whitelistMintingOpen, "Pyrapods: Whitelist minting closed");
    ...
    /**
    @notice One guaranteed mints per HOOKS holder.
    @dev This is specifically tracked because unclaimed tokens will be minted to
    the HOOKS wallet, so the pool guarantees an upper bound.
     */
    uint256 public hooksPoolRemaining = 2000;

    ERC721Redeemer.SingleClaims private redeemedHOOKS;

    /**
    @dev Used by both HOOKS-holder and HOOKS-admin minting from the pool.
     */
    modifier reduceHOOKSPool(uint256 n) {
        require(n <= hooksPoolRemaining, "Pyrapods: HOOKS pool exhausted");
        hooksPoolRemaining -= n;
        _;
    }

    /**
    @notice Flag indicating whether holders of HOOKS passes can mint.
     */
    bool public hooksMintingOpen = false;

    /**
    @notice Sets whether holders of HOOKS passes can mint.
     */
    function setHOOKSMintingOpen(bool open) external onlyOwner {
        hooksMintingOpen = open;
    }

    /**
    @notice Tracks the amount of tokens minted with HOOKS passes.
     */
    uint256 internal mintedWithHooks = 0;

    /**
    @notice Mint as a holder of a HOOKS token.
     */
    function mintHOOKS(uint256[] calldata hooksTokenIds)
        external
        payable
        reduceHOOKSPool(hooksTokenIds.length)
    {
        require(hooksMintingOpen, "Pyrapods: HOOKS minting closed");
        uint256 n = ERC721Redeemer.redeem(redeemedHOOKS, msg.sender, hooks, hooksTokenIds);
        mintedWithHooks += n;
    ...
    function mintPublic(
        ...
        require(publicMintingOpen, "Pyrapods: Public minting closed");
        _purchase(to, n);
    }

    function toggleToHooksMinting() external onlyOwner {
        whitelistMintingOpen = false;
        hooksMintingOpen = true;
    }

    function toggleToPublicMinting() external onlyOwner {
        hooksMintingOpen = false;
        publicMintingOpen = true;
        uint256 newTotalInventory = MAX_PYRA_PODS - mintedWithHooks;
        setPrice(0.05 ether);
    ...
    */
        ...
        onlyApprovedOrOwner(tokenId)
    {
        uint256 start = hibernatingStarted[tokenId];
        if (start == 0) {
            require(hibernatingOpen, "Pyrapods: hibernating closed");
    ...
    /**
    @notice Changes the Pyrapods' hibernating statuss (what's the plural of status?
    statii? statuses? status? The plural of sheep is sheep; maybe it's also the
    plural of status).
    @dev Changes the Pyrapods' hibernating sheep (see @notice).
     */
    ...
    function expelFromHibernate(uint256 tokenId) external onlyRole(EXPULSION_ROLE) {
        require(hibernatingStarted[tokenId] != 0, "Pyrapods: not hibernated");
        hibernatingTotal[tokenId] = 0;
        hibernatingStarted[tokenId] = 0;
        emit Unhibernated(tokenId);
