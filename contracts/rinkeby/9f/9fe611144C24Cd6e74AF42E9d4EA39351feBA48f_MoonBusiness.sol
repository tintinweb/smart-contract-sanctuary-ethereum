...
contract MoonBusiness is ERC721Enumerable, Ownable, ReentrancyGuard {
  
  using Strings for uint256;

  constructor() ERC721("MoonBusiness", "MBS") {}
  ...
  function mint(uint256 _numToMint) external payable nonReentrant() {
    require(block.timestamp > 1658354400, "Sale hasn't started.");	
	...
	require(balanceOf(msg.sender) + _numToMint <= MAX_MINTABLE, "Minting to many.");
    uint256 costForMinting = 0.005 ether * _numToMint; 
    require(
