...
contract ICO {
    ...
    /// @notice Stores the treasury address
    ...
    /**
     * @notice Emitted when SpaceCoins are being redeemed
     * @param amount Amount of SpaceCoins to be redeemed
     * @param user Address of the contributing wallet
     */
    event RedeemEvent(uint256 amount, address indexed user);

    /**
     * @notice Emitted when Spacecoins are withdrawn into the 
     * treasury address.
     * @param amount Amount of SpaceCoins to be redeemed
     */
     ...
     * @param investors List of addresses to be added. No address can be added twice.
     *                  If one address fails, the transaction will revert
     */
    function grantSeedAccess(address[] memory investors) external onlyOwner {
        require(phase == Phase.SEED, "grantSeedAccess: not in seed phase");
        for (uint256 i = 0; i < investors.length; i++) {
            require(investors[i] != address(0), "grantSeedAccess: empty address");
            require(
                !seedInvestorMap[investors[i]],
                "grantSeedAccess: already have access"
            );
            seedInvestorMap[investors[i]] = true;
        }
    }
    ...
    /**
     * @notice Users can redeem SpaceCoin once Phase is open
     * @dev This is still allowed even if phase is paused
     */
    function redeem() external {
        require(phase == Phase.OPEN, "redeem: not in open phase");
        require(
            amountContributed[msg.sender] > 0,
            "redeem: no contributions made"
        );

        // Compute amount of SpaceCoin to give
        uint256 amountToRedeem = amountContributed[msg.sender] *
            NUM_SPACECOIN_PER_ETH;
        // Sanity check: cannot be larger than goal
        require(
            amountToRedeem <= (GOAL * NUM_SPACECOIN_PER_ETH), 
            "redeem: amount redeemed too large"
        );

        // Reset to zero so no double redeems
        /// @dev We do not update `totalAmount` which is needed to check
        //       if the ICO is fully funded. Users can redeem before then
        amountContributed[msg.sender] = 0;

        // Transfer to sender
        spaceCoin.transfer(msg.sender, amountToRedeem);

        emit RedeemEvent(amountToRedeem, msg.sender);
    }

    /**
     * @notice The owner can withdraw ETH into the treasury
     * @dev This can occur at any phase and whether funded or not
     */
    function withdraw() external onlyOwner {
        uint256 balanceEth = address(this).balance;

        // Transfer all ETH to treasury address
        (bool success,) = payable(treasury).call{value: balanceEth}("");
        require(success, "withdraw: failed to transfer ETH");

        emit WithdrawEvent(balanceEth);
    }
