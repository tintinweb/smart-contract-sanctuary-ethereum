...
/// @notice EGGトークンの実装
contract Egg is Ownable, Mintable, Burnable, SupportSig, ERC721Enumerable {
    ...
    */
    function approve(
        ...
        require(SupportSig.validNonce(nonce, ti.lastNonceForApprove), "ERC721: Invalid nonce");
        ...
        require(
            ...
            "ERC721: approve signer is not owner nor approved for all"
        );
        _approve(to, tokenId);
    ...
    */
    function mint(address to, uint256 gene, string calldata metadataHash) external onlyMinter returns (uint256)
    {
        uint256 newTokenId = totalMinted + 1;
        _mint(to, newTokenId);
        _tokens[newTokenId] = TokenInfo(newTokenId, gene, metadataHash,0);
        totalMinted++;
        return newTokenId;
    }
    ...
    */
    ...
    {
        _burn(tokenId);
        _tokens[tokenId] = TokenInfo(0, 0, "",0);
    }

    /// @notice トークンのメタデータを指すURIを取得する
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        TokenInfo memory tokenInfo = _tokens[tokenId];
        return string(abi.encodePacked("ipfs://", tokenInfo.metadataHash));
    }
