...
contract Badge is ERC721, Ownable {
  ...
  event OPCOsAdded(address indexed _op); 
  event CitizensAdded(address indexed _opco);
  ...
  mapping(address => uint256) internal OPCOCitizensIndexMap; 
  ...
  function _newOP(address _adr) internal {
    OP memory op = OP({
      op: _adr
    });
    OPs.push(op);
  ...
  function _newOPCO(address _adr, uint256 _supply) internal {
    OPCO memory opco = OPCO({
      co: _adr, 
      citizens: new address[](_supply), 
      supply: _supply, 
      minted: 0, 
      ipfs: ''
    });
    ...
    OPCOCitizensIndexMap[_adr] = 0; 
  }

  function _newCitizen(address _adr) internal {
    ...
    OPCOs[OPCOIndexMap[msg.sender]].citizens[OPCOCitizensIndexMap[msg.sender]] = _adr;
    OPCOCitizensIndexMap[msg.sender]++; 
  }

  function addOPs(address[] memory _adrs) public {
  ...
  function addOPCOs(address[] memory _adrs, uint256[] memory _supplies) public {
    for (uint256 i = 0; i < _adrs.length; i++) {
      _newOPCO(_adrs[i], _supplies[i]); 
    }
    emit OPCOsAdded(msg.sender);
  }

  function addCitizens(address[] memory _adrs) public {
    for (uint256 i = 0; i < _adrs.length; i++) {
      _newCitizen(_adrs[i]);
    }
    emit CitizensAdded(msg.sender);
  }

  function delegate(address _adr) public {
    Citizens[CitizenIndexMap[msg.sender]].delegate = _adr;
    Citizens[CitizenIndexMap[_adr]].delegations++;
  }

  function undelegate(address _adr) public {
    Citizens[CitizenIndexMap[msg.sender]].delegate = address(0);
    Citizens[CitizenIndexMap[_adr]].delegations--;
  }

  function getOPs() public view returns (OP[] memory) {
    return OPs;
  }

  function getOP(address _adr) public view returns (OP memory) {
    return OPs[OPIndexMap[_adr]];
  }

  function getOPCOs() public view returns (OPCO[] memory) {
    return OPCOs;
  }
