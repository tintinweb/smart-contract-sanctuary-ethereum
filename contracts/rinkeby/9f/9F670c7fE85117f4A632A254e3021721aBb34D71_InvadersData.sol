  ...
  Pausable
{
  ...
  uint256 public REPAIR_COST;

  function updateRepairCost(uint256 repairCost) public onlyOwner {
    REPAIR_COST = repairCost;
  }

  function repair(uint256 shipId, address sender) public onlyGameContract {
    require(ships[shipId].exists, "Ship does not exist");
    require(ships[shipId].disabledUntil > block.timestamp, "Not disabled");
    require(sender == ships[shipId].owner, "Not owner");
    EL69.transferFrom(ships[shipId].owner, address(this), REPAIR_COST);
  }
  ...
  function _removeShipFromPlanet(uint256 shipId) internal {
    ...
    // Replace the planet begin removed with the planet at the end of the list
    if (shipsOnPlanet[ship.planetId].length > 1) {
      shipsOnPlanet[ship.planetId][ship.planetIdx] = shipsOnPlanet[
        ship.planetId
      ][shipsOnPlanet[ship.planetId].length - 1];
    }
  ...
  function removeShip(
    ...
    if (ships[shipId].mining) {
      _claim(shipId, sender, risky);
    }
    _deleteShipData(shipId, sender);
    SHIPS.safeTransferFrom(address(this), sender, shipId);
  }
    ...
    whenNotPendingClaim(shipId)
  {
    ...
    require(planets[planetId].miningShipId == 0, "Planet being mined");
    ...
    whenNotPendingClaim(shipId)
  {
    ...
    _stopMining(shipId, sender, risky);
  }
    ...
    override
  {
    ...
    if (reqIdTypes[requestId] == 1) {
      ...
      if (claimReq.risky) {
        ...
        bool destruct = planetParams.geologicalInstability >
          (randomness % 100_000);
        if (destruct) {
          uint256 shipId = planets[claimReq.planetId].miningShipId;
          if (shipId != 0 && ships[shipId].mining) {
            ships[shipId].mining = false;
          }
          _deletePlanet(claimReq.planetId, planets[claimReq.planetId].owner);
          PLANETS.burn(claimReq.planetId);
        } else {
          awardedAmount = claimReq.amount;
          EL69.transfer(claimReq.claimer, awardedAmount);
        }
      } else {
    ...
    if (reqIdTypes[requestId] == 2) {
      ...
      if (succeeded) {
        ...
        if (planets[pirateReq.planetId].miningShipId != 0) {
          _stopMining(
            planets[pirateReq.planetId].miningShipId,
            planets[pirateReq.planetId].owner,
            false
          );
        }
  ...
  function pirate(
    ...
    require(quotedPrice == priceToSteal, "Price changed");
...
contract PixelglyphPlanets is ERC721Enumerable, Ownable, ReentrancyGuard {
  string public BASE_URI;
  string public ANIMATION_URL;
  ERC20 public saleCurrency;
  uint256 public ffPrice;
  uint256 public planetPrice = 150000 * 10**18;
  ...
  constructor(
    ...
    saleCurrency = ERC20(ffCurrency);
    ffPrice = _ffPrice;
  }

  function setsaleCurrency(address currency) public onlyOwner {
    saleCurrency = ERC20(currency);
  }
  ...
  function updatePlanetPrice(uint256 amount) public onlyOwner {
    planetPrice = amount;
  }
  ...
  function buyForceField(uint256 planetId) public nonReentrant {
    ...
    saleCurrency.transferFrom(msg.sender, owner(), ffPrice);
    _modifyForceField(planetId, true);
  }

  mapping(uint256 => uint256) tokenIdToSectorIdx;

  function buyWithEl69(uint256 amount) public {
    saleCurrency.transferFrom(msg.sender, owner(), amount * planetPrice);
  ...
  function _mintTo(address to, uint256 amount) internal {
    ...
    require(total + amount <= maxSupply, "Cannot mint amount");
