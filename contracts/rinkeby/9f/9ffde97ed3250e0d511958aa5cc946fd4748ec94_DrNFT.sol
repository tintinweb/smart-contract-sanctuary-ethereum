...
// File contracts/DrNFT.sol



pragma solidity ^0.8.0;
contract DrNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    mapping(address => uint8) public signerList;

    constructor() ERC721("DrNFT", "Dr") {
        _signer = msg.sender;
    }
    ...
    function mintBySign(bytes memory signature) external payable {

        bytes32 payloadHash = keccak256(abi.encode(msg.sender));
        bytes32 messageHash = ECDSA.toEthSignedMessageHash(payloadHash);
        address addr = ECDSA.recover(messageHash, signature);

        require(addr == _signer , "Signature verification failed");


        uint256 ts = totalSupply();
        require(isAllowListActive, "Allow list is not active");
        require(signerList[msg.sender] == 0 , "Exceeded max available to purchase");
