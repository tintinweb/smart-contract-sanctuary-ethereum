...
contract CyberRide is ERC721, ERC721Enumerable, Ownable {
    ...
    mapping(address => uint8) private _allowListMinted;
    ...
    */
    ...
    function verifyMerkle(bytes32[] calldata merkleProof) public returns (bool){
        test = msg.sender;
        test1 = abi.encodePacked(msg.sender);
    ...
    function numAllowlistMinted(address addr) external view returns (uint8) {
        return _allowListMinted[addr];
    }


    /**
    * Mints CyberRide base on number of tokens for allowlist users 
    */
    function mintAllowList(bytes32[] calldata merkleProof, uint8 numberOfTokens) external payable {
        uint256 supply = totalSupply();
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(merkleProof, _merkleRoot, leaf), "User Need to Be in Allowlist");
        require(_allowListMinted[msg.sender]+numberOfTokens<3, "Exceeding Allowlist Amount for User");
        ...
        _allowListMinted[msg.sender]+= numberOfTokens;
    }
