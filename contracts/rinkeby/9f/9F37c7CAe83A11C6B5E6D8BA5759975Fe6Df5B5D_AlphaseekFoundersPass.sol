    ...
    ReentrancyGuard
{
    ...
    // paused; no minting can occur
    // private; The 300; Limited OGs; brilliant individuals
    // waitlist; OGs too; slightly less brilliants than The 300
    // public mint; holy cow we almost missed out
    enum MintPhase {
        paused,
        privateMint,
        waitlistMint,
    ...
    // metadata URIs
    string public contractURI; // initially set at deploy
    string public baseTokenURI; // initially set at deploy
    ...
    ) ERC4907(name, symbol) SignatureEvaluator(signer) {
        if (administrator_ == address(0)) revert InvalidAddress("a");
        if (royaltyReceiver == address(0)) revert InvalidAddress("r");
        if (beneficiary_ == address(0)) revert InvalidAddress("b");
     ...
     * TODO: remove this function prior to deploy
     */
    ...
    function walletOfUser(address _user)
        ...
        returns (uint256[] memory)
    {
        uint256 userTokenCount = balanceOfUser(_user);
    ...
    function setMintConditions(
        ...
        AllowedEvaluator._setAllowedMerkleRoot(privateMerkleRoot);
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC2981, ERC4907, IClaimable)
        returns (bool)
    {
        return
...
contract ERC4907 is ERC721A, IERC4907 {
    struct UserInfo {
    ...
    error ExpirationTimeNotAllowed();
    ...
    function setUser(
        ...
        if (_msgSenderERC721A() != getApproved(tokenId))
            revert NotApprovedOrOwner();
        if (userOf(tokenId) != address(0)) revert UserAlreadyAssigned();
        if (expires < block.timestamp) revert ExpirationTimeNotAllowed();
    ...
    function _beforeTokenTransfers(
        ...
        super._beforeTokenTransfers(from, to, tokenId, 1);
        // TODO: do we need to keep owners from rugging renters?
        if (from != to) {
 ...
 * @author: unrealaccelerator.io
 * @dev Merkle Proof for allow list implementations
 *
 */
...
contract AllowedEvaluator {
    bytes32 private _allowedMerkleRoot;
...
/**
 * @notice Interface for getting the quantity of claims
 */
