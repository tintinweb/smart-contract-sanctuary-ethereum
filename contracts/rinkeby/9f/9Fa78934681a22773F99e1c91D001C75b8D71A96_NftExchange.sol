...
contract ExchangeState is OwnableOperatorRole {
    ...
    function setCompleted(ExchangeDomain.OrderKey calldata key, uint256 newCompleted) external onlyOperator {
        ...
        keyList.push(getCompletedKey(key));
        valueList.push(newCompleted);
    }
    ...
    function getList() view  external returns (bytes32[] memory,uint256[] memory) {
...
contract ExchangeOrdersHolder {
    ...
    function add(ExchangeDomain.Order calldata order) external {
        ...
        keyList.push(key);
        valueList.push(OrderParams(order.selling, order.buying, order.sellerFee));
    }
    ...
    function getList() view  external returns (bytes32[] memory,OrderParams[] memory) {
        return (keyList,valueList);
    }
}
