...
contract LiveNftAuction is ReentrancyGuard, Ownable {
    ...
    // Maps NFT ID to it's address
    mapping(uint256 => address) tokenFactoryAddrByListingId;
    // Maps how much an account has bid for auction items.
    ...
    function checkNftStatus(address _tokenFactoryAddr, uint256 _tokenId)
        external
        onlyOwner
    {
        require(
            auctionListings[_tokenFactoryAddr][_tokenId].timeStart +
                5 minutes <=
                block.timestamp,
        ...
        if (tokenIdPosition.current() == auctionTokenIdxIds.length - 1) {
            ...
            _winNft(_tokenFactoryAddr, _tokenId);
        }
    }

    function placeBid(address _tokenFactoryAddr, uint256 _tokenId)
        public
        payable
        nonReentrant
    {
        ...
        require(
            auctionListings[_tokenFactoryAddr][_tokenId].isAuctioning,
            "placeBid: Cannot bid on NFT"
        );
        require(
            msg.value >
                auctionListings[_tokenFactoryAddr][_tokenId].currentPrice,
            "placeBid: Bid needs to be greater than current price"
        );
        auctionListings[_tokenFactoryAddr][_tokenId].bidLeader = msg.sender;
        auctionListings[_tokenFactoryAddr][_tokenId].currentPrice = msg.value;
    ...
    function setAuctionNft() public onlyOwner {
        (address _tokenFactoryAddr, uint256 _tokenId) = _findNextAuctionNft();
    ...
    function _findNextAuctionNft() private view returns (address, uint256) {
        return (
            tokenFactoryAddrByListingId[
