 ...
 *     ― J.K.W. Blockchain
 *
 * Through their voting power, LG token balances govern property directors and their
 * continuous reinvestment of net cash flows from multi-industry development projects
 * and associated Loci Bell (LB) tokens. (LB tokens are single-property multi-tranche
 * fixed-growth ERC1155 debt contracts.)
 *
 * LG's multi-class voting mechanism presents an ERC20 voting interface compatible with
 * popular on-chain governance UX while also presenting an ERC1155 multi-class interface
 * thereby enabling differentiation of expert voting power and common protocol equity.
 *
 * 65% of voting power is held by 97% of LG (a.k.a. ERC1155 LG).  35% of voting power is
 * controlled by DAO Foundation directors with less than 3% of LG (a.k.a. ERC1155 LGY).
 * Aggregate balances of ERC1155 LG/Y present as ERC20 LG token balances.
 * 
 * LG token has built-in emission curves and allocation distribution by director role.
 * Tokens are minted on demand in their respective category according to distinct
 * emission curves or director grant.  Chainges to allocations and curves may only occur
 * via governance vote.
 */
contract LG is ERC1155Votes, AccessControl, AllocationControl, EmissionCurves, IAllocationMinter {
    ...
    constructor(address daoMultisig, string memory baseURI)
        ERC1155Votes("Loci Global", "LG", baseURI, 2)
        AllocationControl(daoMultisig)
        EmissionCurves(daoMultisig)
    {
        ...
        _allocationAllocate( NECTAR_ALLOCATION, 0,  15_100_000 * 1e18); // Core Technology
...
abstract contract AllocationControl is AccessControl {
    ...
    function allocationAllocate(bytes32[] calldata roles, uint256[] calldata tokenIds, uint256[] calldata units, uint256 totalSupplyCap)
        ...
        onlyRole(ALLOCATOR_ROLE)
    {
        _allocationAllocate(roles, tokenIds, units, totalSupplyCap);
    }
    ...
    function _allocationAllocate(bytes32[] calldata roles, uint256[] calldata tokenIds, uint256[] calldata units, uint256 totalSupplyCap)
        internal
    {
        for (uint i = 0; i < roles.length; ++i) {
 ...
 * Author: Mikhail Vladimirov <[email protected]l.com>
 */
pragma solidity ^0.8.0;
