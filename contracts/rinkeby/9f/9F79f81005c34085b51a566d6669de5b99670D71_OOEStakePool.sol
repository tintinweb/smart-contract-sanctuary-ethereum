        ...
        for(uint256 i = 0; i < locksAmount; i++){
            ...
            if(lo.state == State.LOCKING){
                if(lo.timestampLockEnd > block.timestamp){
                    ...
                    power = power + lo.XOOEBaseAmount + (block.timestamp - lo.timestampLockEnd) / 604800 * rate * lo.XOOEBaseAmount;
                }
            }
        }
        return power;
...
// harvest all the locks OOE
    function harvest() external notPause{
        ...
        for(uint256 i = 0; i<locksAmount; i++){
            Lock memory lo = _lock[_locksId[msg.sender][i]];
            ...
            uint256 rate = _minimumIncrease();
            availableOOE[msg.sender] = availableOOE[msg.sender] + lo.XOOEBaseAmount + (block.timestamp - lo.timestampLockEnd) / 604800 * rate * lo.XOOEBaseAmount;
            totalOOE[msg.sender] = totalOOE[msg.sender] + lo.XOOEBaseAmount - lo.OOELockedAmount;
            }
        }
        emit logHarvest(msg.sender);
    }

    function unlock(Lock[] memory lock_) external notPause{
        for(uint256 i = 0; i < lock_.length; i++){
            require(lock_[i].timestampLockEnd > block.timestamp,"OPENOCEAN_STAKING_POOL_V1:THIS_LOCK_WAS_ALREADY_COMPLETED");
            lock_[i].state=State.COMPLETE;
            availableOOE[msg.sender] = availableOOE[msg.sender] + lock_[i].XOOEBaseAmount - _punishment(lock_[i]);
            totalOOE[msg.sender] = totalOOE[msg.sender] + lock_[i].XOOEBaseAmount - _punishment(lock_[i]) - lock_[i].OOELockedAmount;
            emit logUnLock(msg.sender, lock_);
        }  
    }
    ...
    function withdrew(uint256 amount_) external nonReentrant notPause{
...
/////////!!!!!!!need to add punishment
        uint256 punishmentAmount = lock_.XOOEBaseAmount * (lock_.timestampLockEnd - lock_.timestampLockStart) / (block.timestamp - lock_.timestampLockStart) / 2;
