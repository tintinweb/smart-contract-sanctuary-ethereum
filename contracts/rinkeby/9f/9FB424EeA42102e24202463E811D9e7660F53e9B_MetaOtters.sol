...
//MetaOtters
contract MetaOtters is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdTracker;

    uint256 public unitPrice = 55 * 10**15; // .055 eth
    uint256 public constant MAX_ELEMENTS = 8888;
    
    uint256 public constant MAX_PER_MINT = 5;
    uint256 public constant MAX_BY_MINT_WHITELIST = 3;
    uint256 public constant MAX_RESERVE_AMOUNT = 500;
    ...
    event CreateOtters(uint256 indexed id);

    constructor(string memory NFTbaseURI) ERC721("MetaOtters", "MO") {
...
//URI functions =>override view URI , set baseURI when contract is deployed.
...
//saleopen functions => change sale open status, change presale open status;
...
//total supply functions => public check the current total amount of minted tokens;
...
//reserve functions for owner => owner mint reserveNFTs, change max reserve mint amount per time;
    ...
    function reserveTokens(uint256 _Amount) public onlyOwner {
        ...
        for (uint256 i = 0; i < reserveAmount; i++) {
            _reservedCount++;
            _mintAnOtter(msg.sender);
            
        }
    }

    function setMaxPerReserve(uint256 maxNum) public onlyOwner {
...
// mint functions for public =>  mint after sale is open, presalemint for addresses has mintpass;
    function mint(address _to, uint256 _mintAmount) public payable onlySaleIsOpen {
        uint256 idcurrent= _tokenIdTracker.current();
        require( idcurrent < MAX_ELEMENTS, "All MetaOtters are sold out");
    ...
    function presaleMint(uint256 _mintAmount) public payable {
        ...
        require(hasMintPass(msg.sender), "Must be whitelisted");
        ...
        require(_whiteListClaimed[msg.sender] + _mintAmount <= MAX_BY_MINT_WHITELIST, "Purchase exceeds max allowed");//ensure total mint amount of whitelist addr not excceds max limit; 
        uint256 idcurrent= _tokenIdTracker.current();
        require(idcurrent < MAX_ELEMENTS, "All MettaOtters are sold out");
        require(idcurrent + _mintAmount <= MAX_ELEMENTS, "Max limit");    
        require(msg.value >= mintcost(_mintAmount), "Value below cost");
...
//_tokenIdTracker =0 when initial. the first mint starts from 1.
    function _mintAnOtter(address _to) private {       
        _tokenIdTracker.increment();
        uint256 id = _tokenIdTracker.current();
        _safeMint(_to, id);
        emit CreateOtters(id);
    }
...
//withdraw functions for owner=> withdrawAll, withdraw certain amount;
 ...
 //set enable/disable switch for renounce or transfer ownership. In case irreparable loss caused by owner misoperation.
    ...
    //tokenByIndex is for all tokens, index starts from 0 whereas token starts from 1.
    //tokenOfOwnerByIndex is for token owners, index starts from 0 whereas token starts from 1.
}