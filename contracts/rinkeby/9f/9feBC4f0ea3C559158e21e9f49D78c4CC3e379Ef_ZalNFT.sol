...
contract ZalNFT is Ownable, ERC721A, ReentrancyGuard {
  ...
  uint256 public  MINT_PRICE_WRLD_PRESALE = 300000000000000000000;
  uint256 public  MINT_PRICE_WRLD = 350000000000000000000;
  uint256 public  MAX_TOKENS = 10000;
  uint256 public  devAmount = 200;
  uint256 public  maxBatch = 5;
  uint256 public  maxTokensInPresale = 5;
  IWRLD_Token_Ethereum private WRLD_Token_Ethereum;
  string private _baseTokenURI;
  bytes32 public merkleRoot;
  mapping(address => uint256) private _tokensClaimedInPresale;

  constructor()
  ERC721A("ZalNFT", "ZNFT", maxBatch, MAX_TOKENS)
	{
	  WRLD_Token_Ethereum = IWRLD_Token_Ethereum(0xD5d86FC8d5C0Ea1aC1Ac5Dfab6E529c9967a45E9);
    }
	
  function initialize() public onlyOwner {
        WRLD_Token_Ethereum.approve(address(this), 1000000000000000000000000000000);
  }
  ...
  function preSaleWRLDMint(uint256 quantity, bytes32[] memory proof) external {	
    require(preSale, "Not presale");
    if (MerkleProof.verify(proof, merkleRoot, keccak256(abi.encodePacked(msg.sender)))) {
        require(_tokensClaimedInPresale[msg.sender] + quantity <= maxTokensInPresale,
                "Total exceeds 5");
  ...
  function publicSaleWRLDMint (uint256 quantity) external {
    require(publicSale, "Not public");
    require(totalSupply() + quantity <= MAX_TOKENS, "Over max");
  ...
  function devMint(address _to, uint256 quantity) external onlyOwner {
    require(totalSupply() + quantity <= devAmount, "Too many");
    require(quantity % maxBatch == 0, "Req mult of 5");
    uint256 numChunks = quantity / maxBatch;
    for (uint256 i = 0; i < numChunks; i++) {
      _safeMint(_to, maxBatch);
    }
  }
  ...
  function setPresalePriceEth(uint64 price) external onlyOwner{
    MINT_PRICE_ETH_PRESALE = price;
  }
  function setPresalePriceWRLD(uint64 price) external onlyOwner{
	MINT_PRICE_WRLD_PRESALE = price;
  }
  function setPriceEth(uint64 price) external onlyOwner {
    MINT_PRICE_ETH = price;
  }
  function setPriceWRLD(uint64 price) external onlyOwner {
    MINT_PRICE_WRLD = price;
  }
  ...
  function withdrawMoney() external onlyOwner nonReentrant {
        ...
        WRLD_Token_Ethereum.transferFrom(address(this), owner(), WRLD_Token_Ethereum.balanceOf(address(this)));
  }
