    ...
    ERC2771Recipient
{
    ...
    struct Token {
        ...
        uint256 limitPerWallet; // 0 is no limit
    ...
    function claim(uint256 _id) external payable {
        ...
        if (tokens[_id].claimFee > 0) require(msg.value == tokens[_id].claimFee, "eth amount incorrect");
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            _checkMint(_ids[i], _amounts[i], _to);
        }
    ...
    function mintBulk(
        ...
        for (uint256 i = 0; i < _addresses.length; i++) {
            _checkMint(_ids[i], _amounts[i], _addresses[i]);
    ...
    function createToken(
        ...
        emit TokenCreated(id, _name, _maxSupply, _transferEnabled, _limitPerWallet);
        return id;
    }
    ...
    function setTokenClaimFee(uint256 _id, uint256 _claimFee) external onlyRole(MANAGER_ROLE) {
        require(exists(_id), "token does not exist");
    ...
    function setLimitPerWallet(uint256 _id, uint256 _limitPerWallet) external onlyRole(MANAGER_ROLE) {
        require(exists(_id), "token does not exist");
        tokens[_id].limitPerWallet = _limitPerWallet;
    }
    ...
    function _checkMint(
        ...
        if (tokens[_id].maxSupply > 0)
            require(tokens[_id].counter + _amount <= tokens[_id].maxSupply, "max supply reached");
        if (tokens[_id].limitPerWallet > 0)
            require(this.balanceOf(_to, _id) + _amount <= tokens[_id].limitPerWallet, "max per wallet reached");
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) || to != address(0)) {
            for (uint256 i = 0; i < ids.length; i++) {
                require(tokens[ids[i]].transferEnabled, "transafer disabled");
            }
        }
