...
import "../../storage/VolmexRealisedVolVaultStorage.sol";
 ...
 * Any changes/appends in storage variable needs to happen in VolmexRealisedVolVaultStorage.
 * VolmexRealisedVolVault should not inherit from any other contract aside from VolmexVault, VolmexRealisedVolVaultStorage
 */
contract VolmexRealisedVolVault is VolmexVault, VolmexRealisedVolVaultStorage {
    ...
    function initialize(
        ...
        require(
            ...
            "ThetaVault: Zero address provided for template"
        );
        require(
            _priceOracle != address(0),
            "ThetaVault: Zero address provided for price oracle"
        );
        ...
        require(_minVaultDeposit > 0, "!minVaultDeposit");
    ...
    function commitAndClose() external nonReentrant {
        ...
        if (delta > vTokenPrice) {
            vTokenState.vTokenPnL = delta.sub(vTokenPrice) > vTokenPrice
                ? 0
                : delta.sub(vTokenPrice);
            vaultSharePrice[vaultState.round] =
                minVaultDeposit[vaultState.round] +
                delta.sub(vTokenPrice);
        } else {
            vTokenState.vTokenPnL = vTokenPrice.sub(delta);
            vaultSharePrice[vaultState.round] =
                minVaultDeposit[vaultState.round] -
                (vTokenPrice - delta);
        }
    ...
    function redeemByMarketMaker(address vToken, uint256 shares) external {
        require(vToken != address(0), "address cannot be zero");
        require(
            shares >= IVToken(vToken).balanceOf(msg.sender),
            "Insufficient balance"
        );

        uint256 amountToBeWithdraw = shares.mul(vTokenState.vTokenPrice);

        emit VTokenRedeemed(msg.sender, amountToBeWithdraw);

        IVToken(vToken).burn(msg.sender, shares);

        require(amountToBeWithdraw > 0, "!withdrawAmount");
        transferAsset(
            msg.sender,
            amountToBeWithdraw.mul(10**Vault.FEE_MULTIPLIER)
        );
    }
     ...
     */
    function rollToNextVtoken(uint256 _minVaultDeposit, uint256 _minVtokenPremium)
        external
        onlyKeeper
        nonReentrant
    {
...
abstract contract VolmexRealisedVolVaultStorageV1 {
...
abstract contract VolmexRealisedVolVaultStorage is
    VolmexRealisedVolVaultStorageV1
{
    ...
    ERC20Upgradeable
{
    ...
    /// @notice role in charge of weekly vault operations such as rollToNext and burnRemainingOTokens
    ...
    function mint(uint256 _shares) external nonReentrant {
        ...
        require(assets > minVaultDeposit[vaultState.round], "!minVaultDeposit");
    ...
    function initiateWithdraw(uint256 _numShares) external nonReentrant {
        require(_numShares > 0, "!numShares");
    ...
    function initRounds(uint256 _numRounds) external nonReentrant {
        require(_numRounds > 0, "!numRounds");

        uint256 _round = vaultState.round;
        for (uint256 i = 0; i < _numRounds; i++) {
            uint256 index = _round + i;
            require(minVaultDeposit[index] == 0, "Initialized"); // AVOID OVERWRITING ACTUAL VALUES
    ...
    function _completeWithdraw() internal returns (uint256) {
        ...
        uint256 withdrawAmount = ShareMath.sharesToAsset(
            withdrawalShares,
            vaultSharePrice[withdrawalRound],
            vaultParams.decimals
        );
    ...
    function _redeem(uint256 _numShares, bool _isMax) internal {
        ...
        require(_numShares <= unredeemedShares, "Exceeds available");
        ...
        _transfer(address(this), msg.sender, _numShares);
    }
    ...
    {
        require(block.timestamp >= vTokenState.nextVTokenReadyAt, "!ready");

        newVToken = vTokenState.nextVToken;
        require(newVToken != address(0), "!newVToken");
