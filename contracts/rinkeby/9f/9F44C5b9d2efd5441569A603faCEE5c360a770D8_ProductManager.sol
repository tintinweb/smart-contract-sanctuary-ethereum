...
// contract ProductManager is Initializable, ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
contract ProductManager is ERC721, Pausable, Ownable {
    ...
    event AuctionCreate(uint256 indexed auctionID, string name, uint256 startPrice, uint256 productID, uint32 startTime, uint32 duration, address owner);
    ...
    function createAuction(
        ...
        uint32 _startTime,
        uint32 _duration, 
        bool _isPrivate) public {
        // Lay san pham dau gia
        Product product = getProductByID(_productId);

        // Tao phien dau gia
        Auction newAution = new Auction(address(product), _startPrice, _depositRate, _startTime, _duration, _isPrivate);
    ...
    function bid(uint256 auctionIndex, uint256 amount) public payable returns (bool) {
        ...
        // Kiem tra nguoi tao phien dau gia
        address creator = auction.getCreator();
        require(creator != address(0));

        // Kiem tra phien dau gia da bat dau chua
        ...
        // Pre-bidder
        address preBidder = auction.getCurrentBidOwner();
        
        // Ty le dat coc
        uint256 depositRate = auction.getDepositRate();

        // So luong ETH tham gia phai cao hon gia hien tai
        if (amount > currentBidAmount) {
            // Dat lai gia hien tai
            auction.setCurrentBidAmount(amount);

            // Dat lai nguoi dau thau thanh cong
            auction.setCurrentBidOwner(msg.sender);

            // Tang so luong dau thau
            auction.addBidCount(1);

            // Tinh ty le dat coc
            uint256 deposit = amount * depositRate / 100;

            // Gui ETH vao contract
            require(msg.value > deposit);

            emit AuctionBid(auctionIndex, msg.sender, amount, msg.value);

            // Gui ETH tu contract ve pre-bidder
            address payable receiver = payable(preBidder);
            receiver.transfer(currentBidAmount * depositRate / 100);
    ...
    /**
     * @dev Hien thi so luong phien dau gia.
     */
    function getTotalAuctions() public view returns (uint256) { return indexOfAuction; }

    // return a product with id
    function getProductByID(uint256 id) public view returns (Product){
        Product product = productsID[id];
        return product;
    }
    
    /**
     * @dev Hien thi chi tiet phien dau gia khi phien dau gia ket thuc
     *
     * result: `bidOwner` nguoi dau thau thanh cong hien tai
     * result: `bidAmount` so luong ETH tham gia dau thau
     * result: `depositRate` ty le dat coc
     * result: `bidCount` so lan dat coc
     * result: `winner` nguoi dau thau thanh cong sau khi phien dau gia ket thuc
     * result: `creater` nguoi tao phien dau gia
     * result: `startTime` thoi gian bat dau phien dau gia
     * result: `endTime` thoi gian ket thuc phien dau gia
     */
    function getAuctionByID(uint256 id) public view returns (
        ...
        require(id < indexOfAuction);

        Auction auction = auctionID[id];
        
        bidOwner = auction.getCurrentBidOwner();
        bidAmount = auction.getCurrentBidAmount();
        depositRate = auction.getDepositRate();
...
contract Product {
    ...
    function getOwnerOfProduct() public view returns (address) {
        return owner;
    }
}
...
contract Auction {
    ...
    uint256 private currentBidAmount;
    uint256 private depositRate;
    address private currentBidOwner;
    ...
    constructor(
        ...
        uint256 _duration, 
        bool _isPrivate) {
        require(_productAddress.isContract());
    ...
    function getBidCount() public view returns (uint256) { return bidCount; }
    ...
    function addBidCount(uint256 _count) public {
        bidCount = bidCount.add(_count);
    }
}
