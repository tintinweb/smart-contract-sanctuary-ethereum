...
contract LiquidityExamples is IERC721Receiver {
    ...
    ERC20 immutable USDC = ERC20(Usdc_Address);
    ...
    function swapExactInputSingle(
        ...
        require(msg.sender == Owner, "Not Real Owner");

        IUniswapV3Pool pool_contract = IUniswapV3Pool(pooladdress);
        address Token_0 = pool_contract.token0();
        address Token_1 = pool_contract.token1();
        uint24 Pool_Fee = pool_contract.fee();
        console.log("Pool Fee : ", Pool_Fee);
        address Token_Needed;
        if (Token_0 == Usdc_Address) {
            Token_Needed = Token_1;
        } else {
            Token_Needed = Token_0;
        }
        ...
        )
    {
        require(
            ...
            "Please Make Transaction with Gas Wallet"
        );
        ...
        // Note that the pool defined by DAI/USDC and fee tier 0.01% must be already created
    ...
    {
        // Get the Token0 and Token1 from Token ID and run the following command below
    ...
    function Change_Gas_Wallet(address new_gas_wallet) public {
        require(msg.sender == Owner, "Not Real Owner");
