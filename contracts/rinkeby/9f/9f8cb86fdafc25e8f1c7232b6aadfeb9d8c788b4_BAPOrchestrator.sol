...
// File: Interfaces/BAPOrchestratorInterfaceV2.sol
...
contract BAPOrchestrator is ReentrancyGuard, Ownable {
    ...
    function checkBreeding(uint256 tokenId) internal {      
        require(bapGenesis.ownerOf(tokenId) == msg.sender, "Only the owner can breed");
        uint256 currentBreeds = bapGenesis.breedings(tokenId); 

        if(currentBreeds > 0){
            bapGenesis.updateBullBreedings(tokenId);
        } else {
            require(!extraBreedClaimed[tokenId], "This bull already got an extra breed");
