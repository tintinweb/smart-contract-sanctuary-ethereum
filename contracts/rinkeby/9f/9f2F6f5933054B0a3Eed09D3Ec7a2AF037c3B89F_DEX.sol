...
contract DEX {
    ...
    /**
     * @notice Emitted when liquidity is provided to DEX and LPTs are minted.
     */
    ...
    /**
     * @notice Emitted when liquidity is removed from DEX and LPTs are burned.
     */
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        ...
        uint256 ethReserve = address(this).balance.sub(msg.value); //subtract eth to swap from the reserve
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        uint256 ethReserve = address(this).balance.sub(msg.value); // get current eth balance of the dex contract
        uint256 tokenReserve = token.balanceOf(address(this)); // get current token balance of the dex contract
        uint256 tokenDeposit;

        tokenDeposit = (msg.value.mul(tokenReserve) / ethReserve).add(1);
        uint256 liquidityMinted = msg.value.mul(totalLiquidity) / ethReserve;
        liquidity[msg.sender] = liquidity[msg.sender].add(liquidityMinted); // update the provided liquidity mapping
