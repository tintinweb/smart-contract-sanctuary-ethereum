...
contract BettingPools is Ownable {
    ...
    uint256 public totalBetHome;
    uint256 public totalBetAway;
    uint256 public totalBetDraw;
    ...
    function bet(uint8 _pool) public payable {
      ...
      if(_pool == 1) {
        playerMap[msg.sender].betAway = msg.value;
        totalBetAway += msg.value;
      } else if (_pool == 2) {
        playerMap[msg.sender].betHome = msg.value;
        totalBetHome += msg.value;
      } else {
        playerMap[msg.sender].betDraw = msg.value;
        totalBetDraw += msg.value;
      }

      playerAddresses.push(msg.sender);
    }

    function getTotalBetsForPool(uint8 _pool) public view returns(uint256) {
      if(_pool == 1) {
        return totalBetAway;
      } else if (_pool == 2) {
        return totalBetHome;
    ...
    function getBets() public view returns(Player memory) {
      Player memory playerBet = playerMap[msg.sender];
      return playerBet;
    }
    ...
    function getWinnings() public view returns(uint256){
        ...
        if (currentStatus == GameStatus.TEAM_HOME) {
          winnings = playerBet.betHome + ((playerBet.betHome * (totalBetDraw + totalBetAway)) / totalBetHome);
        } else if (currentStatus == GameStatus.TEAM_AWAY) {
          winnings = playerBet.betAway + ((playerBet.betAway) * (totalBetHome + totalBetDraw)) / totalBetAway;
        } else if (currentStatus == GameStatus.DRAW) {
          winnings = playerBet.betDraw + ((playerBet.betDraw) * (totalBetHome + totalBetAway)) / totalBetDraw ;
        }

        return winnings;
    }

    function withdrawWinings() public payable {
      require (!hasPlayerWithdrawn[msg.sender],
        "no withdraw availible"
      );
