...
contract IronOutcasts is ERC721A, Ownable, ReentrancyGuard {
    string private baseURI;

    uint256 public immutable price = 0.06 ether;
    uint256 public immutable specialPrice = 0.04 ether;
    uint256 public immutable maxTokensWallet = 21;  // 20
    uint256 public immutable maxSupply = 8001;      // 8000

    uint256 public saleIsOpen;

    bytes32 private allowListMerkleRoot;
    mapping(address => bool) public addressWhiteListClaimed;
    ...
    function mintAllowList(bytes32[] calldata _proof) external payable {
        require(!addressWhiteListClaimed[msg.sender], "Already claimed");
        require(totalSupply() + 1 < maxSupply, "Max supply reached");
        require(_numberMinted(msg.sender) + 1 < maxTokensWallet, "Max token per wallet reached");
        require(_verify(_leaf(msg.sender), _proof, allowListMerkleRoot), "Address not authorized");
        require(msg.value >= specialPrice, "Insufficient funds!");

        addressWhiteListClaimed[msg.sender] = true;
