...
contract ESCTEST is ERC721A, Ownable, ReentrancyGuard {
    enum Status {
        Waiting,
        Started,
        Finished
    ...
    constructor(string memory initBaseURI) ERC721A("testesc", "test") {
        baseURI = initBaseURI;
    }
    ...
    function freeMintPublic(uint256 quantity ) external payable nonReentrant {
        ...
        require(freeCounter <= FREE_PUBLIC_SUPPLY, "Free mints are sold out");
    ...
    function ogMint (uint256 quantity, bytes32[] calldata _merkleProof)external payable nonReentrant{
        ...
        require( ownerMintedCountOg < OG_MAX_MINT, "Exceeded maximum amount. 2 or less per wallet");
