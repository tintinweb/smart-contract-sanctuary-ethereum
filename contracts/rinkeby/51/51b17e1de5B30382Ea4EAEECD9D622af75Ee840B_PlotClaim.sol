...
contract PlotClaim is  Ownable {
    ...
    address payable _turnWallet = address(0x3244C695758e077422DDB30Ec229595A6A92DC04);
    address public metaplot = address(0xFc7C3de5Fe729E18828Fce6590C4c7B3568609Bf);  
    ...
    function setMetaPlot( address _contractaddr ) external onlyOwner {
        metaplot = _contractaddr;
    }

    function claim_go( address reg_addr,uint256 tokeid) internal returns (uint16) 
    {
        IMetaPlot _metaplot = IMetaPlot(metaplot);
        _metaplot.mint(reg_addr, tokeid);
    }

    function preCliam(uint256 price, uint256 tokenid, uint256 expiry, uint256 _nonce, bytes memory sig) public payable {

            require( expiry > block.number, "Expiry fail" );
            require(!usedNonces[_nonce]);
            usedNonces[_nonce] = true;
        
            bytes32 message = prefixed(keccak256(abi.encodePacked(msg.sender, price, tokenid, expiry, _nonce, this)));

            if( recoverSigner( message, sig ) == checker )
            {
               require(price <= msg.value, "Ether too small");
               claim_go(msg.sender,tokenid);
            }
    }  
