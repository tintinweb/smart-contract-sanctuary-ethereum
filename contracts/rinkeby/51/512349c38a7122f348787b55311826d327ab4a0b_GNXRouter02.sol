...
// File: contracts/GNXRouter02.sol
...
contract GNXRouter02 is IGNXRouter02 {
    using SafeMath for uint;

    address public immutable override factory;
    address public immutable override WETH;

    event EventoTemporal(address dirreccion_del_par);
    ...
    function _addLiquidity(
        ...
        if (IGNXFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IGNXFactory(factory).createPair(tokenA, tokenB);
        }
    ...
    function addLiquidity(
        ...
        liquidity = IGNXPair(pair).mint(to);
    }
    function addLiquidityETH(
        address token,
        uint amountTokenDesired,
    ...
    function removeLiquidity(
        ...
        address pair = AMMLibrary.pairFor(factory, tokenA, tokenB);
        IGNXPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IGNXPair(pair).burn(to);
    ...
    function removeLiquidityWithPermit(
        ...
        IGNXPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IGNXPair(AMMLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        uint fee = 25;
        amounts = AMMLibrary.getAmountsOut(factory, amountIn, path, fee);
        require(amounts[amounts.length - 1] >= amountOutMin, 'Router: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, AMMLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint fee = 25;
        amounts = AMMLibrary.getAmountsOut(factory, msg.value, path, fee);
        require(amounts[amounts.length - 1] >= amountOutMin, 'Router: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    {
        return AMMLibrary.getAmountOut(amountIn, reserveIn, reserveOut, fee);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path, uint fee)
        ...
        returns (uint[] memory amounts)
    {
        return AMMLibrary.getAmountsOut(factory, amountIn, path, fee);
    }
...
library AMMLibrary {
    ...
    event AMMLibrary_Reserves0(uint pair);
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint256(keccak256(abi.encodePacked(
                ...
                hex'2e708642401d738efadc3537052d685dc89273999c30ce611ed3eda6669bf3f8' // init code hash
            )))));
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IGNXPair(pair_address).getReserves();
