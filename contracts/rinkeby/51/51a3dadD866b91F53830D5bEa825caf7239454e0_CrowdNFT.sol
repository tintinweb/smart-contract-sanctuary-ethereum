...
contract CrowdNFT is ERC1155, ERC1155Burnable, ERC1155Pausable, ERC1155Supply, ERC1155URIStorage, Lockable {
    mapping(uint256 => TokenInfo) public tokenInfos;
    mapping(uint256 => uint256[]) private projectItems;
    ...
    function mint(uint256 projectId, uint256 id, uint256 amount, uint price, bytes memory data) public onlyOwner
    {
        ...
        projectItems[projectId].push(id);
    ...
    function buy(uint256 id, uint256 amount) public payable {
        ...
        require(msg.value >= amount * tokenInfos[id].price, 'token price mismatch');
    ...
    function burn(
        ...
        tokenInfos[id].totalBurned = tokenInfos[id].totalBurned + value;
    }
    ...
    function _afterTokenTransfer(
        ...
        if(from == owner()) {
            for (uint256 i = 0; i < ids.length; i++) {
                ...
                tokenInfos[id].totalSells =  tokenInfos[id].totalSells + amount;
                tokenInfos[id].totalBuyers =  tokenInfos[id].totalBuyers + 1;
            }
        }
    }
}
