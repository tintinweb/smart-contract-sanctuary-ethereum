...
contract Lottery is Ownable, VRFConsumerBase {
    ...
    //We can use the constructor of inherited contract inside our own
    constructor(
        address _priceFeed,
    ...
    function getEntranceFee() public view returns (uint256) {
        ...
        uint256 adjustedPrice = uint256(price) * 1e10; // 18 decimals, price feeds already has 8 decimals
        ...
        /*uint256(
            keccak256(
                abi.encodePacked(
                    ...
                    block.difficulty, //manipulate
                    block.timestamp //predictable
                )
            )
        ) % players.length;*/
        //Unacceptable way to do fake randomness
        lottery_state = LOTTERY_STATE.CALCULATING_WINNER;
        bytes32 requestId = requestRandomness(keyHash, fee);
        //From VRFConsumerBase, VRF requires fundings, but the return is paid by the oracle
