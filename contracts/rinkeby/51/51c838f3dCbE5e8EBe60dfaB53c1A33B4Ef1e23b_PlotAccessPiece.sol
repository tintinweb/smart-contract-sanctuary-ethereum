...
contract PlotAccessPiece is IERC721Metadata, ERC721Enumerable, Ownable, ERC721Pausable {
    ...
    // uint256 public constant PRESAL_LIMIT = 1;
    ...
    constructor() ERC721("PlotAccessPiece", "PLOT") {
        setBaseURI("https://shibco-op.dev/access-piece/");
        saleLive = true;
    }
    ...
    function invalidatePresale(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            require(addresses[i] != address(0), "Not Early Adopter");
    ...
    function qtyAvailable(uint256 _count) internal view returns (bool) {
        if (_mintLimit > 0) {
            if (_totalSupply() + _count < _mintLimit) {
                return false;
            }
        }
    ...
    function presaleMint(address _to, uint256 _count) public payable {
        ...
        require(qtyAvailable(_count), "There are not enough Tokens remaining for this Request");
    ...
    function setMintLimit(uint256 totalQty) public onlyOwner {
        require(totalQty >= 0, "invalid");
        _mintLimit = totalQty;
    }
