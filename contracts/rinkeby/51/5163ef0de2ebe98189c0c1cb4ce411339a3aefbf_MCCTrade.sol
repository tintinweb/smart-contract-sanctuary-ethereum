...
contract MCCTrade is Initializable, OwnableUpgradeable {
    ...
    function addNewPack(string memory _pname, uint256 _pfee) public onlyOwner {
    ...
    function getPacks() public view returns (packInfo[] memory) {
        return _pName;
    }
    ...
    function buyPack(uint256 _pid, uint256 _nPack) public payable {
        require(
            _pName[_pid].packPrice.mul(_nPack) == msg.value,
            "Invalid Pack Price"
        );
        aPack[msg.sender] = aPack[msg.sender].add(_nPack);
        payable(owner()).transfer(msg.value);
    }

    function availablePack(address from) public view returns (uint256) {
        return aPack[from];
    }

    function decreasePack(address from) external {
