...
contract NFTCollection is ERC721A, Ownable {
    ...
    uint256 public PUBLIC_WHITELIST_PRICE = 0.001 ether;
    ...
    function whitelistMint(uint256 quantity, bytes32[] memory _merkleProof) external payable {
        ...
        require(quantity <= MAX_PER_WALLET - _presaleWallets[msg.sender], "exceeded max per wallet");
        require(totalSupply() + quantity <= MAX_SUPPLY - reserved, "exceed max supply of tokens");
        require(msg.value >= PUBLIC_WHITELIST_PRICE * quantity, "insufficient ether value");
    ...
    function changePublicWhitelistPrice(uint256 price) external onlyOwner {
        PUBLIC_WHITELIST_PRICE = price;
    }
