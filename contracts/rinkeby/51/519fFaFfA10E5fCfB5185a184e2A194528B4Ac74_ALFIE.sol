...
contract ALFIE is ERC721Tradable {
    ...
    uint256 public alfieReserve = 50;
    ...
    function reserveAlfies(address _to, uint256 _reserveAmount) public onlyOwner {
        ...
        // uint256 memory _startTime = startTime;
        ...
        require(supply + _reserveAmount <= maxAlfies, 'Purchase would exceed max supply of Alfies');
        for (uint256 i = 1; i <= _reserveAmount; i++) {
            _safeMint(_to, supply + i);
            _alfiesId.increment();
        }
        // [David] increments here all at once
    ...
    function presaleMintAlfie(uint256 numberOfTokens, bytes32[] memory proof) public payable {
        ...
        require(_alfiesId.current() - 1 + numberOfTokens <= maxAlfies, 'Purchase would exceed max supply of Alfies');
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = _alfiesId.current();
