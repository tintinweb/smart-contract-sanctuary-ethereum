...
import "./DerivedAPE.sol";



contract BOSSxNiftyTailor is DerivedAPE {

    using Strings for uint256;

    constructor( address _rr, address _newUriChanger) DerivedAPE("BOSSxNiftyTailor", "BOSSNIFTY", address(0), address(0), _rr, _newUriChanger) {}
    ...
    function getParents() public view virtual returns(address[4] memory){
        return [
            // mainnet
            0xBC4CA0EdA7647A8aB7C2061c2E118A18a936f13D, // BAYC
            0x60E4d786628Fea6478F785A6d7e704777c86a7c6, // MAYC
            0xBC4CA0EdA7647A8aB7C2061c2E118A18a936f13D, // WoW
            0x1A92f7381B9F03921564a437210bB9396471050C // Cool Cats
        ];
    }

    function mintDerived(mintBossData[] calldata data) external {
        uint i;
        for (i=0 ; i < data.length; i++) {
            _mintDerivedMulti( data[i].tokenId, data[i].mintpassId, data[i].contractId );
        }
    }

    function _mintDerivedMulti( uint256 tokenId, uint256 mintpassId, uint erc721id )  internal returns (uint256 newTokenId) {
        ...
        require( erc721id < getParents().length, "Wrong ERC721 ID");

        address erc721 = getParents()[erc721id];

        return _mintDerivedForParent(tokenId, mintpassId, erc721 );
        
    }

    function mintDerivedBAYCWithURI(mintDataWithUri[] calldata) external pure override {
        revert("disabled");
    }

    function mintDerivedMAYCWithURI(mintDataWithUri[] calldata) external pure override {
        revert("disabled");
    }

    function mintDerivedBAYC(mintData[] calldata) external pure override {
        revert("disabled");
    }

    function mintDerivedMAYC(mintData[] calldata) external pure override {
        revert("disabled");
    }

    function contractURI() public pure override returns (string memory) {
        return "https://niftytailor.com/contracts/boss.json";
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked("https://resources.niftytailor.com/", block.chainid.toString(), "/", contractAddress(), "/", tokenId.toString()));
    
    }

}

contract BOSSxNiftyTailorRinkeby is BOSSxNiftyTailor {

    address[4] private parents;

    constructor(address _rr, address _newUriChanger) BOSSxNiftyTailor(_rr, _newUriChanger) {
        if (block.chainid == 4) {
            parents = [
                // rinkeby 
                0xbBEdEBD3cA28D476FD231404616c2FceFCB7eDF1, // MAYC
                0x2F6F12b68165aBb483484927919D0d3fE450462E, // BAYC
                0x06936285c9ec8F029aFf20Ea7753428D927C79F8, // WoW
                0x196ca2366426EE7F0140C5DD088948b0CCb93921  // Cool Cats
                // derivativeApe: "0x47e451915Ff7CD79c008e15589387543f092cA0D",
            ];
        } else if (block.chainid == 31337){
            // hardhat internal chain
            parents = [
    ...
    function getParents() public view override returns(address[4] memory){
        return parents;
    }
...
contract DerivedAPE is ERC721URIStorage, UriChanger, DerivedERC2981Royalty, IERC721Enumerable {
    ...
    function mintDerivedBAYCWithURI(mintDataWithUri[] calldata data) external virtual {
        uint i;
        for (i=0 ; i < data.length; i++) {
    ...
    // We dont use tokenURI param. its just for external use, but should be attached to the request to connect token with temporary URI
    function mintDerivedMAYCWithURI(mintDataWithUri[] calldata data) external virtual {
        uint i;
        for (i=0 ; i < data.length; i++) {
    ...
    function mintDerivedBAYC( mintData[] calldata data )  external virtual {
        uint i;
        for (i=0 ; i < data.length; i++) {
            _mintDerived( data[i].tokenId, data[i].mintpassId, BAYC );
        }
    ...
    function mintDerivedMAYC( mintData[] calldata data )  external virtual {
        uint i;
        for (i=0 ; i < data.length; i++) {
            _mintDerived( data[i].tokenId, data[i].mintpassId, MAYC );
        }
    ...
    function getChildrenCounter(uint pointer) internal view returns (uint){
        return _childrenCounter[pointer];
    }

    function _mintDerivedForParent( uint256 tokenId, uint256 mintpassId, address erc721 ) internal returns (uint256 newTokenId) {
