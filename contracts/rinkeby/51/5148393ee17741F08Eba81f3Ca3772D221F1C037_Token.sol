...
contract Token is Ownable, ERC721A, ReentrancyGuard {
    ...
    bytes32 private merkleProofRoot;

    uint256 public constant AUCTION_PRICE = 0 ether;

    ERC721A public constant genesisToken =
        ERC721A(0x8c8130CEcC564706D0504b78aD3BF1EfcC46Cf32);

    mapping(uint256 => bool) private spentGenesisTokens;

    address private burnManager = address(0);
    ...
    modifier onlyBurnManager() {
        require(
            burnManager == msg.sender,
            "the caller is unauthorized to perform this operation"
        );
        _;
    }
    ...
    modifier callerIsUser() {
        require(
            ...
            "the caller must not be another contract"
        );
        _;
    }
        ...
        isWhitelisted(merkleProof)
    {
        ...
        require(
            ...
            "you are not allowed to mint this many tokens"
        );
    ...
    function genesisMint(uint256 tokenId) external payable callerIsUser {
        uint256 quantity = 1;
        require(
            totalSupply() + quantity <= totalAmount,
            "not enough tokens remaining"
        ...
        require(
            getOwner(tokenId) == msg.sender,
            "the caller is not the owner of the Genesis token"
        );
        require(
            spentGenesisTokens[tokenId] == false,
            "the Genesis token has already been spent"
        );
        spentGenesisTokens[tokenId] = true;
        mintQuantity(quantity);
    }
    ...
    function hasGenesisTokenBeenSpent(uint256 tokenId)
        external
        view
        returns (bool)
    {
        return spentGenesisTokens[tokenId];
    }

    function setBurnManager(address burnManagerAddress) external onlyOwner {
        burnManager = burnManagerAddress;
    }

    function burnToken(uint256 tokenId) external onlyBurnManager {
        transferFrom(ownerOf(tokenId), address(0), tokenId);
    }

    function setMerkleProofRoot(bytes32 newMerkleProofRoot) external onlyOwner {
        merkleProofRoot = newMerkleProofRoot;
    }
}
