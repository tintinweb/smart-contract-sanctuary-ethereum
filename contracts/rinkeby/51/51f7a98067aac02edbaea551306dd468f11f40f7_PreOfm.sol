...
contract PreOfm is ERC20("Pre Ocean Floor Music Token", "PreORM"), Ownable, Pausable {
    ...
    function multiTransfer(address[] memory _recipients, uint256[] memory _amount) external returns (bool) {
        require(_recipients.length == _amount.length, "number of recipients and amounts are not equal.");
        uint256 total = 0;
        ...
        emit MultiTransferred(_recipients.length, total);
        return true;
    }

    function multiTransfer2(address[] memory _recipients, uint256 _amount) external returns (bool) {
        uint256 total = _recipients.length.mul(_amount);
        require(balanceOf(msg.sender) >= total, "Insufficient Balance.");
        uint8 i = 0;
        for (i; i < _recipients.length; i++) {
    ...
    function balanceOf(address recipient) public view virtual override returns (uint256) {
        return super.balanceOf(recipient);
    }
