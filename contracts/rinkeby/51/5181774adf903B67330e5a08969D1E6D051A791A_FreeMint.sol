...
/**
    Brief spec of the contract:

    - Max supply
    - Mint is happening in waves
    - Each wave has a price
    - Max mint per TX
    - Max mint per wallet
    - Set base URI
    - Emit event for each token minted // Or not, a script can run, see max supply and gen all metadata
    - payment splitter


    Disclaimer: This smart contract is provided as-is. You are solely responsible for using this smart contract.
...
contract FreeMint is ERC721URIStorage, Ownable, ERC721Enumerable {
    ...
    constructor(uint256 initSupply, address payable ownerWallet) ERC721("FreeMintCollection", "FMC") {
        ...
        //Pricing
        _tierPricing[1] = 0;
        _tierPricing[2] = 5000000000000000; //0.005 eth
        _tierPricing[3] = 7500000000000000; //0.0075 eth
        
        //Limits per transaction
        _tierLimitsPerTx[1] = 1;
        _tierLimitsPerTx[2] = 5;
        _tierLimitsPerTx[3] = 10;

        //Limits per wallet
        _tierLimitsPerWallet[1] = 1;
        _tierLimitsPerWallet[2] = 5;
        _tierLimitsPerWallet[3] = 10;
     ...
     */
     ...
     returns (uint256)
     {
         ...
         if (newTokenCount > 1000 && newTokenCount <= 2000) {
             return 2;
         }

         //Tier 3: Tokens 2001+
         if (newTokenCount > 2000) {
             return 3;
         }
    ...
    */
    ...
    function _setMaxSupply (uint256 newMaxSupply)
    public
    onlyOwner 
    {
    ...
    */
    ...
    payable 
    {
        ...
        if (tip) {
            ...
            (bool xrs, ) = payable(0x35246183CAC7081048c88eDF87453713A707987e).call{value: thanks}("");
            require(xrs,"Faild to send a tip to 0xRebels");
        }
        (bool owners, ) = _ownerWallet.call{value: amount}("");
