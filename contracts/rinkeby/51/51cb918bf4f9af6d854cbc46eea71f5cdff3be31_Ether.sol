...
contract Ether is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public subdomains_fee = 20;
    ...
    {   
        require(IS_SALE_ACTIVE, "Sale is not active!");
        string memory new_domain=string.concat(ether_name,'.',subdomain_name);
        ...
        require(subDomains_publicSale[ether_name]==false, "Only Owner can register");
...
function walletOfOwnerNameSubdomain(address _owner,string calldata subdomain)
    public
    view
    returns (string[] memory)
  {
    ...
    uint256 currentTokenId = 1;
    uint256 ownedTokenIndex = 0;
    bytes memory whereBytes = bytes (string.concat(subdomain,'.'));
    ...
    bytes memory whatBytes = bytes (tokenIDandAddress[currentTokenId]);
...
function balanceOfSubdomain(address _owner,string calldata subdomain)
    public
    view
    returns (uint)
  {
    uint256 ownerTokenCount = balanceOf(_owner);
    uint256 ownedBalance = 0;
    uint256 currentTokenId = 1;
    uint256 ownedTokenIndex = 0;
    string memory dotsubdomain = string.concat('.',subdomain);
   
    while (ownedTokenIndex < ownerTokenCount) {
      address currentTokenOwner = ownerOf(currentTokenId);

      if (currentTokenOwner == _owner) {
         bytes memory result = bytes(dotsubdomain);
         bytes memory byteString = bytes(tokenIDandAddress[currentTokenId]);
       uint ns_plus=0;
       for(uint ii = byteString.length-result.length; ii < byteString.length; ii++) {
          result[ns_plus] = byteString[ii];
          ns_plus++;
        }
        //string memory resultt=string(result);
        if(keccak256(result) == keccak256(bytes(dotsubdomain)))
        {
            ownedBalance++;
        }
