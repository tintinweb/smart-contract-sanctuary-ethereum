...
contract MetaCOMAccessCardDutchAuctionMint is MetaCOMAccessCardLogic {
  ...
  */
  ...
  function mintAllowlist(bytes32[] calldata _merkleProof, uint256 mintAllocation, uint256 mintAmount) external override payable {
    ...
    //bytes memory xs = abi.encodePacked(msg.sender, startPrice);
  }
  ...
  function estimateRefund(address _address) external view returns(uint256) {
    if(!alRefundEnabled){
      return 0;
    }

    uint256 refundALcount = getALrefundCount(_address);
    if(refundALcount == 0){
      return 0;
    }
    ...
    uint256 total = 0;
    for(uint256 c = refundALcount; c > 0; c--){//max possible refundALcount is 31
        MintLog memory mintLog = alRefundDetails[_address][c-1];
        if(minPrice > 0 && mintLog.price > minPrice){
  ...
  function withdrawRefundPool() external onlyOwner {//no pagination
    ...
    for(uint256 c = 0; c < limit; c++){
      address _alAddress = alAddresses.at(c);
      delete alRefundDetails[_alAddress];
    }
    
    delete alAddresses;//double test required, if it indeed cleared all data
  ...
  function withdrawRefundPoolLimited(uint256 limit) external onlyOwner {//with pagination
    ...
    for(uint256 i = 0; i < limit; i++){
      address _alAddress = alAddresses.at(totalLength - i - 1);//to be able to remove address and don't reset the index
      alAddresses.remove(_alAddress);

      uint256 refundALcount = getALrefundCount(_alAddress);
      for(uint256 c = 0; c < refundALcount; c++){//max possible refundALcount is 31
          MintLog memory mintLog = alRefundDetails[msg.sender][c];
