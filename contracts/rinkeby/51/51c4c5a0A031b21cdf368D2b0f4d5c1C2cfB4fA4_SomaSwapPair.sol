...
import "../../SecurityTokens/ERC20PermitSecurity.sol";
...
contract SomaSwapPair is ISomaSwapPair, ERC20PermitSecurity {
    ...
    ERC20PermitSecurity(somaAddress) {}
    ...
    function initialize(address _token0, address _token1) external override {
        ...
        // TODO confirm whether soma should be uppercase
...
library GuardHelper {
    ...
    // require locked and blacklisted to be 1 at all times
    bytes32 internal constant DEFAULT_REQUIRED = bytes32(uint256(3));
...
import "./extensions/ControlledSecurity.sol";

import "hardhat/console.sol";

contract ERC20PermitSecurity is ERC20PermitUpgradeable, ControlledSecurity {
...
import "./IControlledSecurity.sol";

abstract contract ControlledSecurity is IControlledSecurity, GuardableUpgradeable {

    function __ControlledSecurity_init() internal {
        __Guardable__init();
        __ControlledSecurity_init_unchained();
    }

    function __ControlledSecurity_init_unchained() internal onlyInitializing {
        LOCAL_SEIZE_ROLE = keccak256(abi.encodePacked(address(this), GLOBAL_SEIZE_ROLE));
    }

    bytes32 public constant GLOBAL_SEIZE_ROLE = keccak256('ControlledSecurity.SEIZE_ROLE');
    bytes32 public LOCAL_SEIZE_ROLE;

    bool private seizeInProgress;
    ...
    function canTransferFrom(address _from, address _to, uint256, bytes memory) public virtual override view returns (bool) {
        ...
        return  (_from == address(0)    || _guard.check(_from, _access)) &&
                (_to == address(0)      || _guard.check(_to, _access));
    }
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(IControlledSecurity).interfaceId ||
                super.supportsInterface(interfaceId);
    }
    ...
    function _validateTransfer(
        ...
        // seizeInProgress is an admin function in which case we want to override any normal checks, and skip them.
        if (seizeInProgress) {
            return;
        }

        // SomaGuard check, for determining if each user has the requiredAccess for this security
        require(canTransferFrom(from, to, amount, data), 'SOMA: UNAUTHORIZED');

        // emergency pause logic for disabling any transfers during a global pause
...
abstract contract GuardableUpgradeable is IGuardable, AccessibleUpgradeable {
    ...
    function __Guardable__init_unchained() internal onlyInitializing {
        LOCAL_UPDATE_ACCESS_ROLE = keccak256(abi.encodePacked(address(this), GLOBAL_UPDATE_ACCESS_ROLE));
        _updateRequiredAccess(GuardHelper.DEFAULT_REQUIRED);
    }

    bytes32 public immutable DEFAULT_ACCESS = GuardHelper.DEFAULT_ACCESS;
...
interface IControlledSecurity {
    ...
    function canTransferFrom(address from, address to, uint256 amount, bytes memory data) external view returns (bool);
}
