...
contract AlphaBullsV2 is ERC721, Ownable {
    ...
    uint256 public freeMintSupply = 777;

    // Per Address & TX Limits
    uint256 public nftFreeMintPerAddressLimit = 1;
    ...
    // Minted Balances
    mapping(address => uint256) public addressFreeMintMintedBalance;
    ...
    // Constructor
    constructor(bytes32 _root) ERC721("Alpha Bulls", "AB") {
        setHiddenMetadataUri("ipfs://{CID}/");
        root = _root;
    }

    // Mint Compliance
    modifier mintCompliance(uint256 _mintAmount, bytes32[] memory proof) {
        if (freeMint == false && supply.current() >= freeMintSupply) {
            if (presale == true) {
                ...
                uint256 ownerMintedCount = addressPresaleMintedBalance[
                    msg.sender
                ];
                require(
                    ownerMintedCount + _mintAmount <= nftPresalePerAddressLimit,
                    "MSG: Max NFT per address exceeded for presale"
                );
            } else {
                uint256 ownerMintedCount = addressPublicsaleMintedBalance[
                    msg.sender
                ];
                require(
                    ownerMintedCount + _mintAmount <=
                        nftPublicsalePerAddressLimit,
                    "MSG: Max NFT per address exceeded for publicsale"
                );
            }
        } else {
            uint256 ownerMintedCount = addressFreeMintMintedBalance[msg.sender];
            require(
                ownerMintedCount + _mintAmount <= nftFreeMintPerAddressLimit,
                "MSG: Max NFT per address exceeded for free mint"
            );
        }
        ...
        require(
            supply.current() + _mintAmount <= maxSupplyLimit,
    ...
    function mint(uint256 _mintAmount, bytes32[] memory proof)
        public
        payable
        mintCompliance(_mintAmount, proof)
    {
    ...
    ) public mintCompliance(_mintAmount, proof) onlyOwner {
    ...
    // Set NFT Free Mint Per Address Limit
    function setNFTFreeMintPerAddressLimit(uint256 _cost) public onlyOwner {
        nftFreeMintPerAddressLimit = _cost;
    }
    ...
    function withdraw() public payable {
        (bool os, ) = payable(0xDc9AE3AE3Ec8acA2768e1690DD20c76942552Fcf).call{
    ...
    function _mintLoop(address _receiver, uint256 _mintAmount) internal {
        for (uint256 i = 0; i < _mintAmount; i++) {
            ...
            if (freeMint == true) {
                addressFreeMintMintedBalance[msg.sender]++;
