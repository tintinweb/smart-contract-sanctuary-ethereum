...
contract CrazyCarl is ERC1155, Ownable {
	...
	bytes32 public root = 0xca58eac98cf801780b3429b340c203bc9dc8a12fdf3b366c861cb65e8ca77dec;

	string public _baseURI = "ipfs://QmPEZGHQqUQpoTUknQ6ivxCRxQzyv3xgRJibHMnjCabeHC/";
	string public _contractURI = "ipfs://QmdrNApBH3WYuit8f7Dy4e3iRt6EbBa2LP7uPte2iVE7bt";
    ...
    function adminMint(address receiver, uint256 tier, uint256 qty) external onlyOwner {
	...
	function whitelistMint(
		...
		require(tier > 0 && tier <= maxTiers, "tier not in range");
		require(listPurchases[msg.sender] + qty <= tokenID, "wallet limit reached");
		require(tokensMinterTier[tier] + qty <= 1111, "tier out of stock");
		require(tokensMinted + qty <= unlockedSupply, "out of stock");
	...
	function setWhitelistSaleTime(uint256 _startTime, uint256 _endTime) external onlyOwner {
	...
	function setPublicSaleTime(uint256 _startTime, uint256 _endTime) external onlyOwner {
		require(_endTime > _startTime, "endtime > starttime");
		require(_endTime > block.timestamp, "endtime > block timestamp");
	...
	function setNewUnlockPeriod(uint256 _qty, uint256 _price) external onlyOwner {
		unlockedSupply = _qty;
