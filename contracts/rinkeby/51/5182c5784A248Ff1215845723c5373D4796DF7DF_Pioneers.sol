...
contract Pioneers is IERC721, IERC721Metadata{ 
    ...
    uint constant MINT_COST = 0.01 ether;  //TODO:
    ...
    uint constant CHANGE_TRAIL_BASE_PRICE = 0.02 ether; //TODO:
    ...
    bool _startJourney = false;

    function startJourney() public{
        require(msg.sender == owner,"owner");
        _startJourney = true;
    }

    function stage() public view returns(Stage){ // TODO: Enable block.timestamp check
        if(startTime == 0){
             return Stage.Initial;
        }else if(!_startJourney /*&& block.timestamp < startTime + SALE_TIME && tokenIndexToWagon.length < MAX_WAGONS*/){
    ...
    function steer(uint16 _wagonId, uint8[2] calldata _items, bytes1 firstLetter, uint8 _trail, bytes32[] memory _proof) public payable {
        if(msg.sender != owner) {
            ...
            //    require(XXXAccess(access).hasAccess(accessProof,msg.sender),"permission");
            //}
        ...
        // require(MerkleProof.verify(_proof, wagonRoot, leaf), "proof"); // TODO: Reenable MerkleProof

        //wagon has not been steered - XXX adds && coordinates[_cityId][0] == 0 && coordinates[_cityId][1] == 0.
    ...
    function ambush(uint _ambusherTokenId, uint _victimTokenId) public{
        ...
        if(_ambushes < _supplies){
            ...
            /*if(collectibles != address(0)){
                IXXXCollectibles(collectibles).mint(owners[_tokenId],tokenToCity(_tokenId));
            }*/
        }
...
contract PioneersTrails is KeeperCompatibleInterface{
    ...
    function setPioneers(address _pioneers) public{
        //require(msg.sender == pioneers,"sender"); // TODO: Renable for security purposes
        pioneers = _pioneers;
    }
    ...
    function generateInconveniencesForNextTrails() private{

        // pidyjmcwhxbrq
        bytes16 allInconveniences = "pidyjmcwhxbrq";

        IPioneers.Stage _nextStage = IPioneers(pioneers).stage();
    ...
    function checkUpkeep(bytes memory /* checkData */) public view override returns (bool upkeepNeeded, bytes memory /* performData */) {
        require(IPioneers(pioneers).stage() != IPioneers.Stage.Initial 
        && IPioneers(pioneers).stage() != IPioneers.Stage.Minting);
        upkeepNeeded = (getEncodedInconveniencesByTrail(7)[0] == 0x00);
    }
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        ...
        {
            generateInconveniencesForNextTrails();
        }
    }
