...
contract Rewarder is ReentrancyGuard, Pausable, IRewarder, AccessControl {
        ...
        returns (uint256 totalAvailable, uint256 rewardAmount)
    {
        ...
        totalAvailable = _stake.userStakeRewardPerToken.mul(block.timestamp - _stake.lastClaimed).add(_stake.rewards);
    ...
    function claimRewards(uint256[] calldata stakingPools) public nonReentrant onlySagePath {
        ...
        for (uint256 i = 0; i < stakingPools.length; i++) {
            ...
            (, uint256 rewardsAmount) = getStakingReward(msg.sender, poolIndex);
    ...
    modifier updateReward(address user, uint256 duration) {
        ...
        if (user != address(0)) {
            Stake storage _stake = _stakes[user][duration];
            (uint256 totalAvailable, uint256 rewardAmount) = getStakingReward(user, duration);
