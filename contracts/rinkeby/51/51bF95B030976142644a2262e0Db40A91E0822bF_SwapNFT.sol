...
contract SwapNFT is Accept {
    ...
    function purchaseOrder(uint256 orderNumber,string memory _tokenName) public payable { // Buy Order by other Token
       ...
       payable(IERC721(order.nftAddress).creator(order.tokenId)).transfer(_royality);
        ...
        // console.log("order.seller", order.seller,msg.sender );
         if(!order.isSingle){
            //  console.log("!is single");
            for(uint256 i = 0; i< order.creators.length ; i++){
                uint256 calAmount = (order.sameCreator[order.creators[i]] * (_royality)) /(order.bundleIds.length);
                // console.log("create number of nfts per order",(order.sameCreator[order.creators[i]]));
                // console.log("Royalty",_royality,"order Length",order.bundleIds.length);
                // console.log("Amount",calAmount );y
                IBEP20(order.tokenAddress).transferFrom(msg.sender,IERC721(order.nftAddress).creator(order.sameCreator[order.creators[i]]),calAmount);
            }
           } else
        {
            // console.log("is it calling on else part");
            // console.log("msg.sender", msg.sender,"Creator", creatorAdd);
            // console.log("_royality", _royality);
        IBEP20(order.tokenAddress).transferFrom(msg.sender,creatorAdd,_royality);
        // console.log(msg.sender,creatorAdd,_royality);
