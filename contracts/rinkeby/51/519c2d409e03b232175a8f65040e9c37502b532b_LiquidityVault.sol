...
contract LiquidityVault is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    // ERC20 token being held by this contract
    IUniswapV2Router02 public uniswapV2Router;

    receive() external payable {}

    function initialize(address _uniswapAddress, address newOwner)
        public
        initializer
    {
        __Ownable_init();
    ...
    function doLiqStuffCustom(
        address weth,
        address tokenAddr,
        address uniswapPairAddr
    ) public onlyOwner {
        IERC20Upgradeable uniswapPair = IERC20Upgradeable(uniswapPairAddr);
        uint256 uniswapBal = uniswapPair.balanceOf(address(this));
        uniswapPair.approve(address(uniswapV2Router), uniswapBal + 100);
    ...
    function doLiqStuff(address tokenAddr, address uniswapPairAddr)
        public
        onlyOwner
    {
        IERC20Upgradeable uniswapPair = IERC20Upgradeable(uniswapPairAddr);
    ...
    function withdrawErc20tokenForSelf(
    ...
    function withdrawETH(address recipient) public onlyOwner {
        ...
        require(successDev, "Error Sending tokens to dev");
    }
}
