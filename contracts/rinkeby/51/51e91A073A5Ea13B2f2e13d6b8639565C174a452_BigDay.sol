...
contract BigDay is Ownable, ERC721Royalty, Pausable, ReentrancyGuard, SignatureValidator, TokenAsDate {
    ...
    function mint(MintPayload calldata voucher, bytes calldata signature)
        external
        payable
        ...
        whenNotPaused
    {
        ...
        token.attributes = BigDayTokens.Attributes(
            voucher.year,
            voucher.month,
            voucher.day,
            voucher.dateDispl,
    ...
    function addCover(
        ...
        _tokens[tokenId].addCover(maxCoversAmount, collection, artId, label, desc, msg.sender);
    }
...
contract SignatureValidator is EIP712 {
    ...
    bytes32 private constant PERMIT_SIGNATURE =
        keccak256(
            "EIP712(uint256 tokenId,string label,string desc,string dateDispl,string year,string month,string day,string genId,uint256 ts,bytes32 nonce)"
        );
    ...
    function _verifySignature(
        ...
        bool isVerified = _verify(
            msg.sender,
            _hash(tokenId, label, desc, dateDispl, year, month, day, genId, ts, nonce),
            signature
        );
...
library BigDayTokens {
        ...
        returns (JsonWriter.Json memory writer)
    {
        ...
        self._addDateAttribute(writer, "Date", self.attributes.ts);
