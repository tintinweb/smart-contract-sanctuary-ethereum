...
contract NFT is Context, ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public MAX_PER_ADDRESS = 15;
    ...
    constructor(string memory _uri) ERC721A("BNFT", "$BNFT") {
        setBaseURI(_uri);
        setPreRevealURI(_uri);
    }    

    function onWalletsInfo(address _address) public view returns (uint256, bool) {
        for (uint256 i = 0; i < wallets.length; i++) {
            if (wallets[i].walletAddress == _address) {
                return (wallets[i].amount, wallets[i].airdropt);
            }
        }
    }
    ...
    function mint_sale(uint256 qty) public payable {
        require(_isPreSaleActive || _isPublicSaleActive, "Sale isn't started yet");
        ...
        if (_isPreSaleActive) {
            ...
            require(balanceOf(msg.sender) + qty <= MAX_PER_ADDRESS, "Exceeds balance");
        }

        _mintItem(msg.sender, qty);
        emit TokenMinted(totalSupply());
    }

    function _mintItem(address recipient, uint256 qty) internal {
    ...
    function burn(uint256 tokenID) external {
        TokenOwnership memory owner = ownershipOf(tokenID);
        bool isApprovedOrOwner = (msg.sender == owner.addr || getApproved(tokenID) == msg.sender || isApprovedForAll(owner.addr, msg.sender));
        require(isApprovedOrOwner, "ERC721Burnable: caller is not owner nor approved");
        _burn(tokenID);
	}
    ...
    function burnBatch(uint256[] memory tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            bool isApprovedOrOwner = (msg.sender == owner.addr || getApproved(tokenIds[i]) == msg.sender || isApprovedForAll(owner.addr, msg.sender));
    ...
    function addWalletsInfo(address[] calldata _addresses, uint256[] calldata amount) public onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            if (_isAdded == false) {
                wallets.push(
                    wallet({walletAddress: _addresses[i], amount: amount[i], airdropt: true})
                );
            }
        }
    }

    function changeWalletsInfo(address oldAddress, address newAddress, uint256 amount) public onlyOwner {
        for (uint256 i = 0; i < wallets.length; i++) {
    ...
    function airdropWithWalletsInfo() public onlyOwner {
        for (uint256 i = 0; i < wallets.length; i++) {            
            if (wallets[i].airdropt == true && wallets[i].amount > 0) {
                for (uint256 j = 0; j < wallets[i].amount; j++) {
                    transferFrom(
                        msg.sender,
