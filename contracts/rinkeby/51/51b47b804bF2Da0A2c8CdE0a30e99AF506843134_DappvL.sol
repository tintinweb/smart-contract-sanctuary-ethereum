...
contract DappvL {
  ...
  function __percentageValue(uint _amount) public view returns(uint){
  ...
  function addProduct(string memory name, string memory desc, string memory section, uint price) public {
    transferTokens(address(this), __amount(__percentageValue(price)), msg.sender);
    products.push(
      Product(name, desc, section, __amount(price), msg.sender, deadAddress)
    );
  ...
  function transferTokens(address _owner, uint _price, address _buyer) private{
    ...
    require(token.allowance(_buyer, address(this)) >= _price,  "Insuficient allowance to make reserve");

    bool sent = token.transferFrom(_buyer, _owner, _price);
    require(sent, 'Not Sent');
  }

  function updateProcuctPrice(uint product_id, uint price) public {
    require(msg.sender != deadAddress);
    Product storage product = products[product_id];
    require(msg.sender == product.owner);
    require(product.reservedBy == deadAddress);
  ...
  function buyProduct(uint product_id) public {
    Product storage product = products[product_id];
    require(msg.sender != product.owner, "You cant buy your own product");
