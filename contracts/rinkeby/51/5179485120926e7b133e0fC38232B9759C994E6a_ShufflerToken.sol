 ...
 * @notice Require that the descriptor has not been locked.
    ...
    function _internalMint(address to, uint256 count) private {
        ...
        _mint(to, count);
        emit TokenCreated(_msgSenderERC721A(), startTokenId, startTokenId + count - 1);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (isRevealed) {
            ...
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked('{"name":"Onchain Shuffler #', tokenId, '", "description":"world\'s first on-chain composable NFT ", "image": "', descriptor.baseURI(), '"}')
                    )
                )
            );
        }
     ...
     */
    function setOverrideNft(uint256 tokenId, address overrideContract, uint64 overrideTokenId) external override {
        ...
        require(_verifyOwnerOfOther(overrideContract, overrideTokenId), 'Only owner of the target token can compose');
        ...
        emit NftOverriden(tokenId, overrideContract, overrideTokenId);
    }

    function _verifyOwnerOfOther(address overrideContract, uint64 overrideTokenId) internal returns (bool) {
        // todo: handle special contracts like punk
...
interface IShufflerToken {
    event TokenCreated(address indexed creator, uint256 indexed startTokenId, uint256 indexed endTokenId);
