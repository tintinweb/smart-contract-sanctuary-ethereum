...
contract TRexPotions is ERC1155, Ownable {
    ...
    function mintBatch(uint256[] memory ids, uint256[] memory amounts) payable public {
        uint256 totalOne = amounts[0];
        uint256 totalTwo = amounts[1];
        uint256 totalThree = amounts[2];
        uint256 totalFour = amounts[3];
        uint256 total = totalOne + totalTwo + totalThree + totalFour;
        // uint added = total * price;
        require(msg.value > 0, "not enough ether sent");
        require(msg.value >= (total * price), "not enough ether sent"); 
