...
contract NFTStakingPoolFunctions is AccessControl {
  ...
  function initLevels(uint8[] memory levels) external onlyOwner {
    uint256 _rainiNftCount = nftStakingPool.rainiNftCount();
    ...
    for (uint256 i; i < levels.length; i++) {
      ...
      nftStakingPool.setRainiNft(uint24(_rainiNftCount), INFTStakingPool.RainiNft({
        lastStamina: 0,
        maxStamina: 0,
        level: levels[i],
        lastUpdated: 0,
    ...
    nftStakingPool.setRainiNftCount(_rainiNftCount);
    nftStakingPool.setStaminaEventCount(_staminaEventCount);
  }

  function calculateRewardPerToken(uint32 duration, uint32 rainiRewardRate, uint32 _totalSupply) private pure returns (uint56 rewardPerToken) {
    if (_totalSupply == 0) {
      return 0;
    }
    return uint56((uint256(duration) * rainiRewardRate * RAINI_REWARD_DECIMALS) / _totalSupply / REWARD_PER_STAKED_DECIMALS);
  }
  ...
  function getRewardPerToken(uint24 _eventId, uint24 _fromEvent) public view returns (INFTStakingPool.StaminaEvent memory se) {
    ...
    require (rainiRewardPerTokenStored > 0, 'invalid start 1');
  ...
  function balanceUpdate(address _owner, uint24[] memory _eventIds, uint32[] memory nftOrder, int256 _dataUpdate, uint256 _level) internal {
      ...
      uint32[] memory _stakedNFTs = nftStakingPool.stakedNFTs(_msgSender(), _level);
      ...
      for (uint256 n = 0; n <= nftOrder.length; n++) {
        ...
        if (n < nftOrder.length) {
          ...
          nft = INFTStakingPool.RainiNft(0,0,0,0,0,false,false);
        ...
        if (n == nftOrder.length || (_nftEvent.timeStamp > _rewardVars.lastUpdated && _nftEvent.timeStamp <= block.timestamp)) {
          ...
          if (n == nftOrder.length || _nftEvent.timeStamp != lastDate) {
            ...
            _accountVars.rainiRewards += uint104((uint256(_rewardVars.staked) * (_se.rainiRewardPerTokenStored - _rewardVars.rainiRewardPerTokenPaid)) * REWARD_PER_STAKED_DECIMALS / RAINI_REWARD_DECIMALS);
          ...
          if (n == nftOrder.length) {
            if (_owner == address(0)) {
              _se.rainiRewardRate = uint32(uint(_dataUpdate));
            } else {
  ...
  function stake(uint32[] memory _tokenId, address[] memory _contractAddress, uint24[] memory _staminaEventId, uint24[][] memory _eventIds, uint32[][] memory _nftOrder)
    external {
      ...
      uint256[] memory supplyAdded = new uint256[](_eventIds.length);


      for (uint256 i = 0; i < _tokenId.length; i++) {
        require((nftStakingPool.approvedTokenContracts(_contractAddress[i])), "NFTSP: invalid token contract");
        ...
        tokenContract.safeTransferFrom(_msgSender(), address(this), _tokenId[i], 1, bytes('0x0'));

        uint256 _stamina;

        uint256 nftId = nftStakingPool.rainiNftIdMap(_contractAddress[i], _tokenId[i]);
        
        if (nftId == 0) {
          _stamina = nftStakingPool.initNft(_contractAddress[i], _tokenId[i], 0, true, uint24(nftStakingPool.staminaEventCount() + 1));
          nftId = nftStakingPool.rainiNftCount();
        } else {
          INFTStakingPool.RainiNft memory _nft = nftStakingPool.rainiNfts(nftId);
          _stamina = _nft.lastStamina;
          _nft.isStaked = true;
          _nft.lastUpdated = uint32(block.timestamp);
          _nft.staminaEventId = _stamina > 1 ? uint24(nftStakingPool.staminaEventCount() + 1) : 0;
          nftStakingPool.setRainiNft(uint24(nftId), _nft);
        }

        if (_stamina > 1) {
          uint256 endTime = block.timestamp + (_stamina * nftStakingPool.staminaDuration()) / _cl.maxStamina;
          ...
          INFTStakingPool.StaminaEvent memory _se = INFTStakingPool.StaminaEvent({
            ...
            totalSupply: 0,
            rainiRewardRate: 0,
            level: uint8(nftStakingPool.rainiNfts(nftId).level)
          });

          nftStakingPool.insertStaminaEvent(_staminaEventId[i], _se, _tv.level);

          supplyAdded[_tv.level] += _cl.maxStamina;
        }

        nftStakingPool.addStakedNft(_msgSender(), _tv.level, uint32(nftId));
      }

      for (uint256 i = 0; i < _eventIds.length; i++) {
        require(supplyAdded[i] <  2 ** 15, 'staking too much');
        if (supplyAdded[i] > 0) {
          //make sure a balance update is set for the level
          require(_eventIds.length >= i && _eventIds[i].length > 0, "Invalid fields");
          balanceUpdate(_msgSender(), _eventIds[i], _nftOrder[i], int(supplyAdded[i]), i);
        }
      }
  ...
  function withdrawTokens(uint256[] memory _tokenId, address[] memory _contractAddress, uint24[] memory _prevEventId, uint24[][] memory _eventIds, uint32[][] memory _nftOrder)
    ...
    for (uint256 i = 0; i < _tokenId.length; i++) {
      ...
      INFTStakingPool.RainiNft memory _nft = nftStakingPool.rainiNfts(_nftId);
      require(_eventIds.length >= _nft.level + 1 && _eventIds[_nft.level].length > 0, "Invalid fields");

      bool ownsNFT = nftStakingPool.removeStakedNft(_msgSender(), _nft.level, uint32(_nftId));
      require(ownsNFT == true, "NFTSP: Not the owner");
      
      nftStakingPool.removeStaminaEvent(_nft.staminaEventId, _prevEventId[i]);

      supplyRemoved[_nft.level] += _nft.maxStamina;

      _nft.lastStamina = uint32(nftStakingPool.getTokenStaminaTotal(_tokenId[i], _contractAddress[i]));
      _nft.isStaked = false;
      _nft.lastUpdated = uint32(block.timestamp);
      _nft.staminaEventId = 0;
      ...
      nftStakingPool.setRainiNft(uint24(_nftId), _nft);
    }
    
    for (uint256 i = 0; i < _eventIds.length; i++) {
      require(supplyRemoved[i] <  2 ** 15, 'removing too much');
      if (supplyRemoved[i] > 0) {
        //make sure a balance update is set for the level
        require(_eventIds.length >= i && _eventIds[i].length > 0, "Invalid fields");
  ...
  function addRainiRewardPool(uint256 _amount, uint256 _duration, uint32 _level, uint24[] memory _eventIds)
    external onlyOwner {
      ...
      nftStakingPool.rainiToken().safeTransferFrom(_msgSender(), address(this), _amount);
      _generalRewardVars.rainiRewardRate = uint32(_amount / _duration);
  ...
  function withdrawReward(uint24[][] memory _eventIds, uint32[][] memory _nftOrder) external {
    ...
    nftStakingPool.transferRaini(_msgSender(), _accountVars.rainiRewards);
...
interface INFTStakingPool {
  ...
  function stakedNFTs(address _account, uint256 _level) external view returns (uint32[] memory);
  ...
  function approvedTokenContracts(address _contractAddress) external view returns (bool);
  ...
  function addStakedNft(address _account, uint256 _level, uint32 _nftId) external;

  function removeStakedNft(address _account, uint256 _level, uint32 _nftId) external returns (bool);
  ...
  function removeStaminaEvent(uint24 _removeId, uint24 _prevId) external;
