...
contract Minter is IMinter, Ownable {
    ...
    //  * publicMint
    //  */
    function setPublicMintPrice(uint64 price) external override onlyOwner {
    ...
    function _batchMint(address to, uint16 quantity) internal {
        ...
        IPanda(_panda).mint(to, quantity);
    }
    ...
    function _getMaxSupply() internal returns (uint256) {
        return IPanda(_panda).getMaxSupply();
    }

    function _getOverall() internal returns (uint256) {
        return IPanda(_panda).getOverall();
    }
}
...
interface IMinter {
    ...
    event PublicMintPriceChanged(uint64 price);
    ...
    function setPublicMintPrice(uint64 price) external;
