...
contract TestNft is Context, AccessControlEnumerable, ERC721Enumerable {
    ...
    // Immutables are set in constructor and cannot be changed.
    uint256 private immutable _collectionSize; // How many NFTs.
    address payable immutable _beneficiary; // Where is the money sent.
    ...
    // Make the token metadata JSON URI
    ...
    function mint(uint256 amount, uint256 slotId, uint256 validUntil, bytes memory signature) external payable {
        ...
        // this inline saves very little gas. not worth it probably. ~100.
        require(keccak256(abi.encodePacked(msg.sender, amount, validUntil, slotId)).toEthSignedMessageHash().recover(signature) == _signer, "TestNft: must have valid signing");
        ...
        require(totalTokens + amount <= _collectionSize, "TestNft: total amount must not be over the max");
    ...
    function batchMint(address[] memory accounts, uint256[] memory amounts) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(nextTokenId <= _collectionSize, "TestNft: Minting over max total supply");
    }
    ...
    function _verifySignature(address minter, uint256 amount, uint256 slotId, uint256 validUntil, bytes memory signature) internal view returns (bool) {
        return _getSigner(keccak256(abi.encodePacked(minter, amount, validUntil, slotId)), signature) == _signer;
    }
    ...
    function _canMint(address minter, uint256 amount, uint256 slotId, uint256 validUntil, bytes memory signature) internal view returns (bool) {
        return _verifySignature(minter, amount, slotId, validUntil, signature);
    }
