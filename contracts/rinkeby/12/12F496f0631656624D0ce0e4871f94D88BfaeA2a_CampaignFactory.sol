...
contract CampaignFactory is OwnableUpgradeable, PausableUpgradeable {
     ...
     * @return campaigns Created Campaign Address
     */
    ...
    {
        length = getCampaigns[_token].length;
    }

    /**
     * @notice Get platform fee in percent
     * @dev Created campaign call this function for get platform fee
     * @return feeRate Return number of platform fee
     */
    function getPlatformFeeRate() external view returns (uint256 feeRate) {
     ...
     * @return revenueAddress Address of Platform Vault
     */
    function getplatformRevenueAddress()
        external
        view
        returns (address revenueAddress)
    {
        revenueAddress = platformRevenueAddress;
    }
    ...
    function registerCampaign(
        ...
        require(_openTime >= block.timestamp, "ICOFactory::INVALID_OPEN_TIME");
        require(
            _openTime.add(_duration) < _releaseStartDate,
            "ICOFactory::INVALID_OPEN_TIME"
        );
        require(
...
contract Campaign is ReentrancyGuard, Pausable, TierSystem, Blacklist {
    ...
    function initialize(
        ...
        _setupTierSystem(_stakBankAddress, _openTime);
    ...
    function loadCampaign(
        ...
        _setTiers(_tier4, _tier3, _tier2, _tier1, _tier0);
...
contract TierSystem is Ownable {
    ...
    uint256 public tier2Time = 3600;
    uint256 public tier3Time = 7200;
    uint256 public tier4Time = 10800;
    uint256 public tier0Time = 14400;    
    ...
    // Phases A = 0 - 60m: TIER 1 (2,000,000 STAK)
    // Phases B = after 60m: TIER 2 (500,000 STAK)
    // Phases C = after 120m: TIER 3 (100,000 STAK)
    // Phases D = after 180m: whitelisted (10,000 STAK - no staking)
    // Phases E = after 240m: non-whitelisted (NO STAK)


       if (stakBank.balanceOf(_address) >= tier1 )  {

           tier = 1;
            timestamp = campaignOpenTime;
       }  else if (stakBank.balanceOf(_address) >= tier2) {
            tier = 2;
            timestamp = campaignOpenTime.add(tier2Time);
       } else if (stakBank.balanceOf(_address) >= tier3) {
            tier = 3;
            timestamp = campaignOpenTime.add(tier3Time);
       }else if (stakBank.balanceOf(_address) >= tier4 || whitelist[_address]) {
            tier = 4;
            timestamp = campaignOpenTime.add(tier4Time);
       
       }else {
            tier = 0;
            timestamp = campaignOpenTime.add(tier0Time);
       }  
    }
    ...
    function _isTiered(address _address) internal view returns (bool tier) {
        ...
        } else if (block.timestamp >= campaignOpenTime.add(tier4Time) || whitelist[_address] ) {
            tier = stakBank.balanceOf(_address) >= tier4;
        }else {
            tier = stakBank.balanceOf(_address) >= tier0; 
        }
    }

    function _setupTierSystem(IStakBank _stakBank, uint256 _openTime) internal {
        stakBank = _stakBank;
