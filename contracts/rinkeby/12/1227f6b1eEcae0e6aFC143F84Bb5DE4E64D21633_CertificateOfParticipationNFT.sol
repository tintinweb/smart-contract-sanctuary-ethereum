...
contract CertificateOfParticipationNFT is ERC721URIStorage {
    ...
    constructor() ERC721("TECHTALK Certificate Of Participation EP:01 -- test", "COPT") {
    }
    ...
    function generateTokenURL(string memory participentName) internal pure returns (string memory){
        ...
        if (keccak256(bytes(participentName)) == keccak256(bytes(empty))) {
            json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Smart Contracts: TECHTALK EP:1", "description": "This NFT certify that the original holder joined the Smart Contracts training session conducted by OrangeHRM", "image_data": "', imageURL, '"}'))));
        } else {
            // Not supporting special characters. Adding special characters may corrupt your NFTs meta data
            json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Smart Contracts: TECHTALK EP:1", "description": "This NFT certify that the ', participentName ,' joined the Smart Contracts training session conducted by OrangeHRM", "image_data": "', imageURL, '"}'))));
        }
