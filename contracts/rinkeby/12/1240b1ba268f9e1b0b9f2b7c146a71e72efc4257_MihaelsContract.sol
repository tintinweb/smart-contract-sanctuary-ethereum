...
contract MihaelsContract is ERC721A, Ownable {
    ...
    function mintingCommon(uint256 count) public {
        require(
            currentTokenIdCommon.current() < TOTAL_SUPPLY_COMMON,
            "Max supply reached"
        );
    ...
    function mintingRare(uint256 count) public payable {
        require(
            currentTokenIdRare.current() < TOTAL_SUPPLY_RARE,
            "Max supply reached"
        );
        require(
            msg.value >= mintPriceRare,
    ...
    function mintingEpic(uint256 count) public payable {
        require(
            currentTokenIdEpic.current() < TOTAL_SUPPLY_EPIC,
            "Max supply reached"
        );
        require(
            msg.value >= mintPriceEpic,
            "Transaction value did not equal the mint price"
        );
        minting(20000, count);
    }
    ...
    function mintingLegendary(uint256 count) public payable {
        require(
            currentTokenIdLegendary.current() < TOTAL_SUPPLY_LEGENDARY,
            "Max supply reached"
        );
        require(
            msg.value >= mintPriceLegendary,
            "Transaction value did not equal the mint price"
        );
        minting(30000, count);
    }
