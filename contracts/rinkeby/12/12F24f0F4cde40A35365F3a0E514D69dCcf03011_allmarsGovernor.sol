...
contract allmarsGovernor is ERC721Holder, Ownable {
    ...
    uint256 public expireTerm = 20 * 60;


//    uint256 public rate = 50000;
//    uint256 public expireTerm = 30 * 24 * 60 * 60;
    ...
    function depositNFT(uint256 _tokenId) public isInitiated {
        require(IERC20(erc20Contract).balanceOf(address(this)) >= rate * 10 ** 18, "Insufficient ERC20 balance on contract");
        ...
        IERC20(erc20Contract).safeTransfer(_msgSender(), rate * 10 ** 18);
    }
    ...
    function isExpired(uint256 tokenId) public view returns(bool) {
        return _deposits[tokenId].timestamp + expireTerm < block.timestamp;
    }
    ...
    function claimNFT(uint256 tokenId, address token) public isInitiated {
        ...
        if (_deposits[tokenId].timestamp + 86400 > block.timestamp) {
            IERC20(erc20Contract).safeTransferFrom(_msgSender(), address(this), rate * 10 ** 18);
            sendNFT(_msgSender(), tokenId);
        } else {
            if (token == wethContract) {
                IERC20(wethContract).safeTransferFrom(_msgSender(), address(this), 1 * 10 ** 18);
                sendNFT(_msgSender(), tokenId);
                IERC20(wethContract).safeTransfer(bpoolContract, 8 * 10 ** 17);
