...
contract Stake is Initializable, ContextUpgradeable, UUPSUpgradeable, Whitelist{
    ...
    function updateAccount(address account) private {
        ...
        if (pendingDivs > 0) {
            ...
            require(
                Token(depositToken).transfer(account, amountAfterFee.div(userDetails[account].cmctStakePrice).mul(1000)),
                "Could not transfer tokens."
            );
            require(
                Token(depositToken).transfer(owner(), fee.div(userDetails[account].cmctStakePrice).mul(1000)),
                "Could not transfer tokens."
            );
    ...
    function depositUSDT(address userAddress, uint256 amountToStake) public {
        ...
        userDetails[userAddress].depositedTokens = userDetails[userAddress].depositedTokens.add(
            amountToStake.div(1e18).div(cmctRate)
        );
