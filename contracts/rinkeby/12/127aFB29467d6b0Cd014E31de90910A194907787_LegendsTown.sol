...
contract LegendsTown is ERC721A, Ownable, ReentrancyGuard {
    ...
    string internal baseUri = "ipfs://QmY1uEa7wwdcQWNXtT8KLgWb2qYptxKqLhxfwpVQUewUUL/";
    uint256 public mintCost = 0.01 ether;
    uint256 public reservedPublic = 3555;
    uint256 public maxSupply = 5555;
    uint256 public reservedFree = 1000;
    ...
    function mintFree(bytes32[] memory proof) external nonReentrant {
        ...
        require(verifyRoot(proof), "Youre not eligible.");
        require(freeMint[msg.sender] < 1, "You've already minted your Free Legend.");
        require(totalSupply() + freeMintAmount <= reservedFree, "Max supply for Free mints has been reached.");
    ...
    function mintPublic(uint256 _amount) external payable nonReentrant {
        require(saleStarted, "Sale hasnt commenced yet.");
        require(_amount <= maxPerTXN && _amount > 0, "Max 10 per TXN.");
        require(_amount + totalSupply() <= reservedPublic, "There's not enough supply left.");
        require(msg.value >= mintCost * _amount, "One Legend costs 0.01 Ether.");
    ...
    function verifyRoot(bytes32[] memory proof) internal view returns (bool) {
