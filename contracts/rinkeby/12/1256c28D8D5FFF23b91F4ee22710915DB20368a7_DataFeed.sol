...
import {IDataFeed, IConnextSenderAdapter, IUniswapV3Pool, IBridgeSenderAdapter} from '../interfaces/IDataFeed.sol';
...
contract DataFeed is Governable, IDataFeed {
  ...
  function sendObservation(
    IBridgeSenderAdapter _bridgeAdapter,
    uint16 _chainId,
  ...
  function whitelistAdapter(IBridgeSenderAdapter _bridgeAdapter, bool _isWhitelisted) external onlyGovernance {
    _whitelistAdapter(_bridgeAdapter, _isWhitelisted);
  }

  function whitelistAdapters(IBridgeSenderAdapter[] calldata _bridgeAdapters, bool[] calldata _isWhitelisted) external onlyGovernance {
  ...
  function setReceivers(
    IBridgeSenderAdapter[] calldata _bridgeAdapters,
  ...
  function _whitelistAdapter(IBridgeSenderAdapter _bridgeAdapter, bool _isWhitelisted) internal {
...
interface IDataFeed is IGovernable {
  ...
  function whitelistedAdapters(IBridgeSenderAdapter _bridgeAdapter) external view returns (bool _isWhitelisted);

  function receivers(IBridgeSenderAdapter _bridgeAdapter, uint32 _destinationDomainId) external view returns (address _dataReceiver);

  function destinationDomainIds(IBridgeSenderAdapter _bridgeAdapter, uint16 _chainId) external view returns (uint32 _destinationDomainId);

  // EVENTS

  event DataSent(IBridgeSenderAdapter bridgeAdapter, address dataReceiver, uint32 destinationDomainId, uint32 blockTimestamp, int24 tick);

  event AdapterWhitelisted(IBridgeSenderAdapter bridgeAdapter, bool isWhitelisted);

  event ReceiverSet(IBridgeSenderAdapter bridgeAdapter, uint32 destinationDomainId, address dataReceiver);

  event DestinationDomainIdSet(IBridgeSenderAdapter bridgeAdapter, uint16 chainId, uint32 destinationDomainId);
  ...
  function whitelistAdapter(IBridgeSenderAdapter _bridgeAdapter, bool _isWhitelisted) external;

  function whitelistAdapters(IBridgeSenderAdapter[] calldata _bridgeAdapters, bool[] calldata _isWhitelisted) external;
  ...
  function setDestinationDomainIds(
    IBridgeSenderAdapter[] calldata _bridgeAdapter,
...
//TODO: change name to IBridgeReceiverAdapter
