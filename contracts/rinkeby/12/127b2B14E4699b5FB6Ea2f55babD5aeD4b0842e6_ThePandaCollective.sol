...
contract ThePandaCollective is Ownable, ERC721A, ReentrancyGuard {
    ...
    function getCurrentStage() public view returns (STAGE) {
        ...
        if (currentTime >= ogSaleTime && currentTime < wlSaleTime) {
            return STAGE.OG;
        } else if (currentTime >= wlSaleTime && currentTime < publicSaleTime) {
            return STAGE.WL;
    ...
    function getSaleConfig() internal view returns (SaleConfig storage) {
        STAGE stage = getCurrentStage();
        require(stage != STAGE.NOT_STARTED, "Sale has yet to start");
        if (stage == STAGE.OG) {
            return ogSaleConfig;
        } else if (stage == STAGE.WL) {
            return wlSaleConfig;
        }
    ...
    function mint(uint256 quantity) public payable {
        ...
        if(stage == STAGE.OG) {
            require(getAllowList(ogSaleConfig, msg.sender), "Not in OG whitelist");
        }else if(stage == STAGE.WL) {
            require(getAllowList(wlSaleConfig, msg.sender), "Not in WL whitelist");
        }

        SaleConfig storage saleConfig = getSaleConfig();
