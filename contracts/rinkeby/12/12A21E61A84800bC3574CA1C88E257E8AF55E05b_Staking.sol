...
contract Staking is Ownable, ReentrancyGuard {
    ...
    function depositReward() external payable onlyOwner {
        ...
        lastRewardRate += (msg.value * DIVISION / totalStakedAmount);
    }

    function stake() external payable {
        require(msg.value > 0);
        stakedBalance[msg.sender] += msg.value; // stake[address] = amount;
        rewardRate[msg.sender] = lastRewardRate; // S0[address] = S;
        totalStakedAmount += msg.value; // T = T + amount;
    }
    ...
    function withdraw() nonReentrant external {
        ...
        (bool sent, bytes memory data) = payable(msg.sender).call{value: deposited + reward}("");
