...
contract StakingPool is IStakingPool, Ownable, CheckContract{
    ...
    IStakingInterface public stakingInterface;
    ...
    function getUSDCDeposits() external view returns (uint) {
        return stakingInterface.getTotalUSDC();
    }

    function getMaxWithdrawalAmount(address _user) external view returns (uint) {
        uint tUSDCBalance = tUSDCToken.balanceOf(_user);
        return converttUSDCToUSDC(tUSDCBalance);
    } 
    ...
    function requestWithdrawFromStakingPool(uint _amount) external override {
        ...
        require(tUSDCToken.balanceOf(msg.sender) >= tUSDCAmount, "StakingPool: Insufficient tUSDC balance in user wallet");
        ...
        stakingInterface.USDCWitdrawFromStakingPool(_amount);
    ...
    function provideToStakingPool(uint256 _amount) external override {
        ...
        bool sentToStakingInterface = _sendUSDCtoStakingPool(msg.sender, address(stakingInterface), _amount);
        require(sentToStakingInterface, "StakingPool: Transfer to StakingInterface failed");
        stakingInterface.USDCProvidedToStakingPool(_amount);
    }
    ...
    function getConversionRate() public view returns (uint) {
        uint totalUSDC = stakingInterface.getTotalUSDC();
    ...
    function _requireCallerIsStakingInterface() internal {
        require(msg.sender == address(stakingInterface), "StakingPool: Caller is not withdrawal pool");
    }
}