    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier _executorOnly() {
        require(
            msg.sender == ExecutorAddress,
            "must be called by Executor only"
        );
        _;
    }
    ...
    ) public nonReentrant whenNotPaused _executorOnly {
        royaltyFeeInBips = _royaltyFeeInBips;
        INFT721(nftContract).setRoyaltyInfo(_receiver, _royaltyFeeInBips);
        emit RoyaltiesSet(nftContract, _receiver, royaltyFeeInBips);
    }
    ...
    function setTokenUri(
        ...
        INFT721(nftContract).setTokenUri(tokenId, _tokenURI);
    }
    ...
    ) public nonReentrant whenNotPaused _executorOnly returns (uint256) {
    ...
    function mintWeapon(address nftContract, uint256 itemId)
        public
        payable
        nonReentrant
        whenNotPaused
    ...
    function mintWeaponWithAddress(
        ...
        uint256 itemId,
        address recipient
    ) public payable nonReentrant whenNotPaused _executorOnly {
        uint256 charge = idToGameItem[itemId].chargeInXPNDWei;
        uint256 tokenId = idToGameItem[itemId].tokenId;
