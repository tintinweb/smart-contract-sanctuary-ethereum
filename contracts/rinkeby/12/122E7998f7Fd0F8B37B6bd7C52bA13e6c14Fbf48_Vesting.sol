...
contract Vesting is Ownable, ReentrancyGuard {
    ...
    * @param vestingScheduleId is the ID of the vesting schedule
    */
    modifier onlyIfVestingScheduleExists(bytes32 vestingScheduleId) {
        require(vestingSchedules[vestingScheduleId].initialized == true, "Vesting schedule does not exist");
        _;
    }
    ...
    */
    modifier onlyIfVestingScheduleNotRevoked(bytes32 vestingScheduleId) {
        require(vestingSchedules[vestingScheduleId].initialized == true, "Vesting schedule not initialized exist");
        require(vestingSchedules[vestingScheduleId].revoked == false, "Vesting schedule already revoked");
        _;
    }
    ...
    constructor(address token_) {
        require(token_ != address(0x0), "Cannot user address zero");
        _token = IERC20(token_);
    }

    /**
    * @notice Returns the number of vesting schedules associated to a beneficiary.
    * @param _beneficiary is the address who receives the vesting tokens
    ...
    * @param index is the index of the vesting schedule.
    ...
    * @param holder is the address we to show vesting schedules for.
    * @param index is the index of the holder's vesting schedule.
    ...
    */
    function withdraw(uint256 amount)
        ...
        nonReentrant
        onlyOwner {
        require(this.getWithdrawableAmount() >= amount, "Vesting: not enough withdrawable funds");
    ...
    * @param holder address to compute the next vesting schedule id for. 
    */
    ...
    * @param holder address to compute the next vesting schedule for. 
    */
    ...
    * @param holder address to compute the next vesting schedule id for. 
    * @param index index of the vesting schedule id. 
    */
    ...
    * @param vestingSchedule struct of the vesting schedule to compute releasable amount for. 
