...
contract NODERewardManagement {
    ...
    // real : 60 * 60 * 24, test : 60 * 5
    uint256 private DetaforDay = 60 * 5;
    // real : 60 * 60 * 24 * 365, test : 60 * 5 * 10
    uint256 private DetaforYear = 60 * 5 * 10;
    ...
    function distributeRewards() private returns (uint256) {
        ...
        if (numberOfnodeOwners > 0) {
            ...
            uint256 nodesCount;
            while (
                gasUsed < gasForDistribution && iterations < numberOfnodeOwners
            ) {
                if (localLastIndex >= nodeOwners.keys.length) {
                    localLastIndex = 0;
                }
