...
contract KeywordsNFT is ERC1155, Ownable {
  ...
  mapping(uint256 => Owner[]) public getOwnersOfToken;
  ...
  constructor(string memory _name, string memory _symbol, string memory _uri) ERC1155(_uri) {name = _name; symbol = _symbol;}
    ...
    function mintSenderNft(string memory _keyword) public {
      ...
      if (keywordExists[_keyword]) {
        ...
        SENDER_TOKEN_ID++;
        tokenSupply[tokenId]++;
        getOwnersOfToken[tokenId].push(Owner(msg.sender));
      ...
      // if keyword does not exist, mint a sender AND a receiver NFT
        ...
        console.log("A receiver NFT w/ ID %s has been minted to %s", receiverTokenId, msg.sender);
        console.log(receiverTokenUri);
      }
    }

    function mintReceiverNft(string memory _keyword) public {
    ...
    function getOwners(uint256 tokenId) public view returns (Owner[] memory){
        return(getOwnersOfToken[tokenId]);
    }

    function encodeUri(string memory _keyword, string memory _svg, string memory _role) private pure returns (string memory) {
    ...
    function ownersOfToken(uint tokenId) external view returns(address[] memory ownersAddresses) {
