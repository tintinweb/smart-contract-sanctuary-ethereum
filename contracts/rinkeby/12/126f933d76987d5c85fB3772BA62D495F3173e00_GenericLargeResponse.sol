...
contract GenericLargeResponse is ChainlinkClient, ConfirmedOwner {
    ...
    bytes public data;
    bytes public parking_spot_location;
    ...
    mapping(address=>bytes[]) public parkingSpotLocationOwnerArray;
    mapping(address=>bytes) public parkingSpotLocationOwner;
    ...
    function fulfillBytes(bytes32 requestId, bytes memory bytesData) public recordChainlinkFulfillment(requestId) {
        ...
        data = bytesData;
        parking_spot_location = data;
        parkingSpotLocationOwner[msg.sender] = data;
        parkingSpotLocationOwnerArray[msg.sender].push(data);
    ...
    function checkParkingSpotLocationOwner(address _address) public view returns ( bytes memory) {
        return parkingSpotLocationOwner[_address];
    }

       function checkParkingSpotLocationOwnerArray(address _address, uint16 _index) public view returns ( bytes memory) {
