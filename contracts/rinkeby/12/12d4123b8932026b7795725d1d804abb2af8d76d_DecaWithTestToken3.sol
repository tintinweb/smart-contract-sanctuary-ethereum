...
contract DecaWithTestToken3 is VRFConsumerBaseV2, Pausable, Ownable, ReentrancyGuard {
    ...
    function depositHouse(uint256 _amount) external {
        address userAddress = msg.sender;
        IERC20(USDC_ADDRESS).transferFrom(userAddress, address(this), _amount);
        userHouseBalance[userAddress].depositAmount.add(_amount);
        housePoolBalance.depositAmount.add(_amount);
        // lock 처리해야함
    }
    ...
    function withdrawHouse(uint256 _amount) external nonReentrant {
        ...
        userHouseBalance[userAddress].depositAmount.sub(_amount);
        userHouseBalance[userAddress].withdrawAmount.add(_amount);
        housePoolBalance.depositAmount.sub(_amount);
        housePoolBalance.withdrawAmount.add(_amount);
    }
