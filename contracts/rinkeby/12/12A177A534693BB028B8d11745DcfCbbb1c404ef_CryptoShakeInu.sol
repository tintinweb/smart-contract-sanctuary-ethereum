...
contract CryptoShakeInu is IERC20, ReentrancyGuard, Ownable {
  ...
  constructor(
    ...
    excludedFromLimit[_treasuryAddr] = true;
  ...
  function _transfer(
    ...
    // if buy
    if (_sender == dexPair) {
      if (block.timestamp < _releasedTime + 1 hours && !excludedFromLimit[_recipient]) {
    ...
    // else if sell
      ...
      if (block.timestamp < _releasedTime + 1 hours && !excludedFromLimit[_sender]) {
        require(_amount <= maxBuy, "exceeded max buy");
      }
