    ...
    AccessControl
{
    ...
    /**
     * @dev Mapping from token ID to the additional details structure which
    ...
    {
        if (_exists(tokenId) == false || _details[tokenId].creator == address(0x0) || _details[tokenId].royaltyFee == 0) {
            // Zero out everything
            return (address(0x0), 0);
        } else {
            // Return the address of the creator and the royalty proceedings
            return (_details[tokenId].creator, (salePrice * _details[tokenId].royaltyFee) / (100 * 10**DECIMALS));
        }
    }
