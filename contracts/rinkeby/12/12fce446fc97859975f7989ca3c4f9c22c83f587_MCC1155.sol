...
interface IBEP1155Upgradeable is IBEP165Upgradeable {
    ...
    event FeeTransfer(uint256 admin, uint256 creator, uint256 owner);
...
contract BEP1155Upgradeable is
    Initializable,
    ContextUpgradeable,
    BEP165Upgradeable,
    IBEP1155Upgradeable,
    IBEP1155MetadataURIUpgradeable
{
    ...
    function __BEP1155_init_unchained(
        string memory uri_,
        string memory name_,
        string memory symbol_
    ) internal initializer {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(BEP165Upgradeable, IBEP165Upgradeable)
        returns (bool)
    {
        return
    ...
    function _doSafeTransferAcceptanceCheck(
        ...
        if (to.isContract()) {
            ...
            returns (bytes4 response) {
                if (
                    response !=
                    IBEP1155ReceiverUpgradeable.onBEP1155Received.selector
                ) {
    ...
    function _doSafeBatchTransferAcceptanceCheck(
        ...
        if (to.isContract()) {
            ...
            returns (bytes4 response) {
                if (
                    response !=
                    IBEP1155ReceiverUpgradeable.onBEP1155BatchReceived.selector
                ) {
                    revert("BEP1155: BEP1155Receiver rejected tokens");
                }
            } catch Error(string memory reason) {
 ...
 * _Available since v3.1._
 */
abstract contract BEP1155BurnableUpgradeable is
    Initializable,
    BEP1155Upgradeable,
    OwnableUpgradeable
{
...
interface MCCTrade {
    function availablePack(address from) external view returns (uint256, uint256);
    ...
    BEP1155BurnableUpgradeable
{
    ...
    mapping(address => bool) private trade;
    ...
    function initialize() public initializer {
        BEP1155Upgradeable.__BEP1155_init("", "MCC1155", "MCC1155");
        __Ownable_init();
        _tid = 1;
    ...
    function addTradeContract(address from) public onlyOwner {
        TradeAddress = from;
        trade[from] = true;
    }
    ...
    function setServiceValue(uint256 _serviceValue, uint256 sellerfee) public {
        require(trade[msg.sender] == true, "Not Eligible to access");
    ...
    function mint(
        ...
        (uint256 nPack, uint256 pPack) = MCCTrade(TradeAddress).availablePack(msg.sender);
        if ( keccak256(abi.encodePacked((_type))) == keccak256(abi.encodePacked(("Premium")))){
            require( pPack > 0,"Not Eligible to Premium Mint");
        }
        else{
            require( nPack > 0,"Not Eligible to Mint");
        }
        _tid = _tid.add(1);
        uint256 id_ = _tid.add(block.timestamp);
        _mint(msg.sender, id_, nooftoken, "");
        _setURI(ipfsmetadata);
        _creator[id_] = msg.sender;
        _royal[id_] = royal.mul(1e18);
        if (value != 0) {
            orderPlace(id_, value);
        }
        MCCTrade(TradeAddress).decreasePack(msg.sender,_type);
    ...
    function saleWithToken(
        ...
        require(
            amount == order_place[from][tokenId].price.mul(nooftoken) &&
                order_place[from][tokenId].price.mul(nooftoken) > 0,
            "Invalid Balance"
        );
