...
contract CryptoDoodez is ERC721A, Ownable{
    ...
    string private  baseTokenUri = "https://kroniclabz.mypinata.cloud/ipfs/QmZh844CaUe1WDCGq8JDTZCzEX8V3t5xfpZriUXToz9T9s";
    ...
    function allowListMint(bytes32[] memory _merkleProof, uint256 _quantity) external payable callerIsUser{
        require(allowListSale, "Flinch NFT :: Minting is on Pause");
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "Flinch NFT :: Cannot mint beyond max supply");
        require((totalAllowListMint[msg.sender] + _quantity)  <= MAX_ALLOW_LIST_MINT, "Flinch NFT :: Cannot mint beyond whitelist max mint!");
        require(msg.value >= (ALLOW_LIST_SALE_PRICE * _quantity), "Flinch NFT :: Payment is below the price");
        //create leaf node
        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, sender), "Flinch NFT :: You're not on the allowList.");
