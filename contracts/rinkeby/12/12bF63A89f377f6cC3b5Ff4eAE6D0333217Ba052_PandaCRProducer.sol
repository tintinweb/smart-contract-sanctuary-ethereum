...
//########################### CR Buy&Reborn Functions Start ####################
    ...
    function C_buy(uint256 _amountBuyMax, uint256 _deadline, bytes32 _dataHash) external ensure(_deadline)  nonReentrant crOpen {
        ...
        if (_pandaToken.balanceOf(msg.sender) < _price * 1750 / 10000) {
            revert PandaNotEnough(msg.sender, _pandaToken.balanceOf(msg.sender), _price * 10750 / 10000);
        }
        poolKit._safeTransferFromPanda(msg.sender, address(poolKit), _price * 10375 / 10000);
        poolKit._safeTransferFromPanda(msg.sender,  owner(), _price * 375 / 10000);
    ...
    function C_reborn(uint256 _ownerTokenId, uint256 _amountFeeMax, uint256 _deadline, bytes32 _dataHash) external ensure(_deadline)  nonReentrant crOpen {
        ...
        poolKit._safeTransferFromPanda(msg.sender, address(poolKit), _price * 375 / 10000);
        poolKit._safeTransferFromPanda(msg.sender, owner(), _price * 375 / 10000);
...
//########################### CR Buy&Reborn Functions End ####################
     ...
     */
    function setSwitchs(bool _poolOpne, bool _vrfOpen, bool _crOpen) external onlyOwner {
