    ...
    SuperMerkleAccess
{
  ...
  */
    ...
    @param _items The Super1155 address.
  */
    function setItems(ISuper1155[] calldata _items)
        external
        hasValidPermit(UNIVERSAL, SET_ITEMS)
    {
        items = _items;
    }
  ...
  */
    function getPools(uint256[] calldata _ids, uint256 _itemIndex)
        external
        view
        returns (PoolOutput[] memory)
    {
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            PoolItem[] memory poolItems = new PoolItem[](
                pools[id].itemGroups.length
            );
            for (uint256 j = 0; j < pools[id].itemGroups.length; j++) {
                uint256 itemGroupId = pools[id].itemGroups[j];
                bytes32 itemKey = keccak256(
                    abi.encodePacked(
                        pools[id].config.collection,
                        pools[id].currentPoolVersion,
                        itemGroupId
                    )
                );

                // Parse each price the item is sold at.
                DFStorage.Price[] memory itemPrices = new DFStorage.Price[](
                    pools[id].itemPricesLength[itemKey]
                );
                for (
                    uint256 k = 0;
                    k < pools[id].itemPricesLength[itemKey];
                    k++
                ) {
  ...
  */
    function _updatePoolHelper(
        ...
        for (uint256 i = 0; i < _groupIds.length; i++) {
            ...
            // We generate a key from collection address, pool version and groupId.
  ...
  */
    function mintFromPool(
        ...
        uint256 userGlobalPurchaseAmount = _amount +
            globalPurchaseCounts[_msgSender()];
        ...
        uint256 userPoolPurchaseAmount = _amount +
            pools[_id].purchaseCounts[_msgSender()];
        ...
        {
            uint256 result;
            uint256 nextPoolId_ = nextPoolId;
            for (uint256 i = 0; i < nextPoolId_; i++) {
                uint256 length = pools[i].itemGroups.length;
    ...
    function sellingHelper(
        ...
        if (_whiteListPrice) {
            ...
            DFStorage.Price memory sellingPair = pools[_id].itemPrices[itemKey][
                _assetIndex
            ];
            uint256 price = block.timestamp >= pools[_id].config.endTime
                ? sellingPair.finalPrice
                : sellingPair.finalPrice +
                    ((sellingPair.startPrice - sellingPair.finalPrice) *
                        (pools[_id].config.endTime - block.timestamp)) /
                    (pools[_id].config.endTime - pools[_id].config.startTime);
            uint256 summ = price * _amount;

            // Process payment for the user with a check to sell for Ether.
            if (sellingPair.assetType == DFStorage.AssetType.Ether) {
                require(msg.value >= summ, "0x9B");
                (bool success, ) = payable(paymentReceiver).call{
                    value: summ,
                    gas: 0
                }("");
                require(success, "0x0C");
                uint256 diff = msg.value - summ;
                if (diff > 0) {
                    payable(msg.sender).call{value: diff, gas: 0};
                }
  ...
  */
   ...
   */
    ...
    /**
        ...
        @param _items The Super1155 address.
    */
    function setItems(ISuper1155[] calldata _items) external;
