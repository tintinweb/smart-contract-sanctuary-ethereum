...
*/
contract WukongEP is ERC1155, ERC1155Supply, ERC1155Burnable, Ownable {
    ...
    uint256 public purchaseWindowOpens = 1664553599;

    uint256 public equityValue = 5;
    uint256 public royaltyValue = 0;
    ...
    event RoyaltyValue(uint256 indexed royaltyValue);
    event EquityValue(uint256 indexed equityValue);

    constructor(
        string memory baseURI
    ) ERC1155(baseURI){
        _mint(0xa18383E2BF2482A821464680f0b6F06f3234e421, 1, 288, "");
        priceArr[PHOENIX_RAVE-1] = 0.01 ether;
        supplyArr[PHOENIX_RAVE-1] = 288;
        MAX_NFTS--;
        _mint(0xa18383E2BF2482A821464680f0b6F06f3234e421, 2, 288, "");
        priceArr[LEGEND_OF_WFH-1] = 0.01 ether;
        supplyArr[LEGEND_OF_WFH-1] = 288;
        MAX_NFTS--;
        CURRENT_TOKEN_ID = 3;
        setBaseURI(baseURI);
    }
    ...
    *
    * @param numerator numerator for price
    * @param denominator denomiator for price
    * @param individualSupply set how much equity wukong is holding
    * @param maxSupply set how much equity will be giving to supporters of track drop
    */
    function createNewTrack(uint256 numerator, uint256 denominator, uint256 individualSupply, uint256 maxSupply) external onlyOwner {
        require(MAX_NFTS> 0, "No more supply left for soundtracks");
        _mint(0xa18383E2BF2482A821464680f0b6F06f3234e421, CURRENT_TOKEN_ID, individualSupply, "");
        priceArr[CURRENT_TOKEN_ID-1] = numerator/denominator;
        supplyArr[CURRENT_TOKEN_ID-1] = maxSupply;
        CURRENT_TOKEN_ID++;
        MAX_NFTS--;
    }

    /**
    * @notice edit the purchase price
    *
    * @param tokenID token you want to change price of
    * @param mintPrice price you want to change of the token to
    */
    function setPrice(uint256 tokenID, uint256 mintPrice) external onlyOwner {
        priceArr[tokenID-1] = mintPrice;
    }
    ...
    */
    function massMint(uint256 count) external onlyOwner {
        for (uint256 i = 0; i < count; i++){
            _mint(0xa18383E2BF2482A821464680f0b6F06f3234e421,PHOENIX_RAVE, 1, "");
            deductCount(PHOENIX_RAVE);
            _mint(0xa18383E2BF2482A821464680f0b6F06f3234e421,LEGEND_OF_WFH, 1, "");
            deductCount(LEGEND_OF_WFH);
        }
    }
    ...
    *
    */
    function checkSupply() external onlyOwner {
    ...
    */
    function checkPrice(uint256 tokenId) external onlyOwner {
    ...
    */
    ...
    /**
        Shows how much equity holders of contract has of the IP
     */
    function getEquityValue() public { 
        emit EquityValue(equityValue);
    }

    function updateEquityValue(uint256 amount) external onlyOwner{
        equityValue = amount;
    }

    /**
        Royalty Updates to be done off chain. On chain is for decentralisation purposes
     */
    function getRoyaltyValue() public {
        emit RoyaltyValue(royaltyValue);
    }

    function updateRoyaltyValue(uint256 amount) external onlyOwner{
        royaltyValue = amount;
    }
    ...
    * @param denominator denomiator for price
    */
    function _setTokenPrice(uint256 tokenId, uint256 numerator, uint256 denominator) external onlyOwner{
        priceArr[tokenId-1] = numerator/denominator;
    }
    ...
    */
    function _setTokenSupply(uint256 tokenId, uint256 count) external onlyOwner{
