...
contract Q007 is Ownable, ERC20, Pausable {
 ...
 function updateRefFeePercentage(uint256 newPercentage) public onlyOwner {
  ...
  modifier onlyHighUser(address user) {
   require(keccak256(abi.encodePacked((Members[user].role))) == keccak256(abi.encodePacked((SA))) ||  keccak256(abi.encodePacked((Members[user].role))) == keccak256(abi.encodePacked((MA))), "Authorization: caller is not authorized");
_;
}
...
_;
  ...
  function verify_level(address payable user) internal view returns(uint256) {
   if(keccak256(abi.encodePacked((Members[user].role))) == keccak256(abi.encodePacked((SA))) || user==owner()){
    return 0;}
  else {return SA_threshold;}
    }
  ...
  function buyGold(address payable client, address payable ref, uint256 initAmount, string memory clientRole) public onlyHighUser(_msgSender()) {
    uint level_code= verify_level(client);
    require(level_code>initAmount || level_code==0, "not authorized");
