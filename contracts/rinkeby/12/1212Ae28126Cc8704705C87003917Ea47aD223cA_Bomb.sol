 ...
 *@notice 'Why would you guys do this?' Performance art while testing a new 
 *        EIP in production. While this contract is intended to be an
 *        annoying prank mechanism, it serves to highlight the potential
 *        of EIP-2309 and more efficient methods within NFT contracts.
 *@notice Forking is a bombable offence.
 *@dev Implementation of ERC-721 that utilizes EIP-2309 to explode Glitter 
 *     bombs into the wallets of the unsuspecting victims.
 */
contract Bomb is
      ERC721
    , Ownable
{
    ...
    ///@notice the token descriptions of each type
    string[] internal descriptions;

    ///@notice the path of the bomb
    string[] internal outlinePaths;
        ...
        )
    {
        ...
        if(_tokenId < ARMORY_CAPACITY) {
            descriptionString = descriptions[
                _tokenId < MAX_SHIELDS ? 1 : 2
            ];
        } else { 
        ...
        if(
            ...
            metadataString = string(
                abi.encodePacked(
                      metadataString
                    , ',{"display_type":"number","trait_type":"Pieces","value":"'
                    , bombToPieces[_tokenId].toString()
                    , '","max_value":"'
                    , MAX_PIECES.toString()
                    ,'"}'
                )
    ...
    { 
        ...
        bombToPieces[_tokenId] = _bombPieces;
    ...
    { 
        ...
        uint256 _bombPieces = bombToPieces[_tokenId];
        
        require(_bombPieces != 0, "Bomb: not exploded");
        require(
              msg.value == cleanPrice(_bombPieces)
