...
contract Presale is ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event SaleCreated(uint256 id, address token, address currency, uint128[3] startTimes, uint128[3] endTimes, uint128 price, uint128 totalSupply, uint128 tgeTime, uint128 tgePercent, uint128[3] vesting, uint128[2] referralPercent);
    event SaleUpdated(uint256 id, address token, address currency, uint128[3] startTimes, uint128[3] endTimes, uint128 price, uint128 totalSupply, uint128 tgeTime, uint128 tgePercent, uint128[3] vesting, uint128[2] referralPercent);
    event SaleEnabled(uint256 id);
    event SaleDisabled(uint256 id);

    event BoughtInPrimarySale(uint256 saleId, address account, uint256 amount, string referralCode, uint256 referralBonus, uint256 leaderboardBonus);
    event BoughtInFCFS(uint256 saleId, address account, uint256 amount, string referralCode, uint256 referralBonus, uint256 leaderboardBonus);
    ...
    function _checkSaleParams(address token, address currency, uint128[3] memory startTimes, uint128[3] memory endTimes, uint128 price, uint128 totalSupply, uint128 tgeTime, uint128 tgePercent, uint128[2] memory referralPercent)
        internal
        pure
    {
        ...
        require(referralPercent[0] + referralPercent[1] <= ONE_HUNDRED_PERCENT, "Presale: referral percent is invalid");
    }

    function createSale(uint256 id, address token, address currency, uint128[3] memory startTimes, uint128[3] memory endTimes, uint128 price, uint128 totalSupply, uint128 tgeTime, uint128 tgePercent, uint128[3] memory vesting, uint128[2] memory referralPercent)
        external
        onlyOperator
        saleNotExist(id)
    {
        ...
        referrals[id] = Referral(referralPercent[0], referralPercent[1]);
    ...
    function updateSale(uint256 id, address token, address currency, uint128[3] memory startTimes, uint128[3] memory endTimes, uint128 price, uint128 totalSupply, uint128 tgeTime, uint128 tgePercent, uint128[3] memory vesting, uint128[2] memory referralPercent)
        external
        onlyOperator
        saleExist(id)
    {
        ...
        if (referrals[id].percent != referralPercent[0]) {
            referrals[id].percent = referralPercent[0];
        }

        if (referrals[id].leaderboardPercent != referralPercent[1]) {
            referrals[id].leaderboardPercent = referralPercent[1];
        }
        ...
        saleNotPause(saleId)
    {
        ...
        uint256 payment = amount * sale.price / (10 ** IToken(sale.token).decimals());
        ...
        uint256 leaderboardBonus = payment * referrals[saleId].leaderboardPercent / ONE_HUNDRED_PERCENT;

        payment -= (referralBonus + leaderboardBonus);

        IERC20(sale.currency).safeTransferFrom(msgSender, treasury, payment);

        if (referralBonus + leaderboardBonus > 0) {
            IERC20(sale.currency).safeTransferFrom(msgSender, referralContract, referralBonus + leaderboardBonus);
        }
        ...
        if (isPrimarySale) {
            emit BoughtInPrimarySale(saleId, msgSender, amount, referralCode, referralBonus, leaderboardBonus);

        } else {
            emit BoughtInFCFS(saleId, msgSender, amount, referralCode, referralBonus, leaderboardBonus);
        }
    }
