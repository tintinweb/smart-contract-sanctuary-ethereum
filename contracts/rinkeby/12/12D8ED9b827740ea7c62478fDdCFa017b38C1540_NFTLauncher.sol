...
contract NFTLauncher is VRFConsumerBaseV2, ChainlinkClient {
    ...
    constructor(string memory chainLinkJobId, uint64 subscriptionId) VRFConsumerBaseV2(vrfCoordinator) {
    ...
    function purchasePack(
        ...
        LinkTokenInterface(chainlinkTokenAddress()).approve(
            vrfCoordinator,
            CHAINLINK_RANDOMNESS_BASE_FEE
        );
    ...
    function isValidCaller(uint256 projectrid)
        ...
        returns (bool validProject)
    {
        require(projects[projectrid].owner == tx.origin || tx.origin == owner);
        return true;
    }
