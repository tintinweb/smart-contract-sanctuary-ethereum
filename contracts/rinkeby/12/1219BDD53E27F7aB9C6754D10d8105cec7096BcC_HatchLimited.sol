...
contract HatchLimited is ERC721, Ownable {
    ...
    uint256 public limitedMintingFee;
    bool public saleActive = false;
    bool public presaleActive = false;
    bool inside;
    string public baseURI;
    string public baseExtension = ".json";
    address public hatchVault;
    ...
    constructor(
        ...
        uint256 _maxSupply,
        uint256 _maxMintAmount,
        uint256 _limitedMintingFee,
        address _hatchFactoryAddress
    ) ERC721(_name, _symbol) {
        cost = _cost;
        preSalecost = _preSalecost;
        maxSupply = _maxSupply;
        maxMintAmount = _maxMintAmount;
        limitedMintingFee = _limitedMintingFee;
        hatchVault = _hatchFactoryAddress;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        sendEth(hatchVault, (cost / 100 * limitedMintingFee) * _mintAmount);
    }
    ...
    function mintPresale(uint256 _mintAmount) public payable {
        ...
        require(_mintAmount <= reservedAmt, "Can't mint more than reserved");
        require(
            supply.current() + _mintAmount <= maxSupply,
        ...
        _mintLoop(msg.sender, _mintAmount);
        sendEth(hatchVault, (preSalecost / 100 * limitedMintingFee) * _mintAmount);
    }
    ...
    function giveAway(
        ...
        require(
            supply.current() + totalQuantity <= maxSupply,
            "Too many NFTs t o mint"
        );
        delete totalQuantity;
        for (uint256 i = 0; i < addressList.length; ++i) {
            _mintLoop(addressList[i], quantityList[i]);
        }
    }
    ...
    function addPresaleReserved(
        address[] memory _address,
        uint256[] memory _amount
    ) public onlyOwner {
        for (uint256 i; i < _address.length; i++) {
            presaleReserved[_address[i]] = _amount[i];
        }
    }
    ...
    function sendEth(address _destination, uint256 amount) internal {
        (bool sent, ) = _destination.call{value: amount}("");
        require(sent, "Failed to send Ether");
    }

     // Withdraw
    function withdrawAll(address _destination) public onlyOwner {
