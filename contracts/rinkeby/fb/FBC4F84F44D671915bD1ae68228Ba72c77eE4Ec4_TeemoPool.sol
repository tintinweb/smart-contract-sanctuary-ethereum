...
import '../interfaces/ITeemoGovernance.sol';
...
contract TeemoBallot {
    ...
    uint256 private constant MINIMUM_TEEMO_TO_EXEC = 3 * (10**18);

    uint256 public total;
    uint256 public createTime;

    modifier onlyGovernor() {
        require(msg.sender == governor, 'TeemoBallot: FORBIDDEN');
        _;
    }
    ...
    function _stakeCollateralToVote(uint256 collateral) private returns (bool) {
        ...
        require(
            IERC20(TEEMO).allowance(msg.sender, address(this)) >= collateralMore,
            'TEEMOBallot:Collateral allowance is not enough to vote!'
        );
        IERC20(TEEMO).transferFrom(msg.sender, address(this), collateralMore);
        IERC20(TEEMO).approve(governor, collateralMore);
        bool success = ITeemoGovernance(governor).onBehalfDeposit(msg.sender, collateralMore);
        return success;
    }
    ...
    //     if (collateral > 0) {
    //         require(_stakeCollateralToVote(collateral), "TeemoBallot:Fail due to stake Teemo as collateral!");
    //     }
    ...
    function result() public view returns (bool) {
        ...
        if (winner == YES && total >= MINIMUM_TEEMO_TO_EXEC) {
            return true;
        }
        return false;
    }
...
import './TeemoBallot.sol';
import './TeemoBallotRevenue.sol';

contract TeemoBallotFactory {
    address public TEEMO;
    ...
    constructor(address _TEEMO) public {
        TEEMO = _TEEMO;
    }
    ...
    function create(
        ...
        require(_value >= 0, 'TEEMOBallotFactory: INVALID_PARAMTERS');
        address ballotAddr = address(
            new TeemoBallot(TEEMO, _proposer, _value, _endTime, _executionTime, msg.sender, _subject, _content)
        );
    ...
    function createShareRevenue(
        ...
        address ballotAddr = address(
            new TeemoBallotRevenue(TEEMO, _proposer, _endTime, _executionTime, msg.sender, _subject, _content)
        );
...
contract TeemoBallotRevenue {
    ...
    //     if (collateral > 0) {
    //         require(_stakeCollateralToJoin(collateral), "TEEMOBallotRevenue:Fail due to stake TEEMO as collateral!");
    //     }
...
import './interfaces/ITeemoFactory.sol';
import './interfaces/IERC20.sol';
import './interfaces/ITeemoPair.sol';
import './libraries/TeemoSwapLibrary.sol';
import './libraries/SafeMath.sol';

contract TeemoTransferListener is Ownable {
    ...
    function initialize(
        ...
        require(
            ...
            'Teemo TRANSFER LISTENER : INPUT ADDRESS IS ZERO'
        );
        TGAS = _TGAS;
        FACTORY = _FACTORY;
        WETH = _WETH;
    ...
    function updatePairPowers(address[] calldata _pairs, uint256[] calldata _weights) external {
        require(msg.sender == admin, 'Teemo TRANSFER LISTENER: ADMIN PERMISSION');
        require(_pairs.length == _weights.length, 'Teemo TRANSFER LISTENER: INVALID PARAMS');
    ...
    function _setProdutivity(address _pair) internal {
        ...
        address token0 = ITeemoPair(_pair).token0();
        address token1 = ITeemoPair(_pair).token1();
        (uint256 reserve0, uint256 reserve1, ) = ITeemoPair(_pair).getReserves();
    ...
    function upgradeProdutivity(address fromPair, address toPair) external {
        require(msg.sender == PLATFORM, 'Teemo TRANSFER LISTENER: PERMISSION');
    ...
    function transferNotify(
        ...
        require(msg.sender == PLATFORM, 'Teemo TRANSFER LISTENER: PERMISSION');
        if (ITeemoFactory(FACTORY).isPair(from) && token == TGAS) {
            _setProdutivity(from);
        }

        if (ITeemoFactory(FACTORY).isPair(to) && token == TGAS) {
            _setProdutivity(to);
        }
...
library TeemoSwapLibrary {
    using SafeMath for uint256;

    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'TeemoSwapLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'TeemoSwapLibrary: ZERO_ADDRESS');
    }
    ...
    function pairFor(
        ...
        bytes32 rawAddress = keccak256(
            abi.encodePacked(bytes1(0xff), factory, salt, ITeemoFactory(factory).contractCodeHash())
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = ITeemoPair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    function quoteEnhance(
        address factory,
        address tokenA,
        address tokenB,
        uint256 amountA
    ...
    function quote(
        ...
        require(amountA > 0, 'TeemoSwapLibrary: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'TeemoSwapLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, 'TeemoSwapLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'TeemoSwapLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, 'TeemoSwapLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'TeemoSwapLibrary: INSUFFICIENT_LIQUIDITY');
...
import './interfaces/ITeemoGovernance.sol';
import './interfaces/ITeemoConfig.sol';
import './interfaces/IERC20.sol';
import './interfaces/ITeemoFactory.sol';
import './interfaces/ITeemoPair.sol';
import './interfaces/ITeemoPool.sol';
import './modules/Ownable.sol';
import './modules/ReentrancyGuard.sol';
import './interfaces/ITeemoTransferListener.sol';
import './interfaces/ITokenRegistry.sol';
import './interfaces/ITeemoStaking.sol';

contract TeemoPlatform is Ownable {
    uint256 public version = 1;
    address public TEEMO;
    ...
    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, 'TEEMO PLATFORM : EXPIRED');
        _;
    }

    modifier noneTokenCall() {
        require(ITokenRegistry(CONFIG).tokenStatus(msg.sender) == 0, 'TEEMO PLATFORM : ILLEGAL CALL');
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (ITeemoFactory(FACTORY).getPair(tokenA, tokenB) == address(0)) {
            ITeemoConfig(CONFIG).addToken(tokenA);
            ITeemoConfig(CONFIG).addToken(tokenB);
            ITeemoFactory(FACTORY).createPair(tokenA, tokenB);
        }
        require(ITeemoConfig(CONFIG).checkPair(tokenA, tokenB), 'Teemo PLATFORM : ADD LIQUIDITY PAIR CONFIG CHECK FAIL');
        (uint256 reserveA, uint256 reserveB) = TeemoSwapLibrary.getReserves(FACTORY, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal = TeemoSwapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'Teemo PLATFORM : INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = TeemoSwapLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'Teemo PLATFORM : INSUFFICIENT_A_AMOUNT');
                (amountA, amountB) = (amountAOptimal, amountBDesired);
            }
        }
        ITeemoFactory(FACTORY).addPlayerPair(msg.sender, ITeemoFactory(FACTORY).getPair(tokenA, tokenB));
    }
    ...
    {
        require(!isPause, 'Teemo PAUSED');
        (_amountA, _amountB) = _addLiquidity(tokenA, tokenB, amountA, amountB, amountAMin, amountBMin);
        address pair = TeemoSwapLibrary.pairFor(FACTORY, tokenA, tokenB);
        ...
        // notify pool
        ITeemoPool(POOL).preProductivityChanged(pair, msg.sender);
        _liquidity = ITeemoPair(pair).mint(msg.sender);
        ITeemoPool(POOL).postProductivityChanged(pair, msg.sender);
        ...
        )
    {
        ...
        address pair = TeemoSwapLibrary.pairFor(FACTORY, token, WETH);
        ...
        // notify pool
        ITeemoPool(POOL).preProductivityChanged(pair, msg.sender);
        liquidity = ITeemoPair(pair).mint(msg.sender);
        ITeemoPool(POOL).postProductivityChanged(pair, msg.sender);
    ...
    function removeLiquidity(
        ...
        // notify pool
        ITeemoPool(POOL).preProductivityChanged(pair, msg.sender);
        (uint256 amount0, uint256 amount1) = ITeemoPair(pair).burn(msg.sender, to, _liquidity);
        ITeemoPool(POOL).postProductivityChanged(pair, msg.sender);

        (address token0, ) = TeemoSwapLibrary.sortTokens(_tokenA, _tokenB);
        ...
        require(amountA >= amountAMin, 'Teemo PLATFORM : INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'Teemo PLATFORM : INSUFFICIENT_B_AMOUNT');
    ...
    function _getAmountsOut(
        ...
        for (uint256 i = 0; i < path.length - 1; i++) {
            ...
            (uint256 reserveA, uint256 reserveB) = TeemoSwapLibrary.getReserves(FACTORY, inPath, outPath);
            uint256 outAmount = SafeMath.mul(amountOuts[i], SafeMath.sub(PERCENT_DENOMINATOR, percent));
            amountOuts[i + 1] = TeemoSwapLibrary.getAmountOut(outAmount / PERCENT_DENOMINATOR, reserveA, reserveB);
        }
    }
    ...
    function _getAmountsIn(
        ...
        for (uint256 i = path.length - 1; i > 0; i--) {
            ...
            (uint256 reserveA, uint256 reserveB) = TeemoSwapLibrary.getReserves(FACTORY, inPath, outPath);
            uint256 inAmount = TeemoSwapLibrary.getAmountIn(amountIn[i], reserveA, reserveB);
    ...
    function swapPrecondition(address token) public view returns (bool) {
        if (token == TEEMO || token == WETH) return true;
        uint256 percent = ITeemoConfig(CONFIG).getConfigValue(ConfigNames.TOKEN_TO_TGAS_PAIR_MIN_PERCENT);
        if (!existPair(WETH, TEEMO)) return false;
        if (!existPair(TEEMO, token)) return false;
        if (!(ITeemoConfig(CONFIG).checkPair(TEEMO, token) && ITeemoConfig(CONFIG).checkPair(WETH, token))) return false;
        if (!existPair(WETH, token)) return true;
        if (percent == 0) return true;
        (uint256 reserveTEEMO, ) = TeemoSwapLibrary.getReserves(FACTORY, TEEMO, token);
        (uint256 reserveWETH, ) = TeemoSwapLibrary.getReserves(FACTORY, WETH, token);
        (uint256 reserveWETH2, uint256 reserveTEEMO2) = TeemoSwapLibrary.getReserves(FACTORY, WETH, TEEMO);
        uint256 teemoValue = SafeMath.mul(reserveTEEMO, reserveWETH2) / reserveTEEMO2;
        uint256 limitValue = SafeMath.mul(SafeMath.add(teemoValue, reserveWETH), percent) / PERCENT_DENOMINATOR;
        return teemoValue >= limitValue;
    }
    ...
    function _swap(
        ...
        require(!isPause, 'teemo PAUSED');
        require(swapPrecondition(path[path.length - 1]), 'teemo PLATFORM : CHECK teemo/TOKEN TO VALUE FAIL');
        for (uint256 i; i < path.length - 1; i++) {
            require(checkPath(path[i], path) && checkPath(path[i + 1], path), 'DEMAX PLATFORM : INVALID PATH');
            (address input, address output) = (path[i], path[i + 1]);
            require(swapPrecondition(input), 'teemo PLATFORM : CHECK teemo/TOKEN VALUE FROM FAIL');
            require(ITeemoConfig(CONFIG).checkPair(input, output), 'Teemo PLATFORM : SWAP PAIR CONFIG CHECK FAIL');
            (address token0, address token1) = TeemoSwapLibrary.sortTokens(input, output);
            uint256 amountOut = amounts[i + 1];
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2 ? TeemoSwapLibrary.pairFor(FACTORY, output, path[i + 2]) : _to;

            // add k check
            address pair = TeemoSwapLibrary.pairFor(FACTORY, input, output);
            (uint256 reserve0, uint256 resereve1, ) = ITeemoPair(pair).getReserves();
            uint256 kBefore = SafeMath.mul(reserve0, resereve1);

            ITeemoPair(pair).swap(amount0Out, amount1Out, to, new bytes(0));

            (reserve0, resereve1, ) = ITeemoPair(pair).getReserves();
            uint256 kAfter = SafeMath.mul(reserve0, resereve1);
            require(kBefore <= kAfter, 'Burger K');

            if (amount0Out > 0)
                _transferNotify(TeemoSwapLibrary.pairFor(FACTORY, input, output), to, token0, amount0Out);
            if (amount1Out > 0)
                _transferNotify(TeemoSwapLibrary.pairFor(FACTORY, input, output), to, token1, amount1Out);
        }
    ...
    function _swapFee(
        ...
        for (uint256 i = 0; i < path.length - 1; i++) {
            ...
            address currentPair = TeemoSwapLibrary.pairFor(FACTORY, input, output);
            if (input == TEEMO) {
                ITeemoPair(currentPair).swapFee(fee, TEEMO, POOL);
                _transferNotify(currentPair, POOL, TEEMO, fee);
            } else {
                ITeemoPair(currentPair).swapFee(fee, input, TeemoSwapLibrary.pairFor(FACTORY, input, TEEMO));
                (uint256 reserveIn, uint256 reserveTEEMO) = TeemoSwapLibrary.getReserves(FACTORY, input, TEEMO);
                uint256 feeOut = TeemoSwapLibrary.getAmountOut(fee, reserveIn, reserveTEEMO);
                ITeemoPair(TeemoSwapLibrary.pairFor(FACTORY, input, TEEMO)).swapFee(feeOut, TEEMO, POOL);
                _transferNotify(currentPair, TeemoSwapLibrary.pairFor(FACTORY, input, TEEMO), input, fee);
                _transferNotify(TeemoSwapLibrary.pairFor(FACTORY, input, TEEMO), POOL, TEEMO, feeOut);
                fee = feeOut;
            }
            if (fee > 0) {
                ITeemoPool(POOL).addRewardFromPlatform(currentPair, fee);
            }
        }
    }

    function _getSwapFeePercent() internal view returns (uint256) {
        return ITeemoConfig(CONFIG).getConfigValue(ConfigNames.SWAP_FEE_PERCENT);
    }
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'TEEMO PLATFORM : INSUFFICIENT_OUTPUT_AMOUNT');
        address pair = TeemoSwapLibrary.pairFor(FACTORY, path[0], path[1]);
        _innerTransferFrom(
            path[0],
            msg.sender,
            pair,
    ...
    function swapExactETHForTokens(
        ...
        require(path[0] == WETH, 'TEEMO PLATFORM : INVALID_PATH');
        uint256 percent = _getSwapFeePercent();
    ...
    function swapExactTokensForETH(
        ...
        require(path[path.length - 1] == WETH, 'Teemo PLATFORM : INVALID_PATH');
        uint256 percent = _getSwapFeePercent();
        amounts = _getAmountsOut(amountIn, path, percent);
        require(amounts[amounts.length - 1] >= amountOutMin, 'Teemo PLATFORM : INSUFFICIENT_OUTPUT_AMOUNT');
        address pair = TeemoSwapLibrary.pairFor(FACTORY, path[0], path[1]);
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'Teemo PLATFORM : EXCESSIVE_INPUT_AMOUNT');
        address pair = TeemoSwapLibrary.pairFor(FACTORY, path[0], path[1]);
    ...
    function swapETHForExactTokens(
        ...
        uint256 percent = _getSwapFeePercent();
        amounts = _getAmountsIn(amountOut, path, percent);
        require(amounts[0] <= msg.value, 'Teemo PLATFORM : EXCESSIVE_INPUT_AMOUNT');
    ...
    function _transferNotify(
        ...
        ITeemoTransferListener(TRANSFER_LISTENER).transferNotify(from, to, token, amount);
    }

    function existPair(address tokenA, address tokenB) public view returns (bool) {
        return ITeemoFactory(FACTORY).getPair(tokenA, tokenB) != address(0);
    }

    function getReserves(address tokenA, address tokenB) public view returns (uint256, uint256) {
        return TeemoSwapLibrary.getReserves(FACTORY, tokenA, tokenB);
    }

    function pairFor(address tokenA, address tokenB) public view returns (address) {
        return TeemoSwapLibrary.pairFor(FACTORY, tokenA, tokenB);
    }
    ...
    function getAmountOut(
        ...
        return TeemoSwapLibrary.getAmountOut(amount, reserveIn, reserveOut);
    }
    ...
    function getAmountIn(
        ...
        uint256 percent = _getSwapFeePercent();
        uint256 amount = TeemoSwapLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    ...
    function migrateLiquidity(
        ...
        address newPair = ITeemoFactory(FACTORY).getPair(tokenA, tokenB);
        for (uint256 i = 0; i < users.length; i++) {
            uint256 liquidity = ITeemoPair(pair).balanceOf(users[i]);
            if (liquidity > 0) {
                ITeemoPair(pair).burn(users[i], newPair, liquidity);
                ITeemoPair(newPair).mint(users[i]);
                ITeemoFactory(FACTORY).addPlayerPair(users[i], newPair);
            }
        }

        ITeemoTransferListener(TRANSFER_LISTENER).upgradeProdutivity(pair, newPair);
    }
}
...
import './interfaces/ITeemoStaking.sol';
import './interfaces/ITeemoConfig.sol';
import './interfaces/ITeemoBallotFactory.sol';
import './interfaces/ITeemoBallot.sol';
import './interfaces/ITeemoBallotRevenue.sol';
import './interfaces/ITgas.sol';
import './interfaces/ITokenRegistry.sol';
import './libraries/ConfigNames.sol';
import './libraries/TransferHelper.sol';
...
contract TeemoGovernance is TgasStaking, Ownable, AccessControl {
    using SafeMath for uint256;
    ...
    uint256 public MINIMUM_TEEMO_REQUIRED_IN_BALANCE = 100e18;
    ...
    modifier onlyRewarder() {
        require(msg.sender == rewardAddr, 'TEEMOGovernance: ONLY_REWARDER');
        _;
    }

    modifier onlyRole(bytes32 role) {
        require(hasRole(role, msg.sender), 'TEEMOGovernance: sender not allowed to do!');
        _;
    }
    ...
    function initialize(
        ...
        require(
            ...
            'TEEMOGovernance: INPUT_ADDRESS_IS_ZERO'
        );
    ...
    function changeTeemoMinimumRequired(uint256 _newMinimum) external onlyRole(SUPER_ADMIN_ROLE) {
        require(_newMinimum != MINIMUM_TEEMO_REQUIRED_IN_BALANCE, 'TEEMOGovernance::TEEMO required is identical!');
        MINIMUM_TEEMO_REQUIRED_IN_BALANCE = _newMinimum;
    }

    // function changeProposalVoteDuration(uint _newDuration) external onlyRole(SUPER_ADMIN_ROLE) {
    //     require(_newDuration != VOTE_DURATION, "TEEMOGovernance::Vote duration has not changed");
    //     VOTE_DURATION = _newDuration;
    // }

    // function changeProposalFreezeDuration(uint _newDuration) external onlyRole(SUPER_ADMIN_ROLE) {
    //     require(_newDuration != FREEZE_DURATION, "TEEMOGovernance::Freeze duration has not changed");
    ...
    //     require(_newDuration != REVENUE_VOTE_DURATION, "TEEMOGovernance::Vote duration has not changed");
    ...
    //     require(_newDuration != REVENUE_FREEZE_DURATION, "TEEMOGovernance::Freeze duration has not changed");
    ...
    function vote(
        ...
        require(configBallots[_ballot] != REVENUE_PROPOSAL, 'TEEMOGovernance::Fail due to wrong ballot');
        ...
        if (_collateral > collateralRemain) {
            ...
            _transferForBallot(collateralMore, true, ITeemoBallot(_ballot).executionTime());
        }

        ITeemoBallot(_ballot).voteByGovernor(msg.sender, _proposal);
    ...
    function participate(address _ballot, uint256 _collateral) external {
        ...
        _transferForBallot(collateralMore, true, ITeemoBallot(_ballot).executionTime());
        ITeemoBallotRevenue(_ballot).participateByGovernor(msg.sender);
    }
    ...
    function audit(address _ballot) external returns (bool) {
        if (ballotTypes[_ballot] == T_CONFIG) {
            ...
            revert('TeemoGovernance: UNKNOWN_TYPE');
        }
    }

    function auditConfig(address _ballot) public returns (bool) {
        bool result = ITeemoBallot(_ballot).end();
        require(result, 'TeemoGovernance: NO_PASS');
        uint256 value = ITeemoBallot(_ballot).value();
        bytes32 name = configBallots[_ballot];
        result = ITeemoConfig(configAddr).changeConfigValue(name, value);
    ...
    function auditListToken(address _ballot) public returns (bool) {
        ...
        if (result) {
            burnAmount =
                ...
                ITeemoConfig(configAddr).PERCENT_DENOMINATOR();
    ...
    function auditToken(address _ballot) public returns (bool) {
        bool result = ITeemoBallot(_ballot).end();
        uint256 status = ITeemoBallot(_ballot).value();
        address token = tokenBallots[_ballot];
        address user = tokenUsers[token];
        require(ITokenRegistry(configAddr).tokenStatus(token) != status, 'TeemoGovernance: TOKEN_STATUS_NO_CHANGE');
        if (result) {
            ITokenRegistry(configAddr).updateToken(token, status);
        } else {
    ...
    function getConfigValue(bytes32 _name) public view returns (uint256) {
        return ITeemoConfig(configAddr).getConfigValue(_name);
    }
    ...
    function _createProposalPrecondition(uint256 _amount, uint256 _executionTime) private {
        ...
        if (!hasRole(DEFAULT_ADMIN_ROLE, sender)) {
            require(
                IERC20(baseToken).balanceOf(sender).add(balanceOf[sender]) >= MINIMUM_TEEMO_REQUIRED_IN_BALANCE,
                'TEEMOGovernance::Require minimum TEEMO in balance'
            );
            require(
                _amount >= getConfigValue(ConfigNames.PROPOSAL_TGAS_AMOUNT),
                'TEEMOGovernance: NOT_ENOUGH_AMOUNT_TO_PROPOSAL'
            );
            ...
            require(
                ...
                'TEEMOGovernance: COLLATERAL_NOT_ENOUGH_AMOUNT_TO_PROPOSAL'
            );
        ...
        returns (address)
    {
        ...
        address ballotAddr = ITeemoBallotFactory(ballotFactoryAddr).createShareRevenue(
            msg.sender,
            endTime,
            executionTime,
            _subject,
    ...
    function createSnapshotBallot(
        ...
        address ballotAddr = ITeemoBallotFactory(ballotFactoryAddr).create(
            msg.sender,
            0,
            endTime,
            executionTime,
    ...
    function createConfigBallot(
        ...
        require(_value >= 0, 'TeemoGovernance: INVALID_PARAMTERS');
        {
            // avoids stack too deep errors
            (uint256 minValue, uint256 maxValue, uint256 maxSpan, uint256 value, uint256 enable) = ITeemoConfig(
                configAddr
            ).getConfig(_name);
            require(enable == 1, 'TeemoGovernance: CONFIG_DISABLE');
            require(_value >= minValue && _value <= maxValue, 'TeemoGovernance: OUTSIDE');
            uint256 span = _value >= value ? (_value - value) : (value - _value);
            require(maxSpan >= span, 'TeemoGovernance: OVERSTEP');
        }
    ...
    function createTokenBallot(
        ...
        require(!_isDefaultToken(_token), 'TeemoGovernance: DEFAULT_LIST_TOKENS_PROPOSAL_DENY');
        uint256 status = ITokenRegistry(configAddr).tokenStatus(_token);
        require(status == ITokenRegistry(configAddr).PENDING(), 'TeemoGovernance: ONLY_ALLOW_PENDING');
        require(
            _value == ITokenRegistry(configAddr).OPENED() || _value == ITokenRegistry(configAddr).CLOSED(),
            'TeemoGovernance: INVALID_STATUS'
        );
        require(status != _value, 'TeemoGovernance: STATUS_NO_CHANGE');
    ...
    function listToken(
        ...
        require(
            ...
            'TeemoGovernance: LISTED'
        );
        // require(_amount >= getConfigValue(ConfigNames.LIST_TGAS_AMOUNT), "TeemoGovernance: NOT_ENOUGH_AMOUNT_TO_LIST");
    ...
    function collectReward(address _ballot) external returns (uint256) {
        require(block.timestamp >= ITeemoBallot(_ballot).endTime(), 'TeemoGovernance: NOT_YET_ENDED');
        require(!collectUsers[_ballot][msg.sender], 'TeemoGovernance: REWARD_COLLECTED');
        require(configBallots[_ballot] == REVENUE_PROPOSAL, 'TeemoGovernance::Fail due to wrong ballot');
    ...
    //     if (block.timestamp < ITeemoBallot(_ballot).endTime() || collectUsers[_ballot][msg.sender]) {
    //         return 0;
    //     }
    //     uint amount;
    //     uint shares = ballotOf[_ballot];

    //     bool result = ITeemoBallot(_ballot).result();

    //     if (result) {
    //         uint extra;
    //         uint rewardRate = getConfigValue(ConfigNames.VOTE_REWARD_PERCENT);
    //         if ( rewardRate > 0) {
    //            extra = shares * rewardRate / ITeemoConfig(configAddr).PERCENT_DENOMINATOR();
    //            shares -= extra;
    //         }
    //         if (msg.sender == ITeemoBallot(_ballot).proposer()) {
    //             amount = extra;
    //         }
    //     }

    //     if (ITeemoBallot(_ballot).total() > 0) {
    //         uint reward = shares * ITeemoBallot(_ballot).weight(msg.sender) / ITeemoBallot(_ballot).total();
    //         amount += ITeemoBallot(_ballot).proposer() == msg.sender ? 0: reward;
    //     }
    //     return amount;
    // }

    function getRewardForRevenueProposal(address _ballot) public view returns (uint256) {
        if (block.timestamp < ITeemoBallotRevenue(_ballot).endTime() || collectUsers[_ballot][msg.sender]) {
            return 0;
        }
        ...
        if (ITeemoBallotRevenue(_ballot).total() > 0) {
            uint256 reward = (shares * ITeemoBallotRevenue(_ballot).weight(msg.sender)) /
                ITeemoBallotRevenue(_ballot).total();
            amount += ITeemoBallotRevenue(_ballot).proposer() == msg.sender ? 0 : reward;
        }
        return amount;
    }
    ...
    function addReward(uint256 _value) external returns (bool) {
        require(_value > 0, 'TeemoGovernance: ADD_REWARD_VALUE_IS_ZERO');
        uint256 total = IERC20(baseToken).balanceOf(address(this));
        uint256 diff = total.sub(totalSupply);
        require(_value <= diff, 'TeemoGovernance: ADD_REWARD_EXCEED');
    ...
    function _rewardTransfer(
        ...
        require(_value >= 0 && rewardOf[_from] >= _value, 'TeemoGovernance: INSUFFICIENT_BALANCE');
    ...
    function _isDefaultToken(address _token) internal returns (bool) {
        address[] memory tokens = ITeemoConfig(configAddr).getDefaultListTokens();
    ...
    function upgradeApproveReward() external returns (uint256) {
        require(rewardOf[rewardAddr] > 0, 'TeemoGovernance: UPGRADE_NO_REWARD');
        require(ITeemoConfig(configAddr).governor() != address(this), 'TeemoGovernance: UPGRADE_NO_CHANGE');
        TransferHelper.safeApprove(baseToken, ITeemoConfig(configAddr).governor(), rewardOf[rewardAddr]);
        return rewardOf[rewardAddr];
    }

    function receiveReward(address _from, uint256 _value) external returns (bool) {
        require(_value > 0, 'TeemoGovernance: RECEIVE_REWARD_VALUE_IS_ZERO');
...
contract TeemoPool is Ownable {
    ...
    event ClaimReward(address indexed user, address indexed pair, address indexed rewardToken, uint256 amountTEEMO);
    ...
    function upgrade(address _newPool, address[] calldata _pairs) external onlyOwner {
        IERC20(TEEMO).approve(_newPool, totalReward);
        for (uint256 i = 0; i < _pairs.length; i++) {
            if (pairReward[_pairs[i]] > 0) {
                TeemoPool(_newPool).addReward(_pairs[i], pairReward[_pairs[i]]);
    ...
    function addRewardFromPlatform(address _pair, uint256 _amount) external {
        require(msg.sender == PLATFORM, 'Teemo POOL: FORBIDDEN');
        uint256 balanceOf = IERC20(TEEMO).balanceOf(address(this));
        require(balanceOf.sub(totalReward) >= _amount, 'TEEMO POOL: ADD_REWARD_EXCEED');

        uint256 rewardAmount = ITeemoConfig(CONFIG).getConfigValue(ConfigNames.FEE_LP_REWARD_PERCENT).mul(_amount).div(
            10000
        );
        _addReward(_pair, rewardAmount);

        uint256 remainAmount = _amount.sub(rewardAmount);
        uint256 fundingAmount = ITeemoConfig(CONFIG)
            .getConfigValue(ConfigNames.FEE_FUNDME_REWARD_PERCENT)
            .mul(remainAmount)
            .div(10000);

        if (fundingAmount > 0) {
            TransferHelper.safeTransfer(TEEMO, FUNDING, fundingAmount);
        }

        remainAmount = remainAmount.sub(fundingAmount);
        uint256 lotteryAmount = ITeemoConfig(CONFIG)
            .getConfigValue(ConfigNames.FEE_LOTTERY_REWARD_PERCENT)
            .mul(remainAmount)
            .div(10000);

        if (lotteryAmount > 0) {
            TransferHelper.safeTransfer(TEEMO, LOTTERY, lotteryAmount);
        }

        remainAmount = remainAmount.sub(lotteryAmount);
        uint governanceAmount = ITeemoConfig(CONFIG).getConfigValue(ConfigNames.FEE_GOVERNANCE_REWARD_PERCENT).mul(remainAmount).div(10000);
        if (remainAmount > 0) {
            TransferHelper.safeTransfer(TEEMO, GOVERNANCE, remainAmount);
            ITeemoGovernance(GOVERNANCE).addReward(remainAmount);
        }
    ...
    function addReward(address _pair, uint256 _amount) external {
        TransferHelper.safeTransferFrom(TEEMO, msg.sender, address(this), _amount);

        require(ITeemoFactory(FACTORY).isPair(_pair), 'Teemo POOL: INVALID PAIR');
    ...
    function _addReward(address _pair, uint256 _amount) internal {
        ...
        uint256 totalProdutivity = ITeemoPair(_pair).totalSupply();
    ...
    function _auditUser(address _pair, address _user) internal {
        ...
        uint256 balance = ITeemoPair(_pair).balanceOf(_user);
    ...
    function claimReward(address _pair, address _rewardToken) external {
        ...
        if (_rewardToken == TEEMO) {
            TransferHelper.safeTransfer(TEEMO, msg.sender, amount);
        } else if (_rewardToken == WETH) {
            require(ITeemoFactory(FACTORY).isPair(_pair), 'Teemo POOL: INVALID PAIR');
            IERC20(TEEMO).approve(PLATFORM, amount);
            address[] memory path = new address[](2);
            path[0] = TEEMO;
            path[1] = WETH;
            ITeemoPlatform(PLATFORM).swapExactTokensForETH(amount, 0, path, msg.sender, block.timestamp + 1);
        } else {
            require(ITeemoFactory(FACTORY).isPair(_pair), 'Teemo POOL: INVALID PAIR');
            IERC20(TEEMO).approve(PLATFORM, amount);
            address[] memory path = new address[](2);
            path[0] = TEEMO;
            path[1] = _rewardToken;
            ITeemoPlatform(PLATFORM).swapExactTokensForTokens(amount, 0, path, msg.sender, block.timestamp + 1);
        }
...
contract TeemoFunding is Ownable {
    address public teemo;
    ...
    modifier inClaimable(address _userAddress) {
        require(included[_userAddress], 'teemoFunding::User not in claimable list!');
        _;
    }

    modifier notInClaimable(address _userAddress) {
        require(!included[_userAddress], 'teemoFunding::User already in claimable list!');
        _;
    }

    function setTeemo(address _teemo) public onlyOwner {
        teemo = _teemo;
        emit FundingTokenSettled(_teemo);
    }

    function grantClaimable(address _userAddress) public onlyOwner notInClaimable(_userAddress) {
        require(_userAddress != address(0), 'teemoFunding::User address is not legit!');
    ...
    function claim(uint256 _amount) public inClaimable(msg.sender) {
        uint256 remainBalance = IERC20(teemo).balanceOf(address(this));
        require(remainBalance >= _amount, 'teemoFunding::Remain balance is not enough to claim!');

        TransferHelper.safeTransfer(address(teemo), msg.sender, _amount);
...
contract TeemoConfig is TokenRegistry, Ownable {
    uint256 public version = 1;
    ...
    modifier onlyPlatform() {
        require(msg.sender == platform, 'TeemoConfig: ONLY_PLATFORM');
        _;
    }
    ...
    function initialize(
        ...
        require(_tgas != address(0), 'TeemoConfig: ZERO ADDRESS');
        tgas = _tgas;
        platform = _platform;
        dev = _dev;
    ...
    function modifyDev(address _new) public {
        require(msg.sender == dev, 'TeemoConfig: FORBIDDEN');
        dev = _new;
    }
    ...
    function changeConfigValue(bytes32 _name, uint256 _value) external onlyGovernor returns (bool) {
        Config storage config = configs[_name];
        require(config.enable == 1, 'TeemoConfig: DISABLE');
        require(_value <= config.maxValue && _value >= config.minValue, 'TeemoConfig: OVERFLOW');
        uint256 old = config.value;
        uint256 span = _value >= old ? (_value - old) : (old - _value);
        require(span <= config.maxSpan, 'TeemoConfig: EXCEED MAX ADJUST SPAN');
        config.value = _value;
...
import './interfaces/ITgas.sol';
import './interfaces/IERC20.sol';
import './interfaces/ITeemoCallee.sol';
import './interfaces/ITeemoConfig.sol';
import './modules/BaseShareField.sol';
import './libraries/ConfigNames.sol';

contract TeemoPair is BaseShareField {
    uint256 public version = 1;
    using SafeMath for uint256;
    ...
    event ClaimTEEMO(address indexed player, uint256 pairMint, uint256 userMint);
    ...
    //         uint devAmount = reward * ITeemoConfig(CONFIG).getConfigValue(ConfigNames.DEV_PRECENT) / 10000;
    //         address devAddress = ITeemoConfig(CONFIG).dev();
    ...
    //     uint devPercent = ITeemoConfig(CONFIG).getConfigValue(ConfigNames.DEV_PRECENT);
    //     uint pairReward = IERC20(Teemo).take().mul(10000 - devPercent).div(10000);
    ...
    function _safeTransfer(
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'Teemo PAIR : TRANSFER_FAILED');
    }
    ...
    modifier onlyPlatform() {
        address platform = ITeemoConfig(CONFIG).platform();
        require(msg.sender == platform, 'Teemo PAIR : FORBIDDEN');
        _;
    }
    ...
    function initialize(
        ...
        require(msg.sender == FACTORY, 'teemo PAIR : FORBIDDEN');
        token0 = _token0;
        token1 = _token1;
        CONFIG = _config;
        TEEMO = _teemo;
        _setShareToken(TEEMO);
    }
    ...
    function _update(
        ...
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'TEEMO PAIR : OVERFLOW');
    ...
    function mint(address to) external onlyPlatform lock returns (uint256 liquidity) {
        ...
        require(liquidity > 0, 'TEEMO PAIR : INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
        // _mintTGAS();
    ...
    function burn(
        ...
        require(
            block.number >=
                lastMintBlock[from] + ITeemoConfig(CONFIG).getConfigValue(ConfigNames.REMOVE_LIQUIDITY_DURATION),
            'Teemo PLATFORM : REMOVE LIQUIDITY DURATION FAIL'
        );
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        ...
        require(balanceOf[from] >= amount, 'Teemo PAIR : INSUFFICIENT_LIQUIDITY_AMOUNT');
        ...
        require(amount0 > 0 && amount1 > 0, 'Teemo PAIR : INSUFFICIENT_LIQUIDITY_BURNED');
        _burn(from, amount);
        // _mintTGAS();
    ...
    function swap(
        ...
        require(amount0Out > 0 || amount1Out > 0, 'Teemo PAIR : INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'Teemo PAIR :  INSUFFICIENT_LIQUIDITY');
        uint256 balance0;
        uint256 balance1;
        {
            address _token0 = token0;
            address _token1 = token1;
            require(to != _token0 && to != _token1, 'Teemo PAIR : INVALID_TO');
            if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out);
            if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out);
            if (data.length > 0) ITeemoCallee(to).teemoCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = _balanceOf(_token0, address(this));
            balance1 = _balanceOf(_token1, address(this));
        }
        ...
        require(amount0In > 0 || amount1In > 0, 'teemo PAIR : INSUFFICIENT_INPUT_AMOUNT');
        _update(balance0, balance1, _reserve0, _reserve1);
    ...
    function swapFee(
        ...
        require(to != token0 && to != token1, 'teemo PAIR : INVALID_TO');
    ...
    function mintReward() external lock returns (uint256 userReward) {
        ...
        emit ClaimTEEMO(msg.sender, remainReward, userReward);
    }

    function getTEEMOReserve() public view returns (uint256) {
        return _balanceOf(TEEMO, address(this));
    }

    function _balanceOf(address token, address owner) internal view returns (uint256) {
        if (token == TEEMO && owner == address(this)) {
...
contract TeemoLP is BaseShareField {
    // ERC20 Start

    using SafeMath for uint256;

    string public constant name = 'Teemo LP';
    ...
    function _transfer(
        ...
        ITeemoDelegate(owner).addPlayerPair(to);
        _mintReward();
    ...
    function initialize(
        ...
        require(msg.sender == owner, 'TEEMO LP Forbidden');
        tokenA = _tokenA;
        tokenB = _tokenB;
        _setShareToken(_TEEMO);
        PLATFORM = _PLATFORM;
        POOL = _POOL;
        WETH = _WETH;
    }
        ...
        )
    {
        ...
        (_amountToken, _amountETH, _liquidity) = ITeemoPlatform(PLATFORM).addLiquidityETH{value: msg.value}(
            token,
            amountTokenDesired,
            amountTokenMin,
            amountETHMin,
        ...
        )
    {
        ...
        (_amountA, _amountB, _liquidity) = ITeemoPlatform(PLATFORM).addLiquidity(
            tokenA,
            tokenB,
            amountA,
            amountB,
    ...
    function removeLiquidityETH(
        ...
        (_amountToken, _amountETH) = ITeemoPlatform(PLATFORM).removeLiquidityETH(
            token,
            liquidity,
            amountTokenMin,
            amountETHMin,
    ...
    function removeLiquidity(
        ...
        require(msg.sender == owner, 'Teemo LP Forbidden');
        (_amountA, _amountB) = ITeemoPlatform(PLATFORM).removeLiquidity(
            tokenA,
            tokenB,
            liquidity,
            amountAMin,
    ...
    function _currentReward() internal view override returns (uint256) {
        address pair = ITeemoPlatform(PLATFORM).pairFor(tokenA, tokenB);
        uint256 countractAmount = mintedShare.add(IERC20(shareToken).balanceOf(address(this))).sub(totalShare);
        if (pair != address(0)) {
            uint256 poolAmount = ITeemoPool(POOL).queryReward(pair, address(this));
            // (uint pairAmount, ) = ITeemoPair(pair).queryReward();
    ...
    function _mintReward() internal {
        ...
        if (pair != address(0)) {
            ...
            // (uint pairAmount, ) = ITeemoPair(pair).queryReward();
            if (poolAmount > 0) {
                ITeemoPool(POOL).claimReward(pair, shareToken);
            }

            // if(pairAmount > 0) {
            //     ITeemoPair(pair).mintReward();
            // }
        }
    }
...
contract TeemoDelegate is Ownable {
    using SafeMath for uint256;
    ...
    function upgradePlatform(address _PLATFORM) external onlyOwner {
        for (uint256 i = 0; i < allPairs.length; i++) {
            ITeemoLP(allPairs[i]).upgrade(_PLATFORM);
        }
    }
        ...
        )
    {
        ...
        (_amountToken, _amountETH, _liquidity) = ITeemoLP(pair).addLiquidityETH{value: msg.value}(
            msg.sender,
            amountTokenDesired,
            amountTokenMin,
            amountETHMin,
        ...
        )
    {
        ...
        if (tokenA != ITeemoLP(pair).tokenA()) {
        ...
        (_amountA, _amountB, _liquidity) = ITeemoLP(pair).addLiquidity(
            msg.sender,
            amountA,
            amountB,
            amountAMin,
    ...
    function removeLiquidityETH(
        ...
        address pair = getPair[token][WETH];
        (_amountToken, _amountETH) = ITeemoLP(pair).removeLiquidityETH(
            msg.sender,
            liquidity,
            amountTokenMin,
            amountETHMin,
    ...
    function removeLiquidity(
        ...
        address pair = getPair[tokenA][tokenB];
        (_amountA, _amountB) = ITeemoLP(pair).removeLiquidity(msg.sender, liquidity, amountAMin, amountBMin, deadline);
    }

    function _createPair(address tokenA, address tokenB) internal returns (address pair) {
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'Teemo FACTORY: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'Teemo FACTORY: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(TeemoLP).creationCode;
        ...
        isPair[pair] = true;
        ITeemoLP(pair).initialize(token0, token1, TEEMO, POOL, PLATFORM, WETH);
...
contract ERC2917Impl is IERC2917, UpgradableProduct, UpgradableGovernance {
    ...
    ) public UpgradableProduct() UpgradableGovernance() {
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
    ...
    function changeInterestRatePerBlock(uint256 value) external virtual override requireGovernor returns (bool) {
    ...
    // the users' actual share percentage will calculated by:
    // Formula:     user_productivity / global_productivity
    function increaseProductivity(address user, uint256 value) external virtual override requireImpl returns (bool) {
        require(value > 0, 'PRODUCTIVITY_VALUE_MUST_BE_GREATER_THAN_ZERO');
    ...
    // This function will decreases user's productivity by value, and updates the global productivity
    // it will record which block this is happenning and accumulates the area of (productivity * time)
    function decreaseProductivity(address user, uint256 value) external virtual override requireImpl returns (bool) {
        UserInfo storage userInfo = users[user];
        require(value > 0 && userInfo.amount >= value, 'INSUFFICIENT_PRODUCTIVITY');
        _update();
        _audit(user);
    ...
    function take() external view virtual override returns (uint256) {
        return takeWithAddress(msg.sender);
    }

    // Returns how much a user could earn plus the giving block number.
    function takeWithBlock() external view virtual override returns (uint256, uint256) {
    ...
    // When user calls this function, it will calculate how many token will mint to user from his productivity * time
    // Also it calculates global token supply from last time the user mint to this time.
    function mint() external virtual override returns (uint256) {
        _update();
        _audit(msg.sender);
    ...
    // Returns how many productivity a user has and global has.
    function getProductivity(address user) external view virtual override returns (uint256, uint256) {
        return (users[user].amount, totalProductivity);
    }

    // Returns the current gorss product rate.
    function interestsPerBlock() external view virtual override returns (uint256) {
        return accAmountPerShare;
    }
}
...
import './TeemoPair.sol';
import './interfaces/ITeemoConfig.sol';

contract TeemoFactory {
    uint256 public version = 1;
    address public TEEMO;
    address public CONFIG;
    address public owner;
    ...
    constructor(address _TEEMO, address _CONFIG) public {
        TEEMO = _TEEMO;
        CONFIG = _CONFIG;
        owner = msg.sender;
    }
    ...
    function updateConfig(address _CONFIG) external {
        require(msg.sender == owner, 'TEEMO FACTORY: PERMISSION');
        CONFIG = _CONFIG;
        for (uint256 i = 0; i < allPairs.length; i++) {
            TeemoPair(allPairs[i]).initialize(
                TeemoPair(allPairs[i]).token0(),
                TeemoPair(allPairs[i]).token1(),
                _CONFIG,
                TEEMO
            );
        }
    ...
    function addPlayerPair(address _player, address _pair) external returns (bool) {
        require(msg.sender == ITeemoConfig(CONFIG).platform(), 'Teemo FACTORY: PERMISSION');
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(msg.sender == ITeemoConfig(CONFIG).platform(), 'Teemo FACTORY: PERMISSION');
        require(tokenA != tokenB, 'Teemo FACTORY: IDENTICAL_ADDRESSES');
        require(
            ITeemoConfig(CONFIG).checkToken(tokenA) && ITeemoConfig(CONFIG).checkToken(tokenB),
            'Teemo FACTORY: NOT LIST'
        );
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'Teemo FACTORY: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'Teemo FACTORY: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(TeemoPair).creationCode;
        if (uint256(contractCodeHash) == 0) {
            contractCodeHash = keccak256(bytecode);
        }
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        ...
        isPair[pair] = true;
        TeemoPair(pair).initialize(token0, token1, CONFIG, TEEMO);
        getPair[token0][token1] = pair;
        getPair[token1][token0] = pair; // populate mapping in the reverse direction
        allPairs.push(pair);
        emit PairCreated(token0, token1, pair, allPairs.length);
...
contract TeemoToken is ERC20 {
    ...
    constructor(
        ...
        address develop,
        address liquidity
    ) public ERC20('TEEMO', 'TEEMO') {
        require(community != address(0), 'TeemoToken: ZERO ADDRESS');
        require(develop != address(0), 'TeemoToken: ZERO ADDRESS');
        require(liquidity != address(0), 'TeemoToken: ZERO ADDRESS');
    ...
    function addLocker(uint8 pool_index, address pool_address) external {
        ...
        require(pool_address != address(0), 'TeemoToken: ZERO ADDRESS');
