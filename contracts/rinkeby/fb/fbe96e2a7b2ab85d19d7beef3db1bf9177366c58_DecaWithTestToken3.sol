...
contract DecaWithTestToken3 is VRFConsumerBaseV2, Pausable, Ownable, ReentrancyGuard {
    ...
    uint256 private maxBet = 10;
    uint256 private minBet = 1;
    ...
    Random[] private randoms;

    Balance private housePoolBalance;
    Balance private treasuryPoolBalance;
    mapping(address => Balance) private userBankRollBalance;
    mapping(address => Balance) private userHouseBalance;
    ...
    event ReceivedBetResult(uint256 indexed requestId, uint256 response);
    event ClaimPrizesEvent(address indexed requestUser, uint256 claimAmount);
    event DebuggingEvent(uint line);
    ...
    function bet(uint256 _betAmount, uint256 _faceSide) external {
        ...
        emit RequestedBet(s_requestId, msg.sender, _faceSide, _betAmount);
    }
    ...
    function fulfillRandomWords(
        ...
        emit DebuggingEvent(1511);
        handleFee(betAmount);
        emit DebuggingEvent(1513);
        if (userWon) {
            userBankRollBalance[msg.sender].depositAmount.add(winMoney).add(winMoney);
    ...
    function handleFee(uint256 _betAmount) private {
        uint256 _fee = _betAmount.mul(fee).div(100);
