...
import "./StampingDatabaseDeployer.sol";

import "hardhat/console.sol";

/// @title Canonical Uniswap V3 factory
/// @notice Deploys Uniswap V3 pools and manages ownership and control over pool protocol fees
contract StampingHub is IStampingHub, StampingDatabaseDeployer {
    /// @inheritdoc IStampingHub
    address public override owner;
    
    /// @inheritdoc IStampingHub
    mapping(address => address) public override getDatabase;
    ...
    /// @inheritdoc IStampingHub
    function createDatabase(address token) external override returns (address database) {
        require(getDatabase[token] == address(0), "Database already exists");
        database = deploy(address(this), token);
        getDatabase[token] = database;
        emit DatabaseCreated(token, database);
    }
...
import "./StampingDatabase.sol";

contract StampingDatabaseDeployer is IStampingDatabaseDeployer {
    struct Parameters {
        address hub;
        address token;
    }

    /// @inheritdoc IStampingDatabaseDeployer
    Parameters public override parameters;

    /// @dev Deploys a database with the given parameters by transiently setting the parameters storage slot and then
    /// clearing it after deploying the database.
    /// @param hub The contract address of the Stamping Hub
    /// @param token The address of token for database
    function deploy(
        address hub,
        address token
    ) internal returns (address database) {
        parameters = Parameters({hub: hub, token: token});
        database = address(new StampingDatabase{salt: keccak256(abi.encode(token))}());
        delete parameters;
    }
}
...
contract StampingDatabase is IStampingDatabase {
    ...
    function checkValidTrait(bytes32 title, bytes32 trait) private view returns (bool) {
        (bytes32[] memory traits) = abi.decode(categories[title].traits, (bytes32[]));
    ...
    /// @inheritdoc IStampingDatabase
    function addCategory(bytes32 title, bytes32 description, bytes calldata traits, bool editable) external override {
        require(!categories[title].activated, "This category already exist");
        categories[title] = IStampingDatabase.Category(description, traits, editable, true);
        emit CategoryAdded(title, description, traits, editable, msg.sender);
    }
    ...
    function stamp(bytes32 title, bytes32 trait, uint256 tokenId) external override {
        ...
        if (category.traits.length > 0) {
            require(checkValidTrait(title, trait), "Invalid trait for this category");
        }
...
interface IStampingDatabase {
    ...
    function categories(bytes32 title) external view returns (bytes32 description, bytes memory traits, bool editable, bool activate);
    ...
    /// @param traits The trait list that can be use for stamping (can be NULL to allow any trait)
    /// @param editable The status to determine whether the trait can be change later or not
    function addCategory(bytes32 title, bytes32 description, bytes calldata traits, bool editable) external;
