...
import "./FimbMathLib.sol";
import "./IOffer_v1.sol";

contract BaseToken is ERC20Snapshot, Ownable {
    string public constant TEXT__FIELD = "WEBSITE - ";
    uint256 public constant TOTAL_TOKENS = 10414.112 * 1 ether;
    uint256 public constant TOKEN_BASE_RATE = 2500;
    uint256 public constant DATE_INTEREST_START = 1657162800;
    string public constant TEXT__FIELD__1 = "CNPJ - SACADO:xx.xxx.xxx/xxxx-xx";
    string public constant TEXT__FIELD__2 = "NOTA FISCAL - SACADO:xxxx";
    string public constant DOCUMENT = "";
    uint256 public constant DAILY_DISCOUNT_RATE = 0.04141697445660952 * 1 ether;
    uint256[] private PERIOD_VALUES = [26624000 * 1 ether];
    uint256[] private PERIOD_DATES = [1661918400];
    address public constant OWNER = 0x17d6FF647015Eb09877d96c109A4AEe1f788dfeF;
    // Name of the token
    string public constant TOKEN_NAME = "FimbTestToken";
    // Symbol of the token
    string public constant TOKEN_SYMBOL = "FTT01";
    ...
    function days360(
        ...
        // if start date is the last day of the month, it is treated like the 30th day of that month
        ...
        // when the end date is the last day of the month,
        // and the start date is less than 30, the end date is treated as the 1st of the next month,
        // otherwise the end date is treated like the 30th of the same month.
    ...
    function initialize() public {
        require(!bInitialized, "Contract already initialized");
        ...
        for (uint8 i = 0; i < PERIOD_VALUES.length; i++) {
            ...
            uint256 nDays = days360(DATE_INTEREST_START, nPeriodDate, false);
            // total discount = daily discount ^ number of days
            uint256 nDiscountRate = nDailyDiscount;
            for (uint8 j = 1; j < nDays; j++) {
        ...
        nTotalInterest = FimbMathLib.mulDiv(
            nTotalValue,
            1 ether,
        ...
        for (uint8 i = 0; i < PERIOD_VALUES.length; i++) {
            uint256 nPeriodValue = PERIOD_VALUES[i];
            uint256 nInterest = FimbMathLib.mulDiv(
                nPeriodValue,
                1 ether,
                nTotalDiscountedValue
            );
        ...
        returns (uint256)
    {
        ...
        if (nTokenBalance == 0) {
            ...
            uint256 nToReceive = FimbMathLib.mulDiv(
                nTokenBalance,
                nPaymentValue,
                nTokenSuppy
            );
        ...
        returns (uint256)
    {
        ...
        for (uint8 i = 0; i < PERIOD_DATES.length; i++) {
            ...
            if (_nPaymentDate >= nPeriodDate) {
                ...
                uint256 nDifInterest = FimbMathLib.mulDiv(
     ...
     * @notice Método para iniciar uma oferta de venda de token Fimb (parte do sistema interno de deployment)
     */
