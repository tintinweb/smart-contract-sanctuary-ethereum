...
contract TheAcidWalrus is ERC721URIStorage, Ownable {
  ...
  uint public constant NUMBER_RESERVED_WALRUS = 10;
  // uint public constant MAX_WALRUS_PRE_SALE = 3000;
  uint public constant MAX_WALRUS_PRE_SALE = 100;

  // uint256 public PRICE = 0.123 ether;
  uint256 public PRICE = 0.00123 ether;
  ...
  constructor () ERC721("Walrus", "WAL") {}

  function mintWalrus(uint256 amount) external payable {
    require(msg.sender == tx.origin, "No transaction from smart contracts!");
    require(saleIsActive, "Sale must be active to mint");
    require(msg.value > amount * PRICE, "Not enough ETH for transaction");
    require(amount > 0 && amount <= 3, "Max 3 NFTs per transaction");
    require(mintedWalrus.current() < MAX_SUPPLY + amount, "Max Supply Reached");
  ...
  function mintWalrusPreSale(uint256 amount) external payable {
    require(msg.sender == tx.origin, "No transaction from smart contracts!");
    ...
    require(balanceOf(msg.sender) + amount <= 3, "Limit is 3 walrus per wallet on pre-sale, sale not allowed");
  ...
  function withdraw() external onlyOwner {
    ...
    address t1 = 0xa6b95B1b210600aCDa237b0Cad062Cc6C16Ce641;

    uint256 _balance = address(this).balance;
    uint256 _split = _balance.mul(30).div(100);

    require(payable(owner()).send(_split));
