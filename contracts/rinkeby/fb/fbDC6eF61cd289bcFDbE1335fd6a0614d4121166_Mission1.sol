...
contract Mission1 is IERC721Receiver, ReentrancyGuard {
    ...
    event NftStaked(address indexed staker, uint256[] indexed tokenId, uint256 time);
    event NftUnStaked(address indexed staker, uint256[] indexed tokenId, uint256 time);
    event StakePayout(address indexed staker, uint256 tokenId, uint256 stakeAmount, uint256 startTime, uint256 end);
    ...
    function stakeNFT(uint256[] memory tokenId) public nonReentrant {
        ...
        for (uint256 i = 0; i < tokenId.length; i++) {
            ...
            _nftToken.safeTransferFrom(msg.sender, address(this), tokenId[i]);
        }
        receipt[msg.sender].push(Stake({
            id: tokenId,
    ...
    function unStakeNFT(uint256 depositId) external nonReentrant requireTimeElapsed(depositId) {
        ...
        _nftToken.safeTransferFrom(address(this), msg.sender, deposit.id[indx]);
        }

        receipt[msg.sender][depositId] = receipt[msg.sender][receipt[msg.sender].length - 1];
    ...
    function _payoutStake(uint256 depositId) internal {
        ...
        if (_erc20Token.balanceOf(address(this)) < payout) {
            emit StakePayout(msg.sender, depositId, 0, receipt[msg.sender][depositId].start, block.timestamp);
            return;
        }
        ...
        emit StakePayout(msg.sender, depositId, payout, receipt[msg.sender][depositId].start, block.timestamp);
    } 
    ...
    function cumulativeRewardsOf(uint256 depositId) public view returns(uint256) {
        Stake memory deposit = receipt[msg.sender][depositId];
        uint256 cumulativeRewards;
        for(uint indx = 0; indx < deposit.id.length; indx++) {
    ...
    function getUserDeposits(address user) external view returns(Stake[] memory) {
        return receipt[user];
    }
}
