...
contract TempleExploration is ReentrancyGuard {
    function explore(uint256 _difficulty) external nonReentrant {} 

    function worship(uint256 _tokenId) external {}

    function gearUp(uint256 _num_fragment) external nonReentrant {}

    function ranking(address _address) public pure returns (uint256) {}
}
...
contract OhoohAhaah is Ownable, ERC721A {
     ...
     */
    function WLMonkeOut(uint256 _quantity, bytes32[] calldata _proof)
        external
        callerIsUser
    ...
    external 
    atPhase(AdventurePhase.TEMPLE_EXPLORATION)
    onlyOwner {
        templeAddress = TempleExploration(_templeAddress);
    }

    /// @notice Daily event. Oh Ooh Ah Aah Aaah!
    function templeExplore(uint256 _difficulty) external {
        templeAddress.explore(_difficulty);
    }

    function relicWorship(uint256 _tokenId) external {
        templeAddress.worship(_tokenId);
    }

    function chapter2Ranking(address _address) public view returns (uint256) {
       return templeAddress.ranking(_address);
    }

    /// @notice Trade relic fragments for banana harvest gear
    function gearUp(uint256 _num_fragment) external {
        templeAddress.gearUp(_num_fragment);
    } 

    /*=====================================================================
    ==                     Functions for HARVEST Phase                   ==
    =====================================================================*/
    function InitHarvest(address _bananaTokenAddress) 
    external 
    atPhase(AdventurePhase.HARVEST)
    onlyOwner {
    ...
    function _beforeTokenTransfers(
        ...
        if (to != address(0)) {
            IBananaToken(bananaToken).startHarvest(from, _quantity);
        }
