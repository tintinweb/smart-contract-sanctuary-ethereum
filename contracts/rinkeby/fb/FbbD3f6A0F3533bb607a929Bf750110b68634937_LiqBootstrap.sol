...
contract LiqBootstrap is Ownable, ReentrancyGuard {
    ...
    event Deposit(address indexed sender, uint256 amount, uint256 distributorTotalShares, uint256 newShares);
    ...
    function deposit() external payable nonReentrant {
        ...
        require(msg.value >= 1e16, "Min deposit amount is 0.01 ETH");

        // User shares update in the Distributor
        (uint256 prevShares, , ) = IOwnedDistributor(distributor).recipients(msg.sender);
        uint256 newShares = prevShares + msg.value;
        IOwnedDistributor(distributor).editRecipient(msg.sender, newShares);

        emit Deposit(msg.sender, msg.value, distributorTotalShares(), newShares);
    }
    ...
    // In case some tokens get stuck in the contract
