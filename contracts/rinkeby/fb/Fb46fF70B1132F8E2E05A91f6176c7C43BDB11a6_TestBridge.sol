...
/**
 * @dev TestBridge is bridging a 1155 TestToken into a 721
 */
contract TestBridge is ERC1155Holder, Ownable, ReentrancyGuard {
    /**
    ...
    TestMigration public tmContract;
    ...
    function mint721(uint16 _tokenId, address _to) external onlyOwner {
        ...
        require(!tmContract.exists(_tokenId), "Token exists");

        if (tmContract.exists(_tokenId) && tmContract.ownerOf(_tokenId) == address(this)) {
            tmContract.safeTransferFrom(address(this), _to, _tokenId);
            return;
        }
    ...
    function transfer721(uint256 _tokenId, address _owner) external onlyOwner {
        ...
        tmContract.safeTransferFrom(address(this), _owner, _tokenId);
    }
    ...
    function _mint721(uint16 _tokenId, address _owner) private {
        tmContract.mint(_owner, uint16(_tokenId));
    ...
    function mintOnClaiming(address _sender, uint16 _tokenId) internal returns (bool) {
        ...
        require(!tmContract.exists(_tokenId), "Token already minted");

        tmContract.mint(_sender, _tokenId);
     ...
     */
    function settmContract(address _contract) external onlyOwner {
        require(_contract != address(0), "_contract !address 0");
        tmContract = TestMigration(_contract);
    }
     ...
     */
    function gettmContract() external view returns (address) {
        return address(tmContract);
    }
...
abstract contract TestMigration is Ownable, IERC721 {
