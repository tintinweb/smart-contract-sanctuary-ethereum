...
contract Kuroneko is ERC721Base, MerkleTree, Signed, ReentrancyGuard {
    ...
    */
    ...
    function voteToRefund(uint256[] calldata tokenIds) external {
        require(isRefundGuaranteeActive(), "Refund expired");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(msg.sender == ownerOf(tokenId), "Not token owner");
            require(
                agreeVoters[tokenId] == false,
                "Token already use for voting"
            );
            agreeVoters[tokenId] = true;
            agreeVoterCount += 1;
        }
    }

    function cancelVote(uint256[] calldata tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(msg.sender == ownerOf(tokenId), "Not token owner");
            require(agreeVoters[tokenId] == true, "Token not used for voting");
            agreeVoters[tokenId] = false;
            agreeVoterCount -= 1;
        }
    }

    function checkVoter() public view returns (bool) {
        uint256 total = totalSupply();
        uint256 targetSupply = total.mul(10000).div(100).mul(50);
        if (agreeVoterCount.mul(10000) >= targetSupply) {
            return true;
        }
        return false;
    }
    ...
    function refund(uint256[] calldata tokenIds) external {
        require(isRefundGuaranteeActive(), "Refund expired");
        require(checkVoter(), "Agree Voter not fullfiled");
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            refundAmount += mintedPrices[tokenId];
        }
        if (refundAmount != 0) {
