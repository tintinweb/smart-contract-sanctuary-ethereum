...
contract Moonsie is ERC721AQueryable, Ownable {
  ...
  mapping(address => uint256) public mapAddressToLimit_A;
  mapping(address => uint256) public mapAddressToLimit_B;
  mapping(address => uint256) public mapAddressToLimit_C;
  mapping(address => uint256) public mapAddressToLimit_D;

  constructor() ERC721A('Moonsies', 'Moonsies') {}
  ...
  function flipPhaseOneIsActive() external onlyOwner {
    phaseOneIsActive = !phaseOneIsActive;
  }

  function flipPhaseTwoIsActive() external onlyOwner {
    phaseTwoIsActive = !phaseTwoIsActive;
  }

  function flipPhaseThreeIsActive() external onlyOwner {
  ...
  // Phase One Mint, Group A
  function phaseOneMintA(uint256 mintAmount, bytes32[] calldata merkleProof)
    external
    payable
    nonContract
  {
    ...
    require(MerkleProof.verify(merkleProof, hashRoot_A, leaf), 'Not whitelisted');
  ...
  // Phase One Mint, Group B
  function phaseOneMintB(uint256 mintAmount, bytes32[] calldata merkleProof)
    external
    payable
    nonContract
  {
    ...
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(merkleProof, hashRoot_B, leaf), 'Not whitelisted');
  ...
  // Phase Two Mint, Group C
  function phaseTwoMint(uint256 mintAmount, bytes32[] calldata merkleProof)
    external
    payable
    nonContract
  {
    ...
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(merkleProof, hashRoot_C, leaf), 'Not whitelisted');
  ...
  // Phase Three Mint, Group D
  function phaseThreeMint(uint256 mintAmount) external payable nonContract {
