...
contract AmvNftStaker is Ownable {
    ...
    mapping(uint256 => uint8) public tokenIdExist;

    event NFTStaked(address owner, uint256[] tokenIds, uint256 value);
    event NFTUnstaked(address owner, uint256[] tokenIds, uint256 value);
    ...
    function stake(uint256[] calldata tokenIds) external {
        ...
        require(
            tokenIds.length <= 50,
            "maximum 50 tokens can be staked at a time"
        );
        uint256 tokenId;
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (tokenIdExist[tokenId] == 0) {
                tokenIdExist[tokenId] = 1;
    ...
    function unstake(uint256[] calldata tokenIds) external {
        ...
        require(
            tokenIds.length <= 50,
            "maximum 50 tokens can be unstaked at a time"
        );
        uint256 tokenId;
        totalStaked -= tokenIds.length;
        for (uint256 i = 0; i < tokenIds.length; i++) {
