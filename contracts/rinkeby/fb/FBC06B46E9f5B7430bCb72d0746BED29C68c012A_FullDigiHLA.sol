...
contract FullDigiHLA is Ownable, ERC721A, MerkleWhitelist, ReentrancyGuard {
    ...
    uint256 public ALL_AMOUNT = 40;


    uint256 public WL_START_TIME = 1650542400;
    uint256 public WL_END_TIME = 1650542640;

    uint256 public DA_START_TIME = 1650542820;
    uint256 public DA_END_TIME = 1650543180;

    uint256 public MAIN_END_TIME;
    //uint256 public MAIN_LAST_TIME = 10800;
    uint256 public MAIN_LAST_TIME = 600;

    uint256 public EXTRA_START_TIME = 1650543780;
    ...
    //t1总mint数量 ,t1当前mint数量,state当前第几阶段1-4,t1白名单价格,t1开始时间,t1结束时间,当前用户在t1 mint的数量 ,t1每个用户可以mint的数量
    ...
    //da总mint数量 ,da当前mint数量,state当前第几阶段1-4,da白名单价格,da开始时间,da束时间,当前用户在da mint的数量 ,da每个用户可以mint的数量,da开始价格,da最低价格,da每次降低的价格,da每隔多久降价一次
    ...
    //t2总mint数量 ,t2当前mint数量,state当前第几阶段1-4,t2白名单价格,t2开始时间,t2结束时间,当前用户在t2 mint的数量 ,t2每个用户可以mint的数量
    ...
    //t3总mint数量 ,t3当前mint数量,state当前第几阶段1-4,t3白名单价格,t3开始时间,t3束时间,当前用户在t3 mint的数量 ,t3每个用户可以mint的数量
