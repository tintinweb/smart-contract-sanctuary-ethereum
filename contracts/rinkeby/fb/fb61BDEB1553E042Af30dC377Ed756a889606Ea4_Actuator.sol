...
contract Actuator is SynchronV1{
    ...
    */
    event ModifyPrice(address indexed operator,uint256 singlePrice,uint256 optionId,uint256 indexed modifyTime,uint256 endTime);
    /**
    ...
    */
    ...
    function createOptionWithMint(OptionParam calldata optionParam,uint256 amount,uint96 royal) external returns(uint256 optionId){
        require(amount >= optionParam.amount,"Actuator:The mint quantity must not be less than the sales quantity");
        _mintHelper(optionParam,amount, royal);
        optionId = createOption(optionParam);
    }
    ...
    function cancelOption(uint256 optionId) external{
        ...
        if(option.optionParam.optionType == OptionType.AUCTION_OPTION) require(optionCorrespondingOffer[optionId].length ==0 || option.optionParam.endTime <= block.timestamp);
    ...
    function modifyPrice(uint256 optionId,uint256 single) external{
        ...
        emit ModifyPrice(msg.sender, single, optionId, block.timestamp,option.optionParam.endTime);
    }
    ...
    function bidding(uint256 optionId,uint256 price) external returns(uint256 offerId){
        ...
        //Complete the operation to reach the expected price
        if(option.optionParam.endTime - block.timestamp < 600 && option.state == OptionState.SOLDING)
        {
            option.optionParam.endTime += 600;
            emit ModifyPrice(msg.sender, option.optionParam.singlePrice, optionId, block.timestamp, option.optionParam.endTime);
        } 
        initOfferNumber++;
    ...
    function getOptionCorrespondingOffer(uint256 optionId) external view returns(uint256[] memory){
        return optionCorrespondingOffer[optionId];
    }

    function _mintHelper(OptionParam calldata optionParam,uint256 amount,uint96 royal) internal{
        if (IERC165(optionParam.token).supportsInterface(INTERFACE_ID_ERC721)){
            IERC721(optionParam.token).safeMint(optionParam.creator, optionParam.identifier, royal);
        }
        if (IERC165(optionParam.token).supportsInterface(INTERFACE_ID_ERC1155)){
            IERC1155(optionParam.token).safeMint(optionParam.creator, optionParam.identifier, amount,royal);
        }
    }
}