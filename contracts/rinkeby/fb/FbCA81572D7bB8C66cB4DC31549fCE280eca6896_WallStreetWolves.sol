...
contract WallStreetWolves is Delegated, ERC721Staked, Verify, PaymentSplitterMod {
  using Strings for uint;

  uint public MAX_SUPPLY = 10000;
  uint public PRICE = 0.5 ether;

  uint public CURRENT_ROUND = 1;
  uint public ROUND_SUPPLY = 2500;
  uint public ROUND_MINTED = 0;
  uint public PRESALE_QUANTITY = 2;
  uint public PUBLIC_QUANTITY = 20;

  mapping(address => uint) private foundersQuantities;
  mapping(uint => mapping(address => uint)) private presaleMinters;
  ...
  IWSWFounders public WSWFoundersProxy = IWSWFounders(0x48C8eC816F7789C2d4A517fca3D2fa33ac3Cc1c7);
  ...
  uint[] private splitterShares = [3, 85, 12];

  constructor(
    string memory _name, 
    string memory _symbol,
    address _signer
  ) ERC721B(_name, _symbol, 0) PaymentSplitterMod (splitter, splitterShares) {
    setSigner(_signer);
  }
  ...
  function airdrop ( address _address, uint _quantity ) public onlyDelegates {
    require( totalSupply() + _quantity <= MAX_SUPPLY, "airdrop: Invalid quantity." );
    for (uint i; i < _quantity; ++i) {
      mint1(_address);
    }
  }

  function mint ( uint _quantity, bytes memory _signature ) public payable {
    ...
    if (saleState == SaleState.founders) {
      uint balance = WSWFoundersProxy.balanceOf(msg.sender, 0);
      require( balance <= _quantity + foundersQuantities[msg.sender], "mint: Invalid founders quantity" );
      ...
      require( ROUND_MINTED + _quantity <= ROUND_SUPPLY, "mint: Invalid quantity" );
      require( _quantity + presaleMinters[CURRENT_ROUND][msg.sender] <= PRESALE_QUANTITY, "mint: Invalid quantity." );
  ...
  function resetRound ( uint _roundSupply, uint _price, uint _airdropQuantity ) external onlyDelegates {
    ...
    airdrop(0x2d2352a56827515FAf6088c4cDf59befb4d0A67a, _airdropQuantity);
  }

  function setVariables ( uint _maxSupply, uint _price, SaleState _saleState, uint _presaleQuantity, uint _publicQuantity ) external onlyDelegates {
  ...
  function setRarity(uint[] memory _tokensToSet, uint8 rarity) external onlyDelegates {
    require( totalSupply() > _tokensToSet.length, "setRarity: invalid length" );
    for (uint i; i < _tokensToSet.length; ++i) {
      tokens[_tokensToSet[i]].rarity = rarity;
    }
  }

  function setTraitOfTheMonth(uint[] memory _tokensToSet, bool _isItTraitOfMonth) external onlyDelegates {
    require( totalSupply() > _tokensToSet.length, "setRarity: invalid length" );
    for (uint i; i < _tokensToSet.length; ++i) {
      tokens[_tokensToSet[i]].traitOfTheMonth = _isItTraitOfMonth;
    }
  }

  function setTokenURI ( string memory _tokenURIPrefix, string memory _tokenURISuffix ) public onlyDelegates {
  ...
  function tokenURI(uint _tokenId) external view override returns (string memory) {
    ...
    return string(abi.encodePacked(tokenURIPrefix, _tokenId.toString(), tokenURISuffix));
  }

  function mint1( address _to ) internal {
    uint tokenId = _next();
    tokens.push(Token(_to, 1, 1, false));
...
abstract contract ERC721Staked is Delegated, ERC721Batch {
  ...
  mapping(uint8 => uint) public rarityMultiplier;
  uint public traitOfMonthMultiplier;

  function setRarityMultiplier ( uint8 _rarity, uint _multiplier ) external onlyDelegates {
    rarityMultiplier[_rarity] = _multiplier;
  }

  function setTraitOfMonthMultiplier ( uint _multiplier ) external onlyDelegates {
    traitOfMonthMultiplier = _multiplier;
  }
  ...
  function claimTokens( uint256[] calldata tokenIds, bool restake ) external {
    ...
    for(uint256 i; i < tokenIds.length; ++i ){
      ...
      require( token.stakeDate > 1, string(abi.encodePacked("token ", tokenIds[i].toString(), " is not staked")));
      uint baseQuantity = ( time - token.stakeDate ) * stakeConfig.stakeReward / stakeConfig.stakePeriod;

      if (token.traitOfTheMonth) {
        baseQuantity *= traitOfMonthMultiplier; 
      }
  ...
  function stakeTokens( uint256[] calldata tokenIds ) external {
    for(uint256 i; i < tokenIds.length; ++i ){
      ...
      Token storage token = tokens[ tokenIds[i] ];
      require( token.owner == msg.sender, "caller is not owner" );
      require( token.stakeDate < 2, string(abi.encodePacked("token ", tokenIds[i].toString(), " is already staked")));
      tokens[ tokenIds[ i ] ].stakeDate = uint32(block.timestamp);
    }
  }

  //delegated
  function setStakeConfig( StakeConfig calldata stakeConfig_ ) external onlyDelegates{
