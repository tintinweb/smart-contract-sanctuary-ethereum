...
contract DillonNFTCollection3 is ERC721A, Ownable, ReentrancyGuard {
    ...
    address public _NFT2CollectionAddress;

    // ===== Constructor =====
    
    constructor(string memory name_, string memory symbol_, address devWalletAddress_, address ownerWalletAddress_) 
    ...
    function whitelistMint(bytes32[] memory proof,uint256 amount) external onlySender nonReentrant {
        ...
        require(
            NFTHelper(_NFT1CollectionAddress).balanceOf(msg.sender) > 0 && NFTHelper(_NFT2CollectionAddress).balanceOf(msg.sender) > 0,
