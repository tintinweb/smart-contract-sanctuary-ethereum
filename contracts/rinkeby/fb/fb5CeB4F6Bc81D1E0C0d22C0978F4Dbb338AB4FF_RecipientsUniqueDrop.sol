...
contract RecipientsUniqueDrop is ERC721IPFS, Ownable, ReentrancyGuard {
    ...
    /// @dev tokenIdFirst and tokenIdLast indicates a range for minted ids
    /// with range of [tokenIdFirst, tokenIdLast)
    /// tokenIdFirst (inclusive)
    /// tokenIdLast (exclusive)
    event Dropped(
        uint256 dropId,
        uint256 tokenIdFirst,
        uint256 tokenIdLast,
    ...
    function _mintDrop(address[] memory recipients, uint256[] memory uris)
        private
    {
        uint256 tokenIdFirst = _tokenIdCounter.current();
        uint256 tokenId;
        ...
        _drops[dropId].tokenIdFirst = tokenIdFirst;
        _drops[dropId].tokenIdLast = tokenId;
    ...
    function count() external view virtual returns (uint256) {
...
/// @author Amit Molek
contract ERC721IPFS is ERC721, IPFSEfficientUri {
