...
contract Web3InTravelNFTTicket is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function claimByPatrons(bool _airdrop) external payable nonReentrant {
        ...
        price = INITIAL_PRICE + (sumIncrement / (10**18))*10**18;
        _safeMint(_msgSender(), tokenId);
    }

    function sponsorship(string memory _quote) external payable nonReentrant {
        require(!paused, ERR_MINTING_PAUSED);        
        require(bytes(_quote).length < 30, ERR_TOO_MANY_CHARS);
    ...
    function tokenURI(uint256 _tokenId) override public view returns (string memory) {
        ...
        parts[3] = string(abi.encodePacked('<text x="10" y="190" class="b">$',toString(prices[_tokenId] / (10**18)),' xDAI</text><text x="10" y="210" class="b">0x',toAsciiString(mintedBy[_tokenId]),'</text><text x="10" y="230" class="b">',_details_type,'</text>'));
