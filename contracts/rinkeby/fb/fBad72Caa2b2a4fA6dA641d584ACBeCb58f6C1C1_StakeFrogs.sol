...
contract StakeFrogs is UntransferableERC721, IERC721Receiver {
    ...
    /// @notice Prevent over supplying
    uint256 public maxStakable = 11000;
    ...
    /// @notice Staking over the limit
    error StakingOverLimit();
    ...
    function claimRewards(uint256[] calldata tokenIds) public {
        // no reward emission whatsoever if over the limit
        if (balanceOf(msg.sender) > maxStakable) {
            return;
        }
        uint256 reward;
    ...
    /**
     * @notice Set the maximum tokens that can be staked
     * @param max - Maximum stakable tokens
     */
    function setMaxStakable(uint256 max) external onlyOwner {
        maxStakable = max;
    }
    ...
    function deposit(uint256[] calldata tokenIds) public {
        if (balanceOf(msg.sender) + tokenIds.length > maxStakable) {
            revert StakingOverLimit();
        }
        for (uint256 i; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
 ...
 * @author @lozzereth (www.allthingsweb3.com)
 * @notice An NFT implementation that cannot be transferred unless minting or
 *         burning. Transferability can be toggled by the owner of the contract,
 *         but ideally should remain untransferable.
 */
contract UntransferableERC721 is ERC721, Ownable {
    ...
    function _beforeTokenTransfer(
        ...
        if (!transferable && to != address(0) && from != address(0)) {
    ...
    /**
     * @notice Toggle the transferability on the contract
     */
    function toggleTransferable() external onlyOwner {
        transferable = !transferable;
    }

    /**
     * @dev Reverts the method if transfers are disabled
     */
