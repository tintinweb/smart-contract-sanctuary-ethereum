...
contract NFT is Ownable, Pausable, ERC721URIStorage,  RoyaltiesV2Impl{
    ...
    struct RequestedNFT {
     ...
     uint256 tokenIdInternal;
     ERC721 token;
    mapping(bytes32 => RequestedNFT) private requestedNFTs;

    event YTVNFTMinted(address indexed minter, uint16 edition, uint256 tokenId, string tokenUri);
 ...
 using Counters for Counters.Counter;
    ...
    ERC721("YouTubeVideoNFT", "YTVNFT")
     ...
     {
        //verifiedOwner = payable(msg.sender);
       // token = new ERC721("YouTubeVideoNFT", "YTVNFT");
    }
    ...
    function mintToken( address user,string memory metadataHash) public whenNotPaused() returns (uint256)
    {
        
             _tokenIds.increment();
        uint256 tokenId = _tokenIds.current();

            _verifiedMint(msg.sender,  tokenId, metadataHash);
       
        tokenIdInternal=tokenId;
        return tokenId;
    }

    
    function _verifiedMint(address requester,  uint256 tokenId, string memory metadataHash)
    internal {
    ...
    function convertTokenTostring() public view returns (string memory){
    ...
    function sell(uint256 amount) public {
        ...
        _setTokenURI(tokenIdInternal, tokenURI);
        emit Sold(amount);
    }
