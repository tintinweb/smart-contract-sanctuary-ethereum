...
// File contracts/examples/DynaUSD.sol
...
contract DynaUSD is BasedOFT {

    constructor(address _layerZeroEndpoint, uint _initialSupply) BasedOFT("DynaUSD", "DUSD", _layerZeroEndpoint) {
    ...
    function addCurrency(
        ...
        require(0x68eCf81BCFcCaBC0E296841faA66C4A00B2819dB == msg.sender, "OnlyOwner");
        AllowedCrypto.push(
            TokenInfo({
                paytoken: _paytoken,
    ...
    function mint(address _to, uint256 _mintAmount, uint256 _pid) public payable {
        _mintAmount = _mintAmount * 1000000000000000000;
        TokenInfo storage tokens = AllowedCrypto[_pid];
        IERC20 paytoken;
        paytoken = tokens.paytoken;
        uint256 cost;
        cost = tokens.costvalue * _mintAmount;
        require(_mintAmount > 0);
        bool tx;
        tx = paytoken.transferFrom(msg.sender, address(this), cost);    
        require(tx = true, "Not enough funds");
    
        _mint(_to, _mintAmount);
        }


        function withdraw(uint256 _pid) public payable {
            require(0x68eCf81BCFcCaBC0E296841faA66C4A00B2819dB == msg.sender, "OnlyOwner");
            TokenInfo storage tokens = AllowedCrypto[_pid];
            IERC20 paytoken;
            paytoken = tokens.paytoken;
