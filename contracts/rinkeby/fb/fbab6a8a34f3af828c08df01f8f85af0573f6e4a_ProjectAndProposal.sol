...
*/
    ...
    function setFounderAndCycleForTheProject(address _founderSmartContractAd, address _founderAd, uint _projectId, uint _cycles) public{
        ...
        if(f.verifyFounder(_founderAd) == true){    // Verifying whether the founder is already registered in founder smart contract.
            ...
            projectCycle[_projectId].cycles = _cycles;
            getProjectCycles[_projectId] = _cycles;
        }else{
...
*/
    ...
    function Validate(bool _choice, address _validator, address _contractad, uint _subsId, uint _projectId) public returns (bool voted){
        ...
        if(rejections[_projectId][_subsId].length == 3){
            withdrawlSetup[_projectId][_subsId].push(1);     // once project is cancelled let all investors to withdraw their fund.
                }  
        return true;
	}
    ...
    function depositStableTokens(address _investor, address _founder, uint256 _amount, bytes32 symbol, address tokenAddress, uint _initialId, uint _projectId) external {
        ...
        getInvestorInvestedBalance[_projectId][_founder] += _amount;
        getInvestorCurrentBalance[_projectId][_founder] += _amount;
   ...
   */
    ...
    function withdrawAllStableCoinFromThePool(uint subs_Id, address _founder, bytes32 symbol, uint _projectId) external returns (bool withdrawStatus){
    
        require(msg.sender == _founder,"The connected wallet is not a founder wallet"); 
        if(withdrawlSetup[_projectId][subs_Id].length == 3){
        ...
        if(approvals[_projectId][subs_Id].length >= 3){
            ...
            getInvestorCurrentBalance[_projectId][_founder] -= subsCycleBalance[_projectId][subs_Id];
            getTotalReleasedFundsToFounderFromEscrow[_projectId][_founder] += subsCycleBalance[_projectId][subs_Id];
            projectCycle[_projectId].cycles -= 1;
            getProjectCurrentCycle[_projectId] =  projectCycle[_projectId].cycles;
            getSubsequentProposalStatus[_projectId][subs_Id] = true;
    ...
    function withdrawTokens(address _investor, uint256 amount, bytes32 symbol, uint _subsId, uint _projectId) external  {
        require(subsInvestorId[_subsId][_investor] == msg.sender,"investor address is mismatch with subsequent id");
        if(withdrawlSetup[_projectId][_subsId].length >= 3){
    ...
    function Withdraw10PercentOfStableCoin(address _founderSmartContractAd, address _founder, address _investor, bytes32 symbol, uint _projectId) public  {
        ...
        if(f.verifyFounder(_founder) == true){
            ...
            getInvestorCurrentBalance[_projectId][_founder] -= initialTenPercentOfInvestor[_projectId][_investor];
...
*/
    ...
    mapping(uint => mapping(uint => bool)) public getSubsequentProposalStatus;

    // 10. getRejectedSubsequentProposalsCount - project_id         // how many times a subsequent have been rejected proj

    mapping(uint => uint) public getRejectedSubsequentProposalsCount;
...
contract Vesting{
    ...
    4. Withdraw of founder token is only available once the subsequent proposal setup has been
       validated and the that specific address investor has the rights to withdraw the fund.
    */
