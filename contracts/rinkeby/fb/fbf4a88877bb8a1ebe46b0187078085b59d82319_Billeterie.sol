...
contract Billeterie is Initializable, ERC1155MintBurn, TicketrustMiddleware, PaymentHandler, OwnableUpgradeable {
    // Global variables
    ...
    function uri(uint256 _id) public view returns (string memory) {
        ...
        string memory tokenUri = eventOffchainData[creatorOfEvent[_id]][_id];
...
contract Initializable {
  ...
  modifier initializer() {
    require(initializing || isConstructor() || initialized < 1, "Contract instance has already been initialized");
  ...
  function isConstructor() private view returns (bool) {
    ...
    modifier reinitializer(uint8 version) {
        require(!initializing && initialized < version, "Initializable: contract is already initialized");
        initialized = version;
        initializing = true;
        _;
        initializing = false;
