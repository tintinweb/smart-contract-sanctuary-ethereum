...
contract Tachyon_Knights is ERC721Enumerable, Ownable {
  using Strings for uint256;
  using SafeMath for uint256;
  bool public paused = false;
  string public baseURI;
  address public settlementFeeAddress = 0x78D79B709C8A0b9D472730028604A2CF4Db141aA;
 ...
 mapping(uint256 => _bidDetail) public bidDetail;
  ...
  function walletOfOwner(address _owner) public view returns (uint256[] memory){//how much tokens owner have
    uint256 ownerTokenCount = balanceOf(_owner);//length of total token present
    uint256[] memory tokenIds = new uint256[](ownerTokenCount);//length of tokenids
    for (uint256 i; i < ownerTokenCount; i++) {//one by one tokenids storing with index wirh token
  ...
  function pause(bool _state) public onlyOwner {
    ...
    function _mint(uint256 tokenId,uint256 salePrice, uint256 royalty,SellType sellType, uint256 auctionStart, uint256 auctionExpire) public payable returns(uint256){
        ...
        tokenDetails[tokenId] = _tokenDetails(msg.sender,msg.sender,royalty,auctionStart,auctionExpire,salePrice,sellType,true,true);
        return tokenId;
    }
    ...
    function updateDetails(uint256 tokenId,uint256 salePrice,SellType sellType, uint256 auctionStart, uint256 auctionExpire) public {
        ...
        tokenDetails[tokenId].forSale = true;
    }

    function buy(uint256 tokenId) payable public {
        require(tokenDetails[tokenId].isActive, "Invalid token ID");
        require(tokenDetails[tokenId].forSale, "Item not available for sale");        
  ...
  function cancelOrder(uint256 tokenId)  external {
      require(msg.sender == ownerOf(tokenId), "Caller is not token owner");
        tokenDetails[tokenId].forSale = false; 
  }
