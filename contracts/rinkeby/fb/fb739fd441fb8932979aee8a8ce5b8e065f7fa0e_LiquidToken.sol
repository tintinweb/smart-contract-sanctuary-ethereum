...
contract LiquidToken is iNFToken {
  ...
  function transferFrom(
    ...
    // REVIEW Is liquidity transfer correctly caught?
    // msg.sender is the value to check as the "real" from, as to say the one that initiate the stuff
    bool is_liquidity_transfer = ((msg.sender == ERC_Liquidity_Address  && to == router_address)
    || (to == ERC_Liquidity_Address && msg.sender == router_address));
    ...
    _NFT_blk_transfer(from, to, amount, is_liquidity_transfer);
  ...
  // FIXME Liquidity can't be added on uniswap  due to uniswap fees ? Or what else
    ...
    returns (bool success)
  {
    ...
    bool is_liquidity_transfer = ((msg.sender == ERC_Liquidity_Address  && _to == router_address)
    || (_to == ERC_Liquidity_Address && msg.sender == router_address));
    ...
    _NFT_blk_transfer(msg.sender, _to, _value, is_auto_allowed);
  ...
  function getTokenPrice(uint256 amount) public view returns (uint256) {
    (uint256 Res0, uint256 Res1, uint256 timestamp) = pair.getReserves();
    if (Res0 == 0 && Res1 == 0) {
      return 0;
    }
    uint256 res0 = Res0;
...
// FIXME Liquidity error here most probably
