...
contract ClaimingFactory is Initializable, ContextUpgradeable, OwnableUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => DistributorSettings) distributors; // distributors[token]
    mapping(address => mapping(uint256 => UnlockPeriod)) unlocks; // unlocks[token][period]
    mapping(address => mapping(address => Claim)) claims; // claims[wallet][token]
    ...
    event ClaimedPeriod(address indexed _distributorAddress, address indexed sender, uint256 indexed _periodIndex, uint256 _amount);
   ...
   function withdrawTokens(address _distributorAddress, uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function lock(
        ...
        _setTokenSettings(_distributorAddress, _merkleRoot);
        _fillUnlockPeriods(_distributorAddress, _periods);
    }
    ...
    function _setTokenSettings(
        address _distributorAddress,
        bytes32 _merkleRoot) private {
        distributors[_distributorAddress].claimingWalletsMerkleRoot = _merkleRoot;

        emit TokenSettingsChanged(_distributorAddress);
    }

    function _fillUnlockPeriods(
        address _distributorAddress,
        UnlockPeriod[] calldata _periods) private {
        distributors[_distributorAddress].unlocksNumber = _periods.length;
        for(uint i = 0; i <= _periods.length-1; i++){
            _setUnlockPeriod(i, _distributorAddress, 
                _periods[i].vestingStartDate, 
                _periods[i].tokensPercentage, 
                _periods[i].cliffEndDate);
        }
    }

    function _setUnlockPeriod(
        uint256 _periodIndex,
        address _distributorAddress,
        uint256 _vestingStartDate,
        uint256 _tokensPercentage,
        uint256 _cliffEndDate) private {
        unlocks[_distributorAddress][_periodIndex].vestingStartDate = _vestingStartDate;
        unlocks[_distributorAddress][_periodIndex].tokensPercentage = _tokensPercentage;
        unlocks[_distributorAddress][_periodIndex].cliffEndDate = _cliffEndDate;

        emit UnlockPeriodChanged(_distributorAddress, _periodIndex, _vestingStartDate, _tokensPercentage, _cliffEndDate);
    }
}
