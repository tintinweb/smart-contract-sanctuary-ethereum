 ...
 */
contract ChunkNFT is ERC721, Ownable, ReentrancyGuard, VRFConsumerBase {
    using SafeMath for uint256;
    using Strings for uint256;
    using ECDSA for bytes32;

    string public PROVENANCE = "";
    string public ipfsGateway = "https://ipfs.chunknft.com/ipfs/";
    string public metadataIpfsCid = "";
    string public metadataIpfsCidAnimation = "";
    string public placeholderCid = "";
    bool public isMintEnabled = false;
    bool public isRevealed = false;
    bool public isWhiteListSaleEnded = false;
    bool public isAnimationURL = false;
    uint256 public totalMinted = 0;
    uint256 public mintSupplyCount = 0;
    uint256 public startingIndex = 0;
    uint256 public currentIndex = 0; // TODO: set private
    //uint256 public whitelistAddressCount = 0;
    //uint256 public whitelistMintCount = 0;
    uint256 public maxMintPerAddress;
    uint256 public ownerMintReserveCount;
    uint256 private ownerMintCount = 0;
    address private immutable adminSigner;
    uint256 public constant PRICE_PER_TOKEN = 0.015 ether;
    ...
    //event MintValues(uint256 mintCost, uint256 mintSent);
    //event NextEvent(uint256 totalMinted, uint256 mintSupplyCount);
    event TokenMinted(address to, uint256 tokenId, uint256 total);

    mapping(uint256 => string) public tokenIdMetadataIpfsHashes;
    mapping(string => uint256) private ipfsHashTokenIds;
    mapping(address => uint16) private addressMintCount; // mints per address dict
    //mapping(address => bool) private whitelist; // addresses WL state dict

    // Link RNG
    bytes32 public keyhash; // uniquely identify a Chainlink node
    uint256 public fee; // oracle fee

    constructor(
        string memory _metadataIpfsCid,
        string memory _metadataIpfsCidAnimation,
        ...
        ERC721("ChunkNFT", "CHUNKNFT")
    {
        metadataIpfsCid = _metadataIpfsCid;
        metadataIpfsCidAnimation = _metadataIpfsCidAnimation;
    ...
    function setMetadataIpfsCid(string memory _metadataIpfsCid)
        external
        onlyOwner
    {
        require(
            isRevealed == false,
            "The metadata IPFS can not updated after the reveal"
        );
        metadataIpfsCid = _metadataIpfsCid;
    }

    function setMetadataIpfsCidAnimation(
        string memory _metadataIpfsCidAnimation
    ) external onlyOwner {
        require(
            isRevealed == false,
    ...
    function setPlaceholderCid(string memory _placeholderCid)
        external
        onlyOwner
        noItemMinted
    {
    ...
    function updateMetadataIpfsHash(
        ...
        bytes32 digest = hashTransaction(
            msg.sender,
        ...
        tokenIdMetadataIpfsHashes[_tokenId] = _tokenMetadataIpfsHash;
        ipfsHashTokenIds[_tokenMetadataIpfsHash] = _tokenId;
    }

    function enableAnimationURL() external onlyOwner {
        isAnimationURL = true;
    }
    ...
    {
        if (isRevealed) {
            string memory metadataIpfsCidFinal = (
                isAnimationURL ? metadataIpfsCidAnimation : metadataIpfsCid
            );
            return
                bytes(tokenIdMetadataIpfsHashes[_tokenId]).length == 0
                    ...
                    ); // tokenUri set by owner
        } else {
            return string(abi.encodePacked(ipfsGateway, placeholderCid));
        }
    }

    function setDefaultIpfs() external onlyOwner {
    ...
    // function joinWhitelist() external noItemMinted {
    ...
    //         addWhiteList(accounts[account]);
    //     }
    // }

    // function addWhiteList(address account) private {
    ...
    //     return whitelist[account] == true;
    // }

    function setWhiteListSale(bool _isEnded) external onlyOwner {
        isWhiteListSaleEnded = _isEnded;
    }
    ...
    function getNextIndex() private {
        // TEST
        //totalMinted++;
        //emit NextEvent(totalMinted, mintSupplyCount);

        if (totalMinted == mintSupplyCount) {
            currentIndex = 0; // We are done minting
            return;
        }
        ...
        if (currentIndex < startingIndex) {
            ...
            if (currentIndex < mintSupplyCount) {
                currentIndex++;
            } else {
                if (startingIndex == 1) {
                    currentIndex = 0; // We are done minting
    ...
    function mint(uint256 _numberOfTokens, bytes memory signature)
        external
        payable
        nonReentrant
    {
        ...
        require(
            _totalMinted + _numberOfTokens < mintSupplyCount + 1,
            "Can not mint that amount of tokens"
        );

        require(msg.sender == tx.origin, "Smart contracts can not mint");

        if (!isWhiteListSaleEnded) {
            // require(hashTransaction(msg.sender) == hash, "Hash failed");

            bytes32 digest = hashTransaction(msg.sender);
            require(
                matchSigner(digest, signature),
                "Account not whitelisted during whitelist sale"
            );
        }

        uint256 cost = PRICE_PER_TOKEN * _numberOfTokens;

        //emit MintValues(cost, msg.value);
        ...
        if (_msgSender() != owner()) {
            ...
            // owner
            require(
                ownerMintCount + _numberOfTokens < ownerMintReserveCount + 1,
        ...
        for (uint256 i = 0; i < _numberOfTokens; i++) {
            // Safety check
            require(currentIndex > 0, "All tokens minted");

            // increate addr mint count & total mint count
            ...
            _mint(_msgSender(), currentIndex); // We don't allow smart contracts to mint
            emit TokenMinted(_msgSender(), currentIndex, _totalMinted);
    ...
    function hashTransaction(address _sender) public view returns (bytes32) {
    ...
    function hashTransaction(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(_sender, _metadataIpfsCid, _tokenId)
        );
        return hash;
    }

    function isVerifiedCoupon(
        bytes32 _digest,
        uint8 _v,
        bytes32 _r,
        bytes32 _s
    ...
    function requestRNG() public onlyOwner noItemMinted {
        ...
        override
    {
        ...
        startingIndex = _randomNumber % mintSupplyCount;
    ...
    modifier noItemMinted() {
        require(totalMinted == 0, "Minting started");
        _;
    }

    modifier mintNotFinished() {
        require(totalMinted < mintSupplyCount, "Mint not finished");
        _;
    }
    ...
    function setTestingDefaultValues(uint256 _startingIndex)
        public
        onlyOwner
        noItemMinted
    {
