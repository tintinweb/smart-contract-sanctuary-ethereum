...
contract UncleNFT is NFTEnumerable, ERC1155Holder {
  ...
  /** User */
  // verified
  function presaleMint(bytes calldata _signature, uint16 _amount) external payable {
    ...
    require(eligibleByWhitelist(msg.sender, _signature, _amount), "Not eligible"); // eligible to claim
  ...
  // verified
  function eligibleByWhitelist(address _account, bytes memory _signature, uint16 _amount) internal view returns (bool) {
    bytes32 message = keccak256(abi.encodePacked(hashKey, _account));
    return validSignature(message, _signature) && whitelistClaimed[_account] + _amount <= MAX_PER_WALLET;
  }
...
abstract contract NFTEnumerable is BaseControl, IERC721Enumerable, ERC721Royalty, ERC721Burnable {
  ...
  constructor() ERC721("UncleNFT", "UncleNFT") {
