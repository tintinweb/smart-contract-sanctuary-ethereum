...
contract SrcCore is Ownable {
    ...
    mapping(uint256 => address) private multichainDstContracts;
    ...
    constructor(address _safeboxContract, uint256 _srcChainId) {
    ...
    function getMultiChainDstContract(uint256 chainId) public view returns(address) {
        return multichainDstContracts[chainId];
    }

    function createPropto(uint256 dstChainId, address token, uint256 tokenID, uint256 initialPrice, uint8 ProptoType) public {
    ...
    function getPropto(uint256 index) public view returns(uint256, address, uint256, address, uint256, uint8) {
