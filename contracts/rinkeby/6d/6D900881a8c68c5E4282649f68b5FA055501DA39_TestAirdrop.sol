...
contract TestAirdrop is ERC721A, Ownable {
    ...
    uint16 public constant totalNFTs = 20;
    uint16 private constant basicNFTs = 15;
    uint16 private constant reservedNFTs = 5;
    ...
    function mint(address[] memory to) public onlyOwner {
        // Need to sum totalSupply() and the length of the array of addresses passed
        require(totalSupply() <= basicNFTs, "Mint limit reached");
    ...
    function reserveNFT(address[] memory to) public onlyOwner {
        // Need to sum reservedNFTsAmount and the length of the array of addresses passed
        require(reservedNFTsAmount <= reservedNFTs, "Out of stock");

        for (uint i = 0; i < to.length; i++) {
            _mint(to[i], 1);
            reservedNFTsAmount += 1;
        }
    }
