...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 public _communityFee=200;// meanse 2% community Wallet
    uint256 public _burnFee = 200;// meanse 2% burn fee
    uint256 public _liquidityFee = 200; // means 2% add liquidity on each buy and sell
    
    address comminityAddress = 0x23872e3Ed07361bE065D678D06d05CccaD8B5454;//dummy addresss
    ...
    uint256 public maxTxAmount = 5_000e9; // 0.5% limit
    uint256 public _maxWalletToken = 20_000e9; // 2%
    ...
    constructor()  {
        ...
        //Exempt maxWalletAmount from Owner ,Contract
    ...
    function _transfer(
        ...
        if(sender == uniswapV2Pair){
            ...
            // if(!_maxWallet[recipient] && balanceOf(recipient) >= _maxWalletToken){
            //     return;
            // }
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_liquidityFee != 0){
         ...
         if(_communityFee != 0){
            uint256 buyBackFee = amount.mul(_communityFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(buyBackFee);
            _reflectionBalance[comminityAddress] = _reflectionBalance[comminityAddress].add(buyBackFee.mul(rate));
            if (_isExcluded[comminityAddress]) {
                _tokenBalance[comminityAddress] = _tokenBalance[comminityAddress].add(buyBackFee);
            }
            emit Transfer(account,comminityAddress,buyBackFee);
        }
        if(_burnFee != 0){
            uint256 buyBackFee = amount.mul(_burnFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(buyBackFee);
            _reflectionBalance[DEAD] = _reflectionBalance[DEAD].add(buyBackFee.mul(rate));
            if (_isExcluded[DEAD]) {
                _tokenBalance[DEAD] = _tokenBalance[DEAD].add(buyBackFee);
            }
            emit Transfer(account,DEAD,buyBackFee);
        }
    ...
    function setCommunityAddress(address add) external onlyOwner{
        comminityAddress=add;
    }
