...
contract HYFI_Sale is Initializable, AccessControlUpgradeable {
    ...
    modifier canClaim(address user) {
        (uint256 presaleTotalAmountBought, , ) = presale.getBuyerData(user);
        require(
            _availableForClaim(user) > 0,
            "You have not bought any units in order to claim"
        );
        _;
    }
    ...
    function initialize(
        ...
        address _presaleContarctAddress,
        ...
        presale = IHYFI_Presale(_presaleContarctAddress);
        ...
        possiblePurchaseUntilHardcap(amount)
    {
        ...
        uint256 discount = discountPercentageCalculator(amount, msg.sender);
        if (buyWithHYFI) {
        ...
        possiblePurchaseUntilHardcap(amount)
    {
        _buyWithCurrency(
            amount,
            discountPercentageCalculator(amount, msg.sender),
            referralCode
        );
    ...
    {
        //TODO: new quantity discount logic will go here
        // moving it here from the PriceCalculator due to
        // possibility of different quantity dicscounts and whitelist features
    }
    ...
    function setNewSaleTime(uint256 newStartTime)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function _claim(address user, uint256 amount) internal {
        claimed[user].add(amount);
        _mintVaults(user, amount);
        emit VaultsClaimed(user, amount);
    }

    function _mintVaults(address user, uint256 amount) internal virtual {
        vault.safeMint(user, amount);
        emit VaultsMinted(user, amount);
    }

    function _availableForClaim(address user) internal view returns (uint256) {
        return claimed[user].sub(_totalReservedAmount(user));
    }
    ...
    {
        (uint256 presaleTotalAmountBought, , ) = presale.getBuyerData(user);
        // todo:add offline logic here for total reserved amount calculation
        return presaleTotalAmountBought;
    }
}
