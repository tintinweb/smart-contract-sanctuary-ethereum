...
contract TastyBones is Ownable, ERC721A {
    ...
    mapping (uint256 => uint256) public mintedTBforFreeMint;
    mapping (address => uint256) public mintedTBforPresale;
    mapping(address => uint256) addressBlockBought;

    bytes32 private freeMintMerkleRoot = 0x04dafba28d78cea1c319b6659d3f4f7f790c247a3571bda4a853d8f74f0f70f4;
    ...
    function mintFree(uint256 boneTokenId, uint256 numberOfTokens, bytes32[] calldata proof) external payable {
        ...
        if(MerkleProof.verify(proof, freeMintMerkleRoot, keccak256(abi.encodePacked(msg.sender)))) {
            canMint += 1;
        }
        ...
        require(bonesOfOwner,"USER_DO_NOT_OWN_THE_BONE");
        require(boneCount > 0, "NO_BONE_PASS");
        require(mintedTBforFreeMint[boneTokenId] <= canMint, "BONE_USED_FOR_MINTING");
        require(numberOfTokens <= canMint,"MAX_MINT_REACHED");
        require(tokenSupply.current().add(numberOfTokens) <= maxFreeMintSupply, "EXCEEDS_MAX_SUPPLY" );

        addressBlockBought[msg.sender] = block.timestamp;
        _safeMint( msg.sender, numberOfTokens);

        mintedTBforFreeMint[boneTokenId] = mintedTBforFreeMint[boneTokenId].add(numberOfTokens);
    }
     ...
     */
    function checkACBoneBalanceForFreeMint(uint256 boneId) public view returns(uint256) {
        return mintedTBforFreeMint[boneId];
    }
    ...
    function setFreeMerkleRoot(bytes32 freeMintRoot) external onlyOwner {
