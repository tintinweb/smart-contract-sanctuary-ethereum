...
contract CoinFlip is VRFConsumerBaseV2 {
    ...
    event Random(uint256 randomValue);
    event Result(uint256 id, uint256 bet, uint256 randomSeed, uint256 amount, address player, uint256 winAmount,uint256 time);
    event WinResult(uint256 WinId,uint256 WinAmount,address Winplayer);
    ...
    function verdict(uint256 random) public onlyVFRC {
        ...
        if((random>=half && games[i].bet==1) || (random<half && games[i].bet==0)){
           ...
           emit WinResult(games[i].id,games[i].amount,games[i].player);
        //    return lastWinAmount;
        }
        emit Result(games[i].id, games[i].bet, games[i].seed, games[i].amount, games[i].player, winAmount,block.timestamp);
        lastGameId = gameId;
    ...
    function payPlayer(uint256 _id,uint256 money,address payable _player) external {

    for(uint256 i=lastGameId; i<gameId; i++){
        
        require(games[i].id==_id && games[i].amount==money && games[i].player==_player);
        //check bets from latest betting round, one by one
        _player.transfer(money*2);
    }
        lastGameId = gameId;
