...
contract JurasickPets is ERC721X, Ownable, ReentrancyGuard {
    uint256 constant collectionSize = 10000;
    ...
    constructor() ERC721X("JurasickPets", "DINO") {
    ...
    function freeMint(uint64 quantity) external onlyOwner {
        require(quantity <= 2, "Cannot mint this many at a time");
    ...
    function wlMint(
        ...
        require(
         nextId - 1 + quantity <= collectionSize,
            "Reached max supply"
        );
    ...
    function _lfMint(uint256 quantity) private {
        for (uint256 i = nextId - 1; i < nextId - 1 + quantity; i++) {
            updateBirthDate(i);
