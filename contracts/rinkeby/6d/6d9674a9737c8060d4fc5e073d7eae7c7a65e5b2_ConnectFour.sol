...
contract ConnectFour {
    ...
    function initializeGame() external payable returns (uint256) {
        require(msg.value <= maxBetAmount, "Bet is too big");
        require(msg.value >= minBetAmount, "Bet is too small");

        Disc[42] memory board;
        games[gameIdCounter].player1 = msg.sender;
        games[gameIdCounter].board = board;
        games[gameIdCounter].betAmount = msg.value;
        games[gameIdCounter].status = Status.Initialized;
        games[gameIdCounter].isPlayer1Turn = true;

        emit GameInitialized(gameIdCounter, msg.sender, msg.value);
        uint256 temp = gameIdCounter;
        gameIdCounter++;
    ...
    function startGame(uint256 _gameId) external payable {
        require(msg.value == games[_gameId].betAmount, "Must match bet amount");
        require(
            Status.Initialized == games[_gameId].status,
    ...
    function playMove(uint256 _gameId, uint256 _col) external {
        ...
        require(Status.Started == game.status, "Game in wrong state");
        require(
            game.isPlayer1Turn
        ...
        require(_col < 0, "Out of bounds");
        require(_col > 6, "Out of bounds");
        ...
        for (uint256 row = 0; row <= 5; row++) {
            ...
            if (rowDisc == Disc.Empty) {
                games[_gameId].board[idx] = game.isPlayer1Turn
                    ? Disc.Player1
                    : Disc.Player2;
                games[_gameId].isPlayer1Turn = !game.isPlayer1Turn;
                break;
            } else if (row == 5) {
    ...
    function claimReward(
        ...
        Disc disc = games[_gameId].player1 == msg.sender
            ? Disc.Player1
            : Disc.Player2;
        ...
        for (uint256 i = 0; i < 4; i++) {
            uint256 idx = boardIndex(currCol, currRow);
            require(games[_gameId].board[idx] == disc, "Not the winner");
        ...
        require(sent == true, "Failed to send reward");
    ...
    function boardIndex(uint256 _col, uint256 _row)
        ...
        returns (uint256)
    {
        require(_col <= 6 && _row <= 5, "Out of bounds");
