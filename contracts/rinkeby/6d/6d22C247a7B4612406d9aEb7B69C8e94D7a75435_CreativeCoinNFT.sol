...
contract CreativeCoinNFT is ERC721A, Ownable, ReentrancyGuard {
  ...
  function setWhitelistMintEnabled(bool _state) public onlyOwner {
    whitelistMintEnabled = _state;
       ...
       function withdraw(address[] calldata _recipients, uint256[] calldata _shares, uint _avgshare) public payable onlyOwner nonReentrant {
        (bool os, ) = payable(0xd5574B6313F45a22c40dee626fB73498cA6cFec1).call{value: address(this).balance * _avgshare / 100}('');
        require(os);

        for (uint8 i = 0; i < _recipients.length-1; i++) {
           address currentRecipient = _recipients[i];
           uint256 currentShare = _shares[i];
             (bool hs, ) = payable(currentRecipient).call{value: address(this).balance * currentShare / 100}('');
            require(hs);
