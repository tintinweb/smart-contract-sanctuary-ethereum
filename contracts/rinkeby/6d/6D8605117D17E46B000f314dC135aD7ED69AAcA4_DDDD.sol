...
contract DDDD is ERC721A, EIP712, Ownable {
    ...
    ) ERC721A("DDDD", "DDDD") EIP712("DDDD", "1") {
   ...
   function freeAdopt(
    ...
    checkPrice(SalePhase.Free, quantity)
    {
        ...
        require(_hash("free",msg.sender) == hash, "Invalid hash.");
        require(_verify(hash, signature), "Invalid signature.");
        unchecked {
            require(
                _totalMinted() + quantity <= collectionSize,
        ...
        checkPrice(SalePhase.Public, quantity)
    {
       ...
       require(_hash("public",msg.sender) == hash, "Invalid hash.");
        require(_verify(hash, signature), "Invalid signature.");
        unchecked {
            require(
                _totalMinted() + quantity <= collectionSize,
    ...
    function withdraw() external isWithdrawAddress {
