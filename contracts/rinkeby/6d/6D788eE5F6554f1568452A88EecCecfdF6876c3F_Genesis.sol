...
contract Genesis is MultisigOwnable, VRFConsumerBase, ERC721A, PaymentSplitter, BatchReveal {
    ...
    uint private constant MAX_FREE = 50; // Addition of each wallet allowance (free + bronze)
    uint private constant MAX_WHITELIST = 2000;
    uint private constant MAX_SECOND_WHITELIST = 2500;
    uint private constant MAX_ALLOWLIST = 2600;

    uint private constant MAX_MINT_PER_WHITELIST_ACCOUNT = 2;
    uint private constant MAX_MINT_PER_SECOND_WHITELIST_ACCOUNT = 1;

    uint public salePrice = 0.0015 ether;
    uint public maxSupply = 3333;

    bytes32 public whitelistRoot;
    bytes32 public secondWhitelistRoot;
    bytes32 public allowlistRoot;
    bytes32 public teamRoot;
    bytes32 public freeRoot;
    bytes32 public freeBronzeRoot;
    bytes32 public freeSilverRoot;
    bytes32 public freeGoldRoot;

    uint public amountNFTonTeamMint;

    mapping(address => uint) public amountNFTsperWalletWhitelistSale;
    mapping(address => uint) public amountNFTsperWalletSecondWhitelistSale;
    ...
    PaymentSplitter(_team, _teamShares) {
        ...
        secondWhitelistRoot = _secondWhitelistRoot;
    ...
    function whitelistMint(uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        require(sellingStep > Step.Before, "Whitelist sale is not activated");
        require(sellingStep < Step.SoldOut, "Sale is done");
        require(_quantity < 2, "Only 1 NFT can be minted at once");
        require(isAuthorized(msg.sender, _proof, whitelistRoot), "Not whitelisted");
        require(amountNFTsperWalletWhitelistSale[msg.sender] + _quantity < MAX_MINT_PER_WHITELIST_ACCOUNT + 1, "You can only get 2 NFTs on the Whitelist Sale");
        ...
        require(msg.value == salePrice * _quantity, "Wrong payment");
        amountNFTsperWalletWhitelistSale[msg.sender] += _quantity;
        _mint(msg.sender, _quantity, '', false);
        require(totalSupply() <= MAX_WHITELIST, "Limit reached");
    }

    function secondWhitelistMint(uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        require(sellingStep > Step.WhitelistSale, "Second whitelist sale is not activated");
        require(sellingStep < Step.SoldOut, "Sale is done");
        require(_quantity < 2, "Only 1 NFT can be minted at once");
        require(isAuthorized(msg.sender, _proof, secondWhitelistRoot), "Not whitelisted");
        require(amountNFTsperWalletSecondWhitelistSale[msg.sender] + _quantity < MAX_MINT_PER_SECOND_WHITELIST_ACCOUNT + 1, "You can only get 1 NFTs on the Second Whitelist Sale");
        require(totalSupply() + _quantity <= MAX_SECOND_WHITELIST, "Max supply exceeded");
        require(totalSupply() + _quantity <= maxSupply, "Max supply exceeded");
        require(msg.value == salePrice * _quantity, "Wrong payment");
        amountNFTsperWalletSecondWhitelistSale[msg.sender] += _quantity;
        _mint(msg.sender, _quantity, '', false);
        require(totalSupply() <= MAX_SECOND_WHITELIST, "Limit reached");
    }

    function allowlistMint(uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        require(sellingStep > Step.SecondWhitelistSale, "Allow list sale is not activated");
        require(sellingStep < Step.SoldOut, "Sale is done");
        require(_quantity < 2, "Only 1 NFT can be minted at once");
        require(isAuthorized(msg.sender, _proof, allowlistRoot), "Not allowed");
        require(totalSupply() + _quantity <= MAX_ALLOWLIST, "Max supply exceeded");
        require(totalSupply() + _quantity <= maxSupply, "Max supply exceeded");
        require(msg.value == salePrice * _quantity, "Wrong payment");
        _mint(msg.sender, _quantity, '', false);
        require(totalSupply() <= MAX_ALLOWLIST, "Limit reached");
    }

    function teamMint(uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        require(sellingStep < Step.SoldOut, "Sale is done");
        require(isAuthorized(msg.sender, _proof, teamRoot), "Not allowed");
        require(amountNFTonTeamMint + _quantity <= MAX_TEAM, "Max supply exceeded");
        require(msg.value == 0, "Wrong payment");
        amountNFTonTeamMint += _quantity;
        _mint(msg.sender, _quantity, '', false);
        require(amountNFTonTeamMint + _quantity <= MAX_TEAM, "Limit reached");
    }
    ...
    function freeMint(uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(amountNFTsperWalletFreemint[msg.sender] + _quantity < 2, "You can only get 1 free NFT");
        require(totalSupply() + _quantity <= maxSupply, "Max supply exceeded");
        require(msg.value == 0, "Wrong payment");
    ...
    function freeBronzeMint(uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        require(sellingStep < Step.SoldOut, "Sale is done");
        require(isAuthorized(msg.sender, _proof, freeBronzeRoot), "Not allowed");
        require(amountNFTsperWalletFreemint[msg.sender] + _quantity < 4, "You can only get 3 free NFT");
        require(totalSupply() + _quantity <= maxSupply, "Max supply exceeded");
        require(msg.value == 0, "Wrong payment");
        amountNFTsperWalletFreemint[msg.sender] += _quantity;
        _mint(msg.sender, _quantity, '', false);
    ...
    function freeSilverMint(uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        require(sellingStep < Step.SoldOut, "Sale is done");
        require(isAuthorized(msg.sender, _proof, freeSilverRoot), "Not allowed");
        require(amountNFTsperWalletFreemint[msg.sender] + _quantity < 9, "You can only get 8 free NFT");
        require(totalSupply() + _quantity <= maxSupply, "Max supply exceeded");
        require(msg.value == 0, "Wrong payment");
        amountNFTsperWalletFreemint[msg.sender] += _quantity;
        _mint(msg.sender, _quantity, '', false);
    ...
    function freeGoldMint(uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        require(sellingStep < Step.SoldOut, "Sale is done");
        require(isAuthorized(msg.sender, _proof, freeGoldRoot), "Not allowed");
        require(amountNFTsperWalletFreemint[msg.sender] + _quantity < 19, "You can only get 18 free NFT");
        require(totalSupply() + _quantity <= maxSupply, "Max supply exceeded");
        require(msg.value == 0, "Wrong payment");
        amountNFTsperWalletFreemint[msg.sender] += _quantity;
        _mint(msg.sender, _quantity, '', false);
    ...
    function publicMint(uint _quantity) external payable callerIsUser {
        require(sellingStep > Step.AllowlistSale, "Public sale is not activated");
        require(sellingStep < Step.SoldOut, "Sale is done");
        require(_quantity < 2, "Only 1 NFT can be minted at once");
        require(totalSupply() + _quantity <= maxSupply - MAX_TEAM - MAX_FREE, "Max supply exceeded");
        require(msg.value == salePrice * _quantity, "Wrong payment");
        _mint(msg.sender, _quantity, '', false);
        require(totalSupply() <= maxSupply - MAX_TEAM - MAX_FREE, "Limit reached");
    }
    ...
    function setSecondWhitelistRoot(bytes32 _merkleRoot) external onlyOwner {
        secondWhitelistRoot = _merkleRoot;
    }
    ...
    function setTeamRoot(bytes32 _merkleRoot) external onlyOwner {
        teamRoot = _merkleRoot;
    }

    function setFreeRoot(bytes32 _merkleRoot) external onlyOwner {
        freeRoot = _merkleRoot;
    }

    function setFreeBronzeRoot(bytes32 _merkleRoot) external onlyOwner {
        freeBronzeRoot = _merkleRoot;
    }

    function setFreeSilverRoot(bytes32 _merkleRoot) external onlyOwner {
        freeSilverRoot = _merkleRoot;
    }

    function setFreeGoldRoot(bytes32 _merkleRoot) external onlyOwner {
        freeGoldRoot = _merkleRoot;
    }
...
abstract contract BatchReveal {
    uint constant public TOKEN_LIMIT = 5555;
    uint constant public REVEAL_BATCH_SIZE = 1111;
