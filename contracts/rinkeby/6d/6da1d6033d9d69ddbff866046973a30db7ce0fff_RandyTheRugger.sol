...
contract RandyTheRugger is ERC1155, EIP712Whitelisting, ERC2981ContractWideRoyalties, CantBeEvil(LicenseVersion.CBE_PR_HS){
  ...
  constructor() ERC1155("") EIP712Whitelisting(){
    name = "RandyTheRugger";
    symbol = "RANDY";
  }
 
  modifier randyWatch(){
    uint length = randyTokens.length;
    for(uint i = 0; i < length; i++){
      require(!tokenList[randyTokens[i]].userRedeemed[msg.sender], "User already owns Randy.");
    }
    _;
  }
  ...
  function randyMint(uint _tokenId, bytes calldata _signature) public payable randyWatch requiresWhitelist(_signature){
  ...
  function togglePause(uint[] calldata _tokenIds) public onlyOwner {
    uint length = _tokenIds.length;
    for(uint i = 0; i < length; i++) tokenList[_tokenIds[i]].isPaused = false;
  }
