    ...
    MixinResolver
{
    ...
    function burnForMigration(address, uint256[] calldata)
        external
...
contract RewardEscrowV2 is BaseRewardEscrowV2 {
    ...
    uint256 public migrateEntriesThresholdAmount =
        SafeDecimalMath.unit() * 1000; // Default 1000 SNX
        ...
        systemActive
    {
        ...
        if (
            ...
            uint256 numEntries = oldRewardEscrow().numVestingEntries(
                addressToMigrate
            );
            ...
            for (uint256 i = 1; i <= numEntries; i++) {
                uint256[2] memory vestingSchedule = oldRewardEscrow()
    ...
    event BurnedForMigrationToL2(
        address indexed account,
        uint256[] entryIDs,
        uint256 escrowedAmountMigrated,
        uint256 time
    );
