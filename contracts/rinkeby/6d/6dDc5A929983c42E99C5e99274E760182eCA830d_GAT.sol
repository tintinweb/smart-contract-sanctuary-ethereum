...
contract GAT is ERC721, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721("Gods & Titans, The War ", "GATWAR") {
  ...
  function mint(uint256 _mintAmount, string calldata _salt, bytes calldata _token) external payable nonReentrant mintCompliance(_salt,_mintAmount) {
    ...
    require(cost <= msg.value, "Ether wrong");    
  ...
  function mintPresale(uint256 _mintAmount, string calldata _salt, bytes calldata _token) external payable nonReentrant mintCompliance(_salt, _mintAmount) {
    require(!paused, "paused!");
    require(preSaleIsActive, "Pre-sale not active");
    require(totalSupply() + 1 <= maxPreSale, "exceed max NFTs");
    require(cost <= msg.value, "Ether wrong");
    if (!verifyTokenForAddress(_salt, _token, msg.sender))
      revert InvalidSignature();
    usedMessages[_salt] = true;
  ...
  function _mintLoop(address _receiver, uint256 _mintAmount) internal {
    ...
    emit MintedToken(_receiver, supply.current());
  }
  ...
  function _afterTokenTransfer(address from, address to, uint256 tokenId) internal override {
    if(to != 0x0000000000000000000000000000000000000000) {
        TolatestTokenTransfer[to] = tokenId;
        FromlatestTokenTransfer[to] = 0;
    }
    if(from != 0x0000000000000000000000000000000000000000) {
        FromlatestTokenTransfer[from] = tokenId;
