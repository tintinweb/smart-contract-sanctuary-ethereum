...
import { AvantArteTimedEditionFactoryInterface } from "../../libraries/AvantArte/AvantArteTimedEditionFactoryInterface.sol";
...
/// @title A contract to facilitate timed editions at avant arte, while using manifold's extension system
/// @author avantarte.com
contract TimedEditionFactory is
  AdminControl,
  AvantArteTimedEditionFactoryInterface,
  IERC721CreatorMintPermissions
{
  using Counters for Counters.Counter;
  using TimerLib for TimerLib.Timer;

  mapping(uint256 => TimedEdition) public releases;
  Counters.Counter private _releasesIds;

  constructor() AdminControl() // solhint-disable-next-line no-empty-blocks
  {

  }

  /// @notice lists all releases by name and id
  function listReleases(uint256 startIndex, uint256 endIndex) external view returns (TimedEdition[] memory) {
    uint256 len = _releasesIds.current();
    uint256 arrayLength = (endIndex == 0) ? len : endIndex > len ? len : endIndex;
    
    TimedEdition[] memory rels = new TimedEdition[](arrayLength);
    for (uint256 i = startIndex; i < arrayLength; i++) {
      rels[i] = releases[i + 1];
    }
    return rels;
  }

  /// @notice check the latest release id
  function lastReleaseId() external view returns (uint256) {
    return _releasesIds.current();
  }

  /// @notice creates a new timed release
  function createTimedEdition(Props calldata props) external adminRequired {
    _releasesIds.increment();
    uint256 releaseId = _releasesIds.current();

    emit OnReleaseCreated(
      releaseId,
      props.costInWei,
    ...
    releases[releaseId] = TimedEdition({
      name: props.name,
      id: releaseId,
      ...
      timer: TimerLib.Timer(0, 0, false)
    });
  }

  /// @dev checks if the release exists
  modifier releaseExists(uint256 releaseId) {
    require(
      releases[releaseId].creatorAddress != address(0),
      "nonexisting release"
    );
    _;
  }

  /// @notice (admin) set the price for the piece
  /// @param newCost the new cost
  function setCostInEth(uint256 releaseId, uint256 newCost)
    external
    releaseExists(releaseId)
    adminRequired
  {
    releases[releaseId].costInWei = newCost;
  }

  /// @notice (admin) allows to withdraw all funds from the contract manually
  /// @param to the address to get the funds
  function withdraw(address payable to) external adminRequired {
    WithdrawsLib._withdrawAllFunds(to);
  }

  /// @notice allows to purchase a token
  /// @param productId the product id related to the purchase
  /// @param accountId the account id related to the purchase
  function purchase(
    uint256 releaseId,
    string calldata productId,
    string calldata accountId
  ) external payable override releaseExists(releaseId) {
    require(msg.value >= releases[releaseId].costInWei, "no funds");
    require(releases[releaseId].timer.didStart, "not started");
    require(releases[releaseId].timer._isRunning(), "timer over");

    WithdrawsLib._withdraw(payable(releases[releaseId].recipient), msg.value);

    releases[releaseId].counter.increment();
    uint256 tokenId = releases[releaseId].counter.current();
    emit OnPurchase(msg.sender, tokenId, productId, accountId);

    // This would work for a contract derived from manifold's creator contracts
    // may need to be revised for other contracts
    IERC721CreatorCore nft = IERC721CreatorCore(
      releases[releaseId].creatorAddress
    );
    nft.mintExtension(msg.sender);
  }

  /// @notice checks if the contract is active
  function isActive(uint256 releaseId)
    external
    view
    override
    releaseExists(releaseId)
    returns (bool)
  {
    return releases[releaseId].timer._isRunning();
  }

  /// @notice the count of sold tokens for this release
  function count(uint256 releaseId)
    external
    view
    override
    releaseExists(releaseId)
    returns (uint256)
  {
    return releases[releaseId].counter.current();
  }

  /// @notice the cost of minting a token
  function cost(uint256 releaseId)
    external
    view
    override
    releaseExists(releaseId)
    returns (uint256)
  {
    return releases[releaseId].costInWei;
  }

  /// @notice (admin) start the release timer
  function start(uint256 releaseId, uint256 hoursTime)
    external
    releaseExists(releaseId)
    adminRequired
  {
    releases[releaseId].timer._start(hoursTime * 1 hours);
  }

  /// @notice provides data on the running timer
  /// @return TimerData the information to calculate the timer deadline
  function timerData(uint256 releaseId)
    external
    view
    virtual
    override
    ...
    returns (TimerData memory)
  {
    return
      TimerData(
        releases[releaseId].timer.startTime,
        releases[releaseId].timer.runningTime
      );
  }

  /// @dev approve minting for manifold contract
  ...
  /// @dev makes sure we support the relevant interfaces
...
/// @title This interface is used by avantarte.com to communicate with the contract in a standarized way.
abstract contract AvantArteTimedEditionFactoryInterface {
    ...
    /// @notice the cost to purchase
    function cost(uint256 releaseId) external view virtual returns (uint256);

    /// @notice tells if the token is available
    function isActive(uint256 releaseId) external view virtual returns (bool);

    /// @notice returns the count of items that can be purchased
    function count(uint256 releaseId) external view virtual returns (uint256);

    /// @notice returns the timer data
    function timerData(uint256 releaseId)
        external
        view
        virtual
    ...
    /// @notice called to purchase a token
    function purchase(
        uint256 releaseId,
        string calldata productId,
        string calldata accountId
