...
contract TreeOfLife is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        _feeAddrWallet = payable(0xB29033aF28bbc8f1eBd7B6af62483c05702048fd);
    ...
    function updateBuyFee(uint256 _fee) external onlyOwner {
        require(_fee <= 6, 'fee can not set more than 20%');
        _tokensBuyFee = _fee;
    }

    function updateSellFee(uint256 _fee) external onlyOwner {
        require(_fee <= 8, 'fee can not set more than 20%');
        _tokensSellFee = _fee;
    }
