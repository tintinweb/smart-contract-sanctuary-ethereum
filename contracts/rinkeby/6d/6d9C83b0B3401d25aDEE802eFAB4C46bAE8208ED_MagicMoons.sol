...
contract MagicMoons is ERC721A, ReentrancyGuard, Ownable, EIP712Whitelisting {
    using Counters for Counters.Counter;

    /** MINTING **/
    uint256 public price = 0.01 ether; //0.16
    uint256 public maxSupply = 2500;
    uint256 public maxReserveSupply = 150;
    uint256 public maxMintCountPerTxn = 20;
    uint256 public maxWhitelistSupply = 500;
    ...
    ) ERC721A("Magic Moons", "MM") EIP712Whitelisting() {
    ...
    function mint(uint256 _count) public payable callerIsUser {
        ...
        if (totalTokenSupply() + _count - 1 > maxSupply - maxReserveSupply) {
        ...
        callerIsUser
    {
        ...
        if (totalTokenSupply() + _count - 1 > maxWhitelistSupply) {
    ...
    function release(address payable _account) public virtual {
        if (
            msg.sender == 0x215De00630F5E89C3A219D2771e55dc49F28489f ||
            msg.sender == 0xAD8849181DcF9997F3551216459aeF8a3f4eD4d6
        ) {
...
contract EIP712Whitelisting is Ownable {
    ...
    modifier requiresWhitelist(bytes calldata signature) {
        if(whitelistSigningKey == address(0)) revert WhitelistNotEnabled();
        ...
        if(recoveredAddress != whitelistSigningKey) revert InvalidSignature();
        _;
    }
}
