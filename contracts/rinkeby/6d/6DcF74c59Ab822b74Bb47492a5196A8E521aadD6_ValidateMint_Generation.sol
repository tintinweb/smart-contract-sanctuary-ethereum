...
contract ValidateMint_Generation is IValidateMint_Generation_Functions, Ownable {
    ...
    ) external virtual override returns (bool passed) {
...
import { IValidateMint } from "./IValidateMint.sol";
...
interface IValidateMint_Generation_Functions is IValidateMint {
    /// Store data for new generation
    /// @param boxId Generation key to store `sale_time`, and `quantity` values
    /// @param _quantity Maxim amount of tokens available for pre-sale
    /// @param _sale_time When pre-sale is allowed for authorized addresses
    /// @custom:throw "Box must be greater than `0`"
    /// @custom:throw "Quantity must be greater than `0`"
    /// @custom:throw "Sale time not in future"
