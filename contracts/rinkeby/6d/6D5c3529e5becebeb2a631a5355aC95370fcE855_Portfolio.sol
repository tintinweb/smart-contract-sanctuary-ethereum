...
contract Portfolio is ERC721URIStorage, ERC2981ContractWideRoyalties, Ownable {
    ...
    /// Locked token URIs
    mapping(uint256 => uint256) private _lockedTokenURIs;
     ...
     */
    constructor(uint256 _royaltiesPercentage) ERC721("Test Portfolio", "TEST123")    {
        require(
    ...
    function updateTokenURI(uint256 _tokenId, string memory _tokenURI) external onlyOwner {
        ...
        require(_lockedTokenURIs[_tokenId] != _tokenId, 'Token URI Locked');

        _setTokenURI(_tokenId, _tokenURI);
    }

    /**
     * @notice Lock a tokens URI
     * @param _tokenId The token ID
     */
    function lockTokenURI(uint256 _tokenId) external onlyOwner {
        require(_exists(_tokenId), "ERC721URIStorage: URI query for nonexistent token");
        require(_lockedTokenURIs[_tokenId] != _tokenId, 'Token Already Locked');

        _lockedTokenURIs[_tokenId] = _tokenId;
    }
    ...
    function isTokenLocked(uint256 _tokenId) public view returns (bool tokenLocked) {
        ...
        tokenLocked = _lockedTokenURIs[_tokenId] == _tokenId;
    }
