...
contract Farms is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    uint currentTokenId;
    ...
    mapping(string => uint8) propertyNumber;
    ...
    constructor() ERC721("MyToken", "MTK") {
        ...
        propertyNumber["ExclusiveUltraRare"] = 0;
        propertyNumber["UltraRare"] = 1;
        propertyNumber["SuperRare"] = 2;
        propertyNumber["Rare"] = 3;
        propertyCount = 4;
        MAX_SUPPLY = 40000;
        MAX_PER_WALLET = 5;
    }
    ...
    function MintNFT(uint8 _property, string memory _uri, address _to) external payable whenNotPaused {
        ...
        require(_property <= propertyCount, "Enter a valid property");
    ...
    modifier isContractApprove(address _nftContractAddress, uint256 _tokenId) {
        require(
            address(this) ==
                IERC721(_nftContractAddress).getApproved(
                    _tokenId
                ),
    ...
    function isnftOwner(address _nftContractAddress, uint256 _tokenId, address nftSeller) 
      internal
      view
      returns (bool) 
    {
        address owner = IERC721(_nftContractAddress).ownerOf(
           _tokenId
        );
        return owner == nftSeller ? true : false;           
    }

    modifier onlyNFTOwner(address _nftContractAddress, uint256 _tokenId)
    {
        require(isnftOwner(_nftContractAddress, _tokenId, msg.sender), "only owner allowed");
        _;
    }

    function Burn2(address _nftContractAddress, uint256 _tokenId) 
       external
    ...
    function setNewPropPrice(string memory name, uint _price) public {
        propertyPrice[++propertyCount] = _price;
        propertyNumber[name] = propertyCount;
    }
    ...
    // function to add new types of properties
    // params: name - name of property, price - price of that property type

    function addPropertyType(string memory name, uint price) external onlyOwner{
        propertyPrice[propertyCount] = price;
        propertyNumber[name] = propertyCount;
        propertyCount++;
    }
