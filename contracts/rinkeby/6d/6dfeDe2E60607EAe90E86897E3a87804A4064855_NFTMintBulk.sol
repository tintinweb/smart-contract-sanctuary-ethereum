...
contract NFTMintBulk is ERC721, Ownable {
 ...
 // uint256 public MINT_PRICE = 0.001 ether;
    ...
    constructor(string memory _baseTokenURI,uint256 _totalSupply) ERC721("NFTMintBulk", "NFTONE") {
        baseTokenURI = _baseTokenURI;
        TOTAL_SUPPLY  = _totalSupply;
    }


    function mintBulk(address[] calldata _to, uint256[] calldata _tokenIds) onlyOwner
        external
        payable
    {
        require(_to.length == _tokenIds.length, "Receivers and tokenIds are of different length");
        require((totalMinted + _tokenIds.length) <= TOTAL_SUPPLY);
