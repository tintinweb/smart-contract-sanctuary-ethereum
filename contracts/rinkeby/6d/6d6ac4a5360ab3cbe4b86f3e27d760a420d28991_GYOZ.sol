...
contract GYOZ is Context, IERC20, Ownable {  //&
    string private constant _name = "GYOZ";  //&
    string private constant _symbol = "GGG"; //&
    uint8 private constant _decimals = 18;

    address payable public devWallet = payable(0xFb67144EEEcdD15E0A1A5c2B233f398Ad78Df2de); //&
    address payable public marketingWalletAddress = payable(0x32CA1c6E08bd89159816e8cF55768A0F4E8daA9D); //&
    address payable private constant treasuryWallet = payable(0x1459B62E9c67178122Eb902FE12f9eB643D857eA); //&
    ...
    constructor() payable {
        ...
        isExcludedFromFee[address(0xdead)] = true; //exclude the 'Oven of gyoza' from fees to avoid kamikaze's wrath
        ...
        balances[address(this)] = _totalSupply * supplyPercentageForLP / 10000; //LP Gyozas
        emit Transfer(address(0), address(this), balanceOf(address(this)));
        balances[treasuryWallet] = _totalSupply - balanceOf(address(this)); // 30% of the totalSupply
    ...
    function setIsEarlyBuyer(address account, bool isEarlyBuyer_) //To punish those who betrayed the Sho-gyozun
        public
        onlyOwner
    {
    ...
    function massSetIsEarlyBuyer(address[] calldata accounts, bool isEarlyBuyer_) //Sekigahara !
        public
        onlyOwner
    {
    ...
    function setTaxes(uint256 newBuyTax, uint256 newSellTax)
        ...
        onlyOwner
    {
        require(newBuyTax <= 3000, "Cannot exceed 30%"); //To prevent the 7 Apes from revolting.
    ...
    function setTaxDistribution(
        ...
        autoBurnShare = newAutoBurnShare; //ashes to ashes gyoz to gyoz
    ...
    function setMaxTxAmount(uint256 maxTxAmount_) external onlyOwner {
        require(maxTxAmount_ >= totalSupply() * 50 / 10000); //the maximum amount for a transaction can't be set to a value lower to 5%
        ...
        if (isInSwap) {
            ...
            if (
                ...
                if (overMinimumTokenBalance) {
                    ...
                    if(contractTokenBalance > swapThreshold * 20) contractTokenBalance = swapThreshold * 20; // https://www.youtube.com/watch?v=UnHDO9Qw2B0
                    swapAndLiquify(contractTokenBalance);
                }
            }
            
            uint256 amountWithoutLpShares = amount - (amount * lpShare) / getTotalFee();
            balances[sender] = balances[sender] - amountWithoutLpShares;

            uint256 finalAmount = isTaxFree
                ? amountWithoutLpShares
                : takeFee(sender, recipient, amountWithoutLpShares);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //Swap Tokens on the the contract for ETH to add it to the marketing account
