...
//contract JAWS is ERC721, IERC2981, Ownable, ReentrancyGuard, Adminlist {
	...
	modifier isValidData(MintData  memory _data) {
		...
		if( _data.amount > getSupplyRemaining(_data.tokenStyle) ) revert SupplyLimit();
        _;
    }
    ...
    constructor(address[] memory _adminlist, address _openSeaProxyRegistryAddress) ERC721("test", unicode"🚫") {
		...
		returns (uint256)
	{
		...
		if ( ( saleState == State.WhitelistSale ) &&  
			 ( _tokenStyle == STYLE_JAWS ) &&
			 
			 ( _data.amount + mintStyle[_tokenStyle].sold < DISCOUNT_MINT_MAX )
		)
		{
	...
	function getSupplyRemaining(
		...
		returns (uint16)
	{
		return mintStyle[_tokenStyle].end - (mintStyle[_tokenStyle].start + mintStyle[_tokenStyle].sold) + 1;
	}
    ...
    {
		return (address(this), SafeMath.div(SafeMath.mul(_salePrice, 25), 1000));
    ...
    function uncheckedInc(uint16 _i) internal pure returns (uint16) {
        unchecked {
            return _i + 1;
        }
    }
