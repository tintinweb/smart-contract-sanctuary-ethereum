...
contract Cryptomights is ERC721Enum, Ownable, ReentrancyGuard {
	...
	LootBoxes public lootBoxesInstance = LootBoxes(0x2DBf22A48Ad8CdB2653e916D083c9E5335a05120);
	...
	function mintReservedNFT(address[] calldata _to, uint256[] calldata _count) external onlyOwner nonReentrant{
		...
		for(uint i=0; i < _to.length; i++) {
			...
			lootBoxesInstance.mint(_to[i], _count[i]);
		}
    }
	...
	function mintPreSaleNFT(uint256 _count, bytes32[] calldata merkleProof) external payable nonReentrant{
		...
		lootBoxesInstance.mint(msg.sender, _count);
