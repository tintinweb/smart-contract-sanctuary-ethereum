...
contract Kkk is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
  ) ERC721A("Kkk", "kkk", mintLimit, _totalSupply) {
     tokenCount = 0;
  }
  ...
  //URI
  string private _modeATokenURI;
  string private _modeBTokenURI;
  string private _modeCTokenURI;
  string private _modeDTokenURI;

  //style Map
  mapping(uint256 => uint256) public mapToekenMode;
  ...
  function _baseURI(uint256 tokenId) internal view returns (string memory){
    if(mapToekenMode[tokenId] == 1) {
      return _modeATokenURI;
    }
    if(mapToekenMode[tokenId] == 2) {
      return _modeBTokenURI;
    }
    if(mapToekenMode[tokenId] == 3) {
      return _modeCTokenURI;
    }
    if(mapToekenMode[tokenId] == 4) {
