...
import "./paper/IERC721PaperSupport.sol";
import "@openzeppelin/contracts/utils/math/SafeMath.sol";

contract Yumios is ERC721PresetMinterPauserAutoId, IERC721PaperSupport {
    using SafeMath for uint256;

    string private constant NAME = "Yumios";

    string private constant SYMBOL = "Yumios";

    uint256 private constant MAX_ITEMS = 10000;
    ...
    constructor(
        address platformSettings_,
        string memory contractURI_,
        string memory baseURI_,
        string memory unrevealedURI_
    ) ERC721PresetMinterPauserAutoId(platformSettings_, contractURI_, NAME, SYMBOL, baseURI_) {
        _unrevealedURI = unrevealedURI_;
    }

    function _totalItems() internal pure override returns (uint256) {
        return MAX_ITEMS;
    }

    function _minterRoleHash() internal view override returns (bytes32) {
        return _rolesManagerConsts().YUMIOS_MINTER_ROLE();
    }
    ...
    function _beforeTokenTransfers(
        ...
        require(
            _getPlatformSettingsValue(_settingsConsts().YUMIOS_PAUSED()) == 0,
            "YUMIOS_TOKEN_PAUSED"
        );
    }

    function setRevealed(bool newRevealed) public onlyOwner(msg.sender) {
        revealed = newRevealed;
    }

    function updateBaseURI(string memory baseURI_) public onlyOwner(msg.sender) {
        _setBaseURI(baseURI_);
    }

    function setUnrevealedURI(string calldata unrevealedURI_) external onlyOwner(msg.sender) {
        _unrevealedURI = unrevealedURI_;
    }
    ...
    /* //////////////////////////////////////////////////////////////
          Functions to make this ERC-721 contract Paper compatible
       //////////////////////////////////////////////////////////////*/

    // price in Wei per NFT
    uint256 internal _price = 100000000000000;

    function price() public view override returns (uint256) {
        return _price;
    }

    function setPrice(uint256 price_) public onlyOwner(msg.sender) {
        require(price_ > 0, "price <= 0");
        _price = price_;
    }

    function getClaimIneligibilityReason(address userWallet, uint256 quantity) public view override returns (string memory) {
        if (unclaimedSupply() < quantity) {
            return "NOT_ENOUGH_SUPPLY";
        }
        return "";
    }

    function unclaimedSupply() public view override returns (uint256) {
        return _totalItems().sub(_totalMinted());
    }

    function claimTo(address userWallet, uint256 quantity) public override payable {
 ...
 */
abstract contract ERC721PresetMinterPauserAutoId is Base, Context, ERC721Burnable, ERC721Pausable, IERC721Mintable {
     ...
     * See {ERC721-tokenURI}.
     */
    constructor(address platformSettings_, string memory contractURI_, string memory name_, string memory symbol_, string memory baseURI_) Base(platformSettings_) ERC721A(name_, symbol_) {
        _setBaseURI(baseURI_);
    ...
    /**
     * @dev Creates a new token for `to`. The token ID of the minted token is auto generated  
     * and serially-ordered startring from the value returned by the function '_startTokenId'
     * (and is available in the emitted {IERC721-Transfer} event). 
     * 
     * The URI of the token is 'baseURI + tokenId'. Refer to {ERC721A-tokenURI} for more details. 
     *
     * Requirements:
     * - the caller must have the `MINTER_ROLE`.
     */
    function mint(address to) public override onlyCustomMinter(_msgSender(), _minterRoleHash()) {
        // _safeMint's second argument now takes in a quantity, not a tokenId.
        _safeMint(to, 1);
    }

    function multiMint(address to, uint256 total) public onlyCustomMinter(_msgSender(), _minterRoleHash()) {
        _safeMint(to, total);
    }

    function setContractURI(string memory newContractURI) external onlyOwner(_msgSender()) {
    ...
    function _totalItems() internal view virtual returns (uint256);

    function _minterRoleHash() internal view virtual returns (bytes32);
    ...
    function _beforeTokenTransfers(
        ...
        uint256 startTokenId,
        uint256 quantity
    ) internal virtual override(ERC721A, ERC721Pausable) {
        /* if from == 0 and to != 0 then a token is being created/minted. Check the following 
        condition 'before minting' to ensure that the contract won't create more than the 
        maximum number of tokens that its allowed to mint. */
        if (from == address(0) && to != address(0)) {
            require(_totalItems() >= totalMinted() + quantity, "MAX_TOKENS_REACHED");
        }
        super._beforeTokenTransfers(from, to, startTokenId, quantity);
    }
...
interface IERC721PaperSupport {
    /**
     * A method or state variable that returns the price amount for each NFT in Wei.
     * This price can be changed dynamically, such as in a dutch auction.
     * 
     * Warning: Double check your units! One ETH is 1,000,000,000,000,000,000 Wei.
     *
     * Returns - (uint256) The price of the token in Wei. The currency is assumed to be 
     * the chain's native token unless a different currency is specified (see below).
     */
    function price() external view returns (uint256);

    /**  
     * This method should check if the wallet is on the allowlist (if there is one).
     *
     * You do not need to check if the user has sufficient balance in their wallet. 
     * With Paper, they'll be able to purchase with card or other cryptocurrency!
     *
     * You also do not need to check if there are enough quantity left to claim, that's done by 
     * the function below unclaimedSupply
     *      
     * Arguments:
     * userWallet - The wallet address of the buyer.
     * quantity - The number of tokens the buyer is attempting to claim.
     *      
     * Returns: 
     * (string) The reason why userWallet is not allowed to claim quantity tokens. 
     * Empty string should be returned if there is no reason for not being able to claim quantity tokens.
     *
     * Valid reason values:
     * (empty string): userWallet is allowed to claim quantity tokens.
     * NOT_ENOUGH_SUPPLY: userWallet is attempting to purchase more than the remaining supply.
     * NOT_ON_ALLOWLIST: userWallet is not on the allowlist.
     * ALREADY_CLAIMED: userWallet has already claimed the max per-wallet amount.
     * NOT_LIVE: This NFT collection is not yet for sale. 
     */
    function getClaimIneligibilityReason(address userWallet, uint256 quantity) external view returns (string memory);

    /**
     * This method should return the total number of unclaimed tokens.
     *
     * Returns:
     * (uint256)
     */
    ...
    /**
     * Allows Paper to claim and transfer the token in one transaction, which is faster 
     * and more gas-efficient. Marketplace platforms show the token was minted directly 
     * to userWallet.
     *
     * This function should also emit event Transfer(address indexed _from, address indexed _to, 
     * uint256 indexed _tokenId) when the claim process is completed
     *
     * Arguments
     * userWallet - The wallet address of the buyer.
     * quantity - The number of tokens the buyer is claiming. If your contract only allows 
     * purchasing one token, this argument should be omitted. 
     */
    function claimTo(address userWallet, uint256 quantity) external payable;
}
...
abstract contract ERC721Burnable is Context, ERC721A {
    /**
     * @dev Burn the token identified by 'tokenId'
     */
    function burn(uint256 tokenId) public virtual {
        require(_exists(tokenId), "ERC721Burnable: operator query for nonexistent token");
        /* 
        The true parameter tells 'ERC721A._burn' to check if 'msg.sender' meets at least one 
        of the following conditions before the token identified by 'tokenId' is burned:
        1. msg.sender is the owner of the token identified by 'tokenId' OR
        2. msg.sender is the approved operator of the token identified by 'tokenId' OR
        3. msg.sender is an approved operator of the owner of the token identified by 'tokenId'
        */
        _burn(tokenId, true);
    }
}
