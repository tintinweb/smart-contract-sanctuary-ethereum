...
contract DillonNFTCollection3 is ERC721A, Ownable, ReentrancyGuard {
    ...
    function checkIfValidUser() public view returns (bool) {
        if (
            NFTHelper(_NFT1CollectionAddress).balanceOf(msg.sender) > 0 &&
            NFTHelper(_NFT2CollectionAddress).balanceOf(msg.sender) > 0
        ) {
            return true;
        }
        return false;
