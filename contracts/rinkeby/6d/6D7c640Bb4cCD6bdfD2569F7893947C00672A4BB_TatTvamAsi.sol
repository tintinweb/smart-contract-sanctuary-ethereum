...
contract TatTvamAsi is ERC721URIStorage, Ownable {
    ...
    function formatTokenURI(string memory imageURI, string memory name, uint256 tokenId) internal pure returns (string memory){
        return string(abi.encodePacked(
            "data:application/json;base64,",
            Base64.encode(
                bytes(abi.encodePacked(
                     '{"name":"#',Strings.toString(tokenId),' ',name,'", "description": "SVG Fractals Tat Tvam Asi", "image":"',imageURI,'"}')
            ))
        ));
    }
    ...
    function tokenURI(uint256 tokenId) public override view returns (string memory output) {
        ...
        return formatTokenURI(imageURI, names[tokenId], tokenId);
    }
}
