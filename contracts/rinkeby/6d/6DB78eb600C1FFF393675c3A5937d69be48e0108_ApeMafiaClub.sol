...
contract ApeMafiaClub is ERC721, Ownable {
    using Strings for uint256;
    ...
    uint256 public nftPresalePerAddressLimit = 10;
    ...
    constructor() ERC721("ApeMafiaClub", "AMC") {
        setHiddenMetadataUri("ipfs://{CID}/");
    }

    // Mint Compliance
    modifier mintCompliance(uint256 _mintAmount) {
            if(supply.current() > 49){
               if(supply.current() < 1999){
                    if(presale == true ) {
    ...
    // function mintTo(uint256 _mintAmount) external payable  mintCompliance(_mintAmount) {
    //     address crossmint_eth = 0xE09204ab43E5A111676f1bf868D570aB49333628;
    //     require(!paused, "The contract is paused!");
    //     require(
    ...
    function mintTo(uint256 _count, address to) external payable {
        address crossmint_eth = 0xE09204ab43E5A111676f1bf868D570aB49333628;
        require(
        msg.sender == crossmint_eth, // Optional Restriction
        "This function can be called by the Crossmint address only." // Error message
