...
contract PresaleAccessControlHandler is IPresaleAccessControlHandler, DeveloperAccessControl, Pausable, ERC165 {
  ...
  function verifyCanMintPresaleTokens(
    ...
    uint256 nonce,
    bytes calldata signature
  ) external virtual view override whenNotPaused returns (bool, bytes memory) {
    if (count != 1 || balance >= 1) {
      return (false, "wallet exceeded presale limit");
    }

    if (claimedByAddress[minter]) {
      return (false, "Presale limit reached");
    }
...
contract WhiteSandsParcelPass is ERC721, DeveloperAccessControl, Pausable, ReentrancyGuard {
  ...
  function mintPresale(
    ...
    uint256 nonce,
    bytes calldata signature
  ) external payable onlyPresale whenNotPaused nonReentrant {
    console.log("Minting: ", uint256(count), nonce);
  ...
  function destroy() external onlyUnlocked {
    ...
    selfdestruct(payable(state.payoutAddress));
  }
}
