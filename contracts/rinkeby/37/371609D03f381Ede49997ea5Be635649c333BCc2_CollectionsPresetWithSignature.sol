...
contract CollectionsPresetWithSignature is ICollectionsWithSignature, CollectionsPreset, CollectionSignature {
  bytes32 public constant SIGNER_COLLECTIONS_ROLE = keccak256("SIGNER_COLLECTIONS_ROLE");

  constructor(string memory _name, string memory _version) CollectionSignature(_name, _version) CollectionsPreset() {
  ...
  function createCollection(
    ...
    collections[++collectionCounter] = Collection(tokens, implementation, _msgSender());

    for (uint256 i = 0; i < tokens.length; i++) {
      tokenCollection[implementation][tokens[i]] = collectionCounter;
    }

    emit CreateCollection(collectionCounter, _msgSender(), tokens);
  }
}
...
contract CollectionsPreset is ICollections, AccessControlUpgradeable {
  mapping(uint256 => Collection) public collections;
  mapping(address => mapping(uint256 => uint256)) public tokenCollection;

  bytes32 public constant ADMIN_COLLECTIONS_ROLE = keccak256("ADMIN_COLLECTIONS_ROLE");
  bytes32 public constant OWNER_COLLECTIONS_ROLE = keccak256("OWNER_COLLECTIONS_ROLE");
  ...
  event AddTokensToCollection(
    uint256 collectionId,
    address creator,
    uint256[] tokens
  );
  event RemoveTokensFromCollection(
    uint256 collectionId,
    address creator,
    uint256[] tokens
  );
  ...
  constructor() {
    ...
    _setRoleAdmin(ADMIN_COLLECTIONS_ROLE, OWNER_COLLECTIONS_ROLE);
  }
  ...
  /// @dev Create new collection entity in mapping
  /// @dev Possible to create collection only with ADMIN_COLLECTIONS_ROLE
  /// @param tokens verifiable array of tokens
  /// @param implementation address token implementation
  ...
  function createCollection(
    uint256[] calldata tokens,
    address implementation
  ) external onlyRole(ADMIN_COLLECTIONS_ROLE) {
    _checkTokensWithoutCollection(tokens, implementation);
    collections[++collectionCounter] = Collection(tokens, implementation, _msgSender());
  ...
  onlyCreator(collectionId)
  {
    ...
    emit RemoveCollection(collectionId, _msgSender(), collection.tokens);
  }
  ...
  function addTokensToCollection(
    ...
    emit AddTokensToCollection(collectionId, _msgSender(), tokens);
  }
  ...
  function removeTokensFromCollection(
    ...
    emit RemoveTokensFromCollection(
      collectionId,
      _msgSender(),
      tokens
    );
  ...
  function _checkTokensCollection(uint256[] memory tokens, uint256 collectionId)
  internal
  view
  returns (bool)
  {
  ...
  function _checkTokensWithoutCollection(uint256[] memory tokens, address implementation)
  internal
  view
  returns (bool)
  {
...
contract CollectionSignature {
  ...
  keccak256("SignData(uint256[] tokens,address implementation,address creator)");
  ...
  function _getSigner(
    ...
    bytes32 digest = keccak256(
      abi.encodePacked(
        ...
        _hash(SignData({tokens: tokens, implementation: implementation, creator: creator}))
      )
    );
...
interface ICollections {
  ...
  function getCollection(uint256 collectionId) external returns (Collection memory);

  function getCollectionTokens(uint256 collectionId) external returns (uint256[] memory);

  function createCollection(uint256[] calldata tokens, address implementation) external;

  function removeCollection(uint256 collectionId) external;

  function addTokensToCollection(uint256 collectionId, uint256[] memory tokens) external;

  function removeTokensFromCollection(uint256 collectionId, uint256[] memory tokens) external;
}
