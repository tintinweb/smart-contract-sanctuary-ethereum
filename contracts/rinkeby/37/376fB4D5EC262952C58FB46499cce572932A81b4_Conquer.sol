...
{
    address public immutable CitiesTokensManager;

    modifier onlyCitiesTokensManager() {
        address sender = _msgSender();
        require(
            sender == address(CitiesTokensManager),
            "Implementations: Not CitiesTokensManager"
        );
        _;
    }

    constructor(address _CitiesTokensManager) ERC20("Temple Kin", "KIN") {
        require(
            _CitiesTokensManager != address(0),
            "Implementations: CitiesTokensManager is not set"
        );
        CitiesTokensManager = _CitiesTokensManager;
        ...
        onlyCitiesTokensManager
    {
        require(
            ...
            "Conquer: Use liquidityReward to reward liquidity"
        );
        super._mint(account, amount);
    }

    function liquidityReward(uint256 amount) external onlyCitiesTokensManager {
        require(
            address(liquidityPoolManager) != address(0),
            "Conquer: LiquidityPoolManager is not set"
        );
...
import "../contracts/IWalletObserver.sol";
...
contract OwnerRecovery is Ownable {
...
import "../contracts/ILiquidityPoolManager.sol";
