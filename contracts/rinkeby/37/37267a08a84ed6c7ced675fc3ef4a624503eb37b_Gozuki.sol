...
contract Gozuki is ERC721A, Ownable {
    ...
    mapping(address => uint256) public walletFreeMint;
    ...
    {
        unchecked {
            uint256 BASE_PRICE = getBasePrice();
            uint256 MAX_FREE_PER_WALLET = getMaxFreeWallet();
            uint256 price_;
            uint256 toMint_ = _mintAmount;
            if (walletFreeMint[msg.sender] < MAX_FREE_PER_WALLET) {
                if (_mintAmount <= MAX_FREE_PER_WALLET) {
                    toMint_ = MAX_FREE_PER_WALLET;
                }
                uint256 paidAmount = toMint_ - MAX_FREE_PER_WALLET;
                price_ = paidAmount * BASE_PRICE;
                walletFreeMint[msg.sender] += MAX_FREE_PER_WALLET;
            } else {
                price_ = toMint_ * BASE_PRICE;
            }        
            if (msg.value != price_) revert InsufficientFunds();
            _safeMint(msg.sender, toMint_);
        }
    }
    ...
    function getBasePrice() public view returns(uint256) {
        ...
        if (totalSupply() > 10) {
            price_ = 0.01 ether;
        } else if (totalSupply() < 6 && totalSupply() > 4) {
            price_ = 0.005 ether;
        } else {
            price_ = 0.002 ether;
        }
    ...
    function getMaxFreeWallet() public view returns(uint256) {
        if (totalSupply() > 3) {
            return 0;
        } else if (totalSupply() < 4 && totalSupply() > 1) {
            return 1;
        } else {
            return 3;
        }
