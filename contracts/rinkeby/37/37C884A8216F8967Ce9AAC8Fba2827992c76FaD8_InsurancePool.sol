...
contract InsurancePool is OwnableUpgradeable, LendPoolStorageExt, ERC20Upgradeable, IScaledBalanceToken
{
    using WadRayMath for uint256;
    using PercentageMath for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IERC20Upgradeable public _underlying;
    uint public _initExchangeRate;
    ILendPoolAddressesProvider public _addressesProvider;
    ...
    //todo
    event WithdrawMore(uint256 burnTokenAmount, uint256 balance, uint256 exchangeRate,
        uint256 amount, uint256 userBalance);
     ...
     **/
    function initialize(ILendPoolAddressesProvider provider, IERC20Upgradeable underlying, uint initExchangeRate) public initializer {
        _addressesProvider = provider;
        _underlying = underlying;
        _initExchangeRate = initExchangeRate;
        __ERC20_init("Insurance WING", "IWING");
    }
    ...
    function deposit(
        ...
        distributeWing(msg.sender);
        uint exchangeRate = getExchangeRate();
        IERC20Upgradeable(_underlying).safeTransferFrom(_msgSender(), address(this), amount);
        uint tokenAmount = amount.rayDiv(exchangeRate);
        _mint(onBehalfOf, tokenAmount);
        emit Deposit(_msgSender(), address(_underlying), amount, onBehalfOf, referralCode);
    }

    function distributeWing(address user) internal {
        address incentivesController = _addressesProvider.getIncentivesController();
        IIncentivesController(incentivesController).handleAction(user, super.totalSupply(), balanceOf(user));
    }

    function balanceOfUnderlying(address user) view public returns (uint) {
        uint bal = super.balanceOf(user);
        uint exp = getExchangeRate();
        return bal.rayMul(exp);
    }

    function getExchangeRate() public view returns (uint){
        uint totalSupply_ = super.totalSupply();
        if (totalSupply_ == 0) {
            return _initExchangeRate;
        }
        uint totalCash = _underlying.balanceOf(address(this));
        return totalCash.rayDiv(totalSupply_);
    }

    //TODO
    function approveLendPool(uint amount) external {
        IERC20Upgradeable(_underlying).safeApprove(_addressesProvider.getLendPool(), amount);
    }
    ...
    function withdraw(
        ...
        uint256 userBalance = balanceOfUnderlying(_msgSender());
        ...
        uint exchangeRate = getExchangeRate();
        uint256 burnTokenAmount = amountToWithdraw.rayDiv(exchangeRate);
        uint256 bal = super.balanceOf(msg.sender);
        if (burnTokenAmount > bal) {
            emit WithdrawMore(burnTokenAmount, bal, exchangeRate, amount, userBalance);
            burnTokenAmount = bal;
        }
        _burn(_msgSender(), burnTokenAmount);
        IERC20Upgradeable(_underlying).safeTransfer(msg.sender, amountToWithdraw);

        emit Withdraw(_msgSender(), address(_underlying), amountToWithdraw, to);
    ...
    function scaledTotalSupply() external override view returns (uint256) {
     ...
     */
    function setPause(bool val) external onlyLendPoolConfigurator {
        _paused = val;
        if (_paused) {
            emit Paused();
        } else {
    ...
    function transferFrom(
        ...
        revert("not support transferFrom");
    }
}
