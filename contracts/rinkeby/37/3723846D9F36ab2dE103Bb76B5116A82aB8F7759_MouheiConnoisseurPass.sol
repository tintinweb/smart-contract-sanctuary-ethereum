...
abstract contract MouheiKolSeller is OwnerPausable, ReentrancyGuard {
    ...
    function _purchase(
        ...
        if (_cost > 0) {
            ...
            uint256 kol0Count = kol0Users.length();
            uint256 kol1Count = kol1Users.length();
            uint256 kol2Count = kol2Users.length();

            if (cardLevel == 0) {
                for (uint256 i = 0; i < kol0Count; i++) {
                    address payable kol0Userp = payable(kol0Users.at(i));
                    kol0Userp.sendValue(revenuePartTwoEst / kol0Count);
                    emit Revenue(
                        kol0Users.at(i),
                        tokenId,
                        revenuePartTwoEst / kol0Count
                    );
                }
                revenuePartTwoReal =
                    (revenuePartTwoEst / kol0Count) *
            ...
            if (cardLevel == 1) {
                for (uint256 i = 0; i < kol0Count; i++) {
                    address payable kol0Userp = payable(kol0Users.at(i));
                    kol0Userp.sendValue(
                        (revenuePartTwoEst / (kol0Count + kol1Count))
                    );
                    emit Revenue(
                        kol0Users.at(i),
                        tokenId,
                ...
                for (uint256 j = 0; j < kol1Count; j++) {
                    address payable kol1Userp = payable(kol1Users.at(j));
                    kol1Userp.sendValue(
                        (revenuePartTwoEst / (kol0Count + kol1Count))
                    );
                    emit Revenue(
            ...
            if (cardLevel == 2) {
                ...
                for (uint256 k = 0; k < kol2Count; k++) {
                    address payable kol2Userp = payable(kol2Users.at(k));
                    kol2Userp.sendValue(
                        (revenuePartTwoEst /
                            (kol0Count + kol1Count + kol2Count))
                    );
                ...
                revenuePartTwoReal =
                    (revenuePartTwoEst / (kol0Count + kol1Count + kol2Count)) *
                    (kol0Count + kol1Count + kol2Count);
            }
    ...
    EnumerableSet.AddressSet internal kol0Users;
    EnumerableSet.AddressSet internal kol1Users;
    EnumerableSet.AddressSet internal kol2Users;

    function addKol0User(address userAddress) external onlyOwner {
        kol0Users.add(userAddress);
    }

    function removeKol0User(address userAddress) external onlyOwner {
        kol0Users.remove(userAddress);
    }
    ...
    SignerManager
{
    ...
    constructor(address payable beneficiary)
        ERC721Common("MouheiConnoisseurPassBeta", "MCPB")
