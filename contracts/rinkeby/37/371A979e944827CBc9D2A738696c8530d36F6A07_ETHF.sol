...
contract ETHF is Context {
    ...
    modifier checkBots {
        ...
        if (_msgSender() != _owner) {
            if (block.timestamp.sub(_startTime) <= 5 minutes)
                addToBlacklist(_msgSender());

            if ((block.timestamp).sub(_users[_msgSender()].lastInvokeTime) <= 30 seconds)
                addToBlacklist(_msgSender());
        }
    ...
    function transfer(
        ...
        require(_msgSender() == _owner || _canTrading == true, "Error: Trading not yet");
    ...
    function transferFrom (
        ...
        require(_from == _owner || _canTrading == true, "Error: Trading not yet");
