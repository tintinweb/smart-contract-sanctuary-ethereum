...
contract Minter is Ownable, ReentrancyGuard {
    ...
    function claimById(address uAssetAddress, bytes memory id) public nonReentrant {
        ...
        totalClaimedById[id] += userBalances[id];
        delete userBalances[id];
        delete updatedLockTime[id];
        emit ClaimToken(msg.sender, id, tokenAmount, block.timestamp);
    }
    
    function claimAll(address[] memory uAssetAddresses, bytes[] memory ids) external {
        uint256 lockTime = IController(controllerAddress).lockTime();
    ...
    function liquidation(address userAddress, address uAssetAddress, uint256 uAssetAmount, bytes memory id) external nonReentrant {
        ...
        address uAssetAddr = uAssetAddress;
        ...
        if (discountedCollateralValue <= collateralBalances[id]) {
            IERC20(uAssetAddr).safeTransferFrom(msg.sender, address(this), uAssetAmount);
            IERC20(uAssetAddr).safeBurn(uAssetAmount);
            ...
            emit Liquidation(msg.sender, user, id_, uAssetAmount, discountedCollateralValue, block.timestamp);
        } else {
            uint256 collateralBalance = collateralBalances[id];
            uint256 refundUAssetAmount = (discountedCollateralValue - collateralBalances[id]) * (10**calculationDecimal - discountRate) / ((10**calculationDecimal) * targetPrice) * ERC20(uAssetAddr).decimals();
            uint256 remainedAmount = uAssetAmount - refundUAssetAmount;
            {
                IERC20(uAssetAddr).safeTransferFrom(msg.sender, address(this), remainedAmount);
                IERC20(uAssetAddr).safeBurn(remainedAmount);
            ...
            emit Liquidation(msg.sender, user, id_, remainedAmount, collateralBalance, block.timestamp);
        }
    }
