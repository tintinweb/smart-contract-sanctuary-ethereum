...
contract NomoDivisions is OwnableUpgradeable {
    ...
    function setRNG(INomoRNG _generator) external onlyOwner {
        generator = _generator;
    }

    function getRandomNumber(address _generator) external onlyAdmin notZeroAddress(address(generator)) {
        randomNumber = INomoRNG(_generator).requestRandomNumber();
    }
    ...
    function countDivisions() external view returns (uint256) {
        return countPlayers() / PLAYERS_IN_DIVISION;
    }
