 ...
 *    ║*************╚═╝*╚═════╝**╚═════╝*****╚═╝*****╚═╝**╚═╝*╚═════╝*╚══════╝╚══════╝*********j▓▓▓▓▄*▓▓▀▀▓▓▌*****║
 *    ║***** ╔══════════════════════════════════════════════════════════════════════════════╗ *j▓▓*▀▓▓▓▓═*▓▓▌*****║
 *    ║***** ║  5 CHAPTERS OF 20 SONGS IN 42 BPM, 100 TOKENS PER SONG = 10000 TOTAL TOKENS! ║ **▀▓▓▓▄,*▄▄▓▓▀"*****║
 *    ║***** ║  42% OF STREAMING ROYALTIES FOR THIS PROJECT WILL BE SPLIT BETWEEN HOLDERS!  ║ ****'▀▀▓▓▀▀"********║
 *    ║***** ║  STREAMING ROYALTIES WILL BE PAYED TO TOKEN HOLDERS USING PAYMENT SPLITTER   ║ ********************║
 *    ║***** ║  PAYMENTS EVERY MONTH! MORE INFO AT HTTPS://BATISTAONTHEBEAT.COM/100PAGES    ║ ********************║
 *    ║***** ╚══════════════════════════════════════════════════════════════════════════════╝ ********************║
 *    ║***********************************************************************************************************║
 *    ╚═══════════════════════════════════════════════════════════════════════════════════════════════════════════╝
 */
...
contract Page is ERC721, ERC721Enumerable, Ownable, RoyaltiesV2Impl {
    ...
    mapping(uint => Counters.Counter) _timesMinted;
    mapping(uint => bool) _chapterOpenForMinting;
    
    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;

    address[] private _collaboratorsAddress = [0xDF5529D9AC4F342387a525B8495712967FD9dB83, 0x0AA457e8914618a5Fc26b4bc36B7B34385725883, 0x110920f43E2e4ff3F3dbcb839b70734d66bde8F2];
    uint[] collaboratorsSplit =  [
        21, // Token Collaborator 1
        21, // Token Collaborator 2
        42, // BATISTA
        16  // Token Holder's PaymentSplitter
    ];

    CollectorsPaymentSplitter private _collectorsPaymentSplitter;
    ...
    constructor() ERC721("100 Pages", "PAGE") {
        ...
        _collectorsPaymentSplitter = new CollectorsPaymentSplitter(address(this));
        _collaboratorsAddress.push(address(_collectorsPaymentSplitter));
        _collaboratorsPaymentSplitter = new CollaboratorsPaymentSplitter(_collaboratorsAddress, collaboratorsSplit);
    }
    ...
    function mintPage(uint8 page) external payable {
        ...
        require(msg.value == 0.42 ether, "PagesToken: claiming a page costs 0.42 ether");
        ...
        payable(_collaboratorsPaymentSplitter).transfer(0.42 ether);
        // auto release to collaborators on mint
        _collaboratorsPaymentSplitter.release(payable(_collectorsPaymentSplitter));
    }
    ...
    {
        super._beforeTokenTransfer(from, to, tokenId);
        if (from != address(0) && _collectorsPaymentSplitter.pendingPayment(tokenId) > 0){
            _collectorsPaymentSplitter.changePayee(from, to, tokenId);
        }
    }
    ...
    function getCollectorsPaymentSplitter() public view returns (address) {
        return address(_collectorsPaymentSplitter);
    }
...
/**
 * @title Collectors PaymentSplitter uses the main ERC721 contract to get the token owner
 * @dev This contract allows to split Ether payments among a group of accounts. The sender does not need to be aware
 * that the Ether will be split in this way, since it is handled transparently by the contract.
 *
 * The split can be in equal parts or in any other arbitrary proportion. The way this is specified is by assigning each
 ...
 *
 */
contract CollectorsPaymentSplitter is Context {
    event PayeeChanged(address from, address to, uint256 tokenId);
    event PaymentReleased(address to, uint256 amount);
    event PaymentReceived(address from, uint256 amount);
     ...
     * duplicates in `payees`.
     */
    constructor(address pagesContract) payable {
        _pagesContract = Page(pagesContract);
    }
     ...
     */
    function totalShares() public view returns (uint256) {
        return _pagesContract.maxSupply();
    }
     ...
     */
    function shares(address account) public view returns (uint256) {
        return _pagesContract.balanceOf(account);
    }

    /**
     * @dev Getter for the amount of Ether already released for a tokenId.
     */
    ...
    /**
     * @dev Getter for the address of the payee tokenId.
     */
    function payee(uint256 tokenId) public view returns (address) {
        return _pagesContract.ownerOf(tokenId);
    }

    /**
     * @dev Triggers a transfer to `account` of the amount of Ether they are owed, according to how many Page tokens they own.
     */
    function release(address payable account) public virtual {
        uint256 _shares = shares(account);
        require(_shares > 0, "CollectorsPaymentSplitter: account has no shares");
        uint256 totalPayment = 0;
        for (uint256 i = 0; i < (_shares); i++) {
            uint currentToken = _pagesContract.tokenOfOwnerByIndex(account, i);
    ...
    function release(uint256 tokenId) public virtual {
        address tokenPayee = _pagesContract.ownerOf(tokenId);
        require(tokenPayee != address(0), "CollectorsPaymentSplitter: tokenId owner is the zero address");

        uint256 _tokenPendingPayment = _pendingPayment(tokenId);
        require(_tokenPendingPayment > 0, "CollectorsPaymentSplitter: tokenId is not due payment");
        Address.sendValue(payable(tokenPayee), _tokenPendingPayment);
        _totalReleased += _tokenPendingPayment;
        _released[tokenId]+= _tokenPendingPayment;
        emit PaymentReleased(tokenPayee, _tokenPendingPayment);
    }
    ...
    function _pendingPayment(uint256 tokenId) private view returns (uint256) {
        return (address(this).balance + totalReleased()) / totalShares() - _released[tokenId];
    }

    function pendingPayment(uint256 tokenId) public view returns (uint256) {
        require(msg.sender == address(_pagesContract), "CollectorsPaymentSplitter: can only be called by Page contract");
        return _pendingPayment(tokenId);
    }

    /**
     * @dev Change payee for a tokenID.
     * Should only be called by the Page contract on transfer of token.
     */
    function changePayee(address from, address to, uint256 tokenId) public {
        require(msg.sender == address(_pagesContract), "CollectorsPaymentSplitter: can only be called by Page contract");
        if (from != address(0)){
            require(payee(tokenId) == from, "CollectorsPaymentSplitter: from account is not the payee for tokenId");
            // if tokenId has balance, pay it out then emit PayeeChanged
            if (_pendingPayment(tokenId) > 0){
                release(tokenId);
            }
        }
        emit PayeeChanged(from, to, tokenId);
    }
}