...
// File: contracts/MintingContractNoWl.sol
...
contract MintingContractNoWl is ERC721A, Ownable, Pausable, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public maxPerPublicTx = 101; // Actually 5, this is to avoid using <=

    // Withdraw accounts, 
    // Add more wallets here!
    address private constant WALLET_SWE = 0x9193052B1843d7F2cAF37728756F062922eCd51d;
    address private constant WALLET_C = 0x9193052B1843d7F2cAF37728756F062922eCd51d;
    address private constant WALLET_L = 0x9193052B1843d7F2cAF37728756F062922eCd51d;
    address private constant WALLET_F = 0x9193052B1843d7F2cAF37728756F062922eCd51d;
    ...
    function withdraw() external nonReentrant onlyOwner {
        ...
        (bool successSWE, ) = payable(WALLET_SWE).call{ value: (currentBalance * 25) / 100 }("");
        require(successSWE, "Failed to send to SWE");

        (bool successC, ) = payable(WALLET_C).call{ value: (currentBalance * 25) / 100 }("");
        require(successC, "Failed to send to C");

        (bool successL, ) = payable(WALLET_L).call{ value: (currentBalance * 25) / 100 }("");
        require(successL, "Failed to send to L");

        (bool successF, ) = payable(WALLET_F).call{ value: (currentBalance * 25) / 100 }("");
        require(successF, "Failed to send to F");
    } 
