...
contract CBCMigration is Ownable {
     ...
     */
    function migrate(uint256 _amount, uint256 _nonce, uint256 _totalAmount, bytes memory _signature) external {
        require(
        ...
        require(!nonceToValidated[_nonce], "migrate: this nonce was already used");

        // ensure that the sender is allowed by the validator
        address recovered = ECDSA.recover(keccak256(abi.encodePacked(_amount, _totalAmount, _nonce, msg.sender)), _signature);
