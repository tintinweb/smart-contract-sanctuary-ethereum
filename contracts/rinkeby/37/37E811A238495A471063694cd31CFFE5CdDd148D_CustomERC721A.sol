    ...
    RoyaltiesV2Impl
{
    ...
    uint256 constant _MINT_PRICE = 0.035 ether;
    uint256 constant _WHITELIST_MINT_PRICE = 0.025 ether;

    uint256 constant _MAX_SUPPLY = 100;

    uint256 constant _MAX_TOKENS_PER_TX = 2;
    uint256 constant _MAX_TOKENS_PER_WHITELIST_WALLET = 6;
    ...
    constructor() ERC721A('TESTFA', 'TFATOKEN') {}
    ...
    // MINTING
    function mint() external payable callerIsUser publicOpen {
        require(!_paused, 'The contract is paused');
        require(totalMinted() + 1 <= _MAX_SUPPLY, 'Token supply exceeded');
        require(msg.value >= (_MINT_PRICE * 1), 'Insufficient funds');
    ...
    // GETTERS & SETTERS
    function getRoyaltiesWallet() public pure returns (address) {
        return _ROYALTIES_WALLET;
    }

    function getRoyaltiesPercentage() public pure returns (uint96) {
        return _ROYALTIES_PERCENTAGE;
    }
    ...
    function getMaxTokensPerTx() public pure returns (uint256) {
        return _MAX_TOKENS_PER_TX;
    }

    function getMaxTokensPerWhitelistWallet() public pure returns (uint256) {
        return _MAX_TOKENS_PER_WHITELIST_WALLET;
    }
    ...
    function getLatestWinners() public view returns (Winner[] memory) {
        return _raffle_winners[_last_raffle_timestamp];
    }
