...
contract Feetback is ERC721, Ownable {
    ...
    //Date of image to display
    uint public screensaverTimestamp;
    bool public isScreensaverDisplay;
    ...
    // Token Price
    uint256 public tokenPrice = 2 * 1e15 wei;
    uint256 public constant presalePrice = 1 * 1e15 wei;
    ...
    constructor()ERC721("Feetback", "FBK"){
        _signer = msg.sender;
    }
    ...
    function getSignerAddress(address caller, bytes calldata signature) public pure returns (address) {

        string memory message = _addressToString(caller);
        require(SignerVerification.isMessageVerified(caller, signature, message), 'ECDSA: Invalid signature');
        return SignerVerification.getSigner(signature, message);
    }
    ...
    function buyTokensOnPresale(uint256 tokensNumber, bytes calldata signature) public payable {
        ...
        require(tokensNumber <= 4, "You cannot purchase more than 4 tokens at once");
        ...
        require(signer != address(0) && signer == _signer, 'ECDSA: Invalid signature');
    ...
    function changeScreensaverDate(uint _screensaverTimestamp) public onlyOwner{
        require(_screensaverTimestamp>block.timestamp, "Timestamp should be greater than block timestamp");
        screensaverTimestamp = _screensaverTimestamp;
    }
    function toggleScreensaverDisplay() public onlyOwner{
        isScreensaverDisplay = !isScreensaverDisplay;
    }
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");

        string memory baseURI = _baseURI();
        string memory uriEnding = (publicSaleActive && (isScreensaverDisplay || block.timestamp - publicSaleStartTimestamp > screensaverTimestamp)) ? tokenId.toString() : 'screensaver';
