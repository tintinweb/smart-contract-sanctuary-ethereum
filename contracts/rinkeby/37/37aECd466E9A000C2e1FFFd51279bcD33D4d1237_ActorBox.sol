...
contract ActorBox is ERC1155, Ownable, IActorBox {
    ...
    mapping(address => uint256) _orderLimits;
    uint256[] private _issuedLevels;
    uint256[] private _limits;

    /**
@notice Constructor
@param url The URI of the NFT origin
...
@param prices_ The minimal price for the level
@param limits_ The maximum possible limits for the each parameter 
@param limit_ The number of the actors allowed for one account
...
*/
    ...
    function _savePrices(uint256[] memory prices_) private {
        _prices = prices_;
    }
...
*/
    ...
    function _create(address owner_, uint256 level) private returns (uint256) {
        require(_unissued > 0, "ActorBox: sold out");
...
*/
    ...
    function _getLimits(uint256 level) private returns (uint256, uint256) {
        require(level < _limits.length, "ActorBox: wrong level");
        if (level == 0) {
...
*/
    function open(uint256 id_) external override returns (uint256) {
        require(_issuedLevels[id_] > 0, "ActorBox: cannot open");
        require(balanceOf(_msgSender(), id_) == 1, "ActorBox: wrong owner");
        ...
        uint256 beWomanPromille = _woman.compute(abi.encode(power));
...
*/
    function create(address target, uint256 level)
        external
        override
        onlyOwner
        returns (uint256)
    {
        require(_amounts[level] > 0, "ActorBox: sold out");
...
*/
    function withdraw(address payable target, uint256 amount)
        ...
        onlyOwner
    {
        require(target.send(amount), "ActorBox: Failed to send Ether");
    }
...
*/
    ...
    /**
@notice Receive the funds and give the packed actor with level according to the amount of funds transferred
Look the event to get the ID (receive functions cannot return values)
*/
    receive() external payable {
        require(_unissued > 0, "ActorBox: all tokens are issued");
        require(
            _orderLimits[_msgSender()] < _orderLimit,
            "ActorBox: tokens are limited"
        );
        uint256 i = _amounts.length - 1;
        while (i > 0 && _prices[i] > msg.value) {
            i = i - 1;
        }
        if (_prices[i] > 0) {
            require(_amounts[i] > 0, "ActorBox: sold out");
            _create(_msgSender(), i);
        }
        uint256 pos;
        do {
            pos = _random.distributeRandom(_unissued, _amounts);
        } while (_amounts[pos] == 0);
        _create(_msgSender(), pos);
    }
}
