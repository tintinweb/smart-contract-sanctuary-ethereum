...
contract Graveyard_v8 is Initializable, ERC1155Upgradeable, OwnableUpgradeable, PausableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable {
    ...
    function initialize() initializer public {
        ...
        whitelistCost = 10000000000000000;
        whitelistStatus = true;
        maxSupply = 1;
        maxValue = 1000;
        maxCountMinting = 5;
        __ERC1155_init("https://ipfs.moralis.io:2053/ipfs/QmRsFtxBbdm88dGmp2abYVukiT4Qdoi1G3q31KYBoWD9Lr/metadata/{id}.json");
        __Ownable_init();
        __Pausable_init();
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        uint256 supplyItems = mintedIds.length;
        require(_mintAmount > 0);
        require(_mintAmount <= maxValue);
        require(supplyItems + _mintAmount <= maxValue);

        if (msg.sender != owner()) {
            require(_mintAmount <= getMaxMintAmount(_to));
            require(msg.value >= price * _mintAmount);
        }

        uint256 i = 1;
        for (; i <= _mintAmount; i++) {
            uint256 supply = totalSupply(supplyItems + i);
            if(supply <= maxSupply){
                mintedIds.push(supplyItems + i);
                _mint(_to, supplyItems + i, 1, "");
    ...
    function getSupplyAmount() public view virtual returns (uint256) {
        return mintedIds.length;
    }

    function getMaxMintAmount(address _user) public view virtual returns (uint256) {
        return maxCountMinting - mintedCount[_user];
    }
    ...
    function setBaseCost(uint256 _newVeryBaseCost) public onlyOwner {
        baseCost = _newVeryBaseCost;
    }

    function setWhitelistCost(uint256 _newVeryWhitelistCost) public onlyOwner {
        whitelistCost = _newVeryWhitelistCost;
    }

    function setMaxValue(uint256 _newMaxValue) public onlyOwner {
    ...
    function setMaxCountMinting(uint256 _newMaxValue) public onlyOwner {
