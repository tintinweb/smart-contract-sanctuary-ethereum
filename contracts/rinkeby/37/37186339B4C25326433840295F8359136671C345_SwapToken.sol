...
contract SwapToken {
    ...
    function buyToken(uint amount) payable public {
        require(msg.sender.balance >= msg.value ,'Not enough Balance');// Checking caller Eth Balance

        uint numberOfTokens = msg.value * price; // Calculate Token amount according to eth send
