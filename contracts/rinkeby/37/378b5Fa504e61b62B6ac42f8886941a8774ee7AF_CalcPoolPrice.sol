...
contract CalcPoolPrice {
    ...
    function check3(address pair) public view returns (address token0, address token1) {
        token0 = IUniswapV2Pair(pair).token0();
        token1 = IUniswapV2Pair(pair).token1();
    }

    function check4(FixedPoint.uq112x112 memory priceAverage, uint amountIn) public view returns(uint result) {
        result = priceAverage.mul(amountIn).decode144();
    }

    function check5(address oracle) public view returns (address) {
        return UniswapV2PriceOracle(oracle).WETHToken();
    }
