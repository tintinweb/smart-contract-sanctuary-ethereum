...
contract MOAA is ERC721Delegated, SignedAllowance {
    ...
    uint256 internal _selectedToken;
        ...
        )
    {
            ...
            _selectedToken = 1 ; //1 = Automaton, 2 = Memoria //// we avoid 0 for gas saving
    ...
    mapping(address => uint256) private _selectors;


    address private ash = 0xaFF4481D10270F50f203E0763e2597776068CBc5;
...
////////////////////////////////////////
    ...
    function _setSelector(address owner, uint256 _selectedToken) internal {
        _selectors[owner] = _selectedToken;
        }
...
////////////////////////////////////////
    ...
    function _getSelector(address owner) internal view returns (uint256) {
        return _selectors[owner];
        }
    ...
    function totalMinted() public view returns (uint256) {
        ...
        unchecked {
            return _currentMemoriaIndex + _currentAutomatonIndex - 101; //101 for Memoria Index start // 1 for Automaton index start // then we remove 1 because PuppetMaster is not tracked, but accounted for.
        }
    }
...
////////////////////////////////////////
    ...
    function adminMint(address to, uint256 _selectedToken,uint256 tokenId) public onlyOperator {
        ...
        _setSelector(msg.sender, _selectedToken);
        // we dont do a custom check for puppet master - but it's also not accessible outside of admin mint.  
        if (_selectedToken == 1 && tokenId != 0) {
            require(_currentAutomatonIndex < 101, "Exceeds max supply of Automaton.");
            ++_currentAutomatonIndex;
        } else if (_selectedToken == 2 && tokenId != 0) {
            require(_currentMemoriaIndex < 401, "Exceeds max supply of Memoria.");
            ++_currentMemoriaIndex;
        }
        _mint(to, tokenId);
    }
    ...
    function publicPurchase(uint256 _selectedToken,uint256 nonce, bytes memory signature) external {
        ...
        if (_selectedToken == 1) {
            ...
            if (_selectedToken == _getSelector(msg.sender)) {
                return;
            } else {
                _setSelector(msg.sender, _selectedToken);
            }
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        string memory imageUrl = string(abi.encodePacked("ar:///hash/", Strings.toHexString(uint256(uint160((ERC721Base(address(this)).ownerOf(tokenId)))), 20)));
        string memory json;
        if(tokenId == 0) {
            json = string(
            abi.encodePacked( 
                '{"name": "Project5301 #0/0",',
                '"description": "",',
                ...
                '"animation_url": "', PMUrl, '",',
                '"attributes":[',
                '{"trait_type":"Archillect","value":"TV"},{"trait_type":"Artist","value":"NYX x SECONDSTATE"}',
                "]}"
            )
        );  // tokenId must be between the 1 and 100 range 
        } else if (_selectedToken == 1 && tokenId > 0 && tokenId < 101) {
            json = string(
            abi.encodePacked( 
                ...
                '"image": "', imageUrl, '",'
                '"image_url": "', imageUrl, '",',
                '"animation": "', AutomatonUrl, '",',
                '"animation_url": "', AutomatonUrl, '",',
                '"attributes":[',
                '{"trait_type":"Music","value":"DannyWithThreeBrains"},{"trait_type":"Artist","value":"NYX x SECONDSTATE"}',
                "]}"
            )
        );// tokenId must be between the 101 and 400 range
        } else if (_selectedToken == 2 && tokenId > 100 && tokenId < 401) {
            json = string(
            abi.encodePacked( 
