...
contract NFTLotteryPool is ERC721Upgradeable, OwnableUpgradeable, ERC721Holder, ReentrancyGuard {
  ...
  function buyTickets(uint256 numTickets) public payable {
    ...
    require(totalSupply[address(this)].add(numTickets) <= maxTickets, "Exceeds max supply");
    require(msg.value == ticketPrice.mul(numTickets), "Price incorrect");
    require(block.timestamp < endDate, "Lottery over");
    for (uint256 i = 0; i < numTickets; i++) {
      _mint(msg.sender, totalSupply[address(this)] + 1);
    }
  }
  ...
  function getRefund(uint256[] calldata ids) external nonReentrant {
    ...
    require(totalSupply[address(this)] < minTicketsToSell || isRefundOpen, "Enough tickets sold");
  ...
  function distributePrize() public onlyOwner {
    require(totalSupply[address(this)] >= minTicketsToSell, "Not enough tickets sold");
    require(block.timestamp > endDate, "Lottery not over");
    ...
    RNG_DISTRIBUTOR.distributeToNftHolders(REFERENCE.LINK_FEE(), address(this), 1, totalSupply[address(this)], prizeAddress, prizeId);
    hasCalledVRF = true;
  }
