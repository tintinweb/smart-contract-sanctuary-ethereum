 ...
 */
contract NFTEyePassNFT is ERC721A, Ownable, Pausable, ReentrancyGuard {
    bytes32 private _merkleRoot;

    string public constant BASE_URI = "https://api.nfteye.io/api/nft_meta/";
    ...
    uint256 public MAX_SUPPLY = 7721;
    ...
    function preSale(uint256 amount, bytes32[] memory proof)
        public
        payable
        whenNotPaused
        callerIsUser
    {
        if (!presaleActive) revert("PreSale is not available now");
        ...
        require(amount <= remainder, "Exceeded maximum supply per address");
        require(amount >= 1, "Amount should no less than 1");
        require(publicMinted + amount <= MAX_SALE, "Exceeded maximum supply");
        require(
        ...
        if (remainder > 1) {
            ...
            totalValue = amount * PRESALE_PRICE;
        }
        require(msg.value >= totalValue, "Ether sent is not enough");
    ...
    function referralMint(
        uint256 amount,
        address referrarAddress,
        bytes32[] memory proof
    ) public payable whenNotPaused callerIsUser {
        if (!referralMintActive) revert("ReferralMint is not available now");
        require(
            _numberMinted(referrarAddress) > 0,
            "Referral account not minted"
        );
        bytes32 leaf = keccak256(abi.encodePacked(referrarAddress));
        require(
            MerkleProof.verify(proof, _merkleRoot, leaf),
            "Referral address not in whitelist"
        );
        ...
        uint256 totalValue = amount * PRESALE_PRICE;
        require(msg.value >= totalValue, "Ether sent is not enough");
        _mint(msg.sender, amount);
        publicMinted += amount;
    }
    ...
    function publicMint(uint256 amount)
        ...
        callerIsUser
    {
        if (!publicMintActive) revert("PublicMint is not available now");
        require(publicMinted + amount <= MAX_SALE, "Exceeded maximum supply");
        require(
            totalSupply() + amount <= MAX_SUPPLY,
            "Exceeded maximum supply"
        );
        uint256 totalValue = amount * PUBLIC_SALE_PRICE;
        require(msg.value >= totalValue, "Ether sent is not enough");
        _mint(msg.sender, amount);
        publicMinted += amount;
    }
    ...
    function ownerMint(uint256 amount, address to)
        public
        whenNotPaused
        onlyOwner
    {
