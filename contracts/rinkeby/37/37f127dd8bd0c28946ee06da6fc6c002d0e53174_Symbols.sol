...
contract Symbols is ERC721, Ownable {
  ...
  // Maximum quantity of tokens that can be minted in a single transaction
  ...
  address private owner2 = 0xCE063A6E3adF9FA69937d2779b3474c7eA599D54;
  ...
  modifier canBuy(uint256 number) {
    ...
    require(number <= MAX_MINT_NUMBER, "Can't mint that many tokens");
    require(
      minterAddresses[msg.sender] + number <= MAX_MINT_PER_ACCOUNT,
  ...
  function mint(uint256 number) external payable canBuy(number) {
    if (!publicSaleEnabled && verifyWallet(msg.sender)) {
      _batchMint(number);
    } else {
  ...
  function addWallet(address[] memory _address) external onlyOwner {
    for (uint256 i = 0; i < _address.length; i++) {
      address entry = _address[i];
      require(entry != address(0), "NULL_ADDRESS");
      require(!whitelistedAddresses[entry], "DUPLICATE_ENTRY");
      whitelistedAddresses[entry] = true;
    }
  }

  function removeWallet(address[] memory _address) external onlyOwner {
    for (uint256 i = 0; i < _address.length; i++) {
      address entry = _address[i];
      require(entry != address(0), "NULL_ADDRESS");
      whitelistedAddresses[entry] = false;
    }
  }

  function verifyWallet(address _whitelistedAddress)
    public
    view
    returns (bool)
  {
    bool walletIsWhitelisted = whitelistedAddresses[_whitelistedAddress];
  ...
  function withdraw() external onlyOwner {
    payable(owner2).transfer(address(this).balance.mul(5).div(10));
  ...
  function remainingSupplyForAddresses(address _address)
    public
    view
    returns (uint256)
  {
    return MAX_MINT_PER_ACCOUNT - minterAddresses[_address];
  }
}