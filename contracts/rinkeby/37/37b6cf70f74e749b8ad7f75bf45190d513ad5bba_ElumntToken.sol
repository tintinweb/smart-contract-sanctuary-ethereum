...
// [ elumnt ] Token Smart Contract for the Elumnt NFT Marketplace
/**

   [ elumnt ] | Multi-Chain NFT Marketplace

   website: https://elumnt.com

   Smart contract written by Pixul Team combined with open-source contracts for optimization
...
contract ElumntTokenDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ElumntToken contract."
        );
    }
...
contract ElumntToken is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    ElumntTokenDividendTracker public dividendTracker;

    address public rewardToken;

    uint256 public swapTokensAtAmount;

    uint256 public tokenElumntRewardsFee;
    ...
    constructor(
        ...
        address[5] memory addrs, // reward 0, router 1, Treasury wallet 2, dividendTracker 3
        uint256[3] memory feeSettings, // ElumntRewards, liquidity, Treasury
        uint256 minimumTokenBalanceForDividends_,
        address serviceFeeReceiver_,
        uint256 serviceFee_
    ) payable ERC20(name_, symbol_) {
        rewardToken = addrs[0];
        _TreasuryWalletAddress = addrs[2];
        require(
            msg.sender != _TreasuryWalletAddress,
            "Owner and Treasury wallet cannot be the same"
        );

        tokenElumntRewardsFee = feeSettings[0];
        liquidityFee = feeSettings[1];
        TreasuryFee = feeSettings[2];
        totalFees = tokenElumntRewardsFee.add(liquidityFee).add(TreasuryFee);
        ...
        dividendTracker = ElumntTokenDividendTracker(
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "ElumntToken: The dividend tracker already has that address"
        );

        ElumntTokenDividendTracker newDividendTracker = ElumntTokenDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "ElumntToken: The new dividend tracker must be owned by the ElumntToken token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "ElumntToken: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "ElumntToken: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setTokenElumntRewardsFee(uint256 value) external onlyOwner {
        tokenElumntRewardsFee = value;
        totalFees = tokenElumntRewardsFee.add(liquidityFee).add(TreasuryFee);
        require(totalFees <= 6, "Total fee is over 6%");
    }
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "ElumntToken: The Uniswap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "ElumntToken: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "ElumntToken: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "ElumntToken: Cannot update gasForProcessing to same value"
        );
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        IERC20(rewardToken).transfer(_TreasuryWalletAddress, newBalance);
    }
