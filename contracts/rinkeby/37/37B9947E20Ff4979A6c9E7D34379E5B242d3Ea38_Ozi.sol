...
contract Ozi is Ownable, ReentrancyGuard, ERC721A {
    using Strings for uint256;
    uint256 public startTime = 1647770400; // 6pm Mar 20 2022
    uint256 public price = 0.5 ether;
    uint256 public maxTotalSupply = 500;
    address public ozi = 0x24Fe44E1e26872A21de07e5f7eE76b8692ead828;
    string private _baseURIExtended;
    mapping(uint256 => string) _tokenURIs;
    event Minted(address to, uint256 quantity);

    constructor() ERC721A("Ozi", "OZI") {
        _safeMint(ozi, 45);
    }
    ...
    function configureSaleData(uint256 _startTime, uint256 _price)
        external
        onlyOwner
    {
    ...
    function mint(uint256 quantity) external payable nonReentrant {
        ...
        require(startTime != 0, "start time not set yet");
        require(block.timestamp >= startTime, "not started");
        require(quantity > 0, "quantity cannot be 0");
        require(quantity <= 5, "cant buy more than 5 in each tx");
        require(totalSupply() + quantity <= maxTotalSupply, "sold out");
        require(msg.value == price * quantity, "sent ether value incorrect");
