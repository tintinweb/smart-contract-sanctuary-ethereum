...
contract ERC1155 is ERC1155Upgradeable, IHyperverseModule {
	  ...
	  mapping (uint256 => string) private _uris;
		mapping (uint256 => string) private tokenIdToStorjHash;
		mapping (uint256 => string) private tokenIdToPolicyCollection;
		...
		function unSafeMint(uint256 _tokenId, uint256 _amount, string memory storjHash, string memory policyCollection) public returns (uint256, uint256) {
			tokenIdToStorjHash[_tokenId] = storjHash;
			tokenIdToPolicyCollection[_tokenId] = policyCollection;
	    _mint(msg.sender, _tokenId, _amount, "");
    ...
    function dynamicUri(uint256 tokenId) public view returns (string memory) {
        return(
            string(abi.encodePacked(
                _uri,
								tokenIdToStorjHash[tokenId],
								"/",
								tokenIdToPolicyCollection[tokenId],
								"/",
