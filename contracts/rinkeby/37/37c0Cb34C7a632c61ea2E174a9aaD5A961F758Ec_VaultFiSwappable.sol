...
import "../interfaces/IVaultFiFactory.sol";
import "../interfaces/IVaultFiPair.sol";
import "../interfaces/IVaultFiRouter02.sol";

contract VaultFiSwappable is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress =
        payable(0x6EF408ED5660d13303f06f33627e798305638909); // Marketing Address
    ...
    string private constant _name = "VaultFi";
    string private constant _symbol = "VFT";
    ...
    IVaultFiRouter02 public vaultFiRouter;
    address public vaultfiV2Pair;
    ...
    constructor() {
        address newOwner = address(0x6EF408ED5660d13303f06f33627e798305638909);
        _rOwned[newOwner] = _rTotal;
        
        IVaultFiRouter02 _vaultFiRouter = IVaultFiRouter02(
            // RINKEBY
            0xd93E86F2AC6681ED55b85dCbb6423b04a201EFa8
        );
        ...
        address _vaultFiPair = IVaultFiFactory(_vaultFiRouter.factory())
            .createPair(address(this), _vaultFiRouter.WETH());

        vaultFiRouter = _vaultFiRouter;
        vaultfiV2Pair = _vaultFiPair;
        
        _setAutomatedMarketMakerPair(_vaultFiPair, true);
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != vaultfiV2Pair, "The pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _transfer(
        ...
        if(limitsInEffect){
            if (
                ...
                if(from != owner() && to != vaultfiV2Pair && block.number == tradingActiveBlock){
                    boughtEarly[to] = true;
                    emit BoughtEarly(to);
                }
                ...
                if (transferDelayEnabled){
                    if (to != owner() && to != address(vaultFiRouter) && to != address(vaultfiV2Pair)){
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = vaultFiRouter.WETH();
        _approve(address(this), address(vaultFiRouter), tokenAmount);
        vaultFiRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        _approve(address(this), address(vaultFiRouter), tokenAmount);
        vaultFiRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
        ...
        returns (address _pair)
    {
        ...
        IVaultFiRouter02 _vaultFiRouter = IVaultFiRouter02(_router);

        _pair = IVaultFiFactory(_vaultFiRouter.factory()).getPair(
            address(this),
            _vaultFiRouter.WETH()
        );
        if (_pair == address(0)) {
            // Pair doesn't exist
            _pair = IVaultFiFactory(_vaultFiRouter.factory()).createPair(
                address(this),
                _vaultFiRouter.WETH()
            );
        }
    ...
    // To receive ETH from vaultFiRouter when swapping
...
import './IVaultFiRouter01.sol';

interface IVaultFiRouter02 is IVaultFiRouter01 {
