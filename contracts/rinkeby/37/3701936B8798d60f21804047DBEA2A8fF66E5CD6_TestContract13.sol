...
contract TestContract13 is Ownable, ReentrancyGuard, ERC721Enumerable {
  ...
  bool public IS_ALLOWLIST_REQUIRED = false;
  bytes32 private merkleRoot = "";
  string private _tokenUri = "";

  mapping(address => uint256) private balances;

  constructor() ERC721("TestContract12", "HS") {}
  ...
  // function teamMint(uint32 amount) external onlyOwner nonReentrant {
  //   require(amount <= RESERVE_SUPPLY, "Can't mint more than reserved supply");
  ...
  //   RESERVE_SUPPLY = RESERVE_SUPPLY - amount;
  // }

  function teamMint(uint256 _amount, address _receiver) external onlyOwner {
    require(_amount <= RESERVE_SUPPLY, "That would exceed the max reserved.");
  ...
  */
  function setSaleState(bool state) external onlyOwner {
    IS_SALE_ACTIVE = state;
  }

  function setIsAllowlistActive(bool state) external onlyOwner {
    IS_ALLOWLIST_REQUIRED = state;
  }
