...
contract BotXNFT is Pausable, Ownable, ERC721AQueryable, IBotXNFT {
    ...
    modifier whenPublicSaleStarted() {
        require(publicSaleStarted, "Sale has not yet started");
        _;
    }
    ...
    function mint(uint256 _nbTokens) external payable whenNotPaused {
        // Presale minting
        require(publicSaleStarted || preSaleLive, "Sale has not yet started");
        if (!publicSaleStarted && preSaleLive) {
            require(
                preSaleListPurchases[msg.sender] + _nbTokens <=
                    preSaleMintLimit,
                "Exceeded presale allowed buy limit"
            );
            require(
                _nbTokens <= preSaleTransLimit,
                "Exceeded presale translimit"
            );
            require(preSalePrice * _nbTokens <= msg.value, "Insufficient ETH");
            preSaleAmountMinted += _nbTokens;
            preSaleListPurchases[msg.sender] += _nbTokens;
