...
contract InterestViewMAIL is Ownable {
    using SafeCast for *;
    using IntMath for uint256;
    using IntERC20 for address;

    IMAILDeployer private constant MAIL_DEPLOYER =
        IMAILDeployer(0x29B66E6a41eF5B07d9348e59c09EE855f214e3D0);
        ...
        )
    {
        ...
        if (mailMarket == address(0)) return ("???", "???", token, address(0));

        string memory name = token.safeName();
        string memory symbol = token.safeSymbol();

        return (name, symbol, token, mailMarket);
    }
