...
contract IdentityVerifier is AdminControl, IIdentityVerifier {
    ...
    mapping(uint => mapping(uint256 => mapping(uint256 => uint256))) private _gatedMintIndices;
     ...
     */
    function _checkMerkleAndUpdate(uint listingId, uint256 claimIndex, uint32 mintIndex, bytes32[] memory merkleProof) private {
        // Merkle mint, leaf looks like abi,encodePacked(0xabcd, 1)
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, mintIndex));
        require(MerkleProof.verify(merkleProof, _merkleRoots[listingId], leaf), "Could not verify merkle proof");

        // Check if mintIndex has been minted
        uint256 claimMintIndex = mintIndex >> 8;
        uint256 claimMintTracking = _gatedMintIndices[listingId][claimIndex][claimMintIndex];
        uint256 mintBitmask = 1 << (mintIndex & MINT_INDEX_BITMASK);
        require(mintBitmask & claimMintTracking == 0, "Already minted");
        _gatedMintIndices[listingId][claimIndex][claimMintIndex] = claimMintTracking | mintBitmask;
    }
