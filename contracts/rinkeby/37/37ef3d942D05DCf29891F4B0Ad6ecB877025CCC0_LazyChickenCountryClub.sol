...
contract LazyChickenCountryClub is ERC721A, Ownable, ReentrancyGuard {
  using ECDSA for bytes32;

  uint256 public constant MAX_SUPPLY = 10000;
  uint256 private constant AUCTION_START_PRICE = 0.5 ether;
  uint256 private constant AUCTION_STEP_PRICE = 0.025 ether;
  uint256 private constant AUCTION_STEP_SECONDS = 5 minutes;
  ...
  uint256 private _auctionMaxSupply = 7000;
  uint256 private _teamSupply = 4;
  uint256 private _airdropSupply = 20;
  ...
  constructor(address verifier) ERC721A("Lazy Chicken Country Club", "LCCC") {
    _verifier = verifier;
  }
  ...
  function presaleMint(uint256 quantity, bytes32 hash, bytes calldata sig) external payable nonReentrant onlyEOA {
    if (!isPresaleActive())              revert SaleNotStarted();
    if (!isWhitelisted(hash, sig)) revert AccountNotWhitelisted();
  ...
  function isWhitelisted(bytes32 hash, bytes calldata sig) internal view returns (bool) {
    ...
    return ECDSA.recover(messageDigest, sig) == _verifier;  
  }
  ...
  function airDropMint(address[] calldata receivers) external nonReentrant onlyOwner {
    if (totalSupply() + _airdropSupply > MAX_SUPPLY)  revert AmountExceedsSupply();
    if (_airdropSupply < receivers.length) revert AmountExceedsSupply();
  ...
  function setAuctionMaxSupply(uint256 auctionMaxSupply) external onlyOwner {
    if (_teamSupply + _airdropSupply + auctionMaxSupply > MAX_SUPPLY)
