...
contract Sale is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, Whitelisted {
    ...
    mapping(uint256 => bool) private _claimTokenIds;
    ...
    function getReservedTokenIds(address _user) public view returns(uint256[] memory) {
        ...
        uint256[] memory clamiableTokenIds = getClamiableTokens(tokenIds);
        return clamiableTokenIds;
    }
    ...
    */
    ...
    nonReentrant() {
        ...
        uint256[] memory clamiableTokenIds = getClamiableTokens(tokenIds);
        require(clamiableTokenIds.length != 0, "None reserved token for this address");

        for (uint256 i = 0; i < clamiableTokenIds.length; i++) {
            address signer = _verify(clamiableTokenIds[i], _tiers[i], _rates[i], _signature[i]);
            require(signer == owner(), "Signature didn't match data");

            spaceCows.mint(user, clamiableTokenIds[i], _tiers[i], _rates[i]);
            _claimTokenIds[clamiableTokenIds[i]] = true;
        }
    }
    ...
    function freeMint(address _user, uint256[] memory _tiers, uint256[] memory _rates, bytes[] memory _signature) external onlyOwner {
        ...
        for (uint256 i = 0; i < clamiableTokenIds.length; i++) {
            ...
            spaceCows.mint(_user, clamiableTokenIds[i], _tiers[i], _rates[i]);
            _claimTokenIds[clamiableTokenIds[i]] = true;
        }
    }

    function getClamiableTokens(uint256[] memory tokenIds) internal view returns (uint256[] memory) {
        uint256 index = 0;
        uint256[] memory clamiableTokenIds;

        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (_claimTokenIds[tokenIds[i]] == false) {
                clamiableTokenIds[index] = tokenIds[i];
                index += 1;
            } else {
                continue;
            }
