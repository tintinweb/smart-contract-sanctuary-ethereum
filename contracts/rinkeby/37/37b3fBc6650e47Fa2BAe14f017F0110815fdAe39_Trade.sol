...
contract Trade {
    ...
    function tradeAsset(Order memory order, Fee memory fee) internal virtual returns(uint256) {
        ...
        if(order.nftType == BuyingAssetType.LazyMintERC721){
            itemId = transferProxy.erc721mintAndTransfer(IERC721(order.nftAddress), order.seller, order.buyer, order.tokenId, order.fee,order.tokenURI,"" );
        }
        if(order.nftType == BuyingAssetType.LazyMintERC1155){
            itemId = transferProxy.erc1155mintAndTransfer(IERC1155(order.nftAddress), order.seller, order.buyer, order.tokenId, order.fee, order.supply,order.tokenURI,order.qty,"" );
        }
