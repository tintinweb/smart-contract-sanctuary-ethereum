...
contract WebacyBusinessU is Initializable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    modifier _checkERC721(address _address){
        require((IERC721Upgradeable(_address).supportsInterface(type(IERC721Upgradeable).interfaceId) || IERC721(_address).supportsInterface(type(IERC721).interfaceId)),"Does not support a Supported ERC721 Interface");
        _;
    }

    
    modifier _checkERC20(address _address){
        require((IERC20(_address).totalSupply() >= 0 || IERC20Upgradeable(_address).totalSupply() >= 0), "Is not an ERC20 Contract Address");
        _;
    }
    ...
    ) external whenNotPaused _checkERC20(contractAddress) hasPaidMembership(msg.sender) {
    ...
    function storeERC721Data(
        ...
        address[] memory backupAddresses
    ) external whenNotPaused _checkERC721(contractAddress) hasPaidMembership(msg.sender) {
        if (!haveToPayUpdate[msg.sender]) {
            haveToPayUpdate[msg.sender] = true;
        }
        ...
        if (_erc20contracts.length != 0) {
            ...
            for (uint256 i = 0; i < _erc20contracts.length; i++) {
                    ...
                    )
                {
                    ...
                    delete memberToContractToTokenStatus[member][_erc20contracts[i]];
                }
            }
        }
