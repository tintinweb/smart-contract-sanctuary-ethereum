...
contract Boardroom is ContractGuard, Ownable {
    ...
    function stake(uint256 amount) public onlyOneBlock updateReward(msg.sender) {
        ...
        bshare.safeTransferFrom(msg.sender, address(this), amount);
        if (stakeFee > 0) {
            uint256 feeAmount = amount.mul(stakeFee).div(100);
            bshare.safeTransfer(reserveFund, feeAmount);
    ...
    function withdraw(uint256 amount) public onlyOneBlock directorExists updateReward(msg.sender) {
        ...
        bshare.safeTransfer(msg.sender, amount);
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            ball.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        ball.safeTransferFrom(msg.sender, address(this), amount);
        emit RewardAdded(msg.sender, amount);
    }

    function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external onlyOperator {
        // do not allow to drain core tokens
        require(address(_token) != address(ball), "ball");
