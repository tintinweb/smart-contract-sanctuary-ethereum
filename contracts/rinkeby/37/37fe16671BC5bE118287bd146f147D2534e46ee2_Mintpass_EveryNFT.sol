...
contract Mintpass_EveryNFT is ERC721, Ownable {
    ...
    uint256 _presaleprice = 0.05 ether;
    uint256 tokenSupply = 1000;

    uint256 public _perWalletLimit = 10;
    ...
    function mintPresale(uint256 mintCount, bytes32[] memory proof) external payable {
        ...
        require(mintCount < 4, "Mintpass quantity must be 3 or less");
        ...
        require(ownerMintedCount + mintCount <= _perWalletPresaleLimit, "Minting of up to 3 Mintpasses per whitelisted wallet allowed!");
    ...
    function mintPublic(uint256 mintCount) external payable {
        ...
        require(mintCount < 11, "Mintpass quantity must be less than or equal to 10");
        require((_tokenIdCounter.current() + mintCount) <= tokenSupply, "Sold out! No more Mintpasses are available");
        uint256 ownerMintedCount = addressMintedBalance[msg.sender];
        require(ownerMintedCount + mintCount <= _perWalletLimit, "Minting of up to 10 mintpasses per wallet allowed!");
