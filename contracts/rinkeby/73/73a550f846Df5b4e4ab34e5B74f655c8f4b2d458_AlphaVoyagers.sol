...
contract AlphaVoyagers is Ownable, ERC721A, IERC2981, ReentrancyGuard {
    ...
    uint64 public constant MAX_MINT_QUANTITY = 5;
    uint64 public constant MAX_PROMOTIONAL_QUANTITY = 1000;
    ...
    string private _customBaseURI = "ipfs://QmVbJwgktjZHjjSNMMNSAkC5FokmdPNCkQHV2oeoh2C85g";
    ...
    function hasClaimedFreeToken() external view returns (bool) {
        return _getAux(msg.sender) == 1;
    }
    ...
    function withdraw() public {
        ...
        uint256 treasuryBalance_ = balance_ / 10;
        uint256 remBalance_ = (balance_ - treasuryBalance_) / 4;

        bool success_;
        (success_,) = _treasury.call{value: treasuryBalance_}("");
        require(success_, ERROR_TRANSFER_FAILED);

        (success_,) = _owner1.call{value: remBalance_}("");
        require(success_, ERROR_TRANSFER_FAILED);

        (success_,) = _owner2.call{value: remBalance_}("");
        require(success_, ERROR_TRANSFER_FAILED);

        (success_,) = _owner3.call{value: remBalance_}("");
        require(success_, ERROR_TRANSFER_FAILED);

        (success_,) = _dev.call{value: remBalance_}("");
        require(success_, ERROR_TRANSFER_FAILED);
    }
    ...
    function withdrawTokens(IERC20 token) public {
        ...
        token.safeTransfer(_treasury, remBalance_);
        token.safeTransfer(_owner1, remBalance_);
        token.safeTransfer(_owner2, remBalance_);
        token.safeTransfer(_owner3, remBalance_);
        token.safeTransfer(_dev, remBalance_);
    }
