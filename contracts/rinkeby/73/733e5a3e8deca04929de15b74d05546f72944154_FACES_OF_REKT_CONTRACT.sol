...
contract FACES_OF_REKT_CONTRACT is ERC721URIStorage, Ownable {
    ...
    uint mint_price = 0.08 ether;

    constructor() ERC721("Faces of Rekt", "FOR") {

            uint256 half_day = 4320;
   ...
   function give_xp_from_games (uint16 tokenId, uint32 xp_amount) external {
        require(msg.sender == interacter_address || msg.sender == owner(), "Only our interacter address can do that m8");

        token_struct[tokenId].experience = xp_amount;

    }

    function setMarketCap (bytes32[] memory proof, uint8 _capMultiplier, uint8 whitelistID) external {
    ...
    function M_I_N_T(bytes32[] memory proof, address to) public payable {
