...
interface INFT {
  ...
  function numTotalTokens() external view returns (uint256);
...
contract MintingRouter is Ownable, EIP712Whitelisting, ReentrancyGuard {
  ...
  /// @notice The Coffee Frens NFT contract.
  ...
  modifier validateSaleRoundParams(uint256 totalAmount, uint256 limitAmountPerWallet, uint256 maxAmountPerMint) {
    require(_totalTokensLeft() > 0 && totalAmount <= _totalTokensLeft() && totalAmount >= _mintedAmountPerRound[currentSaleIndex], "INVALID_TOTAL_AMOUNT");

    if (totalAmount != UNLIMITED_AMOUNT) {
      require(limitAmountPerWallet > 0 && limitAmountPerWallet <= totalAmount, "INVALID_LIMIT_PER_WALLET");

      require(maxAmountPerMint <= totalAmount, "INVALID_MAX_PER_MINT");
    }

    if (limitAmountPerWallet != UNLIMITED_AMOUNT) {
      require(maxAmountPerMint > 0 && maxAmountPerMint <= limitAmountPerWallet, "INVALID_MAX_PER_MINT");
    }
    _;
  }
  ...
  function enableSaleRound() external onlyOwner {
    ...
    require(saleRound.enabled == false, "SALE_ROUND_ENABLED_ALREADY");
  ...
  function disableSaleRound() external onlyOwner {
    require(saleRound.enabled == true, "SALE_ROUND_DISABLED_ALREADY");
    saleRound.enabled = false;
  }
  ...
  function whitelistMint(
    ...
    require(
      saleRound.saleType == SaleRoundType.WHITELIST,
  ...
  {
    require(
      saleRound.saleType == SaleRoundType.PUBLIC,
  ...
  function tokensLeft() public view returns (uint256) {
    ...
    return saleRound.totalAmount > _mintedAmountPerRound[currentSaleIndex]
    ? saleRound.totalAmount - _mintedAmountPerRound[currentSaleIndex]
    : 0;
  }
  ...
  function _mint(
    ...
    if (saleRound.limitAmountPerWallet != UNLIMITED_AMOUNT) {
      ...
      require(
        mintedAmountSoFar + quantity <= saleRound.limitAmountPerWallet,
        "ALLOWANCE_PER_WALLET_EXCEEDED"
      );
    }

    if (saleRound.maxAmountPerMint != UNLIMITED_AMOUNT) {
      require(quantity <= saleRound.maxAmountPerMint, "ALLOWANCE_PER_TXN_EXCEEDED");
    }

    require(
      value >= saleRound.price * quantity,
      "INSUFFICIENT_FUNDS"
    );
  ...
  function _totalTokensLeft() private view returns (uint256) {
    return _nftContract.numTotalTokens() - _nftContract.totalSupply();
  }
...
contract EIP712Whitelisting {
   ...
   * @dev EIP712Whitelisting constructor
   */
  ...
  /**
   * @notice Sets whitelist signing address
   * @param newSigningKey New signing key
   */
  ...
  /**
   * @param signature Signature calldata
   * @dev Whitelist checking modifier
   */
  modifier requiresWhitelist(bytes calldata signature) {
    require(whitelistSigningKey != address(0), "WHITELIST_NOT_ENABLED");
    ...
    require(recoveredAddress == whitelistSigningKey, "INVALID_SIGNATURE");
    _;
  }
}
