...
contract TestBTM is ERC721, Ownable, ReentrancyGuard {
  ...
  uint256 public startDate = 1648314000; // Epoch for 3/26/22 10:00:00 AM PCT
  ...
  constructor(bytes32 merkleroot) ERC721("Testing", "ABC") {
    setHiddenMetadataUri("ipfs://QmZyadxSGnDzYpppoDvoHW4NdYXD6xJtcGcpx7TCWTjQUx/hidden.json");
    root = merkleroot;
  }

  function mintNFT(bytes32[] calldata proof, uint256 _mintAmount) external payable mintCompliance(_mintAmount) {
    if (msg.sender != owner()) {
      require(!paused, "The contract is paused!");
      if(onlyBumylisted == true) {
        require(isWhiteListed(msg.sender, proof), "Not on the Bumylist");
