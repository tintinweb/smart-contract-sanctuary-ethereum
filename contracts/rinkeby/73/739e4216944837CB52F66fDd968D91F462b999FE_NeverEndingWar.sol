...
contract NeverEndingWar is Ownable, IERC721Receiver {
    ...
    function stakeDuringMint(address account, uint256 tokenId) external {
        ...
        userTeamData[account].totalTeamCp += uint24(IChampion(championAddress).getCombatPower(tokenId));
        vault[tokenId] = Stake({
            owner: account,
    ...
    function stake(uint256[] calldata tokenIds) external {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            userTeamData[msg.sender].totalTeamCp += uint24(IChampion(championAddress).getCombatPower(tokenId));
    ...
    function _unstakeMany(address account, uint256[] calldata tokenIds) internal {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            for (uint j = 0; j < userTeamData[account].tokenIds.length; j++) {
                if (userTeamData[account].tokenIds[j] == tokenIds[i]){
                    userTeamData[account].tokenIds[j] = userTeamData[account].tokenIds[userTeamData[account].tokenIds.length-1];
                    userTeamData[account].tokenIds.pop();
                }
            }
    ...
    function updateTotalWon(address account) internal{
        ...
        userTeamData[account].totalWoNToken = uint160(earned);
    }
    ...
    function getTeamCp(address account) public view returns(uint256){
        return uint256(userTeamData[account].totalTeamCp);
    }
