...
contract ERC1155Marketplace is ERC1155YuzuInternal, ReentrancyGuard {
    ...
    function distributeFunds(uint256 totalPrice, address from, address to, uint256 tokenId) private {
...
contract ERC1155Azuki is ERC1155Marketplace, EIP712 {
    ...
    function redeem(
        uint256 id, uint256 nftPostId, uint256 qty, uint256 price, uint256 collectionId, uint256 chainId, uint256 salt, bytes memory signature, uint256 qtyToMint
        ...
        verify( id, nftPostId, qty, price, collectionId, chainId, salt, address(this), signature, contractOwner);
        ...
        // payments to the contract owner

        uint256 total_value = qtyToMint * price;
        (bool success, ) = contractOwner.call{value:total_value}("");
        require(success, "Transfer failed.");

        // Inform the Lazy Mint
        emit LazyMint(vouchersNFTIds[id], _msgSender(), contractOwner, total_value);
    ...
    function verify(
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256("NFTVoucher(uint256 id,uint256 nftPostId,uint256 qty,uint256 price,uint256 collectionId,uint256 chainId,uint256 salt,address thisContract)"), 
                    id, nftPostId, qty, price, collectionId, chainId, salt, thisContract
                )
            )
        );
