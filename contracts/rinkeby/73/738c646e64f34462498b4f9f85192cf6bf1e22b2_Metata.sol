...
contract Metata is ERC1155, Ownable {
    string public name;
    ...
    uint16 public purchaseCount = 0;

    function setURI(string memory newuri) public onlyOwner {
        _setURI(newuri);
    }

    function mint(address account, uint16 amount, bytes memory data)
        public
    {
        require(purchaseCount + amount <= maxSupply, "Too many requested");
        purchaseCount += amount;
        _mint(account, TOKEN_ID, amount, data);
    }

    function purchaseRemaining() public view virtual returns (uint16) {
        return maxSupply - purchaseCount;
    }
}