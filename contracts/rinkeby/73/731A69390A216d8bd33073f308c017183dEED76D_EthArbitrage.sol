...
contract EthArbitrage is AccessControlUpgradeable {
    ...
    function changeStable(address newStableAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
        stableAddress = newStableAddress;
        stables = IERC20(newStableAddress);
        emit changedStable(newStableAddress);
    }

    function changeBasic(address newBasicAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
        basicAddress = newBasicAddress;
        basics = IERC20(newBasicAddress);
        emit changedBasic(newBasicAddress);
    }

    function withdrawStable(uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256) {
        require(stables.transfer(msg.sender, amount));
        return (amount);
    }

    function withdrawBasic(uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256) {
