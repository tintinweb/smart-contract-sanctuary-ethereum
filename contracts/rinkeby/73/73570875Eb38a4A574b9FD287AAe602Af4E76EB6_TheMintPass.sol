...
contract TheMintPass is ERC721URIStorage, Ownable {
    ...
    uint256 public constant mintPrice = 0.090 ether;
    ...
    function createWLToken(uint8 numberOfTokens) external payable {
        require(isWLMintOpen, 'The white list mint is not open.');
        require(whiteList[msg.sender] >= numberOfTokens, 'Exceeded max available to purchase.');
    ...
    function createToken(uint8 numberOfTokens) external payable {
    ...
    function numAvaibleToMint(address addr) public view returns (uint8) {
