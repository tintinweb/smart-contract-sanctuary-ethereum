...
contract TestAirdrop is Pausable, ReentrancyGuard, Ownable {
     ...
     * @param signature signature to verify completion of tasks
    ...
    function claimAndTransfer(
        ...
        bytes32 node = keccak256(abi.encodePacked(msg.sender, multiplier));
        require(
        ...
        bytes32 hash = ECDSA.toEthSignedMessageHash(
            keccak256(abi.encodePacked(msg.sender, multiplier))
        );
        require(
            ECDSA.recover(hash, signature) == signer,
            "Airdrop: Signature failed to recover"
        );
