...
contract BookMarketplace is Ownable, ReentrancyGuard {
    ...
    // Events
    event ItemListed(uint256 tokenId, address seller, uint256 value);
    event ItemUpdated(uint256 tokenId, address seller, uint256 newPrice);

    event ItemCanceled(uint256 tokenId, address seller);

    event ItemSold(uint256 tokenId, address seller, address buyer, uint256 value);
    event NewOffer(uint256 tokenId, address buyer, uint256 value);
    event RoyaltiesPaid(uint256 tokenId, uint256 royaltiesAmount);
    ...
    modifier isListed(uint256 _tokenId) {
        // require(bytes(listings[_tokenId].length) == 0, "");
        Listing memory listed = listings[_tokenId];
        require(listed.minPrice > 0, "Not listed item");
        _;
    }

    modifier notListed(uint256 _tokenId) {
        Listing memory listed = listings[_tokenId];
        require(listed.minPrice == 0, "Already listed");
        _;
    }

    modifier isMarketable(uint256 _tokenId) {
        require(nftContract.getApproved(_tokenId) == address(this), "Token transfer not approved");
        _;
    }

    modifier tokenOwnerOnly(uint256 _tokenId) {
        require(nftContract.ownerOf(_tokenId) == _msgSender(), "Not token owner");
        _;
    }

    modifier tokenOwnerForbidden(uint256 _tokenId) {
        require(nftContract.ownerOf(_tokenId) != _msgSender(), "Token owner not allowed");
        _;
    }

    constructor(address _nftContractAddress, address payable _feeRecipient, uint16 _platformFee) {
        nftContractAddress = _nftContractAddress;
        nftContract = BookNFT721(nftContractAddress);
    ...
    /// @param _minPrice - minimum price at which the token can be sold
    function listItem(uint256 _tokenId, uint256 _minPrice) external notListed(_tokenId) isMarketable(_tokenId) tokenOwnerOnly(_tokenId) {
        // Create sell offer
        listings[_tokenId] = Listing({ seller : _msgSender(), minPrice : _minPrice });
        // Broadcast sell offer
        emit ItemListed(_tokenId, _msgSender(), _minPrice);
    }

    /// @notice Withdraw a sell offer
    /// @param _tokenId - id of the token whose sell order needs to be cancelled
    function cancelListing(uint256 _tokenId) external isMarketable(_tokenId) {
        require(listings[_tokenId].seller != address(0), "No sale offer");
        require(listings[_tokenId].seller == _msgSender(), "No sale offer");
        // Remove the current sell offer
        delete (listings[_tokenId]);
        // Broadcast offer cancell
        emit ItemCanceled(_tokenId, _msgSender());
    }

    /// @notice Method for updating listed NFT
    /// @param _tokenId -id of NFT token
    /// @param _newPrice New sale price for item
    function updateListing(uint256 _tokenId, uint256 _newPrice) external nonReentrant isListed(_tokenId) tokenOwnerOnly(_tokenId) {
        Listing storage listedItem = listings[_tokenId];
        listedItem.minPrice = _newPrice;
        emit ItemUpdated(_tokenId, _msgSender(), _newPrice);
    }

    /// @notice Purchases a token and transfers royalties if applicable
    /// @param _tokenId - id of the token to sell
    function buyItem(uint256 _tokenId) external tokenOwnerForbidden(_tokenId) payable nonReentrant {
        address seller = listings[_tokenId].seller;

        require(seller != address(0), "Not active sell offer");
        ...
        if (nftContract.getApproved(_tokenId) != address(this)) {
            delete (listings[_tokenId]);
            // Broadcast offer cancel
            emit ItemCanceled(_tokenId, seller);
            // Revert
            revert("Invalid sell offer");
        }

        require(msg.value >= listings[_tokenId].minPrice, "Amount sent to low");
        uint256 saleValue = msg.value;
        uint256 feeAmount = (saleValue * platformFee) / 1000;
        // Pay royalties if applicable
        if (_checkRoyalties(nftContractAddress)) {
        ...
        (bool success2, ) = payable(seller).call{value: saleValue - feeAmount}('');
        require(success2, "Seller funds transfer failed");
        // And token to the buyer
        nftContract.safeTransferFrom(seller, _msgSender(), _tokenId);
        // Remove all sell and buy offers
        delete (listings[_tokenId]);
        // delete (offers[_tokenId]);
        // Broadcast the sale
        emit ItemSold(_tokenId, seller, _msgSender(), msg.value);
    }
    ...
    ///         deducting royalties
    function _deduceRoyalties(uint256 _tokenId, uint256 _grossSaleValue) internal returns (uint256 netSaleAmount) {
        // Get amount of royalties to pays and recipient
        (address royaltiesReceiver, uint256 royaltiesAmount) = nftContract.royaltyInfo(_tokenId, _grossSaleValue);
    ...
    // function makeOffer(uint256 _tokenId, IERC20 _payToken, uint256 _pricePerItem, uint256 _deadline) external tokenOwnerForbidden(_tokenId) {
    //     // Reject the offer if item is already available for purchase at a
    //     // lower or identical price
    //     if (listings[_tokenId].minPrice != 0) {
    //         require(_pricePerItem > listings[_tokenId].minPrice, "Sell order at this price or lower exists");
    //     }
    //     // Only process the offer if it is higher than the previous one or the
    //     // previous one has expired
    //     // require(offers[_tokenId].createTime < (block.timestamp - 1 days) || 
    //     //     msg.value > offers[_tokenId].price, "Previous buy offer higher or not expired");
        
    //     address previousOfferOwner = offers[_tokenId].buyer;
    //     uint256 refundOfferAmount = buyOffersEscrow[previousOfferOwner][_tokenId];
    //     // Refund the owner of the previous buy offer
    //     buyOffersEscrow[previousOfferOwner][_tokenId] = 0;
    //     if (refundOfferAmount > 0) {
    //         (bool success, ) = payable(previousOfferOwner).call{value: refundOfferAmount}('');
    //         require(success, "Transfer funds failed");
    //     }
    //     // Create a new buy offer
    //     offers[_tokenId] = Offer({buyer : _msgSender(),
    //                               price : msg.value,
    //                               createTime: block.timestamp});
    //     // Create record of funds deposited for this offer
    //     buyOffersEscrow[_msgSender()][_tokenId] = msg.value;
    //     // Broadcast the buy offer
    //     emit NewOffer(_tokenId, _msgSender(), msg.value);
    // }
...
contract BookNFT721 is ERC721URIStorage, EIP712, Ownable, AccessControl, Voucher, ERC2981PerTokenRoyalties {
    ...
    uint16 public immutable platformFee;
