...
contract ERC5169 is IERC5169, OwnableUpgradeable {
...
contract MyNFT is ERC721Upgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function getCurrentIndex() internal view returns(uint){
        return _tokenIdIndex;
    }
}

contract MyNFTBulkRetort is MyNFT, ERC5169 {

    address private _retort;

    bytes constant _emptyBytes = new bytes(0x00);

    event retortSet(address oldAddress, address newAddress);

    function setRetort(address newAddr) external onlyOwner{
        // emit retortSet(newAddr, newAddr);
        emit retortSet(_retort, newAddr);
        _retort = newAddr;
    }

    function mintWithRetort(address to, string calldata identifier ,string memory metadataUID) external payable {
        uint mintedId = getCurrentIndex();
        mintSequential(address(this), metadataUID);
        _approve(_retort, mintedId);
        AlchemyRetortV2 retortContract = AlchemyRetortV2(_retort);
        ERC721Token[] memory request = new ERC721Token[](1);
        request[0] = ERC721Token(address(this), mintedId, _emptyBytes);

        (bool success, bytes memory returnData) = _retort.call{value: msg.value}(
            abi.encodeWithSignature("commitNFTAndreturnId((address,uint256,bytes)[],string)", request, identifier));

        require(success, "request failed");

        uint commitmentID = bytesToUint(returnData);
        retortContract.transferFrom( address(this), to, commitmentID);
    }
