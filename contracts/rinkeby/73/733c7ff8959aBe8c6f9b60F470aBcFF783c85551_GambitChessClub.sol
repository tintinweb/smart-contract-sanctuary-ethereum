...
contract GambitChessClub is ERC721Enumerable, Ownable, IERC2981 {
    ...
    uint8 public constant maxSupply = 5; //10000
    ...
    function mintOwner(address owner, uint8 numberOfTokens) private onlyOwner {
        uint256 supply = totalSupply();
        for (uint256 i = 1; i <= numberOfTokens; i++) {
            _safeMint(owner, supply + numberOfTokens);
        }
    }
    ...
    function mintNFT(uint8 numberOfTokens) external payable {
        ...
        if (isSaleActive) {
            ...
            require(whitelistUser(msg.sender), "User is not allowed");
            require(
    ...
    function whitelistUser(address _user) public view returns (bool) {
