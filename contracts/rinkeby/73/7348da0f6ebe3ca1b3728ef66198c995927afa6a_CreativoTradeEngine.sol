   ...
   function bid(uint256 _tokenid, bytes calldata _data ) external payable nonReentrant
    {
        ...
        require(keccak256(_data) == keccak256(abi.encodePacked(app_sec_data)) && !auctionOf[_tokenid].locked,"Security error");
    ...
    function claimTokenOnWin(uint256 _tid, uint256 _adminFeePercent, bytes calldata _data) public nonReentrant
    {
        ...
        auctionOf[_tid].transferred = true;
    ...
    //
    function settleAuction(uint256 _tid, uint256 _adminFeePercent, bytes calldata _data) public onlyOwner
    {
        require(!(auctionOf[_tid].transferred && block.timestamp > (auctionOf[_tid].start+auctionOf[_tid].duration)),"Either auction is not over or Token is alreayd claimed");
        uint256 adminfee = (auctionOf[_tid].highest_bid_value * _adminFeePercent) / 10000;
        //deduct admin fee from the bid value
        uint256 amountToBePaid = auctionOf[_tid].highest_bid_value - adminfee;
        payable(auctionOf[_tid].seller_address).transfer(amountToBePaid);
        auctionOf[_tid].transferred = true;
        CreativoToken.safeTransferFrom(auctionOf[_tid].seller_address, auctionOf[_tid].highest_bidder, _tid, 1, _data);
    }
    ...
    function lockAuctionOperationsOnEmergency (uint256 _tid) public onlyOwner{
        auctionOf[_tid].locked = true;
    }
