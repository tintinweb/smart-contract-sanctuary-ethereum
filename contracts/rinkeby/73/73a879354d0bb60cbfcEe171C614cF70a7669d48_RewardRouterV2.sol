...
contract RewardRouterV2 is ReentrancyGuard, Governable {
    ...
    function handleRewards(
        ...
        if (_shouldClaimWeth) {
            if (_shouldConvertWethToEth) {
                uint256 wethAmount = IRewardTracker(feeGmxTracker).claimForAccount(account, address(this));
    ...
    function signalTransfer(address _receiver) external nonReentrant {
        require(IERC20(gmxVester).balanceOf(_msgSender()) == 0, "RewardRouter: sender has vested tokens");

        _validateReceiver(_receiver);
        pendingReceivers[_msgSender()] = _receiver;
    }
