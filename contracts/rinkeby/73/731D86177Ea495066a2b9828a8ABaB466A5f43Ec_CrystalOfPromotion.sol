...
// File: interfaces/ICrystalOfPower.sol
...
contract CrystalOfPromotion is ERC721, ICrystalOfPower {
    ...
    ) ERC721("Crystal of LEXOR Power", "Crystal of LEXOR Power") {
    ...
    modifier checkMint() {
        require(quantity + 1 <= limit, "Limit exceeded. Max: 1000");
        require(
            balanceOf(msg.sender) == 0,
            "Limit exceeded. Max: 1 per wallet."
        );
        if (gold_list_addresses[msg.sender]) {
            require(msg.value >= 3 * 4 * 10**17, "Not enough funds send");
        } else if (white_list_addresses[msg.sender]) {
            require(msg.value >= 3 * 45 * 10**16, "Not enough funds send");
        } else {
            require(msg.value >= 3 * 55 * 10**16, "Not enough funds send");
        }
        require(
            IERC721(shard_crystal_address).balanceOf(msg.sender) >= 3,
            "Not enough MetaSkynet Shard Crystals in your wallet. Min: 3"
        );
        _;
    }
    ...
    function setLimit(uint256 _amount) public {
        require(msg.sender == project_address, "Permission denied");
        require(_amount <= limit, "Limit exceeded. Max: 1000");
        drop_amount = _amount;
    }
