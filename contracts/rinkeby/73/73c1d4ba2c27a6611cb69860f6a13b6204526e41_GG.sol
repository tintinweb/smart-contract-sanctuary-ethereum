...
// File: SchlongSociety/IERC721A.sol
...
// File: SchlongSociety/ERC721A.sol
...
contract GG is Ownable, ERC721A, ReentrancyGuard {
    string _baseUri = "ipfs://QmbHPfCr9gtUUg8kR2EhaC6XfAZ1zFAaLkqvjpwSZCi5Wm/";
    mapping(address => bool) freeWl;
    ...
    IERC721A public goblinTown = IERC721A(0x62869709Ab24bc5985046bD9Eb857b7cdAFb0f0B);

    constructor(
        uint256 maxBatchSize_,
        uint256 collectionSize_
    )   ERC721A("Goblin Girls", "GG", maxBatchSize_, collectionSize_) {
    }


    function reserve(uint256 quantity) external onlyOwner {
        require(quantity + totalSupply() <= collectionSize, "SS: Not enough tokens left for minting");
    ...
    function mintFree() external {
        if(goblinTown.balanceOf(msg.sender) > 0 && !goblinMinted[msg.sender]) {
            freeWl[msg.sender] = true;
        }
        require(freeWl[msg.sender], "GG: User is not approved for free mint");
        require(hasSaleStarted, "GG: Cannot mint before sale has started");
        require(1 + totalSupply() <= collectionSize, "GG: Total supply exceeded");
        require(freeMinted + 1 <= maxFree, "GG: No more free Goblin Girls available");
