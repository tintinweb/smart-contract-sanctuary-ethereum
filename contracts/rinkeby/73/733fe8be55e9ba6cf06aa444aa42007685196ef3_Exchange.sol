...
contract ExchangeAdmin is Whitelist {
    ...
    function changeOwner(address _newOwner) external onlyWhiteList {
        ...
        owner = _newOwner;
        emit OwnerAddressChanged(_owner, _newOwner);
    }
    ...
    function transferEthToOwner(uint256 _amount) external {
...
contract KeccakHelper is EIP712Upgradeable {
...
contract Validator is KeccakHelper {
	...
	function validateFull(
		...
		validateSignature(sellOrder, sellOrderSignature);
		validateSignature(buyOrder, buyOrderSignature);
    ...
    function validateAssetType(DataStruct.Asset memory offeredAsset, DataStruct.Asset memory expectedAsset) internal pure {
        if (TokenType.ERC721 != offeredAsset.assetType && TokenType.ERC1155 != offeredAsset.assetType) {
            revert("Asset type does not match");
        }

        if (TokenType.ETH != expectedAsset.assetType && TokenType.ERC20 != expectedAsset.assetType) {
            revert("Asset type does not match");
        }
    }
    ...
    function recoverSignerWeb(
        DataStruct.Order memory order,
        bytes memory signature
    ) external pure returns (address) {
        bytes32 hashedOrder = hashOrder(order);
        return recoverSigner(hashedOrder, signature);
    }
}
...
contract TransferManager is ExchangeAdmin {
	...
	function manageOrderTransfer(
        ...
        if(TokenType.ETH == expectedAsset.assetType) {
	        ...
	        ITransfer(expectedAsset.addr).transferFrom(buyer, seller, payment.netAmount);
	        ITransfer(expectedAsset.addr).transferFrom(buyer, feeReceiver, payment.feeAmount);
    	}
	...
	{
		DataStruct.PaymentDetail memory payment;
