...
contract SolarSystem is ERC721Tradable {
   ...
   function decompress(strings.slice memory text) public view returns (string memory) {
       string memory HTML = "";
       string memory HTML2 = "";
       uint count = text.count("|".toSlice()) + 1;
       for(uint i = 1; i < count/2;i++) {
            strings.slice memory mapping_tot = text.split("|".toSlice());
            if(i%2 == 0)
                HTML = HTML.toSlice().concat(parts[mapping_tot.toString()].toSlice());
            else
                HTML = HTML.toSlice().concat(mapping_tot);
       } 
       for(uint i = count/2; i < count;i++) {
            strings.slice memory mapping_tot = text.split("|".toSlice());
            if(i%2 == 0)
                HTML2 = HTML2.toSlice().concat(parts[mapping_tot.toString()].toSlice());
            else
                HTML2 = HTML2.toSlice().concat(mapping_tot);
       }    
       return HTML.toSlice().concat(HTML2.toSlice());
   }
   ...
   function contractURI() public pure returns (string memory) {
    ...
    function getTOKEN(uint t) public view returns (string memory) {
       string memory HTML = decompress(HTML_CODE.toSlice());//decompress(HTML_CODE);
