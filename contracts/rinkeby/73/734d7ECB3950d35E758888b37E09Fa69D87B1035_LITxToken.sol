...
contract LITxToken is 
    ...
    modifier canBridge(uint256 chain) {
        require(chains[chain], "LITxToken: bad chain");
        _;
    }
    ...
    function bridgeGetOn(uint256 amount, uint256 targetChain) external canBridge(targetChain) {
        uint256 tx = _txCounter.current();
        _txCounter.increment();
        super._burn(msg.sender, amount);
        emit BridgeGotOn(msg.sender, amount, targetChain, tx);
    }

    /**
     * @dev Get off the bridge.
     */
    function bridgeGetOff(address beneficiary, uint256 amount, uint256 originChain, uint256 tx) external onlyBridge {
        bytes32 hash = keccak256(abi.encode(originChain, tx));
        require(!txs[hash], "LITxToken: tx replay");
        txs[hash] = true;
        super._mint(beneficiary, amount);
        emit BridgeGotOff(beneficiary, amount, originChain, tx);
    }
