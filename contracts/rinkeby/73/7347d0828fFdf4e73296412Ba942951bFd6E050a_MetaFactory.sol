...
contract MetaFactory is Ownable {
    ...
    struct TokenInfo {
        ...
        uint256 price;
        // Price in Dollar (with 2 decimals)
        // The price is either fixed in Dollar or ETH
        // If the price is fixed in Dollar then usdPrice > 0
        uint256 usdPrice;
    ...
    address public fundsRecipient = 0x3162dC1A02869F653cedCEB5792D79E902B705fA;
    ...
    constructor(address _ethToUsdFeed) {
        require(_ethToUsdFeed != address(0), "Invalid address");
    ...
    * @param to Address where the purchased tokens will be sent to
    ...
    function setTokenInfo(
        ...
        require(price > 0 || usdPrice > 0, "Price must be greater than 0");
        require(beneficiary != address(0), "Invalid beneficiary address");
        require(originalOwner != address(0), "Invalid original owner address");
        // price and usdPrice are mutually exclusive, one of them must be 0
        require(price == 0 || usdPrice == 0, "You cannot fix the price both in USD or ETH");
        // Set the info of the token to allow the deposit later
    ...
    * @param active Whether to enable or disable the sale
     */
    function toggleTokenSale(address tokenAddress, uint256 tokenId, bool active) external {
        ...
        require(details.beneficiary != address(0), "Token not defined");
        // Only the owner of the token or the owner of the contract can call this function
        require(msg.sender == owner() || msg.sender == details.originalOwner, "Not allowed"); 
    ...
    * @param accepted Whether to consider them as accepted or not
    ...
    function getPriceInETH(address tokenAddress, uint256 tokenId) public view returns (uint256) {
        require(tokenDetails[tokenAddress][tokenId].usdPrice > 0, "Price not fixed in USD");
        // Get the price fixed in USD for the token if any
        uint256 priceInUsd = tokenDetails[tokenAddress][tokenId].usdPrice;
