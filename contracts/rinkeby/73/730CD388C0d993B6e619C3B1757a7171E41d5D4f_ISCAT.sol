...
contract ISCAT is ERC721A, Ownable {
    ...
    uint256 public newMaxBatchSize;
    ...
    function mints(uint numberOfTokens, address to) public onlyOwner {
        ...
        require((numberOfTokens % newMaxBatchSize == 0 || numberOfTokens < newMaxBatchSize),"can only mint a multiple of the maxBatchSize");
        if (numberOfTokens < newMaxBatchSize) {
            _safeMint(to, numberOfTokens);
        } else {
            uint256 numChunks = numberOfTokens / newMaxBatchSize;
            for (uint256 i = 0; i < numChunks; i++) {
                _safeMint(to, newMaxBatchSize);
            }
        }
