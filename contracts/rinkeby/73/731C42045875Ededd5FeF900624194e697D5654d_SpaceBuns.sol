...
contract SpaceBuns is ERC721A, ReentrancyGuard, Ownable {
    ...
    uint256 public alMintPrice = 0.005 ether;
    ...
    constructor() payable ERC721A("SPACEBUNS", "BB") {
      ...
      billionBuns = IERC721Enumerable(0xFeE057faCe6048AfEcF80B00974770c2b9899557);
    }

    modifier priceCalc(uint _price, uint _quantity) {
      require(_quantity > 0, "You need to mint at least 1 NFT.");
      require(msg.value >= _price * _quantity, "Insufficient ETH");
    ...
    function publicMint(uint256 _quantity) external payable nonReentrant priceCalc(mintPrice, _quantity) {
    ...
    function alMint(bytes32[] calldata _merkleProof, uint256 _quantity) external payable nonReentrant priceCalc(alMintPrice, _quantity) {
    ...
    function ogMint(bytes32[] calldata _merkleProof) external nonReentrant {
      ...
      require(maxSupply - reserveSupply >= _tokenIds.current() + 1, "Sold out or Exceeds max tokens");
    ...
    function reserveMint(bytes32[] calldata _merkleProof, uint256 _quantity) external nonReentrant {
      ...
      require(_reserveWallets[msg.sender] + _quantity <= maxReserveMint, "You reached max per wallet.");
      require(_quantity > 0, "You need to mint at least 1 NFT.");
      require(reserveSupply >= _tokenIds.current() + _quantity, "Sold out or Exceeds max tokens");
    ...
    function ogMintStatus() public view returns (bool) {
    ...
    function setReserveMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
      reserveMerkleRoot = _merkleRoot;
    }
    ...
    function withdrawSplit() external nonReentrant onlyOwner {
      ...
      (bool wallet2, ) = payable(msg.sender).call{value: address(this).balance}("");
