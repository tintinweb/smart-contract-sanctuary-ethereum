...
contract Project is ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function publish(ProjectInput memory _projectInput, SaleInput[] memory _saleInputs) external payable {
        require(msg.value == createProjectFee, "Invalid value");
        ...
        if (token == address(0)) {
            token = (osbFactory.create(_projectInput.isSingle, _projectInput.tokenName, _projectInput.tokenSymbol, _projectInput.royaltyReceiver, _projectInput.royaltyFeeNumerator)).token;
        }

        _projectInput.isSingle ? IOSB721(token).setApprovalForAll(address(sale), true) : IOSB1155(token).setApprovalForAll(address(sale), true);
        ...
        project.amount    = sale.creates(project, _saleInputs);
...
interface IOSBFactory {
    function create(bool _isSingle, string memory _name, string memory _symbol, address _royaltyReceiver, uint96 _royaltyFeeNumerator) external returns(TokenInfo memory);
}
