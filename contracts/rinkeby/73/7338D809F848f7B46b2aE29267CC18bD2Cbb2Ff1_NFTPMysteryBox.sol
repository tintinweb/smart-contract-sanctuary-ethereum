...
library MysteryBox {
    ...
    function canMint(Collection storage c, uint256 amount) internal view returns(bool) {
        return c.cap >= c.minted + amount;
    }

    function getCap(uint16[][4] memory rarities) internal pure returns(uint16 cap) {
    ...
    function getCopies(Collection storage c) internal view returns(uint16 copies) {
        for(uint16 i=0; i<RARITY_COUNT; i++) {
            copies += c.rarities[i][0];
        }
    }
...
contract NFTPMysteryBox is ERC1155, ReentrancyGuard, Ownable {
    ...
    MysteryBox.Collection[] collections;
    mapping(uint256 => mapping(uint16 => mapping(uint16 => uint16))) collectionPhotoRarityMinted;
    ...
    constructor(string memory uri_, string memory contractMetadataURI, address nftp_, address token) ERC1155(uri_) {
    ...
    */
    function _mint(MysteryBox.Collection storage c, uint256 id, uint256 amount) internal {
        require(c.isOpen(), "Collection not mintable");    
        require(c.canMint(amount), "Cap reached");
        require(c.maxBuy == 0 || amount <= c.maxBuy, "Amount too high");

        //TODO: Limit the minted number
    ...
    */
    function reveal(uint256 id) external {
        ...
        uint16 photo = _random(msg.sender, c.minted) % photoCount;
        uint16 rarity = _random(msg.sender, c.minted) % c.getCopies();
        uint16 raritySum;
        uint16 rarityGroup;
        ...
        while(gasUsed == 0 || gasUsed < gasleft() - GAS_BUFFER) {
            ...
            for(uint16 i=raritySum; i<MysteryBox.RARITY_COUNT; i++) {
                raritySum += c.rarities[i][photo];
                if(rarity < raritySum) {
                    rarityGroup = i;
                    break;
                }
            ...
            if(collectionPhotoRarityMinted[id][photo][rarityGroup] + 1 <= c.rarities[rarityGroup][photo]) {
                collectionPhotoRarityMinted[id][photo][rarityGroup]++;
                _burn(msg.sender, id, 1);
                nftp.mint(msg.sender, tokenId, id, rarityGroup, photo);
                minted = true;
                break;
            } 
            ...
            if(!minted) {
                if (rarity % 2 > 0) {
                    rarityGroup++;
                    if (rarityGroup >= MysteryBox.RARITY_COUNT) {
                        photo = (photo + 1) % photoCount;
                        rarityGroup %= MysteryBox.RARITY_COUNT;
                    }
                } else {
                    photo++;
                    if (photo >= photoCount) {
                        photo %= photoCount;
                        rarityGroup = (rarityGroup + 1) % MysteryBox.RARITY_COUNT;
                    }
                }
            }
        }
        ...
        emit Reveal(id, tokenId, rarityGroup, photo, msg.sender);
    }

    /**
    * @notice Produces a pseudo random uin16 number
    * @param from sender address
    * @param minted number of minted NFTs from the collection
    */
    function _random(address from, uint256 minted) internal view returns (uint16){
        return uint16(uint256(keccak256(abi.encodePacked(_seed, block.difficulty, block.timestamp, blockhash(block.number + 2), from, minted))));
    }
...
interface IPussiesNFT {
    ...
    function newCollection() external;
    function totalSupply() external returns(uint256);
}

contract PussiesNFT is IERC721, ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    */
    function mint(address to, uint256 id, uint256 collection, uint16 rarity, uint16 photo) external { 
        require(msg.sender == mysteryBox || msg.sender == owner(), "Cannot mint");
    ...
    */
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://data.nftpussies.com/";
    }
