...
contract HouseHold is UpgradeablePausable, IHouseHold {
  ...
  mapping(uint256 => MortgagesAddress) internal mortgagesBorrowers;
  ...
  function createMortgage(
    ...
    _addMortgage(clientId, mortgage);
    return mortgage;
  }
  
  function _addMortgage(uint256 clientId, address mortgage) internal {
    if (!mortgagesBorrowers[clientId].init) {
      mortgagesBorrowers[clientId].init = true;
      mortgagesBorrowers[clientId].addresses = [mortgage];
    } else{
      mortgagesBorrowers[clientId].addresses.push(mortgage);
    }
    mortgagesBorrowers[clientId].mortgages[mortgage] = true;
  }

  function getUserMortgages(uint256 clientId) public view override returns (address[] memory) {
    return mortgagesBorrowers[clientId].addresses;
  }

  function getMortgageStatus(uint256 clientId, address mortgage) public view override returns (bool) {
    return mortgagesBorrowers[clientId].mortgages[mortgage];
  }
