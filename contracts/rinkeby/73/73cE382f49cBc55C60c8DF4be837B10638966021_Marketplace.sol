...
error Marketplace__MarketNotApproved();
error Marketplace__ItemAlreadyListed(address nftAddress, uint256 tokenId);
error Marketplace__NotItemOwner(address nftAddress, uint256 tokenId, address spender);
error Marketplace__ItemNotListed(address nftAddress, uint256 tokenId);
error Marketplace__PriceNotMet(address nftAddress, uint256 tokenId, uint256 itemprice);
error Marketplace__NoProceeds();
error Marketplace__WithdrawFailed(address seller);
...
contract Marketplace is ReentrancyGuard {
    ...
    event ItemUpdated(address seller, address nftAddress, uint256 tokenId, uint256 newPrice);

    modifier itemNotListed(address nftAddress, uint256 tokenId) {
    ...
    modifier isOwner(
        ...
        if (spender != owner) {
            revert Marketplace__NotItemOwner(nftAddress, tokenId, spender);
        }
        _;
    }
    ...
    /*
     * @notice A method for listing your NFT on the marketplace
    ...
    function listItem(
        ...
        uint256 tokenId,
        uint256 price
    ) external itemNotListed(nftAddress, tokenId) isOwner(nftAddress, tokenId, msg.sender) {
        if (price <= 0) {
            revert Marketplace__PriceMustBeAboveZero();
        }
        IERC721 nft = IERC721(nftAddress);
        if (nft.getApproved(tokenId) != address(this)) {
            revert Marketplace__MarketNotApproved();
        }
        s_listings[nftAddress][tokenId] = Listing(price, msg.sender);
        emit ItemIsListed(msg.sender, nftAddress, tokenId, price);
    }
    ...
    function buyItem(address nftAddress, uint256 tokenId)
        ...
        payable
        nonReentrant
        itemIsListed(nftAddress, tokenId)
    {
        Listing memory item = s_listings[nftAddress][tokenId];
        if (msg.value < item.price) {
            revert Marketplace__PriceNotMet(nftAddress, tokenId, item.price);
        }
        s_sellerProceeds[item.seller] = s_sellerProceeds[item.seller] + msg.value;
    ...
    function updateListing(
        ...
        uint256 tokenId,
        uint256 newPrice
    ) external isOwner(nftAddress, tokenId, msg.sender) itemIsListed(nftAddress, tokenId) {
    ...
    function withdrawProceeds() external payable nonReentrant {
        ...
        if (!success) {
            revert Marketplace__WithdrawFailed(msg.sender);
        }
    }
