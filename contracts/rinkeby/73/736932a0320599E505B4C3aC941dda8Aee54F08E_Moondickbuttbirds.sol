...
contract Moondickbuttbirds is ERC721A, Ownable {
    ...
    function mint(uint256 _count) external payable {
        ...
        if ( _canClaim && _count <= (freePerWallet - _mintedByAddress)  ) {
            require(msg.value == 0, "Ether sent is incorrect");
        }
        else if (_mintedByAddress >= freePerWallet) {
            require(msg.value == cost * _count, "Ether sent is incorrect");
        }
        else {
            require(msg.value == cost * ( _count + _mintedByAddress - freePerWallet), "Ether sent is incorrect");
        }
    ...
    //     require(canClaim(_wallet), "Nope");
    //     mintedByAddress[_wallet] += 1;
