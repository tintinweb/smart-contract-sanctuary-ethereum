...
interface IOrangeNFT721 {

    function setManager(address newManager) external;

    function mint(address account, uint256 cid, string memory tokenURI) external returns (uint256);

    function setTokenURI(address owner, uint256 cid, uint256 tokenId, string memory uri) external;
...
contract Manager is Ownable {
    ...
    constructor (address _orangeSigner) {
        orangeSinger = _orangeSigner;
    }

    function setNFTAddr(address orangeNFT) external onlyOwner {
        nftAddr = orangeNFT;
    }
    ...
    function mint(address account, uint256 cid, string memory tokenURI, uint256 limit, bytes32 unsigned, bytes memory signature) public returns (uint256) {
        ...
        campaign.minted++;
        participated[key]++;
        return IOrangeNFT721(nftAddr).mint(account, cid, tokenURI);
    }
