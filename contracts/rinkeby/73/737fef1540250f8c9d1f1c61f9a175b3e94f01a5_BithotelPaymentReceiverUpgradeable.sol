...
contract BithotelPaymentReceiverUpgradeable is
    Initializable,
    ...
    UUPSUpgradeable
{
    ...
    event PaymentReceived(address account, uint256 amount, address nftCollection);
    ...
    function initialize(address mOwner, address mClaimProcess) public initializer {
        __AccessControl_init();
        __UUPSUpgradeable_init();
        __BithotelPaymentReceiver_init_unchained(mOwner, mClaimProcess);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __BithotelPaymentReceiver_init_unchained(address mOwner, address mClaimProcess) internal onlyInitializing {
    ...
    function claim(address account) external virtual nonReentrant onlyRole(CLAIM_ROLE) {
    ...
    function receiveBNB(address nftCollection) public payable nonReentrant {
        ...
        require(msg.value == _gasfee, "amount BNB must be equal to gasfee");
        _balanceOf[account] += msg.value;
        _forwardFunds();
        emit PaymentReceived(account, msg.value, nftCollection);
    }
    ...
    function whitelistNft(address nftAddress) public view onlyRole(DEFAULT_ADMIN_ROLE) {
        require(nftAddress != address(0));
        require(hasRole(WHITELISTED_COLLECTION, nftAddress));

        // _grantRole(DEFAULT_ADMIN_ROLE, nftAddress);
    }
    ...
    function _forwardFunds() internal {
        payable(_claimProcess).transfer(msg.value);
    }
