...
contract HomeyDeployer2 is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  function ogMint(uint256 _mintAmount, bytes32[] calldata _ogMerkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    // Verify whitelist requirements
    require(ogMintEnabled, 'The OG sale is not enabled!');
    require(!ogClaimed[_msgSender()], 'Address already claimed!');
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProof.verify(_ogMerkleProof, ogMerkleRoot, leaf), 'Invalid proof!');
