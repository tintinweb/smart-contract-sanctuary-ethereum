...
contract CryptoMason is ERC721A, Ownable {
    ...
    mapping(address => claim) adrToNewOrder;
    ...
    mapping(address => uint256) userToFutureMon;
    ...
    function allowUser(
        ...
        allMoneyForUsers -= userToFutureMon[user1];
        allMoneyForUsers += myReward;
        userToFutureMon[user1] = myReward;
    }
    ...
    function stake() external {
        ...
        require(
            adrToNewOrder[msg.sender].user != address(0),
            "Don't have stakes"
        );

        claimInfo[msg.sender].push(adrToNewOrder[msg.sender]);
        adrToStakes[msg.sender].push(claimInfo[msg.sender].length);
    ...
    function batchUnstakeNFTs(uint256[] memory nfts) external {
        for (uint256 i; i < nfts.length; i++) {
            unStakeNFTs(nfts[i]);
            uint256 index = 10000000000000001;
            for (
                uint256 l;
                l < claimInfo[msg.sender][nfts[i]].nftIds.length;
                l++
            ) {
                for (uint256 j; j < adrToIdsArray[msg.sender].length; j++) {
                    if (
                        adrToIdsArray[msg.sender][j] ==
                        claimInfo[msg.sender][nfts[i]].nftIds[l]
                    ) {
                        index = j;
                        break;
                    }
                }
                if (index != 10000000000000001) {
    ...
    function unStakeNFTs(uint256 numberClaim) internal {
        ...
        if (checkMyRewards(msg.sender, numberClaim + 1) == 0) {
            uint256 index = 0;
    ...
    function getRewards(uint256 numberClaim) internal {
        ...
        require(
            checkMyRewards(msg.sender, numberClaim + 1) > 0,
            "Dont have rewards"
        );
        ...
        if (
            ...
            myReward = checkMyRewards(msg.sender, numberClaim + 1);
            a = true;
        } else {
            uint256 divide = 1 days * 100;
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            myReward -= claimInfo[user][numberClaim].alreadyGiven;
            return myReward;
        } else {
            uint256 divide = 1 days * 100;
