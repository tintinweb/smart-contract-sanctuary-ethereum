...
contract Ocean is Ownable, ReentrancyGuard, Pausable {
    ...
    mapping(uint8 => Upgrade[]) public submarineUpgrades;
    // Store upgrade made on submarine
    mapping(uint8 => Upgrade[]) public krakenUpgrades;
    ...
    */
    function dive(uint8[] calldata tokensId, uint256[] calldata divingsTime) public noContract whenNotPaused nonReentrant{
        ...
        for(uint i = 0; i < tokensId.length; i++){
            ...
            _fuel.burn(msg.sender, fuelNeeded);
    ...
    */
    ...
    function _upgradeByType(uint8 tokenId, uint256 levelAfterUpgrade) internal{

        tokenIsUpgrading[tokenId] = true;
        ...
        if(_isSubmarine(tokenId)){
            ...
            submarineUpgrades[tokenId].push(Upgrade(
                ...
                // TODO: Add price of $PEARL and median level of submarine
            ));
            ...
            if(levelAfterUpgrade <= 10){
                ...
                krakenUpgrades[tokenId].push(Upgrade(
                    ...
                    levelAfterUpgrade,
                    block.timestamp + ( UPGRADE_TIME * 1 hours )
                    // TODO: Add price of $PEARL and median level of kraken
                ));
...
contract Pearl is ERC20, Ownable{
    ...
    */
    function transferTo(address recipient, uint256 amount) public{
        require(controllers[msg.sender], "Only controllers can tranfer");
        this.transfer(recipient, amount * 1 ether);
    }
    ...
    */
    ...
    function setTax(uint256 taxeRate) public onlyOwner{
        _taxRate = taxeRate;
    }
    ...
    function _beforeTokenTransfer(
       ...
       //TODO: Finish taxe 
        ...
        // Tax on
        // if(block.timestamp > _startTime + 24 hours){
...
contract NautilusNFT is ERC721Enumerable, Ownable, ReentrancyGuard, Pausable{
    ...
    uint256 public constant MAX_GEN0_SUPPLY = 10_000;
    ...
    function _mintToken(address _minter, uint256 _amount) private{
        ...
        // Store NFT Metadata
        nfts.push(NFT(totalSupply(),getRandomType(rand),1));

        _safeMint(_minter, totalSupply());

        emit TokenMinted(_minter,totalSupply()-1);

    }

    function mintGen1() public payable nonReentrant whenNotPaused{
        require(tx.origin == msg.sender, "Contracts not allowed");
        require(_gen1Started,"GEN1 not started");
        require(_gen1TokenSelled != _gen1TokenToSell,"GEN1 batch completed.");
        require(msg.value == _gen1Price,"Wrong value");

        _gen1TokenSelled += 1;

        _mintToken(msg.sender, 1);

        // TODO: Finish gen1 function
    } 
    ...
    function collectSale(uint256 amount) public onlyOwner whenNotPaused{
    ...
    function transferFunds(uint _amount, address _receiver) public onlyOwner whenNotPaused{
    ...
    function startGEN1() public onlyOwner{
        _gen1Started = true;
    }
