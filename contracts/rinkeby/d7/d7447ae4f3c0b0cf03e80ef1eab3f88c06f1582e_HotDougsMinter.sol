...
}// care of https://github.com/nibbstack/erc721/blob/master/src/contracts/tokens/erc721-token-receiver.sol
/**
...
/// @author @DangyWing
/// @title HotDougsMinter

contract HotDougsMinter is Ownable, ReentrancyGuard, ERC721TokenReceiver {
    event Received();

    // No Hot Dougs in this Wallet
    error NoTokens(uint256 available, uint256 minimum);

    // No Free Dougs available to be claimed
    error noFreeDougsForAddress();

    // mapping from dougTokenId to claim status. 0 = available, 1 = claimed
    mapping(uint256 => uint256) public claimedDougs;

    /// Max Dougs per transaction
    uint256 public constant paidDougsPerTx = 10;

    /// price of Hot Dougs minted through Minter contract
    uint256 public hotDougPrice = .025 ether;

    /// original HotDougs Contract Address
    address public dougContractAddress;

    /// 0 = closed, 1 = pre-sale, 2 = public
    uint256 public saleState;

    /// Max total supply
    uint256 public maxHotDougs = 6969;

    /// only allow free dougs until a specified tokenId
    ...
    //// @notice return current total number of Dougs
        ...
        returns (uint256[] memory)
    {
        ...
        if (tokenBalance < 1) {
            revert NoTokens({available: tokenBalance, minimum: 1});
        }
    ...
    /// @notice check if the transaction originator can redeem free Dougs
    function checkForFreeDougs() public view returns (bool) {
        HotDougs _hotDougs = HotDougs(dougContractAddress);
        uint256 tokenBalance = _hotDougs.balanceOf(tx.origin);
        if (tokenBalance < 1) {
            revert NoTokens({available: tokenBalance, minimum: 1});
        }
        uint256[] memory addressTokenIds = _getTokenIds(tx.origin);
    ...
    function sendFreeDougs() external {
        // check if address has free valid Hot Dougs to claim

        if (checkForFreeDougs() == false) {
            revert noFreeDougsForAddress();
        }
        uint256[] memory addressTokenIds = _getTokenIds(tx.origin);
        uint256 length = addressTokenIds.length;
    ...
    /// @notice update the maximum Hot Dougs in the collection per Minter Contract
    function updateHotDougMinterMaxDougs(uint256 _newDougMax)
        external
        onlyOwner
    {
        maxHotDougs = _newDougMax;
    }

    /// @notice update the original Hot Doug contract address
    ...
    /// Mints a Doug to the Minter contract then sends to the user who originated the transaction
