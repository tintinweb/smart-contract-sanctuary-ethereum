...
contract WildEXOplanetGame is ERC721, EIP712, Ownable{
    ...
    uint256[] public tierMintTimes = [1643645759, 1643647559, 1643647559, 1643647559, 1643647559];
    ...
    function mint(address to, uint8 amount, uint8 tier) public payable {
      if(tier == 0){
        require(block.timestamp <= revealTime[0], "NFT: Phase 1 Mint Ended!");
      }
      else{
        require(block.timestamp <= revealTime[1], "NFT: Public sale already ended!");
      }
      require(block.timestamp >= tierMintTimes[tier], "NFT: Public sale not enabled!");
    ...
    function updateTierMintTimes(uint256[] memory _tierMintTimes) external onlyOwner{
      tierMintTimes = _tierMintTimes;
    }

    function updateBaseURIs(string[] memory _baseUris) external onlyOwner{
      tierBaseUris = _baseUris;
    }

    function updateGiveawaybaseURIs(string[] memory _giveawayUris) external onlyOwner{
        giveawayBaseUris = _giveawayUris;
    }
