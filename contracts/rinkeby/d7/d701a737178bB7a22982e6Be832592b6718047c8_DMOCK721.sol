...
contract DMOCK721 is Context, ERC721Burnable, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    string private _baseURIExtended;
    uint16 private _totalAllocation;
    ...
    function mintTo(address to) external returns (uint256 tokenId) {
        require(
            _remainingAllocation[_msgSender()] > 0,
            "You do not have any allocations"
        );
        require(
            _maxSupply >= _tokenIds.current() + 1,
            "Cannot mint due to max supply"
        );

        _tokenIds.increment();
        _mint(to, _tokenIds.current());
        _remainingAllocation[_msgSender()] -= 1;

        tokenId = _tokenIds.current();
    }

    function mintTo(address to, uint16 amount)
        external
        returns (uint256 startTokenId, uint256 toTokenId)
    {
        ...
        require(
            ...
            "You do not have enough allocations"
        );
        require(
            _maxSupply >= _tokenIds.current() + amount,
            "Cannot mint due to max supply"
        );
        ...
        toTokenId = _tokenIds.current();
        startTokenId = toTokenId - amount + 1;
    }
    ...
    function batchTransferFrom(
        ...
        require(fromArr.length > 0, "Wrong parameters");
        require(fromArr.length == toArr.length, "Wrong parameters");
        require(fromArr.length == tokenIdArr.length, "Wrong parameters");

        for (uint256 i = 0; i < fromArr.length; i++) {
            transferFrom(fromArr[i], toArr[i], tokenIdArr[i]);
        }
    }
     ...
     */
    function infoWallet(address user) external view returns (uint16, uint16) {
