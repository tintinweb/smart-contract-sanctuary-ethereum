...
contract Rugbirds is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public constant MAX_SUPPLY = 33;//3333
    uint256 public constant MAX_PUBLIC_MINT = 5; 
    uint256 public constant FREE_TOKEN_PRICE = 0.0 ether;
    uint256 public constant PUBLIC_PRICE_PER_TOKEN = 0.0025 ether;//0.025
    ...
    function mint(uint8 numberOfTokens) external payable {
        ...
        if(_mintedFromAddress[msg.sender] == 0){
            require(isFreeMintActive, "Free mint is not active");
            require(numberOfTokens + _mintedFromAddress[msg.sender] <= FREE_MINT_AMOUNT, "Exceeded max available to purchase");
            require(ts + numberOfTokens <= MAX_FREE_SUPPLY, "Purchase would exceed max tokens for free mint");
...
/*
    function mintFree(uint8 numberOfTokens) external payable {
        uint256 ts = totalSupply();
        require(isFreeMintActive, "Free mint is not active");
        require(numberOfTokens + _mintedForFree[msg.sender] <= FREE_MINT_AMOUNT, "Exceeded max available to purchase");
        require(ts + numberOfTokens <= MAX_FREE_SUPPLY, "Purchase would exceed max tokens for free mint");
        require(FREE_TOKEN_PRICE * numberOfTokens <= msg.value, "This is free");

        _mintedForFree[msg.sender] += numberOfTokens;
        for (uint256 i = 0; i < numberOfTokens; i++) {
            _safeMint(msg.sender, ts + i);
        }
    }
