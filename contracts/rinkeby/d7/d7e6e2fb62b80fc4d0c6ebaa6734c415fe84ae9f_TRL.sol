...
import "../interfaces/ITRL.sol";


contract TRL is Context, ITRL {
    ...
    constructor (address _config) {
        ...
        _rTotal = (type(uint256).max - (type(uint256).max % config.trlTotalSupply()));
               
        _rOwned[_msgSender()] = _rTotal;
        decimals = config.trlDecimals();

        isExcludedFromReward[config.deadAddress()] = true;
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = config.uniswapV2Router().WETH();

        _approve(address(this), address(config.uniswapV2Router()), tokenAmount);

        // make the swap
        config.uniswapV2Router().swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        config.uniswapV2Router().addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    function _callTokensReceived(
        ...
        address implementer = config.ERC1820_REGISTRY().getInterfaceImplementer(to, TOKENS_RECIPIENT_INTERFACE_HASH);
    ...
    modifier onlyOwner() {
        require(config.trlOwner() == msg.sender || address(config) == msg.sender , "Ownable: caller is not the owner");
        _;
    }
...
interface IConfig {
    ...
    function ERC1820_REGISTRY() external view returns (IERC1820Registry);
