...
contract SleeFiVestingVault is Initializable, OwnableUpgradeable {
    ...
    mapping(address => mapping(address => VestingSchedule)) _vestingSchedule;
    mapping(address => mapping(address => VestingInfo)) _vestingInfo;
    mapping(address => mapping(address => uint256)) public userClaimedAmount;
    ...
    function initialize(address _slft, address _slgt) external initializer {
        __Ownable_init();
    ...
    function _getUserVestingInfo(
        address _beneficiary,
        address _vestingToken,
        uint256 _onDayOrToday
    ) internal view returns (VestingSummary memory) {
        VestingInfo storage userInfo = _vestingInfo[_beneficiary][_vestingToken];
        VestingSchedule storage userSchedule = _vestingSchedule[_beneficiary][
            _vestingToken
        ];
        uint256 grantedAmount = userInfo.amountOfGrant;
        (uint256 notVestedAmount, uint256 vestedAmount) = _getVestedInfo(
            _onDayOrToday,
            grantedAmount,
            userSchedule.startTime,
            userSchedule.cliffDuration,
            userSchedule.duration,
            userSchedule.unlockPercent
        );

        return
            VestingSummary({
                vestingToken: _vestingToken,
                amountOfGrant: grantedAmount,
                amountVested: vestedAmount,
                amountNotVested: notVestedAmount,
                amountClaimed: userClaimedAmount[_beneficiary][_vestingToken],
                startTime: userSchedule.startTime,
                cliffDuration: userSchedule.cliffDuration,
                duration: userSchedule.duration,
                unlockPercent: userSchedule.unlockPercent
            });
    }
    ...
    function updateVestingSchedule(
        ...
        require(userSchedule.startTime > 0 &&
            block.timestamp < userSchedule.startTime,
            "No vesting info or vesting started"
        );
        userSchedule.startTime = start;
        userSchedule.cliffDuration = cliff;
        userSchedule.duration = dur;
        userSchedule.unlockPercent = percent;
    ...
    function _applyVesting(
        ...
        require(beneficiary != address(0), "Cannot zero address");
        require(amount > 0, "Invalid amount");
        require(percent > 0 && percent <= BASE_PRECISION, "Invalid percent");
    ...
    function claim(address token) external {
        VestingInfo storage user = _vestingInfo[msg.sender][token];
        ...
        uint256 claimAmount = vestedAmount - claimedAmount;
        userClaimedAmount[msg.sender][token] += claimAmount;

        IERC20Upgradeable(token).transfer(msg.sender, claimAmount);
    ...
    function _effectiveDay(uint256 onDayOrToday)
        ...
        returns (uint256)
    {
        return onDayOrToday == 0 ? block.timestamp : onDayOrToday;
    }
    ...
    function _getVestedInfo(
        ...
        if (onDay < _startTime + _cliffDuration) {
            // None are vested (all are not vested)
            return (_grantedAmount, 0);
        }
        // If after end of cliff + vesting, then the not vested amount is zero (all are vested).
        else if (onDay >= _startTime + (_cliffDuration + _duration)) {
            // All are vested (none are not vested)
            return (0, _grantedAmount);
        }
        // Otherwise a fractional amount is vested.
        else {
            // Compute the exact number of seconds vested.
            uint256 secondsVested = onDay - (_startTime + _cliffDuration);
            uint256 monthsVested = secondsVested / SECONDS_PER_MONTH;

            vestedAmount =
                (_grantedAmount * (monthsVested * _unlockPercent)) /
                BASE_PRECISION;
