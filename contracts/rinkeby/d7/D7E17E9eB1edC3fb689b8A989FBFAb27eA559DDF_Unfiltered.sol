...
contract Unfiltered is ERC1155, Ownable {
	...
	string public name = "UNFILTERED";
	string public symbol = "UNFILTERED";
	...
	uint private maxMintAmount = 1000;
	uint private nftPerAddressLimit = 5000;
    ...
    constructor() ERC1155("http://137.184.20.41/unfilteredx/{id}.json") {
		supplies[BRONZE] = 100;
		supplies[SILVER] = 100;
		supplies[GOLD] = 100;
		supplies[PLATINUM] = 100;
    }

	function setNewUri(string memory uri) public onlyOwner
	{
		_setURI(uri);
	}
	...
	function mintNFT(uint _mintAmount, uint _pass, bytes32[] memory proof) public payable {
	
		require((_currentToken + _mintAmount) <= MAX_TOKENS, "Exceeded supply");
		require(msg.value >= CURR_MINT_COST * _mintAmount, "Insufficient funds");
		require(hasSaleStarted == true, "Sale hasn't started");
		require(_mintAmount <= supplies[_pass], "Supply exchausted for this pass");
		...
		supplies[_pass] -= _mintAmount;
		_mint(msg.sender, _pass, _mintAmount,"");
	}

	function getInformations() external view returns (string memory, uint[4] memory, uint, uint, uint, uint,uint,uint, bool,bool)
	{
		uint256[4] memory supplyRemaining;
		for (uint256 i = 0; i <= 3; i++) {
			supplyRemaining[i] = supplies[i];
		}

		return (CURR_ROUND_NAME,supplyRemaining,CURR_ROUND_SUPPLY,CURR_ROUND_TIME,CURR_MINT_COST,maxMintAmount,nftPerAddressLimit, totalSupply(), hasSaleStarted, onlyWhitelisted);
	}
   
	//only owner functions
	
	function setNewRound(uint _supply, uint cost, string memory RoundName, uint perTransactionLimit, uint perAddressLimit, uint theTime, bool isOnlyWhitelisted, bool saleState) external onlyOwner {
	...
	function Giveaways(uint numTokens, address recipient, uint _pass) public onlyOwner {
		require((_currentToken + numTokens) <= MAX_TOKENS, "Exceeded supply");
		require(numTokens <= supplies[_pass], "Supply exchausted for this pass");
		...
		supplies[_pass] -= numTokens;
		_mint(recipient, _pass, numTokens,"");
