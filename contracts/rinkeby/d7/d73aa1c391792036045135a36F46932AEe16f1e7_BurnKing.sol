...
contract BurnKing is ERC721Enumerable, Ownable {
  ...
  function burnERC20(bytes memory _signature, uint256 _tokenCount, uint256 _expiredDateInSeconds, uint256 _usdCost, uint8 _poolType, address _tokenContractAddress) external {
    ...
    require(tokenContractBurnable.allowance(msg.sender, address(this)) >= _tokenCount * 10 ** tokenDecimals, "Tokens for burning are not approved");
   ...
   function checkParamsVerification(bytes memory _signature, string memory _concatenatedParams) public view returns(bool) {
