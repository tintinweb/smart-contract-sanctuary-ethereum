...
contract BattleGround is ERC721URIStorage, VRFConsumerBase{
    ...
    event EmittedToken(uint indexed token, uint randomNumberToCharacter, uint indexChar, uint randomNumberToAddon, uint scaledNumber, uint indexed addon, uint indexed boxType);
        ...
        override
    {
        ...
        if(generatedAllNumbers == true)
        {
            ...
            emit EmittedToken(requests[index].tokenID, requests[index].randomNumberToCharacter, index_char,
                aux, requests[index].randomNumberToAddon, addons, uint(boxType));
