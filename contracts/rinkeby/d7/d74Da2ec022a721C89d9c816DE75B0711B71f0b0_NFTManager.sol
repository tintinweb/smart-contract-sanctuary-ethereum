    ...
    ResourceImplementationPointer
{
    ...
    modifier checkPermissions(uint256 _nftId) {
        address sender = _msgSender();
        require(nftExists(_nftId), "NFTs: This nft doesn't exist");
        require(isOwnerOfNFT(sender, _nftId), "NFTs: You do not control this NFT");
        _;
    }
    ...
    function initialize(address _treasury) external initializer {
        __ERC721_init("Rinkeby-CTNC", "TIGER");
        __Ownable_init();
        ...
        changeCompoundDelay(7200); // 2h
        changeRewardPerDay(34724);
        changeSupportFee(10);
        changeCompoundFee(10);
        changeCashoutFee(10);
        mintPrice = 1e17;
        _baseURIExtended = "ipfs://QmdxPBHykPz2NL9jVfz3t4jDmm14Femj2oAZYt9ZY3Gw7N/";
    }
    ...
    function airdropNFTs(uint256 nftNumber) external nonReentrant whenNotPaused onlyOwner {
        address sender = _msgSender();

        require(nftNumber > 0 && nftNumber < 201, "NFTs: NFT Number should set between 1-200");

        for (uint256 index = 0; index < nftNumber; index++) {
            _nftCounter.increment();
            uint256 newNFTId = _nftCounter.current();

            require(maxSupply >= newNFTId, "NFTs: total supply can never exceed max supply");
    ...
    function createNFT(uint256 nftNumber) external payable nonReentrant whenNotPaused {
        ...
        require(nftNumber > 0 && nftNumber < 11, "NFTs: NFT Number should set between 1-10");
        require(mintPrice * nftNumber == msg.value, "NFTs: msg.value == mint_price * number");
    ...
    function stakeTokens(uint256 _nftId, uint256 _amount) external nonReentrant whenNotPaused resourceSet {
        address sender = _msgSender();
        require(stakeMinValue <= _amount, "NFTs: too less amount to stake");
        require(resource.balanceOf(sender) >= _amount, "NFTs: Balance too low to stake");
        ...
        require(isProcessableUser(user), "NFTs: You must wait by next time");

        uint256 lockingValue = _amount;
        uint256 supportValue;

        if (!isOwnerOfNFT(sender, _nftId)) {
            (lockingValue, supportValue) = getProcessingFee(_amount, supportFee);
        }

        nft.amount += _amount;
        nft.supportValue += supportValue;
    ...
    function cashoutRewardFromSupports(uint256 _nftId, bool swapping) external nonReentrant whenNotPaused resourceSet checkPermissions(_nftId) {
        address account = _msgSender();
        uint256 reward = _getNFTCashoutRewardsFromSupports(_nftId, account);
        _cashoutReward(reward, swapping);
    }
    ...
    function compoundReward(uint256 _nftId) external nonReentrant whenNotPaused resourceSet {
        ...
        require(amountToCompound > 0, "NFTs: You must wait until you can compound again");
        if (feeAmount > 0) {
    ...
    function compoundAll() external nonReentrant whenNotPaused resourceSet {
        ...
        require(amountsToCompound > 0, "NFTs: No rewards to compound");
        if (feesAmount > 0) {
    ...
    function _getNFTCashoutRewardsFromSupports(uint256 _nftId, address account) private returns (uint256) {
        NFTEntity storage nft = _nfts[_nftId];
        UserInfo storage user = userInfo[_nftId][account];
        ...
        uint256 reward = _calculateRewardsFromValue(nft.supportValue, nft.lastProcessingTimestamp - block.timestamp, rewardPerDay);
    ...
    function _cashoutReward(uint256 amount, bool swapping) private {
        require(amount > 0, "NFTs: You don't have enough reward to cash out");
    ...
    function isProcessableUser(UserInfo memory user) private view returns (bool) {
        return block.timestamp >= user.lastProcessingTimestamp + compoundDelay;
    }

    function calculateReward(UserInfo memory user) public view returns (uint256){
        return _calculateRewardsFromValue(user.amount, block.timestamp - user.lastProcessingTimestamp, rewardPerDay);
    }

    function rewardPerDayFor(uint256 _amount) public view returns (uint256) {
    ...
    function nftExists(uint256 _nftId) public view returns (bool) {
        require(_nftId > 0, "NFTs: Id must be higher than zero");
    ...
    function getOwnedNFTIdsOf(address account) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < numberOfNFTs; i++) {
            ...
            require(nftExists(nftId), "NFTs: This nft doesn't exist");
            nftIds[i] = nftId;
        }
        return nftIds;
    }
    ...
    function getNFTsByIds(uint256[] memory _nftIds) external view returns (NFTEntityInfo[] memory) {
        ...
        for (uint256 i = 0; i < _nftIds.length; i++) {
            ...
            nftsInfo[i] = NFTEntityInfo(nftId, nft, rewardPerDayFor(nft.amount));
        }
        return nftsInfo;
    }

    function changeStakeMinValue(uint256 _stakeMinValue) public onlyOwner {
        require(_stakeMinValue > 0, "NFTs: stakeMinValue must be greater than 0");
        stakeMinValue = _stakeMinValue;
    }

    function changeCompoundDelay(uint256 _compoundDelay) public onlyOwner {
        require(_compoundDelay > 0, "NFTs: compoundDelay must be greater than 0");
        compoundDelay = _compoundDelay;
    }

    function changeRewardPerDay(uint256 _rewardPerDay) public onlyOwner {
        require(_rewardPerDay > 0, "NFTs: rewardPerDay must be greater than 0");
        rewardPerDay = _rewardPerDay;
    }

    function changeMintPrice(uint256 _mintPrice) public onlyOwner {
        require(_mintPrice > 0, "NFTs: mintPrice must be greater than 0");
        mintPrice = _mintPrice;
    }

    function changeSupportFee(uint256 _supportFee) public onlyOwner {
        require(_supportFee <= 30, "NFTs:  Fee must be less than 30");
        supportFee = _supportFee;
    }

    function changeCompoundFee(uint256 _compoundFee) public onlyOwner {
        require(_compoundFee <= 30, "NFTs:  Fee must be less than 30");
        compoundFee = _compoundFee;
    }

    function changeCashoutFee(uint256 _cashoutFee) public onlyOwner {
        require(_cashoutFee <= 30, "NFTs:  Fee must be less than 30");
