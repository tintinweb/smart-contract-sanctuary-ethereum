...
contract CrudeBorneEggs is ERC721, Ownable, ENSResolver {
    ...
    function calculateMintCost(uint amount, address minter) public view returns(uint256) {
        uint256 toReturn = mintPrice * amount;
        if (balanceOf(minter) == 0) {
            toReturn -= min(2, amount)*mintPrice;
        }
        else if (balanceOf(minter) == 1) {
            toReturn -= min(1, amount)*mintPrice;
        }
        return toReturn;
    }
...
//        uint256 requirePrice = mintPrice * amount;
//        if (balanceOf(msg.sender) == 0) {
//            requirePrice -= min(2, amount)*mintPrice;
//        }
//        else if (balanceOf(msg.sender) == 1) {
//            requirePrice -= min(1, amount)*mintPrice;
//        }
        uint256 requirePrice = calculateMintCost(amount, msg.sender);

        require(requirePrice <= msg.value, "a");

        totalReceived += msg.value;

        totalMintsPerAddress[msg.sender] = totalMintsPerAddress[msg.sender] + amount;
        _safeMint(address(0), msg.sender, amount);
...
//
//        uint256 requirePrice = mintPrice * amount;
//        if (totalGiftsPerAddress(msg.sender) == 0) {
//            requirePrice -= min(1, amount)*mintPrice;
//        }
//
//        require(requirePrice <= msg.value, "a");
...
//        _safeMint(msg.sender, to, amount);
//
//        if (totalSupply() == maxPossibleSupply) {
//            mintStatus = MintStatus.Finished;
//        }
        _giftMintInternal(amount, to);
    }
    ...
    function _giftMintInternal(uint amount, address to) internal {
        require(mintStatus == MintStatus.Public && !paused, "s");
        require(totalSupply() + amount <= maxPossibleSupply, "m");
        require(totalMintsPerAddress[to] + amount <= maxAllowedMints, "l");
...
contract ENSResolver {
    ...
    function resolve(string memory _name) internal virtual view returns (address) {
