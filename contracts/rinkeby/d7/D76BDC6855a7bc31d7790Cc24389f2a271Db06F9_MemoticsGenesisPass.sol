...
contract MemoticsGenesisPass is ERC721A, ERC721ABurnable, ERC721AQueryable, Ownable {
    using ECDSA for bytes32;
    /**
     * @dev The signer for the signatures. 
     * Randomly generated, fresh address, only for this contract.
     */
    address public constant SIGNER = 0x80C2A10eb26DE37Fe003baF21ba504ca4e033c69;

    /**
     * @dev The unrevealed URI.
     */
    string private _unrevealedURI;
    
    /**
     * @dev The revealed URI, in the format of "ipfs://<CID>/{id}.json", 
    ...
    constructor() ERC721A('MemoticsGenesisPass', 'MEMO') {
        maxSupply = 333;
        maxBatchSize = 9;
    }
    ...
    /**
     * @dev Add `quantity` to the per address specific mint count for `limitType`,
     * and check if the resultant value exceeds `limit`.
     */
    function _addToMintedAndCheckLimit(
        address minter,
        uint256 limitType,
        uint256 quantity,
        uint256 limit
        ...
        unchecked {
            uint256 aux = uint256(_getAux(minter));
            uint256 mask = (1 << 16) - 1;
            uint256 shift = 16 * limitType;
            uint256 minted = (aux >> shift) & mask;
            uint256 nextMinted = minted + quantity;
            aux ^= (minted ^ nextMinted) << shift;
            require(nextMinted <= limit, "No more slots.");
            _setAux(minter, uint64(aux));
        }
    }
        ...
        payable 
    {
        unchecked {
            ...
            if (state == 1) {
                _addToMintedAndCheckLimit(msg.sender, 0, quantity, limit);
                // For checking if whitelisted, and if the `limit` and `price` are correct.
                hash = keccak256(abi.encode(msg.sender, state, limit, price));
            } else if (state == 2) {
                _addToMintedAndCheckLimit(msg.sender, 1, quantity, limit);
                // For checking if whitelisted, and if the `limit` and `price` are correct.
                hash = keccak256(abi.encode(msg.sender, state, limit, price));
            } else if (state == 3) {
                _addToMintedAndCheckLimit(msg.sender, 2, quantity, limit);
                // For checking if the `limit` and `price` are correct.
                hash = keccak256(abi.encode(state, limit, price));
            } else {
                revert('Not open.');
            }
            ...
            require(msg.value == price * quantity, 'Wrong Ether value.');
        }
    ...
    function setSaleState(uint8 value) external onlyOwner {
        if (saleState != 0) {
            ...
            require(maxBatchSize != 0, "Max batch size not set.");
        }
        saleState = value;
    }
