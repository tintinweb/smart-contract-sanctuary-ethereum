...
contract IuvoDaoToken is ERC20, Ownable {
  ...
  uint256 private _taxDaoRewards = 10;
  uint256 private _taxLp = 10;
  uint256 private _totalTax;

  uint256 public earlySellMultiplier = 4;
  ...
  function launch(uint8 _supplyPercentLp) external payable onlyOwner {
  ...
  function _processFees(uint256 amountETH, uint256 amountLpTokens) private {
    uint256 daoRewardsETH = (amountETH * _taxDaoDonations) / _totalTax;
    uint256 daoDonationsETH = (amountETH * _taxDaoRewards) / _totalTax;
