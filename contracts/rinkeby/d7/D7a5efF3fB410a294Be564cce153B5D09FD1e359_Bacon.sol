...
contract Bacon is IBacon, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard, PaymentSplitter {
	...
	uint256 public WHITELIST_MINT_PRICE = .002 ether; // TODO: change to actual whitelist mint price
	// mint price
	...
	function delBuildNumber29() internal pure {} //etherscan trick
	...
	/**
	 * Public mint a token - 90% BACON, 10% COP BACON
	 * The first 20% are free to claim, the remaining cost $GREASE
	 */
	function publicMint(uint256 qty, uint256 _seed) external payable whenNotPaused nonReentrant {
		require(tx.origin == msg.sender, "no...");
		require(_tokenIds.current() + qty <= MAX_TOKENS, "All tokens minted");
		require(qty <= MAX_MINT_QTY, "Invalid mint qty");
