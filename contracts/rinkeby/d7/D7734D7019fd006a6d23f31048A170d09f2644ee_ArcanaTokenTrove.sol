...
contract ArcanaTokenTrove is ERC20("ARCANA", "RCANA"), Ownable, IDragonToken, Pausable, AccessControlEnumerable {
    ...
    // event LegionRateChange(uint256 legionRate);
    // event GenesisRateChange(uint256 genesisRate);
    ...
    function _calculateRewards(address _account) internal view returns(uint256) {
        ...
        if (delta > 0) {
            uint256 pendingBasic = dragons.balanceOf(_account) * (130864000000000 * delta * delta / 3301145202);
            uint256 pendingGenesis = genesis.balanceOf(_account) * (523455000000000 * delta / 3301145202);
