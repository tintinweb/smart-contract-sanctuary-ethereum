...
contract MonsterSuitNFT is ERC721A, Ownable
{
    ...
    uint16 public PRESALE_LIMIT = 10000;
    uint16 public presaleTokensSold = 0;
    uint256 public PRICE = 77700000000000000; //0.0777 eth
    ...
    function mintToken(uint16 amount, bytes32[] memory proof) external payable
    {
        ...
        require(!preSaleIsActive || addressMintedBalance[msg.sender] + amount <= PER_ADDRESS_LIMIT, "Max NFT per address exceeded");
        require(!whitelist || verify(proof), "Address not allowed at this time");
