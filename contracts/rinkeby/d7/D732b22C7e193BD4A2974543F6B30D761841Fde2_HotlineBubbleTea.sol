...
contract HotlineBubbleTea is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => bool) private _hidden;
    ...
    constructor() ERC721("Hotline Bubble Tea", "HBT") {
        _minter = msg.sender;
        _maxSupply = 254;
    }
    ...
    function batchMint(address to, uint256 num) external onlyMinter {
        require(
            totalSupply() + num <= _maxSupply,
            "Runtime error: exceeds max supply"
        );
        for (uint256 i = 0; i < num; i++) mint(to);
    }
        ...
        returns (string memory)
    {
        ...
        return _hidden[tokenId] ? _blindboxURI(tokenId) : _tokenURI(tokenId);
    }
