...
contract Testboard is ERC721 { 
    uint private constant TOKEN_LIMIT = 20; // limit
    string private constant SCRIPT = "let b = []; for (let i = 2; i < 66; i += 4) { b.push(hashString.slice(i, i + 5)); } let decRows = b.map(x => parseInt(x, 16)); let mask = 0x1F; let light = (decRows[0] & mask) > 3; let grid = decRows[8] & mask; let items = grid < 7 || grid > 16 ? 16 : grid; let colors = light ? 210 : 30; let border = 2; let itemBorder = 1 + (decRows[10] & 0x3) * 0.25; let scale, dimHeight, dimWidth; function setup() { dimHeight = (border * 2) + (20 * items); dimWidth = (border * 2) + (25 * items); scale = Math.min(window.innerWidth/dimWidth, window.innerHeight/dimHeight); createCanvas(dimWidth * scale, dimHeight * scale); background(colors); itemBorder *= scale; border *= scale; } function draw() { text(items, 90, 90); text(light, 120, 90);  text(itemBorder, 90, 120); noLoop(); }";
    ...
    address private _oceanAddress = 0x3f4C18d5D8b88C37A1CCDA31Cc3728743D6Cc983; // contract
    mapping (uint => bool) private _presaleId;
    
    constructor() ERC721("Testboard", unicode"⚄") {
        _platform = msg.sender;
        _imageBaseUrl = 'http://test.myartcontract.com/'; // base url
    ...
    function presaleIdUsed(uint tokenId) external view returns (bool) {
        return _presaleId[tokenId];
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (tokenId > 0 && tokenId <= _numTokens) {
            ...
            return StrUtil.strConcat('data:application/json,{"name":"Test No. ', Strings.toString(tokenId), '","description":"Test for Version 1 and Version 2.",', _urls, '"interaction":"None"}');
        } else {
            return "";
        }
    }
