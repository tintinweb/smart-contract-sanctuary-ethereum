...
import "./MetaCOMAccessCardLogic.sol";

contract MetaCOMAccessCardDutchAuctionMint is MetaCOMAccessCardLogic {

  constructor() MetaCOMAccessCardLogic() {}

  uint256 public startTime;
  uint256 public pauseTime;
  uint256 public pauseDuration;
  uint256 public startPrice;//max possible price
  uint256 public minPrice;//current price
  ...
  //price storage if modified
  mapping (address => MintLog[]) public alRefundDetails;
  uint256 public alRefundPool;
  bool public alRefundEnabled;
  
  /**
  @notice initialize dutch auction collection contract
  @param _startPrice start token price in wei for dutch auction
  @param _collectionConfig includes: name, symbol, baseURI, size
  @param _maxMint max tokens allowed to be minted per wallet address (public mint)
  ...
  */
  ...
  // returns token price depending on current timestamp
  function getPrice() public override view returns (uint256) {
    ...
    if(pauseTime > 0){
      _pauseDuration += block.timestamp - pauseTime;//pause
    }
  ...
  function setALRefundStatus(bool _alRefundEnabled) external onlyOwner {
    alRefundEnabled = _alRefundEnabled;
  }
  ...
  function _setPrice(uint256 _startPrice) internal {
    require(_startPrice > 0.01 ether, 'start price < 0.02');
    require(_startPrice <= 10 ether, 'start price > 10 ether');
    ...
    if(_startPrice >= 0.5 ether){
      ...
      require(_startPrice == 0.25 ether, 'not 0.25');
    }else{
  ...
  function mintAllowlist(bytes32[] calldata _merkleProof, uint256 mintAllocation, uint256 mintAmount) external override payable {
    _listMint(IMetaCOMCollection.DropType.allowlist, allowlistMerkleRoot, _merkleProof, mintAllocation, mintAmount);
    _txRefund(startPrice * mintAmount);//total sum for tokens minted

    uint256 refundALcount = getALrefundCount(msg.sender);
    if(refundALcount > 0 && alRefundDetails[msg.sender][refundALcount - 1].price == startPrice){
      alRefundDetails[msg.sender][refundALcount - 1].amount += mintAmount;
    }else{
      alRefundDetails[msg.sender].push(MintLog(startPrice, mintAmount));
    }
  ...
  function _afterTokenTransfers(
    ...
    if(from == address(0) && _totalMinted() == maxSupply()){//zero supply
      alRefundEnabled = true;
    }
  }

  //number of unclaimed allowlist mint transactions at unique price points
  function getALrefundCount(address userAddress) public view returns(uint256){
    return alRefundDetails[userAddress].length;
  }

  
  function getBalance() public override view returns (uint256){
    return address(this).balance - alRefundPool;//balance subtracting allowlist refund pool
  }

  //refund allowlist minters the difference betwen price paid and lowest dutch auction price
  function allowlistRefund(uint256 _stopIndex) external nonReentrant {
    require(alRefundEnabled, 'not yet enabled');

    uint256 refundALcount = getALrefundCount(msg.sender);
    require(refundALcount > 0, 'nothing to refund');
    
    for(uint256 c = refundALcount; c > _stopIndex; c--){
        MintLog memory mintLog = alRefundDetails[msg.sender][c-1];
        if(minPrice > 0 && mintLog.price > minPrice){
          uint256 transferToClient = (mintLog.price - minPrice)*mintLog.amount;
          payable(msg.sender).transfer(transferToClient);
        }
...
abstract contract MetaCOMAccessCardLogic is Ownable, ERC721A, Royalty, ReentrancyGuard {

  address factory;

  IMetaCOMCollection.AccessConfig private accessConfig;

  bytes32 public allowlistMerkleRoot;
  bytes32 public claimlistMerkleRoot;
  mapping(IMetaCOMCollection.DropType => mapping(address => uint256)) public mintList;

  bool public mintState;//public mint state
  bool public mintListState;//allowlist and claim list mint state
  uint256 public maxMint;

  IMetaCOMCollection.FeeConfig feeConfig;
  ...
  //openzepplin proxy clones
  constructor() ERC721A("", "") {}

  /**
  @notice initializes collection. Initiate onсe on deployment by factory.
  @param _collectionConfig includes: name, symbol, baseURI, size
  @param _maxMint maximum tokens allowed to be minted per wallet address (public mint)
  @param _allowlistMerkleRoot merkle tree root for allowlist
  @param _claimlistMerkleRoot merkle tree root for claim list
  @param _royaltiesConfig royalty info for nft marketplaces ERC2981
  @param _feeConfig fees recipient and percentage used in withdraw function 
  ...
  */
  ...
  function mint(uint256 quantity, uint256 timestamp, bytes memory signature) external payable {
    ...
    require(_numberMinted(msg.sender) + quantity <= maxMint, "wallet limit mint reached");
  ...
  function mintClaimlist(bytes32[] calldata _merkleProof, uint256 mintAllocation, uint256 mintAmount) external {
    _listMint(IMetaCOMCollection.DropType.Claimlist, claimlistMerkleRoot, _merkleProof, mintAllocation, mintAmount);
  }
  ...
  //withdraw funds from contract to defined wallet address (_to=0 sends to contract owner address)
  ...
  */
  ...
  function _isValidSignature(bytes32 hash, bytes memory signature) internal view returns (bool isValid) {
      ...
      return ECDSA.recover(signedHash, signature) == IMetaCOMCollection(factory).getPublicMintSigner();
  }
  ...
  function _validateRequest(uint256 timestamp, bytes memory signature) internal {
      ...
      require(usedMessages[msgHash] == false, "Duplicate message");//same hash can be used for dutch and fixed auction
  ...
  function _beforeTokenTransfers(
    ...
    if(from == address(0)){//mint
       require(_totalMinted() + quantity <= accessConfig.size, "reached max supply");
    }
  }

  /**
  @dev mints token for allowlist and claim list if mintListState true
  @param _merkleRoot merkle tree root for allowlist or claim list
  @param _merkleProof merkle proof array of wallets allowed to mint
  @param mintAllocation amount of tokens a wallet can mint
  @param mintAmount amount of available tokens to mint
  */
  function _listMint(
