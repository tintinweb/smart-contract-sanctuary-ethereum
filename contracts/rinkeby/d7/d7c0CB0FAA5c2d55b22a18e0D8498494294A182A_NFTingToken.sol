    ...
    Ownable
{
    ...
    ) ERC721("NFTing Token", "NFTK") {
        baseURI = _initBaseURI;

        MAX_SUPPLY = _maxSupply;
        MINT_PRICE = _mintPrice;

        _setDefaultRoyalty(_royaltyReceiver, (_feeDenominator() * 5) / 100);
    }
    ...
    function publicMint(uint256[] calldata _tokenIds)
        external
        payable
        isMaxBatchMintLimit(_tokenIds.length)
        isMaxSupplyLimit(_tokenIds.length)
    {
        if (msg.value < MINT_PRICE.mul(_tokenIds.length)) {
    ...
    ) external onlyOwner isMaxSupplyLimit(_tokenIds.length) {
        if (_tokenIds.length == 0) {
            revert InvalidArgumentsProvided();
        }
