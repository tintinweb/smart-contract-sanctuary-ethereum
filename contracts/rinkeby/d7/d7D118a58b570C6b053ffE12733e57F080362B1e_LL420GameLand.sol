...
contract LL420GameLand is Ownable, ERC721A, Pausable {
    using ECDSA for bytes32;

    uint256 public constant TOTAL_SUPPLY = 3000;
    uint256 public constant MINT_AMOUNT = 1;

    string public constant NAME = "420.game Game Land Plot";
    string public constant SYMBOL = "420GLP";

    string private _baseTokenURI;
    address private _validator;

    /// @dev address => timestamp to avoid multiple mint
     ...
     */
    constructor(string memory _tokenURI, address _account) ERC721A(NAME, SYMBOL) {
        _baseTokenURI = _tokenURI;
        _validator = _account;
    }

    /**
     * @dev This function allows to mint Game Land plot item
     */
    function mint(
        address _to,
        uint256 _timestamp,
        ...
        require(_verify(_to, _timestamp, _signature), "Not verified");
        require(_totalMinted() < TOTAL_SUPPLY, "Reached max supply");
        require(timestamps[_to] != _timestamp, "Already minted");

        timestamps[_to] = _timestamp;
        
        _safeMint(_to, MINT_AMOUNT);

        emit Mint(_to, _currentIndex);
    }
    ...
    function _verify(
        ...
        bytes32 signedHash = keccak256(abi.encodePacked(_to, _timestamp));
