...
contract ERC721Minter is Base {
    ...
    function setPaymentToken(address newPaymentToken) external onlyOwner(msg.sender) {
        require(address(paymentToken) != newPaymentToken, "new_payment_token");

        paymentToken = IERC20(newPaymentToken);

        emit AddressPropertyUpdated("PaymentToken", newPaymentToken);
    }
    ...
    function mint(address token, uint256 total) external onlyWhitelisted {
        ...
        uint256 paymentAllowance = paymentToken.allowance(msg.sender, address(this));
        uint256 totalPrice = info.pricePerUnit.mul(total);
        require(totalPrice <= paymentAllowance, "!approval_enough");
        info.mintedItems = info.mintedItems.add(total);

        paymentToken.safeTransferFrom(msg.sender, paymentReceiver, totalPrice);
