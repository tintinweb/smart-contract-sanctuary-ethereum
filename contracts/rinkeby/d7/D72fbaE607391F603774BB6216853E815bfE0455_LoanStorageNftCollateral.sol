...
/// @notice one-to-one mapping with loan storage, nft information is stored in this contract
contract LoanStorageNftCollateral is Ownable {
    ...
    /// @dev collateralMapping[loanId]
    mapping(uint256 => LoanCommon.NftCollateral) public collateralMapping;

    /// @notice loan collateral activities
    enum NftCollateralEvent {
        CREATED     // 0
    }

    /// @notice loan nft collateral event
    event NftCollateralActivity(uint256 indexed loanId, NftCollateralEvent activity, uint256 timestamp);
    ...
    modifier onlyServiceContractOrOwner() {
        ...
        require(
            msg.sender == loanServiceNftCollateralAddress || msg.sender == owner(),
            "caller must be service contract or owner"
        );
        _;
    }

    /// @notice get collateral object by loan id
    /// @param _loanId The collateral id
    function collaterals(uint256 _loanId) public view returns(LoanCommon.NftCollateral memory) {
        return collateralMapping[_loanId];
    }

    /// @notice create loan collateral
    ...
    ) public onlyServiceContractOrOwner {

        require(
            _nftContracts.length == _nftTokenIds.length &&
            _nftContracts.length == _nftTokenAmounts.length, "all nft arrays need to be the same size");

        collateralMapping[_loanId] = LoanCommon.NftCollateral({
            loanId : _loanId,
            nftContracts: new address[](0),
            nftTokenIds: new uint256[](0),
            nftTokenAmounts: new uint256[](0),
            standards: new LoanCommon.Standard[](0),
            nftEstimatedValue: _nftEstimatedValue,
            createdDate : block.timestamp
        });

        for (uint i = 0; i < _nftContracts.length; i++) {
            collateralMapping[_loanId].nftContracts.push(_nftContracts[i]);
            collateralMapping[_loanId].nftTokenIds.push(_nftTokenIds[i]);
            collateralMapping[_loanId].nftTokenAmounts.push(_nftTokenAmounts[i]);
            collateralMapping[_loanId].standards.push(_standards[i]);
        }

        // Emit payment event
        emit NftCollateralActivity(_loanId, NftCollateralEvent.CREATED, block.timestamp);
    }
}
