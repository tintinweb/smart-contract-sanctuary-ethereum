...
contract PRBLYGenesisNFT is ERC2981, ERC721, AccessControlEnumerable, Ownable {
    ...
    // EIP2981 properties
    address royaltyAddr = 0x51AaE7357c8baD10DB3532e9AC597efFA5C3820f; //TODO: Replace this with a PN wallet
     ...
     *  @param merkleroot The merkle root for the allowed minters
     */
    constructor(
        string memory name,
    ...
    function mint(bytes32[] calldata proof, uint256 id) external {
        ...
        require(_tokenIds[id] == false, "Token already minted");
    ...
    function ownerMintBlock(address account, uint256 firstId, uint256 lastId) external onlyOwner {
        for (uint256 i = firstId; i < _tokenIds.length && i < lastId; i++) {
            if (_tokenIds[i] != true) {
                _mintOne(account, i);
            }
        }
