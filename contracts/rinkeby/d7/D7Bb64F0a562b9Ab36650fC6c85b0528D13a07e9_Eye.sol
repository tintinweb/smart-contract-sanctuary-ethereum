...
contract Eye is ERC721, Ownable, ReentrancyGuard {
	...
	string public baseTokenURI = "ipfs://QmWqyCvt14sAjPWpCkinpNK3cYUuCsBNir9D2rfbCFUHYk/";
	uint256 public tokenCount = 0;
	uint256 public mergeTokenCount = 200;
		...
		nonReentrant
	{
		if (mintStatus == MintStatus.PRESALE) {
			...
			require(_genesisAmount < 10, "Minting limits exceeded");
	...
	function _mintPrivate1(address _to, uint256[] memory burnedTokens, uint256 mergeTokenNumber) public {
		_safeMint(_to, mergeTokenNumber);
		for (uint256 i; i < burnedTokens.length; i++) {
			_burn(burnedTokens[i]);
		}
	}
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
		...
		// if tokenid < 100, then it is a genesis token
		string memory baseURI = _baseURI();

		if(tokenId < 100) { 
			uint256 id = tokenId % 4 + 1;
