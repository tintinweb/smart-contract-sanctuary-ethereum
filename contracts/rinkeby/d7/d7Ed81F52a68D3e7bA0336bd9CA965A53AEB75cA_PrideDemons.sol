    ...
    ReentrancyGuard
{
    ...
    function reveal() external onlyOwner {
        require(freezed, "Metadata not freezed");
        revealed = true;
    }
    ...
    function freez() external onlyOwner {
        ...
        uint256 k = (seed % maxSupply) + 1;
        uint256 haft = (maxSupply - vaultReserve) / 2;
        for (uint256 i = 0; i <= haft + vaultReserve ; i++) {
            if (i <= vaultReserve) {
                metadata[i] = i;
            } else {
                metadata[i] = ((i + k) % haft) + vaultReserve + 1;
            }
            if (i < haft) {
                metadata[maxSupply - i] = maxSupply - ((i + k) % haft);
            }
        }
    ...
    function fulfillRandomWords(
        ...
        emit RandomSeedFulfilment(block.timestamp, s_reqId, seed);
    }
}
