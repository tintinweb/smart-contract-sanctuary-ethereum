...
contract FurBetPresale is BaseContract
{
    ...
    function presale(uint256 amount_) public whenNotPaused
    {
        ...
        uint256 _tokenAmount_ = amount_ * (10 ** 18);
        address _stakeAddress_ = addressBook.get("furbetstake");
        _token_.mint(address(this), _tokenAmount_);
        _token_.approve(_stakeAddress_, _tokenAmount_ / 4);
        _stake_.stakeFor(msg.sender, 1, _tokenAmount_ / 4);
        _token_.approve(_stakeAddress_, _tokenAmount_ / 4);
        _stake_.stakeFor(msg.sender, 2, _tokenAmount_ / 4);
        _token_.approve(_stakeAddress_, _tokenAmount_ / 4);
        _stake_.stakeFor(msg.sender, 3, _tokenAmount_ / 4);
        _token_.approve(_stakeAddress_, _tokenAmount_ / 4);
        _stake_.stakeFor(msg.sender, 4, _tokenAmount_ / 4);
    }
