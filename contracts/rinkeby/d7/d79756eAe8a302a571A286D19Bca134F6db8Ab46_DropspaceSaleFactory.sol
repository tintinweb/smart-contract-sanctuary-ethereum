...
contract DropspaceSaleFactory is IDropspaceSaleFactory, Ownable {
    ...
    uint256 public override flatFee = 0.001 ether;
    uint256 public override percentageFee = 2000;

    address payable public override revenueWallet;

    bool public override autoWithdraw;

    mapping(uint256 => address) public override sales;

    constructor(address payable _revenueWallet) {
        setRevenueWallet(_revenueWallet);
    }
    ...
    function setRevenueWallet(address payable _revenueWallet) public override onlyOwner {
        revenueWallet = _revenueWallet;
        emit RevenueWalletChanged(revenueWallet);
    }

    function setAutoWithdraw(bool _autoWithdraw) external override onlyOwner {
        autoWithdraw = _autoWithdraw;
        emit AutoWithdrawChanged(autoWithdraw);
    }
    ...
    function withdrawFromSale(uint256 _saleID) external override onlyOwner {
        IDropspaceSale(payable(sales[_saleID])).withdraw();
    }

    function setDevWalletForSale(uint256 _saleID, address payable _devWallet) external override onlyOwner {
        IDropspaceSale(payable(sales[_saleID])).setDevWallet(_devWallet);
    }

    function setDevShareForSale(uint256 _saleID, uint256 _devSaleShare) external override onlyOwner {
        IDropspaceSale(payable(sales[_saleID])).setDevSaleShare(_devSaleShare);
    }
    ...
    function createSale(
        ...
        require(msg.value >= flatFee, "DropspaceSaleFactory:: Flat Fee must be paid for creating the sale");

        DropspaceSale sale = new DropspaceSale(_supplyLimit, _mintLimit, _mintPrice,
            percentageFee, _withdrawalWallet, revenueWallet, _ticketAddress, _name, _ticker, _baseURI);
