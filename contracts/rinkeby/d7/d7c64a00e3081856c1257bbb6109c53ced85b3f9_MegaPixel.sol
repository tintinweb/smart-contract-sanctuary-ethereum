...
contract MegaPixel is ERC721Enumerable, Ownable{
    ...
    constructor() ERC721("MegaPixel", "MPIX") {
        _baseTokenURI = "https://megapix.ngrok.io/meta/?format=json&id=";
    }
    ...
    function checkBlockAvailable(uint256 x_cord, uint256 y_cord) public view returns (bool){
        ...
        address x_address = locations[key];
        if (x_address == 0x0000000000000000000000000000000000000000){
            return true;
        }
        return false;
    }
    ...
    function mint(address to, uint256 tokenId, uint256 x_cord, uint256 y_cord) public virtual onlyOwner{
        ...
        require(checkBlockAvailable(x_cord, y_cord),     "MegaPixelNFT: This Location has already been taken");
        string memory key = string(abi.encodePacked(x_cord,y_cord));
        locations[key] = to;
        _mint(to, tokenId);
    }
    ...
    function purchasePixelBlock(uint256 tokenId, uint256 x_cord, uint256 y_cord) public payable{
        uint256 supply = totalSupply();
        require( !_paused,                          "Currently Not for sale." );
    ...
    function walletOfOwner(address _owner) public view returns(uint256[] memory) {
        ...
        uint256[] memory ppIds = new uint256[](ppCount);
        for(uint256 i; i < ppCount; i++){
            ppIds[i] = tokenOfOwnerByIndex(_owner, i);
        }
        return ppIds;
    }
    ...
    function withdrawAll() public payable onlyOwner{
        ...
        require(payable(devid1).send(_each));
    }
