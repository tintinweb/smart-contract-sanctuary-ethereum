...
contract TradeCoinERC721 is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, RoleControl {
    ...
    mapping(uint => TradeCoin) public tradeCoin;
    // Optional mapping for token URIs
    mapping (uint256 => string) private _tokenURIs;

    mapping(uint => address) public addressOfNewOwner;
    mapping(uint => uint) public priceForOwnership;
    mapping(uint => bool) public paymentInFiat;

    // Definition of Events
    event productPending(uint indexed tokenId, address indexed legalOwner);

    event productCreated(
        uint indexed tokenId,
        address indexed legalOwner,
    ...
    event productSplit(
        ...
        address indexed sender,
        uint[] notIndexedTokenIds
    );

    event productBatched(
        uint[] indexed tokenIds,
        address indexed sender,
        uint[] notIndexedTokenIds
    );
    ...
    event productBurned(address indexed productConsumer, uint indexed tokenId);
    ...
    modifier atState(State _state, uint tokenId) {
        require(tradeCoin[tokenId].state == _state, "ProductNFT not in the right status");
        _;
    }

    modifier notAtStatus(State _state, uint tokenId) {
        require(tradeCoin[tokenId].state != _state, "ProductNFT can not in the current status");
        _;
    }

    modifier onlyNFTOwner(address sender, uint tokenId) {
        require(ownerOf(tokenId) == sender, "Sender is not owner of NFT.");
        _;
    }

    modifier isLegalOwnerOrCurrentHandler(address sender, uint _tokenId){
        require(tradeCoin[_tokenId].currentHandler == sender || ownerOf(_tokenId) == sender, "Given address is not the Owner nor current Handler.");
        _;
    }

    modifier isOwnerOrApproved(address sender, uint tokenId){
    ...
    function initialTokenization(
        ...
        require(keccak256(abi.encodePacked(commodity)) != keccak256(abi.encodePacked("")), 
        ...
        // Store data on-chain
        tradeCoin[id] = TradeCoin(commodity, weight, state, _msgSender(), tempArray, "");

        _setTokenURI(id, id.toString());
        // Fire off the event
        emit productPending(id, msg.sender);
    }
    ...
    function initiateCommercialTx(
        ...
        priceForOwnership[_tokenId] = priceInEth * 1e18;
        addressOfNewOwner[_tokenId] = newOwner;
        paymentInFiat[_tokenId] = payInFiat;
        tradeCoin[_tokenId].rootHash = _rootHash;

        emit offering(_tokenId, _msgSender(), stringToBytes32(_docHash), _docType, _rootHash, payInFiat);
    }

    // warehouse to product handler
    function approveTokenization(uint _tokenId) external payable 
        ...
        atState(State.PendingCreation, _tokenId)
    {
        ...
        emit productCreated(_tokenId, addrOwner, msg.sender);
    }
    ...
    function addTransformation(
        ...
        require(weightLoss > 0 && weightLoss <= tradeCoin[_tokenId].weight, 
            "Altered weight can't be 0 nor more than total weight"
        );

        tradeCoin[_tokenId].transformations.push(_transformationCode);
        uint newWeight = tradeCoin[_tokenId].weight - weightLoss;
        tradeCoin[_tokenId].weight = newWeight;
        tradeCoin[_tokenId].rootHash = _rootHash;
        
        emit transformation(_tokenId, _msgSender(), stringToBytes32(_docHash), _docType, _rootHash, newWeight, _transformationCode);
    }
    ...
    function changeStateAndHandler(
        ...
        emit handlerAndStateUpdate(_tokenId, _msgSender(), stringToBytes32(_docHash), _docType, _rootHash, _newState, _newCurrentHandler);
    }

    function burn(uint _tokenId) public virtual override(ERC721Burnable) 
    ...
    function splitProduct(
        uint _tokenId, 
        uint[] memory partitions
    ) public onlyNFTOwner(_msgSender(), _tokenId) notAtStatus(State.PendingCreation, _tokenId) {
        require(partitions.length <= 3 && partitions.length > 1 , "Token should be split to 2 or more new tokens, we limit the max to 3.");
        // create temp list of tokenIds
        uint[] memory tempArray = new uint[](partitions.length + 1);
        tempArray[0] = _tokenId;
        // create temp struct
        TradeCoin memory temporaryStruct = tradeCoin[_tokenId];

        uint sumPartitions;
        for(uint x; x < partitions.length; x++){
            require(partitions[x] != 0, "Partitions can't be 0");
            sumPartitions += partitions[x];
        }

        require(tradeCoin[_tokenId].weight == sumPartitions, "The given amount of partitions do not equal total weight amount.");

        burn(_tokenId);
        for (uint i; i < partitions.length; i++){
        ...
        emit productSplit(_tokenId, tempArray, _msgSender(), tempArray);
    ...
    function batchProduct(
        ...
        require(_tokenIds.length > 1 && _tokenIds.length <= 3, "Maximum batch: 3, minimum: 2");

        uint cummulativeWeight;
        ...
        string[] memory transformations = tradeCoin[_tokenIds[0]].transformations;
        ...
        uint[] memory tempArray = new uint[](_tokenIds.length + 1);

        for(uint tokenId; tokenId < _tokenIds.length; tokenId++){
            require(ownerOf(_tokenIds[tokenId]) == _msgSender(), "Unauthorized: The tokens do not have the same owner.");
            require(hashed == keccak256(abi.encode(
        ...
        emit productBatched(tempArray, _msgSender(), tempArray);
    }
    ...
    function mintUniqueProductBatch(
        ...
        require(keccak256(abi.encodePacked(_commodity)) != keccak256(abi.encodePacked("")), 
            "Commodity needs to have a value."
        );
        ...
        _safeMint(_msgSender(), id);
        // Store data on-chain
        tradeCoin[id] = TradeCoin(_commodity, _weight, _state, currentHandler, transformations, "");
    ...
    function finishCommercialTransaction(uint _tokenId) public payable {
        require(
            addressOfNewOwner[_tokenId] == msg.sender,
            "You don't have the right to pay"
        );
        ...
        // When not paying in Fiat pay but in Eth
        if(!paymentInFiat[_tokenId]){
            require(priceForOwnership[_tokenId] != 0, "This can't be sold for 0 eth");
            payable(addrOwner).transfer(msg.value);
        }
        // else transfer
        _transfer(addrOwner, msg.sender, _tokenId);
    ...
    function addInformation(uint _tokenId, string memory _docHash, string memory _docType, string memory _rootHash) public onlyInformationHandlerOrAdmin {
        tradeCoin[_tokenId].rootHash = _rootHash;

        emit rootHashUpdate(_tokenId, stringToBytes32(_docHash), stringToBytes32(_docType), stringToBytes32(_rootHash));
    }

    function getISObyIndex(uint _tokenId, uint _isoIndex)
        public
        view
    ...
    function getISOLength(uint _tokenId) public view returns (uint) {
        return tradeCoin[_tokenId].transformations.length;
    }

    function getOwnedTokens(address _owner) public view returns (uint[] memory){
        uint totalTokens = balanceOf(_owner);
        uint[] memory allOwnedTokens = new uint[](totalTokens);

        for(uint token; token < totalTokens; token++){
...
contract RoleControl is AccessControl {
    ...
    function isInformationHandlerOrAdmin(address account) public virtual view returns(bool) {
        return (hasRole(INFORMATION_HANDLER_ROLE, account) || hasRole(INFORMATION_HANDLER_ROLE, account));
    }
    ...
    modifier onlyInformationHandlerOrAdmin() {
        require(isInformationHandlerOrAdmin(msg.sender), "Restricted to ProductHandlers or admins.");
        _;
    }
