...
contract Auction {
    ...
    function cancelOfferPvP(
        ...
        processCancel(Offers[_nftId], _subOfferId);
    ...
    function cancelOfferAuction(
        ...
        processCancel(auctionIdToAuction[_auctionId].offers, _subOfferId);
        if (!whiteList[msg.sender] || _isWeb)
...
interface IAuction {
    ...
    function setTokenStan(address _tokenStan) external;

    function setWhileList(address _user) external;
    ...
    function setStanNFT(address _stanNFT) external;

    function setStanAddress(address _stanAddress) external;

    function setCollectionNFTAddress(address _collectionNFTAddress) external;

    function setFeeSystem(uint256 _stanFee, uint256 _serviceFee) external;
