...
contract Trade {
    ...
    struct Order {
        ...
        address nftAddress;
        bool inEth;
        uint ethPrice; //it is divided by 1000 in the solidity code. Eg: If you have to send 0.1 Eth then you have to pass (0.1*1000) = 100 as a parameter for the struct
    ...
    function isVerifiedOrder(Order memory order, Sign calldata sign) public view {
        bytes32 hash = keccak256(abi.encodePacked(this, order.seller, order.buyer, order.nftAddress, order.inEth, order.ethPrice, order._orderType, order.amount, order.tokenId, order.qty, sign.nonce));
    ...
    */
    function excuteOrder(Order memory order, Sign calldata sign) external payable returns(bool) {
        require(!usedNonce[sign.nonce],"Nonce : Invalid Nonce");
        if (order.inEth) {
            uint price =  (order.ethPrice);
