  ...
  function generateTitle(uint256 _tokenId, Params.Character[][8] memory _characters, uint8[8] memory _charactersIds) internal pure returns (bytes memory) {
    ...
    return
      bytes.concat(
        ...
        "</g>",
        "<g>",
        abi.encodePacked('<text class="colB" style="font-size:28px;font-weight:bold;font-family: \'Courier New\',monospace;" text-anchor="middle" transform="translate(768,840)">', Strings.toString(_tokenId),'</text>'),
        "</g>",
        "</g>",
        "</g>"
      );
