...
// File: contracts/JargoNFTs.sol


pragma solidity ^0.8.4;

contract test222 is ERC721A, Ownable {

    uint256 constant public maxSupply = 2525;
    uint256 public freeSupply = 0;
    uint256 public mintPrice = 0.25 ether;
    uint256 constant public maxMint = 2;
    uint256 constant public MaxMintPerWallet = 20; 
    uint256 constant public MaxFreeMintPerWallet = 2;

    bool public isSaleLive = true;
    string public baseURI = "ipfs://..insert_cdi_here../";
     mapping(address => uint) public addressClaimed;

    constructor() ERC721A("test222", "222") {}

    function mint(uint256 quantity) external payable {
        require(isSaleLive, "Sale is not live");
        require(quantity > 0 && quantity <= maxMint, "Invalid quantity");
        uint256 supply = totalSupply();
        require(supply + quantity <= maxSupply, "Max amount of tokens minted");
        require (addressClaimed[_msgSender()] < MaxFreeMintPerWallet || msg.value >= quantity * mintPrice, "Free mint exceeded or not enough eth sent");
        require(addressClaimed[_msgSender()] < MaxMintPerWallet, "You have already received your Tokens");
