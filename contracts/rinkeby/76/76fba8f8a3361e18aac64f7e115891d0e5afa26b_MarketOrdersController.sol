...
// File: contracts\MarketOrdersController.sol
...
/// @dev Contract use UUPS proxy.
contract MarketOrdersController is
    Initializable,
    UUPSUpgradeable,
    ...
    PausableUpgradeable
{
    ...
    mapping(address => mapping(uint256 => MarketOrder)) public marketOrders;
    ...
    function initialize(address _swapRouter, address _uniswapV3Factory)
        public
        initializer
    {
    ...
    function createMarketOrder(CreateMarketOrderParams memory _params)
        ...
        returns (uint256)
    {
        require(_params.deadline >= block.timestamp, "Deadline expired");
        ...
        if (!AddressUpgradeable.isContract(poolAddress)) {
        ...
        require(
            AddressUpgradeable.isContract(poolAddress),
            "Pool does not exist"
        );

        IERC20Upgradeable(_params.tokenIn).transferFrom(
            _msgSender(),
            address(this),
            _params.amountIn
        );
        IERC20Upgradeable(_params.tokenIn).approve(
            address(swapRouter),
            _params.amountIn
        );
        ...
        marketIdCounter++;
        marketOrders[poolAddress][marketIdCounter] = MarketOrder({
            tokenIn: _params.tokenIn,
            tokenOut: _params.tokenOut,
            fee: _params.fee,
            amountIn: _params.amountIn,
