...
/**
 * @title ProjectPupCult wrapper contract
 */
contract MCIncoGnito is MCGenesisSplits, MCGenesisBase {
    ...
    constructor()
        MCGenesisBase(
            'MCIncoGnito',
            'MCIG',
            'https://gateway.pinata.cloud/ipfs/QmQm4PhUnkneEojbKBPMAgsNXt9YgBHLcS58FqHPRinfz4/', //pre-reveal rinkeby ipfs V1B
            addresses,
            splits
        )
    {
    ...
    {
        return string(abi.encodePacked(super._tokenFilename(tokenId), TOKEN_URI_POSTFIX));
    }
}
    ...
    MerkleLeaves
{
    ...
    uint256 private constant PUBLIC_PHASE = 3;

    uint256 public claimPricePerNft;
    uint256 public presalePricePerNft;
    uint256 public publicPricePerNft;
    ...
    function setMintingState(
        ...
        tempControlFlags = tempControlFlags.setBoolean(PUBLIC_PHASE, __publicActive);
        ...
        if (__publicPricePerNft > 0) {
            publicPricePerNft = __publicPricePerNft;
        }
    }
    ...
    function isPublicActive() external view returns (bool) {
        return mintControlFlags.getBoolean(PUBLIC_PHASE);
    }
    ...
    function claimTokens(bytes32[] calldata proof, uint256 count) external payable nonReentrant {
        ...
        _claimTokens(_msgSender(), proof, count);
    }

    function presaleTokens(bytes32[] calldata proof, uint256 count) external payable nonReentrant {
        ...
        _presaleTokens(_msgSender(), proof, count);
    }
     ...
     * @param count the number of tokens to mint.
     */
    function mintTokens(uint256 count) external payable nonReentrant onlyUsers {
        require(mintControlFlags.getBoolean(PUBLIC_PHASE), 'Minting stopped');
        require(0 < count && count <= MAX_MINT_BATCH_SIZE, 'Invalid count');
        require(publicPricePerNft * count == msg.value, 'Invalid price');
...
contract MCGenesisSplits {
    address[] internal addresses = [
        0xc7377DE253149BF75f89c03BF52F953989bBe5E8
    ];
