...
contract StarDust is ERC20, Ownable, Pausable, ReentrancyGuard {
    IERC721 public immutable TRAINERS;
    ...
    constructor(address trainersaddress) ERC20("StarDust", "SDUST") {
        TRAINERS = IERC721(trainersaddress);
    }
    ...
    modifier isApprovedForAll() {
        require(
            TRAINERS.isApprovedForAll(msg.sender, address(this)),
            "Contract not approved"
        );
        _;
    }

    /// @notice Get Trainers token ID by account and index
    ...
    function getOwnedByIndex(address account, uint256 index)
        public
        view
        returns (uint256)
    {
    ...
    /// @notice Get amount of claimable SDUST tokens
    ...
    function _getPending(address account) internal view returns (uint256) {
        return
            (stakedTokens[account] *
                BASE_RATE *
                (block.timestamp - lastUpdate[account])) / 1 days;
    }
    ...
    /// @notice Claim available SDUST tokens
    ...
    {
        require(
            tokenIds.length <= MAX_PER_TX,
            "Exceeds max tokens per transaction"
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                TRAINERS.ownerOf(tokenIds[i]) == msg.sender,
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            TRAINERS.transferFrom(msg.sender, address(this), tokenIds[i]);
        }
    }

    /// @notice Remove Trainers tokens and optionally collect SDUST tokens
    /// @param tokenIds The tokens IDs to unstake
    /// @param claimTokens Whether SDUST tokens should be claimed
    function unstake(uint256[] calldata tokenIds, bool claimTokens)
        external
        nonReentrant
    {
        require(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 last = ownedTokens[msg.sender][
                stakedTokens[msg.sender] - 1
            ];
            ...
            TRAINERS.transferFrom(address(this), msg.sender, tokenIds[i]);
        }
    }

    /// @notice Burn a specified amount of SDUST tokens
    ...
    function recoveryTransfer(address to, uint256 tokenId) external onlyOwner {
        require(
            ...
            "Token was not transferred accidentally"
        );

        TRAINERS.transferFrom(address(this), to, tokenId);
    }
}