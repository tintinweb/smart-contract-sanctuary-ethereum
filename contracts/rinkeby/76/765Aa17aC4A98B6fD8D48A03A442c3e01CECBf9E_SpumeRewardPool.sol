...
contract SpumeRewardPool is ReentrancyGuard, Ownable, Pausable {
  ...
  uint256 public rewardTokenAvailable;
  ...
  constructor(address stakingToken, address rewardsToken, address Weth, address _spumeStaking) { 
    ...
    lastUpdateTime = spumeStaking.getCreatedAt();
    depositor[msg.sender] = true; 
  }

  //Modifiers 
  modifier availableRewardToken(){
    rewardTokenAvailable += (block.timestamp - lastUpdateTime) * rewardRate; 
  ...
  /*
   * @dev returns the circulating supply of the reward token
   */
  function getRewardTokenCirculatingSupply() internal whenNotPaused availableRewardToken() returns (uint256){
    return rewardTokenAvailable - rewardTokenBalance;
  }

  /*
   * @dev Calculates current rewardAmount per reward Token. 
   */ 
  function rewardAmountCalculation() internal whenNotPaused returns (uint256) {
    rewardPerToken = (poolDistributionTotal / getRewardTokenCirculatingSupply()); 
   ...
   */
  function pauseRewardPool() public onlyOwner whenNotPaused {
    _pause(); 
  }
   ...
   */
  function unPauseRewardPool() public onlyOwner whenPaused {
    _unpause(); 
  }
   ...
   */
  function EmergencyTransferWETH(uint256 amount) public onlyOwner whenPaused {
    poolDistributionTotal -= amount;
...
contract SpumeStaking is ReentrancyGuard, Ownable, Pausable { 
     ...
     */ 
    function getStakedTotalSupply() public view returns (uint256) {
     ...
     * @notice Pauses Contract 
     */
    function pauseSpumeStaking() public onlyOwner whenNotPaused {
        _pause(); 
    }
     ...
     * @notice Unpauses Contract 
     */
    function unPauseSpumeStaking() public onlyOwner whenPaused {
        _unpause(); 
    }
