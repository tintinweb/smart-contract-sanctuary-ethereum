...
contract SeaGoldTrade is Initializable, OwnableUpgradeable {
    ...
    function initialize(address _buybackAdminAddress, uint256 _serviceValue,address _tradeRewardsToken)
        public
        initializer
    {
        __Ownable_init();
    ...
    function updatebuybackAdminAddress(bool _adminApproval) public onlyOwner {
    ...
    function cancelOrder(uint256 tokenId) public isMarketApproved {
        delete order_place[msg.sender][tokenId];
        emit CancelOrder(msg.sender, tokenId);
    }

    function changePrice(uint256 value, uint256 tokenId)
        public
        isMarketApproved
    {
    ...
    function saleToken(
        ...
        _saleToken(from, amount, "BNB", royaltyAddress, royaltyPercentage);
        if (nftType == 721) {
    ...
    function acceptBId(
        ...
        tradePerDay[memorydateStake] = tradePerDay[memorydateStake] + amount;
    }
