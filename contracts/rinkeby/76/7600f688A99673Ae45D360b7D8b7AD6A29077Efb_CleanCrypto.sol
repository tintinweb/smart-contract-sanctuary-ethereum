...
contract CleanCrypto is ERC721URIStorage, Ownable {
  ...
  constructor() ERC721 ("CleanCrypto", "CLC") {
    console.log("NftFactory constructor");
  ...
  function createNft(string memory tokenURI) public payable returns (uint256) {
    ...
    // require that amount paid is at least the minting price
    require(msg.value >= mintingPrice, "Insufficient amount");
    ...
    _createdTokens[msg.sender].push(newItemId);
  ...
  function getCreatedTokens(address owner) public view returns (uint256[] memory, string[] memory) {
    uint256[] memory tokenIds = _createdTokens[owner];
