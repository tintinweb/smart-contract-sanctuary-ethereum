    ...
    IMintableERC721
{
    ...
    ) ERC721(name, symbol) AccessControlMintable() {
        require(administrator != address(0), "Invalid administrator address");
        require(developer != address(0), "Invalid developer address");
        require(royaltyReceiver != address(0), "Invalid receiver address");
        _contractURI = contractURI_;
        _tokenURI = tokenURI_;
        AccessControlMintable._grantRole(DEFAULT_ADMIN_ROLE, administrator);
        AccessControlMintable._grantRole(DEFAULT_ADMIN_ROLE, developer);
    ...
    function mint(address to, uint256 amount) external nonReentrant onlyMinter {
        ...
        require(totalSupply() < TOKEN_SUPPLY, "Not enough supply");
        unchecked {
     ...
     * @notice all passes will have the same metadata
     */
    function setTokenURI(string memory tokenURI_) external onlyAuthorized {
        _tokenURI = tokenURI_;
    }

    /**
     * @dev Returns the contract uri for marketplaces
     */
    ...
    function totalSupply() public view returns (uint256) {
        ...
        } // total supply is one less than next id
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(AccessControlMintable, ERC2981, ERC721)
        returns (bool)
    {
        return
            AccessControlMintable.supportsInterface(interfaceId) ||
            ERC2981.supportsInterface(interfaceId) ||
            ERC721.supportsInterface(interfaceId) ||
            type(IMintableERC721).interfaceId == interfaceId;
    }
}
...
/// @title: AccessControlMintable
/// @author: unrealaccelerator.io

/**
@notice Access control contract that provides functions for:
    - Admin role
    - Minter contract role
 */

import "@openzeppelin/contracts/access/AccessControlEnumerable.sol";
import "@openzeppelin/contracts/utils/introspection/ERC165Checker.sol";
import "../interfaces/IMinterERC721.sol";

contract AccessControlMintable is AccessControlEnumerable {
     ...
     * @notice Account must support the minter interface. See {IMinterERC721}
     */
    function addMinterContract(address account) external {
        require(
            ERC165Checker.supportsInterface(
                account,
                type(IMinterERC721).interfaceId
            ),
...
/**
@notice Interface for minting ERC721 token contracts
*/
...
/**
 * @notice Interface for getting the quantity of ERC721 tokens minted by an address
 * from a Minter contract
 */
interface IMinterERC721 {
