...
contract CosmoGangStaking is Ownable, ReentrancyGuard {
        ...
        private nonReentrant
    {
        ...
        uint256 currentStakedNfts = getUserTotalStaked(receiver);
        uint256[] memory newStakedTokenIds = new uint256[](currentStakedNfts);
        for (uint256 i; i < len; ++i)
        {
        ...
        internal view returns (uint256)
    {
        ...
        // require(stakers[_user].stakedTokenIds.length > 0, "User has no tokens staked");
        require(getUserTotalStaked(_user) > 0, "User has no tokens staked");
