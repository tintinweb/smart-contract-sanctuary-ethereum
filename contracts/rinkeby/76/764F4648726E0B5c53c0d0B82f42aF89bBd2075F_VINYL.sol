...
contract VINYL is ERC1155PackedBalance, Ownable {
  ...
  bytes32 public merkleRoot =
    0xdb9f7b75bd7be5081adf28065e8d2dd1322f281c829a80c122d483779b958616;
   ...
   */
  function mint(address to, uint256 mint_type) internal {
    if (mint_type == 1) {
      _mint(to, uint256(VINYLTYPE.MINT), 1, "");
    } else if (mint_type == 2) {
      _mint(to, uint256(VINYLTYPE.MINT), 2, "");
    } else {
      uint256[] memory Collect = new uint256[](2);
      uint256[] memory Count = new uint256[](2);
      if (mint_type == 5) {
        Collect[0] = uint256(VINYLTYPE.GOLD);
        Collect[1] = uint256(VINYLTYPE.ONYX);
        Count[0] = 10;
        Count[1] = 10;
      } else {
        Collect[0] = uint256(VINYLTYPE.MINT);
        Collect[1] = uint256(VINYLTYPE.GOLD);
        if (mint_type == 3) {
          Count[0] = 2;
          Count[1] = 1;
        } else if (mint_type == 4) {
  ...
  function setURI(string memory _URI) external onlyOwner {
    require(
      ...
      "baseURI is already the value being set."
    );
    baseURI = _URI;
  }
