...
contract PeepsPassport is ERC1155Single, Payable, Signable, IPeepsPassport {
    ...
    // Global sale details
    uint32 public totalMinted = 0;
    uint32 private _totalSupply = 0;
    uint32 private txLimitPlusOne = 11;
    ...
    constructor(address peepsClubAddr_)
        ERC1155Single("https://metadata.peeps.club/passport/passport.json")
        Payable(1000)
    {
    ...
    modifier withinTxLimit(uint32 quantity) {
        require(quantity < txLimitPlusOne, "PeepsPassport: Exceeds transaction limit");
        _;
    }
        ...
        correctValue(tokenPrice * quantity)
    {
        ...
        require((totalMinted + quantity) < publicSaleInfo.maxMintPlusOne, "PeepsPassport: Exceeds available tokens");
        _safeMint(to, quantity);
    }

    /**
     * Mint tokens when authorised by server.
     * @notice This function is only available to VIPeeps.
    ...
    function mintSigned(
        ...
        signed(abi.encodePacked(msg.sender, nonce, quantity), signature)
    {
     ...
     */
    function setPublicSaleInfo(uint32 maxMint, uint64 endTimestamp) external onlyOwner {
        publicSaleInfo = PublicSaleInfo(maxMint + 1, endTimestamp);
    }
     ...
     */
    function burn(address owner, uint32 quantity) external {
