...
contract Breadaverse is ERC721Enumerable, Ownable {
    using MerkleProof for bytes32[];
    using Strings for uint256;

    mapping(address => uint8) public alreadyMinted;
    uint8 public constant maxPerWallet = 50;

    uint16 public reserveBreads;
    uint16 public breadId;
    ...
    uint16 public constant totalBreads = 6000;

    uint256 public breadPrice = 0.05 ether;
    ...
    constructor(string memory name, string memory symbol) ERC721(name, symbol) {
        breadId = 1; // item 1-5500
        reserveBreads = 5500; // item 5501-6000
    }
    ...
    function mint(
        ...
        require(breadId + amount < 5500, "Mint limit reached");
        require(alreadyMinted[msg.sender] + amount <= maxPerWallet, "Address minted max amount");
        ...
        if (publicSaleStarted == false) {
            ...
            require(amount <= 5, "Cannot mint more than 5 in a single transaction");
        }

        alreadyMinted[msg.sender] += amount;

        for (uint8 i = 0; i < amount; i++) _safeMint(msg.sender, breadId++);

        return breadId;
    }

    function reserveBread(address to, uint16 amount) public onlyOwner {
        require(reserveBreads + amount <= 6000, "Out of stock");

        for (uint16 i = 0; i < amount; i++) {
            _safeMint(to, reserveBreads++);
        }
    }
