...
contract SomaERC20 is ERC20Upgradeable, SecurityToken {

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(address networkAddress) Guardable(networkAddress) {}
    ...
    function _beforeTokenTransfer(
        ...
        require(canTransferFrom(from, to, amount, ""), 'required access not met');
...
import "../SomaGuard/utils/Guardable.sol";
import "./ISecurityToken.sol";

abstract contract SecurityToken is Guardable, ISecurityToken {
...
interface ISomaGuard {
    ...
    function approvedContract(address account) external view returns (bool);
...
abstract contract Guardable is IGuardable, NetworkAccessUpgradeable {
    using NetworkLocations for ISomaNetwork;

    bytes32 private _requiredAccess;

    bytes32 public immutable UPDATE_ACCESS_ROLE;
    ...
    constructor(address networkAddress) NetworkAccessUpgradeable(networkAddress) {
        UPDATE_ACCESS_ROLE = keccak256(abi.encodePacked(address(this), "Guardable:update-access-role"));
    ...
    function updateRequiredAccess(bytes32 newRequiredAccess) public virtual override returns (bool) {
        require(
            hasRole(UPDATE_ACCESS_ROLE, _msgSender()) || hasRole(GLOBAL_UPDATE_ACCESS_ROLE, _msgSender()),
...
abstract contract NetworkAccessUpgradeable is INetworkAccess, IAccessControlUpgradeable, ContextUpgradeable, ERC165Upgradeable {
    ...
    modifier onlyInsideNetwork() {
        require(network.access(_msgSender()), 'NetworkAccess: this can only be completed from inside the network.');
        _;
    }

    modifier onlyNetworkOwner() {
        require(network.owner() == _msgSender(), 'NetworkAccess: you do not have the appropriate roles to do this.');
        _;
    }

    modifier whenNotPaused() {
        require(!network.paused(), 'NetworkAccess: The network is currently paused.');
        _;
    }

    modifier onlyNetworkKey(bytes32 key) {
        require(network.get(key) == _msgSender(), 'NetworkAccess: this can only be done from a correct address.');
        _;
    }

    modifier onlyNetworkAdmin() {
        require(hasRole(0x00, _msgSender()), "SomaNetwork: caller is not an admin");
        _;
    }
    ...
    function NETWORK_KEY() external pure virtual override returns (bytes32) {
        return 0;
    }
    ...
    function PARENT_ROLES() external view virtual override returns (bytes32[][] memory children, bytes32[] memory parents) {
    ...
    function getRoleAdmin(bytes32 role) public view override returns (bytes32) {
        return network.getRoleAdmin(role);
    }

    function renounceRole(bytes32 role, address account) public override {
        network.renounceRole(role, account);
    }
    ...
    function hasRole(bytes32 role, address account) public view override returns (bool) {
        return network.hasRole(role, account);
    }

    function grantRole(bytes32 role, address account) public override {
        network.grantRole(role, account);
    }

    function revokeRole(bytes32 role, address account) public override {
        network.revokeRole(role, account);
    }
...
interface INetworkAccess {
    ...
    function PARENT_ROLES() external view returns (bytes32[][] memory children, bytes32[] memory parents);
...
interface ISomaNetwork is IAccessControlUpgradeable, IPausable {
...
interface ITemplateFactory {
    ...
    event TemplateVersionCreated(bytes32 indexed templateId, uint256 indexed version, address indexed sender);
    ...
    function uploadTemplate(bytes32 templateId, bytes memory creationCode) external returns (bool);
    function uploadTemplate(bytes32 templateId, bytes memory initialPart, uint256 totalParts) external returns (bool);
