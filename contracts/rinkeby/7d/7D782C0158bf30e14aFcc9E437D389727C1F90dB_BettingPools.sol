...
contract BettingPools is Ownable {
    ...
    function getWinnings() public view returns(uint256){
        ...
        if (currentStatus == GameStatus.TEAM_HOME) {
          winnings = playerBet.betHome + ((playerBet.betHome * (pools[0] + pools[1])) / pools[2]);
        } else if (currentStatus == GameStatus.TEAM_AWAY) {
          winnings = playerBet.betAway + ((playerBet.betAway) * (pools[2] + pools[0])) / pools[1];
        } else if (currentStatus == GameStatus.DRAW) {
          winnings = playerBet.betDraw + ((playerBet.betDraw) * (pools[1] + pools[2])) / pools[0] ;
        }
