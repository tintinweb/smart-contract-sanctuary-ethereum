...
contract Dragon is ERC721, Ownable, DragonReproduction {
    ...
    function createNewDragon(newDragonData memory _newDragonData)
        public
        returns (uint256 dragonId)
    {
        ...
        dragon memory _newDragon = dragon(
            _dragonId,
            "",
            _newDragonData.generation,
            _newDragonData.motherId,
            _newDragonData.fatherId,
            _newDragonData.gender,
            _newDragonData.color,
            1,
            _newDragonData.dominantGenes,
            _newDragonData.recesiveGenes1,
            _newDragonData.recesiveGenes2,
            _newDragonData.readyTimeToLayAnEgg,
    ...
    function birthNewDragon(
        ...
        uint256 _fatherId,
        address _owner
    ) external returns (uint256 babyDragonId) {
        dragon memory _motherDragon = getDragon(_motherId);
        dragon memory _fatherDragon = getDragon(_fatherId);

        newDragonData memory babyDragonData = _birthNewDragon(
            _motherDragon,
            _fatherDragon,
            _owner
        );

        uint256 _babyDragonId = createNewDragon(babyDragonData);

        return _babyDragonId;
    }

    // TODO: move this _getDragon() fn to DragonAttibutes.sol
    ...
    function reproduceDragons(uint256 _motherId, uint256 _fatherId) external {
        ...
        // TODO: return an array of dragonEggsIds ???
    }
}
...
// birth constants
uint256 constant LAY_EGG_COOLDOWN = 10 minutes;
uint256 constant TIME_TO_BECOME_ADULT = 2 minutes;
...
struct newDragonData {
    ...
    uint256 fatherId;
    genes dominantGenes;
    genes recesiveGenes1;
    genes recesiveGenes2;
    uint256 readyTimeToLayAnEgg;
    address owner;
}
...
contract DragonReproduction {
    ...
    function _birthNewDragon(
        ...
        // TODO: implement here all the logic

        uint256 _readyTimeToLayAnEgg = block.timestamp + TIME_TO_BECOME_ADULT;

        return
            newDragonData(
                _motherDragon.gender,
                _fatherDragon.color,
                _motherDragon.generation,
                _motherDragon.dragonId,
                _fatherDragon.dragonId,
                _motherDragon.dominantGenes,
                _motherDragon.dominantGenes,
                _fatherDragon.dominantGenes,
