...
contract DNAChip is ERC721, Ownable {
    using Counters for Counters.Counter;
    using AnonymiceLibrary for uint8;

    uint256 public constant MAX_SUPPLY = 3350;
    ...
    uint16[] public BASE;
    uint16[] public LEVEL;
    uint16[] public EARRINGS;
    uint16[] public EYES;
    uint16[] public HATS;
    uint16[] public MOUTHS;
    uint16[] public NECKS;
    uint16[] public NOSES;
    uint16[] public WHISKERS;
    ...
    mapping(uint256 => uint8) public tokenIdToBase;
    ...
    constructor() ERC721("DNAChip", "DNA") {
        // Freak, Robot, Underworld, Alien
        BASE = [450, 400, 100, 50];
        // lvl 1, lvl 2, lvl 3
        LEVEL = [750, 200, 50];

        EARRINGS = [900, 95, 5];
        EYES = [390, 390, 205, 10, 5];
        HATS = [570, 270, 145, 10, 5];
        MOUTHS = [150, 150, 150, 150, 150, 150, 100];
        NECKS = [700, 270, 30];
        NOSES = [200, 200, 200, 200, 200];
        WHISKERS = [240, 240, 240, 230, 50];
    }

    function mint() external {
        require(!AnonymiceLibrary.isContract(msg.sender), "no cheaters");
        ERC20Burnable(cheethAddress).burnFrom(msg.sender, getPrice());
    ...
    function reroll(uint256 tokenId) external {
        ...
        ERC20Burnable(cheethAddress).burnFrom(msg.sender, getRerollPrice());
        _tokenIdCounter.increment();
        _randomizeChip(tokenId);
    }
    ...
    function assembleEvolutionPod(uint256 tokenId) external {
        require(!AnonymiceLibrary.isContract(msg.sender), "no cheaters");
        require(msg.sender == ownerOf(tokenId), "not allowed");
        require(tokenIdToTraits[tokenId] == 0, "already assembled");
    ...
    function evolveBreedingMouse(uint256 tokenId, uint256 breedingMouseId) external {
        require(!AnonymiceLibrary.isContract(msg.sender), "no cheaters");
        require(tokenIdToTraits[tokenId] != 0, "not assembled");
    ...
    function getPrice() public view returns (uint256) {
        if (_tokenIdCounter.current() < 710) return 2500 ether;
        if (_tokenIdCounter.current() < 1420) return 5000 ether;
        if (_tokenIdCounter.current() < 2130) return 10000 ether;
        if (_tokenIdCounter.current() < 2840) return 20000 ether;
        if (_tokenIdCounter.current() < 3550) return 30000 ether;
        revert("supply depleted");
    }

    function getRerollPrice() public view returns (uint256) {
        if (_tokenIdCounter.current() < 710) return 500 ether;
        if (_tokenIdCounter.current() < 1420) return 1000 ether;
        if (_tokenIdCounter.current() < 2130) return 2000 ether;
        if (_tokenIdCounter.current() < 2840) return 4000 ether;
        if (_tokenIdCounter.current() < 3550) return 6000 ether;
        revert("supply depleted");
    }
    ...
    function _randomizeChip(uint256 tokenId) internal {
        require(tokenIdToTraits[tokenId] == 0, "already assembled");
        uint8 base = _rarityGen(_getRandomNumber(tokenId, 1000), BASE) + 1;
        uint8 level = _rarityGen(_getRandomNumber(tokenId, 1000), LEVEL) + 1;
        tokenIdToBase[tokenId] = base;
    ...
    function _generateTraits(uint256 tokenId) internal returns (RedactedLibrary.Traits memory traits) {
        traits.base = tokenIdToBase[tokenId];
        traits.level = tokenIdToLevel[tokenId];
        traits.earrings = _rarityGen(_getRandomNumber(tokenId, 1000), EARRINGS) + 1;
        traits.eyes = _rarityGen(_getRandomNumber(tokenId, 1000), EYES) + 1;
        traits.hats = _rarityGen(_getRandomNumber(tokenId, 1000), HATS) + 1;
        traits.mouths = _rarityGen(_getRandomNumber(tokenId, 1000), MOUTHS) + 1;
        traits.necks = _rarityGen(_getRandomNumber(tokenId, 1000), NECKS) + 1;
        traits.noses = _rarityGen(_getRandomNumber(tokenId, 1000), NOSES) + 1;
        traits.whiskers = _rarityGen(_getRandomNumber(tokenId, 1000), WHISKERS) + 1;
    }
}
...
library RedactedLibrary {
    ...
    function traitsToRepresentation(Traits memory traits) internal pure returns (uint256) {
        ...
        representation |= traits.level << 8;
        representation |= traits.earrings << 16;
        representation |= traits.eyes << 24;
        representation |= traits.hats << 32;
        representation |= traits.mouths << 40;
        representation |= traits.necks << 48;
        representation |= traits.noses << 56;
        representation |= traits.whiskers << 64;
    ...
    function representationToTraits(uint256 representation) internal pure returns (Traits memory traits) {
        ...
        traits.level = uint32(representation >> 8);
        traits.earrings = uint8(representation >> 16);
        traits.eyes = uint8(representation >> 24);
        traits.hats = uint16(representation >> 32);
        traits.mouths = uint16(representation >> 40);
        traits.necks = uint8(representation >> 48);
        traits.noses = uint8(representation >> 56);
        traits.whiskers = uint8(representation >> 64);
    }
}
