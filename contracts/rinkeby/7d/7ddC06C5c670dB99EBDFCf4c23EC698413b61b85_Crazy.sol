...
contract Crazy is Ownable, ERC721 {
  ...
  uint256 public MINT_UPPERLIMIT = 5;
  uint256 premiumPrice = 0.05 ether;

  bool public saleOpen;
  bool public finalURISet;
  ...
  // 1 = free, 2 = premium
  mapping (uint256 => uint256) public edition;
  ...
  function buyFree(uint256 numberOfTokens) external payable {

    if(!saleOpen) revert SaleDeactivated();

    require(balanceOf(msg.sender) + numberOfTokens < 6, "Exceeds mint limit per wallet");
  ...
  /// @notice Allows users to buy with fees of 0.05 ETH
  /// @dev Preventing contract buys has some downsides, but it seems to be what the NFT market generally wants as a bot mitigation measure
  /// @param numberOfTokens the number of NFTs to buy
  function buyPremium(uint256 numberOfTokens) external payable {
    ...
    uint256 mintPrice = premiumPrice * numberOfTokens;
  ...
  */
  function verifyProvenanceHash(string calldata _baseURI) view public returns(bool) {
    require(keccak256(abi.encode(_baseURI)) == PROVENANCE_HASH, "Doesn't match with the provenance hash");
    return true;
  }
  ...
  */
  function setBaseURI(string calldata _baseURI) external onlyOwner returns(bool) {
    if(finalURISet) revert("URI cannot be changed anymore"); 
  ...
  Sets the dummy base URI. This URI will be used by all tokens until the reveal URI is set.
  */
  function setDummyURL(string memory theDummyURI) external onlyOwner{
    dummyURI = theDummyURI;
  }

  // Once this function is called, the metadata cannot be changed ever.
  function setURIFinalised() external onlyOwner{
    finalURISet = true;
  }
