...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
        ...
        returns (string memory)
    {
        ...
        string memory baseURI = _baseURI_();
        return
     ...
     */
    function _baseURI_() internal view virtual returns (string memory) {
        return string("https://gateway.pinata.cloud/ipfs/QmNk1d1fHwTRoUZW6c87LaBh73JstKvpDvhxuYwPGNvjxi/");
    }
    ...
    function _safeMint(
        ...
        uint256 newItemId = startTokenId;
    ...
    function removeNFT(address _addr, uint256 _id) internal {
        bool flag = false;
        for(uint8 i = 0; i < _ownerToTokenIds[_addr].length - 1; i++) {
            if(flag == false && _ownerToTokenIds[_addr][i] == _id) {
                flag = true;
            }
            if(flag == true) {
                _ownerToTokenIds[_addr][i] = _ownerToTokenIds[_addr][i + 1];
            }
            if(flag == true && i == _ownerToTokenIds[_addr].length) {
                _ownerToTokenIds[_addr].pop();
            }
            if(flag == false && i == _ownerToTokenIds[_addr].length - 2){
                _ownerToTokenIds[_addr].pop();
            }
        }
        if (flag == true) {
...
contract ERC721RExample is ERC721A, Ownable {
    ...
    uint256 public constant refundPeriod = 100 days;
    ...
    constructor() ERC721A("Test3R", "TEST3") {
    ...
    function refund(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            removeNFT(msg.sender, tokenId);
            setTokenIdsToAddress(refundAddress, tokenId);
        }
