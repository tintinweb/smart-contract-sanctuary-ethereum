...
contract TestNft is Context, AccessControlEnumerable, ERC721Enumerable {

    using ECDSA for bytes32;
    using BitMaps for BitMaps.BitMap;


    // Predetermined amount of NFTs in the collection. Cannot be changed.
    ...
    function mint(uint256 amount, uint256 slotId, uint256 validUntil, bytes memory signature) external payable {
        ...
        require(!_slots.get(slotId), "TestNft: Slot already used");

        // Check args.
        uint256 totalTokens = totalSupply(); // Not burnable so totalSupply never goes down.
        require(totalTokens + amount <= collectionSize, "TestNft: total amount must not be over the max");
        require(msg.value >= (amount * _price), "TestNft: Insufficient eth sent");
        require(block.timestamp <= validUntil, "TestNft: Slot must be used before expiration time");
        require(block.timestamp <= _priceValidUntil, "TestNft: The price has expired");

        // Setting the slot guards for reentrancy.
        _slots.set(slotId);

        // _safeMint will internally revert on attempts to mint the same tokenId twice mitigating reentrancy attacks
        // with other slot ids.
    ...
    function batchMint(address[] memory accounts, uint256[] memory amounts) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        // Revert happens after minting if too many were minted. Gas optimization for happy path.
        // Sad times for sad path. Give it too little gas and it eats it all and reverts.
        require(nextTokenId <= collectionSize, "TestNft: Minting over max total supply");
    }

    function supportsInterface(bytes4 interfaceId) public view override(AccessControlEnumerable, ERC721Enumerable) returns (bool){
