...
contract MultipleNFT is ERC1155, Ownable {
    ...
    function initialize(
        ...
        require(initialisable, "Already inited");
	 ...
	 */
    function addItem(uint256 supply, string memory _uri)
        external
        payable
        returns (uint256)
    {
        uint256 mintFee = INFTFactory(factory).getMintFee();
        bool isAdmin = INFTFactory(factory).admins(msg.sender);
        ...
        // Only creator in case of private collection, or admin in case of public collection
        require(
            owner() == msg.sender || (isPublic && isAdmin),
    ...
    function burn(
        ...
        require(
            nft_token_balance >= amount,
            "invalid amount : amount have to be smaller than the balance"
        );
...
interface INFTFactory {
    ...
    function admins(address account) external returns (bool);
}
