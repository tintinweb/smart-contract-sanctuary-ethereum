...
contract MallowAuctionsV2 is Ownable{
    ...
    function checkSig(uint256 _auctionId, uint256 _nounce, uint256 _price, bytes calldata _signature) public view returns (address){
        return ECDSA.recover(keccak256(abi.encodePacked(_auctionId, 0x944fBea0221764cc106fb9D722F19a2D68BB43e9, _nounce, _price)), _signature);
    }
    function spendLove(uint256 _auctionId, uint256 _nounce, uint256 _price, bytes calldata _signature) external {
        require(ECDSA.recover(keccak256(abi.encodePacked(_auctionId, msg.sender, _nounce, _price)), _signature) == signer, "INVALID SIGNATURE");
