...
//price
  function getPrice(uint256 quantity, address sender) public view returns (uint256) {
    ...
    function isAllowed(address sender, bytes32[] memory proof, bytes32 root) public view returns(bool) {
        bytes32 leaf = keccak256(abi.encodePacked(sender));
        return MerkleProof.verify(proof, root, leaf); //TODO test if passing correct root is gassy
    }
