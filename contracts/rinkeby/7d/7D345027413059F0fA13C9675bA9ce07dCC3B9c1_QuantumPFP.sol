...
contract QuantumPFPv2 is QuantumPFP {
  function updateEmployee(uint tokenId, string memory email, string calldata description, address owner) external onlyOwner {
    require(_exists(tokenId) && !EmployeeList[tokenId].terminated, "invalid employee");
    bytes32 nodeId = computeNamehash(abi.encodePacked(EmployeeList[tokenId].initial, ".", baseDomain)); 
    Resolver resolver = ens.resolver(nodeId);
    ens.setOwner(nodeId, owner);
    resolver.setText(nodeId, "email", email);
    resolver.setText(nodeId, "status", description);
  }

  function getEmployeeData(uint tokenId) external view returns (string[3] memory ) {
    require(_exists(tokenId) && !EmployeeList[tokenId].terminated, "invalid employee");
    bytes32 nodeId = computeNamehash(abi.encodePacked(EmployeeList[tokenId].initial, ".", baseDomain));
    Resolver resolver = ens.resolver(nodeId);
