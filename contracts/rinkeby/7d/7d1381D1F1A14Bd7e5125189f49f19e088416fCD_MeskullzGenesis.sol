...
import "./ERC721A.sol";
import "./MeskullzToken.sol";

contract MeskullzGenesis is ERC721A, ReentrancyGuard, Ownable {
    string public MESKULLZ_PROVENANCE = "";
    string public baseTokenURI;

    uint256 public maxMeskullzPerTxn;
    uint256 public meskullzPrice;
    uint256 public constant MAX_MESKULLZ = 7399;
    ...
    mapping(uint256 => string) public nameMZ;
    mapping(address => uint256) public balanceMZ;

    MeskullzToken public meskullzToken;

    event NameChanged(string name);

    constructor(string memory baseURI) ERC721A("Meskullz", "MZ") {
        setBaseURI(baseURI);
    }

    function setMeskullzToken(address _yield) external onlyOwner {
        meskullzToken = MeskullzToken(_yield);
    }

    function setBurnRate(uint256 _namingPrice, uint256 _claimingPrice)
        external
        onlyOwner
    {
    ...
    function changeName(uint256 _tokenId, string memory _newName) public {
        ...
        meskullzToken.burn(msg.sender, nameChangeTokenPrice);
        nameMZ[_tokenId] = _newName;
    ...
    function claim(uint256 quantity) external {
        meskullzToken.burn(msg.sender, claimTokenPrice);

        _safeMint(msg.sender, quantity);
    }

    function getReward() external {
        meskullzToken.updateReward(msg.sender, address(0), 0);
        meskullzToken.getReward(msg.sender);
    }
    ...
    function transferFrom(
        ...
        meskullzToken.updateReward(from, to, tokenId);
        balanceMZ[from]--;
        balanceMZ[to]++;
    ...
    function reserveMeskullz(uint256 quantity) public onlyOwner {
        require(
            totalSupply() + quantity <= MAX_MESKULLZ,
            "Purchase would exceed max supply of Meskullz"
        );
        _safeMint(msg.sender, quantity);
        balanceMZ[msg.sender] += quantity;
        meskullzToken.updateRewardOnMint(msg.sender);
    }
    ...
    function setMaxPerTransaction(uint256 _maxNFTPerTransaction)
        ...
        onlyOwner
    {
        maxMeskullzPerTxn = _maxNFTPerTransaction;
    }
    ...
    function mintTierA(uint256 quantity) public payable {
        require(saleIsActiveTierA, "Sale must be active to mint Meskullz");
        require(maxMeskullzPerTxn > 0);
        require(totalSupply() <= MintCycleTierA);
        require(
            quantity <= maxMeskullzPerTxn,
        ...
        require(
            meskullzPrice * quantity <= msg.value,
    ...
    function mintTierB(uint256 quantity) public payable {
        require(saleIsActiveTierB, "Sale must be active to mint Meskullz");
        require(maxMeskullzPerTxn > 0);
