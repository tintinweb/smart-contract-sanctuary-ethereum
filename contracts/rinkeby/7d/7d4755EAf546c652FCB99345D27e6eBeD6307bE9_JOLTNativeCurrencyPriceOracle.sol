 ...
 * @author Federico Luzzi - <fedeluzzi00[email protected]>
 * SPDX-License-Identifier: GPL-3.0
 */
...
abstract contract Job is IJob, Context {
    address public immutable JOBS_REGISTRY;

    error ZeroAddressJobsRegistry();
    error NotAWorker();
    error InvalidWorker();

    constructor(address _jobsRegistry) {
        if (_jobsRegistry == address(0)) revert ZeroAddressJobsRegistry();
        JOBS_REGISTRY = _jobsRegistry;
    }
    ...
    modifier needsExecution() {
        IJobsRegistry(JOBS_REGISTRY).initializeWork();
        _;
        IJobsRegistry(JOBS_REGISTRY).finalizeWork(msg.sender);
    }
    ...
    modifier needsExecutionWithRequirements(
        ...
        IJobsRegistry(JOBS_REGISTRY).initializeWorkWithRequirements(
            msg.sender,
            _minimumBonded,
            _minimumEarned,
            _minimumWorksCompleted,
            _minimumAge
        );
        _;
        IJobsRegistry(JOBS_REGISTRY).finalizeWork(msg.sender);
