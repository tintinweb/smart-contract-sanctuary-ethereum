...
contract HDPass is ERC1155, Ownable, Pausable, ERC1155Burnable, ERC1155Supply {
    mapping(address => bool) public hasRedeemed;
    mapping(uint256 => uint256) public price;
    string public uriSuffix = ".json";
    // whitelist of addresses that can mint tokens
    bytes32 public whitelistMerkle =
        0x8d12f809b0d842902aa3c11e86ce5621bbc782db233f7c2fa668a47fb240109e;
    ...
    constructor(string memory _IPFSURL) ERC1155("") {
        ...
        symbol = "HDPass";
        baseURI = string(
            abi.encodePacked("https://ipfs.io/ipfs/", _IPFSURL, "/")
        );
    }
    ...
    function updateBaseURI(string calldata base) external onlyOwner {
        baseURI = string(abi.encodePacked("https://ipfs.io/ipfs/", base, "/"));
    }

    function setMaxSupply(uint256 newMax) public onlyOwner {
        require(
            newMax >= totalSupply(1),
            "Must be higher than or equal to current supply"
        );
        maxSupply = newMax;
    }
    ...
    function setPublicMintStatus(bool _publicStatus) public onlyOwner {
        publicMintActive = _publicStatus;
    }
    ...
    function mint(
        ...
        if (publicMintActive) {
            ...
            // set hasRedeemed to true
            hasRedeemed[msg.sender] = true;
            // increment minted
            minted += amount;
        } else if (freeMintActive) {
            require(
                (totalSupply(1) + amount) <= maxSupply,
            ...
            require(
                ...
                "Flippr: Invalid proof (not on whitelist?)."
            );
            _mint(msg.sender, id, amount, "0");
            // set hasRedeemed to true
            hasRedeemed[msg.sender] = true;
