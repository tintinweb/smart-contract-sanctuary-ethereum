...
contract SwashTeamVestingUpgradable is OwnableUpgradeable {
    ...
    function hasClaim(address recipient) public view returns (uint256) {
        ...
        (uint256 owedAmount,) = calculateAmounts(recipient);
        return owedAmount;
    }

    function calculateAmounts(address recipientAdd)
    internal
    view
    returns (uint256 _owedAmount, uint256 _calculatedAmount)
    {
        uint256 endTime = recipients[recipientAdd].revokedDate > 0 ? recipients[recipientAdd].revokedDate : block.timestamp;
        if(endTime < recipients[recipientAdd].startDate){
            return (0, 0);
        }
        uint256 period = (endTime - recipients[recipientAdd].startDate) / (periodLength);
        Recipient memory recipient = recipients[recipientAdd];
        ...
        _owedAmount = _calculatedAmount - recipients[recipientAdd].withdrawnAmount;
