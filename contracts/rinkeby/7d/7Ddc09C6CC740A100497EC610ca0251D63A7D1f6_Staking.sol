...
contract Staking is IStaking, Ownable, Pausable, IERC721Receiver {
    ...
    uint256 public NB_DAILY_TOKENS_STD = 100 ether;
    uint256 public NB_DAILY_TOKENS_LRG = 150 ether;
    uint256 public NB_DAILY_TOKENS_PRM = 200 ether;
    ...
    function claimMany(uint256[] calldata tokenIds, bool unstake) external whenNotPaused {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (nftIslands.isStandard(tokenIds[i])) {
                owed +=
                    ((block.timestamp - stake.value) * NB_DAILY_TOKENS_STD) /
                    1 days;
            } else if (nftIslands.isLarge(tokenIds[i])) {
                owed +=
                    ((block.timestamp - stake.value) * NB_DAILY_TOKENS_LRG) /
                    1 days;
            } else {
                owed +=
                    ((block.timestamp - stake.value) * NB_DAILY_TOKENS_PRM) /
                    1 days;
            }
...
interface ITheNFTIslands is IERC721A {
    function isStandard(uint256 tokenId) external view returns (bool);

    function isLarge(uint256 tokenId) external view returns (bool);
...
interface IIslandToken is IERC20, IAccessControl {
