...
//bidder details
 ...
 function CreateFixPriceSale(address _addressNFTCollection, uint256 _nftId, uint256 _price , address _collectionOwner) external returns(uint256) {
...
function getFixPriceArray(uint256 _fixIndex) public view returns (FixedPriceSell[] memory){

  FixedPriceSell[] memory Auctions = new FixedPriceSell[](AllFixedPriceSell.length);
        uint count = 0;
        for (uint i=0; i<AllFixedPriceSell.length ; i++)
        {
            if (AllFixedPriceSell[i].index == _fixIndex)
            {
                Auctions[count] = AllFixedPriceSell[i];
                count++;
            }
        }
        return Auctions;
...
function getAuction(uint256 _auctionIndex) public view returns (Auction[] memory){

  Auction[] memory Auctions = new Auction[](allAuctions.length);
        uint count = 0;
        for (uint i=0; i<allAuctions.length ; i++)
        {
            if (allAuctions[i].index == _auctionIndex)
            {
                Auctions[count] = allAuctions[i];
                count++;
            }
        }
        return Auctions;
