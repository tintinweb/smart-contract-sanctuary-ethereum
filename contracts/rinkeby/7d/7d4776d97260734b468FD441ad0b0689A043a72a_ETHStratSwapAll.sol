...
contract ETHStratSwapAll is Ownable, ReentrancyGuard {
    ...
    /// @dev Execute strategy. Swap all ETH to TokenOut.
    function execute(
        address[] calldata tokenIn,
        address tokenOut,
        uint256 /* amountETH */
    ) external payable nonReentrant {
        require(tokenIn.length == 0, "tokenIn not ETH.");
        uint256 balance = address(this).balance;
        require(balance > 0, "Bad amount");
        address[] memory path = new address[](2);
        path[0] = wNative;
        path[1] = tokenOut;
        // 1. Convert that all ETH into tokenOut.
        ...
        // 2. Transfer all tokenOut (as a result of a conversion) back to the caller.
        tokenOut.safeTransfer(msg.sender, tokenOut.myBalance());
    }
