...
contract CapitalPool is ICapitalPool, OwnableUpgradeable, AbstractDependant {
    ...
    function fundReward(address _voter, uint256 _rewardAmount)
        external
        override
        onlyClaimingRegistry
    {
        _withdrawFromLiquidityCushion(
            _voter,
            DecimalsConverter.convertFrom18(_rewardAmount, stblDecimals)
        );

        _updatePoolLiquidity(address(reinsurancePool), _rewardAmount, 0, PoolType.REINSURANCE);
    }
    ...
    function getWithdrawPeriod() external view override returns (uint256) {
        return rebalanceDuration + 10 minutes;
    }
}
