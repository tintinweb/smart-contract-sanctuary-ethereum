...
contract CoolDrinkingAlienEcosystem is OpenSeaERC1151, ReentrancyGuard, Ownable, PullPayment {
    ...
    constructor (uint256 _publicMintFee,
                 ...
                 string memory _globalName,
                 string memory _globalSymbol,
                 uint96 _royaltiesBps) OpenSeaERC1151(_globalName, _globalSymbol, _contractUri, _contractUri, _beneficiary, _royaltiesBps) {

        // validate
        require(bytes(_baseUriCollectables).length > 0, "ERR06");// baseURI cannot be empty"); 
        require(bytes(_baseUriEditions).length > 0, "ERR06");// baseURI cannot be empty"); 
        require(bytes(_baseUriFungibles).length > 0, "ERR06");// baseURI cannot be empty");
        require(bytes(_globalName).length > 0, "ERR07");// global desc cannot be empty"); 
        require(bytes(_globalSymbol).length > 0, "ERR07");// global desc cannot be empty"); 
    ...
    function _addTokenFamily(
        ...
        TokenFamilyInfo memory newTokenFamily = TokenFamilyInfo ({
         ...
         if(_tokenType == TokenType.COLLECTABLES) {
              newTokenFamily.token_id_tracker = newTokenFamily.token_id_range_init;
