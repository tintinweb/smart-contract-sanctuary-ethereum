...
contract p2p is Ownable, ReentrancyGuard, ERC721Holder, ERC1155Holder {
	...
	struct Swap {
        ...
        address[] wishNftAddressesFromInitiator;
		uint256[] wishNftIdsFromInitiator;
		uint256[] wishNftAmountsFromInitiator;
        uint256 wishEtherValueFromInitiator; 
	}
    ...
    * @param secondUserWishNftAddresses array of NFT addressed that initiator wants from second user
    * @param secondUserWishNftIds array of IDs belonging to NFTs that initiator wants from second user
    * @param secondUserWishNftAmounts array of NFT amounts that want to be traded from second user
    * @param secondUserWishETHValue ETH Value that initiator wants from second user
	*/
    function proposeSwap(
        ...
        address[] memory secondUserWishNftAddresses,
		uint256[] memory secondUserWishNftIds,
		uint256[] memory secondUserWishNftAmounts,
        uint256 secondUserWishETHValue
	) external payable chargeAppFee 
                        requireSameLength(nftAddresses, nftIds, nftAmounts) {
        require(secondUserWishNftAddresses.length == secondUserWishNftIds.length, "Wish NFT and Wish ID arrays have to be same length");
		require(secondUserWishNftAddresses.length == secondUserWishNftAmounts.length, "Wish NFT and Wish AMOUNT arrays have to be same length");
		_swapsCounter += 1;
        ...
        swap.wishNftAddressesFromInitiator = secondUserWishNftAddresses;
		swap.wishNftIdsFromInitiator = secondUserWishNftIds;
		swap.wishNftAmountsFromInitiator = secondUserWishNftAmounts;
        swap.wishEtherValueFromInitiator = secondUserWishETHValue;
	
		if (msg.value > fee) {
			swap.initiatorEtherValue = uint256(msg.value) - fee;
		...
		emit SwapProposed(
			...
			nftIds,
			nftAmounts,
            secondUserWishNftAddresses,
            secondUserWishNftIds,
            secondUserWishNftAmounts,
            secondUserWishETHValue
		);
	}
	...
	*/
    function initiateSwap(
		...
		if (msg.value > fee) {
			_swaps[swapId].secondUserEtherValue = uint256(msg.value) - fee;
	...
	*/
	...
	function getSwapDetail(uint64 _swapId) public view returns(Swap memory) {
