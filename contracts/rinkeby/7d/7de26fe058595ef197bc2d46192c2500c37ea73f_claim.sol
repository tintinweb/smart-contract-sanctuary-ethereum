...
contract claim  {
    ...
    function transferERC20(IERC20 token,uint256 tokenId) public {
        if (nft.ownerOf(tokenId) != msg.sender) revert NotOwner();
        if (hasClaimed[tokenId]) revert AlreadyRedeemed();
        uint256 erc20balance = token.balanceOf(address(this));
        require(updateAmount(tokenId) <= erc20balance, "balance is low");
        hasClaimed[tokenId] = true;
        token.transfer(msg.sender, updateAmount(tokenId));
        emit TransferSent(msg.sender, msg.sender, updateAmount(tokenId));
    }    
}