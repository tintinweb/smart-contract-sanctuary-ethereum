...
contract VillageDog is Ownable, ERC721A, ReentrancyGuard,Pausable {
    using Strings for uint256;
    using ECDSA for bytes32;

    uint256 public immutable MAX_SUPPLY = 5000;
    uint256 public immutable maxPerAddressDuringMint = 5;
    uint256 public immutable _mintPrice = 0 ether;
    ...
    function publicSaleMint(uint256 quantity) external payable callerIsUser whenNotPaused {
        require(quantity > 0, "Cant mint 0");
        require(totalSupply() + quantity <= MAX_SUPPLY, "reached max supply");
        require(msg.value >= _mintPrice * quantity);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(currentBaseURI).length > 0 ? string(abi.encodePacked(currentBaseURI, tokenId.toString(), _suffix)) : defaultURI;
    }
