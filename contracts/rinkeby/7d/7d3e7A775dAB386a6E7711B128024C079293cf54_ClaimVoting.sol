...
contract ClaimVoting is IClaimVoting, Initializable, AbstractDependant {
        ...
        returns (uint256[] memory claimIndexes, VotesUpdatesInfo[] memory voteRewardInfo)
    {
        ...
        for (uint256 i = 0; i < notReceivedCount; i++) {
            ...
            voteRewardInfo[i].bmiReward = _voteResults[voteIndex].bmiReward;
            voteRewardInfo[i].stblReward = _voteResults[voteIndex].stblReward;
            voteRewardInfo[i].reputationChange = _voteResults[voteIndex].reputationChange;
            voteRewardInfo[i].stakeChange = _voteResults[voteIndex].stakeChange;
        }
    }
