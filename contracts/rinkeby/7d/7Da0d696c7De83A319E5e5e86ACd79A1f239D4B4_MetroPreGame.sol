...
import "../../nfts/interfaces/IMetroBlockScores.sol";
import "../../MetToken.sol";
...
contract MetroPreGame is Controllable {
    ...
    constructor(address _tokenAddress, address _vaultAddress, address _scoresAddress) {
    ...
    function stake(uint256[] calldata tokenIds) external {
        ...
        vault.stakeUnsafe(msg.sender, tokenIds, 0, 0);
    }

    function unstake(uint256[] calldata tokenIds) external {
        IMetroVaultStorage vault = IMetroVaultStorage(vaultAddress);
        vault.unstakeUnsafe(msg.sender, tokenIds);
    }
    ...
    function _claim(address account, uint256[] calldata tokenIds, bool unstake) internal {
        uint256 tokenId;
        uint256 score;
        uint256 earned = 0;

        IMetroVaultStorage vault = IMetroVaultStorage(vaultAddress);
        IMetroBlockScores scoresContract = IMetroBlockScores(scoresAddress);

        for (uint i = 0; i < tokenIds.length; i++) {
            tokenId = tokenIds[i];

            uint256 score = scoresContract.getBlockScore(tokenId);
        ...
        if (earned > 0) {
            uint256 boost = scoresContract.getHoodBoost(tokenIds);
            earned = boost * earned / 10000;
            MetToken(tokenAddress).mint(account, earned);
        }

        if (unstake) {
            vault.unstakeUnsafe(account, tokenIds);
        }
