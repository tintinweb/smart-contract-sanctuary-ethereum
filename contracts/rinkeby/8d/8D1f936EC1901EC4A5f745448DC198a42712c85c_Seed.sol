...
contract FarmStorage {
    ...
    uint256 public wateringShortenFactor = 500;
    uint256 public weedingShortenFactor = 1000;
    uint256 public harvestingGainFactor = 1000;
    uint256 public landIncreasingPriceFactor = 3000;
...
contract Farm is IFarm,Ownable,Pausable,ReentrancyGuard{
    ...
    function plant(PlantBody[] memory bodies) public nonReentrant whenNotPaused{
        uint256 len = bodies.length;
        require(len > 0 && len <= accountLandCount[msg.sender], "farmer or land is invalid");

        for(uint i =0 ;i < len;i++){
            PlantBody memory body = bodies[i];
            uint256 landPid = body.landPid;
            ISeed seed = body.seed;

            require(landPid > 0 && landPid <= accountLandCount[msg.sender],"An invalid landPid");
            require(shop.isShopSeed(address(seed)),"An invalid seed");

            Land storage land = accountPidLand[msg.sender][landPid];
            require(address(land.seed) == address(0),"The land is already planted");

            land.seed = seed;
            land.harvestTime = seed.matureTime().add(block.timestamp);
            uint256 gainFactor = gainFactor(landPid);
            land.gain = seed.yield().mul(gainFactor);
            land.seed.burnFrom(msg.sender,1*10**seed.decimals());

            emit Planting(msg.sender,address(land.seed),landPid);
        }
    }

    function gainFactor(uint256 pid) internal returns(uint256){
        return pid <= initialLandCount ? 1 : pid.div(4);
    }

    function watering(address farmer, uint256[] calldata landPids) public payable nonReentrant whenNotPaused{
        uint256 len = landPids.length;
        ...
        for(uint i =0 ;i < len;i++){
            uint landPid = landPids[i];
            Land storage land = accountPidLand[farmer][landPid];
            if(address(land.seed) != address(0)){
                continue;
            }

            (bool isOverrun,bool isOvertime) = isWateringOverrun(farmer,landPid);
            if(isOverrun){
                continue;
            }

            Overrun.Limit storage waterLimit = accountLimitWater[farmer][landPid];
            if(isOvertime){
                waterLimit.times = 1;
            ...
            uint256 finalHarvestTime = land.harvestTime.mul(10000 - wateringShortenFactor).div(10000);
            land.harvestTime = finalHarvestTime > block.timestamp ? finalHarvestTime : block.timestamp;
            successTimes += 1;

            emit Watering(msg.sender, farmer, address(land.seed), landPid);
        }
    ...
    function harvesting(address farmer) public nonReentrant{
        ...
        for(uint i = 1;i <= len; i++){
            ...
            if(msg.sender != farmer){
                reaperGain = farmerGain.mul(harvestingGainFactor).div(10000);
    ...
    function buyLand(uint256 _count) public payable{
        ...
        uint256 ownedCount = accountLandCount[msg.sender];
        require(ownedCount.add(_count) <= landLeve.mul(initialLandCount),"More than the current land class allows");

        uint256 cost = calculateIncreasingLandCost(_count);
    ...
    function calculateIncreasingLandCost(uint256 _count) public view returns(uint256){
        ...
        for(uint256 i = toHaveCount; i > ownedCount; i--){
            ...
            uint256 curCost = landBasePrice * ((1 + landIncreasingPriceFactor.div(10000)) ** exponent);
