...
contract PixelSkull is AdminControl, ReentrancyGuard {
    ...
    mapping(address => bool) premintAddresses;
    ...
    constructor(
        ...
        setNftData(hashes, artists, lockdata);
    ...
    modifier presaleLocked(string memory imageHash) {
        if (contractData.isPresaleActive) {
            require(tokensData[imageHash].presaleSale, "Presale Artist is locked");
            _;
        }
    }
    modifier allreadyPreminted(address account) {
        if (contractData.isPresaleActive) {
    ...
    function getPrice() private view returns (uint256 price) {
        ...
        if (contractData.isPresaleActive) {
            ...
            _price = collectionData.presalePrice;
        }
        return _price;
    }
    ...
    function payout(address to) public adminRequired {
