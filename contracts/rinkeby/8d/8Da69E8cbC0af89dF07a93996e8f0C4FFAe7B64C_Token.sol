...
contract Token is ERC721ABurnable, Ownable, ReentrancyGuard {
    ...
    modifier whenSaleActive() {
        require(saleActive, "TT: Sale is not active.");
        _;
    }
    ...
    function setLimitSupply(uint256 limit_) external onlyOwner  {
        require(limitSupply == 0, "TT: Limit supply has already defined.");
        limitSupply = limit_;
        emit LimitSupplyDefined(block.timestamp, limitSupply);
    }

    function setLimitFreeSupply(uint256 limit_) external onlyOwner  {
        require(limitFreeSupply == 0, "TT: Limit free supply has already defined.");
    ...
    function startSale(uint256 price_, uint16 freeLimitPerAddress_, uint16 transactionLimit_) external onlyOwner {
        require(limitSupply > 0, "TT: Limit supply should be defined.");
    ...
    function mint(uint16 tokensAmount) external payable whenSaleActive nonReentrant {
        require(tokensAmount <= transactionLimit, "TT: Limited amount of tokens per transaction.");
        require(totalSupply() + tokensAmount <= limitSupply, "TT: Limited amount of tokens.");

        if (saleSupply < limitFreeSupply && freeReceivePerAddress[msg.sender] < freeLimitPerAddress) {
            uint16 freeTokensAmount = freeLimitPerAddress - freeReceivePerAddress[msg.sender];
            if (limitFreeSupply - saleSupply < freeTokensAmount) {
                freeTokensAmount = uint16(limitFreeSupply - saleSupply);
            }
            if (tokensAmount < freeTokensAmount) {
                freeTokensAmount = tokensAmount;
            }

            require(price * (tokensAmount - freeTokensAmount) <= msg.value, "TT: Insufficient funds.");
            freeReceivePerAddress[msg.sender] += freeTokensAmount;
        } else {
            require(price * tokensAmount <= msg.value, "TT: Insufficient funds.");
        }
