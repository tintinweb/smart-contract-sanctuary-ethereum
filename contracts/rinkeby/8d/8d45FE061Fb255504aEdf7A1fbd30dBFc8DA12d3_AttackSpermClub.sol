...
contract AttackSpermClub is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public constant PRE_SUPPLY = 2000;
    uint256 public constant MAX_PUBLIC_MINT = 5;

    uint256 public constant PUB_PRICE_PER_TOKEN = 0.02 ether;
    uint256 public constant PRE_PRICE_PER_TOKEN = 0.01 ether;
    ...
    constructor() ERC721("AttackSpermClub", "ASP") {
    }
    ...
    function mint(uint8 numberOfTokens) external payable{
        ...
        if(saleStage == 1)//inner sale
        {
            ...
            require(PRE_PRICE_PER_TOKEN * numberOfTokens <= msg.value, "You need to pay enough eth");
        ...
        else if(saleStage == 2)//public sale
        {
            ...
            require(PUB_PRICE_PER_TOKEN * numberOfTokens <= msg.value, "You need to pay enough eth");
    ...
    //mint for person
    function mintTo(address[] calldata addresses, uint8 numberOfTokens) public onlyOwner {
        uint256 ts = totalSupply();
        require(addresses.length*numberOfTokens+ts<=MAX_SUPPLY,"The mint quantity has exceeded the issuance limit");
        for (uint256 i = 0; i < addresses.length; i++) {
