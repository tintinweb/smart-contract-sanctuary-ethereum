...
contract ICO {
    ...
    event PhaseChangedToGeneral();
    event PhaseChangedToOpen();

    event FundraisePaused();
    event FundraiseResumed();
    ...
    function addToAllowList(address _privateContributor) external onlyOwner {
        ...
        if (seedPhaseAllowList[_privateContributor])
            revert AlreadyOnAllowList();
    ...
    function contribute() external payable {
        ...
        if (address(this).balance > icoContributionLimit)
        ...
        if (phase == Phase.SEED) {
            ...
            if (
                msg.value >
                (seedPhaseIndividualContributionLimit -
                    contributorsToAmount[msg.sender])
            ) revert ContributingBeyondMaximumAllowed();
    ...
    function redeem() external {
        ...
        contributorsToAmount[msg.sender] = 0;

        spaceCoin.transfer(msg.sender, redemptionAmount);
...
contract SpaceCoin is ERC20 {
    ...
    function setIco(address _ico) external {
        ...
        if (balanceOf(_ico) > 0) revert ICOStarted();
        ico = _ico;
    ...
    function calculateTax(uint taxableAmount, address from)
        private
        returns (uint tax)
    {
        ...
        console.log("unpaidPreciseTax[from]", unpaidPreciseTax[from]);
