...
contract HDRegistry is Ownable {
    event registryAdd(
        address owner,
    ...
    Counters.Counter private registryCounter;
    ...
    function getFromRegistry(uint256 registryId)
        public
        view
        returns (HDNFT memory)
    {
        return hdNFTs[registryId];
    }
    ...
    function addToRegistry(
        ...
        uint256 registryId = registryCounter.current();
        HDNFT memory hdNFT = HDNFT(
            owner,
            contractAddr,
            hashedData,
            tokenId,
