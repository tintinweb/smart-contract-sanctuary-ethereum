...
import "../../vault/v2/structs/MetroVaultStructs.sol";
import "../../vault/v2/interfaces/IMetroVaultStorage.sol";

interface IMetroBlockBlackout {

    function totalSupply() external view returns (uint256);
}

contract MetroBlockBlackout is ERC721, Controllable {

    uint256 public maxBlockCount = 10_000;

    uint256 public metPrice = 25_000 ether;
    uint256 public ethPrice = 1 ether;
    ...
    uint256 constant BLOCK_ID_OFFSET = 10_000;
    ...
    event BlockMinted(address account, uint256 tokenId);
    ...
    function setMetTokenAddress(address _metTokenAddress) public onlyOwner {
        metTokenAddress = _metTokenAddress;
    }

    function getMaxBlockCount() external view returns (uint256) {
        return maxBlockCount;
    }

    function setMaxBlockCount(uint256 count) public onlyOwner {
        maxBlockCount = count;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        IERC20 metToken = IERC20(metTokenAddress);
        metToken.transfer(owner(), metToken.balanceOf(address(this)));
    }

    function metMint(uint256 amount, bool stake) public {
        require(metSaleActive, "MET sale is not active");
        IERC20 metToken = IERC20(metTokenAddress);
        metToken.transferFrom(msg.sender, address(this), metPrice * amount);
        _mint(amount, stake);
    }
    ...
    function _mint(uint256 amount, bool stake) internal {
        require(totalSupply() + amount <= maxBlockCount, "Exceed max block count");
        if (stake) {require(vaultAddress != address(0x0), "VaultAddress is not defined");}

        uint256 start = _blockIdCounter;
        _blockIdCounter += amount;
        uint256[] memory tokenIds = new uint256[](amount);

        for (uint256 i = 0; i < amount; i++) {
            uint256 newBlockId = start + i + 1;
            _mint(stake ? vaultAddress : msg.sender, newBlockId);
        ...
        if (stake) {
            IMetroVaultStorage(vaultAddress).stakeUnsafe(msg.sender, tokenIds, 0, 0);
        }
    }
        ...
        returns (bool)
    {
        ...
        return isController(operator) || (operator == vaultAddress) || super.isApprovedForAll(owner, operator);
    }
