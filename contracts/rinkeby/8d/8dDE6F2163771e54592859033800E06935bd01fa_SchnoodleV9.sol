...
contract SchnoodleV9 is SchnoodleV9Base, AccessControlUpgradeable {
    ...
    function payFee(uint256 networkId, uint256 amount, address payable to) external {
        require(to == _bridgeOwner, "Schnoodle: Payee must be the bridge owner");
        _feesPaid[_msgSender()][networkId] += amount;
        to.transfer(amount);
    }
