...
contract NFTation is Initializable, 
    ...
    function isMarketPlacesActiveForAccount_Updated() public view returns (bool) {
    ...
    function aprovalAllMarketPlacesForAccount() public {
    ...
    function minting(string memory uri, uint256 royaltyPercentage) public {
    ...
    function checkFirstSale_Update(uint256 _tokenId) external view returns (bool){
    ...
    ///view Function for test  ++++++++++++++++
    function getUintVal1() public view returns (uint256){
        return uintVar1;
    }

    function getUintVal2() public view returns (uint256){
        return uintVar1;
    }
    ///view Function for test  ----------------
            function whatIsYourName() public pure returns (string memory) {
        return "Kazem";
    }
}
...
interface IAuctionMarketPlace {
    ...
    event newevent(uint256 tokenId, uint256 newMinBidPrice);
    ...
    function getUintVal1() external view returns (uint256);
...
contract NFTationStorage {
    ...
    //Add for upgradability test ++++++++++++
    uint256 internal uintVar1;
    string internal strVar2;
    bytes public byteArray3;
    uint[] public dynamicUintArray4;
    address[] public dynamicAddressArray5;
    mapping (address => testStruct) public mappingAddressTestStruct6;
    uint[10] public fixUintArray4;
    //Add for upgradability test ------------
}
...
contract FixedPriceMarketPlace is Initializable,
    ...
    returns(uint256 marketShareAmount,uint256 royaltyAmount ,address  creator){
        bool isFirstSale = NFTationContract.checkFirstSale_Update(tokenId);
        if(isFirstSale){
    ...
    function removeMarketItem(uint256 _itemId) public whenNotPaused nonReentrant {
        MarketItem storage _marketItem = idToMarketItem[_itemId];
    ...
    function createMarketSale(uint256 itemId) public payable whenNotPaused nonReentrant {
        ...
        if(NFTationContract.checkFirstSale_Update(_tokenId)){
...
contract AuctionMarketPlace is IAuctionMarketPlace, Initializable,UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable,ReentrancyGuardUpgradeable,AuctionMarketPlaceStorage {
    ...
    //update for test
        //Change function name
    function removeAuction(uint256 _tokenId) external whenNotPaused  {
        Auction memory auction = auctionMapping[_tokenId];
