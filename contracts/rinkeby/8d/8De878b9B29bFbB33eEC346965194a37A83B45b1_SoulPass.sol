...
contract SoulPass is ERC721A, ERC721AQueryable, Ownable {
    ...
    constructor(string memory baseURI) ERC721A("Soul Pass", "SOULPASS") {
        setBaseURI(baseURI);
        saleEnabled = true;
    ...
    modifier saleIsOpen() {
        require(totalSupply() <= maxTokenSupply, "Soul Pass: Sale ended");
        _;
    }
    ...
    function mint(
        ...
        require(total <= maxTokenSupply, "Soul Pass: Max limit");
        require(total + _count <= maxTokenSupply, "Soul Pass: Max limit");
        require(total + _count <= currentMaxSale, "Soul Pass: Max sale limit");
        require(mintCount[msg.sender] + _count <= maxMintPerWallet, "Soul Pass: Max wallet limit");
        require(_count <= maxMintPerTx, "Soul Pass: Max mint for tx limit");
        require(saleEnabled, "Soul Pass: Sale is not active");
        require(msg.value >= getPrice(_count), "Soul Pass: Value below price");

        if (whitelistEnabled == true) {
            require(merkleRoot != 0x0, "Soul Pass: merkle root not set");

            // Verify if the account has already claimed
            require(
                !isPresaleListClaimed(msg.sender),
                "Soul Pass: account already claimed"
            );

            // Verify we cannot claim more than the max amount
            require(
                _count <= _presaleMaxAmount,
                "Soul Pass: can only claim less than or equal to the max amount"
            );

            // Verify the merkle proof.
            require(
                validClaim(msg.sender, _presaleMaxAmount, _merkleProof),
                "Soul Pass: invalid proof"
            );
    ...
    function _mintElements(address _to, uint256 _amount, bool increaseCount) private {
        ...
        require(totalSupply() <= maxTokenSupply, "Soul Pass: Limit reached");
    }
    ...
    function _payout(address _address, uint256 _amount) private {
        ...
        require(success, "Soul Pass: Transfer failed");
    }
    ...
    function mintReserve(uint256 _count, address _to) public onlyOwner {
        uint256 total = totalSupply();
        require(total <= maxTokenSupply, "Soul Pass: Sale ended");
        require(total + _count <= maxTokenSupply, "Soul Pass: Max limit");
    ...
    function withdrawAll() public payable onlyOwner {
        ...
        require(balance > 0, "Soul Pass: Balance should be above 0");
