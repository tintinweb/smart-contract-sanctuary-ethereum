...
contract Scion is Ownable, ERC721URIStorage, VRFConsumerBaseV2 {
    ...
    mapping(uint256 => uint256) private requestIdToMintPassRarity;
    ...
    constructor(uint64 subscriptionId, address vrfCoordinator, address link, bytes32 _keyHash, address _mintingPass, address _soul) ERC721("SCION", "SCION") VRFConsumerBaseV2(vrfCoordinator) {
    ...
    function requestRandomWords(uint256 mintPassTokenId, uint256 mintPassRarity, uint256 numWords) public onlyOwner returns (uint256 s_requestId) {
    ...
    function fulfillRandomWords(
        ...
        if(requestIdToTokenId[requestId] > 0) {
            if(requestIdToMintPassRarity[requestId] == 0 || requestIdToMintPassRarity[requestId] == 1) {
    ...
    function claimScion(uint256 tokenId) public {
        ...
        uint256 rarity = uint256(mintingPass.getMintingPassData(tokenId));

        // Burning minting pass
        mintingPass.burn(tokenId);

        if(rarity == 0) {
            requestRandomWords(tokenId, rarity, 6);
        } else if(rarity == 1) {
            requestRandomWords(tokenId, rarity, 6);  
        } else if(rarity == 2) {
            requestRandomWords(tokenId, rarity, 6);  
    ...
    function burnForSoul(uint256 tokenId) external {
       ...
       soul.transfer(msg.sender, 100e18);
    }
...
contract MintPasses is Context, ERC721Enumerable, Ownable, ReentrancyGuard, MintPassRarityGenerator {
    ...
    function claimPass() external onlyWhenFinished {
        ...
        require(userBidIds[_msgSender()][userBidIds[_msgSender()].length-1] > latestBidId - totalBidsLimit, "User didn't win auction");

        // A user could have thousands of bids, may provoke a gas problem here
        for(uint i = 0; i < userBidIds[_msgSender()].length; i++) {
  
            if(bids[userBidIds[_msgSender()][i]].bidValue > 0 && userBidIds[_msgSender()][i] > latestBidId - totalBidsLimit) {
                ...
                uint256 position = userBidIds[_msgSender()][i] - (latestBidId - totalBidsLimit);
