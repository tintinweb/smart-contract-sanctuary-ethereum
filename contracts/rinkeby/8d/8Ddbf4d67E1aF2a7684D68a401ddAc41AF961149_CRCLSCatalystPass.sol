...
contract CRCLSCatalystPass is ERC721A, ERC2981Royalties, Ownable, VRFConsumerBaseV2 {
    ...
    Counters.Counter private _retries;
    ...
    constructor(uint256 royaltyPercentage, uint64 linkSubId) ERC721A("CRCLS Catalyst Pass", "OG") VRFConsumerBaseV2(vrfCoordinator) {
        // EIP2987
    ...
    function changeSaleStatus(Status status_) external onlyOwner() {
        saleStatus = status_;
        emit SaleStatus(status_);
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        // transform the result to a number between 1 and totalMinted inclusively
        uint256 tokenId = (randomWords[0] % _totalMinted()) + 1;

        if (!_exists(tokenId)) {
            _retries.increment();

            if (_retries.current() <= 10) {
                this.requestRandomOwner();
            }
