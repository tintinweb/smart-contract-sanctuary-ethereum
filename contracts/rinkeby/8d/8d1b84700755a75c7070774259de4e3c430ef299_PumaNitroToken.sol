...
// File contracts/PumaNitroToken.sol

/**
Smart contract development by @wumbolabs
**/
...
contract PumaNitroToken is ERC721ABurnable, Ownable, ReentrancyGuard {
    using Strings for uint256;

    bytes32 public MERKLEROOT = 0x72b8b19840fe883872963a6ec6701a92e27d051c8988e1f27e185fe80920e241;
    uint256 constant COST = 0 ether;
    ...
    // minting variables
    string public baseURI = "ipfs://QmUHJpkxd8ZwXE9G1tAbDyEk4xa6fptAFjVS6AUQtD2WBh/";
    Status public state;
    ...
    function snapshotClaim() external nonReentrant {
        ...
        require(claimCount <= MAX_SUPPLY, "PUMA Nitro Token: Mint Supply Exceeded");
        holderSnapshot[msg.sender] = 0;
        _safeMint(msg.sender, netAmountClaimable);
    }

    function whitelistClaim(bytes32[] calldata _proof) external nonReentrant {
        require(state == Status.WHITELIST_SNAPSHOT, "PUMA WHITELIST CLAIM NOT ACTIVE");
        require(_numberMinted(msg.sender) < 1, "PUMA Whitelist Claim: Exceeds Max Per Wallet");
    ...
    function publicClaim() external nonReentrant {
        require(state == Status.PUBLIC, "PUMA NITRO TOKEN NOT PUBLIC CLAIM");
        require(_numberMinted(msg.sender) < 1, "PUMA Whitelist Claim: Exceeds Max Per Wallet");
        claimCount++;
        require(claimCount <= MAX_SUPPLY, "PUMA Nitro Token: Mint Supply Exceeded");
        _safeMint(msg.sender, 1);
     ...
     function snapshotAddressClaimableAmount(address _address) external view returns (uint256) {
        return holderSnapshot[_address];
    }
    ...
    function addSnapshot(address _address, uint256 _amount) external onlyOwner {
        require(holderSnapshot[_address] > 0, "Already in whitelist");
        holderSnapshot[_address] = _amount;
    }
  
    function removeSnapshot(address _newEntry) external onlyOwner {
        require(holderSnapshot[_newEntry] == 0, "Previous not in whitelist");
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        return string(abi.encodePacked(base, "pumanitrotoken", ".json"));
    }
