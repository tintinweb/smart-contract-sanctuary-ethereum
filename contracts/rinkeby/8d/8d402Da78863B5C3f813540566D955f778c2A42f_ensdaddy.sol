...
mapping (string => int) public testa;
mapping (string => ensdata) public ensusers;
string[] ensdomains; 

constructor(){}
    
    function register_subdomain(string memory name, string memory subdomain, string memory domain) external payable {
        require(
                 (!_saleIsActive),
                "Minting is not Live"
        );
        bool exist = false;
        if(keccak256(bytes(ensusers[name].subdomain)) == keccak256(bytes(string(abi.encodePacked(subdomain,".", domain)))) ){
            exist = true;
        }
        require(
                (exist),
        ...
        ensusers[name] = ensdata(subdomain, domain, msg.sender, "Under process",msg.value);
        delete subdomain;
        delete exist;
    }
    ...
    /*function all_domains() public view returns (string memory) {
        return ensdomains.length;
    }*/
