...
contract ShellzOrb is
    ...
    ERC721Retreatable
{
    uint256 constant PRICE_PER_TOKEN = 0.125 ether;
    ...
    uint256 public approvalEnableTime;
    ...
    function initialize() public initializer {
        ...
        // Wednesday, August 10, 2022 10:00:00 PM GMT-04:00(EST)
        approvalEnableTime = 1660183200; // Verify @ epochconverter.com
    }
    ...
    /**
        ...
        Time-based approval control: The users cannot list their token on opensea before a give time.

     */

    function isApprovalEnabled() public view returns (bool) {
        return block.timestamp > approvalEnableTime;
    }

    function approve(address to, uint256 tokenId) public override {
        if (!isApprovalEnabled()) {
            revert ApprovalNotEnabled();
        }
    ...
    function setApprovalEnableTime(uint256 approvalEnableTime_)
        external
        onlyOwner
    {
        approvalEnableTime = approvalEnableTime_;
    }
...
contract ERC721Retreatable {
    ...
    function isRetreating(uint256 tokenId) internal view returns(bool) {     
        return _retriveTokenParam(tokenId).retreatingStartTime > 0;
    }

    function retreatingTime(uint256 tokenId) internal view returns (uint256 t){
