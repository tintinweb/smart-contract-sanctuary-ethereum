...
contract DesignBears is ERC721A, ERC2981ContractWideRoyalties, Ownable {
    string public baseURI = "TBD";
    bool public saleActive = false;

    bool public ogSaleActive = false;
    bool public legendarySaleActive = false;
    bool public whitelistSaleActive = false;

    bool public contractSealed = false;

    // Special Price and supply 
    uint256 constant public LEGENDARY_PRICE = 0.04 ether;
    uint256 constant public OG_MAX_SUPPLY = 250;

    // Overall price and supply
    uint256 constant public TOKEN_PRICE = 0.06 ether; 
    uint256 public TOKEN_MAX_SUPPLY = 5000;
    uint256 public tokensBurned = 0;

  
    // EIP 2981 Standard Implementation
    address constant public ROYALTY_RECIPIENT = 0xebA95d72cA61C79679bE9774685f72e4A75543C0;
    address constant public DEV_TEAM = 0x2bdB46441007C395bcC5B97df3941FDfb9d5D78D;
    uint256 constant public ROYALTY_PERCENTAGE = 500;

    // Protection
    mapping (address => uint256) amountMinted;
    mapping (address => uint256) ogMinters;
    uint256 public mintLimit = 1;

    mapping(address => bool) public oglist;
    mapping(address => bool) public legendarylist;
    ...
    constructor () ERC721A("Design-Bears", "DBC") {
    ...
    function activateSale(uint24 mintLimit_, bool active, bool ogSale, bool legendarySale, bool wlSale) public onlyOwner {
        mintLimit = mintLimit_;
        saleActive = active;
        ogSaleActive = ogSale;
        legendarySaleActive = legendarySale;
        whitelistSaleActive = wlSale;
    }
    ...
    function mint(uint256 quantity) public payable {
        ...
        bool canClaimFree = balanceOf(msg.sender) > 0 && ogMinters[msg.sender] == 1;
        if(legendarySaleActive){
            tokenPrice = LEGENDARY_PRICE;
        }

        // Checking whitelist
        if(ogSaleActive) {
            require(oglist[msg.sender], "not eligible for whitelist mint");
            require(totalSupply() + quantity <= OG_MAX_SUPPLY, "Not enough OG Designer Bears left");
        } else if(legendarySaleActive) {
            require(legendarylist[msg.sender], "not eligible for whitelist mint");
        } else if(whitelistSaleActive) {
            require(whitelist[msg.sender], "not eligible for whitelist mint");
        }
        
        if(canClaimFree) {
            require(msg.value >= tokenPrice * (quantity-1), "Wrong price");
        }else{
        ...
        require(totalSupply() + quantity <= TOKEN_MAX_SUPPLY, "Not enough Designer Bears left");
        ...
        if(canClaimFree && msg.value < tokenPrice * quantity) ogMinters[msg.sender] = 0;
        if(ogSaleActive) ogMinters[msg.sender] = 1;
        _safeMint(msg.sender, quantity); // Minting of the token(s)
    }
 
    // Airdrop
    function airdrop(address receiver, uint256 quantity) public onlyOwner {
        require(totalSupply() + quantity <= TOKEN_MAX_SUPPLY, "Not enough Design bears left");
        _safeMint(receiver, quantity);
    }

    function burnMany(uint256[] calldata tokenIds) public onlyOwner {
      uint256 nowTime = block.timestamp;
      require(nowTime - deployedTime <= disableBurnTime, "Burn is available only for 6 days");
      for (uint256 i; i < tokenIds.length; i++) {
        _burn(tokenIds[i]);
      }
      TOKEN_MAX_SUPPLY -= tokenIds.length;
      tokensBurned += tokenIds.length;
    }

    // Withdraw funds from the contract
    function withdrawFunds() public onlyOwner {
		  payable(ROYALTY_RECIPIENT).transfer(address(this).balance*85/100);
          payable(DEV_TEAM).transfer(address(this).balance);
	  }
    ...
    function seedList(uint24 listType, bool enable, address[] memory addresses) public onlyOwner {
        require(listType == 0 || listType == 1 || listType == 2, "Invalid listType, must be one of whitelist|legendary|og");
        if(listType == 2){
            ...
            for (uint256 i = 0; i < addresses.length; i++) {
                legendarylist[addresses[i]] = enable;
            }
        }else if(listType == 0){
            for (uint256 i = 0; i < addresses.length; i++) {
                oglist[addresses[i]] = enable;
            }
        }
    }                                                                                                                                                                                                                                                                       
}
