...
'<text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#F6CE94">',HelperLibrary.toString(numberToDisplay(_tokenId)),'</text>',
  '<style> text{font-size: 40pt; font-weight: 800;}</style></svg>'
    ...
    function numberToDisplay(uint256 _tokenId) private view returns (uint256) {
        uint256 balance = expErc20.balanceOf(ownerOf(_tokenId))/(10**18);
