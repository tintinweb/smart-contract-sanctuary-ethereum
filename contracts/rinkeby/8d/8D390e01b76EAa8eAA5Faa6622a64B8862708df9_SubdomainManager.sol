...
contract SubdomainManager is  IManager, IERC721Receiver, Ownable, ERC165, ERC721, IERC2981, ENSEnumerable {
    ...
    IENSAdmin public PublicResolver = IENSAdmin(0xf6305c19e814d2a75429Fd637d01F7ee0E77d615);
...
function depositEns(string calldata _label, uint256 _royalty, uint256 _mintPrice, string calldata _defaultImageUri) payable external {
    depositEns(_label, _royalty, _mintPrice, _defaultImageUri, DefaultRegister);
}
    ...
    function depositEns(string calldata _label, uint256 _royalty, uint256 _mintPrice, string calldata _defaultImageUri, IRegister _registrationStrategy) payable public {
    ...
    function registerSubdomain(uint256 _id, string memory _label) payable public {
        address owner = TokenOwnerMap[_id];
        //owner can always mint sub-domains of their TLD
        if (msg.sender != owner){
            require(DefaultRegister.canRegister(_id, _label, msg.sender, msg.value));
        }
    ...
    function transferDomainOwnership(uint256 _id, address _newOwner) public {       
...
contract RegistrationRulesV1 is IRegister {
    ...
    function canRegister(uint256 _tokenId, string memory _label, address _addr, uint256 priceInWei) external view returns(bool){
        require(DomainManager.DefaultMintPrice(_tokenId) == priceInWei, "incorrect ether");
        require(DomainManager.DefaultMintPrice(_tokenId) != 0, "not for primary sale");
        return true;
    }
}
...
contract EnsTldWrapper is ERC721, Ownable {
    ...
    SubdomainManager public DomainManager;
    string public BaseUri;

    constructor(SubdomainManager _manager) ERC721("Wrapped ENS", "WENS"){
        DomainManager = _manager;
    }
   ...
   modifier isDomainManager() {
        require(address(DomainManager) == msg.sender, "is not owner");
      _;
   }
