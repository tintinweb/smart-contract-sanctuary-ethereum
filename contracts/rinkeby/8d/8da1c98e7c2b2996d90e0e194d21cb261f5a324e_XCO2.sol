...
contract XCO2 is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    uint[3] private discountPercents;
    uint[2] private bulkDiscountAmounts;
    ...
    function initialize() external initializer{
        __ERC20_init("XC02 Coin", "XC02");
        __Ownable_init();
        __ERC20Burnable_init();
        __Pausable_init();
        pricePerEther = 400;
        discountPercents = [10, 20, 10]; //[tier 1 bulk 1, tier 2 bulk, mint/retire ]
        bulkDiscountAmounts = [100 * 10 ** 18, 10000 * 10 ** 18];
        totalMinted = 0;
        totalRetired = 0;
        txNum = 0;
    ...
    {
        require(amount > 0);
        uint price = pricePerEther;
        if (amount > bulkDiscountAmounts[1]) {
            price += price * discountPercents[1] / 100;
        } else if (amount > bulkDiscountAmounts[0]) {
            price += price * discountPercents[0] / 100;
        }
        require(msg.value == amount / price);
        require(totalSupply() + amount <= maxSupply);
        uint retAmount = amount * 5 / 100; //5 percent
        totalMinted += amount + retAmount;
        totalRetired += retAmount;
        retiredBalanceOf[address(this)] += retAmount;
        _mint(msg.sender, amount);
        txNum++;
        emit Retired(txNum, msg.sender, retAmount, "mint");
    }

    function mintAndBurn(uint amount) 
        public 
        whenNotPaused 
        payable 
    {
        require(amount > 0);
        uint price = pricePerEther + (pricePerEther * discountPercents[2]);
        if (amount > bulkDiscountAmounts[1]) {
            price += price * discountPercents[1] / 100;
        } else if (amount > bulkDiscountAmounts[0]) {
            price += price * discountPercents[0] / 100;
        ...
        txNum++;
        emit Retired(txNum, msg.sender, amount, "mint_burn");
    }
    ...
    function setPrice(uint price) public onlyOwner {
     ...
     function setDiscountPercents(uint bulk1, uint bulk2, uint mintretire) public onlyOwner {
        discountPercents[0] = bulk1;
        discountPercents[1] = bulk2;
        discountPercents[2] = mintretire;
    }
     function setBulkDiscountAmounts(uint bulkAmount1, uint bulkAmount2) public onlyOwner {
        bulkDiscountAmounts[0] = bulkAmount1;
        bulkDiscountAmounts[0] = bulkAmount2;
    }
    ...
    function genCert(address addr) public view returns (string memory) {
        ...
        return string(
            abi.encodePacked(
                ...
                '<text x="15" y="57" class="b small">', StringsUpgradeable.toString(retiredBalanceOf[addr]), ' Tons Offset</text>',
