...
contract GardenOfEther is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public massReserveForShards = 1500 * 1 ether;
    uint256 public massReserveForSplinters = 375 * 1 ether;
    ...
    // for accurate reward
    mapping(uint8 => uint256) private _rewardMapper;

    mapping(uint8 => mapping(uint8 => string)) private _stakeImageMapper;
    mapping(uint8 => mapping(uint8 => string)) private _unstakeImageMapper;
    ...
    mapping(uint8 => string) private _description;
    ...
    constructor() ERC721A("Garden Of Ether", "GOE") {
        ...
        _reserveAdjuster[0] = 1500;
        _reserveAdjuster[1] = 375;
        glowingOrDarkToUint["Glowing"] = 0;
        glowingOrDarkToUint["Dark"] = 1;
        _uintToType[0] = "Shard";
        _uintToType[1] = "Splinter";
        typeToUint["Shard"] = 0;
        typeToUint["Splinter"] = 1;
        _description[0] = "This is the Description for the Shards!";
        _description[1] = "This is the Description for the Splinters!";
        _rewardMapper[0] = 200;
        _rewardMapper[1] = 100;
        ...
        _stakeImageMapper[0][
            0
        ] = "https://gateway.pinata.cloud/ipfs/QmWarri1mTVanoYz1YBP2UjdKPdxzCmPGEaRcCepUjkm5Y"; // Glowing Shard
        _stakeImageMapper[1][
            0
        ] = "https://gateway.pinata.cloud/ipfs/QmNiz9j3B54WsWzqRiBmm5neoMLBSijzdueKedQmfrTES2"; // Dark Shard
        _stakeImageMapper[0][
            1
        ] = "https://gateway.pinata.cloud/ipfs/QmWWzPT1j67UcVE16pwPSY4JsjcSpebAggPh9q6JDcMXJN"; // Glowing Splinter
        _stakeImageMapper[1][
            1
        ] = "https://gateway.pinata.cloud/ipfs/QmWWzPT1j67UcVE16pwPSY4JsjcSpebAggPh9q6JDcMXJN"; // Dark Splinter
        _unstakeImageMapper[0][
            0
        ] = "https://gateway.pinata.cloud/ipfs/QmWWzPT1j67UcVE16pwPSY4JsjcSpebAggPh9q6JDcMXJN"; // Glowing Shard
        _unstakeImageMapper[1][
            0
        ] = "https://gateway.pinata.cloud/ipfs/QmWWzPT1j67UcVE16pwPSY4JsjcSpebAggPh9q6JDcMXJN"; // Dark Shard
        _unstakeImageMapper[0][
            1
        ] = "https://gateway.pinata.cloud/ipfs/QmWWzPT1j67UcVE16pwPSY4JsjcSpebAggPh9q6JDcMXJN"; // ["Glowing"]["Splinter"
        _unstakeImageMapper[1][
            1
        ] = "https://gateway.pinata.cloud/ipfs/QmWWzPT1j67UcVE16pwPSY4JsjcSpebAggPh9q6JDcMXJN"; // needs to change // "Dark""Splinter"
    ...
    function changeState(uint256 _tokenId, uint8 _glowOrDark) external {
        ...
        if (mapToToken[_tokenId].state == _glowOrDark)
            revert AlreadyYourState();
        if (_glowOrDark > 3) revert NotAllowed();
        if (mapToToken[_tokenId].tokenType == 0) {
        ...
        mapToToken[_tokenId].state = _glowOrDark;
    }

    function manipulateReserveOfTokenBySub(
        uint256 _tokenId,
        uint256 _reserveToBeDecreasedBy
    ) internal {
    ...
    function initiateDepletion(uint256 _tokenId) internal {
        ...
        if (mapToToken[_tokenId].massReserve == 0) {
            uint8 tokenType = mapToToken[_tokenId].tokenType;
            uint8 tokenState = mapToToken[_tokenId].state;
    ...
    function editDescriptionForItems(
        ...
        if (isFrozen) revert FrozenForever();
        _description[0] = _newDescForShards;
        _description[1] = _newDescForSplinters;
    }
    ...
    {
        return mapToToken[_tokenId].image;
    }

    function readAnimationVideoForToken(uint256 _tokenId)
        external
        view
        returns (string memory)
    {
        return _animationUrlMapper[mapToToken[_tokenId].image];
    }
    ...
    function readTypeOfToken(uint256 _tokenId)
        ...
        returns (string memory)
    {
        return _uintToType[mapToToken[_tokenId].tokenType];
    }
    ...
    function readStateOfToken(uint256 _tokenId)
        ...
        returns (string memory)
    {
        return glowingOrDarkName[mapToToken[_tokenId].state];
    }
        ...
        returns (string memory)
    {
        ...
        string memory reserveAdjuster = _reserveAdjuster[mapToToken[tokenId].tokenType].toString();
        string memory state;
        string memory state2 = glowingOrDarkName[mapToToken[tokenId].state];
        string memory tokenType = _uintToType[mapToToken[tokenId].tokenType];

        if (isTokenStaked[tokenId]) {
            state = "Bathed";
        } else {
            state = glowingOrDarkName[mapToToken[tokenId].state];
        }
        string memory reserve = getExtract(tokenId);
    ...
    function callZkSync(
        ...
        if (_recentGlowingAmount[msg.sender] == _glowingAmount)
            revert NotAllowed();
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            tokenId = tokenIds[i];
            require(
                ownerOf(tokenId) == msg.sender && msg.sender == account,
                "Not an owner"
            );
    ...
    function _claim(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (mapToToken[tokenId].tokenType == 1) {
                uint256 depleteRate = 0.5 * 1 ether;
                massDepletion =
                    (depleteRate * (block.timestamp - stakedAt)) /
                    1 days /
                    2;
            } else {
                massDepletion = (1 * (block.timestamp - stakedAt)) / 1 days;
            }

            if (mapToToken[tokenId].state == 0) {
                glowingAmount +=
                    (1 ether *
    ...
    function readWaterToBeClaimedForToken(uint256 _tokenId)
        external
        view
        returns (uint256)
    {}
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (mapToToken[tokenId].tokenType == 1) {
                massDepletion = (1 * (block.timestamp - stakedAt)) / 1 days /2;
            } else {
                massDepletion = (1 * (block.timestamp - stakedAt)) / 1 days;

            }
        }
        return massDepletion;
    }
    ...
    function viewEarningOfSplinters() external view returns (uint256) {
        return _rewardMapper[1];
    }

    function viewEarningOfShards() external view returns (uint256) {
        return _rewardMapper[0];
    }

    // depletion for shard and splinter
    function viewDepletionOfSplinters() external pure returns (string memory) {
        return "1/2 g per day";
    }

    function viewDepletionOfShards() external pure returns (string memory) {
        return "1 g per day";
    }
    ...
    // staking part end
    function displayReserveToBeDepleted(uint256 _tokenId) public view returns(uint256){
            if(!isTokenStaked[_tokenId]){
                return 0;
            }else{ 
            uint256 massDepletion;
            if (mapToToken[_tokenId].tokenType == 1) {
                massDepletion = (0.5 * 1 ether * (block.timestamp - tokenTimestamp[_tokenId])) / 1 days;
            } else {
                massDepletion = (1 * 1 ether * (block.timestamp - tokenTimestamp[_tokenId])) / 1 days;
            }
            return massDepletion;
        }
    }

    function getSlice(uint256 begin, uint256 end, uint256 _reserve) public pure returns (string memory) {
        string memory text = Strings.toString(_reserve);
    ...
    function getExtract(uint256 tokenId) public view returns (string memory) {
        uint256 reserve = mapToToken[tokenId].massReserve - displayReserveToBeDepleted(tokenId);
        uint256 end;
        if(reserve > 1000 * 1 ether)
        {
            end = 4;
        }else{
            end = 3;
        ...
        string memory _reserve = Strings.toString(reserve);
        bytes memory reserveDisplay = new bytes(end-1+1);
        for(uint i=0;i<=end-1;i++){
            reserveDisplay[i] = bytes(_reserve)[i+1-1];
        }
        return string(reserveDisplay);    
    }
...
// Iterate through String first four numbers get returned
