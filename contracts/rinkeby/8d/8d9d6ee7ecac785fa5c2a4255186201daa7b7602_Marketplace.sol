...
contract Marketplace is Ownable {
    ...
    uint256[] public checkedIds;
    int256[] public checkedIdsInt;
    ...
    function balanceOf(address alice) public returns (uint256) {
        ...
        for (uint256 i = 0; i < 3333; i++) {
            ...
            if (id != -1) {
                checkedIds.push(uint256(id));
                checkedIdsInt.push(id);
                sum += earned(uint256(id));
            }
        }
    ...
    function stakedTokensBy(address maybeOwner)
        ...
        returns (int256[3333] memory)
    {
        return staking.stakedTokensBy(maybeOwner);
    }
