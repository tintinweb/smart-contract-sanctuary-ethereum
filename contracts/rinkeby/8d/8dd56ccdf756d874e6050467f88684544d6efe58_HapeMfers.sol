...
contract HapeMfers is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public constant FREE_MINT_LIMIT = 10;
    ...
    function freeMint(uint256 nMints) external nonReentrant {
        require(SALE_IS_ACTIVE, "Minting is not active now");
        require(nMints <= FREE_MINT_LIMIT, "Exceeds free mint limit");
        require(msg.sender == tx.origin, "Can't mint through another contract");
        require(balanceOf(msg.sender) + nMints <= MAX_MINTS, "Exceeds mint limit");
        require(totalSupply() + nMints <= FREE_MINT_SUPPLY, "Exceeds free mint limit(only 200)");
    ...
    function mint(uint256 nMints) external payable nonReentrant {
        require(SALE_IS_ACTIVE, "Minting is not active now");
        require(totalSupply() >= FREE_MINT_SUPPLY, "Now is free mint time!");
        require(msg.sender == tx.origin, "Can't mint through another contract");
        require(balanceOf(msg.sender) + nMints <= MAX_MINTS, "Exceeds mint limit");
