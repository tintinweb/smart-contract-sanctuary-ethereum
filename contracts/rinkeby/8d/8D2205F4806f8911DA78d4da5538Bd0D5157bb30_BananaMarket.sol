...
contract BananaMarket is  Ownable, ReentrancyGuard{
    ...
    event SaleComplete(uint256 indexed itemId, address token, uint256 amount, uint256 price, address indexed buyer, bool indexed sold );

    constructor(){}

    ////// Token / Market Management ///
    function addAllowedToken(IERC20 _address) public onlyOwner {      
        s_bananas = _address;
    }
    function openMarket() onlyOwner public{
    ...
    function purchaseItem(uint256 _itemId) external payable nonReentrant{
        ...
        emit SaleComplete(item.itemId, address(item.token), item.amount, item.totalPrice, msg.sender, true);
    }
