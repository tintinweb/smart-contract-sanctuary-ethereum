...
library stringsExt {
    ...
    function concatBytes(slice memory self, slice memory other) internal pure returns (bytes memory) {
    ...
    function joinBytes(slice memory self, slice[] memory parts) internal pure returns (bytes memory) {
        if (parts.length == 0)
            return "";
