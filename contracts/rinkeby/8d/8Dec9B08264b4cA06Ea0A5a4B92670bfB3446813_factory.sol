...
contract factory is Ownable{
    ...
    event RewardReceived(uint256 _amount, address _beneficiary, uint256 _timestamp);
    event RewardWithdrawn(uint256 _amount, address _beneficiary, uint256 _timestamp);
    ...
    function importRewards(address _user, uint256 _amount) external onlyStakingContract{
    ...
    function withdrawRewards(uint _index) public{
        require(userRewards[msg.sender][_index].amount > 0, "No rewards to withdraw");
        require(block.timestamp > (userRewards[msg.sender][_index].claimTime + block.timestamp), "funds are locked");
    ...
    function allRewards(address _user) public view returns(Reward[] memory rewards) {
