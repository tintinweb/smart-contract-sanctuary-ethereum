...
/**
 * @title GameNFTs
 * GameNFTs - ERC1155 contract has create and mint functionality, and supports useful standards from OpenZeppelin,
  like _exists(), name(), symbol(), and totalSupply()
 */
contract Breed is ERC1155, Ownable {
    ...
    // mapping(uint256 => address) public babyOwner;
    // mapping(address => uint256) ownsNumberOfTokens;
    // mapping(uint256 => address) public approvalOneBaby;
    // //which baby is approved to be transfered by an address other than the owner
    ...
    //     require(
    //         babyOwner[_dadId] == msg.sender && babyOwner[_mumId] == msg.sender,
    //         "You can't breed what you don't own"
    //     );

    //     (uint256 _dadDna, , , , uint256 _dadGeneration) = getBaby(_dadId); //discarding redundant data here
    //     (uint256 _mumDna, , , , uint256 _mumGeneration) = getBaby(_mumId); //discarding redundant data here
    //     uint256 _newDna = _mixDna(
    //         _dadDna,
    //         _mumDna,
    //         uint8(block.timestamp % 255), //This will return a number 0-255. e.g. 10111000
    //         uint8(block.timestamp % 1), //seventeenth digit
    //         uint8(block.timestamp % 7), //number to select random pair.
    //         uint8((block.timestamp % 89) + 10) //value of random pair, making sure there's no leading '0'.
    //     );
    ...
    //     return _createBaby(_mumId, _dadId, _newGeneration, _newDna, msg.sender);
    // }
    ...
    //     });
    //     babies.push(_baby);
    //     uint256 newBabyId = SafeMath.sub(babies.length, 1); //want to start with zero.
    //     _transfer(address(0), _owner, newBabyId); //transfer from nowhere. Creation event.
    //     emit Birth(_owner, newBabyId, _mumId, _dadId, _genes);
    //     return newBabyId;
    // }
    ...
    //     require(babyOwner[tokenId] == msg.sender);
    ...
    //     babyOwner[_tokenId] = _to;

    //     if (_from != address(0)) {
    //         ownsNumberOfTokens[_from] = SafeMath.sub(
    //             ownsNumberOfTokens[_from],
    //             1
    //         );
    //         delete approvalOneBaby[_tokenId]; //when owner changes, approval must be removed.
    //     }
    // }

    // function approve(address _approved, uint256 _tokenId) external {
    //     require(
    //         babyOwner[_tokenId] == msg.sender ||
    //             _operatorApprovals[babyOwner[_tokenId]][msg.sender] == true,
    //         "You are not authorized to access this function."
    //     );
    //     approvalOneBaby[_tokenId] = _approved;
    // }

    // function getBaby(uint256 tokenId)
    //     public
    //     view
    //     returns (
    //         uint256 genes,
    ...
    //     )
    // {
    //     require(tokenId < babies.length, "Token ID doesn't exist.");
    //     Baby storage baby = babies[tokenId]; //saves space over using memory, which would make a copy

    //     genes = baby.genes;
    //     birthTime = uint256(baby.birthTime);
    //     mumId = uint256(baby.mumId);
    //     dadId = uint256(baby.dadId);
    //     generation = uint256(baby.generation);
    // }
