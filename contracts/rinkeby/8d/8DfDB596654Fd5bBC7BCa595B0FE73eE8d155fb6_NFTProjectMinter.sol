...
// Interface to interact our main NFT contract
interface NFT{
    function mint(address _address, uint256 _mintAmount) external;
}
//

contract NFTProjectMinter is Ownable, ReentrancyGuard{
    address public NFTAddress = 0x264C75eE156BC0daB2bad73142B6308C30f85267;
    ...
    mapping (address => uint256) ogMintedAmount;
    ...
    function ogMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable isPaused nonReentrant {
        ...
        require(MerkleProof.verify(_merkleProof, ogRoot, leaf), "User is not allowed to mint");
        ...
        // Increment Total Supply and Minted Amount Before the Mint Process
        totalSupply += _mintAmount;
        ogMintedAmount[msg.sender] += _mintAmount;

        // Start the Mint Process By Using Main Contract's Interface
        NFT(NFTAddress).mint(msg.sender, _mintAmount);
    }
    ...
    function setContractAddress(address _newContractAddress) public onlyOwner {
        NFTAddress = _newContractAddress;
    }
