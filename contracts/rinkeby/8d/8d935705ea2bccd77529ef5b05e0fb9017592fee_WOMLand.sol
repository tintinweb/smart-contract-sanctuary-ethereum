...
contract WOMLand is ERC721, Ownable {
  ...
  uint256 public maxMintPerAddress = 50;
   ...
   // uint256 public maxMintsPerTxOpen = 10;
    ...
    function mintBasicLand(uint256 quantity) external payable {
        ...
        //require(quantity <= maxMintsPerTxOpen, "trying to mint too many at a time");
 ...
 //       require(quantity <= maxMintsPerTxOpen, "trying to mint too many at a time");
     require(msg.value >= tokenPriceSupremeLand * quantity, "not enough ether sent");
        require(openMint, "mint not open");
        require(!onlyWhitelisted, "not open to the public yet");
      //  require(!mintedSupremeLand[msg.sender], "can only mint 1 SupremeLand per wallet");
        require(msg.sender == tx.origin, "no contracts please");
         for (uint256 i = 0; i < quantity; i++) {
    ...
    function mintSupremeLandWhitelist() external payable {
        ...
        //require(!mintedPenthouse[msg.sender], "can only mint 1 penthouse per wallet");
        require(msg.sender == tx.origin, "no contracts please");
 ...
 function withdraw() public payable onlyOwner {
    ...
    require(founder2);
    (bool collabYan, ) = payable(0x69abC6eAE2362aCEe665Db1c6790Cc0F90C5c397).call{value: founderBalance * 25 / 1000}("");
    require(collabYan);
...
function setWhitelistSupremeLand(address[] calldata addresses, uint8 numAllowedToMintSupremeLand) external onlyOwner {
    ...
    function numAvailableToMintSpecialLand(address addr) external view returns (uint8) {
        return whitelistSpecialLand[addr];
    }

 function numAvailableToMintSupremeLand(address addr) external view returns (uint8) {
        return whitelistSupremeLand[addr];
    }
       function settokenPriceBasicLand(uint256 _newBasicLandCost) public onlyOwner {
        tokenPriceBasicLand = _newBasicLandCost;
    }
