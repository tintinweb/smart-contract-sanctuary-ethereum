...
contract ThankyouNFT is ERC721URIStorage {
    using Counters for Counters.Counter;
    Counters.Counter internal _tokenIds;
    address payable internal DuncanPeachWallet;
    string[] internal PossibleNFTS;

    constructor(string[] memory _PossibleNFTS) ERC721("ThankYouCards", "Card") {
        DuncanPeachWallet = payable(msg.sender);
        PossibleNFTS = _PossibleNFTS;
    }

    function CreateThankyouNFT() public {
    ...
    function GetRandom_ISH_NFT() internal onlyDuncanPeach returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
