...
contract Metavatar is ERC721, ReentrancyGuard, Ownable {
    ...
    constructor() ERC721("Metavatar", "MVTR") {
        console.log("Metavatar on-chain smart contract");
        _tokenIds.increment(); // start from 1
    }

    function _mint(address destination) private {
        uint256 newItemId = _tokenIds.current();
        console.log("Going to mint NFT for ", destination);
        _safeMint(destination, newItemId);
        _tokenIds.increment();
        _mintedPerAddress[destination] += 1;
    ...
    function _bulkMint(uint256 numTokens, address destination) private {
        ...
        console.log("%s tokens minted for address %s", numTokens, destination);
    }
        ...
        returns (string memory)
    {
        ...
        MetavatarGenerator.MetavatarStruct memory metav;
        string memory svg;
        // string memory seed = Strings.toString(tokenId * 1234);
        (metav, svg) = MetavatarGenerator.genMetavatarWithSeed(
            Strings.toString(seed[tokenId])
        );
        console.log("Number of shapes", metav.numShapes);
