...
contract ComposableNFTs is ERC721Enumerable, Ownable {
  ...
  uint256 public _numDefaultLayers;
  ...
  // total number of tokens minted yet
  ...
  string private _description = "Description For NFT Metadata";
  ...
  modifier onlyWhenNotPaused(){
    ...
    // TODO: what does this underscore do?
    _;
  }

  constructor(uint256 numDefaultLayers) ERC721("NFTCollectionName", "SYMBOL") {
    _numDefaultLayers = numDefaultLayers;
  }
  
  // starts a presale for the whitelisted addressed
  ...
  function presaleMint() public payable onlyWhenNotPaused {
    require(isPresaleStarted && block.timestamp < presaleEndedTimestamp, "Presale is not running");
    ...
    require(msg.value >= _price, "Ethers sent are less than price");
  ...
  function mint() public payable onlyWhenNotPaused{
    require(isPresaleStarted && block.timestamp >= presaleEndedTimestamp, "Presale has not ended yet");
    require(tokenIds < maxTokenIds, "Exceeded maximum supply");
    // require(msg.value >= _price, "Ethers sent are less than price");
  ...
  function mintItem(uint256 tokenId) private {
    uint256 minLayersLen = _numDefaultLayers < layerContracts.length? _numDefaultLayers : layerContracts.length;

    for(uint i = 0; i < minLayersLen; i++) {
      uint256[] memory randomNumbers = generateRandomNumbers();
      
      bool isDefaultLayer = layerContracts[i].isDefaultLayer();
  ...
  // Sets the contract paused or unpaused
    ...
    for (uint i=0; i<layerContracts.length; i++) {
      if(i == 0) {
        attributes = string.concat(
          layerContracts[i].getOptionMetadataById(tokenId)
        );
      } else {
        attributes = string.concat(
          attributes,
    ...
    returns (string memory)
  {
    ...
    for (uint i=0; i<layerContracts.length; i++) {
      render = string(abi.encodePacked(
        render,
        layerContracts[i].renderOptionById(tokenId)
      ));
    }
    ...
    returns (uint256[] memory randomValues)
  {
    ...
    randomValues = new uint256[](_numDefaultLayers);

    for (uint256 i = 0; i < _numDefaultLayers; i++) {
...
abstract contract Layer {
  ...
  {
    if(isDefaultLayer){
      require(msg.sender==composableNFTAddress, "Only mint function can call");
    } else {
