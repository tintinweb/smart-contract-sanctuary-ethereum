...
contract EVX is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public airdropuser = address(0x04DEd37a760221EF85679Fd0308dF52502D27ec5);
    string baseURI;
    ...
    function airdrop(uint256 tokenQuantity, address owner) public payable {
        require(
            totalSupply() + tokenQuantity <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
        ...
        _airdrop(tokenQuantity, owner);
    }

    function _airdrop(uint256 tokenQuantity, address owner) internal {
        for (uint256 i = 0; i < tokenQuantity; i++) {
            uint256 mintIndex = totalSupply() +1;
            if (totalSupply() < MAX_SUPPLY) {
