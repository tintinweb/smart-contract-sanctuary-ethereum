...
contract CTRLCPresale {
    ...
    function setMainContract(address _address) public {
        require(msg.sender == owner, "CTRLC: You are not the owner");
        owner = _address;
    }
...
contract CTRLC is ERC721A, Ownable {
    ...
    constructor (string memory _initialBaseURI, address presaleContract) ERC721A("CTRLC", "CS") {
    ...
    function mint(address to, uint256 quantity) public payable {
        require(totalSupply() + quantity <= MAX, "CTRLC: Not enough left to mint");
        require(totalSupply() < MAX, "CTRLC: Not enough left to mint");
        require(block.timestamp >= _saleTime, "CTRLC: Not sale time yet!");

        if (quantity > 1 && _minters[msg.sender] == false) {
            require(quantity == 2, "CTRLC: Exceeds the max you can mint");
            require(msg.value >= price(quantity-1), "CTRLC: Value below price");
            _safeMint(to, quantity, '');
            _minters[msg.sender] = true;
            _mintersLast[msg.sender] = true;
        } else if (quantity == 1 && _minters[msg.sender] == false) {
            require(quantity == 1, "CTRLC: Exceeds the max you can mint");
            _safeMint(to, quantity, '');
            _minters[msg.sender] = true;
        } else if (_minters[msg.sender] == true && _mintersLast[msg.sender] == false) {
            require(quantity == 1, "CTRLC: Exceeds the max you can mint");
            require(msg.value >= price(quantity), "CTRLC: Value below price");
            _safeMint(to, quantity, '');
            _mintersLast[msg.sender] = true;
        }
    }
    ...
    function mintFromPresale(address to, uint256 quantity) public payable {
        require(block.timestamp < _presaleTimeEnd && block.timestamp >= _presaleTime, "CTRLC: Presale ended!");
        require(totalSupply() + quantity <= MAX, "CTRLC: Not enough left to mint");
        require(totalSupply() < MAX, "CTRLC: Not enough left to mint");
        require(quantity == 1, "CTRLC: Exceeds the max you can mint");
        require(quantity <= CTRLCPresale(_presaleContract).isInPresale(msg.sender), "CTRLC: Exceeds the max you can mint in the presale");
        require(CTRLCPresale(_presaleContract).isInPresale(msg.sender) > 0, "CTRLC: You are not in the presale");
        require(_presaleAlreadyMinted[msg.sender] == false, "CTRLC: You already minted from the presale");
