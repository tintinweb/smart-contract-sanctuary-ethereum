...
contract NounsAnonDAOLogicV1 is NounsAnonDAOStorageV1, NounsAnonDAOEvents {
        ...
        returns (uint96)
    {
        require(
            semaphore != address(0),
            "NounsDAO::castAnonVoteInternal: the semaphore contract is not set."
        );

        require(
            msg.sender == semaphore,
            "NounsDAO::castAnonVoteInternal: only the semaphore contract can this function"
        );
        ...
        require(
            support <= 2,
            "NounsDAO::castAnonVoteInternal: invalid vote type"
        );
        Proposal storage proposal = proposals[proposalId];
        require(
