...
contract VirgoInstantLiquidity is Ownable, Pausable, ReentrancyGuard {
   ...
   //Get the balance of contract
    ...
    //start a eth withdraw request
    function requestWithdrawETH(address _recipient, uint256 amount) public{
        ...
        emit TransactionCreated(_msgSender(), _recipient, amount, transactionId);
    }   
    ...
    function delegate(address delegateTo) public isManager{  
        require(delegateTo !=_msgSender(),"You can not delegate to yourself");
        require(!managers[delegateTo],"You can not delegate to other manager");
        managers[delegateTo] = true;  
        managerIndexes[delegateTo] = managerIndexes[_msgSender()];
    ...
    function closeTransactions(uint256 transacionId) public isManager{        
        require(!transactions[transacionId].isEnd, "This transaction is closed");
        uint256 temp = 0;
        ...
        transactions[transacionId].isEnd = true;   
    }
