...
// File: contracts/new.sol
    ...
    contract NNCQFS is ERC721, Ownable, ERC721Enumerable {
      ...
      mapping(address => bool) public _demandList;
    ...
    //   uint256 [] myArr;
    //   function random() public view returns(uint[] memory){
    ...
    //     return result;        
    // }
      // function examples(uint256 mod) external  payable {
     
      //   // myArr.push(get_random);
      //   for(uint256 i = 0 ;i<5;i++){
      //     if(myArr[i] != get_random){
      //     uint256 get_random = uint256(keccak256(abi.encodePacked(block.timestamp,block.difficulty,msg.sender))) & mod;
      //     myArr.push(get_random);
      ...
      // function randModules(uint256 mod) public view returns(uint){
     ...
     function addToPresaleList(address addr ) external onlyOwner {
        require(addr != address(0), "Can't add the null address");
        require(!_presaleList[addr], "You have already on the Presale List");
        _presaleList[addr] = true;
      }
      function removeFromPresaleList(address addr ) external onlyOwner {
        require(addr != address(0), "Can't add the null address");
        _presaleList[addr] = false;
      }

      function addToDemandList(address addr ) public {
        require(addr != address(0), "Can't add the null address");
        require(!_demandList[addr], "You have already on the Demand List");
        _demandList[addr] = true;
      }
      
      function removeFromDemandList(address addr ) external onlyOwner {
        require(addr != address(0), "Can't add the null address");
        _demandList[addr] = false;
      }
    ...
    //  function setPresaleListRandomly() external onlyOwner{
    //     get_number= randModules(maxMint);
    //     _demandList[msg.sender][get_number];
    ...
    //   }
      ...
      function onPreDemandList(address addr) external view returns (bool) {
        return _demandList[addr];
      }
