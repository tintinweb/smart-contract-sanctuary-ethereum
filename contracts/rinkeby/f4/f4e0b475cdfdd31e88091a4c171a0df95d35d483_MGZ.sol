...
contract MGZ is ERC721A, Ownable {
  ...
  function wlMint(uint8 _mintAmount, bytes32[] calldata _proof) public payable {
    ...
    require(MerkleProof.verify(_proof, wlMerkleRoot, leaf), "Invalid proof!");

    walletBuys[msg.sender] += _mintAmount;
    _safeMint(msg.sender, _mintAmount);
  }

  function ogMint(uint8 _mintAmount, bytes32[] calldata _proof) public payable {
    ...
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_proof, ogMerkleRoot, leaf), "Invalid proof!");
