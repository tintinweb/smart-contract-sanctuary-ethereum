...
contract Forwarder is EIP712, Whitelist {
  ...
  bytes32 private constant _TYPEHASH = keccak256("MetaTransaction(address from,address to,uint256 value,uint256 gas,uint256 nonce,bytes data)");
  ...
  function verify(MetaTransaction calldata req, bytes calldata signature) public view returns (bool) {
  ...
  function execute(MetaTransaction calldata req, bytes calldata signature) public payable returns (bool, bytes memory) {
    require(isWhiteListed(msg.sender), "Sender of the meta-tx is not whitelisted");
...
contract Whitelist is Ownable {
  ...
  function addSenderToWhitelist(address sender) public onlyOwner {
    require(!isWhiteListed(sender), "Sender is already whitelisted");
