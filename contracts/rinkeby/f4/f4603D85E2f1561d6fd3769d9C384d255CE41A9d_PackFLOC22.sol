    ...
    ERC2771ContextUpgradeable
{
    ...
    function setAvailableNFT(
        ...
        AVAILABLE_NFT memory newNFT = AVAILABLE_NFT(price, reward, metadata);
        AVAILABLE_NFTS.push(newNFT);
    }
        ...
        onlyRole(UPDATER_ROLE)
    {
        require(
            ...
            "Only accepts integers from 1 to 100"
        );

        AVAILABLE_DISCOUNT memory newDISCOUNT = AVAILABLE_DISCOUNT(
            _amount,
            _discount
        );
        AVAILABLE_DISCOUNTS.push(newDISCOUNT);
    }
    ...
    function safeMint(address to, uint256 ID) public payable {

        uint256 birrasBalance = getBirrasBalance(msg.sender);

        if (birrasBalance > 0) {
            require(
                msg.value >= getAvailableDiscountForToken(ID, msg.sender),
        ...
        ERC20Upgradeable(discountTokenAddress).transferFrom(address(this), to, AVAILABLE_NFTS[ID].token_reward);
    }
    ...
    function getBirrasBalance(address _address) public view returns (uint256) {
        ERC20Upgradeable discountToken = ERC20Upgradeable(discountTokenAddress);

        return discountToken.balanceOf(_address);
    }
