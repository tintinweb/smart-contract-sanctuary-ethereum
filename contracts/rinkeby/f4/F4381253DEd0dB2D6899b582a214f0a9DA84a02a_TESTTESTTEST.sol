...
contract TESTTESTTEST is AdminControl, ICreatorExtensionTokenURI, ERC721 {
    address private _creator;
    address payable[] splittingWallets;
    ...
    ContractAttributes private data;
    ...
    constructor(
        ...
        data._price = 0.01 ether;
        data._isActive = true;
        _creator = creator;
    }

    modifier collectableIsActive() {
        require(data._isActive == true);
        _;
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(AdminControl, IERC165, ERC721)
        returns (bool)
    {
        return
    ...
    function setIsActive(bool isActive) public adminRequired {
        data._isActive = isActive;
    }
    ...
    function overrideMetadata(
        ...
        nftHolderAttributes[token] = NFTAttributes({
            name: name,
            imageURI: image,
    ...
    function mintCollectable(
        ...
        string memory description,
        string memory attributes
    ) public payable collectableIsActive {
        require(msg.value >= data._price, "Not enough ether to purchase NFTs.");

        nftHolderAttributes[data._tokenIds] = NFTAttributes({
            name: name,
            imageURI: image,
            description: description,
            attributes: attributes
    ...
    function getBalance() public view adminRequired returns (uint256) {
    ...
    function splitWithdraw(address payable _to, uint256 amount)
        public
        payable
        adminRequired
    {
    ...
    function setSplitting(address payable wallet, uint8 percent)
        public
        adminRequired
    {
        splittingWallets.push(wallet);
        percents[wallet] = percent;
    }

    function payout() private adminRequired {
        uint256 amountperPercent = msg.value / 100;
        for (uint8 i = 0; i < splittingWallets.length; i++) {
            uint256 percent = percents[splittingWallets[i]];
            address payable reviever = splittingWallets[i];
            uint256 valueTransfer = percent * amountperPercent;
            require(valueTransfer >= msg.value);
            reviever.transfer(percent * amountperPercent);
        }
    }
}
