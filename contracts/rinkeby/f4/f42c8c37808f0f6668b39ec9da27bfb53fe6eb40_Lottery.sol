...
// File: contracts/FinalProject40771218H.sol
...
contract Lottery {
    ...
    function enter() public payable {
        require(LOTTERY_STATE == STATUS.Start,"LOTTERY not start.");
        require(msg.value == entranceFee,"EntranceFee wrong.");
    ...
    // 3. 並且設定入場費金額
    function startLottery(uint _a) public onlyOwner{
        require(LOTTERY_STATE == STATUS.End,"LOTTERY not start.");
    ...
    function endLottery() public{
        ...
        LOTTERY_STATE = STATUS.Decide;
        uint256 indexOfWinner = uint256(                                               
            keccak256(abi.encodePacked(msg.sender, block.difficulty, block.timestamp))                                                                          
        ) % players.length;
        recentWinner = players[indexOfWinner];
        winningReward = address(this).balance;
        (bool send_success,) = payable(msg.sender).call{value:winningReward}("");
