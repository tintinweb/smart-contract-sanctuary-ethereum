...
contract StarlPAL is Ownable, ERC721Enumerable {
    ...
    /// @notice max mint amount per wallet and transaction, initialized as 30.
    ...
    /// @notice left amount of nfts for the address, will be reduced from maxMintAmount once a user mint some nfts.
    mapping(address => uint256) public addressCanMint;
    /// @notice flag if the address has PAL nfts.
    mapping(address => bool) public addressHasTokens;
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) {
            if (!addressHasTokens[msg.sender]) {
                addressCanMint[msg.sender] = maxMintAmount.sub(_mintAmount);
            } else {
                addressCanMint[msg.sender] = addressCanMint[msg.sender].sub(
                    _mintAmount
                );
            }
            addressHasTokens[msg.sender] = true;
        }
    }
