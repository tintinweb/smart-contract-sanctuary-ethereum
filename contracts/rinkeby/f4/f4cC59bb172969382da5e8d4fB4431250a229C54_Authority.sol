...
contract Authority is IAuthority, AccessControlled {
    ...
    modifier isContract(address addr){
        require(IsContract(addr), "Is not a multsign");
        _;
    }
    ...
    function pushGovernor(address _newGovernor, bool _effectiveImmediately) external onlyGovernor notAddressZero(_newGovernor)isContract(_newGovernor){
