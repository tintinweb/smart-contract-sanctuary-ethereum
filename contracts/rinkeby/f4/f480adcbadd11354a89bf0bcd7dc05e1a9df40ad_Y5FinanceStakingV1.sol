...
/**
  @title Y5FinanaceStaking contract is a upgradable contract which helps to Stake & Reward tokens
  @author RubanRubi
  @notice Add LP's, Stake Token in LP's, Withdraw LP Tokens, Withdraw reward tokens,
 */
contract Y5FinanceStakingV1 is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMathUpgradeable for uint256;
    string public constant name = "Y5Finance - Staking";
...
// Info of each user that stakes LP tokens.
    ...
    */
    function setPoolStakingEndTime(uint256 _pid, uint256 _endTime) external virtual onlyOwner whenNotPaused {
    ...
    */
    function getTotalStakedInPool(uint256 _pid) external view returns (uint256) {
    ...
    */
    function getUserStakedTokenInPool(uint256 _pid) external view returns (uint256) {
    ...
    */
    function addPool(IERC20Upgradeable _lpToken, IERC20Upgradeable _rewardToken, uint256 _startTime, uint256 _endTime, uint256 _rewardInterval, uint256 _rewardRate) public onlyOwner {
    ...
    /**
       ...
       @param _rewardRate (APR) in %
    */
    function _beforeAddPool(uint256 _startTime, uint256 _endTime, uint256 _rewardRate) internal virtual {
        require(block.timestamp >= _startTime, "STAKING: Start Block has not reached");
        require(block.timestamp <= _endTime, "STACKING: Has Ended");
        require(_rewardRate > 0, "Reward Rate(APR) in %: Must be greater than 0");
    }
    ...
    */
    function stakeTokens(uint256 _pid, uint256 _amount) external virtual whenNotPaused {
    ...
    function _beforeStakeTokens(uint256 _pid, uint256 _amount) internal virtual {
        require(_amount > 0, "STAKING: Amount cannot be 0");
        // require(poolInfo.length <= _pid, "STAKING: Pool not exist");
        require(poolInfo[_pid].lpToken.balanceOf(msg.sender) >= _amount, "STAKING: Insufficient stake token balance");
    }
    ...
    */
    function calculateReward(uint256 _pid, address _rewardAddress) public view returns(uint256, uint256){
        ...
        uint balances = user.amount / 10**18;
		uint256 rewards = 0;
        uint256 timeDifferences;
		if(balances > 0){
            if(poolInfo[_pid].endTime > 0){
                if(block.timestamp > poolInfo[_pid].endTime){
                    timeDifferences = poolInfo[_pid].endTime - user.stakingStartTime;
                }
                else{
                    timeDifferences = block.timestamp - user.stakingStartTime;
                }
            }
            else {
                timeDifferences = block.timestamp - user.stakingStartTime;
		    ...
		    */
            //consider 1 minute
            rewards = ((user.amount * poolInfo[_pid].rewardRate) * (timeDifferences / poolInfo[_pid].rewardInterval))/100;
		}
    ...
    */
    function claimMyReward(uint256 _pid) external nonReentrant whenNotPaused {
        require(poolInfo.length <= _pid, "STAKING: Pool not exist");
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.isStaking, "STAKING: No staked token balance available");
        ...
        require(timeDifferences/poolInfo[_pid].rewardInterval >= 1, "STAKING: Can be claimed only after the interval");
        uint256 rewardTokens = poolInfo[_pid].rewardToken.balanceOf(address(this));
        require(rewardTokens > reward, "STAKING: Not Enough Reward Balance");
        ...
        user.stakingStartTime = block.timestamp + poolInfo[_pid].rewardInterval;
	}
    ...
    */
    function SendRewardTo(uint256 _pid, uint256 calculatedReward, address _toAddress) internal virtual returns(bool){
        ...
        require(calculatedReward > 0, "STAKING: Calculated Reward zero");
        require(pool.rewardToken.balanceOf(address(this)) >= calculatedReward, "STAKING: Not enough reward balance");
     
            if(pool.lpToken == pool.rewardToken){
                if((pool.rewardToken.balanceOf(address(this)) - calculatedReward) < totalStakedAmountInPool[_pid]){
                    calculatedReward = 0;
                }
            }
