...
// Part: OpenZeppelin/[email protected]/CountersUpgradeable
    ...
    OwnableUpgradeable
{
    ...
    mapping(uint32 => uint32[]) private tokenIdToMaterialsTokenIds;
    ...
    function getTokenMaterialIds(uint32 index)
        public
        view
    ...
    function setMaterialTypes(
        uint32 start,
        uint8[] calldata _materialTokenTypes
    ) external {
        require(
            start == materialTokenTypes.length,
            "Start index must equal the length of the array."
        );
    ...
    function transferMaterials(uint32[] memory _materials) internal {
        ...
        for (uint8 i = 0; i < _materials.length; i += 1) {
            materialsContract.safeTransferFrom(
                msg.sender,
                address(this),
                _materials[i]
            );
    ...
    modifier isApproved() {
        ...
        require(
            materialsContract.isApprovedForAll(msg.sender, address(this)),
    ...
    modifier sufficientMaterials(uint32[] memory _materials) {
        ...
        require(
            _materials.length >= 3 && _materials.length <= 5,
            "Materials length is < 3 or > 5."
        );
        _;
    }
    ...
    modifier validMaterialSelection(uint32[] memory _materials) {
        ...
        require(
            getMaterialType(_materials[0]) == 1,
            "Token at index 0 must be a weapon."
        );
        require(
            getMaterialType(_materials[1]) == 2,
            "Token at index 1 must be an ore."
        );
        require(
            getMaterialType(_materials[2]) == 3,
            "Token at index 2 must be an orb."
        );

        if (_materials.length > 3) {
            require(
                getMaterialType(_materials[3]) == 3,
                "Token at index 3 must be an orb."
            );
        }

        if (_materials.length > 4) {
            require(
                getMaterialType(_materials[4]) == 3,
                "Token at index 4 must be an orb."
            );
        }
        _;
    }
    ...
    modifier materialsOwnedBySender(uint32[] memory _materials) {
        ...
        require(
            msg.sender == materialsContract.ownerOf(_materials[0]),
            "Tokens not all owned by sender."
        );
        require(
            msg.sender == materialsContract.ownerOf(_materials[1]),
            "Tokens not all owned by sender."
        );
        require(
            msg.sender == materialsContract.ownerOf(_materials[2]),
            "Tokens not all owned by sender."
        );

        if (_materials.length > 3) {
            require(
                msg.sender == materialsContract.ownerOf(_materials[3]),
                "Tokens all not owned by sender."
            );
        }
        if (_materials.length > 4) {
            require(
                msg.sender == materialsContract.ownerOf(_materials[4]),
                "Tokens all not owned by sender."
            );
        }
        _;
...
contract ForgeV2 is ForgeV1 {
    // function initialize(address materialsAddress) public initializer {
    //     ForgeV1.initialize(materialsAddress);
    // }

    function sum(uint256 a, uint256 b) public pure returns (uint256 sum) {
        return (a + b);
    }
}