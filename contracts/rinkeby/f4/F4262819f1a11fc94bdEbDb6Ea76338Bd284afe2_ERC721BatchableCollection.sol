 ...
 *
 * Each batch that the owner releases gets offset by the BATCH_SIZE because doing so makes it possible
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
     ...
     * In order to save gas, _totalMinted is not updated here and needs to be updated when
...
abstract contract ERC721Marketplace is ERC721, Ownable {
    ...
    function _setRevenueShare(
        ...
        require(newBPS[0] + newBPS[1] <= MAX_ROYALTIES_BPS, "New bps too high");
...
contract ERC721BatchableCollection is ERC721Marketplace {
    ...
    uint256 public individualTokenBase;
    /* Will let us know when we need to update the token base */
    uint256 public individualTokensCount;
    /* Token IDs above this are reserved for post burn NFTs */
    uint256 public constant MAX_TOKENS = 2**128;
    /* Having this constant is what allows quick calculation of tokenId to batchId */
    uint256 public constant BATCH_SIZE = 10000;
    uint256 public constant SEQ_MINT_LIMIT = 5;
    uint256 public constant MAX_TRANSACTION = 1000;
    string public baseURI;

    struct BatchData {
        bool isBatchMintable;
        uint64 batchSize;
        uint64 batchMinted;
    }
    ...
    /*
     * Necessary because having a erc721a batch mint feature means owners[tokenId] can be
     * address(0) for a token that does exist.
     */
    mapping(uint256 => bool) private _tokenIdBurned;
    /* Saves the tokenURI for a given batch */
    mapping(uint256 => string) public batchDirectories;
    /* Saves the tokenURI for a individual tokens */
    mapping(uint256 => string) private _individualTokenMetadata;

    event OwnerBatchMint(uint256 indexed batchId, uint256 amount);
    event TokenBought(
        address indexed buyer,
        uint256 indexed tokenId,
        uint256 price,
    ...
    function _init(
        ...
        dualSignerAddress = addresses[2];

        _setRoyaltiesBPS(uints[0]);
        _setRevenueShare(revenueShare, [uints[1], uints[2]]);
        uint256 initialRelease = uints[3];

        baseURI = strings[0];
        _name = strings[1];
        _symbol = strings[2];
        if (initialRelease > 0 && bytes(strings[3]).length > 0) {
            _addTokenDirectoryRelease(strings[3], initialRelease);
        }
    }
    ...
    /**
     * @dev Returns if a given batch is batchMintable still by the owner
     */
    function isBatchMintable(uint256 batchId) external view returns (bool) {
        return _batchIdToData[batchId].isBatchMintable;
    }
    ...
    function totalSupply() public view virtual override returns (uint256) {
        return _totalMinted + individualTokensCount - _totalBurned;
    }
        ...
        returns (string memory)
    {
        ...
        uint256 tokenSlot = tokenId;
        string memory suffix = "";
        if (tokenSlot >= MAX_TOKENS) {
            tokenSlot = tokenSlot - MAX_TOKENS;
            suffix = "/post_burn";
        }

        uint256 batchId = tokenSlot / BATCH_SIZE;
        ...
        if (_isIndividual(batchData)) {
            return
                string(
                    abi.encodePacked(
                        _baseURI(),
                        _individualTokenMetadata[tokenSlot],
                        suffix
                    )
                );
        } else {
    ...
    /**
     * @dev returns if this batch is reserved for individual tokens or not
     */
    ...
    /**
     * @dev returns if this collection can still add another batch
     */
    function _canAddBatch() internal view returns (bool) {
        return nextBatchId * BATCH_SIZE <= MAX_TOKENS - BATCH_SIZE;
    }
    ...
    function _addTokenDirectoryRelease(
        ...
        require(releaseAmount <= BATCH_SIZE, "Release amount too large");
        require(_canAddBatch(), "Token limit reached");

        batchDirectories[nextBatchId] = releaseTokenDirectory;
        _batchIdToData[nextBatchId] = BatchData({
     ...
     * @dev See {ERC721-_beforeTokenTransfer}.
     *
     * makes sure if token was batch minted and the next one over is also by the
     * same owner, then it gets _owners mapping set, which is required for
     * ownersOf() to continue to give the right owner.
     */
    function _beforeTokenTransfer(
        address from,
        address to,
        uint256 tokenId
    ) internal override(ERC721Marketplace) {
        super._beforeTokenTransfer(from, to, tokenId);

        if (tokenId < MAX_TOKENS) {
            BatchData memory batchData = _batchIdToData[tokenId / BATCH_SIZE];
            if (
                batchData.batchMinted > (tokenId % BATCH_SIZE) + 1 &&
    ...
    function _exists(uint256 tokenId)
        ...
        returns (bool)
    {
        // to save gas burning does not set _owners[tokenId] to address(0)
        if (_tokenIdBurned[tokenId]) {
            return false;
        }
        ...
        // to save gas on batch mints, _owners is not set on every batch mint token, so we need
        // to check the batchMinted
        return
            _batchIdToData[tokenId / BATCH_SIZE].batchMinted >
            tokenId % BATCH_SIZE;
    }
        ...
        returns (address)
    {
        ...
        // this gives us the largest multiple of 5 less than or equal to tokenId
        uint256 lowestTokenToCheck = (tokenId / SEQ_MINT_LIMIT) *
            SEQ_MINT_LIMIT;
     ...
     */
    function batchMint(uint256 batchId, uint256 amount) external onlyOwner {
        BatchData memory batchData = _batchIdToData[batchId];
        require(amount <= MAX_TRANSACTION, "Amount too large");
        require(
            batchData.batchMinted + amount <= batchData.batchSize,
            "Cannot mint this quantity from batch"
        );
        ...
        require(
            batchData.isBatchMintable,
            "Selected tokens already minted from batch"
        );

        uint256 firstNum = batchId * BATCH_SIZE + batchData.batchMinted;
        // ensures ownerOf runs quickly even if user is minting a large number like 100
        for (uint256 i = firstNum; i < firstNum + amount; i += SEQ_MINT_LIMIT) {
            _owners[i] = owner();
        }

        // gas is cheaper for two separate loops
        for (uint256 i = firstNum; i < firstNum + amount; i++) {
            emit Transfer(address(0), owner(), i);
        }
        _balances[owner()] += amount;
        _totalMinted += amount;

        batchData.batchMinted += uint64(amount);
        _batchIdToData[batchId] = batchData;
        emit OwnerBatchMint(batchId, amount);
    }
    ...
    function giftBatchedNFT(address giftAddress, uint256 tokenId)
        ...
        onlyOwner
    {
        _createBatchNFT(giftAddress, tokenId, 0, "");
    }

    /**
     * @dev mints an NFT from a released batch if it is within
     * the batch size of that block.
     *
     * Only for NFTs that are already part of a defined batch.
     */
    function _createBatchNFT(
        ...
        require(!_tokenIdBurned[tokenId], "Invalid token");
        uint256 batchId = tokenId / BATCH_SIZE;
        BatchData memory batchData = _batchIdToData[batchId];
        uint256 batchIndex = tokenId % BATCH_SIZE;
        require(batchIndex >= batchData.batchMinted, "Token already minted");
        require(batchIndex < batchData.batchSize, "Token outside batch");
        if (batchData.isBatchMintable) {
            batchData.isBatchMintable = false;
            _batchIdToData[batchId] = batchData;
        }

        _safeMint(collectorAddress, tokenId);
        _totalMinted += 1;
        emit TokenBought(
            _msgSender(),
            tokenId,
            currentPrice,
            batchDirectories[batchIndex],
            salesHash
        );
    }
    ...
    function giftIndividualNFT(address giftAddress, string memory uri)
        ...
        onlyOwner
    {
        _createIndividualNFT(giftAddress, 0, uri, "");
    }
     ...
     * a new batch for individual tokens.
     */
    function _createIndividualNFT(
        address collectorAddress,
        uint256 currentPrice,
        string memory uri,
        bytes32 salesHash
    ) internal {
        uint256 tokenBatchOffset = individualTokensCount % BATCH_SIZE;
        if (tokenBatchOffset == 0) {
            require(_canAddBatch(), "token limit reached");
            individualTokenBase = nextBatchId * BATCH_SIZE;
            nextBatchId += 1;
        }
        uint256 tokenId = tokenBatchOffset + individualTokenBase;
        individualTokensCount += 1;
        _individualTokenMetadata[tokenId] = uri;
        _safeMint(collectorAddress, tokenId);
        emit TokenBought(_msgSender(), tokenId, currentPrice, uri, salesHash);
    }
    ...
    function buyIndividualToken(
        ...
        require(
            _addressToActiveVersion[owner()] == version,
            "Incorrect signature version"
        );
        ...
        _createIndividualNFT(_msgSender(), currentPrice, uri, hash);
        payable(_msgSender()).transfer(msg.value - currentPrice);
    }
    ...
    function hashToSignToSellFromBatch(
        ...
        _checkValidSigner(_msgSender());
        require(
            !_exists(tokenId) && !_tokenIdBurned[tokenId],
            "Invalid tokenId"
        );
        return
            _hashForMintingFromBatch(
    ...
    function buyTokenFromBatch(
        ...
        _createBatchNFT(_msgSender(), tokenId, currentPrice, hash);
        payable(_msgSender()).transfer(msg.value - currentPrice);
    }

    /**
     * @dev For any user who owns a token that is burnable. If the collection has
     * mint on burn set, they will get a newly minted token with the burn hash metadata.l;k
     *
    ...
    function burn(uint256 tokenId) external {
        ...
        require(tokenId < MAX_TOKENS, "Token not burnable");
        require(_exists(tokenId), "Token must exist");

        address msgSender = _msgSender();
        address owner = ownerOf(tokenId);
        require(
            _isApprovedOrOwner(msgSender, tokenId, owner),
        ...
        // We don't call _burn and _mint here to save gas fees on updating balances.
        // Additionally we don't need to set _owners[token] to address(0) because relevent methods
        // also check this _tokenIdBurned mapping
        _tokenIdBurned[tokenId] = true;
        _totalBurned += 1;
        emit Transfer(owner, address(0), tokenId);

        if (mintPostBurn) {
            _totalMinted += 1;
            _owners[tokenId + MAX_TOKENS] = owner;
            emit Transfer(address(0), owner, tokenId + MAX_TOKENS);
        } else {
            _balances[owner] -= 1;
        }
    }
