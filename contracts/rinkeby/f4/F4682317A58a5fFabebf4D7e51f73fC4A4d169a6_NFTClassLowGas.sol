...
contract NFTClassLowGas is ERC721 {
  ...
  function populateTokensSlice(uint256 slice) public _ownerOnly {
    require(_LAST_EXECUTED_INDEX + slice < _MAX_SUPPLY, "Last minted id + slice can't be greater the NFTs limit");
    require(_COINS_MINTED == false, "Coins were already minted, can't mint more");

    uint256 i = _LAST_EXECUTED_INDEX;
    uint256 tempLimit = _LAST_EXECUTED_INDEX + slice;

    if (_LAST_EXECUTED_INDEX + slice == _MAX_SUPPLY) {
      tempLimit = 2500; // manual check to mint from 0 to 9.999 (10.000 in total)
      _COINS_MINTED = true;
    }
