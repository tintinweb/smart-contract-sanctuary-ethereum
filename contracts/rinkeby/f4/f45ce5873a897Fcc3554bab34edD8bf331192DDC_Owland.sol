...
contract Owland is ERC721, Ownable {
    ...
    constructor() ERC721 ("OwLand", "OWLAND") {
        ...
        baseURI = "ipfs://QmULxBcpD1faHAR8Xxa9ombntY2REvrKmQZ6LCKPcgbwrf/";
        darkowls = DarkOwls(0x2849D61bB776A45B862f23C72Fa544Db9Fb3E2d4);
    }
    ...
    function mintRarePlots(uint256[] calldata _tokenIds, bool _stake) external payable {
        ...
        if (rareByAllowance) {
            for (uint256 i = 0; i < _tokenIds.length; i++) {
                require(rareAllowance[sender] > 0, "> allowed amount");
                uint256 _tokenId = _tokenIds[i];
                require(typeBPlots[_tokenId], "Rare");
