...
contract Dobbys is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        rn = IRandomness(_rn);
        ids = new uint256[](_maxSupply - _legendarySupply);
    }
    ...
    function generateRandomByTokenIds(uint256 _tokenId, uint256 _seed) private {
        ...
        uint256 randomId = pickRandomUniqueId(randomSeed); // pick the random number between 0 ~ 1494
        randomValueByTokenId[_tokenId] = randomId + 1;
    }
    ...
    function legendaryMint(address[] calldata _addresses) public onlyOwner {
        require(_addresses.length == legendarySupply, "Invalid legendary addresses count!");
        require(
    ...
    function uploadMetadata(Metadata[] calldata _metadata) public onlyOwner {
        ...
        for (uint256 i = 0; i < _metadata.length; i++) {
            ...
            metadata[metadataCount + i + 1] = Metadata(_metadata[i].dna, _metadata[i].edition);
        }
