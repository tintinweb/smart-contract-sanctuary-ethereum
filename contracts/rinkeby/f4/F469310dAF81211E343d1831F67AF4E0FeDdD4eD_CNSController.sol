...
contract CNSController is Ownable, ENSController {
    ...
    */
    ...
    function setSubDomainLabel(bytes32 _node, string memory _name) internal {
        subDomainLabel[_node] = _name;
    }

    function getSubDomainLabel(bytes32 _node)
        public
        view
        returns (string memory)
    {
        return subDomainLabel[_node];
    }

    function setSubDomainName(bytes32 _node, string memory _name) internal {
        subDomainName[_node] = _name;
    }

    function getSubDomainName(bytes32 _node)
        public
        view
        returns (string memory)
    {
        return subDomainName[_node];
    }

    function setSubDomainOwner(bytes32 _node, address _owner) internal {
        subDomainOwner[_node] = _owner;
    }

    function getSubDomainOwner(bytes32 _node) public view returns (address) {
        return subDomainOwner[_node];
    }

    function setSubDomainTokenId(bytes32 _node, uint256 _domainId) internal {
        subDomainTokenId[_node] = _domainId;
    }

    function getSubDomainTokenId(bytes32 _node) public view returns (uint256) {
        return subDomainTokenId[_node];
    }
    ...
    function unRegisterSubdomain(bytes32 _subnode) public {
    ...
    function unRegisterSubdomainAndBurn(bytes32 _subnode) public {
        require(isRegisterSubdomain(_subnode));
        require(controller[msg.sender]);
        emit BurnCNSToken(
            _subnode,
