    ...
    Ownable
{
    ...
    function setPrice(
        ...
        for (uint256 index = 0; index < tokenAddress.length; index++) {
            ...
            pricePublic[tokenAddress[index]] = tokenPrice[index];
        }
        emit SetPrice(tokenAddress, tokenPrice);
    }
...
contract PuliInuRunners is PuliInuNFT {
    event WithdrawTest(address indexed recepient, uint256 indexed amount);
    event Withdraw(
        address indexed recepient,
        IERC20 indexed tokenAddress,
        uint256 indexed amount
    );
    event SetWithdrawal(address indexed withdrawalAddress);

    address withdrawalAddress;

    constructor(address proxyAddress)
        PuliInuNFT("Puli Inu Runners", "PRNR", proxyAddress, 1050)
    {
    ...
    function setWithdrawalAddress(address _withdrawalAddress) public onlyOwner {
        ...
        emit SetWithdrawal(_withdrawalAddress);
    }
    ...
    function withdraw(address tokenAddress, uint256 amount)
        ...
        onlyOwner
    {
        require(withdrawalAddress != address(0), "Withdrawal addess not set");
        bool isContract = Address.isContract(withdrawalAddress);
        if (tokenAddress == NATIVE) {
            if (isContract) {
                Address.functionCallWithValue(
                    withdrawalAddress,
                    abi.encode("receivePayment"),
                    amount
                );
            } else {
                Address.sendValue(payable(withdrawalAddress), amount);
            }
            emit WithdrawTest(withdrawalAddress, amount);
            return;
        }
        ...
        emit Withdraw(withdrawalAddress, token, amount);
    }
}