...
import "./interface/IJOYSMarketplace.sol";
...
contract JOYSMarketplace is IJOYSMarketplace, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC1155HolderUpgradeable, ERC721HolderUpgradeable {
    ...
    function createOrder(
        ...
        require(nftStandart != NFTStandart.NULL, "JOYSMarketplace: wrong nft standart");
        require(tokenContract != address(0), "JOYSMarketplace: zero contract address");
        if (nftStandart == NFTStandart.ERC721) {
            require(_whitelistErc721.contains(tokenContract), "JOYSMarketplace: nft not registered");
            require(tokenAmount == 1, "JOYSMarketplace: wrong token amount");
        } else if (nftStandart == NFTStandart.ERC1155) {
            require(_whitelistErc1155.contains(tokenContract), "JOYSMarketplace: nft not registered");
            require(tokenAmount >= 1, "JOYSMarketplace: wrong token amount");
        }
        require(settlementToken != address(0), "JOYSMarketplace: zero trade token address");
        require(_settlementTokens.contains(settlementToken), "JOYSMarketplace: settlement token not registered");
        require(price > 0.0001 ether, "JOYSMarketplace: wrong price");
    ...
    function cancelOrder(uint256 orderId) external override nonReentrant returns(bool success) {
        require(orderId < _totalOrders.current(), "JOYSMarketplace: order does not exist");

        Order memory order = _orders[orderId];

        require(order.status == OrderStatus.OPEN, "JOYSMarketplace: only for open orders");
        require(order.seller == msg.sender, "JOYSMarketplace: sender is not the seller");
    ...
    function executeOrder(uint256 orderId) external override nonReentrant returns(bool success) {
        ...
        require(order.status == OrderStatus.OPEN, "JOYSMarketplace: only for open orders");
        require(order.seller != msg.sender, "JOYSMarketplace: not for seller");
    ...
    function addSettlementToken(address erc20) external override nonReentrant onlyOwner returns(bool success) {
        require(IERC20MetadataUpgradeable(erc20).decimals() == 18, "JOYSMarketplace: wrong erc20 decimals");
    ...
    function _setTradeFeePercent(uint256 newTradeFeePercent) internal {
        require(newTradeFeePercent >= 0 && newTradeFeePercent <= 100, "JOYSMarketplace: wrong percent value");
