...
contract Marketplace {
    ...
    uint256 nftsMarketplaceCounter = 0;
    ...
    function GetNftsSalingOnMarketplace() public view returns (uint256[] memory ids, uint256[] memory prices, string[] memory uris){
        uint256[] memory nftIds = new uint256[](nftsMarketplaceCounter);
        uint256[] memory nftPrices = new uint256[](nftsMarketplaceCounter);
        string[] memory nftUris = new string[](nftsMarketplaceCounter);
        ...
        for (uint i = 0; i < marketplaceNfts.length; i++) {
            if (marketplaceNfts[i].isSaling) {
                ...
                nftUris[counter] = database.tokenURI(marketplaceNfts[i].nftId);
                counter++;

            }
        }

        return (nftIds, nftPrices, nftUris);
    }
