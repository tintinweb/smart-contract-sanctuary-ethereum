    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function __StableFarmingPool_init(address _manager) public initializer {
        manager = _manager;
        __Pausable_init();
    }
    ...
    function stableFarmingAddPool(
        ...
        require(
            ((_endJoinTime >= block.timestamp && _endJoinTime > _startJoinTime) || _endJoinTime == 0),
            "stableFarmingStakingPool: invalid end join time"
        );

        require(
            _minInvestment < _maxInvestment,
            "stableFarmingStakingPool: invali min max investment"
        );
    ...
    function _stableFarmingDeposit(
        ...
        require(
            block.timestamp <= pool.endJoinTime || pool.endJoinTime == 0,
            "stableFarmingStakingPool: pool is already closed"
        );
