...
// File: contracts_lou\Decimal.sol
...
// File: contracts_lou\IMarket.sol

pragma solidity 0.6.8;
/**
 * @title Interface for RT3D Protocol's Market
 */
interface IMarket {
    struct Bid {
...
// File: contracts_lou\IMedia.sol

pragma solidity 0.6.8;
/**
 * @title Interface for RT3D Protocol's Media
 */
interface IMedia {
...
// File: contracts_lou\IAuctionHouse.sol
...
// File: contracts_lou\AuctionHouse.sol
...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
    ...
    // The address of the RT3D protocol to use via this contract
    address public rt3d;
     ...
     * Constructor
     */
    constructor(address _rt3d, address _weth) public {
        require(
            IERC165(_rt3d).supportsInterface(interfaceId),
    ...
    nonReentrant
    {
        ...
        // For RT3D Protocol, ensure that the bid is valid for the current bidShare configuration
        if(auctions[auctionId].tokenContract == rt3d) {
            require(
                IMarket(IMediaExtended(rt3d).marketContract()).isValidBid(
    ...
    /**
     * @notice End an auction, finalizing the bid on RT3D if applicable and paying out the respective parties.
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        if(auctions[auctionId].tokenContract == rt3d) {
            // If the auction is running on RT3D, settle it on the protocol
            (bool success, uint256 remainingProfit) = _handleRt3dAuctionSettlement(auctionId);
    ...
    function _handleRt3dAuctionSettlement(uint256 auctionId) internal returns (bool, uint256) {
        address currency = auctions[auctionId].auctionCurrency == address(0) ? wethAddress : auctions[auctionId].auctionCurrency;
        ...
        IERC20(currency).approve(IMediaExtended(rt3d).marketContract(), bid.amount);
        IMedia(rt3d).setBid{value : auctions[auctionId].amount }(auctions[auctionId].tokenId, bid);
        uint256 beforeBalance = (address(this)).balance;
        try IMedia(rt3d).acceptBid(auctions[auctionId].tokenId, bid) {} catch {
            // If the underlying NFT transfer here fails, we should cancel the auction and refund the winner
            IMediaExtended(rt3d).removeBid(auctions[auctionId].tokenId);
            return (false, 0);
        }
