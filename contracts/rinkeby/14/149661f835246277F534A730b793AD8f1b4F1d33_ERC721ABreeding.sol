...
contract ERC721ABreeding is ERC721A, Ownable {
    ...
    constructor(
        ...
        momContract = ParentContract(_momAddress);
        dadContract = ParentContract(_dadAddress);
    ...
    function tokensOf(address _owner, uint _type) external view returns (Token[] memory) {
        ParentContract parentContract = _type == 1 ? dadContract : momContract;
        unchecked {
            uint256 tokenIdsIdx;
            uint256 tokenIdsLength = parentContract.balanceOf(_owner);
            Token[] memory tokenIds = new Token[](tokenIdsLength);
    ...
    function setDadContract(address _address) external onlyOwner {
        dadContract = ParentContract(_address);
    }

    function setMomContract(address _address) external onlyOwner {
        momContract = ParentContract(_address);
    }
    ...
    function breed(uint16[] memory dadIds, uint16[] memory momIds) public {
        ...
        for (uint16 i = 0; i < _quantity; i++) {
            isMom[momIds[i]] = true;
            isDad[dadIds[i]] = true;
            parents[_currentIndex].mom = momIds[i];
            parents[_currentIndex].dad = dadIds[i];
            parents[_currentIndex].exists = true;
