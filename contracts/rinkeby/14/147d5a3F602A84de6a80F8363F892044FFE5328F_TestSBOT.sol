...
contract TestSBOT is ERC721A, Ownable, ReentrancyGuard, DutchAuctionSC {
  ...
  uint256 private STEP_TIME = 15 * 1; //seconds
  uint256 public STEP_PRICE = 0.05 ether;

  uint256 private MAX_MINT = 10;
  uint256[4] private MAX_PER_STATUS = [0, 1, 5];

  // Amount minted
  mapping(address => uint256[3]) private _amountMinted;

  // Merkle tree
  bytes32 public merkleRoot = 0x2e95d9b220e054ce1ffa9e0698bcf436f445e88fc75ef6e9126b547e21356c6e; // TODO: Update it
  ...
  modifier qtyValidation(uint256 _qty) {
    ...
    require(
      _qty + _amountMinted[msg.sender][uint256(status)] <= MAX_PER_STATUS[uint256(status)],
  ...
  // Public mint
  function presaleMint(uint256 _qty, bytes32[] calldata _proof) external payable nonReentrant qtyValidation(_qty) {
    require(status == MintStatus.PRESALE, "Status is not presale");

    require(MerkleProof.verify(_proof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "You are not in presale");
    presale_minted += _qty;
    _privateMint(_qty);
  }
  ...
  // Withdraw function
  function withdrawAll(address payable withdraw_address) external payable nonReentrant onlyOwner {
