...
contract ADD is ERC20, Ownable {
	...
	function setBurnFee(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
	    require(DAOFee[0].add(buy)  <= 3000 , "Max fee limit reached for 'BUY'");
		require(DAOFee[1].add(sell) <= 3000 , "Max fee limit reached for 'SELL'");
		require(DAOFee[2].add(p2p)  <= 3000 , "Max fee limit reached for 'P2P'");
	...
	function setDAOFeeAddress(address payable newAddress) external onlyOwner() {
       require(newAddress != address(0), "zero-address not allowed");
	   DAOFeeAddress = newAddress;
    }
	...
	function _transfer(address from, address to, uint256 amount) internal override {
		...
		if (!swapping && canSwap && swapEnable && automatedMarketMakerPairs[to]) {
			...
			if(newBalance > 0) {
			    payable(DAOFeeAddress).transfer(newBalance);
			}
			swapping = false;
		}
		...
		if(takeFee) 
		{
		    (uint256 DAOfee, uint256 Burnfee) = collectFee(amount, automatedMarketMakerPairs[to], !automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to]);
			if(DAOfee > 0)
			{
			    super._transfer(from, address(this), DAOfee);
			    amount = amount.sub(DAOfee);
			    _moveDelegates(_delegates[from], _delegates[address(this)], DAOfee);
			}
			if(Burnfee > 0)
			{
