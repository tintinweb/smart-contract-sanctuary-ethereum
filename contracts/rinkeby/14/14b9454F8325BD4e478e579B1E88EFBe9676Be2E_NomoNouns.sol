...
contract NomoNouns is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
    /// events
    ...
    constructor(
        ...
        INounsAuctionHouse _auctionHouse,
    ...
    function mint(
        ...
        require(nounId == id, 'NounId invalid');

        // check has minted
        require(hasMinted[id][msg.sender] + quantity <= maxMintPerWallet, 'Max mint exceeds the limit');
        ...
        _mintTo(msg.sender, nounId, blockNumber, quantity);
    }
    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        require(exists(tokenId), 'Token: URI query for nonexistent token');
    ...
    function _mintTo(
        ...
        //uint256 nounId = auctionHouse.auction().nounId;
        INounsSeeder.Seed memory seed = seeds[nounId] = seeder.generateSeed(nounId, blockNumber, descriptor);

        _mint(to, nounId, quantity, '');
