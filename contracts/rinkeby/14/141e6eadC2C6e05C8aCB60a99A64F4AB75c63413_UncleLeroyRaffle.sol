...
contract UncleLeroyRaffle is Ownable {
    ...
    // function getAllRaffles() public view returns(uint256[] memory raffles) { 
    //     raffles = new uint256[](_subRaffleIds.current());
    //     for (uint256 i=0;i<_subRaffleIds.current()+1;i++){
    //         raffles[i] = raffleItems[_gameIds.current()][i].raffleId;
    //     }
    //     return raffles;
    // }

    // get the raffle holder knowing raffleId
    function getRaffleOwner(uint256 raffleId) external view returns (address) {
        for (uint256 i=0;i<_subRaffleIds.current()+1;i++){
            if (raffleItems[_gameIds.current()][i].raffleId == raffleId) {
                return raffleItems[_gameIds.current()][i].owner;
            }
        }
        revert("No owner found error");
    }
    ...
    function purchase(uint256 ticketAmount) external {
        ...
        _balances[msg.sender] = _balances[msg.sender].add(ticketAmount);
    ...
    function pickRaffleWinners() public onlyOwner returns(uint256[] memory winners){

        // if (_subRaffleIds.current() < constantNumberOfWinners) {
        //     winners = getAllRaffles();
        // } else {
        //     winners = getRandomWinners(random(), constantNumberOfWinners, _subRaffleIds.current());
        // }

        if (_subRaffleIds.current() < constantNumberOfWinners) {
            winners = getRandomWinners(random(), _subRaffleIds.current(), _subRaffleIds.current());
        } else {
            winners = getRandomWinners(random(), constantNumberOfWinners, _subRaffleIds.current());
        }
    ...
    function getRandomWinners(uint256 randomValue, uint256 n, uint256 range) private view returns (uint256[] memory expandedValues) {
        ...
        while (
            ...
            uint256 randomWinnerId = raffleItems[_gameIds.current()][
                uint256(keccak256(abi.encode(randomValue, currentIndex))) % range
            ].raffleId;

            if (!valueInArray(expandedValues, randomWinnerId)) {
                expandedValues[currentWinners] = randomWinnerId;
                currentWinners++;
            }
            currentIndex++;
        }
    ...
    // helper pure function to check if a value exists in the array
    function valueInArray(uint256[] memory array, uint256 value) private pure returns(bool) {
    ...
    function getLastWinners() public view returns(uint256[] memory) {
        return lastWinners;
    }
