...
// Part: OpenZeppelin/openzeppelin-[email protected]/OwnableUpgradeable
    ...
    ERC721PausableUpgradeable
{
     ...
     */
    function mint(address _to) public override  returns (uint256) {
    ...
    function getNFTVariables (uint _id) public view returns (uint, uint, uint, uint, uint) {
    ...
    */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        (uint _USDCBalance, uint  _tUSDCBalance, uint  _conversionRate,uint  _usdcFee, uint  _withdrawTime) = getNFTVariables(tokenId);
        return
        _tokenURI(
            tokenId,
            _USDCBalance,
    ...
    function requestWithdrawFromStakingPool(uint _amount) external {
        ...
        uint tokenIndex = mint(msg.sender);
