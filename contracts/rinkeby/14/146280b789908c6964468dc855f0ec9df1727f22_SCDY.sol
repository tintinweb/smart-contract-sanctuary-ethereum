...
contract SCDY is ERC721Enum, Ownable, Pausable,  ReentrancyGuard {

	using Strings for uint256;
	string public baseURI;
    string public baseExtension = ".json";

	uint256 public costPublic = 0.02 ether;
    uint256 public costOG = 0.01 ether;

	uint256 public maxSupply = 1000;

    uint256 public maxFree = 200;  
    uint256 public maxNFTPerFree = 20;

    uint256 public maxNFTPerOG = 2;

    uint256 public maxPerTx = 10;

    bool public salePublic = false;
    bool public saleFree = false;
    bool public saleOG = false;
    ...
    function mintOG(uint256 _mintAmount) public payable nonReentrant{
		uint256 s = totalSupply();
        require(saleOG, "Cannot mint yet!");
        require(oglist[msg.sender], "You are not OG");
        uint256 ownerMintedCount = ogMintedBalance[msg.sender];
        require(ownerMintedCount + _mintAmount <= maxNFTPerOG, "max NFT per OG exceeded");
	...
	function mintPublic(uint256 _mintAmount) public payable nonReentrant{
		uint256 s = totalSupply();
        require(salePublic, "Cannot mint yet!");
		require(_mintAmount > 0, "Cannot mint 0" );
		require(_mintAmount <= maxPerTx, "Cannot mint more than max mint per tx" );
		require(s + _mintAmount <= maxSupply, "Cannot go over supply" );
    ...
    function mintFree(uint256 _mintAmount) public payable nonReentrant{
		uint256 s = totalSupply();
        require(saleFree, "Cannot mint yet!");
		...
		require(_mintAmount <= maxPerTx, "Cant mint more than max mint per tx" );
    ...
    function checkOGStatus() public view returns (bool) {
        return oglist[msg.sender];
    }
    ...
    function setCostOG(uint256 _newCost) public onlyOwner {
	    costOG = _newCost;
	}
    ...
    function setMaxNFTPerFree(uint256 _limit) public onlyOwner {
        maxNFTPerFree = _limit;
    }

    function setMaxNFTPerOG(uint256 _limit) public onlyOwner {
	    maxNFTPerOG = _limit;
	}
    ...
    function setSaleOG(bool _state) public onlyOwner {
	    saleOG = _state;
	}

    function setOGAddresses(address[] memory _addresses) public onlyOwner {
