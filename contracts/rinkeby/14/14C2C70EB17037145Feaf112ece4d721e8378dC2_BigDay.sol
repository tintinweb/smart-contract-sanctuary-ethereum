...
contract BigDay is Ownable, ERC721Royalty, Pausable, ReentrancyGuard, SignatureValidator, TokenAsDate, BoundCurve {
    ...
    function privateMint(PrivateMintPayload calldata payload, PrivateMintCover calldata cover)
        external
        ...
        onlyOwner
    {
        ...
        token.covers.push(Covers.Cover(address(this), payload.tokenId, 0, "", "", payload.imageUrl, false));
        token.id = payload.tokenId;

        if (cover.collection != address(0)) {
            token.covers.push(
                Covers.Cover(
                    cover.collection,
                    cover.artTokenId,
            ...
            token.tokenActiveCover = 1;
        } else {
            token.tokenActiveCover = 0;
        }
        // emit CoverAdded(payload.tokenId, 0);
    }
