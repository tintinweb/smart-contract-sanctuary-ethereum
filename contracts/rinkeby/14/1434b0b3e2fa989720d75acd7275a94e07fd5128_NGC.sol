...
contract NGC is ERC721A, Ownable, ReentrancyGuard  {
    ...
    // VIP Mint Signer address for wallet lists
    address private _winterSignerAddress;
    ...
    function claim(uint256 quantityAllowed, bytes calldata signature, uint256 quantity) external {
    ...
    function presale(uint256 quantityAllowed, bytes calldata signature, uint256 quantity) external payable {
        require(isPreSaleActive, "Presale ale Not Active");
        ...
        _mintToken(_msgSender(), quantity * 2);
    }
    ...
    function winterMint(bytes calldata signature, uint256 quantity) external payable {
        require(msg.value == price * quantity, "Incorrect Value");
        require(_winterSignerAddress == keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                bytes32(uint256(uint160(msg.sender)))
            )
    ...
    {
        return
            _presaleSignerAddress == keccak256(abi.encodePacked(address_, quantityAllowed)).recover(signature);
    }
    ...
    function isEligibleClaim(
        ...
        return
            _claimSignerAddress == keccak256(abi.encodePacked(address_, '-', quantityAllowed)).recover(signature);
    }
    ...
    function setWinterSignerAddress(address signerAddress) external onlyOwner {
        _winterSignerAddress = signerAddress;
    }
