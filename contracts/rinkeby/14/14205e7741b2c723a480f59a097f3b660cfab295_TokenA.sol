...
contract TokenA is ERC20, AccessControl {
    ...
    uint public sellerFee = 700;
     uint public buyerFee = 700;
    uint public txFee = 0;
    ...
    address public fee_address=0xF43ffb3c024d3b091E96082bD46A1B8d7e8EAEB2;
    ...
    function Manualswap() external onlyRole(DEFAULT_OWNER) {
     ...
     function updateMaxWallet(uint _maxWallet) external onlyRole(DEFAULT_OWNER) {
      maxWallet=_maxWallet;
       ...
       function updateMaxTx(uint _maxTx) external onlyRole(DEFAULT_OWNER) {
      maxTx=_maxTx;
    }
    ...
    function _transfer(
        ...
        require(to != address(0), "ERC20: transfer to the zero address");
         ...
         if(!Limtcheck[to]) {
            require(maxTx >=  amount, "ERC20: maxTx >= amount");
