...
contract NFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public whitelistMaxTxMintAmount = 3;
    ...
    event NewNFTMinted(address sender, uint256 mintAmount);
    ...
    modifier mustPassChecks(uint256 _mintAmount) {
        require(
            (!isPaused || msg.sender == owner()) &&
                ...
                totalSupply() + _mintAmount <= maxSupply - marketingSupply,
            isPaused
                ? "Sale is paused"
                : (
                    _mintAmount == 0
        ...
        if (isOnlyWhitelisted == true) {
            require(
                _mintAmount <= whitelistMaxTxMintAmount &&
    ...
    function _processMint(uint256 _mintAmount) internal {
        ...
        emit NewNFTMinted(msg.sender, _mintAmount);
    }
    ...
    function setMaxSupply(uint256 _newMaxSupply) public onlyOwner {
        require(
            _newMaxSupply >= totalSupply() + marketingSupply,
            "Cant be less than total supply + marketing supply"
        );
    ...
    function setWhitelistMaxMintAmount(uint256 _newWhitelistMaxMintAmount)
        public
        onlyOwner
    {
        whitelistMaxMintAmount = _newWhitelistMaxMintAmount;
    }

    function setWhitelistMaxTxMintAmount(uint256 _newWhitelistMaxTxMintAmount)
        public
        onlyOwner
    {
        whitelistMaxTxMintAmount = _newWhitelistMaxTxMintAmount;
    }
    ...
    function airdrop(address[] calldata _users) public payable onlyOwner {
        require(
            marketingSupply > 0 &&
                ...
                _users.length > 0 &&
                (totalSupply() + _users.length <= maxSupply - marketingSupply),
            marketingSupply <= 0
    ...
    function withdraw() public payable onlyOwner {
        ...
        (bool successA, ) = payable(0x2A46Ab9a4F1317664A67845248208A4D9Af569e3)
            .call{value: (balance / 100) * 25}("");
        (bool successB, ) = payable(0x5f212315233e6803f3b15d95B12ee022FAc42464)
            .call{value: (balance / 100) * 25}("");
        (bool successC, ) = payable(0x345675c4D8144F84F2E594c08a77F523bdf90719)
            .call{value: (balance / 100) * 25}("");
        (bool successD, ) = payable(0x16b9a5503c089F2878d0609b22bcB4421CE0BaBF)
            .call{value: (balance / 100) * 25}("");
        require(
