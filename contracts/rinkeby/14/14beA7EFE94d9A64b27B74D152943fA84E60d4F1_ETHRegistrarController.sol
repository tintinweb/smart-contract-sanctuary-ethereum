...
contract ETHRegistrarController is Ownable {
    ...
    function _consumeCommitment(string memory name, uint duration, bytes32 commitment) internal returns (uint256) {
        ...
        require(commitments[commitment] + minCommitmentAge <= block.timestamp,"1111");

        // If the commitment is too old, or the name is registered, stop
        require(commitments[commitment] + maxCommitmentAge > block.timestamp,"222");
        require(available(name),"333");

        delete(commitments[commitment]);
        console.log("0010");
        uint cost = rentPrice(name, duration);
        console.log("0012");
        require(duration >= MIN_REGISTRATION_DURATION,"444");
        console.log("myg.value %d,cost %d",msg.value,cost);
        require(msg.value >= cost,"555");
