...
// Request creation
    function createTransaction(
        ...
        require(_receiver != msg.sender, "Requestor can't be the same as receiver");
        require(_myNFT.ownerOf(_myToken) != _wantNFT.ownerOf(_wantToken), "Can't swap NFT to the same owner"); //Optional
...
// Confirm transaction
    ...
    function confirmTransaction(
        ...
        require(_wantNFT == transactions[_transactionId].wantNFT, "Not requested NFT"); //NFT確認
        require(_wantToken == transactions[_transactionId].wantToken, "Not requested NFT"); //NFT Id 確認
        require(msg.sender == transactions[_transactionId].receiver, "Not correct receiver");// 交換者確認
        require(transactions[_transactionId].state == uint256 (transactionState.Pending), "Already confirmed or Revoked");
        ...
        transactions[_transactionId].state = uint256 (transactionState.Completed);
        
    }

// Revoke confirmation
    function revokeTransaction(uint256 _transactionId) public whenNotPaused {
        require(msg.sender == transactions[_transactionId].requestor, "Need requestor to revoke");
