...
contract ManagedVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, IManagedVault {
     ...
     * {depositNative} function won't work unless this function is set to true.
     *
    ...
    function setPrices(uint blockNumber, uint vaultPrice_, uint nativePrice_, IERC20[] calldata tokenAddresses, uint[] calldata tokenPrices_) external onlyOwner {
        ...
        for (uint i = 0; i < tokenDepositStates.length(); i++) {
            ...
            if(depositTokenAddress != tokenAddresses[i]) {
                revert InvalidTokenAddress({
                    givenTokenAddress: tokenAddresses[i],
                    expectedTokenAddress: depositTokenAddress
                });
            }

            if (tokenPrices[depositTokenAddress] != tokenPrices_[i]) {
                tokenPrices[depositTokenAddress] = tokenPrices_[i];
     ...
     * Emits {Minted} events.
     */
    function mint(uint maxMints) external onlyOwner {
        uint mintCount = 0;

        //put storage variables on stack to avoid SLOAD operations each iteration
        ...
        // Mint for tokens
        uint tokenLength = tokenDepositStates.length();
        for (uint tokenIndex = 0; tokenIndex < tokenLength;) {
     ...
     */
    function invoke(address target, bytes calldata data, uint value) external onlyOwner returns (bytes memory) {
     ...
     * Only considers the blocks at or before setPriceBlockNumber.
