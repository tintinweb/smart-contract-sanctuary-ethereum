...
contract SocalSale is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => uint256) public nftsaleToPrice;
    ...
    function mint(uint256 _count) public payable {
        ...
        require(
            ...
            openPresale,
            // || openPublicsale,
            "Socalsale: minting is either paused or not yet opened."
        );
        ...
        if (msg.sender != owner()) {
            ...
            require(
                IERC20(token).allowance(_msgSender(), address(this)) >= totalPrice,
                "Landsale: plase approve us to spend you MR tokens"
            );
            IERC20(token).transferFrom(_msgSender(), wallet, totalPrice);
        }
    ...
    function addNftsForSale(uint256[] calldata tokenIds,uint256[] calldata prices) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(ownerOf(tokenIds[i]) == msg.sender,"NA");
            nftsaleToPrice[tokenIds[i]] = prices[i];
        }
     ...
     function removeNftsForSale(uint256[] calldata tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(ownerOf(tokenIds[i]) == msg.sender,"NA");
            nftsaleToPrice[tokenIds[i]] = 0;
        }
    }

    function buyNft(uint256 tokenId) external payable {
        require(nftsaleToPrice[tokenId] > 0,"Socalsale: transfer required amount");
        require(ownerOf(tokenId) != msg.sender,"You are the owner");
        require(IERC20(token2).balanceOf(msg.sender) >= nftsaleToPrice[tokenId]);
        IERC20(token2).transferFrom(msg.sender, ownerOf(tokenId), nftsaleToPrice[tokenId]);
    ...
    function updateToken(address newToken_,bool isFirstToken) public onlyOwner {
        isFirstToken ? token = newToken_: token2 = (newToken_);
    }
    ...
    function toggleWhitelist(address newAddress) public onlyOwner {
        require(newAddress != address(0), "Landsale: whitelist address is zero.");
        whitelists[newAddress] = !whitelists[newAddress];
    }
