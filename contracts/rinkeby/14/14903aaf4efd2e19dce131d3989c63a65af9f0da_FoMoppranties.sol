...
contract FoMoppranties is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string public baseURI;
  uint256 public mintCost = 0.0001 ether;
  uint16 public constant maxSupply = 196;
  uint16[maxSupply] private ids;
  uint16 private index=maxSupply/2;
  uint8 public maxMintPerTx = 10;
  uint8 public maxMintPerWallet=20;
  bool public mintPaused = true;
  bool public secondPhase=false;
  ...
  function mint(uint256 _mintAmount)public payable 
  {
    ...
    require(supply + _mintAmount <= maxSupply, "No Mint available, max supply");
    //Mint only 1098 NFTs on first phase
    require(secondPhase || supply+_mintAmount<=(maxSupply/2), "Second phase not ready yet");
    require(_mintAmount > 0, "Need more than 0");
    require(_mintAmount <= maxMintPerTx, "Per Tx limit");

    if (msg.sender != owner()) {
      require(balanceOf(msg.sender)+_mintAmount<=maxMintPerWallet, "Per wallet limit");
  ...
  function _pickRandomUniqueId(uint256 random) private returns (uint256 id) {
    ...
    if(!secondPhase){
      ...
      uint256 newLen=len+(maxSupply/2);
      ids[randomIndex] = uint16(ids[newLen - 1] == 0 ? newLen - 1 : ids[newLen - 1]);
      ids[newLen - 1] = 0;
    }
  }
  ...
  function setmaxMintPerWallet(uint8 _newmaxMintPerWallet) public onlyOwner() {
 ...
 function startSecondPhase() public onlyOwner {
    secondPhase = true;
