    ...
    ReentrancyGuardUpgradeable
{
    ...
    function burnWhalesFromStakingReserve(uint256 amount)
        external
    ...
    {
        require(
            amount <= totalWhalesEarnedPerAddress[msg.sender],
            "not enough whales"
        );
        totalWhalesEarnedPerAddress[msg.sender] -= amount;
     ...
     */
    function _adjustLegendryMultiplierForPassiveYeild(uint16[] memory tokenIds)
        internal
    {
        for (uint256 i = 0; i < tokenIds.length; i++) {
    ...
    modifier updatePassiveRewardsForUser() {
        if (!userTimestampInit[msg.sender]) {
            passiveYeildTimeStamp[msg.sender] = 1646581439;
        ...
        _adjustLegendryMultiplierForPassiveYeild(userLedger);
        delete userLedger;
    ...
    /*
     * @dev The function that burns WHALES without needing to claim them
     * calls the modifier and then burns @param amount if there is sufficent tokens.
     */
    function burnWhalesFromPassiveReserve(uint256 amount)
        external
        updatePassiveRewardsForUser
        nonReentrant
        whenNotPaused
    {
        require(
            amount <= totalWhalesPassivelyEarned[msg.sender],
            "not enough whales"
        );
        totalWhalesPassivelyEarned[msg.sender] -= amount;
        Whales.burn(msg.sender, amount);
    }

    function updatePassiveRewards() external updatePassiveRewardsForUser {}
    ...
    function getPassiveYeildForUser()
        external
        view
        whenNotPaused
        returns (uint256)
    {
        uint16[] memory ledger = Blues_V2.getOwnerLedger(msg.sender);
        uint256 holdings = ledger.length;
        delete ledger;
        uint256 yeild = (holdings * userHoldingsMultiplier[msg.sender]);
        yeild += (legendariesUnstaked[msg.sender] * DAILY_WHALES_RATE_LEGENDRY);

        return yeild;
    }

    function burnWhalesOnHand(uint256 amount) external whenNotPaused nonReentrant {
        require(amount <= Whales.balanceOf(msg.sender),"Not enough whales to burn");
        Whales.burn(msg.sender, amount);
    }
