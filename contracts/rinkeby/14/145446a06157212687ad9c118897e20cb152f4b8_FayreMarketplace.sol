...
contract FayreMarketplace is ERC2771ContextUpgradeable {
    /**
        ...
        E#3: only a valid free minter can mint
    ...
    */
    ...
    function changeAddressIsMarketplaceManager(address marketplaceManagerAddress, bool state) external onlyOwner {
        isMarketplaceManager[marketplaceManagerAddress] = state;
    }
    ...
    function batchMint(MintTokenData[] calldata mintTokensData) external {
        require(remainingFreeMints[_msgSender()] >= mintTokensData.length, "E#21");

        for (uint256 i = 0; i < mintTokensData.length; i++) {
            remainingFreeMints[_msgSender()]--;
    ...
    function mint(AssetType assetType, string memory tokenURI, uint256 amount, uint256 royaltiesPct, string memory collectionName) external returns(uint256) {
        if (bytes(collectionName).length > 0)
            if (mintedCollectionsOwners[collectionName] != address(0))
                require(mintedCollectionsOwners[collectionName] == _msgSender(), "E#30");
            else
                mintedCollectionsOwners[collectionName] = _msgSender();
        
        require(remainingFreeMints[_msgSender()] > 0, "E#3");
    ...
    function acceptFreeOffer(uint256 bidId) external {
        ...
        require(bidTradeRequest.owner != _msgSender(), "E#28");
        require(bidTradeRequest.start > 0 && bidTradeRequest.expiration > block.timestamp, "E#29");
    ...
    function transferMintedCollectionOwnership(string calldata collectionName, address to) external {
        ...
        emit TransferMintedCollectionOwnership(collectionName, _msgSender(), to);
    }
    ...
    function renameMintedCollection(string calldata collectionName, string calldata newCollectionName) external onlyMarketplaceManager {
        require(bytes(newCollectionName).length > 0, "E#27");
    ...
    function _mint(MintTokenData memory mintTokenData) private returns(uint256) {
        ...
        if (mintTokenData.assetType == AssetType.ERC721) {
            ...
            tokenId = IFayreSharedCollection721(fayreSharedCollection721).mint(_msgSender(), mintTokenData.tokenURI);

            _tokensData[_networkId][fayreSharedCollection721][tokenId].creator = _msgSender();
            ...
            tokenId = IFayreSharedCollection1155(fayreSharedCollection1155).mint(_msgSender(), mintTokenData.tokenURI, mintTokenData.amount);

            _tokensData[_networkId][fayreSharedCollection1155][tokenId].creator = _msgSender();
            _tokensData[_networkId][fayreSharedCollection1155][tokenId].royaltiesPct = mintTokenData.royaltiesPct;
        }

        emit Mint(_msgSender(), mintTokenData.assetType, tokenId, mintTokenData.amount, mintTokenData.royaltiesPct, mintTokenData.tokenURI, mintTokenData.collectionName);
