...
contract MoonbirdsAtHome is ERC721A, Ownable {
    ...
    function mint(uint16 quantity) public payable {
        ...
        if (sender != owner()) {
            ...
            require(quantity > 0 && quantity <= 15, "Minting is limited to max. 15 per wallet");
            require(balanceOf(sender) + quantity <= 15, "Minting is limited to max. 15 per wallet");
    ...
    function activate(address ogStatsAddress) external onlyOwner {
