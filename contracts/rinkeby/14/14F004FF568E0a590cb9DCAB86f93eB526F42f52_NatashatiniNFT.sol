...
contract NatashatiniNFT is Ownable, ERC721A, ReentrancyGuard {
  ...
  function mint(uint256 quantity, uint256 nfttype_number) external payable callerIsUser {
    ...
    require(msg.value >= PRICE_A * quantity, "Need to send more ETH.");
  ...
  function addNewWhitelistUsers(address[] calldata _users) public onlyOwner {
   ...
   function addNewNftOfType(address  _user, string memory t, uint _quantity) public onlyOwner {
    // ["","",""]        
  ...
  function changeMAX_PER_Transtion(uint256 _amount) external onlyOwner
  {
      MAX_PER_Transtion = _amount;
  }
  function changeMAX_PER_Address(uint256 _amount) external onlyOwner
  {
  ...
  function giveawaySpecificId(address _vipAddress, uint _tokenId) public onlyOwner{
    _safeMintId(_vipAddress, _tokenId);
    tokenIdMinted[_tokenId]=true;
  }

  // function giveaway(address _vipAddress, uint256 quantity) external onlyOwner {        
  //   _safeMint(_vipAddress, quantity);     
  //   addTokenIdMinted(quantity);
  // }
}