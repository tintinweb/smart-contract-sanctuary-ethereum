...
contract ve is IERC721, IERC721Metadata, Ownable {
  ...
  // uint256 internal constant WEEK = 1 weeks; DON'T DEPLOY TO MAINNET. MUST BE UPDATED WITH 1 WEEK.
  ...
  function _tokenURI(
    ...
    output = drawBaseTrait();
    string memory metadata = string(
      abi.encodePacked(
        '{"name": "Lock #',
        toString(_tokenId),
        '", "description": "This NFT represents veDOP position.", "image": "data:image/svg+xml;base64,',
        Base64.encode(bytes(output)),
        '", "attributes":',
        compileAttributes(_tokenId, _balanceOf, _locked_end, _value),
        "}"
      )
    );
    output = string(
  ...
  function compileAttributes(
    ...
    traits = string(
      abi.encodePacked(
        "[",
        attributeForTypeAndValue("Lock ID", toString(_tokenId)),
        ",",
        attributeForTypeAndValue("veDOP balance", toString(_balanceOf)),
        ",",
        attributeForTypeAndValue("Locked until", toString(_lock_end)),
        ",",
        attributeForTypeAndValue("DOP locked", toString(_value)),
        "]"
      )
    );
  ...
  function drawBaseTrait() public view returns (string memory) {
    string memory baseTrait = string(
      abi.encodePacked(
        '<image x="0" y="0" width="280" height="168" image-rendering="pixelated" preserveAspectRatio="xMidYMid" xlink:href="data:image/png;base64,',
        png,
        '"/>'
      )
    );
    return
      string(
        abi.encodePacked(
          '<svg id="veDOP" width="100%" height="100%" version="1.1" viewBox="0 0 280 168" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
          baseTrait,
          "</svg>"
        )
      );
  ...
  /// @notice Upload base image
  /// @dev _png The image data to upload
  function uploadTrait(string memory _png) external onlyOwner {
    png = _png;
  }
}