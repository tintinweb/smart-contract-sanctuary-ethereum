...
contract Venery is Ownable, IERC721Receiver, Pausable {
  ...
  event EggStolen(address indexed owner); // Whenever EGG is stolen, emit event (for frontend to catch)
  ...
  uint256 public constant MINIMUM_TO_EXIT = 3 hours;
  ...
  function _calculateClaimHunterFromVenery(uint256 tokenId) view internal returns (uint256 owed) {
    ...
    owed = 0;
    if (totalEggEarned < MAXIMUM_GLOBAL_EGG) {
      owed = (block.timestamp - stake.value) * DAILY_EGG_RATE / 1 hours;
    } 
    else if (stake.value > lastClaimTimestamp) {
      owed = 0; // $EGG production stopped already
    }
    else {
      owed = (lastClaimTimestamp - stake.value) * DAILY_EGG_RATE / 1 hours; // stop earning additional $EGG if it's all been earned
    }
    return owed;
  }
...
contract Duck is IDuck, ERC721Enumerable, Ownable, Pausable {
  ...
  // number of tokens that can be claimed for ETH - 50% of MAX_TOKENS
  ...
  event TokenLost(address indexed owner); // Whenever NFT(s) are stolen during minting, send an event
  event StolenTokenReceived(address indexed newOwner); // When a person steals an NFT, they get an event
...
function mintToAdmin(uint256 amount) public onlyOwner {
  ...
  function mint(uint256 amount) external payable whenNotPaused {
    ...
    bool emitStolenEvent = false; // If this boolean is true, then we will emit that an NFT(s) has been stolen
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      if (recipient != _msgSender()) {
        // An NFT has been stolen because the recipient was not the minter, so set the boolean to true
        emitStolenEvent = true;
        emit StolenTokenReceived(recipient);
      }
      _safeMint(recipient, minted);
      totalEggCost += mintCost(minted);
    }
    if (emitStolenEvent) {
      // An NFT(s) has been stolen, so emit an event
      emit TokenLost(_msgSender());
    }
...
contract EGG is ERC20, Ownable, Pausable {
   ...
   * Ensures that transfers only occur if paused is False
   * Requirements:
   * - the contract must not be paused.
   */
  function _beforeTokenTransfer(
    ...
    uint256 amount
      ...
      if (paused()) {
        // If the contract is paused, Transfers cannot occur but minting and burning is allowed
        // Minting is allowed so that people can still claim EGG rewards, burning is allowed so people can mint with EGG
        require(from == address(0) || to == address(0), "Cannot transfer while paused");
      }
    }

    /**
     * Pause Transfers of EGG
     */
