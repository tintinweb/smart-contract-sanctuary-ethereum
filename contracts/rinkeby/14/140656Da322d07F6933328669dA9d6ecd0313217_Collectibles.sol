...
// File contracts/Collectibles.sol
...
contract Collectibles is ERC721URIStorage, IERC2981 {
    ...
    /// @dev Collectible calls cancel listing _beforeTokenTransfer. So, if someday the collectible wants to go independent, then the address can be set to address(0)
    /// @param _newmarketplaceAddress New marketplace proxy address in case changed
    /// @custom:modifier Only contract owner should be change the marketplace address
    function changeMarketplaceAddress(address _newmarketplaceAddress) external {
        require(
            contractOwner == msg.sender,
    ...
    function _beforeTokenTransfer(
        ...
        if (marketplaceAddress != address(0) && from != address(0)) {
            IMarketplace(marketplaceAddress).cancelListing(
                address(this),
                uint32(tokenId)
            );
        }
