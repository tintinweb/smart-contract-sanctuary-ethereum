...
contract BridgeLeveragedPositionController is AbstractController, IERC1155Receiver {
    ...
    function stake(uint256 _amount) external override onlyIndex {
        ...
        uint256 _stblAmount = _swapTokenForToken(
            _amount,
            address(base),
            address(stblToken),
            swapVia
    ...
    function unstake(bytes calldata _signature) external override onlyIndex returns (uint256) {
        ...
        uint256 _baseAmount = _swapTokenForToken(
            _amountStbl,
            address(stblToken),
            address(base),
            swapVia
    ...
    function _sellBMI(uint256 _amount) internal returns (uint256 _baseAmount) {
        ...
        _baseAmount = _swapTokenForToken(
            _amount,
            address(bmiToken),
            address(base),
            swapRewardsVia
    ...
    function netWorth() external view override returns (uint256 _worth) {
        ...
        _worth = _priceFeed
            .howManyTokensAinB(address(base), address(stblToken), swapVia, _stakedStbl, true)
            .add(_rewardsBase);
    }
...
interface IPositionController {
    ...
    function unstake(bytes calldata _signature) external returns (uint256);
