...
contract Vault is IVault, IncreOwnable {
    ...
    */
        ...
        returns (int256)
    {
        ...
        return _getDiscountedCollateralUSDValue(whiteListedCollaterals[tokenIdx], collateralBalance);
    }
    ...
    function _getUserReserveValue(address user, bool isDiscounted) internal view returns (int256) {
        ...
        for (uint256 i = 0; i < collaterals.length; i++) {
            ...
            if (collateralBalance != 0) {
                if (isDiscounted) {
                    reserveValue += _getDiscountedCollateralUSDValue(collaterals[i], collateralBalance);
                } else {
    ...
    function _getDiscountedCollateralUSDValue(Collateral memory collateral, int256 collateralBalance)
        internal
        view
        returns (int256)
    {
    ...
    /// @param collateralBalance Balance in the collateral. 18 decimals
    /// @return collateralUSDValue Value of the collateral in USD. 18 decimals
    function _getUndiscountedCollateralUSDValue(Collateral storage collateral, int256 collateralBalance)
        internal
        view
        returns (int256)
    {
