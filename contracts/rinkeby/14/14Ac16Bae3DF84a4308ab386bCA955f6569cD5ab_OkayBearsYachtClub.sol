...
contract OkayBearsYachtClub is ERC721A, Ownable, ReentrancyGuard {
    ...
    function freeMint(uint256 _mintAmount) public mintCompliance(_mintAmount) {
        require(!paused && isFreeMintOpen, "OBYC: Free mint phase not open yet!");

        require(totalSupply() + _mintAmount <= maxFreeMintSupply, "OBYC: Exceeds max free mint supply!");
        require(_numberMinted(msg.sender) + _mintAmount <= maxFreeMintAmountPerAddr, "OBYC: Exceeds max free mint per wallet!");
        _safeMint(_msgSender(), _mintAmount);
    }

    function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
        require(_numberMinted(msg.sender) + _mintAmount <= 100, "OBYC: Exceeds max mint per wallet!");
