    ...
    Ownable
{
    ...
    function __mint(address _to, uint16 _id) internal {
        _safeMint(_to, _id);
    ...
    function enableWhitelist(
        ...
        require(drop >= 1 && drop <= 5, "Drop not recognized");
        require(
            enabledDrops[drop] == 0 || enabledDrops[drop] > block.timestamp,
        ...
        enabledWhitelists[drop] = start;
        whitelistsPrices[drop] = _price;
        emit EnableWhitelist(addresses, drop, numAllowedToMint, start, _price);
    }
    ...
    function whitelistMint(address to, uint16 _id) external payable {
        uint16 drop = getDrop(_id);
        require(enabledWhitelists[drop] != 0, "Drop not enabled");
        require(
        ...
        require(
            enabledDrops[drop] > block.timestamp,
            "Drop has already started"
        );
