...
contract Market is ERC721, ERC721Burnable, Ownable {
    ...
    function cancel(uint256 _lockId) external onlyLender(_lockId) onlyBeforeLock(_lockId) {
    ...
    // This function is called after the LendRent is removed from cancel(), so it requires a bare argument
    // function _redeemNFT(address _vaultAddress, uint256 _tokenId) internal {
    function _redeemNFT(uint256 _lockId) internal {
        ...
        // oNFT
        // IVault(_vaultAddress).approve(address(this), _lockId);
