    ...
    EIP712Upgradeable
{
    ...
    function hashOrder_t(Order_t memory _order) public view returns (bytes32 hash) {
        bytes memory array = abi.encodePacked(
             keccak256(
                "Order_t_t(address seller,address contractAddress,uint256 royaltyFee,address royaltyReceiver,address paymentToken,uint256 basePrice,uint256 listingTime,uint256 expirationTime,uint256 nonce,uint256 tokenId,string uri,string objId)"
            ),
            _order.seller,
            _order.contractAddress,
            _order.royaltyFee,
