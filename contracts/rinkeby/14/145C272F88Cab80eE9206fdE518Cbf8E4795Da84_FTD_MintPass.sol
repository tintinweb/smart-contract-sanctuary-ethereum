...
contract FTD_MintPass is ERC721, ERC721Enumerable , Ownable {
    ...
    uint256 constant public Public_Supply = 5400;
    uint256 constant public Reserve_Supply = 600;
    ...
    string contractUri = 'https://gateway.pinata.cloud/ipfs/QmfG6Lhi49HStCTWS9LmAAsvR2PKyVuxPHPVFzP7tqamQ5';
    address RoyalityReciever;

    constructor() ERC721("FTD LAB MINT PASS", "FLMP") {
        royalityFeeInBips = 750;
        RoyalityReciever = 0xFCeC718eb15d249Fa74EAdC8cc9e1D2814A1faCB;
    }
    
    function mint(uint256 _mintAmount) public payable {
        checker(_mintAmount);

        require(msg.value == _mintAmount * cost,"Invalid Cost!!");
        require(publicReserve + _mintAmount <= Public_Supply,"Public Passes Sold Out!!");
    ...
    function airDrop(address _recipient) public onlyOwner {
        checker(1);
        require(TeamReserve + 1 <= Reserve_Supply,"Public Passes Sold Out!!");
        unchecked{
            supply.increment();
    ...
    function batchmint(uint256 _mintAmount) public onlyOwner {
        checker(_mintAmount);
        require(TeamReserve + _mintAmount <= Reserve_Supply,"Public Passes Sold Out!!");
        for (uint256 i = 1; i <= _mintAmount; i++) {
            supply.increment();
            _safeMint(msg.sender, supply.current());
            TeamReserve++;
    ...
    function batchAirDrop(address[] calldata _adr) public  onlyOwner {
        ...
        checker(length);
        require(TeamReserve + length <= Reserve_Supply,"Public Passes Sold Out!!");
