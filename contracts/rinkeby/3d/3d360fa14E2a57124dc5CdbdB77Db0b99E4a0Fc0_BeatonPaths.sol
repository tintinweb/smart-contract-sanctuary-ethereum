...
contract BeatonPaths is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function mint(
        ...
        _nftInfos[newTokenId] = NFTInfo({minBidPrice: minBidPrice, auctionDurationSeconds: auctionDurationSeconds});
    ...
    function _requireNFTExists(uint256 tokenId) internal view {
        require(
            _nftInfos[tokenId].minBidPrice != 0 && _nftInfos[tokenId].auctionDurationSeconds != 0,
    ...
    function bid(uint256 tokenId, uint256 bidPrice) public {
        ...
        require(
            ...
            "Unable to bid: bid must be more than current bid"
        );
        require(!_auctionStarted(tokenId) || _auctions[tokenId].expiration > block.timestamp, "Auction has ended");
        ...
        if (_auctionStarted(tokenId)) {
            ...
            (bool isSafe, uint256 auctionExpiration) = SafeMath.tryAdd(
                block.timestamp,
                _nftInfos[tokenId].auctionDurationSeconds
            );
    ...
    function updateAuctionDuration(uint256 tokenId, uint32 auctionDurationSeconds) public onlyOwner {
        _requireNFTExists(tokenId);
        require(!_auctions[tokenId].claimed, "NFT has been claimed");
    ...
    function updateAuctionExpiration(uint256 tokenId, uint256 auctionExpiration) public onlyOwner {
        ...
        require(auctionExpiration != 0, "expiration cannot be zero");
