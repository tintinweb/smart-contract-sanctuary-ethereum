...
contract SrcCore is Ownable {
    ...
    address private safeboxContract;

    uint8 public srcChainId;
    
    mapping(uint8 => address) private multichainDstContracts;

    mapping(uint256 => srcNFTInfo) private lotteries;

    // The Multichain anycall contract on all network
    //address private anycallcontract=0xC10Ef9F491C9B59f936957026020C321651ac078;
    address private anycallcontract=0x273a4fFcEb31B8473D51051Ad2a2EdbB7Ac8Ce02; //rinkeby

    constructor(address _safeboxContract, uint8 _srcChainId) {
    ...
    function setMultiChainDstContract(uint8 chainId, address contractAddress) public {
        multichainDstContracts[chainId] = contractAddress;
    }

    function getMultiChainDstContract(uint8 chainId) public view returns(address) {
        return multichainDstContracts[chainId];
    }

    function createPropto(uint8 dstChainId, address token, uint256 tokenID, uint256 initialPrice, uint8 ProptoType) public {
        ...
        if(dstChainId == srcChainId) {
            ...
            ICallProxyAnyCallV6(anycallcontract).anyCall(
                getMultiChainDstContract(dstChainId),
                abi.encode(_msg),
                address(0),
                dstChainId,
                2
    ...
    function calcSrcFees(uint256 _toChainID,uint256 _dataLength) public view returns (uint256) {
        return ICallProxyAnyCallV6(anycallcontract).calcSrcFees("0", _toChainID, _dataLength);
    }

    function getPropto(uint256 index) public view returns(uint8, address, uint256, address, uint256, uint8) {
