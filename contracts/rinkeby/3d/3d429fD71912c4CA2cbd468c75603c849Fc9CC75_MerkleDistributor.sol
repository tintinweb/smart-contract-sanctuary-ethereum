...
contract MerkleDistributor is IMerkleDistributor, Ownable, Pausable {
    ...
    constructor(address _token, bytes32 _merkleRoot) {
        ...
        emit NewAirdropRound(0, _merkleRoot);
    }
     ...
     */
    function adminSetNewRound(uint256 _newRound, bytes32 _newMerkleRoot) external onlyOwner {
        require(_newRound >= currentRound, "MerkleDistributor: Expired airdrop.");
        currentRound = _newRound;
        merkleRoot = _newMerkleRoot;
        emit NewAirdropRound(_newRound, _newMerkleRoot);
    }
    ...
    function adminWithdrawUnclaimed(address _beneficiary) external onlyOwner {
        ...
        require(IERC20(token).transfer(_beneficiary, _currentBalance), 'MerkleDistributor: Transfer failed.');
    }
    ...
    function claim(uint256 _index, address _account, uint256 _amount, bytes32[] calldata _merkleProof) external override whenNotPaused {
        ...
        require(IERC20(token).transfer(_account, _amount), 'MerkleDistributor: Transfer failed.');
