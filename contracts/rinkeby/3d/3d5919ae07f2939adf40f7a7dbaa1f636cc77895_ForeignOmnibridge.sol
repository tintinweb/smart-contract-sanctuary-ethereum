...
abstract contract TokensRelayer is BasicAMBMediator, ReentrancyGuard {
    ...
    function onTokenTransfer(
        ...
        if (!lock()) {
            ...
            bridgeSpecificActionsOnTokenTransfer(msg.sender, _from, receiver, _value, data, _from);
        }
        return true;
    }
    ...
    function relayTokens(
        ...
        address _receiver,
        uint256 _value,
        address _senderOrigin
    ) external {
        _relayTokens(token, _receiver, _value, new bytes(0), _senderOrigin);
    }
     ...
     * @param _value amount of tokens to be transferred to the other network.
     */
    function relayTokens(IERC677 token, uint256 _value, address _senderOrigin) external {
        _relayTokens(token, msg.sender, _value, new bytes(0), _senderOrigin);
    }
    ...
    function relayTokensAndCall(
        ...
        _relayTokens(token, _receiver, _value, _data, _senderOrigin);
    }
    ...
    function _relayTokens(
        ...
        bridgeSpecificActionsOnTokenTransfer(address(token), msg.sender, _receiver, balanceDiff, _data, _senderOrigin);
    }
    ...
    TokensBridgeLimits
{
        ...
        validAddress(_receiver)
    {
        ...
        _recordBridgeOperation(_messageId, _token, _receiver, diff, _receiver);
    }
    ...
    function _recordBridgeOperation(
        ...
        emit TokensBridgingInitiated(_token, _sender, _value, _messageId, _senderOrigin);
    }
...
contract ForeignOmnibridge is BasicOmnibridge, GasLimitManager, InterestConnector {
    ...
    function bridgeSpecificActionsOnTokenTransfer(
        ...
        bytes32 _messageId = _passMessage(data, true);
        _recordBridgeOperation(_messageId, _token, _from, _value, _senderOrigin);
    }
