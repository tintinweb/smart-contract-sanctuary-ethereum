...
contract ERC721AFacet is ERC721A {
    ...
    function mint(address to, uint256 quantity) public payable whenNotPaused {
        require(
            !s.allowListEnabled,
            "Allow list is enabled, use mintMerkle with proof"
        );
    ...
    function convertUSDtoWei(uint256 _price) internal view returns (uint256) {
        // uint256 oneEth = 1000000000000000000; // 1e18
        // uint256 priceFromOracle = PriceConsumer.getLatestPrice();
        // uint256 decimals = 1000000; // 1e6
        // uint256 ethPerUsd = oneEth.div(
        //     uint256(priceFromOracle).div(uint256(decimals))
        // );
        uint256 ethPerUSD = 1e18 / (PriceConsumer.getLatestPrice() / 1e6);

        // console.log("usdPerEth: ", usdPerEth);
        // console.log("_price: ", _price);
        uint256 priceInWei = ethPerUSD * _price;
