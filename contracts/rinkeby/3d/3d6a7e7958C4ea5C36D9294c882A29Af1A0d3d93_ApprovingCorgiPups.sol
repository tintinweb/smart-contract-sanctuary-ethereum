...
contract ApprovingCorgiPups is ERC721A, Ownable, AccessControl {
    ...
    uint256 public numberOfAllowedBreedingP1 = 1;
    uint256 public numberOfAllowedBreedingP2 = 2;
    ...
    IERC721Enumerable public constant corgiContract = IERC721Enumerable(corgiAddress);
    ...
    constructor() ERC721A("Approving Corgi Pups", "APUPPIES", 50, 9999)  {}
    ...
    modifier isSecured(uint8 mintType) {
        ...
        if(mintType == 1) {
            require(isPhase1Active, "PHASE_2_IS_NOT_YET_ACTIVE");
        }
        _;
    }
     ...
     * Breed Corgis
     */
    function breedCorgisP1(uint256 _parent1, uint256 _parent2, uint64 expireTime, bytes memory sig) external isSecured(1) {
        bytes32 digest = keccak256(abi.encodePacked(msg.sender,expireTime));
        require(isAuthorized(sig,digest),"CONTRACT_MINT_NOT_ALLOWED");
        require(corgiContract.ownerOf(_parent1) == msg.sender, "YOU_DO_NOT_OWN_CORGI_1");
        require(corgiContract.ownerOf(_parent2) == msg.sender, "YOU_DO_NOT_OWN_CORGI_2");
        require(numberOfBreedTimes[_parent1] < numberOfAllowedBreedingP1, "REACHED_MAXIMUM_BREED_ALLOWED_P1");
        require(numberOfBreedTimes[_parent2] < numberOfAllowedBreedingP1, "REACHED_MAXIMUM_BREED_ALLOWED_P1");
    ...
    function breedCorgisP2(address minter, uint256 numToMint) external {
        require(allowedMinters[msg.sender], "THIS_CONTRACT_IS_NOT_ALLOWED_TO_MINT");
        _safeMint(minter, numToMint);
    }

    function breedCorgisP3(address minter, uint256 numToMint) external {
    ...
    /**
     * Returns Pups of the Caller
     */
    function pupsOfOwner(address _owner) external view returns(uint256[] memory) {
        uint256 tokenCount = balanceOf(_owner);
    ...
    function setMinterRole2(address minter) external onlyOwner {
...
contract Thorgi is ERC20Burnable, Ownable {
    ...
    constructor() ERC20("Thorgi", "THORGI") {
    ...
    modifier isSecured(uint8 mintType) {
        ...
        if(mintType == 1) {
            require(stakingLive, "STAKING_IS_NOT_YET_ACTIVE");
        }
        if(mintType == 2) {
    ...
    function stakePups(uint256[] memory tokenIds) external isSecured(1) {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 id = tokenIds[i];
    ...
    function stakeBones(uint256 tokenId) external isSecured(1) {
    ...
    function unstakePups(uint256[] memory tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(stakedBone.length > 0) {
                ...
                uint256 reward = ((block.timestamp - pupTimeStaked[id]) * corgiRatePerDay);
                uint256 multiplier = 1e18 + (numOfDays * 150 / 10000);
                totalRewards += (reward * multiplier) / 1e18;
            } else {
    ...
    function testClaim(uint256 timestamp) external {
        uint256 reward = ((block.timestamp - timestamp) * corgiRatePerDay);
        _mint(msg.sender, reward);
    }

    function testMultiplier(uint256 timestamp) external view returns(uint256) {
        uint256 totalRewards = 0;
        uint256 numOfDays = ((block.timestamp - timestamp) / 1 days) * 1e18;
        uint256 reward = ((block.timestamp - timestamp) * corgiRatePerDay);
        uint256 multiplier = 1e18 + (numOfDays * 150 / 10000);
        totalRewards = (reward * multiplier) / 1e18;
    ...
    function checkAllRewardsFromPups(address _owner) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < pups.length; i++) {
            if(stakedBone.length > 0) {
                uint256 numOfDays = ((block.timestamp - corgTimeStaked[pups[i]]) / 1 days) * 1e18;
                uint256 reward = ((block.timestamp - corgTimeStaked[pups[i]]) * corgiRatePerDay);
                uint256 multiplier = 1e18 + (numOfDays * 150 / 10000);
                totalRewards += (reward * multiplier) / 1e18;
            } else { 
                totalRewards += ((block.timestamp - corgTimeStaked[pups[i]]) * corgiRatePerDay);
            }
        }
    ...
    function airDrop(uint256 amount, uint64 expireTime, bytes memory sig) external isSecured(2) {
