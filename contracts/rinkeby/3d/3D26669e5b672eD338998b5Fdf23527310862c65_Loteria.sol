...
contract Loteria {
    address private dono_banca;
    address[] private jogadores;
    address payable ganhador;
    uint256 constant valor_aposta = .0001 ether;
    uint256 constant valor_taxa =   .00001 ether;
    event NovaAposta(address jogador, uint256 valor);
    event SorteadoVencedor(address vencedor, uint256 valor_ganho);
    enum States {
        Aberta,
        Fechada
    }
    ...
    function apostar() external payable {
        require(
            ...
            "VALOR INSUFICIENTE PARA GARANTIR APOSTA. VALOR DA APOSTA .0001 ether"
        );
    ...
    function aposta_total() public view returns (uint256) {
        return address(this).balance;
    }

    function aposta_dono_banca() public view returns (address) {
        return dono_banca;
    }

    function aposta_ganhador() public view returns (address) {
