...
contract VerumWorld is AbstractERC1155Factory {
    ...
    // Mersmeta Address
    address public mersmetaAddress;
    ...
    mapping(string => uint256) public artistBalance;  
    ...
    function initialize(
        ...
        address addressMersmeta
    ) initializer external {
        name_ = tokenName;
        symbol_ = tokenSymbol;
        mersmetaAddress = addressMersmeta;
        __Ownable_init();
    ...
    function mint(string memory collectionID, bytes32[] calldata merkleProof) external payable override nonReentrant {
        ...
        bool privateRound = activeBatch.isPrivate;
        ...
        if(privateRound){
            if(activeBatch.merkleHash != NULL_BYTES){
                ...
                privateClaimableAddress = mersmetaBalanceCheck(artistID, callerAddress, batchArrayIndex);
            }
        }
    ...
    function whiteListAddressCheck(uint256 artistID, address callerAddress, bytes32[] calldata merkleProof, uint256 artistBatchIndex, Batch memory activeBatch) internal view returns (bool) {
        ...
        IERC1155Upgradeable imersmmeta = IERC1155Upgradeable(mersmetaAddress);
        require(imersmmeta.balanceOf(callerAddress, 0) > 0, "VerumWorld: Insufficient mersmeta pass tokens");
        return true;
    }
    ...
    function mersmetaBalanceCheck(uint256 artistID, address callerAddress, uint256 artistBatchIndex) internal view returns (bool) {
        require(!whitelistClaimed[artistID][artistBatchIndex][callerAddress], "VerumWorld: Whitelisted user already minted for current batch");
        IERC1155Upgradeable imersmmeta = IERC1155Upgradeable(mersmetaAddress);
        require(imersmmeta.balanceOf(callerAddress, 0) > 0, "VerumWorld: Insufficient mersmeta pass tokens");
        return true;
    ...
    function createBatchForArtistCollection(string memory collectionID, uint256 startTime, uint256 endTime, uint256 mintPrice, bytes32 merkleHash, bool isPrivate) external override onlyArtist(collectionID) {
        ...
        Batch memory batch = Batch(startTime, endTime, mintPrice, merkleHash, isPrivate);
        ArtistCollection storage artistCollection = artistCollections[collectionID];
        require(artistCollection.isPaused != true , "VerumWorld: Artist is not active with verum world");
...
interface IVerumWorld {
     ...
     */
    function createBatchForArtistCollection(string memory collectionID, uint256 startTime, uint256 endTime, uint256 mintPrice, bytes32 merkleHash, bool isPrivate) external;
