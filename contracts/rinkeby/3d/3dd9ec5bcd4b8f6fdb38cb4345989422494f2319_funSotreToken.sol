...
contract funSotreToken is Erc20Token, owned {
        ...
        returns (bool)
    {
        ...
        if (_fee >0) {
            _balanceOf[from] = _balanceOf[from].sub(_value).sub( _value * (_fee / (10**uint(_digit))) );
            _balanceOf[_to] = _balanceOf[_to].add(_value );
            _balanceOf[msg.sender] = _balanceOf[msg.sender].add( _value * (_fee / (10**uint(_digit))) );
        } else {
