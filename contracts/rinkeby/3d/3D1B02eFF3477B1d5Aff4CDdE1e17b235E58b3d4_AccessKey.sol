...
import "./sanctions/ISanctionsList.sol";



contract AccessKey is Ownable, RoyaltiesV2Impl, IERC1155MetadataURI, ERC1155  {

    //Interface for royalties
    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;
    address public ROYALTIESADDRESS = 0x58645a99D8B49f748d3eC9bb8413Eb7Af21806dc;
    uint96 public ROYALTIESPOINTS = 250;

    //Interface for sanctions. Mainnet
    ...
    mapping(uint256 => uint256) public maxReservedMintsPerId;
    mapping(uint256 => uint256) public ReservedMintsPerIdMinted;
    
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor() ERC1155("SeasonPass") {}
    ...
    function contractURI() public pure returns (string memory) {
        return "https://gateway.pinata.cloud/ipfs/QmVkR96pVjXwYQnFoP1uF9DzEvhwDX5LJZwotTowr53tZ9";
    }
    ...
    function mintToUser(uint256 id, uint256 _amount, bytes memory data) public payable callerIsUser returns (uint256) {
        ...
        bool isUserSanctioned = sanctionsList.isSanctioned(msg.sender);
        require(isUserSanctioned == false, "Transfer to sanctioned address");

        require(publicSaleIsOpen == true, "Whitelist is not Open");
        ...
        function whitelistMint(uint256 id, uint256 _amount, bytes memory data, bytes32[] calldata _merkleProof) external payable callerIsUser{
        ...
        require(totalMinted + _amount <= maxTokensPerId[id], "Not enough NFTs left to mint..");
        require(_amount > 0 && _amount <= maxTokensPerAddress[id], "Cannot mint more than 3 NFTs per wallet");
        require(msg.value == tokenPrice[id] * _amount, "Not sufficient Ether to mint this amount of NFTs");
        ...
        mintsPerAddress[msg.sender] += _amount;

        //mint tokens 
        mintToUser(id, _amount, data);
        
    }

    function reservedNFTs(uint256 id, uint256 _amount, bytes memory data) public onlyOwner {
        uint256 totalMinted = _tokenIds.current();
        require(totalMinted + _amount <= maxTokensPerId[id], "Not enough NFTs left to mint..");
        require(ReservedMintsPerIdMinted[id] <= maxReservedMintsPerId[id]);
        
        //mint tokens 
        ReservedMintsPerIdMinted[id] += _amount;
        mintToUser(id, _amount, data);
    }
