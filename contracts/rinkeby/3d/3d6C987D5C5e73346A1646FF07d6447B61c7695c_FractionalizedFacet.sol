...
contract FractionalizedFacet is ReentrancyGuard, AdminControl, ERC1155, IERC721Receiver {
     ...
     * @dev Mint an avatar
     */
    function _mintFractionalizedFacet(address to, uint256 id) internal returns(uint256) {
        ERC1155._mint(to, id, NUM_FACETS, "");
        return 0;
    }
    ...
    function _onERC721Received(address from, uint256 id) private {
        ...
        // Burn it
        try IFacet(msg.sender).burn(address(this), id) {
        } catch (bytes memory) {
            revert("Burn failure");
        }

        _mintFractionalizedFacet(from, id);
    }


    function setCollectionURI(string calldata uri) external adminRequired {
        _setURI(uri);
    }
