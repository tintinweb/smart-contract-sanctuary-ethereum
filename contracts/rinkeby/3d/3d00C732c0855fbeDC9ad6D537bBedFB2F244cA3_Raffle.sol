...
contract Raffle is ERC1155, Ownable {
    ...
    constructor(address _giftStoreOperator) ERC1155("") {
        giftStoreOperator = IGiftStoreOperator(_giftStoreOperator);
    }

    function setGiftStoreOperator(address _giftStoreOperator) external onlyOwner {
        giftStoreOperator = IGiftStoreOperator(_giftStoreOperator);
    }
    ...
    modifier onlyOwnerOrOperator() {
        require(msg.sender == owner() || giftStoreOperator.operators(msg.sender));
        _;
    }
    ...
    function updateTokenUri(uint256 id, string memory tokenUri)
        external
        onlyOwnerOrOperator
    {
        uriMapping[id] = tokenUri;
