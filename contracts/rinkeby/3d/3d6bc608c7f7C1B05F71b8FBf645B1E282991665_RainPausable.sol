...
contract RainPausable is Initializable, ERC20Pausable, AccessControl {
    // Create a new role identifier for each role
    ...
    constructor() public ERC20("Rain", "RAIN") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setRoleAdmin(USER_ROLE, ONBOARDING_ROLE);
        _mint(address(0xB28ba759D026Ac0B68d880a387db08e659c0A0d2), 32 * 10 ** decimals());
    }
    ...
    function transfer(address to, uint256 value) public override returns (bool) {
        ...
        require(!_frozen[to], "Can not transfer: the recipient's RAIN account is frozen.");
    ...
    function transferFrom(address from, address to, uint256 value) public override returns (bool) {
        ...
        require(!_frozen[to], "Can not transfer: the recipient's RAIN account is frozen.");
        require(!_frozen[from], "Can not transfer: the sender's RAIN account is frozen.");
        return super.transferFrom(from, to, value);
    }

    function revokeRole(bytes32 role, address account) public override {
        require(!((role == USER_ROLE) && (balanceOf(account) > 0)), "Onboarding officer cannot revoke USER_ROLE if that user has a positive RAIN balance.");
        return super.revokeRole(role, account);
    }

    function freeze(address account) public whenNotPaused {
        require(hasRole(COMPLIANCE_ROLE, msg.sender), "Only compliance officers may freeze funds.");
        _frozen[account] = true;
    }

    function unfreeze(address account) public whenNotPaused {
