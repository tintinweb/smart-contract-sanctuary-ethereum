...
contract MetaSpliffsNFT is ERC721Enumerable, Ownable, ERC721Burnable{
    ...
    uint256 public HIDE_TO = 4420;
    string public sampleTokenURI;
    uint256 public presalePrice = 0.042 ether;
    uint256 public publicsalePrice = 0.08 ether;
    uint256 public MAX_PER_MINT = 4;
    uint256 public constant MAX_ELEMENTS = 4420;
    uint256 public constant PRESALE_MAX_SUPPLY = 420;
    address public constant creatorAddress =
        0xb7899C81C4Dd402c7c0E35275577a20068b5650B;
    ...
    constructor(string memory baseURI) ERC721("MetaSpliffsNFT", "MESP") {
        setBaseURI(baseURI);
        pause(false);
    }
    ...
    function mint(
        ...
        if (!publicsale && presale) {
            require(_isApproved, "You are not WL users");
            require(_count <= MAX_PER_MINT, "exceed max count per mint");
            require(total + _count <= PRESALE_MAX_SUPPLY, "Presale Max Limit");
            require(total <= PRESALE_MAX_SUPPLY, "Sale end");
            require(msg.value >= _count * presalePrice, "value below price");
