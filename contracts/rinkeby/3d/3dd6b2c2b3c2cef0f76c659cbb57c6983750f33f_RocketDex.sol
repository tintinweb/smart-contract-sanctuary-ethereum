    ...
    PausableUpgradeable
{
    ...
    // pool_address -> tokenId -> trade data.
    ...
    function createOrder(OrderParams memory _params)
        ...
        returns (uint256)
    {   
        require(_params.orderType == OrderType.TAKE_PROFIT || _params.orderType == OrderType.BUY_LIMIT, "1");
        require(msg.value == executionFee, "2");
        ...
        require(poolAddress != address(0), "3");
        ...
        ethForExecute[tokenId] = executionFee;
    ...
    function cancelOrder(uint256 _tokenId, address _poolAddress) public {
        Order memory order = orders[_poolAddress][_tokenId];

        require(order.token0 != address(0), "8");
        require(
        ...
        if (order.orderType == OrderType.BUY_LIMIT) {
            orders[_poolAddress][_tokenId].orderType = OrderType
                .BUY_LIMIT_CANCELLED;
        } else if (order.orderType == OrderType.TAKE_PROFIT) {
            orders[_poolAddress][_tokenId].orderType = OrderType
                .TAKE_PROFIT_CANCELLED;
        }
        uint256 reward = ethForExecute[_tokenId];
        ethForExecute[_tokenId] = 0;
        (uint256 transferedToken0, uint256 transferedToken1) = _closePosition(
            _tokenId,
            _poolAddress
        );
        emit CancelOrder(
    ...
    function executeOrder(uint256 _tokenId, address _poolAddress) public {
        (bool isClosable, , , uint256 quoteAmount, , ) = checkOrder(
            _tokenId,
            _poolAddress
        );
        Order memory order = orders[_poolAddress][_tokenId];
        require(order.token0 != address(0), "12");
        require(
            order.orderType != OrderType.BUY_LIMIT_FILLED &&
                order.orderType != OrderType.TAKE_PROFIT_FILLED,
            "13"
        ...
        require(isClosable, "15");
        require(
            quoteAmount >= order.recievedAmount,
            "16"
        );
        if (order.orderType == OrderType.BUY_LIMIT) {
            orders[_poolAddress][_tokenId].orderType = OrderType
    ...
    function checkOrder(uint256 _tokenId, address _pool)
        public
        view
        returns (
            bool isClosable,
    ...
    {
        (baseAmount, quoteAmount) = _getBaseTokenAmount(_tokenId, _pool);
        Order memory order = orders[_pool][_tokenId];
    ...
    function _getBaseTokenAmount(uint256 _tokenId, address _pool)
        private
        view
        returns (uint256 baseAmount, uint256 quoteAmount)
    {
        Order memory order = orders[_pool][_tokenId];
        (uint160 sqrtPriceCurrentX96, , , , , , ) = IUniswapV3Pool(_pool)
            .slot0();
    ...
    function _closePosition(uint256 _tokenId, address _poolAddress)
        internal
        returns (uint256 token0Amount, uint256 token1Amount)
    {
        ...
        uint256 balanceOfToken0Before = IERC20Upgradeable(order.token0)
            .balanceOf(address(this));
        uint256 balanceOfToken1Before = IERC20Upgradeable(order.token1)
            .balanceOf(address(this));

        _removePosition(_tokenId, order.liquidity);

        uint256 balanceOfToken0After = IERC20Upgradeable(order.token0)
            .balanceOf(address(this));
        uint256 balanceOfToken1After = IERC20Upgradeable(order.token1)
            .balanceOf(address(this));

        if (balanceOfToken0After > balanceOfToken0Before) {
            IERC20Upgradeable(order.token0).transfer(
                order.owner,
                balanceOfToken0After - balanceOfToken0Before
            );
        }
        if (balanceOfToken1After > balanceOfToken1Before) {
            IERC20Upgradeable(order.token1).transfer(
                order.owner,
    ...
    function _removePosition(uint256 _positionId, uint128 _liquidity)
        private
        returns (uint256 amount0, uint256 amount1)
    {
