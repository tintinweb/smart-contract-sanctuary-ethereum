...
contract MiniMen is ERC721Enumerable {
    ...
    uint256 public constant LEGENDARY_COUNT = 9;
    ...
    function ownerMint(address receiver, uint256 amount) external onlyOwner {
        if (totalSupply() >= MAX_TOKENS) revert OverSupplyLimit();

        if (totalSupply() + amount >= MAX_TOKENS) {
            _mintRandomIndex(receiver, MAX_TOKENS - totalSupply());
        } else {
    ...
    function claim(uint256 tokenId) external {
        ...
        if (tokenId >= LEGENDARY_COUNT) revert TokenNotClaimable();
