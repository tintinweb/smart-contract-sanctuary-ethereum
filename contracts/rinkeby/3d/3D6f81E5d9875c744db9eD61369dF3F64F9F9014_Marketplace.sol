...
import "./MarketplaceOrdersHolder.sol";
import "./MarketplaceDomain.sol";
import "./MarketplaceState.sol";

contract Marketplace is Ownable, ReentrancyGuard, MarketplaceDomain {
    ...
    MarketplaceOrdersHolder public ordersHolder;

    constructor(
        TransferProxy _transferProxy, ERC20TransferProxy _erc20TransferProxy, MarketplaceState _state,
        MarketplaceOrdersHolder _ordersHolder, address payable _beneficiary, address _buyerFeeSigner
    ) {
    ...
    function exchange(
        ...
        require(order.key.sellAsset.assetType != AssetType.ETH, "ETH is not supported on seller");
    ...
    function transferWithFees(Asset memory firstType, uint value, address from, address to, uint256 sellerFee, uint256 buyerFee, Asset memory secondType) internal {
        ...
        if (
            ...
            require(fees.length == recipients.length, "Fees error");
    ...
    function verifyOpenAndModifyOrderState(OrderKey memory key, uint selling, uint amount) internal {
        ...
        require(newCompleted <= selling, "not enough stock of order");
...
contract MarketplaceOrdersHolder {
    ...
    function add(MarketplaceDomain.Order calldata order) external {
    ...
    function exists(MarketplaceDomain.Order calldata order) external view returns (bool) {
        bytes32 key = prepareKey(order);
    ...
    function prepareKey(MarketplaceDomain.Order memory order) internal pure returns (bytes32) {
...
contract MarketplaceState is OwnableOperatorRole {
    
    // keccak256(OrderKey) => completed
    mapping(bytes32 => uint256) public completed;

    function getCompleted(MarketplaceDomain.OrderKey calldata key) view external returns (uint256) {
        return completed[getCompletedKey(key)];
    }

    function setCompleted(MarketplaceDomain.OrderKey calldata key, uint256 newCompleted) external onlyOperator {
        completed[getCompletedKey(key)] = newCompleted;
    }

    function getCompletedKey(MarketplaceDomain.OrderKey memory key) pure public returns (bytes32) {
