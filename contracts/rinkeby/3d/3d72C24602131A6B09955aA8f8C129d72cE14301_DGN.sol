...
contract DGN is ERC1155, Ownable {
    ...
    mapping(address => uint8) public tokenWhitelist;
    mapping(uint => string) public tokenURI;
    mapping(uint => TokenInformation) private token;
    mapping(address => uint8) public addressMinted;
    MintInformation public mintInformation;
    ...
    function mint(uint _id) public {
        ...
        require(_id == mintInformation.id, "Mint must be active for specified ID");
        require(token[_id].currentSupply + 1 <= token[_id].maxSupply, "Must not exceed max supply!");
        if (mintInformation.status == MintStatus.Whitelist) {
            require(tokenWhitelist[msg.sender] == _id, "Must be whitelisted for specified ID");
            tokenWhitelist[msg.sender] = 0;
        } else {
            require(mintInformation.status == MintStatus.Public, "Public mint must be active or you are not whitelisted.");
            require(addressMinted[msg.sender] < mintInformation.publicMintLimit, "Public mint limit reached");
            addressMinted[msg.sender]++;
        }
        token[_id].currentSupply++;
    ...
    function ownerBatchMint(uint _id, address _to, uint _amount) external onlyOwner {
        require(_amount + token[_id].currentSupply <= token[_id].maxSupply, "Must not exceed max supply of ID!" );
        token[_id].currentSupply += _amount;
    ...
    /*Set max supply for new token ID.
    ...
    /*Sets mint info of given token ID.
    Sets mint status (0: Paused, 1: Whitelist, 2: Public) for specified token ID.
    Sets public mint limit per address IF you want this token to be public mint.*/
    function _setMintInfo(uint _id, MintStatus _status, uint _publicMintLimit) external onlyOwner {
        mintInformation.id = _id;
        mintInformation.status = _status;
        mintInformation.publicMintLimit = _publicMintLimit;
    }

    //Sets mint status (0: Paused, 1: Whitelist, 2: Public).
    function _setMintStatus(MintStatus status) external onlyOwner {
        mintStatus = status;
    }

    //Sets contract status (0: Paused, 1: Active)
    function _setContractStatus(ContractStatus status) external onlyOwner {
    ...
    function getTokenCurrentSupply(uint _id) public view returns (uint) {
        return token[_id].currentSupply;
    }

    //Returns true/false if address is whitelisted for specified token ID.
    function getWhitelistedAddress (uint _id, address) public view returns (bool) {
        if (tokenWhitelist[msg.sender] == _id) {
        return true;
        } else {
            return false;
        }
