...
contract BADStaking is Ownable {
    ...
    event AddedStakeHolder(address _holderAddress, uint256 _time);
    event SafeguardUpdated(bool _newstatus, uint256 _time);

    IERC20 public BADToken;
    IBADStakingNFT public StakingNFT;
    address public rewardPool;
    address public staking;
    uint public dailyReward;
    uint public accBadPerShare;
    ...
    // Constructor will set the address of BAD Token and address of BAD Staking NFT
    constructor(address _BADToken, address _StakingNFT, address _staking, address _rewardPool, uint _dailyReward) Ownable() {
        BADToken = IERC20(_BADToken);
        StakingNFT = IBADStakingNFT(_StakingNFT);
        staking = _staking;
        rewardPool = _rewardPool;

        // Dec-31-2031
        lastRewardBlock = 10222222;
        setDailyReward(_dailyReward);
    }

    // 6500 blocks in average day --- decimals * BAD balance of rewardPool / blocks / 10000 * dailyReward (in hundredths of %) = rewardPerBlock
    function getRewardPerBlock() public view returns(uint) {
        return BADToken.balanceOf(rewardPool).mul(dailyReward).div(6500).div(10000);
    }

    // % of reward pool to be distributed each day --- in hundredths of % 10 == 0.1%
    function setDailyReward(uint _dailyReward) public onlyOwner {
        dailyReward = _dailyReward;
    }

    // Function that will get BAD balance of a certain NFT
    function getNFTBalance(uint _tokenId) public view returns(uint _amountStaked) {
        return NFTDetails[_tokenId]._BADDeposited;
    }

    // Function that will check if a BAD stake NFT is in circulation
    ...
    // Function that returns pending rewards for specific NFT
    function pendingRewards(uint _NFT) public view returns(uint) {
        NFT storage nft = NFTDetails[_NFT];

        uint256 _accBadPerShare = accBadPerShare;

        if (block.number > lastRewardBlock && totalStaked != 0) {
            uint256 blocksToReward = block.number.sub(lastRewardBlock);
            uint256 badReward = blocksToReward.mul(getRewardPerBlock());
            _accBadPerShare = _accBadPerShare.add(badReward.mul(1e18).div(totalStaked));
        }

        return nft._BADDeposited.mul(_accBadPerShare).div(1e18).sub(nft._rewardDebt);
    }

    // Get total rewards pending for all of user's BAD NFTs
    ...
    // Get total amount staked for all of user's BAD nfts
    ...
    // Function that updates BAD pool
    function updatePool() public {
        ...
        //Approve badReward here
        BADToken.transferFrom(rewardPool, address(this), badReward);

        accBadPerShare = accBadPerShare.add(badReward.mul(1e18).div(totalStaked));
        lastRewardBlock = block.number;

        emit PoolUpdated(blocksToReward, badReward, block.timestamp);
    }

    // Function that lets user stake BAD
    function stakeBAD(uint _amount) public {
        require(!safeguard,"safeguard is on");
        require(_amount > 0, "Can not stake 0 BAD");
        require(BADToken.balanceOf(_msgSender()) >= _amount, "Do not have enough BAD to stake");
        ...
        if(nft._BADDeposited > 0) {
            uint _pendingRewards = nft._BADDeposited.mul(accBadPerShare).div(1e18).sub(nft._rewardDebt);

            if(_pendingRewards > 0) {
                BADToken.transfer(_msgSender(), _pendingRewards);
                emit RewardsClaimed(_msgSender(), _pendingRewards, StakingNFT.nftTokenId(_msgSender()), block.timestamp);
            }
        }

        BADToken.transferFrom(_msgSender(), address(this), _amount);
        nft._BADDeposited = nft._BADDeposited.add(_amount);
        totalStaked = totalStaked.add(_amount);

        nft._rewardDebt = nft._BADDeposited.mul(accBadPerShare).div(1e18);

        emit StakeCompleted(_msgSender(), _amount, StakingNFT.nftTokenId(_msgSender()), nft._BADDeposited, block.timestamp);

    }

    function addStakeholder(address _stakeholder) private {
        require(!safeguard,"safeguard is on");
        (bool success, ) = staking.call(abi.encodeWithSignature("mint(address)", _stakeholder));
    ...
    function addStakeholderExternal(address _stakeholder) external onlyPlatform() {
        ...
        emit AddedStakeHolder(_stakeholder, block.timestamp);
    }
    ...
    function claimRewards(uint _tokenId) public {
        ...
        emit RewardsClaimed(_msgSender(), _pendingRewards, _tokenId, block.timestamp);
    }

    // Function that will add BAD rewards to BAD staking NFT
    function compoundRewards(uint _tokenId) public {
        ...
        nft._BADDeposited = nft._BADDeposited.add(_pendingRewards);
        totalStaked = totalStaked.add(_pendingRewards);

        nft._rewardDebt = nft._BADDeposited.mul(accBadPerShare).div(1e18);

        emit RewardsCompounded(_msgSender(), _pendingRewards, _tokenId, nft._BADDeposited, block.timestamp);
    }
    ...
    // Function that lets user unstake BAD in system. A 5% fee gets redistributed back to the Reward Pool
    function unstakeBAD(uint _tokenId) public {
        require(!safeguard,"safeguard is on");
        ...
        uint stakeAfterFees = amountStaked.mul(95).div(100);
        uint userReceives = amountStaked.mul(95).div(100).add(_pendingRewards);

        uint fee = amountStaked.mul(5).div(100);

        uint beingWithdrawn = nft._BADDeposited;
        nft._BADDeposited = 0;
        nft._inCirculation = false;
        totalStaked = totalStaked.sub(beingWithdrawn);
        StakingNFT.revertNftTokenId(_msgSender(), _tokenId);

        (bool success, ) = staking.call(abi.encodeWithSignature("burn(uint256)", _tokenId));
        require(success == true, "mint call failed");

        BADToken.transfer(_msgSender(), userReceives);
        BADToken.transfer(rewardPool, fee);

        emit WithdrawCompleted(_msgSender(), stakeAfterFees, _tokenId, block.timestamp);
        emit RewardsClaimed(_msgSender(), _pendingRewards, _tokenId, block.timestamp);
    }

    // Function that will unstake all of user's BAD and burn all NFTs for user
    ...
    function incrementNFTValue (uint _tokenId, uint _amount) external onlyPlatform() {
        ...
        if(nft._BADDeposited > 0) {
            ...
            if(_pendingRewards > 0) {
                BADToken.transfer(StakingNFT.ownerOf(_tokenId), _pendingRewards);
                emit RewardsClaimed(StakingNFT.ownerOf(_tokenId), _pendingRewards, _tokenId, block.timestamp);
            }
        }

        NFTDetails[_tokenId]._BADDeposited =  NFTDetails[_tokenId]._BADDeposited.add(_amount);
    ...
    function decrementNFTValue (uint _tokenId, uint _amount) external onlyPlatform() {
        ...
        NFTDetails[_tokenId]._BADDeposited =  NFTDetails[_tokenId]._BADDeposited.sub(_amount);
    ...
    function changeSafeguardStatus() onlyPlatform() public{
        ...
        emit SafeguardUpdated(safeguard,  block.timestamp);
    }
