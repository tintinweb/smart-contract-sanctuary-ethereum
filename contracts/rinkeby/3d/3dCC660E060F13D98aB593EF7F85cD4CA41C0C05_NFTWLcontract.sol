...
contract NFTWLcontract is Ownable, ERC721A, PaymentSplitter {
        ...
        PaymentSplitter(distro, distro_shares)
    {
        ...
        accounts[msg.sender] = Account( 0, [0,0,0], true);

        // teamclaim (5 ttl)
        accounts[teamclaim[0]] = Account( 25, [0,0,0], true); // Team_jd
        accounts[teamclaim[1]] = Account( 25, [0,0,0], true); // Team_wd
        accounts[teamclaim[2]] = Account( 25, [0,0,0], true); // Team_gn
        accounts[teamclaim[3]] = Account( 25, [0,0,0], true); // Team_gn
        accounts[teamclaim[4]] = Account( 65, [0,0,0], true); // KCat_ownr
        accounts[teamclaim[5]] = Account(  5, [0,0,0], true); // CB_ownr
        ...
        walletLimit[0] = 10; // Publicsale
        walletLimit[1] = 3; // Presale #1
        walletLimit[2] = 5; // Presale #2
    ...
    function setWalletLimit(uint index, uint _newLimit) external onlyOwner {
        //WALLETLIMIT = _newLimit;
        walletLimit[index] = _newLimit; 
    }
    ...
    function mint(uint8 _amount) external payable noReentrant {
        ...
        require((_amount + accounts[msg.sender].mintedNFTs[0]) <= walletLimit[0], "Error: You would exceed the wallet limit.");
        ...
        //     emit Mint(msg.sender, totalSupply());
        // }
	accounts[msg.sender].mintedNFTs[0] = accounts[msg.sender].mintedNFTs[0] + _amount;
        _safeMint(msg.sender, _amount);
    emit Mint(msg.sender, totalSupply());
    ...
    function allowlistMint(bytes32[] calldata _merkleProof, uint8 _amount, uint8 saleIndex) external payable noReentrant{
        require(_isPreSaleLive, "Error: Allowlist Sale is not active.");
        require(totalSupply() + _amount <= (MAXSUPPLY - _reserved), "Error: Purchase would exceed max supply.");
        //require((_amount + accounts[msg.sender].mintedNFTs) <= WALLETLIMIT, "Error: You would exceed the wallet limit.");
        require((_amount + accounts[msg.sender].mintedNFTs[saleIndex]) <= walletLimit[saleIndex], "Error: You would exceed the wallet limit.");
        require(!isContract(msg.sender), "Error: Contracts cannot mint.");
        require(msg.value >= (PRESALEPRICE * _amount), "Error: Not enough ether sent.");
        ...
        //bug increase mintedNFTs with amount
        
	accounts[msg.sender].mintedNFTs[saleIndex] = accounts[msg.sender].mintedNFTs[saleIndex] + _amount;
        _safeMint(msg.sender, _amount);
    emit Mint(msg.sender, totalSupply());
