...
contract EtherscentMain is Ownable, ERC721A, ReentrancyGuard, EtherscentAccordsDistribution, EtherscentDesc, EtherscentSVG {

    uint256 public currentPreparedFormulaIdx;

    struct TokenMintInfo {
        uint8 formulaType; // 1 not use, 2 use, 3 use special
        uint256 formulaIdx;
        uint256 prevBlockHash;
        AccordsRange accordsRange;
    }

    struct TokenNameAndLink {
        string tokenName;
        string link;
    }
    
    mapping(uint256 => TokenNameAndLink) private __tokenNameAndLink;
    ...
    function mint_prepared(uint256 quantity) public onlyMinter returns (uint256, uint256) {
        require(quantity + currentPreparedFormulaIdx <= _preparedFormulas.length);
        __tokenMintInfo[_currentIndex] = TokenMintInfo({
            formulaType: 2, 
            formulaIdx: currentPreparedFormulaIdx,
            prevBlockHash: uint256(blockhash(block.number - 1)),
            accordsRange: AccordsRange({
                topRange: 0,
                strongRange: 0,
        ...
        currentPreparedFormulaIdx += quantity;
        return _safeMint(minter, quantity);
    }
...
// #####################################
    ...
    function recordAccords(uint256[] memory _accords, string[] memory _accordsName) public onlyOwner {
        ...
        for (uint256 i; i < accords_len; ++i) {
            accordsNameMap[_accords[i]] = _accordsName[i];
        }
    }
    ...
    function addAccordsWithData(uint256 which, uint256[2][] memory _accordsWithData, string[] memory _accordsName) public onlyOwner {
        ...
        if (which == 0) {
            for (uint256 i; i < accordsWithData_len; ++i) {
                ...
                accordsNameMap[_accordsWithData[i][0]] = _accordsName[i];
            }
        } else if (which == 1) {
            for (uint256 i; i < accordsWithData_len; ++i) {
    ...
    function addPreparedFormulas(uint256[2][][] memory _formulas) public onlyOwner {
        uint256 formulas_len = _formulas.length;
        for (uint256 i; i < formulas_len; ++i) {
            _preparedFormulas.push(_formulas[i]);
        }
    }

    function addSpecialFormulas(uint256[2][][] memory _formulas, uint256[4][] memory _filters) public onlyOwner {
        uint256 formulas_len = _formulas.length;
        for (uint256 i; i < formulas_len; ++i) {
...
// #####################################
    ...
    function _setPersonalInfo(uint256 tokenId, string memory tokenName, string memory link) private {
        ...
        __tokenNameAndLink[tokenId] = TokenNameAndLink({tokenName: tokenName, link: link});
    }

    function setPersonalInfo(uint256 tokenId, string memory tokenName, string memory link) external {
        _setPersonalInfo(tokenId, tokenName, link);
    }
    ...
    function bunchSetPersonalInfo(uint256[] memory tokenIds, string[] memory tokenNames, string[] memory links) external {
        ...
        for (uint256 i; i < tokenIds.length; ++i) {
            _setPersonalInfo(tokenIds[i], tokenNames[i], links[i]);
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (tokenMintInfo.formulaType == 1) {
            ...
            return __tokenURI_prepared(tokenId, tokenMintInfo, offset);
        } else if (tokenMintInfo.formulaType == 3) {
            return __tokenURI_prepared(tokenId, tokenMintInfo, offset);
        } else {
            return "";
        }
    ...
    function __tokenURI_random(uint256 tokenId, TokenMintInfo memory tokenMintInfo) private view returns (string memory) {
        ...
        (string memory name, string memory desc, string memory propertyJson) = generateDesc(tokenId, __tokenNameAndLink[tokenId].tokenName, accords, dis);
        JSONGenerator.TokenUriParams memory tokenUriParams = JSONGenerator.TokenUriParams(name, desc, propertyJson, _generateSVG(seed, accords, dis));
        return JSONGenerator.generateUri(tokenUriParams);
    }

    function __tokenURI_prepared(uint256 tokenId, TokenMintInfo memory tokenMintInfo, uint256 offset) private view returns (string memory) {
        uint256 seed = uint256(keccak256(abi.encodePacked(tokenMintInfo.prevBlockHash, tokenId)));
        (uint256[] memory accords, uint256[] memory dis) = _accordsDistribution_prepared(tokenMintInfo.formulaIdx + offset);
        (string memory name, string memory desc, string memory propertyJson) = generateDesc(tokenId, __tokenNameAndLink[tokenId].tokenName, accords, dis);
        JSONGenerator.TokenUriParams memory tokenUriParams = JSONGenerator.TokenUriParams(name, desc, propertyJson, _generateSVG(seed, accords, dis));
        return JSONGenerator.generateUri(tokenUriParams);
    }
...
library PRFAndSort {
    ...
    function pseudorandom_hash(bytes memory seed, uint m) internal pure returns (uint r) {
        unchecked {
            r = uint(keccak256(abi.encodePacked(seed, "Etherscent Library")))%m;
        }
    }
...
contract EtherscentSVGFilters {
    ...
    uint256[4][] internal _specialFormulasFilter;

    function __filterHeader() private pure returns (string memory) {
        return "<filter id='f' color-interpolation-filters='sRGB'><feMerge result='text'><feMergeNode in='SourceGraphic'/></feMerge>";
    }

    function __filterFooter() private pure returns (string memory) {
        return "</filter>";
    }
    ...
    * sc 10 to 300
    */
    function __filterMainPart1(uint256 bf, uint256 xs, uint256 ys, uint256 sc) private pure returns (string memory) {
        bytes4 kws = "RGBA";
        return string(abi.encodePacked("<feTurbulence type='fractalNoise' baseFrequency='0.",
                bf.toString(),
    ...
    function __pickFilterParams(uint256 seed) private pure returns (uint256 bf, uint256 xs, uint256 ys, uint256 sc) {
        bf = PRFAndSort.pseudorandom_hash(seed >> 32, 10, 80);
        xs = PRFAndSort.pseudorandom_hash(seed >> 48, 4);
        ys = PRFAndSort.pseudorandom_hash(seed >> 64, 4);
        sc = PRFAndSort.pseudorandom_hash(seed >> 96, 10, 300);
    }
    
    function __generateFilters(uint256 bf, uint256 xs, uint256 ys, uint256 sc) private pure returns (string memory filters) {
        filters = string(abi.encodePacked(__filterHeader(), __filterMainPart1(bf, xs, ys, sc), __filterFooter()));
    }

    function _generateFilters(uint256 seed) internal pure returns (string memory filters) {
        (uint256 bf, uint256 xs, uint256 ys, uint256 sc) = __pickFilterParams(seed);
        filters = __generateFilters(bf, xs, ys, sc);
    }

    function _generateFilters_special(uint256 idx) internal view returns (string memory filters) {
        (uint256 bf, uint256 xs, uint256 ys, uint256 sc) = (_specialFormulasFilter[idx][0], _specialFormulasFilter[idx][1], _specialFormulasFilter[idx][2], _specialFormulasFilter[idx][3]);
        filters = __generateFilters(bf, xs, ys, sc);
    }
}
...
contract EtherscentSVGCSS {
    ...
    function __cssMainPart(uint256[] memory accords) private pure returns (string memory css_colors) {
        ...
        css_colors = string(abi.encodePacked(css_colors, ".bg{fill:none;}"));
    }
...
contract EtherscentSVG is EtherscentSVGFilters, EtherscentSVGCSS {
    ...
    function __getHeader() private pure returns (string memory header) {
        header = string(abi.encodePacked("<?xml version='1.0' encoding='utf-8'?><!-- Generator: Adobe Illustrator 25.0.1, SVG Export Plug-In . SVG Version: 6.00 Build 0)--><svg version='1.1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' x='0px' y='0px' viewBox='0 0 503 653' style='enable-background:new 0 0 503 653;' xml:space='preserve'>"));
    }

    function __getFooter() private pure returns (string memory footer) {
        footer = "</g><rect class='bg' width='503' height='653'/></g></svg>";
    }
    ...
    function __getTransmission() private pure returns (string memory transmission) {
        transmission = "<g filter='url(#f)'><g>";
    }
...
contract EtherscentDesc {
    ...
    mapping(uint256 => string) internal accordsNameMap;

    function generateDesc(uint256 tokenId, string memory tokenName, uint256[] memory accords, uint256[] memory dis) internal view  returns (string memory name, string memory desc, string memory propertyJson) {
        name = string(abi.encodePacked("Etherscent formula No.", tokenId.toString(), " - ", tokenName));
        desc = string(abi.encodePacked("This formula consists of ", (accords.length).toString(), " accords."));
        propertyJson = _generatePropertyJson(accords, dis);
    }

    function _generatePropertyJson(uint256[] memory accords, uint256[] memory dis) private view returns (string memory propertyJson) {
        uint256 len = accords.length;
        propertyJson = '"attributes":[';
        for (uint256 i; i < len; ++i) {
            propertyJson = string(abi.encodePacked(propertyJson, '{"trait_type":"', accordsNameMap[accords[i]], '","value":"', dis[i].toString(), '"}'));
...
contract EtherscentAccordsDistribution {
    ...
    */
    ...
    function _accordsDistribution_prepared(uint256 idx) internal view returns (uint256[] memory accords, uint256[] memory dis) {
        uint256[2][] memory accordsWithQuantity = _preparedFormulas[idx];
