...
contract WhitelistTest is ERC721A, Ownable {
    bytes32 public root;
    ...
    function setPause(bytes32 _root) external onlyOwner {
		root = _root;
	}
    ...
    function mint(uint count) external payable {
		require(!isPaused, 'Public mint has not started');
		require(count <= MAX_NFT_PER_TRAN,'Exceeds NFT per transaction limit');
		require(_totalMinted() + count <= maxSupply,'Exceeds max supply');
