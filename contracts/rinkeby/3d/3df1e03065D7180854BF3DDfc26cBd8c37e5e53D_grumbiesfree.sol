...
contract grumbiesfree is Ownable, ERC721A, ReentrancyGuard {
    ...
    function releasable(uint256 tokenId) public view returns (uint256) {
        ...
        return _pendingPayment(tokenId, totalReceived, released(tokenId));
    }
    ...
    function releasable(IERC20 token, uint256 tokenId) public view returns (uint256) {
        ...
        return _pendingPayment(tokenId, totalReceived, released(token, tokenId));
    }
    ...
    function release(uint256 tokenId) public virtual {
        ...
        // add requirement that token Id must be less than X
        ...
        emit PaymentReleased(ownerOf(tokenId), payment);
    }
    ...
    function release(IERC20 token, uint256 tokenId) public virtual {
        ...
        SafeERC20.safeTransfer(token, ownerOf(tokenId), payment);
        emit ERC20PaymentReleased(token, ownerOf(tokenId), payment);
    }
    ...
    function _pendingPayment(
        ...
        return (totalReceived * _shares[tokenId]) / _totalShares - alreadyReleased;
    }
}
