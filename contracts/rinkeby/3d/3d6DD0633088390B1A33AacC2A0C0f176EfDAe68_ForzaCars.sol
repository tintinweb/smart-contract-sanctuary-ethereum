...
contract ForzaCars is ERC1155, Ownable, ReentrancyGuard {
    ...
    event MysteryCar(uint256 carAmounts);
        ...
        nonReentrant
    {
        ...
        _mintMysteryCar(_msgSender(), amountToMint);
        if (refund > 0) {
        ...
        internal
    {
        for (uint256 i = 0; i < qty; i++) {
            uint256 id = (i % currentOtherCar) + OTHER_CAR_OFFSET; // random bientot
            _mint(to, id, 1, "");
        }
    }
