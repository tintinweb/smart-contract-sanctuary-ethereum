 ...
 */
    ...
    ) internal isNFTOwner(nftContractAddress, tokenId, msg.sender)
    {
        ...
        nftContractAuctionTimmings.launchTime = (block.timestamp + startTime);
        nftContractAuctionTimmings.deadlineTime = (block.timestamp + startTime + endTime);
        require (nftContractAuctionTimmings.deadlineTime > nftContractAuctionTimmings.launchTime, "End Time Must be after launchtime");
        AuctionState aucState;
        if(block.timestamp<nftContractAuctionTimmings.launchTime)
    ...
    onlyOwner
{
    ...
    function _getAuctionState(
        ...
        if(block.timestamp >= nftContractAuctionTimming[nftContractAddress][tokenId].deadlineTime) {
            ...
            if (block.timestamp >= nftContractAuctionTimming[nftContractAddress][tokenId].launchTime){
    ...
    function getStartTime(address nftContractAddress, uint256 tokenId) external view returns(uint){
        return nftContractAuctionTimming[nftContractAddress][tokenId].launchTime;
    }
    ...
    function getCurrentPrice(address nftContractAddress, uint256 tokenId) public view returns(uint){
        ...
        uint timeElapsed = uint(block.timestamp - nftContractAuctionTimming[nftContractAddress][tokenId].launchTime);
        uint decreaseAmount = nftContractAuctions[nftContractAddress][tokenId].decreaseRate*timeElapsed;
        return nftContractAuctionTimming[nftContractAddress][tokenId].launchTime - decreaseAmount;
    }
    ...
    function getCurrentAuctionTime(address nftContractAddress, uint256 tokenId) public view returns(uint){
        if (nftContractAuctions[nftContractAddress][tokenId].aucState == AuctionState.OPEN){
            ...
            if (nftContractAuctions[nftContractAddress][tokenId].aucState == AuctionState.UPCOMING){
                uint timetoStart = uint(nftContractAuctionTimming[nftContractAddress][tokenId].launchTime);
                return timetoStart;
            }else {
                revert NotFound();
            }
