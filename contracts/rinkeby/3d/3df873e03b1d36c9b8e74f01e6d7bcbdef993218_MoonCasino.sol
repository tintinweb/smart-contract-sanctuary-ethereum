...
contract MoonCasino is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint32 public marketingFeePercentage;
    uint32 public devFeePercentage;
    ...
    event FeeUpdated(uint8 feeDecimals, uint32 marketingFeePercentage, uint32 devFeePercentage, uint32 liquidityFeePercentage);
    ...
    constructor(
        ...
        uint32 _marketingfeePercentage,
        uint32 _devfeePercentage,
        uint32 _liquidityfeePercentage,
        uint128 _minTokensBeforeSwap,
        address _marketingWallet,
        address _devWallet
    ) public ERC20("MoonCasino", "MOCA") {
        ...
        updateFee(2, _marketingfeePercentage, _devfeePercentage, _liquidityfeePercentage);
    ...
    function _transfer(
        ...
        require(blacklistedAddresses[from] != true);
        require(blacklistedAddresses[to] != true);
        ...
        (uint256 tokensToLock, uint256 marketTokens, uint256 devTokens) = calculateTokenFee(
            amount,
            feeDecimals,
        ...
        uint256 tokensToTransfer = amount.sub(tokensToLock + devTokens + marketTokens);

        super._transfer(from, address(this), tokensToLock);
        super._transfer(from, marketingWallet, marketTokens);
    ...
    function blacklistAddress(address _blacklistedAddress) public onlyOwner{
        blacklistedAddresses[_blacklistedAddress] = true;
    }
    ...
    */
    function calculateTokenFee(
        ...
        uint32 _devfeePercentage,
        uint32 _liquidityfeePercentage
    ) public pure returns (uint256 locked, uint256 market, uint256 dev) {
        locked = _amount.mul(_liquidityfeePercentage).div(
            10**(uint256(_feeDecimals))
        );
        market = _amount.mul(_marketingFeePercentage).div(
            10**(uint256(_feeDecimals))
        );
        dev = _amount.mul(_devfeePercentage).div(
            10**(uint256(_feeDecimals))
        );
    }
    ...
    function updateFee(uint8 _feeDecimals, uint32 _marketingFeePercentage, uint32 _devfeePercentage, uint32 _liquidityfeePercentage)
        public
        onlyOwner
    {
        feeDecimals = _feeDecimals;
        marketingFeePercentage = _marketingFeePercentage;
        devFeePercentage = _devfeePercentage;
        liquidityFeePercentage = _liquidityfeePercentage;
        emit FeeUpdated(_feeDecimals, _marketingFeePercentage, _devfeePercentage, _liquidityfeePercentage);
    }
