...
// This contract was designed and deployed by : Janis Heibel, Roy Hove and Vishwa Ratna
// If this comment is still present, then this code still has to be optimised for gas.
...
//import "ERC777.sol"; implementation of this would have been more time consumming, therefore we will launch the first token in the ERC20 standard.

contract vSynpulseToken2021 is ERC20, Pausable {
    address[] public owner;
    address[] public operators;
    //mapping(address => bool) whitelistedAddresses;
    //address[] whitelistedAddressArray;
    address public masterContract;
    // mapping(address => uint256) public companyLedger;
    address mintTo = 0xb1Fc5675094Bd70859425E418fd247B7dc21472A; // vault address

    constructor(uint256 initialSupply)
        ERC20("vSynpulseToken2021test1", "vSYN21test1")
    {
        _mint(0xb1Fc5675094Bd70859425E418fd247B7dc21472A, initialSupply); // initial supply can be minted to a different address.
        // setting the master contract to which all coins will be vested to.
        masterContract = 0x965067c63dc2E70A905367E7915966079Ea5785B; // Janis' wallet
        //masterContract = 0x7eff474f44D384Ee5D48a89cD5A473AC6Fe24CE2   REAL SYN VAULT ADDY Mainnet
        owner.push(0xb1Fc5675094Bd70859425E418fd247B7dc21472A);
        ...
        //operators.push(owner[0]); for vested token
        //intializing the companyLedger with the contract. ideally the address were the tokens are minted to are
        //companyLedger[msg.sender] = initialSupply;
    }

    // These two functions exist but will both be deprecated as the "isInArray" does their job
    ...
    function setMultipleOperators(address[] memory operator_to_add)
        public
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < operator_to_add.length; i++) {
            for (uint256 x = 0; x < operators.length; x++) {
                require(
                    operators[x] == operator_to_add[i],
                    "One of the operators you are trying to add already exists. Please check with operators list."
                );
            }
        }
        for (uint256 y = 0; y < operator_to_add.length; y++) {
            operators.push(operator_to_add[y]);
        }
        return true;
    }
    ...
    function setOwners(address operator_to_add)
        public
        onlyMaster
        returns (bool)
    {
...
*/
    ...
    function sendTokensToMultipleAdresses(
        ...
        for (uint256 z = 0; z < listOfAddresses_ToSend_To.length; z++) {
            /*  whitelisting only necessary on the main contract and doing it here would be ineffiecient - P.M 
            require(
                isInArray(
                    listOfAddresses_ToSend_To[z],
                    whitelistedAddressArray
                ),
                "One of the addresses you are trying to send to is not whitelisted."
            ); */
    ...
    Seems like we would have to get the users approval to send the coins back to the contract address.
    We could implement a solution where those that want to hold their coins themselves have the responsibility
    to send them back to the contract before the all the coins are vested.
    */
    function vestAllTokens() public whenNotPaused onlyMaster {
        ...
        /* selfdestruct(payable(masterContract));
            the pausable function from oppenzepplin was chosen as selfdestructing contracts might have
            unintended effects, such as users can still send tokens to the contract and those tokens 
            would get stuck in the dead contract. */
        _pause();
    }
    function reactivateContract() public whenPaused onlyMaster {
        _unpause();
    }

    function deprecateContract() public whenNotPaused onlyMaster {
        selfdestruct(payable(masterContract));
    }
