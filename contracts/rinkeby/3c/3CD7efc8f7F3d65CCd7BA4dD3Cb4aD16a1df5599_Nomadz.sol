...
contract Nomadz is ERC721A, Pausable, Ownable {

    /*************************
     MAPPING STRUCTS EVENTS
     *************************/

    // mapping from whitelist address to claimed status
    mapping(address => bool) public whitelistClaim;

    // struct for efficiently packing time stamp data for public sale, whitelist
    struct SaleConfig {
        uint16 maxMint;
        uint16 maxSupply;
    ...
    // stores whitelist and sale timestamps
    SaleConfig private saleConfig;

    // merkle tree root for whitelist claim
    ...
    constructor() ERC721A("Nomadz", "NMDZ") {

        // Pre-reveal URI
        setBaseURI("http://OurFutureURL.com/");

        // pause contract during deployment
        pause();

        // Set the timestamps for whitelist and public sales; max entries and max mint
        setSaleConfig(1684778100, 8888, 5);

        // Set price to 0.085 ETH
        _price = 0.0085 ether;
    ...
    */
    function isPublicSaleOn() public view returns (bool) {
        if (block.timestamp >= saleConfig.whitelistEnd) {
            return !_salePaused;
        } else {
            return false;
        }
     ...
     * @param to recipient address of the NFT minted
     * @param amount number of NFTs minted
     */
    function mintNomadz(address to, uint256 amount)
        public
        payable
        whenNotPaused
        callerIsUser
    {
        // require users aren't minting more than maxMint Nomadz
        require(amount <= saleConfig.maxMint, "You can mint a maximum of 5 Nomadz at a time!");

        // require that users aren't minting more than max supply
        require(totalSupply() + amount < saleConfig.maxSupply, "There can only be 8888 Nomadz!");

        // require that user's sent along the correct amount of ETH
        require(msg.value == _price * amount, "Ether amount sent is not correct!");
    ...
    /**
     * @dev Public function for redeeming whitelist tokens
     * Calls _safeMint() for minting process
     * @param amount number of NFTs minted
     * @param merkleProof proof set for claims
     */
    function whitelistMint(uint amount, bytes32[] calldata merkleProof)
        public
        payable
        whenNotPaused
        callerIsUser
    {
        ...
        // require user is minting 5 or less Nomadz
        require(amount < 6, "Can't mint more than 5 Nomadz from the whitelist!");

        // require that user hasn't already claimed these items
        require(!whitelistClaim[user], "Already claimed your whitelist spot!");

        // require that user's sent along the correct amount of ETH
        require(msg.value == _price * amount, "Ether amount sent is not correct!");

        // require that whitelist mint has started
        require(saleConfig.whitelistStart <= block.timestamp, "The whitelist hasn't started!");

        // require that whitelist mint has not ended
        require(saleConfig.whitelistEnd >= block.timestamp, "The whitelist is closed!");

        // require that users aren't minting more than max supply
        require(totalSupply() + amount < saleConfig.maxSupply, "There can only be 8888 Nomadz!");

        // set user's claim status
        whitelistClaim[user] = true;
    ...
    /**
    * @dev Function for only owner to set sale price
    * @param newPrice the amount of ETH required to mint Nomadz
    */
    ...
    /**
    * @dev Function for only owner to pause the contract
    */
    ...
    /**
    * @dev Function for only owner to unpause the contract
    */
    ...
    */
    function setSaleConfig(uint64 startTime, uint16 newMaxSupply, uint16 newMaxMint)
        public
        onlyOwner {
        saleConfig.whitelistStart = startTime;
        saleConfig.whitelistEnd = startTime + 24 hours;
        saleConfig.maxSupply = newMaxSupply;
        saleConfig.maxMint = newMaxMint;
    }

    /**
    * @dev Function for only owner to mint team Nomadz.
    * @param to the address where Nomadz are minted
    * @param amount the number of Nomadz to mint
    */
    function teamNomadz(address to, uint amount) public onlyOwner {
        _safeMint(to, amount);
    }
    ...
    */
    ...
    /**
    * @dev Function for setting the merkle root for the Nomadz
