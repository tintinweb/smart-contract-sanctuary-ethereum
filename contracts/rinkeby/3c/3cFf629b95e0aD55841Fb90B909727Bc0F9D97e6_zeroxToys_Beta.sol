...
contract zeroxToys_Beta is ERC721A, Ownable, ReentrancyGuard {
  ...
  function mint(uint256 quantity) external payable noContracts {
    ...
    require(quantity < maxTokenAmount, "Max token limit");
    require(_addressesMintedCount[msg.sender] < maxTokenAmount, "Max amount minted");
  ...
  function setupSaleConfig(uint64 priceWei, uint32 saleStartTime, uint32 maxSupply, uint32 maxTokenAmount) external onlyOwner {
