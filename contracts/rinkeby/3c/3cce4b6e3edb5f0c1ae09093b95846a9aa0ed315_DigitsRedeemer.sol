...
contract DigitsRedeemer is IDigitsRedeemer, Ownable {
    ...
    function stake(uint[] calldata tokenIds, uint timePeriod) external {
        require(timePeriodOptions[timePeriod], "Invalid Time Period!");
        require(addressToStaked[msg.sender].endTime <= block.timestamp + (timePeriod * 86400), "New time period must be equal or greater!");
        ...
        addressToStaked[msg.sender].timePeriod = timePeriod;
        addressToStaked[msg.sender].endTime = block.timestamp + (timePeriod * 86400);
    ...
    function _unstake(address unstaker) internal {
        ...
        addressToStaked[unstaker].timePeriod = 0;
