...
// File contracts/ILayerZeroReceiver.sol
...
// File contracts/ILayerZeroUserApplicationConfig.sol
...
// File contracts/ILayerZeroEndpoint.sol
...
abstract contract NonblockingReceiver is Ownable, ILayerZeroReceiver {
    ...
    // abstract function
    function _LzReceive( bytes memory _payload) virtual internal;
...
// File contracts/TestContractd.sol
...
//                       _  __                _            __ _   
//                      (_)/ _|              | |          / _| |  
//  ___  _ __ ___  _ __  _| |_ ___   ___   __| |___ _ __ | |_| |_ 
// / _ \| '_ ` _ \| '_ \| |  _/ _ \ / _ \ / _` / __| '_ \|  _| __|
//| (_) | | | | | | | | | | || (_) | (_) | (_| \__ | | | | | | |_ 
// \___/|_| |_| |_|_| |_|_|_| \___/ \___/ \__,_|___|_| |_|_|  \__|
                                                                
contract TestContractd is
    ERC721,
    NonblockingReceiver,
    ILayerZeroUserApplicationConfig
{
    ...
    function publicSaleState(bool _value) external onlyOwner {
        publicSaleIsActive = _value;
    }

    // starts or pauses the presale
    function presaleState(bool _value) external onlyOwner {
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        ...
        require(nextTokenId + _mintAmount <= maxMint, 'Max supply exceeded!');
        _;
    }

    //ekstra ekledim tartış
    ...
    function mint(uint8 numTokens) external payable {
        ...
        require(publicSaleIsActive == true, "Sales is not started");

        require(numTokens <= maxMintAmountPerTx, "Max maxMintAmountPerTx NFT per transaction");
        require(minted[msg.sender] + numTokens <= maxMintPerWallet, "Limit per wallet reached");
        require(nextTokenId + numTokens <= maxMint, "Mint exceeds supply");

        _safeMint(msg.sender, ++nextTokenId);
        if (numTokens == maxMintAmountPerTx) {
            _safeMint(msg.sender, ++nextTokenId);
        }
        minted[msg.sender] += numTokens;
    }

    function foodsOfOwner(address _owner) external view returns(uint256[] memory ) {
        uint256 foodCount = balanceOf(_owner);
        if (foodCount == 0) {
            return new uint256[](0);
        } else {
            uint256[] memory result = new uint256[](foodCount);
            uint256 index = 0;
            for (uint256 tokenId = 1; tokenId <= nextTokenId; tokenId++) {
                if (index == foodCount) break;
    ...
    function traverseChains(
        ...
        require(msg.sender == ownerOf(_tokenId), "Message sender must own the OmniFood.");
    ...
    function mintTokensPresale(uint numberOfTokens) external payable {
        ...
        require(nextTokenId + numberOfTokens <= maxMint, "Minting would exceed maxMint");
