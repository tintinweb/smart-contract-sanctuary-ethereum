...
contract Cryptoliga is ERC2981, ERC721, Pausable, AccessControl, CryptoligaFX {
    ...
    error OverMaxBatch(uint requested, uint max);
     ...
     */
    function wlAllowancesLeft(address _to) internal view returns (uint256) {
        if (maxBatchSize == 0) {
    ...
    {
        if (saleStage == Stages.CLOSED || minter == address(0)) {
            return (0, 0, 0, totalSupply(), maxSupply);
        }
    ...
    ) external payable whenNotPaused atLeastStage(Stages.PRIVATE) {
    ...
    function okToMint(address _to, uint256 _numberToMint) internal view {
        if (maxBatchSize > 0 && _numberToMint > maxBatchSize) {
            revert OverMaxBatch(_numberToMint, maxBatchSize);
        }
    ...
    ) external onlyStage(Stages.PUBLIC) {
        okToMint(to, quantity);
        _fxPurchase(quantity, coinSymbol);
        _safeMint(to, quantity);
    }

    function withdrawCoin(string memory _symbol) external onlyRole(DEFAULT_ADMIN_ROLE) {
 ...
 *
 * @author Peter Smith
 * @dev CryptoligaFX is a smart contract extension that allows
 * users to purchase tokens in any ERC20 coin.
 **/
