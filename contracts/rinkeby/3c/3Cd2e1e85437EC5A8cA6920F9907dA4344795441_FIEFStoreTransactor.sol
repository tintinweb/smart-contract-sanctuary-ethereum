...
contract FIEFStoreTransactor is AccessControlEnumerable, Signable {
    ...
    function buyTokens(BuyLib.Buy calldata buy) external {
        ...
        uint256 startTokenId = buy.startTokenId;
        uint256 total = startTokenId + buy.quantity;
        for (uint256 tokenId = startTokenId; tokenId < total; tokenId++) {
            require(!tokenIdSold[tokenId], "token_id_already_sold");
            tokenIdSold[tokenId] = true;
            IERC721(nft).transferFrom(multisig, msg.sender, tokenId);
        }
