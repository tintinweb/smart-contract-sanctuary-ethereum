...
contract MysteryNovelNFT is ERC721AQueryable, Ownable, MessageSigning, PaymentSplitter, ERC2981 {
    ...
    constructor(string memory name_, 
                string memory symbol_) ERC721A(name_, symbol_) MessageSigning(0x89c414614a488B929D081367B1B9a31bB1E04369)  PaymentSplitter(addressList, shareList) {
    ...
    */
    function publicMint(uint32 amountToMint) external payable{
        ...
        require(_numberMinted(msg.sender) - _getAux(msg.sender) + amountToMint <= maxPublicmint , "max NFTs per address exceeded");
        
        _mintNFT(amountToMint);        
    }
     

    function publicMint(uint32 amountToMint, bytes calldata mintPass) external payable{
        require (!presale, "Presale on");
        require(_numberMinted(msg.sender) - _getAux(msg.sender) + amountToMint <= maxPublicmint , "max NFTs per address exceeded");
    ...
    */
    ...
    function getNumberMinter(address wallet, bool inPresale) external view returns(uint256) {
        if (inPresale){
            return _getAux(wallet);
        } else {
            return _numberMinted(wallet) - _getAux(msg.sender);
        }
    }
