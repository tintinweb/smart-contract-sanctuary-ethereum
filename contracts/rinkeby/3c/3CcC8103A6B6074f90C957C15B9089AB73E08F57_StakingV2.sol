...
contract StakingV2 is Ownable {
    ...
    address public stakeToken; // Uniswap LP token from pool RELIC/USDT
    address public rewardToken; // RELIC token
    ...
    constructor(
        ...
        require(stakeToken_ != address(0), "RELICStaking::constructor: stake token address is 0x0");
        stakeToken = stakeToken_;

        require(rewardToken_ != address(0), "RELICStaking::constructor: reward token address is 0x0");
    ...
    function addPool(uint rewardAmount_, uint startTime_, uint endTime_, uint freezeTime_, uint freezePercent_) public onlyOwner {
        require(getTimeStamp() <= startTime_, "RELICStaking::addPool: bad timing for the request");
        require(startTime_ < endTime_, "RELICStaking::addPool: endTime > startTime");
    ...
    function stake(uint pid, uint amount) public returns (bool) {
        require(amount > 0, "RELICStaking::stake: amount must be positive");

        uint timeStamp = getTimeStamp();
        require(timeStamp < pools[pid].endTime, "RELICStaking::stake: bad timing for the request");
    ...
    function withdraw(uint pid) public returns (bool) {
        require(claim(pid), "RELICStaking::withdraw: claim error");
    ...
    function withdrawInternal(uint pid, address staker, uint amount) internal returns (bool) {
        require(amount > 0, "RELICStaking::withdrawInternal: amount must be positive");
        require(amount <= stakes[pid][msg.sender].amount, "RELICStaking::withdrawInternal: not enough balance");
    ...
    function claim(uint pid) public returns (bool) {
        require(getTimeStamp() > pools[pid].endTime, "RELICStaking::claim: bad timing for the request");
