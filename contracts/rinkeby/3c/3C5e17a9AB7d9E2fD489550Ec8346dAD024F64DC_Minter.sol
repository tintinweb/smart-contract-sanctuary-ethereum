...
contract Minter is Ownable {
    using ExtendSafeERC20 for IEURB;
    ...
    function claimById(bytes memory id) public {
        require(msg.sender == accounts[id], "Different account");
        uint256 lockTime = IController(controllerAddress).lockTime();
    ...
    function borrow(address uAssetAddress, uint256 uAssetAmount, uint256 collateralAmount, bytes memory id) external {
        ...
        IEURB(collateralAddress).safeTransferFrom(msg.sender, address(this), collateralAmount);
        IEURB(uAssetAddress).safeMint(msg.sender, uAssetAmount);
    ...
    function editBorrow(address uAssetAddress, uint256 uAssetAmount, uint256 collateralAmount, bytes memory id) public {
        require(msg.sender == accounts[id]);
        require(collateralBalances[id] > 0);
        require(typeBorrow[id] == 1);
        address collateralAddress = IController(controllerAddress).collateralForToken(uAssetAddress);
    ...
    function close(address uAssetAddress, bytes memory id) external {
        require(msg.sender == accounts[id]);
        require(collateralBalances[id] > 0);
        ...
        if (uAssetAmount > 0) {
            ...
            IEURB(uAssetAddress).safeBurn(uAssetAmount);
        }
    ...
    function short(address uAssetAddress, uint256 uAssetAmount, uint256 collateralAmount,uint256 deadline, uint16 slippage, bytes memory id) external {
        if(accounts[id] == address(0)) accounts[id] = msg.sender;
        ...
        {
            IEURB(collateralAddress).safeTransferFrom(msg.sender, address(this), collateralAmount);
            IEURB(uAssetAddress).safeMint(address(this), uAssetAmount);
            borrowBalances[id] = uAssetAmount;
    ...
    function editShort(address uAssetAddress, uint256 uAssetAmount, uint256 collateralAmount, uint256 deadline, uint16 slippage, bytes memory id) external {
        {
            require(msg.sender == accounts[id]); 
            require(collateralBalances[id] > 0);
            require(typeBorrow[id] == 2);
        ...
        if(block.timestamp - updatedTime > ttl) {
            ...
            if(uAssetAmount < borrowBalances[id]) {
                ...
                IEURB(addr).safeBurn(diff);
            } else if (uAssetAmount > borrowBalances[id]) {
                uint256 diff = uAssetAmount - borrowBalances[id];
                address addr = uAssetAddress;
                uint256 deadline_ = deadline;
                {
                    IEURB(addr).safeMint(address(this), diff);
                }
                address[] memory path = new address[](2);
                uint[] memory reserve = new uint[](2);
                {
    ...
    function liquidation(address userAddress, address uAssetAddress, uint256 uAssetAmount, bytes memory id) external {
        ...
        if (discountedCollateralValue <= collateralBalances[id]) {
            ...
            // uint256 remainedAmount = uAssetAmount - refundUAssetAmount;
            {
                ...
                IEURB(uAssetAddress).safeBurn(uAssetNeeded);
