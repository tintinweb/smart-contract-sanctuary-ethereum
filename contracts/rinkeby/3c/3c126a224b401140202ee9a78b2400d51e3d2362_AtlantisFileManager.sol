...
contract AtlantisFileManager {
    ...
    mapping(uint256 => address[]) access;
    ...
    // @dev this is a method to check if an account has access to a file
    function hasAccess(uint256 tokenId, address account) internal view returns(bool){
        for (uint i; i < access[tokenId].length; i++) {
            if(account == access[tokenId][i]){
                return true;
            }
        }
        return false;
    ...
    // @dev this is a method to return the index of an account in a token access array
    function getIndex(uint256 tokenId, address account) internal view returns(uint256){
        for (uint i; i < access[tokenId].length; i++) {
            if(account == access[tokenId][i]){
                return i;
            }
        }
        require(false, "account does not have access to token");
    }
    ...
    function uploadFile(string memory _name, string memory _url, string memory _description) external returns (uint256) {
        ...
        access[tokenId].push(msg.sender); // give the owner access to the file
        return tokenId;
    }
    ...
    function grantAccess(uint256 _tokenId, address _account) external {
        ...
        require(hasAccess(_tokenId, msg.sender) == true, "you must have access to the file to grant access");

        // grant the account in the parameter access to the file
        access[_file.tokenId].push(_account);
    }
    ...
    function revokeAccess(uint256 _tokenId, address _account) external {
        ...
        uint256 index = getIndex(_tokenId, _account);
        access[_tokenId][index] = access[_tokenId][access[_tokenId].length-1];
        access[_tokenId].pop();
    }
    ...
    function getFileData(uint256 _tokenId) external view returns(uint256 _id, string memory _name, address _owner, string memory _url, string memory _description, string memory _access_level){
        ...
        require(hasAccess(_tokenId, msg.sender) == true, "you must have access to the file to get the file");
