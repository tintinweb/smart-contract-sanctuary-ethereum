...
contract Vault is ReentrancyGuard, IVault {
    ...
    uint256 private constant FUNDING_RATE_PRECISION = 1000000;
    uint256 private constant PRICE_PRECISION = 10 ** 30;
    uint256 private constant MIN_LEVERAGE = 10000; // 1x
    uint256 private constant USDF_DECIMALS = 18;
    uint256 private constant MAX_FEE_BASIS_POINTS = 500; // 5%
    uint256 private constant MAX_LIQUIDATION_FEE_USD = 100 * PRICE_PRECISION; // 100 USD
    uint256 private constant MIN_FUNDING_RATE_INTERVAL = 1 hours;
    uint256 private constant MAX_FUNDING_RATE_FACTOR = 10000; // 1%
    ...
    struct FeeInfo {
        uint256 taxBasisPoints; // Address of LP token contract.
        uint256 stableTaxBasisPoints; // How many allocation points assigned to this pool. FATEs to distribute per block.
        uint256 mintBurnFeeBasisPoints; // Last block number that FATEs distribution occurs.
        uint256 swapFeeBasisPoints; // Accumulated FATEs per share, times 1e12. See below.
        uint256 stableSwapFeeBasisPoints; // Accumulated locked FATEs per share, times 1e12. See below.
        uint256 marginFeeBasisPoints; // Accumulated locked FATEs per share, times 1e12. See below.
    ...
    bool private includeAmmPrice = true;
    bool private useSwapPricing = false;
    ...
    constructor(address _superAdmin, address _admin) public {
        gov = msg.sender;
    ...
    function confirmTransaction(uint feeInfoId)
    public
    feeInfoExists(feeInfoId)
    {
        _onlyAdmin();
        require(isSuperAdmin[msg.sender], "Only super admin can confirm the transaction");
        feeInfo[feeInfoId].executed = true;
    }

    function executeTransaction(uint feeInfoId)
    public
    feeInfoExists(feeInfoId)
    {
        _onlyAdmin();
        require(feeInfo[feeInfoId].executed, "Executed transaction");
        taxBasisPoints = feeInfo[feeInfoId].taxBasisPoints;
        stableTaxBasisPoints = feeInfo[feeInfoId].stableTaxBasisPoints;
        mintBurnFeeBasisPoints = feeInfo[feeInfoId].mintBurnFeeBasisPoints;
        swapFeeBasisPoints = feeInfo[feeInfoId].swapFeeBasisPoints;
        stableSwapFeeBasisPoints = feeInfo[feeInfoId].stableSwapFeeBasisPoints;
        marginFeeBasisPoints = feeInfo[feeInfoId].marginFeeBasisPoints;
        liquidationFeeUsd = feeInfo[feeInfoId].liquidationFeeUsd;
    }
...
//    }
    ...
    function _reduceCollateral(address _account, address _collateralToken, address _indexToken, uint256 _collateralDelta, uint256 _sizeDelta, bool _isLong) private returns (uint256, uint256) {
        ...
        if (hasProfit && adjustedDelta > 0) {
            ...
            position.realisedPnl = uint256(position.realisedPnl).add(uint256(adjustedDelta));
        ...
        if (!hasProfit && adjustedDelta > 0) {
            ...
            position.realisedPnl = position.realisedPnl.sub(uint256(adjustedDelta));
        }
    ...
    function _onlyAdmin() private view {
        _validate(isAdmin[msg.sender], 53);
    }
    ...
    modifier feeInfoExists(uint feeInfoId) {
        require(feeInfo[feeInfoId].swapFeeBasisPoints!= 0);
        _;
    }
}
...
//    function setError(uint256 _errorCode, string calldata _error) external;
    function feeInfo(uint _pid) external virtual view returns (
