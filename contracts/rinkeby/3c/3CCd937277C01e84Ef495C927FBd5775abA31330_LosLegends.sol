...
contract LosLegends is
    ...
    UUPSUpgradeable
{
    using LosLegendsStorage for LosLegendsStorage.Layout;
    ...
    function initialize(
        ...
        __LosLegends_init(admin, quantumTreasury, authorizer);
    }

    function __LosLegends_init(
        address admin,
        address payable quantumTreasury,
        address authorizer
    ) internal onlyInitializing {
        __ERC721Q_init("LosLegends", "QSPACE");
        __Ownable_init();
        __Manageable_init();
        __Pausable_init();
        __UUPSUpgradeable_init();
        __LosLegends_init_unchained(admin, quantumTreasury, authorizer);
    }

    function __LosLegends_init_unchained(
        address admin,
        address payable quantumTreasury,
        address authorizer
    ) internal onlyInitializing {
        LosLegendsStorage.Layout storage qs = LosLegendsStorage.layout();
    ...
    {
        LosLegendsStorage.layout().authorizer = authorizer;
    }
    ...
    function setTreasury(address payable treasury)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        LosLegendsStorage.layout().quantumTreasury = treasury;
    }
    ...
    function setIpfsURI(string calldata ipfsURI)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        LosLegendsStorage.layout().ipfsURI = ipfsURI;
    }
    ...
    {
        LosLegendsStorage.layout().dropCID[dropId] = cid;
    }
    ...
    function unpauseDrop(uint128 dropId) public onlyRole(MANAGER_ROLE) {
        LosLegendsStorage.layout().isDropPaused.unset(dropId);
    }
    ...
    function setDrop(
        ...
        LosLegendsStorage.layout().dropMaxSupply[dropId] = maxSupply;
        LosLegendsStorage.layout().dropNumOfVariants[dropId] = numOfVariants;
    }
...
library LosLegendsStorage {
    ...
    bytes32 internal constant STORAGE_SLOT =
        keccak256("quantum.contracts.storage.loslegends.v1");
