...
interface IFlowers is IERC20 {
    ...
    function getPendingStakeRewards(uint256 tokenId) external returns (uint pending);
}
...
contract StakedGoats is Delegated, ERC721EnumerableLite {
    ...
    event Migrated(uint256 ggToken, address owner, uint256 sggToken, uint256 flowersMinted);
    ...
    function migrate(uint[] memory tokenIds_, address[] memory addresses_) public onlyOwner {
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            ...
            _staker.stakeCount++;
            uint _totalClaim = IFlowers(contractFlowers).getPendingStakeRewards(tokenIds_[i]);
            ...
            emit Migrated(tokenIds_[i], _user, _mintId, _totalClaim);
        }
