...
contract DeployPayment is ReentrancyGuardUpgradeable, NFTChecker, OwnableUpgradeable {
    uint256 public lastedId;
    uint256 public fee;
    uint256 public deployFund;
    ...
    function initialize(uint256 _fee, uint256 _deployFund, address _owner, address _deployer) external initializer {
        require(_fee > 0, "Invalid fee");
        require(_deployFund > 0, "Invalid deployFund");
        require(_owner != address(0), "Invalid owner address");
        require(_deployer != address(0), "Invalid deployer address");
        fee = _fee;
        deployFund = _deployFund;
    ...
    modifier onlyControllerOrDeployer() {
        require(controllers[_msgSender()] || _msgSender() == deployer, "Caller is not the controller or deployer");
        _;
    }
    ...
    event SetDeployer(address indexed oldDeployer, address indexed newDeployer);
    event SetDeployFund(uint256 indexed oldDeployFund, uint256 indexed newDeployFund);
    ...
    function deposit(bool _isSingle, string memory _name, string memory _symbol) external payable {
        ...
        uint256 _balanceDeployer = address(deployer).balance;
        if (_balanceDeployer < deployFund) {
            Helper.safeTransferNative(deployer, msg.value);
        }
    ...
    function setTokenDeployedTo(uint256 _payId, address _token) external onlyControllerOrDeployer {
    ...
    function cancelPayment(uint256 _payId) external onlyControllerOrDeployer {
        require(_payId > 0, "Invalid pay id");
        Pay storage pay = pays[_payId];
        require(pay.status == Status.PAID, "Unpaid user");
        pay.status = Status.CANCELED;
        Helper.safeTransferNative(pay.account, pay.fee);
    ...
    function setDeployer(address _newAccount) external onlyOwner {
        require(_newAccount != address(0), "Invalid account");
        address oldDeployer = deployer;
        deployer = _newAccount;
        emit SetDeployer(oldDeployer, _newAccount);
    }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        Helper.safeTransferNative(fundReceiver, _balance);
        emit Withdraw(fundReceiver, _balance);
    }

    function setFee(uint256 _fee) external onlyOwnerOrController {
        require(_fee > 0, "Invalid fee");
        uint256 oldFee = fee;
        fee = _fee;
    ...
    function setDeployFund(uint256 _newDeployFund) external onlyOwner {
        require(_newDeployFund > 0, "Invalid deployFund");
        uint256 oldDeployFund = deployFund;
        deployFund = _newDeployFund;
        emit SetDeployFund(oldDeployFund, _newDeployFund);
    }
}
