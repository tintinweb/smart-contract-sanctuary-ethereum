...
contract ARHOTVERSE is ERC721A, Ownable {
    ...
    uint256 public priceWL = 70000000000000000;
    ...
    address public constant wlSigner = 0xcd6219BcB9322F5A65D11b3f49cAe646ff192a59;
    
    event LogRSV(string logmsg, address sender, bytes32 r, bytes32 s, uint8 v);
    event LogSigner(string logmsg, address sender, address signer);
    ...
    function whitelistMint(uint256 quantity, bytes32 r, bytes32 s, uint8 v) external payable {
        // emit LogRSV("testing", msg.sender, r, s, v);
        
        require(isWhitelistSale, "Whitelist sale not start yet");
        require(totalMinted()+quantity <= maxSupply, "Exceed max supply");
    
        require(digestSign(r, s, v) == wlSigner, "Incorrect signer");
    ...
    function digestSign(bytes32 r, bytes32 s, uint8 v) internal returns (address){
        // emit LogRSV("testing", msg.sender, r, s, v);
        
        bytes32 digest = keccak256(abi.encodePacked(msg.sender));
        address signer = ecrecover(digest, v, r, s);

        emit LogSigner("testing", msg.sender, signer);
