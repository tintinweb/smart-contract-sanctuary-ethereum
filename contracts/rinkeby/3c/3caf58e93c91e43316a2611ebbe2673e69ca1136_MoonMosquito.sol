...
contract MoonMosquito is ERC2981, ERC721AQueryable, Ownable {
    ...
    string public _metadataURI = "https://gateway.pinata.cloud/ipfs/QmdbU8vN2pk7yjAuGZT4ajWTLXdPgJd1e6FvYNUndCcJRj/";
    ...
    constructor(
        uint256 price,  //  单价
        uint32 maxSupply, // 最大数量
        uint32 teamSupply, // 团队数量
        uint32 instantFreeSupply, // 及时免费总额
        uint32 randomFreeSupply, // 随机免费总额
        uint32 instantFreeWalletLimit, // 当前钱包免费数量
        uint32 maxMintAmount, // 最大mint数量
        uint32 walletLimit // 钱包最大拥有量
    ...
    function mint(uint32 amount) external payable {
        require(_started, "Moon Mosquito : Sale is not started");
        ...
        require(amount + publicMinted <= _publicSupply(), "Moon Mosquito: Exceed max supply");
        ...
        require(msg.value >= requiredValue, "Moon Mosquito: Insufficient fund");
