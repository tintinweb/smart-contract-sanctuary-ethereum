...
// hevm: flattened sources of /nix/store/lvkxdc4lsr4msvf6fa20ngn4wdq1m8rq-geb-lender-first-resort/dapp/geb-lender-first-resort/src/GebLenderFirstResortRewardsVested.sol
pragma solidity =0.6.7 >=0.6.0 <0.8.0;

////// /nix/store/lvkxdc4lsr4msvf6fa20ngn4wdq1m8rq-geb-lender-first-resort/dapp/geb-lender-first-resort/src/utils/ReentrancyGuard.sol
...
////// /nix/store/lvkxdc4lsr4msvf6fa20ngn4wdq1m8rq-geb-lender-first-resort/dapp/geb-lender-first-resort/src/GebLenderFirstResortRewardsVested.sol
...
abstract contract AuctionHouseLike_3 {
...
abstract contract RewardDripperLike_2 {
    ...
    function rewardToken() virtual external view returns (TokenLike_4);
}
...
contract TokenPool_2 {
    TokenLike_4 public token;
    address   public owner;

    constructor(address token_) public {
        token = TokenLike_4(token_);
        owner = msg.sender;
    }
...
contract GebLenderFirstResortRewardsVested is ReentrancyGuard {
    ...
    **/
    ...
    TokenPool_2                public ancestorPool;
    // The token used to pay rewards
    TokenPool_2                public rewardPool;
    // Descendant token
    TokenLike_4                public descendant;
    // Auction house for staked tokens
    AuctionHouseLike_3         public auctionHouse;
    ...
    RewardDripperLike_2        public rewardDripper;
    // Escrow for rewards
    ...
    constructor(
      ...
      uint256 percentageVested_
        ...
        auctionHouse                   = AuctionHouseLike_3(auctionHouse_);
        accountingEngine               = AccountingEngineLike_3(accountingEngine_);
        safeEngine                     = SAFEEngineLike_3(safeEngine_);
        rewardDripper                  = RewardDripperLike_2(rewardDripper_);
        escrow                         = StakingRewardsEscrowLike(escrow_);
        descendant                     = TokenLike_4(descendant_);

        ancestorPool                   = new TokenPool_2(ancestor_);
        rewardPool                     = new TokenPool_2(rewardToken_);
    ...
    */
    function modifyParameters(bytes32 parameter, address data) external isAuthorized {
        ...
        if (parameter == "auctionHouse") {
          auctionHouse = AuctionHouseLike_3(data);
        }
        else if (parameter == "accountingEngine") {
          accountingEngine = AccountingEngineLike_3(data);
        }
        else if (parameter == "rewardDripper") {
          rewardDripper = RewardDripperLike_2(data);
        }
