...
contract FLIP5050 is ERC721A, Ownable, ReentrancyGuard {
    ...
    function rand999() public view returns (uint256) {
    ...
    function rand1() public view returns (uint256) {
        uint256 seed = uint256(
            keccak256(
                abi.encodePacked(
                    block.timestamp +
        ...
        return (seed - ((seed / totalSupply()) * 1000));
    }

    function rand2() public view returns (uint256) {
        uint256 seed = uint256(
            keccak256(
                abi.encodePacked(
                    block.timestamp +
