...
contract PhotoGee is ERC721A, Ownable {
    ...
    constructor() ERC721A("PhotoGee", "PGEE") {}
    ...
    function mint(uint256 _quantity) external payable callerIsUser {
        ...
        require(
            ...
            "Square Bears :: Beyond Max Supply"
        );
        require(
            (totalPublicMint[msg.sender] + _quantity) <= MAX_PUBLIC_MINT,
            "Square Bears :: Already minted 3 times!"
        );
        require(
        ...
        callerIsUser
    {
        ...
        require(
            (totalSupply() + _quantity) <= MAX_SUPPLY,
            "Square Bears :: Cannot mint beyond max supply"
        );
        require(
            (totalWhitelistMint[msg.sender] + _quantity) <= MAX_WHITELIST_MINT,
            "Square Bears :: Cannot mint beyond whitelist max mint!"
        );
        require(
            msg.value >= (WHITELIST_SALE_PRICE * _quantity),
            "Square Bears :: Payment is below the price"
        );
        //create leaf node
        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(_merkleProof, merkleRoot, sender),
            "Square Bears :: You are not whitelisted"
        );
