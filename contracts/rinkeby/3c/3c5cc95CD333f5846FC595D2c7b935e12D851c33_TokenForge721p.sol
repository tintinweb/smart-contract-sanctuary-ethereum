...
/// Either signature is wrong or parameters have been corrupted
    error ErrInvalidSignatureOrParameters();

    error ErrPriceDidNotMatch(uint256 expected, uint256 received);
...
contract TokenForge721p is ERC721, ERC721Enumerable, ERC721Burnable, ERC721Pausable, ERC721URIStorage, Ownable {
    ...
    function validateSignature(
        ...
        if (message.recover(signature) != signer()) {
            revert ErrInvalidSignatureOrParameters();
        }
    ...
    function mintTo(
        ...
        if (msg.value != price) {
            revert ErrPriceDidNotMatch(price, msg.value);
        }
