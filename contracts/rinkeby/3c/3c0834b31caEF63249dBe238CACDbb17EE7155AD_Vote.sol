...
contract Vote is Ownable {
  ...
  function vote(
    ...
    require(
      TAPToken.isRecipient(to),
      "You cannot vote for an account that is not a recipient"
    );
    // Cannot overwrite votes
    require(voteId[key].timestamp == 0, "This vote already exists");
    require(amount > 0, "Vote must be greater than zero");
    // Cannot vote in the time between the vote period ending, and the new vote period starting
    require(votePeriodEnd() > block.timestamp, "The voting period has ended");
    WISH.transferFrom(msg.sender, address(this), amount);
    // Calculates the fees the user has incurred on this transaction (if whitelisted).
    uint256 wishingWellFee;
    uint256 foundationFee;
    if (!WISH.whitelist(msg.sender)) {
      (wishingWellFee, foundationFee) = WISH.getFees(amount);
    }
    voteId[key] = _Vote(
      msg.sender,
      to,
      amount - wishingWellFee - foundationFee, // this amount is equal to the actual amount transferred to this contract
      block.timestamp,
      true
    );
    // Add recipient to array if they are not already in it
    if (!isRecipientCounted[votePeriodCount][to]) {
      recipients.push(to);
      isRecipientCounted[votePeriodCount][to] = true;
    }
    // calculate amplified vote amount
    uint256 amplified = amplify(
      amount - wishingWellFee - foundationFee,
      block.timestamp
    );
    recipientVotes[votePeriodCount][to] += amplified;
    totalVotes += amplified;
    // Emit voting event
    emit Voted(
      msg.sender,
      to,
  ...
  // Unvotes a vote by a given key
  function unvote(bytes32 key) public {
    // You can only unvote a vote you made
    require(
      voteId[key].from == msg.sender,
      "You do not have permission unvote this vote"
    );
    // Cannot unvote a vote that is already unvoted
    require(voteId[key].isVoted, "This vote is already unvoted");
    // Cannot wait until after the voting period has ended to unvote
    require(
      voteId[key].timestamp > votePeriodStart(),
      "You can only unvote a vote from this month"
    );
    // Transfers the amount back to the sender
    WISH.transfer(msg.sender, voteId[key].amount);
    // sets the voted quality to false
    voteId[key].isVoted = false;
    // Gets the amount the vote was amplified and subtracts it from the recipient and total votes.
    uint256 amplified = amplify(voteId[key].amount, voteId[key].timestamp);
    recipientVotes[votePeriodCount][voteId[key].to] -= amplified;
    totalVotes -= amplified;
  }
  ...
  function countVotes() public view returns (RecipientToVote[] memory) {
    ...
    RecipientToVote[] memory recipientToVotes = new RecipientToVote[](length);
    for (uint256 i; i < length; i++) {
      recipientToVotes[i] = RecipientToVote(
        recipients[i],
        recipientVotes[votePeriodCount][recipients[i]]
      );
    }
    return recipientToVotes;
  }

  // Replacing lastMonthVotes with array
  function setLastMonthVotes(RecipientToVote[] memory array) internal {
    // sets as empty array
    delete lastMonthVotes;
    // appends every element from given array to storage array
  ...
  // can be called after the start of each month
  function endVotingPeriod() public {
    // can only end each voting period after the vote period end
    require(
      block.timestamp > votePeriodEnd(),
      "The voting period has not ended"
    );
    // copies last months votes to memory
    setLastMonthVotes(countVotes());
    // sets the new month and year (days are not needed)
    (_currentYear, _currentMonth, ) = BokkyPooBahsDateTimeLibrary
      .timestampToDate(block.timestamp);
    // transfers all the balance to the vault
    uint256 TAPwBalance = WISH.balanceOf(address(this));
    if (TAPwBalance > 0) {
      WISH.transfer(address(vault), TAPwBalance);
    }
    // initiate vault contract monthly distribution
    vault.initiateMonthlyDistribution(totalVotes);
    // removes all month-to-month variables
    delete recipients;
    votePeriodCount++;
    totalVotes = 0;
  }
...
contract Vault is Ownable {
  ...
  constructor(address _foundationWallet) {
    ...
    // Makes contract wait until first call of initiateMonthlyDistribution to allow distribution to be sent.
