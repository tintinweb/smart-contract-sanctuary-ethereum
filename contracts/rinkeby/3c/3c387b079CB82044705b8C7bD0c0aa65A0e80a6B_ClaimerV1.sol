...
// Author: Hagen Hübel (@itinance, [email protected])
...
import "./TokenForge1155.sol";

/// @title ClaimerV1
/// @author Hagen Hübel (@itinance), using code from Simon Fremaux (@dievardump) for ECDSA proof
contract ClaimerV1 is Context, Ownable, ERC1155Receiver {
    using ECDSA for bytes32;

    event Claimed(address indexed beneficiary, uint256 tokenId);

    // list of already used allowances
    mapping(bytes32 => bool) public usedAllowances;
    mapping(address => bool) public claimedAddresses;
    mapping(bytes32 => bool) public claimedCode;

    // address used to sign the allowances
    address private _signer;

    TokenForge1155 private _token;

    modifier onlyValidToken() {
        require(_msgSender() == address(_token), "Invalid token received");
        _;
    }

    constructor(TokenForge1155 token, address signer_) {
        _token = token;
        _signer = signer_;
    }
    ...
    ///         is the same
    /// @param code the nonce
    /// @return the message to sign
    function createMessage(address account, string memory code) public view returns (bytes32) {
        return keccak256(abi.encode(account, code, address(this)));
    }

    function balanceOf(uint256 tokenId) public view returns (uint256) {
        return _token.balanceOf(address(this), tokenId);
    }
    ...
    function claim(
        ...
        claimTo(_msgSender(), tokenId, code, signature);
    }
    ...
    function claimTo(
        ...
        _useAllowance(_msgSender(), code, signature);

        if (!_token.exists(tokenId)) {
            revert(string(abi.encodePacked("Claimer: Item not exist ", tokenId)));
        }

        bytes memory temp;
        _token.safeTransferFrom(address(this), beneficiary, tokenId, 1, temp);

        emit Claimed(beneficiary, tokenId);
    }
    
    function withdraw(uint256 tokenId, uint256 amount) external {
        require(_msgSender() == _signer, "Not allowed");

        bytes memory temp;
        _token.safeTransferFrom(address(this), _msgSender(), tokenId, amount, temp);
    }
    ...
    function _useAllowance(
        ...
        bytes32 message = validateSignature(account, code, signature);
        usedAllowances[message] = true;
        claimedAddresses[account] = true;
        claimedCode[keccak256(abi.encodePacked(code))] = true;
    }

    /// @notice This function verifies that the current request is valid
    /// @dev It ensures that _allowancesSigner signed a message containing (account, masterPieceId, tokenIds, uri, nonce, address(this))
    ...
    function validateSignature(
        ...
        bytes32 message = createMessage(account, code).toEthSignedMessageHash();
        ...
        require(claimedCode[keccak256(abi.encodePacked(code))] == false, "Code has already claimed");
        require(claimedAddresses[account] == false, "Address has already claimed");
    ...
    function onERC1155Received(
        ...
        uint256 value,
        bytes calldata data
    ) public virtual override onlyValidToken returns (bytes4) {
...
contract TokenForge1155 is ERC1155Tradable {
    constructor(address _proxyRegistryAddress)
        ERC1155Tradable("TokenForge", "TF", "https://accessories.token-forge.io/nft/{id}", _proxyRegistryAddress)
    {}

    function contractURI() public pure returns (string memory) {
        return "https://claim.token-forge.io/erc1155/metadata.json";
    }
 ...
 */
contract ERC1155Tradable is ContextMixin, ERC1155, NativeMetaTransaction, Ownable, AccessControl {
    using Strings for string;
    using SafeMath for uint256;

    event ProxyAddressChanged(address indexed proxyRegistryAddress, address indexed oldProxyRegistryAddress);
    ...
    function setProxyAddress(address proxyAddress) public onlyOwner {
        address oldProxyRegistryAddress = proxyRegistryAddress;
        proxyRegistryAddress = proxyAddress;
        emit ProxyAddressChanged(proxyRegistryAddress, oldProxyRegistryAddress);
    }
...
/******************************************************************************************
 *
 * This file was borrowed from OpenSea in order to get OpenSea-Compatibility
 *
