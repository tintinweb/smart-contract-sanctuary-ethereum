...
contract OrderRegistry {
    ...
    function approveTrade(uint256 _tradeId) public {
        ...
        if(msg.sender == users[0]){
            if(!trade.approval1) {
                require(IOrder(trade.order1).confirmBalance(), "order amount not deposited");
            ...
            if(!trade.approval2) {
                require(IOrder(trade.order2).confirmBalance(), "order amount not deposited");
    ...
    function cancelOrder(uint256 _orderId) external {
        IOrder order = IOrder(allOrders[_orderId]);
        require(order.isActive(), "Order is not active or is part of a trade");
        order.returnDeposit();
        order.toggleActive();
    }
