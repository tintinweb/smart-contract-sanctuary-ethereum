    ...
    ReentrancyGuard
{
    ...
    /// @notice Mints NFT in batch and automatic claim of free NFT
    /// if user has some granter NFT
    ...
    /// @param _qty number of (payable) Nfts to mint, excluding amount of free claims
    function batchMint(uint256 _qty) external payable nonReentrant {
        ...
        bool isPublicMintingPeriod = publicMintingStartTimestamp <=
            block.timestamp;
        if (isPublicMintingPeriod || granterTokenIds.length > 0) {
            require(
                _currentTokenId() + _qty <= maxSupply,
                "Exceeds max supply"
            );
        } else {
            require(
                _currentTokenId() + _qty <= maxSupply - reservedFreeAccessMints,
                "Exceeds max supply"
            );
        }

        if (granterTokenIds.length > 0) {
            reservedFreeAccessMints -= granterTokenIds.length;
            addressClaimCount[msg.sender] += granterTokenIds.length;
            for (uint256 i; i < granterTokenIds.length; i++) {
        ...
        if (msg.sender != owner()) {
            ...
            require(
                _qty + addressMintCount[msg.sender] <= maxMintPerAddress,
                "Too many minted"
            );

            require(msg.value == price * _qty, "incorrect price");
        }

        addressMintCount[msg.sender] += _qty;
        _batchMint(_qty + granterTokenIds.length, msg.sender);
    }
}