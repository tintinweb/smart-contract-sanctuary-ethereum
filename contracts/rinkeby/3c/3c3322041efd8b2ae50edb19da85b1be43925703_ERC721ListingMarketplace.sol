...
contract BharatNFTERC721 is GloballyApprovedOperatorsERC721, IERC2981 {
  using Strings for uint256;
  using Address for address;
  ...
  constructor(address[] memory marketplaceAddresses) GloballyApprovedOperatorsERC721("Bharat NFT", "BHARATNFT", marketplaceAddresses) {
  ...
  function mintAndCreateOrder(uint256 _tokenId, uint8 _royaltyPercentage, uint256 _price) external onlyNonContracts returns (bytes32) {
    mint(_tokenId, _royaltyPercentage);

    bytes32 id = orderMarketplace.createOrder(address(this), _tokenId, _price);

    emit MarketplaceOrderCreated(id);

    return id;
  }

  function mintAndCreateListing(uint256 _tokenId, uint8 _royaltyPercentage) external onlyNonContracts returns (bytes32) {
    mint(_tokenId, _royaltyPercentage);

    bytes32 id = listingMarketplace.createListing(address(this), _tokenId);
  ...
  function mintAndCreateAuction(uint256 _tokenId, uint8 _royaltyPercentage, uint128 _endsAt) external onlyNonContracts returns (bytes32) {
    mint(_tokenId, _royaltyPercentage);

    bytes32 id = auctionMarketplace.createAuction(address(this), _tokenId, _endsAt);
    emit MarketplaceOrderCreated(id);
    return id;
  }

//   function _baseURI() internal pure override returns (string memory) {
//     return  "https://artabia.com:3001/metadata/";
//   }
