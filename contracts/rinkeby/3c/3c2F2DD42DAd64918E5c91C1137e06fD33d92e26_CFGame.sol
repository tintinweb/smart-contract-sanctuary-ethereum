...
import "./CFStaking.sol";
import "./library/BaseRelayRecipient.sol";

contract CFGame is ReentrancyGuard, BaseRelayRecipient {
    enum Status {
        Activated,
        Live,
        Completed,
    ...
    address public cfStakingAddress;

    constructor(address _orgAddress, address _cfStakingAddress, address _trustedForwarder) {
        orgAddress = _orgAddress;
        cfStakingAddress = _cfStakingAddress;
    ...
    function initiateGame(
        ...
        CFStaking(_contractAddr).initiateGame(
            _msgSender(),
            _lockIn,
            _entryAmt,
            _entryAmt * _totalPlayers,
            _gameId,
            (cfStakingAddress == _contractAddr)
        );
    }
    ...
    function joinGame(uint256 _gameId) public nonReentrant returns (uint256) {
        ...
        CFStaking(currentGame.contractAddress).joinGame(_msgSender(), _gameId);
    ...
    function expiredGame(uint256 _gameId) public nonReentrant onlyOrgAddress {
        ...
        CFStaking(currentGame.contractAddress).cancelGame(_gameId);
    }
    ...
    function distributePrize(
        ...
        CFStaking(currentGame.contractAddress).distributeReward(
            _gameId,
            _winners,
            _winnerWeights,
            (currentGame.contractAddress == cfStakingAddress)
        );
    }
}
