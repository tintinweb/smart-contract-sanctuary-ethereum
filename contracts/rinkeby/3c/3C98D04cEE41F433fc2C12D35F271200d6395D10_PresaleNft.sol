...
contract PresaleNft is ERC721PresetMinterPauserAutoId
{
    ...
    function buy(uint256 quantity_) external
    {
        ...
        if(_type_ == 1) {
            require(_tokenIdTracker.current() + quantity_ <= presaleOneSupply, "Quantity is too high");
        ...
        if(_type_ == 2) {
            ...
            require(_tokenIdTracker.current() + quantity_ <= presaleOneSupply + presaleTwoSupply, "Quantity is too high");
        ...
        if(_type_ == 3) {
            ...
            require(_tokenIdTracker.current() + quantity_ <= presaleOneSupply + presaleTwoSupply + presaleThreeSupply, "Quantity is too high");
    ...
    {
        require(block.timestamp >= claimStart, "Claim has not started");
