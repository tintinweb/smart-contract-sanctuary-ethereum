...
contract A1 is ERC721Enumerable, ReentrancyGuard, Ownable {
    
    uint256 public price = 100000000000000000; //0.1 ETH

    constructor() ERC721("A1", "A1") {}
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: #fff; font-family: serif; font-size: 11px; font-weight: bold;}</style><rect width="100%" height="100%" fill="#0000ff" /><g transform="translate(0.000000,350.000000) scale(0.100000,-0.100000)" fill="#fff" stroke="none"> <path d="M1675 3268 c-16 -42 -505 -1433 -505 -1435 0 -2 63 -3 139 -3 l140 0 58 175 58 175 270 0 270 0 58 -175 58 -175 140 0 c129 0 140 1 135 18 -3 9 -118 334 -255 722 l-248 705 -157 3 c-120 2 -158 0 -161 -10z m263 -595 l90 -273 -193 0 -193 0 89 268 c108 324 98 297 108 286 5 -5 49 -132 99 -281z"/><path d="M2843 3162 l-183 -117 0 -117 c0 -65 2 -118 5 -118 2 0 79 47 169 105 91 58 168 105 171 105 3 0 5 -268 5 -595 l0 -595 135 0 135 0 0 725 0 725 -127 0 -128 0 -182 -118z"/></g><text x="25" y="230" class="base">';

        parts[1] = getFirst(tokenId);

        parts[2] = '</text><text x="25" y="249" class="base">';

        parts[3] = getSecond(tokenId);

        parts[4] = '</text><text x="25" y="268" class="base">';

        parts[5] = getThird(tokenId);

        parts[6] = '</text><text x="25" y="287" class="base">';

        parts[7] = getFourth(tokenId);

        parts[8] = '</text><text x="25" y="306" class="base">';

        parts[9] = getFifth(tokenId);

        parts[10] = '</text><text x="25" y="325" class="base">';
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "A1 #',
                        toString(tokenId),
                        '", "description": "The Decentralized Reserve Asset", "image": "data:image/svg+xml;base64,',
