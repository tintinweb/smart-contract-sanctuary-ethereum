...
contract PlatziPunksDNA {
    ...
    function deterministricPseudoRandomDNA(uint256 _tokenId, address _minter)
        public
        pure
        returns (uint256)
    {
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("PlatziPunks", "PPK") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _tokenId.current();
        require(current < maxSupply, "No items left, sorryEn");
        _safeMint(msg.sender, current);
        _tokenId.increment();
        tokenDNA[current] = deterministricPseudoRandomDNA(current, msg.sender);
    }
