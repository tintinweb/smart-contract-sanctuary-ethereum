...
contract Yolk is ERC20, Ownable {
    ...
    function setClaim(address _address, uint256 _amount) external {
        require(!rewardPaused,"Claiming reward has been paused"); 
        require(allowedAddresses[msg.sender], "Address does not have permission to distribute tokens");
...
//CURRENTLY SET TO FEB 16 2022 6PM
        uint256 calcTime = block.timestamp > 1645052400 ? 1645052400 : block.timestamp;

        return (Hatchlingz._walletBalanceOfCommon(user) * COMMON_RATE * (calcTime - lastUpdate[user]) / 86400) +
        (Hatchlingz._walletBalanceOfRare(user) * RARE_RATE * (calcTime - lastUpdate[user]) / 86400) +
        (Hatchlingz._walletBalanceOfLegendary(user) * LEGENDARY_RATE * (calcTime - lastUpdate[user]) / 86400);
