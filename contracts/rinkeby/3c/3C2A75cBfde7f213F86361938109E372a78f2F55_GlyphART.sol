...
contract GlyphART is ERC721A, Ownable {
    ...
    constructor() ERC721A("GlyphART", "GLYP") {

    }

    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        require(_exists(_tokenId));
        require(idToSeed[_tokenId] > 0);
    ...
    function mint(uint256 quantity) external {
        ...
        uint seed = _generateRandomDna(Strings.toString(id));
        require((currentSupply + quantity) < MAX_SUPPLY, "Quantity would exeed max supply");
        //require(msg.value >= PRICE, "Value is not right");
        ...
        emit Generated(id, msg.sender, uri);
        _safeMint(msg.sender, quantity);
    }

    function draw(uint id) private view returns (string memory) {
        unchecked {
            uint a = uint(uint160(uint256(keccak256(abi.encodePacked(idToSeed[id])))));
    ...
    function asciiToInteger(bytes32 x) private pure returns (uint256) {
        uint256 y;
