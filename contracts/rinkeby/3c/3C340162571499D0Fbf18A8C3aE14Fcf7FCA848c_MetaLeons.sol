...
contract MetaLeons is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    uint256 public lowerMintPrice = 0.1 ether;
    uint256 public maxPerUser = 5;
    uint256 public maxPerInvestor = 15;
    ...
    //     Constructor      //
    //////////////////////////
    constructor(address _signatureWallet, address _withdrawalWallet) ERC721A("Metaleons", "MLS") {
    ...
    modifier paidFull(uint256 _quantity) {
        require(msg.value >= mintPrice * _quantity, "Not enough money");
        _;
    }

    modifier paidLow(uint256 _quantity) {
        require(msg.value >= lowerMintPrice * _quantity, "Not enough money");
        _;
    }
    ...
    function setLowerMintPrice(uint256 _lowerMintPrice) external onlyOwner {
        lowerMintPrice = _lowerMintPrice;
    }
    ...
    ) external payable canMint(quantity) paidFull(quantity) {
    ...
    function OGMint(
        uint256 quantity, 
        bytes calldata _signature
    ) external payable canMint(quantity) paidLow(quantity) {
    ...
    function InvestorMint(
        uint256 quantity,
        bytes calldata _signature
    ) external payable paidLow(quantity) {
        require(presaleActive || saleActive);
        ...
        if (userMinted[msg.sender] >= maxPerInvestor - 1 && quantity + totalSupply() < maxSupply)  {
            quantity += 1;
    ...
    function airDrop(address recipient, uint256 quantity) external onlyOwner canMint(quantity) {
    ...
    function unfreezeMetadatas(uint256 _tokenId) external isOwnerOfToken(_tokenId) {
