...
contract Auction721 is AuctionStruct721,AuctionBase,TokenToAuction {
    ...
    function startAuction(
        ...
        //transfer nft to this smartcontract
        IERC721Upgradeable(_sellToken).transferFrom(sender, address(this), _sellTokenId);
        setAuctionForToken(_sellToken, _sellTokenId, currentAuctionId);
        emit AuctionCreated(currentAuctionId, sender, endTime);
    }

    function putBid(uint _auctionId) payable public{
        ...
        if(currentAuction.buyer != address(0x0)){
            refundBid(currentAuction.buyer,currentAuction.lastBid.amount);
        }
    ...
    function refundBid(address _oldBuyer,uint _amount) internal{
        balance[_oldBuyer] += _amount;
    }
    ...
    function _auctionId() internal returns(uint){
        uint currentAuctionId = getNextAndIncrementAuctionId();
        return currentAuctionId;
    }

    function checkApproveNFT(address _contract) public view returns(bool){
        return IERC721Upgradeable(_contract).isApprovedForAll(msg.sender,address(this));
    }

    function widthdrawnNFT(address _contract,address _to,uint256 tokenId) external {
        IERC721Upgradeable(_contract).transferFrom(address(this), _to, tokenId);
    }
    ...
    function widthdrawn(uint _amount) external{
        address payable receiver = payable(msg.sender);
        require(balance[receiver]>0,"No balance");
        receiver.transfer(_amount);
    }

    function widthdrawnETH(uint _amount) external{
        address payable receiver = payable(msg.sender);
