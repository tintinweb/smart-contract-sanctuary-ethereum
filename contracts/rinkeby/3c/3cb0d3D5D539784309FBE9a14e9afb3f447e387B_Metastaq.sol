...
contract Metastaq is Ownable, ERC721A, ReentrancyGuard {
    ...
    function allowlistMint(address to, uint256 quantity , bytes32 _type, bytes32[] memory proof ) 
        external
        payable
        callerIsUser
    {
        require(!allowlist[to], "you have already claimed your tokens");
        uint256 price = uint256(saleConfig.mintlistPrice);
        if (_type == "fiat") {
            require(msg.sender == walletAddress, "The caller is not metastaq");
        }
        require(price != 0, "allowlist sale has not begun yet");
        // require(allowlist[to] > 0, "not eligible for allowlist mint");
        require(_verify(_leaf(to, quantity), proof), "Invalid merkle proof");
    ...
    /**
    this function will set markle root
     */
