...
contract Payment is PausableUpgradeable, OwnableUpgradeable {
    ...
    function setSwitch() public onlyOwner {
        if (!paused()) {
            _pause();
        } else {
            _unpause();
    ...
    function setOpened() public onlyOwner {
        _unpause();
    }
    ...
    function redeem(
        ...
        if (!checkSigner(_hash, _signature))
    ...
    function withdraw() public payable onlyOwner {
        ...
        if (!sent) revert WithdrawEthFailed(eth);
    }

    function checkSigner(bytes32 hash, bytes memory signature)
        private
        view
        returns (bool)
    {
        return owner() == recoverSigner(hash, signature);
    }
    ...
    function _payFromToken(
        ...
        ERC20Upgradeable tokenContract = ERC20Upgradeable(_tokenAddress);
