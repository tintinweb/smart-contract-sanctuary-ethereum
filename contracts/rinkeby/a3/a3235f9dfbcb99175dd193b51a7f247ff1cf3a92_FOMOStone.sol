...
contract FOMOStone is ERC721, ERC721Enumerable, Ownable {
    bool public IsActive = true;
    string private _baseURIextended;
    address payable public immutable contract_owner;
    address private signAddr;

    uint32 public r1_num = 0;//第一階段mint數量
    uint32 public r2_num = 0;//第二階段mint數量
    ...
    event _verifySign(address _signer, string _message, bytes _sig, bool _a);

    constructor(address payable _owner) ERC721("FOMO Stone", "FOMO") {
        require(_owner != address(0));
        contract_owner = _owner;
        signAddr = msg.sender;

        saleConfig.round = 1;// 第一階段
        saleConfig.max_num = 2;//最多mint數量=2
        saleConfig.FOMO_total = 32;//發行4200
        saleConfig.round_total = 16;//每階段最多mint個數
        saleConfig.startTime = 1650934800;//2022-04-26 09:00
        saleConfig.endTime = 1650967200;//2022-04-26 18:00
        saleConfig._price = 1*10**17;//價格(0.1eth)
        saleConfig.og_price = 8*1*10**16;//價格(0.08eth)
    }
    ...
    function mint(uint num, uint8 _type, bytes memory _sig) public payable {
        ...
        if(_type==3)
        {
            ...
            emit _verifySign(contract_owner, ver_msg, _sig, a);
        }
        else
        {
            a = verify(contract_owner, toString(msg.sender), _sig);
            emit _verifySign(contract_owner, toString(msg.sender), _sig, a);
        }
