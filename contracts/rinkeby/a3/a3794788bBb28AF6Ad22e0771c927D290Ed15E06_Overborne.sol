...
contract Overborne is ERC721A, Ownable {
    ...
    uint256 public maxMintsPerPersonWhitelist = 2;
    uint256 public maxMintsPerPersonPublic = 5;
    ...
    modifier verifySupply(uint256 _numberOfMints) {
        require(tx.origin == msg.sender,  "No overbots.");
    ...
    function whitelistMerkleMint(bytes32[] calldata _merkleProof, uint256 _numberOfMints) public payable verifySupply(_numberOfMints) {
        require(mintStatus == MintStatus.WHITELIST, "Whitelist is closed.");
        require(msg.value >= price * _numberOfMints, "Incorrect ether sent." );
        require(_numberMinted(msg.sender) + _numberOfMints <= maxMintsPerPersonWhitelist, "Exceeds max mints.");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(verifyMerkle(_merkleProof, leaf), "Invalid Merkle proof.");

        _safeMint(msg.sender, _numberOfMints);
    }

    function publicMint(uint256 _numberOfMints) public payable verifySupply(_numberOfMints) {
        require(mintStatus == MintStatus.PUBLIC, "Public mint is closed.");
        require(msg.value >= price * _numberOfMints, "Incorrect ether sent." );
        require(_numberMinted(msg.sender) + _numberOfMints <= maxMintsPerPersonPublic, "Exceeds max mints.");
    ...
    event EventCharacterKickedOut(uint256 indexed tokenId);

    // currentStakingTime: current staking time in secs (0 = not staking)
    // totalStakingTime: total time of staking (in secs)
    function getStakingInfoForToken(uint256 tokenId) external view returns ( uint256 currentStakingTime, uint256 totalStakingTime )
    {
    ...
    uint256 internal constant SAFE_TRANSFER_ALLOWED = 0;
    uint256 internal constant SAFE_TRANSFER_BLOCKED = 1;
    uint256 private stakingTransferState = SAFE_TRANSFER_BLOCKED;

    function safeTransferWhileStaking( address from, address to, uint256 tokenId ) internal {
        require(ownerOf(tokenId) == _msgSender(), "Only owner can transfer");
        safeTransferFrom(from, to, tokenId);
    }

    // can safely transfer while staking
    function safeTransferWhileStaking( address from, address to, uint256[] calldata tokenIds) external {

        stakingTransferState = SAFE_TRANSFER_ALLOWED;

        uint256 num = tokenIds.length;
        for (uint256 i = 0; i < num; ++i) {
            uint256 tokenId = tokenIds[i];
            safeTransferWhileStaking(from, to, tokenId);
        }

        stakingTransferState = SAFE_TRANSFER_BLOCKED;
    }

    // block transfer while staking
    function _beforeTokenTransfers( address, address, uint256 startTokenId, uint256 quantity ) internal view override {
        for (uint256 tokenId = startTokenId; tokenId < startTokenId + quantity; ++tokenId) {
            // must be currently unstaked or transfering
            require(stakingTransferState == SAFE_TRANSFER_ALLOWED || stakingStartedTimestamp[tokenId] == NULL_STAKED, "Character currently staking");
        }
    }

    function setStakingAllowed(bool allowed) external onlyOwner {
        require(allowed != isStakingAllowed, "Staking allowance already set");
    ...
    function toggleStaking(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < num; ++i) {
            uint256 tokenId = tokenIds[i];
            toggleStaking(tokenId);
        }
    }

    function adminForceUnstake(uint256 tokenId) internal onlyOwner {
        require(stakingStartedTimestamp[tokenId] != NULL_STAKED, "Character not staked");
        ...
        emit EventCharacterUnstaked(tokenId);
        emit EventCharacterKickedOut(tokenId);
    }

    function adminForceUnstake(uint256[] calldata tokenIds) external {
        uint256 num = tokenIds.length;

        for (uint256 i = 0; i < num; ++i) {
            uint256 tokenId = tokenIds[i];
            adminForceUnstake(tokenId);
        }
    }
}
