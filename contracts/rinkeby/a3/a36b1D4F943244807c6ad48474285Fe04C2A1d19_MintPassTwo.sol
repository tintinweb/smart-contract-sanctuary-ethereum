...
contract MintPassTwo is PvSignedAllowlist, AbstractERC1155Factory {
    ...
    function mint(
        ...
        _claimAllowlistSpot(_signature, spotId, amount);
...
contract PvSignedAllowlist {
    ...
    function _claimAllowlistSpot(
        bytes calldata _signature, 
        uint256 spotId, 
        uint256 amount
    ) internal {
        require(_verify(keccak256(abi.encodePacked(msg.sender, spotId, amount)),_signature), "signature invalid");
        require(spotId < _claimGroups.length * 256, "ticket does not exist");
        ...
        require((localGroup >> spotIndex) & uint256(1) != 0, "ticket already used");
    ...
    function isValidSpot(uint256 spotId) public view returns (bool) {
        ...
        return (localGroup >> spotIndex) & uint256(1) == 1;
    }
    ...
    function _verify(bytes32 hash, bytes memory signature) internal view returns (bool) {
        return hash.toEthSignedMessageHash().recover(signature) == _signer;
    }
