...
contract RandomNumberGenerator is VRFConsumerBaseV2, IRandomNumberGenerator, Ownable {
    ...
    function requestRandomWords() external override {
        require(msg.sender == CZLottery, "Only CeeZeeLottery");
    ...
    function setLotteryId(uint256 _lotteryId) external onlyOwner {
        latestLotteryId = _lotteryId;
    }

    /**
     * @notice Set the address for the CeeZeeLottery
     * @param _CZLottery: address of the CeeZee lottery
     */
    function setLotteryAddress(address _CZLottery) external onlyOwner {
        CZLottery = _CZLottery;
    }

    /**
     * @notice Set the subscriptionId on ChainlinkVRF
     * @param _s_subscriptionId: address of the CeeZee lottery
     */
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        randomResult = uint32(1000000 + (randomWords[0] % 1123456));
        latestLotteryId = ICeeZeeLottery(CZLottery).viewCurrentLotteryId();
    }
}
...
interface ICeeZeeLottery {
    /**
     * @notice Start the lottery
     * @dev Callable by operator
     * @param _endTime: endTime of the lottery
     * @param _priceTicketInSAFU: price of a ticket in SAFU
     * @param _rewardsBreakdown: breakdown of rewards per bracket (must sum to 10,000)
     * @param _treasuryFee: treasury fee (10,000 = 100%, 100 = 1%)
     */
    function startLottery(
        uint256 _endTime,
        uint256 _priceTicketInSAFU,
    ...
    /**
     * @notice Draw the final number, calculate reward in SAFU per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in SAFU token
     * @dev Callable by operator
     */
