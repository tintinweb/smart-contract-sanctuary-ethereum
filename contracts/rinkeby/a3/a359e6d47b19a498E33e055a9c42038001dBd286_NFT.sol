...
contract NFT is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 _mintAmount, bytes32[] calldata merkleProof) public payable isValidMerkleProof(merkleProof, whitelistMerkleRoot) {
        ...
        if (msg.sender != owner()) {
            ...
            if (freeMinters[msg.sender] != true) {
                require(msg.value >= freeMint * _mintAmount, "transaction underpriced.");
    ...
    modifier isFreeMinter(address _sender) {
        if(msg.sender != owner()){
            require(freeMinters[_sender] == true, "Address is not free minter");
        }
        _;
    }
