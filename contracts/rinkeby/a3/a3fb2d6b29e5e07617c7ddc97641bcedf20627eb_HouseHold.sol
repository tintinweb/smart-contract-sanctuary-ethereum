...
contract HouseHold is UpgradeablePausable, IHouseHold {
  ...
  function createMortgage(
    ...
    bytes memory payload = abi.encodeWithSignature("initializeContract(address,address,uint32,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256)",
      address(hhConfig),
      borrower,
      name,
      client_id,
    ...
    address mortgage = ProxyFactoryUtil.deployMinimal(mortgageAddress, payload);
  ...
  function getMortgageTerm(address borrower, address mortgage) public view override returns (
    address,
    bytes32,
    uint256,
    uint256,
    ...
    require(mortgagesBorrowers[borrower].mortgages[mortgage], "Mortgage not find");
    IMortgage.MortgageTerms memory mortgageInstance = IMortgage(mortgage).getMortgageTerms();
    return (
      mortgageInstance.borrower,
      mortgageInstance.name,
      mortgageInstance.client_id,
      mortgageInstance.amount,
      mortgageInstance.payback_term,
      mortgageInstance.repayments,
      mortgageInstance.ear,
      mortgageInstance.grace_period,
      mortgageInstance.late_fee,
      mortgageInstance.mortgage_default);
  }
}
...
interface IHouseHold {
  ...
  function getMortgageTerm(address borrower, address mortgage) external view returns (
    address,
    bytes32,
    uint256,
    uint256,
