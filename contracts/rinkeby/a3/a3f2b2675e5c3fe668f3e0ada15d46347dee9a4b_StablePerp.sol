...
contract StablePerp {
    ...
    function fundUSDC(uint64 _UsdDec6) external returns (bool) {
        require(tradeAccount[msg.sender].acctType != 2, "LPs cannot fund");
    ...
    function withDrawUSDC(int256 _usdDec2) external returns (bool success) {
        require(tradeAccount[msg.sender].acctType != 2, "LPs cannot withdraw");
        ...
        _usdDec2 -= usdFee;
        bool success0 = usdcToken.transfer(msg.sender, uint256(_usdDec2 * DEC_USD_ADJ));
    ...
    function removeLiquidity() external returns (bool success) {
        ...
        if ((liqProviderAcct[msg.sender].epoch + EARLY_WD_DEFINED) > block.timestamp) {
            tradeParams.usdFeeSum += int64((FEE_PREC_FACTOR * EARLY_WD_PENALTY) / tradeParams.totLiquidity);
            tradeAccount[msg.sender].vUSD -= int64(EARLY_WD_PENALTY);
        }
    ...
    function swap(
        ...
        if ((block.number % MOD16_DIV) != uint256(tradeParams.lastBlockNum)) {
            ...
            equityBalances.emaSqrtPrice = lastBlockPrc / 30 + (29 * equityBalances.emaSqrtPrice) / 30;
        }
    ...
    function swapLP(
        ...
        int256 liqTotal = int256(tradeParams.totLiquidity);
        int48 lastBlockPrc = tradeParams.lastBlockPrice;
        int256 netEth = (int256(liqProviderAcct[_account].liquidity) * DEC_POOL_ETH) / sqrtPrice + int256(tradeAccount[_account].vETH);
        require(
            (netEth > int256(liqProviderAcct[_account].initEth / 2) && netEth < int256((3 * liqProviderAcct[_account].initEth) / 2)),
            "LP account outside zero fee limit"
        );
        if ((block.number % MOD16_DIV) != uint256(tradeParams.lastBlockNum)) {
            lastBlockPrc = int48(tradeParams.sqrtPrice / DEC_BLOK_PRICE);
            tradeParams.lastBlockPrice = lastBlockPrc;
        ...
        tradeAccount[_account].vUSD += int64(usdToTrader);
        tradeAccount[_account].vETH += int64(_ethD5ToTrader);
        tradeParams.sqrtPrice = int64(newsqrtPrice);
        emit Trade(_account, msg.sender, liqTotal, usdToTrader, _ethD5ToTrader, newsqrtPrice);
        return true;
    }
    ...
    function liquidateLP(address _defaulter) external returns (bool success) {
        ...
        int256 sqrtPrice = int256(equityBalances.emaSqrtPrice) * 10000;
        int256 netETH = int256(tradeAccount[_defaulter].vETH) + (indivLiquidity * DEC_POOL_ETH) / sqrtPrice;
        ...
        sqrtPrice = int256(tradeParams.sqrtPrice);
    ...
    function injectToken(int256 _usdDec6) external payable returns (bool) {
        ...
        bool success0 = usdcToken.transferFrom(msg.sender, address(this), uint256(_usdDec6));
