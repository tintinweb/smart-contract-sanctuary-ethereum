...
contract teste_2 is IERC721Receiver,Ownable{
    ...
    constructor (
        ...
        teste_1Contract = teste_11(_teste_1Address);
    }
    ...
    function viewCont() public view returns (teste_11){
        return teste_1Contract;
    }
    function stakingTokens(uint256[] calldata _tokenIds) public{
        require(staking,"Staking not available yet");
        for (uint i = 0; i < _tokenIds.length; i++) {
            require (!firepit[_tokenIds[i]].exists, 'Already in stake');
            require(msg.sender == teste_1Contract.ownerOf(_tokenIds[i]),"Not the owner of this token");
            teste_1Contract.transferFrom(msg.sender, address(this),_tokenIds[i]);
    ...
    function clamingTokens(uint256[] calldata _tokenIds, bool[] calldata _unstake) public{
        require(staking,"Staking not available yet.");
