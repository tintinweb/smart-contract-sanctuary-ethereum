...
contract LibertasSpaceWar is ERC721, Ownable {
    ...
    ERC721 private constant LibertasSpectrum = ERC721(0x4F7bD090fd42a2aA82cfdBa39DB48fA0cab96049);
    ...
    */
    ...
    function whitelistMint(uint num) public payable {
        require(saleIsActive, "Sale must be active to mint");
        require(num <= maxPurchase, "Can only mint 20 tokens at a time");
        require(msg.value >= whitelistPrice.mul(num), "not enough value for whitelist mint");
        require(balanceOf(msg.sender).add(num) <= LibertasSpectrum.balanceOf(msg.sender), "Purchase would exceed max whiltelist limit");
