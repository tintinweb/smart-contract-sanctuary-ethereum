...
contract WizzyWiz is ERC721, Ownable, ReentrancyGuard {
  ...
  function mintNFT(uint256 amount) public payable callerIsUser returns (uint256) {
    require(
      amount <= salesConfig.amountPerWallet && salesConfig.totalSupply >= amount + s_tokenCounter,
      "Exceeding mint limit"
    );
    require(mintList[msg.sender] + amount <= 2, "Minted max amount per wallet");
