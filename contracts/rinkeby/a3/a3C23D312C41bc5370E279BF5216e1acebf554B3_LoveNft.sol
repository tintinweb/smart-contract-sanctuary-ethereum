...
contract LoveNft is ERC721A, Ownable, ReentrancyGuard {
    ...
    bool private privateStarted = false;
    ...
    uint256 public startingPrice = 0.3 ether;
    uint256 public reservePrice = 0.01 ether;
    uint256 public discountRate = 60; // 1 min
    uint256 public discountPercentage = 5; // 5%
    uint256 public duration = 1740; // 29 minutes
    uint256 public startAt;
    uint256 public expiresAt;

    constructor(string memory _hiddenMetadataURI)
        ERC721A("15 Love nft", "15LoveNft")
    {
    ...
    modifier mintCompliance(uint256 _mintAmount, uint256 _totalAmount) {
        ...
        require(
            totalSupply() + _mintAmount <= _totalAmount,
            "Max supply exceeded."
        );
        _;
    }
    ...
    modifier mintPriceCompliance(uint256 _mintAmount) {
        require(
            msg.value >= startingPrice * _mintAmount,
        ...
        mintPriceCompliance(_mintAmount)
    {
        ...
        require(
            ...
            "Invalid proof, this wallet is not whitelisted."
        );
        ...
        mintCompliance(_mintAmount, maxSupplyTotal)
    {
        ...
        require(block.timestamp < expiresAt, "time over");
        uint256 _price = getPrice();
        require(msg.value >= _price * _mintAmount, "ETH < price");

        _safeMint(_msgSender(), _mintAmount);
        uint256 refund = msg.value - (_price * _mintAmount);
        if (refund > 0) {
        ...
        returns (bool)
    {
        ...
        return MerkleProof.verify(proof_, merkleRoot, _leaf);
    }

    // 0.00025 eth --> price decrease by 5% every 1 min
    function getDiscountRate() public view returns (uint256) {
        return ((startingPrice * discountPercentage) / 100) / discountRate;
    }

    function getPrice() public view returns (uint256) {
        uint256 timeElapsed = block.timestamp - startAt;
        uint256 discount = getDiscountRate() * timeElapsed;
        uint256 currentDutchPrice = startingPrice - discount;
        if (currentDutchPrice < reservePrice) {
            return reservePrice;
        } else {
    ...
    function setDiscountPercentage(uint256 _discountPercentage)
        public
        onlyOwner
    {
    ...
    function setExpiresAt(uint256 _expiresAt) public onlyOwner {
    ...
    function updateMaxSupplyPrivate(uint256 _number) public onlyOwner {
        require(
            _number <= maxSupplyTotal,
            "Private supply can not exceed total supply."
        );
