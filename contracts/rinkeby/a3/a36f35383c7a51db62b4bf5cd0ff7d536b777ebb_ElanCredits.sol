...
contract ElanCredits is ERC20, ERC20Burnable, ERC20Snapshot, Ownable, Pausable {
    ...
    //Custom Claim Addresses and claim limimts
    mapping(address => uint) public custom_claim_records;
    uint public custom_claim_amount = 0;
    uint constant public custom_claim_amount_cap = 25000000 * 10 ** 18; //25M

    constructor() ERC20("Elan Credits", "ELAN") {
        _mint(msg.sender, 10000000 * 10 ** decimals()); //10M minted to deployer for manual distribution
    ...
    function add_custom_claim_records(address[] calldata addresses, uint[] calldata amounts) public onlyOwner {
        for(uint i = 0; i < addresses.length; i++){
            require(custom_claim_amount + amounts[i] < custom_claim_amount_cap, "Given Amounts Exceed 25M Cap!!");

            custom_claim_records[addresses[i]] = amounts[i];
        }
    }

    function CUSTOM_Claim() public whenNotPaused{
        _mint(msg.sender, custom_claim_records[msg.sender] * 10 ** decimals());
        custom_claim_records[msg.sender] = 0;
    }
