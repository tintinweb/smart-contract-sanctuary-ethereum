...
/** Falcaw-9 $FALCAW also known as, Falcon-Caw 9, is the first community token that will fully support $CAW
 movement to the moon while being onboard our rocket, Falcaw-9. We are here to build a solid and strong community
 ...
 Buy Tax: 7%, 5% Marketing and 2% $CAW buyback wallet 
 Sell Tax: 7%, 5% Marketing and 2% $CAW buyback wallet
 Max Buy: 0.5%
 Max Wallet: 2%
...
contract FALCAW is ERC20, Ownable {
    ...
    uint256 public buyCawFee;

    uint256 public sellTotalFees;
    uint256 public sellOperationsFee;
    uint256 public sellLiquidityFee;
    uint256 public sellCawFee;
    ...
    event UpdatedBBAddress(address indexed newWallet);
    ...
    constructor() ERC20 ("FALCAW-9", "FALCAW") payable {
        ...
        buyLiquidityFee = 0;
        buyCawFee = 2;
        buyTotalFees = buyOperationsFee + buyLiquidityFee + buyCawFee;

        sellOperationsFee = 5;
        sellLiquidityFee = 0;
        sellCawFee = 2;
        sellTotalFees = sellOperationsFee + sellLiquidityFee + sellCawFee;

        marketingAddress = address(0xb110f865F969cA206525eB067b2153033b132e10);
        cawBBAddress = address(0x73c00A5dC55444fEa95e936ae1C0a9df4A39835d);
        treasuryAddress = address(0xdead);
        futureOwnerAddress = address(0x9ECE6D9e1184e7c1E7cB420be9b26Ccc3e72596F);
        ...
        _createInitialSupply(newOwner, totalSupply * 20 / 100);
    ...
    function getPrivateSaleMaxSell() public view returns (uint256){
        ...
        return amounts[1] + (amounts[1] * (sellCawFee + sellOperationsFee + sellLiquidityFee))/100;
    }
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _CawFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _CawFee) external onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(takeFee){
            ...
            if((earlyBuyPenaltyInEffect() || (amount >= maxBuyAmount - .9 ether && blockForPenaltyEnd + 3 >= block.number)) && automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && !_isExcludedFromFees[to] && buyTotalFees > 0){
                ...
                tokensForCharity += fees * buyCawFee / buyTotalFees;
            }
            ...
            // on sell
                ...
                tokensForCharity += fees * sellCawFee / sellTotalFees;
            }
    ...
    function swapBack() private {
        ...
        uint256 ethForDev = ethBalance * tokensForCharity / (totalTokensToSwap - (tokensForLiquidity/2));
        ...
        (success,) = address (cawBBAddress).call{value: ethForDev}("");
    ...
    function setBBAddress(address _cawBBAddress) external onlyOwner {
        require(_cawBBAddress != address(0), "address cannot be 0");
        cawBBAddress = payable(_cawBBAddress);
        emit UpdatedBBAddress(_cawBBAddress);
    }
