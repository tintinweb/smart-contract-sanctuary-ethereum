//PTW SPDX-License-Identifier: MIT
...
contract HappyContract is ERC721Enumerable, Ownable {
    ...
    uint256 public CHARITY_1_FEE = 25; // 2.5% to Charity chosen by us (Autism Speaks)
    uint256 public CHARITY_2_FEE = 25; // 2.5% to Charity chosen by us (Autism Radio)
    uint256 public OTHER_CHARITY_FEE = 50; // 5%   to Charity chosen by the community
    uint256 public HCBW_FEE = 120; // 12%  to Happy Club Business Wallet
    uint256 public CSW_FEE = 80; // 8%   to Community Support Wallet      //8+12=20% to Ownership Pool
    ...
    address public constant CHARITY_1_ADDRESS =
        0x0ade51c2b4c417fC6d4Fd9FE2701c60038EBfd42; // Autism Speaks - Twitter: https://twitter.com/autismspeaks
    address public constant CHARITY_2_ADDRESS =
        0x5dC22B9c05a4cD0E369A4C164686d7E293E1170f; // Autism Radio - Twitter: https://twitter.com/autismradio
    address public constant OTHER_CHARITY_ADDRESS =
        0x8d727A0ac890f89218Dc9391a8d3d9B36692a4D5; // The community will choose where we should donate this money
    ...
    function mintHappyFace(uint256 numHappyFace) public payable {
        ...
        require(
            totalSupply() < MAX_HappyFace,
            "We already sold out. Thank you!"
        );
        require(
            numHappyFace > 0 && numHappyFace <= 10,
            "You can mint minimum 1, maximum 10 Happy Faces."
        );
        require(
            totalSupply().add(numHappyFace) <= MAX_HappyFace - _reserved,
            "Not enough Happy Faces left."
        );
        require(
            totalSupply().add(numHappyFace) <= MAX_HappyFace,
            "Exceeds MAX_HappyFace."
        );
        require(
            (numHappyFace * _price) <= msg.value,
            "Ether value sent is not correct."
        );

        for (uint256 i = 0; i < numHappyFace; i++) {
    ...
    function premintHappyFace(uint256 numHappyFace) public payable {
        ...
        require(
            _addresses_for_canpresale.length > 0,
            "Haven't pre-sale address in contract."
        );
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint256(blockhash(_block_ref)) % MAX_HappyFace;
    ...
    function withdraw() public onlyOwner {
        ...
        uint256 _toOWNER = _balance -
            _toCHARITY1 -
            _toCHARITY2 -
            _toOTHERCHARITY -
            _toCSW -
            _toHCBW; // The rest to owner
