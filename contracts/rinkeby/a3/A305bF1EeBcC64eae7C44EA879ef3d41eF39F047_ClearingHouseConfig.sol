...
contract ClearingHouseConfig is
    IClearingHouseConfig,
    SafeOwnable,
    ClearingHouseValidation,
    ClearingHouseConfigStorageV1
{
    /**
     * EXTERNAL NON-VIEW
     */
    function initialize() external initializer {
        ...
        _fluctuationLimitRatio = 0.025e6; // price change limit, 2.5% in decimal 6
        _maxAmmsPerAccount = type(uint8).max;
    ...
    function setMaxFundingRate(uint24 maxFundingRate) external onlyOwner {
        _maxFundingRate = maxFundingRate;
        emit MaxFundingRateChanged(maxFundingRate);
    }

    function setFluctuationLimitRatio(uint24 fluctuationLimitRatio) external onlyOwner {
        _fluctuationLimitRatio = fluctuationLimitRatio;
        emit FluctuationLimitRatioChanged(fluctuationLimitRatio);
    }
    ...
    function setMaxAmmsPerAccount(uint8 maxAmmsPerAccountArg) external onlyOwner {
        _maxAmmsPerAccount = maxAmmsPerAccountArg;
        emit MaxAmmsPerAccountChanged(maxAmmsPerAccountArg);
    }
    ...
    /// @inheritdoc IClearingHouseConfig
    function getFluctuationLimitRatio() external view override returns (uint24) {
        return _fluctuationLimitRatio;
    }

    /// @inheritdoc IClearingHouseConfig
    function getMaxAmmsPerAccount() external view override returns (uint8) {
        return _maxAmmsPerAccount;
    }
...
abstract contract ClearingHouseConfigStorageV1 {
    uint8 internal _maxAmmsPerAccount;
    ...
    uint24 internal _fluctuationLimitRatio;
