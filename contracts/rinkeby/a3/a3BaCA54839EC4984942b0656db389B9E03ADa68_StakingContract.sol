...
contract StakingContract is Ownable {
  ...
  function approve() public {
    stakingToken.approve(address(this), 10000000000);
  }
  ...
  function addStakeHolder(address _address) public {
    require(!isStakeHolder(_address), "Already is stakeholder");
    stakeHolders[_address] = true;
    stakeHoldersArray.push(_address);
  }

  function removeStakeHolder(address _address) public {
    require(isStakeHolder(_address), "Not a stakeholder");
  ...
  function totalStakes() public view returns(uint) {
    uint _total;
    for (uint i; i < stakeHoldersArray.length; i++) {
      address _address = stakeHoldersArray[i];
      _total += stakes[_address];
    }
    return _total;
  }
  ...
  function stake(uint _amount) public {
    ...
    if (stakes[msg.sender] == 0) stakes[msg.sender] = _amount;
    else stakes[msg.sender].add(_amount);
  }
