...
contract FeeCollector {
    ...
    SystemContext public immutable systemContext;

    constructor(SystemContext systemContext_, uint256 minimumFee_, uint256 feeInPercents_) {
        systemContext = systemContext_;
        minimumFee = minimumFee_;
        feeInPercents = feeInPercents_;
    }

    function applyFeeAndRefund(uint256 diff_, uint256 valueReturned_, address payable refundAddress_) external onlyRole(systemContext.getAccessControlList().BRIDGE_ROLE()) {
    ...
    */
    ...
    function withdrawTo(address payable receiver) external onlyRole(systemContext.getAccessControlList().FEE_COLLECTOR_DEFAULT_ADMIN_ROLE()){
    ...
    function setMinimumFee(uint256 minimumFee_) external onlyRole(systemContext.getAccessControlList().FEE_COLLECTOR_DEFAULT_ADMIN_ROLE()){
        minimumFee = minimumFee_;
    }

    function setFeeInPercents(uint256 feeInPercents_) external onlyRole(systemContext.getAccessControlList().FEE_COLLECTOR_DEFAULT_ADMIN_ROLE()){
        feeInPercents = feeInPercents_;
    }
}
...
import "./tokens/OmniteToken.sol";
import "./OmniteBridgeSender.sol";
import "./OmniteBridgeReceiver.sol";
...
contract SystemContext is SystemContextSubscribable {
    ...
    OmniteBridgeSender internal omniteBridgeSender;
    OmniteBridgeReceiver internal omniteBridgeReceiver;
    ...
    OmniteToken internal omniteToken;
    FeeCollector internal feeCollector;
    uint16 public chainId;

    constructor (AccessControlList accessControlList_) {
    ...
    function getBridgeSender() external view returns(OmniteBridgeSender) {
        return omniteBridgeSender;
    }

    function getBridgeReceiver() external view returns(OmniteBridgeReceiver) {
        return omniteBridgeReceiver;
    }

    function setBridgeSender(OmniteBridgeSender bridgeContract_, bool triggerCallbacks) external onlyRole(accessControlList.SYSTEM_CONTEXT_DEFAULT_ADMIN_ROLE()) {
        if (triggerCallbacks) {
            _handleAddressChange(address(omniteBridgeSender), address(bridgeContract_));
        }

        omniteBridgeSender = bridgeContract_;
        accessControlList.grantRoleAddr(accessControlList.BRIDGE_ROLE(), address(bridgeContract_));
    }

    function setBridgeReceiver(OmniteBridgeReceiver bridgeContract_, bool triggerCallbacks) external onlyRole(accessControlList.SYSTEM_CONTEXT_DEFAULT_ADMIN_ROLE()) {
        if (triggerCallbacks) {
            _handleAddressChange(address(omniteBridgeReceiver), address(bridgeContract_));
        }

        omniteBridgeReceiver = bridgeContract_;
        accessControlList.grantRoleAddr(accessControlList.BRIDGE_ROLE(), address(bridgeContract_));
    }
    ...
    function setOmniteToken(OmniteToken omniteToken_, bool triggerCallbacks) external onlyRole(accessControlList.SYSTEM_CONTEXT_DEFAULT_ADMIN_ROLE()) {
        if (triggerCallbacks) {
            _handleAddressChange(address(omniteToken), address(omniteToken_));
        }

        omniteToken = omniteToken_;
        accessControlList.grantRoleAddr(accessControlList.OMNITE_TOKEN_ROLE(), address(omniteToken_));
    }

    function getFeeCollector() external view returns(FeeCollector) {
        return feeCollector;
    }

    function setFeeCollector(FeeCollector feeCollector_, bool triggerCallbacks) external onlyRole(accessControlList.SYSTEM_CONTEXT_DEFAULT_ADMIN_ROLE()) {
        if (triggerCallbacks) {
            _handleAddressChange(address(feeCollector), address(feeCollector_));
        }

        feeCollector = feeCollector_;
        accessControlList.grantRoleAddr(accessControlList.FEE_COLLECTOR_ROLE(), address(feeCollector_));
    }
}
 ...
 */
contract ContractFactory is SimpleFactory {
     ...
     */
    function _deployProxy(string memory blueprintName, bytes memory constructorParams, bytes32 collectionId) internal returns(address) {
        return deployCreate2WithParams(type(BeaconProxy).creationCode, abi.encode(address(beacons[blueprintName]), constructorParams), collectionId);
    }
    ...
    function createContractInstanceByName(string memory blueprintName, bytes memory constructorParams, bytes32 collectionId, string calldata name, address owner) external onlyBridgeRole returns(address) {
        ...
        address newContract = _deployProxy(blueprintName, constructorParams, collectionId);
...
import "../interfaces/IHodlersCollector.sol";
import "../interfaces/IERC20HodlersTracker.sol";
import "../SystemContext.sol";

contract OmniteToken is ERC20, IERC20HodlersTracker, ILayerZeroReceiver {
    IHodlersCollector public hodlersCollector;
    SystemContext public systemContext;
    ...
    function initialize(uint256 initialSupply_) external onlyRole(systemContext.getAccessControlList().TOKEN_DEFAULT_ADMIN_ROLE()) {
    ...
    function setHoldersCollector(IHodlersCollector hodlersCollector_) external onlyRole(systemContext.getAccessControlList().TOKEN_DEFAULT_ADMIN_ROLE())
    {
        hodlersCollector = hodlersCollector_;
    }
    ...
    function _addHodler(
        ...
        if (address(hodlersCollector) != address(0)) {
            hodlersCollector.addHodler(holder_, to_, amount_);
        }
    }

    function getHodlersCollector()
        external
        view
        override
        returns (IHodlersCollector)
    {
        return hodlersCollector;
    }
    ...
    function _afterTokenTransfer(
        ...
        _addHodler(from_, to_, amount_);
    }

    function setRemote(uint16 chainId_, bytes calldata remoteAddress_) external onlyRole(systemContext.getAccessControlList().TOKEN_DEFAULT_ADMIN_ROLE()) {
        remotes[chainId_] = remoteAddress_;
    }

    function sendTokens(uint16 chainId_, bytes calldata dstOmniChainTokenAddr_, uint quantity_) public payable
    {
        // burn the tokens locally.
        // tokens will be minted on the destination.
        require(
            allowance(msg.sender, address(this)) >= quantity_,
            "You need to approve the contract to send your tokens!"
        );

        // and burn the local tokens
        _burn(msg.sender, quantity_);

        // abi.encode() the payload with the values to send
        bytes memory payload = abi.encode(msg.sender, quantity_);

        // send LayerZero message
        endpoint.send{value:msg.value}( // TODO let's check if 200k gas is enough in case of complicated _afterTokenTransfer
            chainId_,
            dstOmniChainTokenAddr_,
            payload,
            payable(msg.sender),
            address(0x0),
            bytes("")
    ...
    function lzReceive(uint16 srcChainId_, bytes memory srcAddress_, uint64, bytes memory payload_) override external {
        require(msg.sender == address(endpoint));
        // owner must have setRemote() to allow its remote contracts to send to this contract
        require(
            srcAddress_.length == remotes[srcChainId_].length && keccak256(srcAddress_) == keccak256(remotes[srcChainId_]),
            "Invalid remote sender address. owner should call setRemote() to enable remote contract"
        );

        // decode
        (address toAddr, uint256 quantity) = abi.decode(payload_, (address, uint256));

        // mint the tokens back into existence, to the toAddr from the message payload
        _mint(toAddr, quantity);
    }

    function emergencyUnlock(address toAddr_, uint256 mintSupply_) external onlyRole(systemContext.getAccessControlList().TOKEN_UNLOCK_ROLE()) {
        _mint(toAddr_, mintSupply_);
    }
}
...
contract OmniteBridgeSender is IBridge {
    ...
    uint64 constant public maxUrlLen = 512;
    uint64 constant public maxCallDataLen = 8096;
    ...
    function setEndpoint(ILayerZeroEndpoint endpoint_) external onlyRole(systemContext.getAccessControlList().BRIDGE_DEFAULT_ADMIN_ROLE()) {
        endpoint = endpoint_;
    }
    ...
    function setMaxNetworks(uint64 maxNetworks_) external onlyRole(systemContext.getAccessControlList().BRIDGE_DEFAULT_ADMIN_ROLE()) {
    ...
    internal nativePayment(params_.refundAddress, params_.value) {
        require(params_.gasAmount >= minGas, "Gas limit too little");
        endpoint.send{value : params_.value}(
            params_.chainId,
            params_.bridge,
            params_.buffer,
            payable(address(systemContext.getFeeCollector())),
            address(this),
            abi.encodePacked(uint16(1), uint256(params_.gasAmount))
        );

        emit SendEvent(params_.chainId, params_.bridge, endpoint.getOutboundNonce(params_.chainId, address(this)));
    }

    modifier nativePayment(address refundAddress_, uint256 valueSent_) {
        FeeCollector feeCollector_ = systemContext.getFeeCollector();
        ...
        feeCollector_.applyFeeAndRefund(diff_, currentAmount_-diff_, payable(refundAddress_));
    }
    ...
    function _sendMessage(uint16 chainId_, bytes calldata bridge_, bytes memory buffer_, address refundAddress_, uint256 gasAmount_) internal {
        _sendMessageWithValue(SendMessageWithValueParams({chainId:chainId_,bridge: bridge_, buffer:buffer_, refundAddress:refundAddress_,value: msg.value,gasAmount: gasAmount_}));
    }
    ...
    function mintOnTargetChain (
        ...
        require(bytes(tokenUri_).length <= maxUrlLen, "token uri to long");
    ...
    function callOnTargetChain(
        ...
        require(callData_.length <= maxCallDataLen, "calldata to long");
    ...
    function _deployExternalCollection(
        ...
        require(deploymentParams_.length <= maxNetworks, "networks config to long");
        ...
        for (uint256 i = 0; i < deploymentParams_.length; i++) {
            ...
            buffer : deployContractEncode(ERC721_BLUEPRINT_NAME, collectionId_, deploymentParams_[i].ctorParams, params.collectionName, params.owner),
    ...
    function deployExternalCollection(
        ...
        return _deployExternalCollection(collectionId_, deploymentParams_, params);
    }
    ...
    function deployExternalCollectionOnNewChains(
        ...
        CollectionsRegistry collectionRegistry = systemContext.getCollectionRegistry();
        require(collectionRegistry.externalToCollection(params.originalCollection) == collectionId_, "Collection not registered yet");
    ...
    function deployNativeCollection(
        ...
        for (uint256 i = 0; i < deploymentParams_.length; i++) {
            ...
            if(deploymentParams_[i].chainId == systemContext.chainId()) {
                ...
                chainId : deploymentParams_[i].chainId,
                bridge : deploymentParams_[i].bridgeAddress,
                buffer : deployContractEncode(ERC721_WITH_SLOTS_BLUEPRINT_NAME, collectionId_, deploymentParams_[i].ctorParams, params.collectionName, params.owner),
                refundAddress : params.refundAddress,
                value : deploymentParams_[i].value,
                gasAmount : params.gasAmount
                }));
...
contract OmniteBridgeReceiver is ILayerZeroReceiver, IBridge {
    using Address for address;
...
import "./IERC20HodlersTracker.sol";

interface IHodlersCollector {
    event HodlerAdded(address indexed holder);
    ...
    function getERC20HodlerTracker()
        external
        view
        returns (IERC20HodlersTracker);
}
...
import "./IHodlersCollector.sol";

interface IERC20HodlersTracker {
    function getHodlersCollector() external view returns (IHodlersCollector);
}
...
contract ERC721Mintable is ERC721EnumerableUpgradeable, ITokenBridgeable, OwnableUpgradeable, IERC721MintableInitializable {
    ...
    */
    ...
    function moveTo(uint16 _l0ChainId, bytes calldata _destinationBridge, uint256 _tokenId, uint256 _gasAmount) external override payable {
        ...
        system.getBridgeSender().mintOnTargetChain{value : msg.value}(_l0ChainId, _destinationBridge, msg.sender, msg.sender, _tokenId, _tokenURIs[_tokenId], _gasAmount);
    }
