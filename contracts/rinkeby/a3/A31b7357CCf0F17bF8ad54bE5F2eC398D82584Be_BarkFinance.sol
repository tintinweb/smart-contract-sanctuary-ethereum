...
contract Boostable is Context, Ownable {
    ...
    function calculateGainedTreatsWithBoost(uint256 nbMiners, address adr) internal returns(uint256) {
...
contract BarkFinance is Context, Ownable, Boostable {
    uint256 private constant TREATS_REQ_PER_DOG = 1_080_000; 
    uint256 private constant INITIAL_MARKET_TREATS = 108_000_000_000;
    ...
    function kickOff() public payable onlyOwner {
        require(marketTreats == 0);
        initialized = true;
        marketTreats = INITIAL_MARKET_TREATS;
    }

    function adoptDogs(address ref) external payable {
        require(initialized);
        
        uint256 dogsAdopted = calculateDogsAdopted(msg.value, address(this).balance - msg.value);

        uint256 treatsDevFee = devFee(dogsAdopted);
        if(treatsDevFee == 0) revert FeeTooLow();

        dogsAdopted -= treatsDevFee;

        uint256 ethDevFee = devFee(msg.value);
        
        devWallet.transfer(ethDevFee);
        claimedTreats[msg.sender] += dogsAdopted;
        trainDogs(ref);
        currentBalance += msg.value - ethDevFee;
    }
 
    function trainDogs(address ref) public {
        require(initialized);
        require(getMyTreats(msg.sender) > TREATS_REQ_PER_DOG);
        ...
        uint256 gainedTreats = calculateGainedTreatsWithBoost(getMyDogs(msg.sender), msg.sender);

        uint256 treatsUsed = getMyTreats(msg.sender);
        treatsUsed += gainedTreats;

        uint256 myCratesRewards = getTreatsSinceLastTrained(msg.sender);
        myCratesRewards += gainedTreats;

        claimedTreats[msg.sender] += myCratesRewards;

        uint256 newDogs = claimedTreats[msg.sender] / TREATS_REQ_PER_DOG;
        claimedTreats[msg.sender] -= (TREATS_REQ_PER_DOG * newDogs);

        dogs[msg.sender] += newDogs;
        lastTrained[msg.sender] = block.timestamp;

        //send referral treats
        claimedTreats[referrals[msg.sender]] += treatsUsed / 8;
        
        //boost market to nerf dogs hoarding
        marketTreats += treatsUsed / 5;
    }
    ...
    function getMyTreats(address adr) public view returns(uint256) {
        return claimedTreats[adr] + getTreatsSinceLastTrained(adr);
    }
    
    function getTreatsSinceLastTrained(address adr) public view returns(uint256) {
        return min(TREATS_REQ_PER_DOG, block.timestamp - lastTrained[adr]) * dogs[adr];
    }
