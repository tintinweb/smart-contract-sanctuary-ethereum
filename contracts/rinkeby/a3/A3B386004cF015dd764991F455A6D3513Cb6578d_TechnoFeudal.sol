...
contract TechnoFeudal is ERC721, IERC2981, Ownable, ReentrancyGuard {
    ...
    constructor(address _royaltyReceiverAddress)
        ERC721("Techno-Feudal Citizens-iv", "TFCiv")
    {
        ...
        assert(MAX_TOKENS_PER_WALLET >= MAX_MINT_LIST_MINTS);
    }
    ...
    modifier mintListActive() {
        require(saleState == SaleState.MintList, "Mint list redemption is not open");
        _;
    }
    ...
    function mintMintList(uint8 numberOfTokens, bytes32[] calldata merkleProof)
        external
        payable
        nonReentrant
        mintListActive
        ...
        isValidPreSaleAddress(merkleProof)
    {
        uint256 numAlreadyMinted = mintListMintCounts[msg.sender];

        require(
            numAlreadyMinted + numberOfTokens <= MAX_MINT_LIST_MINTS,
            "Exceeds max number for mintlist mint"
        );

        mintListMintCounts[msg.sender] = numAlreadyMinted + numberOfTokens;
