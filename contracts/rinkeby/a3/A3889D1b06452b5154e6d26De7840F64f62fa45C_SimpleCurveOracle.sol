...
contract KashiPair is ERC20, BoringOwnable {
    ...
    modifier solvent(uint256 exchangeRateOption) {
        _;
        uint256 _exchangeRate;
        if (exchangeRateOption == 1) {
            (, _exchangeRate) = oracle.peek(oracleData);
        } else if (exchangeRateOption == 2) {
            (, _exchangeRate) = updateExchangeRate();
        } else {
            _exchangeRate = exchangeRate;
        }
        require(_isSolvent(msg.sender, false, _exchangeRate), "KashiPair: user insolvent");
    }
    ...
    function removeCollateral(
        address to,
        uint256 share,
        uint256 exchangeRateOption
    ) public onlyEOA solvent(exchangeRateOption) {
    ...
    function addAsset(
        ...
        bool skim,
        uint256 share
    ) public onlyEOA returns (uint256 fraction) {
        accrue();
    ...
    function removeAsset(address to, uint256 fraction) public onlyEOA returns (uint256 share) {
        accrue();
    ...
    function borrow(
        ...
        uint256 amount,
        uint256 exchangeRateOption
    ) public onlyEOA solvent(exchangeRateOption) returns (uint256 part, uint256 share) {
        accrue();
    ...
    function repay(
        ...
        bool skim,
        uint256 part
    ) public onlyEOA returns (uint256 amount) {
        accrue();
    ...
    ) external payable onlyEOA returns (uint256 value1, uint256 value2) {
