...
contract ILO is Ownable {
    ...
    SentinelToken public rewardToken;
    ...
    function initialize(LiquidityManager _liquidityManager, SentinelToken _rewardToken, uint _rewardBalance, uint _startTime, uint _duration, uint _minimumAmount, uint _linearBonusCoefficient) external onlyOwner {
    ...
    function pay() external payable isInitialized returns (uint, uint) {
        require(msg.value > 1024, "not enough ETH provided");
        ...
        providedEth[msg.sender] += msg.value;
        totalProvidedEth += msg.value;
        uint _effectiveEth = getEffectiveEth(msg.value);
        ...
        lastPriceScaled = msg.value*totalEffectiveEth/_effectiveEth;
    ...
    function refund() external isInitialized returns (uint refundAmount) {
        ...
        SafeTransferEth.transferEth(msg.sender, refundAmount);
    ...
    function spawnPool(address payable receiver) external onlyOwner isInitialized {
        ...
        uint ethForPool = totalProvidedEth/5;
        liquidityManager.createPool{value: ethForPool};
        poolCreated = true;
    ...
    function changeMinAmount(uint amount) external onlyOwner isInitialized {
        minimumEthAmount = amount;
    }
}
