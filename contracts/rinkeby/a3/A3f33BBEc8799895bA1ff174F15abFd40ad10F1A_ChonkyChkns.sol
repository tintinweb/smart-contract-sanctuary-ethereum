...
contract ChonkyChkns is ERC721A, ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    // Map of wallet address -> number of genesis/chonklist tokens minted. Used to enforce max mints per wallet.
    mapping(address => uint256) private _numGenesisMinted;
    mapping(address => uint256) private _numChonkListMinted;

    // Maps of wallet addresses => number of Genesis/Standard NFTs they own. Used for feed balance calculations.
    mapping(address => uint256) public numGenesisOwned;
    mapping(address => uint256) public numStandardOwned;

    // Related contracts, for FEED token generation, user-customized traits, and tokenURI construction based on custom traits
    ...
    constructor() ERC721A("ChonkyChkns", "CHONKYCHKNS") {
        ...
        MAX_CHONKLIST_MINT_AMOUNT_PER_WALLET = 10;
        ...
        returns (string memory)
    {
        ...
        // Role of determining tokenURI per token is delegated to tokenURIManager. This allows the tokenURI format to flexibly change as new features are added to the project, e.g. new traits that may affect metadata.
    ...
    // Call this function if/when MintState = PUBLIC and there are still remaining genesis tokens. All users (including non-OG roles) will be able to mint up to the max per wallet of genesis tokens on a first-come first-serve basis. This function shouldn't be called after all genesis tokens have been minted - it will function the same as publicMint but cost additional gas.
    ...
    function mintForAddresses(address[] calldata _receivers)
        external
        onlyOwner
    {
        for (uint256 i; i < _receivers.length; ) {
            _safeMint(_receivers[i], 1);
            _updateBalancesOnStandardMint(_receivers[i], 1);
            unchecked {
                ++i;
            }
        }
    ...
    function setTokenURIManager(address _tokenURIManager) external onlyOwner {
        ...
        // If there was a pre-existing TokenURIManager, record the previous base URI and set it in the new manager
    ...
    function _calculateAndRegisterGenesisQuantity(uint256 _maxMintAmount)
        ...
        returns (uint256)
    {
        // Allocate as many of _maxMintAmount as possible to be genesis tokens, under wallet and supply constraints.
        unchecked {
            uint256 genesisQty = Math.min(
                Math.min(
                    MAX_GENESIS_MINT_AMOUNT_PER_WALLET -
                        _numGenesisMinted[_msgSender()],
                    _maxMintAmount
                ),
                MAX_GENESIS_SUPPLY - totalGenesisSupply
            );

            // If any genesis tokens are being minted in this transaction, perform pre-mint registration steps for them (set isGenesis status for each tokenId, increment numGenesisMinted for user, increment totalGenesisSupply)
            if (genesisQty > 0) {
                uint256 tokenId = _currentIndex;
                for (uint256 i = 0; i < genesisQty; ++i) {
                    isGenesis[tokenId + i] = true;
                }
                _numGenesisMinted[_msgSender()] += genesisQty;
    ...
    function _registerPresaleStandardQuantity(uint256 _standardTokenQuantity)
        internal
    {
        // If any standard tokens are being minted in this presale transaction, verify that the total minted quantity for the user is within max per wallet constraints, then increment numChonkListMinted for user.
        if (_standardTokenQuantity > 0) {
            require(
                _standardTokenQuantity + _numChonkListMinted[_msgSender()] <=
                    MAX_CHONKLIST_MINT_AMOUNT_PER_WALLET,
                "Exceeded max per wallet"
            );
            _numChonkListMinted[_msgSender()] += _standardTokenQuantity;
        }
    }
...
contract FeedToken is ERC20("Feed", "FEED"), Ownable {
    ...
    // Withdraw (mint) the current feed balance of a particular address to that address. Pending claimable portion of feed balance is withdrawn as well.
