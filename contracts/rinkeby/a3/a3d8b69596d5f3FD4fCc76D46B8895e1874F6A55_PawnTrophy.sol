...
error PawnTrophy__AccountNotAllowed();
error PawnTrophy__AlreadyMinted();

contract PawnTrophy is ERC1155, ERC1155Burnable, ReentrancyGuard {
    /// EVENTS ///
    ...
    constructor(bytes32 merkleRoot_, string memory uri_) ERC1155(uri_) {
    ...
    function mint(bytes32[] calldata merkleProof) external nonReentrant {
        if (!MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender)))) {
            revert PawnTrophy__AccountNotAllowed();
        }
        if (minted[msg.sender]) {
            revert PawnTrophy__AlreadyMinted();
        }
