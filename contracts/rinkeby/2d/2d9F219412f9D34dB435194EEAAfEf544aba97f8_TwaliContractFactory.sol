...
// Implmenentation Contract -- Twali's Base
...
contract TwaliContractFactory is Ownable {
        ...
        constructor(address _contractImplementation) {
                ...
                admin = msg.sender; // confirm we want it to be sender
        }

        // Creates a contract clone of the Logic `Implementation` Contract
        function createTwaliClone(address _admin, address _expert, string memory _sowData, bool _werkApproved, bool _werkPaidOut) external onlyOwner {
                ...
                TwaliContract(clone).initialize(_admin, _expert, _sowData, _werkApproved, _werkPaidOut, block.timestamp);
