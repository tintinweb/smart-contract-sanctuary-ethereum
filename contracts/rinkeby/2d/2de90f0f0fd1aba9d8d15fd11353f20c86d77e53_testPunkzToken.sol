...
contract testPunkzToken is ERC20, Ownable{
    ...
    uint public lockoutTime = 20 minutes; //wannabe 10 days
    uint public dailyRate = 10 minutes; // wannabe 1 day
    ...
    function rewardFromMint(uint[] memory tokenIds) external{
        require(block.timestamp - punkZStart > 7 days, "It's not birthday yet");
        uint totalAmount;
        for(uint i=0;i<tokenIds.length;i++){
    ...
    function getReward(uint tokenId) public view returns(uint){
        ...
        if(block.timestamp - currentToken.lockTime <= lockoutTime){
            ...
            reward = 1 ether*timeInDays*rewards[tokenId/2]*(10+multiplier[combo])/dailyRate - externallyClaimed[tokenId];
        }
        else{
            ...
            if(block.timestamp > unlockTime && currentToken.lastClaim < unlockTime){
                ...
                uint combo = currentToken.lockedPieces;
                reward = 1 ether*timeLocked*rewards[tokenId/2]*(10+multiplier[combo])/ dailyRate;
