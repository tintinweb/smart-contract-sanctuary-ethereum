...
import "./interfaces/IACNFT.sol";
import "erc721a/contracts/ERC721A.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";
import "@openzeppelin/contracts/utils/Strings.sol";

contract ACNFT is IACNFT, ERC721A, Ownable {
    ...
    uint256 public constant TOKENS_PER_ACCOUNT_LIMIT = 2;
    ...
    modifier mintCheck(SaleStatus status, uint256 count) {
        require(saleStatus == status, "ACNFT: Not operational");
        require(
            _totalMinted() + count <= MAX_SUPPLY,
            "ACNFT: Number of requested tokens will exceed max supply"
        );
        require(
            _mintedCount[msg.sender] + count <= TOKENS_PER_ACCOUNT_LIMIT,
            "ACNFT: Number of requested tokens will exceed the limit per account"
        );
        _;
    }
        ...
        mintCheck(SaleStatus.PRESALE, count)
    {
        ...
        require(
            ...
            "ACNFT: You are not whitelisted"
        );
        require(
            msg.value >= count * whiteListPrice,
            "ACNFT: Ether value sent is not sufficient"
        );
        _mintedCount[msg.sender] += count;
        _safeMint(msg.sender, count);
        // if (msg.value > count * whiteListPrice) {
        //     payable(msg.sender).transfer(msg.value - count * whiteListPrice);
        // }
    }
        ...
        mintCheck(SaleStatus.PUBLIC, count)
    {
        ...
        _mintedCount[msg.sender] += count;
        _safeMint(msg.sender, count);
        // if (msg.value > count * mintPrice) {
        //     payable(msg.sender).transfer(msg.value - count * mintPrice);
        // }
    }

    function internalMint(address receiver) external override onlyOwner {
        require(!_internalMinted, "ACNFT: The interior has been mint");
    ...
    function withdraw() external override onlyOwner {
        ...
        require(balance > 0, "ACNFT: Insufficient balance");
        (bool success, ) = payable(_paymentAddress).call{value: balance}("");
        require(success, "ACNFT: Withdrawal failed");
    }
