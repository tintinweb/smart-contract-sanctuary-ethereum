...
contract StakerContract is ReentrancyGuard {
    using SafeERC20 for IERC20;

    ERC721Psi public immutable nftCollection;
    IRewardToken public immutable rewardsToken;

    constructor(IRewardToken _rewardsToken, ERC721Psi _nftCollection) {
    ...
    struct StakeHistoryObj {
        uint256 tokenId;
        uint256 timeOfStake;
        uint256 timeOfUnstake;
    }

    //staker is redundant, but needed to add something otherwise I'd get a recursive error
    struct StakeHistory {
        address staker;
        StakeHistoryObj[] stakeHistoryObj;
    }
    ...
    //Map staker address to stake history
    mapping(address => StakeHistory) public stakerHistory;
    ...
    function stake(uint256 _tokenId) external nonReentrant {
        ...
        //Transfer the token from wallet to the Smart Contract
    ...
    // Check if user has any Tokens Staked and if they tried to withdraw,
    // calculate the rewards and store them in the unclaimedRewards
    // decrement the amountStaked of the user and transfer the token back to them
    ...
    function unstake(uint256 _tokenId) public {
        ...
        // Create the history stake obj whenever a token is un staked
        StakeHistoryObj memory _stakeHistoryObj = StakeHistoryObj(
            stakers[msg.sender].stakedTokens[index].tokenId,
            stakers[msg.sender].stakedTokens[index].timestamp,
            block.timestamp
        );
        // adding the histoy object to users stake history
        stakerHistory[msg.sender].stakeHistoryObj.push(_stakeHistoryObj);
        stakerHistory[msg.sender].staker = msg.sender;
    ...
    function getStakedHistory(address _user)
        public
        view
        returns (StakeHistoryObj[] memory)
    {
        // Check if we know this user
        if (stakerHistory[_user].stakeHistoryObj.length > 0) {
            // Return all the tokens in the stakedToken Array for this user that are not -1
            StakeHistoryObj[] memory _stakedTokens = new StakeHistoryObj[](
                stakerHistory[_user].stakeHistoryObj.length
            );
            uint256 _index = 0;

            for (
                uint256 j = 0;
                j < stakerHistory[_user].stakeHistoryObj.length;
                j++
            ) {
                _stakedTokens[_index] = stakerHistory[_user].stakeHistoryObj[j];
                _index++;
            }
