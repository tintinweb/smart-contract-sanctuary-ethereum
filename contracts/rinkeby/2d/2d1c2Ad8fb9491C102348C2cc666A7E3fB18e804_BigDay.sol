...
import "./BoundCurve.sol";
import "./SignatureValidator.sol";
import "./TokenAsDate.sol";
import "./BigDayToken/BigDayToken.sol";

/// @title The BigDay
/// @author A/S: AddressedToTheSubject
/// @notice The BigDay NFT project's contract

interface IERC20 {
    function transfer(address _to, uint256 _amount) external returns (bool);
}

contract BigDay is Ownable, ERC721Royalty, Pausable, ReentrancyGuard, SignatureValidator, TokenAsDate, BoundCurve {
    using BigDayTokens for BigDayTokens.BigDayToken;
    using Covers for Covers.Cover;

    event CoverAdded(uint256 indexed tokenId, uint256 coverIndex);
    event CoverUpdated(uint256 indexed tokenId, uint256 coverIndex);
    event ActiveCoverChanged(uint256 indexed tokenId, uint256 oldIndex, uint256 newIndex);
    event CoversStatesUpdated(uint256 indexed tokenId);

    string private _contractMetadataURI;
    bool public isPublicSalesOpen = false;
    string public wepAppBaseURL = "ipfs://contract.metadata";
    uint256 public maxCoversAmount = 24;

    // token contracts by uint
    mapping(uint256 => BigDayTokens.BigDayToken) private _tokens;

    modifier _isTokenOwner(uint256 tokenId) {
        require(ownerOf(tokenId) == _msgSender(), "Access restricted to the token owner");
        _;
    }

    modifier _isTokenExists(uint256 tokenId) {
    ...
    constructor(
        ...
        string memory _wepAppBaseURL,
        address _feeReceiver
    )
        SignatureValidator(string.concat(collectionName, ":", tokenName))
        BoundCurve(_feeReceiver)
        ERC721(collectionName, tokenName)
    {
        _contractMetadataURI = _metadataURI;
        wepAppBaseURL = _wepAppBaseURL;
        _setDefaultRoyalty(address(this), (10 * _feeDenominator()) / 100); // 10%
    }

    function mint(SignatureValidator.MintPayload calldata voucher, bytes calldata signature)
        external
        payable
        _dateBounds(voucher.tokenId)
        whenNotPaused
    {
        require(isPublicSalesOpen == true, "Public sales are closed now");
        require(!_exists(voucher.tokenId), "The token already taken");
        _verifySignature(voucher, signature);

        require(msg.value >= voucher.price, "No enough funds");

        _safeMint(msg.sender, voucher.tokenId);

        BigDayTokens.BigDayToken storage token = _tokens[voucher.tokenId];
        token.covers.push(Covers.Cover(address(this), voucher.tokenId, 0, "", "", "", false));
        token.id = voucher.tokenId;
        token.tokenActiveCover = 0;
        token.attributes = BigDayTokens.Attributes(voucher.year, voucher.month, voucher.day, voucher.genId);
        mintRegistration();
        emit CoverAdded(voucher.tokenId, 0);
        _incrPriceMultiplier();
    }

    // TODO more complex payload

    // function privateMint(SignatureValidator.MintPayload calldata voucher)
    //     external
    //     _dateBounds(voucher.tokenId)
    //     onlyOwner
    // {
    ...
    function tokenURI(uint256 tokenId) public view override _isTokenExists(tokenId) returns (string memory) {
        return _tokens[tokenId].generateMetadataJSON(wepAppBaseURL);
    }
    ...
    function addCover(
        ...
        string memory label,
        string memory desc
    ) external _isTokenOwner(tokenId) {
        _tokens[tokenId].addCover(maxCoversAmount, collection, artId, align, label, desc, msg.sender);
        emit CoverAdded(tokenId, _tokens[tokenId].tokenActiveCover);
    }

    function getCovers(uint256 tokenId, bool showHidden) public view returns (Covers.Cover[] memory) {
        return _tokens[tokenId].getCovers(showHidden);
    }

    function getCover(uint256 tokenId, uint256 coverIndex) public view returns (Covers.Cover memory) {
        return _tokens[tokenId].getCover(coverIndex);
    }

    //TODO img URL video URL
    function updateImageUrl(
        uint256 tokenId,
        uint256 coverIndex,
        string memory _imageUrl
    ) external onlyOwner {
        _tokens[tokenId].updateMediaUrl(coverIndex, _imageUrl);
        emit CoverUpdated(tokenId, coverIndex);
    }

    function setCoversStates(uint256 tokenId, bool[] calldata _visibility) external _isTokenOwner(tokenId) {
        _tokens[tokenId].setCoversStates(_visibility);
        emit CoversStatesUpdated(tokenId);
    }

    function changeActiveCover(uint256 tokenId, uint256 newCoverIndex) external _isTokenOwner(tokenId) {
        uint256 oldCoverIndex = _tokens[tokenId].tokenActiveCover;
        _tokens[tokenId].changeActiveCover(newCoverIndex);
        emit ActiveCoverChanged(tokenId, oldCoverIndex, newCoverIndex);
    }
    ...
    function setWebAppURL(string memory _wepAppBaseURL) external onlyOwner {
        wepAppBaseURL = _wepAppBaseURL;
    }

    function setPublicSales(bool _value) public onlyOwner {
        isPublicSalesOpen = _value;
    }

    function setMaxCoversAmount(uint256 _maxCoversAmount) external onlyOwner {
        maxCoversAmount = _maxCoversAmount;
    }
...
contract BoundCurve {
    uint256 constant PENDING_PERIOD = 100;
    mapping(address => uint256) private _shareTokensBalances;
    mapping(address => uint256) private _buyerFullReleaseMintId;
    uint256 private _shareTokensTotalSupply = 0;
    uint256 private _mintsCounter = 1000; //TODO increment
    ...
    function mintRegistration() internal {
        //buyer ballance
        _shareTokensBalances[msg.sender] += 100;

        //service fee 20%
        _shareTokensBalances[feeReceiver] += 25;

        //total supply
        _shareTokensTotalSupply += 125;

        //full release "date"
        //if balance
        _buyerFullReleaseMintId[msg.sender] = _mintsCounter + PENDING_PERIOD + _getUnrealisedShares();

        // mint increment
        _mintsCounter += 1;
    }

    function _getUnrealisedShares() internal view returns (uint256) {
        if (_buyerFullReleaseMintId[msg.sender] <= _mintsCounter) {
            return 0;
        } else {
            return _buyerFullReleaseMintId[msg.sender] - _mintsCounter;
        }
    }

    function _getAvailableShares() internal view returns (uint256) {
        if (_shareTokensBalances[msg.sender] == 0) return 0;
        if (_buyerFullReleaseMintId[msg.sender] <= _mintsCounter || msg.sender == feeReceiver) {
            return _shareTokensBalances[msg.sender];
        } else if (_shareTokensBalances[msg.sender] < _buyerFullReleaseMintId[msg.sender] - _mintsCounter) {
            return 0;
        } else {
            uint256 mintsToFullRelease = _buyerFullReleaseMintId[msg.sender] - _mintsCounter;
            return (_shareTokensBalances[msg.sender] - mintsToFullRelease);
        }
    }

    function withdrawShare() external {
        //TODO redant
        (uint256 availableShares, uint256 availableEths, , , ) = getShareInfo();
        require(availableShares > 0, "Nothing to withdraw");
        _shareTokensBalances[msg.sender] -= availableShares;
        _shareTokensTotalSupply -= availableShares;

        (bool sent, ) = msg.sender.call{ value: availableEths, gas: 20317 }("");
    ...
    function getShareInfo()
        ...
        returns (
            ...
            uint256 availableEths,
            uint256 totalShares,
            uint256 totalEths,
            uint256 currentMint
        )
    {
        availableShares = _getAvailableShares();
        availableEths = (availableShares * address(this).balance) / _shareTokensTotalSupply;
        totalShares = _shareTokensTotalSupply;
        totalEths = address(this).balance;
        currentMint = _mintsCounter;
    }
}
...
contract SignatureValidator is EIP712 {
    using Address for address;
    uint256 constant voucherLifetime = 1 hours;
    ...
    bytes32 private constant PERMIT_SIGNATURE =
        keccak256(
            "EIP712(uint256 tokenId,string year,string month,string day,string genId,uint256 price,uint ts,bytes32 nonce)"
        );

    constructor(string memory name) EIP712(name, "1.0.0") {}

    function _verifySignature(MintPayload calldata voucher, bytes calldata signature) internal {
        require(!_expired[voucher.nonce], "SignatureValidator: Expired signature");
        _expired[voucher.nonce] = true;
        require(block.timestamp < voucher.ts || (block.timestamp - voucher.ts) < voucherLifetime, "Old signature");
        bool isVerified = _verify(msg.sender, _hash(voucher), signature);
        require(isVerified, "SignatureValidator: Invalid signature");
    }

    function _hash(MintPayload calldata voucher) private view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        PERMIT_SIGNATURE,
                        voucher.tokenId,
                        keccak256(bytes(voucher.year)),
                        keccak256(bytes(voucher.month)),
                        keccak256(bytes(voucher.day)),
                        keccak256(bytes(voucher.genId)),
                        voucher.price,
                        voucher.ts,
                        voucher.nonce
                    )
...
abstract contract TokenAsDate {
    ...
    uint256 private _lowTimeWindow = 719528; // the epoch day
    uint256 private _highTimeWindow = 719528; // the epoch day
    uint256 private constant _HALF_MAX_DAYS = 5_000_000; // 2740 years
    uint256 private _priceBase = 10**16; // 0.01Eth
    uint256 private _basePriceStep = 10**15; // 0.001Eth. 1 eth for 1000 purchases
    Counters.Counter private _basePriceStepMultiplier;

    modifier _dateBounds(uint256 day) {
        // todo bounds by epoch
        require(
            day >= 1 &&
                day <= _HALF_MAX_DAYS * 2 &&
                day >= _HALF_MAX_DAYS - _lowTimeWindow &&
                day <= _HALF_MAX_DAYS + _highTimeWindow,
            "Out of date bounds"
        );
        _;
    }
    ...
    function _getPrice() internal view returns (uint256) {
        return _priceBase + (_basePriceStep * _basePriceStepMultiplier.current());
    }

    function _incrPriceMultiplier() internal {
        _basePriceStepMultiplier.increment();
    }

    function setTimeWindow(uint256 ltw, uint256 htw) public {
        require(ltw < _HALF_MAX_DAYS && htw < _HALF_MAX_DAYS, "Time window should be less max window");
        _lowTimeWindow = ltw;
        _highTimeWindow = htw;
    }
}
...
library BigDayTokens {
    ...
    modifier _isValidCoverIndex(BigDayToken storage self, uint256 coverIndex) {
        require(coverIndex < self.covers.length, "Wrong cover index");
        _;
    }

    function addCover(
        BigDayToken storage self,
        uint256 _maxCoversAmount,
        address _collection,
        uint256 _artId,
        uint8 _align,
        ...
        string memory _desc,
        address _caller
    ) public returns (uint256 coverIndex) {
        IERC721 collContract = IERC721(_collection);
        address _owner = collContract.ownerOf(_artId);
        require(_owner == _caller, "BDToken: sender have to be an owner");
        Covers.Cover memory cover = Covers.Cover({
        ...
        if (self.covers.length >= _maxCoversAmount) {
            // save the first cover and shift others
            for (uint256 i = 1; i < self.covers.length; i++) {
                self.covers[i] = self.covers[i + 1];
            }
            self.covers.pop();
        }
        coverIndex = self.covers.length - 1;
        self.tokenActiveCover = coverIndex;
    }

    function getCovers(BigDayToken storage self, bool includeHidden) external view returns (Covers.Cover[] memory) {
        uint256 coversAmount = 0;
        uint256 totalCovers = self.covers.length;

        if (includeHidden) {
            coversAmount = totalCovers;
        } else {
            for (uint256 i = 0; i < totalCovers; i++) {
                if (!self.covers[i].hidden) {
                    coversAmount++;
                }
            }
        }
        Covers.Cover[] memory results = new Covers.Cover[](coversAmount);
        coversAmount = 0;
        if (includeHidden) {
            results = self.covers;
        } else {
            for (uint256 i = 0; i < totalCovers; i++) {
                if (!self.covers[i].hidden) {
                    results[coversAmount++] = self.covers[i];
                }
            }
        }
        return results;
    ...
    function getCover(BigDayToken storage self, uint256 coverIndex) external view returns (Covers.Cover memory cover) {
        uint256 coverToShow = coverIndex == 0 ? self.tokenActiveCover : coverIndex;
        cover = self.covers[coverToShow];
    }
    ...
    function updateMediaUrl(
        ...
        uint256 coverIndex,
        string memory _imageUrl
    ) public _isValidCoverIndex(self, coverIndex) {
        self.covers[coverIndex].imageUrl = _imageUrl;
    }

    function changeActiveCover(BigDayToken storage self, uint256 newIndex) internal _isValidCoverIndex(self, newIndex) {
        require(!self.covers[newIndex].hidden, "Hidden cover could not set as active");

        if (self.tokenActiveCover != newIndex) {
            self.tokenActiveCover = newIndex;
        }
    }

    function setCoversStates(BigDayToken storage self, bool[] calldata _visibility) external {
        require(_visibility.length == self.covers.length - 1, "Wrong array length");
        require(
            self.tokenActiveCover == 0 || _visibility[self.tokenActiveCover - 1] == true,
            "Active cover could not be hidden"
        );
        uint256 amount = _visibility.length;
        for (uint256 i = 0; i < amount; i++) {
            self.covers[i + 1].hidden = !_visibility[i];
        }
    }
        ...
        returns (string memory)
    {
        ...
        Covers.Cover storage activeCover = self.covers[self.tokenActiveCover];
        writer = writer.writeStartObject();
        writer = writer.writeStringProperty("name", activeCover.label);
        writer = writer.writeStringProperty("description", activeCover.desc);
        writer = writer.writeStringProperty("external_url", _webAppUrl);
        writer = writer.writeStringProperty("image", activeCover.imageUrl);
        ...
        returns (JsonWriter.Json memory writer)
    {
        ...
        self._addNumberAttribute(writer, "Covers", self.covers.length);
