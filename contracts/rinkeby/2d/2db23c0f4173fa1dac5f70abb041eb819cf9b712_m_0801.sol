...
//metadata
    ...
    mapping(address => mapping(uint=>string))public IDList;
    ...
    constructor() ERC721("m_0801" , "m_0801" ) {
    }
    ...
    function _afterTokenTransfer(
        ...
        if(from != address(0))
        {
            IDList[from][balanceOf(from)] = "-1";
        }
        if(to != address(0))
        {
            IDList[to][balanceOf(to)] = su.uint2str(tokenId);
        }
    }
...
//tokenIDが0じゃなくなるまで回せば取得できる
    function getTokenID(uint _index,address _address)public view returns(string memory){
