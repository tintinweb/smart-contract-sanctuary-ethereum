...
// File: contracts/OnChainDemoContract.sol
...
contract OnChainDemoContract is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  mapping(uint8 => bytes) private images;
  mapping(uint8 => bytes) private metadatas;
  
  constructor() ERC721("Awesome Words", "AW") {}

  function addImageData(uint8 index, bytes memory _svgData) external onlyOwner 
  {
        images[index] = _svgData;
  }

    function addMetaData(uint8 index, bytes memory _metaData) external onlyOwner 
  {
        metadatas[index] = _metaData;
  }

  function maxSupply() public view returns(uint) 
  {
      return uint(wordsValues.length);
  }

  function mint() public payable {
    uint256 supply = totalSupply();
    require(supply + 1 <= wordsValues.length);
