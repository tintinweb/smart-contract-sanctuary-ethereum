...
import { TraderStorageV1, Funding } from "./storage/TraderStorage.sol";

// never inherit any new stateful contract. never change the orders of parent stateful contracts
contract Trader is ITrader, BlockContext, ClearingHouseCallee, TraderStorageV1 {
    ...
    function initialize(
        ...
        // T_LPNC: LiquidityProvider is not contract
        require(liquidityProviderArg.isContract(), "T_LPNC");
        // T_CHNC: ClearingHouse is not contract
        require(clearingHouseConfigArg.isContract(), "T_CHNC");
    ...
    function setPositionMgmt(address positionMgmtArg) external onlyOwner {
        // T_PMNC: PositionMgmt is not contract
        require(positionMgmtArg.isContract(), "T_PMNC");
    ...
    function swap(SwapParams memory params) external override returns (SwapResponse memory) {
        ...
        int256 traderPositionSize = IPositionMgmt(_positionMgmt).getTraderPositionSize(params.trader, params.baseToken);

        // when traderPositionSize < 0, it's a short position
        // bool isReducingPosition = traderPositionSize == 0 ? false : traderPositionSize < 0 != params.isBaseToQuote;
        bool isReducingPosition = traderPositionSize == 0
            ? false
        ...
        if (params.isClose && traderPositionSize != 0) {
            ...
            if (
                ...
                // T_AOPLO: already over price limit once
                require(timestamp != _lastOverPriceLimitTimestampMap[params.trader][params.baseToken], "T_AOPLO");
            ...
            if (isReducingPosition) {
                // T_AOPLO: over price limit before swap
                require(
        ...
        int256 oldTraderOpenNotional = IPositionMgmt(_positionMgmt).getTraderOpenNotional(
            params.trader,
            params.baseToken
        );
        ...
        if (isReducingPosition) {
            pnlToBeRealized = _getPnlToBeRealized(
                InternalRealizePnlParams({
                    ...
                    traderPositionSize: traderPositionSize,
                    traderOpenNotional: oldTraderOpenNotional,
                    base: response.base,
    ...
    {
        // T_BTNE: base token does not exists
        require(IPoolRegistry(_poolRegistry).hasPool(baseToken), "T_BTNE");
    ...
    function getEstimateAmountLimit(
        ...
        if (isBaseToQuote) {
            if (!isExactInput) {
                uint256 baseBalance = IAmm(pool).getBalances(1);
                // T_IBA: insufficient base token amount
                require(amount < baseBalance, "T_IBA");
            }
            dy = IAmm(pool).getDy(1, 0, amount, isExactInput);
        } else {
            if (!isExactInput) {
                uint256 quoteBalance = IAmm(pool).getBalances(0);
                // T_IQA: insufficient quote token amount
                require(amount < quoteBalance, "T_IQA");
            }
            dy = IAmm(pool).getDy(0, 1, amount, isExactInput);
        }
    ...
    function getPnlToBeRealized(RealizePnlParams memory params) external view override returns (int256) {
        Account.TraderInfo memory info = IPositionMgmt(_positionMgmt).getAccountInfo(params.trader, params.baseToken);

        int256 traderOpenNotional = info.traderOpenNotional;
        int256 traderPositionSize = info.traderPositionSize;
        // when traderPositionSize < 0, it's a short position; when base < 0, isBaseToQuote(shorting)
        bool isReducingPosition = traderPositionSize == 0 ? false : traderPositionSize < 0 != params.base < 0;
            ...
            isReducingPosition
                ? _getPnlToBeRealized(
                    InternalRealizePnlParams({
                        ...
                        baseToken: params.baseToken,
                        traderPositionSize: traderPositionSize,
                        traderOpenNotional: traderOpenNotional,
                        base: params.base,
                        quote: params.quote
                    })
                )
    ...
    function getPendingFundingPayment(address trader, address baseToken) public view override returns (int256) {
        ...
        int256 liquidityCoefficientInFundingPayment = ILiquidityProvider(_liquidityProvider)
            .getLiquidityCoefficientInFundingPayment(trader, baseToken, fundingGrowthGlobal);
    ...
    function getTwapMarkPrice(address baseToken, uint32 twapInterval) public view override returns (uint256) {
        return IAmm(IPoolRegistry(_poolRegistry).getPool(baseToken)).getTwapMarkPrice(twapInterval);
    }
    ...
    function _swap(SwapParams memory params) internal returns (InternalSwapResponse memory) {
        ...
        uint24 exchangeFeeRatio = IPoolRegistry(_poolRegistry).getFeeRatio(params.baseToken);
        uint256 exchangeFee;

        if (params.isBaseToQuote) {
            uint256 quoteAmount = IAmm(pool).swap(1, 0, params.amount, params.amountLimit, params.isExactInput);
            ...
            uint256 quoteAmount = IAmm(pool).swap(0, 1, params.amount, params.amountLimit, params.isExactInput);
        ...
        // update the timestamp of the first tx in this pool
        if (_firstTradedTimestampMap[params.baseToken] == 0) {
            _firstTradedTimestampMap[params.baseToken] = _blockTimestamp();
        }

        uint24 insuranceFundFeeRatio = IPoolRegistry(_poolRegistry).getPoolInfo(params.baseToken).insuranceFundFeeRatio;
        uint256 insuranceFundFee = exchangeFee.mulRatio(insuranceFundFeeRatio);

        // save exchangeFee/totalLiquidity ratio in one tx
        uint256 totalLiquidity = IAmm(pool).getTotalLiquidity();
        // use Q96 precision to avoid in calculation result error
        uint256 feeRatio = ((exchangeFee - insuranceFundFee) * Constant.IQ96) / totalLiquidity;
        ILiquidityProvider(_liquidityProvider).updateExChangeFee(params.baseToken, feeRatio);
    ...
    /// @return pendingFundingPayment the pending funding payment of a trader in one pool,
    ...
    function _isOverPriceLimit(
        ...
        uint256 priceAfter;
        if (isBaseToQuote) {
            (priceAfter, , ) = IAmm(pool).quoteSwap(1, 0, amount, isExactInput);
        } else {
            (priceAfter, , ) = IAmm(pool).quoteSwap(0, 1, amount, isExactInput);
        }

        uint256 currentPrice = IAmm(pool).getPriceCurrent();
        ...
        )
    {
        ...
        if (timestamp == lastSettledTimestamp || lastSettledTimestamp == 0) {
            ...
            uint256 totalLiquidity = IAmm(pool).getTotalLiquidity();
            if (totalLiquidity != 0) {
                uint256 balanceBase = IAmm(pool).getBalances(1);
                fundingGrowthGlobal.twPremiumWithLiquidity =
                    lastFundingGrowthGlobal.twPremiumWithLiquidity +
                    PerpMath.mulDiv(balanceBase.toInt256(), deltaTwPremium, totalLiquidity);
            }
        }
    ...
    function _getDeltaTwap(uint256 markTwap, uint256 indexTwap) internal view returns (int256 deltaTwap) {
        uint24 maxFundingRatio = IClearingHouseConfig(_clearingHouseConfig).getMaxFundingRatio();
        uint256 maxDeltaTwap = indexTwap.mulRatio(maxFundingRatio);
    ...
    function _getPnlToBeRealized(InternalRealizePnlParams memory params) internal pure returns (int256) {
        ...
        uint256 closedRatio = FullMath.mulDiv(params.base.abs(), _FULLY_CLOSED_RATIO, params.traderPositionSize.abs());
        ...
        if (closedRatio <= _FULLY_CLOSED_RATIO) {
            ...
            int256 reducedOpenNotional = params.traderOpenNotional.mulDiv(closedRatio.toInt256(), _FULLY_CLOSED_RATIO);
            ...
            pnlToBeRealized = params.traderOpenNotional + closedPositionNotional;
        }
...
/// @notice For future upgrades, do not change TraderStorageV1. Create a new
/// contract which implements TraderStorageV1 and following the naming convention
/// TraderStorageVX.
abstract contract TraderStorageV1 {
