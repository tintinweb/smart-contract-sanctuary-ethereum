...
contract ERC721Upgradeable is Initializable, ContextUpgradeable, ERC165Upgradeable, IERC721Upgradeable, IERC721MetadataUpgradeable {
    ...
    mapping(address => mapping(uint256 => Rent)) public _NFTRent;
    ...
    mapping(uint256 => Borrower) public borrowdetails;
    /**
    ...
    function _transfer(
        ...
        require(_NFTRent[from][tokenId].status != true, "This NFT not Available this Time because this is Renting NFT");
    ...
    ERC721BurnableUpgradeable
{
    ...
    function initialize() public initializer  {
        __Ownable_init();
        ERC721Upgradeable.__ERC721_init("Milage trade center", "MILAGETECHLLC");
        _tid = 1;
    ...
    function mint(
        ...
        _tid = _tid.add(1);
         ...
         if(keccak256(abi.encodePacked((_type))) != keccak256(abi.encodePacked(("Rent")))){
             ...
             require(value > 0, "Price must greater than Zero");
             borrowOrder(id_, value);
         }
         _royal[id_]=royal*1e18;
        totalQuantity[id_] = supply;
        emit TokenId(msg.sender, id_);
    }
    function rentNFT(uint256 tokenId,uint256 _days,address payable from) public payable{
        uint256 val = pERCent(_NFTRent[from][tokenId].price.mul(_days), serviceValue).add(_NFTRent[from][tokenId].price.mul(_days));
        require(balances[tokenId][from] > 0, "Is Not a Owner");
        require(_NFTRent[from][tokenId].status == true && val == msg.value, "Invalied Borrow Price");
        require(borrowdetails[tokenId].borrowingTime.add(borrowdetails[tokenId].time.mul(1 days)) <= block.timestamp, "Previous Borrow is not completed");
        Borrower memory borrow;
        borrow.tokenId = tokenId;
        borrow.status = true;
        borrow.owner = from;
        borrow.borrower = msg.sender;
        borrow.price = _NFTRent[from][tokenId].price.mul(_days);
        borrow.time = _days;
        borrow.borrowingTime = block.timestamp;
        borrowdetails[tokenId] = borrow;
        address payable admin = payable(owner());
        (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(_NFTRent[from][tokenId].price.mul(_days), 0, serviceValue, sellervalue);
    ...
    function borrowOrder(uint256 tokenId, uint256 _price) public{
        require(_price > 0, "Price must greater than Zero");
        require(balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        Rent memory rent;
        rent.status = true;
        rent.tenantAddress = msg.sender;
        rent.tokenId = tokenId;
        rent.price = _price;
        _NFTRent[msg.sender][tokenId] = rent;
    }
    function claimRentalNFT(uint256 tokenId,string memory meta) public{
        require(balances[tokenId][msg.sender] > 0, "Is Not a Owner");
        require(_NFTRent[msg.sender][tokenId].status == true,"This NFT not a Rental NFT");
        delete _NFTRent[msg.sender][tokenId];
        delete borrowdetails[tokenId];
