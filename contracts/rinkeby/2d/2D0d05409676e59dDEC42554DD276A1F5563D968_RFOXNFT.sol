    ...
    Ownable
{
    ...
    // How many NFTs can be minted in total during presale
    ...
    event WhitelistSettings(
        address indexed sender,
        uint256 maxSupplyPresale,
     ...
     *
     * @param _maxSupplyPresale how many token can be minted during the presale
     * @param _maxMintedPresalePerAddress how many token can be minted per address during the presale
     */
    function settingWhitelist(uint256 _maxSupplyPresale, uint256 _maxMintedPresalePerAddress) external onlyOwner {
        require(_maxSupplyPresale <= MAX_NFT, "Invalid max supply presale");
        ...
        emit WhitelistSettings(msg.sender, _maxSupplyPresale, _maxMintedPresalePerAddress);
    }
        ...
        authorizePresale(proof)
    {
        ...
        require(
            totalSupply().add(tokensNumber) <= maxSupplyPresale,
            "All Presale token is minted"
        );
    ...
    function _buyNFTs(uint256 tokensNumber) private tokenInSupply(tokensNumber) {
        if (saleEndTime > 0)
