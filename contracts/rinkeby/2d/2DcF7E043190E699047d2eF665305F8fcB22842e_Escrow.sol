...
contract Escrow is Ownable {
    ...
    mapping(address => uint) public allAccounts;
    mapping(uint => address) public accountMapperCounter;
    ...
    mapping(uint => uint) public VestingSchedulesRemained;
    ...
    constructor(address _token, uint[] memory _times) {
        require(_times.length == 5, 'not equal to 5 times');
        token = _token;
    ...
    function getVestingSchedule(uint index) external view returns (uint) {
    ...
    function vest() external onlyOwner {
        for(uint i=0; i < countSchedules; i++){
            if  (block.timestamp < VestingSchedulesRemained[i] || VestingSchedulesRemained[i] == 0) {
                continue;
            }
            for (uint j = 1; j <= accountsCount; j++) {
                ...
                require(IERC20(token).transfer(account, amount), 'Not enough balance on contract');
