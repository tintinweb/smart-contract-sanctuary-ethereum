...
// write functions:
    ...
    */
    ...
    function setCliffAndInstallments(tokenStruct memory _tok, uint _cliffMonths, uint _installmentMonths, uint _days) public onlyOwner{
        cliffMonthPeriod[_tok.holder] = block.timestamp + (_cliffMonths * 1 minutes);
        installmentBalPerMonth[_tok.holder] = individualBalanceOfHolder[_tok.holder] / _installmentMonths;
        installmentMonthsForTokenHolder[_tok.holder] = _installmentMonths;
        for(uint i = 0; i < _installmentMonths; i++){
            vestingDues[_tok.holder]._date[i+1] = block.timestamp + ((i+1) * _days * 2 minutes);
            vestingDues[_tok.holder]._fund[i+1] = installmentBalPerMonth[_tok.holder];
        }
    }
    ...
    */
    ...
    function withdrawInstallment(tokenStruct memory _tok, uint _index, bytes32 _symbol) public {
        uint amt;
        require(msg.sender == _tok.holder,"The connected wallet is not an investor wallet");
        if(block.timestamp >= cliffMonthPeriod[_tok.holder]){
            for(uint i = 0; i < tokenHolders.length; i++){
                if(tokenHolders[i] == _tok.holder){
                    installmentsCollected[_tok.holder] += 1;
                    if(block.timestamp >= vestingDues[_tok.holder]._date[_index]){
                        amt = vestingDues[_tok.holder]._fund[_index];
                        ERC20(whitelistedTokens[_symbol]).transfer(_tok.holder, amt);
                        individualBalanceOfHolder[_tok.holder] -= amt;
                        totalContractBalance -= amt;
                        holderRewithdrawTokens[_tok.holder] += amt;
                    }else{
                        revert("The timelock is not unlocked yet");
                    }
                }
            }
        }else{
            revert("The cliff month period is not yet reached");
        }
    }
...
// Read Functions:
    ...
    */
    ...
    function returnTokensWithdrawStatus(tokenStruct memory _tok) public view returns(
        ...
        installmentMonthsForTokenHolder[_tok.holder]
        );
    }
}