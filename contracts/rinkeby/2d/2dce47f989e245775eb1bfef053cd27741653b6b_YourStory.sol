...
//
//  "The rightful owner of this NFT owns all copyright that may exist in the image and text depicted
//   in the corresponding SVG file, subject to the rights of owners of other NFTs minted under the
//   same smart contract. Any grant or assignment of exclusive rights in such copyright shall be null
//   and void except in connection with transfer of the NFT. Using a private key to sign a transaction
//   that transfers this NFT constitutes a writing signed by the transferor assigning all such copyright
//   to the transferee."
//========================================================================
...
contract YourStory is WordTable, AddressShifter, ERC721Royalty, Ownable {
    ...
    mapping(uint256 => uint8) private tokenIdToOffset;
    mapping(address => uint256) public mintCount;

    constructor(address _owner) ERC721("Your Story", "STORY") {
    ...
    function mintTo(address recipient) public payable returns (uint256) {
        ...
        tokenIdToOffset[newTokenId] = uint8(newMintCount - 1);
    ...
    function mintManyTo(address recipient, uint256 amt)
        external
        payable
        returns (uint256[] memory)
    {
        if (msg.value < mintPrice(amt)) {
            revert MintPriceNotPaid();
        }
        ...
        // check minters individual max
        uint256 currentMintCount = mintCount[msg.sender];
        uint256 newMintCount = mintCount[msg.sender] + amt;
        if (newMintCount > MAX_MINTS) {
            revert MaxAddressMints();
        }
        mintCount[msg.sender] = newMintCount;
        ...
        for (uint256 i = 0; i < amt; i++) {
            ...
            tokenIdToMintSeed[tokenId] = bytes20(msg.sender);
            tokenIdToOffset[tokenId] = uint8(currentMintCount);
    ...
    function getTokenSeed(uint256 tokenId) external view returns (address) {
        ...
        returns (string memory)
    {
        ...
        bytes20 tokenSeed = shiftBytes20Address(
            mintSeed,
            tokenIdToOffset[tokenId]
        );

        string[2] memory colors = seedToColorStringsOp(tokenSeed);
        ...
        string memory json = Base64.encode(
            bytes(
                string.concat(
                    ...
                    '", "description": "An on-chain story generated from your address. \\n\\nThe rightful owner of this NFT owns all copyright that may exist in the image and text depicted in the corresponding SVG file, subject to the rights of owners of other NFTs minted under the same smart contract. Any grant or assignment of exclusive rights in such copyright shall be null and void except in connection with transfer of the NFT. Using a private key to sign a transaction that transfers this NFT constitutes a writing signed by the transferor assigning all such copyright to the transferee.',
