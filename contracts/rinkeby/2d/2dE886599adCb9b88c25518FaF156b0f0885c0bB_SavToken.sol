...
contract SavToken is Ownable{
  ...
  function burnAndBuy(uint amount, string memory _data, uint256 _signatureId, bytes memory _signature) external{
    ...
    require(checkSignature(account, amount, _signatureId, _signature) == signAddress, "Signature error : bad owner");
  ...
  function checkSignature(address _wallet, uint256 _amount, uint256 _signatureId, bytes memory _signature) public pure returns(address){
        return ECDSA.recover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encode(_wallet, _amount, _signatureId)))), _signature);
    }
}