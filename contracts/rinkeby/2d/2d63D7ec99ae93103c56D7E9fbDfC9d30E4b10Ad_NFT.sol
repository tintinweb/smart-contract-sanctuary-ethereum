...
contract NFT is ERC721, EIP712Whitelist {
        ...
        external
    {
        ...
        * - The contract is constructed to optimize gas.
        */
        uint256 reserved = _freeMinter[msg.sender];
        require(reserved > 0, "No free mint allowed for your address");
        require(num <= reserved, "Can't mint more than reserved");
        require(_tokenIdCounter.current() + num <= maxSupply - reserved, "Exceeds maximum token supply" );
        if (genesisPhase) {
            require(_tokenIdCounter.current() + num <= genesisSupply, "Exceeds genesis token supply" );
        }
        _freeMinter[msg.sender] = reserved - num;
