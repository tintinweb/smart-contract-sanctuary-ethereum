...
contract LaTableDuChefBooking is ERC1155Supply, Ownable, Pausable {
    ...
    function publicMint(
        ...
        unchecked {
            require(msg.value >= _priceByTokenId[id] * uint16(count), "!ether");
        }
        mint(to, id, count);
    }
    ...
    function mint(
        ...
        unchecked {
            require(
                totalSupply(id) + uint16(count) <= _maxSupplyByTokenId[id],
                "!supply"
            );
        }
        _mint(to, id, uint16(count), new bytes(0));
    }
