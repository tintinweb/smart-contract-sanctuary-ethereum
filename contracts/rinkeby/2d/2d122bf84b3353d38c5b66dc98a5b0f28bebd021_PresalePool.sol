...
contract PresalePool is OwnableUpgradeable, IPool , ReEntrancyGuard {
    ...
    mapping(address => bool) public haveInsurance;
    ...
    mapping(address => bool) private isInsurance;
    ...
    receive() external payable {
        if (msg.value > 0) contribute(false);
    }
    ...
    function contribute(bool _insurance) public payable inProgress onlyWhitelisted{
        require(msg.value > 0, "Cant contribute 0");
        uint256 amount = msg.value;
        if(_insurance){
            (uint256 _rate ,) = IPoolFactory(factory).getUSDRate();
            uint256 insurancePrice = IPoolFactory(factory).getInsurancePrice();
            uint256 paidAmount = insurancePrice.div(100).mul(_rate).div(10**18);
            IPoolFactory(factory).buyInsurance(msg.sender, address(this) , paidAmount );
            amount = amount.sub(paidAmount);
            isInsurance[msg.sender] = true;
        }
