...
contract NFT is ERC721, Ownable {
    ...
    function _setNFT(uint256 nftId, string memory URI,string memory n,string memory des,uint p) internal {
        _tokenURIs[nftId] = URI;
        _names[nftId]=n;
        _descs[nftId]=des;
        _price[nftId]=p;
    ...
    function getName(uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId),"Name not exist on that ID");
        string memory _Nam =  _names[tokenId];
        return _Nam;
    }
    function getPrice(uint256 tokenId) public view returns (uint) {
        require(_exists(tokenId),"Price not exist on that ID");
        uint _p = _price[tokenId];
        return _p;
    }
    function getDescription(uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId),"Description not exist on that ID");
        string memory _Des =  _descs[tokenId];
        return _Des;
    }
    function getAlltoken() public view returns (RenderToken[] memory){
        ...
        for(uint256 i = 0; i  <= latestId ; i++){
            if(_exists(i)){
                ...
                string memory n= getName(i);
                string memory d= getDescription(i);
                uint p=getPrice(i);
                res[i] = RenderToken(i,n,uri,d,p);
            }
        }
        return res;
    }
