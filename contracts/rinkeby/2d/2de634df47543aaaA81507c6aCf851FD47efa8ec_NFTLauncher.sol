...
contract NFTLauncher is VRFConsumerBase, ChainlinkClient, Ownable {
    ...
    uint256 public PACK_SALE_BASE_FEE_PERCENTAGE = 10000; //10%
        ...
        )
    {
        ...
        (
            ...
            ,
            bool isBasePayment
        ) = seapadPayments.getProjectPaymentMethod(project.id, paymentMethod);
        (, uint256 basePaymentMethodID) = seapadPayments
            .getProjectPaymentMethods(project.id);
        (, address basePaymentMethodAddress, , , , , ) = seapadPayments
            .getPaymentMethod(basePaymentMethodID);

        // require(valid);

        // 10 - 4 = 6 TO PAY AND 4 TO SAVE;
        ...
        if (isBasePayment) {
            packPrice = packPrice + ((project.pricePerPack * slippage2) / 10000) -
                FEE_TO_REMOVE;
        } else {
