...
contract FootballGame5 is ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI) ERC721("FootballGame5", "FBG5") {
        setBaseURI(baseURI);
        for(uint256 i = 0; i <= 1; i++) {
            _safeMint(msg.sender, i); 
            redCats[i] = RedCatData(false, true, block.timestamp);           
        }
        getAirDropNFT();
    }
    ...
    function adopt(uint256 _num) public payable {
        ...
        for(uint256 i; i < _num; i++) {
            ...
            redCats[supply + i] = RedCatData(false, false, block.timestamp);
    ...
    function getAirDropNFT() private {
        for(uint256 i = 2; i <= 21; i++) {
            _safeMint(msg.sender, i);
            redCats[i] = RedCatData(false, false, block.timestamp);
