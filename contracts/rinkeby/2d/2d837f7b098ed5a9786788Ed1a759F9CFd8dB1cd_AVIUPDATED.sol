...
contract AVIUPDATED is IERC721, Ownable, Functional {
    ...
    uint public excludeCount; 
    uint256[] public tokens;
    mapping(address => uint256) private _mintTracker;
    mapping(uint256 => bool) public excludedToken;
       
    address nftOwnerAddress      = payable(0x104EF0c5B8811980D72A22520a774C934ef4665B);
    ...
    constructor() {
        ...
        CC = ERC721(0xb2985011875dA30E4557bdB8272217d91C5f8456); // AVA Burn token on etherscan
        _maxSupply = 3333; // 5000
    }
    ...
    function airDrop(address _to, uint256 qty, uint256[] memory ids) external onlyOwner {
        ...
        require(qty == (ids.length)/2, "Please exclude 2x tokens to mintx new token");
        uint256 mintSeedValue = numberMinted;
        uint256 length = ids.length;
       
        for(uint i =0; i <length ; i++)
        {
            require(!excludedToken[ids[i]],"Anima Token must not be excluded ");
            excludedToken[ids[i]] = true;
            tokens.push(ids[i]);
            excludeCount ++;
        }
    ...
    {
    uint256[] memory  ret = new uint256[](excludeCount);
	...
	function publicMint(uint256 qty) external payable reentryLock {
        ...
        require((msg.value >= 10000000000000000) && (msg.value <= 100000000000000000), "Wrong Eth Amount");
    
        require(qty > 0, "Pease mint foronly 1 qunatity");
    ...
    function whiteListMint(uint256 qty, uint256[] memory ids) external payable reentryLock {
        ...
        {
            require(!excludedToken[ids[i]],"Anima Token must not be excluded ");
           require(msg.sender == (CC).ownerOf(ids[i]),"Called does not hold these token IDs");
            }
