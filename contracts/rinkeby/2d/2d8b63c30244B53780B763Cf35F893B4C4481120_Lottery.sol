...
contract Lottery is VRFConsumerBaseV2, KeeperCompatibleInterface {
  ...
  function fulfillRandomWords(uint256, /* requestId */ uint256[] memory randomWords) internal override {
    ...
    s_winner = payable(s_participants[randomNumber]);
    s_participants = new address[](0);
    s_lastTimeStamp = block.timestamp;

    bool sent = s_winner.send(address(this).balance);
    if(!sent)
  ...
  function checkUpkeep(bytes memory /*checkData*/) public override view returns (bool upkeepNeeded, bytes memory) {
    ...
    bool upkeepNeeded = ((block.timestamp - s_lastTimeStamp) > i_interval) &&
  ...
  function performUpkeep(bytes calldata /*performData*/) external override {
    (bool checkUpkeepResult, ) = checkUpkeep("");
    if(!checkUpkeepResult)
