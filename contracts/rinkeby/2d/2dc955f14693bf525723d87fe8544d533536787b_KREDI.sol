...
interface UnitedPunksUnionPunks {
    function tokensOfOwner(address ownwer) external view returns (uint256[] memory);

}

contract KREDI is ERC20, ERC20Burnable, Ownable {

    address constant public UnitedPunksUnionAddress = 0xfb519Aa8C7cadDa1458929d21D97C31c39CABc30;
    ...
    function setSaleState(bool newState) public onlyOwner {
     ...
     function totalAvailable(address ownwer) public view returns (uint256) {
        uint[] memory punks = UnitedPunksUnionPunks(UnitedPunksUnionAddress).tokensOfOwner(ownwer);
    ...
    function claim() public payable  {
        ...
        uint[] memory punks = UnitedPunksUnionPunks(UnitedPunksUnionAddress).tokensOfOwner(msg.sender);
        uint arrayLength = punks.length;
    ...
    function reservedKREDI(address to, uint256 amount) public onlyOwner {
         require(claimed + amount <= MAX_CLAIM, "Value exceeds the max limit");
         _mint(to, amount);
         claimed += amount;
    }

    function unclaimedKREDI() public view onlyOwner returns(uint256) {
         uint256 unclaimed = MAX_CLAIM - claimed;
         return unclaimed;
    }

    function punkKREDI(uint punkid) public view returns(uint256) {
         uint256 rewards = 0;
         uint256 time = block.timestamp;
         uint256 lastTime = lastUpdate[punkid];
         if (lastTime > 0){
