...
contract MultiTrans is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    uint256 public fee = 0;
    address dai = 0xc7AD46e0b8a400Bb3C915120d284AafbA8fc4735;
    address receivewallet = 0x48530cdB906F4E023f97a58e293fF2E6DeE08820;
    
    /**
        Transfer one token, e.g. BTC, ETH, USDC
        From n addresses to n addresses with n values
    **/
    function transOneToken(
        address _token,
        address[] memory from, 
        address[] memory to,
    ...
    /**
        Transfer n token, e.g. BTC, ETH, USDC
        From n addresses to n addresses with n values
    **/
    function transMultiToken(
        ...
        uint256 length = amount.length;
        for (uint256 i = 0; i < length; i++) {
            IERC20 tokenIns = IERC20(token[i]);
            tokenIns.safeTransferFrom(from[i], to[i], amount[i]);
        }
    }
       function transMultiNft(
        address[] memory token,
        address[] memory from, 
        address[] memory to,
        uint256[] memory id
        ...
        for (uint256 i = 0; i < id.length;i++) {
            IERC721 tokenIns = IERC721(token[i]);
