...
contract OxOkayBears is ERC721A, Ownable {
    ...
    uint public constant FIRSTXFREE = 800;
    ...
    string private _hiddenURI = "ipfs://";
    ...
    function calculateTotal(uint count) public view returns (uint total) {
        ...
        uint sum = _totalMinted() + count;         
        if(currentTokenId < 800) {
            if(sum <= 800){
                return 0 ether;
            }
        } else if (currentTokenId < 2000) {
            if(sum <= 2000){
                return count * 0.01 ether;
            }
            return (2000 - currentTokenId) * 0.01 ether + (sum -2000) * 0.02 ether;
        } else if (currentTokenId < 5000) {
            if(sum <= 5000){
                return count * 0.02 ether;
            }
            return (5000 - currentTokenId) * 0.02 ether + (sum - 5000) * 0.03 ether;
        } else if (currentTokenId < 9000) {
            if(sum <= 9000){
                return count * 0.03 ether;
            }
            return (9000 - currentTokenId) * 0.03 ether + (sum - 9000) * 0.035 ether;
        } else {
           return count * 0.035 ether;
        }
    }
    ...
    function mint(uint count) external payable {
        ...
        require(count <= TOKENS_PER_TRAN_LIMIT, "0xOkayBears: Number of requested tokens exceeds allowance (25)");
