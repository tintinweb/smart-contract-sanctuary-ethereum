...
contract Haroburu is ERC721Holder,ERC721A, Ownable {
    ...
    /**
    @notice Returns the length of time, in seconds, that the MoonHaroburusbird has
    nested.
    @dev Nesting is tied to a specific Haroburus, not to the owner, so it doesn't
    reset upon sale.
    @return nesting Whether the Haroburus is currently nesting. MAY be true with
    ...
    @return total Total period of time for which the Haroburu has nested across
    ...
    /**
    @dev Emitted when a Haroburu begins nesting.
     */
    event Nested(uint256 indexed tokenId);

    /**
    @dev Emitted when a Haroburu stops nesting; either through standard means or
    by expulsion.
     */
    ...
    /**
    @notice Changes the Haroburu's nesting status.
    */
    function toggleNesting(uint256 tokenId) public
    {
        uint256 start = nestingStarted[tokenId];
        if (start == 0) {
            require(nestingOpen, "Haroburus: nesting closed");
    ...
    /**
    @notice Changes the Haroburus' nesting statuss (what's the plural of status?
    statii? statuses? status? The plural of sheep is sheep; maybe it's also the
    plural of status).
    @dev Changes the Haroburus' nesting sheep (see @notice).
     */
