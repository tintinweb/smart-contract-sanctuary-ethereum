...
contract MOJI is Ownable, ERC721A, ReentrancyGuard {
    ...
    function lockTraits(uint256 lockSeriesNum) external onlyOwner {
        require(lockSeriesNum <= CurrentSeries, "can't lock future series");

        uint256 lockIndex = InitialStartingSize * ((2 ** lockSeriesNum) - 1); 
        require(traitsLockedIndex() < lockIndex,"can't unlock already locked items");
    ...
    function startNextSeries() external onlyOwner {
        ...
        CurrentSeries += 1;
        maxSupplyWithCurrentSeries = InitialStartingSize * ((2 ** CurrentSeries) - 1);
        currentSeriesSize = InitialStartingSize * (2 ** (CurrentSeries - 1));
