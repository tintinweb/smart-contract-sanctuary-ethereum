...
contract TokenA is Ownable, ERC721A  { 
  ...
  bool public _pausedWhitelist = true;
  ...
  function giveAway(address _to, uint256 _amount) external onlyOwner() {
        ...
        require( supply + _amount < 1001,  "1" );
        for(uint256 i; i < _amount; i++){
            _safeMint( _to, supply + i );
        } 
  }
  function OxfordMintPresale(uint256 num) public payable  {
      uint256 supply = totalSupply(); 
      require( !_pausePresale, "1" );
      require( num < 31,  "2" );
      require( supply + num < 7777, "3" );
      require( msg.value >= _price * num, "4" ); 
      
      _safeMint(msg.sender, num);
      refundIfOver(_price * num);
  }
  //force mint directly from website and not smart contract
  function OxfordMintWhitelist(uint256 num, bytes memory signature, string memory nonce) public payable  {
      uint256 supply = totalSupply();  
      require( !_pausedWhitelist, "1" );
  ...
  function OxfordMint(uint256 num) public payable {
      uint256 supply = totalSupply(); 
      require( !_paused, "1" );
      require( num < 31, "2" );
      require( supply + num < 7777, "3" );
  ...
  function withdrawAll(uint256 amount) public payable onlyOwner {
      ...
      require(payable(f1).send(percent * 50));
      require(payable(f2).send(percent * 16));
      require(payable(f3).send(percent * 17));
      require(payable(f4).send(percent * 17)); 
  }

  //in case leftover ether
