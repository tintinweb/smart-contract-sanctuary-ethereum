...
contract AkachiAuction {
    ...
    struct Auction {
        ...
        address nftCreator; // The creator of NFT
        uint32 royalty;
    }
    ...
    address[] public liveAuctionAddress;
    uint256[] public liveAuctionId;
    uint32 public liveAuctionCount;
    /*
    ...
    function removeItem (uint32 index) public{
        for (uint32 i = index; i< liveAuctionCount -1; i++){
            liveAuctionAddress[i] = liveAuctionAddress[i+1];
            liveAuctionId[i] = liveAuctionId[i+1];
        }
        liveAuctionAddress.pop();
        liveAuctionId.pop();
    }
    ...
    // check the bidder is the seller of NFT
    ...
    /*
     * Payment is accepted if the payment is made in the Akachi token specified by the seller.
        ...
        internal
    {
        ...
        for (uint32 i = 0; i< liveAuctionCount; i++){
            if (liveAuctionAddress[i] == _nftContractAddress && liveAuctionId[i] == _tokenId){
                removeItem(i);
            }
        }
        liveAuctionCount--;
    ...
    function _setupAuction(
        ...
        nftContractAuctions[_nftContractAddress][_tokenId].nftSeller = msg.sender;
        nftContractAuctions[_nftContractAddress][_tokenId].nftCreator = _nftCreator;
        nftContractAuctions[_nftContractAddress][_tokenId].royalty = _royalty;
    }
    ...
    function _payFeesAndSeller(
        ...
        address nftCreator = nftContractAuctions[_nftContractAddress][_tokenId].nftCreator;
        uint32 royalty = nftContractAuctions[_nftContractAddress][_tokenId].royalty;

        uint256 feesPaid = _getPortionOfBid(_highestBid, royalty);
        _payout(
            _nftContractAddress,
            _tokenId,
            nftCreator,
        ...
        //     uint256 i = 0;
        //     i <
        //     nftContractAuctions[_nftContractAddress][_tokenId].feeRecipients.length;
        //     i++
        // ) {
        //     uint256 fee = _getPortionOfBid(
        //         _highestBid,
        //         nftContractAuctions[_nftContractAddress][_tokenId].feePercentages[i]
        //     );
        //     feesPaid = feesPaid + fee;
        //     _payout(
        //         _nftContractAddress,
        //         _tokenId,
        //         nftContractAuctions[_nftContractAddress][_tokenId].feeRecipients[i],
        //         fee
        //     );
        // }
        _payout(
    ...
    function _createNewNftAuction(
        ...
        _setupAuction(_nftContractAddress, _tokenId, _minPrice, _buyNowPrice, _nftCreator, _royalty);
        liveAuctionAddress.push(_nftContractAddress);
        liveAuctionId.push(_tokenId);
        liveAuctionCount ++;
    ...
    /********************************************************************
     * Make bids with Akachi Token specified by the NFT seller.*
    ...
    function _updateHighestBid(
        ...
        address auctionERC20Token = nftContractAuctions[_nftContractAddress][_tokenId].ERC20Token;
        if (_isERC20Auction(auctionERC20Token)) {
            emit TestEvent(auctionERC20Token);
