...
contract Estrella is ERC721,Ownable{
    ...
    mapping(address => uint256[]) transactions_per_wallet;
    mapping(address => uint256[]) owner_NFT;
    mapping(uint => uint) bill_per_Id;

    mapping(uint => datos) datos_transaction;
    ...
    event transaction_event(address, uint256, uint256[], uint256[]);
    ...
    function ConvertDenom(uint256 _value) public payable returns(uint256 [] memory){
        ...
        transactions_per_wallet[msg.sender].push(trans);
        emit transaction_event(msg.sender, _value, billetes, id_billetes);

        datos_transaction[trans] = datos(_value, billetes, id_billetes);
        trans ++;
   ...
   function billetes_return () public view returns(uint256 [] memory){
    ...
    function ChangeStock(uint _100, uint _50, uint _20, uint _10, uint _5, uint _1) public onlyOwner returns(bool){
        require(_100 >= 0 && _50 >= 0 && _20 >= 0 && _10 >= 0 && _5 >= 0 && _1 >= 0 , "No/Negative");
   ...
   function get_transaction(address _address) public view returns(uint256[]memory){
       return transactions_per_wallet[_address];
   }

   function get_datos(uint _id) public view returns(datos memory){
       return datos_transaction[_id];
   }
