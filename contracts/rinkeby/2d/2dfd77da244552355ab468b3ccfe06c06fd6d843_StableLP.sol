...
contract StableLP is Ownable, ReentrancyGuard {
    ...
    event CreateMALP(address receiver, address input, uint256 amount);
    event RedeemMALP(
        address receiver,
        address output,
        uint256 malp,
    ...
    modifier minSwap(uint256 _amount) {
        require(_amount >= 20);
        _;
    }
     ...
     */
    function _createMALP(address _input, uint256 _amount)
        internal
        minSwap(_amount)
    {
        ...
        require(
            IERC20(_input).allowance(msg.sender, address(this)) >= _amount,
        ...
        emit CreateMALP(msg.sender, _input, MALPAmount);
    }

    function createMALP(address _input, uint256 _amount)
        external
        nonReentrant
        minSwap(_amount)
    {
        _createMALP(_input, _amount);
    }

    function createMALPByUSDT(uint256 _amount)
        external
        nonReentrant
        minSwap(_amount)
    {
        _createMALP(USDT, _amount);
    }

    function createMALPByUSDC(uint256 _amount)
        external
        nonReentrant
        minSwap(_amount)
    {
        _createMALP(USDC, _amount);
    }

    function createMALPByDAI(uint256 _amount)
        external
        nonReentrant
        minSwap(_amount)
    {
        _createMALP(DAI, _amount);
    }

    /**
     * @param _amount: the amount into MALP token
     */
    function _redeemMALP(address _output, uint256 _amount) internal {
        uint256 fee = (_amount * feeMalp) / 10**18;
        uint256 outputAmount = _amount - fee; // it will be calculated by current liquidity ratios
        ...
        require(
            IERC20(_output).balanceOf(address(this)) >= outputAmount,
            "Not enough balance"
        );
        ...
        emit RedeemMALP(msg.sender, _output, _amount, outputAmount);
    }

    function redeemMALP(address _output, uint256 _amount)
        external
        nonReentrant
    {
        _redeemMALP(_output, _amount);
    }

    function redeemMALPByUSDT(uint256 _amount) external nonReentrant {
        _redeemMALP(USDT, _amount);
    }

    function redeemMALPByUSDC(uint256 _amount) external nonReentrant {
        _redeemMALP(USDC, _amount);
    }

    function redeemMALPByDAI(uint256 _amount) external nonReentrant {
        _redeemMALP(DAI, _amount);
    }
    ...
    function swap(
        ...
        address _input,
        address _output
    ) external minSwap(_amount) nonReentrant {
        _swap(_amount, _input, _output);
    }
    ...
    function _swap(
        ...
        address _input,
        address _output
    ) internal minSwap(_amount) {
        require(
            IERC20(_input).balanceOf(msg.sender) >= _amount,
