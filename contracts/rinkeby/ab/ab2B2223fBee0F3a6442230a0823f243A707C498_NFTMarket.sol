...
contract NFTMarket is ReentrancyGuard {
    ...
    function getListingPrice(uint256 _price) internal pure returns (uint256) {
        require(_price > 0, "Error: The price is zero.Cannot get the listing price");
        return _price*2/100;
    }
    ...
    function createMarketItem (
        ...
        require(msg.value == getListingPrice(_price), "Price must be equal to listing price");
    ...
    function createMarketSale (address _nftContract, uint256 _itemId) public payable nonReentrant {
        ...
        require(msg.value == price, "Please submit the asking price in order to coomplete the purchase");
    ...
    function fetchMarketItems() public view returns (MarketItem [] memory) {
        ...
        uint _itemCount = itemIds.current();
        uint _unsoldItemCount = itemIds.current() - itemsSold.current();
        uint _currentIndex = 0;    

        MarketItem[] memory items = new MarketItem[] (_unsoldItemCount);
        for ( uint i = 0; i < _itemCount; i++) {
            if(idToMarketItem[i+1].owner == address(0)) {
                uint _currentId = idToMarketItem[i+1].itemId;
    ...
    function fectchMyNFTs() public view returns (MarketItem[] memory) {
        uint _totalItemCount = itemIds.current();
        uint _itemCount = 0;
        uint _currentIndex = 0;
