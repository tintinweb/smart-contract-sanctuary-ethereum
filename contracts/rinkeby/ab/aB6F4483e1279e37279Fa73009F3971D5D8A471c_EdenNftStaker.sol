...
contract EdenNftStaker {
    ...
    function stake(uint256 _tokenId, uint256 amount) public {
        stakes[msg.sender] = Stake(_tokenId, amount, block.timestamp); 
        parentNFT.safeTransferFrom(msg.sender, address(this), _tokenId, amount, "0x00");
    } 
    ...
    function unstake() public {
		...
		reward(lockinPeriod, stakingTime[msg.sender]);
        delete stakes[msg.sender];
    }      

	function lockin(uint256 period) public {
		require(period > 0, "Lock in period must be greater than 0");
		lockinPeriod = period;
	}	
	
	function reward(uint256 period, uint256 stakeTime) public {
		require(period > 0, 'Lock in period must be greater than 0');
