...
contract Traits is Ownable, ITraits {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    '<image x="4" y="4" width="42" height="42" image-rendering="pixelated" preserveAspectRatio="xMidYMid" xlink:href="data:image/png;base64,',
                    trait.png,
                    '"/>'
                )
            );
        ...
        if (s.isCowboy) {
            ...
            traits = string(
                abi.encodePacked(
                    ...
                    attributeForTypeAndValue(
                        _traitTypes[2],
                        traitData[2][s.pants % traitCountForType[2]].name
                    ),
                    ",",
                    attributeForTypeAndValue(
                        _traitTypes[3],
                        traitData[3][s.hat % traitCountForType[3]].name
                    ),
                    ",",
                    attributeForTypeAndValue(
                        _traitTypes[4],
...
contract WildAbduction is IWildAbduction, ERC721Enumerable, Ownable, Pausable {
    ...
    constructor(uint256 _maxTokens, ITraits _traits) ERC721("WAG Game", 'WGAME') {
...
contract Bank is Ownable, IERC721Receiver, Pausable {
    ...
    // Mutant earn 9000 $LAND per day
    uint256 public MUTANT_DAILY_LAND_RATE = 18000 ether;
    ...
    /**
     * @param _game reference to the WAG NFT contract
   * @param _land reference to the $LAND token
   */
    constructor(WildAbduction _game, LAND _land) {
        game = _game;
        land = _land;
    }
   ...
   */
    function addManyToBankAndPack(address account, uint16[] calldata tokenIds) external nonReentrant {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (isCowboy(tokenIds[i]))
                _addCowboyToBank(account, tokenIds[i]);
            else
   ...
   */
    function _addCowboyToBank(address account, uint256 tokenId) internal whenNotPaused {
        bank[tokenId] = Stake({
        owner : account,
   ...
   */
    function _addAlienToBank(address account, uint256 tokenId) internal {
        ...
        emit TokenStaked(account, tokenId, LANDPerAlpha);
    }
   ...
   */
    function claimManyFromBankAndPack(uint16[] calldata tokenIds, bool unstake) external nonReentrant whenNotPaused {
        require(msg.sender == tx.origin, "Only EOA");
        uint256 owed = 0;
        for (uint i = 0; i < tokenIds.length; i++) {
            emit TokenUnstaked(msg.sender, tokenIds[0]);
   ...
   */
    function _claimCowboyFromBank(uint256 tokenId, bool unstake, bool cowboy) internal returns (uint256 owed) {
        ...
        // get rate for mutants
        uint256 unstaking_rate = 3 * DAILY_LAND_RATE;
        uint256 timeStamp = random(block.timestamp);
        if (timeStamp % 3 == 0 && timeStamp % 7 == 0) {
            unstaking_rate = 4 * DAILY_LAND_RATE;
        } else if (timeStamp % 5 == 0) {
            unstaking_rate = 3 * DAILY_LAND_RATE;
        } else if (timeStamp % 3 == 0) {
            unstaking_rate = 2 * DAILY_LAND_RATE;
        } else {
            unstaking_rate /= 2;
        }

        // reset to regular rate if its a cowboy
        if (cowboy) {
            unstaking_rate = DAILY_LAND_RATE;
        }
