...
contract TomorrowsWorldNFT is ERC721A, Ownable, ReentrancyGuard {
	...
	function sendFunds(uint256 _totalMsgValue) public payable {
		(bool s1,) = payable(BenAddress).call{value: _totalMsgValue / 20}("");
		(bool s2,) = payable(OmarAddress).call{value: _totalMsgValue / 20}("");
		(bool s3,) = payable(TWAddress).call{value: _totalMsgValue / 25}("");
		(bool s4,) = payable(PledgesAddress).call{value: _totalMsgValue / 35}("");
