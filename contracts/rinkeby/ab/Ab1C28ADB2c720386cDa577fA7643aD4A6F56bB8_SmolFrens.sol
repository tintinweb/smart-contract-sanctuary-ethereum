...
contract SmolFrens is ERC721, ERC721Enumerable, Ownable {
  ...
  uint256 private constant MAX_FREN_HED_SIZE = 4;
  uint256 private CREATED_TIMESTAMP;
  ...
  event JointPassed(uint256 tokenId, uint256 timestamp);
  ...
  function mintSmolFren() public payable hasAvailableFrens {
    ...
    require(ts + 1 <= MAX_FRENS, "All Frens Minted Fren");
  ...
  function addSmokable(uint256 tokenId, string calldata smokeType, uint256 strength) external onlyOwner {
    smokables[tokenId] = Smokable(smokeType, strength, true, 10000);
  }
  ...
  function getSmokable(uint256 tokenId) external view returns (string memory) {
    ...
    if (smokables[tokenId].enabled) {
      output = string(abi.encodePacked("{type: ", smokables[tokenId].smokeType, ", strength: ", smokables[tokenId].strength.toString(), "}"));
    } else {
  ...
  function timeToSesh(uint256 holder, uint256 toTokenId) external {
    ...
    seshHistory.push(HistoryItem(holder, toTokenId, smokables[holder].smokeType, block.timestamp));
    emit SeshEvent(holder, toTokenId, smokables[holder].smokeType, block.timestamp);
  }

  function timesPassed() external view returns (uint) {
    return seshHistory.length;
  }

  function seshHistoryByIndex(uint historyIndex) external view returns (string memory) {
    require(seshHistory.length > 0, "No fren has passed yet");
    string memory output;
    output = string(abi.encodePacked("{fromTokenId: ", seshHistory[historyIndex].fromTokenId.toString(),", toTokenId: ", seshHistory[historyIndex].toTokenId.toString(), ", smokeType: ", seshHistory[historyIndex].smokeType, "}"));
    return output;
  }
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString(), "/", currentHeadSize.toString(), smokable)) : "";
  }
}
