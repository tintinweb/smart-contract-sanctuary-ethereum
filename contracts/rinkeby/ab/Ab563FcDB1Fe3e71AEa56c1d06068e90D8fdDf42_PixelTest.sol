...
contract PixelTest is ERC1155, Ownable, ERC1155Burnable {
    ...
    */
    ...
    uint256 public platinumPrice = 0.02 ether;
    uint256 public diamondPrice = 0.03 ether;
    uint256 public tripleaPrice = 0.04 ether;
    ...
    function addWhitelist(address _addr, bool _val) public onlyOwner {
        whitelisted[_addr] = _val;
    }
    ...
    {
        if (whitelistOnly) {
            require(whitelistOnly, "You are not whitelisted");
        }
    ...
    function setMintEnabled(bool value) public onlyOwner {
        ...
        if (value) {
            mintStartDate = block.timestamp + 3 days;
        }
    }
    ...
    function checkBalance(uint8 tier) public view returns (uint256) {
        uint256 _bal = IERC1155(address(this)).balanceOf(msg.sender, tier);
