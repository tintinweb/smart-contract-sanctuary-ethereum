...
contract MSKWithdraw is Context, Ownable {
    ...
    event Test1(bytes32 indexed _data);
    event Test2(address indexed _address);
    ...
    function withdraw(uint256 _amount, bytes memory signature) external {
        ...
        emit Test1(messageHash);

        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);

        emit Test1(ethSignedMessageHash);

        emit Test2(recoverSigner(ethSignedMessageHash, signature));
