...
contract CSHOX is Ownable, ERC721A, ReentrancyGuard {
    ...
    Tw private immutable timewrap;

    constructor(
        uint256 maxBatchSize_,
        uint256 maxSupply_,
        address TimeWrapAddress
    ) ERC721A("CSHOX", "CSHOX", maxBatchSize_, maxSupply_) {
        maxPerAddressDuringMint = maxBatchSize_;
        timewrap = Tw(TimeWrapAddress);
    }

    // CHOX level
    mapping(uint => uint) public CSHOXLevel;
    mapping(uint256 => uint256) public CSHOXLevelTime;

    function levelupCSHOX(uint tokenId, uint8 TimeWrapTypes) 
        external        
    {
        require(
        ...
        uint256 levelIndex = (tokenId * 1000000) + (getMonth() * 10000) + getYear();

        if (CSHOXLevel[tokenId] >= 1){
            
            require(
                timewrap.balanceOf(msg.sender, TimeWrapTypes) > 0,
                "Must own at least one of the TimeWraps to level up"
            );
            if (CSHOXLevelTime[levelIndex] >= 4){
                require(
                    timewrap.balanceOf(msg.sender, 9999) > 0,
                    "Max Level a month. Try again next month."
                );
            }        
            if (CSHOXLevelTime[levelIndex] >= 2){
                require(
                    TimeWrapTypes >= 2,
                    "Time Wrap not powerful enough to level up. Need Super TimeWrap to level 3x in a month."
                );
                require(
                    timewrap.balanceOf(msg.sender, TimeWrapTypes) > 0,
                    "Must own at least one of the Rare TimeWraps to level up"
                );
            }

            if (CSHOXLevelTime[levelIndex] >= 1){
                require(
                    TimeWrapTypes >= 1,
                    "Time Wrap not powerful enough to level up. Need Rare TimeWrap to level 2x in a month."
                );
                require(
                    timewrap.balanceOf(msg.sender, TimeWrapTypes) > 0,
                    "Must own at least one of the TimeWraps to level up"
        ...
        CSHOXLevelTime[levelIndex] += 1;
    }    
    
    function getMonth() public view returns (uint month) {
