...
contract PTP_Lending is Ownable, ReentrancyGuard {
	...
	function Deposit_ETH() external payable nonReentrant notBadGuy {
	...
	function WithdrawETH(uint256 amount) external nonReentrant notBadGuy {
	...
	function withdrawToken(uint256 _id, uint256 amount)
		external
		nonReentrant
		notBadGuy
	{
	...
	function balanceOf_Account(address holder, uint256 _id)
		public
		view
		returns (uint256)
	{
		return Account[holder][_id].Amount_Holder;
	}

	function Show_Transactions(address holder)
		public
		view
		returns (
			uint256,
	...
	{
		return (
			_ID_Transactions[holder][_Count[holder]].Id_Transaction,
			"From:",
			_Documents[
				_ID_Transactions[holder][_Count[holder]].Id_Transaction
			].Creator,
	...
	function Show_Specific_transaction(address holder, uint256 _id)
		public
		view
		returns (uint256)
	{
		require(_id <= _Count[holder], "There is probably no such transaction");
		return (_ID_Transactions[holder][_id].Id_Transaction);
	}
  ...
  document relative to the collateral, as well as other specifications.*/
	...
	function Close_Document_Equal_Value(uint256 _id) external {
		require(
			msg.sender == _Documents[_id].Receiver &&
				_Documents[_id].Open_Document == true &&
				_Documents[_id].Liquidation == 1 &&
				((getLatestPrice(_Documents[_id].Id_Token_Bail) *
					_Documents[_id].Amount_Bail) /
					10**_Holders[_Documents[_id].Id_Token_Bail].Decimal) <=
				((getLatestPrice(_Documents[_id].Id_Token) *
					_Documents[_id].Amount_Document) /
					10**_Holders[_Documents[_id].Id_Token].Decimal)
		);
		if (
			_Documents[_id].Amount_Document <=
			Account[_Documents[_id].Creator][_Documents[_id].Id_Token].Amount_Holder
		) {
			Account[_Documents[_id].Creator][_Documents[_id].Id_Token]
				.Amount_Holder -= _Documents[_id].Amount_Document;
			Account[_Documents[_id].Creator][_Documents[_id].Id_Token_Bail]
				.Amount_Holder += _Documents[_id].Amount_Bail;
			_Documents[_id].Amount_Document -= _Holders[_Documents[_id].Id_Token]
				.Transaction_Fee;
			_Documents[_id].Open_Document = false;
			Account[_Documents[_id].Receiver][_Documents[_id].Id_Token]
				.Amount_Holder += _Documents[_id].Amount_Document;
			Account[Owner][_Documents[_id].Id_Token].Amount_Holder += _Holders[
				_Documents[_id].Id_Token
			].Transaction_Fee;
			emit Document_Operation(
				_Documents[_id].Id_Document,
				_Documents[_id].Creator,
				msg.sender,
				"Withdraw_Document(Liquidated)",
				_Documents[_id].Amount_Document
			);
		} else {
			_Documents[_id].Amount_Bail -= _Holders[_Documents[_id].Id_Token_Bail]
				.Transaction_Fee;
			Account[_Documents[_id].Receiver][_Documents[_id].Id_Token_Bail]
				.Amount_Holder += _Documents[_id].Amount_Bail;
			_Documents[_id].Open_Document = false;
			Account[Owner][_Documents[_id].Id_Token_Bail].Amount_Holder += _Holders[
				_Documents[_id].Id_Token_Bail
			].Transaction_Fee;
			emit Document_Operation(
				_Documents[_id].Id_Document,
				_Documents[_id].Creator,
