...
/// @title Zootiez Mint Contract
/// @author GEN3 Studios
contract Zootiez is ERC721A, Ownable {
    using Strings for uint256;
    ...
    // Private Sale 1 Variables
    bool private _privateSaleOpen = false;
    mapping(address => uint256) public privateSaleCount;
    uint256 public privateSaleMax = 10; 


    // Public Sale Variables
    bool private _publicSaleOpen = false;
    mapping(address => uint256) public publicSaleMintCount;
    uint256 public publicSaleMax = 20;
     ...
     */
    modifier onlyEOA() {
        require(msg.sender == tx.origin, "Zootiez: must use EOA");
        _;
    }
    ...
    /**
     * @notice Private Mint 
     * @param signature Signature retrieved from backend for  Allowlist
     * @param nonce Random nonce retrieved from backend for Allowlist
    ...
    function privateMint(
        ...
        require(isPrivateSaleOpen(), "Zootiez: Private Sale Closed!");

        // Check if enough ETH is sent
        require(
            msg.value == _mintAmount * NFT_PRICE,
            "Zootiez: Insufficient ETH!"
        );

        // Check if mints does not exceed MAX_SUPPLY
        require(
            totalSupply() + _mintAmount <= MAX_SUPPLY,
            "Zootiez: Max Supply for Private Sale Reached!"
        );

        // Check that mints does not exceed max wallet allowance for Basic allowlist
        require(
            privateSaleCount[msg.sender] + _mintAmount <=
                privateSaleMax,
            "Zootiez: Wallet has already minted Max Amount for private sale!"
        );
    ...
    function publicMint(uint256 _mintAmount) external payable onlyEOA {
        // Check if public sale is open
        require(isPublicSaleOpen(), "Zootiez: Public Sale Closed!");
        ...
        require(
            totalSupply() + _mintAmount <= MAX_SUPPLY,
            "Zootiez: Max Supply for Public Mint Reached!"
        );

        // Check that mints does not exceed max wallet allowance for public sale
        require(
            publicSaleMintCount[msg.sender] + privateSaleCount[msg.sender] + _mintAmount <= publicSaleMax,
            "Zootiez: Wallet has already minted Max Amount for Public Sale"
        );

        publicSaleMintCount[msg.sender] += _mintAmount;
     ...
     */
    function togglePrivateSale(bool privateSale) public onlyOwner {
        _privateSaleOpen = privateSale;
    }

    /**
     * @dev Set Public Sale Status
     */
    function togglePublicSale(bool publicSale) public onlyOwner {
        _publicSaleOpen = publicSale;
    }
     ...
     * @dev Set Private Sale maximum amount of mints
     */
    function setPrivateSaleMintMax(uint256 amount) public onlyOwner {
        privateSaleMax = amount;
    }
    ...
    function withdrawToTreasury() public payable onlyOwner {
        ...
        require(success, "Zootiez: Withdrawal failed");
    }
}