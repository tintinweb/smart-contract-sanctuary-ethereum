...
contract Lending {
    ...
    mapping(address => string) public borrowerCnic;
    ...
    function requestFundsToBorrow(uint256 _amount, uint256 _duration) public {
        ...
        borrowRequest[requestId].borrowerCNIC = borrowerCnic[sender];
        emit BorrowerRequestAdded(
            requestId,
            sender,
            _amount,
            _duration,
    ...
    function ReportBorrower(bytes32 requestId) public {
        address sender = msg.sender;
        uint256 timestamp = block.timestamp;
        require(
            borrowRequest[requestId].lender == sender,
            "Only Lender can Report"
        );
        require(
            borrowRequest[requestId].status == requestStatus.ONGOING,
        ...
        require(
            timestamp > borrowDuration + (60 * 1),
            "Borrower should be given atleast 1 minute past the due time"
        );
        address(this).call{value: registrationCollateral}("");
        borrowerReported[borrowRequest[requestId].borrower] = true;
        borrower[borrowRequest[requestId].borrower] = false;
        borrowRequest[requestId].status = requestStatus.REPORTED;
        emit BorrowerRequestReported(requestId);
    }
