...
address public paperKey=0xf2fea8FC9cd6A7b87e249Effa7e5e204476e33dC;
function _setPaperKey(address _paperKey) internal {
  ...
  function paperMint(
    ...
    {
        require(msg.sender == paperKey, "Sale is not Active");
        require(status == 2, "Sale is not Active");
        require(totalSupply() + _mintData.quantity <= TotalCollectionSize_, "reached max supply");
        require(numberMinted(_mintData.recipient) + _mintData.quantity <= MAX_PER_Address, "Quantity exceeds allowed Mints" );
        require(_mintData.quantity <= MAX_PER_Transtion,"can not mint this many");
        require(msg.value >= PRICE * _mintData.quantity, "Need to send more ETH.");
        _safeMint(_mintData.recipient, _mintData.quantity);
    }
