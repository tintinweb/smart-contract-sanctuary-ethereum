...
contract getbyte{
    function getmessage(Market.Order memory order)public view virtual returns(bytes memory){
    ...
    function _getverifyInputSignatureHash(Market.RunInput memory input) public view virtual returns(bytes32){
        bytes32 hash = keccak256(abi.encode(input.shared, input.details.length, input.details));
        return (hash);

    }
        function _getverifyOrderSignatureHash(Market.Order memory order) public view virtual returns(bytes32) {
        address orderSigner;
        ...
        if (order.signVersion == Market.SIGN_V1) {
            ...
            bytes32  hex1 =ECDSA.toEthSignedMessageHash(orderHash);
