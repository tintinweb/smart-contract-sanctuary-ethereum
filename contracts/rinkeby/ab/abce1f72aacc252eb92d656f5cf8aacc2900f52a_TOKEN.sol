...
contract TOKEN is Context, IERC20, Ownable {
        ...
        )
    {
        ...
        uint256 tBurn = calculateMarketingFee(tAmount);
    ...
    function _takeLiquidityAndBurn(uint256 tLiquidityAndBurn)
        private
    {
        uint256 tFee = _burnFee.add(_liquidityFee);
        uint256 tburnTokens = tLiquidityAndBurn.div(tFee).mul(
            _burnFee
        );
        uint256 tliquidityTokens = tLiquidityAndBurn.sub(tburnTokens);
      
     
        uint256 currentRate = _getRate();
        uint256 rliquidityTokens = tliquidityTokens.mul(
            currentRate
        );
        ...
        uint256 rburnTokens = tburnTokens.mul(
            currentRate
        );
