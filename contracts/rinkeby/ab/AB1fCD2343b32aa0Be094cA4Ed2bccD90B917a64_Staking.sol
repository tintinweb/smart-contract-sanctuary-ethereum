...
contract Staking is ERC721, ReentrancyGuard, Ownable {
    ...
    function calculateReward(uint256 bundleId) public view returns(uint256) {
        if (ownerOf(bundleId) == address(this)) {
            ...
            totalRewardsTemp = bundles[bundleId].tokensStaked.mul(totalRewardsTemp.add(((block.timestamp - previousTimestamp).div(180)).mul(baseRewardsData[currentIndex].rewardsPerSecond)));
