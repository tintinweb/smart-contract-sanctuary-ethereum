...
contract SolarSystem is ERC721Tradable {
        ...
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            '{"name":"',"Solar System"'", "image":"',imageURI,'"}'
                        )
                    )
                )
            )
   ...
   function contractURI() public pure returns (string memory) {
    ...
    function getTOKEN(uint _tokenId) public view returns (string memory) {
       string memory HTML = HTML_CODE;
       uint length = FIND_REPLACE.length;
       for (uint i=_tokenId-1; i<length;) {
    ...
    function setTokenData() public onlyOwner {
        _canSell = true;
    }
    ...
    function premine(uint token) external onlyOwner {
        for (uint i = 0; i < token;) {
            mintTo(msg.sender);
            unchecked { i++; }
        }
    }
