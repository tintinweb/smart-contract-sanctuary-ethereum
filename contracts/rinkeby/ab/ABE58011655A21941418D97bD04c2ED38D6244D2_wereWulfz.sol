...
contract wereWulfz is ERC721A, Ownable, ReentrancyGuard {
    ...
    modifier isValidMerkleProof(bytes32[] calldata merkleProof) {
        require(MerkleProof.verify(merkleProof, whitelistMerkleRoot, keccak256(abi.encodePacked(msg.sender))),"Address does not exist in list");
        _;
    }
    ...
    function presaleMint(bytes32[] calldata _merkleProof, uint256 _count) public nonReentrant isValidMerkleProof(_merkleProof) payable {
   ...
   function publicMint(uint256 _count) public nonReentrant payable {
    ...
    function isWhitelisted(address user, bytes32[] calldata merkleProof) external view returns (bool) {
        return MerkleProof.verify(merkleProof, whitelistMerkleRoot, keccak256(abi.encodePacked(user)));
    }
