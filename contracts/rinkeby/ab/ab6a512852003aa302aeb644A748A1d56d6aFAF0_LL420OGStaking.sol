  ...
  ERC1155HolderUpgradeable
{
  ...
  /// @dev OG pass staking reward is $4 HIGH per day
  uint64 public constant INITIAL_REWARD = 50;
  ...
  function stake(uint256 amount) external nonReentrant whenNotPaused {
    ...
    require(
      OG_PASS_CONTRACT.balanceOf(_msgSender(), OG_ID) >= amount,
      "Invalid balance"
    );
  ...
  function userDailyReward(address _staker) external view returns (uint256) {
    ...
    return _balances[_staker] * INITIAL_REWARD * 8;
  }
  ...
  function _pendingReward(address _staker) internal view returns (uint256) {
    ...
    return ((block.timestamp - _stakedTimestamp[_staker]) / 450) * INITIAL_REWARD;
  }
