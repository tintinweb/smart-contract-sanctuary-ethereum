...
contract Betting is IBetting{
   ...
   modifier MBet{
    ...
    function addAdmin(address _newAdmin) public Admin{
    ...
    function payWinningBets(uint8 _matchID, string memory _winninType) override external Admin{
        require(players.length != 0, "No bets at the time");
        require(keccak256(abi.encodePacked(_winninType)) == keccak256(abi.encodePacked("Tie")) || keccak256(abi.encodePacked(_winninType)) == keccak256(abi.encodePacked("Team A")) || keccak256(abi.encodePacked(_winninType)) == keccak256(abi.encodePacked("Team B")), "Betting type must be 'Team A' or 'Team B' or 'Tie'");
        ...
        for(uint8 i = 0; i < players.length; i++){
                ...
                if(bets[players[i]].matchID == _matchID && keccak256(abi.encodePacked(bets[players[i]].bettingType)) == keccak256(abi.encodePacked(_winninType))){
