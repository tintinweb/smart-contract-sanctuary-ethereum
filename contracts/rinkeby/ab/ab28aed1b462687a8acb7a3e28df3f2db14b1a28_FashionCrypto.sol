...
contract FashionCrypto is ERC721, Ownable , RandomlyAssigned {
    using Strings for uint256;
    string baseURI="https://ipfs.io/ipfs/QmVJ1qNwD4Nvr5D1jfpGrCm491c7VRQiMnAXcruuCM49Mv/";
    string baseExtension = ".json";

    uint256 preSaleStartTime =1651402800;//May 1, 8PM
    uint256 preSaleEndTime = 1651540917; //May 3, 8PM
    //uint256 preSaleStartTime =1653908400;//May 30, 8PM
    //uint256 preSaleEndTime = 1653994800; //May 31, 8PM
    uint256 cost = 0.003 ether;

    address[] private whitelistedAddressesOne =[0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2];
    address[] private whitelistedAddressesTwo =[0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db];


    constructor() ERC721("FashionCrypto", "FCC")  
    RandomlyAssigned(2000, 1) 
    {
    }

    function mint(uint8 amount) ensureAvailability public payable returns (uint8)    {
        require (block.timestamp >= preSaleStartTime, "Not Sales Period.");
        require(amount < 6, "Maximum of 5 tokens to buy at once.");
        require(msg.value == cost * amount, "Price must be equal to listing price.");
        ...
        if(block.timestamp >= preSaleStartTime && block.timestamp <= preSaleEndTime){
            ...
            if(isWhiteListedOne){
                require(balanceOf(msg.sender) + amount <= 10, "Max mint per address exceeded!");
            }else if(isWhiteListedTwo){
                require(balanceOf(msg.sender) + amount <= 1, "Max mint per address exceeded!");
            }
