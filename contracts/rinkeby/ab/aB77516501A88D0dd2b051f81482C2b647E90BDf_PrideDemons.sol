    ...
    ReentrancyGuard
{
    ...
    mapping(uint256 => string) private _metadataId;
    ...
    function setVaultLock(uint256 _vaultLock) external onlyOwner {
        vaultLock = _vaultLock;
    }
    ...
    function freeze() external onlyOwner {
        uint256[] memory metadata = suffleMetadata();
        for (uint256 i = 1; i <= metadata.length; i += 1) {
            _metadataId[i] = string(
                    abi.encodePacked(
                        _tokenBaseURI,
                        Strings.toString(metadata[i])
                        ,".json"
                    ));
            emit PermanentURI(_metadataId[i], i);
        }
        revealed = true;
    }

    function suffleMetadata() internal view returns (uint256[] memory) {
        ...
        for (uint256 i = vaultLock + 1; i <= maxSupply; i += 1) {
            uint256 j = (uint256(keccak256(abi.encode(seed, i))) % (maxSupply)) + 1;

            if(j >= vaultLock + 1 && j <= maxSupply) {
        ...
        override
    {
        seed = randomNumber;
        if(randomNumber == 0){
            seed = (uint256(keccak256(abi.encode(requestId, totalPrivateSaleMinted))) % (maxSupply)) + 1;
        }
        emit RandomseedFulfilment(block.timestamp, requestId, seed);
    }
}
