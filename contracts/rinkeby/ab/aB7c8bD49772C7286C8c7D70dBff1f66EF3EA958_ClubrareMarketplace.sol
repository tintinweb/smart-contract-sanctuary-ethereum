    ...
    IClubrareMarketplace
{
    ...
    function _splitFee(
        ...
        for (uint256 i; i < len; i++) {
            ...
            if (isToken) {
                if(isClaim){
                    IERC20Upgradeable(_erc20Token).approve(payee,commission);
                }
    ...
    function _chargeAndSplit(
        ...
        uint marketFee = stakingSplit + lpSplit;
        uint256 platformFee;
        ...
        if (isEth) {
            ...
            checkAllowance(ptoken,sender,amt,_user,_isClaim);
    ...
    function checkAllowance(IERC20Upgradeable ptoken,address sender, uint256 _amount, address user,bool isClaim) private {
         if(!isClaim){
                require(ptoken.allowance(sender, address(this)) >= _amount, "insufficient allowance");
            }else if(isClaim){
                ptoken.approve(user,_amount);
            }
    }
