...
// Root file: contracts/MetaSharkMasonAvatars.sol
...
contract MetaSharkMasonAvatars is ERC721, ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    constructor() ERC721("MetaShark Mason Avatars", "MTSA") {}

    function mint(address account, uint256 tokenId) external onlyMinter {
        _checkMint(account, tokenId);
    }

    function mintBatch(address[] memory accounts, uint256[] memory ids)
        external
        onlyMinter
    {
        require(accounts.length == ids.length, "Mismatch");
        for (uint256 i = 0; i < accounts.length; i++) {
            _checkMint(accounts[i], ids[i]);
        }
    }

    function _checkMint(address to, uint256 tokenId) private {
        require(tokenId > 0 && tokenId <= _MAX_SUPPLY, "Range error");
        _safeMint(to, tokenId);
    }
    ...
    function burn(uint256 tokenId) external {
        require(burnAllowed, "ERC721Burnable: burn not allowed");
        require(
    ...
    function setBurnAllowed(bool state) external onlyOwner {
        burnAllowed = state;
    }
