...
contract DegenGoblins is ERC721ABurnable, Ownable, ReentrancyGuard {
  ...
  bool public _isOpenMintLive;



  constructor() ERC721A("Degen Goblins", "DG") {
    _currentStageMaxMint = 75;
    _isMintLive = true;
    _isOpenMintLive = false;
  ...
  function _joinGoblinTown(address to, uint256 quantity) private {
    ...
    require(numberMinted(msg.sender) == 0, "Max 2 mint per wallet.");
    _safeMint(to, quantity);
  }

  function mint() external payable {
    require(_isOpenMintLive 
      || ((totalSupply() + 2 <= _currentStageMaxMint) && _isMintLive), 
      "Mint is not live."
    );
    _joinGoblinTown(msg.sender, 2);
  }
  ...
  function setOpenMint(bool state) external onlyOwner {
    _isOpenMintLive = state;
  }
