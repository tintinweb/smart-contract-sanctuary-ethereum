...
contract OpenTokenBoosterRewardV1 is Initializable, ChainlinkClient {
    ...
    //unlockAdmin
    address public unlockAdmin;

    uint256 public constant WEEK = 600;//604800;
    address public constant ZERO_ADDRESS = address(0);

    uint256 public oraclePayment; // 0.0001 * 10**18
    ...
    // gauge_id -> week time -> claimed record
    mapping (uint256 => mapping (uint256 => ClaimedRecord)) public claimedHistory;
    // gauge_id -> week time
    mapping (uint256 => uint256) public lastClaimedTime;
    // gauge_id -> week time
    mapping (uint256 => uint256) public claimLockTime;

    uint256 public minWeekTime;
    uint256 public maxWeekTime;
    ...
    modifier onlyByUnlockAdmin() {
        require(msg.sender == currentOwner || msg.sender == unlockAdmin , "You are not the unlock admin");
        _;
    }
    ...
    */
    function initialize(address openTokenAddr_,
        address veOpenTokenAddr_) public initializer {
        ...
        unlockAdmin = msg.sender;
        openToken = IOpen(openTokenAddr_);
        veOpenToken = IveOpen(veOpenTokenAddr_);

        oraclePayment = 1 * LINK_DIVISIBILITY / 10000;
        //the min weektime which has cid
        minWeekTime = 0;
        //the max weektime which has cid
        maxWeekTime = 0;
    }
    ...
    */
    ...
    function setCid(uint256 t_, string memory cid_) external onlyByOwner {
        ...
        require(weekTime >= minWeekTime, "Can not set weekTime which earlier than minWeekTime");
        bytes memory oldCid = bytes(cidHistory[weekTime]);
        require( oldCid.length == 0, "Already set cid");
        cidHistory[weekTime] = cid_;
        maxWeekTime = weekTime;
    }

    function setOraclePayment(uint256 rate_) external onlyByOwner {

        require( rate_ > 0 && rate_ <= LINK_DIVISIBILITY, "Rate exceeds the allowable range");
    ...
    * @notice Returns the cid of the target weektime .
    */
    function getCids(uint256 startTime_,uint256 endTime_) external view returns ( string[] memory) {
        uint256 weekTime = startTime_ / WEEK * WEEK;

        if(endTime_ > maxWeekTime)  {
            endTime_ = maxWeekTime;
        }
        uint256 cidCount = (endTime_-weekTime)/WEEK+1;
        string[] memory cidList = new string[](cidCount);
        uint256 tempWeekTime = weekTime;
        for(uint256 i=0; i<cidCount; ++i) {
            cidList[uint256(i)] = cidHistory[tempWeekTime];
            tempWeekTime = tempWeekTime+WEEK;
        }
        return cidList;
    }
    ...
    */
    function requestReward(uint256 claimGaugeId_) public {
        ...
        uint256 startClaimTime = lastClaimedTime[claimGaugeId_] + WEEK;
        ...
        require(startTime <= minWeekTime , "Min reward time need >= open token's starttime ");
        ...
        require(startClaimTime <= curWeekTime, "You can not claimed the reward ");

        require(startClaimTime <= maxWeekTime, "You have claimed all the reward ");

        bytes memory startCid = bytes(cidHistory[startClaimTime]);
        require( startCid.length != 0, "You have no reward data");

        require( claimLockTime[claimGaugeId_] == 0, "You have locked claim reward");
        //the gauge lock the start claim time
        claimLockTime[claimGaugeId_] = startClaimTime;
        ...
        req.addInt('claimGaugeId', int256(claimGaugeId_));
        req.addInt('startClaimTime', int256(startClaimTime));
        req.addInt('endClaimTime', int256(maxWeekTime));
        req.add('boosterRewardAddr', StringsUpgradeable.toHexString(uint256(uint160(address(this))), 20));
    ...
    */
    function fulfillReward(bytes32 requestId_, uint256 claimGaugeId_, address claimedAddress_, uint256 claimTime_, uint256 amount_) public recordChainlinkFulfillment(requestId_){

        emit RequestRewardFulfilled(requestId_, claimGaugeId_, claimedAddress_, claimTime_, amount_);
        require( claimedAddress_ != ZERO_ADDRESS, "need claimedAddress");
        require( amount_ > 0, "claimed amount need > 0");
        require( claimTime_ > lastClaimedTime[claimGaugeId_], "can not repeat claim");
        require( claimLockTime[claimGaugeId_] > lastClaimedTime[claimGaugeId_], "no claim reward is locked ");
        require( claimLockTime[claimGaugeId_] <= claimTime_, "no claim reward is locked ");

        //unlock the claim reward
        claimLockTime[claimGaugeId_] = 0;
        //record the claimed history
        lastClaimedTime[claimGaugeId_] = claimTime_;
        claimedHistory[claimGaugeId_][claimTime_] = ClaimedRecord({
        ts:block.timestamp,
        amount:amount_
        });
        //mint reward open tokens
        openToken.mint(claimedAddress_, amount_);
    }

    /**
     * @notice owner unlock the locked claim booster reward
    */
    function unlockReward(uint256 claimGaugeId_, address claimedAddress_, uint256 claimTime_, uint256 amount_) public onlyByUnlockAdmin{

        emit UnlockRewardFulfilled(claimGaugeId_, claimedAddress_, claimTime_, amount_);
        require( claimedAddress_ != ZERO_ADDRESS, "need claimedAddress");
        require( amount_ > 0, "claimed amount need > 0");
        require( claimTime_ > lastClaimedTime[claimGaugeId_], "can not repeat claim");
        require( claimLockTime[claimGaugeId_] > lastClaimedTime[claimGaugeId_], "no claim reward is locked ");
    ...
    event RequestRewardFulfilled(bytes32 indexed requestId, uint256 indexed claimGaugeId, address indexed claimedAddress, uint256 claimedTime, uint256 amount);
    event UnlockRewardFulfilled(uint256 indexed claimGaugeId, address indexed claimedAddress, uint256 claimedTime, uint256 amount);
...
interface IveOpen {
    ...
    function minWeekTime() external view returns (uint256);
}
...
interface IGaugeV1 {
    ...
    function gaugesPool(uint guageId_) external view returns (GaugePool memory);
}
