 ...
 */
contract PartySpudClub is ERC721, Ownable, RandomlyAssigned, Initializable {
    ...
    struct MintTypes {
        uint256 _numberOfSpudMintsByAddress;
    ...
    uint256 private constant MAX_TEAM_SPUDS = 69; // reserved for the team and marketing
    uint256 private constant MAX_MINTS_PER_ADDRESS = 10; // max total mints
    ...
    address private immutable _communityAddress =
    ...
    uint256 public MAX_SPUDS_SUPPLY = 888;
    ...
    constructor(string memory _uri, address _adminSigner)
        ERC721('The Party Spud Club', 'PSC')
        RandomlyAssigned(
            MAX_SPUDS_SUPPLY,
    ...
    /// @dev Mints the number of tokens passed in as count to the _communityAddress
    ...
    {
        require(
            count + teamTokensMinted <= MAX_TEAM_SPUDS,
            'Exceeds the reserved supply of team tokens'
        );
        for (uint256 i = 0; i < count; i++) {
            _mintRandomId(_communityAddress);
        }
    ...
    function claimFreeMintTokens(
        ...
        require(_isVerifiedVoucher(digest, voucher), 'Invalid voucher');
        require(
            count + addressToMints[msg.sender]._numberOfSpudMintsByAddress <=
                allotted,
            'Exceeds number of earned Spuds'
        );
        addressToMints[msg.sender]._numberOfSpudMintsByAddress += count;
    ...
    /// @notice mints tokens with randomized token IDs to addresses eligible for whitelist
    /// @param count number of tokens to mint in transaction
    /// @param voucher voucher signed by an admin voucher
    function mintWhitelist(uint256 count, Voucher memory voucher)
        external
        payable
        ensureAvailabilityFor(count)
        validateEthPayment(count)
    {
        ...
        bytes32 digest = keccak256(
            abi.encode(VoucherType.Whitelist, msg.sender)
        );
        require(_isVerifiedVoucher(digest, voucher), 'Invalid voucher');
    ...
    function _claimReservedToken(address to, uint256 id) internal {
        assert(id != 0);
        assert(id <= NUMBER_OF_OG_SPUDS + NUMBER_OF_RESERVED_SPUDS);
        if (!_exists(id)) {
            _safeMint(to, id);
        }
    }
    ...
    /// @dev compares the product of the state variable `_spudPrice` and supplied `count` to msg.value
    ...
    // modifier validate25PercentEthPayment(uint256 count) {
    //     require(
    //         (spudPrice * count) * 0.75 <= msg.value,
    //         'You have not sent enough ether'
    //     );
    //     _;
    // }
