...
contract Exchange is Initializable, UUPSUpgradeable, EIP712Upgradeable, OwnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // used IERC20Upgradeable interface to implement safeTransfer
    ...
    address jimRevenueWallet;
    uint256 jimRevenuePercentage;
    ...
    function initialize(address _jimRevenueWallet, uint256 _jimRevenuePercentage) public initializer {
        // initializing
        ...
        jimRevenueWallet = _jimRevenueWallet;
        jimRevenuePercentage = _jimRevenuePercentage;
    }
    ...
    function createOrder(LibOrder.Order memory order, bytes calldata signature) external whenNotPaused nonReentrant {
        ...
        require((order.saleType == CLOSED_AUCTION_SALE_CLASS || order.saleType == OPEN_AUCTION_SALE_CLASS) && order.startTime > block.timestamp, "Exchange: start time must be greater than current time");
        require(order.saleType == CLOSED_AUCTION_SALE_CLASS && order.endTime > order.startTime, "Exchange: end time must be greater than start time");
    ...
    function completeFixedSale(LibOrder.Order memory order) external payable whenNotPaused nonReentrant {
        ...
        // take revenue and send to jim revenue wallet
        if(order.takeRevenue) {
            uint256 revenueAmount = calculateRevenue(order.price);
            (bool success, ) = payable(jimRevenueWallet).call{value: revenueAmount}("");
            order.price -= revenueAmount;
    ...
    function completeBidding(LibBidOrder.BidOrder memory bidOrder, bytes calldata signature) external whenNotPaused nonReentrant {
        ...
        require(bidOrder.saleType == CLOSED_AUCTION_SALE_CLASS && block.timestamp > bidOrder.endTime, "Exchange: Sale not ended yet");
        ...
        // get unique key
        LibOrder.Order memory order = LibOrder.Order(bidOrder.tokenType, bidOrder.saleType, bidOrder.takeRevenue, bidOrder.seller, bidOrder.tokenAddress, bidOrder.tokenId, bidOrder.tokenAmount, bidOrder.price, bidOrder.startTime, bidOrder.endTime, bidOrder.nonce);
        bytes32 hashKey = LibOrder.genHashKey(order);
        ...
        // take revenue and send to jim revenue wallet
        if(order.takeRevenue) {
            uint256 revenueAmount = calculateRevenue(bidOrder.bidAmount);
            WETH.safeTransferFrom(jimRevenueWallet, msg.sender, revenueAmount);
            bidOrder.bidAmount -= revenueAmount;
        }
        // transfer WETH to seller
        WETH.safeTransferFrom(bidOrder.buyer, msg.sender, bidOrder.bidAmount);

        // transfer token from seller to buyer
        if(order.tokenType == ASSET_TYPE_ERC721) IERC721Upgradeable(bidOrder.tokenAddress).safeTransferFrom(msg.sender, bidOrder.buyer, bidOrder.tokenId);
        if(order.tokenType == ASSET_TYPE_ERC1155) IERC1155Upgradeable(bidOrder.tokenAddress).safeTransferFrom(msg.sender, bidOrder.buyer, bidOrder.tokenId, bidOrder.tokenAmount, "");
    }
    ...
    /**
     * @dev Set Revenue wallet and percentage by UPDATER_ROLE.
     *
     * @param _jimRevenueWallet.
     * @param _jimRevenuePercentage.
     */
    function setJimRevenue(address _jimRevenueWallet, uint256 _jimRevenuePercentage) external onlyRole(UPDATER_ROLE) {
        require(_jimRevenueWallet != address(0), "Exchange: Cant be zero address");
        jimRevenueWallet = _jimRevenueWallet;
        jimRevenuePercentage = _jimRevenuePercentage;
    }
     ...
     */
    function calculateRevenue(uint256 price) view internal returns(uint256) {
        return price * jimRevenuePercentage / 100;
    }
...
library LibOrder {
    ...
    bytes32 constant ORDER_TYPEHASH = keccak256(
        "Order(bytes4 tokenType,bytes4 saleType,bool takeRevenue,address seller,address tokenAddress,uint256 tokenId,uint256 tokenAmount,uint256 price,uint256 startTime,uint256 endTime,uint256 nonce)"
    );
    ...
    function genHashKey(Order memory order) internal pure returns(bytes32) {
        bytes32 hashKey = keccak256(
            abi.encode(order.tokenType, order.saleType, order.takeRevenue, order.seller, order.tokenAddress, order.tokenId, order.tokenAmount, order.price, order.startTime, order.endTime, order.nonce)
        );
        return hashKey;
    }
    ...
    function genOrderHash(Order memory order) internal pure returns(bytes32) {
        return keccak256(
            abi.encode(ORDER_TYPEHASH, order.tokenType, order.saleType, order.takeRevenue, order.seller, order.tokenAddress, order.tokenId, order.tokenAmount, order.price, order.startTime, order.endTime, order.nonce)
        );
    }
}
...
library LibBidOrder {
    ...
    bytes32 constant BID_ORDER_TYPEHASH = keccak256(
        "BidOrder(bytes4 tokenType,bytes4 saleType,bool takeRevenue,address seller,address tokenAddress,uint256 tokenId,uint256 tokenAmount,uint256 price,uint256 startTime,uint256 endTime,uint256 nonce,address buyer,uint256 bidAmount)"
    );
    ...
    function genBidOrderHash(BidOrder memory order) internal pure returns(bytes32) {
        return keccak256(
            abi.encode(BID_ORDER_TYPEHASH, order.tokenType, order.saleType, order.takeRevenue, order.seller, order.tokenAddress, order.tokenId, order.tokenAmount, order.price, order.startTime, order.endTime, order.nonce, order.buyer, order.bidAmount)
        );
    }
}
