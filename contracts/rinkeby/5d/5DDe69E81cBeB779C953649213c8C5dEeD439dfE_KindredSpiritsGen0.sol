...
contract KindredSpiritsGen0 is CustomERC721Metadata, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public royalty = 1000; // 10%
    uint256 public pubSaleCost = 10000000000000000;
    uint256 public prvSaleCost = 10000000000000000;
    ...
    // admin function
    function setCost(bool isPublic, uint256 _newCost) external onlyAdmin {
        if(isPublic) {
    ...
    function mint(uint256 _projectId) external payable returns (uint256 _tokenId) {
        require(msg.value >= getCost(publicSale), "Value should be more than mint cost.");
        require(publicSale || isMintWhitelisted[msg.sender], "Must mint from whitelisted minter contract.");
