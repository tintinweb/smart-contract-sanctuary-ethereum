...
import "contracts/strategies/convex/AConvexLPStrategy.sol";
import "contracts/vendor/curve/ICurveV1.sol";
import "contracts/vendor/curve/ICurveV2.sol";
...
/**
 * @title Convex Same LP strategy
 * @notice Senior/Junior assets are invested into the same convex pool.
 */
contract ConvexSameLPStrategy is AConvexLPStrategy {
  using OndoSaferERC20 for IERC20;

  // curve lp settings
  ConvexLPSetting public lpSetting;
  ...
  constructor(
    address _registry,
    address _lpAddress,
    address _lpMinterAddress,
    uint256 _lpAssetCount,
    IBaseRewardPool _cvxReward,
    uint256 _cvxPID,
    bool _isV2Pool,
    address[] memory _rewardTokens
  ) BasePairLPStrategy(_registry) {
    name = "Convex Same LP Strategy";
    lpSetting.lpAddress = _lpAddress;
    lpSetting.lpMinterAddress = _lpMinterAddress;
    lpSetting.lpAssetCount = _lpAssetCount;
    lpSetting.cvxReward = _cvxReward;
    lpSetting.cvxPID = _cvxPID;
    lpSetting.isV2Pool = _isV2Pool;
    rewardTokens = _rewardTokens;
    for (uint256 i = 0; i < _lpAssetCount; i++) {
      // V1/V2 has the same interface for coins()
      //lpSetting.coins[i] = ICurveV1(_lpMinterAddress).coins(i);
    }
  }
   ...
   */
  function claim() public isAuthorized(OLib.STRATEGIST_ROLE) {
    uint256[] memory amountsBefore = _getTokenAmounts(lpSetting);

    // claim rewards
    lpSetting.cvxReward.getReward();

    uint256[] memory amountsAfter = _getTokenAmounts(lpSetting);
    for (uint256 i = 0; i < lpSetting.lpAssetCount; i++) {
      lpSetting.compoundAmounts[i] += amountsAfter[i] - amountsBefore[i];
    }
  }

  /**
   * @dev swap reward tokens
   */
  function swapRewardTokens() public isAuthorized(OLib.STRATEGIST_ROLE) {
    uint256[] memory amountsBefore = _getTokenAmounts();
    ...
    uint256[] memory amountsAfter = _getTokenAmounts();
    for (uint256 i = 0; i < lpSetting.lpAssetCount; i++) {
      lpSetting.compoundAmounts[i] += amountsAfter[i] - amountsBefore[i];
    }
  }
  ...
  /**
   * @dev prepare lp and deposit into convex
   */
  function depositConvex() public isAuthorized(OLib.STRATEGIST_ROLE) {
    _depositConvex(lpSetting, lpSetting.compoundAmounts);

    lpSetting.compoundAmounts = [0, 0, 0, 0];
  }
  ...
  function addVault(
    ...
    bool isValidSenior;
    bool isValidJunior;
    uint256 seniorAssetIndex;
    uint256 juniorAssetIndex;
    for (uint256 i = 0; i < lpSetting.lpAssetCount; i++) {
      if (lpSetting.coins[i] == address(_senior)) {
        isValidSenior = true;
        seniorAssetIndex = i;
      }
      if (lpSetting.coins[i] == address(_junior)) {
        isValidJunior = true;
        juniorAssetIndex = i;
      }
    }
    require(
      _senior != _junior && isValidJunior && isValidSenior,
      "Invalid assets"
    );

    vaults[_vaultId].origin = IPairVault(msg.sender);
    vaults[_vaultId].pool = IERC20(lpSetting.lpAddress);
    vaults[_vaultId].senior = _senior;
    vaults[_vaultId].junior = _junior;
    _vaultAssetIndex[_vaultId].senior = seniorAssetIndex;
    _vaultAssetIndex[_vaultId].junior = juniorAssetIndex;
  }
  ...
  {
    return (
      (lpTokens * totalSupply) / getLPAmount(),
  ...
  {
    return ((shares * getLPAmount()) / totalSupply, vaults[vaultId].shares);
  }

  /**
   * @dev Get total lp amount
   */
  function getLPAmount() public view returns (uint256) {
    return lpSetting.cvxReward.balanceOf(address(this));
  }

  // Internal Functions

  function _getTokenAmounts() internal view returns (uint256[] memory amounts) {
    amounts = new uint256[](lpSetting.lpAssetCount);
    for (uint256 i = 0; i < lpSetting.lpAssetCount; i++) {
      amounts[i] = IERC20(lpSetting.coins[i]).balanceOf(address(this));
    }
  }
  ...
  function _invest(
    ...
    uint256 prevLPAmount = getLPAmount();

    uint256[4] memory amounts;
    amounts[_vaultAssetIndex[vaultId].senior] = seniorAmount;
    amounts[_vaultAssetIndex[vaultId].junior] = juniorAmount;

    uint256 newLPAmount = _depositConvex(lpSetting, amounts);

    // calculate shares
    if (totalSupply == 0) {
      shares = newLPAmount;
    } else {
      shares = (newLPAmount * totalSupply) / prevLPAmount;
    }
  }
  ...
  function _redeem(
    uint256 vaultId,
    uint256 shares,
    uint256 seniorExpected
  ) internal override returns (uint256 seniorReceived, uint256 juniorReceived) {
    uint256 lpAmount = (shares * getLPAmount()) / totalSupply;

    // withdraw from convex
    lpSetting.cvxReward.withdrawAndUnwrap(lpAmount, false);

    IERC20 seniorAsset = vaults[vaultId].senior;
    uint256 balanceBefore = seniorAsset.balanceOf(address(this));

    if (lpSetting.isV2Pool) {
      ICurveV2(lpSetting.lpMinterAddress).remove_liquidity_one_coin(
        lpAmount,
        _vaultAssetIndex[vaultId].senior,
        0
      );
    } else {
      ICurveV1(lpSetting.lpMinterAddress).remove_liquidity_one_coin(
        lpAmount,
        int128(int256(_vaultAssetIndex[vaultId].senior)),
        0
      );
    }

    seniorReceived = seniorAsset.balanceOf(address(this)) - balanceBefore;

    if (seniorReceived >= seniorExpected) {
      uint256 seniorRemaining = seniorReceived - seniorExpected;
      if (seniorRemaining > 0) {
        IERC20(lpSetting.coins[_vaultAssetIndex[vaultId].senior])
          .ondoSafeIncreaseAllowance(
            lpSetting.lpMinterAddress,
            seniorRemaining
          );

        if (lpSetting.isV2Pool) {
          juniorReceived = ICurveV2(lpSetting.lpMinterAddress).exchange(
            _vaultAssetIndex[vaultId].senior,
            _vaultAssetIndex[vaultId].junior,
            seniorRemaining,
            0
          );
        } else {
          juniorReceived = ICurveV1(lpSetting.lpMinterAddress).exchange(
            int128(int256(_vaultAssetIndex[vaultId].senior)),
            int128(int256(_vaultAssetIndex[vaultId].junior)),
            seniorRemaining,
            0
          );
        }
...
import "contracts/strategies/curve/withoutGauge/ACurveLPStrategy.sol";
import "contracts/vendor/convex/IConvexBooster.sol";
import "contracts/vendor/convex/IBaseRewardPool.sol";

abstract contract AConvexLPStrategy is ACurveLPStrategy {
  using OndoSaferERC20 for IERC20;

  struct ConvexLPSetting {
    address lpAddress;
    address lpMinterAddress;
    uint256 lpAssetCount;
    address[4] coins;
    uint256[4] compoundAmounts;
   ...
   */
  function setSwapPaths(address[] memory _assets, SwapPath[] memory _paths)
    external
  ...
  function _getTokenAmounts(ConvexLPSetting memory lp)
    internal
    view
    returns (uint256[] memory amounts)
  {
    amounts = new uint256[](lp.lpAssetCount);
    for (uint256 i = 0; i < lp.lpAssetCount; i++) {
      amounts[i] = IERC20(lp.coins[i]).balanceOf(address(this));
    }
  }

  function _depositConvex(ConvexLPSetting memory lp, uint256[4] memory amounts)
    internal
    returns (uint256 newLpAmount)
  {
    newLpAmount = _depositCurve(
      CurveLPSetting(
        lp.lpAddress,
        lp.lpMinterAddress,
        lp.lpAssetCount,
    ...
    IERC20(lp.lpAddress).ondoSafeIncreaseAllowance(
      address(CONVEX_BOOSTER),
      newLpAmount
    );

    CONVEX_BOOSTER.deposit(lp.cvxPID, newLpAmount, true);
  }

  function _swap(address rewardAsset) internal {
    uint256 amountIn = IERC20(rewardAsset).balanceOf(address(this));

    if (amountIn > 0) {
      _swapExactIn(swapPaths[rewardAsset], amountIn, 0);
    }
  }
}
...
interface ICurveV1 {
  ...
  // Other functions for interacting with crv gauge
...
abstract contract ACurveLPStrategy is BasePairLPStrategy, Multicall {
  using OndoSaferERC20 for IERC20;

  struct CurveLPSetting {
    address lpAddress;
    address lpMinterAddress;
    uint256 lpAssetCount;
    address[4] coins;
  ...
  mapping(uint256 => VaultAssetIndex) internal _vaultAssetIndex;
    ...
    returns (uint256, uint256)
  {
    ...
    uint256 shares = _invest(_vaultId, _seniorMinIn, _juniorMinIn);
    ...
    returns (uint256 seniorReceived, uint256 juniorReceived)
  {
    ...
    uint256 shares = vaults[_vaultId].shares;
    ...
    vault.senior.ondoSafeIncreaseAllowance(
      msg.sender,
      seniorReceived + vault.seniorExcess
    );
    vault.junior.ondoSafeIncreaseAllowance(
      msg.sender,
  ...
  function _getTokenAmounts(CurveLPSetting memory lp)
    internal
    view
    returns (uint256[] memory amounts)
  {
  ...
  function _redeem(
    ...
    uint256 shares,
    uint256 seniorExpected
  ) internal virtual returns (uint256 seniorReceived, uint256 juniorReceived) {}

  function _depositCurve(CurveLPSetting memory lp, uint256[4] memory amounts)
    internal
    returns (uint256 newLpAmount)
  {
    uint256 lpAmountBefore = IERC20(lp.lpAddress).balanceOf(address(this));

    for (uint256 i = 0; i < lp.lpAssetCount; i++) {
      if (amounts[i] > 0) {
        IERC20(lp.coins[i]).ondoSafeIncreaseAllowance(
          lp.lpMinterAddress,
          amounts[i]
        );
      }
    ...
    if (lp.lpAssetCount == 2) {
      // V1/V2 has the same interface for add_liquidity()
      ICurveV1(lp.lpMinterAddress).add_liquidity([amounts[0], amounts[1]], 0);
    } else if (lp.lpAssetCount == 3) {
      // V1/V2 has the same interface for add_liquidity()
      ICurveV1(lp.lpMinterAddress).add_liquidity(
        [amounts[0], amounts[1], amounts[2]],
        0
      );
    } else if (lp.lpAssetCount == 4) {
      // V1/V2 has the same interface for add_liquidity()
      ICurveV1(lp.lpMinterAddress).add_liquidity(
    ...
    newLpAmount =
      IERC20(lp.lpAddress).balanceOf(address(this)) -
      lpAmountBefore;
  }

  function _swapExactIn(
    SwapPath memory data,
    uint256 amountIn,
    uint256 minAmountOut
  ) internal returns (uint256) {
    IERC20(data.path[0]).ondoSafeIncreaseAllowance(data.router, amountIn);
    return
  ...
  function _swapExactOut(
    ...
    IERC20(data.path[0]).ondoSafeIncreaseAllowance(data.router, maxAmountIn);
    return
      IUniswapV2Router02(data.router).swapTokensForExactTokens(
        amountOut,
        maxAmountIn,
        data.path,
        address(this),
  ...
  function _mint(uint256 vaultId, uint256 amount) internal {
    vaults[vaultId].shares += amount;
    totalSupply += amount;
  }

  function _burn(uint256 vaultId, uint256 amount) internal {
    vaults[vaultId].shares -= amount;
    totalSupply -= amount;
  }
}
