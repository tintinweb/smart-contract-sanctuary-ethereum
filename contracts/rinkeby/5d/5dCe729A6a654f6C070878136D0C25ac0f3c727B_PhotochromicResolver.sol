...
contract PhotochromicResolver is ABIResolver, AddrResolver, ContentHashResolver, DNSResolver, InterfaceResolver, NameResolver, PubkeyResolver {
    ...
    modifier authorisedOrOwner(bytes32 node) {
        ...
        require(nodeOwner == msg.sender || owner == msg.sender);
        _;
    }
    ...
    function setPCRecords(
        ...
        setAddr(node, 60, addressToBytes(sender)); // COIN_TYPE_ETH
    ...
    function update(bytes32 node, UpdateRecord[] calldata list) external authorised(node) {
        ...
        for (uint i = 0; i < list.length; i++) {
            ...
            texts[node][list[i].key] = Validator.appendTimestamp(list[i].value, t);
        }
    }
    ...
    ) public authorisedOrOwner(node) {
        require(!Validator.isPhotochromicRecord(key));
        texts[node][key] = Validator.appendTimestamp(value, 0);
    ...
    function validatedText(bytes32 node, string calldata key) public view returns (VerificationStatus, string memory, uint32) {
        ...
        (string memory v, uint32 t) = Validator.removeTimestamp(value);
    ...
    function supportsInterface(bytes4 interfaceID)
        ...
        pure
        virtual
        override(ABIResolver, AddrResolver, ContentHashResolver, DNSResolver, InterfaceResolver, NameResolver, PubkeyResolver)
        returns (bool)
    {
...
library Validator {
    ...
    function packPhotochromicRecord(string memory userId, string[DATA_FIELDS] calldata contents, uint32 t) public pure returns (string memory) {
        return string(abi.encodePacked(
            abi.encodePacked(
                ...
                appendTimestamp(contents[0], t),
                uint8(bytes(contents[1]).length + 4),
                appendTimestamp(contents[1], t),
                uint8(bytes(contents[2]).length + 4),
                appendTimestamp(contents[2], t)
            ),
            uint8(bytes(contents[3]).length + 4),
            appendTimestamp(contents[3], t),
            uint8(bytes(contents[4]).length + 4),
            appendTimestamp(contents[4], t),
            uint8(bytes(userId).length + 4),
            appendTimestamp(userId, t)
        ));
    }

    function appendTimestamp(string memory str, uint32 ts) public pure returns (string memory) {
        bytes memory value = bytes(str);
        bytes4 t = bytes4(ts);
    ...
    function removeTimestamp(string memory str) public pure returns (string memory, uint32) {
        bytes memory value = bytes(str);
