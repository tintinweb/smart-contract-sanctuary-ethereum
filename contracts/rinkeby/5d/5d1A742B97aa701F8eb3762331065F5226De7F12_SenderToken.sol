...
interface IReceiverToken {
  function initialMintReceiver(address to, uint256 tokenId, string memory _keyword) external;
 }
...
contract SenderToken is ERC1155, Ownable {
  ...
  // receiver token contract
  IReceiverToken receiverContract;

  /* ========== MODIFIERS ========== */

  /* ========== EVENTS ========== */
  event SenderInitialMint(address indexed to, uint tokenId, uint timestamp, string keyword);
  event SenderMint(address indexed to, uint tokenId, uint timestamp);
  ...
  mapping (uint256 => string) public getUri;
  ...
  function mintSender(uint256 tokenId, string memory _keyword) external {
    ...
    if (_exists[tokenId]) {
      ...
      emit SenderMint(msg.sender, tokenId, block.timestamp);
      return;
    }
    ...
    _mint(msg.sender, tokenId, 1, new bytes(0));
    receiverContract.initialMintReceiver(msg.sender, tokenId, _keyword);
    ...
    emit SenderInitialMint(msg.sender, tokenId, block.timestamp, _keyword);
  }
  ...
  function burn(address from, uint tokenId, uint amount) external {
    ...
    emit SenderBurn(from, tokenId, amount, block.timestamp);
  }
