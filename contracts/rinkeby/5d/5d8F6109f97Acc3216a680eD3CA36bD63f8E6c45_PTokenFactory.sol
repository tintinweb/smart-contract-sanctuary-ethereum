...
contract PTokenFactory is Ownable {
    ...
    mapping(uint256 => address) createdTokens;

    function createToken(
        string memory name,
        string memory symbol,
        address ProptoContract,
        uint256 VaultId,
        address creator
    ) public returns (address tokenAddress) {
        PToken token = new PToken(name, symbol, ProptoContract, VaultId);
        token.transferOwnership(creator);
        uint256 index = _indexCounter.current();
        _indexCounter.increment();
        tokenAddress = address(token);
        createdTokens[index] = tokenAddress;
    }
}
...
contract PToken is ERC20, Ownable {
    ...
    constructor(
        string memory name,
        string memory symbol,
        address _ProptoCoreContract,
        uint256 _VaultId
    ) ERC20(name, symbol) {
        ProptoCoreContract = _ProptoCoreContract;
        VaultId = _VaultId;
        _mint(ProptoCoreContract, TOTALSUPPLY * 10**decimals());
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 amount
    ) internal override whenProptoEventNotVerified {
        super._beforeTokenTransfer(from, to, amount);
    }

    modifier whenProptoEventNotVerified() {
        (uint8 VStatus, ) = IProptoCore(ProptoCoreContract)
            .ProptoEventVerificationStatusByVaultId(VaultId);
        require(VStatus != 1, "ProptoEvent Verified");
        _;
    }
}
