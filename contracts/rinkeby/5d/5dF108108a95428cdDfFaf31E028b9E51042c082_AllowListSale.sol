...
contract AllowListSale is Ownable {
    ...
    mapping(address => uint256) private _limitPerAddress;
    mapping(address => uint256) private _requirePerAddressLimit;
    mapping(address => uint256) private successfulBuyers;
    ...
    constructor(address _contractAddress, bytes32 _merkleRoot) {
        ...
        _limitPerAddress[_contractAddress] = 1;
        _requirePerAddressLimit[_contractAddress] = 1;
    }
    ...
    function allowlistMint(
        ...
        require(
            _requirePerAddressLimit[_contract] == 0 ||
                (_requirePerAddressLimit[_contract] != 0 &&
                    successfulBuyers[_msgSender()] <
                    _limitPerAddress[_contract]),
            "Already minted max"
        );
        if (_quantity == 0) {
    ...
    function publicMint(address _contract, uint256 _quantity) public payable {
        require(
            _requireAllowlist[_contract] == 0,
    ...
    function setLimitPerAddress(address _contract, uint256 _newLimit)
        public
        onlyOwner
    {
        _limitPerAddress[_contract] = _newLimit;
    }

    function togglePerAddressLimit(address _contract, uint256 _require)
        public
        onlyOwner
    {
        _requirePerAddressLimit[_contract] = _require;
    }
    ...
    function batchSetContracts(
        ...
        uint256 _requireAddressLimit
    ) public onlyOwner {
        for (uint256 c = 0; c < _contracts.length; c += 1) {
            _requireAllowlist[_contracts[c]] = _requireAllowList;
            _limitPerAddress[_contracts[c]] = _addressLimit;
            _requirePerAddressLimit[_contracts[c]] = _requireAddressLimit;
        }
    }
}
