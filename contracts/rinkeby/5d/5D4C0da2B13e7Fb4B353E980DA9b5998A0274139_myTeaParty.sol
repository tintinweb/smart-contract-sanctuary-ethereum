...
contract myTeaParty is Ownable, Pausable, ERC721A, ReentrancyGuard {
    using Strings for uint256;

    bytes32 public root;

    string private baseURI; // public
    string private notRevealedURI; // public
    string private baseExtension; //= ".json";
    address private proxyRegistryAddress; // no private/public

    // control times
    uint256 private constant MAX_SALE_TIMESTAMP = 2**256 - 1;
    uint256 private startPreSaleTimestamp = MAX_SALE_TIMESTAMP;
    uint256 private endPreSaleTimestamp = MAX_SALE_TIMESTAMP;
    uint256 private startPublicSaleTimestamp = MAX_SALE_TIMESTAMP;
    uint256 private revealTimestamp = MAX_SALE_TIMESTAMP;

    uint256 public immutable mintAmountLimit; // = 4;
    uint256 public immutable maxSupply; // = 5000;
    // uint256 public immutable externalSupply; //= 4950;
    // uint256 public immutable marketingSupply; // = 50;
    uint256 private externalSupply; //= 4950;
    uint256 private marketingSupply; // = 50;

    uint256 private constant _presalePrice = 0.1 ether;
    uint256 private constant _publicsalePrice = 0.15 ether;
    ...
    constructor(
        ...
        ERC721A("My Tea Party", "MTP", _mintAmountLimit, _maxSupply)
    // ReentrancyGuard()
    {
        ...
        require(
            (externalSupply + marketingSupply) == maxSupply,
            "Invalid supply values provided."
        );
        proxyRegistryAddress = _proxyRegistryAddress;
        // manually set root + baseURI + timestamps from etherscan
        // updateSaleTimestamp(1661277600, 1661450400, 1661536800, 1662055200);
    }
    ...
    function getSaleStatus() public view returns (uint256) {
        ...
        if (_currTimestamp >= revealTimestamp) {
            ...
            return 3; // Public sale (before reveal)
        } else if (_currTimestamp >= endPreSaleTimestamp) {
            return 2; // After presale (before public sale)
        } else if (_currTimestamp >= startPreSaleTimestamp) {
            return 1; // Pre-sale
        }
        return 0; // Before pre-sale
    }
    ...
    function presaleMint(uint256 _amount, bytes32[] calldata _proof)
        ...
        nonReentrant
    {
        require(!paused(), "The contract is paused."); // remove if using latest erc721a
        bytes32 _leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_proof, root, _leaf), "Invalid proof!");
        require(getSaleStatus() == 1, "It's not time for presale.");
        require(_amount > 0, "Must mint more than 0"); // remove if using latest erc721a
        require(
    ...
    function marketingMint(uint256 _quantity, address _account)
        external
        onlyOwner
        nonReentrant
    {
        // should only mint to standalone wallet meant for marketing mint only
        require(!paused(), "The contract is paused"); // remove if using latest erc721a
        require(getSaleStatus() == 5, "Invalid timestamps.");

        require(
            _quantity > 0,
            "marketing mint quantity must be greater than 0"
        ); // remove if using latest erc721a
        require(
    ...
    function refundIfOver(uint256 _price) private {
        ...
        if (msg.value > _price) {
            // TODO: replace with Address's sendValue() method
            // payable(msg.sender).transfer(msg.value - _price);
            Address.sendValue(payable(msg.sender), msg.value - _price);
            // TODO: check if emit event is needed here
        }
    }
    ...
    function _beforeTokenTransfers(
        ...
        // check if this means we can't put nft on sale/mint/trnasfer if paused
