...
contract TradingRewardsDistributor is IShares, ITradingRewardsDistributor, Pausable, ReentrancyGuard {
  ...
  uint16 public constant TRADING_REWARD_SHARES = 2000;
  ...
  modifier onlyAdmin() {
    require(nftspacexAccessControls.hasAdminRole(_msgSender()), "TRD: only admin");
    _;
  }
  ...
  function shares() public pure override returns (uint16) {
    return TRADING_REWARD_SHARES;
  }

  function maxBalanceNST() public view override returns (uint256) {
    return (nftspacexToken.maxTotalSupply() * TRADING_REWARD_SHARES) / TOTAL_SHARES;
  }
  ...
  function claim(uint256 amount, bytes32[] memory merkleProof) external override whenNotPaused nonReentrant {
    require(!hasUserClaimedForRewardRound[currentRewardRound][_msgSender()], "TRD: Already claimed");
    (bool claimStatus, uint256 adjustedAmount) = _canClaim(_msgSender(), amount, merkleProof);
    require(claimStatus, "TRD: invalid proof");
    require(maximumAmountPerUserInCurrentTree >= amount, "TRD: excceed max amount");
  ...
  function updateTradingRewards(bytes32 merkleRoot, uint256 newMaximumAmountPerUser) external onlyAdmin {
    require(!merkleRootUsed[merkleRoot], "TRD: Already used");
  ...
  function withdrawTokenRewards(uint256 amount) external onlyAdmin whenPaused {
    require(block.timestamp > (lastPausedTimestamp + BUFFER_ADMIN_WITHDRAW), "TRD: still early");
