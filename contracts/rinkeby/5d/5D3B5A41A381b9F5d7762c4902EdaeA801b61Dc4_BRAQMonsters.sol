...
contract BRAQMonsters is ERC721A, Ownable, ReentrancyGuard {
    ...
    function whitelistMint(uint256 _mintAmount, uint8 _maxAllowed, bytes32[] calldata _merkleProof) external mintCompliance(_mintAmount) {
            require(whiteListSale, "Whitelist not active."); 
            require(_verifyPresaleEligible(msg.sender, _maxAllowed, _merkleProof),"Address not found on whitelist"); 
            require(totalMinted[msg.sender] + _mintAmount <= uint256(_maxAllowed), "Purchase exceeds max whitelisted count."); 
            _safeMint(msg.sender, _mintAmount); 

    }

    function mintReserved(address _to, uint256 _mintAmount) external mintCompliance(_mintAmount) onlyOwner {
        require(totalMinted[msg.sender] + _mintAmount <= reservedMonsters, "All reserved Monsters have not been minted"); 
