...
// File: contracts/TokenStakingMMF.sol
...
contract TokenStakingMMF is Ownable {
    using SafeMath for uint256;
    ...
    function changeLockTime(uint _durationInSeconds) external onlyOwner{
        lockTime = _durationInSeconds;

    }
    function whenUnlockRewards(address _user) public view returns (uint) {
        UserInfo storage user = userInfo[_user];

        return lockTime + user.timestampDepositClaim;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        user.timestampDepositClaim = block.timestamp ;
    ...
    function withdraw(uint256 _amount) public {
        ...
        require(user.timestampDepositClaim + lockTime < block.timestamp, "Less than locktime period");
        updatePool(0);
        uint256 pending = user.amount.mul(pool.accRewardTokenPerShare).div(1e30).sub(user.rewardDebt);
        if(pending > 0) {
            uint256 currentRewardBalance = rewardBalance();
