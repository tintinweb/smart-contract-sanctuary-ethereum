...
library MapPurchase {
  ...
  function addValue(
    ...
    if (containsValue(_record, _purchaser)) return; // already exists
...
library Utilities {
  ...
  /**
   * @dev Returns the bonus of tokens and NFT Persona whitelist allocated by purchased number.
   */
  ...
  function getPublicBonus(uint256 purchased, uint16 unit) internal pure returns (uint16, uint8) {
    if (purchased >= 0.5 ether && purchased < 1 ether) {
      return ((unit / 100) * 2, 2); // get 2% bonus + whitelist 2 persona
    }
