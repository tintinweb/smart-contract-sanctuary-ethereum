...
/// @title Bad Cards Genesis
contract BadCardsGenesis is Ownable, Pausable, ERC721A {
  ...
  constructor(string memory newUnrevealedURI, bytes32 newRoot) ERC721A("Bad Cards Genesis", "BAD") {
  ...
  /// @notice Buy one or more genesis tokens.
  /// @dev If minting on public sale, just input an empty array as the proof.
  /// @param amount Amount of tokens to buy.
  /// @param proof Whitelist proof for presale purchases.
  function buy(uint256 amount, bytes32[] memory proof) external payable {
    require(saleState == 1 || saleState == 2, "Genesis sale is closed");
    require(totalSupply() < SUPPLY, "Genesis suply exceeded");

    if (saleState == 1) {
      // Presale
      require(amount > 0 && amount <= perWL, "Invalid buy amount");
      require(msg.value == amount * PRICE_WL, "Invalid ether amount");
      require(amount + boughtPresale[msg.sender] <= perWL, "Whitelist buy cap exceeded");
      ...
      require(msg.value == amount * PRICE, "Invalid ether amount");
  ...
  /// @notice Set per whitelist buy amount.
  /// @param newPerWL New per whitelist buy amount.
