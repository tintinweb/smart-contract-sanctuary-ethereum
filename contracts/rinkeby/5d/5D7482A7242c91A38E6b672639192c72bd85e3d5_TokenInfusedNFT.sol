...
contract TokenInfusedNFT is ERC721Enumerable, ReentrancyGuard {
    ...
    function createDeal(
        ...
        TransferHelper.transferPayment(
            weth,
            token,
            payable(msg.sender),
    ...
    function buyDeal(uint256 dealId, uint256 numberOfNFTs)
        external
        payable
        nonReentrant
    {
    ...
    function redeemNFT(uint256 futureId) external nonReentrant {
        ...
        require(
            future.unlockDate < block.timestamp && future.amount > 0,
            "NFT04"
        );
        _burn(futureId);
        ...
        TransferHelper.withdrawPayment(
            weth,
            token,
            payable(msg.sender),
            future.amount
    ...
    function setBaseURI(string memory tokenBaseURI) external {
        require(msg.sender == admin, "OTC10");
    ...
    function batchMint(
        ...
        require(
            holders.length == amounts.length &&
                amounts.length == unlockDates.length,
            "OTC11"
        );
