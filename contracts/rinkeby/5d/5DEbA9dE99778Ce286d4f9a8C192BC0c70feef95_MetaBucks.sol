...
contract MetaBucks is ERC721A, Ownable {
    ...
    mapping(address => bool) public mintedTBforFreeMintAddress;
    mapping(address => uint256) addressBlockBought;

    address public OWNER_ADDRESS = 0xCb583ace502f4F8b9f72309c56B1195606e68ad9; //OWNER
    address public RL_ADDRESS = 0xc9b5553910bA47719e0202fF9F617B8BE06b3A09; //ROYAL LABS
    ...
    constructor(
        ...
        bytes32 _raffleRoot,
        address _signer) ERC721A("MetaBucks", "METABUCKS",25,3333) {
    ...
    function freeMint(bytes32[] calldata freeMintProof) external isSecured(1) {
        ...
        require(!mintedTBforFreeMintAddress[msg.sender],"CANNOT_MINT_MORE_THAN_ALLOWED");
        require(freeMintCount + 1 <= FREE_SUPPLY, "EXCEEDS_FREE_MINT_SUPPLY" );
        require(1 + totalSupply() <= MAX_SUPPLY,"NOT_ENOUGH_SUPPLY");

        mintedTBforFreeMintAddress[msg.sender] = true;
    ...
    function raffleMint(bytes32[] memory proof) external isSecured(3) payable{
        require(MerkleProof.verify(proof, raffleRoot, keccak256(abi.encodePacked(msg.sender))),"PROOF_INVALID");
        require(1 + totalSupply() <= MAX_SUPPLY,"NOT_ENOUGH_SUPPLY");
        require(1 + raffleMintCount <= (WL_SUPPLY - wlMintcount) + RAFFLE_SUPPLY,"NOT_ENOUGH_PRESALE_SUPPLY");
        require(numberMinted(msg.sender) < 1,"EXCEED_PRAFFLE_MINT_LIMIT");
        require(msg.value == WL_PRICE * 1 , "NOT_ENOUGH_ETH");
    ...
    function publicMint(uint64 expireTime, bytes memory sig) external isSecured(4) payable {
        ...
        require(1 + totalSupply() <= MAX_SUPPLY,"NOT_ENOUGH_SUPPLY");
        require(numberMinted(msg.sender) + 1 <= PUBLIC_MINT_LIMIT,"EXCEED_MINT_LIMIT");
    ...
    function setOwnerWithdrawAddress(address _owner) external onlyOwner{
        OWNER_ADDRESS = _owner;
    }

    function setRLAddress(address _rlAddress) external onlyOwner{
        RL_ADDRESS = _rlAddress;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        payable(RL_ADDRESS).transfer((balance * 850) / 10000);
