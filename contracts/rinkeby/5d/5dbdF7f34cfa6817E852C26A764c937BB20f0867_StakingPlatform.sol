...
interface IStakingPlatform {
     ...
     */
    event Divide(address indexed owner, uint256 compoundTime, uint256 amount);
    
    /**
     * @dev Emitted when harvest
     */
    event Harvest(uint256 harvestTime, uint256 amount);
}
...
contract StakingPlatform is IStakingPlatform, Ownable, ReentrancyGuard {
        ...
        returns (bool)
    {
        ...
        uint256 sumClaimedAmount = getArraySum(claimedAmount);
        uint256 amountOut = address(this).balance - (sumLastRewardAmount - sumClaimedAmount);
        require((startPeriod > 0) && (_totalStaked > 0), "Harvest: not enough");
        require((amountOut > 0), "Harvest: Too little amount");

        for (uint i = 0; i < userList.length; i++) {
            if(staked[userList[i]] > 0) {
                _rewardsToClaim[userList[i]] += staked[userList[i]] * amountOut / _totalStaked;
                emit Divide(userList[i], block.timestamp, staked[userList[i]] * amountOut / _totalStaked);
            }
        }
        ...
        forceHarvest = false;
        emit Harvest(lastRewardTime, amountOut);
        return true;
    }
    ...
    function _claimRewards() private {
        ...
        claimedAmount.push(rewardsToClaim);
