...
contract QRFID_gold is IERC1400, Ownable {
    ...
    function distributeToken(
        ...
        require(bytes(_userId).length == 24, "Invalid userId");
        require(bytes(_goldId).length > 0, "Empty Gold Id");
    ...
    function redeemFrom(
        ...
        uint256 _value,
        bytes calldata _data
    ) external override isUserDataValid(_data) {
    ...
    function transferWithData(
        ...
        uint256 _value,
        bytes calldata _data
    ) public override isUserBlackListed(_to) isUserDataValid(_data) {
    ...
    function transferFromWithData(
        ...
        uint256 _value,
        bytes calldata _data
    ) external override isUserBlackListed(_to) isUserDataValid(_data) {
    ...
    function canTransfer(
        ...
        if (!isBytesEqual(abi.encodePacked(bytes1(0)), _data)) {
            bytes memory userKycUri = bytes(_documents[bytes32(_data)].uri);
