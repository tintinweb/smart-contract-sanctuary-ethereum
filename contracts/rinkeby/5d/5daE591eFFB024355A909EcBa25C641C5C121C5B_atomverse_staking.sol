...
contract atomverse_staking is Ownable, IERC721Receiver {
    ...
    mapping(uint256 => Stake) public cyroChambers;
    ...
    function stake(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(cyroChambers[tokenId].tokenId == 0, "already staked");

            atomverse_nft.transferFrom(msg.sender, address(this), tokenId);
            emit NFTStaked(msg.sender, tokenId, block.timestamp);

            cyroChambers[tokenId] = Stake({
                owner: msg.sender,
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            tokenId = tokenIds[i];
            Stake memory staked = cyroChambers[tokenId];
            require(staked.owner == msg.sender, "not an owner");

            delete cyroChambers[tokenId];
    ...
    function onERC721Received(
        ...
        require(
            from == address(0x0),
            "Cannot send nfts to cyroChambers directly"
        );
...
contract nuclei is ERC20, ERC20Burnable, Ownable {
  ...
  constructor() ERC20("Nuclei", "Nuclei") { }
