...
contract WhitelistNFT is ERC721A, Ownable {
  ...
  uint256 constant public MAX_MINT = 1;
  uint256 constant public WHITELIST_SALE_PRICE = 0 ether;
  uint256 constant public WHITELIST_MAX_MINT = 2;
  uint256 constant public TEAM_MINT_AMOUNT = 80;
  address curryAddress;
  ...
  constructor() ERC721A("WhitelistNFT", "NFT") {
  }
  ...
  function mint(uint256 _nonce, uint256 quantity) external payable callerIsUser{
    ...
    require(quantity <= MAX_MINT, "NFT: Max Mint Exceeded");
    require((quantity * NFT_PRICE)  <= msg.value, "NFT: Insufficient Funds");
    uint256 currentSupply = totalSupply();
    require((currentSupply +quantity) < MAX_NFTS, "NFT: May be Sold out, try a smaller quantity");
  ...
  function whitelistMint(uint256 _nonce, bytes32[] memory _merkleProof, uint256 quantity) external payable callerIsUser{
    require(whiteListSale, "Minting is on Pause");
    require((totalSupply() + quantity) <= MAX_NFTS, "Cannot mint beyond max supply");
    require((totalWhitelistMint[msg.sender] + quantity)  <= WHITELIST_MAX_MINT, "Cannot mint beyond whitelist max mint");
    require(msg.value >= (WHITELIST_SALE_PRICE * quantity), "Payment is below the price");
    //create leaf node
    bytes32 txid = getTransferHash(msg.sender, _nonce);
    require(!usedNonces[txid], "Reused Nonce");
    usedNonces[txid] = true;
    ...
    totalWhitelistMint[msg.sender] += quantity;
    _safeMint(msg.sender, quantity);
    }
  ...
  function getTransferHash(address _to, uint256 _nonce) public view returns (bytes32 txHash) {
  ...
  function curryExplode(uint256 tokenId) external onlyOwner{
