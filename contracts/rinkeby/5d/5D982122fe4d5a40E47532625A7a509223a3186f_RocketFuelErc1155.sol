...
contract RocketFuelErc1155 is ERC1155Supply, Ownable {
    struct MintContext {
        bool isSaleActive;
        bool isMintListSet;
        ...
        uint256 remainingTopGunMintCount;
    }
    ...
    uint256 private payoutPercentage;
    ...
    address payable public incubatorPayoutAddress;
    ...
    constructor(
        ...
        address _incubatorPayoutAddress,
        uint256 _payoutPercentage
    ) ERC1155(_uriBase) {
        name = _name;
        symbol = _symbol;
        // proxyRegistryAddress = _proxyRegistryAddress;
        incubatorPayoutAddress = payable(_incubatorPayoutAddress);
    ...
    function getMintContext() external view returns (MintContext memory) {
        return
            MintContext({
                ...
                remainingGlobalPresaleMintCount: configs[TOKEN_ID_GLOBAL].maxPresaleMint -
                    numberMinted[TOKEN_ID_GLOBAL][msg.sender],
                remainingCaptainPresaleMintCount: configs[TOKEN_ID_CAPTAIN].maxPresaleMint -
                    numberMinted[TOKEN_ID_CAPTAIN][msg.sender],
                remainingTopGunMintCount: configs[TOKEN_ID_TOP_GUN].maxPublicMint -
                    numberMinted[TOKEN_ID_TOP_GUN][msg.sender]
            });
    }
    ...
    function gift(
        ...
        for (uint256 i = 0; i < _receivers.length; i++) {
            numberMinted[_tokenId][msg.sender] += _numberOfTokens[i];
    ...
    function mintTopGun(bytes32[] calldata proof, uint256 _numberOfTokens) external payable {
        uint256 _tokenId = TOKEN_ID_TOP_GUN;
        require(isTopGunSaleActive, "Sale is not active");
        require(isOnTopGunlist(proof), "not on top gun list");
        require(
            _numberOfTokens <= configs[TOKEN_ID_TOP_GUN].maxPublicMint - numberMinted[TOKEN_ID_TOP_GUN][msg.sender],
    ...
    function mintPresale(
        ...
        require(
            _numberOfTokens <= configs[_tokenId].maxPresaleMint - numberMinted[_tokenId][msg.sender],
            "Exceeded max purchase"
        );
    ...
    function withdraw() external onlyOwner {
        (bool incubatorSuccess, ) = incubatorPayoutAddress.call{
            value: (address(this).balance * payoutPercentage) / 100
        }("");
