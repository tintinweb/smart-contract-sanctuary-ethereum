...
import "@manifoldxyz/creator-core-solidity/contracts/extensions/ERC721/ERC721CreatorExtensionBurnable.sol";
import "@manifoldxyz/creator-core-solidity/contracts/extensions/CreatorExtensionBasic.sol";
import "@manifoldxyz/libraries-solidity/contracts/access/AdminControl.sol";
import "@openzeppelin/contracts/utils/Counters.sol";
import {Allowlist} from "./Allowlist.sol";
import {AllowlistAddress} from "./AllowlistAddress.sol";
    ...
    ERC721CreatorExtensionBurnable
{
    ...
    constructor(
        ...
        _allowlistAddress = allowlistAddress;
    }
        ...
        returns (bool)
    {
        return
            ...
            AllowlistAddress.supportsInterface(interfaceId) ||
            CreatorExtensionBasic.supportsInterface(interfaceId) ||
            ERC721CreatorExtensionBurnable.supportsInterface(interfaceId);
    }

    function mint(address to, bytes32[] calldata merkleProof) external payable {
        require(
            block.timestamp >= _launchDate,
            "minting not enabled yet, please wait"
        );
        require(
            _tokenAmountTracker.current() < _maxSupply,
            "Maximum supply reached"
        );
        require(msg.value >= _mintPrice, "Insuficient funds");

        require(ifEnabledCheckAllowlist(msg.sender), "Not on allowlist");
        require(
            ifEnabledCheckAllowlistAddress(merkleProof),
...
abstract contract Allowlist is AdminControl {
    ...
    // struct for the unique identifier of a token between contract address and tokenId
    struct ContractAddressTokenId {
        address contractAddress;
        uint256 tokenId;
    }

    // Array with the contract address and tokenId pair
    ContractAddressTokenId[] contractAddressTokenIdArray;

    // boolean for allowlist enabled or not
    bool _allowlist;

    // interfaces for erc1155 and erc721
    ...
    function ifEnabledCheckAllowlist(address sender) internal returns (bool) {
        return !_allowlist || isOnAllowlist(sender);
    }

    function isOnAllowlist(address sender) internal view returns (bool) {
        // query support of each interface in _interfaceIds
        for (uint256 i = 0; i < contractAddressTokenIdArray.length; i++) {
            // if one is supported returns
            ContractAddressTokenId
                memory contractAddressTokenId = contractAddressTokenIdArray[i];

            if (
                quantityOf(
                    INFT(contractAddressTokenId.contractAddress),
                    sender,
                    contractAddressTokenId.tokenId
                ) > 0
            ) {
                return true;
            }
    ...
    function setAllowlist(
        ContractAddressTokenId[] calldata _contractAddressTokenIdArray
    ) external adminRequired {
        delete contractAddressTokenIdArray;
        ...
        for (uint256 i = 0; i < _contractAddressTokenIdArray.length; i++) {
            address contractAddress = _contractAddressTokenIdArray[i]
                .contractAddress;

            if (!contractAddress.supportsOneInterface(interfaces)) {
                revert ContractWithInvalidInterface();
            }

            contractAddressTokenIdArray[i] = _contractAddressTokenIdArray[i];
        }
    }

    function unsetAllowlist() external adminRequired {
        delete contractAddressTokenIdArray;
    }
...
abstract contract AllowlistAddress is AdminControl {
    bytes32 public merkleRoot;

    // boolean for allowlist enabled or not
    bool _allowlistAddress;

    function toggleAllowlistAddress() external adminRequired {
        _allowlistAddress = !_allowlistAddress;
    }

    function ifEnabledCheckAllowlistAddress(bytes32[] calldata merkleProof)
        internal
        returns (bool)
    {
        return !_allowlistAddress || isOnAllowlistAddress(merkleProof);
    }

    function isOnAllowlistAddress(bytes32[] calldata merkleProof)
        internal
        view
        returns (bool)
    {
...
library ERC165Checker {
    ...
    /**
     * @dev Returns true if `account` supports one of the interfaces defined in
     * `interfaceIds`. Support for {IERC165} itself is queried automatically.
     *
     * Batch-querying can lead to gas savings by skipping repeated checks for
     * {IERC165} support.
