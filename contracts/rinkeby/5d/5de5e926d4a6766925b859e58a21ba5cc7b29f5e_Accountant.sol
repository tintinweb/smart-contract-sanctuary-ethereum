...
contract SeniorPool {
    ...
    constructor(address _seniorPoolAddress, address usdcAddress) {
        seniorPoolAddress = _seniorPoolAddress;
        USDc = IUSDC(usdcAddress);
    }

    // make it only total investment
    struct investor {
        uint256[] investments;
    ...
    modifier onlyInvestor() {
        if (!checkInvestor(msg.sender)) {
            revert SeniorPool__NotInvestor();
        }
        _;
    }
    ...
    function withdrawUser(uint256 withdrawAmount) public payable onlyInvestor {
        investor storage Investor = addressToInvestor[msg.sender];
        if (Investor.totalInvestment < withdrawAmount) {
    ...
    function lend(address projectAddress, uint256 $USDC)
        ...
        onlyAdmin
    {
        USDc.transferFrom(seniorPoolAddress, projectAddress, $USDC * 10**6);
        //TBD
    }
}
