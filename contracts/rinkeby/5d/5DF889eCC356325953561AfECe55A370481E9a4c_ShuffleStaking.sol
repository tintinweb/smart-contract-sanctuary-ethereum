...
contract ShuffleStaking is Ownable {
    using SafeERC20 for IERC20;

    address public immutable MINT;
    address public immutable sMINT;
        ...
        returns (bool)
    {
        ...
        IERC20(MINT).safeTransferFrom(msg.sender, address(this), _amount);
        ...
        warmupInfo[_recipient] = Claim({
            ...
            gons: info.gons + (IsMINT(sMINT).gonsForBalance(_amount)),
            expiry: epoch.number + (warmupPeriod),
            lock: false
        });

        IERC20(sMINT).safeTransfer(warmupContract, _amount);
        return true;
    }
    ...
    function claim(address _recipient) public {
        ...
        if (epoch.number >= info.expiry && info.expiry != 0) {
            ...
            IWarmup(warmupContract).retrieve(
                _recipient,
                IsMINT(sMINT).balanceForGons(info.gons)
            );
        }
    }
    ...
    function forfeit() external {
        ...
        IERC20(MINT).safeTransfer(msg.sender, info.deposit);
    }
    ...
    function unstake(uint256 _amount, bool _trigger) external {
        ...
        IERC20(sMINT).safeTransferFrom(msg.sender, address(this), _amount);
        IERC20(MINT).safeTransfer(msg.sender, _amount);
    }
    ...
    function rebase() public {
        if (epoch.endBlock <= block.number) {
            IsMINT(sMINT).rebase(epoch.distribute, epoch.number);
            ...
            uint256 staked = IsMINT(sMINT).circulatingSupply();
    ...
    function contractBalance() public view returns (uint256) {
        return IERC20(MINT).balanceOf(address(this)) + (totalBonus);
    }
    ...
    function giveLockBonus(uint256 _amount) external {
        ...
        IERC20(sMINT).safeTransfer(locker, _amount);
    }
    ...
    function returnLockBonus(uint256 _amount) external {
        require(msg.sender == locker);
        totalBonus = totalBonus - (_amount);
        IERC20(sMINT).safeTransferFrom(locker, address(this), _amount);
    }
