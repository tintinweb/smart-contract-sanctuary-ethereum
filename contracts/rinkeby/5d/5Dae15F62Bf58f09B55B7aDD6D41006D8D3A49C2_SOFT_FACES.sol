...
abstract contract ERC721URIStorage is ERC721 {
    ...
    function calcEmotion(uint256 tokenId) public view returns (string memory) {
        ...
        //poked emotion
        if (
            getMonth(block.timestamp) == getMonth(pokeDay) &&
            getDay(block.timestamp) == getDay(pokeDay)
        ) {
            return (pokeEmotion);
        }
    ...
    function isBirthday() public view virtual returns (bool) {
        for (uint z = 0; z < 77; z++) {
            //require(_exists(z));
            if (
    ...
    function comparePrices() public view virtual returns (int) {
        int result;
    ...
    function poke(string memory pokeEmo) private {
        ...
        returns (string memory)
    {
        ...
        jsonSlot.json = concatenate(
            unicode'{"description":"🟢 512 generative Soft Face NFTs that interact on chain 🟣 Project by Damjanski 2022 🟡","external_url":"http://something.xyz", "name": "Soft Faces ◑◑ ',
            uint2str(tokenId)
        );
        jsonSlot.json = concatenate(
            jsonSlot.json,
