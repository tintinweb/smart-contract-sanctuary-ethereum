...
contract VisibleFriends is ERC721SmallFriend, EIP712, Ownable, ReentrancyGuard {
    ...
    struct Config {
        uint16 maxSupply;
        uint16 closeFriendSummonLimit;
        uint16 summonLimit;
        bool saleStarted;
    }

    mapping(address => bool) public _closeFriends;
    mapping(address => bool) public _closeFriendsSummoned;
    ...
    function isCloseFriend(address addr) external view returns (bool) {
        return _closeFriends[addr];
    }

    function summonCloseFriends() external nonReentrant {
        Config memory config = _config;
        require(tx.origin == msg.sender, "VisibleFriends: we hates bots");
        require(config.saleStarted, "VisibleFriends: sale is not started");
        require(
            _closeFriends[msg.sender],
            "VisibleFriends: You are not close friend"
        );
        require(
            !_closeFriendsSummoned[msg.sender],
            "VisibleFriends: Can't summon more than once"
        );
        uint256 totalMinted = _totalMinted();
        require(
            totalMinted + config.closeFriendSummonLimit <= config.maxSupply,
            "VisibleFriends: exceed public supply"
        );
        _safeBatchMint(msg.sender, config.closeFriendSummonLimit);
        _closeFriendsSummoned[msg.sender] = true;
        _closeFriends[msg.sender] = false;
    }

    function summonFriends(uint256 amount) external payable nonReentrant {
        Config memory config = _config;
        require(tx.origin == msg.sender, "VisibleFriends: ape hates bots");
        require(config.saleStarted, "VisibleFriends: sale is not started");
        require(
    ...
    function addCloseFriendsList(address[] calldata addresses)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            _closeFriends[addresses[i]] = true;
        }
    }

    function removeCloseFriendsList(address[] calldata addresses)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            _closeFriends[addresses[i]] = false;
        }
    }
}