...
contract CoolLadyClub is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public maxAmountForPublic = 1463; 
    uint256 public maxAmountForPresale = 500;
   ...
   modifier isNotExceedMaxMintPerTx(uint256 amount) {
        ...
        isNotExceedMaxMintPerTx (_mintAmount)
    {
        ...
        require(totalMintOnPublic + _mintAmount <= maxAmountForPublic - totalMintOnPresale, "Not enough remaining supply for public");
        ...
        isPresaleActive
    {
        ...
        require(whiteTokens[msg.sender] + count <= 2, "Max per wallet reached");
        require(count > 0 && count <= 2, "You can mint min 1, maximum 2 Tpkens");
        require(totalMintOnPresale + count <= maxAmountForPresale, "Cannot exceeds max supply");
        _safeMint(msg.sender, count);
        whiteTokens[msg.sender] += count;
        totalMintOnPresale = totalMintOnPresale + count;
        refundIfOver(presalePrice * count);
    }
    ...
    function withdraw() 
        ...
        nonReentrant 
    {
        (bool dev, ) = payable(0xEDB8150008916A0c777E34BB996Da853F052bD17).call{value: address(this).balance * 15 / 100}(""); //TrueNeutral %15
        require(dev);
        (bool st, ) = payable(0x73dF0C88f3B0e553D8bef7605f5750E07A8C36A8).call{value: address(this).balance * 18 / 100}(""); //Clete %15
        require(st);
        (bool hl, ) = payable(0x18456ce597e8469472dCb76e9144A151681cDC52).call{value: address(this).balance * 21 / 100}(""); //Dangros %15
        require(hl);
        (bool cm, ) = payable(0x1BEC0E6266e97f0C7C42aa720959e46598DD8Ce1).call{value: address(this).balance * 18 / 100}(""); //Ashu %10
        require(cm);
        (bool cw, ) = payable(owner()).call{value: address(this).balance * 12 / 100}(""); //Community Wallet %5
        require(cw);
        (bool pm, ) = payable(0x8a8035F056af830B7205c58c1dC037f826fc2B92).call{value: address(this).balance}(""); //Project Manager %40
        require(pm);
    }
