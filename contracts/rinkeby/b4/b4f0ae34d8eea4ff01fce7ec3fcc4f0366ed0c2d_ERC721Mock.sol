...
contract ERC721Mock is ERC721, Ownable {
    ...
    function mintBatch(uint256 numberOfTokens) public {
        ...
        for (uint256 i = mintIndex; i < numberOfTokens; i++) {
            _tokenSupply.increment();
            _safeMint(msg.sender, mintIndex);
        }
    }

    function transfer(address to, uint256[] calldata tokenIds) external {
    ...
    function adminMint(uint256 numberOfTokens) public onlyOwner {
        require(
            _tokenSupply.current() + numberOfTokens <= MAX_SUPPLY,
            "Exceeds max supply"
        );
