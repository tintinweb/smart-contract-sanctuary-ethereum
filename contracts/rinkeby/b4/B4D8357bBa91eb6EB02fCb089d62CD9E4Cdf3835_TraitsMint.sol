...
contract TraitsMint is Ownable {
    ...
    function claimMultiple(uint256[] calldata ids, uint256[] calldata amounts, address account, uint256[] calldata nonces, uint256 deadlineTimestamp, bytes32 signatureR, bytes32 signatureVS) external {
    ...
    function claim(uint256[] calldata ids, uint256[] calldata amounts, address account, uint256 nonce, uint256 deadlineTimestamp, bytes32 signatureR, bytes32 signatureVS) external {
        require(msg.sender == account || _claimers[msg.sender], "Not allowed to claim");
        require(deadlineTimestamp == 0 || deadlineTimestamp > block.timestamp, "Deadline to claim has passed");
...
// SPDX-License-Identifier: MIT
// Adapted from OpenZeppelin Contracts v4.4.1 (token/ERC1155/ERC1155.sol)
...
/**
 * @dev Extended implementation of the basic standard multi-token.
 ...
 * and Enjin: https://github.com/enjin/erc-1155
 *
 * _Available since v3.1._
 *
 */

contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI, OwnableTokenAccessControl {
    ...
    mapping(address => mapping(address => Approval)) private _operatorApprovals;
     ...
     * - `account` cannot be the zero address.
     */
    function balanceOfBatchIds(address account, uint256[] memory ids)
        public
        view
        virtual
        returns (uint256[] memory)
    ...
    function isApprovedForAll(address account, address operator) public view virtual override returns (bool) {
        return _operatorApprovals[account][operator] == Approval.Approved;
    }
    ...
    function safeTransferFrom(
        ...
        require(
            from == _msgSender() || _isApprovedForAllOrHasAccess(from, _msgSender(), Access.Transfer),
    ...
    /**
     * @dev xref:ROOT:erc1155.adoc#batch-operations[Batched] version of {_safeMint}.
     *
     * Requirements:
     *
    ...
    function burn(
        ...
        require(
            account == _msgSender() || _isApprovedForAllOrHasAccess(account, _msgSender(), Access.Burn),
            "ERC1155: caller is not owner nor approved"
        );
    ...
    function _isApprovedForAllOrHasAccess(
        address account,
        address operator,
        Access access
    ) internal virtual returns (bool) {
        Approval aproval = _operatorApprovals[account][operator];
        if (aproval == Approval.Approved) {
            return true;
        }
        if (aproval == Approval.NotApproved) {
            return false;
        }

        return _hasAccess(access, operator);
    }
    ...
    function _setApprovalForAll(
        ...
        _operatorApprovals[owner][operator] = approved ? Approval.Approved : Approval.NotApproved;
