...
contract NFTContract is ERC721Enumerable, ERC721Royalty, Pausable, Ownable, ReentrancyGuard {
    ...
    address public royaltyReceiver = 0xB5f9836414607cE5D1241F4a02DF6094c32d0064;
    address public WeCareWallet = 0xfE4abecf0480CdD528012226054DDB0A3dA18106;
    ...
    mapping(uint => uint) public roundLimits;
    ...
    function mint(uint _mintAmount) external payable whenNotPaused nonReentrant {
        ...
        require(supply < maxMintPerRound[activeRound], "Max Mint amount reached for this round");
    ...
    function _getTokenRound(uint _tokenId) internal view returns (uint) {
        for (uint _i = 1; _i <= totalRound; _i++) {
            uint _roundLimit = roundLimits[_i];
    ...
    function setRoyalityForToken(uint256 _tokenId, address _receiver, uint96 _feeNumerator) external onlyOwner {
    ...
    function setPrice(uint[] calldata _roundIds, uint[] calldata _prices) external onlyOwner {
    ...
    function setWhitelistPrice(uint[] calldata  _roundIds, uint[] calldata _prices) external onlyOwner {
        uint _length = _roundIds.length;
        require(_length <= totalRound, "limit excceded");
    ...
    function setMaxMintPerRound(uint[] calldata _roundIds, uint[] calldata _amounts) external onlyOwner {
        uint _length = _roundIds.length;
        require(_length <= totalRound, "limit excceded");
    ...
    // 1 => 2500, 2 => 5000, 3 => 10000
    function setRoundLimits(uint[] calldata _roundIds, uint[] calldata _limits) external onlyOwner {
        uint _length = _roundIds.length;
        require(_length <= totalRound, "limit excceded");

        for (uint i = 0; i < _length; i++) {
            roundLimits[_roundIds[i]] = _limits[i];
        }
    }
    ...
    function withdraw() external onlyOwner {
        (bool wcr, ) = payable(WeCareWallet).call{value: address(this).balance * 5 / 100}("");
        require(wcr);
