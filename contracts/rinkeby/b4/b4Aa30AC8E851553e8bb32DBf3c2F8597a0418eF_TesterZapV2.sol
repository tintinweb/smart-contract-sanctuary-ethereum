...
contract TesterZapV2 is Ownable, ReentrancyGuard {
    ...
    IDXswapFactory public immutable dxSwapFactory;
    IDXswapRouter public immutable dxSwapRouter;
    ...
    constructor(
        ...
        dxSwapFactory = IDXswapFactory(_factory);
        dxSwapRouter = IDXswapRouter(_router);
    ...
    function zapInFromToken(
...

        {
            ...
            amountFrom = (IERC20(token).balanceOf(address(this))) - (previousBalance);
        }
        ...
        amountTo = _zapInFromToken(amountFrom, amount0Min, amount1Min, pathToPairToken0, pathToPairToken1, router);
    ...
    function zapOutToToken(
        ...
        IDXswapPair pairFrom = IDXswapPair(_getFactoryPair(path0[0], path1[0], router));
        amountTo = _zapOutToToken(pairFrom, amountFrom, amountToMin, path0, path1, msg.sender, router);
    ...
    function zapOutToNativeCurrency(
        ...
        IDXswapPair pairFrom = IDXswapPair(_getFactoryPair(path0[0], path1[0], router));
        amountTo = _zapOutToToken(pairFrom, amountFrom, amountToMin, path0, path1, address(this), router);
    ...
    function _zapInFromToken(
        ...
        uint256 amount0 = _swapExactTokensForTokens(amountFromToken0, 0, pathToPairToken0, address(this), rouuter);
        uint256 amount1 = _swapExactTokensForTokens(amountFromToken1, 0, pathToPairToken1, address(this), rouuter);
    ...
    function _zapOutToToken(
        ...
        (uint256 balance0, uint256 balance1) = _removeLiquidity(pair, amountFrom, router);

        if (path0[0] > path1[0]) {
            (path0, path1) = (path1, path0);
        }

        amountTo = _swapExactTokensForTokens(balance0, 0, path0, to, router);
        amountTo = amountTo + (_swapExactTokensForTokens(balance1, 0, path1, to, router));
    ...
    function _addLiquidity(
        ...
        _approveTokenIfNeeded(token0, amount0, router);
        _approveTokenIfNeeded(token1, amount1, router);

        (, , liquidity) = IDXswapRouter(router).addLiquidity(
            token0,
            token1,
            amount0,
            amount1,
    ...
    function _removeLiquidity(
        ...
        _approveTokenIfNeeded(address(pair), amount, router);
        ...
        IDXswapRouter(router).removeLiquidity(token0, token1, amount, 0, 0, address(this), block.timestamp);
    ...
    function _getFactoryPair(
        address token0,
        address token1,
        address router
    ) internal view returns (address pair) {
        IDXswapFactory factory = IDXswapFactory(IDXswapRouter(router).factory());
