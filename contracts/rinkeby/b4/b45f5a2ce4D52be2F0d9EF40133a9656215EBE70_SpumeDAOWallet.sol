...
/*
 * @title SpumeTeamWallet for custody for the Spume Dev Team 
...
contract SpumeDAOWallet is Pausable, Ownable {
    ...
    // Time buffer for the admin to withdraw SPUME tokens if the contract becomes paused
     ...
     */
    function stakeSpume(uint256 amount) public whenNotPaused onlyOwner returns(uint256){
     ...
     */
    function unstakeSpume(uint256 amount) public whenNotPaused onlyOwner returns(uint256){
       spumeToken.approve(address(spumeStaking), amount);
     ...
     */
    function claimRewards() public whenNotPaused onlyOwner {
        rewardToken.approve(address(spumeStaking), MAX_UINT);
        spumeStaking.claimReward(msg.sender);
    }
    /*
     * @notice Claim Staking Rewards 
     */
    function swapRewards(uint256 _rewardTokenAmount) public whenNotPaused onlyOwner {
        rewardToken.approve(address(spumeStaking), MAX_UINT);
        rewardToken.approve(address(spumeRewardPool), MAX_UINT);
        WETH.approve(address(spumeRewardPool), MAX_UINT);
        spumeRewardPool.rewardSwap(_rewardTokenAmount);
    }

    /*
     * @notice Claim Staking Rewards 
     */
    function claimAndSwapRewards() public whenNotPaused onlyOwner {
        rewardToken.approve(address(spumeStaking), MAX_UINT);
        rewardToken.approve(address(spumeRewardPool), MAX_UINT);
        WETH.approve(address(spumeRewardPool), MAX_UINT);
    ...
    function withdrawWeth(uint256 amount) public whenNotPaused onlyOwner returns(uint256) {
     ...
     */
    function pauseWallet() private onlyOwner whenNotPaused {
     ...
     */
    function unPauseWallet() private onlyOwner whenPaused {
        _unpause(); 
    }
...
interface ISpumeRewardPool {
    ...
    function rewardSwap(uint256 _rewardTokenAmount) external returns (uint256 rewardAmount);
    function rewardClaimAndSwap() external returns(uint256); 
