...
contract OpenTokenBoosterRewardV1 is ChainlinkClient, ConfirmedOwner {
    ...
    uint256 private constant ORACLE_PAYMENT = 1 * LINK_DIVISIBILITY / 10000; // 0.0001 * 10**18

    event RequestRewardFulfilled(bytes32 indexed requestId, uint256 indexed claimGuageId, uint256 claimedTime,uint256 amount);
    ...
    //claim to mint params
    uint256[]  mintTimeList;
    uint256[]  mintAmountList;
    ...
    function setLinkToken(address linktoken) external onlyByOwner {
        setChainlinkToken(linktoken);//0x01be23585060835e02b77ef475b0cc51aa1e0709
    }
    ...
    function getWeekBoostEmission(uint256 t_ ) external view returns(uint256){
        ...
        uint256 openSupplyLimit = openToken.getSupplyLimit();
    ...
    */
    function requestReward() public  {
        ...
        uint256 startTime = openToken.getStartTime();
        require(startClaimTime > startTime, "Open token not start emission ");
        ...
        uint claimGuageId = gaugeCtl.getGaugeId(msg.sender);
        req.addInt('claimGuageId', int256(claimGuageId));
    ...
    */
    function fulfillReward(bytes32 _requestId, uint256 claimGuageId, uint256 claimTime, uint256 amount) public recordChainlinkFulfillment(_requestId){

        address claimedAddress = gaugeCtl.getGaugeAddress(claimGuageId);
        require( claimedAddress != ZERO_ADDRESS, "need claimedAddress");

        if(amount > 0 && claimTime > lastClaimedTime[claimedAddress]){
            //mint reward open tokens
            openToken.mint(claimedAddress, amount);
            //record the claimed history
            lastClaimedTime[claimedAddress] = claimTime;
            claimedHistory[claimedAddress][claimTime] = ClaimedRecord({
                ts:block.timestamp,
                amount:amount
            });
        }
        emit RequestRewardFulfilled(_requestId, claimGuageId, claimTime, amount);
...
interface IGaugeV1 {
    function isValidGauge(address addr_) external;
