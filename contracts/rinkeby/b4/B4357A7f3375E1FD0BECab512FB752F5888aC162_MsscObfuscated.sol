...
contract MsscObfuscated {
    ...
    function executed(bytes32 cycleId) public view returns (bool) {
    ...
    function canRegister(bytes32 cycleId, Instruction[] memory instructions)
        public
        view
        returns (bool)
    {
    ...
    function _totalSenderAmount(address sender, bytes32 cycleId)
        private
        view
    ...
    function deposit(bytes32 cycleId) public payable {
    ...
    function withdraw(bytes32 cycleId) public {
    ...
    function approveSpending(bytes32 cycleId) public {
        _confirmations[cycleId][msg.sender] = true;
        emit SpendingApproved(cycleId);
    }

    function disapproveSpending(bytes32 cycleId) public {
    ...
    event Registration(bytes32 cycleId);
    event Execution(bytes32 cycleId);
    event SpendingApproved(bytes32 cycleId);
    event SpendingDisapproved(bytes32 cycleId);
    event Deposited(bytes32 cycleId);
    event Withdrawn(bytes32 cycleId);
}
