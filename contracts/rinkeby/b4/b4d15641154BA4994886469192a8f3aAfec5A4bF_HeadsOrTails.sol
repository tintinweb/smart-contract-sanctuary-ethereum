...
contract HeadsOrTails is VRFConsumerBase, Ownable {
  ...
  function setInitialBankHash(bytes32 bankHash, address userAddress)
    external
    onlyOwner
  {
  ...
  function placeBet(
    ...
    require(registeredFunds[banker] >= _betAmount, "Not enough bank funds");
  ...
  function sendBankSecretValue(uint256 bankSecretValue, address userAddress)
    external
  {
    ...
    require(
      keccak256(abi.encodePacked(bankSecretValue)) ==
        gameRounds[userAddress].bankHash,
  ...
  function _resetContractFor(address userAddress) private {
    gameRounds[userAddress] = GameRound(0x0, 0, 0, 0, 0);
  }
  ...
  function _evaluateBet(address userAddress) private {
    ...
    bool houseWins = number == 0;

    address winner;

    if (houseWins) winner = banker;
    else winner = userAddress;
