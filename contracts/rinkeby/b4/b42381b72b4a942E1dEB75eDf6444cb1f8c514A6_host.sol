    ...
    for (uint256 i = 0; i < _featureHashes.length; i++) {
      for (uint256 j = 0; j < enabledFeatures.length; j++) {
        if (enabledFeatures[j].featureHash == _featureHashes[i]) {
          ...
          enabledFeatures.pop();
          // enabledFeatures[j].featureHash = 0;
          // enabledFeatures[j].layer = 0;
          // enabledFeatures[j].x = 0;
          // enabledFeatures[j].y = 0;
          // enabledFeatures[j].w = 0;
          // enabledFeatures[j].h = 0;
    ...
    for (uint256 i = 0; i < length; i++) {
      ...
      featureList = string(
        abi.encodePacked(
          featureList,
          '',
          Strings.toString(enabledFeatures[i].layer),
          ':["',
