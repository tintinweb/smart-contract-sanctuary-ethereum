...
contract QueenTraits is BaseContractController, IQueenTraits {
    ...
    function getTraitIdxByName(string memory _traitName)
        private
        view
        returns (uint256 traitIdx, bool found)
    {
        require(
            keccak256(abi.encodePacked(_traitName)) !=
                keccak256(abi.encodePacked("")),
    ...
    function setTrait(string memory _traitName, uint8 _enabled)
        ...
        onlyOnImplementationOrDAO
        whenNotPaused
        returns (RoyalLibrary.sTRAIT memory)
    {
        require(
        ...
        (uint256 traitIdx, bool found) = getTraitIdxByName(_traitName);
        ...
        if (found) //already exists
        {
            ...
            emit TraitCreated(newIdx, _traitName, _enabled);
