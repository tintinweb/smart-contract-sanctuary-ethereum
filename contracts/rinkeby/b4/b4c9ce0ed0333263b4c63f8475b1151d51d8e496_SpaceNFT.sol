...
contract SpaceNFT is ERC721, ERC721Enumerable, SpaceDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("SpaceNFT", "SPC") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No SpaceNFT left :(");
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "SpaceNFT #',
                tokenId.toString(),
                '", "description": "Space NFT are randomized Avataaars stored on chain to teach DApp development on Space", "image": "',
                image,
                '"}'
            )
        );
