    ...
    ReentrancyGuard
{
    ...
    mapping(address => uint8) public mintedMapping;
    mapping(address => uint8) public whiteListMintedMapping;
    mapping(address => int8) public whiteListMapping;
    uint256 public salePrice = 1500000000000000;
    uint256 public whiteListPrice = 1000000000000000;
    uint8 public charityPaymentPercent = 50;
    uint16 public maxSupply = 5000;
    uint8 public maxPerWallet = 100;
    uint8 public maxPerWalletWhiteList = 5;
    uint8 private _giveAwayStartIndex = 50;
    uint8 public giveAwayAmount = 50;
    string private baseURI;
    bool public publicMintStatus = false;
    bool public whiteListMintStatus = false;
    address[] public charityWithdrawAddresses;

    constructor() ERC721A("CryptoCossacks Club", "CC") {}
    ...
    function setMaxPerWallet(uint8 amount) external onlyOwner {
        maxPerWallet = amount;
    }
    ...
    function setCharityWithdrawWallet(address[] memory newWithdrawAddresses)
        external
        onlyOwner
    {
    ...
    function charityWithdrawArray(bool charityPercent) private onlyOwner {
        require(
        ...
        uint256 toWithdraw = (address(this).balance) / amountOfAddresses;
        for (uint8 i = 0; i < charityWithdrawAddresses.length; i++) {
            if (charityPercent) {
                Address.sendValue(
                    payable(charityWithdrawAddresses[i]),
                    (toWithdraw * charityPaymentPercent) / 100
                );
            } else {
                Address.sendValue(
                    payable(charityWithdrawAddresses[i]),
    ...
    function charityWithdrawWithPercent() external onlyOwner nonReentrant {
        charityWithdrawArray(true);
    }

    function charityWithdraw() external onlyOwner nonReentrant {
        charityWithdrawArray(false);
    }

    function fullWithdraw() external onlyOwner nonReentrant {
        charityWithdrawArray(true);
        Address.sendValue(payable(msg.sender), address(this).balance);
    }

    function withdrawGiveAway(address[] memory _addresses) external onlyOwner {
        require(
            _addresses.length <= giveAwayAmount,
            "You try to withdraw more nfts that in giveAwayAmount"
        );
    ...
    function ownerMint(uint8 amount) external onlyOwner {
        require(
            _currentIndex + amount <= maxSupply + 1,
            "Not enough tokens to mint!"
        );
        mintedMapping[msg.sender] += amount;
    ...
    function publicMint(uint8 amount) external payable nonReentrant {
        ...
        require(
            mintedMapping[msg.sender] + amount <= maxPerWallet,
    ...
    // function mintReserved(uint16 amount, address to) external onlyOwner {}

    function whitelistMint(uint8 amount) external payable nonReentrant {
        require(whiteListMintStatus, "WhiteList minting is not active!");
        require(
            whiteListMapping[msg.sender] == 1,
            "Sorry, but you are not in the whitelist"
        );
        require(
            whiteListMintedMapping[msg.sender] + amount <=
                maxPerWalletWhiteList,
            "You can't mint more tokens than maxPerWalletWhiteList"
        );
        require(_currentIndex <= maxSupply, "Not enough tokens to mint!");
        require(
            mintedMapping[msg.sender] + amount <= maxPerWallet,
            "You can't mint more tokens than maxPerWallet"
        );
        ...
        onlyOwner
    {
        for (uint16 i = 0; i < _addresses.length; i++) {
            if (whiteListMapping[_addresses[i]] == 0) {
                whiteListMapping[_addresses[i]] = 1;
            }
        }
    }
}
