 ...
 * VolmexRealizedVolVault should not inherit from any other contract aside from VolmexVault, VolmexRealizedVolVaultStorage
 */
contract VolmexRealizedVolVault is VolmexVault, VolmexRealizedVolVaultStorage {
     ...
     * @param _auction is the contract address that facilitates gnosis auctions
    ...
    function initialize(
        ...
        require(_minVaultDeposit > 0, "VolmexRealizedVolVault: !minVaultDeposit");
    ...
    function withdrawInstantly(uint256 amount) external nonReentrant {
        ...
        require(depositReceipt.round == currentRound, "VolmexRealizedVolVault: Invalid round");

        uint256 receiptAmount = depositReceipt.amount;
        require(receiptAmount >= amount, "VolmexRealizedVolVault: Exceed amount");
    ...
    function commitAndClose() external nonReentrant {
        ...
        uint256 assetDelta = delta.mul(10**6);
        ...
        vTokenPrice = VaultLifecycle.getVTokenPrice(
            GNOSIS_EASY_AUCTION,
            vTokenAuctionID
        );
    ...
    function redeemByMarketMaker(address vToken, uint256 shares) external {
        ...
        require(vToken != address(0), "VolmexRealizedVolVault: address cannot be zero");
        require(
        ...
        require(amountToBeWithdraw > 0, "VolmexRealizedVolVault: !withdrawAmount");
     ...
     * @param auction is the address of the gnosis auction contract
    ...
    function _startAuction(uint256 _minBidPerVtoken) private {
        ...
        auctionDetails.auction = GNOSIS_EASY_AUCTION;
        auctionDetails.asset = vaultParams.asset;
        auctionDetails.assetDecimals = vaultParams.decimals;
        auctionDetails.duration = auctionDuration;
        auctionDetails.minVtokenPremium = _minBidPerVtoken;
        auctionDetails.maxBidPremium = minVaultDeposit[vaultState.round];

        vTokenAuctionID = VaultLifecycle.startAuction(auctionDetails);
    }

    /**
     * @notice Burn the remaining vTokens left over from gnosis auction.
     */
...
import "./Vault.sol";
import "../interfaces/IWrappedAuction.sol";
...
library WrappedAuction {
    ...
    {
        uint256 vTokenSellAmount =
            getVTokenSellAmount(auctionDetails.vTokenAddress);
        ...
        require(
            ...
            "WrappedLib: vTokenSellAmount > type(uint96) max value!"
        );

        uint256 auctionEnd = block.timestamp.add(auctionDetails.duration);
        uint96 maxBidPremium = uint96(auctionDetails.maxBidPremium);
        uint96[2] memory amounts = [uint96(minBidAmount),maxBidPremium];

        auctionID = IWrappedAuction(auctionDetails.auction)
            .initiateAuction(
            ...
            auctionEnd,
            // we are selling all of the vtokens minus a fee taken by gnosis
            uint96(vTokenSellAmount),
            // the minimum we are willing to sell all the vTokens for. A discount is applied on black-scholes price
            amounts,
    ...
    function claimAuctionVTokens(
        ...
        orders[0] = order;
        IWrappedAuction(auction).claimFromParticipantOrder(
        ...
        returns (uint256)
    {
        ...
        // but vtokens will be transferred to gnosis.
        uint256 vTokenSellAmount =
            IERC20Upgradeable(vTokenAddress).balanceOf(address(this));
    ...
    ERC20Upgradeable
{
    ...
    /// @notice Vault's state of the vTokens sold and the timelocked option
    ...
    // GNOSIS_EASY_AUCTION is Wrapped protocol's contract for initiating auctions and placing bids
    ...
    function setFeeRecipient(address newFeeRecipient) external onlyOwner {
        ...
        require(newFeeRecipient != feeRecipient, "VolmexVault: Must be new feeRecipient");
    ...
    function depositETH() external payable nonReentrant {
        require(vaultParams.asset == WETH, "VolmexVault: !WETH");
        require(
            msg.value > minVaultDeposit[vaultState.round],
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        IERC20Upgradeable(vaultParams.asset).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
    ...
    function mint(uint256 _shares) external nonReentrant {
        ...
        require(assets > minVaultDeposit[vaultState.round], "VolmexVault: !minVaultDeposit");

        _depositFor(assets, msg.sender);
     ...
     */
    function depositFor(uint256 _amount, address _creditor)
        external
        nonReentrant
    {
        require(
        ...
        require(_creditor != address(0), "VolmexVault: address of creditor cannot be zero");
    ...
    function initiateWithdraw(uint256 _numShares) external nonReentrant {
        ...
        if (withdrawalIsSameRound) {
            ...
            withdrawals[msg.sender].round = uint16(currentRound - 1);
        }
    ...
    /*
     * @notice Helper function that helps to save gas for writing values into the minVaultDeposit map.
     *         Writing `1` into the map makes subsequent writes warm, reducing the gas from 20k to 5k.
     *         Having 1 initialized beforehand will not be an issue as long as we round down share calculations to 0.
     * @param numRounds is the number of rounds to initialize in the map
     */
    function initRounds(uint256 _numRounds) external nonReentrant {
        require(_numRounds > 0, "VolmexVault: !numRounds");

        uint256 _round = vaultState.round;
        for (uint256 i = 0; i < _numRounds; i++) {
            uint256 index = _round + i;
            require(minVaultDeposit[index] == 0, "VolmexVault: Initialized"); // AVOID OVERWRITING ACTUAL VALUES
            minVaultDeposit[index] = ShareMath.PLACEHOLDER_UINT;
        }
    }
     ...
     * @param _account is the address to lookup balance for
     * @return the amount of `asset` custodied by the vault for the user
     */
    function accountVaultBalance(address _account)
        external
        view
        returns (uint256)
    {
        ...
        return
            ShareMath.sharesToAsset(shares(_account), assetPerShare, _decimals);
    }
    ...
    // TODO: The price per share will be updated as per the minVaultDeposit amount
    /**
    ...
    function _completeWithdraw() internal returns (uint256) {
        ...
        require(withdrawalRound < vaultState.round, "VolmexVault: Round not closed");
    ...
    function _redeem(uint256 _numShares, bool _isMax) internal {
        ...
        require(_numShares <= unredeemedShares, "VolmexVault: Exceeds available");
    ...
    {
        require(block.timestamp >= vTokenState.nextVTokenReadyAt, "VolmexVault: !ready");
    ...
    function transferAsset(address _recipient, uint256 _amount) internal {
        ...
        if (asset == WETH) {
            ...
            require(success, "VolmexVault: Transfer failed");
            return;
        }
...
interface IVolmexRealizedVolVault {
...
library VaultLifecycle {
    ...
    function getVTokenPrice(address easyAuction, uint256 auctionID)
        ...
        returns (uint256)
    {
        AuctionType.AuctionData memory auctionData = IWrappedAuction(easyAuction)
    ...
    function _getPremium(address _easyAuction, uint256 _auctionID)
        ...
        returns (uint256)
    {
        AuctionType.AuctionData memory auctionData = IWrappedAuction(
            _easyAuction
