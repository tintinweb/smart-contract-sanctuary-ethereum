...
contract DistributorERC20V2 is Committee, ReentrancyGuard {
    ...
    event WrapTokenSet(address indexed sender, address indexed from, address indexed to);

    constructor(
        address _committee,
        address[] memory _mintedTokens,
        ...
        for (uint256 i; i < _mintedTokens.length; i++) {
            distributeType[_mintedTokens[i]] = DISTRIBUTE_TYPE.MINT;
        }
    }
