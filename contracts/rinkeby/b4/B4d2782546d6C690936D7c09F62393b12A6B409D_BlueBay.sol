...
contract BlueBay is ERC721A, Ownable {
    ...
    uint256 public startPrice         =  0.01 ether;  // 1e17
    uint256 public endPrice           =  0.02 ether;  // 1e17

    uint256 public WLdiscountPrice    =  0.002 ether;  // 1e17
    uint256 public WLdiscountLimit    =  2;

    uint256 public changedDuration    =  1800;
    uint256 public changedAmount      =  0.001 ether;
    ...
    function reserve(uint256[] memory _tokenIds, bytes32[] memory _merkleProof) public payable {
        ...
        if (msg.sender != owner()) {
            ...
            if (MerkleProof.verify(_merkleProof, merkleRoot, sender)) {
                console.log("whitelisted user");
                uint256 temp;
                if (WLreservedNum[msg.sender] < WLdiscountLimit) {
                    temp = WLdiscountLimit - WLreservedNum[msg.sender];
                    if (_tokenIds.length <= temp) {
                        WLreservedNum[msg.sender] += _tokenIds.length;
                        require(msg.value >= (currentPrice() - WLdiscountPrice) * _tokenIds.length, "Insufficient funds");
                    } else {
                        WLreservedNum[msg.sender] += temp;
                        require(msg.value >= (currentPrice() - WLdiscountPrice) * temp + currentPrice() * (_tokenIds.length - temp), "Insufficient funds");
                    }
                } else {
    ...
    function currentPrice() public view returns (uint256) {
        ...
        if (prevTime <= 0) {
            ...
            price = prevPrice + (block.timestamp - prevTime) / changedDuration * changedAmount;
    ...
    function setChangedDuration(uint256 _changedDuration) public onlyOwner {
        prevPrice = prevPrice + (block.timestamp - prevTime) / changedDuration * changedAmount;
