...
contract DEXINFT is ERC721URIStorage, ERC721Enumerable, IERC721TokenCreator, Ownable {
     ...
     */
    constructor() ERC721("DEXI NFT", "DEXI") {}

    modifier _validateOwnerOfToken(uint256 _id) {
        require(_exists(_id), "Error, token does not exist");
        require(_msgSender() == ownerOf(_id), "Only the owner of the token can burn it");
        _;
    }

    function bulkMint(string memory _tokenURI, uint256 _numberOfToken) external onlyOwner {
    ...
    function mint(string memory _tokenURI) public returns (uint256) {
        ...
        _creators[mintIndex] = msg.sender;

        emit Minted(msg.sender, mintIndex, _tokenURI);
