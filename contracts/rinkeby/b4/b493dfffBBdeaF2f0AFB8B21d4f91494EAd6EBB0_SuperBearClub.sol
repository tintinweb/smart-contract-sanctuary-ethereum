...
contract SuperBearClub is ERC721A, Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) private banagerMinted;
    mapping(address => bool) private ogMinted;
    mapping(address => bool) private teamMinted;
    ...
    string private baseTokenURI = "ipfs://QmTruyAeR6ch31uxYNeWnRthns79RDSD2zgWTgDGqAFEAf/";
    ...
    function getCanFreeMintCount(bytes32[][4] calldata merkleProofs) public view returns (uint256[] memory )
    {
        ...
        if(merkleProofs[WHITELIST_INDEX].length != 0){
            ...
            if(isWlActive && !whitelistMinted[msg.sender]){
        ...
        if(merkleProofs[BANAGER_INDEX].length != 0){
            ...
            if(isBanagerActive && !banagerMinted[msg.sender]){
        ...
        if(merkleProofs[OG_INDEX].length != 0){
            ...
            if(isOGActive && !ogMinted[msg.sender]){
        ...
        if(merkleProofs[TEAM_INDEX].length != 0){
            ...
            if(isTeamActive && !teamMinted[msg.sender]){
    ...
    function mintNFTPresale(uint256[4] calldata amountsToBuy,bytes32[][4] calldata merkleProofs) 
    public
    nonReentrant
    isNotContract
    preSaleActive
    {
        require(amountsToBuy.length == 4, "Not right amountsToBuy length");
        require(merkleProofs.length == 4, "Not right merkleProofs length");
        uint256[] memory counts = getCanFreeMintCount(merkleProofs);
        uint256 totalCount;
        for(uint i = 0;i < amountsToBuy.length;i++){
            require(amountsToBuy[i] == counts[i],"Invalid amounts");
            totalCount += amountsToBuy[i];
        }
        require(totalCount > 0,"Mint count must be greater than 0");
        require(_currentIndex + totalCount <= maxBears,"Not enough bears remaining to mint");

        if(amountsToBuy[WHITELIST_INDEX] > 0){
            whitelistMinted[msg.sender] = true;
            whitelistMintCounts += uint64(amountsToBuy[WHITELIST_INDEX]);
        }
        if(amountsToBuy[BANAGER_INDEX] > 0){
            banagerMinted[msg.sender] = true;
            banagerMintCounts += uint64(amountsToBuy[BANAGER_INDEX]);
        }
        if(amountsToBuy[OG_INDEX] > 0 ){
            ogMinted[msg.sender] = true;
            ogMintCounts += uint64(amountsToBuy[OG_INDEX]);
        }
        if(amountsToBuy[TEAM_INDEX] > 0 ){
            teamMinted[msg.sender] = true;
            teamMintCounts += uint64(amountsToBuy[TEAM_INDEX]);
        }

        _mintNFT(msg.sender,totalCount);
    }
