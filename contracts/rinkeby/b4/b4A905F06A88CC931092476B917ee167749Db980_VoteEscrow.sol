...
/// @title escrow voting mechanism for WTR token
contract VoteEscrow is ERC20Votes, ReentrancyGuard, IVotingEscrow {
    ...
    // days
    uint256 public constant MINDAYS = 36;
    uint256 public constant MAXDAYS = 360;
    uint256 private constant MAXTIME = MAXDAYS * 1 days; // =~ 360 days in secs
    ...
    /// @dev Lock WTR for veWTR. Must have no locked tokens
    /// @param _value Amount to lock
    /// @param _days Number of days to set the lock
    function createLock(uint256 _value, uint256 _days) external override {
        require(locked[_msgSender()].amount == 0, "Lock Already created. Did you mean to call increaseLockAmount ?");
        require(_days >= MINDAYS, "Voting lock can be 36 days min");
        require(_days <= MAXDAYS, "Voting lock can be 360 days max");
        _depositFor(_msgSender(), _value, _days);
    }

    /// @dev Add more WTR for veWTR. Must have locked WTR first
    /// @param _value Amount to add
    function increaseLockAmount(uint256 _value) external override {
        _depositFor(_msgSender(), _value, 0);
    }

    /// @dev Increase lock time for an address
    /// @param _days Number of days to add to the lock
    function increaseLockedAmountTime(uint256 _days) external override {
        require(_days >= MINDAYS, "Voting lock can be 36 days min");
        require(_days <= MAXDAYS, "Voting lock can be 360 days max");
        _depositFor(_msgSender(), 0, _days);
    }

    /// @dev Allow address1 to deposit their WTR for address2. Address2 must have created lock
    /// @param _addr Recipient of more funds
    /// @param _value Amount to be added
    function depositFor(address _addr, uint256 _value) external override {
        _depositFor(_addr, _value, 0);
    }

    /// @dev Withdraw WTR once lock time is over
    ...
    /* ========== Override ERC20 FUNCTIONS veWTR is non-transferrable ========== */
    ...
    function _withdraw() private nonReentrant {
        ...
        require(_now >= _locked.end, "Lock did not expire");
        uint256 _amount = _locked.amount;
        _locked.end = 0;
        _locked.amount = 0;
...
interface IVotingEscrow {
    ...
    function increaseLockedAmountTime(uint256 _days) external;
