...
import {NounsToken} from "nouns-contracts/contracts/NounsToken.sol";
import {NFTReceiver as Receiver} from "../../src/utils/NFTReceiver.sol";

contract MintHelper is Receiver {
    address public nouns;

    function mint() external {
        uint256 tokenId = NounsToken(nouns).mint();
        NounsToken(nouns).safeTransferFrom(address(this), msg.sender, tokenId);
    }

    function setNouns(address _nouns) external {
        nouns = _nouns;
    }
}
