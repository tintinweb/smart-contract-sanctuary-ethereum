...
uint256 public lastHHGiven;
    ...
    function addHH(uint256 _amtTreats) public payable {
       ...
       if(
             (lastOnChainHH -(((block.timestamp - lastHHGiven) / 60) / HHRemInterval)+ (happinessPerTreat * _amtTreats)) <= 0
       ){
             lastOnChainHH = (happinessPerTreat * _amtTreats);
       } else{
           lastOnChainHH = lastOnChainHH -(((block.timestamp - lastHHGiven) / 60) / HHRemInterval)+ (happinessPerTreat * _amtTreats);
       }
         lastHHGiven =block.timestamp;
    }
