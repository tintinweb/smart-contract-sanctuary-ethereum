...
contract EthBox is ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    uint256 public constant maxInboxSize = 21;
    ...
    mapping(address => Claim[]) public messageClaims;
    mapping(address => uint256) public inboxSizes;
    

    //Display and metadata variables
    string private desc;
    string private imageInsert;
    string private htmlInsert;
    ...
    constructor(){
        ...
        messageInsert = 
        '<g id="message-${_INDEX}" style="display:${_VISIBILITY}" onclick="cycleMessage()"><foreignObject x="2.8%" y="0%" width="97.1%" height="14.2%"><div class = "plain-text" xmlns="http://www.w3.org/1999/xhtml" style="color:white;font-family:monospace;font-size:0.9em">to: ${_TO}</div></foreignObject><foreignObject x="2.8%" y="5%" width="97.1%" height="14.2%"><div class = "plain-text" xmlns="http://www.w3.org/1999/xhtml" style="color:white;font-family:monospace;font-size:0.9em">from: ${_FROM} </div></foreignObject><foreignObject x="2.8%" y="10%" width="97.1%" height="90%"><div class = "plain-text" xmlns="http://www.w3.org/1999/xhtml" style="color:#4AF626;font-family:monospace;font-size:1.5em"> ${_MESSAGE} </div></foreignObject><foreignObject x="2.8%" y="85%" width="97.1%" height="90%"><div class = "plain-text" xmlns="http://www.w3.org/1999/xhtml" style="color:red;font-family:monospace;font-size:1.8em;text-align:left"> Bid: ${_VALUE} ETH </div></foreignObject><foreignObject x="82%" y="85%" width="97.1%" height="90%"><div class = "plain-text" xmlns="http://www.w3.org/1999/xhtml" style="color:red;font-family:monospace;font-size:1.5em;text-align:left">(${_POSITION})</div></foreignObject></g>';
        inboxInsert = 
    ...
    function ownerOf(uint256 _tokenId) public view returns (address) {
        ...
        require(_minted[owner], "ERC721: invalid token ID");
        return owner;
    }

    function inboxOf(address _owner) public view returns (uint256) {
        require(_minted[_owner], "Address has not minted");
    ...
    function tokenURI(uint256 _tokenId) public view returns (string memory) {
        require(_minted[address(uint160(_tokenId))]);
    ...
    function approve(address to, uint256 tokenId) public {revert("disabled");}
    function setApprovalForAll(address, bool) public{revert("disabled");}
    function transferFrom(address from, address to, uint256 tokenId) public {revert("disabled");}
    function safeTransferFrom(address from, address to, uint256 tokenId) public {revert("disabled");}
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory data) public {revert("disabled");}

    ////////////////////////////////////
    ////////////////////////////////////
    // Minting functions //
    // todo : add all base 721 minting functions

    function mintInboxFor(address _recipient) public {
        require(airdropAllowed);
        require(!_minted[_recipient]);
    ...
    function setHtmlInsert(string memory _htmlInsert) external onlyOwner {
        htmlInsert = _htmlInsert;
    }

    function setImageInsert(string memory _imageInsert) external onlyOwner {
        imageInsert = _imageInsert;
    }
    ...
    function valueToEthString(uint256 _value) public view returns (string memory) {
    ...
    function buildImage(string memory svg) public view returns (string memory) {
        return string(abi.encodePacked(
    ...
    function buildHtml(string memory svg) public view returns (string memory) {
        return string(abi.encodePacked(
        ...
        returns (string memory)
    {
        ...
        string memory messageSVG = buildSVG(owner, messages, inboxSizes[owner]);
        output = string(abi.encodePacked(
            output,
            buildImage(messageSVG),
            '", "animation_url": "',
            buildHtml(messageSVG),
            '"}'
        ));
        return string(
                abi.encodePacked(
    ...
    function sendMessage(address _to, string calldata _message) public payable{
        ...
        uint256 boxSize = toMessages.length;
        uint256 insertIndex = maxInboxSize;
        //open slots left in inbox
        if (boxSize < defaultInboxSize) {
            _pushMessage(_to, msg.sender, _message, msg.value);
        } 
        // no slots left, find the message to bump
        else {
            uint256 lowIndex;
            uint256 lowValue = toMessages[0].originalValue;
            bool qualifies = false;
            for (uint256 i=0; i< boxSize; i++) {
                if (toMessages[i].originalValue < lowValue) {
                    lowIndex = i;
                    lowValue = toMessages[i].originalValue;
                }
                if ( qualifies == false && msg.value > toMessages[i].originalValue) {
                    qualifies = true;
                }
            }
            if (qualifies){
                insertIndex = lowIndex;
                _insertMessage(_to, msg.sender, _message, msg.value, insertIndex);

            }
            
        }
        if(insertIndex != (maxInboxSize)){
            _payFees(_to, msg.value);
        }
    }
    ...
    function _payFees(address _to, uint256 _value) private {
        (bool successTo, ) = _to.call{value: (_value * (10000 - messageFeeBPS)) / 10000}("");
        require(successTo);
        (bool successRecipient, ) = messageFeeRecipient.call{value: (_value * messageFeeBPS) / 10000}("");
