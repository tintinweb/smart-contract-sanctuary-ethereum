...
// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@//
////////////////////////////////////////////////////////////////////////////////////////////////////////
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard {
    //apple string
    ...
    mapping(address => UserConfig) internal userConfigList;
        ...
        uint256 seed = uint256(
            keccak256(
                abi.encodePacked(
                    ...
                    tokenId,
                    salt,
                    userConfigList[tokenOwner].variant
                )
            )
        );
        return seed;
    ...
    function selectDifficulty(uint256 tokenId)
        ...
        returns (string memory)
    {
        return Strings.toString(generateAppleSeed(tokenId, 69420) % 3);
    }
        ...
        returns (string memory)
    {
        ...
        string[30] memory colors = generateAllColors(tokenId);
        ...
        string memory base64 = Base64.encode(
            ...
            funkSnakeCreator(
                [
                    colors[generateAppleSeed(tokenId, 786) % uint256(19)],
                    colors[generateAppleSeed(tokenId, 69) % uint256(19)],
                    selectDifficulty(tokenId),
                    colors[generateAppleSeed(tokenId, 23) % uint256(19)],
                    colors[0],
                    colors[generateAppleSeed(tokenId, 1337) % uint256(19)],
                    colors[generateAppleSeed(tokenId, 420) % uint256(19)],
                    colors[generateAppleSeed(tokenId, 143) % uint256(19)]
                ]
            )
        );
    ...
    function generateColors(uint256 tokenId, uint256 number)
        ...
        returns (uint256)
    {
        return uint256(generateAppleSeed(tokenId, number)) % uint256(255);
    }

    function generateAllColors(uint256 tokenId)
        private
        view
        returns (string[30] memory)
    {
        string[30] memory colors = retrofy(tokenId);
        uint256 rn = (generateAppleSeed(tokenId, 420) % (colors.length - 10)) +
            10;
    ...
    function generateColorArray(uint256 tokenId, uint256 i)
        private
        view
        returns (uint256[4] memory)
    {
        ...
        for (uint256 index = 0; index < colors.length - 1; index++) {
            colors[index] = i == 1
                ? generateColors(tokenId, changes[index])
                : generateColors(tokenId, changes[index] + i + index);
        }
    ...
    function retrofy(uint256 tokenId)
        internal
        view
        returns (string[30] memory)
    {
        ...
        for (uint256 i = 0; i < parts.length; i++) {
            uint256[4] memory colors = generateColorArray(tokenId, i);
    ...
    function cloneSnake(uint256 tokenId) private view returns (bytes memory) {
        string[30] memory selectedColors = generateAllColors(tokenId);
        bytes memory svgData = "";
        uint256 randomSnakeLength = (generateAppleSeed(tokenId, 69) %
    ...
    function changeVariant(uint256 variant) public payable callerIsSender {
        userConfigList[msg.sender].variant = variant;
    }
        ...
        returns (bytes memory)
    {
        ...
        attributesNumbers[0] = AttributesNumbers(
            "Color Count",
            "",
            (generateAppleSeed(tokenId, 420) % (colors.length - 10)) + 10,
            colors.length
        );
        attributesNumbers[1] = AttributesNumbers(
            "Variance Count",
            "",
            (generateAppleSeed(tokenId, 420) % (5)) + 5,
            10
        );
    ...
    function testOwner(uint256 tokenId) public view returns (address) {
