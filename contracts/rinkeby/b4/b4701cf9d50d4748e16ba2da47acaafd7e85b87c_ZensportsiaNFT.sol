  ...
  ERC721Manageable
{
  ...
  function mintForTeam(address[] memory _accounts, uint256[] memory _allocations)
    external
    override
    onlyOwnerOrGnosis
  {
  ...
  IZensportsia
{
  ...
  /**
   * @notice get sale plans
   */
  function getSalePlans() public view virtual returns (SalePlans memory) {
    return salePlans;
  }
   ...
   */
  function setSalePlans(SalePlans memory _salePlans) public virtual {
    salePlans = _salePlans;
  }

  function setFundAccount(address _fundAccount) public onlyOwnerOrGnosis {
    fundAccount = _fundAccount;
  }

  function setWhitelists(address[] memory accounts) public onlyOwner {
    for (uint256 i = 0; i < accounts.length; i++) {
      whitelists[accounts[i]] = true;
    }
  }

  function setBlacklists(address[] memory accounts) public onlyOwnerOrGnosis {
    for (uint256 i = 0; i < accounts.length; i++) {
      whitelists[accounts[i]] = false;
    }
  }

  function withdrawFunds() public onlyOwnerOrGnosis {
  ...
  function setBaseURI(string memory _baseTokenURI) public onlyOwnerOrGnosis {
...
abstract contract RoyaltyBase is IRoyaltyBase, GnosisProxy {
...
interface IZensportsia {
  struct SalePlans {
    ...
    uint256 totalWhitelistedMembers;
...
abstract contract GnosisProxy is OwnableUpgradeable {
  address private _proxy;

  modifier onlyOwnerOrGnosis() {
    require(owner() == msg.sender || _proxy == msg.sender, "FORBIDDEN");

    _;
  }

  function _gnosisProxy() internal view virtual returns (address) {
    return _proxy;
  }

  function setGnosisProxy(address _newProxy) public virtual onlyOwner {
    _proxy = _newProxy;
  }
}
