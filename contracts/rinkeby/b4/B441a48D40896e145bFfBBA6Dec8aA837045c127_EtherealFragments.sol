...
contract EtherealFragments is ERC20("Ethereal Fragments", "FRAGZ") {
	...
	IEtherealWorlds private _specialWorldsContract;

	event FragzBurnt(address indexed user, uint256 amount, string indexed reason);

	// Link the _worlds contract and mint 2500 tokens to the treasury in order to provide liquidity / donations
	// Also airdrops all users their inital fragz as well as a gift of 2 fragz
	constructor(address _worlds, address _specials) {
		_worldsContract = IEtherealWorlds(_worlds);
		_specialWorldsContract = IEtherealWorlds(_specials);

		_mint(0x17020cBf555670aB1c7f3e64a80dA61b0B4990c0, 2500 ether);

		uint256 _rewards = (BASE_RATE * (block.timestamp - 1643533200) / 86400) + 2;

		for(uint256 i = 0; i < 345; i++) {
			lastUpdate[i] = block.timestamp;
			_mint(_worldsContract.ownerOf(i), _rewards);
		}
	}
	...
	function claimFragz(uint256[] calldata _tokens) external {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
			if(_tokens[0] > 344) {
				require(_specialWorldsContract.ownerOf(_tokens[0]) == msg.sender, "Fragz: Not Owner");
