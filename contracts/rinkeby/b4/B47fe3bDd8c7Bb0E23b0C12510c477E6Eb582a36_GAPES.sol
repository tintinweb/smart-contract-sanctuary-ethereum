...
contract GAPES is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    //set max mint amount per transcation
     ...
     */
   modifier noBots() {
        require(msg.sender == tx.origin, "Gapes: No bots");
        _;
    }
  ...
  // public mint 
  function mint(uint256 _mintAmount) public payable noBots mintCompliance(_mintAmount){
    if (msg.sender != owner()) {
    //sale status
  ...
  function gift(address _receiver, uint256 _mintAmount) public mintCompliance(_mintAmount) onlyOwner {
