...
contract liquidityProvider is Initializable{
   ...
   **/
   function initialize( 
    ...
    function addLiquidity(address tokenA, address tokenB, uint amountA, uint amountB) public {
        console.log(IERC20(tokenA).balanceOf(msg.sender), "balance of msg.sender");
    ...
    function removeLiquidity(address lp, uint amount) public {
        (uint amountA, uint amountB) = IUniswapV2Router02(router).removeLiquidity(IUniswapV2Pair(lp).token0(), 
            IUniswapV2Pair(lp).token1(), amount, 1, 1, address(this), block.timestamp);
    }

    function addLiquidityEth(address token, uint amount) public payable{
    ...
    function removeLiquidityEth(address token, uint amount) public {
        (uint amountA, uint amountBNB) = IUniswapV2Router02(router).removeLiquidityETH(token,amount, 1, 1, address(this), block.timestamp);
    }
}
