...
contract ShopXReserveFactory is Pausable {
  ...
  function createReserveNFT(
    ...
    {
      uint256 totalFee = shopxFee + platformFee;
...
* @dev Mints a token to the msg.sender. It consumes whitelisted supplies.
  ...
  * @dev Only allowed for CrossMint. Mints a token to an address but also checks if the destination address is in the allowList.
  ...
  function crossMint(bytes32[] calldata merkleProof, address to, uint256 quantity) payable external whenNotPaused nonReentrant {
    ...
    require(msg.value == mintPrice * quantity);
    require (merkleRoot == bytes32(0) || MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(to))), "ReserveNFT: Invalid Proof. Valid proof required.");
...
interface IShopXReserveNFT {
    ...
    function grantRole(bytes32 role, address account) external;
    function hasRole(bytes32 role, address account) external view returns (bool);
    function initialize(address _owner, string memory _baseURI, uint256[4] memory _uintArgs, address[3] memory _addressArgs, uint256 _shopxFee, uint256 _platformFee, uint256[] memory _beneficiaryFees, address[] memory _beneficiaryAddresses, address[] memory _brandAdmins, address[] memory _saleAdmins, address[] memory _shopxAdmins) external;
