...
contract AlphaVault is Ownable, IERC721Receiver {
  ...
  // MetroBlock nft; find a way to connect to the other Contract
  AlphaToken token;

  // maps tokenId to stake
  mapping(uint256 => Stake) public vault; 

  constructor(address parentNFT, AlphaToken _token) { 
    token = _token;
    alphaWolves = ERC721(parentNFT);
  }
...
// alphawolves instead of nft
  ...
  // only possible when sold out makse sure to add a setter function
  // should never be used inside of transaction because of gas fee
  function balanceOf(address account) public view returns (uint256) {
    uint256 balance = 0;
    uint256 alphaSupply = 333;
    for(uint i = 1; i <= alphaSupply; i++) {
  ...
  function tokensOfOwner(address account) public view returns (uint256[] memory ownerTokens) {
    ...
    uint256 index = 0;
    for(uint tokenId = 1; tokenId <= alphaSupply; tokenId++) {
...
contract AlphaToken is ERC20, ERC20Burnable, Ownable {
  ...
  uint256 public MAX_SUPPLY_OF_TOKENS = 3330000 * 10 ** 18;

  mapping(address => bool) controllers;
  
  constructor() ERC20("AlphaWolves", "ALPHA") { }

  function mint(address to, uint256 amount) external {
    uint256 ts = totalSupply();
    require(MAX_SUPPLY_OF_TOKENS >= ts + amount, "Exceeds Max Supply");
