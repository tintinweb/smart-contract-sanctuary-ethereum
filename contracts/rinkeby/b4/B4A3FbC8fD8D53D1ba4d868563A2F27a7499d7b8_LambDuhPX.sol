...
contract LambDuhPX is ERC721Enumerable, Ownable, nonReentrant {
    ...
    uint256 public MAX_ROOLAH = 5; // 100 mints
    uint256 public roolahCost = 100000000000000000000; // 100 ROOLAH
    uint256 public MAX_STAR = 5; // 100 mints
    uint256 public starCost = 100000000000000000000; // 100 STAR
    ...
    uint64 public portalSupply;
    uint64 public roolahSupply;
    uint64 public starSupply;
    ...
    // rooStar FALSE = STAR | TRUE = ROOLAH
    function mintWithToken(bool rooStar) public reentryLock {
        require(
        ...
        require(portalSupply < MAX_PXLAMB, "max portals exceeded");

        if (rooStar) {
            require(roolahSupply < MAX_ROOLAH, "max roolah mint exceeded");
            require(
                addressInfo[msg.sender].roolahMinted == false,
                "roolah already claimed"
            );
            addressInfo[msg.sender].roolahMinted = true;
            roolah.transferFrom(msg.sender, address(this), roolahCost);
        } else {
            require(starSupply < MAX_STAR, "max star mint exceeded");
            require(
                addressInfo[msg.sender].starMinted == false,
                "star already claimed"
            );
            addressInfo[msg.sender].starMinted = true;
            star.transferFrom(msg.sender, address(this), starCost);
        }

        portalSupply += 1;

        portalsIDs.push(uint16(supply));
    ...
    function setTokenConfigs(
        ...
        roolah = IERC20(roolahToken);
        roolahCost = _roolahCost;
        star = IERC20(starToken);
        starCost = _starCost;
    }
