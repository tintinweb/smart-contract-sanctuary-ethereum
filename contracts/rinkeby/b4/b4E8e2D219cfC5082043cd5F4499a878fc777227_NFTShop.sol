...
contract NFTShop is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    event OrderCancelled(IERC721 indexed nftContract, uint indexed tokenId, address indexed seller, address canceller);

    /**
     * @dev Throws if called by any account other than the owner of nft.
     * @dev Works only with NFTFactory NFT
     */
    modifier onlyNftContractOwner(IERC721 _nftContract) {
        INFTFull nft = INFTFull(address(_nftContract));
        try nft.isNFTFactoryNFT{gas:20000}() returns (bytes32 _result) {
            require(_result == keccak256("NFTFactoryNFT"), "NFTShop: NFT_IS_NOT_SUPPORTED");
        } catch {
            revert("NFTShop: NFT_IS_NOT_SUPPORTED");
        }

        require(nft.owner() == msg.sender, "NFTShop: AUTH_FAILED");
        _;
    }
    ...
    function setPlatformRequiredTokensToBuy(RequiredToken[] memory _platformRequiredTokensToBuy) public onlyOwner {
        require(_platformRequiredTokensToBuy.length > 0, "NFTShop: EMPTY_ARRAY");

        delete platformRequiredTokensToBuy;

        for (uint i=0; i<_platformRequiredTokensToBuy.length; i++) {
            RequiredToken memory item = _platformRequiredTokensToBuy[i];
            require(address(item.token) != address(0), "NFTShop: ZERO_ADDRESS");
            require(item.amount > 0, "NFTShop: ZERO_AMOUNT");
            checkExistingAddresses(item, _platformRequiredTokensToBuy, i);

            platformRequiredTokensToBuy.push(item);
        }

        emit PlatformRequiredTokenToBuySet(_platformRequiredTokensToBuy);
    }

    function getOwnerRequiredTokensToBuy(IERC721 _nftContract) public view returns (RequiredToken[] memory) {
        return ownerRequiredTokensToBuy[_nftContract];
    }

    function setOwnerRequiredTokensToBuy(IERC721 _nftContract, RequiredToken[] memory _ownerRequiredTokensToBuy) public onlyNftContractOwner(_nftContract) {
        require(_ownerRequiredTokensToBuy.length > 0, "NFTShop: EMPTY_ARRAY");
        ...
        for (uint i=0; i<_ownerRequiredTokensToBuy.length; i++) {
            RequiredToken memory item = _ownerRequiredTokensToBuy[i];
            require(address(item.token) != address(0), "NFTShop: ZERO_ADDRESS");
            require(item.amount > 0, "NFTShop: ZERO_AMOUNT");
            checkExistingAddresses(item, _ownerRequiredTokensToBuy, i);

            nftOwnerRequiredTokensToBuy.push(item);
        }
    ...
    function setProjectTreasury(IERC721 _nftContract, address _treasury) public onlyNftContractOwner(_nftContract) {
    ...
    function sell(IERC721 _nftContract, ItemForSale[] memory _items) public nonReentrant {
        ...
        for (uint i=0; i<_items.length; i++) {
            ...
            tokensForSaleInfo[_nftContract][item.id] = Order(
                item.price,
                item.stable,
                msg.sender
            );
    ...
    function cancelSell(IERC721 _nftContract, uint[] memory _ids) public nonReentrant {
        ...
        for (uint i=0; i<_ids.length; i++) {
            ...
            address seller = tokensForSaleInfo[_nftContract][id].seller;
            require(msg.sender == seller, "NFTShop: AUTH_FAILED");

            require(tokensForSaleInternal[_nftContract].remove(id), "NFTShop: BROKEN_STRUCTURE");
            delete tokensForSaleInfo[_nftContract][id];

            _nftContract.safeTransferFrom(address(this), seller, id);

            emit OrderCancelled(_nftContract, id, seller, msg.sender);
        }
    }

    function buy(IERC721 _nftContract, uint[] memory _ids) public nonReentrant {
        require(_ids.length > 0, "NFTShop: EMPTY_ARRAY");
        ...
        for (uint i=0; i<_ids.length; i++) {
            ...
            require(msg.sender != tokenInfo.seller && msg.sender != treasury, "NFTShop: INVALID_BUYER");
            ...
            emit NFTSold(_nftContract, id, tokenInfo.seller, msg.sender, tokenInfo.stable, tokenInfo.price);
        }
    }
    ...
    function tokensForSale(IERC721 _nftContract) public view returns(uint[] memory _ids, Order[] memory _infos) {
    ...
    function tokensForSaleCount(IERC721 _nftContract) public view returns(uint) {
        return tokensForSaleInternal[_nftContract].length();
    }

    function tokenForSale(IERC721 _nftContract, uint _index) public view returns(uint _id, Order memory info) {
        _id = tokensForSaleInternal[_nftContract].at(_index);
        info = tokensForSaleInfo[_nftContract][_id];
    }

    function getTokenOwner(IERC721 _nftContract, uint _id) internal view returns (address) {
        try _nftContract.ownerOf{gas:20000}(_id) returns (address _result) {
            return _result;
        } catch {}
