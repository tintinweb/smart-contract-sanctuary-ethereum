...
pragma solidity ^0.8.15 ;
    ...
    contract Betting_SC {
        ...
        function add_more() public noReentrancy BettingClose minBalance payable{
            ...
            Database storage last_betting_database = database[j];
            require(last_betting_database.Betters_address == msg.sender, "ERROR6");
            last_betting_database.amount_betted = last_betting_database.amount_betted + msg.value;
        }
        function Refund() public noReentrancy BettingClose{
            ...
            Database storage last_betting_database = database[j];
            if (last_betting_database.Betters_address != msg.sender){
                revert("ERROR7");
            }
            payable(msg.sender).transfer(last_betting_database.amount_betted);
            for (uint256 x = j; x < getLength()-1; ++x){
                database[x] = database[x+1];
            }
            database.pop();
        }
        function ViewContractBalance() public onlyOwner view returns(uint256){
        ...
        function Declare_Winner(string memory winner) external onlyOwner noReentrancy {
