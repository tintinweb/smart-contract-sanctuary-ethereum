...
// File: contracts/SmartcontractShowcase.sol
...
contract SmartcontractShowcase is ERC721A, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory baseURI = _baseURI();
        uint256 imageId = startingIndex + tokenId;
        return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI, imageId.toString())): '';
    }
    ...
    **/
    ...
    function mint(uint numberOfTokens, bytes calldata signature) public payable {
        ...
        require(decodedAddress == signingAddress, "Signer address mismatch.");
