...
contract stakingSimple is Ownable, ReentrancyGuard {
	...
	//deposit function for initial staking setup
    function deposit(bytes[] memory signature, address contractAddress, uint256[] memory tokenIds, uint256[] memory rarityWeight) public nonReentrant {
        require(!depositPaused, "Deposit paused");
        require(stakingLaunched, "Staking is not launched yet");
        require(tokenIds.length == rarityWeight.length, "non-matching array lengths provided");
        require(contractAddress != address(0) && contractAddress == address(baseNFT), "Unknown contract");
    
        //setup rarity on each token
        if (rarityWeight.length > 0) {
            require(_validateSignatureMultiSig(signature, contractAddress, tokenIds, rarityWeight), "Invalid signature");
    ...
    function _setTokensValues(uint256[] memory tokenIds, uint256[] memory rarityWeight) internal {
        for (uint256 i; i < tokenIds.length; i++) {
   ...
   //return true if received signature address matches internal signerAddress
    function _validateSignatureMultiSig(bytes[] memory signature, address contractAddress, uint256[] memory tokenIds, uint256[] memory rarityWeight) internal view returns (bool) {
 
        for (uint256 i; i < tokenIds.length; i++) {
            bytes32 messageHash = keccak256(abi.encodePacked(contractAddress, tokenIds[i], rarityWeight[i]));
            bytes32 EthSignedMessageHash = ECDSA.toEthSignedMessageHash(messageHash);
            address recoveredAddress = ECDSA.recover(EthSignedMessageHash, signature[i]);
            require((recoveredAddress != address(0) && recoveredAddress == signerAddress), "Signature does not match");
        }
        return true;
    }
