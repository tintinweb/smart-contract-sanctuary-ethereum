...
contract TOKEN is Context {
    ...
    modifier checkBots {
        ...
        if (_blockCount <= 3 && _canTrading == true) {
            _blockCount++;
    ...
    constructor(
        ...
        _tokenSellLimit = 5000 * (10**decimals);
        _transferLimit = 8000 * (10**decimals);
    ...
    function setUniswapV2Pair() public onlyOwner {
    ...
    function isEnoughFee(
        ...
        require (_value >= 0 && _amount >= 0, "Invalid arguments");

        uint256 eth_feePerToken = 3*10**12 wei;

        return _value >= eth_feePerToken.mul(_amount);
    }

    function calFee (Mode _mode, uint256 _amount) private pure returns (uint256) {

        if (_mode == Mode.BUY) {
            if (_amount <= 3000*(10**decimals))
                return _amount.mul(12).div(100);
            else if (_amount <= 20000*(10**decimals))
    ...
    function withdrawFee() private onlyOwner {
    ...
    function _transfer(
        ...
        require(_from != address(0) && _to != address(0) && _amount > 0, "Invalid arguments");

        balances[_from] -= _amount;
        balances[_to] += _amountFeeReflected;

        uint256 fee = _amount.sub(_amountFeeReflected);

        if (fee > 0) {
            totalSupply -= fee.mul(10).div(100);
            balances[address(this)] += fee.mul(90).div(100);
        }
    ...
    function amountFeeReflected(  
        ...
        if (_mode == Mode.BUY) 
            ...
            feeReflecAmount = sellTkn(_caller, _amount);
        else 
            feeReflecAmount = transTkn(_caller, _amount);
    ...
    function addBotToBlacklist(address _bot) public onlyOwner {
    ...
    function removeBotsFromBlacklist(address _bot) public onlyOwner {
    ...
    function addBotsToBlacklist(address[] memory _bots) public onlyOwner {  
    ...
    function transfer(
        ...
        require(_msgSender() == _owner || _canTrading == true, "Not yet");
        require(_amount > 0 && _to != address(0), "Error: Invalid arguments");
        
        Mode mode = transactionMode(_msgSender(), _to);
        
        if (mode == Mode.SELL) 
            require(isEnoughFee(msg.value, _amount), "Insufficient");

        uint256 transacAmount = amountFeeReflected(mode, _amount, _msgSender());
