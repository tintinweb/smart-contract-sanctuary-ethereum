...
contract BALMigration is Ownable, Pausable, IERC721Receiver {
    ...
    /// @param _vault address from which tokens will be transferred
    /// @param _bal address of BAL contract
    constructor(address _vault, ERC721A _bal, uint256 _totalLimit, uint256 _nextTokenId) {
        vault = _vault;
        bal = _bal;
    ...
    function increaseTotalLimit(uint256 _increaseBy) external onlyOwner {
        unchecked {
            totalLimit += _increaseBy;
        }
    }
    ...
    function setBALAddress(ERC721A _bal) external onlyOwner {
        require(address(_bal) != address(0), "Cannot be zero address");
        bal = _bal;
    }

    /// @notice sets new BAL vault
    /// @dev vault address cannot be zero
    /// @param _vault address of BAL
    function setVaultAddress(address _vault) external onlyOwner {
        require(address(_vault) != address(0), "Cannot be zero address");
        vault = _vault;
    }
    ...
    function buy(address _buyer, uint256 _quantity)
        external
        payable
        whenNotPaused
    {
        ...
        require(msg.value == 0, "No funds can be transfered");
        unchecked {
            require(
                bal.balanceOf(_buyer) + _quantity <= MAX_LIMIT,
                "out of buying limit"
            );
            require(_quantity <= totalLimit - usedLimit, "low BAL supply");
            usedLimit += _quantity;
        }
        _mint(_buyer, _quantity);
    }
    ...
    function _mint(address _receiver, uint256 _quantity) private {
        ...
        unchecked {
            ...
            for (uint256 i; i < _quantity; i++) {
                bal.safeTransferFrom(vault, _receiver, startId);
                startId++;
            }
        }
    }
