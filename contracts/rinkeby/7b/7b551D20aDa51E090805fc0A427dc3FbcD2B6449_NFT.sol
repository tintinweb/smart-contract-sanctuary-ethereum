...
contract NFT is Context, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function getHash(address _address, uint256 _amount) external pure returns (bytes32) {
        return _hashRegisterForAllowlistWithAmount(_address, _amount);
    }

    function getSigned(bytes32 hash) external pure returns (bytes32) {
        return ECDSA.toEthSignedMessageHash(hash);
    }

    function getRecover(bytes32 _hash, bytes calldata _signature) external pure returns (address) {
        return _recover(_hash, _signature);
    }

    function getRawRecover(bytes32 _hash, bytes calldata _signature) external pure returns (address) {
        return ECDSA.recover(_hash, _signature);
    }

    function getConcatMsg(bytes32 _hash) external pure returns (bytes32) {
        bytes32 messageDigest = keccak256(abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                _hash
        ));
