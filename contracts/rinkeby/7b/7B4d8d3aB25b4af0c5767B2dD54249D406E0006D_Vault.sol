...
contract Vault is IVault, IncreOwnable {
    ...
    */
    ...
    function setClearingHouse(IClearingHouse newClearingHouse) external override onlyGovernance {
    ...
    function setInsurance(IInsurance newInsurance) external override onlyGovernance {
        if (address(newInsurance) == address(0)) revert Vault_InsuranceZeroAddress();
        insurance = newInsurance;
        emit InsuranceChanged(newInsurance);
    }

    function setOracle(IOracle newOracle) external override onlyGovernance {
    ...
    function changeCollateralWeight(IERC20Metadata asset, uint256 newWeight) external override onlyGovernance {
    ...
    ///         Useful as a risk mitigation measure in case one collateral drops in value
    function changeCollateralMaxAmount(IERC20Metadata asset, uint256 newMaxAmount) external override onlyGovernance {
        uint256 tokenIdx = tokenToCollateralIdx[asset];
        if (!((tokenIdx != 0) || (address(asset) == address(UA)))) revert Vault_UnsupportedCollateral();
