...
contract Lottery is Ownable, ReentrancyGuard {
    ...
    uint256 public maxSupply = 890;
    uint256 public cost = 1 ether;
    uint16 public lotteryId;
    uint8 public maxMintPerAddress = 100;
    ...
    function getWinnerByOrder(uint16 lottery, uint winningOrder) public view returns (address payable ) {
        return lotteryHistory[lottery][winningOrder];
    }
    function getHistoryWinnersByLottery(uint16 lottery) public view returns (address payable [] memory ) {
        return lotteryHistory[lottery];
    }
    function getHistoryWinnersCount(uint16 lottery) public view returns (uint) {
        return lotteryHistory[lottery].length - 1;
    }
    ...
    function pickWinner() public nonReentrant onlyOwner {
        uint256 pickNum = getRandomNumber() % players.length;
        ...
        players.pop();      
        lotteryHistory[lotteryId].push(payable(winner));
