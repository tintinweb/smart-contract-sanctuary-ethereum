...
contract Autoglyphs {
    ...
    uint256 public constant PRICE = 200 * 10**15;

    // The beneficiary is dev wallet
    address public constant BENEFICIARY =
        0x283e4500fd7C9E1BF83b828f13A597b47311700C;
    ...
    modifier validNFToken(uint256 _tokenId) {
        require(idToOwner[_tokenId] != address(0), "address(0)");
        _;
    }
    ...
    function draw(uint256 id) public view returns (string memory) {
        uint256 a = uint256((keccak256(abi.encodePacked(idToSeed[id]))));
        ...
        for (int256 i = int256(0); i < SIZE; i++) {
            ...
            for (int256 j = int256(0); j < SIZE; j++) {
                ...
                output[c] = bytes1(uint8(value << 248));
                c++;
            }
            output[c] = bytes1(uint8(0x25));
            c++;
            output[c] = bytes1(uint8(0x30));
            c++;
            output[c] = bytes1(uint8(0x41));
            c++;
        }
    ...
    function createGlyph(uint256 seed)
        external
        payable
    ...
    function _mint(address _to, uint256 seed) internal returns (string memory) {
        ...
        idToSeed[id] = seed;
        seedToId[seed] = id;
        uint256 a = uint256((keccak256(abi.encodePacked(seed))));
