...
contract Lv1Hunter is ERC721A, Payable {
     ...
     * @param proof The Merkle proof used to validate the leaf is in the root.
     */
    function mintOGSale(uint256 quantity, bytes32[] calldata proof, bytes32 leaf)
        external
        payable
        noBots
        isContractState(ContractState.PRESALE)
        ...
        correctValue(tokenPriceOGSale * quantity)
    {
        ...
        require(verify(merkleRootOG, leaf, proof), "Not a valid proof!");
        _safeMint(msg.sender, quantity);
     ...
     * @param quantity The number of tokens to mint.
     */
    function mintPreAndOG(uint256 quantity, bytes32[] calldata proofPresale, bytes32[] calldata proofOG, bytes32 leaf)
        external
        payable
        noBots
        isContractState(ContractState.PRESALE)
        withinMintLimit(quantity)
    {
        ...
        require(verify(merkleRootPresale, leaf, proofPresale), "Not a valid proof for Presale!");
        require(verify(merkleRootOG, leaf, proofOG), "Not a valid proof for OG!");
