...
contract Market1155 is ERC1155Holder, ReentrancyGuard {
    ...
    function createMarketSale(
        ...
        require(
            msg.value == price * amount * 1000000000,
    ...
    /// @notice function to create a batch sale
    function createBatchMarketSale(
        ...
        // times 9 zero to convert price from gwei to wei
        require(
            msg.value == totalPurchase * 1000000000,
            "Please submit the asking price in order to complete purchase"
        );
    ...
    function createMarketGift(
        ...
        require(
            msg.sender == ownerItem || msg.sender == owner,
    ...
    function createBatchMarketGift(
        ...
        if (senderId == 0) {
            setOwnerIndex(msg.sender);
            senderId = ownerMap[msg.sender];
        }
        ...
        for (uint256 i = 0; i < tokenIdLen; i++) {
            ...
            require(
                _IERC1155.balanceOf(address(msg.sender), itemIds[i]) >=
                    amounts[i],
    ...
    function createBatchGiftUsers(
        ...
        uint256 receiverId;
        uint256 tokenIdLen = itemIds.length;
        uint256 userLength = receviers.length;
        require(
            tokenIdLen == amounts.length,
        ...
        for (uint256 i = 0; i < tokenIdLen; i++) {
            ...
            totalGift += _IERC1155.balanceOf(address(msg.sender), itemIds[i]);
