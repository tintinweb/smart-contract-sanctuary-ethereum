...
contract DegenAfterHours is ERC721A, NFTCatalog, ReentrancyGuard {
  ...
  function calculatePrice(string memory sku, uint16 quantity) public view returns (uint256) {
    ...
    bool discount = boxelContract.balanceOf(msg.sender) > 0;
...
contract NFTCatalog is Ownable {
  ...
  //to deactivate an item, change the quantity left to 0
  function deactivateItem(string memory sku) public onlyOwner {
    getItemBySku(sku);  //check existence
    catalog[sku].quantity = 0;
  }

  ///@dev function is public and will return item quantity amd price by SKU
