...
contract Lands is HeroStakes {
    ...
    function estimateDailyReward() public pure override returns (uint256) {
        // estimated daily rate on 22 attribute points
        return 110;
    }
...
abstract contract HeroStakes is Ownable, IERC721Receiver, Pausable {
    ...
    // lock-up period
    uint256 public minDaysToClaim = 0 days;
    ...
    function stakeHeroes(uint256[] calldata _tokenIds) external virtual whenNotPaused {
        require(_tokenIds.length < maxTokensPerTx, "HeroStakes: too much tokens per tx");

        for (uint i; i < _tokenIds.length; i++) {
            stakes[_tokenIds[i]] = Stake(_msgSender(), block.timestamp);
        }
    ...
    function claimReward(uint256[] calldata _tokenIds) external virtual whenNotPaused {
        ...
        uint256 reward;
        for (uint i; i < _tokenIds.length; i++) {
            require(stakes[_tokenIds[i]].owner == _msgSender(), "HeroStakes: not your token");
            require((block.timestamp - stakes[_tokenIds[i]].lastClaim > minDaysToClaim), "HeroStakes: too early to claim");

            // resolves reward
            reward += _resolveReward(_tokenIds[i]);

            // reset last claim
            stakes[_tokenIds[i]].lastClaim = block.timestamp;
        }
    ...
    /**
     * unstakes heroes and claim reward
     * @param _tokenIds an array of tokenIds to claim reward from
     */
    function unstakeHeroes(uint256[] calldata _tokenIds) external virtual {
        ...
        for (uint i; i < _tokenIds.length; i++) {
            require(stakes[_tokenIds[i]].owner == _msgSender(), "HeroStakes: not your token");
            require((block.timestamp - stakes[_tokenIds[i]].lastClaim > minDaysToClaim), "HeroStakes: too early to unstake");
    ...
    {
        require(_owner != address(0), "HeroStakes: balance query for the zero address");
        uint count;
        for (uint i; i < stakes.length; ++i) {
    ...
    function onERC721Received(
        ...
        require(from == address(0x0), "HeroStakes: cannot send tokens directly");
