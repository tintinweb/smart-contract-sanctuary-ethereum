...
contract TokenUSDT is OwnerOperator, ERC20, Pausable {
    using SafeMath for uint256;

    // max amount token: 1 billion
    uint256 public constant cap = 1000000000 ether;

    constructor() OwnerOperator() Pausable() ERC20("USDT", "USDT") {}
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused(), "TokenUSDT: token transfer while paused");
        if (from == address(0)) {
            // When minting tokens
            require(totalSupply().add(amount) <= cap, "TokenUSDT: cap exceeded");
        }
    }
     ...
     */
    function mint(address to, uint256 amount) external virtual operatorOrOwner {
        _mint(to, amount);
    }
