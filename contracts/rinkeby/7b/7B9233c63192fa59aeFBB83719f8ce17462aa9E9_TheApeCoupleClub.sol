...
// File: contracts/The Ape Couple  Club.sol
...
contract TheApeCoupleClub is ERC721, Ownable, ReentrancyGuard {
  using Counters for Counters.Counter;
  using Strings for uint256;

  Counters.Counter private supply;

  uint256 private maxSupplyTotal = 9999;
  uint256 private maxSupplyPrivate = 9000;
  uint256 private constant price = 0.1 ether;
  uint256 private constant maxPerMint = 3;
  ...
  constructor() ERC721("The Ape Couple Club", "TACC") {
    setHiddenMetadataURI("ipfs://QmTUima1RVTghTLvCooFKmRqgC79Rw1kJjcrbMZJqCSHv1/TACChiddenMetadata1.json");
  ...
  function mintCouple() public payable {
    uint256 totalMinted = totalSupply();

    // checks
    require(whitelistedWallets[msg.sender], "This wallet is not authorized to mint.");
    require(!paused, "Minting is paused.");
    require((totalMinted + maxPerMint) <= maxSupplyPrivate, "Not enough Couples.");
    require(msg.value >= price, "Insufficient balance to mint.");
    require(
      mintedWallets[msg.sender] < maxPerWallet,
      "The number of minted Couples will exceed the maximum amount of allowed per wallet."
    );
  ...
  function reserveCouples(uint256 _count) public onlyOwner {
    uint256 totalMinted = totalSupply();

    require((totalMinted + _count) <= maxSupplyTotal, "Not enough Couples.");
