  ...
  ERC721AUpgradeable
{
  ...
  function initialize(
    ...
    _setRoleAdmin(PRODUCER_ROLE, DEFAULT_ADMIN_ROLE);
  ...
  function mint(
    ...
    require(totalSupply() + _quantity < config.mintConfig.maxSupply);
  ...
  function setAccessRegistry(address _accessRegistry) external onlyRole(DEFAULT_ADMIN_ROLE) {
      accessRegistry = _accessRegistry;
  }

  /// @dev Lets a contract admin set the address for the parent project.
  function setProject(address _project) external onlyRole(DEFAULT_ADMIN_ROLE) {
      project = _project;
  }
  ...
  function grantRole(bytes32 role, address account) public virtual override(AccessControlUpgradeable, IAccessControlUpgradeable) onlyRole(DEFAULT_ADMIN_ROLE) {
    // #TODO check if it still adds roles (enumerable)!
    super.grantRole(role,account);
    IPropsAccessRegistry(accessRegistry).add(account, address(this));
  }

  function revokeRole(bytes32 role, address account) public virtual override(AccessControlUpgradeable, IAccessControlUpgradeable) onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function _hasMinRole(bytes32 _role) internal returns (bool) {
    // @dev does account have role?
    if(hasRole(_role, _msgSender())) return true;
    // @dev are we checking against default admin?
    if(_role == bytes32(0)) return false;
    // @dev walk up tree to check if user has role admin role
    return _hasMinRole(getRoleAdmin(_role));
  }

  // TODO hierarchy model roles
  // mapping(func name => minRole)
  // on func
  // traverse role Admin tree, if none match => false, if any match => true
