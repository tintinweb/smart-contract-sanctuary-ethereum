...
contract Summer is ERC721A, iERC998ERC721TopDown {

    SummerMoments public moments;
    uint16 public currentMomentId = 1;

    constructor() ERC721A("Summer", "S") {
        _mint(msg.sender, 1, "", false);
        moments = new SummerMoments(address(this));
        for (uint16 i = 1; i <= 576; i++) {
            emit ReceivedChild(address(0), 0, address(moments), i);
        }
    ...
    function getMoment() external {
        ...
        Summer(address(this)).safeTransferChild(1, msg.sender, address(moments), currentMomentId++);
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "Summer: URI query for nonexistent token");
        if (moments.balanceOf(address(this)) == 0) {
            return "https://ipfs.io/ipfs/[summer_cid]"; // metadata with completely dark image
        }
        return moments.tokenURI(currentMomentId);
    }
    ...
    // TODO fix 
    // function ownerOf(uint256 tokenId) public view override returns(address) { return owner(); }
    ...
    function ownerOfChild(address _childContract, uint256 _childTokenId) public view override returns (bytes32 parentTokenOwner, uint256 parentTokenId) {
        ...
        require(childOwner == address(this), "Summer: Child token is not owned by token of this contract");
        return (ERC998_MAGIC_VALUE << 224 | bytes32(uint256(uint160(ownerOf(0)))), 0);
    }
    ...
    function safeTransferChild(uint256 _fromTokenId, address _to, address _childContract, uint256 _childTokenId) external override { 
        require(_fromTokenId == 1, "Summer: Invalid token id is given");
        address rootOwner = address(uint160(uint256(rootOwnerOf(0))));
        require(msg.sender == rootOwner || isApprovedForAll(rootOwner, msg.sender) || getApproved(0) == msg.sender, "Summer: not an owner nor not approved");
...
contract SummerMoments is ERC721A {
    ...
    // TODO set
    string private __baseURI = "https://ipfs.io/ipfs/[moments_cid]/"; 

    constructor(address summer_) ERC721A("Summer Moments", "SM") {
        summer = summer_;
        _mint(summer, 576, "", false);
    }

    function _startTokenId() internal pure override returns (uint256) { return 1; }
 
    function _baseURI() internal override view virtual returns (string memory) { return __baseURI; }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "SummerMoments: URI query for nonexistent token");
        require(msg.sender == summer || ownerOf(tokenId) != summer, "SummerMoments: Summer holds this token");

        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString(), ".json")) : "";
    }
    
    function owner() public view returns(address) { return summer; }
