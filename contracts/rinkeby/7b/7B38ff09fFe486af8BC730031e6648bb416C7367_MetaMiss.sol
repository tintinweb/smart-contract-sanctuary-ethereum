...
contract MetaMiss is ERC721Enumerable, Ownable, Allowlist, SalesActivation {
    ...
    // Maximum amount of Miss in existance 
    uint256 public _max_miss = 10000;
    uint256 public _max_sales_miss = 9844;
    // Max presale 
    ...
    // Pause grant
    bool public _claim_paused = true;

    // Cool cat contract address
    address internal misscrypto;
    // Save grant token in order to not re-grant for same misscrypto
    mapping (uint256 => bool) public missCryptoMints;

    // team addresses
    address mcc_crypto = 0x9EFf64424F2FeDb6B4D74771Ddb81C5742FE2c2c;
    address luke_crypto = 0x799CD72D86A6AA46876C2689A13637Cbe6ae89da;
    ...
    constructor(string memory tokenURI, address _missAddr, 
    uint256 _publicSalesStartTime, uint256 _preSalesStartTime, uint256 _preSalesEndTime ) 
    ERC721("MetamissTest", "MTT") Allowlist("MetamissTest","1",luke_crypto) 
    ...
    * @dev Mint Miss
    */
    function mint(uint256 numberOfMiss) public payable isPublicSalesActive {
        require( msg.value >= _price * numberOfMiss,             "Ether sent is not correct" );
        require( miss_index + numberOfMiss < _max_sales_miss,      "Exceeds maximum Miss supply" );
        require(numberOfMiss > 0, "You cannot mint 0 Miss.");
        require(numberOfMiss <= 20, "You are not allowed to buy this many miss at once.");

        uint numberOfMissMinted = 0;
        while(numberOfMissMinted < numberOfMiss){
            if(!_exists(miss_index)){
                numberOfMissMinted++;
    ...
    * @dev Grant Miss
    */
    function claimMiss(uint256[] memory tokenId) external {
        require(!_claim_paused, "Claim is not started.");
        ...
        for(uint i =0 ; i<tokenId.length; i++){
            if( msg.sender == MissCryptoClub(misscrypto).ownerOf(tokenId[i])){
                ...
                missCryptoMints[tokenId[i]] = true;
    ...
    * @dev Presale Miss
    */
    function presale(uint256 missNumber, bytes memory _signature) public payable isPreSalesActive isSenderWhitelisted(missNumber, _signature){
        require(miss_index + missNumber < _max_miss, 'Exceeds maximum Miss supply');
        require(_presaleListClaimed[msg.sender] + missNumber <= presaleListMax, 'Purchase exceeds max allowed');
        require( msg.value >= _price * missNumber,             "Ether sent is not correct" );

        uint numberOfMissMinted = 0;
        while(numberOfMissMinted < missNumber){
            if(!_exists(miss_index)){
                numberOfMissMinted++;
                _safeMint(msg.sender, miss_index);
            }
    ...
    */
    function ownerMint(address _to,uint256 missNumber) external onlyOwner() {
        require( totalSupply() + missNumber < _max_miss,      "Exceeds maximum Miss supply" );
    ...
    */
    ...
    /**
    * @dev Change the total Miss
    */
    function setTotalMiss(uint256 totalMiss) public onlyOwner {
        _max_miss = totalMiss;
    }
    ...
    */
    ...
    function claimPause(bool val) public onlyOwner {
        _claim_paused = val;
    }
