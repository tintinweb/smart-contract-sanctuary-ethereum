...
// File: contracts/TheCryptomasksProject.sol

pragma solidity ^0.8.12;

contract TheCryptomasksProject is ERC721,
    IERC2981, ReentrancyGuard, Ownable {
    using Strings for uint256;
    ...
    string private unRevealedMasks = "ipfs://QmRZwuiKMZ7Vvwm6aHztbcYJ6tW5r7yqiNsZSfUfYdoMSD/";
    string private revealedMasks; // will be revealed after minting
    string private collectionMetadata = "ipfs://QmQwYkbVj7TcamQa3NsuGvFVpEGTKWks8RPkSm19sEktVS/";
    ...
    uint256 private royaltyPercentBps = 750;
    
    // Sale records
    bytes32 private whitelist;
    mapping(address => bool) public claimed;

    constructor() ERC721("TheCryptomasksProject", "TCP") {}
    ...
    function mintPresale(uint256 tokenId, bytes32[] calldata proof) public payable canMint() {
    ...
    function mintPublic() public payable canMint() {
    ...
    function airdrop(address[] memory _addresses) public payable onlyOwner canMint() {
        require(_addresses.length + totalSupply() <= maxSupply, "supply not available");
    ...
    function giveaway(address wallet, uint256 tokenId) public payable onlyOwner canMint() {
        require(tokenId > 0 && tokenId < 555, "invalid tokenId");
    ...
    function setPricesInWei(uint256 _common, uint256 _rare, uint256 _epic, uint256 _legend, uint256 _mythic, uint256 _public) public onlyOwner {
        priceCommon = _common;
        priceRare = _rare;
        priceEpic = _epic;
        priceLegendary = _legend;
        priceMythic = _mythic;
        pricePublicSale = _public;
    }

    function setMetadata(string memory _newMasks) public onlyOwner {
        revealedMasks = _newMasks;
    }

    function setCollectionURI(string memory _CollectionMetadata) public onlyOwner {
        collectionMetadata = _CollectionMetadata;
    }

    function setProxyAddress(address _proxyAddress) public onlyOwner {
        proxyAddr = _proxyAddress;
    }
    ...
    function setRevealed(string memory _newMasks) public onlyOwner {
        revealedMasks = _newMasks;
        revealed = true;
    ...
    function withdrawETH() public onlyOwner nonReentrant {
        require(projectWallet != address(0x0), "no wallet found");
        (bool os, ) = payable(projectWallet).call{value: address(this).balance}("");
        require(os);
    }
    ...
    function isApprovedForAll(address owner, address operator) public override view returns (bool isOperator) {
        // Whitelist OpenSea proxy contract for gass free listing.
        ProxyRegistry proxyRegistry = ProxyRegistry(proxyAddr);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        if (revealed == false) {
            return unRevealedMasks;
        }
        return string(abi.encodePacked(revealedMasks, tokenId.toString(), ".json"));
    }
