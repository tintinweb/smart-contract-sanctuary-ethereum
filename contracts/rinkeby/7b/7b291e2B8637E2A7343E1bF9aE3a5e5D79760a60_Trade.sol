...
contract Trade {
    ...
    function testtradeAsset(Order memory order) public returns(bool) {
        IERC721 token = IERC721(order.nftAddress);
        token.safeTransferFrom(order.seller, order.buyer, order.tokenId);
        return true;
    }
}