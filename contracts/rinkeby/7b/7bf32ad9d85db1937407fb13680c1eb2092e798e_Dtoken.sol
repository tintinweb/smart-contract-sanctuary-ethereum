...
contract Dtoken is Initializable, UUPSUpgradeable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    IERC20Upgradeable public add;
    ...
    struct UserInfo {
        ...
        uint256 rewardADDDebt; // Reward debt in ADD.
        //
        // We do some fancy math here. Basically, any point in time, the amount of ADD
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accADDPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accADDPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. ADDs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that ADDs distribution occurs.
        uint256 accADDPerShare; // Accumulated ADDs per share, times 1e12. See below.
        uint256 lastTotalADDReward; // last total rewards
        uint256 lastADDRewardBalance; // last ADD rewards tokens
        uint256 totalADDReward; // total ADD rewards tokens
    }
    ...
    // Bonus muliplier for early ADD makers.
    ...
    // total ADD staked
    uint256 public totalADDStaked;
    // total ADD used for purchase land
    uint256 public totalAddUsedForPurchase;
    ...
    function initialize(        
        ...
        __ERC20_init_unchained("DToken", "DToken");
        __Pausable_init_unchained();
        ERC20PermitUpgradeable.__ERC20Permit_init("DToken");
    ...
    function addlp(uint256 _allocPoint, IERC20Upgradeable _lpToken, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        poolInfo.push(PoolInfo({
            ...
            accADDPerShare: 0,
            lastTotalADDReward: 0,
            lastADDRewardBalance: 0,
            totalADDReward: 0
        }));
    }

    // Update the given pool's ADD allocation point. Can only be called by the owner.
    ...
    function pendingADD(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accADDPerShare = pool.accADDPerShare;
        uint256 lpSupply = totalADDStaked;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 rewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalADDStaked.sub(totalAddUsedForPurchase));
            uint256 _totalReward = rewardBalance.sub(pool.lastADDRewardBalance);
            accADDPerShare = accADDPerShare.add(_totalReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accADDPerShare).div(1e12).sub(user.rewardADDDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 rewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalADDStaked.sub(totalAddUsedForPurchase));
        uint256 _totalReward = pool.totalADDReward.add(rewardBalance.sub(pool.lastADDRewardBalance));
        pool.lastADDRewardBalance = rewardBalance;
        pool.totalADDReward = _totalReward;
        
        uint256 lpSupply = totalADDStaked;
        if (lpSupply == 0) {
            pool.lastRewardBlock = block.number;
            pool.accADDPerShare = 0;
            pool.lastTotalADDReward = 0;
            user.rewardADDDebt = 0;
            pool.lastADDRewardBalance = 0;
            pool.totalADDReward = 0;
            return;
        }
        
        uint256 reward = _totalReward.sub(pool.lastTotalADDReward);
        pool.accADDPerShare = pool.accADDPerShare.add(reward.mul(1e12).div(lpSupply));
        pool.lastTotalADDReward = _totalReward;
    }

    /**
    @notice Sorting the highest ADD staker in pool
    @param _pid : pool id
    @param left : left
    @param right : right
    @dev Description :
        It is used for sorting the highest ADD staker in pool. This function definition is marked
    ...
    */
    ...
    @dev Description :
    DAO governance will be performed by the top 50 wallets with the highest amount of staked ADD tokens.
    */
    ...
    /**
    @notice ADD staking track the Highest 50 staked users
    @param _pid : pool id
    ...
    */
    ...
    // Deposit ADD tokens to MasterChef.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            uint256 addReward = user.amount.mul(pool.accADDPerShare).div(1e12).sub(user.rewardADDDebt);
            pool.lpToken.safeTransfer(msg.sender, addReward);
            pool.lastADDRewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalADDStaked.sub(totalAddUsedForPurchase));
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        totalADDStaked = totalADDStaked.add(_amount);
        user.amount = user.amount.add(_amount);
        user.rewardADDDebt = user.amount.mul(pool.accADDPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        totalADDStaked = totalADDStaked.sub(_amount);
        user.rewardADDDebt = user.amount.mul(pool.accADDPerShare).div(1e12);
    ...
    // Earn ADD tokens to MasterChef.
    function claimADD(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
    ...
    // Safe ADD transfer function to admin.
    function accessADDTokens(uint256 _pid, address _to, uint256 _amount) public {
        require(msg.sender == adminAddress, "sender must be admin address");
        require(totalADDStaked.sub(totalAddUsedForPurchase) >= _amount, "Amount must be less than staked ADD amount");
        PoolInfo storage pool = poolInfo[_pid];
        uint256 AddBal = pool.lpToken.balanceOf(address(this));
        if (_amount > AddBal) {
            pool.lpToken.transfer(_to, AddBal);
            totalAddUsedForPurchase = totalAddUsedForPurchase.add(AddBal);
            emit EmergencyWithdraw(_to, _pid, AddBal);
        } else {
            pool.lpToken.transfer(_to, _amount);
            totalAddUsedForPurchase = totalAddUsedForPurchase.add(_amount);
