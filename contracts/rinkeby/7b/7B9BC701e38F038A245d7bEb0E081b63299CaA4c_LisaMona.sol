...
contract LisaMona is Ownable, ERC1155Holder, Utils {
  ...
  function mintArtistCreation(uint _artistId) public returns (uint) {
    ...
    // block.timestamp is always going to lag "now" but a certain amount of time due to
    // the time it tags to mine a block, so add on a bit of buffer to it. This prevents
