...
contract Perpetual is IPerpetual, Pausable {
        ...
        )
    {
        ...
        if (isNewPosition || (traderPositionSize > 0 ? LibPerpetual.Side.Long : LibPerpetual.Side.Short) == direction) {
            ...
            (quoteProceeds, baseProceeds, profit) = _openReversePosition(account, amount, minAmount);
        }
        ...
        /// @dev position can be increase by either extending a position or opening a (larger) reverse position
        isPositionIncreased = (baseProceeds + traderPositionSize).abs() > traderPositionSize.abs();

        int256 maxPosition = maxBlockTradeAmount / 10;
        if (trader.openNotional.abs() > maxPosition || trader.positionSize.abs().wadMul(indexPrice()) > maxPosition)
    ...
    function removeLiquiditySwap(
        ...
        // Always revert
        revert Perpetual_SwapAfterLiquidityRemoval(baseProceeds);
    }
    ...
    function setParameters(PerpetualParams memory params) public override onlyClearingHouseOwner {
        ...
        )
    {
        ...
        @dev: Weak inequality holds when position is fully closed
        */
        ...
        if (isReducingPosition) {
            ...
            Answer:
                    realized pnl =  quoteProceeds - openNotional * reductionRatio
                    openNotional += openNotional * reductionRatio
            */
            ...
            Question: What part of the quoteProceeds should be settled into pnL vs make up the new `openNotional`?
            Answer:
                    realized pnl =  unrealized pnl of the position
                    openNotional += quoteProceeds - realized pnl
            */

            pnl = _getUnrealizedPnL(user);
            addedOpenNotional = quoteProceeds - pnl;
        }
    }
    ...
    function _settleLpPosition(
        ...
        if (diff > 0 && diff <= 1e17) _donate(diff.toUint256());
    }
