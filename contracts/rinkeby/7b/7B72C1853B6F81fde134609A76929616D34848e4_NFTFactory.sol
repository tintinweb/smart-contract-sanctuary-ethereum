...
contract NFTFactory is Ownable {
    ...
    uint256 private contract_creation_fee = 1 * 10**18; //1 ETH
    ...
    function changeContractCreationFee(uint256 newFee)
        ...
        onlyOwner
        {
            contract_creation_fee = newFee * 10**18;
        }
    ...
    function createNFTProject(uint256 _maxTokens, string memory _assetName, string memory _ticker, uint256 _price, uint256 _preSalePrice, address _verifyingAccount) 
        public
        payable
        returns (NFTContract)
    {
        ...
        NFTContract newNFTContract = new NFTContract(_maxTokens, _assetName, _ticker, _price, _preSalePrice, _verifyingAccount, msg.sender, tax_rate, address(this));
...
contract NFTContract is ERC721, Ownable {
    ...
    constructor(uint256 maxTokens, string memory assetName, string memory ticker, uint256 price, uint256 preSalePrice, address verifyingAccount, address new_owner, uint256 tax_rate, address tax_man) ERC721(assetName, ticker) 
    {
    ...
    function withdraw(address payee) public payable onlyOwner {
        ...
        payable(payee).transfer(balance * (100 - _tax_rate) / 100);
    }
