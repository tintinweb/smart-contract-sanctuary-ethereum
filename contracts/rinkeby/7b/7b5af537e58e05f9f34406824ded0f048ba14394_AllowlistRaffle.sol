 ...
 * @title Allowlist raffle contract
 * @author Augminted Labs, LLC
 * @notice Winners are calculated deterministically off-chain using a provided script
 */
contract AllowlistRaffle is Ownable, ReentrancyGuard, VRFConsumerBaseV2 {
    ...
    /**
     * @notice Set cost to purchase a single raffle entry
     * @param _entryFee Cost of a raffle entry
     */
    function setEntryFee(uint256 _entryFee) public lockable onlyOwner {
        entryFee = _entryFee;
    }

    /**
     * @notice Set token to be used for payment
     * @param _token Burnable ERC20 token to be used for payment
     */
    function setToken(IBurnableERC20 _token) public lockable onlyOwner {
        token = _token;
    }
    ...
    function enter(uint256 amount) public payable nonReentrant {
        ...
        if (address(token) == address(0)) {
            if (entryFee * amount < msg.value) revert InsufficientValue();
        } else {
            token.burn(entryFee * amount);
        }
