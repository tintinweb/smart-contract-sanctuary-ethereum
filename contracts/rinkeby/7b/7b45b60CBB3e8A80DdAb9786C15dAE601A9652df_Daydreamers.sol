...
contract Daydreamers is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public finalMintedTokenBlockNumber;
        ...
        nonReentrant
    {
        ...
        if (totalSupply() + _quantity == COLLECTION_SIZE) {
            finalMintedTokenBlockNumber = block.number;
        }
    ...
    function mint(uint8 _quantity)
        ...
        nonReentrant
    {
        if (msg.value != mintPricePublic * _quantity) {
    ...
    /**
     * @notice Set public mint price. This function will be called prior to public mint
     * using the median OpenSea sale price in the preceeding 24 hours.
     * @notice Use restricted to contract owner
     * @param _mintPricePublic Public mint price in wei
     */
    function setMintPricePublic(uint256 _mintPricePublic) external onlyOwner {
    ...
    function setMintPhase(MintPhase _mintPhase) external onlyOwner {
        if (_mintPhase == MintPhase.PUBLIC_SALE && mintPricePublic == 0) {
            revert PublicMintPriceNotSet();
        }
    ...
    /**
     * @notice Get starting index into previously-ordered metadata for reveal
     */
    function getInitialMetadataSequenceIndex() public view returns (uint256) {
        if (finalMintedTokenBlockNumber == 0) {
            revert FinalMintedTokenBlockNumberNotSet();
        }
        return
            uint256(blockhash(finalMintedTokenBlockNumber)) % COLLECTION_SIZE;
    }
...
/**
 * Final minted token block number not set
 */
error FinalMintedTokenBlockNumberNotSet();
