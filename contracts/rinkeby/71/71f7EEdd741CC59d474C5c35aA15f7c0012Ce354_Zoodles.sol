...
contract Zoodles is ERC721A, Ownable {
    ...
    string public baseURL = "ipfs://zoodles/";
    bool public startMint = false;
    uint256 public maxFreeMintPerWallet = 1;
    uint256 public maxMintPerTx = 10;
    uint256 public maxFreeMintNumber = 555;
    ...
    function freeMint() public {
        ...
        require(msg.sender == tx.origin, "minting from contract not supported");                // not minting from contract    // yes
        require(startMint, "mint not started");                                                 // mint started                 // yes
        require(_totalSupply + maxFreeMintPerWallet <= maxFreeMintNumber, "free mint over");    // can still free mint          // yes yes
        require(!userFreeMinted[msg.sender], "only 1 free");                                    // hasn't already free minted   // yes
        userFreeMinted[msg.sender] = true;                                                                                      // yes
        _safeMint(msg.sender, maxFreeMintPerWallet);                                                                            //
    }

    function paidMint(uint256 quantity) public payable {
        uint256 _totalSupply = totalSupply();
        require(msg.sender == tx.origin);                                                       // not minting from contract
        require(startMint, "mint not started");                                                 // mint started                     // yes
        require(_totalSupply + quantity <= maxNumber, "sold out");                              // still available                  // yes
        require(quantity <= maxMintPerTx, "invalid quantity");                                  // max tx quantity
        require(msg.value == quantity * price, "incorrect eth value");                          // check correct eth value sent     // yes
