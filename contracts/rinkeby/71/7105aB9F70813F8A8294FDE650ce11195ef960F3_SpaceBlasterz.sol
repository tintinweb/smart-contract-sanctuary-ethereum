...
contract SpaceBlasterz is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor() ERC721A("SpaceBlasterz", "SPB") {}
    ...
    function mintNFT(uint256 _cnt) public payable {
        ...
        if(!publicSale) {
            require(whitelist[msg.sender], "Not Eligible to Mint");
            require((getNFTBalance(msg.sender) + _cnt) <= 3, "Exceeded the Minting Count");
        }

        if(publicSale) {
            require((getNFTBalance(msg.sender) + _cnt) <= 4, "Exceeded the Minting Count");
        }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if(!reveal) return bytes(unrevealURI).length > 0 ? string(abi.encodePacked(unrevealURI, (tokenId+1).toString(), ".json")) : "";
