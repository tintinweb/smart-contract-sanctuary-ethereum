...
contract MetaMotorsSales is AccessControl, ReentrancyGuard {
    ...
    //Max token supply
    uint256 public constant MAX_TOTAL_SUPPLY = 3300;

    //Root of merkle tree for allow lists
    ...
    constructor(MetaMotorsToken metaMotorsTokenAddress) {
    ...
    modifier hasSupply(uint256 quantity) {
        require(
            metaMotorsToken.getTotalMinted() + quantity <= MAX_TOTAL_SUPPLY,
    ...
    /**
     * @dev Validate an address is on the allow list
     */
    ...
    function _isValidOrder(
        ...
        bytes32 messagehash = keccak256(abi.encodePacked(quantity, sender));
    ...
    /**
     * @dev mint `quantity` to msg sender on allow list
     *
     * Requirements:
     *
     * - enough token supply left
     * - the msg sender is on the allow list
     */
    function allowListMint(
        ...
        require(
            ...
            "Address is not eligibile for allow list"
        );

        _mintMetaMotors(quantity, signature);
    }
     ...
     * - enough token supply left
     */
    function publicMint(uint256 quantity, bytes memory signature)
        external
        payable
        nonReentrant
        hasSupply(quantity)
     ...
     * - price is valid
     */
    function _mintMetaMotors(uint256 quantity, bytes memory signature)
        internal
    {
        require(
            _isValidOrder(quantity, signature, _msgSender()),
            "Invaild order"
        );
