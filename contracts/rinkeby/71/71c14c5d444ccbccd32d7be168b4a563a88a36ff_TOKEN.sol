...
contract TOKEN is Context, IERC20, Ownable {
    ...
    constructor() {
        ...
        _charityWalletAddress = 0x4AF1a5780DcA8DD46ebc9Ad625F893aA22aB6AF5;
    ...
    function _transferBothExcluded(
        ...
        _takeLiquidityAndBurn(tLiquidity);
    ...
    function _takeLiquidityAndBurn(uint256 tLiquidityAndBurn)
        private
    {
        uint256 currentRate = _getRate();
        uint256 rLiquidityAndBurn = tLiquidityAndBurn.mul(
            currentRate
        );
    ...
    function calculateLiquidityAndBurnFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul((_liquidityFee.add(_burnFee))).div(10**2);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tFee = _charityFee.add(_liquidityFee);
        uint256 burnTokens = contractTokenBalance.div(tFee).mul(
            _burnFee
        );
        _transfer(address(this),_burnAddress,burnTokens);
        uint256 liquidityTokens = contractTokenBalance.sub(burnTokens);
