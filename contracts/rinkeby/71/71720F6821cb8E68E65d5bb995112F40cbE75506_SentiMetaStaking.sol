...
contract SentiMetaStaking is Ownable, ISentiMetaStaking {
    ...
    // Events
    event Stake(address indexed owner, address indexed project, uint96 indexed tokenId,  uint96 totalStaked);
    event Unstake(address indexed owner, address indexed project, uint96 indexed tokenId, uint96 totalStaked);
    ...
    // Extracts the index from a uint256 storage id
    // function _extractIndexFromStorageId(uint256 storageId_) private pure returns (uint256) {
    //     return storageId_ & 0xFFFFFFFFFFFFFFFFFFFFFFFF;
    // }
    ...
    function _stake(uint96 tokenId_, address projectAddress_, address owner_) private {
        ...
        emit Stake(owner_, projectAddress_, tokenId_, _projectAddressToStakedCounts[projectAddress_]);
    }
    ...
    function _unstake(address projectAddress_, uint256 storageId_, uint96 tokenId_, address owner_) private {
        ...
        emit Unstake(owner_, projectAddress_, tokenId_, uint96(lastIndex));
    }
