...
contract SupDucksV3 is ERC721EnumerableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    */
    ...
    function lick(uint16 duckId, uint16 toadId) public {
        require(toadLicked[duckId] == 0, "duck has already licked a toad");
        ...
        uint level;
        (,,,,,level) = megaToads.getTraits(toadId);
        levels[duckId] = uint8(level + 1);
        emit Lick(duckId, toadId, levels[duckId]);
    }

    function claimDuckGodGift(address duckOwner, uint16 duckId, uint price) public returns (uint8) {
    ...
    function unlick(uint16 duckId) public {
