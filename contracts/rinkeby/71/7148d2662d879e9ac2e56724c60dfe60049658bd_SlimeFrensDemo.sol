...
contract SlimeFrensDemo is ERC721A, ReentrancyGuard, Ownable {
    ...
    uint256 private minimumPrice = 0.001 ether;
    uint256 private decreaseAmount = 0.002 ether;
    ...
    /* Executes only when the contract gets deployed */
    constructor(string memory _baseURL, string memory _unrevealedURL) ERC721A("Slime Frens Demo", "SLIMEDEMO") {
        baseURL = _baseURL;
    ...
    /* Public functions */
    //Main function for minting an NFT from the collection.
    function mint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable nonReentrant mintRequirements(_mintAmount, _merkleProof) {
        ...
        if(saleState == SaleState.WHITELIST_SALE)hasClaimedWhitelist[msg.sender] = true;
    ...
    /* Owner Functions */
    //Set a new URL for when the collection is revealed.
    function setNewBaseURL(string memory _newBaseURL) public onlyOwner {
        baseURL = _newBaseURL;
    }

    //Set a new URL for when collection is unrevealed.
    ...
    //Reveal the NFT collection.
    function revealCollection() public onlyOwner {
        revealed = !revealed;
    }

    //Decrease the mint price.
    function decreasePrice(uint256 _decreaseBy) public onlyOwner {
        mintPrice -= _decreaseBy;
    }

    //Set new maximum mint amount per wallet.
    function setNewMaxPerWallet(uint256 _newMaxPerWallet) public onlyOwner {
        maxPerWallet = _newMaxPerWallet;
    }

    //Set a new merkle root to change the whitelist.
    function setNewMerkleRoot(bytes32 _newMerkleRoot) public onlyOwner {
        merkleRoot = _newMerkleRoot;
    }

    //Set a new baseURL suffix.
    function setNewURLSuffix(string memory _newURLSuffix) public onlyOwner {
        URLSuffix = _newURLSuffix;
    }

    //Allows owner to mint any amount to self for free.
    function ownerMint(uint256 _amount) public onlyOwner {
    _safeMint(msg.sender, _amount);
    }

    //Allows owner to mint to any address for free.
    function mintForAddress(address _receiverAddress, uint256 _amount) public onlyOwner {
        _safeMint(_receiverAddress, _amount); 
    }

    //Set a new minimum price.
    function setNewMinimumPrice(uint256 _newMinimumPrice) public onlyOwner {
        minimumPrice = _newMinimumPrice;
    }

    //Set a new amount to decrease the price with.
    function setNewDecreaseAmount(uint256 _newDecreaseAmount) public onlyOwner {
        decreaseAmount = _newDecreaseAmount;
    }
    ...
    //Decreate the price.
    function decreasePrice() public onlyAuthorized nonReentrant {
        require(mintPrice - decreaseAmount >= minimumPrice, "Minimum price reached!");
        mintPrice -= decreaseAmount;
    }

    //Set an address for the keeper to be allowed to decrease the price.
    function setNewKeeperAddress(address _newKeeperAddress) public onlyOwner {
        keeperAddress = _newKeeperAddress;
    }


    /* Modifiers */
    //Requirements to be able to mint successfully.
    modifier mintRequirements(uint256 _mintAmount, bytes32[] calldata _merkleProof){

        if(saleState == SaleState.WHITELIST_SALE){
        ...
        require(amountMinted[msg.sender] + _mintAmount <= maxPerWallet, "Max per wallet exceeded");
        require(msg.value >= mintPrice * _mintAmount, "You did not send enough ETH");
        _;
    }

    //Authorized to run the decrease price function.
