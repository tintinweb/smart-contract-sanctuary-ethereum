...
contract TestPool is ITestPool, IContract, TestPoolNFT, Documents {
    ...
    struct TokenInfo {
        uint128 staked;
        mapping (address => TokenRewards) tokenRewards;
    }
        ...
        internal
    {
        ...
        if (userBalance == 0) {
            ...
            for(uint i = 0; i < rewardTokens.length; i++) {
                ...
                if(tokenInfo[_tokenId].tokenRewards[rewardToken].lastRewardPoints == 0) {
                    tokenInfo[_tokenId].tokenRewards[rewardToken].lastRewardPoints = poolRewards[rewardToken].rewardsPerTokenPoints;
                }
            }
        }
        ...
        totalStaked += BoringMath.to128(_amount);
        ...
        public
    {
        ...
        if (_exists(_tokenId) && totalStaked > 0) {
            address[] memory rewardTokens = rewardsContract.rewardTokens(address(this));
            for(uint i = 0; i < rewardTokens.length; i++) {
                address rewardToken = rewardTokens[i];
                RewardInfo memory rInfo = poolRewards[rewardToken];
                uint256 tokensStaked = tokenInfo[_tokenId].staked;
                TokenRewards memory tRewards = tokenInfo[_tokenId].tokenRewards[rewardToken];
                /// PW: Only update if something has updated, Check if this if() makes sense, might need to be seperate.
                if (tRewards.lastRewardPoints != rInfo.rewardsPerTokenPoints || uint256(rInfo.lastUpdateTime) != block.timestamp) {
                    uint256 currentRewards = rewardsContract.poolRewards(address(this), rewardToken, uint256(rInfo.lastUpdateTime), block.timestamp);
                    uint256 rewardsPerTokenPoints = rInfo.rewardsPerTokenPoints + (currentRewards * 1e18 * pointMultiplier / totalStaked);
                    
                    uint256 rewards = tokensStaked * (rewardsPerTokenPoints - tRewards.lastRewardPoints)
                                                            / 1e18
                                                            / pointMultiplier;

                    poolRewards[rewardToken] = RewardInfo(BoringMath.to64(block.timestamp),rewardsPerTokenPoints);
                    tokenInfo[_tokenId].tokenRewards[rewardToken] = TokenRewards(tRewards.rewardsEarned + BoringMath.to128(rewards),tRewards.rewardsReleased, rewardsPerTokenPoints);
                }
            }
        }
    }
    ...
    function _claimRewards(uint256 _tokenId, address _rewardToken) internal {
        TokenRewards storage _tokenRewards = tokenInfo[_tokenId].tokenRewards[_rewardToken];
