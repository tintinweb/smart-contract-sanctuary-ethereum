...
contract OtherdeedDao is ERC721A, Ownable {
  using Strings for uint256;
  
  uint256 public mintPriceTier1 = 0.01 ether;
  uint256 public mintPriceTier2 = 0.02 ether;
  uint256 public mintPriceTier3 = 0.03 ether;
  uint256 public mintPrice = 0.03 ether;

  uint256 public maxSupply = 8888;
  uint256 public maxFreeMint = 8888;
  uint256 public maxMintBatch = 2000;
  uint256 public freeMintPerWallet = 1;
  string public baseTokenURI = "https://gateway.pinata.cloud/ipfs/QmZZR5GiT9m8LLx2pfeDQXJVvD7bwdrEG6dVde3hGDRu6d";
  ...
  bool public onMintTeam = true;

  mapping(address => uint256) public addressMintedBalance;

  constructor() ERC721A("Otherdeed DAO", "otherdeeddao", 100) {  }
  ...
  function mintFree() external payable {
    ...
    uint256 _mintAmount = freeMintPerWallet;
    if(currentIndex <= 500) {
        _mintAmount = 3;
    }
    else if (currentIndex <= 1500) {
        _mintAmount = 2;
    }
    else if (currentIndex <= 2500) {
        _mintAmount = 1;
    }

    require(balanceOf(_msgSender()) < 1, "Max free mint exceeded!");
    require(currentIndex + _mintAmount <= maxFreeMint, "Max free mint exceeded!");
  ...
  function mintMany(uint256 _mintAmount) external payable {
    require(onMintTeam, "No more team mint");
    require(_mintAmount <= 100, "Amount exceeded 100");
    require(currentIndex + _mintAmount <= maxMintBatch, "Max supply exceeded!");
    require(currentIndex + _mintAmount <= maxSupply, "Max supply exceeded!");
    _safeMint(msg.sender, _mintAmount);
  }
    ...
    returns (uint256)
  {
    ...
    if(currentIndex <= 2000){
        _mintPrice = mintPriceTier1;
    }
    else if(currentIndex <= 5000) {
        _mintPrice = mintPriceTier2;
    }
    else if(currentIndex <= 8000) {
        _mintPrice = mintPriceTier3;
    }
  ...
  function setOnMintTeam(bool _state) public onlyOwner {
    onMintTeam = _state;
  }
