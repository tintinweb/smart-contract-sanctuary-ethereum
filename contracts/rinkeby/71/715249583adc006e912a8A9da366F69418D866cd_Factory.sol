...
contract Buffer is Initializable {
    ...
    mapping(address => mapping(uint256 => address)) public _creatorInfo;
    ...
    address[] public partnersGroup;
    address[] public creatorsGroup;
    ...
    function updateCreatorPairInfo(
        ...
        for (uint256 i = 0; i < creators.length; i++) {
            _creatorInfo[NFTAddress][tokenIDs[i]] = creators[i];
        }
    }
    ...
    receive() external payable {
        ...
        ERC721Enumerable NFTs = ERC721Enumerable(NFTAddress);
        uint256 totalNumOfNFT = NFTs.totalSupply();
        for (uint256 i = 1; i <= totalNumOfNFT; i++) {
            _shareData[NFTAddress][NFTs.ownerOf(i)].shareAmount +=
                (msg.value * share[5]) /
                totalNumOfNFT /
                totalShares;
        }
