...
contract SubdomainManager is  IManager, IERC721Receiver, Ownable, ERC165, ERC721, IERC2981, ENSEnumerable {

    using SafeMath for uint256;
    using Strings for uint256;

    event DepositEns(address indexed _from, uint256 indexed _id, string _domainLabel);
    event WithdrawFunds(address indexed _from, uint256 indexed _id, uint256 _funds);


    ENS private ens = ENS(0x00000000000C2E074eC69A0dFb2997BA6C7d2e1e); 
    IERC721 public EnsToken = IERC721(0x57f1887a8BF19b14fC0dF6Fd9B2acc9Af147eA85);
    IENSToken public ens_func = IENSToken(0x57f1887a8BF19b14fC0dF6Fd9B2acc9Af147eA85);
    ...
    //58555766860124361556866153834322068182971435010206675046809302716045169374029
    //0x7581fe770848f54fc0245a63067c0ae9789285571751c6549f233c2098442335
    event TextChanged(bytes32 indexed node, string indexed indexedKey, string key);

    uint256 public DepositCost = 0 ether;

    constructor() ERC721("ENS sub-domains", "ESD"){
    ...
    function withdrawEns(uint256 _id) external {

        require(address(this) == EnsToken.ownerOf(_id), "not owner");
        require(!TokenLocked[_id], "token is locked");
        EnsToken.safeTransferFrom(address(this), msg.sender, _id);

        //this is so that we can withdraw tokens that have been deposited without safeTransfer
        if (TokenOwnerMap[_id] != address(0)){
            --TLDBalances[msg.sender];
        }
        _beforeTLDTransfer(msg.sender, address(0), _id);
    }

    function uncheckedWithdraw(uint256 _id) external {
        require(address(this) == EnsToken.ownerOf(_id), "not owner");
        require(!TokenLocked[_id], "token is locked");
        EnsToken.safeTransferFrom(address(this), msg.sender, _id);
    }
    ...
    function depositEns(string calldata _label) payable external {
        require(msg.value == DepositCost, "incorrect ether");      
        uint256 id = getIdFromLabel(_label);
        require(ens_func.nameExpires(id) > (block.timestamp + 90 days), "domain expires too early");

        ens_func.reclaim(id, address(this));
    ...
    /// Token owner sub-domain creation method. This is free.
    /// @param _id ID of ENS token
    /// @param _label label of the sub-domain. This should be lower-case and 
    //  use supported characters
    function tokenOwnerRegisterSubdomain(uint256 _id, string memory _label) public tokenOwner(_id) {
        require(msg.sender == TokenOwnerMap[_id], "not authorised");
        register(_id, _label);
    }

    ///function to purchase sub-domain from owner
    ///@param _tokenId token id of the parent ens domain
    ///@param _label label for the subdomain, should be lower-case, numeric not uppercase
    function purchaseSubdomain(uint256 _tokenId, string memory _label) payable public {
        require(msg.value == MintCost[_tokenId], "incorrect ether");
        require(MintCost[_tokenId] != 0, "not for primary sale");
        address owner = TokenOwnerMap[_tokenId];
        register(_tokenId, _label);
        OwnerFunds[owner] = OwnerFunds[owner] + msg.value;
    }

    function register(uint256 _tokenId, string memory _label) private {
        bytes32 encoded_label = keccak256(abi.encodePacked(_label));
        bytes32 domainHash = getDomainHash(IdToDomain[_tokenId]);
        bytes32 hash = keccak256(abi.encodePacked(domainHash, encoded_label));

        uint256 id = currentId++;
        IdToHashMap[id] = hash;
        HashToIdMap[hash] = id;
        _safeMint(msg.sender, id);
        IdToOwnerId[id] = _tokenId;
    ...
    function text(bytes32 node, string calldata key) external view returns (string memory) {
        string memory value;
        string memory _val = texts[node][key];
        if(keccak256(abi.encodePacked(key)) == keccak256("avatar") && bytes(_val).length == 0){
            uint256 id = HashToIdMap[node];
            uint256 owner = IdToOwnerId[id];
            value = IdImageMap[owner];
        }
        else{
            value = texts[node][key];
        }
        return value;
    }
    ...
    function setText(bytes32 node, string calldata key, string calldata value) external {
        ...
        require(ownerOf(token_id) == msg.sender , "not approved");
        
        texts[node][key] = value;
        emit TextChanged(node, key, key);
    }

    function setRoyaltyPercent(uint256 _id, uint256 _percent) external tokenOwner(_id) {
        require(_percent <= 10, "max 10 percent");

        IdRoyaltyMap[_id] = _percent;
    }

    function setMintPrice(uint256 _id, uint256 _price) external tokenOwner(_id) {

        require(_price >= 0.01 ether, "min 0.01 ether");
        MintCost[_id] = _price;
    }

    function setSubdomainImageUri(uint256 _id, string calldata _uri) tokenOwner(_id) external {
    ...
    function transferDomainOwnership(uint256 _id, address _newOwner) external {       
        ...
        --TLDBalances[currentOwner];
        _beforeTLDTransfer(currentOwner, _newOwner, _id);

        unchecked {
            ++TLDBalances[_newOwner];
        }
    }
...
contract MetadataProviderV1 is IMetadata {
   ...
   function tokenURI(uint256 tokenId) public view returns(string memory){
        ...
        string memory locked = Manager.TokenLocked(ownerId) ? 'true' : 'false';
        string memory image = Manager.IdImageMap(ownerId);
        uint256 expiry = ens_func.nameExpires(ownerId);
        ...
        return string(  
            abi.encodePacked(
                ...
                , '","attributes":[{"trait_type" : "parent", "value" : "'
                , parentName
                , '.eth"},{"trait_type" : "ens locked", "value" : "'
                , locked
                , '"},{"trait_type" : "expiry", "display_type": "date","value": ', expiry.toString(), '}]}'
                )
                );
...
abstract contract ENSEnumerable  {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual returns (uint256) {
        require(index < TLDBalances[owner], "ERC721Enumerable: owner index out of bounds");
