  ...
  ERC677ReceiverInterface
{
  ...
  function addDummyUpkeeps(uint256 lim) external onlyOwner {
    for (uint256 idx = 0; idx < lim; idx++) {
      s_upkeepIDs.add(
        uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), address(this), idx)))
      );
    }
  }
    ...
    )
  {
    ...
    (bool success, bytes memory result) = upkeep.target.call{gas: s_storage.checkGasLimit}(
      callData
    );
   ...
   */
  function setPeerRegistryMigrationPermission(address peer, MigrationPermission permission)
    external
    onlyOwner
  {
