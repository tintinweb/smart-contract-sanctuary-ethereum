...
abstract contract ERC20 is Ownable, IERC20 {
    ...
    /// @dev mint reserve token, no time locking.
    function mintFromReserve(address recipient, uint256 amount) public onlyOwner {
        require(amount > 0, "Mint amount too less");
    ...
    function batchMintFromReserve(address[] memory recipients, uint256[] memory amounts) external onlyOwner {
        require(recipients.length == amounts.length, "Unmatched array length");

        for (uint256 i = 0; i < recipients.length; i++) {
            mintFromReserve(recipients[i], amounts[i]);
        }
    }
