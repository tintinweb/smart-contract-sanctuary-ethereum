    ...
    ERC2981
{
    ...
    function addBlacklist(address _user) external {
         require(
    ...
    function resetTokenRoyalty(uint256 tokenId) internal virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "ERC1155Token: must have admin role"
        );
    ...
    function setRoyaltyfeeNumerator(uint96 _royaltyfeeNumerator) external {
        require(
            _royaltyfeeNumerator <= _feeDenominator(),
            "ERC2981: royalty fee will exceed salePrice"
        );
