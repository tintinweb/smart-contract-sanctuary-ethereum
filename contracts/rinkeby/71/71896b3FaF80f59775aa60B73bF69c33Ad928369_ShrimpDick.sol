...
/* Tired of having a shrimp dick? Buy this token hodl and let it grow. */
/*     Max tx: 1%
       Max wallet: 2%
       Taxes: 0 Buy / 0 Sell + 24hr 25% Anti-Jeet tax ( 15% Burns/ 5% LP/ 5% Marketing )*/
       
contract ShrimpDick is ERC20, Ownable {
    ...
    uint256 public blocksForPenalty;
    ...
    function enableTrading(uint256 _deadBlocks)external onlyOwner{
        ...
        blocksForPenalty = _deadBlocks;
        blockForPenaltyEnd = launchBlock + _deadBlocks;
        
    }

    function deadBlocksActive() public view returns(bool){
        return (blockForPenaltyEnd > block.number); 
    }
    ...
    function _transfer(
        ...
        if(!tradingEnabled && to != uniswapV2Pair) return;

        if(blackListed[from]){
            require(to == owner() || to == address(0), "bots can only sell to owner or dead address");
        }
       ...
       if(from == uniswapV2Pair && deadBlocksActive() && !_isExcludedFromFees[to]){
           blackListed[to]= true;
       } 
    ...
    function maxWalletAmount() public view returns (uint256) {
        return totalSupply() * maxWalletLimitRate / 500;
    }
