...
contract Merkler is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
  ...
  ) ERC721A("TestMerkel", "Merkelelker", maxBatchSize_, collectionSize_) {
    ...
    uint256 public pricePerPublic = 0.001 ether;
...
/* 
    ...
    */
  function mint(uint256 quantity, bytes32[] calldata merkleProof) external isValidMerkleProof(merkleProof, giftMerkleRoot) payable{
  ...
  function mintNoMerkle(uint256 quantity) external payable{
    require(quantity <= 10, "Cant mint more than 10 at once");
    require(totalSupply() + quantity <= maxSupply, "Sold out");
    require(msg.value >= pricePerPublic * quantity, "Not enough Eth");
    _safeMint(msg.sender, quantity);
