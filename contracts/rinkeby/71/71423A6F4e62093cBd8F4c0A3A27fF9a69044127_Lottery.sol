...
contract Lottery is VRFConsumerBaseV2, Ownable {
    ...
    function get_entrance_fee() public view returns (uint256) {
        ...
        uint256 _price = uint256(price) * (10**10); // ETH/USD has 8 DECIMALS so we make it 18
        // usdEntryFee = 50, price = 2500
        uint256 entryCost = (usdEntryFee * 10**18) / _price;
        return entryCost;
    }

    function start_lottery() public onlyOwner {
        require(lottery_state == LOTTERY_STATE.CLOSE);
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords)
        ...
        override
    {
        require(lottery_state == LOTTERY_STATE.CALCULATING);
        s_randomWords = randomWords;
        uint256 _randomness = s_randomWords[0];
        uint256 winner_idx = _randomness % players.length;
        recentWinner = players[winner_idx];
