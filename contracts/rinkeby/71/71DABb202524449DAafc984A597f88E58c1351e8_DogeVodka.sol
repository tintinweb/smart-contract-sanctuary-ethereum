...
contract DogeVodka is ERC721Enumerable, Ownable {
  ...
  function mint(
    ...
    require(
      numberOfTokens <= maxVodkasPurchase,
      'Can only mint 5 tokens at a time'
    );
    require(
      totalSupply() + numberOfTokens <= maxVodkas,
      'Purchase would exceed max supply of Doge Vodkas'
    );
    require(
      vodkaPrice * numberOfTokens == msg.value,
    ...
    returns (string memory)
  {
    ...
    if (redeemedToken[tokenId] == true)
      return string(abi.encodePacked(redeemedBaseURI, tokenId.toString()));
