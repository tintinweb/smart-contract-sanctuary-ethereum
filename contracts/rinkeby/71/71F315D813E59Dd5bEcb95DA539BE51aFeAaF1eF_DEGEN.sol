...
contract DEGEN is Context, IERC20, Ownable {
    ...
    uint256 private _currentTaxForTreasury = 6; // modified depending on context of tx
    uint256 public _fixedTaxForTreasury = 6; // unchanged save by owner transaction
    uint256 private _currentBurnRate = 3; // current burn rate of token supply
    uint256 public _fixedBurnRate = 3; // unchanged fixed burn rate
    ...
    )
    {
        ...
        uint256 tokensToTransfer = amountOfTokens.sub(tokensToBeBurnt).sub(
            tokensForTreasury
        );
