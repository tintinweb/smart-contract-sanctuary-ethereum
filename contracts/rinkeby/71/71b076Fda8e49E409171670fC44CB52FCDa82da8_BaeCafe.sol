...
contract BaeCafe is ERC721, Ownable {
    ...
    uint256 public constant totalWhitelistMint = 2500;
    uint256 public constant totalGiveaways = 410;
    ...
    Counters.Counter public countGiveaways;
    Counters.Counter public countTeamMint;
    ...
    function teamMint(address to, uint256 amountToMint) public onlyOwner {
        require(
            _tokenIds.current() + amountToMint <= MAX_HANA,
            "Exceed max supply of Baes"
        );
        require(
            countTeamMint.current() + amountToMint < totalTeamMint,
            "Exceed max TeamMint"
        );
        for (uint256 i = 1; i <= amountToMint; i++) {
            countTeamMint.increment();
            _mintNFT(to);
        }
    }

    /**
     * Reward bae to raffle wallet
     */
    function raffleGiveaway(address to) public onlyOwner {
        require(_tokenIds.current() < MAX_HANA, "Exceed max supply of Baes");
        require(
            countGiveaways.current() < totalGiveaways,
            "Exceed max giveaways"
        );
        countGiveaways.increment();
        _mintNFT(to);
    }
    ...
    function _mintNFT(address to) internal {
        ...
        require(newTokenId < MAX_HANA, "Exceed max supply of Baes");
        _tokenIds.increment();
        _mint(to, newTokenId);
    }
