...
import "contracts/base/BridgeEIP712.sol";
import "./NFT.sol";

contract ReceiptBridge is Ownable, BridgeEIP712, IReceipt {
    using SafeERC20 for IERC20;

    uint256 constant FEE_100_PERCENT = 100e9;
    ...
    function pay(
        ...
        PayRequest memory pr = PayRequest({
            amount: _amount,
            token: _token,
        ...
        _useSignature(
            ...
            pr.recipientName,
            pr.receiptId,
            pr.creationTimestamp,
            pr.signature
        );

        (uint256 toReceive, ) = distributeInvoice(
            pr.token,
            pr.amount,
            msg.sender,
            pr.recipient
        ...
        uint256 tokenId = nft.mintReceiptNFT(
            pr.amount,
    ...
    function useSignature(
        ...
        if (msg.sender != address(nft)) revert();
        _useSignature(
            _tokenId,
            _operator,
            _newName,
    ...
    function redeemFee(address _token) external onlyOwner {
        uint256 amount = adminFee[_token];
        if (amount > 0) {
            delete adminFee[_token];
    ...
    function owner()
        public
        view
        override(Ownable, BridgeEIP712)
        returns (address)
    {
        return Ownable.owner();
    }

    function distributeInvoice(
        address _token,
        uint256 _amount,
        address _from,
        address _to
    ) internal returns (uint256 recipient_, uint256 fee_) {
        fee_ = (_amount * feePercent) / FEE_100_PERCENT;
        recipient_ = _amount - fee_;

        IERC20(_token).safeTransferFrom(_from, _to, recipient_);
        IERC20(_token).safeTransferFrom(_from, address(this), fee_);

        adminFee[_token] += fee_;
    }
}
...
abstract contract BridgeEIP712 is EIP712 {
    using ECDSA for bytes32;

    error AlreadyClaimed(bytes32);
    error SignerNotOwner(address);
    error RequestLifetimeHasExpired();

    uint256 public constant requestLifetime = 15 minutes;

    bytes32 public constant RECEIPT_SCHEMA_HASH =
        keccak256(
            "Receipt(uint256 amount,address token,address from,address to,string fromName,string toName,uint256 receiptId,uint256 creationTimestamp)"
        );
    bytes32 public constant UPDATE_NAME_SCHEMA_HASH =
        keccak256(
            "UpdateName(uint256 tokenId,address operator,string newName,uint256 signTimestamp)"
        );

    mapping(bytes32 => bool) public hashes;

    constructor() EIP712("ReceiptBridge", "1.0") {}
    ...
    function _useSignature(
        ...
        checkRequestLifetime(_creationTimestamp);
        bytes32 hash = getReceiptHash(
            _amount,
            _token,
            _from,
            _to,
    ...
    function _useSignature(
        ...
        checkRequestLifetime(_signTimestamp);
        bytes32 hash = getUpdateNameHash(
            _tokenId,
            _operator,
            _newName,
            _signTimestamp
    ...
    function checkRequestLifetime(uint256 _startTimestamp) internal view {
        if (_startTimestamp + requestLifetime < block.timestamp)
            revert RequestLifetimeHasExpired();
    }

    function recoverOwner(bytes32 _hash, bytes memory _signature) internal {
        if (hashes[_hash]) revert AlreadyClaimed(_hash);
        hashes[_hash] = true;

        address signer = _hash.toEthSignedMessageHash().recover(_signature);
        if (signer != owner()) revert SignerNotOwner(signer);
    }
    ...
    function getReceiptHash(
        ...
        bytes32 structHash = _buildReceiptStructHash(
            _amount,
            _token,
            _from,
            _to,
    ...
    function getUpdateNameHash(
        uint256 _tokenId,
        address _operator,
        string memory _newName,
        uint256 _signTimestamp
    ) public view returns (bytes32) {
        bytes32 structHash = _buildUpdateNameStructHash(
            _tokenId,
            _operator,
            _newName,
            _signTimestamp
    ...
    function _buildReceiptStructHash(
        uint256 _amount,
        address _token,
        address _from,
        address _to,
    ...
    function _buildUpdateNameStructHash(
        uint256 _tokenId,
        address _operator,
        string memory _newName,
        uint256 _signTimestamp
...
import "./ReceiptBridge.sol";

contract NFT is ERC721Holder, ERC721A, Ownable {
    using Counters for Counters.Counter;
    using NFTDescriptor for IReceipt.Receipt;

    Counters.Counter private _tokenIdCounter;

    mapping(uint256 => IReceipt.Receipt) public metadata;

    constructor(uint256 _feePercent) ERC721A("NFT", "NFT") {
        address _receiptBridge = address(
            new ReceiptBridge(_feePercent, address(this), msg.sender)
        );
        _transferOwnership(_receiptBridge);
    }
    ...
    {
        super.tokenURI(_id); // do neccesarry checks for token id
        return metadata[_id].constructTokenURI();
    }
    ...
    function mintReceiptNFT(
        ...
        metadata[currentTokenId] = IReceipt.Receipt({
            amount: _amount,
            timestamp: _timestamp,
            token: _token,
            nonce: uint96(currentTokenId),
            payer: _from,
            recipient: _to,
            payerName: _fromName,
            recipientName: _toName
        });
    ...
    function updateNickname(
        ...
        ReceiptBridge(owner()).useSignature(
            _tokenId,
            msg.sender,
            _newName,
            _signTimestamp,
        ...
        if (metadata[_tokenId].payer == msg.sender) {
            metadata[_tokenId].payerName = _newName;
        } else if (metadata[_tokenId].recipient == msg.sender) {
            metadata[_tokenId].recipientName = _newName;
        }
    }
