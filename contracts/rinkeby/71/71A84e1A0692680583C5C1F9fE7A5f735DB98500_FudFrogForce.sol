...
contract FudFrogForce is ERC721AQueryable, Ownable {
  ...
  uint256 public constant TOTAL_FREE_MINTS = 4000;
  uint256 public constant TEAM_AMOUNT = 888;
  uint256 public constant MAX_FREE_MINT_PER_WALLET = 2;
  uint256 public constant MAX_PUBLIC_MINT_PER_WALLET = 10;
  uint256 public constant TOKEN_PRICE = .0069 ether;
  ...
  modifier underMaxSupply(uint256 _quantity) {
    require(
      _totalMinted() + _quantity <= TOTAL_MAX_SUPPLY - TEAM_AMOUNT,
  ...
  function mint(uint256 _quantity) external payable callerIsUser underMaxSupply(_quantity) {
    ...
    if (_totalMinted() < (TOTAL_MAX_SUPPLY - TEAM_AMOUNT)) {
      if (freeMintCount >= TOTAL_FREE_MINTS) {
        require(msg.value >= _quantity * TOKEN_PRICE, 'FudFrogForce: Ribbit.. Need send more ETH!');
        _mint(msg.sender, _quantity);
      } else if (freeMintClaimed[msg.sender] < MAX_FREE_MINT_PER_WALLET) {
        uint256 _mintableFreeQuantity = MAX_FREE_MINT_PER_WALLET - freeMintClaimed[msg.sender];
        if (_quantity <= _mintableFreeQuantity) {
          ...
          require(
            msg.value >= (_quantity - _mintableFreeQuantity) * TOKEN_PRICE,
