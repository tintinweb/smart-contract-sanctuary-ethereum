...
import "./NFTSpaceXAsset.sol";
import "./TokenIdentifiers.sol";
import "../utils/ReentrancyGuard.sol";

contract NFTSpaceXAssetShared is NFTSpaceXAsset, ReentrancyGuard {
  using TokenIdentifiers for uint256;

  NFTSpaceXAssetShared public migrationTarget;
  ...
  ) NFTSpaceXAsset(_name, _symbol, _proxyRegistryAddress, _templateURI) {
    migrationTarget = NFTSpaceXAssetShared(_migrationAddress);
  }
  ...
  function disableMigrate() public onlyOwnerOrProxy {
    migrationTarget = NFTSpaceXAssetShared(address(0));
  }

  function migrate(Ownership[] memory _ownerships) public onlyOwnerOrProxy {
    NFTSpaceXAssetShared _migrationTarget = migrationTarget;
    require(
      _migrationTarget != NFTSpaceXAssetShared(address(0)),
      "NSAS: migrate disabled"
    );
...
contract NFTSpaceXAsset is ERC1155Custom {
