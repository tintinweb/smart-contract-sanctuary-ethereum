...
contract DoxxedCO is ERC721A, ERC721ABurnable, ERC721AQueryable, Ownable {
     ...
     */
    function _mintedKey(address minter, uint24 subId) internal pure returns (uint256 result) {
        assembly {
        ...
        payable 
    {
        unchecked {
            ...
            hash = keccak256(abi.encode(imageId, imageIdLimit, imageIdMaxSupply, imageIdPrice, salt));
            require(
            ...
            uint256 imageIdTotalMinted = _imageIdsMinted[_mintedKey(address(0), imageId)];
            imageIdTotalMinted += quantity;
            require(imageIdTotalMinted <= imageIdMaxSupply, 'Image ID out of stock.');
            _imageIdsMinted[_mintedKey(address(0), imageId)] = imageIdTotalMinted;

            // Checks and updates `_imageIdsMinted[_mintedKey(msg.sender, imageId)]`.
            uint256 imageIdNumberMinted = _imageIdsMinted[_mintedKey(msg.sender, imageId)];
            imageIdNumberMinted += quantity;
            require(imageIdNumberMinted <= imageIdLimit, 'Image ID per address exceeded.');
            _imageIdsMinted[_mintedKey(msg.sender, imageId)] = imageIdNumberMinted;
        }
