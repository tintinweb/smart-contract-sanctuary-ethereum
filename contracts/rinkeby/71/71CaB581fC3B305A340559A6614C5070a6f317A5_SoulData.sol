...
contract SoulData is ISoulData, Adminable {
    ...
    /**
     * @dev See {ISharkzSoulIDData-Base64Encode}.
     */
    function Base64Encode(bytes memory _data) external pure returns (string memory) {
        return string(abi.encodePacked("data:application/json;base64,", Base64.encode(_data)));
    }
    ...
    function tokenAttributes(string calldata _badgeTraits, string calldata _voteScore, string calldata _creationTime) 
        external 
        pure 
        returns (string memory) 
    {
        return string(abi.encodePacked('"attributes":[',
            '{"trait_type":"Creation Time","value":"', _creationTime, '"},',
            _badgeTraits,
            '{"trait_type":"Vote Score","value":"', _voteScore, '"}]}'
        ));
    }
        ...
        returns (string memory) 
    {
        if (bytes(_text).length > 0) {
            ...
            //  font-size='8.5' transform='rotate(4)'>1626561212#471</text>
            return string(abi.encodePacked(
                "<text text-anchor='middle' x='", _x, "' y='", _y ,"' fill='#8ecad8' font-family='custom' font-size='",_fontSize,
                "' transform='rotate(", _rotate, ")'>",convertSvgSpace(_text),"</text>"
            ));
        }
    ...
    function convertSvgSpace(string memory _name) 
        public  
        pure 
        returns (string memory)
    {
...
interface ISoulData {
    /**
     * @dev Convert bytes to data string prefixed with "data:application/json;base64,{base 64 encoded string}"
     */
     ...
     */
    function tokenAttributes(string calldata badgeTraits, string calldata voteScore, string calldata creationTime) external pure returns (string memory);
    ...
    /**
     * Process the svg text space
     */
    function convertSvgSpace(string memory name) external pure returns (string memory);
