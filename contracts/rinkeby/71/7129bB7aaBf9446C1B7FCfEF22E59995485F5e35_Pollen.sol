...
contract Pollen  {
  ...
  //      uint256 rewards;
    ...
    constructor() {
        ...
        DAI   = IERC20(0x5eD8BD53B0c3fa3dEaBd345430B1A3a6A4e8BD7C);
        PollenNFT = IPollenNft(0x00F4b456577beB4234A00e7Bb4C929AF3D1B165a);
        DAIPriceFeed = AggregatorV3Interface(0x2bA49Aaa16E6afD2a993473cfB70Fa8559B523cF);
      //  operator = IERC721()
    }
    
    function stake(uint256 amount, address operator ) public {
         uint256 newItemId;
        DAI.transferFrom(msg.sender, address(this), amount);
        newItemId =  PollenNFT.createToken(msg.sender, operator);
        rewards[newItemId] = StakedToken(newItemId, amount,  block.timestamp);
    }
     ...
     function calculateReward(uint256 tokenIndex, AggregatorV3Interface _tokenPriceFeed) public  view returns (uint256) {
        ...
        uint256 rewardPriceUsd = (staked.amount * getLatestEthPrice(DAIPriceFeed) / 1e8 * rewardFactor * (block.timestamp - staked.startTimestamp)) / 100 / rewardInterval;
    ...
    function getReward(address _owner, uint256 tokenIndex) public  {
       // require(PollenNFT.ownerOf(tokenIndex) == _owner,"You don't own any Pollen NFT." );
        ...
        dux.transfer(msg.sender, reward);
   }
   ...
   function unstake(address _owner, uint256 tokenIndex) public  {
        ...
        StakedToken memory staked = rewards[tokenIndex];
        uint256 _amount = staked.amount;
        staked.startTimestamp = 0;
        staked.amount = 0;
...
function createToken(address sender, address operator) external returns (uint256 id);
