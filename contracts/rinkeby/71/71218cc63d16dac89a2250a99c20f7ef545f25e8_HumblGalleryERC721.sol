...
contract HumblGalleryERC721 is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable,OwnableUpgradeable, ERC721BurnableUpgradeable, PausableUpgradeable,CommissionUpgradeable, ReentrancyGuardUpgradeable {
...
function initialize(uint96 commissionPercentage, address commissionReceiver) initializer public {
    ...
    enum SaleKind { FixedPrice, Auction }
    struct Payload {
        uint256 tokenId;
        uint256 price;
        uint256 salt;
    ...
    function mint(uint256[3] memory intData, string calldata uri, uint8 ERC20TokenIndex,address seller,address buyer, address[] memory royaltyReceiver,uint96[] memory royaltyPercentage,SaleKind salekind,Signature memory signature) public whenNotPaused returns(uint256) {
        require(royaltyPercentage.length == royaltyReceiver.length, "H105");
        Payload memory payload = Payload(intData[0],intData[1],intData[2],uri,ERC20TokenIndex,seller, buyer, royaltyReceiver, royaltyPercentage,salekind,signature);
    ...
    function lazyMint(uint256[3] memory intData, string calldata uri, uint8 ERC20TokenIndex,address seller,address buyer,address[] memory royaltyReceiver,uint96[] memory royaltyPercentage, Signature memory sig,SaleKind salekind,Signature memory signature) public payable whenNotPaused returns(uint256 tokenId)  {
        Payload memory payload = Payload(intData[0],intData[1],intData[2],uri,ERC20TokenIndex,seller,buyer, royaltyReceiver,royaltyPercentage,salekind,signature);
        bytes32 hash = _Hash(payload);
    ...
    /* Mint and Transfer */
    function transferWithMint(uint256[3] memory intData, string calldata uri, uint8 ERC20TokenIndex,address seller,address buyer, address[] memory royaltyReceiver,uint96[] memory royaltyPercentage,SaleKind salekind,Signature memory signature) public whenNotPaused returns(uint256 tokenId)  {
        Payload memory payload = Payload(intData[0],intData[1],intData[2],uri,ERC20TokenIndex,seller,buyer,royaltyReceiver,royaltyPercentage,salekind,signature);
        payload.tokenId = _mint(payload);
    ...
    function safeBuyNFT(Payload memory payload, Signature memory sig, uint256 tokenId) public payable whenNotPaused nonReentrant {
        bytes32 hash = _Hash(payload);
        payload.buyer = _msgSender();
    ...
    function safeTransferNFT(Payload memory sellerPayload,Payload memory buyerPayload, Signature memory sellerSig, Signature memory buyerSig, uint256 tokenId) public whenNotPaused payable {
        ...
        require(sellerSig.v==buyerPayload.signature.v,"H1571");
        require(sellerSig.r==buyerPayload.signature.r,"H1572"); 
        require(sellerSig.s==buyerPayload.signature.s,"H1573"); 
        ...
        _executeTransferNFT(sellerPayload.seller, buyerPayload.buyer,buyerPayload.tokenId,buyerPayload.price,_token);
    }
    /* Transfer Already minted NFT through Auction by admin or seller */
    function safeTransferNFTERC721(IHumbleNFTERC721Upgradeable ERC721humbleNFT, Payload memory sellerPayload, Payload memory buyerPayload, Signature memory sellerSig, Signature memory buyerSig, uint256 tokenId) public payable 
    {     
        bytes32 sellerHash = _Hash(sellerPayload);
        bytes32 buyerHash = _Hash(buyerPayload);     
        require(validateSignature(sellerPayload.seller, sellerHash, sellerSig), "H154");      
        ...
        address _token = getPaymentToken(sellerPayload.ERC20TokenIndex);  
        _executeTransferERC721(ERC721humbleNFT, sellerPayload.seller, buyerPayload.buyer,buyerPayload.tokenId,buyerPayload.price,_token);   
    }
    /* Update token price per unit */
    function updateTokenPrice(uint256 tokenId, uint256 price) public whenNotPaused {   
        require(_msgSender() != address(0), "H159");
        require(ownerOf(tokenId)==_msgSender(), "H160");
        HumblNFT storage nft = HumblNFTs[tokenId];
        nft.price = price;
    ...
    function _mint(Payload memory payload) internal returns(uint256 tokenId) { 
        ...
        _mint(payload.seller, tokenId);
        if (bytes(payload.tokenURI).length > 0){
           _setTokenURI(tokenId, payload.tokenURI);
        }
    ...
    function _safeTransferNFT(Payload memory payload) internal  {
        ...
        _executeTransferNFT(payload.seller, payload.buyer, payload.tokenId, payload.price, ERC20Token);
    }
    function safeBuyNFTERC721(IHumbleNFTERC721Upgradeable ERC721humbleNFT, Payload memory payload, Signature memory sig) public payable whenNotPaused {
        ...
        _executeTransferERC721(ERC721humbleNFT,payload.seller,_msgSender(), payload.tokenId, payload.price,ERC20Token);
        executedHash[hash] = true;
    }
    
    function _executeTransferERC721(IHumbleNFTERC721Upgradeable ERC721humbleNFT, address seller, address buyer, uint256 tokenId,uint256 price, address ERC20Token) internal {
        ...
        NFT memory _data = NFT(tokenId, seller, buyer,
        price, commissionReceiver, commissionAmount, royaltyReceiver, royaltyAmount, ERC20Token);
        _executeFundTransfer(_data);
        ERC721humbleNFT.safeTransferFrom(seller, buyer, tokenId, '');
    }
    function _executeTransferNFT(address seller, address buyer, uint256 tokenId,uint256 price, address ERC20Token) internal {
        require(_exists(tokenId), "H143");
        require(balanceOf(seller) >=1, "H144");
        HumblNFT storage nft = HumblNFTs[tokenId];
        if(ERC20Token == address(0)){
        ...
        NFT memory _data = NFT(tokenId, seller, buyer, 
        price, commissionReceiver, commissionAmount, nft.royaltyReceiver, royaltyAmount, ERC20Token);
        _executeFundTransfer(_data);
    ...
    function _executeFundTransfer(NFT memory _data) internal {
        ...
        uint256 _transferAbleAmount = (_data.price).sub((totalRoyaltyAmount).add(_data.commissionAmount),'H148');
        if(_data.ERC20Token == address(0)){
        ...
        emit safeSaleNFT(_data.seller, _data.buyer , _data.tokenId, _transferAbleAmount, _data.commissionAmount, _data.commissionReceiver);
...
contract ERC721Upgradeable is Initializable, ContextUpgradeable, ERC165Upgradeable, IERC721Upgradeable, IERC721MetadataUpgradeable {
    ...
    /**
     * @dev See {IERC165-supportsInterface}.O
     */
