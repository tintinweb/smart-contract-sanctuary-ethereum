    ...
    SafeSend
{
    ...
    /// @param _nounletTarget Address of the nouns target contract
    constructor(address _registry, address _nounletTarget)
        NounletModule(_nounletTarget)
    {
        registry = _registry;
    }
...
interface INounletAuction {
    ...
    function vaultFractions(address _vault) external view returns (uint256);
...
contract NounletModule is INounletModule {
    ...
    address public nounletTarget;

    /// @notice Initializes target contracts
    constructor(address _nounletTarget) {
        nounletTarget = _nounletTarget;
    }
    ...
    function _mintFraction(
        ...
        IVault(payable(_vault)).execute(nounletTarget, data, _mintProof);
    }
}
...
interface INounletModule is IModule {
    function nounletTarget() external view returns (address);
}
