...
/**
 * @title ERC-721 Non-CRYPTOFRENSble Token Standard, optional metadata extension
...
/**
 * @dev Implementation of https://eips.ethereum.org/EIPS/eip-721[ERC721] Non-CRYPTOFRENSble Token Standard, including
...
/**
 * @title ERC-721 Non-CRYPTOFRENSble Token Standard, optional enumeration extension
 * @dev See https://eips.ethereum.org/EIPS/eip-721
 */
...
contract CryptoFrens is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant CRYPTOFRENS_PUBLIC = 10000;
    uint256 public constant CRYPTOFRENS_MAX = CRYPTOFRENS_PUBLIC;
    ...
    Counters.Counter private _publicCRYPTOFRENS;

    constructor() ERC721("CRYPTOFRENS", "CRYPTOFRENS") {
    ...
    {
        require(
            _publicCRYPTOFRENS.current() < CRYPTOFRENS_PUBLIC,
            "Purchase would exceed CRYPTOFRENS_PUBLIC"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicCRYPTOFRENS.current();

            if (_publicCRYPTOFRENS.current() < CRYPTOFRENS_PUBLIC) {
                _publicCRYPTOFRENS.increment();
                _safeMint(to, tokenId);
            }
        }
    }
