...
import "../CollectableBox.sol";

contract TestCollectableBox is CollectableBox {
    constructor(
        string memory name_,
    ...
    ) CollectableBox(name_, symbol_, sourceChainId_, collectorContractAddress_, lzEndpoint_, transferDeadline_) {}

    function setTransferDeadline(uint256 deadline_) public onlyOwner {
        transferDeadline = deadline_;
    }
}
...
contract CollectableBox is ERC721Custom, Ownable, NonblockingReceiverCollectableBox {
    using ECDSA for bytes32;
    address public nftContractAddress;
    mapping(uint256 => bool) public claimed; // tokenId => isClaimed
    address public royaltyReceiber;
    ...
    // @TODO: Create a royality info function
    ...
    function setRoyaltyInfo(address receiver_, uint16 perTenThousand_) public onlyOwner {
        royaltyReceiber = receiver_;
        royaltyPerTenThousand = perTenThousand_;
    }

    // @TODO: Create a baseURI function
    ...
    // @TODO: mint unclaimed boxes (with timer)
...
abstract contract NonblockingReceiverCollectableBox is
    Ownable,
    ILayerZeroReceiver
{
    ...
    function lzReceive(
        ...
        if (_srcChainId != sourceChainId)
            revert LayerZero__InvalidSourceChain();
        bytes memory trustedRemote = collectorContractAddress;
        if (
