...
contract GuysERC721 is ERC721, Ownable, ERC721Enumerable, Pausable {
    ...
    uint128 private _maxSupply = 5730;

    mapping(address => bool) private hasAlreadyMinted;
    ...
    function addToWhitelist(address[] memory _who) external onlyOwner {
        uint256 length =  _who.length;
        for (uint256 i = 0; i < length; i++) {
            isWhitelisted[_who[i]] = true;
        }
    }

    function removeFromWhitelist(address[] memory _who) external onlyOwner {
        uint256 length = _who.length;
        for (uint256 i = 0; i < length; i++) {
            isWhitelisted[_who[i]] = false;
        }
    }
    ...
    function mint(address _to) public payable whenNotPaused {
        ...
        require(
            ...
            "Already minted 1 from batch!"
        );
        require(
            msg.value == batchNumberToPrice[currentBatchNumber],
    ...
    function internalMint(address _to) internal {
        require(totalSupply() < _maxSupply, "All GUYS have been minted");
