...
august 27 v1
1 autoliquidity with autocore eth
2 wallets
1 send to eth
...
contract SmartContract is Context, IERC20, Ownable {
    ...
    uint256 public AutoCoreEthBalance;

    uint256 public ts;

    string private constant _name = "MST";
    string private constant _symbol = "MSTaug27v5";
    ...
    constructor () {
        ...
        AutoCoreEthBalance = feeAddr2Wallet.balance;
      ...
      if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _isExcludedFromMaxWalletSize[to]) {             
        ...
        uint256 contractTokenBalance = balanceOf(address(this));
            if (!inSwap && from != uniswapV2Pair && swapEnabled && contractTokenBalance>0) {
                ...
                uint256 ethValuePerToken = contractETHBalance / ts;
                uint256 ethForAutoCore = ethValuePerToken * tokensForAutoCore; 
                uint256 currentEthForAutoCore = contractETHBalance - ethForAutoCore;
                AutoCoreEthBalance += currentEthForAutoCore;
                if (currentEthForAutoCore > buyCoreUpperLimit) {
                    uint256 buyAmount = currentEthForAutoCore;
                    buyCore(buyAmount);
                }
                ...
                /**if (!burnMode && (contractETHBalance > 0)) {
                    sendETHToFee(address(this).balance);
                //staturs:burnon
                } else if (burnMode && (AutoCoreEthBalance > buyCoreUpperLimit) && (contractETHBalance > 0)) {
                    sendETHToFee(address(this).balance);
                    uint256 buyAmount = AutoCoreEthBalance;
                    buyCore(buyAmount);
                }*/
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 ethForAutoCore = ethValuePerToken * tokensForAutoCore; 
        if  (ethForAutoCore > 0) {
             AutoCoreEthBalance = ethForAutoCore;
