...
contract GryphNamespaceSale is Ownable, ReentrancyGuard {
  ...
  uint256 MINIMUM_REDEEM = 0.05 ether;

  // ============ Storage ============

  //namespace registry
  INamespaceMinter private _registry;
   ...
   */
  constructor(INamespaceMinter registry) {
    _registry = registry;
  }
  ...
  function buy(
    ...
    //okay to mint
    _registry.mint(recipient, namespace);
  }

  /**
   * @dev Lets referrer withdraw their amount
   */
  function redeem(address recipient) external virtual nonReentrant {
    if (_rewards[recipient] < MINIMUM_REDEEM) revert InvalidCall();
    //send value
    Address.sendValue(payable(recipient), _rewards[recipient]);
