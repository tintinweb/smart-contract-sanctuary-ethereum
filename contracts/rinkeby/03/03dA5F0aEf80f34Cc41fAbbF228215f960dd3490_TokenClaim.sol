...
contract TokenClaim is AccessControlEnumerable {
    ...
    function getHashPacked(address user, uint256 amount, uint256 timestamp) public pure returns (bytes32)
    {
        return keccak256(abi.encodePacked(user, amount, timestamp));
    }
        ...
        returns (bool)
    {
        ...
        bytes32 hashedParams = getHashPacked(user, amount, timestamp);
        require(!processedTransactions[hashedParams], "TokenClaimt: Transaction already processed");
