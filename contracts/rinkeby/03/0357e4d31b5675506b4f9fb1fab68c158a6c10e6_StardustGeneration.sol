...
contract StardustGeneration is ERC721AQueryable, Ownable {
    ...
    bytes32 public merkleRoot; // for starlist and reserve list

    // minting phase limits and wallet limit
    uint16 public constant MAX_STARLIST = 3;
    uint16 public constant MAX_RESERVELIST = 3;
    ...
    // set merkle root for starlist and reservelist
    ...
    // set max mints allowed per minting phase for public and total per wallet
    ...
    function togglePhase(uint256 _mintingPhase) external onlyOwner {
        if ( _mintingPhase == 1 ) {
            starlistOpen = true;  // set to true
        } else if ( _mintingPhase == 2 ) {
            starlistOpen = false; // toggling to false
            reservelistOpen = true; // toggling to true
        } else if ( _mintingPhase == 3 ) {
            reservelistOpen = false; // toggling to false
            publicOpen = true; // toggling to true
        } else {
            publicOpen = false; // toggling to false
        }
    }
    ...
    function mintPublic(uint256 _mintAmount) 
        ...
        payable  
    {
        // using _numberMinted versus balanceOf, balanceOf is # of nfts you have at time of t(x)
        // users can transfer them out and get more
    ...
    function airdrop(address[] calldata _airdropAddresses) external onlyOwner {
        for (uint256 i = 0; i < _airdropAddresses.length; i++) {
            ...
            require(
                _airdropAddresses.length + totalSupply() <= MAX_SUPPLY,
