...
contract CryptoverseXplore is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256[] public shares = [40, 50, 10];
...
//   }
  ...
  function withdraw(address[] memory _payees) public payable onlyOwner {
    require(_payees.length == shares.length, "PaymentSplitter: payees and shares length mismatch");
    require(_payees.length > 0, "PaymentSplitter: no payees");
    uint256 total_money = address(this).balance;
    for (uint256 i = 0; i < _payees.length; i++) {
      (bool success, ) = payable(_payees[i]).call{value: ((total_money*shares[i])/100)/2}("");
      require(success);
    }
  }
