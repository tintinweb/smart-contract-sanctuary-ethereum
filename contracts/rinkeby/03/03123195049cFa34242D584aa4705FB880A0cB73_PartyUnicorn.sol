...
contract PartyUnicorn is ERC721Enumerable, Ownable {
    ...
    IPartyUnicoinBarn public partyUnicoinBarn;
    ILLove public love;
    ...
    function specialMint(uint _amount, bool _stake) internal{
        ...
        for (uint i = 0; i < _amount; i++) {
            ...
            if (!_stake) {
                ...
                _safeMint(address(partyUnicoinBarn), tokenId);
                tokenIds[i] = tokenId;
            }
        }

        if (_stake) {
            partyUnicoinBarn.addTokensToStake(msg.sender, tokenIds);
        }
    }
    ...
    function selectRecipient() internal returns (address) {
        ...
        address thief = partyUnicoinBarn.randomQueenOwner();
    ...
    function setPartyUnicoinBarn(address _island) external onlyOwner {
        partyUnicoinBarn = IPartyUnicoinBarn(_island);
    }
