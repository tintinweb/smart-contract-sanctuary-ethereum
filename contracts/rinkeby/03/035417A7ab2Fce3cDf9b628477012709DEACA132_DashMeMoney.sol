...
contract DashMeMoney {
    ...
    function convertToETH(uint256 usd) private returns (uint256) {
        return usd * getLatestPrice();
    }

    function convertToGwei(uint256 ethers) private pure returns (uint256) {
        return ethers * 1**8;
    }

    function getLatestPrice() private returns (uint256) {
