...
// FireSushiFetch helps your migrate your existing SushiSwap LP tokens to Fireswap LP ones
contract FireSushiFetch {
    ...
    function migrate(
        ...
        require(deadline >= block.timestamp, 'Fireswap: EXPIRED');
        ...
        if (amountA > pooledAmountA) {
            IBEP20(tokenA).safeTransfer(msg.sender, amountA - pooledAmountA);
        }
        if (amountB > pooledAmountB) {
            IBEP20(tokenB).safeTransfer(msg.sender, amountB - pooledAmountB);
        }
    }
    ...
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, 'FireSushiFetch: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'FireSushiFetch: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function addLiquidity(
        ...
        IBEP20(tokenA).safeTransfer(pair, amountA);
        IBEP20(tokenB).safeTransfer(pair, amountB);
