 ...
 */
abstract contract ERC721Tradable is ERC721, ERC721Enumerable, ContextMixin, NativeMetaTransaction, Ownable, AccessControl {
...
contract Fuzzie is ERC721Tradable {
  ...
  address _airdropMintAddress = 0x392D7258c625C1B5C7a0A6E581025b62FDAa6Bbe;

  string baseURI;
  string baseExtension = '.json';
  string unreleasedPhaseUri = 'ipfs://QmXrMibgmV72mHPfFERh4cpjwCZR4JvWE6bJcytApjPvW2';
  string contractUri = '';

  uint256 _maxPhase = 7; // The number of completion
  uint256 _maxSupply = 0;
  uint256 _mintPhase = 0;
  uint256 _mintFundTrxAmount = 0.03 ether;
  uint256 cost = 0.03 ether;
  uint256 nftPerAddressLimit = 20;
  uint256 _createTokenModulus = 20;
  uint256 lastFreeTokenId = 0;
  ...
  function directMint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
      require(_projectPhase.current() == _mintPhase, string(abi.encodePacked('Direct minting only available up to phase ', Strings.toString(_mintPhase))));
      uint256 ownerMintedCount = addressMintedBalance[msg.sender];
      require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, 'Max NFT per address exceeded');
      require(msg.value >= cost * _mintAmount, string(abi.encodePacked('This mint costs ', Strings.toString(cost), ' per token.')));
    }
  ...
  function randomMint() public payable {
    ...
    if (msg.sender != owner()) {
      ...
      require(msg.value >= (cost * 1) / 2, string(abi.encodePacked('This mint costs ', Strings.toString((cost * 1) / 2), ' per token.')));
    }
  ...
  function airdropMint() public payable onlyRole(SERVICE_ROLE) {
    uint256 winningTokenId = random(totalSupply());
    address addressToMintTo = ownerOf(winningTokenId);
    lastFreeTokenId = _nextTokenId.current();
  ...
  function mintToWrapper(address _to) public onlyOwner {
    mintTo(_to);
  }
  ...
  function mintTo(address _to) internal {
    ...
    if (shouldCreateFreeToken(totalSupply())) {
      emit CreateFreeToken(_nextTokenId.current());
    }
  }
  ...
  function pause(bool _state) public onlyRole(DEFAULT_ADMIN_ROLE) {
    paused = _state;
  }
  ...
  function setMaximumMintPhase(uint256 phase) public onlyRole(DEFAULT_ADMIN_ROLE) {
    _mintPhase = phase;
  }
  ...
  function shouldCreateFreeToken(uint256 tokenId) public view returns (bool) {
    return tokenId % _createTokenModulus == 0 && lastFreeTokenId < tokenId;
  }
...
contract FuzzieFactory is FactoryERC721, Ownable {
    ...
    function mint(uint256 _optionId, address _toAddress) override public {
        ...
        if (_optionId == SINGLE_FUZZIE_OPTION) {
            metaFuzzie.mintToWrapper(_toAddress);
    ...
    function tokenURI(uint256 _optionId) override external view returns (string memory) {
        Fuzzie metaFuzzie = Fuzzie(nftAddress);
        return string(abi.encodePacked(metaFuzzie.baseTokenURI(), Strings.toString(_optionId)));
    }
 ...
 * @title FuzzieLootBox
 *
 * FuzzieLootBox - a tradeable loot box of Fuzzies.
 */
contract FuzzieLootBox is ERC721Tradable {
    uint256 NUM_CREATURES_PER_BOX = 3;
    uint256 OPTION_ID = 0;
    address factoryAddress;

    constructor(address _proxyRegistryAddress, address _factoryAddress)
        ERC721Tradable("FuzzieLootBox", "LOOTBOX", _proxyRegistryAddress)
    {
