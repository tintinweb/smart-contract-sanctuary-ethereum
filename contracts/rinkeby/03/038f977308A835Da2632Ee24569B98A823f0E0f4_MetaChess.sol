...
// METACHESS - A CHESS MOVE GENERATOR COLLECTION FOR HUNDREDELEVEN COLLECTION
...
contract MetaChess {
    ...
    function generateAllImages() public view returns (string[] memory) {
        string[] memory images = new string[](111);
    ...
    function getMetadataJSON(uint256 tokenId) public view returns (string memory) {
        ...
        string memory json = string(abi.encodePacked('{"name": "#', toString(tokenId), '", "description": "HUNDREDELEVEN is a cryptoart metacollection with interchangable metadata. Holders of a NFT from this series can change the active metadata on-chain. Visit https://hundredeleven.art.\n\nCurrent collection shown in this token is MetaChess - randomly on-chain generated 111 chess moves.", "external_url": "https://hundredeleven.art/", "artist": "berk aka princesscamel aka guerrilla pimp minion bastard", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(image)), ',"attributes":[{"trait_type": "Collection", "value": "MetaChess"' , '}'));
        return json;
    }
    ...
    function contractURI() public view returns (string memory) {
        return string(abi.encodePacked('{"name": "MetaChess", "description": "on-chain generated random 111 chess moves.", "external_link": "https://hundredeleven.art/","image": "data:image/svg+xml;base64,', Base64.encode(bytes(generateImage(69))),'}'));
    }
