...
contract ERC721ABreeding is ERC721A, Ownable {
    ...
    function breedableOf(address _address, uint8 _type) public view returns (uint256) {
        ParentContract parentContract = _type == 1 ? dadContract : momContract;
        bool hasBred = false;
        uint256 available = 0;
        for (uint256 i = 0; i < 10069; i++) {
            hasBred = _type == 1 ? isDad[i] : isMom[i];
            if (parentContract.ownerOf(i) == _address && !hasBred) {
                available++;
            }
        }
        return available;
