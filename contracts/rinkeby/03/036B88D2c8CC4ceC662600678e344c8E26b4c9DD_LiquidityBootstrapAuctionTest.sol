...
contract LiquidityBootstrapAuction is ReentrancyGuard, Ownable {
    ...
    mapping(address => bool) public usdcWithdrawnOnLastPhase;
    mapping(address => uint256) public rewardClaimed;
    mapping(address => uint256) public lpClaimed;

    event ASTODeposited(address indexed recipient, uint256 amount);
    event USDCDeposited(address indexed recipient, uint256 amount);
    ...
    event LiquidityAdded(address router, uint256 usdcAmount, uint256 astoAmount, uint256 lpTokenAmount);
    ...
    function depositASTO(uint256 amount) external nonReentrant {
        ...
        emit ASTODeposited(msg.sender, amount);
    ...
    function withdrawableUSDCAmount(address recipient) public view returns (uint256) {
        ...
        } else if (currentTime() >= auctionStartTime + 6 days) {
            uint256 elapsedTime = currentTime() - (auctionStartTime + 6 days);
            uint256 maxAmount = depositedUSDC[recipient] / 2;
            return (maxAmount * (SECONDS_PER_DAY - elapsedTime)) / SECONDS_PER_DAY;
        }
    ...
    function withdrawUSDC(uint256 amount) external nonReentrant {
        ...
        require(!usdcWithdrawnOnLastPhase[msg.sender], "already withdrawn");
        require(amount <= withdrawableUSDCAmount(msg.sender), "amount exceeded allowance");

        if (currentTime() >= usdcDepositEndTime()) {
            usdcWithdrawnOnLastPhase[msg.sender] = true;
        }
    ...
    function addLiquidityToExchange(address router, address factory) external nonReentrant onlyOwner {
        ...
        pair = IUniswapV2Factory(factory).getPair(address(asto), address(usdc));

        emit LiquidityAdded(router, usdcSent, astoSent, lpTokenAmount);
    }

    function claimLPToken() external nonReentrant {
        require(currentTime() >= auctionEndTime() + 12 weeks, "claim not enabled");

        uint256 claimable = claimableLPAmount(msg.sender);
        require(claimable > 0, "all claimed");
        lpClaimed[msg.sender] += claimable;

        require(IUniswapV2ERC20(pair).transfer(msg.sender, claimable), "insufficient ASTO balance");
    }

    function claimableLPAmount(address recipient) public view returns (uint256) {
        uint256 total = ((depositedUSDC[recipient] / totalDepositedUSDC) +
            (depositedASTO[recipient] / totalDepositedASTO)) * lpTokenAmount;
    ...
    function claimRewards(uint256 amount) external nonReentrant {
        ...
        (weeksVested, amountVested) = claimableRewards(msg.sender);

        require(amount <= amountVested, "amount not claimable");
        rewardClaimed[msg.sender] += amount;
        require(asto.transfer(msg.sender, amount), "insufficient ASTO balance");
    ...
    {
        return (auctionStartTime, astoDepositEndTime(), usdcDepositEndTime(), auctionEndTime());
    }

    function depositedStats(address depositor)
        public
        view
        returns (
            uint256,
    ...
    {
        return (totalDepositedASTO, totalDepositedUSDC, depositedASTO[depositor], depositedUSDC[depositor]);
    }
