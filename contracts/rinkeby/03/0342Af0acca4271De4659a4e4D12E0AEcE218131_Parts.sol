...
contract Parts is ERC1155, ERC1155CollectionBase, AdminControl{
    ...
    */
    ...
    //     uint remainder = tokenId % 4;
    //     if(remainder == 0) {
    //         return _calculateUri();
    //     } else {
    //         uint tokenIdToCopy = tokenId;
    //         while(tokenIdToCopy % 4 != 0) {
    //             tokenIdToCopy -= 1;
    //         }
    //         return uri(tokenIdToCopy);
    //     }
    // }

    // function _calculateUri() view internal returns (string memory) {
    //     uint _seed = 1;
    //     uint randomNumber = rand(_seed);

    //     while(!_isRandomNumberValid(randomNumber)) {
    //         _seed++;
    //         randomNumber = rand(_seed);
    //     }
...
abstract contract ERC1155CollectionBase is CollectionBase, IERC1155Collection {
     ...
     */
    function purchase() external virtual override {
