...
contract HauntedSpaces is ERC721, ERC721Enumerable, Ownable, ERC721Burnable {
    // ======== 1. `using` =========
    ...
    uint256 public whiteListMaxSupply = 12;
    uint256 public privateMaxSupply = 8;
    uint256 public publicMaxSupply = 12;

    //settings for saletype
    ...
    bytes32 public whiteListRoot =
        0x861a2b32074f84286119ec8f341bbe0a88912c888c3aadf7541fcb7c46df0e1f;
        ...
        onlyOwner
    {
        ...
        for (uint256 i; i < users.length; i++) {
            for (uint256 j; j < numNFT; j++) {
                ...
                addressMintedBalance[users[i]]++;
            }
        }
    }

    // PUBLIC MINT - Accepts 1 parameter, number of token to mint
    ...
    function publicMint(uint256 _mintAmount) public payable {
        ...
        require(
            supply + _mintAmount <= publicMaxSupply,
        ...
        require(
            owned + _mintAmount <= nftPerAddressLimit,
            "You can mint only 3 ntf per address"
        );
    ...
    // WHITELIST MINT - Accepts two parameters, number of tokens to mint and merkle-proof
        ...
        payable
    {
        ...
        require(
            supply + _mintAmount <= whiteListMaxSupply,
            "max NFT limit exceeded"
        );
    ...
    // PRIVATE SALE - Accepts 3 parameters, mintamount, numberd of token bought in Private, merkle-proof

    function privateMint(
        uint256 _mintAmount, //3
        uint256 _addressReservedTokens, //3
        bytes32[] calldata proof // proof
    ) public payable {
