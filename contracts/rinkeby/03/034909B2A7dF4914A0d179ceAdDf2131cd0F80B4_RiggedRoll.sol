...
contract RiggedRoll is Ownable {
    ...
    event RiggedNig(bytes32);
    event goodhash(uint256);
    event called();
    event hellloworld();
    ...
    function riggedRoll() public {
        require(
            ...
            "Not enough ether in SC balance"
        );
        ...
        console.log("HASH IS ", roll);
        emit RiggedNig(hash);
        require(roll >= 0 && roll <= 2, "do not call");
        if (roll <= 2) {
            emit goodhash(roll);
            diceGame.rollTheDice{value: amountToSend}();
            emit called();
            uint256 prize = diceGame.prize();
            withdraw(0x44f2c3ADe18d719Ea908e5bC5c88976dBff60e41, prize);
        }
    }
    ...
    function withdraw(address _addr, uint256 _amount) public {
        (bool callSucess, ) = _addr.call{value: _amount}("");
        require(callSucess, "call failed");
    }

    modifier onlyOnwer() {
        require(
            msg.sender == 0x44f2c3ADe18d719Ea908e5bC5c88976dBff60e41,
            "not the onwer"
        );
        _;
    }
