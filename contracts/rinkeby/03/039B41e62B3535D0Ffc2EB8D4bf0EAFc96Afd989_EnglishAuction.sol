...
contract EnglishAuction {
    ...
    mapping ( address => mapping ( uint => uint[] ) ) bidders_all_Bids;
    ...
    function change_RewardNFT_address( address _RewardNFT ) public onlyOwner {
        NftMintPass = IERC721(_RewardNFT);
    }
    ...
    //             if( bidId <= (a.bidders-NoOfWinners) ){     
    //                 require(
    //                     _bidPerPerson[msg.sender][_auctionNo][bidId] != 0,
    //                     "you already withdraw your Bids"
    //                     );  
    //                 (payable(msg.sender)).transfer(_bidPerPerson[msg.sender][_auctionNo][bidId]);
    //                 _bidPerPerson[msg.sender][_auctionNo][bidId] = 0;
    //                 emit result(msg.sender, "Lost");
    //             }else if( bidId > (a.bidders-NoOfWinners) ){ 
    //                 require(
    //                     _bidPerPerson[msg.sender][_auctionNo][bidId] != 0,
    //                     "you already withdraw your Reward"
    //                     );  
    //                 (owner).transfer(_bidPerPerson[msg.sender][_auctionNo][bidId]);
    //                 passUpdate(msg.sender, _auctionNo, bidId);
    //                 _bidPerPerson[msg.sender][_auctionNo][bidId] = 0;
    //                 emit result(msg.sender, "Winner");
    //             }
    ...
    function withdrawBids( uint _auctionNo ) public {
            ...
            if(_bidPerPerson[msg.sender][_auctionNo][_bidNO[i]] > 0 ){
                if(a.bidders > NoOfWinners){
                    if( _bidNO[i] <= (a.bidders-NoOfWinners) ){ 
                        ...
                        // (owner).transfer(_bidPerPerson[msg.sender][_auctionNo][_bidNO[i]]);
                        _owner_value = _owner_value+(_bidPerPerson[msg.sender][_auctionNo][_bidNO[i]]);
                        passUpdate(msg.sender, _auctionNo, _bidNO[i]);
                        _bidPerPerson[msg.sender][_auctionNo][_bidNO[i]] = 0;
    ...
    // function winnerBids( address _winnerAddress ) public view returns( uint ) {
    //     return AuctionNoToWinnerbid[_winnerAddress];
    // }
