...
contract Minter {
    ...
    function deploySubAddrs(uint256 _num) public onlyOwner {
    ...
    function batchCall(uint256 times, address _addr, bytes calldata _data) public payable onlyOwner {
        for(uint256 i = 0; i < times; i++){
            subContract(subAddressList[msg.sender][i]).callData(_addr, _data);
        }
    }

    function withdrawNFTs(address _token, address _to, uint256[] memory _tokenIds) public onlyOwner {
        for (uint256 i = 0; i < subAddressList[msg.sender].length; i++) {
            subContract(subAddressList[msg.sender][i]).transferAlltoV2(_token, _to, _tokenIds);
        }
    }

    function withdrawNFTsV2(address _token, address _to, address[] memory _subaddrs, uint256[][] memory _tokenids) public onlyOwner {
        for (uint256 i = 0; i < _subaddrs.length; i++) {
            subContract(_subaddrs[i]).transferAlltoV2(_token, _to, _tokenids[i]);
        }
    }
