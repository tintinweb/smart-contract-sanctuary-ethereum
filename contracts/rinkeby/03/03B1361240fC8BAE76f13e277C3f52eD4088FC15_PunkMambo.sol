...
import {ERC721A, Pausable, AccessControl, Counters, EIP712, ECDSA, ValiantTokenInterface, console} from './PMD.sol';


contract PunkMambo is ERC721A, Pausable, AccessControl, EIP712 {
    ...
    mapping(address => bytes[]) private _redeemedSignatures;
    ...
    function getRedeemedSignatures(address account) external view returns (bytes[] memory){
        return _redeemedSignatures[account];
    }
    ...
    function redeem(
        ...
        require(_redeemedSignatures[msg.sender].length + amount <= balance, "EXCEED_QUOTA");
        ...
        for (uint256 i = startIndex; i < startIndex + amount; i++) {
            ...
            _redeemedSignatures[msg.sender].push(signature);
