...
contract POMEL is ERC721A, Ownable, ReentrancyGuard {
  ...
  function selectShareHolders() public {
  ...
  function getShareHolderRevenue() public view returns(uint256 revenue) {
    require(isShareHolder[msg.sender] == true, "This address is not a share holder");
    for(uint256 i = 0; i < ShareHolderCount; i++) {
      if(shareHolders[i].walletAddress == msg.sender) return shareHolders[i].revenue;
    }
}
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    if (whitelistMintEnabled == false) {
      ...
      (bool hs, ) = payable(shareHolders[i].walletAddress).call{value: cost * _mintAmount / 100 * 6 / 20 }("");
    require(hs);
    }
    }
