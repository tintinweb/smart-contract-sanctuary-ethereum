...
contract Giza is Ownable, ERC721iEnumerable {
    ...
    function _beforeTokenTransfer(
        ...
        if (to == address(sudoPool) || from == address(sudoPool)) {
            // todo fix this cause 5% would take from total msg.value which includes sudo fee so would fail as woulnd't leave enough eth
            // for now doing taking 3% altho fee is 5%
            payable(owner()).safeTransferETH(
                ((msg.value * 10000000 * 3)) / 1000000000
            );
        }
