...
contract Bemuer is ERC721A, Ownable {
  ...
  uint256 public maxOglistMint = 2;
  ...
  mapping(address => uint256) public oglistMintedBalance;
  ...
  function mintedBalanceController(uint256 _mintAmount) internal {
    if(status == 0){
        require(oglistMintedBalance[msg.sender] + _mintAmount <= maxOglistMint, "max NFT limit exceeded for this oglisted user");
        oglistMintedBalance[msg.sender] += _mintAmount;
    } else if (status == 1){
        require(whitelistMintedBalance[msg.sender] + _mintAmount <= maxWhitelistMint, "max NFT limit exceeded for this whitelisted user");
        whitelistMintedBalance[msg.sender] += _mintAmount;
    } else if (status == 2){
        require(freeMintedBalance[msg.sender] + _mintAmount <= maxFreeMint, "max NFT limit exceeded for this freelisted user");
        freeMintedBalance[msg.sender] += _mintAmount;
    }
  }
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable callerIsUser {
    ...
    require(MerkleProof.verify(_merkleProof, whitelistRoot, leaf), "user is not on the list");
    
    // Amount and payments control
    uint256 supply = totalSupply();
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
    require(supply + _mintAmount <= whitelistSupply, "max NFT limit exceeded [whitelist]");
    require(msg.value >= cost * _mintAmount, "insufficient funds");
    //

    mintedBalanceController(_mintAmount);
  ...
  // Whitelist Config
  function setWhitelistConfig(bytes32 _whitelistRoot, uint256 _whitelistSupply, uint256 _newStatus) public onlyOwner{
  ...
  function setMaxOgListMint(uint256 _newMaxOgListMint) public onlyOwner {
    maxOglistMint = _newMaxOgListMint;
  }
