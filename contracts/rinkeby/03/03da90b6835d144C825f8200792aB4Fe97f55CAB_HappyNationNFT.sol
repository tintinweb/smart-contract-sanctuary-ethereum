...
contract HappyNationNFT is ERC721A, Ownable {
    ...
    // states
    enum SaleState { NO_SALE, PRE_SALE, PUBLIC_SALE, SOLD_OUT }
    SaleState internal state = SaleState.NO_SALE;
 
    // general settings
    uint256 public constant MAX_SUPPLY = 10000;
    string public constant PROVENANCE_HASH = "473614501620613bb20a19b8f989b8b6c9d80a1904c3d1fb6c7131a118d4f11f";
    ...
    uint256 public maxBlockMints = 3;
    uint256 public blockMintTokenPrice = 0.06 ether;
    mapping(address => bool) private blockMinters;
    ...
    function blockMint(uint256 number, bytes32[] calldata _merkleProof) public payable {
        require(isPreSale(), "MINTING_DISABLED");
        require(isBlockMinter(_merkleProof), "NOT_A_WHITE_MINTER");
        require(canBlockMint(msg.sender), "DOUBLE_WHITE_MINTINING");
        require(hasExceededTotalSupply(number), "NO_TOKENS");
        require(isValidBlockMintRequest(number), "EXCEED_NUMBER_OF_MINTS");
        require(hasRequestedBlockMintValue(number), "WRONG_CHARGE");

        _mint(number);
        blockMinters[msg.sender] = true;
    }
    ...
    function isBlockMinter(bytes32[] calldata _merkleProof) internal view returns (bool) {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        return MerkleProof.verify(_merkleProof, merkleRoot, leaf);
    }

    function canBlockMint(address candidate) internal view returns (bool) {
        return !blockMinters[candidate];
    }

    function isValidBlockMintRequest(uint256 number) internal view returns (bool) {
        return number <= maxBlockMints;
    }

    function hasRequestedBlockMintValue(uint256 number) internal view returns (bool) {
        return blockMintTokenPrice * number <= msg.value;
    }
