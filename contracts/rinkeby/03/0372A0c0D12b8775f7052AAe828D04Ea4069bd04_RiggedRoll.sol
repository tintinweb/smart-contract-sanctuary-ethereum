...
error RiggedRoll__NotOwner();
error RiggedRoll__NotEnoughEth();
error RiggedRoll__NotWinning();
error RiggedRoll__WithdrawEthFailed();
...
contract RiggedRoll is Ownable {
    ...
    function withdraw(address _addr, uint256 _amount) public {
        if (msg.sender != owner()) revert RiggedRoll__NotOwner();
        (bool success, ) = payable(_addr).call{value: _amount}("");
        if (!success) revert RiggedRoll__WithdrawEthFailed();
    }
    ...
    function riggedRoll() public {
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(currentHash, address(diceGame), diceGame.nonce())
        );
        uint256 roll = uint256(hash) % 16;
        if (roll > 2) revert RiggedRoll__NotWinning();
