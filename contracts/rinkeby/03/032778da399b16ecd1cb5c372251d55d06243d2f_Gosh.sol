...
contract Gosh is ERC721A, Ownable {
    ...
    constructor(string memory baseURI) ERC721A("Gosh", "Gosh") {
    ...
    function PPPrice(uint256 p) external onlyOwner {
        PRICE_WL = p;
    }

    function GGPrice(uint256 p) external onlyOwner {
        PRICE = p;
    }

    function whoWantsFreeList(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            _allowList[addresses[i]] = 1;
        }
    }

    function gMi() external onlyOwner {
    ...
    function ItsFreeLFG(address receiver, uint256 quantity) external onlyOwner {
        _safeMint(receiver, quantity);
    }

    function notFreeHuh(uint256 quantity) external payable mintCompliance(quantity) {
        _safeMint(_msgSender(), quantity);
    }

    function yesItIsFree(uint256 quantity)
        external
        payable
    ...
    function moneyCome() external onlyOwner {
