...
contract IpsumOfLorem is Ownable {
    ...
    mapping(address => bool) public IpsumTokenContractAllowlist;
    ...
    modifier onlyAllowedTokenContract(address tokenContract) {
        require(IpsumTokenContractAllowlist[tokenContract],"tokenContract is not on the allowlist");
        _;
    }
    ...
    function addIpsum(address tokenContract, uint256 tokenId, bool nsfw, string memory loreMetadataURI) 
    public onlyAllowedTokenContract(tokenContract) {
        ...
        tokenIpsum[tokenContract][tokenId].push(
            Ipsum(_msgSender(), true, nsfw, loreMetadataURI)
            );
        emit IpsumAdded(
            tokenContract,
            tokenId,
    ...
    function addIpsumWithSignature(bytes memory signature, address tokenContract, uint256 tokenId, uint256 ipsumIdx,  bool nsfw, string memory loreMetadataURI
    ) public onlyAllowedTokenContract(tokenContract) {
    ...
    function setIpsumTokenAllowlist(address tokenContract, bool isListed) public onlyOwner{
        IpsumTokenContractAllowlist[tokenContract] = isListed;
    }
