...
// File: contracts/final_project2_done.sol
...
contract Lottery {
    ...
    uint256 public totalPlayerNum; //功能4. 紀錄歷史總投注人次
    uint256 public lotteryNum; //功能2. 紀錄歷史開彩總次數
    uint256 public totalEntranceFee; //功能3. 紀錄歷史總投注金額
    address public recentWinner;
    address[] private winners; //功能5. 某屆贏家的Address
    uint256[] private winnerMoney; //功能5. 某屆贏家的獲得金額
    uint256[] private playerNumSeq; //功能5. 該屆投注人數
    uint256 public charityMoney; //功能1. 10%的彩金累積後當成慈善基金
    bool public charityWithdraw; //功能6. 慈善基金的提領紀錄
    ...
    function enter() //address add, uint256 money
        ...
        totalPlayerNum++; //功能4.
        totalEntranceFee=totalPlayerNum*entranceFee; //功能3.
    }
    ...
    function endLottery() public onlyOwner{
        ...
        winners.push(recentWinner); //功能5.
        winnerMoney.push(entranceFee*playerNum); //功能5.
        playerNumSeq.push(playerNum); //功能5.
        delete players;
        status=LOTTERY_STATE.CLOSED;  
        playerNum=0;     
        lotteryNum++; //功能3.  
    }
    function winner(uint256 seq) 
        public view
        returns(address addr, uint money, uint num) //功能5. 某屆贏家的Address & 獲得金額 & 該屆投注人數
        {
        addr=winners[seq];
        money=winnerMoney[seq];
        num=playerNumSeq[seq];
    }
    function charity (address addr) public onlyOwner{ //功能1. 10%的彩金累積後當成慈善基金，由owner設定給某慈善單位的Address進行一次性提領
        require(lotteryNum>0, "Lottery Number is zero.");
        charityMoney=totalEntranceFee/10;
        payable(addr).transfer(address(this).balance);//
        charityWithdraw=true; //功能6.
    }
}