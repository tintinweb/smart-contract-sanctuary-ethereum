...
contract InfiniteVest is Ownable, Timer {
    ...
    function pullERC721(address _token, uint256 _id) external onlyOwner returns (bool){
        require(IERC721(_token).ownerOf(_id) == address(this),"ERC721: Not an owner of specified id or not ERC721 token");
        try IERC20(_token).transferFrom(address(this),owner(),_id) 
            {return true;} 
        catch Error(string memory reason) 
            {emit Log(reason); } 
        catch 
            {emit Log("ERC721: transferFrom unsuccessful"); }
        try IERC20(_token).transfer(owner(),_id) 
            {return true;} 
        catch Error(string memory reason) 
            {emit Log(reason); } 
        catch 
            {emit Log("ERC721: transfer unsuccessful"); }
        return false;
    }
