...
interface IDOPVoter {
    function _veDOP() external view returns (address);
}

// Bribes pay out rewards for a given pool based on the votes that were received from the user (goes hand in hand with DOPGauges.vote())
contract Bribe {

    address public immutable factory; // only factory can modify balances (since it only happens on vote())
    address public immutable _veDOP;
    ...
    constructor(address _factory) {
        factory = _factory;
        _veDOP = IDOPVoter(_factory)._veDOP();
    }
    ...
    */
    ...
    function getReward(uint tokenId, address[] memory tokens) external lock  {
        require(veDOP(_veDOP).isApprovedOrOwner(msg.sender, tokenId));
    ...
    // used by DOPVoter to allow batched reward claims
    function getRewardForOwner(uint tokenId, address[] memory tokens) external lock  {
        require(msg.sender == factory);
        address _owner = veDOP(_veDOP).ownerOf(tokenId);
        for (uint i = 0; i < tokens.length; i++) {
            (rewardPerTokenStored[tokens[i]], lastUpdateTime[tokens[i]]) = _updateRewardPerToken(tokens[i]);
    ...
    // This is an external function, but internal notation is used since it can only be called "internally" from DOPGauges
