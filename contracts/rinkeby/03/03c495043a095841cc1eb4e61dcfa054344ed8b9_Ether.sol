...
contract Ether is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A(".ether Name Service", ".ether") {
        tokenIDandAddress[_currentIndex]="ether";
        tokenAddressandID["ether"]=_currentIndex;
        resolveAddress["ether"]=msg.sender;
    ...
    function setAddress(string calldata eth_name, address newresolve) external {
         TokenOwnership memory Ownership = _ownershipOf(tokenAddressandID[eth_name]);
        if (Ownership.addr != msg.sender) revert("Error");
        resolveAddress[eth_name]=newresolve;

    bytes memory result = bytes(primaryAddress[msg.sender]);
        if (keccak256(result) != keccak256(bytes(eth_name))) {
            primaryAddress[msg.sender]="";
        }
    }

    function setPrimaryAddress(string calldata eth_name) external {
         TokenOwnership memory Ownership = _ownershipOf(tokenAddressandID[eth_name]);
        if (Ownership.addr != msg.sender) revert("Error");
        require(resolveAddress[eth_name]==msg.sender, "Error");
        primaryAddress[msg.sender]=eth_name;
    }


    function setDataAddress(string calldata eth_name,string calldata setArea, string  memory newDatas) external {
         TokenOwnership memory Ownership = _ownershipOf(tokenAddressandID[eth_name]);

        if (Ownership.addr != msg.sender) revert("Error");
        dataAddress[eth_name][setArea]=newDatas;
    ...
    function setRefSettings(uint ref_,uint ref_owner_,uint ref_discount_) external onlyOwner {
        ref = ref_;
    ...
    function nameBurnAndChange(uint256 tokenId , string calldata new_ether_name) external onlyOwner {
    ...
    function getAddress(string memory ether_name) public view returns(address) {
        return resolveAddress[string.concat(ether_name,domain)];
    }
    


function walletOfOwnerID(address _owner)
    public
    view
...
function walletOfOwnerMap(address _owner)
    public
    view
    returns (string[] memory)
  {
    ...
    uint256 ownerTokenCount = balanceOf(_owner);
    string[] memory ownedTokenName = new string[](ownerTokenCount);
    uint256 currentTokenId = 1;
    uint256 ownedTokenIndex = 0;
    ...
    while (ownedTokenIndex < ownerTokenCount) {
      address currentTokenOwner = ownerOf(currentTokenId);
      if (currentTokenOwner == _owner) {
        ownedTokenName[ownedTokenIndex] = string.concat(currentTokenId.toString(),'?',tokenIDandAddress[currentTokenId],domain);
        ownedTokenIndex++;
      }
