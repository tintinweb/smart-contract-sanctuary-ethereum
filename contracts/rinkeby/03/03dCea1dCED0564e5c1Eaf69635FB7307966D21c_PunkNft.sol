...
contract ADNBase {
    ...
    //TODO: change to use a Chainlink oracle
    function getDNA(uint256 _tokenId, address _minter, uint256 _block) public pure returns (uint256) {
        uint256 params = _tokenId + uint160(_minter) + _block;
        bytes memory encodedParams = abi.encodePacked(params);
        bytes32 hashed = keccak256(encodedParams);
...
contract PunkNft is ERC721, ERC721Enumerable,ADNBase {
    ...
    uint256 public max; //maximo nft
    uint256 public price = 0.05 ether;

     constructor(uint256 _max) ERC721("Punkshot", "PKN") {
         max = _max;
     ...
     function mint()public{
         ...
         require(current <= max, "Excede supply Nft");
         //uint256 nftPrice = price * amount;
         tokenDNA[current] = getDNA(current, msg.sender, block.timestamp);
     ...
     function tokenURI(uint256 tokenId)public view override returns(string memory){
         ...
         require(_exists(tokenId), "ERC721 Metadat: URI query for no exits token");
         uint256 dna= tokenDNA[tokenId];
         string memory img = imageByDNA(dna);

         string memory jsonURI = Base64.encode(
             abi.encodePacked(
             '{"name":"Punkshot #"',
             tokenId.toString(),
             '","descrripcion":"Estos son nft punkshot uniques", "image":"',
             img,
             '"}'
             
             ));
         return string (abi.encodePacked("data:applications/json;base64,",jsonURI));
