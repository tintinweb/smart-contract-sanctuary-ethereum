...
contract NftStaker {
  ...
  constructor() {
    nftContract = IERC721(0xED330F9C1AeaBB992276e099440B3b964c13F6eA);
  }

  function stake(uint256 _tokenId) external {
    nftContract.safeTransferFrom(msg.sender, address(this), _tokenId);

    stakes[msg.sender].push(Stake(msg.sender, _tokenId, block.timestamp));
  }

  function unstake(uint256 _tokenId) external {
    uint senderStakes = stakes[msg.sender].length;

    for(uint i = 0; i < senderStakes; i++) {
      if (stakes[msg.sender][i].tokenId == _tokenId && stakes[msg.sender][i].timestamp != 0) {
        ...
        require(elapsedMins >= 20, "You can not unstake before 20 minutes.");
  ...
  function getStakes(address _address) public view returns (Stake[] memory) {
    uint addressStakes = stakes[_address].length;
    Stake[] memory res = new Stake[] (addressStakes);
    
    for(uint i = 0; i < addressStakes; i++) {
