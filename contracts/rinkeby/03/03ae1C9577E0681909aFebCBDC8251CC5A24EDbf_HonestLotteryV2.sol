...
contract HonestLotteryV2 is Ownable,VRFConsumerBaseV2{
  ...
  // Lottery type.
  uint256 public lotteryType = 2;
  // Lottery name
  string public name;
  // Lottery description
  string public description;
  //Required minimal number of players to end the lottery
  uint256 public minPlayers;
  // Prize pool.
  uint256 public prizePoolUsd;
  // Prize pool in eth.
  uint256 public prizePoolEth;
  ...
  constructor(
    ...
    bytes32 _keyhash,
    uint256 _minPlayers,//Minimum number of players to end lottery
    uint256 _prizePoolUsd,// Prize pool in USD
  ...
  function endLottery() external onlyOwner {
    require(getAmountOfPlayers() >= minPlayers);
    require(address(this).balance > getWinPoolEth());
  ...
  function fulfillRandomWords(
    ...
    // Prize pool transferred to the winner
    // The rest goes to contract and website maintnance.
    recentWinner.transfer(getWinPoolEth());
 ...
 function getWinPoolEth() public view returns(uint256){
   (,int256 price,,,)= ethUsdPriceFeed.latestRoundData();
   uint256 adjustedPrice = uint256(price)*1e10; //18 decimals
   return ((prizePoolUsd*1e18)/adjustedPrice);//amount in wei
 }
