    ...
    ReentrancyGuard
{
    ...
    address private immutable narratorAddress;
    ...
    modifier isCorrectPayment(uint256 numberOfArtifacts) {
        if (SALE_PRICE * numberOfArtifacts != msg.value)
            revert IncorrectPaymentReceived();
        _;
    }

    modifier isSignedByNarrator(SignatureData calldata signatureData) {
        if (
            // TODO: is this sketchy?
            // We use the current block number as a nonce for the signature,
            // so the signature can't be indefinitely reused.
            // We use the current block number to avoid using additional
            // storage on the contract to store the nonce.
            // To minimize race conditions that would cause mint to fail,
            // we treat the 5 last block numbers as valid nonces, which
            // means that the nonce will be valid for ~75s, given average
            // block verification time.
            signatureData.blockNumber > block.number ||
            block.number - signatureData.blockNumber > 5
        ) {
            revert InvalidSignatureNonce();
        }

        bytes32 recreatedMessage = Signatures.recreateMintMessage(
            narratorAddress,
            address(this),
            tx.origin,
        ...
        if (
            Signatures.recoverSigner(
                recreatedMessage,
                signatureData.narratorSignature
            ) != narratorAddress
        ) revert InvalidNarratorSignature();
        _;
    }

    modifier isValidClaimSignature(
        ClaimSignatureData calldata claimSignatureData
    ) {
        bytes32 recreatedMessage = Signatures.recreateClaimMessage(
            narratorAddress,
            address(this),
            tx.origin
        );
        if (
            Signatures.recoverSigner(
                recreatedMessage,
                claimSignatureData.claimSignature
            ) != narratorAddress
        ) revert InvalidClaimSignature();
        _;
    }

    modifier canMintArtifact(uint64 artifactId) {
        INarratorsHutMetadata metadataContract = INarratorsHutMetadata(
            metadataContractAddress
        );
        if (!metadataContract.canMintArtifact(artifactId)) {
    ...
    function mint(
        ...
        external
        payable
        isSignedByNarrator(signatureData)
        isCorrectPayment(mintData.length)
    {
        mintArtifacts(mintData);
    }
    ...
    function claimMint(
        ...
        payable
        isSignedByNarrator(signatureData)
        isValidClaimSignature(claimSignatureData)
        isCorrectPayment(mintData.length - 1)
    {
        mintArtifacts(mintData);
    }
    ...
    /**
     * @dev Extracts the artifactId, preassignedIndex and counter
     * that are packed into the uint256 tokenId.
     * @return (uint64 artifactId, uint64 preassignedIndex, uint128 witchId)
     */
    function getDataForToken(uint256 tokenId)
        internal
        view
        returns (
            uint64,
    ...
    {
        uint64 artifactId = _mintData[tokenId].artifactId;
        uint64 preassignedIndex = _mintData[tokenId].preassignedIndex;
        uint128 witchId = _mintData[tokenId].witchId;
        return (artifactId, preassignedIndex, witchId);
    }
    ...
    function mintArtifacts(MintData[] calldata mintData) private {
        for (uint256 i = 0; i < mintData.length; ) {
            mintArtifact(mintData[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function mintArtifact(MintData calldata mintData)
        private
        nonReentrant
        canMintArtifact(mintData.artifactId)
    {
...
library Signatures {
    ...
    function recreateMintMessage(
        ...
        bytes32 message = keccak256(
            abi.encodePacked(signer, contractAddress, userAddress, blockNumber)
        );
        return
            keccak256(
    ...
    // TODO: potentially include witch id
    function recreateClaimMessage(
        address signer,
        address contractAddress,
        address userAddress
    ) internal pure returns (bytes32) {
        bytes32 message = keccak256(
            abi.encodePacked(signer, contractAddress, userAddress)
        );
        return
            keccak256(
                abi.encodePacked("\x19Ethereum Signed Message:\n32", message)
    ...
    function recoverSigner(bytes32 recreatedMessage, bytes calldata signature)
        internal
        pure
        returns (address)
    {
        ...
        return ecrecover(recreatedMessage, v, r, s);
    }
}
...
struct SignatureData {
    uint256 blockNumber;
    bytes narratorSignature;
}

struct ClaimSignatureData {
...
struct MintData {
    uint128 witchId;
    uint64 artifactId;
    uint64 preassignedIndex; // 1-based index, 0 is treated as null
}
...
interface INarratorsHut {
    ...
    function setIsOpenSeaProxyActive(bool _isOpenSeaProxyActive) external;
}
