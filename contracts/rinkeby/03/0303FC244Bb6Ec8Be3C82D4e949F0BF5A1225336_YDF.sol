...
contract YDF is IYDF, ERC20, Ownable {
  ...
  function _transfer(
    ...
    if (_isSell && !(isTaxExcluded[sender] || isTaxExcluded[recipient])) {
      ...
      if (tax > 0) {
        ...
        if (_taxEarlyPerc > 0) {
          burnTaxAmount = (tax * _taxEarlyPerc) / 10000;
        }
      }
    ...
    if (burnTaxAmount > 0) {
      _burn(address(this), burnTaxAmount);
    }
  ...
  function getSellEarlyTax(
    ...
    if (lastBuyTimestamp[_sender] == 0) {
      return _tax;
    }

    if (sellTracker[_sender] + _sellAmount > buyTracker[_sender]) {
      return _tax;
    }

    if (block.timestamp > getSellEarlyExpiration(_sender)) {
      return 0;
    }
    uint256 _secondsAfterBuy = block.timestamp - lastBuyTimestamp[_sender];
    return
      (_tax * (EARLY_SELL_EXPIRATION - _secondsAfterBuy)) /
      EARLY_SELL_EXPIRATION;
  }

  function getSellEarlyExpiration(address _sender)
    public
    view
    returns (uint256)
  {
    return
      lastBuyTimestamp[_sender] == 0
        ? 0
        : lastBuyTimestamp[_sender] + EARLY_SELL_EXPIRATION;
  }
