...
contract CommunityLeaderboard is Ownable {
    ...
    event projectRegistser(
        ...
        uint256 indexed _projectCount
    );

    //func - addOwnerToProject
    event projectOwnerAdd(
        uint256 indexed _projectId,
        address indexed _newOwner
    );

    //func - createLeaderboardNftRequired
    event createNftRequiredLeaderboard(
        uint256 indexed _projectId,
        string indexed _leaderboardName,
        uint256 _time,
        uint256 _timeRemaining, //added
        uint256 indexed _nftsRequired
    );

    //func - createLeaderboardOpen
    ...
    event achriveResetLeaderboard(
        uint256 indexed _projectId,
        uint256 indexed _leaderboardId
    );
    ...
    //added - gets every id of a leaderboard a user has voted on
    ...
    function registerProject(address _nftContract, string memory _name, uint256 _projectCount, uint256 _numberOfLeaderboards) public {
        ...
        newProject.ownersArr.push(msg.sender); //added
        newProject.nftContract = _nftContract;
        newProject.name = _name;
        newProject.projectId = _projectCount;
        newProject.numberOfLeaderboards = 0;
        _numberOfLeaderboards = newProject.numberOfLeaderboards;


        projectCount = projectCount.add(1);

        emit projectRegistser(_nftContract, _name, _projectCount);
    }
    ...
    function createLeaderboardNftRequired(uint256 _projectId, string memory _leaderboardName, uint256 _time, uint256 _timeRemaining, uint256 _nftsRequired) public {
        ...
        if(_timeRemaining <= newLeaderboard.blockEnd) {
            (archiveAndResetLeaderboard);
        }
        //End
        ...
        emit createNftRequiredLeaderboard(_projectId, _leaderboardName, _time, _timeRemaining, _nftsRequired);
    }
    ...
    function createLeaderboardOpen(uint256 _projectId, string memory _leaderboardName, uint256 _time, uint256 _timeRemaining) public {
        ...
        emit createOpenLeaderboard(_projectId, _leaderboardName, _time, _timeRemaining);(_projectId, _leaderboardName, _time);
    }
    ...
    function castVote(uint256 _projectId, uint256 _leaderboardId, address _member, uint256 _nftTokenId) public {
        ...
        emit voteCast(_projectId, _leaderboardId, _member, _nftTokenId);
    }
