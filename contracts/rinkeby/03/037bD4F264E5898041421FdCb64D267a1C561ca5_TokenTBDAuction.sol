...
contract TokenTBDAuction is Ownable {
    ...
    uint8 internal constant DEFAULT_INTERVAL_HOURS = 12;
    ...
    constructor(
        ...
        initializeTimestamps(_timestamp, DEFAULT_INTERVAL_HOURS);
    }
    ...
    function placeBid(uint8 tokenID) external payable notLocked {
        require(
        ...
        Bid memory latestBid = Bid(msg.sender, msg.value);
        if (currentTopBid.bidAmount == 0) {
            ...
            payable(currentTopBid.wallet).transfer(currentTopBid.bidAmount);
        }
        currentTopBids[tokenID] = latestBid;
        bidHistory[tokenID].push(latestBid);
    }

    function endAuction(uint8 tokenId) external {
        require(
            block.timestamp >= bidTimestamps[tokenId].biddingClosed,
            "BIDDING_NOT_OVER"
        );
        require(address(tokenTBDK) != address(0), "NULL_CONTRACT_ADDRESS");
        require(currentTopBids[tokenId].wallet != address(0), "NO_WINNERS");
        tokenTBDK.mintPlatinum(tokenId, currentTopBids[tokenId].wallet);
    }
    ...
    // TODO: currently written as one bid per day. Confirm and/or modify.
    // TODO: Consider adding a shiftTimestamps() function.
...
contract TokenTBD is ERC721, Ownable {
    ...
    uint256 public constant SILVER_PRICE = 0.2 ether;
    ...
    // Number of phantom tokens minted - gold & silver
    uint16 public reservedPhantomGold;
    uint16 public reservedPhantomSilver;
    uint16[] public reservedPhantomTokenIDs;
    ...
    bool public phantomMintingLocked;
    ...
    address internal phantomMinter;
    ...
    modifier phantomMintingNotLocked() {
        require(!phantomMintingLocked, "PHANTOM_MINTING_LOCKED");
        _;
    }
    ...
    function mint(
        ...
        require(
            mintedSilver + silverQuantityToMint + reservedPhantomSilver <=
                silverSupply,
            "OUT_OF_STOCK_SILVER"
        );
        require(
            mintedGold + goldQuantityToMint + reservedPhantomGold <= goldSupply,
            "OUT_OF_STOCK_GOLD"
        );
        require(
    ...
    function reservePublicPhantom(
        uint8 silverQuantityToMint,
        uint8 goldQuantityToMint
    ) external phantomMintingNotLocked returns (uint16[] memory) {
        require(
            publicSaleLive && !presaleLive && !earlyBirdSaleLive,
            "SALE_CLOSED"
        );
        return reservePhantom(silverQuantityToMint, goldQuantityToMint);
    }
    ...
    function reserveEarlyBirdPhantom(
        uint8 silverQuantityToMint,
        uint8 goldQuantityToMint
    ) external phantomMintingNotLocked returns (uint16[] memory) {
        require(
    ...
    function reservePresalePhantom(
        uint8 silverQuantityToReserve,
        uint8 goldQuantityToReserve
    ) external phantomMintingNotLocked returns (uint16[] memory) {
        require(
            presaleLive && !publicSaleLive && !earlyBirdSaleLive,
            "PRESALE_CLOSED"
        );
        return reservePhantom(silverQuantityToReserve, goldQuantityToReserve);
    }
    ...
    event PhantomReserved(uint16[] tokenIDs);

    function getNextSilverTokenID() internal view returns (uint16) {
        return
            mintedSilver +
            reservedPhantomSilver +
            goldSupply +
            PLATINUM_SUPPLY +
            1;
    }
    ...
    function getNextGoldTokenID() internal view returns (uint16) {
        return mintedGold + reservedPhantomGold + PLATINUM_SUPPLY + 1;
    }

    function reservePhantom(
        uint8 phantomSilverQuantityToReserve,
        uint8 phantomGoldQuantityToReserve
    ) private phantomMintingNotLocked returns (uint16[] memory) {
        require(
            msg.sender == phantomMinter || msg.sender == owner(),
            "UNAUTHORIZED"
        );
        require(
            phantomSilverQuantityToReserve + phantomGoldQuantityToReserve <=
                maxMintsPerTx,
            "EXCEED_QUANTITY_LIMIT"
        );
        require(
            mintedSilver +
                phantomSilverQuantityToReserve +
                reservedPhantomSilver <=
                silverSupply,
            "OUT_OF_STOCK_SILVER"
        );
        require(
            mintedGold + phantomGoldQuantityToReserve + reservedPhantomGold <=
                goldSupply,
            "OUT_OF_STOCK_GOLD"
        );

        // array of reserved token IDs
        uint16[] memory reservedTokenIDsLocal = new uint16[](
            phantomSilverQuantityToReserve + phantomGoldQuantityToReserve
        );
        uint16 currentTokenId;

        for (uint8 i = 0; i < phantomSilverQuantityToReserve; i++) {
            currentTokenId = getNextSilverTokenID() + i;
            reservedTokenIDsLocal[i] = currentTokenId;
            reservedPhantomTokenIDs.push(currentTokenId);
        }
        reservedPhantomSilver += phantomSilverQuantityToReserve;

        for (uint8 i = 0; i < phantomGoldQuantityToReserve; i++) {
            currentTokenId = getNextGoldTokenID() + i;
            reservedTokenIDsLocal[
                i + phantomSilverQuantityToReserve
            ] = currentTokenId;
            reservedPhantomTokenIDs.push(currentTokenId);
        }
        reservedPhantomGold += phantomGoldQuantityToReserve;

        // If we're done, set the starting index of the collection.
        trySetStartingIndex();

        emit PhantomReserved(reservedTokenIDsLocal);

        return reservedTokenIDsLocal;
    }

    // When phantom NFT buyer actually gets a wallet; mint the NFT for them upon request
    function mintPhantom(
        address[] calldata purchasers,
        // Array of token IDs purchased by each purchaser
        uint32[][] calldata tokenIDs
    ) external onlyOwner {
        require(purchasers.length == tokenIDs.length, "UNEQUAL_LENGTH");
        uint8 silverPhantomMintedLocal = 0;
        uint8 goldPhantomMintedLocal = 0;
        for (uint32 i = 0; i < tokenIDs.length; i++) {
            for (uint8 j = 0; j < tokenIDs[i].length; j++) {
                require(
                    tokenIDs[i][j] > PLATINUM_SUPPLY,
                    "INVALID_ONLY_MINTING_SILVER_OR_GOLD"
                );
                require(
                    tokenIDs[i][j] <=
                        goldSupply + silverSupply + PLATINUM_SUPPLY,
                    "INVALID_TOKEN_ID"
                );
                if (tokenIDs[i][j] > PLATINUM_SUPPLY + goldSupply) {
                    silverPhantomMintedLocal++;
                    require(
                        silverPhantomMintedLocal <= reservedPhantomSilver,
                        "ERROR_INADEQUATE_RESERVED_SILVER_SUPPLY_REMAINING"
                    );
                } else {
                    goldPhantomMintedLocal++;
                    require(
                        goldPhantomMintedLocal <= reservedPhantomGold,
                        "ERROR_INADEQUATE_RESERVED_GOLD_SUPPLY_REMAINING"
                    );
                }
                _safeMint(purchasers[i], tokenIDs[i][j]);
            }
        }
        if (silverPhantomMintedLocal > 0 && reservedPhantomSilver > 0) {
            mintedSilver += silverPhantomMintedLocal;
            reservedPhantomSilver -= silverPhantomMintedLocal;
        }
        if (goldPhantomMintedLocal > 0 && reservedPhantomGold > 0) {
            mintedGold += goldPhantomMintedLocal;
            reservedPhantomGold -= goldPhantomMintedLocal;
        }
    }
    ...
    function lockPhantomMinting() external onlyOwner {
        phantomMinter = address(0);
        phantomMintingLocked = true;
    }

    function unlockPhantomMinting(address _newPhantomMinter)
        external
        onlyOwner
    {
        phantomMintingLocked = false;
        phantomMinter = _newPhantomMinter;
    }
    ...
    function assignPhantomMinter(address _phantomMinter) external onlyOwner {
        require(_phantomMinter != address(0), "NULL_ADDRESS");
        phantomMinter = _phantomMinter;
    }

    function setReservedPhantomSilver(uint16 _reservedPhantomSilver)
        external
        onlyOwner
    {
        reservedPhantomSilver = _reservedPhantomSilver;
    }

    function setReservedPhantomGold(uint16 _reservedPhantomGold)
        external
        onlyOwner
    {
        reservedPhantomGold = _reservedPhantomGold;
    }
