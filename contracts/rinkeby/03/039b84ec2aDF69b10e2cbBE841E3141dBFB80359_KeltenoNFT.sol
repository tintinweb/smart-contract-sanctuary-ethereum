...
contract KeltenoNFT is ERC721Enumerable, PullPayment, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721("KeltenoKST", "KLTN") {
    ...
    function buyKeltenoTokenFromDeployer(uint256 tokenId,uint256 ,address buyer) external payable nonReentrant {
        require(tokenSellEnabled, "Token Sale Paused");
        require(forSale[tokenId].isExist, "This Token Does not Exist or Not For Sale");
        require(msg.value >= forSale[tokenId].price, string(abi.encodePacked("Insufficient Payment: This Token Costs ",forSale[tokenId].price)));
        transferFrom(owner() ,buyer, tokenId);
    }

    function buyKeltenoTokenFromContract(uint256 tokenId,uint256 ,address buyer) external payable nonReentrant {
        require(tokenSellEnabled, "Token Sale Paused");
        require(forSale[tokenId].isExist, "This Token Does not Exist or Not For Sale");
        require(msg.value >= forSale[tokenId].price, string(abi.encodePacked("Insufficient Payment: This Token Costs ",forSale[tokenId].price)));
        ...
        for (uint256 i = 0; i < _recipients.length; i++) { 
            recipients[_recipients[i]] = false; 
                for(uint256 val = 0; val < recipientsRegistry.length; val++){
                    if(recipientsRegistry[val] == _recipients[i]) {
    ...
    function claim() external  {
        ...
        IERC721(msg.sender).transferFrom(address(this), msg.sender, airdrops[nextAirdropId]);
        nextAirdropId++;
    }
    ...
    function addMintRecipients(address[] memory _recipients) external onlyOwner {
        for (uint256 i = 0; i < _recipients.length; i++) { mintRecipients[_recipients[i]] = true; }
    }

    function removeMintRecipients(address[] memory _recipients) external onlyOwner {
        for (uint256 i = 0; i < _recipients.length; i++) { mintRecipients[_recipients[i]] = false; }
    }
