...
contract ProjectENFT is Ownable, ERC721A, ReentrancyGuard {
  ...
  function _mint(uint256 _mintAmount,  uint256 [] memory nfttype_numbers) internal{
    ...
    for (uint256 i = 0; i < nfttype_numbers.length; i++) {
      ...
      if (nfttype_number==nft_A){
        total += PRICE_A ;      
        addNewNftOfType(msg.sender,'A',_mintAmount);
      } else if (nfttype_number==nft_B){
        total += PRICE_B ;
        addNewNftOfType(msg.sender,'B',_mintAmount);
      } 
    }    
  ...
  function publicMint(uint256 _mintAmount, uint256 [] memory nfttype_numbers) external payable callerIsUser {
    require(status == 2 , "Sale is not Active for public sale"); 
    _mint(_mintAmount, nfttype_numbers);   
  }

  function whitelistMint(uint256 _mintAmount,  uint256 [] memory nfttype_numbers, bytes32[] calldata _merkleProof)
        public
        payable       
  {
    // Verify whitelist requirements      
    require(status == 1 , "Sale is not activated for presale");     
