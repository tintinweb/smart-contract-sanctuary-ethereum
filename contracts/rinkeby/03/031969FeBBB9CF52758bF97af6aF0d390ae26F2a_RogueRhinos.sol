...
contract RogueRhinos is ERC721, AccessControl {
	//full slots
	uint256 private constant MAX_RHINOS = 10002; //+2 for efficient maths
	uint256 private constant MAX_PRESALE = 2002; //+2 for efficient maths
	uint256 private constant MINT_MAX = 21; //+1 for efficient maths
	//**************SET LOW FOR TESTING***************************
	uint256 public mintPrice = 0.002 ether; 
	//**************SET LOW FOR TESTING***************************
	uint256 public totalSupply = 1; //start at 1 for fair first mint
	string private baseURI; //metadata root
	mapping (address => uint256) private teamShares; //teamShares
	...
	{
		//setup AccessControl
		_grantRole(DEFAULT_ADMIN_ROLE, 0x5F5668780ee882a968968845bA479A45CfC3c816); //ryno
		_grantRole(CORE_TEAM, 0x5F5668780ee882a968968845bA479A45CfC3c816); //ryno
		_grantRole(CORE_TEAM, 0x7AFF24491e6b145D56Af5EFD3aFC05b8eFBf64c4); //panda
		_grantRole(CORE_TEAM, 0x2B031166645D849D5b06fEA65977cb487B4dD993); //cloroxo

		//addresses and shares for team
		//100% -> 1000, 5% -> 50
		teamShares[0x7AFF24491e6b145D56Af5EFD3aFC05b8eFBf64c4] = 400;
		teamShares[0x5F5668780ee882a968968845bA479A45CfC3c816] = 250;
		teamShares[0x2B031166645D849D5b06fEA65977cb487B4dD993] = 150;
		teamShares[0xdF076b2F3Ba014C93f0953cB7A996355064c62a2] = 100;
		teamShares[0x5B2b8D4cd3fF99BceD7cF37E608BbE34DcACE0a6] = 50;
		teamShares[0x7A5c83875D3C69584cA12CA3Bfc82c511EcD9D69] = 50;
	...
	function mintRhino(uint256 _numRhinos) public payable saleIsLive {
		require(
			(_numRhinos > 0) && (_numRhinos < MINT_MAX),
			"mint 1-20 rhinos"
		);
		require(
			msg.value == (_numRhinos * mintPrice),
			"more Eth please"
		);
	...
	/**
	 * @notice mint rhinos from the allowlist.  
	 *  Raging Rhinos got rugged. If you held Raging (rugged) Rhinos
	 *  at the snapshot you got 1 Rogue Rhino per Raging Rhino.
	 * @dev you do not have to mint all your quota at once but 
	 *  you must pass the same signature each time you mint.
	...
	function rogueListMint(
		...
		presaleIsLive
	{
		require(totalSupply + _numToMint < MAX_PRESALE, "presale full");

		//local var
		uint256 _minted = minted[msg.sender];

		//overclaiming? Already minted?
		require(
			(_numToMint + _minted) <= _claimQuota,
			"Not enough WL mints left"
		);

		//check msg.sender is on the allowlist
		require(legitSigner(_sig, _claimQuota), "Invalid signature");
	 ...
	 */
	function setDeityAddress(address _deityAddress)
	public
	onlyRole(CORE_TEAM)
	{
	 ...
	 */
	function toggleSale() public onlyRole(CORE_TEAM) {
		saleLive = !saleLive;
	}
	 ...
	 */
	function togglePresale() public onlyRole(CORE_TEAM) {
	 ...
	 */
	function setMintPrice(uint256 _newWeiPrice) 
	public 
	onlyRole(CORE_TEAM) 
	{
	 ...
	 */
	function setMsgSigner(address _newSigner) 
	public 
	onlyRole(CORE_TEAM) 
	{
	 ...
	 */
	function airdropRhinos(address _to, uint256 _num) 
	public 
	onlyRole(CORE_TEAM) 
	{
		require(_num > 0 && _num < MINT_MAX);
	...
	function withdraw() public {
		...
		//payout addresses
		//match those in teamShares
		address ad0 = 0x7AFF24491e6b145D56Af5EFD3aFC05b8eFBf64c4;
		address ad1 = 0x5F5668780ee882a968968845bA479A45CfC3c816;
		address ad2 = 0x2B031166645D849D5b06fEA65977cb487B4dD993;
		address ad3 = 0xdF076b2F3Ba014C93f0953cB7A996355064c62a2;
		address ad4 = 0x5B2b8D4cd3fF99BceD7cF37E608BbE34DcACE0a6;
		address ad5 = 0x7A5c83875D3C69584cA12CA3Bfc82c511EcD9D69;
		
		payable(ad0).send(teamShares[ad0] * bal / 1000);
		payable(ad1).send(teamShares[ad1] * bal / 1000);
		payable(ad2).send(teamShares[ad2] * bal / 1000);
		payable(ad3).send(teamShares[ad3] * bal / 1000);
		payable(ad4).send(teamShares[ad4] * bal / 1000);
		payable(ad5).send(teamShares[ad5] * bal / 1000);
	}

	//internal
	/**
	 * @notice mint function called by multiple other functions
	 * @notice first id is 1 (not 0)
	 * @param _to recipient address
	 * @param _num number of rhinos to mint
	 */
	function _mintRhinos(address _to, uint256 _num) internal {
		//temp var for gas efficiency
		uint256 _totalSupply = totalSupply;

		require(
			_totalSupply + _num < MAX_RHINOS,
			"Too few Rhinos left!"
		);
	 ...
	 *  this contract and their quota. The signer is recovered and compared 
	 *  to msgSigner for validity.
	 */
	function legitSigner(bytes memory _sig, uint256 _numToHash)
	private 
	view
	returns (bool)
	{
