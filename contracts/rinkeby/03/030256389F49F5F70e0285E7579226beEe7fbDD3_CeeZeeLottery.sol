...
contract CeeZeeLottery is ReentrancyGuard, Ownable, ICeeZeeLottery {
    ...
    uint256 public constant MIN_DISCOUNT_DIVISOR = 300; // Minimum discount rate 
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInCZ;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 burnFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] CZPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInCZ;
        uint32 finalNumber;
    }
     ...
     * @param _CZTokenAddress: address of the CeeZee token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _CZTokenAddress, address _randomGeneratorAddress) {
        CZToken = IERC20(_CZTokenAddress);
     ...
     * @param _priceTicketInCZ: price of a ticket in CZ token
    ...
    function startLottery(
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInCZ: _priceTicketInCZ,
            ...
            amountCollectedInCZ: pendingInjectionNextLottery,
            finalNumber: 0
        });
        ...
        nonReentrant
    {
        ...
        // Calculate number of CZ to this contract
        uint256 amountCZToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInCZ,
            _ticketNumbers.length
        );

        // Transfer CZ tokens to this contract
        CZToken.safeTransferFrom(
            address(msg.sender),
            address(this),
            amountCZToTransfer
        );
        ...
        _lotteries[_lotteryId].amountCollectedInCZ += amountCZToTransfer;
    ...
    /**
     * @notice Draw the final number, calculate reward in CZ per group, and make lottery claimable
     * @param _lotteryId: lottery id
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInCZ) *
                (BASE_DENOMINATOR - _lotteries[_lotteryId].burnFee))
        ) / BASE_DENOMINATOR;

        // Initializes the amount to withdraw to burn
        uint256 amountToWithdrawToBurn;

        // Calculate prizes in CZ for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].CZPerBracket[j] =
                ...
                // C. No CZ to distribute, they are added to the amount to withdraw to burn address
            } else {
                _lotteries[_lotteryId].CZPerBracket[j] = 0;
        ...
        amountToWithdrawToBurn += (_lotteries[_lotteryId]
            .amountCollectedInCZ - amountToShareToWinners);

        // Transfer CZ to address dead to burn
        CZToken.safeTransfer(0x000000000000000000000000000000000000dEaD, amountToWithdrawToBurn);
    ...
    function claimTickets(
        ...
        // Initializes the rewardInCZToTransfer
        uint256 rewardInCZToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInCZToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        CZToken.safeTransfer(msg.sender, rewardInCZToTransfer);

        emit TicketsClaim(
            msg.sender,
            rewardInCZToTransfer,
            _lotteryId,
            _ticketIds.length
        );
    }
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in CZ token
        ...
        onlyOwnerOrInjector
    {
        ...
        CZToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInCZ += _amount;
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount)
        ...
        onlyOwner
    {
        require(_tokenAddress != address(CZToken), "Cannot be CeeZee token");
     ...
     * @param _priceTicket price of a ticket (in CeeZee token)
     ...
     * @param _lotteryId: lottery id
     */
    function checkUserInfoForPrice(
        address _user,
        uint256 _lotteryId
    )
        external
        ...
        )
    {
        ...
        for (uint32 i = 0; i < numberTicketsBoughtAtLotteryId; i++) {
            ...
            for (uint32 j = 5; j > 0; j--) {
                uint256 price =  _calculateRewardsForTicketId(_lotteryId, lotteryTicketIds[i], j);
                if (price != 0) {
                    brackets[i] = j;
                    ticketWon[i] = true;
                    break;
    ...
    function _calculateRewardsForTicketId(
        ...
        uint32 winningTicketNumber = _revertNumber(_lotteries[_lotteryId].finalNumber);
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].CZPerBracket[_bracket];
        } else {
            return 0;
        }
    }
...
interface ICeeZeeLottery {
     ...
     * @dev Callable by operator
     * @param _endTime: endTime of the lottery
     * @param _priceTicketInCZ: price of a ticket in CZ
     * @param _discountDivisor: the divisor to calculate the discount magnitude for bulks
     * @param _rewardsBreakdown: breakdown of rewards per bracket (must sum to 10,000)
