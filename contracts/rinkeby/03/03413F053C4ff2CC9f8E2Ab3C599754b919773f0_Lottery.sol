...
contract Lottery is Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Inactivate lottery status
     */
    function inactivateLottery(uint256[] calldata _lotteryIds)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _lotteryIds.length; i++) {
    ...
    function enterLottery(uint256 _lotteryId, uint256 _amount) public payable {
        ...
        require(_amount > 0, "Not enough tickets!");
        require(
    ...
    function endLottery(uint256 _lotteryId) public onlyOwner nonReentrant {
        ...
        require(
            lotteries[_lotteryId].players.length > 0,
            "Error: less than 1 participant"
        );
    ...
    function _drawWinner(uint256 _lotteryId) internal {
        ...
        if (lotteryPlayers.length > 0) {
            if (lotteryPlayers.length == 1) {
                // lotteryPlayers[0].transfer(address(this).balance);
            } else {
                ...
                lotteries[_lotteryId].winner.transfer(address(this).balance);
            }
        }
        lotteries[_lotteryId].status = LOTTERY_STATE.EXPIRED;
    }
}
