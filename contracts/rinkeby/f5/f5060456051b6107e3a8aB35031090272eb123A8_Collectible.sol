...
// 0xLabs NFT Collection Proof Of Concept
//
// Author: https://twitter.com/0xPilou
...
contract Collectible is ERC721, Ownable {
    ...
    uint256 public constant MAX_QTY = 5;
    ...
    */
    ...
    function setBaseURI(string memory _baseTokenURI) public onlyOwner {
        require(bytes(baseTokenURI).length == 0, "Cannot re-update the base URI");
    ...
    // 20% is sent to the dev, remaining 80% split among the artists.
    function withdrawAll() public payable onlyOwner {
        uint256 balance = address(this).balance;
        require(balance > 0);
        // 20% to dev address
        _widthdraw(devAddress, (balance * 20) / 100);
        // 40% to creator 1 address
        _widthdraw(creator1Address, address(this).balance / 2);
        // 40% to creator 2 address
    ...
    function forwardERC20(address _token) public onlyOwner {
        ...
        IERC20(_token).transfer(devAddress, (balance * 20) / 100);

        balance = IERC20(_token).balanceOf(address(this));
        IERC20(_token).transfer(creator1Address, balance / 2);

        balance = IERC20(_token).balanceOf(address(this));
        IERC20(_token).transfer(creator2Address, balance);
    }
