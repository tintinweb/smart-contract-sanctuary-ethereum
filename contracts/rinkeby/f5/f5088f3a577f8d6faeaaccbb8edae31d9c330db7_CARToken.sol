...
// ----------------------------------------------------------------------------
// CAR Token, with the addition of symbol, name and decimals and assisted
// token transfers
// ----------------------------------------------------------------------------
contract CARToken is ERC20Interface, Owned, SafeMath, Context {
	...
	uint32 public constant minimumTimeBetweenMints = 1 seconds * 7;
	...
	function transfer(address to, uint256 tokens) external override returns (bool success) {
		require(to != address(0), "CAR: transfer to the zero address");
		...
		returns (bool success)
	{
		require(
			...
			"CAR: number of receiver addresses and number of amounts should be equal"
		);
	...
	function approve(address spender, uint256 tokens) public override returns (bool success) {
		require(spender != address(0), "CAR: transfer to the zero address");
	...
	function permit(
		...
		require(signatory != address(0), "CAR::permit: invalid signature");
		require(signatory == owner, "CAR::permit: unauthorized");
		require(block.timestamp <= deadline, "CAR::permit: signature expired");
	...
	function burn(address account, uint256 amount) external {
		require(_msgSender() == _minter, "CAR::burn: only the minter can mint");
		require(account != address(0), "CAR: burn from the zero address");
	...
	function mint(address account, uint256 amount) external {
		require(_msgSender() == _minter, "CAR::mint: only the minter can mint");
		require(block.timestamp >= mintingAllowedAfter, "CAR::mint: minting not allowed yet");
		require(account != address(0), "CAR: mint to the zero address");
	...
	function setMinter(address minter_) external {
		require(
			_msgSender() == _minter,
			"CAR::setMinter: only the minter can change the minter address"
		);
