...
contract NFT is ERC721, ERC721Enumerable, Ownable, Pausable {
    using Strings for uint256;
    using Counters for Counters.Counter;

    // Counter of next tokenId
    Counters.Counter private _tokenIdCounter;

    /// URI of where tokens are uploaded, will form the url as <baseURI><tokenId><baseExtension> in tokenURI()
    string public baseURI;
    /// Extension of the baseURI of where tokens are uploaded, will form the url as <baseURI><tokenId><baseExtension> in tokenURI()
    string public baseExtension = ".json";
    /// Cost per mint
    uint256 public cost = 0.0025 ether;
    /// Maximum available tokens
    uint256 public maxSupply = 7777;
    /// Maximum number of nfts that an address can mint in total
    uint256 public nftPerAddressLimit = 5;
    // Flag to indicate if minting is limited to whitelisted addresses
    bool public onlyWhitelisted = true;
    // Flag to indicate if the tokens are revealed
    bool public isRevealed = false;

    // List of whitelisted addresses, that can mint when onlyWhitelisted is true
    address[] public whitelistedAddresses = [
        // Test account1
        0x0cCeDBD2f0FEcc5fDaf9097E513f18C5Da47fca2,
        // Test account2
    ...
    function mint(uint256 _mintAmount) public payable whenNotPaused {
        ...
        // Need to mint at least 1 token
        require(_mintAmount > 0, "need to mint at least 1 NFT");
        // No more than max supply
        require(supply + _mintAmount <= maxSupply, "Max supply reached");

        if (msg.sender != owner()) {
            // Limit miniting to whitelisted addresses only
            if (onlyWhitelisted) {
                require(isWhitelisted(msg.sender) == true, "Not whitelisted");
            }

            // Cannot mint more than nftPerAddressLimit + currently minted
            require(
                _mintAmount + balanceOf(msg.sender) <= nftPerAddressLimit,
                "Cannot mint more than max amount per address"
            );

            // Check value is high enough to mint
