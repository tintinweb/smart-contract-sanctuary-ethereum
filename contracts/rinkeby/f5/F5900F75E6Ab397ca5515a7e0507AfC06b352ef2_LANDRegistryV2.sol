    ...
    ERC721Metadata
{
    ...
    //     require(
    //         msg.sender == owner() || _isApprovedForAll(user, msg.sender),
    //         "Unauthorized user"
    //     );
    //     // solium-disable-next-line security/no-block-members
    ...
    function _updateLandData(
        ...
        emit Update(assetId, owner, msg.sender, name, description);
    }
    ...
    // function setLandBalanceToken(address _newLandBalance) external onlyOwner {
    //     require(
    //         _newLandBalance != address(0),
    //         "New landBalance should not be zero address"
    //     );
    //     emit SetLandBalanceToken(address(landBalance), _newLandBalance);
