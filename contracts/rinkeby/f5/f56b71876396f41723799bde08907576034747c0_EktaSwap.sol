...
contract EktaSwap is Initializable, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize(address _legacyToken) public initializer {
    // initializing
  ...
  function migrate(uint256 amount) external nonReentrant whenNotPaused {
    ...
    legacyToken.transferFrom(msg.sender, address(this), amount);
  ...
  function updateLegacyToken(address token) external virtual onlyOwner {
    require(token != address(0), "Token cant be zero address");
    legacyToken = IERC20Upgradeable(token);
  }
}
