...
contract SlotsV1 is VRFConsumerBaseV2 {
    ...
    event RandomResult(uint256[] randomNumbers);
    event Prize(address player, uint256 value, uint256[] randomNumbers, uint256 prize);
    ...
    function fulfillRandomWords(
        ...
        emit RequestFulfilled(randomWords);
        uint256[] memory randomNumbers;
        for (uint256 i = 0; i < numWords; i++) {
            randomNumbers[i] = (randomWords[i] % 50 ) + 1;
        }
        emit RandomResult(randomNumbers);
        
        uint256 value = history[requestId].value;
        address sender = history[requestId].sender;
        ...
        emit Prize(sender, value, randomNumbers, prize);
    }
