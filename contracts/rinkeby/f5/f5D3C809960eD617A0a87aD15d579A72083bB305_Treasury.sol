...
contract DynamicNft is AccessControl, IDynamic {
    ...
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    bytes32 public merkleRoot = 0x3b68c56bb32b3fd63738d61fa266ef907e7135700575c279511f870192b46efc;
    ...
    uint256 public genesisPrice = 20;
    uint256 public subscriptionPrice = 10;
    uint256 private projectId = 1;
    uint256 public genesisSupply = 2000;
    uint256 public freeGenesisMinted;
    ...
    function mintGenesis(address _to, bytes32[] calldata _merkleProof, uint256 amount) public payable {
        uint256 genesisEthPrice = getGenesisPrice();
        INft GenesisNft = INft(genesisContractAddress);
        require(msg.value >= (genesisEthPrice * amount), "INSUFFICIENT MINTING VALUE");
        ...
        for (uint256 _id = GenesisNft.currentIndex(); _id < (amount + GenesisNft.currentIndex()); _id++) {
            genesisHolder[_id] = Genesis({tokenId: _id, owner: _to, ethPayed: (msg.value/amount)});
    ...
    // function mintGiftSubscription(address _to, uint256 amount) public onlyRole(ADMIN_ROLE) {
    //     INft GenesisNft = INft(subsContractAddress);
    //     require(genesisStatus, "GENESIS MINT CURRENTLY INACTIVE");
    //     require(GenesisNft.totalSupply() + amount <= genesisSupply, "INSUFICIENT GENESIS STOCK");
    //     require(GenesisNft.totalSupply() + amount <= genesisSupply, "INSUFICIENT GENESIS STOCK");
    //     require(freeGenesisMinted + amount <= (genesisSupply * 4) / 100, "INSUFICIENT FREE GENESIS STOCK");

    //     for (uint256 _id = GenesisNft.currentIndex(); _id < (amount + GenesisNft.currentIndex()); _id++) {
    //         genesisHolder[_id] = Genesis({tokenId: _id, owner: _to, ethPayed: 0});
    //         emit GenesisMinted(_to, _id);
    //     }

    //     freeGenesisMinted += amount;
    //     GenesisNft.mint(_to, amount);
    ...
    function mintSubscription(address _to, uint256 amount) public payable {
        ...
        uint256 subscriptionEthPrice = getSubscriptionPrice();
        require(msg.value >= (subscriptionEthPrice * amount), "INSUFFICIENT MINTING VALUE");
    ...
    function claimRefund(uint256[] calldata tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++){
            ...
            if (GenesisNft.ownerOf(tokenId) == msg.sender){
                Genesis storage token = genesisHolder[tokenId];
                uint256 toRefund = token.ethPayed;
    ...
    function renewSubscription(uint256 _tokenId) public payable {
        uint256 subscriptionEthPrice = getSubscriptionPrice();
        require(msg.value >= (subscriptionEthPrice), "INSUFFICIENT MINTING VALUE");
    ...
    function setGenesisPriceUsd(uint256 price) public onlyRole(ADMIN_ROLE) {
        genesisPrice = price;
    }

    function setSubscriptionPriceUsd(uint256 price) public onlyRole(ADMIN_ROLE) {
    ...
    function getGenesisPrice() public view returns (uint256) {
        uint256 dollarPrice = getLatestPrice();
        return (genesisPrice * 10**18) / dollarPrice;
    }

    function getSubscriptionPrice() public view returns (uint256) {
        uint256 dollarPrice = getLatestPrice();
        return (subscriptionPrice * 10**18) / dollarPrice;
    }
