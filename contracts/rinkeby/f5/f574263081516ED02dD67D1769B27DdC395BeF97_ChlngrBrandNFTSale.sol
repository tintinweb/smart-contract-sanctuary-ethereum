...
contract ChlngrBrandNFTSale is Ownable {
    ...
    */
    function canMintForGeneration(uint8 _genIndex, uint16 _quantity) public view returns (bool) {
        return ChlngrBrandNFT(nftAddress).canCreateToken(_genIndex, _quantity);
    }
    ...
    */
    function canMintForAccount(address _account, uint16 _quantity) public view returns (bool) {
        ...
        return ChlngrBrandNFT(nftAddress).balanceOf(_account) + _quantity <= MAX_PER_WALLET;
    }
   ...
   */
    function mint(uint8 _genIndex, uint16 _quantity, bytes calldata _data) public payable {
        ...
        require(canMintForGeneration(_genIndex, _quantity) == true, "Chlngr Brand NFT Sale: Maxium mint reached for the generation");

        require(isWhitelistAccount(msg.sender) == true, "Chlngr Brand NFT Sale: The account is not in the whitelist");

        require(canMintForAccount(msg.sender, _quantity) == true, "Chlngr Brand NFT Sale: Maxium mint reached for the account");
        
        if (!hasGift(msg.sender) && payable(msg.sender) != walletMaster && payable(msg.sender) != walletDevTeam) {
            require(msg.value >= mintFee * _quantity, "NFTSale: Not enough ETH sent");
        ...
        } else { // for master wallet or gift owner
    ...
    * @param _quantity token amount
    */
    function mintWhiltelistToken(uint16 _quantity) public payable {
        
        if (payable(msg.sender) != walletMaster && payable(msg.sender) != walletDevTeam) {
            require(msg.value >= whitelistTokenMintFee * _quantity, "NFTSale: Not enough ETH sent");
