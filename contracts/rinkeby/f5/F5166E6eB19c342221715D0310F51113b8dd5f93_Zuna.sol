    ...
    IZuna
{
    struct NFTVoucher {
        ...
        uint256 royaltyFee; // 3 decimal places
    ...
    function lazyMint(
        ...
        if (tokenOwner != signer) {
            _transfer(signer, tokenOwner, voucher.tokenId);
        }
        return tokenId;
    }
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "NFTVoucher(uint256 tokenId,uint256 price,address erc20Address,uint256 royaltyFee,uint256 collectionId,string tokenUri)"
                        ),
                        voucher.tokenId,
                        voucher.price,
                        voucher.erc20Address,
                        voucher.royaltyFee,
