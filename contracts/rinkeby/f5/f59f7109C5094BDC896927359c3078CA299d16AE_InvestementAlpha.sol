...
contract Investement is Pausable {
     ...
     */
    function calculateMaxDepositAmount( ) external onlyManager {
     ...
     * by the manager for the users.
     */
    function calculateMaxWithdrawAmount( ) external onlyManager {
...
contract Admin is Ownable {
    ...
    uint256 public lockupPeriodManager = 2 hours; 
    ...
    function setInvestement(address _investement) public onlyOwner {
        require(
    ...
    function setLockupPeriodManager(uint256 _lockupPeriodManager) external onlyOwner {
        lockupPeriodManager = _lockupPeriodManager;
    }
    ...
    function setDepositFeeRateParity(uint256 _rate) external onlyOwner {
    ...
    function calculateNetDepositInd(uint256 _depositAmountTotal, 
        ...
        uint256 _maxWithdrawalAmount) public onlyInvestement returns( uint256) {
     ...
     */
    function protectAgainstSlippage(uint256 _withdrawalAmount) public onlyManager 
        returns (uint256) {
        ...
        if (_withdrawalAmount< netAmountEvent){
            ...
            if (_slippage >= slippageTolerance) {
                ...
                if ( _deltaAmount  > 0){
                    ...
                    _alphaAmount = (_deltaAmount * FACTOR_PRICE_DECIMALS)/tokenPrice;
                    token.mint(treasury, _alphaAmount);
          ...
          _alphaAmount = (_amount * FACTOR_PRICE_DECIMALS)/tokenPrice;
          _alphaAmount = Math.min(_alphaAmount, _balanceTokenTreasury);
          if (_alphaAmount >0) {
              _deltaAmount = (_alphaAmount * tokenPrice)/FACTOR_PRICE_DECIMALS;
              stableToken.safeTransfer(treasury, _deltaAmount/amountScaleDecimals);   
              token.burn( treasury, _alphaAmount);
            }
           if ((_amount - _deltaAmount) > 0) {
              ...
              stableToken.safeTransfer(manager, (_amount - _deltaAmount)/amountScaleDecimals); 
            }
        }
        return 0;
