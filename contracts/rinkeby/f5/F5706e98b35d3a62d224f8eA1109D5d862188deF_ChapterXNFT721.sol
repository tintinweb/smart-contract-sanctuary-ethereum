    ...
    AccessControl
{
    ...
    function royaltyInfo(uint256 _tokenId, uint256 price)
        external
        view
        returns (
            uint96[] memory,
        ...
        )
    {
        require(
            _exists(_tokenId),
            "ERC721Royalty: query for nonexistent token"
        );
        require(price > 0, "ERC721Royalty: amount should be greater than zero");
        uint96[] memory royaltyFee = new uint96[](
            tokenRoyalty[_tokenId].royaltyPermiles.length
        );
    ...
    function _setTokenRoyalty(
        ...
        require(
            royaltyFeePermiles.length == receivers.length,
            "ERC721Royalty: length should be same"
        );
