...
contract FengShuiAnimal is AbstractAnimal, ERC2981Royalties, Ownable, ReentrancyGuard {
	using Strings for uint256;

	// Max number to be minted via Diamond/Gold passes
	uint256 public maxEarlyAccessSupply;
	// Max number to be minted
	...
	// Max number of mints for diamond and gold pass holders
	uint8 public MAX_PURCHASE_PER_PASS = 1;

	string public name = "Feng Shui Animal";
	...
	event Purchased(address indexed account, Animal id, uint256 amount);
	...
	/**
	 * @notice Purchase Feng Shui Animals during sale
	 * @notice Diamond and gold pass holders will have earlier access to purchase
	 *
	 * @param _amount the amount of tokens to purchase
	 */
	function purchase(Animal _animal, uint256 _amount) public payable nonReentrant {
		...
		uint256 supply = _validateAnimalSupply(_animal, _amount);
		_validatePurchaseWindows(_amount, supply);

		_mint(msg.sender, uint256(_animal), _amount, "");

		emit Purchased(msg.sender, _animal, _amount);
	}

	/**
	 * @dev Get all tokens balances for all token ids
	 * @return tokenBalances - array of token balances for each token id
	 */
	function tokensOf(address _owner) public view returns (TokenBalance[] memory) {
		TokenBalance[] memory tokenBalances = new TokenBalance[](TOTAL_ANIMALS + 1);
		for (uint256 id = 0; id <= TOTAL_ANIMALS; id++) {
			uint256 balance = balanceOf(_owner, id);
			tokenBalances[id] = TokenBalance(id, balance);
		}
	...
	/**
	 * Set the Feng Shui Animal's metadata URI
	 */
	 ...
	 *
	 * @param _animal the token id
	 * @param _amount the amount of tokens to purchase
	 * @param _owner the address to mint to
	 */
	function reserve(
		Animal _animal,
		uint256 _amount,
		...
		_validateAnimalSupply(_animal, _amount);

		_mint(_owner, uint256(_animal), _amount, "");
	}
	...
	function _validatePurchaseWindows(uint256 _amount, uint256 _supply) internal {
		if (block.timestamp >= publicPurchaseWindowOpens) {
			// Return to purchase
			return;
		} else if (block.timestamp >= goldPassWindowOpens) {
			require(
				diamondPass.balanceOf(msg.sender) > 0 || goldPass.balanceOf(msg.sender) > 0,
				"Not a diamond or gold pass member"
			);
			// Validate if all diamond or gold passes owned have not been redeemed
			_validateDiamondOrGoldPass(_amount, _supply);
		} else if (block.timestamp >= diamondPassWindowOpens) {
			require(diamondPass.balanceOf(msg.sender) > 0, "Not a diamond pass member");
			// Validate if all diamond passes owned have not been redeemed
			require(_validateDiamondPass(_amount, _supply), "Diamond passes all redeemed");
		} else {
			revert("Sale not active");
		}
	}

	function _validateDiamondOrGoldPass(uint256 _amount, uint256 _supply)
		internal
		validateEarlyAccessPurchaseAmount(_amount, _supply)
	{
		if (_validateDiamondPass(_amount, _supply)) return;

		uint256[] memory goldTokens = goldPass.tokensOf(msg.sender);
		// Check if any gold passes have not been redeemed yet
		for (uint256 i = 0; i < goldTokens.length; i++) {
			uint256 currentId = goldTokens[i];
			uint256 redeemed = redeemedGoldPassMatrix[currentId];
			if (redeemed + _amount <= MAX_PURCHASE_PER_PASS) {
				redeemedGoldPassMatrix[currentId] = redeemed + _amount;
				return;
			}
		}
		// If we have not returned then there are no passes available for redemption
		revert("Diamond and gold passes all redeemed");
	}

	function _validateDiamondPass(uint256 _amount, uint256 _supply)
		internal
		validateEarlyAccessPurchaseAmount(_amount, _supply)
		returns (bool)
	{
		...
		// Check if any diamond passes have not been redeemed yet
		for (uint256 i = 0; i < diamondTokens.length; i++) {
			uint256 currentId = diamondTokens[i];
			uint256 redeemed = redeemedDiamondPassMatrix[currentId];
			if (redeemed + _amount <= MAX_PURCHASE_PER_PASS) {
				redeemedDiamondPassMatrix[currentId] = redeemed + _amount;
				return true;
			}
		}
		// If we have not returned then there are no diamond passes available for redemption
		return false;
	}

	function _validateAnimalSupply(Animal _animal, uint256 _amount)
		internal
		view
		returns (uint256 supply)
	{
		supply = totalSupply(uint256(_animal));
		require(supply + _amount <= maxSupply, "Amount to buy exceeds max supply");
	}

	modifier validateEarlyAccessPurchaseAmount(uint256 _amount, uint256 _supply) {
		require(_supply + _amount <= maxEarlyAccessSupply, "Amount exceeds max early supply");
		_;
	}

	function withdraw(address _fengShuiQueen) public onlyOwner {
		uint256 balance = address(this).balance;
		if (balance > 0) {
			// Calculated in basis points
			uint256 cut = (balance * 1000) / 10000;
			// Require statement omitted as it can lock the contract withdrawals
			payable(_fengShuiQueen).call{ value: cut }("");
			// Send the rest to us
...
/// @author 0xJohn / @JCHerts
abstract contract AbstractAnimal is ERC1155 {
	enum Animal {
		SNAKE,
		MONKEY,
		DRAGON,
	...
	uint8 public constant TOTAL_ANIMALS = uint8(type(Animal).max);
	// token id to number minted matrix
	...
	function totalSupply(uint256 _id) public view virtual returns (uint256) {
		require(_id <= TOTAL_ANIMALS, "Invalid token id");
	...
	function exists(uint256 _id) public view virtual returns (bool) {
		return AbstractAnimal.totalSupply(_id) > 0;
	}
