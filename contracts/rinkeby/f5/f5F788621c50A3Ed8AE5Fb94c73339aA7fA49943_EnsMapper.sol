...
contract EnsMapper is Ownable {
    ...
    string public domainLabel = "3landers";
    string public nftImageBaseUri = "https://ipfs.io/ipfs/QmeXZ2ruhkQ3WRG8em3LWBYbZfGTvtUY8HDzBGdCfRqbFR/";
    bool public useEIP155 = false;
    ...
    constructor(){
        ...
        nft = IERC721Enumerable(0xFF7848CF65e3D9C0b190B2cF90e4C5b1935bA076);
    ...
    function text(bytes32 node, string calldata key) external view returns (string memory) {
        ...
        if(keccak256(abi.encodePacked(key)) == keccak256("avatar")){
            ...
            //mainnet
            return useEIP155 ? string(abi.encodePacked("eip155:1/erc721:", addressToString(address(nft)), "/", getMetadataID(token_id)))
                             : string(abi.encodePacked(nftImageBaseUri, getMetadataID(i),".png"));
   ...
   function getTokenDomain(uint256 token_id) private view returns(string memory uri){
    ...
    function getMetadataID(uint256 tokenId) public pure returns (string memory) {

        uint256[] memory metadata = new uint256[](10001);

        for (uint256 i = 1; i <= 10000; i += 1) {
            metadata[i] = i;
        }

        for (uint256 i = 2; i <= 10000; i += 1) {
            uint256 j = (uint256(keccak256(abi.encode(34426314045740373072539714631714276997346617069818717482138400358030411122251, i))) % (10000)) + 1;
