...
contract Vault is BaseContract
{
    ...
    function initialize() initializer public
    {
        ...
        maxReturn = 36000;
        period = 120; // DEV period is 2 minutes.
    ...
    function effectiveClaims(address player_) public view returns (uint256)
    {
        ...
        if((startTime[player_] == 0 || startTime[player_] > _start_) && _periodClaims_ < neutralClaims) {
    ...
    {
        uint256 _effectiveClaims_ = effectiveClaims(player_) + 1;
        if(_effectiveClaims_ > 28) _effectiveClaims_ == 28;
        return rates[_effectiveClaims_];
    }
