...
contract RewardVesting is Ownable, ERC1155Receiver, IRewardVesting {
    ...
    function _calculateAvailablePercentage(uint256 intialTimeStamp)
        private
        view
    ...
    function depositHeros(uint256 collectionId) external {
        require(
            !rewardsInfo[msg.sender].heroDeposited,
            "Hero Already Deposited"
        );
        if (rewardsInfo[msg.sender].isIntialized) {
            rewardsInfo[msg.sender].heroDeposited = true;
            rewardsInfo[msg.sender].heroId = collectionId;
    ...
    function withDrawHeros() external {
        require(rewardsInfo[msg.sender].heroDeposited, "Heros Not Deposited");
        rewardsInfo[msg.sender].heroDeposited = false;
        ...
        heroAddress.safeTransferFrom(
            address(this),
            msg.sender,
            rewardsInfo[msg.sender].heroId,
            1,
            ""
        );
        rewardsInfo[msg.sender].heroId = 0;
    }
