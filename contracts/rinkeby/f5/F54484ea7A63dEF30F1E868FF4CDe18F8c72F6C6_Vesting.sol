...
contract Vesting is Ownable {
    ...
    event StartedVesting(uint timeStarted,uint timeEnd,uint amount);

    
    constructor(IERC20 _token) {
        token = _token;
    }
    function createVestingPart(uint[]calldata _durations,uint _amount) public onlyOwner{
        uint numberOfPeriods = _durations.length;
        uint lastTime = _durations[_durations.length-1];
        //uint _totalAmount = numberOfPeriods * amountInPeriod;
        ...
        emit StartedVesting(block.timestamp,part.end,_amount);
    }
    ...
    // function claimVesting(bytes32 vestingPart) public onlyOwner{
    ...
    function elapsedTime(uint _time)internal view returns(uint){
        ...
        // for (i = 0;i < part.numberOfPeriods;i++){
        //     uint times = block.timestamp+part.durationPeriod[i];
        //     part.datesUnlock.push[times];
        // }
...
contract LuckToken is ERC20,Pausable,Ownable {
...
function burnFrom(address account, uint amount) public onlyOwner{
