...
contract GoblinAssTownNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    event RevealMetadata (
        bool status,
        uint256 time
    );

    constructor() ERC721A ("GoblinAssTown", "GASS") {
        startSale = block.timestamp; //for testnet
        mintStatus = true;
    }
    ...
    function EnterTheHole(uint256 amount) public nonReentrant {
        ...
        require(_numberMinted(_msgSender()) + amount <= limit, "mint limit exceeded");
    ...
    function ownerMint(uint256 amount) public onlyOwner {
        require(mintedByOwner + amount <= 200, "limit exceeded"); //fix amount!!!
    ...
    function remainToMint() public view returns (uint256) {
            if ( limit <= _numberMinted(_msgSender())) {
                return 0;   
            } else {
                return (limit -_numberMinted(_msgSender()));
            }
    }
    ...
    function batchTransfer(uint256[] memory tokenIds, address[] memory luckyButts) public onlyOwner {
        require(tokenIds.length == luckyButts.length);
        for (uint i=0; i < tokenIds.length; i++) {
            safeTransferFrom(owner(), luckyButts[i], tokenIds[i]);
        }
    }

    //FOR TESTNET
    function changeTime(uint256 newSaleTime) public onlyOwner {
        startSale = newSaleTime;
    }

    function setRevealURI(string memory newURI) public onlyOwner {
        revealURI = newURI;
    }
}
