...
contract Akamir is Ownable, ERC721A, ReentrancyGuard {
  ...
  function allowlistMint(uint256 quantity, uint256 lockUntil) external payable callerIsUser {
    ...
    uint256 startTime = uint256(saleConfig.allowlistSaleStartTIme);
    uint256 saleLimit = uint256(saleConfig.saleLimit);

    require(price != 0 && block.timestamp > startTime, "allowlist sale has not begun yet");
  ...
  function isPublicSaleOn(
    ...
    return publicPriceWei != 0 && publicSaleKey_ != 0 && saleLimit != 0 && block.timestamp >= publicSaleStartTime;
  }
