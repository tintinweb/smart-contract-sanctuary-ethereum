...
contract BURNER is Context, ERC721Burnable {
	...
	function mint(uint8 amount) public payable {
		...
		require((ERC721.balanceOf(_msgSender()) + amount) <= (_maxMintPerWallet), "More than max mints allowed for this addy!");
		// Shouldn't attempt to mint more tokens than the allowed by the contract.
		require((ERC721.balanceOf(_msgSender()) + amount) < (_maxCap - _tokenCounter), "This action will surpass the mint cap!");
