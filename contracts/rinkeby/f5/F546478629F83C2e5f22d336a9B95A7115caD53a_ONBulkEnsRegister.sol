...
contract ONBulkEnsRegister {
  ...
  constructor(address _controler) {
    _owner = msg.sender;
    _controllerAddress = _controler;
  }
  ...
  function makeCommitment(string memory name, bytes32 secret) public view returns (bytes32) {
    ETHRegistrarController controller = ETHRegistrarController(_controllerAddress);
    return controller.makeCommitment(name, msg.sender, secret);
  }

  function makeCommitments(string[] calldata names, bytes32 secret) public view returns (bytes32[] memory) {
    ETHRegistrarController controller = ETHRegistrarController(_controllerAddress);
    bytes32[] memory commitments = new bytes32[](names.length);
    for ( uint i = 0; i < names.length; ++i ) {
        commitments[i] = controller.makeCommitment(names[i], msg.sender, secret);
    }
    return commitments;
  }

  function commit(bytes32[] calldata commitments) external {
    require(commitments.length > 0, "Commitment list is empty");
    ETHRegistrarController controller = ETHRegistrarController(_controllerAddress);
  ...
  function register(string[] calldata names, uint256[] calldata durations, bytes32 secret) external payable returns(uint) { 
    require(names.length == durations.length, "Number of names and durations are not equal");
    ETHRegistrarController controller = ETHRegistrarController(_controllerAddress);
    uint total = msg.value;
    for( uint i = 0; i < names.length; ++i ) {
      if(!controller.available(names[i])){
        continue;
      }
  ...
  function renew(string[] calldata names, uint256[] calldata durations) external payable returns(uint) { 
    require(names.length == durations.length, "Number of names and durations are not equal");
    ETHRegistrarController controller = ETHRegistrarController(_controllerAddress);
    uint total = msg.value;
    for( uint i = 0; i < names.length; ++i ) {
  ...
  function multiNamePrices(string[] calldata names, uint256[] calldata durations, bool getAll) external view returns(uint[] memory) {
    require(names.length == durations.length, "Number of names and durations are not equal");
    ETHRegistrarController controller = ETHRegistrarController(_controllerAddress);
    uint[] memory prices = new uint[](names.length);
    for (uint i = 0; i < names.length; ++i) {
      bool available = getAll || controller.available(names[i]);
      if (available) {
