...
contract Deployed is ReentrancyGuard {
    function mint(uint256 _ammount) external payable nonReentrant {}
...
contract contractMint is ReentrancyGuard, IERC721Receiver, Ownable {
    ...
    function multiMint(uint256 times, uint256 _val) external payable onlyOwner nonReentrant {
        for(uint i=0;i<times;++i){
            _mint(_val);
        }
    }

    function _mint(uint256 _val) internal {
        dc.mint{value: 0}(_val);
    }
