    ...
    ERC2771ContextUpgradeable
{
    ...
    /// @inheritdoc IHomeFi
    address public override proposedAdmin;
    /// @inheritdoc IHomeFi
    uint256 public override adminProposalTimestamp;
    /// @inheritdoc IHomeFi
        ...
        nonZero(_tokenCurrency3)
    {
        ...
        // Always ensure that tokenCurrency is NOT a fee token, like SafeMoon.
    ...
    /// @inheritdoc IHomeFi
    function proposeAdminReplace(address _newAdmin)
        external
        override
        onlyAdmin
        nonZero(_newAdmin)
        noChange(proposedAdmin, _newAdmin)
    {
        // Propose admin replace
    ...
    function executeAdminReplace()
        external
        override
        nonZero(proposedAdmin)
        noChange(proposedAdmin, admin)
    {
        require(proposedAdmin == msg.sender, "HomeFi::!ProposedAdmin");
        require(
            block.timestamp >= adminProposalTimestamp + ONE_DAY,
            "HomeFi::too soon"
        );
        // Replace admin
        admin = proposedAdmin;
    ...
    /// @inheritdoc IHomeFi
    function replaceTrustedForwarder(address _newForwarder)
        external
        override
        onlyAdmin
        nonZero(_newForwarder)
        noChange(trustedForwarder, _newForwarder)
    {
        trustedForwarder = _newForwarder;

        emit TrustedForwarderReplaced(_newForwarder);
    }
        ...
        nonReentrant
    {
        ...
        _safeMint(_sender, _projectCount);
        _setTokenURI(_projectCount, string(_hash));

        // Update project related mappings
        projects[_projectCount] = _project;
        projectTokenId[_project] = _projectCount;

        emit ProjectAdded(_projectCount, _project, _sender, _currency, _hash);
    }
