    ...
    ERC2771ContextUpgradeable
{
    ...
    function checkAvailableNFT(uint256 _tokenID)
        public
        view
        returns (AVAILABLE_NFT memory)
    {
        if (_tokenID >= AVAILABLE_NFTS.length) {
            revert("NFT not available");
        }
        return AVAILABLE_NFTS[_tokenID];
    }

    function checkAvailableDiscount(uint256 _index)
        public
        view
        returns (AVAILABLE_DISCOUNT memory)
    {
        if (_index.add(1) >= AVAILABLE_NFTS.length) {
            revert("DISCOUNT not available");
        }
        return AVAILABLE_DISCOUNTS[_index];
    }
    ...
    function safeMint(address to, uint256 ID) public payable {
        ...
        if (birrasBalance > 0) {
            require(msg.value >= getAvailableDiscountForToken(ID, msg.sender), "Insufficient payment");
        } else {
            require(
