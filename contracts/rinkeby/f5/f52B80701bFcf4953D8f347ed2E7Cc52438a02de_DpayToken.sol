...
// File: contracts/DpayToken.sol

pragma solidity 0.6.12;

// DpayToken with Governance.
contract DpayToken is ERC20, AntiBotBlacklist {
    ...
    // Min amount to liquify. (default 10 Dpays)
    uint256 public minAmountToLiquify = 10 ether;
    // The swap router, modifiable. Will be changed to Dpay's router when our own AMM release
    IUniswapV2Router02 public DpayRouter;
    // The trading pair
    address public DpayPair;
    // In swap and liquify
    ...
    // To blocked smart contract functions
    ...
    event DpayRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "Dpay::antiWhale: Transfer amount exceeds the maxTransferAmount");
                require(swapEnabled == true, "Dpay::swap: Cannot transfer at the moment");
            }
        }
        _;
    }
    ...
    modifier antiBots(address _user0,address _user1) {
        require( this.blacklistCheck(_user0),"user0 is in decentralized blacklist");
        require( this.blacklistCheck(_user1),"user1 is in decentralized blacklist");
        _;
    }

    /**
     * @notice Constructs the DpayToken contract.
     */
    constructor() public ERC20("DpayG Token", "DpayG3") {
    ...
    /// @dev overrides transfer function to meet tokenomics of Dpay
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "Dpay::transfer: Burn value invalid");

            // default 92.5% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "Dpay::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the Dpay pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = DpayRouter.WETH();

        _approve(address(this), address(DpayRouter), tokenAmount);

        // make the swap
        DpayRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        DpayRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from DpayRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "Dpay::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "Dpay::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "Dpay::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateDpayRouter(address _router) public onlyOperator {
        DpayRouter = IUniswapV2Router02(_router);
        DpayPair = IUniswapV2Factory(DpayRouter.factory()).getPair(address(this), DpayRouter.WETH());
        require(DpayPair != address(0), "Dpay::updateDpayRouter: Invalid pair address.");
        emit DpayRouterUpdated(msg.sender, address(DpayRouter), DpayPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "Dpay::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "Dpay::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Dpay::delegateBySig: invalid nonce");
        require(now <= expiry, "Dpay::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "Dpay::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying Dpays (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "Dpay::_writeCheckpoint: block number exceeds 32 bits");
