...
██║ ╚═╝ ██║╚██████╔╝██║  ██║██║     ███████║
╚═╝     ╚═╝ ╚═════╝ ╚═╝  ╚═╝╚═╝     ╚══════╝
Contract forked from Morps contract
*/
...
contract Morps is ERC721A, Ownable, ReentrancyGuard {
  ...
  modifier publicMintCompliance(uint256 _mintAmount) {
    require(_mintAmount <= 1, "That amount is too high my fren.");
    require(totalSupply() + _mintAmount <= maxSupply, "Wow, think we sold out already.");
    _;
  }
  ...
  modifier mintPriceCompliance(uint256 _mintAmount) {
    require(msg.value >= cost * _mintAmount, "Morp Morp, you don't have enough money.");
    _;
  }
...
/_/ /_/ /_/_/_/ /_/\__/  
  */
  ...
  // Guaranteed list for 1 or 5 allowed mints (if holds Morps)
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    // Verify whitelist requirements
    require(whitelistMintEnabled, "Morp! You've got to wait for the whitelist sale to be open!");
    ...
    if(MerkleProof.verify(_merkleProof, merkleRootWl2, leaf)) mintLimit = 5;
  ...
  // Public sale - Max 1 per transaction and NOT per wallet
  function mint(uint256 _mintAmount) public payable publicMintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    require(publicSaleEnabled, "Come on, we are not in the public sale yet!");
    require((publicClaimed[msg.sender]+_mintAmount)<=1, "Uh oh! You have already claimed what you are allowed to!");
  ...
  // This function is used to airdrop all the Morps Genesis Pass holders their 5 allowed mints.
  function mintForAddresses(uint256 _mintAmount, address[] memory _receivers) public onlyOwner {
    require(_mintAmount > 0, "You've got to mint at least 1 Morp...");
