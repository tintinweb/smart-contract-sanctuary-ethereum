...
library LooksrareLibV2 {
    ...
    function _buyAsset(LooksrareBuy memory looksrareBuy, bool revertIfTrxFails) internal {
        ...
        if (success) {
            if (looksrareBuy.makerAsk.amount == 1) {  //ERC721
                IERC721(looksrareBuy.makerAsk.collection).transferFrom(address(this), msg.sender, looksrareBuy.makerAsk.tokenId);
            } else {  //ERC1155
                IERC1155(looksrareBuy.makerAsk.collection).safeTransferFrom(address(this), msg.sender, looksrareBuy.makerAsk.tokenId, looksrareBuy.makerAsk.amount, "");
            }
        }
    }

    // amounts are the ETH payment amounts sent to Looksrare contract
    function buyAssetsByData(bytes[] memory dataset, uint256[] memory amounts, address[] memory collections, uint256[] memory tokenIds) public {
        require(dataset.length == tokenIds.length, "LooksrareLib: array length not match");
        for (uint256 i = 0; i < dataset.length; ++i) {
            _buyAssetByData(dataset[i], amounts[i], collections[i], tokenIds[i]);
        }
    }

    function _buyAssetByData(bytes memory data, uint256 amount, address collection, uint256 tokenId) internal {
        ...
        require(success, "Looksrare.call data failed"); 
        IERC721(collection).transferFrom(address(this), msg.sender, tokenId);

            // if (makerAsk.amount == 1) {  //ERC721
            //     IERC721(collection).transferFrom(address(this), msg.sender, tokenId);
            // } else {  //ERC1155
            //     IERC1155(collection).safeTransferFrom(address(this), msg.sender, tokenId, makerAsk.amount, "");
            // }
    }
}
