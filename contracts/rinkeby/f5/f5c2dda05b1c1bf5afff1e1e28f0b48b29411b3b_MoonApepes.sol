...
contract MoonApepes is ERC721A, Ownable {
    ...
    function mint(uint256 quantity) external payable {
        ...
        if((_numberMinted(msg.sender) + quantity) <= MAX_FREE_MINT){
            
            price = 0 ether;
        }
        else if(_numberMinted(msg.sender) < MAX_FREE_MINT){
            price = mintRate * (quantity - (MAX_FREE_MINT - _numberMinted(msg.sender)));
        }
        else{
            price =  mintRate * quantity;
        }
