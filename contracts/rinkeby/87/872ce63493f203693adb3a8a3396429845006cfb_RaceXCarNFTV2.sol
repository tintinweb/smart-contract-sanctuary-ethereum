...
// File: contracts/ERC2981RaceXNFKEYRoyalties.sol
...
abstract contract ERC2981RaceXNFCarRoyalties is ERC165, IERC2981Royalties {
    ...
    {
        uint256 royalty = (value * _royaltiesValue) / 10000;
...
contract RaceXCarNFTV2 is ERC721Enumerable, ERC2981RaceXNFCarRoyalties, Ownable {
  using Strings for uint256;
  ...
  uint256 public cost = 0.0000002 ether;
  uint256 public maxSupply = 10000;
  uint256 public maxDevSupply = 2000;
  uint256 public devIssued = 0;
  uint256 public maxMintAmount = 20;
  uint256 public devRewards = 400;
  uint256 public royaltyValue = 700;  // 700 = 7%
  uint256 public devroyalty = 4; // split 4% to RaceX Team 3% reflection
  uint256 public reflectroyalty = 3; // split 4% to RaceX Team 3% reflection
  bool public paused = false;
  address private payments = 0xdf70419AD34A177Ee43Cb88eb42E0bC17d8D1aDb;
  address private royaltyContractAddress = 0xdf70419AD34A177Ee43Cb88eb42E0bC17d8D1aDb;
  ...
  constructor(
  ) ERC721("RaceX Series 1 Race Cars V2", "RaceXCarV2") {
    setBaseURI("https://test.ethercity.io/");
    _baseTokenURI = "";
    setRoyalties(payable(payments), royaltyValue);
    paused = false;
  }
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    require(msg.value >= ((cost*_mintAmount)/4)*3, "Please ensure the correct price is sent");
  ...
  function tokenMinter(uint256 tokenId) public view returns(address){
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721Enumerable, ERC2981RaceXNFCarRoyalties)
        returns (bool)
    {
        return
            ERC721.supportsInterface(interfaceId) ||
            ERC2981RaceXNFCarRoyalties.supportsInterface(interfaceId);
    }
...
//only owner
  ...
  function setmaxDevMint(uint256 _newmaxDevMint) public onlyOwner() {
    maxDevSupply = _newmaxDevMint;
  }
 ...
 function setPayments(address _payments) public onlyOwner {
  ...
  function withdraw() public payable onlyOwner {
    require(address(this).balance>20,"Balance must be greater than 20");
    require(payable(payments).send(address(this).balance-20));
  }

  function claimReflectionRewards() public {
    uint count = balanceOf(msg.sender);
    uint256 reflectReward = getReflectionBalances();
    for(uint i=0; i < count; i++){
        uint tokenId = tokenOfOwnerByIndex(msg.sender, i);
        reflectBalance[tokenId] = 0;
    }
    payable(msg.sender).transfer(reflectReward);
  }
  ...
  function splitReflections(uint256 amount) public onlyOwner{
      reflectDividend(amount);
  }

  function reflectDividend(uint256 amount) private {
    uint256 supply = _tokenIds.current();
    uint256 reflectAmt = amount/supply;
