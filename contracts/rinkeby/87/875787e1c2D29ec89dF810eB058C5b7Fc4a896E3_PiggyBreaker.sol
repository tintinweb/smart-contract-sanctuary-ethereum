...
contract PiggyBreaker is VRFConsumerBaseV2, IERC721Receiver {
    ...
    uint256 public remainingPiggies = 21;
    ...
    constructor(uint64 subscriptionId) VRFConsumerBaseV2(vrfCoordinator) {
      ...
      MetaPiggyFakeToken = IERC721(MPFAddress);
      for (uint16 prop = 0; prop < piggyIds.length; prop++) {
        ...
        } else if (prop >= 70 && prop <= 276) {
          rarity = Rarities.UNCOMMON;
        }
        piggies[piggyIds[prop]] = Piggy(false, rarity);
      }
    ...
    function initRarities() private {
      rarityPrizes[Rarities.COMMON][1] = 4000;
      rarityPrizes[Rarities.COMMON][3] = 1760;
      rarityPrizes[Rarities.COMMON][4] = 1200;
      rarityPrizes[Rarities.COMMON][7] = 960;
      rarityPrizes[Rarities.COMMON][15] = 800;
      rarityPrizes[Rarities.COMMON][30] = 720;
      rarityPrizes[Rarities.COMMON][40] = 640;

      rarityPrizes[Rarities.LEGENDARY][1] = 100000;
      rarityPrizes[Rarities.LEGENDARY][3] = 44000;
      rarityPrizes[Rarities.LEGENDARY][4] = 30000;
      rarityPrizes[Rarities.LEGENDARY][7] = 24000;
      rarityPrizes[Rarities.LEGENDARY][15] = 20000;
      rarityPrizes[Rarities.LEGENDARY][30] = 18000;
      rarityPrizes[Rarities.LEGENDARY][40] = 16000;
    }
    ...
    function fulfillRandomWords(
      ...
      uint256 rewardPercent = rarityPrizes[piggies[requestorForRequestId[requestId].piggyId].rarity][normalizer(randomWords[0] % 100 + 1)] / (remainingPiggies * 1000);
      if (rewardPercent > 1) {
        rewardPercent = 1;
      }

      // Send ETH to holder
      payable(requestorForRequestId[requestId].holder).transfer(address(this).balance * rewardPercent);

      // Check Additional rewards
      for (uint i = 0; i < nftRewardsAddresses.length; i++) {
        // uint randomValue = randomWords[i + 1] % remainingPiggies;
        uint randomValue = randomWords[i + 1] % 10;
        ...
        if (randomValue < nftRewards[rewardAddress].length) {
          ...
          ERC721(nftRewardsAddresses[i]).safeTransferFrom(address(this), requestorForRequestId[requestId].holder, tokenId);
          // Pop tokenId
    ...
    function breakPiggy(uint16 piggyId) public {
        require(piggyId > 554, "This NFT is not registered in the list of breakable piggies.");
