...
contract CityClashTowers is TowersNFT {
    ...
    event MergeEvent(uint256 tokenId1, uint256 tokenId2, address a);

    address cityClashContractAddress;

    //upgrades one token by burning the other one into it.
    function mergeTowers(uint tokenId1, uint tokenId2) public {
        require(_exists(tokenId1), "Tower Token must exist");
        require(_exists(tokenId2), "Tower Token must exist");
        require(ownerOf(tokenId1) == msg.sender, "You must own both Tower tokens");
        require(ownerOf(tokenId2) == msg.sender, "You must own both Tower tokens");

        uint tokenToBurn = 0;
        uint tokenToUpgrade = 0;
        if(tokenId1 < tokenId2) {
            tokenToBurn = tokenId2;
            tokenToUpgrade = tokenId1;
        } else {
            tokenToBurn = tokenId1;
            tokenToUpgrade = tokenId2;
        }
        idToNumStories[tokenToUpgrade] += idToNumStories[tokenToBurn];
        burn(tokenToBurn);

        emit MergeEvent(tokenId1, tokenId2, address(this));
    }
...
contract TowersNFT is ERC721, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721("City Clash Towers", "Build your towers to the sky!") {
    ...
    function mint(uint256 _tokenId) internal {
        idToNumStories[_tokenId] = _tokenId;
    ...
    function getImage(uint _numStories, uint _tokenId) private view returns (string memory) {
        ...
        if(tp.heightInPixels == tp.maxHeight) {
            uint amountPerWindow = (tp.heightInPixels - 100) / _numStories;
            tp.windowLength = amountPerWindow / (23 / uint(10));
            console.log(tp.windowLength);
        }

        tp.fullWindowWidth = (tp.windowLength * 62 / 100) * (tp.numWindows - 1) + (tp.windowLength * tp.numWindows);
        ...
        tp.whiteBackgroundHeight = 23 / uint(10) * tp.windowLength * _numStories;
    ...
    function getWindows(uint _numStories, TowerTypes.TowerParams memory _tp) private pure returns (string  memory str) {
        ...
        for(uint i = 0; i < _tp.numWindows - 1; i++) {
            uint stripLength = (_tp.windowLength * 62 / 100);
    ...
    function getAntennaOne(TowerTypes.TowerParams memory _tp) private pure returns (string memory str) {
        ...
        return string(
            abi.encodePacked('<rect x="', uintToByteString(_tp.buildingStartingX + _tp.fullWindowWidth - 10, 4),
                '" y="', uintToByteString(2000 + (_tp.fullWindowWidth / 12) - _tp.heightInPixels - heightOne - 20 - _tp.roofHeight, 4),
                '" width="', uintToByteString(_tp.windowLength / 5, 4),
    ...
    function getAntennaTwo(TowerTypes.TowerParams memory _tp) private pure returns (string memory str) {
        ...
        return string(
            abi.encodePacked(
                ...
                '" y="', uintToByteString(2000 + (_tp.fullWindowWidth / 15) - _tp.heightInPixels - heightTwo - 20 - _tp.roofHeight, 4),
                '" width="', uintToByteString(_tp.windowLength / 5, 4),
    ...
    function getSvg1(TowerTypes.TowerParams memory _tp) private pure returns (string memory str) {
        return string(
            abi.encodePacked(
                '<svg width="2000" height="2000" viewBox="0 0 2000 2000" fill="none" xmlns="http://www.w3.org/2000/svg">',
                    '<rect width="2000" height="2000" fill="#D0F9FC"/>',
                    '<rect y="1925" width="2000" height="2000" fill="', '#507a23',
                    '"/>',
    ...
    function getSvg3(TowerTypes.TowerParams memory _tp) private pure returns (string memory str) {
        return string(
            abi.encodePacked(
    ...
    function getSvg5(TowerTypes.TowerParams memory _tp) private pure returns (string memory str) {
        return string(
            abi.encodePacked(
                '.5H', uintToByteString(_tp.buildingStartingX + _tp.towerWidth, 4),
                'V353Z" fill="', _tp.colorForToken,
                '"/>'
                '<rect x="', uintToByteString(_tp.buildingStartingX - (_tp.windowLength * 6 + _tp.windowLength / uint(4)), 4),
    ...
    function getSvg6(TowerTypes.TowerParams memory _tp) private pure returns (string memory str) {
        return string(
            abi.encodePacked(
                '" height="', uintToByteString(3 * _tp.windowLength, 4),
                '" fill="#460509" fill-opacity="0.92"/>',
                '<circle cx="', uintToByteString(_tp.buildingStartingX - (_tp.windowLength * 6) + (uint(13) / uint(10) * _tp.windowLength / 3), 4),
    ...
    function getSvg7(TowerTypes.TowerParams memory _tp) private pure returns (string memory str) {
        return string(
            abi.encodePacked(
                '<ellipse cx="', uintToByteString(1450 + 16 / uint(10) * _tp.windowLength * 2 - 10, 4),
                '" cy="150" rx="', uintToByteString(3 * _tp.windowLength, 4),
                '" ry="', uintToByteString(2 * _tp.windowLength, 4),
                '" fill="#FDFAFA"/>',
                '<ellipse cx="1450" cy="', uintToByteString(157 + 13 / uint(10) * _tp.windowLength * 2 - 14 / uint(10) * _tp.windowLength, 4),
    ...
    function getSvg8(TowerTypes.TowerParams memory _tp) private pure returns (string memory str) {
        return string(
            abi.encodePacked(
                '<ellipse cx="', uintToByteString(1450 + 2 * _tp.windowLength * 2 - 10, 4),
    ...
    function getTowerColor(uint _tokenId) private pure returns (string memory str) {
    ...
    function min(uint _num1,  uint _num2) private pure returns (uint minNum) {
        if(_num1 < _num2) {
            return _num1;
        } else {
            return _num2;
