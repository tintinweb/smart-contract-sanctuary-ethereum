...
import "./CryptoCollectableBase.sol";

contract Yachts is CryptoCollectableBase {

    constructor() CryptoCollectableBase("Yachts", "YACHT", 0.5 ether, 2000, "https://yachts.blob.core.windows.net/meta/"){
    ...
    function mintBatch(uint256 numberOfItems)
        external
        payable
        mintingAllowed
        nonReentrant
    {
        require(availableSupply() - numberOfItems >= 0, "Out of supply!");
        require(
            msg.value >= mintPrice * numberOfItems,
    ...
    function reserve(uint256 numberOfItems) external onlyOwner {
        require(
            getReservedSupply() >= numberOfItems,
...
contract CryptoCollectableBase is
    ERC721,
    ERC721Enumerable,
    ERC721URIStorage,
    ReentrancyGuard, 
    Ownable
{
    ...
    //all funds redrawall function
    function withdrawSomeFunds(uint256 amount) external onlyOwner {
        uint256 balance = address(this).balance;
    ...
    ) internal virtual mintingAllowed {
        require(
            msg.value >= mintPrice,
            "Sent value is insufficient for minting token!"
        );
    ...
    ) internal virtual mintingAllowed nonReentrant {
        // require(
    ...
    function setMintBatchLimit(uint16 _mintBatchLimit) public onlyOwner {
    ...
    function setInitialSupply(uint256 _initialSupply) public onlyOwner {
        require(
            totalSupply() == 0,
            "Initial token supply can only be set if there is no minted tokens (totalSupply must be 0)!"
        );
        require(
            _initialSupply > 0,
            "Initial token supply must be greater then 0!"
        );
    ...
    function getReservedSupply() public view onlyOwner returns (uint256) {
    ...
    //set reserved supply
    function setReservedSupply(uint256 _reservedSupply) public onlyOwner {
        require(
            _reservedSupply < initialSupply,
            "Reserved supply must be less then initial supply!"
        );
        require(
            _reservedSupply <= availableSupply(),
            "Reserved supply must be less then available supply!"
        );
...
//31.08.2021. 22:49: ovo je ispravan template za kreiranje nft collectable, testirano i prikazuje se na OpenSea
