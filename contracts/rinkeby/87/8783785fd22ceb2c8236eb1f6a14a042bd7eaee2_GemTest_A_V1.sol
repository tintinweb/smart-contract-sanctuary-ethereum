...
// SPDX-License-Identifier: MIT// File: GemTest_A_V1_flat.sol
...
// File: contracts/GemTest_A_V1.sol
...
contract GemTest_A_V1 is Ownable, ERC721A, ReentrancyGuard {
    uint256 public tokenCount;
    uint256 public wlMintPrice = 0.003 ether;
    uint256 public psMintPrice = 0.005 ether;
    uint256 public psMintLimit = 2;
    uint256 public wlMintLimit = 2;
    uint256 public _totalSupply = 3699;
    uint256 public currentSupply= 368;
    bool public wlSaleStart = false;
    bool public psSaleStart = false;
  ...
  constructor(
  ) ERC721A("GemTest_A_V1", "GTA1", psMintLimit, _totalSupply) {
     tokenCount = 0;
  }
  ...
  function psMint(uint256 quantity) public payable nonReentrant {
    ...
    require(msg.value == psMintPrice * quantity, "Value sent is not correct");
    require((quantity + tokenCount) <= (currentSupply), "Sorry. No more NFTs");
    require(psSaleStart, "Sale Paused");
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    require(_exists(tokenId), "No metadata.");
  ...
  function setMintLimit(address[] memory list)public virtual onlyOwner{
    for (uint i = 0; i < list.length; i++) {
      wlMinted[list[i]]=0;
      psMinted[list[i]]=0;
    }
  ...
  function setWlSale(bool _state) external onlyOwner {
      wlSaleStart = _state;
  }
  function setPsSale(bool _state) external onlyOwner {
      psSaleStart = _state;
  }
  function setPermitInit(bool _state) external onlyOwner {
      permitInit = _state;
  }
  ...
  function setPsLimit(uint256 newLimit) external onlyOwner {
