...
contract Staking is Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) _collectionPermits;
    ...
    function deposit(address[] memory collections, uint256[] memory tokenIds) external {
        ...
        for(uint256 i; i < collections.length; i++) {
            if(!_collectionPermits[collections[i]])
                revert("Deposit: collection is not permitted");
    ...
    function _calculateReward(address account, uint256 index) internal view returns (uint256) {
        uint256 tokenCount = _stakes[account].tokens.length % 21;
        uint256 bonus;

        if(index == 1) {
            bonus = _stakes[account].deposit * (_tokenMultiplier * tokenCount) / _denominator;
        } else if(index > 1 && index < 4) {
            bonus = _stakes[account].deposit * (_tokenMultiplier * tokenCount + 25) / _denominator;
        } else if(index > 3 && index < 7) {
            bonus = _stakes[account].deposit * (_tokenMultiplier * tokenCount + 50) / _denominator;
        } else if(index > 6 && index < 13) {
            bonus = _stakes[account].deposit * (_tokenMultiplier * tokenCount + 75) / _denominator;
        } else if(index > 12) {
            bonus = _stakes[account].deposit * (_tokenMultiplier * tokenCount + 100) / _denominator;
        }
    ...
    function permitCollection(address collection, bool approval) external onlyOwner {
        _collectionPermits[collection] = approval;
    }

    function setTokenMultiplier(uint256 value) external onlyOwner {
        _tokenMultiplier = value;
    }
