 ...
 */
contract veTokenSeed is Ownable {
    ...
    */
    ...
    function updateStartTime(uint64 startTimestamp) external onlyOwner {
        require(block.timestamp < startTimestamp, "Start Time should be bigger than the current time");
        _start = startTimestamp;
    }
    ...
    function tokenAllocation() public view onlyBeneficiary returns (uint256) {
        return IERC20(_token).balanceOf(address(this)) + _released;
    }

    function tokenVested() public view onlyBeneficiary returns (uint256) {
        return vestedAmount(uint64(block.timestamp));
    }

    function vestRemaining() public view onlyBeneficiary returns (uint256) {
        return tokenAllocation() - tokenVested();
    }

    function tokenReleased() public view onlyBeneficiary returns (uint256) {
        return _released;
    }

    function releaseRemaining() public view onlyBeneficiary returns (uint256) {
