...
//        require(msg.sender == ens.ownerOf(hashKey),  "You are not the owner of this ENS domain.");
    ...
    function getMetadataByDomain(string memory domainName) external view returns (string memory) {
        require(getPurchaseStatus(domainName) == NFTPurchaseStatus.SONGIFIED || purchaseStatus[domainName] == NFTPurchaseStatus.REDEEMED,
            "The domain hasn't been purchased yet.");
    ...
    function getPurchaseStatus(string memory domainName) internal view returns (NFTPurchaseStatus) {
