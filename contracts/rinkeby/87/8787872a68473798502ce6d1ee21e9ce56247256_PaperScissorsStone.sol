...
contract PaperScissorsStone {
    ...
    modifier roomHolding(){
        require(
            room[msg.sender].status == Status.Holder, 
            "You are not the holder of the room or the game has started"
        );
        _;
    }

    function LeaveGame() public existRoom(msg.sender) roomHolding{
        uint256 bet = room[msg.sender].bet;
        reset(msg.sender);
        payable(msg.sender).transfer(bet); 
        Status status = room[msg.sender].status;
        RemoveHolder((msg.sender));
        emit Create_Eliminate_Room_EVENT(msg.sender, bet, status);
    }
