...
contract DGENFRENDS is ERC721A, IERC2981, Ownable, ReentrancyGuard {
    ...
    modifier maxTokensPerPublicSaleMint(uint256 quantity) {
        require(
            quantity <= MAX_PUBLIC_SALE_MINTS,
            "Exceeds public mint max number"
        );
        _;
    }

    modifier maxTokensPerPreSaleMint(uint256 quantity) {
        require(
            preSaleMintCounts[msg.sender] + quantity <= MAX_PRE_SALE_MINTS,
    ...
    modifier canMint(uint256 quantity) {
        require(
            _tokenCounter.current() + quantity <=
    ...
    modifier canReserveTokens(uint256 quantity) {
        require(
            numReservedTokens + quantity <= MAX_RESERVE_TOKENS,
            "Insufficient token reserve"
        );
        require(
            _tokenCounter.current() + quantity <= MAX_TOTAL_SUPPLY,
            "Insufficient tokens remaining"
        );
        _;
    }
    ...
    function mintPublicSale(uint256 quantity)
        ...
        canMint(quantity)
        maxTokensPerPublicSaleMint(quantity)
    {
        for (uint256 i = 0; i < quantity; i++) {
            _safeMint(msg.sender, nextTokenId());
        }

        emit MintPublicSale(msg.sender, quantity);
    }
    ...
    function mintPreSale(uint256 quantity, bytes32[] calldata merkleProof)
        ...
        maxTokensPerPreSaleMint(quantity)
    {
        preSaleMintCounts[msg.sender] = preSaleMintCounts[msg.sender] + quantity;

        for (uint256 i = 0; i < quantity; i++) {
            _safeMint(msg.sender, nextTokenId());
        }

        emit MintPreSale(msg.sender, quantity);
    }
    ...
    function reserveTokens(uint256 quantity)
        ...
        nonReentrant
        onlyOwner
        canReserveTokens(quantity)
    {
        numReservedTokens += quantity;

        for (uint256 i = 0; i < quantity; i++) {
            _safeMint(msg.sender, nextTokenId());
        }

        emit ReserveTokens(quantity);
    }
