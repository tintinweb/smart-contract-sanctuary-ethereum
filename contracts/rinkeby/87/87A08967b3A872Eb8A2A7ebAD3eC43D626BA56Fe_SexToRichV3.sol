...
// File: contracts/SexToRichException.sol


pragma solidity ^0.8.10;


interface SexToRichException is IERC721A {
...
/// @title ERC721 for SexToRich
/// @author @CryptoPhang

contract SexToRichV3 is SexToRichException, Ownable, ERC721A {
    ...
    SaleStates public saleState = SaleStates.CLOSED;
    ...
    uint256 public firstCost = 0.00 ether;
    ...
    constructor(address receiver) ERC721A("SexToRich", "Sex") {
        _mintERC2309(receiver, RESERVED_TOKENS);
    }

    /// @notice Function `PRESALE` sale state.
        ...
        checkState(SaleStates.PRESALE)
    {
       ...
       bool isFirstMint = _numberMinted(msg.sender) == 0;
       bool isWhitelisted = whitelistClaim[msg.sender];
        
       uint256 cost = presaleCost * quantity;
       if (isFirstMint){
           cost = presaleCost * (quantity - 1);
       }

       if (!isWhitelisted) revert("ERROR: You are not whitelisted");
    ...
    /// @notice Function `PUBLIC` sale state.
    ...
    /// @notice Function used to set presale members.
    function setPresaleMembers(
        address[] calldata members
    ) external onlyOwner {
        
        if (members.length > preSaleSupply) revert("ERROR: whitelist members more than presale supply");
        address member;
    ...
    /// @notice Function used to set a new `presaleSupply` value.
    /// @param amount Newly intended `presaleSupply` value.
    function setPresaleSupply(uint256 amount) external onlyOwner {
        if (amount > 2000) revert InvalidTokenCap();
