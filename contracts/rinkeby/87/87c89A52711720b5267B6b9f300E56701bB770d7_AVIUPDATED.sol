...
contract AVIUPDATED is IERC721, Ownable, Functional {
    ...
    mapping(uint256 => bool) private excludedToken;
    ...
    constructor() {
        _name = "AVIUPDATED";
        _symbol = "AUNFT";
        _baseURI = "https://localhost:3000/";
       
        CC = ERC721(0x45eeEd78125386e49dc80c5640d8eACf1Ef4ca89); // AVA Burn token on etherscan
        _maxSupply = 3333;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == AVIUPDATED.onERC721Received.selector;
    }
    ...
    function airDrop(address _to, uint256 qty, uint256 _id1, uint256 _id2) external onlyOwner {
        ...
        require(!(excludedToken[_id1]), "Token must be excluded");
        require(!(excludedToken[_id2]), "Token must be excluded");
        require((_mintTracker[_to] + qty) <= maxPerWallet, "Mint: Max tkn per wallet exceeded");
        require(_id1 > 0 && _id2 > 0 && _id1 <=3333 && _id2 <=3333, "Not a valid token ID");
        require(qty ==1, "Pease mint foronly 1 qunatity");
        uint256 mintSeedValue = numberMinted;
        _mintTracker[_to] = _mintTracker[_to] + qty;
        excludedToken[_id1] = true;
        excludedToken[_id2] = true;
       // for(uint256 i = 1; i <= qty; i++) {
            _safeMint(_to, mintSeedValue + qty);
            numberMinted ++;  //reservedTokens can be reset, numberMinted can not
       // }
    }

    function excludeAnimaToken(uint256 _id1) public view returns(bool )
    {
        return (excludedToken[_id1]);
    }
	function publicMint(uint256 qty, uint256 _id1, uint256 _id2) external payable reentryLock {
		require(publicMintActive, "Mint Not Active");
        require(_msgSender() == (CC).ownerOf(_id1), "Only token owner can call burn");
        require(_msgSender() == (CC).ownerOf(_id2), "Only token owner can call burn");
        ...
        require(!(excludedToken[_id2]), "Anima Token is already excluded");
        require(!(excludedToken[_id1]), "Anima Token is already excluded");
        require(_id1 > 0 && _id2 > 0 && _id1 <=3333 && _id2 <=3333, "Not a valid token ID");
        require(qty ==1, "Pease mint foronly 1 qunatity");
        excludedToken[_id1] = true;
        excludedToken[_id2] = true;
		...
		//for(uint256 i = 1; i <= qty; i++) {
        	_safeMint(_msgSender(), mintSeedValue + qty);
       // }
	}
    function whiteListMint(uint256 qty, uint256 _id1, uint256 _id2) external payable reentryLock {
		require(CC.balanceOf(_msgSender()) >= 2, "Must have Avius Token");
        ...
        require(!(excludedToken[_id1]), "Anima Token is already excluded");
        require(_id1 > 0 && _id2 > 0 && _id1 <=3333 && _id2 <=3333, "Not a valid token ID");
        require(qty ==1, "Please mint for only 1 qunatity");
        excludedToken[_id1] = true;
        excludedToken[_id2] = true;
    	nftprice = msg.value;
		uint256 mintSeedValue = numberMinted;
    ...
    function burn(uint256 tokenID, uint256 tokenIDTwo) external {
        ...
        excludedToken[tokenID] = true;
        excludedToken[tokenIDTwo] = true;
    }
