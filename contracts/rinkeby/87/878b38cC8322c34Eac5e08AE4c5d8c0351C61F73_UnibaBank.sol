...
contract UnibaBank {
    ...
    IERC20 public immutable unibaToken;
    ...
    uint8 private constant PRECISION_FACTOR_MULT = 10;
    uint8 private constant ALTCOIN_MULT = 3;
    uint8 private constant STABLE_MULT = 1;
    uint8 private constant UNIBA_MULT = 6;

    mapping(address => UserInfo) public userInfo; // to change later in private ?
    ...
    */
    function depositETH() external payable {
        ...
        // first deposit so we save the block when the first deposit happens
        if(userInfo[msg.sender].lastBlockClaimedRewards == 0) userInfo[msg.sender].lastBlockClaimedRewards = block.number;
    ...
    */
    function withdrawETH() external {
        uint256 amount = userInfo[msg.sender].ethDeposited;
        userInfo[msg.sender].ethDeposited = 0;
        totalEthDeposited -= amount;

        (bool success, ) = msg.sender.call{value:amount}("");
        require(success);

        // claim tokens when whitdrawing ???
        // update shares
    ...
    function withdraw(address token) public {
        uint256 amount = userInfo[msg.sender].tokenDeposited[token];
        userInfo[msg.sender].tokenDeposited[token] = 0;
        totalTokenDeposited[token] -= amount;
