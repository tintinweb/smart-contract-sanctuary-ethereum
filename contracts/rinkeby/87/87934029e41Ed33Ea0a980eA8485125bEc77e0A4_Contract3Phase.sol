...
contract Contract3Phase is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public WL_PRICE = 0.0001 ether;
    uint256 public MAX_PER_TX = 20;
    uint256 public MAX_PER_FREE = 1;
    uint256 public MAX_WL_SUPPLY = 4000;
    uint256 public MAX_SUPPLY = 10000;
    uint256 private WL_MINT_COUNT = 0;

    bool public paused = true;
    bool public revealed = true;

    constructor() ERC721A("Contract3Phase", "C3P") {
    ...
    function publicMint(uint256 _quantity) external payable {
        ...
        require(
            _quantity > 0 && _quantity <= MAX_PER_TX,
            "Invalid mint amount!"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= _quantity * SALE_PRICE,
        ...
        payable
    {
        ...
        if (msg.sender != owner()) {
            if (!whitelistClaimed[msg.sender]) {
                require(
                    msg.value >= _quantity * WL_PRICE - WL_PRICE * MAX_PER_FREE,
                    "Please send the exact amount."
                );
                whitelistClaimed[_msgSender()] = true;
            } else {
                require(
                    msg.value >= _quantity * WL_PRICE,
                    "Please send the exact amount."
                );
            }
        }
        WL_MINT_COUNT = WL_MINT_COUNT + 1;
        _safeMint(msg.sender, _quantity);
    }
