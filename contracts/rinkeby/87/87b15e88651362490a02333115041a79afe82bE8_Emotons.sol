...
contract Emotons is ERC1155, Ownable {
    ...
    mapping(uint256 => uint256) public vouchersTracker;
    ...
    function setCurrentPhase(bytes32 phase) external onlyOwner {
        _phase = phase;
    }

    function setProvenance(uint256 provenance) external onlyOwner {
    ...
    function mintPrivate(
        ...
        require(
            amounts.length == recipientLength,
        ...
        for (uint256 i = 0; i < recipientLength; ) {
            require(
                ...
                "Requested amount exceeds supply"
            );
    ...
    function mintPreSale(bytes32 leaf, bytes32[] calldata proof)
        ...
        payable
    {
        require(_phase == PRE_SALE, "Pre-sale not open");
        ...
        require(account == msg.sender, "You are not the owner of the leaf");
        uint256 cluster;
        uint256 bitMask;
        ...
        unchecked {
            cluster = index >> 8;
            bitMask = 1 << (index % 256);
            voucherBitmap = vouchersTracker[cluster];
        ...
        vouchersTracker[cluster] = voucherBitmap | bitMask;
    }
    ...
    function merkleProof1(bytes32 leaf, bytes32[] calldata proof)
        public
        payable
    {
        MerkleProof.verify(proof, _root, keccak256(abi.encodePacked(leaf)));
    ...
    function merkleProof2(bytes32 leaf, bytes32[] calldata proof)
        public
        payable
    {
        MerkleProof.verify(proof, _root, keccak256(abi.encodePacked(leaf)));
    ...
    function mintSale(
        ...
        require(_phase == SALE, "Sale not open");

        require(
            _currentTokenId + amount <= _supply,
            "Requested amount exceeds supply"
        );
        require(
            MerkleProof.verify(proof, _root, keccak256(abi.encodePacked(leaf))),
    ...
    function mintPublic(uint256 amount) public payable {
        require(_phase == PUBLIC, "Public sale not open");
        require(
            _currentTokenId + amount <= _supply,
            "Requested amount exceeds supply"
        );
