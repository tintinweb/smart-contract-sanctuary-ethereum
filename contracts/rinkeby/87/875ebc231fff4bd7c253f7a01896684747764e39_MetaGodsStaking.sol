...
contract MetaGodsStaking is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function stakeTokens(StakeSelection[] calldata selections_) external whenNotPaused {
        ...
        for (uint256 i = 0; i < selections_.length; ++i) {
            require(Trusted721Collection(selections_[i].collectionAddress).isOwnerOfBatch(selections_[i].tokens, msg.sender));

            if (selections_[i].collectionAddress != contractsAddressesMap["god"]) {
            ...
            for (uint256 j = 0; j < selections_[i].tokens.length; ++j) {
                ...
                if (selections_[i].pairedTokens.length > 0) {
                    ...
                    address pairedCollectionAddress = pairedContractsAddressesMap[selections_[i].collectionAddress];

                    require(isOwner(msg.sender, pairedCollectionAddress, selections_[i].pairedTokens[j]));

                    require(!stakedTokensData[pairedCollectionAddress][selections_[i].pairedTokens[j]].isPaired);

                    if (block.timestamp > stakedTokensData[pairedCollectionAddress][selections_[i].pairedTokens[j]].claimDate) {
                        amount += _getYieldAndUpdateDate(pairedCollectionAddress, selections_[i].pairedTokens[j]);
                    }
                ...
                Trusted721Collection(selections_[i].collectionAddress).safeTransferFrom(msg.sender, address(this), selections_[i].tokens[j]);
            }
        }
    ...
    function changeTokenLinks(CollectionSelection[] calldata selections_) external whenNotPaused {
        ...
        for (uint256 i = 0; i < selections_.length; ++i) {
            ...
            address pairedContractAddress = pairedContractsAddressesMap[collectionAddress];
            ...
            for (uint256 j = 0; j < selections_[i].tokens.length; ++j) {
                ...
                if (selections_[i].pairedTokens.length > 0) {
                    ...
                    require(isOwner(msg.sender, pairedContractAddress, pairedTokenId));

                    if (stakedTokensData[pairedContractAddress][pairedTokenId].isPaired) {

                        totalAccumulatedYield += _claimAndUnpair(
                            collectionAddress,
                            stakedTokensData[pairedContractAddress][pairedTokenId].pairedTokenId
                        );
                    }

                    totalAccumulatedYield += _getYieldAndUpdateDate(pairedContractAddress, pairedTokenId);
