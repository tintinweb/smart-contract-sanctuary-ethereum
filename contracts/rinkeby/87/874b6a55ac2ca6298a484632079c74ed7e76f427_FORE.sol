...
contract FORE is ERC20("FORE", "FORE"), Ownable {
    ...
    constructor(address sagcAddress, uint256 _baseRate) {
        iSagc = ISagc(sagcAddress);
    ...
    function setContractSettings(uint256 _baseRate, uint256 _start, uint256 _end) public onlyOwner(){
        require(!isLocked, "Cannot modify end dates after lock");
        contractSettings.baseRate = _baseRate  * 10 ** 18;
        contractSettings.start = _start;
    ...
    function claimReward(uint256 _tokenId) public returns (uint256) {
        ...
        require(iSagc.ownerOf(_tokenId) == msg.sender, "Caller does not own the token being claimed for.");
