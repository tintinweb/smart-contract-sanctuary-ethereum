...
contract NautilusNFT is ERC721Enumerable, Ownable, ReentrancyGuard, Pausable{
    ...
    function mint() public payable nonReentrant whenNotPaused{
        ...
        if(presaleStart && !saleStart){
            ...
            if(saleStart && block.timestamp  <= publicSaleStartTime + (publicSaleDuration * 1 hours) ){

                // Public sale has started anbd presale is finished

                if(msg.value == salePrice){
                    _mintToken(msg.sender, msg.value, false);
                }else{
                    revert("Incorrect amount for public sale");
                }

            }else{
                revert("Public sale has not started or is finished");
            }
