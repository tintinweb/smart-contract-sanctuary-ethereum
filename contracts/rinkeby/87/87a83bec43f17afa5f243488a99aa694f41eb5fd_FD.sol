...
contract FD is ERC721A, Ownable, ReentrancyGuard, ERC2981, IERC1155Receiver {
    ...
    uint256 public tokenPrice = .01 ether;
    ...
    address royaltyPayout = 0xB090fc5D0C06804dfbf06efb59Ce1F291f13162a;

    event TokenReceived(address _operator, address _from, uint256 _id, uint256 _value);
    event Increment(address _key, uint256 _val);
    ...
    function onERC1155Received(
        ...
        emit TokenReceived(_operator, _from, _id, _value);
        if(_id == passId){
    ...
    function incrementPass(address _addr, uint256 _val) public {
        passesReceived[_addr] += _val;
        emit Increment(_addr, _val);
    }
    ...
    function validatePass(address _addr, uint256 _val) external onlyOwner{
        ...
        passesToMint[_addr] += 1;
        if(passesReceived[_addr] > 0){
            passesReceived[_addr] -= 1;
        }
    }
}