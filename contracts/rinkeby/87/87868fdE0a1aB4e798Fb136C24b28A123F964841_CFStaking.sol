...
import "./library/BaseRelayRecipient.sol";

contract CFStaking is ReentrancyGuard, BaseRelayRecipient {
    ...
    address public coinGameAddress;
    uint256 private minStakeAmt;
    uint256 private feePercentage;
    uint256 private whiteListPoolSize;

    IERC20 public CFToken;

    constructor(address _coinAddress, address _trustedForwarder) {
        CFToken = IERC20(_coinAddress);
    ...
    function updateOwnerAddress(address ownerAddr) public onlyOwner {
    ...
    function initiateGame(
        ...
        if (_cfToken == true) {
            if (whiteListedCreators[_gameOwner] != Status.WhiteListed) {
                StakeInfo storage currentInfo = stakingInfo[_gameOwner];
                require(
                    currentInfo.activeAmt >= _lockAmount,
                    "Insufficient credit in the address"
                );
                currentInfo.activeAmt -= _lockAmount;
            }
        } else {
    ...
    function stakeEnjin(uint256 _amount) public nonReentrant {
        require(
            CFToken.allowance(_msgSender(), address(this)) >= _amount,
            "Allowance amount is insufficient"
        );
        require(
            CFToken.balanceOf(_msgSender()) >= _amount,
            "Insufficient token balance in the address"
        );
        CFToken.safeTransferFrom(_msgSender(), address(this), _amount);
        ...
        onlyCoinGameContract
    {
        ...
        require(
            ...
            "User does not have enough Enjin token"
        );
        _currentInfo.activeAmt -= _currentGame.entryAmt;
    ...
    function distributeReward(
        ...
        uint256 rewardOwner = (poolSize / 100) * (20 - feePercentage);
        for (uint256 index = 0; index < _winners.length; index++) {
            ...
            poolSize -= _winnerWeights[index];
            CFToken.safeTransferFrom(address(this), _winners[index], _winnerWeights[index]);
        }
        if (
            whiteListedCreators[_currentGame.gameOwner] != Status.WhiteListed &&
            _cfToken == true
        ) {
            require(poolSize >= rewardOwner, "Insufficient reward amount");
            CFToken.safeTransferFrom(address(this), _currentGame.gameOwner, rewardOwner);
        }
    }
    ...
    function claimUnstakedEnjin() public nonReentrant {
        ...
        CFToken.safeTransferFrom(address(this), _msgSender(), claimableAmt);
    }
    ...
    function claimAll() public nonReentrant onlyOwner {
        CFToken.safeTransferFrom(
            address(this),
            _msgSender(),
            CFToken.balanceOf(address(this))
        );
    }
}
