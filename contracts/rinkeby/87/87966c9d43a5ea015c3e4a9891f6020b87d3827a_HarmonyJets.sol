...
contract HarmonyJets is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC721BurnableUpgradeable, EIP712Upgradeable, ERC721VotesUpgradeable {
    /**
     * USINGS
     */
    ...
    function initialize() initializer public {
        __ERC721_init("Harmony Jets", "9H-DFS");
        __ERC721Enumerable_init();
        __ERC721URIStorage_init();
        __Pausable_init();
        __Ownable_init();
        __ERC721Burnable_init();
        __EIP712_init("Harmony Jets", "1");
        __ERC721Votes_init();

        _tokenIdCounter.increment();

        setBaseURI("https://nft.harmony-jets.com/");

        setRoyaltyBps(1000);
        setRoyaltyRecipient(payable(msg.sender));

        setMaxTokensPerTransaction(50);
        setSetTokenPrice(175000000000000000);

        setTotalTokens(2000);
        setTotalTokensBuyable(2000);
    ...
    */
    ...
    function setSetTokenPrice(uint256 amount) public onlyOwner {
        _tokenPrice = amount;
     ...
     * Buy token
     */
    function buy(uint256 amount) whenNotPaused public payable {
        require(amount > 0, "HarmonyJets: Token number must be higher than 0");
        require(amount <= _maxTokensPerTransaction, "HarmonyJets: Max tokens per transaction number exceeded");
        require(_tokenIdCounter.current().add(amount) <= _totalTokensBuyable + 1, "HarmonyJets: Tokens number to mint exceeds number of buyable tokens");
        require(_tokenPrice.mul(amount) <= msg.value, "HarmonyJets: Ether value sent is too low");
    ...
    function directMint(address to, uint256 tokenId) public onlyOwner {
        require(tokenId > _totalTokensBuyable, "HarmonyJets: Tokens number to mint must exceed number of buyable tokens");
        _safeMint(to, tokenId);
    }
    ...
    function setTotalTokens(uint256 amount) public onlyOwner {
        require(amount >= _totalTokensBuyable, "HarmonyJets: Total tokens must be equal or higher than buyable tokens");
        _totalTokens = amount;
    ...
    function setTotalTokensBuyable(uint256 amount) public onlyOwner {
        require(amount <= _totalTokens, "HarmonyJets: Buyable tokens must be equal or less than total tokens");
