...
contract ChiptosX is ERC721A, Ownable {
    ...
    bool public isChipListActive = false;
    uint256 public constant MAX_SUPPLY = 7680;
    uint256 public constant MAX_PUBLIC_MINT = 5;
    uint256 public constant PRICE_PER_TOKEN = .088 ether;
    ...
    constructor() ERC721A("ChiptosX", "Chipto") {
    }
    ...
    function setChipHolderFreeList(address[] calldata addresses, uint8 numHolderFreeAllowedToMint) external onlyOwner callerIsUser{
        for (uint256 i = 0; i < addresses.length; i++) {
            _chipHolderFreeList[addresses[i]] = numHolderFreeAllowedToMint;
        }
    }
    function setChipHolderPreList(address[] calldata addresses, uint8 numAllowedToMint) external onlyOwner callerIsUser{
        for (uint256 i = 0; i < addresses.length; i++) {
...
// Holder Free
    function mintHolderFreeList(uint8 numberOfTokens) external payable callerIsUser{
        uint256 ts = totalSupply();
        require(isChipListActive, "Chiplist is not active");
...
//HolderPresale
    function mintHolderPreList(uint8 numberOfTokens) external payable callerIsUser{
        uint256 ts = totalSupply();
        require(isChipListActive, "Chiplist is not active");
 ...
 function presaleMint(bytes32[] calldata merkleProof, uint8 count)
    ...
    callerIsUser
  {
   require(
    ...
    require(count > 0 && count <= 2, "You can mint minimum 1, maximum 2 NFTs");
    require(totalSupply() + count <= MAX_SUPPLY, "Cannot exceeds max supply");
