 ...
 */
contract GenericNFT is ProxyTarget, Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Pausable {
    ...
    function Initialize() onlyOwner external {
        ...
        //_tokenName          = "";
        //_tokenSymbol        = "";
        _baseURI            = "https://evaverse.com/api/creature_unknown.php?id=";
        _initialized        = true;
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override whenNotPaused {
        address owner = GenericNFT.ownerOf(tokenId);
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(GenericNFT.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
    ...
    function _approve(address to, uint256 tokenId) internal virtual whenNotPaused {
        ...
        emit Approval(GenericNFT.ownerOf(tokenId), to, tokenId);
    }
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256) {
        require(index < GenericNFT.balanceOf(owner), "ERC721Enumerable: owner index out of bounds");
    ...
    function tokenByIndex(uint256 index) public view virtual override returns (uint256) {
        require(index < GenericNFT.totalSupply(), "ERC721Enumerable: global index out of bounds");
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        uint256 length = GenericNFT.balanceOf(to);
    ...
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId) private {
        ...
        uint256 lastTokenIndex = GenericNFT.balanceOf(from) - 1;
    ...
    function SendNFT(address wallet) external onlyOwner {
        _safeMint(wallet);
    }
    
    function SendNFTs(address[] memory wallets) external onlyOwner {
        for(uint ii = 0; ii < wallets.length; ii++) {
            _safeMint(wallets[ii]);
        }
    }

    function DevTokenRename(string memory tokenName, string memory tokenSymbol) external onlyOwner {
        _tokenName = tokenName;
        _tokenSymbol = tokenSymbol;
    }

    function DevSetBaseURI(string memory uri) external onlyOwner {
        _baseURI = uri;
    }
