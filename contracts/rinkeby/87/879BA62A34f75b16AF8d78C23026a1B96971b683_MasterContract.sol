...
contract MasterContract is AccessControl, IMaster, ERC721 {
  ...
  address public constant CROSSMINT_ADDRESS = 0xdAb1a1854214684acE522439684a145E62505233;
  ...
  function crossmint(address _to, uint _count, Round _round, string memory _name) external payable {
    RoundContract round = roundContracts[_round];

    require(round.whiteList(_to), "NFT round: no sender in white list");
    require(msg.value == _roundPrice[_round] * _count, "Incorrect ETH value sent");
    
    uint[] memory tokenIdxs = round.generateUnique(_count);

    // if (tokenIdxs.length == _count) {
  ...
  function setRevewal(uint _newDate) external onlyRole(DEFAULT_ADMIN_ROLE) {
    revealDate = _newDate;
...
contract                      RoundContract is AccessControl {
  ...
  address public constant            WITHDRAW_ADDRESS = 0x5a56C7909b5d55b1b6e7A30c2559FeA8FaA1dcA5; // 0x0867436a889bf9C1abCAf3c505046FC4F7880b50
  ...
  function generateUnique(uint _nTokens) external view returns(uint[] memory) {
