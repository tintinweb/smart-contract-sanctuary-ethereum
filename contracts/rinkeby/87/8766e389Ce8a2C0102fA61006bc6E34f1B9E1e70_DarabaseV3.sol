...
contract DarabaseV3 is NFTContractV3{
...
contract NFTContractV3 is ERC1155, Ownable, Pausable{
    ...
    mapping(uint256 => uint256) public newMap;
    ...
    function mintRWO(
        ...
        // rwo mapping
        rwo[nftId]= Rwo(uint128(nftId), address(to), string(uri), uint256(structVar));
    ...
    function mintRentalToken(
        ...
        rental[pdrId][rentalId] = Rental(uint256(rentalId), address(to), uint256(amount), uint128(startTime), uint128(endTime));
    ...
    //enum testing
    function setTokenType(uint256 index, TokenType _tokenType) external {
        tokenType[index] = _tokenType;
    }

    // function to check new added variables
    function setNewVar(uint256 _newVariable) external checkNewModifier{
        newVar = _newVariable;
    }

    function setNewMap(uint256 index, uint256 _val) external {
        newMap[index] = _val;
    }

    function setNewArr(uint256 _val) external {
        newArr.push(_val);
    }

    // new function added
    function addNewFunction() external view returns(uint256){
       return nftId;
    }
