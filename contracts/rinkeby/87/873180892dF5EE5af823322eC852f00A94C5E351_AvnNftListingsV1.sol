...
contract AvnNftListingsV1 is IAvnNftListingsV1 {
    ...
    isListedForBatchSale(_batchId)
  {
    ...
    unchecked { saleIndex++; }
    batch[_batchId].saleIndex = saleIndex;
    ...
    onlySeller(_batchId)
  {
    ...
    emit LogBatchSaleEnded(_batchId, batch[_batchId].supply - batch[_batchId].saleIndex);
    ...
    onlyAuthority()
  {
    ...
    delete listing[_batchId];
    sendFunds(msg.sender, salesToRefund);
    ...
    returns (uint64 amountAvailable_)
  {
    ...
    amountAvailable_ = supply - saleIndex;
  }
    ...
    returns(string memory uuid_)
  {
    ...
    uuid_ = string(abi.encodePacked(
      hexSlice(0, 4, halfNftId), "-",
      hexSlice(4, 6, halfNftId), "-",
      hexSlice(6, 8, halfNftId), "-",
      hexSlice(8, 10, halfNftId), "-",
      hexSlice(10, 16, halfNftId)));
  }

  function hexSlice(uint256 _start, uint256 _end, bytes16 _halfNftId)
    private
    pure
    returns (bytes memory result)
  {
    result = new bytes((_end - _start) * 2);
    uint256 j;
