...
contract OnchainERC721V2 is ERC721 {    
        
    bytes[] private imageData;

    constructor() {
        initializeERC721("Onchain ERC721 V2", "ONCHAINV2", "");
    }  

    function uploadImagePart(bytes calldata data) public {
        imageData.push(data);
    }     
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        bytes memory base64Png = abi.encodePacked("data:image/webp;base64,", Base64.encode(png));

        byteString = abi.encodePacked(byteString, Json.pushJsonPrimitiveStringAttribute("image_data", string(base64Png), false));
