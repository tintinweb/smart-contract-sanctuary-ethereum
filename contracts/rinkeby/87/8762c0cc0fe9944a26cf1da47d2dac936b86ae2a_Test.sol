...
contract Test is  ERC721Enumerable,  Ownable {
    ...
    //Collections mapping
    mapping(address => string) private _addressToCollections;
    //Names mapping
    ...
    constructor(string memory _baseURI, uint256 _nextTokenID, uint256[] memory _cost) ERC721("Test", "Test") {
        baseURI = _baseURI;
        nextTokenID = _nextTokenID;
        registrationCost = _cost;
        _addressToCollections[address(this)] = ".cns";
    }

    function registerName(string memory _name, address collection) public payable {
        ...
        require(msg.sender == owner() || _checkEligibility(collection), "Not eligible for this collection.");
        bytes32 hash = _computeNamehash(_name);
        require(_exists(hash) == false, "Name already exists.");
        string memory _extension = _addressToCollections[collection];
    ...
    function migrateExtension(string memory _name, address _oldCollection, address _newCollection) public {
        require(_checkEligibility(_newCollection), "Not eligible for this collection.");
        require(ownerOfName(_name, _addressToCollections[_oldCollection]) == msg.sender, "Not owned by caller");
        string memory _newExtension = _addressToCollections[_newCollection];
    ...
    function _checkEligibility(address collection) public view returns (bool) {
        require(_collectionExists(collection), "This collection is not eligible.");
    ...
    function _collectionExists(address collection) internal view returns(bool) {
        string memory _extension = _addressToCollections[collection];
    ...
    function addCollections(address[] memory addresses, string[] memory domains) public onlyOwner {
        require(addresses.length == domains.length, "Not same length");

        for(uint8 i=0; i<addresses.length;i++) {
            _addressToCollections[addresses[i]] = domains[i];
        }
    }
