...
contract KemigawaJinjaCollection is ERC721A, Ownable, Pausable {
    ...
    mapping(uint64 => string) private _amuletNames;
    mapping(uint64 => string) private _amuletDescriptions;
    mapping(uint64 => string) private _amuletNamesExpired;
    mapping(uint64 => string) private _amuletDescriptionsExpired;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (block.timestamp > mintTimestamp + blessingTimeSec) {
            string memory name = _amuletNamesExpired[amuletType];
            string memory description = _amuletDescriptionsExpired[amuletType];
            return string(
                abi.encodePacked(
                    'data:application/json;base64,',
                    Base64.encode(
                        bytes(abi.encodePacked('{"name":"', name, '", "description":"', description, '", "image": "', imageURI, amuletType, '_expired', imageExtension, '"}'))
                    )
                )
            );
        } else {
            string memory name = string(abi.encodePacked(_amuletNames[amuletType], ' Minted at ', BokkyPooBahsDateTimeLibrary.getYear(mintTimestamp), '/', BokkyPooBahsDateTimeLibrary.getMonth(mintTimestamp), '/', BokkyPooBahsDateTimeLibrary.getDay(mintTimestamp)));
            string memory description = _amuletDescriptions[amuletType];
            return string(
                abi.encodePacked(
                    'data:application/json;base64,',
                    Base64.encode(
                        bytes(abi.encodePacked('{"name":"', name, '", "description":"', description, '", "image": "', imageURI, amuletType, imageExtension, '", "animation_url": "', animationURI, amuletType, animationExtension, '"}'))
                    )
                )
            );
        }
    ...
    function getJsonText(uint256 tokenId) public view returns (string memory) {
        ...
        if (block.timestamp > mintTimestamp + blessingTimeSec) {
            string memory name = _amuletNamesExpired[amuletType];
            string memory description = _amuletDescriptionsExpired[amuletType];
            return string(abi.encodePacked('{"name":"', name, '", "description":"', description, '", "image": "', imageURI, amuletType, '_expired', imageExtension, '"}'));
        } else {
            string memory name = string(abi.encodePacked(_amuletNames[amuletType], ' Minted at ', BokkyPooBahsDateTimeLibrary.getYear(mintTimestamp), '/', BokkyPooBahsDateTimeLibrary.getMonth(mintTimestamp), '/', BokkyPooBahsDateTimeLibrary.getDay(mintTimestamp)));
            string memory description = _amuletDescriptions[amuletType];
            return string(abi.encodePacked('{"name":"', name, '", "description":"', description, '", "image": "', imageURI, amuletType, imageExtension, '", "animation_url": "', animationURI, amuletType, animationExtension, '"}'));
        }
    }

    // Mintタイムスタンプを取得
    function getMintTimestamp(uint256 tokenId) public view virtual returns (uint256) {
        require(exists(tokenId), "token is not exist");
