...
contract ArtistReleases is ERC1155, Ownable, IERC2981Royalties {
    ...
    uint256 private constant GENESIS_COUNT = 1;
    uint256 private constant MIN_TOKEN_COUNT = 10;
    uint256 private constant MAX_PROMO_COUNT = 1_000;
    uint256 private constant MAX_OPEN_EDITION_COUNT = 100_000_000;

    function isValidSupply(NFTType nftType, uint256 supply) internal pure returns (bool isValue) {
        if (NFTType.CreatorGenesis == nftType || NFTType.CollectorGenesis == nftType) return GENESIS_COUNT == supply;
        if (NFTType.Promo == nftType) return supply >= MIN_TOKEN_COUNT && supply <= MAX_PROMO_COUNT;
        if (NFTType.OpenEdition == nftType) return supply >= MIN_TOKEN_COUNT && supply <= MAX_OPEN_EDITION_COUNT;
        return false;
    }
    ...
    constructor(address artist_, address fingerprintsRegistry_) ERC1155("") {
    ...
    function mintBatch(
        ...
        require(royaltyAmount <= 1_000, "Invalid royaltyAmount");
        ...
        for (uint256 i = 0; i < nftTypes.length; i++) {
            ...
            require(isValidSupply(nftTypes[i], totalSupplies[i]), "Invalid totalSupply");
        ...
        if (creatorTokenId != 0 && _msgSender() != artist)
            safeTransferFrom(_msgSender(), artist, creatorTokenId, 1, "");
