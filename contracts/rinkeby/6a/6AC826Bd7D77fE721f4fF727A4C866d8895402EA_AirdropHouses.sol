...
contract AirdropHouses is ERC721, Ownable {
    ...
    function getBatchNum() public view returns (uint256) {
        ...
        uint batchNumByTime = pastTime / _batchDuration;
        uint batchNumByCount = _tokenIdCounter.current() / _sheetsPerBatch;
        uint batchNum = batchNumByCount > batchNumByTime ? batchNumByCount : batchNumByTime;

        return batchNum + 1 >=3 ? 3: batchNum + 1;
    }
