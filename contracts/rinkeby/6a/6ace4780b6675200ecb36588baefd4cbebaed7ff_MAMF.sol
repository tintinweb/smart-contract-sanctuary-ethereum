...
contract MAMF is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  function calculateTrainingPointsToClaim(uint256 _tokenId) public view returns (uint points) {
    return block.timestamp - friendsTraining[_tokenId].startTime / 60 / 60;
  }
  ...
  function internalMint(uint256 _amount) internal {
    uint startIndex = _totalMinted();
    for (uint i = 0; i < _amount; i++){
      friends[startIndex + i] = Friend(0,0,0,0,0);
