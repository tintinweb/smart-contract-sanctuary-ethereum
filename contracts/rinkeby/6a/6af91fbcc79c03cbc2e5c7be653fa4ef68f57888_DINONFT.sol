...
contract DINONFT is ERC721Enumerable, Ownable {
    using Address for address payable;

    uint256 public constant DINO_PREMINT = 50;
    uint256 public constant DINO_MAX = 100000;
    uint256 public constant DINO_MAX_PRESALE = 5000;
    uint256 public constant DINO_PRICE = 0.02 ether;
    uint256 public constant DINO_PER_WALLET = 50;
    uint256 public constant DINO_PER_WALLET_PRESALE = 35;
    ...
    function buy(uint256 quantity) external payable {
        require(
            ...
            "DINONFT: Sale is not currently live"
        );
        require(
            totalSupply() + quantity <= DINO_MAX,
            "DINONFT: Quantity exceeds remaining tokens"
        );
        require(
            quantity <= DINO_PER_WALLET - addressToMinted[msg.sender],
            "DINONFT: Wallet cannot mint any new tokens"
        );
        require(quantity != 0, "DINONFT: Cannot buy zero tokens");
        require(
            msg.value >= quantity * DINO_PRICE,
            "DINONFT: Insufficient funds"
        );
    ...
    function setBaseURI(string calldata newBaseURI) external onlyOwner {
        require(!locked, "DINONFT: Contract metadata is locked");
    ...
    {
        require(_exists(tokenId), "DINONFT: URI query for nonexistent token");
        return
