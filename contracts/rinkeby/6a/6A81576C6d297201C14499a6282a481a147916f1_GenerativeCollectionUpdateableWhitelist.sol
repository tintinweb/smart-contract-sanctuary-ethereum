  ...
  ReentrancyGuard
{
  ...
  modifier whenPresale(address sender) {
    if (_isPresale) {
      ...
      require(isEligible, 'This wallet is not eligible for presale');
      // if (!isEligible) {
  ...
  modifier whenNotExceedMaxPurchaseable(uint256 numberOfTokens) {
    require(numberOfTokens > 0, 'Cant mint less than 0 tokens');
    require(numberOfTokens <= MAX_NFT_PURCHASEABLE, 'Cant mint more than the max purchasable per tx');
  ...
  modifier whenNotExceedMaxSupply(uint256 numberOfTokens) {
    require(totalSupply() + numberOfTokens <= (maxSupplyWithBurnCount() - _reserved), 'Cant exceed max supply');
    // if (
  ...
  modifier hasEnoughEther(uint256 numberOfTokens) {
    require(msg.value >= _mintPrice * numberOfTokens, 'Not enough ether to complete tx');
