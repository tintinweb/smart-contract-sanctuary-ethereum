...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(isProxyOwner() || owner() == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract TN is Ownable, ERC721A, ReentrancyGuard {
  using Strings for uint256;

  uint256 public constant COLLECTION_SIZE = 8888;
  uint256 public constant MAX_PER_ADDRESS_DURING_MINT = 5; //maxPerAddressDuringMint
  uint256 public constant AMOUNT_FOR_AUCTION_AND_RESERVATION = 6000; //amountForAuction
  uint256 public constant AMOUNT_FOR_RESERVATION = 200;
 ...
 struct SaleConfig {
  ...
  function auctionMint(uint256 quantity) external payable callerIsUser nonReentrant {
    ...
    require(
      totalSupply() + quantity <= AMOUNT_FOR_AUCTION_AND_RESERVATION,
      "not enough remaining reserved for auction to support desired mint amount"
    );
    require(
      numberMinted(msg.sender) + quantity <= MAX_PER_ADDRESS_DURING_MINT,
  ...
  function reserveMint(uint256 quantity, uint256 batchSize) external onlyOwner {
    require(totalSupply() + quantity <= COLLECTION_SIZE, "Mint exceeds total supply");
