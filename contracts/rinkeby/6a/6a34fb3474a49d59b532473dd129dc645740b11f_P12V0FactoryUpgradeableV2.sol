...
interface IP12Mine {
    ...
    function addLpToken(address _lpToken,bool _withUpdate)external;

    function addLpTokenInfoForGameCreator(address _lpToken,address gamecoinCreator,uint256 _amount)external;
    ...
    OwnableUpgradeable
{
    ...
    function create(
       ...
       (,,liquidity0) = IUniswapV2Router02(uniswapRouter).addLiquidity(
        ...
        // 添加新的质押池
        IP12Mine(p12mine).addLpToken(pair,false);

        // 质押挖矿
        IP12Mine(p12mine).addLpTokenInfoForGameCreator(pair,msg.sender,liquidity0);
