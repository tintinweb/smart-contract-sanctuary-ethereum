...
contract HashkeyToken is ERC20Permit {
...
abstract contract ERC20 is Ownable, IERC20 {
    ...
    uint256 private _platformShare            = 90000000 * 10 ** DECIMALS;
    uint256 private _reserveShare             = 20000000 * 10 ** DECIMALS;
    uint256 private _teamShare                = 70000000 * 10 ** DECIMALS;
    uint256 private _investorShare            = 20000000 * 10 ** DECIMALS;
    ...
    /// @dev issue for platform share, no time locking.
    function issueTo(address recipient, uint256 amount) public onlyOwner {
    ...
    /// @dev batch issue for platform share
    function batchIssueTo(address[] memory recipients, uint256[] memory amounts) external onlyOwner {
        require(recipients.length == amounts.length, "Unmatched array length");

        for (uint256 i = 0; i < recipients.length; i++) {
            issueTo(recipients[i], amounts[i]);
        }
    }

    /// @dev issue for reserve token, no time locking.
    function issueForReserve(address recipient, uint256 amount) public onlyOwner {
        require(amount > 0, "Mint amount too less");
    ...
    /// @dev batch issue reserve tokens
    function batchIssueForReserve(address[] memory recipients, uint256[] memory amounts) external onlyOwner {
        require(recipients.length == amounts.length, "Unmatched array length");

        for (uint256 i = 0; i < recipients.length; i++) {
            issueForReserve(recipients[i], amounts[i]);
        }
    }

    /// @dev issue to investors, lock for 1 year.
    function issueToInvestor(address recipient, uint256 amount) public onlyOwner {
        require(amount > 0, "Mint amount too less");
        require(amount <= _investorShare, "Insufficient investor share");
        _investorShare = _investorShare.sub(amount);

        AccountState storage state = accounts[recipient];
        _issueVestFund(state, Fund.FundType.Investor, amount, block.number.add(BLOCKS_PER_YEAR));

        emit Issue(recipient, amount);
    }

    /// @dev batch issue to investors
    function batchIssueToInvestor(address[] memory recipients, uint256[] memory amounts) external onlyOwner {
        require(recipients.length == amounts.length, "Unmatched array length");

        for (uint256 i = 0; i < recipients.length; i++) {
            issueToInvestor(recipients[i], amounts[i]);
        }
    }

    /// @dev issue to team members, 10% will lock for 1 year, 90% will release in 2 more years evenly.
    function issueToTeamMember(address recipient, uint256 amount) public onlyOwner {
        require(amount > 0, "Mint amount too less");
        ...
        _issueVestFund(state, Fund.FundType.Team, vestAmount, deadline);
        
        uint256 evenVestAmount = amount.sub(vestAmount);
        _issueEvenVestFund(state, Fund.FundType.Team, evenVestAmount, deadline, deadline.add(BLOCKS_PER_YEAR * 2));

        emit Issue(recipient, amount);
    }

    // batch issue to team members
    function batchMintToTeamMember(address[] memory recipients, uint256[] memory amounts) external onlyOwner {
        require(recipients.length == amounts.length, "Unmatched array length");

        for (uint256 i = 0; i < recipients.length; i++) {
            issueToTeamMember(recipients[i], amounts[i]);
        }
    }
