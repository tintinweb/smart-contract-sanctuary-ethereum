...
contract WTCStaking is Context, Ownable {
  ...
  ////////////STAKING FUNCTIONS//////////
   ...
   //function to get the number of NFT's a user owns
   function getNFTCount(address _holder) public view returns(uint){
       uint temp;
       for (uint256 s = 0; s <= wtcsupply; s += 1){
           if (wrappedcompanion(wrappedcompnft).ownerOf(s)==_holder){
               temp +=1;
           }
       }
       return temp;
