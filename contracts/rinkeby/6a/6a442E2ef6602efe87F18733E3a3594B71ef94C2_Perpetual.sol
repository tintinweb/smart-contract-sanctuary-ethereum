...
contract Perpetual is Users, Deposits, Withdrawals, UpdateState, GlobalConfig, Dac, Custom {
    ...
    function initialize(
        ...
        DEPOSIT_CANCEL_TIMELOCK = 0 days; // 2 days;
...
abstract contract UpdateState is ForcedWithdrawals, ConditionalTransfer, Config {
    ...
    struct CommitBlockInfo {
        uint32 blockNumber;                 // TODO : seqence + 1 check
        uint32 timestamp;
    ...
    function getOrderStateHash(
        ...
        if (b.globalFunding.index.length != 0) {
            ...
            bytes memory encode_data = abi.encodePacked(timestamp, encodePackU64Array(b.globalFunding.index, 1, MAX_ASSETS_COUNT, 1 << 63));
...
abstract contract GlobalConfig is Storage {
    ...
    function initGlobalConfig(
        ...
        bytes memory globalConfig =bytes.concat(
            abi.encodePacked(
                _n_SyntheticAssetInfo,
                _funding_validity_period,
                _price_validity_period,
                _max_funding_rate
            ),
...
contract Storage is Governance, ReentrancyGuard {
    ...
    uint32 public TIMELOCK_GLOBAL_CONFIG_CHANGE;
    uint256 public DEPOSIT_CANCEL_TIMELOCK;
    uint256 public FORCED_ACTION_EXPIRE_TIME;

    uint32 public MIN_DAC_MEMBER;
    uint32 public TIMELOCK_DAC_REG;
}
