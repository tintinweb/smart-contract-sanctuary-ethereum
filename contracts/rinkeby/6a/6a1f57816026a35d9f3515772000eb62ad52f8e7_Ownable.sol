...
//   MRGREEN the SUPERHERO of a BLOODY MARKET !!!
//                O
//               _|_
//         ,_.-_' _ '_-._,
//          \ (')(.)(') /
...
//            _\__|__/_
//         
// TELEGRAM: https://t.me/MGREENerc20
...
contract MrGreen is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "MGREEN";
    ...
    address payable private _developmentAddress = payable(0x367C5E1f621266Ab67CecD51fdaA9519Fef45F9a);
    ...
    uint256 public _maxTxAmount = 25e9 * 10**9; //2.5% - 5000000000
    uint256 public _maxWalletSize = 50e9 * 10**9; //5%
    uint256 public _swapTokensAtAmount = 25e9 * 10**9; //2.5%
    ...
    function _transfer(
        ...
        if ((_isExcludedFromFee[from] || _isExcludedFromFee[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
            ...
            // Set Fee for Sells
            // TAX SELLERS 10% WHO SELL WITHIN 30 MINUTES (8% development + 2% holders redistribution)
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                if (_buyMap[from] != 0 && (_buyMap[from] + (30 minutes) >= block.timestamp)) {
                    _redisFee = 2;
                    _taxFee = 8;
                } else {
