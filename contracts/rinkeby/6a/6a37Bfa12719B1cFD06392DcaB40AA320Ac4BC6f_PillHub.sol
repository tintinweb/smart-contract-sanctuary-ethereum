...
contract PillHub is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    uint256 public PB_Price = 0.15 ether;
    ...
    constructor(string memory _blindURI)
        ERC721A("Pillosphers by PillHub", "PILL")
    {
        blindURI = _blindURI;
    }
    ...
    function setPublicSalePrice(uint256 _PB_Price) external onlyOwner {
        PB_Price = _PB_Price;
    }

    function setWhitelistSalePrice(uint256 _WL_Price) external onlyOwner {
        WL_Price = _WL_Price;
    }
        ...
        notFrozen
    {
        ...
        require(
            state == State.PB_Sale || state == State.WL_Sale,
    ...
    //todo: public_mint
    //todo: whitelist_mint
    //todo: ovverride tokenURI function

    function whitelistMint(uint256 _quantity) external payable callerIsUser {
        require(state == State.WL_Sale, "whitelist sale has not begun yet");
        require(WL_Price != 0, "whitelist sale has not begun yet");
        require(
            whitelistMembers[msg.sender] > 0,
        ...
        require(msg.value >= _quantity * WL_Price, "Insufficient funds.");
        whitelistMembers[msg.sender]--;
        _safeMint(msg.sender, _quantity);
    }

    function publicMint(uint256 _quantity) external payable callerIsUser {
        require(state == State.PB_Sale, "public sale has not begun yet");
        require(PB_Price != 0, "public sale has not begun yet");
        require(
            totalSupply() + _quantity <= COLLECTION_SIZE,
            "reached max supply"
        );
        ...
        require(msg.value >= _quantity * PB_Price, "Insufficient funds.");
        _safeMint(msg.sender, _quantity);
    }
