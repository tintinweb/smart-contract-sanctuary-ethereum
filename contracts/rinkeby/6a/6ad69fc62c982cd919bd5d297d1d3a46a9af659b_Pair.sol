...
interface IUSWAPmeFactory {
    event PairCreated(address indexed token0, address indexed token1, address pair, uint256 index);

    function createPair(address tokenA, address tokenB, address pair) external returns(address);
...
contract Pair is TRC20, IPair {
    ...
    function setFactory(address _fatory) external {
        //require(msg.sender == feeToSetter, 'USWAPmeFactory: FORBIDDEN');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns(bool feeOn) {
        address feeTo = IUSWAPmeFactory(factory).feeTo();
...
contract USWAPmeFactory is IUSWAPmeFactory {
    ...
    function createPair(address tokenA, address tokenB, address pair) external returns(address) {
        require(tokenA != tokenB, 'USWAPmeFactory: IDENTICAL_ADDRESSES');

        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);

        require(token0 != address(0), 'USWAPmeFactory: ZERO_ADDRESS');
        require(pairs[token0][token1] == address(0), 'USWAPmeFactory: PAIR_EXISTS');
