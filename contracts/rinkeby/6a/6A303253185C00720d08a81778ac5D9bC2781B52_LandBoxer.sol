...
contract LandBoxer is Context, Ownable {
    ...
    function _metaTradeBasic(address to, bytes32 _zoneId, uint256 _scale, uint256 _amount) private returns (uint256[] memory, uint256[] memory) {
        ...
        if (_isZoneRegistered(_zoneId) && _getCapacityByZoneIdAndScale(_zoneId, _scale) >= _amount) {
            landCapacities[_zoneId].capacities[_getIndexByZoneIdAndScale(_zoneId, _scale)] 
                = landCapacities[_zoneId].capacities[_getIndexByZoneIdAndScale(_zoneId, _scale)].sub(_amount);
    ...
    function _getFirstRandomZoneIdWithEnoughCapacity(uint256 nonce, uint256 _scale, uint256 _amount) private view returns (bytes32 zoneId) {
        ...
        while (landCapacities[zoneId].capacities[_getIndexByZoneIdAndScale(zoneId, _scale)] < _amount) {
