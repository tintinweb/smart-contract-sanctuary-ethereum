...
contract AGOVLPStaking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function deposit(
        ...
        (uint256 tokenId, uint128 liquidity, uint256 agovOutAmount, uint256 ethOutAmount) = _mintPosition(
            msg.value,
            agovInAmount
        );
     ...
     * @param depositId Deposit struct id
     */
    function rewardOfLLC(uint256 depositId) external view returns (uint256 reward) {
        Deposit memory lp = deposits[depositId];
        uint256 amountPerLock = lp.lock == 180 days ? 2 : 1;
        reward = amountPerLock * (ceil(lp.ethAmount) / MIN_ETH_AMOUNT);
    }
    ...
    /**
     * @dev returns ceil value in math
     */
    ...
    function _addLLCReward(
        ...
        uint256 llcRewardAmount = amountPerLock * (ceil(ethAmount) / MIN_ETH_AMOUNT);
