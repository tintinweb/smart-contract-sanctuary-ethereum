...
contract WatchfacesWorld is ERC721, IERC2981, EIP712Whitelisting {
    ...
    constructor(address _whitelistSigningKey, uint256[][4] memory _supply)
        ERC721('Watchfaces', 'WFW')
        EIP712Whitelisting(_whitelistSigningKey)
    {
        for (uint256 partType; partType < 4; partType++) {
            for (uint256 partId; partId < _supply[partType].length; partId++) {
                partsSupply[partType][partId] = _supply[partType][partId];
            }
        }
    ...
    function mint(
        ...
        craftWatch(configuration);

        requiresWhitelist(_tokenId, _usePass, _engraving, _signature);
    ...
    function remainingSupplyBatched(uint256[4] memory _idMax)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory supply = new uint256[](_idMax[0] + _idMax[1] + _idMax[2] + _idMax[3]);

        uint256 total = 0;
        for (uint256 partType; partType < 4; partType++) {
            for (uint256 partId; partId < _idMax[partType]; partId++) {
                supply[total] = partsSupply[partType][partId];
                total++;
            }
        }
        ...
        returns (WatchConfiguration memory configuration)
    {
        ...
        configuration.bezel = uint8((_tokenId / BEZEL_PART_BASE) % 100);
        configuration.face = uint8((_tokenId / FACE_PART_BASE) % 100);
        configuration.mood = uint8((_tokenId / MOOD_PART_BASE) % 100);
        configuration.glasses = uint8((_tokenId / GLASSES_PART_BASE) % 100);
    }

    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        address owner = ownerOf(_tokenId);
        uint256 timestamp = timestamps[_tokenId];
        WatchConfiguration memory configuration = parseTokenId(_tokenId);
    ...
    function _beforeTokenTransfer(
        ...
        if (timestamps[_tokenId] != 0 && !heldForAtLeast8WeeksBeforeTransfer[_tokenId]) {
    ...
    function craftWatch(WatchConfiguration memory _configuration) internal {
        uint256[4] memory partIds;

        partIds[0] = _configuration.bezel;
        partIds[1] = _configuration.face;
        partIds[2] = _configuration.mood;
        partIds[3] = _configuration.glasses;

        for (uint256 partType; partType < 4; partType++) {
            require(partsSupply[partType][partIds[partType]] > 0, 'Part out of stock');
            unchecked {
                // Can't overflow or underflow
                partsSupply[partType][partIds[partType]] -= 1;
            }
        }
    }
...
contract EIP712Whitelisting is Ownable {
    ...
    function setWhitelistSigningAddress(address _whitelistSigningKey) public onlyOwner {
        whitelistSigningKey = _whitelistSigningKey;
    }
...
contract DumbRenderer {
    function render(
        ...
        tokenURI = string.concat(
            Strings.toString(bezel),
            '-',
            Strings.toString(face),
            '-',
            Strings.toString(mood),
            '-',
