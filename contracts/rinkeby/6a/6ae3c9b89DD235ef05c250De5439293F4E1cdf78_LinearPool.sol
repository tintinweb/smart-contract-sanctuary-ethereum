    ...
    PausableUpgradeable
{
    ...
    uint256 public maxSaleNft = 100;
    uint256 public maxNftPerUser = 10;
    ...
    event FundTokenToNftTreasury(
        uint256 poolId,
        address user,
        uint256 amount,
        uint256 count
    );
    event UpdateMaxSaleNft(uint256 value);
    event UpdateMaxNftPerUser(uint256 value);
    ...
    /**
     * @notice Set max sale nft
     * @param value amount of nft to sale
     */
    function setMaxSaleNft(uint256 value) external onlyOwner {
        require(
            value >= totalSoldNft,
            "LinearStakingPool: value must be greater than or equal total sold nft"
        );

        maxSaleNft = value;
        emit UpdateMaxSaleNft(value);
    }

    /**
     * @notice Set max nft for each user
     * @param value max nft for each user
     */
    function setMaxNftPerUser(uint256 value) external onlyOwner {
        maxNftPerUser = value;
        emit UpdateMaxNftPerUser(value);
    }
    ...
    function linearWithdrawAllAndBuyNft(
        ...
        userNftBalance[account] += _count;
        totalSoldNft += _count;
        require(
            userNftBalance[account] <= maxNftPerUser &&
                totalSoldNft <= maxSaleNft,
            "LinearStakingPool: Not enough NFT to buy"
        );
        ...
        require(
            ...
            "LinearStakingPool: not enough amount to buy nft"
        );
        ...
        emit FundTokenToNftTreasury(_poolId, account, _amount, _count);
        emit LinearWithdraw(_poolId, account, withdrawBalance);
    }
