...
// File: contracts/pin-planets-stardust.sol
...
contract PinGalaxyStardust is ERC20, Ownable {
    ...
    address private PINPLANETS_ADDRESS =
        0x52e6f2ED6fFC9063a61e168EF2F87C0BC163511E;
    address private PINGALHERO_ADDRESS;
    ...
    function claimPinPlanetsReward(uint256[] memory tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                msg.sender == IERC721(PINPLANETS_ADDRESS).ownerOf(tokenIds[i]),
                "You do not own that planet."
            );
    ...
    function claimPinGalaxyHeroesReward(uint256[] memory tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                msg.sender == IERC721(PINGALHERO_ADDRESS).ownerOf(tokenIds[i]),
                "You do not own that hero."
            );
