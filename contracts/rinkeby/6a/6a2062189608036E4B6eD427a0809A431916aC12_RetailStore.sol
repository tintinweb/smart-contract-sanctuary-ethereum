...
contract RetailStore {
    ...
    // 0 kartik 78600598 Reccomandations Cart
    struct Customer {
    ...
    mapping(uint256 => uint256[]) fealtyScore;
    ...
    function updateFealty(uint256 customerId) public {
        for (uint256 i = 0; i < cart[customerId].length; i++) {
            uint256 productId = cart[customerId][i].prodId;
            fealtyScore[customerId][productId] += cart[customerId][i].count + 2;
        }
    }

    //reset cart of given customer to empty
    function resetCart(uint256 customerId) public {
        for (uint256 i = 0; i < cart[customerId].length; i++) {
            cart[customerId].pop();
        }
        uint256 size = cart[customerId].length;
        if (size == 1) {
            cart[customerId].pop();
        }
    }
    ...
    function addToCart(
        ...
        cart[customerId].push(CartItem(prodId, count, _price));
        return true;
    }

    //returns the cart of current customer
    function getCart(uint256 customerId)
        public
        view
        returns (CartItem[] memory c)
    {
    ...
    //update fealty
    //reset cart
    function billing(uint256 customerId) public returns (uint256) {
        ...
        for (uint256 i = 0; i < cart[customerId].length; i++) {
            ...
            totalPrice +=
                cart[customerId][i].count *
                Inventory[productId].price;

            quantity[productId] -= cart[customerId][i].count;
        }
