...
contract EIP712Whitelist is Ownable {
    ...
    modifier requiresWhitelist(string calldata signature, uint256 nonce) {
        ...
        address recoveredAddress = digest.recover(bytes(signature));
...
contract NFT is ERC721, EIP712Whitelist {
    ...
    function genesisPresale(string calldata signature, uint256 nonce)
        external
        payable
    ...
    function normalPresale(uint256 num, string calldata signature, uint256 nonce)
        external
        payable
        requiresWhitelist(signature, nonce)
    {
        ...
        returns (uint256[] memory)
    {
        ...
        if (ownerIndex != ownerBalance){
            for (uint256 tokenId=8001; tokenId <= 8080 - reserved; tokenId++){
                if (_owner == ownerOf(tokenId)){
                    ownerTokens[ownerIndex] = tokenId;
                    ownerIndex++;
                    if (ownerIndex == ownerBalance) break;
