...
contract WPunkGateway is IWPunkGateway, IERC721Receiver, Ownable {
     ...
     * @param pool address of the targeted underlying pool
     * @param punkIndexes indexes of aWPunk to withdraw and receive native WPunk
    ...
    function withdrawPunk(
        ...
        INToken aWPunk = INToken(
            Pool.getReserveData(address(WPunk)).xTokenAddress
        );
        for (uint256 i = 0; i < punkIndexes.length; i++) {
            aWPunk.safeTransferFrom(msg.sender, address(this), punkIndexes[i]);
        }
     ...
     * @dev withdraws the WETH _reserves of msg.sender.
     * @param pool address of the targeted underlying pool
     * @param punkIndexes punkIndexes of aWPunks to withdraw and receive native WPunk
     * @param to address of the user who will receive native ETH
    ...
    function withdrawPunkWithPermit(
        ...
        // user will need to grant permit
        // aWPunk.permit(msg.sender, address(this), punkIndexes[i], deadline, permitV, permitR, permitS);
        for (uint256 i = 0; i < punkIndexes.length; i++) {
            WPunk.safeTransferFrom(msg.sender, to, punkIndexes[i]);
            WPunk.burn(punkIndexes[i]);
            Punk.transferPunk(to, punkIndexes[i]);
        }
    }

    // gateway contract to transfer token
    /**
...
interface IWPunkGateway {
    ...
    // TODO maybe eventually remove these since you cannot borrow a punk or repay a punk you have borrowed.
    // function repayPunk(
    // 	address pool,
    // 	uint256 amount,
    // 	uint256 rateMode,
