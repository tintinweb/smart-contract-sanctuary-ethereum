...
contract LoomiWithdraw is ReentrancyGuard, Ownable {
    ILoomi public loomi;

    uint256 constant public MAX_TAX_VALUE = 100;
    ...
    */
    function withdrawLoomi(uint256 amount) public nonReentrant whenNotPaused {
      require(_withdrawableLimit(_msgSender()) >= amount, "Withdraw more than allowed");
      loomi.spendLoomi(_msgSender(), amount);

      uint256 tax = withdrawTaxCollectionStopped ? 0 : (amount * withdrawTaxAmount) / 100;

      _withdrawals[_msgSender()].withdrawalAmounts.push(amount);
      _withdrawals[_msgSender()].withdrawalTimestamps.push(block.timestamp);
      
      loomi.mintFor(_msgSender(), (amount - tax));
    ...
    /**
    * @dev Public function which returns amount user can withdraw.
    */
    function withdrawableLimit(address user) public view returns (uint256) {
      return _withdrawableLimit(user);
    }

    /**
    * @dev Internal function which returns amount user can withdraw.
    */
    function _withdrawableLimit(address user) internal view returns (uint256) {
      uint256 numberOfWithdrawals = _withdrawals[user].withdrawalTimestamps.length;
      if (numberOfWithdrawals == 0) return limitAmount;
      ...
      return limitAmount - withdrawnAmount;
    }
    ...
    */
    ...
    /**
    * @dev Function allows admin to update withdraw limit amount.
    */
    function updateWithdrawlimitAmount(uint256 _limit) public onlyOwner {
      limitAmount = _limit;
    }

    /**
    * @dev Function allows admin to update withdraw time limit.
    */
    function updateWithdrawlimitPeriod(uint256 _limit) public onlyOwner {
      limitPeriod = _limit;
    }

    /**
    * @dev Function allows admin to pause withdrawals.
    */
