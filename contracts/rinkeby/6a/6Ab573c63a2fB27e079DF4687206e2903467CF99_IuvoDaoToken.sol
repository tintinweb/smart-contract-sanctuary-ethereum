...
contract IuvoDaoToken is ERC20, Ownable {
  ...
  uint256 public voteCooldownPeriod = 60 * 60 * 24 * 7;
  mapping(address => uint256) public voteCooldownStart;

  uint256 private _taxDaoDonations = 6;
  uint256 private _taxDaoRewards = 1;
  uint256 private _taxLp = 3;
  uint256 private _totalTax;

  uint256 private _liquifyRate = 10; // 1% of liquidity pair balance -- dividend is 1000
  ...
  constructor() ERC20('Iuvo DAO', 'IUVO') {
  ...
  function launch(uint8 _supplyPercentLp) external payable onlyOwner {
    ...
    require(_supplyPercentLp <= 100, 'cannot add more than supply to LP');

    uint256 _supplyForLp = (totalSupply() * _supplyPercentLp) / 100;
  ...
  function _transfer(
    ...
    bool _isOwner = sender == owner() ||
      recipient == owner() ||
      msg.sender == owner();
    require(_isOwner || amount <= _maxTx(), 'ERC20: too much ser');
    uint256 contractTokenBalance = balanceOf(address(this));

    bool _isSwap = recipient == uniswapV2Pair || sender == uniswapV2Pair;
    if (!_isSwap) {
      voteCooldownStart[recipient] = block.timestamp;
    } else if (balanceOf(recipient) == 0) {
      // reset cooldown if buying and new holder
      voteCooldownStart[recipient] = 0;
    }

    bool _overMin = contractTokenBalance >=
      (balanceOf(uniswapV2Pair) * _liquifyRate) / 1000;
    if (
      !swapping &&
      !_isOwner &&
      _overMin &&
      ...
      // don't allow swapping more than liquifyRate% of what's in the liquidity pool
      uint256 maxToSwap = (balanceOf(uniswapV2Pair) * _liquifyRate) / 1000;
    ...
    uint256 tax = 0;
    if (
      _launchTime != 0 && !(_isTaxExcluded[sender] || _isTaxExcluded[recipient])
    ) {
      tax = (amount * _totalTax) / 100;
      if (tax > 0) {
  ...
  function _maxTx() private view returns (uint256) {
    uint256 numMinutes = 60 * 15; // 15 minutes
    if (
      msg.sender == owner() ||
      _launchTime == 0 ||
      block.timestamp > _launchTime + numMinutes
    ) {
      return totalSupply();
    }
  ...
  function _setTotalTax() private {
    ...
    require(_totalTax <= 25, 'tax cannot be above 25%');
  }
  ...
  // _liquifyRate is divided by 1000, so each number is 0.1%
  function setLiquifyRate(uint256 _rate) external onlyOwner {
    require(_rate <= 100, 'cannot be more than 10%');
    _liquifyRate = _rate;
  }
  ...
  function setVoteCooldownPeriod(uint256 _seconds) external onlyOwner {
    require(_seconds < 60 * 60 * 24 * 30, 'cannot be more than 30 days');
...
contract IuvoDaoRewards is IDaoRewards, Ownable {
  ...
  modifier onlyToken() {
    require(msg.sender == shareholderToken, 'must be owner or token contract');
    _;
  }
