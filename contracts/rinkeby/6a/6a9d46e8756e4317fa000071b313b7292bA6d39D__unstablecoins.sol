...
contract _unstablecoins is ERC721, ERC721Enumerable, ERC721Burnable, ERC721Pausable, ERC721URIStorage {
    using ECDSA for bytes32;
    
    address private _signer;
    
    constructor(address signer_) ERC721("unETH", "USC") {
        _signer = signer_;
    }
    ...
    function createMessage(uint256 tokenId, uint256 price, string memory hash) public view returns (bytes32) {
        return keccak256(abi.encode(tokenId, price, hash, address(this)));
    }
    
    function test(bytes32 code) public {
    ...
    function validateSignature(
        ...
        require(message.recover(signature) == signer(), "ClaimerV1: Signature is invalid");

        return message;
    }


    function isValidPriceForTokenId(uint256 /* tokenId*/, uint256 price) public view returns (bool) {
        if(price == 1 ether) {
            return true;
        }
        return false;
    ...
    function mintTo(address to, uint256 tokenId, uint256 price, string memory hash, bytes memory signature) public payable {
        _mint(to, tokenId);
    }

    function mint(uint256 tokenId, uint256 price, string memory hash, bytes memory signature) external payable {
        mintTo(msg.sender, tokenId, price, hash, signature);
    }
