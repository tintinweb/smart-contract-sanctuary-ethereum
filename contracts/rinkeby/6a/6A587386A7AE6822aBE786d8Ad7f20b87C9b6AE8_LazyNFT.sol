...
contract LazyNFT is ERC721URIStorage, EIP712, AccessControl {
    ...
    event handleMint(uint256 pass);
    event handleMintFail(uint256 fail);
    ...
    function mint(
        ...
        if (signer == signerVerify) {
            ...
            emit handleMintFail(0);
            return "Error in Mint";
        }
    }
    ...
    function _hash(NFTVoucher calldata voucher, bytes memory signature)
        internal
        view
        returns (address)
    {
 ...
 function _verify(NFTVoucher calldata voucher, bytes memory signature)
        ...
        returns (address)
    {
        address digest = _hash(voucher, signature);
