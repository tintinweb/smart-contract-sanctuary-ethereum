...
contract StrategyDutchAuction is ISynchronousStrategy {
    ...
    {
        (uint256 startPrice, uint256 endPrice, uint256 endTime) = abi.decode(
            makerAsk.strategyParams,
            (uint256, uint256, uint256)
        );
        require(startPrice > endPrice, "StrategyDutchAuction: INVALID_PRICE_RANGE");
        require(makerAsk.validFrom < endTime, "StrategyDutchAuction: INVALID_END_TIME");

        uint256 bidPrice = abi.decode(takerBid.strategyParams, (uint256));

        uint256 tickPerBlock = (startPrice - endPrice) / (endTime - makerAsk.validFrom);
        uint256 currentPrice = endTime <= block.timestamp
            ? endPrice
            : startPrice - (block.timestamp - makerAsk.validFrom) * tickPerBlock;
        return (
