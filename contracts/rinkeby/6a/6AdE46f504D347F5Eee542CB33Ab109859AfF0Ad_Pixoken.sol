...
contract Pixoken is ERC721r, Ownable, VRFConsumerBase {
    ...
    constructor(string memory _baseTokenURI,  bytes32 _s_keyHash, address _linkToken, address _linkCoordinator,uint256 _totalSupply) ERC721r("Pixoken", "PXKN",_totalSupply)
    ...
    // mint before release
    function mintPxkn(uint256 quantity)
        public
        payable
    {
        require(tx.origin == msg.sender,"CONTRACTS_NOT_ALLOWED_TO_MINT");
        require((totalMinted + quantity) <= TOTAL_SUPPLY);
        require(stoppedEarlyMint == false, "Early mint has ended");
        require(quantity <= 5, "Only up to 5 mohawks can be minted at once");
        uint cost;
        unchecked {
            cost = quantity * MINT_PRICE;
        }
    ...
    // mint after release 
    function mintReleased(uint256 quantity)
    public
    payable
    {
//        require(block.timestamp >= releaseDate, "Released sale hasn't started yet");
        ...
        uint cost;
        unchecked {
            cost = quantity * RELEASED_MINT_PRICE;
        }
        require(msg.value == cost, "wrong payment");
