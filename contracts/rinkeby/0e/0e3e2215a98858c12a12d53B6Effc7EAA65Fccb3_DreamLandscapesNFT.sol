...
abstract contract DreamSeedProduct is ERC721TradableBurnable {
  ...
  bytes4 constant sigOwnerOfSeed = bytes4(keccak256("ownerOf(uint256)"));
  bytes4 constant sigBurnSeed = bytes4(keccak256("burn(uint256)"));
  ...
  function isOwnerOfSeed(uint256 seedTokenId, address ownerQuery) public view returns (bool) {
    return contractDreamSeeds.ownerOf(seedTokenId) == ownerQuery;

    /*
    // check if owner of seedToken
    bytes memory data = abi.encodeWithSelector(sigOwnerOfSeed, seedTokenId);
    (bool success, bytes memory returnedData) = contractDreamSeeds.call(data);
    require(success);
    address addressSeedOwner =  abi.decode(returnedData, (address));
    //require(addressSeedOwner == ownerQuery, "not owner");

    return addressSeedOwner == ownerQuery;*/
  }


  function burnDreamSeed(uint256 seedTokenId) internal {
    require(isOwnerOfSeed(seedTokenId, msg.sender), "not owner of dream seed");
    contractDreamSeeds.burn(seedTokenId);

    /*// burn
    {
      bytes memory data = abi.encodeWithSelector(sigBurnSeed, seedTokenId);
      (bool success,) = contractDreamSeeds.call(data);
      require(success); 
    }*/

    /*// check if owner of seedToken
    address addressSeedOwner = contractDreamSeeds.ownerOf(seedTokenId);
    require(addressSeedOwner == msg.sender, "only owner can burn seed");
