 ...
 */
 ...
 contract TokenVaultFactory is Ownable {
  /***************************** Public variable ****************************/
    uint256 public topIndex; // The smallest un-used index of 'tokenVaultContracts'
    
    /**************************** Private variable ****************************/
    TokenVault[] private _tokenVaultContracts;
    
    /********************************** Event *********************************/
    event CreateTokenVault(address indexed _address, uint256 indexed _index);
    ...
    function getContractAddressByIndex(uint256 index) public view returns (address) {
        ...
        return address(_tokenVaultContracts[index]);
    }
    
    function getManagerAddressByIndex(uint256 index) public view returns (address) {
        require(index < topIndex, "You cannot query for nonexistent contract.");
        return _tokenVaultContracts[index].owner();
    }
    ...
    function create(
        ...
        // Transfer ownership from "Alice" to "NFT contract"
        newTokenVault.transferOwnership(ownerShip);
        
        emit CreateTokenVault(address(newTokenVault),topIndex);
...
interface IHSNLNFT is IERC721 {
 ...
 contract TokenVault is ERC4626,Ownable{
    ...
    /******************************** Constant ********************************/
    address public immutable paymentToken; //the address of payment token.
    address public immutable manager; // the address who can withdraw the money.
    
    /***************************** Public variable ****************************/
    mapping(uint => address) public tokenBuyer; // tokenId => # of address of buyer
    ...
    ) ERC4626(IERC20Metadata(_paymentToken)) 
    ERC20(_name, _symbol) {
    ...
    function withdrawAll(
        ...
        require(_msgSender()<=manager, "You are not allowed for withdraw token.");

        uint256 assets=totalAssets();
        address buyer=tokenBuyer[tokenId];

        //check the ownership of NFT
        if(IHSNLNFT(owner()).ownerOf(tokenId)==receiver){
            withdraw(assets, receiver, buyer);
        }else{
            revert();
        }
        return true;
    ...
    function mint(uint256 shares, address receiver) public override returns (uint256) {
        require(_msgSender()<=owner(),"You are not allowed for mint token.");
    ...
    function withdraw(
        ...
        require(assets <= maxWithdraw(buyer), "ERC4626: withdraw more then max");

        address caller = _msgSender();
        uint256 shares = previewWithdraw(assets);

        _burn(buyer, shares);
       TransferFromHelper.safeTransferFrom(paymentToken, address(this), receiver, assets);

        emit Withdraw(caller, receiver, buyer, assets, shares);
        return shares;
    }
