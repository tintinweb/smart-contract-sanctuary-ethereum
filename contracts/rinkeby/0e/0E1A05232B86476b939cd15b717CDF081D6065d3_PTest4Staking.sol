...
contract PTest4Staking is Ownable, ReentrancyGuard, IERC721Receiver {
    using SafeERC20 for IERC20;
    
    IERC20 public token = IERC20 (0x89B70d6268DF4686e0958F6a5A908E60FFDB6014);
    address public nftContract = address (0xC64E35893511AFA328FB1DfAe08D2294B63205E5);
    ...
    struct UserLock {
		...
		uint256[] nftIds; 		// ids of nfts being fed
    ...
    function lock(uint256 _amount, uint256[] memory nftIds) public nonReentrant {
		...
		for (uint i = 0; i < nftIds.length; i++) {
			require(pochiContract.ownerOf(nftIds[i]) == msg.sender, 'Not owner of NFT');
        }
		...
		for (uint i = 0; i < nftIds.length; i++) {
			bool fFound = false;
			for (uint j = 0; j < userLocks[msg.sender].nftIds.length; j++) {
				if (nftIds[i] == userLocks[msg.sender].nftIds[j]) {
					fFound = true;
				}
			}
			if (fFound == false) {
				userLocks[msg.sender].nftIds.push(nftIds[i]);
			}
        }
    ...
    function claimLock(uint256 _amount) public nonReentrant {
		...
		if (userLocks[msg.sender].nftIds.length > 0) {
			...
			if (userLocks[msg.sender].tokenAmount == 0) {
				for (uint256 i = userLocks[msg.sender].nftIds.length; i > 0; i--) {
					pochiContract.takeFeedAll(userLocks[msg.sender].nftIds[i-1]);
					userLocks[msg.sender].nftIds.pop();
				}
			}
			else
			{
	...
	function burnTokensForNft(uint256 _nftId) public nonReentrant {
		...
		if (userLocks[msg.sender].nftIds.length > 0) {
			for (uint256 i = 0; i < userLocks[msg.sender].nftIds.length - 1; i++) {
				if (_nftId == userLocks[msg.sender].nftIds[i]) {
					userLocks[msg.sender].nftIds[i] = userLocks[msg.sender].nftIds[userLocks[msg.sender].nftIds.length - 1];
					userLocks[msg.sender].nftIds.pop();
				}
			}
		}
