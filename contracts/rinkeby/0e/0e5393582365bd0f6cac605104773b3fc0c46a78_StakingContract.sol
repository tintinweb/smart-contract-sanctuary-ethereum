...
contract StakingContract {
    ...
    function LeaveBattle(uint256 count) public {
       ...
       Battle storage battle = battles[count];
        ...
        if(!battle.joined){
            ...
            require(block.timestamp > (battle.startingTime + 2 minutes),"You canot leave battle before 2 minutes.");
    ...
    function GenWithdraw(uint256 _percentage) external {
        ...
        if(_percentage == 3){
            ...
            genLastTransaction[msg.sender] = block.timestamp + 2 minutes;
            ...
            require(genLastTransaction[msg.sender] < block.timestamp,"You canot withdraw amount before 24 hours");
            genLastTransaction[msg.sender] = block.timestamp + 3 minutes;
    ...
    function getAreenaReward(uint256 _walletLimit) external {
        
        Player storage player = players[msg.sender];

        uint256 walletBalance = arenaToken.balanceOf(msg.sender);

        require(walletBalance < _walletLimit,"Please Buy Areena Boster To get All of your reward.");

        uint256 sendAmount = _walletLimit - walletBalance;

        arenaToken.transferFrom(areenaWallet, msg.sender, sendAmount);

        player.totalArenaTokens -= sendAmount;

    }

    function initialAmountSend() public view returns (uint256) {
        
        uint256 busdPrice = 1; // 1$
        uint256 tokenAmount = busdPrice.mul(initialPrice);
        return tokenAmount;
    }

    function afterAmountSend() public view returns (uint256) {
        
        uint256 busdPrice = 1; // 1$
        uint256 tokenAmount = busdPrice.mul(afterPrice);
        return tokenAmount;
       
    }

    uint256 private checkValue;

    function sell(uint256 amount) external {
        
        require(amount <= 3, "You can sell only three areena Token per day.");
        ...
        if(checkValue == 0){
            require(busdToken.balanceOf(busdWallet) > 102000,"Selling of Areena token will start when areena wallet reaches 102000.");
            checkValue++;
        }
        
        uint256  lowerMileStone = 101000;
        uint256  uppermileStone = 102999;
        uint256  lowerSetMileStone = 999;
        uint256  upperSetMileStone = 1000;

        if(busdToken.balanceOf(busdWallet) > lowerMileStone && busdToken.balanceOf(busdWallet) <= uppermileStone){

            if(busdToken.balanceOf(owner) > 200000){

                require(block.timestamp > areenaLastTransaction[msg.sender],"You canot sell areena token again before 24 hours.");

                uint256 sendAmount = afterAmountSend();
                busdToken.transferFrom(busdWallet,msg.sender, sendAmount.mul(1e18));   
                ...
                uint256 sendAmount = initialAmountSend();
                busdToken.transferFrom(busdWallet,msg.sender, sendAmount.mul(1e18));   
                areenaLastTransaction[msg.sender] = block.timestamp + 4 minutes;
            }
