...
contract CryptoMaidsOnChain is ERC721, AccessOperatable {
  ...
  // map of token id and timestamp
  mapping (uint256 => uint256) private _acquiredAt;
  ...
  function acquiredAt(uint256 tokenId) external view returns (uint256) {
    require(_exists(tokenId), "Token is not exist.");

    return _acquiredAt[tokenId];
  }

  // only operator can consume staking time
  function consumeStakingTime(uint256 tokenId, uint256 amount) public onlyOperator() {
    require(_acquiredAt[tokenId] != 0, "Invalid token id.");
    require(_acquiredAt[tokenId] + amount <= block.timestamp, "The amount is too large.");
    _acquiredAt[tokenId] += amount;
  }
  ...
  function _afterTokenTransfer(
    ...
    _acquiredAt[tokenId] = block.timestamp;
  }
