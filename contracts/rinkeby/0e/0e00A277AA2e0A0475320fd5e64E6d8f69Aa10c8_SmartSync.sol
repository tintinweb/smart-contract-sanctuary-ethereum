...
contract SmartSync is Ownable {
    ...
    function receivePayment(
        address _payee,
        uint256 _userId,
        string calldata _meetingId,
        uint256 _amountInUsd,
        uint256 _amountInNativeOrToken,
        bool _isNativePay
    ) external payable {
        if (_amountInNativeOrToken == 0) revert ZeroInput();

        // If payment is in native currency
        if (_isNativePay) {
            if (msg.value < _amountInNativeOrToken)
                revert InsufficientPayment();
        }
        // Otherwise, if payment is in tokens
        else {
            token.safeTransferFrom(
                msg.sender,
                address(this),
                _amountInNativeOrToken
            );
        }
