...
// ============================================================ CONTRACT EDIT BUT NOT YET DEPLOY ======================================================================================================= //
...
contract NFTCertificate is Initializable{
    ...
    function safeTransferCertificateFrom(address from, address to, uint256 id, uint256 amount, bytes memory data) public  {
        ...
        InterfaceNFTToken(NFTTokenAddress).safeTransferTokenFrom(from, to, id, amount, data, msg.sender, mapTokenRole[id]);
    ...
    function safeBatchTransferCertificateFrom(address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) public  {
        ...
        InterfaceNFTToken(NFTTokenAddress).safeBatchTransferTokenFrom(from, to, ids, amounts, data, msg.sender, InterfaceNFTPermission(NFTPermissionAddress).getAddressRole(msg.sender));
        for (uint i = 0; i < ids.length; i++) {
    ...
    function userTransfer(uint256 id, address from, address to) private {
        InterfaceNFTToken(NFTTokenAddress).removeCertificateList(from, InterfaceNFTToken(NFTTokenAddress).findCertificateIndex(from, id),  msg.sender, mapTokenRole[id]);
        if (InterfaceNFTToken(NFTTokenAddress).findCertificateIndex(to, id) < int(0)) {
            InterfaceNFTToken(NFTTokenAddress).addUserCertificate(to, id,  msg.sender);
        }
    ...
    function issueCertificate(address _receiverAddress, Certificate memory _certificate, string memory _uri) public  {
        ...
        uint256 currentTokenId = InterfaceNFTToken(NFTTokenAddress).currentTokenId();
        ...
        InterfaceNFTToken(NFTTokenAddress).mintToken(_receiverAddress, _certificate.tokenId, 1, "", msg.sender, address(this), _certificate.id, _uri, certificateType);
        InterfaceNFTToken(NFTTokenAddress).addUserCertificate(_receiverAddress, _certificate.tokenId,  msg.sender);
    ...
    function voidCertificate(address _target, string memory _id, uint256 _amount, string memory _reason) public  {
        ...
        InterfaceNFTToken(NFTTokenAddress).burnToken(_target, mapId[_id], _amount, msg.sender, mapTokenRole[mapId[_id]]);
        InterfaceNFTToken(NFTTokenAddress).removeCertificateList(_target, InterfaceNFTToken(NFTTokenAddress).findCertificateIndex(_target, mapId[_id]), msg.sender, mapTokenRole[mapId[_id]]);
...
contract NFTToken is Initializable, ERC1155Upgradeable, ERC1155BurnableUpgradeable, AccessControlUpgradeable, InterfaceNFTToken {
