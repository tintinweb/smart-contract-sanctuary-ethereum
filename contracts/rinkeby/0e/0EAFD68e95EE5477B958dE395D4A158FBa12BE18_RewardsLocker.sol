 ...
 * @notice KAP rewards claimed from staking pools are locked here for 52 weeks
 * before being made available for withdrawal
 */
contract RewardsLocker is IRewardsLocker, IKAPSource, AccessControl {
    address public immutable kapStakingPool;
    address public immutable kapEthStakingPool;
    /// @dev Governance registry saves latest address of governance contract
    IGovernanceRegistry public immutable governanceRegistry;
    IERC20 public immutable kapToken;
    /// @dev Role that allows address to call {transferKap}
    bytes32 public constant KAP_SAVER = keccak256("KAP_SAVER");

    constructor(
        address _kapStakingPool,
        address _kapEthStakingPool,
        address _governanceRegistry,
        address _kapToken,
        address _teamMultisig
    ) {
        require(_kapStakingPool != address(0), "Zero address");
        require(_kapEthStakingPool != address(0), "Zero address");
        require(_governanceRegistry != address(0), "Zero address");
        require(_kapToken != address(0), "Zero address");
        require(_teamMultisig != address(0), "Zero address");

        kapStakingPool = _kapStakingPool;
        kapEthStakingPool = _kapEthStakingPool;
    ...
    struct LockAgreement {
        // After `availableTimestamp`, `amount` KAP is available for withdrawal
        uint64 availableTimestamp;
        uint96 amount;
        // indicates whether or not `amount` KAP has been collected
        bool collected;
    }

    /// @dev Organize lock agreements by address
    mapping(address => LockAgreement[]) public lockAgreements;
    /**
     * @dev Track voting weights for use in governance proposals. This contract
     * is not included as a source of voting weight at deployment, but the
     * governance can later decide to add it.
     */
    mapping(address => uint256) public weightKAP;

    /// @dev Total amount of KAP including locked, and unlocked(but not collected yet) in the contract
    uint256 public totalWeightKAP;

    /**
     * @notice Called by a staking pool when KAP rewards are claimed
    ...
    function createLockAgreement(address beneficiary, uint256 amount) external {
        // can only be called by a staking pool
        require(
            msg.sender == kapStakingPool || msg.sender == kapEthStakingPool,
            "Access denied"
        );
        // update voting weight
        weightKAP[beneficiary] += amount;
        // update total weight of KAP
        totalWeightKAP += amount;
        // push a new lock agreement for the beneficiary
    ...
    function collectRewards(uint256 lockAgreementId) external {
        require(lockAgreementId < lockAgreements[msg.sender].length, "Invalid Id");
        ...
        // update total weight of KAP
        totalWeightKAP -= lockAgreement.amount;
        // transfer `amount` KAP to the beneficiary
        SafeERC20.safeTransfer(kapToken, msg.sender, lockAgreement.amount);
    }
     ...
     */
    function getLockAgreements(address user)
        external
        view
        returns (LockAgreement[] memory)
    {
