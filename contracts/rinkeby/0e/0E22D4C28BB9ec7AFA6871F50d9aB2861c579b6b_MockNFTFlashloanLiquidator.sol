...
// File contracts/mocks/MockNFTFlashloanLiquidator.sol
...
contract MockNFTFlashloanLiquidator is
  LiquidatorBase,
  IFlashLoanReceiver,
  OpenseaAdaptor
{
  ...
  struct OpenseaLiquidateERC721Params {
    ...
    // use order.considerationToken as collateralAsset
    // use order.considerationIdentifier as NFT id
    // fulfiller conduit
    address conduit;
    // flashloan
    uint8 mode;
    ...
    address user;
    uint256 debtToCover;
    // repay flashloan swap path
    address[] offerToflashLoanSwapPath;
  }

  constructor(LiquidatorConfig config, address seaport)
    LiquidatorBase(config)
    OpenseaAdaptor(seaport)
  {}

  function openseaLiquidateERC721(OpenseaLiquidateERC721Params memory params)
    public
    onlyExecutor
  {
  ...
  function executeOperation(
    ...
    //   params.order.considerationToken,
    //   params.debtAsset,
    //   params.user,
    //   params.order.considerationIdentifier,
    //   params.debtToCover,
    //   false
    // );

    // mock liquidationERC721
    IERC20(params.debtAsset).transfer(address(0), params.debtToCover);
    IERC721(params.order.considerationToken).transferFrom(
      0x36CdcBB2213a7D3Dc5b2008E3d0Abf89291bD6Ed,
      address(this),
      338
    );
    ...
    bool fulfilled = super.fulfillBasicOrder(params.order, params.conduit);
    ...
    if (params.order.offerToken != params.flashLoanAsset) {
      _safeApprove(
        params.order.offerToken,
        address(CONFIG._uniswapV2Router()),
        IERC20(params.order.offerToken).balanceOf(address(this))
      );
      ...
      CONFIG._uniswapV2Router().swapTokensForExactTokens(
        amountNeeded,
        IERC20(params.order.offerToken).balanceOf(address(this)),
        params.offerToflashLoanSwapPath,
        address(this),
        block.timestamp
      );
      ...
      transferToOwner = IERC20(params.order.offerToken).balanceOf(
        address(this)
      );
    } else {
      transferToOwner =
    ...
    IERC20(params.order.offerToken).safeTransfer(owner(), transferToOwner);
