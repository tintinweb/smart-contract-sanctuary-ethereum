...
contract MergeMana is Ownable {
  ...
  uint256 public manaBoostPerToken;
  bool public isChannelingActive = false;
  ChunkedDataStorage public baseManaStorage;
  uint public maxBoostBalance;
  uint public baseManaMultiplier;

  mapping(uint256 => uint256) public mana;
  mapping(uint256 => uint256) public channelingDestTokenId;
  mapping(uint256 => uint256) public channeledMana;
  mapping(uint256 => uint256) public numChannelingDestTokenId;

  event ChangedEmergencyShutdown(bool shutdown);
  event ChangedIsChannelingActive(bool isChannelingActive);
  event BoostedMana(address booster, uint256 mana, uint256[] tokenIds);
  event ChanneledStatue(uint256 from, uint256 to, uint256 manaAmount);

  constructor(
    address _merge,
    address _baseManaStorage,
    uint256 _manaBoostPerToken,
    uint256 _maxBoostBalance,
    uint256 _baseManaMultiplier,
    uint256 _numDecimals
  ) {
    manaBoostPerToken = _manaBoostPerToken;
    merge = Merge(_merge);
    manaBoostPerToken = _manaBoostPerToken;
    baseManaStorage = ChunkedDataStorage(_baseManaStorage);
    maxBoostBalance = _maxBoostBalance;
  ...
  function setMaxBoostBalance(uint256 _maxBoostBalance) public onlyOwner {
    maxBoostBalance = _maxBoostBalance;
  }

  function setManaBoostPerToken(uint256 _manaBoostPerToken) public onlyOwner {
    manaBoostPerToken = _manaBoostPerToken;
  }
  ...
  function setIsChannelingActive(bool _isChannelingActive) public onlyOwner {
    isChannelingActive = _isChannelingActive;
    emit ChangedEmergencyShutdown(isChannelingActive);
  }
  ...
  modifier onlyIsNotMergeOrChannelingNotActive() {
    require(
      !isMergeByDifficulty() && !isChannelingActive,
      'MergeMana: Action only can occur before merge'
    );
    _;
  }

  modifier onlyIsMergeOrActive() {
    require(
      isMergeByDifficulty() || isChannelingActive,
   ...
   * Mana based logic
   */
  function getTotalUsableMana(uint256 id) public view returns (uint256) {
    if (channelingDestTokenId[id] != 0) {
      return 0;
    }
    if (address(baseManaStorage) == address(0)) {
      return manaBoostPerToken * mana[id];
    }
    return manaBoostPerToken * mana[id] + uint256(uint8(baseManaStorage.indexToData(id)[0])) * baseManaMultiplier;
  }

  function _channelMana(uint256 from, uint256 to) internal {
    require(to != 0, 'MergeMana: Can not channel to tokenId zero');
    require(
      channelingDestTokenId[from] == 0,
      'MergeMana: Mana has already been channeled'
    );
    uint256 usableMana = getTotalUsableMana(from);
    channeledMana[to] += usableMana;
    numChannelingDestTokenId[to]++;
    channelingDestTokenId[from] = to;
    mana[from] = 0;
    emit ChanneledStatue(from, to, usableMana);
  }

  function batchChannelMana(uint256[] memory from, uint256 to)
    public
    onlyIsMergeOrActive
    onlyIsNotShutdown
  {
  ...
  function getBoostManaAmount(address booster) public view returns (uint256) {
    uint balance = merge.balanceOf(booster);
    return
      (balance > maxBoostBalance ? maxBoostBalance : balance) *
      merge.getCurrentRarityScore(booster);
  }

  function boostMana(uint256[] memory tokenIds)
    public
    onlyIsNotMergeOrChannelingNotActive
    onlyIsNotShutdown
  {
    uint256 boostedMana = getBoostManaAmount(msg.sender);
    for (uint256 i = 0; i < tokenIds.length; ++i) {
      ...
      mana[tokenIds[i]] = boostedMana;
    }
    emit BoostedMana(msg.sender, boostedMana, tokenIds);
  }
}
