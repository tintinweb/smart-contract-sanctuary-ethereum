...
contract M30WRebellion is ERC721A, Ownable {
    ...
    string public contractURI = ""; //opensea description
    ...
    modifier mintCompliance(uint256 _mintAmount) { //useless
    ...
    function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) { //useless
    ...
    function setMerkleRoot(bytes32 _merkleRoot) public onlyOwner { //useless
        merkleRoot = _merkleRoot;
    }

    function setWhitelistMintEnabled(bool _state) public onlyOwner { //useless
