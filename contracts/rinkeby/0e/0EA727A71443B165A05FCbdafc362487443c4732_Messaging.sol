...
contract Messaging {
    ...
    */
    ...
    function getAllChatsInfo() external view returns (Chat[] memory) {
        string[] memory chats = getChatNames();
        Chat[] memory chatsInfo;
        for (uint256 i = 0; i < chats.length; i++) {
            chatsInfo[i] = chatRoomToMessages[chats[i]];
        }
        return chatsInfo;
    }
    ...
    function getChatNames() public view returns (string[] memory) {
        return chatNames;
    }
