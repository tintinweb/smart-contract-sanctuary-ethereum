...
import "./StarXCelebrity.sol";

contract StarXCelebrityRegistry is Ownable {
    event CelebContractCreated(
        address indexed celebWallet,
        address indexed newCelebContract,
        uint256 newCelebContractCount,
        address starXAddressRegistry
    );
    ...
    //---------------------------------- v1 - functions - start ------------------------------------
    function getCelebrityContractCount() external view returns (uint256) {
        return m_celebCount;
    }

    function getCelebrityContractAtIndex(uint256 _index)
        external
        view
        returns (address)
    {
        return m_celebToIndexRev[_index];
    }
    ...
    function addCelebContract(
        ...
        require(
            _celebWallet != address(0),
            "invalid address for arg: _celebWallet"
        );
        // check: valid address for StarXAddressRegistry
        require(
            _starXAddressRegistry != address(0),
            "invalid address for arg: _starXAddressRegistry"
        );
    ...
    function deployAssetContractForCelebrity(
        ...
        // check: specified celeb contract exists
        require(
            m_celebToIndex[_celebrityContractAddress] > 0,
            "non-existent address for arg: _celebrityContractAddress"
        );
        ...
        return
            IStarXCelebrity(_celebrityContractAddress)
    ...
    function mintNewSecurityForCelebAndAssetContract(
        ...
        require(
            ...
            "non-existent address for arg: _celebContract"
        );
        // effect: do mint
...
contract StarXCelebrity is Ownable, IStarXCelebrity {
    ...
    event UpdateStarXCelebrityRegistry(
        address oldStarXCelebrityRegistry,
        address newStarXCelebrityRegistry
    );
    ...
    address private m_starXCelebRegistry;
    ...
    modifier modOnlyApproved() {
        ...
        require(
            IStarXAddressRegistry(m_starXAddressRegistry).adminNftContracts(
                _msgSender()
            ) || _msgSender() == m_starXCelebRegistry,
            "caller must be either StarXAdmin or StarXAddressRegistry"
        );
        _;
    }
    ...
    constructor(
        ...
        // set starting address of contract = StarXCelebrityRegistry
        m_starXCelebRegistry = _starXCelebRegistry;

        // emit event(s)
        emit UpdateStarXAddressRegistry(address(0), m_starXAddressRegistry);
        emit UpdateStarXCelebrityRegistry(address(0), m_starXCelebRegistry);
    }
        ...
        modOnlyApproved
    {
        require(
            ...
            "update StarXAddressRegistry failed: arg = zero-address"
        );
        // emit event
    ...
    function getStarXCelebrityRegistry() external view returns (address) {
        return m_starXCelebRegistry;
    }

    function updateStarXCelebrityRegistry(address _newAddress)
        public
        modOnlyApproved
    {
        require(
            _newAddress != address(0),
            "update StarXCelebrityRegistry failed: arg = zero-address"
        );
        // emit event
        emit UpdateStarXCelebrityRegistry(m_starXCelebRegistry, _newAddress);
        // update
        m_starXCelebRegistry = _newAddress;
    }
    ...
    function deployFinancialSecurityAssetContract(
        IRequest.DeployAssetContractRequest calldata _req
    ) external virtual override modOnlyApproved returns (address) {
        // init new contract
    ...
    function mintNewSecurity(
        ...
        IRequest.ListAssetRequest calldata _req
    ) external virtual override modOnlyApproved returns (uint256 newTokenId) {
...
contract StarXFinancialSecurityAsset is ERC1155Burnable {
        ...
        returns (AssetInfo memory)
    {
        require(
            ...
            "ERR: getFinancialSecurityAsset; DNE: token id"
        );
        ...
        modOnlyApproved
    {
        require(
            _newAddress != address(0),
            "invalid address arg for StarXCelebrityRegistry"
        );
        // emit event
        emit UpdateStarXCelebrityRegistry(
        ...
        // do update
        m_deployingStarXCelebrity = _newAddress;
    }
        ...
        modOnlyApproved
    {
        ...
        // emit event
        emit UpdateStarXAddressRegistry(
            m_starXAddressRegistry,
            _newAddress,
            m_name
        );
