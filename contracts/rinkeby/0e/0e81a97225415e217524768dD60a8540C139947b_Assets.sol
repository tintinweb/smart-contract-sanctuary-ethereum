...
contract Assets is Ownable, Pausable {
    enum AssetType {
        DATASET,
        MODEL,
        FORECAST
    ...
    /// @dev Treasury Accrued Slashed Balance
    uint256 public treasuryBalance;

    /// @dev
    address public skillScoreVerifier;

    /// @dev number of providers, assets, and order TODO: Replace with Enumerable
    uint256 public numAssets;

    /// @notice slashing grace period, time until which a malicious data poster can be slashed
    uint256 public stakeGracePeriod;

    /// @notice Required amount of payment token to stake in order to post data to prevent malicious actors
    /// @dev if > 0 then this is active in publish()
    uint256 public stakeReq;

    /// @notice Currently pending staked amounts that can be claimed after stakeGracePeriod
    mapping(uint256 => uint256) public staked;

    /// @notice Datasets, Models, Forecasts
    ...
    constructor(
        ...
        skillScoreVerifier = msg.sender;
        providers = IProviders(_providers);
    }

    function setSkillScoreVerifier(address _skillScoreVerifier)
        external
        onlyOwner
    {
        skillScoreVerifier = _skillScoreVerifier;
    }

    function setStakeReq(uint256 _stakeReq) external onlyOwner {
        stakeReq = _stakeReq;
    }
    ...
    function setStakeGracePeriod(uint256 _stakeGracePeriod) external onlyOwner {
        stakeGracePeriod = _stakeGracePeriod * 1 days;
    }

    function withdrawStake(uint256 _assetId) external {
        Asset memory asset = assets[_assetId];
        require(asset.creator == msg.sender, "!creator");
        require(
            asset.timestamp + stakeGracePeriod < block.timestamp,
            "!timestamp"
        );
        uint256 oldBalance = staked[_assetId];
        staked[_assetId] = 0;
        IERC20(paymentToken).transfer(msg.sender, oldBalance);
    }
    ...
    function publish(
        ...
        ) = providers.providers(_providerId);
        require(providerId != 0, "Provider does not exist");

        if (price != 0) {
            require(
                price >= minAssetPrice,
                "Asset Price must be greater than minimum Provider price"
            );
        }
        numAssets++;
        ...
        nftdclim.newAsset(msg.sender, numAssets);
        if (stakeReq > 0) {
            staked[numAssets] = stakeReq;
            require(
        ...
        emit Published(msg.sender, metadataCID, metadata, numAssets, price);
    }

    function updatePrice(uint256 _assetId, uint256 _price) external {
        address assetOwner = nftdclim.ownerOf(_assetId);
        require(assetOwner == msg.sender, "403");
    ...
    function updateSkillScore(
        ...
        require(msg.sender == skillScoreVerifier, "Unauthorized");
        Asset storage asset = assets[_assetId];
        asset.skillScore = _skillScore;
        emit SkillScoreUpdated(_assetId, _skillScore, _reportCID);
    }

    function slashPublisher(uint256 _assetId) external onlyOwner {
        Asset memory asset = assets[_assetId];
        require(
            asset.timestamp + stakeGracePeriod > block.timestamp,
            "!timestamp"
        ...
        uint256 oldBalance = staked[_assetId];
        staked[_assetId] = 0;
        treasuryBalance += oldBalance;
        delete assets[_assetId];
        emit Slashed(_assetId, oldBalance);
    }
