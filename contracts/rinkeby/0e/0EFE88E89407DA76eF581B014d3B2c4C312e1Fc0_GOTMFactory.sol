...
import "./token/ERC721Factory.sol";

contract GOTMFactory is ERC721Factory {
    constructor(string memory baseURI_, OptionConfig memory optionConfig_, address payable erc721OptimizedAddress_, address proxyRegistryAddress_) ERC721Factory(
...
contract ERC721Optimized is Context, SharedOwnable, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    struct AddressData {
        uint128 balance;
        uint64 privateMintCount;
        uint64 publicMintCount;
    }
    ...
    uint256 private _mintedToTeam;
    uint256 private _mintedToOwners;
    uint256 private _raffledToOwners;
    mapping(uint256 => uint256) private _raffleToOwnersHelper;
    uint256 private _raffledToVeteranOwners;
    mapping(uint256 => uint256) private _raffleToVeteranOwnersHelper;
    ...
    constructor(string memory name_, string memory symbol_, string memory baseURI_, MintConfig memory privateMintConfig_, MintConfig memory publicMintConfig_, address erc721FactoryAddress_, address proxyRegistryAddress_) {
        ...
        require(publicMintConfig_.maxMintAmountPerAddress <= publicMintConfig_.maxTotalMintAmount, "ERC721Optimized: maximum mint amount per address can't exceed the maximum total mint amount");
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IERC721).interfaceId || interfaceId == type(IERC721Metadata).interfaceId || interfaceId == type(IERC721Enumerable).interfaceId || interfaceId == type(IERC165).interfaceId;
    }
    ...
    function mintedToTeam() external view returns (uint256) {
        return _mintedToTeam;
    }

    function mintedToOwners() external view returns (uint256) {
        return _mintedToOwners;
    }

    function raffledToOwners() external view returns (uint256) {
        return _raffledToOwners;
    }

    function raffledToVeteranOwners() external view returns (uint256) {
        return _raffledToVeteranOwners;
    }

    function withdraw(address payable recipient) external onlySharedOwners {
    ...
    function privateMint(uint64 amount) external payable {
        require(block.timestamp >= _privateMintConfig.mintStart, "ERC721Optimized: private mint has not yet started");
        require(block.timestamp < _privateMintConfig.mintEnd, "ERC721Optimized: private mint has not yet started");
        address msgSender = _msgSender();

        require(_privateMintWhitelist[msgSender] > 0, "ERC721Optimized: not whitelisted for the private mint");
        require(_totalSupply + amount <= _privateMintConfig.maxTotalMintAmount, "ERC721Optimized: exceeds total mint maximum");
        require(_addresses[msgSender].privateMintCount + amount <= _privateMintConfig.maxMintAmountPerAddress, "ERC721Optimized: exceeds mint maximum");
        uint128 discount = _privateMintDiscountPerMintAmount[amount];
        uint256 discountedPrice = ((amount * _privateMintConfig.pricePerMint) * (100 - discount)) / 100;
        require(msg.value >= discountedPrice, "ERC721Optimized: missing funds");
        _safeMint(msgSender, msgSender, amount);
        _addresses[msgSender].privateMintCount += amount;
    }

    function publicMint(uint64 amount) external payable {
        require(block.timestamp >= _publicMintConfig.mintStart, "ERC721Optimized: public mint has not yet started");
        require(block.timestamp < _publicMintConfig.mintEnd, "ERC721Optimized: public mint has not yet started");
        address msgSender = _msgSender();

        require(_totalSupply + amount <= _publicMintConfig.maxTotalMintAmount, "ERC721Optimized: exceeds total mint maximum");
        require(_addresses[msgSender].publicMintCount + amount <= _publicMintConfig.maxMintAmountPerAddress, "ERC721Optimized: exceeds mint maximum");
        uint128 discount = _publicMintDiscountPerMintAmount[amount];
        uint256 discountedPrice = ((amount * _publicMintConfig.pricePerMint) * (100 - discount)) / 100;
        require(msg.value >= discountedPrice, "ERC721Optimized: missing funds");
        _safeMint(msgSender, msgSender, amount);
        _addresses[msgSender].publicMintCount += amount;
    }

    function factoryMint(address to, uint64 amount) external onlyERC721Factory {
        require(block.timestamp >= _publicMintConfig.mintStart, "ERC721Optimized: public mint has not yet started");
        require(block.timestamp < _publicMintConfig.mintEnd, "ERC721Optimized: public mint has not yet started");
        
        require(_totalSupply + amount <= _publicMintConfig.maxTotalMintAmount, "ERC721Optimized: exceeds total mint maximum");
        require(_addresses[to].publicMintCount + amount <= _publicMintConfig.maxMintAmountPerAddress, "ERC721Optimized: exceeds mint maximum");
        _safeMint(_msgSender(), to, amount);
        _addresses[to].publicMintCount += amount;
    }

    function mintToTeamSingleAddress(uint64 tokensToMint, address account) external onlySharedOwners {
        require(_mintedToTeam + tokensToMint <= MAX_TEAM_MINTS, "ERC721Optimized: not enough tokens left to mint to team");

        _safeMint(_msgSender(), account, tokensToMint);

        _mintedToTeam += tokensToMint;
    }

    function mintToTeamMultipleAddresses(uint256 tokensToMint, address[] calldata addresses) external onlySharedOwners {
        require(_mintedToTeam + tokensToMint <= MAX_TEAM_MINTS, "ERC721Optimized: not enough tokens left to mint to team");
        require(tokensToMint == addresses.length, "ERC721Optimized: array size mismatch");

        for (uint256 index = 0; index < addresses.length; index++)
            _safeMint(_msgSender(), addresses[index], 1);

        _mintedToTeam += tokensToMint;
    }

    function mintToOwners(uint256 tokensToMint, uint256 startTokenId) external onlySharedOwners {
        require(startTokenId + tokensToMint <= _totalSupply, "ERC721Optimized: not enough tokens");

        for (uint256 tokenId = startTokenId; tokenId < startTokenId + tokensToMint; tokenId++)
            _safeMint(_msgSender(), _tokens[tokenId].owner, 1);

        _mintedToOwners += tokensToMint;
    }
    ...
    function raffleToOwners(uint256 tokensToRaffle, uint256 maxIterations) external onlySharedOwners {
        address[] memory owners = new address[](tokensToRaffle);
        uint256 ownersFound = 0;
        uint256 iterations = 0;

        uint256 tokenId;
        while (ownersFound < tokensToRaffle && iterations < maxIterations) {
            tokenId = _getRandomExistingTokenId(iterations++);
            if (_raffleToOwnersHelper[tokenId] == 0) {
                owners[ownersFound++] = _tokens[tokenId].owner;
        ...
        _raffledToOwners += tokensToRaffle;
    }

    function resetRaffleToVeteranOwnersHelper() external onlySharedOwners {
        for (uint256 tokenId = 0; tokenId < _totalSupply; tokenId++)
            if (_raffleToVeteranOwnersHelper[tokenId] != 0)
                _raffleToVeteranOwnersHelper[tokenId] = 0;
    }

    function raffleToVeteranOwners(uint256 minimumOwningDuration, uint256 tokensToRaffle, uint256 maxIterations) external onlySharedOwners {
        address[] memory owners = new address[](tokensToRaffle);
        uint256 ownersFound = 0;
        uint256 iterations = 0;
        ...
        while (ownersFound < tokensToRaffle && iterations < maxIterations) {
            ...
            if (_raffleToVeteranOwnersHelper[tokenId] == 0) {
                ...
                _raffleToVeteranOwnersHelper[tokenId] = 1;
            }
        }
        ...
        _raffledToVeteranOwners += tokensToRaffle;
    }

    function _safeMint(address operator, address to, uint128 amount) private {
...
contract ERC721Factory is Context, SharedOwnable, IERC721, IERC721Factory {
    using Strings for uint256;
    ...
    ERC721Optimized private _erc721Optimized;
    address private _proxyRegistryAddress;
    ...
    constructor(string memory name_, string memory symbol_, string memory baseURI_, OptionConfig memory optionConfig_, address payable erc721OptimizedAddress_, address proxyRegistryAddress_) {
        ...
        ERC721Optimized erc721Optimized = ERC721Optimized(erc721OptimizedAddress_);
        erc721Optimized.privateMintConfig();
    ...
    function _canMint(uint256 _optionId) private view returns (bool) {
        ...
        ERC721Optimized.MintConfig memory publicMintConfig = _erc721Optimized.publicMintConfig();
        if (block.timestamp < publicMintConfig.mintStart)
            return false;

        if (block.timestamp >= publicMintConfig.mintEnd)
            return false;

        uint64 amount = _optionConfig.mintAmount[_optionId];
        if (_erc721Optimized.totalSupply() + amount > publicMintConfig.maxTotalMintAmount)
            return false;
