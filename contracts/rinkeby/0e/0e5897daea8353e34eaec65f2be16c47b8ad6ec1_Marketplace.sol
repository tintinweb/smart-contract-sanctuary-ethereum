...
library console {
    ...
    function setAccessPass(AccessPass calldata pass, address contractAddress) external {
    ...
    function verifyListing(uint256 tokenAmount, uint256 tokenId, address contractAddress, address owner, bool isERC1155) public view returns (bool) {
        if (isERC1155) {
            if (!ERC1155(contractAddress).isApprovedForAll(owner, address(this))) {
                return false;
            }
        } else {
            if (!ERC721(contractAddress).isApprovedForAll(owner, address(this)) && 
    ...
    function listERC721(NewListing[] calldata listings) external {
        ...
        for (; i<size; ++i) {
            ...
            InternalListing storage internalListing = saleListings[msg.sender][contractAddress][tokenId];
    ...
    function buyERC721(BuyOrder[] calldata orders) external payable {
        ...
        for (; i<size; ++i) {
            ...
            if (!verifyListing(0, orders[i].tokenId, orders[i].contractAddress, orders[i].owner, false)) {
                revert OdysseyMarket_BuyError();
            }
            
            AccessPass storage access = accessPassMap[orders[i].owner][orders[i].contractAddress];
            if (access.contractAddress != address(0)) {
                ...
                if (access.isERC1155) {
                    ...
                    for (; n < access.tokenIds.length; ++n) {
                        if (ERC1155(access.contractAddress).balanceOf(msg.sender, access.tokenIds[n]) != 0) {
                            hasPass = true;
                            break;
                        }
                    }
                ...
                } else if (ERC721(access.contractAddress).balanceOf(msg.sender) == 0) {
                    revert OdysseyMarket_NoAccess();
                }
            }

            InternalListing storage listing = saleListings[orders[i].owner][orders[i].contractAddress][orders[i].tokenId];
            for (uint256 j; j<listing.paymentInfo.length; ++j) {
                if (listing.paymentInfo[j].currencyAddress == orders[i].currencyAddress && listing.paymentInfo[j].amount != 0) {
                    _takePayment(listing.paymentInfo[j].currencyAddress, listing.paymentInfo[j].amount, listing.royalties);
                    ERC721(orders[i].contractAddress).transferFrom(orders[i].owner, msg.sender, orders[i].tokenId);
                    delete saleListings[orders[i].owner][orders[i].contractAddress][orders[i].tokenId];
                    break;
                }
            }
        }
    ...
    function listERC1155(NewListing[] calldata listings) external {
        ...
        for (; i<size; ++i) {
            ...
            if (ERC1155(contractAddress).balanceOf(msg.sender, tokenId) <  tokenAmount) {
                revert OdysseyMarket_NotOwner();
            }
    ...
    function buyERC1155(BuyOrder[] calldata orders) external payable {
        ...
        uint256 i;
        for (; i<size; ++i) {
            if (!verifyListing(orders[i].tokenAmount, orders[i].tokenId, orders[i].contractAddress, orders[i].owner, true)) {
                revert OdysseyMarket_BuyError();
            }
            ...
            for (uint256 j; j<listing.paymentInfo.length; ++j) {
                if (listing.paymentInfo[j].currencyAddress == orders[i].currencyAddress && listing.paymentInfo[j].amount != 0) {
                    _takePayment(listing.paymentInfo[j].currencyAddress, listing.paymentInfo[j].amount, listing.royalties);
                    ERC1155(orders[i].contractAddress).safeTransferFrom(orders[i].owner, msg.sender, orders[i].tokenId, orders[i].tokenAmount, "");
                    if (listing.tokenAmount == orders[i].tokenAmount) {
                        delete saleListings[orders[i].owner][orders[i].contractAddress][orders[i].tokenId];
                    } else {
    ...
    function _takePayment (address currency, uint256 amount, Royalties[] storage royalties) internal {
        // Treasury Payment 3%
        ...
        if (currency == address(0)) {
            ...
            for (uint256 i; i<royalties.length; ++i) {
                ...
                (success, ) = royalties[i].receiver.call{
                    value: commission
                }("");
                if (!success) {
                    revert OdysseyMarket_FailedToPayEther();
