  ...
  ReentrancyGuardUpgradeable 
{
  ...
  function adminAmountMint(uint amount, address recipient) external onlyAdmin nonReentrant {
    require(recipient != address(0), "Invalid recipient");
    _randomMint(recipient, amount, mintAllocation);
  }

  function adminSelectionMint(uint[] memory tokenIds, address recipient) external onlyAdmin nonReentrant {
    require(recipient != address(0), "Invalid recipient");
    for (uint i = 0; i < tokenIds.length; i ++) {
      require(tokenIds[i] >= 1 && tokenIds[i] <= cubeInitialMaxItems, "Invalid token id");
      require(!minted[tokenIds[i]], "Already minted");
      uint nftIndex = _getPendingIndexById(tokenIds[i], 1, cubeInitialMaxItems);
      _popPendingAtIndex(nftIndex);
      minted[tokenIds[i]] = true;
      _mint(recipient, tokenIds[i]);
      _setWires(tokenIds[i]);
      emit Mint(recipient, tokenIds[i]);
    }
  }
  ...
  function _setWires(uint tokenId) internal {
    uint voxelType = (tokenId - 1) / 2121;
    if (tokenId <= 1 + voxelType * 2121 + 932) {
      // Wire 1 - 14.67%
      wires[tokenId] = 1;
    } else if (tokenId <= 1 + voxelType * 2121 + 1547) {
      // Wire 2 - 9.67%
      wires[tokenId] = 2;
    } else if (tokenId <= 1 + voxelType * 2121 + 1928) {
      // Wire 3 - 6%
      wires[tokenId] = 3;
    } else {
      // Wire 4 - 3%
    ...
    for (i = 0; i < 3; i++) {
      require(
        tokenIds[i] != 0 &&
          tokenIds[i] >= 1 + i * 2121 &&
          tokenIds[i] <= (i + 1) * 2121,
        string(abi.encodePacked("Must burn Voxel ", Strings.toString(i + 1)))
      );
    }
  ...
  function singleBurn(uint tokenId) external burningWorked whenNotPaused {
    require(
      tokenId >= 1 && tokenId <= cubeInitialMaxItems,
      "singleBurn: Voxel shouldn't be genesis"
    );
    require(
  ...
  function setCubesBurnable(bool _cubesBurnable) external onlyAdmin {
  ...
  {
    // Initial voxels
    if (_tokenId <= cubeInitialMaxItems) {
      return
        string(
