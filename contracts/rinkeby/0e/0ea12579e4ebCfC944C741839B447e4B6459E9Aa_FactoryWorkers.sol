...
contract FactoryWorkers is Ownable,ERC721,ERC721Enumerable {
    ...
    string constant DESCRIPTION = "Our Workers are the real building blocks of the Factory: passion, expertise and dedication are three key elements of their work, which is clearly reflected on the quality of our products. At a first glance their robotic body can make them look just like cold machines; however, their commitment to the job sometimes almost makes us questioning their non-human nature, as if a true heart were beating under their iron chest... FactoryWorkers is a collection of 2000 NFTs living on the Ethereum blockchain, which will be released in 10 distinct rounds of 200 units each. In each round a fixed number of golden tokens will be minted, up to a total number of 33 Golden Workers. Each token grants total engagement and several benefits among the NFT Factory community. By owning a FactoryWorker, you choose to support our Factory giving us the possibility to grow and release all the amazing NFT gaming projects we already have in mind. Thanks for your help!";
    ...
    function contractURI() public view returns (string memory) {
        ...
        Base64.encode(
            ...
            abi.encodePacked(
                '{"name":"',
                "FactoryWorkers",
                '", "description":"', DESCRIPTION , '", "external_link": "https://www.nft-factory.club/factory-workers", "image":"ipfs://Qmdg1AYBdRwwoZBwxje6qqk6pTwnLbcMbcdhhFdPHHkE39"}'
            )
            )
        )
        )
    ...
    function mint(uint256 amount) public payable {
        ...
        if(msg.sender != owner()){
            require(msg.value >= price*amount,"FactoryWorkers: Not enough funds supplied");
        }
        ...
        for(uint i=0; i<amount; i++){
            ...
            if(msg.sender != owner()){
                if(i == amount - 1){
                    payable(owner()).transfer(msg.value - price*(amount-1));
                } else {
    ...
    function mintWithDiscount(uint256 amount,address collectionAddress) public payable {
        ...
        require(amount <= MAX_MINT_AMOUNT, "FactoryWorkers: Supplied amount is greater than max mintable amount");
        require(rounds[currentRound].remaining >= amount, "FactoryWorkers: Supplied amount exceeds round cap");
        require(msg.value >= discountedPrice*amount,"FactoryWorkers: Not enough funds supplied");
        for(uint i=0; i<amount; i++){
            uint256 newId = rounds[currentRound].popRandomId();
            _safeMint(msg.sender,newId,"");
            if(i == amount - 1){
                payable(owner()).transfer(msg.value - discountedPrice*(amount-1));
            } else {
