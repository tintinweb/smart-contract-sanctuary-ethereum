...
contract HifiProxyTarget is IHifiProxyTarget {
    ...
    function depositUnderlyingAndMintHTokenAndAddLiquidity(
        ...
        // Calculate underlying amount required to mint hTokens
        uint256 underlyingRequired = denormalize(hTokenRequired, hifiPool.underlyingPrecisionScalar());

        // Transfer the underlying to the DSProxy.
        IErc20 underlying = hifiPool.underlying();
        uint256 totalUnderlyingAmount = underlyingRequired + underlyingOffered;
        ...
        approveSpender(underlying, address(hToken), underlyingRequired);

        // Deposit the underlying in the HToken contract to mint hTokens.
        hToken.depositUnderlying(underlyingRequired);
        ...
        // // Relay any remaining hTokens to the end user.
        // if (depositAmount > underlyingRequired) {
        //     unchecked {
        //         uint256 underlyingDelta = depositAmount - underlyingRequired;
        //         underlying.transfer(msg.sender, underlyingDelta);
        //     }
        // }
    }
