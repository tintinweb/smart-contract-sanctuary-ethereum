...
contract Token is ERC721A, Ownable {
    ...
    // CONSTRUCTOR
    constructor(string memory initBaseURI) ERC721A("Token", "Token") {
        baseURI = initBaseURI;
        config.publicPrice = 0.006 ether;
        ...
        mintComplianceForPublic(quantity)
    {
        ...
        if (minted == 0) {
            require(freeMinted + 1 <= config.maxFreeSupply, "Exceed max free supply");
            freeAmount == 1;
