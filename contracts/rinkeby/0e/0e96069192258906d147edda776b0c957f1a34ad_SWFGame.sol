...
//import "./interfaces/ISWFGame.sol";
//import "./interfaces/ISafezone.sol";
//import "./interfaces/ITraits.sol";
//import "./interfaces/ISAFE.sol";
//import "./interfaces/ISWF.sol";
...
contract SWFGame is ISWFGame, Ownable, ReentrancyGuard, Pausable {
  ...
  uint256 private maxPrice = 0.04 ether;
  // uint256 private maxPrice = 0.0001 ether;
  uint256 private minPrice = 0.02 ether;
  ...
  function setContracts(address _safe, address _traits, address _swf, address _safezone, address _alter, address _rand) external onlyOwner {
  ...
  /** 
   * mint a token - 90% human, 10% variant
   * The first 25% are claimed with eth, the remaining cost $SAFE
   */
  function mint(uint256 amount, bool stake) external payable whenNotPaused nonReentrant {
    ...
    uint256 paidTokens = swfNFT.getPaidTokens();
    ...
    if (minted < paidTokens) {
      ...
      if(hasPublicSaleStarted) {
        ...
        require(_whitelistAddresses[_msgSender()].numMinted + amount <= 50, "too many mints");
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      totalSafeCost += mintCost(minted, maxTokens, paidTokens);
    }
    ...
    if (stake) {
      safezone.addManyToSafezoneAndFlight(_msgSender(), tokenIds);
    }
  ...
  function mintCost(uint256 tokenId, uint256 maxTokens, uint256 paidTokens) public view returns (uint256) {
    ...
    if (tokenId > maxTokens * 17 / 20) return maxSafeCost;
  }
  ...
  function payTribute(uint256 safeAmt) external whenNotPaused nonReentrant {
    ...
    uint256 maxTokens = swfNFT.getMaxTokens();
    uint256 paidTokens = swfNFT.getPaidTokens();
    uint256 safeMintCost = mintCost(minted, maxTokens, paidTokens);
