...
contract PPContract is ERC721Enumerable, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    address[] private team_ = [0x5B38Da6a701c568545dCfcB03FcB875f56beddC4];
    uint256[] private teamShares_ = [3080];

    constructor(
        //string memory _initBaseURI,
        //string memory _initNotRevealedUri
    ) ERC721("Porkcontract", "PB") PaymentSplitter(team_, teamShares_) {
    ...
    nonReentrant
    {
        ...
        require(workflow == WorkflowStatus.Presale, "Pork: Presale is not started yet!");
        require(tokensPerWallet[msg.sender] < 1, "Pork: Presale mint is one token only.");
    ...
    function publicSaleMint(uint256 ammount) public payable nonReentrant {
        ...
        require(workflow != WorkflowStatus.SoldOut, "Pork: SOLD OUT!");
        require(workflow == WorkflowStatus.Sale, "Pork: public sale is not started yet");
        require(msg.value >= price * ammount, "Pork: Insufficient funds");
        require(ammount <= 5, "Pork: You can only mint up to five token at once!");
        require(tokensPerWallet[msg.sender] + ammount <= 5, "Pork: You can't mint more than 5 tokens!");
        require(supply + ammount <= MAXSUPPLY, "Pork: Mint too large!");
        uint256 initial = 1;
    ...
    function setUpSale() external onlyOwner {
        require(workflow == WorkflowStatus.Presale, "Pork: Unauthorized Transaction");
