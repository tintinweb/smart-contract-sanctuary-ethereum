...
contract MatrixVotingPower is IERC20, IERC20Metadata, Ownable {
    ...
    function totalSupply() external override view returns (uint256) {
        return IERC721Enumerable(nft).totalSupply() - numBlackList;
    }

    function balanceOf(address account) external override view returns (uint256) {
        uint256 _balance = IERC721(nft).balanceOf(account);
        uint256 power = 0;
        for(uint256 i=0;i<_balance;i++) {
            uint256 tokenId = IERC721Enumerable(nft).tokenOfOwnerByIndex(account, i);
