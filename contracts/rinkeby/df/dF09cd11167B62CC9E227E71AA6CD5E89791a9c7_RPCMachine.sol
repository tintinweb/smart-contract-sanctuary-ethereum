...
contract RPCMachine is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function initNftMapping() external onlyOwner {
        for(uint256 i = 0; i < 7117; i++) {
            poolNftMap[i] = i+2000;
        }
    }
...
//############################ Liquid Functions Start #####################
    ...
    function C_buy(uint256 _amountBuyMax, uint256 _deadline, bytes32 _dataHash) external ensure(_deadline)  nonReentrant crOpen callerIsUser {
    ...
    function R_buy(bytes32 revealHash) external  nonReentrant crOpen callerIsUser {
        uint256 _requestId = buyMap[msg.sender];
        if (_requestId == 0) {
            revert RequestInvalid(msg.sender, _requestId, 0);
        }
    ...
    function C_reborn(uint256 _ownerTokenId, uint256 _amountFeeMax, uint256 _deadline, bytes32 _dataHash) external ensure(_deadline)  nonReentrant crOpen callerIsUser {
    ...
    function R_reborn(bytes32 revealHash) external  nonReentrant crOpen callerIsUser {
        uint256 _requestId = rebornMap[msg.sender];
        if (_requestId == 0) {
            revert RequestInvalid(msg.sender, _requestId, 0);
        }
     ...
     */
    function redeem(uint256 _ownerTokenId, uint256 _amountRedeemMin, uint256 _deadline) external ensure(_deadline) nonReentrant poolOpen {
        IERC721 _pandaNFT = IERC721(pandaNFT);
        if (_pandaNFT.ownerOf(_ownerTokenId) != msg.sender) {
            revert NFTOwnerNotMatch(_ownerTokenId,  msg.sender, _pandaNFT.ownerOf(_ownerTokenId));
        }
...
//########################### CR Buy&Reborn Functions Start ####################
    ...
    function ownerCBuy(bytes32 _dataHash) external  nonReentrant callerIsUser  {
    ...
    function ownerRBuy(bytes32 revealHash) external  nonReentrant  callerIsUser {
        if (!buyCompensates[msg.sender]) {
            revert NotCompensate(msg.sender);
        }
    ...
    function ownerCReborn(uint256 _ownerTokenId, bytes32 _dataHash) external nonReentrant crOpen callerIsUser {
    ...
    function ownerRReborn(bytes32 revealHash) external  nonReentrant crOpen callerIsUser {
        if (!rebornCompensates[msg.sender]) {
            revert NotCompensate(msg.sender);
        }
