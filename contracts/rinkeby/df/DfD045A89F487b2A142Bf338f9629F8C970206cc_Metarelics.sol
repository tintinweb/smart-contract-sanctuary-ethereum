...
contract Metarelics is ERC721Enumerable, Ownable, ReentrancyGuard {
   ...
   */
  function presaleMint(uint256 _mintAmount, bytes32[] calldata proof) public payable nonReentrant {
    ...
    if (msg.sender != owner()) {
      ...
      if (useWhitelistedAddressesBackup) {
        ...
        require(_verify(proof, whitelistMerkleRoot, _generateMerkleLeaf(msg.sender)), "user is not whitelisted");
      }
  ...
  function secondaryMint() public payable nonReentrant {
    uint256 supply = totalSupply();
    requireChecks(1);
    if (msg.sender != owner()) {
      require(msg.value >= cost, "insufficient funds");
