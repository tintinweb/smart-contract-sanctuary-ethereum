...
/**
 * @title The Photosynthesis NFT smart contract.
 */
contract NFT is ERC721A, AccessControl, Ownable {
  ...
  /// @notice The flag that indicates if NFTs have been revealed.
  bool public revealed = false;
  /// @notice The URL that used until NFTs are revealed.
   ...
   * @param tokenName The NFT token's name
   * @param tokenSymbol The NFT token's symbol
   * @param notRevealedURI_ The NFT uri which is not revealed
   * @param numTotalTokens_ The max supply of NFT
   */
  constructor(
    ...
    numReservedTokens = 130;
    numTotalTokens = numTotalTokens_;
  }

  /**
   * @notice Mints reserves
   * @param recipient The recipient's address
   * @param quantity The minting quantity
   */
  ...
  function mint(address recipient, uint256 quantity) external onlyRole(MINTER_ROLE) {
    require(areReservesMinted == true, "RESERVES_NOT_MINTED");
    require(totalSupply() + quantity <= numTotalTokens, "MAX_SUPPLY_EXCEEDED");
   ...
   * @param newBaseURI The URL to set as the base URL.
   */
   ...
   * @dev Sets the `revealed` flag to `true` that causes `tokenURI` to return the real NFT URI.
   ...
   * @dev Is only available for the users with the `DEFAULT_ADMIN_ROLE` role.
   * @param uri The URL of the media to show for unrevealed NFTs.
   */
  function setNotRevealedURI(string memory uri) external onlyRole(DEFAULT_ADMIN_ROLE) {
    notRevealedURI = uri;
  }

  /**

   * @notice Gets token's URI
