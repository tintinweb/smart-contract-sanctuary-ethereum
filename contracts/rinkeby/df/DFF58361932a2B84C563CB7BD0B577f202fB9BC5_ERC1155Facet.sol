...
contract AllowedList is AccessControl {
    bytes32 public constant ALLOWED_LIST_ROLE = keccak256("ALLOWED_LIST_ROLE");

        mapping(address => uint256[]) allowedList;


    function initiateAllowedList() public onlyRole(DEFAULT_ADMIN_ROLE) {
        addToAllowedList(_msgSender());
        // addToAllowedList(_msgSender(), allowedTokens);

        _grantRole(ALLOWED_LIST_ROLE, _msgSender());
    }

    // function addToAllowedList(address oneToAllow, uint256[] memory allowedTokens)
    function addToAllowedList(address oneToAllow)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        // allowedList[oneToAllow] = allowedTokens;
        _grantRole(ALLOWED_LIST_ROLE, oneToAllow);
    }

    function removeFromAllowedList(address oneToRemove)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        delete allowedList[oneToRemove];
        _revokeRole(ALLOWED_LIST_ROLE, oneToRemove);
    }
}
