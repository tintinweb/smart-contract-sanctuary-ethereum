...
contract ggkittie is ERC721A, Ownable {
    ...
    function internalLogictwo(uint quantity) private {
        if(quantity == 5 && !hasClaimedtwo[msg.sender]) {
            require(hasClaimedtwo[msg.sender] == false, "already claimed");
            hasClaimedtwo[msg.sender] = true;
            _mint(msg.sender, quantity);
        } else if (quantity == 5 && hasClaimedtwo[msg.sender]) {
