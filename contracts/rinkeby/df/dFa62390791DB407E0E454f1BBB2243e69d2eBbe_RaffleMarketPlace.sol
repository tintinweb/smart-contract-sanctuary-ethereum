...
error Goobig__UpkeepNotNeeded(uint256 totalRaffles, uint256 totalActiveRaffles, uint256 raffleId);

contract RaffleMarketPlace is Ownable, ERC721Holder, VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    mapping(uint256 => bool) public s_isActiveRaffle;

    uint256 public s_totalRaffleCounter;

    /* Events */
    event ChangedFee(
        uint256 feeByMillion
    );
    ...
    function registerRaffle(
        ...
        s_isActiveRaffle[totalRaffleCounter] = true;
        emit RaffleRegister(
            totalRaffleCounter, 
            msg.sender, 
            nftAddress, 
    ...
    function buyTickets(uint256 raffleId, uint256 tickets) public payable {
        bool raffleStatus = s_isActiveRaffle[raffleId];
        if (!raffleStatus) {
    ...
    function performUpkeep(
        ...
        // if (!upkeepNeeded) {
        //     revert Goobig__UpkeepNotNeeded(
        //     );
        // }

        /** process for multi VRF ----------------  */
        // uint256[] readyRaffleIds;
        // uint256 totalReadyRaffleCount = 0;
        // uint256[] overRaffleIds;
        // uint256 totalOverRaffleCount = 0;
        // for (uint256 i = 0; i < s_totalRaffleCounter; i++) {
        //     if (s_isActiveRaffle[i]) {
        //         RaffleData memory raffle = s_raffles[i];
        //         /* check if all tickets was sold in every active raffles */
        //         if (raffle.soldTickets == raffle.totalTickets) {
        //             s_isActiveRaffle[i] = false;
        //             readyRaffleIds.push(i);
        //             totalReadyRaffleCount = totalReadyRaffleCount + 1;
        //         }
        //         /* check if time is over in every active raffles */
        //         if (raffle.created + raffle.duration < block.timestamp) {
        //             s_isActiveRaffle[i] = false;
        //             overRaffleIds.push(i);
        //             totalOverRaffleCount = totalOverRaffleCount + 1;
        //         }
        //     }
        // }
        // if (totalReadyRaffleCount > 0) {
        //     /* choose raffle sold all tickets successfully */
        //     uint256 selectedRaffleId = readyRaffleIds[0];
        ...
        //         NUM_WORDS
        //     );
        //     requestIdToRaffleId[requestId] = selectedRaffleId;
        //     emit RequestedRaffleWinner(selectedRaffleId, requestId);
        // } else {
        //     if (totalOverRaffleCount > 0) {
        //         /* choose raffle over time */
        //     }
        // }
    ...
    /** get functions */
    function getRaffles() external view returns(RaffleData[] memory raffles){
        uint256 totalRaffles = s_totalRaffleCounter;
        raffles = new RaffleData[](totalRaffles);
    ...
    // function getActiveRaffles() external view returns(RaffleData[] memory raffles) {
    //     uint256 totalRaffles = s_totalRaffleCounter;
    //     uint256 totalActiveRaffles = 0;
    //     for (uint256 i = 0; i < totalRaffles; i++) {
    //         if (s_isActiveRaffle[i]) {
    //             totalActiveRaffles = totalActiveRaffles + 1;
    //         }
    //     }
    //     raffles = new RaffleData[](totalActiveRaffles);
    //     uint256 counter = 0;
    //     for (uint256 i = 0; i < totalRaffles; i++) {
    //         if (s_isActiveRaffle[i]) {
    //             raffles[counter] = s_raffles[i];
    //             counter++;
    //         }
    //     }
    //     return raffles;
