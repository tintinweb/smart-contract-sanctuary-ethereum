    ...
    ERC721Stashable
{
    ...
    uint256 constant PRICE_PER_TOKEN = 0.0025 ether; // TODO: CHANGE TO : 0.025
    uint256 constant MAX_SUPPLY = 11111;

    IERC721 public constant TOY =
        IERC721(0x721e0B75098cF0157E1026F95813f498326DC4AB); //TODO: CHANGE TO : 0xBF662A0e4069b58dFB9bceBEBaE99A6f13e06f5a
    IERC721 public constant KIT =
        IERC721(0xd2A2A25B31848e2f9d22129e711cB7a4D4229077); //TODO: CHANGE TO : 0x5ff1863753C1920C38c7BaE94576f2831eF99695
    ...
    // Token Ids are continuous. It's better to use bitmap to save gas.
    BitMaps.BitMap toyClaimed;
    BitMaps.BitMap kitClaimed;

    // Pack into a single storage slot to reduce gas consumption.
    struct PublicSaleParam {
        uint64 startTime;
        uint64 endTime;
        uint32 maxQuantityPerUser;
    }
    PublicSaleParam public publicSaleParam;
    ...
    function initialize() public initializer {
        __ERC2981_init();
        __ERC721Psi_init("Toy Stash", "STASH"); //TODO
        __Ownable_init();
        setPublicSale(1660517255,1760517255,2000);
        signer = address(0xDe1736B2F811a1e43EF92f6A707b198B6C09FAa8);

        _setDefaultRoyalty(msg.sender, 750); //TODO receiver, feeNumerator (1000 => 10%)
    }
    ...
    modifier checkSupply(uint256 quantity) {
        if (_minted + quantity > MAX_SUPPLY) {
    ...
    // Make sure the tokenIds are all unclaimed ones.
    function claim(
        uint256[] calldata tokenIdsTOY,
        uint256[] calldata tokenIdsKIT
    ) external checkSupply(tokenIdsTOY.length + tokenIdsKIT.length) {
        // timestamp check
        if (block.timestamp < publicSaleParam.startTime) {
            revert NotStarted();
        }
        for (uint256 i = 0; i < tokenIdsTOY.length; i++) {
            uint256 tokenId = tokenIdsTOY[i];
        ...
        _mint(msg.sender, tokenIdsTOY.length + tokenIdsKIT.length);
    }
        ...
        checkSupply(quantity)
    {
        ...
        if (block.timestamp >= publicSaleParam.endTime) {
            revert Ended();
        }
        ...
        if (
            quantity + userMinted[msg.sender] >
            publicSaleParam.maxQuantityPerUser
        ) {
            revert ExceedAllowedQuantity();
        }
    ...
    function setPublicSale(
        ...
        publicSaleParam.startTime = startTime;
        publicSaleParam.endTime = endTime;
        publicSaleParam.maxQuantityPerUser = maxQuantityPerUser;
    }
