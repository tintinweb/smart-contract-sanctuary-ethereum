...
contract Fallen is ERC721A, ReentrancyGuard, Ownable {
    ...
    function freeMint(uint256 quantity) external {
        ...
        for(uint256 i = 0; i < quantity; i++){
            ...
            InventoryInterface.generateTokenInfo(tokenId, msg.sender);
        }
    ...
    function _beforeTokenTransfers(address from, address to, uint256 tokenId, uint256 quantity) internal override {
        uint256 tokenSoulScore = InventoryInterface.getSoulScore(tokenId);
        if (from != address(0) && from != address(InventoryInterface)) {
            Souls.stopDripping(from, uint128(tokenSoulScore));
        }

        if (to != address(0) && from != address(InventoryInterface)) {
            Souls.startDripping(to, uint128(tokenSoulScore));
        }
