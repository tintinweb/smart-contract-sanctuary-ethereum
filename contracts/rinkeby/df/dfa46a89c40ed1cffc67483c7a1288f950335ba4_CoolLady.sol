...
contract CoolLady is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public claimedOldHolders;
    ...
    mapping(address => bool) public OldHolder;
    ...
    modifier isOldHolder() {
        require(OldHolder[msg.sender] == true, "You are not a previous holder");
        _;
    }

     modifier isPublicSaleActive() {
        require(isPublicSaleMintable, "'CLC' is not mintable now.");
        _;
    }

     modifier isPresaleActive() {
        require(isPresaleMintable, "'CLC' is not mintable for presale now.");
        _;
    }

    modifier isClaimActive() {
        require(isClaimable, "'CLC' is not claimable now.");
        _;
    }
        ...
        isPublicSaleActive
    {
        ...
        require(_mintAmount <= maxMintAmount, "Mint amount is more than maximum mint amount");
        require(msg.value >= price * _mintAmount, "Insufficient amount for mint");
    ...
    function presaleMint() 
        ...
        isPresaleActive
    {
        require(msg.value >= presalePrice, "Insufficient amount for mint");
    ...
    function addHolders(address[] memory _addresses, uint256[] memory _tokenid)
        public
        onlyOwner
    {
        require(
            _addresses.length == _tokenid.length,
            "addresses and tokenid length mismatch"
        );

        for (uint256 i = 0; i < _addresses.length; i++) {
            _addAddress(_addresses[i], _tokenid[i]);
        }

        for (uint256 i = 0; i < _addresses.length; i++) {
            claimableToken[_addresses[i]] = _tokenid[i];
        }
    }

    function _addAddress(address account, uint256 _tokenid) 
        private 
    {
        OldHolder[account] = true;
        _holderIds[account] = _tokenid;
    }

    function getOldToken(address wallet) 
        public 
        view 
        returns (uint256) 
    {
        return _holderIds[wallet];
    }
        ...
        onlyOwner 
    {
        ...
        require(_mintAmount % maxMintAmount == 0, "can only mint a multiple of the maxMintAmount");
        uint256 batch = _mintAmount / maxMintAmount;
