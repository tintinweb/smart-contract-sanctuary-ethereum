...
contract FeeProcessor is Ownable {
  ...
  modifier onlyOwnerOrPrt() {
    require(
      msg.sender == owner() || msg.sender == PRT,
      'only owner or PRT contract can call'
    );
    _;
  }
  ...
  function _addLp(uint256 tokenAmount, uint256 usdcAmount) internal {
    ...
    _prt.approve(address(uniswapV2Router), tokenAmount);
