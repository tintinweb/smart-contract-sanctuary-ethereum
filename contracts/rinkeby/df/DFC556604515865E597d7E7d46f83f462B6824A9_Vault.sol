...
contract Cardanae is ERC20, Ownable {
    constructor() ERC20("Cardanae", "ADO") {
...
contract Vault {
    ...
    mapping (address => mapping(string => uint256 )) public accountBalance;
    address owner;
    ...
    function getBalanceMapping() public view returns(string[] memory, uint256[] memory) {
        string[] memory nameLists = new string[](name[msg.sender].length);
        uint256[] memory balance = new uint256[](name[msg.sender].length);

        for (uint256 i = 0; i < name[msg.sender].length; i++) {
            nameLists[i] = name[msg.sender][i];
            balance[i] = accountBalance[msg.sender][name[msg.sender][i]];
        }

        return (nameLists, balance);
    }
    
    function getBalance(string memory _name) external view returns (string memory) {
        return string(abi.encodePacked(_name, ": ", Strings.toString(accountBalance[msg.sender][_name])));
    }
