...
contract SingularityNFT is ERC1155, ERC1155Supply, Ownable {
    string public constant name = "Test Serum - SING";
    ...
    mapping(address => mapping(uint256 => uint256)) private _whitelistedWalletAmounts;

    constructor(string memory uri) ERC1155(uri) { //ipfs://bafybeigrd63ayfyzuerjcgrdvewv3xejfdo6tljpmwtwwygszaoguc5pgu/{id}.json
    }
    ...
    function mint(uint256 id) external {
        uint256 amount = _whitelistedWalletAmounts[msg.sender][id];
    ...
    function addToWhitelistMultipleAdress (address[] memory users, uint[] memory amount, uint id) public onlyOwner  {
        require(_items[id].maxSupply>0);
