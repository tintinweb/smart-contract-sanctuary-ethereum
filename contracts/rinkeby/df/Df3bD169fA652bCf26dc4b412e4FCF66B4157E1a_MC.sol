...
contract MC is Ownable, ERC721A, ReentrancyGuard {
    uint256 public MAX_SUPPLY = 5555;
    bool public saleEnabled = false;
    uint256 public price = 0.008 ether;
    uint256 public MaxPerTxn = 15;
    string public baseURI = "ipfs://QmPhEtgeNe8MLPVFcmUS94TrWPPQgU1PkNkQz3JygkxR7S/";
    mapping(address => bool) public ogAddress;
    uint256 public MaxWLFreePerWallet = 2;
    uint256 public presaleLimit = 2000;

    constructor(address[] memory _ogAddress, address[] memory _wlAddress) ERC721A("MC", "MC") {
        for (uint256 i = 0; i < _ogAddress.length; i++) {
            ogAddress[_ogAddress[i]] = true;
        }
        for (uint256 i = 0; i < _wlAddress.length; i++) {
            whitelistAddress[_wlAddress[i]] = true;
        }
    }
    ...
    function publicMint(uint256 quantity) external payable callerIsUser {
        require(saleEnabled, "wait until sale start");
    ...
    function whitelistMint(uint256 quantity) external payable callerIsUser{
        require(saleEnabled, "wait until sale start");
        require(quantity <= MaxPerTxn, "can't mint this many");
        require(totalSupply() + quantity <= presaleLimit, "presale is over!" );

        uint256 count = quantity;
        if (numberMinted(msg.sender) < MaxWLFreePerWallet) {
            if (numberMinted(msg.sender) + quantity <= MaxWLFreePerWallet)
                count = 0;
            else count = numberMinted(msg.sender) + quantity - MaxWLFreePerWallet;
        }
    ...
    function ogMint(uint256 amount) external payable {
        ...
        require(ogAddress[msg.sender], "not a og");
    ...
    function setMaxWLFreePerWallet(uint256 amount) external onlyOwner {
        MaxWLFreePerWallet = amount;
    }
