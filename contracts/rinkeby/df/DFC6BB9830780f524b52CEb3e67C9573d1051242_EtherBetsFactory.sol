...
contract EtherBets{ // how do I name this..
    ...
    function getDetails() public view returns (string memory, uint, uint8, uint8, uint, uint, bool, uint, uint, uint8[] memory, uint256, bool){
        return (name, betCost, maximumNumber, picks, timeBetweenDraws, lastDrawTime, paused, draw, address(this).balance, winningNumbers, randomNumber, randomNumberFetched);
    }
    ...
    function distributePrize() internal{
        address[] memory winners = betToAddresses[uint(keccak256(abi.encode(arrayToUint(winningNumbers), draw)))];
        if(winners.length == 0){
            return;
        }
        uint splitPrize = address(this).balance / winners.length;
        for(uint i = 0; i < winners.length; i++){
            address payable a = payable(winners[i]);
            (bool sent,) = a.call{value: splitPrize}("");
