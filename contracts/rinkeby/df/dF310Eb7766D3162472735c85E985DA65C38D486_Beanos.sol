...
contract Beanos is Ownable, ERC721A {
    ...
    bytes public constant externalLink = "https://smolphunks.io";
    bytes constant tokenDescription = "One of 10,000 tokens in the BeanoPhunks collection, an on-chain transformation of the original CryptoPunks.";
    ...
    function constructTokenURI(uint16 tokenId) private view returns (string memory) {
        ...
        bytes memory title = abi.encodePacked("BeanoPhunk #", tokenId.toString());
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                '"attributes": ', punkAttributesAsJSON(tokenId),
                                '}'
                            )
                        )
                    )
    ...
    function initializePunk(uint16 punkId) private view returns (BeanoPhunk memory) {
        BeanoPhunk memory phunk = BeanoPhunk({
            id: punkId,
    ...
    function tokenImage(uint16 tokenId) public view returns (string memory) {
        ...
        svgBytes.appendSafe('<svg width="48" height="48" shape-rendering="crispEdges" xmlns="http://www.w3.org/2000/svg" version="1.2" viewBox="0 0 24 24"><style>rect{width:1px;height:1px}</style><rect x="0" y="0" style="width:100%;height:100%" fill="#3c2a3c" />');
    ...
    function withdraw() external {
        ...
        Address.sendValue(payable(kenobiAddress), total - half);
    }
    ...
    function punkAttributesAsJSON(uint16 punkId) public view returns (string memory json) {
        BeanoPhunk memory phunk = initializePunk(punkId);
