...
contract Minter is Ownable, ReentrancyGuard {
    ...
    function close(address uAssetAddress, bytes memory id) external nonReentrant {
        ...
        emit Close(msg.sender, id, uAssetAmount, collateralAmount, block.timestamp);
    }
    ...
    function liquidation(address userAddress, address uAssetAddress, uint256 uAssetAmount, bytes memory id) external nonReentrant {
        ...
        if (discountedCollateralValue <= collateralBalances[id]) {
            ...
            uint256 refundUAssetAmount = (discountedCollateralValue - collateralBalances[id]) * (10**calculationDecimal - discountRate) / ((10**calculationDecimal) * targetPrice) * ERC20(uAssetAddress).decimals();
            uint256 remainedAmount = uAssetAmount - refundUAssetAmount;
            {
                IERC20(uAssetAddress).safeTransferFrom(msg.sender, address(this), remainedAmount);
                IERC20(uAssetAddress).safeBurn(remainedAmount);
                IERC20(collateralAddress).safeTransfer(msg.sender, collateralBalance);
            }
            {
                borrowBalances[id] -= remainedAmount;
                collateralBalances[id] = 0;
            }
            emit Liquidation(msg.sender, userAddress, id, remainedAmount, collateralBalance, block.timestamp, discountRate);
        }
    }
