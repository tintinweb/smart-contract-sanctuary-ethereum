...
contract IPEXExchange is Exchange {
    string public constant name = "IPEX Exchange";
   ...
   * @dev Approve an order and optionally mark it for orderbook inclusion. Must be called by the maker of the order
    ...
    * @dev Calculate the price two orders would match at, if in fact they would match (otherwise fail)
    ...
    returns (uint)
    {
        ...
        /* Require price cross. */
        require(buyPrice >= sellPrice, "IPEXExchage::Buy price must greater than sell price!");
    ...
    returns (uint)
    {
        ...
        if (sell.paymentToken != address(0)) {
            require(msg.value == 0, "IPEXExchange::Redundant sent funds!");
        }
        ...
        if (sell.feeRecipient != address(0)) {
            ...
            require(sell.takerRelayerFee <= buy.takerRelayerFee, "IPEXExchange::taker fee is more than to maximum fee specified by buyer");
            ...
            if (sell.makerRelayerFee > 0) {
                ...
                if (sell.paymentToken == address(0)) {
                    ...
                    require(payable(sell.feeRecipient).send(makerRelayerFee), "IPEXExchange::Fee transfer to maker failed!");
                } else {
            ...
            if (sell.takerRelayerFee > 0) {
                ...
                if (sell.paymentToken == address(0)) {
                    ...
                    require(payable(sell.feeRecipient).send(takerRelayerFee), "IPEXExchange::Fee transfer to taker failed!");
                } else {
    ...
    * @dev Atomically match two orders, ensuring validity of the match, and execute all associated state transitions. Protected against reentrancy by a contract-global lock.
    ...
    reentrancyGuard
    {
        ...
        if (buy.maker == msg.sender) {
            require(validateOrderParameters(buy), "IPEXExchange::Invalid buy order params!");
        } else {
        ...
        if (sell.maker == msg.sender) {
            require(validateOrderParameters(sell), "IPEXExchange::Invalid sell order params!");
        } else {
            sellHash = requireValidOrder(sell, sellSig);
        }

        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), "IPEXExchange::Order not matched");
        ...
        require(size > 0, "IPEXExchange::Order target is not a contract!");
        ...
        require(ArrayUtils.arrayEq(buy.callData, sell.callData), "IPEXExchange::Calldata after replacement is invalid!");
        ...
        require(success, "IPEXExchange::ERC721 Transfer failed!");
