...
* Midnight Movie Club
*/
contract MidnightMovieClub is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_PER_PUBLIC_MINT = 10;
    uint256 public constant MAX_PER_PRESALE_SPOT = 2;
    uint256 public constant PUBLIC_MINT_PRICE = 0.09 ether;
    uint256 public constant PRESALE_MINT_PRICE = 0.08 ether;
    
    string public baseURI;

    mapping(address => uint) public addressToPublicMinted;
    mapping(address => uint) public addressToPresaleMinted;
    ...
    modifier isOnPresaleList(bytes32[] calldata proof) {
        require(_verify(msg.sender, presaleMerkleRoot, proof), "Sender not in Presale MerkleTree");
        _;
    }


    constructor(string memory _baseURI, address _treasuryAddress) 
        ERC721("Midnight Movie Club", "MMC") 
    {
        baseURI = _baseURI;
    ...
    *   (numPresaleMintsRemaining, numPublicMintsRemaining, totalMinted, totalRemainingSupply)
    */
    function getMintProgress(address userWallet, bytes32[] calldata presaleProof) 
        external 
        view 
        returns (uint, uint, uint, uint) 
    {
        ...
        uint publicMintsRemaining = MAX_PER_PUBLIC_MINT 
            - addressToPublicMinted[userWallet];

        if (_verify(userWallet, presaleMerkleRoot, presaleProof)) { 
            psMintsRemaining += (MAX_PER_PRESALE_SPOT - addressToPresaleMinted[userWallet]);
        }
    ...
    */
    function presaleMint(uint256 amount, bytes32[] calldata proof) 
        ...
        hasAvailableSupply(addressToPresaleMinted[msg.sender] + amount, MAX_PER_PRESALE_SPOT)
        isOnPresaleList(proof)
        nonReentrant
    {
        ...
        addressToPresaleMinted[msg.sender] += amount;
    ...
    function publicMint(uint256 amount) 
        ...
        hasAvailableSupply(addressToPublicMinted[msg.sender] + amount, MAX_PER_PUBLIC_MINT)
        nonReentrant
    {
        uint256 totalSupplySaveFunctionGas = _owners.length - 1;
        require(IS_PUBLIC_SALE_ACTIVE, 'Public sale is inactive! pls stop');
        require(totalSupplySaveFunctionGas + amount <= MAX_SUPPLY, "Exceeds max supply, MMC are sold out!");

        addressToPublicMinted[msg.sender] += amount;
        for(uint i=0; i < amount;) {
            _mint(msg.sender, totalSupplySaveFunctionGas + i + 1);
            unchecked {++i;}
        }
    ...
    */
    function withdrawETH() external onlyOwner {
        ...
        require(success, "Failed to send to Gnosis Treasury.");
    }
