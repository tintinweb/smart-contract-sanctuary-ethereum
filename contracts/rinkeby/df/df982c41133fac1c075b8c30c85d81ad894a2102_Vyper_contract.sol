...
VOTING_ESCROW: constant(address) = 0x39f9a49C2e71276e7D6b2cD15b9fCBca6b84d5F2
...
def create_boost(
    ...
    y: int256 = _percentage * (convert(VotingEscrow(VOTING_ESCROW).balanceOf(_delegator), int256) - delegated_boost) / MAX_PCT
...
def extend_boost(_token_id: uint256, _percentage: int256, _expire_time: uint256, _cancel_time: uint256):
    ...
    delegated_boost: int256 = point.slope * time + point.bias
    y: int256 = _percentage * (convert(VotingEscrow(VOTING_ESCROW).balanceOf(delegator), int256) - delegated_boost) / MAX_PCT
