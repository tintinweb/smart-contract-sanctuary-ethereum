...
// File: interfaces/badger/IVault.sol
...
// File: contracts/lib/SettAccessControl.sol
...
*/
contract SettAccessControl is Initializable, SeraphProtected {
    ...
    /// @notice Can only be changed by governance itself
    function setGovernance(address _governance) public  withSeraph() {
        _onlyGovernance();
...
// File: contracts/@openzeppelin-contracts-upgradeable/math/SafeMathUpgradeable.sol
...
// File: contracts/@openzeppelin-contracts-upgradeable/token/ERC20/SafeERC20Upgradeable.sol
...
contract Vault is ERC20Upgradeable, SettAccessControl, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function withdraw(uint256 _shares) external whenNotPaused {
        ...
        // Check if it would have to extract from strategy
        uint256 b = token.balanceOf(address(this));

        if (b < r) {
            _seraphprotect( _shares);
        } else{
             _withdraw(_shares);
        }
    ...
    function _seraphprotect (uint256 _shares) internal withSeraph() {
         _withdraw(_shares);
    }
    ...
    ///         Note that withdrawals are not processed when the Sett is paused. 
    /// @dev See `_withdraw` for details on how withdrawals are processed.
    function withdrawAll() external whenNotPaused withSeraph() {
    ...
    function setTreasury(address _treasury) external whenNotPaused withSeraph() {
        _onlyGovernance();
    ...
    function setStrategy(address _strategy) external whenNotPaused withSeraph() {
        _onlyGovernance();
