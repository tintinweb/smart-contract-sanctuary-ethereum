...
contract TestPriceConsumer {
    ...
    //Rinkeby Testnet ETH/USD feed
    constructor() payable {
        priceFeed = AggregatorV3Interface(0x8A753747A1Fa494EC906cE90E9f37563A8AF630e);
        owner = payable(msg.sender);
        //get the USD value of the amount deposited at deployment and store
        initialEthPrice = getLastEthPriceUSD(); //has 8 digits
        usdValue = (initialEthPrice * msg.value) / 10**8;
    ...
    function getPaymentEthAmount(uint periods) public {
        //Get Needed USD amount by dividing USD value by number of periods (2 and 3 for testing)
        paymentUSD = (usdValue * 10**18) / (periods * 10**18); //divide by number of periods (payment USD in 10**18)
        testEthPayment = (paymentUSD * 10**8) / (getLastEthPriceUSD()); 
