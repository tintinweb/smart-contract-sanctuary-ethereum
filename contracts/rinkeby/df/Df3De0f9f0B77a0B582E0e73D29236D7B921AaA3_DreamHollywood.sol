...
contract DreamHollywood is ERC721A, Ownable, AccessControl, ReentrancyGuard {
  ...
  uint256 private constant maxSupply = 340;
  uint256 private maxSupplyTotal = 340;
  uint256 private maxSupplyPrivate = 100;
  uint256 private pricePrivate = 1 ether;
  uint256 private pricePublic = 1.5 ether;
  ...
  constructor() ERC721A("Dream Hollywood", "DH") {
  ...
  function updatePricePrivate(uint256 _number) public onlyRole(DEFAULT_ADMIN_ROLE) {
    require(_number > 0, "Price is invalid");

    pricePrivate = _number * 10^17;
  }

  function updatePricePublic(uint256 _number) public onlyRole(DEFAULT_ADMIN_ROLE) {
    require(_number > 0, "Price is invalid");

    pricePublic = _number * 10^17;
  }
