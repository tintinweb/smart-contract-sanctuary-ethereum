...
contract Boom is ERC721, ERC721Enumerable, Ownable {
    ...
    function claim(uint256 tokenId, bytes memory signature) external {
        ...
        require(tx.origin == msg.sender, "Boom: contract is not allowed to claim.");
        require(_verify(msg.sender, tokenId, signature), "Boom: invalid signature.");
    ...
    function _verify(
        ...
        return keccak256(abi.encodePacked(_sender, _tokenId)).toEthSignedMessageHash().recover(_signature) == _signer;
    }
}
