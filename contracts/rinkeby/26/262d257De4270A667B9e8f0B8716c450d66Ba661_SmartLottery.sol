...
contract SmartLottery is ReentrancyGuard, Ownable {
    ...
    uint public maxNumberTicketsPerBuyOrClaim = 100;

    uint public maxPriceTicketInCake = 50 ether;
    uint public minPriceTicketInCake = 0.005 ether;

    uint public pendingInjectionNextLottery;

    uint public constant MIN_DISCOUNT_DIVISOR = 300;
    uint public constant MIN_LENGTH_LOTTERY = 4 hours - 5 minutes; // 4 hours
    uint public constant MAX_LENGTH_LOTTERY = 4 days + 5 minutes; // 4 days
    uint public constant MAX_TREASURY_FEE = 3000; // 30%
    ...
    struct Lottery {
        ...
        uint startTime;
        uint endTime;
        uint priceTicketInCake;
        uint discountDivisor;
        uint[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint[6] cakePerBracket;
        uint[6] countWinnersPerBracket;
        uint firstTicketId;
        uint firstTicketIdNextLottery;
        uint amountCollectedInCake;
        uint finalNumber;
    }
    ...
    mapping(uint => mapping(uint => uint)) private _numberTicketsPerLotteryId;

    // Keep track of user ticket ids for a given lotteryId
    mapping(address => mapping(uint => uint[])) private _userTicketIdsPerLotteryId;
    ...
    event LotteryClose(uint indexed lotteryId, uint firstTicketIdNextLottery);
    event LotteryInjection(uint indexed lotteryId, uint injectedAmount);
    event LotteryOpen(
    ...
    event LotteryNumberDrawn(uint indexed lotteryId, uint finalNumber, uint countWinningTickets);
    event NewOperatorAndTreasuryAndInjectorAddresses(address operator, address treasury, address injector);
    event NewRandomGenerator(address indexed randomGenerator);
    event TicketsPurchase(address indexed buyer, uint indexed lotteryId, uint numberTickets);
    event TicketsClaim(address indexed claimer, uint amount, uint indexed lotteryId, uint numberTickets);
    ...
    function buyTickets(uint _lotteryId, uint32[] calldata _ticketNumbers)external  notContract nonReentrant
    {
        ...
        uint amountCakeToTransfer = _calculateTotalPriceForBulkTickets(
    ...
    function claimTickets(
        ...
        for (uint i = 0; i < _ticketIds.length; i++) {
            ...
            uint rewardForTicketId = _calculateRewardsForTicketId(_lotteryId, thisTicketId, _brackets[i]);
    ...
    function closeLottery(uint _lotteryId) external  onlyOperator nonReentrant {
    ...
    function drawFinalNumberAndMakeLotteryClaimable(uint _lotteryId, bool _autoInjection)
        external
        onlyOperator
        nonReentrant
    {
        require(_lotteries[_lotteryId].status == Status.Close, "Lottery not close");
        uint finalNumber = _generateRandomNumber();

        uint numberAddressesInPreviousBracket;


        uint amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInCake) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;


        uint amountToWithdrawToTreasury;
    ...
    function injectFunds(uint _lotteryId, uint _amount) external onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");
    ...
    function startLottery(
        uint _endTime,
        uint _priceTicketInCake,
        uint _discountDivisor,
        uint[6] calldata _rewardsBreakdown,
        uint _treasuryFee
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            cakePerBracket: [uint(0), uint(0), uint(0), uint(0), uint(0), uint(0)],
            countWinnersPerBracket: [uint(0), uint(0), uint(0), uint(0), uint(0), uint(0)],
    ...
    function setMinAndMaxTicketPriceInCake(uint _minPriceTicketInCake, uint _maxPriceTicketInCake)
        external
        onlyOwner
    {
    ...
    function setMaxNumberTicketsPerBuy(uint _maxNumberTicketsPerBuy) external onlyOwner {
    ...
    function calculateTotalPriceForBulkTickets(uint _discountDivisor,uint _priceTicket,uint _numberTickets) external pure returns (uint) {
    ...
    function viewCurrentLotteryId() external view  returns (uint) {
        return currentLotteryId;
    }
    function viewLottery(uint _lotteryId) external view returns (Lottery memory) {
        return _lotteries[_lotteryId];
    }
    function viewNumbersAndStatusesForTicketIds(uint[] calldata _ticketIds)
        external
        view
        returns (uint[] memory, bool[] memory)
    {
        uint length = _ticketIds.length;
    ...
    function viewUserInfoForLotteryId(address _user,uint _lotteryId,uint _cursor,uint _size)external view returns
     ( uint[] memory,uint[] memory,bool[] memory, uint)
    {
        uint length = _size;
        uint numberTicketsBoughtAtLotteryId = _userTicketIdsPerLotteryId[_user][_lotteryId].length;

        if (length > (numberTicketsBoughtAtLotteryId - _cursor)) {
            length = numberTicketsBoughtAtLotteryId - _cursor;
        }
        uint[] memory lotteryTicketIds = new uint[](length);
        uint[] memory ticketNumbers = new uint[](length);
        bool[] memory ticketStatuses = new bool[](length);
    ...
    function _calculateRewardsForTicketId(uint _lotteryId,uint _ticketId,uint _bracket ) internal view returns (uint) {
        uint winningTicketNumber = _lotteries[_lotteryId].finalNumber;   
        uint userNumber = _tickets[_ticketId].number;    
        uint transformedWinningNumber = _bracketCalculator[_bracket] +
            (winningTicketNumber % (uint(10)**(_bracket + 1)));
        uint transformedUserNumber = _bracketCalculator[_bracket] + (userNumber % (uint(10)**(_bracket + 1)));
    ...
    function _calculateTotalPriceForBulkTickets(uint _discountDivisor,uint _priceTicket,uint _numberTickets) internal pure returns (uint) {
    ...
    function _generateRandomNumber() internal view returns(uint){
        return uint(keccak256(abi.encode(block.timestamp,block.number,block.difficulty,msg.sender))); 
    }
