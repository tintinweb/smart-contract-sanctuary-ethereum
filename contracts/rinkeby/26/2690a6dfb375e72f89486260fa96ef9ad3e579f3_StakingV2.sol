...
contract StakingV2 is Staking, StakingVerifier {
    function stake(StakeRequest calldata request, bytes calldata signature)
        ...
        onlyIfNotExpired(request.createdAt)
        whenNotPaused
        nonReentrant
    {
    ...
    AdminManagerUpgradable
{
    ...
    function getStakedResources(address account)
        external
        view
        returns (StakedResources memory)
    {
        return
            StakedResources(
                _stakersData[account].plotBalances,
                _stakersData[account].strainBalances,
                _stakersData[account].claims,
                _stakersData[account].lastClaims
            );
    }
...
contract StakingVerifier is EIP712Upgradeable {
    using ECDSAUpgradeable for bytes32;

    uint256 requestDuration;
    ...
    modifier onlyIfStakeAuthorized(
        ...
        require(signer == _authorizedSigner, "Stake not authorized");
        _;
    }
    ...
    modifier onlyIfWithdrawAuthorized(
        ...
        address signer = digest.recover(signature);
        require(signer == _authorizedSigner, "Withdraw not authorized");
        _;
    }
    ...
    modifier onlyIfNotExpired(uint256 createdAt) {
        require(
            block.timestamp - createdAt <= requestDuration,
            "Expired request"
        );
        _;
    }
