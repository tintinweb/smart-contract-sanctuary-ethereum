...
contract VoucherNFT is ERC721r, Ownable {

    constructor () ERC721r("VoucherFilm", "VCF", 32) {
        maxPerWallet = 2;
    }

    function mint(uint256 quantity) external {
        require(balanceOf(msg.sender) + quantity <= maxPerWallet, "exceed max wallet");
        _mintRandom(msg.sender, quantity);
    }

    function setIsPublicMintEnabled(bool action) external onlyOwner {
        isPublicMintEnabled = action;
    }
...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(baseTokenUri, "voucher_", Strings.toString(tokenId), ".json"));
    }
