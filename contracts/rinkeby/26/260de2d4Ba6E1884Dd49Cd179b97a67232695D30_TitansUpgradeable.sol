...
import {iKobalds} from "./interfaces/IKobalds.sol";
import {iShards} from "./interfaces/IShards.sol";
import {iRigToken} from "./interfaces/IRigToken.sol";
error SoldOut();
...
error NotOwner();
error NotEnoughKobaldsSent();

/// @title Titans
/// @author 0xSimon_
/// @notice Explain to an end user what this does
/// @dev Explain to a developer any extra details
contract TitansUpgradeable is ERC721AQueryableUpgradeable, OwnableUpgradeable {
    using ECDSA for bytes32;
    iShards private shards;
    iKobalds private kobalds;
    iRigToken private rigToken;

    uint256 public constant MAX_SUPPLY = 6555;
    uint256 public shardsRequiredToMintOne;
    uint256 public rigRequiredToMintOne;
    uint256 public numKobaldsRequiredToMintOne;
    ...
    function initialize() initializerERC721A initializer public {
        ...
        rigRequiredToMintOne = 200 ether;
        numKobaldsRequiredToMintOne = 1;
        uriSuffix = ".json";
    ...
    function mint(
        ...
        //Make sure enuogh kobalds are sent
        if (kobaldIds.length != amount * numKobaldsRequiredToMintOne)
            revert NotEnoughKobaldsSent();
        //Make sure sender of tx is owner of all kobaldIds
        for (uint256 i; i < kobaldIds.length; ++i) {
            if (_msgSender() != kobalds.ownerOf(kobaldIds[i]))
                revert NotOwner();
            kobalds.burn(kobaldIds[i]);
        }
        ...
        //Burn RigToken. Will revert if user doesen't have sufficient balance
        rigToken.burn(_msgSender(), amount * rigRequiredToMintOne);
    ...
    */
    ...
    function setNumKobaldsRequiredToMintOne(
        uint256 _numKobaldsRequiredToMintOne
    ) public onlyOwner {
        numKobaldsRequiredToMintOne = _numKobaldsRequiredToMintOne;
    }

    function setRigRequiredToMintOne(uint256 _rigRequiredToMintOne)
        public
        onlyOwner
    {
        rigRequiredToMintOne = _rigRequiredToMintOne;
    }
    ...
    function setRigToken(address _address) external onlyOwner {
        rigToken = iRigToken(_address);
    }
