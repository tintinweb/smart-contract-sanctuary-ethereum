    ...
    Signature
{
    ...
    modifier onlyWhiteList(uint256 _poolId) {
        ...
        require(
            ...
            "StableFinanceStakingPool: not able to stake"
        );
        _;
    }
        ...
        stableFinanceValidatePoolById(_poolId)
    {
        ...
        if (
            pool.delayDuration > 0 && pool.option == VestingOption.VestingReward
        ) {
        ...
        if (pool.option == VestingOption.VestingAll) {
            bytes32 msgHash = getMessageHash(_poolId, _amount, account);
