...
contract Today is IERC721, IERC2981, ERC721Enumerable, ReentrancyGuard, Ownable
{
        ...
        uint public constant PRICE_MAX = 0.012 ether;
        uint public constant PRICE_DECREASE = 0.0005 ether;

        uint private constant NY_TIME_DIFF_HOURS = 4;
        uint private constant HOUR_IN_SECONDS = 3_600;
        uint private constant DAY_IN_SECONDS = 86_400;
        uint private constant YEAR_IN_SECONDS = 31_536_000;
        uint private constant LEAP_YEAR_IN_SECONDS = 31_622_400;
        uint16 private constant ORIGIN_YEAR = 1970;

        uint256 private constant LIFESPAN = 29_771 days;
        uint256 private deployedTime;
        uint private royaltyPercentage = 5;
	...
	{
                uint _nyTime = getNYTime(block.timestamp);
                uint hour = (_nyTime / 1 hours) % 24;
	...
	function getTimeStrDebug() public view returns (uint)
	{
                uint256 time = block.timestamp;
		...
		return  uint256(year)   * 100000000 + 
                        uint256(month)  * 1000000 + 
                        uint256(day)    * 10000 + 
                        uint256(hour)   * 100 + 
                        uint256(minute);
	}
        ...
        function parseTime(uint256 time) internal pure returns (uint16 year, uint8 month, uint8 day)
        {
        ...
        function getYear(uint time) internal pure returns (uint16)
        {
                uint secondsAccountedFor = 0;
                uint16 year;
                uint numLeapYears;
        ...
        function getNYTime(uint256 time) internal pure returns (uint)
        {
