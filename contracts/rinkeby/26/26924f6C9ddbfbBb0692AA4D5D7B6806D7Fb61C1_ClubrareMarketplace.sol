    ...
    IClubrareMarketplace
{
    ...
    mapping(uint256 => bool) internal bids;
    ...
    function initialize(
        ...
        payment = IMarketplacePayment(_payments);
    ...
    modifier onlySeller(Order calldata order) {
        require(validator.verifySeller(order), "Not a seller");
        _;
    }
    ...
    {
        validator.checkTokenGate(order);
        ...
        isExpired(order)
    {
        ...
        uint256 id = generateId(order.objId);
        Auction memory _auction = auctions[id];
        Order calldata _order = order;
    ...
    function claim(Order calldata order) external whenNotPaused nonReentrant isNotBlacklisted(msg.sender) {
        ...
        require(validator.verifySeller(order) || msg.sender == auction.highestBidder, "not a valid caller");
    ...
    function _validate(Order calldata order, uint256 amount) internal returns (address, uint256) {
        ...
        require(validator.validateOrder(order, paid), "Invalid Order");
        return (signer, paid);
    }
    ...
    function settlement(
        ...
        uint256 sellerEarning = payment._chargeAndSplit(amt, taker, _order.paymentToken, royaltyAmt, creator);
    ...
    function invalidateSignedOrder(Order calldata order) external whenNotPaused nonReentrant {
        ...
        require(bids[generateId(order.objId)], "bid exit on item");
    ...
    function generateId(string memory _id) public pure returns (uint256) {
        return uint256(keccak256(abi.encodePacked(_id))) % 1000;
    }
