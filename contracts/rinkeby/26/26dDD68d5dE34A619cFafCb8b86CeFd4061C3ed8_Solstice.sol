...
contract Solstice {
    ...
    int256 immutable J2000 = 2451545000000000000000000;
    int256 immutable JulianCentury = 36525000000000000000000;
    int256 immutable D2R = PRBMathSD59x18.pi().div(180000000000000000000);

    // table 27.b - for years from +1000 to +3000
    int256[5] mc = [int(2451623809840000000000000), int(365242374040000000000000), int(51690000000000000), int(-4110000000000000), int(-570000000000000)];
    int256[5] jc = [int(2451716567670000000000000), int(365241626030000000000000), int(3250000000000000), int(8880000000000000), int(-300000000000000)];
    int256[5] sc = [int(2451810217150000000000000), int(365242017670000000000000), int(-115750000000000000), int(3370000000000000), int(780000000000000)];
    int256[5] dc = [int(2451900059520000000000000), int(365242740490000000000000), int(-62230000000000000), int(-8230000000000000), int(320000000000000)];
    ...
    function currentSeason() public view returns (int256) {
        int256 currentYear = timestampToYear(block.timestamp);
        return currentYear;
    }

    function spring(int256 year) public view returns (int256) {
        return equinox(year - 2000000000000000000000, mc);
    }

    function summer(int256 year) public view returns (int256) {
        return equinox(year - 2000000000000000000000, jc);
    }

    function fall(int256 year) public view returns (int256) {
        return equinox(year - 2000000000000000000000, sc);
    }

    function winter(int256 year) public view returns (int256) {
        return equinox(year - 2000000000000000000000, dc);
    }

    function equinox(int256 year, int256[5] storage term) internal view returns (int256) {
        ...
        int256 dL = 1 + Trigonometry.cos(uint(W)).mul(33400000000000000) + Trigonometry.cos(uint(W.mul(2))).mul(700000000000000);
        int256 S = 0;
    ...
    function J2000Century(int256 jde) internal pure returns (int256) {
        return (jde - J2000).div(JulianCentury);
    }

    function timestampToYear(uint256 timestamp) internal pure returns (int256) {
        return PRBMathSD59x18.fromInt(1970 + int(timestamp) / 60 / 60 / 24 / 365);
    }
}
