...
contract Offer {
    ...
    {
        AuctionLibrary.subOffer memory subOfferInstance = Offers[_indexId]
    ...
    function getInforOfferBasic(bytes memory _indexId)
        external
        view
        returns (
            uint256,
    ...
    function updateOwnerOfNFT(bytes memory _indexId, address _user)
        external
        onlyAuction
    {
...
interface IStanNFT {
    ...
    function getOwnerOfNFTMobile(bytes memory _nftId)
        external
        view
        returns (address);

    function updateOwnerOfMobile(bytes memory _nftId, address _owner) external;
}
