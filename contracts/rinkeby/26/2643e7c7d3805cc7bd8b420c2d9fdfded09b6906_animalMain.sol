...
contract animalMain is  ERC721,Ownable {
  ...
  function transfer(address _to, uint256 _tokenId) public {
    require( msg.sender == animalToOwner[_tokenId]);//TO DO 請使用require判斷要轉的動物id是不是轉移者的
    ownerAnimalCount[msg.sender].sub(1);//增加受贈者的擁有動物數量
    ownerAnimalCount[_to].add(1);//減少轉出者的擁有動物數量
