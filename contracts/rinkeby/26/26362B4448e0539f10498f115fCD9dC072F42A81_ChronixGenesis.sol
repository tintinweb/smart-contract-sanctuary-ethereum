...
contract ChronixGenesis is ERC721A, Ownable, ReentrancyGuard {
    ...
    function withdraw() public onlyOwner {
        (bool hs, ) = payable(0x039eA3371Da302cA3ea9192e3801B1E9baD6e207).call{value: address(this).balance * 48 / 100}("");  // 48% Jarveth
        require(hs);
        (bool qs, ) = payable(0x4a40Ef1DA0e73A0df142D06470D7a2cfFC677D2f).call{value: address(this).balance * 20 / 100}("");  // 20% Krassenstein
        require(qs);
        (bool bs, ) = payable(0x36dfA2B19C71F228ED8Aa46de163Cc9a61408527).call{value: address(this).balance * 9 / 100}("");   // 9% Arzeny
        require(bs);
        (bool cs, ) = payable(0x4E0ebdAfACB39Cf80FB0d99DD084af63DEa01A6A).call{value: address(this).balance * 7 / 100}("");   // 7% Medcro
        require(cs);
        (bool ds, ) = payable(0xda7d9d7E5D27c019DCb6f30cFF83FA9B8aD15949).call{value: address(this).balance * 8 / 100}("");   // 8% Camjo
        require(ds);
        (bool es, ) = payable(0x3751bDD78C8409b4aE5101B9206F3a7B192123ad).call{value: address(this).balance * 4 / 100}("");   // 4% Liquidity Pool
        require(es);
        (bool fs, ) = payable(0x5c3A1fa85CEBCB46D3a0bEdADe0453dd7aeD606A).call{value: address(this).balance * 2 / 100}("");   // 2% Viva
        require(fs);
        (bool gs, ) = payable(0xcad86d42672439cAdC077105Cef1Cb057Dc1EAE8).call{value: address(this).balance * 2 / 100}("");   // 2% Leiane
        require(gs);   
    }
}
