...
contract HappyAnimals is ERC721, Ownable {
  ...
  uint256 private maxSupplyTotal = 98;
  uint256 private maxSupplyPrivate = 60;
  uint256 private constant price = 0.01 ether;
  uint256 private constant maxPerMint = 30;
  ...
  ) ERC721("Happy Animals", "HAMS") {
    setURIPrefix(_uriPrefix);
    setHiddenMetadataURI(_hiddenMetadataURI);

    paymentsContract = address(_paymentsContract);
    weth = IERC20(address(0xc778417E063141139Fce010982780140Aa0cD5Ab));
  }

  function setURIPrefix(string memory _uriPrefix) private {
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    return
      bytes(currentBaseURI).length > 0 ? string(abi.encodePacked(currentBaseURI, tokenId.toString(), uriSuffix)) : "";
  }
  ...
  function mintAnimals(uint256 _count, uint256 amount) public {
    uint256 totalMinted = totalSupply();

    require(whitelistedWallets[msg.sender], "This wallet is not authorized to mint.");
    require(!paused, "Minting paused by owner.");
    require(totalMinted.add(_count) <= maxSupplyPrivate, "Not enough NFTs.");
    require(_count > 0 && _count <= maxPerMint, "Cannot mint specified number of NFTs.");
    require(amount >= price.mul(_count), "Not enough balance to mint NFTs.");
    require(
      (balanceOf(msg.sender) + _count) <= maxPerWallet,
      "The number of mints selected will exceed the maximum amount of mints allowed per wallet. Please consider selecting a lower number of mints to proceed."
    );

    weth.transferFrom(msg.sender, paymentsContract, amount);
  ...
  function isWhitelisted(address target) public view returns (bool) {
    return whitelistedWallets[target];
  }
  ...
  function reserveAnimals(uint256 _count) public onlyOwner {
    uint256 totalMinted = totalSupply();

    require(totalMinted.add(_count) <= maxSupplyTotal, "Not enough NFTs.");
