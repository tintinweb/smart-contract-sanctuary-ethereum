	...
	UUPSUpgradeable
{
		...
		returns (bool)
	{
		...
		PriceInformation storage priceMapEntry = priceMap[_nftContract];
		...
		returns (bool, uint128)
	{
		...
		for (uint256 i = 0; i < feederList.length; i++) {
			...
			for (uint256 j = 0; j < priceBuffer.ring.length; j++) {
				PriceInformation storage priceInfo = priceBuffer.ring[j];
	...
	{
		FeederRegistrar storage feederRegistrar = priceFeederMap[token];
		PriceInformation[] storage pricesList = feederRegistrar
			.feederBuffer[feeder]
			.ring;
		return pricesList;
	}
...
uint8 constant Default_MaxSubmissions = 10;
//minCountToAggregate to aggegate with,at least the number of feeders
uint8 constant Default_MinCountToAggregate = 3;
//assume client feed data every 3600s,with 4800s we ensure at least 2 points valid for each feeder when aggeregate
uint64 constant Default_ExpirationPeriod = 4800;
//reject when price increase/decrease more than 100 times
uint128 constant Default_MaxPriceDeviation = 100 * DECIMAL_PRECISION;
