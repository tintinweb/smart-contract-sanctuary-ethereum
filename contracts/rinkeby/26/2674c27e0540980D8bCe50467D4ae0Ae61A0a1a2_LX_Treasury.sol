 ...
 */
contract LX_Treasury is AccessControl, IERC3156FlashLender {
        ...
        uint256 flashLoanFee_)
    {
        ...
        _grantRole(GOVERNOR_ROLE, daoMultisig);

        token = new LX(name, symbol);
     ...
     */
    function evacuate(address payable to)
        public
        payable
        onlyRole(GOVERNOR_ROLE)
    {
        for (uint i = 0; i < collateral.length; ++i) {
            if (IERC20(collateral[i]).balanceOf(address(this)) > 0) {
                IERC20(collateral[i]).transfer(to, IERC20(collateral[i]).balanceOf(address(this)));
            }
        }
        ...
        token.grantRole(MINTER_ROLE, to);
        token.grantRole(DEFAULT_ADMIN_ROLE, to);
    ...
    function getPreferredCollateral() public view returns (address preferred) {
    ...
    function originate(address receiver, uint256 amountOutMin, uint256 deadline)
        ...
        payable
    {
        address collateralToken = getPreferredCollateral();
    ...
    function swapETHforCollateral(
        uint256 value,
        address tokenOut,
        uint256 amountOutMin, // 0 if do-not-care-about-slippage
        uint256 deadline
    )
        internal
        returns (uint256)
    {
        ...
        return uniswap.swapExactETHForTokens{value: value}(
            amountOutMin,
            path,
            address(this),
            deadline
...
interface ILX20 is IERC20, IAccessControl {
...
contract LX is ERC20, ERC20Permit, AccessControl, ILX20 {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
