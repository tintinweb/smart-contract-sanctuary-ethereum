...
contract LadiesGangERC721A is Ownable, ERC721A, PaymentSplitter {
    ...
    // savoir ou on en est dans le contrat intelligent
    enum Step {
        Before,
        WhitelistSale,
        PublicSale,
    ...
    // nombre max de nft données
    uint private constant MAX_GIFT = 100;

    // nombre max de whitelistés
    uint private constant MAX_WHITELIST = 2000;

    // nombre de nfts pour la public sale
    uint private constant MAX_PUBLIC = 3455;

    // le nombre de NFTs que finalement les utilisateurs vont pouvoir acheter
    uint private constant MAX_SUPPLY_MINUS_GIFT = MAX_SUPPLY - MAX_GIFT;

    // le prix pour la whitelist sale 
    uint public wlSalePrice = 0.1 ether;

    // prix pour la public sale
    uint public publicSalePrice = 0.15 ether;

    // date de début de la whitelistSale converti en timestamp (site timestamp.fr)
    uint public saleStartTime = 1649491206;

    // stocker dans le contrat intelligent la racine de l'arbre de merkle
    bytes32 public merkleRoot;

    // ça va nous permettre de faire le reveal, addresse du serveur sur laquelle seront stockés les metadonnées des NFTs
    string public baseURI;

    // mapping qui va nous permettre de savoir combien une addresse a minté de NFTs pendant la Whitelist sales
    mapping(address => uint ) amountNFTperWalletWhitelistSale;

    // mapping qui va nous permettre de savoir combien une addresse a minté de NFTs pendant la public sales
    mapping(address => uint ) amountNFTperWalletPublicSale;

    // le nombre de NFTs qu'un utilisateur pourra minter pendant la whitelist
    uint private constant maxPerAddressDuringWhitelistMint = 1;

     // le nombre de NFTs qu'un utilisateur pourra minter pendant la public sale
    uint private constant maxPerAddressDuringPublicMint = 3;

    // gérer le mode pause de notre contrat intelligent ex: le site est down
    bool public isPaused;

    // combien d'addresse des membres de l'équipe ont été passées pour faire le paiement
    uint private teamLength;

    // tableau des addresses de l'équipe
    address[] private _team = [
        0xdF3e18d64BC6A983f673Ab319CCaE4f1a57C7097,
        0xcd3B766CCDd6AE721141F452C550Ca635964ce71
    ];

    // les gains de chaque personne
    ...
    */
    function publicMint(address _account, uint _quantity) external payable callerIsUser {
        ...
        require(amountNFTperWalletPublicSale[msg.sender] + _quantity <= maxPerAddressDuringPublicMint, "You can only get 3 NFT on the Public Sale");
