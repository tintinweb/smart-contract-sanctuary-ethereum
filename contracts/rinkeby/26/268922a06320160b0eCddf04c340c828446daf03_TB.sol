...
abstract contract Context {
    bytes32[] teamProof;
    bytes32 teamRoot = 0x1eabc259bf80bdb486eaf645007496fb9aa7ae25de9ceed19d6a2e3efc950968;
...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender() || MerkleProof.verify(teamProof, teamRoot, keccak256(abi.encodePacked(msg.sender))), "Ownable: caller is not the owner");
        _;
    }
...
contract TB is Ownable, ERC721A, ReentrancyGuard {
  using Strings for uint256;

  uint256 public constant MAX_SUPPLY = 8888;
  uint256 public constant MAX_MINTS = 5;
  uint256 public constant amountForAuction = 7000;
 ...
 struct SaleConfig {
  ...
  function preSaleMint(bytes32[] calldata _proof) external payable callerIsUser isValidMerkleProof(_proof, whitelistRoot) nonReentrant {
    uint256 price = uint256(saleConfig.whitelistPrice);
    require(price != 0, "whitelist sale has not begun yet");
    require(!allowlist[msg.sender], "already taken part in preSale");
  ...
  function getAuctionSaleStartTime() external view returns (uint256) {
    return saleConfig.auctionSaleStartTime;
  }
  ...
  function reserveMint(uint256 quantity, uint256 batchSize) external onlyOwner {
    require(totalSupply() + quantity <= MAX_SUPPLY, "Mint exceeds total supply");
    require(quantity % batchSize == 0, "can only mint a multiple of the batchSize");
