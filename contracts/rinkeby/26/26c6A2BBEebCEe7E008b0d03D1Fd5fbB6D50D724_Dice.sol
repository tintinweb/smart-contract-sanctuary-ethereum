...
abstract contract Manager is Ownable {
    ...
    uint public lev1Token = 0;
    uint public lev2Token = 0;
    uint public lev3Token = 0;
...
interface IHouse {
    function placeBet(address player, address refAddr,  uint amount, bool isBonus, uint[] memory amountArray) payable external;
...
interface IDicewar {
    function balanceOf(address _userAddr) external returns(uint256);
...
interface IDicewarNft {
    ...
    */
contract Dice is ReentrancyGuard,Manager{
    ...
    function register (address _addr, address _refAddr, string memory _profileName, string memory _profileLink) external returns(bool){
     ...
     function userInfo () external view returns(uint256 id,
        address addr,
        address refAddr,
        uint256 balance,
        uint256 dicewarToken,
        ...
        user[msg.sender].refAddr,
        user[msg.sender].balance,
        user[msg.sender].dicewarToken,
        user[msg.sender].profileName,
        user[msg.sender].profileLink);
    ...
    function placeBet(uint betChoice, uint bonus, uint note) external payable nonReentrant {
        ...
        amountArray[4] = amountToWinnableAmount(amount, rollUnder);
        amountArray[5] = amountToRefConverter(amount - bettableAmount);
        
        house.placeBet{value: msg.value}(msg.sender, user[msg.sender].refAddr ,amount, isBonus, amountArray);
    ...
    function upgradeNft(address _addr, uint8 level) external {
        ...
        if(level == 1){
            require(dicewar.balanceOf(msg.sender) >= lev1Token);
            dicewar.burn(_addr, lev1Token);
        }
        if(level == 2){
            require(dicewar.balanceOf(msg.sender) >= lev2Token);  
            dicewar.burn(_addr, lev2Token);

        }
        if(level == 3){
            require(dicewar.balanceOf(msg.sender) >= lev3Token);
