...
contract WWWV2 is ERC721Ded, Ownable {
    ...
    function setMaxMint(uint256 _maxMint) external onlyOwner {
        require(_maxMint > maxMint, "max mint should be higher");
        maxMint = _maxMint;
    }

    function mint(uint256 _amount) external {
        require(saleIsActive, "sale has not begun yet");
        require(totalSupply() + _amount <= MAX_SUPPLY, "reached max supply");
        require(
            balanceOf(msg.sender) + _amount <= maxMint,
    ...
    function gift(address _dedz, uint256 _amount) external onlyOwner {
        require(totalSupply() + _amount <= MAX_SUPPLY, "reached max supply");
        _safeMint(_dedz, _amount);
    }
