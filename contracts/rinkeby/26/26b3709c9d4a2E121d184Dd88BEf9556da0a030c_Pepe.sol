...
contract Pepe is ERC721A, IERC721CollectionMetadata, IERC2981, Ownable, Pausable, ReentrancyGuard {
    // Total token limit
    ...
    function setMetadataBaseURI(string calldata newMetadataBaseURI) external onlyOwner {
        require(!isMetadataFinalized, 'Pepe: metadata is now final');
        metadataBaseURI = newMetadataBaseURI;
    }

    function finalizeMetadata() external onlyOwner {
        require(!isMetadataFinalized, 'Pepe: metadata is already finalized');
        isMetadataFinalized = true;
    }

    function setRoyaltyBasisPoints(uint16 newRoyaltyBasisPoints) external onlyOwner {
        require(newRoyaltyBasisPoints >= 0, 'Pepe: royaltyBasisPoints must be >= 0');
        require(newRoyaltyBasisPoints < 5000, 'Pepe: royaltyBasisPoints must be < 5000 (50%)');
        royaltyBasisPoints = newRoyaltyBasisPoints;
    }

    function setIsAllowlistSaleActive(bool newIsAllowlistSaleActive) external onlyOwner {
        require(mintPrice >= 0 || !newIsAllowlistSaleActive, 'Pepe: cannot start if mintPrice is 0');
        require(allowlistMerkleRoot != 0, 'Pepe: cannot start if allowlistMerkleRoot not set');
        isAllowlistSaleActive = newIsAllowlistSaleActive;
    }
    ...
    // NOTE(krishan711): this is deifned without the .json extension in ERC721A
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), 'Pepe: URI query for nonexistent token');
        return string(abi.encodePacked(metadataBaseURI, _toString(tokenId), '.json'));
    }
    ...
    modifier callerIsUser() {
        require(tx.origin == _msgSender(), "Pepe: The caller is another contract");
        _;
    }
    ...
    function allowlistMint(uint256 quantity, bytes32[] calldata proof) public payable {
        require(quantity > 0 && quantity <= transactionMintLimit, 'Pepe: invalid quantity');
        require(isAllowlistSaleActive && mintPrice > 0, "Pepe: allowlist sale not active");
        require(msg.value >= mintPrice * quantity, "Pepe: insufficient payment");
        require(mintCounts[_msgSender()] + quantity <= addressMintLimit, "Pepe: adress mint limit reached");
        require(_verifyMerkleLeaf(_generateMerkleLeaf(_msgSender()), allowlistMerkleRoot, proof), "Pepe: invalid proof");
    ...
    function _innerMint(address to, uint256 quantity) internal callerIsUser nonReentrant whenNotPaused {
        require(totalSupply() + quantity <= collectionSize, 'Pepe: quantity out of bounds');
