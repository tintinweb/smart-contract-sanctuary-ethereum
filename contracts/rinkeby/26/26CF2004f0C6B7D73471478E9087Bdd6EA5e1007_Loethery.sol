...
contract Loethery is VRFConsumerBaseV2 {
    ...
    function finishLottery() public payable onlyOwner{
        ...
        for(uint i = 0; i < numWords; i++){
            ...
            lotteryHistory[lotteryId].winners[i] = players[index];
            players[index].transfer(address(this).balance * payments[i] / 100);
        }

        lotteryId++;
        players = new address payable[](0); // resetam array-ul de playeri
        
    }

    function withdraw() public payable onlyOwner {
        payable(msg.sender).transfer(address(this).balance); // Restul revine la noi
    }
