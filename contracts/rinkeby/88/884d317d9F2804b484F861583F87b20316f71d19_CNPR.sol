...
contract CNPR is CNPRadmin, ERC721A {
    // OpenSea's Proxy Registry
    // IProxyRegistry public immutable proxyRegistry;

    // ICnprTokenURI public descriptor;

    constructor(address _adminSigner) ERC721A("CNPR Demo", "CNPROOKIES") {
        adminSigner = _adminSigner;
        descriptor = new CnprDescriptor();
        // _safeMint(WITHDRAW_ADDRESS, 3);
    }
    ...
    function presaleMint(
        ...
        bytes32 digest = CouponSet.createDigest(
            CouponSet.CouponType.Presale,
            _allotted
        );

        // uint64 numWhitelistMinted = _getAux(_msgSender()) + _quantity;

        require(
            CouponSet._isVerifiedCoupon(digest, _coupon, adminSigner),
            "Invalid coupon"
        );

        require(
            addressToMints[msg.sender]._numberOfMintsByAddress + _quantity <=
                _allotted,
    ...
    /**
     *  @notice Make it available for sale at a public sale in an emergency.
     *  @param _quantity Amount of tokens.
     */
    function publicSaleMint(uint256 _quantity) external payable {
        require(phase == SalePhase.PublicSale, "public sale is not activated");

        require(MINT_COST * _quantity <= msg.value, "not enough eth");

        require(
            _quantity <= maxMintAmountForPublicSale,
     ...
     */
    function burnMint(
        uint64[] memory _burnTokenIds,
        uint256 _allotted,
        CouponSet.Coupon memory _coupon
    ) external payable {
        require(phase == SalePhase.BurnMint, "BurnMintis not activated");
        // uint64 numWhitelistMinted = _getAux(_msgSender()) +
        //     uint64(_burnTokenIds.length);
        ...
        require(
            addressToMints[msg.sender]._numberOfBurnMintsByAddress[
                burnMintIndex
            ] +
        ...
        // addressToMints[msg.sender]._numberOfBurnMintsByAddress += _burnTokenIds
        //     .length;
    ...
    // function getAux(address _address) internal view returns (uint64) {
 ...
 *  @dev A collection of functions that can only be operated by the owner.
 */
abstract contract CNPRadmin is CNPRcore {
    ...
    function setAdminSigner(address _adminSigner) external onlyOwner {
        require(_adminSigner != address(0), "_adminSigner shouldn't be 0");
        adminSigner = _adminSigner;
    }
     ...
     *  @dev Only callable by the owner.
     *  @param _phase The phase of the project.
     */
    ...
    /**
     *  @notice Set the max amount that can be minted at a public sale.
     *  @dev Only callable by the owner.
     *  @param _amount The max amount that can be minted at a public sale.
     */
    function setMaxMintAmountForPublicSale(uint256 _amount) external onlyOwner {
        maxMintAmountForPublicSale = _amount;
    }
     ...
     */
    function setBurnMintCost(uint256 _cost) external onlyOwner {
        burnMintCost = _cost;
    }
     ...
     */
    function setMaxBurnMintSupply(uint256 _amount) external onlyOwner {
        maxBurnMintSupply = _amount;
    }

    /**
     *  @notice Set the index to change the burn mint count each time.
     *  @dev Only callable by the owner.
     *  @param _index The index of the burn mint.
     */
    function setBurnMintIndex(uint256 _index) external onlyOwner {
        burnMintIndex = _index;
    }

    function setCnprDescriptor(ICnprDescriptor _descriptor) external onlyOwner {
    ...
    /**
     *  @notice Get the address to be used for ECDSA authentication.
     *  @dev Only callable by the owner.
     *  @return adminSigner The address of the adminSigner.
     */
    function getAdminSigner() external view onlyOwner returns (address) {
        return adminSigner;
    }

    /**
     *  @notice Get the number minted in the presales set for the address.
     *  @param _address The address to be set in addressToMints.
     *  @return The number of presales mints set at the address.
     */
    function getPresaleWhitelistCount(address _address)
        external
        view
        returns (uint256)
    {
        return addressToMints[_address]._numberOfMintsByAddress;
    }

    /**
     *  @notice Get the number of burn mints set for an address.
     *  @param _address The address to be set in addressToMints.
     *  @return The number of burn mints set at the address.
     */
    function getBurnMintWhitelistCount(address _address)
        external
        view
        returns (uint256)
    {
        return
            addressToMints[_address]._numberOfBurnMintsByAddress[burnMintIndex];
    }
}
...
// import {ICouponSet} from "../interface/ICouponSet.sol";

/**
 *  @title Coupon set methodsspecified address.
...
library CouponSet {
     ...
     */
    function createDigest(CouponType _couponType, uint256 _allotted)
        internal
        view
        returns (bytes32)
    {
        return keccak256(abi.encode(_couponType, _allotted, msg.sender));
    }
     ...
     *  @param _digest A 32-byte hash created from the coupon type, the total number of coupons that can be requested, and the sender's address.
     *  @param _coupon Coupon for verifying the signer.
...
contract CnprDescriptor is ICnprDescriptor, Ownable {
    ...
    string public override baseURI =
        "ipfs://Qmf6qDP4wZKzZAppAkUcNdho3xfSB9vTWGZE1zRUjtVLFJ/";

    // The Extension of URI
    string public override baseExtension = ".json";
...
interface ICnprDescriptor {
    ...
    function baseExtension() external returns (string memory);
    ...
    function setBaseExtension(string memory _baseExtension) external;
}
...
abstract contract CNPRcore is Ownable {
    ...
    //     uint256 _numberOfBurnMintsByAddress;
    // }
    // Mint type info
    struct MintInfo {
        uint256 _numberOfMintsByAddress;
        mapping(uint256 => uint256) _numberOfBurnMintsByAddress;
    }
    ...
    address public constant WITHDRAW_ADDRESS =
        0x35e20c7d9d54cB02B43CF8B31aa47BF9e6FBe830;
    ...
    // The CNPR token max mint amount for public sale
    uint256 public maxMintAmountForPublicSale = 2;

    // Maximum number of BurnMint that can be done
    uint256 public maxBurnMintSupply = 7777;
    ...
    // Mapping the number of presale and burn mints to addresses
    mapping(address => MintInfo) public addressToMints;
}
