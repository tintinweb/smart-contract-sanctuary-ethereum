...
// File: contracts/TestingTheThings2.sol
...
contract TestingTheThing is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  uint256 public reservedForThings = 20; // Will be used for promotions and give-aways

  bool public paused = true;
  bool public whitelistMintEnabled = false;
  bool public revealed = false;

  constructor() ERC721A("TestingTheThing", "TST123") {
    setHiddenMetadataUri("ipfs://xxxxx/hidden.json");
  }

// Check one
  modifier mintCompliance(uint256 _mintAmount) {
    require(_mintAmount > 0, "Lol, cannot mint zero things");
    require(_mintAmount <= maxMintAmountPerTx, "Oh no, cannot mint that many things!");
    require(totalSupply() + _mintAmount <= maxSupply, "Maximum of things exceeded!");
    _;
  }
...
// Function whitelist mint
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    require(ownerMintedCount + _mintAmount <= addressLimitPresale, "Maximum things per address exceeded");
...
// Update the value for addressPresaleMintedBalance at this address
    function setPMB(address _addr, uint _i) public onlyOwner {
        addressPresaleMintedBalance[_addr] = _i;
    }

// Reset the value for addressPresaleMintedBalance to the default value.
    function removePMB(address _addr) public onlyOwner {
