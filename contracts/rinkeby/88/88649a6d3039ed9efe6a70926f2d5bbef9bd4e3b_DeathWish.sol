...
contract DeathWish is ReentrancyGuard {
    ...
    function inspectSwitchAs(uint256 id, address _user) external view returns (uint256, address, address, TokenType, uint256, uint256) {
        require(id < counter, "Out of range");
        Switch memory _switch = switches[id];
        return (switchClaimableByAt(id, _user), _switch.user, _switch.tokenAddress, _switch.tokenType, _switch.tokenId, _switch.amount);
    }
