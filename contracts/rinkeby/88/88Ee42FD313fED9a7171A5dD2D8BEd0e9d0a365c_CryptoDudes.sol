...
contract CryptoDudes is Ownable, ERC721A, PaymentSplitter {

    using Strings for uint;

    // *************** selling steps
    // Sale: 1st step, allow mints
    // Gift: only allowed after the presale and public sale
    ...
    uint public MAX_PUBLIC = 6;
    uint public MAX_GIFT = 2;

    uint public MAX_MINT_PER_TRANSACTION = 10;
    uint public MAX_NFT_PER_WALLET_WHITELIST = 1;

    uint public WHITELIST_DURATION = 15 minutes;
    ...
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(sellingStep == Step.Sale, "Sale is not activated");
        require(isWhiteListed(msg.sender, _quantity, _proof), "Not whitelisted");
        require(amountNFTsperWalletWhitelistSale[msg.sender] + _quantity <= MAX_NFT_PER_WALLET_WHITELIST, "You can only get 1 NFT during the Whitelist Sale");
    ...
    function publicSaleMint(address _account, uint _quantity) external payable callerIsUser {
        ...
        require(currentTime() > saleStartTime + WHITELIST_DURATION, "Public Sale has not started yet");
        ...
        // if we reach the max minted here, automatically switch to the Gift Step (team and gift mint)
        if (totalSupply() + _quantity == MAX_WHITELIST + MAX_PUBLIC) {
            sellingStep = Step.Gift;
        }
        _safeMint(_account, _quantity);
    }
    ...
    function gift(address _to, uint _quantity) external onlyOwner {
        require(sellingStep > Step.Sale, "No Gift during the public sale");
        require(totalSupply() + _quantity <= MAX_SUPPLY, "Reached max Supply");
        require(_quantity <= MAX_GIFT, "Can't gift more than MAX_GIFT");
    ...
    function setRevealedUri(string memory _baseURI) external onlyOwner {
        require(sellingStep == Step.End, "Can only update the revealed URI after the sales");
