...
contract AccountCreation is ERC721Enumerable {
    ...
    function createAccount(string memory newName) public virtual {
        require(
            bytes(userName[msg.sender]).length == 0,
            "Account already created"
        );
        require(validateName(newName), "Not a valid new name");
        require(!isNameReserved(newName), "Username already exists");
        _nameReserved[toLower(newName)] = true;
        userName[msg.sender] = newName;
        addrToName[newName] = msg.sender;
        emit AccountCreated(newName);
    }
