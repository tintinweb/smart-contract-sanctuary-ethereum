    ...
    HoloNovelNFTAccessControls
{
    ...
    function withdraw() public payable onlyOwner {
        (bool withdrawSuccess,) = payable(owner()).call{value : address(this).balance}("");
        ...
        returns (uint256)
    {
        ...
        require(msg.value >= mintPrice[voucher.tokenId], "Insufficient funds to redeem");
    ...
    /// @notice Set mint price for given token Id
    /// @dev Only Admin should be callable for this function
    /// @param _tokenId given token Id
    /// @param _mintPrice wei value to mint
    function setMintPrice(uint256 _tokenId, uint256 _mintPrice) external onlyAdmin {
        mintPrice[_tokenId] = _mintPrice;
    }

    /// @dev external function to set the token URI prefix.
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "NFTVoucher(uint256 tokenId,uint256 quantity,bytes data)"
                        ),
                        voucher.tokenId,
                        voucher.quantity,
