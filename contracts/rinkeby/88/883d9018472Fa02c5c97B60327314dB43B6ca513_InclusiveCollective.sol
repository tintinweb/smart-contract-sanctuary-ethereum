...
// File: contracts/InclusiveCollective.sol
...
contract InclusiveCollective is ERC721URIStorage, Ownable, ERC2981 {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    // Mapping from owner address to token ID
    // mapping(address => uint256[]) public _token_owners;
    // string constant PLACEHOLDER = "ifps://QmTqs6hzUEjUbNrUjJg9cb5ECPLQAosNECVrFAx4vat2Pp";
    uint256 constant MIN_MINT_PRICE = 0.03 ether;
    uint256 constant MAX_SUPPLY = 10000;

    // uint256 mint_price_by_owner = 0;
        ...
        returns (string memory)
    {
        if (bytes(base_URI).length == 0)
            return "ifps://QmTqs6hzUEjUbNrUjJg9cb5ECPLQAosNECVrFAx4vat2Pp";
        //PLACEHOLDER;
        else
            return
                string(
    ...
    //     mint_price_by_owner = price;
    // }

    function mint() public payable returns (uint256) {
        // if (mint_price_by_owner > 0) {
        //     require(
        //         msg.value >= mint_price_by_owner * 1 ether,
        //         "Need to pay up!"
        //     );
        // } else {
        uint256 mint_price = 0;
        if (_tokenIdCounter.current() + 1 < 1001) {
            mint_price = MIN_MINT_PRICE;
        } else if (_tokenIdCounter.current() + 1 < 2001) {
            mint_price = MIN_MINT_PRICE + 0.02 ether;
        } else if (_tokenIdCounter.current() + 1 < 5001) {
            mint_price = MIN_MINT_PRICE + 0.04 ether;
        } else {
            mint_price = MIN_MINT_PRICE + 0.06 ether;
        }
