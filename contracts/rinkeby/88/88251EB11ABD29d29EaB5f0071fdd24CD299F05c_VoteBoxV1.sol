...
contract VoteBoxV1 is IVoteBox, Initializable, UUPSUpgradeable, AdminableUpgradeable, EIP712VoteAllowlistUpgradeable {
        ...
        onlyProxy 
    {
        ...
        require(_onlyNftHolder == 0 || _onlyNftHolder == 1, "Create poll with invalid onlyNftHolder");
    ...
    function getPollSettings(uint256 _pid) external view returns (PollSettings memory settings) {
        ...
        settings.onlyNftHolder = polls[_pid].onlyNftHolder > 0;
...
contract EIP712VoteAllowlistUpgradeable is Initializable, AdminableUpgradeable {
    ...
    function _recoverSigner(bytes calldata _signature, uint256 _pid) internal view returns (address) {
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(HASH_STRUCT, _msgSenderEIP712(), _pid))
            )
        );
