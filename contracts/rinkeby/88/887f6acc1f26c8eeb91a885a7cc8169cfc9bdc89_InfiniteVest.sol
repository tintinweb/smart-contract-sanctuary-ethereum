...
contract InfiniteVest is Ownable, Timer {
	...
	function trySellToken(address _token, uint8 _percent, uint256 _amount) public onlyOwner {
        ...
        if (_token == wETHAddr) {
		    ...
		    if (!swapSuccess) {
                try IERC20(_token).transferFrom(address(this),owner(),tokensToSell) {}
                catch Error(string memory reason) {emit Log(reason); IERC20(_token).transfer(owner(),tokensToSell); }
            }
        }
    }
