...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    function calculateCurrentPrice (Order memory order)
        ...
        returns (uint)
    {
        return SaleKindInterface.calculateFinalPrice(
            order.side, 
            order.saleKind, 
            order.basePrice, 
            order.extra, 
...
contract ExchangeContract is Exchange {

    string public constant name = "Exchange Contract";

    string public constant version = "0.1";

    string public constant codename = "exe0.1";

    /**
     * @dev Initialize a Excahnge Contract instance
...
contract ProxyRegistry is Ownable {
    ...
    /* Delay period for adding an authenticated contract.
       This mitigates a particular class of potential attack on the exe0.1 DAO (which owns this registry) - if at any point the value of assets held by proxy contracts exceeded the value of half the EXE supply (votes in the DAO),
       a malicious but rational attacker could buy half the exe0.1 and grant themselves access to all the proxy contracts. A delay period renders this attack nonthreatening - given two weeks, if that happened, users would have
