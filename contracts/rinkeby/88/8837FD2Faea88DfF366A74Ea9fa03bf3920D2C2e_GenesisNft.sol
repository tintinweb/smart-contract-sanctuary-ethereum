...
contract GenesisNft is ERC721, Ownable {
    ...
    // mintPrice is the price of one Genesis NFT
    uint256 public constant mintPrice = 0.01 ether;
    // max supply of Genesis NFTs
     ...
     * Constructor for Genesis NFT takes in the tokenURI to set TOKEN_URI for the collection.
    ...
    function mintNft() public payable onlyWhenNotPaused {
        require(s_tokenCounter < maxSupply, "Exceed maximum Genesis NFT supply");
        require(msg.value >= mintPrice, "Ether sent is not correct");
        s_tokenCounter += 1;
        _safeMint(msg.sender, s_tokenCounter);
        emit NftMinted(s_tokenCounter, mintPrice, msg.sender);
    }
    ...
    /**
     * @dev tokenURI overides the Openzeppelin's ERC721 implementation which by default
     * returned an empty string for the tokenURI
     */
