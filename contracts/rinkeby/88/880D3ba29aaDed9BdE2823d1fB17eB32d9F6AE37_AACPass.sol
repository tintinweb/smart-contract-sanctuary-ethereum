...
contract AACPass is ERC721A, Ownable, Pausable {
    ...
    uint256 public genesisSupply = 200; // tokenType = 1
    uint256 public genesisMinted = 0;
    mapping(address => bool) genesisMintedAccounts;
    uint256 public alphaSupply = 400; // tokenType = 2
    uint256 public alphaMinted = 0;
    mapping(address => bool) alphaMintedAccounts;

    address public systemAddress = address(0);

    uint256 public mintBatchSize = 5;
    uint256 public mintPrice = 0.2 ether;

    uint256 public reservedTotal = 100;
    uint256 public reservedUsed = 0;
    mapping(address => bool) private reservedAccounts;

    bool public isAllowedListActive = false;
    mapping(address => uint256) private allowedList;
    ...
    event DutchAuctionPaused();

    constructor() ERC721A("AAC Pass", "AAC Pass", mintBatchSize, maxSupply) {
        pause();
    }
    ...
    function publicMint(bytes memory salt, bytes memory signature)
        public
        payable
        whenNotPaused
    {
        validSignature(salt, signature);
        require(msg.sender == tx.origin, "We like real users");
        require(!alphaMintedAccounts[msg.sender], "Address has minted");
        require(
            alphaMinted + reservedTotal - reservedUsed + 1 <= alphaSupply,
            "Exceeds max supply"
        );
    ...
    function reservedMint(uint256 n) public payable {
        require(reservedAccounts[msg.sender], "Not reserved account");
        require(
            reservedUsed + n <= reservedTotal,
            "Reached max reserved total amount"
        );
        require(
            alphaMinted + reservedUsed + n <= alphaSupply,
            "Reached max supply"
        );
        require(
            n % mintBatchSize == 0,
        ...
        uint256 size = n / mintBatchSize;
        for (uint256 i = 0; i < size; i++) {
            mint(msg.sender, maxBatchSize, 2);
        }
        reservedUsed += n;
    }

    function allowedListMint() public payable {
        require(msg.sender == tx.origin, "We like real users");
        require(!alphaMintedAccounts[msg.sender], "Address has minted");
        require(isAllowedListActive, "Allowed list mint has not begun yet");
        require(
            allowedList[msg.sender] > 0,
            "Not eligible for allowed list mint"
        );
        require(
            alphaMinted + 1 + reservedTotal - reservedUsed <= alphaSupply,
            "Reached max supply"
        );
        require(msg.value >= mintPrice, "Not enough ETH sent");
    ...
    function dutchAuctionMint(bytes memory salt, bytes memory signature)
        public
        payable
    {
        validSignature(salt, signature);
        ...
        require(isDutchAuctionActive, "Auction must be active to mint");
        require(block.timestamp >= daStartTime, "Auction not start");
        require(
            genesisMinted + 1 <= genesisSupply,
            "Auction would exceed max supply"
        );
        require(msg.value >= getDutchAuctionPrice(), "Not enough ether sent");
    ...
    function setIsAllowedListActive(bool bl) external onlyOwner {
        isAllowedListActive = bl;
    }

    function addAllowedList(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            allowedList[addresses[i]] = 1;
        }
    }

    function inAllowedList(address addr) public view returns (bool) {
        return allowedList[addr] > 0;
    }

    function isMinitedGenesis(address addr) public view returns (bool) {
        return genesisMintedAccounts[addr];
    }

    function isMinitedAlpha(address addr) public view returns (bool) {
        return alphaMintedAccounts[addr];
    }

    function addReservedAccounts(address[] calldata addr) external onlyOwner {
    ...
    function startDutchAuction() external onlyOwner {
        require(!isDutchAuctionActive, "Auction is already began");
        isDutchAuctionActive = true;
        emit DutchAuctionStarted();
    }

    function pauseDutchAuction() external onlyOwner {
        require(isDutchAuctionActive, "Auction is already paused");
        isDutchAuctionActive = false;
        emit DutchAuctionPaused();
    }
    ...
    function setDutchAuction(
        ...
        daTimeStep = timeStep;
        daStartPrice = startPrice;
        daEndPrice = endPrice;
        daPriceStep = priceStep;
        daStepNumber = stepNumber;
    }
    ...
    function getDutchAuctionPrice() public view returns (uint256) {
        ...
        if ((daStartTime + daTimeStep * daStepNumber) < block.timestamp) {
            return daEndPrice;
        }
    ...
    function setSystemAddress(address addr) external onlyOwner {
        systemAddress = addr;
    }
    ...
    function validSignature(bytes memory salt, bytes memory signature)
        internal
        view
    {
        require(systemAddress != address(0), "Invalid signature");
        require(
