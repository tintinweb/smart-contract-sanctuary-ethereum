...
contract qMarket is PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  struct MarketItem {
    ...
    uint256 asking_price; // asking price in wei
    uint256 expires_at; // time in hours when sale end
  }
  ...
  event ItemAdded(bytes32 item_id, uint256 token_id, address token_address, address owner_of, uint256 asking_price, uint256 expires_at);
  ...
  modifier hasTransferApproval(uint256 token_id, address token_address){
    require(IERC721Upgradeable(token_address).getApproved(token_id) == address(this) || IERC721Upgradeable(token_address).isApprovedForAll(_msgSender(), address(this)), "Item is not approved for transfer");
    _;
  }

  // verify the item is listed on the market
  ...
  // verify the item is not listed on the market or expired
  modifier ValidItemNotExists(uint256 token_id, address token_address){
    MarketItem memory item = marketItemByTokenId[token_address][token_id];
    require(item.item_id == 0 || (item.expires_at != 0 && block.timestamp > item.expires_at), "Item is already listed and not expired");
    _;
  }
  ...
  /**
    ...
    * @param asking_price - asking price in wei
    * @param expires_at - time in hours when sale end
    */
  function addItemToMarket(
    ...
    ValidItemNotExists(token_id,token_address)
  ...
  /**
    * @dev Adds a new item to the market
    * if item exists as expired item is updated
    * @param token_address - nft erc721 address
    * @param token_id - nft token id
    * @param asking_price - asking price in wei
    * @param expires_at - time in hours when sale end
  ...
  function _addItemToMarket(
    ...
    internal
  {
    require(asking_price > 0, "Price should be greater than 0");
    if (expires_at != 0) require(expires_at > block.timestamp + 1 minutes, "Listing should be more than 1 minute in the future");
    ...
    internal returns (MarketItem memory)
  {
    ...
    require(msg.value >= item.asking_price, "Not enough funds sent");
    require(seller == owner_of, "You are not the owner of this item");

    require(seller != address(0), "Invalid address");
    require(seller != buyer, "You cannot buy from yourself");
    require(item.asking_price == price, "The price is not correct");
    if (item.expires_at != 0) require(block.timestamp < item.expires_at, "The listing expired");
