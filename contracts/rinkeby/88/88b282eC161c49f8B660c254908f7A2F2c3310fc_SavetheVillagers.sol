...
function whitelistsaleMint(address account,uint256 _mintAmount, bytes32[] calldata _proof) public payable mintCompliance(_mintAmount) 
    mintPriceCompliance(_mintAmount) 
    isValidMerkleProof(_proof) 
    onlyAccounts {
    // Verify presale requirements
    require(whitelistM, 'The presale sale is not enabled!');
