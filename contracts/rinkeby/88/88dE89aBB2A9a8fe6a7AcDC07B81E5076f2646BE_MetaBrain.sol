...
contract MetaBrain is IBrain, IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    event SendToPlayer(uint randNum, address ownerPool, address newOwner);
    ...
    struct ERC20Deposit {
        // 96 since that's the amount of bits left over from `token` taking up 160. For tight packing
        uint96 anftTokenId;
        IERC20 token;
        uint amount;
    }
    ...
    uint private constant MAX_BPS = 1e18;
    ...
    mapping(uint => uint) public bossDifficulty;
    ...
    function newBoss(
        ...
        bossDifficulty[anftTokenId] = difficulty;
        _newReqSendToPlayer(referer, callData);
    }

    function changeDifficulty(uint anftTokenId, uint newdifficulty) external onlyOwner {
        bossDifficulty[anftTokenId] = newdifficulty;
    }

    function _fightBoss(uint anftTokenId) private returns (bool) {
        uint difficulty = bossDifficulty[anftTokenId];
        uint randNum = brainOracle.getRandFightNum();
        uint normalisedRandNum = randNum / (MAX_UINT / MAX_BPS);

        return normalisedRandNum >= difficulty;
    }
    ...
    function depositAssets(ERC20Deposit[] memory erc20Deps, NFTDeposit[] memory nftDeps) public {
        ...
        for (uint i; i < erc20Deps.length; i++) {
            ...
            erc20Balances[erc20Deps[i].anftTokenId][erc20Deps[i].token] += erc20Deps[i].amount;
            erc20Deps[i].token.safeTransferFrom(msg.sender, address(this), erc20Deps[i].amount);
        }
        // Deposit NFTs 2nd
        for (uint i; i < nftDeps.length; i++) {
            nftBalances[nftDeps[i].anftTokenId][nftDeps[i].nft][nftDeps[i].id] = true;
            nftDeps[i].nft.safeTransferFrom(msg.sender, address(this), nftDeps[i].id);
        }
    ...
    function summonWithItem(
        ...
        SummonSet memory summonSet = _anftIdToSummonSets[anftTokenId][summonSetIdx];

        // If the length is 0, then accept any tokenId of this NFT
        if (summonSet.tokenIds.length != 0) {
            require(summonSet.tokenIds[tokenIdIdx] == tokenId, "MB: incorrect tokenId");
        }
        ...
        if (summonSet.spend) {
            ...
            require(
                lastSummonNoSpendTime[anftTokenId][summonSet.nft][tokenId] < _lastExecTimes[anftTokenId],
                "MB: spending item in same epoch"
            );
            lastSummonNoSpendTime[anftTokenId][summonSet.nft][tokenId] = block.timestamp;

            require(summonSet.nft.ownerOf(tokenId) == msg.sender, "MB: not the owner of tokenId");
        }
    ...
    function sendToRandomPlayer(
        ...
        _sendToRandomPlayer(anftTokenId, ownerPools);
    ...
    function _sendToRandomPlayer(
        ...
        uint randNum = oracle.getRandNum(block.number-1);
        ...
        emit SendToPlayer(randNum, ownerPool, newOwner);
