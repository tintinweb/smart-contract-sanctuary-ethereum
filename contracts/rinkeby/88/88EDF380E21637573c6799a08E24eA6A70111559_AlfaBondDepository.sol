...
contract AlfaBondDepository is IBondDepository, NoteKeeper {
    /* ======== DEPENDENCIES ======== */
    ...
    event BondStake(address indexed _user, uint256 sAmount);
    ...
    constructor(
        IAlfaAuthority _authority,
        IERC20 _afa,
        IgAFA _gafa,
        IStaking _staking,
        ITreasury _treasury
    ) NoteKeeper(_authority, _afa, _gafa, _staking, _treasury) {
        // save gas for users by bulk approving stake() transactions
        _afa.approve(address(_staking), 1e45);
    }
     ...
     * @return payout_     the amount of gAFA due
        ...
        )
    {
         ...
         * price = quote tokens : afa (i.e. 42069 DAI : AFA)
         *
         * 1e18 = AFA decimals (9) + price decimals (9)
         */
         ...
         *
         * this is either the number of AFA that the market can sell
        ...
        // purchased, and how much AFA has been sold
        ...
        treasury.manage(address(market.quoteToken), _amount, false);
    ...
    function _tune(uint256 _id, uint48 _time) internal {
        ...
        if (_time >= meta.lastTune + meta.tuneInterval) {
            ...
            // afa decimals (9) + price decimals (9)
             ...
             * is 10,000 AFA. max payout would be 1,000 AFA (10,000 * 1 / 10).
             */
     ...
     * @param _market      [capacity (in AFA or quote), initial price / AFA (9 decimals), debt buffer (3 decimals)]
    ...
    function create(
        ...
        _quoteToken.approve(address(treasury), 1e45);
         ...
         *
         * 1e18 = afa decimals (9) + initial price decimals (9)
         */
        uint64 targetDebt = uint64(_booleans[0] ? ((_market[0] * 1e18) / _market[1]) / 10**decimals : _market[0]);

        /*
         * max payout is the amount of capacity that should be utilized in a deposit
         * interval. for example, if capacity is 1,000 AFA, there are 10 days to conclusion,
         * and the preferred deposit interval is 1 day, max payout would be 100 AFA.
         */
        ...
        emit CreateMarket(id_, address(afa), address(_quoteToken), _market[1]);
    }
     ...
     * @param _id          ID of market
     * @return             price for market in AFA decimals
     *
     ...
     * @param _id          ID of market
     * @return             amount of AFA to be paid in AFA decimals
     *
     * @dev 1e18 = afa decimals (9) + market price decimals (9)
     */
     ...
     * @param _id          ID of market
     * @return             current debt for market in AFA decimals
     */
     ...
     * @param _id          ID of market
     * @return             control variable for market in AFA decimals
     */
...
abstract contract FrontEndRewarder is AlfaAccessControlled {
    /* ========= STATE VARIABLES ========== */
    ...
    IERC20 internal immutable afa; // reward token

    constructor(IAlfaAuthority _authority, IERC20 _afa) AlfaAccessControlled(_authority) {
        afa = _afa;
    }
    ...
    function getReward() external {
        ...
        afa.transfer(msg.sender, reward);
    }
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
    ...
    event Stake(uint256 indexed _user, uint256 sAmount, uint256 gAmount);
    ...
    constructor(
        ...
        IStaking _staking,
        ITreasury _treasury
    ) FrontEndRewarder(_authority, _afa) {
        gAFA = _gafa;
        staking = _staking;
        treasury = _treasury;
    }
     ...
     * @param _payout      the amount of AFA due to the user
    ...
    function addNote(
        ...
        // note that only the payout gets staked (front end rewards are in AFA)
        staking.stake(address(this), _payout, false, true);

        // the new note is pushed to the user's array
        notes[_user].push(Note({payout: gAFA.balanceTo(_payout), created: uint48(block.timestamp), matured: _expiry, redeemed: 0, marketID: _marketID}));
    }
     ...
     * @return payout_     sum of payout sent, in gAFA
     */
    function redeem(
        address _user,
        uint256[] memory _indexes,
        ...
        //before Minted gAFA  is lower ,bug?
        if (_sendgAFA) {
            gAFA.transfer(_user, payout_); // send payout as gAFA
        } else {
            staking.unwrap(_user, payout_); // unwrap and send payout as sAFA
        }
    }
