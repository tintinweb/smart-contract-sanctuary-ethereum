...
contract Marketplace is Ownable {
  ...
  function buyItem(uint256 _tokenId) external payable {
    ...
    require(msg.value >= listedItems[_tokenId].value, "Invalid value");
    listedItems[_tokenId].sold = true;
    nftContract.transferFrom(address(this), msg.sender, _tokenId);
    unspentBids[listedItems[_tokenId].owner] += msg.value;
  ...
  function listItemOnAuction(uint256 _tokenId) external {
    nftContract.transferFrom(msg.sender, address(this), _tokenId);
    auctionItems[_tokenId] = ListedAuctionItem({
      owner: msg.sender,
  ...
  function makeBid(uint256 _tokenId) external payable {
    require(auctionItems[_tokenId].startTime > 0, "Auction not started");
    require(auctionItems[_tokenId].endTime > block.timestamp, "Auction already ended");
    require(msg.value > auctionItems[_tokenId].highestBid, "Bid must be higher");

    uint256 lastBid = auctionItems[_tokenId].highestBid;
    address lastBidder = auctionItems[_tokenId].highestBidder;
    unspentBids[lastBidder] += lastBid;

    if (lastBid > 0 && lastBidder != msg.sender) {
      auctionItems[_tokenId].legit = true;
    }

    auctionItems[_tokenId].highestBid = msg.value;
    auctionItems[_tokenId].highestBidder = msg.sender;
    emit NewBid(_tokenId, msg.sender, msg.value);
  }

  function finishAuction(uint256 _tokenId) external {
    require(auctionItems[_tokenId].endTime < block.timestamp, "Auction not ended");
    require(!auctionItems[_tokenId].finished, "Auction already finished");
    require(auctionItems[_tokenId].legit, "Auction has not enough bidders");
    auctionItems[_tokenId].finished = true;
    nftContract.transferFrom(address(this), auctionItems[_tokenId].highestBidder, _tokenId);
    unspentBids[auctionItems[_tokenId].owner] += auctionItems[_tokenId].highestBid;
    emit AuctionEnd(_tokenId, auctionItems[_tokenId].highestBidder, auctionItems[_tokenId].highestBid);
  }

  function cancelAuction(uint256 _tokenId) external {
    require(auctionItems[_tokenId].owner == msg.sender, "You can't cancel this auction");
    require(auctionItems[_tokenId].endTime > block.timestamp, "Auction already ended");
    auctionItems[_tokenId].startTime = 0; // users can't make bid
    auctionItems[_tokenId].owner = address(0); // owner can't cancel twice
    unspentBids[auctionItems[_tokenId].highestBidder] += auctionItems[_tokenId].highestBid;
    nftContract.transferFrom(address(this), msg.sender, _tokenId);
  }
