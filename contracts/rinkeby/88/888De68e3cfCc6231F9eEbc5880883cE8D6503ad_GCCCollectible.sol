...
// File: contracts/GCCCollectible.sol
...
contract GCCCollectible is ERC1155, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => uint256)) private walletCount;
    ...
    function claim(uint256 _cId, uint256[] memory _tokenIds, uint256 _numOfTokens) public payable nonReentrant collectionIsOpen(_cId) {
        ...
        if (collections[_cId].maxPerWallet > 0) {
            require(walletCount[_cId][msg.sender] <= collections[_cId].maxPerWallet, "Exceeds Max Per Wallet");
        }
