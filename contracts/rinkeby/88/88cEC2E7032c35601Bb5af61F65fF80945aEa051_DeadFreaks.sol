...
contract DeadFreaks is ERC721, ERC2981, Ownable, ReentrancyGuard, NonblockingReceiver {
  ...
  // @dev Provenance for provably fair reveal
  string public constant PROVENANCE_HASH = "TBD";
  ...
  constructor(
    ...
    _setDefaultRoyalty(paymentReceiver, 1000);
  ...
  nonReentrant
  {
    ...
    require(amount < 3, "Max 2 per tx");
    require(nextTokenId + amount <= CHAIN_OFFSET + CHAIN_SUPPLY, "Over supply on this chain");

    uint256 amountMinted = walletMinted[msg.sender];
    require(amountMinted + amount <= 10, "Max 10 per wallet");
    ...
    walletMinted[msg.sender] = amountMinted + amount;
  }
  ...
  function withdraw() external onlyOwner {
    require(payable(PAYMENT_RECEIVER).send(address(this).balance));
  }

  // @notice Set token royalties
  // @param value points (using 2 decimals - 10_000 = 100%, 100 = 1%)
  function setRoyalties(uint24 value) external onlyOwner {
    _setDefaultRoyalty(PAYMENT_RECEIVER, value);
  }

  function setOffsetValue(uint256 offsetValue) external onlyOwner {
