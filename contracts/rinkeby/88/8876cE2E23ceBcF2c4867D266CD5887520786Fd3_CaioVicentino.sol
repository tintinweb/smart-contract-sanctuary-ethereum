...
function getRandomAvailableTokenId(address payable to, uint256 updatedNumAvailableTokens)
  ...
  //Manager Only
  function reveal(bool _state, string memory _newBaseURI) public managerOnly {
      revealed = _state;
      baseURI = _newBaseURI;
  }

  function setFreeMintPerAddressLimit(uint256 _limit) public managerOnly {
    FreeMintPerAddressLimit = _limit;
  }

  function setMaxMintAmount(uint256 _maxMintAmount) public managerOnly {
  ...
  function setBaseExtension(string memory _newBaseExtension) public managerOnly {
  ...
  function pause(bool _state) public managerOnly {
    paused = _state;
  }

  function setOnlyWhitelisted(bool _state) public managerOnly {
    onlyWhitelisted = _state;
  }

  function setAllowAffiliateProgram(bool _state) public managerOnly {
     _allowAffiliateProgram = _state;
  }

  function setAffiliateProgramPercentage(uint256 percentage) public managerOnly {
    _affiliateProgramPercentage = percentage;
  }

  function setRecommendationPercentage(uint256 percentage) public managerOnly {
    _recommendationPercentage = percentage;
  }

  function whitelistUsers(address[] calldata _users) public managerOnly {
  ...
  function setMaxSupply(uint256 _maxSupply) public managerOnly {
  ...
  function setWhitelistedAddress(address _wallet) public managerOnly {
    whitelistedAddresses.push(_wallet);
  }

  function setNftEtherValue(uint256 nftEtherValue) public managerOnly {
    _nftEtherValue = nftEtherValue;
  }

  function setDeadLine(uint256 _newDeadLine) public managerOnly {
     _deadLine = _newDeadLine;
  }

  function setAffiliate(address manager, bool state) public managerOnly {
    _affiliates[manager] = state;
  }

  function setIsFreeMint(bool state, uint qtd) public managerOnly {
      maxMintAmount = qtd;
 ...
 function getWhitelistedAddressIndex(address _user) public view returns (uint256) {
    for (uint256 i = 0; i < whitelistedAddresses.length; i++) {
      if (whitelistedAddresses[i] == _user) {
          return i;
      }
  ...
  function removeWhitelistedAddress(uint i) public managerOnly {
  ...
  function mint(
    ...
    if(_indicationType == 2){
        require(_allowAffiliateProgram, "at the moment this agreement does not allow affiliate program");
    }
    ...
    if(!_isFreeMint ){
      if(!isWhitelisted(msg.sender)){
        ...
        uint256[] memory tokensIds = walletOfOwner(msg.sender);
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      uint256 tokenId = getRandomAvailableTokenId(payable(msg.sender), updatedNumAvailableTokens);
 ...
 function split(uint256 _mintAmount, address payable _recommendedBy, uint256 _indicationType ) public payable{
    require(msg.value >= (_nftEtherValue * _mintAmount), "the transaction value does not match the value defined in the contract");
    ...
    if(_indicationType > 1){
      ...
      ownerAmount = msg.value / (100 - _splitPercentage);
