...
// This contract was designed and deployed by : HEJCH, HORCH and RAVIN on behalf of companyName.
...
contract synpulseToken is ERC777, Pausable {
    address[] public owner;
    address public masterContract;
    address companyNameVault = 0x70196d2C1CB073716ddA3987B0aC4D036EA024B0; // companyName Vault (Testnet, to be changed to companyName Capital Vault)
    address administrator = 0x70196d2C1CB073716ddA3987B0aC4D036EA024B0; // Administrator address (delegate of CFO, Testnet)
    ...
    constructor(uint256 initialSupply, address[] memory defaultOperators)
        ERC777("Company Token 2021_t8", "ticker8", defaultOperators)
    {
        _mint(companyNameVault, initialSupply, "", "");
        masterContract = companyNameVault; // companyName Vault (Testnet, to be changed to companyName Capital Vault)
        owner.push(administrator); // Wallet allowed to pause and unpause the contract (Testnet, to be changed to CFO)
    ...
    No need for a decimals functions. Granularity takes care of this. 
    */
    ...
    // this function checks if something is in an array and returns "true" if yes, else "not"dsf
    function isInArray(
        ...
        returns (bool)
    {
        if (!(msg.sender == companyNameVault)) {
            require(
                (to == companyNameVault),
                "Tranfering tokens is not allowed if the receipient is not the vault, sorry"
            );
        }
        transfer(to, amount); //have to verify if this is the correct transfer for erc777
        return true;
    }
    ...
    This function is replaced by the send function. 
*/
    ...
    /*
Proposed TOM: Users who authorize the vault as an operator will be whitelisted.
Though the benifits of being whitelisted are not clear. 
*/
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < arr.length; i++) {
            require(
                !isInArray(arr[i], whitelistedAddressArray),
                "One of the users you are trying to add already exists"
            );
