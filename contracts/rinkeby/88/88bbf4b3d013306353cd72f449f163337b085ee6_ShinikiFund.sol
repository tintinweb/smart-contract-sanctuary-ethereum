    ...
    ShinikiTransfer
{
    ...
    function claim(
        ...
        uint256 totalClaimed = claimed[token][receiver];
         require(
            totalClaimed + amount == maxAllowce,
            "ShinikiFund:  can not claimed greater than allowce"
        );
        claimed[token][receiver] += amount;
        if (token == address(0)) {
        ...
        emit Claimed(token, receiver, maxAllowce, amount, nonce, totalClaimed);
    }
