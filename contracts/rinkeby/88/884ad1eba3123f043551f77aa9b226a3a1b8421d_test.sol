...
contract test is Context, IERC20, Ownable {
    ...
    uint256 public _liquidityFee = 9; //(3% liquidityAddition + 3% marketing  + 3% dev)
    ...
    constructor () public {
        ...
        wallet = payable(0x89C6D11ADa89f606861B6065EDD157099dccA56d);
        wallet2  = payable(0x89C6D11ADa89f606861B6065EDD157099dccA56d);
    ...
    function _transfer(
        ...
        if(limit == true && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(to != uniswapPair && from == uniswapPair){
                require(((balanceOf(to).add(amount)) <= 400000000000 * 10**4 * 10**9));
            }
            require(amount <= 400000000000 * 10**4 * 10**9, 'Transfer amount must be less');
            }
