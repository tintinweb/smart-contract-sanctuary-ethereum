...
contract Escrow {
    ...
    struct UserDetail {
        string firstName;
        string lastName;
        uint256 premium; //amount in ether
    ...
    function payPremium() onlyClient payable public {
        ...
        require(msg.value >= (userDetails[msg.sender].premium * (1 ether)), "Invalid amount");
    }
    ...
    function approveRequestQuotation(address payable _clientAddress, uint256 _requestId, uint256 _payoutPrice) onlyCapitalQuoter public {
        ...
        if (claimRequestDetails.damageFlag && claimRequestDetails.quotationFlag) {
            _clientAddress.transfer(_payoutPrice * (1 ether));
        }
    }
...
contract PIToken is ERC20, ERC20Burnable, ERC20Snapshot, AccessControl, Pausable, ERC20Permit, ERC20Votes {
    ...
    constructor(address _escrowAddress) ERC20("PIToken", "MTK") ERC20Permit("PIToken") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(SNAPSHOT_ROLE, msg.sender);
        _grantRole(PAUSER_ROLE, msg.sender);
        _mint(msg.sender, 10000 * 10 ** decimals());
        _grantRole(MINTER_ROLE, msg.sender);
        escrowAddress = Escrow(_escrowAddress);
    }
    
    // escrow functions
    function updateUserPremium(address _userAddress, uint256 _newPremium) external {
        escrowAddress.updateUserPremium(_userAddress, _newPremium);
    }

    function updateUserRiskLevel(address _userAddress, uint256 _newRiskLevel) external {
        escrowAddress.updateUserRiskLevel(_userAddress, _newRiskLevel);
    }
    ...
    function insertUser(
        ...
        escrowAddress.insertUser(_firstName, _lastName, _userAddress, _premium, _riskLevel);
    }

    // preset erc20 functions
