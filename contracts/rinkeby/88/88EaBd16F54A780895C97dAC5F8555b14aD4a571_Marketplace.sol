...
import {ERC20Spendable} from "ITRN.sol";
...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    constructor(address _token, address _nftContarct) {
        token = _token;
        marketOwner = payable(msg.sender);
        NFTContract = _nftContarct;
        TotalPayments = 0;
    }
    ...
    function buyNft(uint256 _tokenId) public nonReentrant {
        ...
        require(
            ERC20Spendable(token).balanceOf(buyer) >= nft.price,
            "Tokens transferred must be less or equal to account balance"
        );
        ERC20Spendable(token).spend(buyer, nft.price);
        TotalPayments += nft.price;
        IERC721(NFTContract).transferFrom(address(this), buyer, nft.tokenId);
        nft.owner = buyer;
        nft.listed = false;
        _nftsSold.increment();
        emit NFTSold(NFTContract, nft.tokenId, nft.seller, buyer, nft.price);
    }
    ...
    function _TotalPayments() public view returns (uint256) {
...
contract ITRN is ERC20, ERC20Spendable {
    constructor(uint256 initialSupply) ERC20("iTrainer6", "ITRN6") {
