    ...
    ERC2981PerTokenRoyalties
{
    ...
    event BatchFinalised721(string uri, uint256 token);
        ...
        returns (uint256)
    {
        ...
        emit BatchFinalised721(_currTokenURI, _tokenIds.current());
    ...
    function batchMint(
        ...
        for (uint8 i = 0; i < _currTokenURI.length; i++) {
            ...
            _tokenIds.increment();
            string memory uri = _currTokenURI[i];
            _mint(_msgSender(), _tokenIds.current());
            ...
            emit BatchFinalised721(uri, _tokenIds.current());
        }
        return _tokenIds.current();
    }
}
