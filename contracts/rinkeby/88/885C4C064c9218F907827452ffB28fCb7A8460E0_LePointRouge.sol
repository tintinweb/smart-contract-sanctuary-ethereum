...
contract LePointRouge is Ownable, ERC721Enumerable, ReentrancyGuard {
    /*Errors*/
    error Minting_Not_Started();
    error Mint_Lim_Exceeded();
    ...
    function saleMint(address _account, bytes calldata signature) external payable nonReentrant {
        if (sellingStep != Steps.Sale) {
            revert Minting_Not_Started();
        }

        if (s_nftsPerWallet[msg.sender] + 1 >= 1) {
            revert Mint_Lim_Exceeded();
        }
        if (
            _signerAddress !=
            keccak256(
            ...
            revert Signer_Address_Mismatch();
        }

        if (totalSupply() + 1 >= MAX_SALE) {
            revert Max_Mint_Exceeded();
        }
