...
// часть контракта с мнгновенными сделками
    ...
    function delayFromSender(uint256 _roomNumber) external {
        ...
        // вот эта проверка времени не срабатывает корректно
...
// для этой части контракта можно подготовить отдельный контракт, чтобы тот, который без задержки и тот, который
// с задержкой не пересикались функциями
