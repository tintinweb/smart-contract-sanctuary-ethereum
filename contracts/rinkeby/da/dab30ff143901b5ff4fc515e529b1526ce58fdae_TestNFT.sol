...
contract TestNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _unrevealedURI) ERC721A("TestNFT", "TEST") {
    ...
    function privateMint(uint256 quantity, bytes32[] calldata merkleProof)
        ...
        nonReentrant
    {
        require(saleStatus, "TEST: Private mint not live");
        require(
            block.timestamp < publicMintSaleTime,
            "TEST: Private mint over"
        );
    ...
    function toggleSale() external onlyOwner {
        require(!saleStatus, "Test: Sale already started");
        saleStatus = !saleStatus;
        publicMintSaleTime = block.timestamp + 1 days;
    }
    ...
    modifier mintCompliance(uint256 quantity) {
        require(
            ...
            "TEST: Quantity not within range"
        );
        require(
            numberMinted(msg.sender) + quantity <= MAX_MINTS,
            "TEST: Reached max mint"
        );
        require(
            totalSupply() + quantity <= MAX_SUPPLY,
            "TEST: Reached max supply"
        );
        require(msg.value >= MINT_PRICE * quantity, "TEST: Insufficient funds");
        _;
    }
}
