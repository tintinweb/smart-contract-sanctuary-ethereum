...
contract Giza is Ownable, ERC721iEnumerable {
    ...
    function setup(address _renderer, address _pool) external {
        require(address(sudoPool) == address(0x0), "!can't twice");

        sudoPool = ISudoPair(_pool);
        renderer = IRenderer(_renderer);
    }

    function mint() internal {
        _preMintReceiver = address(sudoPool);
        _balances[_preMintReceiver] += 100;

        //todo test this fromTokenNumber
        // Emit the Consecutive Transfer Event
        emit ConsecutiveTransfer(
            totalSupply() == 0 ? 1 : totalSupply() - 99, //todo verify this
            totalSupply() + 100,
            address(0),
            _preMintReceiver
        );
    ...
    function _beforeTokenTransfer(
        ...
        // if we are trading, we send 5% tax to the devs
        if (to == address(sudoPool) || from == address(sudoPool)) {
            payable(owner()).safeTransferETH(
                ((msg.value * 10000000 * 5)) / 1000000000
            );
        }

        // If we are sending to the pool we:
        // - set correct price based on current supply, overriding sudoswap

        // - if supply greater then 100 we nede to start adjusting the price dynamically
        if (to == address(sudoPool) && totalSupply() > 100) {
            uint128 newPrice = uint128(address(sudoPool).balance) /
                uint128(totalSupply());
            sudoPool.changeSpotPrice(newPrice);
        }
    }
    ...
    function _afterTokenTransfer(
        ...
        if (to == address(sudoPool)) {
            // after the trade is done we burn the nft
            burn(tokenId);
        }
        // if balance of pool 0 we:
        // - mint 100 NFTs
        // - set new price of pool
        if (balanceOf(address(sudoPool)) == 0) {
            mint();

            uint128 newPrice = (uint128(totalSupply()) / 100) * 0.05 ether;

            sudoPool.changeSpotPrice(newPrice); //check later as we might use burning so this would be off
        }
    ...
    //     address pool;
    //     (pool, ) = sudoFactory.createPairEth(
    //         address(this), //nft address
    //         _bondingCurve, //bonding curve
    //         address(0x0), //assetReceipient
    //         _poolTy2pe, // poolType
    //         0, //delta
    //         50000000000000000, //fee
    //         50000000000000000, //initial price
    //         [] //initial nfts
    //     );

    //     // TODO only for testing, transfer ownershsip

    //     sudoFactory.transferOwnership(msg.sender);
    // }
