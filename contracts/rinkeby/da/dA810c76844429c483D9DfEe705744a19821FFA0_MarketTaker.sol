...
contract MarketTaker is IMarketTaker, BlockContext, ClearingHouseCallee, MarketTakerStorageV1 {
    ...
    function swap(SwapParams memory params) external override returns (SwapResponse memory) {
        ...
        if (params.isClose && takerPositionSize != 0) {
            ...
            if (
                ...
                params.amountLimit = getEstimateAmountLimit(
                    params.baseToken,
                    params.amount,
                    params.isBaseToQuote,
                    params.isExactInput
            ...
            if (isReducingPosition) {
                // MT_AOPLO: over price limit before swap
                require(
                    !_isOverPriceLimit(params.baseToken, params.amount, params.isBaseToQuote, params.isExactInput),
                    "MT_AOPLO"
                );
            }
        }
    ...
    function getEstimateAmountLimit(
        ...
        if (isBaseToQuote) {
            if (!isExactInput) {
                uint256 baseBalance = IAmm(amm).getBalances(1);
                // MT_IBA: insufficient base token amount
                require(amount < baseBalance, "MT_IBA");
            }
            dy = IAmm(amm).getDy(1, 0, amount, isExactInput);
        } else {
            if (!isExactInput) {
                uint256 quoteBalance = IAmm(amm).getBalances(0);
                // MT_IQA: insufficient quote token amount
                require(amount < quoteBalance, "MT_IQA");
            }
            dy = IAmm(amm).getDy(0, 1, amount, isExactInput);
        }
    ...
    function _swap(SwapParams memory params) internal returns (InternalSwapResponse memory) {
        ...
        uint24 exchangeFeeRatio = IAmmFactory(_ammFactory).getExchangeFeeRatio(params.baseToken);
        uint256 exchangeFee;

        if (params.isBaseToQuote) {
            uint256 quoteAmount = IAmm(amm).swap(1, 0, params.amount, params.amountLimit, params.isExactInput);
            exchangeFee = quoteAmount.mulRatio(exchangeFeeRatio);

            if (params.isExactInput) {
                exchangedPositionSize = params.amount.neg256();
                exchangedPositionNotional = quoteAmount.toInt256();
                quote = (quoteAmount - exchangeFee).toInt256();
            } else {
                exchangedPositionSize = params.amount.toInt256();
                exchangedPositionNotional = quoteAmount.neg256();
                quote = (quoteAmount + exchangeFee).neg256();
            }
        } else {
            uint256 quoteAmount = IAmm(amm).swap(0, 1, params.amount, params.amountLimit, params.isExactInput);
            exchangeFee = params.amount.mulRatio(exchangeFeeRatio);

            if (params.isExactInput) {
                exchangedPositionSize = quoteAmount.toInt256();
                exchangedPositionNotional = params.amount.neg256();
                quote = (params.amount + exchangeFee).neg256();
            } else {
                exchangedPositionSize = quoteAmount.neg256();
                exchangedPositionNotional = params.amount.toInt256();
                quote = (params.amount - exchangeFee).toInt256();
            }
        }
        ...
        uint256 insuranceFundFee = exchangeFee.mulRatio(insuranceFundFeeRatio);
        ...
        uint256 lpCalcFeeRatio = ((exchangeFee - insuranceFundFee) * Constant.IQ96) / totalLiquidity;
    ...
    function _isOverPriceLimit(
        ...
        uint256 priceAfter;
        if (isBaseToQuote) {
            (priceAfter, , ) = IAmm(amm).quoteSwap(1, 0, amount, isExactInput);
        } else {
            (priceAfter, , ) = IAmm(amm).quoteSwap(0, 1, amount, isExactInput);
        }

        uint256 currentPrice = IAmm(amm).getPriceCurrent();
        uint24 fluctuationLimitRatio = IClearingHouseConfig(_clearingHouseConfig).getFluctuationLimitRatio();
        uint256 upperLimit = currentPrice.mulRatio(fluctuationLimitRatio + 1e6);
        uint256 lowerLimit = currentPrice.mulRatio(1e6 - fluctuationLimitRatio);

        if ((priceAfter <= upperLimit) && (priceAfter >= lowerLimit)) {
            return false;
        }
        return true;
    }
        ...
        )
    {
        ...
        if (timestamp == lastSettledTimestamp || lastSettledTimestamp == 0) {
            ...
            int256 deltaTwPremium = _getDeltaTwap(markTwap.formatX10_18ToX96(), indexTwap.formatX10_18ToX96()) *
