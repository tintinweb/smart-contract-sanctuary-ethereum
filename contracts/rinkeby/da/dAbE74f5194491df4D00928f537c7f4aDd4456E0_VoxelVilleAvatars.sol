...
contract VoxelVilleAvatars is ERC721, Ownable {
  ...
  function _getMaxMints(CouponType couponType) private view returns (uint256) {
    ...
    uint256 balance = IERC721(_voxelVilleContract).balanceOf(msg.sender);
  ...
  function safeMint(address to, bool airdropped) public onlyOwner {
    uint256 tokenId = _tokenIdCounter.current();
    _tokenIdCounter.increment();
    _safeMint(to, tokenId);
    totalSupplyRemaining--;
    if (airdropped) {
      airdroppedTokens[to].push(tokenId);
    } else {
      numberMinted[to]++;
    }
  }

  modifier isMintable(Coupon memory coupon) {
    if (_currentPhase == SalePhase.Locked) {
      require(false, "Voxel Ville Avatars: Cannot mint during locked phase");
    }
    CouponType cType = _findCouponType(coupon, msg.sender);
    require(
      (_currentPhase == SalePhase.Collectors && cType == CouponType.Collector)
        || (_currentPhase == SalePhase.Holders && (cType == CouponType.Collector || cType == CouponType.Holder))
  ...
  modifier isNotExceedingMaxMint(Coupon memory coupon, uint256 count) {
    uint256 max = _getMaxMints(_findCouponType(coupon, msg.sender));
    require(count + numberMinted[msg.sender] <= max, "Voxel Ville Avatars: Cannot mint more than maximum allowed");
    _;
  }

  modifier isValidMintingCoupon(Coupon memory coupon) {
    bytes32 digest = keccak256(
      abi.encode(_findCouponType(coupon, msg.sender), msg.sender)
    );
  ...
  function mint(Coupon memory coupon, uint256 count)
  public
  payable
  isMintable(coupon)
  isValidMintingCoupon(coupon)
  isNotExceedingMaxMint(coupon, count)
  isNotExceedingAvailableSupply(count)
  ...
  function _findCouponType(Coupon memory coupon, address minter) internal view returns (CouponType) {
    bytes32 digest = keccak256(
      abi.encode(CouponType.Collector, minter)
    );
    if (_isVerifiedCoupon(digest, coupon)) {
      return CouponType.Collector;
    }

    digest = keccak256(
      abi.encode(CouponType.Holder, minter)
    );

    if (_isVerifiedCoupon(digest, coupon)) {
      return CouponType.Holder;
    }

    return CouponType.NonHolder;
  }
  ...
  function airdropMint(address to, Coupon memory coupon, uint8 allocated, uint8 count) public {
    ...
    require(_isVerifiedCoupon(digest, coupon), "Voxel Ville Avatars: Invalid Coupon");
    require(airdroppedTokens[to].length + count < allocated, "Voxel Ville Avatars: Cannot exceed max airdrops permitted by this Coupon");
    for (uint8 index = 0; index < count; index++) {
      safeMint(to, true);
    }
  }
