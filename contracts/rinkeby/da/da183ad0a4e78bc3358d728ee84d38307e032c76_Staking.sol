...
contract WhitelistChecker is EIP712{
    ...
    function _hash(whitelisted memory list) internal view returns (bytes32) {
        return _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256("whitelisted(uint256 tokenId,uint256 rarity)"),
                    list.tokenId,
                    list.rarity
                )
            )
...
// File: contracts/interfaces/IJiraToken.sol
...
contract Staking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, WhitelistChecker, ERC721EnumerableUpgradeable, IERC721ReceiverUpgradeable {
	...
	mapping(uint256 => uint256) public gen2Multipliers;
	...
	function _addToBag(
		...
		if (_isValidGenesisId(_genTokenId)) {
			...
			require(msg.sender == genesis.ownerOf(_genTokenId), "not owner");
		...
		if (_gen2Count > 0){		
			for (uint256 i = 0; i < _gen2Count; i++) {
				...
				require(msg.sender == gen2.ownerOf(_gen2TokenIds[i]), "not owner");
	...
	function unstake(
		...
		for (uint256 i = 0; i < totalBags; i++) {
			...
			if (_isValidGenesisId(bags[_bagId].genTokenId) == false && bags[_bagId].gen2TokenIds.length == 0) {
	...
	function setGenesisMultipliers(uint256[] memory mults) external onlyOwner {
		genMultipliers[1] = mults[0];
		genMultipliers[2] = mults[1];
	}

	function setGen2Earnings(uint256[] memory rates) external onlyOwner {
		gen2Earnings[1] = rates[0];
		gen2Earnings[2] = rates[1];
		gen2Earnings[3] = rates[2];
	}
