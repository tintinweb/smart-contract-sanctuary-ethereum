...
contract HelmsForLoot is ERC1155, IERC2981, Ownable {
    ...
    // The original Loot contract address is used to fetch the helm name
    ...
    // Legendary helm ids are stored as a tightly packed array of uint16
    bytes[5] private legendaryIds;

    // Pricing
    uint256 public lootersPriceCommon = 0.02 ether;
    uint256 public publicPriceCommon = 0.05 ether;

    uint256 public lootersPriceEpic = 0.04 ether;
    uint256 public publicPriceEpic = 0.07 ether;

    uint256 public lootersPriceLegendary = 0.06 ether;
    uint256 public publicPriceLegendary = 0.09 ether;

    uint256 public lootersPriceMythic = 0.08 ether;
    uint256 public publicPriceMythic = 0.11 ether;

    constructor(ILoot[] memory lootsList, LMart lmart) ERC1155("") {
        ...
        legendaryIds[1] = hex"00fd14810113102801ee0c02030a0809037013d9039119120f7b14d21d501d88";
        legendaryIds[2] = hex"0200109b064d0a68094114340b611e450f441b04";
        legendaryIds[4] = hex"01400eea01a81d8706fa1c29088616e60b1410870f7c12b5";
    }
    ...
    function purchasePublic(uint256[] memory lootIds) public payable {
        ...
        uint256[] memory amounts = new uint256[](lootIds.length);
        uint256 price = 0;
        ...
        for (uint256 i = 0; i < lootIds.length; i++) {
            ...
            // Reserve Loot IDs 7778 to 1000 for ownerClaim
            require(
                lootIds[i] > 0 && lootIds[i] < 7778,
                "HelmsForLoot: invalid Loot ID"
            );

            // These are taken from Loot to get a rarity approximation of the helm
            uint256 rand = uint256(
                keccak256(
                    abi.encodePacked("HEAD", Strings.toString(lootIds[i]))
                )
            );
            ...
            if (greatness <= 14) {
                ...
                // Legendary or mythic helm
                // Unfortunately we don't know which one it is, since
                // it's based on rarity, not greatness. So check if ID 
                // is in list of legendary ids:
                require(
                    state == SaleState.Phase3,
                    "HelmsForLoot: sale not active"
                );
                bool found = findHelmIndex(legendaryIds[kind], lootIds[i]);
                if (found) {
            ...
            amounts[i] = 1; // No need to aggregate amounts if an tokenID
            // appears twice as the _batchMint function will handle it ok regardless.
        }
    ...
    function purchaseMatching(
        ...
        for (uint256 i = 0; i < lootIds.length; i++) {
            ...
            // Both the original loot bag and matching bag (loot/mloot/genesis adventurer) to be unclaimed
            require(
            ...
            require(
                ...
                "HelmsForLoot: matching bag already claimed"
            );
            ...
            if (greatness <= 14) {
                ...
                price += lootersPriceEpic;
            } else {
                // Legendary or mythic helm
                // Unfortunately we don't know which one it is, since
                // it's based on rarity, not greatness. So check if ID is in list of legendary ids:
                bool found = findHelmIndex(legendaryIds[kind], lootIds[i]);
                if (found) {
                    price += lootersPriceLegendary;
                } else {
                    price += lootersPriceMythic;
                }
            }
            bagClaimed[ogLootContract][lootIds[i]] = true;
            tokenIds[i] = lmartContract.headId(lootIds[i]);
            amounts[i] = 1; // No need to aggregate amounts if an tokenID appears twice as the _batchMint function will handle it ok regardless.
        }
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        string memory hname = lmartContract.tokenName(tokenId);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '"image": ', '"', baseUri, '/', Strings.toString(tokenId), '.glb", ',
                        '}'
                    )
                )
            )
    ...
    // Allows the owner to mint a set of helms for promotional purposes and to reward contributors.
    // Loot IDs 7778->8000.
    function ownerClaim(uint256[] memory lootIds) public payable onlyOwner {
        uint256[] memory tokenIds = new uint256[](lootIds.length);
        uint256[] memory amounts = new uint256[](lootIds.length);
        for (uint256 i = 0; i < lootIds.length; i++) {
    ...
    function id(uint256 tokenId) public view returns (uint256 headId) {
        return lmartContract.headId(tokenId);
    }
    ...
    function setPricesCommon(uint256 newLootersPrice, uint256 newPublicPrice)
        public
        onlyOwner
    {
        lootersPriceCommon = newLootersPrice;
        publicPriceCommon = newPublicPrice;
    }

    function setPricesEpic(uint256 newLootersPrice, uint256 newPublicPrice)
        public
        onlyOwner
    {
        lootersPriceEpic = newLootersPrice;
        publicPriceEpic = newPublicPrice;
    }

    function setPricesLegendary(uint256 newLootersPrice, uint256 newPublicPrice)
        public
        onlyOwner
    {
        lootersPriceLegendary = newLootersPrice;
        publicPriceLegendary = newPublicPrice;
    }

    function setPricesMythic(uint256 newLootersPrice, uint256 newPublicPrice)
        public
        onlyOwner
    {
        lootersPriceMythic = newLootersPrice;
