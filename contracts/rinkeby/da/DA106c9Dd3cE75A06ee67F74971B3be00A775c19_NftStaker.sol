...
contract NftStaker is IERC721Receiver ,  ReentrancyGuard  {  
    ...
    event NftStaked(address indexed staker, uint256 tokenId, uint256 blockTimeStamp);
    event NftUnStaked(address indexed staker, uint256 tokenId, uint256 blockTimeStamp);
    event StakePayout(address indexed staker, uint256 tokenId, uint256 stakeAmount, uint256 fromBlockTimeStamp, uint256 toBlockTimeStamp);
    ...
    constructor(
     ...
     modifier onlyStaker(uint256 tokenId) {
        ...
        require(receipt[tokenId].timestamp != 0, "onlyStaker: Token is not staked");
    ...
    function _stakeNFT(uint256 tokenId) internal returns (bool) {
        ...
        require(receipt[tokenId].timestamp == 0, "Stake: Token is already staked");
    ...
    function _unStakeNFT(uint256 tokenId) internal onlyStaker(tokenId)   returns (bool) {
    ...
    function _getNftTier(uint256 tokenId) internal  returns(uint256) { 
    ...
    function _payoutStake(uint256 tokenId) internal {
        ...
        require(receipt[tokenId].timestamp > 0, "_payoutStake: Can not stake from timestamp 0");
        ...
        if (erc20Token.balanceOf(address(this)) < payout) {
            emit StakePayout(msg.sender, tokenId, 0, receipt[tokenId].timestamp, block.timestamp);
            return;
        }
        ...
        emit StakePayout(msg.sender, tokenId, payout, receipt[tokenId].timestamp, block.timestamp);
    }  
