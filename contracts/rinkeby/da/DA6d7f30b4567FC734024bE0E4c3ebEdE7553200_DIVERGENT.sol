...
contract DIVERGENT is ERC721AUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function mint(uint256 quantity) external payable nonReentrant {
        ...
        require(totalSupply() + quantity <= maxSupply - reserveSupply, "Exceeds total supply");
        require(quantity <= maxPerTransaction, "Exceeds max per transaction");
        require(mintsWallet[msg.sender] + quantity <= maxPerWallet, "Exceeds max per wallet");
        require(msg.value >= mintPrice * quantity, "Not enough ETH");

        uint256 refund = isFreeMint() ? quantity * mintPrice : 0;
    ...
    function mintWhitelist(bytes32[] memory _proof) external nonReentrant {
        require(isMintLive, "Mint not live");
        require(totalSupply() + 1 <= maxSupply - reserveSupply, "Exceeds total supply");
        require(whitelistClaimed[msg.sender] == false, "already claimed");
        require(mintsWallet[msg.sender] + 1 <= maxPerWallet, "Exceeds max per wallet");
        require(MerkleProofUpgradeable.verify(_proof, merkleTreeRoot, keccak256(abi.encodePacked(msg.sender))), "Invalid proof");

        whitelistClaimed[msg.sender] = true;

        mintsWallet[msg.sender] = mintsWallet[msg.sender] + 1;
