...
contract CreatureFactory is FactoryERC1155, Ownable, ReentrancyGuard {
    ...
    uint256 constant NUM_OPTIONS = 6;
    ...
    function _createOrMint(
        ...
        ERC1155Tradable creature = ERC1155Tradable(_erc1155Address);
        if (!creature.exists(_id)) {
            creature.create(_to, _id, _amount, "", _data);
        } else {
            creature.mint(_to, _id, _amount, _data);
        }
    }
        ...
        returns (uint256)
    {
        if (_optionId < NUM_OPTIONS) {
            ...
            ERC1155Tradable creature = ERC1155Tradable(creatureAddress);

            uint256 currentSupply = creature.balanceOf(owner(), _optionId);
