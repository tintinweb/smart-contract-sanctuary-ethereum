...
contract StablePool is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 invested_amount; //How many tokens the owner has invested at curve
    }
    ...
    address public constant USDC_ADDRESS = 0x37a1B8594e8D3e0dc5A033d06926c8eDF75D48A1;
    address public constant USDT_ADDRESS = 0xfa898400a070346825ADA6B01232c45a86BDa73c;
    ...
    function deposit(address _token, uint256 _amount) public {
        //require(_token == USDT_ADDRESS || _token == USDC_ADDRESS, "only stable coin can be deposited");
        //uint i;
        ...
        //    userInfo[_token][msg.sender].amount = userInfo[_token][msg.sender].amount.add(_amount);
        //}
        emit Deposit(_token, msg.sender, _amount);
    }

    function getDepositInfo() public view returns (uint256, uint256, uint256, uint256) {
        return (userInfo[msg.sender][USDC_ADDRESS].amount, userInfo[msg.sender][USDC_ADDRESS].invested_amount, userInfo[msg.sender][USDT_ADDRESS].amount, userInfo[msg.sender][USDT_ADDRESS].invested_amount);
    }
    
    function withdraw(address _token, uint256 _amount) public {
        require(_token == USDT_ADDRESS || _token == USDC_ADDRESS, "only stable coin can be withdrawed");
        UserInfo storage user = userInfo[_token][msg.sender];
        require(user.amount.sub(user.invested_amount) >= _amount, "withdraw: not good");
    ...
    function investAtCurve() public onlyOwner{
        ...
        IERC20(USDC_ADDRESS).safeApprove(curveFi_Deposit, usdcBalance);
        IERC20(USDT_ADDRESS).safeApprove(curveFi_Deposit, usdtBalance);

        //Step 1 - deposit stablecoins and get Curve.Fi LP tokens
        ICurveFi_DepositY(curveFi_Deposit).add_liquidity([0, usdcBalance, usdtBalance, 0], 0); //0 to mint all Curve has to 
        ...
        uint i;
        for (i = 0; i < wallets.length; i ++) {
            userInfo[wallets[i]][USDC_ADDRESS].invested_amount.add(userInfo[wallets[i]][USDC_ADDRESS].amount);
            userInfo[wallets[i]][USDC_ADDRESS].amount = 0;

            userInfo[wallets[i]][USDT_ADDRESS].invested_amount.add(userInfo[wallets[i]][USDT_ADDRESS].amount);
            userInfo[wallets[i]][USDT_ADDRESS].amount = 0;
        }
    }
