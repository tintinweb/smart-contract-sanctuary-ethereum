...
contract  Strategy {
    ...
    function settoken(address token_address) public {
          require(msg.sender == owner);
          token = ERC20(token_address);
      }

    function setvault(address vault_address) public {
        require(msg.sender == owner);
        vault = vault_address;
      }
    ...
    function withdraw(uint256 amount) external {
        uint balance_token = token.balanceOf(address(this));
        if (amount > balance_token){
            token.transfer(vault, balance_token);
        } else{
