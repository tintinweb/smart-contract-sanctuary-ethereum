...
contract PerpetualFutures is IndexHandler, KeeperCompatibleInterface, Ownable {
  ...
  struct PositionOpenCloseMetadata {
    uint256 openTime;
    uint256 openFees;
    uint256 closeTime;
    uint256 closeFees;
    uint256 indexPriceTakeProfit;
    uint256 indexPriceStopLoss;
  }

  struct Position {
    PositionIndexFeed[] feeds;
    PositionOpenCloseMetadata lifecycle;
  ...
  mapping(uint256 => uint256) internal _openPositionsInd;
  ...
  function openPosition(
    ...
    bool _isLong,
    uint256 _takeProfitIndPrice,
    uint256 _stopLossIndPrice
  ) external {
    require(enabled, 'disabled');
    require(_indexInd < indexes.length, 'invalid index');
    require(_leverage >= 10 && _leverage <= maxLeverage, 'LEV1');
    require(_collToken == address(0) || _validColl[_collToken], 'POSTOKEN1');
    ...
    _newPosition.positionAmount = _finalPositionAmount;
    ...
    _setTakeProfit(_newTokenId, _takeProfitIndPrice);
    _setStopLoss(_newTokenId, _stopLossIndPrice);
    ...
    internal
  {
    ...
    if (liquidators[msg.sender] && msg.sender != _user) {
      require(shouldPositionLiquidate(_tokenId), 'LIQ2');
    }
  ...
  function settleUnsettledPosition(
    ...
    _position.mainAmountSettled = _mainAmountToSettle;
  ...
  function setPositionTakeProfitPrice(uint256 _tokenId, uint256 _indexPrice)
    external
    onlyPositionOwner(_tokenId)
  {
    _setTakeProfit(_tokenId, _indexPrice);
  }

  function _setTakeProfit(uint256 _tokenId, uint256 _indexPrice) internal {
    Position storage _position = positions[_tokenId];

    // if passed as zero, we're effectively removing TP so don't do these checks
    if (_indexPrice != 0) {
      if (_position.isLong) {
        require(_indexPrice > _position.indexPriceStart, 'TP1');
      } else {
        require(_indexPrice < _position.indexPriceStart, 'TP2');
      }
    }
    _position.lifecycle.indexPriceTakeProfit = _indexPrice;
  }

  function setPositionStopLossPrice(uint256 _tokenId, uint256 _indexPrice)
    external
    onlyPositionOwner(_tokenId)
  {
    _setStopLoss(_tokenId, _indexPrice);
  }

  function _setStopLoss(uint256 _tokenId, uint256 _indexPrice) internal {
    Position storage _position = positions[_tokenId];

    // if passed as zero, we're effectively removing SL so don't do these checks
    if (_indexPrice != 0) {
      if (_position.isLong) {
        require(_indexPrice < _position.indexPriceStart, 'SL1');
      } else {
        require(_indexPrice > _position.indexPriceStart, 'SL2');
      }
    }
    _position.lifecycle.indexPriceStopLoss = _indexPrice;
  }
  ...
  function setLiquidator(address _wallet, bool _isLiquidator)
    external
    onlyOwner
  {
    require(liquidators[_wallet] != _isLiquidator);
    liquidators[_wallet] = _isLiquidator;
  }
    ...
    for (uint256 _i = 0; _i < _proxies.length; _i++) {
      ...
      _newIndex.priceFeeds.push(
        IndexFeed({
          proxy: _proxy,
          weight: _weights[_i],
          priceWeightMult: (_weights[_i] * FACTOR) / _priceUSD
        })
      );
    }
  }
  ...
  function _addOpenPosition(uint256 _tokenId) internal {
    _openPositionsInd[_tokenId] = allOpenPositions.length;
    allOpenPositions.push(_tokenId);
  }

  function _removeOpenPosition(uint256 _tokenId) internal {
    uint256 _allPositionsIndex = _openPositionsInd[_tokenId];
    uint256 _tokenIdMoving = allOpenPositions[allOpenPositions.length - 1];
    delete _openPositionsInd[_tokenId];
    _openPositionsInd[_tokenIdMoving] = _allPositionsIndex;
  ...
  function _validateAndUpdateOpenAmounts(uint256 _tokenId) internal {
    Position memory _position = positions[_tokenId];
    if (_position.isLong) {
      amtOpenLong[_position.collateralToken] += _position.collateralAmount;
    } else {
      amtOpenShort[_position.collateralToken] += _position.collateralAmount;
    }
    ...
    returns (bool)
  {
    ...
    return
      _priceInLiquidation ||
      _feesExceedReturn ||
      shouldPositionTakeProfit(_tokenId) ||
      shouldPositionStopLoss(_tokenId);
  }

  function shouldPositionTakeProfit(uint256 _tokenId)
    public
    view
    returns (bool)
  {
    Position memory _position = positions[_tokenId];
    if (_position.lifecycle.indexPriceTakeProfit == 0) return false;
    (uint256 _currentIndexPrice, , , , ) = getIndexAndPLInfo(_tokenId);
    if (_position.isLong) {
      // TP for long position
      return _currentIndexPrice >= _position.lifecycle.indexPriceTakeProfit;
    }
    // TP for short position
    return _currentIndexPrice <= _position.lifecycle.indexPriceTakeProfit;
  }

  function shouldPositionStopLoss(uint256 _tokenId) public view returns (bool) {
    Position memory _position = positions[_tokenId];
    if (_position.lifecycle.indexPriceStopLoss == 0) return false;
    (uint256 _currentIndexPrice, , , , ) = getIndexAndPLInfo(_tokenId);
    if (_position.isLong) {
      // SL for long position
      return _currentIndexPrice <= _position.lifecycle.indexPriceStopLoss;
    }
    // SL for short position
    return _currentIndexPrice >= _position.lifecycle.indexPriceStopLoss;
  }
}
