...
contract TESTER is ERC721A, Ownable {
    ...
    uint256 public constant freeMints = 2;
    ...
    function mintNFT(uint256 numAssets) external payable {
        ...
        require(
            numAssets >= 0 && numAssets <= maxPurchase,
            "You can only mint 20 assets at a time!"
        );
        require(
            totalSupply().add(numAssets) <= maxSupply - reservedAssets,
            "Not enough assets available..."
        );

        if (totalSupply().add(numAssets) > freeMints) {
            require(
                msg.value >= _price.mul(numAssets),
                "Not enough ETH for this purchase!"
            );
        } else {
            require(
                totalSupply().add(numAssets) <= freeMints,
                "You would exceed the number of free mints"
            );
            require(
                freeMintsWallet[msg.sender].add(numAssets) <=
    ...
    function assetOfOwner(address _owner)
        external
        view
    ...
    function mintNFTS(address _to, uint256 _amount) external onlyOwner {
        require(
            totalSupply().add(_amount) <= maxSupply - reservedAssets,
    ...
    function withdrawAll() public onlyOwner {
        ...
        require(payable(msg.sender).send(balance), "Withdraw did not work...");
    }
