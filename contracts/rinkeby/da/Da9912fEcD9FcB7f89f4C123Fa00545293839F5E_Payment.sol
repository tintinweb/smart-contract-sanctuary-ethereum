...
contract Payment is Pausable, Ownable {
    ...
    function redeem(
        ...
        require(_blockHeight >= block.number, "The block has expired!");
        require(!nonceMap[_nonce], "Nonce already exist!");
        require(
            redeemHash(
                msg.sender,
                _amount,
        ...
        require(checkSigner(_hash, _signature), "Invalid signature!");
        if (_tokenAddress == address(0x0)) {
            require(_redeemETH(_amount), "_redeemETH failed!");
        } else {
            require(
        ...
        emit RedeemSuccess(_fromAddress, msg.sender, _amount, _tokenAddress, _nonce);
    ...
    function _redeemETH(uint256 amount) internal returns (bool) {
    ...
    function _payFromToken(
        ...
        tokenContract.transferFrom(msg.sender, _toAddress, _amount);
    }
}
