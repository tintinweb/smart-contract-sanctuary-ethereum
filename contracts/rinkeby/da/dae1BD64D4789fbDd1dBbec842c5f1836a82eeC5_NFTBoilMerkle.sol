...
contract NFTBoilMerkle is ERC721Enumerable, Ownable, Pausable {
    ...
    function getBalanceEachWallet() public view returns(address[] memory walletList, uint256[] memory walletCount){
    ...
    for (uint256 id = 1; id <= totalSupply + 1; id++) {
        ...
        if(walletList.length == 0){
            walletList = new address[](1);
            walletCount = new uint256[](1);
        }else{
            for (uint256 i = 0; i <= walletList.length - 1; i++){
                if(walletList[i] == tmp){
                    walletCount[i]++;
                    break;
                }
