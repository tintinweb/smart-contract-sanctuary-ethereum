...
contract ParaNFT is ERC721A, Ownable {
    ...
    string public mintType = "wl1";
    uint16 public mintAmountPerMintType = 5;
    ...
    //         if (mintedTimestamp[from] > 0) {
    //             require(
    //                 block.timestamp - mintedTimestamp[from] >= 600, //  10 min
    //                 "You can sell NFT in 6 months since you minted"
    //             );
    //         }
    //     }
    ...
    function setMintType(string memory _mintType) external onlyOwner {
        if (
            ...
            keccak256(abi.encodePacked(("wl1")))
        ) {
            mintType = _mintType;
            setMintAmountPerMintType(5);
    ...
    function privateMint(bytes32[] calldata _merkleProof) public payable {
        ...
        if (
            ...
            require(totalSupply() <= 5, "1st private sale is finished.");
        } 
