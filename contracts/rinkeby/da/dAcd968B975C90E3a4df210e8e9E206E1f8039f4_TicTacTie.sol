...
contract TicTacTie is ERC721, IERC1155Receiver {
    ...
    // Players can play
    modifier onlyArtist() {
        require(_artist == _msgSender(), "Caller is not the artist");
        _;
    }

    modifier onlyBoardOwner(uint256 boardIndex) {
        require(ownerOf(boardIndex) == msg.sender, "You don't own this board");
        _;
    }
    ...
    {
        require(_prizeTokenId != 0, "Prize already redeemed");
        require(
            victories(boardIndex) == _victoriesToWin,
        ...
        onlyBoardOwner(boardIndex)
    {
        ...
        require(ties > 0, "No mintable ties");
        require(_leftToMint > 0, "No more ties");
    ...
    function mint(uint256 tokenId) public payable {
        ...
        require(tokenId > 0 && tokenId <= _maxBoardSupply, "id out of range");
        super._safeMint(msg.sender, tokenId);
        ...
        onlyBoardOwner(board1Index)
    {
        ...
        require(boardPlayer > 0 && boardOpponent > 0, "Boards empty");

        require(
            _isWaiting(boardPlayer) && _isWaiting(boardOpponent),
            "A player already playing"
        );

        uint256 board1LastBlock;
        uint256 board2LastBlock;
        if (victories(board1Index) >= victories(board1Index)) {
    ...
    function whoAbandoned(uint256 boardIndex) public view returns (uint256) {
        ...
        require(
            !_isWaiting(board) && !_isWaiting(boardOpponent),
            "A player not playing"
        );
    ...
    function endGame(uint256 boardIndex) public {
        ...
        uint256 opponentBoardIndex = opponent[boardIndex];
        require(opponentBoardIndex > 0, "No opponent");

        uint256 boardOpponent = boards[opponentBoardIndex];
        require(
            _didPlayer1Abandon(boardOpponent, board),
            "Opponent didn't abandon"
        );

        boards[boardIndex] =
            (board & victoriesMask & lastActionBlockMask & stateMask) |
            (block.number << lastActionBlockPosition) |
            ((_countVictories(board) + 1) << victoriesPosition) |
            (waitingFlag << statusPosition);
        boards[opponentBoardIndex] =
            (boardOpponent & stateMask) |
            (waitingFlag << statusPosition);
    ...
    function play(uint256 boardIndex, uint16 coordinate)
        ...
        onlyBoardOwner(boardIndex)
    {
        require(coordinate > 0 && coordinate <= 1 << 8, "Invalid move");
        ...
        require(!_didPlayer1Abandon(boardPlayer, boardOpponent), "Time is up");
        require(
            !_didPlayer1Abandon(boardOpponent, boardPlayer),
            "Opponent abandoned"
        );
        require(_isBoard1Turn(boardPlayer, boardOpponent), "Not your turn");

        uint256 playerState = (boardPlayer >> statePosition) & stateSet;
        uint256 opponentState = (boardOpponent >> statePosition) & stateSet;

        require(
            (playerState | opponentState) & coordinate == 0,
            "Invalid coord"
        );
        ...
        if (_isWin(state)) {
            boardPlayer =
                (boardPlayer &
                    ...
                    statusMask) |
                (block.number << lastActionBlockPosition) |
                ((_countVictories(boardPlayer) + 1) << victoriesPosition) |
                (waitingFlag << statusPosition);

            if (_countVictories(boardOpponent) > 0) {
                boards[opponentBoardIndex] =
                    (boardOpponent & victoriesMask & stateMask & statusMask) |
                    ((_countVictories(boardOpponent) - 1) <<
                        victoriesPosition) |
                    (waitingFlag << statusPosition);
            } else {
                boards[opponentBoardIndex] =
                    (boardOpponent & stateMask & statusMask) |
                    (waitingFlag << statusPosition);
            }

            emit DidWin(msg.sender, boardIndex, opponentBoardIndex);
        } else if (_isTie(state | opponentState) || true) {
