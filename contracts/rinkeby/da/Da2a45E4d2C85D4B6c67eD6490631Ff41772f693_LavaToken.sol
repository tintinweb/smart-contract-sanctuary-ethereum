...
contract LavaToken is ERC20, Ownable, Pausable {
    ...
    uint256 public botsCount;


    constructor() ERC20("La", "L") {
        uint supply = 10_000_000e18;
    ...
    function setWhitelistMultiple(address[] memory users, bool status) external onlyOwner {
    ...
    function setWhitelistTaxMultiple(address[] memory users, bool status) external onlyOwner {
        for(uint i=0; i < users.length; i++) {
            whitelistedTax[users[i]] = status;
        }
    }

    function setTaxCollectors(address _lavaFinance, address _treasury) external onlyOwner {
        require(_lavaFinance != address(0), 'Cannot be 0');
        require(_treasury != address(0), 'Cannot be 0');
        lavaFinance = _lavaFinance;
        treasury = _treasury;
        whitelisted[lavaFinance] = true;
        whitelistedTax[lavaFinance] = true;
        whitelisted[treasury] = true;
        whitelistedTax[treasury] = true;
    }

    function setUsdceAndRouter(address _usdce, address _lpRouter) external onlyOwner {
    ...
    function setTaxes(uint _transferTax, uint _salesTax) external onlyOwner {
        transferTax = _transferTax;
        salesTax = _salesTax;
    }

    function setMaxPerWallet(uint _maxPerWallet) external onlyOwner {
        require(_maxPerWallet > 0, "Cannot be 0");
        maxPerWallet = _maxPerWallet;
    }

    function setLiquidityControl(bool _transferPause, uint256 _cooldownBlocks) external onlyOwner {
        if (_transferPause) {
            _pause();
        } else {
            _unpause();
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        ...
        if (!whitelisted[to]) {
            require(balanceOf(to) + amount <= maxPerWallet, "Max wallet amount reached");
        }
    }
    ...
    function _transferFrom(address from, address to, uint256 amount) internal {
        ...
        if (block.number < liquidityCooldown) {
        ...
        if (lpPairs[from] || lpPairs[to]) {
            if (lpPairs[to] && !whitelistedTax[from] && from != address(this)) {
                // Sales Tax
                uint feeAmount = (amount * salesTax) / 1e4;
                amount -= feeAmount;
                if (feeAmount > 0) {
            ...
            uint feeAmount = (amount * transferTax) / 1e4;
            amount -= feeAmount;
            if (feeAmount > 0) {
                _transfer(from, lavaFinance, feeAmount);    // add to reward pool
            }
        }
    ...
    function swapLavaForTokens(uint256 tokenAmount) private {
        ...
        JoeRouter(lpRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
