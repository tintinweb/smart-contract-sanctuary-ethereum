...
// todo: rename contract and file
contract Pray is ERC721A, ERC2981, Ownable {

  // uint public price = 0.5 ether; // todo:
  uint public price = 0.0005 ether; // todo: update for mainnet

  // There are 1,000 tokens
  uint public constant MAX_SUPPLY = 1_000;

  // Is the contract open?
  bool public mintIsOpen = true; // todo

  // Did the team mint?
  bool public teamMinted;

  // The team's wallet
  address public withdrawAddress; // todo: hardcode with gnosis safe address

  // Where our metadata is located
  string public baseTokenURI;

  // The Merkle tree root
  bytes32 public merkleRoot;

  // A mapping to track who has minted
  mapping (address => bool) public alreadyMinted;

  // Constructor

  constructor() ERC721A("Butter", "Butta") { // todo: values
    _mint(msg.sender, 1); // Hello, world.
  }

  // Welcome.

  /**
  @notice Allows approved addresses to mint one (1) {name} // todo
  */
  function mint(bytes32[] calldata _merkleProof) external payable {
    if (mintIsOpen == false) revert SaleNotStarted();
    if (totalSupply() + 1 > MAX_SUPPLY) revert MintedOut();
    // if (alreadyMinted[msg.sender]) revert AlreadyMinted();
    if (price != msg.value) revert WrongPrice();

    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    if (MerkleProof.verify(_merkleProof, merkleRoot, leaf) == false) revert NotOnTheList();
  ...
  /**
  @notice Allows the team to do a 1-time mint to ourselves for marketing, promotions, etc.
  */
  function teamMint() external onlyOwner {
    require(teamMinted == false);
    _mint(msg.sender, 49); // todo 49 right num?
    teamMinted = true;
  }

  /**
  @notice Allows the team to mint to addresses for promotional reasons
  */
  function promoMint(address _to, uint _amount) external onlyOwner {
    if (totalSupply() + _amount > MAX_SUPPLY) revert MintedOut();
    _mint(_to, _amount);
  }
  ...
  */
  ...
  /**
  @notice Sets the Merkle Root used in the welcomeToDevotion function
  */
  function setMerkleRoot(bytes32 _root) external { // onlyOwner { todo
    merkleRoot = _root;
  }
  ...
  */
  function setMintIsOpen(bool _val) external { // onlyOwner { todo
    mintIsOpen = _val;
  }
  ...
  */
  ...
  /**
  @notice Withdraw funds to our withdraw address
  */
