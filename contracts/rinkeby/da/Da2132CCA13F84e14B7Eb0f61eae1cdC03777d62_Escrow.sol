...
contract Escrow is Initializable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => mapping(address => uint256))
        public getEscrowRemainingInput; // amount of money an address has deposited in the contract
    mapping(address => mapping(address => uint256))
        public getWithdrawableBalance; // amount of money an address can withdraw from the contract
    mapping(address => mapping(address => uint256)) public getRefundableBalance; // amount of money an address can refund to a particular participant in the contract
        ...
        for (uint256 i = 0; i < _participants.length; i++) {
            ...
            require(
                _participants[i] != _judge &&
                    ...
                    _participants[i] != _mainContractAddress,
                "corrupt participant found"
            );
            _addParticipant(_participant);
        }

        // make the main contract address a participant
        _addParticipant(_mainContractAddress);

        judge = _judge;
        isFreezed = false;
        blockNewParticipants = _blockNewParticipants;

        IThreescrow _mainContract = IThreescrow(_mainContractAddress);
        ...
        returns (bool _success)
    {
        ...
        emit Withdrew(msg.sender, _token, _amount, uint32(block.timestamp));
    ...
    // /**
    //  * @notice For the judge, to charge a fee.
    //  * @param _token The address of the ERC20 smart contract of the token to be charged
    //  * @param _amount The amount of tokens to be charged
    //  * @param _attemptPayment A boolean that determines if the judge should be paid immediately
    //  * @return _success A boolean that determines if the transaction was successful.
    //  */
    // function chargeFee(
    //     address _token,
    //     uint256 _amount,
    //     bool _attemptPayment
    // ) external nonReentrant judgeCheck freezeCheck returns (bool _success) {
    //     // check if this contract has the token balance
    //     require(
