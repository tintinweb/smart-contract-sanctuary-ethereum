...
// File: contracts/SimpleNFT1155.sol
...
contract NFT is ERC1155, Ownable {
    ...
    function mint(address to) external payable {
        ...
        if (whitelist[msg.sender].amount > 0) {
            ...
            require(amountsNFT[msg.sender] + 1 <= whitelist[msg.sender].amount, "nft collection amount is exceeded");
        } else {
            require(msg.value >= cost, "Insufficient funds");
            require(amountsNFT[msg.sender] + 1 <= nftAmountPerUser, "nft collection amount is exceeded");
        }
    ...
    function isUserInWhitelist(address user) external view returns (uint256) {
        ...
        return whitelist[user].amount;
    }

    function availableNFTs() external view returns (uint amount, uint feeForTransfer, uint costForMint) {
        if (whitelist[msg.sender].amount > 0){
            return(whitelist[msg.sender].amount - amountsNFT[msg.sender], whitelist[msg.sender].transferFee, whitelist[msg.sender].cost);
        }else{
            return(nftAmountPerUser - amountsNFT[msg.sender], transferFee, cost);
        }
    }
