...
contract MonstarzNFT is ERC721Enumerable, Ownable,Whitelist {
  using Strings for uint256;
  using SafeMath for uint256;

  string private baseURI = "https://nft.monstarznft.io/reveal/metadata/";
  string public notRevealedUri = "https://nft.monstarznft.io/notreveal/metadata/";
  bool public isSaleStarted = false;
  uint256 public preSaleCost = 0.07 ether;
  uint256 public generalSaleCost = 0.1 ether;
  uint256 public maxSupply = 10000;
  uint256 public nftPerAddressLimitOnPreSale = 10;
  uint256 public nftPerAddressLimitOnGeneralSale = 50;
  ...
  function getMintFees(uint _mintAmount) external view returns(uint256) {
        if(block.timestamp < generalSaleTime) {
  ...
  function startPresale() external onlyOwner {
      isSaleStarted = true;
      generalSaleTime = block.timestamp + 48 hours;
  }
  ...
  function setGeneralSaleNftcount(uint256 _limit) external onlyOwner {
     nftPerAddressLimitOnGeneralSale = _limit;
  }
  
  function setPreSaleNftcount(uint256 _limit) external onlyOwner {
    nftPerAddressLimitOnPreSale = _limit;
  }
  
  function setGeneralSaleCost(uint256 _newCost) external onlyOwner {
