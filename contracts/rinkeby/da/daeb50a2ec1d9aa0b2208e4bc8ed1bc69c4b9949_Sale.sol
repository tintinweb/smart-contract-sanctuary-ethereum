...
contract Sale is Ownable, ReentrancyGuard, Whitelist {
	...
	function claim(uint256 tokenId) public nonReentrant onlyDuringClaim onlyWhitelisted returns (uint256) {
	...
	function batchClaim(uint256[] calldata ids) external onlyDuringClaim onlyWhitelisted returns (uint256[] memory) {
		...
		for (uint256 i = 0; i < len; i++) {
			claimedIds[i] = 0;
			if (!hasClaimed[ccId][ids[i]]) claimedIds[i] = claim(ids[i]);
		}
		return claimedIds;
	}
