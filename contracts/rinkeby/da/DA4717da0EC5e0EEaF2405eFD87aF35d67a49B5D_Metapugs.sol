...
contract Metapugs is ERC721, IERC2981, Ownable, ReentrancyGuard {
    ...
    modifier maxTokensPerWallet(uint256 numberOfTokens) {
        require(
            ...
            "Quantity exceeds public sale limit for this wallet"
        );
        _;
    }
    ...
    function withdraw() public onlyOwner {
        ...
        uint256 tokensForPrimary;
        uint256 tokensForSecondary;
        uint256 remainder = balance;
        bool success;

        tokensForPrimary = (balance * primaryPercentage) / 100;
        remainder -= tokensForPrimary;

        tokensForSecondary = (balance * secondaryPercentage) / 100;
        remainder -= tokensForSecondary;

        (success, ) = primaryAddress.call{value: tokensForPrimary}("");
        require(success, "Transfer failed");

        (success, ) = secondaryAddress.call{value: tokensForSecondary}("");
        require(success, "Transfer failed");
