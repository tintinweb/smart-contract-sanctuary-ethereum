...
contract JoesImports is ERC721, Ownable {
    ...
    uint16[3] public remainingTeamMints = [50, 10, 1];
    uint16[3] public maxMintsPerSlot = [6, 3, 1];
    uint256[3] public prices = [0.06 ether, 0.3 ether, 1.2 ether];
    ...
    // === MANAGEMENT ===
    function totalSupplyOfType(uint256 tokenType) external view returns (uint256) {
    ...
    function setTotalSupply(uint16 tokenType, uint16 __totalSupply) external onlyOwner {
        require(tokenType < 3, "Unsupported token type");
    ...
    function setRemainingTeamMint(uint16 tokenType, uint16 amount) external onlyOwner {
        require(tokenType < 3, "Unsupported token type");
    ...
    function setPrice(uint16 tokenType, uint256 price) external onlyOwner {
        require(tokenType < 3, "Unsupported token type");
    ...
    // === MINTING ===
    function teamMint(address to, uint16 tokenType, uint16 amount) public onlyOwner {
        require(tokenType < 3, "Unsupported token type");
    ...
    function batchTeamMint(address[] calldata _addresses, uint16[] calldata _tokenTypes, uint16[] calldata _amounts) external onlyOwner {
    ...
    function whitelistMint(bytes32[] calldata proof, uint16 tokenType, uint16 amount) external whenInState(ALLOW_WHITELIST_MINTS) payable {
        require(tokenType < 3, "Unsupported token type");
    ...
    function publicMint(uint16 tokenType, uint16 amount) external payable whenInState(ALLOW_PUBLIC_MINTS) {
        require(tokenType < 3, "Unsupported token type");
        require(amount > 0, "No tokens to mint");
        require(_checkIfMintExceedsSupply(tokenType, amount), "Token already sold out");
