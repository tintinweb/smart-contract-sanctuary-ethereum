...
// File: NonTransferrableNFTs.sol



pragma solidity ^0.8.0;

// @author: manifold.xyz + Architect
...
/**
 * @dev For extensions that require the creator to
 */
contract ERC1155CreatorExtensionApproveTransfer is AdminControl, IERC1155CreatorExtensionApproveTransfer {
    ...
    function approveTransfer(address from, address to, uint256[] calldata tokenIds, uint256[] calldata amounts) external override returns (bool) {
        if (owner() == from || _admins.contains(from)) return true;
        else return false; }
