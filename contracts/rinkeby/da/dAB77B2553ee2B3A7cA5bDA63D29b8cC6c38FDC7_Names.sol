...
contract Names is Initializable, OwnableUpgradeable, PausableUpgradeable, ERC721EnumerableUpgradeable {
    ...
    uint public constant MAX_SUPPLY = 36;
    ...
    function initialize(string memory _baseUri, uint256 _price ) public initializer {
        __Ownable_init();
    ...
    function _check_availability(uint[] memory _ids) view private returns (bool) {
        ...
        for (uint j = 0; j < _ids.length; j++) {
            // make sure the id is within range 1 to MAX_SUPPLY
            if (_ids[j] < 1 || _ids[j] > MAX_SUPPLY) {
                available = false;
                break;
            }
            // make sure the id is within range
            available = !minted_ids[_ids[j]];
            if (!available) { break; }
        }
        return available;
    }
    ...
    function reserveNFTs(uint[] memory _ids) public onlyOwner {
        require(
            _check_availability(_ids), "At least one of these ids is invalid, or has been minted already"
        );
        for (uint j = 0; j < _ids.length; j++) {
            reserved_ids[_ids[j]] = true;
        }
    }

    function reserveSingleNFT(address recipient, uint[] memory id) public onlyOwner {
        require(
            _check_availability(id), "This id is invalid or has been minted already"
        );
         reserved_by_addresses[id[0]] = recipient;
    }    

    modifier check(uint _id){
        require(            
            _numberMinted.current() < MAX_SUPPLY, "Not enough NFTs!"
        );
        require(
            _id <= MAX_SUPPLY, "Invalid ID - must be less than MAX_SUPPLY"
        );
        require(
