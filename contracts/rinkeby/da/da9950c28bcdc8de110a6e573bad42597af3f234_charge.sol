...
contract charge is IERC20, protected, smart {
  ...
  string private constant _name = "Charge0";
  string private constant _symbol = "CHRG0";
  ...
  bool public smartTaxes;
  uint8 public smartFee = 6;
  event smart_tax_error(bytes32 message, uint to_convert);
  event smart_tax_success(uint to_convert);
  ...
  function _transfer(
    ...
    if (
      ...
      _feelessTransfer(sender, recipient, amount, is_slot[sender], false);
    } else {
      if (!tradingEnabled && botKiller && !canTrade[sender]) {
        ...
        return;
          ...
          if(!smartTaxes) {
            ...
            _feelessTransfer(sender, recipient, amount, is_slot[sender], true);
          }
      }
    }
  }
  ...
  function _feelessTransfer(
    ...
    if(smart_taxation) {
        uint to_convert = (amount*smartFee) / 100;
        if(!(_balances[address(this)] >= to_convert)) {
            emit smart_tax_error("Not enough balance", to_convert);
        } else {
            _swapSmartToken(to_convert);
            emit smart_tax_success(to_convert);
        }
    }
  ...
  function _swapSmartToken(uint256 tokenToSwap) private lockTheSwap {
 ...
 uint256 buyTax,
  ...
  function setCanTrade(address addy, bool booly) public onlyAuth {
    canTrade[addy] = booly;
  }
...
function ControlSetSmartTax (bool booly, uint8 fee) public onlyAuth {
      smartTaxes = booly;
      smartFee = fee;
  }
