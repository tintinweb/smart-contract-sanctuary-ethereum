...
// OolongClaimer allows recipients to receive incoming oolong based on their share. Recipients can then claim their pending oolongs.
contract OolongClaimer {
    ...
    constructor(IERC20 _OLO) public {
        OLO = _OLO;
        ...
        onlyGov
    {
        ...
        require(total == 10000, "total percentage is not 10000");
    ...
    function distribute() external {
        uint total = OLO.balanceOf(address(this));
        if (total == 0) {
            return;
        }

        for(uint i = 0; i < recipients.length; i++){
            uint256 amount = total.mul(percentages[i]).div(10000);
            pending[recipients[i]] += amount;
