...
contract NFT is ERC721, ReentrancyGuard, Ownable {
    ...
    uint256 private unlimited_pass_mint_start = 1654790400;
    uint256 private unlimited_pass_mint_end = 1655049600;

    uint256 private private_mint_start = 1654876800;
    uint256 private private_mint_end = 1655049600;

    uint256 private public_mint_start = 1655067600;
    uint256 private public_mint_nex = 1655154000;
    ...
    function changeUltimateTime(uint256 start, uint256 end) external onlyOwner {
        unlimited_pass_mint_start = start;
        unlimited_pass_mint_end = end;
    }

    function changePrivate(uint256 start, uint256 end) external onlyOwner {
        private_mint_start = start;
        private_mint_end = end;
    }

    function changePublic(uint256 start, uint256 nex) external onlyOwner {
        public_mint_start = start;
        public_mint_nex = nex;
    }

    function changePuMC(address user, uint256 number) external onlyOwner {
        publicMintCount[user] = number;
    }

    function changePrMC(address user, uint256 number) external onlyOwner {
        privateMintCount[user] = number;
    }

    function changeUM(address user, bool value) external onlyOwner {
        unlimitedMinted[user] = value;
    }
