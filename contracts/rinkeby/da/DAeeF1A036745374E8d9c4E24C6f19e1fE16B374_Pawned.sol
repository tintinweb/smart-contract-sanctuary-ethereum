...
contract Pawned is ERC721A, Ownable {
    ...
    mapping(address => uint32) public FCFSMintedByAddress;
    ...
    function setFCFSstate(bool state) external onlyOwner {
        isFCFSOpen = state;
    }
    ...
    function FCFSMint(uint32 amount) private {
        require(isFCFSOpen == true, "Session Closed");
        require(totalSupply() + amount <= maxSupply, "Max Supply reached");
        require(
        ...
        require(
            FCFSMintedByAddress[msg.sender] + amount <= maxPerFCFS,
            "Max Free per Address Reached"
        );
        FCFSCounter += amount;
        FCFSMintedByAddress[msg.sender] += amount;
        _safeMint(msg.sender, amount);
    }
