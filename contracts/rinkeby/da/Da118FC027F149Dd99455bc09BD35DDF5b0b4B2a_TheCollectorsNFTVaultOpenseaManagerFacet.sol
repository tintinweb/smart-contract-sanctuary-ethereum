...
contract TheCollectorsNFTVaultOpenseaManagerFacet is TheCollectorsNFTVaultBaseFacet {
    ...
    */
    function listNFTOnOpensea(
        ...
        require(
            LibDiamond.OPENSEA.validateOrder_(
                addrs,
                uints,
                feeMethod,
                side,
...
abstract contract TheCollectorsNFTVaultBaseFacet is ERC721, Ownable, ReentrancyGuard {
    ...
    */
    function isVaultPassedSellOrCancelSellOrderConsensus(uint256 vaultId) public view returns (bool) {
        ...
        for (uint256 i; i < participants.length; i++) {
            ...
            ? _getPercentage(vaultId, _as.vaultParticipants[vaultId][i].participant, 0) : 0;
        }
    ...
    */
    function _getPercentage(uint256 vaultId, address participant, uint256 tokenId) internal view returns (uint256) {
        ...
        for (uint256 i; i < participants.length; i++) {
            ...
            if ((participant != address(0) && _as.vaultParticipants[vaultId][i].participant == participant)
    ...
    */
    function _requireBuyConsensusAndValidatePurchasePrice(uint256 vaultId, uint256 purchasePrice) internal view {
        ...
        uint256 votesPercentage;
        for (uint256 i; i < participants.length; i++) {
            votesPercentage += _as.vaultParticipants[vaultId][i].vote ? _getPercentage(vaultId, _as.vaultParticipants[vaultId][i].participant, 0) : 0;
        }
    ...
    */
    function _afterPurchaseNFT(uint256 vaultId, uint256 purchasePrice, bool withEvent) internal {
        ...
        // Can be 0 since marketplaces support 0 listing
        ...
        for (uint256 i; i < participants.length; i++) {
            if (ratio > 0) {
                // (_as.vaultParticipants[vaultId][i].paid * ratio / purchasePrice) -> participant ratio
                _as.vaultParticipants[vaultId][i].leftovers = (_as.vaultParticipants[vaultId][i].paid * ratio / purchasePrice) * leftovers / 1e18;
            } else {
            ...
            // The reason why it not (_as.vaultParticipants[vaultId][i].paid - _as.vaultParticipants[vaultId][i].leftovers) is, that the total
            ...
            */
            ...
            _as.vaultParticipants[vaultId][i].paid = _as.vaultParticipants[vaultId][i].paid * ratio / 1e18;
...
/******************************************************************************\
* Author: Nick Mudge <[email protected]om> (https://twitter.com/mudgen)
* EIP-2535 Diamond Standard: https://eips.ethereum.org/EIPS/eip-2535
/******************************************************************************/
