...
contract DevialetERC1155Buyer is Ownable {
    ...
    function setPrice(uint256 tokenId, uint256 price) public onlyOwner {
        ...
        require(price > 0, 'DevialetERC1155Buyer: price must be > 0');
    ...
    function setPriceBatch(uint256[] memory tokenIds, uint256[] memory prices) public onlyOwner {
        ...
        require(tokenIds.length == prices.length, 'DevialetERC1155Buyer: tokensIds and prices length do not match');

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(prices[i] > 0, 'DevialetERC1155Buyer: price must be > 0');
    ...
    function buyToken(address to, uint256 tokenId, uint256 amount, bytes memory data) public payable {
        ...
        require(_prices[tokenId] > 0, 'DevialetERC1155Buyer: wrong token id');
        require(amount <= 5, "DevialetERC1155Buyer: can't buy more than 5 tokens");
        require(msg.value >= _prices[tokenId] * amount, "DevialetERC1155Buyer: not enough ETH sent");
    ...
    function buyTokenBatch(address to, uint256[] memory tokenIds, uint256[] memory amounts, bytes memory data) public payable {
        ...
        require(tokenIds.length == amounts.length, 'DevialetERC1155Buyer: tokensIds and amounts length do not match');

        uint256 totalAmount = 0;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(_prices[tokenIds[i]] > 0, 'DevialetERC1155Buyer: wrong token id');
            require(amounts[i] <= 5, "DevialetERC1155Buyer: can't buy more than 5 tokens");
            totalAmount += _prices[tokenIds[i]] * amounts[i];
        }
        require(msg.value >= totalAmount, "DevialetERC1155Buyer: not enough ETH sent");
