...
contract CryptoWillPrevail is ERC721Enumerable, Ownable {
    ...
    constructor(address w1, address w2, string memory uri) ERC721("CryptoWillPrevail NFT", "CWP NFT") {
        for(uint i = 1; i < groups.length; i++) {
            groups[i].normalCount = 600;
    ...
    function getToken(address addr, bool isInfluencer) internal returns(uint) {
        if(isInfluencer) {
            return 1250 + 1 + influencers.length;
        } else {
    ...
    function mintNFT(address addr, GroupID groupID, NFTKind kind, string memory message) internal returns(uint) {
        ...
        require(kind > NFTKind.None && kind <= NFTKind.Influencer);
        if(kind == NFTKind.Influencer) require(groupID == GroupID.Influencer, "Invalid groupID");
        else require(groupID > GroupID.None && groupID < GroupID.Influencer, "Invalid groupID");

        uint gid = uint(groupID) - 1;
        if(kind != NFTKind.Influencer) {
            if(kind == NFTKind.Special) {
                require(groups[gid].specialCount > 0, "Sold out");
                groups[gid].specialCount--;
            } else if(kind == NFTKind.Normal) {
                require(groups[gid].normalCount > 0, "Sold out");
                groups[gid].normalCount--;
            }
        }

        uint tokenID = getToken(addr, kind == NFTKind.Influencer);
    ...
    function getInfluencerFromCode(string memory code) internal view returns(int) {
        bytes32 code_hash = keccak256(abi.encodePacked(code));
        for(uint i = 0; i < influencers.length; i++)
            if (code_hash == keccak256(abi.encodePacked(influencers[i].affiliateCode)))
                return int(i);
        return -1;
    }
    ...
    function mint(GroupID groupID, bool special, string memory message, string memory code) external payable returns(uint) {
        require(bytes(message).length <= 32, "Message is too long");
        int influencerIdx = getInfluencerFromCode(code);

        uint price = (special ? specialPrice : normalPrice);
        if(influencerIdx >= 0) price -= price / 10;

        require(msg.value >= price, "Message value is not enough");

        if(influencerIdx >= 0) {
            uint idx = uint(influencerIdx);
            uint transfer_price = msg.value / 10;
            payable(influencers[idx].addr).transfer(transfer_price);
        }

        return mintNFT(msg.sender, groupID, special ? NFTKind.Special : NFTKind.Normal, message);
    }
    ...
    function isInfluencerRegistered(address addr) public view returns(bool) {
        for(uint i = 0; i < influencers.length; i++)
            if(influencers[i].addr == addr) return true;

        return false;
    }

    function influencersCount() public view returns(uint) {
    ...
    function groupCounts() public view returns(uint16[] memory) {
        ...
        for(uint i = 1; i < 22; i++) {
            counts[i * 2] = groups[i].normalCount;
    ...
    function setNormalPrice(uint price) external onlyOwner {
        normalPrice = price;
    }
    ...
    function registerInfluencer(address addr, string memory message, string memory affiliateCode) external onlyOwner returns(uint) {
        require(!isInfluencerRegistered(addr), "An influencer with this address has already been added");
        for(uint i = 0; i < influencers.length; i++) {
            require(keccak256(abi.encodePacked(influencers[i].affiliateCode)) 
                != keccak256(abi.encodePacked(affiliateCode)), 
                "Affiliate code already in use");
        }

        uint token = mintNFT(addr, GroupID.Influencer, NFTKind.Influencer, message);
        Influencer memory influencer = Influencer(addr, affiliateCode, token);
