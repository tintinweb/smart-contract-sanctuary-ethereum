...
contract UnipilotFarm is IUnipilotFarm, ReentrancyGuard {
    ...
    ///@notice use for whitelist to list of the pools
    ///@param _pools list of pools
    function whitelistPools(address[] memory _pools) public onlyGovernance {
        address[] memory listed;
        ...
        if (listed.length > 0) {
            emit WhitelistPools(listed, block.timestamp);
        }
    }
...
interface IUnipilotFarm {
    ...
    event WhitelistPools(address[] pools, uint256 timestamp);
