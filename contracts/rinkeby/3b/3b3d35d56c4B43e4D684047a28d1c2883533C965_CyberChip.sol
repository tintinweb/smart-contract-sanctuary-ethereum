...
contract CyberChip is ERC721, ERC721Enumerable, Ownable {
    ...
    function _checkAddressEligibilityPhase2(address addr)
        ...
        returns (uint256 res)
    {
        if (phase2AddresMapping[addr]) return 0;
        for (uint256 i = 0; i < ERC721(firstNFTAddress).balanceOf(addr); i++) {
            uint256 id = ERC721Enumerable(firstNFTAddress).tokenOfOwnerByIndex(
                addr,
                i
    ...
    function _mapUsedNFTsAndAddressPhase2(address addr) internal {
        for (uint256 i = 0; i < ERC721(firstNFTAddress).balanceOf(addr); i++) {
            uint256 id = ERC721Enumerable(firstNFTAddress).tokenOfOwnerByIndex(
                addr,
                i
    ...
    function _mintPhase1(address addr) internal {
        ...
        if (price > 0)
            ERC20(tokenAddress).transferFrom(addr, address(this), price);
    ...
    function _mintPhase2(address addr, uint256 count) internal {
        ...
        _mapUsedNFTsAndAddressPhase2(addr);
