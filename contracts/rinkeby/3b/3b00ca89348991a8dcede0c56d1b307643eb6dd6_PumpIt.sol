...
contract PumpIt is ERC721A, Ownable, ReentrancyGuard {
  ...
  string  public  baseTokenURI = "ipfs://QmaMf5PegavppHi2V5zmFUDTP9gdXw5DpJbpyqkR96DoUf";
  ...
  function mint(uint256 numberOfTokens) external payable nonReentrant {
        ...
        require(
            numberMinted(msg.sender) + numberOfTokens <= Max_Per_TX,
            "-This is more than allowed-"
        );
        require(
            totalSupply() + numberOfTokens <= Max_Supply,
            "-Not enough numberOfTokens-"
        );

        uint256 _cost;
        if (INSTANT_FREE_MINTED < Num_Free_Mints) {
            uint256 remainFreeAmont = (numberMinted(msg.sender) <
                Max_Free_Per_Wallet)
                ? (Max_Free_Per_Wallet - numberMinted(msg.sender))
                : 0;

            _cost =
                cost *
                (
                    (numberOfTokens <= remainFreeAmont)
                        ? 0
                        : (numberOfTokens - remainFreeAmont)
                );

            INSTANT_FREE_MINTED += (
                (numberOfTokens <= remainFreeAmont) ? numberOfTokens : remainFreeAmont
            );
        } else {
            _cost = cost * numberOfTokens;
        }
  ...
  function treasuryMint(uint numberOfTokens)
    public
    onlyOwner
  {
    require(
  ...
  function setSaleCost(uint256 _Cost)
      external
      onlyOwner
  {
      cost = _Cost;
