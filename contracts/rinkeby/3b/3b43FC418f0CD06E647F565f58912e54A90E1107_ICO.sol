...
contract ICO {
    ...
    function contribute() external payable {
        ...
        require(address(this).balance <= PHASE_GENERAL_OPEN_CAP, "ICO: Can't go above hardcap of 30k ETH");

        if(currentPhase == 0) {
            require(seedInvestors[msg.sender], "ICO: You are not in the allow list for seed investors");
            require(address(this).balance <= PHASE_SEED_CAP, "ICO: Can't go above hardcap of 15k ETH for seed");
    ...
    function progressPhases() external onlyOwner {
        require(isICOEnable, "ICO: Must be active");
    ...
    /// add/remove seed investors to the ICO 
    function addRemoveSeedInvestors(address _address) external onlyOwner {
        seedInvestors[_address] = !seedInvestors[_address];
    }
