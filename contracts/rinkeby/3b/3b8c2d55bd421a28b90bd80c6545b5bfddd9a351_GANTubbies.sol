...
contract GANTubbies is ERC721A, Ownable {
    using Strings for uint256;

    uint256 public MAX_SUPPLY = 2999;
    ...
    constructor(bytes32 _merkleRoot) ERC721A("GAN Tubby Cats", "GANTUBBY") {
    ...
    function whitelistMint(bytes32[] calldata _merkleProof, uint256 amountToMint, uint256 merkleProofAmount, address _addr) external {
        require(FREE_MINT_SUPPLY <= 1000);

        bytes32 leaf = keccak256(abi.encodePacked(_addr, merkleProofAmount));
        require(
            MerkleProof.verify(_merkleProof, merkleRoot, leaf),
            "Invalid Merkle Proof."
        );
        require(whitelistMinted[_addr] + amountToMint <= merkleProofAmount, "you have no more free mints");
        _mint(msg.sender, amountToMint);
        whitelistMinted[_addr] += amountToMint;
        FREE_MINT_SUPPLY++;
    ...
    function publicMint(uint256 amountToMint) public payable {
        ...
        uint cost;
        unchecked {
            cost = amountToMint * 0.02 ether;
        }
        require(msg.value == cost, "must pay 0.02 eth per token");
        _mint(msg.sender, amountToMint);
        require(totalSupply() <= MAX_SUPPLY, "token supply reached");
    }
}
