...
contract ScalesNFT is ERC721URIStorage, Ownable {
    ...
    uint256 public max_mint_amount = 2;
    uint256 public newAddedAmount;
    ...
    event ownerRegisterSomeUsersToWhitelist ( address[] users );
    event ownerRemoveSomeUsersFromWhitelist ( address[] users );
    event userMintOneNFT( address user, uint256 tokenId);
    event userMintSomeNFTs ( address to, uint256[] tokenIds);
    ...
    function registerToWhitelist ( address user ) public onlyOwner {
        require( whitelisted[user] == 0, "Register To Whitelist : This user is already whitelisted" );
        whitelisted[user] = 1;
        emit ownerRegisterOneUserToWhitelist(user);
    }

    function removeFromWhitelist ( address user) public onlyOwner {
        require(whitelisted[user] == 1, "Remove From Whitelist : This user have never been whitelisted");
        whitelisted[user] = 0;
        emit ownerRemoveOneUserFromWhitelist(user);
    }

    function registerToWhitelistBatch ( address[] memory users ) public onlyOwner {
    ...
    function removeFromWhitelistBatch ( address[] memory users ) public onlyOwner {
        for ( uint i = 0; i < users.length; i++ ) {
    ...
    function setNewAddedAmount ( uint256 _newAddedAmount ) public onlyOwner {
        newAddedAmount = _newAddedAmount;
        totalDroppedAmount += newAddedAmount;
        emit ownerSetNewAddedAmount(_newAddedAmount);
    }
    ...
    function getNewAddedAmount() public view returns (uint256) {
        return newAddedAmount;
    }

    function getTotalDroppedAmount () public view returns (uint256) {
