...
contract PumaNitroPass is ERC721A, Ownable {
    ...
    address public TokenContract = 0xD96463a406e734dEBD0f3eaBF2343bDC7a86Ac83;
    ...
    mapping(address => bool) public visitedExperience;
    ...
    /// Minting Variables
    string public baseURI = "ipfs://QmUHJpkxd8ZwXE9G1tAbDyEk4xa6fptAFjVS6AUQtD2WBh/pumanitrotoken.json";
    Status public state;
    ...
    /// Called by ====== Contract to burn Nitro Token on minting Nitro Pass
    function burnNitroPassForForeverPhysical(uint256 NitroPassId) external {
        require(msg.sender == ForeverContract, "Invalid burner address");
        _burn(NitroPassId);
    }

    function burnAllNitroPasses() external onlyOwner {
        require(state == Status.ENDED, "PUMA Nitro Pass: Still Live");
    ...
    function burnNitroPass(uint256 tokenId) external onlyOwner {
        require(state == Status.ENDED, "PUMA Nitro Pass: Still Live");
        require(_exists(tokenId), "Token does not exist");
        _burn(tokenId);
    }

    function burnNitroTokenforNitroPass(uint256[] memory tokenIds) public {
        require(msg.sender == tx.origin, "PUMA Nitropass: Contract Interaction Not Allowed");
        require(state == Status.TOKEN_MINT, "PUMA Nitropass: Claim Is Not Live");
    ...
    function publicMint() public {
        ...
        require(mintCount + 1 <= MAX_SUPPLY, "PUMA Nitropass: Mint Supply Exceeded");
        mintCount++;
