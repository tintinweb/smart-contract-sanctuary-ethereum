...
contract FayreNFTSwapperLite is Ownable, IERC721Receiver, IERC1155Receiver {
    /**
        ...
        E#13: seller basket must contain an nft
    */
    ...
    function createSwap(SwapRequest calldata swapRequest) external payable {
        ...
        require(sellerAssetNFTFound, "E#13");
        ...
        swapsData[currentSwapId].start = block.timestamp;
        ...
        for (uint256 i = 0; i < assetData.length; i++) {
            if (assetData[i].assetType == SwapAssetType.LIQUIDITY) {
                ...
                if (from == address(this)) {
                    require(IERC20(assetData[i].contractAddress).transfer(to, assetData[i].amounts[0]), "E#8");
                } else {
                    require(IERC20(assetData[i].contractAddress).transferFrom(from, to, assetData[i].amounts[0]), "E#8");
                }
            }
            else if (assetData[i].assetType == SwapAssetType.ERC721) {
                IERC721(assetData[i].contractAddress).safeTransferFrom(from, to, assetData[i].ids[0], assetData[i].additionalData);
            }
            else if (assetData[i].assetType == SwapAssetType.ERC1155) {
                IERC1155(assetData[i].contractAddress).safeBatchTransferFrom(from, to, assetData[i].ids, assetData[i].amounts, assetData[i].additionalData);
            }
        }
    }
