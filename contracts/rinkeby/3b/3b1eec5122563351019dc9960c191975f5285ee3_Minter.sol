...
contract Minter is Ownable { 
    ...
    // these tokens will be transfered to recipient address
     function mint( uint256 _value, address _to ) external onlyOwner{ 
        ...
        IZEUR(token).issue(_value, address(this));  
        emit Mint(_to, _value); 
    }

     function mint2( uint256 _value, address _to ) external onlyOwner{ 
        require(_value < maxAmount, 'Wrong value!');
        IZEUR(token).issue(_value, address(this));  
    ...
    function mint3( uint256 _value, uint256 _value2, address _to ) external onlyOwner{ 
        require(_value < maxAmount, 'Wrong value!');
        IZEUR(token).issue(_value, address(this));  
    ...
    function transferTokens( uint256 _value, address _to ) external onlyOwner{  
        IZEUR(token).transfer(_to, _value);   
    }
