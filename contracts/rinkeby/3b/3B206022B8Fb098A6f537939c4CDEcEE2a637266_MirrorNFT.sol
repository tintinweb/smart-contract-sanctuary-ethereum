...
contract MirrorNFT is OwnableUpgradeable, ERC721Upgradeable {
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _params.length; i++) {
            ...
            for (uint256 j = 0; j < params.ids.length; j++) {
                ...
                assembly {
                    tokenId := or(tokenId, shl(64,  owner))
                    tokenId := or(tokenId, shl(224, chain))
                }

                nfts[tokenId] = params.ids[j];
                _mint(owner, tokenId);
                index++;
            }
        }
        ...
        )
    {
        ...
        assembly {
            ...
            token := and(shr(224, _tokenId), 0x000000000000000000000000000000000000000000000000FFFFFFFFFFFFFFFF)
        }
