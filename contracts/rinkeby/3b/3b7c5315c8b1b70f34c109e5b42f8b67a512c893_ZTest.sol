...
contract ZTest is Context, IERC20, Ownable {
    ...
    string private constant _name = "ZTEST7";
    string private constant _symbol = "ZTEST7";
    ...
    function checkWinner() public{
        if(_timer != 0 && block.timestamp >= _timer){
            if( _winnersBuyTimestamp[_lastBuyer] == 0){
                ...
                _winnersBuyTimestamp[_lastBuyer] = block.timestamp;
            }
    ...
    function payWinners() public{
        ...
        for( uint256 winnerIndex = 0; winnerIndex < _winners.length; winnerIndex++){
            ...
            if(_winnersBuyTimestamp[winner] + (24 hours) >= block.timestamp){
                //pay the winner bc he held for 24hrs
