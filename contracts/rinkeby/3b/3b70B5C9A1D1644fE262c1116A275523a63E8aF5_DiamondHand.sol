...
contract DiamondHand is Ownable, IERC721Receiver, IERC1155Receiver {
    ...
    * @return diamondID
    */
    function createDiamondHands(diamondStruct[] memory _diamondAsset, uint256 _releaseTime, uint256 _breakPrice, diamondSpecialStruct memory _diamondSpecial) payable public returns (uint256) {
        require(_releaseTime > block.timestamp, "Release time is before current time");
        require(_breakPrice >= minBreakPrice, "Emergency break price is less than the minimum");
        ...
        for(i = 0; i < diamondAssets[_diamondHands.id].length; i++) {
            if(diamondAssets[_diamondHands.id][i].tokenType == ERC721) {
                ...
                //Map NFT's (contract address, token ID) to sender's address
                uint256 j;
                for (j = 0; j < diamondAssets[_diamondHands.id][i].tokenId.length; j++){
                    nftToAddressMap[keccak256(abi.encodePacked(diamondAssets[_diamondHands.id][i].contractAddress, diamondAssets[_diamondHands.id][i].tokenId[j]))] = msg.sender;
                }
            }
    ...
    */
    function setDiamondSpecial(address[] memory  _contractAddresses, uint256[] memory _tokenIds, bool isOnList) public onlyOwner {
        uint256 i;
    ...
    */
    function setMinBreakPrice(uint256 _minPrice) public onlyOwner {
