    ...
    function withdrawNFT(address recipient) external onlyOwner {
        ...
        for (uint256 i = 0; i < MAX_PER_WALLET; i++) {
            if (token.balanceOf(address(this)) > 0) {
                ...
                require(token.ownerOf(tokenId) == address(this), "You must own the token");
...
contract MintFactory is Ownable {
    ...
    function createBatchMint(address _t, uint256 _num) external payable onlyOwner {
        require(msg.value >= (MAX_PER_WALLET + 1) * NFT_PRICE * _num, "Not enough eth to pay");
    ...
    function getContracMint() public view returns (ContractMint[] memory) {
        return _mint;
    }
}
