...
contract Lottery is Lobby, Ownable {
    ...
    function autotaskTEST() public view returns (uint256) {
        require(msg.sender == autotask, "Your not a autotask");
        return LotteryCount;
    }
    ...
    function setPromSet(string memory a) public {
        ...
        require(Prom[msg.sender].PromSet == bytes32(0), "You already set Prom");
        require(PromSetReverse[t] == address(0), "Prom Already Used");
    ...
    function setPromInput(string memory a) public {
        bytes32 t = keccak256(abi.encodePacked(a));
        require(PromSetReverse[t] != msg.sender, "Thats your Prom");
        require(
            Prom[msg.sender].PromInput == bytes32(0),
            "You already set Prom"
        );
        require(PromSetReverse[t] != address(0), "Prom do not exist");
    ...
    function findAddressInPlayers(address _a) private returns (bool) {
    ...
    function Play() public {
        require(msg.sender == owner() || msg.sender == autotask, "no allow");
        address winer;
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Mapping from token ID to owner address
    mapping(uint256 => address) internal _owners; //я поменял с external на internal

    // Mapping owner address to token count
    mapping(address => uint256) internal _balances; //я поменял с external на internal
