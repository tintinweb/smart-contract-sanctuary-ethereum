...
contract PaperHands is ERC721, Ownable {
    ...
    function burnNFT(uint256 tokenId) external onlyOwner exists(tokenId) {
        ...
        hodl = tokenTimestamp[tokenId];
        timeHeld = block.timestamp - hodl;
        // convert time held to a int96 usable flow rate
        int96 flowTime = int96(int256(timeHeld));
        timeStampValue = flowTime; // give me the value so I can see
        int96 burnFlowRate =       385802469135;            //flowTime * 385802469; //Multiply times .001 token a month
