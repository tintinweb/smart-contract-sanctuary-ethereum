...
contract AsciiCyberKongz14 is ERC721A, Ownable, ReentrancyGuard {
  ...
  function whitelistMint(uint256 _mintAmount, uint256 nftTokenIdWhitelisted, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    if (nftTokenIdWhitelisted > 0) {
      ...
      require(allowMultipleWhitelistMints || whiteListAddressMintCount[_msgSender()]<maxMintPerWhiteListAddress, 'You reached the maximum mints allowed by whitelist address!');
    }
    if (nftTokenIdWhitelisted > 0) {
