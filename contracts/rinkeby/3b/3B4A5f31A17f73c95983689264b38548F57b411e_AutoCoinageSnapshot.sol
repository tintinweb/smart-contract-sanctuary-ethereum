...
contract AutoCoinageSnapshot is AutoCoinageSnapshotStorage, DSMath {
    ...
    // layer2 에 대한 스냅샷아이디 , 레이어별로 스냅샷아이디가 별도로 존재
    function getCurrentSnapshotId(address layer2) public view returns (uint256) {
        return currentSnapshotId[layer2];
    }
    ...
    function _factorAt(address layer2, uint256 snapshotId) internal view virtual returns (bool, uint256, uint256) {
        ...
        require(snapshotId <= getCurrentSnapshotId(layer2), "ERC20Snapshot: nonexistent id");
    ...
    function _balanceOfAt(address layer2, address account, uint256 snapshotId)
        ...
        returns (bool, uint256, uint256, uint256)
    {
        (bool snapshotted, uint256 balances, uint256 refactoredCounts, uint256 remains)
            = _valueAt(layer2, snapshotId, accountBalanceSnapshots[layer2][account]);
    ...
    {
        (bool snapshotted, uint256 balances, uint256 refactoredCounts, uint256 remains)
            = _valueAt(layer2, snapshotId, totalSupplySnapshots[layer2]);
    ...
    {
        return balanceOfAt(layer2, account, getCurrentSnapshotId(layer2));
    }
    ...
    function balanceOfAt(address layer2, address account, uint256 snapshotId)
        public view returns (uint256)
    {
        (bool snapshotted1, uint256 balances, uint256 refactoredCounts, uint256 remains) = _balanceOfAt(layer2, account, getCurrentSnapshotId(layer2));
        (bool snapshotted2, uint256 factors, uint256 refactorCounts) = _factorAt(layer2, getCurrentSnapshotId(layer2));
    ...
    function totalSupply(address layer2)
        public view returns (uint256)
    {
        return totalSupplyAt(layer2, getCurrentSnapshotId(layer2));
    }

    function totalSupplyAt(address layer2, uint256 snapshotId)
        public view returns (uint256)
    {
        (bool snapshotted1, uint256 balances, uint256 refactoredCounts, uint256 remains) = _totalSupplyAt(layer2, getCurrentSnapshotId(layer2));
        (bool snapshotted2, uint256 factors, uint256 refactorCounts) = _factorAt(layer2, getCurrentSnapshotId(layer2));
    ...
    function _snapshot(address layer2) internal virtual returns (uint256) {
        ...
        if(blockNumberBySnapshotId[layer2][currentSnapshotId[layer2]] < block.number) {
            currentSnapshotId[layer2]++;
            blockNumberBySnapshotId[layer2][currentSnapshotId[layer2]] = block.number;

            uint256 currentId = getCurrentSnapshotId(layer2);
     ...
     /*
    function getAccountBalanceSnapshots(address layer2, address account, uint256 snapshotId) public view
        returns (
        ...
        )
    {
        ...
        if (_lastSnapshotId(accountBalanceSnapshots[layer2][account].ids) < currentId){
            snapShotBalance = accountBalanceSnapshots[layer2][account].balances[snapshotId];
            snapShotRefactoredCount = accountBalanceSnapshots[layer2][account].refactoredCounts[snapshotId];
            snapShotRemain = accountBalanceSnapshots[layer2][account].remains[snapshotId];
        }
    }

    function getTotalBalanceSnapshots(address layer2, uint256 snapshotId) public view
        returns (
                uint256 snapShotBalance,
                uint256 snapShotRefactoredCount,
                uint256 snapShotRemain
        )
    {
        ...
        if (_lastSnapshotId(totalSupplySnapshots[layer2].ids) < currentId){
            snapShotBalance = totalSupplySnapshots[layer2].balances[snapshotId];
            snapShotRefactoredCount = totalSupplySnapshots[layer2].refactoredCounts[snapshotId];
            snapShotRemain = totalSupplySnapshots[layer2].remains[snapshotId];
        }
    }

    function getFactorSnapshots(address layer2, uint256 snapshotId) public view
        returns (
                uint256 snapShotFactors,
                uint256 snapShotRefactorCounts
        )
    {
        uint256 currentId = getCurrentSnapshotId(layer2);

        if (_lastSnapshotId(factorSnapshots[layer2].ids) < currentId){
            snapShotFactors = factorSnapshots[layer2].factors[snapshotId];
            snapShotRefactorCounts = factorSnapshots[layer2].refactorCounts[snapshotId];
        }
    }
    */
    ...
    function update(address layer2, address account) public {
        ...
        //(uint256 tbalances, uint256 trefactoredCounts, uint256 tremains)  = AutoRefactorCoinageI(coinage)._totalSupply();
         AutoRefactorCoinageI.Balance memory totalBalance  = AutoRefactorCoinageI(coinage)._totalSupply();
        ...
        console.log("tbalances %s", tbalances);
        console.log("trefactoredCounts %s", trefactoredCounts);
        console.log("tremains %s", tremains);

        uint256 _factor  = AutoRefactorCoinageI(coinage)._factor();
        uint256 refactorCount  = AutoRefactorCoinageI(coinage).refactorCount();

        console.log("_factor %s", _factor);
        console.log("refactorCount %s", refactorCount);
        ...
        external returns (bool)
    {
        address coinage  = SeigManagerI(seigManager).coinages(layer2);
        require(coinage != address(0), "zero coinages");
        console.log('updateBatchOnline %s', coinage);
        snapshot(layer2);

        for (uint256 i = 0; i < accounts.length; ++i) {
            console.log('accounts %s', accounts[i]);
            AutoRefactorCoinageI.Balance memory accountBalance  = AutoRefactorCoinageI(coinage).balances(accounts[i]);
            console.log('balance %s', accountBalance.balance);
            console.log('refactoredCount %s', accountBalance.refactoredCount);
            console.log('remain %s', accountBalance.remain);
        ...
        external onlyRole(UPDATE_ROLE)  returns (bool)
    {
        ...
        updateLayer2Factor(layer2, layerFactor[0], layerFactor[2]);
    ...
    */
        ...
        )
    {
        address coinage  = SeigManagerI(seigManager).coinages(layer2);
        require(coinage != address(0), "zero coinages");
        console.log('getLayer2TotalSupplyInTokamak %s',coinage);
        totalSupply = AutoRefactorCoinageI(coinage).totalSupply();
        console.log('%s', totalSupply);

        AutoRefactorCoinageI.Balance memory totalBalance  = AutoRefactorCoinageI(coinage)._totalSupply();
        console.log('%s', totalBalance.balance);
        console.log('%s', totalBalance.refactoredCount);
        console.log('%s', totalBalance.remain);
        ...
        )
    {
        address coinage  = SeigManagerI(seigManager).coinages(layer2);
        require(coinage != address(0), "zero coinages");
        console.log('getLayer2BalanceOfInTokamak %s',coinage);
        balanceOf = AutoRefactorCoinageI(coinage).balanceOf(user);
