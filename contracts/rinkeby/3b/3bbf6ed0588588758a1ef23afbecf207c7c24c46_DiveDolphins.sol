...
contract DiveDolphins is ERC721A, Ownable, ReentrancyGuard {
    ...
    // mapping functions :: keeps track of mints per wallet (public and whitelist)
    mapping(address => uint256) public totalPublicMint;
    mapping(address => uint256) public totalWhitelistMint;

    constructor() ERC721A("Dive Dolphins", "DD") {}

    // checks if caller is a user (wallet), not another smart contract (reduces risk of botting and exploits)
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseTokenUri).length > 0 ? string(abi.encodePacked(baseTokenUri, trueId.toString(), ".json")) : ""; // updates metadata to reveal images
    }
