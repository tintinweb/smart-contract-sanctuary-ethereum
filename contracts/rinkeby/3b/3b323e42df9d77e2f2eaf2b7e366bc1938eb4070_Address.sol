...
// File: contracts/BondlyLaunchPad.sol
...
contract BondlyLaunchPad is Ownable {
    ...
    modifier onlyAdmin() {
        require(
            msg.sender == _owner || _adminList[msg.sender],
            "Not Owner or Admin"
        );
        _;
    }
    ...
    function updateTotalNftAmount(uint256 _cardId, uint256 _amount)
        external
        onlyAdmin
    {
        Card storage card = _cards[_cardId];
        uint256 _soldAmount = card.totalAmount - card.currentAmount;
        require(
            _amount >= _soldAmount,
            "Total amount cannot smaller than purchased amount"
        );
        IERC1155 _contract = IERC1155(card.contractAddress);
        require(
            _contract.balanceOf(_salesperson, card.tokenId) >= _amount,
            "Initial supply cannot be more than available supply"
        );
        require(
            _contract.isApprovedForAll(_salesperson, address(this)) == true,
        ...
        if (_amount >= card.totalAmount) {
            card.currentAmount += _amount - card.totalAmount;
        } else {
            card.currentAmount -= card.totalAmount - _amount;
        }
        emit UpdateTotalAmount(msg.sender, _cardId, _amount);
    }
    ...
    function addAdminListAddress(address addr) external onlyOwner {
        _adminList[addr] = true;
    }

    function batchAddAdminListAddress(address[] calldata addr)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addr.length; i++) {
            _adminList[addr[i]] = true;
        }
    }

    function removeAdminListAddress(address addr) external onlyOwner {
        _adminList[addr] = false;
    }

    function batchRemoveAdminListAddress(address[] calldata addr)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addr.length; i++) {
            _adminList[addr[i]] = false;
        }
    }
