...
contract PNDC_ERC721 is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function batchMint(
        ...
        require(_totalNft <= 60, "Minting more than 15 Nfts are not allowe");
        require(
            _totalNft == _uri.length,
            "uri array length should be equal to _totalNFT"
        );
  ...
  NFTV1Storage
{
  ...
  function BuyNFT(uint256[] memory _saleId) external payable nonReentrant {
    require(_saleId.length <= 50);
    uint256 totalAmount;
    for(uint256 i = 0; i < _saleId.length; i ++) {
      LibMeta.checkBuy(_tokenMeta[_saleId[i]]);
      totalAmount += _tokenMeta[_saleId[i]].price;
    }
    ...
    for(uint256 i = 0; i < _saleId.length; i ++) {
      ...
      LibMeta.executeBuy(_tokenMeta[_saleId[i]], PNDCAddress);

      ERC721(_tokenMeta[_saleId[i]].collectionAddress).safeTransferFrom(
        address(this),
        msg.sender,
        _tokenMeta[_saleId[i]].tokenId
      );
    }
  ...
  function sellNFT(
    ...
    LibMeta.checkSale(_collectionAddress, _tokenId, _price);
    // require(msg.sender == ERC721(_collectionAddress).ownerOf(_tokenId));
    ...
    for (uint256 i = 0; i < _price.length; i ++) {
      ...
      ERC721(_collectionAddress[i]).safeTransferFrom(
        msg.sender,
        address(this),
        _tokenId[i]
      );
...
library LibMeta {
   ...
   function transfer(TokenMeta storage token, address _to ) public{
    ...
    function checkSale(address[] memory tokenAddresses, uint256[] memory tokenIds, uint256[] memory prices) external view {
        require(tokenAddresses.length <= 50);
        require(tokenAddresses.length == tokenIds.length);
        require(tokenIds.length == prices.length);
        for (uint256 i = 0; i < prices.length; i ++) {
            require(IERC721(tokenAddresses[i]).ownerOf(tokenIds[i]) == msg.sender);
        }
    }

    function checkBuy(TokenMeta storage meta) external view {
        require(meta.status);
        require(msg.sender != address(0) && msg.sender != meta.currentOwner);
        require(!meta.bidSale);
    }

    function executeBuy(TokenMeta storage meta, address PNDCAddress) external {
        address currentOwner = meta.currentOwner;
        ...
        (bool isSuccess, ) = payable(currentOwner).call{ value: (sum) }("");
