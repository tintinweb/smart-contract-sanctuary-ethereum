...
contract TheArrivalsNft is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() payable ERC721A('TheArrivalsNFT', 'TAN') {
        maxSupply = 6666;
        maxFree = 10;
        claimedSupply = 0;
  ...
  function ownerMint(uint256 quantity) external payable {
      require(claimedSupply + quantity <= maxSupply, "SOLD OUT");
      require(msg.sender == owner(), "You're not allowed for owner mint");
      require(quantity <= buyLimit, "Exceeds max per transaction."); 
      _mint(msg.sender, quantity); 
      claimedSupply += quantity;
  } 

   function publicMint(uint256 quantity) external payable {
        require(claimedSupply + quantity <= maxSupply, "SOLD OUT");
        require(isMintEnabled,"Sale not active");
        require(walletMints[msg.sender] + quantity <= maxPerWallet, 'Exceeds max per wallet');
        if (claimedSupply >= maxFree) {
        ...
        require(quantity <= freeLimit, "Exceeds max per transaction."); 
         _mint(msg.sender, quantity);   
         claimedSupply += quantity;
        } 
    }
    ...
    function walletOfOwner(address _owner) public view returns 
       (uint256[] memory) {
