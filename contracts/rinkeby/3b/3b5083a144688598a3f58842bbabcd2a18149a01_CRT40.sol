...
contract CRT40 is IERC20, Auth {
    ...
    uint256 maxTx = 1 * 10**6 * (10 ** _decimals);
    uint256 public maxTxAmountId = maxTx / (10 ** _decimals); //Id for "Intelligible display"
    uint256 maxWallet = 2 * 10**6 * (10 ** _decimals);
    uint256 public maxWalletId = maxWallet / (10 ** _decimals); //Id for "Intelligible display"
 ...
 */
    ...
    function changeTxLimit(uint256 newLimit) external onlyOwner {
        maxTx = newLimit * (10 * _decimals);
    	maxTxAmountId = newLimit;
    }

    function changeWalletLimit(uint256 newLimit) external onlyOwner {
        maxWallet  = newLimit * (10 * _decimals);
	    maxWalletId = newLimit;
    }
