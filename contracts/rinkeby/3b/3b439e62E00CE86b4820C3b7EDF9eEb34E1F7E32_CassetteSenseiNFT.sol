...
contract CassetteSenseiNFT is ERC721, Ownable, ERC721Pausable, ERC721Enumerable {
    ...
    struct Round {
        ...
        Counters.Counter roundTotalSupply;
    }
    ...
    constructor(
        ...
        rounds[0].roundTotalSupply.increment();
    ...
    function startRound(
        ...
        require(
            (_totalSupplyTracker.current().add(_maxSupply)) <= MAX_SUPPLY,
            "Max supply exceeded"
        );
    ...
    function getRoundInfo(uint256 id) public view returns (string memory) {
        ...
        string memory output = string(
            abi.encodePacked(
                ...
                ",",
                Strings.toString(r.roundTotalSupply.current()),
                ",",
    ...
    function collabMint(uint256 qty) external onlyOwner {
        ...
        require(
            (rounds[rid].roundTotalSupply.current()) > rounds[rid].maxSupply,
        ...
        for (uint256 i = 0; i < qty; i++) {
            ...
            rounds[rid].roundTotalSupply.increment();
            _totalSupplyTracker.increment();
        }
    }
            ...
            Buffer.append(
                ...
                string(
                    abi.encodePacked(
                        ...
                        '", "description": "CASSETTE SENSEI NFT Project is unique 888 collectible NFTs.", "image": "',
                        boxImage,
                        '", "animation_url": "',
                        boxAnimation,
    ...
    function redeemPublic() external payable whenNotPaused {
        ...
        require((_totalSupplyTracker.current()) <= MAX_SUPPLY, "Sold out");
        require((rounds[rid].roundTotalSupply.current()) <= rounds[rid].maxSupply, "Sold out");
        require(_price <= msg.value, "You must send exactly the price of the NFT.");

        _roundIdOfToken[_totalSupplyTracker.current()] = rid;
        _tokenIdAtRound[_totalSupplyTracker.current()] = rounds[rid].roundTotalSupply.current();
    ...
    function mintPrivate(uint256 qty) external onlyOwner {
        ...
        for (uint256 i = 0; i < qty; i++) {
            uint256 id = _roundIdTracker.current();
            require((_totalSupplyTracker.current()) <= MAX_SUPPLY, "Sold out");
            require((rounds[id].roundTotalSupply.current()) <= rounds[id].maxSupply, "Sold out");
            _roundIdOfToken[_totalSupplyTracker.current()] = id;
            _safeMint(msg.sender, _totalSupplyTracker.current());
            _tokenIdAtRound[_totalSupplyTracker.current()] = rounds[id].roundTotalSupply.current();
            _totalSupplyTracker.increment();
            rounds[id].roundTotalSupply.increment();
    ...
    function getCurrentRoundTotalSupply() public view returns (uint256) {
        uint256 rid = _roundIdTracker.current();
        return rounds[rid].roundTotalSupply.current() - 1;
    }
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    ",",
                    Strings.toString(getCurrentRoundTotalSupply()),
                    ",",
