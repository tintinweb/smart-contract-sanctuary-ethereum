...
contract Jews is ERC721URIStorage, Ownable {
	...
	uint256 public creatorFeesPercent = 10;
	uint256 public maxPerWallet = 1;
	uint256 public maxSupply = 5555;
	string public baseUri = 'https://jewjew.thejews.xyz/metadata';
	...
	function mint() external {
		...
		require(_minters[to] + 1 < maxPerWallet, 'Reached max count of free mint');
	...
	function setAllowList(address _address) public onlyOwner {
		_allowList[_address] = 1;
	}

	function massSetWhitelist(address[] calldata _addressArray) public onlyOwner {
		uint i = 0;
    	while (i < _addressArray.length) {
			_allowList[_addressArray[i]] = 1;
			i++;
		}
  	}
