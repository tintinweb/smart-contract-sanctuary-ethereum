...
contract CassetteSenseiNFT is ERC721, Ownable, ERC721Pausable, ERC721Enumerable {
    ...
    event RevealCollection(uint256 roundId, string notRevealedURI, string baseURI);
    event StartRound(
        uint256 roundId,
        uint256 maxSupply,
        uint256 exclusiveSaleStartTime,
    ...
    function revealCollection() external onlyOwner {
        ...
        emit RevealCollection(id, rounds[id].notRevealedURI, rounds[id].baseURI);
    }
    ...
    function getRoundInfo(uint256 id) public view returns (string memory) {
        ...
        string memory output = string(
            abi.encodePacked(
                ...
                ",",
                Strings.toString(r._tokenIdTracker.current()),
                ",",
    ...
    function redeem(uint256 qty, bytes32[] calldata proof) external payable whenNotPaused {
        ...
        uint256 id = _roundIdTracker.current();
        uint256 _price = getPrice();
        require((id + 1) < MAX_SUPPLY, "Sold out");
        require((rounds[id]._tokenIdTracker.current() + 1) <= rounds[id].maxSupply, "Sold out");
