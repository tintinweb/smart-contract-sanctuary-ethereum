...
contract Blackhole is IERC721Receiver, ReentrancyGuard, Ownable {
    ...
    event EnteredBlackhole(address indexed staker, uint256 tokenId, uint256 blockNumber);
    event EscapedBlackhole(address indexed staker, uint256 tokenId, uint256 blockNumber);
    ...
    function _unStakeNFT(uint256 tokenId) internal onlyStaker(tokenId) requireTimeElapsed(tokenId) returns (bool) {
        ...
        // return token
        spaceBatsUtilityToken.safeTransferFrom(address(this), msg.sender, tokenId);

        emit EscapedBlackhole(msg.sender, tokenId, block.number);
    ...
    function _stakeNFT(uint256 tokenId) internal returns (bool) {
        ...
        spaceBatsUtilityToken.safeTransferFrom(msg.sender, address(this), tokenId);
        ...
        emit EnteredBlackhole(msg.sender, tokenId, block.number);
    ...
    function _payoutStake(uint256 tokenId) internal {
        ...
        space.transferController(receipt[tokenId].owner, payout);
        // spaceToken.transfer(receipt[tokenId].owner, payout);
