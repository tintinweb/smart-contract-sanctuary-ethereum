...
contract ProofNFT is ERC721A("Proof NFT", "PFN") {
    ...
    // external contract
    address AssetStoreInterfaceAddress = 0x847A044aF5225f994C60f43e8cF74d20F756187C;
    IAssetStore AssetStoreContract = IAssetStore(AssetStoreInterfaceAddress);
    ...
    function getStrLen(string memory _str) internal pure returns(uint) {
    ...
    function mint(string[] calldata _message) external {
       ...
       for(uint i =0;i < messageLength;i++){
            require(getStrLen(_message[i]) <= 8,"message count over");
    ...
    string constant TextPart_1_1 = '<text x="100" y="550" font-family="Arial" font-size="100" font-weight="bold">';
    string constant TextPart_1_2 = '<text x="100" y="450" font-family="Arial" font-size="100" font-weight="bold">';
    string constant TextPart_2_2 = '<text x="100" y="650" font-family="Arial" font-size="100" font-weight="bold">';
    string constant TextPart_1_3 = '<text x="100" y="350" font-family="Arial" font-size="100" font-weight="bold">';
    string constant TextPart_2_3 = '<text x="100" y="550" font-family="Arial" font-size="100" font-weight="bold">';
    string constant TextPart_3_3 = '<text x="100" y="750" font-family="Arial" font-size="100" font-weight="bold">';
    ...
    function generateSVG(ProofInfo memory assetinfo) public view returns (string memory) {
        bytes memory _svgPart = bytes(AssetStoreContract.generateSVGPart(101, "asset101")); // get Material Icons

        bytes memory _textPart = "";
        bytes memory _textPart_temp = "";
        if(assetinfo.message.length == 1){
                _textPart_temp = abi.encodePacked(  
                TextPart_1_1, assetinfo.message[0], TextPart_End
            );
        }else if(assetinfo.message.length == 2){
                _textPart_temp = abi.encodePacked(  
                TextPart_1_2, assetinfo.message[0], TextPart_End,
                TextPart_2_2, assetinfo.message[1], TextPart_End
            );
        }else{  // 3
                _textPart_temp = abi.encodePacked(  
                TextPart_1_3, assetinfo.message[0], TextPart_End,
                TextPart_2_3, assetinfo.message[1], TextPart_End,
                TextPart_3_3, assetinfo.message[2], TextPart_End
            );
        }
        _textPart =  abi.encodePacked(_textPart_temp,TextPart_Hooter, assetinfo.blocknumber, ' : ', assetinfo.soulbound ,TextPart_End);
    ...
    function _generateTraits(ProofInfo memory _attr) internal pure returns (bytes memory) {
        uint messageLength = _attr.message.length;
    ...
    function proofOfToken(uint256 _tokenId) public view returns(ProofInfo memory ) {
        require(_exists(_tokenId), 'ProofNFT.assetIdOfToken: nonexistent token');
        return proofs[_tokenId];
    }

    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        require(_exists(_tokenId), 'ProofNFT.tokenURI: nonexistent token');
