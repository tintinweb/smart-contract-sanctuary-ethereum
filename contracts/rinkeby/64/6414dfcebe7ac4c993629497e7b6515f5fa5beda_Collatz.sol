...
contract Collatz is ERC1155Supply, Ownable {
    ...
    /// @notice Current number being minted. In storage to avoid recursively passing in calldata.
    uint256 currMint;
    ...
    function mint(uint256 n, bytes memory data) public {
        // check that n is available and valid
        require(!exists(n), "ERC1155: token already minted");
        ...
        returns (CollatzNumber memory)
    {
        ...
        while (curr > 1) {
            // TODO: use bitshift
            curr = (curr.mod(2) == 0) ? curr.div(2) : curr.mul(3).add(1);
        ...
        return
            CollatzNumber({
                ...
                max: max,
                odds: odds,
                next: (n.mod(2) == 0) ? n.div(2) : n.mul(3).add(1),
