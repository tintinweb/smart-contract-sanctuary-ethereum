...
/// @notice Contract module which provides a basic access control mechanism, where there is an account (an owner) that can be granted exclusive access to specific functions.
import "@openzeppelin/contracts/access/Ownable.sol";
/// @notice Implementation of https://eips.ethereum.org/EIPS/eip-721[ERC721] Non-Fungible Token Standard, including the Metadata extension,
...
/// @dev Sweeping declaration follows ERC721 standard, as other NFTs, however certain functionalities from ERC721 token standard will be blocked, such as transfer of tokens (transfer of declarations).
contract sweepingDeclaration is ERC721, Ownable {
    ...
    /// @notice Variable which will store the number of total confirmers (unique addresses) which have made a confirmation.
    /// @dev The variable will be set to 0 in the constructor and incremented each time a new confirmer (new address) makes a confirmation.
    ...
    /// @notice Variable which will store the number of total deniers (unique addresses) which have made a confirmation.
    ...
    /// @notice Variable which will store the limit for a declaration.
    /// @dev The variable will be set to 50, which will be the threshold for total byte-length of the declaration.
    ...
    /// @notice Mapping which will keep track of the declarations of declarers.
    mapping (address => uint256) public declarerDeclarations;

    /// @notice Struct which will store data regarding declarations such as the declaration ID, number of confirmations, the declarer, the declaration, the time of the declaration and the byte length in the declaration.
    struct declaration {

        /// @notice Variable which will store the declaration identifier.
        /// @dev The variable will store the token ID of the ERC721 token.
    ...
    /// @notice Struct which will store data regarding confirmations, such as the confirmer and the confirmed declaration.
    struct confirmation {
    ...
    /// @notice Struct which will store data regarding denies, such as the denier and the denied declaration.
    struct deny {
    ...
    /// @notice Constructor which will be executed only once upon deployment. The constructor will set ERC721 name and symbol, as well as setting numberOfDeclarations and numberOfDeclarers to 0.
    constructor() ERC721("Sweeping declaration", "SDEC") {
    ...
    /// @dev Parameter _declarer is the address calling the function.
    /// @dev Parameter _declaration is the declaration.
    function makeDeclaration( address _declarer, string memory _declaration ) public {

        /// @notice Check to make sure that the address making the declaration is the same as the address passed in the function.
        ...
        /// @notice Increase number of declarations by 1.
        numberOfDeclarations++;

        /// @notice Increase number of declarers if declarer of declaration has no previous declaration.
        ...
        /// @dev _declaration = delcaration
        /// @dev block.timestamp = timeOfDeclaration
        /// @dev uint8(bytes_declaration).length) = byteLength
        declarations.push(declaration( numberOfDeclarations, 0, _declarer, _declaration, block.timestamp, uint8(bytes(_declaration).length) ));

        /// @notice Incrementing the number of declarations for the declarer.
        declarerDeclarations[_declarer]++;

        /// @notice Making a declaration through ERC721 minting function.
        _safeMint( msg.sender, numberOfDeclarations );

    }


    /// @notice Function which will let a confirmer confirm a declaration.
    /// @dev Parameter _confirmer is the address calling the function.
    /// @dev Parameter _declarationID is the ID of the declaration.
    function makeConfirmation( address _confirmer, uint256 _declarationID ) public {

        /// @notice Check to make sure that the address making the confirmation is the same as the address passed in the function.
        ...
        confirmations.push(confirmation( _confirmer, _declarationID ));
    ...
    /// @dev Parameter _denier is the address calling the function.
    /// @dev Parameter _declarationID is the ID of the declaration.
    function makeDeny( address _denier, uint256 _declarationID ) public {

        /// @notice Check to make sure that the address making the deny is the same as the address passed in the function.
        ...
        denies.push(deny( _denier, _declarationID ));
