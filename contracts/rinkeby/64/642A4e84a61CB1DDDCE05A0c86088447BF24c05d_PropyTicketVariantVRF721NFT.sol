...
/**
 * @title PropyTicketVariantVRF721NFT
 ...
 */
contract PropyTicketVariantVRF721NFT is ERC721, Ownable {
  ...
  // Additional init function due to not all args fitting into constructor without compiler errors
  function initialize(
    address _paymentERC20,
    address _royaltyReceiver,
    uint16 _royaltyBasisPoints
  ) external onlyOwner {
    require(isInitialized == false, "ALREADY_INITIALIZED");
    paymentERC20 = IERC20(_paymentERC20);
  ...
  function buy(uint256 quantity) external {
    require(isInitialized == true, "NOT_INIT");
    ...
    // Make payment
    bool success = paymentERC20.transferFrom(msg.sender, address(this), price * quantity);
    require(success == true, "PAYMENT_FAILED");
  ...
  function paymentERC20Address() public view returns (address) {
    return address(paymentERC20);
  }
  ...
  function distributeFees() public onlyOwner {
    ...
    require(balance > 0, "NO_PENDING_PAYOUT_BALANCE");
    for(uint256 i = 0; i < payoutAddresses.length; i++) {
        ...
        feeCutsTotal += feeCut;
        bool feeCutDeliverySuccess = paymentERC20.transfer(payoutAddresses[i], feeCut);
        require(feeCutDeliverySuccess, "FEE_CUT_DELIVERY_FAILURE");
    }
  }
  ...
  function updateFeePayoutScheme(
      ...
      require(_payoutAddresses.length > 0, "PAYOUT_ADDRESSES_EMPTY");
      require(_payoutAddresses.length == _payoutAddressBasisPoints.length, "PAYOUT_ADDRESSES_BASIS_POINTS_MISMATCH");
      uint256 totalBasisPoints;
      for(uint256 i = 0; i < _payoutAddresses.length; i++) {
          require((_payoutAddressBasisPoints[i] > 0) && (_payoutAddressBasisPoints[i] <= 10000), "BASIS_POINTS_NOT_BETWEEN_0_AND_100");
          totalBasisPoints += _payoutAddressBasisPoints[i];
      }
      require(totalBasisPoints == 10000, "BASIS_POINT_TOTAL_NOT_100");
      payoutAddresses = _payoutAddresses;
      payoutAddressBasisPoints = _payoutAddressBasisPoints;
