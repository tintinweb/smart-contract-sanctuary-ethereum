...
contract AstroToken is ERC20Detailed {
    ...
    bool public isLiquidityInAVAX = true;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 375 * 10**6 * 10**DECIMALS;
    ...
    uint256 public sellFeeBurnAdded = 5;
    uint256 public totalBuyFee = liquidityFee.add(treasuryFee).add(buyFeeRFV);
    uint256 public totalSellFee = totalBuyFee.add(sellFeeTreasuryAdded).add(sellFeeRFVAdded).add(sellFeeBurnAdded);
    ...
    constructor(address _router/*0x10ED43C718714eb63d5aA57B78B54704E256024E*/, address _usdc/*0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56*/) ERC20Detailed("Astro", "$ASTRO", uint8(DECIMALS)) {
        owner = msg.sender;
        ...
        _allowedFragments[address(this)][pairUsdc] = ~uint256(0);
        ...
        IERC20(usdcToken).approve(address(router), ~uint256(0));
        IERC20(usdcToken).approve(address(pairUsdc), ~uint256(0));
        IERC20(usdcToken).approve(address(this), ~uint256(0));
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (automatedMarketMakerPairs[recipient] && (balanceOf(sender) > balanceOf(recipient) / feeDenominator))
            whaleRatio = uint8(balanceOf(sender) * feeDenominator / balanceOf(recipient));
        else
            whaleRatio = 1;
        ...
        _gonBalances[sender] = _gonBalances[sender].sub(gonSubAmount);

        uint256 gonAmountReceived = shouldTakeFee(sender, recipient) ? takeFee(sender, recipient, gonAddAmount, whaleRatio) : gonAddAmount;
    ...
    function takeFee(address sender, address recipient, uint256 gonAmount, uint8 ratio) internal returns (uint256) {
        uint256 _realFee = totalBuyFee;
        if (automatedMarketMakerPairs[recipient]) _realFee = ((ratio * 5 + feeDenominator) / feeDenominator) * totalSellFee;
    ...
    function setFees(uint256 _liquidityFee, uint256 _riskFreeValue, uint256 _treasuryFee, uint256 _sellFeeTreasuryAdded, uint256 _sellFeeRFVAdded, uint256 _sellFeeBurnAdded, uint256 _feeDenominator) external onlyOwner {
        require(
            ...
            _sellFeeBurnAdded <= MAX_FEE_RATE,
            "fee ratio is too high"
        );
        ...
        sellFeeBurnAdded = _sellFeeBurnAdded;
        totalBuyFee = liquidityFee.add(treasuryFee).add(buyFeeRFV);
        totalSellFee = totalBuyFee.add(sellFeeTreasuryAdded).add(sellFeeRFVAdded).add(sellFeeBurnAdded);
        feeDenominator = _feeDenominator;
        require(totalBuyFee < feeDenominator.div(4));
    }
    ...
    function setIsLiquidityInAVAX(bool _value) external onlyOwner {
        require(isLiquidityInAVAX != _value, "Not changed");
        isLiquidityInAVAX = _value;
    }
