...
contract BleuplatineERC721A is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private constant MAX_WHITELIST = 15;
    uint private constant MAX_PUBLIC = 25;
    ...
    uint public saleStartTime = 1648936800;
    ...
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(isWhtelisted(msg.sender, _proof), "Not whitelisted");
        require(amountNFTperWalletWhitelistSale[msg.sender] + _quantity <= maxPerAddressDuringWhitelistMint, "You can get 1 NFT on the Whitelist Sale");
    ...
    function publicMint(address _account, uint _quantity) external payable callerIsUser {
        ...
        require(amountNFTperWalletPublicSale[msg.sender] + _quantity <= maxPerAddressDuringPublicMint, "You can get 3 NFT on the Public Sale");
    ...
    /// @return string The token URI of an NFT by his ID
    function tokenURI(uint _tokenId) public view virtual override returns(string memory) {
        require(_exists(_tokenId), "URI query for nonexistent token");
        return string(abi.encodePacked(baseURI, _tokenId.toString(), ".json"));
    }

    /// @notice Set the whitelist sale price
    ...
    /// @param _saleStartTime The new starting time (timestamp) of the whitelist sale
    function setSaleStartTime(uint _saleStartTime) external onlyOwner {
        saleStartTime = _saleStartTime;
    }

    /// @notice Get the current timestamp
    /// @return uint the current timestamp
    ...
    /// @param _account The account to be check
    /// @param _proof The merkle proof
    /// @return true if the address is whitelisted, false otherwise 
    function isWhtelisted(address _account, bytes32[] calldata _proof) internal view returns(bool) {
