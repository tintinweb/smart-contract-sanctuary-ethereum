...
/**
 * @dev Using revert and error for better gas savings
 */
...
contract Coinage is Ownable, ERC1155Supply, ReentrancyGuard {
    ...
    address private signerAddress = 0xf4e92382bd7E260ce55A9a18bf4Ad05f05f319E8;
    ...
    function networkMint(
        ...
        bytes32 msgHash = keccak256(abi.encodePacked(msg.sender, referralCode));
        if (hash != msgHash) {
            revert HashError();
        }
        if (msg.value != Network.price) revert IncorrectFunds();
    ...
    function caucusMint(bytes32 hash, bytes memory signature) external payable {
        ...
        if (msg.value != Caucus.price) revert IncorrectFunds();
        mint(msg.sender, 1, Caucus.id);
    }

    function setSignerWaller(address _signerWalletAddress) external onlyOwner {
