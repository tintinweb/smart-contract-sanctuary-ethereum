    ...
    IERC721ReceiverUpgradeable
{
    ...
    //Bid Strcut Hash
    bytes32 private constant _BID_TYPEHASH = 0x0539ae919312b6df6ee803473ef647a28045105ddae7e33738d7e063f4181776;
        ...
        onlyApproved(order.contractAddress, order.tokenId, order.seller)
    {
        ...
        settlement(nftContract, _tokenId, amt, taker, _order);
    ...
    function _invalidateSignedOrder(
        ...
        isAdmin
            ? require(
                !isMinted || address(this) == IERC721Upgradeable(contractAddress).ownerOf(_tokenId),
                "not minted or not an admin"
            )
            : require(
    ...
    function _adminMint(
        ...
        require(adminContracts[contractAddress], "not a admin contract");
        IToken token = IToken(contractAddress);
    ...
    function _splitFee(
        ...
        uint256 _amount,
        address _erc20Token
    ) internal isNonZero(_amount) returns (uint256) {
        bool isToken = _erc20Token != address(0);
        uint256 amount = _amount;
        for (uint256 i; i < feeSplits.length; i++) {
    ...
    function _chargeAndSplit(
        ...
        uint256 platformFee = _splitFee(user, _amount, _erc20Token);
        IWETH weth = IWETH(WETHAddress);
        if (_erc20Token == address(0)) {
            ...
            weth.transferFrom(user, address(this), (lpSplit + stakingSplit));
        }
    ...
    function verifySeller(Order calldata order) internal view returns (bool) {
        ...
        return admins[signer] ? admins[msg.sender] : signer == msg.sender;
    }
    ...
    function validateOrder(
        ...
        (, bytes32 s, uint8 v) = splitSignature(sig);
        ...
        if (admins[signer] ? address(this) == order.seller : signer == order.seller) {
            return true;
        }
    }
