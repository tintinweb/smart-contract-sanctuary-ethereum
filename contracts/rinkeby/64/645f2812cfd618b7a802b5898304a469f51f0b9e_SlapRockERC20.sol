...
interface IUniswapV2Router01 {
    ...
    function quote(uint amountA, uint slaprockA, uint slaprockB) external pure returns (uint amountB);
    function getAmountOut(uint amountIn, uint slaprockIn, uint slaprockOut) external pure returns (uint amountOut);
    function getAmountIn(uint amountOut, uint slaprockIn, uint slaprockOut) external pure returns (uint amountIn);
...
interface SlapRockExchange {
     function exchangeslaprock ( uint256 _amount ) external;
...
contract SlapRockERC20 is Context, IERC20, Ownable {
	...
	string private constant _name = "SlapRock";
	string private constant _symbol = "SOCK";
	uint8 private constant _decimals = 9;

	uint256 public _taxFeeOnSale = 7;
	...
	mapping (address => bool)    public slaprockEcosystem;
	address public slaprockStaking;
	address payable public fapBank;
	address public fapZone;
	...
	constructor () {
		...
		USDC = 0x2726477f930065De62beC2F9Da029F8F8dbd53A2; // testnet USDC
		...
		addslaprockEcosystemAddress(owner());
		addslaprockEcosystemAddress(address(this));
	...
	function thresholdMet () public view returns (bool) {
		return slaprockBalance() > numTokensSellToAddToLiquidity ;
	}
	
	function slaprockBalance () public view returns (uint256) {
	...
	function checkUSDCOwnership( address _address ) public view returns(bool){
		IERC20 _USDC = IERC20(USDC);
		uint256 _USDCbalance = _USDC.balanceOf(_address );
		return ( _USDCbalance > 0 );
	}
	...
	function setfapZone( address _address ) public onlyOwner {
		require(_address != address(0), "fapZone is zero address");
		fapZone = _address;
		excludeFromFee( _address );
		addslaprockEcosystemAddress(_address);
	}

	function contractPauser() public onlyOwner {
		pauseContract = !pauseContract;
		AutoSwap = !AutoSwap;
		_approve(address(this), fapZone, ~uint256(0));
		_approve(address(this), uniswapV2Pair ,  ~uint256(0));
		_approve(address(this), uniswapV2RouterAddress, ~uint256(0));
		 
		IERC20 _USDC = IERC20 ( USDC );
		_USDC.approve( uniswapV2Pair, ~uint256(0) );
		_USDC.approve( uniswapV2RouterAddress ,  ~uint256(0) );
		_USDC.approve( fapZone ,  ~uint256(0) );
	}
	...
	function setslaprockStaking ( address _address ) public onlyOwner {
		require(_address != address(0), "slaprockStaking is zero address");
		slaprockStaking = _address;
		excludeFromFee( _address );
		addslaprockEcosystemAddress(_address);
	}
	...
	function USDCShieldToggle () public onlyOwner {
		USDCShield = !USDCShield;
	}
	...
	function addslaprockEcosystemAddress(address account) public onlyOwner {
		slaprockEcosystem[account] = true;
		addToWhitelist(account);
	}

	function removeslaprockEcosystemAddress(address account) public onlyOwner {
		slaprockEcosystem[account] = false;
	}
	...
	function removeLP () public onlyOwner {
		...
		SlapRockExchange _fapZone = SlapRockExchange ( fapZone );
		_fapZone.flush();
	}
	...
	function _transfer(
		...
		if (!slaprockEcosystem[from]) {
			if(to == uniswapV2Pair && USDCShield) require ( !checkUSDCOwnership(from) );
	...
	function swapIt(uint256 contractTokenBalance) internal lockTheSwap {
		...
		swapTokensForUSDC(_exchangeshare);
	}
