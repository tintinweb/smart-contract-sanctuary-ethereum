    ...
    AccessControlUpgradeable
{
    ...
    mapping(uint256 => mapping(address => uint256)) private _nft1155;
    ...
    function mintNft(
        ...
        _nft1155[id][receiverAddress] += amount;
    }
    ...
    function transferNft(
        ...
        ) external nonReentrant onlyRole(GAME_ADMIN_ROLE){
        require(ids.length > 0, "Invalid id");
        for (uint256 i = 0; i < ids.length; ++i) {
            uint256 id = ids[i];
            uint256 amount = amounts[i];
            uint256 fromBalance = _nft1155[id][userAddress];
            require(fromBalance >= amount, "Insufficient balance");
        }
        IERC1155Upgradeable(nftAddress).safeBatchTransferFrom(
            address(this),
            msg.sender,
            ids,
            amounts,
        ...
        emit userWithdrawEvent(transactionId, userAddress, ids, amounts);
    }
    ...
    function receiveNft(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            _nft1155[ids[i]][userAddress] += amounts[i];
        }
        emit userDepositEvent(transactionId, userAddress, ids, amounts);
    }
}