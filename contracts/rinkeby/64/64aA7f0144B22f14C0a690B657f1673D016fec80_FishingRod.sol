...
import "./FishingRodMetadataBuilder.sol";
import "../Property.sol";
import "../../Random.sol";
import "./TraitSet.sol";
...
contract FishingRod is Property {
    ...
    FishingRodMetadataBuilder private _metadataBuilder;
    ...
    function initialize(Tier[5] calldata tiers) public initializer {
        __Property_init("Fishing Rod", "FISHROD", tiers);
    }

    function setMetadataBuilder(address address_) external onlyOwner {
        _metadataBuilder = FishingRodMetadataBuilder(address_);
    }
    ...
    function upgrade(uint256 tokenId, uint256 tier) public override {
        super.upgrade(tokenId, tier);
...
abstract contract FishingRodMetadataBuilder is Ownable {
    enum TraitType {
        Handle,
        Material,
        Outline
    ...
    address private _artwork;

    function build(uint256 tokenId, TraitSet calldata traits) external view virtual returns (string memory);
}
    ...
    UUPSUpgradeable
{
    ...
    modifier noContract() {
        ...
        if (account.isContract() || account != tx.origin) {
    ...
    function mint(address to) external whenNotPaused noContract {
        Tier memory tier = _tiers[1];

        if (_msgSender() != owner()) {
            if (_booty.balanceOf(_msgSender()) < tier.mintPrice) {
                revert InsufficientFunds();
            }

            _booty.burnFrom(_msgSender(), tier.mintPrice);
        }
    ...
    function remainingSupply(uint256 tier) external view returns (uint256) {
        if (tier < 3 || tier > 5) {
            revert OutOfRange(3, 5);
        }

        return _tiers[tier].remainingSupply;
    }
    ...
    function upgrade(uint256 tokenId, uint256 tier) public virtual whenNotPaused noContract {
        ...
        if (tier >= 3 && _tiers[tier].remainingSupply == 0) {
            revert SoldOut();
        }

        if (_msgSender() != owner()) {
            if (_booty.balanceOf(_msgSender()) < _tiers[tier].mintPrice) {
                revert InsufficientFunds();
            }

            _booty.burnFrom(_msgSender(), _tiers[tier].mintPrice);
        }

        if (tier >= 3) {
            _tiers[tier].remainingSupply--;
        }
    }
...
error ContractAccountNotAllowed();
error InsufficientFunds();
error InvalidProof();
error InvalidTraitType();
error MintingNotLaunched();
error NotFound();
error OutOfRange(uint256 min, uint256 max);
error SoldOut();
error Unauthorized();
error WhitelistLimitExceeded();
