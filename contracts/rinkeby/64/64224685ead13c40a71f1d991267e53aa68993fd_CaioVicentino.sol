...
function getRandomAvailableTokenId(address payable to, uint256 updatedNumAvailableTokens)
 ...
 function getWhitelistedAddressIndex(address _user) public view returns (uint256) {
  ...
  function removeWhitelistedAddress(uint i) public onlyOwner {
  ...
  function mint(
    ...
    require(_mintAmount + balanceOf(endUser) <= maxMintAmount, "max mint amount per session exceeded");
    require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");

    // if(_deadLine > 0){
    //     require(_deadLine > block.timestamp, "this mint is already closed");
    // }

    if(onlyWhitelisted){
         //assert(isWhitelisted(endUser), "mint open only to whitelisted addresses");
         assert(isWhitelisted(endUser));
    }
