...
IFeatures1 features1;
    ...
    uint256 nextTokenId = 100;
    // uint256 MAX_MINT_ETHEREUM = 3084;
    uint256 MAX_MINT_ETHEREUM = 200;
...
for (uint256 ii=0; ii < 8; ii++) {
  ...
  if ((workingByte >> (7 - ii)) & 1 == 1) {
          rects.appendSafe(abi.encodePacked('<rect width="50" height="50" x="',toString((x*50) + (ii*50)),'" y="',
              toString(y*50),'" fill="',svgBackgroundColor2[colors[0]]));//'" fill="', svgBackgroundColor2[colorByte]));
  } else {
        if ((colorByte >> (7 - ii)) & 1 == 1) {
          rects.appendSafe(abi.encodePacked('<rect width="50" height="50" x="',toString((x*50) + (ii*50)),'" y="',
              toString(y*50),'" fill="',svgBackgroundColor2[colors[1]]));//'" fill="', svgBackgroundColor2[colorByte]));
        } else {

            if ((colorByte2 >> (7 - ii)) & 1 == 1) {
          rects.appendSafe(abi.encodePacked('<rect width="50" height="50" x="',toString((x*50) + (ii*50)),'" y="',
              toString(y*50),'" fill="',svgBackgroundColor2[colors[2]]));//'" fill="', svgBackgroundColor2[colorByte]));

            } else {
            rects.appendSafe(abi.encodePacked('<rect width="50" height="50" x="',toString((x*50) + (ii*50)),'" y="',
              toString(y*50),'" fill="',svgBackgroundColor2[colors[3]]));//'" fill="', svgBackgroundColor2[colorByte]));
            }
        }
  }
