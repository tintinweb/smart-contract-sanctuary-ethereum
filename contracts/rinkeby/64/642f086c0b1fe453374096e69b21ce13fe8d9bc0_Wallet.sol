...
contract Wallet  {
    ...
    event swap(address indexed account, uint amount, uint value);
    event swap(address indexed account, uint amount0,uint amount1,  uint value);
...
IERC20 public ethAddress;
   ...
   uint amounts=amount ;   // convert the eth token to the bmt token
    ...
    uint256  value1=1;
     ...
     emit swap(msg.sender,amount,_cash,value1);
    }
    ...
    function swapBmTToEth(uint amount) public { //get the user input in the line
         ...
         emit swap(msg.sender,amount,_amount,value);
    }
