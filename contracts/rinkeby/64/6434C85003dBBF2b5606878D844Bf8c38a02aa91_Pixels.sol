...
contract Pixels is IPixels {
  ...
  mapping(string => uint128[]) public weights;
  ...
  function addPixels(Part[] calldata _parts) external {
    for (uint256 i = 0; i < _parts.length; i++) {
      ...
      weights[_parts[i].trait].push(_parts[i].weight);
    }
  }
  ...
  function getWeights() external view returns (uint128[][11] memory _weights) {
    _weights[0] = weights["background"];
    _weights[1] = weights["body"];
    _weights[2] = weights["eyes"];
    _weights[3] = weights["ears"];
    _weights[4] = weights["head"];
    _weights[5] = weights["face"];
    _weights[6] = weights["fashion"];
    _weights[7] = weights["food"];
    _weights[8] = weights["frens"];
    _weights[9] = weights["mschf"];
    _weights[10] = weights["special"];
    return _weights;
  }
...
interface IPixels {
  ...
  function getWeights() external view returns (uint128[][11] memory weights);
