    ...
    ReentrancyGuard
{
    ...
    uint256 public immutable amountForIntuitives = 3;
    uint256 public immutable amountForPuzzles = 3;
    uint256 public salePrice = 0.083 ether;

    bool intuitivePoemsSet = false;
    bool puzzlePoemsSet = false;
    ...
    modifier callerIsTokenOwner(uint256 tokenId) {
        require(
            ownershipOf(tokenId).addr == msg.sender,
            "Not owner of token"
        );
        _;
    }
    ...
    modifier tokenInBounds(uint256 tokenId) {
        require(
            tokenId >= 0 && tokenId < collectionSize,
        ...
        allPoemsMinted
    {
        ...
        require(!intuitivePoemsSet, "Intuive Poems have already been set");
        for (uint256 i = 0; i < poems.length; i++) {
            setSpecialPoem(poems[i], PoemType.Intuitive);
        }
        intuitivePoemsSet = true;
    }

    function seedPuzzlePoems(uint256[] memory poems)
        external
        onlyOwner
        allPoemsMinted
    {
        require(poems.length == amountForPuzzles, "Not enough poems to set");
        require(!puzzlePoemsSet, "Puzzles have already been set");
        for (uint256 i = 0; i < poems.length; i++) {
            setSpecialPoem(poems[i], PoemType.Puzzle);
        }
        puzzlePoemsSet = true;
    }

    function setSpecialPoem(uint256 tokenId, PoemType poemType)
        private
        tokenInBounds(tokenId)
    {
        SpecialPoem storage poem = specialPoems[tokenId];
        require(poem.poemType == PoemType.None, "Poem is already set");
    ...
    {
        setSpecialPoemClaimed(tokenId, PoemType.Intuitive);
    }

    function claimPuzzlePoem(uint256 tokenId)
        external
        onlyOwner
        allPoemsMinted
        tokenInBounds(tokenId)
    {
        setSpecialPoemClaimed(tokenId, PoemType.Puzzle);
    }

    function setSpecialPoemClaimed(uint256 tokenId, PoemType poemType) private {
        SpecialPoem storage poem = specialPoems[tokenId];
        require(poem.poemType == poemType, "Poem is not correct type");
        ...
        returns (SpecialPoem memory poem)
    {
        ...
        require(poem.poemType != PoemType.None, "Poem is not special");
        return poem;
    }
