...
contract EpigeonNFT is NFTokenEnumerable, Ownable
  {
	...
	uint256 tokenizationPrice = 1000000000000000;
	...
	function mintingPrice() public view returns (uint256 _price){
		return tokenizationPrice;
	}
	
	function mintingTokenPrice(address ERC20Token) public view returns (uint256 _price){
		return approvedTokenPrice[ERC20Token];
	}
	
	function mintForEther(uint256 factory_id) public payable {
	    require(msg.value >= tokenizationPrice + epigeon.getPigeonPriceForFactory(factory_id), "Not enough value");		
	...
	function setMintingPrice(uint256 _mintingprice) public {
	    require(msg.sender == owner);
		tokenizationPrice = _mintingprice;
	}
	...
	function tokenizePigeon(address pigeon) public payable{
	    require(msg.value >= tokenizationPrice, "Not enough value");
