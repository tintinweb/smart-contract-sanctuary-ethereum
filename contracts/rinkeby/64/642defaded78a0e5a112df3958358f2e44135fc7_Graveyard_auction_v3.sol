...
contract Graveyard_auction_v3 is Initializable, ERC1155Upgradeable, OwnableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable { /* Contract inherits from ERC1155 spec */
    ...
    function distributeEndedAuctions(uint256 endedPrice) external onlyOwner {
        for(uint256 id = 0; id < auctionsCount; id++){
            if(block.timestamp > subauctions[id].end_time && !subauctions[id].ended){
