...
contract MetadataRetriever is Ownable {
    ...
    function get(Part part,string memory id) public view virtual returns (string memory svg){
        address source = 
    ...
    function smt(string memory tokenPart) public view virtual returns (string[] memory) {
     ...
     function tokenBaseURI(uint256 tokenId ,string memory tokenParts)
      public view virtual returns (string memory)
    {
        string[] memory smtPart = smt(tokenParts);

        string memory imageSvg = imageBase64(
            get(Part.body,smtPart[0]),
            get(Part.eye,smtPart[1]),
            get(Part.suit,smtPart[2]),
            get(Part.head,smtPart[3]),
            get(Part.mouth,smtPart[4])
            );
        
        bytes memory dataURI = abi.encodePacked(
            '{',
                '"name": "Golden #', tokenId.toString(), '",',
    ...
    function imageBase64(string memory body,string memory eye,string memory suit,string memory head,string memory mouth) internal view virtual returns (string memory) {
        bytes memory imageSvg = abi.encodePacked(
            ...
            '>',
                '<image x="0" y="0" width="50" height="50" image-rendering="pixelated" preserveAspectRatio="xMidYMid" xlink:href="data:image/png;base64,',body,'" />',
                '<image x="0" y="0" width="50" height="50" image-rendering="pixelated" preserveAspectRatio="xMidYMid" xlink:href="data:image/png;base64,',eye,'" />',
                '<image x="0" y="0" width="50" height="50" image-rendering="pixelated" preserveAspectRatio="xMidYMid" xlink:href="data:image/png;base64,',suit,'" />',
                '<image x="0" y="0" width="50" height="50" image-rendering="pixelated" preserveAspectRatio="xMidYMid" xlink:href="data:image/png;base64,',head,'" />',
                '<image x="0" y="0" width="50" height="50" image-rendering="pixelated" preserveAspectRatio="xMidYMid" xlink:href="data:image/png;base64,',mouth,'" />',
            '</svg>'
        );
