...
contract LavaFinance is ERC721Upgradeable, OwnableUpgradeable {

    event TierUpdated(uint indexed tier, uint indexed cost, uint indexed reward);
    event BoosterTypeUpdated(uint indexed level, uint cost, uint bonus);
    event BoosterBought(address indexed user, uint bonus, uint expiresAt);
    ...
    struct BoosterType {
        bool active;
        uint bonus; // 2 decimals. 105 = 1.05 times
        uint duration;
        uint remaining;
        uint cost;  // 6 decimals. usdc.e amount
        uint nodeCostVariable;  // 6 decimals. usdc.e amount
    }
    ...
    // Stores timestamp till which booster can be applied for a user
    struct OneTimeBooster {
        uint bonus; // 2 decimal multiplier. 105 = 1.05 times
    ...
    mapping(address => OneTimeBooster) public userOneTimeBooster;
    mapping(address => uint) public microNodes;
    mapping(address => uint) public Ratios;
    mapping(address => uint) public LPRatios;

    mapping(uint => Tier) public tiers;
    mapping(uint => BoosterType) public boosterTypes;
    ...
    function initialize(address _router, address _lavaToken, address _pLavaToken, address _gLavaToken, address _usdce, address _lpWallet, address _treasury) public initializer {
        ...
        __ERC721_init_unchained("LavaNodes", "LNODE");
        ...
        usdce = _usdce;
        Ratios[_lavaToken] = 1e4;
        Ratios[_pLavaToken] = 1e4;
        Ratios[_usdce] = 1e4;
    ...
    function setBoosterType(uint _level, uint _cost, uint _nodeCostVariable, uint _bonus, uint _duration, uint _remaining, bool _active) external onlyOwner {
        require(_cost > 0, "Cost 0");
        require(_bonus > 0, "Bonus 0");
        require(_duration > 0, "Duration 0");
        require(_remaining > 0, "Remaining 0");
        require(_level > 0, "Level 0");
        boosterTypes[_level] = BoosterType({
            cost: _cost,
            nodeCostVariable: _nodeCostVariable,
            bonus: _bonus,
            duration: _duration,
            remaining: _remaining,
            active: _active
        });
        emit BoosterTypeUpdated(_level, _cost, _bonus);
    }

    function setBoosterStatus(uint _level, bool _active) external onlyOwner {
        boosterTypes[_level].active = _active;
    }

    function increaseBoosterRemaining(uint _level, uint num) external onlyOwner {
        boosterTypes[_level].remaining += num;
    }

    function setTransferWhitelist(address _address, bool _status) external onlyOwner {
        transferWhitelist[_address] = _status;
    }
    ...
    function setNFTs(address _bronze, address _silver, address _gold) external onlyOwner {
        bronzeNFT = _bronze;
        silverNFT = _silver;
        goldNFT = _gold;
    }

    function addToken(address _token, uint ratio, address _feed) external onlyOwner {
        require(_token != lavaToken && _token != pLavaToken, "Lava ratio can't be changed");
        Ratios[_token] = ratio;
        if (_feed != address(0)) {
    ...
    function addLPToken(address _token, uint ratio) external onlyOwner {
        LPRatios[_token] = ratio;
    }

    function setNodeParameters(uint _maxNodesAllowed, uint _claimCooldown) external onlyOwner {
        require(_maxNodesAllowed >= maxNodesAllowed, "Too low");
        maxNodesAllowed = _maxNodesAllowed;
    ...
    function buyOneTimeBooster(uint _boosterLevel) external returns (uint usdceAmount) {
        BoosterType storage booster = boosterTypes[_boosterLevel];
        require(booster.bonus > minBoosterBonusAllowed(msg.sender), "Not Allowed");
        require(booster.remaining > 0, "Finished");
        require(booster.active, "Inactive");
        require(userOneTimeBooster[msg.sender].expiresAt <= block.timestamp, "Already present");
        usdceAmount = booster.cost + (booster.nodeCostVariable * balanceOf(msg.sender));
        uint expiresAt = block.timestamp + booster.duration;
        IERC20(usdce).transferFrom(msg.sender, treasury, usdceAmount);
        booster.remaining -= 1;
        userOneTimeBooster[msg.sender] = OneTimeBooster({
            bonus: booster.bonus,
            expiresAt: expiresAt
        });
        emit BoosterBought(msg.sender, booster.bonus, expiresAt);
    }

    function mint(uint tier, address token, string memory name) external returns (uint cost) {
        require(Ratios[token] != 0 || LPRatios[token] != 0, "Not accepted");
        require(tiers[tier].active, "No such Tier");
        require(userNodes[msg.sender].length < maxNodesAllowed, "Max node limit");
        uint nodePrice = tiers[tier].cost;
        if (LPRatios[token] != 0) {
            address[] memory path = new address[](2);
            address otherToken;
            if (Pair(token).token0() == lavaToken) {
                otherToken = Pair(token).token1();
            } else {
                require(Pair(token).token1() == lavaToken, "Wrong LP");
                otherToken = Pair(token).token0();
            }
            path[0] = otherToken;
            path[1] = lavaToken;
            uint[] memory amount = new uint[](2);
            amount = JoeRouter(traderJoeRouter).getAmountsIn(nodePrice, path);
            cost = (Pair(token).totalSupply() * amount[0] * LPRatios[token]) / (IERC20(otherToken).balanceOf(token) * 2 * 1e4);
            require(IERC20(token).transferFrom(msg.sender, treasury, cost), "Payment error");
        } else {
            if(token == lavaToken || token == pLavaToken ) {
                cost = (nodePrice * Ratios[token]) / 1e4;
                IERC20(token).transferFrom(msg.sender, address(this), cost);
            } else {
                uint tokenPrice = getNodePriceInToken(token, nodePrice);
                cost = (tokenPrice * Ratios[token]) / 1e4;
                require(IERC20(token).transferFrom(msg.sender, treasury, cost), "Payment error");
            }
        }
        ...
        if (token == lavaToken || token == pLavaToken) {
            uint lavaAmount = (nodePrice * Ratios[token]) / 1e4;
    ...
    function _mintNodeNoGLava(address user, uint tier, string memory name) internal {
        _safeMint(user, nextMinted);
        nodeData[nextMinted] = NodeData({
            name: name,
            tier: tier,
            lastClaim: block.timestamp,
            paymentExpiry: block.timestamp + 30 days
        });
        nextMinted++;
        emit NodeMinted(user, tier, block.timestamp);
    }

    function addMicroNode(address token, uint amount) external {
        require(token == lavaToken || token == pLavaToken, "Only Lava tokens");
        require(amount >= 10 ** lavaDecimals, "Amount too low");
        require(amount + microNodes[msg.sender] <= getMinNodePrice() + 1e16, "Make full node");
    ...
    function _addMicroNodeAmount(address user, uint amount) internal {
        ...
        uint minNodePrice = getMinNodePrice();
        while (totalAmount >= minNodePrice) {
            uint maxTier = getMaxTierBuyable(totalAmount);
            uint tierCost = tiers[maxTier].cost;
            uint numNodes = totalAmount / tierCost;
            for (uint i=0; i < numNodes; i++) {
                _mintNode(user, maxTier, "Lava Node");
            } 
            totalAmount -= numNodes * tiers[maxTier].cost;
        }
    ...
    function payMaintenanceFees(uint[] memory tokenIds, uint numMonths) external returns (uint amount) {
        for (uint i = 0; i < tokenIds.length; i++) {
            uint tokenId = tokenIds[i];
            Tier storage tier = tiers[nodeData[tokenId].tier];
            uint fees = tier.fees;
            require(ownerOf(tokenId) == msg.sender, "User not Owner");
            uint maintenanceAmount = fees * numMonths;
            ...
            nodeData[tokenIds[i]].paymentExpiry += numMonths * 30 days;
        }
        require(IERC20(usdce).transferFrom(msg.sender, treasury, amount), "Payment error");
    }

    // compoundPercent is 4 decimals. 1000 = 0.1 = 10%
    function claim(uint[] memory tokenIds, uint compoundPercent, bool claimMicro) external {
        uint amountToClaim = setClaimAmount(msg.sender, tokenIds, claimMicro);
    ...
    function setClaimAmount(address user, uint[] memory tokenIds, bool claimMicro) internal returns (uint amount) {
        for (uint i = 0; i < tokenIds.length; i++) {
            uint tokenId = tokenIds[i];
            (uint tokenAmount, uint claimTimestamp) = getTokenClaim(user, tokenId);
            amount += tokenAmount;
            if (claimTimestamp > 0) {
        ...
        (uint bonus, bool boosterUsed) = getBoosterBonus(user);
        amount = (amount * bonus) / 100;
        if (boosterUsed) {
            userOneTimeBooster[user].expiresAt = block.timestamp;
        }
    }

    function fuseNodes(uint[] memory tokenIds, uint newTier, string memory name) external {
    ...
    function setNodeName(uint tokenId, string memory name) public {
        require(ownerOf(tokenId) == msg.sender, "User not owner");
    ...
    function getNodePriceInUSD(uint nodePrice) public view returns (uint) {
        address[] memory path = new address[](2);
        path[0] = usdce;
        path[1] = lavaToken;
        uint[] memory amount = new uint[](2);
    ...
    function getNodePriceInToken(address token, uint nodePrice) public view returns (uint) {
        uint nodePriceInUSD = getNodePriceInUSD(nodePrice);
        uint tokenValueInUSD = variableAssetPriceInUSD(token);
        return (nodePriceInUSD * (10 ** IERC20(token).decimals())) / tokenValueInUSD;
    }

    function minBoosterBonusAllowed(address user) public view returns (uint) {
        if (IERC721(goldNFT).balanceOf(user) > 0) {
            return 115;
        } else if (IERC721(silverNFT).balanceOf(user) > 0) {
            return 110;
        } else if (IERC721(bronzeNFT).balanceOf(user) > 0) {
            return 105;
        }
        return 100;
    }

    function getClaimAmount(address user, uint[] memory tokenIds, bool claimMicro) public view returns (uint amount) {
        for (uint i = 0; i < tokenIds.length; i++) {
            uint tokenId = tokenIds[i];
            (uint tokenAmount, ) = getTokenClaim(user, tokenId);
            amount += tokenAmount;
        }
        if (claimMicro && microNodes[user] > 0) {
            amount += getMicroReward(user);
        ...
        (uint bonus, ) = getBoosterBonus(user);
        amount = (amount * bonus) / 100;
    }

    function getTokenClaim(address user, uint tokenId) public view returns (uint amount, uint claimTimestamp) {
    ...
    function getBoosterBonus(address user) public view returns (uint bonus, bool boosterUsed) {
        OneTimeBooster storage booster = userOneTimeBooster[user];
        uint boosterBonus = booster.expiresAt > block.timestamp ? booster.bonus : 100;
        uint nftBonus = 100;
        if (IERC721(goldNFT).balanceOf(user) > 0) {
            nftBonus = 115;
        } else if (IERC721(silverNFT).balanceOf(user) > 0) {
    ...
    function getMinNodePrice() public view returns (uint amount) {
        for (uint i = 1; i < 10; i++) {
    ...
    function getMaxTierBuyable(uint amount) public view returns (uint tier) {
        uint maxAmount;
        for (uint i = 1; i < 10; i++) {
    ...
    function _beforeTokenTransfer(address from, address to, uint tokenId) internal override {
        if (from != address(0)) {
            require(transferWhitelist[from] || to == address(0), "Not transferable");
