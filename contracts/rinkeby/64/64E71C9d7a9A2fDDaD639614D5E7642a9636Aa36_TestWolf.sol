...
contract TestWolf is ERC721A, ERC2981, Ownable, ReentrancyGuard {

  using Strings for uint256;

  uint256 public immutable _teamSupply;
...
modifier mintCompliance(uint256 _mintAmount) {
    ...
    require(_publicMinted() + _mintAmount <= maxSupply, 'Max supply exceeded!');
    _;
  }
  ...
  function developerMint(address to, uint32 amount) external onlyOwner {
    _teamMinted += amount;
    require(_teamMinted <= _teamSupply, "Developer supply: Exceed max supply");
    _safeMint(to, amount);
  }
