    ...
    AccessControlUpgradeable
{
        ...
        returns (bytes32)
    {
        ...
        uint256 orderId = marketOrdersController.createMarketOrder(
        ...
        returns (bytes32)
    {
        ...
        uint256 orderId = limitOrdersController.createOrder{value: msg.value}(
            ILimitOrdersController.OrderParams({
                fee: IUniswapV3Pool(_params.poolAddress).fee(),
        ...
        return keccak256(abi.encode(_params.poolAddress, positionCounter));
    }
    ...
    {
        require(_params.deadline >= block.timestamp, "Deadline");
        Position storage position = positions[_params.id];
        require(position.owner == _msgSender(), "Only owner can confirm");
        require(
            position.positionType == PositionType.OPENED_LONG_COMPLITE ||
                position.positionType == PositionType.OPENED_SHORT_COMPLITE,
            "Position must be opened"
        );
        address secondToken = position.collateralToken ==
            IUniswapV3Pool(position.poolAddress).token0()
            ? IUniswapV3Pool(position.poolAddress).token1()
            : IUniswapV3Pool(position.poolAddress).token0();
        IERC20Upgradeable(secondToken).approve(
            address(marketOrdersController),
            secondToken == IUniswapV3Pool(position.poolAddress).token0()
                ? position.openOrder.receivedAmountOfToken0
                : position.openOrder.receivedAmountOfToken1
        );
        uint256 orderId = marketOrdersController.createMarketOrder(
            IMarketOrdersController.CreateMarketOrderParams({
                receiver: address(this),
                ...
                fee: IUniswapV3Pool(position.poolAddress).fee(),
                amountIn: secondToken ==
                    IUniswapV3Pool(position.poolAddress).token0()
                    ? position.openOrder.receivedAmountOfToken0
                    : position.openOrder.receivedAmountOfToken1,
                minAmountOut: 0,
        ...
        position.positionType = position.positionType ==
            PositionType.OPENED_LONG_COMPLITE
            ? PositionType.CLOSED_LONG_IN_PROGRESS
            : PositionType.CLOSED_SHORT_IN_PROGRESS;
        marketToPositionId[orderId] = position.positionId;
        emit ClosePosition(
            _params.id,
        ...
        payable
    {
        ...
        ILimitOrdersController.OrderType orderType = position
            .collateralToken != IUniswapV3Pool(position.poolAddress).token0()
            ? ILimitOrdersController.OrderType.TAKE_PROFIT
            : ILimitOrdersController.OrderType.BUY_LIMIT;
        uint256 orderId = limitOrdersController.createOrder{value: msg.value}(
            ILimitOrdersController.OrderParams({
                fee: IUniswapV3Pool(position.poolAddress).fee(),
                token0: IUniswapV3Pool(position.poolAddress).token0(),
                token1: IUniswapV3Pool(position.poolAddress).token1(),
                tickLower: _params.tickLower,
                tickUpper: _params.tickUpper,
                amountOfToken0: orderType ==
                    ILimitOrdersController.OrderType.TAKE_PROFIT
                ...
                receivedAmountOfToken0: orderType ==
                    ...
                    ? limitOrdersController.getAmount0FromAmount1(
                        position.openOrder.receivedAmountOfToken1,
                        _params.tickUpper -
                            IUniswapV3Pool(position.poolAddress).tickSpacing(),
                        _params.tickUpper
                    )
                    : 0,
                receivedAmountOfToken1: orderType ==
        ...
        position.positionType = position.positionType ==
            ...
            ? PositionType.CLOSED_LONG_IN_PROGRESS
            : PositionType.CLOSED_SHORT_IN_PROGRESS;
        limitToPositionId[orderId] = position.positionId;
        emit ClosePosition(
            _params.id,
            position.positionType,
            position.owner,
    ...
    // function closePosition(CloseParams memory _params) external payable {
    //     require(_params.deadline >= block.timestamp, "Deadline");
    //     if(!_params.closeByMarket)
    //         require(_params.tickLower == 0 && _params.tickUpper == 0, "Ticks must be zero");
    //     Position storage position = positions[_params.id];
    //     require(position.owner == _msgSender(), "Only owner can confirm");
    //     require(
    //         position.positionType == PositionType.OPENED_LONG_COMPLITE ||
    ...
    //             ? ILimitOrdersController.OrderType.TAKE_PROFIT
    //             : ILimitOrdersController.OrderType.BUY_LIMIT;
    //         uint256 orderId = limitOrdersController.createOrder{
    //             value: msg.value
    //         }(
    //             ILimitOrdersController.OrderParams({
    //                 fee: IUniswapV3Pool(position.poolAddress).fee(),
    ...
    // function cancelPosition(bytes32 _key) external {
    //     Position storage position = positions[_key];
    //     require(position.blockNumber != 0, "Order is not supported");
    //     require(
    //         _msgSender() == position.owner,
    ...
    //         position.positionType == PositionType.OPENED_SHORT_COMPLITE
    //     ) {
    //         limitOrdersController.cancelOrder(
    //             position.openOrder.orderId,
    //             position.poolAddress
    //         );
    //         order = limitOrdersController.orders(
    //             position.poolAddress,
    //             position.openOrder.orderId
    //         );
    //     } else if (
    ...
    //             position.closeOrder.orderId
    //         );
    //     } else revert("Position is filled or cancelled");

    //     uint256 receivedCollateral = position.collateralToken == order.token0
    //         ? order.transferedToken0
    //         : order.transferedToken1;
    //     uint256 receivedSecondToken = position.collateralToken == order.token0
    //         ? order.transferedToken1
    //         : order.transferedToken0;

    //     if (receivedCollateral > 0) {
    //         IERC20Upgradeable(position.collateralToken).transfer(
    //             position.owner,
    //             receivedCollateral
    //         );
    //     }
    //     address secondToken = position.collateralToken == order.token0
    //         ? order.token1
    //         : order.token0;

    //     if (receivedSecondToken > 0) {
    //         IERC20Upgradeable(secondToken).approve(
    //             address(limitOrdersController),
    //             receivedSecondToken
    //         );
    //         limitOrdersController.swapExactInputMultiHop(
    //             secondToken,
    //             position.collateralToken,
    //             receivedSecondToken,
    //             0,
    //             abi.encodePacked(
    //                 secondToken,
    //                 IUniswapV3Pool(position.poolAddress).fee(),
    //                 position.collateralToken
    //             )
    //         );
    //     }
    ...
    //             payable(_msgSender()),
    //             limitOrdersController.ethForExecute(position.openOrder.orderId)
    //         );
    //         position.positionType = position.positionType ==
    //             PositionType.OPENED_LONG_IN_PROGRESS ||
    //             position.positionType == PositionType.OPENED_LONG_COMPLITE
    //             ? PositionType.CANCELLED_LONG
    //             : PositionType.CANCELLED_SHORT;
    //     } else {
    //         AddressUpgradeable.sendValue(
    //             payable(_msgSender()),
    //             limitOrdersController.ethForExecute(
    //                 position.closeOrder.orderId
    //             )
    //         );
    //         position.positionType = position.positionType ==
    ...
    function _executionComplite(uint256 orderId) internal {
        Position storage position;
