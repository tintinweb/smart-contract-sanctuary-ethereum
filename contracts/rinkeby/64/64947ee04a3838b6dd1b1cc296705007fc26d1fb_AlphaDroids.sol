 ...
 */
contract AlphaDroids is ERC721, Ownable {
    using SafeMath for uint256;

    uint256 public DroidPrice = 55000000000000000; // 0.055 ETH
    //uint256 public DROID_PRICE = 0.055 ether;
    //uint public constant maxDroidPurchase = 20;
    uint256 public MAX_DROIDS = 5555;
    ...
    constructor() ERC721("Alpha Droids", "DRIODS") {
    ...
    function reserveDroids(uint amount) public onlyOwner {        
        //require(isLive, "Can not mint any Alpha Droids before the sale has started"); Uncommented this to allow team to mint without exposing sale to anyone watching the contracts
        require(teamTokenCount.add(amount) <= 55, "Can not mint more than 55 tokens for the team");
    ...
    function mintDroids(uint numberOfTokens) public payable {
        require(isLive, "Can not mint any Alpha Droids before the sale has started");
        require(totalSupply().add(numberOfTokens) <= MAX_DROIDS, "Purchase would exceed max supply of Alpha Droids");
        require(DroidPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() < MAX_DROIDS) {
