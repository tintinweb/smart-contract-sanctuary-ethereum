...
/// @notice All strategies are registered through this contract.
    ...
    OwnableUpgradeable
{
    ...
    /// @notice Create NFT for execution bundle
    /// @param name The name of the strategy.
    /// @param execBundle The IPFS reference of the execution bundle.
    /// @param maxGasCost The maximum gas cost of the strategy.
    /// @param maxGasPerAction The maximum gas per action of the strategy.
    /// @return tokenId as the token id of the new NFT.
    function createStrategy(
        string calldata name,
        string calldata execBundle,
        uint128 maxGasCost,
        uint128 maxGasPerAction,
        string memory strategyDecodingInfo
        ...
        if (
            ...
            keccak256(abi.encodePacked(execBundle))
        ) {
            revert("Strategy already exists");
        }

        // Mint a new token to the current sender
        uint256 newStrategyIndex = mint(execBundle);
        ...
        strategies[execBundle] = IStrategyRegistry.RegisteredStrategy(
            ...
            maxGasCost,
            maxGasPerAction,
            strategyDecodingInfo
        );

        // To help with enumeration we also map the token ID to the CID
        tokenIdToExecBundle[newStrategyIndex] = execBundle;

        // Emit StrategyCreated event once a strategy is created
        emit StrategyCreated(msg.sender, newStrategyIndex, name);

        // Return the token id of the new strategy
        return newStrategyIndex;
    }
     ...
     */
    function mint(string calldata bundle) private returns (uint256) {
        uint256 newStrategyId = _tokenIdTracker.current();
        _mint(msg.sender, newStrategyId);
    ...
    /// @notice Get the strategy details by tokenId
    ...
    /// @notice Set the gas parameters for a given strategy
    ...
    function setGasParameters(
        ...
        // Make sure that the owner of the strategy is the only one who can set the gas parameters
        require(
            msg.sender == ownerOf(_tokenId),
            "Only bundle owner can set gas parameters"
        );
