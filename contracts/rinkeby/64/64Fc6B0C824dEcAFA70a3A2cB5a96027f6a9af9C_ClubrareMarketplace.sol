    ...
    EIP712Upgradeable
{
    ...
    function initialize(
        ...
        require(
             keccak256(
                "Order(address seller,address contractAddress,uint256 royaltyFee,address royaltyReceiver,address paymentToken,uint256 basePrice,uint256 listingTime,uint256 expirationTime,uint256 nonce,uint256 tokenId,uint8 orderType,string uri,string objId)"
            ) == _ORDER_TYPEHASH
        );
    ...
    function _deriveDomainSeparator() private view returns (bytes32) {
        return keccak256(abi.encode(_EIP_712_DOMAIN_TYPEHASH, _NAME_HASH, _VERSION_HASH, CHAIN_ID, address(this)));
    }
    ...
    modifier onlySeller(Order calldata order) {
     ...
     modifier onlyApproved(address contractAddress,uint tokenId,address seller) {
        if(tokenId>0)
        {
         require((address(this) ==IERC721Upgradeable(contractAddress).getApproved(tokenId)||IERC721Upgradeable(contractAddress).isApprovedForAll(seller,address(this))),"Token id not approved");
        }
        _;
    }

    modifier onlyPhyiscal(Order calldata _order) {
        require(_order.orderType == Type.Physical, "Not a physical NFT");
        _;
    }

    modifier onlyDigital(Order calldata _order) {
        require(_order.orderType == Type.Digital, "Not a digital NFT");
        _;
    }
    ...
    {
        (bytes32 digest, address signer) = _verifySig(order);
        ...
        if (_tokenId == 0) {
            ...
            _tokenId = (isAdminContract && isAdmin)
                ...
                : nftContract.safeMint(signer, _order.uri, _order.royaltyReceiver, _order.royaltyFee);
        }
    
        uint earning=settlement(nftContract,_tokenId,paid,msg.sender,_order);
        ...
        onlyApproved(order.contractAddress, order.tokenId,order.seller)
    {
        ...
        uint256 amt = _amount;
        address taker = buyer;
        require(validateOrder(digest, _order,_bid,taker, _order.signature, amt), "Invalid Order");
        ...
        uint sellerEarning = settlement(nftContract,_tokenId,amt,taker,_order);
    ...
    function settlement(INFT nftContract,uint _tokenId,uint amt, address taker,Order calldata _order) internal returns(uint){
        ...
        _executeExchange(_order, msg.sender, taker, sellerEarning, true);
    ...
    function InvalidateSignedBid(Bid calldata bid) external whenNotPaused nonReentrant {
        (bytes32 digest, address signer) = _verifySig(bid);
        require(msg.sender == signer, "Not a signer");
        cancelledOrFinalized[digest] = true;
        _bidderNonces[signer].increment();
        emit CancelBid(bid.bidder,
        bid.seller,
        bid.contractAddress,
        bid.tokenId,
        bid.bidAmount,
    ...
    function Shipped(uint256 _orderId, uint256 _trackingId)
        external
        whenNotPaused
        nonReentrant
        isNonZero(_orderId)
        isNonZero(_trackingId)
    {
        escrow.Shipped(_orderId, _trackingId);
    }

    function Received(uint256 _orderId, uint256 _trackingId)
        external
        whenNotPaused
        nonReentrant
        isNonZero(_orderId)
        isNonZero(_trackingId)
    {
        escrow.Received(_orderId, _trackingId);
    }

    function claimPayout(uint256 _orderId, uint256 _trackingId)
        external
        whenNotPaused
        nonReentrant
        isNonZero(_orderId)
        isNonZero(_trackingId)
    {
        escrow.claimPayout(_orderId, _trackingId);
    }

    function cancelOrder(uint256 _orderId, uint256 _trackingId)
        external
        whenNotPaused
        nonReentrant
        isNonZero(_orderId)
        isNonZero(_trackingId)
    {
        escrow.cancelOrder(_orderId, _trackingId);
    }

    function claimRefund(uint256 _orderId, uint256 _trackingId)
        external
        whenNotPaused
        nonReentrant
        isNonZero(_orderId)
        isNonZero(_trackingId)
    {
        escrow.claimRefund(_orderId, _trackingId);
    }

    function raiseDispute(uint256 _orderId) external whenNotPaused nonReentrant isNonZero(_orderId) {
        escrow.raiseDispute(_orderId);
    }
    ...
    function _executeExchange(
        ...
        _invalidateSignedOrder(order.contractAddress, order);
        if(order.orderType != Type.Escrow)
        {
            address _seller = adminContracts[order.contractAddress] && admins[seller] ? address(this) : seller;
            IERC721Upgradeable(order.contractAddress).transferFrom(_seller, buyer, order.tokenId);
            isToken
                ? IERC20Upgradeable(order.paymentToken).safeTransferFrom(buyer, _seller, _amount)
                : payable(_seller).transfer(_amount);
        }else 
        {
            IERC721Upgradeable(order.contractAddress).safeTransferFrom(seller, address(escrow), order.tokenId);
            isToken
    ...
    function _invalidateSignedOrder(address contractAddress, Order calldata order) internal {
        (bytes32 digest, address signer) = _verifySig(order);
        bool isMinted = _isMinted(contractAddress, order.tokenId);
        require(!isMinted || signer == IERC721Upgradeable(contractAddress).ownerOf(order.tokenId));
        cancelledOrFinalized[digest] = true;
        _orderNonces[signer].increment();
    }
    ...
    function checkRoyalties(
        ...
        try nft.royaltyInfo(tokenId, amount) returns (address royaltyReceiver, uint256 royaltyAmt) {
            ...
            if (adminContracts[contractAddress]) {
                ...
                address creator = admins[_sender] ? TREASURY_WALLET : token.creators(tokenId);
    ...
    function verifySeller(Order calldata order) internal view returns (bool) {
        address owner = IERC721Upgradeable(order.contractAddress).ownerOf(order.tokenId);
        (, address signer) = _verifySig(order);
        return owner == signer;
    }
    ...
    function _verifySig(Order calldata order) public view returns (bytes32, address) {
        bytes32 digest = hashToSign(order);
        address signer = ECDSAUpgradeable.recover(digest, order.signature);
        return (digest, signer);
    }

    function _verifySig(Bid calldata bid) public view returns (bytes32, address) {
    ...
    function validateOrder(
        ...
        if(!validateOrder(hash,order,sig,amount))
        {
            return false;
        }
        return true;
    ...
    function hashBid(Bid memory _bid) public pure returns (bytes32 hash) {
        bytes memory array = abi.encode(
            keccak256("Bid(address seller,address bidder,address contractAddress,address paymentToken,uint256 bidAmount,uint256 bidTime,uint256 expirationTime,uint256 nonce,uint tokenId,string objId)"),
            _bid.seller,
            _bid.bidder,
...
interface IToken {
    ...
    function safeMint(string memory uri, uint256 value) external returns(uint);
