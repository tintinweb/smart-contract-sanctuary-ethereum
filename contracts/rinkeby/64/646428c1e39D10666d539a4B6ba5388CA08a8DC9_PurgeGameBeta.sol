...
contract PurgeGameBeta is ERC721, ERC721Enumerable, Pausable, Ownable
{
    ...
    function purge(uint256[] calldata _tokenIds) public  
    {
   ...
   function xxmintAndPurge(uint16 _number) external payable 
    {
        RequireCorrectFunds(_number);
        RequireSale(_number);
        PurgedCoinInterface(purgedCoinContract).mintFromPurge(msg.sender, _number * cost * 1000);
    ...
    function coinMintAndPurge(uint256 _number) external 
    {
        RequireSale(_number);
    ...
    function coinMint(uint256 _number) external
    {
        RequireSale(_number);
        RequireTenMax(_number);
        RequireCoinFunds(_number);
    ...
    function _mintToken(uint256 _number) internal
    {
    ...
    event MintAndPurge(string readme,uint256 indexed number, address indexed _from);
