...
contract AlioitDraft is ERC721URIStorage, Ownable {
    uint256 public tokenCounter;

    event CreatedNFT(address owner, uint256 indexed tokenId);

    constructor() ERC721('RANDOM NFT', 'rsNFT') {        
        tokenCounter = 0;
    }
    ...
    //     pathSvg = string(abi.encodePacked('<path d="M', ' ', uint2str(parameterOne), ' ', uint2str(parameterTwo), ' '));
    ...
    //     pathCommand = 'L';
    //     uint256 parameterOne = uint256(keccak256(abi.encode(_randomNumber, size * 4))) % size;
