...
contract NFTDrop is ERC721A, Ownable, PaymentSplitter {
    ...
    function mintTo(address to, uint256 amount) external payable {
        ...
        require(publicSaleStarted == true, "The public sale has not started yet.");

        // Crossmint needs the amount param to be uint256, but it should be safe to cast it to uint8,
        // since we mint less than 255 NFTs per transaction
        _internalMint(to, uint8(amount));
    }

    function _internalMint(address to, uint8 amount) internal {
        require(
