...
contract VRFBeaconOCR is VRFBeaconReport {
  ...
  uint256 constant internal MAX_NUM_ORACLES = 31;
  ...
  function latestConfigDigestAndEpoch() external view returns (
    bool scanLogs, bytes32 configDigest, uint32 epoch) {
    return (false, s_configInfo.latestConfigDigest, epochOfLastReport);
  }
    ...
    external
  {
    ...
    uint192 juelsPerFeeCoin = _report(hotVars, reportContext[0], epochAndRound, report);
    ...
    external
  {
    ...
    require(LINK.transfer(recipient, _min(linkBalance - linkDue, amount)), "insufficient funds");
  }
...
contract VRFBeaconReport is VRFBeaconExternalAPI {
  ...
  function _report(
    ...
    internal
    returns (uint192 juelsPerFeeCoin)
  {
    ...
    uint16 numOutputs;
    OutputServed[] memory outputsServedFull =
      new OutputServed[](report.outputs.length);
    for (uint256 i = 0; i < report.outputs.length; i++) {
      VRFOutput memory r = report.outputs[i];
      processVRFOutput(r, report.recentBlockHeight, report.juelsPerFeeCoin);
    ...
    for (uint256 i = 0; i < numOutputs; i++) {  // truncate heights
  ...
  uint256 constant public maxErrorMsgLength = 128;

  function processVRFOutput( // extracted to deal with stack-depth issue
    VRFOutput memory r,
    uint64 recentBlockHeight,
    uint192 juelsPerFeeCoin
  ) internal{
    if (r.blockHeight > recentBlockHeight) {
      revert HistoryDomainSeparatorTooOld(recentBlockHeight, r.blockHeight);
    }
    bytes32 seed;
    if (r.vrfOutput.p[0] == 0 && r.vrfOutput.p[1] == 0) {
  ...
  function processCallback( // extracted to deal with stack-depth issue
      uint64 blockHeight,
      ...
      CostedCallback memory c
        ...
        bytes32 cbCommitment = keccak256(abi.encode(
                                           blockHeight,
                                           confDelay,
                                           cb.subID,
                                           cb
        ...
        /* XXX: Probably have to use something lower-leval than this. If
           rawFulfillRandomWords doesn't exist, this reverts. */
        bytes memory resp = abi.encodeWithSelector(consumer.rawFulfillRandomWords.selector,  cb.requestID,
                                                                                                  fOutput,
                                                                                                          cb.arguments);
       s_config.reentrancyLock = true;
       bool success = callWithExactGas(c.callback.gasAllowance,  cb.requester, resp);
...
/// @notice the next beacon output, then call `getRandomness` with the RequestID
...
contract VRFBeaconExternalAPI is VRFBeaconTypes, VRFBeaconBilling {
  ...
  /// @notice Max length of array returned from getRandomness
  uint256 constant public maxNumWords = 1000;
  ...
  function requestRandomness(
    ...
    (RequestID nonce, BeaconRequest memory r, uint64 nextBeaconOutputHeight) =
  ...
  function requestRandomnessFulfillment(
    ...
    (
      RequestID requestID,
      , // BeaconRequest. We do not store this, because we trust the committee
    ...
    s_callbackMemo[requestID] = keccak256(abi.encode(
  ...
  function getRandomness(RequestID requestID)
    public returns (uint256[] memory randomness)
  {
    ...
    uint256 blockHeight =
      SlotNumber.unwrap(r.slotNumber) * i_beaconPeriodBlocks;
    uint256 confThreshold =
      block.number - ConfirmationDelay.unwrap(r.confirmationDelay);
  ...
  /// @notice Emitted when getRandomness is called by an address which does not
  ...
  /// @notice Emitted when getRandomness is called for a block which is too
  ...
  /// @notice Emitted when getRandomness is called for a block where the seed
  ...
  mapping(uint256 /* block height */ =>
          mapping(ConfirmationDelay => bytes32 /* seed */))
    s_seedByBlockHeight;
  ...
  function beaconRequest(
    ...
    uint256 nextBeaconOutputHeight =
      block.number + i_beaconPeriodBlocks - periodOffset;
    ...
    for (i = 0; i < rp.confirmationDelays.length; i++) {
      if (ConfirmationDelay.unwrap(rp.confirmationDelays[i]) ==
  ...
  function finalOutput(
    ...
    if (r.numWords > maxNumWords) { // Could happen if corrupted quorum submits
...
contract VRFBeaconTypes {
  ...
  SlotNumber constant internal MAX_SLOT_NUMBER =
    SlotNumber.wrap(type(uint32).max);

  type ConfirmationDelay is uint24;
  ConfirmationDelay constant internal MAX_CONFIRMATION_DELAY =
    ConfirmationDelay.wrap(type(uint24).max);
  uint8 constant internal CONFIRMATION_DELAY_BYTE_WIDTH = 3;
...
abstract contract IVRFBeaconConsumer {
  ...
  constructor(address _coordinator) { coordinator = _coordinator; }
...
contract VRFBeaconBilling is OwnerIsCreator {
  ...
  function billSubscriberForRequest(
    ...
    address,                            /* requester */
    uint64                              /* subID */
  ) internal pure { /* XXX: "pure" for now just to silence compiler */ // throws on failure
    /* XXX: Fill this in */
  }
  ...
  mapping(address /* consumer */ => uint64 /* subscription */) internal
    s_consumerSubscription;
  ...
  function forgetConsumerSubscriptionID(
...
contract BeaconVRFConsumer is IVRFBeaconConsumer, VRFBeaconTypes {
  ...
  mapping(uint256 /* block height */ =>
            mapping(ConfirmationDelay => VRFBeaconTypes.RequestID)) public
      s_requestsIDs;
  mapping(VRFBeaconTypes.RequestID => VRFBeaconTypes.BeaconRequest) public s_myBeaconRequests;
  ...
  mapping(uint256 /* block height */ =>
              mapping(VRFBeaconTypes.ConfirmationDelay => bytes32))
  ...
  constructor(address coordinator, bool shouldFail, uint256 beaconPeriodBlocks/*, address link*/) IVRFBeaconConsumer(coordinator) {
  ...
  //  LINKTOKEN = LinkTokenInterface(link);
   ...
   function setExpectedSeed(uint64 height, VRFBeaconTypes.ConfirmationDelay confirmationDelayArg,
   ECCArithmetic.G1Point memory vrfOutput) public{
    bytes32 seed = keccak256(abi.encode(vrfOutput));
    s_ExpectedSeeds[height][confirmationDelayArg] = seed;
   }
  function fulfillRandomWords(VRFBeaconTypes.RequestID requestID,
                                  uint256[] memory response,
                                  bytes memory arguments) internal override {
   if(fail ){
...
function testRequestRandomnessFulfillment(uint64 subID,   uint16 numWords, VRFBeaconTypes.ConfirmationDelay confirmationDelayArg,
         uint32 callbackGasLimit,  bytes memory arguments) external returns (VRFBeaconTypes.RequestID) {
      uint256 periodOffset = block.number % i_beaconPeriodBlocks;
       uint256 nextBeaconOutputHeight =
          block.number + i_beaconPeriodBlocks - periodOffset;
     VRFBeaconTypes.RequestID reqId = COORDINATOR.requestRandomnessFulfillment(subID, numWords, confirmationDelayArg, callbackGasLimit, arguments);
  ...
  function testRequestRandomness(
      ...
      uint256 periodOffset = block.number % i_beaconPeriodBlocks;
             uint256 nextBeaconOutputHeight =
                block.number + i_beaconPeriodBlocks - periodOffset;
           VRFBeaconTypes.RequestID reqId = COORDINATOR.requestRandomness(numWords, subID, confirmationDelayArg);
           s_requestsIDs[nextBeaconOutputHeight][confirmationDelayArg] = reqId;
           uint256 slotNumberBig = nextBeaconOutputHeight / i_beaconPeriodBlocks;
            SlotNumber slotNumber = SlotNumber.wrap(uint32(slotNumberBig));
              BeaconRequest memory r = BeaconRequest({
  ...
  function testGetRandomness(RequestID requestID) public{
       uint256[] memory response = COORDINATOR.getRandomness(requestID);
        s_ReceivedRandomnessByRequestID[requestID] = response;
  }


  function computeAndStoreExpectedOutput(uint64 height, VRFBeaconTypes.ConfirmationDelay confirmationDelayArg) public {
    VRFBeaconTypes.RequestID reqId = s_requestsIDs[height][confirmationDelayArg];
...
contract VRFBeaconDKGClient is DKGClient {
  ...
  modifier fromKeyProvider() {
    ...
    if (msg.sender != address(keyProvider)) {
      revert KeyInfoMustComeFromProvider(msg.sender, address(keyProvider));
    }
    _;
  }
...
contract WrapperBeaconCoordinator is VRFBeaconCoordinator, IVRFBeaconConsumer {
  ...
  constructor(
    ...
    VRFBeaconCoordinator(address(link), beaconPeriodBlocksArg, keyProvider, keyID)
...
contract TestECCArithmetic is ECCArithmetic {
  ...
  function testAddG1(G1Point memory p1, G1Point memory p2)
    public view returns (G1Point memory sum) {
    return addG1(p1, p2);
  }
...
contract ExposeReport {
  function exposeReport(
    VRFBeaconReport.Report memory r
...
contract TestVRFConsumer is IVRFConsumer {
  ...
  function fulfillRandomWords(
    bytes32 _requestID, uint256[] memory _response
  ) internal override {
    require(
  ...
  function requestRandomWords(uint32 numwords) external returns (bytes32[2] memory) {
