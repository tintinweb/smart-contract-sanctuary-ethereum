...
contract TestDescriptor is ERC721 {

    constructor(uint traitValue1, uint256 traitValue2, uint256 traitValue3, uint traitValue4) ERC721("DESCRIPTOR_TEST", "DESC") {
        attrs.push(
            NFTDescriptor.Attribute(NFTDescriptor.DisplayType.BOOST_NUMBER, "trait_number_one", abi.encodePacked(traitValue1))
        );
        attrs.push(
            NFTDescriptor.Attribute(NFTDescriptor.DisplayType.NUMBER, "trait_number_two", abi.encodePacked(traitValue2))
        );
        attrs.push(
            NFTDescriptor.Attribute(NFTDescriptor.DisplayType.BOOST_PERCENT, "trait_number_thress", abi.encodePacked(traitValue3))
        );
        attrs.push(
            NFTDescriptor.Attribute(NFTDescriptor.DisplayType.DATE, "trait_number_4", abi.encodePacked(traitValue4))
        );
    }

    NFTDescriptor.Attribute[] public attrs;

    mapping(uint256 => NFTDescriptor.ConstructTokenURIParams) _tokenStats;

    function mint(uint256 tokenId) external {
        _mint(msg.sender, tokenId);
        NFTDescriptor.ConstructTokenURIParams storage stats = _tokenStats[tokenId];
        stats.lastTransfered = block.timestamp;
        stats.image = "IMAGE_LINK_HERE";
        stats.name = "NAME_HERE";
    }

    function tokenURI(uint256 tokenId) public view override returns(string memory) {
        require(_exists(tokenId), "non existen");
        NFTDescriptor.ConstructTokenURIParams storage stats = _tokenStats[tokenId];
        return NFTDescriptor.constructTokenURI(
            NFTDescriptor.ConstructTokenURIParams(
                tokenId,
                stats.lastTransfered,
                stats.image,
                ownerOf(tokenId),
                stats.name,
                attrs
