...
contract BoredPetsNFT is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    event NFTMinted(
        ...
        uint256 indexed MintedAt
    );

    constructor(address token) ERC721("Bored Pets Yacht Club", "BPYC") {
        tokenCounter = 0;
        _token = token;
        _totalPayments = 0;
    }
    ...
    function createCollectible(
        ...
        emit NFTMinted(newTokenId, _tokenURI, block.timestamp);
    ...
    // Buy an NFT
    function buyNft(uint256 _tokenId, address buyer)
        public
        nonReentrant
        onlyOwner
    {
        ...
        require(nft.owner != buyer, "Owner Can't Be Buyer");
        require(nft.listed, "NFT Not Listed For Sale");
        _orderCount.increment();
        uint256 orderCount = _orderCount.current();

        ERC20Spendable(_token).spend(buyer, nft.price);
        _totalPayments += nft.price;
        transferFrom(nft.owner, buyer, nft.tokenId);
        nft.owner = buyer;
        nft.sold = true;
        nft.listed = false;
        ...
        emit NFTSold(nft.tokenId, buyer, nft.seller);
        _nftsSold.increment();
    }

    function resetPayments() public onlyOwner nonReentrant {
        _totalPayments = 0;
    }

    function changeToken(address token) public onlyOwner nonReentrant {
        _token = token;
    }
    ...
    function getUnsoldNft() public view returns (NFT[] memory) {
    ...
    function getmintedNft() public view returns (NFT[] memory) {
        uint256 nftCount = _nftCount.current();
