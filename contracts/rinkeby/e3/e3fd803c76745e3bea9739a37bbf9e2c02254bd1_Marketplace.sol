...
contract Marketplace is ReentrancyGuard {
    ...
    address public arbitrator; //The arbitrator makes the final decision in case of a dispute
    ...
    struct Item {
        ...
        bool sold;
        bool collected; //This shows if the seller has collected the value of the NFT after the redemption period is over or after a successful delivery
    ...
    //Mapping the item number and delivery start time to the struct
    mapping(uint => Item) public items;
    mapping(uint => address) public signatures; //This mapping is used to store the reception signature of each received item
    ...
    //mapping(uint => bool) public collected; //Used to indicate if the NFT value has already been collected by the seller or not

    //Enumerate variable for the delivery state of the NFT
    enum NFTState {Listed, Purchased, RedeemInitiated, EnRoute, Delivered, Disputed}
    ...
    event Dispute(address indexed Seller, address indexed nft, address indexed Buyer, uint tokenId, bytes32 IPFShash);
    ...
    event SignatureVerification(address indexed redeemer, address indexed nft, uint tokenId, bytes32 message, bytes signature);
    ...
    function assignArbitrator(address _arbitrator) public {
        require(msg.sender == MarketplaceOwner, "Only the Marketplace owner is allowed to assign the arbitrator");
    ...
    function purchaseItem(uint _itemId) external payable nonReentrant { //external to prevent accessing it from within the smart contract
        ...
        payable(address(this)).transfer(items[_itemId].price); //The price of the item is kept in the smart contract for the allowed period for redemption
        MarketplaceOwner.transfer(_totalPrice - items[_itemId].price); 
        items[_itemId].sold = true;
        // transfer nft to buyer
        items[_itemId].nft.transferFrom(address(this), msg.sender, items[_itemId].tokenId);
        items[_itemId].NFTstate = NFTState.Purchased;
        items[_itemId].SellingTime = block.timestamp; //Stores the selling time of the NFT by using the _itemID which is used within the marketplace only
        // emit Bought event
        emit Bought(_itemId, address(items[_itemId].nft), items[_itemId].tokenId, items[_itemId].price, items[_itemId].seller, msg.sender);
    }
    ...
    function ClaimNFTValue(uint _itemCount) public{
        ...
        require(!item.collected,"The value of this NFT has already been collected");
        ...
        require(item.NFTstate != NFTState.Disputed, "If there is a dispute, the value of the NFT will be handled by the arbitrator");
    ...
    function startDelivery(uint _itemCount) public{
        ...
        require(item.NFTstate == NFTState.RedeemInitiated, "The buyer should initiate NFT redemption first");
        require(block.timestamp < DeliveryDuration + RedemptionPeriod, "Can't start delivery after the specified delivery duration");
    ...
    function ProofofDelivery(uint _itemCount) public{
        ...
        require(block.timestamp < DeliveryDuration + RedemptionPeriod, "Can't end delivery after the specified delivery duration");
        require(signatures[_itemCount] == item.redeemer, "The seller cannot prove delivery without the signature of the buyer");
        
        item.NFTstate == NFTState.Delivered; //The seller declares delivering the NFT
        item.redeemed = true; //The NFT is redeemed
    ...
    function OpenDispute(uint _itemCount, string memory _IPFShash) public{
        Item storage item = items[_itemCount];
        require(msg.sender == item.nft.ownerOf(item.tokenId) || msg.sender == item.seller, "Only the current owner of the NFT or its seller can execute this function");
        ...
        emit Dispute(item.seller, address(item.nft), item.nft.ownerOf(item.tokenId), item.tokenId, bytes32(bytes(_IPFShash)));
    ...
    //This function is used to store the signature verification on-chain if needed
    function recoverSigner(bytes32 message, bytes memory sig, uint _itemCount) public returns (address) {
       Item storage item = items[_itemCount];
       uint8 v;
       bytes32 r;
       bytes32 s;
       ...
       if(ecrecover(message, v, r, s) == item.redeemer){
        signatures[_itemCount] = ecrecover(message, v, r, s);

        emit SignatureVerification(ecrecover(message, v, r, s), address(item.nft), item.tokenId, message, sig );
       }
       return ecrecover(message, v, r, s); //For testing purposes only
  }



  //This function is used to split the signature into v,r,s which are then returned to recoverSigner function
