...
contract CallbackStorage is OwnableUpgradeable {
    ...
    mapping(address => bool) public registeredEventSources;
    ...
    function isValidSource(address _source) public view returns (bool) {
        return registeredEventSources[_source];
    }
    ...
    function addCallback(address _contract, uint256 tokenId, IHandlerCallback.CallbackType _type, IHandlerCallback.Callback memory _callback) public onlyLatestVersion() {
        registeredEventSources[_contract] = true;
...
contract HasCallbacksUpgradable is HasRegistrationUpgradable {
    ...
    function testCallback(address _from, address _to, uint256 tokenId) public {
        require(CallbackStorage(StorageAddress).isValidSource(_msgSender()), "Not valid source");
        ticks++;
        lastTokenId = tokenId;
        lastTo = _to;
        lastFrom = _from;  
    ...
    function testRevertCallback(address, address, uint256) public view {
        require(CallbackStorage(StorageAddress).isValidSource(_msgSender()), "Not valid source");
