...
import './common/libraries/MerkleProofAddOns.sol';
...
contract Stargazer is ERC721Enumerable, Ownable {
  ...
  uint256 public allowlistCost = 0.5 ether;

  // Treasury wallet
  // TODO: CHANGE ME
  address public treasury = 0x4AB78e58f5BD1BF4EE2254628fd68d18438C675a;

  // Supply limit
  uint256 public supplyStrictUpperBound = 5250;

  constructor() ERC721('Fearless Girl: Stargazer Collection', 'FGSG') {}

  /**
   * @param tokenIds List of token identifiers.
   * @dev Reverts if caller does not own all of the given tokens.
   */
  modifier meetsOwnership(uint256[] calldata tokenIds) {
    require(
      isOwnerOf(_msgSender(), tokenIds),
      'Stargazer: You do not own these tokens.'
    );
    _;
  }
   ...
   */
  modifier correctCost(uint256 numberOfTokens, uint256 msgValue) {
    require(
      numberOfTokens * cost == msgValue,
      'Stargazer: Incorrect amount provided.'
    );
    _;
  }
  ...
  /**
   * @param _contractURI New URL pointing to contract metadata.
  ...
  /**
   * @param _allowlistMerkleRoot The new merkle root of the allowlist.
  ...
  /**
   * @param _cost New collection cost
  ...
  /**
   * @param _allowlistCost New allowlist cost
   * @notice Cost for allowlist
   * @dev Only contract owner can call this function.
   */
  ...
  /**
   * @param amt Number of tokens to mint.
   * @notice Mints the given number of tokwens.
   * @dev Sale must be active.
   * @dev Cannot mint more than supply limit.
   * @dev Correct cost amount must be supplied.
   */
  function publicMint(uint256 amt)
    public
    payable
    mintActive
    correctCost(amt, msg.value)
  {
    uint256 numTokens = totalSupply();

    for (uint256 i = 0; i < amt; i++) {
      _mint(_msgSender(), numTokens + i);
    }
  ...
  function withdraw() public {
    ...
    require(success, 'Stargazer: Failed to withdraw to treasury.');
  }

  /**
   * @param count Number of tokens to mint
   * @param allowance Caller's allowance
   * @param proof Merkle proof of allowlisted status.
   * @notice Mint function for allowlist addresses.
   */
  function allowlistMint(
    ...
    uint256 numTokens = totalSupply();
    require(
      MerkleProofAddOns._verify(
        MerkleProofAddOns._leaf(Strings.toString(allowance), payload),
        allowlistMerkleRoot,
        proof
      ),
      'Stargazer: Invalid Merkle Tree proof supplied.'
    );
    require(
      count * allowlistCost == msg.value,
      'Stargazer: Invalid ether amount provided.'
    );
   ...
   * @param _tokenIds IDs of tokens to be transferred.
   ...
   * @param _tokenIds IDs of tokens we are checking ownership over.
    ...
    onlyOwner
  {
    require(
      ...
      'Stargazer: Amount array length does not match recipient array length.'
    );
...
library AssemblyMath {
    function arraySumAssembly(uint256[] memory _data)
        internal
        pure
        returns (uint256 sum)
    {
