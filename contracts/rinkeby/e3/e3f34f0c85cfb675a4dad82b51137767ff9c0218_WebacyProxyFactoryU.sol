...
contract WebacyProxyFactoryU is IWebacyProxyFactory, Initializable, AccessControlUpgradeable {
    ...
    function createProxyContract(address _memberAddress) external override onlyRole(EXECUTOR_ROLE) {
...
contract WebacyProxy is IWebacyProxy, AccessControl {
    ...
    function transferERC20TokensAllowed(
        ...
        uint256 _amount
    ) external onlyRole(EXECUTOR_ROLE) onlyWhenUnlocked override {
    ...
    function updateExecutorRole(address _address) external onlyRole(DEFAULT_ADMIN_ROLE) onlyWhenUnlocked override {
        revokeRole(EXECUTOR_ROLE, webacyAddress);
        grantRole(EXECUTOR_ROLE, _address);
        webacyAddress = _address;
    }

    function lockContract() external onlyRole(DEFAULT_ADMIN_ROLE) override {
        isUnlock = false;
    }

    function unlockContract() external onlyRole(DEFAULT_ADMIN_ROLE) override {
        isUnlock = true;
    }
}
