...
contract WarrantyNFT is ERC721 {
    ...
    function recordSales(
        ...
        buyer.price = _price;
        trackBuyers[msg.sender].push(objectCounter);
        trackData[objectCounter] = buyer;
    }

    function setNFTWarranty(string memory _tokenURI) public {
        address buyer = msg.sender;
        uint256 objectId = objectCounter;
        _safeMint(buyer, objectId);
        _setTokenURI(objectId, _tokenURI);
        trackTokenURI[objectId] = _tokenURI;
        objectCounter = objectCounter + 1; // Increment object ID
    }

    function verifyOwnership(uint256 _objectId) public view returns (bool) {
        uint256[] memory objects = trackBuyers[msg.sender];
        int256 flag = 0;
    ...
    function returnWarrantyEnd(uint256 _objectId)
        public
        view
        returns (uint256)
    {
        if (verifyOwnership(_objectId)) return trackData[_objectId].endDate;
        else return 0;
    }
    ...
    function buy(uint256 _tokenId) external payable {
        ...
        uint256[] memory objects = trackBuyers[ownerOf(_tokenId)];
        for (uint256 i = 0; i < objects.length; i = i + 1) {
            if (objects[i] == _tokenId) {
                delete trackBuyers[ownerOf(_tokenId)][i];
                break;
            }
        }

        trackBuyers[msg.sender].push(_tokenId);
