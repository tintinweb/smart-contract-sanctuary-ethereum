...
contract PizzaPans is Ownable, ERC721A, ReentrancyGuard {
    uint256 public constant PRICE = 0.001 ether;
    uint256 public constant MAX_PANS_PURCHASE = 2;
    uint256 public constant MAX_PANS = 1000;
    uint256 public constant MAX_BATCH_SIZE = 5;
    address private constant PANS_TEAM = 0x9A4d2B0AA37FF77dBCf8281bcE1Ba2518f5386E1;
    ...
    constructor() ERC721A("PIZZA PANS", "PANS", MAX_BATCH_SIZE, MAX_PANS) {
    }
    ...
    modifier mintGuard(uint256 tokenCount) {
        ...
        require(totalSupply() + tokenCount <= MAX_PANS, "Not enough supply");
        require(tokenCount <= MAX_PANS_PURCHASE, "Exceeds token txn limit ");
        _;
    }

    function reservePANS(uint256 amount) external onlyOwner {
        require(totalSupply() + amount <= MAX_PANS, "Not enough supply");
    ...
    function mint(uint256 amount) external payable mintGuard(amount) {
        ...
        require(publicAddressMintCount[msg.sender] + amount <= MAX_PANS_PURCHASE, "Address already minted allocation");
    ...
    function mintPresale(bytes32[] calldata proof, uint256 amount) external payable mintGuard(amount) {
        ...
        require(presaleAddressMintCount[msg.sender] + amount <= MAX_PANS_PURCHASE, "Address already minted allocation");
    ...
    function withdraw() external onlyOwner {
        payable(PANS_TEAM).transfer(address(this).balance);
    }
}