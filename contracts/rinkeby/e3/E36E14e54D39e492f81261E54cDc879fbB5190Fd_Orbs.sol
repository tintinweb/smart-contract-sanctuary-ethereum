    ...
    OrbsSupply
{
        ...
        canMintWhiteList(maxMintCount, 0, proof, mintCount)
    {
        ...
        if (mintCount == 1) {
            ...
            uint256[] memory orbTypes = new uint256[](mintCount);
            uint256[] memory counts = new uint256[](mintCount);
            for (uint256 index = 0; index < mintCount; index++) {
                uint256 orbType = getOrbType(index);
                orbTypes[index] = orbType;
                counts[index] = 1;
            }
...
abstract contract WhiteListSale {
    ...
    function verifyProof(
        ...
        return
            MerkleProofUpgradeable.verify(
                proof,
                root,
                generateLeaf(maxMintCount, mintPrice, msg.sender)
            );
    }
    ...
    function afterMint(uint256 mintCount) internal virtual {
        _addressToMintCount[msg.sender] += mintCount;
    }
    ...
    modifier canMintWhiteList(
        ...
        require(
            _addressToMintCount[msg.sender] + mintCount < maxMintCount + 1,
