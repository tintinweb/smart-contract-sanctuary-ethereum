...
import "./TokenTypesCollection.sol";
import "@openzeppelin/contracts/token/ERC721/ERC721.sol";
import "@openzeppelin/contracts/utils/Counters.sol";

contract sContract is ERC721 {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    address private _tokenTypesAddr;

    mapping(uint256 => uint256) private _tokenType;
    
    constructor(address tokenTypesAddr) public ERC721("Test", "TES") {
        _tokenTypesAddr = tokenTypesAddr;
    }

    function newToken(uint256 tokenType, address addrTo) public returns(uint256) {
        _tokenIds.increment();

        uint256 newItemId = _tokenIds.current();
        _mint(addrTo, newItemId);
        _tokenType[newItemId] = tokenType;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return string(abi.encodePacked("https://us-central1-smart-contracts-350906.cloudfunctions.net/function-1?token=", tokenId));
    }
}
