...
/// @title Mind The Gap
contract GmStudioMindTheGap is ERC721Common, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    uint32 public constant MAX_NUM_TOKENS = 1000;
    ...
    uint32 internal constant NUM_RESERVED_MINTS = 2;
    ...
    constructor(
        ...
        address[] memory payees,
        uint256[] memory shares
    ) ERC721Common("GmStudio: Mind the Gap", "MTG") {
        _signers.add(signer);
        _baseTokenURI = baseTokenURI;

        paymentSplitter = payable(PaymentSplitterDeployer.instance().deploy(
            payees,
            shares
        ));
