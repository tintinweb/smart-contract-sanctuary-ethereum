...
contract FiveFates is ERC721, Ownable {
    ...
    uint private constant mintCostPhase1 = 1.05 ether;
    uint private constant decrement = 0.05 ether;
    
    //maximum amount of mints allowed per person
    uint256 public constant maxMintPhase1 = 1;
    ...
    constructor() ERC721('Five Fates Collection', 'FFS') {
    ...
    //in case somebody accidentaly sends funds or transaction to contract. We cant receive payment????
    ...
    //gets the tokenID of NFT to be minted. Why not use Counters?
    ...
    function openSale() public onlyOwner {
        ...
        phase2LaunchTime = phase1LaunchTime + 600; //8 hours//28800
        phase3LaunchTime = phase2LaunchTime + 600; //48 hours//172800
    }


    //mint a @param number of NFTs in public sale
    function phase1Mint() public payable {
        ...
        require(mintsPerAddress[msg.sender] == 0, "Maximum 1 Mints per Address allowed!");
        uint256 mintCost_ = mintCost();
        require(msg.value >= mintCost_, "Not sufficient Ether to mint this amount of NFTs");

        uint256 tid = tokenId();
        _safeMint(msg.sender, tid);
        mintsPerAddress[msg.sender] += 1;
        numberOfTotalTokens += 1;
        numberOfTokensPhase1 += 1;
    ...
    //mint a @param number of NFTs in public sale
    function phase2Mint(uint256 number) public payable {
        State saleState_ = saleState();
        ...
        require(msg.value >= mintCost_ * number, "Not sufficient Ether to mint this amount of NFTs");
    ...
    //mint a @param number of NFTs in public sale
    function phase3Mint(uint256 number) public payable {
        State saleState_ = saleState();
    ...
    //retrieve all funds receieved from minting
    ...
    function reservedMintsLeft() public onlyOwner view returns(uint256) {
        ...
        console.log("Reerved Mints Left: ", mintsLeft);
        return mintsLeft;
    ...
    //gets the cost of current mint
      function mintCost() public view returns(uint) {
        ...
        if (saleState_ == State.NoSale) {
            ...
            if (mintCostPhase1 >= 0.15 ether + (((block.timestamp - phase1LaunchTime) / 60) * decrement)) {
                return mintCostPhase1 - (((block.timestamp - phase1LaunchTime) / 60) * decrement);
            }
            else {
                return 0.15 ether;
            }
