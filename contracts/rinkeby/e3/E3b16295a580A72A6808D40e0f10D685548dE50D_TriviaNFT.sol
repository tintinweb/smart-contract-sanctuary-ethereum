...
contract TriviaNFT is ERC721Enumerable, Ownable {
	using Strings for uint256;

	// Contract to recieve ETH raised in sales
	address public vault;

	// To allow Opensea to list
	...
	constructor(address _proxyRegistryAddress) ERC721("TriviaNFT", "TRIV") {
		//setPrice(_price);
	...
	function test(bytes32[] calldata _proof) external payable returns (bool){
		bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
		require(MerkleProof.verify(_proof, presaleMerkleRoot, leaf), 'Proof Invalid');
		return true;
	}
