...
contract XillionIDOMaster is XillionIDOCommon {
    ...
    function joinOrRecalculateAllocation() external whenNotPaused {
        ...
        if(BEP20IdoTokenAddress != address(0)){
            ...
            uint256 minMaxInvestment = Math.min(maxInvestmentAmountPerInvestor, maxInvestmentBEP20AmountPerInvestorInChainCurrency);
    ...
    function finish() external nonReentrant {
        ...
        if (totalInvestedAmountOfIDOInChainCurrency >= idoValueCap * minInvestmentPercentForIDOCompletion / 100 && isSuccessfullyFunded) {
            ...
            for (uint256 i; i < finalInvestorsAmount; i++) {
                ...
                uint256 investmentsPerWalletInChainCurrency;
                if(_investments[_investors[i]] != 0 ){

                    investmentsPerWalletInChainCurrency = _investments[_investors[i]];
                    ...
                    investmentsPerWalletInChainCurrency = _investmentsInBEP20[_investors[i]] * tokenToChainCurrency;

                }

                // calculate amount & release date for investor
                tokensToDistribute = investorTokens * investmentsPerWalletInChainCurrency / totalInvestedAmountOfIDOInChainCurrency;
