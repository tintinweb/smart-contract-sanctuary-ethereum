...
*/
    ...
    //////// REMOVEEE EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE
    ///////////////// EERERERERERERE
    function testMode(uint8 _mode) external {
        if (_mode == 0) {
            _presaleStart = block.timestamp + 10 minutes;
            _presaleEnd = _presaleStart + 30 minutes;
            _saleStart = _presaleEnd;
            _saleEnd = _saleStart + 1 hours;
        } else if (_mode == 1) {
            _presaleStart = block.timestamp;
            _presaleEnd = _presaleStart + 30 minutes;
            _saleStart = _presaleEnd;
            _saleEnd = _saleStart + 30 minutes;
        } else if (_mode == 2) {
            _presaleStart = block.timestamp - 10 minutes;
            _presaleEnd = _presaleStart - 30 minutes;
...
abstract contract dusty is IERC777Recipient, ReentrancyGuard {
    ...
    function tokensReceived(
        ...
        vData memory info;
        uint256 number_of_items;
        require(msg.sender == DUST_TOKEN, "Unauthorised message source");
        if (userData.length == 64) {
            bool isPresaleWithEC = uint256(bytes32(userData[0:32])) != 0;
            uint256 numberToBuy = uint256(bytes32(userData[32:64]));
            if (isPresaleWithEC) {
                require(
                    discountDustPrice != 0,
                    "Dust Presale with EC not available"
                );
                dustPreSaleWithEC(from, amount, numberToBuy);
            } else {
                require(fullDustPrice != 0, "Dust Sale not available");
                dustMainSale(from, amount, numberToBuy);
            }
            return;
        }
        require(userData.length == 257, "Invalid user data");
        ...
        info.dust_price = uint256(bytes32(userData[96:128])); // 128
        info.max_mint = uint256(bytes32(userData[128:160])); // 128
        info.mint_free = uint256(bytes32(userData[160:192])) != 0;
        info.signature = bytes(userData[192:257]); // 160
        require(verify(info), "Unauthorised access secret");
        require(block.timestamp > info.start, "sale period not started");
        require(block.timestamp < info.end, "sale period over");
        require(info.dust_price > 0, "Dust presale minting not available");
        number_of_items = amount / info.dust_price;
        require(
            number_of_items * info.dust_price == amount,
            "incorrect DUST sent"
        );
        uint256 _presold = presold[from];
        require(
        ...
        //  console.log("mint ", number_of_items);
        // console.log("max_purchase", info.max_mint);
        _mintDiscountCards(number_of_items, from);
        _split777(amount);
        emit DustPresale(from, number_of_items, info.dust_price);
