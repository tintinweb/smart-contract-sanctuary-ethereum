    ...
    SuperAdminControl
{
    ...
    */
    ...
    function activateLoan(
        ...
        // uint256 balance = IERC20(borrowerOffers[_loanId].borrowStableCoin).balanceOf(address(this));

        // console.log("balance stable %d", balance);

        /// @dev loan amount sending to borrower
        IERC20Upgradeable(borrowerOffers[_loanId].borrowStableCoin)
            .safeTransfer(borrowerOffers[_loanId].borrower, loanAmountAfterCut);

        // uint256 balanceAfter = IERC20(borrowerOffers[_loanId].borrowStableCoin).balanceOf(address(this));
    ...
    function fullLoanPaybackEarly(uint256 _loanId) internal {
        ...
        IERC20Upgradeable(borrowerOffers[_loanId].borrowStableCoin)
            .safeTransferFrom(
                borrowerOffers[_loanId].borrower,
                address(this),
                borrowerOffers[_loanId].loanAmountInBorrowed - paybackAmount
            );
        IERC20Upgradeable(borrowerOffers[_loanId].borrowStableCoin)
    ...
    */
        ...
        onlyLiquidatorRole(msg.sender)
    {
        ...
        if (lenderDetails.autoSell) {
            ...
            stableCoinWithdrawable[address(this)][
                loanDetails.borrowStableCoin
            ] += autosellFeeinStable + leftAmount + unEarnedAPYFee;
            ...
            if (
                ...
                uint256 exceedAltcoinValue = PriceConsumer.getStablePriceInETH(
                    loanDetails.borrowStableCoin,
                    collateralAmountinStable -
                        (loanDetails.loanAmountInBorrowed +
    ...
    function getAltCoinPriceinStable(
        ...
        if (
            ...
            int256 collateralInUsd = PriceConsumer
                .getNetworkPriceFromChainlinkinUSD();
            (int256 stableInUsd, ) = PriceConsumer.getLatestUsdPriceFromChainlink(_stableCoin);
            uint256 stableDecimals = IERC20Metadata(_stableCoin).decimals();
            uint256 collateralDecimals = IERC20Metadata(PriceConsumer.WETHAddress()).decimals();
            uint256 priceOfOneAltcoinTokeninStable = (uint256(collateralInUsd) * 10 ** stableDecimals) / uint256(stableInUsd);
            return (_collateralAmount * priceOfOneAltcoinTokeninStable) / 10 ** collateralDecimals;
...
interface IPriceConsumer {
    ...
    function getNumberofSunTokenFromStable(
        address _stable,
        address _claimToken,
        address _sunToken,
        uint256 _amount
