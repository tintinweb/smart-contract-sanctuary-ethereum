...
contract SpacecowsLottery is ReentrancyGuard, Ownable {
        ...
        nonReentrant
    {
        ...
        require(_numberOfTickets < selectLottery.maxTickets + 1, "Too many tickets");
     ...
     * @param _maxTickets: Max ticket allow to per transaction
    ...
    function startLottery(
        ...
        tmpLottery.maxTickets = uint32(_maxTickets);
     ...
     * @param _ticketPriceInSmilk: price of a ticket in SMILK
     */
    function setTicketPriceInSmilk(uint256 _lotteryId, uint256 _ticketPriceInSmilk)
        external
        onlyOwner
    {
     ...
     * @dev Only callable by owner
     */
    function setMaxNumberTicketsPerRound(uint256 _lotteryId, uint256 _maxNumberTicketsPerRound) external onlyOwner {
        require(_maxNumberTicketsPerRound != 0, "Must be > 0");

        Lottery storage tmpLottery = _lotteries[_lotteryId];
        tmpLottery.maxTickets = uint32(_maxNumberTicketsPerRound);
    }
     ...
     */
    function _playerTicketCountByLottery(uint256 _lotteryId, address _player) internal view returns (uint256) {
        Lottery memory currentLottery = _lotteries[_lotteryId];
        uint256 index = 0;
        for (uint256 i = 0; i < currentLottery.ticketId; ++i) {
            Player memory tmpPlayer = _tickets[_lotteryId][i];
