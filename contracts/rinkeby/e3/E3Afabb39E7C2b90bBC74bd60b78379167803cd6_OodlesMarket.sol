...
import "./mixins/OodlesTreasuryNode.sol";
import "./mixins/roles/OodlesAdminRole.sol";
    ...
    NFTMarketReserveAuction
{
    ...
    function initialize(address treasury) public initializer {
        OodlesTreasuryNode._initializeOodlesTreasuryNode(treasury);
    ...
    function adminUpdateConfig(
        ...
        uint256 duration,
        uint256 primaryOodlesFeeBasisPoints,
        uint256 secondaryOodlesFeeBasisPoints
    ) public onlyOodlesAdmin {
        _updateReserveAuctionConfig(minPercentIncrementInBasisPoints, duration);
        _updateMarketFees(
            primaryOodlesFeeBasisPoints,
            secondaryOodlesFeeBasisPoints
        );
    }
    ...
    /**
     * @notice Allows Oodles to update the treasury contract address.
     */
    function adminUpdateContract(address treasury) external onlyOodlesAdmin {
        _updateOodlesTreasury(treasury);
    }
}
...
/**
 * @notice A mixin that stores a reference to the Oodles treasury contract.
 */
abstract contract OodlesTreasuryNode is Initializable {
    using AddressUpgradeable for address;

    address private treasury;

    /**
     * @dev Called once after the initial deployment to set the Oodles treasury address.
     */
    function _initializeOodlesTreasuryNode(address _treasury)
        internal
        initializer
    {
        require(
            _treasury.isContract(),
            "OodlesTreasuryNode: Address is not a contract"
        );
        treasury = _treasury;
    }

    /**
     * @notice Returns the address of the Oodles treasury.
     */
    function getOodlesTreasury() public view returns (address) {
        return treasury;
    }

    /**
     * @notice Updates the address of the Oodles treasury.
     */
    function _updateOodlesTreasury(address _treasury) internal {
        require(
            _treasury.isContract(),
            "OodlesTreasuryNode: Address is not a contract"
        );
...
import "../OodlesTreasuryNode.sol";

/**
 * @notice Allows a contract to leverage an admin role defined by the Oodles contract.
 */
abstract contract OodlesAdminRole is OodlesTreasuryNode {
    // This file uses 0 data slots (other than what's included via OodlesTreasuryNode)

    modifier onlyOodlesAdmin() {
        require(
            IAdminRole(getOodlesTreasury()).isAdmin(msg.sender),
            "OodlesAdminRole: caller does not have the Admin role"
        );
        _;
    }

    function _isOodlesAdmin() internal view returns (bool) {
        return IAdminRole(getOodlesTreasury()).isAdmin(msg.sender);
    }
}
...
import "../interfaces/IOodles721.sol";
    ...
    ReentrancyGuardUpgradeable // Adding this unused mixin to help with linearization
{
    ...
    function _getCreator(address nftContract, uint256 tokenId)
        ...
        returns (address)
    {
        try IOodles721(nftContract).tokenCreator(tokenId) returns (
            address creator
        ) {
            return creator;
        } catch {
        ...
        )
    {
        ...
        try IOodles721(nftContract).royaltyInfo(tokenId, price) returns (
    ...
    SendValueWithFallbackWithdraw
{
    ...
    uint256 private _primaryOodlesFeeBasisPoints;
    uint256 private _secondaryOodlesFeeBasisPoints;
    ...
    {
        return (
            _primaryOodlesFeeBasisPoints,
            _secondaryOodlesFeeBasisPoints,
    ...
    {
        (OodlesFee, , creatorSecondaryFee, , ownerRev) = _getFees(
            nftContract,
            tokenId,
            _getSellerFor(nftContract, tokenId),
            price
        ...
        )
    {
        ...
        // SafeMath is not required when dividing by a constant value > 0.
        OodlesFee = price.mul(OodlesFeeBasisPoints) / BASIS_POINTS;
        ownerRev = price.sub(OodlesFee).sub(creatorSecondaryFee);
    }

    /**
     * @dev Distributes funds to oodles, creator, and NFT owner after a sale.
     */
        ...
        )
    {
        ...
        address ownerRevTo;
        (OodlesFee, creatorFeeTo, creatorFee, ownerRevTo, ownerRev) = _getFees(
            nftContract,
            tokenId,
            seller,
            price
    ...
    /**
     * @notice Allows Oodles to change the market fees.
     */
    function _updateMarketFees(
        ...
        require(
            primaryOodlesFeeBasisPoints < BASIS_POINTS,
            "NFTMarketFees: Fees >= 100%"
        );
        require(
            secondaryOodlesFeeBasisPoints < BASIS_POINTS,
            "NFTMarketFees: Fees >= 100%"
        );
        _primaryOodlesFeeBasisPoints = primaryOodlesFeeBasisPoints;
        _secondaryOodlesFeeBasisPoints = secondaryOodlesFeeBasisPoints;
...
import "./roles/OodlesAdminRole.sol";
    ...
    NFTMarketAuction
{
    ...
    /**
     * @notice Allows Oodles to cancel an auction, refunding the bidder and returning the NFT to the seller.
    ...
    /**
     * @notice Allows Oodles to approve the auction for listing in market place;
     */
    function approveAuctionByAdmin(
        uint256 auctionId,
        bool status,
        string memory reason
    ) external onlyOodlesAdmin {
        ReserveAuction memory auction = auctionIdToAuction[auctionId];
        require(
