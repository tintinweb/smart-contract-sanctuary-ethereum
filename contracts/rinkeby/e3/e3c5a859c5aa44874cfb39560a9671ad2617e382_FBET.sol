...
contract FBET is IERC20, protected
{
    ...
    function _distributeFeesMATIC(uint256 MATICamount) private {
        marketingBalance+=MATICamount;
    ...
    function _swapContractToken(uint256 totalMax) private lockTheSwap{
        ...
        uint256 liqMATICToken=tokenForLiquidity-liqToken;

        uint256 swapToken=liqMATICToken+tokenForMarketing;
        uint256 initialMATICBalance = address(this).balance;
        _swapTokenForMATIC(swapToken);
        uint256 newMATIC=(address(this).balance - initialMATICBalance);
        uint256 liqMATIC = (newMATIC*liqMATICToken)/swapToken;
        _addLiquidity(liqToken, liqMATIC);
        uint256 generatedMATIC=(address(this).balance - initialMATICBalance);
        _distributeFeesMATIC(generatedMATIC);
    }
    
    function _swapTokenForMATIC(uint256 amount) private {
        _approve(address(this), address(_UniswapRouter), amount);
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _UniswapRouter.WETH();

        _UniswapRouter.swapExactTokensForMATICSupportingFeeOnTransferTokens(
            amount,
            0,
            path,
            address(this),
    ...
    function _addLiquidity(uint256 tokenamount, uint256 MATICamount) private {
        totalLPMATIC+=MATICamount;
        _approve(address(this), address(_UniswapRouter), tokenamount);
        _UniswapRouter.addLiquidityMATIC{value: MATICamount}(
            address(this),
            tokenamount,
            0,
            0,
    ...
    function Team_WithdrawMarketingMATIC() public onlyAuth{
    ...
    function Control_SwitchManualMATICConversion(bool manual) public onlyAuth{
    ...
    function BETTING_place_bet_MATIC(uint256 id, uint256 side) public safe payable {
        ...
        require(!betters[msg.sender].banned, "Banned");
        require(bets[id].active, "Bet not active");
        if(bets[id].end_time > 7 minutes) {
            require(block.timestamp < (bets[id].end_time - 5 minutes), "Expired bet");
        } else {
            require(block.timestamp < (bets[id].end_time - 1 minutes), "Expired bet");            
        }
    ...
    function BETTING_declare_draft(uint256 id) public onlyAuth {
        require(bets[id].exists, "Bet does not exist");
        require(bets[id].active, "Bet is not active");
        bets[id].active = false;
        bets[id].winner_side = 9;
        bets_ended.push(id);
    }
    ...
    function BETTING_payout_bet(uint256 id) public safe returns(uint, string memory, string memory, bool) {
        ...
        uint payout_value;
        if(bets[id].winner_side==9) {
            payout_value = betters[msg.sender].bet_value[id]; // TODO 
        } else {
            require(betters[msg.sender].bet_side[id] == BETTING_get_winner(id), "Not a winner");
            // Getting the payout_value in percentage
            payout_value = _get_quote(betters[msg.sender].bet_value[id], id, betters[msg.sender].bet_side[id], payout_chain, payout_coin);
        }
