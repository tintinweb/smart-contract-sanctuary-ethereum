...
// File: contracts/Thetestfree.sol

/**
 * @title Thetestfree contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract Thetestfree is ERC721, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public startingIndexBlock;
    uint256 public startingIndex;
    uint256 public privateMintPrice = 0.0 ether;
    uint256 public publicMintPrice = 0.0 ether;
    uint256 public maxToMint = 1;
    uint256 public MAX_MINT_WHITELIST = 0;
    uint256 public MAX_ELEMENTS = 20;
    ...
    function mint(uint256 _count) public payable {
        ...
        if (privateSaleIsActive) {
            ...
            if (isWhitelisted(msg.sender)) {
                require((balanceOf(msg.sender) - whitelist[msg.sender].hasMinted + _count) <= maxToMint, "Can only mint 1 token");
            } else {
                require((balanceOf(msg.sender) + _count) <= maxToMint, "Can only mint 1 token");
            }
            require(
