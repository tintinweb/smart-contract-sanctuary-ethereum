...
contract BaseFixedPriceAuctionERC1155 is ERC1155, ReentrancyGuard, Ownable {
    ...
    mapping(uint256 => MintInfo) private mintInfos;
    ...
    constructor(
        ...
        ERC1155(_uri)
    {
        setMintInfo(_mintInfos);
        _splitter = new PaymentSplitter(payees, shares);
    }

    function setMintInfo(MintInfo[] memory _mintInfos) public onlyOwner {
        uint256 len = _mintInfos.length;
        MintInfo memory tmpInfo;
        uint256 _tokenId;
        for (uint256 i = 0; i < len; i ++) {
            tmpInfo = _mintInfos[i];
            _tokenId = tmpInfo.tokenId;
            mintInfos[_tokenId] = tmpInfo;
    ...
    function setPrice(uint256 _tokenId, uint256 _price) public onlyOwner {
        mintInfos[_tokenId].price = _price;
    }
    ...
    function setWhitelistMaxMint(uint256 _tokenId, uint256 _whitelistMaxMint) external onlyOwner {
        mintInfos[_tokenId].whitelistMaxMint = _whitelistMaxMint;
    }

    function setPublicListMaxMint(uint256 _tokenId, uint256 _publicListMaxMint) external onlyOwner {
        mintInfos[_tokenId].publicListMaxMint = _publicListMaxMint;
    }

    function mintCapsule(uint256 tokenId, uint256 numberOfTokens) external payable {
        require(_publicListClaimed[tokenId][msg.sender] + numberOfTokens <= mintInfos[tokenId].publicListMaxMint, 'You cannot mint this many.');
    ...
    function mintCapsuleWhitelist(bytes32 hash, bytes memory signature, uint256 tokenId, uint256 numberOfTokens) external payable {
        ...
        require(_whitelistClaimed[tokenId][msg.sender] + numberOfTokens <= mintInfos[tokenId].whitelistMaxMint, 'You cannot mint this many.');
    ...
    function _nonReservedMintHelper(uint256 tokenId, uint256 numberOfTokens) internal {
        require(numberOfTokens * mintInfos[tokenId].price == msg.value, "Invalid amount.");
    ...
    function mintReservedCapsule(uint256 tokenId) external onlyOwner {
        ...
        require(mintInfos[tokenId].reservedMax != 0, 'reserved not set');
        _mint(msg.sender, tokenId, mintInfos[tokenId].reservedMax, "");
        mintedAmounts[tokenId] = mintInfos[tokenId].reservedMax;
    }
