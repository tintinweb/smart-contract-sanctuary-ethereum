...
import {StreamPay} from "./StreamPay.sol";


contract StreamPayFactory {
    bytes32 constant INIT_CODEHASH = keccak256(type(StreamPay).creationCode);

    address public parameter;
    uint256 public getStreamPayContractCount;
    address[1000000000] public getStreamPayContractByIndex; // 1 billion indices

    event StreamPayCreated(address token, address streamPay);

    /**
        @notice Create a new Stream Pay Streaming instance for `_token`
        @dev Instances are created deterministically via CREATE2 and duplicate
            instances will cause a revert
        @param _token The ERC20 token address for which a Stream Pay contract should be deployed
        @return streamPayContract The address of the newly created Stream Pay contract
      */
    function createStreamPayContract(address _token) external returns (address streamPayContract) {
        ...
        streamPayContract = address(new StreamPay{salt: bytes32(uint256(uint160(_token)))}());
        // CREATE2 can return address(0), add a check to verify this isn't the case
        // See: https://eips.ethereum.org/EIPS/eip-1014
        require(streamPayContract != address(0));

        // Append the new contract address to the array of deployed contracts
        uint256 index = getStreamPayContractCount;
        getStreamPayContractByIndex[index] = streamPayContract;
        unchecked{
            getStreamPayContractCount = index + 1;
        }

        emit StreamPayCreated(_token, streamPayContract);
    }

    /**
      @notice Query the address of the Stream Pay contract for `_token` and whether it is deployed
      @param _token An ERC20 token address
      @return predictedAddress The deterministic address where the stream pay contract will be deployed for `_token`
      @return isDeployed Boolean denoting whether the contract is currently deployed
      */
    function getStreamPayContractByToken(address _token) external view returns(address predictedAddress, bool isDeployed){
