...
function apMint(uint256 quantity_) external payable whenNotPaused() {
    ...
    function revealRookieCollections(string memory tokenURI_) external onlyOwner
    {
       for (uint256 i = 1; i <= (rookieCounter / 32) * 32; i++)
       {
            if (revealedTokens[rookieTokenID[i]] == false)
            {   
    ...
    function revealVeteranCollections(string memory tokenURI_) external onlyOwner
    {
       for (uint256 i = 1; i < (veteranCounter / 32) * 32; i++)
       {
    ...
    function revealAllProCollections(string memory tokenURI_) external onlyOwner
    {
       for (uint256 i = 1; i < (allProCounter / 32) * 32; i++)
       {
