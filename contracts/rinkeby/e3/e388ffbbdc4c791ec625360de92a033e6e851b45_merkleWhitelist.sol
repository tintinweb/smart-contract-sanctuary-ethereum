...
contract merkleWhitelist is Delegated, ERC721EnumerableLite, PaymentSplitterMod {
    ...
    uint public tokenPrice = 0.125 ether;
    ...
    modifier whiteListMint {
        require(isPresaleActive, "Whitelist Mints are not enabled yet!");
        _;
    }
    ...
    function mint( uint amount_ ) external payable {
        ...
        require( msg.value >= tokenPrice * amount_, "Ether sent is not correct" );

        uint supply = totalSupply();
        require( supply + amount_ <= maxTokens, "Mint/order exceeds supply" );
    ...
    function mintTo(uint[] calldata amount_, address[] calldata recipient_) external payable onlyDelegates {
        ...
        totalQuantity += amount_[i];
        }
        require( supply + totalQuantity <= maxTokens, "Mint/order exceeds supply" );
    ...
    function resurrect( uint[] calldata tokenIds_, address[] calldata recipients_ ) external onlyDelegates{
        ...
        for(uint i; i < tokenIds_.length; ++i ){
        to = recipients_[i];
        require(recipients_[i] != address(0), "resurrect to the zero address" );
    ...
    function setPrice(uint whitelistPrice_, uint tokenPrice_ ) external onlyDelegates{
        require( whitelistPrice != whitelistPrice_ || tokenPrice != tokenPrice_, "New value matches old" );
    ...
    uint public maxWhitelistMint = 2; // maximum tokens that can be minted per whitelisted address
    ...
    function mintWhitelistMerkleTree(bytes32[] memory proof_, uint amount_) public payable onlySender whiteListMint {
        ...
        require(totalSupply() + amount_ <= maxTokens, "Mint request exceeds supply!");
