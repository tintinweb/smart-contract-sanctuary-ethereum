...
contract AlphaGangGenerative is ERC721A, Ownable {
    ...
    // if we go with SIGNER approach
    // address immutable SIGNER = 0x428C85205B0cb951b4e69687c702303384B3523B;
    // if we go with MerkleTree
    // bytes32 public merkle = 0x36bd448259415f8e16e833c018c45c26375ca9174b00186068a9e5bf37d94f7b;
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        ...
        require(!walletMints[msg.sender], "One pass per wallet");
        _;
    }
        ...
        returns (uint256)
    {
        ...
        // TODO do this check in AGStake to avoid reverts
        require(
     ...
     */
    function buyWhiteListI(bytes32[] calldata _merkleProof, bool _stake)
        external
        payable
        mintCompliance(1)
    {
        require(whiteListMintActive, "Sale is not active");
     ...
     *
     */
    function buyWhiteListII(bytes32[] calldata _merkleProof, bool _stake)
        external
        payable
        mintCompliance(2)
    {
     ...
     *
     */
    function buyWaitList(bytes32[] calldata _merkleProof, bool _stake)
        external
        payable
        mintCompliance(1)
    {
        require(waitListMintActive, "Sale is not active");
        require(msg.value >= PRICE, "Insufficient funds!");
        require(
            MerkleProof.verify(
                _merkleProof,
    ...
    /**
     * @dev toggles a state of sales
     *
     * Requirements:
     *
     * - `_saleId` must be in: [0, 1, 2]
     * - 0 - toggles OG sale
     * - 1 - toggles G2 White List sale
     * - 2 - toggles G2 Wait List sale
     * - `_state` bool value
     * - true to enable a sale type
     * - sales are set to false by default
     */
    function setSale(uint256 _saleId, bool _state) public onlyOwner {
        if (_saleId == 0) {
            ogMintActive = _state;
        }
        if (_saleId == 1) {
     ...
     *
     * - `_saleId` must be in: [0, 1, 2]
     * - 0 - sets a proof for { buyWhiteListI }
     * - 1 - sets a proof for { buyWhiteListII }
     * - 2 - sets a proof for { buyWaitList }
     * - `_state` bool value
     */
    ...
    // Withdraw 55%;
    // TODO set community wallet
    // owner wallet(55%): 0x1131C7cB07De89F93fb2aDDafad82F426Ab4e2E6
    // community wallet(45%): 0x0997C26fC2FdEC68627665a22ACC43c361f504Fe
    ...
    function setAGStakeAddress(address _agStake) external onlyOwner {
    ...
    */
    ...
    // Signature fn
    // function checkSignature(bytes memory signature, uint256 count)
    //     private
    //     view
    //     returns (bool)
    // {
...
contract ERC721A is IERC721A {
    ...
    @param _locked The status of the lock; true to lock, false to unlock.
  */
    function _lockAllTransfers(bool _locked) internal virtual {
