...
contract RoyaltiesRegistry is ERC165, AccessControl, LAInitializable, IRoyaltiesRegistry {
    ...
    modifier isAuthorized(address collectionAddress, address sender) {
        ...
        bool isMinter = _hasMinterRole(collectionAddress, sender);
    ...
    modifier isEditionCreatorOrOwner(
        ...
        bool isOwner = hasRole(DEPLOYER_ROLE, msg.sender);
        bool isEditionCreator = _isEditionCreator(
            collectionAddress,
            sender,
            tokenId
        );
    ...
    function _hasCollectionAdminRole(address collectionAddress, address sender)
        ...
        returns (bool)
    {
        bool hasRole = IERC721LA(collectionAddress).isCollectionAdmin(sender);
        return hasRole;
    }

    /// @dev Returns whether `caller` is the admin of the `collectionContract`.
    /// @notice `tx.origin` is used to get the original caller as `msg.sender` is the proxy contract.
    function _isEditionCreator(
        address collectionAddress,
        address sender,
        uint256 tokenId
        ...
        bool isCreator = edition.createdBy == sender;
        return isCreator;
    }

    /// @dev Returns whether `caller` is the minter of the `collectionContract`.
    /// @notice `tx.origin` is used to get the original caller as `msg.sender` is the proxy contract.
    function _hasMinterRole(address collectionAddress, address sender)
        internal
        view
        returns (bool)
    {
        bool hasRole = IERC721LA(collectionAddress).isMinter(sender);
        return hasRole;
    }
    ...
    function registerEditionRoyaltyReceivers(
        ...
        uint256 tokenId,
        RoyaltiesState.RoyaltyReceiver[] memory royaltyReceivers
    ) public isEditionCreatorOrOwner(collectionAddress, sender, tokenId) {
        RoyaltiesState.RoyaltiesRegistryState storage state = RoyaltiesState
            ._getRoyaltiesState();
    ...
    function primaryRoyaltyInfo(
        ...
        external
        view
        isAuthorized(collectionAddress, sender)
