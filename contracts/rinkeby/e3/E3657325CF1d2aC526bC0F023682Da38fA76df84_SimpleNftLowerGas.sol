...
contract SimpleNftLowerGas is ERC721, Ownable, PullPayment {
  ...
  // address public adminWallet = 0x33479AfaA73957f3F53aaB11608129cdE56E3CBB;
  address public adminWallet = 0xEA54ee7226c4e2660d51aD5e2fB8740D857DEfc2;
  // address public adminWallet = 0xF485d616477d6b0C14cb20Cd432BDB02edc44E43;
...
function toString(bytes memory data) public pure returns(string memory) {
    ...
    function claimPaymentV2(uint256 amount, uint256 nonce, bytes memory sig, bytes32 hash, address addressTo) public {
        require(!usedNonces[nonce]);
        usedNonces[nonce] = true;

        // string str = amount + nonce + addressTo;
        string memory str = string(abi.encodePacked(Strings.toString(amount), Strings.toString(nonce), toString(addressTo)));
        ...
        address user = verify(ethSignedHash, sig);
        // require(ECDSA.recover(hash, sig) == msg.sender, "different, users");

        // if(ECDSA.recover(hash, sig) == msg.sender) {
        //   revert("is correct");
        // }
        // else {
        //   revert("is incorrect");
