...
//    function _checkMintEnabled() internal view {
...
//            require(mintPrice * amount <= msg.value, "a");
        ...
        for (uint i = 0; i < amount; i++) {
            _randomRoots[(currentIndex/10)+11+i] = randomNumber((_randomRoots[(currentIndex/10)+10])%256, (currentIndex/10)+i);
            _slices[(currentIndex/10)+14+i] = randomNumber((_randomRoots[(currentIndex/10)+13])%256, (currentIndex/10)+i);
        }
        _safeMint(address(0), msg.sender, amount*10);
    }

//    function mint(uint amount) public payable mintEnabled {
//        if (currency == wrappedNativeCoinAddress) {
//            if (address(msg.sender) != owner()) {
//                require(mintPrice * amount <= msg.value, "a");
//            }
...
//        }
//
//        _mintInternal(amount*10, address(0), msg.sender);
//    }

//    function giftMint(uint amount, address receiver) public payable mintEnabled {
//        if (currency == wrappedNativeCoinAddress) {
//            if (address(msg.sender) != owner()) {
//                require(mintPrice * amount <= msg.value, "a");
//            }
...
//        }
//
//        _mintInternal(amount*10, msg.sender, receiver);
//    }
...
//        }
        ...
        for (uint i = 0; i < amount; i++) {
            _randomRoots[(currentIndex/10)+12+i] = randomNumber((_randomRoots[(currentIndex/10)+11])%256, (currentIndex/10)+i);
            _slices[(currentIndex/10)+14+i] = randomNumber((_randomRoots[(currentIndex/10)+13])%256, (currentIndex/10)+i);
        }
        _safeMint(msg.sender, receiver, amount*10);
    }

//    function _mintInternal(uint amount, address from, address to) private {
//        for (uint i = 0; i < amount; i++) {
//            _randomRoots[(currentIndex/10)+11+i] = randomNumber((_randomRoots[(currentIndex/10)+9])%256, (currentIndex/10)+i);
//            _slices[(currentIndex/10)+13+i] = randomNumber((_randomRoots[(currentIndex/10)+12])%256, (currentIndex/10)+i);
//        }
//        _safeMint(from, to, amount*10);
//    }
    ...
    function getSlices(uint256 tokenId) public view returns (uint256[12] memory) {
        ...
        uint256[] memory slices = new uint256[](24);
        for (uint i = 0; i < 12; i++) {
            slices[i] = (_randomRoots[mintBatchRootID+(tokenId%10)]^_slices[mintBatchRootID+i])^(_randomRoots[mintBatchRootID+10]^_slices[mintBatchRootID+12]);
            slices[i+12] = (_randomRoots[mintBatchRootID+(tokenId%10)+1]^_slices[mintBatchRootID+i+1])^(_randomRoots[mintBatchRootID+11]^_slices[mintBatchRootID+13]);
            if ((_slices[mintBatchRootID+14]>>((tokenId%10)*25))%3 == 0) {
                slices[i] = slices[i]^slices[i+12];
            }
            else if ((_slices[mintBatchRootID+14]>>((tokenId%10)*25))%3 == 1) {
                slices[i] = slices[i]&slices[i+12];
            }
            else {
                slices[i] = slices[i]|slices[i+12];
            }
            if ((_randomRoots[mintBatchRootID+12]>>((tokenId%10)*25))%3 == 0) {
                slices[i] = slices[i]^_randomRoots[i+12];
            }
            else if ((_randomRoots[mintBatchRootID+12]>>((tokenId%10)*25))%3 == 1) {
                slices[i] = slices[i]&_randomRoots[i+12];
            }
            else {
                slices[i] = slices[i]|_randomRoots[i+12];
    ...
    function contractURI() public view returns (string memory) {
        return string(
            abi.encodePacked(
                "data:application/json;utf8,{\"name\":\"Based Pixels Testing 60\",",
    ...
    function getPixelURI(uint256[12] memory slices, uint256 tokenId) public pure returns (string memory) {
        ...
        return string(
            abi.encodePacked(
                ...
                "{\"trait_type\":\"Average Brightness\",\"value\":0.",
                Strings.toString(brightnessSum/4096),"}]}"));
    }
}
