...
contract GAGARIN is ERC721A, Ownable {
  ...
  constructor(string memory baseURI) ERC721A("GAGARIN", "GG") {
    setBaseURI(baseURI);
  }
  ...
  function mint(uint256 _count) public payable saleIsOpen {
    ...
    uint256 counter = 0;
    uint256 totalAmount = balanceOf(msg.sender) + _count;
    ...
    for(uint256 i = balanceOf(msg.sender); i < totalAmount; i++) {
        if(i % 4 == 0) {
            counter++;
        }
    }

    uint256 discount = _count - counter;
    require(msg.value >= (mintPrice * discount), "Insufficient ETH amount sent.");
