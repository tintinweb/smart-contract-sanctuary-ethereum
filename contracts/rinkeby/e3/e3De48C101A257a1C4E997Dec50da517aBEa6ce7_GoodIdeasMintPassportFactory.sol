...
abstract contract AbstractMintPassportFactory is ERC1155Supply, ERC1155Burnable, Ownable {
...
// File: contracts/Copy_GoodIdeasMintPassFactory.sol
...
contract GoodIdeasMintPassportFactory is AbstractMintPassportFactory  {
    ...
    function editMintPass(
        ...
        mintPasses[_mpIndex].saleIsPublic = _saleIsPublic;
    ...
    function claimPublic(
        ...
        require(mintPasses[mpIndex].saleIsPublic, "Sale is not public");
        require(isValidClaim(numPasses,mpIndex));
    ...
    function claimWL(
        ...
        require(isValidClaim(numPasses,mpIndex), "Invalid Claim");
        require(verifyMerkleProof(merkleProof,mpIndex), "Invalid merkle proof");
    ...
    function isValidClaim(
        uint256 numPasses,
        uint256 mpIndex) internal view returns (bool) 
    {
         // verify contract is not paused
        require(mintPasses[mpIndex].saleIsOpen, "Sale is paused");
        // verify mint pass for given index exists
        require(mintPasses[mpIndex].mpExists, "Claim: Mint pass does not exist");
    ...
    function checkRoot(uint256 mpIndex) external onlyOwner view returns (bytes32) {
    ...
    function turnSaleOn(uint256 mpIndex) external onlyOwner {
         mintPasses[mpIndex].saleIsOpen = true;
    }

    function turnSaleOff(uint256 mpIndex) external onlyOwner{
         mintPasses[mpIndex].saleIsOpen = false;
    }

    function togglePublicSaleState(uint256 mpIndex) external onlyOwner{
        mintPasses[mpIndex].saleIsPublic = !mintPasses[mpIndex].saleIsPublic;
    }

    function editRedeemableContract(uint256 mpIndex, address _redeemableContract) external onlyOwner{
        mintPasses[mpIndex].redeemableContract = _redeemableContract;
    }
    ...
    function verifyMerkleProof(bytes32[] calldata merkleProof, uint256 mpIndex) public view returns (bool) {
        return MerkleProof.verify(merkleProof, mintPasses[mpIndex].merkleRoot, _hash(msg.sender));
    }
    ...
    function getClaimedMps(uint256 mpId, address userAdress) public view returns (uint256) {
        return mintPasses[mpId].claimedMPs[userAdress];
    }
