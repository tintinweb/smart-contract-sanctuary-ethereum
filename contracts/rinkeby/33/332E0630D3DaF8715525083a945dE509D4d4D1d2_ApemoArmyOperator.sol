...
contract ApemoArmyOperator is Operatorable, ReentrancyGuard {
    ...
    //Discounted Sale merkle roots
    mapping(DiscountedSaleLists => bytes32) discountedSaleMerkleRoots;

    //Root of the Merkle Tree of the free claim list
    ...
    //Discounted sale purchases cannot exceed allotment, which can never exceed 2.
    mapping(DiscountedSaleLists => mapping(address => uint8))
        public discountedSalePurchases;
    ...
    //Sets the merkle roots for discounted sale lists
    function setDiscountedSaleMerkleRoot(
        DiscountedSaleLists discountList,
        bytes32 merkleRoot
    ) external onlyOperator {
        discountedSaleMerkleRoots[discountList] = merkleRoot;
    }
    ...
    function purchase(uint256 count) external payable nonReentrant {
        ...
        require(currentPhase == 3, "Public sale has not begun yet");
    ...
    function allowListPurchase(
        uint256 count,
        uint256 allotment,
        DiscountedSaleLists list,
        bytes32[] calldata merkleProof
    ) external payable nonReentrant {
        require(purchaseState, "Purchase is disabled");
        require(
            discountedSalePurchases[list][msg.sender] + count <= allotment,
        ...
        if (list == DiscountedSaleLists.PUBLIC_SALE_LIST) {
            require(
                currentPhase == 2,
        ...
        if (list == DiscountedSaleLists.APELIST) {
            price = (price) / 2;
        } else if (
            list == DiscountedSaleLists.CREWMEN_LIST ||
            list == DiscountedSaleLists.TREVOR_JONES ||
            list == DiscountedSaleLists.HACKATAO
        ) {
            price = (price * 8) / 10;
        }
        ...
        require(
            MerkleProof.verify(
                merkleProof,
                discountedSaleMerkleRoots[list],
                keccak256(abi.encodePacked(msg.sender, allotment))
            ),
            "Sender address is not in that discounted sale list"
        );

        discountedSalePurchases[list][msg.sender] += uint8(count);
