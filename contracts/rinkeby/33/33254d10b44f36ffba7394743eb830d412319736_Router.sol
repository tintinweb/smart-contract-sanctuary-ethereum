...
contract Router {
    ...
    ICO private ico; // my ICO contract `is SpaceToken`
    ...
    constructor(address _icoAddress) {
        ico = ICO(_icoAddress);
        pool = new Pool("SpaceToken LP", "SPCLP", address(this), ico);
    }
...
contract Pool is ERC20 {
    ...
    constructor (string memory name, string memory symbol, address _router, ICO _ico) ERC20(name, symbol) { //TODO: double checking passing ICO like this is what I want
        router = _router;
        ico = _ico;
    }
...
contract ICO is SpaceToken {
    ...
    function ethWithdraw(address to) external {
//        if (phase != ICOPhase.OPEN) revert NotOpenPhase();
