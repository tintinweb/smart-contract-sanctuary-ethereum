...
contract CthulhuRisin is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  address public CrAddress;

  struct Mantra { 
      string message;
   }

  mapping (uint256 => Mantra) public pages;
  ...
  constructor(
    ...
    address _initCrAddress
  ) ERC721(_name, _symbol) {
    setBaseURI(_initBaseURI);
    setNotRevealedURI(_initNotRevealedUri);
    setCrAddress(_initCrAddress);
  }
  ...
  function claim() public {
    ...
    IERC721 token = IERC721(CrAddress);
    uint256 ownedAmount = token.balanceOf(msg.sender);
    require(ownedAmount >= requiredAmount, "You don't own enough Cthulhu Rising NFTs");
    uint256 supply = totalSupply();
    require(supply + 1 <= 7500, "Max supply reached");
    Mantra memory newPage = Mantra("");
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
      require(_exists(_tokenId),"ERC721Metadata: URI query for nonexistent token");
        Mantra memory currentPage = pages[_tokenId];
  ...
  function setMessage(uint256 _tokenId, string memory _newMessage) public {
    ...
    require(strBytes.length <= maxCharLimit, "Mantra is too long.");
    Mantra storage currentPage = pages[_tokenId];
  ...
  //only owner
  function setCrAddress(address _newAddress) public onlyOwner {
    CrAddress = _newAddress;
  }
