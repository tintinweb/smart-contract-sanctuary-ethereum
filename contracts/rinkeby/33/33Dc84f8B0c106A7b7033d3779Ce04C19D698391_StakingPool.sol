...
contract StakingPool is OwnableUpgradeable, ReentrancyGuard 
{
   ...
   // address private beneficiary;
    ...
    function initialize(  uint256 _firstPer,uint256 _secondPer,uint256 _thirdPer,uint256 _fourthPer,uint256 _minAmount, uint256 _maxAmount, address _ideaToken) external initializer {
    ...
    function updatefirstPercentage(uint256 _firstPer)external onlyOwner{
        firstPer = _firstPer;
    }

    function updateSecondPercentage(uint256 _secondPer)external onlyOwner{
        secondPer = _secondPer;
    }

    function updateThirdPercentage(uint256 _thirdPer)external onlyOwner{
        thirdPer = _thirdPer;
    }

    function updatefourthPercentage(uint256 _fourthPer)external onlyOwner{
        fourthPer = _fourthPer;
    }
    ...
    function getInfo() external view returns (uint256, uint256, address) {
        return (minAmount, maxAmount, ideaToken);
    }
    ...
    function register(uint256 _weiAmount, uint256 _Percentage) public{

        require(_weiAmount > 10000, "weiamount should be grater then 10000 can't be below 10000!");

        require(_Percentage == 2 && _Percentage ==3 && _Percentage == 4 && _Percentage ==5, "percentage doesnt matches the timeperiod");

        // if (_stakingPeriod == 30 days){
            if (firstPer == 2 && _Percentage == 2){
           stakingPeriod = block.timestamp + 300  seconds; 
           staking = 2;
        // }else if(_stakingPeriod == 90 days){
            }else if(secondPer == 3 && _Percentage == 3){
             stakingPeriod == block.timestamp + 500  seconds;
             staking = 3;
        // }else if(_stakingPeriod == 180 days){
            }else if(thirdPer == 4 && _Percentage == 4){
            stakingPeriod == block.timestamp + 800 seconds;
            staking = 4;
        // }else if(_stakingPeriod == 365 days){
             }else if(fourthPer == 5 && _Percentage == 5){
            stakingPeriod == block.timestamp + 1000 seconds;
            staking = 5;
        }
    ...
    //  function getPoolInfo() external view returns (uint256, uint256, uint256, uint256,uint256 poolId ){
    //       PoolInfo storage poolInfo = poolInfos[poolId];
    //       return ( PoolId.staking, poolInfos.stakingPeriod, poolInfos.weiAmount, poolInfos.base);
    ...
    function applyForStaking(uint256 poolId ) external {
        ...
        uint256 endTime = startTime + 300 seconds;
