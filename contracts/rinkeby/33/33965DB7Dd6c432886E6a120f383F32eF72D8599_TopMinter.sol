...
contract TopMinter is Ownable,ERC721Full,ReentrancyGuard,ITopMinterSetup,RadicalNFT {
    ...
    function setup_constructor(string calldata name,string calldata symbol,
        ...
        numWords =  1;
        uri_prefix="https://sandbox.minter.top/token/";
    }
...
;
    ...
    function getPOWMatchBytes()public view returns(bytes32){
         ...
         bytes32 h= keccak256(abi.encodePacked(msg.sender,address(this),curid));
         return h;
    ...
    function verifyPOW(uint256 pow_index)public view returns(bool){

        
        bytes32 h= keccak256(abi.encodePacked(msg.sender,pow_index));

        bytes32 c=getPOWMatchBytes();
        
         for (uint256 i = 0;i<POWMint; i++) {
             if(c[31-i]!=h[31-i]){
                 return false;
             }
        }
        ...
        external payable
    {
        ...
        require(verifyPOW(pow_index),"verifyPOW fail");
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        require(_exists(tokenId));
        return append(uri_prefix,_name,"/",tokenId);
    }
