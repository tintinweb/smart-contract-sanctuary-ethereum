...
contract RedEnvelop is Ownable {
    ...
    // Contract Fee
    uint16 public protocalFee = 824;
    mapping(address => uint256) public teamProfit;

    constructor() {
    }

    event Create(uint64 indexed envelopId, address indexed creator, uint256 money);
    ...
    function create(uint64 envelopId, address tokenAddr, uint256 money, uint16 count, bytes32 merkelRoot) external payable {
        ...
        uint256 profit = money / 100000 * protocalFee;
        require(money - profit >= count, "Invalid money");
     ...
     */
    function setProtocalFee(uint16 _protocalFee) external onlyOwner {
        require(_protocalFee <= 824, "Exceed 824");
        protocalFee = _protocalFee;
    }
}
