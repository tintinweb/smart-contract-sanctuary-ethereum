...
contract TCCGenesis is ERC721A, CollaborativeOwnable, PaymentSplitter, ReentrancyGuard {
  using SafeMath for uint256;
  using Address for address;
  using Strings for uint256;

  event Regenerated(uint256 tokenId);
  ...
  uint16 public mintLimit = 2;
  bool public mintIsActive = false;

  mapping(address => uint16) public whitelistMinted;
  bytes32 public whitelistMerkleRoot;
  uint256 public whitelistMintPrice = 90000000000000000;
  uint16 public whitelistMintLimit = 4;
  bool public whitelistMintIsActive = false;

  mapping(uint256 => bool) public regeneratedTokens;

  address[] payees = [
    0x3Bd412742678C7cd46dA5141d19d02Ffa41440E8,
    0x32388fd73DCF39Fd6E1a61A570Ec64a692954e6C
  ];
  ...
  constructor(address _proxyRegistryAddress)
    ERC721A("The Collectors Club", "TCC") 
  ...
  function mint(uint16 quantity) external payable nonReentrant {
    ...
    require(quantity > 0 && quantity <= mintLimit && quantity <= remainingForMint(), "quantity");
  ...
  function whitelistMint(bytes32[] calldata merkleProof, uint16 quantity) external payable nonReentrant {
    ...
    require(quantity > 0 && quantity <= whitelistMintLimit && quantity <= remainingForMint(), "quantity");
    require(msg.value >= whitelistMintPrice.mul(quantity), "value");
    
    uint16 alreadyMinted = whitelistMinted[_msgSender()];
    require(quantity + alreadyMinted <= whitelistMintLimit, "already");
  ...
  function regenerate(uint256 tokenId) external {
    require(_msgSender() == ownerOf(tokenId), "owner");
    require(!regeneratedTokens[tokenId], "regenerated");
    emit Regenerated(tokenId);
    regeneratedTokens[tokenId] = true;
  }
  ...
  function reduceMaxSupply(uint256 newMaxSupply) external onlyCollaborator {
    require(newMaxSupply < maxSupply, "max supply");
    require(newMaxSupply >= totalSupply(), "total supply");
    maxSupply = newMaxSupply;
  }

  function setMintPrice(uint256 newPrice) external onlyCollaborator {
    mintPrice = newPrice;
  }

  function setWhitelistMintPrice(uint256 newPrice) external onlyCollaborator {
    whitelistMintPrice = newPrice;
  }

  function airDrop(address to, uint16 quantity) external onlyCollaborator {
    require(to != address(0), "zero");
    require(quantity > 0 && quantity <= remainingForMint(), "quantity");
