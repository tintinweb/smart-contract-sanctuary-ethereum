...
contract wereWulfz is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public totalMaxSupply = 3333;
    ...
    mapping (address => uint256) internal walletNumberMintedPresale;

    constructor() ERC721A("WulfzVX", "WVX") {
    }

    //modifiers
    //checks WL eligibility
    modifier isValidMerkleProofTier1(bytes32[] calldata merkleProof) {
        require(MerkleProof.verify(merkleProof, whitelistMerkleRootTier1, keccak256(abi.encodePacked(msg.sender))),"Address does not exist in list");
        _;
    }

    modifier isValidMerkleProofTier2(bytes32[] calldata merkleProof) {
        require(MerkleProof.verify(merkleProof, whitelistMerkleRootTier2, keccak256(abi.encodePacked(msg.sender))),"Address does not exist in list");
        _;
    }
    //main minting functions

    //presale, public and reserve minting from ids 5556 (inclusive) onwards
    function tier1(bytes32[] calldata _merkleProof, uint256 _count) public nonReentrant isValidMerkleProofTier1(_merkleProof) payable {
        require(presaleActive, "Presale: not active");
        uint256 newCount = walletNumberMintedPresale[msg.sender].add(_count);
        require(newCount > 0, "Invalid purchase amount");
        require(newCount <= maxPerPresale, "Purchase would exceed max number of mints allocated");
        require(totalSupply().add(_count) <= totalMaxSupply - reservedAmount, "Mintable: supply exceeded"); 
        walletNumberMintedPresale[msg.sender] += _count;

        _safeMint(msg.sender, _count);
    }

    function tier2(bytes32[] calldata _merkleProof, uint256 _count) public nonReentrant isValidMerkleProofTier2(_merkleProof) payable {
        require(presaleActive, "Presale: not active");
        uint256 newCount = walletNumberMintedPresale[msg.sender].add(_count);
        require(newCount > 0, "Invalid purchase amount");
        require(newCount <= maxPerPresale, "Purchase would exceed max number of mints allocated");
   ...
   function publicMint(uint256 _count) public nonReentrant payable {
    ...
    function mintForTeam(uint256 _count) external onlyOwner {
