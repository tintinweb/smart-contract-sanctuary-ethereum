...
contract CryptoPhunksMarket is ReentrancyGuard, Pausable, Ownable {
            ...
            if (totalOffers == 0) {
                ...
                for (uint256 i = 0; i < totalOffers; i++) {
                    if (_offers[i].isForSale != false) {
                        resultOfToken[i] = _offers[i].phunkIndex;
                    }
                }
    ...
    function phunkNoLongerForSale(uint phunkIndex) public nonReentrant() {
        ...
        _offers[_tokenIdToOfferId[phunkIndex]] = _offers[_offers.length - 1];
        _offers.pop();
    ...
    function offerPhunkForSale(uint phunkIndex, uint minSalePriceInWei) public whenNotPaused nonReentrant()  {
        if (phunkIndex >= 10000) revert('token index not valid');
        if (phunksContract.ownerOf(phunkIndex) != msg.sender) revert('you are not the owner of this token');
        if (phunksContract.isApprovedForAll(msg.sender, address(this)) != true) revert('Please SetApprovalForAll before offering your Turtle for sale.');
        phunksOfferedForSale[phunkIndex] = Offer(true, phunkIndex, msg.sender, minSalePriceInWei, address(0x0));
        Offer memory _offer = Offer({isForSale: true, phunkIndex: phunkIndex, seller: msg.sender, minValue: minSalePriceInWei, onlySellTo: address(0x0)});
        phunksOfferedForSale[phunkIndex] = _offer;
        _offers.push(_offer);
        uint256 index = _offers.length - 1;
        _tokenIdToOfferId[phunkIndex] = index;
    ...
    function offerPhunkForSaleToAddress(uint phunkIndex, uint minSalePriceInWei, address toAddress) public whenNotPaused nonReentrant() {
        ...
        Offer memory _offer = Offer({isForSale: true, phunkIndex: phunkIndex, seller: msg.sender, minValue: minSalePriceInWei, onlySellTo: toAddress});
        phunksOfferedForSale[phunkIndex] = _offer;
        _offers.push(_offer);
        uint256 index = _offers.length - 1;
        _tokenIdToOfferId[phunkIndex] = index;
    ...
    function buyPhunk(uint phunkIndex) payable public whenNotPaused nonReentrant() {
        ...
        uint royalties = msg.value*1/20;

        pendingWithdrawals[address(0x5B38Da6a701c568545dCfcB03FcB875f56beddC4)] += royalties;
        pendingWithdrawals[seller] += msg.value*19/20;
    ...
    function acceptBidForPhunk(uint phunkIndex, uint minPrice) public whenNotPaused nonReentrant() {
        ...
        pendingWithdrawals[address(0x5B38Da6a701c568545dCfcB03FcB875f56beddC4)] += amount*1/20;
        pendingWithdrawals[seller] += amount*19/20;
