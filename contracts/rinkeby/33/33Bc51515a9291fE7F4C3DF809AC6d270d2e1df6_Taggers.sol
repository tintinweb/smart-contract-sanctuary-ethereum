...
contract Taggers is ERC721AQueryable, AccessControl, ReentrancyGuard, ERC2981 {
    ...
    //Price map
    mapping(uint256 => uint256) public collectionPriceMap;
    ...
    constructor(
        ...
        uint256[] memory collectionsPrices,
        ...
        for (uint256 index = 0; index < collectionsPrices.length; index++) {
            collectionPriceMap[index] = collectionsPrices[index];
        }
    ...
    modifier allowListActive() {
        require(isAllowListActive, "Mint is not active");
        _;
    }
    ...
    /**
     * @dev Update the price for a collection
     *
     * Requirements:
     *
     * - the caller must be an admin role
    ...
    function setCollectionPrice(
        ...
        collectionPriceMap[collectionIndex] = collectionPrice;
    }
    ...
    /**
     * @dev Set the active/inactive state of allow list minting
     *
     * Requirements:
     *
     * - the caller must be an admin role
     */
    function toggleAllowListActive() external onlyRole(DEFAULT_ADMIN_ROLE) {
        isAllowListActive = !isAllowListActive;
    }

    /**
     * @dev Set the active/inactive state of public minting
     *
     * Requirements:
     *
     * - the caller must be an admin role
     */
    function togglePublicMintActive() external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    /**
     * @dev Validate an order is signed by the order signer address
     */
    function _isValidOrder(
        string memory orderId,
        uint256[] memory collectionsQuantities,
        bytes memory signature,
        address sender
    ) internal view returns (bool isValid) {
        string memory message = orderId;
        for (uint256 index = 0; index < collectionsQuantities.length; index++) {
            message = string(
                abi.encodePacked(
                    message,
                    _toString(collectionsQuantities[index])
                )
            );
        }
        bytes32 messagehash = keccak256(abi.encodePacked(message, sender));
    ...
    /**
     * @dev mint `collectionsQuantities` to msg sender on allow list
     *
     * Requirements:
     *
    ...
    function allowListMint(
    ...
    ) external payable nonReentrant notPermanentlyLocked allowListActive {
        ...
        _mintTaggers(orderId, collectionsQuantities, signature);
    }

    /**
     * @dev mint `collectionsQuantities` to msg sender
     *
     * Requirements:
     *
     * - minting is not permanently locked
    ...
    function publicMint(
        ...
        uint256[] memory collectionsQuantities,
        bytes memory signature
    ) external payable nonReentrant notPermanentlyLocked publicMintActive {
        _mintTaggers(orderId, collectionsQuantities, signature);
    }
    ...
    function _mintTaggers(
        ...
        for (uint index = 0; index < collectionsQuantities.length; index++) {
            uint256 collectionQuantity = collectionsQuantities[index];
            totalQuantity += collectionQuantity;
            totalPrice += (collectionPriceMap[index] * collectionQuantity);
        }

        require(msg.value == totalPrice, "Invalid price");

        _mint(_msgSender(), totalQuantity);
    }
