...
abstract contract MemorizeThisDay is ERC1155Pausable, Ownable, VRFConsumerBase {
    ...
    modifier onlyElectoral() {
        require(msg.sender == _holders[0] || msg.sender == owner());
        _;
    }
    //End of Modifiers
    //Enums
    ...
    uint256 private constant DEVELOPER_AIRDROP_COUNT = 20;
    //MintingShares
    uint256 public constant MORALIS_COMMUNITY_MINT_SHARE = 20;
    uint256 public constant ELECTORAL_MINT_SHARE = 15;
    uint256 public constant FOUNDER_MINT_SHARE = 10;
    uint256 public constant DEVELOPERS_TEAM_MINT_SHARE = 9;
    ...
    string public _electoralLogoAddress;
    ...
    uint256 private _communityShare = 0;
    ...
    event PeriodicGiftWinnerSelectedEvent(
    ...
    constructor(
        ...
        _airdrop[_developerTeamShareAddress] = DEVELOPER_AIRDROP_COUNT;
        ...
        _firstInfluencerAddress = address(2);
        _secondInfluencerAddress = address(3);
        _founderShareAddress = address(4);
    ...
    function Memorize(
        ...
        if (!useGift) {
            ...
            _wallet[_holders[0]] = _wallet[_holders[0]] + electoralShare;
            ...
            uint256 smartContractFeesShare = (msg.value *
                SMARTCONTRACT_FEES_MINT_SHARE) / 100;
            _wallet[address(this)] =
                _wallet[address(this)] +
                smartContractFeesShare;

            _communityShare = (msg.value * COMMUNITY_MINT_SHARE) / 100;
            DistributeCommunityShare(_communityShare);

            uint256 moralisCommunityShare = (msg.value *
                MORALIS_COMMUNITY_MINT_SHARE) / 100;
            _wallet[_moralisCommunityAddress] =
                _wallet[_moralisCommunityAddress] +
                moralisCommunityShare;
    ...
    function SelectPeriodicWinner() public whenNotPaused {
        ...
        require(
            _periodicGiftRegisters.length > 0,
            "there is not any registers"
        );
        ...
        override
    {
        ...
        _airdrop[_periodicGiftRegisters[indexOfWinner]] =
            _airdrop[_periodicGiftRegisters[indexOfWinner]] +
            1;
        emit PeriodicGiftWinnerSelectedEvent(
            _periodicGiftRegisters[indexOfWinner],
            _randomness
        );
        _periodicGiftRegisters = new address[](0);
    }

    function DistributeCommunityShare(uint256 totalValue) private {
        uint256 eachShare = totalValue / 5;
        if (_mintedDays.length > 5) {
            for (uint256 i = 0; i < 5; i++) {
                ...
                _wallet[_holders[winnerIndex]] =
                    _wallet[_holders[winnerIndex]] +
                    eachShare;
                emit CommunitySelectedForMintPriceShareEvent(
                    _holders[winnerIndex]
                );
            }
        } else {
            _wallet[_developerTeamShareAddress] =
    ...
    function RegisterForPeriodiclyGift() public whenNotPaused {
        ...
        for (uint256 i = 0; i < _periodicGiftRegisters.length; i++) {
    ...
    function SetFounderAddress(address addrs, bool preserveBalance)
        public
        onlyOwner
    {
        if (preserveBalance) {
    ...
    function SetMoralisTeamAddress(address addrs, bool preserveBalance)
        public
        onlyOwner
    {
        if (preserveBalance) {
    ...
    function SetSecondInfluencerAddress(address addrs, bool preserveBalance)
        public
        onlyOwner
    {
        if (preserveBalance) {
    ...
    function SetFirstInfluencerAddress(address addrs, bool preserveBalance)
        public
        onlyOwner
    {
        if (preserveBalance) {
    ...
    ) public onlyElectoral {
        _electoralLink = link;
        _electoralTitle = title;
        _electoralLogoAddress = logoLink;
    }

    fallback() external payable {
        _wallet[address(this)] = _wallet[address(this)] + msg.value;
    }
