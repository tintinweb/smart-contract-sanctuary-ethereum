...
contract animalMain is  ERC721,Ownable {
  ...
  function transfer(address _to, uint256 _tokenId) public {
    require(animalToOwner[_tokenId] == msg.sender); //TO DO 請使用require判斷要轉的動物id是不是轉移者的
    
    ownerAnimalCount[_to] = ownerAnimalCount[_to].add(1);//增加受贈者的擁有動物數量
    ownerAnimalCount[msg.sender] = ownerAnimalCount[msg.sender].sub(1);//減少轉出者的擁有動物數量
    
    animalToOwner[_tokenId] = _to;//動物所有權轉移
  ...
  function createAnimal() public {
      ...
      //動手玩創意，可以限制每次建立動物需要花費多少ETH
       dna = keccak256(abi.encodePacked(star,now));
       star = uint(dna)%5+5;
       roletype = uint(dna)%8+1;
