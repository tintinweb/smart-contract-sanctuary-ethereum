...
contract Swap is BaseContract
{
    ...
    function buy(uint256 paymentAmount_) external whenNotPaused returns (bool)
    {
        ...
        _swap(_in_, _out_, paymentAmount_, msg.sender);
        return true;
    }
    ...
    function _depositBuy(uint256 paymentAmount_, address referrer_) internal returns (bool)
    {
        ...
        uint256 _amount_ = _swap(_in_, _out_, paymentAmount_, address(_vault_));
    ...
    function sell(uint256 sellAmount_) external whenNotPaused returns (bool)
    {
        ...
        _swap(_in_, _out_, sellAmount_, msg.sender);
        return true;
    }
     ...
     */
    function _swap(IERC20 in_, IERC20 out_, uint256 amount_, address receiver_) internal returns (uint256)
    {
        IUniswapV2Router02 _router_ = IUniswapV2Router02(addressBook.get("router"));
        require(address(_router_) != address(0), "Router not set");
        require(in_.transferFrom(receiver_, address(this), amount_), "In transfer failed");
