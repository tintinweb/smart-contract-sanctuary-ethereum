...
contract PrimezExchange is Ownable {
    ...
    //contractList: ERC721A, ERC1155
    address[2] public contractList;
    ...
    constructor(address[2] memory _erc721) public {
        contractList = _erc721;
        whitelistAddress[msg.sender] = true;
        admin = msg.sender;
    }

    function setNFTAddress(address _nft, uint256 index) public onlyOwner {
        contractList[index] = _nft;
    }
    ...
    function buyNFTNormal(
        ...
        IERC721(contractList[0]).safeTransferFrom(
            tradeInfo.tradeAddress[1],
            msg.sender,
    ...
    function auctionNFT(
        ...
        if (tradeInfo.tradeAddress[1] == admin) {
            require(
                IERC721(contractList[0]).isApprovedForAll(admin, address(this)),
                "tokenId do not approve for contract"
            );
        } else {
            require(
                IERC721(contractList[0]).getApproved(tradeInfo.attributes[1]) == address(this),
                "tokenId do not approve for contract"
            );
        }
    ...
    function buyERC1155Normal(
        ...
        IERC1155(contractList[1]).safeTransferFrom(
            tradeInfo.tradeAddress[1],
            msg.sender,
            tradeInfo.attributes[1],
    ...
    function acceptOfferERC1155(
        ...
        require(
            IERC1155(contractList[1]).isApprovedForAll(tradeInfo.tradeAddress[1], address(this)),
            "tokenId do not approve for contract"
        );
        // check allowance of buyer
        require(
