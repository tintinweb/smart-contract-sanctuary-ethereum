...
contract SecretDelay is Modifier {
  ...
  /// @param _nonce nonce where the txNonce should point
  /// @notice This can only be called by the owner
  function vetoTransactions(uint256 _nonce) public onlyOwner {
    require(_nonce > txNonce, "New nonce must be higher than current txNonce");
    require(_nonce <= queuePointer, "Cannot be higher than queuePointer");
    emit TransactionsVetoed(txNonce, _nonce-txNonce);
    txNonce = _nonce;
  }
