...
contract KNTest is ERC721, ERC721Pausable, Ownable {
    ...
    // TODO: remove this
    event LogA(address indexed _value);
    event LogB(bytes32 indexed _value);
    event LogC(bytes indexed_value);
    ...
    function isSigOk(address to, bytes memory signature) internal view returns (bool) {
        bytes32 digest = generateDigest(to);
        bytes32 ethdigest = digest.toEthSignedMessageHash();
        address signkey = _recoverSigner(ethdigest, signature);
        require(signkey != address(0), "Invalid signature, zero");
        require(signkey == pkey, "Invalid signature");
        return true;
    }

    function mint(address to, uint numberOfTokens, bytes memory signature) public whenNotPaused payable {
        ...
        require(isSigOk(to, signature), "Invalid signature");
