...
// File: contracts/YieldingUPU.sol
...
interface IUPUX is IERC721 {}

contract YieldToken is ERC20("UPUX Token", "UPUX") {
	...
	function updateRewardOnMint(address _user, uint256 _amount) external {
		require(msg.sender == address(_UPUXContract), "Can't call this");
		uint256 time = min(block.timestamp, END);
		uint256 timerUser = _lastUpdate[_user];

		if (timerUser > 0)
			_rewards[_user] = _rewards[_user].add(_UPUXContract.balanceOf(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(86400)
	...
	function updateReward(address _from, address _to, uint256 _tokenId) external {
		require(msg.sender == address(_UPUXContract));

		uint256 time = min(block.timestamp, END);
		uint256 timerFrom = _lastUpdate[_from];

		if (timerFrom > 0)
			_rewards[_from] += _UPUXContract.balanceOf(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(86400);
		...
		if (_to != address(0)) {
			...
			if (timerTo > 0)
				_rewards[_to] += _UPUXContract.balanceOf(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(86400);
	...
	function getTotalClaimable(address _user) external view returns(uint256) {
		uint256 time = min(block.timestamp, END);
		uint256 pending = _UPUXContract.balanceOf(_user).mul(BASE_RATE.mul((time.sub(_lastUpdate[_user])))).div(86400);

		return _rewards[_user] + pending;
	}
}

abstract contract YieldingUPU is ERC721, Ownable {
