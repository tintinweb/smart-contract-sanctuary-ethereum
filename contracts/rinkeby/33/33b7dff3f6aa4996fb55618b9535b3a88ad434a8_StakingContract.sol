...
contract StakingContract is Ownable {
    ...
    constructor(address _genAddress, address _rewardTokenAddress, uint256 _RewardTokenPerBlock) {
        require (_genAddress != address(0), "NFT token can't be adress (0)");
    ...
    function changeAPETokenAddress(address _genAddress) public onlyOwner {
        genAddress = _genAddress;
    }
    ...
    function contractBalance() public view returns(uint256){
        return IERC721(genAddress).balanceOf(address(this));
    }
    ...
    function stake(uint256[] memory tokenIds) public {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(IERC721(genAddress).ownerOf(tokenIds[i]) != msg.sender) continue;

            IERC721(genAddress).transferFrom(address(msg.sender), address(this), tokenIds[i]);
    ...
    function unstake(uint256[] memory tokenIds) public {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            (bool _isStaked,) = getStakingItemInfo(msg.sender, tokenIds[i]);
            if(!_isStaked) continue;
            if(IERC721(genAddress).ownerOf(tokenIds[i]) != address(this)) continue;
            ...
            if(stakingAmount[msg.sender] > 0)
                stakingAmount[msg.sender] = stakingAmount[msg.sender] - 1;
            IERC721(genAddress).transferFrom(address(this), msg.sender, tokenIds[i]);
