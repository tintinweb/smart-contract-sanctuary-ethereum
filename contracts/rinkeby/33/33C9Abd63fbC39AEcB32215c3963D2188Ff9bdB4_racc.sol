...
/// @author Phillip
/// @notice NFT Minting
contract racc is ERC721A, Ownable, ReentrancyGuard {
    ...
    address [] teamWalletList = [
       0x81a96dDC6c4042d8FDF75e38E31A0b43684D3E5D,         // Wallet 1 address
       0x1697090Ca7F731970Bf1648064380A7963081EC6         // Wallet 2 address
    ];
    
    mapping (address => uint8) teamWalletPercent;
    
    // Mint Counter for Each Wallet
    mapping (address => uint8) addressFreeMintCountMap;      // Up to 4
    mapping (address => uint8) addressPreSaleCountMap;       // Up to 4
    mapping (address => uint8) addressPublicSaleCountMap;    // Up to 6
    
    // Minting Limitation
    uint16 public freeMintLimit = 30;
    uint16 public preSaleLimit = 40;
    uint16 public publicSaleLimit = 30;
    uint16 public totalLimit = 100;
    ...
    // Mint Price
    uint public mintPriceDiscount = 0.12 ether;
    uint public mintPrice = 0.15 ether;

    // BaseURI (real, placeholder)
    string private realBaseURI = "https://gateway.pinata.cloud/ipfs/QmTELaH2VmJhvgYYWDvYWZjXgUNmUQwBnHsefepU5YWWEy/";
    string private placeholderBaseURI  = "https://gateway.pinata.cloud/ipfs/QmTELaH2VmJhvgYYWDvYWZjXgUNmUQwBnHsefepU5YWWEy/";

    uint8 private LIMIT4 = 4;
    uint8 private LIMIT6 = 6;

    constructor(uint256 maxBatchSize_, uint256 collectionSize_) 
        ERC721A("racc", "RACC", maxBatchSize_, collectionSize_) {
        maxBatchSize = maxBatchSize_;
        collectionSize = collectionSize_; 
       teamWalletPercent[teamWalletList[0]] = 50;         // Wallet 1 percent
        teamWalletPercent[teamWalletList[1]] = 50;         // Wallet 2 percent
    }
     ...
     * mintCount:   Up to 2
     */
    function mintFree(uint8 _mintCount, bytes32[] memory _proof) external nonReentrant returns (uint256) {
        
        require(mintStep == 0 && _mintCount > 0 && _mintCount <= LIMIT4);
        require(msg.sender != address(0));
        require(addressFreeMintCountMap[msg.sender] + _mintCount <= LIMIT4);
        require(_mintCount <= freeMintLimit);
        require(verifyWhitelist(_leaf(msg.sender), _proof) == true);
        
        require(
            _mintCount % maxBatchSize == 0,
     ...
     * mintStep:    1: free mint
     * mintCount:   Up to 4
     */
    function mintPresale(uint8 _mintCount, bytes32[] memory _proof) external payable nonReentrant returns (uint256) {
        
        require(_mintCount > 0 && _mintCount <= LIMIT4);
        require(msg.sender != address(0));
        require(addressPreSaleCountMap[msg.sender] + _mintCount <= LIMIT4);
        require(       // Presale 
                    mintStep == 1 
                    && (_mintCount <= preSaleLimit)
    ...
    function mintPublic(uint8 _mintCount) external payable nonReentrant returns (uint256) {
        ...
        require(mintStep == 2 && _mintCount > 0 && _mintCount <= LIMIT6);
        require(msg.sender != address(0));
        require(msg.value == (mintPrice * _mintCount));
        require(addressPublicSaleCountMap[msg.sender] + _mintCount <= LIMIT6);
    ...
    function setMintStep(uint8 _mintStep) external onlyOwner returns (uint8) {
        ...
        mintStep = _mintStep;
        emit Setting(mintStep, mintPrice, mintPriceDiscount, totalLimit, LIMIT4, LIMIT6);
        return mintStep;
    }
