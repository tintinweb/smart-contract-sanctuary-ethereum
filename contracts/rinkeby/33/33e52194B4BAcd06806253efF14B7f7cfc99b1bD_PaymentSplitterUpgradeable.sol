...
import "./INaxapNFT.sol";
import "./NaxapNFT.sol";
import "./NaxapMarket.sol";
...
contract PaymentSplitterUpgradeable is Initializable, ContextUpgradeable {
    ...
    initializer
  {
    ...
    fractionsIndex = NaxapNFT(_msgSender()).fractionsIndex();
    buyout = NaxapNFT(_msgSender()).sold();
  ...
  function release() external virtual {
    ...
    if (buyout) {
      // uint256 bal = INaxapNFT(tokenParent).getFractions(_msgSender());
      uint256 bal = NaxapNFT(tokenParent).balanceOf(_msgSender(),NaxapNFT(tokenParent).fractionsIndex());
      INaxapNFT(tokenParent).soldBurn(_msgSender(), fractionsIndex, bal);
    }
    ...
    uint16 marketFee = NaxapMarket(marketPayable).fee();
...
interface INaxapNFT {
    function fractionalize(address _to, uint _tokenId) external;
    function setMajority(uint16 newValue) external;
    function defractionalize() external;
    ...
    function getFractions(address who) external view returns(uint);
    ...
    function getFractionsIndex() external view returns (uint256);
}
...
contract NaxapNFT is ERC1155Upgradeable,ERC721HolderUpgradeable,ERC1155HolderUpgradeable{
  using EnumerableSet for EnumerableSet.AddressSet;
  using EnumerableMap for EnumerableMap.UintToAddressMap;
  address revenueChannelImplementation;
  bool fractionalized;
  bool public sold;
  uint256 public fractionsIndex;
  ...
  event Fractionalized(address holder, address minter, uint256 index);
  event Defractionalized(address holder, uint256 index);
  ...
  // User Functions
  ///////////////////////////
  function fractionalize(address _to, uint256 _tokenId) external {
    require((_tokenId != 0) && 
    ...
    // require(fractionalized == false);
    // require(indexUsed[_tokenId] == false);
    fractionalized = true;
    sold = false;
    fractionsIndex = _tokenId;
    _mint(_to, _tokenId, 10000*10**18, "");
    emit Fractionalized(_msgSender(), _to, _tokenId);
  }

  function defractionalize() external {
    fractionalized = false;
    _burn(_msgSender(), fractionsIndex, 10000*10**18);
    emit Defractionalized(_msgSender(), fractionsIndex);
  }
  ...
  function lockSharesTransfer(
    ...
    require(
      balanceOf(from, fractionsIndex) - lockedShares[fractionsIndex][from] >=
        numShares
    );
    lockedShares[fractionsIndex][from] += numShares;
    lockedToTotal[fractionsIndex][_to] += numShares;
  ...
  function unlockSharesTransfer(address from, address _to) external {
    ...
    uint256 balance = lockedShares[fractionsIndex][from];
    lockedShares[fractionsIndex][from] -= balance;
    lockedToTotal[fractionsIndex][_to] -= balance;
  ...
  function createRevenuePayment(address _marketAddress) external payable returns (address _clone) {
    ...
    uint256[] memory fractions = new uint256[](listLength);
    for (uint256 i = 0; i < listLength; i++) {
      fractions[i] = this.balanceOf(owners[i], fractionsIndex);
    }
    _clone = ClonesUpgradeable.clone(revenueChannelImplementation);
    address payable revenueContract = payable(_clone);
    PaymentSplitterUpgradeable(revenueContract).init(owners, fractions, _marketAddress);
  ...
  function sellItem() external payable {
    ...
    sold = true;
    fractionalized = false;
    indexUsed[fractionsIndex] = true;
    emit ItemSold(_msgSender(), fractionsIndex);
  }

  function cleanUpHolders() internal {
    uint16 cur = 0;
    while(cur < holders.length()){
      if (this.balanceOf(holders.at(cur), fractionsIndex) < 1) {
  ...
  function _beforeTokenTransfer(
    ...
    if (from != address(0) && to != address(0)) {
      if (tokenId[0] == 0) {
        if (fractionalized == true && sold == false) {
          require((lockedToTotal[fractionsIndex][to] > 9999));
        }
      } else {
        require(sold != true);
        require(
          (balanceOf(from, tokenId[0]) - lockedShares[fractionsIndex][from] >=
            amount[0])
        );
      }
      holders.add(to);
  ...
  // function getFractions(address who) external view returns (uint256) {
  //   return this.balanceOf(who, fractionsIndex);
  // }
  ...
  // }
  /**
  *@notice transfer contained ERC721 to the Naxap owner with given address and index
  ...
  *@param index index of the ERC721
  ...
  function claimContainedERC721(address contractAddress, uint256 index) external{
    ...
    require((this.balanceOf(msg.sender, 0) == 1) && !fractionalized && (IERC721(contractAddress).ownerOf(index) == address(this)));
    // require(fractionalized==false);
  ...
  /**
  *@notice transfer contained ERC1155 to the Naxap owner with given address and index
  ...
  *@param index index of the ERC1155
  ...
  function claimContainedERC1155(address contractAddress, uint256 index, uint256 amount) external{
    ...
    require((this.balanceOf(msg.sender, 0) == 1) && !fractionalized);
...
ERC1155Holder
{
  ...
  event NaxapClaimed(address owner, address tokenAddress);
  ...
  function redeemAuctionSeller(
    ...
    //auction failed
      ...
      NaxapNFT(_tokenAddress).safeTransferFrom(
      address(this),
      _msgSender(),
      NaxapNFT(_tokenAddress).fractionsIndex(),
  ...
  function redeemAuctionParticipant(
    ...
    uint256 _auctionReserve = auctionReserve[_seller][_sellerNonce];
    uint256 fractionsIndex = NaxapNFT(_tokenAddress).fractionsIndex();
    //auction successful
    //give participant fractions according to their contribution
    if(_auctionReserve>=auctionListing.reservePrice){
      uint256 auctionNxps = auctionListing.numberOfShares;
      uint256 _participantContribution = participantContribution[_seller][_sellerNonce][_msgSender()];
      uint256 eligibleNxp = (_participantContribution * auctionNxps) / _auctionReserve;
  ...
  function importNaxap(address tokenAddress, uint256 fractionsIndex)
    external
  {
    NaxapNFT(tokenAddress).safeTransferFrom(
      _msgSender(),
      address(this),
      0,
      1,
    ...
    NaxapNFT(tokenAddress).fractionalize(_msgSender(), fractionsIndex);
    NaxapNFT(tokenAddress).lockSharesTransfer(
      _msgSender(),
      10000*10**18,
      address(this)
    );
    NaxapNFT(tokenAddress).unlockSharesTransfer(_msgSender(), address(this));
  }
  ...
  function buyFractions(
    ...
    require(!NaxapNFT(tokenAddress).sold(), "item sold");
    require(
      listing.numberOfShares >= _numberOfShares
    );//"Not enough Fractions on sale"
    ...
    uint256 fractionsIndex = NaxapNFT(tokenAddress).fractionsIndex();
    require(msg.value >= buyPrice);//"NaxapMarket: insufficient funds"
  ...
  function listItem(
    ...
    uint256 fractionsIndex = NaxapNFT(_tokenAddress).fractionsIndex();
    require(
      NaxapNFT(_tokenAddress).balanceOf(address(this), 0) == 1
    );// "nft not in market"
    require(!NaxapNFT(_tokenAddress).sold());//"item sold"
    require(
      NaxapNFT(_tokenAddress).balanceOf(_msgSender(), fractionsIndex) >=
        _numberOfShares
    );//"no valid fractions"
  ...
  function exportNaxap(address tokenAddress) public {
    uint256 fractionsIndex = NaxapNFT(tokenAddress).fractionsIndex();
    NaxapNFT(tokenAddress).safeTransferFrom(_msgSender(), address(this), fractionsIndex, 10000*10**18, '');
    NaxapNFT(tokenAddress).defractionalize();
    NaxapNFT(tokenAddress).safeTransferFrom(address(this), _msgSender(), 0, 1, '');
  }
  ...
  function rejectOffer(address from, address to, address tokenAddress) external {
      NaxapNFT(tokenAddress).unlockSharesTransfer(
      from,
      to
    );
  }
  ...
  function voteOffer(address offerer, address tokenAddress) external {
    ...
    uint256 lockedShares = NaxapNFT(tokenAddress).lockedShares(fractionsIndex,_msgSender());
    uint256 votesAvailable = NaxapNFT(tokenAddress).balanceOf(
      _msgSender(),
      fractionsIndex
    ) - lockedShares;
    NaxapNFT(tokenAddress).lockSharesTransfer(
    ...
    uint256 lockedToOfferer = NaxapNFT(tokenAddress).lockedToTotal(fractionsIndex,offerer);
    bool sold = false;
    if (lockedToOfferer > NaxapNFT(tokenAddress).majority()) {
      NaxapNFT(tokenAddress).sellItem();
      offer.winner = true;
      sold = true;
    }
  ...
  function claimNaxap(address tokenAddress) external {
    uint256 fractionsIndex = NaxapNFT(tokenAddress).fractionsIndex();
    if (NaxapNFT(tokenAddress).sold()) {
      Proposal memory offer = offers[_msgSender()][tokenAddress];
      require(
        NaxapNFT(tokenAddress).lockedToTotal(fractionsIndex,_msgSender())
         > NaxapNFT(tokenAddress).majority(),
        "not buyer"
      );
      NaxapNFT(tokenAddress).createRevenuePayment{ value: offer.value }(address(this));
    ...
    emit NaxapClaimed(_msgSender(), tokenAddress);
  }
