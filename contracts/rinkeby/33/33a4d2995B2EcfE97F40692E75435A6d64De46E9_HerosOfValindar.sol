...
contract HerosOfValindar is ERC721, Ownable, ReentrancyGuard, ProxyRegistry, RandomlyAssigned {
  ...
  function whitelistMint(uint256 tokenAmount, bytes32[] calldata _merkleProof) public payable isValidMerkleProof(_merkleProof) ensureAvailabilityFor(tokenAmount) {
    require(!paused, "Contract is Paused");
    require(whitelistEnabled, "Whitelist is NOT enabled");
    require(maxSupply >= tokenAmount + totalSupply(), "Supply Limit Reached");
  ...
  function publicMint(uint256 tokenAmount) public payable ensureAvailabilityFor(tokenAmount) {
    require(!paused, "Contract is Paused");
  ...
  //  function airdrop(address receiver, uint256 amount) public onlyOwner nonReentrant ensureAvailabilityFor(amount) {
  //       require(maxSupply >= amount + totalSupply(), "Supply limit");
  //       require(
  ...
  function setAllocationLimit(uint256 _whitelistLimit) public onlyOwner {
