...
// TODO: assert not contract depositor
  ...
  /// @notice Deposit `_value` tokens for `_tokenId` and add to the lock
    ...
    function _balanceOfNFT(uint _tokenId, uint _t) internal view returns (uint) {
        ...
        if (_epoch == 0) {
            ...
            uint unweighted_supply = uint(uint128(last_point.bias));
            uint weighted_supply = last_point.amt + (VOTE_WEIGHT_MULTIPLIER * unweighted_supply);
    ...
    function _balanceOfAtNFT(uint _tokenId, uint _block) internal view returns (uint) {
        ...
        uint unweighted_supply = uint(uint128(upoint.bias));
        uint weighted_supply = upoint.amt + (VOTE_WEIGHT_MULTIPLIER * unweighted_supply);
    ...
    function _checkpoint(
        ...
        if (_epoch > 0) {
            ...
            last_point.amt = IERC20(poolToken).balanceOf(address(this));
        }
