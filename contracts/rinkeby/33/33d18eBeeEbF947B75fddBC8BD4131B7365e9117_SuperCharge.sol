...
contract SuperCharge is AccessControl,Initializable{
    ...
    struct superChargeReward{
        uint rewardAmount;
        uint totalStaked;
        mapping(address => uint) previousStakedAmount; 
    ...
    function setUserStateWithDeposit(address user,uint amount) external{
        ...
        superChargeReward storage newSuperChargeReward = superChargeRewards[superChargeCount];
        if(!newSuperChargeReward.isEligible[user]){
            newSuperChargeReward.previousStakedAmount[user] = amount;
    ...
    function stakedAmountForReward(address user, uint superChargeId, uint stakedAmount) public view returns(uint staked){
        if(superChargeRewards[superChargeId].previousStakedAmount[user] != 0){
            staked = superChargeRewards[superChargeId].previousStakedAmount[user];
        }else if(superChargeRewards[superChargeId].previousStakedAmount[user] == 0 && superChargeRewards[superChargeId].isEligible[user]){
            staked = 0;
        }else{
            staked = stakedAmount;
        }
    ...
    function userRewards(address user, uint stakedAmount) public view returns(uint amount, uint end){
        if(userDetails[user].staked && userDetails[user].superChargeId < superChargeCount){
            end = (superChargeCount - userDetails[user].superChargeId) > 150 ? userDetails[user].superChargeId + 150 : superChargeCount;
            for(uint i = userDetails[user].superChargeId+1; i <= end; i++){
                uint staked = stakedAmountForReward(user, i, stakedAmount);
                if(staked != 0){
    ...
    function claimSuperCharge(address user) external{
        require(canClaim(user),"noting to claim");
   ...
   function setAdminContract(address admin_) external  {
        _onlyAdmin();
        admin = IAdmin(admin_);
    }

    function setIonContract (address Ion_) external {
        _onlyAdmin();
        ION = IERC20(Ion_);
    }
