...
contract DiamondFacetsRegistry is IDiamondFacetsRegistry {
    ...
    function ensureFacetDataIsNotEmpty(FacetCutData[] memory _facetData)
        internal
        pure
    {
    ...
    modifier isValidId(bytes32 _facetId) {
        if (_facetId == bytes32(0)) {
            revert InvalidId(_facetId);
        }
        _;
    }

    modifier facetsAreNotEmpty(FacetCutData[] memory _facetData) {
        ensureFacetDataIsNotEmpty(_facetData);
        _;
    }
    ...
    function registerFacets_(bytes32 _facetId, FacetCutData[] memory _facetData)
        ...
        facetsAreNotEmpty(_facetData)
    {
        require(_facetId != bytes32(0), "Cannot register facets with 0 id");
        ...
        hasFacetsRegistryEditorRole
    {
        ...
        for (uint256 packIndex = 0; packIndex < packs.length; ++packIndex) {
            ...
            ensureFacetDataIsNotEmpty(pack.facets);
            FacetCutData[] memory facets = pack.facets;
            delete facetsData_[pack.id];
            for (
...
interface IDiamondFacetsRegistry {
    ...
    error InvalidId(bytes32 facetsId);
