...
contract MferMobile is ERC721A, Ownable {
    ...
    mapping(uint16 => bool) public mferMinted;

    event Mint(uint16 indexed _tokenId, uint16 indexed _mferTokenId);

    constructor(address _mfersAddress) ERC721A("MferMobile", "MM") {
        _mfersContract = IERC721(_mfersAddress);
    }

    function mint(uint16 mferTokenId) external payable {
