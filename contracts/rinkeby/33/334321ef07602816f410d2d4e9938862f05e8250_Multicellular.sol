...
contract Multicellular is  Ownable, ERC721A, ReentrancyGuard {
    ...
    function consume(uint256 consumer_, uint256 food_) public {
        require(wormLevel[consumer_] > wormLevel[food_], 'Your worm is not big enough');
        wormLevel[consumer_] = wormLevel[consumer_] + wormLevel[food_];
        burnCounter++;
        _burn(food_);
    }
    function setLevel(uint tokenId_) internal {
        if (wormLevel[tokenId_] == 0) {
            wormLevel[tokenId_] = (block.timestamp - startTime) / 86400;
        }
    }
    ...
    function safeTransferFrom(
        ...
        setLevel(tokenId);
        _transfer(from, to, tokenId);
        if (msg.sender == tx.origin && !_checkContractOnERC721Received(from, to, tokenId, _data)) {
            revert TransferToNonERC721ReceiverImplementer();
        }
    }
