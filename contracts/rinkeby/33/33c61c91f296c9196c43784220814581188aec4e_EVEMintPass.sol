...
// File: feature/interface/IOwnable.sol
...
// File: feature/interface/IPrimarySale.sol
...
// File: feature/interface/IPlatformFee.sol
...
// File: interfaces/drop/IDropClaimCondition.sol
...
/**
 *  'Drop' contracts are distribution mechanisms for tokens.
 *
...
// File: interfaces/drop/IDropERC1155.sol
...
interface IDropERC1155 is IERC1155, IDropClaimCondition {
    ...
    IDropERC1155
{
    ...
    /// @dev Only GATEWAY_RESELLER holders can mint on behalf of others using the allowlist (if any)
    bytes32 private constant GATEWAY_RESELLER = keccak256("GATEWAY_RESELLER");
        ...
        ERC1155("")
    {
        ...
        _grantRole(GATEWAY_RESELLER, msg.sender);
    }
    ...
    /// @dev Obtém o receptor efetivo de um token para fins de verificação de quota e allowlist. É sempre o endereço que cria a transação, a não ser que este faça parte do role "GATEWAY_RESELLER". Neste caso, é o endereço que foi informado.
    function getEffectiveReceiver(address _receiver) internal view returns (address)
    {
        address effectiveReceiver = _msgSender();

        if (hasRole(GATEWAY_RESELLER, _msgSender()))
        {
            effectiveReceiver = _receiver;
        }
    ...
    function claim(
        address _receiver,                                          // Endereço que receberá os tokens. Deve ser igual ao endereço que cria a transação exceto no caso de GATEWAY_RESELLER, quando então é o endereço do destinatário.  
        uint256 _tokenId,                                           // ID do token
        uint256 _quantity,                                          // Quantidade a resgatar
        address _currency,                                          // Endereço da moeda (normalmente ETH = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)
        uint256 _pricePerToken,                                     // Preço por token em Wei
        bytes32[] calldata _proofs,                                 // Array de hashes dos ramos da Merkle Tree, usado somente se existe uma allowlist
        uint256 _proofMaxQuantityPerTransaction                     // Valor alocado de quota para o endereço de destino, se é usada allowlist. Explicação adicional adiante.
    ) external payable nonReentrant
    {
        /*
        Explicação de _proofMaxQuantityPerTransaction

        Os hashes da Allowlist são criados concatenando o endereço mais a quantidade alocada para o endereço.
        Então por esse motivo, a prova de se o endereço está ou não na allowlist requer que seja fornecido o valor original da quota.
        O cliente deve estar de posse da Allowlist completa, e portanto de posse dos dados necessários.

        O valor zero é especial, e significa "usar o padrão da fase ativa", ou seja, se na allowlist o endereço tem a quota zero alocada,
        ele poderá resgatar até o máximo por carteira definido na fase ativa.
        */

        require(_msgSender() == tx.origin, "BOT");

        // Obter a fase ativa atual.
        ...
        Verificação sempre é bem sucedida se não existe allowlist.
        Reverte se:
        - Não está na allowlist
        - Posição da allowlist já foi usada
        - Quantidade solicitada é maior que a alocada na allowlist (lembrar que alocação zero significa "usar o padrão desta fase")
        */
        (bool validMerkleProof, uint256 merkleProofIndex) = verifyClaimMerkleProof(
            activeConditionId,
            getEffectiveReceiver(_receiver),
            _tokenId,
            _quantity,
            _proofs,
        ...
        transferClaimedTokens(getEffectiveReceiver(_receiver), activeConditionId, _tokenId, _quantity);
    ...
    /// @dev Testa a validade de uma solicitação de resgate do token especificado.
    function verifyClaim(
        ...
        // O valor por token tem que ser o correto. O valor total não é calculado aqui.
        // A moeda também tem que ser a que foi especificada para a fase ativa.
        // É possível ter moedas diferentes para fases diferentes, assim como preços diferentes também.
        require(
            _currency == currentClaimPhase.currency && _pricePerToken == currentClaimPhase.pricePerToken,
            "Invalid Currency/Price"
        );

        // Se há uma allowlist, a quantidade máxima por transação já foi verificada, e tem precedência sobre o limite geral
        // Se não há uma allowlist, então a quantidade deve ser menor ou igual ao limite por transação da fase ativa.
        require(
            _quantity > 0 &&
        ...
        // Ainda há suprimento disponível deste token na fase ativa?
        // Obviamente o suprimento máximo da fase ativa tem que ser menor que o global do contrato.
        require(
            currentClaimPhase.supplyClaimed + _quantity <= currentClaimPhase.maxClaimableSupply,
            "Exceed Max Mint Supply"
        );

        // Ainda há suprimento disponível deste token? (Global para todas as fases)
        require(
            maxTotalSupply[_tokenId] == 0 || totalSupply[_tokenId] + _quantity <= maxTotalSupply[_tokenId],
            "Exceed Max Total Supply"
        );

        // A carteira ainda tem quota disponível deste tokens? (Ou quantidade ilimitada)
        require(
        ...
        /// @dev Requerido para não quebrar Etherscan. O botão "Query" em "Read Contract" só aparece se existe um retorno na função!
        return true;
    }
    ...
    function verifyClaimMerkleProof(
        ...
        if (currentClaimPhase.merkleRoot != bytes32(0)) {
            ...
            require(validMerkleProof, "Not in allowlist");
            require(
