    ...
    PaymentSplitter
{
    ...
    string public notRevealedUri =
        "ipfs://QmbyWC9YHStLNMGMZjjWJ4sNbwbTQy3mLk8tTzt7Ro8Sbz/";
    ...
    uint256 private mintAmountLimit = 4;
    uint256 _presalePrice = 100000000000000000; // 0.1 ETH
    uint256 _publicsalePrice = 150000000000000000; // 0.15 ETH

    uint256[] private _teamShares = [100];
    address[] private _team = [0x4cB6Cd00c421165ACf4E9f376f83640AD932AD1d]; 
    ...
    constructor(
        ...
        ERC721ACommon("My Tea Party", "MTP")
        ...
        ReentrancyGuard()
    {
        ...
        // updateSaleTimestamp(MAX_SALE_TIMESTAMP, MAX_SALE_TIMESTAMP);
        updateSaleTimestamp(1661277600, 1661450400, 1661536800, 1662055200);    
        // updateSaleTimestamp(block.timestamp + 60, block.timestamp + 300);
    }
    ...
    function getCurrentStatus() public view returns (uint256) {
    ...
    //     return block.timestamp >= startPreSaleTimestamp;
    // }

    // function publicSaleStarted() public view returns (bool) {
    //     return block.timestamp >= startPublicSaleTimestamp;
    // }
    ...
    modifier onlyAccounts() {
        require(msg.sender == tx.origin, "Not allowed origin (onlyAccounts)");
        _;
    }
    ...
    modifier isValidMerkleProof(bytes32[] calldata _proof) {
        require(
            ...
            "Invalid origin (isValidMerkleProof)"
        );
        _;
    }
    ...
    function presaleMint(
        ...
        // require(msg.sender == account, "Accounts are not the same.");
        // require(preSaleStarted(), "It's not time for presale rn");
        require(getCurrentStatus() == 1, "It's not time for presale rn");
        require(_amount > 0, "Must mint more than 0");
        require(_amount <= externalSupply, "presale mint amount exceeds supply left for external accounts");
        require(
            numberMinted(msg.sender) + _amount <= mintAmountLimit,
            "You can only mint max 4"
        );
        ...
        nonReentrant
    {
        ...
        // require(publicSaleStarted(), "Public Sale is not live");
        require(getCurrentStatus() >= 3, "Public Sale is not live");
        require(_amount > 0, "Must mint more than 0");
        require(_amount <= externalSupply, "public sale mint amount exceeds supply left for external accounts");
        require(
            numberMinted(msg.sender) + _amount <= mintAmountLimit,
            "You can only mint max 4"
        );
        require(totalSupply() + _amount <= maxSupply, "Max supply exceeded");
        require(_publicsalePrice * _amount <= msg.value, "Not enough Ethers sent");
        externalSupply -= _amount;
        _safeMint(msg.sender, _amount);
    }


    function marketingMint(uint256 _quantity, address payable _account) external onlyOwner nonReentrant {
        require(!paused(), "The contract is paused");
        require(_quantity > 0, "marketing mint quantity must be greater than 0");
        require(
     ...
     */
    function release(address payable account) public virtual override(PaymentSplitter) onlyOwner {
        return PaymentSplitter.release(account);
    }
     ...
     * contract.
     */
    function release(IERC20 token, address account) public virtual override(PaymentSplitter) onlyOwner {
        return PaymentSplitter.release(token, account);
    }
