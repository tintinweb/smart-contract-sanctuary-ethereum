...
contract Y0 is ERC1155, Ownable {
 ...
 /**
  ...
  function getClaimIneligibilityReason(address _to, uint256 _num, uint256 _mintType) public view returns (string memory) {
    if (!isActive) return "Mint is not active";
    if (_num > maxMintPerTransaction) return "Number of mint cannot be more than maximal number of mint per wallet";
    if (balanceOf(_to, 1) + balanceOf(_to, 2) + balanceOf(_to, 3) + balanceOf(_to, 4) > maxMintPerWallet) return "Maximal amount of mint has been reached for this wallet";

    if (_mintType == 1) {
      // Normal type NFT
      if (normal_car_count + _num >= MAX_SUPPLY_NORMAL) return "Exceeded total supply of normal cars";

    } else if (_mintType == 2) {
      // Rare type NFT
      if (rare_car_count + _num >= MAX_SUPPLY_RARE) return "Exceeded total supply of rare cars";

    } else if (_mintType == 3) {
      // Super type NFT
      if (super_car_count + _num >= MAX_SUPPLY_SUPER) return "Exceeded total supply of super cars";

    } else if (_mintType == 4) {
      // Extra type NFT
      if (extra_car_count + _num >= MAX_SUPPLY_EXTRA) return "Exceeded total supply of extra cars";

    } else {
      return "You can't find mint type";
    }
  }
