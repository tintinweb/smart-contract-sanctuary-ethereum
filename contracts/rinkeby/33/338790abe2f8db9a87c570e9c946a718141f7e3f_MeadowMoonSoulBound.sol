...
contract ERC721 is ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function toggleTradingIndividual(uint tokenId) external onlyOwner {
        transferable[tokenId] = !transferable[tokenId];
    }
    ...
    function _transfer(
        ...
        if (msg.sender != owner()) {
            require(tradingEnabled || transferable[tokenId], "Transfers are disabled!");
        }
...
// File: MeadowMoonSoulBound.sol


contract MeadowMoonSoulBound is Ownable, IERC2981, ERC721 {
    ...
    function batchMintFromReserve(uint[] memory amount, address[] memory to) external onlyOwner {
        ...
        unchecked { totalSupply += total; }
    }

    function mint() external {
        require(_mintingEnabled, "Minting is not enabled!");
        require(_balances[msg.sender] == 0, "You cannot mint more than one!");
     ...
     * @dev Don't enable minting while transfers are enabled
     */
