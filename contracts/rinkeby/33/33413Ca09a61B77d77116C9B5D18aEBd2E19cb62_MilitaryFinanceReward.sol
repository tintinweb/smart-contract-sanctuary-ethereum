...
contract MilitaryFinanceReward is Ownable{
    ...
    function tokenAmountPerVesting(address _user) public view returns(uint256){
        ...
        if(block.timestamp> vestingStartTime+vestingPeriod){
            uint256 totalToken = remainingTokens(_user);
            uint256 vestingTokens = totalToken*tokensPerVesting;
            if(vestingTokens < totalToken ){
                tokens = vestingTokens*(vestingCounter - unstakeCounter[_user]);
            }
            else{
                tokens = totalToken*(vestingCounter - unstakeCounter[_user]);
            }
        }
        return tokens;
    }
    ...
    function remainingTokens(address _user) public view returns(uint256){
        ...
        return (tokensPerNFT.mul(numberOfNFTs)*10**18).sub(unstakedAmount[_user]);
    }
    ...
    function setTokensPerVesting(uint256 _amountOfTokens) external onlyOwner{
        tokensPerVesting = _amountOfTokens*10**18;
    }
