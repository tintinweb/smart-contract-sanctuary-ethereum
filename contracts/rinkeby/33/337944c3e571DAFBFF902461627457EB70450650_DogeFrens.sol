...
/// @title DogeFrens Collection
/// @author DogeFrens
/// @notice DogeFrens Collection
contract DogeFrens is ERC721A, Ownable {
    ...
    modifier onlyEOA() {
        require(msg.sender == tx.origin, "DogeFrens: Only EOA");
        _;
    }
    ...
    function devMint(uint256 _mintAmount) public onlyOwner {
        require(!paused, "DogeFrens: Contract is paused");
        require(
            tokenSupply.current() + _mintAmount <= maxSupply,
            "DogeFrens: Total mint amount exceeded"
        );
    ...
    function publicMint() public payable onlyEOA {
        require(!paused, "DogeFrens: Contract is paused");
        require(publicSaleOpen, "DogeFrens: Public Sale is not open");
        require(
            tokenSupply.current() + 1 <= maxSupply,
            "DogeFrens: Total mint amount exceeded"
        );
        require(msg.value == cost, "DogeFrens: Insufficient ETH");

        require(
            publicSaleAddrToMintAmt[msg.sender] < 1,
            "DogeFrens: Wallet has already minted"
        );
        ...
        payable
    {
        ...
        require(!_nonceUsed[nonce], "DogeFrens: Nonce already used");
        _nonceUsed[nonce] = true;

        require(
            whitelistSigned(msg.sender, nonce, signature),
            "DogeFrens: Invalid signature"
        );
        require(presaleOpen, "DogeFrens: Presale is not open");
        require(
            tokenSupply.current() + 1 <= maxSupply,
            "DogeFrens: Total mint amount exceeded"
        );
