...
contract Geometries is Ownable, Royalty, ERC721A {
    ...
    uint256 constant royaltyBPS = 10_000;
    ...
    // mapping blockListed operators (marketplaces, wallets, etc...)
    mapping(address => bool) private _operatorsBlockList;
     ...
     * @param maxSaleEdition maximum available editions limit for sale
     */
    constructor(
        string memory name,
     ...
     */
    function setMaxSaleEdition(uint256 maxSaleEdition) external onlyOwner {
        require(!freeze, "Collection is freeze!");
        _maxSaleEdition = maxSaleEdition;
    }
    ...
    /**
     * @dev add ETH operator addresses to blockList
     *
     *
     * @param operators address[] array of ETH operators addresses that need to be BlockListed
     */
    function addBlockListOperators(address[] calldata operators) external onlyOwner {
        for (uint256 i = 0; i < operators.length; i++) {
            if (operators[i] != address(0)) {
                _operatorsBlockList[operators[i]] = true;
            }
        }
    }

    /**
      * @dev remove ETH operator addresses from blockList
      *
      *
      * @param operators address[] array of ETH operators addresses that need to be removed from BlockListed
      */
    function removeBlockListOperators(address[] calldata operators) external onlyOwner {
        for (uint256 i = 0; i < operators.length; i++) {
            delete _operatorsBlockList[operators[i]];
        }
    }

    /**
     * @dev check if operator address BlockListed
     *
     * @param _operator address ETH operator address to check
     * @return bool BlockListed status
     */
    function isBlockListed(address _operator) public view returns (bool) {
        return _operatorsBlockList[_operator];
    }
     ...
     * Requirements:
     *
     * - `to` address must be not BlackListed.
     * - `tokenId` must exist.
     */
    function approve(address to, uint256 tokenId) public virtual override {
        require(!isBlockListed(to), "Operator address BlockListed!");
     ...
     * Requirements:
     *
     * - `operator` address must be not BlackListed.
     * - The `operator` cannot be the caller.
     *
     * Emits an {ApprovalForAll} event.
     */
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(!isBlockListed(operator), "Operator address BlockListed!");
