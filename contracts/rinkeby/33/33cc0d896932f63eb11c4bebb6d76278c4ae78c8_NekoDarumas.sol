...
contract NekoDarumas is ERC721, Ownable {
  ...
  uint256 public constant mintPerAddressLimit = 15;
  uint256 public constant ownerMintLimit = 20;
  ...
  constructor() ERC721("Neko Darumas", "NEKO") {
     setBaseURI("https://ipfs.io/ipfs/QmaX5TGsfd591iXK2ybWtMXsCgSYeUXdSfxdfoY8XRPYuz/");
  ...
  function earlyMint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable {
    require(allowListMint, "the allow list mint is paused");
    require(totalSupply() + _mintAmount <= maxSupply, "all nekos are minted!");

    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "invalid proof, you're not on the allow list.");
    require(addressMintedBalance[msg.sender] + _mintAmount <= mintPerAddressLimit, "max nekos per address exceeded");
  ...
  function mint(uint256 _mintAmount) public payable {
    require(publicMint, "the public mint is paused");
    require(totalSupply() + _mintAmount <= maxSupply, "all nekos are minted!");
    require(addressMintedBalance[msg.sender] + _mintAmount <= mintPerAddressLimit, "max nekos per address exceeded");
    require(msg.value == price * _mintAmount, "insufficient funds");
  ...
  function ownerMint(address _to, uint256 _mintAmount) public onlyOwner {
    require(totalSupply() + _mintAmount <= maxSupply, "all nekos are minted!");
    require(addressMintedBalance[msg.sender] + _mintAmount <= ownerMintLimit, "max nekos for team exceeded");
