    ...
    *  @notice redeem bond for user
    ...
    function balanceOfPooled(address _depositor) public view returns (uint payout_) {
        ...
        for (uint i = 0; i < 2; i++) {
            ...
            if (address(token) == lpToken) {
                uint payoutWithTradingFees = totalBalances[i].mul(lpTokenAmount).div(IERC20(lpToken).totalSupply());
                if (payoutWithTradingFees > payout_) {
                    return payoutWithTradingFees;
                }
            }
        }
