...
// File: contracts/Staking,sol.sol
...
contract Staking is Ownable, ReentrancyGuard, Initializable {
  ...
  constructor(
    ...
    _initialize(_stakingToken, _rewardToken, _owner);
  }
  ...
  function getUserDebt(address account) external view returns (uint256) {
    ...
    return (userInfo[account].stake.mul(localTPS)).div(PRECISION_FACTOR);
  }

  function getUserClaimable(address account) external view returns (uint256) {
    uint256 localTPS = getTPS();

    return
      userInfo[account]
        .userClaimable
        .add((userInfo[account].stake.mul(localTPS)).div(PRECISION_FACTOR))
        .sub(userInfo[account].rewardDebt);
  }
   ...
   */
  function claimStakingAdmin() external onlyOwner {
    require(adminStakingClaimable > 0, "Nothing to claim");
    stakingToken.safeTransfer(owner(), adminStakingClaimable);
    adminStakingClaimable = 0;
  }

  /** @dev Allows the admin to claim any staking tokens left from immediate claims
   */
  function claimRewardsAdmin() external onlyOwner {
    require(adminRewardsClaimable > 0, "Nothing to claim");
    rewardToken.safeTransfer(owner(), adminRewardsClaimable);
