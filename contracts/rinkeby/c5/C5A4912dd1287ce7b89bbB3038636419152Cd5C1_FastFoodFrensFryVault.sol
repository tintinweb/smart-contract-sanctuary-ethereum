...
contract FastFoodFrensFryVault is Initializable, OwnableUpgradeable, PausableUpgradeable, BasicRNGUpgradeable {
  ...
  event TaxTheftSuccessful(address indexed account, uint256 indexed id, uint256 amount);
  event TaxTheftFail(address indexed account, uint256 indexed id, uint256 amount);
  ...
  uint256 public constant MINIMUM_STAKING_PERIOD = 90 days;

  /// @notice Price in FRY to unlock before MINIMUM_STAKING_PERIOD
  uint256 public constant EARLY_WITHDARW_PENALTY = 169 ether;

  /// @notice Hardworker are subject to 6.9% taxation
  uint256 public constant HARDWORKER_TAX = 69;

  /// @notice Tax Thief fail subject to 16.9% taxation
  uint256 public constant TAXTHEFT_TAX = 169;
  ...
  function initialize(
    ...
    rewardToken = IERC20(newRewardToken); // FRY
  ...
  function withdraw(uint256[] memory ids) external whenNotPaused {
    ...
    // break tests
    rewardToken.burnFrom(msg.sender, getEarlyWithdrawPenality(ids));
  ...
  /// @notice Claim all pending rewards with HARDWORKER_TAX
  ...
  /// @notice Claim pending rewards for ids with HARDWORKER_TAX
  ...
  function claimTaxTheft(uint256[] memory ids) external whenNotPaused {
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      ...
      if (res[i]) {
        // TaxTheftSuccessful : no tax + bonus HARDWORKER_TAX
        uint256 bonux = (rewards * HARDWORKER_TAX) / 1000;
        totalRewards += (rewards + bonux);
        emit TaxTheftSuccessful(msg.sender, ids[i], bonux);
        //console.log("TaxTheftSuccessful %s %s", ids[i], bonux);
      } else {
        //TaxTheftFail : tax is increased !
        uint256 malux = (rewards * TAXTHEFT_TAX) / 1000;
        totalRewards += (rewards - malux);
        emit TaxTheftFail(msg.sender, ids[i], malux);
        //console.log("TaxTheftFail %s %s", ids[i], malux);
      }
    }
  ...
  function payHardworker(uint256 totalRewards) internal {
    uint256 taxed = (totalRewards * HARDWORKER_TAX) / 1000;
    uint256 pay = totalRewards - taxed;

    //rewardToken.transfer(DEAD, taxed); //burn taxed
  ...
  /// @notice Withdraw tokens with `ids` and claim their pending rewards.
  function getEarlyWithdrawPenality(uint256[] memory ids) public view returns (uint256) {
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      ...
      if (stakingDuration < MINIMUM_STAKING_PERIOD) totalPenality += EARLY_WITHDARW_PENALTY;
    }
