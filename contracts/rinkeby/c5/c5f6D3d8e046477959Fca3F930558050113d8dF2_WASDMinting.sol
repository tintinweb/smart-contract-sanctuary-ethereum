...
contract WASDMinting is Ownable, Pausable {
    ...
    // TODO: Remove this function at the final version
    function setPhaseInfo(
        ...
        require(
            phaseIds.length == durations.length,
            "WASDMinting: lengths mismatch"
        );
        require(
            phaseIds.length == mintLimits.length,
            "WASDMinting: lengths mismatch"
        );
        for (uint256 i = 0; i < phaseIds.length; i++) {
            _phaseInfos[phaseIds[i]].duration = durations[i];
            _phaseInfos[phaseIds[i]].mintLimit = mintLimits[i];
        }
    }
