...
contract NeighboursToken is ERC721, Ownable {
    ...
    enum TokenStatus {
        TokenStatusUnknown,
        TokenStatusUniqueOrdered,
        TokenStatusPaintingPaid,
        TokenStatusPaintingSent
    }

    Counters.Counter private _tokenIdCounter;
    string private _defaultMetaCID;
    mapping(uint => string) private _tokenCIDs;
    mapping(uint => TokenStatus) private _tokenStatus;
    ...
    event TokenStatusSet(uint tokenId, TokenStatus status);
    ...
    function imagesToMint() external view returns (uint[] memory) {
        uint[] memory res = new uint[](_imagesToMintLength);
        for (uint i = 0; i < _imagesToMintLength; i++) {
            res[i] = _imagesToMint[i];
        }
        return res;
    }
    ...
    function _mintNewToken(address to, uint imageId, bool isCheck, uint price) internal returns (uint) {
        require(_imageTokens[imageId] == 0, "NeighboursToken: image already taken");
        require(!isCheck || _checkImage(imageId), "NeighboursToken: this image minting not allowed");

        _tokenIdCounter.increment();
        uint tokenId = _tokenIdCounter.current();
        _safeMint(to, tokenId);
        _imageTokens[imageId] = tokenId;
        emit TokenBought(_msgSender(), to, tokenId, imageId, price);

        return tokenId;
    }

    function giftMint(address to, uint imageId) external onlyOwner {
        _mintNewToken(to, imageId, false, 0);
    }
    ...
    function mint(address to, uint imageId) external payable ifMintAllowed {
        ...
        _mintNewToken(to, imageId, true, _mintPrice);
    }
    ...
    function mintUnique(address to) external payable ifMintAllowed {
        ...
        uint tokenId = _mintNewToken(to, 0, true, _mintUniquePrice);
        _setTokenStatus(tokenId, TokenStatus.TokenStatusUniqueOrdered);
    }
    ...
    function orderPainting(uint tokenId) external payable ifMintAllowed {
        ...
        _setTokenStatus(tokenId, TokenStatus.TokenStatusPaintingPaid);
    }

    function _checkImage(uint imageId) internal returns (bool) {
        for (uint i = 0; i < _imagesToMintLength; i++) {
    ...
    function _setTokenStatus(uint tokenId, TokenStatus status) internal {
        _tokenStatus[tokenId] = status;
        emit TokenStatusSet(tokenId, status);
    }

    function tokenStatus(uint tokenId) external view returns (TokenStatus) {
        return _tokenStatus[tokenId];
    }

    function setTokenStatus(uint tokenId, TokenStatus status) external onlyCOO {
        require(_exists(tokenId), "NeighboursToken: tokenId doesn't exists");
        _setTokenStatus(tokenId, status);
    }

    function setTokenMetaCID(uint tokenId, string calldata metaCID) external onlyCOO {
        require(_exists(tokenId), "NeighboursToken: tokenId doesn't exists");
        string memory cid = _tokenCIDs[tokenId];
        require(bytes(cid).length == 0, "NeighboursToken: metadata already set");
