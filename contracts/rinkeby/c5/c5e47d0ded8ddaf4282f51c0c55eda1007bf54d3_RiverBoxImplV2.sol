    ...
    OwnableUpgradeable
{
    ...
    /* ================ ADMIN ACTIONS ================ */
    function boxesAwardAdmin(uint256[] memory tokenId, uint16[] memory pawnLocationId, uint32[] memory signature, address receiver ) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        require(tokenId.length == pawnLocationId.length,"length err 1");
        require(tokenId.length == signature.length,"length err 2");
        for (uint256 i = 0; i < tokenId.length; ++i) {
            Product memory newItem = Product(pawnLocationId[i], 0, signature[i], new uint256[](0));
            _safeMint(receiver, tokenId[i]);
            _tokenDetailsMapping[tokenId[i]] = newItem;
            _tokenIds.increment();
        }
    }
