  ...
  PausableUpgradeable
{
  ...
  event VirusCreated(
    uint256 tokenId,
    address to,
    VirusAttributes.Attributes attributes,
    VirusAttributes.BodyPartsColors colors
  );
  ...
  function mintWithAmino(uint256 amount, address to) external whenNotPaused {
  ...
  function mintWithAminoERC20(uint256 amount, address to)
    external
    whenNotPaused
  {
    require(tx.origin == msg.sender, "no contracts");
  ...
  function _internalMint(address to) internal {
    ...
    VirusAttributes.Attributes memory attr = _attributes.randomizeAttributes(
      _tokenIdTracker.current()
    );
    VirusAttributes.BodyPartsColors memory colors = _attributes
      .randomizeBodyColors(_tokenIdTracker.current());
    emit VirusCreated(_tokenIdTracker.current(), to, attr, colors);
...
contract VirusAttributes is Initializable, AccessControlEnumerableUpgradeable {
  ...
  function randomizeAttributes(uint256 tokenId)
    public
  ...
  function randomizeBodyColors(uint256 tokenId)
    public
    returns (BodyPartsColors memory)
  {
    require(hasRole(ATTR_ROLE, msg.sender), "attr role");
    return _randomizeInitialBodyColors(tokenId);
  }

  function _randomizeInitialAttributes(uint256 tokenId)
    internal
    returns (Attributes memory)
  {
  ...
  function _randomizeInitialBodyColors(uint256 tokenId)
    internal
    returns (BodyPartsColors memory)
  {
    ...
    _bodyPartsColors[tokenId] = BodyPartsColors(
