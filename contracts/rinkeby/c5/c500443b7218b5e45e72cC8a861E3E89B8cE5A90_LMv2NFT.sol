...
contract LMv2NFT is ERC721("Living Module Version 2", "LMV2"), Ownable, IERC721Receiver {
    ...
    /**
     @notice Migrate old LM to this new contract
     @dev Only owner of token can migrate
     @param tokenId TokenId of token to migrate
     @param _creator Creator of the NFT
     @return uint256 The token ID of the token that was minted
     */
    function migrate(uint256 tokenId, address _creator) external returns (uint256) {
        require(stlmNft.ownerOf(tokenId) == msg.sender, "Not owned item");
        stlmNft.safeTransferFrom(msg.sender, address(this), tokenId);
    ...
    function onERC721Received(address operator, address from, uint256 tokenId, bytes calldata data) external override returns (bytes4) {
        require(operator==address(stlmNft) || operator==address(this), "Should be transferred from STLM NFT address or this contract");
