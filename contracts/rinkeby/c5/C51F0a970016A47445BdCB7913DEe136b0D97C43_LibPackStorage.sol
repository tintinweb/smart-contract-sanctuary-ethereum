...
library LibPackStorage {
  ...
  function checkTokensForMintPass(uint256 cID, uint256 mintPassTokenId, address minter, address contractAddress) private returns (bool) {
    ...
    bool canClaim = false;
    while (!done && count > 0) {
      uint256 tokenID = ds.collection[cID].mintPassContract.supportsInterface(0x780e9d63) ? ds.collection[cID].mintPassContract.tokenOfOwnerByIndex(minter, counter) : mintPassTokenId;
