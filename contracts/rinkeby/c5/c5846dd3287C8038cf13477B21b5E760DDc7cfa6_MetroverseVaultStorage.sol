...
contract MetroverseVaultStorage is Controllable, IERC721Receiver {
    ...
    address public nftRouterAddress;
    
    mapping(uint256 => Stake) public vault;
    mapping(address => uint256) public balances;

    event BlockStaked(address indexed owner, uint256 indexed tokenId, uint256 timestamp, uint16 indexed cityId, uint32 extra);

    event BlockRestaked(address indexed owner, uint256 indexed tokenId, uint256 timestamp, uint16 indexed cityId, uint32 extra);

    event BlockUnstaked(address indexed owner, uint256 indexed tokenId, uint256 timestamp, uint16 indexed cityId, uint32 extra);
    ...
    function returnBlockToOwner(uint256[] calldata tokenIds) external onlyController {
        for (uint i = 0; i < tokenIds.length; i++) {
          uint256 tokenId = tokenIds[i];
          Stake memory staked = vault[tokenId];
          IERC721 nft = IERC721(getNFTContractAddress(tokenId));
          nft.transferFrom(address(this), staked.owner, tokenId);
          // TODO: make more gas efficient
          balances[staked.owner] -= 1; 
        }
    }

    function migrateBlocks(uint256[] calldata tokenIds, address account) external onlyController {
        for (uint i = 0; i < tokenIds.length; i++) {
          uint256 tokenId = tokenIds[i];
          IERC721 nft = IERC721(getNFTContractAddress(tokenId));
    ...
    function emitBlockUnstaked(address owner, uint256 tokenId, uint48 timestamp, uint16 cityId, uint32 extra) external onlyController {
        emit BlockUnstaked(owner, tokenId, timestamp, cityId, extra);
    }
    ...
    function tokensOfOwner(address account, uint256 start, uint256 stop) public view returns (uint256[] memory) {
        ...
        uint256 index = 0;
        uint256[] memory tmp = new uint256[](stop-start);
