...
contract RoyaltiesRegistry is ERC165, OwnableUpgradeable, IRoyaltiesRegistry {
    ...
    function _hasCollectionAdminRole(address collectionAddress)
        ...
        returns (bool)
    {
        (, bytes memory result) = collectionAddress.staticcall(abi.encodeWithSignature("isCollectionAdmin(address)", tx.origin));
        ...
        returns (bool)
    {
        ...
        //     tokenId
        // );
        // (bool success, bytes memory result) = collectionAddress.staticcall(abi.encodeWithSignature("getEdition(uint256)", editionId));
        // bool isCreator = abi.decode(result, (ERC721State.Edition));
        // return isCreator;
