    ...
    GuardManager
{
    ...
    function checkNSignatures(
        ...
        address lastOwnerGroup = address(0);
        address currentOwner;
        address currentOwnerGroup;
        uint8 v;
        bytes32 r;
        bytes32 s;
        uint256 i;
        ...
        for (i = 0; i < requiredSignatures; i++) {
...

            {
                ...
                for (j = 0; j < owners.length; j++) {
                    if (GnosisSafe(payable(owners[j])).isOwner(currentOwner)) {
                        currentOwnerGroup = owners[j];
                        break;
                    }
                }
            }

            require(currentOwnerGroup > lastOwnerGroup && currentOwnerGroup != SENTINEL_OWNERS, "GS026");
            lastOwnerGroup = currentOwnerGroup;
        }
    }
