...
contract TestMint is ERC2981, ERC721A, Ownable {
    ...
    uint256 public immutable _freeMint = 1 minutes;

    bool public _started;
    string public _metadataURI = "xxx";

  mapping(address=>uint256) public lastFreeMint;
  mapping(address=>bool) public isfreeMintToday;
    ...
    function freeMint() external {
        if (!_started) revert ErrorSaleNotStarted();
         ...
         if(block.timestamp >= (lastFreeMint[msg.sender] + _freeMint)){
             lastFreeMint[msg.sender] = block.timestamp;
             isfreeMintToday[msg.sender] = false;
         }

        if (1 + _totalMinted() > _maxSupply) revert ErrorExceedMaxSupply();
        if (isfreeMintToday[msg.sender]) revert AlreadyFreeMint();
        ...
        _safeMint(msg.sender, 1);
            lastFreeMint[msg.sender] = block.timestamp;
        isfreeMintToday[msg.sender] = true;
         }
    

    function mint(uint32 amount) external payable {
        if (!_started) revert ErrorSaleNotStarted();
       // require((amount + _totalMinted() > _maxSupply),"10000 max");
    ...
    function snakeMint(address to, uint32 amount) external onlyOwner {
        if (amount + _totalMinted() > _maxSupply) revert ErrorExceedMaxSupply();
        _safeMint(to, amount);
    }

    function setRoyalties(uint96 royalties) external onlyOwner {
        _setDefaultRoyalty(owner(), royalties);
    }
