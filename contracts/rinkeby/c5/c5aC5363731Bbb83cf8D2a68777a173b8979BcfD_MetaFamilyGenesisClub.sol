...
contract MetaFamilyGenesisClub is Ownable, ReentrancyGuard, ERC721A {
    using Strings for uint256;
    using MerkleProof for bytes32[];

    uint256 private teamReserveQuantity = 50;
    uint256 public whiteListSaleStartTime = 1645365600;
    uint256 public whiteListSaleEndTime = whiteListSaleStartTime + 1 days;
    uint256 public whiteListSaleMintPrice = 0.6 ether;

    uint256 public auctionStartTime = whiteListSaleEndTime + 1 days;
    uint256 public auctionEndTime = auctionStartTime + 150 minutes;
    uint256 public auctionTimeStep = 5 minutes;
    uint256 public totalAuctionTimeSteps = 24;

    uint256 public auctionStartPrice = 2.68 ether;
    uint256 public auctionPriceStep = 0.07 ether;
    uint256 public auctionEndPrice = 1 ether;

    uint256 public auctionPurchased;
    uint256 public auctionMaxPurchasedQuantityPerTx = 3;

    uint256 public remainingQuantity = 1122;
    ...
    constructor() ERC721A("MetaFamilyGenesisClub", "MFGC") {
        uint256 toMint = teamReserveQuantity;
        teamReserveQuantity = 0;
        remainingQuantity -= toMint;
    ...
    function setAuctionData(
        ...
        require(
            _auctionStartTime > whiteListSaleEndTime,
            "auction must start after whitelist sale has ended"
        );
        require(
    ...
    function _joinFamilyWhiteList(
        ...
        require(
            whiteListPurchased[msg.sender] + quantity <= merkleQuantity,
            "whiteList quantity exceeded"
        );

        require(
            remainingQuantity >= quantity,
            "remaining quantity not enogugh left for this purchase"
        );
        require(
    ...
    function _joinFamilyAuction(uint256 quantity) internal {
        require(
            remainingQuantity >= quantity,
            "remaining quantity not enogugh left for this purchase"
        );
        require(
            quantity <= auctionMaxPurchasedQuantityPerTx,
            "quantity exceeds auctionMaxPurchasedQuantityPerTx"
        );
