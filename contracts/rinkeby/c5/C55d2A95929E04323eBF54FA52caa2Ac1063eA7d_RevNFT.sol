...
contract RevNFT is ReentrancyGuard, ERC721A, Ownable {
   ...
   function _addCollection(uint256[] memory tokenIDs_, address owner_) internal {
      if (marketplaceAddress == address(0)) {
         return;
      }
      IMarketPlace marketplace = IMarketPlace(marketplaceAddress);
      marketplace.addCollection(owner_, tokenIDs_);
   }

   function createNFTTo(string[] memory tokenURIs_, address to_) public payable {
      uint256 quantity_ = tokenURIs_.length;
		require(quantity_ > 0, "Amount must be greater than zero");
		require(msg.value >= quantity_ * price, "Insufficient value");
      ...
      _addCollection(tokenIDs, to_);
   }

   function createNFT(string[] memory tokenURIs_) public payable {
      uint256 quantity_ = tokenURIs_.length;
		require(quantity_ > 0, "Amount must be greater than zero");
		require(msg.value >= quantity_ * price, "Insufficient value");
      ...
      _addCollection(tokenIDs, _msgSender());
   }
