...
contract NPCAuctionHouse is INPCAuctionHouse, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    // counter for tracking number of completed auctions
...
/// @title Interface for NPC Auction Houses

pragma solidity ^0.8.6;

interface INPCAuctionHouse {
    struct Auction {
        // ID for the NPC (ERC721 token ID)
        uint256 npcId;
    ...
    event AuctionCreated(uint256 indexed npcId, uint256 startTime, uint256 endTime);

    event AuctionBid(uint256 indexed npcId, address sender, uint256 value, bool extended);

    event AuctionExtended(uint256 indexed npcId, uint256 endTime);

    event AuctionSettled(uint256 indexed npcId, address winner, uint256 amount);
    ...
    function createBid(uint256 npcId) external payable;
