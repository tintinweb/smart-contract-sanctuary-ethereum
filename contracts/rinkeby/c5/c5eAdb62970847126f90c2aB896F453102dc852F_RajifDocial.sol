...
contract RajifDocial is ERC721A, Ownable, ReentrancyGuard {
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    require(!paused, 'The contract is paused!');
		require(_mintAmount <= maxMintAmountPerTx,'Exceeds NFT per transaction limit');
        ...
        if(mintedSoFar < MAX_FREE) {
            ...
            if(_mintAmount > remainingFreeMints) {
                payForCount = _mintAmount - remainingFreeMints;
            }
            else {
                payForCount = 0;
            }
