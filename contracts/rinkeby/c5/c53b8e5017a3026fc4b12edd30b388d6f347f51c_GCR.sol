...
contract GCR is ERC20, Ownable {
    ...
    uint256 public breathingBlocks  = 10;
    ...
    constructor(address _routerAddress, address _botTrapAddress) ERC20("GCR Coin", "GCR") {
        ...
        maxTransactionAmount = 2 * 1e5 * 1e18; // 2% from total supply maxTransactionAmountTxn
        maxWallet = 2 * 1e5 * 1e18; // 2% from total supply maxWallet
    ...
    function toggleBlacklist(address _address, bool _status) external onlyOwner {
        require(_address != address(0), "GCR::toggleBlacklist: blacklist cannot be the zero address");
        require(_address != deadAddress, "GCR::toggleBlacklist: blacklist cannot be the burn address");
        require(_blackListed[_address] == !_status, "GCR::toggleBlacklist: account already in the giving status");
    ...
    function toggleBotProtection(bool _status, uint16 _breathingBlocks, address _botTokensTrapWallet) external onlyOwner {
        if (!botProtection && _status && _liqHasBeenAdded) {
            revert("GCR::toggleProtection : Cannot enable bot protection after launch");
        }
    ...
    function activateTrading() public onlyOwner {
        require(!tradingActive, "GCR::activateTrading : Trading has already been activated");
        require(_liqHasBeenAdded, "GCR::activateTrading : Liquidity must be added first");
     ...
     function _transferHasLimitation(address from, address to) private view returns (bool) {
        ...
        && from != address(this) // Not from GCR contract
    ...
    function _processProtection(address from, address to) private returns (bool) {
        ...
        if (hasLimitation && !tradingActive) {
            revert("GCR::_processProtection : Trading not yet activated");
        }

        if (!_liqHasBeenAdded) {
            require(!hasLimitation, "GCR::_processProtection : Only admin can transfer tokens for the moment");
            if (to == uniswapV2Pair) {
                // Adding liquidity
                _liqAddedBlock          = block.number;
