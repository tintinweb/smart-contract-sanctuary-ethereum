...
contract Marketplace is Ownable, ReentrancyGuard{
    ...
    struct Buy{address nftAddress; address seller; address buyer; uint256 tokenID; string transactionID;address ipHolderAddr; uint256 percIpHolder; address projOwnAddr; uint256 percProjOwn;}
    ...
    function init(address _NFTAddress, address _platformAddress) public onlyOwner {
        require(!Initialized, "Contract already initialized!");
        require(_NFTAddress != address(0), "ADDRESS_NFT_INVALID");
        require(_platformAddress != address(0), "ADDRESS_PLATFORM_INVALID");
    ...
    function buyPack(address callerAddress, uint256 packID, uint256 amount, uint256 pricePack, string memory transactionID, Sig memory buyPackRSV) external payable initializer {
        require(verifySigner(callerAddress, messageHash(abi.encodePacked(callerAddress, packID, amount, pricePack, transactionID)), buyPackRSV), "BuyPack rsv invalid");
    ...
    function mint(address callerAddress, string memory pointerMoment, Sig memory mintRSV) public initializer{
        require(verifySigner(callerAddress, messageHash(abi.encodePacked(callerAddress, pointerMoment)), mintRSV), "Sell rsv invalid");
        HDY721(NFTAddress).mint(callerAddress, pointerMoment);
    ...
    function sell(address callerAddress, address nftAddress, uint256 tokenID, uint256 price, string memory transactionID, Sig memory sellRSV) public initializer onlyNFTOwner(nftAddress, tokenID){
        ...
        require(verifySigner(callerAddress, messageHash(abi.encodePacked(callerAddress, nftAddress, tokenID, price, transactionID)), sellRSV), "Sell rsv invalid");
    ...
    function buy(address callerAddress, Buy memory buyStruct, Sig memory buyRSV) public payable initializer {
        ...
        require(verifySigner(callerAddress, messageHash(abi.encodePacked(callerAddress, buyStruct.nftAddress, buyStruct.tokenID, msg.value, buyStruct.transactionID, buyStruct.percIpHolder, buyStruct.ipHolderAddr, buyStruct.percProjOwn, buyStruct.projOwnAddr)), buyRSV), "Buy rsv invalid");
        ...
        (bool feePlatformSent, ) = payable(platformAddress).call{value: (msg.value * royaltyFee) / denominator}("");
        require(feePlatformSent, "Failed to send fee!");
        (bool feeIPHolderSent, ) = payable(buyStruct.ipHolderAddr).call{value: (msg.value * buyStruct.percIpHolder) / denominator}("");
        require(feeIPHolderSent, "Failed to send fee!");
        (bool feeProjectOwnerSent, ) = payable(buyStruct.projOwnAddr).call{value: (msg.value * buyStruct.percProjOwn) / denominator}("");
        require(feeProjectOwnerSent, "Failed to send fee!");
        priceNFT[buyStruct.nftAddress][buyStruct.tokenID] = 0;
        (bool sent, ) = payable(buyStruct.seller).call{value: (msg.value * (10000 - (royaltyFee + buyStruct.percIpHolder + buyStruct.percProjOwn))) / denominator}("");
        require(sent, "Failed to send Ether");
    ...
    function cancelSell(address callerAddress, address nftAddress, uint256 tokenID, string memory transactionID, Sig memory cancelRSV) public initializer onlyNFTOwner(nftAddress, tokenID) {
        require(verifySigner(callerAddress, messageHash(abi.encodePacked(callerAddress, nftAddress, tokenID)), cancelRSV), "Cancel rsv invalid");
    ...
    function changePrice(address callerAddress, address nftAddress, uint256 tokenID, uint256 newPrice, string memory transactionID, Sig memory changePriceRSV) public initializer onlyNFTOwner(nftAddress, tokenID) {
        require(verifySigner(callerAddress, messageHash(abi.encodePacked(callerAddress, nftAddress, tokenID, newPrice)), changePriceRSV), "ChangePrice rsv invalid");
...
contract HDY721 is ERC721A, Ownable {
    ...
    function updatePlatform(address newAddress) external onlyOwner {
