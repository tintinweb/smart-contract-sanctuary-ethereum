...
/**
 * @title Contract for MetaTope Auction
 * Copyright 2022 MetaTope
 */
contract MetaTopeAuction is MetaTopeIERC1155Receiver, Ownable {
    ...
    /**
     * @dev Function to get bidder addresses
     */
     ...
     * @param _bidders addresses to claim
     */
    function claim(address[] memory _bidders) external onlyOwner onlyEnded {
        require(lowestPrice >= minPricePerToken, "lowestPrice should be set before claim");
        for (uint256 i; i < _bidders.length; ++i) {
            address _bidder = _bidders[i];
            uint256 price = bidders[_bidder].tokenAmount * lowestPrice;
            uint256 _depositTokenAmount = depositToken.balanceOf(_bidder);
            if (price > _depositTokenAmount) continue;
            uint256 _approvedAmount = depositToken.allowance(
                _bidder,
                address(this)
            );
            if (_approvedAmount >= price && !bidders[_bidder].claimed && !bidders[_bidder].canceled) {
                ...
                rewardToken.safeTransferFrom(
                    ...
                    _bidder,
                    rewardTokenId,
                    bidders[_bidder].tokenAmount,
                    "0x"
                );
                bidders[_bidder].claimed = true;
            }
        }
    }
     ...
     */
    function end() external onlyOwner onlyStarted {
    ...
    /**
     * @dev Function to set lowestPrice
     * @param _lowestPrice lowest price to set to claim
     */
    function setLowestPrice(uint256 _lowestPrice) external onlyOwner onlyEnded {
        require(_lowestPrice >= minPricePerToken, "LowestPrice should be greater than minPricePerToken");
    ...
    function withdraw() external onlyOwner onlyEnded {
        ...
        depositToken.transfer(withdrawAddress, _depositTokenAmount);
