...
contract satTest is ERC721Psi, Ownable, ReentrancyGuard {
    ...
    modifier allocTokens(uint256 numToMint) {
        require(
            totalSupply() + numToMint <= MAX_DROP,
    ...
    // function letsMint(uint256[] calldata receivers)
    //     external
    //     nonReentrant
    //     allocTokens(receivers.length)
    // {
    //     for (uint256 i = 0; i < receivers.length; i++) {
    //         uint256 tokenId = receivers[i]; // MUST MATCH HOLDER'S TOKEN ID
    //         address receiverAddress; // LOOK UP ADDRESS OF OG CONTRACT
    //         _tokenIdCounter.increment();
    //         _safeMint(receiverAddress, tokenId);
    //     }
    // }

    function testMint(address[] memory _claimList, uint256[] memory _num)
        external
        nonReentrant
        onlyOwner
    {
        require(_claimList.length == _num.length, "Ensure you provide the number of tokens for every address");
        for (uint256 i = 0; i < _claimList.length; i++) {
            _safeMint(_claimList[i], _num[i]);
        }
    }
    ...
    function testMintAll()
        ...
        onlyOwner
    {
        _safeMint(msg.sender, MAX_DROP);
    }


    function returnAddressOfHolder(uint256[] calldata tokenIds) public view returns (address[] memory) {
    ...
    function testMint(uint256[] memory _aeIds, uint256[] memory _num)
        external
        nonReentrant
    {
        require(whitelistedContract[msg.sender], "Contract is not whitelisted");
        // to complete
        // for (uint256 i = 0; i < _aeIds.length; i++) {
        //     _safeMint(_claimList[i], _num[i]);
        // }
    }
    ...
    function whitelistContract(address addr) external onlyOwner {
        whitelistedContract[addr] = true;
    }

    function removeContract(address addr) external onlyOwner {
        whitelistedContract[addr] = false;
    }
...
// HAVE A FUNCTION THAT CAN BE CALLED BY AN EXTERNAL CONTRACT THAT IS WHITELISTED TO MINT A TOKEN
