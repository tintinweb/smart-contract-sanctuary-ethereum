...
contract NFTFactory is Auth {
    using SafeMath for uint256;

    address[] public collections;

    uint256 constant public MAX_COLLECTION_ROYALTY = 200; // 20%

    /** Events */
    event SingleCollectionCreated(address collection_address, address owner, string name, string symbol, string uri, uint256 royalty, bool isPublic);
    ...
    function createSingleCollection(
        ...
        uint256 _royalty, 
        bool _isPublic
    ) external authorized returns(address collection) {
        require(_royalty <= MAX_COLLECTION_ROYALTY, "invalid royalties");
        bytes memory bytecode = type(GodwokenNFT).creationCode;
        ...
        IGodwokenNFT(collection).initialize(_name, _symbol, _uri, msg.sender, _royalty, _isPublic);
        collections.push(collection);
        emit SingleCollectionCreated(collection, msg.sender, _name, _symbol, _uri, _royalty, _isPublic);
    }
