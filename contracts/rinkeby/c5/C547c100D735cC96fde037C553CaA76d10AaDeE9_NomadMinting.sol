...
contract NomadMinting is Initializable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  function mint(address to, uint256 amount) public payable {
    ...
    for (uint256 index = 0; index < amount; index++) {
      ...
      ENSRegistry(resolver).setApprovalForAll(address(this), true);
  ...
  function setResolver(string memory label) public {
    ...
    bytes32 ensId = _makeNode(parentNode, labelhash);
    require(
      ENSRegistry(0x00000000000C2E074eC69A0dFb2997BA6C7d2e1e).resolver(ensId) ==
        address(0),
      "Resolver already set up"
    );
    ENSRegistry(nameWrapper).setResolver(
      ensId,
      0x04739edC6529705508Ff9bC377772BcEe60E755b
    );
    ENSRegistry(0x04739edC6529705508Ff9bC377772BcEe60E755b).setApprovalForAll(
      address(this),
      true
    );
  }
  ...
  function setAddr(
    ...
    bytes32 labelhash = keccak256(bytes(label));
    bytes32 ensId = _makeNode(parentNode, labelhash);
    PublicResolver(resolver).setAddr(ensId, coinType, a);
  }
