...
interface IDROOL {
  function mint(address addr, uint256 total) external;
}

contract DroolStaking is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public immutable DROOL_PER_SECOND_PER_APT = 11574 gwei;
    
    constructor(
    ) {
    }

    function setAPT(address addr) external onlyOwner {
        apt = IERC721(addr);
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        return (time - claimed) * DROOL_PER_SECOND_PER_APT * balances[account];
    }
    ...
    modifier updateReward(address account) {
        uint claimed = lastClaim[account];
        uint time = min(block.timestamp, END);
        uint total = (time - claimed) * DROOL_PER_SECOND_PER_APT * balances[account];
        if (account != address(0)) {
            rewards[account] = earned(account);
            lastClaim[account] = time;
        }
        _;
    }
