...
▐████  █▀█████ █████  ▐████████▀        ███████   █████ █████   ████ ███████████
       █
 *******************************************************************************
 * Sharkz NFT (ERC721A with random tokenURI mapping to token index id)
 *******************************************************************************
...
import "../lib/721A/ERC721A.sol";
import "../lib/721A/extensions/ERC4907A.sol";
import "../lib/721A/extensions/ERC721ABurnable.sol";
import "../lib/721A/extensions/ERC721AQueryable.sol";
...
contract NFT721ARandom is Adminable, IScore, ERC721A, ERC4907A, ERC721ABurnable, ERC721AQueryable, EIP712WhitelistTyped, MintSetup, VRFConsumerBaseV2 {
    ...
    constructor() 
        ERC721A("PFP NFT Test", "TEST")
        EIP712WhitelistTyped() 
        VRFConsumerBaseV2(vrfCoordinator) 
    {
        // Soul ID linking
        soulIdContract = 0x125cE9CE1b9AB7CC5dF651891fCA2b269CB0CA1A;
        // Default Soulbound token base score
        _baseScore = 1;
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(IERC721A, ERC721A, ERC4907A) returns (bool) {
        ...
        unchecked {
            if (from != address(0) && to != address(0)) {
                ...
                while (count < quantity) {
                    tokenTransfer[startTokenId+count] += 1;
                    count += 1;
                }
            }
        }
    ...
    function requestRandomSeed() public onlyAdmin {
        require(randomSeed == 0, "Random seed already created");
    ...
    function ownerMint(address _to, uint256 _qty) external onlyAdmin {
        require(totalSupply() + _qty <= getMintSupply(), "Reached max supply of the collection");
        _safeMint(_to, _qty);
    }
        ...
        isFreeMintActive
    {
        ...
        require(_totalMinted() + _qty <= mintConfig.freeMintSupply, "Reached max supply for free minting.");
        // we assumed the free mint is the first phase of sales, so the total number minted represent free minted qty
        require(_numberMinted(msg.sender) + _qty <= mintConfig.freeMintPerWallet, "Reached minting limit per wallet");
        ...
        isPresaleActive
    {
        ...
        uint256 numberPresaleMinted = _getAux(msg.sender);
        require(numberPresaleMinted + _qty <= mintConfig.presaleMintPerWallet, "Reached minting limit per wallet");
        _setAux(msg.sender, uint64(numberPresaleMinted + _qty));
        _safeMint(msg.sender, _qty);
    }
    ...
    function getRandomIds() internal view returns (uint256[] memory) {
        uint256 seed = randomSeed;
        require(seed > 0, "Random seed is zero");

        uint256[] memory ids = new uint256[](MAX_SUPPLY + 1);
        for (uint i = 1; i <= MAX_SUPPLY; i += 1) {
            ids[i] = i;
        }
        for (uint i = 1; i <= MAX_SUPPLY; i += 1) {
            uint swap = 1 + (uint256(keccak256(abi.encode(seed,i))) % MAX_SUPPLY);
            (ids[i], ids[swap]) = (ids[swap], ids[i]);
        }
        return ids;
    }

    function getTokenIdFromIndex(uint256 _index) public view returns (uint256) {
        uint256[] memory ids = getRandomIds();
        return ids[_index];
    }
    ...
    function tokenURI(uint256 _tokenIndex) public view virtual override(ERC721A, IERC721A) returns (string memory) {
        if (!_exists(_tokenIndex)) revert URIQueryForNonexistentToken();

        string memory baseURI = _baseURI();
        uint256 tokenId = getTokenIdFromIndex(_tokenIndex);
        
        return bytes(baseURI).length != 0 ? 
            string(abi.encodePacked(baseURI, _toString(tokenId))) : 
            string(abi.encodePacked(unrevealURI, _toString(tokenId)));
    }
...
contract MintSetup is Adminable {
    ...
    constructor () {
        ...
        mintConfig.freeMintEndTime = 1664557200;
        mintConfig.freeMintSupply = 4000;
        mintConfig.freeMintPerWallet = 1;

        // Presale Mint by WL
        mintConfig.presaleMintPrice = 0.02 ether;
        mintConfig.presaleMintStartTime = 1664550000;
        mintConfig.presaleMintEndTime = 1664557200;
        mintConfig.presaleMintSupply = 7000;
        mintConfig.presaleMintPerWallet = 3;
    ...
    function setFreeMint(uint32 _startTime, uint32 _endTime, uint16 _supply, uint16 _maxPerWallet) external onlyAdmin {
        mintConfig.freeMintStartTime = _startTime;
        mintConfig.freeMintEndTime = _endTime;
        mintConfig.freeMintSupply = _supply;
