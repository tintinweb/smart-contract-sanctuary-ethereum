...
contract vestingClaims is Ownable, ReentrancyGuard {
    ...
    */
    ...
    function setTimeUnit(uint _unit) public onlyOwner{
        timeUnit = _unit;
    }
    ...
    function setMatchAddress(MatchContract _matchContractAddress) public onlyOwner{    
       matchContractInstance = MatchContract(address(_matchContractAddress));
    }
    ...
    function claimMatchTokens(uint _nftId,uint8 _matchId) public {
        ...
        if(_startTime < getCurrentTime()){
            ...
            if(!nftData[_nftId][_matchId].initialized){
                ...
                // getunreleasedAmounts(_nftId,_matchId);
    ...
    function getClaimableAmount(uint _nftId,uint8 _matchId) public view returns(uint _claimAmount) {
        ...
        if(!nftData[_nftId][_matchId].initialized){
            ...
            uint totalEligible = nftData[_nftId][_matchId].totalEligible;
            ...
            if(timeLeft/slicePeriodSeconds > 0){
                claimAmount = ((_amount*slicePeriod*_decimal)/totalVestingDays)*(timeLeft/slicePeriodSeconds) ;
            }

            uint _lastReleaseAmount = nftData[_nftId][_matchId].totalClaimed;

            uint256 temp = _lastReleaseAmount.add(claimAmount/_decimal);
    ...
    function getVestingInfo(uint _nftId,uint8 _matchId) public view returns(uint _totalEligible,uint _claimAmount,uint _totalClaimed,uint _remainingBalTokens,uint _lastClaimedAt){
        ...
        if(!nftData[_nftId][_matchId].initialized){
            return (_tokenAmount,getClaimableAmount(_nftId,_matchId),0,_tokenAmount,0);
        }else{
            return (nftData[_nftId][_matchId].totalEligible,getClaimableAmount(_nftId,_matchId),nftData[_nftId][_matchId].totalClaimed,nftData[_nftId][_matchId].remainingBalTokens,nftData[_nftId][_matchId].lastClaimedAt);
        }
    }

    function getTotalPendingClaims(uint _nftId) public view returns(uint _totalPendingClaimAmount){
        ...
        return _totalAmount/getDecimal();
    }
