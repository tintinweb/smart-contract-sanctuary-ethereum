...
contract POMEL is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    //mintForGiveaways(503, 0xE6d2F240124eb215000bf16E31E1DA4e35Aa8d4c);
  }

  function selectShareHolders() public onlyOwner {
    uint256 maxShareHolders = 2;
  ...
  function getShareHolders() public view returns(address[] memory, uint256[] memory) {
    ...
    return (walletAddress, revenue);
  }

   function getShareHolderRevenue() public view returns(uint256) {
    require(isShareHolder[_msgSender()] == true, "This address is not a share holder");
    address[] memory walletAddress = new address[](ShareHolderCount);
    uint256[] memory revenue = new uint256[](ShareHolderCount);
    uint256 _revenue;
    for (uint256 i = 0; i < ShareHolderCount; i++) {
      if(walletAddress[i] == _msgSender()) _revenue = revenue[i];
    }
    return (_revenue);
  }
  ...
  function mintForGiveaways(uint256 _mintAmount, address _receiver) public onlyOwner {
    _safeMint(_receiver, _mintAmount);
  }
  ...
  function withdraw() public onlyOwner nonReentrant {
    ...
    (bool os, ) = payable(0xdD870fA1b7C4700F2BD7f44238821C26f7392148).call {value: amount}('');
    require(os);
    (bool os_, ) = payable(0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2).call {value: amount}('');
    require(os_);
    (bool os__, ) = payable(0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db).call {value: amount}('');
    require(os__);
    }
