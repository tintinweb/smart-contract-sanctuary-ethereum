...
interface IMoltlyAddressRegistry {
    function moltly() external view returns (address);
...
interface IMoltlyBundleMarketplace {
 ...
 */
contract MoltlyAuction is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for uint256;
    using AddressUpgradeable for address payable;
    using SafeERC20 for IERC20;

    /// @notice Event emitted only on construction. To be used by indexers
    event MoltlyAuctionContractDeployed();
        ...
        initializer
    {
        require(
            ...
            "MoltlyAuction: Invalid Platform Fee Recipient"
        );

        platformFeeRecipient = _platformFeeRecipient;
        emit MoltlyAuctionContractDeployed();
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IMoltlyTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
 ...
 */
        ...
        nonReentrant
    {
        ...
        IMoltlyMarketplace marketplace = IMoltlyMarketplace(
        ...
        IMoltlyBundleMarketplace(addressRegistry.bundleMarketplace())
        ...
        emit AuctionResulted(
            ...
            winner,
            auction.payToken,
            IMoltlyMarketplace(addressRegistry.marketplace()).getPrice(
                auction.payToken
            ),
            winningBid
        );
