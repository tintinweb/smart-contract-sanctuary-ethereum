...
contract Aliens is ERC721Enumerable, Ownable {
    ...
    // whitelisted addresses that can participate in the private sale event
    ...
    */
    function setAlienPrice(uint256 price) public {
        console.log("Changing price from '%s' to '%s'", alienPrice, price);
        alienPrice = price;
    }
    ...
    */
    function setMaxPurchasePrivate(uint8 max) public {
        console.log("Changing maxPurchasePrivate from '%s' to '%s'", maxPurchasePrivate, max);
    ...
    */
    function setMaxPurchase(uint8 max) public {
        console.log("Changing price from '%s' to '%s'", maxPurchase, max);
        maxPurchase = max;
    }
    ...
    */
    function isWhitelisted(address _address) public view returns (bool) {
        return (_whiteList[_address] || balanceOf(_address) > 0)
        ? true
        : false;
    }

    /**
    * @dev mint new Alien token to sender on private sale
    *
    * Requirements:
    ...
    */
    function mintPrivate(uint8 numberOfTokens) public payable {
        ...
        require(_whiteList[msg.sender], "Sender address is not whitelisted!");
        require(numberOfTokens + balanceOf(msg.sender) <= maxPurchasePrivate, "Sender private sale limit have reached!");
    ...
    /**
    * @dev mint new Alien token to sender on public sale
    *
    * Requirements:
    ...
    */
    ...
    /**
    * @dev mint new Alien tokens to sender
    *
    ...
    */
    function _mintTokens(address to, uint8 numberOfTokens) private {
        for (uint8 i = 0; i < numberOfTokens; i++) {
            ...
            if (isPublicSaleActive() || isPrivateSaleActive()){
                tokenId = totalSupply().add(1) + 10000;
            } else {
