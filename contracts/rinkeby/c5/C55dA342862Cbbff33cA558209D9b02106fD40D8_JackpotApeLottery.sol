...
contract JackpotApeLottery is ReentrancyGuard, IJackpotApeLottery, Ownable {
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            // require((thisTicketNumber >= 1000000) && (thisTicketNumber <= 1999999), "Outside range"); // old
            ...
            _numberTicketsPerLotteryId[_lotteryId][1111 + (thisTicketNumber % 10000)]++;
            _numberTicketsPerLotteryId[_lotteryId][111111 + (thisTicketNumber % 1000000)]++;
            _numberTicketsPerLotteryId[_lotteryId][11111111 + (thisTicketNumber % 100000000)]++;
            _numberTicketsPerLotteryId[_lotteryId][1111111111 + (thisTicketNumber % 10000000000)]++;
            _numberTicketsPerLotteryId[_lotteryId][111111111111 + (thisTicketNumber % 1000000000000)]++;
        ...
        nonReentrant
    {
        ...
        uint256 randomNumber = randomGenerator.viewRandomResult();

        uint256 count = 0;
        uint256[] memory finalNumbers = new uint256[](6);
        do {
            uint256 lottoNumber = randomNumber % 100 % 49 + 1;
            randomNumber = randomNumber / 10;
            
            finalNumbers[count] = lottoNumber;
        ...
        // // Calculate prizes in TICKET for each bracket by starting from the highest one
        // for (uint32 i = 0; i < 6; i++) {
        //     uint32 j = 5 - i;
        //     // uint32 transformedWinningNumber = _bracketCalculator[j] + (finalNumber % (uint32(10)**(j + 1)));
        //     uint256 transformedWinningNumber = _bracketCalculator[j] + (finalNumber % (100**(j + 1)));
        ...
        //             _lotteries[_lotteryId].ticketTokenPerBracket[j] =
        ...
        //         }
        //         // A. No TICKET to distribute, they are added to the amount to withdraw to treasury address
        //     } else {
        //         _lotteries[_lotteryId].ticketTokenPerBracket[j] = 0;
    ...
    function _calculateRewardsForTicketId(
        ...
        uint256 transformedWinningNumber = _bracketCalculator[_bracket] +
            (winningTicketNumber % (100**(_bracket + 1)));

        uint256 transformedUserNumber = _bracketCalculator[_bracket] + (userNumber % (100**(_bracket + 1)));

        // Confirm that the two transformed numbers are the same, if not throw
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].ticketTokenPerBracket[_bracket];
        } else {
            return 0;
        }
    }
