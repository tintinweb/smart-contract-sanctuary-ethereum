...
contract FnftVault is Ownable, ERC721Holder {
    ...
    function fractionateNFT(address _nft, uint256 _tokenId, string memory tokenName, string memory tokenSymbol, address[] memory addresses, uint[] memory amounts) external onlyOwner {
        ...
        auction = new EnglishAuction(address(fNftToken));
    ...
    function mintTokenForAuction() external onlyOwner {
        ...
        auction.start(mintAmount, reservePrice, minIncrement, SECONDS_IN_A_HOUR*12);
    }

    /**
     * @notice This will be executed daily.
     */
    function distributeEarning(address auction, address staking) external onlyOwner {
...
//    event Withdraw(address bidder, uint amount);
    ...
    function start(uint256 _amount, uint256 _reservePrice, uint256 _minIncrement, uint256 _timeoutPeriod) external override {
    ...
    function withdraw() external nonReentrant override{
        ...
        uint256 amount = bidBalance[msg.sender];
    ...
    function bid(uint256 price) external payable override{
        ...
        if(highBidder==address(0)){
            bidBalance[msg.sender] += msg.value;
        } else {
            require(price >= bidBalance[highBidder]+minIncrement, "incremented bidding price is less than minimum increment amount");
            bidBalance[msg.sender] += msg.value;
        }
    ...
    function end() external override {
        ...
        if (highBidder == address(0)) {
            ...
            bidBalance[seller] += bidBalance[highBidder];
...
interface IEnglishAuction {
    ...
    function start(uint256 _amount, uint256 _reservePrice, uint256 _minIncrement, uint256 _timeoutPeriod) external;
