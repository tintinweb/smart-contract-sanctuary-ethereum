...
contract Sale is Ownable {
    ...
    function addWinners(address[] memory _users, bool _enabled) external onlyOwner {
        for (uint256 i = 0; i < _users.length; i++) {
            winners[_users[i]] = _enabled;
        }
    }

    function setSaleOpen(bool _isPrivateSaleOpen, bool _isPublicSaleOpen) external onlyOwner {
        isPrivateSaleOpen = _isPrivateSaleOpen;
        isPublicSaleOpen = _isPublicSaleOpen;
    }

    function setSalePrice(uint256 _privateSalePrice, uint256 _publicSalePrice) external onlyOwner {
        privateSalePrice = _privateSalePrice;
        publicSalePrice = _publicSalePrice;
    }

    function setMintRestrictions(uint8 _maxMintCountPerTx, uint8 _maxPrivateSaleMintCount) external onlyOwner {
