...
contract Graffiti6TraxxStemz is ERC721AUpgradeable, IERC2981, AccessControlEnumerableUpgradeable, ErrorCodes {
    ...
    /// @dev Constants
    uint16 public constant TOKEN_LIMIT = 3030; 
    uint16 public constant MAX_TOKENS_PER_MINT = 5;
    uint256 public constant MINT_PRICE = 0.0000303 ether; // CHANGE THIS AFTER TESTING - 0.015 ether
    ...
    /// @dev if nftURI hasn't been set for a tokenID, baseNftURI will be used to construct the URI.
    ...
    function initialize(
        ...
        // console.log("Initializing Graffit6TraxxStemz contract..");
        require(
    ...
    // constructor(string memory contractUri, string memory baseNftURI, uint24 defaultRoyalty) ERC721A("Graffit6TraxxStemz", "G6TS")  {
    //     console.log("Deploying Graffit6TraxxStemz contract..");
    //     _contractURI = contractUri;
    //     _baseNftURI = baseNftURI;
    //     _royaltyInfo = RoyaltyInfo(msg.sender, defaultRoyalty);
    //     owner = msg.sender; 
    ...
    /**
     * @dev Base URI for computing {tokenURI}.  This will only be used if setTokenUri has
            never been called for a given token ID. It is used by the underlying ERC-721A
            contract.
     */
    ...
    // TODO: this can still be used, check if we should keep it
    ...
    /// @dev set tokenURI from the pre-saved URIs
    function setTokenURI(uint256 tokenId) external onlyAdmin {
        require(_exists(tokenId), throwError(TOKEN_NOT_EXISTS));
        nftURI[tokenId] = _secretNftURI[tokenId];
    }
    ...
    function mint(uint256 quantity, bytes32[] memory proof) whenNotPaused external payable {
        ...
        for (uint256 i = 0; i < quantity; ++i) {
            ...
            approve(market, id); // Marketplace approval TODO: can be removed, check
            id += 1;
            currentIndex+=1;
        }

        // Set approval for market to transact on the marketplace
        setApprovalForAll(market, true);  // TODO: can be removed, check
    ...
    function internalMint(address to, uint256 quantity) external onlyAdmin{
        ...
        // TODO: verify if this is required, remove if not
        if(msg.sender == to) {
    ...
    // ContractURI for OpenSea support
