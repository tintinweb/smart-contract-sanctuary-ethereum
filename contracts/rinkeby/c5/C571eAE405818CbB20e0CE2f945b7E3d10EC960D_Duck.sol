...
contract Duck is IDuck, ERC721Enumerable, Ownable, Pausable {
   ...
   */
  function mintWhitelist(bytes32[] calldata merkleProof, uint256 amount, bool stake) external payable whenNotPaused {
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      if (!stake) {
        ...
        _safeMint(address(venery), minted);
        tokenIds[i] = minted;
      }
    }
    if (stake) venery.addManyToVeneryAndQuackery(_msgSender(), tokenIds);
  }
