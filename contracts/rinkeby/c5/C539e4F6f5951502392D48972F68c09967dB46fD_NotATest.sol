...
contract NotATest is ERC721A , Ownable, PaymentSplitter, Pausable,  ReentrancyGuard {
    ...
    //founders wallet
    0xB37601959Ebd13AB6EA72D62B4c419a430E6b571,
    //community wallet
    0xA21d72D0C7393B553284C3E4073b3dC50c6A2E7c
    ];

    uint[] private shareList = [90, 10];

    constructor()ERC721A("NotATest", "NAT") PaymentSplitter(addressList, shareList){
    }
    ...
    function mint(uint256 _mintAmount) public payable nonReentrant{
        ...
        if (t + _mintAmount >= 1000) {
            require(msg.value >= _mintAmount * cost, "Need to send more ETH");
        }
        //require(cost * _mintAmount == msg.value, "Need to send more ETH");
    ...
    function gift(uint256[] calldata _mintAmount, address[] calldata recipient) external onlyOwner{
        for(uint i = 0; i < recipient.length; ++i){
            require(totalSupply() + _mintAmount[i] <= maxSupply, "reached max supply");
