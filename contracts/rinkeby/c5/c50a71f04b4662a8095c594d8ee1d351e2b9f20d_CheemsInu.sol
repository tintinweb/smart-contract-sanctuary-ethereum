...
contract CheemsInu is IBEP20, Ownable
{
    ...
    //Used for changing the taxes X time after launch.
    uint256 private tradingEnabledAt; //DO NOT CHANGE, THIS IS FOR HOLDING A TIMESTAMP
	uint256 private taxChangeTime = 600; // 10 Minutes in seconds. Change to however long you want to wait before the taxes automatically change.
	uint256 private enableTimedTaxChange = 1; //Leave 1 if using, set to 0 if not using.
    uint256 private _startingSellTax = 25; //Starting sell tax, will change to the taxes below inb the contructor section after taxChangeTime has been met.
    uint256 private _startingBuyTax = 25; //Starting buy tax, will change to the taxes below inb the contructor section after taxChangeTime has been met.
    uint256 private _startingTransferTax = 25; //Starting transfer tax, will change to the taxes below inb the contructor section after taxChangeTime has been met.
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            //Timed tax changes
            if(block.timestamp >= tradingEnabledAt + taxChangeTime && enableTimedTaxChange== 1) {
                _buyTax=_buyTax;
                _sellTax=_sellTax;
                _transferTax=_transferTax;
                enableTimedTaxChange = 0;
            }
            tax=_sellTax;
