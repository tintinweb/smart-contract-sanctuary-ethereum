    ...
    */
    function createVault(
        ...
        vaultsExtensions[_vaultIdTracker.current()] = VaultExtension(
            !privateVault, minimumFunding, 0, IERC165(collection).supportsInterface(type(IERC1155).interfaceId)
        );
    ...
    */
    function _vote(uint256 vaultId, Participant storage participant, bool yes, VoteFor voteFor) internal {
        ...
        if (voteFor == VoteFor.Buying) {
            ...
            Participant[] memory participants = _vaultParticipants[vaultId];
            for (uint256 i; i < participants.length; i++) {
                if (participants[i].votedOnSellingOrCancellingSellOrder) {
                    firstProposalVote = false;
                    break;
                }
            }
            participant.votedOnSellingOrCancellingSellOrder = true;
