  ...
  //StakeWithAmd
   ...
   function withdrawInAmd(uint256 count) public {
       ...
       User storage user = users[msg.sender];
        ...
        require(endingTime > stakeTimeForEndind, "You cannot withdraw amout before Time.");
    ...
    function extendStake(uint256 count,uint256 newplan) public {
       ...
       checkEndingTime(msg.sender, count, user.stakerecord[count].plan);
        ...
        newDays = (currentTime  - timeBefore).div(1 minutes);
        
        calculateNewReward(msg.sender,count,user.stakerecord[count].amount, newDays,user.stakerecord[count].plan);
    ...
    // function checkAfterTimeBonus(uint256 userAddress,uint count) public {

    //     User storage user = users[userAddress];
    //     uint256 daysAccording = durations[user.stakerecord[user.stakecount].plan];
    //     daysAccording = 
    //     if(user.stakerecord[user.stakecount].staked.since > user.stakerecord[user.stakecount].plan);

    //     uint256 timeBefore = user.stakerecord[count].withdrawTime;
    //     uint256 currentTime = block.timestamp;
    //     nextDays = (currentTime  - timeBefore).div(1 days);
    ...
    function calculateNewReward(address userAddress, uint256 userStakeCount, uint256 amount, uint256 _newDuration, uint256 plan) public returns(uint256){
        ...
        user.stakerecord[userStakeCount].beforeExtendBonus = user.stakerecord[userStakeCount].beforeExtendBonus + amount.mul(newPercentage).div(percentDivider);
