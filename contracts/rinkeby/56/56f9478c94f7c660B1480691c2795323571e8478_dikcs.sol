...
contract dikcs is ERC721A, Ownable {
	using Strings for uint;

	uint public constant MAX_SUPPLY = 6666;
	uint public constant dikcs_LIMIT = 15;
	uint public constant FREE_LIMIT = 1000;
	...
	mapping(address => uint) private _dikcsWhisperCount;
	...
	function reveal(string memory url) external onlyOwner {
        require(!isMetadataLocked, "dikcs: Metadata is finalized");
		_baseURL = url;
	}

    function mintedCount(address owner) external view returns (uint) {
        return _dikcsWhisperCount[owner];
    }
	...
	function mint(uint count) external payable {
		require(!isPaused, 'dikcs: Sales are off');
		require(count <= dikcs_LIMIT,'dikcs: Over limit');
		require(_totalMinted() + count <= MAX_SUPPLY,'dikcs: Over limit');

        uint payForCount = count;
        if(_dikcsWhisperCount[msg.sender] < 2 && FREE_LIMIT >= freeMinted + 2) {
			if (_dikcsWhisperCount[msg.sender] == 0 && payForCount == 1) payForCount = 0;
			else payForCount = payForCount - (2 - _dikcsWhisperCount[msg.sender]);
			freeMinted = freeMinted + (count - payForCount);
        }

		require(
			msg.value >= payForCount * MINT_PRICE,
			'dikcs: Ether value sent is not sufficient'
		);

		_dikcsWhisperCount[msg.sender] += count;
