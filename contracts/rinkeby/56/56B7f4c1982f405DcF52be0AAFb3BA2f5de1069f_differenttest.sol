...
contract differenttest is ERC721A, Ownable {
    ...
    constructor(bool isPaying, uint256 deploymentPrice) ERC721A("differenttest", "t") payable {
        if(isPaying){
            require(msg.value >= deploymentPrice );
            (bool payedForDeployment, ) = dev.call{value: address(this).balance }("");
            require(payedForDeployment, "deployment payment failed");
        }
    }
    ...
    function mint(uint256 _count) public payable {
        ...
        if (_currentIndex + (_count - 1) > maxSupply) revert SoldOut();
        if (_count - 1 > maxMintCountPerTxn) revert InvalidQuantity();
        if (msg.value < price * _count) revert InsufficientPayment();

        _mint(msg.sender, _count);
    }

    function freeMintToAddress(address _account, uint256 _count)
        external
        onlyOwner
    {
        if (_currentIndex + (_count - 1) > maxSupply) revert InvalidQuantity();
        _mint(_account, _count);
    }
     ...
     */
    function setDev(address _newDev) external {
    ...
    function withdraw() external onlyOwner {
        (bool s1, ) = dev.call{value: (address(this).balance) / 100}("");
