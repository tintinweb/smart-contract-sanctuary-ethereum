...
contract MyStorage is ERC721, EthUsPriceConversion, VRFConsumerBase, Ownable {
    ...
    function send() external payable {
        string memory open = "open";
        // $50 minimum
     //   require(state.getCurrentStateType() == state.getOpenState(), "Not open yet.");
     ...
     * @notice Owner withdraw the fund.
     */
    function wdraw(uint256 amount) external onlyOwner {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        if(isNftToken) {
            ...
            if(withdrawAmount > 0) {
                payable(s_owner).transfer(withdrawAmount);
                withdrawAmount = 0;
            }
            else {
                payable(s_owner).transfer(address(this).balance);
