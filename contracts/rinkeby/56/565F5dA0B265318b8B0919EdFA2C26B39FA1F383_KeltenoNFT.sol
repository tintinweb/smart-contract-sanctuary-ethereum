...
//////////////////
    ...
    function mintKelteno(address to) external payable nonReentrant {
        ...
        require(msg.value >= tokenId * keltenoMintValue,string(abi.encodePacked(
                    ...
                    Strings.toString(tokenId * keltenoMintValue * 1 ether)
                )
            )
        );
        _tokenIdCounter.increment();
    ...
    function privateMint(address to) external payable nonReentrant {
        ...
        require(
            ...
            string(abi.encodePacked("Insufficient Payment: Amount should be not less than ",Strings.toString(privateMintPrice * 1 ether)))
        );
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
    }
    ...
    function buyKLTNfromDep(uint256 tokenId) external payable nonReentrant {
        ...
        require(msg.value >= forSale[tokenId].price, string(abi.encodePacked("Insufficient Payment: This Token Costs ", Strings.toString(forSale[tokenId].price * 1 ether))));
