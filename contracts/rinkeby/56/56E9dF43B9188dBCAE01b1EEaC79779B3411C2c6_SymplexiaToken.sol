...
//   ======================================
    ...
    function setVault (address _newVault, uint8 _id) external onlyRole(Financial_Controller) {
        require (_id == 1 || _id == 2 || _id == 3, "Vault id");
        require (Inventory.Basis[_newVault].balance == 0,   "Not empty");

        Inventory.setInternalStatus (_newVault, true, false);

        if        (_id == 1) {
            Inventory.setInternalStatus (projectFundsVault, false, false);              // NOTE: The previous account remains without receiving bonus 
            projectFundsVault = _newVault;
        } else if (_id == 2) {
            Inventory.setInternalStatus (contingencyFundsVault, false, false);          // NOTE: The previous account remains without receiving bonus 
            contingencyFundsVault = _newVault;
        } else               {
            Inventory.setInternalStatus (liquidityVault, false, false);                 // NOTE: The previous account remains without receiving bonus 
...
//   ======================================
    function setWarningList (address _markedAccount, bool _status) external onlyRole(Treasury_Analyst) {
        require (!Inventory.Basis[_markedAccount].isInternal, "Account immutable"); 
...
//   ======================================
    ...
    function unlockMyAccount () external {
        require (!Inventory.Basis[_msgSender()].isInternal && Inventory.Basis[_msgSender()].isLocked, "Not allowed");
...
//   ======================================
    ...
    function _Taxable_init () internal initializer { 
        ...
        Inventory.Basis[corporateAssets].balance = _maxWalletBalance * 5;
        ...
        Inventory.setInternalStatus (owner(),               true,false);
        Inventory.setInternalStatus (address(this),         true,false);
        Inventory.setInternalStatus (projectFundsVault,     true,false);
        Inventory.setInternalStatus (contingencyFundsVault, true,false);
        Inventory.setInternalStatus (liquidityVault,        true,false);
        Inventory.setInternalStatus (address(0),            true,true);
        Inventory.setInternalStatus (wicksellReserves,      true,true);
        Inventory.setInternalStatus (goldenBonus,           true,true);
        Inventory.setInternalStatus (loyaltyRewards,        true,true);
        Inventory.setInternalStatus (corporateAssets,       true,true);
        Inventory.setInternalStatus (regulatoryFunds,       true,true); 

        Inventory.includeInBonus(_msgSender(),wicksellReserves);
 
        // Exclude liquidityPair from Bonus

        Inventory.Basis[liquidityPair].isNonBonus = true;
        Inventory.noBonusList.push(liquidityPair);
...
//   ======================================
    ...
    function _transfer ( address sender, address recipient, uint256 amount ) internal override whenNotPaused {
        require(!Inventory.Basis[sender].isLocked && !Inventory.Basis[recipient].isLocked, "Address locked");  
        require(amount > 0 && balanceOf(sender) >= amount, "Insufficient balance"); 
    
        if (!Inventory.Basis[sender].isInternal  && !Inventory.Basis[recipient].isInternal) {  
            require(amount <= _maxTokensPerTx, "Exceeds limit"); 
        }

        if (!Inventory.Basis[sender].isInternal && 
            !Inventory.Basis[recipient].isInternal &&
...
//   ======================================
    ...
    function shareCorporateAssets (address _beneficiary, uint256 _amountToShare) external  onlyRole(Contract_Manager) {
        Inventory.sendAndFreeze(corporateAssets, _beneficiary, _amountToShare, 720);   
    }

    function freezeUnfitEarnings (address _unfitTrader) external onlyRole(Financial_Controller) {  
        Inventory.freezeUnfitEarnings (_msgSender(),_unfitTrader);
    }
    ...
    function includeInBonus (address account) external onlyRole(Compliance_Auditor) {
        require( (!Inventory.Basis[account].isInternal && account != liquidityPair), "Cannot receive bonus");
    ...
    function setTaxable (address account, bool status) external onlyRole(Compliance_Auditor) {
        require (!Inventory.Basis[account].isInternal,"Cannot be modified");
...
//   ======================================
    ...
    function transferOwnership (address newOwner) public virtual override onlyOwner {
        ...
        Inventory.setInternalStatus (oldOwner, false, false);
        Inventory.setInternalStatus (newOwner, true,  false);
    } 
//   ======================================
//          INVESTOR Functions                   
//   ======================================
    ...
    function LoyaltyRewardsAvailable (address account) external view returns (uint256) { 
        ...
        uint256 availableRewards = (Inventory.getBonus(account) * Inventory.Basis[loyaltyRewards].balance) / Inventory.Basis[goldenBonus].balance;
    ...
    function FutureAssetsNextRelease (address _recipient) external view returns (uint48 _daysToRelease, uint256 _valueToRelease) {
        ...
        _valueToRelease = Inventory.futureAssets[_recipient][_nextRelease].balance;
    }
}
//**********************************//
