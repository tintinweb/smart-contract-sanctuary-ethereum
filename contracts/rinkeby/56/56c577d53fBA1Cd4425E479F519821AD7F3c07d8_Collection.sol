...
contract CollectionDescriptor {
    ...
    function generateRGB(uint256 tokenId, uint256 hashIndex) public pure returns (uint256) {
       ...
       // bytes memory hash = abi.encodePacked(bytes32(tokenId));
        ...
        //uint256 fillg = uint256(toUint8(hash,1))*250/256;
        //uint256 fillb = uint256(toUint8(hash,2))*250/256;
        //return string(abi.encodePacked('rgb(',fillr,', ',fillg,', ',fillb));
        return fillr;
     }
...
contract Collection is ERC721 {
    ...
    function generateRGB(uint256 tokenId, uint256 hashIndex) public view returns (uint256) {
        return descriptor.generateRGB(tokenmetadata[tokenId].hash,hashIndex);
    }
