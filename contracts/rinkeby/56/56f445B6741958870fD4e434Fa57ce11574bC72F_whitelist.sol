...
contract whitelist is ERC721A, Pausable, Ownable {
    constructor() ERC721A("whitelist", "WL") {}

    bytes32 public WLmerkleRoot =
        0x31bf0ba99ab5ebd585d97bca830f44e69228f242679d23aad2c5ddfb6909a682;
    ...
    uint256 public WLquantity = 2;

    function whitelistMint(bytes32[] calldata _proof) public {
        // require(block.timestamp >= WLStartTime, "Sale not started");   if want to set time, use this
        // require(quantity <= maxMinting, "Minting need to be less then maxMinting");
        require(
            totalSupply() + WLquantity <= maxSupply,
            "All Bears Are Ready!"
        );
        require(
            MerkleProof.verify(
        ...
        require(!usedWL[msg.sender], "Your Bear Is Ready!");
        _safeMint(msg.sender, WLquantity);

        usedWL[msg.sender] = true;
        tokenCounter += WLquantity;
    }
}
