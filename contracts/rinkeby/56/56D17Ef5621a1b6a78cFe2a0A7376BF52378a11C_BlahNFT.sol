...
contract BlahNFT is BlahNFTScore {
    ...
    function uploadMetadata(BlahNFTTypes.CityWithId[] memory _cities, BlahNFTTypes.CountryToFaction[] memory _countryToFaction) external onlyOwner {
        ...
        for(uint i = 0; i < _cities.length; i++) {
            ...
            if(idToCities[_cities[i].id].points == 0 && idToCities[_cities[i].id].cityFaction != 0) {
        ...
        for(uint i = 0; i < _countryToFaction.length; i++) {
            countryToFaction[_countryToFaction[i].country] = _countryToFaction[i].faction;
        }
    }
    ...
    function _transfer(
        ...
        if(addressToFaction[from] != addressToFaction[to]) {
            ...
            uint8 winningFaction = countryToFaction[city.country];
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        string memory imageString = "https://gateway.pinata.cloud/ipfs/Qmf5gV47UyV3j6D3oY49vC6t4J8ArhWBCYJ3RoUzgLjV9u";//string(abi.encodePacked('data:image/svg+xml;base64,', image));
        ...
        // nameString = toHex(keccak256(abi.encodePacked(city.city)));
...
contract BlahNFTScore is BlahNFTNFT {
    ...
    function setCountryWinningFaction(string memory _country, uint8 _faction) internal {
        countryToFaction[_country] = _faction;
    }
    ...
    function calculateCountryScore(string memory _country) public view returns (uint, uint, uint) {
        ...
        for(uint i = MAX_CITIES - FOUNDERS_RESERVE_AMOUNT; i < getReservedTokenIds(); i++) {
            uint id = i + 1;
            BlahNFTTypes.City memory city = idToCities[id];
            if(!hasUploadedMetadata(city)) {        //only count cities that have their metadata
                continue;
...
contract BlahNFTNFT is ERC721, Ownable, ReentrancyGuard {
    ...
    function getTokenIds() internal view returns (uint256) {
        return _tokenIds.current();
    }

    function getReservedTokenIds() internal view returns (uint256) {
