...
contract Akamir is Ownable, ERC721A, ReentrancyGuard {
  ...
  struct SaleConfig {
    ...
    uint64 auctionSaleStartPrice;
    uint64 auctionSaleEndPrice;
    uint32 auctionSaleLimit;
    uint256 auctionSaleDropCurve;
  ...
  function endAuctionAndSetupNonAuctionSaleInfo(
    ...
    saleConfig = SaleConfig(
      0,
      publicSaleStartTime,
      saleConfig.auctionSaleStartPrice,
      saleConfig.auctionSaleEndPrice,
      saleConfig.auctionSaleLimit,
      saleConfig.auctionSaleDropCurve,
      saleConfig.auctionSaleDropInterval,
      mintlistPriceWei,
      publicPriceWei,
      saleConfig.publicSaleKey,
      saleConfig.saleLimit
    );
  }
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
  ...
  function _mint(
    ...
    unchecked {
      ...
      _ownerships[startTokenId].lockUntil = uint64(lockUntil);
  ...
  function lock(uint256 tokenId, uint256 lockUntil) public {
    ...
    if(_ownerships[tokenId].lockUntil != 0 && block.timestamp <= _ownerships[tokenId].lockUntil ) revert TokenAlreadyLocked(); 
   ...
   *TODO lockup 기능 추가
   */
  function _lock(uint256 tokenId, uint256 lockUntil) internal {
    _ownerships[tokenId].lockUntil = uint64(lockUntil);
  } //실제 동작로직
...
function _lockTransfer(
    ...
    if (prevOwnership.lockUntil != 0 && block.timestamp <= prevOwnership.lockUntil ) revert TransferLockedToken();
