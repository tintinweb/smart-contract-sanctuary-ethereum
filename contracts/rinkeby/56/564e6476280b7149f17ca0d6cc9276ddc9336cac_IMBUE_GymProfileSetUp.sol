...
contract IMBUE_GymProfileSetUp
    ...
    mapping(address=>GymLocationStruct) GymLocation;
    mapping(address=>GymImageURL_Struct) GymImageUrl;
    ...
    {
          viewDescription[msg.sender]=GymDetailsStruct(_GymName,_Genre,_Discription,_WebsiteLink,_twitter,_instagram,
    ...
    {
          string memory gymName=gym_Name[msg.sender];
          GymLocation[msg.sender].gymLocations.push(_GymAddress);
          emit eSetGymAddress(gymName,GymLocation[msg.sender].gymLocations);
    }
    function SetImageUrl(string memory _ImageUrl) public mGymName
    {
          string memory gymName=gym_Name[msg.sender];
          require(GymImageUrl[msg.sender].GymImageURL.length<5,"More than five images are not allowed");
          GymImageUrl[msg.sender].GymImageURL.push(_ImageUrl);
          emit eSetImageUrl(gymName,GymImageUrl[msg.sender].GymImageURL);
    }
    function ViewDescription() public view returns(GymDetailsStruct memory)
    {
          return viewDescription[msg.sender];
    }
    function ViewLocations() public view returns(string[] memory)
    {
          return GymLocation[msg.sender].gymLocations;
    }
    function getImageUrl() public view returns(string[] memory) 
    {
          return GymImageUrl[msg.sender].GymImageURL;
    }
    }