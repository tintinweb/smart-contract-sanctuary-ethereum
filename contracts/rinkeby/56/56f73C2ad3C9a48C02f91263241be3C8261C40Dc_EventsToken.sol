...
contract EventsToken is OwnableInternal, ERC1155NSBase, ERC1155MetadataInternal, ERC165 {
	...
	modifier onlyAuthorized() {
		require(EventsStorage.layout().authorized[msg.sender] = true, "Ownable: sender must be owner");
		_;
	}
	...
	function mintEdition(
		...
		require(count > 0, "mintEdition: mint a quantity");
		uint256 index = EventsStorage.layout().index;

		unchecked {
			ERC1155NSBaseStorage.layout().balances[index][recipient] = count;
			if (count == 1) {
				emit TransferSingle(_msgSender(), address(0), recipient, index, count);
			} else {
				emit TransferBatch(
					msg.sender,
					address(0),
					recipient,
					_asSingletonArray(index),
					_asSingletonArray(count)
				);
			}
			_setTokenURI(index, tokenURI);
			EventsStorage.layout().index++;
		}
	}
}
