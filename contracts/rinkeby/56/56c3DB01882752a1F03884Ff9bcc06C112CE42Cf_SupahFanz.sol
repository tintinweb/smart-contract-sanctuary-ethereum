...
contract SupahFanz is ERC721A, Ownable {
    using Address for address;
    using Strings for string;

    uint256 public constant MAX_SUPPLY = 2015;
    ...
    function mint(uint256 _amount) public {
        require(isSaleActive, "Sorry love no sale");
        require(tx.origin == msg.sender, "NO CONTRACTS YO");
        require(
            _amount + _numberMinted(msg.sender) <= walletLimit,
            "Only 3 please - greed gooodbye"
        );
    ...
    function freezeMetadata() public onlyAuthorized {
        require(!metadataFrozen, "Metadata is ice-cold frozen");
        metadataFrozen = true;
    }
    ...
    function _performMint(address _to, uint256 amount) private {
        require(_to != address(0), "Cannot mint to 0x0 yo");
        require(amount > 0, "pick a better number");
        require(amount + totalSupply() <= MAX_SUPPLY, "last one left");
