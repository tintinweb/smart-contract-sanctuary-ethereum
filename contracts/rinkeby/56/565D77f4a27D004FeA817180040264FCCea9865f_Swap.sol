...
contract Swap is Initializable, OwnableUpgradeable, ChainlinkClientUpgradeable {
    ...
    function fetchPrice(uint _index,uint _amount) internal returns(uint) {
        if(chainlinkFeedEnabled){
            ...
            return fetchFromChainLink() * _amount / (10**8);
                ...
                return _amount * (10**8) / fetchFromChainLink();
            }
        }
        else{
    ...
    function addLiquidity(uint256 amountA, uint256 amountB) external onlyOwner {
        require(fetchPrice(SELL_INDEX, amountA) == amountB, "amountA should be equal with amountB");
        TransferHelper.safeTransferFrom(tokenA, msg.sender, address(this), amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, address(this), amountB);
        reserveA = reserveA + amountA;
        reserveB = reserveB + amountB;
    ...
    function removeLiquidity(uint256 amountA, uint256 amountB) external onlyOwner {
        // TODO: check the fetch price logic for addLiquidity and removeLiquidity
        require(fetchPrice(SELL_INDEX, amountA) == amountB, "amountA should be equal with amountB");
