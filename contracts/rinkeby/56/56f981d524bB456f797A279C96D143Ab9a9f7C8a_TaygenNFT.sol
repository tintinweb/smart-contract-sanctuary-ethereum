    ...
    ERC721PausableUpgradeable
{
    ...
    */
    ...
    function _tokenURI(uint _tokenId, uint _USDCBalance, uint _tUSDCAmount, uint _conversionRate, uint _usdcFee, uint _withdrawTime) internal pure returns (string memory output) {
        ...
        output = string(abi.encodePacked(output, "conversionRate ", toString(_conversionRate), '</text></svg>'));
        output = string(abi.encodePacked(output, "USDC_Withdrawal_Fee ", toString(_usdcFee), '</text></svg>'));
        output = string(abi.encodePacked(output, "withdraw_time ", toString(_withdrawTime), '</text></svg>'));
