...
contract EggoFactory is FactoryERC721, Ownable {
     ...
     */
    uint256 EGGO_SUPPLY = 41;

    /*
     * Three different options for minting Eggo (basic, premium, and gold).
     */
    uint256 NUM_OPTIONS = 3;
    uint256 SINGLE_EGGO_OPTION = 0;
    uint256 PACK_3_EGGO_OPTION = 1;
    uint256 PACK_7_EGGO_OPTION = 2;
    uint256 NUM_EGGO_IN_PACK_3 = 3;
    uint256 NUM_EGGO_IN_PACK_7 = 7;
    ...
    function mint(uint256 _optionId, address _toAddress) override public {
        ...
        Eggo openseaEggo = Eggo(nftAddress);
        if (_optionId == SINGLE_EGGO_OPTION) {
            openseaEggo.mintTo(_toAddress);
        } else if (_optionId == PACK_3_EGGO_OPTION) {
            for (
                uint256 i = 0;
                i < NUM_EGGO_IN_PACK_3;
                i++
            ) {
                openseaEggo.mintTo(_toAddress);
            }
        } else if (_optionId == PACK_7_EGGO_OPTION) {
            for (
                uint256 i = 0;
                i < NUM_EGGO_IN_PACK_7;
                i++
            ) {
                openseaEggo.mintTo(_toAddress);
            }
    ...
    function canMint(uint256 _optionId) override public view returns (bool) {
        ...
        Eggo openSeaEggo = Eggo(nftAddress);
        uint256 eggoSupply = openSeaEggo.totalSupply();

        uint256 numItemsAllocated = 0;
        if (_optionId == SINGLE_EGGO_OPTION) {
            numItemsAllocated = 1;
        } else if (_optionId == PACK_3_EGGO_OPTION) {
            numItemsAllocated = NUM_EGGO_IN_PACK_3;
        } else if (_optionId == PACK_7_EGGO_OPTION) {
            numItemsAllocated = NUM_EGGO_IN_PACK_7;
        }
        return eggoSupply < (EGGO_SUPPLY - numItemsAllocated);
    }

    function tokenURI(uint256 _optionId) override external view returns (string memory) {
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, Strings.toString(_optionId), ".jpg")) : "";
    }
...
contract Eggo is ERC721, ContextMixin, ERC721Enumerable, ReentrancyGuard, Ownable{
    ...
    // Pending Ids
    uint256[41] private _pendingIds;
    ...
    function _randomMint(address _to) internal returns (uint256) {
        require(totalSupply() < MAX_NFT_SUPPLY, "Eggo: max supply reached");
