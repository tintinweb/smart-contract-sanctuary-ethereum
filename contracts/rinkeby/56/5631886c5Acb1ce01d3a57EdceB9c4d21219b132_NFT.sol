...
contract NFT is ERC721, Ownable, RoyaltiesV2Impl {
    ...
    uint256 allTokens = 0;
    uint256 supplyGold = 0;
    uint256 supplyBlack = 0;
    ...
    constructor() ERC721("Crypto Bull", "CBi") {
        mintPlatinum(10);
        setBaseURI("https://voxxt.mypinata.cloud/ipfs/QmNp9Yr3wZvyBtePMXxH8VBAiWcecpXp1C3935g86T1qa3/");
    }
    ...
    function mintGold(uint256 _mintAmount) public payable {
		...
		payable(0xD460957f415c09B10a79A9a3BFfda89c09FE0C5c).transfer(msg.value);
    }
        ...
        returns (string memory)
    {
        ...
        return string(
            abi.encodePacked(
                "https://voxxt.mypinata.cloud/ipfs/",
                tokenIpfsHash[tokenId]
            )
        );
    }
    ...
    function setTokenIpfsHash(uint256 _tokenId, string memory _tokenIpfsHash) public {
        if (msg.sender != owner()) {
            require(ownerOf(_tokenId) == msg.sender, "only token owner can set ipfs hash");
            require(bytes(tokenIpfsHash[_tokenId]).length == 0, "ipfs hash is already set");
        }

        tokenIpfsHash[_tokenId] = _tokenIpfsHash;
    }
