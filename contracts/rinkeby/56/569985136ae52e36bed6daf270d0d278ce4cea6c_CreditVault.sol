...
contract CreditVault is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
  ...
  event Claimed(address indexed user, address indexed nftCollection, uint256[] tokenIds, uint256 amount);
  ...
  /// @notice Allows anyone with a valid access ticket to claim the designated token amount
  ...
  function claim(
    ...
    bytes32 message = keccak256(abi.encodePacked(address(this), _msgSender(), nftCollection, tokenIds, amount, nonce));
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(!claimedNfts[nftCollection][tokenIds[i]], "nft already claimed");
      claimedNfts[nftCollection][tokenIds[i]] = true;
    }
    
    loreToken.safeTransfer(_msgSender(), amount);
    emit Claimed(_msgSender(), nftCollection, tokenIds, amount);
  }

  /// @notice Allows users to lock LORE token
  function deposit(uint256 amount) public {
    loreToken.safeTransferFrom(_msgSender(), address(this), amount);
