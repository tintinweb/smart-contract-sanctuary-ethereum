...
contract CyberChip is ERC721, ERC721Enumerable, Ownable {
    ...
    address private firstNFTAddress;
    address private secondNFTAddress;
    ...
    mapping(uint256 => bool) private phase1NFTMapping;

    mapping(address => bool) private phase2AddresMapping;
    mapping(uint256 => bool) private phase2NFT1Mapping;
    mapping(uint256 => bool) private phase2NFT2Mapping;

    constructor(
        address _tokenAddress,
        address _fistNFTAddress,
        address _secondNFTAddress
    ) ERC721("Cyber Chip", "CHIPS") {
        tokenAddress = _tokenAddress;
        firstNFTAddress = _fistNFTAddress;
        secondNFTAddress = _secondNFTAddress;
        ...
        returns (bool res)
    {
        ...
        for (uint256 i = 0; i < ERC721(firstNFTAddress).balanceOf(addr); i++) {
            uint256 id = ERC721Enumerable(firstNFTAddress).tokenOfOwnerByIndex(
                addr,
                i
            );
            if (!phase1NFTMapping[id]) return true;
        }
        return false;
    }
    ...
    function _mapUsedNFTsAndAddressPhase1(address addr) internal {
        for (uint256 i = 0; i < ERC721(firstNFTAddress).balanceOf(addr); i++) {
            ...
            if (!phase1NFTMapping[id]) phase1NFTMapping[id] = true;
        }
        ...
        returns (uint256 res)
    {
        ...
        for (uint256 i = 0; i < ERC721(firstNFTAddress).balanceOf(addr); i++) {
            ...
            if (!phase2NFT1Mapping[id]) {
                if (phase1NFTMapping[id] || phase1AddresMapping[addr]) return 4;
                else return 5;
            }
        }
        for (uint256 i = 0; i < ERC721(secondNFTAddress).balanceOf(addr); i++) {
            uint256 id = ERC721Enumerable(secondNFTAddress).tokenOfOwnerByIndex(
                addr,
                i
            );
            if (!phase2NFT2Mapping[id]) {
    ...
    function _mapUsedNFTsAndAddressPhase2(address addr, uint256 count) internal {
        for (uint256 i = 0; i < ERC721(firstNFTAddress).balanceOf(addr); i++) {
            ...
            if (!phase2NFT1Mapping[id]) phase2NFT1Mapping[id] = true;
        }
        for (uint256 i = 0; i < ERC721(secondNFTAddress).balanceOf(addr); i++) {
            uint256 id = ERC721Enumerable(secondNFTAddress).tokenOfOwnerByIndex(
                addr,
            ...
            if (!phase2NFT2Mapping[id]) phase2NFT2Mapping[id] = true;
        }
