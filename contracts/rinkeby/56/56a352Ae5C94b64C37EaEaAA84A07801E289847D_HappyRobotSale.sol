...
contract HappyRobotSale is Ownable {
    ...
    uint256 private nftMintingFee = 0.3 ether;
    uint256 private wlTokenMintingFee = 0.2 ether;
    ...
    */
    ...
    /**
    * get nft minting fee
    * @return nft minting fee
    */
    function getNFTMintingFee() public view returns (uint256) {
        return nftMintingFee;
    }

    /**
    * set nft minting fee
    * @param _fee nft minting fee
    */
    function setNFTMintingFee(uint256 _fee) public onlyMaster {
        nftMintingFee = _fee;
    }
    ...
    */
    function setWhitelistTokenMintFee(uint256 _wlTokenMintFee) public onlyMaster {
        wlTokenMintingFee = _wlTokenMintFee;
    }
   ...
   */
    function mint(uint8 _saleType, uint16 _quantity, bytes calldata _data) public payable {
        ...
        require(canMintNFTForSaleType(_saleType, _quantity) == true, "Happy Robot: Maximum NFT minting already reached for the generation");

        require(canMintNFTForAccount(msg.sender, _quantity) == true, "Happy Robot: Maximum NFT minting already reached for the account");
        ...
        if (saleStauts == SALE_STATUS_PRE) { // presale
            if (!hasGift(msg.sender) && !isMaster(msg.sender)) {
                ...
                if (hasWhitelistToken) {
                    require(whitelistToken.balanceOf(msg.sender) >= _quantity, "Happy Robot: Not enough whitelist token for NFT minting quantity");
                }
            }
        ...
        if (isFreeMinting) {
            ...
            require(msg.value >= nftMintingFee * _quantity, "Happy Robot: Not enough ETH sent");

            // perform minting
            nft.mint(msg.sender, _saleType, _quantity, _data);

            uint256 mintingFee = nftMintingFee * _quantity;

            uint256 feeForDev = (uint256)(mintingFee / 200); // 0.5% to the dev
            walletDevTeam.transfer(feeForDev);

            // return back remain value
            uint256 remainVal = msg.value - mintingFee;
            address payable caller = payable(msg.sender);
    ...
    */
    function mintWhitelistToken(uint16 _quantity) public payable {
        ...
        require(canMintWLTokenForAccount(msg.sender, _quantity) == true, "Happy Robot: Maximum whitelist token minting  already reached for the account");
        ...
        if (!isMaster(msg.sender)) {
            require(msg.value >= wlTokenMintingFee * _quantity, "Happy Robot: Not enough ETH sent");

            // perform minting
            token.mint(msg.sender, _quantity);

            uint256 mintingFee = wlTokenMintingFee * _quantity;
