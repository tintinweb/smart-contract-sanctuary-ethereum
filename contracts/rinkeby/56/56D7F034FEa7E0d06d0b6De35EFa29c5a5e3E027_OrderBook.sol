...
contract OrderBook is ReentrancyGuard, IOrderBook, Governable{
    ...
    uint256 private constant USDF_PRECISION = 1e18;
    ...
    mapping (address => mapping(uint256 => IncreaseOrder)) private increaseOrders;
    mapping (address => uint256) private increaseOrdersIndex;
    mapping (address => mapping(uint256 => DecreaseOrder)) private decreaseOrders;
    mapping (address => uint256) private decreaseOrdersIndex;
    mapping (address => mapping(uint256 => SwapOrder)) private swapOrders;
    mapping (address => uint256) private swapOrdersIndex;

//    address private gov;
    address private weth;
    address private usdf;
    address private router;
    address private vault;
    uint256 private minExecutionFee;
    uint256 private minPurchaseTokenAmountUsd;
...
//        require(msg.sender == gov, "Governable: forbidden");
        require(isAdmin[msg.sender], "Governable: forbidden");
        _;
    }
    ...
    function removeGov(address _gov) external onlyGov {
        isAdmin[_gov] = false;
    }
}