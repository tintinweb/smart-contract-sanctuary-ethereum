...
contract ObserverForTest {
  ...
  function fetchObservations(
    ...
    address _token1,
    uint24 _fee,
    uint32[] calldata _observations
  ) external returns (int56[] memory _tickCumulatives) {
    IUniswapV3Pool _pool = IUniswapV3Pool(UNISWAP_FACTORY.getPool(_token0, _token1, _fee));

    _tickCumulatives = new int56[](_observations.length);

    for (uint256 _i; _i < _observations.length; _i++) {
      (, _tickCumulatives[_i], , ) = _pool.observations(_observations[_i]);
    }
  }
}
