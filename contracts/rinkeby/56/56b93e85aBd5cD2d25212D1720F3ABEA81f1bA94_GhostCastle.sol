...
contract GhostCastle is ERC721, AccessControl {
    ...
    string public baseTokenURI = "https://nft.may.social/nft-metadata/1/";
    ...
    constructor(address weirdoGhostGangAddress) ERC721("GhostCastle", "GC") {
    ...
    function mint(uint256 tokenId, bytes memory sign) external {
        require(Address.isContract(msg.sender) == false, "mint: Prohibit contract calls");
        require(!mintedToken[tokenId], "mint: This token has minted");
        require(_tokenIdCounter.current() + 1 <= TOTAL_SUPPLY, "mint: Mint would exceed max supply");
//        require(_weirdoGhostGang.ownerOf(tokenId) == msg.sender, "mint: This token is not yours");
        //        require(PRICE <= msg.value, "mint: Ether value sent is not correct");

        bytes32 digest = keccak256(abi.encodePacked(tokenId));
    ...
    function burnMintedToken(uint256 tokenId) external onlyRole(MAINTAIN_ROLE){
        mintedToken[tokenId] = false;
    }
