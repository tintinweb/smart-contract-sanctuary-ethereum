...
contract Pixellations is ERC721, Ownable {
    ...
    function random(uint256 input, uint256 min, uint256 max) public view returns (uint256) {
        ...
        return max - (uint256(keccak256(abi.encodePacked(input, msg.sender, tokenCount.current(), block.timestamp))) % range) - 1;
    }
    ...
    constructor() ERC721("PixellationsDelta", "PXL") {}
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        Metadata memory metadata = Metadata(0, 0, 0, 0, random(tokenId, 2, 7), smallStarOptions[random(tokenId, 0, 4)]);
        
        uint256 bigStarRefX = random(tokenId * metadata.bigStars, 0, 17) * 2;
        uint256 bigStarRefY = random(tokenId * metadata.smallStars, 0, 17) * 2;
        ...
        for (uint256 i = 0; i < metadata.bigStars; i++) {
            svg = string(
                abi.encodePacked(
                    svg, '<rect x="', uint2str(random((tokenId + i), bigStarRefX, bigStarMaxX)), '" y="', uint2str(random((tokenId + i * 2), bigStarRefY, bigStarMaxY)), '" width="2" height="2" fill="#FFF"></rect>'
                    )
                );
        }
        ...
        for (uint256 i = 0; i < metadata.smallStars; i++) {
            ...
            svg = string(
                abi.encodePacked(
                    svg, '<rect x="', uint2str(random((i), 0, 64)), '" y="', uint2str(random((tokenId + i), 0, 64)), '" width="1" height="1" fill="', getColor(i), '" opacity="', currentOpacity, '"></rect>' 
                    )
                );
        }
        ...
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            '{"name": "Pixellation #', uint2str(tokenId), '",',
    ...
    function mint(uint256 numberOfTokens) external payable {
        ...
        //require(msg.value == numberOfTokens * mintFee, "Wrong ETH value sent.");
        require(numberOfTokens > 0, "Quantity must be greater than 0.");
        require(numberOfTokens < 4, "Quantity must be less than 4.");
