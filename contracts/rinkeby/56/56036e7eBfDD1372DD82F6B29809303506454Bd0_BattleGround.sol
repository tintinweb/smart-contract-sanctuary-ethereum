...
contract BattleGround is ERC721URIStorage, VRFConsumerBase{
    ...
    //     emit NftPurchase(acquiredTokens[0], addonsGenerated[0], acquiredTokens[1], addonsGenerated[1], acquiredTokens[2], addonsGenerated[2], msg.sender);
    //     tokenCounter+=3;
        ...
        override
    {
        ...
        if(generatedAllNumbers == true)
        {
            ...
            if(boxType==BoxType.Common){
                uint index_char = requests[index].randomNumberToCharacter%listofCommonChars[addons].length;
                _setTokenURI(requests[index].tokenID, listofCommonChars[addons][index_char]);
            }
            else if(boxType==BoxType.Epic){
                uint index_char = requests[index].randomNumberToCharacter%listofEpicChars[addons].length;
                _setTokenURI(requests[index].tokenID, listofEpicChars[addons][index_char]); 
            }
            else if(boxType==BoxType.Legendary){
                uint index_char = requests[index].randomNumberToCharacter%listofLegendaryChars[addons].length;
