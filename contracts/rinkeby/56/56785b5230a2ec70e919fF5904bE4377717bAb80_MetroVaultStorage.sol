...
contract MetroVaultStorage is Controllable, IMetroVaultStorage {
    ...
    event BlockTransfer(uint256 indexed tokenId, address indexed from, address indexed to);
    ...
    function stakeFromMint(address owner, uint256[] calldata tokenIds, uint16 cityId, uint40 extra) external onlyController {
        for (uint i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
    ...
    function setStake(uint256 tokenId, Stake calldata newStake) external onlyController {
        ...
        if (vault[tokenId].owner != newStake.owner) {
          ...
          emit BlockTransfer(tokenId, vault[tokenId].owner, newStake.owner);
        }
    ...
    function setStakeOwner(uint256[] calldata tokenIds, address newOwner, bool resetTimestamp) external onlyController {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
          ...
          emit BlockTransfer(tokenId, staked.owner, newOwner);
