...
contract Vendor is Ownable {
  ...
  function sellTokens(uint256 _amount) public {
    require(_amount > 0, "Amount of sell must be greater than 0");

    uint256 userBalance = yourToken.balanceOf(msg.sender);
    require(userBalance >= _amount, "User does not have sufficient tokens amount to sell");

    uint256 ethAmtToTransfer = _amount / tokensPerEth;
    uint256 ownerBalance = address(this).balance;
    require(ownerBalance >= ethAmtToTransfer, "Owner has insufficent balance to transfer");
