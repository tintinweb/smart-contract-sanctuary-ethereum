...
contract Land is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    function mintTransfer(address to, uint256 amount_req) external {
        ...
        bytes32 response_id = APIConsContract.requestcoords(
            amount_req,
            _tokenIdCounter
        );
        string memory testcond = "True";
        string memory response = APIConsContract.volume();
        // require(
        //     keccak256(abi.encodePacked(response)) ==
        //         keccak256(abi.encodePacked(testcond))
        // );
    ...
    function apiTest(uint256 num_req, uint256 tokenCount)
        public
        onlyOwner
        returns (bytes32)
    {
        CLClient APIConsContract = CLClient(APIConsAddress);
        return APIConsContract.requestcoords(num_req, tokenCount);
    }
