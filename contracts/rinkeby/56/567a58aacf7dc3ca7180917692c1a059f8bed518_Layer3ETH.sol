...
abstract contract MarketplaceEventsAndErrors {
    ...
    /// @param chainId The chain id that token is sold on.
    ...
    /// @param chainId The chain id that token is listed on.
    /// @param seller The seller and old owner of the token.
    /// @param collectionAddress The address that the token is from.
    /// @param tokenId The id of the token.
    /// @param price The price of the listing.
    ...
    /// @param chainId The chain id that token was sold on.
    ...
    /// @param chainId The chain id that token was sold on.
    /// @param seller The seller and lister of the listing.
...
contract Layer3ETH is IStargateReceiver, Ownable, MarketplaceEventsAndErrors {
    ...
    function buyCrosschain(
        ...
        // nativePrice is how much of the domestic token you need in order to buy in the foreign token
        // TODO: currently in order to buy crosschain, buyer is forced to take on the messaging costs (confirm logic on this)
        // If we do this, on the receiving end, it still fails the price check because of DEX fees
        // Do some sort of pre-check here to ensure message isn't sent unless msg.value can cover fees + nativePrice
        // if (msg.value < fee + nativePrice) revert InsufficientFunds();
        uint256 amountAfterFee = nativePrice - fee;

        // swap to usdc
        uint256 amountStable = _swapForPurchase(amountAfterFee);
    ...
    function buyLocal(
        ...
        if (listing.status != ListingStatus.ACTIVE_LOCAL)
    ...
    function editPrice(
        ...
        sellerListings[key].price = newPrice;
