...
contract Staker {
  ...
  //   After some deadline if it has at least some threshold of ETH,
  ...
  // collects ETH from numerous addresses using a payable stake() function and keeps track of balances.
  ...
  function stake() public payable deadlineReached(false) {
    ...
    console.log(balances[msg.sender], 'balance from sender');
  ...
  modifier thresholdReached() {
    require(address(this).balance >= threshold, 'Not enough ETH to reach the threshold');
    _;
  }

  modifier deadlineReached(bool beforeDeadline) {
    if (beforeDeadline) {
      require(block.timestamp >= deadline, 'time has past the deadline');
      _;
    } else {
      require(block.timestamp <= deadline, 'time has not past the deadline');
      _;
    }
  }
