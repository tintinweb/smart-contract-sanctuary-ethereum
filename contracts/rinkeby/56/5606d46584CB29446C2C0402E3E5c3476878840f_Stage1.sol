...
contract Stage1 is ERC721AQueryable, ERC721ABurnable, ERC2981, Ownable {

  uint256 public MAX_Supply = 10000; // maximum supply is 10K.
  uint8 public freePerWallet = 100;
  ...
  mapping(address => uint256) public whitelistedAddresses; // whitelisted Mint allowed for airdrops and all.
  mapping(address => uint256) public whitelistMinted; // amount minted by an address during whitelist period.
  ...
  constructor(string memory baseUri_, string memory unrevealedUri_, address feeReceiver_) ERC721A("Token Name", "Token Symbol") {
    baseUri = baseUri_;
  ...
  function setFreePerWallet(uint8 _amount) public onlyOwner {
  ...
  function setTokenRoyalty(uint256 tokenId, address receiver, uint96 feeNumerator) public onlyOwner {
    return _setTokenRoyalty(tokenId, receiver, feeNumerator);
  }
  ...
  function enablePublicMint(bool _state) public onlyOwner {
    publicMintEnabled = _state;
  }

  function enableWhitelistMint(bool _state) public onlyOwner {
  ...
  function adminMint(address _address, uint256 amount) public onlyOwner checkMintSupply(amount) {
    _safeMint(_address, amount);
  }

  function whitelistMint(uint256 _amount) public checkMintSupply(_amount) {
    require(whitelistMintEnabled, "whitelist is not enabled");
    require(whitelistedAddresses[msg.sender] >= _amount, "whitelist requirements do not meet.");
  ...
  function publicMint(uint256 amount) public checkMintSupply(amount) {
    require(publicMintEnabled, "public mint is not enabled");
    require(whitelistMinted[msg.sender] < freePerWallet - freeMinted[msg.sender] && freePerWallet - freeMinted[msg.sender] >= amount, "You have already minted");
    freeMinted[msg.sender] += amount;
    _safeMint(msg.sender, amount);
  }

  // This function is to set whitelisted by same address.
  function setWhitelistBySameAmount(address[] memory _addresses, uint256 _amount) public onlyOwner {
    for (uint8 i; i < _addresses.length; i++) {
      whitelistedAddresses[_addresses[i]] = _amount;
    }
  }

  function setWhitelistByDifferentAmount(address[] memory _addresses, uint256[] memory _amount) public onlyOwner {
    for(uint8 i; i < _addresses.length; i++) {
      whitelistedAddresses[_addresses[i]] = _amount[i];
    }
  }
}
