 ...
 */
contract CeeZeeLottery is ReentrancyGuard, Ownable, ICeeZeeLottery {
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInSAFU;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 rolloverFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] SAFUPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInSAFU;
        uint32 finalNumber;
    }
     ...
     * @param _SAFUTokenAddress: address of the SAFU token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _SAFUTokenAddress, address _randomGeneratorAddress) {
        SAFU = IERC20(_SAFUTokenAddress);
     ...
     * @param _priceTicketInSAFU: price of a ticket in SAFU token
    ...
    function startLottery(
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInSAFU: _priceTicketInSAFU,
            ...
            amountCollectedInSAFU: pendingInjectionNextLottery,
            finalNumber: 0
        });
        ...
        nonReentrant
    {
        ...
        // Calculate number of SAFU to this contract
        uint256 amountSAFUToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].priceTicketInSAFU,
            _ticketNumbers.length
        );

        // Transfer SAFU tokens to this contract
        SAFU.safeTransferFrom(
            address(msg.sender),
            address(this),
            amountSAFUToTransfer
        );

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInSAFU += amountSAFUToTransfer;
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInSAFU) *
        ...
        // Calculate prizes in SAFU for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].SAFUPerBracket[j] =
                ...
                // C. No SAFU to distribute, they are added to the amount to withdraw to burn address
            } else {
                _lotteries[_lotteryId].SAFUPerBracket[j] = 0;
        ...
        pendingInjectionNextLottery += (_lotteries[_lotteryId]
            .amountCollectedInSAFU - amountToShareToWinners);
    ...
    function claimTickets(
        ...
        // Initializes the rewardInSAFUToTransfer
        uint256 rewardInSAFUToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInSAFUToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        SAFU.safeTransfer(msg.sender, rewardInSAFUToTransfer);

        emit TicketsClaim(
            msg.sender,
            rewardInSAFUToTransfer,
            _lotteryId,
            _ticketIds.length
        );
    }
        ...
        onlyOwnerOrInjector
    {
        ...
        SAFU.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInSAFU += _amount;
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount)
        ...
        onlyOwner
    {
        require(_tokenAddress != address(SAFU), "Cannot be SAFU token");
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].SAFUPerBracket[_bracket];
        } else {
            return 0;
        }
    }
