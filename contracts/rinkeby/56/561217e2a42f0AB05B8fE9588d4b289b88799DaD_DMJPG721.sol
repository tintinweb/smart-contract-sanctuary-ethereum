  ...
  ERC165, IERC721, IERC721Metadata, Ownable, OnChainMeta
{
  ...
  */
  ...
  /**
    ...
    @param _content The content of the token being minted.
  */
  function mint (
    uint256 _parent,
    address _recipient,
    ...
    uint256 amount = 1;
    if (nextId - 1 + amount > cap) { revert CapExceeded(); }

    // you can't reply unless you own the message you're replying to
    // if (!ownerOf(_parent) == msg.sender) { revert NotOwnerOfMessage(); }
    ...
    */
    ...
    unchecked {
      ...
      owners[nextId] = _recipient;
      parent[nextId] = _parent;
      children[_parent].push(nextId);
      content[nextId] = _content;
      emit Transfer(address(0), _recipient, nextId);
      nextId++;
    }
  }
...
contract OnChainMeta {
    ...
    string public metaDescription = 'This is a stupid jpeg, you are now -1 intelligence';
    ...
    function _buildMeta(
      ...
      uint256 r = uint256(keccak256(abi.encodePacked(_tokenId, _parent, address(this)))) % 256;
      uint256 g = uint256(keccak256(abi.encodePacked(_tokenId, _owner, address(this)))) % 256;
      uint256 b = uint256(keccak256(abi.encodePacked(_parent, _owner))) % 256;
      ...
      string memory metaJSON = string(abi.encodePacked(
        '{"name":"DM-JPG #',
           _tokenId.toString(),
          '",',
          '"description":"',
          ...
          'data:image/svg+xml;base64,',
            Base64.encode(bytes(_generateSVGImage(_tokenId, _parent, _owner, color, _content))),
          '", "attributes":[',
             _getMetadata(_tokenId, _parent, r, g, b),
          ']',
        '}')
      );

      string memory encodedMetaJSON = string(abi.encodePacked(
        'data:application/json;base64,',
        Base64.encode(bytes(metaJSON))
      ));

      return encodedMetaJSON;
    }
    ...
    function _getMetadata(
      ...
      uint256 totalC = _r + _g + _b;
      string memory metadata = string(abi.encodePacked(
        //_wrapTrait("Generation", groupId.toString()),',',
        _wrapTrait("Replying to", _parent.toString()),',',
        _wrapTrait("Identifier", _tokenId.toString()),',',
        _wrapTrait("_R Value", _r.toString()),',',
        _wrapTrait("_G Value", _g.toString()),',',
        _wrapTrait("_B Value", _b.toString()),',',
        _wrapTrait("_T Value", totalC.toString())
      ));
    ...
    function _generateSVGImage(uint256 _tokenId, uint256 _parent, address _owner, string memory _color, string memory _content) internal view returns (string memory svg) {
...
library NFTSVG {
  ...
  function _generateSVGPaths(SVGParams memory params) private view returns (string memory svgPaths) {
    ...
    //svgPaths = string(abi.encodePacked('<text x="42" y="90" style="width: 580px;height: 480px; border:none; font-family:monospace; fill:white; text-shadow:1px 1px 1px #000;font-size:1.6em;">Dear Ser, </text>'));
    svgPaths = string(abi.encodePacked(
      svgPaths,
      '<foreignObject width="500" height="480" x="40" y="90" ><textArea xmlns="http://www.w3.org/1999/xhtml" style="width: 500px;height: 480px; border:none; font-family:monospace; color:white; text-shadow:1px 1px 1px #000;font-size:1.6em; background:rgba(0,0,0,0)" >',
      string(abi.encodePacked(params.content)),
      '</textArea></foreignObject>'
    ));

    if(params.parent != 0){
      uint256 instance = uint256(uint160(address(this)));
      svgPaths = string(abi.encodePacked(
        svgPaths,
        '<a href="http://testnets.opensea.io/assets/',
        instance.toHexString(),
        '/',
        params.parent.toString(),
        '"><text x="40" y="50" style="font-family:monospace; fill:white; text-shadow:1px 1px 1px #000;font-size:1.6em;">>>',
        params.parent.toString(),
        '</text></a>'
      ));
    }
