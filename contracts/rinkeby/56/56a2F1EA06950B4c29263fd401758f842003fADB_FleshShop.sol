...
contract FleshShop {
  ...
  event OfferPurchased(uint256 offerId, address account, uint256 key, uint256 cost);
  ...
  function purchase(uint256 _id, uint256 _key, uint256 _cost, uint256 _spots, uint256 _expires, bytes memory _signature) external {
    if (!validate(_id, _cost, _spots, _expires, _signature)) revert InvalidPurchaseRequest();
   ...
   */
  function getParams(uint256 _id, uint256 _cost, uint256 _spots, uint256 _expires) public pure returns (bytes32) {
    return keccak256(abi.encodePacked(_id, _cost, _spots, _expires));
  }

  /**
   * Validate offer parameters.
   */
  function validate(uint256 _id, uint256 _cost, uint256 _spots, uint256 _expires, bytes memory _signature) private view returns (bool) {
    return ECDSA.recover(
      getParams(_id, _cost, _spots, _expires).toEthSignedMessageHash(),
      _signature
    ) == verifier;
  }
}
