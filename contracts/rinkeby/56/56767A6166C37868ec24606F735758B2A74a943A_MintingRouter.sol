...
contract MintingRouter is Ownable, ReentrancyGuard, EIP712Whitelisting {
    ...
    mapping(uint256 => uint256) private _numberTokensMintedPerRound;
    /// @notice The number of NFTs minted during a sale round per wallet.
    mapping(uint256 => mapping(address => uint256))
        private _mintedAmountPerAddressPerRound;
     ...
     * @param maxMintsRound The total amount of NFTs available for the current sale round.
     * @param maxMintsPerWallet The total number of NFTs that can be minted by a single wallet during the sale round.
     * @param maxMintsPerTx The maximum number of tokens available for minting per single transaction.
     */
    modifier validateSaleRoundParams(
        ...
        require(
            _totalTokensLeft() > 0 &&
            maxMintsRound <= _totalTokensLeft() &&
            maxMintsRound >= _numberTokensMintedPerRound[currentSaleIndex],
            "INVALID_TOTAL_AMOUNT"
        );

        if (maxMintsRound != UNLIMITED_AMOUNT) {
            require(maxMintsPerWallet <= maxMintsRound,"INVALID_LIMIT_PER_WALLET");
            require(maxMintsPerTx <= maxMintsRound, "INVALID_MAX_PER_MINT");
        }

        if (maxMintsPerWallet != UNLIMITED_AMOUNT) {
            require(maxMintsPerTx <= maxMintsPerWallet, "INVALID_MAX_PER_MINT");
        }
    ...
    function changeSaleRoundParams(
        ...
        currentSaleRound.maxMintsRound = maxMintsRound;
        currentSaleRound.maxMintsPerWallet = maxMintsPerWallet;
        currentSaleRound.maxMintsPerTx = maxMintsPerTx;
    }
     ...
     */
    function maxMintsAllowedPerAddress(address minter) public view returns (uint256) {
        ...
        if (currentSaleRound.maxMintsPerWallet != UNLIMITED_AMOUNT) {
            allowedWalletCount = currentSaleRound.maxMintsPerWallet - _mintedAmountPerAddressPerRound[currentSaleIndex][minter];
        }
        ...
        if (allowedAmountPerMint != UNLIMITED_AMOUNT) {
            allowedAmountPerMint = currentSaleRound.maxMintsPerTx;
        }
    ...
    function tokensLeft() public view returns (uint256) {
        ...
        if (currentSaleRound.maxMintsRound == UNLIMITED_AMOUNT) {
            return _totalTokensLeft();
        }

        return currentSaleRound.maxMintsRound - _numberTokensMintedPerRound[currentSaleIndex];
    }
    ...
    function _mint(
        ...
        require(currentSaleRound.enabled, "SALE_ROUND_DISABLED");

        if (currentSaleRound.maxMintsRound != UNLIMITED_AMOUNT) {
            // We have limited amount of tokens for this sale round.
            require(
                _numberTokensMintedPerRound[currentSaleIndex] + quantity <=
                    currentSaleRound.maxMintsRound,
        ...
        if (currentSaleRound.maxMintsPerWallet != UNLIMITED_AMOUNT) {
            ...
            uint256 mintedAmountSoFar = _mintedAmountPerAddressPerRound[
                currentSaleIndex
            ][recipient];
            require(
                mintedAmountSoFar + quantity <= currentSaleRound.maxMintsPerWallet,
                "ALLOWANCE_PER_WALLET_EXCEEDED"
            );
        }

        if (currentSaleRound.maxMintsPerTx != UNLIMITED_AMOUNT) {
            require(quantity <= currentSaleRound.maxMintsPerTx, "ALLOWANCE_PER_TXN_EXCEEDED");
        }
        ...
        _mintedAmountPerAddressPerRound[currentSaleIndex][recipient] += quantity;
        _numberTokensMintedPerRound[currentSaleIndex] += quantity;
