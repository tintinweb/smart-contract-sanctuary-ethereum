...
contract Lentil is Ownable, ERC721A, ERC721ABurnable, MerkleWhitelist, ReentrancyGuard{
    ...
    uint public MAX_BATCH_SIZE = 1;
    mapping(address => bool) public userHasMinted;
    uint[10000] bids;


    constructor() ERC721A("Lentil", "LENTIL") {}
        ...
        onlyPublicWhitelist(proof)
    {
        ...
        require(totalSupply() + 1 <= COLLECTION_SIZE, "Max Collection Size reached!");
        require(msg.value >= 0, "Must send enough eth for WL Mint");
        userHasMinted[msg.sender] = true;
        bids[totalSupply()] = msg.value;
        ...
        callerIsUser
    {
        ...
        require(!userHasMinted[msg.sender], "Can only mint once!");   
        require(totalSupply() + 1 <= COLLECTION_SIZE, "Max Collection Size reached!");
        require(msg.value >= 0, "Must send enough eth for public mint");
        userHasMinted[msg.sender] = true;
        bids[totalSupply()] = msg.value;        
        _safeMint(msg.sender, 1);
    }

    function increaseBid(uint _tokenId) 
        external
        payable
        callerIsUser
    {
        bids[_tokenId] += msg.value;
    }

    function getAllBids() public view returns (uint[10000] memory){
        return bids;
    }

    function getBid(uint _tokenId) public view returns (uint){
