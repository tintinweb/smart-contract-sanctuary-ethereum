...
contract Strategy is StratManager, FeeManager, GasThrottler {
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ...
        if (outputBal > 0) {
            ...
            emit StratHarvest(_msgSender(), wantHarvested, balanceOf());
        }
    }

    // performance fees
    function chargeFees(address callFeeRecipient) internal {
        uint256 callFeeAmount = (((IERC20(output).balanceOf(address(this)) *
            45) / 1000) * callFee) / MAX_FEE;
...
abstract contract FeeManager is StratManager {
    ...
    uint256 public diaryFee = MAX_FEE - STRATEGIST_FEE - callFee;

    function setCallFee(uint256 _fee) public onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");

        callFee = _fee;
        diaryFee = MAX_FEE - STRATEGIST_FEE - callFee;
    }
...
contract GasThrottler {
    ...
    address public gasprice =
        address(0xA43509661141F254F54D9A326E8Ec851A0b95307); //change to actual
...
contract StratManager is Ownable, Pausable {
    ...
    address public diaryFeeRecipient;
     ...
     * @param _diaryFeeRecipient address where to send Diary's fees.
     */
    constructor(
        address _keeper,
        address _strategist,
        ...
        vault = _vault;
        diaryFeeRecipient = _diaryFeeRecipient;
    }

    // checks that caller is either owner or keeper.
    modifier onlyManager() {
        require(_msgSender() == owner() || _msgSender() == keeper, "!manager");
        _;
    }
    ...
    function setStrategist(address _strategist) external {
        require(_msgSender() == strategist, "!strategist");
        strategist = _strategist;
    }
    ...
    /**
     * @dev Updates diary fee recipient.
     * @param _diaryFeeRecipient new diary fee recipient address.
     */
    function setDiaryFeeRecipient(address _diaryFeeRecipient)
        external
        onlyOwner
    {
        diaryFeeRecipient = _diaryFeeRecipient;
