    ...
    AccessControlEnumerable
{
    ...
    revved.
    @dev Revving is tied to a specific Moonbird, not to the owner, so it doesn't
    reset upon sale.
    @return revving Whether the Moonbird is currently revving. MAY be true with
    ...
    @return total Total period of time for which the Moonbird has revved across
    ...
    /**
    @dev Emitted when a Moonbird begins revving.
     */
    event Revved(uint256 indexed tokenId);

    /**
    @dev Emitted when a Moonbird stops revving; either through standard means or
    by expulsion.
     */
    event Unrevved(uint256 indexed tokenId);

    /**
    @dev Emitted when a Moonbird is expelled from the revving.
     */
    ...
    /**
    @notice Changes the Moonbird's revving status.
    */
