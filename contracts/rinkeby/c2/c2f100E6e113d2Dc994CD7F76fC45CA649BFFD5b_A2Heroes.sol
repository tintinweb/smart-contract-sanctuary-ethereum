    ...
    Ownable
{
    ...
    constructor() ERC721("A2Heroes", "A2H") {
        ...
        _setupRole(
            DEFAULT_ADMIN_ROLE,
            0x2dC900A489DA7d5Eb5E1eAc7F6B9f9246f4dD16d
        );
    ...
    uint64 public redeemStartDate;
    uint64 public redeemStopDate;
    ...
    function transferFrom(
        ...
        uint256 royaltyAmount = tokenPrice[tokenId].mul(royaltyRate).div(100);
        (bool sent, bytes memory data) = royaltyAddress.call{
            value: royaltyAmount
        }("");
        require(sent, "Failed to send royalty.");
        emit RoyaltySent(data);
    }
    ...
    function redeem(uint256 _id) public {
        ...
        require(
            currentTimeStamp > redeemStartDate,
            "Today is not for Redemption"
        );
        require(
            currentTimeStamp < redeemStopDate,
            "Today is not for Redemption"
        );
        require(_exists(_id), "No such token");
        burn(_id); //requires id is owned or allowed by sender.
        delete tokenNames[_id];
        delete character_minted[_id];
    ...
    function setRoyaltyAmount(uint16 royalty, address royaltyaddress) public {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "Caller has not the role of admin"
        );
    ...
    function setRedeemDate(uint64 timestamp_start, uint64 timestamp_stop)
        public
    {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
        ...
        redeemStartDate = timestamp_start;
        redeemStopDate = timestamp_stop;
    }
