 ...
 *
 */
contract SamoraiMaskERC721A is ERC721A, Ownable {
    using Strings for uint256;

    uint256 private constant MAX_SUPPLY = 1500;
    uint256 private constant MAX_WHITELIST = 500;
    uint256 private constant MAX_PUBLIC = 1000;
    Steps public sellingStep;
    uint256 public preSalePrice = 0.025 ether;
    uint256 public publicSalePrice = 0.03 ether;
    bytes32 public merkleRoot;
    uint256 public maxMintAllowed = 3;
    uint256 public saleStartTime = 1640991600;
    ...
    constructor(
        ...
        bytes32 _merkleRoot
    ) ERC721A("Samorai Mask NFT", "SMRIM") {
    ...
    function whitelistMint(
        ...
        require(sellingStep == Steps.Presale, "We are not in the Presale step");

        require(isWhiteListed(msg.sender, _proof), "Not whitelisted");
        require(
            nftsPerWallet[msg.sender] + _quantity <= 1,
            "You can only get 1 NFT in the Presale"
        );
        require(
        ...
        callerIsUser
    {
        ...
        require(
            totalSupply() + _quantity <= MAX_WHITELIST + MAX_PUBLIC,
            "Max supply exceeded"
        );
        require(msg.value >= price * _quantity, "Not enough funds");
        require(
            _quantity <= maxMintAllowed,
        ...
        returns (string memory)
    {
        ...
        return string(abi.encodePacked(super.tokenURI(_nftId), ".json"));
    }
    ...
    function setMaxMintAllowed(uint256 _maxMintAllowed) external onlyOwner {
