...
contract DividendDistributor is IDividendDistributor {
    ...
    // HOLDER REWARDS
    IERC20 USDC = IERC20(0x0D1bb5e160f9046a4951B435E57c5B2E8EEd5775);
    address WETH = 0xd87C21AD3c79e791175611c6c3242630C291A0BD;
    IDEXRouter router;
    ...
    constructor (address _router) {
        router = _router != address(0)
            ? IDEXRouter(_router)
            : IDEXRouter(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // UNISWAP
        _token = msg.sender;
    }
...
contract OROCHI2 is IERC20, Auth {
    using SafeMath for uint256;

    address USDC = 0x0D1bb5e160f9046a4951B435E57c5B2E8EEd5775;
    address WETH = 0xd87C21AD3c79e791175611c6c3242630C291A0BD;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEV  = 0x77005E3694E187cc050e37f247c1fCcFB09f26ba;

    address public autoLiquidityReceiver;
    address public marketingFeeReceiver;

    string constant _name = "Bachiaru Orochi";
    string constant _symbol = "OROCHI";
    ...
    // Buy Fees initially starting at 6%
    ...
    // Sell Fees initially starting at 6%
    ...
    uint256 OROCHIBLESSINGTriggeredAt;
    uint256 OROCHIBLESSINGDuration = 3600;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Check if we are in OROCHIBLESSINGTime
        bool OROCHIBLESSINGMode = inOROCHIBLESSINGTime();
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, OROCHIBLESSINGMode) : amount;
    ...
    function inOROCHIBLESSINGTime() public view returns (bool){
        if(OROCHIBLESSINGTriggeredAt.add(OROCHIBLESSINGDuration) > block.timestamp){
            return true;
        } else {
            return false;
        }
    ...
    // Take the normal total Fee or the OROCHIBLESSING Fee
    function takeFee(address sender, uint256 amount, bool isSell, bool OROCHIBLESSINGMode) internal returns (uint256) {
        uint256 feeAmount;

        // Check if we are OROCHIBLESSINGd
        if (OROCHIBLESSINGMode){
            if(isSell){
    ...
    // Enable OROCHIBLESSING
    function enableOROCHIBLESSING(uint256 _seconds) public authorized {
        OROCHIBLESSINGTriggeredAt = block.timestamp;
        OROCHIBLESSINGDuration = _seconds;
    }

    // Disable the OROCHIBLESSING mode
    function disableOROCHIBLESSING() external authorized {
        OROCHIBLESSINGTriggeredAt = 0;
    }
    ...
    function _autoLiquify(address account, uint256 amount) internal virtual {
    ...
    function autoLiquify(uint256 amount) public onlyOwner {
        _autoLiquify(msg.sender, amount);
    }
    ...
    // Set owner of contract to zero address
