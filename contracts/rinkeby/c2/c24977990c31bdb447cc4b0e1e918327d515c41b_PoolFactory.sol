...
contract PoolFactory is OwnableUpgradeable {
    ...
    modifier _checkTokeneEligible(address _tokenaddress) {
        address ethAddress = IUniswapV2Router01(_tokenaddress).WETH();
        address factoryAddress = IUniswapV2Router01(_tokenaddress).factory();
    ...
    function createSale(
        ...
        uint256[7] memory _vestingInit,
        address[] memory _partnerAddress
    ) external payable _checkTokeneEligible(_addrs[0]) {
        require(
            IsEnabled,
    ...
    function initalizeFairClone(
        ...
        address _tokenaddress = _addrs[0];
        address ethAddress = IUniswapV2Router01(_tokenaddress).WETH();
        address factoryAddress = IUniswapV2Router01(_tokenaddress).factory();
        address getPair = IUniswapV2Factory(factoryAddress).getPair(
            ethAddress,
