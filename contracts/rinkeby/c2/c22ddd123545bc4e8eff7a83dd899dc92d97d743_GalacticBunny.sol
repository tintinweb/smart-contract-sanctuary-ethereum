...
contract GalacticBunny is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 markBNB = newBalance.mul(marketingTokens).div(tokenSwapedBase);
            uint256 devBNB = newBalance.mul(devTokens).div(tokenSwapedBase);

            if (markBNB > 0) {
                payable(marketingWallet).transfer(markBNB);
            }
            ...
            uint256 liqBNB = newBalance.mul(liquidityTokens).div(tokenSwapedBase);

            addLiquidity(tokensLeft, liqBNB);

            emit SwapAndLiquify(liquidityTokens, liqBNB, tokensLeft);
