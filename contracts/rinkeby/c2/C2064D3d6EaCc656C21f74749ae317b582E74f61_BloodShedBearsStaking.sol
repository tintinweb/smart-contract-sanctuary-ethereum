...
contract BloodShedBearsStaking is Ownable, Pausable {
    ...
    function _exitStakeStanceMany(
        ...
        for (uint256 i = 0; i < tokenIds_.length; ++i) {
            require(_isOwner(msg.sender, collection_, tokenIds_[i]));
    ...
    function claimYieldForAll() external whenNotPaused {
        ...
        for (uint256 i = 0; i < collections.length; ++i) {
            if (stakedTokensByWallet[msg.sender][collections[i]].length > 0) {
                uint256[] memory levelBoosts = IGameStats(contractsAddressesMap["gameStats"])
                    .getLevelBoosts(collections[i], stakedTokensByWallet[msg.sender][collections[i]]);
