...
contract poopie is ERC721A, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "Stinky - The Contract is Paused");
        require(
            tokenCount + _amount <= maxSupply,
            "Stinky - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "Stinky - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "Stinky - max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "Stinky- insufficient ethers"
            );
        }
