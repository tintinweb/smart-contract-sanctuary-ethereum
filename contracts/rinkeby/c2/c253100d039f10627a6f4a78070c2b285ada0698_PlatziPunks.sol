...
contract PlatziPunksDNA {
  ...
  function deterministicPseudoDNA(uint256 _tokenId, address _minter) public pure returns (uint256) {
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
  ...
  function _paramsURI(uint256 _dna) internal view returns(string memory) {
    ...
    // Cuando excedemos el número de variables se coloca entre llaves y se concatena luego
    {
  ...
  function tokenURI(uint256 tokenId) public view override returns(string memory) {
    ...
    string memory jsonURI = Base64.encode(
      abi.encodePacked(
        '{ "name": "PlatziPunk #',
        tokenId.toString(),
        '", "description": "My first NFTs created uing hardhat and solidity", "image": "',
        image,
        '"}'
      )
    );
