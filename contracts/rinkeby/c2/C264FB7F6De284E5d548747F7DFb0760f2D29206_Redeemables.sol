...
contract Redeemables is ERC1155, Ownable {
    ...
    function addRedeemableItemForProject(
        ...
        for (uint256 index = 0; index < itemIDs.length; index++) {
            ...
            ValidContractsItemsIn storage contractItems = projects[
                projectAddress
            ];
            ...
            if (toPush.length == 0) {
                ...
                nextItemID = toPush[toPush.length];
            }
            toPush[index] = nextItemID;

            // uint256 newItemID = lastItemID + 1;

            contractItems.validItemTypesIDs.push(nextItemID);
