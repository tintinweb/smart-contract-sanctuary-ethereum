...
import "./TheAnimalKindomEngineers.sol";
...
contract NFTSale is ReentrancyGuard, Ownable {
    ...
    modifier bidExist(uint256 auctionId){
        BidDetail memory bidDetail = currentBid[auctionId];
        require(bidDetail.currentPrice != 0);
        require(bidDetail.currentBidder != address(0));
        require(bidDetail.startTime <= block.timestamp);
        require(bidDetail.endTime >= block.timestamp);
        _;
    }
    //Bid section
    struct BidDetail{
    ...
    event BidUpdated(uint256 auctionId,uint256 newPrice);
    function setInitialBid(uint256 auctionId,uint256 _initialPrice) external onlyOwner preventContractCalls{
        currentBid[auctionId].currentBidder = msg.sender;
        currentBid[auctionId].currentPrice = _initialPrice;
        emit BidUpdated(auctionId, _initialPrice);
    }

    function bid(uint256 auctionId) external payable preventContractCalls bidExist(auctionId){
        require(msg.value > currentBid[auctionId].currentPrice,"You must bid higher");
        bidTotalAmount[msg.sender] += msg.value;
        currentBid[auctionId].currentPrice = msg.value;
        currentBid[auctionId].currentBidder = msg.sender;
        emit BidUpdated(auctionId, msg.value);
    }

    function claimNFTFromAuction(uint256 auctionId) external preventContractCalls bidExist(auctionId){
        require(block.timestamp >= currentBid[auctionId].endTime,"The auction is not ended yet");
        require(currentBid[auctionId].currentBidder == msg.sender ,"You are not the highest bidder");
        TheAnimalKindomEngineers(nftContract).mint(msg.sender,1);
    }

    function withdrawUnsuccessfulBids(uint256 auctionId) external preventContractCalls bidExist(auctionId){
        require(bidTotalAmount[msg.sender] > 0);
        uint256 withdrawableAmount = bidTotalAmount[msg.sender];
        bidTotalAmount[msg.sender] = 0;
        ...
        roundAvailable(_roundId)
    {
        ...
        TheAnimalKindomEngineers(nftContract).mint(msg.sender,_mintAmount);
