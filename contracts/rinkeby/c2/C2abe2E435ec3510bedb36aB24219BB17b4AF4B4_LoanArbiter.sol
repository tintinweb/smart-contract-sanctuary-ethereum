...
contract LoanArbiter is Ownable {
    ...
    constructor(
        address _loanParameters,
        address _loanProvider,
        address _loanBorrower,
        address payable _loanTreasury,
        address _loanOrchestrator
    ) {
        loanParameters = LoanParameters(_loanParameters);
        loanProvider = LoanProvider(_loanProvider);
        loanBorrower = LoanBorrower(_loanBorrower);
        loanTreasury = LoanTreasury(_loanTreasury);
        loanOrchestrator = LoanOrchestrator(_loanOrchestrator);
    }
    ...
    function createLoanCampaign(
        ...
        // reference TFL-0016
        /////////////////////////////////////////
        // TODO temporarily commented out so to create a test loan
...
//        loanTreasury.transferFunds(msg.sender, address(loanTreasury), amount);
    ...
    /// @notice _loanBids should be passed in sorted by least interest and earliest date
    /// @notice so depending on the application
    /// @dev sorting is expensive to do in the contract so depending on front end
    /// @param _loanId The ID of the loan
    /// @param _loanBids The loan bids that the borrower is applying against their loan
    function activateLoan(uint256 _loanId, uint256[] memory _loanBids) public payable {
        loanOrchestrator.activateLoan(msg.sender, _loanId, _loanBids);
    }

    /// @notice Convenience method for getting loan ids for a borrower
    /// @param _borrowerAddress The address of the loan borrower
    /// @return a list of of loan IDs
    function loanIdsByAddress(address _borrowerAddress) public view returns (uint256[] memory)
    {
        return loanBorrower.loanIdsByAddress(_borrowerAddress);
    }

    /// @notice get the contract address for the loan parameters
    /// @return address of the loan parameters contract
    function getLoanParametersContractAddress() public view returns (address) {
        return address(loanParameters);
    }

    function getLoanOrchestratorContractAddress() public view returns (address) {
        return address(loanOrchestrator);
    }
    ...
    /// @notice Get the contract address of the loanTreasury
    function getLoanTreasuryAddress() public view returns (address) {
        return address(loanTreasury);
    }

    /// @notice Create a loan bid
    /// @param _loanId The ID of the loan
    /// @param _offered The amount being offered
    /// @param _apr The offered APR of this loan bid
    ...
    function makePayment(uint256 _loanId, address payable _loanTreasuryAddress) public payable {
        require(_loanTreasuryAddress == address(loanTreasury),
        ...
        // TODO rmageden need to divvy out payments to providers
    }

    /// @notice pass through to LoanBorrower.loanIdTracker
    /// @return loan id tracker
    function loanIdTracker() public view returns (uint256) {
        return loanBorrower.loanIdTracker();
    }

    /// @notice pass through to LoanProvider.loanBidIdTracker
    /// @return loan bid id tracker
    function loanBidIdTracker() public view returns (uint256) {
        return loanProvider.loanBidIdTracker();
    }

    /// @param _loanId The Loan ID
    /// @return The loan object
    function loans(uint256 _loanId) public view returns (LoanLibrary.Loan memory) {
        return loanBorrower.loans(_loanId);
    }

    /// notice pass through to LoanBorrower.loanList()
    /// @param _start the start index
    /// @param _end the end index
    /// @return tuple ids, loans
    function loanList(uint256 _start, uint256 _end)
    public view returns (uint256[] memory, LoanLibrary.Loan[] memory) {
        return loanBorrower.loanList(_start, _end);
    }

    /// @param _loanId The Loan ID
    /// @return The list of Loan Bid IDs
    function loanBidsOfLoan(uint256 _loanId) public view returns (LoanLibrary.LoanBid[] memory) {
        return loanProvider.loanBidsOfLoan(_loanId);
    }

    /// @notice get loan bid by loan bid ID
    /// @return _loanBidId The loan bid ID
    function loanBids(uint256 _loanBidId) public view returns (LoanLibrary.LoanBid memory) {
        return loanProvider.loanBids(_loanBidId);
    }

    /// @notice Get the remaining principle on a loan
    /// @param _loanId The ID of the loan
    function remainingPrinciple(uint256 _loanId) public view returns (uint256) {
        return loanBorrower.remainingPrinciple(_loanId);
    }

    /// @notice Get payoff amount
    function payoffAmount(uint256 _loanId) public view returns (uint256) {
        return loanBorrower.calculatePayoffAmount(_loanId);
    }

    /// @notice Calculate the loan interest to date
    /// @param _loanId The ID of the loan
    function loanInterestToDate(uint256 _loanId) public view returns (uint256){
        return loanBorrower.calculateActualInterestToDate(_loanId);
    }

    /// @notice Calculate the loan interest to collection - pass through to LoanLibrary
    /// @param _startDate The timestamp of the start of the loan
    /// @param _amount The loan amount
    /// @param _apr The annual percentage rate of the loan
    function interestToDate(
        uint256 _startDate,
        uint256 _amount,
        uint256 _apr
    ) public view returns (uint256) {
        return LoanLibrary.calculateInterestRevenueForDays(
            _amount, _apr, loanParameters.precision(),
            LoanLibrary.calculateDurationInDays(_startDate, block.timestamp));
    }
    ...
    /// @notice number of days left on a loan
    /// @param _loanId The Loan ID
    /// @return The number of days left on a loan
    function numberOfDayLeftOnLoan(uint256 _loanId) public view returns(uint256) {
        return loanBorrower.numberOfDaysLeftOnLoan(_loanId);
    }
    ...
    function setMinDurationBeyondSellableDateInMinutes(uint256 _value) public onlyOwner {
        loanParameters.setMinDurationBeyondSellableDateInMinutes(_value);
    }

    function setMinAuctionPercentOfLoanAmount(uint256 _value) public onlyOwner {
        loanParameters.setMinAuctionPercentOfLoanAmount(_value);
    }

    function setMaxAuctionDurationInMinutes(uint256 _value) public onlyOwner {
        loanParameters.setMaxAuctionDurationInMinutes(_value);
    }

    function setProtocolAuctionFeePercentage(uint256 _value) public onlyOwner {
        loanParameters.setProtocolAuctionFeePercentage(_value);
    }
    ...
    function loanLibraryAddress() public view returns(address) {
        return LoanLibrary.libraryAddress();
    }

    function loanProviderAddress() public view returns(address) {
        return address(loanProvider);
    }

    function loanBorrowerAddress() public view returns(address) {
        return address(loanBorrower);
    }
}
