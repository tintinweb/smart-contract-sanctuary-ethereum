...
contract Fetih is ERC721, ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => uint256) _barrack;
    ...
    function battleResult(uint256 attackerTokenId, uint256 defenderTokenId, bool isSucceed) public onlyOracleClient whenWarContinues {
        ...
        if (isSucceed || defenderSoldiers == getMinimumDefenderSoldier()) {
            //if wins
            emit WonBattle(attackingEmperor, defenderTokenId);
            ...
            //if loses
            emit LostBattle(attackingEmperor, attackerTokenId, defenderTokenId);
    ...
    function produceSoldiers(uint256 tokenId) external whenWarContinues {
        ...
        _barrack[tokenId] = block.timestamp + 300;
    }

    function isTheBarrackBusy(uint256 tokenId) public view returns (bool) {
        if (_barrack[tokenId] == 0) return false;

        return true;
    }

    function isSoldiersClaimable(uint256 tokenId) external view returns(bool) {
        uint256 claimTime = _barrack[tokenId];
        if (claimTime != 0 && block.timestamp >= claimTime) {
            return true;
        }
    ...
    function claimSoldiers(uint256 tokenId) external whenWarContinues {
        ...
        require(block.timestamp >= _barrack[tokenId], "Soldiers are not ready!");
