...
contract UnitrollerV2 is Unitroller, Whitelist {
    function mintAllowed(address cToken, address minter, uint mintAmount) external returns (uint) {
        require(members[msg.sender], "Sender must be in whitelist");
        _fallback();
    }
    ...
    function repayBorrowAllowed(address cToken, address redeemer, uint redeemTokens) external returns (uint) {
        require(members[msg.sender], "Sender must be in whitelist");
        _fallback();
    }
...
contract Whitelist is ComptrollerErrorReporter {
