...
contract TheVault{
    ...
    // map index of array
    mapping(uint=>uint) indexOfVault;
    ...
    function removeAssetFromArray(uint _assetToDelete) internal {
    uint index = indexOfVault[_assetToDelete];
    if (mainVault.length > 1) {
        mainVault[index] = mainVault[mainVault.length-1];
    }
    ...
    function withdraw() public  {
       ...
       // goes to users wallet
        nftContract.transferFrom(address(this), msg.sender, randomNft.tokenId);
