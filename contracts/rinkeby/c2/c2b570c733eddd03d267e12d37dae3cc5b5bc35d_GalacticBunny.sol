...
contract GalacticBunny is Context, IERC20, Ownable {
    ...
    address public devWallet = 0x51b41c4b63191B7638eAd1f43F29C088B374d942;
    address public marketingWallet = 0xB7bFc608Be9356A049440a0E329681b90B76fF1c;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 devTokens = contractTokenBalance.mul(_devFee).div(totalTaxForSwap);
            uint256 liquidityTokens = contractTokenBalance.mul(_liquidityFee.div(2)).div(totalTaxForSwap);
            
            uint256 tokensToSwap = contractTokenBalance.sub(liquidityTokens).sub(devTokens).sub(marketingTokens);
            uint256 tokensLeft = contractTokenBalance.sub(tokensToSwap);

            uint256 initialBalance = address(this).balance;
            swapTokensForEth(tokensToSwap);
            uint256 newBalance = address(this).balance.sub(initialBalance);

            uint256 tokenSwapedBase = marketingTokens.add(devTokens).add(liquidityTokens);
            
            uint256 markEth = newBalance.mul(marketingTokens).div(tokenSwapedBase);
            uint256 devEth = newBalance.mul(devTokens).div(tokenSwapedBase);

            if (markEth > 0) {
                payable(marketingWallet).transfer(markEth);
            }
            
            if (devEth > 0) {
                payable(devWallet).transfer(devEth);
            }
            
            uint256 liqEth = newBalance.mul(liquidityTokens).div(tokenSwapedBase);

            addLiquidity(tokensLeft, liqEth);

            emit SwapAndLiquify(liquidityTokens, liqEth, tokensLeft);
