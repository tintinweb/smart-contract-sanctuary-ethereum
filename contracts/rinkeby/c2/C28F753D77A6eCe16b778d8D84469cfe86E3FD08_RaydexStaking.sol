...
contract RaydexStaking is Ownable, ReentrancyGuard {
    ...
    uint256 public overSaturation = 1e6 * 1e18;

    uint256 public maxFee = 3; // 3%
    ...
    function setOverSaturation(uint256 amount) external onlyOwner {
        overSaturation = amount;
    }

    function getCurrentFee() public view returns (uint256 rate) {
        rate = poolInfo.deposit / overSaturation;
        if (rate > maxFee) {
            rate = maxFee;
        }
    }

    function getFee(uint256 amount) public view returns (uint256 fee) {
        fee = (amount * getCurrentFee()) / 100;
    }
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        if (amount > 0) {
            ...
            RDX.transferFrom(msg.sender, address(this), amount - fee);
            RDX.transferFrom(msg.sender, treasury, fee);
            user.amount += (amount - fee);
            poolInfo.deposit += (amount - fee);
        }
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        RDX.transfer(msg.sender, amount);
