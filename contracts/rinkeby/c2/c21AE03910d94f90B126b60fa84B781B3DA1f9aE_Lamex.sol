 ...
 * {ERC721Enumerable}.
 */
contract Lamex is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable  {
  ...
  mapping(uint256 => string) private _baseUriByTier;
  mapping(address => uint256) private _depositedLamex;
  ...
  function initialize(address _loomi, address _staking) external initializer {
    loomi = ILoomi(_loomi);
    ...
    // 1 lamex = 1000 loomi
    loomiToLamexRate = 1000;

    __ERC721_init("Lamex", "LMX");
    __Ownable_init();
  ...
  function setBaseURI(uint256 tier, string memory newBaseURI) public onlyOwner {
    _baseUriByTier[tier] = newBaseURI;
  }
  ...
  function _topUpLamex(address _user, uint256 _amount) internal {
    require(_amount > 1000 ether, "Min deposit 1 loomi");

    loomi.spendLoomi(_user, _amount);

    uint256 lamexFromLoomi = _amount / loomiToLamexRate * 1 ether;
    
    _depositedLamex[_user] += lamexFromLoomi;
    _nonces[_user]++;

    emit LamexTopUp(_user, _nonces[_user], lamexFromLoomi);
  }

  function _baseUriByCreepzStaked(uint256 numberOfCreepz, bool hasUserSwitched) internal view returns (string memory) {
    uint256 currentTier;
    if (!hasUserSwitched) return _baseUriByTier[currentTier];

    if (numberOfCreepz > 0 && numberOfCreepz < 10) currentTier = 1;
    if (numberOfCreepz >= 10 && numberOfCreepz < 25) currentTier = 2;
    if (numberOfCreepz >= 25) currentTier = 3;

    return _baseUriByTier[currentTier];
  }
