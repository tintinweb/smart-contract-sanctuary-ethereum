...
contract CONTRewardManagement is Initializable {
    ...
    uint256 private constant ADDITION_TIME_FOR_OLD = 1 days;
    uint256 private constant ADDITION_TIME_FOR_NEW = 2 days;
    uint256 public constant ONE_MONTH = 1 days;
    uint256 public constant TWO_MONTH = 2 days;
    uint256 public constant THREE_MONTH = 3 days;
    uint256 public constant FOUR_MONTH = 4 days;
    // uint256
    ...
    */
    ...
    function extendContract(uint256[] memory time, uint256[] memory indexes) external {
        ...
        // require(time == ONE_MONTH || time == TWO_MONTH || time == THREE_MONTH, "MONTH_FEE: Not valid time");
    ...
    */
    ...
    function getExtendContractFee(address account, uint256[] memory time, uint256[] memory indexes) public view returns (uint256) {
        require(time.length == indexes.length, "INPUT: INVALID");
        uint256 totalFee;
        uint256[] memory numberOfContType = new uint256[](4);
        numberOfContType[(uint256)(ContType.Tesseract)] = getNumberOfConts(account, ContType.Tesseract);
        numberOfContType[(uint256)(ContType.Cube)] = getNumberOfConts(account, ContType.Cube);
        ...
        for (uint256 i = 0; i < indexes.length; ++i) {
            ...
            uint256 feeForContOneMonth = feeInMonth[cont.cType] - feeInMonth[cont.cType] * totalDecreasePercent / HUNDRED_PERCENT;
            totalFee += (feeForContOneMonth * _time) / ONE_MONTH;
        }
        return totalFee;
    }
