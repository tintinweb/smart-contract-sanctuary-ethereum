...
contract NFTingMarketplace is NFTingAuction {
    struct Listing {
        address nftAddress;
        uint256 tokenId;
        uint256 amount;
    ...
    function listOnSale(
        ...
        isApprovedMarketplace(_nftAddress, _tokenId)
    {
...
contract NFTingAuction is Ownable, NFTingBase {
    ...
    event AuctionCreated(bytes32 _auctionId);
    ...
    function _finishAuction(bytes32 _auctionId)
        ...
        isAuctionCreatorOrOwner(_auctionId)
    {
        Auction storage auction = auctions[_auctionId];
