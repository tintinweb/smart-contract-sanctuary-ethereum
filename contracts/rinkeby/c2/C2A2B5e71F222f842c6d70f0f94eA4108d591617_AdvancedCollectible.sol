...
contract AdvancedCollectible is ERC721URIStorage, VRFConsumerBase {
    ...
    // SVG parameter, immutable is read only and can be assigned by construtor only -> save Gas
    // Component: paths (like layer), PathCommands: types of line and numbers of line in each layers, size of SVG image and color
    uint256 private immutable maxNumberOfPaths; // SVG paths
    uint256 private immutable maxNumberOfPathCommands; // SVG parameter of each path
    uint256 private immutable maxSize;
    string[] private pathCommands;
    string[] private colors;

    // RequestID is returned by  after being called "requestRandomness" function
    // As _safeMint (ERC721) will mint tokenID to Sender
    // We need mapping between (RequestID to tokenID) and (RequestID to Sender) to fulfil the map
    ...
    event CreateRandomNumbervsTokenID(
        uint256 indexed tokenID,
    ...
    function createCollectible() public returns (bytes32 requestId) {
        ...
        uint256 newTokenId = tokenCounter; // Gas Saving
        requestIdToTokenId[requestId] = newTokenId;
        _safeMint(requestIdToSender[requestId], newTokenId);
        emit requestedCollectible(requestId, newTokenId);
    }
        ...
        override
    {
        ...
        tokenIdToRandomNumber[newTokenId] = randomNumber;
        emit CreateRandomNumbervsTokenID(newTokenId, randomNumber);
    }

    function setTokenURI(uint256 tokenId) public {
        // Check tokenID is minted & what is its corresponding randomNumber
        // Generate SVG code according to the randomNumber
        // Generate ImageURI
        // Generate TokenURI(metadata) containing ImageURI
        // _setTokenURI to map "tokenId" and "TokenURI"
        require(
        ...
        string memory Token_URI = formatTokenURI(imageURI);
        _setTokenURI(tokenId, Token_URI);
        emit CreateRandomSVG(tokenId, Token_URI);
    }
        ...
        returns (string memory finalSvg)
    {
        ...
        for (uint256 i = 0; i < numberOfPaths; i++) {
            ...
            string memory pathSvg = generatePath(newRandomNumber);
        ...
        returns (string memory pathSvg)
    {
        ...
        for (uint256 i = 0; i < numberOfPathCommands; i++) {
            string memory pathCommand = generatePathCommand(
                uint256(keccak256(abi.encode(randomNumber, maxSize + i)))
            );
        ...
        returns (string memory pathCommand)
    {
        ...
        uint256 parameterOne = uint256(
            keccak256(abi.encode(randomNumber, maxSize * 2))
        ) % maxSize;
        uint256 parameterTwo = uint256(
            keccak256(abi.encode(randomNumber, maxSize * 2 + 1))
        ) % maxSize;
        pathCommand = string(
            abi.encodePacked(
                pathCommand,
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    "data:application/json;base64,", // this is needed to
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"',
