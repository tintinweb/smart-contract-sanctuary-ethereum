...
interface IApexWolves {
    function getWolfOwnerTokenRate(address wolfOwner) external view returns(uint256);
}

contract MeatToken is ERC20, Ownable {

    IApexWolves public ApexWolves;
    ...
    function setWolfAddress(address _wolfAddress) public onlyOwner {
        ApexWolves = IApexWolves(_wolfAddress);
    }

    function updateReward(address from, address to) external {
        require(msg.sender == address(ApexWolves));
        if(from != address(0)){
    ...
    function burn(address user, uint256 amount) external {
        require(allowedAddresses[msg.sender] || msg.sender == address(ApexWolves), "Address does not have permission to burn");
        _burn(user, amount);
    }
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return ApexWolves.getWolfOwnerTokenRate(user) * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400;
    }
