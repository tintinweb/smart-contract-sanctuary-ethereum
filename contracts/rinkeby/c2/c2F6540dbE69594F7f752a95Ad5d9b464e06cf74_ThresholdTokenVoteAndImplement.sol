...
contract ThresholdTokenVoteAndImplement is VoteAndImplementContract {
    ...
    uint256 public shouldBeThree;
    uint256 public shouldBeThreeToo;
    constructor(){
        shouldBeThree = uint256(uint8(VotingStatus.active));
        shouldBeThreeToo = uint256(VotingStatus.active);
    }
...
abstract contract VoteContractPrimitive is IERC165, RegisterVoteContract, VotingStatusHandling, IVoteContract {
    ...
    modifier activateNewVote {
        ...
        _;
        votingStatus[msg.sender][getCurrentVoteIndex(msg.sender)] = 3; //uint256(uint8(VotingStatus.active));
    }
