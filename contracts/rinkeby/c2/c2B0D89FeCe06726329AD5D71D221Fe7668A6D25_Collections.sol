...
/**
 * @title Collections Contract minimized
 * @notice This is used to create a collection with ERC721 contract as child
 */

contract Collections is ICollection, Ownable {
    /* *******
     * Globals
     * *******
     */
    ...
    /**
     * @notice Get Specific Collection Data with new media address.
     */
    function getCollectionMediaData(address collectionAddress)
        external
        view
        override
        returns (CollectionData memory)
    {
        CollectionData memory collectionData = collections[collectionAddress];
     ...
     */
    function setMarketContract(address _marketContractAddress)
        external
        onlyOwner
    {
        require(_marketContractAddress != address(0), "invalid address");
        marketContractAddress = _marketContractAddress;
    }

    /**
     * @notice Create A New Collection With Media Contract.
     */
    function createNewMediaForCollection(CollectionData calldata collectionData)
        external
        override
        returns (address)
    {
        Media media = new Media(
            collectionData.nftSymbol,
            collectionData.nftName
        );

        address mediaCreatedAddress = address(media);

        media.setMarketAddress(marketContractAddress);
        media.transferOwnership(msg.sender);

        collections[mediaCreatedAddress] = CollectionData(
        ...
        return mediaCreatedAddress;
    }

    /**
     * @notice check if a contract address is a registered media contract
     */
    function validMedia(address _mediaAddress)
        external
        view
        override
        returns (bool)
    {
        return collections[_mediaAddress].mediaAddress != address(0);
    }
}
    ...
    IMedia
{
    ...
    function setMarketAddress(address _marketContract) external onlyOwner {
    ...
    function mintToken(
        ...
        IMarket(marketContract).setAsk(user, address(this), id, ask);
    ...
    function burnToken(uint256 tokenId) external onlyExistingToken(tokenId) {
...
interface IMarket {
    ...
    function currentAskForToken(address mediaContract, uint256 tokenId)
        external
        view
        returns (Ask memory);
