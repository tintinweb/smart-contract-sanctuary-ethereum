...
contract PhysicalRedeemable is Ownable {
    ...
    event Redeemed(
        ...
        uint256[] physicalItemTokenAmounts,
        address userAddress,
        uint256 ethValue,
        uint256 nonce
    );
    ...
    function setRouterContractAddress(address _address) public onlyOwner {
        routerContractAddress = _address;
    }

    function setPhysicalItemTokenIds(uint256[] memory _newAddr)
        public
        onlyOwner
    {
        physicalItemTokenIds = _newAddr;
    }
    ...
    function collect(
        ...
        require(!isDisabled, "collect is disabled");
    ...
    function handleReceive(
        ...
        uint256, /*_type*/
        uint256, /*_id*/
        uint256 _ethValue, /*_ethValue*/
        uint256, /*_primeValue*/
        uint256[] memory _physicalIds,
        uint256[] memory _physicalQuantities,
        bytes memory _data
    ) external onlyParallelAuxiliaryItemsRouter {
        require(!isDisabled, "redeem is disabled");
        require(
            keccak256(abi.encodePacked(_physicalIds)) ==
                keccak256(abi.encodePacked(physicalItemTokenIds)),
            "invalid token ids sent"
        );
        (uint256 _nonce, bytes memory _signature) = abi.decode(
            _data,
            (uint256, bytes)
        );
        require(!nonce[_userAddress][_nonce], "signature already used");
        nonce[_userAddress][_nonce] = true;
        require(
            verify(trustedSigner, _userAddress, _ethValue, _nonce, _signature),
            "invalid signature"
        );
        // emit redeemed event
        emit Redeemed(
    ...
    //  @param _userAddress address of the target
    //  @param _ethValue the amount the user has to pay for the tokens in ETH
    //  @param _nonce nonce to ensure signature won't be reused
    ...
    function verify(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(_userAddress, _ethValue, _nonce)
        );
