...
// File contracts/vault/VaultsStorageLibrary.sol
...
// File contracts/vault/VaultManagerInterface.sol
...
// File contracts/tokens/MultiToken.sol
...
// File contracts/tokens/StableTokenInterface.sol
...
// File contracts/random/RandomNumberGeneratorInterface.sol
...
// File contracts/strategy/PrizeStrategyInterface.sol
...
// File contracts/vault/VaultManager.sol
    ...
    PausableUpgradeable
{
    ...
    IERC20Upgradeable private stableToken;
    ...
    IRandomNumberGenerator private rngService;

    //address of Prize Strategy contract
    IPrizeStrategy private prizeStrategyService;
    ...
    function initialize(
        ...
        stableToken = IERC20Upgradeable(_stableERC20Address);
        AccessControlUpgradeable.__AccessControl_init();
    ...
    {
        require(
            _minimumParticipants >= 1,
            "Vault Manager: Number of participants must be greater than one"
        );
    ...
    function participantDeposit(
        ...
        address _participant,
        uint256 _amount
    ) public nonReentrant onlyRole(BACKEND_ADMIN_ROLE) {
        VaultsStorageLibrary.VaultState state = vaultsList.get(_vaultId).state;
        uint256 blockNumberUpdated = vaultsList
            .get(_vaultId)
            .blockNumberUpdated;
        uint256 participantsNumber = vaultsList
            .get(_vaultId)
            .participantsNumber;
        uint256 minimumParticipants = vaultsList
            .get(_vaultId)
            .minimumParticipants;

        require(_isOpen(state), "Vault is not open");
        require(
            stableToken.balanceOf(_participant) >= _amount,
            "Vault Manager: Participant don't have enough founds!"
        );

        IRandStableToken randStableToken = IRandStableToken(
            address(stableToken)
        );

        // burning the rand stable tokens from the participant balance
        randStableToken.burn(_participant, _amount);
        ...
        if (multiToken.balanceOf(_participant, _vaultId) == 0) {
            ...
            // yielding can start
            if (participantsNumber.add(1) >= minimumParticipants) {
                if (_isWaiting(state)) {
        ...
        if (state == VaultsStorageLibrary.VaultState.Yielding) {
            uint256 duration = vaultsList.get(_vaultId).duration;
            ...
            uint256 endBlock = blockNumberUpdated.add(duration);
            uint256 twaAmount = endBlock.sub(block.number);
            twaAmount = twaAmount.mul(_amount).div(duration);
    ...
    function participantWithdraw(
        ...
        VaultsStorageLibrary.VaultState state;
        uint256 blockNumberUpdated;
        uint256 duration;
        uint256 lockDuration;

        state = vaultsList.get(_vaultId).state;
        blockNumberUpdated = vaultsList.get(_vaultId).blockNumberUpdated;
        duration = vaultsList.get(_vaultId).duration;
        lockDuration = vaultsList.get(_vaultId).lockDuration;

        uint256 feeAmount;
        if (state == VaultsStorageLibrary.VaultState.Yielding) {
            // getting the remaining period until the yielding will stop
            feeAmount = blockNumberUpdated.add(duration).sub(block.number);
            // calculating the percent of the remaining period relative to the yielding duration
            feeAmount = feeAmount.mul(100).div(duration);
        ...
        require(
            state != VaultsStorageLibrary.VaultState.Rewarding &&
                state != VaultsStorageLibrary.VaultState.Finished,
            "Vault Manager: You can not withdraw now"
        );
        require(
            multiToken
                .getParticipantLastDepositDate(_vaultId, _participant)
                .add(lockDuration) <= block.number,
            "Vault Manager: You can not withdraw until the lock period has passed!"
        );
        ...
        onlyRole(BACKEND_ADMIN_ROLE)
    {
        ...
        require(
            state == VaultsStorageLibrary.VaultState.WaitingParticipants ||
                state == VaultsStorageLibrary.VaultState.Created,
            "Vault Manager: You cannot withdraw after Yielding started"
        );
        address[] memory participants = vaultsList.getParticipants(_vaultId);
        require(
            participants.length > 0,
            "Vault Manager: There must be at least one participant"
        );
        for (uint256 i = 0; i < participants.length; i++) {
    ...
    function startYielding(
        ...
        uint256 _duration,
        bool _forceStart
    ) public onlyRole(BACKEND_ADMIN_ROLE) {
        require(
            _duration != 0,
            "Vault Manager: Vault duration must be greater than zero."
        );
        ...
        require(
            state == VaultsStorageLibrary.VaultState.Created || _forceStart,
            "Vault is not ready"
        );
        vaultsList.updateState(
            _vaultId,
    ...
    function stopYielding(
        ...
        if (_timeWeightNonRadom && _allToCreatorNonRandom) {
            revert(
                "Vault Manager: Choose between Time Weight or All To Creator distribution"
            );
        }
        ...
        if (
            ...
            revert(
                "Vault Manager: When Vault is recurrent, only distribution based on tiers is allowed"
            );
        }

        require(
            vaultstate.state == VaultsStorageLibrary.VaultState.Yielding,
            "Vault is not yielding"
        );

        require(
            block.number >= vaultstate.blockNumberUpdated + vaultstate.duration,
            "Vault can't stop yielding before duration"
        );
        ...
        if (_timeWeightNonRadom) {
            require(
                ...
                "Vault Manager: When choosing time weight based, it must be a non-recurrent vault."
            );
        ...
        if (_allToCreatorNonRandom) {
            require(
                !vaultstate.isRecurrent,
                "Vault Manager: When choosing All To Creator based, it must be a non-recurrent vault."
            );
        ...
        require(
            ...
            "Vault Manager: Distribution based on tiers needs to bet set true"
        );

        // verify that Prizes >= Participants in vault
        require(
            _verifyPrizesLessThanParticipants(_vaultId, _tiers),
            "Vault Manager: Prize in tiers cannot be greater than participants."
        );
    ...
    {
        VaultsStorageLibrary.VaultState state = vaultsList.get(_vaultId).state;
        bool isRecurrent = vaultsList.get(_vaultId).isRecurrent;

        address[] memory participants = vaultsList.getParticipants(_vaultId);

        require(
            state == VaultsStorageLibrary.VaultState.Rewarding,
            "Vault is not rewarding"
        );

        uint256 vaultTwaBalance = 0;
        for (uint256 i = 0; i < participants.length; i++) {
            vaultTwaBalance = vaultTwaBalance.add(
                multiToken.twaBalanceOf(participants[i], _vaultId)
            );
        }

        _handleRewards(_vaultId, participants, vaultTwaBalance, _randomNumber);

        if (isRecurrent) {
            prizeStrategyService.computeRewards(_vaultId, participants);
        }
    }
     ...
     */
    function transferAndBurn(uint256 _vaultId, address _participant) private {
        IRandStableToken randStableToken = IRandStableToken(
            address(stableToken)
        );
        ...
        randStableToken.mint(_participant, amount);
     ...
     * @param _participants Array of addresses of the users which have funds in the vault
     * @param _vaultTwaBalance The sum of the twa balances of the users in the vault
    ...
    function _handleRewards(
        ...
        if (isRecurrent) {
            ...
            for (uint256 i = 0; i < _participants.length; i++) {
                transferAndBurn(_vaultId, _participants[i]);
            }
    ...
    function _distributeRewards(
        ...
        uint256[] memory winnersTiers;

        (winners, winnersTiers) = prizeStrategyService.getWinners(
            _vaultId,
            _participants,
            _vaultTwaBalance,
            _randomNumber
        ...
        uint256[] memory prizes = prizeStrategyService.getWinnersPrizes(
            _vaultId,
            winnersTiers
        );
        ...
        for (uint256 i = 0; i < prizes.length; i++) {
            multiToken.mint(
                ...
                _vaultId,
                prizes[i],
                multiToken.calculateTwaAmount(_vaultId, winners[i], prizes[i]),
                ""
            );
            uint256 winnerPrize = multiToken.prizeOf(winners[i], _vaultId).add(
                prizes[i]
            );
            multiToken.setPrize(winners[i], _vaultId, winnerPrize);
            vaultsList.increasePrizes(_vaultId, prizes[i]);
            vaultsList.increaseBalance(_vaultId, prizes[i]);
        }

        uint256 prizesAmount;
        prizesAmount = vaultsList.get(_vaultId).prizesAmount;

        emit RewardsDistributed(_vaultId, prizesAmount, winners, prizes);
    }
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < _participants.length; i++) {
            ...
            uint256 winnerPrize = multiToken
                .prizeOf(_participants[i], _vaultId)
                .add(participantPrize);
            multiToken.setPrize(_participants[i], _vaultId, winnerPrize);
    ...
    {
        address creator;
        creator = vaultsList.get(_vaultId).creator;
        multiToken.mint(
            creator,
            _vaultId,
            _prize,
            multiToken.calculateTwaAmount(_vaultId, creator, _prize),
            ""
        );

        uint256 winnerPrize = multiToken.prizeOf(creator, _vaultId).add(_prize);

        multiToken.setPrize(creator, _vaultId, winnerPrize);
    ...
    function _handleParticipantWithdraw(
        ...
        require(
            ...
            "VaultManager: insufficient founds!"
        );
        ...
        randStableToken.mint(_participant, _amount.sub(_feeAmount));
        ...
        emit ParticipantWithdraw(_vaultId, _participant, _amount);
    }
    ...
    function _burn(uint256 _amount) internal {
        ...
        randStableToken.burn(address(this), _amount);
    }
     ...
     * @param _state state of the vault
     */
    function _isWaiting(VaultsStorageLibrary.VaultState _state)
        internal
        pure
        returns (bool)
    {
        if (_state == VaultsStorageLibrary.VaultState.Created) return false;
        if (_state == VaultsStorageLibrary.VaultState.Yielding) return false;
        if (_state == VaultsStorageLibrary.VaultState.Rewarding) return false;
        if (_state == VaultsStorageLibrary.VaultState.Finished) return false;
        if (_state == VaultsStorageLibrary.VaultState.Cancelled) return false;
        return true;
    ...
    function _verifyPrizesLessThanParticipants(
        ...
        return participantsNumber >= numberPrizes;
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            ...
            "Vault Manager: Backend contract address cannot be address 0!"
        );
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        rngService = IRandomNumberGenerator(_rngAddress);
    }

    /**
     * @notice used to change the address of the Prize Strategy contract
     * @param _prizeStrategyAddress new address of the Prize Strategy contract
     */
    function setPrizeStrategyAddress(address _prizeStrategyAddress)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            _prizeStrategyAddress != address(0),
            "Vault Manager: Prize Strategy contract address cannot be address 0!"
        );
        prizeStrategyService = IPrizeStrategy(_prizeStrategyAddress);
    }
}