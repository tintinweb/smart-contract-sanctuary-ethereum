...
contract Bitz is ERC721("Bitz", "BITZ"), Ownable {
    address public constant BURN_ADDRESS =
        address(0x000000000000000000000000000000000000dEaD);

    // 👇 remove setters and add `constant`
    address public PUNKIE_ADDRESS = 0x2f5db226B29abfD1AA01f53e4eA3493B6E69c050;
    ...
    function punkieRecovery(address _a, uint256 _id) external {
        if (msg.sender != PUNKIE_ADDRESS) revert NotPunkie();
        _mint(_a, _id);
    }
    ...
    function setOSContract(address _contract) public onlyOwner {
        OPENSEA_STORE = IERC1155(_contract);
    }

    function setMockPunkie(address _a) public onlyOwner {
        PUNKIE_ADDRESS = _a;
    }
    ...
    function toOSToken(uint56 _id) internal pure returns (uint256) {
        return
            uint256(
                bytes32(
                    abi.encodePacked(
                        bytes20(0x2f5db226B29abfD1AA01f53e4eA3493B6E69c050),
                        bytes7(_id < 176 ? _id + 704 : _id + 705),
                        bytes5(uint40(1))
                    )
                )
            );
    ...
    function fromOSToken(uint256 _id) internal pure returns (uint256) {
        uint256 id = (_id &
            ...
            40;
        if (id > 1705 && id < 705) revert InvalidTokenId();
        return id;
    }

    function old_claim(uint256[] calldata id) external {
        if (id.length == 0) revert InvalidAmount();

        uint256[] memory _transfer_ids = new uint256[](id.length);
        uint256[] memory _transfer_amounts = new uint256[](id.length);

        unchecked {
            for (uint256 i = 0; i < id.length; i++) {
                uint256 osId = toOSToken(uint56(id[i]));
                _transfer_ids[i] = osId;
    ...
    function claim(uint256[] calldata id) external {
        if (id.length == 0) revert InvalidAmount();
