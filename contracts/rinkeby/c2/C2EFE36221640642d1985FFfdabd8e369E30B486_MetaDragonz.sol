...
contract MetaDragonz is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => string) internal dragonzMetadata; //bracket => hash on ipfs
    mapping(uint256 => string) internal skinsMetadata; //bracket => hash on ipfs
    mapping(uint256 => uint256) internal dragonLevels; //tokenId => bracket
    mapping(bytes32 => uint256) internal collectionIssuance; //key => issued
    mapping(bytes32 => uint256) internal maxIssuance; //key => maxIssuance for collection
    ...
    event SkinsMetadata(uint256 _bracket, string _hash);
    event DragonzMetadata(uint256 _bracket, string _hash);
    event SetMintPrice(uint256 _price);
    event DragonsLevelUp(uint256[] _dragonz);
    event DragonsLevelDown(uint256[] _dragonz);
    ...
    function issueToken(address _beneficiary, bytes32 _key) external payable {
        require(
            allowed[msg.sender]
                ? msg.value >= 0
        ...
        uint256 tokenId = collectionIssuance[_key];
        mint(_beneficiary, tokenId, _key);
    }
    ...
    function mint(
        ...
        require(
            !isPreMint || allowed[msg.sender] || isWhiteListed[_beneficiary],
            "Only an `whitelisted` address can call this method"
        );
        require(collectionIssuance[_key] > 0, "Collection issuance is not set");
        ...
        collectionIssuance[_key] = collectionIssuance[_key] + 1;
        ...
        returns (string memory)
    {
        ...
        if (tokenId <= 10000) {
            string memory _hash = dragonzMetadata[_bracket];
            return
                string(
                    abi.encodePacked(
                        baseURI,
            ...
            string memory _hash = skinsMetadata[_bracket];
            return
                string(
                    abi.encodePacked(
                        baseURI,
    ...
    function setMetaDragonzHash(uint256 _bracket, string memory _hash)
        external
        onlyAllowed
    {
        dragonzMetadata[_bracket] = _hash;
        emit DragonzMetadata(_bracket, _hash);
    }

    function setSkinsHash(uint256 _bracket, string memory _hash)
        external
        onlyAllowed
    {
        skinsMetadata[_bracket] = _hash;
        emit SkinsMetadata(_bracket, _hash);
    }

    function setCollectionIssuance(bytes32 key, uint256 _issued)
        external
        onlyAllowed
    {
        collectionIssuance[key] = _issued;
    }

    function setMaxIssuance(bytes32 key, uint256 _maxIssuance)
        external
        onlyAllowed
    {
    ...
    function setWhitelist(address[] memory user, bool[] memory val)
        external
        onlyOwner
    {
    ...
    function levelUp(uint256[] calldata _dragonz) external onlyAllowed {
        for (uint256 index = 0; index < _dragonz.length; index++) {
            dragonLevels[_dragonz[index]] += 1;
        }
        emit DragonsLevelUp(_dragonz);
    }
    ...
    function levelDown(uint256[] calldata _dragonz) external onlyAllowed {
        ...
        emit DragonsLevelDown(_dragonz);
    }
    ...
    function getIssued(bytes32 key) external view returns (uint256) {
        return collectionIssuance[key];
    }
