...
contract DutchAuctionSeller is Ownable {
    ...
    function getCurrentPrice2() external view returns(uint256) {
        uint256 nowSeconds = block.timestamp;
        ///require(nowSeconds>=startDate, "auction didn't start yet");
        //require(!paused, "smart contract is on pause");
        if(nowSeconds < startDate) {
       ...
       return ((nowSeconds - endDate) * (startPrice - endPrice));
    }
    ...
    function getCurrentPrice3() external view returns(uint256) {
        uint256 nowSeconds = block.timestamp;
        ///require(nowSeconds>=startDate, "auction didn't start yet");
        //require(!paused, "smart contract is on pause");
        if(nowSeconds < startDate) {
       ...
       return  (startDate - endDate) + endPrice;
