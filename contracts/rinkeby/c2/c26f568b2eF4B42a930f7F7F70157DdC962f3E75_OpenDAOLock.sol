...
contract OpenDAOLock is Ownable {
    ...
    function lock(uint256 _amount) external {
        ...
        locks[msg.sender].unlockTime = uint64(block.timestamp + lockDuration);
    }

    function unlock(uint256 _amount) external {
        require(_amount > 0, "OpenDAOLock: Invalid amount");
        require(block.timestamp >= locks[msg.sender].unlockTime, "OpenDAOLock: Cannot unlock");
        locks[msg.sender].amount -= uint192(_amount);
