    ...
    AccessControl
{
    ...
    /* ================= INITIALIZATION =================== */
    /** 
     @notice Constructor for the liqd NonFungibleToken
     @param _maxMint  uint256 the max number of mints on this chain
     @param _baseTokenURI string token metadata URI
     */

    constructor(uint256 _maxMint, string memory _baseTokenURI)
        ERC721A("Liqd NFT", "LIQD-NFT")
    {
    ...
    */
    ...
    /**
        @notice burn NFT from admin
     */
    function burn(uint256 _tokenId) external onlyOwner {
        require(
            hasRole(BURN_ROLE, _msgSender()),
            "LIQDNFT: must have BURN role to burn"
        );
        _burn(_tokenId);
    }
    ...
    /**
        @notice set role of admin for burn
        @param _admin address
     */
    function setRole(address _admin) public onlyOwner {
    ...
    function mint(address _user)
        ...
        returns (uint256)
    {
        require(_totalSupply + 1 <= maxMint, "LIQDNFT: Max limit reached");
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC721A, ERC721Enumerable, AccessControl)
        returns (bool)
    {
        return
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _safeMint(
        ...
        _addressData[to].balance = uint64(
            _addressData[to].balance.add(uint64(quantity))
        );
        _addressData[to].numberMinted = uint64(
            _addressData[to].numberMinted.add(uint64(quantity))
        );
