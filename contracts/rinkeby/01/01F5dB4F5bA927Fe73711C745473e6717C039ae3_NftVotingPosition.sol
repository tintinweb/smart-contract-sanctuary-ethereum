...
contract NftVotingPosition is ERC721, Ownable
{
	...
	function withdrawDaiFromVotingPosition(uint256 votingPositionId, uint256 daiNumber) external onlyVotingOwner(votingPositionId)
	{
		nftBattleArena.withdrawDaiFromVoting(votingPositionId, msg.sender, daiNumber, false);//beneficiary, false);
	}
	...
	function swapVotesFromPosition(uint256 votingPositionId, uint256 daiNumber, uint256 newStakingPositionId, uint256 newVotingPosition) external onlyVotingOwner(votingPositionId)
	{
		...
		nftBattleArena.swapPositionVotes(votingPositionId, msg.sender, daiNumber, newStakingPositionId, newVotingPosition);
	}
	...
	function batchWithdrawDaiFromVoting(uint256[] calldata votingPositionIds, uint256 daiNumber) external
	{
		for (uint256 i = 0; i < votingPositionIds.length; i++)
		{
			require(msg.sender == ownerOf(votingPositionIds[i]), "Not the owner of voting");

			nftBattleArena.withdrawDaiFromVoting(votingPositionIds[i], msg.sender, daiNumber, false);
		}
	}
