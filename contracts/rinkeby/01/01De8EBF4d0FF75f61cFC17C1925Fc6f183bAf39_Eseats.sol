...
contract Eseats is Ownable, ERC721EnumerableM {
    ...
    // function addTicket(uint256 _supply, uint256 _expiry, uint256 _price) external onlyOwner {
    //     supply.push(_supply);
    //     expiry.push(_expiry);
    //     price.push(_price);
    // }
    ...
    function _mintTicket(uint256 _ticketType, address _receiver, bytes32[] memory _metadata) internal {
        ...
        Ticket storage ticket = _tickets[currentTokenId];
        ticket.expiredAt = block.timestamp + expiry[_ticketType];
        ticket.ticketType = _ticketType;
        ticket.state = TicketState.UNUSED;
        ticket.metadata = _metadata;
    ...
    function offlineCheckIn(address attendee, uint256 tokenId) external {
        require(_admins[msg.sender], "e4");
        require(isCheckInClosed == false, "e5");
        require(_eventType == EventType.OFFLINE, "e6");
    ...
    function _checkIn(address attendee, uint256 tokenId) internal {
        ...
        require(ticket.state == TicketState.UNUSED, "e14");
    ...
    function tokenURI(uint256 tokenId) external override view returns (string memory) {
        ...
        if(supply.length > 1) {
            ...
            return string(abi.encodePacked(_baseTokenURI, ticket.ticketType.toString()));
        }

        return _baseTokenURI;
    }

    function closeCheckin() external onlyOwner whenActive {
        isCheckInClosed = true;
    }

    function openCheckIn() external onlyOwner whenActive {
        isCheckInClosed = false;
    }

    function addAdmin(address admin) external onlyOwner whenActive {
        require(_eventType == EventType.OFFLINE, "e6");

        _admins[admin] = true;
    }

    function removeAdmin(address admin) external onlyOwner whenActive {
        require(_eventType == EventType.OFFLINE, "e6");

        _admins[admin] = false;
    }
    
    function activateEvent() external onlyOwner {
        _isEventActive = true;
    }
    ...
    function rewardAmount() public view returns (uint256) {
        ...
        return rewardVolume / _checkedIn.length;
    }

    function sendReward() external onlyOwner {
        require(_isRewardable == true, "e9");
        require(isEventFinished == true, "e10");
        require(rewardVolume <= IERC20(rewardToken).balanceOf(address(this)), "e11");
    ...
    function setRewardVolume(uint256 _volume) external onlyOwner {
        rewardVolume = _volume;
    }

    function viewTicket(uint256 _tokenId) external view returns(Ticket memory){
