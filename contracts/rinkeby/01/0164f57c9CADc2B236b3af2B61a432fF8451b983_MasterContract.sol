...
contract MasterContract is AccessControl, IMaster, ERC721 {
  ...
  address constant WITHDRAW_ADDRESS = 0x5a56C7909b5d55b1b6e7A30c2559FeA8FaA1dcA5; // 0x0867436a889bf9C1abCAf3c505046FC4F7880b50
  address constant OWNER_ADDRESS = 0x5a56C7909b5d55b1b6e7A30c2559FeA8FaA1dcA5; // 0x4Ec75C4925E14631a4104ABA596a2Ff4Bb3C846a
  ...
  mapping(Round => RoundContract) private _roundContract;
  ...
  mapping(uint => Round) _tokenRound;
  ...
  constructor(
    ...
    address[] memory rounds
      ...
      waitReveal = revealDate;
      
      for(uint8 round = uint8(Round.Legendary); round <= uint8(Round.Public); round++ ) {
        _roundContract[Round(round)] = RoundContract(rounds[round]);
        _roundPrice[Round(round)] = RoundContract(rounds[round]).mintPrice();
      }
      _setupRole(ROLE_ADMIN, OWNER_ADDRESS);
  }

  function setMaxSupply(uint16 _maxSupply) external onlyRole(ROLE_ADMIN) {
    maxSupply = _maxSupply;
    emit ChangeMaxSupply(_maxSupply);
  }

  /// @notice Withdraw funds to recipient
  function withdraw() external onlyRole(ROLE_ADMIN) {
    payable(WITHDRAW_ADDRESS).sendValue(address(this).balance);
    emit Withdraw(WITHDRAW_ADDRESS);
  }

  /// @notice Сhecks whether the collection is revealed 
  function isRevealed() public view returns(bool) {
    return block.timestamp - createDate >= waitReveal;
  }

  function tokenRoundString(uint tokenId) public view returns(string memory) {
    Round round = _tokenRound[tokenId];
    if (round == Round.Legendary) return 'legendary';
    else if (round == Round.Epic) return 'epic';
    else if (round == Round.SuperRare) return 'super-rare';
    else if (round == Round.Rare) return 'rare';
    else return 'public';
  }
  ...
  function tokenURI(uint256 tokenId)
    ...
    returns (string memory)
    {
      ...
      return bytes(currentBaseURI).length > 0
          ? string(abi.encodePacked(currentBaseURI, tokenRoundString(tokenId), "/metadata/", Strings.toString(tokenId), ".json"))
          : "";
  }
  ...
  function fulfillMetaDataRequest(string memory json, uint id, uint tokenId) external override onlyRole(ROLE_ORACLE) {
  ...
  function setMetaDataOracleAddress(address newAddress) external override onlyRole(ROLE_ADMIN) {
    metaDataOracle = IMetaDataOracle(newAddress);
    _setupRole(ROLE_ORACLE, newAddress);
  ...
  function _getAttributes(uint tokenId) private {
  ...
  function roundAddress(Round round) public view returns(address) {
    return address(_roundContract[round]);
  }

  function mintedIds() public view returns(uint[] memory) {
    return _mintedIds;
  }

  function getRoundTotalSupply(Round round) external view returns(uint) {
    return _roundContract[round].roundTotalSupply();
  }
  ...
  /// @param round - targer round contract
  /// @param nTokens - number of tokens for mint
  ...
  function mint(Round round, uint nTokens) payable override external {
    RoundContract currRound = _roundContract[round];
    require(msg.value >= currRound.mintPrice() * nTokens || (currRound.isFreeRoundNow() && currRound.checkPayableWhitList(msg.sender)), "NFT round: Not enough funds for this round");
    require(totalSupply <= maxSupply, "NFT: too many token");
    require(nTokens <= maxSupply - totalSupply, "NFT:  There are not enough tokens left");

    uint[] memory idxs = currRound.getRandoms(msg.sender, nTokens);

    if (idxs.length < nTokens) {
      payable(msg.sender).sendValue(msg.value);
      revert("NFT: The required number of free tokens was not found, try again");
    }

    currRound.setUserPurchase(idxs.length, msg.sender);
    currRound.setRoundTotalSupply(uint16(idxs.length));
    totalSupply += uint16(idxs.length);

    for (uint256 i = 0; i < idxs.length; i++) {
      _getAttributes(idxs[i]);
      _tokenRound[idxs[i]] = round;
      occupiedIdxs[idxs[i]] = true;
      _mintedIds.push(idxs[i]);
      _safeMint(msg.sender, idxs[i]);
      emit MintRand(msg.sender, idxs[i]);
    }
  }
}
...
contract RoundContract is AccessControl {
  ...
  function createAdder(address user) external onlyRole(ROLE_ADMIN) {
    _setupRole(ROLE_ADDER, user);
  }
  ...
  function setMaster(address _master) external onlyRole(ROLE_ADMIN) {
  ...
  function toggleWhitelist(bool enable) external onlyRole(ROLE_ADMIN) {
    _enableWhitelist = enable;
  }

  /// @notice enable while list
  /// @param enable - true or false value
  function togglePayableWhitelist(bool enable) external onlyRole(ROLE_ADMIN) {
    _enablePayableWhitelist = enable;
  }

  /// @notice add user to whitelist
  /// @param user - target user
  function addToPayableWhitelist(address user) external onlyRole(ROLE_ADMIN) {
  ...
  function addToPayableWhitelistBatch(address[] calldata users) external onlyRole(ROLE_ADMIN) {
  ...
  /// @param user - target user
  function removeFromPayableWhitelist(address user) external onlyRole(ROLE_ADMIN) {
  ...
  /// @notice add user to whitelist
  /// @param user - target user
  function addToWhitelist(address user) external onlyRole(ROLE_ADMIN) {
    whiteList[user] = true;
  ...
  /// @notice add array of users to whitelist
  /// @param users - array of target users
  function addToWhitelistBatch(address[] calldata users) external onlyRole(ROLE_ADMIN) {
    for (uint256 i = 0; i < users.length; i++) {
  ...
  /// @notice remove user from whitelist
  /// @param user - target user
  function removeFromWhitelist(address user) external onlyRole(ROLE_ADMIN) {
...
contract MetaDataOracle is AccessControl, IMetaDataOracle {
  ...
  function takeRole(address caller) external override onlyRole(ROLE_ADMIN) {
  ...
  function addProvider(address provider) external override onlyRole(ROLE_ADMIN) {
  ...
  function removeProvider(address provider) external override onlyRole(ROLE_ADMIN) {
  ...
  function setProvidersThreshold(uint threshold) external override onlyRole(ROLE_ADMIN) {
