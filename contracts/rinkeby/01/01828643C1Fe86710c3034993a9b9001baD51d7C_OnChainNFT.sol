...
contract OnChainNFT is ERC721, VRFConsumerBase, Ownable {
  ...
  //Will be used later
  //uint256 public chainLinkRate = 5;
  mapping(uint256 => uint256) public bodyColor;
  mapping(uint256 => uint256) public earColor;
  mapping(uint256 => uint256) public eyeColor;
  ...
  function _generateTraits(uint256 randomNumber, uint256 tokenId) internal {
    bodyColor[tokenId] = ((randomNumber >> 8) % 360) + 1;
    earColor[tokenId] = ((randomNumber << 8) % 360) + 1;
    eyeColor[tokenId] = ((randomNumber >> 16) % 360) + 1;
  }
          ...
          Base64.encode(
            bytes(
              abi.encodePacked(
                ...
                id.toString(),
                '", "attributes": [{"trait_type": "color", "body color": "',
  ...
  function renderEyes(uint256 id) public view returns (string memory) {
    string memory render = string(
      abi.encodePacked(
        '<ellipse fill="hsl(',
  ...
  function renderEars(uint256 id) public view returns (string memory) {
    string memory render = string(
      abi.encodePacked(
        '<ellipse fill="hsl(',
