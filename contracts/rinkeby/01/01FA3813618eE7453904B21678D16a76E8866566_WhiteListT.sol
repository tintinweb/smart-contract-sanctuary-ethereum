...
contract WhiteListT is ERC721A, Ownable, ReentrancyGuard {
    bytes32 public merkleRoot =
        0x09a45af50b8267d9eeef627f571e9f3ab54f011098cae3429b67bb9959c4cc48; // root to be generated after full whitelist addresses are done;

    constructor() ERC721A("WhiteListT NFT", "WHTLST") {}

    string public hiddenMetadataUri =
        "ipfs://bafkreigzq25wf7eypm4z2vaslch5pm6w5um76o33zkf5qzf6xlaxo543hq"; //default hidden metadata
    string public baseURI; //the reveal URI to be set a later time

    uint256 public cost = 0.01 ether; //mint price
    uint256 public currentMaxSupply = 1000; //porject supply
    uint256 public maxMintAmountPerWallet = 2; // max amount to mint per wallet
    ...
    mapping(address => uint256) private _claimed; //mapping of addresses that claimed and how much they have claimed

    modifier mintCompliance(uint256 _mintAmount, address _sender) {
        //check the amount to mint is in the allowed max amount
        require(
        ...
        //check that the address still can claim and mint
        require(
            _claimed[_sender] + _mintAmount <= maxMintAmountPerWallet,
    ...
    function mint(bytes32[] calldata _merkleProof, uint256 quantity)
        ...
        mintCompliance(quantity, msg.sender)
    ...
    function setMaxMintAmountPerWalletx(uint256 _maxMintAmountPerWallet)
        public
        onlyOwner
    {
