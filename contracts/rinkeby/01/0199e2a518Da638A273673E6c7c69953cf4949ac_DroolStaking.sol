...
contract DroolStaking is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    ...
    function setStakingToken(address addr) external onlyOwner {
        stakingToken = IERC721(addr);
    }

    function setRewardsToken(address addr) external onlyOwner {
        rewardsToken = IERC20(addr);
    }

    function earned(address account) public view returns (uint256) {
        uint claimed = lastClaim[account];
        uint256 time = min(block.timestamp, END);
        return (time - claimed) * DROOL_PER_SECOND_PER_APT;
    }
    ...
    modifier updateReward(address account) {
        ...
        uint total = (time - claimed) * DROOL_PER_SECOND_PER_APT;
