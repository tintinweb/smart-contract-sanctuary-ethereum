...
import "./lib/Pledge.sol";
import "./lib/Loan.sol";
import "./lib/CapitalPool.sol";


contract PledgeLoanSetUP {

    event SetUp(address capitalPool, address pledge, address loan);

    constructor() {

        CapitalPool capitalPool = new CapitalPool();

        Pledge pledge = new Pledge(address(capitalPool));
        Loan loan = new Loan(address(pledge), address(capitalPool));

        capitalPool.setPledge(address(pledge));
        capitalPool.setLoan(address(loan));

        pledge.setLoan(address(loan));

        emit SetUp(address(capitalPool), address(pledge), address(loan));
...
// contract PledgeLoanSetUPWithManager{
...
//         pledge.transferManagership(manager);
//         loan.transferManagership(manager);
...
contract Loan is LoanStructs, LoanInterface, Manageable, ReentrancyGuard {
    ...
    address internal loanCapitalPool;
    ...
    function loan(
        ...
        require(bookKeep(businessId, NFTAddr, baseInfo[0], eachAmounts));
        ...
        for (uint256 i = 0; i < LoanTimes; ++i) {
            ...
            for (uint256 j = 1; j <= loanPeriods; ++j) {
                if (!list.bill[businessId][j].status) {
                    if (_blockTimestamp() > list.billInfo[businessId].creationTimestamp + list.billInfo[businessId].interval * j) {
                        return false;
                    } else {
                        break;
                    }
