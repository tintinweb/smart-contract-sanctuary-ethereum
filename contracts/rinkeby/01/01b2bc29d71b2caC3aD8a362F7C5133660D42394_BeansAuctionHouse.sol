...
/// @title The Beans DAO auction house
...
// LICENSE
// BeansAuctionHouse.sol is a modified version of Zora's AuctionHouse.sol:
// https://github.com/ourzora/auction-house/blob/54a12ec1a6cf562e49f0a4917990474b11350a2d/contracts/AuctionHouse.sol
//
// AuctionHouse.sol source code Copyright Zora licensed under the GPL-3.0 license.
// With modifications by Beanders DAO.
...
import { IBeansAuctionHouse } from "IBeansAuctionHouse.sol";
import { IBeansToken } from "IBeansToken.sol";
import { IWETH } from "IWETH.sol";

contract BeansAuctionHouse is IBeansAuctionHouse, PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    // The Beans ERC721 token contract
    IBeansToken public beans;
    ...
    // The active auction
    IBeansAuctionHouse.Auction public auction;
    ...
    /**
     * @notice Settle the current auction, mint a new Bean, and put it up for auction.
     */
    ...
    /**
     * @notice Create a bid for a Bean, with a given amount.
     * @dev This contract only accepts payment in ETH.
     */
    function createBid(uint256 beanId) external payable override nonReentrant {
        IBeansAuctionHouse.Auction memory _auction = auction;

        require(_auction.beanId == beanId, 'Bean not up for auction');
        ...
        emit AuctionBid(_auction.beanId, msg.sender, msg.value, extended);

        if (extended) {
            emit AuctionExtended(_auction.beanId, _auction.endTime);
        }
    }

    /**
     * @notice Pause the Beans auction house.
    ...
    /**
     * @notice Unpause the Beans auction house.
     * @dev This function can only be called by the owner when the
    ...
    function _createAuction() internal {
        try beans.mint() returns (uint256 beanId) {
            ...
            emit AuctionCreated(beanId, startTime, endTime);
     ...
     * @dev If there are no bids, the Bean is burned.
     */
    function _settleAuction() internal {
        ...
        if (_auction.bidder == address(0)) {
            beans.burn(_auction.beanId);
        } else {
            beans.transferFrom(address(this), _auction.bidder, _auction.beanId);
        }
        ...
        emit AuctionSettled(_auction.beanId, _auction.bidder, _auction.amount);
    }
...
interface IBeansToken is IERC721 {
    event BeanCreated(uint256 indexed tokenId, IBeansSeeder.Seed seed);
