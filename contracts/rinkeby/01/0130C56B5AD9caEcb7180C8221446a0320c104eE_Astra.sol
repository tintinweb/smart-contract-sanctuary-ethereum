...
contract Astra is ERC721, IERC2981, Ownable, ReentrancyGuard {
    ...
    modifier maxAstraPerWallet(uint256 numberOfTokens) {
        require(
            balanceOf(msg.sender) + numberOfTokens <= MAX_Astra_PER_WALLET,
            "Max Astra to mint is three"
        );
        _;
    }

    modifier canMintAstra(uint256 numberOfTokens) {
        require(
            tokenCounter.current() + numberOfTokens <=
                maxAstra - maxGiftedAstra,
            "Not enough Astra remaining to mint"
        );
        _;
    }

    modifier canGiftAstra(uint256 num) {
        require(
            numGiftedAstra + num <= maxGiftedAstra,
            "Not enough Astra remaining to gift"
        );
        require(
            tokenCounter.current() + num <= maxAstra,
            "Not enough Astra remaining to mint"
        );
        _;
    }
    ...
    constructor(
        ...
        uint256 _maxAstra,
        uint256 _maxCommunitySaleAstra,
        uint256 _maxGiftedAstra
    ) ERC721("Astra", "ASTRA") {
        openSeaProxyRegistryAddress = _openSeaProxyRegistryAddress;
        maxAstra = _maxAstra;
        maxCommunitySaleAstra = _maxCommunitySaleAstra;
        maxGiftedAstra = _maxGiftedAstra;
    }
    ...
    function mint(uint256 numberOfTokens)
        ...
        publicSaleActive
        canMintAstra(numberOfTokens)
        maxAstraPerWallet(numberOfTokens)
    {
        ...
        isValidMerkleProof(merkleProof, communitySaleMerkleRoot)
    {
        ...
        require(
            numAlreadyMinted + numberOfTokens <= MAX_Astra_PER_WALLET,
            "Max Astra to mint in community sale is three"
        );

        require(
            tokenCounter.current() + numberOfTokens <= maxCommunitySaleAstra,
            "Not enough Astra remaining to mint"
        );
    ...
    function claim(bytes32[] calldata merkleProof)
        ...
        canGiftAstra(1)
    {
        require(!claimed[msg.sender], "ASTRUM already claimed by this wallet");
    ...
    function reserveForGifting(uint256 numToReserve)
        ...
        nonReentrant
        onlyOwner
        canGiftAstra(numToReserve)
    {
        numGiftedAstra += numToReserve;
    ...
    function giftAstra(address[] calldata addresses)
        external
        nonReentrant
        onlyOwner
        canGiftAstra(addresses.length)
    {
        uint256 numToGift = addresses.length;
        numGiftedAstra += numToGift;
    ...
    function rollOverAstra(address[] calldata addresses)
        external
        nonReentrant
        onlyOwner
    {
        require(
            ...
            "All Astra are already rolled over"
        );
