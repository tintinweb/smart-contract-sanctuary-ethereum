...
contract Puffers is ERC721AQueryable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    using MerkleProof for bytes32;

    uint256 public price = 0.08 ether;
    uint256 public maxPuffers = 10000;
    uint256 public maxPuffersPerTx = 5;
    uint256 public maxPuffersPerAddressLimit = 10;
    uint256 public reservedPuffers = 25;
    ...
    mapping(address => uint256) public puffersMintedBalance;

    constructor(string memory _hiddenTokenUri, string memory _baseTokenUri)
        ERC721A("The Puffers Club", "PUFFER")
    {
    ...
    function _mintNfts(address _receiver, uint256 _mintAmount) internal {
        ...
        puffersMintedBalance[msg.sender] =
            puffersMintedBalance[msg.sender] +
            _mintAmount;
    }
    ...
    // Modifiers
    modifier isPaused() {
        require(!paused, "Puffers sales is paused currently !!");
        _;
    }
    ...
    modifier mintCompliance(uint256 _mintAmount, address _user) {
        require(
            _mintAmount > 0 && _mintAmount <= maxPuffersPerTx,
            "Invalid mint amount !!"
        );
        require(
            totalSupply() + _mintAmount <= maxPuffers,
            "NFTs are solded out !!"
        );

        require(
            puffersMintedBalance[_user] + _mintAmount <=
                maxPuffersPerAddressLimit,
            "You cannot mint more Puffers !!"
        );
    ...
    // Public Functions
    function mintPuffers(uint256 _mintAmount)
        public
        payable
        nonReentrant
        isPaused
    ...
    // Admin only
    function claimReservedPuffers(address _receiver, uint256 _amount)
        public
        onlyOwner
    {
        require(
            reservedClaimed <= reservedPuffers,
            "You have minted all reserved puffers !!"
        );
    ...
    function setPuffersPerAddressLimit(uint256 _newLimit) public onlyOwner {
        maxPuffersPerAddressLimit = _newLimit;
    }
    ...
    function setMaxPuffersPerTx(uint256 _newLimit) public onlyOwner {
        maxPuffersPerTx = _newLimit;
    }
