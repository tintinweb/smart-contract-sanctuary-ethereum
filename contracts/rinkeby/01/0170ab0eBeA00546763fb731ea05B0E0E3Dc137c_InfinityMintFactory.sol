...
contract InfinityMintStickers is Stickers, ERC721 {
	...
	function burn(uint64 stickerId) public {
		...
		removeFromRegisteredOwners(_sender(), stickerId);
...
abstract contract Stickers is StickerInterface {
	...
	function removeFromRegisteredOwners(address addr, uint64 stickerId)
		internal
	{
		if (registeredOwners[addr].length - 1 == 0) {
			registeredOwners[addr] = new uint64[](0);
			return;
		}

		uint64[] memory temp = new uint64[](registeredOwners[addr].length - 1);
		uint64[] memory copy = (registeredOwners[addr]);
		uint256 index = 0;
		for (uint256 i = 0; i < copy.length; i++) {
			if (copy[i] == stickerId) continue;
			temp[index++] = copy[i];
		}
		registeredOwners[addr] = temp;
	}
