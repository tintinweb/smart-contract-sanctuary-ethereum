    ...
    EIP712Upgradeable
{
    ...
    bytes32 private constant _ORDER_TYPEHASH = 0xa67e3e4af2475c46593e2e3755ef2e32c16813c1ac804fdc5b59a057e8669704;
    ...
    function initialize(
        ...
        require(
            keccak256(
                "Order(address seller,address contractAddress,uint256 royaltyFee,address royaltyReceiver, address paymentToken,uint256 basePrice,uint256 listingTime,uint256 expirationTime,uint256 nonce,uint256 tokenId,Type orderType,bytes signature,string uri,string objId)"
            ) == _ORDER_TYPEHASH
        );
    ...
    modifier onlyApproved(address contractAddress, uint256 tokenId) {
        require(address(this) == IERC721Upgradeable(contractAddress).getApproved(tokenId), "Token id not approved");
        _;
    }
    ...
    function buy(
        ...
        onlyApproved(contractAddress, tokenId)
        returns (uint256)
    {
    ...
    function acceptOffer(
        ...
        isAllowedToken(order.paymentToken)
        isNotBlacklisted(msg.sender)
        onlyApproved(order.contractAddress, order.tokenId)
    {
        (bytes32 digest, address signer) = _verifySig(order);
    ...
    function _verifySig(Order calldata order) internal view returns (bytes32, address) {
     ...
     */
    function hashOrder(Order memory _order) internal view returns (bytes32 hash) {
