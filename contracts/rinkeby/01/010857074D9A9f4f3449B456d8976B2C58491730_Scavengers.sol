...
contract Scavengers is Ownable, ERC721A, ReentrancyGuard {
  IWETH WETH = IWETH(0xc778417E063141139Fce010982780140Aa0cD5Ab);//0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2);
  ...
  constructor(
    ...
    phase3Price = 300000000000000000; // 0.3 ETH
  }
  ...
  function publicMint(uint256 quantity) external payable callerIsUser {
    ...
    if(phase1) {
        ...
        if(totalSupply() < phase1Amount){
            uint256 freeQuantity = phase1Amount - totalSupply();
        ...
        if(totalSupply() < phase2Amount){
            uint256 phase2Quantity = phase2Amount - totalSupply();
            require(msg.value==phase2Price*phase2Quantity);
            _safeMint(msg.sender, phase2Quantity);
            quantity -= phase2Quantity;
        }
        require(msg.value==phase3Price*quantity);
        _safeMint(msg.sender, quantity);
    }
    else {
  ...
  function setIsTeam(address addr, bool _isteam) public onlyOwner() {
    isTeam[addr] = _isteam;
  }
