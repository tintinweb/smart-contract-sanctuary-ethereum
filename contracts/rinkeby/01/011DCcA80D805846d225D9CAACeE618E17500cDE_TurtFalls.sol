...
contract TurtFalls is ERC1155, Ownable, ERC1155Burnable {
    using SafeMath for uint256;

    uint256 TurtFallsTokenId = 0;
    uint256 amountMinted = 0;

    // Note: In the claim function the maximum number of tokens will be based on whether not the avatar id being used
    // is equal to or below this value. Transferring TurtFall tokens to an address without using a avatar can be done
    ...
    string public name = "Silks - Turt Falls";
    ...
    function setSilksAvatarContractAddress(address contractAddress)
        public
        onlyOwner
    {
    ...
    function setSilksMigrateToContract(address contractAddress)
        public
        onlyOwner
    {
    ...
    function claim(uint256[] calldata tokenIds) public returns (uint256) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                silksGenesisAvatarContract.ownerOf(tokenIds[i]) == msg.sender,
                "Doesn't own the token"
            );
            require(
                checkIfRedeemed(tokenIds[i]) == false,
        ...
        uint256 prevTurtFallTokenId = TurtFallsTokenId;
        TurtFallsTokenId++;
        amountMinted = amountMinted + amount;
        _mint(msg.sender, TurtFallsTokenId, amount, "");

        return prevTurtFallTokenId;
    }

    // Allowing direct drop for gievaway
    function airdropGiveaway(
        address[] calldata to,
        uint256[] calldata amountToMint
    ) public onlyOwner {
        for (uint256 i = 0; i < to.length; i++) {
            TurtFallsTokenId++;
            amountMinted = amountMinted + amountToMint[i];
            _mint(to[i], TurtFallsTokenId, amountToMint[i], "");
        }
    }

    // Allow to use the ERC-1155 to get the SilksLand ERC-721 final token
    function migrateTokens(uint256[] calldata ids, uint256[] calldata amounts)
        public
    {
        require(migrationStarted == true, "Migration has not started");
        require(
            ids.length == amounts.length,
            "Mismatch between ids and amounts lengths"
        );
        ...
        SilksMigrateToInterface silksLandContract = SilksMigrateToInterface(
            silksMigrateToContractAddress
        );
    ...
    function checkIfRedeemed(uint256 _tokenId) public view returns (bool) {
        return usedToken[_tokenId];
    }

    function checkIfRedeemedBatch(uint256[] calldata _tokenIds)
        public
        view
        returns (bool[] memory)
    {
