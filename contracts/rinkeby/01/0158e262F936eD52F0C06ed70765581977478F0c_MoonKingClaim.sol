...
contract MoonKingClaim is Ownable{
    ...
    */
    ...
    function addClaimableToken(address _token, uint256 supply, bool state) external{
        onlyOwner();
        if(supply>0)
    ...
    function claimAirdrop(address account, uint256 amount,address token,uint256 nonce,bytes calldata signature) external {
        ...
        require(!usedNonce[token][nonce] && !claimed[account][token], "used nonce|claimed|!claimable");
        require(claimable[token] && balanceClaimable[token]>=amount, "!claimable");
