...
contract WutziBot is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 public PRICE = 1 * 10**17; // 0.1 ETH
    ...
    // Admin mint
    function adminMint(uint256 _tId, address _to) external payable onlyOwner {
        require(_tId <= MAX_ELEMENTS, "Id should be smaller than MAX");
        require(_tId > 0, "Id should not be zero");
        if (_tId <= PUBLIC_COUNT) {
            _tokenIdTracker.increment();
        }
        _safeMint(_to, _tId);
    }
