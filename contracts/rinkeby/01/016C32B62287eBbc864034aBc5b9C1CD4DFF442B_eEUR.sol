    ...
    WhitelistUpgradeable
{
    ...
    function initialize() public initializer {
        __ERC20_init("eEUR", "eEUR");
        __ERC20Burnable_init();
        __Pausable_init();
        __AccessControl_init();
        __UUPSUpgradeable_init();

        multiSigTreasury = 0x9d9133c8191Af631231c28bCfad8cCF1d5f7F8fC;
        _grantRole(eEUR_ADMIN, multiSigTreasury);
 ...
 *
 * The admin role for all roles is `eEUR_ADMIN`, which means
 * that only accounts with this role will be able to grant or revoke other
 * roles. More complex role relationships can be created by using
 * {_setRoleAdmin}.
 *
 * WARNING: The `eEUR_ADMIN` is also its own admin: it has permission to
...
abstract contract AccessControlUpgradeable is Initializable, ContextUpgradeable, IAccessControlUpgradeable, ERC165Upgradeable {
    ...
    bytes32 public constant eEUR_ADMIN = 0x00;
