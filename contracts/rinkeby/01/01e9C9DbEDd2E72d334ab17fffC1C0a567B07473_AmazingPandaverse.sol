...
contract AmazingPandaverse is Ownable, ERC721, VRFConsumerBaseV2 {
  ...
  uint256 public constant PNDV_SUPPLY = 8888;
  uint256 public constant PNDV_PRESALE_SUPPLY = 8100;
  ...
  function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
      ...
      uint256 left = PNDV_SUPPLY - totalSupply;

      require(left >= randomWords.length, "fulfillRandomness: left < randomWords.length");

      for (uint256 i = 0; i < randomWords.length; i++) {
        uint256 randomness = randomWords[i];
        uint256 index = randomness % left;
        if (indexMinted[index]) {
          uint256 li = index;
          while (indexMinted[li] && li > 0) {
            li --;
          }
          if (indexMinted[li]) {
            uint256 ri = index;
            while (indexMinted[ri] && ri < PNDV_SUPPLY - 1) {
              ri ++;
            }
            index = ri;
          } else {
        ...
        indexMinted[index] = true;
      }

      totalSupply += randomWords.length;
  }

  function ownerMint(address to, uint32 numberOfTokens) external onlyOwner {
    mint(to, numberOfTokens);
  }

  function checkWhitelisting(
    address buyer,
    bytes calldata signature,
    uint32 approvedLimit) external view returns (bool) {
    bytes32 digest = keccak256(abi.encodePacked(TYPEHASH, buyer, approvedLimit));
    address signer = digest.toEthSignedMessageHash().recover(signature);
    return signer != address(0) && signer == whitelistSigner;
  }
  ...
  function buyPresale(
    ...
    require(
      (totalSupply + numberOfTokens) <= PNDV_PRESALE_SUPPLY,
      "Not enough PNDV remaining"
    );
  ...
  function buyPublic(uint32 numberOfTokens) external payable {
    ...
    require(
      (totalSupply + numberOfTokens) <= PNDV_SUPPLY,
      "Not enough PNDV remaining"
    );
    require(publicActive, "Sale is not active");
    require(numberOfTokens <= 3, "Transaction limit exceeded");
  ...
  function mint(address to, uint32 numberOfTokens) private {
  ...
  function withdraw(address payable to, uint256 amount) external onlyOwner {
    ...
    // Address.sendValue(0x22, perbalance * 5);
  }
}
