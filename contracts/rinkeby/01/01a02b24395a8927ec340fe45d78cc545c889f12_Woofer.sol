...
contract Woofer is IERC20, Extendable {
    ...
    function woofDog(bytes32 tweetId, address acct) external view viewExtend returns (uint cowoofAmt, uint yieldPerToken, uint yield, uint yieldPaid, uint rewoofPrin, uint rewardPerPrin, uint reward, uint rewardPaid) {
        Dog storage dog = $.woofs[tweetId].dogs[acct];
        return (dog.cowoofAmt, dog.yieldPerToken, dog.yield, dog.yieldPaid, dog.rewoofPrin, dog.rewardPerPrin, dog.reward, dog.rewardPaid);
    }
