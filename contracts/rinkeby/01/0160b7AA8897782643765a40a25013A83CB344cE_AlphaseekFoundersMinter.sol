...
contract AlphaseekFoundersMinter is
    AccessControlMinter,
    ReentrancyGuard,
    SignatureEvaluator,
    AllowedEvaluator,
    ...
    IMinterERC721
{
    IMintableERC721 public tokenContract;
    ...
    uint256 public constant MINTABLE_TOKENS = 1333;
    uint256 public constant PROMOTION_TOKENS = 50;
    uint256 public mintPrice = 3.33 ether;
        ...
        AccessControlMinter()
    {
        ...
        require(promoter_ != address(0), "Invalid promoter address");
        promoter = promoter_;
        tokenContract = IMintableERC721(mintable);
        AccessControlMinter._grantRole(DEFAULT_ADMIN_ROLE, administrator);
        AccessControlMinter._grantRole(DEFAULT_ADMIN_ROLE, developer);
        AccountLimiter._setAccountLimitDefault(0);
    }

    /**
     * @dev premint of promotional tokens
     */
    function premint() external onlyAuthorized {
        require(!preminted, "Already preminted");
        AccountLimiter._incrementAccountCount(promoter, PROMOTION_TOKENS);
        preminted = true;
        _mint(promoter, PROMOTION_TOKENS);
    }
    ...
    function mint(
        ...
        uint256 amount = 1; // one per transaction
        require(mintPrice * amount <= msg.value, "Wrong amount");
        require(
            !signatureRequired ||
        ...
        unchecked {
            minterCount[_msgSender()] += amount; // for claims
        }
        _mint(to, amount);
        delete amount;
    }
    ...
    /**
     * @dev Admin mint an amount to an account
     */
    function adminMint(address to, uint256 amount) external onlyAuthorized {
        AccountLimiter._incrementAccountCount(to, amount);
        _mint(to, amount);
    }
    ...
    function setTokenContract(address tokenContract_) external onlyAuthorized {
        ...
        tokenContract = IMintableERC721(tokenContract_);
    }
    ...
    /**
     * @dev Flag to control Private minting
     */
    function startPrivateMint(bytes32 privateMerkleRoot, uint256 walletLimit)
        external
        onlyAuthorized
    {
        require(preminted, "Must premint before sale");
        require(phase != MintPhase.privateMint, "Private mint already started");
    ...
    /**
     * @dev Flag to control Waitlist minting
     */
    function startWaitlistMint(bytes32 waitlistMerkleRoot, uint256 walletLimit)
        external
        onlyAuthorized
    {
        require(preminted, "Must premint before sale");
        require(
            phase != MintPhase.waitlistMint,
            "Waitlist mint already started"
        );
        AllowedEvaluator._setAllowedMerkleRoot(waitlistMerkleRoot);
        AccountLimiter._setAccountLimitDefault(walletLimit);
        phase = MintPhase.waitlistMint;
        emit MintPhaseSet(phase);
    }
     ...
     */
    function startPublicMint(uint256 walletLimit) external onlyAuthorized {
        require(preminted, "Must premint before sale");
        require(phase != MintPhase.publicMint, "Public mint already started");
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(AccessControlMinter, IMinterERC721)
        returns (bool)
    {
        return
            AccessControlMinter.supportsInterface(interfaceId) ||
            type(IMinterERC721).interfaceId == interfaceId;
    }
    ...
    function _mint(address to, uint256 amount) internal nonReentrant {
        require(to != address(0), "Cannot mint to 0 address");
        require(
            tokenContract.totalSupply() + amount <=
                MINTABLE_TOKENS + PROMOTION_TOKENS,
            "Exceeds supply"
        );
    ...
    function validateActiveSale() private view {
        require(phase != MintPhase.paused, "Minting paused");
    }
}
 ...
 * --- the number of mints or claims, mints and claims, but not mints and claims independently.
...
contract Provisioner is PaymentSplitter {
    uint256 private numberOfPayees;

    constructor(address[] memory payees, uint256[] memory shares)
        payable
        PaymentSplitter(payees, shares)
    {
        numberOfPayees = payees.length;
    }
