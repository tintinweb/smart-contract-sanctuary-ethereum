...
contract DanLand is ERC721, ERC721Enumerable, EIP712, IERC2981, Ownable, AccessControl {
    ...
    constructor(address signer, address wnk, string memory tokenUriUrl)
        ERC721("DanLand", "DGL")
        EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) {
            _setupRole(ADMIN_ROLE, signer);
            _royaltiesRecipient = msg.sender;
            WNK_TOKEN = IERC20(wnk);
    ...
    function setBaseTokenUri(string memory _baseTokenUri) public {
        require(hasRole(ADMIN_ROLE, msg.sender), "Only authorized addresses can set the base token URI.");
    ...
    function buyLandWnk(NFTVoucher calldata voucher) public {
        ...
        require(hasRole(ADMIN_ROLE, signer), "Signature invalid or unauthorized for signer.");
    ...
    function withdrawWnk() public onlyOwner {
    ...
    function mintByOwner(WalletNfts[] calldata nftsToMint) public onlyOwner {
        for (uint walletCpt = 0; walletCpt < nftsToMint.length; walletCpt++) {
            ...
            for (uint tokenIdCpt = 0; tokenIdCpt < walletNfts.tokenIds.length; tokenIdCpt++) {
                _mint(msg.sender, walletNfts.tokenIds[tokenIdCpt]);
                if (msg.sender != walletNfts.walletAddress) {
                    _transfer(msg.sender, walletNfts.walletAddress, walletNfts.tokenIds[tokenIdCpt]);
                }
            }
        }
    }
