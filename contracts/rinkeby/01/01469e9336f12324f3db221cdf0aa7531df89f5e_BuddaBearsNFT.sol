 ...
 */
contract BuddaBearsNFT is ERC721A, Ownable, Pausable, ReentrancyGuard {
    using Strings for uint256;

    //////// VARIABLES & CONSTANTS ////////

    // Address of OpenZeppelin Defender's Relayer
    address private immutable _defender;

    string public baseURI;
    string public baseExtension = ".json";
    string public defaultURI;
    bool private _defaultURIActive = true;

    uint16 public constant MAX_MINT_COUNT_PUBLIC = 1;
    uint16 public constant MAX_SUPPLY = 8888;
    uint16 public constant MAX_TEAM_MINT = 888;
    ...
    // Address that will can mint on behalf of the team.
    ...
    modifier supplyAvailable(uint256 quantity) {
        uint256 teamMintsLeft = MAX_TEAM_MINT - _numberMinted(teamAddress);
        require(
            _totalMinted() + quantity <= MAX_SUPPLY - teamMintsLeft, "Max Supply"
        );
        _;
    }
    ...
    function setDefaultURI(string calldata newDefaultURI)
        external
        onlyOwner
    {
        ...
        supplyAvailable(quantity)
    {
        ...
        bytes32 leaf =
            keccak256(abi.encodePacked(msg.sender, Strings.toString(quantity)));
        require(MerkleProof.verify(proof, merkleRoot, leaf), "Invalid Proof");

        require(
            mintedAllowlist[msg.sender] == false, "Already allowlist minted"
        );
    ...
    function publicMint(
        ...
        nonReentrant
        callerIsUser
        supplyAvailable(MAX_MINT_COUNT_PUBLIC)
    {
        require(publicActive, "Public minting is not active");

        require(
            hash == keccak256(abi.encode(msg.sender, address(this))),
            "Invalid hash"
        );
        require(
        ...
        require(mintedPublic[msg.sender] == false, "Already minted");

        mintedPublic[msg.sender] = true;
        _safeMint(msg.sender, MAX_MINT_COUNT_PUBLIC);
    }

    function teamMint(uint256 quantity) external onlyTeam {
        require(
