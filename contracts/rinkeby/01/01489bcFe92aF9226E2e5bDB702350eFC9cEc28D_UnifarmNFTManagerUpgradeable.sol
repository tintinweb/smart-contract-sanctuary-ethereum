...
contract UnifarmNFTManagerUpgradeable is IUnifarmNFTManagerUpgradeable, Initializable, ERC721Upgradeable, OwnableUpgradeable {
    /// @notice reciveing ETH
    receive() external payable {}

    /// @notice math calculation
    ...
    */
    ...
    function unstakeOnUnifarm(uint256 tokenId) external payable override {
        ...
        refundExcessEth((msg.value.sub(fees.feeAmount)));
    }
    ...
    function claimOnUnifarm(uint256 tokenId) external payable override {
        require(_msgSender() == ownerOf(tokenId), 'INO');
        require(msg.value >= fees.feeAmount, 'FAR');
        address cohortId = tokenIdToCohortId[tokenId];
    ...
    function _buyBooster(
        ...
        if (msg.value > 0) {
            ...
            refundExcessEth((msg.value.sub(boosterPackAmount)));
        } else {
