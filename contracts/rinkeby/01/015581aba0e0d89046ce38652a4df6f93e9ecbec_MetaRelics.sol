...
contract MetaRelics is ERC721Enumerable, Ownable {
	using Strings for uint256;
		...
		payable
	{
		...
		if (msg.sender != owner()) {
			...
			require(
				addressMintedBalance[msg.sender] <= maxMintable + 1,
	...
	// Sets new max # of NFTs mintable per address.
	function setNewMax(uint256 max) public onlyOwner {
		maxMintable = max;
	}
	...
	function setMaxMintable(uint256 count) public onlyOwner {
		maxMintable = count;
	}

	// Withdraws the Ethers in the smart contract to the wallet that specified in the smart contract.
	function withdraw() public payable onlyOwner {
		(bool os, ) = payable(0x120Ee9220CcB8F594483D6e0D5300babb78FAaC7).call{value: address(this).balance}("");
		require(os);
	}
}