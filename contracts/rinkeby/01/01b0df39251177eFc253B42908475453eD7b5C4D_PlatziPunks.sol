...
contract PlatziPunksDNS {
    ...
    // If its mandatory production check chainlink: docs.chain.link/docs/get-a-random-number/
...
pragma solidity ^0.8.0;
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNS {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "PlatziPunks #',
                        tokenId.toString(),
                        '", "description": "Platzi are randomized Avatars stored on chain to teach DApps development on Platzi", "image": "',
                        image,
                        '"}'
                    )
                )
