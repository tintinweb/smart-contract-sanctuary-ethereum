...
contract ctest is ERC721A, EIP712, Ownable {
    // EIP712 Feature
...
// rinkeby ether addresses

    address private constant core1Address =
        0xa2B9f8336a3923291249EA7E19f03ef727cD8f1E;
    uint256 private constant core1Shares = 74000;
    ...
    /**
        Presale with Hash Verification Method - Max Pass Amount
     */
    function mintPresaleHash(
        ...
        require(presaleOn, "Presale Not On");
        require(premintedAmount[msg.sender] + amount <= _passAmount, "Claiming Too Many");
        require(totalSupply() + amount < MAX_SUPPLY_PLUS_ONE, "Exceeds max supply"); 
        ...
        emit MintPresaleHash(msg.sender, amount);
    }
    ...
    function fashionistaClaim(
        ...
        require(claimedAmount[msg.sender] + amount <= _passAmount, "Claiming Too Many");
        require(totalSupply() + amount < MAX_SUPPLY_PLUS_ONE, "Exceeds max supply");

        //Lower requested amount if too many claimed
        uint256 _balance = bfNft.balanceOf(msg.sender);

        if ( _balance < _passAmount){
            amount = _balance - claimedAmount[msg.sender];
        }
        // if (msg.value < cost) revert InsufficientPayment();
...
/*
    /**
        Test Mint if we're doing Free Mints, combined function
     /
    function mintInclusive(address to, uint256 amount) external payable callerIsUser {
        require(tokenSupply + amount < MAX_SUPPLY_PLUS_ONE, "Exceeds max supply");

        uint256 amountPaid;

        if (freeMints < FREE_MINTS_TOTAL){
            amountPaid = amount-1;
        } else {
            amountPaid = amount;
        }

        uint256 cost = amountPaid * price;
        if (msg.value < cost) revert InsufficientPayment();
        freeMints += 1;
...
*/
    ...
    function _splitAll(uint256 _amount) private {
        ...
        _withdraw(core6Address, address(this).balance);
    }
