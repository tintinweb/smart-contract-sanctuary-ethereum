...
contract RemnantBadges is Ownable, ERC721A, SaleEvents {

    string public baseURI;  // Metadata

    constructor() ERC721A("RTestBadges1", "RTB1") {
     ...
     */
    function mintBadges(uint256 _saleEventIndex, uint256 _amount) public payable {
        require(globalSaleIsActive, "Sale off");
        require(isSaleInOngoingList(_saleEventIndex), "Event off");
        require(_amount > 0);

        _mintBadges(saleEvents[_saleEventIndex].saleIdType, _saleEventIndex, _amount);
    }

    function mintBadgesMerkle(uint256 _saleEventIndex, uint256 _amount, bytes32[] calldata _merkleProof) public payable {
        require(globalSaleIsActive, "Sale off");
        require(isSaleInOngoingList(_saleEventIndex), "Event off");
        require(_amount > 0);

        _mintBadgesMerkle(saleEvents[_saleEventIndex].saleIdType, _saleEventIndex, _amount, _merkleProof);
    }

    /**
     * Internal mint based on sale type and sale event index
     */
    function _mintBadges(uint256 _saleType, uint256 _index, uint256 _amount) internal {
        // Check the user mint count mapping (total minted per wallet for this sale event)
        require(_userMintCounts[msg.sender][_index] + _amount <= saleEvents[_index].maxMintCountPerWallet, "Max exceeded");

        // Check the total amount allowed to be minted for this sale event)
        require(saleEvents[_index].currentMintCountTotal + _amount <= saleEvents[_index].maxMintCountTotal, "Exceeded remaining");
        ...
        if (_saleType == 1 || _saleType == 3) { // see SaleEvents.sol for _saleTypes
            require(saleEvents[_index].mintPrice * _amount <= msg.value, "Incorrect ether");
        }
        ...
        if (_saleType == 2 || _saleType == 4) {
            require(MakeRemnPayment(saleEvents[_index].mintPrice * _amount), "Not enough REMN");
        }

        // Increment mints of the sale index to track user mint counts (for limit per wallet per sale event)
        _userMintCounts[msg.sender][_index] += _amount;
        ...
        if (_saleType == 3 || _saleType == 4) {
            _whitelistAvailable[msg.sender][_index] -= _amount;
        }
        ...
        emit MintedBadge(msg.sender, _index, _amount, currentIndex()); 
    }

    function _mintBadgesMerkle(uint256 _saleType, uint256 _index, uint256 _amount, bytes32[] calldata _merkleProof) internal {
...
contract SaleEvents is Ownable {
    ...
    bool public blockBots = true;       // Block bots?
    ...
    struct SaleEvent {
       ...
       uint128 mintPrice;               // In ETHER or REMN (10^18)
    }
    ...
    function createNewSaleEvent (
        ...
        saleEvent.currentMintCountTotal = _currentMintCountTotal;
        saleEvent.mintPrice = _mintPrice * 10 ** 18;
    ...
    function setMintPrice(uint256 _saleEventIndex, uint32 _valueInEther) external onlyOwner {
        saleEvents[_saleEventIndex].mintPrice = _valueInEther * 10 ** 18;
    }
