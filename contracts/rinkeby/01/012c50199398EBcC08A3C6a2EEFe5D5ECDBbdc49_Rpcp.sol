      ...
      if (saleLogs[from].length > 0) {
          for (uint256 i = 0; i < saleLogs[from].length; i++) {
            ...
            if (isInTokenRange) {
                ...
                if (headLogQty > 0) {
                    saleLogs[from].push(SaleLog(log.price, headLogQty, log.startTokenId, log.lastWithdraw));
                }

                if (tailLogQty > 0) {
                    saleLogs[from].push(SaleLog(log.price, tailLogQty, startTokenId + quantity, log.lastWithdraw));
                }             

                // Add new sale log for buyer
                saleLogs[to].push(SaleLog(log.price, quantity, startTokenId, log.lastWithdraw));
            }
        }
      }
    }
