...
contract DollIslandBaby is ERC721A, Ownable {
    string private _baseTokenURI;
    bool public isMintActive;
    uint256 public constant maxSupply = 4444;
    uint256 public constant maxFreeMintPerWallet = 1;
    uint256 public constant maxPerTx = 10;
    uint256 public constant price = 0.004 ether;

    constructor(string memory basetokenURI) ERC721A("DollIslandBaby", "DOll") {
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(quantity <= maxPerTx, "Can not mint this many");

        if (_numberMinted(msg.sender) > 0) {
            require(quantity * price <= msg.value, "Not enough ether");
            _mint(msg.sender, quantity);
        } else {
            require((quantity - 1) * price <= msg.value, "Not enough ether");
            _mint(msg.sender, quantity);
        }
    }
