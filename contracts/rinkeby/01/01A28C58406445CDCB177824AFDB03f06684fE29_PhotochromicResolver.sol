...
contract PhotochromicResolver is ResolverStub, AddrResolver, TextResolver {
    ...
    // Permits modifications only by the owner of the specified node or the owner of the NFT.
    modifier authorisedOrOwner(bytes32 node) {
        require(ens.owner(node) == msg.sender || registrar.ownerOf(uint256(node)) == msg.sender);
        _;
    }
    ...
    constructor(
        ...
        string memory _pcRegistrar,
        address _signerAddress
    ) ResolverStub(_resolver) {
        ens = _ens;
        registrar = _registrar;
        pcRegistrar = _pcRegistrar;
    ...
    function setPCRecords(
        ...
        _addresses[node][60] = Validator.appendTimestamp(addressToBytes(sender), uint32(block.timestamp));
        emit TextChanged(node, "avatar", "avatar");
    }

    function clearPCRecords(bytes32 node) external authorised(node) {
    ...
    function update(bytes32 node, UpdateRecord[] calldata list) external authorised(node) {
        ...
        for (uint i = 0; i < list.length; i++) {
            ...
            texts[node][list[i].key] = string(Validator.appendTimestamp(bytes(list[i].value), t));
    ...
    ) external override(TextResolver) authorisedOrOwner(node) {
        require(!Validator.isIORecord(key));
        texts[node][key] = string(Validator.appendTimestamp(bytes(value), 0));
    ...
    function validatedText(bytes32 node, string calldata key) public view returns (VerificationStatus, string memory, uint32) {
        ...
        if (bytes(value).length == 0 && keccak256(abi.encodePacked(key)) == keccak256("avatar")) {
            ...
            return (VerificationStatus.VERIFIED, string(abi.encodePacked("eip155:1/erc721:", pcRegistrar, "/", nodeString)), 0);
        }
        
        (bytes memory v, uint32 t) = Validator.removeTimestamp(bytes(value));
    ...
    function setAddr(
        ...
        uint coinType,
        bytes memory a
    ) public override(AddrResolver) authorisedOrOwner(node) {
        _addresses[node][coinType] = Validator.appendTimestamp(a, 0);
    ...
    function validatedAddr(bytes32 node, uint coinType) public view returns (VerificationStatus, bytes memory, uint32) {
        ...
        (bytes memory v, uint32 t) = Validator.removeTimestamp(a);
        if (t == 0) return (VerificationStatus.UNVERIFIED, v, 0);
        if (t == 1) return (VerificationStatus.INVALID, v, 0);
    ...
    function deleteValidityInfo(bytes32 node) external authorised(node) {
        delete texts[node][Validator.KYC_VALIDITYINFO];
    }

    function supportsInterface(bytes4 interfaceID) override(ResolverStub, AddrResolver, TextResolver) public pure returns(bool) {
...
contract ResolverStub is ISupportsInterface, IABIResolver, IContentHashResolver, IDNSRecordResolver, IDNSZoneResolver, IInterfaceResolver, INameResolver, IPubkeyResolver {
