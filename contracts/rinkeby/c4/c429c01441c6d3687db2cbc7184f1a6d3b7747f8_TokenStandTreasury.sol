...
import "./TokenStandFarming.sol";

contract TokenStandTreasury is
    Initializable,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /* ========== CONSTRUCTOR ========== */
    function initialize(address _farm, TokenStand _stand) public initializer {
    ...
    event ClaimFriends(address indexed ref, address admin, uint256 standAmount);
    ...
    modifier onlyFarming() {
        require(
            msg.sender == farm,
            "TokenStand Treasury: Only TokenStand Farming can call this function"
        );
        _;
    }
    ...
    function setTokenStandFarming(address _tokenStandFarming)
        external
        restricted
    {
        farm = _tokenStandFarming;
    }

    function transferRewardToRef(address _ref, uint256 _amount)
        external
        onlyFarming
    {
        UserInfo storage user = userInfo[_ref];
        user.stands = user.stands.add(_amount);
    }

    function transferGiftRewardToRef(
        address _ref,
        uint256 _amount,
        address _gift
    ) external onlyFarming {
        UserInfo storage user = userInfo[_ref];
        user.gifts[_gift] = user.gifts[_gift].add(_amount);
    }

    function claimFarming(uint256 _amount, address[] calldata _gifts) external {
        UserInfo storage user = userInfo[msg.sender];
        require(
            user.stands >= _amount,
            "TokenStand Treasury: not enough reward to claim"
        );
        user.stands = user.stands.sub(_amount);
        safeStandTransfer(msg.sender, _amount);
        emit ClaimFarming(msg.sender, _amount, address(stand));
        if (_gifts.length > 0) {
            for (uint256 i = 0; i < _gifts.length; i++) {
                if (user.gifts[_gifts[i]] > 0) {
                    TransferHelper.safeTransfer(
                        _gifts[i],
                        msg.sender,
                        user.gifts[_gifts[i]]
                    );
                    emit ClaimFarming(msg.sender, user.gifts[_gifts[i]], _gifts[i]);
                    user.gifts[_gifts[i]] = 0;
                }
            }
        }
    ...
    function claimFriends(address _ref, uint256 _amount) external restricted {
        safeStandTransfer(_ref, _amount);
        emit ClaimFriends(_ref, msg.sender, _amount);
    }
    ...
    function pendingStand(address _ref)
        external
        view
    ...
    function pendingGift(address _ref, address _gift)
        external
        view
        returns (uint256 pending)
    {
        UserInfo storage user = userInfo[_ref];
        return user.gifts[_gift];
    }
}
...
interface ITokenStandTreasury {
    ...
    function transferGiftRewardToRef(address _ref, uint256 _amount, address _giff) external;
}
        ...
        if (user.amount > 0) {
            ...
            if (giftReward > 0) {
                ...
                if (referral[msg.sender] != address(0)) {
                    ...
                    tokenStandTreasury.transferGiftRewardToRef(referral[msg.sender], refGiftReward, address(farm.gift));
                }
                TransferHelper.safeTransfer(
                    address(farm.gift),
                    msg.sender,
