...
contract ANCApeStake is ANCStake {
    ...
    function multiStake(uint256[] calldata tokenIDs, uint32 stakingDuration) external stakingGate(stakingDuration){
        ...
        /*for (uint256 id = 0; id < tokenIDs.length; id++) {
            tokenId = tokenIDs[id];
            require(_ancApe.ownerOf(tokenId) == msg.sender, "Can only stake tokens you own"); TODO: costs a lot of gas (up to 80k)
        }*/ 
        for (uint256 id = 0; id < tokenIDs.length; id++) {
            tokenId = tokenIDs[id];
            _ancApe.safeTransferFrom(msg.sender, address(this), tokenId);
        ...
        addSharesToWeeks(currentWeek, stakingDuration, getShares(stakingDuration)*tokenIDs.length); // TODO: Check that you can't add the same ID twice
    }
    ...
    function getIDsDoneStaking(address address_) public view returns(uint256[] memory){
        uint256 currentWeek = timestamp2week(block.timestamp);
...
abstract contract ANCStake is Ownable{
    ...
    uint256 private constant ONE_WEEK = 300; // 1 week = 604800
