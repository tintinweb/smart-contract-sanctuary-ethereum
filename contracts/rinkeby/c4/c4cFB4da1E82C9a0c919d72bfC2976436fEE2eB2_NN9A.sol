...
contract NN9A is ERC721A, Ownable, ReentrancyGuard {
...
constructor() ERC721A("NN99AA", "NN9A",50,10000)
{}
...
function toString(bytes memory data) public pure returns(string memory) {
	...
	address aa;
	aa = ownerOf(tokenId);
	string memory aid = toString(aa);
	string memory o1 = string(abi.encodePacked("<svg version=\"1.1\" xmlns=\"http:/","/www.w3.org/2000/svg\" xmlns:xlink=\"http:/","/www.w3.org/1999/xlink\" width=\"500px\" height=\"500px\" viewBox=\"0 0 500 500\"><rect x=\"0\" y=\"0\" width=\"500\" height=\"500\" fill=\"#ffffff\" /><text x=\"0\" y=\"30\" fill=\"red\">Wallet: ",aid,"</text><path id=\"orbit2\" d=\"M 100, 100 m -75, 0 a 75,75 0 1,0 150,0 a 75,75 0 1,0 -150,0\" fill = \"none\" stroke = \"black\" stroke-width = \"3\" opacity=\"0.1\" /><circle r = \"20\" fill = \"yellow\" stroke = \"black\" stroke-width = \"3\"><animateMotion begin=\"0s\" dur=\"12s\" repeatCount=\"indefinite\"><mpath xlink:href=\"#orbit2\"/></animateMotion></circle></svg>"));
	string memory svgData = string(abi.encodePacked(o1));
	string memory svgData64 = string(abi.encodePacked('data:image/svg+xml;base64,', base64encode(svgData)));
	string memory pid = Strings.toString(tokenId);
	string memory json = base64encode(string(abi.encodePacked('{"name": "MY NFT',pid,'", "description": "", "image": "', svgData64, '"}')));
	string memory outstring = string(abi.encodePacked('data:application/json;base64,', json));
	return outstring;
}
    ...
    function base64encode(string memory _str) internal pure returns (string memory) {
        uint i = 0;        uint j = 0;
        uint padlen = bytes(_str).length;        if (padlen%3 != 0) padlen+=(3-(padlen%3));
        bytes memory _bs = bytes(_str);
        bytes memory _ms = new bytes(padlen);        for (i=0; i<_bs.length; i++) {            _ms[i] = _bs[i];
        }
        uint res_length = (padlen/3) * 4;        bytes memory res = new bytes(res_length);
