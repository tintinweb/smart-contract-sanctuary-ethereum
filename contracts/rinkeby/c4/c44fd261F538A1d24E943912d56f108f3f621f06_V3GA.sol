  ...
  ReentrancyGuard
{
  ...
  //uint256 public CURRENT_PRICE = 1E16; // 0.08ETH
  uint256 public CURRENT_PRICE = 0.001 ether; // 0.08ETH
  bool    public hasSaleStarted = false;
  

  constructor() ERC721("V3GA", "v3ga") {
    _state = State.Setup;
    _tokenUriBase = "https://v3ga.ai/?api/";
    _whitelistDetails[0].whitelistType = "Not Whitelisted";
    _whitelistDetails[0].amountToMint = 0;
    _whitelistDetails[1].whitelistType = "V3GA Collective";
    _whitelistDetails[1].amountToMint = 20;
  }
...
//whitelist functions
  function addUserToWhitelistByIndex(
    ...
    _whitelistUserStatus[userToWhitelist].whitelistLevel = whitelistDetailsIndex;
  ...
  function checkUserMintStatus(address user, uint256 amountToMint) public view {
    ...
    require(
      amountToMint + _whitelistUserStatus[user].amountMinted <=
  ...
  function mintToken(uint256 numTOKEN) public payable nonReentrant {
    require(hasSaleStarted == true, "Sale hasn't started");
    require(_state != State.Setup, "Contract is in 'Setup' state");
    require(totalSupply() < MAX_SUPPLY, "Sale has already ended");
    require(numTOKEN > 0 && numTOKEN <= MAX_MINT, "You can adopt minimum 1, maximum 20 Tokens");
    require(totalSupply().add(numTOKEN) <= MAX_SUPPLY, "Exceeds MAX_SUPPLY");
    require(msg.value >= calculatePrice().mul(numTOKEN), "Ether value sent is below the price");
    if (_state == State.Whitelist) {
      checkUserMintStatus(msg.sender, numTOKEN);
    }
    for (uint i = 0; i < numTOKEN; i++) {
      ...
      if (_state == State.Whitelist) {
        updateUserWhitelistStatus(msg.sender);
      }
    }
