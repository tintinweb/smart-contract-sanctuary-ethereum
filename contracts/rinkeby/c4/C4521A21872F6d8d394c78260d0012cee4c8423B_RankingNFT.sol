 ...
 */
contract RankingNFT is ERC721A, Ownable {
    
  uint16 private length = 6667;
  ...
  constructor() ERC721A("Ranking NFT", "RANKINGNFT") {    
  }

  function initialize(uint16 _length) public {
    length = _length;
    unchecked {
      for (uint16 i = 1; i <= length; i++) {
        Attribute memory item = Attribute(i, uint8((i % 10) + 1), uint24(i), uint16(6668 - i));
        attribute[i] = item;
      }
    }
  }
  ...
  function isInMembers(uint256 value) public returns (bool) {
  ...
  */
  function getTotalAttributes() public view returns (Attribute[] memory) {
    Attribute[] memory attrList = new Attribute[](length);
  ...
  * refresh ranking
  */
  function refreshRanking() internal {
    for (uint16 i = 1; i <= length; i++) {
      ...
      for (uint16 j = 1; j <= length; j++) {
        ...
        if (attribute[i].currentFloor > attribute[j].currentFloor) 
          continue;

        if (attribute[i].currentFloor < attribute[j].currentFloor) {
          ranking++;
        } else if (attribute[i].totalTime > attribute[j].totalTime) {
          ranking++;
        }
      }

      attribute[i].ranking = ranking;
    }
  }
  ...
  */
  function setTotalTime(uint256 _id, uint24 _totalTime) external {
    attribute[_id].totalTime = _totalTime;
    refreshRanking();
  }
