...
contract MasterContract is AccessControl, IMaster, ERC721 {
  ...
  function tokenRoundString(uint tokenId) public view returns(string memory) {
    ...
    if (round == Round.Epic) return 'epic';
    if (round == Round.SuperRare) return 'super-rare';
    if (round == Round.Rare) return 'rare';
    else return 'public';
  }
  ...
  function tokenURI(uint256 tokenId)
    ...
    returns (string memory)
    {
      ...
      return bytes(currentBaseURI).length > 0
          ? string(abi.encodePacked(currentBaseURI, tokenRoundString(tokenId), '/', tokenId, ".json"))
          : "";
  }
