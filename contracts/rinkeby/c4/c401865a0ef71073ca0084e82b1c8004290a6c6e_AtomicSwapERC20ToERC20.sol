...
contract AtomicSwapERC20ToERC20 {
  ...
  function getList () public view returns (Swap[] memory) {
    Swap[] memory result = new Swap[](nSwaps);
    // uint256[] memory failedBuyOrderArray = new uint256[](nFailedBuyOrders);
    ...
    //   failedBuyOrderArray[i] = failedBuyOrders[i].closeValue;
    // }
    return result;
  }

  function buyToken (address _closeTrader, address _closeContractAddress, uint256 _closeValue, uint256 _closeCost) public returns (bool success) {
      
      for (uint256 i = 0; i < nSwaps; i++) {
        if (swaps[i].openValue >= _closeValue && swaps[i].openCost <= _closeCost) {
