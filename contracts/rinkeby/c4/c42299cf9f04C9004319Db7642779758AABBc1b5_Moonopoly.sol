...
contract Moonopoly is ERC1155, Ownable, Pausable, ReentrancyGuard
{
    ...
    // Mint Mapping
    mapping(uint256 => uint256) public _CARD_ID_MAPPING;
    mapping(uint256 => uint256) public _CARD_ID_MINTED;
    ...
    // Card Random Mapping
    // mapping (uint256 => uint256) private _CARD_ID_MAPPING;
    ...
    { 
        _CARD_ID_MAPPING[1] = 1; // Lagos Full Miner
        _CARD_ID_MAPPING[2] = 3; // Lagos 4 Node
        _CARD_ID_MAPPING[3] = 5; // Lagos 3 Node
        _CARD_ID_MAPPING[4] = 7; // Lagos 2 Node
        _CARD_ID_MAPPING[5] = 9; // Lagos 1 Node
        _CARD_ID_MAPPING[6] = 40; // Miami
        _CARD_ID_MAPPING[7] = 40; // NYC
        _CARD_ID_MAPPING[8] = 60; // Beijing
        _CARD_ID_MAPPING[9] = 60; // Shanghai
        _CARD_ID_MAPPING[10] = 60; // Hong Kong
        _CARD_ID_MAPPING[11] = 90; // Mumbai
        _CARD_ID_MAPPING[12] = 90; // New Delhi
        _CARD_ID_MAPPING[13] = 90; // Kolkata
        _CARD_ID_MAPPING[14] = 100; // Zurich
        _CARD_ID_MAPPING[15] = 100; // Geneva
        _CARD_ID_MAPPING[16] = 100; // Basel
        _CARD_ID_MAPPING[17] = 150; // Lima
        _CARD_ID_MAPPING[18] = 150; // Cusko
        _CARD_ID_MAPPING[19] = 150; // Arequipa
        _CARD_ID_MAPPING[20] = 250; // Istanbul
        _CARD_ID_MAPPING[21] = 250; // Ankara
        _CARD_ID_MAPPING[22] = 250; // Izmir
        _CARD_ID_MAPPING[23] = 300; // Davao
        _CARD_ID_MAPPING[24] = 300; // Manila
        _CARD_ID_MAPPING[25] = 300; // Bohol
        _CARD_ID_MAPPING[26] = 400; // Saigon
        _CARD_ID_MAPPING[27] = 400; // Hanoi
        _CARD_ID_MAPPING[28] = 200; // Coinbase
        _CARD_ID_MAPPING[29] = 200; // Binance
        _CARD_ID_MAPPING[30] = 200; // Gemini
        _CARD_ID_MAPPING[31] = 200; // Kraken
        _CARD_ID_MAPPING[32] = 200; // Solar
        _CARD_ID_MAPPING[33] = 200; // Wind
    }
    ...
    function MoonopolyMint(uint numberOfTokens) public payable nonReentrant
    {
        ...
        {
            uint256 cardID = _drawCard();
            _CARD_ID_MINTED[cardID] += 1;
            _CARDS_MINTED += 1;
    ...
    function MintCollection(uint256 numberOfTokens) external onlyOwner
    {
        require(_CARDS_MINTED + numberOfTokens <= _MAX_CARDS, "Cannot Mint More Than Max Cards");
    ...
    function _drawCard() internal virtual returns (uint256 id) 
    {
        for (uint256 i = 0; i < 5; i++) 
        {
            uint256 cardID = _pseudoRandom(i + _CARDS_MINTED);
            if (_canMint(cardID)) { return cardID; }
        }

        // If Pseudorandom Card Is Not Valid After 5 Tries, Draw From Top Of The Stack
        return _drawAvailableCard();
    }

     function _drawAvailableCard() private view returns (uint256 id) 
     {
        for(uint256 i = _UNIQUE_CARDS; i > _MINIMUM_CARD_INDEX; i--)
        {
            if(_canMint(i)) { return i; }
        }
    }
    
    function _canMint(uint256 cardID) private view returns (bool) { return (_CARD_ID_MAPPING[cardID] > _CARD_ID_MINTED[cardID] && _CARD_ID_MAPPING[cardID] > 0); }

    function _pseudoRandom(uint256 salt) private returns (uint256) 
    {
            ...
            1 + uint256(
                    keccak256(
                        abi.encodePacked(
                            salt,
                            block.timestamp,
                            blockhash(block.difficulty % salt),
                            block.number,
                            _randomSeed,
                            'MOONOPOLY',
                            'WEN MOON?',
