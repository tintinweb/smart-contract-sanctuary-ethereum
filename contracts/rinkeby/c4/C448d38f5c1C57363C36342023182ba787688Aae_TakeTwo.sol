...
contract TakeTwo is ERC721A, Ownable {
    ...
    mapping(address => uint256) private _presaleMintsLeft;
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(_numberMinted(msg.sender) + quantity <= 4, "Take Two: Exceeds Max Per Wallet");

        if(state == Status.PRESALE_LIVE) {
            require(msg.value >= presalePrice * quantity, "Take Two: Insufficient ETH");
            require(quantity <= _presaleMintsLeft[msg.sender], "Take Two: Insufficient Presale Mints Left");
        } else {
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            _presaleMintsLeft[addresses[i]] = numAllowedToMint;
        }
    }
