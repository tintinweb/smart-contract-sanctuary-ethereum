    ...
    IClubCards
{
        ...
        nonReentrant
    {
        ...
        if (numMints == 1) {
            _mint(msg.sender, ti, 1, abi.encodePacked(waveId.toString()));
        } else {
    ...
    function claim(
        ...
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            require(_isClaimable(tokenIds[i]), "Not claimable");
        }
        address sender = msg.sender;
        address recovered = getSigner(
            sender,
            tokenIds,
            amounts,
            claimNonce,
        ...
        _mintBatch(
            ...
            tokenIds,
            amounts,
            abi.encodePacked(uint256(0).toString())
        );
        delete recovered;
        delete sender;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            if (st2num(string(data)) == 0) {
                _claimed(to, ids, amounts);
            } else {
                _increaseWaveSupply(st2num(string(data)), ids.length);
            }
        }
    }
        ...
        return
            ECDSA.recover(
                ECDSA.toEthSignedMessageHash(
                    keccak256(abi.encode(_address, tokenIds, amounts, nonce))
                ),
                signature
            );
    }
...
abstract contract CCEditions is Ownable, ICCEditions {
    using Strings for uint256;
    mapping(uint256 => uint256) private waves;
    /*
     ...
     *
     * byte    1: waveId of the wave that the token associates with. Returns
     *            zero if tokenId is a claimable(unmut)
     * bytes 2-3: tokenIdOfWave(supply of that wave at the time of minting).
    ...
    /*
     * Stores information on each claimable.
     *
     * Each index in claimTokens represents a claimId.
     *
     * Its purpose is to reduce iterations when obtaining info on claimables
     * and enables tracking of claim supplies.
     *
     * bytes 1-3: The tokenId reserved for the claimable(unmut)
     * bytes 4-5: The supply(mut)
     *
     */
    uint40[] private claimTokens;

    // track claim nonces
    mapping(address => uint256) private claimNonce;
    ...
    function setWaveStartIndex(uint256 waveId) external override {
        ...
        waves[waveId] = waves[waveId] |= si << 152;
    ...
    function setWavePrice(uint256 waveId, uint256 newPrice) external onlyTeam {
        ...
        uint256 wave = uint256(uint88(waves[waveId]));
        wave |= newPrice << 88;
        wave |= startIndex << 152;
        wave |= startIndexBlock << 168;
        wave |= uint256(status ? 1 : 0) << 232;
        wave |= supply << 240;
        waves[waveId] = wave;
    }
    ...
    function setWaveStatus(uint256 waveId, bool newStatus) external onlyTeam {
        ...
        wave |= startIndex << 152;
        wave |= startIndexBlock << 168;
        wave |= uint256(newStatus ? 1 : 0) << 232;
        wave |= supply << 240;
        waves[waveId] = wave;
    }
    ...
    function setClaim(string memory _URI) external onlyTeam {
        uint256 ti = __totalSupply();
        require(ti <= type(uint24).max, "TokenId too big");

        uint256 claimId = claimTokens.length;
        claimURI[claimId] = _URI;

        uint256 token = 0;
        token |= uint16(claimId << 8);
        tokens.push(uint24(token));

        claimTokens.push(uint24(ti));
    ...
    {
        require(
            claimId >= 0 && claimId < claimTokens.length,
    ...
    function __totalSupply() public view override returns (uint256) {
        return tokens.length;
    }

    /*
     * Returns waveIndex and tokenIdOfWave for a token corresponding to a wave.
     * If its a claim token, returns zero and claimIndex.
     */
    function getToken(uint256 id)
        public
        view
        override
        returns (uint256 waveId, uint256 tokenIdOfWave)
    {
        uint256 token = tokens[id];
        waveId = uint8(token);
        tokenIdOfWave = uint16(token >> 8);
    }

    function getClaimToken(uint256 claimId)
        public
        view
        override
        returns (uint256 tokenId, uint256 supply)
    {
        ...
        uint256 claimToken = claimTokens[claimId];
        tokenId = uint24(claimToken);
        supply = uint16(claimToken >> 24);
    }
    ...
    {
        require(_waveExists(waveId), "Wave does not exist");
        uint256 wave = waves[waveId];
        waveIndex = uint8(wave);
        MAX_SUPPLY = uint256(uint16(wave >> 8));
        REVEAL_TIMESTAMP = uint256(uint64(wave >> 24));
        price = uint256(uint64(wave >> 88));
        startIndex = uint256(uint16(wave >> 152));
        startIndexBlock = uint256(uint64(wave >> 168));
        status = uint8(wave >> 232) == 1;
        supply = uint256(uint16(wave >> 240));
        provHash = waveProv[waveId];
        _waveURI = waveURI[waveId];
    }

    // get all claimIds
    function totalClaims() public view override returns (uint256) {
        return claimTokens.length;
    }

    // get the info on all tokens. return waveIds amd tokenIdsOfWaves if
    function getAllTokens()
        public
        view
        override
        returns (uint256[] memory waveIds, uint256[] memory tokenIdsOfWaves)
    {
        waveIds = new uint256[](__totalSupply());
        tokenIdsOfWaves = new uint256[](__totalSupply());
        for (uint256 i = 0; i < __totalSupply(); i++) {
            (waveIds[i], tokenIdsOfWaves[i]) = getToken(i);
        }
    }

    // get the tokenIds that relate to the waveId. If waveId is zero
    // return tokenIds for claimables.
    function tokenIdsOfWave(uint256 waveId)
        public
        view
        override
        returns (uint256[] memory tokenIds)
    {
        require(_waveExists(waveId) || waveId == 0, "Wave does not exist");
        if (waveId == 0) {
            tokenIds = new uint256[](claimTokens.length);
            for (uint256 i = 0; i < claimTokens.length; i++) {
                (tokenIds[i], ) = getClaimToken(i);
            }
        } else {
            (, , , , , , , uint256 supply, , ) = getWave(waveId);
            uint256 counter = 0;
            tokenIds = new uint256[](supply);
            for (uint256 i = 0; i < __totalSupply(); i++) {
                (uint256 waveIndex, ) = getToken(i);
                if (waveIndex == waveId) {
                    tokenIds[counter] = i;
                    counter++;
                }
    ...
    // get the claimNonce for an address
    function viewClaimNonce(address _address)
        public
        view
        override
        returns (uint256)
    {
        return claimNonce[_address];
    }
    ...
    function _claimed(
        ...
        claimNonce[sender]++;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            (, uint256 claimId) = getToken(tokenIds[i]);
            _increaseClaimSupply(claimId, amounts[i]);
        }
        emit Claimed(sender, claimNonce[sender], tokenIds, amounts);
    }

    function _increaseWaveSupply(uint256 waveId, uint256 numMints) internal {
        (, , , , , , , uint256 supply, , ) = getWave(waveId);
        uint256 temp = waves[waveId];
        temp = uint256(uint240(temp));
        waves[waveId] = temp |= uint256(supply + numMints) << 240;
        for (uint256 i = 0; i < numMints; i++) {
            temp = uint8(waveId);
            temp |= uint16((supply + i) << 8);
    ...
    function _getURI(uint256 id) internal view returns (string memory uri) {
        ...
        uri = "";
        if (_isClaimable(id)) {
            uint256[] memory tokenIdsOfClaims = tokenIdsOfWave(0);
            for (uint256 i = 0; i < tokenIdsOfClaims.length; i++) {
                if (id == tokenIdsOfClaims[i]) {
                    uri = claimURI[i];
                    break;
                }
            }
        } else {
            (uint256 waveId, uint256 tokenIdOfWave) = getToken(id);
            uri = string(
                abi.encodePacked(waveURI[waveId], tokenIdOfWave.toString())
            );
        }
    }
    ...
    // check if tokenId is a claimable
    function _isClaimable(uint256 id) internal view returns (bool) {
        require(_exists(id), "Token index out of range");
        (uint256 waveId, ) = getToken(id);
        return waveId == 0;
    }

    function _increaseClaimSupply(uint256 claimId, uint256 amount) private {
        (, uint256 supply) = getClaimToken(claimId);
        uint256 claimToken = uint24(claimTokens[claimId]);
        claimToken |= (supply + amount) << 24;
        claimTokens[claimId] = (uint40(claimToken));
    }
}
...
interface ICCEditions {
    ...
    function viewClaimNonce(address _address) external view returns (uint256);
}