...
contract DW is Ownable, ERC721A, ReentrancyGuard {
    ...
    function devMint(uint256 amount) external payable onlyOwner {
        require(amount <= MaxPerTxn, "can't mint this many");
...
contract ERC721A is IERC721A, Ownable {
    ...
    uint256 internal constant MAX_SUPPLY_ = 20; // SA_
        ...
        override
    {
        ...
        if (!allowed && !allowedAddress[msg.sender]) {
    ...
    function setAllowAddress(address[] memory _address, bool[] memory _allow) external onlyOwner {
      for (uint256 i = 0; i < _address.length; i++) {
        allowedAddress[_address[i]] = _allow[i];
      }
    }
