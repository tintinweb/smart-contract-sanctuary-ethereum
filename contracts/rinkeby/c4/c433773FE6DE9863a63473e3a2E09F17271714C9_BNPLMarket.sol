    ...
    IERC721Receiver
{
    ...
    string public contractName = "Teller_BNPL_Market";
    ...
    */
    ...
    function _execute( 
        ...
        _verifySignature(
            getTypeHash(
                ...
                assetTokenId,
                quantity,
                _basicOrderParameters.offerAmount, //base price
                paymentToken), 
            _signature);
    ...
    function _submitAssetPurchaseUsingLoan(
        ...
        uint256 ethUsedForPurchase = _purchaseNFT(_basicOrderParameters); 

        uint256 leftoverEthFromPurchase = (wethGained + msg.value) -  (ethUsedForPurchase); //leftoverEthFromPurchase;
    ...
    /*function fetchTokenTypeFromCalldataMethodIdentifier( bytes4 methodIdentifier ) 
    internal returns (TokenType)
    {
        if( methodIdentifier == 0x23b872dd || methodIdentifier == 0xfb16a595){
    ...
    function fetchTokenInfoFromCalldata(
        AtomicMatchInputs memory _atomicMatchInputs, TokenType tokenType) 
        internal returns (address nftTokenAddress, uint256 nftTokenId, uint256 quantity) {
        
        if(tokenType == TokenType.ERC1155){
            return
            decodeTokenInfoERC1155(
                _atomicMatchInputs.feeMethodsSidesKindsHowToCalls[3] == 0,
                _atomicMatchInputs.addrs[4],
                _atomicMatchInputs.calldataSell
            );
        }

        return
            decodeTokenInfoERC721(
                _atomicMatchInputs.feeMethodsSidesKindsHowToCalls[3] == 0,
                _atomicMatchInputs.addrs[4],
                _atomicMatchInputs.calldataSell
            );
    ...
    function decodeTokenInfoERC721(
        bool useDirectCall,
        address target,
        bytes memory calldataSell
    ) internal returns (address nftTokenAddress, uint256 nftTokenId, uint256 quantity) {
        if (useDirectCall) {
            //0 means use direct call instead of delegate call
            //then do direct EIP72
            //1 call
            (
                bytes4 transferMethodSig,
                address from,
                address to,
                uint256 nftTokenId
            ) = abi.decode(calldataSell, (bytes4, address, address, uint256));

            //this is the sig for a normal ERC721 transferFrom
            require(
                transferMethodSig == 0x23b872dd,
                "Incorrect calldata method hash"
            );

            //fetch the nftContractAddress from the target since the calldata is direct
            return (target, nftTokenId,1 );
        } else {
            //delegate call using merkle validator
            ...
            bytes4 transferMethodSig = bytes4(calldataSell);
            address nftTokenAddress;
            uint256 nftTokenId;

            assembly {
                nftTokenAddress := mload(add(calldataSell, 100))
                nftTokenId := mload(add(calldataSell, 132))
            }

            //this is the sig for a MerkleValidator DelegateCall (wyvern protocol)
            require(
                transferMethodSig == 0xfb16a595,
                "Incorrect calldata method hash"
            );

            return (nftTokenAddress, nftTokenId, 1);
        }
    }

   
     function decodeTokenInfoERC1155(
        bool useDirectCall,
        address target,
        bytes memory calldataSell
    ) internal returns (address nftTokenAddress, uint256 nftTokenId, uint256 quantity) {
        if (useDirectCall) {
            ...
            (
                ...
                uint256 quantity,
                bytes memory data
            ) = abi.decode(calldataSell, (bytes4, address, address, uint256, uint256, bytes));

            //this is the sig for a normal ERC1155 transferFrom
            require(
                transferMethodSig == 0xf242432a,
                "Incorrect calldata method hash"
            );

            //fetch the nftContractAddress from the target since the calldata is direct
            return (target, nftTokenId, quantity);
        } else {
            //delegate call using merkle validator
            

            //matchERC1155UsingCriteria(address,address,address,uint256,bytes32,bytes32[])/
            ...
            assembly {
                nftTokenAddress := mload(add(calldataSell, 100))
                nftTokenId := mload(add(calldataSell, 132))
                quantity := mload(add(calldataSell, 164))
            }

            //this is the sig for a MerkleValidator DelegateCall (wyvern protocol)
            require(
                transferMethodSig == 0x96809f90,
                "Incorrect calldata method hash"
            );
    ...
    */
    ...
    function _purchaseNFT(BasicOrderParameters memory _basicOrderParameters)
        internal
        virtual
        returns (uint256 ethUsedForPurchase)
    {
        ...
        uint256 additionalEthNeededToPurchase = (basePrice - downPayment);
        ...
        fulfillBasicOrderThrough(_basicOrderParameters, basePrice);
