...
contract Lumens is ERC721A, Ownable, PaymentSplitter {
    ...
    uint16 public currentStageNFTsAmount;

    uint16 private constant vipNFTs = 140;
    uint16 private constant tierOneNFTs = 4500;
    uint16 private constant tierTwoNFTs = 2000;
    uint16 private constant reservedNFTs = 237;
    uint16 private constant publicMintingNFTs = 2011;
    ...
    function mintTo(address to) external payable {
        ...
        require(currentStage == 4, "The public sale has not started yet.");
    ...
    function _internalMint(address to, uint8 amount) internal {
        ...
        unchecked {
            require(currentStageNFTsAmount + amount <= getNFTAmountByStage(true), "Mint limit reached");
            require(
                alreadyMinted[msg.sender] + amount <= getNFTAmountByStage(false),
                "Amount requested is over max amount per wallet"
            );
            alreadyMinted[to] += amount;
            currentStageNFTsAmount += amount;
        }
    ...
    function getNFTAmountByStage(bool max) private view returns (uint16) {
        uint8 stage = currentStage;
        // VIP NFTs
        if (stage == 1) return max ? vipNFTs : 6;
        // Tier 1 NFTs
        else if (stage == 2) return max ? tierOneNFTs : 4;
        // Tier 2 NFTs
        else if (stage == 3) return max ? tierTwoNFTs : 2;
        // Public Minting NFTs
        else if (stage == 4) return max ? publicMintingNFTs : 10;
        else return 0;
    }
}
