...
contract MetaBolts is ERC721Enumerable, ERC721Burnable, ERC721Pausable, Ownable
{
	...
	function purchase(uint256 quantity) external payable
	{
		...
		require(msg.value >= _pricePerToken * quantity, "Ether sent is not correct");
		require(!paused(), "Sale paused");

		uint256 supply = totalSupply();
		require(supply + quantity <= _maxTokenSupply, "No more tokens");
	...
	function withdrawAll() external payable onlyOwner()
	{
		...
		require(payable(owner()).send(all));
	}
	...
	function raiseCap() external onlyOwner()
	{
