...
contract ERC1155Sale is ReentrancyGuard, Ownable {
    ...
    IIkonicToken private ikonicTokenContract;
     ...
     */
     function sell(
        ...
        require(isCurrencyValid(_currencyId), "ERC1155Sale.sell: Currency is not supported");
        for (uint256 index = 0; index < _accCurrencyIds.length; index++) {
            require(isCurrencyValid(_accCurrencyIds[index]), "ERC1155Sale.sell: Currency is not supported");
        }
    ...
    function distributePayment(
        ...
        if (_token.supportsInterface(_INTERFACE_ID_FEES)) {
            uint256 sumAffFee = distributeAffiliateFee(_token, _tokenId, tokenPrice, _currencyId);
    ...
    function distributeAffiliateFee(
        IIkonicERC1155Token _token,
        uint256 _tokenId,
        uint256 _price,
        uint8 _currencyId
    ...
    */
    ...
    function getCurrencyName(uint8 currencyIndex) external view returns (string memory) {
        require(isCurrencyValid(currencyIndex), "ERC1155Sale.getCurrencyName: Currency is not supported");
        return supportCurrencyName[currencyIndex];
    }

    function addSupportCurrency(string memory newCurrencyName, address newCurrencyAddress) external onlyOwner {
        uint8 i = 0;
        uint256 curLength = supportCurrencyName.length;
        for (i = 1; i < curLength; i++) {   
            require(supportCurrencies[i] != newCurrencyAddress, "ERC1155Sale.addSupportCurrency: This currency already exists");
        }
        supportCurrencies[i] = newCurrencyAddress;
        supportCurrencyName.push(newCurrencyName);
    }
    
    function updateCurrencyAddress(uint8 currencyIndex, address currencyAddress) external onlyOwner {
        require(isCurrencyValid(currencyIndex), "ERC1155Sale.updateCurrencyAddress: Currency is not supported");
        supportCurrencies[currencyIndex] = currencyAddress;
    }

    function isCurrencyValid(uint8 currencyIndex) public view returns (bool) {
        return (currencyIndex == 0 || supportCurrencies[currencyIndex] != address(0x0)) ? true : false;
    }
