...
contract BlueBay is ERC721A, Ownable {
    ...
    uint256 public initPrice          =  0.001 ether;  // 1e17
    ...
    function reserve(uint256[] memory _tokenIds, bytes32[] memory _merkleProof) public payable {
        ...
        require(currentPrice() > 0 && currentPrice() < 0.002 ether, "Not suitable minting price");
        ...
        if (msg.sender != owner()) {
            ...
            if (MerkleProof.verify(_merkleProof, merkleRoot, sender)) {
                ...
                uint256 temp;
                if (reservedNumOfWL[msg.sender] < 2) {
                    temp = 2- reservedNumOfWL[msg.sender];
                    if (_tokenIds.length <= temp) {
                        reservedNumOfWL[msg.sender] += _tokenIds.length;
                        require(msg.value >= (currentPrice() - 0.0002 ether) * _tokenIds.length, "Insufficient funds");
                    } else {
                        reservedNumOfWL[msg.sender] += temp;
                        require(msg.value >= (currentPrice() - 0.0002 ether) * temp + currentPrice() * (_tokenIds.length - temp), "Insufficient funds");
                    }
                } else {
    ...
    function currentPrice() public view returns (uint256) {
        ...
        if (startTime <= 0) {
            ...
            if (block.timestamp - startTime > 36000 * 10) {
                price = 0.002 ether;
            } else {
                for (uint256 i = 0; i < 10; i++) {
                    if (block.timestamp - startTime > 36000 * i && block.timestamp - startTime <= 36000 * (i + 1)) {
                        price = initPrice + i * 1e18 / 10000;
                    }
                }
            }
        }
