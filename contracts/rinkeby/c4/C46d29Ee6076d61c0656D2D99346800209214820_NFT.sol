...
contract NFT is /*INFT,*/ ERC721A, Operators {

    IMetadata public metadataContract;

    event TypeChange(uint tokenId, uint8 newType);

    constructor(IMetadata _metadataContract)
        ERC721A('Propertiez', 'NFT')
    {
    ...
    function _lastTokenId() internal view returns (uint) {
    ...
    function mintRoom(address to, uint count) external onlyOperator {
        _mint(to, count);
    }

    function mintWithType(address to, uint8 _type) external onlyOperator {
        _mint(to, 1);
        _changeType(_lastTokenId(), _type);
    }

    function _changeType(uint tokenId, uint8 newType) internal {
        _ownerships[tokenId]._type = newType;
        emit TypeChange(tokenId, newType);
    }
    ...
    function getTokenInfo(uint tokenId)
        external view returns (uint8 estateType)
    {
        require(_exists(tokenId), "query for nonexistent token");
        estateType = _ownerships[tokenId]._type;
    }
