...
contract Minter is Context, Ownable, Pausable {
    ...
    /// generates a random unique id from a given pool.
    /// param _to The address that will receive the NFT.
    /// param _tokenId The ID of the NFT.

    function mintNft() public payable {
        /// @notice check if the pool is empty
        // Make sure that the pool is not at 1.
        if (pool.length == 1) {
            revert PoolSupplyDepleated();
        }

        /// @notice create token reference
        NFTToken Token = NFTToken(token);

        // Generate a psudo random TokenId from pool
        uint8 randomTokenId = uint8(
            (block.timestamp + block.difficulty) % (pool.length - 1)
        );

        // move tokenId to the end of the pool
        uint8 tokenId = pool[randomTokenId];
        pool[randomTokenId] = pool[pool.length - 1];

        // remove the last item from pool
        pool.pop();

        Token.externalMint(msg.sender, tokenId, baseUri);
    }
