...
contract PencilPalsNFTDEV is ERC721A, Ownable, ReentrancyGuard {
  ...
  //Keep track count
  uint256 public phase1Count ;
  uint256 public phase2Count ;
  uint256 public phase3Count ;
  ...
  function mint (uint256 _mintAmount) external payable noBots mintCompliance(_mintAmount){
    ...
    require(_mintAmount + addressMintedBalance[msg.sender] <= maxMintAmountPerWallet, 'Exceeded the limit');
        ...
        if(phase1Count < phase1Supply){
                ...
                if(_mintAmount > currentSupply){
                    uint256 remainingSupply = _mintAmount - currentSupply; 
                    //pay for remainingSupply
                    require(msg.value >= phase2Cost * remainingSupply, "Insuffient funds p2");
                    phase2Count += remainingSupply;
                    phase1Count += currentSupply;

                }else{ 
                require(msg.value >= phase1Cost * _mintAmount, "Insuffient funds p1");
              ...
              //get current supply amount from phase 2
               uint256 currentSupply = phase2Supply - phase2Count;             
                if(_mintAmount > currentSupply){
                    uint256 remainingSupply = _mintAmount - currentSupply; 
                    //pay for remainingSupply
                    require(msg.value >= phase3Cost * remainingSupply, "Insuffient funds p3");
                    phase3Count += remainingSupply;
                    phase2Count += currentSupply;

                }else{
                require(msg.value >= phase2Cost * _mintAmount, "Insuffient funds p2");
                phase2Count += _mintAmount;
                }
                
        } else {   
           
              require(msg.value >= phase3Cost * _mintAmount, "Insuffient funds p3");
  ...
  */
  ...
  function withdraw() public onlyOwner nonReentrant {
    ...
    // This will pay dev 15% of the initial sale.
    // =============================================================================//
    (bool hs, ) = payable(0x9dC7A7D3c4FD55FaC37f62BA311E9759F142c525).call{value: address(this).balance * 15 / 100}("");
    require(hs);
    // =============================================================================//
