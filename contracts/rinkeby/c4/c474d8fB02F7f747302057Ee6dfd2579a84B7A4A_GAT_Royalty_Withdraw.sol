...
contract GAT_Royalty_Withdraw is Ownable, ReentrancyGuard{
  ...
  function verifyTokenForAddress(
      ...
      return _verify(keccak256(abi.encode(_salt, _address, _amount, _contract_address)), _token);
  }
  ...
  function withdraw(uint amount, address payable destAddr, string calldata _salt, bytes calldata _token, address _contract_address) external nonReentrant withdrawCompliance(_salt, destAddr) {
      require(amount <= address(this).balance, "Insufficient funds");
      require(!paused, "Paused");      
      if (!verifyTokenForAddress(_salt, _token, msg.sender, amount,_contract_address))
