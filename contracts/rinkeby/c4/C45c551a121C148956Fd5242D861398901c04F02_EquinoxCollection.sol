...
contract EquinoxCollection is Base721 {
    constructor(address _signAddress)
        Base721("ENS NFT", "E-NFTs", _signAddress)
    {}
}
...
contract Base721 is Generic721, controllerPanel {
    ...
    function mintTest(uint64 _cardType) external onlyAllowed {
    ...
    function changeMintTime(
        uint64 _cardID ,uint128 _mintTime, uint128 _mintEndTime
    ) external onlyOwner{
...
abstract contract Generic721 is ERC721Enumerable {
    ...
    uint128 public currentTestSeries;
        ...
        returns (uint64, string memory)
    {
        for (uint64 i = 1; i < (CardTypeCount + 1); i++) {
            if (CardType[i].start <= _tokenID && _tokenID <= CardType[i].end) {
                //     console.log("Start %s " , CardType[i].start);
                //    console.log("End %s " , CardType[i].end);
                //    console.log("tokenID =  %s " , _tokenID);
                //     console.log("Name =  %s " , CardType[i].className);
                //     console.log("\n");
        ...
        )
    {
        ...
        uint128[] memory _mintTime = new uint128[](CardTypeCount);
    ...
    // REMOVE EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEe

    function setCurrentTestSeries(uint128 _number) public {
        require(_number!=0,'!0');
        require(_number < 13,'!<13');
        currentTestSeries = _number;
    }
