  ...
  ERC2981
{
    ...
    modifier onlyOperator() {
        require (operator == msg.sender, "ERC721: caller is not a Editor");
        _;
    }
    ...
    function transferURIEditor(address _newEditor) external onlyURIEditor returns(bool) {
    ...
    function updateTokenURI(uint256 _tokenId, string memory _newURI) external onlyURIEditor returns(bool) {
        _setTokenURI(_tokenId, _newURI);
        return true;
    }

    function mint(address _creator, string memory _tokenURI) external onlyOperator virtual nonReentrant returns(uint256 _tokenId) {
