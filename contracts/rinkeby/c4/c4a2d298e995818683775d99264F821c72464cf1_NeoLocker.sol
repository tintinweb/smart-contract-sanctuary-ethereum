...
import "./interfaces/IGenetixToken.sol";
...
contract NeoLocker is ReentrancyGuard, Ownable {
    ...
    function lock(uint256 tokenId) external nonReentrant {
        ...
        if (_getPendingReward(msg.sender) > 0 && !rewardPaused) {
            _harvestReward();
        }
    ...
    function _harvestReward() internal {
        ...
        IGenetixToken(genetix).mint(msg.sender, _getPendingReward(msg.sender));
        lastClaimed[msg.sender] = block.timestamp;
    }
}
...
contract NeoAnunnaki is ERC721A, Ownable, SignedAllowance, PaymentSplitter {
        ...
        returns (string memory)
    {
        ...
        if(neoLocker.lockedBy(_tokenId) == address(0)) {
            return
    ...
    /// @param nonce current nonce of a wallet
