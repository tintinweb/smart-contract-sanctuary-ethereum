...
contract GatorGang is ERC721Enumerable, Ownable {
     ...
     */
    event Withdraw(address indexed owner, address to, uint256 amount);
    ...
    constructor(string memory BASEURI) ERC721(TOKEN_NAME, TOKEN_SYMBOL) {
        ...
        publicSalePrice = 160000000000000000;
        _indicatorNFT = 0;
    ...
    function setPresaleMaxCounter(uint256 counter) external onlyOwner {
        ...
        require(
            counter <= maxTotalSupply - _tokenIds.current(),
            "setPresaleMaxCounter: could not bigger than remain tokens"
        );
        require(
            counter != presaleMaxCounter,
            "setPresaleMaxCounter: param not equal old"
        );
    ...
    function setPublicSaleMaxCounter(uint256 counter) external onlyOwner {
        ...
        require(
            counter <= maxTotalSupply - _tokenIds.current(),
            "setPresaleMaxCounter: amount could not bigger than reamain tokens"
        );
        require(
            counter != publicSaleMaxCounter,
            "setPublicSaleMaxCounter: param not equal old"
        );
    ...
    function setPresalePrice(uint256 price) external onlyOwner {
        ...
        require(price != presalePrice, "setPresalePrice: param not equal old");
        require(price != presalePrice, "setPresalePrice: param not equal old");
    ...
    function setPublicSalePrice(uint256 price) external onlyOwner {
        ...
        require(
            ...
            "setPublicSalePrice: param not equal old"
        );
    ...
    function setPublicSaleStatus(bool status) external onlyOwner {
        require(
            ...
            "setPublicSaleStatus: param not equal old"
        );
    ...
    function setPresaleStatus(bool status) external onlyOwner {
        require(
            ...
            "setPresaleStatus: param not equal old"
        );
    ...
    function presaleMint(uint256 amount) internal {
        ...
        emit PublicSale(msg.sender, amount, msg.value * amount);
    }
    ...
    function publicSaleMint(uint256 amount) internal {
        ...
        emit Presale(msg.sender, amount, amount * msg.value);
    }
