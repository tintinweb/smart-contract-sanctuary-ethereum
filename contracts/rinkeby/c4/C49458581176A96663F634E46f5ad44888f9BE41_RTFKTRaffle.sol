...
contract RTFKTRaffle is VRFConsumerBase {  

    constructor(address vrfCoordinator_, address linkToken_, uint256 vrfFee_, bytes32 vrfKeyHash_) VRFConsumerBase(vrfCoordinator, linkToken) {
    ...
    // Raffle based on tokenId entries. Each tokenId is one entry meaning the same token owner can win multiple slots
    ...
    function fulfillRandomness(bytes32, uint256 randomNumber) internal override {
        vrfRandomValue = randomNumber;
    }

    function getVrfSeed() external isAuthorizedOwner returns (bytes32) {
        require(LINK.balanceOf(address(this)) >= vrfFee, "SP:NOT_ENOUGH_LINK");
    ...
    function setVrfParams(uint256 fee, bytes32 keyHash, address cooridnatorAddress, address linkTokenAddress) public isAuthorizedOwner {
        vrfFee = fee;
        vrfKeyHash = keyHash;
        vrfCoordinator = cooridnatorAddress;
