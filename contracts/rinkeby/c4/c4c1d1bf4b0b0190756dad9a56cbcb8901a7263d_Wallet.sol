...
contract Wallet is Ownable  {
    ...
    event CallBytes(bytes callbytes);
    ...
    function encode1(string memory str) external view returns (bytes memory) {
        bytes memory callbytes = abi.encodeWithSignature(str);

        return callbytes;
    }

    function encode2(string memory str) external view returns (bytes memory) {
        bytes memory callbytes = bytes(str);
