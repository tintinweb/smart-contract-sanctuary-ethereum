...
contract BaseTokenTestWrapper is BaseToken {
    uint256 internal price_ = 0.09 ether;
    uint256 internal presalePrice_ = 0.055 ether; // price in wei
    ...
    function _maxTotalSupply() internal view override returns (uint256) {
    ...
    //
    function _presalePrice() internal view override returns (uint256) {
        return presalePrice_;
    }

    function setPresalePrice(uint256 _newPrice) external onlyOwner {
        presalePrice_ = _newPrice;
    }

    //
    function _rewardSharePresale() internal view override returns (uint256) {
        return rewardSharePresale;
    }

    function setRewardSharePresale(uint256 _rewardSharePresale) external onlyOwner {
        rewardSharePresale = _rewardSharePresale;
    }

    //
    function _rewardShare() internal view override returns (uint256) {
        return rewardShare;
    }

    function setRewardShare(uint256 _rewardShare) external onlyOwner {
        rewardShare = _rewardShare;
    }

    function setTraits(uint256 _tokenId, uint8 _material, uint8 _edging, uint8 _suit, uint16 _rank) external onlyOwner {
        baseTokenMainTraits_[_tokenId - 1].Material = _material;
        baseTokenMainTraits_[_tokenId - 1].Edging = _edging;
        baseTokenMainTraits_[_tokenId - 1].Suit = _suit;
        baseTokenMainTraits_[_tokenId - 1].Rank = _rank;
    }

    function setCombinationTraits(uint256 _startTokenId, uint8 _material, uint8 _edging, uint16 _rank) external onlyOwner {
        for (uint256 _tokenIdIncrementor = 0; _tokenIdIncrementor < 4; _tokenIdIncrementor++) {
            uint256 _currentTokenId = _startTokenId + _tokenIdIncrementor;
            baseTokenMainTraits_[_currentTokenId - 1].Material = _material > 0 ? _material: baseTokenMainTraits_[_currentTokenId].Material;
            baseTokenMainTraits_[_currentTokenId - 1].Edging = _edging > 0 ? _edging: baseTokenMainTraits_[_currentTokenId].Edging;
            baseTokenMainTraits_[_currentTokenId - 1].Suit = uint8(2 ** _tokenIdIncrementor);
            baseTokenMainTraits_[_currentTokenId - 1].Rank = _rank > 0 ? _rank: baseTokenMainTraits_[_currentTokenId].Rank;
        }
    }
...
contract BaseToken is IBaseToken, CombinableTokenBasis {
    ...
    uint256 public maxPresaleTotalSupply = 2_000;
