...
contract Amulet is IAmulet, ERC721A, IERC2981, Ownable, ReentrancyGuard {
  /* --------------------------------- ****** --------------------------------- */
  ...
  /*                                   ERRORS                                   */
  /* -------------------------------------------------------------------------- */
  error NotAmuletOwner();
  error AmuletNotFound();
  ...
  modifier onlyOwnerOf(uint256 tokenId) {
    if (msg.sender != ownerOf(tokenId)) revert NotAmuletOwner();
    _;
  }
  /* --------------------------------- MINTING -------------------------------- */
  ...
  /* -------------------------------------------------------------------------- */
  constructor(
  ) ERC721A("Amulet", "Amulet") {
  }

  /* ---------------------------------- ADMIN --------------------------------- */
  /// @notice Allows owner to set a merkle root for Whitelist.
  ...
  function mintWhitelist(bytes32[] calldata merkleProof)
    external
    payable
    onlyIfValidMerkleProof(_whitelistMerkleRoot, merkleProof)
    nonReentrant
  {
    emit MintingWhitelist();
  ...
  /* -------------------------------------------------------------------------- */
  /* ------------------------------- AMULET OWNERS ------------------------------ */
  /// @notice Toggles the off-chain renderer for your Amulet.
  /// @param tokenId The OKPC to toggle.
  function switchAmuletRenderer(uint256 tokenId) external onlyOwnerOf(tokenId) {
    useOffchainMetadata[tokenId] = !useOffchainMetadata[tokenId];
  }
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    if (!_exists(tokenId)) revert AmuletNotFound();
    return IAmuletMetadata(metadataAddress).tokenURI(tokenId);
  }
