...
contract DutchAuctionRefundable is Ownable, Pausable, ReentrancyGuard {
  ...
  /// @param maxSupplyAndTeamAlloc The max supply of NFT to be minted and the team allocation
  ...
  constructor(
    ...
    uint256[2] memory maxSupplyAndTeamAlloc,
    ...
    maxSupply = maxSupplyAndTeamAlloc[0];
    teamAllocation = maxSupplyAndTeamAlloc[1];
    whitelistMerkleRoot = _whitelistMerkleRoot;
    whitelistPercPrice = _whitelistPercPrice;
  }

  /// @notice Allows the owner to mint the team allocation
  /// @dev Only owner can call this method
  function teamMint() external onlyOwner {
    token.mint(teamAllocation, treasury);
  }
