...
contract Vault is IVault, IncreOwnable {
    ...
    */
    ...
    /// @return amountUSDValue USD value of the deposited amount because pools' vQuote are USD-based. 18 decimals
    function deposit(
        address user,
        uint256 amount,
        ...
        onlyClearingHouse
    {
        ...
        if (debtSize == 0) revert Vault_LiquidationDebtSizeZero();
        ...
        for (uint256 i = 1; i < collaterals.length; i++) {
            ...
            if (collateralBalance > 0) {
                ...
                uint256 collateralLiquidationValue = (
                    _getUndiscountedCollateralUSDValue(collaterals[i], collateralBalance).toUint256()
                ).wadMul(LIQUIDATION_DISCOUNT);
    ...
    function setClearingHouse(IClearingHouse newClearingHouse) external onlyGovernance {
    ...
    function setInsurance(IInsurance newInsurance) external onlyGovernance {
        if (address(newInsurance) == address(0)) revert Vault_InsuranceZeroAddress();
        insurance = newInsurance;
        emit InsuranceChanged(newInsurance);
    }

    function setOracle(IOracle newOracle) external onlyGovernance {
    ...
    function changeCollateralWeight(IERC20Metadata asset, uint256 newWeight) external onlyGovernance {
    ...
    ///         Useful as a risk mitigation measure in case one collateral drops in value
    function changeCollateralMaxAmount(IERC20Metadata asset, uint256 newMaxAmount) external onlyGovernance {
        uint256 tokenIdx = tokenToCollateralIdx[asset];
        if (!((tokenIdx != 0) || (address(asset) == address(UA)))) revert Vault_UnsupportedCollateral();
    ...
    function getReserveValue(address trader) external view override returns (int256) {
        return _getUserReserveValue(trader, false);
    }
    ...
    function getTotalValueLocked() external view override returns (int256) {
        ...
        for (uint256 i = 0; i < collaterals.length; i++) {
            ...
            if (collateralBalance > 0) {
                tvl += _getUndiscountedCollateralUSDValue(collaterals[i], collateralBalance);
            }
        }
    ...
    function _deposit(
        ...
        Collateral memory coll = whiteListedCollaterals[tokenIdx];
        ...
        return _getCollateralUSDValue(whiteListedCollaterals[tokenIdx], wadAmount.toInt256());
    }
        ...
        for (uint256 i = 0; i < collaterals.length; i++) {
            ...
            if (collateralBalance != 0) {
                if (isDiscounted) {
                    ...
                    reserveValue += _getUndiscountedCollateralUSDValue(collaterals[i], collateralBalance);
                }
            }
        }
        ...
        returns (int256)
    {
        ...
        int256 weightedCollateralBalance = collateralBalance.wadMul(collateral.weight.toInt256());
        int256 usdPricePerUnit = oracle.getPrice(address(collateral.asset));
    ...
    /// @dev Same as _getCollateralUSDValue, expect without weight
    /// @param collateral Collateral to evaluate
    /// @param collateralBalance Balance in the collateral. 18 decimals
    /// @return collateralUSDValue Value of the collateral in USD. 18 decimals
    function _getUndiscountedCollateralUSDValue(Collateral memory collateral, int256 collateralBalance)
        internal
        view
        returns (int256)
    {
