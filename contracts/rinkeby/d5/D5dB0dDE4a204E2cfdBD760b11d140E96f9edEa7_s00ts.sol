...
contract s00ts is ERC721A, ERC2981, Merkle{
  ...
  function mint( uint16 quantity, bytes32[] calldata proof ) external payable{
    ...
    uint8 pubState = uint8(SaleState.PUBLIC);
    uint8 wlState = uint8(SaleState.WHITELIST);
    if( cfg.saleState & wlState == wlState && proof.length > 0 ){
      require( owners[ msg.sender ].claimed + quantity <= cfg.maxClaim, "Claim limit exceeded" );
    ...
    else if( cfg.saleState & pubState == pubState ){
      //ok
      require( owners[ msg.sender ].purchased + quantity <= cfg.maxMint, "Mint limit exceeded" );
  ...
  function setConfig( MintConfig calldata config_ ) external onlyDelegates{
    ...
    require( uint8(config_.saleState) <= 3, "invalid sale state" );
