...
contract SajaToken is Context, IERC20, Ownable {
    ...
    string private _symbol = "SAJA TOKEN";
    ...
    uint256 public _maxBuyTxAmount = 1000000000000000 * 10**9;
    uint256 public _maxSellTxAmount = 1000000000000000 * 10**9;
    ...
    address public charityWallet = 0x800C3C9b2b132543917cbB5a54A6E99d47e39A7B;
    address public devWallet = 0x16ce829696166C382e1Dbbb5A1E23e4203Bd06D1;
    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    struct Distribution {
        uint256 distributionSharePercentage;
    ...
    constructor () {
        ...
        taxFees = TaxFees(false,false,2,8,1,10);
        distribution = Distribution(75, 80, 10, 10);
    ...
    function setMaxTxAmount(uint256 maxBuyTxAmount, uint256 maxSellTxAmount) external onlyOwner() {
    ...
    function setDistribution(uint256 marketingFeePercentage, uint256 charityFeePercentage, uint256 devFeePercentage,
        ...
        distribution.distributionSharePercentage = distSharePercentage;
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        ...
        if(taxFees.doTakeFees) {
            reflectionFee = taxFees.reflectionFee;
            if(taxFees.isSellTxn) {
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        ...
        return _amount.mul(taxFees.doTakeFees ? taxFees.liquidityFee : 0).div(10**2);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != uniswapV2Pair) {
            ...
            isSellTxn = to != uniswapV2Pair;
            sellTaxTokens();
        }
        if(from != owner() && to != owner() && to != deadWallet) {
            
            if(isSellTxn) {
                require(amount <= _maxSellTxAmount, "Transfer amount exceeds the max sell txn amount.");
            } else {
                require(amount <= _maxBuyTxAmount, "Transfer amount exceeds the max buy txn.");
            }
        }
        bool doTakeFees = !_isExcludedFromFee[from] && !_isExcludedFromFee[to] && from != owner() && to != owner();
        _tokenTransfer(from, to, amount, doTakeFees, isSellTxn);
    }
    ...
    function sellTaxTokens() private {
        ...
        if (contractTokenBalance >= numTokensSellToAddToLiquidity && !inSwapAndLiquify && swapAndLiquifyEnabled) {
            ...
            uint256 balanceToShareTokens = contractTokenBalance.mul(distribution.distributionSharePercentage).div(100);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool doTakeFees, bool isSellTxn) private {
        taxFees.doTakeFees = doTakeFees;
        taxFees.isSellTxn = isSellTxn;
