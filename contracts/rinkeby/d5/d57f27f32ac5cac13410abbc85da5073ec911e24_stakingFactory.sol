...
contract Smartchef is  Ownable, ReentrancyGuard {
    ...
    // The block number when vvt mining ends.
    uint256 public bonusEndBlock;

    // The block number when vvt mining starts.
    uint256 public startBlock;

    // The block number of the last pool update
    uint256 public lastRewardBlock;

    // vvt tokens created per block.
    ...
    uint256 public totalLockTime = 15 minutes;
    ...
    function deposit(uint256 _amount, uint256 _lockTime) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                rewardToken.safeTransfer(address(msg.sender), ((pending.mul(user.lockTime)).div(totalLockTime).mul(1e4)).div(1e4));
            }
        }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 vvtReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(vvtReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function pendingRewardoriginal(address _user) external view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        uint256 stakedTokenSupply = stakedToken.balanceOf(address(this));
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 vvtReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(vvtReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
        lastRewardBlock = block.number;
    }
...
contract Stake {
    ...
    uint256 public poolMaturityTime;
    ...
    bool public isEarlywithdrawAvailable;
    ...
    constructor(
        ...
        poolMaturityTime = startTime.add(rewardInterval);
        poolOpenTill = startTime.add(_poolOpenTill);
        isClaimable = false;
        isEarlywithdrawAvailable = false;
        earlyWithdrawreward = 0;
    }
    
    function setRewardRate(uint256 _rate) public onlyOperator {
        // require(msg.sender == operator, "Only operator");
        rewardRate = _rate;
    }

    function setHardcap(uint256 _hardcapValue) public onlyOperator {
        // require(msg.sender == operator, "Only operator");
        hardcap = _hardcapValue;
    }

    function setMinAmount(uint256 _amount) public onlyOperator {
        // require(msg.sender == operator, "Only operator");
        minAmount = _amount;
    }

    function setRewardInterval(uint256 _rewardInterval) public onlyOperator {
        // require(msg.sender == operator, "Only operator");
        rewardInterval = _rewardInterval;
    }
    ...
    function setOperator(address _operator) public onlyOperator {
     ...
     function setPoolMaturityTime(uint256 _time) public onlyOperator {
         if(poolMaturityTime < block.timestamp)
            poolMaturityTime = block.timestamp.add(_time);
        else poolMaturityTime = poolMaturityTime.add(_time);
    }

    function setPoolOpenTill(uint256 _time) public onlyOperator {
        require(poolMaturityTime > block.timestamp.add(_time), "Staking time should be smaller than maturity time");
        poolOpenTill = block.timestamp.add(_time);
        // poolOpenTill = poolOpenTill.add(_time);
    }

    function setIsClaimable(bool _isClaimable) public onlyOperator {
        // require(msg.sender == operator, "Only operator");
        isClaimable = _isClaimable;
    }

    function setIsEarlywithdrawAvailable(bool _isEarlywithdrawAvailable) public onlyOperator {
        // require(msg.sender == operator, "Only operator");
        isEarlywithdrawAvailable = _isEarlywithdrawAvailable;
    }

    function setEarlyWithdrawreward(uint256 _earlyWithdrawreward) public onlyOperator {
        // require(msg.sender == operator, "Only operator");
        earlyWithdrawreward = _earlyWithdrawreward;
    }

    function updateAccount(address account, bool _isEarlywithdrawAvailable) private {
        uint256 pendingDivs = getPendingDivs(account);
        if(pendingDivs > 0) {
        if (!_isEarlywithdrawAvailable) {
             require(
                IBEP20(rewardToken).transfer(account, pendingDivs),
        ...
        else if(_isEarlywithdrawAvailable) {
            if(earlyWithdrawreward > 0){
                uint fee = pendingDivs.mul(earlyWithdrawreward).div(1e4);
                require(
                    IBEP20(rewardToken).transfer(account, fee),
                    "Could not transfer tokens."
                );
                totalEarnedTokens[account] = totalEarnedTokens[account].add(
                    fee
                );
                totalClaimedRewards = totalClaimedRewards.add(fee);
            }
            emit RewardsTransferred(account, pendingDivs);
        }
    ...
    function deposit(uint256 amountToStake) public {
        ...
        if(poolOpenTill > 0){
            require(poolOpenTill > block.timestamp, "Pool closed");
        }
        require(totalStaked <= hardcap, "Total staked amount should be less than hardcap");
        require(
    ...
    function withdraw(uint256 amountToWithdraw) public {
        ...
        // );
        require(
             block.timestamp > poolMaturityTime,
            "Please wait for pool maturity."
        );

        updateAccount(msg.sender, false);

        require(
            IBEP20(depositToken).transfer(msg.sender, amountToWithdraw),
            "Could not transfer tokens."
        );
    ...
    function earlyWithdraw(uint256 amountToWithdraw) public {

        require(isEarlywithdrawAvailable, "Early withdraw is not available");
        require(
            depositedTokens[msg.sender] >= amountToWithdraw,
            "Invalid amount to withdraw"
        );
    ...
    function claimDivs() public {
        require(isClaimable, "You can't claim now");
        updateAccount(msg.sender, false);
    }
...
contract stakingFactory is Ownable{
    ...
    enum STAKING_TYPE{FIXEDAPY, SMARTCHEF}
    ...
    function createStack( address _depositTokens,address _rewardToken, uint256 _rewardRate, uint256 _hardcap, uint256 _rewardInterval, uint256 _poolOpenTill, address _operator) public onlyOwner returns(address){
        require(_rewardInterval > _poolOpenTill, "Staking time should be smaller than maturity time");
        Stake stake =  new Stake( _depositTokens, _rewardToken, _rewardRate, _hardcap, _rewardInterval, _poolOpenTill, _operator);
        stakeData[address(stake)].contractAddress = address(stake);
        stakeData[address(stake)].stakingType = STAKING_TYPE.FIXEDAPY;
        stakingAddresses.push(address(stake));
        return address(stake);
    }

    function createSmartchef(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        uint256 _rewardPerBlock,
        uint256 _startBlock,
        ...
        stakeData[address(smartChefAddress)].contractAddress = address(smartChefAddress);
        stakeData[address(smartChefAddress)].stakingType = STAKING_TYPE.SMARTCHEF;
        stakingAddresses.push(address(smartChefAddress));
    }
}