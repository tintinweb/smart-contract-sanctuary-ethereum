...
contract GAG is Context, IERC20, Ownable {
  ...
  address public anonxContractAddress;
  ...
  struct ReserveValues {
    ...
    uint256 reserveTransferAmountAnonx;
    uint256 reserveTransferAmountAnonxRoyalty; 
  }
  ...
  struct TransactionValues {
    ...
    uint256 anonxTax;
    uint256 anonxRoyalty;
  }

  constructor(
    address _anonxContractAddress,
    address _opsWallet
  ) {
    anonxContractAddress = _anonxContractAddress;

    // 50% backhole burn. The burn address receives reflect, further accelerating the token's deflationary scheme by allowing the token to be burned without having to worry about the burn address.

    uint256 blackHole = _totalSupply.div(2);

    // 30% of remaining tokens for presale purposes.
    uint256 presale = blackHole.mul(3).div(10);

    // 51% to deployer for LP addition 
    uint256 lp = blackHole.mul(51).div(100);

    // 18% to the DAO contract for DAO token minting
    uint256 anonx = blackHole.mul(18).div(100);
    ...
    _reserveTokenBalance[_anonxContractAddress] = anonx.mul(rate);
  ...
  function applyTaxx() public onlyOwner() {
    taxx = true;
  }
  function removeTaxx() public onlyOwner() {
    taxx = false;
  }

  /// @notice Change DAO contract address.
  function setAnonxAddress(address _anonxContractAddress) public onlyOwner() {
    anonxContractAddress = _anonxContractAddress;
  }
  ...
  function applyExternalTransactionTax(
    ...
    _reserveTokenBalance[anonxContractAddress] = _reserveTokenBalance[anonxContractAddress].add(reserveValues.reserveTransferAmountAnonx);
    emit Transfer(
      sender,
      anonxContractAddress,
      transactionValues.anonxTax
    );  
    _reserveTokenBalance[royaltyAddress] = _reserveTokenBalance[royaltyAddress].add(reserveValues.reserveTransferAmountAnonxRoyalty);
    emit Transfer(
      sender,
      royaltyAddress,
      transactionValues.anonxRoyalty
    );     
  }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 if (!taxx) {
    if (recipient == tokenPairAddress) {
      ...
      _circulatingTokenBalance[recipient] = _circulatingTokenBalance[recipient].add(transferAmount.div(10));
      ...
      _reserveTokenBalance[recipient] = _reserveTokenBalance[recipient].add(reserveValues.reserveAmount.div(50));
      _circulatingTokenBalance[recipient] = _circulatingTokenBalance[recipient].add(transferAmount.div(10));
      emit Transfer(
        sender,
        recipient,
  ...
  function _getTValues(
    ...
    transactionValues.anonxTax = transferAmount.mul(9).div(200);
    // 0.5% royalty.
    transactionValues.anonxRoyalty = transferAmount.div(200);
    // Net transfer amount to recipient
    transactionValues.netTransferAmount = transferAmount.sub(transactionValues.transactionFee).sub(transactionValues.anonxTax).sub(transactionValues.anonxRoyalty);
  ...
  function _getRValues(
    ...
    reserveValues.reserveTransferAmountAnonx = transactionValues.anonxTax.mul(currentRate);
    reserveValues.reserveTransferAmountAnonxRoyalty = transactionValues.anonxRoyalty.mul(currentRate);
    reserveValues.reserveTransferAmount = reserveValues.reserveAmount.sub(
      reserveValues.reserveFee
      ).sub(
        reserveValues.reserveTransferAmountAnonx
        ).sub(
          reserveValues.reserveTransferAmountAnonxRoyalty
        );
