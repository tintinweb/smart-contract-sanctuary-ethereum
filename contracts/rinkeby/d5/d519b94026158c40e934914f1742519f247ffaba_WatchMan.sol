...
contract WatchMan is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
...
//   uint256 public PRICE = 0.06 ether; 
  ...
  constructor() ERC721A("WatchMan","WTA") {
 ...
 function inChecklist(bytes32[] memory _proof, address _owner) public view returns (bool) {
        return MerkleProof.verify(_proof, listMerkleRoot, keccak256(abi.encodePacked(_owner)));
}


  function mint(uint256 quantity , bytes32[] calldata _proof) external payable callerIsUser {
    ...
    require(inChecklist(_proof, msg.sender), "You are not authorized to mint");
