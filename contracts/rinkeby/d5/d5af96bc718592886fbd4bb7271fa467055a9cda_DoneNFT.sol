...
contract DoneNFT is ERC721A, ERC2981, ReentrancyGuard, Ownable {
    ...
    uint256 public FREE_maxToken = 1;
    ...
    address defaultTreasury = 0x8e2f2c1790004703fd4e2440638d5e97C8E25879;
    
    constructor() ERC721A("Done NFT", "DNFT") {
    ...
    function mint(uint256 quantity) external payable callerIsUser {
        ...
        require(maxToken >= totalSupply() + quantity, "Exceeds max supply");
        require(tx.origin == _caller, "No contracts");
      

		if(freeMinted[msg.sender] == true){
		    require(maxPerTx < quantity , "Excess max per paid tx");
            require(quantity * price == msg.value, "Invalid funds provided, attention 1 free per wallet");
			countPay = countPay +quantity;
		}else if(FREE_maxToken > totalSupply() - countPay){
            require(maxPerTx >= quantity , "Excess max per tx");
            require((quantity - 1) * price == msg.value, "Invalid funds provided, attention 1 free per wallet"); 
            countPay = countPay + (quantity-1);
            countFree++;
		    freeMinted[msg.sender] = true;
        }else{
            require(maxPerTx >= quantity , "Excess max per paid tx");
            require(quantity * price == msg.value, "Max FREE reached, Invalid funds provided"); 
            countPay = countPay +quantity;
        }
