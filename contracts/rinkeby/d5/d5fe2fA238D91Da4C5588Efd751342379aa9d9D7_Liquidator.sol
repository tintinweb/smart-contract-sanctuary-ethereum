...
contract Liquidator is Owned, MixinSystemSettings, ILiquidator {
    ...
    function getLiquidationCallerForAccount(address account)
        external
        view
        returns (address)
    {
    ...
    function isLiquidationOpen(address account, bool isSelfLiquidation)
        external
        view
        returns (bool)
    {
        uint256 accountCollateralisationRatio = synthetix()
        ...
        if (!isSelfLiquidation) {
            ...
            if (
                _deadlinePassed(liquidation.deadline) && _hasEnoughSNX(account)
            ) {
                return true;
            }
            return false;
        ...
        rateNotInvalid("SNX")
    {
        ...
        uint256 accountsCollateralisationRatio = synthetix()
        .collateralisationRatio(account);
