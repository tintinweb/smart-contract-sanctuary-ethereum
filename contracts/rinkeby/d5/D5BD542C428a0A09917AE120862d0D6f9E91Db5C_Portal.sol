...
contract Portal is NonblockingLzApp {
    ...
    event Bridged(uint16 chainId, uint256 count);
    event Received(uint16 fromChainId, uint256 count);
    ...
    function bridge(
        ...
        // abi.encode() the payload with the current count
        ...
        (uint messageFee, ) = lzEndpoint.estimateFees(chainId, address(this), payload, false, adapterParams);
    ...
    function _nonblockingLzReceive(
        ...
        uint256 srcCount = abi.decode(_payload, (uint256));

        count++;

        emit Received(_srcChainId, srcCount);
    }
