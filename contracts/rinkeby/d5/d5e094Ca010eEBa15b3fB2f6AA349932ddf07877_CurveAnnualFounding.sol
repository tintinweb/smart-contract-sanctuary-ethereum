...
contract CurveAnnualFounding is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    uint256 public price = 0.1 ether; //CHANGE TO 2ETH
    ...
    modifier hasExpired() {
        ...
        if (msg.sender != owner()){
            ...
            require(
            (dateMinted[msg.sender] + (1 * 24 * 60 * 60)) > block.timestamp, //1 year 
    ...
    function timeTilExpire(address tokenHolder) public view hasExpired returns (uint256 timeLeft) {
        if (msg.sender == tokenHolder || msg.sender == owner()){
            if (dateMinted[msg.sender] > 0) {
                timeLeft =
                    (dateMinted[tokenHolder] + (1 * 1 * 24 * 60 * 60)) -
                    block.timestamp;
                return timeLeft;
            }
        }
