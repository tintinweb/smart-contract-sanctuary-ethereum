...
contract CollectiveVault is Ownable {
    // uint256 public resolvedCounter;
    IERC20 public XIV;
    ...
    function initialize(
        ...
        fees = 500;
        for (uint8 i = 1; i <= 3; i++) {
            counter[i][address(0)] = 1;
            counter[i][_btc] = 1;
            counter[i][_eth] = 1;
            slotPlan[i].minimumAmt = 5000e18;
            slotPlan[i].slot = 3600; //1 hour
            if (i == 1 || i == 2) {
                slotPlan[i].userlimit = 10;
            } else {
                slotPlan[i].userlimit = 2;
            }
        }
        XIV = IERC20(xiv);
        validTokens[address(0)] = true;
        validTokens[_btc] = true;
        validTokens[_eth] = true;
        finalPrice = 100;
    }
    ...
    function predict(
        ...
        require(
            _amount >= slotDetails.minimumAmt,
            "CV: Amount should be greater than zero"
        );
    ...
    function resolving(
        ...
        if (
            predictionDetails.status == false && predictionDetails.endTime > 0
        ) {
            if (predictionDetails.endTime > block.timestamp) {
                //call oracal for the final price;
                //uint finalPrice = 100;
                if (_predictionType == 2) {
                    //shared
                    resolvingShared(predictionDetails);
                } else {
                    ...
                    for (uint8 i = 0; i < predictionDetails.user.length; i++) {
                        ...
                        //checking the price
                        // if(userDetails.price == finalPrice){
                        //     // winners.push(user);
    ...
    function fundTransfer(
        ...
        // if(_predictionType == 2){ //shared
        //     uint8 c = 0;
        //     uint amt = amount/5;
        //     if(winners.length < 5){
        //         amt = amount/winners.length;
        //     }
        //     for(uint8 i=uint8(winners.length-1); i>=0;i--){
        //         if(c==5){
        //             break;
        //         }
        //         XIV.transfer(winners[i], amt);
        //         c++;
        //     }
        // } else {// solo , user-user
        for (uint8 i = 0; i < winners.length; i++) {
            ...
            //last winner
            if (winners[i] == winners[winners.length - 1]) {
                XIV.transfer(winners[winners.length - 1], amount);
    ...
    function resolvingShared(PredictionSlot storage _predictionDetails)
        internal
    {}
    ...
    //_temp final price
    function tempFinalPrice(uint256 _finalPrice) public onlyOwner {
        require(_finalPrice > 0, "CV: FinalPrice should be greater then zero");
        finalPrice = _finalPrice;
    }

    function setAdminFees(uint256 per) external onlyOwner {
