...
contract ConnectFour {
    ...
    function initializeGame() external payable returns (uint256) {
        require(
            ...
            "Connect Four: invalid bet amount"
        );

        uint256 gameID = gameIdCounter;

        Disc[42] memory emptyBoard;

        games[gameID].player1 = msg.sender;
        games[gameID].board = emptyBoard;
        games[gameID].betAmount = msg.value;
        games[gameID].status = Status.Initialized;
        games[gameID].isPlayer1Turn = true;
    ...
    function startGame(uint256 _gameId) external payable {
        require(
            ...
            "Connect Four: the game does not exist, or has already been started"
        );
        require(
            games[_gameId].player1 != msg.sender,
            "Connect Four: player 1 cannot be player 2"
        );
        require(
            games[_gameId].betAmount == msg.value,
            "Connect Four: must send the same wei as bet amount"
        );
    ...
    function playMove(uint256 _gameId, uint256 _col) external {
        ...
        require(
            ...
            "Connect Four: can only play a move if game has been started"
        );
        require(_col >= 0 && _col <= 6, "Connect Four: invalid column");
        require(
            msg.sender == game.player1 || msg.sender == game.player2,
            "Connect Four: con only be called by a player of the game"
        );
        require(
            game.isPlayer1Turn
                ? msg.sender == game.player1
                : msg.sender == game.player2,
            "Connect Four: it is the other players turn"
        );

        uint256 MAX_ROW = 5;

        for (uint256 row = 0; row <= MAX_ROW; row++) {
            uint256 gameIndex = boardIndex(_col, row);

            Disc rowDisk = game.board[gameIndex];
            if (rowDisk == Disc.Empty) {
                game.board[gameIndex] = game.isPlayer1Turn
                    ? Disc.Player1
                    : Disc.Player2;
                game.isPlayer1Turn = !game.isPlayer1Turn;
                break;
            } else if (row == MAX_ROW) {
                revert("Connect Four: column is full");
            }
        }
    }
    ...
    function claimReward(
        ...
        require(
            game.status == Status.Started,
            "Connect Four: Cannot claim the game does not exist, or has not started"
        );
        require(
            _startingWinDiscCol >= 0 && _startingWinDiscCol <= 6,
            "Connect Four: Column out of bounds"
        );
        require(
            _startingWinDiscRow >= 0 && _startingWinDiscRow <= 5,
            "Connect Four: Row out of bounds"
        );

        Disc callerDisc = game.player1 == msg.sender
            ? Disc.Player1
            : Disc.Player2; // TODO This leaves player 2 vulnerable for exploite.  I assume that is what is refered to in the messaging.

        uint256 currentColumn = _startingWinDiscCol;
        uint256 currentRow = _startingWinDiscRow;
        for (uint i = 0; i < 4; i++) {
            uint256 index = boardIndex(currentColumn, currentRow);

            require(
                game.board[index] == callerDisc,
                "Connect Four: four in a row must only contain the callers discs"
            );
        ...
        require(sent, "Connect Four: Failed to transfer bet to recipent");
    ...
    function boardIndex(uint256 _col, uint256 _row)
        ...
        returns (uint256)
    {
        require(_col >= 0 && _col <= 6, "invalid column");
        require(_row >= 0 && _row <= 5, "invalid row");
