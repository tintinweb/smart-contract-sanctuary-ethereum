...
contract cellNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    function _is_eukaryote(uint256 tokenId) external view returns (uint256) {
        return cells[tokenId].eukaryote;
    }

    function _get_cell_type(uint256 tokenId) external view returns (uint256) {
    ...
    function breed_n_prokaryotes(uint256 n_cells) external {
        ...
        while ((repro.cycles < n_cells) && (repro.loop_idx < user_cells.length)) {
            ...
            if (loop_cell.eukaryote == 0) {
                uint256 repro_outcome = SVGManip.breedProkaryote(loop_cell_id, loop_cell.generation);
                if (repro_outcome == 1) {
                    _burn(loop_cell_id);
