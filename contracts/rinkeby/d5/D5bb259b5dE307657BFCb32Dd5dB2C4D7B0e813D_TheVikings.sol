...
contract TheVikings is ERC721A, Ownable, ReentrancyGuard {
  ...
  modifier mintCompliance(uint256 _mintAmount) {
    require(_mintAmount > 0 && _mintAmount <= maxMintAmountPerTx, 'Ginnungagap mint amount!');
    require(totalSupply() + _mintAmount <= maxSupply, 'Max pain supply exceeded!');
    _;
  }
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    require(whitelistMintEnabled, 'The kingsLoot sale is not enabled!');
    require(!whitelistClaimed[_msgSender()], 'Raid already claimed!');
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), 'Ginnungagap!');
