...
contract vendingMachineDDRL is Ownable{
    ...
    //owner sets a price for the vote token
    function setVotePrice(uint _price) public onlyOwner {
        price = _price ;
    }
    ...
    function buyVotes(uint amountOfVotes) public payable returns (uint256){
        //amountTickets = _amountTickets;
        require(msg.value >= amountOfVotes * price * 1 wei, "Not enough Eth to buy the tickets");
        
        // check if the Vendor Contract has enough amount of tokens for the transaction
        uint256 vendorBalance = _DDRLToken.balanceOf(address(this));
        require(vendorBalance >= amountOfVotes, "Vendor contract has not enough tokens in its balance");
        
        // Transfer token to the msg.sender
        (bool sent) = _DDRLToken.transfer(msg.sender, amountOfVotes);
        require(sent, "Failed to transfer token to user");

        // emit the event
        emit BuyTickets(msg.sender, msg.value, amountOfVotes);
