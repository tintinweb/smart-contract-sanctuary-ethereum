...
contract LootBox is ERC721Enumerable, Ownable {
  ...
  event LootBoxBurned(uint256 tokenId, address burner);
  ...
  function _validateSignToRedeem(
    ...
    bytes32 BUYORDER_TYPEHASH = keccak256(
      "NFTBurn(uint256 tokenId,uint256 amount)"
    );

    bytes32 structHash = keccak256(
      abi.encode(BUYORDER_TYPEHASH, tokenId, amount)
    );
  ...
  function burnAndReveal(
    ...
    address signer = _validateSignToRedeem(tokenId, amount, signature);
    ...
    super._burn(tokenId);
    emit LootBoxBurned(tokenId, msg.sender);
  }
