...
contract ToysTest is ERC721A, Ownable, ReentrancyGuard {
  ...
  function mint(uint256 quantity) external payable noContracts {
    ...
    require(_saleStarted(price, saleStartTime), "Sale not started");
  ...
  function whitelistMint(bytes32[] calldata _merkleProof, uint256 quantity) external payable noContracts {
    SaleConfig memory config = saleConfig;
    uint256 price = uint256(config.price);
    ...
    require(_saleStarted(price, whitelistSaleStartTime), "Sale not started");
  ...
  function _saleStarted(uint256 priceWei, uint256 saleStartTime) internal view returns (bool) {
    return priceWei != 0 && saleStartTime != 0 && block.timestamp > saleStartTime;
  }
