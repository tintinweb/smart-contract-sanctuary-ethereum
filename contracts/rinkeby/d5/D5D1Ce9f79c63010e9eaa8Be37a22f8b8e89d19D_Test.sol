...
contract Test is Initializable {
    ...
    function getTestData() public pure returns(string memory data) {
        data = "This is test 1";
    }

    function getTestData2() public pure returns(string memory data) {
        data = "This is test 2";
    }

    function trySomethingElse() public pure returns(int256 data) {
        data = 3;
    }
    ...
    function getBalances() public view returns (int ETHBalance, int WBTCBalance, int DAIBalance) {
        ...
        ETHBalance = 1; //int(CETH.balanceOf(address(this)) * (CETH.exchangeRateStored()) / 1e18); //check for overflow
        WBTCBalance = 2; //int(CWBTC.balanceOf(address(this)) * (CWBTC.exchangeRateStored()) / 1e18) * 1e10; //WBTC only has 8 digits, scale up to match other units
        DAIBalance = 3; //int(CDAI.balanceOf(address(this)) * (CDAI.exchangeRateStored()) / 1e18);
    }
