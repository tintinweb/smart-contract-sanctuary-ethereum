...
contract Assets is Administration, NFTEvents {
    ...
    uint public stripperSupply = 999999;
...
contract StripperVille is Assets, ERC721 {
    ...
    function multiMint(address[] calldata to) external onlyOwner {
        require(strippersCount + to.length <= stripperSupply, "SUPPLY: qty exceeds total suply");
        for (uint i = 0; i < to.length; i++) {
            uint tokenId = strippersCount + 1;
            assets.push(Asset(tokenId, STRIPPER, 85 ether, 0, block.timestamp, "", true));
