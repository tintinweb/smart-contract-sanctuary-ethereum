...
contract cosmos is ERC721Enumerable, Ownable {
  using Address for address payable;

  uint256 public constant cosmos_PREMINT = 0;
  uint256 public constant cosmos_MAX = 10;
  uint256 public constant cosmos_MAX_PRESALE = 10;
  uint256 public constant cosmos_PRICE = 0 ether;
  uint256 public constant cosmos_PER_WALLET = 10;
  uint256 public constant cosmos_PER_WALLET_PRESALE = 1;
  ...
  address private _devAddress = 0xE7c7aC2e83e9327C019559b160593122e9a1f9bD;
  ...
  constructor(
    ...
    ERC721("CosMosTest", "cos")
  {
    _baseTokenURI = newBaseTokenURI;
    _contractURI = newContractURI;
    root = _root;

    _preMint(cosmos_PREMINT);
  }
  ...
  function presaleBuy(uint256 quantity, bytes32[] calldata proof) external payable {
    require(presaleLive && !saleLive, "cosmos: Presale not currently live");

    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(proof, root, leaf), "cosmos: Caller is not eligible for presale");

    require(totalSupply() + quantity <= cosmos_MAX_PRESALE, "cosmos: Quantity exceeds remaining tokens");
    require(quantity <= cosmos_PER_WALLET_PRESALE - addressToMinted[msg.sender], "cosmos: Wallet cannot mint any new tokens");
    require(quantity != 0, "cosmos: Cannot buy zero tokens");
    require(msg.value >= quantity * cosmos_PRICE, "cosmos: Insufficient funds");
  ...
  function buy(uint256 quantity) external payable {
    require(saleLive && !presaleLive, "cosmos: Sale is not currently live");
    require(totalSupply() + quantity <= cosmos_MAX, "cosmos: Quantity exceeds remaining tokens");
    require(quantity <= cosmos_PER_WALLET - addressToMinted[msg.sender], "cosmos: Wallet cannot mint any new tokens");
    require(quantity != 0, "cosmos: Cannot buy zero tokens");
    require(msg.value >= quantity * cosmos_PRICE, "cosmos: Insufficient funds");
  ...
  function setBaseURI(string calldata newBaseURI) external onlyOwner {
    require(!locked, "cosmos: Contract metadata is locked");
  ...
  function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
    require(_exists(tokenId), "cosmos: URI query for nonexistent token");
