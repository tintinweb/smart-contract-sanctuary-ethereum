...
contract TAC is ERC721A("Testing contract", "TAC") {
    ...
    uint256 public itemPriceToken = 100 ether;
    uint256 public itemPricePresaleToken = 50 ether;
    ...
    function purchasePresaleTokensPayable(uint256 _howMany)
        external
        payable
        ...
        tokensAvailable(_howMany)
    {
        ...
        require(
            token.balanceOf(msg.sender) >= _howMany * itemPricePresaleToken,
            "Not have enough balance"
        );
        
        token.transferFrom(msg.sender, address(this),_howMany * itemPricePresaleToken);
        allowlistClaimedBy[msg.sender] += _howMany;
    ...
    function setPricePresaleToken(uint256 _itemPricePresale) external onlyOwner {
        itemPricePresaleToken = _itemPricePresale;
    }
    ...
    // Purchase multiple NFTs at once
    function purchaseTokensPayable(uint256 _howMany)
        external
        payable
        tokensAvailable(_howMany)
    {
        ...
        tokensAvailable(_howMany)
    {
        require(isSaleActive, "Sale is not active");
        require(_howMany > 0 && _howMany <= 20, "Mint min 1, max 20");
        require(token.balanceOf(msg.sender) >= _howMany * itemPriceToken, "Try to send more ETH");
        token.transferFrom(msg.sender, address(this), _howMany * itemPriceToken);
        _safeMint(msg.sender, _howMany);
    }
    ...
    function setPriceToken(uint256 _newPrice) external onlyOwner {
        itemPriceToken = _newPrice;
    }
