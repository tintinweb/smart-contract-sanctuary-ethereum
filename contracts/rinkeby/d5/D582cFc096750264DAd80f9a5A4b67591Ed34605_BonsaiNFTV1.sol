...
contract BonsaiNFTV1 is ERC721Upgradeable {
    // Royalty
    ...
    // map id to BonsaiNFT obj
    mapping(uint256 => BonsaiNFT) public allBonsaiNFT;
    ...
    function mint(uint256 numberOfToken) public payable {
        ...
        for (uint256 i = 1; i <= numberOfToken; i++) {
            ...
            BonsaiNFT memory newBonsaiNFT = BonsaiNFT(
                newItemId,
                msg.sender,
                msg.sender,
                0,
            ...
            // add the token id to the allBonsaiNFT
            allBonsaiNFT[newItemId] = newBonsaiNFT;
        }
    }
    ...
    function setPriceForSale(
        ...
        BonsaiNFT memory bonsai = allBonsaiNFT[_tokenId];
        bonsai.price = _newPrice;
        bonsai.forSale = isForSale;
        allBonsaiNFT[_tokenId] = bonsai;
    ...
    function getAllSaleTokens() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= _totalSupply; i++) {
            if (allBonsaiNFT[i].forSale == true) {
                _tokenForSales[counter] = allBonsaiNFT[i].tokenId;
                counter++;
            }
        }
    ...
    function buyToken(uint256 _tokenId) public payable {
        ...
        // get that token from all BonsaiNFT mapping and create a memory of it defined as (struct => BonsaiNFT)
        BonsaiNFT memory bonsai = allBonsaiNFT[_tokenId];
    ...
    function _beforeTokenTransfer(
        ...
        BonsaiNFT memory bonsai = allBonsaiNFT[tokenId];
        bonsai.currentOwner = to;
        bonsai.numberOfTransfers += 1;
        bonsai.forSale = false;
        allBonsaiNFT[tokenId] = bonsai;
        if (from == address(0)) {
