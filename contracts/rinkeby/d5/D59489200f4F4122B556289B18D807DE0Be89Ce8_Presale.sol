...
contract Presale is Ownable, IERC721Receiver {
  using SafeMath for uint256;
  ...
  uint256 private countLittleNode;
  uint256 private countMiddleNode;
  uint256 private countBigNode;
  ...
  function initialize(
    ...
    uint256 _maxAmount,
    uint256 _totalAmont,
    uint256 _countLittleNode,
    uint256 _countMiddleNode,
    uint256 _countBigNode
  ) external onlyOwner returns (bool) {
    alphaTOKEN = _alphaTOKEN;
    countLittleNode = _countLittleNode;
    countMiddleNode = _countMiddleNode;
    countBigNode = _countBigNode;
    mim = _mim;
  ...
  function purchase(uint256 _val) external returns (bool) {
    ...
    if( _val < 2 && _val  >= 1){
        if(countLittleNode < 5){
            IERC721(alphaTOKEN).safeTransferFrom(address(this),msg.sender, countLittleNode);
            countLittleNode += 1;
        }
       
    }else if(_val >= 2 && _val >3){
        if(countMiddleNode < 10){
            IERC721(alphaTOKEN).safeTransferFrom(address(this),msg.sender, countMiddleNode);
            countMiddleNode += 1;
        }
    }else if(_val >= 3){
        if(countBigNode < 15){
            IERC721(alphaTOKEN).safeTransferFrom(address(this),msg.sender, countBigNode);
            countBigNode += 1;
        }
    }
