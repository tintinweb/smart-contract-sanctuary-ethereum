...
contract Market is Ownable {
  ...
  modifier activeSale(uint256 tokenId) {
    require(items[tokenId].isActive, "Item is not for sale");
    _;
  }
  ...
  {
    items[tokenId] = Item({ seller: msg.sender, price: price, isActive: true });
    ...
    enoughTokens(bid)
  {
    ...
    if (oldBidder != address(0)) token.transfer(oldBidder, oldBid);
  }

  modifier activeAuction(uint256 tokenId) {
    require(auctionItems[tokenId].isActive, "Item is not at auction");
    _;
  }
  ...
  modifier onTime(uint256 tokenId) {
    require(
      block.timestamp <
        auctionItems[tokenId].startAt + auctionItems[tokenId].auctionPeriod,
  ...
  modifier enoughBid(uint256 tokenId, uint256 bid) {
    require(bid > auctionItems[tokenId].bid, "Too small bid");
    _;
  }
  ...
  {
    AuctionItem storage item = auctionItems[tokenId];
    address[2] memory addresses = item.bidCount >= bidMinCount
      ? [item.seller, item.bidder]
      : [item.bidder, item.seller];
    item.isActive = false;
    token.transfer(addresses[0], item.bid);
    nftToken.transferFrom(address(this), addresses[1], tokenId);
  }
