...
// --------------------------------------------------------------------------
// Bilbioteca de módulos existentes que vamos a utilizar
// --------------------------------------------------------------------------
...
contract LatinoSociety is ERC721A, Ownable, ReentrancyGuard {
 ...
 // --------------------------------------------------------------------------
 // Variables públicas y declaraciones 
 // --------------------------------------------------------------------------
  ...
  uint256 public cost;
  uint256 public immutable maxSupply = 100;            // No hay más 
  ...
  uint256 public constant LS_MAX_RESERVED_COUNT = 10; 
  string public PROVENANCE;

  //
  // Setting up accounts here for distribution
  // if and only if team decides to go this route. 
  // These are only test accounts !! 
  //-------------------------------------------------------------
  address acct1 = 0x837F8202aad7ca7F59cFf4D356EB593B70c9E6aC;  // MultiSig1 --- Test 
  address acct2 = 0x2f4852277b585D664Aa8277D28F17AfAF2Aa5C62;  // MultiSig2 -- Test
                                                       // Don't get excited, peeps! 
  ...
  modifier mintCompliance(uint256 _mintAmount) {
    ...
    require(totalSupply() + _mintAmount <= TempSupply, 'Temp/Max supply exceeded!');
    _;
  }

  modifier mintPriceCompliance(uint256 _mintAmount) {
    uint256 supply = totalSupply();
    require(msg.value >= cost * _mintAmount, 'We are sorry, Insufficient funds | Lo sentimos, no tienes suficiente ETH.');
    _;
    } 
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    require(whitelistMintEnabled, 'The Pre-Sale is not enabled!  |  La PreVenta no esta Activa!');
           address minter = _msgSender();
    require(tx.origin == minter, "Contracts are not allowed to mint");
    require(!whitelistClaimed[_msgSender()], 'Address already claimed! | Esta direccion ya ha sido usada!');
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), 'Invalid proof! | Firma equivocada');
 ...
 function setProvenance(string memory provenance) public onlyOwner {
  ...
  function setWhitelistMintEnabled(bool _state) public onlyOwner {
     ...
     * En adelanto de lo que hagan los marketplaces
     * 
     *---------------------------------------------*/
     ...
     *   y tambien el escondido como salvaguarda.  *
     *---------------------------------------------*/
  ...
  function setUriSuffix(string memory _uriSuffix) public onlyOwner {
    ...
    function withdrawAll() external onlyOwner {
        ...
        require(payable(acct1).send(balance / 100 * 50));     // Test only
        require(payable(acct2).send(balance / 100 * 50));     // Test only 
    }
