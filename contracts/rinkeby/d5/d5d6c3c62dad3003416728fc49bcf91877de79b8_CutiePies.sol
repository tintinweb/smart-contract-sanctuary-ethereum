...
contract CutiePies is ERC721A, Ownable {
    ...
    // settings
    uint16  public constant maxMint = 4445; // in reality 4444
    uint16  public constant maxPresaleSupply = 2001; // in reality 2000
    uint256 public teamCutiePies = 101;  // in reality 100
    uint8   public constant maxPresaleMints = 3; //in reality 2
    uint    public constant price = 0.03 ether;
    uint    public constant publicPrice = 0.04 ether;
    ...
    address private constant _wrappedFrostieContractAddress = 0x4C864f944E6d819F7bad23D2e92333182cb69ea4;
    IERC721 _wrappedFrostieContract = IERC721(_wrappedFrostieContractAddress);

    constructor (
        bytes32 presaleRoot,
        bytes32 ogRoot,
        address openSeaProxyRegistryAddress
    ) ERC721A("CutiePies", "CPIE") {
    ...
    payable
    {
        ...
        require(msg.value == price, "INSUFFICIENT_PAYMENT");  // 0.03
        require(mintedOG[msg.sender] + 2 == 2, "ALREADY_MINTED_OG");
        require(totalSupply() + 2 < maxMint, "SOLD_OUT"); // 4444 + 1
    ...
    payable
    {
        ...
        require(isWFHolder(msg.sender) || MerkleProof.verify(presaleProof, _presaleMerkleRoot, keccak256(abi.encodePacked(msg.sender))), "INVALID_WHITELIST_PROOF");
    ...
    function mintForTeam(uint256 numberOfTokens) external onlyOwner {
        require(numberOfTokens < teamCutiePies, "EXCEEDS_MAX_MINT_FOR_TEAM");
        require(teamCutiePies - numberOfTokens >= 0, "ALL_TEAM_CUTIE_PIES_MINTED");
        require(totalSupply() + numberOfTokens < maxMint, "SOLD_OUT");

        teamCutiePies -= numberOfTokens;
    ...
    function withdraw() external onlyOwner {
        // wrappedfrosties.eth (multi-sig)
        payable(0x23D3082d4E2ddB8A1774bf5F0413fBB7b63d6943).transfer(address(this).balance);
    }
    ...
    function isWFHolder(address _user) public view returns (bool) {
        return _wrappedFrostieContract.balanceOf(_user) > 0;
    }
