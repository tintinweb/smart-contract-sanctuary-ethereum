...
contract RightsRegistry {
  ...
  event RightsDeclared(address nftContractAddr, uint256 tokenID, uint256 rightsID, string rightsURI, address declarer);
  ...
  function declare(address contractAddr, uint256 tokenID, string calldata rightsURI_) public returns (uint256) {
    ...
    require(Ownable(contractAddr).owner() == tx.origin, "RightsRegistry: Only the owner of the NFT contract can declare Rights");
