...
contract NftStakingPosition is ERC721, Ownable
{
	...
	function claimIncentiveStakerReward(uint256 stakingPositionId, address beneficiary) external returns (uint256)
	{
		...
		zoo.transfer(msg.sender, reward);
...
contract ListingList is Ownable, ERC721
{
	...
	event VotedForCollection(address indexed collection, address voter, uint256 amount);

	event ZooUnlocked(address voter, uint256 amount);
	...
	{
		require(_minTimelock >= _duration);
	...
	{
		return (timestamp - startDate) / epochDuration;
	}
	...
	function getVectorForEpoch(address collection, uint256 epochIndex) public view returns (uint256)
	{
		...
		require(collectionRecord.wasUpdated, "Epoch record was not updated");
	...
	function poolWeight(address collection, uint256 epochIndex) public view returns(uint256 weight) // check it
	{
		require(collectionRecords[collection][epochIndex].wasUpdated, "Epoch and colletion records were not updated");

		return collectionRecords[collection][epochIndex].weightAtTheStart;
	}

	function updateCurrentEpoch(address collection) public
	{
		uint256 epochNumber = getEpochNumber(block.timestamp);
		updateCollectionRecord(collection, epochNumber);
	}

	function updateCollectionRecord(address collection, uint256 epochIndex) public
	{
		if (epochIndex == 0) return;

		CollectionRecord storage collectionRecord = collectionRecords[collection][epochIndex];
		if (collectionRecord.wasUpdated)
			return;
		else
		{
			updateCollectionRecord(collection, epochIndex - 1);

			CollectionRecord storage collectionRecordOfPreviousEpoch = collectionRecords[collection][epochIndex - 1];
			collectionRecord.wasUpdated = true;
			collectionRecord.weightAtTheStart = collectionRecord.weightAtTheStart + collectionRecordOfPreviousEpoch.weightAtTheStart - getVectorForEpoch(collection, epochIndex - 1) * epochDuration;
...
/* ========== Ve-Model voting part ===========*/
	...
	function voteForNftCollection(address collection, uint256 amount, uint256 lockTime) external
	{
		...
		updateCollectionRecord(collection, currentEpoch);
		updateCollectionRecord(address(0), currentEpoch);

		uint256 unlockEpoch = getEpochNumber(block.timestamp + lockTime);
		vePositions[vePositionIndex] = VePositionInfo(unlockEpoch, amount);
		...
		collectionRecords[address(0)][unlockEpoch].weightAtTheStart += weight; // check
		collectionRecords[collection][unlockEpoch].rateOfIncrease += decay;
		collectionRecords[collection][unlockEpoch].weightAtTheStart += weight; // check
	...
	function unlockZoo(uint256 positionId) external
	{
		...
		require(currentEpoch >= vePosition.expirationEpoch, "time lock doesn't expire");

		zoo.transfer(msg.sender, vePosition.zooLocked);
		_burn(positionId);

		emit ZooUnlocked(msg.sender, vePosition.zooLocked);
	}
}
...
contract NftBattleArena is Ownable
{
	...
	event SwappedPositionVotes(uint256 indexed currentEpoch, address indexed voter, uint256 indexed newStakingPositionId, uint256 votingPositionId, uint256 daiNumber, uint256 newVotingPositionId);
	...
	event WithdrawedDaiFromVoting(uint256 indexed currentEpoch, address indexed voter, uint256 indexed stakingPositionId, uint256 votingPositionId, uint256 daiNumber);
	...
	uint256 public firstStageDuration = 5 minutes;// hours;        //todo:change time //3 days;    // Duration of first stage(stake).
	uint256 public secondStageDuration = 5 minutes;// hours;       //todo:change time //7 days;    // Duration of second stage(DAI)'.
	uint256 public thirdStageDuration = 5 minutes;// hours;        //todo:change time //2 days;    // Duration of third stage(Pair).
	uint256 public fourthStageDuration = 5 minutes;// hours;       //todo:change time //5 days;    // Duration fourth stage(ZOO).
	uint256 public fifthStageDuration = 5 minutes;// hours;        //todo:change time //2 days;    // Duration of fifth stage(Winner).
	...
	// address collection => number of tokens from collection.
	mapping (address => uint256) public collectionNumber;
	...
	function _createVotingPosition(uint256 stakingPositionId, address voter, uint256 yTokens, uint256 amount) internal returns (uint256 votes, uint256 votingPositionId)
	{
		...
		votes = zooFunctions.computeVotesByDai(amount, collection);                                         // Calculates amount of votes.
	...
	function _liquidateVotingPosition(uint256 votingPositionId, address voter, address beneficiary, uint256 lastEpoch, uint256 stakingPositionId, bool toSwap) internal
	{
	...
	function swapPositionVotes(uint256 votingPositionId, address voter, uint256 daiNumber, uint256 newStakingPositionId, uint256 newVotingPositionId) external only(nftVotingPosition)
	{
		...
		withdrawDaiFromVoting(votingPositionId, voter, daiNumber, true); // Calls internal withdrawDai.
		...
		emit SwappedPositionVotes(currentEpoch, voter, newStakingPositionId, votingPositionId, daiNumber, newVotingPositionId);
	}
	...
	function recomputeDaiVotes(uint256 votingPositionId) public
	{
		...
		uint256 newVotes = zooFunctions.computeVotesByDai(daiNumber, collection);                 // Recomputes dai to votes.
	...
	function addDaiToVoting(uint256 votingPositionId, address voter, uint256 amount, uint256 _yTokens) public only(nftVotingPosition) returns (uint256 votes)
	{
		...
		votes = zooFunctions.computeVotesByDai(amount, stakingPositionsValues[stakingPositionId].collection);                                 // Gets computed amount of votes from multiplier of dai.
	...
	// @ param beneficiary - address of recipient.
	function withdrawDaiFromVoting(uint256 votingPositionId, address voter, uint256 daiNumber, bool toSwap) public only(nftVotingPosition)
	{
		VotingPosition storage votingPosition = votingPositionsValues[votingPositionId];
		uint256 stakingPositionId = votingPosition.stakingPositionId;               // Gets id of staker position.
		updateInfo(stakingPositionId);
		...
		{
			_liquidateVotingPosition(votingPositionId, voter, voter, lastEpoch, stakingPositionId, toSwap);
			return;
		}
		uint256 yTokens = votingPosition.yTokensNumber;
		...
		for (uint256 i = votingPosition.startEpoch; i < lastEpoch; i++)
		{
			if (rewardsForEpoch[stakingPositionId][i].pricePerShareCoef != 0)
			{
				yTokens -= votingPosition.daiInvested / rewardsForEpoch[stakingPositionId][i].pricePerShareCoef;
			}
		}
		...
		emit WithdrawedDaiFromVoting(currentEpoch, voter, stakingPositionId, votingPositionId, daiNumber);
	}
	...
	function claimRewardFromVoting(uint256 votingPositionId, address voter, address beneficiary) external only(nftVotingPosition) returns (uint256 daiReward)
	{
		...
		if (rewardsForEpoch[stakingPositionId][currentEpoch].yTokens >= yTokenReward * 98 / 100)
		{
	...
	function chooseWinnerInPair(uint256 pairIndex) external
	{
		...
		// Tokens at battle end - tokens at battle start.
		uint256 income1 = battleRewardOfToken1.yTokens - tokensToShares(battleRewardOfToken1.tokensAtBattleStart);
		uint256 income2 = battleRewardOfToken2.yTokens - tokensToShares(battleRewardOfToken2.tokensAtBattleStart);

		if (pair.win)                                                                        // If 1st candidate wins.
		{
			battleRewardOfToken1.yTokensSaldo += int256(income1 + income2);                                    // Records income to token1 saldo.
			battleRewardOfToken2.yTokensSaldo -= int256(income2);                                    // Subtract income from token2 saldo.

			rewardsForEpoch[pair.token1][currentEpoch + 1].yTokens = battleRewardOfToken1.yTokens + income1 + income2;
			rewardsForEpoch[pair.token2][currentEpoch + 1].yTokens = battleRewardOfToken2.yTokens - income2;
		}
		else                                                                                 // If 2nd candidate wins.
		{
			battleRewardOfToken1.yTokensSaldo -= int256(income1);                                    // Subtract income from token1 saldo.
			battleRewardOfToken2.yTokensSaldo += int256(income1 + income2);                                    // Records income to token2 saldo.
			
			rewardsForEpoch[pair.token1][currentEpoch + 1].yTokens = battleRewardOfToken1.yTokens - income1;
			rewardsForEpoch[pair.token2][currentEpoch + 1].yTokens = battleRewardOfToken2.yTokens + income1 + income2;
		}
	...
	function calculateIncentiveRewardForVoter(uint256 votingPositionId) external only(nftVotingPosition) returns (uint256)
	{
		...
		require(votingPosition.incentiveRewardDate != currentEpoch, "already rewarded");
		...
		uint256 reward = 0;
		for (uint i = votingPosition.startEpoch; i < lastEpoch; i++) // Need different start epoch and last epoch.
		{
			reward += baseVoterReward * votingPosition.daiVotes * veZoo.poolWeight(collection, i) / veZoo.poolWeight(address(0), i) / rewardsForEpoch[votingPosition.stakingPositionId][i].votes;
		}
		votingPosition.incentiveRewardDate = currentEpoch;
	...
	function calculateIncentiveRewardForStaker(uint256 stakingPositionId) external only(nftStakingPosition) returns (uint256)
	{
		...
		require(stakingPosition.incentiveRewardDate != currentEpoch, "already rewarded");
		...
		uint256 reward = 0;
		for (uint256 i = stakingPosition.startEpoch; i < end; i++)
		{
			reward += baseStakerReward * veZoo.poolWeight(collection, i) / veZoo.poolWeight(address(0), i) / collectionNumber[collection];
		}
		stakingPosition.incentiveRewardDate = currentEpoch;
...
interface IZooFunctions {
	...
	function computeVotesByDai(uint256 amount, address collection) external view returns (uint256);
