...
contract StarCards is Ownable, ERC721Token, ERC165 {
  ...
  uint256 constant public timeBetweenEditions = 0;
  uint256 constant public initializationDelay = 1;
  ...
  function isReleased(uint id) public view onlyValidTokenIds(id) returns (bool) {
    return true; // getEdition(id) <= getCurrentEdition();
  }
  ...
  function releaseAuctionEnded(uint id) public view onlyValidTokenIds(id) returns (bool) {
    return true; // (isReleased(id) && (getReleaseAuctionEndTime(id) < now));
  }
  ...
  function completeReleaseAuction(uint id) payable external {
