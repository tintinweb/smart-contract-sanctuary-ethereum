...
contract Pridetown is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAXIMUM_HOLDINGS_PER_HOLDER = 10;
    bool private saleActive = false;
    bool private revealed = false;
    string private revealedBaseURI = "https://revealedBaseURI/";
    string private notRevealedBaseURI = "https://notRevealedBaseURI/";

    modifier basicCheck(uint256 quantity) {
    ...
    function mint(uint256 quantity) public basicCheck(quantity) nonReentrant {
        require((balanceOf(msg.sender) + quantity) <= MAXIMUM_HOLDINGS_PER_HOLDER, "The mint quantity exceeds the holding quantity limit");
        _mint(msg.sender, quantity);
    }

    function airdrop(address to, uint256 quantity) public basicCheck(quantity) onlyOwner {
        _mint(to, quantity);
    }
