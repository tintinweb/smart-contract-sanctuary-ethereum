...
contract EthGamesEntry is ERC721A, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public constant MAX_TOKEN_SUPPLY = 20000;
    address public constant DEV_ADDRESS = 0x0000000000000000000000000000000000000000;
    ...
    constructor(string memory baseURI) ERC721A("EthGames Series 0", "ETHGAMES") {
        setBaseURI(baseURI);
        _whitelist = true;
        _sale = false;
        _price = 0 ether;
    ...
    function mint(
        ...
        require(total <= MAX_TOKEN_SUPPLY, "EthGamesEntry: Max limit");
        require(total + _count <= MAX_TOKEN_SUPPLY, "EthGamesEntry: Max limit");
        require(total + _count <= _maxSale, "EthGamesEntry: Max sale limit");
        require(_count <= _maxMintPerTx, "EthGamesEntry: Max mint for tx limit");
        require(_sale, "EthGamesEntry: Sale is not active");
        ...
        if (_whitelist == true) {
            ...
            require(
                merkleRoot != 0x0,
                "EthGamesEntry: Invalid merkle root"
            );
    ...
    function _mintElements(address _to, uint256 _amount) private {
        ...
        require(totalSupply() <= MAX_TOKEN_SUPPLY, "EthGamesEntry: Limit reached");
    }
    ...
    function mintReserve(uint256 _count, address _to) public onlyOwner {
        uint256 total = totalSupply();
        require(total <= MAX_TOKEN_SUPPLY, "EthGamesEntry: Sale ended");
        require(total + _count <= MAX_TOKEN_SUPPLY, "EthGamesEntry: Max limit");
