...
contract ReserveOracle is IReserveOracleGetter, OwnableUpgradeable, BlockContext {
    ...
    address public weth;
    AggregatorV3Interface public wethAggregator;// ETH/USD
    mapping(address => bool) public needTransfer;
    ...
    function setWETHAggregators(address _wethAggregator) external onlyOwner {
        require(_wethAggregator != address(0), "zero address");
        wethAggregator = AggregatorV3Interface(_wethAggregator);
    }

    function addNeedTransfer(address key, bool flag) external onlyOwner {
        needTransfer[key] = flag;
    }
    ...
    function getAssetPrice(address _priceFeedKey) external view override returns (uint256) {
        ...
        uint256 tokenPrice = getPriceFromAggregator(aggregator);
        if (needTransfer[_priceFeedKey]) {
            uint256 ethPrice = getPriceFromAggregator(wethAggregator);
            tokenPrice = tokenPrice * TOKEN_DIGIT / ethPrice;
        }
        return tokenPrice;
    }

    function getPriceFromAggregator(AggregatorV3Interface aggregator) internal view returns (uint256){
