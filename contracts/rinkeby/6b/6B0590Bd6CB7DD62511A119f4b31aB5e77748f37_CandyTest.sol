...
contract CandyTest is ERC1155Supply, Ownable, ReentrancyGuard {
    ...
    function setMaxPaid(uint256 _maxPaid) external onlyOwner {
        maxPaid = _maxPaid;
    }

    // function numPaid(address addr) public view returns (uint256) {
    //     return paidCounter[addr];
    // }
    ...
    // function numClaimed(address addr) public view returns (uint256) {
    //     return claimedCounter[addr];
    // }
        ...
        nonReentrant
    {
        ...
        require(
            paidCounter[msg.sender] + _amount <= maxPaid,
            "Exceeds max per address"
        );
        require(price * _amount == msg.value, "Incorrect funds");
        require(totalSupply(0) + _amount <= max_supply, "Reached max supply");
