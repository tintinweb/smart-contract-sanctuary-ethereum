    ...
    ERC721LockerUpgradeable
{
    ...
    function isClaimed(uint256 collectionId, uint256 index) external view returns (bool) {
        _collectionExists(collectionId);
        return _isClaimed(proofsUsed[collections[collectionId].merkleIndex], index);
    }

    function minted(uint256 collectionId, address account) external view returns (uint256) {
        _collectionExists(collectionId);
        return collections[collectionId].mints[account];
    }
