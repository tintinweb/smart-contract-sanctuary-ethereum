...
contract Tile is ERC165, IERC1155, IERC1155MetadataURI, IERC2981, Ownable {
    ...
    /**
     * @dev Gets the color of a single tile.
     * @return 4 channel RGBA hex of the tile  (#rrggbbaa).
     */
    ...
    /**
     * @dev Gets the color of a batch of tiles.
     * @return array of 4 channel RGBA hex values of the tiles (#rrggbbaa).
     */
    ...
    /**
     * @dev Gets the cost of saving colors to the chain.
     */
    ...
    /**
     * @notice Set the following tile colors: `@formatColors(ids, colors)`
     * @dev Sets the colors of a batch of tiles.
     * @param ids the ids of the tiles (color at id i will be applied to tile at id i).
     * @param colors the colors of the tiles (color at id i will be applied to tile at id i).
     */
    function setColors(uint256[] memory ids, uint32[] memory colors) external payable {
        require(msg.sender == _stasher || msg.value >= _fees.colorFee, "Must pay for color");
        require(ids.length == colors.length, "ids/colors mismatch");
        for (uint256 i = 0; i < ids.length; ++i) {
            uint256 id = ids[i];
            uint32 color = colors[i];
            require(
                _tileOwners[id] == msg.sender ||
                    isApprovedForAll(_tileOwners[id], msg.sender) ||
                    (_tileOwners[id] == address(0) && msg.sender == _stasher),
    ...
    function getMintCost(uint16 amount) public view returns (uint256) {
        ...
        if (msg.sender == _deployer) {
            exemptAmount = uint16(
                Math.min(Math.max(0, _limits.deployerDiscountedTiles - mintedTiles), 2**16)
            );
        }
    ...
    /**
     * @dev Generates a pseudo-random number - used for random tile color.
     */
    function _pseudorandom() private view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), block.timestamp))); // solhint-disable-line
    }
    ...
    function setFees(Fees memory newFees) external onlyOwner {
        ...
        require(newFees.colorFee < 1000000, "Color fee exceeds 1 finney");
        _fees = Fees({
