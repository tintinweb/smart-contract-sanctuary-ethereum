...
contract AdvancedCollectible is ERC721, VRFConsumerBase {
    ...
    //everytime we update a mapping, best to create an event and emit event
    mapping(uint256 => BREED) public tokenIdToBreed; //basically NFT number
    mapping(bytes32 => address) public requestIdToSender;

    event requestCollectible(bytes32 indexed requestId, address requester);
    event tokenId_Breed(uint256 indexed tokenId, BREED breed);
    ...
    function createCollectible() public returns (bytes32) {
        bytes32 requestId = requestRandomness(keyHash, fee); //function in VRFConsumerbase
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        BREED breed = BREED(randomness % 3); //get a random breed
        uint256 newTokenId = tokenCounter; //set current token counter to the breed
        tokenIdToBreed[newTokenId] = breed; //map current token counter to breed
        emit tokenId_Breed(newTokenId, breed);

        _safeMint(requestIdToSender[requestId], newTokenId); //cannot use msg.sender since fulfillRandomness not initiated by sender

        tokenCounter++;
    }

    function setTokenURI(uint256 tokenId, string memory _tokenURI) public {
        //can do mapping as well
        require(
