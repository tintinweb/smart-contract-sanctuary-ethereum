...
import "../libraries/precog/PrecogTierSystem.sol";
...
contract PrecogOrphanCampaign is ReentrancyGuard, Pausable, PrecogTierSystem, Blacklist {
    ...
    uint256 public platformFeeRate = 1;

    // Address of revenue collector
    address public platformRevenueAddress = 0xDEE9231941c00d56ce5Fdc087d3D7138493ab68f;
    ...
    uint256 public maxBuyAmount = 20000;

    // Min amount in USD of tokens an user can buy
    uint256 public minBuyAmount = 2500000000000000000000;
    ...
    // ToDo: Remove this - add to the deploy script
    ...
    event RefundedTokenForIDOWhenEndIDO(address wallet, uint256 amount);
     ...
     * @param _duration Number of IDO time in seconds
     * @param _openTime Number of start IDO time in seconds
    ...
    constructor(
        ...
        fundingWallet = _msgSender();
        _setupTierSystem(_stakBankAddress, _storeStakingAddress, _openTime);
        ...
        returns (uint256 conversionRateDecimals)
    {
     ...
     /**
     * @notice Returns amount of tokens bought by user
     * @return amount amount of tokens bought by user.
     */
    function getTokensBought(address _address)
        external
        view
        returns (uint256 amount)
    {
        amount = userTokensMapping[_address].tokensBought;
    }
     ...
     * @return finalized true if the IDO ended.
     */
     ...
     * @return open true if the IDO is open.
     */
        ...
        nonReentrant
    {
        ...
        require(isLoaded, "IDO_CAMPAIGN::NOT_LOADED");
        require(_validPurchase(), "IDO_CAMPAIGN::PURCHASE_NOT_ALLOWED");
        ...
        pure
    {
        require(
            ...
            "IDO_CAMPAIGN::INVALID_BENEFICIARY"
        );
        require(_weiAmount != 0, "IDO_CAMPAIGN::INVALID_WEI_AMOUNT");
    }
    ...
    function _forwardFunds(uint256 _value) internal {
        ...
        require(success, "IDO_CAMPAIGN::WALLET_TRANSFER_FAILED");
    }
    ...
    function _updatePurchasingState(
        ...
        require(
            ...
            "IDO_CAMPAIGN::MAX_BUY_AMOUNT_EXEDED"
        );
        require(
            getAvailableTokens() >= _tokenAmount,
            "IDO_CAMPAIGN::AVAILABLE_TOKENS_NOT_ENOUGH"
        );
     ...
     * @dev  Only pay when use Lemonade to register IDO Campaign
     * @return platformFee Return number of platform fee
     */
    function _payPlatformEtherFee() private returns (uint256 platformFee) {
        address payable _platformRevenueAddress = address(
            uint160(platformRevenueAddress)
        );
        uint256 payment = msg.value;
        platformFee = payment.mul(platformFeeRate).div(100);

        (bool success, ) = _platformRevenueAddress.call{value: platformFee}("");
        require(success, "IDO_CAMPAIGN::PAY_PLATFORM_FEE_FAILED");
    }

    /**
     * @notice Call to set Platform Fee
     * @dev  pass on a fixed number fee of Platform
     */
        ...
        onlyOwner
    {
     ...
     /**
     * @notice Call to set revenue address
     * @dev  pass on address of revenue vault
     */
    function setPlatformRevenueAddress(address _platformRevenueAddress)
        external
        onlyOwner
    {
    ...
    function loadCampaign(
        ...
        require(!isLoaded, "IDO_CAMPAIGN::LOAD_ALREADY_VERIFIED");
        require(
            block.timestamp < openTime,
            "IDO_CAMPAIGN::CAMPAIGN_ALREADY_STARTED"
        );
        require(
            token.balanceOf(address(this)) >= raiseAmount,
            "IDO_CAMPAIGN::INVALID_BALANCE"
        );
        _setTiers(_tier1, _tier2, _tier3, _tier4);
    ...
    function setEtherConversionRate(uint256 _rate) external onlyOwner {
        require(etherConversionRate != _rate, "IDO_CAMPAIGN::RATE_INVALID");
     ...
     * @param _rate Fixed number of USD
     */
    function setMaxBuyAmountRate(uint256 _rate) external onlyOwner {
        require(maxBuyAmount != _rate, "IDO_CAMPAIGN::RATE_INVALID");
        maxBuyAmount = _rate;
        emit CampaignStatsChanged();
    }

    /**
     * @notice Owner can set min of tokens of an user can buy
     * @param amount Fixed number of tokens
     */
    function setMinBuyAmount(uint256 amount) external onlyOwner {
        require(minBuyAmount != amount, "IDO_CAMPAIGN::AMOUNT_INVALID");
        minBuyAmount = amount;
        emit CampaignStatsChanged();
    }
    ...
    function setLiquidityLockingPercentage(uint8 _percentage)
        ...
        onlyOwner
    {
        require(_percentage <= 100, "IDO_CAMPAIGN::OVERFLOW_LOCKING");
        liquidityLocking = _percentage;
    }

    /**
     * @notice Owner can set the fundingWallet where funds are collected
     * @param _address Address of funding IDO wallets. Sold tokens in eth will transfer to this address
     */
    function setFundingWallet(address _address) external onlyOwner {
        require(_address != address(0), "IDO_CAMPAIGN::ZERO_ADDRESS");
    ...
    // ToDo: adpt this function - remove releaseStartDate and releaseEndDate
    /**
     ...
     */
    function setOpenTimeAndDuration(
        uint256 _openTime,
        uint256 _duration,
        ...
        require(
            ...
            "IDO_CAMPAIGN::INVALID_OPEN_TIME"
        );
        openTime = _openTime;
        closeTime = _openTime.add(_duration);
        _updateOpenTime(_openTime);
        emit CampaignStatsChanged();
    }

    /**
     * @notice Owner can receive their remaining tokens when IDO Ended
     * @dev  Can refund remainning token if the IDO ended
     * @param _wallet Address wallet who receive the remainning tokens when IDO end
     */
    function refundTokenForIDOOwner(address _wallet) external onlyOwner {
        require(getAvailableTokens() > 0, "IDO_CAMPAIGN::EMPTY_BALANCE");
        uint256 availableToken = getAvailableTokens();
        _deliverTokens(_wallet, availableToken);
        emit RefundedTokenForIDOWhenEndIDO(_wallet, availableToken);
    }
...
contract PrecogTierSystem is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public tier1Time = 0;
    uint256 public tier2Time = 4800;
    uint256 public tier3Time = 9600;
    uint256 public tier4Time = 14400;


    mapping(address => bool) whitelist;
    event TierSystemSetUp(IStakBank stakBank, uint256 openTime);
    event TierSystemUpdated(uint256 openTime);
    event StakBankAddressChanged(IStakBank stakBank);
    event StoreContractChanged(IStoreStakingUsers storeStaking);

    modifier onlyTiered() {
        require(_isTiered(_msgSender()), "ICO_CAMPAIGN::NOT_TIERED");
        _;
    }
    ...
    function setTierTimes(
        ...
        tier1Time = _tier1Time;
        tier2Time = _tier2Time;
        tier4Time = _tier3Time;
        tier4Time = _tier4Time;
    }

    function setStakBankContract(IStakBank _stakBank) public onlyOwner {
        stakBank = _stakBank;
        emit StakBankAddressChanged(_stakBank);
    }

    function setStoreStakingContract(IStoreStakingUsers _storeStaking) public onlyOwner {
        storeStakingUsers = _storeStaking;
        emit StoreContractChanged(_storeStaking);
    }
        ...
        returns (uint8 tier, uint256 timestamp)
    {
         ...
         uint256 senderBalance = stakBank.balanceOf(_address);
         uint256 senderPolygonBalance = storeStakingUsers.getUsers(_address);
         uint256 totalBalance = senderBalance + senderPolygonBalance;


       if (totalBalance >= tier1 )  {

            tier = 1;
            timestamp = campaignOpenTime;
       }  else if (totalBalance >= tier2) {
            tier = 2;
            timestamp = campaignOpenTime.add(tier2Time);
       } else if (totalBalance >= tier3) {
            tier = 3;
            timestamp = campaignOpenTime.add(tier3Time); 

       } else  {
            tier = 4;
            timestamp = campaignOpenTime.add(tier4Time);
       } 
    }
    
    function _isTiered(address _address) internal view returns (bool tier) {
         if (block.timestamp >= campaignOpenTime.add(tier1Time)) {
            tier = true;
        } else if (block.timestamp >= campaignOpenTime.add(tier2Time)) {
            tier = stakBank.balanceOf(_address) >= tier2;
        } else if (block.timestamp >= campaignOpenTime.add(tier3Time)) {
            tier = stakBank.balanceOf(_address) >= tier3;
        } else {
            tier = stakBank.balanceOf(_address) >= tier4; 
        }
    }
    ...
    function _setupTierSystem(IStakBank _stakBank, address _storeStaking, uint256 _openTime) internal {
        stakBank = _stakBank;
        storeStakingUsers = IStoreStakingUsers(_storeStaking);
        campaignOpenTime = _openTime;
        emit TierSystemSetUp(stakBank, campaignOpenTime);
    }

    function _updateOpenTime(uint256 _openTime) internal {
        campaignOpenTime = _openTime;
        emit TierSystemUpdated(campaignOpenTime);
    }
}
...
interface IStoreStakingUsers {
    function getUsers(address  _user) external view returns (uint256);
}
