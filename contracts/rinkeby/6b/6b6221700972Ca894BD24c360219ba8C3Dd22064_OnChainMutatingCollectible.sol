...
import "ERC721.sol"; // For ERC721 and totalSupply() method
import "IERC721Enumerable.sol";
import "Ownable.sol";
import "Base64.sol";

contract OnChainMutatingCollectible is IERC721Enumerable, ERC721, Ownable {
    ...
    constructor() ERC721("BasisLabsOnChainNFT", "BLOCN") public {}

    // public, AKA mint function
    function createCollectible(string memory _userText) public payable {
        uint256 supply = totalSupply();
        bytes memory strBytes = bytes(_userText);
        require(strBytes.length <= stringLimit, "String input exceeds limit.");
        require(exists(_userText) != true, _userText );
    ...
    function buildMetadata(uint256 _tokenId) private view returns (string memory)
    {
        ...
        // TO-DO: ADD SOME MUTATION FUNCTIONS HERE, AS A RESULT OF THE RANDOM SEED. GET THE ORIGINAL METADATA FIRST.
    ...
    // Return the metadata, which in our case, means building and returning the actual metadata
    // in an encoded format. This will not point to a link to an off-chain storage.
