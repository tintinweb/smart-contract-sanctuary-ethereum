  ...
  OwnableUpgradeable 
{
  ...
  uint public constant CHANGE_PRICE = 0.01 ether;
  ...
  // (tokenId => index of SVG contract in shapesAddresses)
  mapping (uint => uint[]) public shapeIndexes;
  ...
  // emit some events
  event ShapeChanged(uint tokenId, uint shapeIndex);
  event SeedChanged(uint tokenId, uint newSeed);
  ...
  function _canBeChanged(uint tokenId) public view {
   if (changeDeadline[tokenId] > block.timestamp) {
     revert Wait(changeDeadline[tokenId]);
   }
   if(isLocked[tokenId] == true) {
     revert Locked();
   }
  }
  ...
  function _mintShape(
    ...
    _setShape(currentTokenId, _shapeId);
    
    if(_seed == 0) {
      // assign random seed
      _setNextSeed(currentTokenId);
    } else {
      // assign existing seed
      seeds[currentTokenId].push(_seed);
    }
  ...
  function batchMint(address _recipient, uint256 numberOfNfts) public payable {
    ...
    if(msg.value < PRICE * numberOfNfts) {
      revert InsufficientPrice();
    }
              ...
              Base64.encode(
                  bytes(
                      abi.encodePacked(
                        ...
                        '","image":"',
                        svgString,
                        '", "external_link": "https://makio135.io/shapes", "seller_fee_basis_points": 500 }'
                        )
                  )
              )
          )
  ...
  function tokenURI(uint256 tokenId) 
    ...
    returns (string memory) 
    {
      // get latest shapeType and seed
      uint shapeIndex = shapeIndexes[tokenId][shapeIndexes[tokenId].length - 1];
      uint seed = seeds[tokenId][seeds[tokenId].length - 1];

      IShape svg = IShape(shapesAddresses[shapeIndex]);
      string memory svgString;
      string memory attributes;
  ...
  function setShapesAddresses(address[] calldata _shapesAddresses) public onlyOwner {
    for (uint256 i = 0; i < _shapesAddresses.length; i++) {
      if(_shapesAddresses[i] == address(0)) {
        revert ZeroAddress();
      }
    }
   ...
   * User features
   */
  function changeSeed(uint _tokenId) public payable {
    _projectNotEnded();
    if(ownerOf(_tokenId) != msg.sender) {
      revert Unauthorized();
    }
    if(msg.value < CHANGE_PRICE) { 
      revert InsufficientPrice(); 
    }
    _canBeChanged(_tokenId);
    _setNextSeed(_tokenId);
    changeDeadline[_tokenId] = block.timestamp + ONE_DAY;
  }

  function changeShape(uint _tokenId, uint _shapeIndex) public payable {
    _projectNotEnded();
    if(ownerOf(_tokenId) != msg.sender) {
      revert Unauthorized();
    }
    ...
    _canBeChanged(_tokenId);
    _setShape(_tokenId, _shapeIndex);
    changeDeadline[_tokenId] = block.timestamp + ONE_DAY;
  }

  function _setShape(uint _tokenId, uint _shapeIndex) internal {
    if(_shapeIndex > shapesAddresses.length - 1) {
      revert WrongNumber();
    }
    shapeIndexes[_tokenId].push(_shapeIndex);
    emit ShapeChanged(_tokenId, _shapeIndex);
  }

  function _setNextSeed(uint _tokenId) internal returns (uint newSeed) {
    // we skip the first 1024 to make sure we dont get identical slashes
    newSeed = 1024 + _nextSeed.current();
    seeds[_tokenId].push(newSeed);
    _nextSeed.increment();
    emit SeedChanged(_tokenId, newSeed);
  }

  function countShapeChanges(uint _tokenId) public view returns (uint) {
    return shapeIndexes[_tokenId].length;
  }

  function countSeedChanges(uint _tokenId) public view returns (uint) {
