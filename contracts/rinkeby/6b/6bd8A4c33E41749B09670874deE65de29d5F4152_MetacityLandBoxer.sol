...
import "../../tokens/interfaces/IERC721WithAutoId.sol";

import "../../utils/LibSeedGenerator.sol";
import "../../utils/LibArrayHelper.sol";
...
contract MetacityLandBoxer is Context, Ownable {
    ...
    struct ZoneInfo {
        ...
        address[] tokens;
        uint256[] prices;
        ICheckouter.BillingType[] billingTypes;
        uint256[] capacities;
    }
    ...
    mapping(uint256 => PriceInfo) globalPrices;
    ...
    modifier onlyCapacityEnoughBatch(string[] memory _zoneIds, uint256[] memory _scales, uint256[] memory _amounts) {
        ...
        for(uint256 i=0; i<_zoneIds.length; i++) {
            require(_isValidScale(_scales[i]));
            require(_getCapacityByZoneIdAndScale(_getZoneId(_zoneIds[i]), _scales[i]) >= _amounts[i], "no enough capacity");
        }
        _;
    }
    
    modifier onlyValidScale(uint256 _scale) {
    ...
    function setBasicInfo(address _landAddress, address _checkoutCounter) public onlyOwner {
    ...
    function capacityRemaining(string memory _zoneId, uint256 _scale) public view onlyValidScale(_scale) returns (uint256 amount) {
        bytes32 zoneId = _getZoneId(_zoneId);
        return landCapacities[zoneId].capacities[_getIndexByZoneIdAndScale(zoneId, _scale)];
    }

    function correspondingPrice(string memory _zoneId, uint256 _scale) public view onlyValidScale(_scale) returns (address, uint256) {
        bytes32 zoneId = _getZoneId(_zoneId);
        PriceInfo memory priceInfo = _getPriceByZoneIdAndScale(zoneId, _scale);
        return (priceInfo.token, priceInfo.price);
    }

    function correspondingPriceGlobal(uint256 _scale) public view onlyValidScale(_scale) returns (address, uint256) {
        PriceInfo memory priceInfo = globalPrices[_scale];
        return (priceInfo.token, priceInfo.price);
    }

    function setSupportedScalesAndGlobalPrices(uint256[] memory _scales, PriceInfo[] memory _prices) public onlyOwner {
        require(_scales.length == _prices.length, "args error");
        ...
        onlyOwner
    {
        ...
        for (uint8 i=0; i<_zoneIds.length; i++) {
            _metaTradeAdmin(to, _getZoneId(_zoneIds[i]), _scales[i], _amounts[i]);
            IERC721WithAutoId(landAddress).mint(to, _amounts[i]);
        }
    }
        ...
        onlyCapacityEnoughBatch(_zoneIds, _scales, _amounts)
    {
        ...
        for (uint256 i=0; i<_zoneIds.length; i++) {
            ...
            PriceInfo memory scalePrice = _getPriceByZoneIdAndScale(zoneId, _scales[i]);
            remainingValue = remainingValue.sub(_ethPurchaseLand(zoneId, _scales[i], scalePrice, _amounts[i]));
            IERC721WithAutoId(landAddress).mint(_msgSender(), _amounts[i]);
        }
        ...
        onlyCapacityEnoughGlobalBatch(_scales, _amounts)
    {
        ...
        for (uint256 i=0; i<_scales.length; i++) {
            ...
            while (currentScaleAmountRemains > 0) {
                bytes32 zoneId = _getFirstRandomZoneIdWithEnoughCapacity(currentScaleAmountRemains.add(i), _scales[i], 1);
                remainingValue = remainingValue.sub(_ethPurchaseLand(zoneId, _scales[i], globalPrices[_scales[i]], 1));
        ...
        onlyCapacityEnoughGlobalBatch(_scales, _amounts)
    {
        ...
        for (uint256 i=0; i<_tokens.length; i++) {
            uint256 currentScaleAmountRemains = _amounts[i];
            while (currentScaleAmountRemains > 0) {
                bytes32 zoneId = _getFirstRandomZoneIdWithEnoughCapacity(i.add(currentScaleAmountRemains), _scales[i], 1);
                _tokenPurchaseLand(_tokens[i], zoneId, _scales[i], globalPrices[_scales[i]], _amounts[i]);
                currentScaleAmountRemains = currentScaleAmountRemains.sub(1);
            }
            IERC721WithAutoId(landAddress).mint(_msgSender(), _amounts[i]);
        }
    ...
    function _ethPurchaseLand(bytes32 _zoneId, uint256 _scale, PriceInfo memory _priceInfo, uint256 _amount) private returns (uint256 purchaseCost) {
        ...
        IETHCheckouter(checkoutCounter).ethPurchase{
            ...
            _priceInfo.billingType
        );
        _metaTrade(_zoneId, _scale, _priceInfo, _amount);
        purchaseCost = _amount.mul(_priceInfo.price);
    }

    function _tokenPurchaseLand(address token, bytes32 _zoneId, uint256 _scale, PriceInfo memory _priceInfo, uint256 _amount) private {
    ...
    function _metaTrade(bytes32 _zoneId, uint256 _scale, PriceInfo memory _priceInfo, uint256 _amount) private {
        uint256[] memory tokenIds;
        uint256[] memory seeds;
        (tokenIds, seeds) = _metaTradeBasic(_zoneId, _scale, _amount);
        ...
        emit LandPurchased(
            ...
            _priceInfo.price,
            _amount,
            _getZoneIdentifierByZoneId(_zoneId),
            tokenIds,
            seeds
        );
    }

    function _metaTradeAdmin(address to, bytes32 _zoneId, uint256 _scale, uint256 _amount) private {
        uint256[] memory tokenIds;
        uint256[] memory seeds;
        (tokenIds, seeds) = _metaTradeBasic(_zoneId, _scale, _amount);
    ...
    function _metaTradeBasic(bytes32 _zoneId, uint256 _scale, uint256 _amount) private returns (uint256[] memory, uint256[] memory) {
    ...
    function _getZoneId(string memory input) private pure returns (bytes32 zoneId) {
    ...
    function _getPriceByZoneIdAndScale(bytes32 zoneId, uint256 _scale) private view returns (PriceInfo memory price) {
        ZoneInfo storage zoneInfo = landCapacities[zoneId];
        for (uint256 i=0; i<zoneInfo.scales.length; i++) {
            if (zoneInfo.scales[i] == _scale) {
                price = PriceInfo(zoneInfo.tokens[i], zoneInfo.prices[i], zoneInfo.billingTypes[i]);
            }
        }
    ...
    function _getZoneIdentifierByZoneId(bytes32 zoneId) private view returns (string memory _zoneId) {
        _zoneId = landCapacities[zoneId].zoneIdentifier;
        // require(keccak256(abi.encode(_zoneId)) != keccak256(abi.encode("")), "invalid zoneId");
    }

    function _getFirstRandomZoneIdWithEnoughCapacity(uint256 nonce, uint256 _scale, uint256 _amount) private view returns (bytes32 zoneId) {
