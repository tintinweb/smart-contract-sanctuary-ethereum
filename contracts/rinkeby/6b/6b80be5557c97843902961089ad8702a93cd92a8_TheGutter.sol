...
contract TheGutter is Ownable, IERC721Receiver  {
    ...
    mapping (address => uint16) stakedPerAddress; 
    ...
    function withdraw() external onlyOwner {
        ...
        uint256 tmpStakingPayout = 0; 

        uint16 stakedCount = 0;
        for(uint16 i = 0;i < stakedPunk.length;i++) {
            if(stakedPunk[i] != address(0x0)) {
                stakedCount++; 
                stakedPerAddress[stakedPunk[i]]++;
            }
        }
        
        for(uint16 i = 0;i < stakedPunk.length;i++) { 
            if(stakedPunk[i] != address(0x0) && stakedPerAddress[stakedPunk[i]] > 0) { 
                tmpStakingPayout = stakedPerAddress[stakedPunk[i]] / stakedCount * stakedShare;
                stakedPerAddress[stakedPunk[i]] = 0;
                totalStakingPayout += tmpStakingPayout;
                payable(stakedPunk[i]).transfer(tmpStakingPayout);
                emit StakingPayout(stakedPunk[i], tmpStakingPayout);
                tmpStakingPayout = 0;
            }
        }
