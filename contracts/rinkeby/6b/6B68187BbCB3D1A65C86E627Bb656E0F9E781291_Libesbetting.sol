...
contract Libesbetting is Ownable {
  ...
  function betTournament(
    ...
    require(
      IERC721A(_tokenAddress).ownerOf(_tokenId) == msg.sender,
  ...
  function cancelBet(
    ...
    for (uint256 i = 0; i < betters[_tournamentId].length; i++) {
      delete betters[_tournamentId][i].TokenId;
    }
    delete better[_betId];

    IERC721A(_tokenAddress).Unlocktoken(_tokenId);
    emit CancelBet(_betId, _tokenId, _tokenAddress);
  }
