...
contract Nova is ERC721Enumerable, Ownable {
    ...
    //PUBLIC
    function whitelistMint(bytes32 _merkleRoot, bytes32[] calldata _merkleProof, uint256[] calldata _position) external payable {
        require(!paused, "Minting not started yet!");
        require(totalSupply() <= maxSupply, "NFT supply exceeded!");
        ...
        uint256 newId = useRandomAvailableToken();
    ...
    function useRandomAvailableToken() internal returns (uint256){
        uint256 randomNum = uint256(
            keccak256(
                abi.encode(msg.sender,tx.gasprice,block.number,block.timestamp,blockhash(block.number - 1)
                )
            )
        );
