...
contract NftMarketplace is ReentrancyGuard{
    ...
    // Mapping to keep track of how much people have earned by selling NFTs
    ...
    function listItem(address nftAddress, uint256 tokenId, uint256 price) external notListed(nftAddress, tokenId, msg.sender) isOwner(nftAddress, tokenId, msg.sender) {
        ...
        // 1. Send the NFT to the contract. In this case, contract will own the NFT and we will have to withdraw the NFT from the cotract
        // 2. Owners can hold their NFT, and give the marketplace approval to sell NFT for them. This approval can be withdrawn anytime
    ...
    function buyItem(address nftAddress, uint256 tokenId) external payable isListed(nftAddress, tokenId) nonReentrant{
        ...
        // Instead of sending the money to the user we make them withdraw the money later.
        // Concept - Pull over Push
        s_proceeds[listedItem.seller] = s_proceeds[listedItem.seller] + msg.value;

        // Once the item is bought we have to delete the listing from the marketplace
