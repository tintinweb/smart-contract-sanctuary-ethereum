...
contract Polyroll is VRFConsumerBase, Ownable, ReentrancyGuard {
    ...
    address public constant LINK_TOKEN = 0x01BE23585060835E02B77ef475b0Cc51aA1e0709;   // Rinkeby
    ...
    uint public chainlinkFee = 100000000000000000; // 0.1 LINK
    ...
    uint public wealthTaxThreshold = 200 ether;  // main network setted 500000000000000000000
    ...
    uint public balanceMaxProfitRatio = 1;//24;
    ...
    function getGamblerBalance(address gambler) external view returns(uint256) { 
        return address(gambler).balance;
    }

    function getTop100GamblerBalances(address[] memory gamblers) external view returns(uint256[100] memory balances) { 
        for ( uint256 i = 0; i < gamblers.length; i++) { 
            balances[i] = address(gamblers[i]).balance;
        } 
    }
    ...
    struct IndexBet { 
        ...
        uint256[100] balance;
        uint8[100] modulo;
        uint[100] placeAmount;
        bool[100] isSettled;
        uint[100] winAmount;
    }

    function getIndexBet(uint256 startIndex, uint256 endIndex) external view returns(IndexBet memory indexBet) { 
        for( startIndex = 0; startIndex < endIndex; startIndex++ ) { 
            indexBet.gambler[startIndex] = bets[startIndex].gambler;
            indexBet.balance[startIndex] = address(bets[startIndex].gambler).balance;
            indexBet.modulo[startIndex] = bets[startIndex].modulo;
            indexBet.placeAmount[startIndex] = bets[startIndex].amount;
            indexBet.isSettled[startIndex] = bets[startIndex].isSettled;
            indexBet.winAmount[startIndex] = bets[startIndex].winAmount;
        }
    }

    // function getIndexBet2(uint256 ids) external view returns(IndexBet memory indexBet) { 
    //     for( uint256 i=0; i<ids; i++ ) { 
    //         indexBet.bet[i] = bets[i] ;
    //     }
    // }
    ...
    // function getIndexBet3(uint256 ids) external view returns(IndexBet3 memory indexBet) { 
    //     for( uint256 i=0; i<ids; i++ ) { 
    //         indexBet.bet[i] = bets[i] ;
    //         indexBet.balance[i] = address(bets[i].gambler).balance;
    //     }
    // }
    ...
    // function getIndexBet(uint256 ids) external view returns(IndexBet[] memory indexBets) { 

    //     for (uint256 i = 0; i < ids; i++) { 
    //         indexBets[i].gambler = bets[i].gambler;
    //         indexBets[i].balance += address(bets[i].gambler).balance;
    //         indexBets[i].modulo += bets[i].modulo;
    //         indexBets[i].placeAmount += bets[i].amount;
    //         indexBets[i].isSettled = bets[i].isSettled;
    //         indexBets[i].winAmount += bets[i].winAmount;
    //     }
    // }
    ...
    // function getIndexBet(uint256 ids) external view returns(IndexBet memory indexBet) { 
    ...
    function balanceLinkToken() external view returns (uint) { 
