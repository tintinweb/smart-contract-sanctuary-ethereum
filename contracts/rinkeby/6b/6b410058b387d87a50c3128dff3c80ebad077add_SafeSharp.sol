...
contract SafeSharp is Ownable {
    ...
    // function getSurvivorPicksByAddress(address _address) public view returns (uint8[][][18] memory) {
    //     uint8[][][18] memory addressPicks; 
    ...
    function setSurvivorSmall(uint8[18] memory _picks, address _picker, address _referral) external payable { 
        ...
        survivorPoolSmall += (msg.value * 1 - HOUSE_PAY_BIP/100);
        uint256 HOUSE_PAY_NET_BIP = HOUSE_PAY_BIP;
        if (_referral != address(0)) {
            payable(_referral).transfer(msg.value * REFERRAL_PAY_BIP / 100);
            HOUSE_PAY_NET_BIP = HOUSE_PAY_BIP - REFERRAL_PAY_BIP;
        }
        payable(HOUSE_ADDRESS).transfer(msg.value * HOUSE_PAY_NET_BIP / 100);
    }

    function setSurvivorMedium(uint8[18] memory _picks, address _picker, address _referral) external payable { 
        require(msg.value == mediumBet, "Not enough for a small bet");
        SurvivorBet memory _survivorBet = SurvivorBet(_picks, _picker, _referral);
        survivorBetSmall.push(_survivorBet);
        survivorPoolSmall += msg.value - (msg.value * HOUSE_PAY_BIP/100);
        uint256 HOUSE_PAY_NET_BIP = HOUSE_PAY_BIP;
        if (_referral != address(0)) {
            payable(_referral).transfer(msg.value * REFERRAL_PAY_BIP / 100);
            HOUSE_PAY_NET_BIP = HOUSE_PAY_BIP - REFERRAL_PAY_BIP;
    ...
    function setSurvivorLarge(uint8[18] memory _picks, address _picker, address _referral) external payable { 
    ...
    function setWeeklyPickSmall(uint256 _week, uint256[] memory _picks, address _picker, address _referral) external payable { 
        ...
        weeklyPoolSmall[_week]+= msg.value - (msg.value * HOUSE_PAY_BIP/100);
        uint256 HOUSE_PAY_NET_BIP = HOUSE_PAY_BIP;
        if (_referral != address(0)) {
            payable(_referral).transfer(msg.value * REFERRAL_PAY_BIP / 100);
            HOUSE_PAY_NET_BIP = HOUSE_PAY_BIP - REFERRAL_PAY_BIP;
    ...
    function setWeeklyPickMedium(uint256 _week, uint256[] memory _picks, address _picker, address _referral) external payable { 
        ...
        weeklyPoolMedium[_week]+= msg.value - (msg.value * HOUSE_PAY_BIP/100);
        uint256 HOUSE_PAY_NET_BIP = HOUSE_PAY_BIP;
        if (_referral != address(0)) {
            payable(_referral).transfer(msg.value * REFERRAL_PAY_BIP / 100);
            HOUSE_PAY_NET_BIP = HOUSE_PAY_BIP - REFERRAL_PAY_BIP;
    ...
    function setWeeklyPickLarge(uint256 _week, uint256[] memory _picks, address _picker, address _referral) external payable { 
        ...
        weeklyPoolLarge[_week]+= msg.value - (msg.value * HOUSE_PAY_BIP/100);
        uint256 HOUSE_PAY_NET_BIP = HOUSE_PAY_BIP;
        if (_referral != address(0)) {
            payable(_referral).transfer(msg.value * REFERRAL_PAY_BIP / 100);
            HOUSE_PAY_NET_BIP = HOUSE_PAY_BIP - REFERRAL_PAY_BIP;
