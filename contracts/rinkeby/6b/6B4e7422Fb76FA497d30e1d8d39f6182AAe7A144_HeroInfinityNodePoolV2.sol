...
contract HeroInfinityNodePoolV2 is Ownable {
  ...
  uint256 public maxNodesPerWallet = 25;
    ...
    returns (uint256)
  {
    ...
    uint256 rewards = 0;
    console.log("days passed %s", passedDays);
    for (uint256 i = 1; i <= passedDays; i++) {
      uint256 dayReward = getRewardsForDay(i);
      if (todayPassedSeconds > 0 && i == passedDays) {
        dayReward = (dayReward * todayPassedSeconds) / 86400;
      }
      rewards += dayReward;
      console.log("reward at day %s is %s", i, dayReward);
    }
    return rewards;
  }
    ...
    returns (uint256)
  {
    ...
    return (lastFee * (100 - feeDeductionRate)) / 100;
  }
    ...
    returns (uint256)
  {
    ...
    for (uint256 i = 0; i < nodesCount; i++) {
      ...
      console.log("node reward at %s is %s", i, nodeReward);
