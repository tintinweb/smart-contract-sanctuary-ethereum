...
contract AssetManagerUniswap is MultiToken("(AdamUniswap)") {
    ...
    event MockMint(address _user, uint256 _tokenId, uint256 _amount);
    event MockBurn(address _user, uint256 _tokenId, uint256 _amount);
    ...
    receive() external payable {
        // _mint(msg.sender, addressToId[address(0x0)], msg.value, "");
        // emit Deposit(msg.sender, addressToId[address(0x0)], msg.value);
    }

    function execTransaction(address to, bytes memory _data) external payable returns (bool, bytes memory) {
    ...
    function _handleWETH(bytes memory _data) private returns(address, address, uint256, uint256) {
        ...
        if(BytesLib.toBytes4(_data, 0) == 0xd0e30db0) {
            // deposit()
            _mint(msg.sender, addressToId[address(0xc778417E063141139Fce010982780140Aa0cD5Ab)], msg.value, "");
            _burn(msg.sender, addressToId[address(0x0)], msg.value);
        } else if (BytesLib.toBytes4(_data, 0) == 0x2e1a7d4d) {
            // withdraw(uint256)
            _mint(msg.sender, addressToId[address(0x0)], msg.value, "");
            _burn(msg.sender, addressToId[address(0xc778417E063141139Fce010982780140Aa0cD5Ab)], msg.value);
        }
    ...
    function _handleUniswap(bytes memory _data, bytes[] memory _results) private {
        ...
        for(uint i=0; i < multicallBytesArray.length; i++) {
            ...
            if(BytesLib.toBytes4(multicallBytesArray[i], 0) == 0x5023b4df) {
                ...
                _mint(msg.sender, addressToId[address(tokenOut)], amountOut, "");
                _burn(msg.sender, addressToId[address(tokenIn)], amountIn);
            }
