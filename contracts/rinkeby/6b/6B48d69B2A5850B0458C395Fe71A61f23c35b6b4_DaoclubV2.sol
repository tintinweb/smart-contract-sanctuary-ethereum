...
contract DaoclubV2 is ReentrancyGuard, IERC20 {
    ...
    ***********/
    ...
    event LpRedemption(address indexed lpAddress, uint256 daoAmount, uint256 insrueAmount);
    ...
    function init(
        ...
        if (compareStr(targetSymbol, "USDT")) {
            ...
            _targetToken = IERC20(0x0C41477f886F910d285f6d0893780f4D92A8cEE1);  //USDC合约地址0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48 测试地址0x0C41477f886F910d285f6d0893780f4D92A8cEE1
        } else { //weth
    ...
    function lpRedemption() public returns(uint256 ){
        ...
        if(getBalance() < balanceOf(msg.sender)) {
            ...
            _transfer(msg.sender, address(_insuranceDao), insureAmount);
        ...
        pay(msg.sender, daoAmount.add(insureAmount));
        emit LpRedemption(msg.sender, daoAmount, insureAmount);
        return daoAmount.add(insureAmount);
    }

    function pay(address lp, uint256 amount) private {
        if(isETH()) {
            payable(lp).transfer(amount);
        }else {
            _targetToken.safeTransfer(lp, amount);
        }
        if(balanceOf(lp) < amount) {
            amount = balanceOf(lp);
        }
        _burn(lp, amount);
    }
    ...
    function gpLiquidate() public {
        ...
        for(uint i = 0; i < _members.length; i++ ) {
            if(_balances[_members[i]] > 0 && _members[i] != address(_insuranceDao)) {    
                ...
                pay(_members[i], distributeAmount_);
                emit LpRedemption(_members[i], distributeAmount_, 0);
            }
        }        
    }
