...
// File: contracts/reallylongline.sol
...
contract ReallyLongLine is Ownable, ERC721 {
    ...
    uint256 public constant MINT_PRICE = 37500000000000000; // 0.0375 ETH

    uint256 public constant MAX_SUPPLY = 10000;

    bool public MINT_ACTIVE = true;

    string[MAX_SUPPLY]  public _textMessages;



    constructor() ERC721("ReallyLongLine", "SEGMENTS") {}



    function getImage(string[4][3] memory colors) 

            internal

            pure

            returns (string memory) { // non-payable
    ...
    function getColors(uint _tokenId)

            internal pure returns (string[4][3] memory){
    ...
    function randMod(uint min, uint max, uint tokenId, string memory salt)
    ...
    function getPropertiesPartial(string[4][3] memory colors) internal pure returns (string memory) {

        return string(abi.encodePacked('{"c1":"(', colors[0][0], ',', colors[0][1], ',', colors[0][2], ',0.', colors[0][3], ')"',

            ', "c2":"(', colors[1][0], ',', colors[1][1], ',', colors[1][2], ',0.', colors[1][3], ')"',

            ', "c3":"(', colors[2][0], ',', colors[2][1], ',', colors[2][2], ',1.000)"'));
                ...
                Base64.encode(
                    ...
                    bytes(
                        ...
                        abi.encodePacked(
                            ...
                            '{"name":"', "LineSegment #", Strings.toString(_tokenId),

                            '", "description": "Part of the Really Long Line"',
    ...
    function updateTokenText(uint256 _tokenId, string memory _newText) public {

        require(msg.sender == ownerOf(_tokenId), "Only token owner");

        _textMessages[_tokenId] = _newText;
    ...
    function clearTokenText(uint256 _tokenId) public onlyOwner {

        _textMessages[_tokenId] = "";
    ...
    function safeMint(string memory _text, uint numberOfTokensMax5) public payable {
        ...
        require((nextTokenId + numberOfTokensMax5) < MAX_SUPPLY, "Exceeds max supply");

        require(msg.value == (MINT_PRICE * numberOfTokensMax5), "Not enough ETH sent"); 

        for (uint i = 0; i < numberOfTokensMax5; i++) {

            _mint(msg.sender, _nextTokenId);

            _textMessages[_nextTokenId] = _text;

            emit tokenChanged(_nextTokenId);
    ...
    function setMintActive(bool newStatus) public onlyOwner {

        MINT_ACTIVE = newStatus;
