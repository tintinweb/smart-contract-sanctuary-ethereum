...
contract Web3text is ERC721A, Ownable {
    ...
    constructor(address signer) ERC721A("Web3text", "WEB3TEXT") {
        _signer = signer;
    }
    ...
    function mint(string calldata imageURI, bytes calldata signature)
        ...
        payable
    {
        require(status == Status.Started, "WEB3TEXT: Public mint is unactive.");
        require(
            tx.origin == msg.sender,
            "WEB3TEXT: contract isn't allowed to mint."
        );
        
        require(
            _verify(_hash(imageURI), signature),
            "WEB3TEXT: Invalid signature."
        );
    ...
    function refundIfOver(uint256 total) private {
        require(msg.value >= total, "WEB3TEXT: Invalid value.");
        if (msg.value > total) {
    ...
    function withdraw(address payable recipient) external onlyOwner {
        ...
        require(success, "WEB3TEXT: Transfer failed.");
    }
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '","description": "Web3Text is a texts NFT Editors in Web3. Get a FREE texts NFT by web3text.",',
                        '"image": "',
                        imageURI,
                        '","attributes": []}'
                    )
