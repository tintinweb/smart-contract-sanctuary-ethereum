...
// File: contracts/alphaSharksTickets.sol
...
contract AlphaSharksTickets is Ownable {
    ...
    uint256 public whitelistTicketPrice = 0.0000002 ether;
    uint256 public publicTicketPrice = 0.0000003 ether;
    ...
    function phaseOneBuyTicketsMerkle(bytes32[] calldata proof)
        external
        payable
    {
        ...
        /* STEP 2:  Check if user is allowed to buy mint tickets */
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(proof, phaseOneMerkleRoot, leaf),
            "INVALID PROOF"
        );

        /* STEP 3:  Check the tickets amount */
        // Tickets should be 0 initially, if it is not 0, it means user has already bought the ticket
        ...
        require(msg.value == whitelistTicketPrice, "INCORRECT AMOUNT PAID");
    ...
    function phaseOneBuyTicketsMultiple(uint256 _amount) external payable {
        /* STEP 1:  Check if phase is active */
        require(phaseOneActive, "PHASE ONE NOT ACTIVE");
        ...
        /* STEP 5:  Buy the mint tickets */
        addressToTickets[msg.sender] = _amount;
        ticketSales = ticketSales.add(_amount);
    }

    function phaseTwoBuyTicketsMerkle(uint256 _amount, bytes32[] calldata proof)
        external
        payable
    {
        /* STEP 1:  Check if phase is active */
        ...
        /* STEP 5:  Buy the mint tickets */
        // Add total number of tickets for the user
