 ...
 */
contract KingdomsOfEtherAdventurers is PublicSale {
    /* state */
     ...
     */
    function transferToTreasury() external onlyOwner returns (bytes memory) {
        require(treasury != address(0), "zero address");
        (bool _sent, bytes memory _data) = treasury.call{value: address(this).balance}("primary");
...
contract PublicSale is PreSale {
    ...
    PublicSaleConfig public publicSaleConfig = PublicSaleConfig({
        price: 0.145 ether,
        tokensPerTransaction: 11 // 10 + extra 1 for <
    });
    ...
    function mintPublic(uint _count) external payable returns (uint oldIndex, uint newIndex) {
        ...
        require(_count < _cfg.tokensPerTransaction, "publcsale: invalid count");
    ...
    function setPublicSaleConfig(uint128 _price, uint32 _tokensPerTransaction) external onlyOwner {
        publicSaleConfig = PublicSaleConfig({
            price: _price,
            tokensPerTransaction: _tokensPerTransaction + 1
        });
    }
}
 ...
 */
contract Reveal is ERC712 {
    uint public constant GROUPS_PER_ARRAY_ITEM = 16; // 1 uint256 = 16 * uint16
    ...
    function tokenURI(uint _tokenId) external virtual override(IERC721Metadata) view returns (string memory) {
        require(exists(_tokenId), UNKNOWN_TOKEN);
 ...
 */
abstract contract ERC712 is IERC721Enumerable, IERC721Metadata, Ownable {
        ...
        internal returns (uint _oldIndex, uint _newIndex)
    {
        ...
        require(_minted + _amount < maxSupply, TOKENS_OVER);
        ...
        internal returns (uint _oldIndex, uint _newIndex)
    {
        ...
        require(_minted + _total < maxSupply, TOKENS_OVER);
        minted = _minted;
        return (_minted - _total, _minted);
    }

    function exists(uint256 _tokenId) public view returns (bool) {
        return minted > (_tokenId + 1);
    }
    /**
    ...
    function _transfer(
        ...
        require(msg.sender == _owner || getApproved(_tokenId) == msg.sender || isApprovedForAll(_owner, msg.sender), MUST_OWN_OR_APPROVED);
        require(_owner == _from, MUST_OWN);
    ...
    function setApprovalForAll(address _operator, bool _approved) external {
        require(msg.sender != _operator, APPROVE_SELF);
    ...
    /* errors */
    string public constant UNKNOWN_TOKEN = "erc-721: nonexistent token";
    string public constant MUST_OWN_OR_APPROVED = "erc-721: caller is not owner nor approved for all";
    string public constant MUST_OWN = "erc-721: caller is not owner";
    string public constant BAD_SIGNATURE = "eip-721: invalid signature";
    string public constant APPROVE_SELF = "erc-721: approve to caller";
    string public constant PAYMENT_AMOUNT_INCORRECT = "incorrect payment amount";
    string public constant BAD_AMOUNT = "erc-721: bad amount";
    string public constant MINT_CAPACITY_EXCEEDED = "erc-721: mint capacity exceeded";
    string public constant ZERO_ADDRESS = "zero address";
    string public constant TOKENS_PER_ADDR = "tokens per address capacity exceeded";
    string public constant TRANSFER_FAILED = "Failed to send Ether";
    string public constant TOKENS_OVER = "tokens are over";
    string public constant WRONG_STAGE = "wrong stage";
    string public constant SIGNATURE_USED = "SIGNATURE USED";
    string public constant LENGTH_MISMATCH = "array lengths mismatch";
}
