...
contract Frame is 
    ...
    function getTokenList(address owner) public view returns (uint256[] memory) {
        uint256[] memory list = new uint256[](balanceOf(owner));
        uint256 lastTokenId = _tokenIdCounter.current();
        uint256 index;
        console.log("%d, %d",lastTokenId, list.length);
        console.log("%s", _exists(3));
        for (uint256 i; i != lastTokenId;) {
            if(!_exists(i)){
                unchecked {
                    ++i;
                }
