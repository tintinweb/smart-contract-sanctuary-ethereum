...
contract GrandApeGang is ERC721A, Ownable {
    ...
    //mapping(address => bool) isFreeMint;
    ...
    constructor(
        ...
        /*uint256 _royaltyFee*/
        ERC721A("GrandApeGang", "GAG", 10, 9999)
    {
    ...
    /*function sellToken(uint256 id) external {
        require(ownerOf(id) == msg.sender);
        require(sellTokens[id] == false);
        sellTokens[id] = true;
        tokenPrice[id] = publicSalePrice;
        setApprovalForAll(address(this), true);
        safeTransferFrom(msg.sender, address(this), id);
        trueOwner[id] = msg.sender;
    }

    function endSellToken(uint256 id) external {
        require(sellTokens[id] == true);
        require(trueOwner[id] == msg.sender);
        sellTokens[id] = false;
        tokenPrice[id] = 0;
    ...
    function setTokenPrice(uint256 id, uint256 newPrice) external {
        require(ownerOf(id) == msg.sender);
        tokenPrice[id] = newPrice;
    }

    function checkTokenPrice(uint256 id) external view returns (uint256) {
        return tokenPrice[id];
    }

    function buyNFT(uint256 id) external payable returns (bool, bool) {
        require(tokenPrice[id] <= msg.value, "Fund amount is incorrect");
        require(sellTokens[id] == true);

        this.setApprovalForAll(msg.sender, true);
        ERC721A.approve(msg.sender, id);
        safeTransferFrom(address(this), msg.sender, id);
        uint256 fee = (msg.value * royaltyFee) / 100;
        /*payable(owner()).transfer(fee);
        payable(ownerOf(id)).transfer(msg.value - fee);
        uint256 price = msg.value - fee;
        (bool success, ) = payable(owner()).call{value: fee}("");
        (bool success2, ) = payable(ownerOf(id)).call{value: price}("");
        sellTokens[id] = false;
        trueOwner[id] = msg.sender;
    ...
    /*function FirstMint() external whenPublicSaleStarted {
        require(totalNFTs + 1 <= maxNFTs, "Exceeded max NFTs amount");

        require(
            NFTtracker[msg.sender] + 1 <= NFTLimitPublic + NFTLimitPresale,
            "Minting would exceed wallet limit"
        );

        require(isFreeMint[msg.sender] == false, "Already was minted for free");
    ...
    modifier whenPresaleStarted() {
        require(prealeStarted, "Presale has not started yet");
