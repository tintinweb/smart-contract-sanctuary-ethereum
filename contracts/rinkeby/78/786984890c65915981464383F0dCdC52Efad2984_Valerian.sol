...
contract Valerian is ERC721A, Ownable {
    ...
    uint256 internal constant MAX_SALE_LIMIT = 8499;
    uint256 internal constant MAX_PRESALE_LIMIT = 7500;
    uint256 internal constant NORMAL_SALE_ONCE_LIMIT = 3;
    uint256 internal constant PRESALE_ONCE_LIMIT = 2;
    ...
    function presale(uint256 num, bytes calldata signature) public payable {
        require(
            _totalMinted() + num <= MAX_PRESALE_LIMIT,
            "Valerian: reached max limit at presale"
        );
        require(
            preSaleStartTime <= block.timestamp &&
                block.timestamp < preSaleEndTime,
            "Valerian: presale not started"
        );
        require(
            msg.value == num * preSalePrice,
            "Valerian: presale price error, please check msg.value."
        );
        require(
            owner() ==
                keccak256(
        ...
        require(
            _numberMinted(_msgSender()) + num <= PRESALE_ONCE_LIMIT,
    ...
    function mint(uint256 num) public payable {
        require(
            ...
            "Valerian: reached max limit at public sale"
        );
