...
contract tubbytestgan is ERC721A, Ownable {
    ...
    constructor(bytes32 _merkleRoot) ERC721A("tubbyxpgan", "TUBBYPXGAN") {
...
function whitelistMint(bytes32[] calldata _merkleProof, uint256 amountToMint, uint256 merkleProofAmount, address _addr) external {
    ...
    require(whitelistMinted[_addr] <= merkleProofAmount, "you have no more free mints");

    FREE_MINT_SUPPLY -= amountToMint;
    
    _mint(msg.sender, amountToMint);
    
    bytes32 leaf = keccak256(abi.encodePacked(_addr, merkleProofAmount.toString()));
    require(
