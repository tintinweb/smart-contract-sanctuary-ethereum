...
contract BASECONTRACT is ERC721A, Ownable {
    ...
    function flipPauseStatus() public onlyOwner {
      ...
      function flipHoldersPausable() public onlyOwner {
        HoldersPausable = !HoldersPausable;
    }
    ...
    function setPresale(uint256 _presale) public onlyOwner {
        require(_presale < maxSupply, "amount exceeds");
        presale = _presale;
    }
    ...
    function setDeployedDragon(address  contractaddress) public onlyOwner {
        DeployedDragons = IERC721(contractaddress);
    }


    function mint(uint256 quantity) public payable {
        require(quantity > 0 ,"quantity should be greater than 0");
        require(totalSupply()+quantity<=maxSupply-reserve,"all tokens have been minted");
        ...
        payable
    {
        ...
        require(pausable == true, "turn on minting");
         require(_preSaleAddresses[msg.sender].counter + quantity <= maxPerWallet, "Sorry can not mint more than maxwallet");
        ...
        function mintForNftHolders(bytes32[] calldata _merkleProof)
        public
        
    {
       
        require(HoldersPausable == true, "turn on minting");
        // require(quantity > 0, "zero not allowed");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require( MerkleProof.verify(_merkleProof, merkleRoot, leaf),"Invalid Proof");
        uint holderBalance=DeployedDragons.balanceOf(msg.sender);
