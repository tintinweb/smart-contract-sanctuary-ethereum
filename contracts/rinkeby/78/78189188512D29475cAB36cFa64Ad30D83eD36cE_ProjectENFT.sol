...
contract ProjectENFT is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
  ...
  constructor(string memory _unrevealURI, string memory _initbaseURI) ERC721A("ProjectEToken","ProET", MaxMintPerBatch_, TotalCollectionSize_) {
    setURIbeforeReveal(_unrevealURI);
  ...
  function mint(uint256 quantity, uint256 [] memory nfttype_numbers) external payable callerIsUser {
    ...
    for (uint256 i = 0; i < nfttype_numbers.length; i++) {
      ...
      require(nfttype_number == 0 || nfttype_number == 1, "nfttype_number must be 0, or 1");
  ...
  function changeMintPrice(uint256 _newPriceA,uint256 _newPriceB) external onlyOwner
  {
      PRICE_A = _newPriceA;
      PRICE_B = _newPriceB;
  }
