...
contract LANDRegistry is Storage, FullAssetRegistry, ILANDRegistry {
  ...
  mapping(int=>mapping(int=>uint))public getlandIds;
  mapping(int=>mapping(int=>bool)) public isMintedLand; 
  ...
  function buyNewParcel(int x, int y,uint tokenId, address beneficiary,string memory  uri) external{
      require(!isMintedLand[x][y],"Already Co-Ordinates are Minted");
      require(Operator==msg.sender,"!No Permission");
      getlandIds[x][y]=tokenId;
    _generate(tokenId, beneficiary);
    __uris[tokenId]=uri;
    isMintedLand[x][y]=true;
...
//   }
  ...
  function _ownerOfLand(int x, int y) internal view returns (address) {
    return _ownerOf(getlandIds[x][y]);
  }
  ...
  function landData(int x, int y) external view returns (string memory) {
    return _tokenMetadata(getlandIds[x][y]);
  }
  ...
  function transferLand(int x, int y, address to) external {
    uint256 tokenId = getlandIds[x][y];
    _doTransferFrom(
      _ownerOf(tokenId),
      to,
      tokenId,
  ...
  function transferManyLand(int[] memory x, int[] memory  y, address to) external {
    ...
    for (uint i = 0; i < x.length; i++) {
      uint256 tokenId = getlandIds[x[i]][y[i]];
      _doTransferFrom(
        _ownerOf(tokenId),
        to,
        tokenId,
  ...
  */
  ...
  function _updateLandData(
    ...
    internal
    onlyUpdateAuthorized(getlandIds[x][y])
  {
    uint256 assetId = getlandIds[x][y];
