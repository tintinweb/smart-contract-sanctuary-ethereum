...
contract Trade {
    ...
    function buyAsset(Order calldata order, Sign calldata sign) external returns(bool) {
        require(IERC20(order.erc20Address).allowance(order.buyer, address(transferProxy)) >= order.amount,"ERC20: transfer amount exceeds balance");
        require(IERC20(order.erc20Address).balanceOf(order.buyer) >= order.amount, "ERC20: insufficient allowance");
