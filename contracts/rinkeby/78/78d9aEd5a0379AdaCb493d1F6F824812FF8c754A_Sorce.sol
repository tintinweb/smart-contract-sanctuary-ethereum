...
contract Sorce is ERC1155, Ownable, ERC1155Burnable, ReentrancyGuard {
    constructor() ERC1155("") {}
  ...
  function migrateToken() public payable  {
      require(migrationStarted == true, "Migration has not started");
      require(balanceOf(msg.sender, tokenId) > 0, "Doesn't own the token"); // Check if the user own one of the sorce
      burn(msg.sender, tokenId, 1); // Burn the Sorce
      MintableInterface dr1verContract = MintableInterface(dr1verContractAddress);
      dr1verContract.mintTransfer{value:msg.value}(msg.sender); // Mint the Dr1ver
  }
