...
contract Collectible is ERC721MULTI {   
  using Strings for uint256;
    ...
    constructor() {
        owner = _msgSender();
        baseUri = "https://gateway.pinata.cloud/ipfs/QmaBLiYLVMoTc7GV9ZuL1fkMxm6vFA1oJe7FnhWCsCwV94/";
    }
...
contract ERC721MULTI is Context {
    ...
    function transferFrom(
        ...
        if(from != ownings[id]) revert TokenAlreadyMinted();
        if(to == address(0)) revert ZeroAddress();
        if(_msgSender() != from 
            || _msgSender() != approvements[id] 
    ...
    function safeTransferFrom(
        ...
        if(!calledByUser() || !_checkOnERC721Received(from, to, data)) revert TransferToNonERC721ReceiverImplementer();
    }
    ...
    function ownerOf(uint256 tokenId) external view returns (address)
    {
        ...
        uint256 _tokenCounter = state.tokenCounter;
        uint256 _maxMintsPerUser = state.maxMintsPerUser;
        uint256 lowestTokenToCheck;

        unchecked{
            if(currentId < _tokenCounter)
            {
                if (tokenId >= _maxMintsPerUser)
                {
                  lowestTokenToCheck = _tokenCounter - _maxMintsPerUser + 1;
                }

                do
                {
                    address ownership = ownings[currentId];
