    ...
    ReentrancyGuardUpgradeable
{
    ...
    */
    function spectrumMint(uint256[] memory _tokenIds, address _to, uint256[] memory _amounts)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(spectrumPass.ownerOf(_tokenIds[i]) == _to, "Spectrum: !Owner");
    ...
    */
    function presaleMint(whitelist memory _whitelist, address _to, uint256 _amount)
        external
        whenNotPaused
        nonReentrant
        checkSupply(_amount)
    ...
    @param _amount Amount of tokens to mint   
    */
    function publicMint(uint256 _amount, address _to)
        external 
        whenNotPaused 
        nonReentrant 
        checkSupply(_amount) 
    {
        require(_to == tx.origin, "PublicSale: Only wallets allowed");
