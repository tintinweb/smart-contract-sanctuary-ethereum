...
contract LazyDragon is ERC721AUpgradeable, OwnableUpgradeable {
    ...
    uint256 public totalSupplyAdmin;
    ...
    function whitelistMint(uint256 _amount, bytes32[] calldata _merkeProof)
        external
        payable
        whitelistSaleCompliance(_amount)
        whitelistCompliance(_merkeProof, msg.sender)
    {
        require(
            _numberMinted(msg.sender) + _amount <= maxPerAddressTotal,
            "Max mint limit"
        );
        require(_whitelistMinted[msg.sender] + _amount <= maxPerTx);
        ...
        whitelistCompliance(_merkeProof, msg.sender)
    {
        ...
        require(_presaleMinted[msg.sender] + _amount <= maxPerTx);
    ...
    function numberWhitelistMinted(address _owner) public view returns (uint256) {
    ...
    // Owner functions
    function adminMint(uint256 _amount) external payable onlyOwner {
        require(totalSupply() + _amount <= MAX_SUPPLY, "Not enough mints left");
        totalSupplyAdmin += _amount;
        _safeMint(msg.sender, _amount);
    }
    ...
    function setMaxPerTxPublic(uint256 _maxPerTxPublic) external onlyOwner {
        maxPerTxPublic = _maxPerTxPublic;
    }

    function setMaxPerAddressTotal(uint256 _maxPerAddressTotal)
        external
        onlyOwner
    {
        maxPerAddressTotal = _maxPerAddressTotal;
    }
    ...
    // Modifiers
    modifier preSaleCompliance(uint256 _amount) {
        require(tx.origin == msg.sender, "No contract minting");
        require(isPresaleActive, "Presale is not open");
        require(_amount <= maxPerTx, "Too many mints per tx");
        require(
            totalSupplyPresale + _amount <= presaleSupply,
            "Not enough presale mints left"
        );
        _;
    }

    modifier whitelistSaleCompliance(uint256 _amount) {
        require(tx.origin == msg.sender, "No contract minting");
        require(isWhitelistSaleActive, "Whitelist sale is not open");
        require(_amount <= maxPerTx, "Too many mints per tx");
        require(
            totalSupplyWhitelist + _amount <= whitelistSupply,
            "Not enough whitelist mints left"
        );
        _;
    }
    ...
    modifier mintCompliance(uint256 _amount) {
        ...
        require(_amount <= maxPerTxPublic, "Too many mints per tx");
        require(totalSupply() + _amount <= MAX_SUPPLY, "Not enough mints left");
        _;
    }
