...
contract EasyDemons is ERC721EnumerableM, Ownable {
    using Strings for uint256;
    bytes32 public root;
    bytes32 public devMintRoot;
    ...
    uint256 public constant RAFFLE_PRICE = 0.13 ether;
    // end::Prod
    ...
    address constant EASYDEMONS_WALLET = 0x71D54d6Dd26339B4d69C5cd922C4846aC6fb5E95;
    ...
    function setDevMintRoot(bytes32 merkleroot) onlyOwner public {
        devMintRoot = merkleroot;
    }

    function setRaffleRoot(bytes32 merkleroot) onlyOwner public {
        raffleRoot = merkleroot;
    }

    function devMintQtyRemaining() public view returns (uint256) {
        return MAX_DEVMINT_SUPPLY - devMintQtyMinted;
    }

    function presaleQtyRemaining() public view returns (uint256) {
        return MAX_PRESALE_SUPPLY - presaleQtyMinted;
    }
    ...
    function raffleSaleSupply() public view returns (uint256) {
        return presaleQtyRemaining() + publicSaleQtyRemaining();
    }
    ...
    function devMint(address to, uint256 tokenQuantity) external onlyOwner {
        ...
        require(stage == Stage.Presale || stage == Stage.RaffleSale, "not active");
        require(supply <= TOTAL_SUPPLY, "all minted");
        require(devMintQtyMinted < MAX_DEVMINT_SUPPLY, "empty");
    ...
    function presaleMint(uint256 tokenQuantity, uint256 allowance, bytes32[] calldata proof) external payable {
        uint256 supply = totalSupply();
        require(_verify(_leaf(msg.sender, allowance), proof), "invalid MP");
        require(stage == Stage.Presale, "not active");
        require(supply <= TOTAL_SUPPLY, "exceed sale alloc");
        require(presaleQtyMinted <= MAX_PRESALE_SUPPLY, "exceed presale alloc");
    ...
    function raffleMint(uint256 tokenQuantity, uint256 allowance, bytes32[] calldata proof) external payable {
        uint256 supply = totalSupply();
        require(_verify(_leaf(msg.sender, allowance), proof), "invalid MP");
        require(stage == Stage.RaffleSale, "not active");
        require(supply <= TOTAL_SUPPLY, "exceed sale alloc");
        require(raffleQtyMinted <= raffleSaleSupply(), "exceed raffle alloc");
        require(tokenQuantity <= allowance, "exceed trx allow");
        require(balanceOf(msg.sender) <= tokenQuantity + allowance, "exceed wal allow");
        require(price() * tokenQuantity == msg.value, "ETH not match");

        raffleSalePurchases[msg.sender] += 1;
    ...
    function publicMint(uint256 tokenQuantity) external payable {
        ...
        require(supply + tokenQuantity <= TOTAL_SUPPLY, "exceed sale alloc");
        ...
        if(isDutchAuction == true) {
            require(block.timestamp >= _startAt, "auction not started");
        }
    ...
    function _claimableAmount(address buyer) internal view returns (uint256) {
        TokenBatchPriceData[] memory histories = userToTokenBatchPriceData[buyer];
        uint256 claimableAmount = 0;


        for(uint256 i; i < histories.length; i++) {
            claimableAmount += histories[i].pricePaid - ( finalPrice * histories[i].qtyMinted );
        }
    ...
    function claimRefund() external {
        ...
        require(address(this).balance >= _claimableAmount(msg.sender), "not enough balance");

        Address.sendValue(payable(msg.sender), _claimableAmount(msg.sender));
    }
