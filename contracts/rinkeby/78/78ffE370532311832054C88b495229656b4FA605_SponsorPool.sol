...
abstract contract ManagedPool is AccessControl, ERC20 {
  ...
  function mint(uint256 _stakeAmount) external returns (uint256 shares) {
    ...
    if (oldShares != 0 && oldBalance != 0) {
      ...
      // SponsorPool - all tokens are removed as bonds  or pool fees are 100%
      // Both Pools - Fees are set to 100% 
      
      // we make the new shares worth 99.9999% of the pool
      shares = oldShares * 10**4;
