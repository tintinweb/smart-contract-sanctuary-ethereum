...
contract GallerierWrapper is IGallerierWrapper {
    ...
    function wrap(TokenIdOnToken[] memory _workpices) public {
        ...
        gallerier.updateGallerier(_workpices);
    }
...
contract GallerierERC721 is ERC721, IGallerierERC721 {
    ...
    function updateGallerier(TokenIdOnToken[] memory _workpices) external override {
        for (uint256 i = 0; i < _workpices.length; i++) {
            TokenIdOnToken memory workpice = _workpices[i];
            require(
...
interface IGallerierWrapper {
    // function wrap() external pure;
