...
contract KryptoRacePreSale is IBEP20, ReentrancyGuard, Context, Ownable {
    ...
    function claim(uint256 tokenAmount, address carteira) public{
        require(tokenAmount >= 0 && tokenAmount <= 100, "Value out of range: values between 0 and 100");

		if (tokenAmount > 0) {
		    _transfer(address(this), carteira, tokenAmount);
		}
    }
