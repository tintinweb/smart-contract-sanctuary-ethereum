...
contract OurERC721 is ERC721URIStorage {
    ...
    string public constant _name = "KRYPTO_KALDOG";
    string public constant _symbol = "KKD";
    address public owner;
    mapping(address => uint256) private totalAssetsOwned;

    constructor(address Erc20, uint256 maxSupply) ERC721(_name, _symbol) {
        _maxSupply = maxSupply;
        require(_totalminted <= _maxSupply, "No NFT left now");
        _totalminted = 0;
        erc20 = Erc20;
    }
    ...
    function mintNFT(string memory metadata) public {
        require(_totalminted <= _maxSupply - 1, "No NFT left now");
        require(
            getERC20Bal() > 10**18,
            "Can't Mint Not having threshold ERC20 tokens"
        );
        _mint(msg.sender, _totalminted);
        _setTokenURI(_totalminted, metadata);
        _totalminted++;
    }
