...
/**
 * @title GLRAirdrop
 * @notice It distributes GLR tokens with a Merkle-tree airdrop.
 */
contract GLRAirdrop is Pausable, ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    IERC20 public immutable GLRToken;
     ...
     * @param _GLRToken address of the GLR token
    ...
    function claim(uint256 amount, bytes32[] calldata merkleProof) external whenNotPaused nonReentrant {
        ...
        // Transfer tokens
        GLRToken.safeTransfer(msg.sender, amount);

        emit AirdropRewardsClaim(msg.sender, amount);
    }

    /**
     * @notice Check whether it is possible to claim (it doesn't check claimed)
    ...
    function withdraw() external onlyOwner {
        uint256 balanceToWithdraw = GLRToken.balanceOf(address(this));
        GLRToken.safeTransfer(msg.sender, balanceToWithdraw);
