...
interface IVeNFT {
  ...
  function nftLocked(uint256 _tokenId) external view returns (LockedBalance memory);
...
contract Lend is IERC721Receiver, ReentrancyGuard {
  ...
  function getAmount(uint256 _tokenId, address _veNFTAddress) internal view returns (uint256) {
    return (IVeNFT(_veNFTAddress).nftLocked(_tokenId)).amount;
  }

  function getMaxCheeTokenAmount(address _veNFTAddress, uint256 _nftId) public view returns (uint256) {
    return getAmount(_nftId, _veNFTAddress).mul(maxLTV).div(1e18);
  }
  ...
  function mint(
    ...
    require(_cheeTokenAmount <= getAmount(_nftId, _veNFTAddress).mul(maxLTV).div(1e18), "Value greater than max");
