...
// Thanks to the CellToken($CELL) author for his work and made a little optimization on this basis( _remove() ).
contract CaesarToken is ERC20Burnable,Ownable {

    address public IAPTokenAddress;
    uint256 public constant MAX_WALLET_STAKED = 10;
    uint256 public constant EMISSIONS_RATE = 115_740_740_740_741;
    uint256 public constant MAX_CT = 1e26;
    ...
    constructor(address tokenAddr) ERC20("CaesarToken","CT") {
        IAPTokenAddress  = tokenAddr;
    }

    function setStakeToken(address newStakeToken) external onlyOwner {
        IAPTokenAddress = newStakeToken;
    }
    ...
    function stakeByIds(uint256[] calldata tokenIds) external {
        require(
            ...
            "Must have less than 10 token staked!"
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721Enumerable(IAPTokenAddress).ownerOf(tokenIds[i]) == msg.sender,
                "You don't own this token!"
            );
            require(tokenIdToStaker[tokenIds[i]] == address(0), "Token is already being staked!");

            IERC721Enumerable(IAPTokenAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
    ...
    function getRewardsByTokenId(uint256 tokenId) external view returns (uint256) {
        require(tokenIdToStaker[tokenId] != address(0), "token is not staked!");
    ...
    function _remove(address staker, uint256 index) internal {
        ...
        if (index != stakerToTokenIds[staker].length - 1) {
            uint256 last = stakerToTokenIds[staker].length - 1;
            uint256 t = stakerToTokenIds[staker][last];
            stakerToTokenIds[staker][last] = stakerToTokenIds[staker][index];
            stakerToTokenIds[staker][index] = t;
        }
    ...
    function _getMaximumRewards(uint256 calculatedRewards) internal view returns(uint256) {
        uint256 totalCTAvailable = MAX_CT - totalSupply();
        return totalCTAvailable > calculatedRewards ? calculatedRewards : totalCTAvailable;
    }
}
