...
contract NFTSpaceXAuction is NFTAuction {
  constructor(
...
contract NFTAuction is Context, Ownable, ReentrancyGuard {
  ...
  uint16 public constant DELAY_SETTLE_TIME = 60;
  ...
  modifier onlyAdmin() {
    require(nftspacexAccessControls.hasAdminRole(_msgSender()), "NSA: only admin");
    _;
  }

  modifier onlyAdminOrMinter() {
    require(nftspacexAccessControls.hasAdminRole(_msgSender()) || nftspacexAccessControls.hasMinterRole(_msgSender()), "NFTA: only admin or minter");
    _;
  }
  ...
  function _doesSettleMeetRequirements(address _nftContractAddress, uint256 _tokenId) internal view returns(bool) {
    ...
    if (_msgSender() == owner() && nftContractAuctions[_nftContractAddress][_tokenId].expiration + DELAY_SETTLE_TIME <= block.timestamp) {
        return true;
    }
    return false;
  }
  ...
  function settleAuction(address _nftContractAddress, uint256 _tokenId)
    ...
    settleAuctionMeetsRequirements(_nftContractAddress, _tokenId)
  {
  ...
  /**
    ...
    */
  function withdrawAllFailedCredits() public onlyAdminOrMinter nonReentrant {
