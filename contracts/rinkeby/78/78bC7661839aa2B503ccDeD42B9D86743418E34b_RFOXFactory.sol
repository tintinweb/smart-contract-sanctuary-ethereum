...
contract RFOXFactory is Ownable {
    ...
    function createNFT(
        ...
        bytes memory bytecode = type(RFOXNFT).creationCode;
        bytes32 salt = keccak256(
            abi.encodePacked(allNFTs.length, name, symbol)
        );
    ...
    Ownable
{
    ...
    event MaxMintedPresalePerAddress(
        address indexed sender,
        uint256 oldValue,
        uint256 newValue
    );
     ...
     * 1. If whitelist activated:
            - If users is not whitelisted, they can only mint after the publicSaleStartTime and before the saleEndTime.
            - If users is whitelisted, they can mint after saleStartTime and before the saleEndTime.
    ...
    function initialize(
        ...
        IERC20 token,
        uint256 price,
        uint256 maxNft, /// Max total supply of NFT
        ...
        require(
            publicSaleStartTime_ >= saleStartTime_,
    ...
    function setMaxMintedPresalePerAddress(uint256 newMaxMintedPresale) external onlyOwner {
        require(newMaxMintedPresale <= MAX_NFT, "Invalid value");
        uint256 _oldMaxMintedPresalePerAddress = maxMintedPresalePerAddress;
        maxMintedPresalePerAddress = newMaxMintedPresale;

        emit MaxMintedPresalePerAddress(
            msg.sender,
            _oldMaxMintedPresalePerAddress,
    ...
    function withdraw() external onlyOwner {
        ...
        if (address(saleToken) == address(0)) {
            ...
            (bool succeed, bytes memory data) = msg.sender.call{value: balance}(
                ""
            );
    ...
    /// @notice Function that allow user to buy NFTs for whitelisted address / presale
    /// @dev Only one purchase is allowed
    ...
    function _buyNFTs(uint256 tokensNumber) private {
        // require(
        //     _tokenIdCounter.current().add(tokensNumber) <= MAX_NFT,
