...
contract SleepySniperTest is ERC721A {
    /// @notice baseURI, usually represents ipfs gateway link
    string _baseURIVal;
    string constant _prerevealUri = "https://gateway.pinata.cloud/ipfs/QmcXT1SH7UDq1U9Br1H8Q6hjMjRWXK7WhyChqFaNBuXajm";
    bool _revealed = false;
    bytes32 _allowListMerkleRoot;
    mapping(address => uint) _allowListMinted;
    mapping(address => bool) _admins;

    enum CurrentSalePhase { AllowList2, AllowList3, PublicSale, Paused, Stopped } // allowlist 2 tokens, allowlist 3 tokens, public sale
    CurrentSalePhase public currentPhase = CurrentSalePhase.AllowList2;

    uint public constant AllowListMaxMintsFirstPhase = 2;
    uint public constant AllowListMaxMintsSecondPhase = 3;

    uint constant AllowListPrice = 90000000000000000; // 0.09eth in wei
    uint _publisSalePrice = 120000000000000000; // 0.12eth in wei

    uint public constant TotalSupplyCap = 5000;

    /**
     * @dev Throws if called by any account other than admins (_admins mapping).
     */
    modifier onlyAdmin() {
        require(_admins[msg.sender], "AdminCheck: caller is not the admin");
        _;
    }

    constructor(string memory name, string memory symbol, bytes32 whiteListMerkleRoot, uint premintTokensNumber, address[] memory admins ) ERC721A(name, symbol){
        _allowListMerkleRoot = whiteListMerkleRoot;
        for (uint i=0; i<admins.length; i++) {
            _admins[admins[i]] = true;
        }
        _safeMint(msg.sender, premintTokensNumber);
    }

    /// @notice sets current phase of sale (for now for simplicity and debugging)
    /// @param phase CurrentSalePhase  - current phase of sale
    function setCurrentPhase(CurrentSalePhase phase) external onlyAdmin {
        currentPhase = phase;
    }

    /// @notice Sets allow list merkle root
    /// @param merkleRoot byte32
    function setAllowListMerkleRoot(bytes32 merkleRoot) external onlyAdmin {
        _allowListMerkleRoot = merkleRoot;
    }

    /// @notice Sets _publisSalePrice
    /// @param publicSalePrice uint of new public sale price
    function setPublicSalePrice(uint publicSalePrice) external onlyAdmin {
        _publisSalePrice = publicSalePrice;
    }
    ...
    function _allowListTotalTokenAllowed() view internal returns (uint) {
        require(currentPhase == CurrentSalePhase.AllowList2 || currentPhase == CurrentSalePhase.AllowList3,  "AllowList mint are not allowed on this phase");
        if(currentPhase == CurrentSalePhase.AllowList2) {
    ...
    function mint(uint numberTokensToMint) external payable {
        ...
        require(msg.value >= numberTokensToMint*_publisSalePrice, "Insufficient funds provided");
    ...
    function withdrawMoneyTo(address payable _to, uint amount) public  onlyAdmin {
