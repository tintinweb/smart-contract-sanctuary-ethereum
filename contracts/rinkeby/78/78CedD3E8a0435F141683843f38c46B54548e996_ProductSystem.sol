...
contract ProductSystem{
    ...
    //根据交易下标查询消费记录
    function messageQueryByIndex(uint256 _index) public view returns(Message memory){
    require(_index < roles[msg.sender].messageCount,"error,_index>=message count!");
    ...
    //根据仓库下标返回仓库信息
    function warehouseQueryByIndex(uint256 _index) public view returns(Warehouse memory){
        require(_index < roles[msg.sender].warehouseCount,"error,_index>=warehouse count!");
        Warehouse memory warehouse;
        if(_index<roles[msg.sender].warehouseCount) { 
            warehouse=roles[msg.sender].warehouses[_index];
        } 
       return warehouse;
    }
