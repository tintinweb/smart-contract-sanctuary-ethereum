...
contract BAPOrchestrator is Ownable, IERC721Receiver {
    ...
    function openChest(
        ...
        uint256 num = random(seed) % 100; // NUM BETWEEN 0 - 999
        uint256 prize;

        if (num < 10) {
            prize = 20 + guild;
            bapUtilities.airdrop(msg.sender, 1, (prize)); // UTILITIE #20 - 23 METH MAKER - 10% (num between 0 - 9)
        } else if (num < 40) {
            prize = 30 + guild;
            bapUtilities.airdrop(msg.sender, 1, (prize)); // UTILITIE #30 - 33 RESURRECTION - 30% (num between 10 - 39)
        } else {
            prize = 40 + guild;
            bapUtilities.airdrop(msg.sender, 1, (prize)); // UTILITIE #40 - 43 BREED REPLENISH - 60% (num between 40 - 099)
        }
    ...
    function useItem(
        ...
        require(
            _verifyHashSignature(
                keccak256(abi.encode(msg.sender, item, tokenId)), // update this signature
                signature
            ),
            "Signature is invalid"
        );
    ...
    function _burnTeen(uint256 tokenId) internal {
        require(teenLastClaim[tokenId] == 0, "this teen has already resurrect");
        require(
            bapTeenBulls.ownerOf(tokenId) == msg.sender,
    ...
    function initializeGodMintingDate(
        ...
        for (uint256 i; i < gods.length; i++) {
            godsMintingDate[gods[i]] = mintingDates[i];
        }
    }
