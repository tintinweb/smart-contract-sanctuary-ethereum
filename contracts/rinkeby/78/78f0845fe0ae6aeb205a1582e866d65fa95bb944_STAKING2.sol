...
contract STAKING2 is Ownable {
    ...
    uint256 private _lockTimePeriod = 300;
    // lockTime period default 1 week
    // 60 * 60 * 24 * 7 =604800 sec
    uint256 private coolingTime = 180;
...
function unlockTimer(address addr) public view returns(uint256){
    require(unlockTime[addr] > 0 ,"stake some amount first");
    return unlockTime[addr] - block.timestamp;
}
function coolTimer(address addr) public view returns(uint256){
     require(_coolingTime[addr] > 0 ,"stake some amount first");
    return _coolingTime[addr] - block.timestamp;
}
}