...
contract TestB is ERC721Enumerable, Ownable {
    ...
    uint256 public _numAvailableTokens = 10;

    string public _tokenBaseURI;

    mapping(uint256 => bool) public _indexMap;
    ...
    function burnAndMint(uint256[] memory tokenIds) public {
        ...
        require(
            Contract(ContractA).isOwnerOfAll(msg.sender, tokenIds),
            "Your not the owner of all the token in list"
        );
        for (uint256 i = 0; i < tokenIds.length; i++) {
            Contract(ContractA).burn(tokenIds[i]);
            uint256 randomTokenId = useRandomAvailableToken(i, i);
    ...
    function setContractAddr(address contractAddr) public onlyOwner {
        ContractA = contractAddr;
    }
