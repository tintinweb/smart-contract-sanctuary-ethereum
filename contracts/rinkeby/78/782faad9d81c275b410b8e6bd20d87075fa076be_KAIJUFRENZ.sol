...
contract KAIJUFRENZ is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable {
    ...
    uint256 private constant MAX_SUPPLY = 6666; // 6666 KAIJUFRENZ
    ...
    uint256 private constant PRICE_PUBLIC = 0.15 ether; // Public Sale Price

    uint256 private _price;
    uint256 private _maxMint;

    address private devWallet = payable(0x68df7639ef63fA25D618cdEEBbdd1093d37c8e18); // Developer Wallet Address
    address private fundWallet = payable(0x476E35cc46f86B3028Ce0c2bD0C8496b6e79f47f); // Owner Fund Wallet Address

    event KAIJUFRENZummoned(address to, uint256 indexed id);

    modifier saleIsOpen {
        if (_msgSender() != owner()) {
            require(SALE_OPEN == true, "SALES: Please wait a big longer before Skull Toon ;)");
        }
    ...
    constructor() ERC721("KAIJUFRENZNFT", "KAIJUFRENZ") {
    ...
    //     emit KAIJUFRENZummoned(msg.sender, newItemId);
    // }

    // Trying to mint multiple nfts at once
    function mint(uint times) public payable saleIsOpen {
        uint256 total = _totalSupply();
        uint256 maxNFTSupply = MAX_SUPPLY;
        // uint256 maxMintCount = _maxMint;
        uint256 price = _price;
        ...
        for(uint i = 0; i < times; i++){
            uint256 newItemId = _tokenIdCounter.current();
            _safeMint(msg.sender, newItemId);
            string memory finalTokenUri = "QmWnkgUp1Pc4nc4o72MY8Xr86HmnyycHQMaMvZrJUkUZfn";
            _setTokenURI(newItemId, finalTokenUri);
            _tokenIdCounter.increment();
            emit KAIJUFRENZummoned(msg.sender, newItemId); 
        }
