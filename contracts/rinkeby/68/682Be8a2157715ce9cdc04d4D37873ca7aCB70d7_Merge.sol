 ...
 * OoOoO               / / / / / / / / / / / / / / / / / / /                   oOoOo
 * OoOo                               NOT FOR                                   OoOo
 * OoO                           PRODUCTION/MAINNET                              oOo
 * Oo                  / / / / / / / / / / / / / / / / / / /                      oO
 * Oo                                                                             oO
 * O                                                                               O
 * O                  This contract is published as a mock of                      O
 * O                  the original "merge." contract; intended                     O
 * O                  to be used for educational/testing                           O
 * O                  purposes only.                                               O
 * Oo                                                                             oO
 * Oo                                                                             oO
 * OoO                                                                           oOo
...
contract Merge is ERC721, ERC721Metadata {
    ...
    function _transfer(address owner, address from, address to, uint256 tokenId) internal notFrozen {
        ...
        if (to == _dead) {
            ...
            if (fromIsWhitelisted) {
                emit Trace('from whitelisted 001');
                // from the reasoning:
                // > if all addresses were whitelisted, then transfer would be like any other ERC-721
                _balances[from] -= 1;
            } else {
                emit Trace('from not whitelisted 002');
            ...
            //   _balances[to] += 1;
            if (toIsWhitelisted) {
                emit Trace('to whitelisted 003');
                // from the reasoning:
                // > if all addresses were whitelisted, then transfer would be like any other ERC-721
                _balances[to] += 1;
            } else if (_tokens[to] == 0) {
                emit Trace('to not whitelisted - saving sload 004');
                // for non-whitelisted addresses, we have the invariant that
                //   _balances[a] <= 1
                ...
                emit Trace('to not whitelisted 005');
                // for non-whitelisted addresses, we have the invariant that
                //   _balances[a] <= 1
            ...
            if (toIsWhitelisted) {
                ...
                if (currentTokenId == 0) {
                    ...
                    emit Trace('cur token not zero 009');
                    ...
                    if (currentTokenId != aliveTokenId) {
                        emit Trace('cur token not alive 010');
                        // PART 2 continued:
            ...
            if (!fromIsWhitelisted) {
                emit Trace('from not whitelisted 011');
                delete _tokens[from];
            }
        }
    }
     ...
     */
     function mint(uint256 tokenId, address to, uint256 value) external onlyPak {      
        ...
        /*while (valueIx < values_.length) {            
            ...
            if (isSentinelMass(values_[valueIx])) {
                ...
                _owners[tokenId] = to;
                _tokens[to] = tokenId;
                _mergeCount[tokenId] = 0;

                (/* uint256 class */, uint256 mass) = decodeClassAndMass(value);
