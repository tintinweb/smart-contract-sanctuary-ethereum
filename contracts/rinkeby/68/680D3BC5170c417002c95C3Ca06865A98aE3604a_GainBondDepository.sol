...
interface INoteKeeper {
  ...
  struct Note {
    uint256 payout; // gGAIN remaining to be paid
  ...
  function redeem(address _user, uint256[] memory _indexes, bool _sendgGAIN) external returns (uint256);
  function redeemAll(address _user, bool _sendgGAIN) external returns (uint256);
...
interface IBondDepository {
  ...
  struct Market {
    ...
    bool capacityInQuote; // capacity limit is in payment token (true) or in GAIN (false, default)
  ...
  struct Terms {
    ...
    uint64 maxDebt; // 9 decimal debt maximum in GAIN
  }
...
// File: interfaces/IgGAIN.sol


pragma solidity >=0.5.17 <0.9.0;


interface IgGAIN is IERC20 {
  ...
  function migrate( address _staking, address _sGAIN ) external;
}

// File: interfaces/IGainAuthority.sol


pragma solidity >=0.5.17 <0.9.0;
interface IGainAuthority {
    /* ========== EVENTS ========== */
...
// File: types/GainAccessControlled.sol


pragma solidity >=0.5.17 <0.9.0;


abstract contract GainAccessControlled {

    /* ========== EVENTS ========== */

    event AuthorityUpdated(IGainAuthority indexed authority);

    string UNAUTHORIZED = "UNAUTHORIZED"; // save gas

    /* ========== STATE VARIABLES ========== */

    IGainAuthority public authority;


    /* ========== Constructor ========== */

    constructor(IGainAuthority _authority) {
    ...
    function setAuthority(IGainAuthority _newAuthority) external onlyGovernor {
        authority = _newAuthority;
        emit AuthorityUpdated(_newAuthority);
    }
}

// File: types/FrontEndRewarder.sol
...
abstract contract FrontEndRewarder is GainAccessControlled {
  ...
  IERC20 internal immutable GAIN; // reward token

  constructor(
    IGainAuthority _authority, 
    IERC20 _GAIN
  ) GainAccessControlled(_authority) {
    GAIN = _GAIN;
  }
  ...
  function getReward() external {
    ...
    GAIN.transfer(msg.sender, reward);
  }
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
  ...
  IgGAIN internal immutable gGAIN;
  ...
  constructor (
    ...
    IStaking _staking,
    ITreasury _treasury
  ) FrontEndRewarder(_authority, _GAIN) {
    gGAIN = _gGAIN;
    staking = _staking;
    treasury = _treasury;
  }
...
/* ========== ADD ========== */
   ...
   * @param _payout      the amount of GAIN due to the user
  ...
  function addNote(
    ...
    notes[_user].push(
      Note({
        payout: gGAIN.balanceTo(_payout),
    ...
    // note that only the payout gets staked (front end rewards are in GAIN)
...
/* ========== REDEEM ========== */
   ...
   * @param _sendgGAIN    send payout as gGAIN or sGAIN
   * @return payout_     sum of payout sent, in gGAIN
   */
  function redeem(address _user, uint256[] memory _indexes, bool _sendgGAIN) public override returns (uint256 payout_) {
    ...
    if (_sendgGAIN) {
      gGAIN.transfer(_user, payout_); // send payout as gGAIN
    } else {
      staking.unwrap(_user, payout_); // unwrap and send payout as sGAIN
    }
  }
   ...
   * @param _sendgGAIN    send payout as gGAIN or sGAIN
   * @return             sum of payout sent, in gGAIN
   */ 
  function redeemAll(address _user, bool _sendgGAIN) external override returns (uint256) {
    return redeem(_user, indexesFor(_user), _sendgGAIN);
  }
...
/* ========== VIEW ========== */
   ...
   * @return payout_     the payout due, in gGAIN
...
contract GainBondDepository is IBondDepository, NoteKeeper, Ownable {
...
/* ======== CONSTRUCTOR ======== */
    ...
    constructor(
        ...
        IStaking _staking,
        ITreasury _treasury
    ) NoteKeeper(_authority, _GAIN, _gGAIN, _staking, _treasury) {
        // save gas for users by bulk approving stake() transactions
        //_GAIN.approve(address(_staking), 9000);
        Gain = _GAIN;
    }

    function initializeApprove(address staking, uint256 amount) public onlyOwner {
      Gain.approve(staking,amount);
    }
...
/* ======== DEPOSIT ======== */
   ...
   * @return payout_     the amount of gGAIN due
  ...
  function deposit(
     ...
     * price = quote tokens : GAIN (i.e. 42069 DAI : GAIN)
     *
     * 1e18 = GAIN decimals (9) + price decimals (9)
     */
  ...
  function _tune(uint256 _id, uint48 _time) internal {
    ...
    if (_time >= meta.lastTune + meta.tuneInterval) {
      ...
      // GAIN decimals (9) + price decimals (9)
       ...
       * is 10,000 GAIN. max payout would be 1,000 GAIN (10,000 * 1 / 10).
       */  
...
/* ======== CREATE ======== */
   ...
   * @param _market      [capacity (in GAIN or quote), initial price / GAIN (9 decimals), debt buffer (3 decimals)]
  ...
  function create(
     ...
     *
     * 1e18 = GAIN decimals (9) + initial price decimals (9)
     */
     ...
     * interval. for example, if capacity is 1,000 GAIN, there are 10 days to conclusion, 
     * and the preferred deposit interval is 1 day, max payout would be 100 GAIN.
     */
    ...
    emit CreateMarket(id_, address(GAIN), address(_quoteToken), _market[1]);
  }
...
/* ======== EXTERNAL VIEW ======== */
   ...
   * @param _id          ID of market
   * @return             price for market in GAIN decimals
   *
   ...
   * @param _id          ID of market
   * @return             amount of GAIN to be paid in GAIN decimals
   *
   * @dev 1e18 = GAIN decimals (9) + market price decimals (9)
   */
   ...
   * @param _id          ID of market
   * @return             current debt for market in GAIN decimals
   */
   ...
   * @param _id          ID of market
   * @return             control variable for market in GAIN decimals
   */
