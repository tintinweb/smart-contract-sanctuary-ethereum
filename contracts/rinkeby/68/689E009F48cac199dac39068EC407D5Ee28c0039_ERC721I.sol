...
/* abstract contract PayableGovernance is Ownable {
    ...
    constructor() payable {
        _payableGovernanceSetter = msg.sender;
    }

    modifier onlyPayableGovernanceSetter() {
        require(
            msg.sender == _payableGovernanceSetter,
            "PayableGovernance: Caller is not Setter!"
        );
        _;
    }

    function reouncePayableGovernancePermissions()
        public
        onlyPayableGovernanceSetter
    {
    ...
    function setPayableGovernanceShareholders(
        address payable[] memory addresses_,
        uint256[] memory shares_
    ) public onlyPayableGovernanceSetter {
        require(
            _payableGovernanceAddresses.length == 0 &&
                _payableGovernanceShares.length == 0,
            "Payable Governance already set! To set again, reset first!"
        );
        require(
            addresses_.length == shares_.length,
    ...
    function resetPayableGovernanceShareholders()
        public
        onlyPayableGovernanceSetter
    {
    ...
    function payableGovernanceAddresses()
        public
        view
    ...
    function withdrawEther() public onlyOwner {
        ...
        require(
            _payableGovernanceAddresses.length ==
        ...
        for (uint256 i = 0; i < _payableGovernanceAddresses.length; i++) {
            uint256 _ethToWithdraw = ((_totalETH *
                _payableGovernanceShares[i]) / 1000);
            _withdraw(_payableGovernanceAddresses[i], _ethToWithdraw);
        }
    }

    function viewWithdrawAmounts()
        public
        view
        onlyOwner
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < _payableGovernanceAddresses.length; i++) {
            _withdrawals[i] = ((_totalETH * _payableGovernanceShares[i]) /
                1000);
        }
        ...
        for (uint256 i = 0; i < _payableGovernanceAddresses.length; i++) {
            _withdrawals[
                _payableGovernanceAddresses.length + 1
            ] += _withdrawals[i];
        }
    ...
    modifier onlyEmergency() {
        for (uint256 i = 0; i < _payableGovernanceAddresses.length; i++) {
            require(
                addressToEmergencyUnlocked[_payableGovernanceAddresses[i]],
...
abstract contract PublicMint {
    ...
    modifier publicMintEnabled() {
        require(
            _publicMint && _publicMintTime <= block.timestamp,
...
/* abstract contract WhitelistMint {
    // Whitelist Minting
    bool internal _whitelistMint;
    uint256 public _whitelistMintTime;

    function _setWhitelistMint(bool bool_, uint256 time_) internal {
        _whitelistMint = bool_;
        _whitelistMintTime = time_;
    }

    modifier whitelistMintEnabled() {
        require(
            _whitelistMint && _whitelistMintTime <= block.timestamp,
            "Whitelist Mint is not enabled yet!"
        );
        _;
    }
...
/* abstract contract SatelliteMint {
    // Satellite Minting
    bool internal _satelliteMintEnabled;
    uint256 public _satelliteMintTime;

    function _setSatelliteMint(bool bool_, uint256 time_) internal {
        _satelliteMintEnabled = bool_;
        _satelliteMintTime = time_;
    }

    modifier satelliteMintEnabled() {
        require(
            _satelliteMintEnabled && _satelliteMintTime <= block.timestamp,
            "Satellite Mint is not enabled yet!"
        );
        _;
    }
...
/* abstract contract SatelliteReceiver {
    ...
    function _satelliteMint(uint256 amount_) internal {
        require(
            msg.sender == satelliteStationAddress,
            "_satelliteMint: msg.sender is not Satellite Station!"
        );
        require(
            SSTokensAvailable >= SSTokensMinted + amount_,
            "_satelliteMint: amount_ requested over maximum avaialble tokens!"
        );
        require(
            SSMintsPerAddress >= SSAddressToMints[tx.origin] + amount_,
            "_satelliteMint: amount exceeds mints available per address!"
        );
...
contract FFYetisInu is ERC721I, Ownable, PublicMint {
    ...
    function _mintMany(address to_, uint256 amount_) internal {
        require(
            ...
            "_mintMany: amount exceeds maxSupply"
        );
    ...
    // Onwer Mint
    function ownerMintMany(address[] memory tos_, uint256[] memory amounts_)
        external
        onlyOwner
    {
        require(
            tos_.length == amounts_.length,
            "ownerMintMany: array length mismatch!"
        );
