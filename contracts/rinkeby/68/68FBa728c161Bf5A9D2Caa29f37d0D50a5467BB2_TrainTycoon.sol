...
contract TrainTycoon is ERC721, Ownable {
    ...
    // ERC20 public zug = new ERC20();

    constructor(address _recipient) ERC721("TrainTycoonV2", "TRAIN_V2") {
        recipient = _recipient;
        startTimestamp = 100;
        _baseTokenURI = "ipfs://QmQuLfhHiazQH76FdzV1XUSuZ92K96iJeJZcEfQ2XpgbkJ/";

        // Deploy Zug
        // zug = new ERC20("FUNDING-V100","FUN-001");

        // owner = msg.sender;
        // this.setTokenAddress(_recipient);
    }

    mapping(uint256 => Train) public trains;
    mapping(uint256 => Action) public activities;

    function setTokenAddress(ERC20 _simpleAddress) public payable {
        zug = _simpleAddress;
    ...
    function _claim(uint256 id) internal noCheaters {
        ...
        if (action.action == Actions.FARMING)
            zug.mint(action.owner, claimableZug(timeDiff, train.zugModifier));
    ...
    function claimableZug(uint256 timeDiff, uint16 zugModifier)
        internal
        pure
        returns (uint256 zugAmount)
    {
