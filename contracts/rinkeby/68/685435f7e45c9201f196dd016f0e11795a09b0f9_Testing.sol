...
abstract contract ERC721URIStorage is ERC721 {
    ...
    */
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory token_uri) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        ...
        if(revealed == false && tokenId <= 10000){
          // string memory baseURI = _baseURI();
            ...
            if(bytes(_tokenURIs[tokenId]).length > 0){
                return string(abi.encodePacked(_tokenURIs[tokenId], tokenId.toString(),".json"));
            } else{
            return bytes(baseUri).length > 0 ? string(abi.encodePacked(baseUri, tokenId.toString(),".json")) : "";
         }
        }
...
library IterableMapping {
    // Iterable mapping from string to string[];
    struct Map {
        string[] keys;
    ...
    function get(Map storage map, string memory key) internal view returns (string memory) {
        return map.values[key];
    }

    function getKeyAtIndex(Map storage map, uint index) internal view returns (string memory) {
        return map.keys[index];
    }

    function getAll(Map storage map) internal view returns(string[] memory){
        return map.keys;
    }
    ...
    function set(Map storage map, string memory key, string memory val) internal {
        if (map.inserted[key]) {
            map.values[key] = val;
        } else {
    ...
    function remove(Map storage map, string memory key) internal {
        require(map.inserted[key], "Key not found.");
...
contract Testing is ERC721URIStorage{
    ...
    uint256 public updateNameFees = 0.5 ether;      // Fees for updating your NFT name, First time it is free
    ...
    mapping(uint256 => uint256) public newlyPurchased;
    ...
    // ITERABBLE CODE STARTS FROM HERE
    function updateName(string memory tokenIDAddress, uint256 _tokenId, string memory name) public payable {
        require(ownerOf(_tokenId) == msg.sender,"You are not an owner.");
        if(newlyPurchased[_tokenId] != 1){
            require(msg.value >= updateNameFees, "You are passing wrong value.");
        }
        ...
        if(!success){
            ...
            map.set(tokenIDAddress, name);
            newlyPurchased[_tokenId] = 0;
        }
    }

    function removeRequest(string memory _key) public {
        map.remove(string(_key));
    }
    ...
    function getkey(string memory _key) public view returns(string memory){
        return map.get(string(_key));
    }

    // ITERABBLE CODE END TILL HERE
    ...
    function setUpdateNameFees(uint256 _updateNameFees) public onlyOwner{
        require(_updateNameFees > 0, "You have passed wrong value.");
        updateNameFees  = _updateNameFees;
    }
