...
contract FaPunks is ERC721, ERC721Enumerable, FaPunksDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("FaPunks", "FPKS") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No FaPunks available :(");
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "FAPunks #',
                tokenId.toString(),
                '", "description": "Festival de Artes Punks are randomized Avataaars stored on chain to teach DApp development on Mariachon", "image": "',
                image,
                '"}'
            )
        );
