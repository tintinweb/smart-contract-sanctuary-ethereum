...
// File: contracts/DemoSignature.sol
...
/// @author Kushal Chukanatti
contract WRSContractV2 is ERC721A, Ownable, Pausable, ReentrancyGuard, SignatureDemo {

    uint public ownerCurrentMints;    
    uint public constant MAX_SUPPLY = 4545;
    uint public constant PRICE = 0.000001 ether; //1000 Gwei
    uint public constant MAX_PER_MINT = 5;
    uint public constant MAX_PER_WALLET = 10;
    uint public constant MAX_MINTS_BY_OWNER = 22;
    uint public constant WHITELIST_START_TIME = 1650462600; //Wed, 20 Apr 2022 19:20:00
    uint public constant WHITELIST_END_TIME = WHITELIST_START_TIME + 1 days;
    uint public constant PUBLIC_MINT_START_TIME = WHITELIST_END_TIME + 2 minutes;
    address public multisigWallet = 0x471FFaaCdCbd3bda451365a4d0808c973Eb3C8E7; 
    ...
    function ownerMint(uint256 _quantity) external payable whenNotPaused onlyOwner {
        require(_quantity > 0, "Quantity cannot be zero");

        require(ownerCurrentMints + _quantity <= MAX_MINTS_BY_OWNER,"You have reached your limits to mint");
    ...
    /// @notice mints allowed only by the owner or the whitelisted addresses with signature data
    function whitelistMint(uint256 _quantity, bytes memory _signature) external payable whenNotPaused nonReentrant {
        require(_quantity > 0, "Quantity cannot be zero");
            ...
            _safeMint(msg.sender, _quantity);
            (bool sent,) = payable(multisigWallet).call{value: msg.value}("");
    ...
    function mint(uint256 _quantity) external payable whenNotPaused nonReentrant {
        require(block.timestamp >= PUBLIC_MINT_START_TIME, "Public mint has'nt yet started");
        require(_quantity > 0, "Quantity cannot be zero");
        uint totalMinted = totalSupply();
        require(_quantity <= MAX_PER_MINT, "Can mint at max 5 in each batch");
        require(totalMinted + _quantity < MAX_SUPPLY, "Not enough NFTs left to mint");
    ...
    /// @return address of the owner, of a randomly selected token minted till date
    function getRandomAddress(uint _seed) external view onlyOwner returns(address){
        require(totalSupply() > 0,"No owners exist");
        uint randNum = (uint(keccak256(abi.encodePacked(block.timestamp, owner(), _seed))) % totalSupply());
        address addr = ownerOf(randNum);
        return addr;             
    }
