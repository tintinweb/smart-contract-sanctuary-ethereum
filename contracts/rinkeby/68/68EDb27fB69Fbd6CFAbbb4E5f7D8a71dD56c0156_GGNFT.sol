...
contract GGNFT is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mint(address to, uint256 rarity) public adminOrOwner returns (uint256) {
        ...
        if (rarity == 0) {
            ...
            return _mintRandom(to, rareCount, totalRare, totalCommon + totalUncommon);
        } else if (rarity == 3) {
            return _mintRandom(
                to,
                superRareCount,
    ...
    function _mintRandom(
        ...
        emit Minted(_msgSender(), index + startIndex);

        return index + startIndex;
    }

    function testEmit() external {
        emit Minted(_msgSender(), 17);
    }
