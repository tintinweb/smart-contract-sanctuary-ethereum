...
contract TEST_NBRL is ERC721Enumerable, Ownable, ReentrancyGuard {
        ...
        )
    {
        ...
        if (status == MintStatus.CLOSED) {
            return ("CLOSED", 0, PRICE, "Minting is closed", public_supply);
        } else if (available <= 0) {
            return (
                "SOLD OUT",
                0,
                PRICE,
                "Weekly batch sold out",
                public_supply
            );
        } else {
            return ("PUBLIC", available, PRICE, "Public sale", public_supply);
        }
    }
    ...
    function setAvailable(uint256 _available) external onlyOwner {
        ...
        require(
            ...
            "Cannot set availabe grater then max supply"
        );
