...
contract SneaksOfNature is ERC721A, ERC2981, Pausable, Ownable, ReentrancyGuard {
  ...
  function closedMint(address receiver, uint16 tokenAmount) external payable whenNotPaused nonReentrant {
    ...
    MintAllowanceLib.MintAllowance[] memory mintAllowance = getMintAllowance(receiver, false, false);
    ...
    for (uint256 i = 0; i < tokenAmount; i++) {
      MintAllowanceLib.MintAllowance memory mint = mintAllowance[i];
  ...
  function sneaksListMint(address receiver, bytes32[] calldata proof) external payable whenNotPaused nonReentrant {
    ...
    // Check that the correct price is paid
    require(msg.value == MintAllowanceLib.SNEAKS_LIST_PRICE, "Incorrect amount of ether payed");
  ...
  function guaranteedListMint(address receiver, bytes32[] calldata proof) external payable whenNotPaused nonReentrant {
    ...
    // Check that the correct price is paid
    require(msg.value == MintAllowanceLib.FULL_PRICE, "Incorrect amount of ether payed");
  ...
  function publicMint(
    ...
    // Check that the correct price is paid
    require(msg.value == tokenAmount * MintAllowanceLib.FULL_PRICE, "Incorrect amount of ether payed");
  ...
  ) public view returns (MintAllowanceLib.MintAllowance[] memory mints) {
  ...
  ) internal view returns (MintAllowanceLib.MintAllowance[] memory mints) {
    return
      MintAllowanceLib.mintAllowance(
...
library MintAllowanceLib {
  ...
  uint256 public constant FULL_PRICE = 0.45 ether;
  ...
  function mintAllowance(
  ...
  ) internal pure returns (MintAllowance[] memory mints) {
    uint256 relicPassCount = relicPassIds.length;

    // Allocate enough memory to fit all tokens
    MintAllowance[] memory tempMints = new MintAllowance[](
    ...
    for (uint256 i = 0; i < relicPassCount; ++i) {
      ...
      if (relicPassesUsage[i] == 0) {
        tempMints[mintIndex++] = MintAllowance(uint8(MintAllowanceType.RELIC_PASS), 0, relicPassId, false);
      }
    }

    // Add the unused tokens from the Discounted Sneaks List
    for (uint256 i = 0; i < discountedSneaksListTokensLeft; ++i) {
      tempMints[mintIndex++] = MintAllowance(
    ...
    for (uint256 i = 0; i < relicPassCount; ++i) {
      ...
      if (relicPassesUsage[i] <= 2) {
        tempMints[mintIndex++] = MintAllowance(uint8(MintAllowanceType.RELIC_PASS), FULL_PRICE, relicPassId, true);
      }
    }
    ...
    if (relicPassCount == 0 && discountedSneaksListTokensLeft == 0 && currentBalance == 0) {
      if (isSneaksList) {
        if (!isSneaksListUsed) {
          tempMints[mintIndex++] = MintAllowance(uint8(MintAllowanceType.SNEAKS_LIST), SNEAKS_LIST_PRICE, 0, false);
        }
      } else if (isGuaranteedAllowlist) {
        if (!isGuaranteedAllowlistUsed) {
          tempMints[mintIndex++] = MintAllowance(uint8(MintAllowanceType.GUARANTEED_ALLOWLIST), FULL_PRICE, 0, false);
        }
      }
    }
    ...
    if (relicPassCount == 0 && discountedSneaksListTokensLeft == 0 && !isGuaranteedAllowlist && !isSneaksList) {
      for (uint16 i = 0; i < maxTokensPerUser - currentBalance; ++i) {
        tempMints[mintIndex++] = MintAllowance(uint8(MintAllowanceType.PUBLIC_MINT), FULL_PRICE, 0, true);
      }
    }

    // Copy only the filled elements to the result
    mints = new MintAllowance[](mintIndex);
