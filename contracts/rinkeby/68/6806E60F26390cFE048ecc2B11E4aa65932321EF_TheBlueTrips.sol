...
pragma solidity ^0.8.0;
contract TheBlueTrips is Ownable, ERC721A, ReentrancyGuard {

    bool public publicSale = false;
    uint256 public maxPerTx = 10;
    uint256 public maxPerWallet = 10;
    uint256 public maxPerAddress = 1000;
    uint256 public maxToken = 9506;
    uint256 public price = 0.10 ether;
    string private _baseTokenURI = "http://66.94.102.65:3333/metadata/";
    string public notRevealedUri = "";
    mapping (uint256 => bool) public linkedArt;
    bytes32 public merkleRoots;
    mapping(address => bool) public isClaimed;
    bool public airdrop;
    event memeBuilt(address _from, uint256 _meme, uint256 _newspaper);
    ...
    function mint(uint256 quantity) external payable callerIsUser {
        ...
        require(((numberMinted(msg.sender) + ( quantity * 2 ))  / 2 ) <= maxPerWallet, "PER_WALLET_LIMIT_REACHED");
        require(totalSupply() + ( quantity * 2 ) <= maxToken, "NOT_ENOUGH_SUPPLY_TO_MINT_DESIRED_AMOUNT");
        require(msg.value == (price * quantity), "INVALID_ETH");
        _safeMint(msg.sender, ( quantity * 2 ));
    }

    function buildMeme(uint256 meme, uint256 newspaper) external {
        require(!linkedArt[meme] && !linkedArt[newspaper], "ALREADY_USED");
        require(ownerOf(meme) == msg.sender && ownerOf(newspaper) == msg.sender, "YOU_DONT_OWN_THESE_TOKENS");
        require(meme != newspaper, "ID_CANT_BE_SAME");
        require(!(meme % 2 == 0) && (newspaper % 2 == 0), "WRONG_ID");
        linkedArt[meme] = true;
        linkedArt[newspaper] = true;
        emit memeBuilt(msg.sender, meme, newspaper);
    }

    function claimDrop(bytes32[] calldata _proof, address _who, uint256 quantity) external {
        quantity = (quantity * 2);
        require(airdrop, "AIRDROP_NOT_YET_STARTED");
        require(isClaimed[_who] != true, "CLAIMED_ALREADY");
        require(quantity > 0, "INVALID_QUANTITY");
        require(_checkProof(_proof, _leafFromAddressAndNumTokens(_who, quantity)), "WRONG_PROOF");
    ...
    function airdropRoot(bytes32 _merkleRoot) external onlyOwner {
