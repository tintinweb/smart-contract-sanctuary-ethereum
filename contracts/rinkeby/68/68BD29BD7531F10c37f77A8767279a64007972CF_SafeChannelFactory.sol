...
/**
 * @title SafeChannelFactory
 */
contract SafeChannelFactory is FactoryVersion {
    ...
    modifier isWhitelisted() {
        require(
            registry.whitelisted(msg.sender),
            "Factory Error: Not Whitelisted"
        );
        _;
    }

    /**
     * @notice Deploy and initialize safe channel
     * @param initializer   Function call for safe initialization
     * @param saltNonce     Arbitrary number to make safe unique
     * @return safe         Deployed safe address
     */
    function deploySafe(bytes memory initializer, uint256 saltNonce)
        public
        isWhitelisted
        returns (GnosisSafeProxy safe)
    {
        safe = GnosisSafeProxyFactory(gnosisSafeProxyFactory)
                  .createProxyWithNonce(implementation, initializer, saltNonce);

        address safeAddress = address(safe);

        registry.addDeployment(keccak256("CHANNEL"), safeAddress);

        emit ChannelDeployed(safeAddress, msg.sender);
    }

    /**
     * @notice Enable safe deployed modules
     * @dev should only be called by ModuleManager `setupModules` function
     * @param modules   Array of module addresses to enable
     */
    function enableModules(address[] memory modules) public {
