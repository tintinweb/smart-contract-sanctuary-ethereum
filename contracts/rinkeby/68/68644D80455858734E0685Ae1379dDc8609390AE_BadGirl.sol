...
contract BadGirl is ERC721A, Ownable {
    uint32 public PRE_SALE_START_DATE = 1651377780;
    uint32 public PUBLIC_SALE_DATE = 1651378080;
    ...
    mapping(address => uint256) public freeMintCnt;
    mapping(address => uint256) public highOrderMintCnt;
    mapping(address => uint256) public whitelistMintCnt;


    constructor(bytes32 free, bytes32 highOrder, bytes32 whitelist) ERC721A("Bad Girl", "BG") {
        // to the deployer 
        freeMintMerkleRoot = free;
    ...
    function setMerkleRoot (bytes32 free, bytes32 highOrder, bytes32 whitelist) public onlyOwner {
        freeMintMerkleRoot = free;
        highOrderMerkleRoot = highOrder;
        whitelistMerkleRoot = whitelist;
    }
    ...
    function freeMint (bytes32[] calldata merkleProof, uint256 eligibleAmount, uint256 amount) public 
        isValidMerkleProof(merkleProof, freeMintMerkleRoot, getTreeLeaf())
    {
        require(block.timestamp >= PRE_SALE_START_DATE, "Free mint has not yet started!");
        require(
            freeMintCnt[msg.sender] + amount <= eligibleAmount,
            "Exceeds the maximum free mint limit"
        );
        ...
        freeMintCnt[msg.sender] += amount;
        mintedCount += amount;
    }

    function highOrderMint (bytes32[] calldata merkleProof, uint256 eligibleAmount, uint256 amount)
        public payable
        isValidMerkleProof(merkleProof, highOrderMerkleRoot, getTreeLeaf())
    {
        ...
        require(
            highOrderMintCnt[msg.sender] + amount <= eligibleAmount,
            "Exceeds the maximum high order mint limit"
        );
    ...
    function whitelistMint (bytes32[] calldata merkleProof, uint256 eligibleAmount, uint256 amount)
        public payable
        isValidMerkleProof(merkleProof, whitelistMerkleRoot, getTreeLeaf())
    {
        require(block.timestamp >= PRE_SALE_START_DATE, "Whitelist mint has not yet started!");
        require(
            whitelistMintCnt[msg.sender] + amount <= eligibleAmount,
            "Exceeds the maximum whitelist mint limit"
        );
