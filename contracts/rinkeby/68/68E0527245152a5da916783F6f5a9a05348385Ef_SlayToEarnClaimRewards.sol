...
contract SlayToEarnClaimRewards {
    ...
    constructor(ISlayToEarnItems itemCollection, address signerAddress) {
        itemCollection.ping();
        // test contract invocation as sanity check

        _itemCollection = itemCollection;
        _signerAddress = signerAddress;
    }

    // Is emitted when rewards have been successfully claimed for a match.
    event ClaimRewards(uint256 seed, uint256[] itemStacks, uint256[] usedConsumables);
    ...
    function claimRewards(
        uint256 seed,
        uint256[] memory itemStacks,
        uint256[] memory usedConsumables,
        ...
        // this is the place where we can get attacked. Users must not be allowed to change order, or move items between
        // sets without changing the signature, otherwise they could claim consumables instead of burning them, for instance.
        bytes memory message = abi.encodePacked(
            uint256(uint160(msg.sender)), // claimant
            seed, // game seed
            uint256(itemStacks.length),
            itemStacks, // item stacks awarded
            uint256(usedConsumables.length),
            usedConsumables // consumables used
        );
        ...
        _removeConsumablesForPlayer(usedConsumables);
        _addItemStacksToPlayer(itemStacks);

        emit ClaimRewards(seed, itemStacks, usedConsumables);
    }

    function _removeConsumablesForPlayer(uint256[] memory usedConsumables) private {
        (uint256[] memory consumables, uint256[] memory amounts) = _unpackItemStacks(usedConsumables);

        _itemCollection.burnBatch(msg.sender, consumables, amounts);
    }

    function _addItemStacksToPlayer(uint256[] memory itemStacks) private {
        (uint256[] memory items, uint256[] memory amounts) = _unpackItemStacks(itemStacks);

        _itemCollection.mintBatch(msg.sender, items, amounts);
    }
    ...
    function _unpackItemStacks(uint256[] memory itemStacks) private view returns (uint256[] memory, uint256[] memory) {
        ...
        for (uint i = 0; i < itemStacks.length; i++) {
            ...
            amounts[i] = itemStacks[i] & 0xFFFFFFFF;
        }
