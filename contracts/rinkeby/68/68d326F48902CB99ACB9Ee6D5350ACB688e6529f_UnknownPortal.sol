...
contract UnknownPortal is AbstractERC1155Factory {
    ...
    // event NewTokenIdAdded(uint256 indexed newTokenId);
    ...
    // /**
    // * @notice owner adds new tokenid
    // */
    // function addNewToken() external onlyOwner {
    //     currentTokenId += 1;

    //     emit NewTokenIdAdded(currentTokenId);
    // }
    ...
    */
    ...
    function renounceAddWhitelist(bool _renounceAddWhitelist) external onlyOwner {
        canAddWhitelist = !_renounceAddWhitelist;
    }

    function renounceChangeMetadata(bool _renounceChangeMetadata) external onlyOwner {
        canChangeMetadata = _renounceChangeMetadata;
    }

    /**
    * @notice mint cards during public sale
    *
    */
    function mint() external payable whenNotPaused {
        ...
        require(totalSupply(0) + MAX_CLAIMABLE_AMOUNT <= MAX_SUPPLY, "Mint: Max supply reached");
        require(msg.value == MAX_CLAIMABLE_AMOUNT * mintPrice, "Mint: Incorrect payment");

        // purchaseTxs[msg.sender] += 1;
        _mint(msg.sender, 0, MAX_CLAIMABLE_AMOUNT, "");
        whitelisted[msg.sender] = false;

        emit Minted(0, msg.sender, MAX_CLAIMABLE_AMOUNT);
    }

    /**
    * @notice returns the metadata uri for a given id
    *
    * @param _id id to return metadata for
    */
    ...
    function setURI(string memory _baseURI) external onlyOwner {
        require(canChangeMetadata, "Can't change metadata");
        _setURI(_baseURI);
