...
contract LibexMarketplace is IERC721Receiver, Ownable {
    ...
    function bidAuction(address _contractAddress, uint256 _tokenId)
        external
        payable
    {
        ...
        require(
            msg.value > auction.nftHighestBid,
            "Your bid value is less than the highest bid"
        );
        require(
    ...
    function createAuction(
        ...
        emit AuctionCreated(_contractAddress, _tokenId, msg.sender, _minPrice);
    }
    ...
    function createListing(
        ...
        emit ListingCreated(_contractAddress, _tokenId, msg.sender, _price);
    }

    function fillListing(address _contractAddress, uint256 _tokenId)
        external
        payable
    {
        Listing storage listing = listings[_contractAddress][_tokenId];
        ...
        require(
            msg.value == listing.listingPrice,
            "This amount does not match the listing price"
        );
        require(
        ...
        if (platformPercentage > 0) {
            platformFee = msg.value.mul(platformPercentage).div(100);
    ...
    function getActiveAuctions() external view returns (Combo[] memory) {
    ...
    function getActiveListings() external view returns (Combo[] memory) {
    ...
    function getActiveOffers() external view returns (Combo[] memory) {
    ...
    function makeOffer(address _contractAddress, uint256 _tokenId) external payable {
        IERC721WithRoyalties nftCollection = IERC721WithRoyalties(
            _contractAddress
        );
        address owner = nftCollection.ownerOf(_tokenId);
        ...
        offers[_contractAddress][_tokenId].push(Offer(msg.sender, msg.value));

        emit OfferCreated(_contractAddress, _tokenId, msg.sender, msg.value);
    }
