...
contract PriceOracle is IPriceOracle, OwnableUpgradeable {
    ...
    // Stores the current index of the feeds
    uint8 public feedIndex;
     ...
     */
    function initialize(address _owner, address[] memory _aggregators)
        external
        initializer
    {
        uint256 aggregatorLength = _aggregators.length;
        uint8 index;
        for (index = 0; index < aggregatorLength; index++) {
            require(
                _aggregators[index] != address(0),
                "PriceOracle: priceoracle address can't be zero"
            );
     ...
     * @param _aggregators Addresses of required chainlink aggregators
     */
    function setAggregators(address[] calldata _aggregators)
        external
        onlyOwner
    {
        uint256 aggregatorLength = _aggregators.length;
        uint8 count = feedIndex;
        for (uint256 index; index < aggregatorLength; index++) {
            require(
                _aggregators[index] != address(0),
                "PriceOracle: priceoracle address can't be zero"
            );
            priceFeeds[count] = AggregatorV3Interface(_aggregators[index]);
            count++;
        }
        feedIndex = count;
