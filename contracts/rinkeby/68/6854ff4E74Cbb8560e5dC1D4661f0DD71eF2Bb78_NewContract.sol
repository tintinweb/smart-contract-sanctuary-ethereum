...
contract NewContract {
    ...
    function addItems(itemInfo[] memory _itemInfo) public {
        ...
        for (uint256 i = 0; i < _itemInfo.length; i++) {
            ...
            newItem.name = _itemInfo[i].name;
            newItem.width = _itemInfo[i].width;
            newItem.height = _itemInfo[i].height;
            newItem.length = _itemInfo[i].length;
            newItem.email = _itemInfo[i].email;
            newItem.mobile_no = _itemInfo[i].mobile_no;
