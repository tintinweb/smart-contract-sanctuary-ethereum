...
contract Lottery is Ownable, PriceFeedConsumer{

    enum LotteryStatus {Ended, Open, PickingWinner}
    LotteryStatus public status;

    constructor(address _priceFeedAddress)PriceFeedConsumer(_priceFeedAddress){
        status = LotteryStatus.Ended;
    }

    function getLotteryPrice() public returns (int) {
        int _price_per_eth = getLatestPrice();
        return 1e18*50*1e8/_price_per_eth;
    }
