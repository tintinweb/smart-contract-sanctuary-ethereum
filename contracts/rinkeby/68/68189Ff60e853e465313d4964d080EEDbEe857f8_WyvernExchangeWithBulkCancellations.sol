...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    bytes32 private constant _ORDER_TYPEHASH = 0x5fc4e551bf2e8dbca276b345d89b36a4cf8c8ae0e5a6ee8bde78e226ba102e62;
    ...
    bytes32 public DOMAIN_SEPARATOR;// = 0x763ec933101f7afbd15d409b1cdcb774113a929e11e1524cb1e23e0ce1b4e757;
    ...
    struct Order {
        ...
        /* Taker cashback fee of the order, or maximum taker fee for a taker order. */
    ...
    constructor () public {
        ...
        require(keccak256("Order(address exchange,address maker,address taker,uint256 makerRelayerFee,uint256 takerRelayerFee,uint256 makerProtocolFee,uint256 takerProtocolFee,uint256 takerCashbackFee,address feeRecipient,uint8 feeMethod,uint8 side,uint8 saleKind,address target,uint8 howToCall,bytes calldata,bytes replacementPattern,address staticTarget,bytes staticExtradata,address paymentToken,uint256 basePrice,uint256 extra,uint256 listingTime,uint256 expirationTime,uint256 salt,uint256 nonce)") == _ORDER_TYPEHASH);
        ...
        returns (uint)
    {
        ...
        uint cashback = SafeMath.div(SafeMath.mul(sell.takerCashbackFee, price), INVERSE_BASIS_POINT);
        ...
        if (sell.feeRecipient != address(0)) {
            ...
            if (sell.feeMethod == FeeMethod.SplitFee) {
                ...
                if (sell.makerRelayerFee > 0) {
                    ...
                    //Adding multiple nested-if statements to help during diamond-implementation
                    if (sell.paymentToken == address(0)) {
                        receiveAmount = SafeMath.sub(receiveAmount, makerRelayerFee);
                        if(cashback > 0){
                            makerRelayerFee = SafeMath.sub(makerRelayerFee, cashback);
                            buy.maker.transfer(cashback);
                        }
                        sell.feeRecipient.transfer(makerRelayerFee);
                    } else {
                        if(cashback > 0){
                            makerRelayerFee = SafeMath.sub(makerRelayerFee, cashback);
                            transferTokens(sell.paymentToken, sell.maker, buy.maker, cashback);
                        }
                ...
                if (sell.takerRelayerFee > 0) {
                    ...
                    if (sell.paymentToken == address(0)) {
                        ...
                        if(cashback > 0){
                            takerRelayerFee = SafeMath.sub(takerRelayerFee, cashback);
                            buy.maker.transfer(cashback);
                        }
                ...
                if (sell.makerProtocolFee > 0) {
                    ...
                    if (sell.paymentToken == address(0)) {
                        ...
                        if(cashback > 0){
                            makerProtocolFee = SafeMath.sub(makerProtocolFee, cashback);
                            buy.maker.transfer(cashback);
                        }
                ...
                if (sell.takerProtocolFee > 0) {
                    ...
                    if (sell.paymentToken == address(0)) {
                        ...
                        if(cashback > 0){
                            takerProtocolFee = SafeMath.sub(takerProtocolFee, cashback);
