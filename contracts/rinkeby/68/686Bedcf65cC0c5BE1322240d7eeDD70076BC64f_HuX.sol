...
import "Ownable.sol";
import "ICouponRedeem.sol";

contract HuX is ERC721Permit, Ownable, ICouponRedeem {
    using Counters for Counters.Counter;

    // solhint-disable-next-line var-name-mixedcase
    bytes32 private immutable _COUPON_TYPEHASH =
        keccak256("HuX(address minter,uint256 quota,uint256 deadline)");

    Counters.Counter private _tokenIdCounter;

    constructor() ERC721Permit("HuX NFT", "HUX") {}

    function _baseURI() internal pure override returns (string memory) {
        return "https://nft102.smartcontractthailand.com/";
    }
    ...
    function mintPublicSale() public payable {
        // require fee
        // require PeriodEnum == Public
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
    ...
    function mintPresale(
        ...
        _mintPresale(quota, deadline, signature);
    }
    ...
    function _mintPresale(
        ...
        require(block.timestamp <= deadline, "Coupon: expired deadline");

        bytes32 structHash = keccak256(
            abi.encode(_COUPON_TYPEHASH, msg.sender, quota, deadline)
        );
...
///
/// @dev Interface for coupon redeem for ERC-721
///
interface ICouponRedeem is IERC165 {
    function mintPresale(
        uint256 quota,
        uint256 deadline,
        bytes memory sig
