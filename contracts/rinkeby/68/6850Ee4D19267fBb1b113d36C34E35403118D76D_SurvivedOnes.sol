...
contract SurvivedOnes is ERC721A, Ownable {
  event Survived(address indexed wallet, uint256 indexed tokenId);
  event NotSurvived(address indexed wallet);

  using Strings for uint256;

  uint256 public constant MAX_SUPPLY = 400;
  uint256 public constant TEAM_SUPPLY = 20;
  uint256 public constant DOSE_PRICE = 0.005 ether;
  uint256 public constant SURVIVE_CHANCE = 4;

  uint256 public deadCount = 0;
  uint256 public usedTeamSupply = 0;
  uint256 private mintStage = 0;
  ...
  address DEAD_CONTRACT;

  constructor(address signerAddress_) ERC721A("SurvivedOnes", "SONE") {
    _signerAddress = signerAddress_;
  }

  function gangBang(uint256 pplCount) external onlyOwner {
    if (usedTeamSupply + pplCount > TEAM_SUPPLY) {
      revert LogicError("team_supply_exceeded");
    }
    usedTeamSupply += pplCount;
    _safeMint(msg.sender, pplCount);
  }
  ...
  function cum(
    ...
    // }
    if (totalSupply() - usedTeamSupply >= MAX_SUPPLY) {
      revert LogicError("sold_out");
    }
    if (dose < 1) {
      revert LogicError("dose_lt_1");
    }
    if (dose > 20) {
      revert LogicError("dose_gt_20");
    }
    // if (verifySignature(msg.sender, luckyNumber, signature) != true) {
    //   revert InvalidSignature("wrong_signature");
    // }
    // if (msg.value < dose * DOSE_PRICE) {
    //   revert PriceError("not_enough_money");
    // }

    for (uint256 i = 0; i < dose; i++) {
      uint256 rand = random(signature, luckyNumber + i, 100);
      uint256 currentSpermCount = totalSupply() - usedTeamSupply;
      if (currentSpermCount >= MAX_SUPPLY) {
        break;
      }
      uint256 tempDeath = 0;
      if (rand < SURVIVE_CHANCE) {
        _safeMint(msg.sender, 1);
        emit Survived(msg.sender, _currentIndex - 1);
      } else {
        deadCount += 1;
        tempDeath += 1;
        emit NotSurvived(msg.sender);
      }

      if (tempDeath > 0) {
        DeadProxy proxy = DeadProxy(DEAD_CONTRACT);
        proxy.gangBang(msg.sender, tempDeath);
      }
    }
  }
  ...
  function getMessageHash(address _receiverAddress, uint256 _randSeed)
    internal
    pure
    returns (bytes32)
  {
    return keccak256(abi.encodePacked(_receiverAddress, _randSeed));
  }
  ...
  function verifySignature(
    ...
    bytes32 messageHash = getMessageHash(_receiverAddress, _randSeed);
  ...
  function setMintStage(uint256 _mintStage) external onlyOwner {
...
abstract contract DeadProxy {
  function gangBang(address to, uint256 count) public virtual;
}
