...
contract Oubita is ERC721, Ownable {
   ...
   //uint256 public NFTperAddressLimit= 20;
    ...
    uint public costETH_whitelist = 2000000000000000;
    uint public costETH_cost = 5000000000000000;
    uint public costETH_costNormal = 10000000000000000;

    //cost in TOKEN

    uint public cost = 20000000000000000000;
    uint public costWhitelist= 10000000000000000000;
    uint public costNormal= 30000000000000000000;
    //Badse URI
    string baseURI;
    ...
    function multiTokensTransfer(address _to, uint _type, uint _amount, uint _counter) public payable{
        if(_type == 1){
            if(onlyWhitelisted == true){
                ...
                require(msg.value >= costETH_cost * _amount, "You dont have enough balance");
            }else if(_counter >= 20){
                
                require(msg.value >= costETH_costNormal * _amount, "You dont have enough balance");
            }
            
        }else if (_type == 2){
            if(onlyWhitelisted == true){
                uint precio = costWhitelist * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }else if(_counter < 20){
                uint precio = cost * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }else if(_counter >= 20){
                uint precio = costNormal * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }
        }
    } 
