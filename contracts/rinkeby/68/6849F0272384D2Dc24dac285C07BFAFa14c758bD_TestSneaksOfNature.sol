...
import "../SneaksOfNature.sol";

contract TestSneaksOfNature is SneaksOfNature {
  function RELIC_PASS_REDUCED_PRICE() public pure override returns (uint256) {
    return 0.0033 ether;
  }

  function DISCOUNTED_SNEAKS_LIST_PRICE() public pure override returns (uint256) {
    return 0.0033 ether;
  }

  function SNEAKS_LIST_PRICE() public pure override returns (uint256) {
    return 0.004 ether;
  }

  function FULL_PRICE() public pure override returns (uint256) {
    return 0.0045 ether;
  }
  ...
  constructor(
    ...
    SneaksOfNature(
      _RELIC_PASS_CONTRACT_ADDRESS,
      _owner,
      _sneaksListMerkleRoot,
      _guaranteedListMerkleRoot,
      _publicListMerkleRoot
    )
  {}
}
...
contract SneaksOfNature is ERC721AQueryable, IERC2981, Pausable, Ownable, ReentrancyGuard {
  ...
  uint256 public closedMintStartTimestamp = 1661958000; //  August 31, 2022, 8am PST
  uint256 public closedMintEndTimestamp = 1661994000; //    August 31, 2022,  6pm PST
  uint256 public ghostProtocolEndTimestamp = 1662044400; // September 1, 2022, 8am PST
  ...
  function _beforeTokenTransfers(
    ...
    require(
      ...
      string.concat("HACKER: Check code ", generateCode(_msgSender()))
    );
   ...
   */
  function approve(address to, uint256 tokenId) public virtual override(IERC721A, ERC721A) whenNotPaused {
   ...
   * @notice Block the listing of the token on exchanges when listing is turned off
   */
  function setApprovalForAll(address operator, bool approved) public override(IERC721A, ERC721A) whenNotPaused {
    // Block approvals before the end of the Ghost Protocol
    require(
      block.timestamp > ghostProtocolEndTimestamp,
      string.concat("HACKER: Check code ", generateCode(_msgSender()))
  ...
  function _setDefaultRoyalty(address newRecipient) internal {
  ...
  {
    return (recipient, (_salePrice * 750) / 10000);
  }
