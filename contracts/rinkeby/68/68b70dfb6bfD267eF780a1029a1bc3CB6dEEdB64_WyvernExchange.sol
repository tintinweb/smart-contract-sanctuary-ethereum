...
contract WyvernExchange is Exchange {
    ...
    constructor (uint chainId, address[] memory registryAddrs, bytes memory customPersonalSignPrefix, IRFOXStaticManager _rfoxStaticManager) public {
        ...
        rfoxStaticManager = _rfoxStaticManager;
    }
...
contract Exchange is ExchangeCore {
    ...
    function bulkSetOrderFill_(bytes32[] calldata hashes, uint256[] calldata fills) external {
        uint256 hashesLength = hashes.length;
        require(hashesLength == fills.length, "BulkCancel: Invalid data length");
        for(uint256 i = 0;  i < hashesLength; i++) {
            setOrderFill(hashes[i], fills[i]);
        }
    }
...
import "../interfaces/IRFOXStaticManager.sol";

/**
 * @title ExchangeCore
 * @author Wyvern Protocol Developers
 */
contract ExchangeCore is Ownable, ReentrancyGuarded, StaticCaller, EIP712 {
    ...
    IRFOXStaticManager public rfoxStaticManager;

    /* Events */
    event StaticManagerChanged(IRFOXStaticManager indexed oldStaticManager, IRFOXStaticManager indexed newStaticManager);
    ...
    function updateRFOXStaticManager(IRFOXStaticManager _rfoxStaticManager) external onlyOwner {
        require(address(_rfoxStaticManager) != address(0), "ERR_ZERO_ADDRESS");
        IRFOXStaticManager oldStaticManager = rfoxStaticManager;
        rfoxStaticManager = _rfoxStaticManager;

        emit StaticManagerChanged(oldStaticManager, rfoxStaticManager);
    }
    ...
    function executeStaticCall(Order memory order, Call memory call, Order memory counterorder, Call memory countercall, address matcher, uint value, uint fill)
        ...
        returns (uint)
    {
        require(rfoxStaticManager.isStaticWhitelisted(order.staticTarget), "Exchange: Static is not allowed");
        ...
        reentrancyGuard
    {
        ...
        if (msg.value > 0) {
            ProxyRegistryInterface registry = ProxyRegistryInterface(secondOrder.registry);

            (bool success,) = address(registry.proxies(secondOrder.maker)).call{value: msg.value}("");
...
interface IRFOXStaticManager {
  function whitelistStatic(address _static) external;
  function unwhitelistStatic(address _static) external;
  function bulkWhitelistStatic(address[] memory _statics) external;
  function bulkUnwhitelistStatic(address[] memory _statics) external;
  function isStaticWhitelisted(address _static) external view returns (bool);
  function totalWhitelistedStatic() external view returns (uint256);
  function getWhitelistedStatics(uint256 _cursor, uint256 _limit) external view returns(address[] memory whitelistedResults, uint256 nextCursor);
}
