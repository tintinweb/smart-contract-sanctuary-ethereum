...
interface IUSDBNFT {
    function mint(address wallet, string memory uri, Bond memory Info) external payable returns(uint256);
...
interface IVault {
    ...
    function withdraw(uint _nftTokenId) external returns(uint);
}
...
contract UsdbNftBondDepository is Ownable, ReentrancyGuard{
    ...
    address public immutable vaultMangerAddress;// vaultManager Address
    ...
    constructor (
        ...
        require( _vaultManagerAddress != address(0) );
        vaultMangerAddress = _vaultManagerAddress;
    ...
    function deposit(
        ...
        Bond memory info = Bond({
                gonsPayout:  IsFHM(sFHM).gonsForBalance(payout),
                fhmPayout: payout,
                usdbAmount: _amount,
        ...
        uint tokenId = IUSDBNFT(UsdbNftAddress).mint{value: msg.value}(_depositor , _nftTokenUri, info);

        vault =  IVaultManager(vaultMangerAddress).getVaultAddress(tokenId);
        
        //check the vault address by token ID
        require(vault != address(0), "Vault have to be set");
        IERC20( FHM ).approve( vault, payout );
        IERC20( principle ).approve( vault, _amount);
        
        IVault(vault).deposit(tokenId, _amount, payout);
    ...
    */
    function bondInfo(uint256 _nftTokenId) public view returns ( uint payout,uint vestingSeconds,uint vesting,uint lastBlock,uint pricePaid ) {
        Bond memory info = IUSDBNFT(UsdbNftAddress).getNftInfo(_nftTokenId);
    ...
    function pendingPayoutFor( uint256 _nftTokenId ) external view returns ( uint pendingPayout_ ) {
        ...
        Bond memory info = IUSDBNFT(UsdbNftAddress).getNftInfo(_nftTokenId);
        uint payout = IsFHM( sFHM ).balanceForGons( info.gonsPayout );
