...
contract Stakes is Owner, ReentrancyGuard {
    ...
    function end(uint256 i) external nonReentrant {
        ...
        if(block.timestamp - ledger[msg.sender][i].from < maturity) {
            ...
            asset.transfer(msg.sender, ledger[msg.sender][i].amount - _penalization);
    ...
    function set(uint256 _lower, uint256 _maturity, uint8 _rate, uint8 _penalization) external isOwner {
