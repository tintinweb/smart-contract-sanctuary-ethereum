...
contract MiamiNFT is Context, ERC721Enumerable, Ownable, ReentrancyGuard, VRFConsumerBase {
    ...
    uint256 public totalReservedForOwner;
    ...
    constructor(
        ...
        require(_limits[3] <= totalLimit);
        totalReservedForOwner = _limits[3];
        start = _start;
    ...
    function setOwnerClaimActive(bool _isOwnerClaimActive) external onlyOwner {
        isOwnerClaimActive = _isOwnerClaimActive;
    }
    ...
    function ownerClaim(address _to) external onlyOwner {
        require(isOwnerClaimActive, "Claim by owner is not active");

        for (uint256 i; i < totalReservedForOwner; ++i) {
            _safeMint(_to, _tokenIdTracker.current());
            _tokenIdTracker.increment();
        }

        totalMinted += totalReservedForOwner;
        totalReservedForOwner = 0;
    }
