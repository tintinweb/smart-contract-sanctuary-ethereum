...
contract HollySkully is ERC721A, Ownable, ReentrancyGuard {
    ...
    string public baseURI = ""; // config
    string public uriSuffix = ".json"; // config
    string public hiddenMetadataUri; // config
    
    uint256 public publicSaleCost = 0.05 ether; // config
    uint256 public preSaleCost = 0.03 ether; // config
    uint256 public maxMintAmountPerTx = 5; // config
    uint256 public maxWhiteListMint = 5;
    uint256 private maxSupply = 6; // config need to +1 from intended supply
    ...
    constructor() ERC721A("HollySkully", "HSY") {
    }
    ...
    function initialize(string memory _hiddenMetadataUri, uint256 _publicSaleCost , uint256 _maxSupply, uint32 _revealedTime, uint32 _preSaleStartTime) public onlyOwner {
    ...
    function whiteListMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
        require(block.timestamp >= preSaleStartTime, "Pre sale not start!");
        require(msg.value >= preSaleCost * _mintAmount, "Insufficient funds!");
        require(_numberMinted(msg.sender) + _mintAmount <= maxWhiteListMint, "Max whitelist mint reached!");
