...
contract PirateGames is IPirateGames, Pausable {
    ...
    function pirateAttempt(address addr) internal {
        ...
        pendingMintAmt -= 1;
        uint16 colonistId = _mintCommits[addr][commitIdCur].tokenId;
        uint256 rawCost = rawMintCost(); 
        uint256 mined = pytheas.getColonistMined(addr, colonistId);
         require(
            mined >= rawCost,
            "You have not mined enough to attempt this action"); 
        uint16 piratesMinted = pirateNFT.piratesMinted();
        ...
        if (yayNay == 0) {
            pytheas.payUp(colonistId, mined, addr);
        } else {
            pytheas.handleJoinPirates(addr, colonistId);
            uint256 outStanding = mined - rawCost;
            raw.updateMintBurns(1, mined, rawCost);
            raw.mint(1, outStanding, addr);
            piratesMinted++;
     ...
     */
    function rawMintCost()
        public
        returns (uint256)
    {
        uint256 tokenId = colonistNFT.minted();
