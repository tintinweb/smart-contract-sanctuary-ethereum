...
contract Exchange is Ownable {
            ...
            keccak256(
                abi.encode(
                    keccak256(
                        ...
                        "uint256 optionAmount,address maker,uint256 accountId,uint256 expiry,uint256 salt,uint8 direction,uint8 close)"
                    ),
                    order.premiumToken,
                    order.optionId,
                    order.premiumAmount,
    ...
    function batchSettlement(
        OptionOrder[] memory bOrders,
        Sign[] memory bSigns,
        OptionOrder[] memory sOrders,
        Sign[] memory sSigns,
        uint256[] memory dealOptionAmounts,
        uint256[] memory dealPremiumAmounts
    ) external onlyBot(){
        require(bOrders.length > 0, "bOrders length = 0");
        require(bOrders.length == bSigns.length 
        && bSigns.length == sOrders.length 
        && sOrders.length == sSigns.length
        && sSigns.length == dealOptionAmounts.length
        && dealOptionAmounts.length == dealPremiumAmounts.length,"length error");
        for(uint8 i = 0; i < bOrders.length;i++){
            settlement(bOrders[i],bSigns[i],sOrders[i],sSigns[i],dealOptionAmounts[i],dealPremiumAmounts[i]);
        }
    }
    ...
    function settlement(
        ...
        require(bOrderHash == bOrder.orderHash && sOrderHash == sOrder.orderHash,"bOrderHash error");
        ...
        OptionFactoryInterface.Option memory option = OptionFactoryInterface(optionFactory).idToOption(sOrder.optionId);
        ...
        (
            ...
            uint256 sellerExpectPremium
                ...
                bOrder,
                sOrder,
                sOrderInfo.filledOptionAmount,
                bOrderInfo.filledPremiumAmount,
                dealOptionAmount,
                dealPremiumAmount
            );
    ...
    function getCollateralAmount(
        ...
        if (option.isPut) {
            return (10**decimals) * option.strikePrice * oAmount / (10**(OPTION_DECIMALS*2));
        } else {
            return (10**decimals) * oAmount / (10**OPTION_DECIMALS);
        }
    }
    function batchCancelOrder(bytes32[] memory orderHashs) public onlyBot{
        require(orderHashs.length > 0,"orderHash.length = 0");
        for(uint8 i=0;i < orderHashs.length;i++){
            cancelOrder(orderHashs[i]);
        }
    }

    function cancelOrder(bytes32 orderHash) public onlyBot{
