...
contract TestNft is ReserveNFT, ERC721, Ownable {
    ...
    function setBatchCount(uint256 _batchCount) external onlyOwner {
        require(
            _batchCount <= _remainingPublicSupply(),
            "Batch limit overflow"
        );
    ...
    function _isValidBatch(uint256 _batchCode) private view returns (bool) {
        return (_batchCode == 1 || batchCount > 0);
    }

    function airdropToken(address to) public onlyOwner {
        require(
    ...
    function _mintAction(address to, uint256 _batchCode)
        private
        returns (
            uint256,
            string memory,
        ...
        )
    {
        ...
        if (_batchCode == 0) reservedCount.increment();
        tokenId.increment();
        uint256 newId = tokenId.current();
        whitelistClaimed[to].increment();
        if (_batchCode > 1) batchCount = batchCount - 1;
        setBlockNumberOnLast();
        _safeMint(to, newId);
        ...
        )
    {
        ...
        require(_isValidBatch(meta.batchCode), "Batch is sold out"); 
        ...
        return _mintAction(msg.sender, meta.batchCode);
    } 
