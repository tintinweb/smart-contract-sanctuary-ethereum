...
import './LoveCoin.sol';
import './SkullsInLove.sol';

contract NFTStaking is Ownable, IERC721Receiver {
    uint256 public constant REWARD_PER_DAY = 1 ether;
    ...
    SkullsInLove public nft;
    LoveCoin public token;

    // maps tokenId to stake
    mapping(uint256 => Stake) public vault;

    constructor(SkullsInLove _nft, LoveCoin _token) {
        nft = _nft;
        token = _token;
    }
    ...
    function _claim(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            earned = (REWARD_PER_DAY * (block.timestamp - stakedAt)) / 86400;
            vault[tokenId] = Stake({
                owner: account,
                tokenId: uint24(tokenId),
                timestamp: uint48(block.timestamp)
...
contract LoveCoin is ERC20, ERC20Burnable, Ownable {
    ...
    function addController(address controller) external onlyOwner {
        require(!controllers[controller], 'Controller is already added');
        controllers[controller] = true;
    }

    function removeController(address controller) external onlyOwner {
        require(controllers[controller], 'Controller cannot be found');
...
contract SkullsInLove is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public maxMintAmountPerTxn;
    uint256 public maxMintAmountForHof;

    bool public isAllowlistSaleOpen = false;
    ...
    {
        require(isAllowlistSaleOpen, 'Allowlist sale is closed now!');
    ...
    function mintAL(uint256 _mintAmount, bytes32[] calldata _merkleProof)
        external
        payable
        mintCompliance(_mintAmount, maxMintAmountPerTxn)
        mintPriceCompliance(_mintAmount)
    {
        require(isAllowlistSaleOpen, 'Allowlist sale is closed now!');
    ...
    function mint(uint256 _mintAmount)
        ...
        mintPriceCompliance(_mintAmount)
    {
        require(isPublicSaleOpen, 'Public sale is closed now!');
    ...
    function mintDev(uint256 _mintAmount)
        external
        mintCompliance(_mintAmount, amountForDev)
        onlyOwner
    {
    ...
    function setMaxMintAmountPerTxn(uint256 _maxMintAmountPerTxn)
        external
        onlyOwner
    {
        maxMintAmountPerTxn = _maxMintAmountPerTxn;
    ...
    function setIsAllowlistSaleOpen(bool _state) external onlyOwner {
        isAllowlistSaleOpen = _state;
    }

    function setIsPublicSaleOpen(bool _state) external onlyOwner {
        isPublicSaleOpen = _state;
    }
    ...
    function withdraw() external onlyOwner nonReentrant {
        (bool success, ) = payable(DONATION_ADDRESS).call{
