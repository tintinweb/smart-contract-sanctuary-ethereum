...
contract AddLiquidity is BaseContract
{
    ...
    function addLiquidity() external 
    {
        ...
        address _RewardPool_ = addressBook.get("lpRewardPool");
        ...
        uint256 LiquidityAmount = _token_.balanceOf(address(this));
        uint256 amountToLiquify = LiquidityAmount / 2;
        uint256 amountToSwap = LiquidityAmount - amountToLiquify;

        if (amountToSwap == 0) {
            return;
        }

        _token_.approve(address(_router_), amountToSwap);
        address[] memory path = new address[](2);
        path[0] = address(_token_);
        path[1] = address(_payment_);
        uint256 balanceBefore = _payment_.balanceOf(address(this));
        ...
        uint256 amount_payment_Liquidity = _payment_.balanceOf(address(this)) -balanceBefore;

        _payment_.approve(address(_router_), amount_payment_Liquidity);
       _token_. approve(address(_router_), amountToLiquify);

        if (amountToLiquify > 0 && amount_payment_Liquidity > 0) {
            _router_.addLiquidity(
                address(_token_),
                address(_payment_),
                amountToLiquify,
                amount_payment_Liquidity,
                0,
                0,
                _RewardPool_,
                block.timestamp + 3600
