...
contract BasicERC721 is ERC721,Ownable,ContextMixin {
    ...
    uint256 public MAX_MINT_PER_ADDRESS_PRESLAE = 3;
    ...
    bool public GIFT_MINT = false;
    
    string private BASE_URI = "https://api.erc721.com/token/";
    string private CONTRACT_URI = "https://api.erc721.com/contract";
    ...
    function set_max_min_per_transaction(uint256 _max_mint) external onlyOwner {
        MAX_MINT_PER_TRANSACTION = _max_mint;
    }
    
    // Set maximum mintable token per address on public sale
    function set_max_min_per_address(uint256 _max_mint) external onlyOwner {
        MAX_MINT_PER_ADDRESS = _max_mint;
    }
    
    // Set maximum mintable token per address on pre sale
    function set_max_mint_per_address_presale(uint256 _max_mint) external onlyOwner {
        MAX_MINT_PER_ADDRESS_PRESLAE = _max_mint;
    }
    ...
    function verify_merkel_proof(bytes32[] calldata proof) public view returns (bool){
    ...
    function mint(uint256 amount)  payable public {
        ...
        require ( msg.value >= PRICE.mul(amount) , "Invalid Price.");
    ...
    function presaleMint(uint256 amount,bytes32[] calldata proof) payable public {
        ...
        require (PRE_SALE_ACTIVE  && !PUBLIC_SALE_ACTIVE, "Presale is not active.");
        require(amount > 0 , "Invalid Amount");
        require (amount + totalSupply() <= TOTAL_SUPPLY,"Maximum number of mintable tokens has been reached.");
    ...
    function toggleGiftMint() external onlyOwner {
        GIFT_MINT = !GIFT_MINT;
    }
    
    function giftMint(uint256 amount, address _to_address) public payable {
        require(GIFT_MINT, "Gift minting is not active!");
        require (amount > 0 ,"invalid amount");
        require (amount + totalSupply() <= TOTAL_SUPPLY,"Maximum number of mintable tokens has been reached.");
        require (PUBLIC_SALE_ACTIVE , "Public is not active.");
        require (amount <= MAX_MINT_PER_TRANSACTION , "The number of tokens selected exceeds the maximum allowed per transaction.");
    ...
    function togglePreSale() external onlyOwner {
        ...
        PUBLIC_SALE_ACTIVE = !(PRE_SALE_ACTIVE);
    }
    ...
    function toggleSale() external onlyOwner{
        ...
        PRE_SALE_ACTIVE = !(PUBLIC_SALE_ACTIVE);
    }
    ...
    function bulkMint(address[] memory _addresses, uint256 _amount) external onlyOwner{
        ...
        require((_amount * _addresses.length) + totalSupply() <= TOTAL_SUPPLY,"Maximum number of mintable tokens has been reached.");
