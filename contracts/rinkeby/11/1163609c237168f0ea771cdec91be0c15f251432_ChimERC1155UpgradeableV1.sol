...
contract ChimERC1155UpgradeableV1 is Initializable, ContextUpgradeable, OwnableUpgradeable, ERC1155SupplyUpgradeable, ERC1155BurnableUpgradeable, ERC1155PausableUpgradeable {
    ...
    // Mapping for approved minter list
    mapping(address => bool) private _approvedMinterList;
    ...
    // Emitted when `minterAddress` added to approved minter list.
    event AddToApprovedMinterList(address minterAddress);
    // Emitted when `minterAddress` removed from approved minter list.
    event RemoveFromApprovedMinterList(address minterAddress);

    modifier onlyApprovedMinter(address minterAddress_) {
        require(_approvedMinterList[minterAddress_], "ChimERC1155: caller is not approved");
        _;
    }
    ...
    function initialize(
        ...
        __ChimERC1155V1_init(name_, symbol_, baseUri_);
    }

    function __ChimERC1155V1_init(
        string memory name_,
        string memory symbol_,
        string memory baseUri_
        ...
        __ChimERC1155V1_init_unchained(name_, symbol_, baseUri_);
    }

    function __ChimERC1155V1_init_unchained(
        string memory name_,
        string memory symbol_,
        string memory /*baseUri_*/
    ) internal initializer {
    ...
    function tokenInfoBatch(uint256[] memory tokenIds_) external view virtual returns (uint256[] memory batchTokenSupplies, string[] memory batchTokenURIs) {
        batchTokenSupplies = new uint256[](tokenIds_.length);
        batchTokenURIs = new string[](tokenIds_.length);
        for (uint256 i = 0; i < tokenIds_.length; ++i) {
            batchTokenSupplies[i] = totalSupply(tokenIds_[i]);
            batchTokenURIs[i] = uri(tokenIds_[i]);
        }
        return (
            batchTokenSupplies,
            batchTokenURIs
    ...
    function isApprovedMinter(address minterAddress_) external view virtual returns (bool) {
        return _approvedMinterList[minterAddress_];
    }
    ...
    function mint(
        ...
        uint256 amount_,
        bytes memory data_
    ) external virtual onlyApprovedMinter(_msgSender()) {
    ...
    function setTokenURI(uint256 tokenId_, string memory tokenURI_) external virtual onlyApprovedMinter(_msgSender()) {
    ...
    function updateRoyaltyParams(address royaltyAddress_, uint256 royaltyPercent_) external virtual onlyOwner {
        require(royaltyPercent_ <= _MAX_PERCENT, "ChimERC1155: invalid percent");
        _royaltyAddress = royaltyAddress_;
        _royaltyPercent = royaltyPercent_;
        emit RoyaltyParamsUpdated(royaltyAddress_, royaltyPercent_);
    }

    function addToApprovedMinterList(address minterAddress_) external virtual onlyOwner {
        _approvedMinterList[minterAddress_] = true;
        emit AddToApprovedMinterList(minterAddress_);
    }

    function removeFromApprovedMinterList(address minterAddress_) external virtual onlyOwner {
        _approvedMinterList[minterAddress_] = false;
        emit RemoveFromApprovedMinterList(minterAddress_);
    }
