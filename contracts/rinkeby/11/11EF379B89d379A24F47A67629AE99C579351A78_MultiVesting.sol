...
contract MultiVesting is Ownable {
  ...
  struct Vesting {
    ...
    uint256 totalAmount; // Vested amount MV
  ...
  // Constants
  // ===============================================================================================================
  uint256 public constant STEPS_AMOUNT = 20; // 20 steps, each step unlock 1/13 of funds after 90 days
  ...
  function addVesting(
    ...
    //require(
    //  _startedAt <= (block.timestamp + 180 days),
    //  "TIMESTAMP_CANNOT_BE_MORE_THAN_A_180_DAYS_IN_FUTURE"
    //);
  ...
  function getAvailableAmountAtTimestamp(
    ...
    uint256 periodPassed = _timestamp.sub(vesting.startedAt).div(90 days); // We say that 3 months are always 90 days

    uint256 alreadyReleased = vesting.releasedAmount;

    if (periodPassed >= STEPS_AMOUNT) {
      return vesting.totalAmount.sub(alreadyReleased);
    }

    uint256 rewardPer3periodsPhase1 = vesting.totalAmount.mul(7).div(100);
    uint256 rewardPer3periodsPhase2 = vesting.totalAmount.mul(2).div(100);

    uint256 phase1Count = 12;
    if (periodPassed <= phase1Count) {
      return rewardPer3periodsPhase1.mul(periodPassed).sub(alreadyReleased);
    } else {
      uint256 rewardPhase1 = rewardPer3periodsPhase1.mul(phase1Count);
      uint256 phase2Count = periodPassed.sub(phase1Count);
      uint256 rewardPhase2 = rewardPer3periodsPhase2.mul(phase2Count);
