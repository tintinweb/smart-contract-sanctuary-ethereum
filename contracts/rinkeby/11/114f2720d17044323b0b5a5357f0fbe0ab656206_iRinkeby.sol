...
// Data structure that defines the elements of stakedToken
    ...
    uint256 public constant GOOP_INTERVAL = 30 minutes; // The interval upon which Goop is calcualated
    uint256 public goopPerInterval = 1; // Goop awarded per interval
  ...
  /**
    ...
    function reroll(uint256[] calldata _tokenIds) external {
        ...
        require(rerollState == true, "reroll not enabled");
        require(goop[_msgSender()] >= amount, "not enough goop for reRoll");

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tId = _tokenIds[i];

            require(stakedToken[tId].ownerOfNFT == _msgSender(), "you dont own this token or its not staked");

            rollTracker[periodCounter + 1].push(tId);
            stakedToken[tId].lastRerollPeriod = periodCounter;
        }
    ...
    function stakeNFT(uint256[] calldata _tokenIds) external {
        require(stakingState == true, "staking not enabled");
    ...
    function unstakeNFT(uint256[] calldata _tokenIds) public {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(stakedToken[tId].ownerOfNFT == _msgSender(), "you are not the owner");
            require(canBeUnstaked(tId), "token in reroll or cool down period");

            _transfer(address(this), _msgSender(), tId);

            delete stakedToken[tId].ownerOfNFT;
            delete stakedToken[tId].timestamp;
            delete stakedToken[tId].lastRerollPeriod;

            /**
             * @dev - iterates the array of tokens staked and pops the one being unstaked
             */
            for (uint256 j = 0; j < staker[_msgSender()].length; j++) {
                if (staker[_msgSender()][j] == tId) {
    ...
    function unclaimedGoop() external view returns (uint256) {
        ...
        for (uint256 i = 0; i < staker[addr].length; i++) {
            uint256 tId = staker[addr][i];

            stakedTime += block.timestamp - stakedToken[tId].timestamp;
        }
    ...
    function claimGoop() public {
        ...
        for (uint256 i = 0; i < staker[addr].length; i++) {
            uint256 tId = staker[addr][i];
            stakedTime += block.timestamp - stakedToken[tId].timestamp;
            stakedToken[tId].timestamp = block.timestamp;
        }
    ...
    function spendGoop(uint256 _item, uint256 _count) public override {
        ...
        require(saleItems[_item].saleStatus == true, "Item not currently for sale");
    ...
    function verify122() public returns(uint256){
        return 5;
    }
}
