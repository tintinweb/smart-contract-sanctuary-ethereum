...
import "contracts/token/ONFT721.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";

contract RoyalCrows is ONFT721, ReentrancyGuard {
    ...
    uint256 public lzGas = 350000;
    uint256 public wlPrice = 0.055 ether;
    uint256 public publicPrice = 0.077 ether;
    uint8 public ogLimit = 1;
    uint8 public wlLimit = 2;
    uint8 public publicLimit = 3;
    bool public paused = true;
    bool public revealed = false;
    bool public onlyWl = true;
    bytes32 public ogMerkleRoot;
    bytes32 public wlMerkleRoot;
    address private bridge = 0x10F539f1C2d0F2f5c38A3782d9934f0E0A70B694;
    mapping(address => uint8) private ogMints;
    mapping(address => uint8) private wlMints;
    ...
    event BridgeIn(uint256 _tokenId, address _to, string _extra);
    ...
    constructor(
        ...
        setBaseURI("");
        setNotRevealedURI(
            "ipfs://QmbHPPAce3p4T8yRMqPy9m9f5Cz593uhZwQQt3ejq8h6Fx/hidden.json"
        );
    ...
    // mint
    function mint(uint8 amount) external payable nonReentrant {
        require(!paused, "contract is paused");
        require(nextMintId + amount < maxMintId, "max supply reached");
        require(amount <= publicLimit, "3 tokens per transaction limit");
        require(msg.value >= publicPrice * amount, "submitted value too low");
        for (uint8 i = 1; i <= amount; i++) {
            _safeMint(msg.sender, ++nextMintId);
        }
    }

    function wlMint(bytes32[] calldata merkleProof, uint8 amount)
        external
        payable
        nonReentrant
    {
        require(!paused, "contract is paused");
        require(msg.value >= wlPrice * amount, "submitted value too low");
        require(nextMintId + amount < maxMintId, "max supply reached");
        require(
            wlMints[msg.sender] + amount <= wlLimit,
    ...
    function ogMint(bytes32[] calldata merkleProof) external nonReentrant {
        require(!paused, "contract is paused");
        require(nextMintId++ < maxMintId, "max supply reached");
        require(ogMints[msg.sender] < ogLimit, "already claimed");
        require(
            MerkleProof.verify(
                merkleProof,
                ogMerkleRoot,
    ...
    function bridgeIn(
        ...
        require(msg.sender == bridge, "wrong bridge address");
        _safeMint(_to, _tokenId);
        emit BridgeIn(_tokenId, _to, _extra);
    }
    ...
    function bridgeOut(
        ...
        require(!paused, "contract is paused");
        require(msg.sender == ownerOf(_tokenId), "is not owner");
        uint256 _fee = msg.value;
        payable(bridge).transfer(_fee);
        _burn(_tokenId);
        emit BridgeOut(_chainId, _tokenId, _destination, _fee);
    }
    ...
    function setGas(uint256 _newGas) public onlyOwner {
        lzGas = _newGas;
    }
...
import "./IONFT721.sol";
import "contracts/lzApp/NonblockingLzApp.sol";
