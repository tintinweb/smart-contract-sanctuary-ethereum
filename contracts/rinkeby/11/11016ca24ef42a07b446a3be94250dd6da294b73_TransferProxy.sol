...
contract Trade {
    ...
    function verifySellerSign(address seller, uint256 tokenId, uint256 amount, address paymentAssetAddress, address assetAddress, bytes memory sign) public pure {
    ...
    function verifyBuyerSign(address buyer, uint256 tokenId, uint256 amount, address paymentAssetAddress, address assetAddress, uint256 qty, bytes memory sign) public pure {
        ...
        )
    {
      ...
      function getFees(uint256 paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint256 tokenId) public view returns(Fee memory){
        address tokenCreator;
        uint256 platformFee;
        uint256 royaltyFee;
        uint256 assetFee;
        uint256 royaltyPermille;
        uint256 temp = paymentAmt.mul(1000);
        uint256 temp1 = 1000+buyerFeePermille;
        uint256 price =temp.div((temp1));
        ...
        royaltyFee = price.mul(royaltyPermille);
        royaltyFee=royaltyFee.div(1000);
        // assetFee = price.sub(royaltyFee);
        // assetFee= assetFee.sub(sellerFee);
