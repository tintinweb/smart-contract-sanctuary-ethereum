...
contract WPunkGateway is IWPunkGateway, IERC721Receiver, Ownable {
    ...
    function withdrawPunk(
        ...
        INToken aWPunk = INToken(Pool.getReserveData(address(WPunk)).xTokenAddress);
        //console.log('before safe transfer from');
        uint256 userBalance = aWPunk.balanceOf(msg.sender);
        //console.log('user balance is', userBalance);
        for (uint256 i = 0; i < punkIndexes.length; i++) {
            aWPunk.safeTransferFrom(msg.sender, address(this), punkIndexes[i]);
        }
        //console.log('before pool withdraw ERC721');
