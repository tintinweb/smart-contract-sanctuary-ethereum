 ...
 */
  ...
  */
  ...
  function mintNft(uint256 _baseId) external {
    ...
    require(itemNFTIdRangeAndUri[_baseId].mintedCount < (itemNFTIdRangeAndUri[_baseId].maxId -  itemNFTIdRangeAndUri[_baseId].minId), "Can't mint anymore please talk to admins!");
...
/*     itemNFTIdRangeAndUri[_baseId].mintedCount = _minId; */
  ...
  function _afterReceive(
    ...
    //itemId transfers here for the first time
    if(!transfarredLockToken[_tokenId].isLocked) {
      mintAfterTransfer(_tokenId, _baseId, _toAddress);
    } else {
      safeTransferFrom(address(this),_toAddress,_tokenId,1,"0x00");
      transfarredLockToken[_tokenId].isLocked = false;
    }
  }
