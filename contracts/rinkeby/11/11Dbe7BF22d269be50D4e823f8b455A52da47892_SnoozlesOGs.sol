...
// File: contracts/Snoozles-OG.sol
...
contract SnoozlesOGs is ERC721A, Ownable {
    ...
    uint256 public constant SALE_PRICE = 0 ether;
    ...
    constructor() ERC721A("Snoozles OG Membership Pass", "SNZLSOG") {
    ...
    function WaitlistMint(bytes32[] memory _merkleProof) external payable callerIsUser {
        require(!pause, "Snoozles OG Membership Pass mint has been paused.");
        require(waitlistSale, "Waitlist minting is paused.");
        require((totalSupply() + 1) <= MAX_SUPPLY, "Out of Snoozles OG Membership Passes.");
        require((totalMint[msg.sender] + 1) <= MAX_MINT, "Only one Snoozles OG Membership Pass per member.");
        require(msg.value >= SALE_PRICE, "Attribute some value for a Snoozles OG Membership Pass.");

        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, waitlistMerkleRoot, sender), "Not on the OG waitlist.");

        totalMint[msg.sender] += 1;
        _safeMint(msg.sender, 1);
    }

    function OGMint(bytes32[] memory _merkleProof) external payable callerIsUser {
        require(!pause, "Snoozles OG Membership Pass mint has been paused.");
        require(OGSale, "OG minting is paused.");
        require((totalSupply() + 1) <= MAX_SUPPLY, "Out of Snoozles OG Membership Passes.");
        require((totalMint[msg.sender] + 1) <= MAX_MINT, "Only one Snoozles OG Membership Pass per member.");
        require(msg.value >= SALE_PRICE, "Attribute some value for a Snoozles OG Membership Pass.");

        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, OGMerkleRoot, sender), "Not on the OG list.");
    ...
    function setWaitlistMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
    ...
    function getWaitlistMerkleRoot() external view returns (bytes32) {
        return waitlistMerkleRoot;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        payable(0xD1b9b5401f6F96C4C8D0530F8D22b805B7aaE4f1).transfer(withdrawAmount_10);
        // withdraw funds: Foundation's Treasury
        payable(0x15CD9b5E8a44DFA15695B4162F1EC7964A820b15).transfer(withdrawAmount_10);
        // withdraw funds: Snoozles Company
        payable(0x735FDB93A4777F57ca1D0f19956Da8cbd5589Efc).transfer(withdrawAmount_80);
