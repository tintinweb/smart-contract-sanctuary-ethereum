...
contract Treasury is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function get_tokens(uint reward, uint requestedAmount0, uint requestedAmount1, address payable to) external onlyLsc returns (uint sentToken, uint sentBlxm) {
        uint liquidityToRetrieve = calculate_liquidity_amount(requestedAmount0, requestedAmount1);
        require(balances[to] >= liquidityToRetrieve, "No enough balance to retreive tokens");
        ...
        uint liquidityToRetrieveReserves = Math.max(liquidityToRetrieve * reserve1 * requestedAmount0 / (reserve0 * requestedAmount1), liquidityToRetrieve * reserve0 * requestedAmount1 / (reserve1 * requestedAmount0));
        ...
        cash -= liquidityToRetrieve;
        balances[to] -= liquidityToRetrieve;
        totalLiquidity -= liquidityToRetrieve;
    }
    ...
    function find_pool_to_add(uint amountLiquidity) private view returns (uint[] memory, uint[] memory){
        ...
        for (uint i = 0; i < numberOfPools; i++) {
            uint currentPoolPercentage = pools[i].amountLiquidity * PERCENT_PRECISION * 100 / liquidityInPools;
    ...
    function find_pool_to_fill_reserves(uint amountLiquidity, uint expectedToken, uint expectedBlxm) private view returns (uint[] memory, uint[] memory) {
        ...
        if (finalLiquidity == 0) {
            ...
            uint poolIndex;
            for (uint i = 0; i < numberOfPools; i++) {
                uint poolPercentage = pools[i].amountLiquidity * PERCENT_PRECISION * 100 / liquidityInPools;
            ...
            if (poolToRemove.amountLiquidity < amountLiquidity ||
                (poolToRemove.percentage * PERCENT_PRECISION > (poolToRemove.amountLiquidity - amountLiquidity) * PERCENT_PRECISION * 100 / finalLiquidity &&
                poolToRemove.percentage * PERCENT_PRECISION - (poolToRemove.amountLiquidity - amountLiquidity) * PERCENT_PRECISION * 100 / finalLiquidity > threshold)
            ) {
                poolsIndexes = new uint[](numberOfPools);
                amountsToRemove = new uint[](numberOfPools);
