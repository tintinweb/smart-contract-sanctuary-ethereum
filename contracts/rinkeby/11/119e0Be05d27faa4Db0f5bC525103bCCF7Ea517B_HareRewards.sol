    ...
    VRFConsumerBaseV2
{
    ...
    uint32 private chainlinkCallbackGasLimit = 1200000;
    uint16 private chainlinkRequestConfirmations = 3;
    ...
    // Pending Ids
    uint256[51] private _pendingIds;

    string public baseURI =
        "ipfs://QmWKQQmSYaTfg2Yrbgm6VFiJbhhB8DqDvwszfUuYJf4nnE/";
    ...
    constructor(
        ...
        VRFConsumerBaseV2(_chainlinkCoordinatorAddress)
        ERC721("Hare Rewards", "HRW")
    {
        chainlinkVrfCoordinator = _chainlinkCoordinatorAddress;
        chainlinkSubscriptionId = _chainlinkSubId;
    ...
    function setVRFSettings(
        ...
        chainlinkRequestConfirmations = _requestConfirmations;
        chainlinkCallbackGasLimit = _callbackGasLimit;
        chainlinkKeyHash = _keyHash;
        chainlinkSubscriptionId = _subscriptionId;
    }

    function purchase(uint256 _mintAmount) external payable {
        require(_mintAmount > 0, "Mint Amount ! > 0");
        require(
            _mintAmount <= maxMintAmount,
            "Mint Amount ! < Max Mint Amount"
        );
        if (msg.sender != owner()) {
            require(
                msg.value == _calculatePrice(_mintAmount),
                "Incorrect funds sent"
            );
        }

        requestRandomValues(uint32(_mintAmount), msg.sender);
    }

    function requestRandomValues(uint32 numValues, address _to) internal {
    ...
    function fulfillRandomWords(
        ...
        uint256 size = _randomWords.length;

        for (uint256 i; i < size; i++) {
            _randomMint(mintersByRequestId[requestId], _randomWords[i]);
        }
    }
    ...
    function _getPendingAtIndex(uint256 _index)
        internal
        view
        returns (uint256)
    {
    ...
    function _calculatePrice(uint256 _mintAmount)
        internal
        view
        returns (uint256)
    {
        return nftPrice * _mintAmount;
    }
    ...
    function sweepTokens(address tokenAddress, uint256 amount)
        external
        onlyOwner
    {
    ...
    function setPrice(uint256 _newPrice) public onlyOwner {
        uint256 someWeiValue = _newPrice * 1 ether;
        nftPrice = someWeiValue;
    }
