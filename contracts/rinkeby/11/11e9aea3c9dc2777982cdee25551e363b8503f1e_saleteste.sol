...
contract saleteste is IBEP20, ReentrancyGuard, ERC721Holder, Context, Ownable {
    ...
    mapping(address => uint256) public _convidados;
   ...
   function stake(uint256 tokenId, uint256 cargo) external {
    ...
    function unstake(uint256 tokenId, uint256 cargo) external {
        if(cargo == 1){
            ...
            uint256 burnfees = calculateTokens(tokenId).mul(fees).div(10**2);
            _transfer(address(this), msg.sender, calculateTokens(tokenId).sub(burnfees));
            _transfer(address(this), address(0), burnfees); // burn tokens
            ...
            uint256 burnfees = calculateTokens2(tokenId).mul(fees).div(10**2);
            _transfer(address(this), msg.sender, calculateTokens2(tokenId).sub(burnfees));
            _transfer(address(this), address(0), burnfees); // burn tokens
            ...
            uint256 burnfees = calculateTokens3(tokenId).mul(fees).div(10**2);
            _transfer(address(this), msg.sender, calculateTokens3(tokenId).sub(burnfees));
            _transfer(address(this), address(0), burnfees); // burn tokens
            ...
            uint256 burnfees = calculateTokens4(tokenId).mul(fees).div(10**2);
            _transfer(address(this), msg.sender, calculateTokens4(tokenId).sub(burnfees));
            _transfer(address(this), address(0), burnfees); // burn tokens
    ...
    function stakefees(uint256 burnfees) external onlyOwner {
	    require(burnfees >= 0 && burnfees <= 100, "Value out of range: values between 0 and 100");
