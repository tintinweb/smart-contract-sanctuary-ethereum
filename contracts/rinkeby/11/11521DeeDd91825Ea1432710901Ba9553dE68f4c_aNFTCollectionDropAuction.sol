...
contract aNFTCollectionDropAuction is aNFTLogic {
  ...
  mapping (address => MintLog[]) public alRefundDetails;//TODO: set private on production deploy
  ...
  */
  ...
  function _addAlRefund(address _address, uint256 _price, uint256 _amount) internal {
    ...
    for(uint256 c = 0; c < refundALcount; c++){//max possible refundALcount is 31
  ...
  function claimAllowlistRefund() external nonReentrant {
    ...
    for(uint256 c = refundALcount; c > 0; c--){//max possible refundALcount is 31
  ...
  function withdrawRefundPool() external onlyOwner {//no pagination
    uint256 limit = alAddresses.length();
    
    for(uint256 c = 0; c < limit; c++){
      delete alRefundDetails[alAddresses.at(c)];
    }
  ...
  function withdrawRefundPoolLimited(uint256 limit) external onlyOwner {//with pagination
    
    uint256 freedEth;
    uint256 totalLength = alAddresses.length();

    require(limit <= totalLength, 'limit > totalLength');

    for(uint256 i = 1; i <= limit; i++){
      address _alAddress = alAddresses.at(totalLength - i);//to be able to remove address and don't reset the index
      alAddresses.remove(_alAddress);

      uint256 refundALcount = alRefundCount(_alAddress);
      MintLog memory mintLog;
      for(uint256 c = 0; c < refundALcount; c++){//max possible refundALcount is 31
          mintLog = alRefundDetails[msg.sender][c];
          freedEth += mintLog.price*mintLog.amount;
      }
     
      delete alRefundDetails[_alAddress];
    }
    
    alRefundPool -= freedEth;
    payable(msg.sender).transfer(freedEth);
  }
