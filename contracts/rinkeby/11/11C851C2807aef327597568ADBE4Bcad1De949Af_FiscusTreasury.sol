...
contract FiscusTreasury is FiscusAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    constructor(
        ...
        require(_fisc != address(0), "Zero address: FISC");
        FISC = IFISC(_fisc);
    ...
    /**
     * @notice allow approved address to deposit an asset for FISC
    ...
    function deposit(
        ...
        // mint FISC needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        FISC.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn FISC for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        FISC.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new FISC using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        FISC.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or FISC from the treasury, using sFISC as collateral. This might allow an
     * sFISC holder to provide FISC liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(FISC)) {
            require(permissions[STATUS.FISCDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sFISC.changeDebt(value, msg.sender, true);
        require(sFISC.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(FISC)) {
            FISC.mint(msg.sender, value);
            fiscDebt = fiscDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sFISC.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with FISC
     * @param _amount uint256
     */
    function repayDebtWithFISC(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.FISCDEBTOR][msg.sender],
            notApproved
        );
        FISC.burnFrom(msg.sender, _amount);
        sFISC.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        fiscDebt = fiscDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(FISC), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SFISC) {
            sFISC = IsFISC(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SFISC) {
            // 9
            sFISC = IsFISC(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(FISC.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns FISC valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(FISC)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return FISC.totalSupply() - fiscDebt;
    }
}
...
interface IsFISC is IERC20 {
    function rebase(uint256 fiscProfit_, uint256 epoch_) external returns (uint256);
...
abstract contract FiscusAccessControlled {
    /* ========== EVENTS ========== */

    event AuthorityUpdated(IFiscusAuthority indexed authority);

    string UNAUTHORIZED = "UNAUTHORIZED"; // save gas

    /* ========== STATE VARIABLES ========== */

    IFiscusAuthority public authority;

    /* ========== Constructor ========== */

    constructor(IFiscusAuthority _authority) {
    ...
    function setAuthority(IFiscusAuthority _newAuthority) external onlyGovernor {
