...
contract TrollTownElixir is ERC1155, Ownable {
    ...
    function claimDrop() external {
        ...
        for (uint256 i; i < trolltown.balanceOf(msg.sender); i++) {
            uint256 tokenId = trolltown.tokenOfOwnerByIndex(msg.sender, i);
            if (!isClaimed(tokenId)) {
                _setClaimed(tokenId);
                numTokensClaimed++;
            }
        }
        _mint(msg.sender, 0, numTokensClaimed, "");
    }
    ...
    function burnElixirForAddress(uint256 typeId, address burnTokenAddress) external {
        ...
        require(balanceOf(msg.sender, typeId) > 0, "YOU_DONT_HAVE_ENOUGH_ELIXIR");
