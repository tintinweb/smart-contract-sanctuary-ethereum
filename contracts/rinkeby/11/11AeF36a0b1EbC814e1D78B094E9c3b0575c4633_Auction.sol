...
contract Auction {
    ...
    function cancelListingAuction(
        ...
        listing.cancelListingAuction(_listingId, msg.sender, _isWeb);
    ...
    function placeBidAuction(
        ...
        if (!whiteList[msg.sender] || _isWeb)
            stanFund[msg.sender] -= (reOffer(_subOfferId, _auctionId, _amount) +
                feeSystem.stanFee);

        address maker = getOwnerWebOrMobile(_tokenId, _isWeb);
    ...
    function acceptOfferPvP(
        ...
        if (
            ...
            msg.sender != stanNFT.getOwnerOfNFTMobile(tokenId)
        ) revert InvalidOwner();
    ...
    function acceptOfferAuction(
        ...
        if (!_isWeb && msg.sender != stanNFT.getOwnerOfNFTMobile(tokenId))
            revert InvalidOwner();
    ...
    function cancelOfferAuction(
        ...
        if (!_isWeb && maker != stanNFT.getOwnerOfNFTMobile(tokenId))
            revert InvalidOwner();
    ...
    function transferNFTPvP(
        ...
        processNFTOwner(msg.sender, _receiver, _tokenId, _isWeb);
