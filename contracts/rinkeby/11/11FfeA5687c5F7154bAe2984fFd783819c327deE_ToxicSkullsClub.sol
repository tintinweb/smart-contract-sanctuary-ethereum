...
contract ToxicSkullsClub is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public costAllowlist = 0.07 ether;
    uint256 public maxSupply = 9999;
    uint256 public maxMint = 10;
    uint256 public maxMintAL = 2;
    bool public paused = false;
    bool public allowlistPaused = false;

    //backend settings
    string public baseURI;
    address internal immutable pagzi = 0xF4617b57ad853f4Bc2Ce3f06C0D74958c240633c;
    address internal immutable founder = 0x1ff63DF1077a40ec7A4f5a85a07eA7aC773EF368;
    address internal immutable cofounder = 0x9C3EDD974552898350bbD3425608CE03cDE2426a;
    mapping(address => bool) public projectProxy;

    //date variables
    uint256 public publicDate = 1649324629;
    uint256 public allowlistDate = 1649324629;
    ...
    modifier checkallowlistDate() {
        require(!allowlistPaused, "Allowlist sale is paused!");
        require((allowlistDate <= block.timestamp),"Allowlist is not yet!");
        _;
    }
    ...
    modifier checkALPrice(uint256 _mintAmount) {
        require(msg.value >= costAllowlist * _mintAmount, "Insufficient funds!");
        _;
    }
    ...
    function mintPass(uint256 count) external payable checkLimit(count) checkALPrice(count) checkallowlistDate {
        uint256 totalSupply = _owners.length;
    ...
    function mintAllowlist(uint256 count, bytes32[] calldata _merkleProof) public payable checkLimit(count) checkALPrice(count) checkallowlistDate {
        // Verify allowlist requirements
        require((claimed[_msgSender()] + count) > (maxMintAL + 1), "Address has no allowance!");
    ...
    function walletOfOwner(address _owner) public view returns (uint256[] memory) {
        ...
        uint256 currentTokenId = _owners.length;
        uint256 ownedTokenIndex = 0;
        while (ownedTokenIndex < ownerTokenCount && currentTokenId <= maxSupply) {
            if(_owners[currentTokenId] == _owner){
    ...
    function setALCost(uint256 _costAllowlist) external onlyOwner {
        costAllowlist = _costAllowlist;
    }
    function setPublicDate(uint256 _publicDate) external onlyOwner {
        publicDate = _publicDate;
    }
    function setallowlistDate(uint256 _allowlistDate) external onlyOwner {
        allowlistDate = _allowlistDate;
    }
    function setDates(uint256 _publicDate, uint256 _allowlistDate) external onlyOwner {
        publicDate = _publicDate;
        allowlistDate = _allowlistDate;
    }
    ...
    function setallowlistPaused(bool _allowlistPaused) public onlyOwner {
        allowlistPaused = _allowlistPaused;
    }
    ...
    function withdraw() public onlyOwner {
        ...
        payable(founder).transfer((balance * 720) / 1000);
        payable(cofounder).transfer((balance * 80) / 1000);
    }
}
