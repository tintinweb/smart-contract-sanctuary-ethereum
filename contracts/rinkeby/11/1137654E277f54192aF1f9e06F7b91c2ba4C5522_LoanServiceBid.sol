...
contract LoanServiceBid is Ownable {
    ...
    function create(uint256 _loanId, uint256 _amount) public {
        address loanStorageBidAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageBidAddress();
        LoanStorageBid_I(loanStorageBidAddress).create(msg.sender, _loanId, _amount);
    }
    ...
    function bids(uint256 _bidId) public view returns(LoanCommon.Bid memory) {
        address loanStorageBidAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageBidAddress();
        return LoanStorageBid_I(loanStorageBidAddress).bids(_bidId);
    }

    /// @notice convenient method to get a list of bids
    /// @param _start The starting index
    /// @param _end The ending index
    function list(uint256 _start, uint256 _end) public view returns(LoanCommon.Bid[] memory) {
        require(_end >= _start, "The end index must be greater than or equal to the start index");
        LoanCommon.Bid[] memory bidList = new LoanCommon.Bid[](_end.sub(_start).add(1));

        uint256 index = 0;
        for (uint256 i = _start; i <= _end; i++) {
            bidList[index] = bids(i);
            index++;
        }
    ...
    /// @notice get a list of bid IDs by loan ID
    /// @param _loanId The Loan ID
    function bidsByLoanId(uint256 _loanId) public view returns(uint256[] memory) {
        address loanStorageBidAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageBidAddress();
        return LoanStorageBid_I(loanStorageBidAddress).bidsByLoanId(_loanId);
    }


    /// @notice list of bids by loan id
    /// @notice _loanId The loan id
    function listByLoanId(uint256 _loanId) public view returns(LoanCommon.Bid[] memory) {
        uint256[] memory bidIds = bidsByLoanId(_loanId);
        LoanCommon.Bid[] memory bidList = new LoanCommon.Bid[](bidIds.length);
