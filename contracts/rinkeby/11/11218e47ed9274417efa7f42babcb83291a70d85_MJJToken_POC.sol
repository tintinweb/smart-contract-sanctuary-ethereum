...
contract MJJToken_POC is ERC721, Ownable
{
    uint256 public tokenCounter;
    mapping(uint256 => string) private productURIs;
    string public baseURI;
    
    constructor() ERC721("MJJToken_POC", "MJJ")
    {
        baseURI = "https://mjjones.com/mjjtoken/";
    }

    function mint(address to, string memory productURI) external onlyOwner
    {
        unchecked{tokenCounter++;}
        productURIs[tokenCounter] = productURI;
        _safeMint(to, tokenCounter);
    }

    function tokenURI(uint256 id) public view virtual override returns (string memory)
    {
        require(id > 0 && id <= tokenCounter, "token not minted");
        return string(abi.encodePacked(baseURI, productURIs[id]));
    }

    function setProductURI(uint256 id, string memory productURI) external onlyOwner
    {
        require(id > 0 && id <= tokenCounter, "token not minted");
        productURIs[id] = productURI;
    }
    ...
    function getProductTokens(string memory product) external view returns (uint256[] memory)
    {
        uint256[] memory tokens = new uint256[](0);

        for(uint256 i = 1; i <= tokenCounter; i++)
        {
            if(keccak256(bytes(productURIs[i])) == keccak256(bytes(product)))
            {
                unchecked
                {
                uint256[] memory oldTokens = tokens;
                tokens = new uint256[](oldTokens.length + 1);
