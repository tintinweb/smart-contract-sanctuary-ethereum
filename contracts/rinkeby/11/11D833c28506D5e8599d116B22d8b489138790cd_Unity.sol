...
// File: contracts/NBS.sol

pragma solidity ^0.8.7;

contract Unity is ERC721Enumerable, Ownable {
    ...
    function freeMint(uint256 count) public payable {
        uint256 totalSupply = _owners.length;
        require(freeMintActive, "Free Mint is inactive");
        require(totalSupply + count <= MAX_SUPPLY, "Exceeds max supply.");
        require(count < MAX_PER_TX, "Exceeds max per transaction.");
        require(
            addressToMinted[_msgSender()] + count < 6,
            "Exceeds maximum mint per wallet"
        );
