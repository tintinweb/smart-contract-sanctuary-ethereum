...
// File: The Sevens/ERC721ALowCap.sol
...
contract Eve is ERC721A, ERC721ALowCap, Ownable {
    ...
    uint public constant ReservedSupply = 25;
    uint public constant PublicSupply = MaxSupply - ReservedSupply;

    // Mint Settings
    uint public constant MintPassWhitelistMintPrice =  0.000049 ether;
    uint public constant WhitelistMintPrice =  0.00006 ether;
    uint public constant PublicMintPrice =  0.00007 ether;
    ...
    string baseURI = "testing/metadata/";
    ...
    function rollRandomStartingIndex() external directOnly {
        require(keccak256(abi.encodePacked(provenanceHash)) != keccak256(abi.encodePacked('')), "Provenance hash has not been set");
        require(randomStartingIndex == 0, "Random index has already been set permanently");
    ...
    function airdrop(address[] calldata to, uint[] calldata amounts) external onlyOwner {
        uint totalDropped = 0;
        unchecked {
            uint len = to.length;
            require(len == amounts.length, "INVALID_LENGTH");
            for(uint i = 0; i < len; i++) {
                totalDropped += amounts[i];
                _airdropMint(to[i], amounts[i]);
            }
        }
        require((reservedMinted += totalDropped) <= ReservedSupply, "Mint would go over max supply");
    }
