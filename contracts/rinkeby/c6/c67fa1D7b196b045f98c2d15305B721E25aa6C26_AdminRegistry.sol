...
contract AdminRegistry is AdminBase {
    ...
    function transferSuperAdmin(address _newSuperAdmin) public returns(bool){
        ...
        onlyAddGovAdminRole(msg.sender)
    {
        ...
        require(
            ...
            "GAR: addDefaultAdmin as owner first. "  // no need as modifer will retrict from going further if no admin exist
        );
        require(
    ...
    {
        require(pendingAdminKeys[PENDING_ADD_ADMIN_KEY].length == 0 && pendingAdminKeys[PENDING_EDIT_ADMIN_KEY].length == 0 && pendingAdminKeys[PENDING_REMOVE_ADMIN_KEY].length == 0, "GAR: only one admin can be add, edit, remove at once");
         // this statement checks if msg.sender is already in pending, so that he cannot place new request until his pending request is approved or rejected
        require(isPending(msg.sender), "GAR: caller already in pending");
