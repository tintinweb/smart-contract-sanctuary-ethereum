...
contract Vendor is Ownable {
    ...
    function sellTokens(uint256 _amount) public {
        console.log(_amount);
        // 价差是否合理
        require(_amount > 0, "Amount to sell must be greater than 0");

        // 检查用户是否有足够的代币
        uint256 userBalance = yourToken.balanceOf(msg.sender);
        require(userBalance >= _amount, "Not enought tokens");

        // 检查承销商是否有足够的 ETH
        uint256 amountOfEthNeeded = _amount / tokensPerEth;
        ...
        emit SellTokens(msg.sender, _amount, amountOfEthNeeded);
    }
}
