...
import "../lib/IVoteScore.sol";
import "../lib/Adminable.sol";
import "../lib/ERC4973SoulContainer.sol";
import "../lib/712/EIP712Whitelist.sol";

contract SharkzSoulID is IVoteScore, ReentrancyGuard, Adminable, ERC4973SoulContainer, EIP712Whitelist {
    ...
    // Returns token vote score
    function voteScore(uint256 _tokenId) external view override returns (uint256) {
        if (_exists(_tokenId)) {
            return 1;
        }
        return 0;
    ...
    // Returns max vote score
    function maxVoteScore() external pure override returns (uint256) {
        return 1;
    }
    ...
    function publicMint() external nonReentrant callerIsUser() {
        if (nftContract != address(0)) {
            require(_isExternalNftOwner(nftContract, msg.sender), "Caller is not a NFT owner");
        }
        _runMint(msg.sender);
    }
        ...
        onlyAdmin 
    {
        ...
        **/
        ...
        // record the deployed contract data store
        saveImageDataRecord(
            _key,
            _pageNumber,
            dataContract,
     ...
     */
    function _isExternalNftOwner(address _nftContract, address _ownerAddress) internal view returns (bool) {
        uint256 balance = 0;

        try IERC721(_nftContract).balanceOf(_ownerAddress) returns (uint256 retBalance) {
            balance = retBalance;
        } catch (bytes memory reason) {
            if (reason.length == 0) {
                revert("Contract balanceOf function reverted without reason");
            } else {
                assembly {
...
interface IVoteScore {
    /**
     * @dev Get the value of a token voting score
     */
    function voteScore(uint256 tokenIndex) external view returns (uint256);

    /**
     * @dev Get the max value of voting score
     */
 ...
 *
 */
contract ERC4973SoulContainer is IERC4973, IERC4973SoulContainer, IERC721Metadata {
 ...
 */
interface IERC4973SoulContainer {
  /**
