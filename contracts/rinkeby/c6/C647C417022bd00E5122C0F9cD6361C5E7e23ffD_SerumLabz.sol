...
contract SerumLabz is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _initBaseURI, bytes32 _root, bytes32 _mintKey) ERC721A("SerumLabz", "SL") {
    ...
    // ===== Private mint =====
    function privateMint(bytes32[] memory proof, uint8 quantity, bytes32 suppliedKey) external payable nonReentrant whenPrivateMint {
    ...
    // ===== Public mint =====
    function mint(uint8 quantity, bytes32 suppliedKey) external payable nonReentrant whenPublicMint {
        require(msg.value >= cost * quantity, "[Value Error] Not enough funds supplied for mint");
        require(totalSupply() + quantity < maxSupply, "[Supply Error] Not enough left for this mint amount");
        require(mintKey == suppliedKey, "[Mint Security] This NFT can only be minted from the dapp");
