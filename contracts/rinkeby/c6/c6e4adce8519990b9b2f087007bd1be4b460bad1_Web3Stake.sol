...
contract Web3Stake is ERC721Holder, ReentrancyGuard, Auth {
    ...
    function unstake() external nonReentrant {
        ...
        require(user.startDate.add(locktime) < block.timestamp, "This NFT is locked for 365 Days from the start date!");
        stakedSupply = stakedSupply.sub(user.amount);
        uint256[] memory tokenIds = user.tokenIds;
        user.amount = 0;
        user.endDate = block.timestamp;
    ...
    // change status
    function setStatus() external authorized {
        isStatus = !isStatus;
    }
}