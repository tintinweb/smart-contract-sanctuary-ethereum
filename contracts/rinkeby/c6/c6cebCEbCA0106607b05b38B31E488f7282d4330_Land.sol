...
contract Land is ERC721, ReentrancyGuard, Ownable, Pausable {
    ...
    // **NOTE: MAX_SUPPLY value should be change to 1000 before deploying to mainnet
    ...
    modifier isNFTBalanceExceedsMaxMintPerAddress(address _address, uint8 _nftQty) {
        require(
            (balanceOf(_address) + _nftQty) <= maxMintPerWallet,
    ...
    function setMaxMintPerWallet(uint16 _maxMintPerWallet)
        external
        onlyOwner
    {
