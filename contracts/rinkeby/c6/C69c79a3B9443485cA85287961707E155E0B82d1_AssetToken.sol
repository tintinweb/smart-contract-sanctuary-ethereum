...
contract AssetToken is ERC20, CompoundRateKeeper {
    ...
    event FreezeStateChanged(bool _freezeState, address indexed _caller);
    event ChangedKya(string _kya, address indexed _caller);
    ...
    event ChangedToSafeGuard(uint256 _amount, uint256 _stakedPercent, address indexed _caller);
    ...
    constructor(
        ...
        require(_issuer != address(0), "Issuer 0x0 provided");
        require(_guardian != address(0), "Guardian 0x0 provided");
        require(_statePercent > 0, "Invalid MIN State Percent");
        require(_statePercent <= hundredPercent, "Invalid MAX State Percent");

        require(bytes(_kya).length > 3, "Invalid KYA");
        ...
        guardian = _guardian;
        issuer = _issuer;
        _setupRole(DEFAULT_ADMIN_ROLE, _guardian);
        _setupRole(DEFAULT_ADMIN_ROLE, _issuer);
    ...
    function _beforeTokenTransfer(
        ...
        address _to,
        uint256 _amount
    ) internal virtual override onlyUnfreezed {
        /// @dev on safeguard the only available transfers are from allowed addresses and guardian
        /// @dev or from an authorized user to this contract
        /// @dev address(this) is added as the _from for approving redemption (burn)
        /// @dev address(this) is added as the _to for requesting redemption (transfer to this contract)
        /// @dev address(0) is added to the condition to allow burn on safeguard
        if (state == ContractState.safeguard) {
            // if (_to != address(this) && _from != address(this) && _from != guardian && _to != address(0)) {
            if (_to != address(this) && _from != address(this) && _from != guardian) {
                require(hasRole(SAFEGUARD_TRANSFER_ROLE, _from), "BTT: SAFEGUARD_TRANSFER_ROLE missing");
            }
        } else {
            /// @dev State is ACTIVE
            /// @dev It is not a mint
            if (_from != address(0)) {
                require(mustBeAuthorizedHolder(_from), "BTT: From _account not authorized");
            }
        }
        ...
        if (_to != address(0)) {
            require(mustBeAuthorizedHolder(_to), "BTT: To _account not authorized");
        }
    ...
    function setKya(string memory _kya) external onlyIssuer onlyUnfreezed onlyActiveState {
        require(bytes(_kya).length > 3, "SKY: invalid KYA");
        emit ChangedKya(_kya, _msgSender());
        kya = _kya;
    }
    ...
    function setFreezeState(bool _freezeState) external onlyIssuerOrGuardian {
        if (freezeState) {
            require(!_freezeState, "SFS: contract Freezed");
        } else {
            require(_freezeState, "SFS: contract NOT Freezed");
        }

        emit FreezeStateChanged(_freezeState, _msgSender());
    ...
    /// @param _destination the receiver of the tokens
    /// @return The request ID to be referenced in the mapping
    function _requestMint(uint256 _amount, address _destination)
        private
        onlyActiveState
        onlyUnfreezed
        onlyIssuerOrAgent
        returns (uint256)
    {
        require(_amount > 0, "RQM: invalid _amount");
    ...
    function approveMint(uint256 _mintRequestID, string memory _referenceTo) public onlyIssuer onlyActiveState {
        require(mintRequests[_mintRequestID].destination != address(0), "APM: invalid MintRequestID");
        ...
        emit MintApproved(
            ...
            mintRequests[_mintRequestID].amount,
            _msgSender()
        );
        _mint(
            mintRequests[_mintRequestID].destination,
            mintRequests[_mintRequestID].amount.mul(CompoundRateKeeper.decimal).div(
                CompoundRateKeeper.compoundRate.rate
            )
        );
    ...
    function requestRedemption(uint256 _assetTokenAmount, string memory _recipient)
        ...
        returns (uint256)
    {
        require(_assetTokenAmount > 0, "RRD: invalid _assetTokenAmount");
        require(balanceOf(_msgSender()) >= _assetTokenAmount, "RRD: not enough to redeem");
    ...
    function cancelRedemptionRequest(uint256 _redemptionRequestID, string memory _motive) external onlyActiveState {
        ...
        if (_msgSender() != issuer) {
            ...
            require(redemptionRequests[_redemptionRequestID].sender == _msgSender(), "CRR: not request owner");
        }
    ...
    function approveRedemption(uint256 _redemptionRequestID, string memory _approveTxID) external onlyIssuerOrGuardian {
        ...
        if (redemptionRequests[_redemptionRequestID].fromStake) {
            require(state == ContractState.safeguard, "APR: state not Safeguard");

            /// @dev unreachable code. Modifier checks if on safeguard, caller should be guardian
            // require(_msgSender() == guardian, "APR: not Guardian");
        }
    ...
    function safeguardStake(uint256 _amount, string memory _recipient) external onlyActiveState {
        ...
        uint256 stakedPercent = totalStakes.mul(hundredPercent).div(totalSupply());

        if (stakedPercent >= statePercent) {
            state = ContractState.safeguard;
            emit ChangedToSafeGuard(_amount, stakedPercent, _msgSender());
        }
        ...
        approve(guardian, redemptionRequests[_requestID].assetTokenAmount);
    }
    ...
    function _safeguardUnstake(uint256 _amount) private onlyActiveState {
        ...
        require(safeguardStakes[_msgSender()] >= _amount, "SFU: _amount exceeds staked");
...
abstract contract CompoundRateKeeper is AccessManager {
    ...
    function setInterestRate(uint256 _interestRate, bool _pos) external onlyIssuer {
        ...
        // TODO 20 digits - if percent per second this will reach 0.7 (70%) ??
...
abstract contract AccessManager is AccessControl {
    ...
    bytes32 public constant SAFEGUARD_TRANSFER_ROLE = keccak256("SAFEGUARD_TRANSFER_ROLE");
    ...
    modifier onlyIssuer() {
        require(_msgSender() == issuer, "not ISSUER");
        _;
    }
    ...
    modifier onlyAgent() {
        require(agents[_msgSender()], "not AGENT");
        _;
    }

    /// @notice Check if sender is AGENT_or ISSUER
    modifier onlyIssuerOrAgent() {
        require(_msgSender() == issuer || agents[_msgSender()], "not AGENT nor ISSUER");
        _;
    }
    ...
    modifier onlyIssuerOrGuardian() {
        if (state == ContractState.active) {
            require(_msgSender() == issuer, "onActive: not Issuer");
        }

        if (state == ContractState.safeguard) {
            require(_msgSender() == guardian, "onSafeguard: not Guardian");
        }
        _;
    }
    ...
    modifier isNotCaller(address _account) {
        require(_account != _msgSender(), "Role present");
        _;
    }
    ...
    modifier onlyIssuerOrAgentOrAuthorizedHolder() {
        if (state == ContractState.active) {
            require(
                agents[_msgSender()] || _msgSender() == issuer || mustBeAuthorizedHolder(_msgSender()),
                "Active State: invalid caller"
            );
        }

        if (state == ContractState.safeguard) {
            require(mustBeAuthorizedHolder(_msgSender()), "onSafeguard: not authorized");
        }
        _;
    }
    ...
    function mustBeAuthorizedHolder(address _account) public view returns (bool) {
        require(_account != address(0), "MBA: address 0x0");
        require(!blacklist[_account], "MBA: blacklisted");

        /// @dev this contract is an authorized holder
        ...
        if (state == ContractState.active) {
            ...
            if (_account == issuer || agents[_account]) {
                return true;
            }
        } else {
            /// @dev on safeguard state, guardian is authorized holder
            if (_account == guardian) {
                return true;
            }
        }

        require(authorizationContracts.length > 0, "MBA: empty auth list");
    ...
    /// @param _account to be granted the DEFAULT_ADMIN_ROLE
    ...
    /// @param _account to be granted the SAFEGUARD_TRANSFER_ROLE
    function grantSafeguardTransferRole(address _account) external onlyAdmin {
        grantRole(SAFEGUARD_TRANSFER_ROLE, _account);
    }
    ...
    function transferAgentList(address _newAgent, address _oldAgent) external onlyIssuerOrAgent {
        require(authorizationContracts.length > 0, "Empty list");
    ...
    function addToAuthorizationList(address _contractAddress) external onlyAgent {
        ...
        require(authorizationsPerAgent[_contractAddress] == address(0), "_contractAddress belongs to agent");
    ...
    function removeFromAuthorizationList(address _contractAddress) external onlyAgent {
        require(_isContract(_contractAddress), "_contractAddress is not Contract");
        require(authorizationsPerAgent[_contractAddress] != address(0), "_contractAddress not found");
        require(authorizationsPerAgent[_contractAddress] == _msgSender(), "_contract not managed by caller");
