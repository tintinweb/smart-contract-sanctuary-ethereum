    ...
    INFTAuctionsFeature
{
    ...
    //         "uint256 endOrReserveErc20TokenAmount,",
    //         "AuctionFee[] fees,",
    //         "address nftToken,",
    //         "uint256 nftTokenId,",
    //         "uint128 nftTokenAmount",
    //     ")",
    //     "AuctionFee(bool isMatcher,address recipient,uint256 amount,bytes feeData)"
    // ));
    uint256 constant public _AUCTION_TYPEHASH = 0x2d1c3a8bfe67eefbdb31b2ba841938189f2937c047d4bf9f5d2a9f9f42aeff72;
    ...
    //     ")"
    // ));
    uint256 constant public _FEE_TYPEHASH = 0x952db27ecd9808fcc69fdc3be21fdfc19848abba64c47d239631089125659aaf;
    ...
    //     ")",
    //     "AuctionFee(bool isMatcher,address recipient,uint256 amount,bytes feeData)",
    //     "NFTAuction(uint8 auctionKind,uint8 nftKind,address maker,uint128 startTime,uint128 endTime,uint256 nonce,address erc20Token,uint256 startErc20TokenAmount,uint256 endOrReserveErc20TokenAmount,AuctionFee[] fees,address nftToken,uint256 nftTokenId,uint128 nftTokenAmount)"
    // ));
    uint256 constant public _EN_AUCTION_BID_TYPEHASH = 0x1e199c84ba39aededb4f6dfdcedd58a2c3a08cc6e2df33ebbe54af3aa1b5fca5;
    ...
    function _bidDutchAuction(
        ...
        erc20FillAmount = LibMathV06.getPartialAmountCeil(
            ...
            auction.startErc20TokenAmount.safeSub(auction.endOrReserveErc20TokenAmount)
        ).safeAdd(auction.endOrReserveErc20TokenAmount);
    ...
    function _payFees(
        ...
        for (uint256 i = 0; i < auction.fees.length; ++i) {
            ...
            if (fillAmount == auction.nftTokenAmount) {
    ...
    function batchCancelAuctions(uint256[] calldata auctionNonces) external override {
        for (uint256 i = 0; i < auctionNonces.length; ++i) {
    ...
    function getNFTAuctionInfo(NFTAuction memory auction) public override view returns (NFTAuctionInfo memory auctionInfo) {
        (auctionInfo, ) = _getNFTAuctionInfo(auction);
    }

    function _getNFTAuctionInfo(NFTAuction memory auction) private view returns (NFTAuctionInfo memory auctionInfo, bytes32 auctionStructHash) {
        auctionStructHash = getNFTAuctionStructHash(auction);
        auctionInfo.auctionHash = _getEIP712Hash(auctionStructHash);

        if (auction.endTime <= auction.startTime) {
            auctionInfo.status = AuctionStatus.INVALID;
            return (auctionInfo, auctionStructHash);
            ...
            return (auctionInfo, auctionStructHash);
        } else if (auction.nftTokenAmount == 0) {
            auctionInfo.status = AuctionStatus.INVALID;
            return (auctionInfo, auctionStructHash);
        } else if (auction.nftKind == NFTKind.ERC721 && auction.nftTokenAmount != 1) {
            auctionInfo.status = AuctionStatus.INVALID;
            return (auctionInfo, auctionStructHash);
        }  else if (
            auction.auctionKind == NFTAuctionKind.DUTCH &&
            auction.endOrReserveErc20TokenAmount >= auction.startErc20TokenAmount) {
            auctionInfo.status = AuctionStatus.INVALID;
            return (auctionInfo, auctionStructHash);
        }
        ...
        if (auctionCancellationBitVector & flag != 0 || auctionInfo.remainingAmount == 0) {
    ...
    function _validateAuctionSignature(
        ...
        if (signature.signatureType == LibSignature.SignatureType.PRESIGNED) {
            ...
            bool isPreSigned = LibNFTAuctionsStorage.getStorage()
                .auctionState[auctionHash].preSigned;
            if (!isPreSigned) {
    ...
    function getNFTAuctionHash(NFTAuction memory auction) public override view returns (bytes32 auctionHash) {
        return _getEIP712Hash(getNFTAuctionStructHash(auction));
    }

    function getNFTAuctionStructHash(NFTAuction memory auction) internal pure returns (bytes32 structHash) {
        ...
        //     uint128 endTime;4
        //     uint256 nonce;5
        //     address erc20Token;6
        //     uint256 startErc20TokenAmount;7
        //     uint256 endOrReserveErc20TokenAmount;8
        //     AuctionFee[] fees;9
        //     address nftToken;10
        //     uint256 nftTokenId;11
        //     uint128 nftTokenAmount;12
        // ));
        assembly {
            if lt(auction, 32) { invalid() } // Don't underflow memory.

            let typeHashPos := sub(auction, 32) // auction - 32
            let feesHashPos := add(auction, 288) // auction + (32 * 9)
            ...
            structHash := keccak256(typeHashPos, 448 /* 32 * 14 */ )
    ...
    function getEnAuctionBidHash(EnglishAuctionBid memory enBid) public override view returns (bytes32 bidHash) {
        return _getEIP712Hash(getEnAuctionBidStructHash(enBid));
    }

    function getEnAuctionBidStructHash(EnglishAuctionBid memory enBid) internal pure returns (bytes32 structHash) {
        bytes32 auctionHash = getNFTAuctionStructHash(enBid.auction);
        return getEnAuctionBidStructHash(enBid, auctionHash);
    }

    function getEnAuctionBidStructHash(EnglishAuctionBid memory enBid, bytes32 auctionHash) internal pure returns (bytes32 structHash) {
        // Hash in place, equivalent to:
        // return keccak256(abi.encode(
    ...
    function preSignNFTAuction(NFTAuction memory auction) public override {
        {
            require(
                auction.maker == msg.sender,
                "NFTAuctionsFeature::preSignNFTAuction/ONLY_MAKER"
            );
            bytes32 _hash = getNFTAuctionHash(auction);
            LibNFTAuctionsStorage.getStorage().auctionState[_hash].preSigned = true;
        }
        
        emit NFTAuctionPreSigned(
            auction.auctionKind,
            auction.nftKind,
            auction.maker,
            auction.startTime,
            ...
            auction.endOrReserveErc20TokenAmount,
            auction.fees,
            auction.nftToken,
            auction.nftTokenId,
            auction.nftTokenAmount
    ...
    function canTake(address taker, EnglishAuctionBid memory enBid) private pure returns(bool) {
        if (taker == enBid.auction.maker) {
            return true;
        }
        for (uint256 i = 0; i < enBid.auction.fees.length; ++i) {
            if (
                enBid.auction.fees[i].isMatcher && 
                enBid.auction.fees[i].recipient == taker &&
                enBid.erc20TokenAmount >= enBid.auction.endOrReserveErc20TokenAmount
            ) {
                return true;
            }
        }
    ...
    function acceptBid(
        ...
        require(canTake(msg.sender, enBid), "NFTAuctionsFeature::acceptBid/NO_PERMISSION_TO_TAKE");
        require(enBid.erc20TokenAmount >= enBid.auction.startErc20TokenAmount, "NFTAuctionsFeature::acceptBid/PRICE_TOO_LOW");
        (NFTAuctionInfo memory auctionInfo, bytes32 auctionStructHash) = _getNFTAuctionInfo(enBid.auction);
        // Check that the auction can be filled.
        _validateNFTAuction(enBid.auction, auctionSig, auctionInfo, enBid.auction.nftTokenAmount);

        require(
            address(enBid.auction.erc20Token) != NATIVE_TOKEN_ADDRESS,
            "NFTAuctionsFeature::acceptBid/NATIVE_TOKEN_NOT_ALLOWED"
        );

        _validateAuctionSignature(_getEIP712Hash(getEnAuctionBidStructHash(enBid, auctionStructHash)), bidSig, enBid.bidMaker);
        _updateAuctionState(
            enBid.auction.maker,
            enBid.auction.nonce,
            auctionInfo.auctionHash,
            auctionInfo.remainingAmount,
        ...
        if (enBid.auction.nftKind == NFTKind.ERC721) {
            _transferERC721AssetFrom(IERC721Token(enBid.auction.nftToken), enBid.auction.maker, enBid.bidMaker, enBid.auction.nftTokenId);
        } else {
            _transferERC1155AssetFrom(IERC1155Token(enBid.auction.nftToken), enBid.auction.maker, enBid.bidMaker, enBid.auction.nftTokenId, enBid.auction.nftTokenAmount);
        }

        {
            uint256 len = enBid.auction.fees.length;
            for (uint256 i = 0; i < len; ++i) {
                if (enBid.auction.fees[i].isMatcher && enBid.auction.fees[i].recipient != msg.sender) {
                    enBid.auction.fees[i].amount = 0;
                }
            }
        }
...
interface INFTAuctionsFeature {
    ...
    /// @dev Emitted when an auction is pre-signed.
    ///      Contains all the fields of the auction.
    event NFTAuctionPreSigned(
    ...
    /// @param auction The auction.
    /// @return auctionInfo Info about the auction.
    function getNFTAuctionInfo(NFTAuction memory auction) 
