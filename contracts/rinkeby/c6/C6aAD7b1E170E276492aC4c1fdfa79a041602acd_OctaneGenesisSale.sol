...
// File contracts/OctaneGenesisSale.sol
...
contract OctaneGenesisSale is Ownable, Pausable, ReentrancyGuard {
    ...
    function buy(uint256 NFTType, bytes32[] memory proof)
        external
        payable
        nonReentrant
        whenNotPaused
        onlyWhileOpen
    {
        _preValidatePurchase(NFTType, proof);

        paymentSplitter.sendValue(msg.value);

        sold[NFTType] += 1;
        alreadyMinted[msg.sender] += 1;
        octaneGenesis.safeTransferFrom(
            tokenWallet,
            msg.sender,
            NFTType,
            1,
    ...
    function _preValidatePurchase(uint256 NFTType, bytes32[] memory proof)
        internal
    {
        if (applyMerkle()) {
            require(isWhitelisted(proof), "Not whitelisted For presale");
        }
        ...
        if ((NFTType == 0 || NFTType == 1) && sold[NFTType] == 2) {
