...
contract PNFTTest4 is ERC721A , Ownable{  
    constructor() ERC721A("PNFTTest4", "PNFT4") {}  
	...
	uint256 private TOKEN_FACTOR = 10000000000000000000000;		//10000	// should be 100m Pochi Tokens base rate per growth age
	...
	uint256 private BREEDING_DELAY = 604800;	// 1 week phase
	...
	// Remove Feed from Pochi puppy (Un-Staking Pochi Inu Tokens)
	...
	// Remove All Feed from Pochi puppy (Un-Staking Pochi Inu Tokens)
    ...
    function getBurnAmount(uint256 tokenId) public view returns(uint256){
        ...
        if (_feeds[tokenId].amt > 0){
			...
			if (curAge == MAX_AGE) {
				return (_feeds[tokenId].amt / BURN_FACTOR_POST);
			}
		}
	...
	function timeSinceAdult(uint256 tokenId) public view returns(uint256){
		require(_exists(tokenId), "NFT does not exist");
		require(_burnToAdults[tokenId] == false, "NFT burned to Adult!");
	...
	//    pass 0 to get all eligible, or comma-delimited to check specific
	function getEligibleForBreeding(uint256[] memory nftIds) public view returns (uint256[] memory) {
		uint256[] memory nftIdsToReturn;
		uint256 nftCount = 0;
		
		bool loopAll = (nftIds.length == 1 && nftIds[0] == 0)? true : false;
		uint256 loopCount = loopAll? _publicPNFT.current() : nftIds.length;
		
		for (uint256 i = 0; i < loopCount; i++) {
			uint256 nftToCheck = loopAll? i : nftIds[i];
			if (_burnToAdults[nftToCheck] == true || ((calcAge(nftToCheck) == MAX_AGE) && (timeSinceAdult(nftToCheck) > BREEDING_DELAY))) {
				nftIdsToReturn[nftCount] = nftToCheck;
				nftCount++;
			}
		}
		
		return nftIdsToReturn;
	}
