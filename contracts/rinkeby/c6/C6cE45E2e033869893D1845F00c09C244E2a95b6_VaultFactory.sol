...
import {IVault, Vault} from './Vault/Vault.sol';
...
contract VaultFactory is AccessControl {
  ...
  EnumerableSet.AddressSet private _yieldTokens;
  mapping(address => EnumerableSet.AddressSet) private _yieldTokenVaults;
  ...
  function createVault(IVault.VaultInitializeParameters calldata _initParams) public onlyRole(DEPLOYER) returns (IVault _vault) {
    _vault = IVault(new Vault{salt: keccak256(abi.encode(_initParams._yieldToken, _yieldTokenVaults[_initParams._yieldToken].length()))}());
    _vault.initialize(_initParams);

    _yieldTokens.add(_initParams._yieldToken);
    _yieldTokenVaults[_initParams._yieldToken].add(address(_vault));
  }

  function yieldTokenVaults(address _yieldToken) external view returns (address[] memory) {
    return _yieldTokenVaults[_yieldToken].values();
  }

  function yieldTokens() external view returns (address[] memory) {
    return _yieldTokens.values();
  }
}
