 ...
 */
contract EthereumClockToken is IEthereumClockToken, Raffle
{
     ...
     */
    constructor(string memory _pendingURI, address _chainLinkVRFCoordinator, address _linkTokenAddress) ERC721("Ethereum Clock Token", "Eth-Clock") Raffle(_chainLinkVRFCoordinator, _linkTokenAddress) {
        _CHAIN_LINK_VRF_COORDINATOR_ = _chainLinkVRFCoordinator;
        _LINK_TOKEN_ADDRESS_ = _linkTokenAddress;
    ...
    function init(
        ...
        _CHAIN_LINK_KEY_HASH_ = _chainLinkKeyHash;
        _CHAIN_LINK_FEE_ = _chainLinkFee;
 ...
 */
abstract contract Raffle is Admin, VRFConsumerBase {
    ...
    event UpdateRaffle(address indexed ownerAddress);

    // ============ Functions ============
    // Constructor inherits VRFConsumerBase
    constructor(address _chainLinkVRFCoordinator, address _linkTokenAddress) VRFConsumerBase(_chainLinkVRFCoordinator, _linkTokenAddress) {
    ...
    /**
     * @notice Register for Drop function - this should be called for the unregistered wallet
     */
    function register() external {
        require(!userList[msg.sender], "ALREADY REGISTERED");
    ...
    function registerWhiteList() external {
        ...
        require(_PRESALE_ALLOWED_, "PRESALE ALREADY FINISHED");
     ...
     * @notice Register for Drop function - this should be called for the unregistered wallet
     */
    function raffle() external onlyOwner onlyRaffle returns (bytes32) {
        bool chainLinkNeedFlag = true;
        if ((_PRESALE_ALLOWED_ && _PRESALE_COUNT_ >= _userList.length) || (!_PRESALE_ALLOWED_ && _WHITE_LIST_COUNT_ >= _userList.length)) {
            chainLinkNeedFlag = false;
        }

        bytes32 requestId = 0;
        if(chainLinkNeedFlag) {
            require(LINK.balanceOf(address(this)) >= _CHAIN_LINK_FEE_, "NOT ENOUGH LINK - FILL CONTRACT WITH FAUCET");
            requestId = requestRandomness(_CHAIN_LINK_KEY_HASH_, _CHAIN_LINK_FEE_);
            requestIdToAddress[requestId] = msg.sender;
        } else {
            requestIdToAddress[requestId] = msg.sender;
            fulfillRandomness(requestId, 1);
        }
        emit UpdateRaffle(msg.sender);
        return requestId;
    }

    /**
     * @notice Get randomness value function
     */
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override
    {
        _RANDOM_VALUE_ = randomness;
        address to = requestIdToAddress[requestId];
        require(to != address(0), "REQUEST_ID DOES NOT EXIST");
        if (to == owner()) {
            if (_PRESALE_ALLOWED_) {
                distributeWhitelist(_PRESALE_COUNT_);
            } else {
                distributeWhitelist(_WHITE_LIST_COUNT_);
            }
        }
    ...
    /**
     * @notice distribute to whitelist from userList
     */
    function distributeWhitelist(uint256 walletCount) internal onlyRandom {
        require(_userListLength() > 0, "INSUFFICIENT USER LIST");
        for (uint256 i = 0; i < (_userListLength() < walletCount ? _userListLength() : walletCount); i++) {
            uint256 index = _userListLength() - 1;
            if (_userListLength() > walletCount) {
                index = (uint256(keccak256(abi.encode(_RANDOM_VALUE_, i))) % _userListLength());
            }
            address selected = _userList[index];
            whiteList[selected] = true;
            _WHITELIST_LENGTH_ ++;
            _userList[index] = _userList[_userListLength() - 1];
            _userList.pop();
    ...
    function _userListLength() public view returns (uint256) {
 ...
 */
abstract contract Admin is Storage, IAdmin, ERC721URIStorage {
...
contract Storage is Ownable, ReentrancyGuard {
    ...
    address public _CHAIN_LINK_VRF_COORDINATOR_;
    address public _LINK_TOKEN_ADDRESS_;
    uint256 public _CHAIN_LINK_FEE_;
    bytes32 public _CHAIN_LINK_KEY_HASH_;
