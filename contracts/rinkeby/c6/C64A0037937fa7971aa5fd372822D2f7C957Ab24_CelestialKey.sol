...
contract CelestialKey is Ownable, Pausable, ERC721A("Celestial Key", "CKEY") {
  ...
  uint256 public constant CKEY_PER_TX = 10;
  ...
  function claimEarly(uint256 amount_, bytes32[] memory proof_) external payable {
    if (msg.sender != owner()) {
      require(saleState == 1, "Invalid sale state");
      require(amount_ > 0 && amount_ + earlyClaimed[msg.sender] <= CKEY_PER_TX, "Invalid claim amount");
      require(msg.value == CKEY_EARLY_PRICE * amount_, "Invalid ether amount");
      bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
      require(proof_.verify(earlyAccessRoot, leaf), "Invalid proof");
    }

    earlyClaimed[msg.sender] += amount_;
    _safeMint(msg.sender, amount_);
  }

  /// @notice Claim one or more tokens.
  function claim(uint256 amount_) external payable {
    require(totalSupply() + amount_ <= CKEY_MAX, "Max supply exceeded");
    if (msg.sender != owner()) {
      require(saleState == 2, "Invalid sale state");
      require(amount_ > 0 && amount_ <= CKEY_PER_TX, "Invalid claim amount");
      require(msg.value == CKEY_PRICE * amount_, "Invalid ether amount");
    }
  ...
  /// @notice Set freeClaimRoot to `merkleRoot_`.
  function setFreeClaimRoot(bytes32 merkleRoot_) external onlyOwner {
    freeClaimRoot = merkleRoot_;
  }

  /// @notice Set earlyAccessRoot to `merkleRoot_`.
  function setEarlyAccessRoot(bytes32 merkleRoot_) external onlyOwner {
    earlyAccessRoot = merkleRoot_;
  }
