...
contract PotLottery is Initializable, ReentrancyGuardUpgradeable {
    ...
    */
    ...
    /**   @dev set the PLSP address
        * @param _address the PLSP address
        * @notice only the admin or owner can call this function
    */
    function setPLSPAddress(address _address) public onlyAdmin {
        PLSP_Address = _address;
    }

    /**   @dev set the PLSP minimum balance to get 50% reduction in fee
        * @param _amount the PLSP minimum balance for 50% reduction in fee
        * @notice only the admin or owner can call this function
    */
    ...
    */
    ...
    /**   @dev set token usd value
        ...
        * @notice set PLSP price to 30usd when price is below 30usd on dex
        * @notice add extra 10% to price of PLSP when price above 30usd on dex
    */
    ...
    */
    ...
    function EnterPot(
        ...
        for (uint256 index = 0; index < _tokenNames.length; index++) {
            if((keccak256(bytes(_tokenNames[index])) == keccak256(bytes('PULSE')))){
                continue ;
            }
    ...
    */
    ...
    /**   @dev remove the amount to pay as fee
        ...
        * @notice _address current PLSP holding determine how much fee reduction you get
    */
    ...
    */
    ...
    function swapAccumulatedFees() external validPLSP nonReentrant {
        ...
        if (hotWalletBalance <= hotWalletMinBalance) {
            ...
            if (hotWalletFee > (pulseFee * 8) / 10) {
                hotWalletFee = (pulseFee * 8) / 10;
            }
        }
        ...
        uint256[] memory amounts = router.swapExactETHForTokens{ value: pulseFee - hotWalletFee }(
            amountOutMinForPLSP,
            path,
            address(this),
            block.timestamp
        );
        adminFeeTokenValues['PULSE'] -= (amounts[0] + hotWalletFee);
    ...
    receive() external payable {
        ...
        _EnterPot('PULSE', msg.value, msg.sender);
    }
