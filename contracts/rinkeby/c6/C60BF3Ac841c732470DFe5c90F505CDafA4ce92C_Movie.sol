...
contract Movie is ERC721Optimized, Ownable, ReentrancyGuard {
   ...
   mapping(address => uint256) minteds;
    ...
    function buy(address seller,uint256 tokenId) public nonReentrant {
        ...
        //检查tokenId是否归属于卖方
        require(_isApprovedOrOwner(seller, tokenId), "transfer caller is not owner nor approved");

        //检查是否授权给合约
        require(getApproved(tokenId)==address(this),"tokenId is not contract approved");
       
        //检查当前余额要大于购买的金额
        uint256 amount=tokenIdsPrice[tokenId];
        //emit Log(msg.sender.balance);
       // emit Log(amount);
        require(msg.sender.balance > amount, "buy address insufficient balance");
