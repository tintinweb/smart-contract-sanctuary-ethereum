...
contract Tygarare is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
  ...
  uint256 public MAX_PER_Transaction_public = 2; // maximam amount that user can mint
  uint256 public MAX_PER_Address_public = 10; // maximam amount that user can mint

  uint256 public  PRICE = 0.077 ether;

  uint256 private constant TotalCollectionSize_ = 7777; // total number of nfts
  uint256 private constant MaxMintPerBatch_ = 12; //max mint per traction
  ...
  bytes32 public merkleRoot = 0x04848aa88443a1927366580040f94888367a0228bfb2db7765e361ee733d6164;
  uint public whitelistedSpots = 1300;
  uint public reserved = 30;

  constructor() ERC721A("Tygarare NFT","TGR", MaxMintPerBatch_, TotalCollectionSize_) {
    _uriBeforeReveal = "https://bafkreiauwau2fip35sq4bu7secyyd4vmqmdnxttshccliz6y7ugvvtkcia.ipfs.nftstorage.link/";
  }
  ...
  function getwhitelistedSpots() public view returns(uint){
    return whitelistedSpots;
  }
  function setwhitelistedSpots(uint w) public onlyOwner{
    whitelistedSpots = w;
  }
 function mint(uint256 quantity) external payable nonReentrant {
    ...
    require( ( numberMinted(msg.sender) + quantity <= MAX_PER_Address_public ) , "Quantity exceeds allowed Mints" );
    require(  quantity <= MAX_PER_Transaction_public,"can not mint this many");
  ...
  function whitelistMint(uint256 quantity, bytes32[] calldata merkleproof) external payable nonReentrant {
    require(totalSupply() + quantity <= whitelistedSpots, "Whitelist sold out");
  ...
  function changeMAX_PER_Transaction_public(uint256 q) external onlyOwner
  {
      MAX_PER_Transaction_public = q;
  }
  function changeMAX_PER_Address_public(uint256 q) external onlyOwner
  {
      MAX_PER_Address_public = q;
  }
