    ...
    function lock(uint256 _amount, uint256[] memory nftIds) public nonReentrant {
		...
		for (uint i = 0; i < nftIds.length; i++) {
			...
			for (uint j = 0; j < userLocks[msg.sender].nftIds.length; j++) {
				...
				if (userLocks[msg.sender].nftIds[j] == 0 && nftIds[i] != 0) {
					zero = j;
					fFoundZero = true;
				}
			}
			if (fFoundZero == true && fFound == false) {
				userLocks[msg.sender].nftIds[zero] = nftIds[i];
			}
		...
		if (userLocks[msg.sender].nftIds.length > 0) {
			uint256 lengthToZero = userLocks[msg.sender].nftIds.length;
			for (uint256 j = userLocks[msg.sender].nftIds.length; j > 0; j--) {
				if (userLocks[msg.sender].nftIds[j-1] == 0) {
					lengthToZero = j-1;
				}
			}
		
			if (lengthToZero > 0) {
				uint256 tokensPerNFT = userLocks[msg.sender].tokenAmount / lengthToZero;
				uint256 tokensLeft = userLocks[msg.sender].tokenAmount;
				uint256 i = 0;
				
				for (i = 0; i < lengthToZero - 1; i++) {
		...
		if (userLocks[msg.sender].nftIds.length > 0) {
			...
			if (userLocks[msg.sender].tokenAmount == 0) {
				for (uint256 i = userLocks[msg.sender].nftIds.length; i > 0; i--) {
					if (userLocks[msg.sender].nftIds[i-1] > 0) {
						pochiContract.takeFeedAll(userLocks[msg.sender].nftIds[i-1]);
						delete userLocks[msg.sender].nftIds[i-1];
					}
				}	
			}
			else
			{
				...
				uint256 tokensPerNFT = _amount / lengthToZero;
		...
		if (userLocks[msg.sender].nftIds.length > 0) {
			for (uint256 i = 0; i < userLocks[msg.sender].nftIds.length - 1; i++) {
				if (_nftId == userLocks[msg.sender].nftIds[i]) {
					...
					delete userLocks[msg.sender].nftIds[userLocks[msg.sender].nftIds.length - 1];
				}
			}
		}
