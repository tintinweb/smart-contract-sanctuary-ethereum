...
contract NFTMediavalSTRVGame is BurningContract, Ownable, VRFConsumerBase {
    ...
    function publicMint(uint256 id, uint256 amount) public payable { 
        require(0 <= id && id <= totalGearSupplies.length, "Gear does not exists!");
        require(mintedPublicly <= maxAvailableForPublicMint, "Not enough supply left in public mint");
        require(getSum(mintedTotal) + amount <= getSum(totalGearSupplies), "Not enough tokens - total supply reached.");
    ...
    external returns (bool) {
        ...
        require(_maxAvailableForPublicMint >= mintedPublicly, "New value must be higher than already minted tokens");
    ...
    function setTotalGearSupplies(uint256[] memory _newTotalGearSupplies) external returns (bool) {
      require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "BurningContract: must have admint role to change variables");
      require(getSum(_newTotalGearSupplies) >= getSum(mintedTotal), "New supply cannot be lower than already existing.");
