...
contract DemonsBlood is Ownable, ReentrancyGuard, ERC1155Supply {
    ...
    function checkClaimable(uint256[] memory tokenIds) public view returns (uint256[] memory) {
        uint256[] memory claimable = new uint256[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            if (claimedTokens[tokenId] == false) {
                claimable[i] = tokenId;
            }
        }
        return claimable;
    ...
    function burnForAddress(uint256 typeId, address burnTokenAddress) external {
