...
contract Mario is ERC721Enumerable, Ownable {
    ...
    // uint256 public constant marioPrice = 1000000000000000; //0.08 ETH // 0.15 ETH 150000000000000000

    uint256 public constant marioPrice = 0.0001 ether;
    ...
    mapping (address => uint256) public whitelistPostPay;
    ...
    function reserveTeam() public onlyOwner {
        uint256 supply = totalSupply();
        // require(supply.add(amount) <= MAX_SUPPLY, "Not enough to reserve");
        uint256 amount = 30;
    ...
    function addWhitelistPostPay(Whitelist[] memory accounts) external onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            whitelistPostPay[accounts[i].account] = accounts[i].amount;
        }
    }
    ...
    function whitelistPostPayAmount(address account) external view returns(uint256) {
        return whitelistPostPay[account];
    }
    ...
    */
    function mint(uint numberOfTokens) public payable {
        ...
        require(numberOfTokens <= MAX_PER_USER, "Can only mint 3 tokens at a time");
        ...
        } else if(whitelistPostPay[msg.sender] > 0) {
            require(marioPrice.mul(numberOfTokens) == msg.value, "Ether value sent is not correct");
            require(whitelistPostPay[msg.sender] - purchases[msg.sender] >= numberOfTokens, "no tokens left for you here");
        }
        require(MAX_PER_USER >= purchases[msg.sender] + numberOfTokens, "too many mints per user");
