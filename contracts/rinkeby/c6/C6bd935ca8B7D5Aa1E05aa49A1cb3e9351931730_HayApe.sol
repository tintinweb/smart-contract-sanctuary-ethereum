...
contract HayApe is ERC721, Ownable {
    ...
    uint256 public constant TOKENS_PER_RANGE = 20;

    uint256 public constant START_PRICE = 0.01 ether;
    uint256 public constant PRICE_INCREMENT_PER_RANGE = 0.01 ether; 
    ...
    function getStats() public view returns(uint, uint, uint, uint, uint, uint, uint, uint, bool) {
        uint currentPrice = getPrice(1);
        uint currentRange = getCurrentRange();
        uint maxRanges = totalRanges();
        bool mintingAllowed = mintingOpen;
    ...
    function getPrice(uint256 count) public view returns (uint256) {
        ...
        uint256 currRangeMax = TOKENS_PER_RANGE.add(currRange.mul(TOKENS_PER_RANGE)) - 1;
    ...
    function mintTo(address recipient, uint256 count) public payable
    {
        ...
        require(count <= MAX_MINT_PER_TX, "Cannot mint more than max mint per transaction");
        ...
        require(msg.value == price, "Transaction value did not equal to the total mint price");
