...
*/                                                                   
  ...
  modifier mintCompliance(address _sender) {
    ...
    require(!claimed[keccak256(abi.encodePacked(currentPage, _sender))], "Already minted this page!");
    _;
  }
  ...
  /**
   * Admin - Mint a comic book page to a specified address
   */
  ...
  function pageShowAllTokens(uint _pageId) public view returns (uint256[] memory){
    uint256[] memory pageIds = new uint256[](pages[_pageId].endIndex - pages[_pageId].startIndex);
    for(uint tokenId=pages[_pageId].startIndex; tokenId<=pages[_pageId].endIndex; tokenId++){
