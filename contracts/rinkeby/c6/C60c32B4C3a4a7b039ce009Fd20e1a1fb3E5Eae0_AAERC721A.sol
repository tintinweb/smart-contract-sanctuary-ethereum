...
contract AAERC721A is Ownable, ERC721A, ReentrancyGuard {
    ...
    function publicMint(uint256 quantity) external payable callerIsUser {
        ...
        require(
            totalSupply() - amountForDevs - amountForFree + quantity <= amountForSale || (wlMintStartTime != 0 && block.timestamp - wlMintStartTime >= 5*60),
    ...
    function canPublicMintStart()
        ...
        returns (bool)
    {
        return (freeMintStartTime != 0 && price != 0 && (remainingFreeApes() == 0 || block.timestamp - freeMintStartTime >= 5*60));
    }     
    ...
    function canWLMintStart()
        ...
        returns (bool)
    {
        return publicMintStartTime != 0 && _wlRoot != ""  && price != 0 && (totalSupply() >= amountForDevs + amountForFree + amountForSale || block.timestamp - publicMintStartTime >= 5*60); // ensure that the whitelist mint starts
    }                                                                             
