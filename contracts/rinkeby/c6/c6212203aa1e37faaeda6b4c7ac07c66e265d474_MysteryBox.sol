...
/// @title Mintable ERC721 MysteryBox to claim your Metakicks.
contract MysteryBox is ERC721, Ownable, ReentrancyGuard {
    ...
    /// @dev Emitted when a MysteryBox is migrated to a Metakicks
    ...
    /// @dev Emitted when the Portal Key contract is updated
    /// @param portailKeyContract The new Portal Key contract address
    event PortalKeyContractUpdated(ERC721 portailKeyContract);
    ...
    /// @notice Portal Key contract
    ERC721 public portalKeyContract;

    /// @notice The mint price for each box
    uint256 public mintPrice = 0.15 ether;

    /// @notice Max amount to mint per address (public sale)
    uint256 public maxPerTx = 3;

    /// @notice Address of the "OpenedBox" contract to migrate tokens
    address public metakicksContract;

    /// @notice baseURI for the boxes
    ...
    /// @notice All token ids of used Portal Key (allowed to mint during whitelist)
    mapping(uint256 => bool) public portalKeyMinted;
    ...
    constructor(
        ...
        address _treasury,
        address _portalKeyContract
    ) ERC721("Metakicks: The Box", "MTKSBOX") {
        require(_treasury != address(0), "Invalid address");
        require(_portalKeyContract != address(0), "Invalid address");

        baseURI = _baseURI;
        treasury = _treasury;
        portalKeyContract = ERC721(_portalKeyContract);
    }
    ...
    /// @notice Public sale, 1 Token/Amount per users
    /// @param amount The amount to mint (ERC1155 amount)
    /// @param to The address receiving the MysteryBox
    ...
    function whitelistSale() external payable nonReentrant {
        _whitelistSale(msg.value, msg.sender);
    }

    /// @notice Whitelist sale for whitelisted users
    /// @param to The address receiving the MysteryBox
    /// Note: Will mint the amount specified during the whitelist (by the owner)
    function whitelistSale(address to) external payable nonReentrant {
        _whitelistSale(msg.value, to);
    }

    /// @notice One free MysteryBox for one Portal Key
    /// @param keyId The Portal Key id
    function portalKeySale(uint256 keyId) external nonReentrant {
        _portalKeySale(keyId, msg.sender);
    }

    /// @notice One free MysteryBox for one Portal Key
    /// @param keyId The Portal Key id
    /// @param to The address receiving the Portal Key
    function portalKeySale(uint256 keyId, address to) external nonReentrant {
        _portalKeySale(keyId, to);
    }
    ...
    /// @notice Update the Portal Key Contract address
    /// @param _portalKeyContract The new Portal Key contract address
    function setPortalKeyContract(ERC721 _portalKeyContract)
        external
        onlyOwner
    {
        require(address(_portalKeyContract) != address(0), "invalid address");
        portalKeyContract = _portalKeyContract;
        emit PortalKeyContractUpdated(_portalKeyContract);
    }
    ...
    function setMetakicksContract(address _metakicksContract)
        ...
        onlyOwner
    {
        require(_metakicksContract != address(0), "invalid address");
    ...
    function _whitelistSale(uint256 msgValue, address to) private {
        require(
            currentSaleStatus == SaleStatus.Whitelist,
            "MINT: not in whitelist sale"
        );
        uint256 amount = whitelist[to];
        require(amount != 0, "MINT: Not allowed");
        require(msgValue >= mintPrice * amount, "MINT: Not enough eth");
        require(
            currentTokenId + amount <= MAX_AMOUNT,
    ...
    function _portalKeySale(uint256 keyId, address to) private {
        require(
            currentSaleStatus == SaleStatus.Whitelist,
            "MINT: not in whitelist sale"
        );
        require(!portalKeyMinted[keyId], "MINT: key already used");
        require(
            portalKeyContract.ownerOf(keyId) == msg.sender,
            "MINT: not key owner"
        );
        require(currentTokenId + 1 <= MAX_AMOUNT, "MINT: Max amount reached");

        portalKeyMinted[keyId] = true;
