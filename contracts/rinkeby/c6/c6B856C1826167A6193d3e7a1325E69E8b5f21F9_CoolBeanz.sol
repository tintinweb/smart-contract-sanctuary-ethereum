...
contract CoolBeanz is ERC721Enumerable, Ownable {
    ...
    uint public startingPrice = 1 ether;
    uint public pricedroprate = 0.1 ether;
    uint public minimalPrice = 0.2 ether;
    ...
    function getPrice() public view returns(uint){
        ...
        uint timepassedrate = timepassed / (auctionDetails.duration / 4) * 3600 seconds;
        uint pricerate = timepassedrate * pricedroprate;

        uint auctionDuration = auctionDetails.startTime + (auctionDetails.duration * 1 hours);

        if(auctionDuration > block.timestamp) {
            mintprice = startingPrice - pricerate;
        } else if(pricerate >= startingPrice) {
            mintprice = minimalPrice;
        }else {
            mintprice = minimalPrice;
        }
    ...
    function startAuction(uint _duration) public onlyOwner {
        ...
        auctionDetails.duration = _duration;
    ...
    function getBeanzOwner(address _to) public view returns (uint[] memory){
        ...
        uint array = 0;
        for(uint idBeanz = 0; idBeanz < 5555; idBeanz++){
