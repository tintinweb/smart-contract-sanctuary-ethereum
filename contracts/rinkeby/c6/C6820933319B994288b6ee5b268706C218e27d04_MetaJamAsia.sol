...
contract MetaJamAsia is ERC721A, Ownable {
    ...
    constructor() ERC721A("MetaJam Asia - MetaPass 2022", "MJA") {}
    ...
    function reserveMint(uint256 nMints, uint256 batchSize, address user) external onlyOwner {
        require(totalSupply() + nMints <= MAX_SUPPLY, "Mint exceeds total supply");
        require(nMints % batchSize == 0, "Can only mint a multiple of batchSize");

        for (uint256 i = 0; i < nMints / batchSize; i++) {
            _safeMint(user, batchSize);
        }
    }
