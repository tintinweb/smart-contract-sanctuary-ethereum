...
/**
 * @title InvestementAlpha
 * @dev Implementation of InvestementAlpha
 */

contract InvestementAlpha is Investement {

constructor(address _admin,  address _safeHouse, address _stableTokenAddress, address _alphaToken,
        address _depositNFTAdress, address _withdrawalNFTAdress) Investement( _admin, _safeHouse,  _stableTokenAddress,  _alphaToken,
         _depositNFTAdress,  _withdrawalNFTAdress) {
...
import "./WithdrawConfirmation.sol";

/**
 * @title Investement
 * @dev Implementation of Investement
 */
...
contract Investement is Pausable {
    ...
    uint256 public  DEPOSIT_FEE_RATE_PARITY;
    ...
    uint256 public netAmountEventAfterFees;
    uint256 public maxDepositAmount;
    uint256 public maxWithdrawAmount;
    uint256 public withdrawAmountTotal;
    uint256 public withdrawAmountTotalOld;
    uint256 public depositAmountTotal;
    uint256 public finalizedDepositParityAmount = 0;
    uint256 public finalizedWithdrawParityAmount = 0;
    uint256 public alphaParityAmount = 0;
   ...
   // Other variables for Alpha strategy  
    ...
    DepositConfirmation public depositNFT;
    WithdrawConfirmation public withdrawalNFT;
    event DepositRequest(address indexed _user, uint256 _amount);
    event CancelDepositRequest(address indexed _user, uint256 _amount);
    event WithdrawRequest(address indexed _user, uint256 _amount);
    event CancelWithdrawRequest(address indexed _user, uint256 _amount);
    event FinalizeDeposits(address indexed _user, uint256 _finalizedAmount, uint256 _mintedAmount);
    event FinalizeWithdrawals(address indexed _user, uint256 _finalizedAmount, uint256 _SentAmount);
    ...
    constructor(address _admin, address _safeHouse, address _stableTokenAddress, address _alphaToken,
        ...
        require(
            ...
            "Formation.Fi: safeHouse address is the zero address"
        );
        require(
        ...
        depositNFT = DepositConfirmation(_depositNFTAdress);
        withdrawalNFT = WithdrawConfirmation(_withdrawalNFTAdress);
    ...
    function set_PARITY(address _parity) external 
      onlyOwner{
         parity = _parity;      
    }

     function set_SafeHouse(address _safeHouse) external 
      onlyOwner{
          require(
            _safeHouse != address(0),
            "Formation.Fi: safeHouse address is the zero address"
    ...
    function set_stableToken(address _stableTokenAddress) external 
      onlyOwner{
         ...
         require(
            (depositAmountTotal == 0) && (withdrawAmountTotal==0),
            "Formation.Fi: _cannot change the stable token"
        ); 
         stableToken = IERC20(_stableTokenAddress);
      }
    function updateAdminData() internal {
      ...
      DEPOSIT_FEE_RATE_PARITY = admin.DEPOSIT_FEE_RATE_PARITY();
    ...
    function calculateAcceptedWithdrawRequests(address[] memory _users) 
        ...
        acceptedWithdrawPerAddress[_user] = _amountLP;
        }   
    }
    ...
    function finalizeDeposits( address[] memory _users) external 
        ...
        for (uint256 i = 0; i < _users.length  ; i++) {
            ...
            if (maxDepositAmount <= _amountStableTotal) {
             ...
             if (_user == parity) {
             _feeStable =  (_amountStable * DEPOSIT_FEE_RATE_PARITY ) /
            ...
            _feeStable =  (_amountStable * DEPOSIT_FEE_RATE ) /
             ...
             if (_user == parity) {
              finalizedDepositParityAmount = _amountStable;
              alphaParityAmount = _depositAlpha;
             }
             ...
             emit FinalizeDeposits( _user, _amountStable, _depositAlpha);
        }
        ...
        if ( _feeStableTotal >0){
           ...
           netAmountEventAfterFees = netAmountEvent - _feeStableTotal;
        }
    }
    ...
    function finalizeWithdrawals(address[] memory _users) external
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            if (_user == parity) {
              finalizedWithdrawParityAmount =  _amountStable;
              alphaParityAmount = _amountLP;
             }
            ...
            emit FinalizeWithdrawals(_user,  _amountLP, _amountStable);
        }
        if ((tokensToBurn) > 0){
    ...
    function depositRequest(address _user, uint256 _amount) external whenNotPaused {
        ...
        if (depositNFT.balanceOf( _user)==0){
            ...
            depositNFT.mint( _user, tokenIdDeposit, _amount);
        }
        else {
            uint256 _tokenIdDeposit = depositNFT.getTokenId(_user);
            depositNFT.updateDepositData (_user,  _tokenIdDeposit, _amount, true);
        }
    ...
    function cancelWithdrawalRequest( uint256 _amount) external whenNotPaused {
        ...
        emit CancelWithdrawRequest(msg.sender, _amount);
    }
    ...
    function sendToSafeHouse(uint256 _amount) external 
        ...
        stableToken.safeTransfer(safeHouse, _amountScaled);
    }
    
    // Send stable tokens to the contract
    function sendStableToken(uint256 _amount) external 
      whenNotPaused onlyManager {
      require( _amount > 0,  "Formation.Fi: amount is zero");
      stableToken.safeTransfer(safeHouse, _amount/AMOUNT_SCALE_DECIMALS);
    } 
...
contract SafeHouse is  Pausable {
    ...
    uint256 public PRICE_SCALE_DECIMALS = 1e8;
    uint256 public MAX_WITHDRAW = 1000000 * 1e18;
    uint256 public maxWithdraw =  1000000 * 1e18; 
    ...
    constructor( address _assets, address _admin) payable{
        require(
            _assets != address(0),
            "Formation.Fi: assets address is the zero address"
        );
         require(
            _admin != address(0),
            "Formation.Fi: admin address is the zero address"
    ...
    modifier onlyManager() {
     ...
     function set_MAX_WITHDRAW( uint256 _MAX_WITHDRAW) external whenNotPaused onlyOwner {
     MAX_WITHDRAW = _MAX_WITHDRAW;
     }
    
    function set_MaxWithdraw( uint256 _maxWithdraw) external whenNotPaused onlyOwner {
     maxWithdraw = _maxWithdraw;
     }

    function set_Tolerance( uint256 _tolerance) external whenNotPaused onlyOwner {
    ...
    function sendAsset( address _asset, uint256 _amount) external whenNotPaused onlyManager payable {
        
       
        uint256 _price;
        _price= uint256(getLatestPrice( _asset));
       
        uint256  _maxWithdraw = maxWithdraw + (_amount * _price) /PRICE_SCALE_DECIMALS ;
        maxWithdraw = Math.min(_maxWithdraw, MAX_WITHDRAW);
        ERC20 asset = ERC20(_asset);
          if ( _asset == address(0)) {
          require ( _amount == msg.value, "Formation.Fi: amount is not correct");
        }
        else {
    ...
    function withdraw( address _asset, uint256 _amount) external whenNotPaused onlyManager {
        uint256 _price;
        _price = uint256(getLatestPrice( _asset));
        uint256 _delta = (_amount * _price)  /PRICE_SCALE_DECIMALS;
        require ( Math.min(maxWithdraw, MAX_WITHDRAW) >= _delta , "Formation.Fi: maxWithdraw is lower than delta");
        maxWithdraw = maxWithdraw  - _delta  + (_delta * tolerance)/ PRICE_SCALE_DECIMALS;
         if ( _asset == address(0)) {
    ...
    function getLatestPrice( address _asset) public view returns (uint256) {
        require (assets.isWhitelist(_asset) ==true, "Formation.Fi: asset is not in allowed");
        uint256 _index =  assets.getIndex(_asset);
        uint256 _price;
        address _oracle;

        (, _oracle, _price) = assets.assets(_index);
    ...
    function sendToVault( address _asset, address _vault,  uint256 _amount) external whenNotPaused onlyManager {
         require (_vault ==address(0) , "Formation.Fi: the vault address is zero");
         require (vaultsList[_vault] == true , "Formation.Fi: vault address is not approved");
        if ( _asset == address(0)){
        require (_amount <= address(this).balance , "Formation.Fi: the balance of the contract is not enough");
        payable (_vault).transfer(_amount);
        }
        else{
        ERC20 asset = ERC20(_asset);
         require (_amount <= asset.balanceOf(address(this)) , "Formation.Fi: the balance of the contract is not enough");
        asset.transfer(_vault, _amount);   
...
/**
 * @title WithdrawConfirmation
 * @dev Implementation of WithdrawConfirmation "The Withdrawal Proof"
 */

contract WithdrawConfirmation is ERC721, Ownable {
    ...
    // NFT mapping 
    mapping(address => uint256) private tokenIdPerAddress;
    event MintWithdrawal(address indexed _user, uint256 _id);
    event BurnWithdrawal(address indexed _user, uint256 _id);

    constructor ()  
      ERC721 ("WITHDRAW proof",  "WDP"){
    }
    ...
    function burn(uint256 tokenId) internal {
        ...
        _burn(tokenId);   
        emit BurnWithdrawal(owner, tokenId);
    }
