...
import {StakingPoolsTimeLock} from "./StakingPoolsTimeLock.sol";

/// @title StakingPoolsTimeLock
...
contract StakingPoolsTimeLock is ReentrancyGuard, Ownable {
  ...
  /// @dev How long liquidity & rewards will be locked
  ...
  function deposit(uint256 _poolId, uint256 _depositAmount) external nonReentrant {
    ...
    _stake.timelockEnds = block.number.add(timelockDuration);
  ...
  //   Pool.Data storage _pool = _pools.get(_poolId);
  //   Stake.Data storage _stake = _stakes[msg.sender][_poolId];
  //   require(_stake.timelockEnds < block.number,"StakingPools: Tokens still timelocked");
  ...
  /// @dev Gets the block that a user can withdraw rewards or liquidity.
  ///
  /// @param _account The account to query.
  /// @param _poolId  the identifier of the pool.
  ///
  /// @return the amount of deposited tokens.
  function getTimelockEndBlock(address _account, uint256 _poolId) external view returns (uint256) {
    Stake.Data storage _stake = _stakes[_account][_poolId];
    return _stake.timelockEnds;
  }
...
library FixedPointMath {
  ...
  //   // return FixedDecimal(uint256(-1)); // type(uint).max
  //   return type(FixedDecimal(uint256).max);
  // }
