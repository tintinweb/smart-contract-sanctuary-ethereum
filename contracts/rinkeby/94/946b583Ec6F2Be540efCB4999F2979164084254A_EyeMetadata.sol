...
contract EyeMetadata is IEyeMetadata, Ownable {
    ...
    /* -------------------------------------------------------------------------- */
    IEye private _eye;
    string private _svgURIBase;
    string private _animationURIBase;
    address private _divineAddress;
    ...
    /*                               INITIALIZATION                               */
    /* -------------------------------------------------------------------------- */
    constructor(address eyeAddress, address divineAddress) {
        _eye = IEye(eyeAddress);
        _divineAddress = divineAddress;
    }
    ...
    /// @param svgBaseURI The SVG base URI to set.
    function setSvgBaseURI(string calldata svgBaseURI) external onlyOwner {
        _svgURIBase = svgBaseURI;
    }

    /// @notice Sets the base URI for the token JSON
    /// @param animationBaseURI The animation base URI to set.
    function setAnimationBaseURI(string calldata animationBaseURI) external onlyOwner {
        _animationURIBase = animationBaseURI;
    }
    ...
    function _getSvgURL(uint256 tokenId) 
        internal 
        view 
        returns (bytes memory) 
    {
      ...
      returns (bytes memory)
    {
      ...
      abi.encodePacked(
        ...
        ((_eye.getAttunement(tokenId) == 0) ? 'Dark' : 'Light'),
        '"}, {"trait_type":"Order", "value": "',
        _getOrderName(_eye.getOrder(tokenId)),
        '"}, {"trait_type": "DIVINE Energy", "value": "',
        _confirmERC20Balance(_eye.ownerOf(tokenId), _divineAddress) ? "Charged" : "Depleted",
        '"}]'
      );
    } 
        ...
        returns (bytes memory) 
    {
        return
            abi.encodePacked(
                _confirmERC20Balance(_eye.ownerOf(tokenId), _divineAddress) ? "1" : "0",
                "_",
                _toString(_eye.getAttunement(tokenId)),
                "_",
                _toString(_eye.getOrder(tokenId))
            );
    }
    /* solhint-enable */
    ...
    function _getOrderName(uint256 order) internal pure returns (string memory orderName) {
      if (order == 0) {
        orderName = "Power";
      } else if (order == 1) {
        orderName = "Giants";
