...
contract NFT is ERC1155, Ownable {
    ...
    uint256 nftAmountPerUser;
    uint256 transferFee = 0.001 ether;
    ...
    function mint(address to) external payable {
        ...
        if (whitelist[msg.sender] > 0) {
            require(amountsNFT[msg.sender] + 1 <= whitelist[msg.sender]);
        } else {
            require(amountsNFT[msg.sender] + 1 <= nftAmountPerUser);
        }
        ...
        items[tokenIds] = Item(tokenIds, msg.sender, 1);
    ...
    function addToWhitelist(address user, uint256 amount) external {
        require(
    ...
    function isUserInWhitelist(address user) external view returns (uint256) {
