...
/**
@author: dotyigit - Core Developer of Fluffy Polar Bears
$$$$$$$$\ $$$$$$$\  $$$$$$$\
$$  _____|$$  __$$\ $$  __$$\
$$ |      $$ |  $$ |$$ |  $$ |
$$$$$\    $$$$$$$  |$$$$$$$\ |
...
contract FluffyPolarBears is ERC721, Ownable, Pausable {
    ...
    address public oldFpbContract;
    address public oldFpb99Contract;
    ...
    // Token based transfer lock for gaming scenarios
    mapping(uint256 => bool) public isTokenLocked;

    // Token based locking requirements
    modifier onlyOwnerOrHolder(uint256 tokenId) {
        require(msg.sender == owner() || msg.sender == this.ownerOf(tokenId));
        _;
    }

    // When not locked modifier to check whether the tokenId can be transferrable
    modifier whenNotLocked(uint256 tokenId) {
        require(!isTokenLocked[tokenId], "FPB: Token is locked.");
        _;
    }
    ...
    function migrateFluffyPolarBears(uint256 quantity) external onlyOwner {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            _mint(
                IERC721(oldFpbContract).ownerOf(lastCheckedFPB),
                lastCheckedFPB
            );
            lastCheckedFPB++;
        }
    ...
    function migrateSpecialEditions(uint256 quantity) external onlyOwner {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            _mint(
                IERC721(oldFpb99Contract).ownerOf(lastCheckedFPB99),
                lastCheckedFPB99 + 9342
            );
            lastCheckedFPB99++;
        }
    }
    ...
    // Security functions
    function lockTokenTransfer(uint256 tokenId)
        external
        onlyOwnerOrHolder(tokenId)
    {
        require(!isTokenLocked[tokenId], "FPB: Token is already locked.");
        isTokenLocked[tokenId] = true;
    }

    function unlockTokenTransfer(uint256 tokenId)
        external
        onlyOwnerOrHolder(tokenId)
    {
        require(isTokenLocked[tokenId], "FPB: Token is already unlocked.");
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal override(ERC721) whenNotPaused whenNotLocked(tokenId) {
        super._beforeTokenTransfer(from, to, tokenId);
    }

    // In case of any balance occurences in contract
    function withdrawAll() external onlyOwner {
        uint256 balance = address(this).balance;
        require(balance > 0, "FPB: No balance to withdraw money.");
    ...
    function _withdraw(address _address, uint256 _amount) internal {
        ...
        require(success, "FPB: Transfer failed.");
    }
}
