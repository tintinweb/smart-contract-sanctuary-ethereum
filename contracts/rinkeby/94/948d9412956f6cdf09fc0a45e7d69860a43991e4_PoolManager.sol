...
interface IPoolManager {
  ...
  function getUSDRate() external view returns (uint,uint32);
...
contract PoolManager is OwnableUpgradeable, IPoolManager {
  ...
  function bnbLiquidity(address payable _reciever, uint256 _amount) public onlyOwner {
    ...
    function getUSDRate() public override view returns (uint,uint32){
        ...
        uint balance0 = IERC20Upgradeable(reserve0).balanceOf(USDPAIR);
        uint balance1 = IERC20Upgradeable(reserve1).balanceOf(USDPAIR);

        if(reverseCheck){
            uint rate = balance1.div(balance0).mul(10**USDPAIRDECIMALS).mul(10**18);
            return (rate , uint32(block.timestamp));
        } 
        else{
            uint rate = balance0.div(balance1).mul(10**USDPAIRDECIMALS).mul(10**18);
            return (rate , uint32(block.timestamp));
        }
    }
}