...
contract YomazukaFamily is ERC721A, Ownable {
  ...
  uint256 public publicUnitsSold = 0;
  ...
  /* function isMintValid(uint256 _quantity) internal view returns (bool) {
    if ((publicUnitsSold + _quantity) < NUMBER_OF_UNITS_DROP_1 && !drop1) {
      return false;
    } else if (
      (publicUnitsSold + _quantity) < NUMBER_OF_UNITS_DROP_2 && !drop2
    ) {
      return false;
    } else {
      return true;
  ...
  function mint(uint256 _quantity) external payable callerIsUser {
    if ((publicUnitsSold + _quantity) < NUMBER_OF_UNITS_DROP_1) {
