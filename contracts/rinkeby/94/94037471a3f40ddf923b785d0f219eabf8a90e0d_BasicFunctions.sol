...
contract BasicFunctions {
    ...
    // returns a string
    function getMyCoinName() public view returns(string memory) { // strings need memory keyword allocation
        return coinName;
    }

    // that can only called externally
    //  changes value of balance variable above to multiplied value
    function multiplyBalance(uint _multiplier) external {
        myBalance = myBalance * _multiplier;
    }

    // that uses a for loop and multiplies aprams and string comparisons
    function findCoinIndex(string[] memory _myCoins, string memory _find, uint _startFrom) public pure returns (uint) {
        for (uint i = _startFrom; i < _myCoins.length; i++) {
            ...
            // keccak256 method is required to compare string values
            if (keccak256(abi.encodePacked(coin)) == keccak256(abi.encodePacked(_find))) {
                return i;
            }
        }
        return 9999; // if above condition isnt met, requires a return value on exit 
                     // otherwise error
    }
    // update a mapping
    // pass arguments into struct above to create coin object
    // save to user address
    ...
    // function to view myCoin mapping
