...
contract The10kClub is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 public constant price = 0.00555 ether;
    ...
    error SaleClosed();
    error NotOwnerOfTokenENS10kClub();
    error AllMinted();
    error AlreadyMinted();
    ...
    function length4Pad(uint256 tokenNum) public pure returns (string memory) {
    ...
    // phase 1, 3 - token #abc buyable by abc.eth
        ...
        callerIsUser
    {
        ...
        if (salePhase == 1 && ownerOfENS(tokenId) != msg.sender)
            revert NotOwnerOfTokenENS10kClub();
    ...
    // phase 2 - token #abc buyable by xxx.eth
    ...
    function setSaleState(uint8 _salePhase) external onlyOwner {
        salePhase = _salePhase;
    }
}
