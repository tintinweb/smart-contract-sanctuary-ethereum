...
contract MPWRStaking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function harvestFee() external whenNotPaused nonReentrant {
        ...
        uint256 reward = getHavestAmount(_msgSender());
        require(reward > 0, "Reward already claimed");
        uint256 balance = WETH.balanceOf(address(this));
        if (balance == 0 || accMarketFee == 0) {
            return;
        }
        uint256 rewardFee = reward / PRECISION_FACTOR;
    ...
    function getHavestAmount(address _user) public view returns (uint256) {
        ...
        if (
            stakers[_msgSender()].lastharvested != 0 &&
            ((block.timestamp - stakers[_msgSender()].lastharvested) / 3600) <= 24
        ) {
            return 0;
        }
        return ((stakers[_user].amount / totalAmountStaked) * accMarketFee) / PRECISION_FACTOR;
    }
