...
contract Morra is ERC20Burnable, Operator {
    ...
    function _getMorraPrice() internal view returns (uint256 _morraPrice) {
        try IOracle(morraOracle).consult(address(this), 1e18) returns (uint144 _price) {
    ...
    function _updateTaxRate(uint256 _morraPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_morraPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMorraOracle(address _morraOracle) public onlyOperatorOrTaxOffice {
        require(_morraOracle != address(0), "oracle address cannot be 0 address");
        morraOracle = _morraOracle;
    }
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMorraPrice = _getMorraPrice();
            currentTaxRate = _updateTaxRate(currentMorraPrice);
            if (currentMorraPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_morraPool != address(0), "!_morraPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_morraPool, INITIAL_TOMB_POOL_DISTRIBUTION);
