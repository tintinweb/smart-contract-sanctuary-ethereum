 ...
 *
 * #LetsBTF
 * (BTF == Build The Future)
 *
 */
contract LetsBTF is ERC721AQueryable, ReentrancyGuard, VRFConsumerBaseV2, ERC2981ContractWideRoyalties, EIP712Signature {

    /* =========== MAPPINGS & ARRAYS =========== */

    mapping(uint256 => TokenContainer) public requestIdToTokenId;
    mapping(uint256 => Credit) public tokenCredits;
    ...
    address public osProxyRegistryAddress;
    ...
    constructor(
      address _osProxyRegistryAddress, // OpenSea proxy
    ...
    ERC721A ("Let's BTF", "LETSBTF")
    VRFConsumerBaseV2(_vrfCoordinator) {
      osProxyRegistryAddress = _osProxyRegistryAddress;
    ...
    */
    function devMint(uint256 _quantity) external onlyOwner {
      if ((_quantity > devMintSupplyRemaining) || totalSupply() + _quantity > MAX_SUPPLY) { revert MaxSupplyReached(); }
    ...
    /**
    * @notice Public mint. Must contain a signature that is signed by our dApp.
    ...
    */
    function publicMint(bytes calldata _signature, uint256 _quantity) external payable requiresValidSignature(_signature) {
        ...
        if ((totalSupply() + devMintSupplyRemaining) + _quantity > MAX_SUPPLY) { revert MaxSupplyReached(); } 
    ...
    */
    function _performMint(uint256 _quantity) private {
        ...
        // requestIdToTokenId[startingTokenId] = TokenContainer(uint16(startingTokenId), uint16(_quantity));
    ...
    */
    ...
    function creditIncreaseTest(uint256 requestId, uint256 _seed) 
      public {
        TokenContainer memory container = requestIdToTokenId[requestId];
        delete requestIdToTokenId[requestId];
        ...
        unchecked {
          ...
          for (uint256 i = 0; i < container.quantity; ++i) {
            uint256 tokenId = container.startId + i;
            uint256 seed = uint256(keccak256(abi.encode(_seed, tokenId))) % 1000;
            tokenIds[i] = tokenId;

            Credit storage credit = tokenCredits[tokenId];
            if (credit.totalCredit == 0) {
                // ChainLink recommends not to revert inside fulfillRandomWords
                uint256 cred;
    ...
    */
    function getTotalCreditForToken(uint256 _id) external view returns(uint256) {
      return tokenCredits[_id].totalCredit;
    }
    ...
    */
    ...
    /**
    * @dev When using this internally gas is reduced slightly compared to using `getAvailableCreditForToken` 
    * because we don't need to read from storage.
    */
    function _calculateAvailableCredit(uint256 _totalCredit, int256 _claimedCredit) internal pure returns(uint256) {
      if (_claimedCredit < 1) {
        return _totalCredit;
      } else {
        return _totalCredit - uint256(_claimedCredit);
      }
    }
    ...
    */
    function claimCredit(uint256 _id, uint256 _amountToClaim)
      ...
      amountGreaterThanZero(_amountToClaim)
      {
        ...
        // emit CreditClaimed(block.timestamp, _id, _amountToClaim, 1); //credit.totalCredit - credit.claimedCredit
    }
    ...
    */
    function repayCredit(uint256 _id) 
      ...
      amountGreaterThanZero(msg.value) {
        ...
        // emit CreditRepaid(block.timestamp, _id, msg.value, 1); //credit.totalCredit - credit.claimedCredit
    }
    ...
    */
    ...
    /**
    * @dev Set the OpenSea proxy address
    */
    function setosProxyRegistryAddress(address _osProxyRegistryAddress) external onlyOwner {
        osProxyRegistryAddress = _osProxyRegistryAddress;
    }
    ...
    */
    function amountAvailableForOwnerWithdrawal(uint256 _percentage) 
      ...
      returns(uint256) {
        ...
        uint256 totalCredit = mintCreditCounter - 1;        
        uint256 totalAmount = MINT_PRICE * totalSupply();

        // console.log("");
        // console.log("totalAmount (excl prefunds): %s eth", totalAmount / 1 ether);
        // console.log("totalCredit: %s", totalCredit);
        // console.log("totalWithdrawedAmount: %s", totalWithdrawedAmount);
        ...
        // console.log("totalAmount attempting to claim: %s", totalAmount);
        // console.log("Current bal: %s", address(this).balance);
