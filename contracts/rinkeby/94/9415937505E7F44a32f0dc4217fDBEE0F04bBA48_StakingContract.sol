...
contract StakingContract is Ownable {
    ...
    function pendingReward(address _user, uint256 _tokenId) public view returns (uint256) {
        ...
        (bool _isStaked, uint256 _startBlock, bool firstClaim) = getStakingItemInfo(_user, _tokenId);
        if(!_isStaked) return 0;
        uint256 currentBlock = block.number;
        uint256 contractBal = contractBalance();
        if(firstClaim) 
            return uint256(5).mul(RewardTokenPerBlock).div(contractBal);
        if(_startBlock + 15 >=  currentBlock)
            return 0;
    ...
    function getStakingItemInfo(address _user, uint256 _tokenId) public view returns(bool _isStaked, uint256 _startBlock, bool firstClaim) {
        for(uint256 i = 0; i < userInfo[_user].length; i++) {
            if(userInfo[_user][i].tokenId == _tokenId) {
                _isStaked = true;
                _startBlock = userInfo[_user][i].startBlock;
                firstClaim = userInfo[_user][i].firstClaim;
                break;
            }
        }
    }
        ...
        for (uint256 i = 0; i < userInfo[msg.sender].length; i++)
            {
                ...
                userInfo[msg.sender][i].firstClaim = false;
