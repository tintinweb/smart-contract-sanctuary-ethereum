...
contract NftDueWallet is ERC721URIStorage, VRFConsumerBase, Ownable {
    ...
    function getRandomNumber(uint256 tokenId) public returns(uint256 randomNumber) {
    ...
    function generateSVG(uint256 _randomness) public view returns (string memory finalSvg) {
        uint256 numberOfPaths = (_randomness % maxNumberOfPaths) + 2;
    ...
    function generatePath(uint256 _randomness) public view returns(string memory pathSvg) {
        uint256 numberOfPathCommands = (_randomness % maxNumberOfPathCommands) + 3;
        pathSvg = "<path d='";
        ...
        string memory filler = fillers[_randomness % fillers.length];
        pathSvg = string(abi.encodePacked(pathSvg, "' fill='", filler, "' stroke='", color,"'/>"));
    }
    ...
    // https://stackoverflow.com/a/65707309/11969592
