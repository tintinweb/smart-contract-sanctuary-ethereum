...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata{
    ...
    string baseUri = "ipfs://QmUsj9NLXx58nK9UkjgMncswepSDWHEQX71TLn4QifJs3g/";

    // metadata api before reveal
    string notRevealedUri = "ipfs://QmVS8WMpLpBrxanSHxHFTtXvVPJhHJdDsZzsnp2JxEt7J3"; 
 ...
 contract MetaToads is ERC721, Ownable{
    ...
    uint256 public specialMinted = 0;               // Special Minted Supply

    uint256 public publicMintLimit = 10000;         //  Public mint limit
    uint256 public giftMintLimit = 30;              //  Gift mint limit
    uint256 public specialMintLimit = 20;           //  Special mint limit 

    uint256 private giftMintId = 10000;
    uint256 private specialMintId = 10030;

    uint256 public mintPrice = 0.0001 ether;        // minting price 0.0001 Ethereum
    
    bool public publicSaleIsActive = false;
    bool public privateSaleIsActive = true; 
    bool public isActive = true;  //All the sales are active (Private + public)
    ...
    function mintByUser(uint256 quantity, bytes memory signature, address signer) public isPause isHalt payable{
        ...
        require(whiteListMinted[msg.sender] + quantity <= 20, "Max NFT per wallet exceeded in Private Sale");
...
/*
    function setPauseOrHaltStatus() public onlyOwner{
        isActive = false;   //for setting isActive false 
        isPaused = !isPaused;
        isHalted = !isPaused;
    ...
    // sets active status;
    // but we cannot know the state of isHalt or isPaused
    //while calling setPauseOrHaltStatus doesn't change
    function setActiveStatus() external onlyOwner{
        ...
        if(isActive == true){
           ...
           setPauseOrHaltStatus();
        }
    }
    

    //setActiveStatus to true
    function SetActiveStatus() external onlyOwner{
        require(isActive == false, "Active status is already true");
        isActive = !isActive; //true
        isHalted = !isActive; //false
...
*/
    ...
    // 1 for Pause
    // 2 for Halt
    function setMintingStatus(uint8 status) external onlyOwner{
        if(status == 0){ 
            isActive = true;
            isHalted = false;
            isPaused = false; 
    ...
    function giftMint(address _address, uint256 quantity) public isHalt onlyOwner{
        require((giftMinted + quantity) <= giftMintLimit, "You are exceeding gift mint supply.");
    ...
    function specialMint(address _address) public isHalt onlyOwner{
        require((specialMinted + 1) <= specialMintLimit, "You are exceeding special mint supply.");

        specialMinted++;
        specialMintId++;
        totalMinted++;
        _safeMint(_address, specialMintId);
