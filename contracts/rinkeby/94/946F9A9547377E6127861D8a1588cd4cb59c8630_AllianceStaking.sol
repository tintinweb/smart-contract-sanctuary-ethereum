...
contract AllianceStaking is Managable {
    ...
    // Storage
    mapping(string => Stake[]) public stakings;
    mapping(address => string[]) public addressStakings;
    address stakingTokenAddress;

    // Events
    event AddedStake(address _addr, string _id, uint256 _amount);
    event RemovedStake(address _addr, string _id, uint256 _amount);
    event ChangedTokenAddress(address _addr);
    event ChangedAllianceStaking(string _id, uint256 _amount);
    ...
    function stake(string memory _id, uint256 _amount) external {
        _stake(msg.sender, _id, _amount);
    }

    function unstake(string memory _id, uint256 _amount) external {
        _unstake(msg.sender, _id, _amount);
    }

    function allianceTotalStaking(string memory _id) external view returns(uint256) {
        return _allianceTotalStaking(_id);
    }

    function addrStakings(address _addr) external view returns(string[] memory, uint256[] memory) {
        string[] memory _alliances = addressStakings[_addr];
    ...
    // Internal functions
    function _stake(address _sender, string memory _id, uint256 _amount) internal {
    ...
    function _unstake(address _sender, string memory _id, uint256 _amount) internal {
        (uint256 idx, bool found) = _findStakeIndex(_sender, _id);
        if (found) {
            Stake memory _s = stakings[_id][idx];
    ...
    function _findStakeIndex(address _sender, string memory _id) internal view returns(uint256, bool){
    ...
    function _addAllianceToAddress(address _addr, string memory _id) internal {
        string[] memory _stakes = addressStakings[_addr];
        for (uint256 i = 0; i < _stakes.length; i++) {
            string memory _s = _stakes[i];
            if (keccak256(bytes(_s)) == keccak256(bytes(_id))) {
                return;
            }
        }
    ...
    function _allianceTotalStaking(string memory _id) internal view returns(uint256 _amount) {
