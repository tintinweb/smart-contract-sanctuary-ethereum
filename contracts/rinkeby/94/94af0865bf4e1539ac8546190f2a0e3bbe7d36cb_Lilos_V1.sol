...
contract Lilos_V1 {
	...
	function repay(uint256 listingId_) public payable {
		ListingItem storage listingItem = _listingItems[listingId_];
	...
	function liquidate(uint256 listingId_) public payable {
		ListingItem storage listingItem = _listingItems[listingId_];
		require(listingItem.status == ListingStatus.Leased, "Token is not leased.");
		...
		payable(listingItem.lessee).transfer(listingItem.collateral_value);
		isTokenListed[listingItem.collection][listingItem.tokenId] = false;
		listingItem.status = ListingStatus.Delisted;
	...
	function getIsTokenListed(address collection_, uint256 tokenId_) public view returns (bool) {
		return isTokenListed[collection_][tokenId_];
	}

	function getListingTotal() public view returns (uint256) {
		uint256 listingTotal;
		if(_listingId == 0){
			listingTotal = 1;
		}
		return _listingId.sub(1);
	}
