...
contract Staker {
  ...
  function stake() public payable {
    require(deadline > block.timestamp, "Times end, no more funds accepted");
    uint amount = msg.value;
    address caller = msg.sender;
    // (bool success,) = address(this).call{value: amount}("");
    // require(success, "Ether no staked");
  ...
  function execute() public notCompleted{
    require(deadline < block.timestamp, "Deadline has not been met");
    require(timesExecuted < 1, "Execute function must be executed only once");
    uint stakeBalance = address(this).balance;
  ...
  function withdraw() public notCompleted{
    if (!openForWithdraw) {
      revert("The stake is not open for withdraw");
    }
    if (balances[msg.sender] == 0) {
      revert("You dont have funds in the stake");
    }
    uint userBalance = balances[msg.sender];
    (bool success, ) = address(msg.sender).call{value: userBalance}("");
    require(success, "Error, no ether was withdraw");
