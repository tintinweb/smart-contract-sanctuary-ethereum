...
contract DoILookLikeICare is NFTokenMetadata, Ownable {
    // Properties
    address payable feeAddress;

    // OmegaM
    uint256 public constant nftPrice = 0.25 ether;
    uint256 public mintPrice = nftPrice;
    uint256 public constant maxNft = 5000; // Total nfts
    ...
    constructor() {
        // OmegaM
        nftName = "Do I Look Like I Care Test";
        nftSymbol = "DILLIC";
    ...
    function mint(uint8 _mintNum, bytes32[] calldata _proof) public payable {
        require(
            isMintingActive,
            "Minting not available yet or paused for next round"
        );
        require(_mintNum > 0, "At least one art is needed");
        require(
            _mintNum + nftClaimed[msg.sender] <= mintLimit,
            "Can't mint more than 1 limit per Wallet."
        );
        require(
            msg.value >= mintPrice * _mintNum,
            "NFTs are expensive, need more ETH to afford that."
        );
        require(
            current_minted + _mintNum <= maxNft,
            "Not enough of those left in stock."
        );
        ...
        if (isMerkleActive) {
            require(
                verify(_proof),
                "Not in the Merkle tree, only for exclusive invitees"
            );
        }
        ...
        current_minted += _mintNum;
        nftClaimed[msg.sender] += _mintNum;

        emit Minted(msg.sender, _mintNum);
    }

    // Emergency Devmint function if something gets messed up.
    function devMint(uint16 _mintNum) external onlyOwner {
        require(
            _mintNum + devMinted <= devMintLimit,
            "Cannot dev mint more the limit for devMint."
        );

        require(
            _mintNum + current_minted <= maxNft,
            "Cannot mint more the total supply."
        );
        for (uint256 i = 0; i < _mintNum; i++) {
            super._mint(msg.sender, current_minted);
        }
    ...
    function updatedevMintLimit(uint256 newLimit) external onlyOwner {
        devMintLimit = newLimit;
    }
    ...
    // Update how many can be purchased at a time if need be. OmegaM
    function updateMintPrice(uint256 newPrice) external onlyOwner {
        mintPrice = newPrice * 1 ether;
    }
}