...
{
    using NFTContractLibrary for uint8;
     ...
     */
    function hash(uint256 _t, address _a, uint256 _c) internal returns (string memory) 
    {
        require(_c < 10);
        ...
        {
            SEED_NONCE++;
            uint16 _randinput = uint16(uint256(keccak256(
                        abi.encodePacked(
                            block.timestamp,
                            block.difficulty,
                            _t,
    ...
    function mintInternal() internal {
        ...
        require(!NFTContractLibrary.isContract(msg.sender));
    ...
    function hashToSVG(string memory _hash) public view returns (string memory)
    {
        ...
        for (uint8 i = 0; i < 9; i++) {
            uint8 thisTraitIndex = NFTContractLibrary.parseInt(NFTContractLibrary.substring(_hash, i, i + 1));

            for (uint16 j = 0;j < traitTypes[i][thisTraitIndex].pixelCount;j++) 
            {
                string memory thisPixel = NFTContractLibrary.substring(
                ...
                uint8 x = letterToNumber(
                    NFTContractLibrary.substring(thisPixel, 0, 1)
                );
                uint8 y = letterToNumber(
                    NFTContractLibrary.substring(thisPixel, 1, 2)
                );
                ...
                svgString = string(
                    abi.encodePacked(
                        svgString,
                        "<rect class='c",
                        NFTContractLibrary.substring(thisPixel, 2, 4),
                        "' x='",
                        x.toString(),
                        "' y='",
                        y.toString(),
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    NFTContractLibrary.encode(
                        bytes(
                            string(
                                abi.encodePacked(
                                    '{"name": "Anonymice #',
                                    NFTContractLibrary.toString(_tokenId),
    ...
    function _tokenIdToHash(uint256 _tokenId) public view returns (string memory)
    {
        ...
        if (ownerOf(_tokenId) == 0x000000000000000000000000000000000000dEaD) 
        {
            tokenHash = string(
                abi.encodePacked(
                    "1",
                    NFTContractLibrary.substring(tokenHash, 1, 9)
                )
            );
        }
        return tokenHash;
    ...
    function addTraitType(uint256 _traitTypeIndex, Trait[] memory traits) public onlyOwner
    {
