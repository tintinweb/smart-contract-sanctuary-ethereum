...
contract NFTFactory is Ownable {
    ...
    constructor(address _earlyAccessPass) {
        proxyImplementation = address(new TemplateNFT());

        earlyAccessPass = IERC721AUpgradeable(_earlyAccessPass);

        maxPerMintLimit = TemplateNFT(payable(proxyImplementation))
            .MAX_PER_MINT_LIMIT();
    ...
    function createNFT(
        ...
        TemplateNFT(payable(clone)).initialize(
            _startPrice,
            _maxSupply,
            _nReserved,
            _maxTokensPerMint,
        ...
        TemplateNFT(payable(clone)).transferOwnership(msg.sender);
    ...
    function createNFTWithSettings(
        ...
        if (miscParams & SHOULD_START_SALE != 0) {
            TemplateNFT(payable(clone)).startSale();
        }

        if (payoutReceiver != address(0)) {
            TemplateNFT(payable(clone)).setPayoutReceiver(payoutReceiver);
        }

        if (miscParams & SHOULD_LOCK_PAYOUT_CHANGE != 0) {
            TemplateNFT(payable(clone)).lockPayoutChange();
        }
