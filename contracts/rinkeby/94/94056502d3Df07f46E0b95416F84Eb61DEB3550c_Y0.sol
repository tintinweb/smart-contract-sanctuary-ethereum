...
contract Y0 is ERC1155, Ownable {
  ...
  */
  ...
  function price(uint256  _mintType) public view returns (uint256) {
     if (_mintType == 1) {
      // Normal type NFT
      return uint256(_mintType * normal_car_price);
    } else if (_mintType == 2) {
      // Rare type NFT
      return uint256(_mintType * rare_car_price);
    } else if (_mintType == 3) {
      // Super type NFT
      return uint256(_mintType * super_car_price);
    } else if (_mintType == 4) {
      // Extra type NFT
      return uint256(_mintType * extra_car_price);
    }
  }
