...
// not using or importing these 2:
...
contract Thr33zi3sNG is ERC721Omnibus, Royalties, Signable, Withdrawable {
    using Address for address;        
    
    // notes: set on ERC721 initialization
    // name, symbol "thr33zi3sNG", "333NG"
    // string baseMetadataURI = "https://api.thr33zi3s.com/token/";
    //unused function in royalties: function _getNiftyType(uint256 tokenId)
    ...
    function burn(uint256 tokenId) public {
        _burn(tokenId);
        collectionStatus.amountCreated -= 1;
    }

    function mint() public {
        _requireOnlyValidSender();
        require(collectionStatus.amountCreated + 1 <= 100, "Exceeds Maximum Supply");
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        balances[to] += 1;
        // ownership will be handled on transfer or replacement mint
        // owners[tokenId] = to; 
    ...
    function burnAndReplace(uint256 initialTokenId) public {
        ...
        _burn(initialTokenId); // calls require(isApprovedOrOwner, ERROR_NOT_OWNER_NOR_APPROVED);
        ...
        counter += 1;
        _setOwnership(_msgSender(), newTokenId);
    }
    ...
    function _isValidTokenId(uint256 tokenId) internal virtual view override returns (bool) {        
        return tokenId > 1000 && tokenId <= counter + 1000; // || ownersOptimized[tokenId].transferred; would need this extra check if using totalSupply for upper bound
    }    

    function totalSupply() public view returns (uint88) {
        return collectionStatus.amountCreated;
    }
}
