...
contract Market is Ownable {
  ...
  function listItemOnAuction(uint256 tokenId)
    external
    existsItem(tokenId)
    onlyNftOwner(tokenId)
  {
    ...
    enoughTokens(bid)
  {
    ...
    if (oldBid > 0) token.transfer(oldBidder, oldBid);
  }
  ...
  modifier onTime(uint256 tokenId) {
    require(
      block.timestamp < auctionItems[tokenId].startAt + auctionPeriod,
  ...
  function finishAuction(uint256 tokenId)
    ...
    existsItem(tokenId)
    activeAuction(tokenId)
    onlyAuctioneer(tokenId)
    afterTime(tokenId)
  {
    AuctionItem memory item = auctionItems[tokenId];
    address[2] memory addresses = item.bidCount >= bidMinCount
      ? [item.seller, item.bidder]
      : [item.bidder, item.seller];
    sendTokensAfterAuction(item.bid, tokenId, addresses);
  }
  ...
  {
    AuctionItem memory item = auctionItems[tokenId];
    sendTokensAfterAuction(item.bid, tokenId, [item.bidder, item.seller]);
  }

  function sendTokensAfterAuction(
    uint256 value,
    uint256 tokenId,
    address[2] memory addresses
  ) private {
    auctionItems[tokenId].isActive = false;
    token.transfer(addresses[0], value);
    nftToken.transferFrom(address(this), addresses[1], tokenId);
  }

  modifier afterTime(uint256 tokenId) {
    require(
      auctionItems[tokenId].startAt + auctionPeriod < block.timestamp,
      "Auction is not gone"
    );
    _;
  }

  modifier onlyAuctioneer(uint256 tokenId) {
    require(auctionItems[tokenId].seller == msg.sender, "Caller is not auctioneer");
    _;
  }
}
