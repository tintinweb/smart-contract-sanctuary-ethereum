    ...
    Ownable
{
    ...
    // Lion price 0.04 ETH
    uint256 public constant LION_PRICE = 40000000000000000;

    // Maximum purchase at once
    uint256 public constant MAX_PURCHASE = 20;

    // Maximum supply
    uint256 public constant MAX_LIONS = 9000;
    ...
    /**
     * @dev function to set/unset sale active
    */
    ...
    */
     ...
     *
     * @param numberOfTokens amount of NFTs for minting.
    */
    function mintLion(uint256 numberOfTokens) external payable {
        ...
        require(
            msg.value >= LION_PRICE.mul(numberOfTokens),
        ...
        uint priceDiff = msg.value - LION_PRICE.mul(numberOfTokens);

        if (priceDiff > 0) {
            (bool success, ) = msg.sender.call{ value: priceDiff }("");
            require(success, "Mint failed");
        }
    }

    /**
     * @dev Function reservs the amount of NFTs and sends them to the executor's address
     *
     * @param to address for reserved tokens.
     *
     * @param numberOfTokens amount of NFTs for reserving.
    */
    function reserveLions(address to, uint256 numberOfTokens) external onlyOwner {
    ...
    */
    ...
    /**
     * @dev function to set the hash for provenance
     *
     * @param provenance hash of the provenance
    */
    ...
    /**
     * @dev function returns the amount of tokens of owner
     *
     * @param tokensOwner address of the tokens owner
    */
    function tokensOfOwner(address tokensOwner)
        external
        view
        returns (uint256[] memory)
    {
        uint256 tokenCount = balanceOf(tokensOwner);
        ...
        if (tokenCount > 0) {
            ...
            for (index = 0; index < tokenCount; index++) {
                result[index] = tokenOfOwnerByIndex(tokensOwner, index);
            }
    ...
    /**
     * @dev function returns the base URI of the collection
    */
