...
// File: testContracts/testpunkxNFT.sol
...
contract testPunkX is ERC721A, Ownable {
    using Address for address;
    ...
    uint256 public maxItemsPerTx = 2000;
    uint256 public maxItemsPerTxForXlist = 3000;
    ...
    // constructor
    constructor() ERC721A("PunkX", "PUNKX", 30000) {}
...
// File: testContracts/testPunkx.sol
...
//@notice This contract is used for staking Punkx NFT's and mint Degen tokens according to the tokenomics mentioned in the whitepaper
//@dev All functions are currently working and having the exact value they should have. TESTS ARE YET TO BE DONE.


contract testDegenToken is ERC20, Ownable, ReentrancyGuard{

    //@Interfaces
    testPunkX Punkx;

    //@Times
    uint public punkXMintingTime = 1643915443;
    uint public punkXStart;
    uint public lockoutTime = 4 minutes; //wannabe 10 days
    uint public dailyRate = 2 minutes; // wannabe 1 day

    //@Rewards
    mapping(uint=>bool) mintRewardClaimed;
    ...
    function putTimeOfFirstMint(uint _time) external onlyOwner {
        punkXMintingTime = _time;
    }

    function ownerMint (uint _amount) external onlyOwner nonReentrant{
    ...
    //@param uint token-ids are taken in a array format and also a bool value if you want to lock immediately
    function stakeTokens (uint[] memory tokenIds,bool lock) external {
    ...
    function lockTokens(uint[] memory tokenIds) public {
        require (tokenIds.length > 1,'Min Lock Amount Is 2');
    ...
    //@notice This function is used to reward those first 2 tokens which were minted before the staking contract was launched
    //@param uint token-ids are taken in an array format
    function rewardFromMint(uint[] memory tokenIds) external {
        ...
        for(uint i=0;i<tokenIds.length;i++){
            ...
            uint timeInDays = (block.timestamp - punkXMintingTime)/ 1 days;
            totalAmount += timeInDays*rewards[0];
        }
    ...
    //@param uint token-ids are taken in an array format
    function claimRewards(uint[] memory tokenIds) public nonReentrant{
        uint totalAmount;
        for(uint i=0;i<tokenIds.length;i++){
            stake storage currentToken = stakedTokens[tokenIds[i]];
            ...
            currentToken.lastClaim += ((block.timestamp - currentToken.lastClaim)/ dailyRate) * dailyRate;
        }
        _mint(msg.sender,totalAmount);
    }

    //@notice This function is to be used by external contract to deduct values from rewards.
    //@param  from user from which amount needs to be deducted
    //@param amount which is to be deducted.
    function claimExternally(address from,uint amount,uint[] memory tokenIds) external {
    ...
    function getReward(uint tokenId) public view returns(uint) {
        ...
        if(block.timestamp - currentToken.lockTime <= lockoutTime){
            ...
            uint timeInDays = (block.timestamp - currentToken.lastClaim)/dailyRate;
            uint combo = currentToken.lockedPieces;
            reward = 1 ether*timeInDays*rewards[tokenId/22]*(10+multiplier[combo])/10 - externallyClaimed[tokenId];
        }
        else{
            ...
            if(block.timestamp > unlockTime && currentToken.lastClaim < unlockTime){
                ...
                uint timeLocked = (currentToken.lockTime + lockoutTime - currentToken.lastClaim)/dailyRate;
                uint timeUnlocked = (block.timestamp - (currentToken.lockTime + lockoutTime))/dailyRate;
                uint combo = currentToken.lockedPieces;
                reward = 1 ether*timeLocked*rewards[tokenId/22]*(10+multiplier[combo])/ 10;
                reward += 1 ether*timeUnlocked*rewards[tokenId/22] - externallyClaimed[tokenId];
            }
            else{
    ...
    function mintExternally(uint _amount) external {
        require(isApproved[msg.sender], "Not approved yet");
        _mint(_msgSender(), _amount);
    }

    function burnExternally(address _from, uint _amount) external {
        require(isApproved[msg.sender], "Not approved yet");
        _burn(_from, _amount);
    }
