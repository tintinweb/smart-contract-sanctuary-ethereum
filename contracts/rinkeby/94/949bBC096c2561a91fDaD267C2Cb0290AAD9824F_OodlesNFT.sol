...
import "./mixins/roles/OodlesOperatorRole.sol";
import "./mixins/OodlesMarketNode.sol";
...
/**
 * @title Oodles NFTs implemented using the ERC-721 standard.
 * @dev This top level file holds no data directly to ease future upgrades.
 */

contract OodlesNFT is
    OodlesTreasuryNode,
    OodlesAdminRole,
    OodlesOperatorRole,
    ERC721Upgradeable,
    ...
    RoyaltiesV2Impl
{
    ...
    function initialize(
        ...
        OodlesMarketNode._initializeOodlesMarketNode(market);
    ...
    /**
     * @notice Allows a Oodles admin to update NFT config variables.
        ...
        returns (uint256 tokenId)
    {
        tokenId = mint(tokenIPFSPath);
        _updateTokenCreator(tokenId, msg.sender);
        setApprovalForAll(getOodlesMarket(), true);
    }
...
/**
 * @notice Allows a contract to leverage the admin role defined by the Oodles treasury.
 */
...
/**
 * @notice Allows a contract to leverage the operator role defined by the Oodles treasury.
 */
abstract contract OodlesOperatorRole is OodlesTreasuryNode {
    // This file uses 0 data slots (other than what's included via OodlesTreasuryNode)

    function _isOodlesOperator() internal view returns (bool) {
        return IOperatorRole(getOodlesTreasury()).isOperator(msg.sender);
    }
}
...
/**
 * @notice A mixin that stores a reference to the Oodles market contract.
 */
abstract contract OodlesMarketNode is Initializable {
    using AddressUpgradeable for address;
     ...
     */
    function _initializeOodlesMarketNode(address _market) internal initializer {
        require(
            _market.isContract(),
            "Oodles MarketNode: Address is not a contract"
        );
        market = _market;
    }

    /**
     * @notice Returns the address of the Oodles market.
     */
    function getOodlesMarket() public view returns (address) {
        return market;
    }
     ...
     */
    function _updateOodlesMarket(address _market) internal {
        require(
            _market.isContract(),
            "Oodles MarketNode: Address is not a contract"
        );
 ...
 */
abstract contract NFT721Metadata is OodlesAdminRole, ERC721Upgradeable {
    using StringsUpgradeable for uint256;
