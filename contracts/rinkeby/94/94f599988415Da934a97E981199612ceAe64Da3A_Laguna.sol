...
contract Laguna is ERC721, Ownable {
  using Strings for uint256;
  ...
  mapping(address => uint256[]) private addressMintedIds;
  mapping(uint256 => address) private mintedIds;
  bytes32 private merkleRoot = "";

  constructor() ERC721("Laguna", "Laguna") {
    setUriPrefix("");
  ...
  function mint(uint256 _mintAmount, uint256[] memory ids) public payable mintCompliance(_mintAmount) {
    ...
    require(!checkIfMintingSameIdOfShoe(ids), "Shoe with id already minted");
  ...
  function verify(bytes32 _merkleRoot, bytes32 leaf, bytes32[] memory proof) internal pure returns (bool) {
   ...
   function checkIfMintingSameIdOfShoe(uint256[] memory _ids) public view returns (bool) {
       if(addressMintedBalance[msg.sender] == 0) return false;
       uint256[] memory tokenIds = addressMintedIds[msg.sender];
       bool alreadyMinted = false;
       for(uint256 i = 0; i < _ids.length; i++) {
           uint256 showId = getShoeId(tokenIds[i]);
           uint256 currentId = getShoeId(_ids[i]);
           if(showId == currentId) {
               alreadyMinted = true;
           }
       }
   ...
   function checkIfBothInSameRange(uint256[] memory _ids) public view returns (bool) {

   }

   function getShoeId(uint256 id) internal pure returns (uint256) {
       if(id >= 1 && id <= 999) {
           return 0;
       }else if (id >= 1000 && id <= 1998) {
           return 1;
       }else if (id >= 1999 && id <= 2997) {
           return 2;
       }else if (id >= 2998 && id <= 3996) {
           return 3;
       }else {
           return 4;
       }
  ...
  function _mintLoop(address _receiver, uint256 _mintAmount, uint256[] memory ids) internal {
    ...
    for (uint256 i = 0; i < _mintAmount; i++) {
      ...
      addressMintedIds[_receiver].push(ids[i]);
