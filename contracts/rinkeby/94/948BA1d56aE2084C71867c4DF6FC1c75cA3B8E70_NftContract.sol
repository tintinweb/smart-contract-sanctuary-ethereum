...
error NftContract__onlyOwnerCanWithdraw();
...
contract NftContract is ERC721URIStorage, Ownable, KeeperCompatibleInterface {
  ...
  string[3] private s_stages = ["young ", "middleAged ", "old "];
  uint256 private s_indexStage = 1;

  // nft image variables //
  string private s_young_cat;
  string private s_ma_cat;
  string private s_old_cat;
  ...
  constructor(
    uint256 mintPrice,
    uint256 interval,
    string memory youngCat,
    string memory maCat,
    string memory oldCat
  ) ERC721("Dynamic Coin NFTs", "DCN") {
    i_mintPrice = mintPrice;
    s_tokenCounter = 1;
    i_interval = interval;
    s_young_cat = svgToImageURI(youngCat);
    s_ma_cat = svgToImageURI(maCat);
    s_old_cat = svgToImageURI(oldCat);
  ...
  function performUpkeep(
    ...
    emit nftUpdate(s_tokenCounter);
  }

  function updateStage() public returns (string memory) {
  ...
  function withdrawFunds() public onlyOwner {
    ...
    if (!success) {
      revert NftContract__onlyOwnerCanWithdraw();
    }
  }
