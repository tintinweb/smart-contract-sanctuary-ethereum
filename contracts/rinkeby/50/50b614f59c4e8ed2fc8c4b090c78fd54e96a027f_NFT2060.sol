...
// File: contracts/NFT2060Meta.sol
...
contract NFT2060 is NFT2060Meta, Ownable, ERC721A {
  ...
  //  address private _signer;
    ...
    uint256 public preSaleSupply = 900;
    uint256 public mintCost = 500;
    ...
    constructor(address receiver) ERC721A("NFT 2060", "2060") {
    ...
    function presaleMint(uint256 quantity, bytes calldata signature)
        external
        payable
        checkState(SaleStates.PreSale)
    {
         if (erc20Contract == address(0)) revert InvalidErc20Token();
         SimpleToken paymentTokensContract = SimpleToken(
            erc20Contract
        );
        if (paymentTokensContract.balanceOf(msg.sender) < mintCost * quantity) revert NotEnoughTokens();
        if (_numberMinted(msg.sender) + quantity > WALLET_LIMIT)
            revert WalletLimitExceeded();
        if (_totalMinted() + quantity > maxSupply) revert SupplyExceeded();
       // if (!_verifySignature(signature, "PRESALE")) revert InvalidSignature();
        if (paymentTokensContract.allowance(msg.sender, address(this)) < mintCost * quantity) revert InvalidAllowance();
        (bool success) = paymentTokensContract.transferFrom(msg.sender, address(this), mintCost*quantity);
 ...
 //       _signer = newSigner;
  //  }
    ...
    /// @dev 0 = CLOSED, 1 = PRESALE, 2 = PUBLIC
    ...
    function setTeamClaim(
        ...
        if (members.length != quantities.length) revert ArrayLengthMismatch();
...
//        return _signer;
  //  }
    ...
    /// @dev No more than 1000 tokens, EVER!
    function setMaxSupply(uint256 amount) external onlyOwner {
        if (amount > 1000) revert InvalidTokenCap();
        maxSupply = amount;
    }

    /// @notice Function used to set a new `raffleSupply` value.
    /// @param amount Newly intended `raffleSupply` value.
    function setPresaleSupply(uint256 amount) external onlyOwner {
        if (amount > 900) revert InvalidTokenCap();
        preSaleSupply = amount;
    }

    function setErc20Token(address erc20Contract_) external onlyOwner {
           require(erc20Contract_ != address(0), "Invalid address");
    ...
    function withdraw(address to) external onlyOwner {
        ...
        uint256 balance = paymentTokensContract.balanceOf(address(this));
        if (balance == 0) revert WithdrawFailed();
        (bool success) = paymentTokensContract.transfer(to, balance);
        if (!success) revert paymentFailed();
    }
