...
contract SharkzSoulIDV1 is IScore, Initializable, UUPSUpgradeable, AdminableUpgradeable, EIP712WhitelistUpgradeable, ERC4973SoulContainerUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _linkBadgeContract(address _contract, uint256 _scoreMultiplier) internal virtual {
        ...
        if (_soulData.isImplementing(_contract, type(IScore).interfaceId)) {
            // copy the base score to avoid future access to external contract
            object.baseScore = uint80(_soulData.getBadgeBaseScore(_contract));
        } else {
            object.baseScore = 0;
        }
    ...
    function _tokenVotingScore(uint256 _tokenId) internal virtual view returns (uint256) {
        ...
        for (uint256 i = 0; i < badgeSettings.length; i++) {
            ...
            if (_soulData.isImplementing(badge.contractAddress, 0x80ac58cd)) {
                // for ERC721
                totalScore += badge.scoreMultiplier * badge.baseScore * _soulData.getERC721Balance(badge.contractAddress, _ownerOf(_tokenId));
            } else {
                // for Soul Badge
                totalScore += badge.scoreMultiplier * badge.baseScore * _soulData.getSoulBadgeBalanceForSoul(address(this), _tokenId, badge.contractAddress);
            }
        }
        ...
        onlyAdmin 
    {
        ...
        if (approved) {
            ...
            require(_soulData.isImplementing(_contract, 0x5b5e139f), "Target contract need to support ERC721Metadata");
        ...
        callerIsUser 
    {
        if (_publicMintMode == 1) {
            ...
            require(_soulData.getERC721Balance(_tokenContract, _msgSenderERC4973()) > 0, "Caller is not a target token owner");
        }
    ...
    function setTokenCustomName(uint256 _tokenId, string calldata _name) external virtual {
        ...
        require(_soulData.isValidTokenCustomName(_name), "Custom name contains invalid characters");
    ...
    function tokenURI(uint256 _tokenId) external virtual view override returns (string memory) {
        ...
        return _soulData.tokenURI(_tokenId, _metaName, _metaDesc, tokenBadgeTraits(_tokenId), _tokenVotingScore(_tokenId), _tokenCreationTime(_tokenId), tokenCustomNames[_tokenId]);
    }
...
contract AdminableUpgradeable is Initializable {
    ...
    function __Adminable_init_unchained() internal onlyInitializing {
        _createAdmin(_msgSenderAdminable());
    }

    modifier onlyAdmin() {
        require(_msgSenderAdminable() != address(0), "Adminable: caller is the zero address");

        bool found = false;
        for (uint256 i = 0; i < _admins.length; i++) {
            if (_msgSenderAdminable() == _admins[i]) {
                found = true;
            }
        }
    ...
    function transferAdmin(address to) public virtual onlyAdmin {
        ...
        _admins[_adminIndex(_msgSenderAdminable())] = to;
        emit AdminTransfer(_msgSenderAdminable(), to);
    }
    ...
    function renounceLastAdmin() public virtual onlyAdmin {
        ...
        delete _admins;
        emit AdminRemoved(_msgSenderAdminable());
    }
