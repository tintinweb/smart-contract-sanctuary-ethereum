...
contract IpsumOfLorem is Ownable {
    struct Ipsum {
        address creator;
        bool userflag;
        bool adminflag;
    ...
    function isCCOtoken(uint256 tokenId) internal pure returns(bool){
        return tokenId>=9900 && tokenId<=9999;

    }

    function addIpsum(address tokenContract, uint256 tokenId, bool userflag, string memory loreMetadataURI) 
    public onlyAllowedTokenContract(tokenContract) {
        address tokenOwner = IERC721(tokenContract).ownerOf(tokenId);
        require(tokenOwner == _msgSender() || isCCOtoken(tokenId), "caller is not the token owner or token is not CCO");
        tokenIpsum[tokenContract][tokenId].push(
            Ipsum(_msgSender(), userflag, true, loreMetadataURI)
            );
        emit IpsumAdded(
            tokenContract,
            tokenId,
            tokenIpsum[tokenContract][tokenId].length - 1
        );
    }

    function addIpsumWithSignature(bytes memory signature, address tokenContract, uint256 tokenId, uint256 ipsumIdx,  bool userflag, string memory loreMetadataURI
    ) public onlyAllowedTokenContract(tokenContract) {
        ...
        require(numIpsum(tokenContract, tokenId) == ipsumIdx,"Ipsumid is stale");
    ...
    function updateIpsumMetadataURI(
        ...
        address tokenOwner = IERC721(tokenContract).ownerOf(tokenId);
        require(tokenIpsum[tokenContract][tokenId][ipsumIdx].creator == _msgSender(), "caller is not the Ipsum creator");
        require(tokenOwner == _msgSender() || isCCOtoken(tokenId), "caller is not the token owner or token is not CCO");
    ...
    function updateIpsumUserflag(
        ...
        address tokenOwner = IERC721(tokenContract).ownerOf(tokenId);
        require(tokenOwner == _msgSender() || owner() == _msgSender(), "caller is neither the token owner nor the Ipsum Master");
        tokenIpsum[tokenContract][tokenId][ipsumIdx].userflag = newuserflag;
        emit IpsumUpdated(tokenContract, tokenId, ipsumIdx);
    }
    
    //onlyOwner
    function updateIpsumAdminflag(
        address tokenContract,
        uint256 tokenId,
        uint256 ipsumIdx,
        bool newadminflag
        ...
        tokenIpsum[tokenContract][tokenId][ipsumIdx].adminflag = newadminflag;
        emit IpsumUpdated(tokenContract, tokenId, ipsumIdx);
    }
