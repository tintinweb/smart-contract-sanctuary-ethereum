...
contract UniswapV2HelperFacet is IUniswapV2Callee {
  ...
  function _getAmountsOut (address router, address[] memory path, uint amountIn) internal view returns(uint[] memory amounts) {
    amounts = IUniswapV2Router02(router).getAmountsOut(amountIn, path);
  }
  function getAmountsOut (address router, address[] memory path, uint amountIn) external view returns(uint[] memory amounts) {
    amounts = _getAmountsOut(router, path, amountIn);
  }
  function _getAmountsIn (address router, address[] memory path, uint amountOut) internal view returns(uint[] memory amounts) {
    amounts = IUniswapV2Router02(router).getAmountsIn(amountOut, path);
  }
  function getAmountsIn (address router, address[] memory path, uint amountOut) external view returns(uint[] memory amounts) {
    amounts = _getAmountsIn(router, path, amountOut);
  }
