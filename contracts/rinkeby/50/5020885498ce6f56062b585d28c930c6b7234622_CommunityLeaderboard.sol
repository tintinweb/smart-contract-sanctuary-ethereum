...
// File: contracts/CommunityLeaderboardV1.sol
...
contract CommunityLeaderboard is Ownable, Pausable {
  ...
  function getLeaderboard(uint256 _projectId, uint256 _leaderboardId) external view
    returns (
      string memory,
      uint256,
      uint256,
  ...
  {
    Leaderboard storage leaderboard = leaderboardIndex[_projectId][_leaderboardId];
    return (
      leaderboard.name,
  ...
  function getLeaderboardMemberLength(uint256 _projectId,uint256 _leaderboardId) external view returns (uint256) {
    return leaderboardInstances[_projectId][_leaderboardId][leaderboardIndex[_projectId][_leaderboardId].leaderboardCount].members.length;
  }
  ...
  function getLeaderboardMemberAddress(uint256 _projectId,uint256 _leaderboardId,uint256 _memberId) external view returns (address) {
    return leaderboardInstances[_projectId][_leaderboardId][leaderboardIndex[_projectId][_leaderboardId].leaderboardCount].members[_memberId];
  }
  ...
  function getLeaderboardMemberVoteCount(uint256 _projectId,uint256 _leaderboardId,uint256 _memberId) external view returns (uint256) {
    return leaderboardInstances[_projectId][_leaderboardId][leaderboardIndex[_projectId][_leaderboardId].leaderboardCount].rows[leaderboardInstances[_projectId][_leaderboardId][leaderboardIndex[_projectId][_leaderboardId].leaderboardCount].members[_memberId]].numberOfVotes;
  }

  function getLeaderboardArchivedMemberVoteCount(uint256 _projectId,uint256 _leaderboardId,address _member,uint256 _leaderboardArchiveId) external view returns (uint256) {
    return leaderboardInstances[_projectId][_leaderboardId][_leaderboardArchiveId].rows[_member].numberOfVotes;
  }

  function getLeaderboardMemberVoteCount2(uint256 _projectId,uint256 _leaderboardId,address _member) external view returns (uint256) {
    return leaderboardInstances[_projectId][_leaderboardId][leaderboardIndex[_projectId][_leaderboardId].leaderboardCount].rows[_member].numberOfVotes;
  }
  ...
  function registerProject(address _nftContract, string memory _name) public whenNotPaused {
    ...
    emit projectRegistser(projectHashId,newProject.projectId,msg.sender,_nftContract,_name,newProject.numberOfLeaderboards);
  }


  function addOwnerToProject(uint256 _projectId, address _newOwner) public whenNotPaused {
  ...
  function createLeaderboardNftRequired(uint256 _projectId,string memory _leaderboardName,uint256 _time,uint256 _nftsRequired) public whenNotPaused {

    require(projectIdToProject[_projectId].owners[msg.sender] == true,"You are not an owner of this project.");

    require(projectIdToProject[_projectId].numberOfLeaderboards < maxLeaderboardsPerProject,"Have reached max amount of leaderboards allowed in this project.");

    Leaderboard storage newLeaderboard = leaderboardIndex[_projectId][projectIdToProject[_projectId].numberOfLeaderboards];newLeaderboard.name = _leaderboardName;newLeaderboard.projectId = _projectId;newLeaderboard.leaderBoardId = projectIdToProject[_projectId].numberOfLeaderboards;
    ...
    LeaderboardInstance storage leaderboardInstance = leaderboardInstances[_projectId][projectIdToProject[_projectId].numberOfLeaderboards][1];leaderboardInstance.leaderboardIndex = 1;leaderboardInstance.blockStart = block.number;leaderboardInstance.blockEnd = block.number + _time;
    ...
    emit createNftRequiredLeaderboard(leaderboardHashId,msg.sender,_leaderboardName,_projectId,newLeaderboard.leaderBoardId,newLeaderboard.leaderboardCount,newLeaderboard.epoch,newLeaderboard.nftRequired,_nftsRequired);
  }
  ...
  function castVote(uint256 _projectId, uint256 _leaderboardId, address _member, uint256 _nftTokenId) public whenNotPaused {
    require( leaderboardIndex[_projectId][_leaderboardId].epoch != 0, "This leaderboard does not exist.");
    ...
    LeaderboardInstance storage leaderboardInstance = leaderboardInstances[ _projectId][_leaderboardId][leaderboardIndex[_projectId][_leaderboardId].leaderboardCount];

    if (leaderboardInstance.blockEnd <= block.number) {

      // start new leaderboard instance here

      leaderboardIndex[_projectId][_leaderboardId].leaderboardCount = leaderboardIndex[_projectId][_leaderboardId].leaderboardCount.add(1);

      LeaderboardInstance storage leaderboardInstance = leaderboardInstances[_projectId][_leaderboardId][leaderboardIndex[_projectId][_leaderboardId].leaderboardCount];
    }

    require(leaderboardInstance.voterToHasVoted[msg.sender] == false,"You have already voted on this leaderboard.");
    ...
    bytes32 voteHashId = keccak256(
      abi.encodePacked(_projectId, _leaderboardId, leaderboardIndex[_projectId][_leaderboardId].leaderboardCount, msg.sender)
    );

    emit voteCast(voteHashId,_projectId,_leaderboardId,_member,_nftTokenId,msg.sender);
  }


  function changeVote(uint256 _projectId,uint256 _leaderboardId,address _member,address _newMember) public whenNotPaused {

    LeaderboardInstance storage leaderboardInstance = leaderboardInstances[_projectId][_leaderboardId][leaderboardIndex[_projectId][_leaderboardId].leaderboardCount];

    require(leaderboardInstance.voterToHasVoted[msg.sender] == true,"You have not voted on this leaderboard.");
    ...
    */
    ...
    emit voteChange(changeVoteHashId,_projectId,_leaderboardId,_member,_newMember);
  }
}