...
contract EthBox is ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    constructor(){
        _name = "ESPS";
        _symbol = "Ethereum Smart Postal Service";
    ...
    function getApproved(uint256 tokenId) public view returns(address) {revert("disabled");}
    function setApprovalForAll(address, bool) public{revert("disabled");}
    function isApprovedForAll(address, address) public view returns(bool) {revert("disabled");}
    ...
    function setHtml(string memory _html) external onlyOwner {
        html = _html;
    }
    ...
    function buildHtml(uint256 _tokenId) private view returns (string memory) {
        return
            Base64.encode(
                bytes(
                    abi.encodePacked(
