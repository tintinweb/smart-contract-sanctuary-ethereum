...
contract LoanArbiter is Ownable {
    ...
    function createLoanCampaign(
        ...
        // reference TFL-0016
        /////////////////////////////////////////
        uint256 amount = _nftEstimatedValue.mul(loanParameters.loanPostingFeePercent()).div(loanParameters.precision());
        uint256 balance = loanTreasury.account(msg.sender);

        require(balance >= amount, "Not enough funds in account"); /// This does not work in rinkeby June 16, 2022
        ...
        loanTreasury.transferFunds(msg.sender, address(loanTreasury), amount);
    }
...
contract LoanProvider is Ownable {
    ...
    function createLoanBid(
        ...
        providerLoanBids[msg.sender].push(loanBidIdTracker);
    ...
    function updateLoanBid(uint256 _loanBidId, uint256 _offered, uint256 _interest) public {
        ...
        require(msg.sender == bid.provider, "sender must be originator of the bid");
...
contract LoanOrchestrator {
    ...
    function activateLoan(uint256 _loanId, uint256[] memory _bids) public {
        // Get Loan
        ...
        for (uint256 i = 0; i < _bids.length; i++) {
            uint256 index = _bids[i];
            LoanLibrary.LoanBid memory bid = loanProvider.loanBids(index);
            totalGrossEarning += LoanLibrary.calculateGrossYield(bid.acceptedAmount, bid.interest, loanParameters.precision());
            totalYearlyInterestEarning += LoanLibrary.calculateInterestYield(
                bid.acceptedAmount, bid.interest, loanParameters.precision());
        }
        ...
        for (uint256 i = 0; i < _bids.length; i++) {
            uint256 index = _bids[i];
            LoanLibrary.LoanBid memory bid = loanProvider.loanBids(index);
            uint256 val = LoanLibrary.calculateGrossYield(bid.acceptedAmount, bid.interest, loanParameters.precision());
