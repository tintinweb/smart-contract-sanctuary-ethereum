...
contract CrowdFunding {
    ...
    */
    ...
    event record(address addr, uint256 amount);

    function checkFunder(uint256 campaignID, uint256 funderID) public returns(address, uint256)
    {
        Funder storage c = campaigns[campaignID].funders[funderID];
        emit record(c.addr, c.amount);
        return (c.addr, c.amount);
    }
}