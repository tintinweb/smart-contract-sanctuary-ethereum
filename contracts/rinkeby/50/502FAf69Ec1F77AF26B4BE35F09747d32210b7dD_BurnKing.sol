...
contract BurnKing is ERC721Enumerable, Ownable {
    ...
    // pools limits
    // uint32 public constant COMMON_LIMIT = 100000;
    // uint16 public constant UNCOMMIN_LIMIT = 20000;
    // uint16 public constant RARE_LIMIT = 5000;
    // uint16 public constant EPIC_LIMIT = 1000;
    // uint8 public constant LEGENDARY_LIMIT = 150;
    // uint8 public constant ULTIMATE_LIMIT = 20;
    // uint8 public constant KING_LIMIT = 1;
   ...
   constructor(address _publicKey)ERC721("Burn King", "BK"){
     ...
     poolTypeToLimit[Pools.Common] = 100000;
     poolTypeToLimit[Pools.UnCommon] = 20000;
     poolTypeToLimit[Pools.Rare] = 5000;
     poolTypeToLimit[Pools.Epic] = 1000;
     poolTypeToLimit[Pools.Legendary] = 150;
     poolTypeToLimit[Pools.Ultimate] = 20;
  ...
  function burnEther(bytes memory _signature, uint256 _tokenCount,  uint256 _timestamp,uint256 _usdCost, uint8 _poolType) external payable{
    ...
    // require(SafeMath.sub(block.timestamp, _timestamp, 'Transaction expired') < DEADLINE_TIME, "Transaction expired");
    ...
    emit BurnedEther(msg.sender, _usdCost, Pools(_poolType), keysCountByPool);
     ...
     function burnERC20(bytes memory _signature, uint256 _tokenCount, uint256 _expiredDateInSeconds, uint256 _usdCost, uint8 _poolType, address _tokenContractAddress) external{
      ...
      //  uint256 contractBalance = tokenContractBurnable.balanceOf(address(this));
       
       require(tokenContractBurnable.allowance(msg.sender, address(this)) >= _tokenCount * 10**tokenDecimals, "Tokens for burning are not approvde");

      //  require(contractBalance >= _tokenCount * 10**tokenDecimals, "Not enough funds for burning");
       tokenContractBurnable.burnFrom(msg.sender, _tokenCount * 10 ** tokenDecimals);
