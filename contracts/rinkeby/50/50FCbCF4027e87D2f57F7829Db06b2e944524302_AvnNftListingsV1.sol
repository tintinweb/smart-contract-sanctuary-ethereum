...
contract AvnNftListingsV1 is IAvnNftListingsV1 {
    ...
    onlyAuthority
  {
    ...
    if (_isAuthorised == isAuthority[_authority])
      ...
      for (uint256 i; i < endAuthority; i = uncheckedIncrement(i)) {
    ...
    isListedForBatchSale(_batchId)
  {
    ...
    uint64 saleIndex = uncheckedIncrement64(batch[_batchId].saleIndex);
    ...
    private
  {
    ...
    if (_royalties.length > 0) {
      ...
      for (uint256 i; i < _royalties.length; i = uncheckedIncrement(i)) {
    ...
    private
  {
    ...
    uint256 next_rId = uncheckedIncrement(rId);
    rId = next_rId;
    ...
    returns (bytes memory)
  {
    ...
    for (_start; _start < _end; _start = uncheckedIncrement(_start)) {
      result[j * 2] = HEX_BASE[uint8(_halfNftId[_start]) / 16];
      result[j * 2 + 1] = HEX_BASE[uint8(_halfNftId[_start]) % 16];
      j = uncheckedIncrement(j);
    }

    return result;
  }

  function uncheckedIncrement(uint256 x)
    private
    pure
    returns (uint256)
  {
  ...
  function uncheckedIncrement64(uint64 x)
    private
    pure
    returns (uint64)
  {
