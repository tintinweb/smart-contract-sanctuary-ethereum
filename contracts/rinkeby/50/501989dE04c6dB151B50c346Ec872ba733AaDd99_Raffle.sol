...
if(!(end == 172800 || end == 1800 || end == 86400 )) {
...
s_ID++;
uint m_ID = s_ID;
rafflesId[m_ID] = Raffles(m_ID, tickets, block.timestamp + end, end, State.Active, payable(0x0), 0, price, 0, msg.value, _nftAddress, payable(msg.sender), _nftId, _type); 
...
function getWaitingRaffles() public view returns(uint[] memory) {
    ...
    for(uint i = 0; i + s_LastId < s_ID; i++) {
        ...
        ||
        (rafflesId[i + s_LastId + 1].endTime <= block.timestamp) && (rafflesId[i + s_LastId + 1].funds > 0)) {
