...
contract Dugo is IDugo, ERC20, Ownable, ReentrancyGuard {
    ...
    function currentEpoch() public view returns (uint256) {
        return (block.timestamp - STARTED_AT) / HALVING_INTERVAL;
    }
    ...
    function claimable(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 epochEndsAt = STARTED_AT + HALVING_INTERVAL;
    ...
    function burn(address account, uint256 amount) public override {
        if (_msgSender() != address(ASWANG)) revert();
    ...
    /**
     * @notice Claim $DUGO for specified token
     * @param tokenId Token to claim $DUGO for
     */
    function claim(uint256 tokenId) external {
        _claim(_msgSender(), tokenId);
        _lastClaimedAt[tokenId] = block.timestamp;
    }
