...
// @author:      Dan Wale (@pixelSmart21)
// @website:     https://cyclenft.io/
...
contract CycleNFT is ERC721A, Ownable {

    using ECDSA for bytes32;
    using Strings for uint256;    

    uint256 public MAX_SUPPLY = 6000;
    uint256 public constant TEAM_RESERVED_SUPPLY = 100;

    uint256 public constant WL_B_MINT_PRICE = 0.05 ether;
    uint256 public constant WL_1_MINT_PER_WALLET = 2;

    uint256 public teamMintCount = 0;

    int256 public mintStage = 0; // 0 - not started | 1 - wl mint

    string public tokenBaseUrl =
        "http://mint.cycleft.io/cyclenft-metadata/";
    string public tokenUrlSuffix = ".json";

    // whitelist sign address
    address private _signerAddress;

    address COMPANY_WALLET = 0x18006a34D50C46754c405D1374e49B3c7D08E721;

    constructor(address signerAddress_) ERC721A("CycleNFT", "CYCLE") {
        _signerAddress = signerAddress_;
        console.log("Welcome to the Cycle NFT Smart Contract.");
    }
    ...
    function mint(uint256 numTokens, bytes calldata signature) 
        external 
        payable
        onlyOrigin
    ...
    function setMintStage(int256 _mintStage) external onlyOwner {
        require(_mintStage < 2, "Unsupported mint stage");
    ...
    modifier mintCompliance(uint256 _numTokens) {
        ...
        totalSupply() + _numTokens <
            MAX_SUPPLY - (TEAM_RESERVED_SUPPLY + 1),
        "Max supply exceeded!"
        );
        require(
        _numberMinted(msg.sender) + _numTokens < WL_1_MINT_PER_WALLET + 1,
