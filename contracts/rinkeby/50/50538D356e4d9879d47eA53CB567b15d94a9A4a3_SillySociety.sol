...
contract SillySociety is 
    ...
    uint256 public constant TOTAL_SUPPLY = 33; //pls insert correct supply
    ...
    constructor(bool _paused, 
     ...
     uint256 _presaleEnd,
     uint256 _presaleTwoStart,
     uint256 _presaleTwoEnd,
     ...
     PresaleTwo(_presaleTwoStart, _presaleTwoEnd)
    ...
    function getPrice() public view returns (uint256) {
        ...
        if(isPresaleTwoActivated()) { return WHITELIST_PRICE; }
    ...
    {
        require(!paused, "Paused!");
        require(_mintAmount == 1, "1 mint only!");
        require(addressData[msg.sender] + _mintAmount == 1, "Claimed");
        require(msg.value >= getPrice() * _mintAmount, "Insufficient Funds!");
        require(totalSupply() + _mintAmount <= TOTAL_SUPPLY, "Supply Maxed!");
    ...
    // WhitelistMint2
    function whitelistMintTwo(bytes32[] calldata _merkleProof, uint256 _mintAmount) 
        public 
        payable
        isPresaleTwoActive
        isYouAreWhitelisted(_merkleProof)
    ...
    {
        require(!paused, "Paused!");
        require(_mintAmount <= MINT , "Maxed Mint Exceeded!");
        require(addressData[msg.sender] + _mintAmount <= 3, "Can't have more than 3!");
        require(msg.value >= getPrice() * _mintAmount, "Insufficient Funds!");
        require(totalSupply() + _mintAmount <= TOTAL_SUPPLY, "Supply Maxed!");
        addressData[msg.sender]  += _mintAmount;
        _safeMint(msg.sender, _mintAmount);
...
contract PresaleTwo is Ownable, ReentrancyGuard {

    modifier isPresaleTwoActive() {
        require(isPresaleTwoActivated(), "Presale Not Active!");
        _;
    }

    uint256 public presaleTwoStart;
    uint256 public presaleTwoEnd;

     constructor(uint256 _presaleTwoStart, uint256 _presaleTwoEnd) {
        presaleTwoStart = _presaleTwoStart;
        presaleTwoEnd = _presaleTwoEnd;
    }

    function isPresaleTwoActivated() public view returns (bool) {
        return  presaleTwoStart > 0 &&
                presaleTwoEnd > 0 &&
                block.timestamp >= presaleTwoStart &&
                block.timestamp <= presaleTwoEnd;
    }

    function setTimeStampPresaleTwo(uint256 _presaleTwoStart, uint256 _presaleTwoEnd) external onlyOwner nonReentrant{
        presaleTwoStart = _presaleTwoStart;
        presaleTwoEnd = _presaleTwoEnd;
    }
