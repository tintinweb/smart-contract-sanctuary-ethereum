...
contract Trade {
    ...
    function getFees(uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId, bool _isImport) internal view  returns(Fee memory){
        address tokenCreator;
    ...
    function buyAsset(Order memory order,  bool _isImport, Sign memory sign) public returns(bool) {
        require(!usedNonce[sign.nonce],"Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        Fee memory fee = getFees(order.amount, order.nftType, order.nftAddress, order.tokenId, _isImport);
    ...
    function executeBid(Order memory order, bool _isImport, Sign memory sign) public returns(bool) {
        require(!usedNonce[sign.nonce],"Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        Fee memory fee = getFees(order.amount, order.nftType, order.nftAddress, order.tokenId, _isImport);
