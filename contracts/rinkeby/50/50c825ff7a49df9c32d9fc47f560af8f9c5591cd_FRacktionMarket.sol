...
// File @openzeppelin/contracts-upgradeable/utils/[email protected]v3.4.1-solc-0.7
    ...
    NFTMarketAuction
{
    ...
    struct ReserveAuction {
      ...
      struct Bid {
        address payable originalBidder;
        uint originalAmount;
    }
   ...
   Bid[] public AllBids;
    ...
    function placeBid(uint256 auctionId) public payable nonReentrant {
        ReserveAuction storage auction = auctionIdToAuction[auctionId];
        //Bid storage bid = auctionIdToAuction[auctionId];
        require(
            auction.amount != 0,
        ...
        if (auction.endTime == 0) {
            ...
            //     originalAmount
            // );
           AllBids.push(Bid(originalBidder,originalAmount));
        }
        ...
        //emit cancerefund(AllBids);
    }

     function CancelAuction(uint256 auctionId)public nonReentrant{
        ReserveAuction memory auction = auctionIdToAuction[auctionId];
         //Bid memory bid = auctionIdToAuction[auctionId];
        require(
            auction.amount > 0,
            "NFTMarketReserveAuction: Auction not found"
        );
        ...
        uint lastIndex = AllBids.length;

        for(uint i=0; i<lastIndex; i++){
        if (AllBids[i].originalBidder == address(this)) {
            AllBids[i].originalBidder.transfer(AllBids[i].originalAmount);
    ...
    /**
     * @notice Allows Foundation to cancel an auction, refunding the bidder 
     */
