...
contract NFTLauncher is VRFConsumerBase, ChainlinkClient, Ownable {
    ...
    function getPackPrice(
        ...
        uint256 paymentMethod,
        uint256 slippage
    ) public view returns (uint256 packPrice) {
        Projects storage project = projects[projectid];
        ...
        (,address basePaymentMethodAddress,,,,,) = seapadPayments.getPaymentMethod(basePaymentMethodID);
        require(valid);
        if (isBasePayment) {
            packPrice = packPrice + ((packPrice * slippage) / 10000);
        } else {
            (uint256[] memory result) = seapadPayments.getEstimatedTokenNeededToRecieveOtherToken(
                paymentMethodAddress,
                basePaymentMethodAddress,
                packPrice
            );
            packPrice = result[0] + ((result[0] * slippage) / 10000);
        }
        return packPrice;
    }
    ...
    function purchasePack(
        ...
        if (isBasePayment) {
            pricePerPackInBaseToken = project.pricePerPack; //100 DAI
        } else {
            //get how much tokens it would cost us to get the baseTokenAmount.
            //this is not the base payment method
        }
        ...
        uint256 tokensRemainingAfterFee = SeapadPayments(paymentAddress)
            .buyLinkToken(
                paymentMethodAddress,
                pricePerPackInBaseToken,
                fee,
    ...
    function createTokenPart2(
        ...
        // );
        require(msg.value >= PACK_LAUNCH_COST, "Please include pack cost price.");
...
contract NFT is ERC1155, Ownable {
        ...
        returns (string memory)
    {
        ...
        //         Redeemables _redeemables = Redeemables(redeemables);
        //        (,string memory ___uri,,) = _redeemables.isValidItem(_tokenId, address(this));
        //         return ___uri;
        //     }
        // }
        return
