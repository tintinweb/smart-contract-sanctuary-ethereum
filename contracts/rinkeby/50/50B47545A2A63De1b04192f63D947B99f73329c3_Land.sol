...
contract Land is ERC721, VRFConsumerBaseV2, ReentrancyGuard, Ownable, Pausable {
    ...
    struct Token {
        uint256[] tokenIds;
        mapping(uint256 => uint256) requestIdToTokenIdIndex;
    }
    ...
    function mint(bytes32[] memory _merkleProof)
        ...
        isValidAmount
    {
        requestRandomTokenId(1);
    }

    function requestRandomTokenId(uint32 _numOfRandomTokenIdToGenerate)
        internal
    {
        uint256 requestId = VRFCOORDINATOR.requestRandomWords(
            ...
            vrfCallbackGasLimit,
            _numOfRandomTokenIdToGenerate
        );

        address minter = msg.sender;

        requestIdToAddress[requestId] = minter;

        addressToToken[minter].tokenIds.push(0);
        uint256 tokenIdIndex = (addressToToken[minter].tokenIds.length - 1);
        addressToToken[minter].requestIdToTokenIdIndex[
            requestId
        ] = tokenIdIndex;
    }
    ...
    function fulfillRandomWords(
        ...
        // Randomize value within the range of 1 -> MAX_SUPPLY
        uint256 randomTokenId = ((_randomNumbers[0] % MAX_SUPPLY) + 1);

        address minter = requestIdToAddress[_requestId];

        uint256 tokenIdIndexToUpdate = addressToToken[minter]
            .requestIdToTokenIdIndex[_requestId];
        addressToToken[minter].tokenIds[tokenIdIndexToUpdate] = randomTokenId;

        totalSupply = totalSupply + 1;
        _safeMint(requestIdToAddress[_requestId], randomTokenId);
    }

    function getMintingStatus(address _address)
        external
        view
        returns (uint256[] memory)
    {
        return addressToToken[_address].tokenIds;
    }
