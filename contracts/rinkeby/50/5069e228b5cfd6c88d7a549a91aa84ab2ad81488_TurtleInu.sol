...
contract TurtleInu is ERC20, Ownable {
    ...
    constructor() payable ERC20("Turtle Inu", "TINU") {
        ...
        uint256 totalSupply = 10 * 1e9 * 1e18; // 10 billion

        // Reference: 1 ETH = 0.23% at launch
        maxBuyAmount = (totalSupply * 15) / 10000; // 0.15%
        maxSellAmount = (totalSupply * 5) / 10000; // 0.05%
        ...
        originalSellOperationsFee = 9;
        originalSellLiquidityFee = 3;
        ...
        operationsAddress = address(0x2c3d4293F04EAFa141eecA4C066d11bc575b03cc);
        treasuryAddress = address(0x0a0BEDF9605697332670bE2B6B18C2efEE69e88a);
        ...
        _excludeFromMaxTransaction(0xb820733b574Df5173ACF9688facD32BBb4dc2D43, true); // Team tokens wallet
        _excludeFromMaxTransaction(0xf2eaFea68dC662De7e3Ce237c59a6d2515E0322e, true); // Exchange tokens wallet
        _excludeFromMaxTransaction(0x87E64a130Fffe1C784cbe9308B3AD220Af07cc5E, true); // Shill competitions tokens wallet
        ...
        excludeFromFees(0xb820733b574Df5173ACF9688facD32BBb4dc2D43, true); // Team tokens wallet
        excludeFromFees(0xf2eaFea68dC662De7e3Ce237c59a6d2515E0322e, true); // Exchange tokens wallet 
        excludeFromFees(0x87E64a130Fffe1C784cbe9308B3AD220Af07cc5E, true); // Shill competitions tokens wallet

         _createInitialSupply(newOwner, (totalSupply * 36) / 100); // 36% Tokens available for aidrop to old holders
         _createInitialSupply(address(0xdead), (totalSupply * 591) / 1000); // 59.1% Initial Burn
         _createInitialSupply(address(this), (totalSupply * 49) / 1000); // 4.9% Tokens for liquidity
  ...
  function updateMaxBuyAmount(uint256 newNum) external onlyOwner {
        ...
        if (limitsInEffect) {
            if (
                ...
                //when sell
                    ...
                    require(nextInvestorSellDate[from] <= block.timestamp, "Cannot sell yet");
                    nextInvestorSellDate[from] = block.timestamp + timeBetweenBuys;
                } 
                else if (!_isExcludedMaxTransactionAmount[to] && !_isExcludedFromFees[to]) {
                    revert("Investors cannot transfer and must sell only or transfer to a whitelisted address.");
                }
            }
        }
    ...
    function swapBack() private {
        ...
        if(tokensForTreasury > 0 && balanceOf(address(this)) >= tokensForTreasury) {	
            super._transfer(address(this), address(treasuryAddress), tokensForTreasury);	
        }	
        tokensForTreasury = 0;
    ...
    function launch(
        ...
        for (uint256 i = 0; i < wallets.length; i++) {
            address wallet = wallets[i];
            nextInvestorSellDate[wallet] = block.timestamp + privateSaleCooldown; // No sales in the first 3 hours after launch
            uint256 amount = amountsInTokens[i];
            super._transfer(msg.sender, wallet, amount);
        }
