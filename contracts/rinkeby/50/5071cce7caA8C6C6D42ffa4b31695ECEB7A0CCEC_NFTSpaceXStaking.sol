...
contract NFTSpaceXStaking is ERC1155Holder, IShares, INFTSpaceXStaking, Ownable, ReentrancyGuard, Pausable {
  ...
  uint16[6] public TICKET_CARD = [1, 39, 219, 1089, 5499, 31299];
  ...
  function canStake(StakingOption option, uint256 tokenId, uint256 amount) public view override returns (bool) {
    return (
      balanceOfTicketCard(nftspacexTicketCard.currentIdSupported()) >= estimatedTC + _calculateTicketCard(option, tokenId, amount) &&
      currentBalanceNST() >= estimatedReward + _calculateReward(option, tokenId, amount)
    );
  }

  function canHarvest(address user, uint256 slotIndex) public view override returns (bool) {
    require(slotIndex < numberStaking[user], "NSS: NE");

    StakingInfo memory stakingInfo = userStakings[user][slotIndex];
    if (stakingInfo.harvest) {
      return false;
    }

    if (stakingInfo.option == StakingOption.basic) {
      if ((stakingInfo.startTime + (30 * 86400)) <= block.timestamp) {
        return true;
      }
    } else if (stakingInfo.option == StakingOption.premium) {
      if ((stakingInfo.startTime + (60 * 86400)) <= block.timestamp) {
        return true;
      }
    } else if (stakingInfo.option == StakingOption.pro) {
      if ((stakingInfo.startTime + (90 * 86400)) <= block.timestamp) {
        return true;
      }
    }
  ...
  function rewardAfterHarvest(StakingOption option, uint256 tokenId, uint256 amount) public view override returns (uint256) {
    return _calculateReward(option, tokenId, amount);
  }

  function ticketCardAfterHarvest(StakingOption option, uint256 tokenId, uint256 amount) public view override returns (uint256) {
    return _calculateTicketCard(option, tokenId, amount);
  }
  ...
  function harvest(uint256 slotIndex) public override whenNotPaused {
    ...
    require(canHarvest(_msgSender(), slotIndex), "NSS: EARLY");
  ...
  function withdraw(uint256 slotIndex) public override whenNotPaused {
    require(slotStaking[_msgSender()].contains(slotIndex), "NSS: NE");
    require(!canHarvest(_msgSender(), slotIndex), "NSS: SF");
  ...
  function _transferNFTAsset(address _from, address _to, uint256 _tokenId, uint256 _amount) internal {
    require(nftspacexAssetERC1155.balanceOf(_from, _tokenId) >= _amount, "NSS: IF");
  ...
  function _calculateReward(StakingOption _option, uint256 _tokenId, uint256 _amount) internal view returns (uint256 _reward) {
    ...
    } else if (_option == StakingOption.pro) {
