...
contract BearStaking is Ownable, IERC721Receiver{
  ...
  function getClaimableToken(uint256 tokenId) external view returns(uint256){
    uint256 reward = 0;
    if(stakedBear[tokenId].isEmergencyOut)
        reward += calculateReward(stakedBear[tokenId].lastClaimed, stakedBear[tokenId].emergencyOutTime);
    else
        reward += calculateReward(stakedBear[tokenId].lastClaimed, block.timestamp);
    return reward;
  }
