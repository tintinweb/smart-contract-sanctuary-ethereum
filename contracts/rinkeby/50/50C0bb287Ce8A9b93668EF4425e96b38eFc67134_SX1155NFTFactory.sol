...
contract SX1155NFTFactory is Ownable {
    event SX1155NFTDeployed(address deployedAt);
    ...
    function deploy(
        ...
        SX1155NFT newContract = new SX1155NFT(_name, _symbol, _issuer, _editor);
        emit SX1155NFTDeployed(address(newContract));
    }
}
...
/// @title SX11550NFT
/// @notice SX1155 NFT
contract SX1155NFT is ERC1155, RoleManager, AuthorizeManager {
    /// SX1155NFT KYA
    string public kya;
    ...
    constructor(
        ...
        address _issuer,
        address _editor
    ) ERC1155("") RoleManager(_issuer, _editor) {
    ...
    /// @notice returns if this contract supports the given EIP165 interface.
    /// This contracts supports EIP1155
    /// @param _interfaceId EIP165 interfaceID
    /// @return bool true or false
    function supportsInterface(bytes4 _interfaceId) public view override(ERC1155, RoleManager) returns (bool) {
    ...
    /// @notice sets KYA of a tokenId. Only EDITOR can do this
    /// @param _id token id
    /// @param _kya KYA string
    function setTokenKya(uint256 _id, string calldata _kya) external onlyEditor {
        require(tokenSupply[_id] > 0, "SX1155: Token doesn't exist");
        tokenKya[_id] = _kya;
    ...
    /// @notice sets URI of a token id, allowed to set only once.
    /// @param _id token id
    /// @param _token_uri URI string
    function setTokenURI(uint256 _id, string calldata _token_uri) external onlyIssuer {
        require(tokenSupply[_id] > 0, "SX1155: Token doesn't exit");
        require(bytes(tokenURIs[_id]).length == 0, "SX1155: URI already set");
        tokenURIs[_id] = _token_uri;
        emit URI(_token_uri, _id);
    }
    ...
    function mintBatch(
        ...
        require(_ids.length == _quantities.length, "SX1155: Mismatched array lengths");
    ...
    function burnBatch(uint256[] calldata _ids, uint256[] calldata _quantities) external {
    ...
    /// token id. Only ISSUER can do this.
    /// @param _id token id
    ...
    /// @notice called internally before each token transfer. This depends on authorization contract
    /// to decide whether to allow or deny the transaction.
    function _beforeTokenTransfer(
        ...
        if (to != address(0)) {
            ...
            for (i = 0; i < ids.length; i++) {
                require(mustBeAuthorizedHolder(from, to, ids[i], data), "SX1155: Not authorized to");
            }
        }
    }
}
...
contract RoleManager is AccessControl {
    ...
    /// @param account address
    /// @return true or false
    function hasEditorRole(address account) external view returns (bool) {
...
contract AuthorizeManager {
    ...
    /// @param id token id
    /// @param authContract authorization contact address
    event TokenAuthContractSet(address from, uint256 id, address authContract);
    ...
    function mustBeAuthorizedHolder(
        ...
        // return true if no auth contract set
        if (authContract == address(0)) return true;

        // 'from' address validation
        try IAuthorizationContract(authContract).isAccountAuthorized(_from) returns (bool response) {
        ...
        try IAuthorizationContract(authContract).isAccountAuthorized(_to) returns (bool response) {
            return response;
        } catch Error(string memory) {
            // Call rejected/reverted
            return false;
