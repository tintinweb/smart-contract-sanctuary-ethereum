             ...
             if(!isStart)return(0,0,0);
                   ...
                   if(Investors[_addr].lastVestedTime == 0) {
                       if(Investors[_addr].investorType == 3){
                           if(block.timestamp>=teamVestingEndDate)return(Investors[_addr].reminingUnitsToVest*Investors[_addr].tokensPerUnit,0,0);
                           if(block.timestamp<lockDateteam) return(0,0,0);
                           if(lockDateteam + day> 0)return (((block.timestamp-teamLockEndDate)/day) *Investors[_addr].tokensPerUnit,0,0);//, "Active lockdate was zero");
                                timeDifference = (block.timestamp) -(lockDateteam);}
                     else if(Investors[_addr].investorType == 5){
                           if(block.timestamp>=marketdevelopmentVestingEndDate)return(Investors[_addr].reminingUnitsToVest*Investors[_addr].tokensPerUnit,0,0);
                           if(block.timestamp<lockDatemarket) return(0,0,0);
                           if(lockDatemarket + day> 0)return (((block.timestamp-marketdevelopmentLockEndDate)/day) *Investors[_addr].tokensPerUnit,0,0);
                           }
                     else if(Investors[_addr].investorType == 1){
                           if(block.timestamp>=seedVestingEndDate)return(Investors[_addr].reminingUnitsToVest*Investors[_addr].tokensPerUnit,0,0);
                           if(block.timestamp<seedLockEndDate) return(0,0,0);
                           if(lockDatemarket + day> 0)return (((block.timestamp-seedLockEndDate)/day) *Investors[_addr].tokensPerUnit,0,0);
                     }
                     else if(Investors[_addr].investorType == 2){
                          if(block.timestamp>=privateVestingEndDate)return(Investors[_addr].reminingUnitsToVest*Investors[_addr].tokensPerUnit,0,0);
                           if(block.timestamp<privateLockEndDate) return(0,0,0);
                           if(lockDatemarket + day> 0)return (((block.timestamp-privateLockEndDate)/day) *Investors[_addr].tokensPerUnit,0,0);
                     }else if(Investors[_addr].investorType == 4){
                           if(block.timestamp>=launchpadVestingEndDate)return(Investors[_addr].reminingUnitsToVest*Investors[_addr].tokensPerUnit,0,0);
                           if(block.timestamp<launchpadLockEndDate) return(0,0,0);
                           if(lockDatemarket + day> 0)return (((block.timestamp-launchpadLockEndDate)/day) *Investors[_addr].tokensPerUnit,0,0);
                     }else if(Investors[_addr].investorType == 6){
                           if(block.timestamp>=airdropcampaignVestingEndDate)return(Investors[_addr].reminingUnitsToVest*Investors[_addr].tokensPerUnit,0,0);
                           if(block.timestamp<airdropcampaignLockEndDate) return(0,0,0);
                           if(lockDatemarket + day> 0)return (((block.timestamp-airdropcampaignLockEndDate)/day) *Investors[_addr].tokensPerUnit,0,0);
                     }
            }
            else{ 
