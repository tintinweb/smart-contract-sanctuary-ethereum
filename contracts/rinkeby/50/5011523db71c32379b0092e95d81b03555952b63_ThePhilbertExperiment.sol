...
contract ThePhilbertExperiment is ERC721A, ERC721AQueryable, Ownable {
  ...
  uint256 constant MAX_PER_TRANSACTION_PLUS_ONE = 6;
  ...
  constructor(
    string memory _uri
  ) ERC721A("PhilbertExperiment", "PBXP")  {
    seturi(_uri);
  }
  ...
  function mint(uint256 _quantity) external payable {
    ...
    require(_quantity < MAX_PER_TRANSACTION_PLUS_ONE, "Exceeds Max Per Txn");

    // Free Mints
    uint256 payForCount = _quantity;
    uint256 freeMintCount = _freeMintedCount[msg.sender];
    uint256 freeMintsLeft = 2 - freeMintCount;

    if (freeMintsLeft < 2) {
      if (_quantity > freeMintsLeft) {
        payForCount = _quantity - freeMintsLeft;
      } else {
        payForCount = 0;
      }

      if(_quantity == 1) {
        _freeMintedCount[msg.sender] = _freeMintedCount[msg.sender] + 1;
      }
      else 
      {
        _freeMintedCount[msg.sender] = _freeMintedCount[msg.sender] + 2;
      }
    }
