...
contract Vendor is Ownable {
  ...
  function withdraw() public onlyOwner {
    ...
    require(vendorBalance > 0, "Vendor does not have enough ETH");
  ...
  function sellTokens(uint256 amount) public {
    require(amount > 0, "Amount to sell must be more than 0");
    //validate if seller has tokens to sell
    address seller = msg.sender;
    uint256 sellerBalance = yourToken.balanceOf(seller);
    require(sellerBalance >= amount, "Seller does not have enough tokens");

    //validate if vendor has ETH
    uint256 vendorBalance = address(this).balance;
    uint256 amountOfEth = amount / tokensPerEth;
    require(vendorBalance >= amountOfEth, "Vendor does not have enough eth");

    //transfer tokens
    address vendorAddress = address(this);
    (bool sent) = yourToken.transferFrom(seller, vendorAddress, amount);
    require(sent, "Fail to send tokens to vendor");

    //tranfer eth
    (bool sentEth, ) = seller.call{value: amountOfEth}("");
    require(sentEth, "Fail to send eth to seller");
