...
contract RoleToken is IERC721Metadata, Ownable, ERC721URIStorage {
  address _owner = msg.sender;
  uint256 _uniqueRoleID=0;
  ...
  struct RoleTokens{
    ...
    uint _uniqueRoleID;
    string _MainSkill;
    string _SecondarySkill;
    string _SocietyRights;
  }

  mapping(uint => RoleTokens) public TMWRoleToken; 




  function GenerateRoleToken(address _RoleOwner, string memory _MainSkill, string memory _SecondarySkill, string memory _SocietyRights) external  onlyOwner {
    _mint(msg.sender, _uniqueRoleID);
    TMWRoleToken[_uniqueRoleID]=RoleTokens(_RoleOwner,_uniqueRoleID, _MainSkill, _SecondarySkill, _SocietyRights);
    _uniqueRoleID++;
     _safeTransfer(_RoleOwner, msg.sender, _uniqueRoleID, "");
  }
