...
contract BellyRecipes is AccessControl, IRecipes, Pausable {
  ...
  function addRecipe(
    ...
    _recipes[recipeId].resultTokenId = resultTokenId;

    emit RecipeAdded(recipeId, resultTokenId);
  }
  ...
  {
    return (
      _recipes[recipeId].tokenIds,
      _recipes[recipeId].amounts,
      _recipes[recipeId].resultTokenId
    );
  }
...
error RecipeIdAlreadyExists();
error RecipeDoesNotExist();
error EmptyInputs();
error MismatchInputs();
