...
contract ApexHumanity is Ownable, ERC721Enumerable, ERC2981, AccessControlEnumerable, ERC721URIStorage {
    ...
    event Locked(uint16 tokenId, bool isLocked);
    ...
    constructor(
        ...
        _setupRole(OWNER_MARKETPLACE_ROLE, _marketplace);
        setBaseURI(baseURI);
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenURI) external onlyRole(OWNER_MARKETPLACE_ROLE) {
    ...
    /// note that if you set customTokenURI with "", customTokenURI will be set to tokenId. I do not know why

    function mint(address to) external onlyRole(OWNER_MARKETPLACE_ROLE) returns (uint16) {
    ...
    /// @notice Lock or unlock the token. If the tokenId is locked - transfer is not available
    /// @param tokenId Id of ERC721 token
    /// @param _locked true to lock, false to unlock

    function setLocked(uint16 tokenId, bool _locked) external onlyOwner {
        require(_exists(tokenId), "Token is not exist");
        locked[tokenId] = _locked;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlEnumerable, ERC721, ERC2981, ERC721Enumerable) returns (bool) {
