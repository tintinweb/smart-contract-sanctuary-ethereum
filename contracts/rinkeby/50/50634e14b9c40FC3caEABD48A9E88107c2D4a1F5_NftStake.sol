...
contract NftStake is ERC721A, ERC721Holder, Ownable {
    IERC721A public nftBoy;
    IERC721A public nftGirl;
    uint8 public typeNftBoy = 1;
    uint8 public typeNftGirl = 2;
    uint8 public maxUserStake = 5;
    uint8 public updateStakeDays = 90;
    ...
    constructor(
        ...
        nftBoy = IERC721A(nft_);
        _wallet = wallet_;
    }

    function setNftGirl(address nft_) public onlyOwner {
        nftGirl = IERC721A(nft_);
        maxUserStake = 10;
    }
    ...
    function stake (uint256 tokenId, uint256 _type, uint256 _days) external {
        ...
        if (_type == typeNftBoy) {
            nftBoy.safeTransferFrom(msg.sender, address(this), tokenId);
            isConfirm = true;
        } else if (_type == typeNftGirl && nftGirl != IERC721A(address(0))) {
            nftGirl.safeTransferFrom(msg.sender, address(this), tokenId);
            isConfirm = true;
        }
    ...
    function unstake (uint64 tokenId, uint8 _type) external {
        ...
        if (_type == typeNftBoy) {
            nftBoy.safeTransferFrom(address(this), msg.sender, tokenId);
        } else if (_type == typeNftGirl && nftGirl != IERC721A(address(0))) {
            nftGirl.safeTransferFrom(address(this), msg.sender, tokenId);
        }
    ...
    function _nftGirl() public view returns (address) {
        return address(nftGirl);
    }
