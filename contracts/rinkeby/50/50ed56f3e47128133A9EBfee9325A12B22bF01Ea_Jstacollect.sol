...
contract Jstacollect is ERC721, Ownable, MerkleProof {

    uint256 public mintPrice = 0.06 ether;
    uint256 private maxToken = 1000;
    ...
    constructor () ERC721 ("Jstacollect","JAC") {}
    ...
    function whitelistMint(bytes32[] memory proof_) external payable onlySender wlSale {
        require(isWhitelisted(msg.sender, proof_), "You are not whitelisted!");
        require(wlMinted[msg.sender] == 0, "You have no WL mints remaining!");
    ...
    function publicMint(uint256 _amount) public payable onlySender plSale {
        require(_amount <= 2, "more than max per transaction");
        require(wlMinted[msg.sender] <= 2, "You have no mints remaining!");
    ...
    function setWhitelist(bool bool_, uint256 set_) external onlyOwner {
        whitelistSaleEnabled = bool_; whitelistSale = set_; 
    }

    function setPublic(bool bool_, uint256 set_) external onlyOwner {
        publicSaleEnabled = bool_; publicSale = set_; 
    }

    function whitelistSaleIsEnabled() public view returns (bool) {
        return (whitelistSaleEnabled && block.timestamp >= whitelistSale); 
    }

    function publicSaleIsEnabled() public view returns (bool) {
        return (publicSaleEnabled && block.timestamp >= publicSale); 
    }
    ...
    modifier wlSale {
        require(whitelistSaleEnabled && block.timestamp >= whitelistSale, "whitelistMint Paused"); _; 
    }

    modifier plSale {
        require(publicSaleEnabled && block.timestamp >= publicSale, "publicMint Paused"); _; 
    }
