...
///@title RealEstateERC1155Token
///@author Jeisson Niño
///@notice Technical Test for deviants factions
contract RealEstateERC1155Token is ERC1155,  Ownable {
    ///===================================================
    ///============= Custom errors =======================

    error WrongAmountUsd(address user, uint256 idTiket, uint256 amountTiket, uint256 amountUSD);
    ...
    /// address => idToken => catidad del tiket comprada
    mapping(address => mapping(uint256 => uint256)) public amountTiketsByUser;


    //cantidad de participacion vendida por franquicia
    //franquicia => tiket => catidad
    mapping(uint256 => mapping(uint256 => uint256)) public currentAmountStakeSaleByFranchise;
    ...
    constructor() ERC1155("") {
        USD = IERC20(0x9d71eDbE4983381ff6FB46c617AAB751c8f908Ad);
        uint8 idFranchiseInit = 1;
        uint8 idTiketInit = 1;
        uint256 balanceToken1Init = 72000;
        uint256 maxStakeToken1Init = 120;
        uint256 amountStakeTiket1Init = balanceToken1Init / maxStakeToken1Init;
        uint256 valueTiket1UsdInit = 4500e6;
    ...
    modifier onlyProfileChangesBalances() {
        require(profileChangesBalances[msg.sender] || msg.sender == owner(),
    ...
    function buyTiket(
        uint256 _idFranchise,
        uint256 _idTiket,
        uint256 _tiketAmount, //catidad de tickets
        uint256 _amountUSD) external {

        uint256 amountUsddByTiket = tikets[_idTiket].amountUsd;
        ...
        //el tiket esta activo?
        if (!tikets[_idTiket].activate) {
            revert TikeDoesNotActivate(msg.sender, _idFranchise, _idTiket);
        }

        //no hay para vender de ese tiket
        uint256 currentSaleByFranchise = currentAmountStakeSaleByFranchise[_idFranchise][_idTiket];
        if(_tiketAmount > tikets[_idTiket].limitByFranchise - currentSaleByFranchise){
            revert LimitByFranchise(msg.sender, _idFranchise, _idTiket);
        }

        //limite de tiket por usuario
        if(_tiketAmount > (tikets[_idTiket].limitByUser - amountTiketsByUser[msg.sender][_idTiket]) ){
           revert LimitByTiket(msg.sender, _idTiket, _tiketAmount);
        }

        //cantidad usd envida erronea
        if(_amountUSD != amountUsddByTiket * _tiketAmount ){
            revert WrongAmountUsd(msg.sender, _idTiket, _tiketAmount, _amountUSD);
        }

        //tranferir usd al owner
        if (!USD.transferFrom(msg.sender, owner(), _amountUSD)) {
            revert ErrorSendingFunds(msg.sender, _amountUSD);
        }

        emit BuyTiket(msg.sender, _idFranchise, _idTiket, _tiketAmount);

        //hacer el registro de la cantidad de tiket comprados
        currentAmountStakeSaleByFranchise[_idFranchise][_idTiket] += _tiketAmount;
    ...
    function tranferTikets(
        ...
        require(fromBalance >= tiketAmount, "ERC1155: insufficient balance for transfer");
