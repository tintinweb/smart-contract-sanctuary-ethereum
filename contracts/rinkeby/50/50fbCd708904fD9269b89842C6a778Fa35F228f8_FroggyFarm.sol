...
contract FroggyFarm is ERC721, Pausable, ERC721Burnable, NFTArt {
    ...
    /* function formatSVG (uint256 tokenId) public view returns (string memory){
        string memory tokenScript = string(abi.encodePacked(SVGinfoTop,jsScriptTop,toHexA(tokenId),jsScriptBot,SVGinfoBot));
        string memory baseURL = "data:image/svg+xml;base64,";
        string memory scriptBase64Encoded = Base64.encode(bytes(string(abi.encodePacked(tokenScript))));
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory _tokenURI = formatTokenURI(renderSvg(tokenIdToAssets[tokenId]));
...
function startMint(address to, uint96 packedAssets) public {
        ...
        tokenIdToAssets[tokenId];
        AssetsToTokenId[packedAssets];
...
contract NFTArt {
    string public constant SVG_HEADER = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" version="1.2" viewBox="0 0 24 24">';
...
bytes memory encoding,
    ...
    function renderURI(uint96 packed)public view returns (string memory){
        return formatTokenURI(renderSvg(packed));
    }
