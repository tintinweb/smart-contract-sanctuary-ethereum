...
contract PreBook is Ownable, ReentrancyGuard {
  ...
  mapping(uint256 => IPrebook) public contractAddr;
  ...
  function getPresaleState(uint256 _id) public view returns (bool) {
    return contractAddr[_id].getPresaleState();
  }
  ...
  function mintWL(uint256 _id) external passHolder nonReentrant {
    ...
    //Check presale state
    require(contractAddr[_id].getPresaleState(), "Presale OFF");

    //Mint the NFT and send the amt for minting..
    uint256 amt = project[_id].cost*cnt;
    contractAddr[_id].PBmint{value: amt}(msg.sender, cnt);
    delete amt;
  ...
  function directMint(uint256 _id, uint16 _cnt) external passHolder payable {
    ...
    contractAddr[_id].PBmint{value: amt}(msg.sender, _cnt);
    delete amt;
