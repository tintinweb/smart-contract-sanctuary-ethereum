    ...
    UUPSUpgradeable
{
    ...
    function stake(uint256 _catId, uint256 _boostType) external nonReentrant {
        require(
        ...
        checkinBooks[_catId] = CatRoom(
            ...
            _catId,
            booster.boostRate,
            block.timestamp + booster.boostTime
        );
    ...
    function unstake(uint256 _catId) external nonReentrant {
        ...
        emit Unstaked(msg.sender, _catId, room.unlockTime);
    }
        ...
        onlyRole(DAO_MANAGER_ROLE)
    {
        ...
        delete checkinBooks[room.catId];

        lasercat.safeTransferFrom(address(this), room.owner, room.catId);

        emit EmergencyWithdraw(room.owner, room.catId);
    }
    ...
    function setExtraCooldown(uint256 _catId, uint256 _cooldown)
        public
        onlyRole(DAO_MANAGER_ROLE)
    {
        CatRoom storage room = checkinBooks[_catId];
        room.unlockTime += _cooldown;
    }
