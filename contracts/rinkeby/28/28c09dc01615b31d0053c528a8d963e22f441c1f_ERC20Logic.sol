...
abstract contract TokenEventManagerStorage {
    bytes32 internal constant EVENT_MANAGER_DATA_SLOT =
        keccak256("token.transferdata.events");
    ...
    function eventManagerData()
        internal
        pure
        returns (EventManagerData storage ds)
    {
    ...
    function _on(
        ...
        require(
            !emd.listeningCache[callbackAddress][eventId].listening,
            "Address already listening for event"
        );

        uint256 eventIndex = emd.listeners[eventId].length;

        emd.listeners[eventId].push(
            SavedCallbackFunction(callbackAddress, callbackSelector)
        );

        ExtensionListeningCache storage elc = emd.listeningCache[
            callbackAddress
        ][eventId];
        elc.listening = true;
    ...
    function _clearListeners(address extension) internal {
        ...
        for (uint256 i = 0; i < eventIds.length; i++) {
            ...
            uint256 callbackIndex = emd
            .listeningCache[extension][eventId].listenIndex;
            ...
            // an 'if' statement
            SavedCallbackFunction storage lastCallback = emd.listeners[eventId][
                lastCallbackIndex
            ];

            emd.listeners[eventId][callbackIndex] = lastCallback;
            emd
            .listeningCache[lastCallback.callbackAddress][eventId]
                .listenIndex = callbackIndex;
