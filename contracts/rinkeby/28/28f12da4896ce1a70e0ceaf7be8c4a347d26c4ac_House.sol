...
interface IDiceWarNft {
    function getLv1List () external  returns(address[] memory _addresses);
    function getLv2List () external  returns(address[] memory _addresses);
    function getLv3List () external  returns(address[] memory _addresses);
    function getLv4List () external  returns(address[] memory _addresses);
}
...
contract House is ReentrancyGuard, Ownable {
    ...
    address public dicewarAddress = 0x6Fdba1A4f28D7077Cfd3080f70C13020BC5B5865;
    ...
    function placeBet(address player, address refAddr, uint amount, bool isBonus, uint[] memory amountArray) payable external isHouseLive admin nonReentrant {
        ...
        if(refAddr != address(0)){
            payable(refAddr).transfer(amountArray[5]);
        }else{
    ...
    function distributeNftHoldersRewards() external onlyOwner {
        ...
        address[] memory lv1Addr = dicewarNft.getLv1List();
        address[] memory lv2Addr = dicewarNft.getLv2List();
        address[] memory lv3Addr = dicewarNft.getLv3List();
        address[] memory lv4Addr = dicewarNft.getLv4List();
