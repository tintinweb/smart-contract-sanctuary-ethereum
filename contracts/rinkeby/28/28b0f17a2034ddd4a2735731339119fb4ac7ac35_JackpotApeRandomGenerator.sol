...
contract JackpotApeRandomGenerator is VRFConsumerBaseV2, IJackpotApeRandomGenerator {
  ...
  mapping(uint256 => bool) private checkDuplicated;
  ...
  function fulfillRandomWords(
    ...
    s_randomWord = 1000000000000;
    uint256 randomWord = randomWords[0];
    uint256[] memory lottoNumbers = new uint256[](6);
    count = 0;

    for (uint32 i = 0; i <= 34; i ++) {
      uint256 lottoNumber = randomWord % 100 % 49 + 1;
      randomWord = randomWord / 100;
      if (!checkDuplicated[lottoNumber]) {
        if (count == 0) {
          s_randomWord = s_randomWord + lottoNumber;
        }
        else {
          s_randomWord = s_randomWord + (lottoNumber * (100 ** count));
        }
    ...
    for (uint32 i = 0; i < 6; i ++) {
      checkDuplicated[lottoNumbers[i]] = false;
    }
