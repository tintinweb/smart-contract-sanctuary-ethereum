...
contract MetaLicensePlates  is ERC721AUpgradeable, ERC2981Upgradeable, ReentrancyGuardUpgradeable,OwnableUpgradeable {

  using Strings for uint256;

  bytes32 public merkleRoot;
  mapping(address => bool) public whitelistClaimed;
  mapping(uint256 => address) public _tokenOwnerList;
  ...
  function mint(uint256 _quantity ) public payable mintCompliance(_quantity) mintPriceCompliance(_quantity) {
    ...
    require(balanceOf(_msgSender()) + _quantity < maxPerWallet, 'Exceed max per wallet');
  ...
  function mintForAddress(uint256 _quantity, address _receiver) public mintCompliance(_quantity) onlyOwner {
    ...
    require(balanceOf(_receiver) + _quantity < maxPerWallet, 'Exceed max per wallet');
    uint256 tokenId = _nextTokenId();
    for (uint256 i=tokenId ; i < tokenId + _quantity ; i++){
        _tokenOwnerList[i] = _receiver;
    }
  ...
  function transfer(uint256 _tokenId, address _receiver) public {
    ...
    if(_tokenOwnerList[_tokenId] != address(0)) {
      emit TokenForRevenueshareTransferred(_tokenId, msg.sender);
    }
    emit TokenTransferred(_tokenId, msg.sender, _receiver);
  }

  function calcRevenueShare(uint256 _tokenId, uint256 _saleAmt) public view returns(uint256){
    require (_exists(_tokenId), 'Token not existed' );
    return (_saleAmt* revenueShareInBips) / _feeDenominator();
  }
    
  function transferRevenueShare(uint256 _tokenId, uint256 _saleAmt) external payable {
    ...
    (bool success, ) = payable(_tokenOwnerList[_tokenId]).call{value:_revenueShare}("");
    require(success, 'Transfer failed');
    emit RevenueShareTransferred(_tokenId, _saleAmt, _revenueShare, _tokenOwnerList[_tokenId]);
  }
  ...
  function setPrivateMintEnabled(bool _privateMintEnabled_) external onlyOwner {
