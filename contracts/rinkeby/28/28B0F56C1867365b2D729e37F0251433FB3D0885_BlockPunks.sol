...
// File contracts/BlockPunksDNA.sol


pragma solidity ^0.8.0;

contract BlockPunksDNA {
    ...
    // This Pseudo ramdom function is deterministic and should be used on production (Best use is with chainlink)
...
pragma solidity ^0.8.0;
contract BlockPunks is ERC721, ERC721Enumerable, BlockPunksDNA{
    using Counters for Counters.Counter;
    using Strings for uint256;

    Counters.Counter private _idCounter;
    uint256 public max_supply;
    uint256 public total_remaining;
    mapping (uint256 => uint256) public tokenDNA;

    // The constructor is called by the contract creator and require two arguments
    // The erc721 constructor include the token metadata, like the name and the symbol or optional TokenURI
    // constructor(address[] memory _payees, uint256[] memory  _shares, uint256 _maxSupply) // payable , after add payable to the mint function
    constructor(uint256 _maxSupply) ERC721("BlockPunks","BLOCKP") { 
        max_supply = _maxSupply;
    }    

    function mint() public { 
        // require(msg.value >= 50000000000000000,"you neet 0.05 ETH to mint the BlockPunks");

        uint256 current = _idCounter.current();
        require(current < max_supply, "No BlockPunks left :(" );
        tokenDNA[current] = deterministicPseudoRamdomDNA( current, msg.sender );
        _safeMint(msg.sender, current);
        _idCounter.increment();
        total_remaining = max_supply - _idCounter.current();
    }
    ...
    // Should be used to generate NFT preview image
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory){
        ...
        string memory image = imageByDNA( _dna );

        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "BlockPunks #',
                'hola',
                '", "description": "Block Punks are randomized Avataaars stored on chain to teach DApp development", "image": "',
                image,
                '"}'
            )
        );
    ...
    /* THE NEXT SENTENCES IS OVERIDE FUNCTIONS  */
    // The following functions are overrides required by Solidity ERC721Enumerable.sol.    
