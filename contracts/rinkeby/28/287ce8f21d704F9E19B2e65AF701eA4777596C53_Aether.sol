...
contract Aether is ERC721, Ownable {
    ...
    modifier lockMinting() {
        require(
            !inMinting,
            "Contract is minting, This function cannot be called while Minting."
        );
        inMinting = true;
        _;
        inMinting = false;
     ...
     */
    function mint() public payable lockMinting {
        uint256 tokenIndex = _tokenIdCounter.current() + 1;
        uint256 discountValue = discount[msg.sender];
        ...
        require(
            msg.value == tokenPrice.sub(discountValue),
     ...
     */
    function ownerRenewTokenLong(uint256[] calldata _tokenIds)
        external
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            uint256 _currentexpiryTime = expiryTime[_tokenIds[i]];
    ...
    function addPartner(address partner, uint256 discountValue)
        public
        onlyOwner
    {
        require(tokenPrice > discountValue, "Discount can't be bigger than the token price.");
        discount[partner] = discountValue;
    }
