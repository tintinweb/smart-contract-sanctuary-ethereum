...
contract NFT2ERC20 is ReentrancyGuard, ERC20Burnable, AdminControl, INFT2ERC20 {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165, AdminControl) returns (bool) {
        return interfaceId == type(INFT2ERC20).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    function setRateEngine(address rateEngine) external override adminRequired {
        require(
            ERC165Checker.supportsInterface(rateEngine, type(INFT2ERC20RateEngine).interfaceId),
    ...
    function _burnToken(
        ...
        if (args.length > 1) {
            ...
            // Burn the token
            (bool success, ) = tokenContract.call(
                abi.encodeWithSelector(_specTransferFunction[spec], msg.sender, address(0xdEaD), args[0])
            );
            require(success, "NFT2ERC20: Burn failure");
        }
...
contract MetaMorpH is NFT2ERC20 {
    constructor() NFT2ERC20("MetaMorpH", "MMH") {}
}