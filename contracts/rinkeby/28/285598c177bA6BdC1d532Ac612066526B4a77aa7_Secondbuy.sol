...
contract Secondbuy is Initializable, ERC2981Upgradeable, EIP712Upgradeable {
    ...
    bytes4 private constant FUNC_SELECTOR = bytes4(keccak256("royaltyInfo(uint256,uint256)"));  
    mapping(uint256 => bool) public batchAmountStatus;   // for verifying if the voucher has NFTs greater than zero
    mapping(uint256 => uint256) public nftBatchamountleft;   // for verifying if the voucher has NFTs greater than zero
    ...
    function _hashpriMarketItem(FNVvoucher.marketItem memory voucher)
        internal
        view
        returns (bytes32)
    {
        return
            _hashTypedDataV4(keccak256(
                abi.encode(
                    keccak256(
                        "marketItem(address nftAddress,address owner,uint256 tokenId,uint256 unitPrice,uint256 nftBatchAmount,uint256 counterValue,string tokenURI,bool listed)"
                        ),
                        voucher.nftAddress,
                        voucher.owner,
                        voucher.tokenId,
    ...
    function _verifypriListing(FNVvoucher.marketItem memory voucher)
        public /////////////////////////////////////////////internal
        view
        returns (address)
    {
    ...
    function _hashSecAuctionItemSeller(FNVvoucher.secAuctionItemSeller memory auctionSeller)
        internal
        view
        returns (bytes32)
    {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "secAuctionItemSeller(address nftAddress,address owner,uint256 tokenId,uint256 nftBatchAmount,string tokenURI,uint256 minimumBid,uint256 startTime,uint256 endTime,bool started,bool ended)"
                        ),
    ...
    function _verifySecAuctionItemSeller(FNVvoucher.secAuctionItemSeller memory auctionSeller)
        internal
        view
        returns (address)
    {
    ...
    function _hashAuctionItemBuyer(FNVvoucher.auctionItemBuyer memory auctionBuyer)
        internal
        view
        returns (bytes32)
    {
    ...
    function _verifyAuctionItemBuyer(FNVvoucher.auctionItemBuyer memory auctionBuyer)
        internal
        view
        returns (address)
    {
    ...
    function auctionWinner(
        ...
        require(    
            ITemplate1155(auctionSeller.nftAddress).balanceOf(
                ...
                auctionSeller.nftBatchAmount == 1,
        ...
        require(keccak256(abi.encode(auctionSeller.tokenURI)) == keccak256(abi.encode(auctionBuyer.tokenURI)), "auctionWinner: Enter Same tokenURI");
        require(
        ...
        // console.log("auctionSeller.owner: ",auctionSeller.owner);

        require(seller == auctionSeller.owner, "auctionWinner: Invalid seller");
        // console.log("SELLER VERIFIED");

        address buyer = _verifyAuctionItemBuyer(auctionBuyer);
        // console.log("buyer: ",buyer);
        // console.log("auctionBuyer.buyer: ",auctionBuyer.buyer);

        require(buyer == auctionBuyer.buyer, "auctionWinner: Invalid buyer");
        ...
        // if(checkFunc(auctionSeller.nftAddress, auctionSeller.tokenId, auctionSeller.nftBatchAmount)){
        //     uint256 serviceFees = ((msg.value * marketFees) / 10000);
        //     (address receiver, uint256 royalty) = ITemplate1155(auctionSeller.nftAddress)
        ...
        uint preownerBal = (auctionSeller.owner).balance;
        console.log("preownerBal: ", preownerBal);

        (bool success, ) = payable(auctionSeller.owner).call{value: nftPrice}(
            ""
        );

        uint postownerBal = (auctionSeller.owner).balance;
        console.log("postownerBal: ", postownerBal);

        require(success, "auctionWinner: Failed to transfer the funds");

        uint preAdminBal = (Admin).balance;
        console.log("preAdminBal: ", preAdminBal);

        (bool success1, ) = payable(Admin).call{value: serviceFees}("");

        uint postAdminBal = (Admin).balance;
        console.log("postAdminBal: ", postAdminBal);
    ...
    function secondarybuy(FNVvoucher.marketItem memory seller, address redeemer, uint256 nftAmount) ///////////////////////////////////
        public
        payable
    {
        require(!batchAmountStatus[seller.tokenId]  , "MintVoucher has expired");
        ...
        require(address(seller.nftAddress) != address(0), "invalid address");
        address signer = _verifypriListing(seller);
        // console.log("signer",signer);
        require(signer == seller.owner, "invalid seller");
        require((seller.unitPrice*(nftAmount)) == msg.value , "pay more");
        ...
        require(nftBatchamountleft[seller.counterValue] >= 0, "Invalid amount");
        if(nftBatchamountleft[seller.tokenId] == 0){
            batchAmountStatus[seller.tokenId] = true;
        }

        if(checkFunc(seller.nftAddress, seller.tokenId, seller.nftBatchAmount)){
            uint256 serviceFees = ((msg.value * marketFees) / 10000);
            (address receiver, uint256 royalty) = ITemplate1155(seller.nftAddress)
            .royaltyInfo(seller.tokenId, msg.value);
    ...
    function checkFunc(address nftAddress, uint256 tokenId, uint256 amount) internal returns(bool) {
        bool success;
        bytes memory data = abi.encodeWithSelector(FUNC_SELECTOR, tokenId, amount);
    ...
    // }
    function safeApproveForAllUser(address _userAddress,address _nftAddress)external {
