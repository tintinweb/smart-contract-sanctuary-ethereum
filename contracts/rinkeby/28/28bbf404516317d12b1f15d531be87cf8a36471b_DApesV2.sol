...
import "./DApesV2Base.sol";
import "@openzeppelin/contracts-upgradeable/access/AccessControlUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/utils/cryptography/ECDSAUpgradeable.sol";

contract DApesV2 is DApesV2Base, AccessControlUpgradeable {
    ...
    function initialize(address aGatekeeper, string memory uri) public initializer {
        __DApesV2Base_init(aGatekeeper, uri);
        __AccessControl_init_unchained();
    }

    function rolesSetup() public onlyOwner {
    ...
    function addCollection(uint256 startID, uint256 endID) public onlyOwner {
        ...
        collections.push(Collection({ nextID: CountersUpgradeable.Counter(startID), endID: endID, startID: startID }));
    }
    ...
    function authorizedMint(uint256 collectionID, address to) public onlyRole(MINTER_ROLE) {
        Collection storage collection = collections[collectionID];
        uint256 newID = collection.nextID.current();
        require(newID < collection.endID, "Minted out");
     
        _supply.increment();
        _usedNonces.set(newID);
        collection.nextID.increment();
     
        emit CollectionMinted(collectionID, newID, newID, to);
        _mint(to, newID, 1, "");
    }

    function keyHash(uint256 collection, uint256 nonce, address addr) public pure returns (bytes32) {
        return ECDSAUpgradeable.toEthSignedMessageHash(abi.encodePacked(collection, nonce, addr));
    }
    ...
    function mint(uint256 collectionID, uint256 nonce, bytes memory signature) public {
        ...
        require(ECDSAUpgradeable.recover(kh, signature) == gatekeeper, "Invalid access key");
...
abstract contract DApesV2Base is ERC1155PausableUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using BitMapsUpgradeable for BitMapsUpgradeable.BitMap;

    struct Collection {
        uint256 startID;
        CountersUpgradeable.Counter nextID;
        uint256 endID;
    }
    ...
    CountersUpgradeable.Counter internal _supply;
    BitMapsUpgradeable.BitMap internal _usedNonces;

    Collection[] public collections;
    
    function __DApesV2Base_init(address aGatekeeper, string memory uri) public initializer {
        ...
        gatekeeper = aGatekeeper;
        _supply = CountersUpgradeable.Counter(0);
    }
