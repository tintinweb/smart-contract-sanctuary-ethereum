...
contract RichApeWives is ERC721URIStorage, Ownable {
    ...
    constructor(address _dataFeed, string memory _networkBaseURI, address _releaseTickets) ERC721("0xRichApeWives", "0xRAW") {
        ...
        accounts[msg.sender] = Account(0, true, false);
        ...
        for (uint i = 0; i < totalSeeders; i++) {
            ...
            accounts[seeder] = Account(0, false, true);
            seeders.push(seeder);
        }
    }
    ...
    function seedersAirdrop() external onlyAdmin {
        for (uint i = 0; i < seeders.length; i++) {
            Account memory acc = accounts[seeders[i]];
            if (acc.mintedNFTs < 30) {
                for (uint x = acc.mintedNFTs; x < 30; x++) {
