...
contract DNAChip is ERC721, Ownable {
    ...
    function _generateTraits(uint256 tokenId) internal returns (RedactedLibrary.Traits memory traits) {
        ...
        traits.earrings = _rarityGen(_getRandomNumber(tokenId, 1000), EARRINGS);
        traits.eyes = _rarityGen(_getRandomNumber(tokenId, 1000), EYES);
        traits.hats = _rarityGen(_getRandomNumber(tokenId, 1000), HATS);
        traits.mouths = _rarityGen(_getRandomNumber(tokenId, 1000), MOUTHS);
        traits.necks = _rarityGen(_getRandomNumber(tokenId, 1000), NECKS);
        traits.noses = _rarityGen(_getRandomNumber(tokenId, 1000), NOSES);
        traits.whiskers = _rarityGen(_getRandomNumber(tokenId, 1000), WHISKERS);
    }
}
