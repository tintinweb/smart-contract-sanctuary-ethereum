    ...
    IERC721Receiver
{
    ...
    function setStartWLPeriode(uint256 _startTime) public onlyOwner {
        startWLPeriod = _startTime;
    }

    function setEndWLPeriode(uint256 _startTime) public onlyOwner {
        endWLPeriod = _startTime;
    }
    ...
    mapping(uint256 => Soldiers) public Barrack;
    ...
    function BecomeSoldier(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(Barrack[tokenId].tokenId == 0, "already staked");
    ...
    function WAR(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _burn(tokenIds[i]);
        }
    }
    ...
    function AirForce(
        ...
        uint256[] memory _totalSoldiers
    ) public onlyOwner {
        for (uint256 i = 0; i < _address.length; i++) {
            _mint(_address[i], _totalSoldiers[i]);
        }
    }
}
