...
contract GSRChallenge2PoolArbitrage is Ownable {
        ...
        )
    {
        ...
        price0 = _getPrice(_token0, _token1, _factory0);
        price1 = _getPrice(_token0, _token1, _factory1);

        if (price0 != price1) {
            arbitrage = price0 > price1 ? price0 - price1 : price1 - price0;
        } else {
            arbitrage = 0;
        }
    }
    ...
    function executeArb(
        ...
        (
            ...
            uint256 price0,
            uint256 price1
        ) = maxArbitragePossible(_token0, _token1, _factory0, _factory1);
        require(arbitrage > 0, "no_arbitrage");
        _depositToken(_token0, _amount0);
        uint256 _amount1;
        if (price0 > price1) {
            // sell token0 at dex0 and swap for token1 at dex0
            _amount1 = _swapToken(_token0, _token1, _amount0, _router0);
            amountOut = _swapToken(_token1, _token0, _amount1, _router1);
        } else if (price1 > price0) {
            _amount1 = _swapToken(_token0, _token1, _amount0, _router1);
            amountOut = _swapToken(_token1, _token0, _amount1, _router0);
        }
    ...
    function _swapToken(
        ...
        path[0] = token0;
        path[1] = token1;
        IUniswapV2Router01 exchange = IUniswapV2Router01(routerAdd);
    ...
    // get exchange rate for token1/token2 from dex
    function _getPrice(
        address _token0,
        address _token1,
        address factory
