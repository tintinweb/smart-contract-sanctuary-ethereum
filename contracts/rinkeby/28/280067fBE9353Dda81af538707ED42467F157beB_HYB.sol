...
contract HYB is ContractFactory, HYBCards {
    ...
    function mint(bytes memory signature, uint256[] memory ids, uint256[] memory amounts) external payable {
        ...
        require(signerAddress == messagehash.recover(signature));
        require(!isSalePaused);
        
        for (uint256 i = 0; i < ids.length; i++) {
            require(cards[i].isCardForSale);
            require(cards[i].amountPurchased + amounts[i] <= cards[i].maxSupply);
            require(totalSupply(i) + amounts[i]  <= cards[i].maxSupply);
...
abstract contract HYBCards is Ownable {
    ...
    */
    function addCard(
        ...
        c.currentSupply = _currentSupply;
    ...
    */
    function editCard(
        ...
        require(cards[_cardIndex].currentSupply > 0);

        cards[_cardIndex].mintPrice = _mintPrice;  
        cards[_cardIndex].maxSupply = _maxSupply;     
        cards[_cardIndex].currentSupply = _currentSupply;                           
