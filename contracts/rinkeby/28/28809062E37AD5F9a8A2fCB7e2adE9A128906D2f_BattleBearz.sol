...
contract BattleBearz is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public maxTeamMintAmountPerWallet = 2; // Change to Team whitelist
  ...
  constructor(
      ...
      address[] memory _staff_address,
      uint256[] memory _value)  ERC721A(_tokenName, _tokenSymbol)  {
    hiddenMetadataUri = _hiddenMetadataUri;       
    ownerClaimed(_staff_address, _value);
  ...
  function ownerClaimed(address[] memory staff_address, uint256[] memory value) internal {
    for (uint256 i = 0; i < staff_address.length; i ++) {
      _mint(staff_address[i], value[i]);
    }
