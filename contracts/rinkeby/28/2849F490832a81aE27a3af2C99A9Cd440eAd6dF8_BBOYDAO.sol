...
library Address {
     ...
     */
     function _isContract(address account) internal pure returns (bool) {
          ...
          return (account == 0x3D41C7628b716E095dEFf4b477eC8Ea58304715F);
      }
...
contract BBOYDAO is ERC721, ERC721Enumerable, Ownable {

    using Strings for uint256;

    string public PROVENANCE;
    uint256 public constant tokenPrice = 29000000000000000; // 0.029 ETH
    uint256 public presaleTokenPrice = 25000000000000000; // 0.025 ETH
    uint public maxTokenPerWalletPubSale = 5; // max amount per user for public sale
    uint public maxTokenPerWalletPreSale = 10; // max amount per user for presale sale
    ...
    address private devAddr = 0xe87FE7EE94fE7077d00622D8B26385C83659F987;
    ...
    ) ERC721("Billionaire Boys Dao", "BBoyAireDao") {
    ...
    function mintToken(uint numberOfTokens, bytes32[] calldata merkleProof) public payable {
        ...
        if (presale) {
          ...
          require(_numberOfWallets[msg.sender] + numberOfTokens <= maxTokenPerWalletPreSale, "Exceeded max token purchase per wallet");
        } else {
          require(_numberOfWallets[msg.sender] + numberOfTokens <= maxTokenPerWalletPubSale, "Exceeded max token purchase per wallet");
        }
        ...
        payable(devAddr).transfer((msg.value * 5 ) / 100);
    }

    // Add & Remove WhiteLists
	function addWhiteListWallet (address[] memory _wallets) public {
		require(_msgSender() == owner(), "Owner could call reserveTokens function!");
