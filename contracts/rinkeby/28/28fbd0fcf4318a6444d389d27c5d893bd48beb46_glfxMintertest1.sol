...
contract glfxMintertest1 is CreatorExtension, ICreatorExtensionTokenURI, IManifoldERC721Edition, ReentrancyGuard {
    ...
    uint256 class1 = uint256(1 ether) / (1 ether * 2);
    uint256 class2 = uint256(1 ether) / (1 ether * 4);
    uint256 class3 = uint256(1 ether) / (1 ether * 8);
    uint256 class4 = uint256(1 ether) / (1 ether * 16);
    uint256 class5 = uint256(1 ether) / (1 ether * 32);
    uint256 class6 = uint256(1 ether) / (1 ether * 64);
    uint256 class7 = uint256(1 ether) / (1 ether * 128);
    ...
    function tokenURI(address creator, uint256 tokenId) external view override returns (string memory) {
        ...
        uint16 picker;
        uint16 randClass = randomClass(picker);
        return string(abi.encodePacked(_tokenPrefix[creator][series], randClass, "/", (index+1).toString(), ".json"));
    }
    ...
    function randomClass(uint16 pickedClass) internal view returns(uint16){

        uint random = uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty, msg.sender))) % 128;
        uint256 ran = (1 ether * random) / (1 ether * 128);
    
        if (ran <= class7){ pickedClass = 7; }
        if (ran <= class6){ pickedClass = 6; }
        if (ran <= class5){ pickedClass = 5; }
        if (ran <= class4){ pickedClass = 4; }
        if (ran <= class3){ pickedClass = 3; }
        if (ran <= class2){ pickedClass = 2; }
        if (ran <= class1){ pickedClass = 1; }
