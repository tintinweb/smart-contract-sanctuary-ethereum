...
library LendingPoolStateLogic {
  ...
  function validateAfterDisbursement(LendingPoolStateData memory _self, LendingPoolConstraints memory _constraints)
    public
    view
  {
    uint256 reservePercentage = getReservePercentage(_self);
    satisfiesMinReserve(_constraints, reservePercentage);
    satisfiesMaxPoolSize(_constraints, _self.totalActiveLoanValue.add(_self.totalNonActiveLoanValue));
  }
  ...
  function _getTotalValue(LendingPoolStateData memory _self) internal view returns (uint256) {
    ...
    return _self.totalActiveLoanValue.add(_self.totalNonActiveLoanValue).add(poolReserve);
  }
