...
contract Reputation is Initializable, ERC20Upgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable {
    ...
    function batchMint(address[] memory toArray, uint256[] memory amountArray) public onlyOwner {
        require(toArray.length == amountArray.length, "Input arrays must have the same length");
        require(toArray.length >= 1, "Input arrays must have at least one element");
        for(uint256 i = 0 ; i < toArray.length; i++){ mint(toArray[i], amountArray[i]); }
    }     
