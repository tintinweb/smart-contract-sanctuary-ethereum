...
contract GYOZ is Context, IERC20, Ownable {  //&
    ...
    uint256 public swapThreshold = 200 * 10**_decimals; //&
    ...
    function takeFee(
        ...
        if (feeAmount > 0) {
            //Re-inject directly the liquidity shares into the LP contract
            uint256 lpFee = feeAmount * (lpShare / getTotalFee());
            balances[lpPair] += lpFee;
            emit Transfer(sender, lpPair, lpFee);
            //Inject the rest of the token into the contract itself
            uint256 remainingFees = feeAmount - lpFee; 
            balances[feeReceiver] += remainingFees;
            emit Transfer(sender, feeReceiver, remainingFees);
        }

        return amount - feeAmount;
    }

    function fee1() public view returns(uint256){
        return getTotalFee();
    }

    function fee2() public view returns(uint256){
        return (lpShare / getTotalFee());
    }

    function fee3(uint feeAmount) public view returns(uint256){
        return feeAmount * (lpShare / getTotalFee());
    }

    function fee4(uint feeAmount) public view returns(uint256){
        return feeAmount * lpShare;
    }

    function fee5(uint feeAmount) public view returns(uint256){
        return feeAmount / getTotalFee();
    }

    function fee6(uint feeAmount) public view returns(uint256){
        return (feeAmount * lpShare) / getTotalFee();
    }

    function fee7(uint feeAmount) public view returns(uint256){
        return feeAmount * lpShare / getTotalFee();
    }
