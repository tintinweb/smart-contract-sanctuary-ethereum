...
contract dust_for_punkz_traditional is Ownable,  dust_redeemer, IRNGrequestor,IERC777Recipient, ReentrancyGuard {

    RegistryConsumer   constant public reg = RegistryConsumer(0x7B3c35D7D339A3A0f70D06b8852a35D674bEB1dC);
    // IRNG          public rng;
    // address       public DUST_TOKEN;

    uint256                                   next_redeemable;
    ...
    function numberOfVaults() external view returns (uint) {
        return next_redeemable;
    }
    ...
    function rngAddress() internal view returns (address) {
        return reg.getRegistryAddress("RANDOM_V2");
    }

    function rng() internal view returns (IRNG) {
        return IRNG(rngAddress());
    }
