...
contract ERC721 is ERC165, IERC721, IERC721Metadata {
     ...
     * Emits a {Transfer} event.
     */
    function _mint1(address to) internal virtual {
...
contract CocoSwirls is Ownable, IERC2981, ERC721 {
    ...
    modifier checkSupply(uint16 x) {
        require(totalSupply < x, "Purchase will exceed the max supply!");
        _;
    }

    modifier whitelistCheck(bytes calldata sig) {
        require(_onlyWhitelist, "Minting is not enabled!");
        require(isWhitelisted(msg.sender, sig), "User not whitelisted!");
        usedSigs[sig] = true;
        _;
    }

    function mint1() external payable checkSupply(1000) {
        require(_mintingEnabled, "Minting is not enabled!");
        require(msg.value == 42e15, "ETH Amount is not correct!");
        _mint1(msg.sender);
    }

    function mint2() external payable checkSupply(1000) {
        require(_mintingEnabled, "Minting is not enabled!");
        require(msg.value == 84e15, "ETH Amount is not correct!");
        _mint2(msg.sender);
    }

    function preMint1(bytes calldata sig) external payable checkSupply(120) whitelistCheck(sig) {
        require(msg.value == 42e15, "ETH Amount is not correct!");
        _mint1(msg.sender);
    }

    function preMint2(bytes calldata sig) external payable checkSupply(120) whitelistCheck(sig) {
        require(msg.value == 84e15, "ETH Amount is not correct!");
        _mint2(msg.sender);
    }


    function isWhitelisted(address _wallet, bytes memory _signature) public view returns(bool) {
    ...
    function royaltyInfo(uint tokenId, uint salePrice) external view returns(address receiver, uint256 royaltyAmount) {
        ...
        return(royaltyReceiver, salePrice / 20); // 5#%
    }
