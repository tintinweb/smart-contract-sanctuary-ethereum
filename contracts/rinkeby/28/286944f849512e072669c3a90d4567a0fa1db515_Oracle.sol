...
interface IOracle {
	...
	function getStrikePrice(
		...
		uint256 stikeValue
	) external view returns (uint256 startPrice, uint256 strikePrice);

	function getRoundTS(PeriodType periodType) external view returns (uint256 start, uint256 end);

	function getNextRoundTS(PeriodType period) external view returns (uint256 start, uint256 end);

	function updatePriceByExternal(string memory symbol, uint256 ts) external returns (uint256 price_);

	function getSpacePrice(uint256 rawPrice) external pure returns (uint256 price);
}
...
contract Oracle is Ownable, IOracle {
	...
	mapping(string => mapping(uint256 => uint256)) public override historyPrice;
	uint256[] public monStartTS;
	...
	function getStrikePrice(
		...
		PeriodType periodType,
		uint256 strikeValue
	) external view override returns (uint256 startPrice, uint256 strikePrice) {
		(uint256 startTS, ) = getRoundTS(periodType);
		startPrice = historyPrice[symbol][startTS];
		strikePrice = getSpacePrice(strikeValue);
	}

	function getSpacePrice(uint256 rawPrice) public pure override returns (uint256 price_) {
		uint256 i = 12;
		while (rawPrice / 10**i >= 10) {
			i += 1;
		}
		uint256 maxI = i - 1;
	...
	function getTS(PeriodType periodType, uint256 offset) public view returns (uint256 start, uint256 end) {
		...
		} else if (periodType == PeriodType.BIWEEK) {
			// 1 => BIWEEK
			start = block.timestamp - ((block.timestamp - 115200) % 604800);
			start = start + 1209600 * offset;
			end = start + 1209600;
		} else if (periodType == PeriodType.MONTH) {
			// 2 => MONTH
			for (uint256 i; i < monStartTS.length; i++) {
				if (block.timestamp >= monStartTS[i] && block.timestamp <= monStartTS[i + 1]) {
	...
	function getRoundTS(PeriodType periodType) public view override returns (uint256 start, uint256 end) {
		return getTS(periodType, 0);
	}

	function getNextRoundTS(PeriodType periodType) external view override returns (uint256 start, uint256 end) {
		return getTS(periodType, 1);
	}
