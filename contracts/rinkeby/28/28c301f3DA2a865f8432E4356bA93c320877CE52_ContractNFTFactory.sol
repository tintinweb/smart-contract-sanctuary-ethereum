...
contract ContractNFTFactory is OwnableUpgradeable {
  ...
  function createClone(address _newOwner) public onlyOwner returns (address clone){
    ...
    ContractNFT(_clone).initialize(_newOwner, "", "", 1, ""); // owned by this contract
...
contract ContractNFT is OwnableUpgradeable {
    ...
    function tokenURI(uint256 _tokenId) public view returns (string memory){
        return string(abi.encodePacked(uri, _tokenId));
    }

    function initialize(address newOwner, string memory _name, string memory _symbol, uint256 _tokenId, string memory _uri) public initializer {
        __Ownable_init();
        tokenId = _tokenId;
        tokenName = _name;
        tokenSymbol = _symbol;
    ...
    function totalSupply() public view returns (uint256) {
        return owner() == address(0) ? 0 : tokenId;
    }

    function balanceOf(address account) public view returns (uint256) {
        return owner() == account ? tokenId: 0;
    }
    ...
    function safeTransferFrom(address from, address to, uint256 _tokenId, bytes calldata) public {
        require(from == owner(), "Not Owner");
        transferOwnership(to);
        emit Transfer(from, to, _tokenId);
    }
