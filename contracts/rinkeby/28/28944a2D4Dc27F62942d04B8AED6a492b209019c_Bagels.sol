...
contract Bagels is ERC721Enumerable, Ownable {
    ...
    uint256 public constant priceInWei = 0.02 ether;
    uint8 private constant RESERVE_AMOUNT = 100;

    constructor(address payable withdrawAddr_, string memory _baseURI) ERC721("___Bagel", "___BAGEL") {
    ...
    function reserveForMomAndFrens() public onlyOwner {
        require(_owners.length == 0, 'Reserves have been issued.');
        for (uint256 i; i < RESERVE_AMOUNT; i++)
            _mint(_msgSender(), i);
    }
    ...
    function mintPresale(uint numTokensRequested) public payable {
        require(presaleIsActive, "Presale is not active");
        require(isOnWhitelist(), "Presale requires mfer ownership");
        mint(numTokensRequested);
    }

    function mintPublic(uint numTokensRequested) public payable {
        require(saleIsActive, "Sale is not active");
        mint(numTokensRequested);
    }

    function mint(uint numTokensRequested) internal {
    ...
    function setMferAddr(address _mferAddr) public onlyOwner {
        mferAddr = _mferAddr;
    }

    function isOnWhitelist() public view returns (bool) {
        IERC721 mferContract = IERC721(mferAddr);
        return mferContract.balanceOf(_msgSender()) != 0;
    }
