...
// TODO : First Draft of Staking Contract need to test throughly and improve
// TODO : Harvest for WETH marketfee reward

contract Staking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    /// @dev Token rewards per seconds for staking
    uint256 internal rewardsPerSecForStaking;
    /* ==================== EVENTS ==================== */
     ...
     * @param _circulatingSupply supply for staking
     */
    function initialize(
        address _stakeToken,
        address _feeToken,
        ...
        require((_period > 0 && _circulatingSupply > 0), "Invalid params");
        ...
        totalSupply = _circulatingSupply;
        START_BLOCK = _startBlock;
        rewardsPerSecForStaking = 317097919800000000;
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        stakers[_msgSender()].amount += (amount + pendingRewards);
        stakers[_msgSender()].reward = (stakers[_msgSender()].amount * accTokenPerShare) / PRECISION_FACTOR;
        stakers[_msgSender()].since = block.timestamp;
        // Increase totalAmountStaked
        totalAmountStaked += (amount + pendingRewards);

        emit Deposit(_msgSender(), amount, pendingRewards);
    }
    ...
    function receiveWETHFee(uint256 amount) external onlyOwner nonReentrant {
        require(amount > 0, "Collect Fee: Amount must be > 0");
        WETH.safeTransferFrom(_msgSender(), address(this), amount);
        accMarketFee += amount;
        emit Deposit(_msgSender(), amount, accMarketFee);
    }
     ...
     *
     */
    function harvestFee() external nonReentrant {
        require(stakers[_msgSender()].amount > 0, "Harvest: Not a staker");
        uint256 balance = WETH.balanceOf(address(this));
        if (balance == 0 || accMarketFee == 0) {
            return;
        }
        uint256 rewardFee = getHavestAmount(_msgSender()) / PRECISION_FACTOR;
    ...
    /**
     * @notice get WETH Market fee reward
     *
     */
    function getHavestAmount(address _user) public view returns (uint256) {
        ...
        return ((stakers[_user].amount * PRECISION_FACTOR) / totalAmountStaked) * accMarketFee;
    }
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        // Adjust user information
        stakers[_msgSender()].amount = stakers[_msgSender()].amount + pendingRewards - amount;
        stakers[_msgSender()].reward = (stakers[_msgSender()].amount * accTokenPerShare) / PRECISION_FACTOR;
        stakers[_msgSender()].since = block.timestamp;
        ...
        emit Withdraw(_msgSender(), amount, pendingRewards);
    }
    ...
    function withdrawAll() external nonReentrant {
        ...
        // Adjust total amount staked
        totalAmountStaked = totalAmountStaked - stakers[_msgSender()].amount;
    ...
    function compound() internal {
        ...
        if (stakers[_msgSender()].amount > 0) {
            pendingRewards =
                ((stakers[_msgSender()].amount * accTokenPerShare) / PRECISION_FACTOR) -
                stakers[_msgSender()].reward;
        }
    ...
    function calculatePendingRewards(address user) public view returns (uint256) {
        if ((block.number > lastRewardBlock) && (totalAmountStaked != 0)) {
            uint256 tokenRewardForStaking = ((block.timestamp - stakers[user].since) * rewardsPerSecForStaking);

            uint256 tokenPerShare = accTokenPerShare + (tokenRewardForStaking * PRECISION_FACTOR) / totalAmountStaked;

            return (stakers[user].amount * tokenPerShare) / PRECISION_FACTOR - stakers[user].reward;
        } else {
            return (stakers[user].amount * accTokenPerShare) / PRECISION_FACTOR - stakers[user].reward;
        }
    }
    ...
    function _updatePool() internal {
        ...
        compound();
        // allocate tokens only if token rewards for staking are not null
        uint256 tokenRewardForStaking = mpwrToken.balanceOf(address(this));
