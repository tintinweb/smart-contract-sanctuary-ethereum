...
contract AeroPup is Ownable, ERC721A {
  ...
  uint public PRICE_PUBLIC        = 0.08 ether;
  uint public MAX_WHITELIST_BATCH = 5;

  mapping(address => uint8) private whitelistBuyList;

  constructor() ERC721A("The AeroPups Official Collection", "AeroPups") {
    revealed = false;
  }
  ...
  function _Mint(uint8 quantity) external payable callerIsUser{
    require(totalSupply() < MAX_SUPPLY, "PUBLIC_SALE: Please go to the Opensea to buy The AeroPups.");
  ...
  function withdrawAll() external onlyOwner {
    ...
    _withdraw(msg.sender, totalBalance);
  }
