...
// hevm: flattened sources of /nix/store/2wysvya3sw7phy3rscpcvj019zll14im-geb-basic-multisig/dapp/geb-basic-multisig/src/MultisigWallet.sol
pragma solidity >=0.6.7 <0.7.0;

////// /nix/store/2wysvya3sw7phy3rscpcvj019zll14im-geb-basic-multisig/dapp/geb-basic-multisig/src/MultisigWallet.sol
...
contract MultiSigWallet {
    ...
    modifier onlyWallet() {
        if (msg.sender != address(this))
            revert("This wallet is not the msg.sender");
        _;
    }
    ...
    modifier validRequirement(uint ownerCount, uint _required) {
        if (ownerCount > MAX_OWNER_COUNT) {
          revert("Owner count exceeds MAX_OWNER_COUNT");
        }
        if (_required > ownerCount) {
          revert("Required exceeds owner count");
        }
        if (_required == 0) {
          revert("Required is zero");
        }
        if (ownerCount == 0) {
          revert("Owner count is zero");
        }
        _;
    }
        ...
        validRequirement(_owners.length, _required)
    {
        for (uint i=0; i<_owners.length; i++) {
            if (isOwner[_owners[i]] || _owners[i] == address(0))
                revert("Owner null or already specified");
