...
contract Treasury is IERC20, Context, Ownable {
    ...
    string private _name = "Router Tracker";
    string private _symbol = "tROUTER";
    uint8 private _decimals = 9;
    uint256 public totalDividendsDistributed;
    IterableMapping private tokenHoldersMap = new IterableMapping();
    uint256 public minimumTokenBalanceForDividends = 500_000 * 10 **  _decimals;
    RouterXYZ private routerToken;
    ...
    function setTokenBalance(address account) external {
        uint256 balance = routerToken.balanceOf(account);
        if(!routerToken.isExcludedFromRewards(account)) {
    ...
    function setERC20Contract(address contractAddr) external onlyOwner {
        routerToken = RouterXYZ(payable(contractAddr)); 
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 60 && newClaimWait <= 86400, "ClaimWait must be updated below 24 hours");
...
contract RouterXYZ is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "Router Token";
    string private _symbol = "ROUTER";
    uint8 private _decimals = 9;
    ...
    uint256 public _maxWalletAmount = _tTotal.div(3).mul(100) * 10** _decimals;
    
    address public marketingAddress = 0xEe269ABfA6a79818f2a31B5a98645E37FA96B051;
    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public treasuryContractAddress = address(0);
    uint256 public gasForProcessing = 50000;
    address public USDCAddress = 0x4DBCdF9B62e891a7cec5A2568C3F4FAF9E8Abe2b; //Rinkeby
    // address public USDCAddress = 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48; //Mainnet

    event ProcessedTreasury(uint256 iterations, uint256 claims, uint256 lastProcessedIndex, bool indexed automatic,uint256 gas, address indexed processor);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(pair != address(0),"UniswapV2Pair has not been set");
        bool isSell = false;
        bool takeFees = !_isExcludedFromFee[from] && !_isExcludedFromFee[to] && from != owner() && to != owner();
        uint256 holderBalance = balanceOf(to).add(amount);

        //anti bot
        if (live && block.number <= activeTradingBlock + 10) {
        ...
        if(from == pair || isExchangeWallet[from]) {
            require(holderBalance <= _maxWalletAmount, "Wallet cannot exceed max Wallet limit");
        }
        if(from != pair && to == pair || (!isExchangeWallet[from] && isExchangeWallet[to])) { //if sell
            //only tax if tokens are going back to Uniswap
            isSell = true;
            sellTaxTokens();
        }
        if(from != pair && to != pair && !isExchangeWallet[from] && !isExchangeWallet[to]) {
    ...
    function distributeShares(uint256 balanceToShareTokens) private lockTheSwap {
        ...
        USDCToken.transfer(treasuryContractAddress, dividendShare);

    }

    function setTreasury(address treasuryContractAddress_) external onlyOwner {
        treasury = Treasury(payable(treasuryContractAddress_));
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFees, bool isSell, bool doUpdateDividends) private {
        ...
        if(doUpdateDividends) {
            try treasury.setTokenBalance(sender) {} catch{}
            try treasury.setTokenBalance(recipient) {} catch{}
            try treasury.process(gasForProcessing) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                emit ProcessedTreasury(iterations, claims, lastProcessedIndex, true, gasForProcessing, tx.origin);
            }catch {}
        }
    }
    ...
    function internalApprove(
        ...
        require(
            owner != address(0),
            "ERC20: Can not approve from zero address"
        );
        require(
            spender != address(0),
            "ERC20: Can not approve to zero address"
        );
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) internal {
        internalApprove(address(this), address(router), tokenAmount);
    ...
    function goLive() external onlyOwner returns (bool) {
        ...
        internalApprove(address(this), address(router), totalSupply());
