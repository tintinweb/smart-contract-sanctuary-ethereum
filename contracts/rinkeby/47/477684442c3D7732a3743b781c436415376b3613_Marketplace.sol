    ...
    ERC1820Implementer
{
    ...
    function withdraw(uint256 _amount, address _direction) external {
        ...
        IERC777(token).send(_direction, _amount, "");
    }
    ...
    function tellMyBalance() public returns (uint256) {
        myBalance = IERC777(token).balanceOf(msg.sender);
        return myBalance;
    }

    address account2 = 0xf6EFDE76B7dCECF3e18891D0904919698B9B407b;

    function sendTokensViaContract(uint256 _amount) public {
        IERC777(token).send(account2, _amount, "");
    }

    function sendToContract(address _toAccount, uint256 _amount) public {
        IERC777(token).send(_toAccount, _amount, "");
    }

    function regristerOperator(address _operator) public {
        IERC777(token).authorizeOperator(_operator);
    }

    bool public testOperatorVar = false;

    function testOperator(address _operator, address _tokenHolder)
        public
        returns (bool)
    {
        testOperatorVar = IERC777(token).isOperatorFor(_operator, _tokenHolder);
        return testOperatorVar;
    }
    ...
    function sendOperator(
        ...
        IERC777(token).operatorSend(_sender, _recipient, _amount, "", "");
    }
}
