...
contract WallStMoms is ERC721A, Ownable {
    ...
    enum SaleType{ WHITELIST_SUPER_FRENS, WHITELIST, WHITELIST_FREE_MINT, PUBLIC_SALE }
    ...
    mapping(address => uint256) blockTransferTimestamp; // blocks Super Frens from transferring or listing NFTs for a set amount of time
    ...
    uint32 public blockTransferSecondsSF = 14 days; 
    ...
    uint16 public maxMintAmountWhitelist = 3;
    uint16 public maxMintAmountPublicSale = 3;
    ...
    SaleType currSaleType = SaleType.WHITELIST_SUPER_FRENS;

    constructor(
        string memory _name, //InternetMade
        string memory _symbol, //IMT
    ...
    function mintWhitelistedSF(
        uint16 _mintAmount, 
        bytes32[] calldata _merkleProof
    ) external payable {
        require(currSaleType == SaleType.WHITELIST_SUPER_FRENS, "Super Frens sale is not active");
        require(
            isWhitelistedSF(msg.sender, _merkleProof),
            "You are not whitelisted"
        );
        require(mintedSF[msg.sender] == false, "You already minted");
        require(
            balanceOf(msg.sender) + _mintAmount <= maxMintAmountWhitelistSF,
            "Max mint amount exceeded"
        );
        require(
            msg.value >= costWhitelistedSF * _mintAmount,
            "Insufficient ETH amount"
        );

        mintedSF[msg.sender] = true;
        blockTransferTimestamp[msg.sender] = block.timestamp + uint256(blockTransferSecondsSF); 
    ...
    function mintFreeWithDads(uint16 _mintAmount, bytes32[] calldata _merkleProof) external {
        ...
        require(!freeMintedWithDads[msg.sender], "You already minted your free moms");
        ...
        canWhitelistMint(_mintAmount, _merkleProof)
    {
        ...
        require(
            amountMintedWhitelistPhase1[msg.sender] + _mintAmount <= maxMintAmountWhitelist,
            "Max mint amount exceeded"
        );
        ...
        canWhitelistMint(_mintAmount, _merkleProof)
    {
        ...
        require(
            amountMintedWhitelistPhase2[msg.sender] + _mintAmount <= maxMintAmountWhitelist,
            "Max mint amount exceeded"
        );
        ...
        canWhitelistMint(_mintAmount, _merkleProof)
    {
        ...
        require(
            amountMintedWhitelistPhase3[msg.sender] + _mintAmount <= maxMintAmountWhitelist,
            "Max mint amount exceeded"
        );
    ...
    modifier canWhitelistMint(uint16 _mintAmount, bytes32[] calldata _merkleProof) {
        ...
        require(
            msg.value >= costWhitelisted * _mintAmount,
            "Insufficient ETH amount"
        );
        _;
    }
    ...
    function mintWithWhiteYeti(uint16 _mintAmount) external payable {
        ...
        require(!mintedWhiteYeti[msg.sender], "You already minted");        
    ...
    function mintPublicSale(uint16 _mintAmount) external payable {
        ...
        require(
            _mintAmount <= maxMintAmountPublicSale,
            "Max mint amount exceeded"
        );
        require(
            msg.value >= costPublicSale * _mintAmount,
            "Insufficient ETH amount"
        );
    ...
    function mintOnlyOwner(uint16 _mintAmount) external payable onlyOwner {
        _mint(_mintAmount);
    }

    function isWhitelistedSF(address _user, bytes32[] calldata _merkleProof)
        public
        view
        returns (bool)
    {
    ...
    function isWhitelistedForFreeMint(address _user, uint16 _mintAmount, bytes32[] calldata _merkleProof)
        public
        view
        returns (bool)
    {
    ...
    function _mint(uint16 _mintAmount) internal {
        ...
        require(
            _totalMinted() + _mintAmount <= currPhaseMaxSupply,
            "Max supply exceeded"
        );
    ...
    function getPhaseForTokenId(uint256 tokenId) public view returns(uint8){
        if(tokenId <= phaseMaxSupply1)
            return 1;
        if(tokenId <= phaseMaxSupply1 + phaseMaxSupply2) 
            return 2;
        return 3;
    }
    ...
    function setPhase(uint8 _phase) public onlyOwner validPhase(_phase) {
        require(_phase >0 && _phase <= 3, "Phase must be between 1 and 3");
        phase = _phase;
    ...
    //SETTERS FOR COSTS
    function setCostWhitelistedSF(uint256 _newCostWhitelisted)
        public
        onlyOwner
    {
    ...
    // Free mints are allowed for 0g dad holders in phase 2(modern moms)
    function setWhitelistForFreeMint(bytes32 _merkleRoot) external onlyOwner {
        merkleRootFreeMint = _merkleRoot;
    }

    function addWhiteYetiAddresses(address[] memory _yetiAddresses) external onlyOwner {
        for(uint8 i = 0; i < _yetiAddresses.length; i++) {
            whiteYetiAddresses[_yetiAddresses[i]] = true;
        }
    }

    //WITHDRAWALS
    function withdraw() public payable {
        (bool phunsuccess, ) = payable(
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(!isSFBlockedForTransfer(_msgSender()), "Cannot list free minted tokens until enough time has passed");
        super.setApprovalForAll(operator, approved);
    }

    function isSFBlockedForTransfer(address sender) public view returns (bool) {
        return block.timestamp < blockTransferTimestamp[sender];
    }

    function setBlockTransferSecondsSF(uint32 _blockTransferSecondsSF) public onlyOwner {
        blockTransferSecondsSF = _blockTransferSecondsSF;
    }
    ...
    function _transfer(
        ...
        require(!isSFBlockedForTransfer(from), "Cannot transfer free minted tokens until enough time has passed");
