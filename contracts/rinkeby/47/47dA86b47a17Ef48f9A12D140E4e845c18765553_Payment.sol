...
contract Payment is PaymentInterface {
    ...
    function getMockAdminChannel() external view returns(AdminChannel memory) {
        UserSubscription[] memory userSubscriptions = new UserSubscription[](1);
        userSubscriptions[0] = UserSubscription(
            User(msg.sender, 'telegramId', true),
            Subscription(1, 1, msg.sender, 0, 0)
        );
        AdminChannelPlan[] memory adminChannelPlans = new AdminChannelPlan[](1);
        adminChannelPlans[0] = AdminChannelPlan(
            Plan(
                1,
                'channelTelegramId',
                5,
    ...
    function getMyUserAccount() external view returns(User memory) {
    ...
    function getMyAdminInfo() external view returns (AdminChannel[] memory) {
        return getAdminInfo(addressToUser[msg.sender].userTelegramId);
    }

    function getAdminInfo(string memory adminTelegramId) public view returns (AdminChannel[] memory) {
        address adminAddress = getUserByTelegramId(adminTelegramId).userAddress;
        require(msg.sender == owner || msg.sender == adminAddress, 'You are not allowed to call this method');
        require(userExists(adminAddress), 'No such user');
        require(userVerified(adminAddress), 'Only for verified users');
        Channel[] memory channels = getAdminChannels(adminAddress);
        AdminChannel[] memory adminChannels = new AdminChannel[](channels.length);

        for (uint i=0; i<channels.length; i++) {
            Plan[] memory channelPlans = getChannelPlans(channels[i].telegramId);
            AdminChannelPlan[] memory adminChannelPlans = new AdminChannelPlan[](channelPlans.length);
            for (uint j=0; j<channelPlans.length; j++) {
                Subscription[] memory subscriptions = getPlanSubscriptions(channelPlans[j].id);
                UserSubscription[] memory userSubscriptions = new UserSubscription[](subscriptions.length);
                for (uint k=0; k<subscriptions.length; k++) {
                    userSubscriptions[k] = UserSubscription(addressToUser[subscriptions[k].subscriberAddress], subscriptions[k]);
                }
                adminChannelPlans[j] = AdminChannelPlan(channelPlans[j], userSubscriptions);
            }
            adminChannels[i] = AdminChannel(channels[i], adminChannelPlans);
        }

        return adminChannels;
    }

    function getAdminChannels(address adminAddress) internal view returns(Channel[] memory) {
        ...
        for (uint i=0; i<channelTelegramIds.length; i++) {
            ...
            if (telegramIdToChannel[channelTelegramId].adminAddress == adminAddress) {
    ...
    function getSubscriberSubscriptions(
        string memory subscriberTelegramId
    ) public view returns(SubscriptionFullInfo[] memory) {
        address subscriberAddress = getUserByTelegramId(subscriberTelegramId).userAddress;
        require(msg.sender == owner || msg.sender == subscriberAddress, 'You are not allowed to call this method');
        require(userExists(subscriberAddress), 'No such user');
        require(userVerified(subscriberAddress), 'Only for verified users');

        User memory subscriber = getUserByTelegramId(subscriberTelegramId);
    ...
    function getMyDebtors() external view returns(UserSubscription[] memory) {
        require(userVerified(msg.sender), 'Only for verified users');
        require(userExists(msg.sender), 'No such user');
        Channel[] memory channels = getAdminChannels(msg.sender);
        UserSubscription[] memory debtors = new UserSubscription[](nextSubscriptionId);
        uint debtCount = 0;
        for (uint i=0; i<channels.length; i++) {
    ...
    function accountVerified(string memory userTelegramId) public view returns(bool) {
...
interface PaymentInterface {
  ...
  struct AdminChannel {
    Channel channel;
    AdminChannelPlan[] plans;
  }
  struct AdminChannelPlan {
    Plan plan;
  ...
  function getMockAdminChannel() external view returns(AdminChannel memory);
  function isRegistered() external view returns(bool);
  function accountVerified(string memory userTelegramId) external view returns(bool);
  function addUser(string memory userTelegramId) external;
  ...
  // for admin
  function getMyAdminInfo() external view returns (AdminChannel[] memory);
  function getAdminInfo(string memory adminTelegramId) external view returns (AdminChannel[] memory);
  function getAllDebtors() external view returns(UserSubscription[] memory);
  function getMyDebtors() external view returns(UserSubscription[] memory);
  function createPlan(
      uint amount, 
