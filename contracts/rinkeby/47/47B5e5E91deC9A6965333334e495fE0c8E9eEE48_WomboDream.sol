 ...
 */
contract WomboDream is ERC721, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;

    // Public key used to check the signature, which allows public minting of 
    // authorized URIs.
    address private _PUBLIC_VERIFICATION_KEY = 0xB2FE81febA6C245347e325487F1D654bbed95b2a;

    // To guard against replay attack, and ensure only 1 NFT can be minted for
    // a given Dream.
    mapping(string => bool) private _taskIDs;

    constructor() ERC721("Wombo Dream", "WBO") {}
     ...
     * @param target_addresses an array of addresses that will receive the NFTs
     * @param batchSize the number of tokens that we are minting
     */
    function privateBulkAirdrop(string[] memory uris, address[] memory target_addresses, uint batchSize) public onlyOwner {
         // Ensure that the input arrays are all the same size
         require(uris.length == batchSize, "input arrays must be equal to batchSize");
         require(target_addresses.length == batchSize, "input arrays must be equal to batchSize");

         for (uint i = 0; i < batchSize; i++) {
             privateMint(uris[i], target_addresses[i]);
         }
    }
     ...
     * @param target_address the address of the receiver of this NFT
     */
    function privateMint(string memory uri, address target_address) public onlyOwner {
        _safeMint(target_address, _tokenIdCounter.current());
     ...
     */
    function mint(string memory uri, bytes memory signature, string memory taskID) public {
        // Ensure that the dream hasn't been minted already
