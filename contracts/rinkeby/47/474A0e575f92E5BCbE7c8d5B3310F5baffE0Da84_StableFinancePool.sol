    ...
    Signature
{
    ...
    mapping(address => bool) public stableFinanceBlackList;
    ...
    modifier onlyWhiteList(uint256 _poolId) {
        ...
        require(
            ...
            "StableFinanceStakingPool: you are not in white list"
        );
        _;
    }
    ...
    modifier notInBlackList(address _user) {
        require(
            !stableFinanceBlackList[_user],
            "StableFinanceStakingPool: you are in black list"
        );
        _;
    }
    ...
    function setSigner(address _signer) external onlyRole(ADMIN) {
        signer = _signer;
    }
        ...
        onlyRole(ADMIN)
    {
        ...
        require(
            ...
            "StableFinanceStakingPool: invalid min max investment"
        );
        ...
        if (_payload.option == VestingOption.NoVesting) {
            require(
                ...
                "StableFinanceStakingPool: invalid delay duration"
            );
        } else {
            require(
                _payload.lockDuration > 0,
    ...
    function stableFinanceUpdatePool(
        ...
        require(pool.startJoinTime > block.timestamp, "StableFinanceStakingPool: update pool after pool started");
     ...
     */
    function stableFinanceSetSuperAdmin(
        address _newSuperAdmin
    ) external onlyRole(SUPER_ADMIN) {
        require(
            _newSuperAdmin != address(0),
            "StableFinanceStakingPool: invalid address"
        );
    ...
    function stableFinanceAddWhiteList(
        ...
        require(
            _whitelist.length > 0,
            "StableFinanceStakingPool: no items in whitelist array"
        );
     ...
     */
    function stableFinanceAddBlackList(address[] memory _blackList)
        external
        onlyRole(ADMIN)
    {
        require(
            _blackList.length > 0,
            "StableFinanceStakingPool: no items in blacl list array"
        );

        for (uint256 i; i < _blackList.length; i++) {
            stableFinanceBlackList[_blackList[i]] = true;
        }
     ...
     * @param _blackList whitelist
     */
    function stableFinanceRemoveBlackList(address[] memory _blackList)
        external
        onlyRole(ADMIN)
    {
        require(
        ...
        for (uint256 i; i < _blackList.length; i++) {
            stableFinanceBlackList[_blackList[i]] = false;
        }
        ...
        notInBlackList(msg.sender)
    {
        ...
        if (
            ...
            require(
                ...
                "StableFinanceStakingPool: invalid signature"
            );
        ...
        stableFinanceValidatePoolById(_poolId)
    {
        ...
        require(
            ...
            "StableFinanceStakingPool: invalid rewards token length"
        );

        for (uint256 i; i < stakingData.pendingReward.length; i++) {
            require(
                stakingData.pendingReward[i] >= _amount[i],
                "StableFinanceStakingPool: Insufficient reward amount"
            );
    ...
    function _claimAllReward(uint256 _poolId)
        private
        stableFinanceValidatePoolById(_poolId)
    {
        address account = msg.sender;
        ...
        for (uint256 i; i < stakingData.pendingReward.length; i++) {
            uint256 totalReward = stakingData.pendingReward[i];
            // transfer the reward to the user

            stakingData.pendingReward[i] = 0;
            stakingData.claimedReward[i] += totalReward;
    ...
    /**
     * @notice Gets number of nonce in a pool by an user
     * @param _poolId id of the pool
     * @param _account address of a user
     * @return total token claimed in a pool by a user
     */
    function stableFinanceNonceOf(uint256 _poolId, address _account)
        external
        view
        stableFinanceValidatePoolById(_poolId)
        returns (uint256)
    {
        return nonce[_poolId][_account];
    }
    ...
    function _stableFinanceDeposit(
        ...
        require(
            ...
            "StableFinanceStakingPool: pool is not started yet"
        );

        require(
            block.timestamp <= pool.endJoinTime,
            "StableFinanceStakingPool: pool is already closed"
        );
