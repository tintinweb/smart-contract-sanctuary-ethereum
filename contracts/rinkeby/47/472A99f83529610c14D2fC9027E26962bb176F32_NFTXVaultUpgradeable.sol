    ...
    INFTXVault
{
    ...
    mapping (uint => uint) public ratio;
    ...
    function setLevel(uint idToken, uint quality) public onlyOwner override returns(uint) {
        return level[idToken] = quality;
    }

    function getLevel(uint idToken) public override returns(uint) {
        return level[idToken];
    }

    function setRatio(uint price, uint quality) public onlyOwner override returns(uint) {
        require(quality != 0);
        return ratio[quality] = price;
    }

    function getRatio(uint quality) public override returns(uint) {
        require(quality != 0);
    ...
    function mintTo(
        ...
        for(uint i = 0; i < count; i++){
            uint quality = getLevel(tokenIds[i]);
            balance = balance + getRatio(quality);
        }
        _mint(to, base / 100 * balance);
        ...
        returns (uint256[] memory)
    {
        ...
        for(uint i = 0; i < amount; i++){
            uint quality = getLevel(specificIds[i]);
            balance = balance + getRatio(quality);
        }
        _burn(msg.sender, base / 100 * balance);
...
interface INFTXVault is IERC20Upgradeable {
    ...
    function setLevel(uint idToken, uint quality) external returns(uint);
    function getLevel(uint idToken) external returns(uint);
    function setRatio(uint price, uint quality) external returns(uint);
    function getRatio(uint quality) external returns(uint);
