...
contract ERC721StorefrontTest is ERC721Base {

    event CreateERC721Storefront(address owner, string name, string symbol);

    function __ERC721StorefrontTest_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI) external initializer {
        __ERC721StorefrontTest_init_unchained(_name, _symbol, baseURI, contractURI);
        emit CreateERC721Storefront(_msgSender(), _name, _symbol);
    }

    function __ERC721StorefrontTest_init_unchained(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI) internal {
        _setBaseURI(baseURI);
    ...
    // @dev TODO: should be removed later (and be part of the initial migration)
    function setDefaultApproval(address defaultApprovalAddress, bool hasApproval) external onlyOwner {
        //setting default approver for transferProxies
        _setDefaultApproval(defaultApprovalAddress, hasApproval);
    }
