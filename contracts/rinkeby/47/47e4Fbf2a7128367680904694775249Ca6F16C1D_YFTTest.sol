...
contract YFTTest is ERC721, IERC2981, Ownable, ReentrancyGuard {
    ...
    uint256 public SellPrice = 0.25 ether;
    ...
    function mint(uint256 mintAmount) public payable {
        ...
        require(mintAmount > 0, "It is not possible to mint less than 1");
        require(
            _currentId + mintAmount <= MAX_SUPPLY,
            "It is not possible to exceed the max supply"
        );

        if (msg.sender != owner()) {
            require(
                msg.value >= SellPrice * mintAmount,
                "Insufficient amount to perform minting"
            );
        }
