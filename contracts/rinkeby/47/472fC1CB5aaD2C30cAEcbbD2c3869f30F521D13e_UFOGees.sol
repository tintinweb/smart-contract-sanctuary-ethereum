...
contract UFOGees is ERC721A, Ownable, AccessControlEnumerable {
    uint256 public maxTotalSupply = 10000;
    uint256 public pricePerToken = 0.01 ether;
    bool public showArtwork = false;
    uint256 public presaleSupply = 0;
    uint256 public presaleStartTime;
    bytes32 private merkleRootGroup;
    uint8 private maxTokenWlGroup = 3;
    mapping(address => uint256) presaleMinted;
    string _baseTokenURI;
    string _placeholderURI;

    event MintEvent(address indexed wallet, uint256 amount, uint256 mintType);

    bool public presaleState = false;
    bool public publicState = false;

    constructor() ERC721A("UFOGees", "UFOGees") {
    ...
    function updatePresaleStartTime(uint256 time) public onlyOwner {
        presaleStartTime = time;
    }

    function setPresaleState(bool newState) public {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "Cannot set presale state"
        );
        presaleState = newState;
    }

    function setPublicState(bool newState) public {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "Cannot set public state"
        );
        publicState = newState;
    }

    function setShowArtwork(bool show) public {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "Cannot set to show artwork"
        );
        showArtwork = show;
    }
        ...
        returns (string memory)
    {
        ...
        if (showArtwork == true || totalSupply() == maxTotalSupply) {
            string memory baseURI = _baseURI();
            string memory strTokenId = Strings.toString(tokenId);
            return
                bytes(baseURI).length > 0
                    ? string(abi.encodePacked(baseURI, strTokenId))
                    : "";
        } else {
            string memory thumbURI = _placeholderURI;
            return string(abi.encodePacked(thumbURI));
        }
    }

    function setMerkleRoot(bytes32 newRootGroup) public {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "Cannot set merkle root"
        );
        merkleRootGroup = newRootGroup;
    }

    function presale(uint256 amount, bytes32[] calldata proof) public payable {
        require(presaleState == true, "Sale state should be presale");
        require(totalSupply() < maxTotalSupply, "Max supply reached");
        bool isValidGroup = MerkleProof.verify(
            proof,
            merkleRootGroup,
            keccak256(abi.encodePacked(msg.sender))
        );
        require(isValidGroup, "You are not in the valid whitelist");
        require(
            amount + presaleMinted[msg.sender] <= maxTokenWlGroup,
            "Your token amount reached out max"
        );
        require(
            presaleMinted[msg.sender] < maxTokenWlGroup,
            "You've already minted all"
        );
        uint256 amountToPay = (amount * pricePerToken * 9) / 10;
        require(
            amountToPay <= msg.value,
            "Provided not enough Ether for purchase"
        );
        presaleMinted[msg.sender] += amount;
        presaleSupply += amount;
        _safeMint(_msgSender(), amount);

        emit MintEvent(msg.sender, amount, 0);
    }

    function publicsale(uint256 amount) public payable {
        require(publicState == true, "Sale state should be public");
        require(totalSupply() < maxTotalSupply, "Max supply reached");
        ...
        emit MintEvent(msg.sender, amount, 1);
    }

    function checkWLUser(bytes32[] calldata proof) public view returns (bool) {
        bool isValidGroup = MerkleProof.verify(
            proof,
            merkleRootGroup,
            keccak256(abi.encodePacked(msg.sender))
