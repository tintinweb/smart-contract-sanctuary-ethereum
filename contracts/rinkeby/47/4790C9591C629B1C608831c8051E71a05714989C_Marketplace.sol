...
contract Marketplace is Ownable, Pausable {
    ...
    function removeTokenFromSale(uint256 tokenId) external returns (bool) {
        require(nft.ownerOf(tokenId) == msg.sender, "YOU_DO_NOT_OWN_THE_NFT");
    ...
    function putTokenOnSale(
        ...
        require(nft.ownerOf(_tokenId) == msg.sender, "YOU_DO_NOT_OWN_THE_NFT");
        require(
            nft.isApprovedForAll(nft.ownerOf(_tokenId), address(this)),
            "NFT_NOT_APPROVED_FOR_TRANSFER"
        );

        require(
            msg.value == platformFee,
            "INSUFFICIENT_BNB_AMOUNT"
        );
        ...
        (bool sent, bytes memory data) = owner().call{value: platformFee}("");
        require(sent, "Failed to send BNB");

        emit PutTokenOnSale(_tokenId, _amount, msg.sender);
        return true;
    }
        ...
        returns (orderDetails memory)
    {
        ...
        require(
            msg.value == saleItems[tokenId].price,
            "INSUFFICIENT_BNB_AMOUNT"
        );
        ...
        // Transfer BNB to token owner
        (bool sent, bytes memory data) = saleItems[tokenId].owner.call{value: msg.value}("");
        require(sent, "Failed to send BNB");
