...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    constructor(address token, address nftContarct) {
        _token = token;
        _nftContract = nftContarct;
        _totalPayments = 0;
    }

    function pushData(NFTListing[] calldata _data) public {
        for (uint256 i = 0; i < _data.length; i++) {
            uint256 _tokenId = _data[i].tokenId;
            uint256 _price = _data[i].price;
        }
    }

    // List the NFT on the marketplace
    function listNft(NFTListing[] memory _data) public nonReentrant onlyOwner {
        _nftCount.increment();
        ...
        for (uint256 i = 0; i < _data.length; i++) {
            ...
            _idToNFT[_tokenId] = NFT(
                ...
                _price,
                true,
                ERC721(_nftContract).tokenURI(_tokenId)
            );
