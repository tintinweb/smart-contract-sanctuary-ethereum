...
contract MonolithYachtClub is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    uint private price = 0.16 ether;
    string private URL;
    uint private counter = 0;
    bool private frozen = false;

    constructor() ERC721("Monolith Yacht Club", "YAHT") {
        setURI("https://gateway.pinata.cloud/ipfs/QmXAQ6SVTGz26C7VyrKL6TREob8nuMTzzALuwtXcSqPLH7/");
        address wallet = 0xDC85F0742eac777dE243529F08Ed657634ecbE39;
        uint x = 1;
        uint y = 5;
        for (uint i = x; i <= y; i++) {
            _mint(wallet, ++counter);
            string memory uri = _createTokenURI(counter);
    ...
    function publicMint(uint tokenAmount) external payable{
         if (counter <= 10000){
            revert('publicMint not open yet');
        }
        if (tokenAmount > 100){
            revert('Maximum 100 per request');
        }
        if (counter + tokenAmount > 60000) {
            revert('Supply Not Available, Try Less');
        }
        require(msg.value <= msg.sender.balance, "Not enough ETH in wallet");
        require(price*tokenAmount <= msg.value, "Not enough ETH sent; Price is .16 eth per NFT");
    ...
    function privateMint(uint tokenAmount, address wallet) external onlyOwner{
        if (counter + tokenAmount > 60000) {
            revert('Supply Not Available, Try Less');
        }
        for (uint i = 1; i <= tokenAmount; i++) {
    ...
    function _createTokenURI(uint id) internal view returns (string memory) {
        if (frozen == true){
            revert('Token URI Frozen');
        }
    ...
    function setTokenURI(uint startId, uint endId, string memory Newuri) external onlyOwner {
        if (frozen == true){
            revert('Token URI Frozen');
        }
