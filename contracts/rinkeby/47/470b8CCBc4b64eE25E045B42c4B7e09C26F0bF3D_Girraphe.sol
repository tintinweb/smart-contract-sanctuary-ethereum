...
contract Girraphe is ERC721A, Ownable, ReentrancyGuard {
  ...
  function publicMint(uint256 amount) external payable nonReentrant{
        ...
        require(totalSupply() + amount <= maxSupply, "Maximo suply atingido!");
        require(msg.value >= amount * publicPrice,"Nenhum ETH foi enviado!");
        require(publicMints[msg.sender] + amount <=maxPublicMints,"Voce atingiu o maximo para mint!");
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    require(_exists(_tokenId), "ERC721Metadata: Consulta URI para ficha inexistente");
  ...
  function setMaxMintAmountPerTx(uint256 _maxMintAmountPerTx) public onlyOwner {
    maxPublicMints = _maxMintAmountPerTx;
  }
