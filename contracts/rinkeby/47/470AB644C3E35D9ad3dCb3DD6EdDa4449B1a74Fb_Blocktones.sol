...
contract Blocktones is ERC721Enumerable, Ownable {
    ...
    bool publicLive = false;
    bool whitelistLive = false;
    ...
    function publicMint(uint256 quantity) external payable {
        require(publicLive);
        require(mintedCount[msg.sender] + quantity < 3, 'Individual mint limit exceeded');
    ...
    function whitelistMint(uint256 quantity, bytes32[] calldata proof) external payable {
        require(whitelistLive);
        require(mintedCount[msg.sender] + quantity < 3, 'Individual mint limit exceeded');
        require(quantity > 0, 'Invalid quantity entered');
        require(msg.value >= quantity * price);
        require(verify(merkleRoot, keccak256(abi.encodePacked(msg.sender)), proof));
        mint(msg.sender, quantity);
    }
    ...
    function withdraw() public onlyOwner {
        // todo: the splits for the team on withdrawals
