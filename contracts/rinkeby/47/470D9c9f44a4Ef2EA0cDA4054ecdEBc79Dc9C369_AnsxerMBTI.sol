...
contract AnsxerMBTI is ERC721Psi, Ownable, Pausable {
  ...
  event NFTStaked(address owner, uint256[] tokenIds);
  event NFTUnstaked(address owner, uint256[] tokenIds);
  ...
  uint32 public constant STAKING_DEADLINE = 4_294_967_295; // 2106-02-07
  ...
  function setAllowlist(address[] calldata addrs, uint256[] calldata maxMints) external onlyOwner {
    ...
    for (uint i; i < addrs.length; i++) {
      allowlists[addrs[i]] = Allowlist(uint8(maxMints[i]), 0);
    }
  }
  ...
  function allowlistMint(Order[] calldata orders, bool staked, uint8 v, bytes32 r, bytes32 s) external whenNotPaused callerIsUser {
    require(isAllowlistSaleOn, "Contract is not ready for allowlist");
    require(_isAllowlist(msg.sender, v, r, s), "You are not eligible for allowlist mint");
    ...
    allowlists[msg.sender].minted += uint8(totalAmount);
  ...
  function _isAllowlist(address addr, uint8 v, bytes32 r, bytes32 s) private view returns (bool) {
    bytes32 msgHash = keccak256(abi.encodePacked(address(this), addr));
    bytes32 messageDigest = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", msgHash));
    return _signer == ecrecover(messageDigest, v, r, s);
  }

  function getAllowlist(address addr, uint8 v, bytes32 r, bytes32 s) private view returns (address) {
    bytes32 msgHash = keccak256(abi.encodePacked(address(this), addr));
    bytes32 messageDigest = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", msgHash));
