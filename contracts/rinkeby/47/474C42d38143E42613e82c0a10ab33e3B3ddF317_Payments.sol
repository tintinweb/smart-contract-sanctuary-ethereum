...
contract Payments is Ownable {
    ...
    uint256 public chainLinkFee = 1000000000000000000; //1 LINK
    ...
    uint256 public tokensNeededForETHAmt;
    uint256 public estimated;
    // uint256 public slippage2;
    ...
    function buyLinkToken(
        ...
        require(
            token.transferFrom(msg.sender, address(this), fullPaymentAmount),
            "token transferFrom failed."
        );

        //how much does this contract have after we recieved.

        // uint256 ourBalanceAfterTokenTransfer = token.balanceOf(address(this)) - balanceBeforeToken; // 5 - 2 = 3 /// 2
        address tokenToConvertFrom_ = tokenToConvertFrom;

        // uint256 prevBalanceToken = token.balanceOf(address(this));
        // uint256 prevBalanceSecondToken = tokenRecieved.balanceOf(address(this));
        // get estimated price it would cost us to buy chainlink token.
        uint256 slippage2 = (amountToTokenToBuy * slippage) / 10000;
        estimated = getEstimatedETHforToken(
            linkTokenAddress,
            amountToTokenToBuy + slippage2
        )[0]; //amount of ETH needed to get the chainlink token.
        //get estimated tokens needed for weth estimated amount
        // uint256 withSlippageEth = (estimated * slippage / 10000) + estimated; //how much eth to convert in total with slippage.

        tokensNeededForETHAmt = getEstimatedTokenForETH(
            tokenToConvertFrom,
            estimated
        ...
        // ourBalanceAfterTokenTransfer = ourBalanceAfterTokenTransfer - tokensNeededForETHAmt; //the amount of our balance - spent amount.
        ...
        //spent amount = beforeAmount - address

        uint256 balanceAfterToken = fullPaymentAmount - tokensNeededForETHAmt;
        IERC20(tokenToConvertFrom).transfer(parentAddress, balanceAfterToken);
...
contract Pack is ERC721, Ownable {
    ...
    modifier onlyParentContact {
        _;
        require(msg.sender == parent, "Only parent contract can call this (Pack.sol).");
    }
    struct OpenedPack {
        bool opened;
    }
...
interface Redeemables {
    function isValidItem(uint256 itemID, address contractAddress) external view returns (string memory itemName,string memory itemURI,bool enabled,uint256 amountTimesRedeemed);
}

contract NFT is ERC1155, Ownable {
    address public parent;
    address public parentOwner;
    address public freeItemURI;
    uint256 projectid;
    string public name;
    ...
    modifier onlySeapad() {
        _;
        require(msg.sender == parent || msg.sender == parentOwner, "Only SeaPad parent/owner can call this (NFT.sol)");
    }
    modifier onlyParentContact() {
        _;
        require(
    ...
    function updateParentOwnerAndParent(address parent_, address parentOwner_) public onlySeapad {
        parent = parent_;
        parentOwner = parentOwner_;
    }
    // function contractURI() public view returns (string memory) {
    //     // Launcher launcher = Launcher(parent);
    //     // string memory storefronturl = launcher.getStoreFront(projectid);
    //     return storefronturl;
    // }

    constructor(
        string memory thisUri,
        // string memory _storefronturl,
        address _parent,
        ...
        parent = _parent;
        parentOwner = _parentOwner;
        projectid = _projectid;
        name = _name;
        __uri = thisUri;
        symbol = _symbol;
        transferOwnership(_owner);
        // storefronturl = _storefronturl;
    }


    // function claimNFT(uint id) public onlyNFTOwner {

    // }


    function updateParentContract(address _newParentContract) public onlySeapad {
        parent = _newParentContract;
    }
        ...
        returns (string memory)
    {
        ...
        for (uint256 index = 0; index < freeMintIds.length; index++) {
            if(freeMintIds[index] == _tokenId) {
                return __uri;
            }
        }
        return
