...
contract StakingClaim is IStakingClaim, Ownable, ReentrancyGuard {
    ...
    function ClaimToStaking(uint256 _roothashIdx, bytes32 _root, bytes32[] memory _proof, bool[] memory _proofFlags, bytes32[] memory _leaves) 
        external override 
        nonReentrant
    {
        bytes32 root = rootHashInfos[_roothashIdx].rootHash;
        //require(_root==root, "ClaimToStaking_ERR00");
...
interface IStakingClaim {
    ...
    // [allUser] 스테이킹 보상 수령
    function ClaimToStaking(uint256 _roothashIdx, bytes32 _root, bytes32[] memory _proof, bool[] memory _proofFlags, bytes32[] memory _leaves) external;
