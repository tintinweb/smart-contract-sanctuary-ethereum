...
contract MDAContract is ERC721Enumerable, Ownable, ERC721Burnable {
    ...
    constructor(string memory baseURI) ERC721("Mutated Anti Doodles", "MAD") {
        setBaseURI(baseURI);// set base url of Ipfs
        pause(true);// set _pause true when deploy contract
    }

    modifier saleIsOpen {
        require(_totalSupply() <= MAX_ELEMENTS, "Sale end");// if total_supply > max_element => sale ended
        if (_msgSender() != owner()) {
