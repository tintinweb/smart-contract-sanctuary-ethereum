...
contract JuniorNFT is IJuniorNFT, ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _baseURI, string memory _defaultURI)
        ERC721A("Atomrigs Junior's NFT", "Junior's NFT")
        Ownable()
    {
        baseURI = _baseURI;
    ...
    modifier onlyOwnerOrAdmin() {
        ...
        if (msg.sender != Admin && msg.sender != owner())
            revert NotOwnerNorAdmin("caller is not the Admin");
        _;
    }
    ...
    {
        if (
            keccak256(abi.encodePacked(tokenURIfix)) !=
            ...
            return
                string(
                    abi.encodePacked(defaultURI, _toString(tokenId), ".json")
                );
        }
    }
