...
contract Drop is ERC721, ReentrancyGuard, Ownable {
    ...
    uint public metadataRandomizedCounter = 0;
    ...
    constructor(
        ...
        for (uint i; i < chances[2].length; i++) {
    ...
    function randomMetadata(uint tokenId) private {
        ...
        for (uint i; i < metadata.traitsChance.length; i++) {
            if (uint(keccak256(
                ...
                for (uint j; j < metadata.valuesChance[i].length; j++) {
    ...
    function randomMonthlyTrait(uint tokenId) private {
        for (uint i; i < metadata.dynamicChance.length; i++) {
            if (uint(keccak256(
    ...
    function randomzieMonthlyTraits(uint start, uint end) public onlyOwner {
    ...
    function mint(uint16[] calldata traits, bytes32[] calldata _merkleProof, string[] calldata uris) external payable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory) {
        ...
        for (uint i; i < traitIndexes[tokenId].length; i++) {
        ...
        for (uint i; i < metadata.choosableTraits.length; i++) {
            encodedMetadata = string(abi.encodePacked(
                encodedMetadata,
                '{"trait_type":"',
