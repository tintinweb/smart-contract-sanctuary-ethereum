...
contract TEST is ERC721Enumerable, Ownable {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(balanceOf(msg.sender) + _mintAmount <= maxMintAmountPerWallet, "Trying to mint more than max allowed per wallet");
        if (msg.sender != owner()) {
            if(onlyWhitelisted == true) {
                require(isWhitelisted(msg.sender), "User is not whitelisted!");
                uint256 ownerMintedCount = addressMintedBalance[msg.sender];
                require(ownerMintedCount + _mintAmount <= NFTperWhitelistAddressLimit, "Max NFT per Whitelist Address Exceeded");
            }
  ...
  function gift(address[] calldata addresses) public onlyOwner {
    ...
    for (uint256 i = 0; i < addresses.length; i++) {
        _safeMint(addresses[i], (supply + (i+1)));
        }
  }    
