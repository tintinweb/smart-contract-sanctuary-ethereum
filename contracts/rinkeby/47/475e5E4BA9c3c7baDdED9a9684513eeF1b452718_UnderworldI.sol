...
contract UnderworldI is ReentrancyGuard, AdminControl, ERC721 {
  ...
  function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
    ...
    // avoids recursively calling safeTransferFrom below infinitely
    if(locked) {
      return;
    }

    // so that the next two safeTransferFrom calls don't cause recursion
    locked = true;

    if (tokenId == 1) {
      safeTransferFrom(from, to, 2);
      safeTransferFrom(from, to, 3);
    } else if (tokenId == 2) {
      safeTransferFrom(from, to, 1);
      safeTransferFrom(from, to, 3);
    } else if (tokenId == 3) {
      safeTransferFrom(from, to, 1);
      safeTransferFrom(from, to, 2);
