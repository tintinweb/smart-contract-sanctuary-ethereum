...
contract Testnet is Twinesis {
    address private recipient = 0x8fE5EFB29F4D4F48dFDA2D72454152587b59A95F;

    constructor()
        Twinesis(
            0x623236cC70D5EF2c0233d13EC906b6E69658a5e4,
            0xB5D94cDD855f587b5f90C7699e6018419Ee02571
        )
    {
 ...
 *                          -------------                     line
 *
 *      TWINESIS Contract .................................... 84
 *
 *          1.  Libraries .................................... 85
 *
 *          2.  Events ....................................... 94
 *
 *          3.  State variables .............................. 98
 *
 *          4.  Constructor .................................. 148
 *
 *          5.  Access Control ............................... 183
 *
 *                  A.  Withdrawal ........................... 200
 *
 *          6.  Minting ...................................... 216
 *
 *                  A.  Public minting ....................... 259
 *
 *                  B.  Pre sale Minting ..................... 290
 *
 *                  C.  Internal minting ..................... 334
 *
 *          7.  Metadata ..................................... 368
 *
 *                  A.  Contract URI ......................... 379
 ...
 */
abstract contract Twinesis is ITwinesis, ERC721, AccessControl {
    ...
    address private constant ARTIST =
        0x31a23E694c8c6Ce4B7C44f7f78A36f3aC9666989; // twinnytwin.eth

    /// @notice Developer address
    /// @dev Used to add addresses to pre sale goodlist
    address private constant DEV = 0x60A74153d196B0537248f5a6254e7C6751c02229; // nicoacosta.eth
    ...
    /// @dev Used in pre sale and public minting functions
    /// @return PUBLIC_MINTING_START_DATE Public minting start date
    // uint256 public constant PUBLIC_MINTING_START_DATE = 1646801940; // 3/9/2022 01:59:00
    uint256 public constant PUBLIC_MINTING_START_DATE = 1647298800; // 2022-03-14 20:00:00

    /// @notice Pre sale minting start date
    /// @dev Used in pre sale minting functions
    /// @return PRE_SALE_START_DATE Pre sale minting start date
    // uint256 public constant PRE_SALE_START_DATE = 1646801520; // 3/9/2022 01:52:00
    uint256 public constant PRE_SALE_START_DATE = 1647126000; // 2022-03-12 20:00:00
    ...
    constructor(address withdrawalAddress1_, address withdrawalAddress2_)
        ERC721("CNFTLOL", "CNFT2")
    {
        // Set withdrawal addresses
        _withdrawalAddress1 = withdrawalAddress1_;
        _withdrawalAddress2 = withdrawalAddress2_;

        // Set creator role for artist and dev. Enables pre sale list goodlist control
    ...
    /// --------------------------
    /// 6.A. Pre sale minting
    /// --------------------------

    /// @notice Verifies that pre sale period is active
    modifier preSalePeriod() {
        // Pre sale must have started
        require(
            block.timestamp > PRE_SALE_START_DATE,
            "Pre sale has not started yet"
        );
        // Public minting must not have started
        require(
            block.timestamp < PUBLIC_MINTING_START_DATE,
            "Pre sale has already ended"
        );
        _;
    }

    /// @notice Mint one token (pre sale)
    /// @dev Verifies public minting start date, max tokens and ETH received. Mints next id available.
    function preSaleMintTwin()
        external
        payable
        onlyRole(PRE_SALE_GOODLIST_ROLE)
        preSalePeriod
        canMintOne
    {
        _mintOneToken();
    ...
    /// @notice Mint several tokens (pre sale)
    /// @dev Verifies public minting start date, max tokens and ETH received. Mints next ids available.
    /// @param amount The number of tokens to mint
    function preSaleMintTwins(uint256 amount)
        external
        payable
        onlyRole(PRE_SALE_GOODLIST_ROLE)
        preSalePeriod
    ...
    //     return
    //         "https://ipfs.io/ipfs/bafybeifqa53kejaliy6lmo5auu54vv4vobnc2q7qqsb54k6nm6apzohg2q/";
    // }
    ...
    function _metadataBaseURI() private view returns (string memory) {
        ...
        return
            // unrevealed metadata base URI
            "https://ipfs.io/ipfs/bafybeifyn6rh2lkr7hb3us7ach6k7z3essseq3h3amigrtauvxfcal4qne/";
    }
    ...
    function revealRarities(string memory _newBaseURI)
        public
        onlyRole(CREATOR_ROLE)
    {
        require(!raritiesHaveBeenRevealed(), "Metadata already revealed");
        _revealedBaseURI = _newBaseURI;
    }
    ...
    // {
    //     if (metadataHasBeenRevealed()) {
    //         return _reavealedTokenURI(tokenId);
    //     }
    //     return string(abi.encodePacked(_metadataBaseURI(), "unrevealed.json"));
    // }
