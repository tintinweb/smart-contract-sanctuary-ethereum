...
error MaxPresaleMintsExceeded(uint256 requested, uint256 maximum);
...
contract RicRacStorefront is Pausable, Ownable, PaymentSplitter {
    ...
    function mintPresale(uint8 numberOfTokens, bytes32[] calldata merkleProof)
        ...
        whenNotPaused
    {
        if (numberOfTokens + _presalePurchases[_msgSender()] > _maxPresaleMints) {
            revert MaxPresaleMintsExceeded({
                requested: numberOfTokens,
                maximum: _maxPresaleMints
            });
        }
