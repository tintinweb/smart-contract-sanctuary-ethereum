...
contract RTFKTRaffle is VRFConsumerBaseV2 {  
    constructor(
        ...
        // Setup VRF Subscription and add this contract as consumer
    ...
    // Much of the algorithm credit to https://gist.github.com/cleanunicorn/d27484a2488e0eecec8ce23a0ad4f20b
    function tokenIdRaffle(uint256[] calldata tokenIds, uint256 numOfWinners) external view returns (uint256[] memory) {
        ...
        require(tokenIds.length > 0, "Entry list cannot be empty");
        ...
        // Set the initial random seed from VRF
    ...
    function walletRaffle(address[] calldata addresses, uint256 numOfWinners) external view returns (address[] memory) {
        require(vrfRandomValue != 0, "VRF fetch not complete");
        require(addresses.length > 0, "Entry list cannot be empty");

        uint256 uniqueWallets = _getUniqueWalletCount(addresses);
        require(numOfWinners <= uniqueWallets, "Entries and winners length mismatch");
    ...
    function _getUniqueWalletCount(address[] calldata addresses) internal pure returns (uint256) {
        address[] memory uniqueAddresses;
        uint256 count = 0;

        for (uint256 i = 0; i < addresses.length; i++) {
            bool exists;
            for (uint256 j = 0; j < uniqueAddresses.length; j++) {
                if (addresses[i] == uniqueAddresses[j]) {
                    exists = true;
                }
            }

            if (!exists) {
                uniqueAddresses[count] = addresses[i];
                count++;
            }
        }
    ...
    function setupVrfSubscription() public isAuthorizedOwner {
        address[] memory consumers = new address[](1);
        consumers[0] = address(this);

        vrfSubscriptionId = vrfCoordinator.createSubscription();
        vrfCoordinator.addConsumer(vrfSubscriptionId, consumers[0]);
    }

    function getVrfSubscriptionDetails() external view returns (
        uint96 balance,
        uint64 reqCount,
        address owner,
        address[] memory consumers
    ) {
        return vrfCoordinator.getSubscription(vrfSubscriptionId);
    }

    function addVrfConsumers(address consumer) external isAuthorizedOwner {
        vrfCoordinator.addConsumer(vrfSubscriptionId, consumer);
    }

    function removeVrfConsumer(address consumer) external isAuthorizedOwner {
        vrfCoordinator.removeConsumer(vrfSubscriptionId, consumer);
    }

    // Cancels VRF subscription and sends LINK balance to the addr param
    function cancelVrfSubscription(address addr) external isAuthorizedOwner {
        vrfCoordinator.cancelSubscription(vrfSubscriptionId, addr);
    }

    function fundVrfSubscription(uint256 amount) external isAuthorizedOwner {
        require(linkToken.balanceOf(address(this)) >= amount, "Not enough LINK in contract");
    ...
    function getVrfSeed() external isAuthorizedOwner returns (uint256) {
        require(vrfSubscriptionId != 0, "VRF subscription not set");
        require(vrfRandomValue == 0, "VRF random number needs reset");
    ...
    function setVrfParams(
        ...
        vrfCoordinator = VRFCoordinatorV2Interface(cooridnatorAddress);
