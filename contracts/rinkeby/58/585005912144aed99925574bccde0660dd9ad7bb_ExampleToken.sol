...
contract ExampleToken is ERC721SeaDrop {
    ...
    function _tokenJson(uint256 tokenId) internal view returns (string memory) {
        ...
        "<style>"
        "  .base { fill: white; font-family: serif; font-size: 24px; }"
        "  .roll { fill: yellow; font-family: serif; font-size: 48px; text-decoration: underline }"
        "  .block { fill: red; font-family: serif; font-size: 24px; }"
        "</style>"
        "<rect width='100%' height='100%' fill='black' />"
        "<text x='50%' y='50%' dominant-baseline='middle' text-anchor='middle' style='color: white; font: ";
        parts[1] = _font(tokenId);
        parts[2] = "', font-size: ";
        parts[3] = _fontSize(tokenId);
        parts[4] = "'>Test #";
        ...
        string memory json = string(
            abi.encodePacked(
                ...
                ' Please note that this token has no value or warranty of any kind.", "image_data": "',
                output,
    ...
    /**
     * @notice Returns a decreasing font size based on token id.
     */
    function _fontSize(uint256 tokenId) internal pure returns (string memory) {
      if (tokenId == 0) {
        return "100";
      }

      uint256 size = 1 / tokenId * 100;

      // Return minimum size of 10
      return size > 10 ? Strings.toString(size) :  "10";
    }
}
