   ...
   enum ContractStatus {
    ...
    function seedIntuitivePoems(uint256[] memory poems) external onlyOwner {
        ...
        for (uint256 i = 0; i < poems.length; i++) {
            specialPoems[poems[i]].poemType = PoemType.Intuitive;
        }
    ...
    function seedPuzzlePoems(uint256[] memory poems) external onlyOwner {
        ...
        for (uint256 i = 0; i < poems.length; i++) {
            specialPoems[poems[i]].poemType = PoemType.Puzzle;
        }
        puzzlePoemsSet = true;
    }

    function setSpecialPoemClaimed(uint256 tokenId) external view onlyOwner {
        require(
            tokenId >= 0 && tokenId < collectionSize,
            "Token is out of bounds"
        );
        SpecialPoem memory poem = specialPoems[tokenId];
    ...
    function getSpecialPoem(uint256 tokenId) external view returns (SpecialPoem memory poem) {
      require(
            tokenId >= 0 && tokenId < collectionSize,
            "Token is out of bounds"
        );
