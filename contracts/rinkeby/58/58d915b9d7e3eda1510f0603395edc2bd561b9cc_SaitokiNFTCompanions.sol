...
contract SaitokiNFTCompanions is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public CAT_CHANCE = 20;
    uint256 public NINJA_CHANCE = 20;
    uint256 public PANDA_CHANCE = 20;
    uint256 public SAMURAI_CHANCE = 20;

    mapping(address => bool) private _allowList;
    mapping(uint256 => uint256) internal _genes;
    mapping(uint256 => uint256) internal _races;
    ...
    event TokenMinted(uint256 indexed tokenId, uint256 newGene, uint256 newRace);
    ...
    function mintAllowList(uint8 numberOfTokens) external payable nonReentrant {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            _genes[tokenId] = getGen(i);
            _races[tokenId] = getRace(i);
            emit TokenMinted(tokenId, _genes[tokenId], _races[tokenId]);
        }
    }
    ...
    function setTypeChance(uint256 common, uint256 uncommon, uint256 rare, uint256 epic) public onlyOwner {
    ...
    function setRaceChance(uint256 cat, uint256 ninja, uint256 panda, uint256 samurai) public onlyOwner {
        CAT_CHANCE = cat;
        NINJA_CHANCE = ninja;
        PANDA_CHANCE = panda;
        SAMURAI_CHANCE = samurai;
    }
    ...
    function hasDiscount(address account) public view returns (bool) {
        uint256 ethAmountMononoke = getTokenAmountPrice(IERC20(MononokeInu).balanceOf(account), MononokeInu);
        uint256 ethAmountFortune = getTokenAmountPrice(IERC20(Fortune).balanceOf(account), Fortune);
        uint256 ethAmountSaitoki = getTokenAmountPrice(IERC20(SaitokiInu).balanceOf(account), SaitokiInu);
    ...
    function raceOf(uint256 tokenId) public view returns (uint256 gene) {
        return _races[tokenId];
    }

    function getGen(uint i) private view returns (uint256) {
        uint p = (random(i) % 100) + 1;
        if (p <= COMMON_CHANCE) {
            return 0;
    ...
    function getRace(uint i) private view returns (uint256) {
        uint p = (random(i) % 100) + 1;
        if (p <= CAT_CHANCE) {
            return 0;
        } else if (p <= (CAT_CHANCE + NINJA_CHANCE)) {
            return 1;
        } else if (p <= (CAT_CHANCE + NINJA_CHANCE + PANDA_CHANCE)) {
            return 2;
        } else if (p <= (CAT_CHANCE + NINJA_CHANCE + PANDA_CHANCE + SAMURAI_CHANCE)) {
            return 3;
        } else {
            return 4;
        }
    ...
    function random(uint i) private view returns (uint8) {
        return uint8(uint256(keccak256(abi.encodePacked(block.timestamp.add(i), block.difficulty.add(i), blockhash(block.number.add(i)%257))))%251);
    }
