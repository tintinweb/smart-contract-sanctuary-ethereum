...
contract MetaBloqSignature is EIP712Upgradeable{
    ...
    function _hash(MetaBloqs memory metabloqs) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
                keccak256("MetaBloqs(address userAddress,uint256 landPrice,uint256 nonce)"),
...
interface LANDRegistry{
      function buyNewParcel(int x, int y, address beneficiary, string memory _uri) external returns(uint);
      function buyMultipleParcels(int[] memory x, int[] memory y, address beneficiary, string memory _uri) external returns(uint);
}
abstract contract ERC721Verifiable is IERC721Upgradeable {
...
contract Marketplace is OwnableUpgradeable,MarketplaceStorage,PausableUpgradeable,MetaBloqSignature{
     using AddressUtils for address;
//   LANDRegistry public MetaBloqLands;
  address  public Signer;
  event LandMinted(address user, uint tokenId, uint amount);
  ...
  function setLegacyNFTAddress(address _legacyNFTAddress) external onlyOwner {
   ...
   function initialize(address _acceptedToken, address _legacyNFTAddress,string memory _domain, string memory _version)external initializer{
     require(_acceptedToken.isContract(), "The accepted token address must be a deployed contract");
      legacyNFTAddress = _legacyNFTAddress;
      acceptedToken =IERC20Upgradeable (_acceptedToken);
      __Ownable_init();  
      __Pausable_init() ;
      __MetaBloqSigner_init(_domain, _version);
      Signer=msg.sender;
   }
  ...
  mapping(address=>mapping(uint=>bool)) public isUsedNounce;

  function mintLands(int x, int y, string memory _uri,MetaBloqs memory signer) external {
    require(getSigner(signer)==Signer,"!Signer");
    require(signer.userAddress==msg.sender,"!User");
    require(!isUsedNounce[msg.sender][signer.nonce],"Invalid Nonce");
    isUsedNounce[msg.sender][signer.nonce]=true;
    acceptedToken.transferFrom(msg.sender, address(this), signer.landPrice);
    uint tokenId= LANDRegistry(legacyNFTAddress). buyNewParcel(x,y,msg.sender, _uri);
    emit LandMinted(msg.sender,tokenId, signer.landPrice);

  }

  function mintsMultipleLands(int[] memory x, int[] memory y, string memory  _uri,MetaBloqs memory signer)external{
    require(getSigner(signer)==Signer,"!Signer");
    require(signer.userAddress==msg.sender,"!User");
    require(!isUsedNounce[msg.sender][signer.nonce],"Invalid Nonce");
    isUsedNounce[msg.sender][signer.nonce]=true;
     ...
     uint tokenId = LANDRegistry(legacyNFTAddress). buyNewParcel(x[i],y[i],signer.userAddress, _uri);
      emit LandMinted(msg.sender,tokenId, signer.landPrice);
     }
  ...
  /**
    ...
    * @param assetId - ID of the published NFT
    */
  function cancelLandOrder(uint256 assetId) public whenNotPaused {
   ...
   internal returns (Order memory)
  {
    ...
    if (ownerCutPerMillion > 0) {
      ...
      saleShareAmount = (price *ownerCutPerMillion)/1000000;
  ...
  function withdrawERC20Tokens(address _user, uint amount) external onlyOwner{
    acceptedToken.transfer(_user, amount);
  }
