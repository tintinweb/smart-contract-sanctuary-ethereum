...
contract NudieCommunityAirdrop is ERC1155, ERC2981, AccessControl, Pausable, ERC1155Burnable, ERC1155Supply {

    uint256 private randNonce = 0;
    mapping(address => bool) private holders;

    constructor() ERC1155("https://nudiecommunity.mypinata.cloud/ipfs/QmUnLQUxKJKwFFbTbNQJN3kqLckQbwprNpTgS1vQJqyp3d/{id}.json") {
    ...
    function airdrop_cycle(address[] calldata recipients, uint256 _modulus) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < recipients.length; i++) {
            uint256 rndTokenId = randMod(_modulus) + 1;
            if(holders[recipients[i]] != true) {
                _mint(recipients[i], rndTokenId, 1, "");
