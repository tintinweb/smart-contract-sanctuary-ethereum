...
//                                                                                 //
/////////////////////////////////////////////////////////////////////////////////////
contract HOA is ERC721Enumerable, Ownable {
    ...
    address public constant creatorAddress = 0x525997255bd880dFDf908471c33255B51659f63c;
    mapping (address => uint) public ownWallet;

    event PauseEvent(bool pause);
    event welcomeToHOA(uint256 indexed id);
    event NewPriceEvent(uint256 price);
    event NewMaxElement(uint256 max);

    bytes32 public merkleRoot;

    constructor(string memory baseURI, string memory sampleURI) ERC721("House of Assets", "HOA"){
        setBaseURI(baseURI);
        setSampleURI(sampleURI);
    }
    ...
    function isStatus() public view returns (uint256) {
      if(block.timestamp < 1653925660) {
          return 0; // Before Mint
      } else if(block.timestamp >= 1653925660 && block.timestamp < 1653925660 + 10 minutes) {
        return 1; // Allowlist Sale Period
      } else {
        return 2; // Public Sale Period
      }
    }
    ...
    function whitelistMint(uint256 _tokenAmount, bytes32[] memory _merkleProof) public payable saleIsOpen {
      bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
      require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Invalid proof");
      uint256 total = totalToken();
      require(_tokenAmount <= 10, "Max limit");
      require(ownWallet[msg.sender] + _tokenAmount <= 10, "Maxium is 10");
    ...
    function mintItem(uint256 _tokenAmount) public payable saleIsOpen {

        uint256 total = totalToken();
        require(_tokenAmount <= 2, "Max limit");
        require(ownWallet[msg.sender] + _tokenAmount <= 2, "Maxium is 2");
    ...
    function _mintAnElement(address _to, uint256 _tokenId) private {
        ...
        emit welcomeToHOA(_tokenId);
    }

    function giftMint(address _addrs, uint256 _tokenAmount) public onlyOwner {
        uint256 total = totalToken();
        require(total + _tokenAmount <= MAX_ELEMENTS, "Max limit");
        require(ownWallet[_addrs] + _tokenAmount <= 100, "Maxium is 100");
        for(uint8 i = 1; i <= _tokenAmount; i++){
          _mintAnElement(_addrs, total + i);
        }
    }
