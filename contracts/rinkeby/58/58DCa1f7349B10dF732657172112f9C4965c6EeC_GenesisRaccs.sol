...
contract GenesisRaccs is ERC721A, Ownable {
    ...
    { setUriPrefix(_initBaseURI); setHiddenMetadataUri(_hiddenMetadataUri); }

    modifier mintCompliance(uint256 _mintAmount) { 
        require(_mintAmount > 0 && _mintAmount < MAX_MINT_AMOUNT + 1, "Invalid mint amount!");
        require(totalSupply() + _mintAmount <= maxSupply, "Max supply exceeded. Raccs sold out!");
    ...
    function mint(bytes32[] calldata _merkleProof) public payable mintCompliance(MAX_MINT_AMOUNT) mintPriceCompliance(MAX_MINT_AMOUNT) {
        require(!paused, "Oops! The contract is paused!");
        require(!claimed[msg.sender], "Already claimed.");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Invalid merkle proof.");
        
        _safeMint(_msgSender(), MAX_MINT_AMOUNT);
        claimed[msg.sender] = true;
    }

    function mintForAddress(address _receiver) public mintCompliance(MAX_MINT_AMOUNT) onlyOwner { _safeMint(_receiver, MAX_MINT_AMOUNT); }
    ...
    function walletOfOwner(address _owner) public view returns (uint256[] memory) {
        ...
        while(ownedTokenIndex < ownerTokenCount && currentTokenId <= maxSupply) {
            ...
            if(!ownership.burned && ownership.addr != address(0)) { latestOwnerAddress = ownership.addr; }
    ...
    function genesisListValidation(bytes32[] memory proof, bytes32 leaf) public view returns (bool) { return MerkleProof.verify(proof, merkleRoot, leaf); }
