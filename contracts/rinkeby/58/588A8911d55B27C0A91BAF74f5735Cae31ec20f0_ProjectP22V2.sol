    ...
    IVestedRandomness
{
    ...
    bool public asyncRandomnessProvider;
    ...
    mapping(uint32 => mapping(uint32 => bool)) public isToadzIdClaimedFreePack;
        ...
        returns (uint256)
    {
        ...
        uint256 newImageIndex = 0;
        for (
            j = randomNumber;
            j <= maxAvailableIndex[packId] && matchFound == false;
            j = j + 1
        ) {
    ...
    {
        if (droppedPoint > 32)
            return ((droppedPoint - 32) *
                250000000000000000 +
                1000000000000000000);
        else if (droppedPoint > 13)
            return ((droppedPoint - 13) *
                50000000000000000 +
                50000000000000000);
        else if (droppedPoint > 9)
            return ((droppedPoint - 9) * 10000000000000000 + 10000000000000000);
        else if (droppedPoint > 0)
            return ((droppedPoint) * 1000000000000000 + 1000000000000000);
    ...
    function startPackSale(
        ...
        packSaleData[packId].saleStartPrice = _packStartPrice;
        packSaleData[packId].saleMinPrice = _packMinPrice;
        packSaleData[packId].saleDuration = _saleDuration;
        packSaleData[packId].saleStartTime = block.timestamp;
    ...
    function setRandomness(
        address _randomnessProviderAddress,
        bool _asyncRandomnessProvider
    ) external onlyOwner {
    ...
    function numOfUnclaimedFreePacksOfUser(address userAddress)
        external
        view
        returns (uint32)
    {
    ...
    function MintPrice(uint32 packId) public view returns (uint256) {
        ...
        if (elapsed >= packSaleData[packId].saleDuration) {
            ...
            uint256 currentPrice = getDroppedPrice(
    ...
    {
        return
            ((totalRevenue * 6) / (10 * tokenRevenueSharingMembers)) -
            perTokenClaimedAmount[tokenId];
    }

    /**
    @notice Enter wallet to view unclaimed amount of all of owner's OG Cryptoadz tokenIDs 
     */
    function unclaimedAllOwnsTokensShare(address ownerAddress)
        external
        view
        returns (uint256)
    {
        ...
        uint256 perTokenClaimableAmount = ((totalRevenue * 6) /
            (10 * tokenRevenueSharingMembers));
        for (index = 0; index < tokenCount; index++) {
            alreadyClaimedAmount += perTokenClaimedAmount[
                revenueSharingContract.tokenOfOwnerByIndex(ownerAddress, index)
            ];
        }
    ...
    function claimTokenShareByToken(uint256 tokenId) external {
        ...
        uint256 claimValue = ((totalRevenue * 6) /
            (10 * tokenRevenueSharingMembers)) - perTokenClaimedAmount[tokenId];
        (bool success, ) = msg.sender.call{value: claimValue}("");
