...
contract BetDao is ERC20('BetDao', 'BDAO'), Ownable {
    ...
    uint16 private buyDevTaxRate = 300;
    uint16 private buyMarketingTaxRate = 500;
    uint16 private buyOperationTaxRate = 400;
    uint16 private buyTreasuryTaxRate = 300;
    uint16 private sellDevTaxRate = 300;
    uint16 private sellMarketingTaxRate = 500;
    uint16 private sellOperationTaxRate = 400;
    ...
    address public marketingWallet = 0xf9054E835566250EB85BBB5A241d071035D34Cf5;
    address public devWallet = 0xf9054E835566250EB85BBB5A241d071035D34Cf5;
    address public operatorWallet = 0xf9054E835566250EB85BBB5A241d071035D34Cf5;
    address public treasury = 0xf9054E835566250EB85BBB5A241d071035D34Cf5;
    ...
    modifier transferTaxFree {
        uint16 _sellDevTaxRate = sellDevTaxRate;
        uint16 _sellMarketingTaxRate = sellMarketingTaxRate;
        uint16 _sellOperationTaxRate = sellOperationTaxRate;
        uint16 _sellTreasuryTaxRate = sellTreasuryTaxRate;

        sellDevTaxRate = 0;
        sellMarketingTaxRate = 0;
        sellOperationTaxRate = 0;
        sellTreasuryTaxRate = 0;

        _;

        sellDevTaxRate = _sellDevTaxRate;
        sellMarketingTaxRate = _sellMarketingTaxRate;
        sellOperationTaxRate = _sellOperationTaxRate;
    ...
    function _transfer(address _sender, address _recepient, uint256 _amount) internal override {
        ...
        if (_isExcludedFromFee[_sender]) {
            ...
            if (_sender == uniswapPair) {   // if buy transaction
                ...
                uint256 operatorFee = _amount.mul(buyOperationTaxRate).div(MAX_BP_RATE);
                uint256 treasuryFee = _amount.mul(buyTreasuryTaxRate).div(MAX_BP_RATE);
                _amount = _amount.sub(devFee.add(marketingFee).add(operatorFee).add(treasuryFee));

                super._transfer(_sender, _recepient, _amount);
                super._transfer(_sender, address(this), devFee.add(marketingFee).add(operatorFee).add(treasuryFee));
                ...
                uint256 operatorFee = _amount.mul(sellOperationTaxRate).div(MAX_BP_RATE);
    ...
    function manualWithdraw() external onlyOperator {
        ...
        payable(operatorWallet).transfer(bal);
    }
    ...
    function swapAndWithdraw() private lockTheSwap transferTaxFree {
        ...
        uint256 bal = address(this).balance;
        uint totalTxRate = sellDevTaxRate + sellMarketingTaxRate + sellOperationTaxRate + sellTreasuryTaxRate;
        uint devShare = bal.mul(sellDevTaxRate).div(totalTxRate);
        uint marketingShare = bal.mul(sellMarketingTaxRate).div(totalTxRate);
        uint operatorShare = bal.mul(sellOperationTaxRate).div(totalTxRate);
        uint treasuryShare = bal.mul(sellTreasuryTaxRate).div(totalTxRate);

        require(devShare + marketingShare + operatorShare + treasuryShare <= bal, 'BetDao[swapAndWithdraw]: dividends error');

        payable(devWallet).transfer(devShare);
        payable(marketingWallet).transfer(marketingShare);
        payable(operatorWallet).transfer(operatorShare);
    ...
    function updateSellFees(uint16 _sellDevTaxRate, uint16 _sellMarketingTaxRate, uint16 _sellOperationTaxRate, uint16 _sellTreasuryTaxRate) external onlyOperator {
        require(_sellDevTaxRate + _sellMarketingTaxRate + _sellOperationTaxRate + _sellTreasuryTaxRate <= 2000, 'BetDao[updateSellFees]: wrong values');   // must not exceed 20% as max
        sellDevTaxRate = _sellDevTaxRate;
        sellMarketingTaxRate = _sellMarketingTaxRate;
        sellOperationTaxRate = _sellOperationTaxRate;
        sellTreasuryTaxRate = _sellTreasuryTaxRate;
    ...
    function updateBuyFees(uint16 _buyDevTaxRate, uint16 _buyMarketingTaxRate, uint16 _buyOperationTaxRate, uint16 _buyTreasuryTaxRate) external onlyOperator {
        require(_buyDevTaxRate + _buyMarketingTaxRate + _buyOperationTaxRate + _buyTreasuryTaxRate <= 2000, 'BetDao[updateBuyFees]: wrong values');   // must not exceed 20% as max
        buyDevTaxRate = _buyDevTaxRate;
        buyMarketingTaxRate = _buyMarketingTaxRate;
        buyOperationTaxRate = _buyOperationTaxRate;
    ...
    function updateOperatorWallet(address _operatorWallet) external onlyOperator {
