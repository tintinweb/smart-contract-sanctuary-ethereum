...
contract animalMain is  ERC721,Ownable {
  ...
  string public name_ = "aniodentoken";
  uint256 public constant mint = 0.1 ether;
  struct animal {
    bytes32 dna; //DNA
    uint8 star; //幾星級(抽卡的概念) (0-9)
    uint16 roletype; //(哪種動物 0丸子  1菜頭  2豆腐  3天婦羅)
  }

  animal[] public animals;
  string public symbol_ = "ANIODEN";
  ...
  function transfer(address _to, uint256 _tokenId) public {
    ...
    require(ownerOf(_tokenId) == msg.sender,"只有擁有者才能轉移");
  ...
  function getRandom(uint range) private view returns (uint) {
        return uint(getRandom())%range;
  }
