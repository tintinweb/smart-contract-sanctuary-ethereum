...
contract cellNFT is ERC721A, Ownable, ReentrancyGuard, svgManip {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        output.svg_data_bytes = abi.encodePacked(
                ...
                _get_rand_in_range_toStr(random(abi.encodePacked("CELLS SVG ROTATION", _seed_phrase, tokenId)), 0, 1000), 
                'deg); transform-origin: 175px 175px;>'
            );
        if (cell._type == 3) {
            ...
            output.svg_data_bytes = abi.encodePacked(
                output.svg_data_bytes, 
                '\n<g>', 
                _get_nerve_path(tokenId_str, false), '\n</g>'
            );
        } else {
            // Cell body
...
contract svgManip {
    ...
    string[2] center_coords = ["150", "225"];
    ...
    function _add_path_point(string memory prefix, string memory tokenId, string memory points) view internal returns (string memory) {
        return (random(abi.encodePacked(prefix, tokenId, "ADD PATH POINT", _seed_phrase)) % 10 == 8) ? '' : points;
    }

    function _get_nerve_path(string memory tokenId, bool blur) view internal returns (bytes memory) {
        return abi.encodePacked(
            '<path style="fill: url(#radial_grad); filter: url(#', _get_blur_data(blur)[1], ')" d="M -4 155 ', 
    ...
    function add_spooky_overlay(string memory tokenId, bytes memory output, uint64 eukaryote, uint64 _type) view internal returns (bytes memory) {
        if (_type == 3) {
            output = abi.encodePacked(
                output, 
                '\n<g style="fill-opacity:0.6; fill: url(#radial_grad); filter: url(#displacementFilter_blur)">', 
                _get_nerve_path(tokenId, true),
                '\n</g>'
            );
        } else {
            output = abi.encodePacked(output, 
