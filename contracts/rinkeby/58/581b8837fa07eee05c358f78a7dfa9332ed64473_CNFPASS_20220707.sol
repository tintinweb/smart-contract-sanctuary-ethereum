...
contract CNFPASS_20220707 is ERC721A, Ownable, ReentrancyGuard {
    using MerkleProof for bytes32[];

    // ===== Variables =====
    uint16 constant presaleSupply = 200;    //プレセール数
    uint16 private maxSupply = 10000;   //トータル発行数上限

    bool private privateMintStarted;
    bool private publicMintStarted;

    uint8 private maxMintsPerWL = 3;
    ...
    // uint96 public royaltyFee = 500;     //5% = 500

    // ===== Constructor =====
    constructor() ERC721A("CNFPASS_20220707", "CNF") {
    ...
    function ownerMint(uint8 quantity) external onlyOwner {
        if(totalSupply() + quantity > maxSupply) revert ExceedSupply();
    ...
    function privateMint(bytes32[] memory proof, uint8 quantity) external payable nonReentrant whenPrivateMint {
        ...
        if(_numberMinted(msg.sender) + quantity > maxMintsPerWL) revert ExceedMaxPerWallet();
    ...
    function _extension() internal view returns (string memory) {
        return extension;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // string memory baseURI = _baseURI();
        // string memory extension = _extension();
        return bytes(baseURI).length != 0 ? string(abi.encodePacked(_baseURI(), _toString(tokenId), _extension())) : '';
    }
    ...
    function setMaxMintsPerWL(uint8 value) external onlyOwner {
        maxMintsPerWL = value;
    }
    ...
    // // ===== Change the royalty fee for the collection =====
