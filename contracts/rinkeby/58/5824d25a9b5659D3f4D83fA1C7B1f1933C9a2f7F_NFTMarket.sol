...
contract NFTMarket is ReentrancyGuard {
 ...
 function onERC721Received(
  ...
  function fetchMarketItemsUnAvailable() public view returns (MarketItem[] memory) {
    uint itemCount = _itemIds.current();
    uint unAvailableItemsCount = _itemsUnAvailable.current();
    uint currentIndex = 0;
    
    MarketItem[] memory items = new MarketItem[](unAvailableItemsCount);
    for (uint i = 0; i < itemCount; i++) {
  ...
  function fetchMyNFTsOwner() public view returns (MarketItem[] memory) {
   ...
   function fetchMyNFTsSeller() public view returns (MarketItem[] memory) {
    uint totalItemCount = _itemIds.current();
    uint itemCount = 0;
    uint currentIndex = 0;
