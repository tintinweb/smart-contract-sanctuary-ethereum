...
import './ISafariSurfers.sol';

contract SafariSurfers is JupiterNFT, ISafariSurfers{
    event MintSafariSurfer(address indexed to, uint8 option, uint256 indexed tokenId);
    ...
    function mintSafariSurfers(uint8 option) payable external override{
        require(msg.value >= options[option], "Not enough ETH sent; check price!");

        currentTokenId++;
		_safeMint(_msgSender(), currentTokenId);
        emit MintSafariSurfer(_msgSender(), option, currentTokenId);
        return;
    }

    function mintBatchSafariSurfers(uint8 option, uint amount) payable external override{
        require(msg.value >= options[option], "Not enough ETH sent; check price!");
        ...
        for(uint i = 0; i < amount; i++) {
            ...
            emit MintSafariSurfer(_msgSender(), option, buf);
		}
		currentTokenId = buf;	
...
interface ISafariSurfers {
	function mintSafariSurfers(uint8 option) payable external;
	function mintBatchSafariSurfers(uint8 option, uint amount) payable external;
}
