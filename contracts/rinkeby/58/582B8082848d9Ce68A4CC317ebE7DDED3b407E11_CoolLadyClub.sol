...
contract CoolLadyClub is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public maxAmountForPublic = 1463; // totalSupply - totalOldHolderTokens
   ...
   modifier isNotExceedMaxMintPerTx(uint256 amount) {
    ...
    function presaleMintMerkel(bytes32[] calldata merkleProof, uint8 count)
    external
    payable
    callerIsUser
    isPresaleActive
  {
   require(MerkleProof.verify(merkleProof, whitelistMerkleRoot, keccak256(abi.encodePacked(msg.sender))), "Address not in the whitelist");
    ...
    require(ts + count <= maxAmountForPresale, "Cannot exceeds max supply");
