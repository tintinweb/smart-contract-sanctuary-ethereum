...
contract IFOByProxy is ReentrancyGuard, Initializable {
  ...
  // presale
  uint256 public rewardPerBCH;    // 13333;
  ...
  function initialize(
      ...
      uint256 _minInWallet,
      uint256 _rewardPerBCH,
      uint256 _divisor
  ) public initializer {
      lpToken = _lpToken;
      offeringToken = _offeringToken;
      helperToken = _helperToken;
      ...
      minInWallet = _minInWallet;
      rewardPerBCH = _rewardPerBCH;
      divisor = _divisor;
  }
  ...
  function deposit(uint256 _amount) public {
    ...
    require (_amount >= minContribution, 'need _amount higher than min contribution');
    require(_amount.add(userInfo[msg.sender].amount) <= maxContribution, 'contribution too large');
    require(totalAmount.add(_amount) <= maxCap, 'contribution goes above max cap'); 
    require(helperToken.balanceOf(msg.sender) >= minInWallet, 'not enough BPAD in wallet');
...
// to change starting block 
 ...
 // to change end block (to be called as soon as max cap is reached)
  ...
  function harvest() public nonReentrant {
    ...
    require (canWithdraw == true, "no withdrawal activated yet");
    uint256 offeringTokenAmount = userInfo[msg.sender].amount;
    uint256 refundingTokenAmount = userInfo[msg.sender].amount;
    if(totalAmount >= raisingAmount ){
        offeringToken.safeTransfer(address(msg.sender), offeringTokenAmount.mul(rewardPerBCH).div(divisor));
    }
    else {
  ...
  function getOfferingAmount(address _user) public view returns(uint256) {
    ...
    //   uint256 allocation = getUserAllocation(_user);
      return userInfo[msg.sender].amount.mul(rewardPerBCH).div(divisor);
    }
    else {
  ...
  function withdrawRemainingTokens() public onlyAdmin{
    ...
    if(offeringToken.balanceOf(address(this)) > 0) {
      offeringToken.safeTransfer(msg.sender, totalToBurn.mul(rewardPerBCH).div(divisor));
    }
