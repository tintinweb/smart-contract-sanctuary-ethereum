...
contract LPStaking is BaseContract, ERC20Upgradeable {
    ...
    function initialize() initializer public
    {
        ...
        dividendsPerShareAccuracyFactor = 1e36;
    }
    ...
    uint256 public totalReflection; //Reflection amount for LP holders
    uint256 public LPLockAmount; //Locked LP amount
    address public LPLockReceiver; //address for LP lock
    address[] LPholders; //For LP Reflection
    ...
    function updateAddresses() public{
        ...
        LPLockReceiver = addressBook.get("lpLockReceiver");
    }
    ...
    function pendingReward( address _stakerAddress) external view returns (uint256 _pending) {
        
        require(stakers[_stakerAddress].stakingAmount > 0, "Don't exist staked token!");
        
        _pending = stakers[_stakerAddress].stakingAmount.mul(accLPPerShare).div(dividendsPerShareAccuracyFactor).sub(
    ...
    function updateRewardPool() public {
        ...
        if(deltaTime < 24 hours) return;
        uint256 times = deltaTime.div(24 hours);
        if(times > 40) times = 40;

        uint256 lpSupply = IERC20(lpAddress).balanceOf(address(this));
        if (lpSupply == 0) {
            lastUpdateTime = block.timestamp;
            return;
        }

        totalReward = IERC20(lpAddress).balanceOf(address(this)).sub(totalStakingAmount).sub(totalReflection);
        uint256 amountForReward = totalReward.mul(25).div(1000).mul(times);
        uint256 RewardPerShare = amountForReward.mul(dividendsPerShareAccuracyFactor).div(totalStakingAmount);
        accLPPerShare = accLPPerShare.add(RewardPerShare);

        totalReward = totalReward.sub(amountForReward);
        lastUpdateTime = lastUpdateTime.add(times.mul(24 hours));
    }

    function stake(uint256 _amount, uint _durationIndex) public {
        if(lpAddress == address(0) || LPLockReceiver == address(0)) updateAddresses();

        uint256 boostedAmount = _amount;
        address stakerAddress = msg.sender;
        uint256 balance = IERC20(lpAddress).balanceOf(stakerAddress);
        require(balance >= _amount, "Insufficient balance!");
        require(_durationIndex <= 3, "Non exist duration!");

        // IERC20(lpAddress).approve(address(this), balance);

        if (stakers[stakerAddress].stakingAmount == 0) stakerNum ++;

        updateRewardPool();

        if (stakers[stakerAddress].stakingAmount > 0) {
            uint256 pending = stakers[stakerAddress].stakingAmount.mul(accLPPerShare).div(dividendsPerShareAccuracyFactor).sub(
        ...
        if(_durationIndex == 0) { 
            boostedAmount = _amount;
            stakers[stakerAddress].stakingPeriod = 0;
        }

        if(_durationIndex == 1) { 
            boostedAmount = _amount.mul(102).div(100);
            stakers[stakerAddress].stakingPeriod = 30 days;
        }
        if(_durationIndex == 2) { 
            boostedAmount = _amount.mul(105).div(100);
            stakers[stakerAddress].stakingPeriod = 60 days;
        }
        if(_durationIndex == 3) { 
            boostedAmount = _amount.mul(110).div(100);
            stakers[stakerAddress].stakingPeriod = 90 days;
        }
        

        stakers[stakerAddress].stakingAmount += boostedAmount.mul(900).div(1000);
        stakers[stakerAddress].rewardDebt = stakers[stakerAddress].stakingAmount.mul(accLPPerShare).div(dividendsPerShareAccuracyFactor);

        totalStakingAmount = totalStakingAmount + boostedAmount.mul(900).div(1000);
        totalReflection += _amount.mul(20).div(1000);
        LPLockAmount += _amount.mul(30).div(1000);

        stakers[stakerAddress].lastStakingUpdateTime == block.timestamp;

        distributeReflectionRewards();

        emit Stake(stakerAddress, _amount, stakers[stakerAddress].stakingPeriod);
    }
    ...
    function unstake() public {
        ...
        require(block.timestamp - stakers[stakerAddress].lastStakingUpdateTime >= stakers[stakerAddress].stakingPeriod, 
        ...
        totalReflection += _amount.mul(20).div(1000); //Reflection for LP holders
        LPLockAmount += _amount.mul(30).div(1000);
    ...
    function registerAddress() public {
        if(LPLockReceiver == address(0)) updateAddresses();
        require(msg.sender != LPLockReceiver, "LP lockreceiver can't receive LP reflection reward");
        LPholderIndexes[msg.sender] = LPholders.length;
        LPholders.push(msg.sender);
    }

    function removeShareholder(address _holder) public {
        LPholders[LPholderIndexes[_holder]] = LPholders[LPholders.length-1];
        LPholderIndexes[LPholders[LPholders.length-1]] = LPholderIndexes[_holder];
        LPholders.pop();
    }

    function distributeReflectionRewards() internal {
        if(lpAddress == address(0)) updateAddresses();

        uint256 total = IERC20(lpAddress).totalSupply().sub(totalStakingAmount).sub(LPLockAmount);
        uint256 ReflectionPerShare = totalReflection.mul(dividendsPerShareAccuracyFactor).div(total);

        for(uint256 i = 0; i<= LPholders.length -1 ; i++){
            uint256 balance = IERC20(lpAddress).balanceOf(LPholders[i]);
            if(balance > 0)
                IERC20(lpAddress).transfer(LPholders[i], ReflectionPerShare.mul(balance).div(dividendsPerShareAccuracyFactor));
            if(balance == 0)
                removeShareholder(LPholders[i]);
        }
        totalReflection = 0;
    }
