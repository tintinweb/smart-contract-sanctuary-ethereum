...
contract MetaGodsWeapons is ERC721Enum, Ownable, Pausable {
    ...
    uint256 constant private MAX_SUPPLY = 7000;
    uint256 private reserveSupply = 100;
    uint256 private cost = 50000 ether;
    address public godTokenContract = 0x0000000000000000000000000000000000000000;

    string private baseURI;

    constructor(string memory _initBaseURI) ERC721P("MetaGods Weapons", "MGW") {
    ...
    function mint(uint256 mintAmount_) external payable whenNotPaused {
        ...
        require(totalSupply + mintAmount_ <= _publicSupply(), "TOO MANY");
        require(IToken(godTokenContract).walletsBalances(msg.sender) >= mintAmount_ * cost, "NOT ENOUGH");
        IToken(godTokenContract).spend(msg.sender, mintAmount_ * cost);
    ...
    function setGodTokenContract(address contractAddress_) external onlyOwner {
        godTokenContract = contractAddress_;
    }
