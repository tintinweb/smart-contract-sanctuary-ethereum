...
contract WaltFilm is ERC721Enumerable, Ownable, ReentrancyGuard {
    event Mint(address indexed account, uint256 indexed tokenid);

    uint256 constant public MAX_TOKEN = 20;
    uint256 constant public MAX_TOKENS_PER_ACCOUNT_FOR_FREE = 1;
    mapping(address => uint256) public freeMintNum;

    uint256 public TEAM_KEEP = 10;
    ...
    function freeMint(uint256 num) external callerIsUser mintStarting nonReentrant {
        require(totalSupply() + num + TEAM_KEEP <= MAX_TOKEN, "over free supply");
    ...
    function batchClaim(address[] calldata _accounts, uint256[] calldata _quantity) external onlyOwner nonReentrant {
        ...
        require(totalSupply() + total + TEAM_KEEP <= MAX_TOKEN, "over free supply");
