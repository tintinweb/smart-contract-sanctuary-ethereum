...
contract NomeGenesisA is ERC721A, Ownable, Pausable, PaymentSplitter {
    // constructor() ERC721A("NM-TEST", "NM") {}

    constructor(address[] memory _payees, uint256[] memory _shares)
        payable
        ERC721A("TestToken", "TT2")
    ...
    uint256 public totalTokenAmount = 1000;
    string public baseTokenURI = "https://testnet.nomedao.xyz/api/genesis/";
    ...
    function mint(uint256 _amount) external payable {
        if (msg.value != batchPrice(_amount)) revert MintIncorrectPrice();
    ...
    function publicMint(uint256 _amount) external payable {
        require(msg.value == mintPrice * _amount, "Wrong price");
    ...
    function batchPrice(uint256 _amount) private view returns (uint256) {
        return mintPrice * _amount;
    }

    function tokensAvailable() private view returns (uint256) {
        return totalTokenAmount - _totalMinted();
    }
}
