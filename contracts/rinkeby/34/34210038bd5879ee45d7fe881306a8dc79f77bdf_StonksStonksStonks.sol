...
contract StonksStonksStonks is ERC20, Ownable, VRFConsumerBaseV2 {
    // Basic units
    ...
    constructor(
        ...
        bytes32 _keyHash,
        address _NFTaddress
    ) ERC20("Stonks Stonks Stonks", "STONKS") VRFConsumerBaseV2(_vrfCoordinator) {
        ...
        if (
            ...
            if (tax > 0) {
                if (_isSell && _taxIsElevated) {
                    ...
                    // whatever the sellTaxUnwageredMultiplier is (let's call it n), we take 1/n to burn
                    _burn(sender, tax);

                    // amount of tokens to divide between different taxes, except burn
                    uint256 taxWithoutBurn = tax * (sellTaxUnwageredMultiplier - 1);
                    ...
                    super._transfer(
                        sender,
                        devAndMarketingWallet,
                        taxWithoutBurn * (taxMarketingAndDevelopment / _totalTax)
                    );                    
                } else {
    ...
    function _swap(uint256 _amountToSwap) private swapLock {
        ...
        uint256 liquidityTokens = (_amountToSwap * taxLp) / (taxLp + taxBuyer) / 2; // takes half of the token balance in the contract to add to swap into ETH for adding LP
