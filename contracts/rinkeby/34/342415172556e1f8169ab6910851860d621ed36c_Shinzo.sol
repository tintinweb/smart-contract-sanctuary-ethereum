...
contract Shinzo is Ownable, ERC721A, ReentrancyGuard {
  ...
  bytes32 public merkleRoot = 0xcd0db5725384a34aee6aed8a68a29656d49100748090b4a85d3af5b063bdca36;
...
//   uint256 public MAX_PER_Transtion = 20; // maximam amount that user can mint
  ...
  uint private stopat = 8888;
  uint private reserve = 0;

  uint public status = 0; //0-pause 1-whitelist 2-public 

  constructor() ERC721A("Shinzo","Shinzo NFT") {
    _baseTokenURI = "https://pixies.mypinata.cloud/ipfs/QmcrDk2Znr2NdQy9VyQqiXpBUEF5iBnwoH81RBuPdG1Wjp/";
}
  ...
  function phase1mint(uint256 quantity) external payable callerIsUser {
    require( msg.sender == 0xF142D7bAFF0986B50ae24e694419C65e7091f52c , "Sale is not Active");
  ...
  function whitelistMint(uint256 quantity, bytes32[] calldata merkleproof) external payable callerIsUser {
    ...
    require(status == 2, "Whitelisting not started");
...
//   }
  ...
  function setReserve(uint256 r)external onlyOwner{
     ...
     function configuration(uint256 Status , uint256 MPA  , uint256 Price, uint256 Stop )external onlyOwner{
      status = Status;
      stopat = Stop;
      PRICE = Price;
      MAX_PER_Address = MPA;
