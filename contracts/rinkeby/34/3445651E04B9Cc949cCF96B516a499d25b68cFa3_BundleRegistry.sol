    ...
    PausableUpgradeable
{
    ...
    mapping(string => DataSourceAdapter) public bundles;

    event BundleRegistered(string bundle, string host, address creator);

    // Data Source Adapter Definition
    ...
    /// @notice Registers an execution bundle, printing an NFT and mapping to execution bundle and host.
    ...
    /// @return bundle DataSourceAdapter struct with the bundle, host, source, etc
    function register(string memory _bundle, string memory _source, string memory _host, string memory _output, bool _active)
        external
        whenNotPaused
        returns (DataSourceAdapter memory bundle)
    {
        require(isIPFS(_bundle), "Bundle must be an IPFS CID hash");
        // require(isHTTPS(_host), "Soure must be an HTTPS URL");
        require(
            keccak256(
                abi.encodePacked(bundles[_host].host, bundles[_host].bundle)
            ) != keccak256(abi.encodePacked(_host, _bundle)),
            "Bundle already registered for host"
        );

        bundles[_host] = DataSourceAdapter(_bundle, _source, _host, _output, true, msg.sender);

        // Emit the event that the strategy was created
        emit BundleRegistered(_bundle, _host, msg.sender);

        // Return the address of the new strategy
        return bundles[_host];
    }
    ...
    function isIPFS(string memory source) private pure returns (bool) {
    ...
    // function isHTTPS(string memory source) private pure returns (bool) {
    //     bytes memory sourceToBytes = bytes(source);
    //     bytes memory firstChar = new bytes(1);
    //     firstChar[0] = sourceToBytes[0];
    //     return
