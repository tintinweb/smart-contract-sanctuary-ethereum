...
contract NinjaNFT is ERC721A, Ownable, ReentrancyGuard {
  ...
  function whitelistMint(bytes32[] calldata _merkleproof, uint256 _mintAmount) public payable callerIsUser{
      uint256 ownerMintedCount = presaleAddressMintedBalance[msg.sender];
      require(privateSaleActive, "the private sale is not active");
      require(_mintAmount <= presaleMintLimit, "max mint amount per session exceeded");
      require(mintRequirements(_mintAmount));
      require(ownerMintedCount + _mintAmount <= presaleMintLimit, "max NFT per address exceeded");
      ...
      _safeMint(msg.sender, _mintAmount);
      presaleAddressMintedBalance[msg.sender] = presaleAddressMintedBalance[msg.sender] + _mintAmount;
  }

  //Reserved tokens used for giveways/partnerships/etc..
  ...
  address community = 0xf72B3e7A4624217c8F73ABAd9BD2DF57ea49bCa2;
  
  function withdraw() public onlyOwner nonReentrant {
    uint256 balance = address(this).balance;
    require(balance > 0, "contract balance is 0");
    payable(dev1).transfer((balance * 190) / 1000);
    payable(dev2).transfer((balance * 190) / 1000);
    payable(dev3).transfer((balance * 190) / 1000);
    payable(dev4).transfer((balance * 190) / 1000);
    payable(dev5).transfer((balance * 50) / 1000);
    payable(community).transfer((balance * 190) / 1000);
  }
