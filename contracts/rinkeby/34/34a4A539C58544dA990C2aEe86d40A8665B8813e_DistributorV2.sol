...
contract DistributorV2 is Policy {
    using SafeMath for uint256;
    ...
    function distribute() external returns (bool) {
        if (nextEpochBlock <= block.number) {
            ...
            if (reward > 0) {
                ITreasury(treasury).mintRewards(newStaking, reward);
            }
            adjust();
            return true;
        } else {
    ...
    function split(
        ...
        if (reward == 0) {
            ...
            if (favouriteFor2 < percent1)
                percent1 = percent1.sub(favouriteFor2);
            else percent1 = 0;
    ...
    function setNewStaking(address _newStaking, address _sHecNew)
        external
        onlyPolicy
    {
        require(_newStaking != address(0));
