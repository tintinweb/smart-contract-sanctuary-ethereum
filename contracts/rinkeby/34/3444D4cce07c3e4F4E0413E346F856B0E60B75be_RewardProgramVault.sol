...
contract RewardProgramVault is  RewardProgramVaultStorage, VaultStorage, ProxyAccessCommon, IRewardProgramVaultEvent, IRewardProgramVaultAction
{
    ...
    function LogEvent(bytes4 _eventName, bytes memory data) internal {
        if(boolLogEvent)
            IEventLog(logEventAddress).logEvent(
                bytes32(keccak256("RewardProgramVault")),
                _eventName,
                address(this),
                data);
    }
    ...
    function initialize(
        ...
        LogEvent(bytes4(keccak256("Initialized")), abi.encode(_totalAllocatedAmount, _claimCounts, _claimTimes, _claimAmounts));
    ...
    function createIncentive(IUniswapV3Staker.IncentiveKey memory key, uint256 reward) internal  {
        ...
        LogEvent(bytes4(keccak256("IncentiveCreatedByRewardProgram")), abi.encode(idx, address(key.rewardToken), address(key.pool), key.startTime, key.endTime, key.refundee, reward));
        ...
        nonZeroAddress(address(pool))
    {
        ...
        LogEvent(bytes4(keccak256("IncentiveEndedByRewardProgram")), abi.encode(address(programs[idx].key.rewardToken), address(programs[idx].key.pool), programs[idx].key.startTime, programs[idx].key.endTime, programs[idx].key.refundee, refund));
