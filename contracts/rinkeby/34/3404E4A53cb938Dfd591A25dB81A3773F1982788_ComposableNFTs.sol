...
contract ComposableNFTs is ERC721Enumerable, Ownable {
  ...
  /// @notice when the 1st wave (early access) ends
  uint256 public firstWaveEndDate;
  ...
  constructor(
    ...
    firstWaveEndDate = block.timestamp + 1 weeks;
  }
  ...
  function mint() public payable onlyWhenNotPaused{
    if(mintWave == 1){
      ...
      require(block.timestamp < firstWaveEndDate, "1st minting wave has ended");
    }
  ...
  function checkIfSpecialMint() private view returns(bool){
    ...
    if(tokenIds>9960){
      ...
      //generates random number up to 10000, corresponding to the max number of mints
      pseudoRandomness = pseudoRandomness % 10000;
  ...
  function mintItem(uint256 tokenId) private {
    ...
    if(shouldMintSpecialNft==true){
      ...
      for(uint i = 0; i < numDefaultLayers; i++) {
          ...
          layerContracts[i].selectOption(tokenId, randomNumbers[i]);
    ...
    returns (uint256[] memory randomValues)
  {
    ...
    for (uint256 i = 0; i < numDefaultLayers; i++) {
      randomValues[i] = uint256(keccak256(abi.encode(pseudoRandomness, i))) % 100;
    }
  ...
  function incrementMintWave () public onlyOwner {
    require(mintWave == 1, "Already incremented");
    require(block.timestamp > firstWaveEndDate, "Can only begin 2nd wave after 1st wave");
    mintWave = mintWave + 1;
  }
