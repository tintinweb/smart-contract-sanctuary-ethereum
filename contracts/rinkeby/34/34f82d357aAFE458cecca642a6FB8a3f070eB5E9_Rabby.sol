...
contract Rabby is ONFT, IMintNFT {
    ...
    constructor(address _layerZeroEndpoint, uint256 _startingMint)
        ONFT("Cyber Rabby", "Rabby", _layerZeroEndpoint, _startingMint)
    {
        capSupply = 10000;
    }

    function mintFor(address to, uint8 numTokens)
        external
        override
        onlyManager
    {
    ...
    function mintBundleFor(
        ...
        _mintFromIndex(to, startingIndex, numTokens);
    }
...
contract ONFT is IONFT, NonblockingLzApp, ERC721A {
    ...
    constructor(
        ...
        address _lzEndpoint,
        uint256 _startingMint
    ) ERC721A(_name, _symbol, _startingMint) NonblockingLzApp(_lzEndpoint) {}
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    constructor(
        ...
        _name = name_;
        _symbol = symbol_;
        _startTokenId = _startingMint;
        _currentIndex = _startingMint;
    }
    ...
    function _mintFromIndex(
        address to,
        uint256 index,
        uint256 quantity
    ) internal {
        uint256 startTokenId = index;
        if (to == address(0)) revert MintToZeroAddress();
        if (quantity == 0) revert MintZeroQuantity();
        if (index + quantity >= _startTokenId) revert MintToZeroAddress();
        _beforeTokenTransfers(address(0), to, startTokenId, quantity);
...
interface IMintNFT {
    function mintFor(address to, uint8 numTokens) external;
