...
// File: contracts/nft-sale/IWaitingSlot.sol
...
abstract contract IWaitingSlot is IERC721 {
    function setUsed(uint256 tokenId) external virtual returns (bool success);
}

// File: contracts/nft-sale/WaitingSlot.sol
...
error OnlyWaitingSlotOwnerAllowed();

contract WaitingSlot is ERC721A, ERC721AQueryable, Ownable, ReentrancyGuard, IWaitingSlot {
    ...
    uint256 private _salePrice = 0.05 ether;
    ...
    constructor(address verifier) ERC721A("WaitingSlot", "UWS") {
        _verifier = verifier;
    }

    // IWAITINGSLOT
    function setUsed(uint256 tokenId)
        external
        override
    ...
    modifier onlyTokenOwner(uint256 tokenId) {
        if (ownerOf(tokenId) != tx.origin) revert OnlyWaitingSlotOwnerAllowed();
        _;
    }
    ...
    function presaleMint(bytes calldata sig)
        external
        payable
        nonReentrant
        onlyEOA
    ...
    function isWhitelisted(address account, bytes calldata sig)
        internal
        view
        returns (bool)
    {
        return
            ECDSA.recover(
                keccak256(abi.encodePacked(account)).toEthSignedMessageHash(),
                sig
            ) == _verifier;
    }
