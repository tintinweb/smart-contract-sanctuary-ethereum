...
contract whitelistChecker is EIP712 {
    ...
    string private constant SIGNING_DOMAIN = "Azo_Escrow";
...
function _hash(Whitelist memory whitelist) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
                keccak256("Whitelist(address buyerAddress,address sellerAddress,uint256 PaymentType,uint256 timestamp)"),
...
contract DoxaEscrow is OwnableUpgradeable,whitelistChecker {
...
// address _buyer, address _seller, uint _amount, string memory id, string memory userId, uint paymentType

    function escrowPayment(string memory id, string memory userId, uint256 _amount , Whitelist memory _escrow) public {
        ...
        if(_escrow.PaymentType == 2) {
            ...
            wallet.withdraw(_escrow.buyerAddress, id, _amount);
