...
contract SENSHI is Ownable, ERC721A, EIP712Whitelisting, PaymentSplitter, VRFConsumerBaseV2{
    ...
    bool public prioritySaleLive = true;

    uint8 public priorityMintPerWallet = 1;
    ...
    function privateMint(uint8 amount, bytes calldata signature) external payable callerIsUser requiresWhitelist(signature)
    {
        ...
        if(prioritySaleLive){
            require(uint8(aux >> 16 & ~uint8(0)) + amount <= priorityMintPerWallet, "Priority mint per wallet exceeded");
        }else{
            require(uint8(aux >> 8 & ~uint8(0)) + amount <= privateMintPerWallet, "Private mint per wallet exceeded");
        }
        require(_totalMinted() + reservedAirdrops + amount <= collectionSize, "reached max supply");
        require(msg.value >= price, "Insufficient funds.");
        uint8 shift = prioritySaleLive ? 16 : 8;
        _setAux(msg.sender, aux | amount << shift);
    ...
    function publicMint(uint8 amount) external payable callerIsUser {
        ...
        uint64 aux = _getAux(msg.sender);
        require(uint8(aux & ~uint8(0)) + amount <= publicMintPerWallet, "Public mint per wallet exceeded");
        require(_totalMinted() + reservedAirdrops + amount <= collectionSize, "reached max supply");
        require(msg.value >= price, "Insufficient funds.");
    ...
    function burnRemainingUnmintedTokens() external onlyOwner
    {
        require(collectionSize != _totalMinted());
        uint256 from = _totalMinted();
        _safeMint(address(this), collectionSize - _totalMinted());
        for(uint256 i = from; i < collectionSize; i++){
            _burn(i);
        }
    }
    ...
    function setPrioritySale(bool live) external onlyOwner {
        prioritySaleLive = live;
    }
