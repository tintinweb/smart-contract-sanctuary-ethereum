...
contract EzyGaming is ERC721Enumerable, Ownable, Pausable {
    ...
    constructor(string memory baseURI) ERC721("EzyGaming", "EZY") {
        _setBaseURI(baseURI);
    }
    ...
    function safeMint(address _to, uint256 _mintAmount) public payable whenNotPaused{
        require(saleStartDate <= block.timestamp && saleEndDate > block.timestamp, "Presale ended or not started yet");
        require(isWhitelisted(_to), "User is not whitelisted");     
        require(totalSupply() <= maxSupply, "Supply limit reached!");            
        require(ownedToken[_to] <= 0, "You can't buy more tokens"); 
        require(msg.value >= salePrice * _mintAmount, "Wrong price!");          
    ...
    function setSaleEndTime(uint256 time) public onlyOwner {
        saleEndDate = time;
    }
