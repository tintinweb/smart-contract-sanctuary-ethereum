...
import {CiweiRoles} from "./CiweiRoles.sol";

contract Ciwei is CiweiRoles, ERC721Enumerable, Pausable {
    //  ============ Structs ============
    ...
    event StartingIndexBlockSet(uint256 startingIndexBlock);

    event StartingIndexValueSet(uint256 startingIndexValue);

    event BaseURISet(string baseURI);

    event DistributionMintRateSet(uint256 distributionMintRate);

    event DistributionOffsetSet(uint256 distributionOffset);
    ...
    uint256 public immutable TIER_ZERO_MINTS_UNLOCK_TIMESTAMP;

    uint256 public immutable TIER_ONE_MINTS_UNLOCK_TIMESTAMP;

    uint256 public immutable TIER_TWO_MINTS_UNLOCK_TIMESTAMP;

    uint256 public immutable TIER_ZERO_MINTS_MAX_SUPPLY;

    uint256 public immutable TIER_ONE_MINTS_MAX_SUPPLY;

    uint256 public immutable TIER_TWO_MINTS_MAX_SUPPLY;
    ...
    constructor(
        ...
        CiweiRoles(roleOwners[0], roleOwners[1], roleOwners[2])
        ERC721("Ciwei", "CIWEI")
    {
     ...
     */
    function finalize() external uriNotFinalized onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function reserveHedgies(uint256 maxMint, address recipient)
        external
        whenNotPaused
     ...
     */
    function mintHedgieTo(address recipient, uint256 tokenId)
        external
        whenNotPaused
        onlyRole(MINTER_ROLE)
    {
        ...
        uint256 availableHedgiesToMint = ((_HEDGIE_DISTRIBUTION_MINT_RATE_ *
            (block.timestamp - _HEDGIE_DISTRIBUTION_OFFSET_)) / 10**18);
        require(
     ...
     */
    function setDistributionMintRate(uint256 distributionMintRate)
        external
     ...
     */
    function setDistributionOffset(uint256 distributionOffset)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     */
    function mintHedgie(uint256 tier, bytes32[] calldata merkleProof)
        external
        whenNotPaused
    {
        // Get the tier minting information and verify that it is past the time when tier minting started.
        TierMintingInformation
            memory tierMintingInformation = getMintsAndMintTimeForTier(tier);
        require(
    ...
    function setStartingIndex() external {
        ...
        require(!_STARTING_INDEX_SET_, "Starting index is already set");
     ...
     */
    function getMintsAndMintTimeForTier(uint256 tier)
        public
        view
        returns (TierMintingInformation memory)
    {
        // Verify the tier is a valid mint tier.
        require(tier < NUM_TIERS, "Invalid tier provided");
     ...
     */
    function _setStartingIndexBlock(uint256 startingIndexBlock) internal {
     ...
     * @param  distributionMintRate  The mint rate to distribute Hedgies at.
     */
    function _setDistributionMintRate(uint256 distributionMintRate) internal {
     ...
     * @param  distributionOffset  The offset for the mint rate to distribute Hedgies at.
     */
    function _setDistributionOffset(uint256 distributionOffset) internal {
...
contract CiweiRoles is AccessControl {
