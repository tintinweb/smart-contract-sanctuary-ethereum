...
contract SoulSquads is Ownable, ERC721A, ReentrancyGuard {
   uint256 public  cost = 0.2 ether;
   uint256 public  nftPerAddressLimit = 4; //max possible Buy amount / address
   uint256 public  MAX_BATCH_SIZE = 5; // whats that
   uint256 public  maxSupply = 8888;

   address private teamAddress = 0x6CB9033bb1B37a7eCC3Efe4bC4c5bf73451C2d87;
   ...
   string public longName = "SoulSquads";
   string public shortName = "Souly";
   ...
   function reserveSOULY(uint256 amount) external onlyOwner {
...
function mint(address to, uint256 amount) external payable mintGuard(amount) {
     if (msg.sender != owner()) {
        require(!paused);  
   ...
   function setMAX_BATCH_SIZE(uint256 newAmount)external onlyOwner{
       MAX_BATCH_SIZE = newAmount;
   }
