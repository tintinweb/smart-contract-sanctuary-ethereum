    ...
    RoyaltiesV2Impl
{
    ...
    modifier mintStatus() {
        if (!publicMintEnabled) {
            require(_isWethioAdmin(), "Only admin minting is allowed");
        }
        _;
    }
    ...
    ) public mintStatus returns (uint256 tokenId) {
        require(
    ...
    /**
     * @notice Allows the wethio admin to update mint status
     */
    function adminUpdateMintStatus(bool status) external onlyWethioAdmin {
