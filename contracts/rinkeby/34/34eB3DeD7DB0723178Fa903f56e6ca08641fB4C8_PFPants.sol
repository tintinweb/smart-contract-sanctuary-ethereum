...
contract PFPants is Ownable, ERC721A {
    ...
    constructor(
        uint256 _initPublicTimestamp,
        string memory _initBaseUri,
        string memory _initNotRevealedUri
    ) ERC721A("PFPants", "PFP") {
        setPublicStartingTimestamp(_initPublicTimestamp);
    ...
    function setPublicStartingTimestamp(uint256 _newTimestamp)
        public
        onlyOwner
    {
        publicStartTimestamp = _newTimestamp;
    }
        ...
        returns (string memory)
    {
        ...
        string memory directTokenURI = super.tokenURI(tokenId);

        // if (
        //     !revealed || block.timestamp <= whitelistStartTimestamp + 1 minutes
        // ) {
    ...
    function publicMint(uint256 amount) external payable noContracts {
        ...
        if (totalSupply() + amount > totalAmount) revert SoldOut();
        require(
