...
contract Clokies is IClokies, ERC721A, Ownable, MerkleWhitelist, Pausable {
    uint256 public maxClokies = 8888;
    uint256 public price = 0.05 ether;
    
    uint256 public constant RESERVED_CLOKIES = 50;
    uint64 public constant WHITELIST_MINT_AMOUNT = 1;

    string private _baseTokenURI;

    address artistAddress = 0xE22306CBa5eaB8B9Baa1284D5dB721a832562e02;
    address devAddress = 0x1eFEF8b085C75b7974B710e85494088ff52b9bfF;
    address founderAddress = 0xa6E5a9cFF78a15627Ff4474AfB3Bf2c92360c3F3;
    address vaultAddress = 0x303CA0B8FB4046F56D35632679b5d9e01b399DD9;
    ...
    /// @notice Function used during the whitelist mint. 
    /// @param proof Whitelist merkle tree proof.
    /// @dev whenNotPaused to check sale state.
    /// @dev isWhitelisted verify proof in merkle tree.
    function mint(bytes32[] calldata proof) 
        ...
        isWhitelisted(proof)
    {
        if (msg.value != WHITELIST_MINT_AMOUNT * price) revert InvalidEtherAmount();
        if (_totalMinted() + WHITELIST_MINT_AMOUNT > maxClokies) revert SupplyExceeded();
        ...
        _setAux(msg.sender, WHITELIST_MINT_AMOUNT);

        _mint(msg.sender, WHITELIST_MINT_AMOUNT);
        emit Minted(msg.sender, WHITELIST_MINT_AMOUNT);
    }
    ...
    function withdraw() public onlyOwner {
        ...
        if (!devWithdrawalSuccess) revert WithdrawFailedDev();         // TBD NEW ERROR CODE

        (bool founderWithdrawalSuccess, ) = payable(founderAddress).call{value: teamSplit}("");
        if (!founderWithdrawalSuccess) revert WithdrawFailedFounder();     // TBD NEW ERROR CODE
        
        (bool vaultWithdrawalSuccess, ) = payable(vaultAddress).call{value: address(this).balance}("");
        if (!vaultWithdrawalSuccess) revert WithdrawFailedVault();       // TBD NEW ERROR CODE
    }
    ...
    /// @notice Function used to change mint price.
    /// @param newPrice Newly intended `price` value.
    /// @dev Price can never exceed the initially set mint price (0.05E), and can never be increased over it's current value.
    function changePrice(uint256 newPrice) external onlyOwner {
        if (newPrice > price) revert InvalidNewPrice();
        price = newPrice;
    }
...
contract MerkleWhitelist is Ownable {
  ...
  /*  READ FUNCTIONS */
  /// @notice Verifies if the given address is whitelisted. 
  /// @param userAddress Address to verify.
  /// @param proof Whitelist merkle tree proof.
  /// @return bool Address whitelist status.
  function verifyPublicSender(address userAddress, bytes32[] memory proof) public view returns (bool) {
    return _verify(proof, _hash(userAddress), whitelistMerkleRoot);
  }

  /// @notice Verifies if the sender is whitelisted. 
  /// @param proof Whitelist merkle tree proof.
  /// @return bool Sender whitelist status.
  function _verifyPublicSender(bytes32[] memory proof) internal view returns (bool) {
    return _verify(proof, _hash(msg.sender), whitelistMerkleRoot);
  }

  /// @notice Validates a merkle tree proof.
  /// @param proof Whitelist merkle tree proof.
  /// @param addressHash Hash of address to verify.
  /// @param merkleRoot Root of merkle tree.
  /// @return bool Validity of given proof.
  function _verify(bytes32[] memory proof, bytes32 addressHash, bytes32 merkleRoot)
    internal
    pure
    returns (bool)
  {
  ...
  /// @notice Computes the Keccak-256 hash of the input address.
  /// @param _address Address to retrieve Keccak-256 hash.
  /// @return bytes32 Hash of input address.
  function _hash(address _address) internal pure returns (bytes32) {
    return keccak256(abi.encodePacked(_address));
  }

  /*  OWNER FUNCTIONS */
  /// @notice Sets the merkle tree root for whitelist. 
  ...
  /* MODIFIERS */
  /// @notice Validates user trying to mint is whitelisted, 
  /// @param proof Whitelist merkle tree proof.
