...
contract FloorPricePrediction is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IFloorPricePrediction {
        ...
        notContract
    {
        ...
        uint256 nth = period.ledger[epoch][msg.sender].length - 1;
        uint256 lockTimestamp = getEpochLockTimestamp(period.genesisTimestamp, period.lockIntervalSeconds, epoch);
        uint256 closeTimestamp = getEpochCloseTimestamp(
            period.genesisTimestamp,
            period.lockIntervalSeconds,
            period.intervalSeconds,
            epoch
        );
        treasuryAmount += _calculateFee(amount);

        emit NewBet(msg.sender, amount, market, intervalSeconds, epoch, lockTimestamp, closeTimestamp, true, nth);
    }
        ...
        notContract
    {
        ...
        emit NewBet(msg.sender, amount, market, intervalSeconds, epoch, lockTimestamp, closeTimestamp, false, nth);
    }
    ...
    function getEpochStartTimestamp(
        ...
        return genesisTimestamp + ((epoch - 1) * lockIntervalSeconds);
    }

    function getEpochLockTimestamp(
        uint256 genesisTimestamp,
        uint256 lockIntervalSeconds,
        uint256 epoch
    ) public pure returns (uint256) {
        return genesisTimestamp + (epoch * lockIntervalSeconds);
    }

    function getEpochCloseTimestamp(
        uint256 genesisTimestamp,
        uint256 lockIntervalSeconds,
        uint256 intervalSeconds,
        uint256 epoch
    ) public pure returns (uint256) {
        return genesisTimestamp + (epoch * lockIntervalSeconds) + intervalSeconds;
    }
    ...
    function getOracleIdsByBetInfo(BetInfo[] memory betInfos) external view returns (OracleIds[] memory) {
        ...
        for (uint256 i = 0; i < betInfos.length; i++) {
            ...
            r.lockTimestamp = getEpochLockTimestamp(p.genesisTimestamp, p.lockIntervalSeconds, info.epoch);
            r.closeTimestamp = getEpochCloseTimestamp(
                p.genesisTimestamp,
            ...
            oracleIds[i] = OracleIds({
                lockPriceOracleId: oracle2.getOracleIdByTimestamp(m.nftContract, r.lockTimestamp),
                closePriceOracleId: oracle2.getOracleIdByTimestamp(m.nftContract, r.closeTimestamp)
            });
        }
        return oracleIds;
    }
     ...
     */
    // NC: Not claimable
    function claim(BetInfo[] memory betInfos, OracleIds[] memory oracleIds) external nonReentrant notContract {
        require(betInfos.length == oracleIds.length, "ID"); // ID: invalid data
        ...
        for (uint256 i = 0; i < betInfos.length; i++) {
            ...
            // console.log("fuck r.closeTimestamp", block.timestamp, r.closeTimestamp);
            // require(r.epoch == info.epoch, "RNI"); //RNI: round not init
            require(block.timestamp > r.closeTimestamp, "RNC"); // RNC:round not closed

            (bool lockPriceVerified, uint256 lockPrice) = oracle2.getNftPriceByOracleId(
                m.nftContract,
                oracleIds[i].lockPriceOracleId,
                r.lockTimestamp
            );
            (bool closePriceVerified, uint256 closePrice) = oracle2.getNftPriceByOracleId(
                m.nftContract,
                oracleIds[i].closePriceOracleId,
                r.closeTimestamp
            );
            require(lockPriceVerified && closePriceVerified, "IO"); // IO: Invalid oracleId
            r.lockPrice = lockPrice;
            r.closePrice = closePrice;

            Bet storage bet = m.periods[info.intervalSeconds].ledger[info.epoch][msg.sender][info.nth];
            if (!claimable(r, bet)) continue;
            uint addedReward = _calculateRewards(r, bet);
            reward += addedReward;
            bet.claimed = true;
            emit Claim(msg.sender, addedReward, m.nftContract, info.intervalSeconds, info.epoch, info.nth);
        }
        if (reward > 0) {
        ...
        for (uint256 i = 0; i < houseWinRounds.length; i++) {
            ...
            if (!r.houseWinClaimed) {
                ...
                if (lockPriceVerified && closePriceVerified && (lockPrice == closePrice)) {
                    currentTreasuryAmount += (r.totalAmount * (10000 - treasuryFee)) / 10000;
                }
    ...
    /**
     * @notice Returns true if `account` is a contract.
     * @param cursor: To get rounds from epoch, set to 0 will get current epoch
     *
     */
    function getRounds(
        address user,
    ...
    {
        uint256 _cursor = cursor == 0 ? getCurrentEpoch(market, intervalSeconds) : cursor;
        ...
        for (uint i = 0; i < length; i++) {
            ...
            rounds[i].startTimestamp = getEpochStartTimestamp(p.genesisTimestamp, p.lockIntervalSeconds, rounds[i].epoch);
            rounds[i].lockTimestamp = getEpochLockTimestamp(p.genesisTimestamp, p.lockIntervalSeconds, rounds[i].epoch);
            rounds[i].closeTimestamp = rounds[i].closeTimestamp = getEpochCloseTimestamp(
                p.genesisTimestamp,
                p.lockIntervalSeconds,
                p.intervalSeconds,
                rounds[i].epoch
            ...
            if (block.timestamp >= rounds[i].lockTimestamp) {
                rounds[i].lockPrice = oracle2.getNftPriceByTimestamp(market, rounds[i].lockTimestamp);
            }
            if (block.timestamp >= rounds[i].closeTimestamp) {
                rounds[i].closePrice = oracle2.getNftPriceByTimestamp(market, rounds[i].closeTimestamp);
            }
            if (user != address(0)) {
    ...
    function setOracleAddress(address oracleAddress) external {
        oracle2 = INftPriceOracle2(oracleAddress);
    }
}
