    ...
    SuperAdminControl
{
        ...
        )
    {
        ...
        if (dexRouter != address(0x0)) {
            swapRouter = IUniswapV2Router02(dexRouter);
        } else {
            swapRouter = swapRouterv2;
        }
        address pair = IDexFactory(swapRouter.factory()).getPair(
            token0,
            token1
        );
        uint256 reserve0;
        uint256 reserve1;
        if (pair != address(0)) {
            (reserve0, reserve1, ) = IDexPair(pair).getReserves();
        }

        return (IDexPair(pair), reserve0, reserve1);
    }
    ...
    function getDexTokenPrice(
        ...
        (
            IDexPair altStablePair,
            uint256 reserve0,
            uint256 reserve1
        ) = getPairAndReserves(_alt, _stable, _dexRouter);

        if (address(0) == address(altStablePair)) {
            (pairALTWETH, reserve0, reserve1) = getPairAndReserves(
                _alt,
                WETHAddress(),
                _dexRouter
            );

            if (address(pairALTWETH) == address(0)) return 0;
            token0Decimals = IERC20Extras(pairALTWETH.token0()).decimals();
            token1Decimals = IERC20Extras(pairALTWETH.token1()).decimals();
            ...
            if (pairALTWETH.token0() == WETHAddress()) {
                priceOfCollateralinWETH =
                    (_amount *
                        ((reserve0 * (10**token1Decimals)) / (reserve1))) /
                    (10**token1Decimals);
            } else {
                priceOfCollateralinWETH =
                    (_amount *
            ...
            (pairWETHSTABLE, reserve0, reserve1) = getPairAndReserves(
                _stable,
                WETHAddress(),
                _dexRouter
            );
            token0Decimals = IERC20Extras(pairWETHSTABLE.token0()).decimals();
            token1Decimals = IERC20Extras(pairWETHSTABLE.token1()).decimals();

            //identify the stable token out  of token0 and token1, get price of WETH in stable
            ...
            token0Decimals = IERC20Extras(altStablePair.token0()).decimals();
            token1Decimals = IERC20Extras(altStablePair.token1()).decimals();

            //stable pair exists fetch price directly
            if (altStablePair.token0() == _stable) {
                return
                    (_amount *
                        ((reserve0 * (10**token1Decimals)) / (reserve1))) /
                    (10**token1Decimals);
    ...
    function getClaimTokenPrice(
        ...
        if (claimTokenData.dexRouter == address(0)) {
            claimTokenData = govClaimTokenContract.getClaimTokensData(_stable);
        }
    ...
    function getNumberofSunTokenFromStable(
        ...
        return (claimTokenPrice * pegTokensPricePercentage) / 100;
    }
    ...
    function getAltCoinPriceinStable(
        ...
        if (
            ...
            (int256 collateralInUsd, ) = this
                .getLatestUsdPriceFromChainlink(_altCoin);
            (int256 stableInUsd, ) = this
            ...
            uint256 altRateInStable = (uint256(collateralInUsd) * 10 ** stableDecimals) / uint256(stableInUsd);
            return  (_collateralAmount * altRateInStable) / 10 ** collateralDecimals;
        } else {
            address claimToken = govClaimTokenContract.getClaimTokenofSUNToken(_altCoin);
            if (govClaimTokenContract.isClaimToken(claimToken)) {
                return(
                    this.getSUNTokenPrice(
                        claimToken,
                ...
                Market memory marketData = govProtocolRegistry
    ...
    function getStablePriceInAltcoin(
        ...
        if (
            ...
            int256 collateralInUsd = this.getNetworkPriceFromChainlinkinUSD();
            (int256 stableInUsd, ) = this.getLatestUsdPriceFromChainlink(_stableCoin);
            uint256 stableDecimals = IERC20Metadata(_stableCoin).decimals();
            uint256 collateralDecimals = IERC20Metadata(this.WETHAddress()).decimals();
            uint256 stableRateInAltcoin = (uint256(stableInUsd) * 10 ** collateralDecimals) / uint256(collateralInUsd);
            return (_stableCoinAmount * stableRateInAltcoin) / 10 ** stableDecimals;
        } else {
    ...
    function getStablePriceInETH(
        ...
        if (
            this.isChainlinFeedEnabled(this.WETHAddress()) &&
            this.isChainlinFeedEnabled(_stableCoin)
        ) {
            int256 collateralInUsd = this.getNetworkPriceFromChainlinkinUSD();
            (int256 stableInUsd, ) = this.getLatestUsdPriceFromChainlink(_stableCoin);
            ...
            return (this.getETHPriceFromDex(this.WETHAddress(), _stableCoin, _stableCoinAmount));
        }
    }
