...
import "./OwnCollectibledNft1155.sol";
...
contract Factory1155 {
	...
	function deploy(bytes32 _salt, string memory name, string memory symbol, string memory tokenURIPrefix) external returns(address addr) {
		addr = address(new CollectibledUserToken1155{salt: _salt}(name, symbol, tokenURIPrefix));
		CollectibledUserToken1155 token = CollectibledUserToken1155(address(addr));
...
contract CollectibledUserToken1155 is ERC1155 {
