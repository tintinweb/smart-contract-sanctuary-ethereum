...
contract PancakeRouter is IPancakeRouter02 {
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IPancakePair(IPancakeFactory(factory).getPair(input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            IPancakeFactory(factory).getPair(path[0], path[1]),
            amounts[0]
        );
    ...
    function swapExactTokensForETH(
        ...
        require(path[path.length - 1] == WETH, "PR: IP");
        amounts = PancakeLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, "PR: IOA");
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            IPancakeFactory(factory).getPair(path[0], path[1]),
    ...
    function swapETHForExactTokens(
        ...
        require(path[0] == WETH, "PR: IP");
        amounts = PancakeLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, "PR: EIA");
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(IPancakeFactory(factory).getPair(path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(amountOut >= amountOutMin, "PR: IOA");
...
library PancakeLibrary {
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex"20ad0a83afc8bba99bbe0e4e11d4df85f43e550f02573971493f6d9d872e7c04" // init code hash
                        )
                    )
                )
            )
    ...
    function quote(
        ...
        require(amountA > 0);
        require(reserveA > 0 && reserveB > 0, "IL");
