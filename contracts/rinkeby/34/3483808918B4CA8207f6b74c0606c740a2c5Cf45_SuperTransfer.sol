...
contract SuperTransfer is Ownable  {
    ...
    function deposit(address token, address account, uint256 amount) external {
        require(IERC20(token).transferFrom(account,address(this), amount), 'SuperTransfer: Transfer failed.');
    ...
    function isWithdrawed(address account) public view  returns (bool) {
        ...
        //max 1 withdraw in two slot
        if((WithdrawedBitMap[account]==mindex)||(WithdrawedBitMap[account]==(mindex-1))){
            return true;
        }else{
            return false;
        }
    ...
    function withdraw(uint256 index,address token, address account, uint256 amount, bytes32[] calldata merkleProof) external  {
        require(!isWithdrawed(account), 'SuperTransfer: already Withdrawed.');

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(index,token, account, amount));
        require(MerkleProof.verify(merkleProof, merkleRoot, node), 'SuperTransfer: Invalid proof.');

        // Mark it Withdrawed and send the token.
        _setWithdrawed(account);
        require(IERC20(token).transfer(account, amount), 'SuperTransfer: Transfer failed.');

        emit Withdrawed(token,mindex, account, amount);
    }
}