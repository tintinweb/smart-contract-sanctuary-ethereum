...
contract Ca22Characters is ERC721A, Ownable, ReentrancyGuard {
    ...
    address public scenariosContractAddressStates;
    address public verifyAddress = 0xD8A7fd1887cf690119FFed888924056aF7f299CE;
    bool public locked;
    bool public presalelocked;
    bool public wllocked;
    bool public salelocked;

    uint256 private constant CA22_MAX = 50;
    uint256 private constant PRESALE_PRICE = 0.04 ether;
    uint256 private constant PUBLIC_PRICE = 0.06 ether;
    ...
    string private tokenBaseURIStates = "https://ca22.xyz/api/metadata/";
    string public roleMerkelRoot = "0xed5af388653567af2f388e6224dc7c4b3241c544";
    ...
    function mintPublicSale(uint256 _count) external payable nonReentrant whenPublicSaleActive {
        ...
        require(_count > 0 && _count <= MAX_PER_ADDRESS, "Invalid CA22 characters count");
        require(_tokenIds.current() + _count <= CA22_MAX, "All CA22 characters have been minted");
        require(_count * PUBLIC_PRICE == msg.value, "Incorrect amount of ether sent");
        require(msg.value >= _count * PUBLIC_PRICE, "Insufficient fund");
    ...
    function mintPreSale(uint256 _count, uint256 _nonce, bytes calldata signature) external payable nonReentrant whenPreSaleActive {
        ...
        require(_count != 0 && _count <= MAX_PER_ADDRESS, "Invalid CA22 characters count");
        require(_tokenIds.current() + _count <= CA22_MAX, "All early access CA22 characters have been minted");
        require(msg.value >= _count * PRESALE_PRICE, "Incorrect amount of ether sent");

        uint256 userMintedAmount = numberMinted(msg.sender) + _count;
        require(userMintedAmount <= MAX_PER_ADDRESS, "Max NFT count per address exceeded");
    ...
    function setScenariosContractAddress(address _scenariosAddress) public onlyOwner {
        scenariosContractAddressStates = _scenariosAddress;
    }

    function scenariosContractAddress() public view returns (address) {
        return scenariosContractAddressStates;
    }

    function setRoleMerkelRoot(string memory _role) public onlyOwner {
        roleMerkelRoot = _role;
    }

    function verifyProof(uint256 _value, uint256[] calldata _proof)
        public view returns (bool) {
        uint tmp = _value;
        for(uint i = 0; i < _proof.length; i++) {
            tmp = pairHash(tmp, _proof[i]);
        }
        string memory root = Strings.toString(tmp);
        return keccak256(abi.encode(root)) == keccak256(abi.encode(roleMerkelRoot)); 
    }

    function pairHash(uint _left, uint _right) internal pure returns(uint) {
        return uint(keccak256(abi.encode(_left ^ _right)));
    }
