...
contract OnChainTarzan is ERC721A, Ownable, Pausable {
    using SafeMath for uint256;

    constructor(uint256 _initPreMint) ERC721A("fasfasf", "gsdgsdg") {
    ...
    returns (uint256)
    {
        uint256 tokenHash = uint256(keccak256(bytes(Strings.toString(tokenId)))) %
        weight;
        uint i = 0;
