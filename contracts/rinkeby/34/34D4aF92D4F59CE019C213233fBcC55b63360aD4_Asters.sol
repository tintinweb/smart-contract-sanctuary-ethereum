...
contract Asters is Ownable, ERC721A, ERC721AQueryable, ReentrancyGuard{
    ...
    uint private constant MAX_WHITELIST = 9990;
    uint private constant MAX_DEVS = 10;
    uint private constant MAX_PUBLIC = MAX_SUPPLY - MAX_WHITELIST - MAX_DEVS;
    uint public MAX_MINT_PER_ADDRESS = 2;
    ...
    uint256 public saleStartTime = 1650654000; 
    uint256 public saleDuration = 1 weeks;

    mapping (address => uint256 ) public amountWalletWhitelistSale;
    ...
    function whitelistMint(uint _quantity, bytes32[] calldata _proof) external payable 
        ...
        require(totalSupply() + _quantity <= MAX_WHITELIST, "WHITELIST MINT: Max supply exceeded");
        require(amountWalletWhitelistSale[msg.sender] + _quantity <= MAX_MINT_PER_ADDRESS, "Max Whitelist Mint Per Address Reached"); 
    ...
    function publicSaleMint(uint _quantity) external payable 
        ...
        require(msg.value >= COST * _quantity , "Need to send more ETH");
        require(totalSupply() + _quantity <= MAX_PUBLIC + MAX_WHITELIST, "PUBLIC MINT: Max supply exceeded");
