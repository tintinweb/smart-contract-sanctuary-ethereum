    ...
    ERC2981SinglePercentual
{
    ...
    function _processMint(address to, uint256 num) internal {
        ...
        uint256 nextTokenIdAfterBatch = tokenId + num;

        if (nextTokenIdAfterBatch > MAX_NUM_TOKENS)
            revert InsufficientTokensRemanining();

        for (; tokenId < nextTokenIdAfterBatch; ++tokenId) {
            _seeds[tokenId] = _computeSeed(to, tokenId);
        }
        _mint(to, num);
    }
    ...
    /// derive fair seeds.
    function _computeSeed(address to, uint256 tokenId)
        internal
        view
        returns (bytes32)
    {
