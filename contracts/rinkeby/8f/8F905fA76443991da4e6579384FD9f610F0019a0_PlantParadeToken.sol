...
contract PlantParadeToken is ERC721Enumerable, WithLimitedSupply, ERC2981, Ownable {
    ...
    /**
     * @dev Withdraw whole amount from Contract to registered wallet as per distribution percentage.
     */
    function withdraw() public onlyOwner {
        uint amount = address(this).balance;
        require(amount > 0, "No amount left to withdraw");
        uint marketingAmt = amount * 3 /100;
        uint communityAmt = amount * 11 /100;
        uint donationsAmt = amount * 12 /100;
        uint owner1Amount = amount * 37 /100;
        uint owner2Amount = amount * 37 /100;
