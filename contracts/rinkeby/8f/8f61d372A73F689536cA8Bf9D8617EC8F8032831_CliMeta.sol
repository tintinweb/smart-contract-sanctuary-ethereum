...
contract CliMeta is ERC721A, Ownable {
    ...
    constructor(uint256 _maxBatchSize) ERC721A("Cli Meta", "NM", _maxBatchSize)
    {
    }

    function mintCliMeta(uint256 tokenQuantity, bytes32[] calldata merkleProof) external payable {
        require(
            totalSupply() + tokenQuantity <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );
        require(_isSaleActive, "Sale must be active to mint CliMetas");
        require(
        ...
        _mintCliMeta(tokenQuantity, merkleProof);
    }
    ...
    function _mintCliMeta(uint256 tokenQuantity, bytes32[] calldata merkleProof) internal isValidMerkleProof(merkleProof, saleMerkleRoot) {
        _safeMint(msg.sender, tokenQuantity);
    }
    ...
    function checkAddress(address _addr) external returns (bool isC)  {
