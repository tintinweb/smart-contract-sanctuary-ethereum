...
contract BurnKingTest is ERC721Enumerable, Ownable {
    ...
    uint256 private deadlineTime = 1200;

    // pools limits
    uint256 public constant commonLimit = 2000000;
    uint256 public constant uncommonLimit = 1000000;
    ...
    uint256 public constant ultimateLimit = 30;
   ...
   constructor(address _publicKey)ERC721("Burn King Test", "BSPT"){
    ...
    function transferNftFromPool(address _burner, uint256 _burningUSD, Pools _poolType) internal {
     ...
     function burnEther(bytes memory _signature, uint256 _tokenCount,  uint256 _timestamp,uint256 _usdCost, uint8 _poolType) external payable{
      require(block.timestamp - _lastBurnedByUser[msg.sender] >= secondsInDay, "You can burn tokens only once per day!");
      require(block.timestamp - _timestamp > deadlineTime, "Transaction expired");
      require((msg.sender).balance >= _tokenCount, 'you do not have enough ethers');
   ...
   function splitSignature(bytes memory _signature)
    ...
    function concatParamsERC(uint256 _tokenCount, uint256 _timestamp, uint256 _usdCost, uint8 _poolType, address _contractAddress) internal pure returns(string memory){
        return string(abi.encodePacked(Strings.toString(_tokenCount),Strings.toString(_timestamp),Strings.toString(_usdCost),Strings.toString(_poolType),abi.encodePacked(_contractAddress)));
    }
}
