...
contract GAGGLE is Context, IERC20, Ownable {
    ...
    address public teamContractAddress = 0x99990Ab0E073Ecf018ad5d6C4D1D0815Aa3D33A1;
    address public marketingAddress = 0x767355382FcCC2C6E9b4C26CcbaA0398Dfb76bbD;
    address public  _psWallet = 0xfCbDdbe9DC61cB5e4Ebf6135C52be50B9cab0837;
    ...
    uint256 private constant _totalSupply = 2000000000 * 10**9;
    ...
    constructor() {
        ...
        uint256 presale = blackHole.mul(30).div(100);

        // 59% to deployer for LP addition
        uint256 lp = blackHole.mul(23).div(100);

        // 10% to the DAO
        uint256 team = blackHole.mul(18).div(100);

        uint256 marketing  = blackHole.mul(29).div(100);
     
        uint256 rate = getRate();

        _reserveTokenBalance[burnAddress] = blackHole.mul(rate);
        _reserveTokenBalance[_psWallet] = presale.mul(rate);
        _reserveTokenBalance[_msgSender()] = lp.mul(rate);
        _reserveTokenBalance[teamContractAddress] = team.mul(rate);
        _reserveTokenBalance[marketingAddress] = team.mul(rate);

        emit Transfer(address(0), burnAddress, blackHole);
        emit Transfer(address(0), _psWallet, presale);
        emit Transfer(address(0), _msgSender(), lp);
        emit Transfer(address(0), teamContractAddress, team);
        emit Transfer(address(0), marketingAddress, marketing);
    }
    ...
    function setTeamAddress(address _teamContractAddress) public onlyOwner {
        teamContractAddress = _teamContractAddress;
    }
    ...
    function applyExternalTransactionTax(
        ...
        _reserveTokenBalance[teamContractAddress] = _reserveTokenBalance[teamContractAddress].add(reserveValues.reserveTransferAmountTeam);
        _reserveTokenBalance[marketingAddress] = _reserveTokenBalance[marketingAddress].add(reserveValues.reserveTransferAmountMarketing);
        _reserveTokenBalance[burnAddress] = _reserveTokenBalance[burnAddress].add(reserveValues.reserveTransferAmountBurnEm);
    
        emit Transfer(sender, teamContractAddress, transactionValues.teamTax);
        emit Transfer(sender, marketingAddress, transactionValues.teamTax);
    ...
    function _getRValues(
        ...
        reserveValues.reserveTransferAmountMarketing = transactionValues.transactionFee.mul(currentRate);
        reserveValues.reserveTransferAmountTeam = transactionValues.teamTax.mul(currentRate);
        reserveValues.reserveTransferAmountBurnEm = transactionValues.burnEm.mul(currentRate);
