...
// Same production implementation with lower total supply.
...
/**
 * @title Metabeauts NFT
 * @author Adaptive Code
 * @notice Implements Metabeauts NFT collection contract.
 */
contract MetabeautsShort is Ownable, ERC721x {
    using Strings for uint256;

    uint256 public constant INIT_ID = 1;

    uint256 private _numAvailableTokens;
    uint256 private _publicMintInit;
    ...
    function publicMint(uint256 _mintAmount) public payable isPaused {
        ...
        if (block.timestamp < _publicMintInit + cappedTime) {
            require(
                _mintAmount + _mintedTokens[msg.sender] <= MAX_PER_WALLET, // previously, balanceOf(msg.sender)
                "Minting more than max allowed"
            );
        }
    ...
    function revealSituation() public view returns (string memory) {
        if (revealPhase == 0) {
            return "Not revealed";
        } else if (revealPhase == 1) {
            return "Gender and Position Reveal";
        } else if (revealPhase == 2) {
            return "Status and Level Reveal";
        } else {
            return "Full Reveal";
        }
    }
    ...
    function withdraw() external {
        require(
            ...
            "Only owner and taxWallet can withdraw"
        );
        ...
        (success, ) = payable(taxWallet).call{value: (balance * 3500) / 10000}(
            ""
        );
        require(success);
    }

    function nextRevealPhase(string memory baseURI_) public onlyOwner {
        require(revealPhase < 3, "Reveal phase already finished.");
        revealPhase++;
        setBaseURI(baseURI_);
    }

    function resetRevealPhase(string memory baseURI_) public onlyOwner {
    ...
    function isRevealed() public view returns (bool) {
        return revealPhase == 3;
    }

    function setTaxWallet(address payable _taxWallet) external {
        require(
            taxWallet == msg.sender,
            "Only taxWallet can execute this function"
        );
        taxWallet = _taxWallet;
    }

    function freezeMetadata() external onlyOwner {
        require(isRevealed(), "Metadata is not revealed yet");
    ...
    function setPresale(bool _presale) external onlyOwner {
        presale = _presale;
        if (!_presale) {
            _publicMintInit = block.timestamp;
        }
    }
...
contract ERC721x is ERC721, LockRegistry {
    ...
    {
        return
            interfaceId == _INTERFACE_ID_LOCKABLE ||
            super.supportsInterface(interfaceId);
    }
