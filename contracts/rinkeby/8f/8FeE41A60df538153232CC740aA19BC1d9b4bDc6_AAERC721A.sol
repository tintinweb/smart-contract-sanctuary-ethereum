...
contract AAERC721A is Ownable, ERC721A, ReentrancyGuard {
    ...
    ) ERC721A("test", "test", maxBatchSize_, collectionSize_) {
    ...
    function allowlistMint(uint256 quantity, bytes32[] calldata _proof) external payable callerIsUser {
        require(isWhiteListSaleOn(), "allowlist sale has not begun yet");
    ...
    function publicSaleMint(uint256 quantity) external payable callerIsUser {
        ...
        require(
            totalSupply() - amountForDevs - amountForFree + quantity <= amountForSale,
            "reached max supply"
        );
        require(
    ...
    function canPublicSaleStart()
        public
        view
        returns (bool)
    {
        return (price != 0 && (remainingFreeApes() == 0 || block.timestamp - freeMintStartTime >= 24*60*60));
    }     

    function isWhiteListSaleOn()
        public
        view
        returns (bool)
    {
        return !isPublicMintActive && _wlRoot != ""  && price != 0 && (totalSupply() >= amountForDevs + amountForFree + amountForSale || block.timestamp - publicMintStartTime >= 24*60*60); // ensure that the whitelist mint starts
    }                                                                             
    ...
    function setAmountForDevs(uint256 amount) external onlyOwner { //Really a solution for dealing with the possible remaining after minting phases
        amountForDevs = amount;                                    //Mint them for the dev wallet and then send them into oblivion
    }

    function toggleFreePhase() external onlyOwner {
        isFreeMintActive = !isFreeMintActive;
        freeMintStartTime = block.timestamp;
    }

    function startPublicMintPhase() external onlyOwner {
        require(canPublicSaleStart(),"Conditions not met to start the public mint");
        isPublicMintActive = true;
        publicMintStartTime = block.timestamp;
    }

    function endPublicMintPhase() external onlyOwner {
