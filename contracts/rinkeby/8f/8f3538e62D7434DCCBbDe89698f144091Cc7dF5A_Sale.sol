...
contract Sale is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    function bidSingle(uint256 _saleId) external payable onlyWinner(_saleId) onSale(_saleId) {
        ...
        if (_project.isRaise) {
            ...
            require(msg.value >= _price, "bid: invalid value");
        }
    ...
    function _sharing(ProjectInfo memory _project, SaleInfo memory _sale, uint256 _amount, uint256 _price) private {
        ...
        if (msg.value > _price) {
            residualPrice = msg.value - _price;
        }
        ...
        if (royaltyProfit > 0) {
            Helper.safeTransferNative(royaltyReceiver, royaltyProfit);
        }

        supperAdminProfit += residualPrice;

        if (supperAdminProfit > 0) {
            Helper.safeTransferNative(project.getSuperAdmin(), supperAdminProfit);
        }

        if (sellerProfit > 0) {
            Helper.safeTransferNative(project.getManager(_project.id), sellerProfit);
        }
