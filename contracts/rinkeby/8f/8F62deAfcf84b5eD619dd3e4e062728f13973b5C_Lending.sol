...
contract Lending {
    ...
    mapping(address => bool) public borrowerReported;
    ...
    event BorrowerRemoved(address removedBorrower);
    ...
    event BorrowerRequestReported(bytes32 id);
    ...
    function registerAsBorrower(string memory Cnic) public payable {
        address sender = msg.sender;
        require(!borrowerReported[sender], "You are Reported");
    ...
    function removedAsBorrower() public {
        address sender = msg.sender;
        require(!sender.isContract(), "Sender Cannot be Contract");
        require(borrower[sender], "Sender should be a borrower");
        borrower[sender] = false;
        sender.call{value: registrationCollateral}("");
        emit BorrowerRemoved(sender);
    }
