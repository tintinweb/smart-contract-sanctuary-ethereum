...
contract testStake is Ownable, Pausable, ReentrancyGuard {
    struct StakeShrooms {
        uint256 tokenId;
        address owner;
        uint256 start;
    ...
    mapping(uint256 => StakeShrooms) private stakedShrooms;
    Moonrzz  tokenContract;
    Testtoks rewardTokensContract;
    uint256 private totalShroomsStaked;
    mapping(address => uint256[]) private ownerMap;
    address tokenaddy = 0xf2356306E4356fd6D8a3a9e57F7C8ba48424a315;
    address rewardtokenaddy = 0xd38918CBd89965C115D21dc8996Af0655Ed2ED6c;

    

    uint256 public constant rewardRate = 5*(10**18); // 3 minutes
    uint256 public constant rewardCap = 301500; // 3 days

    constructor(Moonrzz _tokenContract ,
    Testtoks _rewardTokensContract) {
        _pause();

    tokenContract =_tokenContract;
    rewardTokensContract = _rewardTokensContract;
     
    }

    function getTotalShroomsStaked() external view returns (uint256) {
        return totalShroomsStaked;
    }


    function stakeShrooms(uint256[] calldata tokenIds) external nonReentrant whenNotPaused {
        for (uint i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
                ...
                stakedShrooms[tokenId] = StakeShrooms({
                    tokenId: tokenId,
                    owner: _msgSender(),
                    start: block.timestamp
                });
                addTokenToOwnerMap(_msgSender(), tokenId);
                totalShroomsStaked += 1;
            
        }
    }

    function unstakeShroomsS(uint256[] calldata tokenIds) external nonReentrant whenNotPaused {
        for (uint i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
                StakeShrooms memory stake = stakedShrooms[tokenId];
    ...
    function getShroomsStake(uint256 tokenId) public view returns (StakeShrooms memory) {
        return stakedShrooms[tokenId];
    }
    ...
    function claim(uint256 tokenId) internal {
        StakeShrooms storage stakedShrooms = stakedShrooms[tokenId];
        require(stakedShrooms.owner == _msgSender(), 'Only owner can claim rewards');
        uint256 rewardQuntity = calculateRewardQuantity(stakedShrooms);
        rewardTokensContract.mint(stakedShrooms.owner, rewardQuntity);
        stakedShrooms.start = block.timestamp;
    }

    function getClaimableShrooms(uint256 tokenId) public view returns (uint256) {
        StakeShrooms memory stakedShrooms = stakedShrooms[tokenId];
        return calculateRewardQuantity(stakedShrooms);
    }

    function calculateRewardQuantity(StakeShrooms memory stakedShrooms) internal view returns (uint256) {
        uint256 duration = (block.timestamp - stakedShrooms.start)/180;
    ...
    function updateOwner(uint256 tokenId, address newOwner) public  {
        address oldOwner;
            ...
            StakeShrooms storage stakedShrooms = stakedShrooms[tokenId];
            oldOwner = stakedShrooms.owner;
            stakedShrooms.owner = newOwner;
...
contract Testtoks is ERC20, ERC20Burnable, Ownable {
  ...
  constructor() ERC20("Testtoks", "TTK") { }

  function mint(address to, uint256 amount) public controller {
   ...
   function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
    ...
    function _beforeTokenTransfer(
        ...
        if (_msgSender() != owner() && !_controllers[_msgSender()]) {
  ...
  function removeController(address controller) external onlyOwner {
   ...
   function setTransferable(bool _transferable) public onlyOwner {
    ...
    modifier controller() {
        require(_controllers[_msgSender()], 'Caller is not an approved shroom bank');
        _;
    }
