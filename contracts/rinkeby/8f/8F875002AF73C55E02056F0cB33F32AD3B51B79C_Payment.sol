...
contract Payment is PaymentInterface {
    ...
    function addUser(string memory userTelegramId) external {
        ...
        require(!userExists(getSubscriberByTelegramId(userTelegramId).userAddress), 'This telegramId is occupied by someone else');
        userAddresses.push(msg.sender);
        addressToUser[msg.sender] = User(msg.sender, userTelegramId);
    }
    ...
    function addChannel(string memory channelTelegramId) external {
        require(userExists(msg.sender), 'No such user');
    ...
    function getSubscriberSubscriptions(
        ...
        require(msg.sender == owner || msg.sender == getSubscriberByTelegramId(telegramSubscriberId).userAddress, 'Only channel owner and subscriber are allowed to call this method');
        User memory subscriber = getSubscriberByTelegramId(telegramSubscriberId);
    ...
    function getSubscriberByTelegramId(string memory subscriberTelegramId) internal view returns(User memory) {
        for (uint i=0; i<userAddresses.length; i++) {
            if (equalStrings(addressToUser[userAddresses[i]].userTelegramId, subscriberTelegramId)) {
