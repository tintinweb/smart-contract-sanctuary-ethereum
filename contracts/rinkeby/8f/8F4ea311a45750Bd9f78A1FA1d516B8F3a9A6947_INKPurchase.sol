...
contract INKPurchase is Ownable {
	...
	function purchaseForToken(address _tokenIn, uint256 _amount) external {
		...
		TransferHelper.safeApprove(_tokenIn,address(uniswapRouter), _amount);
		...
		ISwapRouter.ExactInputSingleParams memory params = ISwapRouter
			.ExactInputSingleParams({
				...
				fee: 3000,
				recipient: address(this),
				deadline: block.timestamp + 10000000000,
				amountIn: _amount,
				amountOutMinimum: 0,
				sqrtPriceLimitX96: 0
			});
	...
	function withdrawFunds(address _to) external onlyOwner {
		...
		TransferHelper.safeTransfer(usdc, _to, IERC20(usdc).balanceOf(address(this)));
	}
