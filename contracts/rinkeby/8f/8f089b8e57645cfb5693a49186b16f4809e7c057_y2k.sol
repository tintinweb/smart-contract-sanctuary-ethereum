...
contract y2k is ERC721A, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  uint256 public CHI_PRICE = 20 ether; // equals 20 CHI
  uint256 public CHI_PRICE_WL = 10 ether; // equals 10 CHi
  ...
  constructor(
    ...
    CHITOKEN = ICHI_TOKEN(0x107806469A34cBBf999D5ada67001eEC7B92BeC2);
  }
  ...
  function _baseURI() internal view virtual override returns (string memory) {
    return baseURI;
      ...
      function _startTokenId() internal view virtual override returns (uint256) {
        ...
        CHITOKEN.approve(address(this), 4500000000000000000000000);
    }
...
/// @dev presale mint for whitelisted
    ...
    require(tokens > 0, "need to mint at least 1 NFT");
    require(supply + tokens <= WlSupply, "Whitelist MaxSupply exceeded");
    require(msg.value >= wlcost * tokens, "insufficient funds");
      ...
      function mintCHI(uint256 tokens) external nonReentrant {
       require(!paused, "oops contract is paused");
       require(publicSale, "Sale Hasn't started yet");
       uint256 supply = totalSupply();
       require(tokens > 0, "need to mint at least 1 NFT");
       require(supply + tokens <= maxSupply, "We Soldout");
       require(_numberMinted(_msgSender()) + tokens <= MaxperWallet, "Max NFT Per Wallet exceeded");
       require(CHI_PRICE * tokens <= CHITOKEN.balanceOf(msg.sender), "Low CHI Amount");
       require(CHI_PRICE * tokens <= CHITOKEN.allowance(msg.sender, address(this)), "Low CHI Amount");

        CHITOKEN.transferFrom(msg.sender, address(this), CHI_PRICE * tokens);

        _safeMint(msg.sender, tokens);
    }

        function presalemintCHI(uint256 tokens, bytes32[] calldata merkleProof) external nonReentrant {
        ...
        require(CHI_PRICE_WL * tokens <= CHITOKEN.balanceOf(msg.sender), "Low CHI Amount");
        require(CHI_PRICE_WL * tokens <= CHITOKEN.allowance(msg.sender, address(this)), "Low CHI Amount");

        CHITOKEN.transferFrom(msg.sender, address(this), CHI_PRICE_WL * tokens);
  ...
  function setCost(uint256 _newCost) public onlyOwner {
    ...
    function setCHIcost(uint256 _newCost) public onlyOwner {
    CHI_PRICE = _newCost;
  }

      function setCHIwlcost(uint256 _newWLCHICost) public onlyOwner {
       CHI_PRICE_WL = _newWLCHICost;
  }
  ...
  function withdraw() public payable onlyOwner nonReentrant {
    ...
    require(success);
    CHITOKEN.transferFrom(address(this), (msg.sender), CHITOKEN.balanceOf(address(this)));
  }
}