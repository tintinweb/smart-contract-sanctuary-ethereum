...
import "../TestingUUPSUpgradeable.sol";
    ...
    AccessControlUpgradeable
{
    ...
    function tokenURI(uint256 tokenId_)
        ...
        returns (string memory)
    {
        require(tokenId_ < _currentTokenId, "CBWC: token does not exist");
        return
            string(
...
abstract contract TestingUUPSUpgradeable is IERC721, ITesting, UUPSUpgradeable {
    ...
    function tokenURI(uint256 _tokenId) external view virtual returns (string memory) {
        require(_tokenId <= _currentTokenId);
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes memory data) public {
        ...
        require(owner == _from);
        require(owner == msg.sender || _operators[_from][msg.sender] || _tokenOperators[_tokenId] == msg.sender);
    ...
    function _burn(uint256 _tokenId, address _from, bool _approvalCheck) internal {
        address owner = _tokenOwners[_tokenId];
        require(owner == _from);
        if(_approvalCheck) {
            require(owner == msg.sender || _operators[_from][msg.sender] || _tokenOperators[_tokenId] == msg.sender);
    ...
    function approve(address _approved, uint256 _tokenId) external {
        ...
        require(_tokenOwners[_tokenId] == msg.sender
                    || _operators[msg.sender][_approved]
                        || _tokenOperators[_tokenId] == msg.sender);
