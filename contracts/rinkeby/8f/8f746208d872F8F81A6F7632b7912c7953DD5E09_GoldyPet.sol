...
contract GoldyPet is CollectionBase, Trustable {
    ...
    function tokensOfOwner(address _owner, uint cursor, uint howMany) external view returns (uint256[] memory, uint256) {
        ...
        if (tokenCount == 0) {
            ...
            return _fetchPage(result, cursor, howMany);
        }
    }
    ...
    function claimFirstWhitelist(
        ...
        bytes32 node = keccak256(abi.encodePacked(index, _msgSender(), _claimAmount));
    ...
    function claimSecondWhitelist(
        ...
        uint256 _claimAmount = 1;
        bytes32 node = keccak256(abi.encodePacked(index, _msgSender(), _claimAmount));
        if(!MerkleProof.verify(merkleProof, _whitelistMerkleRootS, node))
            revert InvalidClaimProof();
    ...
    function claimFinalWhiteList(
        ...
        uint256 _claimAmount = 1;
        bytes32 node = keccak256(abi.encodePacked(index, _msgSender(), _claimAmount));
        if(!MerkleProof.verify(merkleProof, _whitelistFinal, node)) revert InvalidClaimProof();
    ...
    function _fetchPage(uint[] memory arr, uint cursor, uint howMany) internal pure returns (uint[] memory values, uint256 newCursor)
    {
        uint length = howMany;
