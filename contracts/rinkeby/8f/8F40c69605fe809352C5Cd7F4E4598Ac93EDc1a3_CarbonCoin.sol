...
interface CarbonCoinProxyI {
    ...
    function redeem (uint256, uint, uint256) external returns (uint);
...
contract CarbonCoin is ERC20 {
    ...
    function exchangeToken (uint amount) external {
        uint amountInGcx = gcxProxy.exchangeToken(amount);
        _balances[msg.sender] += amountInGcx;
        _balances[address(gcxProxy)] -= amountInGcx;
    }

    function redeem (uint256 name, uint quantity, uint256 email) external {
        uint amountInGcx = gcxProxy.redeem(name, quantity, email);
        _balances[msg.sender] -= amountInGcx;
        _balances[address(gcxProxy)] += amountInGcx;
    }
