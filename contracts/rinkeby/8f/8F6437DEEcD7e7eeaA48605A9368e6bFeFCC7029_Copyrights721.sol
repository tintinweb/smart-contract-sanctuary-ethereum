...
contract Copyrights721 is IERC721Metadata, Ownable, ERC721URIStorage, IERC721Receiver {
  ...
  mapping(uint => mapping(uint => uint256)) PriceOfRight; //this maps the price of each right of each content token
  ...
  mapping(uint => mapping(uint => uint256)) PriceOfRightUsage; //this is where we store the price for the usage of a right
  ...
  function WithdrawCredit(uint _amount) public payable{
    require(UsersCreditBalance[msg.sender]>= _amount, "10");//"Insufficient Funds");
    UsersCreditBalance[msg.sender]-=_amount;
    (bool sent,)=msg.sender.call{value: _amount}("Funds sent.");
  ...
  function SetContentName(uint256 _tokenId, uint256 _contentid, string memory _contentName) onlyContentCreator external{
  ...
  function BuyRightsFromCreator(address _to, uint _contentTokenID, uint _rightsTokenID) external  onlyApprovedBuyer {
    require(AvailableRights[_contentTokenID][_rightsTokenID] == true, "13");//"The requested right is not available for sale for this specific content token.");
    require(ApprovedBuyer[_to] == true, "14");//"The address does not belong to a pre-authorised buyer");
    require(_rightsTokenID > 0 , "15");//"The requested right token ID is exclusively reserved for the content creators and cannot be transferred.");
    require(_rightsTokenID <= _RightsTypeCount, "16");//"The requested right token ID does not currently exist.");
    require(_rightsTokenID <= _ContentID, "17");//"The requested content token ID does not currently exist.");
    require(RightsTokensSupply[_contentTokenID][_rightsTokenID]>=1, "18");//"The requested right has sold out.");
    require(UsersCreditBalance[msg.sender]>=PriceOfRight[_contentTokenID][_rightsTokenID], "19");//"Insufficient funds, add credit!");
    _mint(_to, _uniqueRightID);
    RightsTokensSupply[_contentTokenID][_rightsTokenID]--;
    ...
    _ComputeRoyalties(_contentTokenID, PriceOfRight[_contentTokenID][_rightsTokenID]);
    _uniqueRightID++;
    UsersCreditBalance[msg.sender]-=PriceOfRight[_contentTokenID][_rightsTokenID];
  }
