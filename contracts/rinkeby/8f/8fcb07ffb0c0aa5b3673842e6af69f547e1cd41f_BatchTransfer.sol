...
contract BatchTransfer is Ownable{
    ...
    function batchTransferToken(address payable[] memory addrArr, 
        ...
        uint totalAmount = sum(amountArr, decimals);
        ...
        for (uint i=0; i < addrArr.length; i++) {
           ...
           _tokenTransfer(token, addrArr[i], msg.sender, amountArr[i], decimals);
        }
    ...
    function _tokenTransfer(IERC20 token, address payable addr, address sender, uint amount, uint8 decimals) internal {
        token.transferFrom(sender, addr, amount * (10**decimals));
   } 

    function sum(uint[] memory amountArr, uint8 decimals) internal pure returns(uint totalAmount){
        totalAmount = 0;

        for (uint i=0; i < amountArr.length; i++) {
            totalAmount += (amountArr[i] * (10**decimals));
        }
    }
