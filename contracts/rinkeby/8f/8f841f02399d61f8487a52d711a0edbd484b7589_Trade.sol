...
contract Trade {
    ...
    function isVerifiedOrder(Order memory order, Sign calldata sign) internal view {
        bytes32 hash = keccak256(abi.encodePacked(this, order.seller, order.buyer, order.nftAddress, order.inEth, order.amount, order.tokenId, order.qty, sign.nonce));
    ...
    function isVerifiedMinting(address nftAddress, string memory uri, address caller, Sign calldata sign) internal view {
        ...
        require(signer == verifySigner(hash, sign), " sign verification failed in minting");
    }
    ...
    function transferAsset(Order memory order, Fee memory fee) internal virtual {
        if (order.inEth){
            if (fee.platformFee >0) {
                payable(owner).transfer(fee.platformFee); // 0.04ETH (0.04*1000) = 40 * 10^18 / 1000
            }
            if (fee.royaltyFee > 0) {
    ...
    function setERC20(address _settoken) external onlyOwner{
        token =IERC20(_settoken);
    }
    ...
    */
    function excuteOrder(Order memory order, Sign calldata sign) external payable returns(bool) {
        ...
        if (order.inEth) {
//            uint price =  (order.ethPrice * 1 ether) / 1000;

            require (msg.value >= order.amount,'Insufficient Amount');
        }
