...
abstract contract ReentrancyGuard {
    ...
    function _isAuthorizedRe() internal view {
...
contract Snowball is Ownable, ERC721Enumerable, ReentrancyGuard {
    uint256 public immutable MAXSNOW;
    ...
    constructor(
        ...
        uint16 feeAmount,
        uint256 _offsetFloor
    ) Ownable() ERC721(_name, _symbol) {
        MAXSNOW = maxSupply;
        price = cost;
        startSupply = startTokens;
        devFund = devAddress;
        feeX = feeAmount;
        fee = (price * feeX) / 1000;
        offsetFloor = _offsetFloor;
    }

    function redeem(uint256 tokenId) external nonReentrant {
        require(ownerOf(tokenId) == msg.sender);
        _burn(tokenId);
        payable(msg.sender).transfer(address(this).balance / (totalSupply() + 1));
    }
    function changeDevFund(address payable newDev) external nonReentrant {
        require(devFund == msg.sender);
        devFund = newDev;
      }

    function getredeemfloor() external view returns (uint256) {
        return (address(this).balance / totalSupply());
    }

    function mint() external payable nonReentrant{
        require(totalSupply() + 1 <= MAXSNOW, "Not enough Tokens remaining.");
        if (totalSupply() >= startSupply) {
            price = ((((address(this).balance - msg.value) / totalSupply()) *
                offsetFloor) / 100);
            fee = (price * feeX) / 1000;
        }
        require(price + fee <= msg.value, "Incorrect amount sent!");
        _safeMint(msg.sender, trackid + 1);
        trackid += 1;
        payable(devFund).transfer(fee);
        payable(msg.sender).transfer(msg.value - price - fee);
    }
    ...
    function getpricecurrent() external view returns (uint256) {
        if (totalSupply() >= startSupply) {
            return ((((address(this).balance / totalSupply()) * offsetFloor) /
                100) +
                (((((address(this).balance / totalSupply()) * offsetFloor) /
                    100) * feeX) / 1000));
        } else {
            return (price + fee);
        }
     ...
     * SNOW for the team.
     */
    function reserveSNOW() public onlyOwner {
    ...
    function cumulativeHODL(address user) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < bal; i++) {
            ...
            uint256 timeHodld = block.timestamp -
                lastTransferTimestamp[tokenId];
