...
contract LiquidityVault is LiquidityVaultStorage, AccessiblePlusCommon {
        ...
        public afterSetUniswap
    {
        ...
        if(getPool == address(0)){
            ...
            uint160 encodePriceSqrt = initialSqrtPriceX96();
            require(encodePriceSqrt > 0, "encodePriceSqrt zero");
            pool.initialize(encodePriceSqrt);
        ...
        returns (uint256)
    {
        ...
        if(sqrtPriceX96 == 0 ) {
            ...
            if(token0Address == address(TOS)) {
                price = initialTosPrice.mul(1e18).div(initialTokenPrice) ;
            } else {
                price = initialTokenPrice.mul(1e18).div(initialTosPrice) ;
            }
            return price;
        } else {
        ...
        returns (uint256)
    {
        ...
        uint160 sqrtPriceX96 = 1174336783993468204589730522253;
        uint256 price = uint(sqrtPriceX96) * uint(sqrtPriceX96);
        console.log("getPriceTest %s",price);
        price = price >> (96 * 2);
    ...
    function getPriceTest2()
        external
        view
        returns (uint256)
    {
        uint160 sqrtPriceX96 = 1174336783993468204589730522253;
        uint256 price = uint(sqrtPriceX96).mul(uint(sqrtPriceX96)).mul(1e18) >> (96 * 2) ;
        console.log("getPriceTest2 %s",price);
    ...
    function tickBaseInfos(int24 tick) public view returns (int24, int24, uint24) {
        int24 minTick = (TickMath.MIN_TICK / int24(fee)) * int24(fee);
        int24 maxTick = (TickMath.MAX_TICK / int24(fee)) * int24(fee);
    ...
    function tickInfos(int24 tick) public view returns (int24, int24, uint128) {
        ...
        // int24 maxTick = (TickMath.MAX_TICK / int24(fee)) * int24(fee);
        // uint24 numTicks = uint24(int24((maxTick - minTick) / int24(fee))) + 1;
        (int24 minTick, int24 maxTick, uint24 numTicks) = tickBaseInfos(tick) ;

         // 25 % ;
        uint128 tickInterval = uint128(numTicks) * uint128(tickIntervalPercaentage) / 100 ;
    ...
    function getToken0Price(uint160 _sqrtPriceX96)
        external
        view
        returns (address, uint256)
    {
        //
        _sqrtPriceX96 = 1174336783993468204589730522253;

        if(_sqrtPriceX96 != 0) {

            uint256 price = uint(_sqrtPriceX96).mul(uint(_sqrtPriceX96)).mul(1e18) >> (96 * 2);
            console.log("getToken0Price %s %s", _sqrtPriceX96 , price);
            return (token0Address, price );
        } else {
            (uint160 sqrtPriceX96,,,,,,) =  pool.slot0();
            return (token0Address, uint(sqrtPriceX96).mul(uint(sqrtPriceX96)).mul(1e18) >> (96 * 2));
        }
    }

    function getToken1Price(uint160 _sqrtPriceX96)
        external
        view
        returns (address token1, uint256 price)
    {
        if(_sqrtPriceX96 != 0) {
            return (token1Address, (96 * 2) >> uint(_sqrtPriceX96).mul(uint(_sqrtPriceX96)).mul(1e18));
        } else {
            (uint160 sqrtPriceX96,,,,,,) =  pool.slot0();
            return (token1Address, (96 * 2) >> uint(sqrtPriceX96).mul(uint(sqrtPriceX96)).mul(1e18));
        }
    }
    */
    ...
    function createLP(uint256 tokenAmount, bool applySwapPrice)
        external
    {
        setPool();
        ...
        require(tokenAmount > 0
            ...
            "token is zero balance or insufficient.");

        uint256 tosBalance = TOS.balanceOf(address(this));
        console.log("tosBalance %s ", tosBalance);

        (, int24 tick,,,,,) =  pool.slot0();
        console.log("tick %s",uint24(tick));

        (int24 tickLower, int24 tickUpper, ) = tickInfos(tick);
        console.log("tickLower %s , tickUpper %s ", uint24(tickLower), uint24(tickUpper));

        // 현재 프로젝트 토큰대, 토스의 변환가격 확인하기.
        uint256 needTosAmount = 0;
        uint256 canUseTokenAmount = 0;
        uint256 amount0Desired = 0;
        uint256 amount1Desired = 0;

        if(!applySwapPrice){
            needTosAmount = tokenAmount * initialTokenPrice / initialTosPrice;


            if(needTosAmount > tosBalance){
                needTosAmount = tosBalance;
                canUseTokenAmount = tosBalance * initialTosPrice / initialTokenPrice ;
            } else {
                canUseTokenAmount = tokenAmount;
            }

            console.log("needTosAmount %s ", needTosAmount);
            console.log("canUseTokenAmount %s ", canUseTokenAmount);
        ...
        if(token0Address == address(TOS)){
            amount0Desired = needTosAmount;
            amount1Desired = canUseTokenAmount;
            if(amount0Desired > TOS.allowance(address(this), address(NonfungiblePositionManager)) ) {
                require(TOS.approve(address(NonfungiblePositionManager),TOS.totalSupply()),"TOS approve fail");
            }
            if(amount1Desired > token.allowance(address(this), address(NonfungiblePositionManager)) ) {
                require(token.approve(address(NonfungiblePositionManager),token.totalSupply()),"token approve fail");
            }
        } else {
            amount0Desired = canUseTokenAmount;
            amount1Desired = needTosAmount;
            if(amount0Desired > token.allowance(address(this), address(NonfungiblePositionManager)) ) {
                require(token.approve(address(NonfungiblePositionManager),token.totalSupply()),"token approve fail");
            }
            if(amount1Desired > TOS.allowance(address(this), address(NonfungiblePositionManager)) ) {
                require(TOS.approve(address(NonfungiblePositionManager),TOS.totalSupply()),"TOS approve fail");
            }
        }

        console.log("amount0Desired %s, amount1Desired %s ", amount0Desired, amount1Desired);
        require(amount0Desired > 0 && amount1Desired > 0, "amount0Desired & amount1Desired are zero");
        ...
        external
    {
        ...
        // 클래임 금액만큼의 토스가 있는지 확인하자. 클래임양+추가로 수수료받은양, 또는 더 있는 금액을 같이 한다.
        // 토스가 충분히 있으면 페어로 넣어서 민트한다.
        uint256 tosBalance = token.balanceOf(address(this));
        ...
        if(curRound == 0){
            needTosAmount = amount * initialTokenPrice / initialTosPrice;
            if(needTosAmount > tosBalance){
                needTosAmount = tosBalance;
                canUseTokenAmount = tosBalance * initialTosPrice / initialTokenPrice ;
            } else {
                canUseTokenAmount = amount;
            }
        } else {
            // 현재 스왑비에 맞게 해야한다.
        }
    ...
    function goMint(
        ...
        public
    {
         console.log("goMint token0Address %s, token1Address %s ",token0Address, token1Address );
         console.log("goMint fee %s  ",fee  );
         console.log("goMint tickLower %s, tickUpper %s ",uint256(uint24(tickLower)), uint256(uint24(tickUpper)));
         console.log("goMint amount0Desired %s, amount1Desired %s ",amount0Desired, amount1Desired);
         console.log("goMint amount0Min %s, amount1Min %s ",amount0Min, amount1Min);
         console.log("goMint address(this) %s  ",address(this));

         console.log("goMint deadline %s  ", block.timestamp + 100000 );
