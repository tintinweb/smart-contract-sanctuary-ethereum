...
// @author Thierry Chiret   https://twitter.com/thierry_chiret
...
// Le propriétaire du contrat est l'adresse avec laquelle le contrat a été déployé !!!

contract CameleonERC721A is Ownable, ERC721A, ERC721AQueryable, PaymentSplitter {

using Strings for uint;

// Before = 0, WhiteListSale = 1
 enum Step {
        Before,
        WhitelistSale,
        PublicSale,
...
Step public sellingStep; // Public donc geter généré automatiquement
...
uint public wlSalePrice = 0.01 ether; // Moins cher pour les whitelistés
uint public publicSalePrice = 0.15 ether;

uint public saleStartTime = 1659962859; // 11/08/2022  14h47
bytes32 public merkleRoot; // Contiendra la liste des whitelistés
string public baseURI;  // @ du serveur des métadonnées NFT

mapping(address => uint) amountNFTperWalletWhitelistSale; // Permet de savoir combien une adresse à minté de NFT pendant la WL
mapping(address => uint) amountNFTperWalletPublicSale; // Même chose mais pendant la public sale

// Une @ ne peux pas faire plus de mint que :
uint private constant maxPerAddressDuringWhitelistMint = 1;
uint private constant maxPerAddressDuringPublicMint = 3;

bool public isPaused;    // Mise en pause du SmartContract en cas de soucis/maintenance
uint private teamLength; // Nombre de personne dans la team

// Les wallet de la team => Pour le paiement
...
// % de rémunération de la team par ordre d'adresse
...
PaymentSplitter(_team, _teamShares) {
 ...
 /**
    ...
    * @param _proof The Merkle Proof  exemple de paramètre: ["0x075a2e1888f10966778c39ae49934e78dc46c4db765e57d4cb00211eacbfecde","0xfafaf5a53df1aa0613c46c1663058f55fd30987ef4e76ee56fa672946fb2e3c4"]
    **/
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser{
        ...
        require(isWhitelisted(msg.sender, _proof), "Not whitelisted"); // msg.sender est la personne qui mint
 ...
 function tokenURI(uint _tokenId) public view virtual override(ERC721A, IERC721A) returns (string memory) {
    ...
    */
    ...
    /**
    * @notice Release the gains on every accounts (Paiement de l'équipe)
    */
