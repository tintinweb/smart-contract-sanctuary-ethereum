...
contract TestTask {
    bytes32 private immutable passwd2;
    address private factory;
    uint24 public nonce;
    bytes32 private passwd1;

    constructor() {
        passwd1 = _hash(314159265358979323846264338327950288419716939937510582097494459);
        passwd2 = _hash(271828182845904523536028747135266249775724709369995957496696762);
    ...
    function secret1(bytes32 password_) external {
        ...
        require(password_ == bytes32(secret), "Denied");
        nonce += 1;
    }


    function secret2(bytes32 password_) external {
        bytes32 secret = passwd1;
        require(password_ == keccak256(abi.encode(secret, nonce)), "Denied");
        nonce += 1;
    }

    function secret3(bytes32 password_) external {
        bytes32 secret = passwd2;
        require(password_ == keccak256(abi.encode(secret, nonce)), "Denied");
        nonce += 1;
    }
