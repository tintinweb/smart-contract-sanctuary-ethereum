...
// File contracts/Hego.sol

pragma solidity ^0.8.6;
contract HEGO is ERC721A, ReentrancyGuard, Ownable {
    string public _baseTokenURI;
    uint256 public _reserved = 50;
    uint256 public _maxNFTPerWallet = 2;
    uint256 public _totalToBeMinted = 5555;
    uint256 public _totalwhitelistsale = 3333;
    ...
    constructor(
        ...
        uint256 totalwhitelistsale,
        uint256 reserved,
        uint256 presaleReserved,
        bytes32 merkleRoot
    ) ERC721A("Hego Project", "HEGO") {
        setBaseURI(baseURI);
        _merkleRoot = merkleRoot;
        _team = team;
        _reserved = reserved;
        _totalwhitelistsale = totalwhitelistsale;
        _totalToBeMinted = totalToBeMinted;
        _presaleReserved = presaleReserved;
        // team gets the first nft for giveaway
        _safeMint(team, 1);
    }
    ...
    function summon(uint256 amount) external payable {
        ...
        require(
            amount + balanceOf(msg.sender) <= _maxNFTPerWallet && amount > 0,
            "!NftAmount"
        );
    ...
    function whitelistSummon(uint256 amount, uint256 allowance, bytes32[] memory proof)
        external
        payable
    {
        require(!_paused, "Paused");
        require(_presale, "!presale");
        require(verify(proof, allowance, _merkleRoot), "!whitelist");
        require(
            amount + balanceOf(msg.sender) <= allowance && amount > 0,
            "!NftAmount"
        );
        uint256 supply = totalSupply();
        require(
        ...
        require(amount + supply + _reserved <= _totalwhitelistsale,
            ">availableSupply White List");
    ...
    function setTotalWhitelistSale(uint256 _newMaxwhiteList) external onlyOwner {
        _totalwhitelistsale = _newMaxwhiteList;
    }
    ...
    function setMaxNFTPerWallet(uint256 _newMax) external onlyOwner {
        _maxNFTPerWallet = _newMax;
    }
    ...
    function isWhitelisted(address account, uint256 allowance, bytes32[] memory proof)
        external
        view
        returns (bool)
    {
    ...
    function verify(bytes32[] memory proof, uint256 allowance, bytes32 root)
        internal
        view
        returns (bool)
    {
