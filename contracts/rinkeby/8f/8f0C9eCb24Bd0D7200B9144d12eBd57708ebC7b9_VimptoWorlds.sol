...
contract VimptoWorlds is Ownable, ERC721, ReentrancyGuard, VerifySignature {
    ...
    uint256 public normalMintPrice = 0.02 ether;
    ...
    function mintWhitelist(uint8 _toMint) public payable onlyWhitelisted nonReentrant {

        require(mintSupply > _tokenIds.current(), "All 10,000 NFT's have been minted.");
        require(msg.value == whitelistMintPrice * _toMint, "Mint price incorrect");
        if (partnerAddresses[msg.sender] == false) {
    ...
    function mintToPartner(address _address, uint8 _toMint) public onlyOwner returns(uint256) {
        require(mintSupply > _tokenIds.current(), "All 10,000 NFT's have been minted.");
        uint256 newItemId;
    ...
    function mintWorld(MintPayload calldata _payload, bytes memory _signature) public payable nonReentrant {

        require(mintSupply > _tokenIds.current(), "All 10,000 NFT's have been minted.");
        require(msg.value == normalMintPrice * _payload._toMint, "Mint price incorrect");
    ...
    function verifyOwnerSignature(MintPayload calldata _payload, bytes memory _signature) internal view returns(bool) {
        bytes32 ethSignedHash = getEthSignedMessageHash(getMessageHash(_payload.nonce, _payload.to));
    ...
    function setAdmin(address _verficationAdmin) external onlyOwner {
        verficationAdmin = _verficationAdmin;
    }
