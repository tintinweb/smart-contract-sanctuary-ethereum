...
contract portfolioFactory is Ownable {
    ...
    constructor( address buyerImplementation){
    ...
    function create_portfolio(string memory name, string memory symbol,uint randomNumber) external {
        bytes memory buyerProxy = type(UnstructuredProxy).creationCode;
        bytes memory tokenProxy = type(portfolioTokens).creationCode;
        bytes memory tokenCodeWithConsArgs = abi.encodePacked(tokenProxy,abi.encode(name,symbol));
        ...
        assembly {
            newBuyerProxyAddress := create2(0,add(buyerProxy,32),mload(buyerProxy),buyerSalt)
            if iszero(extcodesize(newBuyerProxyAddress)) {
                revert('Buyer Proxy Code', 0)
            }
            newTokenProxyAddress := create2(0,add(tokenCodeWithConsArgs,0x20),mload(tokenCodeWithConsArgs),tokenSalt)
            if iszero(extcodesize(newTokenProxyAddress)) {
