...
contract Domains is ERC721URIStorage {
    ...
    function register(string calldata _name) public payable {
        ...
        string memory name = string(abi.encodePacked(_name, ".", my_domain));
        ...
        console.log(
            "Registering %s.%s on the contract with my tokenId %d",
            _name,
            my_domain,
            newRecordId
        );
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                name,
                '", "description": " My Domain Name Service", "image": "data:image/svg+xml;base64,',
    ...
    function price(string calldata _name) public pure returns (uint) {
        uint length = StringUtils.strlen(_name);
        require(length > 0, "Name should be longer than 0");
        if (length == 3) {
        ...
        if (length >= 5) {
            return 1 * 10**16; // 0.01 ETH
        }
    }

    function setRecord(string calldata name, string calldata record) public {
        if (domains[name] != msg.sender) {
            revert Domains__OnlyOwnerCanSetRecord();
        }
        records[name] = record;
    }
