...
contract StakingPool is IStakingPool, Ownable, CheckContract{
    ...
    function resetWithdrawAmount(uint _amount) external override {
        _requireCallerIsWithdrawalPool();
    ...
    function requestWithdrawFromStakingPool(uint _amount) external override {
        ...
        taygenNFT.tokenURI(tokenIndex, _amount, tUSDCAmount, conversionRate, USDCFee, block.timestamp + MAXIMUM_WITHDRAWAL_PERIOD);
    ...
    function _requireCallerIsWithdrawalPool() internal {
        require(msg.sender == address(withdrawalPool), "StakingPool: Caller is not withdrawal pool");
    }
}