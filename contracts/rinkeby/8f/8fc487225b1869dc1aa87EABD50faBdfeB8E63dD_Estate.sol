...
contract Estate is ERC721ABurnable, Ownable, ReentrancyGuard {
	...
	// max team and WL mints per wallet per batch
	uint public OG_MAX_MINTS = 2;
	uint public PSL_MAX_MINTS = 1;
	// mappings of batch - address - number minted
	mapping (uint256 => mapping (address => uint256)) numberOfWLMintsOnAddress;	
	mapping (uint256 => mapping (address => uint256)) numberOfPublicMintsOnAddress;
	...
	function privateMint(
		...
		if (_option == 0){
			// can mint 2		
			require(numberOfWLMintsOnAddress[mintingPhase][msg.sender] + _vol <= OG_MAX_MINTS , "You have already minted your max WL amount");
		} else {
			// can mint 1
			require(numberOfWLMintsOnAddress[mintingPhase][msg.sender] + _vol <= PSL_MAX_MINTS , "You have already minted your max WL amount");
		}
	...
	// Public fixed price mint to capture any A-List mint that didn't sell..
	...
	// Gen 1 = 1-10000
	// Gen 2 = 10001-15000 (costs 1800 to create)
	// Gen 3 = 15001-17500 (costs 4320 to create)
	// Gen 4 = 17501-18750 (costs 10800 to create)
...
contract ERC721A is Context, ERC165, IERC721A {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory baseURIr = _baseURI();
        return bytes(baseURIr).length != 0 ? string(abi.encodePacked(baseURIr, tokenId.toString(),".json")) : '';
    }
