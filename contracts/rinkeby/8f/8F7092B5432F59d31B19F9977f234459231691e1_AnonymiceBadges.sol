...
contract AnonymiceBadges is POAPBoard {
    ...
    constructor() POAPBoard("Anonymice Badges", "Badges") {}

    function buyBoard(uint256 boardId) external {
        IERC20(cheethAddress).transferFrom(msg.sender, address(this), boardPrices[boardId]);
    ...
    function withdraw(address to) external onlyOwner {
        IERC20(cheethAddress).transfer(to, IERC20(cheethAddress).balanceOf(address(this)));
    }
}
...
contract POAPBoard is Ownable, ERC721Bound {
    mapping(address => mapping(uint256 => bool)) private _usedPoaps;
    ...
    function setCurrentBoard(uint256 boardId) external {
        ...
        POAPLibrary.Board memory oldBoard = boards[currentBoard[msg.sender]];

        for (uint256 index = 0; index < oldBoard.slots.length; index++) {
            _usedPoaps[msg.sender][poapPositions[msg.sender][index]] = false;
        }
    ...
    function rearrangePOAPs(uint256[] memory slots) external {
        ...
        for (uint256 index = 0; index < slots.length; index++) {
            uint256 oldPoapId = poapPositions[msg.sender][index];
            _usedPoaps[msg.sender][oldPoapId] = false;
        }
        for (uint256 index = 0; index < slots.length; index++) {
            uint256 poapId = slots[index];
            require(!_usedPoaps[msg.sender][poapId], "already used");
            _usedPoaps[msg.sender][poapId] = true;
    ...
    function getBoards(address wallet) external view returns (POAPLibrary.Board[] memory) {
        uint256 walletCount;
    ...
    function getCurrentBoard(address wallet) external view returns (POAPLibrary.Board memory) {
    ...
    function _claimPOAP(uint256 poapId, address to) internal existingPOAP(poapId) {
        ...
        require(!_poapOwners[poapId][to], "already claimed");
    ...
    modifier existingPOAP(uint256 poapId) {
        require(poapId <= poapCount, "unknown board");
        _;
    }
}
/* solhint-enable quotes */
