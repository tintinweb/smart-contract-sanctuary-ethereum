...
// File: contracts/Copy_nftteste.sol
...
contract MyToken is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public MAX_SUPLY;



    uint256 public ReservedPotatos = 50;



    uint256 public constant potatoPrice = 0.00 ether; 



    uint256 public constant maxPotatoPurchase = 20;
    ...
    constructor(uint256 maxNftSuply) ERC721("nftdeez", "nftt3") {

       MAX_SUPLY = maxNftSuply;      
    ...
    function withdraw() public onlyOwner{
        ...
        (bool sucess,) =  msg.sender.call{value: totalbalance}("");  
    ...
    function reservePotato() public onlyOwner{

        require((totalSupply() + ReservedPotatos)  <= MAX_SUPLY, "Not enough tokens left");

        uint supply = totalSupply();

        uint index;

        for (index = 0; index < ReservedPotatos; index++) {

            _safeMint(msg.sender, supply + index);
    ...
    function getPrice(uint256 tokenQty) public view returns(uint256){

        if(totalSupply() + tokenQty >20){
    ...
    //mint
     ...
     function mintNft(uint256 tknNumbers) public payable{

        require(SaleActive, "You can't do it now, sale inactive");

        //require(tknNumbers <= maxPotatoPurchase,"You can only mint a max of 20 tokens");

        require((getPrice(tknNumbers) * tknNumbers) <= msg.value, "Incorrect Ether value");

        require((totalSupply() + tknNumbers) <= MAX_SUPLY, "Not enough tokens left");



        for(uint256 x = 0; x < tknNumbers; x++) {

            uint index = totalSupply();

            if (totalSupply() < MAX_SUPLY) {
    ...
    function setStartingIndex() public {         
        ...
        require(startingIndex == 0, "Starting index ok");        

        require(startingIndexBlock != 0, "Starting index block not check");



        

        startingIndex = uint256(blockhash(startingIndexBlock)) % MAX_SUPLY;



        if ((block.number - startingIndexBlock) > 255) {

            startingIndex = uint(blockhash(block.number - 1)) % MAX_SUPLY;

        }



        //prevent starting index to be 0
