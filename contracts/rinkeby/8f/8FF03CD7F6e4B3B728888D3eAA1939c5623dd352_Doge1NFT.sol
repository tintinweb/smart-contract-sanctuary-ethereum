...
contract Doge1NFT is ERC721Enumerable, Ownable {
    ...
    function mintTo(address _to, uint _tokenId) public onlyOwner {
        require (_tokenId != 0 && _tokenId <= MAX_SUPPLY, "DOGE1NFT: Invalid token id");

        _safeMint(_to, _tokenId);
    }

    function batchMintTo(address[] memory _to, uint[] memory _tokenIds) external onlyOwner {
        require(_to.length == _tokenIds.length, "DOGE1NFT: Invalid array length");

        for(uint i = 0; i < _to.length; i++) {
            mintTo(_to[i], _tokenIds[i]);
        }
    }
