...
contract Marketplace {
    ...
    function listItemOnAuction(
        ...
        _auctionInfo.auctionStartTime = block.timestamp;
        _auctionInfo.auctionEndTime = block.timestamp + _auctionDuration;
    ...
    function listItem(
        ...
        if (BasicToken(_collection).ownerOf(_nftId) != msg.sender) {
    ...
    function removeItem(address _collection, uint256 _nftId) external {
        ...
        if (oldState == State.ONAUCTION) {
            if (items[_collection][_nftId].auctionInfo.bids > 0) {
                moneyTransfer(
                    items[_collection][_nftId].price.price,
                    items[_collection][_nftId].price.tokenAddr,
                    address(this),
                    items[_collection][_nftId].auctionInfo.lastBidder
                );
            }
        }
    ...
    function buyItem(address _collection, uint256 _nftId) external payable {
        ...
        BasicToken(_collection).safeTransferFrom(
            item.seller,
            msg.sender,
            _nftId
        );
    ...
    function acceptOffer(uint256 _offerId) external {
        ...
        BasicToken(offer.collection).safeTransferFrom(
            item.seller,
            offer.maker,
            offer.nftId
        );
...
contract BasicToken is ERC721, ERC721Burnable, AccessControl {
