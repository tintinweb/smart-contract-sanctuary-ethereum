...
contract BlockpartyContract is ERC165, ERC721URIStorage, EIP712, AccessControl, Ownable {
    ...
    // Default platform percentage fee
    uint8 platformFee = 5;
    ...
    /// @param fee - the NFT asset queried for royalty information
    function setFeePercentage(uint8 fee) external onlyOwner {
        platformFee = fee;
    }
    ...
    function royaltyInfo(
        ...
        require(creator != address(0), "Unidentifier Creator");
    ...
    /// @param voucher A signed Voucher that describes the NFT to be redeemed.
    function redeem(address buyer, Voucher calldata voucher) public payable returns (uint256) {
        // make sure signature is valid and get the address of the creator
        uint256 tokenId = tokenIdentityGenerator.current();
        // address creator = _verify(tokenId, voucher);
        address creator = voucher.from;
        ...
        // Stlores the creator for royalties tracking
        tokenCreators[tokenId] = creator;

        uint256 cut = (msg.value * platformFee) / 100;

        payable(owner()).transfer(cut);
        payable(creator).transfer(msg.value - cut);
    ...
    /// @notice Verifies the signature for a given Voucher, returning the address of the creator.
    /// @dev Will revert if the signature is invalid. Does not verify that the creator is authorized to mint NFTs.
    /// @param tokenId A token identification number.
    /// @param voucher An Voucher describing an unminted NFT.
    function _verify(uint256 tokenId, Voucher calldata voucher) internal view returns (address) {
        bytes32 digest = _hash(tokenId, voucher);
    ...
    function _hash(uint256 tokenId, Voucher calldata voucher) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("Voucher(uint256 price, address from, address to, string uri)"),
            tokenId,
            voucher.price,
            voucher.from,
            voucher.to,
