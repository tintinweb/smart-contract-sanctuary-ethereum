...
contract FuturesMarketManager is Owned, MixinResolver, IFuturesMarketManager {
    ...
    function _marketsForKeys(bytes32[] memory marketKeys)
        internal
        view
        returns (address[] memory)
    {
    ...
    function totalDebt() external view returns (uint256 debt, bool isInvalid) {
        ...
        for (uint256 i = 0; i < numOfMarkets; i++) {
            (uint256 marketDebt, bool invalid) = IMarketViews(
                _markets.elements[i]
            ).marketDebt();
    ...
    function _marketSummaries(address[] memory addresses)
        internal
        view
    ...
    function marketSummaries(address[] calldata addresses)
        external
        view
        returns (MarketSummary[] memory)
    {
    ...
    function marketSummariesForKeys(bytes32[] calldata marketKeys)
        external
        view
        returns (MarketSummary[] memory)
    {
     ...
     */
    function removeMarkets(address[] calldata marketsToRemove)
        external
        onlyOwner
    {
     ...
     */
    function removeMarketsByKey(bytes32[] calldata marketKeysToRemove)
        external
        onlyOwner
    {
     ...
     * it reverts if not called by a known market.
     */
    function burnSUSD(address account, uint256 amount)
        external
        onlyMarkets
        returns (uint256 postReclamationAmount)
    {
