...
interface ZomBunniesIF {
    function mintTokens(address _mintTo, uint256 quantity) external returns (bool);
...
contract ZombunniesDistributor is AccessControl {
    ...
    ZomBunniesIF zbToken;
    ChainBunniesIF cbToken;

    uint256 public tokenPrice = uint256(15 * 10**16); // = 0.15 ETH
    ...
    uint256 public _cap = 5000;// 3024 through burn, 1976 through mint
    uint256 public _burnToMint_cap = 3024;
    uint256 public _burnToMint_count = 0;
    uint256 public _totalBunniesBurned = 0;

    uint256 public _mint_cap = 1976;
    uint256 public _mint_count = 0;

    //zombunny token  id -> 0 -> chain bunny token id 1
    //zombunny token  id -> 1 -> chain bunny token id 2
    mapping(uint256 => mapping(uint256 => uint256) ) public mintsUsingNft;

    //mapping(address => uint256) public totalMintsPerAddress;

    constructor(ZomBunniesIF _zbToken, ChainBunniesIF _cbToken) {
    ...
    function mint(uint256 _num) public payable returns (bool) {
        ...
        uint256 tokenId = zbToken.getCurrentTokenId();
        require(tokenId + _num < _cap, "Maximum cap of mints reached");
        
        require(_mint_count + _num < _mint_cap, "Maximum cap of pay-to-mint reached");


        zbToken.mintTokens(msg.sender, _num);
    ...
    function mintUsingNFT(uint256[] calldata _tokenIds) noReentrant public returns (bool) {
        require(address(0) == upgradedToAddress, "Contract has been upgraded to a new address");
        require(whiteList[msg.sender] || !whitelistOnly, "ONLY WHITELIST"); //either on whitelist or whitelist is false
        require(!_mintingPaused, "Minting paused");

        uint256 tokenId = zbToken.getCurrentTokenId();
        require(tokenId + 1 < _cap, "Maximum cap of mints reached");

        require(_burnToMint_count + 1 < _burnToMint_cap, "Maximum cap of burn-to-mint reached");

        require(cbToken.isApprovedForAll(msg.sender, address(this)), "Sender has not approved this contract");
        
        require((_tokenIds.length == 2), "You can only swap 2 for 1");

        for (uint256 index = 0; index < _tokenIds.length; index++) {
            //cbToken.transferFrom(msg.sender, address(0x000000000000000000000000000000000000dEaD), _tokenIds[index]);
            cbToken.transferFrom(msg.sender, address(this), _tokenIds[index]);
            mintsUsingNft[tokenId + 1][index] = _tokenIds[index];
            _totalBunniesBurned++;
        }

        _burnToMint_count++;
        //zbToken.mintNextToken(msg.sender);
        zbToken.mintTokens(msg.sender,1);
        return true;
    }
    ...
    function giveAway(address _to, uint256 _amount) external {
        ...
        uint256 supply = zbToken.totalSupply();

        require( supply + _amount <= zbToken.cap(), "Cap reached");

        zbToken.mintTokens(_to, _amount);
    ...
    function adminWithdrawERC721Multi(address token, uint256[] calldata _tokenIds) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not admin");
