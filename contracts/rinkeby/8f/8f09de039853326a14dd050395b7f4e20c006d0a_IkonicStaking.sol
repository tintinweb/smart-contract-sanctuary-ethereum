...
contract IkonicStaking is Ownable,ReentrancyGuard {
    ...
    event SetAPR(uint[] _APR);
    event Poolsize(uint[] _poolSize);
    event LockTime(address _address,uint[] _time);
   ...
   // Set Reward Token Address
    ...
    function setLockPeriod(uint[] memory _locktime) public onlyOwner{
        lockTime=_locktime;
        emit LockTime(msg.sender,_locktime);
    }
