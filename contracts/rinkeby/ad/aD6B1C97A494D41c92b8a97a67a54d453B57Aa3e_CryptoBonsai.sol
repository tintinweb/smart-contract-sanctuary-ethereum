...
contract CryptoBonsai is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function random(string memory input) internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(input, msg.sender, block.timestamp))); //can change back to pure if msg.sender and blocks are removed
    }
    ...
    function getBackgroundType(uint256 tokenId) internal view returns (uint256) {
        ...
        function getAccessories(uint256 tokenId) internal view returns (uint256) {
    ...
    //     string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Bonsai #', toString(tokenId), '", "description": "Bonsais are 100% on chain and randomly generated to bring tranqulity to your blockchain. Very zen"', getTraits(bonsai), '"image": "data:image/svg+xml;base64,', Base64.encode(bytes(getBonsaiSVG(bonsai))), '"}'))));
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        cryptoBonsai memory bonsai = randomBonsai(tokenId);
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Bonsai #', toString(tokenId), '", "description": "Flowers are fully on-chain, randomly generated unique flowers. For you, or a special someone in your life."', getTraits(bonsai), '"image": "data:image/svg+xml;base64,', Base64.encode(bytes(getBonsaiSVG(bonsai/*,tokenId*/))), '"}'))));
        json = string(abi.encodePacked('data:application/json;base64,', json));
        return json;
    }
    ...
    constructor() ERC721("Crypto Bonsai Test Garden", "BONSAI") Ownable() {}
}
