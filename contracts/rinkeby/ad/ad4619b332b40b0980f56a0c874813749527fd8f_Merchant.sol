...
contract Merchant is IMerchant, AccessControl, ReentrancyGuard {
  ...
  uint256 public whitelistedAmount = 0;
  ...
  function availableWhitelistCapOf(address user_)
    public
    view
    override
    returns (uint256)
  {
    return _whitelist[user_];
  }
  ...
  {
    return SalesInfo({
      whitelistedAmount: whitelistedAmount,
  ...
  function reserve(ReservePayload[] memory payload_)
    ...
    onlyRole(OWNER_ROLE)
  {
    uint256 remaining = tier2RemainingAmount + tier3RemainingAmount;
    for (uint256 i = 0; i < payload_.length; i++) {
      address account = payload_[i].account;
      uint256 amount = payload_[i].amount;
      if (_purchased[account] + _whitelist[account] + amount > userCap) {
        continue;
      }
      require(
        whitelistedAmount + amount <= remaining,
        "Merchant: reserve exceeds the remaining amount"
      );
      _whitelist[account] += amount;
      whitelistedAmount += amount;
    }
  }
  ...
  function _processPresale(uint256 amount_, uint256 tier_, uint256 msgValue_) internal {
    require(amount_ <= _whitelist[_msgSender()], "Merchant: amount exceeded whitelist user cap");
    if (tier_ == 2) {
    ...
    _whitelist[_msgSender()] -= amount_;
    _purchased[_msgSender()] += amount_;
    whitelistedAmount -= amount_;
  }
...
interface IMerchant {
  function availableWhitelistCapOf(address user_) external view returns (uint256);
