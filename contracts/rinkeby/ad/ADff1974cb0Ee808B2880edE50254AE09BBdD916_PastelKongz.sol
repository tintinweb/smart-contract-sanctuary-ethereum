...
// File: contracts/PastelKongz.sol

pragma solidity ^0.8.0;

contract PastelKongz is ERC721A, Ownable {
    using Strings for uint256;
    ...
    constructor() ERC721A("Pastel Kongz", "PKZ", MAX_MINT_PER_TX) {}
    ...
    modifier isPaymentSufficient(uint256 amount) {
        if (totalSupply() + amount > 500 && totalSupply() < 500) {
            require(
                PRICE * (totalSupply() + amount - 500) <= msg.value,
                "There was not enough/extra ETH transferred to mint an NFT."
            );
        } else if (totalSupply() + amount > 500 && totalSupply() > 500) {
            require(
