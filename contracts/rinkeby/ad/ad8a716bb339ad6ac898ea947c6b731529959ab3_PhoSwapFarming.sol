    ...
    ReentrancyGuardUpgradeable
{
    ...
    function setPhoAddress(PhoToken _pho) external restricted {
        pho = _pho;
    }
    ...
    function stake(uint256 fid, uint256 amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            phoReward = user.amount.mul(farm.accPhoPerShare).div(1e12).sub(
                user.rewardDebt
            );
    ...
    function withdraw(uint256 fid, uint256 amount) external nonReentrant {
        ...
        uint256 phoReward = user.amount.mul(farm.accPhoPerShare).div(1e12).sub(
            user.rewardDebt
        );
