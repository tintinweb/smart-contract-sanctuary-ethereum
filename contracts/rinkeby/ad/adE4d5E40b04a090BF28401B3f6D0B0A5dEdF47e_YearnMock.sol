...
contract YearnMock is ERC20, VaultAPI {
	...
	/// @notice Function to convert yTokens to dai
	...
	/// @notice function to convert dai to yTokens.
	/// @param amount - amount of dai.
	function _sharesForValue(uint256 amount) public view returns (uint256)
	{
		if (totalSupply() > 0)
		{
			return amount * (10**18) / (pricePerShare());
		}
		else
		{
			return amount;
	...
	function deposit(uint256 amount) public override returns (uint256 shares) {
		shares = _sharesForValue(amount);  // convert dai to yTokens.
	...
	/// @param daiNumber - amount of dai
	/// @return shares - amount of yTokens
	function withdrawTokens(uint256 daiNumber, address receiver) public override returns (uint256 shares) {
		shares = _sharesForValue(daiNumber);  // convert dai to yTokens.

		_burn(msg.sender, shares);

		dai.transfer(receiver, daiNumber);

		mockBalance -= daiNumber;

		emit Withdrawed(shares, daiNumber, receiver);

		return daiNumber;
	}

	function increaseMockBalance(uint256 amount) public
	{
