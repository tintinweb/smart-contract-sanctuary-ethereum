...
contract RobofrensStaking is ERC20, ReentrancyGuard {
    ...
    mapping (address => RoboStaked[]) public depositsByUser;
    ...
    modifier updateUserRewards (address user) {
        RoboStaked[] memory stakedTokens = depositsByUser[user];
        unchecked {
            for(uint256 i = 0; i < stakedTokens.length; i++) {
                depositsByUser[msg.sender][i].lastClaimBlock = uint32(block.number);
                pendingRewards[user] += (block.number - stakedTokens[i].lastClaimBlock) * (rewardPerToken[stakedTokens[i].tokenId]) / (DAY / BLOCK_TIME);
            }        
        }
        _;
    }

    constructor(uint256[776] memory _rewards, address _stakingToken) ERC20("Robofrens", "ROBO") {
        stakingToken = IERC721(_stakingToken);
        for(uint256 i = 0; i < _rewards.length;i++) {
            rewardPerToken[i] = _rewards[i] * 10 ** 18;
        }
    ...
    function stake(uint16[] calldata tokenIds) external updateUserRewards(msg.sender) {
        unchecked {
            for(uint256 i = 0; i < tokenIds.length; i++) {
                uint16 currentId = tokenIds[i];
                depositsByUser[msg.sender].push(
                    RoboStaked({
                    tokenId: currentId,
                    lastClaimBlock: uint32(block.number)
                    })
                );
    ...
    function unstake(uint16[] calldata tokenIds) external updateUserRewards(msg.sender) {
        require(depositsByUser[msg.sender].length > 0, "0_NFTS_STAKED");
        RoboStaked[] memory userStaked = depositsByUser[msg.sender];
        unchecked {
            for(uint256 i = 0; i < tokenIds.length; ++i) {
                uint256 currentId = tokenIds[i];
                for(uint256 j = 0; j < userStaked.length; ++j) {
                    if(userStaked[j].tokenId == currentId) {
                        depositsByUser[msg.sender][j] = depositsByUser[msg.sender][userStaked.length - 1];
                        depositsByUser[msg.sender].pop();
    ...
    function claimRewards() external nonReentrant updateUserRewards(msg.sender) {
        uint256 userClaim = pendingRewards[msg.sender];
        require(userClaim > 0, "Nothing to claim");
    ...
    function getEarned(address user) public view returns (uint256) {
        RoboStaked[] memory staked = depositsByUser[user];
        uint256 rewards;
        unchecked {
            for(uint256 i = 0; i < staked.length;i++) {
                rewards += (block.number - staked[i].lastClaimBlock) * (rewardPerToken[staked[i].tokenId]) / (DAY / BLOCK_TIME);
            }
        }
    ...
    function getStaked(address user) public view returns (uint256[] memory) {
        RoboStaked[] memory roboStakedUser = depositsByUser[user];
        uint256[] memory staked = new uint256[](roboStakedUser.length);
        for(uint256 i = 0; i < roboStakedUser.length; i++) {
            staked[i] = roboStakedUser[i].tokenId;
        }
        return staked;
    }
