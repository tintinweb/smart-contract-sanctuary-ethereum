    ...
    AcceptedTokenList
{
    ...
    function initialize(uint256 maxBatchSize_, uint256 collectionSize_, uint256 amountMintDev_ )
        external
        initializer
    {
        __ERC721_init("Shiniki", "SIK", maxBatchSize_, collectionSize_);
        __Ownable_init();
        maxPerAddressDuringMint = maxBatchSize_;
        currentIndex[TYPE_1] = 0;
        currentIndex[TYPE_2] = collectionSize_ / 2;
        startIndexTYPE_2 = currentIndex[TYPE_2];
        revealed = false;
        defaultURI = "ipfs://defaultURI";
    ...
    function preSaleMint(
        ...
        require(isTokenAccepted(token), "Shiniki: token payment is not accept");
        require(isPreSaleOn(token), "Shiniki: pre sale has not begun yet");
        require(
        ...
        require(
            ...
            numberMinted(msg.sender) + quantity <= allowceInWhiteList(msg.sender),
            "Shiniki: can not mint this many"
        );
        uint256 price = uint256(salePrice[token].preSalePrice);
        if (token == address(0)) {
            transferETH(price * quantity);
        } else {
            transferToken(token, price * quantity);
        }
    ...
    function publicSaleMint(
        ...
        require(isTokenAccepted(token), "Shiniki: token payment is not accept");
        require(
            isPublicSaleOn(token),
            "Shiniki: public sale has not begun yet"
        );
        require(
            totalSupply() + quantity <= collectionSize,
            "Shiniki: reached max supply"
        ...
        uint256 price = uint256(salePrice[token].publicPrice);
        if (token == address(0)) {
            transferETH(price * quantity);
        } else {
            transferToken(token, price * quantity);
    ...
    function privateMint(uint256 quantity) public onlyOwner {
        uint256 checkQuantity = quantity % 2;
        require(checkQuantity == 0, "Shiniki: quantity is even number");
        uint256 quantityType = quantity / 2;
        uint256 numChunks = quantityType / maxBatchSize;
        for (uint256 i = 0; i < numChunks; i++) {
            _safeMint(msg.sender, maxBatchSize, TYPE_1);
        }
        for (uint256 i = 0; i < numChunks; i++) {
            _safeMint(msg.sender, maxBatchSize, TYPE_2);
        }
        if (quantityType % maxBatchSize != 0) {
            _safeMint(msg.sender, quantityType % maxBatchSize, TYPE_1);
            _safeMint(msg.sender, quantityType % maxBatchSize, TYPE_2);
        }
    }

    function setIsMarket(bool _isMarket)
        external
        onlyOwner
    {
        isMarket = _isMarket;
    ...
    function setBaseURI(bytes32 typeMint, string memory baseURI)
        external
        onlyOwner
    {
        _baseTokenURI[typeMint] = baseURI;
    }
    ...
    function isPreSaleOn(address token) public view returns (bool) {
        return
            salePrice[token].preSalePrice != 0 &&
            block.timestamp >= saleConfig.preSaleStartTime &&
            block.timestamp <=
            saleConfig.preSaleStartTime + saleConfig.preSaleDuration;
    }

    function isPublicSaleOn(address token) public view returns (bool) {
        return
            salePrice[token].publicPrice != 0 &&
    ...
    function setPreSaleConfig(
        ...
        uint256[] memory _preSalePrices
    ) external onlyOwner {
        require(_tokens.length == _preSalePrices.length, "invalid lengths");
        saleConfig.preSaleStartTime = _preSaleStartTime;
        saleConfig.preSaleDuration = _preSaleDuration;
        for (uint256 i = 0; i < _tokens.length; i++) {
            require(
                isTokenAccepted(_tokens[i]),
                "Shiniki: tokens is not accept"
            );
            salePrice[_tokens[i]] = SalePrice({
                preSalePrice: _preSalePrices[i],
                publicPrice: salePrice[_tokens[i]].publicPrice
            });
        }
    }
    ...
    function setPublicSaleConfig(
        ...
        address[] memory _tokens,
        uint256[] memory _publicPrices
    ) external onlyOwner {
        require(_tokens.length == _publicPrices.length, "invalid lengths");
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            salePrice[_tokens[i]] = SalePrice({
                preSalePrice: salePrice[_tokens[i]].preSalePrice,
                publicPrice: _publicPrices[i]
            });
        }
    }

    function addWhiteList(address[] memory addresses, uint256[] memory values) external onlyOwner {
        require(addresses.length == values.length, "Shiniki: input is invalid");
        for (uint256 i = 0; i < addresses.length; i++) {
            require(values[i] <= maxPerAddressDuringMint, "Shiniki: value is invalid");
    ...
    function getConfig(address token)
        public
        view
        returns (SaleConfig memory _saleConfig, SalePrice memory _salePrice)
    {
        _saleConfig = saleConfig;
        _salePrice = salePrice[token];
    }

    receive() external payable {}

    function transferETH(uint256 totalPrice) private {
    ...
    function transferToken(address token, uint256 amount) private {
        require(
            IERC20Upgradeable(token).balanceOf(msg.sender) >= amount,
            "Shiniki: Insufficient balance"
        );
        balanceToken[token] += amount;
    ...
    function withdrawAll() external onlyOwner nonReentrant {
        for (uint256 i = 0; i < numberAcceptedTokens(); i++) {
            if (acceptedTokenAt(i) == address(0)) {
                (bool success, ) = msg.sender.call{
                    value: address(this).balance
                }("");
                require(success, "Transfer failed.");
            } else {
                balanceToken[acceptedTokenAt(i)] = 0;
                IERC20Upgradeable(acceptedTokenAt(i)).safeTransferFrom(
                    address(this),
                    msg.sender,
                    balanceToken[acceptedTokenAt(i)]
                );
            }
        }
    }
    ...
    IERC721EnumerableUpgradeable
{
    ...
    uint256 public startIndexTYPE_2;
    ...
    // Revealed
    bool public isMarket;

    // type NFT
    bytes32 public constant TYPE_1 =
        0x41521001c25927dafc3766b88ee8dfd9c6579920a252eecbf0c4133d1c95ea99;
    bytes32 public constant TYPE_2 =
        0x4aa7aad171e412a987ede1c19fed2eeda893917809a9ce17846a9ed5c94a00a3;
    ...
    function __ERC721_init(
        ...
        __ERC721_init_unchained(name_, symbol_, maxBatchSize_, collectionSize_);
    }
    ...
    function totalSupply() public view override returns (uint256) {
        return currentIndex[TYPE_1] + (currentIndex[TYPE_2] - startIndexTYPE_2);
    }
    ...
    function safeTransferFrom(
        ...
        require(isMarket, "ERC721C: transfer when not accept");
        _transfer(from, to, tokenId);
        require(
...
contract AcceptedTokenList is OwnableUpgradeable {
    ...
    EnumerableSetUpgradeable.AddressSet internal acceptedTokens;
    // min priority fee per second for each token, to prevent spamming
    mapping(address => SalePrice) internal salePrice;
    ...
    function addAcceptedTokens(address[] memory _tokens) public onlyOwner {
        for (uint256 i = 0; i < _tokens.length; i++) {
