...
contract RomaCollectorPass1 is ERC721A, Ownable, ReentrancyGuard {
  ...
  mapping(string => uint256) public referalCodeMintCount;
  ...
  function whitelistMint(uint256 _mintAmount, string memory _referalCode, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    // Verify whitelist requirements
    require(whitelistMintEnabled, 'The whitelist sale is not enabled!');
    require(allowMultipleWhitelistMints || !whiteListClaimed[_msgSender()], 'WhiteList address already claimed!');
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), 'Your address is not whitelisted!');
    whiteListClaimed[_msgSender()] = true;
    referalCodeMintCount[_referalCode] = referalCodeMintCount[_referalCode] + 1;
    _safeMint(_msgSender(), 1);
  }

  function premintlistMint(uint256 _mintAmount, string memory _referalCode, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    // Verify premintlist requirements
    require(premintlistMintEnabled, 'The premint list sale is not enabled!');
    require(allowMultiplePremintlistMints || !premintlistClaimed[_msgSender()], 'Premint List Address already claimed!');
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), 'Your address is not premint listed!');
  ...
  function getReferalCodeMintCount(string memory _referalCode) public view returns (uint256) {
    return referalCodeMintCount[_referalCode];
  }
  ...
  function withdraw() public onlyOwner nonReentrant {
    // This will transfer a 50% of the contract balance to creator
    (bool hs, ) = payable(0x75d183d150286DB34228faCb0402748627155E3C).call{value: address(this).balance * 50 / 100}('');
    require(hs);
