...
contract Cactus is ERC721A, ReentrancyGuard, Ownable {

    uint256 public cost = 1 ether;
    uint256 public costPresale = 0.00025 ether;
...
function changeCostPresale(uint256 _costPresale) external onlyOwner {
    costPresale = _costPresale;
}
...
function presaleMint(address _account, bytes32[] calldata _proof, uint8 _amount) external payable nonReentrant whenNotPaused {
        //Are we in Presale ?
        ...
        require(msg.value == costPresale*_amount, "Not enought funds.");
...
function mint(uint256 quantity) public payable whenNotPaused nonReentrant {
    ...
    require(quantity > 0);
    require(msg.sender == tx.origin, "Only EAO");
    ...
    require(supply + quantity <= totalSupply(), "Sale is almost done and we don't have enought NFTs left.");
    nftsPerWallet[msg.sender] += quantity;
    ...
    function setUpPresale() external onlyOwner {
        require(sellingStep == Steps.Close, "Presale has not started yet.");
