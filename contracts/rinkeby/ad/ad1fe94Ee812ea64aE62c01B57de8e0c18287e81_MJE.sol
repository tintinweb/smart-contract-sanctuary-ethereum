...
contract MJE is ERC721Enum, Ownable, ReentrancyGuard {
    using Strings for uint256;
	
    uint256 public MAX_NFT = 9776;
    uint256 public TIER1_NFT = 5555;
    uint256 public TIER2_NFT = 2222;
    uint256 public TIER3_NFT = 1111;
    uint256 public TIER4_NFT = 888;
	...
	uint256 public MAX_BY_MINT_IN_TRANSACTION_SALE = 6;
	
	uint256 public TIER1_PRICE = 0.01 ether;
	uint256 public TIER2_PRICE = 0.00 ether;
	uint256 public TIER3_PRICE = 0.8 ether;
	uint256 public TIER4_PRICE = 0.01 ether;
	// uint256 public PRESALE_PRICE = 0.03 ether;
	// uint256 public CHANCELIST_PRICE = 0.03 ether;
	
    uint256 public SALE_MINTED;
	uint256 public TIER1_MINTED;
	uint256 public TIER2_MINTED;
	uint256 public TIER3_MINTED;
	uint256 public TIER4_MINTED;
	
    bool public saleEnableTIER1 = false;
    bool public saleEnableTIER3 = false;
    bool public saleEnableTIER4 = false;
    ...
    constructor() ERC721P('Marijuana Eats WTF', 'MJE') {}
	...
	function mint_TIER1(uint256 _count) public payable nonReentrant{
		uint256 totalSupply = totalSupply();
		require(
			saleEnableTIER1, 
			"TIER 1 sale is not enabled"
		);
        require(
			TIER1_MINTED + _count <= TIER1_NFT, 
			"Exceeds max limit"
		);
		require(
			users[msg.sender].TIER1 + _count <= MAX_MINT_SALE,
			"Exceeds max mint limit per wallet"
		);
		require(
		...
		users[msg.sender].TIER1 = users[msg.sender].TIER1 + _count;
    }

    function mint_TIER2(uint256 _count) public payable nonReentrant onlyOwner{
		uint256 totalSupply = totalSupply();
        require(
			TIER2_MINTED + _count <= TIER2_NFT, 
			"Exceeds max limit"
		);
		require(
			msg.value >= TIER2_PRICE * _count,
			"Value below price"
		);
		for (uint256 i = 0; i < _count; i++) {
            _safeMint(msg.sender, totalSupply + i);
		...
		users[msg.sender].TIER2 = users[msg.sender].TIER2 + _count;
    }

    function Airdrop_TIER2(address[] calldata _to, uint256[] calldata _id) public onlyOwner{
        require(_to.length == _id.length, "Receivers and IDs are different length");
        require(_id.length <= 2222, "Cannot Airdrop more than Supply");
        for (uint256 i = 0; i < _to.length; i++) {
            safeTransferFrom(msg.sender, _to[i], _id[i]);
        }
        // TIER2_MINTED = _id.length;
        // SALE_MINTED = SALE_MINTED + _id.length;

    }

    function mint_TIER3(uint256 _count, bytes32[] calldata merkleProof) public payable nonReentrant{
		bytes32 node = keccak256(abi.encodePacked(msg.sender));
		uint256 totalSupply = totalSupply();
		require(
			saleEnableTIER3,
			"TIER 3 Sale is not enabled"
		);
        require(
			TIER3_MINTED + _count <= TIER3_NFT, 
			"Exceeds max limit"
		);
		require(
		...
		require(
			users[msg.sender].TIER3 + _count <= MAX_MINT_PRESALE,
			"Exceeds max mint limit per wallet"
		);
		require(
		...
		users[msg.sender].TIER3 = users[msg.sender].TIER3 + _count;
    }

    function mint_TIER4(uint256 _count) public payable nonReentrant{
		uint256 totalSupply = totalSupply();
		require(
			saleEnableTIER4, 
			"Sale is not enable"
        ...
        require(
			TIER4_MINTED + _count <= TIER4_NFT, 
			"Exceeds max limit"
		);
		require(
			users[msg.sender].TIER4 + _count <= MAX_MINT_SALE,
			"Exceeds max mint limit per wallet"
		);
		require(
			_count <= MAX_BY_MINT_IN_TRANSACTION_SALE,
		...
		users[msg.sender].TIER4 = users[msg.sender].TIER4 + _count;
    }
	...
	function updateTIER1SalePrice(uint256 newPrice) external onlyOwner {
        TIER1_PRICE = newPrice;
    }

	function updateTIER3SalePrice(uint256 newPrice) external onlyOwner {
        TIER3_PRICE = newPrice;
    }

	function updateTIER4SalePrice(uint256 newPrice) external onlyOwner {
        TIER4_PRICE = newPrice;
    }
	...
	function setTIER1SaleStatus(bool status) public onlyOwner {
        require(saleEnableTIER1 != status);
		saleEnableTIER1 = status;
    }
	
	// function setTIER2SaleStatus(bool status) public onlyOwner {
	//    require(saleEnableTIER2 != status);
    //    saleEnableTIER2 = status;
    // }

	function setTIER3SaleStatus(bool status) public onlyOwner {
	   require(saleEnableTIER3 != status);
       saleEnableTIER3 = status;
    }

	function setTIER4SaleStatus(bool status) public onlyOwner {
	   require(saleEnableTIER4 != status);
       saleEnableTIER4 = status;
    }
	...
	function updateMaxMintSaleLimit(uint256 newLimit) external onlyOwner {
	    require(MAX_NFT >= newLimit, "Incorrect value");
        MAX_MINT_SALE = newLimit;
    }
	
	function updateTier3MintLimit(uint256 newLimit) external onlyOwner {
	    require(TIER3_NFT >= newLimit, "Incorrect value");
