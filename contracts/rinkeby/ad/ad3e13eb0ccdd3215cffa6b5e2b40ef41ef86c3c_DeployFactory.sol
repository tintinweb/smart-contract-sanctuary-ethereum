...
library Address {
    ...
    function functionDelegateCall(
        ...
        // console.log("msg.senderererer: ", msg.sender);
...
// File: contracts/utils/IBeacon.sol
...
contract DeployFactory is Ownable {
    address public currentImplemtation;
    mapping(address => address) public walletAddress;

    event UpdateImplemtation(address oldImplementation, address newImplementation);

    function updateImplemtation(address newImplementation) external onlyOwner {
        address oldImplementation = currentImplemtation;
        currentImplemtation = newImplementation;
        emit UpdateImplemtation(oldImplementation, newImplementation);
    }

    function newFilter(address admin, bool reCreate) public returns(address) {
        if(!reCreate && walletAddress[msg.sender] != 0x0000000000000000000000000000000000000000) {
            return 0x0000000000000000000000000000000000000000;
        }
        TransparentUpgradeableProxy c = new TransparentUpgradeableProxy(currentImplemtation, msg.sender, abi.encodeWithSignature("initialize(address,address)", msg.sender, admin));
        walletAddress[msg.sender] = address(c);
        return address(c);
    }
}