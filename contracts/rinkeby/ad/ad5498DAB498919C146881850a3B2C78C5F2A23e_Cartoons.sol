...
/*
    CARTOONS NFT Contract
*/
contract Cartoons is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public _reserved = 69; // for giveaway
    ...
    // Cartoons
    bytes32 public root;
    uint256 public rootMintAmt;

    constructor (bytes32 _root, uint256 _rootMintAmt) ERC721("Rinkeby Test Cartoons", "TOONS"){
        baseURI = "https://gateway.pinata.cloud/ipfs/QmWBuGHQE8vBFEvnXfHzH33H5Jk9A8n5qNUYfnFeRvPXGF/";
    ...
    function whitelistMint(bytes32[] calldata _proof, uint256 _amt) public payable nonReentrant {
        require(claimed[msg.sender] + _amt <= MAX_SUPPLY, "Requested Claim Amount Invalid");
        require(msg.value >= itemPrice * _amt,  "Insufficient Payment");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_proof,root,leaf), "Invalid Proof");

        // CHECK PRICE

        claimed[msg.sender] += _amt;

        uint256 mintAmount = _amt;
    ...
    // Add new root
    function plantRoot(bytes32 _root, uint256 _amt) public onlyOwner {
        root = _root;
        rootMintAmt = _amt;
    }
    ...
    function getAllowedMintAmount(bytes32[] calldata _proof, address _user) public view returns (uint256 amount) {
        ...
        require(MerkleProof.verify(_proof,root,leaf), "Invalid Proof");
        amount = rootMintAmt - claimed[_user];
