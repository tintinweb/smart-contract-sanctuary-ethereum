    ...
    Ownable
{
    ...
    uint256 public constant EARN_RATE = 100; // 11574074074000;
    ...
    constructor(NFT memory metadata) ERC721A("Shilladel Pass", "SHILL") {
    ...
    function earned(uint256 tokenId) public view virtual returns (uint256) {
        ...
        return base.add(tiers[_tier[tokenId]]).sub(_spent[tokenId]);
    }
    ...
    function mint(uint256 quantity) public virtual onlyMinter {
        ...
        returns (string memory)
    {
        ...
        for (uint256 i; i < _uriParts.length; i++) {
            if (_checkTag(_uriParts[i], _NAME_TAG)) {
                ...
                byteString = abi.encodePacked(byteString, tokenTier);
            } else {
    ...
    function _beforeTokenTransfers(
        ...
        // Return if highest tier
        uint256 index = tierCount.sub(1);
        if (_tier[startTokenId] == index) return;

        // Update tier if appropriate
        uint256 tier_ = tier(startTokenId);
        _tier[startTokenId] = tier_;
    }
