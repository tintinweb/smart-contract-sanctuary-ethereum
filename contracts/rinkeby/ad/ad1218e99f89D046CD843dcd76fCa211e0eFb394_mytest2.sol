...
contract mytest2 is IERC20, Ownable {
    ...
    constructor(
        ...
        uint16 taxFeeLpt_,
        uint16 liquidityFeeLpt_,
        uint16 charityFeeLpt_,
        uint8 decimalsLpt_
    )  {
        require(taxFeeLpt_ >= 0, "Invalid tax fee");
        require(liquidityFeeLpt_ >= 0, "Invalid liquidity fee");
        require(charityFeeLpt_ >= 0, "Invalid charity fee");
        ...
        require(
            taxFeeLpt_ + liquidityFeeLpt_ + charityFeeLpt_ <= 10**4 / 4,
            "Total fee is over 25%"
        );

        _name = name_;
        _symbol = symbol_;
        _decimals = decimalsLpt_;
        ...
        _liquidityFee = liquidityFeeLpt_;
        _previousLiquidityFee = _liquidityFee;

        _charityAddress = charityAddress_;
        _charityFee = charityFeeLpt_;
    ...
    function setTaxFeePercent(uint256 taxFeeLpt) external onlyOwner {
        _taxFee = taxFeeLpt;
        require(
    ...
    function setLiquidityFeePercent(uint256 liquidityFeeLpt)
        external
        onlyOwner
    {
        _liquidityFee = liquidityFeeLpt;
        require(
            _taxFee + _liquidityFee + _charityFee <= 10**4 / 4,
            "Total fee is over 25%"
        );
