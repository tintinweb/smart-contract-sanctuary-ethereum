    ...
    Ownable
{
    ...
    // tokenId => expire unix timestamp
    mapping(uint256 => uint256) public tokenIdToExpireTime;
    using Counters for Counters.Counter;
    Counters.Counter private _idCounter;

    constructor(string memory _uri, address _futureTokenPriceAddress) ERC1155(_uri) {
        futureTokenPrice = FutureTokenPrice(_futureTokenPriceAddress);
    }

    function setExchangeAndOwnerAddress(address _exchangeAddress, address _owner) external onlyOwner {
    ...
    function setNewExpireTimeOfTokenId(uint256 _tokenId, uint256 _newUnixTime) public onlyRole(DEFAULT_ADMIN_ROLE) {
        tokenIdToExpireTime[_tokenId] = _newUnixTime;
    }
    ...
    function mint(
        ...
        require(tokenIdToExpireTime[id] > 0, "future token does not create yet");
        require(
    ...
    function createNew(
        ...
        require(tokenIdToExpireTime[id] == 0, "future token already exists");
        _mint(account, id, amount, data);
        tokenIdToExpireTime[id] = expire;
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < expires.length; i++) {
            tokenIdToExpireTime[ids[i]] = expires[i];
        }
    }
        ...
        returns (FutureTokenData memory)
    {
        return
            FutureTokenData({
                ...
                expire: tokenIdToExpireTime[_futureTokenId],
