...
contract AstralTreasury is AstralAccessControlled, ITreasury {

	using SafeMath for uint256;
	using SafeERC20 for IERC20;

	ISTRL public immutable STRL;
	IsSTRL public sSTRL;
	...
	constructor(
		...
		require(_strl != address(0), "Treasury: zero address for STRL");
		STRL = ISTRL(_strl);

		blocksNeededForQueue = _timelock;
	}

	// give to smart contract SLP or SC
	// and take back STRL
	function deposit(
		uint256 _amount,
		address _token,
		...
		// mint STRL needed and store amount of rewards for distribution
		send_ = value.sub(_profit);
		STRL.mint(msg.sender, send_);
	...
	function withdraw(
		...
		uint256 value = tokenValue(_token, _amount);
		STRL.burnFrom(msg.sender, value);

		totalReserves = totalReserves.sub(value);

		IERC20(_token).safeTransfer(msg.sender, _amount);

		emit Withdraw(_token, _amount, value);
	}

	// allow aprroved address to withdraw excess reserves 
	// as SLP or SC
	...
	// mint new STRL using excess reserves
	// not more then excess reserves
	function mint(
		address _recipient,
		uint256 _amount
	) external override {
		...
		STRL.mint(_recipient, _amount);
		emit Minted(msg.sender, _recipient, _amount);
	}

	/**
	 * DEBT: The debt functions allow approved addresses to borrow treasury assets
	 * or STRL from the treasury, using sSTRL as collateral. This might allow an
	 * sSTRL holder to provide STRL liquidity without taking on the opportuity cost 
	 * of unstaking, or alter their backing without imposing risk onto the treasury.
	 * Many of these use cases are yet not be defined, but they appear promising.
	 * However, we urge the community to think critically and move slowly upon
	 * proposals to acquire these permissions.
	 */

	// allow approved address to borrow reserves in SC
	function incurDebt(uint256 _amount, address _token) external override {
		uint256 value;
		if (_token == address(STRL)) {
			require(permissions[STATUS.STRLDEBTOR][msg.sender], notApproved);
			value = _amount;
		} else {
            require(permissions[STATUS.RESERVETOKEN][_token], notAccepted);
		...
		sSTRL.changeDebt(value, msg.sender, true);
		require(sSTRL.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
		totalDebt = totalDebt.add(value);

		if (_token == address(STRL)) {
			STRL.mint(msg.sender, value);
			strlDebt = strlDebt.add(value);
		} else {
			totalReserves = totalReserves.sub(value);
			IERC20(_token).safeTransfer(msg.sender, _amount);
		}
	...
	function repayDebtWithReserve(
		...
		IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
		uint256 value = tokenValue(_token, _amount);
		sSTRL.changeDebt(value, msg.sender, false);
		totalDebt = totalDebt.sub(value);
		totalReserves = totalReserves.add(value);
		emit RepayDebt(msg.sender, _token, _amount, value);
	}

	// allow approved address to repay borrowed reserves with STRL
	function repayDebtWithSTRL(uint256 _amount) external {
		require(
			permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.STRLDEBTOR][msg.sender], 
			notApproved
		);

		STRL.burnFrom(msg.sender, _amount);
		sSTRL.changeDebt(_amount, msg.sender, false);
		totalDebt = totalDebt.sub(_amount);
		strlDebt = strlDebt.sub(_amount);
		emit RepayDebt(msg.sender, address(STRL), _amount, _amount);
	}

	// takes inventory of all tracked assets
	// always consolidate to recognize reserves before audit
	...
	function enable(
		...
		require(timelockEnabled == false, "Treasury: use queueTimelock");
		if (_status == STATUS.SSTRL) {
			// reset sStrl
			sSTRL = IsSTRL(_address);
		} else {
	...
	function disable(STATUS _status, address _toDisable) external {
		require(
			...
			"Treasury: only governor or guardian"
		);
	...
	// check if registtry contains address
	...
	function queueTimelock(
		...
		require(_address != address(0), "Treasury: address could not be zero");
		require(timelockEnabled == true, "Treasury: timelock is disabled, use enable");
	...
	function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SSTRL) {
            sSTRL = IsSTRL(info.toPermit);
        } else {
	...
	// cancel timelock action
	function nullify(uint256 _index) external onlyGovernor {
		permissionQueue[_index].nullify = true;
	}

	// disable timelocked functions
	function disableTimelock() external onlyGovernor {
		require(timelockEnabled == true, "Treasury: timelock already disabled");
	...
	// enables timelocks after initialization
	function initialize() external onlyGovernor {
		require(initialized == false, "Treasury: already initialized");
		timelockEnabled = true;
		initialized = true;
	}

	function excessReserves() public view override returns (uint256) {
		return totalReserves.sub(STRL.totalSupply().sub(totalDebt));
	}

	// returns STRL valuation asset
	function tokenValue(
		address _token, 
		uint256 _amount
	) public view override returns (uint256 value_) {
		value_ = _amount.mul(10**IERC20Metadata(address(STRL)).decimals()).div(10**IERC20Metadata(_token).decimals());
	...
	function baseSupply() external view override returns (uint256) {
		return STRL.totalSupply().sub(strlDebt);
	}
}
