...
contract ICO {
    ...
    constructor(address _treasuryAddress, address _owner) {
        require(address(this) != address(0), "ICO: address must be valid");
        require(_owner != address(0), "ICO: owner must be valid");
        owner = _owner;
        treasuryAddress = _treasuryAddress;
        SpaceCoin newSpacecoin = new SpaceCoin(
            _treasuryAddress, // treasuryAddress
            address(this), // icoAddress
            _owner // owner
        );
        spacecoin = newSpacecoin;
    }
    ...
    modifier canContributeForCurrentPhase() {
        if (currentPhase == Phases.SEED) {
            require(
                ...
                "ICO: phase SEED_CONTRIBUTIONS_CAP reached"
            );
            require(
            ...
            require(
                addressToContributions[msg.sender] <
                    GENERAL_CONTRIBUTIONS_PER_ADDRESS,
                "ICO: contribution maximum reached"
            );
        }
        _;
    ...
    function buyTokens()
        ...
        uint256 amountToReturn = _refundExcessEther(msg.value);
    ...
    function _refundExcessEther(uint256 _messageValue)
        private
        view
        canContributeForCurrentPhase
        returns (uint256)
    ...
    function addInvestors(address[] memory addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            approvedSeedInvestors[addresses[i]] = true;
        }
    }
    ...
    function claimTokens() external {
        ...
        addressToContributions[msg.sender] = 0;
        require(
            amountToTransfer <= spacecoin.balanceOf(address(this)),
            "Insufficient SPC balance in contract"
        );
...
contract SpaceCoin is ERC20 {
    uint256 public constant TAXES_IN_PERCENT = 2;
    uint256 public constant TREASURY_ALLOTMENT = 350_000 * 10**18;
    uint256 public constant ICO_ALLOTMENT = 150_000 * 10**18;
    bool public taxesOn = false;
    address public treasuryAddress;
    ...
    constructor(
        ...
        owner = _owner;
        _mint(_icoAddress, ICO_ALLOTMENT);
        _mint(_treasuryAddress, TREASURY_ALLOTMENT);
    }
    ...
    function toggleTaxesOn() external onlyOwner {
        taxesOn = !taxesOn;
    }

    //  override the _transfer function inside Open Zeppelin's' ERC20 contract
    // in order to be able to tax tokens during the transfer process
    function _transfer(
        address _from,
        address _to,
        uint256 _amount
        ...
        if (taxesOn) {
            uint256 taxAmount = (_amount * TAXES_IN_PERCENT) / 100;
