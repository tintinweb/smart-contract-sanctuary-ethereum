...
contract AveragePFPsTESTGAS5 is ERC721, Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    function claimDrop() external nonReentrant {
        ...
        _mintLoop(msg.sender, _avgcreatures.walletOfOwner(msg.sender), _avgcreatures.walletOfOwner(msg.sender).length);
    }

    // Froot Loops
    function _mintLoop(address _receiver, uint256[] memory _mintArray, uint256 _mintArrayLength) internal {
      for (uint256 i; i < _mintArrayLength; i++) {
