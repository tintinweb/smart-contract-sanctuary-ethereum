...
contract AltMint is PackDropBase {
    ...
    constructor()
        PackDropBase(
            ...
            0xc4eED00fd150b5d3F4553ECfa8c2E6fb5AAFE95F,                         // Alt Vault Proxy deployed on Rinkeby - to be changed on mainnet
    ...
    TokenMintingPool
{
     ...
     */
    function lockTokenSupply() external onlyAdmin {
        _lockTokenSupply();
    }

    /* =========================================== ALOWLIST LIST HELPERS =========================================== */
    ...
    /**
     * @dev allows users to check if they have presale access at their own discretion, through Alt UI
     */
    function selfCheckPresaleAccess(bytes calldata _signature)
        external
        view
        returns (bool)
    {
        return _hasAccess(_msgSender(), _signature);
    }
...
abstract contract TokenMintingPool is VRFConsumerBaseV2 {
    ...
    uint64 private _vrfSubscriptionId; // Chainlink Subscription ID
    ...
    function _lockTokenSupply() internal {
        ...
        uint256 numberSeeds = Math.min(initialSupply, 10);
        require(!tokenSupplyLocked(), 'TokenMintingPool: Token supply is already locked.');
        s_requestId = COORDINATOR.requestRandomWords(
            _vrfKeyHash,
            _vrfSubscriptionId,
            3, // Default number of confirmations
            200000, // See https://docs.chain.link/docs/vrf-contracts/
            uint32(numberSeeds) // This value should be lower than 500 (VRF v2 can only provide 500 seeds by call)
        );
    }
    ...
    function _mintTokens(
        ...
        for (uint256 ii = 0; ii < numTokens; ii++) {
            uint256 index = _randomSeeds[_seedCounter % initialSupply] % remainingSupplyCount();
