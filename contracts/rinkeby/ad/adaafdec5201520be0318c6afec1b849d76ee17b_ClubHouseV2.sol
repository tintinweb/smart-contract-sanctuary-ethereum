...
contract ClubHouseV2 is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    ERC721Upgradeable public ClubHouseAddress;

    /**
     * @dev Emitted when new token is minted by owner.
     */
    event createdNft(address user, uint256 referenceId, uint256 tokenId, uint256 price, uint256 dollar, uint256 sizeId);

     /**
     * @dev Emitted when token is purchased by user.
     */
    event purchaseNft(address previousOwner, address newOwner, uint256 tokenId, uint256 price, uint256 dollar);
    ...
    function initialize() public initializer {
        ...
        margin = 10;
        merchantWalletAddress = payable(0x25fC99eF8C2bE73c303f7e54A0c2Da423E48326b);
    }
    /**
    ...
    function updateAddress() external onlyOwner {
        ClubHouseAddress = ERC721Upgradeable(address(this));
    }
    ...
    function updateMargin(uint256 _margin)
        external
        virtual
        onlyOwner
    {
    ...
    function mint(MintOrder1 calldata _order, bytes memory _signature)
        external
        payable
        virtual
        nonReentrant
        returns (bool)
    {
        require(SignatureCheckerUpgradeable.isValidSignatureNow(owner(), _order.message, _signature), "ClubHouseV1: Not allowed to mint"); // for verification
        ...
        emit createdNft(msg.sender, _order.id, tokenCounter, msg.value, _order.dollarPrice, _order.size_id);
    ...
    function purchase(Order1 calldata _order, bytes memory _signature)
        external
        payable
        virtual
        nonReentrant
        returns (bool)
    {
        ...
        require(SignatureCheckerUpgradeable.isValidSignatureNow(_previousOwner, _order.message, _signature), "ClubHouseV1: Not allowed to purchase"); // for verification
        require(_order.price == msg.value, "ClubHouseV1: Price is incorrect"); // checks the price

        transferFrom(ownerOf(_order.id), address(this), _order.id);
        ClubHouseAddress.approve(msg.sender, _order.id);
        transferFrom(address(this), msg.sender, _order.id);

        uint256 _fee = feeCalulation(msg.value);
        merchantWalletAddress.transfer(_fee);
        payable(_previousOwner).transfer(msg.value - _fee);

        emit purchaseNft(_previousOwner, msg.sender, _order.id, msg.value, _order.dollarPrice);
