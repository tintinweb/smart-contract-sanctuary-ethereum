...
contract NodeManager is Ownable, HelperOwnable, IERC721, IERC721Metadata, IMansionManager {
    ...
    uint64 public nodeLifeCycle = 90 days;
    ...
    function setNodeLifeCycle(uint64 _lifeCycle) external onlyOwner {
        require(_lifeCycle > 0, "Node Life Cycle should not be zero");
        nodeLifeCycle = _lifeCycle;
    }
    ...
    function claim(address account, uint256 _id, bool _isClaim) external onlyIfExists(_id) onlyContract override returns (uint) {
        ...
        if (interval > _node.timeLeft && _isClaim) {
            interval = _node.timeLeft;
            _node.timeLeft = 0;
        } else if (interval <= _node.timeLeft && _isClaim) {
            _node.timeLeft = _node.timeLeft - uint64(interval);
        }
    ...
    function reboost(uint256 _id) external onlyContract {
        Mansion storage mansion = _nodes[_id];
        mansion.claim = uint64(block.timestamp);
        mansion.timeLeft = nodeLifeCycle;
    }
    ...
    function getNodePrice(uint256 _id) external view returns(uint256) {
        uint256 tier = _nodes[_id].tier;
        if (tier == 0) { return vbuy1; }
        if (tier == 1) { return vbuy2; }
        if (tier == 2) { return vbuy3; }
        if (tier == 3) { return vbuy4; }
        if (tier == 4) { return vbuy5; }
        if (tier == 5) { return vbuy6; }
        if (tier == 6) { return vbuy7; }
        return vbuy1;
    }
