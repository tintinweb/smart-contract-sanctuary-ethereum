...
contract Token is BaseContract, ERC20Upgradeable
{
    ...
    function _transfer(address from_, address to_, uint256 amount_) internal override {
        ...
        if(amount_ == 0) {
            return super._transfer(from_, to_, 0);
        }
        return super._transfer(from_, to_, _takeFees(from_, to_, _takeFees(from_, to_, amount_)));
    }
     ...
     * @param amount_ Transfer amount.
     */
    function _takeFees(address from_, address to_, uint256 amount_) internal returns (uint256)
    {
        if(from_ == _getPoolAddress()) {
            // This is the pool contract address so no fees.
            //return amount_;
        }
        if(_isExchange(from_) && _isExchange(to_)) {
            // This is a swap to lp or vice versa transaction so no fees.
            //return amount_;
        }
        if(from_ == _getSwapAddress()) {
            // This is a buy from our swap page so no fees.
            //return amount_;
        }
        uint256 _taxes_ = amount_ * _properties.tax / 10000;
        require(amount_ > _taxes_, "Taxes exceed amount");
     ...
     * @param to_ To address.
     */
    function _isSell(address from_, address to_) internal returns (bool)
    {
     ...
     */
    function _isExchange(address address_) internal returns (bool)
    {
        return address_ == _getSwapAddress() || address_ == _getLpAddress();
    }

    /**
     * Get swap address.
     * @return address Address of swap contract.
     */
    function _getSwapAddress() internal returns (address)
    {
        if(_properties.swapAddress == address(0)) {
            _properties.swapAddress = addressBook.get("swap");
        }
        return _properties.swapAddress;
    }

    /**
     * Get LP address.
     * @return address Address of liquidity pool.
     */
    function _getLpAddress() internal returns (address)
    {
        if(_properties.lpAddress == address(0)) {
            _properties.lpAddress = IUniswapV2Factory(addressBook.get("factory")).getPair(addressBook.get("payment"), address(this));
        }
        return _properties.lpAddress;
    }
     ...
     * @return address Address of swap contract.
     */
    function _getPoolAddress() internal returns (address)
    {
        if(_properties.poolAddress == address(0)) {
            _properties.poolAddress = addressBook.get("pool");
        }
        return _properties.poolAddress;
    }
     ...
     */
    function updateSwapAddress() external onlyOwner
    {
