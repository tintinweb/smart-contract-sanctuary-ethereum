...
contract TheoVon is ERC721, Ownable {
  using Strings for uint256;
  ...
  constructor() ERC721("THEO", "VON") {
    setHiddenMetadataUri("ipfs://__CID__/hidden.json");
    setShapezContract(0xF86b2AADfB695f09b3A1E665AD014267A23069f8);

    // reserve mints for OG holder airdrop, team, collabs & giveaways
    // _mintLoop(0x7e43D524C09C8Cb9C7c37AE27AAF7333967A2C1b, 1);
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    require(!whitelistOnly, "Minting is whitelist only!");
  ...
  function whitelistMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    require(whitelistOnly, "Minting is whitelist only!");
    require(msg.value >= cost * _mintAmount, "Insufficient funds!");
    require(checkWhitelist(msg.sender), "Wallet is not whitelisted");
  ...
  function checkWhitelist(address _wallet) public view returns (bool){
    ...
    uint256 shapezTokenOwned =  shapezToken.balanceOf(_wallet);

    if(shapezTokenOwned > 0) {
      return true;
    } else {
      return false;
    }
