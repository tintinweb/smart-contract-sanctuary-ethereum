...
contract Reveal is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    bytes32 public root;
    uint256 public constant totalTokenCount = 5;
    ...
    // A new drop can also be  crreted if previous tokens are not yet sold out .. 
    // Do we need any such functionlity ? 
    //@TODO do we need a check to restrict it until all previous token sold out.
    // uint256 currSupply = totalSupply();
    // require(supply == currSupply, "Supply is available for remaining tokens." )
    function createDrop(uint256 _redeemTime, uint256 _price , uint256 _tokenAmount) external onlyOwner {
        require(_redeemTime >= block.timestamp , "Invalid Redeem Time");
        require(_price > 0, "Invalid price");
        require(_tokenAmount > 0, "Invalid Supply");
        require(supply + _tokenAmount <= totalTokenCount, "Total Supply Reached");
        drop += 1;
    ...
    function updateRedeemTime(uint256 _drop, uint256 _redeemTime) external onlyOwner {
        // Check redeem time greater then current time ..
        require(_redeemTime > block.timestamp , "Invalid Redeem Time ");
