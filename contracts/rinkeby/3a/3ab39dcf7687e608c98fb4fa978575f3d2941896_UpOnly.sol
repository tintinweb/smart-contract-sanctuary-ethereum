...
contract UpOnly is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 constant public CREATOR_FEE = 5;
    uint256 constant public SERVICE_FEE = 2;
    ...
    function mint(string memory imageUrl, string memory animationUrl) external nonReentrant {
        uint256 tokenId = supply + 1;
        
        _safeMint(msg.sender, tokenId);
        
        tokens[tokenId] = UpOnlyToken(msg.sender, 0, 0, imageUrl, animationUrl);
    ...
    function list(uint256 tokenId, uint256 listPrice) external nonReentrant {
        require(listPrice > 0);
        require(listPrice > tokens[tokenId].lastSalePrice, "ser, this is up only.");
    ...
    function buy(uint256 tokenId) external nonReentrant payable {
        ...
        require(listPrice > lastSalePrice, "ser, this is up only");
        ...
        (bool success, ) = creator.call{value: msg.value * CREATOR_FEE / 100 }("");
        require(success, "Creator fee payment failed.");

        (success, ) = seller.call{value: msg.value * (100 - SERVICE_FEE - CREATOR_FEE) / 100}("");
    ...
    function transferFrom(address, address, uint256) public virtual override {
        require(false, "Public transfer functions disabled.");
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked(
            '{',
            '"name": "#', tokenId, '",',
            '"description": "UpOnly NFTs can only be sold for more than they were bought. Use contract to buy or a community marketplace.",',
            '"image_data": "', tokens[tokenId].imageUrl, '",',
            '"animation_url": "', tokens[tokenId].animationUrl, '",',
            '"attributes": [',
            '  {"trait_type": "Creator", "value": "', toString(abi.encodePacked(tokens[tokenId].creator), 8), '"} ',
            ']}'
        ))));
    ...
    function toString(bytes memory data, uint256 length) public pure returns(string memory) {
