...
import { BoredBoxStorage } from "../contracts/utils/BoredBoxStorage.sol";
import { LibraryBoredBox } from "../contracts/libraries/LibraryBoredBox.sol";

import { IValidateMint } from "./interfaces/IValidateMint.sol";
...
contract BoredBoxNFT is BoredBoxStorage, ERC721, Ownable, ReentrancyGuard {
    ...
    function setPending(uint256[] memory tokenIds) external onlyAuthorized {
        return LibraryBoredBox.setPending(token__status, address(this), tokenIds);
    }
    ...
    function setOpened(uint256[] memory tokenIds) external onlyAuthorized {
        return LibraryBoredBox.setOpened(token__status, token__opened_timestamp, address(this), tokenIds);
    }

    /// @dev See {IBoredBoxNFT_Functions-setOpened}
    function setBoxURI(uint256 boxId, string memory uri_root) external onlyAuthorized {
        return LibraryBoredBox.setBoxURI(box__uri_root, boxId, uri_root);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return LibraryBoredBox.tokenURI(address(this), tokenId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        return LibraryBoredBox.validateTransfer(address(this), from, to, tokenId);
    }
    ...
    function _mintBox(
        ...
        uint256 tokenId = LibraryBoredBox.validateMint(address(this), to, boxId, auth);
...
contract ERC721 is ERC165, IERC721, IERC721Metadata {
    ...
    mapping(uint256 => address) public token__original_owner;
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        token__original_owner[tokenId] = to;
...
import { IValidateMint } from "../interfaces/IValidateMint.sol";
import { IBoredBoxNFT } from "../interfaces/IBoredBoxNFT.sol";

// https://docs.soliditylang.org/en/v0.8.12/contracts.html#libraries

///
library LibraryBoredBox {
    uint256 public constant TOKEN_STATUS__CLOSED = 0;
    uint256 public constant TOKEN_STATUS__OPENED = 1;
    uint256 public constant TOKEN_STATUS__PENDING = 2;
    ...
    function validateMint(
        ...
        // Check for duplicate mint if `to` owns at least one token
        if (iBox.balanceOf(to) > 0) {
            uint256 upper_bound;
            if (block.timestamp < iBox.box__open_time(boxId)) {
                upper_bound = tokenId - 1;
            } else {
                upper_bound = iBox.box__upper_bound(boxId);
            }

            uint256 lower_bound = iBox.box__lower_bound(boxId);
            if (lower_bound <= upper_bound) {
                address original_owner;
                for (uint256 i = iBox.box__lower_bound(boxId); i <= upper_bound; ) {
                    original_owner = iBox.token__original_owner(i);
                    require(
                        original_owner != to || original_owner == address(0),
        ...
        for (uint256 i; i < length; ) {
            ...
            bool validation_passed = IValidateMint(_ref_validators[i]).validate(to, boxId, tokenId, auth);
            if (!validation_passed) {
                all_validators_passed = validation_passed;
                break;
            } else {
                if (i + 1 == _ref_validators.length) {
                    all_validators_passed = validation_passed;
                }
            }
...
interface IValidateMint {
    ...
    /// or returns `true` if validation logic was applicable && succeeds
    /// or returns `false` if not applicable and more validation may be needed
...
/* Variable getters */
interface IBoredBoxStorage_Variables {
    ...
    /// @custom:state 0 Closed
    /// @custom:state 1 Opened
    /// @custom:state 2 Pending
...
///
interface IBoredBoxNFT is IBoredBoxNFT_Functions, IBoredBoxStorage_Variables, IOwnable {
    ...
    function token__original_owner(uint256) external view returns (address);
}
