	...
	UUPSUpgradeable
{
	...
	function _setOracles(address[] memory nodes) internal {
		for (uint256 i = 0; i < feederList.length; i++) {
			revokeRole(UPDATER_ROLE, feederList[i]);
		}
		...
		returns (bool)
	{
		...
		if (_price > price) {
			...
			percentDeviation = ((price - _price) * DECIMAL_PRECISION) / _price;
		}
	...
	function initializeBuffer(PricePriceInformationBuffer storage priceBuffer)
		internal
	{
		for (uint256 i = 0; i < config.maxSubmissions; i++) {
			priceBuffer.ring.push(
				PriceInformation({ twap: 0, lastUpdateTime: 0 })
			);
		}
	}
	...
	function addRawValue(address token, uint128 twap) internal {
		...
		if (priceBuffer.ring.length == 0) {
			initializeBuffer(priceBuffer);
		}
	...
	function getFeederList() external view virtual returns (address[] memory) {
		return feederList;
	}
	...
	{
		FeederRegistrar storage feederRegistrar = priceFeederMap[token];
		PriceInformation[] memory pricesList = feederRegistrar
			.feederBuffer[feeder]
			.ring;
		return pricesList;
	}

	function getConfig() external view virtual returns (OracleConfig memory) {
		return config;
	}
}
...
//maxSubmissions for each feeder
...
//expirationPeriod at least interval of client to feed
//currently 21600s=6h,double here so we have at least 2 prices for each feeder
uint64 constant Default_ExpirationPeriod = 21600 * 2;
//reject when price increase/decrease more than 50 times
uint128 constant Default_MaxPriceDeviation = 50 * DECIMAL_PRECISION;
//decimal precision for the derivated price
uint128 constant DECIMAL_PRECISION = 10**18;
