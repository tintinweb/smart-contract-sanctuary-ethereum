...
contract AudioburstAuction is Ownable, ERC721Holder {
    ...
    function finalizeAuction(uint256 _auction_id) public {
        ...
        if (bidsLength == 0) {
            ...
            require(governanceToken.transfer(myAuction.owner, sellerAmount), "transfer to seller failed");
            if(feeAmount > 0) require(governanceToken.transfer(swapFeeAddress, feeAmount));
            if(creatorAmount > 0) require(governanceToken.transfer(myAuction.creator, creatorAmount));
     ...
     * @param _auction_id uint256 ID of the created auction
     */
    function bidOnAuction(uint256 _auction_id, uint256 amount) external AuctionExists(_auction_id) {
        ...
        require(amount >= tempAmount, "bid amount is not valid");
