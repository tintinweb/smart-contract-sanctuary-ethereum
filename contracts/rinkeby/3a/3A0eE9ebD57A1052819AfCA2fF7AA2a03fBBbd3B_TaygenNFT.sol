...
// Part: OpenZeppelin/[email protected]/IERC721MetadataUpgradeable
...
// Part: OpenZeppelin/[email protected]/ERC721Upgradeable
...
// Part: OpenZeppelin/[email protected]/ERC721PausableUpgradeable
    ...
    ERC721PausableUpgradeable
{
    ...
    modifier isStakingPool() {
        require(msg.sender == stakingPoolAddress, "Caller is not stakingPoolAddress contract");
        _;
    }

    function initialize(string memory name_, string memory symbol_, address _stakingPoolAddress)
        external
        initializer
    {
        __Context_init_unchained();
     ...
     */
    function mint(address _to) external override isStakingPool returns (uint256) {
     ...
     */
    function burn(uint256 _tokenId) external override isStakingPool {
        _burn(_tokenId);
    }
    ...
    /**
     * @dev Set stakingPoolAddress contract address
     * @param _stakingPoolAddress - address of the stakingPoolAddress contract
     */
    function setStakingPoolAddress(address _stakingPoolAddress) external override onlyOwner {
        stakingPoolAddress = _stakingPoolAddress;
    ...
    */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        (uint _USDCBalance, uint  _tUSDCBalance, uint  _conversionRate,uint  _usdcFee, uint  _withdrawTime) = stakingPool.getNFTVariables(tokenId);
        return
        _tokenURI(
            tokenId,
            _USDCBalance,
    ...
    function _tokenURI(uint _tokenId, uint _USDCBalance, uint _tUSDCAmount, uint _conversionRate, uint _usdcFee, uint _withdrawTime) internal pure returns (string memory output) {
        ...
        output = string(abi.encodePacked(output, "withdrawable_usdc ", toString(_USDCBalance), '</text><text x="10" y="60" class="base">'));
        output = string(abi.encodePacked(output, "tUSDC_amount ", toString(_tUSDCAmount), '</text><text x="10" y="80" class="base">'));
        output = string(abi.encodePacked(output, "conversion rate ", toString(_conversionRate), '</text><text x="10" y="100" class="base">'));
        output = string(abi.encodePacked(output, "USDC Withdrawal Fee ", toString(_usdcFee), '</text><text x="10" y="120" class="base">'));
        output = string(abi.encodePacked(output, "withdraw time ", toString(_withdrawTime), '</text></svg>'));

        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "NFT #', toString(_tokenId), '", "description": "Taygen NFT", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
