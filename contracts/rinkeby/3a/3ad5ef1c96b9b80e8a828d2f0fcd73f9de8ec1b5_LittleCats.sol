...
contract Whitelist {
    ...
    modifier onlyOwnerW() {
      require(msg.sender == theowner, "Ownable: caller is not the owner");
      _;
    }
    ...
    function addUser(address _addressToWhitelist) public onlyOwnerW {
...
contract LittleCats is ERC721, NonblockingReceiver, ILayerZeroUserApplicationConfig, Whitelist {
    ...
    uint256 public constant FREE_MINT_FOR_PUBLIC = 1;
    uint256 public constant FREE_MINT_FOR_WL = 2;

    string public thecontractURI = "https://wenmint.mypinata.cloud/ipfs/QmTM4Dd7XA9GtFZVYLAMFkBV1kYzQ38Nc28fFEUcGLRffv";
    ...
    function mint(uint8 numTokens) external payable {
        ...
        uint toPay = numTokens;
        uint mintedSoFar = minted[msg.sender];
        if (whitelistedAddresses[msg.sender]) {
            require(minted[msg.sender] + numTokens <= maxMintPerWallet, "limit per wallet reached");
            if(mintedSoFar < FREE_MINT_FOR_WL) {
                uint remainingFreeMint = FREE_MINT_FOR_WL - mintedSoFar;
                if(numTokens > remainingFreeMint) {
                    toPay = numTokens - remainingFreeMint;
                }
                else {
                    toPay = 0;
                }
            ...
            if(mintedSoFar < FREE_MINT_FOR_PUBLIC) {
                uint remainingFreeMint = FREE_MINT_FOR_PUBLIC - mintedSoFar;
                if(numTokens > remainingFreeMint) {
                    toPay = numTokens - remainingFreeMint;
                }
                else {
        ...
        require(nextTokenId + numTokens <= maxMint, "Mint exceeds supply");
        require(msg.value >= toPay * MINT_PRICE, "You need more money");
 ...
 function traverseChains(
        ...
        require(msg.sender == ownerOf(_tokenId), "Message sender must own the Little Cat.");
        ...
        require(
            ...
            "Little Cat: msg.value not enough to cover messageFee. Send gas for message fees"
        );
