...
contract Alienverse is ERC721A, Ownable, PaymentSplitter {
    ...
    ) ERC721A("Alienverse", "AVS") PaymentSplitter(_team, _teamShares) {
    ...
    function giveaway(address to, uint256 qty) external onlyOwner {
        require(qty + minted <= maxSupply, "Value exceeds total supply");
        _safeMint(to, qty);
        minted += qty;
    }

    // First contacts get 2 mints during presale
    function mintWhitelistFc(uint256 qty, bytes32[] calldata _merkleProof)
        external
        payable
    {
    ...
    // Whitelisters get 3 mints during presale
        ...
        payable
    {
        ...
        require(qty + minted <= maxSupply, "Value exceeds total supply");
        require(
            _numberMinted(msg.sender) + qty <= 3,
            "Can't mint anymore during presale"
        );
        require(msg.value == price * qty, "Invalid value");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    ...
    // VIPs get 5 mints during presale
    function mintWhitelistVIP(uint256 qty, bytes32[] calldata _merkleProof)
        external
        payable
    {
        require(presaleActive, "Pre-sale isn't active");
        ...
        require(qty + minted <= maxSupply, "Value exceeds total supply");
        require(
            _numberMinted(msg.sender) + qty <= 5,
            "Can't mint anymore during presale"
        );
        require(msg.value == price * qty, "Invalid value");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
