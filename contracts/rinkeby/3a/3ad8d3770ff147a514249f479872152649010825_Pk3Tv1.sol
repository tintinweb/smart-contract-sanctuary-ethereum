...
/**
 * @title BEP721 Non-Fungible Token Standard basic implementation
 * @dev see https://eips.ethereum.org/EIPS/eip-721
 */
...
contract Pk3Tv1 is BEP721, Ownable {
    ...
    uint256 public constant tokenPrice = 1000000000000000; //0.001 BNB

    uint public constant maxTokenPurchase = 5;
    uint256 constant public maxTokenPurchasePresale = 5 ;
    uint256 constant public max_bombs=1125;
    ...
    */
    function mintToken(uint numberOfTokens) public payable {
        ...
        require(totalSupply().add(numberOfTokens) <= max_bombs, "Purchase would exceed max supply.");
       ...
       if(privateSaleIsActive) {
        ...
        if (startingIndexBlock == 0 && (totalSupply() == max_bombs || block.timestamp >= reveal_timestamp)) {
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint(blockhash(startingIndexBlock)) % max_bombs;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint(blockhash(block.number - 1)) % max_bombs;
        }
