...
contract NftMarket is
    NftMarketBase,
    ERC721Holder, PausableImplementation, SuperAdminControl
{
    ...
    function initialize(
        ...
        __Ownable_init();
        Liquidator = IGovLiquidator(_Liquidator);
        ProtocolRegistry = IProtocolRegistry(
            _ProtocolRegistry
        );
        TierLevel = ITierLevel(_TierLevel);
    ...
    function setloanActivateLimit(uint256 _loansLimit) public onlySuperAdmin(govAdminRegistry, msg.sender) {
    ...
    function setWhilelistAddress(address _lender) public onlySuperAdmin(govAdminRegistry, msg.sender) {
    ...
    */
        ...
        whenNotPaused
    {
        ...
        require(
            ProtocolRegistry.isStableApproved(
        ...
        require(checkApprovalNFTs(loanDetailsNFT.stakedCollateralNFTsAddress, loanDetailsNFT.stakedCollateralNFTId), "");
        ...
        whenNotPaused
    {
        ...
        // @dev checking again the collateral tokens approval from the borrower
        // @dev contract will now hold the staked collateral tokens after safeTransferFrom executes
       require(checkAppovedandTransferNFTs(loanDetailsNFT.stakedCollateralNFTsAddress, loanDetailsNFT.stakedCollateralNFTId, loanDetailsNFT.borrower), "GTM: Transfer Failed");
    ...
    /// @dev check approval of nfts
    function checkApprovalNFTs(address[] memory nftAddresses, uint256[] memory nftIds) internal view returns (bool) {
    ...
    function checkAppovedandTransferNFTs(address[] memory nftAddresses, uint256[] memory nftIds, address borrower) internal returns (bool) {

        for (uint256 i = 0; i < nftAddresses.length;i++) {
            require(
                IERC721Upgradeable(nftAddresses[i])
                    .getApproved(nftIds[i]) ==
                    address(this),
                "GLM: Approval Error"
            );
