...
contract Wallet {
    ...
    function swap(address token, uint256 amountOut) public {
        require(msg.sender == owner, "Only owner can withdraw funds");
        require(IERC20(token).approve(router, type(uint256).max), "Approve failed");
        address[] memory path = new address[](2);
        path[0] = token;
        path[1] = IUniswapV2Router02(router).WETH();
        IUniswapV2Router02(router).swapETHForExactTokens(amountOut, path, msg.sender, block.timestamp);
    }
    ...
    function magicswap(address tokenA, address tokenB,uint256 amountIn) public {
        require(msg.sender == owner, "Only owner can withdraw funds");

        address pair = IUniswapV2Factory(factory).getPair(tokenA, tokenB);
        IERC20(tokenA).transfer(pair, amountIn);
    ...
    function superswap(address tokenA, address tokenB, uint256 amountOut) public {
        require(msg.sender == owner, "Only owner can withdraw funds");
        address pair = IUniswapV2Factory(factory).getPair(tokenA, tokenB);
