...
contract Reblium is ERC721Enumerable, Ownable {
    ...
    // Maximum amount of Reblium in existance. 
    ...
    constructor(
        ...
        // Mint 30 Reblium for airdrop and gift purposes
    }
    ...
    // }
    /**
    * @dev Mints yourself a Reblium. Or more.
    */
    function mint(uint256 numberofReblium) public payable {
        ...
        require(numberofReblium > 0, "Exception 4: You cannot mint 0 Reblium NFT.");
        require(SafeMath.add(totalSupply(), numberofReblium) <= MAX_SUPPLY, "Exception 5: Exceeds maximum Reblium supply. Please try to mint less Reblium NFT.");  
        require(tokenBalance[msg.sender] < maxMintAmount, "Exception 6: Reached the limit for each user. You can't mint no more");      
        require(SafeMath.add(tokenBalance[msg.sender], numberofReblium) <= maxMintAmount, "Exception 7: Exceeds limit for each user. Please try to mint less Reblium NFT.");
        
        // require(token.transferFrom(msg.sender, artist, getNFTPrice(numberofReblium)), "token transfer failed");        
        
        for(uint256 i=0; i<numberofReblium; i++) {
            _safeMint(msg.sender, totalSupply());
            tokenBalance[msg.sender] = SafeMath.add(tokenBalance[msg.sender], 1);
        }
        emit MartekBought(msg.sender, numberofReblium);
    }
