...
contract Machines is Ownable, ERC721DEYE, ReentrancyGuard {
    ...
    function setAllowlist(address[] memory addresses, bool[] memory presaleOneAlloed, bool[] memory presaleTwoAllowed) external onlyOwner {
        require(addresses.length == presaleOneAlloed.length && addresses.length == presaleTwoAllowed.length, "addresses does not match allowance length");
        for (uint256 i = 0; i < addresses.length; i++) {
            _setMintAllowance(addresses[i], presaleOneAlloed[i], presaleTwoAllowed[i]);
        }
    }
