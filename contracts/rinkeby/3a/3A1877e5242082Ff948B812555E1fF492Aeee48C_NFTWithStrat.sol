...
contract NFTWithStrat is ERC721A, Ownable, VRFConsumerBase {
    ...
    mapping(address => uint256) public ERC20Amount;
    ...
    modifier pay(address[] calldata _addr, uint256 _amount) {
        ...
        if (len == 0) {
            ...
            if (msg.value > price * _amount) {
                (bool success, ) = msg.sender.call{
                    value: msg.value.sub(price * _amount)
                }("");
                require(success, "Refund failed");
            }
        } else {
            require(len == ERC20Address.length, "Bad len");
            for (uint256 idx = 0; idx < len; idx++) {
                require(_addr[idx] == ERC20Address[idx], "Bad address");
                require(ERC20Amount[ERC20Address[idx]] > 0, "Bad amount");
                uint256 amt = _amount.mul(ERC20Amount[ERC20Address[idx]]);
    ...
    function publicMint(
        ...
        uint256 _amount,
        address _strat
    ) public payable callerIsUser pay(_addr, _amount) {
        require(saleState > 0, "Public sale not live");
        require(_amount > 0, "Bad amount");
        require(_amount <= 20, "You can mint a maximum of 20 fantastic token");
        require(
            totalSupply() + _amount <= MAX_SUPPLY,
            "Exceeds maximum token supply"
        );
        require(okStrats[_strat], "Unapproved strategy");
        // if (useETH) {
        //     address[] memory tokenIn = new address[](1);
        //     tokenIn[0] = wNative;
        //     IStrategy(strat).execute(tokenIn, tokenOut, price * amount);
        // }
        // else {
        //     for(){

        //     }
        //     IStrategy(strat).execute(ERC20Address, tokenOut, price * amount);
        // }
    ...
    function setERC20(address[] calldata _addr, uint256[] calldata _amount)
        external
        onlyOwner
    {
        require(_addr.length == _amount.length, "length not equal");
        ERC20Address = _addr;
        for (uint256 idx = 0; idx < _addr.length; idx++) {
            ERC20Amount[_addr[idx]] = _amount[idx];
        }
    }

    function getERC20Addr() external view returns (address[] memory) {
        return ERC20Address;
    }

    function getERC20AddrLength() public view returns (uint256) {
        return ERC20Address.length;
    }

    function setERC20Amount(address _addr, uint256 _amount) external onlyOwner {
        ERC20Amount[_addr] = _amount;
    }

    function getERC20Amount(address _addr) public view returns (uint256) {
        return ERC20Amount[_addr];
    }
...
interface IStrategy {
  ...
  /// @param tokenOut The token address to receive after swap.
  /// @param amountETH depend on strategy.
  function execute(address[] calldata tokenIn, address tokenOut, uint256 amountETH) external;
}
