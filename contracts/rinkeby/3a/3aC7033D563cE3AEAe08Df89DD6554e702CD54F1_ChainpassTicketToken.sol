...
contract ChainpassTicketToken is ERC721, Ownable {
    ...
    function allowlistMint(uint256 amountAllowed, bytes32[] calldata merkleProof, uint256 amount) external payable {
        require(whitelistMerkleRoot != 0, "Merkle root not set yet.");
        require(msg.value >= amount * allowlistPrice, "Invalid price.");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, amountAllowed));
        bool valid = MerkleProof.verify(merkleProof, whitelistMerkleRoot, leaf);
