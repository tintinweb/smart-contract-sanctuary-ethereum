...
contract RFOXFactory is Ownable {
    ...
    function createNFT(
        ...
        emit NewRFOXNFT(name, symbol, newNFT, saleToken, price, maxNft, maxTokensPerTransaction, saleStartTime, saleEndTime, owner);
    ...
    Ownable
{
    ...
    // Timestamp of selling started
    uint256 public saleStartTime;

    // Timestamp of selling ended
    ...
    event MaxTokensPerTransaction(address indexed sender, uint256 oldValue, uint256 newValue);
    ...
    function setMaxTokensPerTransaction(uint256 newMaxTokensPerTransaction) external onlyOwner {
        ...
        emit MaxTokensPerTransaction(msg.sender, _oldMaxTokensPerTransaction, maxTokensPerTransaction);
    }
    ...
    function buyNFTs(uint256 tokensNumber) external payable whenNotPaused {
        require(
        ...
        if(saleEndTime > 0) require(block.timestamp <= saleEndTime, "Sale has been finished");
