...
contract Moo is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("Moo", "MOO") {
        _price = 0.015 ether;

        _maxMintPerAllowlist = 10;
        _maxMintPerTxn = 10;

        _teamAddress = 0xfF3C7d4E5304a9bE9072C95a78c5D169A957Ebd8;
        _baseTokenURI = "https://todo.todo.com/";
    }

    function mintTokens(bytes32[] calldata merkleProof, address to, uint256 quantity) external payable mintGatekeeper(quantity) {
        if (mintStatus == MintStatus.CLOSED) {
            revert("Minting is closed");
        } else if (mintStatus == MintStatus.ALLOWLIST) {
            mintGatekeeperAllowlist(merkleProof, to, quantity);
        } else {
            mintGatekeeperPublic(quantity);
        }
        _mint(to, quantity);
    }
    ...
    function mintGatekeeperAllowlist(bytes32[] calldata merkleProof, address to, uint256 quantity) private view {
    ...
    function mintGatekeeperPublic(uint256 quantity) private view {
