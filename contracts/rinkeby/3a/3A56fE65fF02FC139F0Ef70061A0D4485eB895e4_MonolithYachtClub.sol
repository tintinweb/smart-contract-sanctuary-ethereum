...
contract MonolithYachtClub is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function publicMint(uint256 nTokens) public payable{
         if (mintOpen == false){
            revert('Public Mint not open yet');
        }
    ...
    function privateMint(uint256 nTokens, address wallet) external onlyOwner{
        uint256 localCounter = counter;
        if (localCounter + nTokens > 60000) {
            revert('Supply Not Available, Try Less');
        }
    ...
    function mintInfo() public pure returns (string memory){
        return("0.16 ether per NFT, Maximum 100 per Mint request. Mint ends at 60k supply.");
    }
    ...
    function freezeMetadata() external onlyOwner {
      ...
      function openMint() external onlyOwner {
        if (counter >= 10000) {
            ...
            revert('Supply has not reached 10000');
        }
    }
    ...
    function _createTokenURI(uint256 id) internal view returns (string memory) {
        if (frozen == true){
            revert('Token URI Frozen');
        }
