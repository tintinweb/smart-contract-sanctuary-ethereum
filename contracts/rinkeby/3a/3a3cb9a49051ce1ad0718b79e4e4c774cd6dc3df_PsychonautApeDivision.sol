...
contract PsychonautApeDivision is Ownable, IERC2981, ERC721 {
    ...
    modifier mintlistFunc(uint amount) {
        ...
        require(amount + amountMinted[msg.sender] < 21 && amount != 0, "Request exceeds max per wallet!");
    ...
    function mintlistMint(bytes calldata sig, uint256 amount) external payable mintlistFunc(amount) mintFunc(amount, 1e17)  {
    ...
    function vipMintlistMint(uint256 amount, bytes calldata sig) external payable mintlistFunc(amount) mintFunc(amount, 89e15) {
