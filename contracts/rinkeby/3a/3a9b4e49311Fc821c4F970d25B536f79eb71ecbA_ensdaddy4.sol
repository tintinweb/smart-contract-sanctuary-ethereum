...
constructor(){}
    ...
    function update_subdomain(string memory subdomain, string memory domain) public {
        for (uint i=0; i< ensdomains_count; i++) {
            if(keccak256(bytes(ensdomains[i].domain)) == keccak256(bytes(string(abi.encodePacked(domain)))) ){
                if(ensdomains[i].ownner == msg.sender ){
                    ensusers[string(abi.encodePacked(subdomain,'.',domain))].status =  "Registered";
                }
            }
        }
    }
