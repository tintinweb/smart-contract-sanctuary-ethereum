...
contract OrderContract is  Ownable, EIP712, ERC721Holder{
...
struct offerStatus{
        bool isvalid;}
        mapping(bytes=>offerStatus)offerMapping;   
// **********   offer section   **********
// **********   offer section   **********
// **********   offer section   **********
// **********   offer section   ********** 
    ...
    mapping(bytes=>OrderStatus) public map;
...
function recover(Order calldata order) public view returns (address) {
        ...
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
         ...
         // keccak256("Order(uint tokenId,uint ethPrice,uint tokenPrice,address tokenAddress,address coinAddress,address seller)"),
            
            keccak256("Order(uint256 tokenId,uint256 ethPrice,uint256 tokenPrice,address tokenAddress,address coinAddress,address seller)"),
            order.tokenId,
            order.ethPrice,
            order.tokenPrice,
            order.tokenAddress,
    ...
    function buyOrder1(Order calldata order) public payable
    {
        ...
        require (map[order.signature].isvalid == true,"order not active");
 ...
 // getting values for royalties
    ...
    if (order.coinAddress == 0x0000000000000000000000000000000000000000 ){
       ...
       map[order.signature].isvalid=false;
    ...
    function cancelOrder(Order calldata order) public returns(bool){
        ...
        map[order.signature].isvalid=true; 
        return map[order.signature].isvalid;}
...
// }  
    ...
    function recoverOffer(Offer calldata offer) public view returns (address) {
            ...
            bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
            keccak256("Offer(uint256 tokenId,uint256 ethPrice,address tokenAddress,uint256 time)"),
            offer.tokenId,
            offer.ethprice,
            offer.tokenAddress,
            offer.time
    ...
    function acceptOffer(Offer calldata offer)public{
        ...
        require(offer.ethprice  <=  tokenRegistry.balanceOf(recoverOffer(offer)),"not enough ether sent");


        // sending money 
        tokenRegistry.transferFrom(recoverOffer(offer),msg.sender,offer.ethprice);
        // sending token
