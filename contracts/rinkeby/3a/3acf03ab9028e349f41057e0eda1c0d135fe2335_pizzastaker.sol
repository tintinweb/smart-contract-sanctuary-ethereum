...
contract pizzastaker is ERC1155, Ownable {
    ...
    IERC721 public PizzaPals;

    mapping(uint => string) public tokenURI;

    //Mapping of PizzaPals to timestamp
    mapping(uint256 => uint256) internal tokenIdToTimeStamp;

    //Mapping of PizzaPals to staker
    mapping(uint256 => address) internal tokenIdToStaker;

    //Mapping of staker to PizzaPals
    ...
    function setPizzaAddress(address _Address) public onlyOwner {
        pizzaAddress = _Address;
        return;
    }
    ...
    function safeTransferFrom(
        ...
        require(tx.origin == address(this), "Only Callable by this Contract");
        _safeTransferFrom(from, to, id, amount, data);
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(pizzaAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(pizzaAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
        ...
        safeTransferFrom(address(this),msg.sender,1,count,"");
    }
    ...
    function unstakeAll() public {
        ...
        uint256 count = stakerToTokenIds[msg.sender].length;
        ...
        safeTransferFrom(msg.sender,address(this),1,count,"");
    }
