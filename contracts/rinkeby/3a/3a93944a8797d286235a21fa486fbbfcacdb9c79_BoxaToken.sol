...
* Main Contract (Contract): Boxa – Boxa Coin
* Name: Boxa Coin
* Symbol: BOXA
...
* 6% marketing fee buy/sell
* 3% development fee buy/sell
* 0% reflection
* 3% liquidity buy/sell
*/
...
contract BoxaToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0x1BdF741638ddc3770aF68652bff2B68c74Dd9DeD); // Marketing fee Address
    address payable public devAddress = payable(0x6bD2aB6D83D78Cc37d1B8B6a45A63eAa3F0d6886); // development fee Address
    address payable public rewardAddress = payable(0x1BdF741638ddc3770aF68652bff2B68c74Dd9DeD); // Reward Address
    address payable public liquidityAddress = payable(0x1BdF741638ddc3770aF68652bff2B68c74Dd9DeD); // Liquidity Address, owner address
    ...
    string private constant _name = "Boxa Coin";
    string private constant _symbol = "BOXA";
    ...
    uint256 public _buyTaxFee = 0;          // reflection fee 0%
    uint256 public _buyLiquidityFee = 3;    // liqudity fee in buying case 3%
    uint256 public _buyRewardFee = 0;       // burn fee in buying case 2%
    uint256 public _buyMarketingFee = 6;    // marketing fee in buying case 6%
    uint256 public _buyDevFee = 3;          // development fee in buying case 2%

    uint256 public _sellTaxFee = 0;         // reflection fee 0%
    uint256 public _sellLiquidityFee = 3;   // liqudity fee in buying case 3%
    uint256 public _sellRewardFee = 0;      // burn fee in buying case 2%
    uint256 public _sellMarketingFee = 6;   // marketing fee in buying case 6%
    uint256 public _sellDevFee = 3;         // development fee in buying case 2%    
    ...
    function _transfer(
        ...
        if(limitsInEffect){
            if (
                ...
                require(tradingActive, "Trading is not enabled yet");
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyRewardFee + _buyMarketingFee + _buyDevFee;
                if(_liquidityFee > 0){
                    buyOrSellSwitch = BUY;
                }
            }
            // Sell
                ...
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellRewardFee + _sellMarketingFee + _sellDevFee;
                if(_liquidityFee > 0){
    ...
    function swapBack() private lockTheSwap {
        ...
        bool success;
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _rewardTokens + _marketingTokensToSwap + _devTokensToSwap;
        ...
        (success,) = address(marketingAddress).call{value: address(this).balance.sub(ethForDev)}("");
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyRewardFee, uint256 buyMarketingFee, uint256 buyDevFee)
        external
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyRewardFee + _buyMarketingFee + _buyDevFee <= 15, "Must keep taxes below 15%");
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellRewardFee, uint256 sellMarketingFee, uint256 sellDevFee)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellRewardFee + _sellMarketingFee + _sellDevFee <= 25, "Must keep taxes below 25%");
    }
