...
contract POAPSheet is Ownable, ERC721Bound {
    mapping(uint256 => mapping(address => bool)) private _claimStatusById;
    ...
    function claimPOAP(uint256 id, bytes32[] calldata proof) external {
        require(_minted(msg.sender), "needs sheet");
        require(!_claimStatusById[id][msg.sender], "already claimed");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(proof, merkleRootsById[id], leaf), "not in whitelist");
        _claimStatusById[id][msg.sender] = true;
        _poaps[msg.sender][poapsBalanceOf[msg.sender]] = id;
        poapsBalanceOf[msg.sender]++;
    }

    function getPOAPs(address wallet) public view returns (uint256[] memory) {
    ...
    function rearrangePOAPs(uint256[] memory oldPositions) external {
        uint256 poapsCount = poapsBalanceOf[msg.sender];
        require(oldPositions.length == poapsCount, "invalid indexes");
        uint256[] memory poaps = new uint256[](poapsCount);

        for (uint256 index = 0; index < oldPositions.length; index++) {
            uint256 oldPosition = oldPositions[index];
            require(oldPosition < poapsCount, "invalid indexes");
            poaps[index] = _poaps[msg.sender][oldPosition];
        }
        for (uint256 index = 0; index < poaps.length; index++) {
            _poaps[msg.sender][index] = poaps[index];
        }
    }
