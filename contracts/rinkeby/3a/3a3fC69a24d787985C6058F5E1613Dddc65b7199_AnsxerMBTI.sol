...
contract AnsxerMBTI is ERC721AQueryable, Ownable, Pausable {
  ...
  mapping(uint256 => uint256) private _mbtiTotalMints;
  mapping(uint256 => uint256) private _mbtiOfToken;
  ...
  function teamMint() external onlyOwner {
    ...
    for (uint i; i < 16; i++) {
      for (uint n; n < TEAM_EACH_MBTI_RESERVED; n++) {
        _mbtiOfToken[(TEAM_EACH_MBTI_RESERVED * i) + n] = i;
      }
