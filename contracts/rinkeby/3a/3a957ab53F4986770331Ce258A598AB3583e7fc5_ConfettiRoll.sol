...
contract ConfettiRoll is AccessControlEnumerable, Ownable, Pausable {
    ...
    function getPendingGames(address player) public view returns (bytes32[] memory) {
        bytes32[] memory pendingGames = new bytes32[](pendingRewards[player].length());
        for (uint i = 0; i < pendingRewards[player].length(); i++) {
            pendingGames[i] = pendingRewards[player].at(i);
        }
        return pendingGames;
    }
