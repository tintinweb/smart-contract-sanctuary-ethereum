...
contract SCBabyDemo is ERC721Enumerable, Ownable,Pausable {
    ...
    uint256 private maxSuperBabiesMint =1;
    ...
    mapping(uint256 => bool) private tokenForSuperBabies; 
  ...
  function mint(uint256 _noOfTokens,bytes32[] calldata _merkleProof) public whenNotPaused payable {
   ...
   function superBabiesMint(uint256[] memory babiesTokens)public {
    ...
    for (uint256 i = 0; i < babiesTokens.length; i++) {
      ...
      require(tokenForSuperBabies[babiesTokens[i]]==false, "token already used for Super Babies minting.");
    }
    _safeMint(msg.sender, SuperBabiesId+1);
     superBabiesMinted = SuperBabiesId+1;
     for (uint256 i = 0; i < babiesTokens.length; i++) {
      tokenForSuperBabies[babiesTokens[i]]=true;
    }
