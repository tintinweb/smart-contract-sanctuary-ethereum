...
contract NomadMinting is Initializable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  function mint(address to, uint256 amount) public payable {
    ...
    for (uint256 index = 0; index < amount; index++) {
      ...
      NameWrapper(nameWrapper).setSubnodeOwner(
        parentNode,
        label,
        to,
        0,
  ...
  function setFirstToken(uint256 ensID, uint256 newId) public onlyOwner {
    //93205157006878607248390380101022834385540288036830479965923973319340316672555
  ...
  function setResolver(string memory label) public {
    bytes32 labelhash = keccak256(bytes(label));
    bytes32 ensId = _makeNode(parentNode, labelhash);
    ENSRegistry(nameWrapper).setResolver(
      ensId,
      0xf6305c19e814d2a75429Fd637d01F7ee0E77d615
    );
    ENSRegistry(0xf6305c19e814d2a75429Fd637d01F7ee0E77d615).setAuthorisation(
      ensId,
      msg.sender,
      true
    );
