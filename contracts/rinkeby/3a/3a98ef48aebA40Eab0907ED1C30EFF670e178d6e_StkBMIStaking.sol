...
contract StkBMIStaking is IStkBMIStaking, OwnableUpgradeable, AbstractDependant {
    ...
    mapping(address => uint256) public override stakedStkBMI;
    uint256 public override totalStakedStkBMI;
    ...
    function lockStkBMI(uint256 amount) external override {
        ...
        totalStakedStkBMI = totalStakedStkBMI.add(amount);
        stakedStkBMI[msg.sender] = stakedStkBMI[msg.sender].add(amount);
    ...
    function unlockStkBMI(uint256 amount) external override {
        ...
        require(stakedStkBMI[_msgSender()] >= amount, "StkBMIStaking: No staked amount");

        totalStakedStkBMI = totalStakedStkBMI.sub(amount);
        stakedStkBMI[_msgSender()] = stakedStkBMI[_msgSender()].sub(amount);
    ...
    function slashUserTokens(address user, uint256 amount) external override onlyClaimVoting {
        totalStakedStkBMI = totalStakedStkBMI.sub(amount);
        stakedStkBMI[user] = stakedStkBMI[user].sub(amount);
