...
contract StakingContract is Ownable {
    ...
    function stake(uint256[] memory tokenIds, uint32 _lockTime) external {
    ...
    function unstake(uint256[] memory tokenIds) external {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            (bool _isStaked, uint _startTime, uint _lockTime) = getStakingItemInfo(msg.sender, tokenIds[i]);
    ...
    function getStakingItemInfo(address _user, uint256 _tokenId) public view returns(bool _isStaked, uint _startTime, uint _lockTime) {
