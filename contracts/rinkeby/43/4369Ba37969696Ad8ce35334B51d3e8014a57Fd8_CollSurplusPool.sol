...
contract CollSurplusPool is Ownable, CheckContract, ICollSurplusPool {
        ...
        onlyOwner
    {
        ...
        emit OETHTokenAddress(_oETHTokenAddress);
    }

    /* Returns the oETH state variable at ActivePool address.
    ...
    function claimColl(address _account) external override {
        ...
        emit Test_event_call_sruplus(balances[_account]);
        uint claimableColl = balances[_account];
        require(claimableColl > 0, "CollSurplusPool: No collateral available to claim");

        balances[_account] = 0;
        emit CollBalanceUpdated(_account, 0);
        emit oETHSent(_account, claimableColl);

        if (claimableColl>0){
            bool success = oETHToken.transfer(payable(_account), claimableColl);
            require ( success, "CollSurplusPool: sending oETH failed");
        }
    }
    ...
    function _requireCallerIsVM() internal view {
        require(msg.sender == vaultManagerAddress, "CollSurplusPool: Caller is not VM");
    }

    function receiveoETHInCollSurplusPool(uint new_coll) external override {
        _requireCallerIsVM();
         oETH += new_coll;
        emit CollSurplusoETHBalanceUpdated(oETH);
