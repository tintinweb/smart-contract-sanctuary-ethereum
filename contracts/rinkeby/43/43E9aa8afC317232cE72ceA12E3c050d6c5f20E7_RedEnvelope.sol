...
contract RedEnvelope is ERC1155, Ownable {
    ...
    function preMint(address[] memory _addresses) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; ++i) {
            ...
            require(totalNFTClaimed[to] == 0, "Too many claims per wallet");
            for (uint256 j = 0; j < 2; ++j) {
                 RedEnvelopeType mintedType = mintNFT(to);
    ...
    function claim(uint256 _amount) external payable {
        require(totalNFTClaimed[msg.sender] + _amount <= MAX_CLAIM_PER_WALLET, "Too many claims per wallet");
        require(_amount > 0 && _amount <= MAX_CLAIM_PER_WALLET, "Wrong claim amount");
    ...
    function mintNFT(address _to) internal returns (RedEnvelopeType) {
        ...
        uint256 id = uint256(keccak256(
                abi.encodePacked(block.difficulty, block.timestamp))) % MAX_SUPPLY;
