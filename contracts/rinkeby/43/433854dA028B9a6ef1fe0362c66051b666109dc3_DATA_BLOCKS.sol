...
contract DATA_BLOCKS is ERC1155, Ownable {
  ...
  bool public mintZenBlockActive = false;
  bool public upgradeZenBlockActive = false;
  ...
  uint256 constant public priceInASH = 8000000000000000000;

  uint256 private _totalSupply = 0;
  uint256 constant private _mintPerTxLimit = 2; // per tx
  uint256 constant private _mintPerAddressLimit = 2; // per wallet mint limit
  ...
  string constant private _eip191prefix = "\x19Ethereum Signed Message:\n34AL_DATA_BLOCKS";
  string constant private _eip191prefixFREE = "\x19Ethereum Signed Message:\n36FREE_DATA_BLOCKS";

  event MintActivated();
  event MintDeactivated();
  event MintZenBlockActivated();
  event MintZenBlockDeactivated();
  event UpgradeZenBlockActivated();
  ...
  function mintAllowlistedWithASH(uint256 _mintAmount, uint8 v, bytes32 r, bytes32 s) public {
    ...
    require(_mintAmount <= _mintPerTxLimit, "max mint amount per session exceeded");
    require((_totalSupply + _mintAmount) <= maxSupply, "max NFT limit exceeded");

    require(validatorAddress != address(0x0), "validator not set");
    bytes32 hash = keccak256(abi.encodePacked(_eip191prefix, msg.sender));
    require(validatorAddress == ecrecover(hash, v, r, s), "signature is wrong!!");

    uint256 userMintedCount = balanceOf(msg.sender, _tokenId);
    require(userMintedCount + _mintAmount <= _mintPerAddressLimit, "max NFT per address exceeded");
  ...
  function mintFree(uint256 _mintAmount, uint8 v, bytes32 r, bytes32 s) public {
    require(mintActive, "mint is not active");
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(_mintAmount <= _mintPerTxLimit, "max mint amount per session exceeded");
    require((_totalSupply + _mintAmount) <= maxSupply, "max NFT limit exceeded");
    ...
    require(validatorAddress != address(0x0), "validator not set");
    bytes32 hash = keccak256(abi.encodePacked(_eip191prefixFREE, msg.sender));
    require(validatorAddress == ecrecover(hash, v, r, s), "signature is wrong!!");
  ...
  function burnToMintZenBlock(uint256 _burnAmount) public {
    require(mintZenBlockActive, "mint zen block is not active");
  ...
  function burnToUpgradeZenBlock(uint256 _zenBlockTokenId) public {
    require(upgradeZenBlockActive, "upgrade zen block is not active");
  ...
  function activateMint() external onlyOwner {
    require(!mintActive, "Already activated");
    mintActive = true;
    emit MintActivated();
  }
  ...
  function activateZenBlockMint() external onlyOwner {
    require(!mintZenBlockActive, "Already activated");
    mintZenBlockActive = true;
    emit MintZenBlockActivated();
  }
  function deactivateZenBlockMint() external onlyOwner {
    mintZenBlockActive = false;
    emit MintZenBlockDeactivated();
  }

  function activateZenBlockUpgrade() external onlyOwner {
    require(!upgradeZenBlockActive, "Already activated");
    upgradeZenBlockActive = true;
    emit UpgradeZenBlockActivated();
  }
  function deactivateZenBlockUpgrade() external onlyOwner {
    upgradeZenBlockActive = false;
