    ...
    PausableUpgradeable
{
    ...
    function storeAssetsApprovals(
        ...
        // Retrieve memberUID from message sender - in this case any wallet of a specific user can store ERC721. TODO = check if we want only primaryWallet to do this or any wallet is ok.
        string memory _memberUID = IMember(IMemberAddress).getUID(msg.sender);
        ...
        for (uint256 i = 0; i < approval_.length; i++) {
            if (approval_[i].approvalId == approvalId) {
                ...
                emit ApprovalsEvent(
                    ...
                    approval_[i].beneficiary.beneficiaryAddress,
                    approval_[i].token.tokenId,
                    approval_[i].token.tokenAddress,
                    approval_[i].token.tokenType,
                    approval_[i].token.tokensAllocated,
                    approval_[i].dateApproved,
                    approval_[i].claimed,
                    approval_[i].active,
                    approval_[i].approvalId
                );
    ...
    PausableUpgradeable
{
    ...
    */
        ...
        override
    {
        ...
        require(
            _member.wallets[_member.primaryWallet] == msg.sender,
            "Only primary wallet can add a factory"
        );
