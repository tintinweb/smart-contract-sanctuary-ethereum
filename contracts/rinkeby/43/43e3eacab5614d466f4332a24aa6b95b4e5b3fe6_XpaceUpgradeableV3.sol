...
contract XpaceUpgradeableV3 is ERC721EnumerableUpgradeable, OwnableUpgradeable, PausableUpgradeable, RevealableUpgradeable, WhitelistUpgradeable {
  ...
  function mintPrice() public view returns (uint) {
    if (_mintPrices.length > 0) {
      uint curTimestamp = block.timestamp;
      if (curTimestamp > _startAt) {
        uint index = (curTimestamp - _startAt) / _priceDuration;
        index = index >= _mintPrices.length ? (_mintPrices.length - 1) : index;
        return _mintPrices[index];
      }
      return _mintPrices[0];
    }
