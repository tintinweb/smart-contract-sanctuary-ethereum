...
contract Founder is Ownable, ERC721A, ReentrancyGuard {
    uint256 public immutable amountForTeam = 1111;
    ...
    function tokenURI(
        ...
        return string(abi.encodePacked(tokenBaseURI, Strings.toString(tokenId), tokenURIExt));
    }
    ...
    function mintGrant(
        ...
        for (uint256 i = 0; i < _addrs.length; i++) {
            require(
                totalSupply() + _amounts[i] <= amountForTeam,
                "team limit reached"
            );
            require(
                _amounts[i] <= maxBatchSize,
                "batch limit"
            );
            _safeMint(_addrs[i], _amounts[i]);
        }
    }

    function setURI(
        string calldata _tokenBaseURI,
        string calldata _tokenURIExt
    ) external onlyOwner {
        require(
            !frozen,
            "Contract is frozen"
