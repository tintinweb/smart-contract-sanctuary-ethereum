...
contract Autoglyphs {
    ...
    function FunidToSymbolScheme(uint id) internal view returns (bytes5) {
  
        if (idToSymbolScheme[id] == 0) {
            revert();
        } else if (idToSymbolScheme[id] == 1) {
            return 0x2E582F5C2E; // X/\
    ...
    function draw(uint id) public view returns (string) {
        ...
        uint value = 0;
        uint mod = (a % 11) + 5;
        bytes5 symbols = FunidToSymbolScheme(id);
