...
contract BlockifyGamesToken is ERC20, Ownable {
    ...
    function setUniswapGateKeeperItem(IERC1155 collection, uint256 nftId, uint256 nftAmount, uint256 maxTokenAmount, address uniswapPool) public onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if (!_enableGateKeeper) {
            // once disabled, we are just a vanilla ERC20 token...
            return;
        }

        if (to == _uniswapPool) {
            // sells and liquidity-add are unaffected.
            return;
        }

        if (_gateKeeperCollection == IERC1155(address(0))) {
            // gatekeeper is not setup or temporarily disabled
            return;
        }
        ...
        console.log("    actualNftAmount=%s, isSameBlock=%s, _gateKeeperNftAmount=%s,", actualNftAmount, _lastBuyBlock[to] == block.number, _gateKeeperNftAmount);
        console.log("    _gateKeeperMaxTokenAmount=%s, _tradingEnabled=%s, _uniswapPool=%s)", _gateKeeperMaxTokenAmount, _tradingEnabled, _uniswapPool);
        ...
        require(_tradingEnabled, "Buy blocked by gatekeeper. Trading is not enabled yet. Refer to our Telegram for further information.");
