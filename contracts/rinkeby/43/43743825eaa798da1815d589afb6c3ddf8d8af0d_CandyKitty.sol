    ...
    SignerManager
{
    ...
    function preSaleMint(
        ...
        require(_quantity <= 5, "_quantity must lower than 5");
        require(_quantity > 0, "_quantity must greater than zero");
        require(tokenIdx + _quantity <= maxSupply, "exceed max presale amount");
    ...
    function mintTo(address to, uint size) external onlyLaunchpad {
        require(status == Status.PublicSale, "not in public sale mint period");
        require(tokenIdx + size <= maxSupply, "exceed max mint amount");
