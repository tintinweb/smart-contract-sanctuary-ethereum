...
contract Glamour is Ownable, ERC721A {
    ...
    uint private constant MAX_SUPPLY = 188;
    ...
    constructor(bytes32 _merkleRoot, string memory _baseURI, string memory _hiddenURI) ERC721A("GlamourQueen", "GQ") {
    ...
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(currentTime() < saleStartTime + 86400 minutes, "Whitelist Sale is finished");
        require(sellingStep == Step.PreSale, "Whitelist sale is not activated");
        require(isWhiteListed(msg.sender, _proof), "Not whitelisted");
        require(amountNFTsperWalletWhitelistSale[msg.sender] + _quantity <= 2, "You can only get 3 NFT on the Whitelist Sale");
