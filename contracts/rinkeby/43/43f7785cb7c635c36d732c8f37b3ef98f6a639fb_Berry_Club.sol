...
contract Berry_Club is ERC721A,Pausable,Ownable{

  uint256 public  MINT_PRICE = 0.0001 ether;
  uint256 constant public init_supply = 5;
  ...
  constructor() ERC721A("Berry_Club", "BRC ",5,10000) {
    InitMinting();
  }

  function InitMinting() internal  {
      _safeMint(msg.sender, init_supply);
      totalInitialMinted=init_supply;
  }

  function mintOwner(uint256 _count) external onlyOwner {
      require(totalSupply() + _count <= collectionSize, "MAX_SUPPLY_REACHED");
      _safeMint(msg.sender, _count);
  }

  function claim_NFT() external 
  {
      require(preSaleStarted, "MINT_NOT_STARTED");
      require(totalSupply() + userInfo[msg.sender].quantity <= collectionSize, "MAX_SUPPLY_REACHED");
      require(userInfo[msg.sender].userStatus!=false,"Address isn't Whitelisted");
      // require(totalWhiteListMinted[msg.sender]+userInfo[msg.sender].quantity<=maxBatchSize,"you can't mint many");

      _safeMint(msg.sender, userInfo[msg.sender].quantity);
      totalWhiteListMinted[msg.sender]+=userInfo[msg.sender].quantity;
      addToMintData(userInfo[msg.sender].quantity);
  }
  ...
  require(totalSupply() + _count <= collectionSize, "MAX_SUPPLY_REACHED");
  require(msg.value >=(_count * MINT_PRICE), "Invalid_BNB_Sent");
  _safeMint(msg.sender, _count);
  ...
  uint256[] memory tokenIds = new uint256[](ownerTokenCount);
      ...
      // addWhiteList()
  function addWhiteList(address[] memory _addresses, uint256[] memory _quantity) external onlyOwner {
      for(uint i = 0; i < _addresses.length; i++) {
      // isWhitelisted[_addresses[i]] = true;
      userInfo[_addresses[i]].userStatus = true;
      userInfo[_addresses[i]].quantity = _quantity[i];
