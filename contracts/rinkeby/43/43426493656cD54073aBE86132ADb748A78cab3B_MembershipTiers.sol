...
contract MembershipTiers {
    ...
    /// @param tokensBurned     Amount of brand tokens burned to NFT
    ...
    /// @param burn_     Bool whether or not to deposit or burn brand tokens
    function recieveCredit(
        ...
        IdInfo storage _idInfo = idInfo[nft_][id_];
        if (IERC20(_nftInfo.brandToken).balanceOf(msg.sender) < amount_) revert BalanceTooLow();

        if (burn_) {
            IERC20Burn(_nftInfo.brandToken).burn(msg.sender, amount_);
            _idInfo.tokensBurned += amount_;
        } else {
    ...
    function renew(
        ...
        IdInfo storage _idInfo = idInfo[nft_][id_];
        if(nft_ != msg.sender) {
            IERC20Burn(_nftInfo.brandToken).burn(msg.sender, _nftInfo.renewalPrice * periods_);
    ...
    function brandTokenCredit(address nft_, uint256 id_) public view returns (uint256 credit_) {
        ...
        credit_ +=
            ...
            (_idInfo.tokensBurned +
                ((_idInfo.tokensBurned * _nftInfo.extraBurnCredit) / 100));
    }
