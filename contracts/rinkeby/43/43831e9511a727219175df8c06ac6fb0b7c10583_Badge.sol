...
// File: contracts/ERC20/GETSInterface.sol
...
// File: contracts/ERC1155/ERC1155Receiver.sol
...
// File: contracts/ERC1155/new2.sol
...
contract Badge is ERC1155, BaseContract {
 ...
 mapping(uint256 => address) public ownerOfID;
    ...
    // Mapping from NFT to status of transfer
     mapping(address => mapping(uint256 => uint8)) private isBadgeAssigned;
    ...
    function isBadgeAssign(address user ,uint256 ids) external view returns (uint256) {
        return isBadgeAssigned[user][ids];
    }
    ...
    function createBadge(
        ...
        GETSInterface gets = GETSInterface(getsAddress);
            require(
        _priceAmount > 0,
            "price of badge should be greater than zero "
        );
        if (flagFee == true) {
        ...
        isBadgeAssigned[msg.sender][ids] = 0;
 ...
 require(
        ...
        onlySponsor
    {
        ...
        for (uint256 i = 0; i < badgeIds.length; i++) {
            ...
            isBadgeAssigned[to][badgeIds[i]] = 1;
...
function _tokenClaimedTime(address owners , uint badgeId) external view returns(uint){
    return  tokenClaimedTime[owners][badgeId];
}
    ...
    function tokenOfOwnerByIndex(address owners, uint256 index)
        external
        view
        virtual
        returns (uint256)
    {
        return mapUserBadge[owners].at(index);
    }

    function tokenOf(address owners) external view virtual returns (uint256) {
        require(
            owners != address(0),
            "ERC721: balance query for the zero address"
        );
        return mapUserBadge[owners].length();
    }
}