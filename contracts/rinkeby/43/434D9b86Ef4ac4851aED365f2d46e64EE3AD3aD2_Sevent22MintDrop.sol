...
contract Sevent22MintDrop is Ownable, Pausable, ReentrancyGuard, ERC721A {
  ...
  uint256 public immutable preSaleStartTs;
  uint256 public immutable publicSaleStartTs;
  uint256 public immutable publicSaleEndTs;
  ...
  constructor(
    ...
    address _treasury,
    uint256[3] memory maxSupplyAndTeamAlloc,
    uint256[5] memory timestamps,
    bytes32[] memory provenanceAndMerkeRoot,
    address _randomGenerator
  ) ERC721A(name, symbol) {
    require(timestamps[0] <= timestamps[1],"preSaleStartTs > preSaleEndTs");
    require(timestamps[1] <= timestamps[2],"preSaleEndTs > publicSaleStartTs");
    ...
    maxPreSaleSupply = maxSupplyAndTeamAlloc[1];
    teamAllocation = maxSupplyAndTeamAlloc[2];
    preSaleStartTs = timestamps[0];
    preSaleEndTs = timestamps[1];
    publicSaleStartTs = timestamps[2];
    publicSaleEndTs= timestamps[3];
    revealTimestamp = timestamps[4];
    provenanceHash = provenanceAndMerkeRoot[0];
    merkleRoot = provenanceAndMerkeRoot[1];
    randomGenerator = IRandomNumberGenerator(_randomGenerator);

    _mint(teamAllocation, treasury);
  }

  function startingIndex() private view returns (uint256) {
  ...
  {
    require(randomNumberReqSent, "nfts not revealed yet");
  ...
  function availableForPublicSale() external view returns (uint256) {
    return maxSupply - whitelistSold - teamAllocation;
  }
