...
contract ZEN_BLOCKS is ERC721A, AccessControl {
  ...
  uint256 public tokenRefreshStartTs = 0;
  ...
  event TokenRefreshWillActivateOn(uint256 timestamp);
  event TokenRefreshDeactivated();
  ...
  function requestTokenRefresh(uint256 _tokenId) public {
    require(tokenRefreshStartTs != 0, "token refresh time is not set");
    require(block.timestamp >= tokenRefreshStartTs, "wait for token refresh time");

    require(ownerOf(_tokenId) == msg.sender, "not your token");
    require(tokenRefreshRequestStatus[_tokenId] > 0, "not eligible to refresh");
    tokenRefreshRequestStatus[_tokenId] -= 1;
  ...
  function refreshRequestStatusOf(uint256 _tokenId) public view returns (uint256) {
    require(_exists(_tokenId), "token does not exist");
  ...
  function _afterTokenTransfers(
      ...
      uint256 quantity // this is always 1 for normal transfers
  ...
  function activateTokenRefresh(uint256 _tokenRefreshStartTs) external onlyAdmin {
    require(tokenRefreshStartTs == 0, "Already activated");
    tokenRefreshStartTs = _tokenRefreshStartTs;
    emit TokenRefreshWillActivateOn(tokenRefreshStartTs);
  }
  function deactivateTokenRefresh() external onlyAdmin {
    tokenRefreshStartTs = 0;
    emit TokenRefreshDeactivated();
  }

  function setCurrentUpgradeId(uint256 _currentUpgradeId) external onlyAdmin { // NOTE: must be set to powers of 2
