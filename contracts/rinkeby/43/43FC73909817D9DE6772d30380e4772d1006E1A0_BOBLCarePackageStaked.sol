...
contract BOBLCarePackageStaked is Ownable {
    ...
    function claimCarePackage(address _owner, uint16[] calldata _tokenIds, bytes32[] memory _hashes, bytes[] memory _signatures) external {
        ...
        for (uint16 i = 0; i < _tokenIds.length; i++) {
            require(isOwner(_tokenIds[i]), "only owners approved");
    ...
    function isOwner(uint16 _tokenId) public view returns (bool){
        if (erc721Token.ownerOf(_tokenId) == msg.sender) {
            return true;
        }
        uint16[] memory stakedIds = stakingSystem.getStakedTokens(msg.sender);
        for(uint16 i; i<stakedIds.length; i++) {
