...
contract Nft is ERC721, Ownable {
    ...
    */
    ...
    function mint(bytes memory _signature) external returns(uint256) {
        require (verify(signer, msg.sender, _signature), "Invalid signature");
        require(alreadyMinted[msg.sender] == false, "Already claimed");
        require(tokenId.current() < maxSupply, "Max supply limit reached");
        tokenId.increment();
    ...
    */
    ...
    /*  
    * only owner can set the max supply
    * Requirements:
    * '_supply' should not be equal to zero 
    */

    function setMaxSupply(uint256 _supply) external onlyOwner {
        require(_supply != 0 , "Invalid max supply");
        maxSupply = _supply + maxSupply;
    }
