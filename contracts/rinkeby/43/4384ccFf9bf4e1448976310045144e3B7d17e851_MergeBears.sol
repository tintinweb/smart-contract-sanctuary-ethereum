...
// adding comment
contract MergeBears is ERC721A {
  ...
  string constant BLACK_BEAR_HEAD =
  ...
  string constant WHITE_BEAR_HEAD =
  ...
  function animationURI(uint256 tokenId) public view returns (bytes memory) {
    ...
    if (dna % 2 == 0) {
      HEAD = BLACK_BEAR_HEAD;
    } else {
      HEAD = WHITE_BEAR_HEAD;
    }
    ...
    return
      abi.encodePacked(
        ...
        Base64.encode(
          abi.encodePacked(
            '<html><head><style>html,body{margin:0;display:flex;justify-content:center;align-items:center;}.a{width:min(100vw,100vh);height:min(100vw,100vh);background:red 100%/100%;image-rendering:pixelated;position:absolute;</style></head><body>',
              '<div class="a" style="background-image:url(data:image/png;base64,', HEAD , ')"></div>',
            '</body></html>'
          )
        )
      );
  ...
  {
    //TODO: throw an error if tokenId is not found
    require(tokenIdToDNA[tokenId] != 0, "Not found");
    // prettier-ignore
    return
      string(
        abi.encodePacked(
          ...
          Base64.encode(
            abi.encodePacked(
              "{",
                ...
                '"external_url":"https://mergebears.eth.limo",',
