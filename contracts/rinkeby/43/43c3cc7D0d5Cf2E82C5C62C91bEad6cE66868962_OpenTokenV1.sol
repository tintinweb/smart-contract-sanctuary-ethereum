...
contract OpenTokenV1 is ERC20Upgradeable {
    ...
    // decimalsBase 10 ** 18
    uint256 public constant decimalsBase = 1000000000000000000;
    // supply limit 1 Billion
    uint256 public constant supplyLimit = 10000000000 * decimalsBase;
    uint256 public constant firstWeekEmission = 3161335 * decimalsBase;
    uint256 public constant emissionWeekCount = 520;

    uint256 public constant WEEKLY_REDUCTION_RATE = 1007827884862117171; //1.5 ** (1/52) * 1e18
    uint256 public constant WEEK = 300;//7 * 86400;
    ...
    uint256 public lastWeekNum;
    uint256 public totalOpenSupply;
    ...
    modifier onlyByOwner() {
        require(msg.sender == currentOwner || msg.sender == boostEmissionerAddr
    ...
    function mint(address to_, uint256 amount_) external onlyByOwner {
        uint256 newTotalOpenSupply = totalOpenSupply + amount_;
        require(supplyLimit >= newTotalOpenSupply, "supply limit");

        totalOpenSupply = newTotalOpenSupply;
        _mint(to_, amount_);
        emit Minted(msg.sender, to_, amount_, block.timestamp);
    }

    function generateWeekEmissions(uint256 maxWeekNum_) external onlyByOwner returns (bool) {

        require(maxWeekNum_ <= emissionWeekCount, "target week num need <= emissionWeekCount");
        require(maxWeekNum_ > lastWeekNum, "target week num need > lastWeekNum");

        uint256 lastWeekTotalAmount = weekEmissionList[lastWeekNum];
        for(uint i = lastWeekNum+1; i <= maxWeekNum_; i++){
            lastWeekTotalAmount =  lastWeekTotalAmount / WEEKLY_REDUCTION_RATE * decimalsBase;
            weekEmissionList[i] = lastWeekTotalAmount;
        }
        lastWeekNum = maxWeekNum_;
        return true;
    }
    ...
    function weekAvailableSupply(uint256 t_) external view returns (uint256) {
        ...
        require(t_ >= startTime, "t_ week time >= start time");

        uint256 offset_time = t_.sub(startTime, "time error");

        uint256 tWeekNum = offset_time / WEEK;

        return weekEmissionList[tWeekNum];
    }
