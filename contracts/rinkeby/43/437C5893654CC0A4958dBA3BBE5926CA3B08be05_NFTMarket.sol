...
contract NFTMarket is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    mapping(address => uint256) private listedCounts;
    ...
    function createMarketItem(
        ...
        listedCounts[msg.sender]++;

        IERC721Upgradeable(nftContract).transferFrom(msg.sender, address(this), tokenId);

        emit MarketItemCreated(nftContract, uint128(itemId), tokenId, msg.sender, address(0), price, false);
    }
    ...
    function createMarketSale(address nftContract, uint256 itemId) public payable nonReentrant {
        ...
        uint256 _treasuryFee = (price * treasuryPercentage) / 10000;
        uint256 actualPrice = price - treasuryFee;
        if (paymentCurrency == address(0)) {
            require(msg.value == price, "ERR: insufficient funds");
            idToMarketItem[itemId].seller.transfer(actualPrice);
        } else {
            TransferHelper.safeTransferFrom(paymentCurrency, msg.sender, address(this), price);
            TransferHelper.safeTransfer(paymentCurrency, idToMarketItem[itemId].seller, actualPrice);
        }
        ...
        listedCounts[idToMarketItem[itemId].seller]--;
        listedCounts[msg.sender]++;
        treasuryFee = _treasuryFee;
    }

    function purchaseAttributes() external nonReentrant {
        require(priceForAttribute != 0, "ERR: no determined price yet");
        uint256 _treasuryFee = (priceForAttribute * treasuryPercentage) / 10000;
        TransferHelper.safeTransferFrom(paymentCurrency, msg.sender, address(this), priceForAttribute);
        TransferHelper.safeTransfer(paymentCurrency, stakingContract, priceForAttribute - _treasuryFee);
        treasuryFee += _treasuryFee;
    ...
    function fetchMyNFTs() public view returns (MarketItem[] memory) {
        ...
        uint256 itemCount = listedCounts[msg.sender];
        uint256 currentIndex = 0;
    ...
    function setTreasuryPercentage(uint256 _percentage) external onlyOwner nonReentrant {
        require(_percentage <= 100, "ERR: overflow percentage");
        treasuryPercentage = _percentage * 100;
    }
    ...
    function setStakingContract(address _stakingContract) external onlyOwner nonReentrant {
    ...
    function withdrawPremium(address _currency, uint256 _amount) external onlyOwner nonReentrant {
        ...
        if (_currency == address(0)) {
            ...
            treasuryFee = treasuryFee - _amount;
        }
