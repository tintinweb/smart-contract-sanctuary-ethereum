...
contract HYFI_Lottery is Initializable, PausableUpgradeable, AccessControlUpgradeable {
        ...
        returns (uint256[] memory, uint256)
    {
        ...
        uint256[] memory rewardsAmounstById = new uint256[](_rewardsManagers.length);
        // is used on to calculate total won amounts of each reward
        uint256[] memory totalRewardsAmounstById = new uint256[](_rewardsManagers.length);
        uint256 j;
        uint256 i;
        ...
        returns (uint256[] memory)
    {
        ...
        uint256 randomValue = getRandomValue(packNumber) % (_rangeMax + 1); // 0 - 9
