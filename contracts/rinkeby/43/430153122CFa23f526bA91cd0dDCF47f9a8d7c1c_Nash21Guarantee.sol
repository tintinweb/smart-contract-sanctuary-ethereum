...
contract Nash21Guarantee is INash21Guarantee, UUPSUpgradeableByRole {
    ...
    bytes32 private constant GUARANTEE_ADMIN_ROLE = keccak256('GUARANTEE_ADMIN_ROLE');

    bytes32 private constant GUARANTEE_FUNDER_ROLE = keccak256('GUARANTEE_FUNDER_ROLE');
    ...
    /// @inheritdoc INash21Guarantee
    function extractFunds(address token, address to, uint256 amount) public onlyRole(GUARANTEE_FUNDER_ROLE) {
        IERC20(token).transfer(to, amount);
    }

    /// @inheritdoc INash21Guarantee
    function fund(address token, address from, uint256 amount) public {
    ...
    function claim(
        ...
        address account = factoryInterface.recipients(id);
    ...
    function claimable(
        ...
        INash21Factory factoryInterface = INash21Factory(INash21Manager(manager).get(keccak256('FACTORY')));
        return factoryInterface.getReleased(id) - distributed[id];
    }
    ...
    ) public onlyRole(GUARANTEE_ADMIN_ROLE) {
    ...
    function transformCurrency(
        ...
        AggregatorV3Interface aggregatorInterface = AggregatorV3Interface(feeds[currency]);
