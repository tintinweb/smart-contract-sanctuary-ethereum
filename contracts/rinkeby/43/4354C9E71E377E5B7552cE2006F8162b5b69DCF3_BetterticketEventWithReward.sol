...
contract BetterticketEventWithReward is KeeperCompatibleInterface {

    error CannotObtainRewardsAnymore();
    ...
    constructor(uint updateInterval, address _smtAddress, address _ownerAddress) {
        counter = 0;
        owner = msg.sender;
    ...
    function addevents(
        ...
        require(msg.sender == owner, "Only owner of smart contract can put events");
    ...
    modifier endTicketSale{
        require (block.timestamp <= endSale, "The Ticket sale is over");
        _;
    }
    ...
    function end(uint totalTime) public{
        endSale = totalTime + startSale;
    }

    function getTimeLeft() public endTicketSale view returns(uint){
        return endSale - block.timestamp;
    }
    ...
    function performUpkeep( bytes calldata) external {
        ...
        if (getTimeLeft() <= 0)  {
        revert CannotObtainRewardsAnymore();
    }

    token.transfer(msg.sender,rewardAmount * 100000000000000);
    ...
    function getTicket(uint256 id, string[] memory newTicket) public payable {
        ...
        emit newTicketPurchased(newTicket, id, msg.sender, events[id].eventLocation,  events[id].imgUrl);
