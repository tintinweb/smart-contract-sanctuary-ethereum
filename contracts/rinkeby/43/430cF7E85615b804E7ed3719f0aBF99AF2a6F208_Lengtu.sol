...
contract Lengtu is Ownable, ERC721A, ReentrancyGuard, VRFConsumerBaseV2 {
    ...
    constructor() ERC721A("lengtu", "LENGTU") VRFConsumerBaseV2(vrfCoordinator) {
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
        _safeMint(amadeusAddress, 10);
    }
    ...
    // auction
    uint256 public amountForAuction = 10;
    ...
    function auctionMint(uint256 quantity) external payable callerIsUser {
        require(auctionSaleStartTime != 0 && block.timestamp >= uint256(auctionSaleStartTime), "not begun");
        require(amountForAuction >= quantity, "reached max amount");
        require(quantity <= auctionPerMint, "reached max amount per mint");
        require(totalSupply() + quantity <= collectionSize, "reached max supply");
