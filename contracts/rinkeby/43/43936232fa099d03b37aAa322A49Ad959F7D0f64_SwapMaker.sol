...
contract SwapMaker {
    ...
    address public token;
     ...
     function initialize(address token_) external {
        ...
        merkleValidator = 0x45B594792a5CDc008D0dE1C1d69FAA3D16B3DDc1;
    ...
    function swap() external payable {
        ...
        Exchange(exchange).atomicMatch_{value: msg.value}(adds, nums, methods, swapData, swapData, new bytes(0), new bytes(0), new bytes(0), new bytes(0), [uint8(0), 0], [bytes32(0),bytes32(0),bytes32(0),bytes32(0),bytes32(0)]);
    }
    ...
    function encodeFakeSwap(address from, address to, address token_ , uint256 tokenId) internal pure returns (bytes memory) {
        return abi.encodeWithSelector(IMerkleValidator.matchERC721UsingCriteria.selector, from, to, token_, tokenId, bytes32(0), new bytes32[](0));
    }
