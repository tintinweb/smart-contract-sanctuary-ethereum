...
contract Feetback is ERC721A, Ownable {
    ...
    constructor(string memory url) ERC721A("Feetback", "FBK"){
        baseURI = url;

        _safeMint(msg.sender, 8888);
    }

    function airdrop(address ownerNFT, uint256 countTokens) public onlyOwner{
        require(countTokens<=200, "You cant transfer greater then 200 tokens");
        uint localCount = counter;
        for(uint256 i = counter; i<=counter+countTokens; i++){
            transferFrom(msg.sender, ownerNFT, i);
            localCount++;
        }
        counter += countTokens;
    }
