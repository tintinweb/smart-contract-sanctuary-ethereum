...
contract Theirsverse is Ownable, ERC721A, Pausable, AccessControl, ReentrancyGuard {
    ...
    function _batchMint(address account, uint256 quantity) internal {
        uint256 batchSize = 10;
        if (quantity <= batchSize) {
            _safeMint(account, quantity);
        } else {
            uint256 batchNumber = quantity / batchSize;
            uint256 remainder = quantity % batchSize;
            uint256 i = 0;
            while (i < batchNumber) {
                _safeMint(account, batchSize);
                i++;
            }
            if (remainder > 0) {
                _safeMint(account, remainder);
            }
        }
    }
