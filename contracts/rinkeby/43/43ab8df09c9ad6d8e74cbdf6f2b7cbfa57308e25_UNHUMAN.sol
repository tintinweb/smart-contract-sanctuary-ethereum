...
abstract contract ERC721URIStorage is ERC721 {
    ...
    function makeArt(uint256 numberOs) public view virtual returns (string memory){
        uint256 yearAdjust;
        yearAdjust = getCurrentYear() - 2018 - 4;
        ...
        uint score = numberOs - 3 + yearAdjust;
        uint256[10] memory textYArray;
        textYArray[0] = 35;
        textYArray[1] = 65;
        textYArray[2] = 95;
        textYArray[3] = 125;
        ...
        OGArray[9] = "";
        uint rowCounter = 0;
        rowCounter = ((score + 1) / 13);
        string memory finalString;
        uint charCounter = 0;

        for (uint256 i = 0; i < score; i++) {            
            OGArray[(i / 13)] = concatenate(OGArray[(i / 13)], "O");
            if (i + 1 == score) {
                OGArray[((i + 1) / 13)] = concatenate(OGArray[((i + 1) / 13)], "G");
            }
        ...
        for (uint256 i = 0; i <= rowCounter; i++) {
            ...
            texts = concatenate(texts,  uint2str(textYArray[i]));
            texts = concatenate(texts,  '" class="base">');
            texts = concatenate(texts,  OGArray[i]);
            texts = concatenate(texts,  "</text>");
        }

        finalString = "<svg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='xMinYMin meet' viewBox='0 0 350 350'><style>.base { fill: black; font-family: sans-serif; font-size: 32px; }</style><rect width='100%' height='100%' fill='white' />";
...
constructor(address initialRoyaltiesReceiver) ERC721("UNHUMAN COMPOSITIONS", "UNHUMAN") {
    ...
    function mint(uint256 noYears) external payable {
        ...
        _setOGs(MAX_SUPPLY-artBalance[address(this)]+1, noYears);
