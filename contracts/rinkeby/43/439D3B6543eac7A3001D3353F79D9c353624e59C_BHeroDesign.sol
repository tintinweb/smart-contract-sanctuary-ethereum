...
contract BHeroDesign is AccessControlUpgradeable, UUPSUpgradeable, IBHeroDesign {
  ...
  function initialize() public initializer {
    ...
    mintCost = 10 ether; // burda bi token en az kaç ether olacak biz dinamik yapcaz  
    maxLevel = 5;
    upgradeCosts.push([1 ether, 2 ether, 4 ether, 7 ether]); // level 0
    upgradeCosts.push([2 ether, 4 ether, 5 ether, 9 ether]); // level 1
    upgradeCosts.push([2 ether, 4 ether, 5 ether, 10 ether]); // level 2
    upgradeCosts.push([3 ether, 7 ether, 11 ether, 22 ether]); // level 3
    upgradeCosts.push([7 ether, 18 ether, 40 ether, 146 ether]); // level 4
    upgradeCosts.push([9 ether, 25 ether, 56 ether, 199 ether]); // level 5
    // bunu adama soralım level up costları ne olsun diye 
  }
  ...
  function getRarityStats() external view returns (Stats[] memory) {
    uint256 size = dropRate.length; // 6
    Stats[] memory result = new Stats[](size);
    for (uint256 i = 0; i < size; ++i) {
      result[i] = rarityStats[i]; // okay here is struct array with rarity stats  common rarity vs yes
    }
    return result;
  }
  ...
  function createRandomToken( // this is important function
    uint256 seed,
    uint256 id,
    uint256 rarity
    ...
    if (rarity == BHeroDetails.ALL_RARITY) {
      // Random rarity.
      (seed, details.rarity) = Utils.weightedRandom(seed, dropRate); // weighted function is in the uitls.sol 
    } else {
      // Specified rarity.
      details.rarity = rarity - 1; // 0 - 5 o yüzden -1 var
    }
    details.level = 1;

    Stats storage stats = rarityStats[details.rarity]; // burda rarity statsı alıyoruz
    details.bombCount = stats.bombCount; // her rairty için bomb count fixed 
    details.bombRange = stats.bombRange; // her rairty için bomb range fixed

    (seed, details.color) = Utils.randomRangeInclusive(seed, 1, COLOR_COUNT); // 1 - 6 arası random color
    (seed, details.skin) = Utils.randomRangeInclusive(seed, 1, SKIN_COUNT); // 1 - 8 arası random skin
    (seed, details.stamina) = Utils.randomRangeInclusive(seed, stats.stamina.min, stats.stamina.max); // rarity statsı alıp stamina min max arası random stamina
    (seed, details.speed) = Utils.randomRangeInclusive(seed, stats.speed.min, stats.speed.max); // rarity statsı alıp speed min max arası random speed
    (seed, details.bombSkin) = Utils.randomRangeInclusive(seed, 1, BOMB_SKIN_COUNT); // 1 - 20  arası random bomb skin
    (seed, details.bombPower) = Utils.randomRangeInclusive(seed, stats.bombPower.min, stats.bombPower.max); // rarity statsı alıp bomb power min max arası random bomb power
    // okay bu sistemi de anladık adam değişiklik isterse yapabiliriz tamam
