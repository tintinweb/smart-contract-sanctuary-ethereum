...
abstract contract ERC20 is IERC20{
   ...
   function approve(address spender, uint256 amount) public override returns (bool success){
    ...
    function transferFrom(address from, address to, uint256 amount) external returns (bool success){
        if(from != msg.sender){
            uint allowaceAmount = _allowances[from][msg.sender];
            require(amount <= allowaceAmount,"transfer amount exceeds allowance");
            _approve(from,msg.sender, allowaceAmount-amount);
        }
    ...
    function _transfer(address from, address to, uint amount) internal{
        ...
        require(from != address(0), "transfer from zero adderess");
...
contract AEKC is ERC20 {
    constructor() ERC20("Aek Coin","AEKC"){
