...
contract veTokenPresale is Ownable, ReentrancyGuard {
    ...
    address public constant usdcAddress = 0x2d7dFcf8837318aC028D13F276615f3BF605b4Ec;
    ...
    uint256 oneDayETH = oneHourETH * 24; // 6,456
    uint256 yearHalfDaysETH = oneDayETH * 548;  
    ...
    address public immutable  teamSpenderAddress;
    ...
    constructor(
        uint256 _startBlock,
        address _teamSpenderAddress,
        address _VE3DAddress
    ) {
        require(block.number < _startBlock, "cannot set start block in the past!");
        require(
            _teamSpenderAddress != _VE3DAddress,
            "_teamSpenderAddress cannot be equal to _VE3DAddress"
        );
        require(_teamSpenderAddress != address(0), "_VE3DAddress cannot be the zero address");
        ...
        teamSpenderAddress = _teamSpenderAddress;
    }

    function buyVE3D(uint256 usdcToSpend) external nonReentrant {
        require(msg.sender != teamSpenderAddress, "treasury address cannot partake in presale");
        require(
        ...
        require(
            ERC20(VE3DAddress).balanceOf(address(this)) > 0,
            "No more VE3D left! Come back next time!"
        );
        ...
        if (VE3DPurchaseAmount > maxVE3DPurchase) VE3DPurchaseAmount = maxVE3DPurchase;

        // if we dont have enough left, give them the rest.
        if (VE3DRemaining < VE3DPurchaseAmount) VE3DPurchaseAmount = VE3DRemaining;
        ...
        require(
            VE3DPurchaseAmount <= ERC20(VE3DAddress).balanceOf(address(this)),
            "not enough VE3D in contract"
        );
        ...
        if (usdcSpent > 0)
            ERC20(usdcAddress).safeTransferFrom(msg.sender, teamSpenderAddress, usdcSpent);

        whitelist[msg.sender] = false;
        if (remainingBuyers > 0) remainingBuyers--;

        emit VE3DPurchased(msg.sender, usdcSpent, VE3DPurchaseAmount);
    }

    function sendUnclaimedsToTeamSpenderAddress() external onlyOwner {
        ...
        ERC20(VE3DAddress).safeTransfer(teamSpenderAddress, VE3DRemainingBalance);
    ...
    function addToWhiteList(address participant, bool included) external onlyOwner {
        require(
            block.number < startBlock,
            "cannot change whitelist if sale has already commenced"
        );

        if (whitelist[participant] && !included && remainingBuyers > 0) remainingBuyers--;
        else if (!whitelist[participant] && included) remainingBuyers++;
    ...
    function setSalePriceE35(uint256 _newSalePriceE35) external onlyOwner {
        require(
            block.number < startBlock - (oneHourETH * 4),
