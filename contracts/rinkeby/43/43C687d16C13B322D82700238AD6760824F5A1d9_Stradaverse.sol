  ...
  function tokenURI(uint256 id_) public view override returns (string memory) {
    ...
    string memory traitsA = string(
      abi.encodePacked(
        ...
        _makeAttribute(
          "Accesories",
          TraitsNames.accesories(traits.accesoriesId)
        ),
        ","
      )
    );
...
library TraitsNames {
  ...
  function accesories(uint8 id) internal pure returns (string memory) {
    if (id == 0) {
      return "Earrings";
    } else if (id == 1) {
      return "Gum";
