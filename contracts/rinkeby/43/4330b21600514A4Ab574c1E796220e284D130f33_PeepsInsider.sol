    ...
    ERC2981
{
   ...
   /**
    @notice Flag indicating that dutch auction minting is open.
     */
    bool public dutchAuction;

    constructor(address payable beneficiary, address royaltyReceiver)
        ERC721ACommon("PEEPS Insider", "PeepsInsider")
        BaseTokenURI("https://gateway.ipfs.io/ipns/meta.peeps.art/")
        LinearDutchAuction(
            LinearDutchAuction.DutchAuctionConfig({
                startPoint: 0, // disabled at deployment
                startPrice: 1 ether,
                decreaseInterval: 300, // 5 minutes
                decreaseSize: 0.25 ether,
                numDecreases: 3,
    ...
    /**
    @notice Set the `dutchAuction` flag to start the auction mint.
     */
    function startDutchAuction(bool _dutchAuction) external onlyOwner {
        dutchAuction = _dutchAuction;
    }

    /**
    @notice Mint during dutch auction
     */
    function mintDutchAuction(address to, uint256 n) external payable {
        require(dutchAuction, "All tokens have been minted");
        _safeMint(to, n);
    }
