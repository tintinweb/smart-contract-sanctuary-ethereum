...
contract DGLDToken is ERC20Upgradeable, OwnableUpgradeable {
    ...
    // Stores the multi signature wallet address. 
    ...
    /// @notice Logs any multisig address modification 
    /// @param oldAddress the old multisig address
    /// @param newAddress the new multisig address
    event MultisigAddressUpdated(address oldAddress, address newAddress);
    ...
    event Burn(address from, uint256 amount, string barId, uint256 redemptionFeeAmount);
    ...
    function initialize(
        ...
        address _feeAddress,
        bool isFeeActivated) public virtual initializer {
        __ERC20_init(name, symbol);
        __Ownable_init();

        require(_multiSigAddress != address(0), "DGLD: Multisig address cannot be zero address");
        require(_feeAddress != address(0), "DGLD: fee address cannot be zero address");
    ...
    /// @notice Set the multi signature wallet address
    /// @dev only multi signature address can call this function. Emits MultisigAddressUpdated event
    /// @param multiSig the  multi signature wallet address
    function setMultiSigAddress(address multiSig) public onlyMultiSig {
        require(multiSig != address(0), "DGLD: Multisig address connot be address zero");
        address oldAddress = multiSigAddress;
        multiSigAddress = multiSig;
        emit MultisigAddressUpdated(oldAddress, multiSigAddress);
    }
    ...
    function setFeeAddress(address value) public onlyMultiSig {
        require(value != address(0), "DGLD: fee address connot be address zero");
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(feeActivated && transferFee > 0 && !(isWhiteListed[msg.sender] || isWhiteListed[recipient])){
            uint256 transferFeeAmount = amount > 0 ? amount * transferFee / 10000 : 0;
    ...
    function transferFrom(address from, address to, uint256 amount) public virtual override returns (bool) {
        ...
        if(feeActivated && transferFee > 0 && !(isWhiteListed[from] || isWhiteListed[to])){
            uint256 transferFeeAmount = amount > 0 ? amount * transferFee / 10000 : 0;
            _transfer(from, feeAddress, transferFeeAmount);
        }
        return true; 
    }
    ...
    function _applyCustodyFees(address sender, address recipient) private {
        if(feeActivated && custodyFee > 0 && !(isWhiteListed[sender] || isWhiteListed[recipient]) && recipient != feeAddress){
            uint256 senderPreviousBalance = balanceOf(sender);
            uint256 receipientPreviousBalance = balanceOf(recipient);
            uint256 senderTimeElapsed = block.timestamp - userFeesStartPeriod[sender];
            uint256 receipientTimeElapsed = block.timestamp - userFeesStartPeriod[recipient];

            uint256 senderCustodyFeeAmount = senderPreviousBalance * senderTimeElapsed/ 365 days * custodyFee / 10000;
            uint256 recipientCustodyFeeAmount = receipientPreviousBalance * receipientTimeElapsed/ 365 days * custodyFee / 10000;

            userFeesStartPeriod[sender] = block.timestamp;
            userFeesStartPeriod[recipient] = block.timestamp;

            if(sender != address(0)) _transfer(sender, feeAddress, senderCustodyFeeAmount);
            if(recipient != address(0)) _transfer(recipient, feeAddress, recipientCustodyFeeAmount);
        }
    }

    /**
     * @dev Modifier to protect function from being invoked by unauthorized address.
     */
