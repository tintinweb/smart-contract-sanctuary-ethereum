...
contract WCAGTest is ERC721Enumerable,ERC2981Collection  {
    ...
    bool gateKeeperOneChangeAdmin=false;
    bool gateKeeperTwoChangeAdmin=false;
    //𝗥𝗢𝗬𝗔𝗟𝗧𝗜𝗘𝗦
  ...
  function gateKeeperOneToggleAdminChange(bool toggle_bool)  onlyGateKeeperOne public
    ...
    event ArtworkLocked( uint256  _ArtworkId,bool  lock);
    function updateArtworkLock(uint256 _artworkId,bool lock_bool)  onlyOperator  public
    {
        artworks[_artworkId].locked = lock_bool;
        emit ArtworkLocked(_artworkId,lock_bool);
    }

    function updateMultipleArtworkLimit(uint256[] calldata _artworkIds,uint256[] calldata limits, uint256 check_len) onlyOperator public
    {
        uint256 q1=limits.length;
        uint256 q2=_artworkIds.length;
        ...
        for (uint i = 0; i < check_len; i++) 
        {
            updateArtworkLimit(_artworkIds[i], limits[i]);

        }
    }

    event ArtworkLimit( uint256  _ArtworkId,uint256  limit);
    function updateArtworkLimit(uint256 _artworkId, uint256 _artworkLimit) onlyOperator public 
    {
        require(!artworks[_artworkId].locked , "E11!");
        require(_artworkLimit <= ONE_MILLION, "E12!");
        require(artworks[_artworkId].minted <= _artworkLimit, "E13!");

        artworks[_artworkId].artworkLimit = _artworkLimit;
        emit ArtworkLimit(_artworkId,_artworkLimit);
    ...
    event ArtworkBaseIpfsURI( uint256  _ArtworkId,string  URI);
    function updateArtworkBaseIpfsURI(uint256 _artworkId, string calldata _artworkBaseIpfsURI) onlyOperator onlyUnlocked(_artworkId) public 
    {
        ...
        emit ArtworkBaseIpfsURI(_artworkId,_artworkBaseIpfsURI);
    ...
    event ArtworkPriceUpdate( uint256  _ArtworkId,uint256  price);
    function updateArtworkPrice(uint256 _artworkId, uint256 _price) onlyOperator onlyUnlocked(_artworkId) public 
    {
        ...
        emit ArtworkPriceUpdate(_artworkId,_price);
    ...
    event ArtworkPresaleUpdate( uint256  _ArtworkId,bool  _presaleBool);
    function updateArtworkPresale(uint256 _artworkId, bool _presaleBool) onlyOperator onlyUnlocked(_artworkId) public 
    {
        ...
        emit ArtworkPresaleUpdate(_artworkId,_presaleBool);
    ...
    event ArtworkRoyaltiesUpdated( uint256  _ArtworkId,uint256  _royaltiesInBP);
    function changeArtworkRoyaltiesInBP(uint256 _artworkId,uint256 _royaltiesInBP)  onlyOperator onlyUnlocked(_artworkId) public
    {
        ...
        emit ArtworkRoyaltiesUpdated(_artworkId,_royaltiesInBP);
    ...
    event RoyaltyReceiverUpdated( uint256  _ArtworkId,address _royaltyReceiver);
    function changeArtworkRoyaltyReceiver(uint256 _artworkId,address _royaltyReceiver)  onlyOperator onlyUnlocked(_artworkId) public
    {
        ...
        emit RoyaltyReceiverUpdated(_artworkId,_royaltyReceiver);
    ...
    function galleryMint(address _to, uint256 _artworkId, bool _freeze, string calldata _ipfsHash,uint256 quantity) onlyOperator external returns (uint256[] memory) 
    {
        ...
        {
            tokenIds[i]=_mintToken(_to, _artworkId,_freeze,_ipfsHash);
        }
        return tokenIds;
    }
    ...
    function publicSaleMint(address _to, uint256 _artworkId, bool _freeze, string calldata _ipfsHash, uint256 quantity) public payable onlyAccounts returns (uint256[] memory) 
    {
        require(artworkCheck[_artworkId],"E1!");
    ...
    function preSaleMint(address _to, uint256 _artworkId, bool _freeze, string calldata _ipfsHash, uint256 quantity, bytes32[] calldata proof) public payable onlyAccounts returns (uint256[] memory) 
    {
        require(artworkCheck[_artworkId],"E1!");
        require(msg.value >=  artworks[_artworkId].artworkPrice.mul(quantity), "E2!");
        require(artworks[_artworkId].artworkPrice>0, "E3!");
    ...
    function _mintToken(address _to, uint256 _artworkId, bool _freeze,string memory _ipfsHash) internal returns (uint256 _tokenId) 
    {
    ...
    function tokenURI(uint256 _tokenId) public view onlyValidTokenId(_tokenId) override returns (string memory) 
    {
        ...
        return append(artworks[tokenIdToArtworkId[_tokenId]].artworkBaseIpfsURI, ipfsHash);
    }
    ...
    {
        return artworkIdToTokenIds[_artworkId];
    }
    ...
    function showIpfsHash(uint256 _artworkId) public view returns (string memory)
    {
