...
contract Stellar is ERC721, ERC721Metadata, Ownable {
    ...
    function checkRankBonus() internal {
        ...
        if(m_bonusDayTime > 0 && m_bonusDayMass > 0) {
            if(tNow >= m_bonusDayTime + DAY_IN_SECONDS || getWeekday(tNow) != getWeekday(m_bonusDayTime)) {
                // 达到发放条件
                m_mapBonus[m_bonusDayAddress] += 1;
                m_bonusDayTime = 0;
                m_bonusDayMass = 0;
            }
        }
        ...
        if(m_bonusWeekTime > 0 && m_bonusWeekMass > 0) {
            if(tNow / DAY_IN_SECONDS >= (m_bonusWeekTime / DAY_IN_SECONDS  +  7) ) {
                // 达到发放条件
                m_mapBonus[m_bonusWeekAddress] += 3;
                m_bonusWeekTime = 0;
                m_bonusWeekMass = 0;
            }
        }
    ...
    function _getTokenHexAppearance(uint256 uTokenId) internal view returns (string memory) {
        ...
        // byteString = abi.encodePacked(byteString,"0x");

        uint256 n256Appear = m_mapAppearance[uTokenId];
        // 先高128位
        uint256 n256HighAppear = n256Appear / (2**128);
        if(n256HighAppear > 0) {
            for(uint32 i = 0; i < u32Size ; ++i) {
                uint256 _oneAppear = (n256HighAppear & arrTrackBitFlag[u32Size- i - 1]) / arrTrackBitShift[u32Size- i - 1];
                byteString = abi.encodePacked(byteString, _oneAppear.toLast2HexString());
            }
            byteString = abi.encodePacked(byteString,"00000000");
        }
        // 低128位
        n256Appear = n256Appear & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
        for(uint32 i = 0; i < u32Size ; ++i) {
            uint256 _oneAppear = (n256Appear & arrTrackBitFlag[u32Size- i - 1]) / arrTrackBitShift[u32Size- i - 1];
            byteString = abi.encodePacked(byteString, _oneAppear.toLast2HexString());
        }
    ...
    function mint2(address to, uint256[] memory values) external onlyOwner {
        require(!m_bMintingFinalized, " minting is Finalized");
    ...
    function _transfer(address from, address to, uint256 uTokenId) internal {
        ...
        // bonus
        m_totalMergeNum += 1;
        if(0 == m_totalMergeNum % 5) {      // TODO: add 1 mass by every 5 merge
            uAddBonus += 1;
        }
