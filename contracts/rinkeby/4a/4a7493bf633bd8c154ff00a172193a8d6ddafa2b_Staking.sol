    ...
    function getReward(address account,uint id,uint poolType) public view returns (uint256) {
        ...
        if(poolType ==1){
           ...
           if(!hell[msg.sender][id][poolType]){
             ...
             totalDays=(block.timestamp - Stakers[msg.sender][id].lastUpdatedTime1)/ 1 minutes;
           }
            ...
            if(!hell[msg.sender][id][poolType]){
               ...
               totalDays=(block.timestamp - Stakers[msg.sender][id].lastUpdatedTime2)/ 1 minutes;
            }
            uint finalAmount = rewardStored[account]+amount * totalDays;
            return finalAmount;
        }
            ...
            if(!hell[msg.sender][id][poolType]){
             totalDays = (block.timestamp - Stakers[account][id].stakeTime)/ 1 minutes;
                ...
                totalDays=(block.timestamp - Stakers[msg.sender][id].lastUpdatedTime3)/ 1 minutes;
            }
            uint finalAmount = rewardStored[account]+amount * totalDays;
            return finalAmount;
