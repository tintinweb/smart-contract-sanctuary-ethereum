...
contract BullClub is ERC721 , ERC721Enumerable, Ownable {
    ...
    uint public  preMintPrice = 0.07 ether;
    uint public  MAX_Pre_Mint = 3;

    // Wallet Addresses
    address payable private bull_wallet;
    address payable private bull_lottery_wallet;
    ...
    constructor(string memory _uri,address payable _bull, address payable _bull_lottery_wallet, address payable _team_wallet) ERC721("Bull Runners", "BULL") {
        notRevealedUri = _uri;
        bull_wallet = _bull;
        bull_lottery_wallet = _bull_lottery_wallet;
    ...
    function changeBullWallet(address payable _address) external onlyOwner {
        bull_wallet = _address;
    }

    function changeBullLotteryWallet(address payable _address) external onlyOwner {
        bull_lottery_wallet = _address;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory) {
        ...
        if (reveal == false) {
            ...
            return string(abi.encodePacked("ipfs://",baseURI_revealed, "/", toString(tokenId)));
        }
    }
    ...
    function refMint(uint256 numberOfTokens, address refAddress) external payable {
        require(refAddress != address(0), "ERC721: mint to the zero address");
        require(mintStatus, "No sale for now");
        uint256 ts = totalSupply();
        uint256 total_price = mintPrice * numberOfTokens;
        ...
        uint ref_balance = total_price * 20 / 100;
        payable(refAddress).transfer(ref_balance);
    ...
    function preMint(bytes32[] calldata _merkleTree, uint256 numberOfTokens) external payable {
        require(premintStatus, "No presale for now");
        uint256 ts = totalSupply();
        ...
        // require(balanceOf(msg.sender) + numberOfTokens <= MAX_Pre_Mint, "Exceeded max token Mint");
        require(ts + numberOfTokens <= MAX_SUPPLY, "Purchase would exceed MAX Tokens Supply");
        require(preMintPrice * numberOfTokens <= msg.value, "Ether value sent is not correct");

        for (uint256 i = 0; i < numberOfTokens; i++) {
            WLaddressesMintedBalance[msg.sender]++;
            _safeMint(msg.sender, ts + i);
        }
    }
    ...
    function withdraw() public onlyOwner {
        ...
        uint bull_value = total_balance * 80 / 100;
        payable(bull_wallet).transfer(bull_value);

        uint bull_lottery_wallet_value = total_balance * 10 / 100;
        payable(bull_lottery_wallet).transfer(bull_lottery_wallet_value);

        uint team_wallet_value = total_balance * 10 / 100;
        payable(team_wallet).transfer(team_wallet_value);
    }
}