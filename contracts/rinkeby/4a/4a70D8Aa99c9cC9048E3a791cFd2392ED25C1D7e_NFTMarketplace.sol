...
contract NFTMarketplace is ReentrancyGuard, ERC1155Holder, ERC721Holder {
  ...
  function bid(uint256 tokenId) external payable nonReentrant {
    ...
    if (item.auctionInfo.highestBidder != address(0)) {
      ...
      item.auctionInfo.bids[item.auctionInfo.bids.length] = bidItem;
    }
  ...
  function endAuction(uint256 tokenId) external payable nonReentrant {
    MarketItem storage item = idMarketItemMapping[tokenId];
    // require(item.auctionInfo.startAt < block.timestamp, "You need to start first!");
  ...
  function createNFT1155(uint256 _id, uint256 _amount, string memory _tokenUri) public {
    _multiToken.mintNFT(msg.sender, _id, _amount, _tokenUri);
  }
