...
import "../extensions/royalties/IRoyaltiesRegistry.sol";
...
contract ERC721LA is IERC721LA, Initializable, AccessControl {
     ...
     *
     */
    function createEdition(string calldata _baseURI, uint48 _maxSupply, address _creator)
        public
        onlyMinter
        returns (uint256)
    {
        ...
        returns (uint256 supply)
    {
        ...
        uint256 totalEditionSupply = state._editionSupplies[editionId] - 1;

        Edition memory edition = state._editions[editionId];
        return totalEditionSupply - edition.burnedSupply;
    }
    ...
    function _transfer(
        ...
        // In this block we check that the token after the one being transfer
        // belong to the batch, if so it needs to be updated to the proper owner
        ...
        // This emulate a mint event on first transfer
