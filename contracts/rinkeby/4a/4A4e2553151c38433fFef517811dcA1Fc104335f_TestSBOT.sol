...
contract TestSBOT is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public saleDuration = (DUTCH_START_PRICE - DUTCH_END_PRICE) * STEP_TIME / STEP_PRICE;
    ...
    function getPrice() public view returns (uint256) {
        if (status == MintStatus.PRESALE || status == MintStatus.CLOSED) {
            ...
            if (elapsed < saleDuration) {
                uint256 price = DUTCH_START_PRICE.sub(
                    elapsed.mul(DUTCH_START_PRICE.sub(DUTCH_END_PRICE)).div(
                        saleDuration
                    )
                );
    ...
    function roundPrice(uint256 _price) private view returns(uint256){
        uint256 round = STEP_PRICE;
        uint256 result = (_price / round);
        if(result > 0) {
            result ++;
        }
        return result * round;
