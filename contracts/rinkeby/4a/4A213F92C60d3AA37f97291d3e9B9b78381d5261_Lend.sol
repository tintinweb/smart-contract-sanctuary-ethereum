...
contract Lend is IERC721Receiver, ReentrancyGuard {
  ...
  event Mint(address _veNFTAddress, uint256 _nftId, uint256 _cheeTokenAmount, uint256 _lockDurationBlocks, uint256 _tokenAmount, address minter);
  event Redeem(address _veNFTAddress, uint256 _nftId, uint256 _tokenAmount, address redeemer);
  event Liquidate(address _veNFTAddress, uint256 _nftId, address liquidator);
  ...
  function liquidate(address _veNFTAddress, uint256 _nftId) external nonReentrant onlyPermissioned {
    ...
    IERC20(supportedInterestToken[_veNFTAddress]).safeTransferFrom(msg.sender, treasuryAddress, _total);
    IVeNFT(_veNFTAddress).safeTransferFrom(address(this), msg.sender, _nftId);
    emit Liquidate(_veNFTAddress, _nftId, msg.sender);
  }
