...
contract MetaGodsToken is Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    event Withdrawal(address fromAddress, uint256 amount, uint256 internalIdentifier);
    event Deposit(address toAddress, uint256 amount);
    ...
    function _validateWithdrawSignature(uint256 internalIdentifier_, uint256 amount_, bytes calldata signature_) internal view {
        bytes32 dataHash = keccak256(abi.encodePacked(internalIdentifier_, msg.sender, amount_));
    ...
    function withdraw(uint256 internalIdentifier_, uint256 amount_, bytes calldata signature_) external whenNotPaused {
        _validateWithdrawSignature(internalIdentifier_, amount_, signature_);
        uint256 taxed = amount_ * withdrawTax / 100;
        withdrawTaxFunds += taxed;
        _mint(msg.sender, amount_ - taxed);
        emit Withdrawal(msg.sender, amount_, internalIdentifier_);
    }
