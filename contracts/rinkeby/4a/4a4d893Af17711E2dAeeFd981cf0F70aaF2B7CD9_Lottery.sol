...
//this gets remmaped into browni-config
...
contract Lottery is VRFConsumerBase , Ownable{
    ...
    function getEntranceFee() public view returns(uint256){
        ...
        uint256 adjustedPrice = uint256(price) * 10**10; //not sure of the math conversions going on here 
        uint256 costToEnter = (usdEntryFee* 10 **18) / adjustedPrice;
        return costToEnter;  
    }
    //this onlyOwner modifier is being used from import zepplen
    function startLottery() public onlyOwner{
        require(lottery_state == LOTTERY_STATE.CLOSED, "cant start a new lottery yet");
        lottery_state = LOTTERY_STATE.OPEN;
    }
    function endLottery() public onlyOwner{
        //keccack256 hashing algorithm , generating a random number (not a good way for prod)
        //abi.encodePacked lowlevel keyword
        ...
        //The better way below (Chainlink VRF)
        lottery_state = LOTTERY_STATE.CALCULATING_WINNER;
        //requestRandomess is inhereted function from VRF
