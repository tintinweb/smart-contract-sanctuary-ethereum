...
/// @title Hero
/// @notice Hero is a composable NFT designed to equip other ERC721 and ERC1155 tokens
contract Hero is
  ERC721Enumerable,
  ERC998TopDown,
  HeroInfoURIStorage,
  ...
  AccessControl
{
  ...
  function claimWithItems(ClaimHeroVoucher calldata voucher) external payable {
    ...
    _setHeroInfoURI(voucher.tokenId, voucher.heroInfoURI);
  ...
  function _transferItemIn(
    ...
    if (_itemAddress.supportsInterface(ERC_721_INTERFACE)) {
      ...
      require(
        false,
        "Hero: Item does not support ERC-721 nor ERC-1155 standards"
      );
    }
  }
  ...
  function _hashMintData(ClaimHeroVoucher calldata voucher)
    internal
    view
    returns (bytes32)
  {
  ...
  function _verifyMintData(ClaimHeroVoucher calldata voucher)
    internal
    view
    returns (address)
  {
  ...
  function onERC721Received(
    ...
    require(
      ...
      "Hero: Only the Hero contract can pull items in"
    );
...
contract ERC998TopDown is ERC721, IERC998ERC721TopDown, IERC998ERC1155TopDown {
    ...
    returns (uint256[] memory)
  {
    ...
    for (
      ...
      childTokenIds[i] = _childrenForChild721Contracts[tokenId][childContract]
        .at(i);
    }
...
abstract contract HeroInfoURIStorage is ERC721 {
  using Strings for uint256;

  mapping(uint256 => string) private _heroInfoURIs;

  function heroInfoURI(uint256 tokenId)
    public
    view
    virtual
    returns (string memory)
  {
    require(
      _exists(tokenId),
      "HeroInfoURIStorage: Hero Info URI query for nonexistent token"
    );

    return _heroInfoURIs[tokenId];
  }

  /**
   * @dev Sets `_heroInfoURI` as the heroInfoURI of `tokenId`.
   *
   * Requirements:
   *
   * - `tokenId` must exist.
   */
  function _setHeroInfoURI(uint256 tokenId, string memory _heroInfoURI)
    internal
    virtual
  {
    require(
      _exists(tokenId),
      "HeroInfoURIStorage: Hero Info URI set of nonexistent token"
    );
    _heroInfoURIs[tokenId] = _heroInfoURI;
  }
}
