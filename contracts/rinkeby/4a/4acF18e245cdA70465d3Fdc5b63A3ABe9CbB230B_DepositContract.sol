...
contract DepositContract is AccessControl, ReentrancyGuard, IDepositContract {
    ...
    function get_deposit_count() override external view returns (bytes memory) {
        return to_bytes8(uint64(deposit_count));
    }
    ...
    function deposit(
        ...
        // Emit `DepositEvent`
        bytes memory amount = to_bytes8(uint64(deposit_amount));
        emit DepositEvent(
            pubkey,
            amount,
            to_bytes8(uint64(deposit_count))
        );
    ...
    function to_bytes8(uint64 value) internal pure returns (bytes memory ret) {
        ret = new bytes(8);
