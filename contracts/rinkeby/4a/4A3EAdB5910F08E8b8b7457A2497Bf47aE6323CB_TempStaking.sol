  ...
  PausableUpgradeable 
{
  ...
  IERC721 public voxelsNFT;
  IERC20 public wireToken;

  uint256 public expiration;
  // 1 $WIRE per day = 173,611,111,111,111
  uint256 public rate;
  // Bonues rate for staking 3 different cubes
  ...
  function setBonusRate(uint256 _bonusRate) public onlyAdmin {
    bonusRate = _bonusRate;
  }
  ...
  {
    uint256 reward = 0;
    uint256[3] memory matchedCubes;
    uint256 lastBlockNumber;

    for (uint256 i; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      uint16 isDeposited = _deposits[account].contains(tokenId) ? 1 : 0;
      matchedCubes[(tokenId - 1) / 2121] += isDeposited;
      if (
        isDeposited == 1 && lastBlockNumber < _depositBlocks[account][tokenId]
      ) {
        lastBlockNumber = _depositBlocks[account][tokenId];
      }
    ...
    // Add bonus rewards for "3 different cubes" holders
    reward +=
      bonusRate *
      Math.min(Math.min(matchedCubes[0], matchedCubes[1]), matchedCubes[2]) *
      (Math.min(block.number, expiration) - lastBlockNumber);
  ...
  function claimRewards(uint256[] calldata tokenIds) public whenNotPaused {
    uint256 reward = calculateRewards(msg.sender, tokenIds);
