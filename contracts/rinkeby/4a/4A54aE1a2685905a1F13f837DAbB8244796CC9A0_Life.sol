...
contract Life is ERC20, Ownable {
  ...
  event StakingIitialization(address staker);

  constructor() ERC20("Life", "LIFE") {}

  function setManageLifeToken(address manageLifeToken_) external onlyOwner {
  ...
  function initStakingRewards(uint256 tokenId, address indexedAddress) external {
    require(address(_manageLifeToken) != address(0), "ManageLife token is not set");
    require(msg.sender == address(_manageLifeToken), "Only ManageLife token");
    _stakingRewards[tokenId] = uint64(block.timestamp);
    emit StakingIitialization(indexedAddress);
  }

  function claimableStakingRewards(uint256 tokenId) public view returns (uint256) {
    if (_stakingRewards[tokenId] == 0 || _manageLifeToken.fullyPayed(tokenId)) {
      return 0;
    }
    return (uint64(block.timestamp) - _stakingRewards[tokenId]) * _manageLifeToken.lifeTokenIssuanceRate(tokenId);
  }

  function batchClaimableStakingRewards(uint256[] memory tokenIds) external view returns (uint256) {
    uint256 claimable = 0;
  ...
  function claimStakingRewards(uint256 tokenId) public {
    require(address(_manageLifeToken) != address(0), "ManageLife token is not set");
    require(_manageLifeToken.ownerOf(tokenId) != owner(), "platformWallet cannot claim");
    _mint(_manageLifeToken.ownerOf(tokenId), claimableStakingRewards(tokenId));
    _stakingRewards[tokenId] = uint64(block.timestamp);
  }

  function batchClaimStakingRewards(uint256[] memory tokenIds) external {
    for (uint256 index; index < tokenIds.length; index++) {
...
contract ManageLife is ERC721, ERC721URIStorage, ERC721Burnable, Ownable {
  ...
  event RateUpdated(uint256 tokenId, uint256 rate);
  ...
  function setLifeTokenIssuanceRate(uint256 tokenId, uint256 lifeTokenIssuanceRate_) external {
    _lifeToken.claimStakingRewards(tokenId);
    _lifeTokenIssuanceRate[tokenId] = lifeTokenIssuanceRate_;
    emit RateUpdated(tokenId, lifeTokenIssuanceRate_);
  }
  ...
  function _baseURI() internal pure override returns (string memory) {
    return "https://managelifeapi.co/nft/";
  }

  function mint(string memory uri, uint256 lifeTokenIssuanceRate_) external onlyOwner {
  ...
  function approve(address to, uint256 tokenId) public override {
    require(fullyPayed(tokenId) || ownerOf(tokenId) == owner() || to == address(_marketplace), "Approval restricted");
  ...
  function _beforeTokenTransfer(
    ...
    require(
      fullyPayed(tokenId) || from == owner() || to == owner() || msg.sender == address(_marketplace),
      "Transfers restricted"
    );
    if (!fullyPayed(tokenId)) {
      if (from == owner()) {
        _lifeToken.initStakingRewards(tokenId, to);
      }
...
contract Marketplace is ReentrancyGuard, Pausable, Ownable {
  ...
  event Offered(uint256 indexed id, uint256 minValue, address indexed toAddress);
  event BidEntered(uint256 indexed id, uint256 value, address indexed fromAddress);
  event BidWithdrawn(uint256 indexed id, uint256 value);
  event BidCancelled(uint256 indexed id, uint256 value, address indexed bidder);
  event Bought(
    uint256 indexed id,
    uint256 value,
  ...
  function offerForSale(uint256 id, uint256 minSalePrice) external onlyMLifeOwner(id) whenNotPaused {
  ...
  /* Allows users to enter bids for any */
  ...
  function acceptBid(uint256 id, uint256 minPrice) external onlyMLifeOwner(id) whenNotPaused nonReentrant {
