...
contract BurnKing is ERC721A, Ownable {
	...
	constructor(address _publicKey) ERC721A('Burn King', 'BK') {
	...
	function burnEther(
		...
		// transferNftFromPool(Pools(_poolType));

		// emit BurnedEther(msg.sender, _usdCost, Pools(_poolType), _nftTypesCount[Pools(_poolType)]);
	...
	function burnERC20(
		...
		// try/catch burnFrom()
		// try IERC20Burnable(_tokenContractAddress).burnFrom(msg.sender, _tokenCount) returns() {
	...
	// function transferNftFromPool(Pools _poolType) internal {
	// function transferNftFromPool() internal {
	// console.log(_poolType);
	...
	function mint() internal {
		...
		emit tokenMintedFor(msg.sender, _currentIndex);
		...
		//   _nftIdTypes[_currentIndex] = Pools(tokenType);
