...
// order of operations: team mint, public mint, airdrop // todo: remove
contract TimelineTransitCompany is ERC721A, ERC2981, Ownable {
    ...
    uint256 public constant REBOOT_SUPPLY = 9_415;
    ...
    // An address can only mint 2 Timelines per transaction
    ...
    mapping(address => uint8) public mintCountPerAddress;
    ...
    */
    function publicMint(string calldata _nonce, uint _amount, bytes calldata _signature) external reentrancyGuard() {
      require(mintOpen, "Mint closed");
      require(msg.sender == tx.origin, "Real users only");
      require(_amount <= MAX_MINTS_PER_TXN, "Per txn amount exceeded");
      require(mintCountPerAddress[msg.sender] + _amount <= MAX_MINTS_PER_WALLET, "Wallet mints exceeded");
      require(totalSupply() + _amount <= REBOOT_SUPPLY, 'Exceeds reboot supply');
      require(!usedNonces[_nonce], "Nonce already used");

      require(isValidSignature(keccak256(abi.encodePacked(msg.sender, _amount, _nonce)), _signature), "Invalid signature");

      usedNonces[_nonce] = true;
      mintCountPerAddress[msg.sender] += uint8(_amount);
    ...
    */
    ...
    /**
    * @notice A one-time use function. Reserves 500 for the team for marketing, giveaways, etc.
    */
    function teamMint() external onlyOwner {
      require(!teamMinted, "Already minted");
      require(totalSupply() + 500 <= MAX_SUPPLY, 'Exceeds max supply');
    ...
    /**
    @notice Allows team to airdrop pieces to holders who minted the first colleciton before it was refunded
    */
    function airdrop(address[] calldata _addresses, uint8[] calldata _counts) external onlyOwner {
      require (_addresses.length == _counts.length, "Array mismatch");
      require (totalSupply() != MAX_SUPPLY, "Supply reached");

      // Sum the counts of the airdrops coming in, compare to totalSupply before proceeding
      uint16 countToBeAirdropped;
      for (uint8 i; i < _counts.length;) {
        countToBeAirdropped += _counts[i];
        unchecked { ++i; }
      }
      require(totalSupply() + countToBeAirdropped <= MAX_SUPPLY, "Supply exceeded");
