...
contract Market is IMarket, Ownable {
     ...
     * *********
     */
    modifier onlyApprovedOrOwnerorMedia(uint256 tokenId) {
        IERC721 tokenContract = IERC721(mediaAddress);
        address tokenOwner = tokenContract.ownerOf(tokenId);
        require(
            tokenContract.getApproved(tokenId) == msg.sender ||
                msg.sender == tokenOwner ||
                msg.sender == mediaAddress ||
                tokenContract.isApprovedForAll(tokenOwner, msg.sender),
            "Market: only approved or owner or media"
        );
        _;
    }
    ...
    modifier onlyValidCaller(address caller) {
        require(
            caller == msg.sender || mediaAddress == msg.sender,
    ...
    modifier onlyExistingToken(uint256 tokenId) {
        require(
            IERC721(mediaAddress).ownerOf(tokenId) != address(0),
    ...
    function bidForTokenBidder(uint256 tokenId, address bidder)
        ...
        returns (Bid memory)
    {
        return _tokenBids[tokenId][bidder];
    }
    ...
    function setMediaContractAddress(address _mediaContractAddress)
        external
        override
        onlyOwner
    {
        require(_mediaContractAddress != address(0), "invalid address");
        mediaAddress = _mediaContractAddress;
    }
    ...
    function setAsk(
        ...
        uint256 tokenId,
        Ask memory ask
    ) public override onlyApprovedOrOwnerorMedia(tokenId) onlyValidCaller(tokenOwner) {
        require(ask.amount != 0, "Market: 0 amount");
        _tokenAsks[tokenId] = ask;
        emit AskCreated(tokenId, ask.amount, ask.currency);
    }

    /**
     * @notice removes an ask for a token and emits an AskRemoved event
     */
    function removeAsk(address tokenOwner, uint256 tokenId)
        public
        override
        onlyApprovedOrOwnerorMedia(tokenId)
    ...
    function setBid(
        ...
        Bid memory bid,
        address spender
    ) public override onlyExistingToken(tokenId) {
        ...
        Bid storage existingBid = _tokenBids[tokenId][bid.bidder];
        ...
        _tokenBids[tokenId][bid.bidder] = Bid(
            afterBalance.sub(beforeBalance),
            bid.currency,
            bid.bidder,
            bid.recipient
        );
        emit BidCreated(tokenId, bid.bidder, bid.amount, bid.currency);
     ...
     */
    function removeBid(uint256 tokenId, address bidder) public override {
        require(bidder == msg.sender, "Market: bidder should be the caller");

        Bid storage bid = _tokenBids[tokenId][bidder];
        ...
        emit BidRemoved(tokenId, bid.bidder);
    ...
    function acceptBid(
        ...
        onlyValidCaller(tokenOwner)
    {
        Bid memory bid = _tokenBids[tokenId][expectedBid.bidder];
    ...
    function _finalizeNFTTransfer(uint256 tokenId, address bidder) private {
        ...
        if (discount[bid.currency]) {
            feeAmount = (bid.amount * discountFee) / 100;
        } else {
            feeAmount = (bid.amount * fee) / 100;
        }
        uint256 transferAmount = bid.amount - feeAmount;
        address tokenOwner = IERC721(mediaAddress).ownerOf(tokenId);

        token.safeTransfer(tokenOwner, transferAmount);
        token.safeTransfer(feeAddress, feeAmount);

        // Transfer media to bid recipient
        IERC721(mediaAddress).safeTransferFrom(tokenOwner, bid.bidder, tokenId);

        // Remove the accepted bid
        delete _tokenBids[tokenId][bidder];
        // Remove the token ask
        delete _tokenAsks[tokenId];

        emit BidFinalized(tokenId, bid.bidder, bid.amount, bid.currency);
    }
    ...
    function setDiscountFee(uint256 _discountFee) external override onlyOwner {
        discountFee = _discountFee;
    }

    function addCurrencyToDiscount(address currency, bool addOrRemove)
        external
        override
        onlyOwner
    {
        discount[currency] = addOrRemove;
    }
...
interface IMarket {
    ...
    event AskCreated(uint256 indexed tokenId, uint256 amount, address currency);
    event AskRemoved(uint256 indexed tokenId);

    // event MintCreated(uint256 indexed tokenId, Ask ask,address mediaContract);
    ...
    // function setMintingPrice(address mediaContract,uint256 tokenId, Ask memory ask) external;
    ...
    function removeAsk(address tokenOwner, uint256 tokenId) external;
    ...
    function addCurrencyToDiscount(address currency, bool addOrRemove) external;

    function setDiscountFee(uint256 _discountFee) external;

    function setFee(uint256 _fee) external;

    function setMediaContractAddress(address _mediaContractAddress) external;
}
