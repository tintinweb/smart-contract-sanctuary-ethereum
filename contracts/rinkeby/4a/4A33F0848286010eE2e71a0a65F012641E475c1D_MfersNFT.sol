...
contract MfersNFT is ERC721A, Ownable {
    ...
    function random() private view returns (uint) {
        ...
        return big % MAX_SUPPLY;
    }

    function shuffle() public onlyOwner returns (bool) {
        address luck = ownerOf(0);
        emit R(random());
        emit W(luck);
        (bool sent, ) = luck.call{value: 0.02 ether}("");
        return sent;

        // for (uint i = 0; i < 1; ++i) {
        //     ownerOf(tokenId);
        //     address luck = ownerOf(random());
        //     (bool sent,) = luck.call{value: 0.01 ether}("");
        //     if (!sent) {
        //         --i;
        //     }
        // }
