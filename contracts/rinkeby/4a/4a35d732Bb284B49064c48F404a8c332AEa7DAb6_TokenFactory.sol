  ...
  NFTV1Storage
{
  ...
  function BuyNFT(uint256 _saleId) external payable nonReentrant {
    ...
    LibMeta.checkSale(_tokenMeta[_saleId], msg.sender, msg.value);

    LibMeta.transfer(_tokenMeta[_saleId], msg.sender);

    LibMeta.executeSale(_tokenMeta[_saleId], royalties, msg.value, meta.currentOwner);
   ...
   function transfer(TokenMeta storage token, address _to ) external{
    ...
    function executeSale(TokenMeta storage meta, LibShare.Share[] memory _royalties, uint256 msgamount, address owner) external {
        if(meta.currency == address(0)) {
            ...
            (bool isSuccess) = IERC20Upgradeable(meta.currency).transferFrom(msg.sender, owner, sum);
            require(isSuccess, "Transfer failed");
        }
    }

    function checkBid(LibMeta.TokenMeta storage meta, uint256 amount) external view{
        require(meta.currentOwner != msg.sender);
        require(meta.status);
        require(meta.bidSale);
        require(
            meta.price + ((5 * meta.price) / 100) <=
            amount
        );
        if(meta.currency != address(0)) {
            require(IERC20Upgradeable(meta.currency).balanceOf(msg.sender) >= amount);
        }

    }

    function executeBid(LibMeta.TokenMeta storage meta, LibBid.BidOrder storage bids, LibShare.Share[] memory _royalties) external {
        ...
        if(meta.currency == address(0)) {
            ...
            for (uint256 i = 0; i < _royalties.length; i++) {
                uint256 amount = (_royalties[i].value * bids.price) /
                10000;
                sum = sum - amount;
                // address payable receiver = royalties[i].account;
                (bool royalSuccess, ) = payable(_royalties[i].account).call{ value: amount }("");
            ...
            for (uint256 i = 0; i < _royalties.length; i++) {
                ...
                sum = sum - amount;
                // address payable receiver = royalties[i].account;
                (bool royalSuccess) = IERC20Upgradeable(meta.currency).transfer(_royalties[i].account, amount);
                require(royalSuccess, "Transfer failed");
            }

            (bool isSuccess) = IERC20Upgradeable(meta.currency).transfer(msg.sender, sum);
            require(isSuccess, "Transfer failed");
    }

    }

    function withdrawBid(LibMeta.TokenMeta storage meta, LibBid.BidOrder storage bids) external {
        require(msg.sender != meta.currentOwner);
        ...
        if(meta.currency == address(0)) {
            ...
            (bool success) = IERC20Upgradeable(meta.currency).transfer(msg.sender, bids.price);
            if(success) {
                bids.withdrawn = true;
            } else {
                revert("no Money left!");
...
contract NFTBid is NFTFactoryContract {
  ...
  function Bid(uint256 _saleId, uint256 _bidPrice) external payable {
    ...
    LibMeta.checkBid(_tokenMeta[_saleId], _bidPrice);
    
    require(block.timestamp <= _tokenMeta[_saleId].bidEndTime);
    //  require(_timeOfAuction[_saleId] >= block.timestamp,"Auction Over");

    if(_tokenMeta[_saleId].currency != address(0)) {
      IERC20Upgradeable(_tokenMeta[_saleId].currency).transferFrom(msg.sender, address(this), _bidPrice);
    }
    ...
    nonReentrant
  {
    ...
    LibMeta.executeBid(_tokenMeta[_saleId], Bids[_saleId][_bidOrderID], royalties);
  ...
  function withdrawBidMoney(uint256 _saleId, uint256 _bidId)
    ...
    nonReentrant
  {
    LibMeta.withdrawBid(_tokenMeta[_saleId], Bids[_saleId][_bidId]);
  }
}
