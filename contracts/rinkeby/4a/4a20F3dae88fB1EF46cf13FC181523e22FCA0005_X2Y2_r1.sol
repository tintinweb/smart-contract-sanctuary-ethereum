    ...
    IX2Y2Run
{
    ...
    {
        require(
            input.shared.deadline > block.timestamp,
            "input deadline reached"
        );
        ...
        for (uint256 i = 0; i < input.details.length; i++) {
            ...
            if (input.shared.canFail) {
                try
                    IX2Y2Run(address(this)).run1(order, input.shared, detail)
                returns (uint256 ethPayment) {
    ...
    function _run(
        ...
        if (detail.op == Market.Op.COMPLETE_SELL_OFFER) {
            ...
            require(
                detail.executionDelegate.executeSell(
                    order.user,
                    shared.user,
                    data
                ),
