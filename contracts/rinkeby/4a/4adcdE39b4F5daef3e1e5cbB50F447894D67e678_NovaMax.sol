...
contract NovaMax is ERC20, Ownable {
    ...
    *
     */
    function status() public view returns(uint8 status_) {
    ...
    function withdraw(address to) external onlyOwner {
        require(status() == 2, "NovaMax: can only be withdrawn after the fundraising is completed");
        require(getBalance() > 0, "NovaMax: the balance is zero");
        ...
        if(_targetIsETH) {
            ...
            _targetToken.safeTransfer(to, getBalance());
        }
    }
    ...
    function subscriptionByETH() external payable {
        ...
        uint256 shares = msg.value.mul(_exchangeRate);
    ...
    function subscriptionByToken(uint256 amount) external {
        ...
        uint256 shares = amount.mul(_exchangeRate);
        _transfer(address(this), msg.sender, shares);
    ...
    function redemption() external {
        ...
        uint256 scale = 10**18;
        uint256 rate = rateOfReturn.add(scale);
        uint256 amountWithProfit = shares.div(_exchangeRate).mul(rate).div(scale);
