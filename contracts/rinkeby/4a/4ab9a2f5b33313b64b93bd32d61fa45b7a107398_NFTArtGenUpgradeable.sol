...
// File @openzeppelin/contracts-upgradeable/utils/ContextUpgrade[email protected]
...
contract NFTArtGenUpgradeable is
  ...
  function __NFTArtGen_init(
    ...
    updateSale(_open, _cost, maxPerWallet, maxPerMint);
  ...
  function airdrop( address[] memory _recipients, uint256[] memory _amount) public onlyOwner {
    ...
    for (uint256 i = 0; i < _amount.length; i++) {
      require(totalSupply() + _amount[i] <= maxSupply, "reached max supply");
    ...
    returns (string memory)
  {
    ...
    if (revealed == false) {
      return
        string(
          abi.encodePacked(uriNotRevealed, StringsUpgradeable.toString(_tokenId), ".json")
        );
    }

    return
      string(abi.encodePacked(baseUri, StringsUpgradeable.toString(_tokenId), ".json"));
  }
  ...
  modifier preMintChecks(uint256 count) {
    ...
    require(totalSupply() + count <= maxSupply + 1, "reached max supply");
    require(_numberMinted(msg.sender) + count <= maxPerWallet, "can not mint this many");
