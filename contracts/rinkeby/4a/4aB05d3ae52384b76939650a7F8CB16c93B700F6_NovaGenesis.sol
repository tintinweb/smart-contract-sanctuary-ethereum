...
contract NovaGenesis is IERC721, Proxy, Functional {
    ...
    uint256 private _revealedTokens;  //for URI redirects
    ...
    mapping(address => uint256[]) public enumeration;
     ...
     */
    constructor() {
        _name = "Nova Genesis Pass";
        _symbol = "NGP";
        _baseURI = "https://website/metadata/";
        _postURI = ".json";
        
        totalTokens = 5000;
        price = 120 * (10 ** 15); // Replace leading value with price in finney
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == NovaGenesis.onERC721Received.selector;
    }
    ...
    //Functionality for possible exchange or contract swap functionality
    function adminBurn(uint256 tokenId) external proxyAccess {
    	_burn(tokenId);
    }
    
    //Functionality for future staking contract
    function adminTransfer(address from, address to, uint256 tokenId) external proxyAccess {
    ...
    function airDrop(address[] memory _to) external onlyOwner {
        ...
        for(uint256 i = 0; i < qty; i++) {
            _safeMint(_to[i], numberMinted + i);
        }
        numberMinted += qty;
    }
    ...
    function mint(uint256 qty) external payable reentryLock {
        ...
        require(msg.value == qty * price, "Mint: Wrong Price");
    ...
    function ownerHoldings(address holder) external view returns ( uint256[] memory) {
    	return enumeration[holder];
    }
    
    function _addHolding(address holder, uint256 tokenId) internal {
    	enumeration[holder].push(tokenId);
    }
    
    function _removeHolding(address holder, uint256 tokenId) internal {
    	uint256 index = 99999999;
    	uint256 length = enumeration[holder].length;
    	
    	for (uint256 i; i < length; i++){
    		if (index > numberMinted){
    			if (enumeration[holder][i] == tokenId) {
                    index = i;
    			    if (i+1 < length) {
    				    enumeration[holder][i] = enumeration[holder][i + 1];
    			    }                    
                }
    		} else {
    			if (i+1 < length) {
    ...
    function setPostURI(string memory newPost) public onlyOwner {
    	_postURI = newPost;
    }
    ...
    function revealTokens(uint256 maxTokenId) external onlyOwner {
        _revealedTokens = maxTokenId;
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual {
    	//MODIFIED TO ALLOW ENUMERATION
        if(from != address(0)){
            _removeHolding(from, tokenId);
        }
        if(to != address(0)){
            _addHolding(to, tokenId);
        }
    }
