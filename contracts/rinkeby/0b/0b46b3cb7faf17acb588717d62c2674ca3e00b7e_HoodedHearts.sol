...
contract HoodedHearts is ERC721, PaymentSplitter, Ownable {
    // Setup
    ...
    address private teamWallet = 0x66BaA982E93353Bb6A1302b39586F01cB0e8De25;
    ...
    constructor(address[] memory _payees, uint256[] memory _shares) ERC721("Hooded Hearts", "HH") PaymentSplitter(_payees, _shares) {
        _mintCards(teamWallet, 5);
    }
    ...
    function allowListMint(bytes32[] calldata _merkleProof, uint _amount) external payable isNotPaused(allowListMintEnabled) {
        ...
        require(totalSupply() + _amount < 6_667, 'Exceeds max supply');
    ...
    function mint(uint _amount) external payable isNotPaused(mintEnabled) {
        require((_amount > 0 && _amount < 10), "Wrong amount");
        require(totalSupply() + _amount < 6_667, 'Exceeds max supply');
        require(price * _amount == msg.value, "Wrong ETH amount");
