...
contract Moonwalkers is ERC721A, Pausable, Ownable, ReentrancyGuard {
  ...
  /// @dev Mapping of the allowlist addresses and the amount of NFT they can mint.
  ...
  function allowlistMint(uint256 _mintAmount) external payable whenNotLocked callerIsUser {
    SaleConfig memory config = saleConfig;
    ...
    require(_mintAmount < allowList[msg.sender].add(1), "You cannot mint that much.");
  ...
  /// @dev Sets the allowList. To run twice : once for the whitelisted and once for the alpha.
  function setAllowList(address[] calldata addresses, uint256 _maxWhitelistMintPerWallet) external onlyOwner {
    for (uint256 i = 0; i < addresses.length; i++) {
        allowList[addresses[i]] = _maxWhitelistMintPerWallet;
    }
  }
