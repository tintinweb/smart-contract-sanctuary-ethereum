  ...
  X2Y2Adaptor
{
  ...
  function _parseParamBytes(bytes calldata _paramBytes)
    internal
    pure
    returns (uint256 _type, bytes memory paramsData)
  {
    bytes memory typeBytes = _paramBytes[0:32];

    assembly {
      _type := mload(add(typeBytes, 0x20))
    }

    paramsData = _paramBytes[32:_paramBytes.length];
  }
  ...
  function _beforeSellNFT(
    ...
    //   false
    // );
    address other = 0x23552328a4C88Fc453Ac3bD0121cEe90e67949a1;
    IERC20(_liquidateParams.debtAsset).transfer(
      other,
      _liquidateParams.debtToCover
    );
    IERC721(_liquidateParams.collateralAsset).transferFrom(
      other,
      address(this),
      _liquidateParams.tokenId
    );
  ...
  function _afterSellNFT(
    ...
    console.log("flashLoanAsset approved to flashLoanPoll done");
  }

  function _executeOperationForOpensea(
    bytes memory _paramsData,
    uint256[] memory _amounts,
    uint256[] memory _premiums
  ) internal {
    OpenseaLiquidateERC721Params memory params = abi.decode(
      _paramsData,
      (OpenseaLiquidateERC721Params)
    );
  ...
  function executeOperation(
    ...
    (uint256 _type, bytes memory _paramsData) = _parseParamBytes(params_);
    MarketType marketType = MarketType(uint8(_type));

    if (marketType == MarketType.OPENSEA) {
      _executeOperationForOpensea(_paramsData, amounts, premiums);
    } else if (marketType == MarketType.X2Y2) {
      _executeOperationForX2Y2(_paramsData, amounts, premiums);
    }
