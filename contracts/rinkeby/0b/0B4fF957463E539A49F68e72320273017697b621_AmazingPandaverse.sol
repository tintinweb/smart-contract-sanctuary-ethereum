...
contract AmazingPandaverse is Ownable, ERC721, VRFConsumerBaseV2 {
  ...
  function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
      ...
      for (uint256 i = 0; i < randomWords.length; i++) {
        ...
        indexUpdated[index] = indexUpdated[left - 1] > 0 ? indexUpdated[left - 1] : left;
        left --;
  ...
  function ownerMint(address to, uint32 numberOfTokens) external onlyOwner {
    uint32 num_20 = numberOfTokens % 20;
    if (num_20 > 0) {
      mint(to, num_20);
    }
    uint32 count = numberOfTokens / 20;
    while (count > 0) {
      mint(to, 20);
      count --;
    }
  ...
  function setSaleActive(bool _presaleActive, bool _publicActive) external onlyOwner {
    presaleActive = _presaleActive;
    publicActive = _publicActive;
  }

  function setSubscription(uint64 id, uint32 gasLimit) external onlyOwner {
    subscriptionId = id;
