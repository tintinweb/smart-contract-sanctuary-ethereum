...
/** 
 *  SourceUnit: /Users/xuezhao/Documents/hardhat_space/double/contracts/market/Market.sol
*/
...
interface IComplexDoNFT is IBaseDoNFT,IRoyalty {
    ...
    function initialize(string memory name_, string memory symbol_,address nftAddress_,address market_,address tempRoyaltyAdmin_) external ;
...
interface IMarket {
    ...
    event CreateLendOrder(address lender,address nftAddress,uint256 nftId,uint64 maxEndTime,uint64 minDuration,uint256 pricePerDay,address paymentToken);
    event CancelLendOrder(address lender,address nftAddress,uint256 nftId);
    event FulfillOrder(address renter,address lender,address nftAddress,uint256 nftId,uint64 startTime,uint64 endTime,uint256 pricePerDay,uint256 newId,address paymentToken);
    ...
    function fulfillOrder(address nftAddress,uint256 tokenId,uint256 durationId,uint64 startTime,uint64 endTime,address user) external payable returns(uint256 tid);

    function fulfillOrderNow(address nftAddress,uint256 tokenId,uint256 durationId,uint64 duration,address user) external payable returns(uint256 tid);
...
contract Market is OwnableContract,ReentrancyGuardUpgradeable,IMarket{
    ...
    function onlyApprovedOrOwner(address spender,address nftAddress,uint256 tokenId) internal view {
        address _owner = ERC721(nftAddress).ownerOf(tokenId);
        require(spender == _owner || ERC721(nftAddress).getApproved(tokenId) == spender || ERC721(nftAddress).isApprovedForAll(_owner, spender),"only approved or owner");
    }
    ...
    function mintAndCreateLendOrder (
        ...
        if(!supportERC20){
            require(paymentToken==address(0),"not support ERC20 now");
        }
        address oNftAddress = IComplexDoNFT(doNftAddress).getOriginalNftAddress();
        if(IERC165(oNftAddress).supportsInterface(type(IWrapNFT).interfaceId)){
            address gameNFTAddress = IWrapNFT(oNftAddress).originalAddress();
            bool isStaked = ERC721(gameNFTAddress).ownerOf(oNftId) == oNftAddress;
            if(isStaked){
        ...
        uint256 nftId = IComplexDoNFT(doNftAddress).mintXNft(oNftId);
        createLendOrder(doNftAddress, nftId, maxEndTime,minDuration, pricePerDay,paymentToken);
    }
    ...
    function createLendOrder(
        ...
        require(paymentToken==address(0) || IERC165(paymentToken).supportsInterface(type(IERC20).interfaceId),"paymentToken is not ERC20");
        require(maxEndTime > block.timestamp,"invalid maxEndTime");
        require(minDuration <= IComplexDoNFT(nftAddress).getMaxDuration(),"Error:minDuration > max");
        require(IERC165(nftAddress).supportsInterface(type(IComplexDoNFT).interfaceId),"not doNFT");
        (,,uint64 dEnd) = IComplexDoNFT(nftAddress).getDurationByIndex(nftId, 0);
        if(maxEndTime > dEnd){maxEndTime = dEnd;}
        if(maxEndTime > block.timestamp + maxIndate){maxEndTime = uint64(block.timestamp) + maxIndate;}
        ...
        emit CreateLendOrder(_owner,nftAddress, nftId, maxEndTime,minDuration,pricePerDay,paymentToken);
    }

    function cancelLendOrder(address nftAddress, uint256 nftId) public whenNotPaused {
        onlyApprovedOrOwner(msg.sender,nftAddress,nftId);
    ...
    function fulfillOrder(address nftAddress,uint256 tokenId,uint256 durationId,uint64 startTime,uint64 endTime,address user) public whenNotPaused nonReentrant payable virtual returns(uint256 tid){
        ...
        tid = IComplexDoNFT(nftAddress).mint(tokenId, durationId, startTime, endTime, msg.sender);
        emit FulfillOrder(msg.sender, lending.lender, lending.nftAddress, lending.nftId, startTime, endTime, lending.pricePerDay,tid,lending.paymentToken);
    }

    function fulfillOrderNow(address nftAddress,uint256 tokenId,uint256 durationId,uint64 duration,address user) public whenNotPaused nonReentrant payable virtual returns(uint256 tid){
        require(isLendOrderValid(nftAddress,tokenId),"invalid order");
        Lending storage lending = creditMap[nftAddress].lendingMap[tokenId];
    ...
    function distributePayment(address nftAddress,uint256 nftId,uint64 startTime,uint64 endTime) internal returns (uint256 totalPrice,uint256 leftTotalPrice,uint256 curFee,uint256 curRoyalty){
        Lending storage lending = creditMap[nftAddress].lendingMap[nftId];
        totalPrice = lending.pricePerDay * (endTime - startTime + 1) / 86400;
        curFee = totalPrice * fee / E5;
        curRoyalty = totalPrice * IComplexDoNFT(nftAddress).getRoyaltyFee() / E5;
        leftTotalPrice = totalPrice - curFee - curRoyalty;

        royaltyMap[nftAddress].balance[lending.paymentToken] += curRoyalty;
        balanceOfFee[lending.paymentToken] += curFee;
        ...
        if(lending.paymentToken==address(0)){
            ...
            SafeERC20.safeTransferFrom(IERC20(lending.paymentToken), msg.sender, address(this), totalPrice);
            SafeERC20.safeTransferFrom(IERC20(lending.paymentToken), address(this),ERC721(nftAddress).ownerOf(nftId),leftTotalPrice);
        }
    }
    ...
    function claimFee(address[] calldata paymentTokens) public whenNotPaused nonReentrant{
        ...
        for (uint256 index = 0; index < paymentTokens.length; index++) {
            ...
            if(balance > 0){
                if(paymentTokens[index]== address(0)){
                    ...
                    SafeERC20.safeTransferFrom(IERC20(paymentTokens[index]), address(this),beneficiary,balance);
                }
    ...
    function balanceOfRoyalty(address nftAddress,address paymentToken) public view returns(uint256){
        return royaltyMap[nftAddress].balance[paymentToken];
    }

    function claimRoyalty(address nftAddress,address[] calldata paymentTokens) public whenNotPaused nonReentrant{
        address payable _beneficiary = IComplexDoNFT(nftAddress).getBeneficiary();
        require(msg.sender==_beneficiary,"not beneficiary");
        for (uint256 index = 0; index < paymentTokens.length; index++) {
            uint256 balance = royaltyMap[nftAddress].balance[paymentTokens[index]];
            if(balance > 0){
                if(paymentTokens[index]== address(0)){
                _beneficiary.transfer(balance);
                }else{
                    SafeERC20.safeTransferFrom(IERC20(paymentTokens[index]), address(this),_beneficiary,balance);
                }
                royaltyMap[nftAddress].balance[paymentTokens[index]] = 0;
            }
        }
    }
    ...
    function setSupportERC20(bool value)public onlyAdmin{
        require(value != supportERC20);
        supportERC20 = value;
    }
