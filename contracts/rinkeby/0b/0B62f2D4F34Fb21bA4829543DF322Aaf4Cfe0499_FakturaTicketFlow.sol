...
UUPSUpgradeable
{
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721AUpgradeable) returns (string memory) {
        ...
        uint256 category = tokenCategory[tokenId];
        if(categories[category].revealed) {
            return string(abi.encodePacked(categories[category].baseURI, _toString(tokenId), ".json"));
        } else {
            return string(abi.encodePacked(categories[category].baseURI, "unrevealed.json"));
        }
    }
    ...
    function safeMint(address to, uint8 amount, uint256 sales, uint256 category, bytes32 r, bytes32 s, uint8 v) public payable whenNotPaused {
        ...
        require(_totalMinted() + amount <= categories[category].tokenSupply - categories[category].mintReserve, "Collection soldout.");
        ...
        _setTokenURI(ERC721AStorage.layout()._currentIndex - 1, category);
    }
     ...
     * - `tokenId` must exist.
     */
    function _setTokenURI(uint256 tokenId, uint256 category) internal virtual {
    ...
    function safeReserveMint(address to, uint256 amount, uint256 sales, uint256 category) public onlyRole(ADMIN_ROLE) {
        require(_totalMinted() + amount < categories[category].tokenSupply, "There's no token to mint.");
        _safeMint(to, amount, sales);
    }
