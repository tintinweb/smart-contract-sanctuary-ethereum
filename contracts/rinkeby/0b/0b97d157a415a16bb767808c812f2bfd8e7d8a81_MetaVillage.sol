...
// File: contracts/MetaVillage.sol
...
contract MetaVillage is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor( string memory _setBaseURI ) ERC721( "MetaVillage", "MVIL" ) {
    ...
    // mapping to check if presale is giveaway
    function giveAwayMint(address to, uint256 amount) public onlyOwner {
        
        require(_amountMinted + amount <= _maximumSupply, "METAVILLAGE: Mint will exceed maximum supply.");
        
        _balances[to] += amount;
        _giveaways[to] += amount;

        for( uint256 i = 1; i <= amount; i++ ){
            _safeMint(to, _amountMinted + i);
        }

        emit TokenMint(to, amount, 0);
