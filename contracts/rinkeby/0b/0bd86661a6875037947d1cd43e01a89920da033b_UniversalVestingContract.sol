...
contract UniversalVestingContract is Ownable {
...
//        setPauseStatus(true);
    ...
    mapping(uint=>uint) public uintPerDay;
    ...
    uint[] public linearVestingTimePeriod=[0,39600,32400,21900,10800,54000,65700,63000,300,81000,28800];
    ...
    function addInvestors(Investor[] memory vest) external onlyOwner {
        for (uint i = 0;i < vest.length;i++) {
            ...
            vesting.linearToBeClaimed = (linearVestingAmountReleased[vest[i].investorType] * _amount ) / 1000;
             uintPerDay[vest[i].investorType]= vesting.linearToBeClaimed/(linearVestingTimePeriod[vest[i].investorType]/60);
