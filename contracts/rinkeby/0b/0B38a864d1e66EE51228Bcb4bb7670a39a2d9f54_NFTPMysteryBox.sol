...
contract NFTPMysteryBox is ERC1155, ReentrancyGuard, Ownable {
    ...
    * @param account box owner
    * @param id collection id
    * @param amount number of boxes to reveal
    * @param seed random number provided by admin
    */
    function revealFulfill(address account, uint256 id, uint256 amount, uint256 seed, uint256 counter) external onlyAdmin {
        MysteryBox.Collection storage c = collections[id];
        require(c.revealable(), "Cannot be revealed yet");
        require(balanceOf(account, id) >= amount, "Not enough MysteryBoxes");

        uint16[] memory rarities = new uint16[](amount);
        uint16[] memory photos = new uint16[](amount);
        ...
        uint256 start = nftp.mintBatch(account, id, rarities, photos);
        uint256[] memory nftIds = new uint256[](amount);
        for(uint256 i=0; i<amount; i++) {
            nftIds[i] = start + i;
        }
        emit Reveal(id, nftIds, rarities, photos, account, counter);
    ...
    */
    function reveal(uint256 id, uint256 amount) external {
        ...
        emit RevealRequest(msg.sender, id, amount, nonceCounter, block.timestamp);
    }
    ...
    */
    ...
    function _reveal(uint256 id, uint16 photoCount, uint16 photoRand, uint16 rarityRand) internal nonReentrant returns(RarityPhoto memory rp) {
        ...
        while(gasUsed == 0 || gasUsed < gasleft() - GAS_BUFFER) {
