...
contract AuctionManager {
    ...
    mapping(uint => uint) public auctionstoken; // auctions

    // create an auction
    function createAuction(uint256 _endTime, uint256 _minIncrement, uint256 _directBuyPrice,uint256 _startPrice,address _nftAddress,uint256 _tokenId)  external returns (bool){
        ...
        require(_endTime > 100); // end time must be greater than 5 minutes (setting it to 5 minutes for testing you can set it to 1 days or anything you would like)
        _minIncrement=_minIncrement;
        _directBuyPrice=_directBuyPrice;
        ...
        auctionstoken[_tokenId]=auctionId;
        return true;
    }
    ...
    function getAuctionByToken(uint tokenid)  external
        view
        returns (
            uint256   directBuy,
            address   owner,
            ...
            uint256 auctionid = auctionstoken[tokenid];
            directBuy  = Auction(auctions[auctionid]).directBuyPrice(); // get the direct buy price
            owner  = Auction(auctions[auctionid]).creator(); // get the owner of the auction
            highestBid  = Auction(auctions[auctionid]).maxBid(); // get the highest bid
            tokenIds  = Auction(auctions[auctionid]).tokenId(); // get the token id
            endTime  = Auction(auctions[auctionid]).endTime(); // get the end time
            startPrice  = Auction(auctions[auctionid]).startPrice(); // get the start price
            auctionState  = uint(Auction(auctions[auctionid]).getAuctionState()); // get the auction state
...
contract Auction {
    ...
    // Auction constructor
    constructor(address _creator,uint256 _endTime,uint256 _minIncrement,uint256 _directBuyPrice, uint256 _startPrice,address _nftAddress,uint256 _tokenId){
