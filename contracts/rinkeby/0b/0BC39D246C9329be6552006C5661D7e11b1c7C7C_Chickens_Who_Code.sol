...
contract Chickens_Who_Code is Ownable, ERC721Enumerable, ReentrancyGuard {
    ...
    modifier mintModifier(uint8 _amount) {
        require(mintStatus, "Minting not yet open");
        require(_tokenIds.current() + _amount <= totalNfts, "Exceeds total supply");
        require(_amount > 0, "Can't mint 0 amount");
        require(_amount <= 10, "Exceeded max token purchase");
        _;
    }
    ...
    payable
    {
        ...
        if (publicMint == false) {
            require(_amount <= _premintList[msg.sender], "Exceeded mint limit");
            require(msg.value >= (0.03 ether * _amount), "Ether value sent is not correct");
