...
contract Master {
    ...
    mapping(address => uint256) private contractLogGame;

    function createBet(uint256 _gameID) public payable {
        gID = _gameID;
        ContractBetAddress = address(
            new ContractBet(gID, msg.sender, msg.value)
        );
        contractLogGame[ContractBetAddress] = _gameID;
    }

    function getChildContractGameID(address _inputAddress)
        public
        view
        returns (uint256)
    {
        return contractLogGame[_inputAddress];
    }
}
...
contract ContractBet is ChainlinkClient {
    ...
    uint256 public gID_uint;
    string public gID_String;
    ...
    constructor(
        ...
        gID_uint = _gameID;
        gID_String = Strings.toString(gID_uint);
