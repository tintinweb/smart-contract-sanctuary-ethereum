 ...
 */
contract Vesting is AccessControl, IKAPSource {
    /// @dev Role needed to call {createVestingAgreement}
    bytes32 public constant VESTING_CREATOR = keccak256("VESTING_CREATOR");
    /// @dev Multisig wallet owned by the Kapital DAO core team
    address public immutable teamMultisig;
    IERC20 public immutable kapToken;
    /// @dev Tracks the latest governance contract address
    ...
    constructor(
        ...
        _grantRole(VESTING_CREATOR, teamMultisig);
        // `msg.sender` {VESTING_CREATOR} role is renounced after the
        // creation of initial vesting agreements
        _grantRole(VESTING_CREATOR, msg.sender);
        kapToken = IERC20(_kapToken);
        governanceRegistry = IGovernanceRegistry(_governanceRegistry);
    }

    struct VestingAgreement {
        // timestamp at which vesting starts, acts as a vesting delay
        uint64 vestStart;
        // time period over which vesting occurs
        uint64 vestPeriod;
        // total KAP amount to which the beneficiary is promised
        uint96 totalAmount;
        // portion of `totalAmount` which has already been collected
        uint96 amountCollected;
    }

    /// @dev Organize vesting agreements by address
    mapping(address => VestingAgreement[]) public vestingAgreements;
    /// @dev Track uncollected vesting balances
    mapping(address => uint256) public balances;
    /// @dev Track voting delegates
    mapping(address => address) public delegates;
    /// @dev Track voting weights based on delgate choice
    mapping(address => uint256) public weightKAP;
    /// @dev Track timestamp of last {undelegate} call
    ...
    function createVestingAgreement(
        ...
        require(vestStart >= block.timestamp, "Invalid vest start");
        require(vestPeriod > 0, "Invalid vest period");
        require(amount > 0, "Invalid amount");

        // caller provides KAP for the vesting agreement
        SafeERC20.safeTransferFrom(kapToken, msg.sender, address(this), amount);
        // update balance
    ...
    function collect(uint256 vestingAgreementId) external {
        require(
            vestingAgreementId < vestingAgreements[msg.sender].length,
            "Invalid Id"
        );
        ...
        // if `vestingAgreement.vestPeriod` has passed, the entire `totalAmount` is unlocked
        ...
        SafeERC20.safeTransfer(kapToken, msg.sender, collectionAmount);
    }
    ...
    function appointDelegate(address newDelegate) external {
        ...
        require(delegates[msg.sender] == address(0), "Must undelegate first");
        uint256 votingPeriod = IGovernance(
            governanceRegistry.governance()
        ).votingPeriod();
        // prevent switching delegates and voting again on same proposal
        require(
    ...
    function undelegate() external {
        require(delegates[msg.sender] != address(0), "Delegate already zero");
    ...
    function _appointDelegate(address newDelegate) internal {
        ...
        if (balance > 0) {
            weightKAP[oldDelegate] -= balance;
            weightKAP[newDelegate] += balance;
        }
    }
}
