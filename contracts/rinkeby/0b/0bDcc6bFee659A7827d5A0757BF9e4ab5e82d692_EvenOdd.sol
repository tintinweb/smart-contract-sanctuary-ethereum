...
contract EvenOdd is Ownable, ReentrancyGuard {
    ...
    event Bet(address player, uint256 amount, bool isEven);
    event DiceRolled(uint8 diceNumber_1, uint8 diceNumber_2, bool isEven);
    ...
    function bet(bool _isEven) external payable nonReentrant {
        ...
        require(
            (totalBetAmount + msg.value) * 2 <= getDealerBalance(),
        ...
        emit Bet(_msgSender(), msg.value, _isEven);
    }

    function rollDice() external onlyOwner {
        uint8 diceNumber_1 = generateRandomNumber();
        uint8 diceNumber_2 = generateRandomNumber();
        bool isEven = (diceNumber_1 + diceNumber_2) % 2 == 0;
        emit DiceRolled(diceNumber_1, diceNumber_2, isEven);
