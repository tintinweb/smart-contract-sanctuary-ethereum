...
contract Dojo is IDojo, ERC165, IERC721, IERC721Metadata, ReentrancyGuard, AdminControl {
    ...
    function chimpBelt(
        ...
        for (uint256 i = _belts.length; i > 0; i--) {
            if (_xp[stakedId].value >= _belts[i-1].xpThreshold) {
                return _belts[i-1].name;
            }
        }
    ...
    function belts() external view override returns(Belt[] memory) {
        return _belts;
    }

    function setBelts(
        uint32[] calldata xpThresholds,
        string[] calldata beltNames
    ) external override adminRequired {
        uint256 numberOfBelts = xpThresholds.length;

        for (uint256 i = 1; i < numberOfBelts; i++) {
            require(xpThresholds[i] > xpThresholds[i-1], "Belts must be sorted");
        }

        delete _belts;

        for (uint256 i = 0; i < numberOfBelts; i++) {
            _belts.push(Belt({
    ...
    function collectXP(
        ...
        uint256 stakedId = _translateTokenId(tokenId, isGenesisChimp);
        require(_owners[stakedId].ownerAddress != address(0), "ERC721: invalid token ID");
        ...
        uint48 lastEventTime = _xp[stakedId].lastUpdateTime == 0 ?
            _owners[stakedId].entryTime :
            _xp[stakedId].lastUpdateTime;
        uint256 daysElapsed = (block.timestamp - lastEventTime) / 86400;
        require(xp <= daysElapsed * _maxDailyXP, "Exceeds daily limit");
        
        _usedNonces[nonce] = true;
        _xp[stakedId].lastUpdateTime = uint48(block.timestamp);
        _xp[stakedId].value += xp;

        emit CollectedXP(msg.sender, tokenId, isGenesisChimp, xp);
    }
    ...
    function exitDojo(
        ...
        uint256 stakedId = _translateTokenId(tokenId, isGenesisChimp);
        require(msg.sender == _owners[stakedId].ownerAddress, "Address is not owner");

        collectXP(tokenId, isGenesisChimp, message, signature, nonce, xp);

        _owners[stakedId].ownerAddress = address(0);
        _balances[msg.sender] -= 1;
        _xp[stakedId].lastUpdateTime = 0;

        emit Transfer(msg.sender, address(this), stakedId);

        // Return original chimp
        if (isGenesisChimp) {
            IERC721(_chimpersGenesis).transferFrom(address(this), msg.sender, tokenId);
        } else {
            IERC721(_chimpersGenerative).transferFrom(address(this), msg.sender, tokenId);
        }
    }
 ...
 * Dojo Interface
 */
interface IDojo is IERC721Receiver {

    event CollectedXP(
        address indexed sender,
        uint256 indexed tokenId,
        bool indexed isGenesisChimp,
        uint32 amount
    );
    ...
    /**
     * @dev Get a chimp's finalized belt
     */
    function chimpBelt(uint256 tokenId, bool isGenesisChimp) external view returns(string memory);

    /**
     * @dev Get the list of available belts
     */
    function belts() external view returns(Belt[] memory);
    ...
    /**
     * @dev Set the XP thresholds and belt levels 
     *
     * Requirements:
     * 
     * - The caller must be an admin
     */
    function setBelts(uint32[] calldata xpThresholds, string[] calldata beltNames) external;
    ...
    /**
     * @dev Collect accumulated XP. Used for starting or finishing quests, or for claiming passive
     * XP.
     */
    function collectXP(
        uint256 tokenId,
    ...
    /**
     * @dev Leave the Dojo. Returns original chimp to caller and reclaims staked chimp
