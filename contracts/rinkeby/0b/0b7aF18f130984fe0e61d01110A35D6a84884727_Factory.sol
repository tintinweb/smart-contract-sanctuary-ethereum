...
import "./OwnCollectibledNft721.sol";
...
contract Factory {
	...
	function deploy(bytes32 _salt, string memory name, string memory symbol, string memory tokenURIPrefix) external returns(address addr) {
		addr = address(new CollectibledUserToken721{salt: _salt}(name, symbol, tokenURIPrefix));
		CollectibledUserToken721 token = CollectibledUserToken721(address(addr));
...
contract CollectibledUserToken721 is ERC721 {
