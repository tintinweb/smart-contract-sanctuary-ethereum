...
contract BlueBay is ERC721A, Ownable {
    ...
    uint256 public startPrice         =  0.001 ether;  // 1e17
    uint256 public endPrice           =  0.002 ether;  // 1e17

    uint256 public WLdiscountPrice    =  0.0002 ether;  // 1e17
    uint256 public WLdiscountLimit    =  2;

    uint256 public increaseDuration   =  10 hours;
    uint256 public increaseAmount     =  0.0001 ether;
    ...
    function currentPrice() public view returns (uint256) {
        ...
        if (startTime <= 0) {
            ...
            uint256 steps = (endPrice - startPrice) / increaseAmount;
            
            if (block.timestamp - startTime > increaseDuration * steps) {
                price = endPrice;
            } else {
                for (uint256 i = 0; i < steps; i++) {
                    if (block.timestamp - startTime > increaseDuration * i && block.timestamp - startTime <= increaseDuration * (i + 1)) {
                        price = startPrice + i * increaseAmount;
                    }
                }
            }
        }
    ...
    function setIncreaseDuration(uint256 _increaseDuration) public onlyOwner {
