...
contract BasedPixels is ERC721, Ownable {
    ...
    function mint(uint amount) public payable {
        ...
        for (uint i = 0; i < amount; i++) {
            ...
            _slices[(currentIndex/10)+12+i] = randomNumber((_randomRoots[(currentIndex/10)+11])%256, (currentIndex/10)+i);
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        for (uint i = 0; i < 12; i++) {
            slices[i] = _randomRoots[mintBatchRootID]^_randomRoots[mintBatchRootID + (tokenId%10)];
            slices[i] = slices[i]^(_slices[mintBatchRootID+i]^_slices[mintBatchRootID+12]);
        }
    ...
    function contractURI() public view returns (string memory) {
        return string(
            abi.encodePacked(
                "data:application/json;utf8,{\"name\":\"Based Pixels Testing 42\",",
