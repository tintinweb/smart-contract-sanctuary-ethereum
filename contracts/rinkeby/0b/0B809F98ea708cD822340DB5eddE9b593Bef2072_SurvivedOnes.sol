...
contract SurvivedOnes is ERC721A, Ownable {
  ...
  mapping(uint256 => uint256) public KILLED_TO_SURVIVE_MAP;
  ...
  uint256 private MINT_STAGE = 0;

  string public TOKEN_BASE_URL = "ipfs://tba";
  string public TOKEN_URL_SUFFIX = ".json";
  string public NOT_REVEALED_URL = "ipfs://not_revealed";
  ...
  function cum(
    ...
    if (totalSupply() - USED_TEAM_SUPPLY >= MAX_SUPPLY) {
    ...
    for (uint256 i = 0; i < dose; i += 1) {
      ...
      uint256 currentSpermCount = totalSupply();
      if (currentSpermCount + (TEAM_SUPPLY - USED_TEAM_SUPPLY) > MAX_SUPPLY) {
        break;
      }
      ...
      if (rand < tempSurviveChange) {
        KILLED_TO_SURVIVE_MAP[_nextTokenId() + totalSurvived] = tempDeath;
        totalSurvived += 1;
        tempDeath = 0;
      } else {
        tempDeath += 1;
    ...
    if (totalSurvived > 0) {
      uint256 currentSpermCount = totalSupply();
      if (
        currentSpermCount + (TEAM_SUPPLY - USED_TEAM_SUPPLY) + totalSurvived >
        MAX_SUPPLY
      ) {
        revert LogicError("sold_out");
      }
