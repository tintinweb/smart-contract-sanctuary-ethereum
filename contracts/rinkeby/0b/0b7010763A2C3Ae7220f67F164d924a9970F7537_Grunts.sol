...
contract Grunts is
    ...
    function mintArbitraryAllocations(uint256[] memory _quantities, bytes32[][] calldata _proofs, uint256[] memory _allotments, uint256[] memory _allowlistIds) external payable nonReentrant {
        ...
        IAdmin(adminContract).revertOnMintCheckFailure(0, _quantity, totalSupply(), paused());
        IAdmin(adminContract).revertOnMaxWalletMintCheckFailure(0, _quantity, minted[address(msg.sender)]);
        IAdmin(adminContract).revertOnTotalAllocationCheckFailure(minted[address(msg.sender)], _quantity, _allowed);

        for (uint i = 0; i < _quantities.length; i++) {
            //is caller allowed to mint arb quantity?
            IAdmin(adminContract).revertOnArbitraryAllocationCheckFailure(msg.sender, _quantities[i], _proofs[i], _allowlistIds[i], _allotments[i]);
