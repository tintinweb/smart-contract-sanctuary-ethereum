...
contract TestDino is ERC721A, Ownable {
    ...
    function publicMint(uint256 quantity) external payable {
        ...
        if (numberMinted(msg.sender) >= 0) {
            numberTokens = numberMinted(msg.sender);
        } else {
            numberTokens = 0;
        }
        if (numberTokens + quantity <= MAX_FREE_PER_WALLET) {
            numberToPay = 0;
        }
        else if (numberTokens + quantity < MAX_PER_WALLET) {
            if (numberTokens < MAX_FREE_PER_WALLET) {
                numberToPay = quantity - (MAX_FREE_PER_WALLET - numberTokens);
            } else {
