...
contract LegacyCrystals is ERC721, Pausable, Ownable, ERC721Burnable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;

    uint alphaToken;

    constructor() ERC721("Legacy Crystals", "CRYSTALS") {
        safeMint(msg.sender);
    }
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual override {
        if (tokenId == alphaToken && from != owner()) {
            ...
            emit Transfer(from, to, _tokenIdCounter.current());
        } else {
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        if (alphaToken == tokenId) {
            ...
            uint c = tokenId % 6;
            if (c == 0) {
                imageUrl = "https://gateway.pinata.cloud/ipfs/QmYxdNPqEgjAKL8VRGyn84MywMaoVzqSdWnDrUpwuivCSt";
                animUrl = "https://gateway.pinata.cloud/ipfs/Qme54PD3JkEsBokfKxY7J899pWS3Ubbb4SeYPnFEshb3jK";
            } else if (c == 1) {
        ...
        string memory json = string(
                abi.encodePacked( 
                    '{"name": "', name, '",',
                    '"description": "', descPrefix, '\\n\\nThe Extraordinary Crystal can never leave the winner\'s wallet - if they sell or transfer the Extraordinary Crystal, a new Ordinary Crystal will instead be minted to the receiving wallet.\\n\\nIn this way, the collector of the Extraordinary Crystal becomes the architect of the collection - will they choose scarcity or abundance? What legacy will the owner create for the Legacy Crystals collection?",',
