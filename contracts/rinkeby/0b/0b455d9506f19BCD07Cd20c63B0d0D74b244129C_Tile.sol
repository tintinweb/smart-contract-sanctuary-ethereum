...
contract Tile is ERC165, IERC1155, IERC1155MetadataURI, Ownable {
    ...
    // Tile on-chain colors (may be different on web).
    uint32[] public tileColors;
    ...
    // Fees - use getMintCost / getColorCost to query.
    Fees private _fees;
    ...
    // Events
    event MintedTiles(uint256[] tokenIds);

    constructor(Limits memory initialLimits, Fees memory initialFees, string memory apiUrl, address proxyRegistryAddress) {
        require(initialFees.mintFeeMin < initialFees.mintFee, "Minimum fee cannot be greater than starting fee");
        require(initialLimits.size % 2 == 0, "Can only set size to even numbers");
        _baseUrl = apiUrl;
        _fees = Fees({
    ...
    function getColors(uint256[] memory ids) external view returns (string[] memory) {
    ...
    function setColors(uint256[] memory ids, uint32[] memory colors) external payable {
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            require(
                _tileOwners[id] == msg.sender || isApprovedForAll(_tileOwners[id], msg.sender),
    ...
    function getMintCost(uint16 amount) public view returns (uint256) {
        ...
        // same as below:
        require (mintedTiles + amount <= maxTiles(), "Purchase would mint more tiles than available");
        uint16 exemptAmount = 0;
        if (msg.sender == _deployer) {
            exemptAmount = uint16(Math.min(Math.max(0, _limits.deployerDiscountedTiles - mintedTiles), 2 ** 16));
        }
        if (exemptAmount >= amount) {
            return 0;
        }
        uint256 base = Math.min(amount - exemptAmount, (_fees.mintFee-_fees.mintFeeMin)/_fees.bulkDiscount);
        return base * (_fees.mintFee - (base - 1) * _fees.bulkDiscount / 2) + (amount - exemptAmount - base) * _fees.mintFeeMin;
    }

    function getColorCost() external view returns (uint256) {
        return _fees.colorFee;
    }
    ...
    function mintTo(uint16 amount, address to) external payable {
        require (mintedTiles + amount <= maxTiles(), "Cannot mint more tiles than available");
        require (amount <= maxMintSize(), "Too many tiles to mint in one batch");
        ...
        for (uint16 i = 0; i < amount; i++) {
            ...
            ones[i] = 1;
            _tileOwners.push(to);
            tileColors.push(uint32(random % 2 ** 32));
    ...
    function stashBatch(address from, uint256[] memory tileIds) external onlyStasher {
        ...
        for (uint256 i = 0; i < tileIds.length; ++i) {
            ...
            require(_tileOwners[id] == from, "ERC1155: insufficient balance for transfer");
    ...
    function safeTransferFrom(
        ...
        require(id < mintedTiles, "Tile does not exist");
        require(amount == 1, "Each tile is unique - can only transfer 1 of each.");
        require(
    ...
    function setLimits(Limits memory newLimits) external onlyOwner {
        require(newLimits.size % 2 == 0, "Can only set size to even numbers");
        require(newLimits.size >= _limits.size, "Cannot reduce limit of tiles.");
        require(newLimits.deployerDiscountedTiles == _limits.deployerDiscountedTiles, "Cannot change Limits.deployerDiscountedTiles.");
        _limits = newLimits;
    }
