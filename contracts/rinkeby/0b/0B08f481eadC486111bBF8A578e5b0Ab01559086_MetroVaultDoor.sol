...
contract MetroVaultDoor is Controllable, IERC721Receiver {
    ...
    uint256 constant BLACKOUT_TOKEN_ID = 20_000;
    ...
    function stake(uint256[] calldata tokenIds) external {
        ...
        vault.stakeUnsafe(_msgSender(), tokenIds, 0, 0);
    }
    ...
    function unstake(uint256[] calldata tokenIds) external {
        ...
        IMetroVaultStorage vault = IMetroVaultStorage(vaultAddress);
        vault.unstakeUnsafe(_msgSender(), tokenIds);
    }

    function claim(uint256[] calldata tokenIds) external {
        require(!claimDisabled, 'Claim is disabled');
        require(blockInfoAddress != address(0x0), 'Bloc Scores are not Set');
        _claim(_msgSender(), tokenIds);
    }
    ...
    function tokensOfOwner(address account) public view returns (uint256[] memory) {
      return IMetroVaultStorage(vaultAddress).tokensOfOwner(account, 0, 20000);
    }
