...
interface IMintableERC721 {
  ...
  /**
   * @dev Burns existing tokens starting with token ID specified
   *      and runs for a specific amount of tokens.
   *
   ...
   * @dev Should have a restricted access handled by the implementation
   *
   * @param _tokenId ID of the first token to burn
   * @param _n how many tokens to burn, sequentially increasing the _tokenId
   */
  function burnBatch(uint256 _tokenId, uint256 _n) external;
...
// File contracts/sale/MintableSale.sol
