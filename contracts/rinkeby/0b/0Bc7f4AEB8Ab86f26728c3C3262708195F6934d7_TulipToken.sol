...
contract TulipToken is Ownable, ERC721, ITulipToken {
    ...
    mapping(address => uint256[]) public winnersToTokenIds;
        ...
        returns (uint256)
    {
        ...
        winnersToTokenIds[_winner].push(_id);
    ...
    function mintAll() external override {
        uint256[] memory _idsOfSender = winnersToTokenIds[msg.sender];

        for (uint i = 0; i < _idsOfSender.length; i++) {
            _mint(msg.sender, _idsOfSender[i]);
        }

        delete winnersToTokenIds[msg.sender];
    }
    ...
    /// @return the ids of NFTs to be minted given an address.
    function tokenWinner(address winner) public view returns (uint256[] memory) {
        return winnersToTokenIds[winner];
    }
