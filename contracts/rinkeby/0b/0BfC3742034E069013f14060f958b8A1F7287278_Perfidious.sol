...
contract Perfidious is ERC721, Ownable, ReentrancyGuard {
    uint constant public MAX_SUPPLY = 999;
    string public baseURI = "";
    string public contractURI = "";
    bytes32 public presaleMerkleRoot;
    uint constant public presaleMintQuantity = 2;
    ...
    function mint() payable external nonReentrant() {
        ...
        require(MAX_SUPPLY > totalSupply, "Exceeds max supply");
        require(mintTracker[msg.sender] == false, "Exceeds wallet limit");
    ...
    function presaleMint(bytes32[] memory proof) payable external nonReentrant() {
        ...
        require(MerkleProof.verify(proof, presaleMerkleRoot, keccak256(abi.encodePacked(msg.sender))), "You are not on the presale");
        require(MAX_SUPPLY >= totalSupply + presaleMintQuantity, "Exceeds max supply");
        require(mintTracker[msg.sender] == false, "Exceeds wallet limit");
        mintTracker[msg.sender] = true;
        _mintTokens(msg.sender, presaleMintQuantity);
    }

    function perfidiousMint(address addr, uint quantity) external onlyOwner {
          require(MAX_SUPPLY >= totalSupply + quantity, "Exceeds max supply");
    ...
    function _mintTokens(address to, uint256 numberToMint) internal {
        ...
        for (uint256 i; i < numberToMint; ++i) {
            _safeMint(to, currentSupply_++);
        }
        totalSupply = currentSupply_;
    }

    function clearMintTracker(address addr) external
    {
