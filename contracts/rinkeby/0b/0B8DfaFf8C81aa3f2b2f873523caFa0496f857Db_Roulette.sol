...
contract Roulette is IERC20 {
    ...
    uint256 private constant startingSupply = 2_000_000;

    string private constant _name = "Roulette";
    string private constant _symbol = "ROULETTE";
    ...
    address payable public treasuryWallet =
        payable(0x12a410e35646991f750E1d980E35d83ac9742B5C);
    address payable public safeWallet =
        payable(0x979AfE9530301Ab9DA15969d5146411E188FCFd9);
    address payable public teamWallet =
        payable(0xDce443131b43F65B6e716206BB4316380e006d80);
    ...
    constructor(
        address _manager
        // address _rouletteNFTAddress
        // address _mansion
    ) {
        // rouletteNFT = RouletteNFT(_rouletteNFTAddress);
    ...
    function nodeApprove(address spender, uint256 amount)
        external
        returns (bool)
    {
        _allowances[msg.sender][spender] = amount;
    ...
    function setProtectionSettings(bool antiBlock) external onlyOwner {
    ...
    //     require(
    //         _mansion != IERC20(address(this)),
    //         "Mansion address cannot be this address"
    //     );
    //     mansion = IERC20(_mansion);
    // }
    ...
    function _transfer(
        ...
        require(amount <= balanceOf(from), "Transfer amount must be greater than zero");
    ...
    function takeTaxes(
        ...
        uint256 currentFee;
        if (from == lpPair) {
            currentFee = _taxRates[0].buyFee;
            // currentFee = rouletteNFT.colorNftWin(to) ? 0 : _taxRates[0].buyFee;
        } else if (to == lpPair) {
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        uint256 amountToLiquify = ((numTokensToSwap * _ratios.liquidity) /
            (_ratios.total)) / 2;
        uint256 amountToRewardsPool = (numTokensToSwap * _ratios.rewards) /
            (_ratios.total);

        if (amountToRewardsPool > 0) {
            _tOwned[pool] += amountToRewardsPool;
            emit Transfer(
                address(this),
                pool,
                amountToRewardsPool
        ...
        // uint256 amountAVAXLiquidity = ((amountAVAX * _ratios.liquidity) /
        //     (_ratios.total)) / 2;
