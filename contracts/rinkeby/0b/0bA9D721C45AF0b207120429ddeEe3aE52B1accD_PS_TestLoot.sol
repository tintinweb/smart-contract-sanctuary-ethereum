...
LootItems
{
    ...
    function mintPublic(
        ...
        uint256[] calldata mintAmounts) public payable whenNotPaused
    {
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    // Signature Params
        ...
        {
            // Verify Signature
            if(!verify(Settings.signerAddress, msg.sender, tokenIndices, mintAmounts, signature))
    ...
    function CheckRequires(
        uint256[] calldata tokenIndices,
        uint256[] calldata mintAmounts,
        bool shouldCheckCost) internal view {
        ...
        for(uint index = 0; index < tokenIndices.length; index++)
        {
            ...
            if(supplyCount + mintAmount > SourceData.TotalSupply) revert ErrorCodes.SoldOut();
            // Check the balance is not more than mint amount
            if(balanceOf(msg.sender, tokenID) + mintAmount > SourceData.MaxMintPerUser) revert ErrorCodes.MaxAmountMinted(SourceData.MaxMintPerUser, balanceOf(msg.sender, tokenID) + mintAmount);
...
contract LootItems is AccessControl, Settings, Roles
{
    using Strings for uint;
    ...
    */
    ...
    // TODO 
    function updateLootItem(uint tokenIndex, LootItem memory newLootItem) public onlyRole(Roles.ROLE_ADMIN)
    {
        if(false == lootItemList[tokenIndex].exists) revert ErrorCodes.LootItem_ItemDoesntExist();
        lootItemList[tokenIndex] = newLootItem;
    }
