...
interface IMasterChef {
    ...
    function matrix() external view returns (address);

    function matrixPerBlock() external view returns (uint256);
...
contract BoringHelperV1 is Ownable {
    ...
    address public maker; // IMatrixMaker(0xE11fc0B43ab98Eb91e9836129d1ee7c3Bc95df50);
    IERC20 public matrix; // IMatrixToken(0x6B3595068778DD592e39A122f4f5a5cF09C90fE2);
    IERC20 public WMATIC; // 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    IFactory public matrixFactory; // IFactory(0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac);
    IFactory public quickswapFactory; // IFactory(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f);
    ...
    constructor(
        ...
        IERC20 matrix_,
        IERC20 WMATIC_,
        IFactory matrixFactory_,
        IFactory quickswapFactory_,
        IERC20 bar_
    ) public {
        chef = chef_;
        maker = maker_;
    ...
    function getMATICRate(IERC20 token) public view returns (uint256) {
        if (token == WMATIC) {
            return 1e18;
        }
        IPair pairQuickswap;
        IPair pairMatrix;
        if (quickswapFactory != IFactory(0)) {
            pairQuickswap = IPair(quickswapFactory.getPair(token, WMATIC));
        }
        if (matrixFactory != IFactory(0)) {
            pairMatrix = IPair(matrixFactory.getPair(token, WMATIC));
        }
        ...
        if (address(pairQuickswap) != address(0)) {
            (uint112 reserve0Quickswap, uint112 reserve1Quickswap, ) = pairQuickswap.getReserves();
            reserve0 += reserve0Quickswap;
            reserve1 += reserve1Quickswap;
            token0 = pairQuickswap.token0();
        }

        if (address(pairMatrix) != address(0)) {
            (uint112 reserve0Matrix, uint112 reserve1Matrix, ) = pairMatrix.getReserves();
            reserve0 += reserve0Matrix;
            reserve1 += reserve1Matrix;
            if (token0 == IERC20(0)) {
                token0 = pairMatrix.token0();
            }
        }
    ...
    struct UIInfo {
        ...
        uint256 matrixBarBalance;
        uint256 xmatrixBalance;
        uint256 xmatrixSupply;
        uint256 matrixBarAllowance;
        Factory[] factories;
        uint256 maticRate;
        uint256 matrixRate;
        uint256 btcRate;
    ...
    function getUIInfo(
        ...
        if (currency != IERC20(0)) {
            info.maticRate = getMATICRate(currency);
        }

        if (matrix != IERC20(0)) {
            info.matrixRate = getMATICRate(matrix);
            info.matrixBalance = matrix.balanceOf(who);
            info.matrixBarBalance = matrix.balanceOf(address(bar));
            info.matrixBarAllowance = matrix.allowance(who, address(bar));
        }

        if (bar != IERC20(0)) {
            info.xmatrixBalance = bar.balanceOf(who);
            info.xmatrixSupply = bar.totalSupply();
        }
        ...
        if (chef != IMasterChef(0)) {
            ...
            for (uint256 i = 0; i < poolLength; i++) {
                (uint256 pendingMatrixAmt, , , ) = chef.pendingTokens(i, who);
                pendingMatrix += pendingMatrixAmt;
            }
            info.pendingMatrix = pendingMatrix;
        }
    ...
    function getBalances(address who, IERC20[] calldata addresses) public view returns (BalanceFull[] memory) {
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            balances[i].rate = getMATICRate(token);
        }
