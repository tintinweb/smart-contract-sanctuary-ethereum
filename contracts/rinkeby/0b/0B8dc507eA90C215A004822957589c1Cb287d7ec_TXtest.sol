...
contract TXtest is ERC721A, Ownable, ReentrancyGuard {
    ...
    //Mint limits
    uint256 public ADDRESS_MAX_MINTS = 10;
    uint256 public BULK_ADDRESS_MAX_MINTS = 10; //thinking of changing bulk mint batch
    uint256 public ADDRESS_OG_MAX_MINTS = 2;
    uint256 public ADDRESS_WL_MAX_MINTS = 3;
    uint256 public PUBLIC_MINT_PER_TX = 10;

    //Supply
    uint256 public maxSupply;
    uint256 public reservedSupply = 1111;

    //Pricing
    uint256 public OGprice = 0.15 ether;
    uint256 public WLprice = 0.20 ether;
    uint256 public price = 0.25 ether;

    // Sign Hash
    address private _signer = 0xdeadc0DEDADd13535dC04A830FD313E40385B765;
    string private _proveOwner;
    ...
    constructor(string memory proveOwner_) ERC721A("TXtest", "TXTEST") {
        _proveOwner = proveOwner_;
    }
    ...
    function claim(uint256 count, bytes32[] calldata proof)
        ...
        nonReentrant
    {
        require(isFreeClaimActive, "Free Claim Not Live");
        require(_claimVerify(_claimLeaf(msg.sender, count), proof), "Invalid");
        require(totalClaimed[msg.sender] != count, "Already claimed"); // check this out, if count is > than claim
    ...
    function bulkMint(uint256 quantity, address tx_user) external onlyOwner {
        require(
            totalSupply() + quantity <= reservedSupply,
            "Too many minted to public to perform dev mint"
        );
        ...
        for (uint256 i = 0; i < numChunks; i++) {
            _safeMint(tx_user, ADDRESS_MAX_MINTS);
        }
    }

    /* old phase
    function toggleOGSale(bool) external onlyOwner {
...
*/
    ...
    function setProvenanceOfOwner(string memory _new) external onlyOwner {
        _proveOwner = _new;
    }

    function getProvenanceOfOwner() external view returns (string memory) {
        return _proveOwner;
    }
    ...
    function setMaxBulkAddress(uint256 _new) external onlyOwner {
        BULK_ADDRESS_MAX_MINTS = _new;
    }
    ...
    function setReservedSupply(uint256 _new) external onlyOwner {
        reservedSupply = _new;
    }

    //verify whitelist merkle tree
}
