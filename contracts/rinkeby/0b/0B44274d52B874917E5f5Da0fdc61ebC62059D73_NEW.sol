...
contract NEW is ERC721AQueryable, Ownable, ReentrancyGuard {
using Strings for uint256;

//Initial Settings -> 
string public tokenName = "NEW";
string public tokenSymbol = "NEW";
uint256 public maxSupply = 5;
uint256 public maxReservedSupply = 2;
uint256 public claimedReservedSupply;

uint256 public maxMintAddress = 3;
bytes32 public merkleRoot;
mapping(address => bool) public numberMintedByUser; 
...
function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintPriceCompliance(_mintAmount) {
  require(whitelistMintEnabled == true, 'The Whitelist Mint is not enabled');
  require(!paused, 'The contract is paused!');
  bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
  require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), 'Invalid proof!');
  require(_mintAmount > 0 && _mintAmount <= maxMintAddress, 'Invalid mint amount!');
  require(totalSupply() + _mintAmount <= (maxSupply - maxReservedSupply + claimedReservedSupply), 'Max supply exceeded!');
  require(!numberMintedByUser[_msgSender()], 'Address already claimed!');

  numberMintedByUser[_msgSender()] = true;
  _safeMint(_msgSender(), _mintAmount);
}

function mint(uint256 _mintAmount) public payable mintPriceCompliance(_mintAmount) {
  require(whitelistMintEnabled == false, 'The Whitelist Mint is enabled');
  require(!paused, 'The contract is paused!');
  require(_mintAmount > 0 && _mintAmount <= maxMintAddress, 'Invalid mint amount!');
  require(totalSupply() + _mintAmount <= (maxSupply - maxReservedSupply + claimedReservedSupply), 'Max supply exceeded!');
  require(!numberMintedByUser[_msgSender()], 'Address already claimed!');
...
function mintByOwner(uint256 _mintAmount, address _receiver) public onlyOwner {
  require((totalSupply() + _mintAmount) <= maxSupply, 'Max supply exceeded!');

  if (_receiver == owner()){
    require(_mintAmount <= maxReservedSupply - claimedReservedSupply, 'Max reserve supply exceeded!');
  }

  _safeMint(_receiver, _mintAmount);
  claimedReservedSupply = claimedReservedSupply + _mintAmount;
}
...
function updateMintCost(uint256 _amount) internal view returns (uint256 _cost) {
  ...
  } else if (whitelistMintEnabled == false) {
    if (_amount == 1){
      return 0.01 ether;
    }
      if (_amount == 2){
