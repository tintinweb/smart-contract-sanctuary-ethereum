...
contract NFT is ERC721, ERC2981, Ownable {
    ...
    function presaleMintDefender(uint256 count, bytes memory signature) external payable validateEthPaymentDefender(count) {
        ...
        require(ECDSA.recover(ECDSA.toEthSignedMessageHash(messageHash), signature) == getAdminSigner(), 'Signature invalid or unauthorized.');
        
        if (presaleSupplyLimit > 0) {
            require((defenderMinted + count) <= presaleSupplyLimit, 'Exceeds max presale supply of Defender.'); // presaleSupplyLimit should be updated during all presale iterations, supplyLimit <= NUMBER_OF_DEFENDER
        }
        require((defenderMinted + count) <= NUMBER_OF_DEFENDER, 'Exceeds max supply of Defender.');
    ...
    function presaleMintConqueror(uint256 count, bytes memory signature) external payable validateEthPaymentConqueror(count) {
        ...
        if (presaleSupplyLimit > 0) {
            require((conquerorMinted + count) <= presaleSupplyLimit, 'Exceeds max presale supply of Conqueror.'); // presaleSupplyLimit should be updated during all presale iterations, supplyLimit <= NUMBER_OF_CONQUER - NUMBER_OF_RESERVED_CONQUER
        }
        require((conquerorMinted + count) <= NUMBER_OF_CONQUER - NUMBER_OF_RESERVED_CONQUER, 'Exceeds max supply of Conqueror.');
        require(balanceOf(_msgSender()) + count < MAX_PRESALE_MINTS_PER_ADDRESS, 'Exceeds number of presale mints allowed.');
        require(count < MAX_PER_TX, 'Exceeds max per transaction.');
    ...
    function reserveMintConqueror(uint256 count, bytes memory signature) external payable validateEthPaymentConqueror(count) {
        require(phase == SalePhase.LimitedSale, 'Limited sale is not active.');

        bytes32 messageHash = keccak256(abi.encodePacked(_msgSender()));
        require(ECDSA.recover(ECDSA.toEthSignedMessageHash(messageHash), signature) == getAdminSigner(), 'Signature invalid or unauthorized.');

        require(conquerorMinted + count <= NUMBER_OF_CONQUER, 'Exceeds max supply of Conqueror.');
        require(count < MAX_PER_TX, 'Exceeds max per transaction.');
