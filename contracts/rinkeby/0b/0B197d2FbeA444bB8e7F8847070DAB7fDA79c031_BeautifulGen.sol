...
contract BeautifulGen is ERC721A, IERC2981, VerifySignature, ReentrancyGuard {
    using Strings for uint256;

    uint256 public MAX_SUPPLY = 1111;
    uint256 public ROYALTY_PERCENT = 10;
    uint256 public MAX_PER_WALLET = 2;
    uint256 public PRICE_PER_FREE_NFT = 0 ether;
    uint256 public PRICE_PER_NFT = 0.005 ether;

    string private baseURI = "ipfs://xxx/";
    string private baseURISuffix = ".json";

    address public royaltyAddress;

    constructor() 
        ERC721A("BeautifulGen", "BGN")
    ...
    function getNFTPrice(uint256 quantity) public view returns (uint256) {
        if(totalSupply() <= 2) {
            return PRICE_PER_FREE_NFT * quantity;
        } else {
