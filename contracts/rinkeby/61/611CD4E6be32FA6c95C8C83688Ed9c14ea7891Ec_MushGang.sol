...
contract MushGang is ERC721A, ERC721ABurnable,ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    function getAvailableForMintByCurrentStage() public view returns (uint8) {
        if(saleState==Stage.MushVIP)
            return _vipList[msg.sender] - getRedemptionsVipList();
        if(saleState==Stage.MushList)
            maxByWalletPerMushSale-getRedemptionsVipList();
        if(saleState==Stage.Public)
            maxByWalletPerPublic-getRedemptionsPublic();
        return 0;
    }
