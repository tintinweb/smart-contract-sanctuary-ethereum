...
contract QuantumPFP is Initializable, UUPSUpgradeable, ERC721Upgradeable, OwnableUpgradeable, Namehash {
  ...
  string public constant baseDomain = "bstestaddress.eth";
  ...
  string[5] public specialtyNames;
  ...
  function initialize(string memory __baseURI, string memory _terminatedURI, address ensRegistrar) initializer public {
    ...
    specialtyNames = ["Unknown", "Social", "Tech", "Sales", "Art"];
  ...
  function nameToENS(string memory _name) public view returns(address){
    bytes32 toResolve = computeNamehash(abi.encodePacked(_name, ".", baseDomain));
  ...
  function addEmployee(string memory _name, address targetAddress, string memory email, Designations designation, Specialties specialty) external onlyOwner {

    EmployeeList[historicalEmployeeCount] = Employee(specialty, designation, _name, email, false);

    bytes32 rootNodeId = computeNamehash(abi.encodePacked(baseDomain));
    Resolver rootResolver = ens.resolver(rootNodeId);
    ens.setSubnodeRecord(computeNamehash(abi.encodePacked(baseDomain)), keccak256(abi.encodePacked(_name)), address(this), address(rootResolver), 0);

    bytes32 nodeId = computeNamehash(abi.encodePacked(_name,  ".", baseDomain));
    ...
    resolver.setText(nodeId, "specialty", specialtyNames[uint256(specialty)]);
  ...
  function terminateAccess(uint tokenId) external onlyOwner {
    ...
    bytes32 nodeId = computeNamehash(abi.encodePacked(EmployeeList[tokenId].name,  ".", baseDomain));
    Resolver resolver = ens.resolver(nodeId);
 ...
 function safeTransferFrom(
  ...
  function burn(uint256 tokenId) public onlyOwner {
    ens.setSubnodeRecord(computeNamehash(abi.encodePacked(baseDomain)), keccak256(abi.encodePacked(EmployeeList[tokenId].name)), address(0), address(0), 0);
