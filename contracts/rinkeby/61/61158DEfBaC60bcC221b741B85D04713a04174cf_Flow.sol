...
contract Flow is GenerativeArtNFTWithClaiming  {
    constructor(string memory initialWeb2BaseURI_, address claimContractAddress_) 
    GenerativeArtNFTWithClaiming(
        "Flow", //token name
        "MIN_1", //token symbol, minimizer's 2nd project
        initialWeb2BaseURI_, //address of base URI, while minting is active
        888, //max supply
        5, //max mint at once
        1, //preminted tokens
...
/* @title GenerativeArtNFTWithClaiming
 * @author minimizer <[email protected]>; https://www.minimizer.art/
 */
 ...
 * Earlier version was designed for use with p5 or similar libraries, was deployed to ethereum mainnet 
 * for Waves project (MIN_0, contract address 0x46f1c444a9b10173c52ee7351eBa1e49C8bC5851).
 * 
 * This version is designed for use with pure javascript, and can produce a fully functional, 
 * self-contained HTML5/Canvas API file with all included javascript to render the art.
 * 
 * 
 * Key features:
...
contract GenerativeArtNFT is IERC2981, ERC721, Ownable, Freezable {
    ...
    function royaltyInfo(uint256, uint256 salePrice_) external view override returns (address, uint256) {
        return (owner(), (salePrice_ * royaltyBasisPoints) / 10000);
    }
    ...
    // Retreive the rendering code in plain text, for a given token, by combining the token id 
    // and hash with the rendering code. This assumes the language is javascript.
    function renderingCodeForToken(uint tokenId_) public view onlyValidTokenId(tokenId_) returns (string memory) {
...
contract ClaimTracker {
    ...
    function claimableTokens() public view returns (uint[] memory) {
        ...
        for (uint i = 0; i < tokenCount; i++) {
            if(claimContract.ownerOf(i) == msg.sender && !claimed.get(i)) {
    ...
    function _claimTokens(uint[] memory tokenIds_) internal {
        ...
        for (uint i = 0; i < tokenIds_.length; i++) {
            ...
            require(!claimed.get(tokenId), "Token already claimed");
            claimed.set(tokenId);
        }
    }
    ...
    function _tokensOfClaimStatus(bool claimedStatus_) private view returns (uint[] memory) {
        ...
        for (uint i = 0; i < initialResult.length; i++) {
            if(claimed.get(i)==claimedStatus_) {
                initialResult[index] = i;
                index += 1;
            }
        }
