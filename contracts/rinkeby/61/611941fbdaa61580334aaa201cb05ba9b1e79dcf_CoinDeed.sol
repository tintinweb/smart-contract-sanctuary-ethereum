...
contract CoinDeed is ICoinDeed, AccessControl {
    ...
    // TODO USDT change between network, this is not constant
    ...
    function stake(uint256 amount) onlyInState(DeedState.SETUP) external override {
        ...
        ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).emitStakeAdded(msg.sender, amount);
    }
    ...
    function editBrokerConfig(BrokerConfig memory brokerConfig_) onlyRole(DEFAULT_ADMIN_ROLE) onlyInState(DeedState.SETUP) public override {
        if (brokerConfig_.allowed == false) {
    ...
    function edit(DeedParameters memory deedParameters_,
        ...
        ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).emitDeedEdited(manager, saleId_);
    }
    ...
    function cancel() onlyInStates([DeedState.SETUP, DeedState.READY]) external override {
        ...
        if (wholesaleId > 0) {    
            IWholesaleFactory wholesaleFactory = IWholesaleFactory(coinDeedAddressesProvider.wholesaleFactory());
            IWholesaleFactory.Wholesale memory wholesale = wholesaleFactory.getWholesale(wholesaleId);
            if (block.timestamp < wholesale.deadline) {
                wholesaleFactory.cancelReservation(wholesaleId);
            }
        }
    ...
    function buyIn(uint256 amount) onlyInState(DeedState.READY) external payable override {
        ...
        ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).emitBuyIn(msg.sender, buyInAmount);
    }
    ...
    function buy(uint256 minAmountOut) onlyInState(DeedState.READY) external override {
        uint256 purchasedTokenPrice = coinDeedAddressesProvider.tokenPrice(pair.tokenB);
        ...
        ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).emitStateChanged(state);
        ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).emitBuySucceed(totalSupply, purchasedTokenPrice, totalStake);
    }
    ...
    function executeRiskMitigation() external payable override onlyInState(DeedState.OPEN) {
        require(deedParameters.leverage > 1, "BAD_RISK_MITIGATION_TRIGGER");
        if (riskMitigation.leverage == 1 && riskMitigationTriggered) {
            return;
        }
        else {
            ILendingPool lendingPool = ILendingPool(coinDeedAddressesProvider.lendingPool());
            uint256 totalBorrowAmount = lendingPool.totalBorrowBalance(pair.tokenA, address(this));
            ...
            (uint256 sellAmount, uint256 buyAmount) = coinDeedAddressesProvider.checkRiskMitigationAndGetSellAmount(
                pair, deedParameters, riskMitigation, riskMitigationTriggered, pendingToken);

            // Caculate the actual number of purchased token that need to withdraw from the pool
            uint256 actualWithdraw = riskMitigationTriggered ? lendingPool.depositAmount(pair.tokenB, address(this)) : sellAmount - lendingPool.pendingToken(pair.tokenB, address(this));

            // Calculate liquidator discount
            uint256 maxDiscountPercentage = ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).liquidationBonus();
            uint256 discount;

            if (!riskMitigationTriggered) {
                discount = buyAmount * maxDiscountPercentage / BASE_DENOMINATOR;
            }
            // Caculate second liquidation time discount
            else {
                if (buyAmount > totalBorrowAmount) {
                    uint256 allowableReductionPercentage = (buyAmount - totalBorrowAmount) * BASE_DENOMINATOR / buyAmount;
                    uint256 discountPercentage = allowableReductionPercentage > maxDiscountPercentage ? maxDiscountPercentage : allowableReductionPercentage;
                    discount = buyAmount * discountPercentage / BASE_DENOMINATOR;
                }
                else {
                    discount = 0;
                }
            ...
            if (riskMitigationTriggered) {
                ...
                if (sellAmount == lendingPool.totalDepositBalance(pair.tokenB, address(this)) + pendingToken) {
                    state = DeedState.CLOSED;
                }              
            }

            if(state == DeedState.CLOSED) ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).emitStateChanged(state);
            ...
            if(pair.tokenA == address(0x00)) {
                require(msg.value >= buyAmount - discount, "Coindeed: Value ETH sent for risk mitigation is not enough");
                payable(msg.sender).transfer(msg.value - (buyAmount - discount));
            }
            else {
                require(IERC20(pair.tokenA).allowance(msg.sender, address(this)) >= (buyAmount - discount), "Coindeed: Allowance for token A must be higher!");
                IERC20(pair.tokenA).safeTransferFrom(msg.sender, address(this), buyAmount - discount);
            }

            //Repay the debt for the lending pool
            uint256 amountRepay = buyAmount - discount;
            if (amountRepay > totalBorrowAmount) {
                _repay(totalBorrowAmount);
            }
            else {
                _repay(amountRepay);
            }
            ...
            coinDeedAddressesProvider.withdrawFromPool(pair.tokenB, actualWithdraw);
            _transfer(pair.tokenB, payable(msg.sender), sellAmount);
            pendingToken = IERC20(pair.tokenB).balanceOf(address(this));
            ICoinDeedFactory(
                coinDeedAddressesProvider.coinDeedFactory()).emitRiskMitigationTriggered(
                    sellAmount, amountRepay, state == DeedState.CLOSED);
        }
    }
...
interface ICoinDeedFactory {
    ...
    event BuySucceed(
        ...
        uint256 indexed purchasedTokenPrice,
        uint256 totalStake
    );
    ...
    */
    ...
    function permitDeedManagers(address[] calldata _managers) external;

    function unpermitDeedManagers(address[] calldata _managers) external;
    ...
    function emitBuySucceed(uint256 totalSupply, uint256 purchasedTokenPrice, uint256 totalStake) external;
}
...
interface ICoinDeedAddressesProvider {
    event StableCoinAddressChanged(address stableCoinAddress);
...
library CoinDeedAddressesProviderUtils {
    ...
    function tokenRatio(
        ...
        uint256 answerA = tokenPrice(coinDeedAddressesProvider, tokenA);
        uint256 answerB = tokenPrice(coinDeedAddressesProvider, tokenB);
        uint8 decimalsA = 18; //Case token A is ETH
        if(tokenA != address(0)) {
            decimalsA = IERC20MetadataUpgradeable(tokenA).decimals();
        }         
        uint8 decimalsB = 18; //Case token A is ETH
        if(tokenB != address(0)) {
            decimalsB = IERC20MetadataUpgradeable(tokenB).decimals();
        }         
        require(answerA > 0 && answerB > 0, "Invalid uniswap answer");

        //Amount of token B = tokenAAmount * answerA/decimalsA / answerB/decimalsB
    ...
    function readyCheck(
        ...
        uint256 tokenAPrice = tokenPrice(coinDeedAddressesProvider, tokenA);
        // *Note: Token A price has the decimals of the stable coin that used for price fetched in uniswap
        uint8 priceDecimals = IERC20MetadataUpgradeable(coinDeedAddressesProvider.stableCoinAddress()).decimals();
        ...
        require(tokenAPrice > 0, "Invalid uniswap answer");
        if (
            ...
            tokenAPrice / (10 ** priceDecimals) * // Oracle Price in USD of down payment token
    ...
    function checkRiskMitigationAndGetSellAmount(
        ...
        ICoinDeed.DeedParameters memory parameters, 
        ...
        ILendingPool lendingPool = ILendingPool(coinDeedAddressesProvider.lendingPool());
        uint256 totalDeposit = lendingPool.totalDepositBalance(pair.tokenB, address(this)) + pendingToken; //Include the B token that is currently stay in the deed
        ...
        /** To figure out how much to sell, we use the following formula:
          ...
          * If risk mitigation hasn't trigger but the adjusted leverage is 1, pay all the debt for the deed 
        **/
        if (!riskMitigationTriggered && riskMitigation.leverage == 1) {
            uint256 discountPercentage = ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).liquidationBonus();
            buyAmount = totalBorrow * BASE_DENOMINATOR / (BASE_DENOMINATOR - discountPercentage);
            sellAmount = tokenRatio(
                coinDeedAddressesProvider,
                pair.tokenA,
                buyAmount,
                pair.tokenB);
            if (sellAmount > totalDeposit) {
    ...
    function checkRiskMitigation(
        ...
        uint256 trigger = riskMitigationTriggered ?
            (riskMitigation.secondTrigger - riskMitigation.trigger) :
            riskMitigation.trigger;

        uint8 leverage = riskMitigationTriggered ? riskMitigation.leverage : parameters.leverage;
    ...
    function validateTokens(
        ...
        return(
            tokenPrice(coinDeedAddressesProvider, pair.tokenA) > 0 &&
            tokenPrice(coinDeedAddressesProvider, pair.tokenB)  > 0
        );
    }
    ...
    function validateExecutionTime(
        ...
        if (wholesaleId != 0) {
            ...
            if (wholesale.exitDeedLock > 0) {
                require(executionTime_.buyTimestamp + wholesale.exitDeedLock < executionTime_.sellTimestamp, "BAD_EXIT_LOCK_TIME");
            }
    ...
    /** Helps to fetch the price of token from uniswap.
    * Returns the token price in dollar. */
    function tokenPrice(
        ICoinDeedAddressesProvider coinDeedAddressesProvider,
        address _tokenAddress
    ) public view returns (uint256 tokenBAmount){
        ...
        address stableCoinAddress = coinDeedAddressesProvider.stableCoinAddress();
        ...
        if (_tokenAddress == address(0x00)) {
            ...
            tokenDecimals = IERC20MetadataUpgradeable(_tokenAddress).decimals();
            path[0] = _tokenAddress;
            path[1] = stableCoinAddress;
        }
        uint256 oneToken = 10 ** tokenDecimals;
        uint[] memory amounts = uniswapRouter1.getAmountsOut(oneToken, path);
        return amounts[1];
    }
    ...
    function _buyWithLeverageAndDeposit(
        ...
        ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).emitSwapExecuted(
            amountReceived, 
            tokenPrice(coinDeedAddressesProvider, pair.tokenB)
        );
    ...
    function exitDeed(
        ...
        if (payOff) {
            ...
            //Swap amount exit deed from token B to token A
            address[] memory path = new address[](2);
            if (pair.tokenA == address(0x00)) {
                path[0] = pair.tokenB;
            ...
            uint[] memory amounts = uniswapRouter1.getAmountsOut(exitAmount, path);

            ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).emitPayOff(msg.sender, amounts[1]);
        } else {
            uint256 amountReceived = _dexSwap(
                coinDeedAddressesProvider,
                pair.tokenB,
            ...
            * if the position is insolvent. Don't let it get to this point.*/
            ...
            ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory()).emitExitDeed(msg.sender, amountReceived - userBorrow);
        }
    }
    ...
    // Withdraw an amount of purchased token from lending pool
    function withdrawFromPool (
        ...
        if (lendingPool.poolActive(tokenAddress)) {
                lendingPool.withdraw(tokenAddress, amount);
        }
    }
