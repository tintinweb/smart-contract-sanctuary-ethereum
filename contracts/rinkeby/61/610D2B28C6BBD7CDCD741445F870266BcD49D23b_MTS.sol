...
abstract contract Ownable is Context {
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "Own");
        _setOwner(newOwner);
    }
...
contract MTS is Ownable, ERC721A, ReentrancyGuard {
  ...
  function setNFTUpgraded(uint256 tokenId) external {
    require(msg.sender == address(upgradePlan), "limits");
  ...
  function tokenURI(uint256 tokenId) override public view returns (string memory)
  {
    ...
    {
      string memory baseURI = _baseURI();
      return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString(),".txt")) : "";
    }
  }
