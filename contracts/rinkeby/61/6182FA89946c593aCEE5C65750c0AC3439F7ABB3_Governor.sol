...
/// @notice The DAO governor
contract Governor is UUPS, Ownable, EIP712, GovernorStorageV1 {
    ///                                                          ///
    ///                         CONSTANTS                        ///
    ///                                                          ///
    ...
    function initialize(
        ...
        if (_timelock == address(0)) revert INVALID_INIT();
        if (_token == address(0)) revert INVALID_INIT();
        ...
        settings.quorumVotesBps = uint16(_quorumVotesBPS);
    ...
    function propose(
        ...
        // require(_getVotes(msg.sender, block.timestamp - 1) >= proposalThreshold(), "BELOW_PROPOSAL_THRESHOLD");
        if (_getVotes(msg.sender, block.timestamp - 1) < proposalThreshold()) revert BELOW_PROPOSAL_THRESHOLD();

        uint256 numTargets = _targets.length;

        // require(numTargets != 0, "NO_TARGET");
        // require(numTargets == _values.length, "LENGTH_MISMATCH");
        // require(numTargets == _calldatas.length, "LENGTH_MISMATCH");
        ...
        // require(proposal.voteStart == 0, "PROPOSAL_EXISTS");
    ...
    function queue(uint256 _proposalId) public returns (uint256) {
        // require(state(_proposalId) == ProposalState.Succeeded, "PROPOSAL_NOT_SUCCESSFUL");
    ...
    function veto(uint256 _proposalId) external {
        // require(msg.sender == settings.vetoer, "ONLY_VETOER");
        // require(state(_proposalId) != ProposalState.Executed, "PROPOSAL_EXECUTED");
    ...
    function state(uint256 _proposalId) public view returns (ProposalState) {
        ...
        // require(proposal.voteStart != 0, "INVALID_PROPOSAL");
    ...
    function updateQuorumVotesBps(uint256 _newQuorumVotesBps) external onlyOwner {
        // TODO safecast + event
        settings.quorumVotesBps = uint16(_newQuorumVotesBps);
    }

    event VetoerUpdated(address oldVetoer, address newVetoer);
...
/// @title Timelock
/// @author Rohan Kulkarni
/// @notice This contract acts as a DAO treasury and transaction executor
contract Timelock is UUPS, Ownable, TimelockStorageV1 {
    ///                                                          ///
    ///                         CONSTANTS                        ///
    ///                                                          ///
    ...
    /// @dev The timestamp that represents an executed transaction
    ...
    /// @notice If a proposal is scheduled to execute or was already executed
    /// @param _proposalId The proposal id
    function isQueuedOrExecuted(uint256 _proposalId) public view returns (bool) {
        return timestamps[_proposalId] > 0;
    }

    /// @notice If a proposal is scheduled to execute
    /// @param _proposalId The proposal id
    function isQueued(uint256 _proposalId) public view returns (bool) {
        return timestamps[_proposalId] > EXECUTED;
    }

    /// @notice If a proposal is ready to execute
    /// @param _proposalId The proposal id
    ...
    /// @notice If a proposal was executed
    /// @param _proposalId The proposal id
    function isExecuted(uint256 _proposalId) public view returns (bool) {
        return timestamps[_proposalId] == EXECUTED;
    }

    /// @notice If a proposal was not executed after 14 days of being eligible
    /// @param _proposalId The proposal id
    ...
    function schedule(uint256 _proposalId) public onlyOwner {
        if (isQueuedOrExecuted(_proposalId)) revert ALREADY_QUEUED(_proposalId);
    ...
    function updateMinDelay(uint256 _newDelay) external {
    ...
    function _authorizeUpgrade(address _newImpl) internal view override onlyOwner {
        if (!manager.isValidUpgrade(_getImplementation(), _newImpl)) revert InvalidUpgrade(_newImpl);
    }
}
...
contract TimelockStorageV1 {
    /// @notice The minimum time between a queued transaction and its execution
