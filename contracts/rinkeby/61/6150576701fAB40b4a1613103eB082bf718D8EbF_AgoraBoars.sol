...
contract AgoraBoars is ERC721A, ERC2981, Ownable, ReentrancyGuard {

  bool public isWhitelistMintingOpened;
  bool public isPublicMintingOpened;

  uint16 public MAXCOLLECTIONSIZE = 7765;
  uint16 public WLLIMIT = 444;
  ...
  event ProxyRegistrySet(address proxyAddress);

  constructor(
    address owner,
    string memory _uri,
    address _proxyRegistryAddress)
    ERC721A("AgoraBoars","AB")
    {
      require(owner != address(0), "Please provide a valid owner");
      baseURI = _uri;
      proxyRegistryAddress = _proxyRegistryAddress;
      setDefaultRoyalty(owner, 450); // 4.5% fees
  ...
  function setProxyRegistry(address _proxyRegistryAddress) public onlyOwner {
    proxyRegistryAddress = _proxyRegistryAddress;
    emit ProxyRegistrySet(_proxyRegistryAddress);
  }
  ...
  function mint(uint16 count) external payable nonReentrant {
    require(msg.value == count * publicPrice(),"Invalid amount sent");
  ...
  function merkleTreeWLMint(uint16 _count, bytes32[] memory _proof) external payable nonReentrant {
    require(msg.value == _count * presalePrice(),"Invalid amount sent");
