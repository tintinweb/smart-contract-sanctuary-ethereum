...
// 1,641,013,200 Timestamp of Jan 1 2022
//    18,399,338 Difference
...
contract FinanceDept is AccessControl, Pausable {
    ...
    uint public maxClaimAmount = 10000 ether;
    ...
    constructor() { 
        ...
        timestamps = TimestampInterface(0xC842C8458D7Fb40b91cc49F785F46E6a00831ea8);
    }
...
// View Functions
    ...
    function unclaimedDuration2(uint _jobId) public view returns (uint) {
        uint _duration = block.timestamp - timestamps.get(_jobId);
        return Math.min(_duration, maxClaimTime);
    }

    function unclaimedByJob2(uint _jobId) public view returns (uint) {
        return salaries.salary(_jobId) * unclaimedDuration2(_jobId) / payDuration;
    } 
...
// CLAIM
    ...
    // Accepts a 2D array, with the first element of each sub-array being the companyId -- in numerical order
    function claim2(uint[][] memory _sortedIds, address _ownerAddr) public whenNotPaused {
        uint _numCompanies = _sortedIds.length;
        require(_numCompanies > 0, "nothing sent");
        uint _unclaimedWithBonus = 0;
        for (uint i = 0; i < _numCompanies; i++) {
