...
contract Theirsverse is ERC721A, Ownable, AccessControl, ReentrancyGuard, PaymentSplitter {
    using Strings for uint256;

    uint256 public immutable maxSupply;
    uint256 public immutable poapEventId;
    ...
    constructor(
        ...
        address _poapAddress,
        uint256 _poapEventId,
        uint256 _maxSupply,
        address _owner,
        string memory _baseURI
    ) ERC721A("Theirsverse NFT", "TVN") PaymentSplitter(_team, _teamShares) {
    ...
    modifier _saleBetweenPeriod(uint256 start, uint256 end) {
        require(currentTime() >= saleStartTime + start * 1 days, "sale has not started yet");
        require(currentTime() < saleStartTime + end * 1 days, "sale is finished");
        _;
    }
    ...
    function glodMemberMint(uint256 _quantity) external payable _notContract _saleBetweenPeriod(0, 1) nonReentrant {
        ...
        uint256 neededPoap = _quantity / 10;
        uint256[] memory poaps = verifiedPoap(msg.sender, neededPoap);
        require(poaps.length == neededPoap, "Goldmember does not own enought verified POAPs");
        for (uint256 i = 0; i < neededPoap; i++) {
            amountNFTsGoldMemberSale[poaps[i]] += 10;
        }
    ...
    function claim(address account, uint256 _quantity) external _notContract onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        require(totalSupply() + _quantity <= maxSupply, "Max supply exceed");
    ...
    function goldMemberMaxCount(address account) public view returns (uint256) {
        uint256 popaCounts = poap.balanceOf(account);
        uint256 count = 0;
        for (uint256 i = 0; i < popaCounts; i++) {
            (uint256 tokenId, uint256 eventId) = poap.tokenDetailsOfOwnerByIndex(account, i);
            if (eventId == poapEventId && amountNFTsGoldMemberSale[tokenId] < 1) {
                count = count + 1;
            }
        }
        return count;
    ...
    function setMerkleRoot(bytes32[4] memory _merkleRoots) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function verifiedPoap(address account, uint256 quantity) private view returns (uint256[] memory) {
        uint256 counts = poap.balanceOf(account);
