...
import "../extensions/NFTCollectionAirdrop.sol";
import "../extensions/NFTCollectionWhitelistPresale.sol";

contract NFTCollectionPresetAirdropWhitelist is
    NFTCollectionRoyalties,
    NFTCollectionPausableMint,
    NFTCollectionAirdrop,
    NFTCollectionWhitelistPresale
{
    constructor(
        string memory _name,
        ...
        NFTCollectionWhitelistPresale(_whitelistRoot)
    {}

    function mint(uint256 _amount)
        public
        payable
        override(NFTCollection, NFTCollectionWhitelistPresale)
    {
        NFTCollectionWhitelistPresale.mint(_amount);
    }
...
abstract contract NFTCollectionAirdrop is NFTCollection {
    function _mintTo(address[] calldata _recipients) internal {
        uint256 recipientsLength = _recipients.length;
        if (_totalMinted() + recipientsLength > maxSupply) {
...
abstract contract NFTCollectionWhitelistPresale is NFTCollection {
    ...
    function mint(uint256 _amount) public payable virtual override {
        if (!publicSale) revert NotInPublicSale();
        super.mint(_amount);
    }
