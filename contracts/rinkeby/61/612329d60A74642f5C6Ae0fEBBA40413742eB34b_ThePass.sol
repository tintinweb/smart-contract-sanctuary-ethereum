    ...
    ERC2981
{
    ...
    modifier onlyPublic(uint256 passSaleId, uint256[] calldata quantities) {
        _checkPurchase(passSaleId, quantities, 0);
        require(
    ...
    modifier onlyPresale(
        ...
        _checkPurchase(passSaleId, quantities, allowed);
        bytes32 merkleRoot = MultiSaleStorage.layout().merkleRoot[passSaleId];
        require(
            MultiSaleStorage.layout().presaleActive[passSaleId],
            "Presale not active"
        );
        ...
        require(
            MerkleProof.verify(
                proof,
                MultiSaleStorage.layout().merkleRoot[passSaleId],
    ...
    function purchasePass(
        ...
        uint256[] calldata quantities
    ) external payable onlyPublic(passSaleId, quantities) {
        _batchPurchasePasses(passSaleId, passTypeIds, quantities);
    }
    ...
    function presalePurchasePass(
        ...
        bytes32[] calldata proof,
        uint256 allowed
    ) external payable onlyPresale(passSaleId, quantities, allowed, proof) {
        _batchPurchasePasses(passSaleId, passTypeIds, quantities);
    }
    ...
    function createPassOption(
        ...
        AppStorage.layout().passesForSaleById[passSaleId][
            passTypeId
        ] = AppStorage.PricingEntry({price: price, isValue: true});
    ...
    function updatePassOption(
        ...
        AppStorage
        .layout()
        .passesForSaleById[passSaleId][passTypeId].isValue = active;
        AppStorage
        .layout()
        .passesForSaleById[passSaleId][passTypeId].price = newPrice;

        emit PassOptionUpdated(passSaleId, passTypeId, newPrice, active);
    }
    ...
    function startSale(
        ...
        MultiSaleStorage.layout().maxAmount[passSaleId] = newMaxAmount;
        MultiSaleStorage.layout().maxPerWallet[passSaleId] = newMaxPerWallet;
    ...
    function setMerkleRoot(uint256 passSaleId, bytes32 newRoot)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        MultiSaleStorage.layout().merkleRoot[passSaleId] = newRoot;
    }
    ...
    function getPrice(uint256 passSaleId, uint256 passTypeId)
        ...
        returns (uint256)
    {
        AppStorage.PricingEntry memory pass = AppStorage
            .layout()
    ...
    function maxAmount(uint256 passSaleId) external view returns (uint256) {
        return MultiSaleStorage.layout().maxAmount[passSaleId];
    }
    ...
    function maxPerWallet(uint256 passSaleId) external view returns (uint256) {
        return MultiSaleStorage.layout().maxPerWallet[passSaleId];
    }
    ...
    function _batchPurchasePasses(
        ...
        for (uint256 i = 0; i < length; ) {
            _purchasePass(passSaleId, passTypeIds[i], quantities[i]);
            unchecked {
                i++;
            }
        }
    ...
    function _mintPass(
        ...
        unchecked {
            ...
            MultiSaleStorage.layout().mintCount[passSaleId][to] += quantity;
        }
    ...
    function _checkPurchase(
        ...
        uint256 maxForSale = MultiSaleStorage.layout().maxAmount[passSaleId];
        uint256 quantity = 0;
        uint256 length = quantities.length;
        for (uint256 i = 0; i < length; ) {
            unchecked {
        ...
        require(
            quantity <= MultiSaleStorage.layout().maxPerMint[passSaleId],
            "Exceeded max per mint"
        );
        require(
            allowed > 0 ? quantity <= allowed : true,
            "Exceeded max per wallet"
        );
        require(
            MultiSaleStorage.layout().mintCount[passSaleId][_msgSender()] +
                quantity <=
...
library AppStorage {
    ...
    struct Layout {
        ...
        string symbol;
        // passes per sale id
        mapping(uint256 => mapping(uint256 => PricingEntry)) passesForSaleById;
    }
