...
contract Sphinx is IERC721Receiver {
    INonfungiblePositionManager public nonfungiblePositionManager;
    address public constant WTH = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
    address public constant TEST = 0x53470bE8b7E49BCF452ee5177B2bED14B6dB928a;
    ...
    /// For this example we are providing 1000 WTH and 1000 TEST in liquidity
        ...
        )
    {
        ...
        TransferHelper.safeTransferFrom(WTH, msg.sender, address(this), amount0ToMint);
        TransferHelper.safeTransferFrom(TEST, msg.sender, address(this), amount1ToMint);

        // Approve the position manager
        TransferHelper.safeApprove(WTH, address(nonfungiblePositionManager), amount0ToMint);
        TransferHelper.safeApprove(TEST, address(nonfungiblePositionManager), amount1ToMint);
        ...
        // Note that the pool defined by WTH/TEST and fee tier 0.3% must already be created and initialized in order to mint
        ...
        if (amount0 < amount0ToMint) {
            TransferHelper.safeApprove(WTH, address(nonfungiblePositionManager), 0);
            uint256 refund0 = amount0ToMint - amount0;
            TransferHelper.safeTransfer(WTH, msg.sender, refund0);
        }

        if (amount1 < amount1ToMint) {
            TransferHelper.safeApprove(TEST, address(nonfungiblePositionManager), 0);
            uint256 refund1 = amount1ToMint - amount1;
            TransferHelper.safeTransfer(TEST, msg.sender, refund1);
        }
    }
