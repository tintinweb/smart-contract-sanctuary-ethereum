...
contract CrossMintDrop is ERC721A, Ownable, ReentrancyGuard {
    ...
    modifier requireCrossmint() {
        require(msg.sender == crossmintAddress , "Crossmint only");
        _;
    }
    ...
    // Set crossmint merkleroot
    function setCrossMintMerkleRoot(bytes32 root) external onlyOwner {
        crossmintMerkleRoot = root;
    }

    function setCrossMintAddress(address _address) external onlyOwner {
        crossmintAddress = _address;
    }

    // Crossmint public minting without a proof
    function crossmintTo(
        uint256 qty, 
        address to
    ) external payable requireCrossmint {
        ...
        _mint(to, qty);
        quantityPerWallet[to] += qty;
    }

    // Crossmint presale or public minting. Requires proof if presale
        ...
        requireCrossmint
    {
        ...
        require(
            quantityPerWallet[to] + qty <= MAX_SUPPLY_PRESALE_PER_WALLET,
        ...
        require(verify(crossmintMerkleRoot, merkleProof, leaf), "PreSale: Not mintlisted");
