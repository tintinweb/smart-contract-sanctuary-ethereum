  ...
  ReentrancyGuard
{
    ...
    if (_isPresale) {
      ...
      for (uint256 i = 0; i < presaleContractsLength; i++) {
        ...
        if (
          ...
          PresaleContract721Interface(presaleContract.contractAddress)
            .balanceOf(sender) >
          0
        ) {
          isEligible = true;
    ...
    hasEnoughEther(1)
  {
    ...
    if(!MerkleProof.verify(proof, presaleMerkleRoot, keccak256(abi.encodePacked(msg.sender)))) {
  ...
  function setPresaleMerkleRoot(bytes32 _presaleMerkleRoot) external onlyRole(DEFAULT_ADMIN_ROLE) {
