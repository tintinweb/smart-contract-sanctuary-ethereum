...
contract DemiVault is IERC4626, ERC20 {
    ...
    function deposit(uint256 assets, address receiver) public payable returns (uint256) {
        require (assets > 0, "Deposit less than Zero");
        uint256 shares = (assets * totalAssets()) / totalSupply;

        asset.transferFrom(receiver, address(this), assets);
        shareHolder[receiver] += assets;
