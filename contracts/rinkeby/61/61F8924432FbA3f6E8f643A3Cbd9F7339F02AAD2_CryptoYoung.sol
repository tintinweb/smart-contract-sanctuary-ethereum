...
// File: CryptoYoung.sol

pragma solidity ^0.8.0;

contract CryptoYoung is ERC721A, Ownable, ReentrancyGuard {
    ...
    function reserve(address recipient, uint256 tokenQuantity) external onlyOwner {
        require(recipient != address(0), "CryptoYoung: zero address");
        require(tokenQuantity > 0, "CryptoYoung: invalid amount");
        require(
            totalSupply() + tokenQuantity <= collectionSize,
            "CryptoYoung: Exceed max supply"
        );
        require(
            totaltokensReserved + tokenQuantity <= maxreserveAmount,
            "CryptoYoung: Exceed max reserve"
        );
    ...
    function mint(uint256 tokenQuantity) external payable {
        require(status == Status.PublicSale, "CryptoYoung: Public mint is not active");
        require(
            tx.origin == msg.sender,
            "CryptoYoung: contract is not allowed to mint."
        );
        require(
            numberMinted(msg.sender) + tokenQuantity <= MAX_MINT,
            "CryptoYoung: Max public mint amount per wallet exceeded"
        );
        require(
    ...
    returns (string memory)
    {
        require(
            _exists(tokenId),
            "CryptoYoung: URI query for nonexistent token"
        );
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "CryptoYoung: Not enough ether sent");
        if (msg.value > price) {
