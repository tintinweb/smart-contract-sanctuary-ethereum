...
contract RareEvent is ERC721URIStorage, OwnableExtended {
	...
	mapping(uint => string) public eventIdToFilmId;

	constructor(string memory tokenURI, string memory contractURI) ERC721("RareEvent", "ROE"){
	...
	function _setBaseContractURI(string memory uri) internal virtual {
		baseContractURI = uri;
	}

	function setBaseURI(string memory tokenURI, string memory contractURI) public onlyOperator {
		_setBaseTokenURI(tokenURI);
		_setBaseContractURI(contractURI);
	}

	function createEvent(address _to, string memory _filmId) external onlyOperator returns (address to, string  memory filmId, uint eventId) {
		tokenIds.increment();
		...
		eventIdToFilmId[_tokenId] = _filmId;

		emit Create(_to, _filmId, _tokenId);
		return (_to, _filmId, _tokenId);
	}
...
contract OwnableExtended is Ownable {
	...
	modifier onlyOperator() virtual {
		...
		require(
			caller == owner() || operatorApprovals[caller],
