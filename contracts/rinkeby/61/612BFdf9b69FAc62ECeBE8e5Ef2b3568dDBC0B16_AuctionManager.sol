...
contract AuctionManager is IAuctionManager, AccessControl {
        ...
        returns (uint104)
    {
        ...
        if (auction.endsAt < block.timestamp) revert AuctionManager_AuctionEnded();

        uint104 newBid = normalizeBid(value);
        uint104 increase = newBid - bids[id][sender];

        if (newBid < auction.reservePrice) revert AuctionManager_InvalidBid();

        if (auction.endsAt - block.timestamp > bidConfig.snipeThreshold)
            auction.endsAt += bidConfig.timeExtension;
