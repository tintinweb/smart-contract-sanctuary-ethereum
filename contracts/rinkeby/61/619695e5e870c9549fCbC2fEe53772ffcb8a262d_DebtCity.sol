...
*/
    ...
    constructor() ERC721("DebtCity", "BANKERS") {
        _owner = msg.sender;
        payableWallet = payable(address(0x94B47E4fFa7a7143561AD953C835121475434Ec1));
    ...
    function hash(
        ...
        for (uint8 i = 0; i < 15; i++) {
            ...
            uint8 randDigit = applyBankerRules(i, bstyle, randVal);
    ...
    function mintInternal() internal {
        ...
        uint256 thisBankerId = _totalSupply;
        bankerIdToHash[thisBankerId] = hash(thisBankerId, msg.sender, 0);
        hashToMinted[bankerIdToHash[thisBankerId]] = true;

        _mint(msg.sender, thisBankerId);
    }

    function mintBanker() public {
    ...
    function applyBankerRules(uint8 i, uint8 bstyle, uint8 randDigit) pure internal returns (uint8) {
        ...
        if (i == 14 && randDigit > 7) randDigit = 0;
        else if (i == 6 && randDigit > 5) randDigit = 0;
        else if(i == 7 && (randDigit > 5 || bstyle == 5)) randDigit = 0; 
        else if (i == 8 || i == 9) {
        ...
        else if (i == 13 && bstyle > 1) randDigit = 0;
        ...
        for (uint8 i = 0; i < 15; i++) {
            ...
            metadataString = string(
                abi.encodePacked(
                    metadataString,
                    '{"trait_type":"',
                    traitData[i][thisTraitIndex].ttype,
                    '","value":"',
                    traitData[i][thisTraitIndex].name,
                    '"}'
                )
            );
    ...
    function outputFullSVG(string memory _hash) public view returns (string memory) {
        ...
        for (uint8 i = 0; i < 15; i++) {
            ...
            svgString = string(abi.encodePacked(svgString, drawTrait(traitData[i][thisTraitIndex])));
        }
