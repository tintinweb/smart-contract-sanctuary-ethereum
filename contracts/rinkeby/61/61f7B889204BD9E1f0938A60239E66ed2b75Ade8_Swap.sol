...
contract Swap is Initializable, OwnableUpgradeable, ChainlinkClientUpgradeable {
    ...
    uint256[] public rate = new uint256[](2);//0 index contains buy and 1 contains sell
    uint public rateFromChainlink;
    bool public chainlinkFeedEnabled; // true=> price to be fetched from chainLink, False => fetch from API
    ...
    //TODO implement checks for verified Node
    ...
    event RateTimeoutChanged(uint256 newTimeout);
    ...
    function initialize(
        ...
        rateFromChainlink = uint256(price); // to make the value 10**18 compatible
        
        //Load both buy and sell rate
        setPriceFeed(BUY_INDEX);
        setPriceFeed(SELL_INDEX);   
    }
    ...
    function swap(uint256 _amountIn, address _from, address _to) external {
        ...
        if (_from == tokenA) {
            ...
            uint256 amountB = fetchPrice(SELL_INDEX,amountA);

            if(chainlinkFeedEnabled){
                setPriceFeed(SELL_INDEX);
                require(!isRateTimeout(SELL_INDEX), "Rate timeoeut expired");
            }
            ...
            emit Swapped(msg.sender, _amountIn, amountA, SELL_INDEX);
        } else {
            uint256 amountB = _amountIn - amountFee;
            uint256 amountA = fetchPrice(BUY_INDEX, amountB);
            if(chainlinkFeedEnabled){
                setPriceFeed(BUY_INDEX);
                require(!isRateTimeout(BUY_INDEX), "Rate timeoeut expired");
            }
            ...
            emit Swapped(msg.sender, _amountIn, amountA, BUY_INDEX);

        }
    }

    function fetchFromChainLink() internal returns(uint256){
         (
            /*uint80 roundID*/,
            int price,
            /*uint startedAt*/,
        ...
        require(price >= 0,"ChainLink price error");
        rateFromChainlink = uint256(price);
        return rateFromChainlink;
    }

    function fetchFromAPI(uint _index,uint _amount) internal view returns(uint256){
        if(_index==SELL_INDEX){
           return _amount * lastPriceFeed[SELL_INDEX] / (10**8);
        }
        else{
            if(msg.sig == this.addLiquidity.selector || msg.sig == this.removeLiquidity.selector){
                return _amount * (10**8) / rate[BUY_INDEX];
            }
           return _amount * (10**8) / lastPriceFeed[BUY_INDEX];
        }
    }

    function fetchPrice(uint _index,uint _amount) internal returns(uint) {
        if(chainlinkFeedEnabled){
            return fetchFromChainLink() * _amount / (10**8);
        }
        else{
            return fetchFromAPI(_index,_amount);
        }
    }

    function setPriceFeed(uint256 _isSale) internal {
        if(isRateTimeout(_isSale)) { 
            chainlinkRequestId[_isSale] = requestVolumeData(_isSale); 
            lastTimeStamp[_isSale] = block.timestamp; 
            lastPriceFeed[_isSale] = rate[_isSale]; 
        }
    }

    function isRateTimeout(uint256 _isSale) view internal returns(bool){
        if(lastTimeStamp[_isSale] == 0)
            return true;
        if(block.timestamp - lastTimeStamp[_isSale] > rateTimeOut)  
            return true; // yes the price feed is expired
        else
            return false; // no the price feed is not expired
    }

    function addLiquidity(uint256 amountA, uint256 amountB) external onlyOwner {
        require(fetchPrice(BUY_INDEX, amountA) == amountB, "amountA should be equal with amountB");
        TransferHelper.safeTransferFrom(tokenA, msg.sender, address(this), amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, address(this), amountB);
        reserveA = reserveA + amountA;
        reserveB = reserveB + amountB;
    ...
    function requestVolumeData(uint256 flag) public returns (bytes32 requestRate) {
        ...
        if(flag == BUY_INDEX) // get buy request
    ...
    function fulfill(bytes32 _requestId, uint256 _rate) public recordChainlinkFulfillment(_requestId) {
        ...
        if(_requestId == chainlinkRequestId[BUY_INDEX]){
            rate[BUY_INDEX] = _rate;
            if(lastPriceFeed[BUY_INDEX]== 0)
                lastPriceFeed[BUY_INDEX] = _rate;
        }
        if(_requestId == chainlinkRequestId[SELL_INDEX]){
            rate[SELL_INDEX] = _rate;
            if(lastPriceFeed[SELL_INDEX]== 0)
                lastPriceFeed[SELL_INDEX] = _rate;
        }
    }
   ...
   function setTradeFee(uint256 _newTradeFee) external onlyOwner {
    ...
    function toggleChainLinkFeed(bool _flag) external onlyOwner{
        require(_flag!= chainlinkFeedEnabled, "Already in desired pause state");
    ...
    function getRate(uint256 _rateIndex) external view returns (uint256) {
        return rate[_rateIndex];
    }

    /// @dev Get buy or sell lastPricefeed 
    /// @param _flag 0 for buy 1 for sell
    /// @return Either buy  or sell lastPricefeed
    function getLastPriceFeed(uint _flag) external view returns(uint256){
        return lastPriceFeed[_flag];
    }
}
...
abstract contract ChainlinkClientUpgradeable {
  ...
  function initialize() public notInitialized() {
    ...
    LINK_TOKEN_POINTER = 0xa36085F69e2889c224210F603D836748e7dC0088;
    s_requestCount = 1;
  }
