...
contract W3bScholarsV2 is ERC721A, IERC2981, Ownable, ReentrancyGuard {
    ...
    function isAllowListAlreadyMinted(address addr) external view returns (uint256) {
        return _alreadyMinted[addr];
    }

    function mintAllowList(bytes32[] calldata merkleProof)
        public
        nonReentrant
    {
    ...
    function collabMint(uint256[][] calldata tokenIndexes) external {
        require(tokenIndexes.length == collabAddresses.length, "Mismatch tokenIndexes length");
        require(isSaleStarted, "Sale must be started to mint");
        require(!isSaleFinalized, "Sale is already finalized");
        ...
        for (uint256 i = 0; i < collabAddresses.length; i++) {
            for (uint256 n = 0; n < tokenIndexes[i].length; n++) {
                ...
                if (collabMintCounts[i] == MAX_MINT_PER_COLLAB || totalSupply() + quantity >= MAX_SUPPLY) {
                    break;
                } else {
    ...
    function mintGenesis(uint256[] calldata tokenIndexes) external {
        require(isSaleStarted, "Sale must be started to mint");
        require(!isSaleFinalized, "Sale is already finalized");
        require(totalSupply() + tokenIndexes.length <= MAX_SUPPLY, "Purchase would exceed max supply");
        ...
        for(uint256 i = 0; i < tokenIndexes.length; i++) {
            require(tokenIndexes[i] > 0 && tokenIndexes[i] < 22, "Token number is invalid");
            require(scholarContract.ownerOf(tokenIndexes[i]) == msg.sender, "Token is not owned by sender");
            // what happens if token number provided goes outside the bounds of array?
            require(!genesisMinted[tokenIndexes[i]-1], "Token already used to mint");
        }
    ...
    function isGenesisMinted(uint256[] calldata tokenIndexes) external view returns (bool[] memory) {
    ...
    function withdraw() public onlyOwner {
        ...
        uint256 minterProceeds = ((contractBalance * MINTER_EQUITY_BASIS) / 10000) / ts; // check for overflow
