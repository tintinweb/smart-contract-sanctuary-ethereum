...
contract RewardVesting is Ownable, ERC1155Receiver, IRewardVesting {
        ...
        onlyStakingcontract
    {
        if (rewardsInfo[account].isIntialized) {
            ...
            rewardsInfo[account] = Reward(false, 0, 0, tokenAmount, 0, true);
        }
    }
        ...
        returns (uint256 tokens)
    {
        uint256 availablePercentage = _calculateAvailablePercentage(
            reward._initialTimestamp,reward.numberOfHeros
        );
    ...
    function _calculateAvailablePercentage(uint256 intialTimeStamp,uint256 rate)
        private
        view
        returns (uint256 availablePercentage)
    {
        ...
        uint256 noOfSecondsRemaining = uint256(182).mul(86400).div(rate);
    ...
    function withDrawHeros(uint256 numberOfHeros) external {
        require(rewardsInfo[msg.sender].numberOfHeros>=numberOfHeros, "Heros Not Deposited");
