...
function closeOrder(bytes32 _createdOrder) public {
...
function createOrder(uint _price,address _target,uint _nftId,uint _amount,IERC20 _paymentToken,bytes calldata _data)public returns(bytes32){
    ...
    order.paymentToken.transferFrom(address(msg.sender),address(this),order.price);
    order.paymentToken.transfer(order.seller, _amount);
    if (_amount >=1){
...
// token.transfer(address(this), address(msg.sender), _amount);
// }
}
