...
contract Racers is RewardRance {
    ...
    function tukracers(uint256 tokenId) public {
        ...
        uint16 rankCm = victoryProbabilityCM[rand];
        uint16 rankRare = victoryProbabilityRare[rand];
        uint16 rankEpic = victoryProbabilityEpic[rand];
        console.log(rankCm);
        console.log(rankRare);
        console.log(rankEpic);
...
contract RewardRance is Tuktukfactory {
    ...
    function getRewardByCar(address _owner,uint256 productId)
        external
        view
        returns (uint16)
    {
        uint16 coins = 0;
        for (uint256 i = 0; i < rewards.length; i++) {
            if (rewardToOwner[i] == _owner && productId == rewards[i].productId) {
                coins +=  rewards[i].coin;
            }
        }
        return coins;
    ...
    function resetRewardCar(address _owner,uint256 productId)
        external
    {
        for (uint256 i = 0; i < rewards.length; i++) {
            if (rewardToOwner[i] == _owner && productId == rewards[i].productId) {
