...
contract CompetitionMinter is IRelicMinter, Ownable
{
    ...
    mapping(uint16 => Competition) private _competitions;
    ...
    function setCompetition(uint16 competitionId, string memory provenance, uint256 startTokenId, uint256 endTokenId, bytes32 allowlistRoot) public onlyOwner {
        Competition storage comp = _competitions[competitionId];
        comp.nextTokenId = startTokenId;
        comp.endTokenId = endTokenId;
        comp.merkleRoot = allowlistRoot;
        comp.provenance = provenance;
        // _competitions[competitionId] = comp; // eh?
    }

    function getCompetition(uint16 competitionId) public view returns(CompetitionInfo memory) {
        if (_competitions[competitionId].merkleRoot == 0) {
        ...
        return CompetitionInfo({
           nextTokenId: _competitions[competitionId].nextTokenId,
           provenance: _competitions[competitionId].provenance,
           valid: true
        });
    }

    function canClaim(uint16 competitionId, address claimer, bytes32[] calldata proof) public view returns(bool) {
        bytes32 leaf = keccak256(abi.encodePacked(claimer));
        // confirm not already claimed
        return !_competitions[competitionId].claimed[claimer] &&
            _competitions[competitionId].merkleRoot != 0 &&
            _competitions[competitionId].nextTokenId <= _competitions[competitionId].endTokenId &&
            MerkleProof.verify(proof, _competitions[competitionId].merkleRoot, leaf);
    }

    function claim(uint16 competitionId, uint16 orderIndex, bytes32[] calldata proof) public {
        // check claimable
        require(canClaim(competitionId, msg.sender, proof), "not able to claim");
        // mark as claimed
        _competitions[competitionId].claimed[msg.sender] = true;
        // stamp with order and competition
        bytes12 data = packData(competitionId, orderIndex);
        _relic.mint(msg.sender, _competitions[competitionId].nextTokenId, data);
        // inc token for any future claims
        _competitions[competitionId].nextTokenId++;
    }

    function packData(uint16 competitionId, uint16 orderIndex) public pure returns (bytes12) {
        return bytes12(bytes2(competitionId)) >> 16 | bytes2(orderIndex);
    }

    function unpackData(bytes12 data) public pure returns (uint16 competitionId, uint16 orderIndex) {
        return (uint16(bytes2(data << 16)), uint16(bytes2(data)));
    }

    function getTokenOrderIndex(uint256 /*tokenId*/, bytes12 data) external override pure returns(uint) {
        uint16 competitionId;
        uint16 orderIndex;
        (competitionId, orderIndex) = unpackData(data);
        return orderIndex;
    }

    function getTokenProvenance(uint256 /*tokenId*/, bytes12 data) external override view returns(string memory) {
        uint16 competitionId;
        uint16 orderIndex;
        (competitionId, orderIndex) = unpackData(data);
        return _competitions[competitionId].provenance;
    }

    function getAdditionalAttributes(uint256 /*tokenId*/, bytes12 /*data*/) external override pure returns(string memory) {
        return "";
    }

    function getImageBaseURL() external override view returns(string memory) {
        require(bytes(imageBaseURL).length > 0, "imageBaseURL not set");
        return imageBaseURL;
    }
}
