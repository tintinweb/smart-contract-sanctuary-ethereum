...
contract DemonicSkulls is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 MAX_LEVEL_TWO_AMOUNT = 25;
    ...
    function mintForAddress(address wallet, uint256 level, uint256 id) virtual external {
        require(msg.sender == restorationContract, "Invalid caller");
        if (level == LEVEL_ONE_BLOOD_AMOUNT) {
            _safeMint(wallet, id);
        }
        if (level == LEVEL_TWO_BLOOD_AMOUNT) {
        ...
        if (level == LEVEL_THREE_BLOOD_AMOUNT) {
    ...
    function mintRefund(address wallet) virtual external {
        ...
        for (uint256 i = 0; i < 2; i++) {
            ...
            _safeMint(wallet, ++levelTwoIndex);
        }
    }
    ...
    function claimSkulls(uint256[] memory ids, uint256 bloodAmount) public {
        require(claimIsActive, "Claim not active");
        require(bloodAmount == LEVEL_ONE_BLOOD_AMOUNT || bloodAmount == LEVEL_TWO_BLOOD_AMOUNT,
            "Wrong level");
