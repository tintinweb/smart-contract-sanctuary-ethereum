...
contract Elements is ERC721Enumerable, Ownable {
    ...
    string public baseURI = "https://elements.blue/nft/";
    ...
    constructor() ERC721("Elements", "Elements") {
        ...
        elements["M"] = Element({name: "M Creature", parents: _parents2,supply: 0,supplyMass:0, tier:1});
        ...
        elements["L"] = Element({name: "L Creature", parents: _parents3,supply: 0,supplyMass:0, tier:1});
        elementIds.push(bytes1('L'));

        bytes1[] memory _parents4=new bytes1[](2);
        _parents4[0]=bytes1('2');
        _parents4[1]=bytes1('4');
        elements["E"] = Element({name: "E Creature", parents: _parents4,supply: 0,supplyMass:0, tier:1});
        ...
        elements["D"] = Element({name: "D Creature", parents: _parents5,supply: 0,supplyMass:0, tier:1});
        ...
        elements["F"] = Element({name: "F Creature", parents: _parents7,supply: 0,supplyMass:0, tier:2});
    ...
    function combine (uint256[]  memory  _tokenIds, uint256    _burnTokenId) external payable {
        ...
        //_mintWithoutValidation(msg.sender, _resultElementId, newMass,newPower,newImmunity,newExperience);
    }
        ...
        // Burn Parents and Mint if there is additional mass
         ...
         // Burn Burn Element
          ...
          if(burnTokenDetails.mass>1){
                ...
                //_mintWithoutValidation(msg.sender, burnTokenDetails.elementId, burnTokenDetails.mass-1);  
          }else{
    ...
    function _burnToken(uint256 tokenId  ) internal {
        //NFTDetails memory tokenDetails= tokenInfo[tokenId];
        //elements[tokenDetails.elementId].supply-=tokenDetails.mass;
    ...
    function _mintWithoutValidation(address to, bytes1 elementId, uint32 mass, uint32 power,  uint32 immunity,  uint32 experience) internal {
        ...
        // Add token Count
        //elements[elementId].supply+=mass;
        lastTokenId++;
        // Add token metadata
    ...
    function totalSupply() public view returns (uint256) {
        ...
        for(uint i = 0; i < elementIds.length; i++) {
            total+= elements[elementIds[i]].supply;
        }
        return total;
    }*/
    ...
    function updateNftElement(
        ...
        //elements[elementId].description =bytes(description);
    ...
    function elementExists(bytes1  _elementId) internal view returns (bool ){
     ...
     /*  function getElement(string memory elementId) public view returns (uint32[] memory ){
        bytes1   _elementId= bytes1( bytes(elementId) );
         require(elementExists(_elementId),"No such element exists"); 
         uint32[] memory supply = new uint32[](2);
         supply[0]=elements[_elementId].supply;
         supply[1]=elements[_elementId].supplyMass;
         return supply;
    } */
