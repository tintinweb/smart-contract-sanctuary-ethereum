...
contract Something is ERC721A, Ownable, ReentrancyGuard {
    ...
    // 0 -- None period
    // 1 -- Free mint period
    // 2 -- Whitelist mint period
    // 3 -- Public sale mint period
    uint256 public periodState = 0;

    constructor(string memory uri) ERC721A("It's Something", "STH") {
        baseURI = uri;
        // freemint period
        _addPeriod(2, 2222, 0, false);
        // whitelist period
        _addPeriod(2, 4000, 0, true);
        // public sale period
    ...
    function _addPeriod(uint256 limitPerWallet, uint256 supply, uint256 price, bool sign) internal {
        ...
        periods[idx].limitPerWallet = limitPerWallet;
        periods[idx].supply = supply;
        periods[idx].curSupply = 0;
        periods[idx].price = price;
        periods[idx].sign = sign;
    }
    ...
    function mint(uint256 quantity, bytes calldata sign) external payable nonReentrant {
        ...
        require((period.curSupply + quantity <= period.supply), "period supply limit");
        require((period.mintList[msg.sender] + quantity <= period.limitPerWallet) && (quantity > 0), "quantity limit");

        period.mintList[msg.sender] += quantity;

        period.curSupply += quantity;

        uint256 price = period.price * quantity;
