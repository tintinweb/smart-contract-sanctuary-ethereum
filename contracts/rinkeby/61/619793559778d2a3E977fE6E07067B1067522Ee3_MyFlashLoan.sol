...
contract MyFlashLoan is IUniswapV2Callee,Ownable{
    ...
    function execute(address _tokenA,address _tokenB,uint amount) external onlyOwner{
        IUniswapV2Pair(IUniswapV2Factory(v2Router.factory()).getPair(_tokenA, _tokenB))
            .swap(amount,0,address(this),bytes('do'));//*10**18
    }
    ...
    function uniswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external override{
        ...
        IUniswapV2Factory factory = IUniswapV2Factory(v2Router.factory());
        ...
        //还钱
        uint balanceTokenA = ERC20(address(tokenA)).balanceOf(address(this));
        require(balanceTokenA>amount0, "balance must gt amount0");
        ERC20(address(tokenA)).approve(pairAddress,MAX_INT);
        ERC20(address(tokenA)).transferFrom(address(this),msg.sender, amount0);
    }
}
