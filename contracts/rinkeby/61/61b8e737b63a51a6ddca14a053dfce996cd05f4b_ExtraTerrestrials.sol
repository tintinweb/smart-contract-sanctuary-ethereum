...
// File: contracts/ExtraTerrestrials.sol
...
contract ExtraTerrestrials is ERC721r, Pausable, Ownable {
    using Strings for uint256;

    uint256 public constant MAX_SUPPLY = 7777;
    uint256 public constant MAX_PUBLIC_MINT = 5;

    string public BASE_URI = "https://ipfs.io/ipfs/QmRT9TsLRV76dvaVZXHWBZNvFYpJAqJSjUb8TX1AmT8dXg/";

    mapping(address => uint256) public totalPublicMint;

    constructor() ERC721r("Extra Terrestrials", "ETW", MAX_SUPPLY) {}
    ...
    modifier callerIsUser() {
        require(tx.origin == msg.sender, "Extra Terrestrials :: Cannot be called by a contract");
        _;
    }

    function PriceUpdate(uint256 _supply) internal pure returns (uint256 _cost){
        if(_supply < 2){            
            return 0.091 ether;
        }
        if(_supply < 4 ) {
    ...
    function getPrice() public view returns (uint256 price){
        return PriceUpdate(totalSupply());
    }

    function mint(uint _quantity) public payable whenNotPaused callerIsUser {
        uint256 supply = totalSupply();
        require(_quantity > 0, "Extra Terrestrials :: Invalid Amount!");
        require(_quantity <= MAX_PUBLIC_MINT, "Extra Terrestrials :: Invalid Amount!");
        require((supply + _quantity) <= MAX_SUPPLY, "Extra Terrestrials :: Beyond Max Supply");
        require((totalPublicMint[msg.sender] +_quantity) <= MAX_PUBLIC_MINT, "Extra Terrestrials :: Already minted 5 times!");                
        require(msg.value >= PriceUpdate(supply) * _quantity, "Extra Terrestrials :: Not enough ETH");
