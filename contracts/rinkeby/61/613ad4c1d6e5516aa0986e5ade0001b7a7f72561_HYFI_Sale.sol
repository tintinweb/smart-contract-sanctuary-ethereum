...
contract HYFI_Sale is Initializable, AccessControlUpgradeable {
        ...
        possiblePurchaseUntilHardcap(amount)
    {
        ...
        uint256 discount = discountPercentageCalculator(amount);
        if (buyWithHYFI) {
        ...
        possiblePurchaseUntilHardcap(amount)
    {
        _buyWithCurrency(
            amount,
            discountPercentageCalculator(amount),
            referralCode
        );
    ...
    function discountPercentageCalculator(uint256 unitAmount)
        public
        view
        virtual
    ...
    function _mintVaults(address user, uint256 amount) internal {
    ...
    function _availableForClaim(address user) internal view returns (uint256) {
        return _totalReservedAmount(user).sub(claimed[user]);
    }

    //changed to public temporary, should be
    function _totalReservedAmount(address user) public view returns (uint256) {
    ...
    function _totalReservedAmountTest(address user) public returns (uint256) {
        (uint256 presaleTotalAmountBought, , ) = presale.getBuyerData(user);
        // todo:add offline logic here for total reserved amount calculation
        return presaleTotalAmountBought;
    }
    ...
    //changed to public temporary, should be
    function _totalReservedAmountSimple(address user)
        public
        view
        returns (uint256)
    {
        uint256 presaleTotalAmountBought = presale.getBuyerReservedAmount(user);
        // todo:add offline logic here for total reserved amount calculation
        return presaleTotalAmountBought;
    }

    function _totalReservedAmountTestSimple(address user)
        public
        returns (uint256)
    {
        uint256 presaleTotalAmountBought = presale.getBuyerReservedAmount(user);
