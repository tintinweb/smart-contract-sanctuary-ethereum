...
contract TreasureTracker is ERC721Enumerable, Ownable {
    ...
    uint256 private trackingIndex = 0;
    string private trackingURI;
    string private trainingURI = "http://localhost/trackers/";
    ...
    function track(uint numTokens) external payable {
    ...
    function whitelistTrack(bytes32[] calldata proof, uint numTokens) external payable
    {
        ...
        require(MerkleProof.verify(proof, whitelistHash, leaf), "Not on the whitelist");
        require(PRICE_PER_TOKEN * numTokens <= msg.value, "Incorrect Ether Value sent");
    ...
    function sendTracking(uint256 index, string memory uri) external onlyOwner {
        require(index <= MAX_TOTAL_SUPPLY, "Index set too high");
        require(index >= trackingIndex, "New index can not be lower");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if(tokenId < trackingIndex)
            return super.tokenURI(tokenId);
        
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");

        return string(abi.encodePacked(trainingURI, tokenId.toString()));
    }
