...
contract FunFren is ERC721AQueryable, Ownable, ReentrancyGuard {

  using Strings for uint256;

  bytes32 public merkleRootWL;
  mapping(address => uint256) public whitelistClaimed;
  mapping(address => bool) public whitelistFreeClaimed;
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable isValidMerkleProof(_merkleProof, merkleRootWL) {
    ...
    if(totalSupply() + _mintAmount > noFreeNft){
      require(whitelistClaimed[_msgSender()] + _mintAmount <= maxMintAmountPerTxWL, "max NFT per address exceeded");
      require(msg.value >= cost * _mintAmount, "Insufficient funds!");
      whitelistClaimed[_msgSender()] += _mintAmount;
    }else{
      require(_mintAmount <= maxMintAmountPerTxWLFree, "max free NFT per address exceeded");
      require(!whitelistFreeClaimed[_msgSender()], "free Whitelist claimed");
      whitelistFreeClaimed[_msgSender()] = true;
    }
