...
contract BoneheadsLimited is Diamond {
	using ERC165Storage for ERC165Storage.Layout;
...
contract BoneheadsLimitedInitializer is IERC1155Internal {
	function init(
		...
		// Init Opensea Proxy
		OpenSeaProxyStorage.layout().opensea721Proxy = opensea721Proxy;
		OpenSeaProxyStorage.layout().opensea1155Proxy = opensea1155Proxy;
	...
	function _mint(uint256 count) internal {
		unchecked {
			for (uint256 i = 1; i < count + 1; i++) {
				...
				ERC1155NSBaseStorage.layout().balances[i][msg.sender] = 1;
