...
contract Room is ReentrancyGuard {
  ...
  function withdraw () public {
    ...
    if (msg.sender == winner) {
      uint256 amount = (info.baseAmount.sub(info.baseAmount.mul(devFee).div(100))).mul(2).mul(info.rate).div(100);
      withdrawn[msg.sender] = true;
      token.transfer(msg.sender, amount);
    } else {
      uint256 amount = (info.baseAmount.sub(info.baseAmount.mul(devFee).div(100))).mul(2).mul(100 - info.rate).div(100);
      withdrawn[msg.sender] = true;
      token.transfer(msg.sender, amount);
    }
  }
