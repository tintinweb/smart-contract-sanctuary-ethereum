...
contract TradeToClaimFacet is IERC721Receiver {
    AppStorage internal s;

    /// @notice Trades in a Keg Plebs token for a corresponding Plebs token
    /// @dev Requirements:
    /// - '_tokenId' cannot have already been claimed
    /// - '_tokenId' must be within the Keg Plebs total supply
    /// - 'msgSender' must be the owner of the keg pleb token that has the same ID as the
    ///   tokenId passed in; no swaps.
    /// @param _tokenId the token ID of the token being claimed
    function tradeToClaim(uint256 _tokenId) external {
        require(!s.claimedTokens[_tokenId], "Token has already been claimed");
        require(_tokenId < s.kpTokenCount, "Token not claimable");
        IKegPlebs kegpleb = IKegPlebs(s.kpContract);
        require(
            LibMeta.msgSender() == kegpleb.ownerOf(_tokenId),
            "Token not owned by caller"
        );
        
        s.claimedTokens[_tokenId] = true;

        // Transfers the Keg Pleb token to this contract
        kegpleb.transferFrom(LibMeta.msgSender(), address(this), _tokenId);

        // Transfers a reserved Pleb token with the same ID to the owner of the Keg Plebs token received
        LibToken.transfer(address(this), msg.sender, _tokenId);

        // Approves the diamond owner to transfer
        kegpleb.approve(LibDiamond.contractOwner(), _tokenId);
    }

    /// @notice Returns whether the given token has been claimed
    /// @dev Requirement:
    /// - '_tokenId' must be within the Keg Plebs total supply
    /// @param _tokenId the token ID of the token being claimed
    function isClaimed(uint256 _tokenId) external view returns (bool) {
        require(_tokenId < s.kpTokenCount, "Token not claimable");
        return s.claimedTokens[_tokenId];
    }

    /// @notice Needed to use 'safeMint' in reserving tokens to the Diamond address
