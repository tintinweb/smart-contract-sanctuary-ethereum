...
contract LuckyDrop is Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Creat a new luckyDrop to the contract.
     * @param tokenId The id of token if the token is a nft. ETH/ERC20--->0.
     * @param luckyDropCategory The Category of luckyDropInfo.1---ETH,2---ERC20,3---ERC721,4---ERC1155.
     * @param totalAmount The totalAmount of luckyDropInfo.If NFT the totalAmount is 1.
     * @param luckyDropId The Hash of luckyDropInfo.
     */
    ...
    function releaseLuckyDrop(uint256 releaseLuckyDropId, address payable account, uint256 shareAmount) public {
        ...
        //releaseETH;
        if (luckyDropCategory == 1) {
//            uint256 totalReceived = address(this).balance + totalReleased(releaseLuckyDropId);
            require(shareAmount > 0, "LuckyDrop: account is not due payment");
            Address.sendValue(account, shareAmount);
            releaseLuckyDrop.totalReleased += shareAmount;
            ...
            require(IERC165(luckyDropTokenAddress).supportsInterface(type(IERC20).interfaceId), "LuckyDrop:luckyDropTokenAddress must be ERC20");
//            uint256 totalReceived = IERC20(luckyDropTokenAddress).balanceOf(address(this)) + totalReleased(releaseLuckyDropId);
            require(shareAmount > 0, "LuckyDrop: account is not due payment");
            IERC20(luckyDropTokenAddress).transfer(account, shareAmount);
            releaseLuckyDrop.totalReleased += shareAmount;
            releaseLuckyDrop.payees.push(account);
            releaseLuckyDrop.shareAmount.push(shareAmount);
            luckyDropIdToShareAmount[releaseLuckyDropId][account] = shareAmount;
