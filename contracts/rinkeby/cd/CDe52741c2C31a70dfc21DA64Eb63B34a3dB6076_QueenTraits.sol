...
contract QueenTraits is BaseContractController, IQueenTraits {
        ...
        returns (RoyalLibrary.sRARITY[] memory raritiesList)
    {
        ...
        uint256 newArrayIdx = 0;
        for (uint256 idx = 0; idx < rarities.length; idx++) {
            if (_onlyWithArt) {
                if (arts[_traitId][rarities[idx].id].length > 0)
                    _availableRarities[newArrayIdx++] = rarities[idx];
            } else _availableRarities[newArrayIdx++] = rarities[idx];
        }
        ...
        returns (RoyalLibrary.sRARITY memory rarity)
    {
        ...
        emit RarityCreated(rarities.length, _rarityName);
    }
        ...
        returns (RoyalLibrary.sRARITY memory rarity)
    {
        ...
        rarities[idx].rarityName = keccak256(
            abi.encodePacked(_rarityNewName)
        ) != keccak256(abi.encodePacked(""))
            ? _rarityNewName
            : rarities[idx].rarityName;
        rarities[idx].percentage = _newPercentage > 0
            ? _newPercentage
            : rarities[idx].percentage;
    ...
    function getTrait(uint256 _id)
        public
        view
        onlyEcosystemOrOwnerOrArtist
        returns (RoyalLibrary.sTRAIT memory trait)
    {
        require(_id > 0, "Id must be lower than 0!");
        ...
        returns (RoyalLibrary.sTRAIT[] memory _traits)
    {
        ...
        for (uint256 idx = 0; idx < traits.length; idx++) {
            ...
            else if (checkTraitEnabled(traits[idx].id)) itens++;
        }
     ...
     *IN
     *OUT
     *traits: quantity of enabled traits
     */
    function getEnabledTraitsIdList()
        external
        view
        onlyEcosystemOrOwnerOrArtist
        returns (uint256[] memory)
    {
        RoyalLibrary.sTRAIT[] memory enabledTraits = getTraits(true);
        uint256[] memory _traitIdList = new uint256[](enabledTraits.length);

        for (uint256 idx = 0; idx < enabledTraits.length; idx++) {
            _traitIdList[idx] = enabledTraits[idx].id;
        }
        return _traitIdList;
    }
     ...
     *traits: all traits written in contract
     */
    function checkTraitEnabled(uint256 _traitId) private view returns (bool) {
        require(_traitId > 0, "Trait id must be greater than zero!");
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    ...
                    Strings.toString(_artIdx),
                    ")! ",
                    "traitId: ",
                    Strings.toString(_traitId),
                    " rarityId: ",
                    Strings.toString(_rarityId),
                    " arts length: ",
                    Strings.toString(arts[_traitId][_rarityId].length)
                )
            )
        );
        ...
        returns (RoyalLibrary.sART memory art)
    {
        ...
        require(!exists, "Art uri already in store");

        //check if art is on removed list
        (bool found, uint256 index) = CheckIfArtInRemoved(
            _traitId,
            _rarityId,
            _artUri
        );
        ...
        returns (bool result)
    {
        ...
        (bool foundRemoved, uint256 removedIdx) = CheckIfArtInRemoved(
            _traitId,
            _rarityId,
            _artUri
        );
        ...
        if (found) {
            for (
                ...
                arts[_traitId][_rarityId][idx] = arts[_traitId][_rarityId][
                    idx + 1
                ];
            }

            //delete last index
            delete arts[_traitId][_rarityId][
                arts[_traitId][_rarityId].length - 1
            ];
            arts[_traitId][_rarityId].pop();
        }
        return true;
