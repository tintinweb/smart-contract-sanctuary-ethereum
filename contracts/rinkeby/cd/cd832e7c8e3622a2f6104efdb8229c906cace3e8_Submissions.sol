...
contract Submissions is ERC1155Base,Ownable {
    ...
    event Approved(string uri,bytes32 uriHash);

    modifier onlyReviewer {
        require(msg.sender == review,"Not revisors");
        _;
    }
    ...
    function approve(string calldata uri) external onlyReviewer {
        bytes32 uriHash = keccak256(abi.encodePacked(uri));
        require(uriHashUsed[uriHash],"Uri was not submited");
        approved[uriHash] = true;
        emit Approved(uri,uriHash);
    }
    function mint(Fee[] calldata fees,uint256 supply, string calldata uri) external onlyOwner {
        bytes32 uriHash = keccak256(abi.encodePacked(uri));
        require(uriHashUsed[uriHash],"Uri was not submited");
        require(approved[uriHash],"Uri was not approved");
        _totalSupply = _totalSupply.add(1);
        _mint(submissionAuthor[uriHash],_totalSupply, fees, supply, uri);
    }
