    ...
    ReentrancyGuard
{
    ...
    uint256 totalRemainingMintAmount = 0;
    // provenance
    ...
    {
        require(
            saleConfig.saleMintedAmount +
                saleConfig.auctionMintedAmount +
                quantity <=
    ...
    uint256 public constant AUCTION_START_STARS_PRICE = 74862 ether;
    uint256 public constant AUCTION_END_STARS_PRICE = 14972400000000 gwei; // 14,972.4 eth
    uint256 public constant AUCTION_DROP_PER_STEP_STARS = 14972400000000 gwei; // 14,972.4 eth
    ...
    function rebate() external auctionAndSalesEnded {
        ...
        for (uint256 i = 0; i < sales[msg.sender].length; i++) {
            ...
            if (sales[msg.sender][i].isStars) {
                ...
                // differenceValEth = differenceInEth * ethUSDPrice
                // rebateAmount = (differenceValEth * rateBIP / starsUSDPrice)/ 10000
    ...
    function mintRemaining() external callerIsUser auctionAndSalesEnded {
        ...
        if (totalRemainingMintAmount == 0) {
            totalRemainingMintAmount = totalAmount - totalSupply();
        }

        // get base amount to mint per valid user
        if (tier1UsersNotMinted + tier1UsersMinted > totalRemainingMintAmount) {
            quantity = 0;
        } else if (
            tier1UsersNotMinted + tier1UsersMinted == totalRemainingMintAmount
        ) {
            quantity = 1;
        } else {
            quantity =
                totalRemainingMintAmount /
                (tier1UsersNotMinted + tier1UsersMinted);
        }
        ...
        if (
            ...
            totalRemainingMintAmount % (tier1UsersNotMinted + tier1UsersMinted)
        ) {
            quantity++;
        }
