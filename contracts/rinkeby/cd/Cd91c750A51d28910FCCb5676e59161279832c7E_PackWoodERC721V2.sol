...
interface IPackWoodERC721V1 {
   ...
   function createChild() external returns(uint256);
...
abstract contract PackWoodERC721StorageV2{
    ...
    IERC721Upgradeable public MonsterParent;
    uint256 processPrice;

    event createChild(uint256 _monsterTokenId, uint256 _packwoodTokenId, uint256 childTokenId);
...
import "./IPackWoodERC721V1.sol";
import "./PackWoodERC721StorageV2.sol";
...
contract PackWoodERC721V2 is ERC721URIStorageUpgradeable, OwnableUpgradeable, PackWoodERC721StorageV2{
    using StringsUpgradeable for uint256;
    

    
    function uriConcate(string memory before_, uint256 _token_id, string memory after_) private pure returns (string memory){
        string memory token_uri = string( abi.encodePacked(before_, _token_id.toString(), after_));
        return token_uri;
    }

    function updateMonsterAddress(address _address) external onlyOwner returns(bool){
        MonsterParent = IERC721Upgradeable(_address);
        return true;
    }


    function updateParentAddress(address _parentAddress) external onlyOwner returns(bool){
    ...
    function process(uint256 _monsterTokenId, uint256 _packwoodERC1155TokenId) external returns(uint256){
        require(MonsterParent.ownerOf(_monsterTokenId) == msg.sender, "PackwoodERC721: You are not owner of token");
