    ...
    ReentrancyGuard 
{
    ...
    ) ERC721(tokenName, tokenSymbol) EIP712Whitelisting() ReentrancyGuard() {
        totalSupply = totalSupply_;
        customBaseURI = customBaseURI_;
        paused = paused_;
        setWhitelistSigningAddress(signingKey);
        phase = Phase.Private;
    ...
    */
    function checkWhitelist(bytes calldata signature) 
        public 
        view 
        requiresWhitelist(signature) returns (bool) 
    {
    ...
    constructor() {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("NieuxWhitelist")),
                keccak256(bytes("1")),
                getChainId(),
                address(this)
            )
