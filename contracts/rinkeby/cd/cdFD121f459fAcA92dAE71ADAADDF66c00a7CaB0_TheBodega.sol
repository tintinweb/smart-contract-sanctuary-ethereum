...
contract TheBodega is ERC721A16, Pausable, ReentrancyGuard {
	...
	constructor(string memory secret) ERC721A16("The Bodega", "") {
		_secret = secret;
    	...
    	returns (string memory) 
    {
        ...
        return string(abi.encodePacked(_baseURI(), _tokenHash));
    }
    ...
    {
    	_mintInternal(to, qty);
    }

    //@dev Allows public addresses (non-owners) to purchase
    function purchase(address payable to, uint16 qty, string calldata secret) 
    	external payable enoughSupply(qty) saleActive
    {
    	...
    	_mintInternal(to, qty);
    	_refundIfOver(to, qty, msg.value);//should never have to send change
    }
	...
	function setBaseTokenURI(string calldata newBaseURI) external isSquad
	{
		require(
			!_stringsEqual(_baseTokenURI, newBaseURI),
			"TheBodega: new uri must be different"
		);
	...
	function setContractURI(string calldata newContractURI) external isSquad
	{
		require(
			!_stringsEqual(_contractURI, newContractURI), 
			"TheBodega: newContractURI must be different"
		);
    ...
    function setTokenHash(string calldata hash) external isSquad
    {
    	require(
    		!_stringsEqual(_tokenHash, hash),
    		"TheBodega: already the asset hash"
    	);
	...
	{
		require(
			!_stringsEqual(_secret, newSecret), 
			"TheBodega: secret must be different"
		);
	...
	//@dev Mints `qty` tokens to `to`
	function _mintInternal(address to, uint16 qty) private
	{
		_safeMint(to, uint256(qty));
	}

	//@dev Send change back if necessary
	function _refundIfOver(address payable to, uint16 qty, uint256 amount) private
	{
		if (amount > 0) {
    		uint256 diff = amount.sub(weiPrice*uint256(qty));
    		if (diff > 0) {
    	    	to.transfer(diff);
    		}
    	}
 ...
 *
 * Originally ERC721A (by Azuki) optimized slightly further for smaller supply.
 * -> Optimizations assume a supply lower than 2**16 - 1 (65,535)
 *
...
contract ERC721A16 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    constructor(string memory name_, string memory symbol_) {
        _name = name_;
        _symbol = symbol_;
        currentIndex = 1;//token id's start at 1
    }
    ...
    function tokenByIndex(uint256 index) public view override returns (uint256) {
        require(index < totalSupply(), 'ERC721A16: global index out of bounds');
        return index;
    }
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view override returns (uint256) {
        require(index < balanceOf(owner), 'ERC721A16: owner index out of bounds');
        uint16 numMintedSoFar = uint16(totalSupply());
        uint16 tokenIdsIdx;
        ...
        revert('ERC721A16: unable to get token of owner by index');
    }
    ...
    function balanceOf(address owner) public view override returns (uint256) {
        require(owner != address(0), 'ERC721A16: balance query for the zero address');
        return uint256(_addressData[owner].balance);
    }

    function _numberMinted(address owner) internal view returns (uint16) {
        require(owner != address(0), 'ERC721A16: number minted query for the zero address');
    ...
    function ownershipOf(uint256 tokenId) internal view returns (TokenOwnership memory) {
        require(_exists(tokenId), 'ERC721A16: owner query for nonexistent token');

        unchecked {
            uint16 curr;
            for (curr = uint16(tokenId); curr >= 0; curr--) {
        ...
        revert('ERC721A16: unable to determine the owner of token');
    }
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner = ERC721A16.ownerOf(tokenId);
        require(to != owner, 'ERC721A16: approval to current owner');

        require(
            _msgSender() == owner || isApprovedForAll(owner, _msgSender()),
            'ERC721A16: approve caller is not owner nor approved for all'
        );
    ...
    function getApproved(uint256 tokenId) public view override returns (address) {
        require(_exists(tokenId), 'ERC721A16: approved query for nonexistent token');
    ...
    function setApprovalForAll(address operator, bool approved) public override {
        require(operator != _msgSender(), 'ERC721A16: approve to caller');
    ...
    function safeTransferFrom(
        ...
        require(
            ...
            'ERC721A16: transfer to non ERC721Receiver implementer'
        );
    }
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        return tokenId < uint256(currentIndex);
    }
    ...
    function _mint(
        ...
        require(to != address(0), 'ERC721A16: mint to the zero address');
        require(quantity != 0, 'ERC721A16: quantity must be greater than 0');

        _beforeTokenTransfers(address(0), to, uint256(startTokenId), uint256(quantity));

        // Overflows are incredibly unrealistic.
        unchecked {
            _addressData[to].balance += uint16(quantity);
            _addressData[to].numberMinted += uint16(quantity);
            ...
            uint16 i;
            for (i = 0; i < uint16(quantity); i++) {
                emit Transfer(address(0), to, uint256(updatedIndex));
                if (safe) {
                    require(
                        _checkOnERC721Received(address(0), to, uint256(updatedIndex), _data),
                        'ERC721A16: transfer to non ERC721Receiver implementer'
                    );
                }
        ...
        _afterTokenTransfers(address(0), to, uint256(startTokenId), uint256(quantity));
    }
    ...
    function _transfer(
        ...
        require(isApprovedOrOwner, 'ERC721A16: transfer caller is not owner nor approved');

        require(prevOwnership.addr == from, 'ERC721AO: transfer from incorrect owner');
        require(to != address(0), 'ERC721A16: transfer to the zero address');
        ...
        unchecked {
            ...
            _ownerships[uint16(tokenId)].addr = to;
            _ownerships[uint16(tokenId)].startTimestamp = uint64(block.timestamp);

            // If the ownership slot of tokenId+1 is not explicitly set, that means the transfer initiator owns it.
            // Set the slot of tokenId+1 explicitly in storage to maintain correctness for ownerOf(tokenId+1) calls.
            uint16 nextTokenId = uint16(tokenId) + 1;
            if (_ownerships[nextTokenId].addr == address(0)) {
                if (_exists(uint256(nextTokenId))) {
    ...
    function _checkOnERC721Received(
        ...
        if (to.isContract()) {
            try IERC721Receiver(to).onERC721Received(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert('ERC721A16: transfer to non ERC721Receiver implementer');
                } else {
                    assembly {
