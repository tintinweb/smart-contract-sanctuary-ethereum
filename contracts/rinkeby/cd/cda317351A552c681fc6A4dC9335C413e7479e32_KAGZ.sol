...
contract KAGZ is ERC721Enumerable, Ownable {
    using Strings for uint256;
    string private baseURI; // 
    string private mythicBaseURI; //
    string public baseExtension = ".json"; //
    string public mythicBaseExtension; //
    string public notRevealedUri; //
    string public notRevealedMythicUri; //
    string public ogProvenanceHash; 
    string public mythicProvenanceHash; 
    uint256 public preSaleCost = 0.002 ether; //****//
    uint256 public cost = 0.003 ether; //****//  
    uint256 public maxSupply = 10000; //****/ Maximum supply of OGs
    ...
    uint256 public preSaleDate = 1644373152; //****// Feb8 9:50PM
    uint256 public publicSaleDate = 1644492538; //****// Feb 10
    uint256 public freeMintReserve = 100; //****//
    uint256 public mythicCount = 0;
    uint256 public maxMythicSupply = 5000; //****//
    uint256 public energizeRequirement = 10 weeks; //****//
    uint256 public mythicRevealGap = 10 days; //****// Time after publicSaleDate + energizeRequirement during which Myhtics will be unrevealed
    uint256 private revealDate = 1639971120; //****// OG Reveal date
    ...
    mapping(uint256 => uint256) public timeOfAcquisitionOrLastBreeding; //reset for OG on mint, transfer, and breeding

    constructor(string memory _name, string memory _symbol, string memory _initNotRevealedUri) ERC721(_name, _symbol) {
        setNotRevealedURI(_initNotRevealedUri);
    }

    bytes32 public merkleRoot = 0xdde0b8ebc4f412615567e07c27bc6dcb070f5ea22a7c19b9abfa738f627be8e4;//** */
    ...
    event MythicMinted(uint256 _mythicId, uint256 _kongenzaOne, uint256 _kongenzaTwo, address indexed _breederAddress); 

    /* 
        * PROVENANCE SETTER ONLY OWNER
        
        * @dev
        * The folder containing the metadata for each OG Kongenza has a first IPFS hash.
        * The folder containing the metadata for each Mythic Kongenza has a second IPFS hash.
        * And the metadata for each Kongenza contains the IPFS hash for its GIF.
        * Each of the first IPFS hash and the second IPFS hash are hashed with SHA-256.
        * Each SHA-256 hash is set here prior the commencment of the presale.
        * Each SHA-256 hash is immutable upon the commencement of the presale, thereby serving as proof 
        * of no tampering or changes to the collection after sale. 
    */
    function setProvenanceHashes(string memory _ogHash, string memory _mythicHash) public onlyOwner {
        require(block.timestamp < preSaleDate, "presale has commenced");
        ogProvenanceHash = _ogHash;
        mythicProvenanceHash = _mythicHash;
    }


    //BREEDING MODIFIERS
    modifier notBegun {
        require(mythicCount == 0, "Fusion has begun");
        _;
    }

    //ONLY OWNER BREEDING SETTERS    
    function setBreedingPaused(bool _pauseBreeding) public onlyOwner {
        breedingPaused = _pauseBreeding;
    }

    function setMaxMythicSupply(uint256 _newMaxMythicSupply) public notBegun onlyOwner {
        maxMythicSupply = _newMaxMythicSupply;
    }

    function setEnergizeRequirement (uint256 _newEnergizeRequirement) public notBegun onlyOwner {
        energizeRequirement = _newEnergizeRequirement;
    }

    function setMythicRevealGap (uint256 _newMythicRevealGapInSeconds) public notBegun onlyOwner {
        mythicRevealGap = _newMythicRevealGapInSeconds;
    }    

    //BREEDING FUNCTION
    function breed(uint256 _kongenzaOne, uint256 _kongenzaTwo) public allowedSender {
        require(!breedingPaused, "Breeding is paused");
        ...
        require(ownerOfAllIds(_kongenzaOne, _kongenzaTwo), "You must select Kongenzaz that you own");
        require(noMythicsInIds(_kongenzaOne, _kongenzaTwo), "Cannot fuse any Mythics");
        require(allIdsUnique(_kongenzaOne, _kongenzaTwo), "You must enter unique Kongenzaz");
        require(allEnergized(_kongenzaOne, _kongenzaTwo), "Every entered Kongenza must be energized"); 
        resetTimeOfAcquisitionOrLastBreeding(_kongenzaOne, _kongenzaTwo);
        mythicCount++;
        uint256 _mythicId = getRandomIdForMythic(_kongenzaOne);
        _mint(msg.sender, _mythicId);
        emit MythicMinted(_mythicId, _kongenzaOne, _kongenzaTwo, msg.sender);
    }
    

    //INTERNAL VIEW BREEDING FUNCTIONS
    function getRandomIdForMythic(uint256 _kongenzaOne) internal view returns (uint256) { 
        uint256 _randomIdForMythic = (uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, mythicCount, msg.sender, _kongenzaOne))) % maxMythicSupply) + maxSupply + 1;
    ...
    function sufficientMythicSupply() internal view returns (bool) {
        return mythicCount < maxMythicSupply;
    }
    ...
    function noMythicsInIds(uint256 _kongenzaOne, uint256 _kongenzaTwo) internal view returns (bool) {
    ...
    function allEnergized(uint256 _kongenzaOne, uint256 _kongenzaTwo) internal view returns (bool) {
        if (isEnergized(_kongenzaOne) && isEnergized(_kongenzaTwo)) {
            return true;
        }
        else {
            return false;
    ...
    function isEnergized(uint256 _id) internal view returns (bool) { 
        require(_exists(_id), "ID does not exist"); 
        require((_id <= maxSupply), "Mythics do not energize");
        if ((block.timestamp - timeOfAcquisitionOrLastBreeding[_id]) >= energizeRequirement) {
            return true;
        }
        else {
            return false;
    ...
    function _mythicBaseURI() internal view returns (string memory) {
        return mythicBaseURI;
    } 

    //INTERNAL STATE BREEDING FUNCTIONS
    function resetTimeOfAcquisitionOrLastBreeding(uint256 _kongenzaOne, uint256 _kongenzaTwo) internal {
        timeOfAcquisitionOrLastBreeding[_kongenzaOne] = block.timestamp;
        timeOfAcquisitionOrLastBreeding[_kongenzaTwo] = block.timestamp;
    }


    //PUBLIC VIEW BREEDING FUNCTIONS
    function timeUntilEnergized(uint256 _id) public view returns (uint256) { 
        if (isEnergized(_id)) {
            return 0;
        }
        else {
            return timeOfAcquisitionOrLastBreeding[_id] + energizeRequirement - block.timestamp;
        }
    }

    function transferFrom(address from, address to, uint256 tokenId) public override {
        timeOfAcquisitionOrLastBreeding[tokenId] = block.timestamp;
    ...
    function isMythicsRevealed() public view returns (bool) {
        if (block.timestamp > publicSaleDate + energizeRequirement + mythicRevealGap) {
            return true;
        }
        return false;
    }
    ...
    function mint(uint256 _mintAmount, bytes32[] memory _proof) public payable notPaused saleStarted allowedSender minimumMintAmount(_mintAmount) {
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            timeOfAcquisitionOrLastBreeding[supply + i] = block.timestamp;    
    ...
    //Such as for those who earned giveaways prior to sale
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (!isRevealed()) {
            ...
            if (!isMythicsRevealed()) {
                return notRevealedMythicUri;
            }
            else {
                string memory currentMythicBaseURI = _mythicBaseURI();
                return bytes(currentMythicBaseURI).length > 0 ? string(abi.encodePacked(currentMythicBaseURI,tokenId.toString(), mythicBaseExtension)) : "";
            }
        }
    }
    ...
    function getMythicBaseURI() public view onlyOwner returns (string memory) {
        return mythicBaseURI;
    }

    function getRevealDate() public view onlyOwner returns (uint256) {
        return revealDate;
    }

    function timeUntilReveal() public view onlyOwner returns (uint256) {
        if (revealDate > block.timestamp) {
            return revealDate - block.timestamp;
        }
        else {
            return 0;
        }
    ...
    function setRevealDate(uint256 _newRevealDate) public onlyOwner {
    ...
    function setMythicBaseURI(string memory _newMythicBaseURI) public onlyOwner {
        mythicBaseURI = _newMythicBaseURI;
    }
    ...
    function setMythicBaseExtension(string memory _newMythicBaseExtension) public onlyOwner {
        mythicBaseExtension = _newMythicBaseExtension;
    }
    ...
    function setNotRevealedMythicURI(string memory _notRevealedMythicURI) public onlyOwner {
        notRevealedMythicUri = _notRevealedMythicURI;
    }
    ...
    function setMaxSupply(uint256 _newMaxSupply) public onlyOwner {
        require(block.timestamp < publicSaleDate, "public sale has already begun");
        maxSupply = _newMaxSupply;
    }

    function setPreSaleDate(uint256 _preSaleDate) public onlyOwner {
        require(block.timestamp < preSaleDate, "presale has already begun");
