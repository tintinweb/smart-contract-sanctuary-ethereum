...
contract PriceFeed is PriceAwareOwnable, AccessControl, IPriceFeed {
  ...
  function setPrice(bytes32 _symbol) external onlyRole(SIGNER_ROLE) {
    uint256 previous = _prices[_symbol];
    (uint256 current, uint256 timestamp) = getPriceFromMsg(_symbol);
    if(lastPriceUpdate[_symbol]<timestamp){
      _prices[_symbol] = current;
      lastPriceUpdate[_symbol] = timestamp;
      emit PriceUpdated(_symbol, previous, current, timestamp);
      return;
    }
    revert OldPriceData(_symbol);
  }

  /// @notice Get prices form oracle and update them on feed
  /// @param _symbols Hash-symbols of authorized assets
  function setPriceExt(bytes32[] memory _symbols)
    external
    onlyRole(SIGNER_ROLE)
  {
    for (uint256 i; i < _symbols.length; i++) {
      uint256 previous = _prices[_symbols[i]];
      (uint256 current, uint256 timestamp) = getPriceFromMsg(_symbols[i]);
      _prices[_symbols[i]] = current;
      lastPriceUpdate[_symbols[i]] = timestamp;
      emit PriceUpdated(_symbols[i], previous, current, timestamp);
    }
  }
