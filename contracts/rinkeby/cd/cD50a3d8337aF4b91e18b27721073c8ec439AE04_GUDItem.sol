...
contract GUDItem is AccessControl, ERC1155Supply, IGUDItem {
  ...
  function grantRole(bytes32 role, address account) public virtual override(IAccessControl, AccessControl) onlyRole(getRoleAdmin(role)) {
    if (role == MINTER && !hasRole(role, account)) {
      require(Address.isContract(account), "Only shop contract addresses can be authorized.");
    }
  ...
  function mintBatch(
      address to,
      uint[] memory ids,
      uint[] memory amounts) external override {
    _checkRole(MINTER, _msgSender());
