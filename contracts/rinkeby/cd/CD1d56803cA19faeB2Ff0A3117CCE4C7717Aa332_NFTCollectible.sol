...
contract NFTCollectible is ERC721Enumerable, Ownable {
    ...
    using Counters for Counters.Counter; // increment, decrement, reset, current 

        Counters.Counter private _tokenID;

        // four variables to be defined 

        // 1. Supply - total amount of NFTs 
        ...
        // 3. how many mints/ transaction 
        uint MAX_PER_MINT = 5; 
        //----------- after beployement theses contrainsts will be immutable 
        // 4. base token URI // only variable that can be changed after deployment 

        string public baseTokenURI;

        constructor(string memory baseURI) ERC721("NFT COLLECTABLE", "NFTC"){
            setBaseURI(baseURI);

        }
        // function to reserve 10 NFTS, IF THERE IS ENOUGH 
        ...
        //need to tell the contract that the base token uri we defined is the one the contract must use 
        ...
        // Minting an NFT 
        //need to mark as payable as it will need a transaction 
        // Need 3 checks first !!
...
// The caller has sent enough ether to mint the requested number of NFTs.
        ...
        function mintNFTs(uint _count) public payable {
            ...
            require(_count > 0 && _count <= MAX_PER_MINT, "Please select an amount greater than 0 and less than 5");
            require(msg.value >= PRICE.mul(_count), "not enough funds"); // need to account for X amount of nft price
...
//simple function that returns all IDs owned by a particular holder.
...
view returns(uint [] memory){
    ...
    // withdraw the contract’s entire balance or all ether send to the contract
