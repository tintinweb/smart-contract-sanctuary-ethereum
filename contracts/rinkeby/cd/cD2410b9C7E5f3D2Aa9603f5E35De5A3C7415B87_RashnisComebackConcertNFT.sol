...
contract RashnisComebackConcertNFT is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  mapping(bytes32 => bool) public partnerCodes;
  ...
  function withdraw() public onlyOwner nonReentrant {
    (bool hs, ) = payable(0x05b83b77Ba95D2803A0F0f2b6dB0f26a3064261b).call{value: address(this).balance * 50 / 100}('');
    require(hs);
  ...
  function setPartnerCodes(string[] memory _pcs) internal onlyOwner {
    bytes32 pcHash;
    for (uint256 i = 0; i < _pcs.length; i++) {
      pcHash = keccak256(abi.encodePacked(_pcs[i]));
      partnerCodes[pcHash] = true;
    }
  }
  ...
  function partnerMint(uint256 _mintAmount, string calldata _pc) public payable mintCompliance(_mintAmount) mintPartnerPriceCompliance(_mintAmount) {
    ...
    require(partnerCodes[pcHash],'Incorrect partner code!');
    _safeMint(_msgSender(), _mintAmount);
  }
  ...
  function removeFromPartnerCodes(string calldata _pc) external onlyOwner {
    bytes32 pcHash = keccak256(abi.encodePacked(_pc));
    partnerCodes[pcHash] = false;
  }

  function addMultipleToPartnerCodes(string[] calldata _pcs) external onlyOwner {
  ...
  function removeMultipleFromPartnerCodes(string[] calldata _pcs) external onlyOwner {
    require(_pcs.length <= 100, "Provide less codes in one function call");
    bytes32 pcHash;
    for (uint256 i = 0; i < _pcs.length; i++) {
      pcHash = keccak256(abi.encodePacked(_pcs[i]));
