...
contract MasterChefV3 is ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier poolNotPause(uint256 _id) {
        require(_pools[_id].enable, "MasterChef: pool was disabled");
        _;
    }
        ...
        returns(address, address[] memory, uint256[] memory, uint256[] memory, uint256[] memory, uint128, uint128, uint128, uint128, uint256, bool)
    {
        ...
        return (pool.stakingToken, pool.rewardTokens, pool.rewardPerBlocks, pool.rewardPerShares, pool.totalRewards, pool.lastRewardBlock, pool.depositFeePercent, pool.startBlock, pool.endBlock, pool.totalStaked, pool.enable);
    }
    ...
    function pendingReward(uint256 _pid, address _account) public view returns (uint256[2] memory pendings) {
        ...
        bool flg = block.number > pool.lastRewardBlock && pool.totalStaked > 0;

        for (uint256 i = 0; i < 2; i++) {
            uint256 rewardPerShare = pool.rewardPerShares[i];

            if (flg) {
                uint256 reward = (block.number - pool.lastRewardBlock) * pool.rewardPerBlocks[i];

                if (reward > pool.totalRewards[i]) {
                    reward = pool.totalRewards[i];
                }
    ...
    function updatePool(uint256 _id)
        public
        poolNotPause(_id)
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < 2; i++) {
            ...
            uint256 remaining = pool.totalRewards[i];
            ...
            if (reward > 0) {
                ...
                pool.totalRewards[i] = remaining - reward;
