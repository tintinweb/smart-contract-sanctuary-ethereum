...
contract RandomNasAcademySVG is Ownable {
    struct Layer {
        string name;
        string base64Svg;
    }
    ...
    function setLayers(LayerInput[] calldata toSet) external onlyOwner {
        for (uint16 i = 0; i < toSet.length; i++) {
            layers[toSet[i].layerIndex][toSet[i].itemIndex] = Layer(toSet[i].name, toSet[i].base64Svg);
        }
    }
    ...
    function splitNumber(uint256 _number) internal pure returns (uint16[NUM_LAYERS] memory numbers) {
        for (uint256 i = 0; i < numbers.length; i++) {
            ...
            // _number = uint256(keccak256(abi.encodePacked(_number % 10000))) or something like this, depending on how random hashes are
            _number >>= 14;
        }
        return numbers;
    }
