...
contract StakingPool is OwnableUpgradeable, ReentrancyGuard 
{
   ...
   // address private beneficiary;
    ...
    function claimAmount(uint256 poolId) external {
        ...
        // IERC20(ideaToken).approve(address(this), amount);

        // IERC20(ideaToken).allowance(address(this), address(this));

        IERC20(ideaToken).safeTransferFrom(address(this),msg.sender, amount);
    }
    ...
    function applyForStaking(uint256 poolId ) external {
        ...
        IERC20(ideaToken).approve(address(this), poolInfo.weiAmount);

        IERC20(ideaToken).allowance(address(this), address(this));

     //   IERC20(ideaToken).allowance(msg.sender, address(this));
    ...
    function stakingClaim(uint256 poolId ) public {
      ...
      //  }
       ...
       // IERC20(ideaToken).allowance(address(this), msg.sender);

        IERC20(ideaToken).approve(address(this), stakingAmount);

        IERC20(ideaToken).allowance(address(this), address(this));

        IERC20(ideaToken).safeTransferFrom(address(this), msg.sender, stakingAmount);
    ...
    function withdrawAllToken(address to) external onlyOwner{
        ...
        IERC20(ideaToken).approve(address(this), totalamount);

        IERC20(ideaToken).safeTransferFrom(address(this),to,totalamount);
