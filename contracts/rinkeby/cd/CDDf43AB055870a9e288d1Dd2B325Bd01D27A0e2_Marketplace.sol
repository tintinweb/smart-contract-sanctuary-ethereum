...
pragma solidity ^0.8.0; //Safemath [math operations without overflow] integrated since solidity 0.8.0 https://soliditydeveloper.com/solidity-0.8
    ...
    ERC1820Implementer
{
    ...
    function buyItem(uint256 _index) public {
        ...
        require(!i.sold, "item sold already");
        require(
            i.price <= IERC777(token).balanceOf(msg.sender),
            "not enough tokens"
        );
        i.buyer = msg.sender;
        i.sold = true;
        IERC777(token).send(i.seller, i.price, "");
    }

    function myBalance() public returns (uint256) {
        uint256 balance = IERC777(token).balanceOf(msg.sender);
        return balance;
    }
}
