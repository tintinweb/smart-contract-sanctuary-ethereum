...
contract HyewonNft is IHyewonNft, ERC721A, Ownable, ReentrancyGuard {
    mapping(uint16 => Round) rounds; // minting rounds
    mapping(uint16 => mapping(address => bool)) whitelists;

    uint16 private _currentRound;
    uint256 private _maxRevealedId;
    ...
    modifier onlyOwnerOrAdmin(uint16 roundNumber) {
        ...
        // TODO: check the case that r.admin == address(0)
        if (msg.sender != owner() && msg.sender != rounds[roundNumber].admin) {
            revert NotOwnerNorAdmin(msg.sender);
        }
        _;
    }

    constructor() ERC721A("HyewonNft", "Hyewon") Ownable() {}

    function mint(uint256 quantity) external payable override nonReentrant {
        Round memory currentRound = getCurrentRound();
    ...
    function whitelistMint(
        ...
        _updateWhitelistUsed(_currentRound, v, r, s, currentRound.admin);
        ...
        private
    {
        ...
        // check if not exceeding the maxAllowedMintingCount
        if (
    ...
    function adminMint(uint16 roundNumber, uint256 quantity)
        external
        payable
        override
        onlyOwnerOrAdmin(roundNumber)
    {
        Round memory r = rounds[roundNumber];
        // check if minting does not exceed the maximum tokens for the round
        if (!_isTokenAvailable(quantity, r.maxMintingId)) {
            revert ExceedMaximumForTheRound(r.maxMintingId - _nextTokenId());
        }

        // check if proper fee is received
        uint256 neededFee = r.mintingFee * quantity;
        if (neededFee != msg.value) revert NoMatchingFee();

        _safeMint(msg.sender, quantity);
    }

    // return tokenURI (before reveal, return the before-revealed-uri)
    ...
    {
        Round memory roundForToken = _getRoundByTokenId(tokenId);

        if (roundForToken.revealed == false) {
            if (
                keccak256(abi.encodePacked(roundForToken.unrevealedURI)) !=
                keccak256(abi.encodePacked(""))
            ) {
                return roundForToken.unrevealedURI;
            } else {
                return defaultUnrevealedURI;
            }
        }
    ...
    function newRound(
        uint256 maxMintingId,
        string memory tokenURIPrefix,
        uint256 mintingFee,
        uint16 maxAllowedMintingQuantity,
        bool whitelisted,
        bool revealed,
        ...
        if (_currentRound < 0) {
            rounds[_currentRound].lastMintedId = _nextTokenId() - 1;
        }
        ...
        uint256 random = uint256(blockhash(block.number + 256));

        rounds[++_currentRound] = Round({
            maxMintingId: maxMintingId,
            lastMintedId: maxMintingId,
    ...
    function setCurrentRound(uint16 currentRound) external onlyOwner {
    ...
    function setMaxMintingId(uint256 maxId) external {
        setMaxMintingId(_currentRound, maxId);
    }
    ...
    function setTokenURIPrefix(string memory prefix) external {
        setTokenURIPrefix(_currentRound, prefix);
    }
    ...
    function setMintingFee(uint256 fee) external {
        setMintingFee(_currentRound, fee);
    }
    ...
    function setMaxAllowedMintingQuantity(uint16 quantity) external {
        setMaxAllowedMintingQuantity(_currentRound, quantity);
    }
    ...
    function setWhitelisted(bool whitelisted) external {
        setWhitelisted(_currentRound, whitelisted);
    }
    ...
    function setRevealed() external {
        setRevealed(_currentRound);
    }

    // set reveal for the specified round
    ...
    function setUnrevealedURI(string memory uri) external {
        setUnrevealedURI(_currentRound, uri);
    }
    ...
    function setStartTime(uint256 time) external {
        setStartTime(_currentRound, time);
    }
    ...
    function setOnlyAdminRound(bool onlyAdmin) external {
        setOnlyAdminRound(_currentRound, onlyAdmin);
    }
    ...
    // withdraw funds in this contract
    ...
    {
        // check if minting does not exceed the maximum tokens for the round
        if (_nextTokenId() + quantity > maxId) {
            return false;
        }
        ...
        returns (Round memory r)
    {
        ...
        while (roundNumber <= _currentRound) {
    ...
    function _updateWhitelistUsed(
        ...
        // check if the address already claimed
        if (whitelists[round][_msgSender()]) {
            revert WhitelistAlreadyClaimed();
        }
        ...
        whitelists[round][_msgSender()] = true;
    }
...
interface IHyewonNft {
    ...
    event BaseExtensionUpdated(string baseExtension);
    ...
    function adminMint(uint16 roundNumber, uint256 quantity) external payable;
