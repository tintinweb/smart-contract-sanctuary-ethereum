...
contract NFTbridge is Ownable, IERC721Receiver {
  ...
  NFTBridgeTokenFactory tokenFactory;
  INFTV02 muon;
  ...
  constructor(address _muon) {
    ...
    //TODO: update
    tokenFactory = new NFTBridgeTokenFactory(address(this));
    muon = INFTV02(_muon);
  }
  ...
  function depositFor(
    ...
    INFTBridgeERC721 token = INFTBridgeERC721(tokens[tokenId]);
    if (mintable[tokenId]) {
  ...
  function claim(
    ...
    INFTV02.SchnorrSign[] calldata _sigs
  ) public {
    ...
    INFTBridgeERC721 token = INFTBridgeERC721(tokens[txParams[2]]);
  ...
  function addMainToken(address _tokenContract) public {
    ...
    INFTBridgeERC721 token = INFTBridgeERC721(_tokenContract);
  ...
  function emergencyWithdrawERC721Tokens(
    ...
    INFTBridgeERC721(_tokenAddr).safeTransferFrom(address(this), _to, _id);
  }
...
import "./NFTBridgeToken.sol";

contract NFTBridgeTokenFactory{
    ...
    function create(string calldata _name, string calldata _symbol) external returns(address created){
        created = address(new NFTBridgeToken(bridge, _name, _symbol));
...
contract NFTBridgeToken is ERC721Enumerable{
