...
contract Renkon is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor(bytes32 verifier, uint256 presaleStar, uint256 auctionStart, uint256 publicSaleStart, address treasury, string memory baseURI) ERC721A("test", "test1") {
    _verifier = verifier;
    _presaleStart = presaleStar;
    _auctionStart = auctionStart;
    _publicSaleStart = publicSaleStart;
    _treasury = treasury;
  ...
  function setSaleDate(uint256 presaleStar, uint256 auctionStart, uint256 publicSaleStart) external onlyOwner {
    _presaleStart = presaleStar;
    _auctionStart = auctionStart;
    _publicSaleStart = publicSaleStart;
  }
  ...
  function isAuctionActive() public view returns (bool) {
    return block.timestamp > _presaleStart && block.timestamp > _auctionStart && block.timestamp < _publicSaleStart;
  }
  ...
  function presaleMint(bytes32[] calldata proof) external payable nonReentrant onlyEOA {
  ...
  function withdraw() external payable onlyOwner {
    (bool sent, bytes memory data) = payable(_treasury).call{ value: msg.value }("");
