...
//    }
    ...
    function mint(uint256 [] memory tokenIds, uint8 style, uint8 cIndex, KIND kind, bytes memory signature) external payable returns (bool){
        // params check
        checkMintConfig(configs[cIndex], tokenIds.length) ;
        configs[cIndex].current += tokenIds.length;
        // check signers
    ...
    function mintSuit(uint256 [] memory sIds, uint256 [] memory dIds, uint256 [] memory hIds, uint8 style, uint8 cIndex, bytes memory signature) external payable returns (bool){
        // params check
        checkMintConfig(configs[cIndex], sIds.length) ;
        configs[cIndex].current += sIds.length ;
    ...
    // check mint config
    function checkMintConfig (Config memory config, uint256 mintCount) private {
    ...
    private view returns (uint256) {
        ...
        uint16 qualityRandom = uint16(uint256(keccak256(abi.encodePacked("quality", "_", Strings.toString(tokenId), "_", addressToString(owner), "_", Strings.toHexString(blockHash)))) % 10000) ;
...
contract BaseManager is AccessControlEnumerable, Pausable, EIP712 {
