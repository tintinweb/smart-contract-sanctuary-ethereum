...
contract UnicornClub is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint public reservedForTeam = 150;
    ...
    SaleStatus public saleStatus = SaleStatus.OG;
...

    {
        ...
        //////////////////////////////////////
       /* 
        todo: Whitelist will be able to mint 10 total
        todo: Purchase: 9
        todo: Claim 1
todo: 
todo: 
        todo: OG Will be able to mint 11 total
        todo: Purchase 9
        todo: Claim 2

        Todo: Subtract one from everything
        todo: modify signatures
        */
    ...
    function teamMint(address to ,uint256 amount) public onlyOwner  {
        if(amount + totalSupply() > maxSupply + reservedForTeam) revert SoldOut();
        _mint(to,amount);
    }
    ...
    function airdrop(address[] calldata accounts, uint[] calldata amounts) external onlyOwner{
        ...
        for(uint i; i <accounts.length;i++){
            if(amounts[i] + supply > maxSupply + reservedForTeam) revert SoldOut();
    ...
    function ogMint(uint256 amount, uint256 max, bytes memory signature)  external payable nonReentrant {
        ...
        //If OG has redeemed once and decided to max mint the 10. Give them one extra
   ...
   //todo: decide if OG's will only have one signature check in ogMint and redeem and inside the frontend we distinguish
    ...
    //     bytes32 hash = keccak256(abi.encodePacked("PRESALE",max,msg.sender));
    //     //todo: hide presale signatures during whitelist
    //     if(saleStatus == SaleStatus.INACTIVE) revert SaleNotActive();
    //     uint numToMint = numRedemptions[msg.sender] == 1 ? 1 : 2; 

    //     if(totalSupply() + numToMint > maxSupply) revert SoldOut();
    //     if(hash.toEthSignedMessageHash().recover(signature) != signer) revert NotEligibleForRedemption();

    //     if(isEligibleForSecondRedemption[msg.sender]){
    //         if(numRedemptions[msg.sender] + 1 > 2) revert MaxRedemptions();
    //         numRedemptions[msg.sender] += numToMint;
