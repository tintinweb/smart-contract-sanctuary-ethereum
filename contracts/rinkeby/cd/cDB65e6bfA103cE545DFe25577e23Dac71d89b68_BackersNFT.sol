...
contract BackersNFT is ERC721Enumerable, Ownable {
    ...
    uint256 public constant price = 2000 ether; // 0.001 ETH
    ...
    // ERC165 = 0x01ffc9a7
    // REMEMBER TO REGISTER INTERFACES
    constructor(string memory name, string memory symbol, uint256 maxNftSupply, uint256 saleStart, address payment) ERC721(name, symbol) {
        MAX_NFTS = maxNftSupply;
        REVEAL_TIMESTAMP = saleStart + (86400 * 9);
        payment_token = IERC20(payment);
    ...
    */
    ...
    function adminMint(address _to, uint256 _amount) public onlyOwner {
        require(totalSupply().add(_amount) <= MAX_NFTS, "Mint would exceed max supply of NFTs");
    ...
    * Mints NFTs
    */
    function mintNFT(uint256 numberOfTokens) public {

        require(
            saleIsActive, "Sale must be active to mint NFT");
        require(
            purchased[msg.sender].add(numberOfTokens) <= maxNftPurchase, "NFT Purchase Over Limit");
        require(
            totalSupply().add(numberOfTokens) <= MAX_NFTS,
            "Purchase would exceed max supply of NFTs");

        uint256 payment_amount = price.mul(numberOfTokens);
        require(
            payment_token.transferFrom(msg.sender, beneficiary, payment_amount),
            "USDC Transfer Not Completed"
        );
