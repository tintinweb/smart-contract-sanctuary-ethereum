...
contract Guilds is ERC1155Supply {
        ...
        payable
    {
        ...
        require(getGuildByName(_name) == 0, "Name already exists");
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            if (balanceOf(to, ids[i]) == 0) {
                ...
                if (AllGuilds[ids[i]].TokenId == ids[i]) {
                    if(indexOfAddress(AllGuilds[ids[i]].GuildMembers, to) > 0) {
                    return;
                    } else {
    ...
    function modMint(uint256 _modId, uint8 _amount, uint256 _id) public payable {
        uint256 totalRate;
    ...
    function assignModerator(
        ...
        ModAddressById[_nextModId] = _account;
    ...
    function burn(uint256 _id, uint256 _amount, address _address) public onlyRole(ADMIN, _id) {
        if(totalSupply(_id) == _amount) {
            _totalGuilds -= 1;
            delete GuildByName[AllGuilds[_id].GuildName];
        }
