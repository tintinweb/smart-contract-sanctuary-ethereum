...
contract LasquareExchange is Exchange {
...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    bytes4 constant ERC1155_SIGNATURE = 0x474d492f;
     ...
     * @param sell Sell-side order
     */
    function executeFundsTransfer(Order memory buy, Order memory sell, LibPart.Part memory royalty, bool isERC1155)
        internal
        returns (uint)
    {
        uint buyAmount = 1;
        ...
        if (isERC1155) {
            (buyAmount, sellAmount) = _decodeSellBuyAmount(buy, sell);
        }
        ...
        if (price > 0 && sell.paymentToken != address(0)) {
            transferTokens(sell.paymentToken, buy.maker, sell.maker, SafeMath.mul(price, buyAmount));
        }
        /* Amount that will be received by seller (for Ether). */
        uint receiveAmount = SafeMath.mul(price, buyAmount);

        /* Amount that must be sent by buyer (for Ether). */
        uint requiredAmount = SafeMath.mul(price, buyAmount);
        ...
        if (royalty.account != address(0) && royalty.value > 0) {
            uint royaltyAmount = SafeMath.div(SafeMath.mul(SafeMath.mul(royalty.value, price), buyAmount), INVERSE_BASIS_POINT);
    ...
    function _validatePartialSellingAmount(Order memory buy, Order memory sell) internal returns(bool) { 
        uint buyAmount;
        uint sellAmount;

        bytes32 hash = hashToSign(sell);
        SellRemaining storage sellAmountRemaining = sellOrderAmountRemaining[hash];
        ...
        require(SafeMath.sub(sellAmountRemaining.left, buyAmount) >= 0, "LasquareExchange::ERC1155 remaining is not enough!");

        sellAmountRemaining.left = SafeMath.sub(sellAmountRemaining.left, buyAmount);
    ...
    function _decodeSellBuyAmount(Order memory buy, Order memory sell) internal pure returns(uint, uint) {
        uint buyAmount;
        uint sellAmount;
        ...
        assembly {
            ...
            let tokenId := mload(add(sellCallData, add(mul(0x20, 2), 0x04)))
        ...
        reentrancyGuard
    {
        ...
        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), "LasquareExchange:: Order not matched");
        ...
        if (signature == ERC1155_SIGNATURE) {
            soldOut = _validatePartialSellingAmount(buy, sell);
        }
        ...
        uint price = executeFundsTransfer(buy, sell, RoyaltiesV2(sell.target).getLasquareV2Royalties(tokenId), signature == ERC1155_SIGNATURE);
        // /* Log match event. */
