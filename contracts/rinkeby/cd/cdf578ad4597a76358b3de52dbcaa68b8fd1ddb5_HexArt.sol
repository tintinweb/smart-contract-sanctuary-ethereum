...
contract HexArt is ERC721URIStorage, ERC2981, Ownable{
    ...
    uint256 public amountToRemoveAsset = 12300000000000000;
    ...
    function removeNFTAsset(
        ...
        address _assetContract
    ) external isNFTOwner(_nftTokenId) isLocked(_nftTokenId) isNotListedOnSale(_nftTokenId) {
    ...
    /*
     * @dev  Internal function to remove NFT(HSI) assets to a given hexart.
     * @param _assetContract , NFT asset contract address.
     * @param _assetId, Array of asset(HSI) token Ids
     * @param _nftTokenId, Hexart token ID.
     */
    function removeNFT(
        ...
        for (uint256 i = 0; i < _assetId.length; i++) {
          (bool check,uint256 index) = isAssetAttached(_nftTokenId, _assetId[i], _assetContract);
            if(check) {
                require(
        ...
        isNotListedOnSale(_tokenId)
    {
        bool check = false;
         ...
         TransferHelper.safeTransferFrom(_assetContract, _msgSender(),address(this),_amount);   
    ...
    function removeErc20(address _assetContract, uint256 _nftTokenId) internal {
        for (uint256 i = 0; i < erc20AssetDetails[_nftTokenId].length; i++) {
            if (erc20AssetDetails[_nftTokenId][i].assettype == _assetContract) {
                TransferHelper.safeTransfer(_assetContract,_msgSender(),erc20AssetDetails[_nftTokenId][i].balance);
