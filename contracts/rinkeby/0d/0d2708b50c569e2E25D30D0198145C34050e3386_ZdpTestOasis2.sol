...
contract ZdpTestOasis2 is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  function addRevenue() public payable {
    withdrawPool = withdrawPool + (msg.value / 2);
    revenuePool = revenuePool + (msg.value / 2);
  }
  ...
  function withdraw() public onlyOwner nonReentrant {
    if (block.timestamp < refundEndTime) {
      ...
      (bool os, ) = payable(owner()).call{value: withdrawPool+refundPool+revenuePool}('');
      withdrawPool = 0;
      refundPool = 0;
      revenuePool = 0;
      require(os);
