...
contract OkxNFTMarketAggregator {
    // keccak256("initialized.solt.okx.com")
    bytes32 private constant INITIALIZED_SLOT =
        0xafea1f7e0ffaedf8397dc361736ef92ea03e0765db68769c3d00556903523e72;
    // keccak256("market.registry.slot.okx.com")
    bytes32 private constant MARKET_REGISTRY_SLOT =
        0x3808cf44db8d31ce4aa26fc1a256011b32fd223c8ebd45c5caeeec3c57df28db;
    // keccak256("aggregator.owner.slot.okx.com");
    bytes32 private constant OWNER_SLOT =
        0x495126d7ef81511263f0e8c560c06364d2e8af9bf336e736a7de6c0f9b03ee92;

    event MatchOrderResults(bytes32[] orderHashes, bool[] results);

    function init(address _owner, address _marketRegistry) external {
        assembly {
            if not(iszero(sload(INITIALIZED_SLOT))) {
                // TODO: error message.
                revert(0, 0)
            }
            sstore(INITIALIZED_SLOT, 0x1)
            sstore(MARKET_REGISTRY_SLOT, _marketRegistry)
    ...
    function _marketRegistry() internal view returns (address registry) {
        assembly {
            registry := sload(MARKET_REGISTRY_SLOT)
        }
    }
        ...
        payable
    {
        ...
        address marketRegistry = _marketRegistry();

        for (uint256 i; i < length; ) {
            (address proxy, bool isLib, bool isActive) = MarketRegistry(
                marketRegistry
