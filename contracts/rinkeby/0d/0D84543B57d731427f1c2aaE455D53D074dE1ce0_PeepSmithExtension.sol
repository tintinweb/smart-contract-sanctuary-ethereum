...
contract PeepSmithExtension is AdminControl, ICreatorExtensionTokenURI {
    ...
    modifier validToken(uint _tokenId) {
      require(_tokenId >= _startTokenId &&  _tokenId <= _currentTokenId, 'Invalid token ID');
      _;
    }
    ...
    function initialize(address core, uint256 currentTokenId) public adminRequired {
      _core = core;
      _currentTokenId = currentTokenId;
      _startTokenId = currentTokenId + 1;
    }

    function createNewToken(uint256 preSalePrice, uint256 publicSalePrice, uint256 tokenPreSaleSupply, uint256 tokenMaxSupply, string memory baseURI) public adminRequired {
    ...
    function mint(uint tokenId, bytes32[] calldata _merkleProof) payable public validToken(tokenId) {
      require(saleActive[tokenId] == true, 'Sale has not started yet.');
      require(!tokenClaimed[tokenId][_msgSender()], 'Address already claimed!');
    ...
    function _mintPreSale(uint256 _tokenId, bytes32[] calldata _merkleProof) private mintCompliancePreSale(_tokenId) mintPriceCompliancePreSale(_tokenId) {
    ...
    function _mintPublic(uint256 _tokenId) private mintCompliancePublicSale(_tokenId) mintPriceCompliancePublicSale(_tokenId) {
      tokenClaimed[_tokenId][_msgSender()] = true;
      address[] memory to = new address[](1);
      to[0] = msg.sender;
      uint256[] memory tokenIds = new uint256[](1);
    ...
    function setBaseURI(uint256 tokenId, string memory baseURI) public validToken(tokenId) adminRequired {
    ...
    function setPreSalePrice (uint256 tokenId, uint256 price) public adminRequired {
      costPreSale[tokenId] = price;
    }

    function setPublicSalePrice (uint256 tokenId, uint256 price) public adminRequired {
