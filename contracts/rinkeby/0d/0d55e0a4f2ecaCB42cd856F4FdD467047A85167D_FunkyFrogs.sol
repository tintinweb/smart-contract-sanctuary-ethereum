...
contract FunkyFrogs is ERC721, Ownable {
    ...
    function mint(uint256 _amount) public payable _checkMint(_amount) {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        if (!revealed) {
            ...
            return bytes(_baseURI()).length > 0 ? string(abi.encodePacked(baseURI, _tokenId.toString(), ".json")) : "";
        }
    }

    function whitelistMint(uint256 _amount, uint256 _allowance, bytes32[] calldata _proof) public payable _checkMint(_amount) {
    ...
    function getPrerevealURI() public view onlyOwner returns (string memory) {
        return prerevealURI;
    }
    ...
    function withdraw() public onlyOwner {
        ...
        (bool success1, ) = payable(wallet1).call{value: (balance * 40) / 1000}("");
        (bool success2, ) = payable(wallet2).call{value: (balance * 10) / 1000}("");
        (bool success3, ) = payable(owner()).call{value: (balance * 950) / 1000}("");
        require(success1 && success2 && success3);
    }
}
/**
