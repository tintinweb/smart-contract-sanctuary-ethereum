...
contract BOSSxNiftyTailorPass is MintPassOptimized {

    using SafeMath for uint256;

    uint256 constant BossMintPrice = 1 ether * 269 / 1000 ;
    uint256 constant BossMaxAllowed = 5000;

    uint256 constant stage1length = 2 * 24 * 60 * 50; // 48h
    uint256 constant stage1perNFT = 2; 
    uint256 constant stage2length = 5 * 24 * 60 * 50; // 120h
    uint256 constant stage2perAddress = 10; 

    mapping(address => uint256) public _mintedPerOG;

    address GENESIS = 0x0C8b0e8f975bF3Dfb99904005385E825a391af81;

    constructor( address, address _dape, address _rr, uint256 _mintStartTime) MintPassOptimized(
        "BOSSxNiftyTailorPass",
        "BOSSNIFTYPASS", 
        0x0000000000000000000000000000000000000000, // BAYC 
        0x0000000000000000000000000000000000000000, // MAYC
        _dape, // DAPE
        _rr,
        _mintStartTime 
    ){
    ...
    function getGenesisAddress() internal view virtual returns (address){
        return GENESIS;
    }

    function getMintPrice() internal view virtual override returns (uint){
        return BossMintPrice;
    } 

    function getMaxAllowed() internal view virtual override returns (uint){
        return BossMaxAllowed;
    } 
    
    function getStage() public view returns (uint stage){
        if (block.timestamp < getMintStartTime()){
            stage = 0;
        } else if(block.timestamp < getMintStartTime() + stage1length){
            stage = 1;
        } else if(block.timestamp <= getMintStartTime() + stage1length + stage2length){
            stage = 2;           
        } else {
            stage = 3;
        }
    ...
    function mintedForOG( address addr ) public view returns (uint256)  {
        return _mintedPerOG[addr];
    }
    function setMintedForOG( address addr, uint value ) internal {
        _mintedPerOG[addr] = value;
    }

    function _mintFor( uint256 mintpassNumber ) internal override {

        require( getMintPrice().mul(mintpassNumber) == msg.value, "Ether value sent is not correct");

        uint stage = getStage();

        // this check already filtered in mintMintPass()
        // require( stage > 0, "Minting not started");
        require( stage < 3, "Minting finished");
        require( getMaxAllowed() >= (mintedTotal() + mintpassNumber), "Limit reached");


        if (stage == 1) {
            uint256 minted = mintedForOG(_msgSender());
            uint256 originsNumber = _getBalance(getGenesisAddress());

            require(originsNumber * stage1perNFT >= minted + mintpassNumber, "Not enough origins.");
            setMintedForOG(_msgSender(), minted + mintpassNumber);
        } else {
            uint256 minted = mintedForAddress(_msgSender());
            require( stage2perAddress >= (minted + mintpassNumber), "Too much MintPasses requested");
            setMintedForAddress(_msgSender(), minted + mintpassNumber);
        }        
    ...
    function contractURI() public pure override returns (string memory) {
        return "https://niftytailor.com/contracts/bosspass.json";
    }

    function tokenURI(uint256 tokenId) public view override virtual returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");

        return "https://niftytailor.com/token/bosspass.json";
    }
    
}

contract BOSSxNiftyTailorPassRinkeby is BOSSxNiftyTailorPass {

    uint private __mintStartTime;
    uint private __maxAllowed;
    uint private __BossMintPrice;
    address private immutable __GENESIS;

    constructor( address _genesis, address _dape, address _rr, uint256 _mintStartTime) BOSSxNiftyTailorPass(_genesis, _dape, _rr, _mintStartTime){
        __BossMintPrice = BossMintPrice;
        __mintStartTime = _mintStartTime;
        __maxAllowed = BossMaxAllowed;
        __GENESIS = _genesis;
    }

    function getGenesisAddress() internal view override returns (address){
        return __GENESIS;
    }

    function getMintStartTime() public override view returns (uint256){
        return __mintStartTime;
    }

    function setMintStartTime(uint newTime) public onlyOwner {
        __mintStartTime = newTime;
    }

    function getMaxAllowed() internal view override returns (uint){
        return __maxAllowed;
    } 

    function setMaxAllowed(uint value) public onlyOwner {
        __maxAllowed = value;
    }

    function getMintPrice() internal view override returns (uint){
        return __BossMintPrice;
    } 

    function setMintPrice(uint value) public onlyOwner {
        __BossMintPrice = value;
    }
