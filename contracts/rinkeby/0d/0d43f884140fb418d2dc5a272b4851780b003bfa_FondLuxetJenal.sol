...
contract FondLuxetJenal is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("Fond Luxet Jenal", "FLJ") {
        setHiddenMetadataUri("ipfs://QmPKCqZRNgtbZMLLXwM2yCLNqr6chX2ikknbrPaiJ6NXYG/1.json");
    }
    ...
    function mintPublic(uint256 _amount) external payable nonReentrant {
        ...
        require(_amount <= maxPerTXN && _amount > 0, "You can not mint that much per one transaction");
        require(_amount + totalSupply() <= publicSupply, "There is not enough Public supply left");
    ...
    function mintWL(bytes32[] calldata _merkleProof) external payable nonReentrant {
        require(wlMintStart, "WL mint has not commenced yet");
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(MerkleProof.verify(_merkleProof, rootWL, leaf), "You are not eligible");
        require(WLmint[msg.sender] < 1, "You have already WL minted.");
        require(totalSupply() + wlamount <= maxTotalSupply, "There is not enough supply left");
        WLmint[msg.sender] += wlamount;
    ...
    function startWLmint(bool _state) external onlyOwner {
        wlMintStart = _state;
    }
