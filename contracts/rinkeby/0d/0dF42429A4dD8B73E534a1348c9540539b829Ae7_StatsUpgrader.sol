...
contract StatsUpgrader is CharacterAccessControl {
    ...
    mapping(uint256 => mapping(string => mapping(uint256 => uint256))) levelToShroom;
    //mapping(string => mapping(uint256 => uint256)) shroomToLevel;
    mapping(string => uint256) maxLevel;
    ...
    event StatsUpgrade(address user, uint256 tokenId, string skill, uint256 oldLevel, uint256 increment);
    ...
    function _setMaxLevel(string memory _skill, uint256 _max) internal {
        maxLevel[_skill] = _max;
    }

    function setMaxLevel(string memory _skill, uint256 _max) public onlyRole(HANDLER_ROLE) {
        _setMaxLevel(_skill, _max);
    }

    function setMaxLevels(string[] memory _skills, uint256[] memory _maxs) public onlyRole(HANDLER_ROLE) {
        for(uint256 i=0;i<_skills.length;i++) {
            _setMaxLevel(_skills[i], _maxs[i]);
        }
    }
    ...
    function _upgrade(
        ...
        bytes32 hash
            ...
            for(uint256 i=0;i<skills.length;i++) {
                ...
                click = clicks[i];
                require(currLevel == statsManager.get(tokenId, skills[i]), "Skill current level mismatch");
                //require(currLevel <= maxLevel[skills[i]], "Skill stats already max");
                if(click > maxClicks) {
                    click = maxClicks;
                }
                for(uint256 j=0;j<click;j++) {
                        shroomUsed += levelToShroom[character][skills[i]][currLevel];
                        (hash,random) = _generateRandomNumber(hash,10*i+j);
                        if((currLevel + random) > maxLevel[skills[i]]) {
                            random = maxLevel[skills[i]] - currLevel;
                        }
                        emit StatsUpgrade(user, tokenId, skills[i], currLevel, random);
                        currLevel += random;
                }
    ...
    function upgrade(
        ...
        _upgrade(msg.sender,tokenId,skills,currentLevels,clicks,hash);
        lastUser = msg.sender;
    ...
    function _generateRandomNumber(bytes32 hash, uint256 index) internal view returns (bytes32 seed, uint256 random) {
        ...
        random = uint256(seed) % 10;
        if(random == 9) {
            random = (uint256(seed) / 10) % 10;
        }
        random = random % 3 + 1;
    }
