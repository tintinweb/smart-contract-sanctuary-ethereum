...
contract Jamaal is ERC721A, Ownable {
	...
	uint256 public constant WHITELIST_SALE_PRICE = 0.08 ether;
	...
	constructor(
		...
		baseTokenUri = uri;
		placeholderTokenUri = placeholderUri;
		root = merkleRoot;
	}

	modifier callerIsUser() {
		require(tx.origin == msg.sender, "Jamaal :: Cannot be called by a contract");
		_;
	}

	function mint(uint256 _quantity) external payable callerIsUser {
		require(publicSale, "Jamaal :: Not Yet Active.");
		require((totalSupply() + _quantity) <= MAX_SUPPLY, "Jamaal :: Beyond Max Supply");
		require(
			(totalPublicMint[msg.sender] + _quantity) <= MAX_PUBLIC_MINT,
			"Jamaal :: Already minted 3 times!"
		);
		require(msg.value < (PUBLIC_SALE_PRICE * _quantity), "Jamaal :: Below ");
	...
	function whitelistMint(bytes32[] memory _merkleProof, uint256 _quantity)
		...
		callerIsUser
	{
		require(whiteListSale, "Jamaal :: Minting is on Pause");
		require(
			(totalSupply() + _quantity) <= MAX_SUPPLY,
			"Jamaal :: Cannot mint beyond max supply"
		);
		require(
			(totalWhitelistMint[msg.sender] + _quantity) <= MAX_WHITELIST_MINT,
			"Jamaal :: Cannot mint beyond whitelist max mint!"
		);
		require(
			msg.value >= (WHITELIST_SALE_PRICE * _quantity),
			"Jamaal :: Payment is below the price"
		);
		//create leaf node
		bytes32 sender = keccak256(abi.encodePacked(msg.sender));
		require(
			MerkleProof.verify(_merkleProof, merkleRoot, sender),
			"Jamaal :: You are not whitelisted"
		);
	...
	function teamMint() external onlyOwner {
		require(!teamMinted, "Jamaal :: Team already minted");
		teamMinted = true;
