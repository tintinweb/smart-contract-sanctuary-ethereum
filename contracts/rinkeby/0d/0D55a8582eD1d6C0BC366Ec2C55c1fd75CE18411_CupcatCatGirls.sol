// SPDX-License-Identifier: MIT
 ...
 contract CupcatCatGirls is ERC721Enumerable, Ownable, ReentrancyGuard {
     ...
     modifier wlIsOpen {
         require(wlState, "WL Sale is Closed");
         require(!paused, "Contract Paused");
         _;
     }

     // public
     function whiteListMint(bytes memory signature, uint8 _count, uint8 wlSpot) public payable wlIsOpen nonReentrant {
         require(_tokenIdTrackerSale.add(1) <= MAXSALE, "Sold Out!");
         require(msg.value == costWl.mul(_count), "Ether value sent is not correct");
         require(addressMintedBalance[msg.sender] + _count <= wlSpot, "All WL Spots have been consumed");
     ...
     function claim(uint256[] memory _tokensId) public claimIsOpen nonReentrant {
