    ...
    OwnableUpgradeable
{
    ...
    event ChangeStableToken(address newStableToken, address oldStableToken);

    event ChangeWETH(address newWETH, address oldWETH);

    event ChangeRouter(address newRouter, address oldRouter);
    ...
    function lock(
        ...
        address _tempTokenSource = pathsEst[0];

        if (msg.value > 0) {
            IWETH(WETH).deposit{value: msg.value}();
            pathsEst[0] = WETH;
            _tempTokenSource = address(0);
            _amountTokenIn = msg.value;
        }

        if (pathsEst.length != 0) {
            if (pathsEst[pathsEst.length - 1] != stableToken)
                pathsEst[pathsEst.length] = stableToken;
        } else pathsEst[pathsEst.length] = stableToken;
        ...
        address[] memory paths = pathsEst;
        // paths[0] = _tempTokenSource;
        // paths[1] = stableToken;

        // swap token -> usdc
        if (pathsEst[0] != stableToken) {
    ...
    function unlock(
        ...
        require(pathsEst[0] == stableToken, "Est wrong path");
        address _tempTokenDestination = pathsEst[pathsEst.length - 1];
        for (uint256 i = 0; i < pathsEst.length; i++) {
            if (pathsEst[i] == address(0)) {
                pathsEst[i] = WETH;
            }
        }
        require(
        ...
        uint256 _beforeSwap = IERC20(pathsEst[pathsEst.length - 1]).balanceOf(
            address(this)
        );
        if (_tempTokenDestination != stableToken) {
            ...
            transferAmount =
                IERC20(pathsEst[pathsEst.length - 1]).balanceOf(address(this)) -
                _beforeSwap;
        }

        if (_tempTokenDestination == address(0)) {
            IWETH(WETH).withdraw(transferAmount);
            _recipient.transfer(transferAmount);
        } else {
            IERC20(pathsEst[pathsEst.length - 1]).safeTransfer(
                _recipient,
                transferAmount
            );
        }
    ...
    function changeStableToken(address newStableToken)
        public
        nonReentrant
        onlyOwner
    {
        require(stableToken != newStableToken, "No change stable token");
        emit ChangeStableToken(newStableToken, stableToken);

        stableToken = newStableToken;
    }

    function changeWeth(address _weth) public nonReentrant onlyOwner {
        require(WETH != _weth, "No change weth");
        emit ChangeWETH(_weth, WETH);
        WETH = _weth;
    }

    function changeRouter(address _routerV2) public nonReentrant onlyOwner {
        require(uniswapV2Router02 != _routerV2, "No change router");

        emit ChangeRouter(_routerV2, uniswapV2Router02);
        uniswapV2Router02 = _routerV2;
    }
    ...
    function _getExactTokenForToken(
        ...
        uint256[] memory results = IUniswapV2Router02(uniswapV2Router02)
            .getAmountsOut(_amountTokenIn, path);
    ...
    function _getTokenForExactToken(
        ...
        uint256[] memory results = IUniswapV2Router02(uniswapV2Router02)
            .getAmountsIn(_amountTokenOut, path);
        return results[0];
    }
        ...
        returns (uint256, uint256)
    {
        if (path[0] == path[path.length - 1])
            return (_amountTokenIn, _amountTokenIn);
        address factory = IUniswapV2Router02(uniswapV2Router02).factory();
        address getSushiTokenIn = IUniswapV2Factory(factory).getPair(
            path[0],
            path[1]
        );

        address getSushiTokenOut = IUniswapV2Factory(factory).getPair(
            path[path.length - 2],
            path[path.length - 1]
        );

        uint256[2] memory reserve = [
            IERC20(path[0]).balanceOf(getSushiTokenIn) + _amountTokenIn,
            IERC20(path[1]).balanceOf(getSushiTokenOut) -
    ...
    {
        if (path[0] == path[path.length - 1])
            return (_amountTokenOut, _amountTokenOut);
        address factory = IUniswapV2Router02(uniswapV2Router02).factory();
        ...
        uint256[2] memory reserve = [
            IERC20(path[0]).balanceOf(getSushiTokenIn) +
                _getTokenForExactToken(_amountTokenOut, path),
            IERC20(path[path.length - 1]).balanceOf(getSushiTokenOut) -
                _amountTokenOut
        ];
    ...
    function getImpactGetExactTokenForStableToken(
        ...
        return _getImpactTokenOut(_amountTokenIn, _paths);
    }
    ...
    function getImpactGetTokenForExactStableToken(
        ...
        return _getImpactTokenIn(_amountStableTokenOut, _paths);
    }
