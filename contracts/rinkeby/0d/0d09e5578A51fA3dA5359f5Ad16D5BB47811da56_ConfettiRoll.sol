...
contract ConfettiRoll is AccessControlEnumerable, Ownable, Pausable {
    ...
    function withdrawTip() public onlyOwner {
        confetti.transfer(msg.sender, tipAmount);
        tipAmount = 0;
    }
    ...
    // We piggyback on the RaidParty batch seeder for our game round abstraction
    function currentRound() public view returns (uint256) {
        return seeder.getBatch();
    }

    // Return generated random words for a given game round
    ...
    function withdrawRewards() public whenNotPaused {
        ...
        for (uint256 i = 0; i < pendingGames[msg.sender].length(); ) {
            bytes32 gameId = pendingGames[msg.sender].at(i);
            if (isGameFinished(gameId)) {
                rewards += gameResults[gameId].prizeShare;
                pendingGames[msg.sender].remove(gameId);
    ...
    // Given a game result, return an address of a player who lost
    ...
    // Calculated the game identifier for a given game initializer and roundNum
    ...
    // Calculates a game identifier for the currently running, global game
    ...
    // Given an unitialized yet game, simulate the game and commit the results
    // to the storage
    function simulateGame(bytes32 gameId) internal onlyOwner whenNotPaused {
        require(!isGameFinished(gameId), "Game already finished");
        Game memory game = games[gameId];
