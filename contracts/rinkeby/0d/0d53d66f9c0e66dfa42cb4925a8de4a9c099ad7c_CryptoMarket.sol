...
contract CryptoMarket is ReentrancyGuard {
  ...
  uint256 commission_for_marketplace = 0;
  ...
  function change_commision_fees(uint256 fees_percentage) public{

    require(msg.sender == owner);

    commission_for_marketplace =fees_percentage;
  ...
  function createMarketSale_erc721(uint256 itemId) public payable nonReentrant{
    ...
    require(msg.value >= price*(royality+1+commission), 'Please submit the asking price in order to continuee the transcation');
    ...
    if(markettokens[itemId].is_fixed==true){
      ...
      owner.transfer(price*commission);
