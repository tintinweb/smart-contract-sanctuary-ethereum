...
contract MNFTFactory is IMNFTFactory {
   ...
   function createMNFT721(
      ...
      MNFT721 token = new MNFT721(name_, symbol_);
      token.setMarketplaceAddress(marketplaceAddress);

      string[] memory tokenURIs = new string[](1);
      tokenURIs[0] = tokenURI_;
      token.mintNFT(tokenURIs, owner_);
   ...
   function createMNFT1155(
      ...
      MNFT1155 token = new MNFT1155(name_, symbol_);
      token.setMarketplaceAddress(marketplaceAddress);
      token.mintNFT(owner_, tokenURI_, amount_);
