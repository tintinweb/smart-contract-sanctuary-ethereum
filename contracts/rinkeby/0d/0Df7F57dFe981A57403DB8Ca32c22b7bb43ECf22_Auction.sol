...
import "./openzeppelin-contracts/Math.sol";
...
contract Auction is Ownable {
    ...
    modifier atStageOrderPlacementAndCancelation(uint256 auctionId) {
        require(
            ...
            "Auction: no longer in order placement and cancelation phase"
        );
        _;
    }
    ...
    modifier atStageSolutionSubmission(uint256 auctionId) {
        {
            ...
            require(
                ...
                "Auction: Auction not in solution submission phase"
            );
        }
        _;
    }
    ...
    modifier atStageFinished(uint256 auctionId) {
        require(
            auctionData[auctionId].clearingPriceOrder != bytes32(0),
            "Auction: Auction not yet finished"
        );
        _;
    }
    ...
    function setFeeParameters(
        ...
        require(
            newFeeNumerator <= 15,
            "Auction: Fee is not allowed to be set higher than 1.5%"
        );
    ...
    function initiateAuction(
        ...
        require(_auctionedSellAmount > 0, "Auction: cannot auction zero tokens");
        require(_minBuyAmount > 0, "Auction: tokens cannot be auctioned for free");
        require(
            minimumBiddingAmountPerOrder > 0,
            "Auction: minimumBiddingAmountPerOrder is not allowed to be zero"
        );
        require(
            orderCancellationEndDate <= auctionEndDate,
            "Auction: time periods are not configured correctly"
        );
        require(
            auctionEndDate > block.timestamp,
            "Auction: auction end date must be in the future"
        );
    ...
    ) internal returns (uint64 userId) {
        {
            ...
            if (allowListManager != address(0)) {
                require(
                    ...
                    "Auction: user not allowed to place order"
                );
            }
        }
        {
            ...
            for (uint256 i = 0; i < _minBuyAmounts.length; i++) {
                require(
                    _sellAmounts[i].div(_minBuyAmounts[i]) <= 500* 10**6,
                    "Auction: premium cannot greater than min deposit"
                );
                require(
                    _minBuyAmounts[i].mul(buyAmountOfInitialAuctionOrder) <
                        sellAmountOfInitialAuctionOrder.mul(_sellAmounts[i]),
                    "Auction: limit price not better than mimimal offer"
                );
            }
        }
        ...
        atStageOrderPlacementAndCancelation(auctionId)
    {
        ...
        for (uint256 i = 0; i < _sellOrders.length; i++) {
            ...
            if (success) {
                ...
                require(
                    userIdOfIter == userId,
                    "Auction: Only the user can cancel his orders"
                );
    ...
    function precalculateSellAmountSum(
        ...
        require(
            ...
            "Auction: reached end of order list"
        );
        ...
        require(
            ...
            "Auction: too many orders summed up"
        );
        ...
        )
    {
        for (uint256 i = 0; i < orders.length; i++) {
            ...
            require(
                ...
                "Auction: order is no longer claimable"
            );
        }
        ...
        for (uint256 i = 0; i < orders.length; i++) {
            ...
            require(
                userIdOrder == userId,
                "Auction: only allowed to claim for same user"
            );
    ...
    function registerUser(address user) public returns (uint64 userId) {
        ...
        require(
            ...
            "Auction: User already registered"
        );
        userId = numUsers;
