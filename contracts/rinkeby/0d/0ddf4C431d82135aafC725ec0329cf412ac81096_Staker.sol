...
contract Staker {
  ...
  // Deadline Over
  modifier deadlineOver(bool deadlineDead) {
    if (deadlineDead) {
  ...
  function stake() public payable deadlineOver(true) {
  ...
  function execute() public deadlineOver(false) stakeNotCompleted {
    if(address(this).balance >= threshold) {
      (bool sent, ) = address(exampleExternalContract).call{value: address(this).balance}(abi.encodeWithSignature("complete()"));
      require(sent, "ExternalContract.complete failed.");
    } else {
      console.log("Couldn't executed.");
    }
  }

  // Add a `withdraw()` function to let users withdraw their balance
  function withdraw() public deadlineOver(false) stakeNotCompleted {
    if (address(this).balance < threshold) {
      uint userBalance = balances[msg.sender];
      require(userBalance > 0, "You don't have balance for withdrawal.");
