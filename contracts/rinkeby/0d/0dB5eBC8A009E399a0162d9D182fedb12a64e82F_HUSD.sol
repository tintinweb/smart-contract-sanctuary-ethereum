    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(address => bool) private usdPeggedERC20;
    ...
    function swapIn(address _erc20, uint256 _amount) external nonReentrant {
        require(
            isWhitelistedERC20(_erc20),
            "SwapIn:: _erc20Token ERC20 not whitelisted"
        );
        uint256 _erc20Decimal = getTokenDecimals(_erc20);
        if (_erc20Decimal == 18) {
        ...
        } else if (_erc20Decimal < 18) {
            uint256 _diff = 18 - _erc20Decimal;
            uint256 _exp = 10**_diff;
            uint256 amount = _amount * _exp;
            TransferHelper.safeTransferFrom(
                _erc20,
                msg.sender,
                address(this),
    ...
    function swapOut(address _erc20, uint256 _amount) external nonReentrant {
        require(
            isWhitelistedERC20(_erc20),
            "SwapOut:: _erc20Token ERC20 not whitelisted"
        );
        uint256 _erc20Decimal = getTokenDecimals(_erc20);
        if (_erc20Decimal == 18) {
            TransferHelper.safeTransfer(_erc20, msg.sender, _amount);
            _burn(msg.sender, _amount);
        } else if (_erc20Decimal < 18) {
            uint256 _diff = 18 - _erc20Decimal;
            uint256 _exp = 10**_diff;
            uint256 amount = _amount * _exp;
            TransferHelper.safeTransfer(_erc20, msg.sender, amount);
            _burn(msg.sender, _amount);
        }
    }

    function getTokenDecimals(address _erc20) internal view returns (uint256) {
        return ERC20Upgradeable(_erc20).decimals();
    }

    function isWhitelistedERC20(address _erc20) public view returns (bool) {
        return usdPeggedERC20[_erc20];
    }

    function addBlackList(address _who) public onlyRole(GOVERNANCE_ROLE) {
        _updateBlackList(_who, true);
        emit AddedBlackList(_who, block.number);
    }

    function removeBlackList(address _who) public onlyRole(GOVERNANCE_ROLE) {
    ...
    function updateERC20Whitelist(address _erc20Token, bool _status)
        ...
        onlyRole(GOVERNANCE_ROLE)
    {
        usdPeggedERC20[_erc20Token] = _status;
