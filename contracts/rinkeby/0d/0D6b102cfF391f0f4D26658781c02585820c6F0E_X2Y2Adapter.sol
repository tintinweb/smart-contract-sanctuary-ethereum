...
contract X2Y2Adapter is BaseAdapter {
    ...
    bytes32 internal constant _ORDER_TYPEHASH =
        keccak256(
            "Order(uint256 salt,address user,uint256 network,uint256 intent,uint256 delegateType,uint256 deadline,address currency,bytes dataMask,OrderItem[] items,bytes32 r,bytes32 s,uint8 v,uint8 signVersion)OrderItem(uint256 price,bytes data)"
        );

    bytes32 internal constant _SETTLE_DETAIL_TYPEHASH =
        keccak256(
            "SettleDetail(uint8 op,uint256 orderIdx,uint256 itemIdx,uint256 price,bytes32 itemHash,address executionDelegate,bytes dataReplacement,uint256 bidIncentivePct,uint256 aucMinIncrementPct,uint256 aucIncDurationSecs,Fee[] fees)Fee(uint256 percentage,address to)"
        );

    bytes32 internal constant _SETTLE_SHARED_TYPEHASH =
        keccak256(
            "SettleShared(uint256 salt,uint256 deadline,uint256 amountToEth,uint256 amountToWeth,address user,bool canFail)"
        );

    bytes32 internal constant _ORDER_ITEM_TYPEHASH = keccak256("OrderItem(uint256 price,bytes data)");
    bytes32 internal constant _FEE_TYPEHASH = keccak256("Fee(uint256 percentage,address to)");
