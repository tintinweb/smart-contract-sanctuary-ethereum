...
contract Wordle is ERC721, Ownable, Pausable, IERC721Receiver, ReentrancyGuard {
  ...
  function depositTokens(uint256[] calldata tokenIds) external whenNotPaused {
  ...
  function withdrawTokens(uint256[] calldata tokenIds) external whenNotPaused nonReentrant() {
    for (uint256 i; i < tokenIds.length; i++) {
      require(
          _deposits[tokenIds[i]] == msg.sender,
  ...
  // function updateScore(uint256 tokenId, bytes32 score) external {
