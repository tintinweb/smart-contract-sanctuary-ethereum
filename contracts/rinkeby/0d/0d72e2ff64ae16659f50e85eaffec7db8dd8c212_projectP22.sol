...
contract projectP22 is ERC1155, Ownable {
    ...
    function checkIndexIsAvailbaleTest(uint randomNumber, uint32 packId) public returns (uint) {
        return checkIndexIsAvailbale(randomNumber, packId);
    }
    ...
    function OpenPack(uint32 packId) external {
        ...
        uint tokenStartIndex = uint(startTokenIdOfRelease[packId] + (newImageIndex * numOfTokensInPerPack[packId]));
