...
//Bug1 - BridgeEnabled Checks are the wrong way - FIXED TO TEST
//Can remove getmerkleroot function
// Get Received Count ?
// Get Bridged Count??

//Bug 2 - Missing a Way to Set Lizardcontract Address .. - FIXED TO TEST
...
contract OGLizardBridge is ERC1155Holder, Ownable, ReentrancyGuard {
    ...
    // Received and successfully claimed (this contains newly 721 ids)
    uint256[] public idsMigrated;  

    //keeps all the ids that are sent and the owners of them
    mapping(uint256 => address) public OSTokenOwner;
    mapping(address => uint256[]) public OSIDByAddress;

    //Mapping of all OS IDS that have been Migrated 
    mapping(address => uint256[]) public OSIDMigratedByAddress;
    //mapping(uint256 => bool) public tokenMigrated;
    
   
    bytes32 public merkleRoot;


    //OpenSea and New OG Lizards ERC721 contract
   ...
   //triggered by 1155 transfer only from openSea
    function onERC1155Received(
        ...
        //Basically Updates all the Arrays to record which Tokens received and Mappings from Address ->Token ID
    ...
    //ALL OPEN
    function migrate(uint256 _oldId, uint256 _newId) external nonReentrant {  //MERKLESHIT REMOVED
      ...
      //  require(MerkleProof.verify(_merkleProof, merkleRoot, _leaf), "Invalid proof.");
        ...
        //CHECK THIS PROBABLY DONT EVEN NEED IT
        //require(!OGLizardContract.exists(_newId), "Token exists");
        ...
        //Sets Bridged Flag to True
        // tokenMigrated[_oldId] = true;
    }


    //NO BURN
        function migrate2(uint256 _oldId, uint256 _newId) external nonReentrant {  //MERKLESHIT REMOVED
      ...
      //  require(MerkleProof.verify(_merkleProof, merkleRoot, _leaf), "Invalid proof.");
       ...
       //MINT AT WILL - NO BURN
        function migrate3(uint256 _oldId, uint256 _newId) external nonReentrant {  //MERKLESHIT REMOVED
     ...
     //MINT AT WILL - NO BURN
        function migrate4(uint256 _oldId, uint256 _newId) external nonReentrant {  //MERKLESHIT REMOVED
     ...
     //MINT AT WILL - NO BURN
        function migrate5(uint256 _newId) external nonReentrant {  //MERKLESHIT REMOVED
   ...
   // =======================    EMERGENCY MINTS AND TRANSFERS ONLY ========================================== //
    ...
    // Record Details of Tokens Received from Opensea
    function triggerReceived1155(address _sender, uint256 _tokenId) internal {
        require(_sender != address(0), "Update from address 0");

        //This just keeps a Total TokenID's Received, Pushes latest one to the end of the array
        idsReceived.push(_tokenId);

        //Maps the Opensea Token to a Single Address
        OSTokenOwner[_tokenId] = _sender;

        //Adds The Token ID to the Senders Mapping (Address)
    ...
    //sets Existing Genesis Lizard Opensea Contract 
   ...
   //Get Total Number of ID's Migrated
   ...
   //Get list of Tokens Received
    ...
    function burnTheOldToken(uint256 _oldId) external onlyOwner {
       // Burn the old token from Opensea contract
        openSeaSF.safeTransferFrom(address(this), BURN_ADDRESS, _oldId, 1, "");
    }

    //TEST FUNCTION TO REMOVE
    function burnThetokenExternal(uint256 _oldId) external {
       // Burn the old token from Opensea contract
        openSeaSF.safeTransferFrom(address(this), BURN_ADDRESS, _oldId, 1, "");
    }
