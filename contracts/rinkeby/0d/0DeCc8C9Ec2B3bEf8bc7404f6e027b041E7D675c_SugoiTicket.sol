...
contract SugoiTicket is Satoshigoat {

	//@dev Plug instance: mainnet!
	Plug constant public thePlug = Plug(0x2Bb501A0374ff3Af41f2009509E9D6a36D56A6c0);

	string [MAX_NUM_TOKENS] _hashes = ["QmcACQRDcWwhVXCRCKgFyy6jYUVbxTProhwWLVMdBn3b2z",
									   "QmTtw7j9LjHsHMxWC9LsBZPtKES2ioqrLaQsscN1DpPoux",
									   "QmYF5LdngPYTM3kqHBGSN3npXjtcDzHnzxKBLD7Ya3LtBv"];
	...
	constructor() Satoshigoat("Sugoi NYC 2022 Ticket", "", "ipfs://") 
	{
	...
	function claim(address to, uint256 qty, uint256 plugTid) external enoughSupply(qty) nonReentrant
	{	
		...
		unchecked {
			...
			{
				// hustlers get the vip ticket
				for (i = currentIdx; i < nextIdx; i++) {
					if (i == currentIdx) 
						_tokens[i] = _hashes[2];//hustler ticket
					else 
						_tokens[i] = _hashes[0];//frens ticket
				}
			} else {
				// non-hustlers get the regular ticket
				for (i = currentIdx; i < nextIdx; i++) {
					if (i == currentIdx) 
						_tokens[i] = _hashes[1];//plug ticket
					else 
						_tokens[i] = _hashes[0];//frens ticket
				}
			}
	...
	//@dev Only Plug holders can have more than one ticket in a wallet
    ...
    {
    	if (thePlug.balanceOf(to) == 0 && balanceOf(to) == 1)
    		revert DataError("only plug holders can have more than 1 ticket");
    }
