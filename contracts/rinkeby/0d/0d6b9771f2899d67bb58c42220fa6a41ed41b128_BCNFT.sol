...
contract BCNFT is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    function mint(uint256 _level, address _to) public returns (uint256) {
        ...
        _tokenIds ++;
        _tokenBalance[_to] += 1;
        tokens[curId].tokenId = curId;
        tokens[curId].creator = _to;
        tokens[curId].owner = _to;
        tokens[curId].level = _level;
        _safeMint(_to, curId);
        updateHolders(_to);
        _updateOwnIds(_to, curId, 1);
        return curId;
    }
    ...
    function checkAccountLevels(address _account) external view returns (uint256[] memory) {
        require(_tokenBalance[_account] > 0, "No balance");
        uint256 tokenCount = _tokenBalance[_account];
        uint256[] memory ownIds = _ownIds[_account];
        uint256[] memory levels = new uint256[](tokenCount);
        for (uint256 i = 0; i < tokenCount; i++) {
            levels[i] = tokens[ownIds[i]].level;
        }
    ...
    function tokensOf(address _owner) public view returns (uint256[] memory) {
        require(_tokenBalance[_owner] > 0, "No balance");
        uint256 tokenCount = _tokenBalance[_owner];
        uint256[] memory ownIds = _ownIds[_owner];
        uint256[] memory _tokens = new uint256[](tokenCount);
        for (uint256 i = 0; i < tokenCount; i++) {
            _tokens[i] = tokens[ownIds[i]].tokenId;
        }
