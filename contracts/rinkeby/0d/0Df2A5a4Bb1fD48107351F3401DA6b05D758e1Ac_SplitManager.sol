...
contract SplitManager is BaseGovernanceWithUserUpgradable, ISplitManager {
    ...
    mapping(uint => bool) internal _isStoraged;
    mapping(uint => uint) internal _lockedPart;

    uint private constant TOTAL = 1e18;
    ...
    function __SplitManager_init_unchained(ILock _lock, uint256 _initialLockedPart) internal onlyInitializing {
        require(_initialLockedPart > 0 && _initialLockedPart < 1e18, "ERROR: The initial lockedpart must be >0 and <1e18");
        ...
        returns(bool)
    {
            ...
            uint locked = getLockedPart(originId);
            require(
                splitParts[0] >= locked, 
            ...
            for(uint i; i < splitParts.length; i++){
     ...
     */
    function getLockedPart(uint id) public view override returns(uint) {
        if(_isStoraged[id]){
    ...
    function _checkSplitPart(uint[] memory _splitParts) internal pure returns(bool) {
        uint splitPartsSum;
        for(uint i; i < _splitParts.length; i++) {
