...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    mapping(address => uint256) public exchangePrice;
    ...
    function setTreasureContract(address _treasureContract) external {
        address _manager = IUniswapV2Factory(factory).manager();
        require(
            IManager(_manager).isSuperAdmin(msg.sender),
    ...
    function setExchangePriceFor(address[] calldata _token, uint256[] calldata _price) external {
        address _manager = IUniswapV2Factory(factory).manager();
        require(
            IManager(_manager).isSuperAdmin(msg.sender),
            "UniswapV2Router: FORBIDDEN"
        ...
        require(_token.length == _price.length);

        for(uint i = 0; i < _token.length; i++) {
            exchangePrice[_token[i]] = _price[i];
        }
    }
        ...
        private
    {
        if (_option == LiquidityFeeOptions.PRINCIPLE) {
            uint256 _price = exchangePrice[_token];
            uint256 _fee = SYSTEM_FEE.mul(_price) / 1 ether;
            ...
            uint256 _price = exchangePrice[HYBEX];
            uint256 _fee = SYSTEM_FEE.mul(_price) / 1 ether;
            TransferHelper.safeTransferFrom(
                HYBEX,
                msg.sender,
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"1e3104608d2b81cebe5f40baffa9db81d0441c05301b147d5eac2ba896fa36d1" // init code hash
                    )
                )
            )
        );
