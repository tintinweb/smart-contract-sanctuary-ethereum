...
contract KarmGovernanceV0 is KarmSentry , AccessControl {
    using SafeMath for uint256;


    constructor(  ) KarmSentry() {
    ...
    function executeProposal( address[] memory _to , bytes[] memory _data ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 length = _data.length;
        require( _to.length == _data.length ,"KarmGovernance : Length Mismatch");
        require( _to.length <= 10 ,"KarmGovernance : Spell Limit exceeded");
        for ( uint256 i = 0; i <= length; i = i+1 ) {
            address caller = _to[i];
            ( bool success , ) = caller.call( _data[i] );

            if( !success ){
                revert();
            }
        }
//        return ( resultArr , returnArr );
    }
