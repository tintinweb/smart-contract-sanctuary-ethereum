...
// hevm: flattened sources of src/DegenCapital.sol
...
////// src/DegenCapital.sol
/**

DegenCapital

- Microcap DeFi 3.0 investment fund
- You buy DEGEN token, we invest in micro cap coins and you receive a % of the profits
...
contract DegenCapital is Ownable, IERC20 {
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "DegenCapital: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "DegenCapital: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "DegenCapital: Sender is blacklisted");
        require(!isBlacklisted[recipient], "DegenCapital: Recipient is blacklisted");

        require(sender != address(0), "DegenCapital: transfer from the zero address");
        require(recipient != address(0), "DegenCapital: transfer to the zero address");
        ...
        require(
            ...
            "DegenCapital: transfer amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "DegenCapital: approve from the zero address");
        require(spender != address(0), "DegenCapital: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "DegenCapital: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    }
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "DegenCapital: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "DegenCapital: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }
    ...
    function _executeSwap(uint256 tokens, uint256 native) private {
        ...
        if (nativeDev > 0) {
            payable(devWallet).transfer(nativeTreasury);
        }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "DegenCapital: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "DegenCapital: DEX pair can not be removed");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "DegenCapital: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "DegenCapital: the router is already set to the new address"
        );
    ...
    function compound() public {
        require(compoundingEnabled, "DegenCapital: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "DegenCapital_DividendTracker";
    string private _symbol = "DegenCapital_DividendTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "DegenCapital_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "DegenCapital_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "DegenCapital_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "DegenCapital_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("DegenCapital_DividendTracker: method not implemented");
    }
