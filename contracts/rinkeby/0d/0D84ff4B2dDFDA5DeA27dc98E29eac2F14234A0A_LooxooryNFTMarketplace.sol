...
contract LooxooryNFTMarketplace is ReentrancyGuard,Ownable {
    ...
    function createCampaign(
      CreateCampaignStruct memory _campaignDTO
        ...
        for(uint256 i=0;i<_campaignDTO.nfts.length;i++){
          if(nftItems[_campaignDTO.nfts[i]].owner==nftHolder && nftItems[_campaignDTO.nfts[i]].campaignID==0 && !nftItems[_campaignDTO.nfts[i]].isOnAuction ){
  ...
  function createMarketSale(
    ...
    if(donation){
        ...
        if(nftItems[itemId].fractionAmount==1){
          nftItems[itemId].campaignID = 0;
        }

        emit Donated(itemId, tokenId, nftItems[itemId].owner , msg.sender, msg.value);
    ...
    function listNftForAuction(uint256 nftID,uint256 bidEndTime) external onlyOwner{
