...
contract Minter is AccessControlEnumerable, Provenance, PaymentSplitter {
  ...
  function signedMint(
    ...
    require(
      numberOfTokens <= maxPermitted,
      "Minter: numberOfTokens exceeds maxPermitted"
    );
  ...
  function maxPurchaseBehavior(
    ...
    require(
      ...
      "Minter: Sender already minted this block"
    );
    lastBlock[msg.sender] = block.number;

    if (maxPerBlock != 0) {
      require(
        numberOfTokens <= maxPerBlock,
        "Minter: maxBlockPurchase exceeded"
      );
    }
    ...
    if (maxPerWallet != 0) {
      ...
      require(
        totalMinted[msg.sender] <= maxPerWallet,
        "Minter: Sender reached mint max"
      );
    }
  }

  function sharedMintBehavior(uint256 numberOfTokens) internal {
...
contract Provenance {
  ...
  function _finalizeStartingIndex(uint256 maxSupply) internal {
    ...
    startingIndex = (uint256(blockhash(startingIndexBlock)) % maxSupply) + 1;
    // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
    if (block.number - startingIndexBlock > 255) {
      startingIndex = (uint256(blockhash(block.number - 1)) % maxSupply) + 1;
    }
  ...
  function _setStartingBlock(uint256 currentCount, uint256 maxSupply) internal {
    ...
    if (
      ...
      (currentCount == maxSupply || block.timestamp >= revealTimestamp)
    ) {
