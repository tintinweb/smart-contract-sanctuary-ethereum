...
contract MMRaffle is Ownable, VRFConsumerBase {
    ...
    address winOnePrize = 0x01BE23585060835E02B77ef475b0Cc51aA1e0709; // address of Prize token for WinGroup(1)
    uint256 public amountWinOnePrize = 5 ether; // amount of Prize for WinGroup(1)
    address buyTicketToken = 0x01BE23585060835E02B77ef475b0Cc51aA1e0709; // address of 2omb or 3omb token
    address ownerWallet = 0x1c932aED5dE394e7D0eA8467A3Ee0989689a3B32;
    ...
    event WinnerChosen(uint256 raffleId, address winner1);
    ...
    /// @param _id raffle id
    /// @param user address of ticket owner
    function GetTicketBalance(uint256 _id, address user)
        public
        view
        returns (uint256 balance)
    {
        return Raffles[_id].ticketBalances[user];
    }

    /// @notice get ticket Owners
    /// @param _id raffle id
    function GetTicketOwners(uint256 _id)
        public
        view
        returns (address[] memory)
    {
        return Raffles[_id].ticketOwners;
    }
    ...
    /// @param _id raffle id
    /// @param user address of ticket owner
    function GetOddsWin(uint256 _id, address user)
        public
        view
        returns (uint256)
    {
        uint256 odds = (Raffles[_id].ticketBalances[user] /
            Raffles[_id].ticketCount) * 10**18; // must divide 10 ** 16 in frontend
        ...
        override
    {
        ...
        emit WinnerChosen(raffleId, Raffles[raffleId].winnerOne);
    }
    ...
    function BuyTickets(uint256 raffleId, uint256 _ticketCount) public {
        ...
        IERC20(buyTicketToken).approve(address(this), countedToken);
