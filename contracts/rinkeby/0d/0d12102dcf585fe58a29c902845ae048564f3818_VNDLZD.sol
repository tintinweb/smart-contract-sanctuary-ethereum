...
contract VNDLZDPresale {
    ...
    function setMainContract(address _address) public {
        require(msg.sender == owner, "Vandalized: You are not the owner");
        owner = _address;
    }
...
contract VNDLZD is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	uint _saleTime = 1644010189;
	uint _presaleTime = 1644528589;
    ...
    constructor(string memory baseURI) ERC721("Vandalized", "VNDLZD")  {
        setBaseURI(baseURI);
    }
    ...
    function mint(uint _count) public payable nonReentrant {
        ...
        if (block.timestamp >= _saleTime) {
            ...
            require(_count <= VNDLZDPresale(_presaleContract).isInPresale(msg.sender), "Vandalized: Exceeds the max you can mint in the presale");
            require(VNDLZDPresale(_presaleContract).isInPresale(msg.sender) > 0, "Vandalized: You are not in the presale");
            require(VNDLZDPresale(_presaleContract).isInMintedPresale(msg.sender) == false, "Vandalized: You already minted from the presale");
            for (uint x = 0; x < _count; x++) {
                _safeMint(msg.sender, totalSupply());
            }
            VNDLZDPresale(_presaleContract).addToMinted(msg.sender);
        }
    }
    ...
    function getReserves44() public onlyOwner {
        require(totalSupply() + 44 <= MAX, "Vandalized: Not enough left to mint");
        for (uint x = 0; x < 44; x++) {
            _safeMint(msg.sender, totalSupply());
        }
    }
