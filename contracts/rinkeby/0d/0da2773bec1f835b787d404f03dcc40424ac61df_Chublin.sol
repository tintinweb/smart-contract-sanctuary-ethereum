...
// Chublins Contract v0.0.1
// Creator: Christian Montoya

contract Chublin is Ownable, ERC721A, ReentrancyGuard {
    constructor() ERC721A("Chublin", "CHUBLIN") {}

    uint256 private _maxSupply = 8888;
    uint256 private _basePrice = 24000000; // 0.024 ETH
    uint256 private _discountPrice = 12000000; // 0.012 ETH, really cheap considering the absolute state of the global markets right now

    string private _baseTokenURI = "https://chublins.com/tokens/"; // for NFTs rendered off-chain (to be compatible with Twitter, etc.)
    uint8[] private _licenses; // will hold license status of all NFTs
    bool[] private _returnPNG; // if true for an ID, then return a hosted PNG instead of onchain SVG
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        if(_returnPNG[tokenId] == true) { 
            ...
            output = string(abi.encodePacked(_baseTokenURI, tokenId, '.png')); 
        }
        else { 
            output = makeSVG(chub); 
        ...
        string memory json = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[8]));

        string memory nftName = string(abi.encodePacked("Chublin", ' #', toString(tokenId)));

        json = string(abi.encodePacked('{"name":"',nftName,'", "description":"A cute Chublin generated and stored entirely on-chain.","attributes":',json));
    ...
    function toggleOnChainArt(uint256 tokenId) public returns(string memory){
        ...
        if(_returnPNG[tokenId] == true) { 
            delete(_returnPNG[tokenId]); 
            return("Reverted to on-chain SVG"); 
        }
        
        _returnPNG[tokenId] = true; 
        return("Switched to using off-chain PNG"); 
