...
contract SpumeRewardPool is Ownable, Pausable, AccessControl {
    ...
    */ 
    ...
    function rewardClaimAndSwap(uint256 _rewardTokenAmount) public whenNotPaused() returns (uint256) {
        ...
        bool success = IERC20(s_rewardsToken).transferFrom(msg.sender, address(this), _rewardTokenAmount); 
     ...
     */
    function unPauseRewardPool() private onlyOwner whenPaused() {
        _unpause(); 
    }
...
contract SpumeStaking is ReentrancyGuard, Ownable, Pausable { 
     ...
     */
    function depositRewardToken(uint256 amount) public onlyOwner whenNotPaused() nonReentrant returns(uint256) {
        emit Deposited(amount);
        s_rewardsToken.safeTransfer(address(this), amount); 
        return(amount);
    }
