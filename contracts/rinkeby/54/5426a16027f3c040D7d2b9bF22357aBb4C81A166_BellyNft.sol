  ...
  ReentrancyGuard
{
  ...
  function combine(uint256 recipeId)
    public
    payable
    virtual
    nonReentrant
  ...
  {
    (bool success, bytes memory returndata) = combineContract.delegatecall(
      abi.encodeWithSignature("combine(uint256)", recipeId)
    );
  ...
  function setCombineContract(address addr)
    public
    whenAddressIsContract(addr)
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    combineContract = addr;
  }
