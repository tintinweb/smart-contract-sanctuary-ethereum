...
contract BaseSynthetixBridge is
    Owned,
    MixinSystemSettings,
    IBaseSynthetixBridge
{
    ...
    function messenger() internal view returns (iAbs_BaseCrossDomainMessenger) {
        return
            iAbs_BaseCrossDomainMessenger(
                requireAndGetAddress(CONTRACT_EXT_MESSENGER)
            );
    }
    ...
    function onlyAllowFromCounterpart() internal view {
        ...
        require(
            _messenger.xDomainMessageSender() == counterpart(),
    ...
    function initiateSynthTransfer(
        ...
        require(
            getCrossChainSynthTransferEnabled(currencyKey) > 0,
        ...
        bytes memory messageData = abi.encodeWithSelector(
            this.finalizeSynthTransfer.selector,
            currencyKey,
            destination,
            amount
        );
        ...
        messenger().sendMessage(
            ...
            uint32(
                getCrossDomainMessageGasLimit(
                    CrossDomainMessageGasLimits.Withdrawal
                )
            )
        );
    ...
    function _incrementSynthsTransferCounter(
        ...
        bytes32 key = keccak256(
            abi.encodePacked(SYNTH_TRANSFER_NAMESPACE, group, currencyKey)
        );

        uint256 currentSynths = flexibleStorage().getUIntValue(
            CONTRACT_NAME(),
            key
        );
    ...
    function _sumTransferAmounts(bytes32 group)
        internal
        view
        returns (uint256 sum)
    {
        ...
        // get all values
        bytes32[] memory transferAmountKeys = new bytes32[](
            currencyKeys.length
        );
        for (uint256 i = 0; i < currencyKeys.length; i++) {
            transferAmountKeys[i] = keccak256(
                abi.encodePacked(
        ...
        uint256[] memory transferAmounts = flexibleStorage().getUIntValues(
            CONTRACT_NAME(),
            transferAmountKeys
        );
...
contract SynthetixBridgeToOptimism is
    BaseSynthetixBridge,
    ...
    iOVM_L1TokenGateway
{
    ...
    constructor(address _owner, address _resolver)
        public
        BaseSynthetixBridge(_owner, _resolver)
    {}
    ...
    function resolverAddressesRequired()
        ...
        returns (bytes32[] memory addresses)
    {
        bytes32[] memory existingAddresses = BaseSynthetixBridge
        .resolverAddressesRequired();
    ...
    function migrateEscrow(uint256[][] memory entryIDs)
        public
        requireInitiationActive
        requireZeroDebt
    {
        ...
        requireInitiationActive
    {
        ...
        messenger().sendMessage(
            ...
            uint32(
                getCrossDomainMessageGasLimit(
                    CrossDomainMessageGasLimits.CloseFeePeriod
                )
            )
        );
    ...
    function _depositReward(address _from, uint256 _amount) internal {
        ...
        ISynthetixBridgeToBase bridgeToBase;
        bytes memory messageData = abi.encodeWithSelector(
            bridgeToBase.finalizeRewardDeposit.selector,
            _from,
            _amount
        );
        ...
        messenger().sendMessage(
            ...
            uint32(
                getCrossDomainMessageGasLimit(
                    CrossDomainMessageGasLimits.Reward
                )
            )
        );
    ...
    function _initiateDeposit(address _to, uint256 _depositAmount) private {
        ...
        bytes memory messageData = abi.encodeWithSelector(
            bridgeToBase.finalizeDeposit.selector,
            _to,
            _depositAmount
        );
        ...
        messenger().sendMessage(
            ...
            uint32(
                getCrossDomainMessageGasLimit(
                    CrossDomainMessageGasLimits.Deposit
                )
            )
        );

        emit iOVM_L1TokenGateway.DepositInitiated(
            msg.sender,
            _to,
            _depositAmount
        );
    ...
    function _migrateEscrow(uint256[][] memory _entryIDs) private {
        ...
        for (uint256 i = 0; i < _entryIDs.length; i++) {
            ...
            require(
                _entryIDs[i].length <= MAX_ENTRIES_MIGRATED_PER_MESSAGE,
            ...
            (escrowedAccountBalance, vestingEntries) = rewardEscrowV2()
            .burnForMigration(msg.sender, _entryIDs[i]);
            ...
            if (escrowedAccountBalance > 0) {
                ...
                messenger().sendMessage(
                    ...
                    uint32(
                        getCrossDomainMessageGasLimit(
                            CrossDomainMessageGasLimits.Escrow
                        )
                    )
                );

                emit ExportedVestingEntries(
                    msg.sender,
                    escrowedAccountBalance,
                    vestingEntries
                );
