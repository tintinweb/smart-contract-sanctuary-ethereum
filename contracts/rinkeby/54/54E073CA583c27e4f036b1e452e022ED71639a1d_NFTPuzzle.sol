...
contract NFTPuzzle is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mintForAll(uint32 _amount) external payable nonReentrant {
...
*/
  ...
  function addressInfo(address _address) external view _onlyWhiteListContracts returns(uint256 _amount /* Address total tokens amount*/, uint256[] memory _tokenIds /*All address token ids sotred in array */){
    _amount = balanceOf(_address);
    for(uint i = 0; i < _amount; i++){
      _tokenIds[i] = (tokenOfOwnerByIndex(_address, i));
    }
