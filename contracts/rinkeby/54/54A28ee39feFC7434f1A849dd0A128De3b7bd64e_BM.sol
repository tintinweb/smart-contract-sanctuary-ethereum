...
contract BM is ERC721, IBM, Validator, Killswitch {
    ...
    function _levelUp(address owner, uint256 tokenId) internal honey {
        ...
        if (_tokenRank[tokenId] > _godfatherRank) {
            ...
            if (prevId != _godfatherId) {
                // solhint-disable-next-line not-rely-on-time
                emit NewGodFather(owner, tokenId, block.timestamp);
            }
        }
    }
    ...
    function mint(uint256 amount) external override payable killswitch {
...
interface IBM is IERC721, IERC721Metadata, IValidator, IKillswitch {
    event NewGodFather(address owner, uint256 tokenId, uint256 timestamp);
