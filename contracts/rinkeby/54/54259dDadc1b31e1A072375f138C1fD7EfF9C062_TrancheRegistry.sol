...
contract TrancheRegistry is ITrancheRegistry, Ownable, Operators {
    ...
    mapping(uint256 => address[]) private _trancheList; 
        
    // tranche address -> was created from this registry
    mapping(address => bool) private _allTranches;
    ...
    /// @notice Get the total number of tranche implementations this registry created
    function implDetailsLength() external view returns (uint256) {
        return _implDetails.length;
    }

    /// @notice Disable a particular tranche implemenation ID template from future use.
    ...
    /// @notice Disable a particular tranche instance which was created by this registry.
