...
contract Voucher is AccessControl, EIP712, IERC2981, Ownable {
  ...
  function buyVoucherWhitelist(WhiteListVoucherInfo calldata voucherInfo)
    external
    payable
  {
    require(_msgSender() == voucherInfo.wallet, "Voucher: Invalid wallet");
    address signer = verifyBuyVoucherWhitelistInfo(voucherInfo);
    require(
      hasRole(GENERATE_VOUCHER_ROLE, signer),
      "Voucher: Signature invalid or unauthorized"
    );
    _buyVoucher(voucherInfo.price, voucherInfo.data);
  }

  function buyVoucherPublicSale(PublicSaleVoucherInfo calldata voucherInfo)
    external
    payable
  {
    address signer = verifyBuyVoucherPublicSaleInfo(voucherInfo);
    require(
      hasRole(GENERATE_VOUCHER_ROLE, signer),
      "Voucher: Signature invalid or unauthorized"
    );
  ...
  function _buyVoucher(uint256 price, string memory data) internal {
    uint256 currentVoucherCounter = _voucherCounter.current();
    uint256 maxSupply = ISupply(targetContractAddress).getMaxSupply();

    require(msg.value == price, "Voucher: Invalid price amount");
    require(
      currentVoucherCounter <= maxSupply,
      "Voucher: No more vouchers available"
    );
    _voucherCounter.increment();
    paymentSplitter.receiveFromPrimarySale{value: msg.value}();
    emit VoucherSold(_msgSender(), targetContractAddress, data);
  }

  function verifyBuyVoucherPublicSaleInfo(
    PublicSaleVoucherInfo calldata voucher
  ) internal view returns (address) {
    bytes32 digest = hashPublicSaleVoucherInfo(voucher);
    return ECDSA.recover(digest, voucher.signature);
  }

  function hashPublicSaleVoucherInfo(PublicSaleVoucherInfo calldata voucherInfo)
    internal
    view
    returns (bytes32)
  {
    bytes memory info = abi.encodePacked(voucherInfo.price, voucherInfo.data);
  ...
  function verifyBuyVoucherWhitelistInfo(WhiteListVoucherInfo calldata voucher)
    internal
    view
    returns (address)
  {
    bytes32 digest = hashWhitelistVoucherInfo(voucher);
    return ECDSA.recover(digest, voucher.signature);
  }

  function hashWhitelistVoucherInfo(WhiteListVoucherInfo calldata voucherInfo)
    internal
    view
    returns (bytes32)
  {
