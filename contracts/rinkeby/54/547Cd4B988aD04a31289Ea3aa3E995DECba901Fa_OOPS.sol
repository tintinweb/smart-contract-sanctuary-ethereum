...
contract OOPS is Ownable, EIP712, ERC721A, ERC721AQueryable {
	...
	// Sales variables
	// ------------------------------------------------------------------------
	uint256 public MAX_OOPS = 8888;
	uint256 public STAGE_LIMIT = 2888;
	...
	string private _baseTokenURI = "ipfs://QmWbc4j8TcbvdTX4Tj4zHYsAFTDkYYM9GnH7r6EMUXTe5C/";
	...
	mapping (address => uint256) public hasWhitelistMinted;
	...
	function giveaway(address _to, uint256 quantity) external onlyOwner{
		require(totalSupply().add(quantity) <= MAX_OOPS, "EXCEEDS_MAX_OOPS");
	...
	function claim(uint256 quantity, uint256 maxClaimNum, bytes memory SIGNATURE) external callerIsUser{
		...
		require(totalSupply().add(quantity) <= STAGE_LIMIT, "STAGE_SOLD_OUT");
		require(totalSupply().add(quantity) <= MAX_OOPS, "EXCEEDS_MAX_OOPS");
	...
	// Whitelist functions
	// ------------------------------------------------------------------------
	function mintWhitelist(uint256 quantity, uint256 maxQuantity, bytes memory SIGNATURE) external payable callerIsUser{
		require(hasWhitelistStarted == true, "WHITELIST_NOT_ACTIVE");
        require(block.timestamp >= saleTimestamp, "NOT_IN_WHITELIST_TIME");
		require(totalSupply().add(quantity) <= STAGE_LIMIT, "STAGE_SOLD_OUT");
		require(verify(maxQuantity, SIGNATURE), "NOT_ELIGIBLE_FOR_WHITELIST");
		require(quantity > 0 && hasWhitelistMinted[msg.sender].add(quantity) <= maxQuantity, "EXCEEDS_WHITELIST_QUANTITY");
		require(totalSupply().add(quantity) <= MAX_OOPS, "EXCEEDS_MAX_OOPS");
		require(msg.value >= PRICE.mul(quantity), "ETHER_NOT_EUQAL_THE_PRICE");

		hasWhitelistMinted[msg.sender] = hasWhitelistMinted[msg.sender].add(quantity);
	...
	// ------------------------------------------------------------------------
	function mintOOPS(uint256 quantity) external payable callerIsUser{
		require(hasSaleStarted == true, "SALE_NOT_ACTIVE");
		require(msg.value >= PRICE.mul(quantity), "ETHER_NOT_EUQAL_THE_PRICE");
		require(block.timestamp >= saleTimestamp, "NOT_IN_SALE_TIME");
		require(totalSupply().add(quantity) <= STAGE_LIMIT, "STAGE_SOLD_OUT");
		require(totalSupply().add(quantity) <= MAX_OOPS, "EXCEEDS_MAX_OOPS");
		require(quantity > 0 && hasPublicMinted[msg.sender].add(quantity) <= MAX_ADDRESS_TOKEN, "EXCEEDS_MAX_ADDRESS_TOKEN");
		
		hasPublicMinted[msg.sender] = hasPublicMinted[msg.sender].add(quantity);
	...
	function setMAX_OOPS(uint256 _MAX_num) external onlyOwner {
		MAX_OOPS = _MAX_num;
	}
