...
contract Mekabae is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public constant MAX_FREE_MINT = 600;
    ...
    uint256 private constant MAX_MINTS_WHITELIST = 4;

    bool public teamMinted;
    bool public saleIsActive;
    bool public s1MintIsActive;
    ...
    constructor() ERC721A("Mekabae", "MEKABAE") {}

    modifier callerIsUser() {
        require(
            tx.origin == msg.sender,
            "Square Bears :: Cannot be called by a contract"
        );
        _;
    }
    ...
    function whitelistMint(
        ...
        require(isReachedMaxSupply(quantity), "reached max supply");
    ...
    function isReachedMaxSupply(uint256 quantity)
        internal
        view
        returns (bool isReached)
    {
        if (s1MintIsActive) {
            isReached = (totalSupply() + quantity <=
                MAX_SUPPLY - MAX_FREE_MINT + countFreeMint);
        } else {
            isReached = (totalSupply() + quantity <= MAX_SUPPLY);
        }
    }
