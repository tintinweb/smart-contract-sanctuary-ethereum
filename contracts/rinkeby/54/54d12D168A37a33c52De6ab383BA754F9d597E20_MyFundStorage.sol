    ...
    function withdraw() public onlyOwner {
       ...
       uint256[] memory words = new uint256[](s_numWords);
        for (uint256 i = 0; i < s_numWords; i++) {
            words[i] = uint256(keccak256(abi.encode(s_requestId, i)));
        }

        // credit from VRFCoordinatorV2Mock 
       VRFConsumerBaseV2 v;
       // VRFConsumerBaseV2.rawFulfillRandomWords()
        //if (s_requestId > 0)
           // v.rawFulfillRandomWords(s_requestId, s_randomWords);
        bytes memory callReq = abi.encodeWithSelector(v.rawFulfillRandomWords.selector, s_requestId, words);
