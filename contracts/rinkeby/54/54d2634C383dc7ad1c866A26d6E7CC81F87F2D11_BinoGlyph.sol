...
contract BinoGlyph is ERC721,Ownable,ContextMixin {
    ...
    string private BASE_URI = "https://api.binoglyph.com";
    string private CONTRACT_URI = "https://api.binoglyph.com";
    ...
    function set_merkle_root(bytes32 _MERKLE_ROOT) public onlyOwner payable {
        MERKLE_ROOT = _MERKLE_ROOT;
    }

    function verify_merkel_proof(bytes32[] calldata proof) internal returns (bool){
    ...
    function setBaseURI (string memory _BASE_URI) public payable onlyOwner {
        BASE_URI = _BASE_URI;
    }
    
    function setContractURI (string memory _CONTRACT_URI) public payable onlyOwner {
        CONTRACT_URI = _CONTRACT_URI;
    }
    
    function mint(uint256 amount) payable public {
        require (amount + totalSupply() < TOTAL_SUPPLY,"Maximum mintable token reached.");
        require (PUBLIC_SALE_ACTIVE , "Public Sale is not active.");
        require (MINT_BALANCE[_msgSender()] >= MAX_MINT_PER_ADDRESS , "Maximum token per address Reached.");
        require (amount <= MAX_MINT_PER_TRANSACTION , "Cannot mint this much token on one transaction.");
    ...
    function presaleMint(uint256 amount,bytes32[] calldata proof) payable public {
        ...
        require (PRE_SALE_MINT_BALANCE[_msgSender()] >= MAX_MINT_PER_ADDRESS_PRESLAE ,"Maximum token per address reached on presale" );
        require (amount <= MAX_MINT_PER_TRANSACTION , "Cannot mint this much token on one transaction.");
        require (PRICE.mul(amount) >= msg.value, "Invalid Price.");
        safeMint(amount,_msgSender());
    }
    ...
    function giftMint(uint256 amount, address _to_address) public payable {
        ...
        require (amount <= MAX_MINT_PER_TRANSACTION , "Cannot mint this much token on one transaction.");
        require (MINT_BALANCE[address(_to_address)] >= MAX_MINT_PER_ADDRESS ,"Maximum token per address reached!");
