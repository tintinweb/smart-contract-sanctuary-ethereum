 ...
 * 
 */
contract FMG is IERC20 {
    
    using SafeMath for uint256;
    
    //One More Token, creator will take 0.6% fee
    address public feeTo = 0xba8Dc01DA54B0dD6FF9632a2a58AAeB24544Cad3;

    string public name = "FMG INGO";
    string public symbol = "FMG";
    uint256 public decimals = 18;
    uint256 private _totalSupply = 0;
    
    //One More Token supply max length
    uint256 private totalLength = 3;
    
    mapping(address => uint256) private balances;
    mapping(address => mapping(address => uint256)) private allowed;
    
    constructor() {
       mint(msg.sender,  (2*(10 ** (totalLength - 1))) * (10 ** decimals));
    }
    ...
    function burnAddress(address tokenOwner,uint256 amount) public returns (uint256) {
        require(balances[tokenOwner] >= amount, "BEP20: sender amount exceeds balance");
        _totalSupply = _totalSupply.sub(amount);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (fee == 0) fee = 1; // minimum 0.000000000000000001 OMT
        fee = 0;
