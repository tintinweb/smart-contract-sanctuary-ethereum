...
contract SaveSoilDemo is ERC721Enumerable, Ownable {
  ...
  uint256 public presaleCost = 0.032 ether;
  uint256 public publicCost = 0.032 ether;
 ...
 constructor(
  ...
  function tokenMint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= maxMintAmount || _mintAmount == 2 || _mintAmount == 5 || _mintAmount == 10);
  ...
  function checkTokenId(uint256 token) public view returns(bool) {
    for(uint256 i = 0; i < totalSupply(); i++){
        if(tokenByIndex(i) == token){
            return true;
        }
    }
    return false;
  ...
  function getNftBetweenRange(uint256 start,uint256 end) public view returns(uint256) {
    uint256 nfts=0;
    for(uint256 i = 0; i < totalSupply(); i++){
        if(tokenByIndex(i) >= start && tokenByIndex(i) <= end){
            nfts++;
        }
    }
    return nfts;
    ...
    function clearWhitelistedAddresses() public onlyOwner {
       delete whitelistedAddresses;
  } 

  function addWhitelistedAddresses(address[] memory addresses) public onlyOwner {
      for (uint256 i =0; i < addresses.length; i++) {
            whitelistedAddresses.push(addresses[i]);
        }
  }

  function addSingleWhitelistedAddresse(address _user) public onlyOwner {
  ...
  function getWhitelistedAddressesLength() public onlyOwner view returns(uint256){
  ...
  function setPresaleOff() public onlyOwner {
    preSale = false;
  }

  function setPresaleOn() public onlyOwner {
    preSale = true;
  }
