...
contract MocGenesis is ERC721AQueryable, Ownable, MerkleProofWrapper {
    ...
    uint256 public maxAddrFreeSupply = 1;
    ...
    function getFreeMintCount(address addr) public view returns (uint256) {
        return addrFreeMintCount[addr];
    }
    ...
    function setFreeMint(bool _canFreeMint) public onlyOwner {
        canFreeMint = _canFreeMint;
    }

    function freeMint(uint256 _amount,bytes32[] memory proof) public {
        require(canFreeMint, "free mint is paused");
        require(_amount>=1, "mint amount error");
        require(_amount<=maxAddrFreeSupply, "mint amount exceed");
        ...
        require(( _freeMintCount + _amount) <= maxFreeSupply, "not enough tokens for free buyer");
        require(addrFreeMintCount[msg.sender] + _amount <= maxAddrFreeSupply, "exceed max personal supply");
        ...
        addrFreeMintCount[msg.sender] += _amount;
