...
contract BookNFT721 is ERC721URIStorage, EIP712, Ownable, AccessControl, Voucher, ERC2981PerTokenRoyalties {
    ...
    constructor(string memory name, string memory symbol, address payable _feeRecipient, uint16 _platformFee) ERC721(name, symbol) {
    ...
    function _sendFunds(address beneficiary, uint256 value) internal returns (bool) {
        uint256 feeAmount = (value * platformFee) / 1000;
...
contract Voucher is EIP712 {
    ...
    function _verify(NFTVoucher calldata voucher) internal pure returns (address) {
        // bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
        //     keccak256("NFTVoucher(uint256 tokenId,uint256 sellingPrice,uint16 royaltyBasisPoints,string tokenUri)"),
        //     voucher.tokenId,
        //     voucher.sellingPrice,
        //     voucher.royaltyBasisPoints,
