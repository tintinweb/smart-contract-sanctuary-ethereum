...
contract Pepeth is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(publicClaimed[msg.sender] <= 2, "Address already minted max allowed");
        require(_mintAmount > 0 && _mintAmount <= 2, 'Invalid mint amount!');
        require(totalSupply() + _mintAmount <= MAX_TOKENS - (50 - reservedTokensMinted), "Purchase would exceed max supply");
        require(msg.value >= PRICE * _mintAmount, "Insufficient funds!");

        publicClaimed[msg.sender] = publicClaimed[msg.sender] + _mintAmount;
        _safeMint(msg.sender, totalSupply() + _mintAmount);
    }
