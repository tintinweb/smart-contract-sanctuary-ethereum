...
interface IMeskullz {
    function balanceMZ(address _user) external view returns (uint256);
}
...
contract MeskullzToken is ERC20, Ownable {
    //Start 1645833600 - Sat, Feb 26, 2022 12:00:00 AM
    //End   1803513600 - Thu, Feb 25, 2027 12:00:00 AM, 5 years, 157680000
    uint256 public constant END = 1803513600;
    ...
    IMeskullz public nouns3dContract;
    ...
    constructor(address _nouns3d) ERC20("NOUN", "NOUN") {
        nouns3dContract = IMeskullz(_nouns3d);
    ...
    // updated_amount = (balanceMZ(user) * base_rate * delta / 86400) + amount * initial rate
    function updateRewardOnMint(address _user) external {
        ...
        if (timerUser > 0) {
            rewards[_user] =
                rewards[_user] +
                (nouns3dContract.balanceMZ(_user) *
                    BASE_RATE *
                    ((time - timerUser) / 86400));
    ...
    function updateReward(
        ...
        if (_tokenId < 7400) {
            ...
            if (timerFrom > 0) {
                rewards[_from] +=
                    nouns3dContract.balanceMZ(_from) *
                    BASE_RATE *
                    ((time - timerFrom) / 86400);
    ...
    function getTotalClaimable(address _user) external view returns (uint256) {
        uint256 time = min(block.timestamp, END);
        uint256 pending = nouns3dContract.balanceMZ(_user) *
            BASE_RATE *
            ((time - lastUpdate[_user]) / 86400);
