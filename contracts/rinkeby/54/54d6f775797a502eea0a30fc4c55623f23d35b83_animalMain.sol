...
contract animalMain is  ERC721,Ownable {
  ...
  struct animal {
    bytes32 dna;    //DNA
    uint8 star;     //Level of each animal
    uint16 roletype; // 1 rabbit  2 dog  3 tiger  4 dolphine
  }
  ...
  function balanceOf(address _owner) public view returns (uint256 _balance) {
    return ownerAnimalCount[_owner]; // 此方法只是顯示某帳號 餘額(number of animal)
  }
  ...
  function transfer(address _to, uint256 _tokenId) public {
    ...
    ownerAnimalCount[_to].add(1);                         //增加受贈者的擁有動物數量
    ownerAnimalCount[msg.sender].sub(1);                  //減少轉出者的擁有動物數量
  ...
  function transferFrom(address _from, address _to, uint256 _tokenId) external {
    ...
    //?
    // require(_to == animalApprovals[_tokenId]);
  ...
  function takeOwnership(uint256 _tokenId) public {   //what does this for?
    require(animalToOwner[_tokenId] == msg.sender);
