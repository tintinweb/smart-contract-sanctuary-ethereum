...
contract SpinTheWheel {
    ...
    // Wheel info
    uint256 public _maxSpinBoolPercentage; // (5%)
    uint256 public _qualifiedSpinBoolPercentage; //1 (1%)
    ...
    function spin(uint256 amount) external notPaused {
        ...
        //If amount > maxSpinAmount, then only take maxSpinAmount
        ...
        Reward memory reward = _rewards[getRandomReward()];
    ...
    function getMaxSpinAmount() public view returns (uint256){
        return IERC20(_spinInuAddress).balanceOf(address(this)) / 100 * _qualifiedSpinBoolPercentage;
    }

    //// internal
    //// private
    //TODO: Use chainlink
    function getRandomReward() private returns (uint256) {
        ...
        uint256 rnd = seed - ((seed / _totalWeight) * _totalWeight) + 1;
