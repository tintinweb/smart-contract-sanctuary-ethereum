...
contract NoRugs is ERC721A, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 _mintAmount)
        ...
        mintFreeMintCompliance(_mintAmount)
    {
   ...
   modifier freeMintCompliance(uint256 _mintAmount) {
        ...
        require(_mintAmount > 0 , "Invalid free mint amount, the amount must be 1 or more!");
        require(_mintAmount > 0 && _mintAmount <= maxFreeMintAmountPerTx, "Invalid free mint amount, the amount will execeed max per transaction!");
        require( totalSupply() + _mintAmount <= maxFreeMintSupply, "Amount will exceed max free mint supply");
        require( totalSupply() + _mintAmount <= maxSupply, "Amount will exceed max mint supply!");              
        _;
    }

    modifier mintFreeMintCompliance(uint256 _mintAmount) {
        require(hasFreeMint == false || hasFreeMint == true && totalSupply() + _mintAmount == maxFreeMintSupply, "Free mint is available. The freee mint supply must be minted first!");             
        _;
    }
}
