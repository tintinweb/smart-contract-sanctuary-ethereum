...
contract CloneMachine is ERC721A, Ownable, Pausable, ReentrancyGuard {
	...
	string private _contractBaseURI = "ipfs://xxxxxxxxxx/";
	string private _contractURI = "ipfs://yyyyyy";

	event JuiceBurned(uint256 juiceId, uint256 speciesType, uint256 speciesId);
	event CloneUpgraded(uint256 tokenID);
	...
	modifier burnValid(
		...
		require(hashTransaction(_msgSender(), speciesID, speciesType) == hash, "invalid hash");
		require(IJuice(juiceContract).ownerOf(juiceID) == _msgSender(), "not the owner");
		_;
	}
	...
	function cloneWithCat(
		...
		IJuice(juiceContract).burn(juiceId);

		_safeMint(_msgSender(), 1);

		emit JuiceBurned(juiceId, CAT, speciesId);
	}
	...
	function cloneWithRat(
		...
		emit JuiceBurned(juiceId, RAT, speciesId);
	}
	...
	function cloneWithPigeon(
		...
		require(IJuice(juiceContract).ownerOf(juiceId) == _msgSender(), "not the juice owner");
		...
		emit JuiceBurned(juiceId, PIGEON, speciesId);
	}
	...
	function cloneWithDog(
		...
		require(IJuice(juiceContract).ownerOf(juiceId) == _msgSender(), "not the juice owner");
		require(IERC721(gutterDogs).ownerOf(speciesId) == _msgSender(), "not the pigeon owner");
		...
		emit JuiceBurned(juiceId, DOG, speciesId);
	}
	...
	function cloneWithoutSpecies(
		...
		emit JuiceBurned(juiceId, NONE, 0);
	}
	...
	function upgradeClone(
		...
		require(matchAddresSigner(hash, sig), "invalid signer");
		require(hashTransaction(_msgSender(), juiceId, tokenID) == hash, "invalid hash");
		require(ownerOf(tokenID) == _msgSender(), "not the owner");
		require(IJuice(juiceContract).ownerOf(tokenID) == _msgSender(), "not juice owner");
		IJuice(juiceContract).burn(juiceId);

		emit CloneUpgraded(tokenID);
	}
