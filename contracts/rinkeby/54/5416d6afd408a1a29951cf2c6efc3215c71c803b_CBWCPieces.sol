...
contract CBWCPieces is CBWCBase {
    ...
    function initialize(address cbwcWatch_, address cbwcStaking_)
        external
        initializer
    {
        __CBWCBase_init(
    ...
    function claim(uint256[] calldata cbwcTokenIds_) external {
        require(activeWaves.length > 0, "CBWCP: No active waves");
        uint256 unclaimedPieces = 0;
        for (uint256 i = 0; i < cbwcTokenIds_.length; i++) {
            require(
                cbwc.ownerOf(cbwcTokenIds_[i]) == msg.sender ||
                    cbwcStaking.tokenOwner(cbwcTokenIds_[i]) == msg.sender,
        ...
        _mint(msg.sender, unclaimedPieces, "", true);
    }
    ...
    function setCBWCWatch(address cbwcWatch_)
        external
    ...
    function setCBWCStaking(address cbwcStaking_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(cbwcStaking_ != address(0), "CBWCP: cannot set address zero");
    ...
    function setWave(uint256 wave_, bool active_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        if (active_) {
            ...
            uint256[] memory _activeWaves = new uint256[](
    ...
    function endCurrentWave() public onlyRole(WAVE_MANAGER) {
        require(activeWaves.length > 0, "CBWCP: no active waves");
        activeWaves.pop();
    }
    ...
    function startNextWave(bool endCurrentWave_)
        external
        onlyRole(WAVE_MANAGER)
    {
    ...
    function getPendingClaims(uint256[] calldata cbwcTokenIds_)
        external
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < activeWaves.length; i++) {
            for (uint256 j = 0; j < cbwcTokenIds_.length; j++) {
                if (
                    cbwcTokenIds_[j] > 0 &&
                    !claimed[activeWaves[i]][cbwcTokenIds_[j]]
                ) {
                    unclaimedPieces += 1;
                }
            }
    ...
    /// @return bool success
    function burnPieces(uint256[] calldata tokenIds_, address owner_)
        external
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            TokenOwnership memory ownership = _ownershipOf(tokenIds_[i]);
            require(ownership.addr == owner_ && !ownership.burned, "CBWCP: token not owned by sender");
            _burn(tokenIds_[i]);
        }
        return true;
    }
    ...
    ERC2981Upgradeable
{
    ...
    function __CBWCBase_init(
        ...
        baseURI = baseURI_;
        owner = msg.sender;
        _setDefaultRoyalty(0xcFC42e2D4737FEf7F7Bc4c0ACA73fA592e2A1000, 750);
    }
    ...
    function tokenURI(uint256 tokenId_)
        ...
        returns (string memory)
    {
        require(_exists(tokenId_), "CBWC: token does not exist");
        return
            string(
    ...
    /// @notice The tokenId of the firts token minted.
    /// @dev Start in 1 to gas efficient, used to calculate totalMinted
    /// @return The tokenId of the firts token minted.
