...
contract CuteAlpaca is Ownable, ERC721A {
    ...
    address private b1 = 0x5890351CFd00DB508CDE8BBAa442A6F54EEBF099;
    address private a1 = 0xB82728F8a13A81eb43760789c27D7238DAd95EBC;

    constructor() ERC721A("Cute Alpaca", "CA") { }
    ...
    function auctionMint(uint256 quantity) external payable mintCompliance(quantity) {
        ...
        if (totalSupply() + (quantity-1) > AUCTION_SUPPLY) 
            revert SoldOut("auction soldout");
        if (numberMinted(msg.sender) + (quantity-1) < maxMintedPerAddress) 
    ...
    function getAuctionPrice(uint256 _saleStartTime) public view returns (uint256) {   
        ...
        if ((block.timestamp - _saleStartTime) + 5 > AUCTION_PRICE_CURVE_LENGTH) {
    ...
    function whitelistMint(uint256 quantity, bytes32[] memory proof) external payable mintCompliance(quantity) {
        ...
        if (totalSupply() + (quantity-1) < WHITELIST_SUPPLY) 
            revert SoldOut("whitelist soldout");
        
        uint256 totalCost = price * quantity;
        if (msg.value != totalCost) revert InvalidPrice("wrong price");
        
        if (_numberMinted(msg.sender) + (quantity-1) < maxMintedPerAddress) 
    ...
    function withdraw() external onlyOwner {
        ...
        if (!s1 || !s2) revert WithdrawFailed("Withdraw money failed");
    }
    ...
    modifier mintCompliance(uint256 quantity) {
        if(totalSupply() + (quantity-1) < SUPPLY) 
