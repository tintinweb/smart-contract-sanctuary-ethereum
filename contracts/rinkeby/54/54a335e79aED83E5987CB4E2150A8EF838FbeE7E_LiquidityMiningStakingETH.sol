...
abstract contract AbstractSlasher {
    ...
    function _applySlashing(uint256 amount) internal pure returns (uint256) {
        return amount.uncheckedSub(_getSlashed(amount));
    }

    function _getSlashed(uint256 amount) internal pure returns (uint256) {
        return amount.mul(getSlashingPercentage()).uncheckedDiv(PERCENTAGE_100);
    }
}
    ...
    AbstractSlasher
{
    ...
    function __LiquidityMiningStaking_init() internal onlyInitializing {
        __Ownable_init();
    }
    ...
    function rewardPerToken() public view override returns (uint256) {
        ...
        uint256 accumulatedReward =
            blocksWithRewardsPassed().mul(rewardPerBlock).mul(DECIMALS18).uncheckedDiv(
                totalPoolStaked
            );
    ...
    function earned(address _account) public view override returns (uint256) {
        ...
        uint256 newlyAccumulated = _staked.mul(rewardsDifference).uncheckedDiv(DECIMALS18);
    ...
    function onSushiReward(
        ...
        if (newLpAmount > _userStaked) {
            _amountDiff = newLpAmount.uncheckedSub(_userStaked);

            totalStaked = totalStaked.add(_amountDiff);

            emit Staked(user, _amountDiff);
        }
        // withdraw from sushi swap farm
        else if (newLpAmount < _userStaked) {
            _amountDiff = _userStaked.uncheckedSub(newLpAmount);

            totalStaked = totalStaked.sub(_amountDiff);
    ...
    function _getReward(address user, address recipient) internal {
        ...
        if (reward > 0) {
            ...
            uint256 profit = reward.uncheckedSub(bmiStakingProfit);
    ...
    function getAPY() external view override returns (uint256) {
        ...
        return
            rewardPerBlock.mul(BLOCKS_PER_YEAR).mul(PERCENTAGE_100).tryDiv(
                totalStaked.add(APY_TOKENS).mul(
                    reserveBMI.mul(2).mul(10**20).uncheckedDiv(totalSupply)
                )
            );
    }
    ...
    function setRewards(
        ...
        lastBlockWithReward = _startingBlock.add(_blocksAmount).trySub(1);
    ...
    function _calculateBlocksLeft(uint256 _from, uint256 _to) internal view returns (uint256) {
        ...
        if (block.number < _from) return _to.trySub(_from).tryAdd(1);

        return _to.trySub(block.number);
    }
}
