...
contract OnchainNft is ERC721 {
    ...
    constructor(IERC20 _exp) ERC721("Ethernaut Experience Point", "EthernautEXP") {
        exp = _exp;
        owner = msg.sender;
    }
...
'<text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#F6CE94">',numberToDisplay(_tokenId),'</text>',
  '<style> text{font-size: 40pt; font-weight: 800;}</style></svg>'
    ...
    function numberToDisplay(uint256 _tokenId) private view returns (uint256) {
        uint256 balance = balanceOf(ownerOf(_tokenId));
        if(balance == 0){
            return 0;
        }
   ...
   // function updateMetaData(uint _tokenId) {
     //   uint balance = exp.balanceOf(msg.sender);
    //}
...
library HelperLibrary {
    ...
    /**
     * @dev converts a string to a uint8
     */
