...
contract GameController is Ownable {
    ...
    function TransferERC721(
        ...
        require(sender == msg.sender && IERC721(ERC721).ownerOf(tokenId) == msg.sender ||
                IERC721(ERC721).isApprovedForAll(sender, address(this)) && msg.sender == owner(), "You're not an sender");
        
        if (sender == msg.sender && IERC721(ERC721).ownerOf(tokenId) == msg.sender) {
            (bool success, bytes memory data) = ERC721.delegatecall(
                abi.encodeWithSignature("safeTransferFrom(address,address,uint256)", sender, recipient, tokenId)
            );
            require(success, "Transfer error");
        }
        
        if (IERC721(ERC721).isApprovedForAll(sender, address(this)) && msg.sender == owner()) {
    ...
    function TransferERC721Batch(
        ...
        require(msg.sender == sender, "You're not an sender");
        require(
    ...
    function TransferERC20(
        ...
        require(sender == msg.sender && IERC721(ERC20).balanceOf(sender) >= amount ||
                IERC20(ERC20).allowance(sender, address(this)) >= amount && msg.sender == owner(), "You're not an sender");

        if (sender == msg.sender && IERC721(ERC20).balanceOf(sender) >= amount) {
            (bool success, bytes memory data) = ERC20.delegatecall(
                abi.encodeWithSignature("transferFrom(address,address,uint256)", sender, recipient, amount)
            );
            require(success, "Transfer error");
        }

        if (IERC20(ERC20).allowance(sender, address(this)) >= amount && msg.sender == owner()) {
    ...
    function GetERC721BalanceBysender(
        address[] calldata ERC721,
        address sender
        ...
        for (uint256 i = 0; i < ERC721.length; i++) {
            result[i] = IERC721(ERC721[i]).balanceOf(sender);
        }

        return result;
    }

    function GetERC721TokenBysender(
        address ERC721,
        uint256 startIndex,
        uint256 finishIndex,
        address sender
        ...
        for (uint256 i = startIndex; i < finishIndex; i++) {
            result[i] = IERC721Enumerable(ERC721).tokenOfOwnerByIndex(sender, i);
        }

        return result;
    }

    function GetERC20AllowedBysender(
        address[] calldata ERC20, 
        address sender
    ) view public returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < ERC20.length; i++) {
            result[i] = IERC20(ERC20[i]).allowance(sender, address(this));
        }
    ...
    function GetERC20BalanceByOwner(
        ...
        for (uint256 i = 0; i < ERC20.length; i++) {
            result[i] = IERC20(ERC20[i]).balanceOf(sender);
        }
