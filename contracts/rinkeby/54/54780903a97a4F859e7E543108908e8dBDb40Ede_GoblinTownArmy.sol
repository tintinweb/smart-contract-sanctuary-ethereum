...
contract GoblinTownArmy is ERC721Optimized, Ownable, ReentrancyGuard {
    ...
    uint256 public GTAPrice = 0.069 ether;
    string public _baseGTAURI;
    
    event GTAMinted(address indexed mintAddress, uint256 indexed tokenId);
    event PermanentURI(string _value, uint256 indexed _id);

    constructor(string memory baseURI) ERC721Optimized("GoblinTownArmy", "GTA") {
        _baseGTAURI = baseURI;
    }
    ...
    function mintGTA (uint256 numberOfTokens) public payable nonReentrant {
        require((GTAPrice * numberOfTokens) <= msg.value, "Ether value sent is not correct");
        require(numberOfTokens <= maxGTAPurchase, "You can mint max 10 GTA s per transaction");
        require((totalSupply() + numberOfTokens) <= MAX_GTA , "Purchase would exceed max supply of GTAs");
