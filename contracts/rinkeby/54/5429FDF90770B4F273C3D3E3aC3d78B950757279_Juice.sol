...
contract Juice is WizardsERC1155 {
        ...
        whenNotPaused
    {
        ...
        if (quantity == 1) {
            _mint(_msgSender(), _weightedSelection(), quantity, "");
        } else {
            uint256[] memory selected = _expandedWeightedSelection(quantity);
            for (uint256 i = 0; i < selected.length; i++) {
                _mint(_msgSender(), i, quantity, "");
            }
        }
    }

    function randomMint(uint256 quantity) public isMinter whenNotPaused {
        if (quantity == 1) {
            _mint(_msgSender(), _weightedSelection(), quantity, "");
        } else {
            uint256[] memory selected = _expandedWeightedSelection(quantity);
    ...
    function burn(
        ...
        if (allowMerge) _merge(account, id, value);
    }

    function _payment(uint256 quantity, uint256 paymentToken) internal {
    ...
    function _weightedSelection() private view returns (uint256) {
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < _juiceInventory.length; i++) {
        ...
        returns (uint256[] memory expandedValues)
    {
        ...
        unchecked {
            ...
            for (uint256 i = 0; i < n; i++) {
                uint256 r = _determanisticRandom(i, maxWeight);
                for (uint256 j = 0; j < _juiceInventory.length; j++) {
                    if (cumulativeWeights[j] >= r) {
                        expandedValues[j]++;
                    }
                }
            }
    ...
    function _juiceCumulativeWeights() private view returns (uint256[] memory) {
        uint256[] memory cumulativeWeights = new uint256[](4);
        unchecked {
            for (uint256 i = 0; i < _juiceInventory.length; i++) {
                cumulativeWeights[i] =
                    _juiceInventory[i] +
                    (i == 0 ? uint256(0) : cumulativeWeights[i - 1]) -
                    totalMinted(i);
            }
        }
    ...
    Administration
{
    ...
    function setMaxSupply(uint256 id, uint256 maxSupply_) external isAdmin {
