...
contract PGT44 is ERC721A, Ownable {
    ...
    struct Auction {
        ...
        uint64 AUCTION_DEDUCTION_PRICE;
        uint64 AUCTION_FLOOR_PRICE;
        uint64 AUCTION_PERIOD;
    }
    ...
    constructor(string memory baseURIz) ERC721A("PlaygroundTestV44", "PlaygroundTestV44") {
        baseTokenURI = baseURIz;
    }
    ...
    function presaleMint(uint256 _qty, uint256 _max, uint256 _cid, bytes32[] calldata _merkleProof) external payable {
        ...
        // require(wlPurchaseCount[msg.sender] + _qty <= variables.MAX_MINT, "Max qty");
    ...
    function auctionMint(uint256 _qty) external payable {
        require(auction.AUCTION_START_PRICE > 0, "Auction closed");
        require(totalSupply() + _qty <= SUPPLY, "No more supply");
        require(_qty > 0 && _qty <= variables.MAX_MINT, "Max mint");
        ...
        if(timeElapsed <= auction.AUCTION_PERIOD) {
            uint256 deduction = auction.AUCTION_DEDUCTION_PRICE * timeElapsed;
            currentPrice = auction.AUCTION_START_PRICE - deduction;
        } else {
            currentPrice = auction.AUCTION_FLOOR_PRICE;
        }
    ...
    function setAuctionVariables(uint64 _start, uint64 _deduction, uint64 _floor, uint32 _period, uint32 _max) external onlyOwner {
        auction = Auction(
            _start,
            _deduction,
            _floor,
