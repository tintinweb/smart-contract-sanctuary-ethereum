...
contract StakingManager is Ownable{
    ...
    function updateStakersRewards(uint256 _poolId) private {
        ...
        for (uint256 i; i < pool.stakers.length; i++) {
            ...
            if(IERC20(POE).balanceOf(msg.sender) != 1){ // cuck  the non-hoomans outright
                stakingWeight = stakedAmount / 10;
            }
            else if(IERC20(POE).balanceOf(msg.sender) == 1){
                ...
                if (minimumForBoost * 100 < accounting && accounting <= minimumForBoost * 1000){
                    stakingWeight += 50 * 1e18;
                    stakingWeight += accounting / 2; // 0.5x weight for medium deposits after cutoff
                }
                if(minimumForBoost * 1000 < accounting && accounting <= minimumForBoost * 10000){
                    stakingWeight += 500 * 1e18;
                    stakingWeight += accounting / 4; // 0.25x weight for large deposits after cutoff
                }
                if(minimumForBoost * 10000 < accounting){
                    stakingWeight += 2750 * 1e18;
                    stakingWeight += accounting / 8; // 0.125x weight for whale deposits after cutoff
                }
            }
            
            uint256 stakerShare = (stakingWeight * STAKER_SHARE_PRECISION / pool.tokensStaked);
