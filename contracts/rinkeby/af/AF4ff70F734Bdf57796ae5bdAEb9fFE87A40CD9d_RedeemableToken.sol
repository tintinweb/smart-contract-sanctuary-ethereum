...
contract RedeemableToken is ERC1155, Ownable, ERC1155Burnable {    
    ...
    address cloneXContractAddress = 0xddE6627574A43Dfe7D8cCc8A3112B71B7f6d37b7;
    address cloneXMetadataAddress = 0x746b3367852487FE43c97e6de15EB93e22943Edb;
    ...
    function redeemBatch(address owner, address initialCollection, uint256[] calldata cloneXIds, uint256[] calldata wearableIds, uint256[] calldata amount) public payable {
        ...
        for(uint256 i = 0; i < cloneXIds.length; ++i) {
            ...
            if(wearableIds[i] >= 6 && wearableIds[i] <= 12) require(dna == 0x31, "Mismatch of wearable ID for Human DNA");
            else if(wearableIds[i] >= 13 && wearableIds[i] <= 20) require(dna == 0x32, "Mismatch of wearable ID for Robot DNA");
            else if(wearableIds[i] >= 21 && wearableIds[i] <= 28) require(dna == 0x33, "Mismatch of wearable ID for Demon DNA");
            else if(wearableIds[i] >= 29 && wearableIds[i] <= 36) require(dna == 0x34, "Mismatch of wearable ID for Angel DNA");
            else if(wearableIds[i] >= 37 && wearableIds[i] <= 44) require(dna == 0x35, "Mismatch of wearable ID for Reptile DNA");
            else if(wearableIds[i] >= 45 && wearableIds[i] <= 52) require(dna == 0x36, "Mismatch of wearable ID for Undead DNA");
            else if(wearableIds[i] >= 53 && wearableIds[i] <= 60) require(dna == 0x37, "Mismatch of wearable ID for Alien DNA");
            else if(wearableIds[i] >= 61 && wearableIds[i] <= 68) require(dna == 0x38, "Mismatch of wearable ID for Murakami DNA");
            else if(wearableIds[i] >= 69 && wearableIds[i] <= 73) require(isMurakamiDrip, "Mismatch of wearable ID for DRIP");

            // Check avail quantity 
            if(wearableIds[i] >= 1 && wearableIds[i] <= 5) require(wearableSupply[cloneXIds[i]][wearableIds[i]] + amount[i] <= 1, "Can't redeem more");
            else require(wearableSupply[cloneXIds[i]][wearableIds[i]] + amount[i] <= 2, "Can't redeem more");

            // Updating supplyinh
            wearableSupply[cloneXIds[i]][wearableIds[i]] = wearableSupply[cloneXIds[i]][wearableIds[i]] + amount[i];
            generalSupply[wearableIds[i]] = generalSupply[wearableIds[i]] + amount[i];

            // Minting batch
            _mint(owner, wearableIds[i], amount[i], "");
        }
    }
