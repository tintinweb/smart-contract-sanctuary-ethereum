...
contract YourNFT is Ownable, ERC721A {
    uint256 constant public MAX_SUPPLY = 5555; // 总供应量
    uint256 public price = 0.005 ether; // 公售单价
    uint256 constant public PUBLIC_MINT_LIMIT_TXN = 5; //公售单笔mint上限
    uint256 constant public PUBLIC_MINT_LIMIT = 10; //单钱包mint上限
    uint currentFreeNum = 0;
    uint256 freeLimit = 1555;
    string public baseURI;
    // OpenSea CONTRACT_URI - https://docs.opensea.io/docs/contract-level-metadata
    string public CONTRACT_URI;
    bool public paused = true;

    mapping(address => bool) public userMintedFree; // free mint用户
    mapping(address => uint256) public numUserMints; // 记录用户mint数目
    ...
    function freeMint(uint256 quantity) external payable mintCompliance(quantity) {
        ...
        require(currentFreeNum < freeLimit,"Exceed FreeMint limit");
        userMintedFree[msg.sender] = true;
        currentFreeNum += quantity;
    ...
    function withdraw() external payable onlyOwner {
        //  收取收入到开发者地址
        address DEV_ADDRESS = 0x250238204c9859C303D44b414A546FEe4999E79f;
        (bool succ, ) = payable(DEV_ADDRESS).call{
            value: address(this).balance / 3
        }("");
