...
contract Helem is ERC721Enumerable, Ownable, ReentrancyGuard  {
    ...
    uint256 private PRE_PRICE = 0.01 ether;
    uint256 private PUB_PRICE = 0.01 ether;
  ...
  event Rejected(address indexed _user);
    ...
    constructor(string memory _uri) ERC721("Helem Verce", "Helem") {
        setHiddenURI(_uri);
        start = true;
    }
    ...
    function mint(uint256 amount) public payable nonReentrant {
        ...
        if ((block.timestamp - startTime) <= 10800) {
            ...
            require(
                ...
                "In PUBLIC Stage, you can buy ONLY 4 Helems!"
            );
        }
        ...
        returns (string memory)
    {
        if (!revealStart) return hiddenURI;
            ...
            return
                string(
                    abi.encodePacked(_baseTokenURI, tokenId, ".json")
                );
    }
    ...
    function setRevealState(bool startReveal) external onlyOwner {
        revealStart = startReveal;
    }

    

    function startSale2() external onlyOwner {
        // require(start == false, "PRESALE is not Started!");
        startTime = block.timestamp;
        start = true;
    }
