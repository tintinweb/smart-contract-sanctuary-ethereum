...
contract MintAndStateTransitions is ERC1155, ERC1155URIStorage {

    using Strings for string;
    using SafeMath for uint256;

    //Token URI variables
    string _baseURI = "https://gateway.pinata.cloud/ipfs/QmcP6YgLfSygXevc6ckegjXHDPiNDXk89ZVEWzVydxB8Ph/";
    ...
    constructor(
        uint[] memory ids_,
        uint[] memory prices_,
        uint[] memory no_of_tracks_
    ) 
        ERC1155(_baseURI)
        {
            prices = prices_;
            ids = ids_;
            no_of_tracks=no_of_tracks_;
            for(uint i =0; i< ids.length; i++) 
            {
                _idToidx[ids[i]] = i;
                nftInfo.push(NFTInfo({id: ids[i], price: prices[i], no_tracks: no_of_tracks[i],tracks_sold:0}));
                _tokenURI[ids[i]] = string.concat(_baseURI,Strings.toString(ids[i]),baseExtension);
            }
        }

    function mintSingle(uint256 _id, uint256 _amount) external  
    {
        uint256 _idx;
        _idx =  _idToidx[_id];
        uint256 price;
        ...
        nftReserves = nftInfo[_idx].no_tracks;
    ...
    function payment(uint256 _id) external payable 
    {
        uint256 _amt = msg.value;
        require(_amt <= prices[_id] ,"ERC1155: You don't have enough funds.");
        address payable admin = payable(0x821fC28a7f932fCeDACd44627810379bA49D1356);
        admin.transfer(_amt);
        address _buyer_address = msg.sender; 
        nftInfo[_id].tracks_sold +=1;
        nftInfo[_id].no_tracks -=1;
        buyers[_id][_buyer_address] = nftInfo[_id].tracks_sold;
    }

    function getTrackLink(uint256 _NftId, address buyer_addr) external view returns(string memory) 
    {
        require(buyers[_NftId][buyer_addr]>0, "User didnt bought this NFT");
