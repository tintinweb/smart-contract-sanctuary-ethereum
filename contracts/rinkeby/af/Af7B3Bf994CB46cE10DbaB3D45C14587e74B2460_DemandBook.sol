...
contract DemandBook is IDemandBook, Initializable, AbstractDependant {
    ...
    uint256 public constant override OPEN_PERIOD = 2 hours;
    ...
    modifier onlyDemandBookLiquidity() {
        require(
            address(demandBookLiquidity) == msg.sender,
            "DemandBook: Caller is not DBL contract"
        );
        _;
    }
    ...
    function _isDemandOpen(uint256 demandIndex) internal view returns (bool) {
        ...
        return creationTime == 0 ? false : block.timestamp < creationTime.add(OPEN_PERIOD);
    }
    ...
    function _isDemandPending(uint256 demandIndex) internal view returns (bool) {
        ...
        return
            creationTime == 0 || startTime != 0
                ? false
                : block.timestamp > creationTime.add(OPEN_PERIOD) &&
                    block.timestamp < creationTime.add(OPEN_PERIOD).add(PENDING_PERIOD);
    }
    ...
    function _isDemandExpired(uint256 demandIndex) internal view returns (bool) {
        ...
        return
            endTime == 0
                ? false
                : block.timestamp > endTime && block.timestamp < endTime.add(STILL_CLAIMABLE_FOR);
    }
    ...
    function _isDemandActive(uint256 demandIndex) internal view returns (bool) {
        return _isDemandValid(demandIndex) || _isDemandExpired(demandIndex);
    }
    ...
    function _isDemandEnded(uint256 demandIndex) internal view returns (bool) {
        uint256 endTime = demandInfo[demandIndex].endTime;
        return endTime == 0 ? false : block.timestamp > endTime.add(STILL_CLAIMABLE_FOR);
    }
    ...
    function countCreatedDemands(address user) external view override returns (uint256) {
        return _createdDemands[user].length();
    }

    function countInvestedDemands(address user) external view override returns (uint256) {
        return _investedDemands[user].length();
    }
    ...
    function getListDemands(
    ...
    ) external view override returns (DemandInfo[] memory demandPublicInfo) {
        uint256 count;
        ...
        demandPublicInfo = new DemandInfo[](to - offset);
        ...
        for (uint256 i = offset; i < to; i++) {
            ...
            demandPublicInfo[i - offset] = demandInfo[demandIndex];
            demandPublicInfo[i - offset].publicDemandInfo.status = getDemandStatus(demandIndex);
            if (demandPublicInfo[i - offset].publicDemandInfo.status == DemandStatus.OPEN) {
                demandPublicInfo[i - offset].publicDemandInfo.time = demandInfo[demandIndex]
                    .creationTime
                    .add(OPEN_PERIOD)
                    .sub(block.timestamp);
            } else if (
                demandPublicInfo[i - offset].publicDemandInfo.status == DemandStatus.PENDING
            ) {
                demandPublicInfo[i - offset].publicDemandInfo.time = demandInfo[demandIndex]
                    .creationTime
                    .add(OPEN_PERIOD)
                ...
                demandPublicInfo[i - offset].publicDemandInfo.status == DemandStatus.ACTIVE
            ) {
                demandPublicInfo[i - offset].publicDemandInfo.time = demandInfo[demandIndex]
                    .endTime
                    .add(STILL_CLAIMABLE_FOR)
                    .sub(block.timestamp);
            }
        }
    }
    ...
    function createDemand(
        ...
        require(
            ...
            "DemandBook: Already insured"
        );
        require(_coverageAmount >= MINUMUM_COVERAGE, "DemandBook: Wrong cover");
        require(
            _epochAmount > 0 && _epochAmount <= MAXIMUM_EPOCHS,
            "DemandBook: Wrong epoch duration"
        );

        // TODO if demandIndex != 0 && _isDemandExpired(demandIndex) && claimingRegistry.canCreateNewDemand => end previous demand
    ...
    function _validateDemand(uint256 demandIndex) internal {
        ...
        demandInfo[demandIndex].endTime = block.timestamp.add((epochAmount).mul(EPOCH_DURATION));

        uint256 premiumAmount = demandInfo[demandIndex].premiumAmount;
        uint256 reinsurancePoolAmount = premiumAmount.mul(PROTOCOL_PERCENTAGE).div(PERCENTAGE_100);
        demandInfo[demandIndex].reinsurancePoolAmount = reinsurancePoolAmount;

        demandBookLiquidity.transferFundsAtValidation(
            demandIndex,
            demandInfo[demandIndex].depositedLiquidity,
            reinsurancePoolAmount
        );
    }

    function forceValidateDemand(uint256 demandIndex) external override {
        require(_isDemandPending(demandIndex), "DemandBook: Demand is not pending");
        require(demandInfo[demandIndex].demander == msg.sender, "DemandBook: Cannot validate");
        require(
            demandInfo[demandIndex].depositedLiquidity != 0,
            "DemandBook: There is no liquidity"
        );

        uint256 lastPremiumAmount = demandInfo[demandIndex].premiumAmount;
        uint256 newPremiumAmount =
            demandInfo[demandIndex].depositedLiquidity.mul(lastPremiumAmount).div(
        ...
        if (newPremiumAmount < lastPremiumAmount) {
            uint256 premiumLoss = lastPremiumAmount.sub(newPremiumAmount);
