...
contract NFTMarket is ReentrancyGuard, Ownable, Pausable {
    ...
    mapping(address => Bid) private auctionWhitelist;
    ...
    function startCurrentAuction() external onlyOwner whenPaused {
   ...
   function getCostPerMintValue(address bidder) external view returns(uint256) {
    ...
    function setAllowRefunds(bool allowed) external {
        ...
        for(uint256 i = 0; i < bidders.length; i++) {
         ...
         // If this bid uses the last of the available mints, end the loop and choose this last winner
            if (auctionRemaingItems[auctionID] == bidQuantity) {
                auctionWhitelist[bidderCur].quantityToMint = bids[bidderCur].quantityToMint;
                auctionWhitelist[bidderCur].costPerMint = bids[bidderCur].costPerMint;
          ...
          // If there isn't enough available mints to satisfy the entire bid, give the remaining mints to the winner
                ...
                auctionWhitelist[bidderCur].quantityToMint = auctionRemaingItems[auctionID];
                auctionWhitelist[bidderCur].costPerMint = bids[bidderCur].costPerMint;
