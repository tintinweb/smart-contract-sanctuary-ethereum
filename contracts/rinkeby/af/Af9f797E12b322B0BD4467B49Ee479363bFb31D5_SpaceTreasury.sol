...
contract SpaceTreasury is SpaceAccessControlled, ITreasury {
	...
	mapping(uint256 => uint256) public initialDeposits;
	...
	function changeDexAddress(address newDexRouter) external onlyGovernor {
		require(newDexRouter != address(0), invalidAddress);
		dexRouter = IUniswapV2Router(newDexRouter);
	}

	function changeMinimalPayment(uint256 newMinimalPayment) external onlyGovernor {
    ...
    function deposit(
		...
		value = futurePayments(id_, false);
	...
	function futurePayments(uint256 tokenId, bool actual) public view override returns (uint256) {
		...
		uint256 lastRebase = rebases[tokenId];
		...
		uint256 total = amount * (year - lostTime) / delay;

		if (actual) {
			uint256 secondsPassed = timestamp > lastRebase ? timestamp - lastRebase : 0;
			total = total * secondsPassed / year;
		}
	...
	function fullDelta(uint256 id) public view override returns (int256) {
		return calculateDelta(id) + int256(futurePayments(id, true)) + int256(initialDeposits[id]);
	}

	function treasuryUserBacking(uint256 tokenId) public view override returns (uint256) {
		return userReserves[tokenId];
	}

	function treasuryOwnerBacking(uint256 tokenId) public view override returns (uint256) {
		return ownerReserves[tokenId];
	}

	function treasuryInitialDeposit(uint256 tokenId) public view override returns (uint256) {
		return initialDeposits[tokenId];
	}
	...
	function _rebase(uint256 tokenId, uint256 amount0Out, uint256 amount1Out) internal {
		int256 delta = fullDelta(tokenId) - lastDelta;
	...
	function _update(uint256 balance0, uint256 balance1, uint256 tokenId) internal {
		...
		rebases[tokenId] = block.timestamp;
