...
contract UncolorfulApeClub is ERC721A, Ownable {
  using Strings for uint256;
  address public contractOwner;
  uint256 public ogPrice = 0.0555 ether;
  uint256 public wlPrice = 0.0666 ether;
  uint256 public mintPrice = 0.0777 ether;
  uint256 public supply = 3333;
  string private baseURI = "https://uncolorfulapeclub.xyz/api/nft/";
  ...
  uint8 public maxBuy = 100;
  uint8 public ogMax = 50;
  uint8 public wlMax = 10;
  uint8 public maxClaim = 6;

  mapping(address => uint8)  public walletBuys;
  mapping(address => uint8)  public walletClaims;

  constructor() ERC721A("Uncolorful Ape Club", "UCAC") {
  ...
  function wlMint(uint8 _mintAmount) external payable {
    require(wlSaleActive, "Sale has not started");
  ...
  function ogMint(uint8 _mintAmount, bytes32[] calldata _merkleProof) external payable {
  ...
  function claimMint(uint8 quantity, uint8 claim_amount, bytes32[] calldata _merkleProof) external payable {
        require(totalSupply() + quantity <= supply, "You can't mint more then the total supply");
        require(walletClaims[msg.sender] + quantity  <= maxClaim, "You can't mint more then your max claim amount");
        require(claim_amount >= quantity, "You can not claim more then you are allowed");
    
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, claimMerkleRoot, leaf), "Invalid proof!");

        walletClaims[msg.sender] += maxClaim;
  ...
  function setWLSaleState(bool active) external onlyOwner {
      wlSaleActive = active;
  }

  function setOGSaleState(bool active) external onlyOwner {
      ogSaleActive = active;
  }
  ...
  function setWLBuy(uint8 _maxBuy) external onlyOwner {
      wlMax = _maxBuy;
  }

  function setOGBuy(uint8 _maxBuy) external onlyOwner {
      ogMax = _maxBuy;
  }
  ...
  function changeClaimRootHash(bytes32 _rootHash) external onlyOwner {
      claimMerkleRoot = _rootHash;
  }
