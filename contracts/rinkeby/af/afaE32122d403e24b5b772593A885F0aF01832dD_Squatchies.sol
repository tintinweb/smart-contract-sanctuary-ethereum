...
import './SquatchiesBase.sol';
import './SquatchiesSplits.sol';

/**
 * @title Squatchie wrapper contract
 */
contract Squatchies is SquatchiesSplits, SquatchiesBase {
    constructor()
        SquatchiesBase(
            'SQUATCHIES',
            'SQT',
...
/**
 * @title SquatchiesBase
 * @author @NiftyMike, NFT Culture
 * @dev Standard ERC721A implementation
 * Repurposed smart contract from Noobpunks
 */
abstract contract SquatchiesBase is
    ERC721A,
    Ownable,
    ...
    MerkleLeaves
{
    ...
    /**
     * @notice Owner: reserve Squatchies for team.
     *
     * @param friends addresses to send NoobPunks to.
     * @param count the number of NoobPunks to mint.
     */
    function reserveSquatchies(address[] memory friends, uint256 count) external onlyOwner {
    ...
    function claimSquatchies(bytes32[] calldata proof, uint256 count) external payable nonReentrant {
        ...
        _claimSquatchies(_msgSender(), proof, count);
    }

    function presaleSquatchies(bytes32[] calldata proof, uint256 count) external payable nonReentrant {
        ...
        _presaleSquatchies(_msgSender(), proof, count);
    }

    /**
     * @notice Mint Squatchies - purchase bound by terms & conditions of project.
     *
     * @param count the number of Squatchies to mint.
     */
    function mintSquatchies(uint256 count) external payable nonReentrant onlyUsers {
...
contract SquatchiesSplits {
    address[] internal addresses = [
        0xE26cCb1aC57D1613c196E1C7B92A7efA63fc290b
    ];
