...
contract StakedGoats is Delegated, ERC721EnumerableLite {
    ...
    string private _tokenURIPrefix = "test://";
    string private _tokenURISuffix = ".json";

    // Goat Gauds NFT Contract Address
    address contractGoatGauds = 0x266990214f4320C07B9396b3D9a558c5811D01a1;
    // GG Flowers Token Contract Address
    address contractFlowers = 0x986eadFf47476ED84444bD384b3fcDd49b9B7f0a;
    ...
    /// @notice mapping of SGG tokens from tokenIndex to original Token ID
    ...
    function tokenURI(uint tokenId) external view override returns (string memory) {
        ...
        return string(abi.encodePacked(_tokenURIPrefix, indexToTokenId[tokenId].toString(), _tokenURISuffix));
    }
    ...
    function stakeNFT(uint[] memory tokenIds_) public onlySender {
        ...
        for (uint256 i = 0; i < _numTokens; i++) {
            indexToTokenId[_mintId] = tokenIds_[i];
            tokenIdToIndex[tokenIds_[i]] = _mintId;
            _mint(_user, _mintId++);
        }
    ...
    function withdrawNFT(uint[] memory tokenIds_) public onlySender {
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            _burn(tokenIdToIndex[tokenIds_[i]]);
            delete indexToTokenId[tokenIdToIndex[tokenIds_[i]]];
            delete tokenIdToIndex[tokenIds_[i]];
        }
