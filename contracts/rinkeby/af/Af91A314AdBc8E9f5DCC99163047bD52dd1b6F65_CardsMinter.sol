...
/// @author Roger S
/// @notice This contract mints new Game Cards for Endless Crawler
...
contract CardsMinter is ERC1155, ERC1155Burnable, ERC1155Supply, Ownable {
  ...
  function _beforeTokenTransfer(address operator, address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal override(ERC1155, ERC1155Supply) {
	...
	/// @return bool if paused (true) or not (false)
	...
	/// @param id Token id
	/// @return price Price of the token price, in WEI
	function getPrice(uint256 id) external view returns (uint256) {
		return _store.getCardPrice(id);
	}

	/// @notice Run all the required tests for a successful purchase(id)
	/// @param id Token id
	/// @param value Value that will be sent to purchase(), in WEI
	/// @return bool If purchase is allowed (true) or not (false)
	/// @return reason The reason, if purchase nto allowed
	...
	/// @notice Purchases 1 of Token for the Sender. The message value must be equal or higher than getPrice(id)
	/// @param id Token id
  ...
  function purchase(uint256 id, bytes memory data) public payable {
	...
	/// @notice Burn tokens in batch. Sender must be owner or approved
	/// @param account Token owner
	/// @param ids Token id
	/// @param amounts The amounts of tokens to burn, must match ids
	function burnBatch(address account, uint256[] memory ids, uint256[] memory amounts) public override {
		...
		for (uint256 i = 0; i < ids.length; i++) {
			emit Burned(account, ids[i], amounts[i]);
		}
	}
