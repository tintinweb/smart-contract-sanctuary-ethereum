...
contract NftMarketplace is ReentrancyGuard {
    ...
    mapping(address => uint256)[] private s_nftAddressToTokenId;
        ...
        theOwner(nftAddress, tokenId, msg.sender)
    {
        ...
        s_completeListing.push(
            CompleteListing(nftAddress, tokenId, price, msg.sender)
        );
        ...
        theOwner(nftAddress, tokenId, msg.sender)
    {
        ...
        for (uint256 i = 0; i < s_completeListing.length; i++) {
            if (
                s_completeListing[i].nftAddress == nftAddress &&
                s_completeListing[i].tokenId == tokenId
            ) {
                delete s_completeListing[i];
                break;
            }
        }
        ...
        theOwner(nftAddress, tokenId, msg.sender)
    {
        ...
        for (uint256 i = 0; i < s_completeListing.length; i++) {
            if (
                ...
                s_completeListing[i].price = newPrice;
            }
        }
