...
contract TNStoken is Ownable {
  ...
  /**
  @dev _owner will be prevented from sending _amount of tokens. Anything beyond this amount will be spendable.
  */
  function increaseLockedAmount(address _owner, uint256 _amount) public onlyOwner returns (uint256) {
    ...
    /**
    @dev _owner will be allowed to send _amount of
    tokens again. Anything
    remaining locked will still not be spendable. If
    the _amount is greater
    than the locked amount, the locked amount is
    ...
    */
    ...
    /**
    @dev Returns number of tokens the address is
    still prevented from using
    */
