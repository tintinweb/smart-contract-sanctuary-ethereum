    ...
    ReentrancyGuardUpgradeable
{
  ...
  */
    ...
    function swap(uint256 _capAmount, uint256 visToken) external nonReentrant whenNotPaused {
        ...
        if (_stakedAmount < tier3StakeAmount) {
            ...
            emit SwapEmit(msg.sender, 0, 0, 0, _capAmount, visToken);
        }
        //calculating tier3 requirements
            ...
            if (tier3CurrentCappedAmount <= tier3) {
                ...
                emit SwapEmit(msg.sender, 0, 0, _capAmount, 0, visToken);
            } else if (
                ...
                emit SwapEmit(msg.sender, 0, 0, tier3Cap, tier4Cap, visToken);
            }
        }
        //calculating tier2 requirements
            ...
            if (tier2CurrentCappedAmount <= tier2) {
                ...
                emit SwapEmit(msg.sender, 0, _capAmount, 0, 0, visToken);
            } else if (
                ...
                tier3PreCappedAmount += tier3Cap;
                emit SwapEmit(msg.sender, 0, tier2Cap, tier3Cap, 0, visToken);
            } else if (
                tier2PreCappedAmount <= tier2 &&
                _capAmount > tier2CapBalance + tier3CapBalance
            ) {
  ...
  */
    ...
    function swapCalculate(address _account) external view returns(uint256, uint256) {
        if(stakedAmountOf[_account] >= tier2StakeAmount && stakedAmountOf[_account] < tier1StakeAmount) {
            //tier 2
            uint256 tier2Balance = tier2- tier2PreCappedAmount;
            uint256 Intier = 2;
            return (Intier, tier2Balance);
        } else if(stakedAmountOf[_account] >= tier3StakeAmount && stakedAmountOf[_account] < tier2StakeAmount) {
            //tier 3
            uint256 tier3Balance = tier3 - tier3PreCappedAmount;
            uint256 Intier = 3;
            return (Intier, tier3Balance);
        } else if(stakedAmountOf[_account] > tier1StakeAmount) {
            //tier 1
            uint256 tier1Balance = tier1 - tier1PreCappedAmount;
            uint256 Intier = 1;
            return (Intier, tier1Balance);
        } else {
            //tier 4
            uint256 Intier = 4;
            return (Intier, 0);
