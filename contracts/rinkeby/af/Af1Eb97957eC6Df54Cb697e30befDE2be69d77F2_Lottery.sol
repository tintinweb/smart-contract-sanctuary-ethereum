...
contract Lottery is VRFConsumerBaseV2, KeeperCompatible {
    ...
    address private immutable i_shibToken;
    ...
    function enterLottery(uint256 _tokenAmount) external {
        ...
        if (
            IERC20(i_shibToken).allowance(msg.sender, address(this)) >=
            _tokenAmount
        ) {
        ...
        IERC20(i_shibToken).transferFrom(
            msg.sender,
            address(this),
            _tokenAmount
        );
    ...
    function approveSpending(uint256 _tokenAmount) external {
        IERC20(i_shibToken).approve(address(this), _tokenAmount);
    }
    ...
    function getAllowance() external view returns (uint256) {
        return IERC20(i_shibToken).allowance(msg.sender, address(this));
    }
