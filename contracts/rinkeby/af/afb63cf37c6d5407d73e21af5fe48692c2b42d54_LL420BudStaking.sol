...
contract LL420BudStaking is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @dev Initial amount of reward within first 14 days of lock period.
    uint16 public constant INITIAL_REWARD = 200;
    ...
    uint32 public constant SECONDS_IN_DAY = 24 * 60 * 60;
    uint32[6] public thcInfo;
    ...
    /// user address => user bud id array
    mapping(address => EnumerableSet.UintSet) private _userBudInfo;
     ...
     */
    function stakedNfts(address _user) external view returns (uint256[] memory, uint256[] memory) {
        require(_user != address(0), "LL420BudStaking: user address cant be zero");
        return _stakedNfts(_user);
    }

    /**
     * @param _ids The BUD NFT ids to deposit.
     */
    function deposit(uint256[] memory _ids) external nonReentrant onlyStarted {
        ...
        if (_userBudInfo[_msgSender()].length() > 0) {
        ...
        for (uint256 i; i < _ids.length; i++) {
            ...
            require(!_userBudInfo[_msgSender()].contains(_ids[i]), "LL42-BudStaking: Cant stake same BUDs");

            BUD_CONTRACT.transferFrom(_msgSender(), address(this), _ids[i]);
            _userBudInfo[_msgSender()].add(_ids[i]);
     ...
     */
    function withdraw(uint256[] memory _ids) external nonReentrant {
        return _withdraw(_ids);
    }
    ...
    /**
     */
    function withdrawGameKey(uint256[] memory _ids) external nonReentrant {
    ...
    function getDailyReward(address _user) external view returns (uint256) {
        ...
        uint256 length = _userBudInfo[_user].length();
        uint256 dailyReward;

        for (uint256 i; i < length; i++) {
            uint256 tokenId = _userBudInfo[_user].at(i);
     ...
     * Todo
     */
    function withdrawReward() external nonReentrant rewardable {
        UserInfo storage user = _userInfo[_msgSender()];
        ...
        emit WithdrawReward(_msgSender());
    }
     ...
     *
     */
    function _stakedNfts(address _user) internal view returns (uint256[] memory, uint256[] memory) {
        require(_user != address(0), "LL420BudStaking: user address cant be zero");

        uint256 budLength = _userBudInfo[_user].length();
        uint256 gameKeyLength = _gameKeyInfo[_user].length();
        uint256[] memory budIdArray = new uint256[](budLength);
        uint256[] memory gameKeyIdArray = new uint256[](gameKeyLength);

        for (uint256 i; i < budLength; i++) {
            budIdArray[i] = _userBudInfo[_user].at(i);
        }
        for (uint256 i; i < gameKeyLength; i++) {
            gameKeyIdArray[i] = _gameKeyInfo[_user].at(i);
        }

        return (gameKeyIdArray, budIdArray);
    }

    /**
     * @param _ids The BUD NFT ids to deposit.
     */
    function _withdraw(uint256[] memory _ids) internal onlyStarted {
        ...
        require(_userBudInfo[_msgSender()].length() >= _ids.length, "LL420BudStaking: Amount NFTs is wrong");
        require(_gameKeyInfo[_msgSender()].length() > 0, "LL420BudStaking: Need more than 1 GAMEKEY staked");
        ...
        for (uint256 i; i < _ids.length; i++) {
            require(_userBudInfo[_msgSender()].contains(_ids[i]), "LL420BudStaking: Unautorized id");

            BudInfo storage bud = _budInfo[_ids[i]];

            BUD_CONTRACT.transferFrom(address(this), _msgSender(), _ids[i]);
            _userBudInfo[_msgSender()].remove(_ids[i]);
            bud.timestamp = 0;
        }
    ...
    function _withdrawGameKey(uint256[] memory _ids) internal {
        ...
        if (_gameKeyInfo[_msgSender()].length() == _ids.length) {
            uint256[] memory budIds;
            (, budIds) = _stakedNfts(_msgSender());
            _withdraw(budIds);
        }

        for (uint256 i; i < _ids.length; i++) {
            require(_gameKeyInfo[_msgSender()].contains(_ids[i]), "LL420BudStaking: Unauthroized GAMEKEY id");
    ...
    function _getPendingReward(address _user) internal view returns (uint256) {
        ...
        for (uint256 i; i < length; i++) {
            ...
            if (block.timestamp - bud.timestamp <= LOCK_PERIOD) {
                ...
                pendingReward +=
                    INITIAL_REWARD *
                    ((bud.timestamp + LOCK_PERIOD - user.lastCheckpoint) / SECONDS_IN_DAY);
                pendingReward +=
    ...
    function emergencyWithdraw(address _user) external onlyOwner {
        ...
        uint256 gameKeyLength = _gameKeyInfo[_user].length();
        for (uint256 i; i < gameKeyLength; i++) {
            uint256 tokenId = _gameKeyInfo[_user].at(0);
            require(
                GAME_KEY_CONTRACT.ownerOf(tokenId) == address(this),
                "LL420BudStaking: Game Key is not belong to this contract"
            );

            GAME_KEY_CONTRACT.safeTransferFrom(address(this), _user, tokenId);
            _gameKeyInfo[_user].remove(tokenId);
        }
        totalGameKeyStaked -= uint16(gameKeyLength);

        uint256 budLength = _userBudInfo[_user].length();
        for (uint256 i; i < budLength; i++) {
            uint256 tokenId = _userBudInfo[_user].at(0);
            require(
            ...
            _userBudInfo[_user].remove(tokenId);
        }
