...
contract OwnerVerifier {
    ...
    function hasRole(address contractAddr, address caller, bytes32 role) public view returns (bool) {
        return toBool(readOptional(contractAddr, abi.encodeWithSignature("hasRole(bytes32,address)", role, caller))) == true;
    }

    function toBool(bytes memory _bytes)
    internal
    pure
    returns (bool value) {
        assembly {
            value := mload(add(_bytes, 1))
        }
    }
