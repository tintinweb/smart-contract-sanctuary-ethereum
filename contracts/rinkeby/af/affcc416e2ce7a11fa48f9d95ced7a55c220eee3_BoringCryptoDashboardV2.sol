...
interface IFlameToken is IBEP20{
...
interface IKnightGrandCross {
    ...
    function flame() external view returns (address);
    function flamePerBlock() external view returns (uint256);
    function totalAllocPoint() external view returns (uint256);

    function poolLength() external view returns (uint256);
    function poolInfo(uint256 nr) external view returns (address, uint256, uint256, uint256);
    function userInfo(uint256 nr, address who) external view returns (uint256, uint256);
    function pendingFlame(uint256 nr, address who) external view returns (uint256);
}
...
contract BoringCryptoTokenScanner
{
    ...
    function getPairsFull(address who, address[] calldata addresses) public view returns(PairFull[] memory) {
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            pairs[i].totalSupply = IBEP20(token).totalSupply();
        }
        return pairs;
    }
}
...
contract BoringCryptoDashboardV2
{
    ...
    IKnightGrandCross knightgrandcross;
    ...
    constructor(
        address _knightgrandcross,
        address _uniFactory,
        address _sushiFactory,
        address _shibaFactory,
        address _weth
    ) public {
        knightgrandcross = IKnightGrandCross(_knightgrandcross);
    ...
    function getPools(uint256[] calldata pids) public view returns(PoolsInfo memory, PoolInfo[] memory) {
        ...
        info.totalAllocPoint = knightgrandcross.totalAllocPoint();
        uint256 poolLength = knightgrandcross.poolLength();
        ...
        for (uint256 i = 0; i < pids.length; i++) {
            pools[i].pid = pids[i];
            (address lpToken, uint256 allocPoint,,) = knightgrandcross.poolInfo(pids[i]);
    ...
    function findPools(address who, uint256[] calldata pids) public view returns(PoolInfo[] memory) {
        ...
        for (uint256 i = 0; i < pids.length; i++) {
            (uint256 balance,) = knightgrandcross.userInfo(pids[i], who);
            if (balance > 0) {
                count++;
            }
        }
    ...
    function getETHRate(address token) public view returns(uint256) {
        ...
        if (token != weth)
        {
            ...
            if (address(pairFire) != address(0)) {
                (reserve0Fire, reserve1Fire,) = pairFire.getReserves();
            }

            if (address(pairFire) == address(0) || reserve0UniV2 > reserve0Fire || reserve1UniV2 > reserve1Fire) {
                // return uni rate
            ...
            } else if (reserve0Sushi > reserve0Fire || reserve1Sushi > reserve1Fire) {
                ...
                if (pairFire.token0() == weth) {
                    eth_rate = uint256(reserve1Fire).mul(1e18).div(reserve0Fire);
                } else {
                    eth_rate = uint256(reserve0Fire).mul(1e18).div(reserve1Fire);
                }
            }
        }
        return eth_rate;
    ...
    struct UserPoolInfo {
        ...
        uint256 lpAllowance; // LP tokens approved for KnightGrandCross
        uint256 reserve0;
        uint256 reserve1;
        uint256 token0rate;
        uint256 token1rate;
        uint256 rewardDebt;
        uint256 pending; // Pending FLAME
    }
    ...
    function pollPools(address who, uint256[] calldata pids) public view returns(UserPoolInfo[] memory) {
        ...
        for (uint256 i = 0; i < pids.length; i++) {
            (uint256 amount,) = knightgrandcross.userInfo(pids[i], who);
            pools[i].balance = amount;
            pools[i].pending = knightgrandcross.pendingFlame(pids[i], who);

            (address lpToken,,,) = knightgrandcross.poolInfo(pids[i]);
            pools[i].pid = pids[i];
            IPair uniV2 = IPair(lpToken);
            pools[i].totalSupply = uniV2.balanceOf(address(knightgrandcross));
            pools[i].lpAllowance = uniV2.allowance(who, address(knightgrandcross));
