...
contract HMW is ERC721Enum, Ownable, ReentrancyGuard {
	...
	uint256 public PRESALE_PRICE = 0 * 10**16;
	uint256 public SALE_PRICE = 0 * 10**16;
	
    bool public presaleEnable = false;
	bool public alphaMintEnable = false;
	bool public saleEnable = false;
	
    string private baseURI;
	bytes32 public merkleRootPreSale;
	bytes32 public merkleRootAlphaOne;
	bytes32 public merkleRootAlphaTwo;
	bytes32 public merkleRootAlphaThree;
	
	struct User {
		uint256 presalemint;
		uint256 salemint;
		uint256 alphamintone;
		uint256 alphaminttwo;
		uint256 alphamintthree;
		uint256 rafflesmint;
    }
	mapping (address => User) public users;

    constructor() ERC721P('Howling Meta Wolves', 'HMW') {}
	
    function _baseURI() internal view virtual returns (string memory) {
        return baseURI;
    }
	
	function mintNFT(address _to, uint256 _count) public onlyOwner{
	...
	function mintNFT(uint256 _count, bytes32[] calldata merkleProof, uint256 _type) public payable{
		bytes32 node = keccak256(abi.encodePacked(msg.sender));
		uint256 totalSupply = totalSupply();
		require(
			alphaMintEnable, 
			"Alpha mint is not enable"
		);
        require(
			NFT_MINTED + _count <= MAX_NFT, 
			"Exceeds max limit"
		...
		{
		    require(
			   MerkleProof.verify(merkleProof, merkleRootAlphaOne, node), 
			   "MerkleDistributor: Invalid proof."
			);
			require(
				users[msg.sender].alphamintone + _count <= 1,
				"Exceeds max mint limit per wallet"
			);
			users[msg.sender].alphamintone = users[msg.sender].alphamintone + _count;
		}
		else if(_type==2)
		{
		    require(
			   MerkleProof.verify(merkleProof, merkleRootAlphaTwo, node), 
			   "MerkleDistributor: Invalid proof."
			);
			require(
				users[msg.sender].alphaminttwo + _count <= 2,
				"Exceeds max mint limit per wallet"
			);
			users[msg.sender].alphaminttwo = users[msg.sender].alphaminttwo + _count;
		}
		else
		{
		   require(
			   MerkleProof.verify(merkleProof, merkleRootAlphaThree, node), 
			   "MerkleDistributor: Invalid proof."
			);
			require(
				users[msg.sender].alphamintthree + _count <= 3,
				"Exceeds max mint limit per wallet"
			);
			users[msg.sender].alphamintthree = users[msg.sender].alphamintthree + _count;
		}
		for (uint256 i = 0; i < _count; i++) {
	...
	function mintNFT(uint256 _count, bytes32[] calldata merkleProof) public payable{
		bytes32 node = keccak256(abi.encodePacked(msg.sender));
		uint256 totalSupply = totalSupply();
		require(
			presaleEnable, 
	...
	function setAlphaMintStatus(bool status) public onlyOwner {
	   require(alphaMintEnable != status);
       alphaMintEnable = status;
    }
	...
	function updateAlphaMintMerkleRoot(bytes32 mintOneMerkleRoot, bytes32 mintTwoMerkleRoot, bytes32 mintThreeMerkleRoot) external onlyOwner {
	    merkleRootAlphaOne = mintOneMerkleRoot;
        merkleRootAlphaTwo = mintTwoMerkleRoot;
	    merkleRootAlphaThree = mintThreeMerkleRoot;
	}
}