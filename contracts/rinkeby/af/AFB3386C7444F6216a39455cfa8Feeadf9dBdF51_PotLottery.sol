...
contract PotLottery is Initializable, ReentrancyGuardUpgradeable {
    ...
    function deductAmountToPayAsFees(string memory _tokenName, uint256 _value) private {
        ...
        if (keccak256(bytes(_tokenName)) == keccak256(bytes('PLSP'))) {
            _distributeToTokenomics(_value);
        }
    }
    ...
    function swapAccumulatedFees() external validPLSP nonReentrant {
        ...
        _distributeToTokenomics(amounts[1]);
    }
     ...
     */
    function airdrop() external validPLSP returns (uint256) {
        IPLSP PLSPToken = IPLSP(PLSP_Address);
    ...
    function _distributeToTokenomics(uint256 value) internal {
