...
contract AikoVirtual is Ownable, ERC721A {
	struct AccountInfo {
		...
		uint64 purchasedHolder;
	}

	uint256 internal immutable MAX_SUPPLY;
	uint256 internal constant FIRST_SALE_MAX = 3333;
	uint256 internal constant WALLET_MAX = 6;

	uint256 public constant TEAM_PRICE = 0 ether;
	uint256 public constant FIRST_ORB_PRICE = 0.08 ether;
	uint256 public constant SECOND_ORB_PRICE = 0.1 ether;
	uint256 public constant HOLDER_PRICE = 0.12 ether;

	uint256 public constant TEAM_MAX = 6;
	uint256 public constant FIRST_ORB_MAX = 2;
	uint256 public constant SECOND_ORB_MAX = 4;
	uint256 public constant HOLDER_MAX = 2;
	...
	uint64 public holderSaleStartTime = type(uint64).max;
	uint64 public holderSaleEndTime = type(uint64).max;

	mapping(address => AccountInfo) public accountInfo;

	bytes32 public freeRoot;
	bytes32 public orbRoot;
	...
	function devMintTo(address who, uint256 amount) public onlyOwner {
		...
		// If still supply after last sale, let team mint.
		if (block.timestamp > holderSaleEndTime) {
			maxCanMint = MAX_SUPPLY - totalSupply();
		}

		// If gt remaining, cap off.
		uint256 _devMinted = devMinted;
		if (_devMinted + amount > maxCanMint) {
			amount = maxCanMint - _devMinted;
		}
	...
	function setSaleRoots(
		bytes32 _freeRoot,
		bytes32 _cyberOrbRoot,
		bytes32 _holderRoot
	) external onlyOwner {
		freeRoot = _freeRoot;
		orbRoot = _cyberOrbRoot;
	...
	function setStartAndEndTimes(uint64[3] memory _startTimes, uint64[3] memory _endTimes) external onlyOwner {
		...
		holderSaleStartTime = _startTimes[2];
		holderSaleEndTime = _endTimes[2];
	}
	...
	function teamAiko(
		...
		require(_saleOpen(firstSaleStartTime, holderSaleEndTime), "sale ended");
		...
		uint256 purchased = accountInfo_.purchasedFirst + accountInfo_.purchasedSecond + accountInfo_.purchasedHolder;
		uint256 fromCurrentSale = accountInfo_.freeMinted + amountToBuy;
		require(purchased + fromCurrentSale <= WALLET_MAX, "above max");
		require(fromCurrentSale <= TEAM_MAX, "above sale max");
	...
	function firstCyberOrbAiko(
		...
		require(amountToBuy > 0, "0");
		require(merkleProof.length != 0, "No proof given");
		require(totalSupply() + amountToBuy <= FIRST_SALE_MAX, "Sold out");
		require(_saleOpen(firstSaleStartTime, firstSaleEndTime), "sale ended");

		bytes32 node = keccak256(abi.encodePacked(msg.sender, amount));
		require(MerkleProof.verify(merkleProof, orbRoot, node), "MerkleProof: Invalid proof");
		require(msg.value == amountToBuy * FIRST_ORB_PRICE, "Not right ETH sent");

		AccountInfo memory accountInfo_ = accountInfo[msg.sender];
		uint256 purchased = accountInfo_.freeMinted;
	...
	function secondCyberOrbAiko(
		...
		AccountInfo memory accountInfo_ = accountInfo[msg.sender];
		uint256 purchased = accountInfo_.freeMinted + accountInfo_.purchasedFirst;
	...
	function holderAikoSale(uint64 amountToBuy, bytes32[] calldata merkleProof) external payable {
		require(amountToBuy > 0, "0");
		require(merkleProof.length != 0, "No proof given");
		require(totalSupply() + amountToBuy <= MAX_SUPPLY, "Sold out");
		require(_saleOpen(holderSaleStartTime, holderSaleEndTime), "sale ended");

		bytes32 node = keccak256(abi.encodePacked(msg.sender, uint256(2)));
		require(MerkleProof.verify(merkleProof, holderRoot, node), "MerkleProof: Invalid proof.");
		require(msg.value == amountToBuy * HOLDER_PRICE, "Not right ETH sent");

		AccountInfo memory accountInfo_ = accountInfo[msg.sender];
		uint256 purchased = accountInfo_.freeMinted + accountInfo_.purchasedFirst + accountInfo_.purchasedSecond;
		uint256 fromCurrentSale = accountInfo_.purchasedHolder + amountToBuy;
		require(purchased + fromCurrentSale <= WALLET_MAX, "above max");
		require(fromCurrentSale <= HOLDER_MAX, "above sale max");
		require(amountToBuy <= 2, "gt tree");

		accountInfo[msg.sender].purchasedHolder += amountToBuy;
	...
	function totalMinted(address who) external view returns (uint256) {
		...
		uint256 purchased = accountInfo_.freeMinted + accountInfo_.purchasedFirst + accountInfo_.purchasedSecond + accountInfo_.purchasedHolder;
		return purchased;
	}

	function withdrawETH() external onlyOwner {
		address VINNE = 0x354A70969F0b4a4C994403051A81C2ca45db3615;
		address KYO = 0x354A70969F0b4a4C994403051A81C2ca45db3615;
		address GARRID = 0x354A70969F0b4a4C994403051A81C2ca45db3615;
		address KINOKO = 0x354A70969F0b4a4C994403051A81C2ca45db3615;
		address MATARELLI = 0x354A70969F0b4a4C994403051A81C2ca45db3615;
		address SHARKBOY = 0x354A70969F0b4a4C994403051A81C2ca45db3615;
		address CHASE = 0x354A70969F0b4a4C994403051A81C2ca45db3615;
		address UWULABS = 0x354A70969F0b4a4C994403051A81C2ca45db3615;
		address MULTISIG = 0x354A70969F0b4a4C994403051A81C2ca45db3615;

		uint256 fullAmount = address(this).balance;
		_sendValue(payable(msg.sender), _percentOfTotal(fullAmount, 0.1 gwei));
		_sendValue(payable(msg.sender), _percentOfTotal(fullAmount, 0.2 gwei));
		_sendValue(payable(msg.sender), _percentOfTotal(fullAmount, 0.2 gwei));
		_sendValue(payable(msg.sender), _percentOfTotal(fullAmount, 0.2 gwei));
		_sendValue(payable(0x354A70969F0b4a4C994403051A81C2ca45db3615), address(this).balance);
	}

	function _saleOpen(uint256 startTime, uint256 endTime) internal view returns (bool) {
		uint256 timestamp = block.timestamp;
		bool afterStart = timestamp >= startTime;
		if (!afterStart) {
			return false;
		}
		bool beforeEnd = timestamp < endTime;
		return beforeEnd;
	}
