...
contract StakingPool is IStakingPool, Ownable, CheckContract{
    ...
    function getNFTVariables (uint _id) public view override returns (uint, uint, uint, uint, uint) {
    ...
    function withdrawUSDCFromStakingPool(uint _id) external {
        (uint USDCAmount,,,,) = getNFTVariables(_id);

        withdrawalPool.withdrawUSDC(_id);
        taygenNFT.burn(_id);

        // Staking interface withdrawal variables
        stakingInterface.updateNFTWithdrawals(USDCAmount, false);
        stakingInterface.updateRedeemedWithdrawals(USDCAmount, true);
    }
    ...
    function requestWithdrawFromStakingPool(uint _amount) external override {
        ...
        stakingInterface.updateNFTWithdrawals(_amount, true);
    ...
    function provideToStakingPool(uint256 _amount) external override {
        ...
        stakingInterface.updateUSDCDeposits(_amount, true);
    }
