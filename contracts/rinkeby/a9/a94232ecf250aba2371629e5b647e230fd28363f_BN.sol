...
contract BN {
    ...
    function mul_BN(uint128[16] memory a, uint128[16] memory b) private pure returns(uint128[32] memory results){
    ...
    function addmod_BN(string[16] calldata a, string[16] calldata b, string[16] calldata n_square) public {
        uint128[16] memory temp_a;
        uint128[16] memory temp_b;
        uint128[16] memory temp_n2;
        for(uint i=0; i<16; i++) {
            temp_a[i] = stringToUint(a[i]);
            temp_b[i] = stringToUint(b[i]);
            temp_n2[i] = stringToUint(n_square[i]);
        }      
        uint128[32] memory c = mul_BN(temp_a, temp_b);
        mod_BN(c, temp_n2);
    }
