...
contract BlueBay is ERC721A, Ownable {
    ...
    string public notRevealedUri      =  "https://monaco.mypinata.cloud/ipfs/QmYsxS2UFwQP8EEQ91MFuVQGKYUWejnDzRWBWQsBpFM5aM/0.json";

    uint256 public remainTokenAmount  =  5000;
    uint256 public revealed           =  0;

    uint256 public startTime          =  0;
    uint256 public initPrice          =  1 ether;  // 1e17
    ...
    // uint256 public whitelistDiscountNum = 2;
    ...
    // mapping(uint256 => address) public addrOfTokenID;
    // mapping(uint256 => uint256) public priceOfTokenID;

    // mapping(address => uint256[]) public tokenIDsOfUser;
    mapping(address => uint256) public reservedNumOfWL;
    ...
    function reserve(uint256[] memory _tokenIds, bytes32[] memory _merkleProof) public payable {
        ...
        require(currentPrice() > 0 && currentPrice() < 2 ether, "Not suitable minting price");
        ...
        if (msg.sender != owner()) {
            ...
            if (MerkleProof.verify(_merkleProof, merkleRoot, sender) && reservedNumOfWL[msg.sender] < 3) {
                console.log("whitelisted user");
                uint256 temp = 3- reservedNumOfWL[msg.sender];

                if (_tokenIds.length <= temp) {
                    reservedNumOfWL[msg.sender] += _tokenIds.length;
                    require(msg.value >= (currentPrice() - 0.5 ether) * _tokenIds.length, "Insufficient funds");
                } else {
                    reservedNumOfWL[msg.sender] += temp;
                    require(msg.value >= (currentPrice() - 0.5 ether) * temp + currentPrice() * (_tokenIds.length - temp), "Insufficient funds");
                }
            } else {
                console.log("not whitelisted user");
                require(msg.value >= currentPrice() * _tokenIds.length, "Insufficient funds");
            }
        }
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            // addrOfTokenID[_tokenIds[i]] = msg.sender;
            // priceOfTokenID[_tokenIds[i]] = currentPrice();
            
            // tokenIDsOfUser[msg.sender].push(_tokenIds[i]);
            remainTokenAmount--;
        }

        // users[calledNum] = User(tokenIDs, msg.sender);
        // users[calledNum].userTokenIDs = tokenIDs;
    ...
    function currentPrice() public view returns (uint256) {
        ...
        if (startTime <= 0) {
            ...
            if (block.timestamp - startTime > 600 * 10) {
                price = 2 ether;
            } else {
                for (uint256 i = 0; i < 10; i++) {
                    if (block.timestamp - startTime > 600 * i && block.timestamp - startTime <= 600 * (i + 1)) {
                        price = initPrice + i * 1e18 / 10;
                    }
                }
            }
        }
    ...
    function setRevealedURI(string memory _RevealedURI) public onlyOwner {
        setReveal(1); setBaseURI(_RevealedURI);
    }

    // function setWhitelistDiscountNum(uint256 _whitelistDiscountNum) public onlyOwner {
    //     whitelistDiscountNum = _whitelistDiscountNum;
    // }
