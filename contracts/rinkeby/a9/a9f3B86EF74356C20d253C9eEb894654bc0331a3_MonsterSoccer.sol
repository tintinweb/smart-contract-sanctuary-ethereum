...
contract MonsterSoccer is ERC1155, Ownable {
    ...
    //Constants
    uint256 private constant COACH_POSITION = 10;
    uint256 private constant COACHASSISTANT_POSITION = 11;
    ...
    function BuyNFT(uint256 id) public {
        ...
        IERC20(_MSTTokenAddress).transferFrom(
            ...
            (_sellOrders[index].priceInMST - refferalShare) * (10**18)
        );
        IERC20(_MSTTokenAddress).transferFrom(
            msg.sender,
            address(this),
    ...
    function CalculatePlayWater(address playerAddress)
        ...
        returns (uint256)
    {
        return (CalculateBallReward(playerAddress) * _ballToWaterRatio) / 100;
    }
        ...
        returns (uint256)
    {
        ...
        for (
            ...
            if (
                ...
                ballCount += (
                    ((_mintPrices[
                        uint256(_playground[playerAddress][playgroundIndex]) - 1
                    ] * 10000) / 360)
                );
            }
        }
    ...
    function Play(address playerAddress) public {
    ...
    function PlayByCoaches() public {
        for (
            ...
            if (
                block.timestamp >=
                _addressToPlayers[_players[playersIndex]].nextAvailTime
            ) {
                bool doAction = false;
                if (GetCurrentPower(_players[playersIndex]) <= 0) {
                    continue;
                }
                if (
                    _playground[_players[playersIndex]][
                        uint256(COACH_POSITION)
                    ] !=
                    MONSTER_TYPE.NONE &&
                    _addressToPlayers[_players[playersIndex]].coachEnabled &&
                    _addressToPlayers[_players[playersIndex]].coachPlayCount > 0
                ) {
                    _addressToPlayers[_players[playersIndex]]
                        .coachEnabled = false;
                    _addressToPlayers[_players[playersIndex]].coachPlayCount--;
                    doAction = true;
                } else if (
                    _playground[_players[playersIndex]][
                ...
                if (doAction) {
                    address playerAddress = _addressToPlayers[
                        _players[playersIndex]
                    ].playerAddress;
                    uint256 newBalls = CalculateBallReward(playerAddress);
                    uint256 waterRequired = CalculatePlayWater(playerAddress);
                    if (
                        _addressToPlayers[_players[playersIndex]].waterCount <
                        waterRequired
                    ) {
                        continue;
                    }
                    ...
                    _addressToPlayers[playerAddress].nextAvailTime =
                        block.timestamp +
                        (_nextPlayPeriod * 60);
                    _addressToPlayers[playerAddress].ballCount += newBalls;
                    _addressToPlayers[playerAddress]
                        .waterCount -= waterRequired;
                    emit PlayHistory(playerAddress, newBalls);
                }
            }
        }
    }
    ...
    function Withdrawal(uint256 amount) public onlyOwner {
        if (amount == 0) {
            ...
            IERC20(_MSTTokenAddress).transfer(msg.sender, amount);
        }
    }
    ...
    function UpdateConfigs(
        ...
        if (stakePeriod > 0) {
            _stakePeriod = stakePeriod * 86400;
        }
