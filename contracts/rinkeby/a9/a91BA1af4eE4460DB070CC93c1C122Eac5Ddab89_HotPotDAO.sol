...
contract HotPotDAO is Ownable, ERC721A, MerkleWhitelist, ReentrancyGuard, VRFConsumerBase, BatchReveal {
    ...
    string public UNREVEALED_URI = "ipfs://QmdFmPdarpg5JHkV5Dj8P7oCHhjba2zfWMA2fZKZ5qzbJU";  //TODO: set this, needs to point to metadata, not the image, hardcoded (saves gas) here with no way to update the unrevealed URI later
    ...
    //uint16 public COLLECTION_SIZE = 888;  //defined in BatchReveal
    uint256 public MINT_PRICE = 0.00 ether;
    uint16 public MAX_BATCH_SIZE = 1;
    uint16 public MAX_WL_BATCH_SIZE = 1;

    //VRF constants https://docs.chain.link/docs/vrf-contracts/v1/  //TODO migrate to vrf-v2
        ...
        onlyPublicWhitelist(proof)
    {
        ...
        require(!userToHasMintedPublicWL[msg.sender], "Can only mint once during public WL!");   
    ...
    // RANDOMIZATION
    // Can be made callable by everyone but restricting to onlyOwner for extra security
    // batchNumber belongs to [0, COLLECTION_SIZE/REVEAL_BATCH_SIZE]
    // if fee is incorrect chainlink's coordinator will just revert the tx so it's good
    function requestRandomSeed(uint s_fee) public onlyOwner returns (bytes32 requestId) {
...
abstract contract BatchReveal {
    uint constant public COLLECTION_SIZE = 12;
