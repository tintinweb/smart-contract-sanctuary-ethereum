...
contract CyberRoos is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public saleState; // 0 = Sale inactive, 1 = BOXING active, 2 = WL sale, 3 = WL + public sale
    ...
    function boxForRoo(uint256 _attack) external returns(uint256) {
        ...
        require(balanceOf(_msgSender()) > 0, "You don't have a Roo");
        ...
        uint256 _outcome = _getRandom((_attack + _totalMinted), bounds);
        uint96 _currentTime = uint96(block.timestamp);
        if(_outcome < _totalMinted) {
            _mint(ownerOf(_outcome), 1);
    ...
    */
    ...
    function _chooseTokenWeightedSequentially() internal view returns(uint256) {
        ...
        uint256 _attemptCount;
        unchecked {
            uint256 _totalMinted = _totalMinted();
            uint256 rand1 = _getRandom(_attemptCount, _totalMinted);
            uint256 rand2 = _getRandom((_attemptCount + rand1), _totalMinted);
            if(rand1 > rand2) {
    ...
    function addController(address controller) external onlyOwner {
        gameContracts[controller] = true;
    }

    /**
     * disables a game contract
     * @param controller the address to disable
     */
    function removeController(address controller) external onlyOwner {
        gameContracts[controller] = false;
    }
