...
contract Wormhole is Ownable {
    ...
    function joinEvent(
        ...
        uint256 totalCheeth = (babiesIds.length + genesisIds.length) * 200;
    ...
    function _getEventReward(uint64 id, address wallet) internal view returns (uint64) {
        ...
        require(_getIsEventUnclaimed(id, wallet), "already claimed");
        ...
        for (uint256 index = 0; index < eventGenesis.length; index++) {
            ...
            if (_ownerOfToken(mouseId, babiesAddress) != wallet) continue;
    ...
    function _getIsEventUnclaimed(uint64 id, address player) internal view returns (bool) {
