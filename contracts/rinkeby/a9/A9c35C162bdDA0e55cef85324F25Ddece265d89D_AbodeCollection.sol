...
contract AbodeCollection is ERC721, ERC721URIStorage, Pausable,
    ...
    function isAddressWhitelisted(address _member) public view returns (bool) {
        return whitelistAddresses[_member];
    }

    function addWhitelistAddresses(address[] calldata _addresses) external onlyRole(VALIDATE_ADDRESSES_ROLE){
        for (uint i = 0; i < _addresses.length; i++) {
            for(uint j = 0; j < ownerTokens[_addresses[i]].length; j++) {
                _setTokenURI(ownerTokens[_addresses[i]][j], "tokenuri");
            }
     ...
     */
    function withdrawMoneyTo(address to) public onlyRole(DEFAULT_ADMIN_ROLE) {
