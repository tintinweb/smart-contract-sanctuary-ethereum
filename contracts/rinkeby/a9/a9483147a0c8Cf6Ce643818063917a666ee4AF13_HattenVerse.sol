...
contract HattenVerse is ERC1155, Ownable, Pausable, IERC2981, ERC1155Burnable {
    string public name;
    ...
    constructor()
        ERC1155("ipfs://QmUh1zh2sbQz19C86wV6akD1yFBbn8pGzYj4aoMVjZyABk/")
    {
        name = "Hatten Verse";
        symbol = "HTV";
        _recipient = owner();
    ...
    uint256 internal constant TOKEN_ID = 0; // Token ID
    uint256 public constant maxSupply = 999; // Total token qty supply
    uint256 public mintPrice = 100000000000000000; // Price for mint the token (wei)
    uint256 public mintCount = 0; // Current minted token qty
    uint256 public maxPerMint = 5; // Only can mint 6 per time
    ...
    function mintTheVerse() public payable {
        uint256 tokenQty = 1;
        require(
            mintCount + tokenQty <= maxSupply,
            "Sale would exceed max supply"
        );
        require(tokenQty * mintPrice <= msg.value, "Not enough ether sent");
        require(tokenQty <= maxPerMint, "Max mint qty is 5");
        if (msg.sender != owner()) {
            require(whitelisted[msg.sender] == true, "Not in whitelist");
            require(
    ...
    function _mintTheVerse() internal {
        _mint(msg.sender, TOKEN_ID, 1, "0x00");
    }

    function purchaseRemaining() public view virtual returns (uint256) {
    ...
    {
        tokenId = tokenId;
        return (_recipient, (_salePrice * 800) / 10000);
    }
