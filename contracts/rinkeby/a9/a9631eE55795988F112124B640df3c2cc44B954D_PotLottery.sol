...
contract PotLottery is Initializable, ReentrancyGuardUpgradeable {
    ...
    function EnterPot(
        ...
        if ((potLiveTime + potDuration + 7) <= block.timestamp && (participantCount > 1)) {
            calculateWinner();
        }
    ...
    function getAmountToPayAsFees(address _address) internal view returns (uint256) {
        ...
        return ((userBalance / PLSP_Standard) >= 1) ? baseFee/2 : (baseFee - ((userBalance / PLSP_Standard) * baseFee)/2);
    }

    bool public isPriceUpdated = false;
    function UpdatePrice() internal {
        IUniswapV2Factory factory = IUniswapV2Factory(uniswapV2FactoryAddr);
        for (uint256 index = 0; index < tokenWhiteListNames.length; index++) {

            IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair((tokenWhiteList[tokenWhiteListNames[index]]).tokenAddress, usdtAddr));
            IPRC20 wethToken = IPRC20(pair.token0());

            (uint256 Res0, uint256 Res1, ) = pair.getReserves();
            uint256 res1 = Res1 * (10**wethToken.decimals());
            uint256 price = res1 / Res0;
            updateTokenUsdValue(tokenWhiteListNames[index], price * 10**10 / (10**(tokenWhiteList[tokenWhiteListNames[index]]).tokenDecimal));
        }
        isPriceUpdated = true;
    }
