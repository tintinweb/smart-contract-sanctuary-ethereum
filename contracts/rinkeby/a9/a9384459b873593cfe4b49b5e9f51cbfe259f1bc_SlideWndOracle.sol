...
contract SlideWndOracle is Ownable{
    using FixedPoint for *;
    using SafeMath for uint;
    struct Observation {
        uint timestamp;
    ...
    function setBtchDayPrice(uint256[] calldata timestamps,uint256[] calldata dayprices) external onlyOwner{
        ...
        for(;i<timestamps.length;i++) {
            ...
            require(sushiBtchDayPrice[dayidx]==0,"bad input time");
    ...
    function setBtc365AvgPrice(uint256[] calldata timestamps,uint256[] calldata dayprices) external onlyOwner{
        ...
        for(;i<timestamps.length;i++) {
            uint256 dayidx = timestamps[i]/daySeconds;
            require(uiniAvgBtc365Price[dayidx]==0,"bad 3b5 avg time");
            uiniAvgBtc365Price[dayidx] = dayprices[i];
        }
    }

    //btch realtime price in wbtc
    function btch2BtcCurPrice() public view returns (uint256){
        (uint256 btcAmount/*reserveA*/,uint256 usdcAmount/*reserveB*/)= UniswapV2Library.getReserves(susiFactory, wbtc/*tokenA*/, usdt/*tokenB*/);
        if(btcAmount==0) {
            return 0;
        }

        //uint256 btcprice = ((usdcamount/usdcDecimal)*priceDecimal)/(btcAmount/btcDecimal)
        uint256 btcprice = (priceDecimal.mul(usdcAmount).mul(btcDecimal)/btcAmount)/usdcDecimal;
        uint256 btchAmount;
        (btchAmount/*reserveA*/,btcAmount/*reserveB*/) = UniswapV2Library.getReserves(susiFactory, btch/*tokenA*/, wbtc/*tokenB*/);
        if(btchAmount==0) {
            return 0;
        }

       // return priceDecimal*(btcAmount/btcDecimal)/(btchAmount/btchDecimal));
        uint256 btchPrice = (priceDecimal.mul(btcAmount).mul(btchDecimal)/btchAmount)/btcDecimal;

        return priceDecimal.mul(btchPrice)/btcprice;
    }

    //btch avg price(24 hours) in wbtc
    function btch2BtcDayPrice(uint256 timestamp) public view returns (uint256 ){
        uint256 dayidx = timestamp/daySeconds;
        return sushiBtchDayPrice[dayidx];
    }

    //btch realtime price in wbtc
    function btc2UsdCurPrice() public view returns (uint256){
        (uint256 btcAmount/*reserveA*/,uint256 usdcAmount/*reserveB*/)= UniswapV2Library.getReserves(susiFactory, wbtc/*tokenA*/, usdt/*tokenB*/);
        if(btcAmount==0) {
            return 0;
        }
        //uint256 btcprice = ((usdcamount/usdcDecimal)*priceDecimal)/(btcAmount/btcDecimal)
        return (priceDecimal.mul(usdcAmount).mul(btcDecimal)/btcAmount)/usdcDecimal;
    }

    //btc avg price(24 hours) in wbtc
    function btc2UsdDayPrice(uint256 timestamp) public view returns (uint256 ){
        uint256 dayidx = timestamp/daySeconds;
        return uiniAvgBtcDayPrice[dayidx];
    }

    //
    function btc2Usd365DayAvgPrice(uint256 timestamp) public view returns (uint256) {
         uint256 dayidx = timestamp/daySeconds;
    ...
    function getFirstObservationInWindow(address pair) private view returns (Observation storage firstObservation) {
        ...
        firstObservation = pairObservations[pair][dayidx][firstObservationIndex];
    }

    function needUpdate() public view returns (bool) {
        uint256 dayidx = block.timestamp/daySeconds;
        return  uiniAvgBtc365Price[dayidx]==0;
    }
    ...
    function updateOberverVation(address factory,address tokenA, address tokenB) private {
        ...
        if(pairObservations[pair][dayidx].length==0) {
            for (uint i = 0; i < granularity; i++) {
                pairObservations[pair][dayidx].push();
            }
        }

        // get the observation for the current period
        uint8 observationIndex = observationIndexOf(block.timestamp);
        Observation storage observation = pairObservations[pair][dayidx][observationIndex];
        //if the data is updated already, then return;
    ...
    function computeAvgPrice (
        ...
        return priceAverage.mul(1).decode144();
        //amountOut = priceAverage.mul(amountIn).decode144();
    }

    function calDayPrice(address factory,address tokenIn,address tokenOut) private {
        uint256 dayidx =  block.timestamp/daySeconds;
        if( sushiBtchDayPrice[dayidx]!=0
            ||uiniAvgBtcDayPrice[dayidx]!=0)
        {
            return;
        }
       ...
       // (address token0,) = UniswapV2Library.sortTokens(tokenIn, tokenOut);
        ...
        if(factory==uniFactory) {
            uiniAvgBtcDayPrice[dayidx] = computeAvgPrice(firstObservation.price0Cumulative, price0Cumulative, timeElapsed);
        } else {
            sushiBtchDayPrice[dayidx] = computeAvgPrice(firstObservation.price0Cumulative, price0Cumulative, timeElapsed);
        }
    ...
    function caluiniAvgBtc365Price() private {
        ...
        if(uiniAvgBtc365Price[dayidx]!=0
           ||uiniAvgBtcDayPrice[dayidx]==0) {
            return;
        }
        uint256 daycnt = 0;
