...
/*
               ...
               _______|[email protected]@@@@[email protected]@@@@sssss|s
             /         [email protected]@sssssssss|s
...
contract TipsyTest is ERC721A, Ownable {
    uint256 public NUM_FREE_MINTS = 5;
    ...
    uint256 public MAX_WHITELIST_TOKENS = 5;
    uint256 public MAX_WHITELIST_PER_WALLET = 3;
    ...
    constructor() ERC721A("Tipsy Test", "TIPSY") {}
    ...
    function whitelistMint(uint256 quantity, bytes32[] calldata merkleProof) external payable isValidMerkleProof(merkleProof, whitelistMerkleRoot) {
        require(isWhitelistSaleOpen, "Minting has not started yet");
        require(totalSupply() + quantity <= maxSupply, "Not enough tokens left");
        require(whitelistMinted + quantity <= MAX_WHITELIST_TOKENS, "Not enough whitelist tokens left");
        require(quantity <= MAX_PER_TX, "Cannot mint more than max mint per transaction");
        require(whitelistMintCounts[msg.sender] + quantity <= MAX_WHITELIST_PER_WALLET,
            "Reached whitelist mints limit on the wallet");

        uint256 requiredValue = quantity * whitelistMintPrice;
        uint256 userFreeMinted = freeMintCounts[msg.sender];
        uint256 freemintAmount;

        if (userFreeMinted < MAX_FREE_PER_WALLET && (freemintedAmount + quantity) <= NUM_FREE_MINTS) {
            freemintAmount = min((MAX_FREE_PER_WALLET - userFreeMinted), quantity);

            requiredValue = quantity <= (MAX_FREE_PER_WALLET - userFreeMinted)
                ? 0
                : requiredValue - (whitelistMintPrice * freemintAmount);
        }

        freemintedAmount += freemintAmount;
        freeMintCounts[msg.sender] += freemintAmount;

        require(msg.value >= requiredValue, "Insufficient funds!");

        _safeMint(msg.sender, quantity);
        whitelistMintCounts[msg.sender] += quantity;
        whitelistMinted++;
    }

    function publicMint(uint256 quantity) external payable {
        require(isPublicSaleOpen, "Minting has not started yet");
        require(totalSupply() + quantity <= maxSupply, "Not enough tokens left");
        require(quantity <= MAX_PER_TX, "Cannot mint more than max mint per transaction");
        require(publicMintCounts[msg.sender] + quantity <= MAX_PUBLIC_PER_WALLET,
            "Reached public mints limit on the wallet");

        uint256 requiredValue = quantity * publicMintPrice;
        uint256 userFreeMinted = freeMintCounts[msg.sender];
        uint256 freemintAmount;
        ...
        if (userFreeMinted < MAX_FREE_PER_WALLET && (freemintedAmount + quantity) <= NUM_FREE_MINTS) {
            ...
            requiredValue = quantity <= (MAX_FREE_PER_WALLET - userFreeMinted)
                ? 0
                : requiredValue - (publicMintPrice * freemintAmount);
        }
    ...
    function setHiddenURI(string memory hiddenURI_) public onlyOwner {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory _hiddenURI = hiddenURI;
        if (revealed) {
            return bytes(_baseURI).length != 0 ? string(abi.encodePacked(_baseURI, Strings.toString(tokenId), ".json")) : '';
        } else {
            return bytes(_hiddenURI).length != 0 ? string(abi.encodePacked(_hiddenURI, Strings.toString(tokenId), ".json")) : '';
        }
    }
