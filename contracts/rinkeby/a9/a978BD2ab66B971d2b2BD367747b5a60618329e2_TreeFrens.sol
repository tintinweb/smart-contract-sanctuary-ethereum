...
contract TreeFrens is ERC721, Ownable {
    ...
    uint256 public whitelistSaleCount = 1; 
    ...
    constructor() ERC721("Tree Frens", "NUTS") {
        setHiddenURI("ipfs://___CID___/hidden.json"); 
    ...
    function totalFrens() public view returns (uint256) {
    ...
    function whitelistMint(uint256 _frenAmount, uint8 _maxAllowed, bytes32[] calldata _merkleProof) public payable whitelistHelper(_frenAmount){
        require(!paused && !frenpassSale && !publicMint && whitelistSale, "Whitelist Sale not active yet");
        require(_verifyPresaleStatus(msg.sender, _maxAllowed, _merkleProof), "Address not on whitelist");
        require(msg.value >= price * _frenAmount, "Please send more ETH");
    ...
    function _verifyPresaleStatus(address _account, uint8 _maxAllowed, bytes32[] calldata _merkleProof) private view returns (bool) {
