...
// File: contracts/junglefreaks/Jungle.sol
...
contract Jungle is ERC20Burnable, ERC1155Holder, Ownable, ReentrancyGuard {
    uint256 public constant MAX_SUPPLY = 184200000 ether;
    uint256 public constant EMISSION_RATE = 86400 ether / 2;
    uint256 public constant EMISSION_RATE_COTF = 86400 ether;
    uint256 public constant EMISSION_RATE_MTFM = 86400 ether * 2;
    address public constant JUNGLE_FREAKS_ADDRESS = 0x90A9a4ea9cDC0f57eB7fa468D01A93e2c6E33731; //0x9712228cEeDA1E2dDdE52Cd5100B88986d1Cb49c;
    address public constant LEGENDARY_VAULT_ADDRESS = 0x2bc4113e327FE17c854cD47ACfc1cFAA9Bb2A923; //0x495f947276749Ce646f68AC8c248420045cb7b5e;
    ...
    mapping(address => bool) public tokenControllers;
    ...
    modifier onlyTokenController {
        require(tokenControllers[msg.sender], "NOT_CONTROLLER");
        _;
    }
    ...
    function getAllRewards(address staker) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            totalRewards += (block.timestamp - timeStaked[tokens[i]]) * EMISSION_RATE / 86400;
        }
    ...
    function getLegendariesRewards(address staker) public view returns (uint256) {
        ...
        totalRewards += (((block.timestamp - stakingState.cotfLastStaked) * stakingState.cotfStaked) + stakingState.cotfAccumulatedTime) * EMISSION_RATE_COTF / 86400;
        totalRewards += (((block.timestamp - stakingState.mtfmLastStaked) * stakingState.mtfmStaked) + stakingState.mtfmAccumulatedTime) * EMISSION_RATE_MTFM / 86400;
    ...
    function stakeLegendaries(uint8 cotf, uint8 mtfm) external stakingEnabled nonReentrant {
        ...
        if (cotf > 0) {
            stakingState.cotfAccumulatedTime += (uint32(block.timestamp) - stakingState.cotfLastStaked) * stakingState.cotfStaked;
            stakingState.cotfLastStaked = uint32(block.timestamp);
        ...
        if (mtfm > 0) {
            stakingState.mtfmAccumulatedTime += (uint32(block.timestamp) - stakingState.mtfmLastStaked) * stakingState.mtfmStaked;
            stakingState.mtfmLastStaked = uint32(block.timestamp);
    ...
    function unstakeLegendaries(uint8 cotf, uint8 mtfm) external stakingEnabled nonReentrant {
    ...
    function claimLegendaries() external stakingEnabled nonReentrant {
        LegendariesStaked storage stakingState = legendariesStaked[msg.sender];
        uint256 totalRewards;
    ...
    function unstakeByIds(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            totalRewards += (block.timestamp - timeStaked[id]) * EMISSION_RATE / 86400;
    ...
    function setController(address controller, bool set) external onlyOwner {
        tokenControllers[controller] = set;
    }
