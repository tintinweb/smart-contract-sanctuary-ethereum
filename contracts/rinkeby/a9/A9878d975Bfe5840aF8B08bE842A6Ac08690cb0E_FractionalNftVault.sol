    ...
    IFractionalNftVault
{
    ...
    function initialize(address _nft, uint256 _fee, address _liquidityWallet, uint256 _holdoutPeriod) external initializer {
        __Ownable_init();
        __ERC721Holder_init();
        nftContract = _nft;
        fee = _fee;       // 25 ---> 2.5 %
    ...
    function buyShare(
        ...
        require(
            _totalAmount > 0,
            "FractionalNftVault: zero address"
        );

        // implement decimals
        uint256 amount = (_totalAmount / item.unitPrice) * 10 ** 18;
        ...
        Shares(item.shareToken).transfer(msg.sender, amount);
    ...
    function getPaymentToken(uint16 paymentIndx) external view returns(address) {
        return paymentToken[paymentIndx];
    }
}
...
contract Shares is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        _mint(liquidityWallet, (supply * _liquidityShare * 10e17) / 1000);
        _mint(msg.sender, (supply * (1000 - _liquidityShare * 10e17)) / 1000);
    }
        ...
        private
    {
        ...
        if (msgSender == ownerAddress || msgSender == platformAddress || msgSender == liquidityWallet) {
            super._transfer(msgSender, recipient, amount);
        } else {
            ...
            super._transfer(msgSender, ownerAddress, _ownerFee);
            super._transfer(msgSender, platformAddress, _platformFee);
            super._transfer(msgSender, recipient, _amount);
        }
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        if (from == ownerAddress || from == platformAddress || to == liquidityWallet || to == platformAddress) {
