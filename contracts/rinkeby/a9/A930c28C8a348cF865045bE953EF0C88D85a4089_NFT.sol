...
contract NFT is ERC721 , Ownable , ReentrancyGuard{
    ...
    */
    ...
    function mintForAll(uint256 _amount) external payable nonReentrant {
        require (_amount <= maxPerMint, "Cant mint this much tokens");
  ...
  function setPostUri (string calldata _postURI) external onlyOwner {
    post_uri = _postURI;
  }

  function tokenURI(uint256 tokenId) public view override returns (string memory)
  {
      string memory _post_uri = string(abi.encodePacked(base_uri ,'/', Strings.toString(tokenId)));
      return _post_uri ;
