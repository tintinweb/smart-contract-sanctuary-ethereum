...
contract Furychain is Ownable, ERC721A, PaymentSplitter {
    ...
    uint public pricePresale = 0.00003 ether;
    uint public priceSale = 0.00005 ether;

    bytes32 public merkleRoot;

    uint public saleStartTime = 1647532030;
    ...
    constructor(string memory _notRevealedURI, string memory _baseURI, bytes32 _merkleRoot) ERC721A("Nom de la collection", "SYMBOL") PaymentSplitter(_team, _teamShares) {
    ...
    function presaleMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(currentTime() < saleStartTime + 500 minutes, "Whitelist Sale is finished");
        require(sellingStep == Step.WhitelistSale, "Whitelist sale is not activated");
        require(isWhiteListed(msg.sender, _proof), "Not whitelisted");
        require(amountNFTsperWalletWhitelistSale[msg.sender] + _quantity <= max_mint_presale, "You can only get 4 NFT on the Whitelist Sale");
