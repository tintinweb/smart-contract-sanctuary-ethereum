    ...
    AdminManagerUpgradable
{
    ...
    function burn(address account, uint256 strainId) external onlyAdmin {
        require(ownerOf(strainId, 1) == account, "Not the owner");
        require(
            _stakersData[msg.sender].pendingClaims.nodeToValue[strainId] <=
                block.timestamp,
            "Strain not fully spent"
        );
        // It should accrue only required RAKS unit test that
    ...
    function setBredStrainToken(address strainTokenAddress) external onlyAdmin {
        _bredStrainToken = IBredStrain(strainTokenAddress);
    }
