...
contract DerangedApes is ERC721A, Ownable, ReentrancyGuard, IERC1155Receiver {
    ...
    Derangements[9998] private derangementData;
    ...
    function mutateSODA(address from, uint id, uint[] calldata flavors) public nonReentrant {
        if(soda.ownerOf(id) != from) {
            revert NotOwnerOfSODA();
        }

        Derangements memory dr = derangementData[id];
        if(dr.mutationCt >= 2) {
            revert MaxMutationsUsed();
        }

        uint[] memory burnArr = new uint[](flavors.length);
        for(uint i = 0; i < flavors.length; i++) {
            uint32 f = uint32(flavors[i]);
            if(getBit(dr.flavorsUsed, f) != 0) {
                revert FlavorAlreadyUsed();
            }
            burnArr[i] = 1;
            dr.flavorsUsed = setBit(dr.flavorsUsed, f);
        }
        _safeMint(from, 1);

        dr.mutantIds[dr.mutationCt] = uint32(totalSupply()-1);
        dr.mutationCt++;
        derangementData[id] = dr;
    }
    ...
    function timesMutated(uint sodaId) external view returns (uint) {
        return uint(derangementData[sodaId].mutationCt);
    }

    function flavorsUsed(uint sodaId) external view returns (uint[] memory) {
        Derangements memory data = derangementData[sodaId];
        if(data.mutationCt == 0) {
            return new uint[](0);
        }

        uint flavorCt;
        for(uint i=0; i<7;i++) {
            if(getBit(data.flavorsUsed, uint32(i))==1) {
                flavorCt++;
            }
        }
    ...
    function getDerangedIds(uint sodaId) external view returns (uint[] memory) {
        uint[] memory arr = new uint[](derangementData[sodaId].mutationCt);
        for(uint i=0; i<arr.length;i++) {
            arr[i] = uint(derangementData[sodaId].mutantIds[i]);
        }
        return arr;
    }
