...
import './MCGenesisSplitsTestnet.sol';

/**
 * @title MCIncoGnito wrapper contract
 */
contract MCIncoGnito is MCGenesisSplitsTestnet, MCGenesisBase {
    constructor()
        MCGenesisBase(
            'MCIncoGnito',
            'MCIG',
            'https://nftculture.mypinata.cloud/ipfs/QmXJUb2XrSfdYXqW4rS3CmJJnYwZTYFcC4H6QBHYyi4zSc/', // Rinkeby V6, start 0, no extension
            addresses,
            splits
        )
    {
        ...
        // Beta Release: Contract Address = 0xdB118e19bE1b73a016Ce738DbfdfA08ca48f7B45
        // Final Rinkeby Release: Contract Address = 0xe5aD317aFA4500B38E1B46dC3C69FD4aD38aa62F   V1
        // Final Rinkeby Release: Contract Address = 0xeE6Ca286328EC56Cb2ef4C553e51FBd7f4B38a6E   V2
        // Final Rinkeby Release: Contract Address = 0x20d89c2ba8d9a4cc067caacd978bd2c84e3dc8b1   V3
        // Final Rinkeby Release: Contract Address = 0x03965226Bb3B4AFEc7Afa976286323B6d4EF2000   V4 
        // Final Rinkeby Release: Contract Address = 0xFAB181225B3fF9376B4dB0370d32B00a74539132   V5 - token0, noobpunk pr meta v2, no json ext
    }
    ...
    AuxHelper32
{
    ...
    function auxMintValues(address wallet) external view returns (uint32 presalePurchases, uint32 publicMintPurchases) {
        // Unpack single value from _getAux() to determine presalePurchases and publicMintPurchases
        return _unpack32(_getAux(wallet));
    }

    function getPresaleTokensPurchased(address wallet) external view returns (uint32) {
    ...
    function presaleTokens(bytes32[] calldata proof, uint256 count) external payable nonReentrant isPresale {
        ...
        (uint32 presalePurchases, uint32 publicMintPurchases) = _unpack32(_getAux(msg.sender));

        if (presalePurchases + count > AVAILABLE_PRESALE_QUANTITY) revert ExceedsAvailablePresaleMints();

        _presaleTokens(msg.sender, proof, count);

        _setAux(msg.sender, _pack32(uint32(presalePurchases + count), publicMintPurchases));
    }
    ...
    function mintTokens(uint256 count) external payable nonReentrant onlyUsers isPublicMinting {
        ...
        if (publicMintPurchases + count > AVAILABLE_PUBLIC_QUANTITY) revert ExceedsAvailablePublicMints();

        _internalMintTokens(msg.sender, count);

        _setAux(msg.sender, _pack32(presalePurchases, uint32(publicMintPurchases + count)));
    }
...
contract MCGenesisSplitsTestnet {

    address[] internal addresses = [
        0xc7377DE253149BF75f89c03BF52F953989bBe5E8 // Rinkeby
    ];
