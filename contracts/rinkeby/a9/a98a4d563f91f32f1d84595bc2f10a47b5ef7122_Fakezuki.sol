...
contract Fakezuki is Ownable, ERC721A, ReentrancyGuard {
  uint256 public immutable maxPerAddressDuringMint;       // 每个钱包的最大Mint数量
  uint256 public immutable maxPerAllowlistAddressMint;    // 白名单最大允许Mint数量
  uint256 public immutable amountForDevs;                 // 提供给开发者的最大数量
  uint256 public immutable amountForAllowlist;            // 提供给白名单的最大数量
  bytes32 private merkleRoot;                             // 白名单地址的Merkle树根

  struct SaleConfig {
    uint32 allowlistSaleStartTime;     // 白名单销售开始时间
    uint32 publicSaleStartTime;        // 公售开始时间
    uint64 allowlistPrice;             // 白名单Mint价格
  ...
  ) ERC721A("CodeCat", "CODECAT", maxBatchSize_, collectionSize_) {
  ...
  function allowlistMint(uint256 quantity, bytes32[] calldata _merkleProof) external payable callerIsUser {
    ...
    require(
      numberMinted(msg.sender) + quantity <= maxPerAllowlistAddressMint,
  ...
  // 设置公售和白单Mint配置
  function setPublicAndAllolistSaleInfo(
  ...
  // 设置白单销售价格
  function setAllowlistSalePrice(uint64 allowlistPriceWei) external onlyOwner {
