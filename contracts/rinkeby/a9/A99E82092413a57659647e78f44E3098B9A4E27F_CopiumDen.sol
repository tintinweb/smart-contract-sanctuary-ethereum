...
//    using ECDSA for bytes32;
    ...
    address public immutable royaltyReceiverAddress;
...
//    mapping(uint256 => mapping(uint256 => address)) public offerAddressPosMap;
//    mapping(uint256 => mapping(address => uint256)) public offerAddressRevMap;
//    mapping(uint256 => uint256) public offerCounts;
    struct OfferData {
        address addr;
        uint256 offer;
    }
//    mapping(uint256 => mapping(uint256 => OfferData)) public offers;
//    mapping(uint256 => mapping(address => uint256)) public offerAddressMap;
//    mapping(uint256 => uint256) public offerCounts;
    mapping(uint256 => mapping(address => uint256)) public offers;
    ...
    //    function _isPublicMint() internal view virtual {
...
//            balances[payees[i]] = 0;
        ...
        storageLayer = new ERC721Storage(_name, _symbol, _maxAllowedMints);
    }
    ...
    function preMint(uint amount, address to) public onlyOwner {
        require(mintStatus == MintStatus.PreMint && (totalSupply() + amount <= maxPossibleSupply), "s/m");
        storageLayer._safeMint(msg.sender, msg.sender, to, amount);
    }
    ...
    function changeMintStatus(MintStatus _status) external onlyOwner {
        require(_status != MintStatus.PreMint && mintStatus != MintStatus.NotStarted);
        mintStatus = _status;
    }
    ...
    function giftMint(uint amount, address to) public payable {
        ...
        //        require(totalMintsPerAddress[to] + amount <= maxAllowedMints, "l");
    ...
    function _mint(uint _amount, address _from, address _to) internal {
        ...
        require(totalMintsPerAddress[_to] + _amount <= maxAllowedMints, "l");
        //////////
        if (currency == wrappedNativeCoinAddress) {
            ...
            _currency.transferFrom(msg.sender, address(this), _amount * mintPrice);
        }
        ...
        totalMintsPerAddress[msg.sender] = totalMintsPerAddress[msg.sender] + _amount;
        storageLayer._safeMint(msg.sender, _from, _to, _amount);
...
//
//        if (currentOffer == 0) {
//            offerAddressPosMap[tokenId][offerCounts[tokenId]] = msg.sender;
//            offerAddressRevMap[tokenId][msg.sender] = offerCounts[tokenId];
...
//    }
//
//    function mkt_manageOfferInternal(uint256 tokenId, uint256 currentOffer, address offerer) private {
//        offers[tokenId][offerer] = 0;
//        offerAddressPosMap[tokenId][offerAddressRevMap[tokenId][offerer]] = offerAddressPosMap[tokenId][offerCounts[tokenId] - 1];
//        offerAddressPosMap[tokenId][offerCounts[tokenId] - 1] = address(0);
//        offerCounts[tokenId] -= 1;
//        offerAddressRevMap[tokenId][offerer] = 0;
//
//        offers[tokenId][offerer] = 0;
//        payable(msg.sender).transfer(currentOffer);
//    }
//
//    function MKT_cancelOffer(uint256 tokenId) public {
//        uint256 currentOffer = offers[tokenId][msg.sender];
//        require(ownerOf(tokenId) != msg.sender && currentOffer != 0, "io/odne");
//        require(offerAddressPosMap[tokenId][offerAddressRevMap[tokenId][msg.sender]] == msg.sender, "nso");
//
//        mkt_manageOfferInternal(tokenId, currentOffer, msg.sender);
//    }
//
//    function MKT_acceptOffer(uint256 tokenId, address offerer, uint256 price) public {
//        uint256 currentOffer = offers[tokenId][offerer];
//        require(ownerOf(tokenId) == msg.sender && currentOffer != 0 && currentOffer == price, "mbo/odne/pm");
//
//        mkt_manageOfferInternal(tokenId, currentOffer, offerer);
//
//        listings[tokenId][msg.sender] = 0;
//        storageLayer.transferBySale(msg.sender, offerer, tokenId);
//    }
//
////    function MKT_approveMarketplace() public {
...
//            OfferData memory od = OfferData({addr: offerAddressPosMap[tokenId][i], offer: offers[tokenId][offerAddressPosMap[tokenId][i]]});
//            offerList[i] = od;
//        }
//
//        return offerList;
...
//
//        for (uint i = 0; i < offerCounts[tokenId]; i++) {
//            if (offers[tokenId][offerAddressPosMap[tokenId][i]] > highestOffer) {
//                highestOffer = offers[tokenId][offerAddressPosMap[tokenId][i]];
//                highestOfferAddress = offerAddressPosMap[tokenId][i];
//            }
//        }
//
//        return OfferData({addr: highestOfferAddress, offer: highestOffer});
//    }
...
//        payForSale(currentOffer, msg.sender);
    ...
    function MKT_cancelOffer(uint256 tokenId, uint256 position) public {
        uint256 currentOffer = offers[tokenId][msg.sender];
        require(ownerOf(tokenId) != msg.sender && currentOffer != 0, "io/odne");
        require(offerAddressPositions[tokenId][position] == msg.sender, "nso");
    ...
    function MKT_acceptOffer(uint256 tokenId, address offerer, uint256 price, uint256 position) public {
        ...
        require(ownerOf(tokenId) == msg.sender && currentOffer != 0
            && currentOffer == price && msg.sender == offerAddressPositions[tokenId][position], "mbo/odne/wp1/wp2");
...
//        payForSale(currentOffer, msg.sender);
    ...
    function MKT_getOffers(uint256 tokenId) public view returns (OfferData[] memory) {
        ...
        for (uint i = 0; i < offerCounts[tokenId]; i++) {
            ...
            OfferData memory od = OfferData({addr: addr_i, offer: offers[tokenId][addr_i]});
            offerList[i] = od;
        }
...
//        payable(royaltyReceiverAddress).transfer(royaltyPayment);
        payable(paymentReceiver).transfer(paymentValue - royaltyPayment);
    }
...
//        ERC721TopLevel(msg.sender).deList(tokenId);
