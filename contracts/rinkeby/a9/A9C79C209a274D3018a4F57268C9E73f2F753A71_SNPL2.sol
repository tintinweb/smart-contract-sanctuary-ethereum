...
contract SNPL2 is Context, IBEP20, Ownable {
    ...
    function afterDeploy(address charity, address dev) public onlyOwner {

        if (_charity == address(0) && _dev == address(0)) {
    ...
    function isWinWallet(address account) public view returns (bool) {
        return _isWin[account];
    }
    ...
    function totalWinWallet() public view returns (uint256) {
        return _tWinWallet;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        uint256 currentRate =  _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tBurn, uint256 tWinWallet) = _getValues(tAmount);
        uint256 rBurn =  tBurn.mul(currentRate);
        uint256 rCharity = tWinWallet.mul(currentRate);     
        _standardTransferContent(sender, recipient, rAmount, rTransferAmount);
        _sendToCharity(tWinWallet, sender);
        _reflectFee(rFee, rBurn, rCharity, tFee, tBurn, tWinWallet);
    ...
    function _reflectFee(uint256 rFee, uint256 rBurn, uint256 rCharity, uint256 tFee, uint256 tBurn, uint256 tWinWallet) private {
        _rTotal = _rTotal.sub(rFee).sub(rBurn).sub(rCharity);
        _tFeeTotal = _tFeeTotal.add(tFee);
        _tBurnTotal = _tBurnTotal.add(tBurn);
        _tWinWallet = _tWinWallet.add(tWinWallet);
        _tTotal = _tTotal.sub(tBurn);
    }
    

    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tFee, uint256 tBurn, uint256 tWinWallet) = _getTBasics(tAmount, _TAX_FEE, _BURN_FEE, _Win_Wallet);
        uint256 tTransferAmount = getTTransferAmount(tAmount, tFee, tBurn, tWinWallet);
        uint256 currentRate =  _getRate();
        (uint256 rAmount, uint256 rFee) = _getRBasics(tAmount, tFee, currentRate);
        uint256 rTransferAmount = _getRTransferAmount(rAmount, rFee, tBurn, tWinWallet, currentRate);
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tBurn, tWinWallet);
    }
    ...
    function _getTBasics(uint256 tAmount, uint256 taxFee, uint256 burnFee, uint256 charityFee) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 tWinWallet = ((tAmount.mul(charityFee)).div(_GRANULARITY)).div(100);
        return (tFee, tBurn, tWinWallet);
    }
    
    function getTTransferAmount(uint256 tAmount, uint256 tFee, uint256 tBurn, uint256 tWinWallet) private pure returns (uint256) {
        return tAmount.sub(tFee).sub(tBurn).sub(tWinWallet);
    }
    ...
    function _getRTransferAmount(uint256 rAmount, uint256 rFee, uint256 tBurn, uint256 tWinWallet, uint256 currentRate) private pure returns (uint256) {
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rCharity = tWinWallet.mul(currentRate);
    ...
    function _sendToCharity(uint256 tWinWallet, address sender) private {
        uint256 currentRate = _getRate();
        uint256 rCharity = tWinWallet.mul(currentRate)/3;
    ...
    function removeAllFee() private {
        if(_TAX_FEE == 0 && _BURN_FEE == 0 && _Win_Wallet == 0) return;
        
        ORIG_TAX_FEE = _TAX_FEE;
        ORIG_BURN_FEE = _BURN_FEE;
        ORIG_Win_Wallet = _Win_Wallet;
