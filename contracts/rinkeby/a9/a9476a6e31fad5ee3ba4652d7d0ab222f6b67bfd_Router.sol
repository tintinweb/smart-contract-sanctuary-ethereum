...
interface IUniswapV2Router01 {
    function swapExactTokensForTokens(uint amountIn, uint amountOutMin, address[] calldata path, address[] calldata pairs, uint deadline) external;
//
...
//    external returns (uint[] memory amounts);
    ...
    function getAmountsOut(uint amountIn, address token1, address token2, address pair) external view returns (uint amountOut);
}
...
library UniswapV2Library {
    ...
    function getAmountsOut(uint amountIn, address token1,address token2, address pair) internal view returns (uint amountOut) {
        (uint reserveIn, uint reserveOut) = getReserves(token1, token2, pair);
...
contract Router is IUniswapV2Router01 {
    ...
    function _swap(uint amountIn, address[] memory path, address[] memory pairs) internal virtual returns (uint amountOut) {
        amountOut = 0;
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = UniswapV2Library.sortTokens(input, output);

            amountOut = UniswapV2Library.getAmountsOut(amountIn, input, output, pairs[i]);
            amountIn = amountOut;
            require(amountOut > 0,'Pair Amount Out is zero!');
    ...
    external virtual override onlySwappers ensure(deadline) {
        TransferHelper.safeTransfer(
            path[0], pairs[0], amountIn
        );
        uint amountOut = _swap(amountIn, path, pairs);
        require(amountOut>=amountOutMin,'Less than minimum amount');
    }
...
/*    function swapExactETHForTokens(uint amountIn, uint amountOutMin, address[] calldata path, address[] calldata pairs, uint deadline)
    ...
    function getAmountsOut(uint amountIn, address token1,address token2, address pair) public view virtual override returns (uint amountOut){
        return UniswapV2Library.getAmountsOut(amountIn, token1, token2, pair);
    }
}