...
contract LayerZeroBridge is ILayerZeroReceiver {
    ...
    event CallSuccess(address calledContract, bytes returnData, uint16 index);
    event CallFailed(address calledContract,uint16 index);
    ...
    function handleCall(CallData memory callData) internal returns (bytes memory) {
        ...
        if (success) {
            emit CallSuccess(target, returnData, 0);
        } else {
            emit CallFailed(target, 0);
        }
        return returnData;
    }

    function handleMultiCall(MultiCallData memory multiCallData) internal returns (bytes[] memory) {
        ...
        for (uint256 i = 0; i < multiCallData.destinationContracts.length; i++) {
            ...
            if (success) {
                emit CallSuccess(target, returnData, uint16(i));
                multipleReturnData[i] = returnData;
            } else {
                emit CallFailed(target, uint16(i));
                break;
            }
        }
    ...
    function callMultipleOnSecondChain(
        uint16 chainId,
        bytes calldata bridge_,
...
contract ContractFactory {
    ...
    * @dev Registers whitelisted bytecode at its hash.
     ...
     */
    function registerContract(bytes memory bytecode) external onlyRole(systemContext.getAccessControlList().CONTROL_LIST_ADMIN_ROLE()) {
    ...
    * @dev Registers whitelisted bytecode at its hash.
     * @param bytecode - contract bytecode to whitelist.
     * @param hash - contract keccak hash
     */
    function registerContractByHash(bytes memory bytecode, bytes32 hash) external onlyRole(systemContext.getAccessControlList().CONTROL_LIST_ADMIN_ROLE()) {
        contractsBytecode[hash] = bytecode;
        hashRegistered[hash] = true;
    }
...
contract ERC721Mintable is ERC721Enumerable, AccessControl {
    ...
    uint256 public slotStart;
    uint256 public slotEnd;
    SystemContext public system;
    string public baseTokenURI;


    constructor(string memory name_, string memory symbol_, bytes32 collectionId_,
        address owner_, uint256 slotStart_, uint256 slotEnd_, string memory _baseTokenURI, SystemContext system_) ERC721(name_, symbol_) {
    ...
    function mintTo(address _to, uint256 tokenId) external {
        if (address(system.getBridge()) != msg.sender) {
            ...
            if (isOwner(address(this), tokenId)) {
                this.transferFrom(address(this), _to, tokenId);
            }
            else {
                _mint(_to, tokenId);
            }
