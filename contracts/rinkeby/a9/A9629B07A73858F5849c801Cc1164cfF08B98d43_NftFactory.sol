    ...
    OwnableUpgradeable
{
    ...
    function uri(uint256 id)
        public
        view
        override(ERC1155Upgradeable, INftFactory)
     ...
     * @params nftId: Nft id
     * @params nftStatus: Nft status
     * @params attribute: NFT attribute
     */
    function addNftDetail(uint256 nftId, string memory attribute)
        external
        onlyOwner
    {
        require(!supportedTypes[nftId], "NftFactory: type already initialized");
        typeDetails[nftId] = NftDetails(nftId, attribute);
        supportedTypes[nftId] = true;
        allTypes.push(nftId);
        typesCount++;
        emit NftDetailAdded(nftId, attribute);
    }
    ...
    function externalMint(
        ...
        require(
            supportedTypes[nftId],
            "NftFactory: Unsupported type of ERC1155"
        );
        mint(to, nftId, amount, _data);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                require(supportedTypes[ids[i]],'NftFactory: Unsupported type of ERC1155');
        ...
        if (to == address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                uint256 id = ids[i];
                require(supportedTypes[id],'NftFactory: Unsupported type of ERC1155');
                uint256 amount = amounts[i];
                uint256 supply = totalSupply[id];
                require(
                    supply >= amount,
                    "NftFactory: burn amount exceeds totalSupply"
                );
                unchecked {
