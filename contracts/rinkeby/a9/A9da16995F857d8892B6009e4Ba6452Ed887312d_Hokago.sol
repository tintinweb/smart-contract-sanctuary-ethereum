...
contract Hokago is HokagoCore{
        ...
        callerIsUser
    {
        ...
        require(requestTokenID <= uint256(saleLimit), "Request character is not available yet");
        require(_numberMinted[_msgSender()] < 30, "Reached mint limit (=30)");
        ...
        returns (uint256[] memory)
    {
        uint256 counter = 0;
        uint256 startNo = 1;
        uint256 endNo = uint256(saleLimit);
        uint256 array_length = endNo - totalSupply();
        uint256[] memory mintableIDs = new uint256[](array_length);
    ...
    IHokagoCore
{
    ...
    mapping(uint256 => title) private _characterTitles;
    ...
    function viewCharacterTitles(uint256 tokenId)
        public
        view
        returns (string[] memory)
    {
        string[] memory arrayMemory = new string[](5);
        arrayMemory[0] = _characterTitles[tokenId].Dream;
        arrayMemory[1] = _characterTitles[tokenId].FavoriteSubject;
        arrayMemory[2] = _characterTitles[tokenId].FavoriteSport;
        arrayMemory[3] = _characterTitles[tokenId].LuckyColor;
        arrayMemory[4] = _characterTitles[tokenId].Nickname;
    ...
    function setTitle(uint256 tokenId, uint256 titleNo, string memory titleStr) external {
        require(_msgSender() == ownerOf(tokenId), "you don't own this token");

            if(titleNo == 1){
                require(_characterLevels[tokenId].Creativity == 10, "not reached max level(=10) yet");
                _characterTitles[tokenId].Dream = titleStr;
            }
            else if(titleNo == 2){
                require(_characterLevels[tokenId].Intelligence == 10, "not reached max level(=10) yet");
                _characterTitles[tokenId].FavoriteSubject = titleStr;
            }
            else if(titleNo == 3){
                require(_characterLevels[tokenId].Energy == 10, "not reached max level(=10) yet");
                _characterTitles[tokenId].FavoriteSport = titleStr;
            }
            else if(titleNo == 4){
                require(_characterLevels[tokenId].Luck == 10, "not reached max level(=10) yet");
                _characterTitles[tokenId].LuckyColor = titleStr;
            }
            else if(titleNo == 5){
                require(_characterLevels[tokenId].CommunicationSkills == 10, "not reached max level(=10) yet");
                _characterTitles[tokenId].Nickname = titleStr;
            }
    }
        ...
        callerIsUser
    {
        ...
        emit LevelUp(tokenId, "Intelligence", _characterLevels[tokenId].Intelligence);
    ...
    function _communicationLevelUp(uint256 tokenId) private {
        if(isLevelUp){
            if(_characterLevels[tokenId].CommunicationSkills < 10){
                ...
                emit LevelUp(tokenId, "CommunicationSkills", _characterLevels[tokenId].CommunicationSkills);
            }
        }
    }
    ...
    function energyLevelUp(uint256 tokenId) external {
        ...
        if(levelUpCount > 0){
            ...
            emit LevelUp(tokenId, "Energy", _characterLevels[tokenId].Energy);
        }
    }
    ...
    function creativityLevelUp(uint256 tokenId) external {
        ...
        if(isLevelUp){
            if(_characterLevels[tokenId].Creativity < 10){
                ...
                emit LevelUp(tokenId, "Creativity", _characterLevels[tokenId].Creativity);
            }
        }
    }
...
interface IHokagoCore{

    event LevelUp(uint256 indexed tokenId, string indexed levelName, uint32 indexed level);
