    ...
    ReentrancyGuardUpgradeable
{
    ...
    // uint256 private pendingWithdrawls;
    ...
    */
    function initialize(
        ...
        spectrum.startTime = block.timestamp;
        spectrum.endTime = block.timestamp + 24 hours;
        spectrum.limit = 2;
        spectrum.remainingTokens = 1000;
        // spectrum.price = 0 ether;

        presale.startTime = block.timestamp; // T = NOW FOR TESTING
        presale.endTime = presale.startTime + 24 hours;
        presale.limit = 1;
        presale.remainingTokens = 4800;
        // presale.price = 0 ether;

        publicSale.startTime = block.timestamp; // T = NOW FOR TESTING
        publicSale.endTime = publicSale.startTime + 24 hours;
        publicSale.limit = 2;
        publicSale.remainingTokens = 4800;
        // publicSale.price = 0 ether;
    ...
    */
    function sprectrumMint(uint256[] memory _tokenIds, uint256[] memory _amounts)
        external
        payable
        whenNotPaused
        nonReentrant
    {
        ...
        require(
            block.timestamp > spectrum.startTime && block.timestamp <= spectrum.endTime,
            "Spectrum: Closed"
        );
        ...
        for(uint i=0; i<_tokenIds.length; i++){
            ...
            require(
                spectrumPass.ownerOf(_tokenIds[i]) == msg.sender,
                "Spectrum: !Owner"
            );
            require(
                _amounts[i] + spectrumTracker[_tokenIds[i]] <= spectrum.limit, 
        ...
        require(
            totalAmountToMint + totalSupply() <= maxSupply - ownerRemainingTokens,
            "Spectrum: Sold Out"
        );
        require(totalAmountToMint <= spectrum.remainingTokens, "Spectrum: Not Enough Remaining");
        // require(msg.value == totalAmountToMint * spectrum.price, "Spectrum: !(Exact Fee)");
        ...
        // pendingWithdrawls += msg.value;

        _mint(msg.sender, totalAmountToMint);
    }

    /**
    @notice This function allows presaleed addresses to mint
    ...
    */
        ...
        checkSupply(_amount)
    {
        ...
        // require(msg.value == _amount * presale.price, "Presale: !(Exact Fee)");
    ...
    */
        ...
        checkSupply(_amount) 
    {
        ...
        require(
            ...
            "PublicSale: Sale Closed"
        );
        require(
            _amount <= publicSale.limit,
            "PublicSale: Individual Cap exceeding"
        );
        // require(msg.value == _amount * publicSale.price, "PublicSale: Pay exact fees");
    ...
    /**
    @notice This function allows owner to withdraw the accumulated funds to treasury  
    ...
    function withdraw() 
        ...
        nonReentrant 
    {
        require(treasury != address(0), "Withdraw: msg.sender to the zero address");
        
        address payable receiver = payable(treasury);
        uint256 amount = pendingWithdrawls;
        pendingWithdrawls = 0;
    ...
    */
    ...
    /**
    @notice Public function to get accumulated funds  
    @return uint256 Accumulated funds
    
    function availableToWithdraw() external view onlyOwner returns (uint256) {
        return pendingWithdrawls;
    }
    */
    ...
    */
    ...
    // function setSpectrumPrice(uint256 _price) external onlyOwner {
    //    spectrum.price = _price;
    // }
    ...
    //    presale.remainingTokens = _price;
    // }
