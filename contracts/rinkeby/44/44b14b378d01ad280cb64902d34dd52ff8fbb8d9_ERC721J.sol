...
// File @openzeppelin/contracts-upgradeable/proxy/utils/[email protected]4.2
    ...
    OwnableUpgradeable
{
    ...
    function changeSongURI(
        ...
        if (!_exists(songId)) revert URISetOfNonexistentSong();
        _songURIs[(songId * (10**15)) + songGeneration] = _songURI;
    }
    ...
    mapping(address => mapping(uint256 => bool)) private _migrationClaim;
    //Address to TokenId to bool
    mapping(address => mapping(uint256 => bool))
        private _migrationClaimSpecific;
    ...
    function setFreeClaim(
        ...
        uint256 _songOut = (songIdOut * (10**15)) + generationOut;
    ...
    function setFreeClaimSpecific(
        ...
        uint256 _songIn = (songIdIn * (10**15)) + generationIn;
        uint256 _songOut = (songIdOut * (10**15)) + generationOut;
        ...
        returns (uint256 _songIdOut, uint256 _generationOut)
    {
        ...
        _songIdOut = _songOut / (10**15);
        _generationOut = _songOut % (10**15);
    }
    ...
    function migrationClaimCheck(address _contract, uint256 tokenId)
        public
        view
        virtual
        returns (bool)
    {
        return _migrationClaim[_contract][tokenId];
    }

    //
    //Checks if a token from a contract has been claimed through the ERC721J specific whitelist.
    function migrationClaimCheckSpecific(address _contract, uint256 tokenId)
        public
        view
        virtual
        returns (bool)
    {
        return _migrationClaimSpecific[_contract][tokenId];
    }
    ...
    function freeClaim(address _contract, uint256 tokenId) public {
        ...
        if (_migrationClaim[_contract][tokenId] != false)
        ...
        uint256 _songOut = _addressClaim[_contract];
        uint256 _songIdOut = _songOut / (10**15);
        uint256 _generationOut = _songOut % (10**15);
        ...
        if (_songIdOut == 0)
            songId = _songIds;
            //sets the songId to the songIdOut
        else songId = _songIdOut;
        ...
        _migrationClaim[_contract][tokenId] = true;
    ...
    function freeClaimSpecific(address _contract, uint256 tokenId) public {
        ...
        ERC721J _claimContract = ERC721J(_contract);
        //Checks if the token has been claimed
        if (_migrationClaimSpecific[_contract][tokenId] != false)
            revert TokenAlreadyClaimed();
        //Checks if msg.sender is the owner of the token from the contract
        if (_claimContract.ownerOf(tokenId) != msg.sender)
            revert SenderNotOwner();
        ...
        uint256 _songIn = (_songIdIn * (10**15)) + _generationIn;
        //Grabs songOut for the contract, and gets the songIdOut and generationOut from it
        uint256 _songOut = _addressClaimSpecific[_contract][_songIn];
        uint256 _songIdOut = _songOut / (10**15);
        uint256 _generationOut = _songOut % (10**15);
        ...
        _migrationClaimSpecific[_contract][tokenId] = true;
    ...
    function freeClaimSongReward(address _contract, uint256 tokenId) public {
        ...
        uint256 _songIdIn = _claimContract.songOfToken(tokenId);
        uint256 _songIn = _songIdIn * (10**15);
        //Grabs songOut for the contract, and gets the songIdOut and generationOut from it
        uint256 _songOut = _addressClaimSpecific[_contract][_songIn];
        uint256 _songIdOut = _songOut / (10**15);
        uint256 _generationOut = _songOut % (10**15);
