...
// 0xfb4216a514d702d68b0ea83dba5fd2f0f2466a41
...
import "./interfaces/IMetaeraArtwork.sol";
import "./interfaces/IMetaeraToken.sol";
import "./lib/LibArtwork.sol";

/// @title NFT MetaeraToken合约
/// @notice NFT MetaeraToken合约接口
contract MetaeraToken is
    IMetaeraToken,
    Initializable,
    ERC1155Upgradeable,
    PausableUpgradeable,
    ...
    UUPSUpgradeable
{
    ...
    IMetaeraArtwork private _artworkStore;
    ...
    /// @dev 只有合约拥有者和授权的创建者可以调用
    modifier onlyOwnerOrCreator() {
        require(
            owner() == _msgSender() || _approvedCreators[_msgSender()],
    ...
    function setStoreAddress(address storeAddress) public override onlyOwner {
        _artworkStore = IMetaeraArtwork(storeAddress);
    }
    ...
    function setApprovedCreator(address[] calldata creators, bool ok) external onlyOwner {
        for (uint256 idx = 0; idx < creators.length; idx++) {
            _approvedCreators[creators[idx]] = ok;
        }
    }

    /// @notice 获取艺术品
    /// @param  artworkId 艺术品ID
    function getArtwork(uint256 artworkId) external view override returns (LibArtwork.Artwork memory artwork) {
        artwork = _getArtwork(artworkId);
    }
    ...
    function createArtwork(
        ...
        _createArtwork(_msgSender(), artworkId, supply);
    }
    ...
    function createArtworkAndReleases(
        ...
        _createArtwork(_msgSender(), artworkId, supply);
        _batchArtworkRelease(receiver, artworkId, count);
    }
    ...
    function releaseArtwork(
        ...
        _batchArtworkRelease(_msgSender(), artworkId, count);
    }
    ...
    /// @notice 销毁艺术品。一旦成功，此艺术品将无法再铸造任何NFT。不过，现有NFT需要单独销毁。
    /// @param  artworkId 艺术品ID
    function burnArtwork(uint256 artworkId) public onlyOwner {
        LibArtwork.Artwork memory artwork = _getArtwork(artworkId);

        uint32 increment = artwork.totalSupply - artwork.releaseCount;
        _incrementNFTRelease(artworkId, increment);
    ...
    function _createArtwork(
        ...
        _artworkStore.createArtwork(creator, artworkId, supply);
    }
    ...
    function _batchArtworkRelease(
        ...
        LibArtwork.Artwork memory artwork = _getArtwork(artworkId);
        require(artwork.creator != address(0), "artwork not exists");
        require(count < 10000, "count must < 10k");
        require(count > 0, "count must > 0");

        _incrementNFTRelease(artworkId, count);
        ...
        _mintBatch(nftOwner, tokenIds, amounts, "0x0");
        emit ArtworkReleaseCreated(nftOwner, artworkId, tokenIds);
    }
    ...
    function _incrementNFTRelease(uint256 artworkId, uint32 increment) internal {
        _artworkStore.incrementNFTRelease(artworkId, increment);
    }

    /// @notice 获取艺术品
    /// @param  artworkId 艺术品ID
    function _getArtwork(uint256 artworkId) internal view returns (LibArtwork.Artwork memory) {
        return _artworkStore.getArtwork(artworkId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        bytes memory data
    ) internal virtual override(ERC1155Upgradeable) onlyOwnerOrCreator {
...
/// @title 艺术品数据存储合约
/// @notice 这是Metaera NFT艺术品数据存储的接口。
interface IMetaeraArtwork {
    /// @notice 艺术品创建事件
    /// @param artworkId 艺术品ID
    /// @param creator 创建者
    /// @param supply 总发行量
    ...
    /// @notice 艺术品索引增加事件
    /// @param artworkId 艺术品ID
    /// @param increment 艺术品索引增量
    /// @param releaseCount 艺术品已发行量
    event ArtworkReleaseIncrement(
        uint256 indexed artworkId,
        uint32 increment,
        uint32 releaseCount
    );
