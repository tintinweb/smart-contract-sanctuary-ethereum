...
contract DisplateManager is CloneFactory, Ownable {
    ...
    function NFTDrop(bytes memory _data) external isOperationalAddress {
        ...
        for (uint32 i = 0; i < amountOfNFT * 160; i += 160) {
            ...
            NFT clonedNFT = NFT(clonedNFTAddress);
...
import "./DisplateManager.sol";

contract NFT is ERC721URIStorage, EIP712, INFT, IERC2981 {
    ...
    IAccessManager accessManager;
    DisplateManager displateManager;
    ...
    function init(
        ...
        ) = abi.decode(_staticData, (uint32, uint32, uint32, string, string));
        ...
        ) = abi.decode(_dynamicData, (uint32, uint32, uint32, uint32, address));
        ...
        displateManager = DisplateManager(msg.sender);
        ...
        if (artistAddress == address(0)) {
            ...
            require(
                initialRoyaltySalePercentage > 0,
                "NFT init: initialRoyaltySalePercentage must be higher then zero"
            );
        }
    }
    ...
    function awardToken(address _user, uint32 _tokenID) public override {
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        if (_exists(tokenId)) {
            _royaltyAmount = salePrice.mul(artistRoyaltiesPercentage).div(
                10000
            );
        } else {
            require(
            ...
            _royaltyAmount = salePrice.mul(initialRoyaltySalePercentage).div(
                10000
            );
        }
    ...
    function delegatedTransfer(
        ...
        // TODO: https://displate.atlassian.net/browse/NERF-1159
...
contract PurchaseManager is EIP712 {
    IAccessManager accessManager;
    INFT nft;
    ...
    bytes32 immutable PURCHASE_TYPEHASH =
        keccak256(
            "Purchase(address addressTo,address nftAddress,uint32 tokenId,uint256 ethPrice)"
        );
    ...
    {
        require(
            accessManager.isOperationalAddress(msg.sender),
            "This method is only for admin addresses"
        );
    ...
    function purchaseToken(
        ...
        require(
            accessManager.isOperationalAddress(owner),
            "Signature was generated by using address which is not admin address"
        );

        nft = INFT(_tokenAddress);

        nft.awardToken(_addressTo, _tokenId);
