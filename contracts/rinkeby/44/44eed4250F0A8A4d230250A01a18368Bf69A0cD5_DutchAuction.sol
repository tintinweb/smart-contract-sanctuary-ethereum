    ...
    PriceGetter
{
    ...
    function initialize(
        ...
        MaxEtraBidPercentages = _MaxEtraBidPercentages;
    ...
    uint32 public MaxEtraBidPercentages;
  ...
  /*   modifier isAuctionNotStartedByOwner(
    ...
    function getAuctionState(
        ...
        if(block.timestamp >= nftContractAuctions[nftContractAddress][tokenId].endTime) return AuctionState.ENDED; // The auction is over if the block timestamp is greater than the end timestamp, return ENDED
        if (block.timestamp >= nftContractAuctions[nftContractAddress][tokenId].startTime) return AuctionState.OPEN; //Auction start later
        else
    ...
    function getCurrentPrice(address nftContractAddress, uint256 tokenId) public view returns(uint){
        ...
        /* if(nftContractAuctions[nftContractAddress][tokenId].highestBid==0){
        uint timeElapsed = uint(time - nftContractAuctions[nftContractAddress][tokenId].startTime) / 60 / 60 / 24;
        uint decreaseAmount = nftContractAuctions[nftContractAddress][tokenId].decreaseRate*timeElapsed;
        return nftContractAuctions[nftContractAddress][tokenId].startPrice - decreaseAmount;}
        else 
            return nftContractAuctions[nftContractAddress][tokenId].startPrice; */
    function getReservedPrice(address nftContractAddress, uint256 tokenId) external view returns(uint){
        return nftContractAuctions[nftContractAddress][tokenId].reservedPrice;
    }
  ...
  /*   function _isERC20Auction(address _auctionERC20Token)
        ...
        internal
    {
        ...
        nftContractAuctions[nftContractAddress][tokenId].nftContractAddress = nftContractAddress;
        nftContractAuctions[nftContractAddress][tokenId].tokenId = tokenId;
        nftContractAuctions[nftContractAddress][tokenId].startTime = startTime;
        nftContractAuctions[nftContractAddress][tokenId].endTime = endTime;
        nftContractAuctions[nftContractAddress][tokenId].startPrice = startPrice;
        nftContractAuctions[nftContractAddress][tokenId].reservedPrice = reservedPrice;
        nftContractAuctions[nftContractAddress][tokenId].ERC20Token = erc20Token;
    ...
    function _createNewNftAuction(
        ...
        isWhitelisted(erc20Token)
    {
        _updateDefaultAuctionParameters(
            nftContractAddress,
            tokenId
        );
        require(block.timestamp >= startTime, "Auction start time must be in future");
        uint diff = (endTime - startTime);
        require(startPrice >= defaultDecreaseRate * diff, "starting price < min");
        AuctionState aucState;
        ...
        _transferNftToAuctionContract(nftContractAddress,tokenId);
    }
  ...
  /*   function _updateHighestBid(
    ...
    function _reverseAndResetPreviousBid(
        ...
        if (_isERC20Sale(auctionERC20Token)){
            ...
            _resetBids(_nftContractAddress, _tokenId);
            (bool success, ) = payable(nftHighestBidder).call{value: nftHighestBid}("");
            require(success, "error sending");
        }
    } */
   ...
   /*  function _transferNftAndPaySeller(
        ...
        address _nftRecipient = defaultFeeRecipient;
...
/*     function _payFeesAndSeller(
 ...
 /*    function settleAuction(address _nftContractAddress, uint256 _tokenId)
    ...
    {
        uint256 _nftHighestBid = nftContractAuctions[_nftContractAddress][_tokenId]
        .nftHighestBid;
        address _nftHighestBidder = nftContractAuctions[_nftContractAddress][_tokenId]
        .nftHighestBidder;
        address _nftSeller = nftContractAuctions[_nftContractAddress][_tokenId]
        .nftSeller;
        if (_nftHighestBid == 0){
            ...
            require(msg.sender == _nftHighestBidder, "Only NFT Highest Bidder");
            _transferNftAndPaySeller(_nftContractAddress, _tokenId);
        } 
    ...
    function changeMaxEtraBidPercentages(uint32 _newMaxEtraBidPercentages) external onlyOwner {
        MaxEtraBidPercentages = _newMaxEtraBidPercentages;
    }
...
contract PriceGetter is Initializable, OwnableUpgradeable{
  ...
  function __PriceGetter_init() internal onlyInitializing {
      __PriceGetter_init_unchained();
  }

  function __PriceGetter_init_unchained()
    internal
    onlyInitializing
  {
    __Ownable_init();
    bnb_usd_price_feed = AggregatorV3Interface(0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526);
  }
...
/*     event whitelistedToken(address indexed tokenaddressToWhitelist);
  ...
  //============== VARIABLES ==============
    ...
    AggregatorV3Interface internal bnb_usd_price_feed;
  ...
  function getBnbUsd() public view returns (uint256) {
    (
        , int price, , , 
    ) = bnb_usd_price_feed.latestRoundData();
...
/*   function convertBnbUsd(uint _amountInUsd) public view returns (uint256) {

    uint256 BnbUsd = getBnbUsd();
    uint256 FinalPrice = ((_amountInUsd.mul(10 ** 26)).div(BnbUsd));
  ...
  function convertBnbUsd(uint256 _amountInUSD) public view returns (uint256) {
    ...
    return (_amountInUSD *1e8) / uint256(getBnbUsd());
}
}



/*   function convertBNBUSD(uint _amountInUsd) public view returns (int) {

    int BNBUSD = getbnbusdLatestPrice();
    int bnbAmountInUsd =  (BNBUSD * _amountInUsd) / 1e18;
    return bnbAmountInUsd;
    //return _amountInUsd.mul(10 ** 16).div(BNBUSD);
