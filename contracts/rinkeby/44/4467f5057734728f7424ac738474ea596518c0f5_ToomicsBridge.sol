    ...
    ReentrancyGuardUpgradeable
{
    ...
    function refundNft(
        ...
        require(
            _checkRefundNftSign(sigV, sigR, sigS, _recipient, _tokenId, _nonce, timeEnd),
            "Invalid signature"
        );
        require(
...
contract Validator {
    ...
    function _checkRefundSig(
        ...
        // digest the data to transactionHash
        bytes32 inputHash = keccak256(
            abi.encode(funcHash, _recipient, _amount, _nonce, _timeEnd)
        );
        for (uint256 index = 0; index < sigV.length; index++) {
            // address recoveredAddress = ecrecover(inputHash, sigV[index], sigR[index], sigS[index]);
            if (
    ...
    function _checkUnlockNftSign(
        ...
        bytes32 funcHash = keccak256("unlocknft");
    ...
    function _checkMintNftSign(
        ...
        require(_chainIdDest == getChainID(), "wrong networks");
        bytes32 funcHash = keccak256("mintnft");
    ...
    function _checkRefundNftSign(
        ...
        bytes32 funcHash = keccak256("refundnft");

        // digest the data to transactionHash
        bytes32 inputHash = keccak256(
            abi.encode(funcHash, _recipient, _tokenId, _nonce, timeEnd)
        );
        for (uint256 index = 0; index < sigV.length; index++) {
            // address recoveredAddress = ecrecover(inputHash, sigV[index], sigR[index], sigS[index]);
            if (
    ...
    function _checkEmergencySig(
        ...
        // digest the data to transactionHash
        bytes32 inputHash = keccak256(abi.encode(funcHash, _amount));
        for (uint256 index = 0; index < sigV.length; index++) {
            // address recoveredAddress = ecrecover(inputHash, sigV[index], sigR[index], sigS[index]);
            if (
                !_checkSignature(
