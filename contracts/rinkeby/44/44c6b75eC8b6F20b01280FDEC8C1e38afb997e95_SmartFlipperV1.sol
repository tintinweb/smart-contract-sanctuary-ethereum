    ...
    VRFConsumerBaseV2
{
    ...
    event GameSessionPlayed(
        ...
        uint8 indexed gameResult,
        uint256 betAmount,
        uint8 bet,
        uint256 payoutAmount
    );
    event GamePayoutFailed(address indexed user, uint256 payoutAmount);
     ...
     */
    function play(uint8 _bet, address player)
        public
        payable
        onlyOwnerOrRootCallerAccount
    {
        ...
        Structs.ReservedAmount memory _reservedAmount;
        _reservedAmount = gamesPoolContract.reservedAmount(address(this));
        require(
            _reservedAmount.amount >=
                msg.value +_reservedAmount.reserved + msg.value,
            "not enough funds on contract"
        );
        uint256 _toReserve = (msg.value / 100) * winnerCoefficient;
        ...
        uint256 payoutAmount = msg.value + _toReserve;
        ...
        if (session.amount > 0) {
            if (session.bet == randomValue) {
                bool success = gamesPoolContract.withdrawReservedAmount{
                    value: 0
                }(session.payoutAmount, session.reservedAmount, session.player);
                if (!success) {
               ...
               gamesPoolContract.withdrawReservedAmount{
                    value: 0
                }(0, session.reservedAmount, session.player);
                paidAmount = 0;
            }
     ...
     */
    function clearReservedAmount() public onlyOwner {
        reservedAmount = 0;
    }

    function is_contract(address _addr) internal view returns (bool _isContract) {
        uint32 size;
        assembly {
