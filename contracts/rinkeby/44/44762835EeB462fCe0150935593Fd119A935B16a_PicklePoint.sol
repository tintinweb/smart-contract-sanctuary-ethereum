...
contract PicklePoint is ERC20, ERC20Burnable, AccessControl {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < length; ) {
            ...
            StakingStorage.layout().collections[collection] = IERC721Enumerable(
                collection
            );
    ...
    function setApprovalForAll(address collection) public {
        StakingStorage.layout().collections[collection].setApprovalForAll(
            address(this),
            true
        );
    }
    ...
    function pointsPerDay(address collection) public view returns (uint256) {
    ...
    function collectionBalanceOf(address collection, address user)
        public
        view
        returns (uint256)
    {
        return StakingStorage.layout().collections[collection].balanceOf(user);
    }

    function collectionTokenOfOwnerByIndex(
        address collection,
        address user,
        uint256 index
    ) public view returns (uint256) {
        return
            StakingStorage.layout().collections[collection].tokenOfOwnerByIndex(
                user,
                index
            );
    }
...
library StakingStorage {
    struct Layout {
        ...
        mapping(address => IERC721Enumerable) collections;
