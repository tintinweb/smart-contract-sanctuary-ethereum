...
contract PublicMint is ERC721Enumerable, Ownable, TimeLock {
    // events
    ...
    uint256 private _mintPrice; // price in WEI (smallest unit)
    uint16 private _maxNfts;
    uint16 private _maxNftMintAtOnce;
    uint16 private _maxOwnerMint;
    string private _provenanceHash;
    string private _provenanceHashOfProvenanceHashes;
    ...
    constructor(
        ...
        uint256 mintStart,
        uint16 maxNfts,
        uint16 maxNftMintAtOnce,
        uint16 maxOwnerMint,
        string memory provenanceHash,
        string memory provenanceHashOfProvenanceHashes
    ) ERC721(name, symbol) TimeLock(mintStart) {
        _baseTokenURI = baseURI;
        _mintPrice = mintPrice;
        _maxNfts = maxNfts;
        _maxNftMintAtOnce = maxNftMintAtOnce;
        _maxOwnerMint = maxOwnerMint;
        _provenanceHash = provenanceHash;
        _provenanceHashOfProvenanceHashes = provenanceHashOfProvenanceHashes;
    }
    ...
    function mint(address newOwner, uint16 amount)
        external
        payable
        onlyStarted
    {
        require(_minted + amount <= _maxNfts - _maxOwnerMint, "M2");
        require(amount > 0, "M3");
        require(amount <= _maxNftMintAtOnce, "M4");
        require(msg.value >= _mintPrice * amount, "M5");
        for (uint16 i = 0; i < amount; i++) {
            _safeMint(newOwner, _tokenId);
            emit NftMinted(newOwner, _tokenId);
            _minted++;
            _tokenId++; // has to be the last call in this loop
        }
    }

    function ownerMint(address newOwner, uint16 amount) external onlyOwner {
        require(_minted + _ownerMinted + amount <= _maxNfts, "M2");
        require(_ownerMinted + amount <= _maxOwnerMint, "M6");
        require(amount > 0, "M3");
        require(amount <= _maxNftMintAtOnce, "M4");
        for (uint16 i = 0; i < amount; i++) {
            _safeMint(newOwner, _tokenId);
...
abstract contract Ownable {
    ...
    function acceptOwnership() external virtual {
        require(msg.sender == _newOwner, "O2");
...
abstract contract TimeLock {
    uint256 private _start; // second since unix epoch
