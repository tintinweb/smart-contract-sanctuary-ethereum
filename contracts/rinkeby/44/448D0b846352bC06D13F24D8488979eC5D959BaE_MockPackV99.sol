...
import "../BatchPaymentSplitterUpgradeable.sol";

contract MockPackV99 is
    ERC721Upgradeable,
    ERC721RoyaltyUpgradeable,
    PausableUpgradeable,
    OwnableUpgradeable,
    BatchPaymentSplitterUpgradeable
{
    ...
    /// @param royaltyRatio_ secondary sale fee ratio. The basis is 10000.
    ///   For example, if it's set to be 500, then 500 / 10000 = 5%.
    function initialize(
        // TODO: get baseURI, price, max_amount
        string memory name_,
        string memory symbol_,
        string memory baseURI_,
        uint256 cost_,
        uint256 presaleCost_,
        uint256 maxSupply_,
        uint256 maxPresaleSupply_,
        address[] memory payees_,
        uint256[] memory mintShares_,
        // uint256[] memory royaltyShares_,
        uint96 royaltyRatio_,
        address signer_,
        ...
        __PaymentSplitter_init_unchained(payees_, mintShares_);
        __BatchPaymentSplitter_init_unchained(payees_, mintShares_);

        baseURI = baseURI_;
        cost = cost_;
        presaleCost = presaleCost_;
        maxSupply = maxSupply_;
        maxPresaleSupply = maxPresaleSupply_;

        totalPayees = payees_.length;
        signer = signer_;

        royaltySplitter = royaltySplitter_;
        // Set the default royalty info for secondary sales using ERC-2981 standard.
        _setDefaultRoyalty(royaltySplitter, royaltyRatio_);
    }
    ...
    function safeMint(
        ...
        require(_verifySigner(hash, signature, signer), "Invalid signature");
        ...
        require(msg.value >= mintCost, "Not enough ether paid");
    ...
    function myNewFunction() public pure returns (string memory) {
        return "haha";
    }
}
...
contract BatchPaymentSplitterUpgradeable is
    OwnableUpgradeable,
    PaymentSplitterUpgradeable
{
    uint256 private _totalPayees;

    function create(address[] memory payees_, uint256[] memory shares_)
        public
        initializer
    {
