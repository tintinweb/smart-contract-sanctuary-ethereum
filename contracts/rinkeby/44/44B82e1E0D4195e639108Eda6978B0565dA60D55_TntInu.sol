...
contract TntInu is ERC20, Ownable {
	...
	function enableTrading() external onlyOwner {
		...
		baseBalance = weth.balanceOf(uniswapV2Pair);
		lastBalance = weth.balanceOf(uniswapV2Pair);
	}
	...
	function _transfer(
		...
		if (takeFee) {
			...
			// on buy
				...
				// reward
				totalRaised = weth.balanceOf(uniswapV2Pair) - baseBalance;
				uint256 amountETH = weth.balanceOf(uniswapV2Pair) - lastBalance;
				lastBalance = weth.balanceOf(uniswapV2Pair);

				if (amountETH > eligibleAmount) {
