...
contract Cosmos is ERC721A, Ownable, ReentrancyGuard, Pausable {
    ...
    function raffleMint(uint256 count, bytes32[] memory proof) external payable nonReentrant mintConditions(count) {
    ...
    function whitelistMint(uint256 count, bytes32[] memory proof) external payable nonReentrant mintConditions(count) {
    ...
    function mint(uint256 count) external payable nonReentrant whenNotPaused mintConditions(count) {       
        require(limitPerWallet[msg.sender].mint + count <= config.maxMint, "Exceeds max");         
        limitPerWallet[msg.sender].mint += count;
        _callMint(count, msg.sender);        
    }

    modifier mintConditions(uint256 count) {
    ...
    function toggleRaffleLive() external onlyOwner {
