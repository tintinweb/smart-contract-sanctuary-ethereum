...
contract LitedexInitalOffering is TimeCrowdsale{
    ...
    function getTotalReferrer() external view returns(uint){
        return allReferrer.length;
    }
    function allReferrers() external view returns(address[] memory){
        return allReferrer;
    }
    ...
    function leaderboardDetail(uint position) external view returns(bytes3 refid, address userAddress, uint tRef, uint tRefUsd, uint point){
        ...
        if(allReferrer[position - 1] != address(0)){
            _acc = allReferrer[position - 1];
            _refId = accounts[_acc].referralId;
            _tRef = accounts[_acc].totalReferred;
            _tRefUsd = accounts[_acc].totalReferred;
            _points = accounts[_acc].totalReferred;
        }
        
        return (_refId, _acc, _tRef, _tRefUsd, _points);
    }
    function updateLeaderboard() public returns(bool) {
        for (uint i = 0; i < allReferrer.length; i++) {     
            for (uint j = i+1; j < allReferrer.length; j++) {  
                if(accounts[allReferrer[i]].totalReferredUsd < accounts[allReferrer[j]].totalReferredUsd) {    
                    address _temp = allReferrer[i];    
                    allReferrer[i] = allReferrer[j];    
                    allReferrer[j] = _temp;    
                }     
            }     
        }    
    ...
    function purchase(uint256 pid, uint256 amount, bytes3 refId) external onlyWhileOpen returns(bool){
        ...
        require(amount >= min, 'Litedex: amount lower than minimum contribution');
        require(amount <= max, 'Litedex: amount higher than maximum contribution');
        ...
        if(checkRefId(refId) && refId != bytes3(0)){
            ...
            accounts[_acc].points += tokenReached.div(purchasePoint);
        }
        //use default referral / others referral also get points 
        accounts[msg.sender].points += tokenReached.div(purchasePoint);
        homePoints += tokenReached.div(purchasePoint);
...
interface ILitedexInitalOffering {
    function allReferrer() external view returns(address[] memory);
    function getTotalReferrer() external view returns(uint);
    function getBnbPrice() external view returns(uint256);
    function safeTransferBNB(address to, uint256 value) external;
}
contract LitedexLeaderboard is TimeCrowdsale{
    using SafeMath for uint256;
    ...
    mapping(address => buyer) private checkClaim;
    address[] private rank;
    bool private claimLocked;
    uint256 private bnbRate;
    bool private fixrate = true;

    LitedexInitalOffering litedexico;
    constructor(address icoAddress, uint256 rate) public {
        litedexico = LitedexInitalOffering(icoAddress);
        bnbRate = rate;
    }

    modifier isClaimLocked {
        require(!claimLocked, "Litedex: Claim is locked");
        _;
    }
    modifier isAlreadyClaim {
        require(!alreadyClaim(msg.sender), "Litedex: has Claim BNB");
        _;
    }
    modifier isNotZeroAddress(address acc) {
        require(acc != address(0), "Litedex: address is zero address");
        _;
    }
    ...
    function setFixRate(bool status) external onlyOwner returns(bool){
        fixrate = status;
        return true;
    }
    function alreadyClaim(address account) public view isNotZeroAddress(account) returns(bool){
        return checkClaim[account].hasClaim;
    }
    function setClaimLocked(bool status) external onlyOwner returns(bool){
        claimLocked = status;
        return true;
    }
    function getCustomerData(address account) public view isNotZeroAddress(account) returns(bytes3 referralId,
        uint totalReferred,
        uint totalReferredUsd,
        uint usdPurchased,
        uint tokenPurchased,
        uint usdToConverted,
        uint points){
        return litedexico.accounts(account);
    }
    function getId(address account) public view isNotZeroAddress(account) returns(bytes3 id){
        (bytes3 rid, , , , , , ) = litedexico.accounts(account);
        return rid;
    }
    function getTotalReffered(address account) public view isNotZeroAddress(account) returns(uint){
        (, uint totalReferred, , , , , ) = litedexico.accounts(account);
        return totalReferred;
    }
    function getTotalRefferedUsd(address account) public view isNotZeroAddress(account) returns(uint ){
        (, , uint totalReferredUsd, , , , ) = litedexico.accounts(account);
        return totalReferredUsd;
    }
    function getTokenPurchase(address account) public view isNotZeroAddress(account) returns(uint){
        (, , , , uint tokenPurchased, , ) = litedexico.accounts(account);
        return tokenPurchased;
    }
    function getUsdToConverted(address account) public view isNotZeroAddress(account) returns(uint ){
        (, , , , , uint usdToConverted, ) = litedexico.accounts(account);
        return usdToConverted;
    }
    function setRate(uint256 rate) external onlyOwner returns(bool){
        bnbRate = rate;
    }
    function getBnbToConverted(address account) public view isNotZeroAddress(account) returns(uint){
        uint256 _bonus = getUsdToConverted(account);
        uint256 _bnbPrice;
        uint256 bnbConverted;
        if(fixrate){
            _bnbPrice = bnbRate;
            bnbConverted = _bonus.mul(1e18).div(_bnbPrice);
        }
        // else{
        //     _bnbPrice = ILitedexInitalOffering(address(litedexico)).getBnbPrice();
        //     if(_bnbPrice <= bnbRate){
        //         subt = bnbRate.sub(_bnbPrice);
        //         fallpercent = subt/bnbRate;
        //         _bonus = _bonus.add(_bonus).mul()
        //         bnbConverted = _bonus/_bnbPrice;
        //     }else{
    ...
    function getPoint(address account) public view isNotZeroAddress(account) returns(uint point){
        (, , , , , , uint points) = litedexico.accounts(account);
        return points;
    }
    function claimBonus() external isClaimLocked isAlreadyClaim returns(bool){
        require(block.timestamp > end, "Litedex: Claim after Crowdsale");
        uint256 _bnbConverted = getBnbToConverted(msg.sender);

        require(address(this).balance > _bnbConverted, "Litede: Not Enough Balance");
        ILitedexInitalOffering(address(litedexico)).safeTransferBNB(address(this), _bnbConverted);
        return true;
    }
}