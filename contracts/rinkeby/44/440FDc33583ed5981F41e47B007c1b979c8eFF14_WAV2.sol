...
contract WAV2 is Ownable, ERC721A, ReentrancyGuard, ERC2981{
    uint256 public tokenCount;
    uint256 public batchSize = 100;
    uint256 private wLMintPrice = 0.003 ether;
    uint256 private mintPrice = 0.005 ether;
    uint256 public wlMintLimit = 3;
    uint256 public MintLimit = 5;
    uint256 public _totalSupply = 15;
  ...
  constructor(
  ) ERC721A("WAV2", "W2",batchSize, _totalSupply) {
     tokenCount = 0;
     setHiddenBaseURI("https://soco-engineering.com/www/R.json");
  }
  ...
  function wlMint(uint256 quantity, bytes32[] calldata _merkleProof) public payable nonReentrant {
    ...
    require(wlMintLimit >= wlMinted[msg.sender] + quantity, "You have no Mint left");
  ...
  function withdrawRevenueShare() external onlyOwner {
    ...
    address artist = payable(0xb3c897C51957b2f8c7B051b61045C2E56Db67788); 
    address engineer = payable(0x2aE5393CFE777a65f30692dBf0aE9Aaa20352b40); 
    address marketer = payable(0x7a1824E35FFb5c76AFd06984700957C5DF26cC93);
    address manager = payable(0x2901f5FdAe6f75f0e1f0D6fD3905D3096ced3E86); 
    bool success;
  ...
  //SAL//
  IERC20 public SAL;
  function setSAL(address _address) external onlyOwner {
        SAL = IERC20(_address);
  }
  //Treasury//
  ...
  string public _modeX1TokenURI;
  string public _modeX2TokenURI;
  string public _modeX3TokenURI;
  string public _modeX4TokenURI;
  string public _modeX5TokenURI;
  ...
  //key Project
  address public x1_address = 0x2F333Dc02FF628c1a333cBc0569eD5c6Ea68b0d9;
  address public x2_address;
  address public x3_address;
  address x4_address;
  address private x5_address;
  ...
  balanceOfInterface x4_Contract = balanceOfInterface(x4_address);
  balanceOfInterface x5_Contract = balanceOfInterface(x5_address);

  bool releaseX1=false;
  bool releaseX2=false;
  bool releaseX3=false;
  bool releaseX4=false;
  bool releaseX5=false;
  ...
  function _baseURI(uint256 tokenId) internal view returns (string memory){
    ...
    if(mapTokenMode[tokenId] == 2) {
      return _modeX1TokenURI;
    }
    if(mapTokenMode[tokenId] == 3) {
      return _modeX2TokenURI;
    }
    if(mapTokenMode[tokenId] == 4) {
      return _modeX3TokenURI;
    }
    if(mapTokenMode[tokenId] == 5) {
      return _modeX4TokenURI;
    }
    if(mapTokenMode[tokenId] == 6) {
      return _modeX5TokenURI;
    }
      return _modeATokenURI;
  }
  ...
  function ModeX3(uint256 tokenId) public{
    require(ownerOf(tokenId) == msg.sender, "You are not the holder of this token.");
    require(releaseX3);
    require(SAL.balanceOf(msg.sender) >= SalCost,  "You don't have enough $SAL");
  //transfer//
    SAL.transferFrom(msg.sender, treasuryaddy, SalCost);
  ...
  function setModeX1_URI(string calldata baseURI) external onlyOwner {
    _modeX1TokenURI = baseURI;
  }
  function setModeX2_URI(string calldata baseURI) external onlyOwner {
    _modeX2TokenURI = baseURI;
  }
  function setModeX3_URI(string calldata baseURI) external onlyOwner {
    _modeX3TokenURI = baseURI;
  }
  function setModeX4_URI(string calldata baseURI) external onlyOwner {
    _modeX4TokenURI = baseURI;
  }
  function setModeX5_URI(string calldata baseURI) external onlyOwner {
    _modeX5TokenURI = baseURI;
  }
  ...
  function setX3Adress(address c_address) external onlyOwner {
    x3_address = c_address;
  }
  function setX4Adress(address c_address) external onlyOwner {
    x4_address = c_address;
  }
  function setX5Adress(address c_address) external onlyOwner {
    x5_address = c_address;
  }
  //set release flag
  function setReleaseX1Flag(bool bool_) external onlyOwner {
    releaseX1 = bool_;
  }
  function setReleaseX2Flag(bool bool_) external onlyOwner {
    releaseX2 = bool_;
  }
  function setReleaseX3Flag(bool bool_) external onlyOwner {
    releaseX3 = bool_;
  }
  function setReleaseX4Flag(bool bool_) external onlyOwner {
    releaseX4 = bool_;
  }
  function setReleaseX5Flag(bool bool_) external onlyOwner {
    releaseX5 = bool_;
  }
