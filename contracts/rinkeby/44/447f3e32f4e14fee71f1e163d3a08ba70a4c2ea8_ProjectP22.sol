...
contract ProjectP22 is ERC1155Upgradeable, OwnableUpgradeable {
    ...
    // Revenue
    uint256 public currentRevenueYear;
    mapping(uint256 => uint256) public totalRevenue;
    struct RevenueShares {
        uint256 ownerShare;
        uint256 creatorShare;
        uint256 tokenMembersShare;
    }
    RevenueShares public revenueShares;
    mapping(uint256 => mapping(uint256 => uint256))
        public perTokenClaimedAmount;
    mapping(uint256 => uint256) public ownerClaimedAmount;
    mapping(uint256 => uint256) public creatorClaimedAmount;
    ...
    function resetRevenueShare(uint256 _revenueYear) external onlyOwner {
    ...
    mapping(uint256 => uint256) public preMintPrice;

    // Toadz Free Claim
    mapping(uint256 => mapping(uint256 => bool))
        private isToadzIdClaimedFreePack;
    mapping(uint256 => bool) public isToadzFreeClaimPack;
    uint256 private toadzFreeClaimRelease;

    string private baseURI;

    mapping(uint256 => uint256) private claimedReservePacks;
    mapping(address => uint256) private numOfPreMinted;
    uint256 public preAccountPreMintLimit;

    modifier publicRevenueClaim() {
        require(
            420000000000000000000 > totalRevenue[currentRevenueYear],
            "total revenue must be greater than 420 eth"
        );
        _;
    }
    ...
    // ***************************** internal : Start *****************************
    function checkIndexIsAvailbale(uint256 randomNumber, uint256 packId)
        internal
        returns (uint256)
    {
        uint256 j = 0;
    ...
    function getElapsedSaleTime(uint256 packId)
        internal
        view
        returns (uint256)
    {
    ...
    function getRemainingSaleTime(uint256 packId)
        internal
        view
        returns (uint256)
    {
    ...
    function getRevenueShare(uint256 share) internal view returns (uint256) {
    ...
    function addNewPack(
        ...
        preMintPrice[newPackId] = _preMintPrice;
    ...
    function startPackSale(
        ...
        numOfDropPointsInPack[packId] = IVestedUtils(VestedUtilsAddress)
            .getNumOfDropPoints(_packStartPrice);
    }
    ...
    function claimOwnerShare() external onlyOwner {
        uint256 claimValue = getRevenueShare(revenueShares.ownerShare) -
            ownerClaimedAmount[currentRevenueYear];
    ...
    function setToadzFreeClaimPack(uint256 packId, bool _isToadzFreeClaimPack)
        external
        onlyOwner
        activePack(packId)
    {
    ...
    function setRevenueShares(
        ...
        require(
            totalRevenue[currentRevenueYear] == 0,
            "You can't change revenue share now"
        );
        require(
    ...
    function claimReservePacks(uint256 count, uint256 packId)
        external
        onlyOwner
        activePack(packId)
    {
        ...
        require(
            claimedReservePacks[packId] + count <= 69,
            "Count exceeds the maximum allowed supply."
        );
        _packMint(msg.sender, packId, count);
    }

    function setPerAccountPreMintLimit(uint256 newLimit) external onlyOwner {
        preAccountPreMintLimit = newLimit;
    }
    ...
    /**
    @notice Pre Mint pack
     */
    function PreMint(uint256 packId, bytes memory mintKey)
        external
        payable
        activePack(packId)
    {
        ...
        require(
            numOfPreMinted[msg.sender] + 1 <= preAccountPreMintLimit,
            "PreMint limt exceeds for this account"
        );
        require(msg.value >= preMintPrice[packId], "Not enough ether.");
        bool isValidMintKey = IVestedUtils(VestedUtilsAddress)
            .validateEligibilityKey(
                msg.sender,
                "Vested-",
                mintKey,
                0x20Ff11c0383C3E84D2D251Ab77eBBaD667c2964C
            );
        require(isValidMintKey, "Eligibility key is not valid");

        _packMint(msg.sender, packId, 1);
        numOfPreMinted[msg.sender] += 1;
    }
    ...
    function claimToadzFreePack(uint256 toadzTokenId, uint256 packId)
        external
        activePack(packId)
    {
        require(
            isToadzIdClaimedFreePack[toadzFreeClaimRelease][toadzTokenId] ==
                false,
            "Free pack already claimed for toadzId"
        );
        require(
    ...
    function claimToadzAllFreePacks(uint256 packId)
        external
        activePack(packId)
    {
        ...
        for (index = 0; index < tokenCount; index++) {
            ...
            if (
                isToadzIdClaimedFreePack[toadzFreeClaimRelease][tokenId] ==
                false
            ) {
                qty += 1;
                isToadzIdClaimedFreePack[toadzFreeClaimRelease][tokenId] = true;
            }
        }
        _packMint(msg.sender, packId, qty);
    }

    function toadzIdAvailableForFreePackClaim(uint256 toadzTokenId)
        external
        view
        returns (bool)
    {
        ...
        returns (uint256)
    {
        if (getElapsedSaleTime(packId) >= packSaleData[packId].saleDuration) {
            ...
            uint256 currentPrice = IVestedUtils(VestedUtilsAddress)
                .getDroppedPrice(
    ...
    function remainingReservePacks(uint256 packId)
        ...
        returns (uint256)
    {
        return 69 - claimedReservePacks[packId];
    }
    ...
    {
        return
            (getRevenueShare(revenueShares.tokenMembersShare) / 7025) -
    ...
    function unclaimedOwnerShare() external view returns (uint256) {
        return
            getRevenueShare(revenueShares.ownerShare) -
            ownerClaimedAmount[currentRevenueYear];
    }
    ...
    function unclaimedCreatorShare() external view returns (uint256) {
        return
            getRevenueShare(revenueShares.creatorShare) -
            creatorClaimedAmount[currentRevenueYear];
    }

    /**
    @notice Claim individual token share by OG Cryptoadz tokenID
     */
    function claimTokenShareByToken(uint256 tokenId)
        external
        publicRevenueClaim
    {
        require(
        ...
        uint256 claimValue = (getRevenueShare(revenueShares.tokenMembersShare) /
            7025) - perTokenClaimedAmount[currentRevenueYear][tokenId];
        transferFund(msg.sender, claimValue);
    ...
    function claimTokenShare() external publicRevenueClaim {
        ...
        for (index = 0; index < tokenCount; index++) {
            ...
            alreadyClaimedAmount += perTokenClaimedAmount[currentRevenueYear][
                tokenId
            ];
            perTokenClaimedAmount[currentRevenueYear][
                tokenId
            ] = perTokenClaimableAmount;
        }
...
interface IVestedUtils {
    ...
    function validateEligibilityKey(
        address accountAddress,
        string memory prefix,
        bytes memory eligibilityKey,
        address validaterAddress
