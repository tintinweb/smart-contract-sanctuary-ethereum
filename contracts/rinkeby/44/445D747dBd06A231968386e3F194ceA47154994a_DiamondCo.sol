...
contract DiamondCo is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public publicMintCost = 0.12 ether;
  uint256 public whitelist1MintCost = 0.099 ether;
  uint256 public whitelist2MintCost = 0.099 ether;
  ...
  function whitelistMint1(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
    ...
    require(whitelist1MintEnabled, 'The Guardian sale is not enabled!');
    require(mintedTokens[_msgSender()] + _mintAmount <= maxWhitelist1MintAmountPerWallet, 'Max limit per wallet!');
    require(msg.value >= whitelist1MintCost * _mintAmount, 'Insufficient funds for Guardian mint!');
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProof.verify(_merkleProof, merkleRoot1, leaf), 'Invalid proof for Guardian mint!');
  ...
  function whitelistMint2(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
    // Verify Sentinel requirements
    require(whitelist2MintEnabled, 'The Sentinel sale is not enabled!');
    require(mintedTokens[_msgSender()] + _mintAmount <= maxWhitelist2MintAmountPerWallet, 'Max limited per wallet');
    require(msg.value >= whitelist2MintCost * _mintAmount, 'Insufficient funds for Sentinel!');
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProof.verify(_merkleProof, merkleRoot2, leaf), 'Invalid proof for Sentinel mint!');
