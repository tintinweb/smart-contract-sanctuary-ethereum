...
abstract contract ERC1155Supply is ERC1155 {
    ...
    function indexOfAddress(address[] storage arr, address searchFor) internal view returns (uint256) {
    for (uint256 i = 0; i < arr.length; i++) {
      if (arr[i] == searchFor) {
        return i;
      }
    ...
    revert("Address Not Found");
     ...
     function indexOfUint256(uint256[] storage arr, uint256 searchFor) internal view returns (uint256) {
    for (uint256 i = 0; i < arr.length; i++) {
      if (arr[i] == searchFor) {
        return i;
      }
