...
contract TastyBones is Ownable, ERC721A {
    ...
    bytes32 private freeMintBoneMerkleRoot = 0xfd58e1d0cb9c5f1a40302b7c8015ee20cedf8c8ff89f2a75c544865d9817b406;
    ...
    function mintFreeWithBone(
        uint256 boneTokenId, 
        bytes32[] calldata boneProof) external payable {
        ...
        require(tokenSupply.current().add(1) <= maxFreeMintSupply, "EXCEEDS_FREE_MINT_SUPPLY" );
        ...
        addressBlockBought[msg.sender] = block.timestamp;
        mintedTBforFreeMintBone[boneTokenId] += 1;
    }
    ...
    function mintFreeWL(
        ...
        require(!Address.isContract(msg.sender),"CONTRACTS_NOT_ALLOWED_TO_MINT");
        require(isFreeActive, "FREE_MINT_IS_NOT_YET_ACTIVE");
        require(tokenSupply.current().add(numberOfTokens) <= maxFreeMintSupply, "EXCEEDS_FREE_MINT_SUPPLY" );
        require(mintedTBforFreeMintAddress[msg.sender].add(numberOfTokens) <= maxMint,"CANNOT_MINT_MORE_THAN_ALLOWED");
        require(mintedTBforFreeMintAddress[msg.sender].add(numberOfTokens) <= maxFreeMint,"ALREADY_MINTED_MAX_2");
    ...
    function mintPresale(uint256 numberOfTokens, bytes32[] calldata proof) external payable {
        ...
        require(msg.value >= price.mul(numberOfTokens), "INSUFFICIENT_PAYMENT");
        require(mintedTBforPresale[msg.sender].add(numberOfTokens) <= maxPresaleMint, "EXCEEDS_MAX_PRESALE_MINT" );
     ...
     */
    function mintRaffle(bytes32[] calldata proof) external payable {
        require(addressBlockBought[msg.sender] < block.timestamp, "CANNOT_MINT_ON_THE_SAME_BLOCK");
        require(!Address.isContract(msg.sender),"CONTRACTS_NOT_ALLOWED_TO_MINT");
        require(isRaffleActive, "RAFFLE_MINT_IS_NOT_YET_ACTIVE");
        require(msg.value >= price.mul(maxRaffleMint), "MAX_MINT_REACHED");
        require(tokenSupply.current().add(maxRaffleMint) <= maxTastyBones, "EXCEEDS_MAX_SUPPLY" );
        require(mintedTBforRaffle[msg.sender].add(maxRaffleMint) <= maxRaffleMint, "EXCEEDS_MAX_RAFFLE_MINT" );
