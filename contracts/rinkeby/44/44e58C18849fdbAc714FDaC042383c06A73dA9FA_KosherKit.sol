...
contract KosherKit is ERC721, Ownable {
  using Counters for Counters.Counter;
  using Strings for uint256;

  bytes32 public merkleRoot;
  mapping(address => mapping(uint256 => uint256)) private _ownedTokens;
  mapping(address => mapping(uint256 => uint256)) private _ownedTokensByOwner;
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    expiryTime[tokenIndex] = block.timestamp + 1 days;
  }

  function mint(uint256 _mintAmount, uint256 tokenId) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    require(!paused, 'The contract is paused!');
    require(!_exists(tokenId), "ERC721: token already minted");
    uint256 currentTokenId = _startTokenId();
    require(currentTokenId <= 450, "ERC721: max supply reached");
    _safeMint(_msgSender(), _mintAmount);
    expiryTime[currentTokenId] = block.timestamp + 1 days;
  }
  
  function mintForAddress(uint256 _mintAmount, address _receiver) public mintCompliance(_mintAmount) onlyOwner {
    _tokenIdCounter._value += 1;
    uint256 tokenIndex2 = _tokenIdCounter.current();
    _safeMint(_receiver, _mintAmount);
    expiryTime[tokenIndex2] = block.timestamp + 1 days;
  }

  function burn(uint256 _tokenId) public onlyOwner {
    transferFrom(ownerOf(_tokenId), 0x1243cFF98A1066381D760578a9b4432B31e3c693, _tokenId);
  }
