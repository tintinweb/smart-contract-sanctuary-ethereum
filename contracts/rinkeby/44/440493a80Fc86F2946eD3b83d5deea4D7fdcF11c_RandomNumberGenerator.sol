    ...
    AccessControl
{
    ...
    /**
     * @dev A list of blocks to be locked at based on past requests mapped by vault id
     */
    mapping(uint256 => uint32) internal requestLockBlock;

    /**
     * @dev A mapping from Chainlink request id to id of the vault for which the request has been made
     */
    mapping(bytes32 => uint256) internal chainlinkRequestVaultId;

    /**
     * @dev A mapping from the vault id to the latest random number obtained for this vault
     */
    mapping(uint256 => uint256) internal randomNumbers;

    /**
     * @dev A mapping from vault id to a boolean representing if a reuqest is ongoing for that vault
     */
     ...
     * @param _vaultId The ID of the vault for which random number request has been made
     ...
     */
    function requestRandomNumber(uint256 _vaultId)
        external
        override
        returns (uint32 lockBlock)
    {
        require(
            address(vaultManager) == _msgSender(),
            "RNG: This function can be called only by the Vault Manager!"
        );
        ...
        require(
            !requestOngoing[_vaultId],
            "RNG: A request for this vault is already ongoing!"
        );

        lockBlock = uint32(block.number);
        requestLockBlock[_vaultId] = lockBlock;

        // mark the request
        requestOngoing[_vaultId] = true;

        // send request (costs fee)
        bytes32 requestId = _requestRandomness();

        chainlinkRequestVaultId[requestId] = _vaultId;

        emit RandomNumberRequested(_vaultId, requestId);
    }
     ...
     */
    function isRequestComplete(uint256 _vaultId)
        external
        view
        override
        returns (bool isCompleted)
    {
        return !requestOngoing[_vaultId];
    }
    ...
    {
        return requestLockBlock[_vaultId];
    }
     ...
     */
    function getRandomNumber(uint256 _vaultId)
        external
        view
        override
        returns (uint256 randomNumber)
    {
        return randomNumbers[_vaultId];
    }
    ...
    function setVaultManagerAddress(address _vaultManagerAddress) external {
        ...
        require(
            ...
            "RNG: Vault Manager contract address cannot be address 0!"
        );
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        uint256 vaultId = chainlinkRequestVaultId[requestId];

        // Store random value
        randomNumbers[vaultId] = randomness;

        requestOngoing[vaultId] = false;

        emit RandomNumberCompleted(vaultId, requestId, randomness);

        vaultManager.distributeRewards(vaultId, randomness);
    }
     ...
     */
    function withdrawLink(address _account, uint256 _amount_link)
        external
    {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
