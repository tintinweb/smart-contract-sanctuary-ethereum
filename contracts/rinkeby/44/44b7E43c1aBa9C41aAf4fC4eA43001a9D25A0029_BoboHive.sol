...
contract BoboHive is ERC721A, Ownable, ERC721ABurnable, ReentrancyGuard, PaymentSplitter {

    using Strings for uint256;

    // Provenance Hash of the Bees
    string public constant BEES_PROVENANCE = "";  //TODO Decide to keep or not 
    ...
    mapping(address => uint64) private _presale3MintTracking; 
	...
	address[] private addressList = [
		...
		0xAEC39e71866b78A3C81916024Ad01ad266180598 // Community Wallet
	];
    ...
    constructor ( string memory name_, 
                    ...
                    uint256 price_
                    ) ERC721A(name_, symbol_) PaymentSplitter(addressList, shareList) {    
    ...
    */
    ...
    /**
    * @dev enables presale phase 3
    */
    function startPresale3() external onlyOwner{
        require(presale, "Presale already ended");
        require(!isPresale3, "Presale phase 3 already active");
        isPresale3 = true;
    }
    ...
    */
    function whitelistMinting(uint amountToMint) external payable {
        ...
        if(!isPresale3){
            ...
            require(_numberMinted(msg.sender) - _getAux(msg.sender) + amountToMint <= presaleWalletLimit, "max NFTs per address exceeded");
            _presale3MintTracking[msg.sender] += uint64(amountToMint);                
        }
    ...
    */
    function publicMint(uint amountToMint) external payable{
        ...
        require(_numberMinted(msg.sender) - _getAux(msg.sender) - _presale3MintTracking[msg.sender] + amountToMint <= publicSaleWalletLimit, "max NFTs per address exceeded");
    ...
    */
    function getNbrOfMintedToken(address wallet, uint32 presalePhase) external view returns (uint256) {
        ...
        if(presalePhase == 3) {
            return _presale3MintTracking[msg.sender];
        }

        return _numberMinted(wallet) - _getAux(wallet) - _presale3MintTracking[msg.sender];
    }
    ...
    **/
	function lockMetadata() public onlyOwner {
        ...
        require (revealed, "Cannot lock metadata before revealing collection"); 
		isLocked = true;
	}
}