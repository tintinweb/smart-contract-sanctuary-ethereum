 ...
 * @author BaseLabs
 */
contract CheersUpPeriod is ERC721, Ownable, Pausable, ReentrancyGuard {

    uint64 public constant MAX_TOKEN = 10000;
    uint64 public constant MAX_TOKEN_PER_MINT = 3;
    string public baseURI;
    address public cheersUpContractAddress;

    constructor() ERC721("Cheers UP Period", "CUP") {
    }
    function whitelistSale(uint64 numberOfTokens_) external payable nonReentrant {
        _sale(numberOfTokens_, 1000000 gwei);
    }
    ...
    function reveal(uint256 tokenId_) external callerIsUser nonReentrant returns (uint256) {
        ...
        _burn(tokenId_);
        CheersUpInterface cheersUpContract = CheersUpInterface(cheersUpContractAddress);
    ...
    function setCheersUpContractAddress(address cheersUpContractAddress_) external onlyOwner {
        cheersUpContractAddress = cheersUpContractAddress_;
    }
