...
contract StakeForMCSlot is Ownable {
    ...
    mapping(uint256 => uint256) public currentStaked;
    ...
    */
     function _withdrawStake(uint256 stakeindex) internal returns(uint256){
        ...
        currentStaked[current_stakes[stakeindex].sessionIndex]-=current_stakes[stakeindex].amount;
        currentReward[current_stakes[stakeindex].sessionIndex]-=current_stakes[stakeindex].reward;
    ...
    *
    ...
    function stakeForSlot(uint256 quantity) public  {
        ...
        stakeholders[index].address_stakes.push(Stake(stakeconfigs.length-1, msg.sender, currentConfig.tokenAddress,_amount, timestamp, timestamp+currentConfig.stakeDuration, _amount*currentConfig.rewardPerMillion/1000000,quantity));
        // Emit an event that the stake has occured
        totalStaked[stakeconfigs.length-1]+=_amount;
        totalReward[stakeconfigs.length-1]+=_amount*currentConfig.rewardPerMillion/1000000;

        currentStaked[stakeconfigs.length-1]+=_amount;
        currentReward[stakeconfigs.length-1]+=_amount*currentConfig.rewardPerMillion/1000000;
    ...
    function startNewStakeSession(
        ...
        totalStaked[stakeconfigs.length-1]=0;
        totalReward[stakeconfigs.length-1]=0;
        currentStaked[stakeconfigs.length-1]=0;
        currentReward[stakeconfigs.length-1]=0;
