...
//   ==========================================
    ...
    function _beforeTokenTransfer (address sender, address recipient, uint256 amount) internal override { 
        ...
        if (isAdjustable) {
            ...
            (_tradingTrack.buyingStack, _tradingTrack.sellingStack) = _updateStack(_newTokenPrice);
...
//   ======================================
    function _updateStack (uint256 _newTokenPrice) internal view returns (uint256 buyingStack, uint256 sellingStack) {  
...
//   ======================================
    ...
    function FisherAttenuation () external onlyRole(Treasury_Analyst) {
        uint256 _newTokenPrice = getTokenPrice();
        uint256 _attenuationPoint = (Wallet[liquidityPair].balance  / 10);
       (uint256 _buyingStack, ) = _updateStack(_newTokenPrice);
        require (_buyingStack >= _attenuationPoint, "Adjust not allowed at the moment");
    ...
    function DeviationLevel () external view returns (bool needAttenuation, uint256 buyingStack, uint256 sellingStack) {
        uint256 _newTokenPrice = getTokenPrice();
        (buyingStack, sellingStack) = _updateStack(_newTokenPrice);
...
contract SymplexiaToken is  Taxable {
   ...
   // swapRouterAddress
    ...
    constructor ()  ERC20("Symplexia Labs", "SWH78") BaseToken ( 
