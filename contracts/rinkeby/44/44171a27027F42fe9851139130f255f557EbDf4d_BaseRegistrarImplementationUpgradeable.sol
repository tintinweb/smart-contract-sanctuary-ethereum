...
import {ENS} from "../registry/interfaces/IENS.sol";
import {IERC721MetadataUpgradeable} from "@openzeppelin/contracts-upgradeable/token/ERC721/extensions/IERC721MetadataUpgradeable.sol";

// import "@openzeppelin/contracts/access/Ownable.sol";

abstract contract BaseRegistrar is IERC721MetadataUpgradeable {
    ...
    // The base URI for ERC721 Metadata
     ...
     */
    function reclaim(uint256 id, address nameOwner) external virtual;
}
...
contract BaseRegistrarImplementationUpgradeable is
    Initializable,
    UUPSProxy,
    ERC721Upgradeable,
    BaseRegistrar
{
     ...
     *    is an operator of the nameOwner, or is the nameOwner of the token
     */
    function _isApprovedOrOwner(address spender, uint256 tokenId)
        ...
        returns (bool)
    {
        address nameOwner = ownerOf(tokenId);
        return (spender == nameOwner ||
            getApproved(tokenId) == spender ||
            isApprovedForAll(nameOwner, spender));
    }
    ...
    /**
     * @dev Initialise a new ENS registrar.
     */
    function initialize(
        ENS _ens,
        bytes32 _baseNode,
    ...
    modifier live() {
        require(ens.owner(baseNode) == address(this), "BaseRegistrar::live:");
        _;
    }
     ...
     *@param baseUri_ new base uri, with mandatory ending `/`
     */
     ...
     * @param nameOwner The address that should own the registration.
    ...
    function register(
        ...
        return _register(id, nameOwner, duration, true);
    }
    ...
    function registerOnly(
        ...
        return _register(id, nameOwner, duration, false);
    }
    ...
    function _register(
        ...
        require(available(id), "BaseRegistrar::_register(): not available");
        require(
            block.timestamp + duration + GRACE_PERIOD >
                block.timestamp + GRACE_PERIOD,
            "BaseRegistrar::_register(): duration less than grace period"
        ...
        _mint(nameOwner, id);
        if (updateRegistry) {
            ens.setSubnodeOwner(baseNode, bytes32(id), nameOwner);
        }

        emit NameRegistered(id, nameOwner, block.timestamp + duration);
     ...
     * @dev Reclaim ownership of a name in ENS, if you own it in the registrar.
     */
    function reclaim(uint256 id, address nameOwner) external override live {
        require(
            _isApprovedOrOwner(msg.sender, id),
            "BaseRegistrar:reclaim(): sender not approved"
        );
        ens.setSubnodeOwner(baseNode, bytes32(id), nameOwner);
    }
...
abstract contract UUPSProxy is OwnableUpgradeable, UUPSUpgradeable {
    function __UUPSProxy_init() internal onlyInitializing {
        __Ownable_init();
        __UUPSUpgradeable_init();
        __UUPSProxy_init_unchained();
    }

    function __UUPSProxy_init_unchained() internal onlyInitializing {} // solhint-disable-line

    function _authorizeUpgrade(address) internal override onlyOwner {} // solhint-disable-line

    modifier onlyProxyAdmin() {
        require(_getAdmin() == msg.sender, "Caller must be Proxy Admin");
        _;
    }

    /**
     * @dev Returns the active proxy admin.
     */
    function getProxyAdmin() public view returns (address) {
        return _getAdmin();
    }

    function changeProxyAdmin(address newAdmin) external onlyOwner {
