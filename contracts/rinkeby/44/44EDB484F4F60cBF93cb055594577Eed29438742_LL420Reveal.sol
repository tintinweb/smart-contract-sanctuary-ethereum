 ...
 *
 */
contract LL420Reveal is VRFConsumerBaseV2, Ownable {
    ...
    uint16 public totalRevealed = 0;
    uint16[4] public totalBudPerThc = [12000, 5000, 2000, 1000];
    ...
    address public budContractAddress;

    ///
    mapping(uint256 => uint256[]) public _vrfRequestStack;

    event GenerateRandomWords(uint256[] randomWords);
    event GenerateRandomNumber(uint256 number);
    ...
    constructor(
        ...
        require(_stakingAddress != address(0) && _budAddress != address(0), "Zero address error");
        ...
        budContractAddress = _budAddress;
    }
     ...
     * @param _ids Array of buds
     *
     * TODO
     * - check ids are belong to right owner
     * - check thc was set and revealed before
     * - check if bud is ready to reveal
     */
    function reveal(uint256[] memory _ids) external {
        require(stakingContractAddress != address(0), "Staking contract is not set yet");
        require(budContractAddress != address(0), "Bud contract is not set yet");
    ...
    function _requestRandomWords(uint256[] memory _ids) internal {
        ...
        _vrfRequestStack[requestId] = _ids;
    }

    /**
     */
    function _updateBudTHC(uint256 _requestId, uint256[] memory _randomWords) internal {
        emit GenerateRandomWords(_randomWords);

        uint256[] memory budIds = _vrfRequestStack[_requestId];
        uint256[] memory thcInfo = new uint256[](budIds.length);
        uint256 length = budIds.length;
        uint256 restTHC0 = totalBudPerThc[0] - revealedBudPerThc[0];
        uint256 restTHC1 = totalBudPerThc[1] - revealedBudPerThc[1];
        uint256 restTHC2 = totalBudPerThc[2] - revealedBudPerThc[2];
        uint256 restTHC3 = totalBudPerThc[3] - revealedBudPerThc[3];

        require(restTHC0 + restTHC1 + restTHC2 + restTHC3 == TOTAL_SUPPLY - totalRevealed);

        for (uint256 i = 0; i < length; i++) {
            uint256 rest = TOTAL_SUPPLY - totalRevealed;
            require(rest > 0, "Cant mod with zero");

            uint256 pos = _random(_randomWords[0], i) % rest;
            emit GenerateRandomNumber(pos);
     ...
     *
     * @param _id: The id of subscription
     */
     ...
     * @param _budAddress The address of LL420Bud contract
     */
    function setContracts(address _stakingAddress, address _budAddress) external onlyOwner {
        if (_stakingAddress != address(0)) stakingContractAddress = _stakingAddress;
        if (_budAddress != address(0)) budContractAddress = _budAddress;
    }
