...
contract Genesis is MultisigOwnable, VRFConsumerBase, ERC721A, ERC721ABurnable, PaymentSplitter, BatchReveal {
    ...
    uint private constant MAX_TEAM = 3;
    uint private constant MAX_FREE = 5; // Addition of each wallet allowance (free + bronze + silver + gold)
    uint private constant MAX_WHITELIST = 20;
    uint private constant MAX_SECOND_WHITELIST = 25;
    uint private constant MAX_ALLOWLIST = 26;
    ...
    function whitelistMint(bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(amountNFTsperWalletWhitelistSale[msg.sender] + 1 < MAX_MINT_PER_WHITELIST_ACCOUNT + 1, "You can only get 2 NFTs on the Whitelist Sale");
        require(totalSupply() + 1 <= MAX_WHITELIST, "Max supply exceeded");
        require(totalSupply() + 1 <= maxSupply, "Max supply exceeded");
        require(msg.value == salePrice, "Wrong payment");
        amountNFTsperWalletWhitelistSale[msg.sender] += 1;
        _mint(msg.sender, 1);
    }

    function secondWhitelistMint(bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(amountNFTsperWalletSecondWhitelistSale[msg.sender] + 1 < MAX_MINT_PER_SECOND_WHITELIST_ACCOUNT + 1, "You can only get 1 NFTs on the Second Whitelist Sale");
        require(totalSupply() + 1 <= MAX_SECOND_WHITELIST, "Max supply exceeded");
        require(totalSupply() + 1 <= maxSupply, "Max supply exceeded");
        require(msg.value == salePrice, "Wrong payment");
        amountNFTsperWalletSecondWhitelistSale[msg.sender] += 1;
        _mint(msg.sender, 1);
    }
    ...
    function allowlistMint(bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(totalSupply() + 1 <= MAX_ALLOWLIST, "Max supply exceeded");
        require(totalSupply() + 1 <= maxSupply, "Max supply exceeded");
        require(msg.value == salePrice, "Wrong payment");
        _mint(msg.sender, 1);
    }
    ...
    function winterMint(address _to) external payable {
        require(sellingStep > Step.AllowlistSale, "Public sale is not activated");
        require(sellingStep < Step.SoldOut, "Sale is done");
        require(totalSupply() + 1 <= maxSupply - MAX_TEAM - MAX_FREE, "Max supply exceeded");
        require(msg.value == salePrice, "Wrong payment");
        _mint(_to, 1);
        require(totalSupply() <= maxSupply - MAX_TEAM - MAX_FREE, "Limit reached");
    }
    ...
    function setRoots(bytes32 _whitelistRoot, bytes32 _secondWhitelistRoot, bytes32 _allowlistRoot, bytes32 _teamRoot, bytes32 _freeRoot, bytes32 _freeBronzeRoot, bytes32 _freeSilverRoot, bytes32 _freeGoldRoot) external onlyOwner {
        whitelistRoot = _whitelistRoot;
        secondWhitelistRoot = _secondWhitelistRoot;
        allowlistRoot = _allowlistRoot;
        teamRoot = _teamRoot;
        freeRoot = _freeRoot;
        freeBronzeRoot = _freeBronzeRoot;
        freeSilverRoot = _freeSilverRoot;
        freeGoldRoot = _freeGoldRoot;
    }
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        if (id >= lastTokenRevealed) {
            ...
            return string(abi.encodePacked(baseURI, getShuffledTokenId(id, 0).toString(), ".json"));
        }
    }
...
abstract contract BatchReveal {
    uint constant public TOKEN_LIMIT = 33;
    uint constant public REVEAL_BATCH_SIZE = 33;
