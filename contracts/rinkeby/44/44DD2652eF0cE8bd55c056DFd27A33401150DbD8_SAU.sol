...
contract SAU is  ERC721A, Ownable {
    ...
    function presaleMintHero(uint256 _unitSize) external payable preSaleIsLive{
        ...
        require(heroWhiteList[msg.sender], "Transaction Error: The Connect Wallet is Not Whitelisted");
    ...
    function presaleMintVIllain(uint256 _unitSize) external payable preSaleIsLive{
        ...
        require(tx.origin == msg.sender, "Transaction Error: Contracts Not Allowed");
         require(totalSupply() < maxSupplyCollection, "Sale Closed: The Collection is Sold Out");
        require(villainWhiteList[msg.sender], "Transaction Error: The Connect Wallet is Not Whitelisted");
    ...
    function addToHeroWhiteList(address[] calldata addresses) public onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++)
            heroWhiteList[addresses[i]] = true;
    }

    function addToVillainWhiteList(address[] calldata addresses) public onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++)
