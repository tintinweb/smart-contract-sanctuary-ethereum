...
//contract to be able to accept some type of payment
...
contract fund_me{
    ...
    //setup as owner of the current sender address
    ...
    //function to store address and the amount in the mapping type datatype
    function fund() public payable{
        //msg.sender & msg.value is default keywords in any transaction(
        uint256 min_usd = 50*10*8;
        //it is causing an issue 
        require(getConversionRate(msg.value) >= min_usd ,"you need to spend more eth");
        require(true,uint2str(min_usd));
        require(0 <= 1 ,uint2str(getConversionRate(msg.value)));
        address_to_amount_funded[msg.sender] += msg.value;
        funders.push(msg.sender);
    }

    function get_version() public view returns (uint256){
        //address of the contract where the function defined 
        return price_feed.version();
    }

    //get the price of eth which is being funded
    ...
    //convert the eth to the usd
    function getConversionRate(uint256 ethAmount) public view returns (uint256){
        uint256 eth_price = get_price();
        uint256 eth_amount_in_usd = (eth_price * ethAmount) / 10**18;
    ...
    //we are the only one who can withdraw from the contract
    modifier onlyowner{
        require(msg.sender == owner);
        //run rest of the function after the above statement
        _;
    }

    //get all the ether back which was funded
    //only callable by owner fue to above modifier defined onlyowner
