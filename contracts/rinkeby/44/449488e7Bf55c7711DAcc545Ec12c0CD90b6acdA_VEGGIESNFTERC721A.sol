...
contract VEGGIESNFTERC721A is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private constant MAX_WHITELIST = 9;
    uint private constant MAX_PUBLIC = 88;
    uint private constant MAX_GIFT = 3;
    uint private constant maxMintAmount = 3;


    uint public wlSalePrice = 0.002 ether;
    uint public publicSalePrice = 0.003 ether;

    bytes32 public merkleRoot;

    uint public saleStartTime =    1655992800;

    mapping(address => uint) public amountNFTsperWalletWhitelistSale;

    uint private teamLength;

    constructor(address[] memory _team, uint[] memory _teamShares, bytes32 _merkleRoot, string memory _baseURI) ERC721A("VEGGIES NFT", "VEG")
    ...
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(currentTime() < saleStartTime + 30 minutes, "Whitelist Sale is finished");
