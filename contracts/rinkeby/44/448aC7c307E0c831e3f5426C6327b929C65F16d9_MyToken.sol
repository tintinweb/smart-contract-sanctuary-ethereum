...
contract MyToken is ERC721, ERC721URIStorage, AccessProtected {
    ...
    constructor(address _whitelistAddress) ERC721("NFTPFP", "PFP") {
    ...
    function safeMint(string memory uri) public payable {
        ...
        require(msg.value>=fee,"amount not Sufficient to mint");
        require(nftCounter[msg.sender]<3,"You cannot mint more than 3 NFTs");
        require(tokenId<100,"More than 100 NFTs cannot be minted");
        require(saleEnabled,"Sale isn't started yet");
        if(startSale < block.timestamp && startSale + 1800 > block.timestamp){
            require(whitelist.checkWhitelist(msg.sender),"You are not a whitelisted member");
        }
    ...
    function safeMintBatch(string[] memory uri) public {
        require(nftCounter[msg.sender] +  uri.length <= 3,"You cannot mint more than 3 NFTs");
    ...
    function setBatchTokenUri(uint256[] memory _tokenId, string[] memory _tokenURI) external onlyAdmin {
        require(_tokenId.length == _tokenURI.length,"Parameter Mismatch");
        for(uint8 i = 0; i<_tokenId.length; i++){
            require(_exists(_tokenId[i]), "ERC721URIStorage: URI set of nonexistent token");
        }
        for(uint8 j = 0; j<_tokenId.length; j++){
            _setTokenURI(_tokenId[j], _tokenURI[j]);
        }
    }
...
contract Whitelist is Ownable {
   ...
   /**
    ...
    function addToWhitelist(address _user) public onlyOwner {
        require(whitelistedCounter <= 60,"You cannot add more than 60 addresses to whitelist");
    ...
    function addAddressesToWhitelist(address[] memory _userAddresses) public onlyOwner {
        require(whitelistedCounter +  _userAddresses.length <= 60,"You cannot add more than 60 addresses to whitelist");
