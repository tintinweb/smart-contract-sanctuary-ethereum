...
contract EasyDemons is ERC721EnumerableM, Ownable {
    ...
    function devMint(bytes32[] calldata proof) external {
        ...
        require(_verify(_leaf(msg.sender, 1), proof), "invalid MP");
    ...
    function raffleSaleMint(uint256 tokenQuantity, uint256 allowance, bytes32[] calldata proof) external payable {
        uint256 supply = totalSupply();
        require(_verify(_leaf(msg.sender, allowance), proof), "invalid MP");
    ...
    function publicSaleMint(uint256 tokenQuantity) external payable {
        uint256 supply = totalSupply();
