...
contract SpacePool is ERC20 {
    ...
    event Minted(address indexed minter, uint256 amount); // TODO write tests
    event Burned(address indexed burner, uint256 amount); // TODO write tests
    ...
    function mint(address to) external payable returns (uint256 amountToMint){
        // TODO What happens when tax is on
    ...
    /**
     * @notice Burns a certain amount of SPC-LP tokens 
     *  and returns eth and spc proportional to the LP share
     * @param amount an amount of LP to burn
     * @param to address to return eth and spc to
     */
    function burn(uint256 amount, address to) external returns (uint256 spaceAmountToRefund, uint256 ethAmountToRefund) {
        if (amount > balanceOf(to)) revert NotEnoughTokens();
        if (amount == 0) revert InvalidAmount(amount);

        spaceAmountToRefund = (reserveSPC * amount) / totalSupply();
        ethAmountToRefund = (reserveEth * amount) / totalSupply();
        ...
        spaceCoin.transfer(to, spaceAmountToRefund);
        (bool success, ) = to.call{ value: ethAmountToRefund}("");
    ...
    function swapEthForSpc(address to, uint256 minSpc) external payable {
        ...
        uint256 spaceAmountOut = getOutputAmount(amountIn, reserveEth, reserveSPC);
        if (spaceAmountOut < minSpc) revert InsufficientTokenAmounts(minSpc, spaceAmountOut);

        reserveEth += amountIn;
        reserveSPC -= spaceAmountOut;

        spaceCoin.transfer(to, spaceAmountOut);
    ...
    function swapSpcForEth(address to, uint256 minEth) external returns (uint256 ethAmountOut){
        ...
        ethAmountOut = getOutputAmount(amountIn, reserveSPC, reserveEth);
    ...
    /**
     * @notice Helper function that returns the output token amount given 
     *  an input token amount and a fee (1%)
