...
contract BASECONTRACT is ERC721A, Ownable {
    uint256 public maxquantity = 1000;
    uint256 public reserve = 1;
    uint256 public presale = 200;
    uint256 public price = 1 ether;

    bytes32 private merkleRoot;
    string public _baseURI1;
    bool public pausable = false;
    ...
    constructor(string memory baseUri) ERC721A("name", "symbol") {
        _baseURI1= baseUri;
    }
    ...
    function setRoot(bytes32 a) public onlyOwner {
        merkleRoot = a;
    }
    ...
    function setMaxQuantity(uint256 _max) public onlyOwner {
        maxquantity = _max;
    }

    function setReserve(uint256 _reserve) public onlyOwner {
        require(_reserve < maxquantity, "amount exceeds");
        reserve = _reserve;
    }

    function setPresale(uint256 _presale) public onlyOwner {
        require(_presale < maxquantity, "amount exceeds");
        presale = _presale;
    }
    ...
    function mint(uint256 quantity) public payable {
        ...
        require(totalSupply()+quantity<=maxquantity-reserve,"all tokens have been minted");
        require(price*quantity == msg.value, "Sent ether value is incorrect");
        _safeMint(msg.sender, quantity);
    }

    function mintreserve(uint256 quantity) public onlyOwner {
        require(quantity <= reserve, "the quantity exceeds reserve");
        reserve -= quantity;
        _safeMint(msg.sender, quantity);
    }

    function mintPreSale(bytes32[] calldata _merkleProof, uint256 quantity)
        public
        payable
    {
        require(pausable == true, "turn on minting");
        ...
        require(totalSupply() + quantity <= presale, "presale amount exceeds");
        require(price*quantity==msg.value,"invalid amount");
        _safeMint(msg.sender,quantity);
    }

        function MINTforTokensHolders(uint256 quantity)
        public
        payable
    {
        require(pausable == true, "turn on minting");
    ...
    function tokensOfOwner(address _owner)public view
