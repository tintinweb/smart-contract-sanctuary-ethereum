...
contract MetacityLandBoxer is Context, Ownable {
    ...
    modifier onlyCapacityEnoughBatch(string[] memory _zoneIds, uint256[] memory _scales, uint256[] memory _amounts) {
        ...
        for(uint256 i=0; i<_zoneIds.length; i++) {
            ...
            if (purchaseLimit[zoneId][_scales[i]] >= 1) {
                require(purchaseLimit[zoneId][_scales[i]] >= _amounts[i] + 1, "exceed purchase limit");
            }
        }
        _;
    }
        ...
        onlyOwner
    {
        ...
        for (uint8 i=0; i<_zoneIds.length; i++) {
            _metaTradeSubCapacity(_getZoneId(_zoneIds[i]), _scales[i], _amounts[i]);
            _mintNftAndEmitEvents(to, _getZoneId(_zoneIds[i]), _scales[i], PriceInfo(ZERO_ADDRESS, 0, ICheckouter.BillingType.FIXED_AMOUNT), _amounts[i]);
        }
    }
        ...
        onlyCapacityEnoughBatch(_zoneIds, _scales, _amounts)
    {
        ...
        for (uint256 i=0; i<_zoneIds.length; i++) {
            ...
            if (purchaseLimit[zoneId][_scales[i]] >= 1) {
                purchaseLimit[zoneId][_scales[i]] = purchaseLimit[zoneId][_scales[i]].sub(_amounts[i]);
            }
        ...
        onlyCapacityEnoughGlobalBatch(_scales, _amounts)
    {
        ...
        for (uint256 i=0; i<_scales.length; i++) {
            ...
            while (idx < _amounts[i]) {
                bytes32 zoneId = _getFirstRandomZoneIdWithEnoughCapacity(idx.add(i), _scales[i], 1);
                remainingValue = remainingValue.sub(_ethPurchaseLand(zoneId, _scales[i], globalPrices[_scales[i]], 1));
                _zoneIds[idx] = zoneId;
                idx = idx.add(1);
            }
            _globalMintNftAndEmitEvents(_msgSender(), _zoneIds, _scales[i], globalPrices[_scales[i]], _amounts[i]);
        }
        _refundRemainEth(remainingValue);
    }
    ...
    function _ethPurchaseLand(bytes32 _zoneId, uint256 _scale, PriceInfo memory _priceInfo, uint256 _amount) private returns (uint256 purchaseCost) {
        ...
        _metaTradeSubCapacity(_zoneId, _scale, _amount);
        purchaseCost = _amount.mul(_priceInfo.price);
    }

    function _metaTradeSubCapacity(bytes32 _zoneId, uint256 _scale, uint256 _amount) private {
    ...
    function _mintNftAndEmitEvents(address to, bytes32 _zoneId, uint256 _scale, PriceInfo memory _priceInfo, uint256 _amount) private {
    ...
    function _globalMintNftAndEmitEvents(address to, bytes32[] memory _zoneIds, uint256 _scale, PriceInfo memory _priceInfo, uint256 _amount) private {
        require(_amount == _zoneIds.length, "params not match");
        uint256[] memory tokenIds = new uint256[](_amount);
        uint256[] memory seeds = new uint256[](_amount);
        uint256 tokenId = IERC721WithAutoId(landAddress).currentId();
        for (uint256 i=0; i<_amount; i++) {
        ...
        for (uint256 i=0; i<_amount; i++) {
            emit LandPurchased(
                ...
                _priceInfo.price,
                _amount,
                _getZoneIdentifierByZoneId(_zoneIds[i]),
                tokenIds,
                seeds
            );
        }
