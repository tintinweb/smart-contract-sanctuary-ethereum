...
contract CrowdFunding {
    ...
    event Log(address fundaddr, uint256 amount);
    ...
    */
    ...
    function unko(uint256 campaignID, uint256 fundaddr)
        public
        returns (address addr, uint amount)
    {
        Campaign storage c = campaigns[campaignID];
        emit Log(c.funders[fundaddr].addr, c.funders[fundaddr].amount);

        return (c.funders[fundaddr].addr, c.funders[fundaddr].amount);
    }
}