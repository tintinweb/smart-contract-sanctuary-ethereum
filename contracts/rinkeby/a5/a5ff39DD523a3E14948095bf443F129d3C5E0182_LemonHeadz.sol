...
contract LemonHeadz is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string public baseURI;
  string public baseExtension = ".json";
  string public notRevealedUri;
  uint public cost = 0.028 ether;
  uint public maxSupply = 10000;
  uint public maxMintAmount = 20;
  uint public nftPerAddressLimit = 5;
  ...
  function mint( uint256 _mintAmount) public payable {
    ...
    if(msg.sender != owner()){
    require(_mintAmount <= maxMintAmount, "You can't mint this many NFTs");
    }
    require(supply + _mintAmount <= maxSupply, "The Max supply has been met!");

    if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
         require(isWhitelisted(msg.sender), "Only whitelisted can Mint!");
         uint ownerTokenCount = balanceOf(msg.sender);
