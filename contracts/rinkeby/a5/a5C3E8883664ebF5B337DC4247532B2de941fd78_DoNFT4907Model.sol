    ...
    IDoNFTV2
{
        ...
        returns (string memory)
    {
        ...
        bool flag ;
        if(IDoNFTV2(this).getModelType() == IDoNFTV2.DoNFTModelType.WrapModel){
            if(IERC165(IWrapNFT(info.originalNftAddress).originalAddress()).supportsInterface(type(IERC721Metadata).interfaceId)){
                originalSymbol = IERC721Metadata(IWrapNFT(info.originalNftAddress).originalAddress()).symbol();
                flag = true;
            }
        }else{
            if(IERC165(info.originalNftAddress).supportsInterface(type(IERC721Metadata).interfaceId)){
                originalSymbol = IERC721Metadata(info.originalNftAddress).symbol();
                flag = true;
            }
        }
        if(!flag){originalSymbol = "unknown";}
    ...
    function _checkIn(uint256 originalNftId, address originalNftAddress, address user,  uint40 expires)
     internal virtual {
        IERC4907(originalNftAddress).setUser(originalNftId, user, uint64(expires));
    }
    ...
    function checkIn(address to, uint256 tokenId) public virtual {
        ...
        IERC4907(info.originalNftAddress).setUser(info.originalNftId, to, uint64(info.endTime));
    }
...
// File contracts/v2/DoNFT4907Model.sol
...
contract DoNFT4907Model is DoNFTV2 {
    ...
    {   
        ...
        address lastOwner = ERC721(originalNftAddress).ownerOf(originalNftId);
        ERC721(originalNftAddress).safeTransferFrom(lastOwner, address(this), originalNftId);
