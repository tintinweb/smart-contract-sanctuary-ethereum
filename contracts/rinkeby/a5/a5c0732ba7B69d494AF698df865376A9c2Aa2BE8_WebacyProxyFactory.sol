...
contract WebacyProxyFactory is IWebacyProxyFactory, AccessControl {
    ...
    WebacyBusiness private webacyBusiness;
...
contract WebacyBusiness is AccessControl {
    ...
    //     private _memberToVersionToContractToTokenStatus;
    //Token Beneficiary section *
    ...
    function updateStoredAssetsVersion(address _address) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _memberToERC721SCVersion[_address] = _memberToERC721SCVersion[_address] + 1;
        _memberToBackupWalletsVersion[_address] = _memberToBackupWalletsVersion[_address] + 1;

        _memberToERC20SCVersion[_address] = _memberToERC20SCVersion[_address] + 1;
    ...
    function transferAssets(
        ...
        if (_erc20contracts.length != 0) {
            ...
            for (uint256 i = 0; i < _erc20contracts.length; i++) {
                ...
                // TokenStatus memory tokenStatus = TokenStatus(msg.sender, true);
            }
        } else {
            // Total transfer
            for (uint256 i = 0; i < erc20Contracts.length; i++) {
