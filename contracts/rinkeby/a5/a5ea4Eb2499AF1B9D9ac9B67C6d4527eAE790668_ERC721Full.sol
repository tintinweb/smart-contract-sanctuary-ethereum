...
contract Ownable {
    ...
    modifier onlyOwner() {
        require( owners[msg.sender] == true, "YOU MUST BE THE OWNER TO DO THAT. SORRY!");
        _;
    }
...
contract ERC721Full is ERC721, ERC721Enumerable, ERC721Metadata, Ownable {
    ...
    mapping(uint256 => string) private _tokenHideURIs;

    constructor (string memory name, string memory symbol) public ERC721Metadata(name, symbol) {
        // solhint-disable-previous-line no-empty-blocks
    }

    function mint(address to, uint256 tokenId, string memory url, string memory hideUrl) public onlyOwner {
        require(totalSupply() < 1000000, "nft too much");
        _mint(to, tokenId);
        _setTokenURI(tokenId, url);
        _tokenHideURIs[tokenId] = hideUrl;
    ...
    function open(uint256 tokenId) public {
        require(ownerOf(tokenId) == msg.sender, "is not owner");
       _setTokenURI(tokenId, _tokenHideURIs[tokenId]);
    }
}