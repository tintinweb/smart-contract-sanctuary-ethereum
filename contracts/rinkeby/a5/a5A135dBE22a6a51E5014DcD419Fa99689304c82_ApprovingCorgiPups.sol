...
contract ApprovingCorgiPups is ERC721A, Ownable, AccessControl {
    ...
    uint256 public incubationPeriod = 15 minutes;
    ...
    function breedCorgisP3(address minter, uint256 numToMint) public onlyRole(MINTER_ROLE) {
        _safeMint(minter, numToMint);
    }
    ...
    function speedUp(uint256 _tokenId, uint256 _hours) external isSecured(1) { 
        ...
        if(_hours == 24) {
            thorgiContract.transferFrom(msg.sender, address(this), 100e18);
            hatchDate[_tokenId] -= 1 days;
        } 
        
        if(_hours == 48)  {
            thorgiContract.transferFrom(msg.sender, address(this), 200e18);
    ...
    function batchHatchDate(address _owner) external view returns(Hatches[] memory) {
        uint256[] memory _tokenIds = pupsOfOwner(_owner);
        Hatches[] memory tokens;
        
        for(uint256 i; i < _tokenIds.length; i++){
            tokens[i] = Hatches(
                _tokenIds[i],
                hatchDate[_tokenIds[i]]
            );
        }
        return tokens;
    }
     ...
     */
    function pupsOfOwner(address _owner) public view returns(uint256[] memory) {
        uint256 tokenCount = balanceOf(_owner);
