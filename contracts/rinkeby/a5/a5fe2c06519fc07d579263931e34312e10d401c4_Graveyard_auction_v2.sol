...
contract Graveyard_auction_v2 is Initializable, ERC1155Upgradeable, OwnableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable { /* Contract inherits from ERC1155 spec */
    ...
    function distributeEndedAuctions(uint256 endedPrice) external onlyOwner {
        for(uint256 id = 0; id < auctionsCount; id++){
            require(block.timestamp > subauctions[id].end_time, "Auction has not ended");
    ...
    function bid(uint256 id) payable public {
        ...
        if(subauctions[id].bidcount > 0){
            ...
            subauctions[id].bids[subauctions[id].bidcount].value = 0;
