    ...
    PausableUpgradeable
{
    ...
    // Proceedings for each party that needs to be payed amounts expressed in coins or tokens, not in percentages
    struct Proceedings {
        uint256 creator;
        uint256 investor;
        uint256 marketplace;
    ...
    function tradeForCoins(
        ...
        // Assemble the fees
        Proceedings memory proceedings = Proceedings({
            creator: _calculateProceedingsForFee(percentages.creator, price),
            investor: _calculateProceedingsForFee(percentages.investor, price),
            marketplace: _calculateProceedingsForFee(marketplaceFee, price)
        });

        // Calculate the base owner proceedings
        uint256 ownerProceedings = _calculateOwnerProceedings(
            price,
            proceedings
        );

        // Payments
        _sendViaCall(payable(nft.owner), ownerProceedings);
        if (proceedings.investor > 0) {
            _sendViaCall(payable(nft.investor), proceedings.investor);
        }
        if (proceedings.creator > 0) {
            _sendViaCall(payable(nft.creator), proceedings.creator);
        }
        if (proceedings.marketplace > 0) {
            _sendViaCall(payable(_autentica), proceedings.marketplace);
        }
    ...
    function tradeForTokens(
        ...
        // Payments
        IERC20(token).transferFrom(buyer, nft.owner, ownerProceedings);
        if (proceedings.investor > 0) {
        ...
        if (proceedings.creator > 0) {
            IERC20(token).transferFrom(buyer, nft.creator, proceedings.creator);
        }
        if (proceedings.marketplace > 0) {
            IERC20(token).transferFrom(
                buyer,
                _autentica,
                proceedings.marketplace
            );
        }
        // Finally transfer the NFT
        IERC721(collection).safeTransferFrom(nft.owner, _msgSender(), tokenId);
     ...
     */
    function _calculateProceedingsForFee(uint256 fee, uint256 price)
        private
        pure
        returns (uint256)
    {
    ...
    /**
     * Returns the owner proceedings.
     */
    function _calculateOwnerProceedings(
        uint256 price,
        Proceedings memory proceedings
