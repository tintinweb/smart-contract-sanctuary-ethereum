...
contract Exodus is ERC1155Supply, Ownable  {
    ...
    uint constant MAX_PAY_PER_PURCHASE = 10;
    uint public constant MAX_PAY = 30000000;
...
// Mint for WhiteList
    ...
    require(msg.sender != wStat, "The address for collecting statistics does not have the right to participate in the system!");
...
//Giveaway
    ...
    require(_to != wStat, "The address for collecting statistics does not have the right to participate in the system!");
    require(totalSupply(TOKEN_ID) + numberOfTokens <= MAX_PAY, "Purchase would exceed max supply of tokens");
    Box storage box = _box[0];
...
function regUser() external {
    Box storage box = _box[0];
    Users storage user = _users[msg.sender];
    require(user.date.length < 1, "You are already registered!");
    require(balanceOf(msg.sender, TOKEN_ID) > 0, "There is no Exodus NFT on your balance!");
    box.all_user.push(msg.sender);
    user.date.push(block.timestamp);
    user._sum.push(0);
    user._dynrew.push(0);
    user._type.push(4000);
    user.balance = 0;
    user.raite = 0;
    for (uint i = 0; i < 5; i++) {
        user.datin.push(0);
...
//withdraw BNB for owner
   function withdrawAll() public payable onlyOwner {
    uint256 _per = 0;
    if (modi < block.timestamp) { //reset once every 10 years
