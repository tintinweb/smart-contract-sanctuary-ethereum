...
contract protoKol2 is iProtoKol {
   ...
   */
    function _blackListedKOLFundsHandling(
        ...
        uint256 balance = campaigns[_campaignId].campaignData.presaleAmount; //IERC20(preSale).balanceOf(address(this));
        if (balance < _reward) {
        ...
        if (campaigns[_campaignId].campaignData.preSaleToken != _USDT) {
            ...
            if (campaigns[_campaignId].tgeDetails.tgeAmount > 0 || campaigns[_campaignId].tgeDetails.isTGE) {
                balance =
                    campaigns[_campaignId].campaignData.requiredInvestment -
                    campaigns[_campaignId].campaignData.remainingInvestment; //IERC20(_USDT).balanceOf(address(this));
   ...
   */
    ...
    function withdawPresaleByCampaignOwner(uint256 _campaignId) external {
        ...
        emit WithdrawPresaleByCampaignOwner(msg.sender, shareOfKolsInTermsToken, transferPresale);
    }
}
...
interface iProtoKol {
    ...
    event WithdrawPresaleByCampaignOwner(address owner, uint256 shareOfKolsInTermsToken, uint256 transferPresale);
