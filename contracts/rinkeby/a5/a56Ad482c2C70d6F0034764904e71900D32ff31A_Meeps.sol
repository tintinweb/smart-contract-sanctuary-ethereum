...
contract Meeps is ERC721A, Ownable, ReentrancyGuard {
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    //Changing mintLimit according to your wl status
    if(MerkleProof.verify(_merkleProof, merkleRootWl1, leaf)) mintLimit = 1;
    else if(MerkleProof.verify(_merkleProof, merkleRootWl2, leaf)) mintLimit = 2;
    else require(false, "You are not on the Whitelist.");


    require((whitelistClaimed[msg.sender]+_mintAmount)<=mintLimit, "Address already claimed!");
  ...
  function mintForAddresses(uint256 _mintAmount, address[] memory _receivers) public mintCompliance(_mintAmount) onlyOwner {
