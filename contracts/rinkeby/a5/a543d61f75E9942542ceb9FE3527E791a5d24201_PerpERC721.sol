    ...
    ERC721Burnable
{
    ...
    mapping(uint256 => Types.BidDetails) public properties;

    // contract address for perpetuals registry
    // which will be responsible for minting of these ERC721 tokens
    address public perpetualsRegistry;

    constructor() ERC721("FloorWavePerps", "FWP") {
    ...
    function setPerpetualsRegistry(address _perpetualsRegistry)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _grantRole(PAUSER_ROLE, _perpetualsRegistry);
        _grantRole(MINTER_ROLE, _perpetualsRegistry);
        perpetualsRegistry = _perpetualsRegistry;
    }
    ...
    function safeMint(address to, Types.BidDetails memory _props)
        external
    ...
    function burn(uint256 tokenId)
        public
        virtual
        override(ERC721Burnable, IPerpERC721)
    {
        require(
            _isApprovedOrOwnerOrRegistry(_msgSender(), tokenId),
            "ERC721Burnable: caller is not owner nor approved nor registry"
        );
        _burn(tokenId);
    }
    ...
    function _isApprovedOrOwnerOrRegistry(address spender, uint256 tokenId)
        internal
        view
        virtual
        returns (bool)
