...
contract SimplePool is Ownable, ERC721Holder, IERC1271 {
    ...
    address immutable openSeaProxyRegister;
    address immutable openSeaTokenProxy;
    address immutable openSeaExchange;
    ...
    function prepareOpenSea() external onlyOwnerOrManager {
        ...
        require(wrappedEther.approve(openSeaTokenProxy, type(uint).max));
    }

    function exchangeOpenSea(bytes calldata _calldata) external onlyOwner {
        (bool _success, bytes memory _returnData) = openSeaExchange.call(_calldata);
