...
contract Qwerty is Ownable, ERC721A, ReentrancyGuard {
    ...
    event QwertyMinted(address indexed mintAddress, uint8 indexed numTokens, uint8 mintType);
    ...
    function mint(uint8 numTokens) external payable callerIsUser {
        ...
        if (msg.value > 0) {
            ...
            emit QwertyMinted(msg.sender, numTokens, 2);
        } else {
            require(freeSupplyLeft > 0, "Reached free mint supply");
            ...
            _safeMint(msg.sender, numTokens);
            emit QwertyMinted(msg.sender, numTokens, 0);
        }
    }
    ...
    function reservedMint(uint8 numTokens) external callerIsUser {
        ...
        freeSupplyLeft = freeSupplyLeft - numTokens;
        _safeMint(msg.sender, numTokens);
        emit QwertyMinted(msg.sender, numTokens, 1);
    }
    
    function ownerMint() external onlyOwner {
        require(!ownerMinted, "Owner already minted");
        ownerMinted = true;
        freeSupplyLeft = freeSupplyLeft - 1;
        _safeMint(msg.sender, 1);
        emit QwertyMinted(msg.sender, 1, 0);
    }
