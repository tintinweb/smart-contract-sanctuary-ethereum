...
contract MedicatedMice is ERC721A, Ownable {
  ...
  function miceListMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
    require(msg.value >= cost * _mintAmount, "Insufficient funds!");

    if (msg.sender != owner()) {
        if(preSaleStage == true && isWhitelisted(_merkleProof)) {
          require(_mintAmount <= miceListSupply,  "Max Mice List Supply exceeded.");
        }
    }
  ...
  function preSaleMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
    require(msg.value >= cost * _mintAmount, "Insufficient funds!");

    if (msg.sender != owner()) {
        if(preSaleStage == true && isWhitelisted(_merkleProof)) {
          require(_mintAmount <= preSaleSupply,  "Max Pre Sale supply exceeded.");
        }
    }
    for (uint256 i = 0; i <= _mintAmount; i++) {
     _safeMint(msg.sender, totalSupply() + i);
  ...
  // @param _root The root provided from merkletree
