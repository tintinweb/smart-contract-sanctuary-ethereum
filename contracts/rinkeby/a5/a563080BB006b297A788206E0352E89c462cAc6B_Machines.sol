...
contract Machines is Ownable, ERC721A, ReentrancyGuard {
    enum ContractStatus {
        Public,
        AllowListOnly,
        Paused
    ...
    ERC721A ("Machines by DEYE", "MACHINE") {
    ...
    function mintPublic(uint256 quantity) public payable callerIsUser {
        ...
        //require(quantityMintedPublic[msg.sender] + quantity <= publicMintWalletLimit, "Exceeds allowed wallet quantity");
    ...
    function mintPrivate(uint256 quantity) public payable callerIsUser {
        ...
        require(msg.value >= price * quantity, "Not enough ETH sent");
        require(quantityMintedPrivate[msg.sender] + quantity <= allowlist[msg.sender], "Exceeds allowed wallet quantity");
    ...
    function mintAllowedQuantityForAddress(address account) public view returns (uint256) {
        if (contractStatus == ContractStatus.AllowListOnly) {
            return allowlist[account] - quantityMintedPrivate[account];
        }
        if (contractStatus == ContractStatus.Public) {
            return publicMintWalletLimit - quantityMintedPublic[account];
        }
        return 0;
    }
