...
contract Shibnobi is IERC20, Ownable {
    ...
    address constant public burnWallet = address(0);
    ...
    constructor () {
        ...
        uint256 totalEthSide = 69e8 * 10 ** _decimals;
        
        _tTotal = 69e8 * 10 ** _decimals;
        uint256 r = totalEthSide / _tTotal;
        _rTotal = (MAX - (MAX % _tTotal)) / r;     
        ...
        _taxFeeTransfer = 100;
        _marketingFeeTransfer = 100;
        _liquidityFeeTransfer = 100;
        _burnFeeTransfer = 100;

        _feeDenominator = 1000;
        
        minETHValueBeforeSwapping = 1 * 10 ** 10;
        ...
        bridgeAddress = 0x073200f8Ed07bE21CD72231A22eC80eBFA6a4cAf;
        //_tokenTransfer(owner(), 0x27F63B82e68c21452247Ba65b87c4f0Fb7508f44, 1e7 * 10 ** decimals(), false);
        emit Transfer(address(0), _msgSender(), _tTotal);
    }

    function setBridgeAddress(address newAddress) external onlyOwner {
    ...
    function totalSupply() public view override returns (uint256) {
        return _tTotal - balanceOf(bridgeAddress);
    }
    ...
    function includeInReward(address account) external onlyOwner() {
        require(account != burnWallet && account != address(0), "Don't include it, it's not a good idea");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (hasLiquidity && !inSwapAndLiquify && !automatedMarketMakerPairs[sender] && recipient != burnWallet){
            _handleLimited(sender, amount * (_feeDenominator - _taxFee - _liquidityFee - _marketingFee - _burnFee) / _feeDenominator);
        }
