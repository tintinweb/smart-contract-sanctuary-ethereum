...
contract sale is SaleContract {
    ...
    function Rinkeby_setBeforePresale() public {
        block_timestamp = _presaleStart - 1;
    }

    function Rinkeby_setInPresale() public {
        block_timestamp = _presaleStart + 1;
    }

    function Rinkeby_setInSale() public {
        block_timestamp = _saleStart + 1;
    }

    function Rinkeby_setAfterSale() public {
        block_timestamp = _saleEnd + 1;
    }

    function Rinkeby_setSigner(address _sig) public {
        _projectSigner = _sig;
    }

    function Rinkeby_setPreRevealURI(string calldata _tokenPreRevealURI)
        public
    {
        token.setPreRevealURI(_tokenPreRevealURI);
    }

    function Rinkeby_setRevealURI(string calldata _tokenRevealURI) public {
        token.setRevealURI(_tokenRevealURI);
    }

    function Rinkeby_revealAtCurrentSupply() public {
        token.revealAtCurrentSupply();
    }

    function getRealBlockTimestamp() public view returns (uint256) {
...
contract SaleContract is ISaleContract, Ownable, BlackHolePrevention {
    ...
    constructor(SaleConfiguration memory config) {
        ...
        // GRD overrides this so we can deploy a DUST "sale" starting at 3k upto 9k
    ...
    /**
     * @dev Mint the 50 reserved tokens
     */
    ...
    function mint(uint256 _numberOfCards) external payable {
        ...
        uint256 _sold = _mintedByWallet[msg.sender];
        ...
        emit Sale(msg.sender, msg.sender, number_of_items, msg.value);
    }
