...
contract NFT is ERC721 {
  ...
  mapping (uint => uint) public itemNft;
  mapping (uint256 => string) private _tokenURIs;

  uint maxValue = 100000; // Max value of an item
  ...
  */
  ...
  function getNftUniqueId() external view returns (uint){
    return _nftCounter;
  }

  function getNftId(uint nftUniqueId) external view returns (uint){
    return itemNft[nftUniqueId];
  }
  ...
  function getItem(uint nftUniqueId ) payable public returns (uint256) {
    require(msg.sender != address(0));
    require(msg.value == 1 ether); // 0.5 AVAX is the cost of an item
    ...
    itemValue[newItemId] = random() % maxValue; 
 
    itemNft[nftUniqueId] = newItemId;
    // Set the item value to a random number modulus used to make sure that the value isn't bigger than maxValue
