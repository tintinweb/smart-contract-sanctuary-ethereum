...
contract PccToken is ERC20, Ownable, ExclusivesYieldMap {
    ...
    bytes32 public MerkleRoot = 0xe0e5ed585fab11ca91136127d7c1f57d225275c75797737af56e61d1957c20ee;
    ...
    IERC721 public EXCLUSIVES_CONTRACT = IERC721(address(1));

    uint256 public constant EXCLUSIVES_START_TIME = 1658514630;
    ...
    constructor() ERC20("PCCT", "PCC Token", 18) {
        TierTwoItem = new PccTierTwoItem(address(this), msg.sender);

        FirstClaimTime[
            IERC721(0x9759226B2F8ddEFF81583e244Ef3bd13AAA7e4A1)
        ] = 1626539430; //PCC
        FirstClaimTime[
            IERC721(0x0c6218D95735d3E12AE7C4703106E4b8e0b61010)
        ] = 1637015430; //kittens
        FirstClaimTime[
            IERC721(0x9e5bF3026A4F77971057cad44C0B6e02EFA9a7cD)
        ] = 1652725830; //grandmas

        addNewContract(IERC721(address(TierTwoItem.TierTwo())), 5);

        if (block.chainid == 1){
            addNewContract(IERC721(0x9759226B2F8ddEFF81583e244Ef3bd13AAA7e4A1), 10);
            addNewContract(IERC721(0x0c6218D95735d3E12AE7C4703106E4b8e0b61010), 1);
            addNewContract(IERC721(0x9e5bF3026A4F77971057cad44C0B6e02EFA9a7cD), 1);
        }
        else{
            addNewContract(IERC721(address(new Mock721())), 10);
            addNewContract(IERC721(address(new Mock721())), 1);
            addNewContract(IERC721(address(new Mock721())), 1);
        }
    }
    ...
    function setLastClaimedTime(IERC721 _contract, uint256 _id) public {
        LastClaimedTimes[_contract][_id] = block.timestamp;
    }
        ...
        onlyOwner
    {
        ...
        amountToMint +=
            (uint256(
                (block.timestamp - bucket.LastClaimTimestamp) *
                    (uint256(bucket.DailyYield))
            ) * SECOND_RATE_FOR_ONE_PER_DAY) /
            86400;

        bucket.CurrentYield += uint64(_amount);
        require(
        ...
        bucket.AvailableCurrently = uint64(amountToMint - uint256(_amount));
...
contract PccTierTwoItem is ERC1155, Ownable {
    ...
    PccToken immutable TokenContract;
    ...
    constructor(address _token, address _owner) {
        TokenContract = PccToken(_token);
        TierTwo = new PccTierTwo(address(this));
...
contract Mock721 is ERC721 {
    ...
    function mintToken(address _addr, uint256 _id) public {
        _mint(_addr, _id);
    }
...
contract PccTierTwo is ERC721, Ownable {
    ...
    constructor(address _ticketContract) ERC721("PCC Tier Two", "PTT") {
    ...
    function mint(
        ...
        for (uint256 i; i < _quantity; ) {
            ...
            _safeMint(
                _to,
                ((Ids[_ticketId][index] == 0) ? index : Ids[_ticketId][index]) +
