...
contract BrokerDefiPartner is ERC721, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => uint16) public partnerCodes;

    // mapping for partner codes to token ids
    mapping(uint16 => uint256) public codeOwners;

    // additional mapping for partner codes verification for easy one step code check
    mapping(uint16 => bool) public partnerCodesVerification;
    ...
    */
    ...
    function setCode(uint tokenId) private {
        uint16 code = uint16(block.timestamp + tokenId);
    ...
    function publicMint(uint amount, uint16 code) public payable nonReentrant {
