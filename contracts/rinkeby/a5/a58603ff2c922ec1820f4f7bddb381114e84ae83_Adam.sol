...
contract Adam is Initializable, UUPSUpgradeable {
    ...
    function getConstantWETH() public pure returns (address) {
        return Constant.getWETH();
    }

    function getConstantAddress() public view returns (address) {
        return Constant.getConstantAddress();
    }
}
...
interface IDao {
    ...
    function initialize(
        ...
        uint256[3] memory budgetApproval,
        uint256[3] memory revokeBudgetApproval,
        uint256[3] memory general
    ) external;
...
library Constant {
    address public constant WETH_ADDRESS = address(0x0);
    address public constant UNISWAP_ROUTER = address(0x0);
