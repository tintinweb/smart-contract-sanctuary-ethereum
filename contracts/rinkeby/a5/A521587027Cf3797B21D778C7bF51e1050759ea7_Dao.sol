...
contract Dao is Initializable, UUPSUpgradeable, MultiToken, IDao, ERC721HolderUpgradeable {
    ...
    //     if (memberTokenId == 0) {
    //         address member = IMembership(_membership).createMember(_from);
...
contract MultiToken is ERC1155Upgradeable {
    ...
    function tokenName(uint256 _id) public view returns (string memory) {
        ...
        return IERC20Metadata(tokenRegistry[_id].contractAddress).name();
    }

    function tokenDecimals(uint256 _id) public view returns (uint8) {
        if (_id == ethId) return 18;
        try IERC20Metadata(tokenRegistry[_id].contractAddress).decimals() returns (uint8 _decimals) {
            return _decimals;
        } catch {
            return 0;
        }
