...
import "@openzeppelin/contracts/access/Ownable.sol";
import "./approving-corgis.sol";
import "./corgi-pups.sol";
import "./thorgi.sol";
import "./ERC721A.sol";

interface IApprovingCorgiPups {
    function breedCorgisP2(address minter, uint256 numToMint) external;
}

contract ApprovingCorgiPupsP2 is Ownable, ERC721A {
    using Address for address;
    ...
    address public pupsAddress = 0xc9f9E68526dCFb7207DC0b65F16c2d26563dAF85;

    IERC721Enumerable public constant corgiContract = IERC721Enumerable(corgiAddress);
    IApprovingCorgiPups public pupsContract = IApprovingCorgiPups(pupsAddress);

    address signer;

    constructor() ERC721A("Approving Corgi Pups Caller", "APUPPIESCALLER", 50, 9999)  {}
    ...
    modifier isSecured(uint8 mintType) {
        ...
        if(mintType == 2) {
            require(isPhase2Active, "PHASE_2_IS_NOT_YET_ACTIVE");
        }
        _;
    }
     ...
     * Breed Corgis
     */
    function breedCorgis(uint256 _parent1, uint256 _parent2, uint64 expireTime, bytes memory sig) external isSecured(2) {
        ...
        require(numberOfBreedTimes[_parent1] < numberOfAllowedBreedingP2, "REACHED_MAXIMUM_BREED_ALLOWED_P1");
        require(numberOfBreedTimes[_parent2] < numberOfAllowedBreedingP2, "REACHED_MAXIMUM_BREED_ALLOWED_P1");
        ...
        pupsContract.breedCorgisP2( msg.sender, 1 );
    }

    function testBreed() external isSecured(2) {
        // bytes32 digest = keccak256(abi.encodePacked(msg.sender,expireTime));
        // require(isAuthorized(sig,digest),"CONTRACT_MINT_NOT_ALLOWED");
        // require(corgiContract.ownerOf(_parent1) == msg.sender, "YOU_DO_NOT_OWN_CORGI_1");
        // require(corgiContract.ownerOf(_parent2) == msg.sender, "YOU_DO_NOT_OWN_CORGI_2");
