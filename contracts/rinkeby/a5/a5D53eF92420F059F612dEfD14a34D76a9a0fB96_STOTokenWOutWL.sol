...
contract STOTokenWOutWL is ContextUpgradeable, AccessControlUpgradeable, OwnableUpgradeable, ERC20CappedUpgradeable, ERC20PausableUpgradeable {
    ...
    constructor(string memory name, string memory symbol, uint256 supply, uint256 minPoolPercent, uint256 maxPoolPercent) {
      initialize(name, symbol, supply, minPoolPercent, maxPoolPercent);
    }
    
    function initialize(string memory name, string memory symbol, uint256 supply, uint256 minPoolPercent, uint256 maxPoolPercent) public initializer {
    ...
    function fullySold() public view returns(bool){
      ...
      if (totalSupply() >= cap - (cap * (_maxPoolPercent) / (100))) {
        return true;
      }
      return false;
    }
    ...
    function couldReceiveContribution(uint256 tokens) public view returns(bool){
      uint256 cap = cap();
      if (cap - (cap * (_minPoolPercent) / (100)) >= totalSupply() + (tokens)) {
        return true;
      }
      return false;
    }
    ...
    function getPoolTokensByAmount(uint256 amount) public onlyOwner {
      require(cap() - (totalSupply()) >= amount, "STOToken: not enough funds");
