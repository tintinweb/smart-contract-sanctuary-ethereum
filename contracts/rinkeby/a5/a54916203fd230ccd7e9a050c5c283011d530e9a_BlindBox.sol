...
contract BlindBox is OwnableUpgradeable, Verify {
    ...
    function buyBox(uint256 _id, bytes memory data) external payable {
        ...
        bytes32 _hash;
        _hash = keccak256(abi.encodePacked(sender, _id, solt));
        require(verify(_hash, data), "buyBox: Authentication failed");
        ...
        require(timestamp > box.startTime && timestamp < box.endTime, "BuyBox: no this time");
        require(box.totalSupply > totalSell[_id] || !soldOut[_id], "BuyBox: box is sold out");
        require(box.purchaseLimit == 0 || box.purchaseLimit > userPurchase[sender][_id], "BuyBox: not enought quota");
        require(box.price == msg.value, "BuyBox: invalid amount");

        // 使用ape换成使用eth/bnb
        // box.token.transferFrom(sender, address(this), box.price);

        uint256 ftTimes = randomTimes(box.propsNum, box.weightProp);
        seed = seed.add(box.propsNum);

        for (uint256 i=0; i<box.propsNum.sub(ftTimes); i++) {
          uint256 _tokenID = randomDraw(_id);
          ERC721.mintTo(sender, _tokenID);
          emit GetNFTByBox(sender, _id, _tokenID);
        }

        if (ftTimes > 0) {
          tokenFT.adminMint(sender, ftTimes.mul(box.tokenNum));
        }

        totalSell[_id] = totalSell[_id].add(1);
        userPurchase[sender][_id] = userPurchase[sender][_id].add(1);
        if (box.totalSupply <= totalSell[_id]) {
          soldOut[_id] = true;
        }
        emit BuyBox(sender, _id);
    }

    function randomTimes(uint256 len, uint256 weight) internal returns(uint256) {
      uint256 times;
      for (uint256 i=0; i<len; i++) {
        uint256 _num = randomNum(100);
        if (_num > weight) {
          times = times.add(1);
        }
      }
    ...
    function setBoxOpen(uint256 _id, bool _open) external onlyAdmin {
        ...
        Box memory box = boxs[_id];
        uint256 unSupply = box.totalSupply.sub(totalSell[_id]);
        canceled[_id] = _open;
        emit Cancel(_id, box.totalSupply, unSupply);
    }
    ...
    function deleteBox(uint256 _id) external onlyAdmin {
        ...
        emit Delete(_id, box.totalSupply, unSupply);
    }
