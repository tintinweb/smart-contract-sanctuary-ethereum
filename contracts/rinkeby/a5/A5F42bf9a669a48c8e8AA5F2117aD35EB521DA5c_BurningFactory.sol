...
contract Operators is Common  {
    ...
    modifier onlyOperator() {
        require(msg.sender == operator1 || msg.sender == operator2, "the sender is not the rescuer");
        _;
    }

    function initializeOperators(address _account1, address _account2)
        internal isNotZeroAddress(_account1) isNotZeroAddress(_account2) {
        require(operator1 == address(0), "the operator1 can only be initiated once");
        require(operator2 == address(0), "the operator2 can only be initiated once");
...
contract Rescuer is Common  {
    ...
    function initializeRescuer(address _account) internal isNotZeroAddress(_account) {
...
contract Token_v3 is Token_v2, Rescuer, Operators {
    ...
    modifier notExecuted(uint transactionId) {
        require (!mintTransactions[transactionId].executed, "mint transaction had been executed.");
        _;
    }
    ...
    function changeOperator(address _account, uint256 _index) public onlyAdmin whenNotPaused isNotZeroAddress(_account) {
        ...
        if(_index == 1){
            ...
            operator1 = _account;
            emit OperatorChanged(old, operator1, 1, msg.sender);
        }else{
            address old = operator2;
            operator2 = _account;
            emit OperatorChanged(old, operator2, 2, msg.sender);
        }
    }

    function addMintTransaction(address _acount, uint _amount)
        internal
        returns (uint transactionId)
    {
        require(_acount != address(0), "mint destination is the zero address");
        ...
        emit PendingMintTransaction(transactionId, _acount, _amount, msg.sender);
    }
    ...
    function initializeV3(address _rescuer, address _operator1, address _operator2) public {
