...
contract LENDPOOL is IMasterContract{
    ...
    function encode(address _colleteral, address _borrowabletoken)internal pure returns (bytes memory){
        return abi.encode(_colleteral,_borrowabletoken);
    ...
    function createPool(ERC20 _collateral, address _borrowabletoken, string calldata _ticker)external payable{
       ...
       // if (!callSuccess)revert FAILURE();
        ...
        bytes memory data =abi.encode( _collateral, _borrowabletoken, _ticker);
