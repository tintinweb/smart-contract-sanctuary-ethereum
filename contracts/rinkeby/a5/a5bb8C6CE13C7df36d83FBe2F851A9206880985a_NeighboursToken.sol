...
contract NeighboursToken is ERC721, Ownable {
    ...
    // Start ID for unique Neighbours
    uint constant UNIQUE_IMAGE_ID_START = 10000;

    // Tokens and unique tokens counters
    Counters.Counter private _tokenIdCounter;
    Counters.Counter private _uniqueImageIdCounter;

    // Default metadata CID in IPFS
    string private _defaultMetaCID;

    // Maps
    mapping(uint => string) private _tokenCIDs; // CID for token
    mapping(uint => uint) private _imageTokens; // token ID for image
    mapping(uint => bool) private _tokenPaintingOrdered; // is painting ordered for token

    // Mint allowing flag
    bool private _mintAllowed = false;

    // Current images pool for minting with length
    ...
    // Prevent call function if minting not allowed
    modifier ifMintAllowed() {
        require(_mintAllowed, "NeighboursToken: minting is not allowed");
        _;
    }

    // Prevent call function from any wallets except COO or owner
    modifier onlyCOO() {
        require(
            (owner() == _msgSender()) ||
    ...
    // Get current images pool for minting
    ...
    // Set current images pool for minting
    ...
    // Order original painting for token
    ...
    // Set token metadata CID oin IPFS
