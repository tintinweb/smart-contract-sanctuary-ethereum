...
@external
def mintDyad(tokenId: uint256, wethAmt: uint256, poolDepositAmt: uint256 = max_value(uint256)):
    assert msg.sender == self.idToOwner[tokenId]
    ...
    # if we're depositing everything
    if poolDepositAmt == max_value(uint256):
        ...
        assert new_dyad >= poolDepositAmt # fail if we didn't mint enough
        self.dyad.approve(self.dyad_pool.address, poolDepositAmt)
        dyad_returned: uint256 = new_dyad - poolDepositAmt
        self.dyad_pool.deposit(poolDepositAmt)
        self.virtualDyadBalance[tokenId] += convert(poolDepositAmt, int256)
        self.erc20_safe_transfer(self.dyad.address, msg.sender, dyad_returned)
