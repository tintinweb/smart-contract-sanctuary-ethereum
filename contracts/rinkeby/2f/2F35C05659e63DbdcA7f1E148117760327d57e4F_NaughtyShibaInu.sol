...
contract NaughtyShibaInu is ERC20, Ownable{
  ...
  uint public _lpBurnBasisPoints = 25; //25 = .25%
  ...
  constructor(
    ...
    _swapTokensThreshold = 500_000 * 1e18; // .0125% from total supply
    ...
    // Mint only once upon inception. No more coins can be minted after this.
  ...
  /// @notice Control the settings for auto burn of tokens from liquidity pool
  /// @param frequencyInSeconds Frequency of calling auto burn in seconds
  /// @param basisPoints Amount to burn in basis points (e.g. 10 = 0.1%)
  /// @param enabled Toggle enable/disable
  function setAutoLPBurnSettings(
    ...
    require(frequencyInSeconds >= 600, "burn frequency < 10 minutes");

    require(basisPoints <= 1000, "LP burn > 10%");

    _lpBurnFrequency = frequencyInSeconds;
    _lpBurnBasisPoints = basisPoints;
    _lpBurnEnabled = enabled;
  }

  /// @notice Manually burn tokens from liquidity pool
  /// @param basisPoints Amount to burn in basis points (e.g. 10 = 0.1%)
  ...
  function _transfer(address from, address to, uint amount) internal override {
    ...
    // Apply buy / sell taxes. Only take fees on buys / sells on AMM.
    // Do not take fees on wallet transfers.
    if(takeFee){
      ...
      if(_AMMPairs[to] && _sellTotalFees > 0) {
        ...
        // User is selling tokens. Calculate the total sell fees to be taken
        ...
        // User is buying tokens. Calculate the total buy fees to be taken
  ...
  function _autoBurnTokensFromPool() internal {
    ...
    // Calculate amount to burn
    uint amountToBurn = poolTokenBalance * _lpBurnBasisPoints / 10000;
