...
contract Joseph is Ownable, RoyaltiesAddon, ERC2981 {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public MAX_ELEMENTS = 5000;
    uint256 public PRICE = 4 ether;
    ...
    event welcomeToJoseph(uint256 indexed id);
    event NewPriceEvent(uint256 price);
    event NewMaxElement(uint256 max);

    constructor(string memory baseURI) ERC721("Joseph", "JH") {
        setBaseURI(baseURI);
    }
