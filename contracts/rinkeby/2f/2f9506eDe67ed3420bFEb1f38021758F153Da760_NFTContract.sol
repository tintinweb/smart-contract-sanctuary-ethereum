...
contract NFTContract is ERC721A, EIP712Whitelisting, VRFv2Consumer {
    constructor() ERC721A("NFT Contract", "NFTC") {}

    bool public REVEALED = false;
    string public _baseURI;
    string public _placeholderURI;
    mapping(address => uint256) public addressDonated;
    mapping(uint256 => uint256) public nftDonated;

    event Donation(address indexed _from, uint256 [] _tokenIds, uint _value);
    ...
    function safeMint(uint256 quantity, bytes calldata signature) public requiresWhitelist(signature) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (REVEALED) {
            uint256 offset = tokenId + getOffset();
            if (offset > 9999) offset = offset - 10000;
            return string(abi.encodePacked(_baseURI, _toString(offset)));
        }
        return _placeholderURI;
    }

    function reveal(bool status, string memory uri) external onlyOwner {
        REVEALED = status;
        setBaseURI(uri);
    }
    ...
    function getOffset() public view returns (uint256) {
        uint256 offset = getRandomWord() % 10000;
        return offset;
    }

    function donate(uint256 [] calldata tokenIds) public payable {
        uint256 valuePerNft = msg.value / tokenIds.length;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (!_exists(tokenIds[i])) revert URIQueryForNonexistentToken();
            nftDonated[tokenIds[i]] += valuePerNft;
        }
        addressDonated[msg.sender] += msg.value;
        emit Donation(msg.sender, tokenIds, msg.value);
    }

    function getAddressDonated(address addr) public view returns (uint256) {
        return addressDonated[addr];
    }

    function getNftDonated(uint256 tokenId) public view returns (uint256) {
        return nftDonated[tokenId];
    }
}
...
contract VRFv2Consumer is VRFConsumerBaseV2, Ownable {
  ...
  // Settings are for rinkeby test network
  uint64 s_subscriptionId = 20794;
  ...
  function _randomWords() internal view virtual returns (uint256) {
      return s_randomWords[0];
  }

  function overrideRandom(uint256[] memory value) external onlyOwner {
    s_randomWords = value;
  }
}
