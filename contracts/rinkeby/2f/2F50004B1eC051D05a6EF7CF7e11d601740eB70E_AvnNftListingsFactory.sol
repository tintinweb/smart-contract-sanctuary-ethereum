...
contract AvnNftListingsFactory {
  address constant beacon = 0x45A8cde3AA8EE3C03D2a08D1af71706282091b78;
  event LogNewClient(address tokenAddress);
    ...
    (address)
  {
    BeaconProxy proxy = new BeaconProxy(beacon,
        abi.encodeWithSelector(AvnNftListingsV1.initialize.selector, _clientName, _initialAuthority));
...
contract AvnNftListingsV1 is IAvnNftListingsV1 {
    ...
    returns(string memory uuid_)
  {
    ...
    uuid_ = string(abi.encodePacked(
      hexSlice(halfNftId, 0, 2), "-",
      hexSlice(halfNftId, 2, 4), "-",
