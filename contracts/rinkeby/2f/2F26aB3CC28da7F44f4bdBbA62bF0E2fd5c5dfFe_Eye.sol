...
contract Eye is ERC721, Ownable {
    ...
    uint256 public immutable pricePerEYE;
    uint256 public immutable maxPerTx;
    uint256 public immutable maxEye;
    ...
    function reserveEYEz() public onlyOwner {   
     ...
     function mintEYEz(uint256 numberOfTokens) public payable {
        require(isSaleActive, "Sale is not active");
        require(numberOfTokens <= maxPerTx, "No more than 20 tokens per transaction");
        require(totalSupply().add(numberOfTokens) <= maxEye, "Purchase would exceed max supply of EYEz");
        require(pricePerEYE.mul(numberOfTokens) == msg.value, "Ether value is not correct");
        
        payable(owner()).transfer(msg.value);

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply();
            if (totalSupply() < maxEye) {
