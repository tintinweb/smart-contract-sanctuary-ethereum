...
contract PionautMarket is ReentrancyGuard, Ownable, Pausable {
        ...
        nonReentrant
    {
        ...
        if (IRoyaltyContract(nftContract).hasRoyalityFee()){
            require(IRoyaltyContract(nftContract).getRoyaltyFee() < maxRoyaltyFee, "Should be restricted");
        ...
        idToMarketItem[itemId].seller.transfer(msg.value - fee - royaltyFee);        
    ...
    function countCreatedNFTs() public view returns (MarketItem[] memory) {
        uint256 totalItemCount = _itemIds.current();
        uint256 itemCount = 0;
        uint256 currentIndex = 0;
...
interface IRoyaltyContract {
    function hasRoyalityFee() external view returns (bool);
