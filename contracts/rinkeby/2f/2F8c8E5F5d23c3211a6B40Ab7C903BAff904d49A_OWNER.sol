...
contract OWNER is Ownable, ERC721A {
    ...
    function setWhitelist(
        ...
        require(addresses.length == eligible.length, "array length mismatch");
        for (uint256 i = 0; i < addresses.length; i++) {
            whitelist[addresses[i]] = eligible[i];
        }
    }

    function setPassClaim(
        address[] memory addresses,
        uint256[] memory amountToClaim
    ) external onlyOwner {
        require(addresses.length == amountToClaim.length, "array length mismatch");
        for (uint256 i = 0; i < addresses.length; i++) {
            passClaim[addresses[i]] = amountToClaim[i];
        }
    }

    function withdrawFunds(address payable addr1, address payable addr2)
        external
        onlyOwner
    {
        uint256 balance = address(this).balance;
        payable(addr1).transfer((balance * 96) / 100);
        payable(addr2).transfer((balance * 4) / 100);
    }
