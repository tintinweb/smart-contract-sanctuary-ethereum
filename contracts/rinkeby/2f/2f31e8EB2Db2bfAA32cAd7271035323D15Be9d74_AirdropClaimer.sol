...
contract AirdropClaimer is Oracle {

    uint _id;
    string _urlToQuery;
    string _attributeToFetch;
    uint _headOffset;
    uint _tailOffset;
    uint public _response;
    Oracle public ContractOracle;

    constructor( Oracle _Oracle ) {
        _urlToQuery = "http://13.59.27.85:5000/api/";
        _attributeToFetch = "circulating_supply";
        _headOffset = 23;
        _tailOffset = 2;
        ContractOracle = _Oracle;
    }

    function requestClaim() public {
        ContractOracle.createRequest( _urlToQuery, _attributeToFetch, _headOffset, _tailOffset );
    }

    function responseClaim() public {
        string memory responseValue = ContractOracle._getResult( _id );
        _response = ContractOracle.stringToUint( responseValue );
    }
...
contract Oracle {
    ...
    function _strLength( string memory s ) internal pure returns ( uint256 ) {
    ...
    function applyOffset ( uint _id, uint headOffset, uint tailOffset ) public view returns ( string memory result ) {
    ...
    function stringToUint( string memory _str ) public pure returns( uint256 result) {
