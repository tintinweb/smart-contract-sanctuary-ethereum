...
contract OVRRegistrar is ERC721Full, AccessControl {
     ...
     */
    function register(string calldata _subdomain, address _beneficiary, string memory _land)
        external
    ...
    function setResolver(uint256 _tokenId, address _resolver) internal {
        require(
        ...
        bytes32 label = keccak256(
            abi.encodePacked(_toLowerCase(subdomains[bytes32(_tokenId)]))
        );
        bytes32 node = keccak256(abi.encodePacked(domainNameHash, label));
    ...
    function changeNotice(uint256 _tokenId, string memory _value) public {
        ...
        bytes32 node = keccak256(abi.encodePacked(domainNameHash, label));
        IResolver resolver = IResolver(registry.resolver(node));
        resolver.setText(node, "notice", _value);
    }
    ...
    function _register(
        ...
        setResolver(uint256(subdomainLabelHash), address(resolver));
        changeNotice(uint256(subdomainLabelHash), _land);
        transferFrom(address(this), _beneficiary, uint256(subdomainLabelHash));
