...
contract DotcMain is owned {
   ...
   */
	...
	require(sides.length == order_ids.length && sides.length > 0, "bounds check");
    ...
    function getTokenAvailableAmount(address token, address account) view public returns(uint256) {
	...
	if(len>0)
	{
		...
		for(uint i=0; i<len; i++)
		{
			...
			amounts[i] = getTokenAvailableAmount(tk.addr, account); 
		}
	}
    }
    //获得用户某个账户的抵押资产额度(USD)
