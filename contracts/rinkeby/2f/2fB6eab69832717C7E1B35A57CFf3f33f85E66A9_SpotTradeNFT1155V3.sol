...
contract SpotTradeNFT1155V3 is ERC1155 {
    ...
    */
    ...
    function mint(string memory uri, uint256 supply, address[] memory royaltyAddress, uint256[] memory royaltyFee, Sign memory sign) external {
        require(!usedNonce[sign.nonce], "Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        verifySign(uri, _msgSender(), sign);
        _mint(msg.sender, tokenCounter, supply, uri);
        setRoyaltyFee(tokenCounter, royaltyAddress, royaltyFee);
        tokenURIs[uri] = true;
        tokenCounter += 1;
    }
