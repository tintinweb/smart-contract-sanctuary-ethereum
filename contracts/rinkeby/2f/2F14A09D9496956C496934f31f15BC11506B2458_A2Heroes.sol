...
contract A2Heroes is ERC721, ERC721Enumerable, ERC721Burnable, AccessControl, Ownable {
  ...
  function sendRoyalty(uint256 id, uint64 amount) public {
    uint256 royaltyFee = tokenPrice[id].mul(royaltyRate).div(100);
    uint256 royaltyAmount = royaltyFee * amount;
  ...
  function ownerMint(string memory _name, string memory cid, uint256 characterId) public payable onlyOwner{
    ...
    function batchMint(string memory _name, string memory cid, uint256[] calldata characterId) public payable{
      for (uint idx=0; idx < characterId.length; idx++){
        ...
        require(_mintedAmount[msg.sender] < walletLimit, "Exceeds personal wallet limits");
        require(totalSupply() < MAX_SUPPLY, "Exceeds max supply of contract");
        require(msg.value >= character_price[characterId[idx]], "The price is not correct");
  ...
  function presaleMint(string memory _name, string memory cid, uint256 characterId) public payable {
  ...
  function transferOwnership(address newOwner) public virtual override onlyOwner {
    ...
    revokeRole(DEFAULT_ADMIN_ROLE, currentAdmin);
    revokeRole(MINT_ROLE, currentAdmin);
    revokeRole(REDEEM_ROLE, currentAdmin);
    revokeRole(BURI_ROLE, currentAdmin);
    revokeRole(PRESALE_ROLE, currentAdmin);

    currentAdmin = newOwner;
    _setupRole(DEFAULT_ADMIN_ROLE, newOwner);
    _setupRole(MINT_ROLE, newOwner);
