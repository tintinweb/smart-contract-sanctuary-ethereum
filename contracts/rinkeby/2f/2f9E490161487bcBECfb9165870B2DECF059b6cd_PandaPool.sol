...
contract PandaPool is Ownable, ReentrancyGuard, IERC721Receiver, VRFConsumerBaseV2, IPandaPool {
    ...
    mapping(uint256 => SwapInfo) public reqSwaps;
    mapping(uint256 => BuyInfo) public reqBuys;
    ...
    mapping(uint256 => uint8) reqToFunc;

    uint256 constant buyGas = 1_243_315;
    uint256 constant rebornGas = 1_160_269;
    uint8 public constant REBRON_FUNC_TYPE = 1;
    uint8 public constant BUY_FUNC_TYPE = 2;
    ...
    event Reborn(address indexed recipient,uint256 ownerTokenId, uint256 targetTokenId, uint256 fee);
    ...
    event Request(uint256 requistId);
    event FullFil(uint256 random);
    ...
    function reborn(uint256 _ownerTokenId, uint256 _amountFeeMax, uint256 _deadline, uint8 _gasLevel) payable external override ensure(_deadline)  nonReentrant poolOpen {
        ...
        reqSwaps[requestId] = SwapInfo(msg.sender, _ownerTokenId, 0, _amountFeeMax, _deadline);
        reqToFunc[requestId] = REBRON_FUNC_TYPE;
        emit Request(requestId);
    }


    function buy(uint256 _amountBuyMax, uint256 _deadline, uint8 _gasLevel) payable external override ensure(_deadline)  nonReentrant poolOpen {
        // Will revert if subscription is not set and funded.
        ...
        reqBuys[requestId] = BuyInfo(msg.sender, 0, _amountBuyMax, _deadline);
        reqToFunc[requestId] = BUY_FUNC_TYPE;
        emit Request(requestId);
    }
    ...
    function fulfillRandomWords(
        ...
        uint256 index = randomWords[0] % poolSize;
        uint256 targetIndex = poolNftMap.getValueByIndex(index, poolSize);
        uint8 funcType = reqToFunc[_requestId];
        if (funcType == REBRON_FUNC_TYPE) {
            SwapInfo memory swap = reqSwaps[_requestId];
            _reborn(swap.sender,swap.ownerTokenId, targetIndex, swap.amountFeeMax, swap.deadline);
            _poolRemoveNft(index);
        } else {
            BuyInfo memory buyInfo = reqBuys[_requestId];
            _buy(buyInfo.buyer, targetIndex, buyInfo.amountFeeMax, buyInfo.deadline);
            _poolRemoveNft(index);
        }
    }
    ...
    /**
     * @dev swap NFT.
     * @param _ownerTokenId The NFT Id you want to exchange
     * @param _targetTokenId  target NFT Id
     * @param _amountFeeMax max fee you want to pay for swapping
     * @param _deadline deadline
     */
    function _reborn(address sender, uint256 _ownerTokenId, uint256 _targetTokenId, uint256 _amountFeeMax, uint256 _deadline) internal ensure(_deadline) nonReentrant poolOpen {
        IERC721 _pandaNFT = IERC721(pandaNFT);
        ...
        if (_pandaToken.balanceOf(sender) < _price * 750 / 10000) {
            revert PandaNotEnough(sender, _pandaToken.balanceOf(sender), _price * 750 / 10000);
        }
        _pandaToken.safeTransferFrom(sender, address(this), _price * 375 / 10000);
        _pandaToken.safeTransferFrom(sender, owner(), _price * 375 / 10000);
        _pandaNFT.safeTransferFrom(sender, address(this), _ownerTokenId, "");
        _pandaNFT.safeTransferFrom(address(this), sender, _targetTokenId, "");
        _poolPushNft(_ownerTokenId);
        emit Reborn(sender, _ownerTokenId, _targetTokenId, _price * 750 / 10000);
    }


    /**
     * @dev buy NFT, you will receive $PANDA.
     * @param _targetTokenId The NFT Id you want to buy
     * @param _amountBuyMax  Above this value you will not make a purchase
     * @param _deadline deadline
     */
    function _buy(address buyer, uint256 _targetTokenId, uint256 _amountBuyMax, uint256 _deadline) public ensure(_deadline) nonReentrant poolOpen {
        IERC721 _pandaNFT = IERC721(pandaNFT);
        if (_pandaNFT.ownerOf(_targetTokenId) != address(this)) {
            revert NFTOwnerNotMatch(_targetTokenId,  address(this), _pandaNFT.ownerOf(_targetTokenId));
        }
        ...
        _pandaToken.safeTransferFrom(buyer,  owner(), _price * 375 / 10000);
     ...
     * @param _deadline deadline
     */
    function addLiquidity(uint256[] calldata _supplyTokens, uint256 _amountTokenMax, uint256 _updateMinPoolNftCount, uint256 _deadline) external ensure(_deadline) onlyOwner nonReentrant {
     ...
     * @param _updateMinPoolNftCount update pool min nfts 
     */
    function removeLiquidity(uint256[] calldata _targetTokens, uint256[] calldata _targetPoolIndexs, uint256 _amountTokenMin, uint256 _updateMinPoolNftCount) external onlyOwner nonReentrant {
    ...
    function _poolRemoveNft(uint256 _index) internal {
        poolNftMap.remove(_index, poolSize);
        poolSize--;
    }

    function _poolPushNft(uint256 _value) internal {
        poolNftMap.push(_value, poolSize);
        poolSize++;
    }

    function _getIdByIndex(uint256 _index) public returns(uint256) {
        return poolNftMap.getValueByIndex(_index, poolSize);
    }
...
library PandaMap {
    ...
    function getValueByIndex(mapping(uint256 => uint256) storage _map, uint256 _idx, uint256 _size) public returns(uint256) {
        require(_idx < _size, "Array Index Out Of Bounds");
        if(_map[_idx] == 0) {
            _map[_idx] = _idx + 1;
            return _idx + 1;
