...
error PandaNft__AlreadyVaultMinted();
...
contract PandaNft is ERC721, Ownable {
    ...
    // vault mint event
    event VaultMint(address recipient);
    ...
    // vault mint count
    uint256 public immutable vaultMintCount;
    ...
    constructor(
        ...
        vaultMintCount = _vaultMintCount;
    ...
    // vault mint
    function vaultMint(address recipient) public onlyOwner { 
        if (state != State.Pending) {
            revert PandaNft__NotPendingState();
        }

        if (totalSupply() > 0) {
            revert PandaNft__AlreadyVaultMinted();
        }

        myMint(recipient, vaultMintCount);

        emit VaultMint(recipient);
    }
