...
contract MintPass is ERC1155Supply, Ownable {
    ...
    uint256 immutable TIER_2_PRICE;
    uint256 immutable TIER_3_PRICE;
    uint256 immutable TIER_4_PRICE;
    ...
    constructor(uint256[] memory _tokenPrices, string[] memory _tokenURIs)
        ERC1155("")
    {
        require(_tokenPrices.length == 3, "There should only be 3 tokens max");
        require(
            _tokenPrices.length == _tokenURIs.length,
            "Token prices array size and token uris array size do not match"
        );

        TIER_2_PRICE = _tokenPrices[0];
        TIER_3_PRICE = _tokenPrices[1];
        TIER_4_PRICE = _tokenPrices[2];
    ...
    function mintTo(
        ...
        if (id == 1) {
            require(
                msg.value >= (TIER_2_PRICE * qty),
                "Ether value sent is incorrect"
            );
        } else if (id == 2) {
            require(
                msg.value >= (TIER_3_PRICE * qty),
                "Ether value sent is incorrect"
            );
        } else if (id == 3) {
            require(
                msg.value >= (TIER_4_PRICE * qty),
                "Ether value sent is incorrect"
            );
        } else {
            revert("No such token id");
        }
    ...
    function mintToMany(
        ...
        if (id == 1) {
            require(
                msg.value >= (TIER_2_PRICE * qty * to.length),
                "Ether value sent is incorrect"
            );
        } else if (id == 2) {
            require(
                msg.value >= (TIER_3_PRICE * qty * to.length),
                "Ether value sent is incorrect"
            );
        } else if (id == 3) {
            require(
                msg.value >= (TIER_4_PRICE * qty * to.length),
                "Ether value sent is incorrect"
            );
        } else {
            revert("No such token id");
    ...
    function donateFunds() external payable {
        ...
        if (donatedIndexMap[msg.sender] == 0) {
            DonationStruct memory donation = DonationStruct(
                msg.sender,
                msg.value
            );
