...
contract Crecodiles is 
    ...
    ERC721Pausable
 {
    ...
    address payable public beneficiary; // receiver of revenues
    bool public frozenDNA;
    string public baseTokenURI;
    mapping(uint256 => uint8) public dna; // 0 if not exists, 1 if exists
    uint public tokenTracker;

    mapping(uint256 => uint256) private tokenIdToDNA; // tokenId => dna
    
    IMetadata public metadata; // on-chain metadata
    ...
    function setMetadataContract(address _contractAddress) public {
      require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ERC721Access: must have Admin role to mint");
    ...
    function mintTo(address _to) external payable returns(uint256) {
      require(hasRole(MINTER_ROLE, _msgSender()), "ERC721Access: must have minter role to mint");
      tokenTracker++;
      require(tokenTracker <= MAX_NFT_SUPPLY, "max supply reached");
      _mint(_to, tokenTracker);
      beneficiary.sendValue(msg.value); // send to safe
      return tokenTracker;
    }
    ...
    // // TODO arg just for testing -> remove function
    // function finalizeStartIndexTest(uint _idx) public {
    //   startIndex = _idx;
    // }

    // freeze DNA, so it is final
    ...
    function finalizeStartIndex() public {
      require(!startIndexSet, "startIndex has already been set.");
      require((tokenTracker == MAX_NFT_SUPPLY), "not all tokens minted");
      startIndexSet = true;
      startIndex = uint(blockhash(block.number)) % MAX_NFT_SUPPLY;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
      ...
      if(address(metadata) != address(0x0)) {
        // console.log('forward to metadata %s => %s', tokenId, tokenDNA);
    ...
    function pause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "ERC721Access: must have pauser role to pause");
        _pause();
    }

    function unpause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "ERC721Access: must have pauser role to unpause");
        _unpause();
    }
    ...
    //   require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ERC721Access: must have Admin role to mint");
    //   beneficiary.sendValue(payable(self).balance); // send ETH to safe
    // }
