// HAY QUE DESPLEGAR UN CONTRATO FACTORY CON UNA NUEVA CUENTA; PROBARLO Y LUEGO METERLO EN LA VARIABLE col1
...
contract Marketplace is ReentrancyGuard {
    ...
    //uint256 constant coleccion = "0xB38F31F88e1D5e6De94934E456b95E0f6E375d90";

    //CONTRATO FACTORY DESPLEGADO EN ESPAÑOL
    address coleccion = 0xE4bEBaaDbee3f7958A0C0E9aA3c6281c027Dd95b;
    ...
    function comprarNFT(uint256 _idProducto) external payable nonReentrant {
        ...
        //Aquí tiene sentido porque es payable
        reembolsoSiSobrepasa(item.precio);

        //METER FUNCIÓN DE TRANSFER DE COMISIÓN PARA ENVIO A CARTERA DE CLIENTE
    ...
    // Retirar Venta
    function retirarVenta(uint256 _idProducto) external payable nonReentrant {
        Producto storage item = productos[_idProducto];
        require(
            _idProducto > 0 && _idProducto <= totalNft,
            "this NFT doesn't exist"
        ...
        require(!item.vendido, "this NFT has already been sold");
        require(msg.sender == item.vendedor, "Only the NFT owner can sell it");
        // update item to sold
        item.vendido = true;
        // transfer nft to back to seller
        item.coleccion.transferFrom(address(this), item.vendedor, item.idNft);
    }

    function reembolsoSiSobrepasa(uint256 precio) private {
    ...
    function retirarVenta()  {

        como la funcion de compra, pero no va a ser payable
        Devolver el nft al dueño 
            Transfieres desde el market al dueño antiguo    
        Poner como vendida a True

        Normalmente se usa transferfrom (comprueba antes de enviar...)
        Cambiar transfer por transferfrom

        De hecho es mejor aún "safetransferfrom"

        Googlea como transferir ETH

        //Esto es para cuando usas para pagar un token distinto al del token nativo______
        transferFrom(address from, address to, uint256 amount) → bool

    }

    ###########Mostrar NFTs en Venta
    Creamos una función VIEW 
    Podemos reducir los registros traídos desde BK para dejar 15 (por ejemplo) en la vista

    Futuro:::::::
        Base de datos que tenga las ofertas activas, ventas...
