...
contract DMarketDemoAuctionNFTToken is MinterAccess, ERC721 {

    // Base Token URI
    string private _baseTokenURI;

    constructor (address newOwner, string memory tokenURIPrefix) ERC721("DMarket Demo Auction", "DDA") {
    ...
    function burn(uint256 tokenID) public virtual {
        require(_isApprovedOrOwner(_msgSender(), tokenID), "DMarket Demo Auction Token: caller is not owner nor approved");
        _burn(tokenID);
    }
    ...
    function mintTokenBatch(address[] memory receivers, uint64[] memory tokenIDs) public virtual onlyMinter {
        require(receivers.length > 0,"DMarket Demo Auction Token: must be some receivers");
        require(receivers.length == tokenIDs.length, "DMarket Demo Auction Token: must be the same number of receivers/tokenIDs");
