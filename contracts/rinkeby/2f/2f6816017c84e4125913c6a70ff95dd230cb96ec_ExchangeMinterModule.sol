...
contract ExchangeMinterModule is IMetadataRenderer, MetadataRenderAdminCheck {
    ...
    constructor(ERC721Drop _source, ERC721Drop _sink, SharedNFTLogic _sharedNFTLogic) {
        source = _source;
        sink = _sink;
    ...
    function setColorLimits(ColorSetting[] calldata colorSettings) requireSenderAdmin(address(sink)) external {
        uint128 maxCountCache = maxCount;
        for (uint256 i = 0; i < colorSettings.length;) {
            string memory color = colorSettings[i].color;
            require(colors[color].claimedCount <= colorSettings[i].maxCount, "Cant decrease beyond claimed");
    ...
    function exchange(uint256[] calldata fromIds, string memory color) external {
        require(
        ...
        require(colors[color].claimedCount + targetLength <= colors[color].maxCount, "ran out of color");
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        return sharedNFTLogic.createMetadataEdition({
            ...
            animationUrl: string(abi.encodePacked(colorInfo.baseUri, '/animation')),
