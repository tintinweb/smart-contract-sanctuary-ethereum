...
abstract contract ErrorCodes {
    ...
    int32 internal constant BALANCE_PLUS_QUANTITY_TO_BUY_MUST_BE_LESS_THAN_QUANTITY_ALLOWED = 46;
    ...
    ErrorCodes
{
    ...
    function createListing(ListingParameters calldata _params) external override {
        ...
        _tokenListings[_params.assetContract][_params.tokenId].push(listingIdTracker);
    ...
    function validateMaxQuantityAllowed(
        ...
        if(IERC1155Custom(assetContract).getMaxQuantityAllowed(tokenId) > 0){
            require(
                IERC1155(assetContract).balanceOf(buyer, tokenId) + quantityToBuy <= IERC1155Custom(assetContract).getMaxQuantityAllowed(tokenId),
                throwError(BALANCE_PLUS_QUANTITY_TO_BUY_MUST_BE_LESS_THAN_QUANTITY_ALLOWED)
            );
        }
    }
