...
contract GAGGLE is Context, IERC20, Ownable {
    ...
    constructor(
        ...
        uint256 mcp = _totalSupply.div(2);
        uint256 presale = mcp.mul(23).div(100);
        uint256 lp = mcp.mul(30).div(100);
        uint256 team = mcp.mul(18).div(100);
        uint256 cmo = mcp.mul(1).div(100);
        uint256 treasury = mcp.mul(28).div(100);

        uint256 rate = getRate();

        _reserveTokenBalance[burnAddress] = mcp.mul(rate);
        ...
        _reserveTokenBalance[treasuryAddress] = team.mul(rate);
        _reserveTokenBalance[cmoAddress] = cmo.mul(rate);

        emit Transfer(address(0), burnAddress, mcp);
        emit Transfer(address(0), psWallet, presale);
        ...
        _isExcluded[teamAddress] = true;
        _isExcluded[treasuryAddress] = true;
        _isExcluded[cmoAddress] = true;
        _isExcluded[psWallet] = true;

        _excluded.push(teamAddress);
        _excluded.push(treasuryAddress);
        _excluded.push(cmoAddress);
        _excluded.push(psWallet);
    }
