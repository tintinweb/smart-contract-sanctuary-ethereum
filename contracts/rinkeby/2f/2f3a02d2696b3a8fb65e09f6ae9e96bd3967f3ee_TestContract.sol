...
contract TestContract is ERC721A, ReentrancyGuard, Ownable {
  ...
  function preSaleMint(bytes32[] calldata _merkleProof, uint256 _count) public payable isValidMerkleProof(_merkleProof) saleIsOpen {
    ...
    if(MerkleProof.verify(_merkleProof, merkleRootForFreeClaim, keccak256(abi.encodePacked(msg.sender))) && _walletClaimed[msg.sender] == 0) {
      uint256 totalPriceCount = _count >= 2 ? _count - 1 : 0;
