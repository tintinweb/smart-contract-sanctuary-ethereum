...
contract GoBlockchain is ERC1155, Ownable, Pausable, ReentrancyGuard, ERC1155Supply {
    ...
    constructor(uint256 feePercent, string memory _contractURI, string memory _name) ERC1155(_contractURI){
    ...
    function mint(address account, uint64 amount, string memory _uriOfId) public onlyOwner whenNotPaused {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _mint(account, tokenId, amount, "");
        _uris[tokenId] = _uriOfId;
    ...
    function buyToken(uint256 item, uint64 amount) public payable whenNotPaused nonReentrant returns (SellOffer memory) {
