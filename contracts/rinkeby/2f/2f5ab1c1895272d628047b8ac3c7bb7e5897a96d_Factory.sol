...
contract ProjectAndProposal{
    ...
    function returnAllValidators() public view returns(address[] memory){
        return allValidators;
    }

    function readValidatorWhoApproved() public view returns(address[] memory){
        return validatorWhoApproved;
    }

    function readValidatorWhoRejected() public view returns(address[] memory){
        return validatorWhoRejected;
    }
    ...
    function withdrawAllStableCoinFromThePool(uint256 amount, bytes32 symbol, uint _subsId, address _foun) external founderAction {

        require(founderAndInitialId[_foun] == _subsId, "The withdrawl is not matched with subsequent id");
        require(validatorWhoApproved.length >= 3,"maximum validators has not voted yet");
        uint i;
        string memory local;
        for(i = 0;i < validatorWhoApproved.length -1; i++){
            if(validatorWhoApproved[i] == address(0)){
                revert("One of the address is 0 address so reverted");
            }else{
                local = "The loop is corrupted";
                local;
            }
        }
        totalDepositedStableCoinsInThePot -= amount;
    ...
    function withdrawAllFounderTokenFromThePool(uint256 amount, bytes32 symbol, uint _subsId, address _foun) external investorAction {
    ...
    function withdrawTokens(uint256 amount, bytes32 symbol) external investorAction {
...
// /*
// Testing the subsequent validations 
// a. use another validation to test the features.
// b. according to that test the features.

// */  address[] private subsequentApproved;
//     address[] private subsequentRejected;
//     address[] private allSubsequentValidators;

//     function subsequentValidation(bool _choice, address _ad, address _contractad, uint _subsId, address _foun) public {
...
//         require(founderAndInitialId[_foun] == _subsId, "The validation is not matched with subsequent id");            
//             if(_choice == true){
//                 subsequentApproved.push(msg.sender);
//             }
                                
//             if(_choice == false){
//                 subsequentRejected.push(msg.sender);
//             }
                                    
//             if(_choice == false && subsequentRejected.length > 3){
//                 proposalCancelledRevertWithdrawlToInvestors = true;
//             }
//             allSubsequentValidators.push(_ad);
// 	}



//     function withdrawAllStableCoinFromThePool1(bool _choice, address _ad, address _contractad, uint256 amount, bytes32 symbol, uint _subsId, address _foun) external founderAction {

//         require(founderAndInitialId[_foun] == _subsId, "The withdrawl is not matched with subsequent id");
        
//         if(totalDepositedStableCoinsInThePot > 0 && subsequentApproved.length > 3){
//             subsequentValidation(_choice,  _ad, _contractad, _subsId,  _foun);
//             totalDepositedStableCoinsInThePot -= amount;
//             ERC20(whitelistedTokens[symbol]).transfer(msg.sender, amount);
