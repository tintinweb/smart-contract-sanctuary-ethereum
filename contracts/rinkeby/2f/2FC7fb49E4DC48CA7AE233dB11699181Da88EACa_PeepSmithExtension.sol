...
contract PeepSmithExtension is AdminControl, ICreatorExtensionTokenURI {
    ...
    modifier mintCompliancePreSale(uint _tokenId) {
      require(count[_tokenId] < maxPreSaleSupply[_tokenId] && count[_tokenId] < maxSupply[_tokenId], 'Max supply exceeded!');
      _;
    }

    modifier mintPriceCompliancePreSale(uint _tokenId) {
      require(msg.value >= costPreSale[_tokenId], 'Insufficient funds!');
      _;
    }

    modifier mintCompliancePublicSale(uint _tokenId) {
      require(count[_tokenId] < maxSupply[_tokenId], 'Max supply exceeded!');
      _;
    }

    modifier mintPriceCompliancePublicSale(uint _tokenId) {
      require(msg.value >= costPublicSale[_tokenId], 'Insufficient funds!');
      _;
    }

    function initialize(address core, uint256 preSalePrice, uint256 publicSalePrice, uint256 tokenPreSaleSupply, uint256 tokenMaxSupply, string memory baseURI) public adminRequired {
      _core = core;
    ...
    function mint(uint tokenId, bytes32[] calldata _merkleProof) public validToken(tokenId) {
      require(saleActive[tokenId - 1] == true, 'Sale has not started yet.');
      if (preSaleActive[tokenId]) {
        mintPreSale(tokenId, _merkleProof);
      } else {
        mintPublic(tokenId);
      }
    }

    function mintPreSale(uint256 _tokenId, bytes32[] calldata _merkleProof) public payable mintCompliancePreSale(_tokenId) mintPriceCompliancePreSale(_tokenId) {
      require(!whitelistClaimed[_tokenId][_msgSender()], 'Address already claimed!');
      bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
      require(MerkleProof.verify(_merkleProof, merkleRoot[_tokenId], leaf), 'Invalid proof!');

      whitelistClaimed[_tokenId][_msgSender()] = true;
      address[] memory to = new address[](1);
      to[0] = msg.sender;
    ...
    function mintPublic(uint256 _tokenId) public payable mintCompliancePublicSale(_tokenId) mintPriceCompliancePublicSale(_tokenId) {
      address[] memory to = new address[](1);
      to[0] = msg.sender;
      uint256[] memory tokenIds = new uint256[](1);
      tokenIds[0] = _tokenId;
    ...
    function setBaseURI(uint256 tokenId, string memory baseURI) public adminRequired {
      _baseURIs[tokenId] = baseURI;
    }

    function tokenURI(address core, uint256 tokenId) external view override validToken(tokenId) returns (string memory) {
    ...
    function setMerkleRoot(uint256 tokenId, bytes32 _merkleRoot) public adminRequired {
