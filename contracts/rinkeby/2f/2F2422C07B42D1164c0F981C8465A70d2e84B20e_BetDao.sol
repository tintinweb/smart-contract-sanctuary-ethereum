...
contract BetDao is ERC20('BetDao', 'BDAO'), Ownable {
    ...
    function swapAndWithdraw() private lockTheSwap {
        ...
        (bool devSent,) = payable(devWallet).call{value: devShare}("");
        require(devSent, "!failed sent to dev wallet");
        (bool marketingSent,) = payable(marketingWallet).call{value: marketingShare}("");
        require(marketingSent, "!failed sent to marketing wallet");
        (bool operationSent,) = payable(operatorWallet).call{value: operatorShare}("");
        require(operationSent, "!failed sent to operation wallet");
        (bool treasurySent,) = payable(treasury).call{value: treasuryShare}("");
        require(treasurySent, "!failed sent to operation wallet");
    }
