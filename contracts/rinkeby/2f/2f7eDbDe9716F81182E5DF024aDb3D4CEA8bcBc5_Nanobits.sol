...
contract Nanobits is ERC20Burnable, Ownable {
    ...
    // function testFunction(uint256 _timestamp, uint256 _rarity) public view returns(uint256) {
    ...
    //     }
    //     uint256 numOfDays = ((block.timestamp - _timestamp) / 1 days) * 1e18; // number of days
    //     if(numOfDays > 14) {
    //         uint256 reward = ((block.timestamp - _timestamp) * ratePerday); // days reward
    ...
    //     if(numOfDays < 14) {
    //         totalRewards += ((block.timestamp - _timestamp) * ratePerday);
    //     }
    ...
    function testClaimAll(uint256 _timestamp) public {
        require(tokenIds[msg.sender].length > 0, "NO_STAKED_YAGI");
        uint256 totalRewards = 0;
        ...
        for (uint256 i = 0; i < _tokensIds.length; i++) {
            ...
            uint256 rewards = testClaim(id, _timestamp);
            totalRewards += rewards;
        }
    ...
    function testClaim(uint256 id, uint256 timestamp) internal view returns(uint256) {
        uint256 totalRewards = 0;
        uint256 ratePerday = 0;
        ...
        uint256 numOfDays = ((block.timestamp - timestamp) / 1 days) * 1e18; // number of days
        if(numOfDays > 14) {
    ...
    // CHECKERS
    function checkRewardsByIds(uint256 tokenId) external returns (uint256) {
        require(stakedData[tokenId].owner != address(0), "TOKEN_NOT_BURIED");

        uint256 totalRewards = claim(tokenId);
        return totalRewards;
    }

    function checkAllRewards(address _owner) external returns (uint256) {
        uint256 totalRewards = 0;
        uint256[] memory tokens = tokenIds[_owner];

        for (uint256 i = 0; i < tokens.length; i++) {
            uint256 rewards = claim(tokens[i]);
            totalRewards += rewards;
        }
