...
import "../base/ChamberBase.sol";
import "../interface/IChamberController.sol";
import "../lib/StatLib.sol";


contract SimpleChest is ChamberBase {
  ...
  string internal constant _CHAMBER_NAME = "Chamber with Simple Chest";
  uint internal constant _CHAMBER_TYPE = uint(IChamberController.ChamberType.CHEST);
  bytes32 internal constant _CHAMBER_LEVEL_SLOT = bytes32(uint256(keccak256("eip1967.Chest.ChamberLevel")) - 1);
  ...
  function init(
    ...
    _setMintItems(mintItems_, mintItemsChances_);
    _CHAMBER_LEVEL_SLOT.set(chamberLevel_);
  }
  ...
  function chamberLevel() external view override returns (uint) {
    return _CHAMBER_LEVEL_SLOT.getUint();
  }
  ...
  function _action(
    ...
    damage : 0,
    manaConsumed : 0,
    mintItems : _mintRandomItem(),
    attackerDmgHistory : new uint[](0),
    defenderDmgHistory : new uint[](0)
    });
  }

  function _mintRandomItem() internal returns (address[] memory) {
    uint length = mintItems.length;
    address[] memory minted = new address[](1);

    uint random = _oracle().getRandomNumber(1e18);
    for (uint i; i < length;) {
      if (random < mintItemsChances[i]) {
        minted[0] = mintItems[i];
        break;
      }
    unchecked{++i;}
    }
...
abstract contract ChamberBase is Controllable, IChamber {
  ...
  function chamberName() external view virtual override returns (string memory);
  ...
  function open(
    ...
    return _action(heroToken, heroTokenId, data);
  }
