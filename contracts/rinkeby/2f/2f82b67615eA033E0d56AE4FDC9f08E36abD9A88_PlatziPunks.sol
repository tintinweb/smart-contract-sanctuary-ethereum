...
contract PlatziPunksDNA{
    ...
    //Calculamos el ADN en formato numerico, es una funcion es deermista y debe ser usado en produccion, Podemos unsa ChainLink para generar un oraculo y consumir datos aleatorios.

    function deterministicPseudoRandomDNA(uint256 _tokenId, address _minter) public pure returns(uint256) {
        //combinamos el tokenid con la adrress de la billetera que va a mintear para que siempre le de un nft distinto , es para solcionar el problema del determinismo en blockchain. Con esto lo hacemos pseudodeterminista , y para este caso nos sirve
        uint256 combinedParams = _tokenId + uint160(_minter);
        //combierte nuestro entero en un dato representado en bytes
        bytes memory encodedParams = abi.encodePacked(combinedParams);
        //parameros hasheados , es una funcion determinasta que toma una entrada y te da una salida (keccak256)
        bytes32   hashedParams = keccak256(encodedParams);

        return uint256(hashedParams);
    }

    //tamaño del ADN de cada una de las secciones
    ...
    //funcion que nos recorta las especificaciones
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    constructor(uint256 _maxSupply) ERC721 ('PlatziPunks', 'PLPKS'){
        //definimos cantidad maxima de nft que se pueden minterar
        maxSupply = _maxSupply;
    }
    //FUNCION PARA MINTEO 
    function mint() public {
        //Funcion para asignar un numero id a cada nft que se mintea 
        uint256 current = _idCounter.current();
        //Condicion para que no haya mas minteos que el total suplly perimitido
        require(current < maxSupply, 'Cantidad Maxima de NFT alcanzada');
        // Asignamos un nft aleatoriamente al minteo
        tokenDNA[current] = deterministicPseudoRandomDNA((current), msg.sender);
        //Minteo , esta funcion viene de erc721enum y se asigna el nft a la direccion que ejecuta la funcion con su correspondiente ID
        _safeMint(msg.sender, current);
        //funcion para incrementar el id de cada nft que se mintea
       // _tokenId.increment();

    }
    //Lugar o dominio donde esta colocado la informacion del NFT
    function _baseURI() internal pure override returns (string memory){
        return 'https://avataaars.io/';
    }
    //Parametros que añadimos
    ...
    //funcion que nos retorne la imagen una vez que recibe el ADN
    ...
    //Funcion para armar la metadata del NFT
    function tokenURI(uint256 tokenId)public view override returns(string memory){
        require(_exists(tokenId),'ERC721 Metadata : No existe este Token');

        uint256 dna = tokenDNA[tokenId];
        string memory image = imageByDNA(dna);

        string memory jsonURI= Base64.encode(
            abi.encodePacked('{"name": "PlatziPunks #', tokenId,'", "description": "PlatziPunks Es un NFT de lo mejor del mundo , esta desarrollado por Miguel Maidana","image":"',image,'"}')
        );
        return string(abi.encodePacked('data:application/json;base64,',jsonURI));
    }

         // override requeridos para soportar el enumerable
