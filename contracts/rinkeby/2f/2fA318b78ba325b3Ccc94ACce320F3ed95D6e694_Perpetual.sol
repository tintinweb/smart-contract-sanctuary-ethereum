...
contract Perpetual is IPerpetual, Pausable {
    ...
    modifier onlyClearingHouseOwner() {
        if (msg.sender != clearingHouse.owner()) revert Perpetual_SenderNotClearingHouseOwner();
        _;
    }
        ...
        )
    {
        ...
        bool isNewPosition = !isTraderPositionOpen(account);

        if (
            isNewPosition || (trader.positionSize > 0 ? LibPerpetual.Side.Long : LibPerpetual.Side.Short) == direction
        ) {
            (openNotional, positionSize, profit) = _extendPosition(account, amount, direction, minAmount);
            isPositionIncreased = true;
        } else {
            (openNotional, positionSize, profit) = _openReversePosition(account, amount, minAmount, isReducingPosition);
        }
    }
    ...
    {
        (openNotional, positionSize, profit) = _openReversePosition(account, amount, minAmount, true);
    }
        ...
        returns (int256 fundingPayments)
    {
        ...
        if (isTraderPositionOpen(account)) {
    ...
    function provideLiquidity(
        ...
        if (isLpPositionOpen(account)) {
            ...
            (int256 baseFeesEarned, int256 quoteFeesEarned) = _getVirtualTokensEarnedAsCurveTradingFees(lp, globalP);
        ...
        lp.totalBaseFeesGrowth = globalP.totalBaseFeesGrowth;
        lp.totalQuoteFeesGrowth = globalP.totalQuoteFeesGrowth;

        lp.cumFundingRate = globalP.cumFundingRate;

        lp.liquidityBalance += liquidity;

        globalP.totalLiquidityProvided += wadAmount;
    }
    ...
    function removeLiquidity(
        ...
        globalP.totalLiquidityProvided -= quoteAmount;
    }
    ...
    function getTraderFundingPayments(address account) external view override returns (int256 upcomingFundingPayment) {
        ...
        LibPerpetual.TraderPosition memory trader = traderPosition[account];
        LibPerpetual.GlobalPosition memory globalP = globalPosition;
        bool isLong = trader.positionSize > 0;

        return _getFundingPayments(isLong, trader.cumFundingRate, globalP.cumFundingRate, trader.positionSize.abs());
    }
    ...
    function isTraderPositionOpen(address account) public view override returns (bool) {
        LibPerpetual.TraderPosition memory trader = traderPosition[account];
        ...
        returns (LibPerpetual.TraderPosition memory)
    {
        ...
        // TODO: replace by get lp position getter function (added by nathan)
        // slither-disable-next-line incorrect-equality
    ...
    function getLpPositionHealth(address account) external view override returns (int256 pnL, int256 openNotional) {
        ...
        return (pnL, lp.openNotional);
    }

    function isLpPositionOpen(address account) public view override returns (bool) {
        LibPerpetual.LiquidityProviderPosition memory lp = lpPosition[account];
        ...
        )
    {
        /*
            ...
            Note: When we extend a position - no pnl is settled of the trader.
        ...
        */
        ...
        (quoteProceeds, baseProceeds) = _extendPositionOnMarket(amount, isLong, minAmount);
        ...
        // update position
        tradingFees = -_chargeQuoteFees(quoteProceeds);
        trader.openNotional += quoteProceeds;
    ...
    function _extendPositionOnMarket(
        ...
        bool isLong,
        uint256 minAmount
    ) internal returns (int256 quoteProceeds, int256 baseProceeds) {
        /*  if long:
        ...
        */
        if (isLong) {
            ...
            baseProceeds = _quoteForBase(proposedAmount, minAmount).toInt256();
        } else {
            quoteProceeds = _baseForQuote(proposedAmount, minAmount).toInt256();
            baseProceeds = -proposedAmount.toInt256();
        }

        return (quoteProceeds, baseProceeds);
    }
        ...
        )
    {
        ...
        */
        ...
        LibPerpetual.TraderPosition storage trader = traderPosition[account];
        if (!isTraderPositionOpen(account)) revert Perpetual_NoOpenPosition();

        int256 positionPnL;
        int256 addedOpenNotional;
        (baseProceeds, quoteProceeds, addedOpenNotional, positionPnL) = _openReversePositionOnMarket(
            trader,
        ...
        trader.openNotional += addedOpenNotional;
        trader.positionSize += baseProceeds;
    ...
    /// @param minAmount Minimum amount that the user is willing to accept, in vQuote if LONG, in vBase if SHORT. 18 decimals
    function _openReversePositionOnMarket(
    ...
    {
        int256 positionSize = user.positionSize;
        ...
        // Question: When is the person reducing a position vs reversing a position?

        /*

        Case I: positionSize > 0
            A) baseProceeds + positionSize < 0 => reversing
            B) baseProceeds + positionSize > 0 => reducing

        Case II: positionSize < 0
            A) baseProceeds + positionSize < 0 => reducing
            B) baseProceeds + positionSize > 0 => reversing
        ...
        */
        ...
        if (isReducingPosition) {
            /* is reducing position */
            uint256 additionalQuoteProceeds = 0;

            _checkProposedAmount(positionSize > 0, positionSize, proposedAmount);

            // calculate reduction ratio
            realizedReductionRatio = (baseProceeds.abs().wadDiv(positionSize.abs())).toUint256();

            if (
                isLong && realizedReductionRatio > 1e18
            ) /* if closing a short position and bought too much base tokens*/
            {
                uint256 baseRemaining = (baseProceeds + positionSize).toUint256();

                // try selling vBase tokens bought in excess, else donate to governance
                if (_canSellBase(baseRemaining)) {
                    additionalQuoteProceeds = _baseForQuote(
                        baseRemaining,
                        0 /* minAmount is zero, since second trade should be very small*/
                    );
                } else {
                ...
                quoteProceeds += additionalQuoteProceeds.toInt256();

                // baseRemaining will be 0 if proposedAmount not more than what's needed to fully buy back short position
                baseProceeds -= baseRemaining.toInt256();

                realizedReductionRatio = 1e18;
            }

            // take the realized reduction ratio when calculating the pnl
            addedOpenNotional = -user.openNotional.wadMul(realizedReductionRatio.toInt256());
            pnl = quoteProceeds - addedOpenNotional;
        }
        /* is reversing position */
        else {
            addedOpenNotional = quoteProceeds;
            pnl = 0;
        }

        /*
            Question: What part of the quoteProceeds should be settled into pnL vs make up the new `openNotional`?
            Answer:

            A) position is reduced:

                => When a position is reduced, carry over quoteProceeds towards pnl.

            B) position is reversed:

                => When a position is reversed, carry over all quoteProceeds towards the new openNotional
        */
    }
    ...
    function _baseForQuote(uint256 baseAmount, uint256 minAmount) internal returns (uint256 vQuoteAdjusted) {
        ...
        globalPosition.totalQuoteFeesGrowth += feesGrowth.wadDiv(vQuote.totalSupply()); // @dev: totalSupply is safer than balanceOf
    }
    ...
    function _settleLpPosition(
        ...
        (baseProceeds, quoteProceeds, , pnl) = _openReversePositionOnMarket(
            positionToClose,
        ...
        if (diff > 0) {
            emit DustGenerated(diff.toUint256());
            _donate(diff.toUint256());
        }
    }
    ...
    function _updateFundingRate() internal {
        ...
        globalP.cumFundingRate += fundingRate;
        globalP.timeOfLastTrade = uint128(currentTime);
    ...
    function _checkProposedAmount(
        ...
        */
        ...
        if (isLong) {
            ...
            int256 maxVQuoteAmount = ((marketTwap * 3) / 2).wadMul(positivePositionSize); // 50% above the TWAP
    ...
    function _getLpTradingFees(
        ...
        return lp.openNotional.abs().toUint256().wadMul(globalP.totalTradingFeesGrowth - lp.totalTradingFeesGrowth);
    }
...
import {IIncreOwnable} from "./IIncreOwnable.sol";
import {ICryptoSwap} from "./ICryptoSwap.sol";

// libraries
import {LibPerpetual} from "../lib/LibPerpetual.sol";

interface IClearingHouse is IIncreOwnable {
    /* ****************** */
    /*     Errors         */
    /* ****************** */
...
interface IVault {
    ...
    function getReserveValue(address trader) external view returns (int256);
