...
contract ConfigMintTest is ERC721A, Signable {
    ...
    constructor(
        uint256 config_,
        address[] memory erc20Contracts_,
        address[] memory holdingContracts_,
        address signer_
    ) Signable(signer_) ERC721A("ConfigMintTest", "ERC721A") {
        config = config_;
    ...
    {
        if (block.timestamp >= expireAt_) revert ExpiredSignature();
    ...
    function validateERC20(uint256[] calldata erc20Values_)
        internal
        returns (bool erc20Verified)
    {
        ...
        for (uint256 i; i < erc20Contracts.length; i++) {
            if (
                !IERC20(erc20Contracts[i]).transferFrom(
                    msg.sender,
                    address(this),
                    erc20Values_[i]
                )
    ...
    function validateEth(uint256 _ethValue)
        internal
        view
        returns (bool ethVerified)
    {
        ethVerified = msg.value == _ethValue;
    }

    function validateHoldings(uint256[][] memory _tokenIds, uint256 status)
        internal
        returns (bool holdingsVerified)
    {
        ...
        for (uint256 i; i < holdingContracts.length; i++) {
            for (uint256 j; j < _tokenIds[i].length; j++) {
                if (
                    IERC721(holdingContracts[i]).ownerOf(_tokenIds[i][j]) !=
                    msg.sender
                ) {
                    return false;
                }
                ...
                if (config > 0) {
                    if (
                        checkTokenStatus(holdingContracts[i], _tokenIds[i][j])
                    ) {
                        setTokenStatus(
                            holdingContracts[i],
                            _tokenIds[i][j],
    ...
    function checkTokenStatus(address holdingContract, uint256 tokenId)
        internal
        view
        returns (bool)
    {
