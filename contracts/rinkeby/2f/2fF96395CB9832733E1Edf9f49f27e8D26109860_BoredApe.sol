    ...
    PaymentSplitter 
{
    ...
    uint256[] private _teamShares = [50]; // 3 PEOPLE IN THE TEAM
    address[] private _team = [
        0x7EB3507f70d4FCB2f285FF2B9203F36735f80f48 // VIP Account gets 40% of the total revenue
    ];
    ...
    function presaleMint(address account, uint256 _amount, bytes32[] calldata _proof, uint256[] calldata tokenId)
    external
    payable
    ...
    function mintInternal(uint256[] calldata tokenId1) internal nonReentrant {
        _tokenIds.increment();

        uint256 tokenId = _tokenIds.current();
        for(uint i = 0; i < tokenId1.length; i++){
        _safeMint(msg.sender, tokenId1[i]);
        }
    }
