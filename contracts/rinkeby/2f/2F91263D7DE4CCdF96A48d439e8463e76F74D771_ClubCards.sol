...
contract ClubCards is ReentrancyGuard, CCEditions {
    ...
    function whitelistMint(
        ...
        address recovered = ECDSA.recover(
            ECDSA.toEthSignedMessageHash(
                keccak256(abi.encode(sender, numMints, waveId, nonce))
            ),
            signature
        );
        require(
    ...
    function prepMint(
        ...
        require(whitelistStatus == privateSale, "Not authorized to mints");
    ...
    function claim(
        ...
        address recovered = ECDSA.recover(
            ECDSA.toEthSignedMessageHash(
                keccak256(abi.encode(sender, tokenIds, amounts, nonce))
            ),
            signature
        );
        require(
    ...
    function allStatus() public view returns (bool) {
        return _allStatus;
    }
...
abstract contract CCEditions is ERC1155Supply, Ownable, ICCEditions {
    using Strings for uint256;

    /*
     * Tracks waves.
     */
    mapping(uint256 => uint256) public _waves;
    /*
     * Tracks claims.
     */
    mapping(uint256 => uint64) private _claims;
    /*
     ...
     * bytes 4-5: TokenIdOfEdition/Claim supply
     */
    uint40[] private tokens;

    // track claim nonces
    mapping(address => uint256) private _purchaseNonce;

    /* 
    // get the info on all tokens. return waveIds and tokenIdsOfWaves if
    function getAllTokens()
        ...
        returns (
            ...
            uint256[] memory tokenIdsOfEditions
        )
    {
        isClaim = new bool[](__totalSupply());
        editionIds = new uint256[](__totalSupply());
        tokenIdsOfEditions = new uint256[](__totalSupply());
        for (uint256 i = 0; i < __totalSupply(); i++) {
            (isClaim[i], editionIds[i], tokenIdsOfEditions[i]) = _getToken(
                tokens[i]
            );
        }
    }
 */
    ...
    function _increaseClaimSupply(uint256 claimId, uint256 amount) private {
        ...
        temp = uint256(uint40(temp));
        temp |= uint256(supply + amount) << 40;
        _claims[claimId] = uint64(temp);
    }
    ...
    function setClaim(uint256 claimId, string memory uri) external onlyTeam {
    ...
    function _getURI(uint256 id) internal view returns (string memory) {
        ...
        (bool isClaim, uint256 editionId, uint256 tokenIdOfEdition) = getToken(
            id
        );
        if (isClaim) {
    ...
    function purchaseNonce(address _address) public view returns (uint256) {
        return _purchaseNonce[_address];
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            ...
            if (edId == 0) {
                ...
                emit Claimed(to, _purchaseNonce[to], ids, amounts);
            } else {
