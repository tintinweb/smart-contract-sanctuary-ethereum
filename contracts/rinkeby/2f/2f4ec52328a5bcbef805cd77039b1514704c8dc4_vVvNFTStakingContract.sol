...
contract vVvNFTStakingContract is Ownable {
    ...
    function setNFTRace(uint[] memory _whaleTokenIds, uint256[] memory _sharkTokenIds) public onlyOwner {
        for(uint256 j = 0; j < 5000; j++){
            bool IsWhale = false;
            bool IsShark = false;
            for(uint256 i = 0; i < _whaleTokenIds.length; i++) {
                if(j == _whaleTokenIds[i]){
                    nftRace[j] = 0;
                    IsWhale = true;
                    break;
                }
    ...
    function setDolphinRace() external onlyOwner {
        for(uint i = 0; i < 5000; i++) {
            if(nftRace[i] != 1 || nftRace[i] != 2){
                nftRace[i] = 0;
            }
        }
    }
