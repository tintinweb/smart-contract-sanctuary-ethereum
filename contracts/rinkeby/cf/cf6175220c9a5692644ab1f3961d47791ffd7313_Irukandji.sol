...
contract Irukandji is ERC721, ERC721Pausable, Ownable {
    ...
    event PublicMaxMintUpdated(uint256 newAmount);
    ...
    event BaseURIConfigured(string uri);
    ...
    error PublicMaxMintCannotBeZero();
    error PublicSaleTimestampIsPassed();
    error WhitelistSaleTimestampIsPassed();
    ...
    /// @notice Set publicMintPrice value
    /// @dev Only owner can call this function
    function setPublicMintPrice(uint256 _price) external onlyOwner {
        if (_price == publicMintPrice) revert ValueNotChanged();
        publicMintPrice = _price;
        emit PublicMintPriceUpdated(_price);
    }

    /// @notice Set publicMaxMint value
    /// @dev Only owner can call this function
    function setPublicMaxMint(uint256 _amount) external onlyOwner {
        if (_amount == publicMaxMint) revert ValueNotChanged();
        if (_amount == 0) revert PublicMaxMintCannotBeZero();
        publicMaxMint = _amount;
        emit PublicMaxMintUpdated(_amount);
    }

    /// @notice Set publicSaleTimestamp value
    /// @dev Only owner can call this function
    function setPublicSaleTimestamp(uint256 _timestamp) external onlyOwner {
        if (_timestamp == publicSaleTimestamp) revert ValueNotChanged();
        if (_timestamp < block.timestamp) revert PublicSaleTimestampIsPassed();
        publicSaleTimestamp = _timestamp;
        emit PublicSaleTimestampUpdated(_timestamp);
    }

    /// @notice Set whitelistMintPrice value
    /// @dev Only owner can call this function
    function setWhitelistMintPrice(uint256 _price) external onlyOwner {
        if (_price == whitelistMintPrice) revert ValueNotChanged();
    ...
    function setTeamReserve(uint256 _amount) external onlyOwner {
        if (_amount == teamReserve) revert ValueNotChanged();
        teamReserve = _amount;
    ...
    function setMerkleRoot(bytes32 _root) external onlyOwner {
        if (_root == root) revert ValueNotChanged();
        root = _root;
    ...
    function mintPublic(uint256 _amount) external payable {
        // Checks
        if (_amount > publicMaxMint) {
            revert MintAmountInvalid({
                maxMint: publicMaxMint,
                got: _amount
            });
        }
        if (msg.value != (publicMintPrice * _amount)) {
            revert MintPriceInvalid({
                expected: publicMintPrice * _amount,
                got: msg.value
            });
        }
        if (totalSupply + _amount > (maxSupply - teamReserve)) revert OutOfStock();
        if (block.timestamp < publicSaleTimestamp) revert PublicSaleNotStarted();
