...
contract TokenStableCoinSwap is Singleton, ProxyUtility {
    ...
    /// @notice Function to initialize the values
    function initialize(
        IModusRegistry _modusRegistry,
        uint256 _projectId,
        uint256 _PXTStableCoinConversionRate,
        address _tokenAddress
    ) external initializer {
        utilInitialize(_modusRegistry);
        projectId = _projectId;
        PXTStableCoinConversionRate = _PXTStableCoinConversionRate; ///conversion rate with 18 decimal places
        tokenAddress = _tokenAddress;
    }

    /// @notice Function to allow investors to unstake PXT for StableCoins
    function swap(address investor, uint256 amount) external {
        address pxtStaking = modusRegistry
            .getProjectDetails(projectId)
            ._pxtStaking;
        require(
            msg.sender == pxtStaking,
            "PXTStableCoinSwap: Caller is not PXTStaking"
        );
        ...
        IERC20(modusRegistry.projectStableCoin(projectId)).safeTransfer(
            investor,
            (amount * PXTStableCoinConversionRate) / 1e18
        );
    }
}
