...
import './interfaces/IsAdzOne.sol';
import './interfaces/IDistributor.sol';
import './interfaces/IWarmup.sol';

import './types/Policy.sol';

contract AdzOneStaking is Policy {
  using SafeMath for uint256;
  using SafeMath for uint32;
  using SafeERC20 for IERC20;
  using SafeERC20 for IsAdzOne;

  IERC20 public immutable AdzOne;
  IsAdzOne public immutable sAdzOne;
  ...
  function stake(uint256 _amount, address _recipient) external returns (bool) {
    ...
    AdzOne.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    warmupInfo[_recipient] = Claim({
      ...
      gons: info.gons.add(sAdzOne.gonsForBalance(_amount)),
      expiry: epoch.number.add(warmupPeriod),
      lock: false
    });

    sAdzOne.safeTransfer(address(warmupContract), _amount);
  ...
  function claim(address _recipient) external {
    ...
    if (epoch.number >= info.expiry && info.expiry != 0) {
      ...
      uint256 amount = sAdzOne.balanceForGons(info.gons);
  ...
  function forfeit() external {
    ...
    uint256 memoBalance = sAdzOne.balanceForGons(info.gons);
    warmupContract.retrieve(address(this), memoBalance);
    AdzOne.safeTransfer(msg.sender, info.deposit);
  ...
  function unstake(uint256 _amount, bool _trigger) external {
    ...
    sAdzOne.safeTransferFrom(msg.sender, address(this), _amount);
    AdzOne.safeTransfer(msg.sender, _amount);
  ...
  function rebase() public {
    if (epoch.endTime <= uint32(block.timestamp)) {
      sAdzOne.rebase(epoch.distribute, epoch.number);
      ...
      uint256 staked = sAdzOne.circulatingSupply();
  ...
  function contractBalance() public view returns (uint256) {
    return AdzOne.balanceOf(address(this)).add(totalBonus);
  }
