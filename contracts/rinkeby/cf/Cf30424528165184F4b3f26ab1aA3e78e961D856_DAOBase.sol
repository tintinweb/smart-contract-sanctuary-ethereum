...
contract DAOBase is OwnableUpgradeable, IDAOBase {
    ...
    event CreateProposal(uint256 indexed proposalId, address indexed proposer);
    ...
    modifier onlySigner() {
        require(
            IDAOFactory(factoryAddress).isSigner(msg.sender),
            "DAOBase: caller is not the signer."
        );
        _;
    }
    ...
    //------------------------ public ------------------------//
    /**
     * @dev create proposal, TODO: check signature
     */
    function createProposal(
        ...
        emit CreateProposal(_proposalIndex, msg.sender);
    }

    /**
     * @dev vote for proposal, TODO: check signature
     */
    function vote(
        uint256 proposalId_,
        ...
        for (uint256 _index = 0; _index < optionIndexes_.length; _index++) {
            ...
            emit Vote(proposalId_, msg.sender, optionIndexes_[_index], amount_[_index]);
        }
    }
