...
contract NFT is ERC721, Pausable, Ownable, ERC721Burnable, ReentrancyGuard {
    ...
    function safeMint(string memory _tokenURI, uint _tokenId, uint _price) public {
        require(mintStatus, "Currently minitng is not active");
    ...
    //     }
    // }
    // function unstakeNFT(uint[] memory _tokenIDs) external nonReentrant(){
    //     uint arrayLength = _tokenIDs.length;
    //     for(uint i = 0; i < arrayLength; i++){
    ...
    //     IERC20(tokenContract).transferFrom(address(this),msg.sender,amount);
    // }
function stakeNFT(uint[] memory _tokenIDs) public {
    ...
    function unstakeNFT(uint[] memory _tokenIDs) external nonReentrant() returns (uint) {
        ...
        for(uint i = 0; i < arrayLength; i++){
            ...
            uint differenceTime = currentTime - stakeTime;
            differenceTime = differenceTime / rewardTimeSeconds;
            totalRewardTime += differenceTime;
            _transfer(address(this), msg.sender, _tokenIDs[i]);
        }
        totalRewardTokens = rewardAmount * arrayLength * totalRewardTime;
        // payable(msg.sender).transfer(address(this).balance);
        //IERC20(tokenContract).transferFrom(address(this),msg.sender,totalRewardTokens);
...
//98980988999999999999999700000
            // IERC20(tokenContract).transfer(idToDetail[tokenId]._sellerAddress, _price);
            // IERC20(tokenContract).transfer(idToDetail[tokenId]._sellerAddress, _royality);
            // IERC20(tokenContract).transfer(address(this), fees);
