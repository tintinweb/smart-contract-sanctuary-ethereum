...
contract OrdinaryAstroChart is BaseOraclizedAstroChart {
    ...
    uint private constant PRIME_TOKENID_UPPER_BOUND = 366;

    event AstroChartBred(uint256 bredTokenId, uint256 fromTokenId, address bredTokenOwner, uint price);

    constructor(address _primeAstroChartAddress) ERC721("AstroChart", "O-ASTRO") {
    ...
    function setUpParams(
        address linkTokenAddress, address _oracle, bytes32 _jobId, 
        uint256 _feeInLink, uint256 _oracleGasFee
    ) external onlyOwner {
       oracle = _oracle;
       jobId = _jobId;
       fee = _feeInLink;
    ...
    function withdrawOracleGasDeposit(uint256 amount) external onlyOwner {
        require(amount <= AstroChartLib.initialDeposit(), "WAMLTI");
     ...
     */
    function breedFrom(uint256 fromTokenId, uint16[] calldata datetimeOfBirth, int16[] calldata cityOfBirth) external payable {
        ...
        } else /** if (fromTokenId > 366) say it's ordinary AstroChart */ {
            argsOfFromToken = AstroChartLib.getAstroArgsOf(fromTokenId);
            ownerOfFromToken = ownerOf(fromTokenId);
        }

        uint256 bredTokenId = AstroChartLib.breedFromDry(fromTokenId, datetimeOfBirth, cityOfBirth, ownerOfFromToken, argsOfFromToken);

        _safeMint(msg.sender, bredTokenId);

        // interactions
        sendRequestToOracle(bredTokenId, datetimeOfBirth, cityOfBirth);
        emit AstroChartBred(bredTokenId, fromTokenId, msg.sender, msg.value); 
    }

    //TODO: move to astro-data-consumer, do the test
...
abstract contract BaseOraclizedAstroChart is ERC721Enumerable, Ownable, ChainlinkClient {
     ...
     */
    function sendRequestToOracle(uint256 tokenId, uint16[] memory datetimeOfBirth, int16[] memory cityOfBirth) internal returns(bytes32) {
        
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        
        string memory url = OracleUtils.genRequestURL(datetimeOfBirth, cityOfBirth);
     ...
     */ 
    function fulfill(bytes32 _requestId, uint16[] calldata _cusps, uint16[] calldata _planets) external recordChainlinkFulfillment(_requestId)
    {
    ...
    function getChartData(uint256 tokenId) external view returns (AstroChartResponse memory response) {
        bytes32 requestId = tokenId2OracleRequestId[tokenId];
        if (requestId != "0x0") {
            response = request2Response[requestId];
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (!response.exists) {
            return SVGGenerator.nonExistSVGInOpenSeaFormat(tokenId);
        } else {
            AstroChartArgs memory args = AstroChartLib.getAstroArgsOf(tokenId);
            return SVGGenerator.genSVGInOpenSeaFormat(response.cusps, response.planets, args.generation, tokenId, args.datetimeOfBirth[1], args.datetimeOfBirth[2]);
        }
    }
...
contract PrimeAstroChart is BaseOraclizedAstroChart {
    ...
    constructor()  ERC721("meta-astro", "P-ASTRO") {
    ...
    function setUpParams(
        address linkTokenAddress, address _oracle, bytes32 _jobId, 
        uint256 _feeInLink, uint256 _oracleGasFee, 
        uint256 _salesStartTime, uint256 _salesEndTime
    ) external onlyOwner {
       oracle = _oracle;
       jobId = _jobId;
       fee = _feeInLink;
    ...
    require salesStartTime > 0, or else throw sales start time should larger than zero as "SSTSLTZ"
     ...
     */
    function initialMint(address to, uint16[] calldata datetimeOfBirth, int16[] calldata cityOfBirth) 
    external payable {
        
        uint256 tokenId = AstroChartLib.initialMintDry(datetimeOfBirth, cityOfBirth);
        _safeMint(to, tokenId);

        //interactions
        sendRequestToOracle(tokenId, datetimeOfBirth, cityOfBirth);
    }
    ...
    function isDateAvailable(uint16 month, uint16 day) external view returns (bool) {
        return AstroChartLib.isDateAvailable(month, day);
    }
}
...
library OracleUtils {
    ...
    function genRequestURL(uint16[] memory datetimeOfBirth, int16[] memory cityOfBirth) internal pure returns(string memory url){
        url = StringLib.concat("http://3.115.127.203/api/v1/calc-chart-data-degrees?houses=Placidus&zodiac=Krishnamurti&datetimeOfBirth=", convertDatetimeToString(datetimeOfBirth));
        url = StringLib.concat(StringLib.concat(url, "&cityOfBirth="), convertCityToString(cityOfBirth));
    }
    
    function convertDatetimeToString(uint16[] memory datetime) private pure returns (string memory res) {
        uint16 year = datetime[0]; uint16 month = datetime[1]; uint16 day = datetime[2]; uint16 hour = datetime[3]; uint16 minute = datetime[4]; uint16 second = datetime[5];
 
        res = string(abi.encodePacked(
            StringLib.uintToString(year), ",", StringLib.uintToString(month), "," , StringLib.uintToString(day), ",", StringLib.uintToString(hour), ",",  StringLib.uintToString(minute), ",", StringLib.uintToString(second))
        );
    }

    function convertCityToString(int16[] memory city) private pure returns(string memory res) {
        int16 longititude = city[0]; int16 latitude = city[1]; int16 timezone = city[2];
        
        res = string(abi.encodePacked(
            StringLib.intToString(longititude), ",", StringLib.intToString(latitude), ",",  StringLib.intToString(timezone)
        ));
    }
...
library SVGGenerator {
    ...
    function nonExistSVGInOpenSeaFormat(uint256 tokenId) public pure returns (string memory) {
        string memory output = '<svg width="600" height="600" xmlns="http://www.w3.org/2000/svg"><text id="svg_6" font-size="10" y="311" x="301">generating...</text><svg>';
        return encodeInSVGFormat(output, tokenId);
    }

    function genSVGInOpenSeaFormat(uint16[] memory cusps, uint16[] memory planets, uint32 generation, uint256 tokenId, uint16 month, uint16 day) public pure returns (string memory) {
        string memory output = soGenSVG(cusps, planets, generation, month, day);
        return encodeInSVGFormat(output, tokenId);
    }

    function encodeInSVGFormat(string memory _rawSVG, uint256 tokenId) private pure returns (string memory) {
    ...
    function soGenSVG(uint16[] memory cusps, uint16[] memory planets, uint32 generation, uint16 month, uint16 day)
        public
        pure
        returns (string memory result)
    {
        ...
        '            <stop offset="12%" stop-color="#1b1b1b" />'
        '            <stop offset="50%" stop-color="#000000" />'
        '        </radialGradient>'
        ''
        '        <linearGradient id="goldBody" gradientTransform="rotate(60)">'
        '            <stop offset="0%" stop-color="#e3d8ab" />'
        '            <stop offset="100%" stop-color="#ab6e13" />'
        '        </linearGradient>'
        '    </defs>'
        ...
        '    </circle>'
        '    <rect width="1180" height="1180" x="10" y="10" stroke="#b49d5d" stroke-width="4" fill="none" />'
        '    <g>'
        '        <path d="M30,30 H60 V80 H30 V1120 H60 V1170 H30 V1140 H80 V1170 H1120 V1140 H1170 V1170 H1140 V1120 H1170 V80 H1140 V30 H1170 V60 H1120 V30 H80 V60 H30 V30" stroke="#b49d5d" stroke-width="4" fill="none" />'
        '    </g>'
        '    <g fill="url(#goldBody)" stroke="url(#goldBody)">'
        '        <circle cx="600" cy="600" r="450" stroke-width="15" fill="none">'
        '            <animateTransform attributeType="xml" attributeName="transform" type="rotate" from="0 600 600" to="360 600 600" begin="0" dur="5s" repeatCount="indefinite" />'
        '        </circle>'
        '        <circle cx="600" cy="600" r="430" stroke-width="3" fill="none" />'
        '        <circle cx="600" cy="600" r="420" stroke-width="3" fill="none" />'
        '        <circle cx="600" cy="600" r="410" stroke-width="3" fill="none" />'
        '        <circle cx="600" cy="600" r="400" stroke-width="10" fill="none" stroke-dasharray="1 62">'
        '            <animateTransform attributeType="xml" attributeName="transform" type="rotate" from="0 600 600" to="360 600 600" begin="0" dur="20s" repeatCount="indefinite" />'
        '        </circle>'
        '        <circle cx="0" cy="0" r="800" stroke-width="10" fill="none" stroke-dasharray="15 82" opacity="0.3">'
        '            <animateTransform attributeType="xml" attributeName="transform" type="rotate" from="0 0 0" to="360 0 0" begin="0" dur="40s" repeatCount="indefinite" />'
        '        </circle>'
        ''
        '        <circle cx="600" cy="600" r="355" stroke-width="4" fill="none" />'
        '        <circle cx="600" cy="600" r="230" stroke-width="4" fill="none" />'

        '        <line x1="942.9036683326192" y1="691.8807610113948" x2="996.029588778518" y2="706.1158084920335" stroke-width="4" />'
        '        <line x1="942.9036683326192" y1="508.11923898860516" x2="996.029588778518" y2="493.8841915079665" stroke-width="4" />'
        '        <line x1="851.0229073212244" y1="348.97709267877565" x2="889.9137802864846" y2="310.08621971351556" stroke-width="4" />'
        '        <line x1="691.8807610113948" y1="257.09633166738075" x2="706.1158084920335" y2="203.97041122148198" stroke-width="4" />'
        '        <line x1="508.1192389886051" y1="257.09633166738075" x2="493.8841915079664" y2="203.97041122148198" stroke-width="4" />'
        '        <line x1="348.97709267877565" y1="348.97709267877562" x2="310.08621971351556" y2="310.0862197135155" stroke-width="4" />'
        '        <line x1="257.0963316673808" y1="508.11923898860505" x2="203.97041122148204" y2="493.88419150796636" stroke-width="4" />'
        '        <line x1="257.09633166738075" y1="691.8807610113948" x2="203.97041122148198" y2="706.1158084920335" stroke-width="4" />'
        '        <line x1="348.97709267877556" y1="851.0229073212244" x2="310.08621971351545" y2="889.9137802864844" stroke-width="4" />'
        '        <line x1="691.8807610113947" y1="942.9036683326193" x2="706.1158084920334" y2="996.029588778518" stroke-width="4" />'
        '        <line x1="851.0229073212244" y1="851.0229073212245" x2="889.9137802864844" y2="889.9137802864846" stroke-width="4" />'
        '        <line x1="508.11923898860516" y1="942.9036683326192" x2="493.88419150796653" y2="996.029588778518" stroke-width="4" />'
        ...
        '        <text font-size="25" transform="translate(395, 290) rotate(-30)" fill="#b9353b" font-weight="bold" stroke="none">LEO</text>'
        '        <text font-size="25" transform="translate(260, 445) rotate(-60)" fill="#e4be2f" font-weight="bold" stroke="none">VIRGO</text>'
        '        <text font-size="25" transform="translate(210, 565) rotate(90)" fill="#086e46" font-weight="bold" stroke="none">LIBRA</text>'
        '        <text font-size="25" transform="translate(235, 750) rotate(60)" fill="#1d81d9" font-weight="bold" stroke="none">SCORPIO</text>'
        '        <text font-size="25" transform="translate(335, 895) rotate(30)" fill="#b41718" font-weight="bold" stroke="none">SAGITTARIUS</text>'
        '        <text font-size="25" transform="translate(525, 990) rotate(0)" fill="#cdba52" font-weight="bold" stroke="none">CAPRICORN</text>'
        '        <text font-size="25" transform="translate(740, 970) rotate(-30)" fill="#0e6b4c" font-weight="bold" stroke="none">AQUARIUS</text>'
        '        <text font-size="25" transform="translate(920, 830) rotate(-60)" fill="#2465b7" font-weight="bold" stroke="none">PISCES</text>'
        ;
        result = concat(
            result,
            concat(genCuspsAsLines(cusps), genPlanetsAsText(planets))
        );
        string memory date = concat(concat(uintToString(month), "."), uintToString(day));
        result = concat(result, text("600", "600", concat(concat(unicode"「Gen ", uintToString(generation)), unicode"」"), "2.5rem"));
        result = concat(result, text("600", "650", concat(concat(unicode"✦ ", date), unicode" ✦"), "1.5rem"));
        result = concat(result, "</g></svg>");
    }
        ...
        returns (string memory res)
    {
        ...
        for (uint32 i = 0; i < cusps.length; i += 1) {
            uint256[4] memory xy = genLineCoordinates(cusps64x64[i], 230, 355);
        ...
        returns (string memory res)
    {
        ...
        for (uint8 i = 1; i < copiedPlanets.length; i++) {
            uint8 j = i - 1;
            // detect and move it
        ...
        for (
            ...
            res = concat(res, text(uintToFixedNx4String(xy[2]), uintToFixedNx4String(xy[3]), PLANETS_TEXT[i], fontSize));
...
library AstroChartLib {
    ...
    */
    ...
    function initialMintDry(uint16[] calldata datetimeOfBirth, int16[] calldata cityOfBirth) public returns ( uint256 tokenId ){
        ...
        require(libStorage().initialMintDate2Exists[dateToBytes32(datetimeOfBirth[1], datetimeOfBirth[2])] == false, "IMDAE");
        
        _checkForDateAndCity(datetimeOfBirth, cityOfBirth);
        
        //effects
        AstroChartArgs memory args = AstroChartArgs({datetimeOfBirth: datetimeOfBirth, cityOfBirth: cityOfBirth, exists: true, generation: 0});

        tokenId = _nextTokenId();
        libStorage().tokenIdToAstroData[tokenId] = args;
        libStorage().initalMintCount ++;
        libStorage().initialDeposit += msg.value;
        libStorage().initialMintDate2Exists[dateToBytes32(datetimeOfBirth[1], datetimeOfBirth[2])] = true;
    }

    function isDateAvailable(uint16 month, uint16 day) public view returns (bool) {
        return libStorage().initialMintDate2Exists[dateToBytes32(month, day)]; 
    }
    ...
    function getBreedPrice(uint256 tokenId) public view returns (uint256) {
        return libStorage().tokenIdToBreedConfig[tokenId].breedPrice;
    }
    ...
    function breedFromDry(
        uint256 fromTokenId, uint16[] calldata datetimeOfBirth, int16[] calldata cityOfBirth,
        address ownerOfFromToken, AstroChartArgs memory astroDataOfParentToken
    ) public returns (uint256 bredTokenId){

        //checks
        BreedConfig storage breedConfig = libStorage().tokenIdToBreedConfig[fromTokenId];
        require(msg.value >= breedConfig.breedPrice + libStorage().oracleGasFee, "LTBP+O");
        _checkForDateAndCity(datetimeOfBirth, cityOfBirth);
        ...
        AstroChartArgs memory args = AstroChartArgs({
            ...
            exists: true, generation: astroDataOfParentToken.generation + 1
        });
