...
contract DefaultHoldingPeriod is Compliance, Ownable {

    IToken public token;
    IIdentityRegistry public identityRegistry;
    uint256 private _holdEnd;

    /// @dev start error codes at 4 (1-3 is used by default, see Compliance.sol) 
    ...
    /**
     * @dev See {ERC1404}. Overriden function compliant with 1404 standard
     */
    function detectTransferRestriction(address _from, address _to, uint256 _value) public override view returns (uint8 restrictionCode) {
        uint fromBalance = IToken(msg.sender).balanceOf(_from) - IToken(msg.sender).getFrozenTokens(_from);
        if (block.timestamp < _holdEnd && !IToken(msg.sender).isAgent(_from)) {
            return HOLD_FAIL_CODE;
        } 
        if (IToken(msg.sender).isFrozen(_from) || IToken(msg.sender).isFrozen(_to)) {
            return FROZEN_ACCOUNT_CODE;
        } 
        if (_value > fromBalance && !IToken(msg.sender).isAgent(_from)) {
     ...
     * @dev See {ERC1404}. Overriden function compliant with 1404 standard
     */
    function messageForTransferRestriction(uint8 _restrictionCode) public override view returns (string memory message) {
