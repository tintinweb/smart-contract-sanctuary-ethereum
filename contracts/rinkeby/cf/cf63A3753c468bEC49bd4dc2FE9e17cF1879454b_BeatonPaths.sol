...
contract BeatonPaths is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function mint(
        ...
        require(auctionDurationSeconds >= 3600, "Auction must last at least one hour");

        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();
        _nftInfos[newTokenId] = NFTInfo({
    ...
    function _requireNFTExists(uint256 tokenId) internal view {
        require(
            _nftInfos[tokenId].minBidPrice != 0 &&
            _nftInfos[tokenId].auctionDurationSeconds != 0,
    ...
    function bid(uint256 tokenId, uint256 bidPrice) public {
        ...
        require(bidPrice >= _nftInfos[tokenId].minBidPrice, "Unable to bid: bid must be at least the minimum bid");
        require(!_auctionStarted(tokenId) || bidPrice > _auctions[tokenId].bids[0].bidPrice, "Unable to bid: bid must be more than current bid");
        require(
            !_auctionStarted(tokenId) || _auctions[tokenId].expiration > block.timestamp,
            "Auction has ended"
        );
        ...
        if (_auctionStarted(tokenId)) {
            ...
            (bool isSafe, uint256 auctionExpiration) = SafeMath.tryAdd(block.timestamp, _nftInfos[tokenId].auctionDurationSeconds);
            require(isSafe, "Failed to calculate auction expiration");

            _auctions[tokenId].expiration = auctionExpiration;
            _auctions[tokenId].bids.push(Bid({ bidPrice: bidPrice, bidder: msg.sender }));
        }
    }
    ...
    function claim(uint256 tokenId) external payable nonReentrant  {
        ...
        require(
            _auctions[tokenId].expiration != 0 && _auctions[tokenId].expiration < block.timestamp,
        ...
        for (uint i = 0; i < Math.min(_auctions[tokenId].bids.length, _auctions[tokenId].numExtraClaimers + 1); i++) {
            if (_auctions[tokenId].bids[i].bidder == msg.sender) {
                require(msg.value == _auctions[tokenId].bids[i].bidPrice * (1 ether), "Incorrect amount of ether sent");
    ...
    function allowNextBidderToClaim(uint256 tokenId) public onlyOwner {
        ...
        uint16 numExtraClaimers = _auctions[tokenId].numExtraClaimers + 1;
    ...
    function getAuctionInfo(uint256 tokenId) public view returns (PublicAuctionInfo memory) {
        _requireNFTExists(tokenId);

        return PublicAuctionInfo({
            minBidPrice: _nftInfos[tokenId].minBidPrice,
            bids: _auctions[tokenId].bids,
            auctionExpiration: _auctions[tokenId].expiration,
    ...
    function updateAuctionDuration(uint256 tokenId, uint16 auctionDurationSeconds) public onlyOwner {
        _requireNFTExists(tokenId);
        require(!_auctions[tokenId].claimed, "NFT has been claimed");
        require(!_auctionStarted(tokenId), "Auction already started. Update auction expiration instead");

        _nftInfos[tokenId].auctionDurationSeconds = auctionDurationSeconds;
    }

    function updateAuctionExpiration(uint256 tokenId, uint256 auctionExpiration) public onlyOwner {
        _requireNFTExists(tokenId);
        require(!_auctions[tokenId].claimed, "NFT has been claimed");
    ...
    function sort(Bid[] storage bids) internal {
        quickSort(bids, 0, int(bids.length - 1));
    }

    function quickSort(Bid[] storage bids, int left, int right) internal {
        int i = left;
        int j = right;
        if (i == j) {
            return;
        }
        uint pivot = bids[uint(left + (right - left) / 2)].bidPrice;
        while (i <= j) {
            while (bids[uint(i)].bidPrice > pivot) {
                i++;
            }
            while (pivot > bids[uint(j)].bidPrice) {
                j--;
            }
            if (i <= j) {
                Bid memory iVal = bids[uint(i)];
