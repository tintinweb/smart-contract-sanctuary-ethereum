...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    bytes32 public DOMAIN_SEPARATOR = 0x72982d92449bfb3d338412ce4738761aff47fb975ceb17a1bc3712ec716a5a68;
     ...
     * @param calldata1 Calldata (appended to extradata)
     * @param extradata Base data for STATICCALL (probably function selector and argument encoding)
     * @return The result of the call (success or failure)
     */
    function staticCall(address target, bytes memory calldata1, bytes memory extradata)
    public
    view
    returns (bool result)
    {
        bytes memory combined = new bytes(calldata1.length + extradata.length);
        uint index;
        assembly {
            index := add(combined, 0x20)
        }
        index = ArrayUtils.unsafeWriteBytes(index, extradata);
        ArrayUtils.unsafeWriteBytes(index, calldata1);
        assembly {
    ...
    returns (bytes32 hash)
    {
        ...
        index = ArrayUtils.unsafeWriteBytes32(index, keccak256(order.calldata1));
    ...
    returns (bytes32)
    {
        ...
        require(validateOrder(hash, order, sig), "failed at validateOrder(hash, order, sig)");
        return hash;
    }
    ...
    internal
    {
        ...
        {
            emit OrderApprovedPartTwo(hash, order.howToCall, order.calldata1, order.replacementPattern, order.staticTarget, order.staticExtradata, order.paymentToken, order.basePrice, order.extra, order.listingTime, order.expirationTime, order.salt, orderbookInclusionDesired);
        }
    }
    ...
    reentrancyGuard
    {
        ...
        if (buy.maker == msg.sender) {
            require(validateOrderParameters(buy), "Failed at atomicMatch validateOrderParameters(buy)");
        } else {
        ...
        if (sell.maker == msg.sender) {
            require(validateOrderParameters(sell), "failed at validateOrderParameters(sell)");
        } else {
            sellHash = _requireValidOrderWithNonce(sell, sellSig);
        }

        /* Must be matchable. */
        require(ordersCanMatch(buy, sell), "failed at ordersCanMatch(buy, sell)");
        ...
        require(size > 0, "failed at size > 0");

        /* Must match calldata after replacement, if specified. */
        if (buy.replacementPattern.length > 0) {
            ArrayUtils.guardedArrayReplace(buy.calldata1, sell.calldata1, buy.replacementPattern);
        }
        if (sell.replacementPattern.length > 0) {
            ArrayUtils.guardedArrayReplace(sell.calldata1, buy.calldata1, sell.replacementPattern);
        }
        require(ArrayUtils.arrayEq(buy.calldata1, sell.calldata1), "failed at ArrayUtils.arrayEq(buy.calldata1, sell.calldata1)");

        /* Retrieve delegateProxy contract. */
        OwnableDelegateProxy delegateProxy = registry.proxies(sell.maker);

        /* Proxy must exist. */
        require(delegateProxy != address(0), "failed at delegateProxy != address(0)");
        ...
        require(delegateProxy.implementation() == registry.delegateProxyImplementation(), "failed at delegateProxy.implementation() == registry.delegateProxyImplementation()");

        /* Execute specified call through proxy. */
        require(proxy.proxy(sell.target, sell.howToCall, sell.calldata1), "failed at proxy.proxy(sell.target, sell.howToCall, sell.calldata1)");

        /* Static calls are intentionally done after the effectful call so they can check resulting state. */

        /* Handle buy-side static call if specified. */
        if (buy.staticTarget != address(0)) {
            require(staticCall(buy.staticTarget, sell.calldata1, buy.staticExtradata), "failed at staticCall(buy.staticTarget, sell.calldata1, buy.staticExtradata)");
        }

        /* Handle sell-side static call if specified. */
        if (sell.staticTarget != address(0)) {
            require(staticCall(sell.staticTarget, sell.calldata1, sell.staticExtradata), "failed at staticCall(sell.staticTarget, sell.calldata1, sell.staticExtradata)");
        }
