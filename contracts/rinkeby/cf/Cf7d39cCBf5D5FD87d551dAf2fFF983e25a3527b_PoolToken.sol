...
contract PoolToken is Ownable {
    ...
    function addUserInfo(address customer, uint256 dailyReceived, uint256 maxTokenReceived) external onlyAdmins {
        ...
        _listUsers.push(customer);
        _userData.push(UserData(customer, dailyReceived));
    ...
    function transferTokenSameValue(address _tokenAddress, uint256 _amount) external onlyOwner {
        for (uint256 i = 0; i < _listUsers.length; i++) {
            IERC20(_tokenAddress).transfer(_listUsers[i], _amount);
        }
    }

    function transferTokenAnyValue(address _tokenAddress) external onlyOwner {
        for (uint256 i = 0; i < _listUsers.length; i++) {
            IERC20(_tokenAddress).transfer(_userData[i].user, _userData[i].amount);
        }
    }
}
