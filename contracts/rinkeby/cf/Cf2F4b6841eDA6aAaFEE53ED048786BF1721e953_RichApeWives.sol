...
contract RichApeWives is ERC721URIStorage, Ownable, PaymentSplitter {
    ...
    constructor(
        ...
        for (uint i = 0; i < _team.length; i++) {
            accounts[_team[i]] = Account(0, true, false, 0);
        }
        uint256 totalSeeders = releaseTicketsContract.totalSupply();
    ...
    function adminMint(uint _amount) external onlyAdmin {
        require(wivesCounter + 1 <= maxWives, "Ticket limit already reached");
    ...
    function mintWife(uint _amount) external payable noReentrant {
        ...
        require(_amount > 0, "You must mint at least one NFT and under 10000");
        require(wivesCounter + _amount <= maxWives, "Purchase would exceed the max supply of tickets");
        require(msg.value >= (RAWPrice * _amount), "Not enough ether send to buy the desired tickets");
    ...
    function seedersFirstAirdrop() external onlyAdmin {
        uint half = uint(seeders.length / 2);
        for (uint i = 0; i < half; i++) {
            accounts[seeders[i]].releaseTickets++;
            if (accounts[seeders[i]].mintedNFTs < (accounts[seeders[i]].releaseTickets * 30)) {
                for (uint x = accounts[seeders[i]].mintedNFTs; x < (accounts[seeders[i]].releaseTickets * 30); x++) {
                    accounts[seeders[i]].mintedNFTs++;
                    _safeMint(seeders[i], wivesCounter);
                    _setTokenURI(wivesCounter, string.concat(_baseURIExtended, Strings.toString(wivesCounter+1)));
                    wivesCounter++;
                    emit Mint(seeders[i], wivesCounter);
                }
            }
        }
    ...
    function seedersSecondAirdrop() external onlyAdmin {
        uint half = uint(seeders.length / 2);
        for (uint i = half; i < seeders.length; i++) {
            accounts[seeders[i]].releaseTickets++;
            if (accounts[seeders[i]].mintedNFTs < (accounts[seeders[i]].releaseTickets * 30)) {
                for (uint x = accounts[seeders[i]].mintedNFTs; x < (accounts[seeders[i]].releaseTickets * 30); x++) {
                    accounts[seeders[i]].mintedNFTs++;
