...
contract ImmortlStaking is Ownable {
    ...
    IERC20 public constant IMMORTL =
    ...
    function remainingRewards() external view returns (uint256) {
        return IMMORTL.balanceOf(address(this)) - totalTokensInPools;
    }
    ...
    function compound(uint256 _pid) external ignoreVesting {
        PoolInfo storage pool = poolInfo[_pid];
        require(pool.stakingToken == IMMORTL, "Can compound only IMMORTL");
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.stakingToken) == address(IMMORTL)) {
    ...
    function safeTokenTransfer(
        ...
        uint256 balance = IMMORTL.balanceOf(address(this));
        if (balance > totalTokensInPools) {
            ...
            if (_amount >= rewardBalance) {
                ...
                IMMORTL.transfer(_to, rewardBalance);
            } else if (_amount > 0) {
                failedRewardsCache[_pid][_to] = 0;
                IMMORTL.transfer(_to, _amount);
            }
        } else {
    ...
    function emergencyRewardWithdraw() external onlyOwner {
        uint256 rewardAmount = IMMORTL.balanceOf(address(this)) -
            totalTokensInPools;
        IMMORTL.transfer(msg.sender, rewardAmount);
    ...
    uint256 balance = IMMORTL.balanceOf(user);
    return balance;
    }
}