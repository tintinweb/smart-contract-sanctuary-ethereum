...
contract CuteAlpaca is Ownable, ERC721A {
    bytes32 public merkleRoot;
    using Strings for uint256;
    
    uint256 public constant SUPPLY = 6868;
    uint256 public WHITELIST_SUPPLY = 500;
    uint256 public AUCTION_SUPPLY = 500;

    uint256 public price = 0.08 ether;
    uint256 public maxMintedPerAddress = 2;
    
    bool public REVEALED;
    string public _baseTokenURI;
    string public CONTRACT_URI;
    string private UNREVEALED_URI = "ipfs://";
   
    bool public paused = true;
    bool public salePublicActive = false;
    bool public saleWLActive = false;
    bool public saleAuctionActive = false;
    ...
    uint256 public startTime = 1652286340;

    constructor() ERC721A("Cute Alpaca V5", "CA") { }

    function auctionMint(uint256 quantity) external payable mintCompliance(quantity) {
        if (!saleAuctionActive) 
            revert SaleInactive("auction sale inactive");

        uint256 _saleStartTime = startTime;
        if (_saleStartTime != 0 && block.timestamp < _saleStartTime)
            revert SaleInactive("auction sale inactive");

        if (totalSupply() + quantity > AUCTION_SUPPLY) 
            revert SoldOut("auction soldout");

        if (_numberMinted(msg.sender) + quantity > maxMintedPerAddress) 
            revert LimitMinted("auction mint reached");
        
        uint256 totalCost = getAuctionPrice(_saleStartTime) * quantity;
        if (msg.value < totalCost)
            revert InvalidPrice("wrong price");
    ...
    function whitelistMint(uint256 quantity, bytes32[] memory proof) external payable mintCompliance(quantity) {
        if (!saleWLActive) 
            revert SaleInactive("wl. sale inactive");
        if (totalSupply() + quantity > WHITELIST_SUPPLY) 
            revert SoldOut("whitelist soldout");
        
        uint256 totalCost = price * quantity;
        if (msg.value != totalCost) revert InvalidPrice("wrong price");
                                        
        if (_numberMinted(msg.sender) + quantity > maxMintedPerAddress) 
            revert SoldOut("whitelist mint reached");
        if (isProof(proof, keccak256(abi.encodePacked(msg.sender)))) 
            revert InvalidProof("address not whitelisted");
    ...
    function publicMint(uint256 quantity) external payable mintCompliance(quantity) {
        if (!salePublicActive) 
            revert SaleInactive("public sale inactive");

        uint256 totalCost = price * quantity;
        if (_numberMinted(msg.sender) + quantity > maxMintedPerAddress) 
            revert SoldOut("public mint reached");
    ...
    function refundMoney(uint256 money) private {
        require(msg.value > money, "Need to send more ETH.");
        if (msg.value > money) {
            payable(msg.sender).transfer(msg.value - money);
        }
    }
    ...
    function setAuctionSupply(uint256 _newSupply) external onlyOwner {
        AUCTION_SUPPLY = _newSupply;
    }
    ...
    function setAuctionSaleState(bool _active) external onlyOwner {
        saleAuctionActive = _active;
