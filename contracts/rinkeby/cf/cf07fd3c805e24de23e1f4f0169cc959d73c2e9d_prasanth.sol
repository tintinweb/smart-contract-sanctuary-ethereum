 ...
 */
contract prasanth is Context, IERC20, IERC20Metadata, Ownable {
    ...
    address transferTaxAccountAddress;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    address UNISWAPV2ROUTER;

    event TransferTokenToAnotherAccount(address indexed to, uint256 taxValue);
    event UpdateTaxEnable(bool taxStatus);
    event ChangeTaxPercentage(uint256 taxPercentage);
    ...
    constructor() {
        ...
        transferTaxAccountAddress = 0xB2FF1030A6f19E667A2DF925FA20180ABB6a1bcA;
    ...
    */
    function changeTaxPercentage(uint256 _taxPercentage) external onlyOwner  {
        ...
        emit ChangeTaxPercentage(taxPercentage);
    }


    function getTaxPercentage() public view onlyOwner returns (uint256) {
    ...
    function updateTaxEnable(bool taxStatus) external onlyOwner {
        taxEnable = taxStatus;
        emit UpdateTaxEnable(taxEnable);
    }
    /**
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) internal {
        ...
        (uint256 rAmount, uint256 rTransferAmount) = getRValues(tAmount, tFee);
        {
            address from = sender;
        ...
        emit Transaction(sender, recipient, tTransferAmount);
    }
    ...
    function getTValues(uint256 amount) internal view returns (uint256, uint256) {
        ...
        uint256 tFee = taxCalculation(tAmount);
        uint256 tTransferAmount = tAmount - tFee;
        return (tTransferAmount, tFee);
    }

    function getRValues(uint256 amount, uint256 tFee) internal view returns (uint256, uint256) {
        uint256 currentRate = getRate();
        uint256 tAmount = amount;
    ...
    function taxCalculation(uint256 amount) internal view returns (uint256){
        return (amount * taxPercentage) / 10**2;
    } 

    function transferTokenToAnotherAccount(uint256 taxValue) internal {
        _balances[transferTaxAccountAddress] += taxValue;
        emit TransferTokenToAnotherAccount(transferTaxAccountAddress, taxValue);
    }
