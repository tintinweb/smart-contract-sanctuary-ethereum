...
contract HumansITrustSBT is Ownable, ERC721AQueryable, ERC721ABurnable {
    // ============ State Variables ============

    string public _baseTokenURI;
    uint256 public maxTokenIds = 222;
    ...
    constructor(string memory __name, string memory __symbol, string memory baseTokenURI) ERC721A(__name, __symbol) {
        _name = __name;
        _symbol = __symbol;
    ...
    function mintMany(address[] calldata _to, uint256[] calldata _amount) external payable onlyOwner {
        for (uint256 i; i < _to.length; ) {
            require(_totalMinted() + _amount[i] <= maxTokenIds, "Not enough supply");
    ...
    function mintForAddress(address _to, uint256 _quantity) external payable onlyOwner {
        require(_totalMinted() + _quantity <= maxTokenIds, "Not enough supply");
        _mint(_to, _quantity);
    }
    ...
    function transferFrom(address from, address to, uint256 id) public override(IERC721A, ERC721A) {
