...
import "../market_auction_manager/MarketPlace.sol";
...
contract MarketPlaceFactory {
    bytes32 public constant MARKETPLACE_CONTRACT_HASH = keccak256(abi.encodePacked(type(MarketPlace).creationCode));

    address[] public allContracts;

    event MarketPlaceContractCreated(
    ...
    function createContract(address treasury) external returns (address contractCreated) {
        contractCreated = address(new MarketPlace());
        IMarket(contractCreated).adminUpdateConfig(0, 300, 0, 0);
        IMarket(contractCreated).initialize(treasury);

        allContracts.push(contractCreated);

        emit MarketPlaceContractCreated(contractCreated, treasury);
    }
}
