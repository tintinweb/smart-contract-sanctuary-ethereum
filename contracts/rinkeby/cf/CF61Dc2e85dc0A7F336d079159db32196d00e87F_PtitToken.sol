...
contract PtitToken {
    ...
    modifier auth() {
        require(checks[msg.sender] == 1, "ptit/not-authorized");
        _;
    }

    // --- ERC20 Data ---
    string public constant name = "Ptit Token";
    string public constant symbol = "ptit";
    ...
    function transferFrom(
        ...
        require(balanceOf[src] >= amt, "ptit/insufficient-balance");
        if (
            src != msg.sender && allowance[src][msg.sender] != type(uint256).max
        ) {
            require(
                allowance[src][msg.sender] >= amt,
                "ptit/insufficient-allowance"
            );
    ...
    function permit(
        ...
        require(holder != address(0), "ptit/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "ptit/invalid-permit");
        require(
            expiry == 0 || block.timestamp <= expiry,
            "ptit/permit-expired"
        );
        require(nonce == nonces[holder]++, "ptit/invalid-nonce");
