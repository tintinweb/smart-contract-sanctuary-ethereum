...
contract BingDwenDwen is Ownable, ERC721A, ReentrancyGuard {
    ...
    function mint(uint256 _mintAmount) public payable nonReentrant callerIsUser {
        ...
        require(_mintAmount <= 6, "Cant mint more then max tx limit");
        require(totalSupply() + _mintAmount <= collectionSize, "reached max supply");
        require(msg.value >= _mintPrice * _mintAmount);
        require(numberMinted(msg.sender) + _mintAmount <= maxPerAddressDuringMint, "can not mint this many");
