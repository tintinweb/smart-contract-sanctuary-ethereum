...
// File: contracts/custom_kampoeng.sol

pragma solidity ^0.8.4;

contract KampoengTest is ERC721A, Ownable{ 
    using Strings for uint256;
    ...
    // storing all addresses to freeMintAdressData;
    mapping(address => uint256) private freeMintAdressData;

    // constructor comes before SC is initialized;
    // constructor updates SC
    constructor(uint256 price, uint256 maxSupply, string memory baseUri, uint256 freeMintLimitPerWallet, uint256 maxMintPerTX, bool isSaleActive, uint256 freeMintSupply) ERC721A("KTest1", "KVV"){
        // get owner inputs from constructors
        PRICE = price;
    ...
    // ----------FREE MINT FUNCS
        // UPDATE THE CURRENT MINTERS USED FREE MINT
        // used to update the current minters used free mint and increment by how many was used (count: passed through function)
    function updateFreeMintsAdresses(address minter, uint256 count) private{
        // find the address of the minter in data & increment count
        freeMintAdressData[minter] += count;
    }

        // RETURN CURRENT MINTERS FREE MINT COUNT COUNT
        // ** used to check the current minter's free mint used.
    function returnCurrentMinterFreeMint(address minter) public view returns (uint256){
        // gets data from the passed minter address and return the current count
    ...
    // ----------GETTERS 
        ...
        // GET NFT BASE URI
        // Base URI is stored from the owner's input and is taken from the memory of the smart contract:
    ...
    // ----------SETTERS OWNER FUNCS
        // SET PRICE
        // to set the price anytime and is accessible only by owner;
    function setPrice(uint256 price) external onlyOwner(){
        PRICE = price;
    }

        // LOWER THE MAX SUPPLY;
    function lowerMaxSupply(uint256 newMaxSupply) external onlyOwner{
        // Check if newMaxSupply is lesser than the current max, if yes proceed
        // if not throw "MAX SUPPLY MORE THAN NEW MAX SUPPLY" error
        require (newMaxSupply < MAX_SUPPLY, "MAX SUPPLY MORE THAN NEW MAX SUPPLY");
        // Check if newMaxSupply is lesser than the _currentIndex, if yes proceed
        // if not throw "NEW MAX SUPPLY IS BIGGER OR EQUAL TO CURRENT INDEX" error
        // the _currentIndex variable comes from ER721a Azuki
    ...
    function setBaseURI(string memory baseUri) external onlyOwner {
        // check if metadata has been frozen by owner, if not proceed.
        // if yes throw error "METADATA FROZEN."
        require(!METADATA_FROZEN, "METADATA FROZEN");
        BASE_URI = baseUri;
    }

    // SET THE MAX FREE MINT LIMIT PER WALLET
    ...
    // SET TOTAL FREE MINT SUPPLY
    function setFreeMintSupply(uint256 freeMintSupply) external onlyOwner{
        FREE_MINT_SUPPLY = freeMintSupply;
    }

    function freezeUnFreezeMetaData() external onlyOwner{
    // METADATA FROZEN STAT = OPPOSITE OF CURRENT METADATA STAT;
    ...
    function mint(uint256 _mintAmount) public payable mintRegulation(_mintAmount) {
        ...
        // initial checking before transaction
        // if balance is more than zero, transfer
        // else return "You have zero balance text"
        require(address(this).balance > 0, "You have zero balance");
        // payable is to transact an ether.
        // transfer to the current logged in address.
        // get the current balance and transfer all
