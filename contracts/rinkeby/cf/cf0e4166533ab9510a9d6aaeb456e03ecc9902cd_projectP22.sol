...
contract projectP22 is ERC1155, Ownable, IVestedRandomness {
    ...
    mapping(uint32 => bool) public isToadzIdClaimedFreePack;
    ...
    function startPackSale(uint32 packId, uint128 _packStartPrice, uint128 _packMinPrice, uint256 _saleDuration) external onlyOwner {
        ...
        require(isToadzFreeClaimPack[packId], "Can't start sale for Toadz free claim Pack");
    ...
    function setToadzFreeClaimPack(uint32 packId, bool _isToadzFreeClaimPack) external {
        require(activePacks[packId], "Pack not active");
    ...
    function claimToadzFreePack(uint32 toadzTokenId, uint32 packId) external {
        require(activePacks[packId], "Pack not active");
        require(isToadzIdClaimedFreePack[toadzTokenId] == false, "Free pack already claimed for this toadz token id");
    ...
    function Mint(uint32 count, uint32 packId) external payable {
        ...
        require(isToadzFreeClaimPack[packId], "Can't Mint Toadz free claim Pack");
        require(
