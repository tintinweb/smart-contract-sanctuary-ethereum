    ...
    KIP17Pausable
{
    ...
    function setKIP7Token(address tokenAddress) external onlyRole(MINTER_ROLE) {
        ...
        bytes4 interface_ = bytes4(0x65787371);
        ...
        // console.log("interface:", type(IKIP7).interfaceId);
        console.log("ahihi:", result.length);
        console.log("ahuhu:", string(result));
        console.log("success:", success);
        console.log("result:", isSupported);

        require(success, "New token's type must be KIP7");
    ...
    function trim(string memory str) public pure returns (string memory) {
        ...
        bytes memory newStringAfterTrimHead = new bytes(bStr.length);
        uint256 lengthStr = 0;
        for (uint256 i = 0; i < bStr.length - 1; i++) {
            if (bStr[i] == 0x20 && bStr[i + 1] == 0x20) {
                continue;
            } else {
                newStringAfterTrimHead[lengthStr] = bStr[i + 1];
                lengthStr++;
            }
        }
        uint256 tail = lengthStr;
        for (uint256 i = lengthStr; i > 1; i--) {
            if (
                newStringAfterTrimHead[i] == 0x20 &&
                newStringAfterTrimHead[i - 1] == 0x20
            ) {
                tail--;
            } else {
                tail--;
        ...
        bytes memory finalStringAfterTrim = new bytes(tail);
        for (uint256 i = 0; i < tail; i++) {
            finalStringAfterTrim[i] = newStringAfterTrimHead[i];
        }
        return string(finalStringAfterTrim);
    }
