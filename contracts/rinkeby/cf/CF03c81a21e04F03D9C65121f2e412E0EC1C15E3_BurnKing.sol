...
contract BurnKing is ERC721Enumerable, Ownable {
   ...
   constructor()ERC721("Burn Spot", "BSP"){}
    ...
    function transferNftFromPool(address _burner, uint256 _burnAmountUSD) internal {
     ...
     function burnERC20(uint256 _amount, address _spender, uint256 _burnAmountUSD, address _tokenAddress) external{
       ...
       require(contractBalance >= _amount * 10**tokenDecimals, "Not enough funds for burning");
   ...
   function concatParams(string memory _signer, uint256 _tokenCount, uint256 _usdCost, uint256 _timestamp) public pure returns(string memory){
       return string(abi.encodePacked(_signer,' ',Strings.toString(_tokenCount),' ',Strings.toString(_usdCost),' ',Strings.toString(_timestamp)));
   }
