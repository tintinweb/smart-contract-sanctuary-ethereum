...
// File: contracts_common/src/Interfaces/ERC721MandatoryTokenReceiver.sol
...
// File: contracts/ERC721BaseToken.sol
...
// File: contracts/LandBaseToken.sol
...
contract LandBaseToken is ERC721BaseToken {
     ...
     */
    function mintLand(address to, uint256 size, uint256 x, uint256 y, bytes calldata data) external {
    ...
    /// @param data additional data
    function transferLand(address from, address to, uint256 size, uint256 x, uint256 y, bytes calldata data) external {
        require(from != address(0), "from is zero address");
        require(to != address(0), "can't send to zero address");
        bool metaTx = msg.sender != from && _metaTransactionContracts[msg.sender];
