...
contract npay is ERC721A, Ownable {

    /** VARIABLES **/
    address private dev = 0x215De00630F5E89C3A219D2771e55dc49F28489f;
    uint256 public price = 0.01 ether;
    uint256 public maxSupply = 120;
    uint256 constant public MAX_MINT_COUNT_PER_TXN = 30;
    string private customBaseURI = "ipfs://placeholder-link";
    enum SaleState {
        CLOSED,
        OPEN
    }
    ...
    constructor(bool isPaying, uint256 deploymentPrice) ERC721A("npay", "pay") payable {
        if(isPaying){
    ...
    function mint(uint256 count) external payable {
        ...
        if (_currentIndex + (count - 1) > maxSupply) revert SoldOut();
        if (count - 1 > MAX_MINT_COUNT_PER_TXN) revert InvalidQuantity();
        if (msg.value < price * count) revert InsufficientPayment();

        _mint(msg.sender, count);
    }

    function freeMintToAddress(address account, uint256 count)
        external
        onlyOwner
    {
        if (_currentIndex + (count - 1) > maxSupply) revert InvalidQuantity();
        _mint(account, count);
    }

    /** ADMIN FUNCTIONS **/
    /**
     * @dev Sets sale state to CLOSED (0) or OPEN (1).
     */
