...
contract BBFabric is SmartLZBase {
    ...
    event NewBasicBlockCreated(uint256 strategyId, address indexed implementation, address indexed proxy);
    ...
    function lzReceive(
        ...
        // require _actionPool caller todo
    (uint256 strategyId, address[] memory _implementations) = abi.decode(payload, (uint256, address[]));
        for(uint i = 0; i < _implementations.length; i++) {
            _initNewProxy(_implementations[i], strategyId);
        }
    }


    // DEBUG
    function createNewBB(address implementation, uint256 strategyId) external {
        _initNewProxy(implementation, strategyId);
    }


    function _initNewProxy(address implementation, uint256 strategyId) private {
        // new Proxy
        bytes memory data;
        ERC1967Proxy proxy = new ERC1967Proxy(implementation, data);

        emit NewBasicBlockCreated(strategyId, implementation, address(proxy));
    }
