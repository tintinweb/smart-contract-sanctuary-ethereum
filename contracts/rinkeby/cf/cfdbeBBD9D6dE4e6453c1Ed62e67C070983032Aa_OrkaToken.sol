...
contract OrkaToken is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public MAX_GENESIS_SUPPLY = 2000;
    ...
    modifier orkaOwner(uint256 orkaId) {
        require(
            ownerOf(orkaId) == msg.sender,
            "Cannot interact with a ORKA you do not own"
        );
        _;
    }

    constructor() ERC721A("Orka Hideout", "ORKA") {}

    // Main Sale Minting
    function mintGensisOrkas(uint256 _amount) external payable {
    ...
    function spawn(uint256 parent) external orkaOwner(parent) {
        uint256 supply = totalSupply();
        require(supply <= MAX_SUPPLY, "All babies have been minted");
        require(
            parent <= MAX_GENESIS_SUPPLY,
            "You need to spawn a baby from a genesis Orka"
        );
        Pod.burn(msg.sender, SPAWN_PRICE);
        babyCount++;
        _safeMint(msg.sender, 1);
    }

    function setPodAddress(address podAddress) external onlyOwner {
        Pod = IPod(podAddress);
    }
