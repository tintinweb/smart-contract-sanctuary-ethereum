...
contract CrazyChihuahuaMysteryCamp is ERC1155, Ownable {
    ...
    constructor(string memory _metadataUri) public ERC1155(_metadataUri) {
    ...
    function mintCrazyChihuahuas(uint256 _numberOfAssets) public payable mintConditions(_numberOfAssets) {
        ...
        uint256[] memory assetAmounts = new uint256[](_numberOfAssets);
        uint256[] memory assetIds = new uint256[](_numberOfAssets);

        for (uint256 i = 0; i < _numberOfAssets; i++) {
            CCMCSupply.increment();
            assetAmounts[i] = 1;
            assetIds[i] = CCMCSupply.current();
        }

        _mintBatch(msg.sender, assetIds, assetAmounts, "");
