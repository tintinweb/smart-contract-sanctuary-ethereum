...
contract dynamicStakingContract {
    //////////// CHANGES IN CONTRACT ////////////////
    ...
    uint256[] public percentReward = [400, 600, 800, 900];
    ...
    /////////// EVENTS //////////
    event tokenStaked(address indexed owner, uint256 indexed userStakeCount);
    event tokenUnstaked(address indexed owner, uint256 indexed userStakeCount);
    ...
    event newPlanAdded(uint256 indexed timeInSeconds, uint256 indexed rewardPercentPlus_2_Zeros);
    ...
    // add new plan
    function addNewPlan(uint256 timeInSeconds, uint256 rewardPercentPlus_2_Zeros) public {
        require(isLocked == false, "Contract is lock.");      

        require(msg.sender == owner, "only owner can add");   
        durations.push(timeInSeconds);       // push duration at the end of Durations array 
        percentReward.push(rewardPercentPlus_2_Zeros);       // push tokenPercent (to Measure amount of reward) at the end of rewardcalculatebasedamount array

        emit newPlanAdded(timeInSeconds,rewardPercentPlus_2_Zeros);
    }
