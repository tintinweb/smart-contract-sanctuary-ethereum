...
contract ERC721AVault is Ownable, IERC721Receiver {
    ...
    constructor(address _erc721Address) {
        god = God(_erc721Address);
    }

    function getClaimable(address _address) public view returns (uint256[] memory) {
        uint256 _currentSupply = god.totalSupply();
        uint256[] memory tokenIds = new uint256[](_currentSupply);
        for (uint256 i = 0; i < _currentSupply; i++) {
            if (god.ownerOf(i) == _address && !claimed[i]) {
                tokenIds[i] = i;
            }
        }
        return tokenIds;
    ...
    function claim(uint256[] memory _tokenIds) public {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(god.ownerOf(_tokenIds[i]) == msg.sender, "Not owner");
