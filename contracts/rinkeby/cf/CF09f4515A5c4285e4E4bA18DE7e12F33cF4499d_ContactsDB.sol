    ...
    function FactoryReset() public onlyOwner {
        ...
        for (uint256 i = 0; i < Roles.length; i++) {
            for (uint256 i2 = 0; i2 < Countries.length; i2++) {
                for (
                    uint256 i3 = 0;
                    i3 < RoleCountry_Address[uint8(i)][uint8(i2)].length;
                    i3++
                ) {
                    RoleCountry_Address[uint8(i)][uint8(i2)][i3] = address(0);
                }
            }
        }
        for (uint256 i = 0; i < Countries.length; i++) {
            for (uint256 i2 = 0; i2 < Countries.length; i2++) {
                for (
                    uint256 i3 = 0;
                    i3 < CountryCountry_Address[uint8(i)][uint8(i2)].length;
                    i3++
                ) {
                    CountryCountry_Address[uint8(i)][uint8(i2)][i3] = address(
                        0
                    );
                }
            }
