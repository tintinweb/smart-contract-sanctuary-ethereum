...
contract CRCLSCatalystPass is ERC721A, ERC2981Royalties, Ownable, VRFConsumerBaseV2 {
    ...
    uint256 public constant PRICE = 0.35 ether;
    ...
    constructor(uint256 royaltyPercentage, uint64 linkSubId, string memory baseTokenURI) ERC721A("CRCLS Catalyst Pass", "OG") VRFConsumerBaseV2(vrfCoordinator) {
        // EIP2987
        _setRoyalties(msg.sender, royaltyPercentage);

        // Initialize Chainlink coordinator
        _oracle = VRFCoordinatorV2Interface(vrfCoordinator);
    ...
    function mint(uint8 amount, bytes32[] calldata proof) external payable {
        ...
        require(_totalMinted() < MAX_SUPPLY, "SOLD OUT!");
        require(msg.value >= amount * PRICE, "Not enough eth");
        require(amount <= MAX_PUBLIC_MINT, "Requesting too many");
        require(_totalMinted() + amount <= MAX_SUPPLY, string(abi.encodePacked("Only ", (MAX_SUPPLY - _totalMinted()).toString(), " left.")));
        ...
        if (saleStatus == Status.PRESALE_LIVE) {
            ...
            require(_numberMinted(sender) < MAX_PUBLIC_MINT, "Max amount exceeded");
        }

        _safeMint(sender, amount);

        if (_totalMinted() == MAX_PUBLIC_MINT) {
    ...
    function requestRandomOwner() external onlyOwner() {
        _oracle.requestRandomWords(keyHash, _linkSubId, vrf_confs, callbackGasLimit, 1);
    }
    ...
    function isOnAccessList(bytes32[] calldata proof) public view returns (bool) {
