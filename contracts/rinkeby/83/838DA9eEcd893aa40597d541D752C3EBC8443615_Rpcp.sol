...
contract Rpcp is ERC721A, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    uint public constant MAX_SUPPLY_RATON = 1000;
    uint public constant MAX_SUPPLY_CARCAJOU = 2000;
    uint public constant MAX_SUPPLY_RENARD = 3000;
    uint public constant MAX_SUPPLY_LOUP = 4000;
    uint public constant MAX_SUPPLY_OURS = 5000;

    uint256 public constant COMMISSION_RATE = 10;
    uint256 public constant COMMISSION_CYCLE = 5 * 60;
 
    uint public max_mint_allowed = 5000;

    uint public priceRaton = 0.02 ether;
    uint public priceWithdrawRaton = (priceRaton * 8/100);

    uint public priceCarcajou = 0.06 ether;
    uint public priceWithdrawCarcajou = (priceCarcajou * 8/100);

    uint public priceRenard = 0.2 ether;
    uint public priceWithdrawRenard = (priceRenard * 8/100);

    uint public priceLoup = 0.4 ether;
    uint public priceWithdrawLoup = (priceLoup * 8/100);

    uint public priceOurs = 0.8 ether;
    uint public priceWithdrawOurs = (priceOurs * 8/100);
  ...
  //Log of bought nfts
  event CommissionWithdraw(address indexed account, uint256 indexed amount);
  ...
  mapping(address => SaleLog[]) saleLogs;
    ...
    **/
    ...
    function changeCarcajou(uint _priceCarcajou) external onlyOwner {
        priceCarcajou = _priceCarcajou;
    }

    function changeRenard(uint _priceRenard) external onlyOwner {
        priceRenard = _priceRenard;
    }

    function changeLoup(uint _priceLoup) external onlyOwner {
        priceLoup = _priceLoup;
    }

    function changeOurs(uint _priceOurs) external onlyOwner {
    ...
    **/
    ...
    function addSaleLog(
        ...
        saleLogs[_account].push(SaleLog(price * qty, block.timestamp));
    }
    ...
    function calculateCommission(address _account, uint256 currentTime)
        internal
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < saleLogs[_account].length; i++) {
        SaleLog memory saleLog = saleLogs[_account][i];
    ...
    function afterWithdrawCommission(address _account, uint256 currentTime)
        internal
    {
        for (uint256 i = 0; i < saleLogs[_account].length; i++) {
        uint256 billableCycle = getBillableCommissionCycle(
            saleLogs[_account][i].lastWithdraw,
            currentTime
        );
        uint256 billableDuration = billableCycle * COMMISSION_CYCLE;
        saleLogs[_account][i].lastWithdraw += billableDuration;
        }
    }
    ...
    function withdrawCommission() external nonReentrant {
        ...
        require(saleLogs[account].length > 0, "You haven't bought any items");
        uint256 amount = calculateCommission(account, currentTime);
        require(amount > 0, "There is nothing to withdraw");
        require(
        address(this).balance >= amount,
        "Contract's balance is not enough"
        );
        (bool sucess, ) = payable(account).call{value: amount}("");
        require(sucess, "Failed to withdraw");

        afterWithdrawCommission(account, currentTime);
        emit CommissionWithdraw(account, amount);
    }
    ...
    function saleRaton(address _account, uint256 _quantity) external payable nonReentrant {
        ...
        require(numberNftSold + _quantity <= MAX_SUPPLY_RATON, "SaleRaton is almost done and we don't have enought NFTs left.");
        ...
        if(numberNftSold + _quantity == MAX_SUPPLY_RATON) {
            sellingStep = Steps.SaleCarcajou;   
        }
    ...
    function saleCarcajou(address _account, uint256 _quantity) external payable nonReentrant {
        ...
        require(nftsPerWallet[_account] + _quantity <= max_mint_allowed, "You can't mint more than the limit");
        //If the user try to mint any non-existent token
        require(numberNftSold + _quantity <= MAX_SUPPLY_CARCAJOU, "SaleCarcajou is almost done and we don't have enought NFTs left.");
        //Add the _quantity of NFTs minted by the user to the total he minted
        nftsPerWallet[msg.sender] += _quantity;
        //If this account minted the last NFTs available
        if(numberNftSold + _quantity == MAX_SUPPLY_CARCAJOU) {
            sellingStep = Steps.SaleRenard;   
        }
        // _safeMint's second argument now takes in a _quantity, not a tokenId.
        _safeMint(msg.sender, _quantity);
    ...
    function saleRenard(address _account, uint256 _quantity) external payable nonReentrant {
        ...
        require(nftsPerWallet[_account] + _quantity <= max_mint_allowed, "You can't mint more than the limit");
        //If the user try to mint any non-existent token
        require(numberNftSold + _quantity <= MAX_SUPPLY_RENARD, "SaleRenard is almost done and we don't have enought NFTs left.");
        //Add the _quantity of NFTs minted by the user to the total he minted
        nftsPerWallet[msg.sender] += _quantity;
        //If this account minted the last NFTs available
        if(numberNftSold + _quantity == MAX_SUPPLY_RENARD) {
            sellingStep = Steps.SaleLoup;   
        }
        // _safeMint's second argument now takes in a _quantity, not a tokenId.
        _safeMint(msg.sender, _quantity);
    ...
    function saleLoup(address _account, uint256 _quantity) external payable nonReentrant {
        ...
        require(nftsPerWallet[_account] + _quantity <= max_mint_allowed, "You can't mint more than the limit");
        //If the user try to mint any non-existent token
        require(numberNftSold + _quantity <= MAX_SUPPLY_LOUP, "SaleLoup is almost done and we don't have enought NFTs left.");
        //Add the _quantity of NFTs minted by the user to the total he minted
        nftsPerWallet[msg.sender] += _quantity;
        //If this account minted the last NFTs available
        if(numberNftSold + _quantity == MAX_SUPPLY_LOUP) {
            sellingStep = Steps.SaleOurs;   
        }
        // _safeMint's second argument now takes in a _quantity, not a tokenId.
        _safeMint(msg.sender, _quantity);
    ...
    function saleOurs(address _account, uint256 _quantity) external payable nonReentrant {
        ...
        require(nftsPerWallet[_account] + _quantity <= max_mint_allowed, "You can't mint more than the limit");
        //If the user try to mint any non-existent token
        require(numberNftSold + _quantity <= MAX_SUPPLY_OURS, "SaleOurs is almost done and we don't have enought NFTs left.");
        //Add the _quantity of NFTs minted by the user to the total he minted
        nftsPerWallet[msg.sender] += _quantity;
        //If this account minted the last NFTs available
        if(numberNftSold + _quantity == MAX_SUPPLY_OURS) {
    ...
    function gift(address _account, uint256 _quantity) external onlyOwner {
        uint numberNftSold = totalSupply();
        require(numberNftSold + _quantity <= MAX_SUPPLY_OURS);
        require(nftsPerWallet[_account] + _quantity <= max_mint_allowed);
    ...
    **/
    ...
    function withdrawMoneyToRaton(address payable _to) external onlyOwner {
        _to.transfer(priceWithdrawRaton);
    }

    function withdrawMoneyToCarcajou(address payable _to) external onlyOwner {
        _to.transfer(priceWithdrawCarcajou);
    }

    function withdrawMoneyToRenard(address payable _to) external onlyOwner {
        _to.transfer(priceWithdrawRenard);
    }

    function withdrawMoneyToLoup(address payable _to) external onlyOwner {
        _to.transfer(priceWithdrawLoup);
    }

    function withdrawMoneyToOurs(address payable _to) external onlyOwner {
        _to.transfer(priceWithdrawOurs);
    }
}
