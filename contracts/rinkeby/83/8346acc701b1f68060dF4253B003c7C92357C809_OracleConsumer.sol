...
// TODO: Refactor to OracleConsumer calling internal REST API
...
contract OracleConsumer is ChainlinkClient, ConfirmedOwner {
 ...
 function requestCollectItemsLevel(address _oracle, string memory _jobId)
    public
    onlyOwner
  {
    Chainlink.Request memory req = buildChainlinkRequest(stringToBytes32(_jobId), address(this), this.fulfillCollectItemsLevel.selector);
    req.add("get", "http://simpleservice.test/collect-items/");
    req.add("path", "items,0,collect_level");
  ...
  {
    emit RequestCollectItemsLevelFulfilled(_requestId, _level);
    itemLevel = _level;
  }
