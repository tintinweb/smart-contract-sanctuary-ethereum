    ...
    OwnableUpgradeable
{
    ...
    */
    ...
    /**
    @notice Change status of the multiple bowls, can only be set by contract owner.
    @param tokenIds_ Array  of the token ids
    @param bowlStatus_ new bowl status
     onlyOwner
    */
    function setMultipleBowlsStatus(uint256[] calldata tokenIds_, bool bowlStatus_)
        external
        onlyOwner
    {
        uint256 tokenId;
        for (uint256 i = 0; i < tokenIds_.length; ) {
            ...
            if(_exists(tokenId) && bowls[tokenId] != bowlStatus_){
                bowls[tokenId] = bowlStatus_;
            }
            unchecked {
                ++i;
            }
