...
contract CoolCows is ERC721A, Ownable, ReentrancyGuard {
    ...
    address private constant TEAM = 0xfF3C7d4E5304a9bE9072C95a78c5D169A957Ebd8;
    address private constant LAIR_LABS = 0x944A10521bE41A3867C25a72F74c62f059f04624;
    ...
    function mintGatekeeper(uint256 quantity) private view {
        require(msg.sender == tx.origin, "No minting from a contract");
        require(totalSupply() + quantity <= MAX_SUPPLY, "Maximum supply exceeded");
    }

    function mintPresaleTokens(bytes32[] calldata merkleProof, uint256 quantity) external payable {
        ...
        require(getPresaleStatus(merkleProof), "Invalid merkle proof");
        require(_numberMinted(msg.sender) + quantity <= MAX_MINT_PER_PRESALE, "Maximum presale mints reached");
        _mint(msg.sender, quantity);
    }

    function mintTokens(uint256 quantity) external payable {
    ...
    function mintTokensXMint(address to, uint256 _count) external payable {
    ...
    function getPresaleStatus(bytes32[] calldata _merkleProof) public view returns (bool) {
