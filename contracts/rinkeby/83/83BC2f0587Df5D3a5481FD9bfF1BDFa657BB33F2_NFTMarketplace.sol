...
contract NFTMarketplace is ReentrancyGuard {
    ...
    function setNewOwner(address payable _address) public {
        require(msg.sender == marketowner, "you are not an owner");
        marketowner = _address;
    }

    function setPercent(uint _amount) public {
        require(msg.sender == marketowner, "you are not an owner");
        require(_amount < 1000, "too large");
        percent = _amount;
    }
    ...
    function createMarketSaleForTokens(uint256 id) public payable nonReentrant {
        //users
        MarketItem storage item = marketItems[id]; //should use storge!!!!
        uint price = item.price;
        uint tokenId = item.tokenId;

        if (isSupervisorFunction(msg.sender) == true) {
            price = (item.price / 1000) * (1000 - percent);
        } else {
            price = item.price;
        }
        ...
        //     "NFT must be approved to market"
        // );
        _MetaVanguardToken.transferFrom(address(msg.sender), address(item.seller), price);
        item.buyer = payable(msg.sender);
        item.state = State.Release;
        _itemSoldCounter.increment();
        //что то вроде записываем за сколько купил, за сколько продал
    ...
    function getPrice(uint256 id) public view returns (uint) { //for users
        MarketItem storage item = marketItems[id]; //should use storge!!!!
        uint price = item.price;
        if (isSupervisorFunction(msg.sender) == true) {
             price = (item.price / 1000) * (1000 - percent);
