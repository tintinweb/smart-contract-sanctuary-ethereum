...
contract Gauge {
    ...
    address public immutable _veDOP; // the veDOP token used for gauges
    ...
    constructor(address _stake, address _bribe, address  __veDOP, address _voter) {
        stake = _stake;
        bribe = _bribe;
        _veDOP = __veDOP;
        voter = _voter;
    ...
    function _claimFees() internal returns (uint claimed0, uint claimed1) {
        (claimed0, claimed1) = IDOPCore(stake).claimFees();
        if (claimed0 > 0 || claimed1 > 0) {
            uint _fees0 = fees0 + claimed0;
            uint _fees1 = fees1 + claimed1;
            (address _token0, address _token1) = IDOPCore(stake).tokens();
    ...
    */
    ...
    function derivedBalance(address account) public view returns (uint) {
        ...
        uint _adjusted = 0;
        uint _supply = erc20(_veDOP).totalSupply();
        if (account == veDOP(_veDOP).ownerOf(_tokenId) && _supply > 0) {
            _adjusted = veDOP(_veDOP).balanceOfNFT(_tokenId);
    ...
    function deposit(uint amount, uint tokenId) public lock {
        ...
        if (tokenId > 0) {
            require(veDOP(_veDOP).ownerOf(tokenId) == msg.sender);
...
contract DOPGaugeFactory {
    address public last_gauge;

    function createGauge(address _pool, address _bribe, address _veDOP) external returns (address) {
        last_gauge = address(new Gauge(_pool, _bribe, _veDOP, msg.sender));
        return last_gauge;
    }

    function createGaugeSingle(address _pool, address _bribe, address _veDOP, address _voter) external returns (address) {
        last_gauge = address(new Gauge(_pool, _bribe, _veDOP, _voter));
        return last_gauge;
    }
}