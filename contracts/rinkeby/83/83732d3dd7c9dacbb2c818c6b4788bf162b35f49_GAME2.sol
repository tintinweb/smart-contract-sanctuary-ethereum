...
contract GAME2 is Ownable {
    ...
    event Player1Joined(address player1);
    event Player2JoinedGameStarted(address player1, address player2);
    ...
    function joinGame() public payable returns(address playerAddress, uint256 playerNumber, uint256 gameNumber) {
        ...
        if (playerOneTrue) {
            ...
            playerOneTrue = false;
            emit Player1Joined(msg.sender);
            return(msg.sender, 1, gameCount);
        } else {
            Game storage game = games[gameCount];
            ...
            playerOneTrue = true;
            emit Player2JoinedGameStarted(game.player1, msg.sender);
            return(msg.sender, 2, gameCount);
        }
    }
