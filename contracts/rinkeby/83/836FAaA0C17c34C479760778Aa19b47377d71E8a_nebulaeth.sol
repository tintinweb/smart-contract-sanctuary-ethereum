...
/// ============ the contract ============
contract nebulaeth is ERC721Delegated {
    /// ============ get epochs ============
    ...
    string[] private colors = [
        string(abi.encodePacked("ff", getSlice(1, 4, addressToString(msg.sender)))),
        getSlice(5, 10, addressToString(msg.sender)),
        getSlice(11, 16, addressToString(msg.sender)),
        getSlice(17, 22, addressToString(msg.sender)),
        getSlice(23, 28, addressToString(msg.sender)),
        getSlice(29, 34, addressToString(msg.sender)),
        getSlice(35, 40, addressToString(msg.sender))
    ];

    function getColor1() internal view returns (string memory) {
        return pluck(getCurrentEpoch()[0], "COLORS", colors);
    }
    function getColor2() internal view returns (string memory) {
        return pluck(tokenCount, "COLORS", colors);
    }
    function getColor3() internal view returns (string memory) {
        return pluck(block.number, "COLORS", colors);
    }

    function getSvg() private view returns (string memory) {
        string[18] memory parts;
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 1000 1000"><radialGradient id="a"><stop stop-color="#';
        parts[1] = getColor1();
        parts[2] = '"/><stop offset=".25" stop-color="#';
        parts[3] = getColor2();
        parts[4] = '"/><stop offset=".4" stop-color="#';
        parts[5] = getColor3();
        parts[6] = '"/><stop offset=".5"/></radialGradient><filter id="b"><feTurbulence baseFrequency=".';
        parts[7] = uintToString(getCurrentEpoch()[0]);
        parts[8] = '" seed="';
        parts[9] = uintToString(block.number);
        parts[10] = '"/><feColorMatrix values="0 0 0 9 -5 0 0 0 9 -5 0 0 0 9 -5 0 0 0 0 1" result="s"/><feTurbulence type="fractalNoise" baseFrequency=".0';
        parts[11] = uintToString(getCurrentEpoch()[0]);
        parts[12] = uintToString(getCurrentEpoch()[1]);
        parts[13] = '" numOctaves="';
        parts[14] = uintToString(getCurrentEpoch()[2]);
        parts[15] = '" seed="';
        parts[16] = uintToString(getCurrentEpoch()[3]);
        parts[17] = '"/><feDisplacementMap in="SourceGraphic" scale="99"/><feBlend in="s" mode="screen"/></filter><circle cx="50%" cy="50%" r="50%" fill="url(#a)" filter="url(#b)"/></svg>';
        ...
        string memory metadata = string(
            abi.encodePacked('{"name": "nebulaeth #')
        );
        metadata = string(
            abi.encodePacked(
                metadata,
                uintToString(tokenCount + 1),
                '", "description": "A nebulaeth discovered  in ',
                getEpochLabels()[6],
                " ",
                uintToString(getCurrentEpoch()[6]),
                " ",
                getEpochLabels()[5],
                " "
            )
        ...
        metadata = string(
            abi.encodePacked(
                metadata,
                uintToString(getCurrentEpoch()[5]),
                " ",
                getEpochLabels()[4],
                " ",
                uintToString(getCurrentEpoch()[4]),
                " by the ",
                getStricture(),
                "0x",
        ...
        metadata = string(
            abi.encodePacked(
                ...
                getEpochLabels()[3],
                '","value": "',
                uintToString(getCurrentEpoch()[3]),
                '"},{"trait_type": "',
                getEpochLabels()[2],
                '","value": "',
                uintToString(getCurrentEpoch()[2]),
                '"},{"trait_type": "',
                getEpochLabels()[1],
                '","value": "',
                uintToString(getCurrentEpoch()[1]),
                '"},{"trait_type": "',
                getEpochLabels()[0],
                '","value": "',
                uintToString(getCurrentEpoch()[0]),
                '"}]'
            )
        );
    ...
    /// ============ token functions ============
    /// @notice mint a nebulaeth
    function mint() public {
        require(tokenCount < (maxTokens), "No more Nebulaeth to discover.");
        string memory svgData = getSvg();
        metadataJson[tokenCount] = svgData;
    ...
    function pluck(
        ...
        uint256 rand = random(
            string(abi.encodePacked(keyPrefix, uintToString(tokenId)))
        );
