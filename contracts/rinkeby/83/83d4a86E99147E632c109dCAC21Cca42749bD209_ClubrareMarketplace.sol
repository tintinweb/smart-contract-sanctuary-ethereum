    ...
    EIP712Upgradeable
{
    ...
    function initialize(
        ...
        require(
            keccak256(
                "Order_t(address seller,address contractAddress,uint256 royaltyFee,address royaltyReceiver, address paymentToken,uint256 basePrice,uint256 listingTime,uint256 expirationTime,uint256 nonce,uint256 tokenId,Type orderType,string uri,string objId)"
            ) == _ORDER_TYPEHASH
        );
    ...
    function getCurrentOrder_tNonce(address owner) public view returns (uint256) {
    ...
    modifier onlySeller(Order_t calldata order,bytes memory signature) {
        require(verifySeller(order,signature), "Not a seller");
        _;
    }
    ...
    // modifier onlyPhyiscal(Order_t_t calldata _order) {
    //     require(_order.orderType == Type.Physical, "Not a physical NFT");
    //     _;
    // }

    // modifier onlyDigital(Order_t calldata _order) {
    ...
    {
        (bytes32 digest, address signer) = _verifySig(order,signature);
        bool isToken = order.paymentToken == address(0) ? false : true;
        uint256 paid = isToken ? amount : msg.value;
        require(paid > 0, "invalid amount");
        require(validateOrder(digest, order, signature, paid), "Invalid Order_t");
        Order_t calldata _order = order;
    ...
    function acceptOffer(
        ...
        whenNotPaused
        nonReentrant
        onlySeller(order,signature)
        isAllowedToken(order.paymentToken)
        isNotBlacklisted(msg.sender)
        onlyApproved(order.contractAddress, order.tokenId,order.seller)
    {
        (bytes32 digest, address signer) = _verifySig(order,signature);
        require(validateOrder(digest, order, signature, _amount), "Invalid Order_t");
        Order_t calldata _order = order;
        uint256 amt = _amount;
        address taker = buyer;
    ...
    function InvalidateSignedOrder(Order_t calldata order,bytes memory signature) external whenNotPaused nonReentrant {
        (bytes32 digest, address signer) = _verifySig(order,signature);
    ...
    function cancelOrder_t(uint256 _orderId, uint256 _trackingId)
        external
        whenNotPaused
        nonReentrant
        isNonZero(_orderId)
    ...
    function _executeExchange(
       ...
       // _invalidateSignedOrder_(order.contractAddress, order);
    ...
    function _invalidateSignedOrder_t(address contractAddress, Order_t calldata order,bytes memory signature) internal {
        (bytes32 digest, address signer) = _verifySig(order,signature);
    ...
    function verifySeller(Order_t calldata order, bytes memory signature) internal view returns (bool) {
        address owner = IERC721Upgradeable(order.contractAddress).ownerOf(order.tokenId);
        (, address signer) = _verifySig(order,signature);
        return owner == signer;
    }

    // =================== Validators ===================

    function _verifySig(Order_t calldata order, bytes memory signature) public view returns (bytes32, address) {
        bytes32 digest = hashOrder_t(order);
        address signer = ECDSAUpgradeable.recover(digest, signature);
        return (digest, signer);
    }

    function _verifySi(Order_t calldata order,bytes memory signature) public view returns (bytes32, address) {
        bytes32 digest = hashToSign(order);
        address signer = ECDSAUpgradeable.recover(digest,signature);
        return (digest, signer);
    }

    function _verifyR(Order_t calldata order,bytes memory signature) public view returns (bytes32, address) {
        bytes32 digest = hashOrder_t(order);
    ...
    function _verifyV(Order_t calldata order,bytes memory signature) public view returns (bytes32, address) {
        bytes32 digest = hashToSign(order);
        (bytes32 r, bytes32 s,uint8 v) = splitSignature(signature);
        address signer = ecrecover(digest, v, r,s);
        return (digest, signer);
     ...
     * @param hash Order_t hash (already calculated, passed to avoid recalculation)
     * @param order Order_t to validate
     * @param sig ECDSA signature
     */
    function validateOrder(
        bytes32 hash,
        Order_t memory order,
        bytes memory sig,
        uint256 amount
    ) internal view returns (bool validated) {
        /* Not done in an if-conditional to prevent unnecessary ecrecover evaluation, which seems to happen even though it should short-circuit. */

        /* Order_t must have valid parameters. */
        if (!validateOrder_tParameters(order, amount)) {
            return false;
        }

        /* Order_t must have not been canceled or already filled. */
     ...
     * @param order Order_t to validate
     */
    function validateOrder_tParameters(Order_t memory order, uint256 amount) internal pure returns (bool) {
        /* Order_t must have a maker. */
     ...
     * @param _order Order_t to hash
     * @return hash Hash of order
     */
    function hashOrder_t(Order_t memory _order) public view returns (bytes32 hash) {
        bytes memory array = abi.encodePacked(
             keccak256(
                "Order_t(address seller,address contractAddress,uint256 royaltyFee,address royaltyReceiver,address paymentToken,uint256 basePrice,uint256 listingTime,uint256 expirationTime,uint256 nonce,uint256 tokenId,string uri,string objId)"
            ),
            _order.seller,
            _order.contractAddress,
            _order.royaltyFee,
     ...
     * @param order Order_t to hash
     * @return Hash of message prefix and order hash per Ethereum format
     */
    function hashToSign(Order_t memory order) public view returns (bytes32) {
        return keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, hashOrder_t(order)));
    }
