...
// File: contracts/AddressArrayLib.sol
...
// File: contracts/RolesManager.sol

pragma solidity ^0.8.0;
pragma experimental ABIEncoderV2;

abstract contract RolesManager {
    event UpdateRole(address user);
    ...
    function _removeAdmin(address _admin) internal roleRequired(ADMIN_ROLE) {
        uint256[5] memory _roles = [
            WITHDRAWER_ROLE,
            ADMIN_ROLE,
            COLLECTION_CREATER_ROLE,
    ...
    function renounceOwnership() public roleRequired(ADMIN_ROLE) {
    ...
    RolesManager
{
    ...
    // Struct to store multiple time frames
    ...
    function createCollection(collection memory _collection)
        public
        roleRequired(COLLECTION_CREATER_ROLE)
    {
        // Validate the VIP timeings.
        require(
            _collection.timings.VIPStartTiming <
                _collection.timings.VIPEndingTiming &&
        ...
        require(
            _collection.timings.whitelistedStartTiming <
                _collection.timings.whitelistedEndingTiming &&
    ...
    function updateCollectionTimePrice(
        ...
        collectionTimeings memory _timings
    ) public roleRequired(COLLECTION_CREATER_ROLE) {
        // Validate the VIP timeings.
        require(
            _timings.VIPStartTiming < _timings.VIPEndingTiming &&
                _timings.VIPEndingTiming > block.timestamp,
            "ERC1155: VIP start time must be less than VIP end time, VIP end time must be greater than current time "
        );
        require(
            _timings.VIPStartTiming >= block.timestamp,
            "ERC1155: VIP start time must be greater than or equal to current time"
        );

        // Validate the Whitelisted timeings.
        require(
            _timings.whitelistedStartTiming <
                _timings.whitelistedEndingTiming &&
                _timings.whitelistedEndingTiming > block.timestamp,
            "ERC1155: Whitelisted start time must be less than whitelisted end time , whitelisted end time greater than current time "
        );
        require(
            _timings.whitelistedStartTiming >= _timings.VIPEndingTiming,
            "ERC1155: whitelisted start time must be greater than or equal to VIP end time"
        );

        collection storage _collection = collections[collectionIndex];

        _collection.timings.VIPStartTiming = _timings.VIPStartTiming;
        _collection.timings.VIPEndingTiming = _timings.VIPEndingTiming;
        _collection.timings.whitelistedStartTiming = _timings
            .whitelistedStartTiming;
        _collection.timings.whitelistedEndingTiming = _timings
            .whitelistedEndingTiming;
        _collection.timings.normalStartingTime = _timings.normalStartingTime;
        _collection.timings.normalEndingTime = _timings.normalEndingTime;

        if (_priceForVIP > 0 && _priceForWhitelisted > 0 && _price > 0) {
            _collection.priceForVIP = _priceForVIP;
            _collection.priceForWhitelisted = _priceForWhitelisted;
            _collection.price = _price;
        }

        emit CollectionUpdated(collectionIndex, block.timestamp, msg.sender);
    }

    function UpdateCollectionUri(
        uint256 collectionIndex,
        string memory newCollectionUri
    ) public roleRequired(COLLECTION_CREATER_ROLE) {
        collection storage _collection = collections[collectionIndex];
        _collection.uri = newCollectionUri;
    }

    function UpdateTokenURIs(
        uint256[] memory _tokenIDs,
        string[] memory _tokenURIs
    ) public roleRequired(ADMIN_ROLE) {
        require(
            _tokenIDs.length == _tokenURIs.length,
            "tokenID and tokenURI length mismatch !"
        );
    ...
    //Get all tokenIDs of a  collection 

    function getCollectionTokenIDs(uint256 _collectionIndex) public view returns(uint256[] memory ){
    ...
    function mint(
        ...
        // Check if collection is in whitelisted minting phase:
            ...
            require(
                _collection.timings.normalStartingTime <= block.timestamp &&
        ...
        tokenIdIndex++;
        // emit MintToken(collectionIndex, _collection.totalSupply, _tokenURI, to);
    }
    ...
    // Method to Mint NFT to toAddress without the payment.
    function multipleMinting(
        address toAddress,
        uint256 collectionIndice,
        uint256[] memory editions,
        string[] memory _tokenURIs
    ) public {
        require(
            editions.length == _tokenURIs.length,
            "Length should be same between  'editions' and '_tokenURIs'"
        );

        for (uint256 index = 0; index < editions.length; index++) {
            require(
                collectionIndice < collections.length,
                "ERC1155: Collection doesn't exist."
            );
            collection storage _collection = collections[collectionIndice];
            require(
                editions[index] == _collection.totalSupply + 1,
                "ERC1155: Edition not allowed to create."
            );
            ...
            collectionTokens[collectionIndice].push(tokenIdIndex);
            _collection.totalSupply++;
            tokenURIs[tokenIdIndex] = _tokenURIs[index];
            tokenIdIndex++;
        }
