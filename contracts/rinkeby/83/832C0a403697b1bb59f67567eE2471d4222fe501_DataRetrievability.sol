 ...
 */
contract DataRetrievability is Ownable {
    ...
    struct Deal {
        ...
        uint256 value;
        // Quantity of token already canceled
        // uint256 payouts;
        // Address of provider
    ...
    mapping(address => Referee) private referees;
    ...
    // Multipliers
    uint256 slashing_multiplier = 10;
    uint256 deposit_multiplier = 100;
    ...
    */
    function getRound(uint256 appeal_index) public view returns (uint256) {
        ...
        // console.log("Appeal duration is %s", appeal_duration);
        uint256 appeal_end = appeals[appeal_index].origin_timestamp +
            appeal_duration;
        // console.log("Appeal end timestamp is %s", appeal_end);
        if (appeal_end >= block.timestamp) {
            uint256 remaining_time = appeal_end - block.timestamp;
            // console.log("Remaining time is %s", remaining_time);
            uint256 remaining_rounds = remaining_time / round_duration;
            // console.log("Remaining rounds are %s", remaining_rounds);
    ...
    */
    function cancelDealProposal(uint256 deal_index) external {
        require(
    ...
    */
    function acceptDealProposal(uint256 deal_index) external {
        ...
        uint256 slash_deposit = deals[deal_index].value * slashing_multiplier;
        require(
            vault[msg.sender] >= slash_deposit &&
        ...
        // Deposit slashing deposit to contract
        vault[msg.sender] -= slash_deposit;
        vault[address(this)] += slash_deposit;
    ...
    */
    function redeemDeal(uint256 deal_index) external {
        require(
            deals[deal_index].accepted == msg.sender,
        ...
        // Giving back slash deposit to provider
        uint256 slash_deposit = deals[deal_index].value * slashing_multiplier;
        vault[address(this)] -= slash_deposit;
        vault[msg.sender] += slash_deposit;
        // Close the deal
    ...
    */
    function processAppeal(
        ...
        if (appeals[appeal_index].slashes >= returnSlashesThreshold()) {
            ...
            uint256 slash_deposit = deals[deal_index].value *
                slashing_multiplier;
            vault[address(this)] -= slash_deposit;
            vault[protocol_address] += slash_deposit;
