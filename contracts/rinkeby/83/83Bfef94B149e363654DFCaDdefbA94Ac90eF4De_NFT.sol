...
contract NFT is ERC721, ChainlinkClient, ConfirmedOwner {
  ...
  function _afterTokenTransfer(address from, address to, uint256 tokenId) internal override {
    // Chainlink.Request memory transferOwnershipRequest = buildChainlinkRequest('7d80a6386ef543a3abb52817f6707e3b', address(this), this.fulfillTokenTransfer.selector);
    // transferOwnershipRequest.add('get', "http://3.26.73.78/owner.php");
    // transferOwnershipRequest.add('path', 'response');

    // sendChainlinkRequest(transferOwnershipRequest, chainlinkFee);

    updateTokenOwner();
  }

  function updateTokenOwner() public returns (bytes32 requestId) {
    Chainlink.Request memory transferOwnershipRequest = buildChainlinkRequest('7d80a6386ef543a3abb52817f6707e3b', address(this), this.fulfillTokenTransfer.selector);
    transferOwnershipRequest.add('get', "http://3.26.73.78/owner.php");
    transferOwnershipRequest.add('path', 'response');
  ...
  function fulfillTokenTransfer(bytes32 _requestId, string memory _id) public recordChainlinkFulfillment(_requestId) {
