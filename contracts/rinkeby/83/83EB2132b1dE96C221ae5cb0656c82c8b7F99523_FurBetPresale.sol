...
contract FurBetPresale is BaseContract
{
    ...
    {
        __BaseContract_init();
        start = 1659463200; // Tue Aug 02 2022 18:00:00 GMT+0000
     ...
     * Properties.
     */
    IFurBetStake public stake; // Stake contract.
    uint256 public start; // Presale start time.
    uint256 public sold; // Amount of tokens sold.
     ...
     * Mappings.
     */
    mapping(address => uint256) public purchased; // Maps address to amount of tokens purchased.
    ...
    function presaleType() public view returns (uint256)
    {
        ...
        if(block.timestamp < start + _presaleOneTime) {
            return 1;
        }
        if(block.timestamp < start + _presaleTwoTime) {
            return 2;
        }
        if(block.timestamp < start + _presaleThreeTime) {
            return 3;
        }
        if(block.timestamp < start + _presaleFourTime) {
            return 4;
        }
        if(block.timestamp < start + _presaleFiveTime) {
            return 5;
        }
        return 6;
    }
     ...
     */
    function presale(uint256 amount_) public
    {
        ...
        require(sold + amount_ <= _maxForSale_, "Presale is full.");
        if(_maxPerAddress_ > 0) {
            require(purchased[msg.sender] + amount_ <= _maxPerAddress_, "You have reached the maximum amount of tokens you can purchase.");
        }
