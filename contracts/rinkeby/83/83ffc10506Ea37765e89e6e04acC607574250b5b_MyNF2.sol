...
contract MyNF2 is ERC721Enumerable, Ownable {
    ...
    function setStartTime(uint _mintStartTime) public onlyOwner {
    ...
    function testURI(uint256 tokenId) public view virtual returns (string memory){
      string memory baseURI = _baseURI();
      return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.div(4096).toString(), "/", tokenId.toString())) : "";
    }
    ...
    function mintColors(uint _count) public payable {
      ...
      require(
        block.timestamp >= mintStartTime, "Minting has not started yet."
      );
