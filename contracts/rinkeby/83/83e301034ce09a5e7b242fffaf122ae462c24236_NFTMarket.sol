...
contract DNABase {
    ...
    function deterministicPseudoRandomDNA(uint256 _tokenId, address _minter)
        ...
        returns (uint256)
    {
        uint256 combinedParams = _tokenId + uint160(_minter) + block.timestamp;
    ...
    function getAvatarStyle(uint256 _dna) public view returns (string memory) {
...
// File contracts/1_NFTMarket.sol
    ...
    DNABase
{
    ...
    function _imageByDNA(uint256 _dna) internal view returns (string memory) {
    ...
    modifier checkMaxSupply() {
        require(_tokenIdCounter.current() < maxSupply, "No NFT left");
        _;
    }

    modifier checkValue() {
        require(msg.value >= 0.01 * 1e18, "You neet 0.01 ETH to mint");
        _;
    }

    function mint() public payable checkMaxSupply {
