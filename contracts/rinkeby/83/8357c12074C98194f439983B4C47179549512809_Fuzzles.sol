...
{   
    ...
    event onConsoleLog(string data);
    ...
    function onERC1155Received(
        ...
        require(value <= MAX_PURCHASE, "Fuzzles: amount exceeds the max of exchange"); 
        require(from != address(0), "Fuzzles: from is address(0)");
        require(!paused(), "Fuzzles: paused");
        require(value + pending + totalSupply() <= maxSupply, "Fuzzles: Cannot buy that many");
        emit onConsoleLog("OnERC115RECEIVED");
    ...
    {        
        ...
        emit onConsoleLog("fulfillrandom words1");
    ...
    function test11(string memory data) external onlyOwner {
        emit onConsoleLog(data);
    }

    function _baseURI() internal pure override returns (string memory) {
        return "https://www.collectfuzzles.com/metadata/fuzzles/";        
    }
    ...
    function updateVrfSubscriptionId(uint64 _vrfSubscriptionId) external onlyOwner {
        emit onConsoleLog("update vrf sub id " );

        vrfSubscriptionId = _vrfSubscriptionId;
        emit onConsoleLog(Strings.toString(vrfSubscriptionId));
    }
