...
contract Furychain is ERC721Enumerable, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    uint public max_mint_presale = 4;
    ...
    **/
    function presaleMint(address _account, bytes32[] calldata _proof, uint256 _ammount) external payable nonReentrant {
        ...
        //The user can only mint max 4 NFTs
        require(_ammount <= max_mint_presale, "You can't mint more than 4 tokens");
        //The user can only mint max 4 NFTs
        require(nftsPerWallet[_account] <= max_mint_presale, "You can't mint more than 4 tokens");
    ...
    **/
    function saleMint(address _account, uint256 _ammount) external payable nonReentrant {
        ...
        require(nftsPerWallet[_account] <= 6, "You can only get 6 NFT");
        //Did the user then enought Ethers to buy ammount NFTs ?
        require(msg.value >= price * _ammount, "Not enought funds.");
        //The user can only mint max 6 NFTs
        require(_ammount <= max_mint_allowed, "You can't mint more than 6 tokens");
        //The user can only mint max 6 NFTs
        require(nftsPerWallet[_account] <= max_mint_presale, "You can't mint more than 6 tokens");
        //If the user try to mint any non-existent token
        require(numberNftSold + _ammount <= MAX_SUPPLY, "Sale is almost done and we don't have enought NFTs left.");
        //Add the ammount of NFTs minted by the user to the total he minted
        nftsPerWallet[msg.sender] += _ammount;
