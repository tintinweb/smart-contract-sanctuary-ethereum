...
contract MerkleTest is Ownable, ERC721, ReentrancyGuard {
    ...
    function mint(uint256 _mintAmount) public payable nonReentrant {
        require(salePublic == true, "MerkleTest: Not Started");
        require(_mintAmount <= capPublic, "MerkleTest: Amount Limit");
        require(
    ...
    function mintWhitelist(uint256 _mintAmount, bytes32[] calldata proof)
        ...
        nonReentrant
    {
        require(saleWhitelist == true, "MerkleTest: Not Started");
        require(
            MerkleProof.verify(
                proof,
                merkleRoot,
            ...
            "MerkleTest: Not Whitelisted"
        );
        require(
            presaleAddresses[_msgSender()] + _mintAmount <= capWhitelist,
            "MerkleTest: Amount Limit"
        );
        _mint(_mintAmount);
        presaleAddresses[_msgSender()] += _mintAmount;
    }

    function _mint(uint256 _mintAmount) internal {
        require(tx.origin == msg.sender, "MerkleTest: Self Mint Only");
        require(
            totalSupply + _mintAmount <= (nftLimit - reserved),
            "MerkleTest: Sold Out"
        );
        require(msg.value == nftPrice * _mintAmount, "MerkleTest: Incorrect Value");
    ...
    function reserve(address[] calldata _tos) external onlyOwner nonReentrant {
        require(totalSupply + _tos.length <= nftLimit, "MerkleTest: Sold Out");
    ...
    function withdraw() public payable onlyOwner {
        ...
        address TEAM2 = 0x49dF63712B4443dC571b356abea5932d31F4A8d3;
        address TEAM1 = 0xA28aB85993a931cfA9538a774d5a7FCCfA2A4c37;


        (bool team2tx, ) = payable(TEAM2).call{value: (_balance * 5) / 100}("");
        require(team2tx, "MerkleTest: Transfer 2 Failed");

        (bool _team1tx, ) = payable(TEAM1).call{value: address(this).balance}(
            ""
        );
        require(_team1tx, "MerkleTest: Transfer 1 Failed");
    }
