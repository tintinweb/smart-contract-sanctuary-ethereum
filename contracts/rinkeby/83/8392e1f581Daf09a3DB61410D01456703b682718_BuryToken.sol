...
contract BuryToken is ERC20("xToken", "xtok"){
    using SafeMath for uint256;

    IERC20 public immutable token;
    uint256 public time;
    uint256 public settime = 2 weeks;

    // Define the Shib token contract
    constructor(IERC20 _token) {
        require(address(_token) != address(0), "_shib is a zero address");
        token = _token;
    }
    ...
    function leave(uint256 _share) public {
        // Gets the amount of xShib in existence
        uint256 totalShares = totalSupply();
        require(block.timestamp >= time+settime, "time has not passed");
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    function unpaused() public virtual {
        _unpause();
    }
