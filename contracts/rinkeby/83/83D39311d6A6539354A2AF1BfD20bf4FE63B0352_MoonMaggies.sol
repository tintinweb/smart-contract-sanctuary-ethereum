...
contract MoonMaggies is ERC721A, Ownable, ReentrancyGuard{
    ...
    function mint(uint256 _quantity) external payable callerIsUser{
        require(publicSale, "Moon Maggies :: Not Yet Active.");
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "Moon Maggies :: Beyond Max Supply");
        require((totalPublicMint[msg.sender] +_quantity) <= MAX_PUBLIC_MINT, "Moon Maggies :: Already minted 4 times!");
        require(msg.value >= (PUBLIC_SALE_PRICE * _quantity), "Moon Maggies :: Below ");

        totalPublicMint[msg.sender] += _quantity;
        _safeMint(msg.sender, _quantity);
    }
    //function to allow team to mint before public sale for marketing purposes or to dump on plebs - only 4 allocated to the team
