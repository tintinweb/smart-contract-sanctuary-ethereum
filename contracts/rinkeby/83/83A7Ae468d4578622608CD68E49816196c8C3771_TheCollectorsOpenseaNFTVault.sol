...
import "./TheCollectorsOpenseaNFTVaultAssetsManager.sol";

/*
    @title
    The collectors Opensea NFT Vault is the first fully decentralized product that allows a group of people to handle
    together the lifecycle of an NFT and all inside Opensea (other marketplaces will be supported by new contracts).
    ...
    other marketplaces. 50% of a BAYC with a floor price of 80 ETH worth 40 ETH. And the third one is the buying it
    ...
    @dev
    Please notice that because this contract works together with @TheCollectorsOpenseaNFTVaultAssetsManager contract
    in a proxy/implementation design patten where the proxy delegate calls to the implementation, both contracts must
    inherit from NFTVaultBase in order to make sure there isn't any conflicts with variables.
    @TheCollectorsOpenseaNFTVaultBase should be the first contract to inherit from and only
    @TheCollectorsOpenseaNFTVault can have additional inheritances and variables.
*/
contract TheCollectorsOpenseaNFTVault is TheCollectorsOpenseaNFTVaultBase, Proxy, ERC721, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    /*
        @dev
        To separate concerns and support a larger contract size, a new contract is deployed with the asset business
        logic code. It will handle all buying, selling & listing operations.
        It is used as in a proxy/implementation pattern where @TheCollectorsOpenseaNFTVault is the proxy which
        delegates call the implementation @TheCollectorsOpenseaNFTVaultAssetsManager. However, it cannot be upgraded.
    */
    address public immutable openseaAssetManager;

    constructor(
        string memory __baseTokenURI,
        address _openseaAssetManager,
        address __nftVaultAssetHolderImpl,
        address _liquidityWallet
    ) ERC721("The Collectors Opensea NFT Vault", "TheCollectorsOpenseaNFTVault") {
        // Moving tracker to 1 so we can use 0 to indicate that user doesn't have any tokens
        _tokenIdTracker.increment();
        // The base uri of the tokens
        _baseTokenURI = __baseTokenURI;
        // The opensea assets manager contract address, was used to separate concerns and support a larger contract size
        // Important to notice that this address is NOT upgradable
        openseaAssetManager = _openseaAssetManager;
        /*
            @dev
            The implementation for asset holder. Is used to significantly reduce the creation cost of a new vault.
            Please read @TheCollectorsNFTVaultOpenseaAssetProxy contract to learn more about it.
            Important to notice that this address is NOT upgradable
        */
        _nftVaultAssetHolderImpl = __nftVaultAssetHolderImpl;
    ...
    */
    function createVault(
        ...
        require(sellOrCancelSellOrderConsensus > 50 ether && sellOrCancelSellOrderConsensus <= 100 ether, "E4");
        require(buyConsensus > 50 ether && buyConsensus <= 100 ether, "E5");
        ...
        emit VaultCreated(_vaultIdTracker.current(), collection, privateVault);
        Participant[] storage participants = _vaultParticipants[_vaultIdTracker.current()];
        for (uint256 i; i < _participants.length; i++) {
            participants.push(Participant(_participants[i], 0, 0, false, address(0), 0, 0, false));
            ...
            emit ParticipantJoinedVault(_vaultIdTracker.current(), _participants[i]);
        }
        vaults[_vaultIdTracker.current()] = Vault(_vaultIdTracker.current(), vaultName, collection, 0, VoteFor.WaitingToSetTokenInfo, 0, 0,
            sellOrCancelSellOrderConsensus, 0, buyConsensus, gracePeriodForSellingOrCancellingSellOrder, 0, maxParticipants);
        vaultsExtensions[_vaultIdTracker.current()] = VaultExtension(
            !privateVault, privateVault ? 0 : minimumFunding, 0, IERC165(collection).supportsInterface(type(IERC1155).interfaceId), false
        );
        // Creating a separate contract to hold and operate the NFT
        Address.functionDelegateCall(
            _implementation(),
            abi.encodeWithSelector(TheCollectorsOpenseaNFTVaultAssetsManager.createNFTVaultOpenseaAsset.selector, _vaultIdTracker.current())
        );
        _vaultParticipants[_vaultIdTracker.current()] = participants;
    ...
    */
    function joinPublicVault(uint256 vaultId) external payable {
        ...
        require(vaultsExtensions[vaultId].publicVault, "E2");
        // There is room
        require(_vaultParticipants[vaultId].length < vaults[vaultId].maxParticipants, "E3");
        ...
        require(msg.value >= vaultsExtensions[vaultId].minimumFunding, "E5");
        Participant[] storage participants = _vaultParticipants[vaultId];
        participants.push(Participant(msg.sender, 0, 0, false, address(0), 0, 0, false));
        fundVault(vaultId);
    ...
    */
    function addParticipant(uint256 vaultId, address participant) external {
        ...
        for (uint256 i; i < participants.length; i++) {
            if (participants[i].participant == msg.sender) {
                require(participants[i].paid > 0, "E5");
                break;
            }
        }
        participants.push(Participant(participant, 0, 0, false, address(0), 0, 0, false));
    ...
    /*
        ...
        In case the % of ownership for buying is higher then the buying consensus (DAO decided to buy),
    ...
    */
        ...
        */
        if (!vaultsExtensions[vaultId].publicVault) {
            ...
            if (vault.collection != collection) {
                ...
                vaultsExtensions[vaultId].isERC1155 = IERC165(collection).supportsInterface(type(IERC1155).interfaceId);
            }
            vault.collection = collection;
        }
        vault.tokenId = tokenId;
        vault.votingFor = VoteFor.Buying;
        vaultsExtensions[vaultId].maxPriceToBuy = maxBuyPrice;
        Participant[] storage participants = _vaultParticipants[vaultId];
        bool isParticipant;
        ...
        for (uint256 i; i < participants.length; i++) {
            ...
            if (participants[i].participant == msg.sender) {
                ...
                isParticipant = true;
                participantVoteYes = participants[i].vote;
                emit NFTTokenWasSet(vault.id, vault.collection, tokenId, maxBuyPrice);
                _vote(vaultId, participants[i], true, vault.votingFor);
            } else {
        ...
        // As written in the method documentation, even if the votesPercentage is higher then the buy consensus
    ...
    /*
        ...
        In case the % of ownership for selling is higher then selling consensus (DAO decided to sell),
    ...
    */
    function setListingPrice(uint256 vaultId, uint256 listFor) external {
        ...
        for (uint256 i; i < participants.length; i++) {
            if (participants[i].participant == msg.sender) {
                // Only participants who paid can be part of the decisions making
                require(participants[i].paid > 0, "E2");
        ...
        // As written in the method documentation, even if the votesPercentage is higher then the sell consensus
        // a participant who voted yes can still reset the process
    ...
    function vote(uint256 vaultId, bool yes) external {
        ...
        for (uint256 i; i < participants.length; i++) {
            if (participants[i].participant == msg.sender) {
                ...
                require(participants[i].paid > 0, "E2");
                isParticipant = true;
                _vote(vaultId, participants[i], yes, vaults[vaultId].votingFor);
                /*
                    @dev
    ...
    */
    function fundVault(uint256 vaultId) public payable {
        ...
        for (uint256 i; i < participants.length; i++) {
            if (participants[i].participant == msg.sender) {
                isParticipant = true;
                participants[i].paid += msg.value;
                if (vaultsExtensions[vaultId].publicVault) {
                    // This is a public vault
                    require(participants[i].paid >= vaultsExtensions[vaultId].minimumFunding, "E2");
                }
    ...
    */
    function withdrawFunds(uint256 vaultId, uint256 amount) public nonReentrant {
        _requireVotingForBuyingOrWaitingForSettingTokenInfo(vaultId);
        Participant[] storage participants = _vaultParticipants[vaultId];
        for (uint256 i; i < participants.length; i++) {
            if (participants[i].participant == msg.sender) {
                require(amount <= participants[i].paid, "E2");
                if (vaultsExtensions[vaultId].publicVault && (participants[i].paid - amount) < vaultsExtensions[vaultId].minimumFunding) {
                    ...
                    amount = participants[i].paid;
                }
                participants[i].paid -= amount;
                TheCollectorsNFTVaultOpenseaAssetImpl(assetsHolders[vaultId]).sendValue(payable(participants[i].participant), amount);
                if (participants[i].paid == 0 && vaultsExtensions[vaultId].publicVault) {
    ...
    */
    ...
    /*
        ...
        was lower then the total amount that the vault was funded for
    */
    function claimVaultTokenAndGetLeftovers(uint256 vaultId) external nonReentrant {
        require(vaults[vaultId].votingFor == VoteFor.Selling || vaults[vaultId].votingFor == VoteFor.CancellingSellOrder, "E1");
        Participant[] storage participants = _vaultParticipants[vaultId];
        for (uint256 i; i < participants.length; i++) {
            if (participants[i].participant == msg.sender) {
                // Only if participate doesn't already have a partial NFT
                require(participants[i].partialNFTVaultTokenId == 0, "E2");
                // Only participants who paid can redeemToken
                require(participants[i].paid > 0, "E3");
                participants[i].partialNFTVaultTokenId = _tokenIdTracker.current();
                vaultTokens[_tokenIdTracker.current()] = vaultId;
                _mint(msg.sender, _tokenIdTracker.current());
                if (participants[i].leftovers > 0) {
                    ...
                    TheCollectorsNFTVaultOpenseaAssetImpl(assetsHolders[vaultId]).sendValue(payable(participants[i].participant), participants[i].leftovers);
                    participants[i].leftovers = 0;
                }
                emit VaultTokenClaimed(vaultId, msg.sender, _tokenIdTracker.current());
                _tokenIdTracker.increment();
                break;
            }
        }
    ...
    */
    function redeemToken(uint256 tokenId) external nonReentrant {
        ...
        uint256 vaultId = vaultTokens[tokenId];
        ...
        for (uint256 i; i < participants.length; i++) {
            if (participants[i].partialNFTVaultTokenId == tokenId) {
                _burn(tokenId);
                ...
                uint256 salePriceDeductingFees = vaults[vaultId].listFor * (PERCENTAGE_DENOMINATOR - vaults[vaultId].marketplaceAndRoyaltiesFees) / (PERCENTAGE_DENOMINATOR * 100);
                ...
                uint256 protocolFee = participants[i].collectorOwner != address(0) ? 0 : profits * PROTOCOL_FEE / PERCENTAGE_DENOMINATOR;
                // Liquidity fee, will be zero if a Collector was staked
                uint256 liquidityFee = participants[i].collectorOwner != address(0) ? 0 : profits * LIQUIDITY_FEE / PERCENTAGE_DENOMINATOR;
                // Sending proceeds
                TheCollectorsNFTVaultOpenseaAssetImpl(
                    assetsHolders[vaultId]).sendValue(payable(participants[i].participant),
                    profits - protocolFee - liquidityFee
                );
                if (protocolFee > 0) {
                    TheCollectorsNFTVaultOpenseaAssetImpl(assetsHolders[vaultId]).sendValue(payable(owner()), protocolFee);
                }
                if (liquidityFee > 0) {
                    TheCollectorsNFTVaultOpenseaAssetImpl(assetsHolders[vaultId]).sendValue(payable(liquidityWallet), liquidityFee);
                }
                if (participants[i].collectorOwner != address(0)) {
                    // In case the partial NFT was sold to someone else, the original collector owner still
                    // going to get their token back
                    TheCollectorsNFTVaultOpenseaAssetImpl(
                        assetsHolders[vaultId]).transferToken(false, participants[i].collectorOwner,
                        address(THE_COLLECTORS), participants[i].stakedCollectorTokenId
                    );
                }
                emit VaultTokenRedeemed(vaultId, participants[i].participant, tokenId);
                // Removing participate and partial NFT from storage
                // The reason to not use _removeParticipant() is because we do not need to reduce the size of the array
                delete _vaultParticipants[vaultId][i];
                delete vaultTokens[tokenId];
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return string(abi.encodePacked(_baseTokenURI, vaultTokens[tokenId].toString(), "/", tokenId.toString(), ".json"));
    }

    function getVaultParticipants(uint256 vaultId) public view returns (Participant[] memory) {
        return _vaultParticipants[vaultId];
    }

    function getParticipantPercentage(uint256 vaultId, address participant) public view returns (uint256) {
        return _getPercentage(vaultId, participant, 0);
    }

    function getTokenPercentage(uint256 tokenId) public view returns (uint256) {
        return _getPercentage(vaultTokens[tokenId], address(0), tokenId);
    }
    ...
    function isVaultSoldNFT(uint256 vaultId) public view returns (bool) {
        ...
        if (vaults[vaultId].votingFor != VoteFor.CancellingSellOrder) {
            return false;
        }
        if (vaultsExtensions[vaultId].isERC1155) {
            return IERC1155(vaults[vaultId].collection).balanceOf(assetsHolders[vaultId], vaults[vaultId].tokenId) == 0;
        } else {
            return IERC721(vaults[vaultId].collection).ownerOf(vaults[vaultId].tokenId) != assetsHolders[vaultId];
        }
    }
    ...
    */
    ...
    /*
        @dev
        This is not a standard implementation but just a business logic separation to handle
        all buying, listing and cancelling sell order operations
        It is NOT upgradable
    */
    function _implementation() override internal view virtual returns (address) {
        return openseaAssetManager;
    }

    /*
        @dev
        Overriding transfer as the partial NFT can be sold or transfer to another address.
        In case that happens, the new owners is become a participate in the vault.
    ...
    */
    function _transfer(
        ...
        if (msg.sender == IProxyRegistry(0x1E525EEAF261cA41b809884CBDE9DD9E1619573A).proxies(from)) {
            ...
            // This is an opensea sale of the vault token
            // In order to make sure no side is getting rekt,
            // a token of a sold vault cannot be traded but just redeemed so there won't be a situation where
            // a token that only worth 10 ETH is sold for more, or the other way around
            require(!isVaultSoldNFT(vaultTokens[tokenId]), "Cannot sell, only redeem");
        }
    ...
    */
    function _vote(uint256 vaultId, Participant storage participant, bool yes, VoteFor voteFor) internal {
        participant.vote = yes;
        if (voteFor == VoteFor.Buying) {
            emit VotedForBuy(vaultId, participant.participant, yes, vaults[vaultId].collection, vaults[vaultId].tokenId);
        } else {
            ...
            if (firstProposalVote) {
                ...
                vaults[vaultId].endGracePeriodForSellingOrCancellingSellOrder = block.timestamp + vaults[vaultId].gracePeriodForSellingOrCancellingSellOrder;
            }
            if (voteFor == VoteFor.Selling) {
                emit VotedForSell(vaultId, participant.participant, yes, vaults[vaultId].collection, vaults[vaultId].tokenId, vaults[vaultId].listFor);
            } else if (voteFor == VoteFor.CancellingSellOrder) {
                emit VotedForCancel(vaultId, participant.participant, yes, vaults[vaultId].collection, vaults[vaultId].tokenId, vaults[vaultId].listFor);
            }
        }
    ...
    */
    function salvageERC721Token(address collection, uint256 tokenId) public onlyOwner {
