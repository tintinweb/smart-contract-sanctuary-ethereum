...
contract Marketplace is OwnableUpgradeable {
    ...
    function addListing(string calldata _name, string calldata _image, uint256 _slots, uint256 _expiry, uint256 _price, uint256 _maxCount) public onlyAdmin {
        Listing memory listing = Listing(_name, _image, _slots, _expiry, _price, _maxCount, false);
        listings.push(listing);
    }

    function editListing(uint256 _index, string calldata _name, string calldata _image, uint256 _slots, uint256 _expiry, uint256 _price, uint256 _maxCount) public onlyAdmin {
        listings[_index].name = _name;
