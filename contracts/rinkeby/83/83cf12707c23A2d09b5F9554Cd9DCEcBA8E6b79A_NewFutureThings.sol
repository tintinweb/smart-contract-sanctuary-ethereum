...
contract NewFutureThings is ERC721A, Ownable {
    ...
    function communityMint(uint quantity, address addr) external payable {
        ...
        require(presalePrice * quantity <= msg.value, "Not enough ETH");
        require(isPresaleCommunity[addr], "Not a presale community address");
        ERC721 communityContract = ERC721(addr);
        require(communityContract.balanceOf(msg.sender) > 0, "You don't anything from this collection");
        require(presaleClaimed[msg.sender] == false, "You already claimed your presale mint");
    ...
    function _mintTokens(address addr, uint quantity) internal {
        ...
        require(end <= supply, "Exceeds supply");
        require(quantity <= transactionLimit, "Over transaction limit");

        _safeMint(addr, quantity);

        if(address(kaboomBoxMP) != 0x0000000000000000000000000000000000000000) {
    ...
    function resetPresale(address addr) external onlyOwner {
        presaleClaimed[addr] = false;
    }
