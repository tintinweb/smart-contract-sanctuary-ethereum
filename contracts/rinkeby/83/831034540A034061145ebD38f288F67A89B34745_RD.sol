...
// File @openzeppelin/contracts/token/BEP721/[email protected]
...
// File @openzeppelin/contracts/token/BEP721/extensions/[email protected]
...
error TransfBEPallerNotOwnerNorApproved();
 ...
 */
contract BEP721A is Context, BEP165, IBEP721, IBEP721Metadata, IBEP721Enumerable {
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner = BEP721A.ownerOf(tokenId);
    ...
    function safeTransferFrom(
        ...
        _transfer(from, to, tokenId);
        if (!_checkOnBEP721Received(from, to, tokenId, _data)) revert TransferToNonBEP721ReceiverImplementer();
    }
     ...
     * Requirements:
     *
     * - If `to` refers to a smart contract, it must implement {IBEP721Receiver-onBEP721Received}, which is called for each safe transfer.
    ...
    function _mint(
        ...
        unchecked {
            ...
            for (uint256 i; i < quantity; i++) {
                ...
                if (safe && !_checkOnBEP721Received(address(0), to, updatedIndex, _data)) {
                    revert TransferToNonBEP721ReceiverImplementer();
                }
    ...
    function _transfer(
        ...
        if (!isApprovedOrOwner) revert TransfBEPallerNotOwnerNorApproved();
    ...
    function _checkOnBEP721Received(
        ...
        if (to.isContract()) {
            try IBEP721Receiver(to).onBEP721Received(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) revert TransferToNonBEP721ReceiverImplementer();
                else {
                    assembly {
...
// File contracts/RD.sol

contract RD is BEP721A, Ownable {

    using Strings for uint256;

    string public PROVENANCE;
    uint256 public tokenPrice = 100000000000; // 0.000001
    ...
    // Test wallet
    // address walletAddress = 0x50780800F96742881793B7221f6928402238C851;
     ...
     * 3:   not started
     */ 
    uint8 public mintStep = 3;

    // uint8 private LIMIT1 = 20; // presale for 8 hours : can mint only 1
    // uint256 firstPresaleTime = 3600 * 24;
    uint256  presaleEndTime = 0;

    uint256 public presaleStartTime = 0;


    // mapping (address => uint8) addressFirstPresaleCountMap;     // Up to 1
  ...
  /**
    * Token Price : Pre = Pub
      ...
      * Public Sale
      * mintStep:    2
      * mintCount:   Up to Unlimited
     */
    function onMint(bytes32[] calldata merkleProof, uint numberOfTokens) public payable {
      if(mintStep == 1) {
          if(block.timestamp > presaleEndTime) {
              mintStep = 2;
              presaleEndTime = 0;
          }
          if( presaleEndTime > block.timestamp && block.timestamp > presaleStartTime && totalSupply() <= MAX_PRESALE) {
              require(
                      MerkleProof.verify(
                          merkleProof,
                          whitelistMerkleRoot,
              ...
              require(numberOfTokens + totalSupply() <= MAX_PRESALE, "Have to mint less than that.");
      ...
      {
          require(mintStep == 2, "Not Public Mint Step, Correct you mintStep");
          require(msg.sender != address(0));
    ...
    function setMintStep(uint8 _mintStep) external onlyOwner {
        require(_mintStep >= 0 && _mintStep <= 3);
        mintStep = _mintStep;
        if(mintStep == 1) {
          require(presaleEndTime > 0, "You have to set Presale End Time");
          require(presaleEndTime > block.timestamp, "Your presale endtime must be later than now.");
          presaleStartTime = block.timestamp;
        }
    }
    
    function setPremintStop() external onlyOwner {
        presaleEndTime = 0;
        mintStep = 3;
    }
    ...
    function setPresaleStart() external onlyOwner {
      require(presaleEndTime > 0, "You must set Presale endtime.");
      require(presaleEndTime > block.timestamp, "Your presale endtime must be later than now.");
        presaleStartTime = block.timestamp;
        mintStep = 1;
    }
    ...
    function setPreStopAndPubStart() external onlyOwner {
      if(mintStep == 1) {
        mintStep = 2;
      } else {
        mintStep = 2;
    ...
    function setPresaleEndTime(uint256 _unixTimestamp) external onlyOwner {
      // require(mintStep != 1, "Can't set presale endtime during presale");
      require(_unixTimestamp > block.timestamp, "Your UnixTimestamp must be later than now.");
      presaleEndTime = _unixTimestamp;
    }
    ...
    // Get mintStep
    function getMintStep() public view returns (uint256) {
        return mintStep;
    }
    
    function getSetting() public view returns (uint256[] memory) {
        uint256[] memory setting = new uint256[](3);
        setting[0] = mintStep;
        setting[1] = tokenPrice;
        setting[2] = MAX_TOKENS;
        setting[3] = maxTokenPurchase;
        return setting;
    }
    ...
    function getPresaleDurationSec() public view returns (uint256) {
      if(mintStep == 1 && block.timestamp < presaleEndTime && block.timestamp >= presaleStartTime) {
