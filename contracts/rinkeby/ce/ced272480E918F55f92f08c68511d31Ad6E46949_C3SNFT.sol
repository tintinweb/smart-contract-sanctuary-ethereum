...
contract C3SNFT is ERC721Enumerable, Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    // constants
    uint256 constant MAX_ELEMENTS = 5439;
    uint256[] public PURCHASE_LIMIT = [2, 4];
    uint256[] public STAGE_PRICE = [0.2 ether, 0.25 ether];
    ...
    constructor() ERC721("C3SNFT", "C3SNFT"){
    }
    ...
    function privateSale(bytes32[] calldata merkleProof, uint256 numberOfTokens) external payable isValidMerkleProof(merkleProof, whiteListMerkleRoot) nonReentrant {
    ...
    function publicSale(uint256 numberOfTokens) external payable nonReentrant {
        require(!MINTING_PAUSED, "Minting is not active");
