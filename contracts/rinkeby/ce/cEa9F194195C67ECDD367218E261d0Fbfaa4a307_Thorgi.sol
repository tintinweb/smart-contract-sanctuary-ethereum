...
contract Thorgi is ERC20Burnable, Ownable {
    ...
    function stakePups(uint256[] memory tokenIds) public stakingEnabled {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 id = tokenIds[i];
            require(pupIERC721Enumerable.ownerOf(id) == msg.sender && pupOwner[id] == address(0), "TOKEN_IS_NOT_YOURS");
    ...
    function checkRewardsbyCorgiIds(uint256 tokenId) public view returns (uint256) {
    ...
    function checkRewardsPupsIds(uint256 tokenId) public view returns (uint256) {
    ...
    function checkAllRewardsFromCorgis() public view returns (uint256) {
        uint256 totalRewards = 0;
        uint256[] memory stakedBone = getStakedBone(msg.sender);
        uint256[] memory corgis = corgiTokenIds[msg.sender];
    ...
    function checkAllRewardsFromPups() public view returns (uint256) {
        uint256 totalRewards = 0;
        uint256[] memory stakedBone = getStakedBone(msg.sender);
        uint256[] memory pups = pupTokenIds[msg.sender];
    ...
    function checkAllRewards() public view returns (uint256) {
        uint256 totalRewards = 0;
        uint256[] memory stakedBone = getStakedBone(msg.sender);
    ...
    function airDrop(uint256 amount, uint64 expireTime, bytes memory sig) external {
