...
contract IconGirlNFT is ERC721Enumerable, Ownable, Pausable {
    ...
    uint256 public fashion_item_price = 8 * 10**16; // 0.08 ETH
    uint256 public iconGirl_price = 8 * 10**16; // 0.08 ETH
    ...
    constructor () ERC721 ("IconGirl", "$IGT") {
        setBaseURI("https://testnet-api.icon.fashion/api/getTokenURI/");
        pause(true);
    }
    ...
    function mintFashionItem(uint256 amount) public payable saleIsOpen {
        ...
        if(whitelisted[msg.sender] == false) {
            require(msg.value >= multiprice(amount), "Value below price");
        }
    ...
    function checkBurnValidity(address account, uint256 [] memory fashion_tokenIds) internal view returns (bool) {
    ...
    function mintIconGirl(uint256 [] memory fashion_tokenIds) public payable saleIsOpen {
        require(checkBurnValidity(msg.sender, fashion_tokenIds), "Mint: invalid tokenIds");
        for (uint256 i = 0; i < fashion_tokenIds.length; i++) {
            uint256 tokenId = fashion_tokenIds[i];
            // require(msg.sender==owner(), "Only owner can burn");
            _burn(tokenId);
        }
        firstTokenId[msg.sender] = fashion_tokenIds[0];
        if(whitelisted[msg.sender] == false) {
            require(msg.value >= iconGirl_price, "Value below price");
        }
        burnedFashionItems[msg.sender][fashion_tokenIds[0]] = fashion_tokenIds;
        _safeMint(msg.sender, firstTokenId[msg.sender]);
    ...
    function undressIconGirl(uint256 collectionId) public saleIsOpen {
        ...
        tokenIds = burnedFashionItems[msg.sender][collectionId];
        for (uint i = 0; i < tokenIds.length; i++) {
            _safeMint(msg.sender, tokenIds[i]);
        }
    }

    function burnIconGirl(uint256 tokenId) public payable saleIsOpen {
        // require(msg.sender==owner(), "Only owner can burn");
        _burn(tokenId);
    }
    ...
    function multiprice(uint256 _count) public view returns (uint256) {
        return fashion_item_price.mul(_count);
    }

    // Set a different price for fashion item in case ETH changes drastically
    function setfashionItemPrice(uint256 newPrice) public onlyOwner {
        fashion_item_price = newPrice;
    }
