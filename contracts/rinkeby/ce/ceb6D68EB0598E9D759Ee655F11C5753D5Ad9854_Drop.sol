...
contract Drop is ERC721, ERC721Enumerable, ReentrancyGuard {
    ...
    function removeUserFromWhitelist(address _addressToRemove) public {
        require(msg.sender == deployer);
    ...
    function randomMetadata(uint tokenId) private {
        ...
        for (uint i = 0; i < metadata.traitsChance.length; i++) {
            if (uint(keccak256(
                    abi.encodePacked(block.difficulty, block.timestamp, randomWord, tokenId, uint(blockhash(block.number-1))
    ...
    function randomzieMontlyTraits() public {
        require(msg.sender == deployer);
        randomWord = uint(keccak256(
    ...
    function mint(uint16[] calldata traits) public payable nonReentrant isWhitelisted(msg.sender) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory) {
        ...
        encodedMetadata = string(abi.encodePacked(
            ...
            "Founders Art",
            '", "value":"',
            HelperFunctions.toString(foundersArts[tokenId-1]),
            '"}')
        );
        ...
        string memory encoded = string(
            ...
            Base64.encode(
                ...
                abi.encodePacked(
                    ...
                    '", "image": "',
                    "https://mint-stage.lagoframe.com/my-nft?tokenId=",
                    HelperFunctions.toString(tokenId),
                    '", "attributes": [',
                    encodedMetadata,
                    '] }'
