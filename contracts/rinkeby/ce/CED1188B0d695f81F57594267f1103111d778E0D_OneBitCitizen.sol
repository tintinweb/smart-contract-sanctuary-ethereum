...
contract OneBitCitizen is ERC721, Pausable, AccessControl {
    using Random for Random.Manifest;

    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    string baseURI;
    uint public price = 1 ether / 40; // .025 Ether
    Random.Manifest private deck;    
    address beneficiary;        
    
    modifier withPayment(uint256 _price) {
        require(msg.value >= _price, 'Insufficient payment');
        _;
        Address.sendValue(payable(beneficiary), msg.value);
    }        

    constructor() ERC721("One-Bit-Citizens", "CTZNS") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(PAUSER_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        deck.setup(50); // set count
        beneficiary = _msgSender();   
        baseURI = "ipfs://QmTFXJ9p1TXkwFZMg3VAhyqbjjPBGTuYzs4xxbk9K2xtJv/";
    }

    function freeMint(uint256 numberOfTokens) public { // Temporary
        _mintBatch(msg.sender,numberOfTokens);
    }

    function mint(address to, uint256 numberOfTokens) public onlyRole(MINTER_ROLE) {
        _mintBatch(to,numberOfTokens);
    }


    // function safeMint(address to) public onlyRole(MINTER_ROLE) {
    //     _safeMint(to,  deck.draw());
    // }

    function mintGift(uint256 numberOfTokens, address recipient) external onlyRole(MINTER_ROLE) {
