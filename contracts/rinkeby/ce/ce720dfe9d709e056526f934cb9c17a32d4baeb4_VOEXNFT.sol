...
contract VOEXNFT is ERC721URIStorage, Ownable {
    ...
    //event 
    event LogSetRoyaltyPrice(uint256 price);
    event LogMint(address indexed minter, uint256 id);
    event LogBurn(address indexed burner, uint256 id);



    constructor(string memory _baseTokenURI) ERC721("VOEX NFT", "VOEX") {
     ...
     */
    function mint(uint256 _price) external {
        tokenIDs++;
        uint256 id = tokenIDs;
        prices[id] = _price;
        super._safeMint(msg.sender, id);
        emit LogMint(msg.sender, id);
    }
