...
contract NounsToken is ERC721A, INounsToken, Ownable, ReentrancyGuard {
 ...
 * @notice Require that the descriptor has not been locked.
    ...
    function setWhitelistSaleStatus() external onlyOwner {
        isWhitelistSaleStarted = !isWhitelistSaleStarted;
    }
    ...
    /**
     * @notice Mint a Noun to the messenger sender
     */
     ...
     */
    function ownerMint(address to, uint256 count) public onlyOwner override{
    ...
    function mintWhiteList(
        ...
        require(_verify(merkleProof, sender, maxCount), "Invalid proof");
        require(count * MINT_PRICE == msg.value, "Incorrect payable amount");
    ...
    function _internalMint(address to, uint256 count) private {
        ...
        _mint(to, count);
        emit NounCreated(_msgSenderERC721A(), startTokenId, startTokenId + count - 1);
    }
    ...
    function _verify(
        ...
        bytes32 leaf = keccak256(abi.encodePacked(sender, Strings.toString(maxCount)));
     ...
     */
    function setOverrideNft(uint256 tokenId, address override_contract, uint64 override_token_id) external override {
        require(_exists(tokenId), 'Nonexistent token can not be modified');
        require(_msgSenderERC721A() == ownerOf(tokenId), 'Only owner can update the card');
        require(isRevealed, 'token not revealed yet');

        // todo: verify the override_contract is one of the supported list
        uint256 curSeedId = seedId(tokenId);
        seeds[curSeedId].override_contract = override_contract;
        seeds[curSeedId].override_token_id = override_token_id;

        emit NftOverriden(tokenId, override_contract, override_token_id);
    }
}
...
interface INounsToken {
    event NounCreated(address indexed creator, uint256 indexed startTokenId, uint256 indexed endTokenId);
    
    event NftOverriden(uint256 indexed tokenId, address indexed override_contract, uint256 indexed override_token_id);
    ...
    function setOverrideNft(uint256 tokenId, address override_contract, uint64 override_token_id) external;
}
...
interface INounsDescriptor {
    ...
    function tokenURI(uint256 tokenId, INounsToken.Seed memory seed) external view returns (string memory);

    function dataURI(uint256 tokenId, INounsToken.Seed memory seed) external view returns (string memory);

    function genericDataURI(
        string calldata name,
        string calldata description,
        INounsToken.Seed memory seed
    ) external view returns (string memory);

    function generateSVGImage(INounsToken.Seed memory seed) external view returns (string memory);
}
