...
contract HoneypotCheck is OKLGWithdrawable {
  ...
  function buyThenSellNoSlippage(address token) external payable {
    ...
    router.swapExactETHForTokensSupportingFeeOnTransferTokens{
      ...
      0, // TODO: use this to determine buy taxes
      buyPath,
      address(this),
      block.timestamp
    );
    ...
    router.swapExactTokensForETHSupportingFeeOnTransferTokens(
      currentBalance,
      0, // TODO: use this to determine sell taxes
      sellPath,
      address(this),
      block.timestamp
    );
    ...
    payable
  {
    ...
    address pair = IUniswapV2Factory(factory).getPair(wrappedNative, token);
    ...
    uint256 token0Decimals = IERC20Decimals(token0).decimals();
    uint256 token1Decimals = IERC20Decimals(token1).decimals();

    // TODO: reserves ratio and calculate slippage as output in buy and sell
  }
