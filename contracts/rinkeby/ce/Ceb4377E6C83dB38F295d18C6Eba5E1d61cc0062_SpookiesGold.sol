...
contract SpookiesGold is Context, Ownable, AccessControlEnumerable, ERC1155Burnable, ERC1155Pausable {
    ...
    function mintGold(uint256 amount, uint256 id, uint256 quantity, bytes memory data) external payable {
    ...
    function burnToMint(uint256 quantity, bytes memory data) external {
    ...
    function merge(bytes memory data) external {
        require(
    ...
    function merge2(bytes memory data) external {
        require(
            balanceOf(msg.sender, 1) >= 0 
            && balanceOf(msg.sender, 2) >= 0 
            && balanceOf(msg.sender, 3) >= 0 
