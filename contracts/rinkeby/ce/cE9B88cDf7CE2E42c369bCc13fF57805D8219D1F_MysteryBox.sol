...
contract MysteryBox is Context, Ownable, ReentrancyGuard  {
    ...
    constructor (address _loomi) {
        points.push([int256(-1),int256(-1),int256(-1),int256(0),int256(0),int256(1),int256(2),int256(3),int256(4),int256(5)]);
        loomi = ILoomi(_loomi);
    ...
    function random(uint256 num) internal view returns(uint){
    ...
    function spinMysteryBox(uint256 numberOfSpins) public nonReentrant whenNotPaused {
      ...
      require(points.length > currentWeek, "Prises for this week are not set");

      loomi.spendLoomi(_msgSender(), spinPrice * numberOfSpins);

      for (uint256 i; i < numberOfSpins; i++) {
          uint256 rand = random(i) % ITEMS_PER_WEEK;
