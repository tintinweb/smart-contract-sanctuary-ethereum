...
contract FrogeeToken is Context, IERC20, Ownable {
    ...
    address private _FrogeeMarketingWallet = 0x424Ea8c2436613d81485cDaea0a28Dc79eb98268;
    address private _donationWalletAddress = 0x424Ea8c2436613d81485cDaea0a28Dc79eb98268;
    address private _FrogeeArGameWalletAddress = 0x424Ea8c2436613d81485cDaea0a28Dc79eb98268;
    address private _serviceWalletAddress = 0x424Ea8c2436613d81485cDaea0a28Dc79eb98268;

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1 * 10**11 * 10**10;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "FROGEE";
    string private _symbol = "FRGE";
    uint8 private _decimals = 10;

    uint256 public _taxFee = 2;
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _fundOrBurnFee = 0;
    ...
    constructor () {
        ...
        _isExcludedFromFee[_FrogeeMarketingWallet] = true;
        _isExcludedFromFee[_donationWalletAddress] = true;
        _isExcludedFromFee[_FrogeeArGameWalletAddress] = true;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tMarketing, uint256 tLiquidity, uint256 tFundOrBurn) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, fees.tFee, fees.tMarketing, fees.tLiquidity, fees.tFundOrBurn, _getRate());
        return (rAmount, rTransferAmount, rFee, fees.tTransferAmount, fees.tFee, fees.tMarketing, fees.tLiquidity, fees.tFundOrBurn);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (Fees memory) {
        ...
        fees.tMarketing = calculateDevFee(tAmount);
        fees.tLiquidity = calculateLiquidityFee(tAmount);
        fees.tFundOrBurn = calculateFundOrBurnFee(tAmount);
        fees.tTransferAmount = tAmount.sub(fees.tFee).sub(fees.tMarketing).sub(fees.tLiquidity).sub(fees.tFundOrBurn);
        return (fees);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tMarketing, uint256 tLiquidity, uint256 tFundOrBurn, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rFundOrBurn = tFundOrBurn.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rMarketing).sub(rLiquidity).sub(rFundOrBurn);
    ...
    function _tMarketing(uint256 tMarketing) private {
        uint256 currentRate =  _getRate();
        uint256 rMarketing = tMarketing.mul(currentRate);
        _rOwned[_FrogeeMarketingWallet] = _rOwned[_FrogeeMarketingWallet].add(rMarketing);
        if(_isExcluded[_FrogeeMarketingWallet])
            _tOwned[_FrogeeMarketingWallet] = _tOwned[_FrogeeMarketingWallet].add(tMarketing);
    }
