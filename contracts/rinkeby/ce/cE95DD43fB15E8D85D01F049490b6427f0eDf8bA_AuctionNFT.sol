    ...
    function finalizeAuction(
        ...
        if (isConfirm) {
            ...
            for(uint256 i = 0; i< order.tokenDet[orderNumber].creators.length ; i++){
                ...
                uint256 calAmount = (order.tokenDet[orderNumber].sameCreator[creatorAdd] * (_royality)) /(order.tokenDet[orderNumber].creators.length);
                payable(IERC721(order.tokenDet[orderNumber].nftAddress).creator(order.tokenDet[orderNumber].sameCreator[creatorAdd])).transfer(calAmount);
            }
                }
                ...
                for(uint256 i = 0; i< order.tokenDet[orderNumber].creators.length ; i++){
                address creatorAdd = order.tokenDet[orderNumber].creators[i];
                address nftAdd = order.tokenDet[orderNumber].nftAddress;
    ...
    function addNftBundle(
        ...
        orderList.push(orderNumber);
          for(uint256 i =0; i<  pendingOrders[orderNumber].bundleIds.length; i++){
                ...
                pendingOrders[orderNumber].tokenDet[orderNumber].sameCreator[IERC721(nftAddress).creator(pendingOrders[orderNumber].bundleIds[i])] = 1;
                pendingOrders[orderNumber].tokenDet[orderNumber].creators.push(IERC721(nftAddress).creator(pendingOrders[orderNumber].bundleIds[i]));
                }
            }
