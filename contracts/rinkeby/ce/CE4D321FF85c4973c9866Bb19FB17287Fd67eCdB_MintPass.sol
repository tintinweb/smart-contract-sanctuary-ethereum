...
contract MintPass is ERC1155Supply, Ownable {
  ...
  //Track addresses that have donated to the contract
  mapping(address => uint256) public donatedIndexMap;
  ...
  struct DonationStruct {
    ...
    uint256 donatorFunds;
  }

  DonationStruct[] donationsArray;
  ...
  function donateFunds() external payable {
    ...
    if (donatedIndexMap[msg.sender] == 0) {
      DonationStruct memory donation = DonationStruct(msg.sender, msg.value);
      donationsArray.push(donation);
      donatedIndexMap[msg.sender] = donationsArray.length - 1;
    } else {
      uint256 arrayIndex = donatedIndexMap[msg.sender];
      donationsArray[arrayIndex].donatorFunds += msg.value;
    }
    emit Donation(msg.sender, msg.value);
  }

  function getDonationsArray() external view returns (DonationStruct[] memory) {
