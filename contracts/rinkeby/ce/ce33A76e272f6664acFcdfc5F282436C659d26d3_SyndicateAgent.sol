...
contract SyndicateAgent is ERC1155, Ownable {
    ...
    constructor(string memory _uri, uint256 _totalSupply) ERC1155(_uri) {
        totalSupply = _totalSupply;
    }

    function _airdrop(address _user) internal {
        uint256 _tokenId = id.current();
        id.increment();
        if (_tokenId < totalSupply) _mint(_user, _tokenId, 1, "");
    }

    function airdrop(address _user) external onlyOwner {
        require(id.current() < totalSupply);
        _airdrop(_user);
    }

    function airdropBatch(address[] memory _users) external onlyOwner {
        require(_users.length > 0, "Cannot be empty list");
        require(
            (id.current() + _users.length) <= totalSupply,
            "not enough tokens"
        );
