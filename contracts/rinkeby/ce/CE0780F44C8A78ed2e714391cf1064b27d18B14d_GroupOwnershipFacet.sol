...
contract GroupOwnershipFacet is IOwnership {
    ...
    function memberAt(uint256 index) external view override returns (address) {
...
library LibOwnership {
    ...
    function _addOwner(address account) internal {
        ...
        // Can only add a non existing member
        require(
            !ds.ownershipUnits.contains(account),
    ...
    function _acquireMoreOwnershipUnits(address account) internal {
        ...
        ds.ownershipUnits.set(account, currentOwnerUnits + units);
        ds.totalOwnedOwnershipUnits += units;
    }
    ...
    function _depositGuard() internal {
        uint256 amount = msg.value;
        uint256 smallestUnit = _smallestUnit();

        require(amount > 0, "Ownership: deposit must be bigger than 0");

        require(
            amount % smallestUnit == 0,
            "Ownership: deposit not divisible by smallest unit"
        );

        require(
            amount + _totalOwnedOwnershipUnits() <= _totalOwnershipUnits(),
            "Ownership: deposit exceeds total ownership units"
        );
    }
    ...
    function _memberAt(uint256 index) internal view returns (address member) {
        StorageOwnershipUnits.DiamondStorage storage ds = StorageOwnershipUnits
            .diamondStorage();

        (member, ) = ds.ownershipUnits.at(index);
    }
