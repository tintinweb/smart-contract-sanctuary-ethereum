...
contract FunkyPass is ERC721Enumerable, Ownable, ERC2981, ReentrancyGuard {
    using Strings for uint256;

    //NOTE Token values incremented for gas efficiency

    // Max supply in Collection
    uint256 private maxSale = 501;

    // Supply reserved for team
    uint256 public reservedSupply = 100;

    // Whitelisting Merkle Root
    bytes32 public merkleRoot;

    // Price for public sale
    uint256 public tokenPricePublic = 0.05 ether;

    // Mints executed per wallet address
    mapping(address => uint) private _mintsPerAddress;
    ...
    constructor() ERC721("Funky Pass", "FP") {

        //address that will recieve mint funds
        _setRoyalties(msg.sender, 500); //5% Royalties
    }
    ...
    correctValue(tokenPricePublic)
    {
        require(totalSupply() < maxSale - reservedSupply, "Exceeds available tokens");
        require(_mintsPerAddress[msg.sender] == 0, "Exceeds allowance!");
     ...
     */
    function mintPresale(bytes32[] calldata proof, bytes32 leaf)
    external
    noBots
     ...
     */
    function mintReserved(address[] calldata recipients) external onlyOwner {
        require(reservedSupply - recipients.length + 1 > 0, "Exceeds available reserved tokens");
     ...
     * @param newMaxSale The maximum number of tokens available for sale.
     */
    function setMaxSale(uint256 newMaxSale) external onlyOwner {
        require(newMaxSale < maxSale, "Can only reduce supply");
        maxSale = newMaxSale;
    }

    /**
     * Update price in public sale.
     * @param newPrice updated price of public sale minting
     */
    ...
    /**
     * Gets the amount of NFTs an address has minted
     * @param minter address of minter
     */
    function mintsPerAddress(address minter) external view returns(uint) {
