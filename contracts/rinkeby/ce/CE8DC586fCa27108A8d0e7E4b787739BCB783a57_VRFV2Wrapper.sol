...
contract VRFV2Wrapper is ConfirmedOwner, TypeAndVersionInterface, VRFConsumerBaseV2, VRFV2WrapperInterface {
  ...
  // TODO: revert if config not set
  int256 public s_fallbackWeiPerUnitLink;
  // s_stalenessSeconds is how long before we consider the feed price to be stale
  // and fallback to fallbackWeiPerUnitLink.
  // s_stalenessSeconds should match the value in VRFCoordinatorV2
  uint32 public s_stalenessSeconds;
  uint32 public s_fulfillmentFlatFeeLinkPPM;
  // s_gasAfterPaymentCalculation should
  uint32 public s_gasAfterPaymentCalculation;
  ...
  // s_keyHash is the keyhash to use when requesting randomness. Fees are paid based on current gas
  ...
  function calculatePrice(uint32 callbackGasLimit) public view override onlyConfigured returns (uint256) {
    ...
    uint256 feeNoWrapperPremium = (1e18 * tx.gasprice * (callbackGasLimit + s_wrapperGasOverhead)) /
