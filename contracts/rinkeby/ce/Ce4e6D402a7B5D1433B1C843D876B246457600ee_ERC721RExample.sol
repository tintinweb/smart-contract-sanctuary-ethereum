...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function removeNFT(address _addr, uint256 _id) internal {
        for(uint256 i = 0; i < _ownerToTokenIds[_addr].length; i++) {
            if(_ownerToTokenIds[_addr][i] == _id) {
                _ownerToTokenIds[_addr][i] = _ownerToTokenIds[_addr][_ownerToTokenIds[_addr].length - 1];
