    ...
    FirstDibsERC2771Context
{
    ...
    struct Auction {
        ...
        address payable payee; // address of auction proceeds recipient
    ...
    function setManifoldRoyaltyEngineAddress(address _manifoldRoyaltyEngineAddress) external {
        onlyAdmin();
        require(
            _manifoldRoyaltyEngineAddress.isContract() &&
                IRoyaltyEngineV1(_manifoldRoyaltyEngineAddress).supportsInterface(
                    type(IRoyaltyEngineV1).interfaceId
                ),
            '_manifoldRoyaltyEngineAddress must support the getRoyalty interface'
        );
        manifoldRoyaltyEngineAddress = _manifoldRoyaltyEngineAddress;
    }
    ...
    function createAuction(
        ...
        uint16 _minimumBidIncrementArg,
     ...
     * @param _auctionDurationArg (optional) auction duration in seconds
     * @param _fundsRecipient (optional) address to send auction proceeds to
    ...
    function adminCreateAuction(
        ...
        // change to paymentAddress on token
        ...
        Auction memory auction = Auction({
            ...
            payee: payable(_fundsRecipient), // pass in the payee
    ...
    function endAuction(uint256 _auctionId) external nonReentrant {
        ...
        uint256 commissionFee = (auctions[_auctionId].currentBid.amount *
            auctions[_auctionId].settings.commissionRate) / 10000;
        // don't attempt to transfer fees if there are none
        if (commissionFee + auctions[_auctionId].currentBid.buyerPremiumAmount > 0) {
            _tryTransferThenEscrow(
                iFirstDibsMarketSettings.commissionAddress(),
                commissionFee + auctions[_auctionId].currentBid.buyerPremiumAmount
            );
        }
        ...
        (address payable[] memory recipients, uint256[] memory amounts) = IRoyaltyEngineV1(
            manifoldRoyaltyEngineAddress
                ...
                auctions[_auctionId].nftAddress,
                auctions[_auctionId].tokenId,
                auctions[_auctionId].currentBid.amount
            );
        uint256 arrLength = recipients.length;
        for (uint256 i = 0; i < arrLength; ) {
            royaltyAmount += amounts[i];
            _sendFunds(recipients[i], amounts[i]);
            unchecked {
                ++i;
            }
        }
        uint256 sellerFee = auctions[_auctionId].currentBid.amount - royaltyAmount - commissionFee;
        _sendFunds(auctions[_auctionId].payee, sellerFee);

        // send the NFT to the winning bidder
        IERC721(auctions[_auctionId].nftAddress).safeTransferFrom(
            address(this), // from
            auctions[_auctionId].currentBidder, // to
            auctions[_auctionId].tokenId
        );
        emit AuctionEnded(
            _auctionId,
    ...
    function cancelAuction(uint256 _auctionId) external nonReentrant {
        ...
        if (!hasRole(DEFAULT_ADMIN_ROLE, _msgSender())) {
            ...
            require(
                ...
                'auction with bids may not be canceled'
            );
        }
