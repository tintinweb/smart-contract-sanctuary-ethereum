...
contract VehicleStore is AccessControl {
	...
	address[] public serviceWorkers;
  	...
  	event AddVehicle(uint index, address owner, string VIN, string name, string make, string model, string color);
    ...
    function setAdmin(address admin) external onlyAdmin{
  	...
  	function addVehicle (
		...
		uint index = vehicles.length - 1;

		vehicleToVIN[index] = VIN;
		VINToVehicle[VIN] = index;

		emit AddVehicle(index, _msgSender(), VIN, name, make, model, color);
	}
	...
	function getVehicle(string memory VIN) public view returns (address, string memory, string memory, string memory, string memory) {
	    ...
	    return (vehicles[index].owner, vehicles[index].name, vehicles[index].make, vehicles[index].model, vehicles[index].color);
  	}
}
