...
// File: new-contract/ERC721Token.sol
...
contract ERC721Token is Initializable, ERC721Upgradeable, EIP712Upgradeable, ERC721VotesUpgradeable {
    /// @custom:oz-upgrades-unsafe-allow constructor
    ...
    function initializerole(address DAOaddress) public isGovernor{
        console.log("TokenContract Initialised Role");
        Governor = DAOaddress;
    }
...
// File: new-contract/ERC20Transferable.sol
...
contract ERC20Transferable is Initializable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable {
        ...
        function initialize(string calldata name, string calldata symbol) initializer public {
               console.log("TokenCOntract Initiated");
...
// File: new-contract/ERC20NonTransferable.sol
...
/// @title StationX Governance Token Contract
/// @dev Base Contract as a reference for DAO Governance Token contract proxies
contract ERC20NonTransferable is Initializable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable {
        ...
        function tokenDetails() external view returns( string memory ,  string memory, uint){	
...
// File: new-contract/GovernorProposalOffChain.sol
...
contract GovernorProposalOffChain is  Initializable, AccessControl{
    ...
    struct Proposals {
        ...
        address[] executiveRoles;		// addresses to be assign executive role under 2nd Execution function
    }

    mapping(uint => mapping(uint => bool)) public executionId;

    /// @dev balances mapping of each DAO members
    ...
    /// @dev members stoing mapping + Array
    mapping (address => uint) memberStatus;

    /* 
    Events Deposited,Start Deposit, Close Deposit, 
    Update Min and Max Deposit,Update Owner fee 
    */
    event Deposited   (address indexed, address indexed, uint256 amount);
    ...
    /// @param _totalDeposit Max Total deposit allowed in DAO
    /// @param _minDeposit Minimum deposit for DAO members
    /// @param _maxDeposit Maximum deposit for DAO members
    /// @param _ownerFee Fee asign to DAO owner on member's deposits
    /// @param _days referring to closing of deposit period 
    /// @param _ownerAddress Address of DAO owner
    /// @param _feeUSDC bool value to decide fee mode of DAO owner: 1 for USDC, 0 for Governance Tokens 
    /// @param _tresuryAddress address of launched Gnosis treasury contract
    /// @param _quorum referring to closing of deposit period 
    function initialize( 
                address _governanceToken, uint256 _totalDeposit,
                uint256 _minDeposit, uint256 _maxDeposit, 
                uint256 _ownerFee, uint256 _days,
                bool _feeUSDC,address _tresuryAddress,
                uint256 _quorum,uint256 _threshold,address _ownerAddress, address _USDC
                ) initializer public {
      
         console.log("MasterCOntract Initiated");
    ...
    function updateminmaxdeposit (
       ...
       if(_minDeposit > 0){ minDeposit =_minDeposit; }
       if(_maxDeposit > 0){ maxDeposit =_maxDeposit;}
    ...
    function startDeposit (uint256 _days,uint256 _totalDeposit,
                           uint256 _minDeposit, uint256 _maxDeposit ) external onlyOwner  {
       require(closeDate < block.timestamp , "Deposit ");
       closeDate = block.timestamp.add(_days.mul(86400));
       totalDeposit    = _totalDeposit;     
       minDeposit      = _minDeposit;
       maxDeposit      = _maxDeposit; 
    ...
    function deposit (
        ...
        require(daoAmount < totalDeposit,"DAO exceeded total deposit limits");
        depositIntilize = true;

        _amount = _amount * 10 ** 18;
        uint amt = _amount.sub((_amount.mul(ownerFee)).div(100)); 

        if (balances[msg.sender] == 0) {
            members.push(msg.sender);
        }
        if (feeUSDC == true){ 
             ERC20(_token).transferFrom(msg.sender, tresuryAddress, amt);
             ERC20(_token).transferFrom(msg.sender, ownerAddress, (_amount.sub(amt)));
             ERC20NonTransferable(governanceToken).mintToken(msg.sender, _amount);
             daoAmount = daoAmount.add(amt.div(1*10**18));
             balances[msg.sender] = balances[msg.sender].add(_amount);
        }else{
             ERC20(_token).transferFrom(msg.sender, tresuryAddress, _amount);
             ERC20NonTransferable(governanceToken).mintToken(msg.sender, amt);
             ERC20NonTransferable(governanceToken).mintToken(ownerAddress, (_amount.sub(amt)));
             daoAmount = daoAmount.add(_amount.div(1*10**18));
             balances[msg.sender] = balances[msg.sender].add(amt);
        }

        emit Deposited(msg.sender, _token, _amount.mul(1*10**18));
    }
    
    // Proposal Flow
    function updateProposalAndExecution(    
        Proposals memory params ) external onlyRole(EXECUTOR_ROLE){

        if(params.executionIds[0] == true){
            airdropToken(params.airDropToken, params.airDropAmount);
        }
        if(params.executionIds[1] == true){
            mintGTToAddress( params.mintGTAmounts,  params.mintGTAddresses );  
        }
        if(params.executionIds[2] == true){
            assignExecutorRole( params.executiveRoles); 
        }
        if(params.executionIds[3] == true){
            updateGovernanceSettings( params.quorum,  params.threshold);
        }
        if(params.executionIds[4] == true){
            startDepositExecute( params.day,  params.totalDeposits,
                          params.minDeposits,  params.maxDeposits);
        }
        ...
        if(params.executionIds[7] == true){
            sendCustomToken(params.customToken, params.customTokenAmounts,  params.customTokenAddresses);
        }
        if(params.executionIds[8] == true){
            sendEth(params.sendEthAmounts,  params.sendEthAddresses);
        }
    }

    function sendEth(uint[] memory _amount, address[] memory _addresses) internal {
         for(uint i = 0; i < _amount.length; i++){
             payable(_addresses[i]).transfer(_amount[i]);   // can check for total balance > needed
        } 
    }

    function sendCustomToken(address _token, uint[] memory _amount, address[] memory _addresses) internal {
         for(uint i = 0; i < _amount.length; i++){
             ERC20(_token).transferFrom(tresuryAddress, _addresses[i], _amount[i]);   // can check for approval first
        } 
    }

    function assignExecutorRole(address[] memory _user) internal onlyRole(EXECUTOR_ROLE) {
        for(uint i = 0; i < _user.length; i++){
            _grantRole(EXECUTOR_ROLE, _user[i]);  
        } 
    }

    function mintGTToAddress(uint[] memory _amount, address[] memory _userAddress) internal onlyRole(EXECUTOR_ROLE) {
        for(uint i = 0; i < _amount.length; i++){
             ERC20NonTransferable(governanceToken).mintToken(_userAddress[i], _amount[i].mul(1*10**18));
             balances[_userAddress[i]] = balances[_userAddress[i]].add(_amount[i].mul(1*10**18));
        } 
    }

    function updateGovernanceSettings(uint _quorum, uint _threshold) internal onlyRole(EXECUTOR_ROLE){
        quorum = _quorum;
        threshold = _threshold;
    }
    
    function airdropToken(address _token, uint _amount) internal onlyRole(EXECUTOR_ROLE){
        uint supply = ERC20NonTransferable(governanceToken).totalSupply();
        for(uint i = 0; i < members.length; i++){ 
            uint share = balances[members[i]]. mul(10 ** 24). div(supply); 
            console.log(share);
            console.log( _amount * share * 10 ** 12);
            ERC20(_token).transferFrom(tresuryAddress, members[i], _amount * share * 10 ** 12);   // can check for approval first 
        }

        }
    
    function updateRaiseAmount(uint _amount) internal onlyRole(EXECUTOR_ROLE){
        totalDeposit = _amount;
    }

    /// @dev Function to close deposit
    function closeDepositExecute () internal onlyRole(EXECUTOR_ROLE)  {
       closeDate = block.timestamp;
       emit CloseDeposit(msg.sender, block.timestamp);  
    }
    ...
    /// @dev Function to start deposit
    /// @param _days New close date
    function startDepositExecute (uint256 _days,uint256 _totalDeposit,
                           uint256 _minDeposit, uint256 _maxDeposit ) internal onlyRole(EXECUTOR_ROLE)  {
    //    require(closeDate < block.timestamp , "Deposit ");
       closeDate = block.timestamp.add(_days.mul(86400));
       totalDeposit    = _totalDeposit;     
       minDeposit      = _minDeposit;
    ...
    function getGovernorDetails() external view returns(uint, uint, uint, uint, uint, uint){	
        return(closeDate, minDeposit, maxDeposit, quorum, totalDeposit, members.length);
    }
...
// File: new-contract/FactoryCloneContract.sol
...
contract FactoryCloneContract {
    ...
    address GovernorProposalOffChainAddress;
    address erc20NonTransferable;
    address erc20Transferable;
    ...
    constructor (address _USDC) {
        erc20NonTransferable = address(new ERC20NonTransferable());
                console.log("ERC20NonTransferable contract address:", erc20NonTransferable);
        erc20Transferable = address(new ERC20Transferable());
                console.log("ERC20Transferable contract address:", erc20Transferable); 
        erc721Address = address(new ERC721Token());
                console.log("ERC721Token contract address:", erc721Address);                
        GovernorProposalOffChainAddress = address(new GovernorProposalOffChain());
                console.log("GovernorProposalOffChain contract address:", GovernorProposalOffChainAddress);       
        owner = msg.sender;
    ...
    function changeDAOImplementation(address _newDAO) external isOrganiser{
        ...
        {
            GovernorProposalOffChainAddress = _newDAO;
    ...
    /// @param _totalDeposit Total Deposit
    /// @param _minDeposit Minimum deposit for DAO members
    /// @param _maxDeposit Maximum deposit for DAO members
    /// @param _ownerFee Fee asign to DAO owner on deposits
    /// @param _days Timestamp referring to closing of deposit period for DAO members
    /// @param _feeUSDC bool value to decide fee mode of DAO owner: 1 for USDC, 0 for Governance Tokens 
    /// @param _tresuryAddress address of launched Gnosis treasury contract
    function createDAO(
                // uint _tokenType,
                ...
                uint256 _totalDeposit,uint256 _minDeposit, uint256 _maxDeposit,
                uint256 _ownerFee, uint256 _days, bool _feeUSDC, 
                address _tresuryAddress, uint256 _quoram,uint256 _threshold
                )external  {
                 
        uint _tokenType = 1;         
        address tokenAddress;            
        if(_tokenType == 1){            
         tokenAddress = Clones.clone(erc20NonTransferable);
        ERC20NonTransferable(tokenAddress).initialize(tokenName, tokenSymbol);
        } else if(_tokenType == 2){
         tokenAddress = Clones.clone(erc20Transferable);
        ERC20Transferable(tokenAddress).initialize(tokenName, tokenSymbol);    
        } else if(_tokenType == 3){
         tokenAddress = Clones.clone(erc721Address);
        ERC721Token(tokenAddress).initialize(tokenName, tokenSymbol);    
        }

        address GovernorAddress = Clones.clone(GovernorProposalOffChainAddress);
        GovernorProposalOffChain(payable(GovernorAddress)).initialize(
                tokenAddress,_totalDeposit,
                _minDeposit,_maxDeposit,
                _ownerFee, _days,
                _feeUSDC,_tresuryAddress,_quoram,_threshold,msg.sender,USDC
                );     
        
        if(_tokenType == 1){            
        ERC20NonTransferable(tokenAddress).initializerole(GovernorAddress);
        } else if(_tokenType == 2){
        ERC20Transferable(tokenAddress).initializerole(GovernorAddress); 
        } else if(_tokenType == 3){
        ERC721Token(tokenAddress).initializerole(GovernorAddress); 
        }
