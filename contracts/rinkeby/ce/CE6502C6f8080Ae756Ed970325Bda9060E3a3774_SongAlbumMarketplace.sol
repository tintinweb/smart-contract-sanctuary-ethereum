...
interface ISongAlbumBundleMarketplace {
...
interface ISongAlbumNFTFactory {
...
interface ISongAlbumPriceFeed {
    function wFTM() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract SongAlbumMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _buyItem(
        ...
        ISongAlbumBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createOffer(
        ...
        ISongAlbumAuction auction = ISongAlbumAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        // require(_isSongAlbumNFT(_nftAddress), "invalid nft address");
    ...
    function _isSongAlbumNFT(address _nftAddress) internal view returns (bool) {
        return
            // addressRegistry.artion() == _nftAddress ||
            ISongAlbumNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            ISongAlbumNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            ISongAlbumNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            ISongAlbumNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        ...
        ISongAlbumPriceFeed priceFeed = ISongAlbumPriceFeed(
            priceFeedAddress
        );
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    ISongAlbumTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
