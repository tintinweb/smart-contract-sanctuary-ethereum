...
contract NENDBridge is Ownable {
    ...
    event CrossChainTransferRequested(uint targetChainId, address requester, uint amount);

    modifier chainSupported(uint chainId) {
    ...
    uint[] public supportedChainIds;

    constructor(NENDCore _token, uint[] memory _supportedChainIds) {
        require(_supportedChainIds.length > 0, "Must have at least one destination chain");
        supportedChainIds = _supportedChainIds;
        require(!_isChainSupported(block.chainid), "The hosted chain can not be one the the supported chains");
        token = _token;
    }

    function setToken(NENDCore _token) external onlyOwner {
        token = _token;
    }

    function enterBridge(uint targetChainId, uint amount) external chainSupported(targetChainId) {
        require(amount > 0, "Invalid amount");

        token.burnFrom(msg.sender, amount);

        emit CrossChainTransferRequested(targetChainId, msg.sender, amount);
    }

    function leaveBridge(uint targetChainId, address requester, uint amount) external {
        require(block.chainid == targetChainId, "Transfer not meant for this chain");

        token.mint(requester, amount);
    }

    function _isChainSupported(uint chainId) internal view returns (bool) {
