...
contract OffchainMath {
    ...
    function first(uint256 param1, uint256 param2) public {
        // strong validation
        require(param1 > 0 && param2 > 0, "wrong param1");
    ...
    function second(uint256 result, bytes memory signature) public {
        ...
        // verify input data
        require(
            isDataValid(request.param1, request.param2, signature),
            "invalid signature"
        );

        requests[msg.sender].result = result;
        requests[msg.sender].status = Status.DONE;
    }
    ...
    function getResult(address addr) public view returns (uint256 result) {
        ...
        require(request.status != Status.IDLE, "not initiated");
        require(request.status != Status.PENDING, "pending second tx");
        require(request.status == Status.DONE, "wtf");
    ...
    function isDataValid(
        ...
        bytes32 msgHash = keccak256(
            abi.encodePacked(msg.sender, param1, param2)
        );
