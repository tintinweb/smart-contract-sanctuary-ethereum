...
contract Boxes is Ownable, VRFConsumerBaseV2 {
    ...
    function claimReward(uint256 contestId, uint256 boxId) external {
        ...
        uint256 userReward;
        // check q1
        if (!contest.q1Paid && gameScores.qComplete > 1 && gameScores.homeQ1LastDigit == rowScore && gameScores.awayQ1LastDigit == colScore) {
            userReward += contest.totalRewards * q1Payout / percentDenominator;
            contest.q1Paid = true;
        }
        // check q2
        if (!contest.q2Paid && gameScores.qComplete > 2 && gameScores.homeQ2LastDigit == rowScore && gameScores.awayQ2LastDigit == colScore) {
            userReward += contest.totalRewards * q2Payout / percentDenominator;
            contest.q2Paid = true;
        }
        // check q3
        if (!contest.q3Paid && gameScores.qComplete > 3 && gameScores.homeQ3LastDigit == rowScore && gameScores.awayQ3LastDigit == colScore) {
            userReward += contest.totalRewards * q3Payout / percentDenominator;
            contest.q3Paid = true;
        }
        // check final
        if (!contest.finalPaid && gameScores.qComplete > 99 && gameScores.homeFLastDigit == rowScore && gameScores.awayFLastDigit == colScore) {
