...
// File: contracts/EquitySplitter.sol
...
contract EquitySplitter is Context {
    ...
    constructor() payable {
        address communityWalletAddress = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;
        uint256 communityWalletEquity = 600;
        _addShareHolder(communityWalletAddress, communityWalletEquity);

        address banditAddress = 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2;
        _addShareHolder(banditAddress, _getEquity(325));

        address winkyAddress = 0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db;
        _addShareHolder(winkyAddress, _getEquity(225));

        address spliffyAddress = 0x856f4eB0CAb52C7470f79905E929409101143DEB;
        _addShareHolder(spliffyAddress, _getEquity(150));

        address zamalekAddress = 0xCEe59eCcEe6309B8c7bAaf0D0e864dE3Ca38e8Dc;
        _addShareHolder(zamalekAddress, _getEquity(150));

        address vcmAddress = 0x17F6AD8Ef982297579C203069C1DbfFE4348c372;
        _addShareHolder(vcmAddress, _getEquity(50));

        address tonyPAddress = 0x5c6B0f7Bf3E7ce046039Bd8FABdfD3f9F5021678;
        _addShareHolder(tonyPAddress, _getEquity(50));

        address aliasOrphanAddress = 0x03C6FcED478cBbC9a4FAB34eF9f40767739D1Ff7;
        _addShareHolder(aliasOrphanAddress, _getEquity(50));

        require(_totalShares == 1000, "Equity split needs to equal 1000");
    }
    ...
    function release(address payable _address) public virtual {
        ...
        uint256 totalReceived = address(this).balance + _totalReleased;
        uint256 payment = (totalReceived * _shares[_address]) /
            _totalShares -
            _released[_address];
...
// File: contracts/HouseOfBandits.sol
...
contract HouseOfBandits is ERC1155Base, EquitySplitter {
    uint8 constant PREMIUM = 0;
    uint8 constant STANDARD = 1;

    uint16 constant MAX_SUPPLY_PREMIUM = 1000;
    uint16 constant MAX_SUPPLY_STANDARD = 3500;
    ...
    constructor(
        ...
        // test values
        ogMintStartTime = block.timestamp + 10;
        ogMintEndTime = block.timestamp + 600000;
        alphalistMintStartTime = block.timestamp + 10;
        alphalistMintEndTime = block.timestamp + 600000;
    }
    ...
    function teamMint(
        ...
        require(
            totalSupply(STANDARD) + _amountStd <= MAX_SUPPLY_STANDARD &&
                totalSupply(PREMIUM) + _amountOg <= MAX_SUPPLY_PREMIUM,
            "Max supply reached"
        );

        _mint(_address, PREMIUM, _amountOg, "");
        _mint(_address, STANDARD, _amountStd, "");
    }

     function testMint(uint8 _amountOg, uint8 _amountStd)
        external
        payable
        whenNotPaused
    {
                require(
            msg.value == (_amountOg + _amountStd) * mintPrice,
            "Invalid amount of funds sent"
        );

        // require merkle proof

        uint8[] memory tokenIds = new uint8[](2);
        if (_amountOg > 0) {
            _mintToken(PREMIUM, _amountOg);
            tokenIds[0] = (PREMIUM);
        }

        if (_amountStd > 0) {
            _mintToken(STANDARD, _amountStd);
            tokenIds[1] = STANDARD;
        }
    }

    function ogMint(uint8 _amountOg, uint8 _amountStd)
        external
        payable
        whenNotPaused
    {
        require(
            block.timestamp >= ogMintStartTime &&
                block.timestamp <= ogMintEndTime,
            "OG mint is closed"
        );
        require(_amountOg <= 1, "Too many OG tokens");
        require(_amountStd <= 1, "Too many STD tokens");
        require(!(_amountOg == 0 && _amountStd == 0), "Invalid token amounts");
        require(
            totalSupply(STANDARD) + _amountStd <= MAX_SUPPLY_STANDARD &&
                totalSupply(PREMIUM) + _amountOg <= MAX_SUPPLY_PREMIUM,
            "Max supply reached"
    ...
    function alphalistMint(uint8 _amountStd) external payable whenNotPaused {
        require(
            block.timestamp >= alphalistMintStartTime &&
                block.timestamp <= alphalistMintEndTime,
            "Alphalist mint is closed"
        );
        require(_amountStd <= 1 && _amountStd > 0, "Invalid amount of tokens");
        require(
            totalSupply(STANDARD) + _amountStd <= MAX_SUPPLY_STANDARD,
            "Max supply reached"
        );
        ...
        require(
            msg.value == _amountStd * mintPrice,
            "Invalid amount of funds sent"
        );
        ...
        uint8[] memory tokenIds = new uint8[](2);
        tokenIds[0] = STANDARD;
        emit Minted(tokenIds, msg.sender, _amountStd);

        mintTxs[msg.sender] += 1;
    }

    function _mintToken(uint8 _tokenId, uint8 _amount) private {
        require(_amount > 0 && _amount <= 1, "Invalid amount");
