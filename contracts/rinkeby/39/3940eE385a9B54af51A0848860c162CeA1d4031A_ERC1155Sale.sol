...
import "../interfaces/IERC1155Base.sol";
...
contract ERC1155Sale is ReentrancyGuard, Ownable {
    ...
    function buy(
        ...
        require(
            ...
            "ERC1155Sale.sell: Buying amount exceeds balance"
        );
        ...
        transferEther(_token, _tokenId, _orderId, _buying);
    ...
    function transferEther(
        ...
        uint256 sellerServiceFeeBps = serviceFeeProxy.getSellServiceFeeBps(saleInfos[address(_token)][_orderId][_tokenId].owner);
        uint256 buyerServiceFeeBps = serviceFeeProxy.getBuyServiceFeeBps(msg.sender);
        ...
        if (_token.supportsInterface(_INTERFACE_ID_HAS_SECONDARY_SALE)) {
            ...
            if(isSecondarySale) {
                ...
                require(royaltySent, "ERC1155Sale.transferEther: Royalty transfer failed");
        ...
        require(serviceFeeSent, "ERC1155Sale.transferEther: ServiceFee transfer failed");
        // ikonicTokenContract.transfer(serviceFeeProxy.getServiceFeeRecipient(), totalServiceFeeAmount);
        (bool ownerReceivingAmountSent, ) = payable(saleInfos[address(_token)][_orderId][_tokenId].owner).call{value: ownerReceivingAmount}("");
        require(ownerReceivingAmountSent, "ERC1155Sale.transferEther: ownerReceivingAmount transfer failed");
...
contract ServiceFeeProxy is AdminRole {
    ...
    function setSellServiceFeeBps(uint256 _sellerFee) external onlyAdmin {
        require(
            _sellerFee != 0,
            "ServiceFeeProxy.setSellServiceFeeBps: SellerFee should be positive"
        );
        
        serviceFeeContract.setSellServiceFeeBps(_sellerFee);
    }
    ...
    function setBuyServiceFeeBps(uint256 _buyerFee) external onlyAdmin {
        require(
            _buyerFee != 0,
            "ServiceFeeProxy.setBuyServiceFeeBps: BuyerFee should be positive"
        );
        
        serviceFeeContract.setBuyServiceFeeBps(_buyerFee);
    }
...
interface IServiceFee {
     ...
     * @param _sellerFee seller fee
     */
    function setSellServiceFeeBps(uint256 _sellerFee) external;

    /**
     * @notice Set buyer service fee
     * @param _buyerFee buyer fee
     */
    function setBuyServiceFeeBps(uint256 _buyerFee) external;
}
...
/// @title IERC1155Base
/// @dev Interface for ERC1155Base
interface IERC1155Base is IERC1155, IERC2981Royalties{
