...
/// @title Coin
contract LepeicoAuthority is MultiRolesAuthority {
    constructor(address _lepeico) MultiRolesAuthority(_lepeico, Authority(address(0))) {
        // Set up roles
        setRoleCapability(0, bytes4(keccak256("mint(address,uint256)")), true);
        setRoleCapability(0, bytes4(keccak256("burn(address,uint256)")), true);

        // Set up accounts
        setUserRole(_lepeico, 0, true);
    }
}
