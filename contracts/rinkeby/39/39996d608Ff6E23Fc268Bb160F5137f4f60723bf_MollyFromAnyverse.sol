...
contract MollyFromAnyverse is ERC721Enumerable, Ownable, Creators, RG, Signable {
    ...
    bool private NFTsReserved = false; //skip for now
    bool private revealed = false; //ширма,занавес,

    Counters.Counter private _tokenCount; //примерка

    uint256 public constant maxSupply = 10000; //maxLooks
    uint256 public constant mintPrice = 0.15 ether; //price
    uint256 public constant tokenPerAddress = 5; //maxSet

    mapping(uint256 => address) public creatorOf; //buyerOf
    mapping(address => uint256) public chargeOf; //privateSet
    ...
    constructor() ERC721("MFAN V2", "MFAN")
    {
    ...
    //skip for now
    function _reserveNFTs() private lock {
        require(NFTsReserved == false, "Reserved NFTs already minted");
        uint256 amount = 50;
    ...
    function buy(uint256 amount, bytes calldata signature) public payable costs(mintPrice * amount) {
    ...
    function _buy(uint256 amount, bytes calldata signature) private lock {
        ...
        uint256 minted = chargeOf[msg.sender];

        require(minted + amount <= tokenPerAddress, "You can't mint more than 5 tokens");
        require(_verify(signer(), _hash(msg.sender), signature), "Invalid signature");

        for (uint i; i < amount; i++) {
            _tokenCount.increment();
            _safeMint(msg.sender, totalToken());
            creatorOf[totalToken()] = msg.sender;
            chargeOf[msg.sender] = minted + 1;
            minted++;
        }
    }
    ...
    function reward() public onlyCreators {
