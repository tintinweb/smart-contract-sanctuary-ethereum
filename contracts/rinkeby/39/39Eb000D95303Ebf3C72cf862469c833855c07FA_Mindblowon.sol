...
contract Mindblowon is
    ...
    Withdrawable
{
    uint256 internal constant TOTAL_MAX_QTY = 6969;
    uint256 internal constant DASALES_MAX_QTY = 2000;
    uint256 internal constant DA_MAX_QTY_PER_MINTER = 1;
    uint256 internal constant PRIVATE_SALES_MAX_QTY_PER_MINTER = 2;
    ...
    function refundAll() external {
        require(isRefundActivated, "Refund Not Activated");
    ...
    function toggleRefundStatus() external onlyOwner {
        isRefundActivated = !isRefundActivated;
    }
...
contract PrivateSalesActivation is Ownable {
        ...
        onlyOwner
    {
        require(
            _endTime >= _startTime,
            "PrivateSalesActivation: End time should be later than start time"
        );
...
contract DASalesActivation is Ownable {
    ...
    modifier isDASalesActive() {
        require(
            isDASalesActivated(),
            "DASalesActivation: Sale is not activated"
        );
        _;
    }
        ...
        onlyOwner
    {
        require(
            _endTime >= _startTime,
            "DASalesActivation: End time should be later than start time"
        );
