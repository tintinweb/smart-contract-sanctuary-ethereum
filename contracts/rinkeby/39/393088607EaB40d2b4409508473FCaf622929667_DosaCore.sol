...
interface IVault {
    function initialize(address treasury, address _vault_token, uint256 rewardsAmt) external returns(bool);
...
contract DosaCore is Ownable {
    ...
    function createVault(address vault_token, address[] calldata topHoldersList, uint256 rewardsAmount) external onlyOwner{
        ...
        require(IVault(vaultCreated).initialize(treasury, vault_token, rewardsAmount), "Vault initialization failed");
