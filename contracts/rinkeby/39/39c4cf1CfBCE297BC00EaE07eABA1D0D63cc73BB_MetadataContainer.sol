...
contract MetadataContainer is IMetadataContainer, BOOMBase {

    bytes32 public constant METADATA_CONTAINER_GROUPID = keccak256(abi.encodePacked('METADATA_CONTAINER_GROUPID'));

    TagStorage public tags;

    modifier URIisMutable(uint256 tokenId) {
	bytes32 tag_key = Util.getTokenURIFrozenKey(tokenId);
	if(tags.getTagType(tag_key) != StatType.None)
	    require(!tags.getBooleanValue(tag_key),"Token URI must not be frozen");
	_;
    }

    function getId() override view public returns(bytes32) {
	return METADATA_CONTAINER_CONTRACT_ID;
    }

    function getName() override view public returns(string memory) {
	return METADATA_CONTAINER_CONTRACT_NAME;
    }
    ...
    **/
    ...
    // Get base URI prefixing all partially defined token URIs
    function baseURI() override public view virtual returns (string memory) {
	return _baseURI();
    }

    // Get default token URI if token-specific URI is missing. The token URI must contain substitution substring "{token-id}"
    function defaultTokenURI() override public view virtual returns (string memory) {
	return _defaultTokenURI();
    }

    function tokenURI(uint256 _tokenId) override public view virtual returns (string memory) {
	bytes32 tag_key = Util.getTokenURIKey(_tokenId);
//	if(b_token_uri.length == 0)
	if(tags.getTagType(tag_key) == StatType.None){
	    bytes32 tag_tokenid_key = Util.getDefaultTokenURIasIDKey();
	    if(tags.getTagType(tag_tokenid_key) != StatType.None)
		if(tags.getBooleanValue(tag_tokenid_key)){
		    bytes32 tag_baseuri_key = Util.getBaseURIKey();
		    if(tags.getTagType(tag_baseuri_key) != StatType.None)
	    ...
	    string memory token_uri = tags.getStringValue(tag_key);
	    bytes memory b_token_uri = bytes(token_uri);
	    if(b_token_uri[0] == '/')
		return string(abi.encodePacked(_baseURI(),token_uri));
	    else
		return token_uri;
	}
    }

    function setBaseURI(string calldata uri) override public virtual onlyMetaManager {
	bytes32 tag_key = Util.getBaseURIKey();
	tags.setTag(METADATA_CONTAINER_GROUPID,tag_key,uri);
    }

    function setDefaultTokenURI(string calldata uri) override public virtual onlyMetaManager {
	bytes32 tag_key = Util.getDefaultTokenURIKey();
	tags.setTag(METADATA_CONTAINER_GROUPID,tag_key,uri);
    }

    function setDefaultTokenURIasID() override public virtual onlyMetaManager {
	bytes32 tag_key = Util.getDefaultTokenURIasIDKey();
	tags.setTag(METADATA_CONTAINER_GROUPID,tag_key,true);
    }

    function resetDefaultTokenURIasID() override public virtual onlyMetaManager {
	bytes32 tag_key = Util.getDefaultTokenURIasIDKey();
	tags.setTag(METADATA_CONTAINER_GROUPID,tag_key,false);
    }

    function setTokenURI(uint256 tokenId, string calldata uri) override public virtual onlyMetaManager URIisMutable(tokenId) {
	bytes32 tag_key = Util.getTokenURIKey(tokenId);
	tags.setTag(METADATA_CONTAINER_GROUPID,tag_key,uri);
    }

    function setTokenURIFrozen(uint256 tokenId) override public virtual onlyMetaManager {
	bytes32 tag_key = Util.getTokenURIFrozenKey(tokenId);
	tags.setTag(METADATA_CONTAINER_GROUPID,tag_key,true);
    }
	...
	if(tags.getTagType(tag_key) == StatType.None)
	    return "";
	return tags.getStringValue(tag_key);
    }

    // Get default token URI if token-specific URI is missing. The token URI must contain substitution substring "{token-id}"
    function _defaultTokenURI() internal view virtual returns (string memory) {
	bytes32 tag_key = Util.getDefaultTokenURIKey();
	if(tags.getTagType(tag_key) == StatType.None)
	    return "";
	string memory default_uri = tags.getStringValue(tag_key);
	if(bytes(default_uri)[0] == '/')
	    return string(abi.encodePacked(_baseURI(),default_uri));
	else
	    return default_uri;
    }
