...
import "../IMatcherProcessor.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract FixedPriceMatcherProcessor is IMatcherProcessor {
    function matchTakerOrderWithMakerOrder(
        OrderTypes.TakerOrder calldata takerOrder,
        OrderTypes.MakerOrder calldata makerOrder
    ) external view override returns(bool) {
        return
        (makerOrder.startedAt <= block.timestamp) &&
        (makerOrder.expiredAt > block.timestamp) &&
        (makerOrder.price == takerOrder.price) &&
        (makerOrder.amount == takerOrder.amount) &&
        (makerOrder.collectionAddress == takerOrder.collectionAddress) &&
