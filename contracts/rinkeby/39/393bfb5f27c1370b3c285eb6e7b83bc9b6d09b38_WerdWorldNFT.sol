...
contract WerdWorldNFT is ERC721, ERC2981Royalties, AccessControl {
    ...
    function mint(uint8 _amount) external payable {
        ...
        uint8 allowedAmount = MAX_PUBLIC_MINT;

        if (isPreSaleActive) {
            // Use the given amount instead
            allowedAmount = _whitelist[msg.sender];
            // Make sure the sender in on the whitelist.
            require(allowedAmount > 0, "Not on the whitelist");
        ...
        // Final checks before mint
        require(_mintCount[msg.sender] + _amount <= allowedAmount, "Max amount exceeded");
    ...
    function giftMint(address to, uint256 id) external onlyOwner {
        require(
          _exists(id) == false,
          "Already Minted"
        );
    ...
    function updateWhitelist(address[] calldata _addresses, uint8 _amount) public onlyRole(ADMIN_ROLE) {
        for (uint256 i = 0; i < _addresses.length; i++) {
            _whitelist[_addresses[i]] = _amount;
        }
    }
