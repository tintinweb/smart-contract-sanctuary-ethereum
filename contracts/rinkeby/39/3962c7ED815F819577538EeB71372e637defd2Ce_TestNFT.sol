...
contract TestNFT is ERC721URIStorage, Ownable {
  ...
  constructor() ERC721("TestNFT", "TNA") {
      ...
      returns (uint256)
  {
    ...
    function mintBatchItems(address[] memory destinations, string[] memory tokenURIs)
      public
      returns (bool)
  {
      require(destinations.length > 0 && destinations.length == tokenURIs.length, "Invalid length");
      //カウンタを１上げる。（カウンタの役割＜１：後のガチャの上限を見るため ２：特定アドレスと今のカウンタの数値を紐づけるため＞）
