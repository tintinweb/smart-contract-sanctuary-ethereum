...
contract JustCubes is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(address _paymentsAddress) ERC721A("JustCubes", "Cube", 5, 8888) {
        paymentsAddress = payable(_paymentsAddress);
    }
    ...
    function mintOGsale(uint256 numberOfMints, bytes32[] calldata _merkleProof) external payable {
        ...
        require(numberOfMintsOnAddress[msg.sender] + numberOfMints <= ADDRESS_MAX_MINTS, "Sender is trying to mint more than allocated tokens");
        ...
        require(numberOfMints * OGprice <= msg.value, "Not enough ether to mint");

        numberOfMintsOnAddress[msg.sender] += numberOfMints;
    ...
    function mintWLsale(uint256 numberOfMints, bytes32[] calldata _merkleProof) external payable {
        ...
        require(numberOfMints * WLprice <= msg.value, "Amount of ether is not enough");
    ...
    function reserveMint(uint256 quantity) external onlyOwner {
        ...
        require(quantity % maxBatchSize == 0, "Must only mint a multiple of the maxBatchSize");
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        (bool success, ) = payable(paymentsAddress).call{value: address(this).balance}("");
