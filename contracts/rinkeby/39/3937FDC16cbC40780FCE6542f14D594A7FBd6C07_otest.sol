...
contract otest is ERC721A, Ownable {
    enum Status {
        Pending,
        EarlyBirdSale, 
        PreSale,
    ...
    string[3][] public baseURIs; // baseURIs for different branches, getting transposed after setting up
    uint8 public maxEvolutionLevel; // Evolution levels
    uint8 public maxBranchNumber; // Branch numbers
    ...
    event maxEvolutionLevelChanged(uint8 maxLevel);
    event maxBranchNumberChanged(uint8 maxNumber);
    event BaseURIChanged(string newBaseURI);
    event BaseURIsSet(string[3] newBaseURIs);
    event BaseURIsChanged(uint8 level, uint8 branch, string newBaseURI);
    event PriceChanged(uint256 PRICE);
    event adminContractSet(address adminContract);
    ...
    modifier onlyAdmin() {
        require(msg.sender == adminContract, "transaction is not sent from an admin contract for ERC721");
        _;
    }
    ...
    mapping(uint256 => tokenEvolutionData) private _tokenEvolutionData;
    ...
    // Set each token status --- evolution and branch

    function setTokenStatus(uint256 tokenId, uint8 level, uint8 branch) external onlyAdmin {
        require(level <= maxEvolutionLevel, "The level is not open yet");
        require(branch <= maxBranchNumber, "The branch is not open yet");
        // address owner = ownerOf(tokenId);
        // require(msg.sender == owner, "Only owner of the token can change its level and branch");
        _tokenEvolutionData[tokenId].level = level;
        _tokenEvolutionData[tokenId].branch = branch;
    }
    ...
    */
        ...
        returns (string memory)
    {
        ...
        string memory base_URI = _baseURIs(_tokenEvolutionData[tokenId].level, _tokenEvolutionData[tokenId].branch);
        return bytes(base_URI).length > 0 ? string(abi.encodePacked(base_URI, Strings.toString(tokenId), ".json")) : "";
    }
    ...
    function _baseURIs(uint8 level, uint8 branch) internal view returns (string memory) {
        return baseURIs[level][branch];
    }

    // set max evolution level and branch numbers

    function setMaxEvolutionLevel(uint8 maxLevel) external onlyOwner {
        maxEvolutionLevel = maxLevel;
        emit maxEvolutionLevelChanged(maxLevel);
    }

    function setMaxBranchNumber(uint8 maxNumber) external onlyOwner {
        maxBranchNumber = maxNumber;
        emit maxBranchNumberChanged(maxNumber);
    }
    ...
    // push 3 baseURIs for the next level

    function setNewBaseURIs(string calldata newBaseURI1, string calldata newBaseURI2, string calldata newBaseURI3) external onlyOwner {
        string[3] memory newBaseURIs = [newBaseURI1, newBaseURI2, newBaseURI3];
        baseURIs.push(newBaseURIs);
        emit BaseURIsSet(newBaseURIs);
    }

    // change the existing baseURIs for the existing levels

    function changeBaseURIs(uint8 level, uint8 branch, string calldata newBaseURI) external onlyOwner {
        require(level <= maxEvolutionLevel, "The level is not yet open");
        require(branch <= maxBranchNumber, "The branch is not yet open");
        baseURIs[level][branch] = newBaseURI;
        emit BaseURIsChanged(level, branch, newBaseURI);
    }
