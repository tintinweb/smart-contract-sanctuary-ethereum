...
/// @title The Quantum Key Unlocks. Speciality drops for the quantum keys are released here
/// @author jcbdev
contract QuantumKeyUnlocks is ERC721S, ERC2981, Auth {
    ...
    string private _baseURI = "https://core-api.quantum.art/v1/unlockMetadata/";
    string private _ipfsURI = "ipfs://";
    address private _minter;

    /// >>>>>>>>>>>>>>>>>>>>>  CONSTRUCTOR  <<<<<<<<<<<<<<<<<<<<<< ///

	/// @notice Deploys QuantumKeyUnlocks
    ...
    constructor(address owner, address authority) ERC721S("QuantumKeyUnlocks", "QKU") Auth(owner, Authority(authority)) {
        _baseURI = "https://core-api.quantum.art/v1/unlockMetadata/";
        _ipfsURI = "ipfs://";
    }
    ...
    */
    ...
    function mint(address to, uint128 dropId, uint256 variant) public returns (uint256) {
        ...
        if (variant > 0) tokenVariant[tokenId] = variant;
        return tokenId;
    }
