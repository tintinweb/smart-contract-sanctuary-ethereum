...
contract GPC2 is ERC721A, Ownable, ReentrancyGuard {
    ...
    IGPC GPC_CHEMISTRY_CONTRACT = IGPC(0x88B48F654c30e99bc2e4A1559b4Dcf1aD93FA656); //OS OpenStore Smart Contract Address
    ...
    function mutantMint(
        ...
        require(GPC_CHEMISTRY_CONTRACT.balanceOf(msg.sender, _osTokenId) > 1,
            "Incorrect serum ID passed in"
        );
        require(quantity <= GPC_CHEMISTRY_CONTRACT.balanceOf(msg.sender, _osTokenId),
            "Quantity is greater than serum count"
        );
        require(currentMutantsMaxSupply[_osTokenId] != 0,
            "Incorrect type passed in"
        );

        require(
            mutantsSupply[_osTokenId] + quantity <= currentMutantsMaxSupply[_osTokenId],
            "MAX MUTANTS MINTED"
        );
    ...
    function setMegaMutantMetadataUri(string memory _mergaMutantURI)
        public
        onlyOwner
    {
        mergaMutantURI = _mergaMutantURI;
    }
    function setCurrentMutantsMaxSupply(uint256  _mutantsSupply , uint256  _osTokenId)
        public
        onlyOwner
    {
    ...
    function setOSContract(address  _adddress) public onlyOwner {
      GPC_CHEMISTRY_CONTRACT = IGPC(_adddress);
    }
