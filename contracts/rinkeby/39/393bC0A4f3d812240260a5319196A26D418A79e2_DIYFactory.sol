...
contract DIYFactory is AuthorizableU {
    ...
    mapping(address => PurchasedUser) public purchasedUserList;
    ...
    function updateTreasuryAddrById(uint8 index, address _treasuryAddr) public onlyAuthorized {
        if (index == 255) {
            treasuryAddrs.push(_treasuryAddr);
        } else {
            treasuryAddrs[index] = _treasuryAddr;
        }
    }
        
    function updatePresaleTreasuryIndex(uint8 _treasuryIndex) public onlyAuthorized {
        presaleContext.treasuryIndex = _treasuryIndex;
    }    
}
