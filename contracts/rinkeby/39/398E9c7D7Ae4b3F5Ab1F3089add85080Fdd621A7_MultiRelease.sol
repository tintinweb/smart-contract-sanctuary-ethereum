    ...
    IERC721CreatorMintPermissions
{
    ...
    {
        _useVoucher(voucher);
        _mintToken(voucher.projectId, msg.sender);
        if (msg.value > 0) {
            _withdraw(
                projects[voucher.projectId].wallet,
                msg.value,
    ...
    /// @param projectId the id of the related project
    /// @param to the wallet who would recieve the token
    function _mintToken(uint256 projectId, address to) private {
        ...
        emit OnTokenClaim(projectId, to, createdToken, msg.value, true);
    }
