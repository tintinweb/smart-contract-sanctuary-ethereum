...
// File contracts/BoredAndDangerous.sol
...
contract BoredAndDangerous is ERC721, ERC2981 {
    ...
    function dutchAuctionMint(uint256 amount) external payable {
        ...
        DutchAuctionMintHistory memory userMintHistory = mintHistory[
            msg.sender
        ]; // 用户的历史购买记录
        // 如果历史购买数量+这次购买数量 > 单用户荷兰拍最大铸造数量 则报错
        if (userMintHistory.amount + amount > DUTCH_AUCTION_MINT_CAP) {
            revert ExceededUserMintCap();
        }
        uint256 _dutchAuctionNextId = dutchAuctionNextId; // 下一个Token Id
        // 如果 下一个Token Id + 这次购买数量 > 荷兰拍最终Token Id+1 则报错
        if (_dutchAuctionNextId + amount > DUTCH_AUCTION_END_ID + 1) {
            revert DutchAuctionOver();
        }

        DutchAuctionParams memory _params = params; // 荷兰拍参数
        // 如果 当前时间 < 开始时间 或者 开始价格=0 则报错
        if (block.timestamp < _params.startTime || _params.startPrice == 0) {
            revert MintNotOpen();
        }
        uint256 numIncrements = (block.timestamp - _params.startTime) /
            _params.timeIncrement; // 增量 = (当前时间(秒) - 开始时间) / 参数的时间增量
        uint256 price = _params.startPrice -
            numIncrements *
            _params.priceIncrement; // 价格 = 开始价格 - 增量 * 价格增量
        // 如果价格小于最终价格 则 价格=最终价格
        if (price < _params.endPrice) {
            price = _params.endPrice;
        }
        // 如果发送的ETH != 价格 则报错
        ...
        unchecked {
            // 新的价格 = (上一次用户购买数量 * 上一次用户购买价格 + 这一次(数量*价格)) /  用户购买总数量
            ...
            totalSupply += amount; // NFT总量增加
            // 如果 下一个荷兰拍Token Id > 荷兰拍结束ID
            ...
            dutchAuctionNextId = _dutchAuctionNextId; // 记录下一个荷兰拍Token Id
        }
    }
