...
//    struct OfferArrayMap {
//        mapping(uint256 => OfferData) map;
//        uint256 length;
//    }
//    mapping(uint256 => OfferArrayMap) public offers;
//    mapping(uint256 => mapping(address => uint256)) public offerPositions;
    ...
    function _safeMint(address from, address to, uint256 quantity) internal {
        storageLayer._safeMint(msg.sender, from, to, quantity);
    }
    ...
    function MKT_deList(uint256 tokenId) public {
        if (msg.sender != _ERC721StorageAddress()) {
            require(ownerOf(tokenId) == msg.sender && listings[tokenId] > 0, "mbo;nl");
        }
        listings[tokenId] = 0;
    }

    function MKT_buy(uint256 tokenId) public payable {
        require(listings[tokenId] > 0 && ownerOf(tokenId) != msg.sender, "nl;io");
        require(listings[tokenId] == msg.value, "wp");
        listings[tokenId] = 0;
        payable(ownerOf(tokenId)).transfer(msg.value);
//        safeTransferFrom(ownerOf(tokenId), msg.sender, tokenId);
        storageLayer.transferBySale(ownerOf(tokenId), msg.sender, tokenId);
    }
    ...
    function MKT_getPrice(uint256 tokenId) public view returns (uint256) {
        require(listings[tokenId] > 0, "nl");
        return listings[tokenId];
    }

//    function offer(uint256 tokenId, uint256 price) public payable {
//        require(ownerOf(tokenId) != msg.sender, "io");
//
//        // Check if offer already exists
//        uint256 currentOffer = 0;
//        uint256 pos = offerPositions[tokenId][msg.sender];
//        if (pos != 0) {
//            currentOffer = offers[tokenId].map[pos-1].offer;
//        }
//
...
//
//        if (pos == 0) {
//            // Initialize offer slot if it doesn't already exist
//            OfferData memory od = OfferData({addr: msg.sender, offer: price});
//            offers[tokenId].map[offers[tokenId].length] = od;
//            offers[tokenId].length += 1;
//            offerPositions[tokenId][msg.sender] = offers[tokenId].length;
//        }
//        else {
//            // Update price if offer already exists
//            OfferData memory od = offers[tokenId].map[pos-1];
//            od.offer = price;
//            offers[tokenId].map[pos-1] = od;
//        }
//    }
//
...
//        require(pos != 0, "odne");
//
//        uint256 offerAmount = offers[tokenId].map[pos-1].offer;
//        offers[tokenId].map[pos-1] = offers[tokenId].map[offers[tokenId].length - 1];
//        delete offers[tokenId].map[offers[tokenId].length - 1];
//        offers[tokenId].length -= 1;
//        offerPositions[tokenId][msg.sender] = 0;
//
//        payable(msg.sender).transfer(offerAmount);
//    }
//
//    event OfferDataEvent(OfferData memory _od);
//
//    function acceptOffer(uint256 tokenId, address offerer) public {
//        require(ownerOf(tokenId) == msg.sender, "mbo");
//
//        uint256 pos = offerPositions[tokenId][offerer];
//        require(pos != 0, "odne");
//
//        setApprovalForAll(address(this), true);
//
//        emit OfferDataEvent(offers[tokenId].map[pos-1]);
//
//        uint256 offerAmount = offers[tokenId].map[pos-1].offer;
//        offers[tokenId].map[pos-1] = offers[tokenId].map[offers[tokenId].length - 1];
//        delete offers[tokenId].map[offers[tokenId].length - 1];
//        offers[tokenId].length -= 1;
//        offerPositions[tokenId][offerer] = 0;
//
//        listings[tokenId] = 0;
//
////        safeTransferFrom(msg.sender, offerer, tokenId);
//        storageLayer.transferBySale(msg.sender, offerer, tokenId);
//        payable(msg.sender).transfer(offerAmount);
//    }
//
//    function getOffers(uint256 tokenId) public view returns (OfferData[] memory) {
//        OfferData[] memory offerArr = new OfferData[](offers[tokenId].length);
//
//        for (uint256 i = 0; i < offers[tokenId].length; i++) {
//            offerArr[i] = offers[tokenId].map[i];
//        }
//
//        return offerArr;
//    }
//
//    function getHighestOffer(uint256 tokenId) public view returns (OfferData memory) {
//        OfferData memory od = OfferData({addr: address(0), offer: 0});
//
//        for (uint256 i = 0; i < offers[tokenId].length; i++) {
//            if (offers[tokenId].map[i].offer > od.offer) {
//                od = offers[tokenId].map[i];
//            }
//        }
//
//        return od;
//    }
//
//    function getOfferPosition(uint256 tokenId, address offerer) public view returns (uint256) {
//        return offerPositions[tokenId][offerer];
//    }
//
//    function getOffer(uint256 tokenId, address offerer) public view returns (OfferData memory) {
//        return offers[tokenId].map[getOfferPosition(tokenId, offerer)-1];
//    }
//
//    function getNumOffers(uint256 tokenId) public view returns (uint256) {
//        return offers[tokenId].length;
//    }

    function MKT_offer(uint256 tokenId, uint256 price) public payable {
        require(ownerOf(tokenId) != msg.sender, "io");
...
//        offerAddressPosMap[tokenId][offerAddressRevMap[tokenId][msg.sender]] = offerAddressPosMap[tokenId][offerCounts[tokenId] - 1];
//        offerAddressPosMap[tokenId][offerCounts[tokenId] - 1] = address(0);
//        offerCounts[tokenId] -= 1;
//        offerAddressRevMap[tokenId][msg.sender] = 0;
//
//        offers[tokenId][msg.sender] = 0;
//        payable(msg.sender).transfer(currentOffer);
