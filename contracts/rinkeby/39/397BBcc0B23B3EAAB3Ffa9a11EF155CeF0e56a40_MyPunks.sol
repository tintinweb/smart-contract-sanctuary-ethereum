...
contract MyPunksDNA {
    ...
    // 
    // Calculate DNA, This pseudo random function is determistic and should not be used on production
        ...
        returns (uint256)
    {
        ...
        bytes32 hashedParams = keccak256(encodedParams);//funcion de hashing
    ...
    //funcion interna porqje solo este contrato lo va utilizar
...
contract MyPunks is ERC721, ERC721Enumerable, MyPunksDNA{
    ...
    constructor(uint256 _maxSupply) ERC721("MyPunks", "MYPKS") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No MyPunks left :(");
    ...
    function _paramsURI(uint256 _dna) internal view returns (string memory) {
        ...
        return string(abi.encodePacked(params, "&topType=", getTopType(_dna))); //estrategia para evitar limitacion de la EVM de 16 varuables
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "MyPunks #',
                tokenId.toString(),
                '", "description": "My Punks are randomized Avatars stored on chain to teach DApp development by eCituk", "image": "',
                image,
                '"}'
            )
        );
