...
contract sampleNFT is ERC721A, Ownable {
  ...
  constructor() payable ERC721A('sampleNFT', 'TESTFALSEC3') {
    maxSupply = 6942;
    maxPerTxnAllowlist = 5;
    maxPerWalletAllowlist = 5;
    maxPerTxnPublic = 1000;
    maxPerWalletPublic = 1000;
  ...
  */
  function publicMint(uint256 numberOfTokens) public payable {
    ...
    require(msg.value == getPrice(numberOfTokens), 'wrong value');
    require(
      _numberMinted(msg.sender) + numberOfTokens <= maxPerWalletPublic,
      'exceeds max wallet'
    );
    require(totalSupply() < maxSupply, 'sold out');
    require(totalSupply() + numberOfTokens <= maxSupply, 'exceeds max supply');
    require(numberOfTokens <= maxPerTxnPublic, 'exceeds max per txn');

    _safeMint(msg.sender, numberOfTokens);
  }

  function mintAllowlist(uint256 numberOfTokens, bytes32[] calldata merkleProof)
    public
    payable
    isValidMerkleProof(merkleProof, whitelistMerkleRoot)
  {
    ...
    require(msg.value == getPrice(numberOfTokens), 'wrong value');
    require(
      _numberMinted(msg.sender) + numberOfTokens <= maxPerWalletAllowlist,
      'exceeds max wallet allowlist'
    );
    require(totalSupply() < maxSupply, 'sold out');
    require(totalSupply() + numberOfTokens <= maxSupply, 'exceeds max supply');
    require(numberOfTokens <= maxPerTxnAllowlist, 'exceeds max per txn');
    _safeMint(msg.sender, numberOfTokens);
