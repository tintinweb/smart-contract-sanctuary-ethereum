...
contract eegods is VRFConsumerBaseV2 {
   ...
   modifier onlyAdmin {
    ...
    //actually min...
    function getFloor() public view returns(Heap.Node memory){
        Heap.Node memory node = data.getFloor();
        return node;
    }
    ...
    function buyPunk(uint256 punkIndex) payable public {
        ...
        if (!data.isFloor(punkIndex)){
            data.extractById(punkIndex);
            pendingWithdrawals[seller] += int256(msg.value);
            return;
        }
        ...
        holdingWithdrawals[s_requestId] = PendingWithdrawal(seller,int256(msg.value));
...
library Heap {
  ...
  function getFloor(Data storage self) internal view returns(Node memory){
