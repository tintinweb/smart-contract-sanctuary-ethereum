...
contract GriffinToken is ERC20Detailed, Ownable, ReentrancyGuard {
    ...
    function getEarned(address user) external view returns (uint256) {
        return (_gonBalances[user].div(_gonsPerFragment)).sub(_depositBalances[user]);
    }
    ...
    function presaleMigrate() external nonReentrant {
        require(isMigrate[msg.sender] == false, 'You already have migrate');
        _gonBalances[msg.sender] += presale.balanceOf(msg.sender).mul(_gonsPerFragment);
        isMigrate[msg.sender] = true;
    }
