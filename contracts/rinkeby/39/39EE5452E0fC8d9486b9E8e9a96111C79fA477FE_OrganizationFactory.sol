...
contract OrganizationFactory {
    ...
    /* 
        This functionality requires one extra transaction to open a page with organization details
        So, When An organization is Created, The data of the organization will be stored in the 
        blockchain, but it should also be stored in the website database, so that this extra 
        transaction can be avoided, and we can just access database for details of the 
        organization.
    */
    ...
    /* 
        When an organization Request for spending it's donators' money, We can do either of these
        two things:
        1. Send request to "m" donators individually, which requires "m" transactions.
        2. Create a list of "m" donators address, this list is sent to font_end (by some means)
        and stored (the )
    */
    // function request() external returns (bool, address[] memory) {
    //     for (
    //         uint256 organizationIndex = 0;
    //         organizationIndex < organizations.length;
    //         organizationIndex++
    ...
    //             msg.sender
    //         ) {
    //             address[] memory requestedDonators = organizationIndex[
    //                 organizationIndex
    //             ].RequestedDonators();
    ...
    function approval(address _organizationAddress) external {}
}
...
contract Organization is Ownable {
    ...
    function RequestedDonators() public view returns(address[] memory){
