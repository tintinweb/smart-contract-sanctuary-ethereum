...
contract SpanishflyNFT is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable, Ownable {
    using Counters for Counters.Counter;

    uint256 private constant _price = 0.005 ether;
    uint256 private constant _maxNft = 6666;
    uint256 private constant _freeNft = 2222;
    uint256 private constant _freeNum = 2;
    uint256 private constant _payNum = 5;
    uint256 private constant _start = 1655996400;
     ...
     * See {ERC721-tokenURI}.
     */
    constructor() ERC721("Spanishfly NFT", "SpanishflyNFT") {
    ...
    function setTokenAuthor(uint256 tokenId, address author) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_exists(tokenId), "Token is not existed.");
    ...
    function setTokenURI(uint256 tokenId, string calldata uri) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_exists(tokenId), "Token is not existed.");
    ...
    function mint(address to,uint number) public payable {
        ...
        if(totalSupply() + number < _freeNft){
            require(balanceOf(to) + number <= _freeNum, "Up to two nft");
        } else if(totalSupply() + number >= _freeNft){
            require(balanceOf(to) + number <= _payNum, "Up to nft");
        }
        // top 2222 free mint
        if(totalSupply() + number > _freeNft){
            require(msg.value >= _price * number, "Ether value sent is not correct");
        }
    ...
    function airdrop(address to) public {
        uint max = 5;
        require(block.timestamp > _start,"Not started");
        require(_airdrop[to],"No permission");
        require(max + totalSupply() <= _maxNft, "Purchase would exceed max supply of NFTs");
    ...
    function setAirdrop(address[] calldata addrs,bool status) public onlyRole(DEFAULT_ADMIN_ROLE){
