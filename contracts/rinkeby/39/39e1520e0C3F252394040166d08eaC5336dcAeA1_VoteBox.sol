...
error NonExistPoll();
error NonExistPollOption();
error NonExistNftContract();
...
error VoterDisallowed();
error VoterIsNotNFTHolder();
...
contract VoteBox is IVoteBox, Ownable, EIP712VoteAllowlist, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => PollOption)) public pollOptions;


    constructor (address _signer) EIP712VoteAllowlist() {
        ...
        onlyOwner 
    {
        ...
        if (_optionCount < 1) revert CreatePollWithInvalidOptionCount();
    ...
    function setPollOptionName(uint256 _pid, uint256 _option, string calldata _name) external onlyOwner {
        if (!_existsPoll(_pid)) revert NonExistPoll();
        if (!_existsPollOption(_pid, _option)) revert NonExistPollOption();
        pollOptions[_pid][_option].name = _name;
    }
    ...
    function vote(uint256 _pid, uint256 _value, bytes calldata _signature) callerIsUser external {
        ...
        if (!_existsPollOption(_pid, _value)) revert NonExistPollOption();
        if (!_isPollStarted(_pid)) revert VoteForNonStartedPoll();
        if (!isVoterAllowed(_pid, _signature)) revert VoterDisallowed();
        if (!isVoterNftHolder(_pid)) revert VoterIsNotNFTHolder();
    ...
    function getPollOptionName(uint256 _pid, uint256 _option) external view override returns (string memory) {
        if (!_existsPoll(_pid)) revert NonExistPoll();
        if (!_existsPollOption(_pid, _option)) revert NonExistPollOption();
        return pollOptions[_pid][_option].name;
    }
    ...
    function getPollOptionVoteCount(uint256 _pid, uint256 _option) external view override returns (uint256) {
        if (!_existsPoll(_pid)) revert NonExistPoll();
        if (!_existsPollOption(_pid, _option)) revert NonExistPollOption();
    ...
    function isVoterAllowed(uint256 _pid, bytes calldata _signature) public view override returns (bool) {
        // allowlist checking may be disabled
    ...
    function isVoterNftHolder(uint256 _pid) public view override returns (bool) {
        ...
        address addr = polls[_pid].nftContract;
        if (addr == address(0)) revert NonExistNftContract();

        return IERC721(addr).balanceOf(msg.sender) > 0;
    }
    ...
    function _getPollVoteCount(uint256 _pid) internal view returns(uint256) {
        ...
        uint256 count = 0;
        do {
            count += pollOptions[_pid][index].vote;
            index ++;
        }
    ...
    function _getPollOptionVoteCount(uint256 _pid, uint256 _option) internal view returns(uint256) {
        return pollOptions[_pid][_option].vote;
    }
}
...
interface IVoteBox {
    ...
    function isVoterNftHolder(uint256 _pid) external view returns(bool);
}
