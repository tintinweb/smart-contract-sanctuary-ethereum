...
contract Crypts is ERC721Optimized, ReentrancyGuard, Ownable(5, true, true) {
    ...
    function whitelistMint(uint256 qty) external payable nonReentrant {
        ...
        require(graveyard.isWhitelisted(sender, qty), "Invalid whitelist address/qty");
    ...
    /// Multiplier of 10 * token balance.
    ...
    function setSupply(uint256 maxSupply) external onlyOwner {
        _setSupply(maxSupply);
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        IGraveyard graveyard = IGraveyard(GRAVEYARD_ADDRESS);
        if (from != address(0)) graveyard.updateClaimable(from, 0);
        if(from != address(0) && to != address(0)) graveyard.updateClaimable(to, 0);
    }
}
...
contract ERC721Optimized is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function _setSupply(uint256 maxSupply) internal virtual {
        require(maxSupply >= _tokensMinted, "Invalid args");
        _maxSupply = maxSupply;
    }
