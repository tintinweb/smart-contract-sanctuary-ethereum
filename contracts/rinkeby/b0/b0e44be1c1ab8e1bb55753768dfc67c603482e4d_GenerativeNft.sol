...
import "./SharedNft.sol";

contract GenerativeNft is SharedNft
{
    function initialize(address config) public initializer {
        __SharedNft_init(config, "GenerativeNft", "0.3.0");
    }

    function name() public pure override returns (string memory) {
        return "GenerativeNft";
    }
    ...
    function _verifySignature(
        Collaborator[] memory collaborators,
        uint256 tokenId,
        uint256 mintCount,
        ...
        uint256 collaboratorsHash = hashCollaborators(collaborators);

        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "GenerativeNftSignData(uint256 tokenId,uint256 mintCount,uint256 maxMintCount,uint256 price,uint256 collaboratorsHash)"
                    ),
                    tokenId,
                    mintCount,
                    maxMintCount,
    ...
    function _feePercentMinting() internal view override returns (uint256) {
        return SharedConfig(_config)._feePercentGenerativeNfts();
    }
}
...
contract SharedConfig is AccessControlEnumerableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        _feePercentGenerativeNfts = 250; // 2.5%
    ...
    function setSigner(address signer) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        _signer = signer;
    }

    function setWithdrawalWallet(address payable withdrawalWallet) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        _withdrawalWallet = withdrawalWallet;
    }

    function setNftHost(string memory nftHost) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        _nftHost = nftHost;
    }

    function setFeePercentMintingTracks(uint256 fee) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        require(fee <= 10000, "Invalid fee percent");
        _feePercentMintingTracks = fee;
    }

    function setFeePercentGenerativeNfts(uint256 fee) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        require(fee <= 10000, "Invalid fee percent");
        _feePercentGenerativeNfts = fee;
    }

    function setFeePercentPurchasingProducts(uint256 fee) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        require(fee <= 10000, "Invalid fee percent");
...
abstract contract SharedNft is
    SharedLogic,
    ContextUpgradeable,
    ...
    UUPSUpgradeable
{
    ...
    function __SharedNft_init(address config, string memory name, string memory version) internal initializer {
        ...
        __SharedNft_init_unchained(config, name);
    }

    function __SharedNft_init_unchained(address config, string memory name) internal initializer {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _config = config;
        _name = name;
    }
    ...
    function contractURI() public view virtual returns (string memory) {
        return string(abi.encodePacked(SharedConfig(_config)._nftHost(), "/api/v1/contracts/MatterNft/metadata"));
    }

    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        return SharedConfig(_config)._nftHost();
    }
    ...
    function redeem(
        ...
        _verifySignature(collaborators, tokenId, mintCount, maxMintCount, price, signature);
        _setMaxMintingCount(tokenId, maxMintCount);
        _validateMintCountIsAvailableToMint(tokenId, mintCount, maxMintCount);
        _processPayment(msg.value, price, collaborators);
        _mintNft(buyerAddress, tokenId, mintCount);
    }

    function hashCollaborators(Collaborator[] memory collaborators) public pure returns (uint256) {
        return uint256(keccak256(abi.encode(collaborators)));
    }
    ...
    function _processPayment(
        ...
        for (uint256 i = 0; i < collaborators.length; i++) {
            uint256 revenue = (sellerAmount / PERCENTAGE) * collaborators[i].rate * 100;

            // solhint-disable-next-line
            (bool wasSent, ) = payable(collaborators[i].wallet).call{value: revenue}("");
    ...
    function _feePercentMinting() internal view virtual returns (uint256) {
        return 0;
    }
