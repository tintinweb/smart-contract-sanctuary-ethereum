...
contract NoNos is ERC721A, Ownable {
    ...
    uint256 private _maxNoNos = 101;
    uint256 private _maxTX = 3;
    uint256 private _maxNoNosPerAddressDuringMint = 3;
    bool public _pausedPublic = false;
    bool public _pausedPrivate = false;
    uint256 private _publicSaleKey = 111;
    mapping(address => uint256) public nonolist;
    ...
    function buyTicketNoNoList(uint256 quantity, uint256 callerPublicSaleKey)
        external
        payable
        callerIsUser
    {
        ...
        require(
            nonolist[msg.sender] > 0,
            "No No List is not available for you."
        );
        require(
            quantity < _maxTX,
    ...
    function addNoNoList(address[] memory addresses, uint256[] memory numSlots)
        external
        onlyOwner
    {
        require(
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            nonolist[addresses[i]] = numSlots[i];
        }
    }
