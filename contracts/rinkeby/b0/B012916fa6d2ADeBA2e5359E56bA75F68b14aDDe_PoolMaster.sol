...
abstract contract PoolBase is PoolBaseInfo {
        ...
        returns (uint256)
    {
        ...
        // Base delta is difference between last borrows and warning utilization minimal borrows
        uint256 baseDelta = _poolSize(_info).mulDecimal(warningUtilization) -
            _info.borrows;
        // Delta factor is factor on which base delta should be divided to consider reserves & insurance
        // Based on following formula:
        // (IFWU = InterestForWarningUtilization, BFWU = BorrowsForWarningUtilization)
        // borrows + IFWU + IFWU * (reserveFactor + insuranceFactor) = BFWU
        // baseDelta = IFWU * (1 + reserveFactor + insuranceFactor)
        uint256 deltaFactor = Decimal.ONE +
            (reserveFactor + insuranceFactor).mulDecimal(warningUtilization);
        // Delta borrows is how much borrows should actually increase for warning
        uint256 deltaBorrows = baseDelta.divDecimal(deltaFactor);

        uint256 interestPerSec = _info.borrows * interestRate;
        // Time delta is calculated as delta borrows divided by interest per sec (rounded up)
        uint256 timeDelta = (deltaBorrows * Decimal.ONE + interestPerSec - 1) /
            interestPerSec;
