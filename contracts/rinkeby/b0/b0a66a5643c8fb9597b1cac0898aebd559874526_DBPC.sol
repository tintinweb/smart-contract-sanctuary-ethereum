...
// File: contracts/DBPC.sol


pragma solidity >=0.7.0 <0.9.0;

contract DBPC is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string public baseURI;
  string public baseExtension = ".json";
  string public notRevealedUri;
  uint256 public wlcost = 0.0267 ether;
  uint256 public pcost = 0.2621 ether;
  ...
  uint256 public wlSupplyLimit = 3800;
  uint256 public publicSupplyLimit = 6000;    
  ...
  function whitelistmint(uint256 _mintAmount) public payable {
    ...
    require(wlMinted + _mintAmount <= wlSupplyLimit, "max wlSupplyLimit NFT limit exceeded");
    if (msg.sender != owner()) {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(publicMinted + _mintAmount <= publicSupplyLimit);
  ...
  function airdrop(address[] memory airdropList) public onlyOwner{
        uint256 supply = totalSupply();
        for(uint256 X = 0; X < airdropList.length; X++){
         require(supply + 1 <= maxSupply, "Max Limit reached ");   
        _safeMint(airdropList[X], supply++);
        }
    }
