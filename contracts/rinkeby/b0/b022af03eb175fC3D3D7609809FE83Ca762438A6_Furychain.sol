...
contract Furychain is ERC721Enumerable, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    uint public max_mint_allowed = 6;
    ...
    constructor(string memory _theBaseURI, string memory _notRevealedURI, bytes32 _merkleRoot) ERC721("Furychain", "FURY") PaymentSplitter(_team, _teamShares) {
    ...
    **/
    function presaleMint(address _account, bytes32[] calldata _proof, uint256 _ammount) external payable nonReentrant {
        //Are we in Presale ?
        require(sellingStep == Steps.Presale, "Presale has not started yet.");
        //Did this account already mint an NFT ?
        require(nftsPerWallet[_account] < 4, "You can only get 4 NFT on the Presale");
    ...
    **/
    function saleMint(uint256 _ammount) external payable nonReentrant {
        ...
        require(msg.value >= price * _ammount, "Not enought funds.");
        //The user can only mint max 10 NFTs
        require(_ammount <= max_mint_allowed, "You can't mint more than 6 tokens");
