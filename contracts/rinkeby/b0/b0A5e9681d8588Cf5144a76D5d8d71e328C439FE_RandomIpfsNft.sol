...
contract RandomIpfsNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
	...
	uint256 internal immutable s_mintFee;
	...
	constructor(
		...
		uint256 _mintFee
	) VRFConsumerBaseV2(_vrfCoordinatorAddr) ERC721("Doggy Styles", "DOG") {
		i_vrfCoordinator = VRFCoordinatorV2Interface(_vrfCoordinatorAddr);
	...
	function getBreedFromRng(uint256 _rng) internal pure returns (Breed) {
		uint256[3] memory chanceArray = getChanceArray();
		for (uint8 i = 0; i < chanceArray.length; i++) {
			if (_rng < chanceArray[i]) {
				return Breed(i);
			}
		}
	...
	function getDogTokenURIs(uint256 _index) external view returns (string memory) {
