...
contract AmNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
     ...
     */
    function setWhitelistMintSwitch(bool _whitelistMintSwitch) external onlyOwner {
        whitelistMintSwitch = _whitelistMintSwitch;                                         
    }
    
    /**
     * @dev setter for mint
     * @param _mintSwitch - bool for whitelist mint is active
     */
    function setMintSwitch(bool _mintSwitch) external onlyOwner {
        mintSwitch = _mintSwitch;                                         
    }

    function isTokenValid(uint256 _tokenId) public view returns (bool) {
        require(_exists(_tokenId), "Token does not exist.");
        return (expiryTime[_tokenId] <= block.timestamp);
    } 
    ...
    function mint() external payable noHaxxor {
        ...
        expiryTime[tokenIndex] = block.timestamp + passDuration;//
     ...
     */
    function renewPassOwner(uint _tokenId) public payable onlyOwner{
        require(msg.value == renewalPrice, "Incorrect amount of ether sent.");
        require(_exists(_tokenId), "Pass does not exist.");

        // TODO: ci je aj vlastnikom checknut a null adresu pre burnnute tokeny
        // TODO: ako checkovat ci ma user viac tokenov - osetrit ze bude mat len jeden alebo nechat viac a nejak ich zobrazit

        expiryTime[_tokenId] += passDuration;
    ...
    function addSlotsToMaxSupply(uint256 numberOfNewSlots) public onlyOwner {
