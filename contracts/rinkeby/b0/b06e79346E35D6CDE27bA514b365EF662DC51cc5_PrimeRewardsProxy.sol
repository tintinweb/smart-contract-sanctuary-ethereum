...
contract PrimeRewardsProxy is AccessControl, Proxy {
    address public immutable primeRewardsAddress;
    PrimeRewards public immutable primeRewards;

    constructor(address _primeRewardsAddress) {
        primeRewardsAddress = _primeRewardsAddress;
    ...
    {
        require(
            block.timestamp < primeRewards.endTimestamp() ||
                primeRewards.endTimestamp() == 0,
            "Cannot use addPool outside of a period"
        );
    ...
    function renounceRole(bytes32 role, address account)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(role == DEFAULT_ADMIN_ROLE, "Ownership cannot be renounced");
