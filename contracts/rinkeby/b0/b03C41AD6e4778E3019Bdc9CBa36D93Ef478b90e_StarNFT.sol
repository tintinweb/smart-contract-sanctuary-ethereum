...
contract StarNFT is ERC721A, Ownable,  ReentrancyGuard{
    uint256 public immutable MAX_SUPPLY = 9141;
    uint256 public constant PRICE = 0.05 ether;
    // uint256 public maxPublic = 5; // max total for public mint
    uint256 public immutable PRESALE_SUPPLY = 6000; // Presale supply
    ...
    mapping(address => uint256) private _preClaimed;
    mapping(address => uint256) private _publicClaimed;

    constructor(
    ) ERC721A("StarNFT", "StarNFT")
    {
    }
    ...
    //     MAX_PER_WALLET_PUBLIC = maxMints_;
    // }
    ...
    function preSaleMint() external payable onlyPresale {
        ...
        require(_preClaimed[msg.sender] < 1, "ALREADY_CLAIMED");
        ...
        onlyPublicSale
    {
        ...
        // require(
        //     _publicClaimed[msg.sender] + quantity <= maxPublic,
        //     "exceeds max per address"
        // );
        require(_publicClaimed[msg.sender] + quantity <= MAX_PER_WALLET_PUBLIC, 
            "EXCEEDS_MAX_ALLOWANCE"
        );
        require(totalSupply() + quantity <= MAX_SUPPLY, "EXCEEDS_MAX_SUPPLY");

        _safeMint(msg.sender, quantity);
        _publicClaimed[msg.sender] = _publicClaimed[msg.sender] + quantity;
    }
}
