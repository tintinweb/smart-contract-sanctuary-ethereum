    ...
    OwnableUpgradeable
{
    ...
    function setRewardCap(
        ...
        for (uint8 v = 0; v < rewardTokenAddresses.length; v++) {
    ...
    function sendOne(
        ...
        uint256 rewardValue = pbr1.mul(rValue).div(1e12);
        require(rewardCap[cohortId][rewardTokenAddress] >= rewardValue, 'RCR');
        uint256 refEarned = (rewardValue.mul(referralPercentage).div(100000));
        uint256 userEarned = rewardValue.sub(refEarned);
        ...
        rewardCap[cohortId][rewardTokenAddress] = rewardCap[cohortId][rewardTokenAddress].sub(rewardValue);
    }
    ...
    function sendMulti(
        ...
        for (uint8 r = 1; r < rewardTokens.length; r++) {
            uint256 exactReward = pbr[r].mul(rValue).div(1e12);
            require(rewardCap[cohortId][rewardTokens[r]] >= exactReward, 'RCR');
            uint256 refEarned = (exactReward.mul(referralPercentage).div(100000));
            uint256 userEarned = exactReward.sub(refEarned);
            ...
            rewardCap[cohortId][rewardTokens[r]] = rewardCap[cohortId][rewardTokens[r]].sub(exactReward);
        }
    }
