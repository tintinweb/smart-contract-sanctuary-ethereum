...
contract Nanobits is ERC20Burnable, Ownable {
    ...
    function testFunction(uint256 _timestamp, uint256 _rarity) external view returns(uint256) {
    ...
    function unstake(uint256[] memory _tokenIds) public {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            uint256 rewards = claim(_tokenIds[i]);
    ...
    function claimAll() public {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            uint256 id = tokens[i];
            uint256 rewards = claim(tokens[id]);
            totalRewards += rewards;
        }
    ...
    function checkRewardsByIds(uint256 tokenId) external view returns (uint256) {
        ...
        uint256 totalRewards = ((block.timestamp - stakedData[tokenId].timeStaked) * commonRatePerDay);
        return totalRewards;
    }

    function checkAllRewards(address _owner) external view returns (uint256) {
        uint256 totalRewards = 0;
        uint256[] memory pups = tokenIds[_owner];

        for (uint256 i = 0; i < pups.length; i++) {
            totalRewards += ((block.timestamp - stakedData[pups[i]].timeStaked) * commonRatePerDay);
        }
