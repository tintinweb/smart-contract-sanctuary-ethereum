...
contract GutterMerch is ERC1155, Ownable, ReentrancyGuard {
    using Strings for uint256;
	using ECDSA for bytes32;

	uint256 public itemPrice = 10000000000000000; //0.01 ETH
	...
	mapping (uint256 => uint256) public burnedCounter;
	...
	function _validateMint(
		...
		bytes32 resultHash = ECDSA.toEthSignedMessageHash(
			keccak256(abi.encodePacked(sender, id, qty, price, currency, nonce))
		);
		require(resultHash == hash, "Invalid hash");
        require(ECDSA.recover(hash, signature) == signerAddress, "Wrong signer");

		require(saleLiveById(id), "sale is not live");
		require(currency == 1, "incorrect currency");
		require(msg.value == price * qty && price == itemPriceById(id), "incorrect price");
		require(qty <= 50, "only 50 can be minted at once");
		require(!usedNonces[nonce], "nonce is used");
	}

    // accepted IDS: 1 - XS, 2 - S, 3 - M, 4 - L, 5 - XL, 6 - XXL, 7 - XXXL
	function mint(
		bytes memory signature,
		bytes32 hash,
		string memory nonce,
		...
		totalMinted[id] = totalMinted[id] + qty;
	...
	function saleLiveById(uint256 id) public view returns (bool) {
		return true;
	}
	...
	function itemPriceById(uint256 id) public view returns (uint256) {
		return itemPrice;
	}
	...
	function burn(
		...
		require(burnLiveById(id), "burn is not enabled");
		require(
		...
		burnedCounter[id] = burnedCounter[id] + qty;
