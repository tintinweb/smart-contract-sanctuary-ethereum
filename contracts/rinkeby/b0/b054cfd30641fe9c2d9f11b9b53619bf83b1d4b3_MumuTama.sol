...
contract MumuTama is Context, IERC20, ERC20Ownable {
    using SafeMath for uint256;

    string private constant _name = "MUMUTAMA";
    string private constant _symbol = "MUMUTAMA";
	...
	uint256 private constant BUY = 0;
    uint256 private constant SELL = 0;
    uint256 private constant TRANSFER = 0;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(_initiateTrades == true) {
            ...
            //earlyBuyPenaltyEnd = block.timestamp + 0 hours;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (from == uniV2Pair) {
              ...
              ////  if(isSniper[from] && earlyBuyPenaltyEnd >= block.timestamp){
               //     _marketingTax = _marketingTax * 0;
             //       _liquidityTax = _liquidityTax * 0;
             //   }
            } else {
