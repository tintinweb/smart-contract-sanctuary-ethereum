    ...
    Mintable
{
    ...
    function initialize(Sale sale, address preSaleSigner) public initializer {
        __ERC721_init("Tropical Birds", "CUICUI");
    ...
    function _baseURI() internal pure override returns (string memory) {
        return
            "https://ipfs.io/ipfs/QmXAAj4AHgLBKf6pXmytMjs9PUHk4LQzL7BJm7Epihmc23/";
    }
    ...
    function _burn(uint256 tokenId)
        internal
        override(ERC721Upgradeable, ERC721URIStorageUpgradeable, Mintable)
    {
        super._burn(tokenId);
    }
...
abstract contract Config {
    ...
    string public constant ERROR_MAX_PRE_SALE_MINT =
        "Max pre-sale mint reached";
    string public constant ERROR_MAX_SUPPLY = "Max supply reached";
    string public constant ERROR_SALE = "Invalid sale status";
    string public constant ERROR_PRE_SALE_SIGNATURE =
        "Invalid pre-sale signature";
    string public constant ERROR_RESERVED_MINTED =
    ...
    Config
{
    ...
    mapping(address => uint256) private _preSaleMintedTokens;
    bool private _hasMintedReserved;

    // solhint-disable-next-line func-name-mixedcase
    function __Mintable_init(Sale _sale, address _preSaleSigner)
        internal
        onlyInitializing
    {
        __Mintable_init_unchained(_sale, _preSaleSigner);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __Mintable_init_unchained(Sale _sale, address _preSaleSigner)
        internal
        onlyInitializing
    {
        sale = _sale;
    ...
    {
        _preSaleMintedTokens[msg.sender] = count.add(
            _preSaleMintedTokens[msg.sender]
        );
        _mint(count);
    }
    ...
    function _mint(uint256 count) private checkFunds(count) checkSupply(count) {
    ...
    function _mintToken() private {
        ...
        _setTokenURI(tokenId, string(abi.encodePacked(StringsUpgradeable.toString(tokenId), ".json")));
    }
    ...
    modifier checkCount(uint256 count) {
        require(count > 0 && count <= MAX_PER_MINT, ERROR_MAX_PER_MINT);
        _;
    }

    modifier checkFunds(uint256 count) {
        require(msg.value >= PRICE.mul(count), ERROR_FUNDS);
        _;
    }

    modifier checkSupply(uint256 count) {
        require(
            _tokenIdCounter.current().add(count) <= MAX_SUPPLY - RESERVED_TOKENS,
            ERROR_MAX_SUPPLY
        );
        _;
    }
    ...
    modifier checkPreSaleCount(uint256 count) {
        require(
            count.add(_preSaleMintedTokens[msg.sender]) <= MAX_PRE_SALE_MINT,
    ...
    modifier checkPreSaleSignature(bytes memory signature) {
        ...
        require(
            signer != address(0) && signer == preSaleSigner,
            ERROR_PRE_SALE_SIGNATURE
        );
        _;
    }
