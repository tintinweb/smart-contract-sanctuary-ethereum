...
contract Moonknight is Ownable, ERC721A, PaymentSplitter {
    ...
    mapping(address => uint) public MAX_NFT_PER_MLSALE;
    mapping(address => uint) public MAX_NFT_PER_PUBSALE;
    ...
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(wlsaleIsActive, "Whitelist Sale has not activated yet");
        require(isWhiteListed(msg.sender, _proof), "Not whitelisted");
        require(MAX_NFT_PER_MLSALE[msg.sender] + _quantity <= 1, "You can only get 1 NFT on the Whitelist Sale");
        require(totalSupply() + _quantity <= MAX_MOONLIST, "Max supply exceeded");
        require(msg.value >= price * _quantity, "Not enought funds");
        MAX_NFT_PER_MLSALE[msg.sender] += _quantity;
    ...
    function publicSaleMint(address _account, uint _quantity) external payable callerIsUser {
        ...
        require(MAX_NFT_PER_PUBSALE[msg.sender] + _quantity <= 10, "You can only get 20 NFT on the Public Sale");
        require(msg.value >= price * _quantity, "Not enought funds");
        _safeMint(_account, _quantity);
    }
