...
import "./common/BOOMAuth.sol";

import "./RoleManager.sol";
import "./common/ICrossContractManListener.sol";

contract CrossContractMan is BOOMAuth {
    ...
    function getId() view public returns(bytes32) {
	return MANAGER_CONTRACT_ID;
    }

    function getName() view public returns(string memory) {
	return MANAGER_CONTRACT_NAME;
    }

    function getVersion() virtual view external returns(bytes32) {
	...
	bool is_listener = _isListener(contract_addr);
	if(is_listener){
	    require(ICrossContractManListener(contract_addr).getId() == h_name, "Contract's ID does not match the indicated ID");
	    require(ICrossContractManListener(contract_addr).getId() == keccak256(abi.encodePacked(
		    ICrossContractManListener(contract_addr).getName()
		)
	    ), "Contract's ID does not match the indicated ID");
	}
	bool is_new = false;
	if(contracts[h_name] != address(0)){
	    if(is_listener)
		ICrossContractManListener(contract_addr).onUpdate(contracts[h_name], address(this));
	    if(_isListener(contracts[h_name]))
		ICrossContractManListener(contracts[h_name]).onReplaced(contract_addr);
	    if(address(authManager) != address(0))
		RoleManager(address(authManager)).transferRoles(h_name, contracts[h_name], contract_addr);
	...
	for(uint i=0;i<hnames.length;i++){
	    ...
	    if(_isListener(contracts[hnames[i]]))
	...
	if(h_name == ROLE_MANAGER_CONTRACT_ID){
	    _linkRoleManager(contract_addr);
//	    console.log("AUTH MANAGER SET TO %s",uint256(uint160(contract_addr)).toHexString());
	}

	contracts[h_name] = contract_addr;
	ids[contract_addr] = h_name;
    }

    function removeContract(string calldata name) public virtual onlyOwner {
	bytes32 h_name = keccak256(abi.encodePacked(name));
	if(contracts[h_name] == address(0))return;
	bool is_listener = _isListener(contracts[h_name]);
	if(_isListener(contracts[h_name])){
	    ICrossContractManListener(contracts[h_name]).onRemoved();
	}
	for(uint i=0;i<hnames.length;i++){
	    if(contracts[hnames[i]] == address(0))break;
	    if(!_isListener(contracts[hnames[i]]))break;
	...
	delete ids[contracts[h_name]];
	delete contracts[h_name];
    }

    function isListener(address instance) public view returns(bool){
	return _isListener(instance);
    }

    function _isListener(address instance) internal view returns(bool) {
    ...
    function getContractId(address addr) public view returns(bytes32) {
	return ids[addr];
    }
...
*/
//contract RoleManager is ContextMixin, AccessControlEnumerable, Ownable {
contract RoleManager is BOOMBase {
    using Strings for uint256;
	...
	return ROLE_MANAGER_CONTRACT_ID;
    }

    function getName() override view public returns(string memory) {
	return ROLE_MANAGER_CONTRACT_NAME;
    }
	...
	bytes32 hname = CrossContractMan(manager).getContractId(account);
	if(hname == bytes32(0))return;

	roleIds[hname][role] = roles[hname].length;
	roles[hname].push(role);

//	console.log("ADDED ROLE %s for %s",uint256(role).toHexString(),uint256(uint160(account)).toHexString());
    }
...
//	console.log("REVOKED ROLE %s for %s",uint256(role).toHexString(),uint256(uint160(account)).toHexString());
    }

    function transferRoles(bytes32 hname, address old_instance, address new_instance) public virtual onlyManager {
//	console.log("MOVING ROLES FOR %s from %s to %s",uint256(hname).toHexString(),uint256(uint160(old_instance)).toHexString(),
//		uint256(uint160(new_instance)).toHexString());

	for(uint i=0;i<roles[hname].length;i++){
	    _revokeRole(roles[hname][i],old_instance);
	    _grantRole(roles[hname][i],new_instance);
//	    console.log("MOVED ROLE %s from %s to %s",roles[hname][i],old_instance,new_instance);
//	    console.log("MOVED ROLE %s from %s to %s",uint256(roles[hname][i]).toHexString(),uint256(uint160(old_instance)).toHexString(),
//		uint256(uint160(new_instance)).toHexString());
	}
    }
...
/*    function grantRole(bytes32 role, address account) public override virtual {
	console.log("GRANT ACCOUNT %s", account);
	console.logBytes(abi.encodePacked(role));
	super.grantRole(role, account);
    }*/

/*    function hasRole(bytes32 role, address account) public view virtual override returns (bool) {
	console.log("HAS_ROLE ACCOUNT %s", account);
	console.logBytes(abi.encodePacked(role));
	bool has_role = super.hasRole(role, account);
	console.log("HAS_ROLE: %s", has_role);
	return has_role;
//	return true;
    }*/
