...
contract RQ is ERC721EnumerableDelegable, IERC721Receiver, IERC2981, ReentrancyGuard, Ownable {
  ...
  // ERC165 interface ID for gatedURI
  bytes4 private constant _INTERFACE_ID_GATED_URI = 0xaea86397;
  ...
  // Mapping for locking RQ tokens gatedURI changes
  mapping(uint256 => bool) private _lockedGatedURIs;
   ...
   */
  constructor(address DQContract_) ERC721("RQ-EVA", "RQ-EVA"){
    _DQContract = DQContract_;
    _dq = IDQ(DQContract_);
  }

  /**
  * @dev Requires that token exists
  */
  ...
  /**
   * @dev Gates access to only the token holder of `_tokenId`
   */
  modifier onlyHolder(uint256 _tokenId) {
    require(_msgSender() == ownerOf(_tokenId), "RQ: Not token holder");
    _;
  }
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165, ERC721EnumerableDelegable) returns (bool) {
    return interfaceId == type(ERC721EnumerableDelegable).interfaceId ||
      interfaceId == _INTERFACE_ID_ERC2981 ||
      interfaceId == _INTERFACE_ID_GATED_URI ||
   ...
   * delegate token.
   */
  function wrap(address _sourceContract, uint256 _sourceTokenId, bytes calldata _data) external virtual nonReentrant onlyWrappableToken(_sourceContract, _sourceTokenId) {
   ...
   */
  function getWrappedTimestamp(uint256 _tokenId) public view tokenExists(_tokenId) returns(uint256 wrappedTimestamp) {
  ...
  /**
   * @dev Returns a gated URI for the wrapped token only visible by the holder
   * @param _tokenId RQ NFT token to query
   * @return string value of the token gated URI
   */
  function gatedURI(uint256 _tokenId) public view onlyHolder(_tokenId) returns(string memory) {
    Source memory _sourceToken = _sources[_tokenId];

    if (
      ERC165Checker.supportsInterface(_sourceToken.sourceContract, _INTERFACE_ID_GATED_URI)
    ) {
      return IGatedURI(_sourceToken.sourceContract).gatedURI(_sourceToken.sourceTokenId);
    }

    return _wrappableTokens[_sourceToken.sourceContract][_sourceToken.sourceTokenId].gatedURI;
  }

  /**
   * @dev Change gatedURI for a given wrapped token
   * @param _tokenId RQ NFT token to query
   * @param _newGatedURI New RQ NFT token gatedURI
   */
  function setGatedURI(uint256 _tokenId, string memory _newGatedURI) public tokenExists(_tokenId) {
    Source memory _sourceToken = _sources[_tokenId];
    Wrappable memory _wrappableToken = _wrappableTokens[_sourceToken.sourceContract][_sourceToken.sourceTokenId];

    require(!_lockedGatedURIs[_tokenId], "RQ: Token gatedURI is locked");
    require(
      _msgSender() == _wrappableToken.tokenCreator ||
      _msgSender() == owner(),
      "RQ: Not token creator or contract owner"
    );

    _wrappableToken.gatedURI = _newGatedURI;
    _wrappableTokens[_sourceToken.sourceContract][_sourceToken.sourceTokenId] = _wrappableToken;
  }

  /**
   * @dev Lock/Unlock changes to gatedURI for a given wrapped token
   * @param _tokenId RQ NFT token to query
   * @param _lock Whether gatedURI should be locked or unlocked
   */
  function lockGatedURI(uint256 _tokenId, bool _lock) public tokenExists(_tokenId) onlyDelegate(_tokenId) {
    _lockedGatedURIs[_tokenId] = _lock;
  }
   ...
   * @param _tokenCreator address of the token creator
   * @param _gatedURI value of the gated URI
   */
  function addWrappableToken(address _contractAddress, uint256 _tokenId, address _tokenCreator, string memory _gatedURI) public onlyOwner {
  ...
  function setContractURI(string memory _newContractURI) public onlyOwner {
    _contractURI = Base64.encode(abi.encodePacked(_newContractURI));
  }
...
/**
* @title IGatedURI
* @dev Interface for gatedURI function used by RQDQ.
* @author zerodens (kanon.art)
*/
interface IGatedURI {
  function gatedURI(uint256 _tokenId) external view returns(string memory);
}
