...
interface IFliperPass {

    function setOwnerToken(address owner, uint256 tokenId, bool stake) external;
...
contract Staking is Ownable {
    ...
    */
    constructor(address _fliperpass, address _fliper) { 
        fliperpass = IFliperPass(_fliperpass);
        fliper = IFLIPER(_fliper);
    }
    ...
    */
    function staking(address account, uint16[] calldata tokenIds) external {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            fliperpass.setOwnerToken(account, tokenId, true);
        }
    }
    ...
    function claim(uint16[] calldata tokenIds) external {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (totalFliperEarned < MAXIMUM_GLOBAL_FLIPER) {
                ...
                owed = 0; // $FLIPER production stopped already
            } else {
                owed = (lastClaimTimestamp - stake.value) * DAILY_FLIPER_RATE / 1 days; // stop earning additional $WOOL if it's all been earned
            }

            fliperpass.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // send back Sheep
            delete stakes[tokenId];
            totalNFTStaked -= 1;
            emit TokenClaimed(tokenId, owed, false);

            fliperpass.setOwnerToken(_msgSender(), tokenIds[i], false);
        }
        if (owed == 0) return;
