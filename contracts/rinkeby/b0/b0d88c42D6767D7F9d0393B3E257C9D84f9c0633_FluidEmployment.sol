...
contract FluidEmployment {
    ...
    bool public isFired;
    bool public onStrike;
    uint public lastArbiterCall;

    constructor(ISuperfluid host, 
                address _employee,
                address _arbiter) payable {
    ...
    function addNewEmployee(address _newEmployee, ISuperfluidToken _token, int96 _flowRate) external {
        require(msg.sender == company);
        require(!isFired, "Can't add a new employee while still firing current one!");
        employee = _newEmployee;
        cfaV1.createFlow(employee, _token, _flowRate);
    }

    function modifyEmployeeFlow(ISuperfluidToken _token, int96 _flowRate) external {
        cfaV1.updateFlow(employee, _token, _flowRate);
    }

    function deleteEmployeeFlow(ISuperfluidToken _token) external {
        cfaV1.deleteFlow(address(this), employee, _token);
    }
}
