...
contract ExampleMainONFT is ONFT, Pausable, ReentrancyGuard {
    ...
    bool public reveal;
    mapping(uint => bool) public received;

    /// @notice Constructor for the ExampleMainONFT
    ...
    function assignNewONFT(IERC721 _erc721, uint startTokenId, uint endTokenId) external onlyOwner {
        for (uint i = startTokenId; i <= endTokenId; i++) {
            require(!received[i] , "ExampleONFT: NFT already received.");
            require(startMintId <= endMintId, "ExampleONFT: Max Mint limit reached.");
            received[i] = true;
            _safeMint(_erc721.ownerOf(i), startMintId++);
        }
    }
