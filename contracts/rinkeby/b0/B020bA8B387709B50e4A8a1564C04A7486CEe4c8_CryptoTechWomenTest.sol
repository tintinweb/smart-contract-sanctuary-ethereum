...
import "../CryptoTechWomen.sol";


contract CryptoTechWomenTest is CryptoTechWomen {
    ...
    constructor (
        ...
        _communityFundAddress,
        _bpsToDonationFund,
        _triggerIds
    ) { 
        maxTotalSupply = 50;
        amountForGiveAway = 10;
    }
...
contract CryptoTechWomen is ERC721, Ownable, ReentrancyGuard, Pausable {
    ...
    * @param _userId ID of the user in the whitelist
    ...
    */
    function publicSaleMint(uint256 _amount) external payable nonReentrant whenNotPaused {
        ...
        require(totalSupply + _amount <= maxTotalSupply_, "already sold out");
        _buyAndRefund(_amount, publicSalePrice);
        if (totalSupply == maxTotalSupply_) emit SoldOut();
    ...
    * IDs must be in the range of 8738 to 8887 (the last 150 tokens)
    * @param _accounts The list of addresses to mint tokens to
    * @param _ids The list of token IDs to mint
    */
    function giveaway(address[] memory _accounts) external onlyOwner {
        ...
        require(currentTotalSupply + _accounts.length <= maxTotalSupply, "maxTotalSupply exceeded");
        for (uint256 i; i < _accounts.length; i++) {
            _safeMint(_accounts[i], currentTotalSupply++);
            // currentTotalSupply++;
        }
        airdropCounter += uint16(_accounts.length);
        totalSupply = uint16(currentTotalSupply);
    ...
    */
    function _preSaleMint(uint256 _amount) private {
        ...
        require(totalSupply + _amount <= preSaleTokenLimit, "presale token limit exceeded");
        _buyAndRefund(_amount, preSalePrice);
    ...
    */
    function _buyAndRefund(uint256 _amount, uint256 _price) internal {
        ...
        for (uint256 i; i < _amount; i++) {
            _safeMint(_msgSender(), counter + i);
        }
        totalSupply += uint16(_amount);
        _checkMilestones(counter, _amount, _price);
    ...
    * @param _totalCost The total cost
    * @param _mintCounter The mint counter 
    */
