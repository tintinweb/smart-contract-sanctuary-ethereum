...
contract StatusPass is ERC721, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private m_TokenIdCounter;

    uint256 private constant MAX_SUPPLY = 1111;

    uint8[] private m_Tiers;

    uint256 private m_PreMintPrice = 0.1 ether; // 0.1Eth
    uint256 private m_PublicMintPrice = 0.15 ether; // 0.1Eth

    bool private m_IsReveal = false;

    uint256 private m_PreMaxMintPerAddress = 3;
    uint256 private m_PublicMaxMintPerAddress = 7;
    ...
    bytes32 public m_WhiteListMerkleRoot =
        0x2e35b61278fbcec3f3b0bb361d928e373e089a61758af09690ce0a5391078ff2;

    constructor() ERC721("Status Pass", "StatusPass") {}
    ...
    function totalSupply() public view returns (uint256) {
        ...
        if (m_IsReveal == false) return m_TokenIdCounter.current();

        for (uint256 i = 1; i <= m_Tiers.length; i++) {
    ...
    function preMint(uint256 _numberOfTokens, bytes32[] calldata _merkleProof)
        public
        payable
    {
        ...
        require(m_IsPublic == false);

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(_merkleProof, m_WhiteListMerkleRoot, leaf),
            "Invalid WhiteList."
        );

        require(
            balanceOf(msg.sender).add(_numberOfTokens) <=
                m_PreMaxMintPerAddress,
    ...
    function publicMint(uint256 _numberOfTokens) public payable {
        require(m_IsMintable, "must be active");
        require(m_IsPublic == true);

        require(_numberOfTokens > 0);

        require(
            balanceOf(msg.sender).add(_numberOfTokens) <=
                m_PublicMaxMintPerAddress,
            "Over Max Mint per Address"
        );
    ...
    function upgradeTier(uint8 _tierLevel, uint256[] memory _tier1Tokens)
        external
    {
        require(m_IsReveal == true);
        require(_tier1Tokens.length == (_tierLevel - 1) * 2);

        for (uint256 i = 0; i < _tier1Tokens.length; i++) {
            require(ownerOf(_tier1Tokens[i]) == msg.sender);
            require(m_Tiers[_tier1Tokens[i] - 1] == 1);
        }

        for (uint256 i = 0; i < _tier1Tokens.length; i++) {
            _burn(_tier1Tokens[i]);
        }

        m_Tiers.push(_tierLevel);
        _safeMint(msg.sender, m_Tiers.length);
    }
    ...
    function setPreMaxMintPerAddress(uint256 _preMaxMintPerAddress)
        external
        onlyOwner
    {
        m_PreMaxMintPerAddress = _preMaxMintPerAddress;
    }

    function getPreMaxMintPerAddress() external view returns (uint256) {
        return m_PreMaxMintPerAddress;
    }

    function setPublicMaxMintPerAddress(uint256 _publicMaxMintPerAddress)
        external
        onlyOwner
    {
        m_PublicMaxMintPerAddress = _publicMaxMintPerAddress;
    }

    function getPublicMaxMintPerAddress() external view returns (uint256) {
        return m_PublicMaxMintPerAddress;
    }
    ...
    function setRevealEnabled(bool _enabled) external onlyOwner {
        m_IsReveal = _enabled;
    }

    function getRevealEnabled() external view returns (bool) {
        return m_IsReveal;
    }

    function setTiers(uint256 _startIndex, uint8[] memory _tiers)
        external
        onlyOwner
    {
        if (m_Tiers.length < _startIndex - 1) {
            for (uint256 i = m_Tiers.length; i < (_startIndex - 1); i++)
                m_Tiers.push(1);
        }
        for (uint256 i = _startIndex; i < _startIndex + _tiers.length; i++) {
            if (m_Tiers.length >= i) {
                m_Tiers[i - 1] = _tiers[i - _startIndex];
            } else {
                m_Tiers.push(_tiers[i - 1]);
            }
        }
    }

    function getTierByTokenId(uint256 _tokenId) public view returns (uint256) {
        require(_exists(_tokenId));
        if (m_IsReveal == false) return 0;
        return m_Tiers[_tokenId - 1];
    }

    function setWhiteListMerkleRoot(bytes32 _rootHash) external onlyOwner {
        m_WhiteListMerkleRoot = _rootHash;
    }
}
