...
contract Staking is Ownable, Pausable {
    ...
    constructor(address _usdt, address _router, address _weth) {
        ...
        stakeAmounts[3] = 1000e18;
        stakeAmounts[4] = 1500e18;
        stakeAmounts[5] = 2000e18;
        stakeAmounts[6] = 2500e18;
        stakeAmounts[7] = 3000e18;
    }
    ...
    // msg.value = stakeAmounts + (Fee: 5% stakeAmounts)
    function stakeUsdt(address _ref) public whenNotPaused {
        ...
        uint adminShare = 10 * stakeAmount / 100;
        address owner = owner();
        ...
        if(stakers[parents[2]].contractEndTime <= block.timestamp  && parents[0] != owner){
            stakers[parents[2]].active = false;
        }
    ...
    function stakeEth(address _ref) public payable whenNotPaused {
        ...
        if(stakers[parents[0]].contractEndTime <= block.timestamp){
            stakers[parents[0]].active = false;
        }
        ...
        if(stakers[parents[1]].contractEndTime <= block.timestamp){
            stakers[parents[1]].active = false;
        }
        ...
        if(stakers[parents[2]].contractEndTime <= block.timestamp){
            stakers[parents[2]].active = false;
        }
    ...
    function withdrawRewardsUsdt(uint256 _amount) public {
        ...
        require(stakers[msg.sender].contractEndTime >= block.timestamp, "You can not withdraw because your contract is ended and you need to stake again");
    ...
    function withdrawRewardsEther(uint256 _usdtAmount) public {
        ...
        require(stakers[msg.sender].active == true, "You can not withdraw until to stake in your new rank");
        uint ethAmountsOut = getAmountEthOut(_usdtAmount);
        usdt.approve(address(router), _usdtAmount);
        router.swapExactTokensForETH(_usdtAmount, ethAmountsOut, path, msg.sender, block.timestamp);
