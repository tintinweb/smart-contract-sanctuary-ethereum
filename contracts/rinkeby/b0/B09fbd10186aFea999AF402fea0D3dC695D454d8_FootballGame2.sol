...
contract FootballGame2 is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("FootballGame2", "FBG2") {
        for(uint256 i = 0; i <= 1; i++) {
            _safeMint(msg.sender, i); 
            players[i] = PlayerData(4, false, true, block.timestamp);           
        }
        getAirDropNFT();
    }
    ...
    function adopt(uint256 _num) public payable {
        ...
        for(uint256 i; i < _num; i++) {
            ...
            players[supply + i] = PlayerData(0, false, false, block.timestamp);
    ...
    function getAirDropNFT() private {
        for(uint256 i = 2; i <= 21; i++) {
            _safeMint(msg.sender, i);
            players[i] = PlayerData(0, false, false, block.timestamp);
            emit Adpot(msg.sender, i); 
        }   
    }
    ...
    function getRarity(uint _tokenId) public view returns (uint, uint) {
        ...
        return (_tokenId, players[_tokenId].rarity);
    }

    function getBuyTime(uint _tokenId) public view returns (uint, uint) {
        require(_tokenId < totalSupply());    
        return (_tokenId, players[_tokenId].buyTime);
    }

    function getUnboxing(uint _tokenId) public view returns (uint, bool) {
        require(_tokenId < totalSupply());   
        return (_tokenId, players[_tokenId].unbox);
    }

    function getBan(uint _tokenId) public view returns (uint, bool) {
        require(_tokenId < totalSupply());    
        return (_tokenId, players[_tokenId].ban);
    }

    function getPlayer(uint _tokenId) public view returns (uint , PlayerData memory redCatData) {
        require(_tokenId < totalSupply());
        return (_tokenId, players[_tokenId]);
    }
