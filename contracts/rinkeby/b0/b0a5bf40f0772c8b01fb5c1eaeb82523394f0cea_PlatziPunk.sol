...
// File contracts/PlataziPunksDNA.sol
...
contract PlatziPunksDNA {
    ...
    // funcion que retorna un random number
    function deterministicPseudoRandomDNA(uint256 _tokenId, address _minter) public pure returns(uint256){

        uint256 combinateParams = _tokenId + uint160(_minter);
        bytes memory encodeParams = abi.encodePacked(combinateParams);
...
contract PlatziPunk is ERC721, ERC721Enumerable, PlatziPunksDNA {

    // Usamos la libreria
    using Counters for Counters.Counter;

    using Strings for uint256; // utiliza Strings para todos los uint256 sobrecargando sus funcionalidades

    // Definimos variables, mapas, etc
    Counters.Counter private _idCounter;
    uint256 public maxSupply;
    mapping(uint256 => uint256) public tokenDNA;

    constructor(uint256 _maxSupply) ERC721("PlatziPunks", "PLP"){
    ...
    function mint() public {
        ...
        // Validamos...si el actual llega a ser mayor a lo permitido, lanzamos error
        ...
        _idCounter.increment(); // incrementamos el contador en 1
    }
    ...
    function _paramsUri(uint256 _dna) internal view returns(string memory){
        ...
        /*
            tenemos que guardar dentro de un bloque entre {} la variable params, porque no tiene mas espacio en memoria.
            El error que da si no es este:
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory){
        ...
        // el tokenId debe existir
        require(
            _exists(tokenId),
            "ERC21 Metadata token not exist"
        );
        ...
        string memory jsonUri = Base64.encode(abi.encodePacked(
            '{"name": "PlatziPunls #',
            tokenId.toString(),
            '", "description": "Es una hermosa coleccion de NFT", "image": "',
            image,
            '"',
            '}'
        ));
