...
contract LitedexLeaderboard is LitedexRoles{
    using SafeMath for uint256;
    ...
    uint256 private startClaim;
    uint256 private endClaim;
    ...
    modifier onlyWhenClaimable{
        require(block.timestamp > startToClaimBNB(), "Litedex: Claim BNB is not started");
        require(block.timestamp < endToClaimBNB(), "Litedex: Claim BNB has over");
        _;
    }
    ...
    function startToClaimBNB() public view returns(uint256){
        return startClaim;
    }
    function endToClaimBNB() public view returns(uint256){
        return endClaim;
    }
    function setTimeClaimBNB(uint256 start, uint256 end) external onlyOwner returns(bool){
        startClaim = start;
        endClaim = end;
    }

    function claimBonus() external isClaimLocked isAlreadyClaim onlyWhenClaimable returns(bool){
        uint256 _bnbConverted = getBnbToConverted(msg.sender);

        require(address(this).balance > _bnbConverted, "Litede: Not Enough Balance");
        ILitedexInitalOffering(address(litedexico)).safeTransferBNB(msg.sender, _bnbConverted);
        return true;
    }
    //Function to recover stuck tokens in contract
    function recoverStuckToken(address to, address tokenAddress) external onlyOwner {
    ...
    //Function to recover stuck BNB in contract
    function recoverStuckBNB() external onlyOwner {
        return ILitedexInitalOffering(address(litedexico)).safeTransferBNB(owner, address(this).balance);
    }
}