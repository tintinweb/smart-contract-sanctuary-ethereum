...
contract NFTSwapAgent is Initializable,OwnableUpgradeable,ERC721HolderUpgradeable{
    ...
    function createSwapPair(string memory registerTxHash,address fromTokenAddr,uint256 fromChainId,string calldata baseURI_,string calldata tokenName,string calldata tokenSymbol) public onlyOwner {
        require(
    ...
    function swap(address tokenAddr,address recipient,uint256 tokenId,uint256 dstChainId) external payable {
        ...
        if (dstTokenAddr != address(0x0)) {
            ...
            emit BackwardSwapStarted(tokenAddr,msg.sender,recipient,dstChainId,tokenId,msg.value);
    ...
    function fill(string memory swapTxHash,address fromTokenAddr,address recipient,uint256 fromChainId,uint256 tokenId,string calldata tokenURI) public onlyOwner {
        ...
        if (mirroredTokenAddr != address(0x0)) {
            ...
            emit SwapFilled(swapTxHash,fromTokenAddr,recipient,mirroredTokenAddr,fromChainId,tokenId);
            return;
        }
        ...
        if (registeredToken[fromChainId][fromTokenAddr]) {
            ...
            emit BackwardSwapFilled(swapTxHash,fromTokenAddr,recipient,fromChainId,tokenId);
            return;
        }
