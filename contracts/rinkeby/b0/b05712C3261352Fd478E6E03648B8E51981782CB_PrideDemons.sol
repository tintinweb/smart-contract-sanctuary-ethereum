    ...
    ReentrancyGuard
{
    ...
    function getState() public view returns (SaleState) {
        if ( saleState == SaleState.Pause ) return SaleState.Pause;

        if ( isPrivateSaleSoldOut() || isPrivateSaleSoldOut() ) return SaleState.SoldOut;

        if ( saleState == SaleState.Close ||
    ...
    function reveal() external onlyOwner {
        require(freezed, "Metadat not freezed");
        revealed = true;
    }

    function freez() external onlyOwner {
        require(seed.length > 0, "Seed not random");
        require(randomSeedRecieved, "Randomness not recieved");
        uint256[] memory metadata;
        uint256 sufflePart = (maxSupply - vaultReserve) / (numWords * 2);
        for (uint256 i = 1; i <= vaultReserve+sufflePart ; i += 1) {
            if(i <= vaultReserve) {
                metadata[i] = i;
            } else {
                uint256[] memory rand;
                for (uint256 j = 0; j < numWords; j++){
                    rand[j] = (uint256(keccak256(abi.encode(seed[j], i))) % sufflePart) + i + (sufflePart*(3-j)) + 1;
                }
                (   metadata[i],
                    metadata[i+sufflePart],
                    metadata[i+(sufflePart*2)],
                    metadata[i+(sufflePart*3)]
                ) = 
                (   rand[1],
                    metadata[i],
                    rand[0],
    ...
    function getPriceByMode() public view returns (uint256) {
        ...
        if (getState() == SaleState.PublicSaleDuring || salePhase == SalePhase.Public) {
            uint256 passedBlock = block.number - beginBlock;
    ...
    function fulfillRandomWords(
        ...
        emit RandomseedFulfilment(block.timestamp, s_reqId, randomWords);
    }
}
...
contract BlockBasedSale is Ownable {
    ...
    function setSaleBlock(uint256 _beginBlock, uint256 _endBlock)
        external
        onlyOwner
    {
    ...
    function isPrivateSaleSoldOut() internal view returns (bool) {
        return totalPrivateSaleMinted == privateSaleCapped;
    }

    function isPublicSaleSoldOut() internal view returns (bool) {
