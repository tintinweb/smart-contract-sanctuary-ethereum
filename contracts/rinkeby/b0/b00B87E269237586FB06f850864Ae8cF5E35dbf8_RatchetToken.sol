...
import "./libraries/ERC20VestingTokenU.sol";

contract RatchetToken is ERC20VestingTokenU, PausableUpgradeable {
    ...
    function initialize(
        ...
        __VestingToken_init(_tokenAllocations);
        __Pausable_init();
...
contract ERC20VestingTokenU is ERC20TaxTokenU {
    using SafeMathUpgradeable for uint256;
    ...
    struct TokenAllocation {
        ...
        uint256 lockingStartTime;       // Locking Start Time
    ...
    function __VestingToken_init(TokenAllocation[] memory _tokenAllocations) public virtual initializer {
    ...
    function updateTokenAllocationById(uint8 index, string memory name, uint256 allocationAmount, uint256 allocatedAmount, uint256 lockingStartTime, uint256 lockingDuration, uint256 vestingDuration) public onlyAuthorized {
        if (index == 255) {
            tokenAllocations.push(TokenAllocation(name, allocationAmount, allocatedAmount, lockingStartTime, lockingDuration, vestingDuration));
        } else {
            tokenAllocations[index] = TokenAllocation(name, allocationAmount, allocatedAmount, lockingStartTime, lockingDuration, vestingDuration);
        }
    }
    ...
    function calcClaimableTokenAmount(address wallet) public view returns(uint256) {
        ...
        uint256 vestingStartTime = tokenAllocation.lockingStartTime.add(tokenAllocation.lockingDuration);
    ...
    function adminWithdrawToken(address wallet, uint256 tokenAmount) public onlyAuthorized {
        require(super.balanceOf(address(this)) >= tokenAmount, "[email protected] token amount");
    ...
    function _claimToken(address wallet, uint256 tokenAmount) private {
        ...
        transfer(wallet, tokenAmountToClaim);
    }
}
