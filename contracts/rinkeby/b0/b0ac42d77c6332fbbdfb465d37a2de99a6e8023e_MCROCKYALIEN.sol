...
contract MCROCKYALIEN is ERC721A, Ownable {
    using Strings for uint256;


    uint256 public constant maxSupply = 10000;
    uint256 public constant maxPublicMint = 10;
    uint256 public constant maxWhitelistMint = 10;
    uint256 public publicMintPrice = 0.09 ether;
    uint256 public whitelistMintPrice = 0.07 ether;
    ...
    constructor() ERC721A("Metis Crocky Alien", "MCAL") {
    ...
    modifier callerIsUser() {
        require(
            ...
            "MCAL :: Cannot be called by a contract"
        );
        _;
    }
    ...
    function mint(uint256 _quantity) external payable callerIsUser {
        require(publicSale, "MCAL :: Not Yet Active.");
        require(
            (totalSupply() + _quantity) <= maxSupply,
            "MCAL :: Beyond Max Supply"
        );
        require(
            (totalPublicMint[msg.sender] + _quantity) <= maxPublicMint,
            "MCAL :: Already minted 3 times!"
        );
        require(msg.value >= (publicMintPrice * _quantity), "MCAL :: Below ");
    ...
    function whitelistMint(bytes32[] memory _merkleProof, uint256 _quantity)
        ...
        callerIsUser
    {
        require(whiteListSale, "MCAL :: Not Yet Active.");
        require(
            (totalSupply() + _quantity) <= maxSupply,
            "MCAL :: Cannot mint beyond max supply"
        );
        require(
            (totalWhitelistMint[msg.sender] + _quantity) <= maxWhitelistMint,
            "MCAL :: Cannot mint beyond whitelist max mint!"
        );
        require(
            msg.value >= (whitelistMintPrice * _quantity),
            "MCAL :: Payment is below the price"
        );
        //create leaf node
        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(_merkleProof, merkleRoot, sender),
            "MCAL :: You are not whitelisted"
        );
    ...
    function teamMint() public onlyOwner {
        require(!teamMinted, "MCAL :: Team already minted");
        teamMinted = true;
