...
contract FactoryCloneContract is Ownable {
    ...
    function createDAO(DAO calldata _params)external returns(address) {
        bytes memory data = abi.encodeWithSignature("initializeERC20(string,string,uint256,uint256,uint256,uint256,uint256,bool,uint256,uint256,address,address,address)",_params.DaoName,_params.DaoSymbol,_params.totalRaiseAmount,_params.minDepositPerUser,_params.maxDepositPerUser,_params.ownerFeePerDeposit,_params.depositFunctioningDays,_params.feeUSDC,_params.quorum,_params.threshold,_params.daoOwnerAddress,USDCAddress,emitterAddress);
        address owner = owner();
        address proxy = address(new ProxyContract(ImplementationAddress,owner,data));
        Emitter(emitterAddress).createDao(_params.daoOwnerAddress, proxy, _params.DaoName, _params.DaoSymbol, _params.totalRaiseAmount, _params.minDepositPerUser, _params.maxDepositPerUser, _params.ownerFeePerDeposit, _params.depositFunctioningDays, _params.feeUSDC, _params.quorum, _params.threshold, emitterAddress);
        return proxy;
    }
