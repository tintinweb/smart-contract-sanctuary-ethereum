...
contract SamPunkA is ERC721A, Ownable{
    using Strings for string;
    uint16 public constant MAX_TOKENS = 10;
    uint16 public PRESALE_LIMIT = 10;
    ...
    uint16 public preSaleAddressLimit = 2;
    ...
    constructor() ERC721A("SamPunkA", "SPA")
    {

    }

    function mintToken(uint16 amount, bytes32[] memory proof) external payable{
        //PreSale - when PS true check < PS limit and <= PSALimit, when false just skip but sale must be active
        ...
        require(!preSaleIsActive || addressMintedBalance[msg.sender] + amount <= preSaleAddressLimit, "Max NFT per address exceeded");

        //Whitelist - when WL is true you need to verify proof, when false you skip this line
        ...
        require(msg.sender == tx.origin, "No transaction from smart contracts!");
         ...
         if (preSaleIsActive) {
            presaleTokensSold += amount; //Increase PreSale token sold count
            addressMintedBalance[msg.sender] += amount; //Increase address count 
        }
