...
contract PlatformToken is ERC20, Ownable {
    ...
    uint256 public totalFeesForSelling = nativeRewardsFeeForSelling
            .add(projectFeeForSelling)
            .add(liquidityFeeForSelling);
    uint256 public totalFeesForBuying = nativeRewardsFeeForBuying
            .add(projectFeeForBuying)
            .add(liquidityFeeForBuying);
    uint256 public totalFeesForTransfering = nativeRewardsFeeForTransfering
    ...
    modifier ownerOrMinter() {
        require(owner() == _msgSender() || minter == _msgSender(), "caller is not the owner or the minter");
        _;
    }
    ...
    function airdropMint(address _to, uint256 _amount) external onlyMinter {
        _mint(_to, _amount);
    }
