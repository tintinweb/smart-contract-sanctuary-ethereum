...
// File: contracts/DevFierceCrowd.sol
...
contract DevFierceCrowd is ERC721, ERC721URIStorage, ERC2981, Pausable, Ownable, RandomlyAssigned {
    
    string baseURI = "";
    string public contractURI = "ipfs://QmPnrxvrVB4hoN6Bo84uFfgABeB4G4YR3mvxNro4iJowcv";
    string public baseExtension = ".json";
    string public hiddenExtension = ".mp4";
    uint256 public maxMintAmount = 1500;
    uint16 maxSupply = 100;//real 1500 for test 100
    ...
    uint96 royaltyFeesInBips = 800;//8%
    ...
    function safeMint(uint8 _amount) public payable whenNotPaused{
        require( _amount > 0, "Need to mint at least 1 NFT");
        require( _amount <= maxMintAmount, "Max mint amount per session exceeded");
        require( tokenCount() + _amount <= totalSupply(), "Max NFT limit exceeded");
        require( availableTokenCount() - _amount >= 0, "Max NFT limit exceeded");
        ...
        if (msg.sender != owner()) {
            if(onlyWhitelisted == true) {
                ...
                require(msg.value >= whitelistCost * _amount, "Whitelist insufficient funds");
            }
            else{
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721, ERC721URIStorage) returns (string memory){
        ...
        if(revealed) {
            ...
            return string(abi.encodePacked(currentBaseURI, "hidden", hiddenExtension));
        }
    }
    ...
    function setHiddenExtension(string memory _newHiddenExtension) public onlyOwner {
        hiddenExtension = _newHiddenExtension;
    }
