...
contract SAU is  ERC721A, Ownable {
    ...
    uint256 public preSalePrice = 0 ; //in wei
    uint256 public mainSalePrice = 0 ; // in wei
    uint256 public maxSupplyHero = 2750; //For Sale
    uint256 public maxSupplyVillain = 2750; //For Sale
    ...
    uint256 public genesisApe = 1;
    uint256 public maxSupplyCollection = maxSupplyHero + maxSupplyVillain + genesisApe + teamReserve;
    uint256 public maxMintPerTnx = 5;
    uint256 public nftPerWalletPreSale;
    uint256 public nftPerWalletMainSale;
    ...
    mapping(address => bool) private heroWhiteList;
    mapping(address => bool) private villainWhiteList;
    ...
    constructor(
    uint256 _nftPerWalletPreSale,
    uint256 _nftPerWalletMainSale,
    string memory initBaseURI,
    string memory initNotRevealedUri
    ) ERC721A ("Super Ape University", "SAU", _nftPerWalletPreSale, _nftPerWalletMainSale) {
    nftPerWalletPreSale = _nftPerWalletPreSale;
    nftPerWalletMainSale = _nftPerWalletMainSale;
    baseURI = initBaseURI;
    ...
    function presaleMintHero(uint256 _unitSize) external payable preSaleIsLive{
        ...
        require(heroWhiteList[msg.sender], "Your wallet is not in Whitelist");
        require(totalSupply() <= maxSupplyCollection, "Collection Sold out");
        require(heroMintCount + _unitSize <= maxSupplyHero, "No Tokens available to Mint in Hero");
        require(totalSupply() + _unitSize <= maxSupplyCollection, "Minting would exceed max supply.");
        require(_unitSize <= maxMintPerTnx, "Please select the allowed mint per Tnx"  );
         require(minted + _unitSize <= nftPerWalletPreSale, "Mint Over Max Allocation");
    ...
    function presaleMintVIllain(uint256 _unitSize) external payable preSaleIsLive{
        ...
        require(tx.origin == msg.sender, "contracts not allowed");
         require(totalSupply() < maxSupplyCollection, "Collection Sold out");
        require(villainWhiteList[msg.sender], "You are not in Whitelistd");
        require(villainMintCount + _unitSize <= maxSupplyVillain, "No Tokens available to Mint in Villain");
        require(totalSupply() + _unitSize <= maxSupplyCollection, "Minting would exceed max supply.");
        require(_unitSize <= maxMintPerTnx, "Please select the allowed mint per Tnx"  );
        require(minted + _unitSize <= nftPerWalletPreSale, "mMint Over Max Allocation");
        require(msg.value >= _unitSize * preSalePrice,"Try to send more ETH");
    ...
    function mainsaleMintHero(uint256 _unitSize) external payable mintIsLive{
        ...
        require(totalSupply() + _unitSize <= maxSupplyCollection, "Minting would exceed max supply.");
        require(_unitSize <= maxMintPerTnx, "Please select the allowed mint per Tnx"  );
        require(minted + _unitSize <= nftPerWalletMainSale, "Mint Over Max Allocation");
    ...
    function airdropHero(address _to, uint256 _unitSize) external onlyOwner {
        ...
        _safeMint(_to, _unitSize);
    }

    function airdropVillain(address _to, uint256 _unitSize) external onlyOwner {
    ...
    function setMintPerTnx(uint256 _newMintPerTnx) external onlyOwner {
        maxMintPerTnx = _newMintPerTnx;
    }

    function setNftPerWalletPreSale(uint256 _nftPerWalletPreSale) external onlyOwner
    {
        nftPerWalletPreSale = _nftPerWalletPreSale;
    }

    function setNftPerWalletMainSale(uint256 _nftPerWalletMainSale) external onlyOwner
    {
        nftPerWalletMainSale = _nftPerWalletMainSale;
    }

    function setPreSalePriceWei(uint256 _newPreSalePrice) external onlyOwner {
        preSalePrice = _newPreSalePrice;
    }

    function setVillainSupply(uint256 _newMaxSupplyVillain) external onlyOwner {
        maxSupplyVillain = _newMaxSupplyVillain;
    }

    function setHeroSupply(uint256 _newMaxSupplyHero) external onlyOwner {
        maxSupplyHero = _newMaxSupplyHero;
    }

    function setTeamReserveHero(uint256 _newTeamReserveHero) external onlyOwner {
        teamReserveHero = _newTeamReserveHero;
    }

    function setTeamReserveVillain(uint256 _newTeamReserveVillain) external onlyOwner {
        teamReserveVillain = _newTeamReserveVillain;
    }

    function setMainSalePriceWei(uint256 _newMainSalePrice) external onlyOwner {
        mainSalePrice = _newMainSalePrice;
    }

    function addToHeroWhiteList(address[] calldata addresses) external onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++)
            heroWhiteList[addresses[i]] = true;
    }

    function addToVillainWhiteList(address[] calldata addresses) external onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++)
            villainWhiteList[addresses[i]] = true;
    }

    function enableMintLive() external onlyOwner {
        if (mintLive) {
            mintLive = false;
            return;
        }
     ...
     function enablePreSaleLive() external onlyOwner {
        if (preSaleLive) {
            preSaleLive = false;
            return;
        }
    ...
    function isWhiteListedHero(address _address) public view returns (bool){
        return heroWhiteList[_address];
    }

    function isWhiteListedVillain(address _address) public view returns (bool){
        return villainWhiteList[_address];
    }
