...
contract MartekCrew is ERC721Enumerable, Ownable {
    ...
    // Maximum amount of MartekCrew in existance. 
    uint256 public MAX_SUPPLY;
    uint256 public cost = 0.02 ether;
    uint256 public presale_cost = 0.01 ether;
    ...
    event MartekBought (address buyer, uint256 amount);
    ...
    constructor(
        ...
        // Mint 30 MartekCrew for airdrop and gift purposes
    }
    ...
    function getCost(bool isPublic) public view returns (uint256) {
        if(isPublic) {
            return cost;
        } else {
            return presale_cost;
    ...
    // }
    /**
    * @dev Mints yourself a Martek. Or more.
    */
    function mint(uint256 numberofMartek) public payable {
        ...
        if(!publicSale) {
            require(whitelisted[msg.sender], "Exception 2: The signer should be whitelisted.");
        }
        require(totalSupply() < MAX_SUPPLY, "Exception 3: Sale has already ended.");
        require(numberofMartek > 0, "Exception 4: You cannot mint 0 MartekCrew NFT.");
        require(SafeMath.add(totalSupply(), numberofMartek) <= MAX_SUPPLY, "Exception 5: Exceeds maximum MartekCrew supply. Please try to mint less MartekCrew NFT.");  
        require(tokenBalance[msg.sender] < maxMintAmount, "Exception 6: Reached the limit for each user. You can't mint no more");      
        require(SafeMath.add(tokenBalance[msg.sender], numberofMartek) <= maxMintAmount, "Exception 7: Exceeds limit for each user. Please try to mint less MartekCrew NFT.");
        
        // require(token.transferFrom(msg.sender, artist, getNFTPrice(numberofMartek)), "token transfer failed");        
        
        for(uint256 i=0; i<numberofMartek; i++) {
            _safeMint(msg.sender, totalSupply());
            tokenBalance[msg.sender] = SafeMath.add(tokenBalance[msg.sender], 1);
        }
        emit MartekBought(msg.sender, numberofMartek);
    }
    ...
    */
        ...
        returns (string memory)
    {
        ...
        return bytes(currentBaseURI).length > 0
            ? string(abi.encodePacked(currentBaseURI, tokenCID[tokenId]))
            : "";
    }
    ...
    function setCID(uint256 _tokenId, string memory _cid) public {
