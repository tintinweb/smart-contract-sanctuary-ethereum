...
contract Lock is BaseGovernanceWithUserUpgradable, ERC721EnumerableUpgradeable {
    ...
    event Claimed(uint256 indexed nftId, address indexed bebficiary, uint256 amount);
    ...
    function getInfoBySingleID(uint256 id) view external returns (address owner, uint256 lockedAmount, uint256 claimable, uint256 startLock, uint256 endLock){
        owner = ownerOf(id);
        (, uint256 _startLock, uint256 initAmount, uint256 claimed) = depositManager.getProperties(id);
    ...
    function removeBeneficiaries(bytes calldata data) external onlyBeneficiaryManager {
        ...
        for (uint256 i = 0; i < IDs.length; i++) {
            (address beneficiary, uint256 additionTime, uint256 initialAmount, uint256 claimed) =  depositManager.getProperties(IDs[i]);
            uint256 unlockedAmount = schedule.unlockedAmount(additionTime, initialAmount);
            unlockedAmount -= claimed;
            require(unlockedAmount != 0, "ERROR: Not enought free tokens - Schedule");
            depositManager.removeDeposits(IDs[i]);
            tokenERC20.safeTransfer(beneficiary, unlockedAmount);
