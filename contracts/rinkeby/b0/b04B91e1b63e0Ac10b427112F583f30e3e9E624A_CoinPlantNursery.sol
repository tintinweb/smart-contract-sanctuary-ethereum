    ...
    ReentrancyGuard
{
    ...
    IERC721 public CoinPlant;
    IERC20 public CoinPollen;

    address pollenOwnerAddress = 0x36947bDaDeaAfe55Ae6B569b5ae8d851bc294201;
    uint256 public constant BASE_RATE = 10;
    uint256 public constant GENESIS_BONUS = 2;
    uint256 public constant MAX_PER_TX = 25;
    uint256 public constant PERIOD = 10 minutes;
    uint256 public constant COIN_PLANT_MAX_SUPPLY = 2500;
    uint256 public constant COIN_PLANT_GENESIS_SUPPLY = 95;
    ...
    constructor(address _nft, address _token) {
        CoinPlant = IERC721(_nft);
        CoinPollen = IERC20(_token);
    }

    /**
     * @notice Get the owner of a specified CoinPlant
     * @param tokenId CoinPlant to return the owner of
    ...
    /**
     * @notice Get list of CoinPlant tokens by account
     * @param account Address of the CoinPlant owner
     * @return uint256[] The CoinPlant tokens owned by the specified account
     */
    function getAllOwned(address account)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory indexMap = new uint256[](COIN_PLANT_MAX_SUPPLY);

        uint256 index;
        for (uint256 tokenId; tokenId < COIN_PLANT_MAX_SUPPLY; ++tokenId) {
    ...
    /**
     * @notice Get amount of spendable $POLLEN
     * @param account Address to return spendable $POLLEN for
     * @return uint256 Amount of spendable $POLLEN
     */
    function getSpendable(address account) public view returns (uint256) {
     ...
     * @param account Address to return the pending $POLLEN rewards of
    ...
    function _getPending(address account) internal view returns (uint256) {
        ...
        return
            (_accountInfo.shares *
                BASE_RATE *
                (block.timestamp - _accountInfo.lastUpdate)) / PERIOD;
    }

    /**
     * @notice Get $POLLEN stash of a specified account
     * @param account Address of the account to get the $POLLEN stash of
     */
    ...
    /**
     * @notice Move pending $POLLEN rewards to account stash and reset the timer
    ...
    /**
     * @notice Withdraw available $POLLEN
     */
     ...
     * @param tokenIds CoinPlant tokens to stake
     */
    function stake(uint256[] memory tokenIds)
        ...
        nonReentrant
    {
        if (tokenIds.length == 0 || tokenIds.length > MAX_PER_TX)
            revert Scales_InvalidTokenAmount();
        ...
        for (uint256 i; i < tokenIds.length; ++i) {
            ...
            CoinPlant.safeTransferFrom(_msgSender(), address(this), tokenId);
            tokenOwners[tokenId] = _msgSender();

            if (tokenId <= COIN_PLANT_GENESIS_SUPPLY) ++genesisCount;

            emit Stake(tokenId, _msgSender());
        }

        accountInfo[_msgSender()].shares += uint16(
            (tokenIds.length - genesisCount) + (genesisCount * GENESIS_BONUS)
        );
    }

    /**
     * @notice Unstake CoinPlant tokens
     * @param tokenIds CoinPlant tokens to unstake
     */
    function unstake(uint256[] calldata tokenIds) public nonReentrant {
        ...
        for (uint256 i; i < tokenIds.length; ++i) {
            ...
            if (tokenOwners[tokenId] != _msgSender())
                revert Scales_SenderNotTokenOwner();

            CoinPlant.safeTransferFrom(address(this), _msgSender(), tokenId);
     ...
     * @param to Account to send the CoinPlant to
     * @param tokenId CoinPlant to recover
     */
    function recoveryTransfer(address to, uint256 tokenId) external onlyOwner {
        if (tokenOwners[tokenId] != address(0))
            revert Scales_TokenNotStranded();

        CoinPlant.safeTransferFrom(address(this), to, tokenId);
    }
