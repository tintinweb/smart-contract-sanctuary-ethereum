...
contract Graveyard_v3 is Initializable, ERC1155Upgradeable, OwnableUpgradeable, PausableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable {
    ...
    function getSupplyAmount() external view virtual returns (uint256) {
    ...
    function getMaxMintAmount(address _user) public view virtual returns (uint256) {
        ...
        require(mintedIds.length + count <= maxValue);
    ...
    function uri(uint256 tokenId) public view override virtual returns (string memory){
        if(revealed == true){
            ...
            return string(abi.encodePacked(revealURI, Strings.toString(tokenId)));
        }
    }
    ...
    function setRevealUrl(string memory newuri) external onlyOwner {
        revealURI = newuri;
    }
