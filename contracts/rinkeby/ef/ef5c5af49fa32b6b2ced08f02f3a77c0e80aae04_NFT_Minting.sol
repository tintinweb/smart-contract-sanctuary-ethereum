...
contract NFT_Minting is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 _amount) external payable nonReentrant{
        ...
        require(balanceOf(msg.sender).add(_amount) <= maxMint, "NFT: You can't mint so much tokens");
        if(msg.sender != owner()){
    ...
    function updateMaxMint(uint _maxMint) external onlyOwner {
        maxMint = _maxMint;
    }
