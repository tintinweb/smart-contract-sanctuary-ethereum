...
// listing price change it usdt 
  ...
  function setTokenAddress(address _tokenaddress) public payable{
  ...
  function ApprovalAmount(uint256 _tokenamount) public view returns(uint256){
    return _tokenamount;
  }
  function Approvetokens(uint256 _tokenamount) public returns(bool){
       bool _success = tokenAddress.approve(address(this), _tokenamount);
       return _success;
  }
function applyForVerification(uint _amount) public {
    //require(msg.sender != admin, "admin can't apply verification");
    console.log(tokenAddress.balanceOf(msg.sender));
    bool _success = tokenAddress.approve(address(this), _amount);
    if (_success) {
        console.log(tokenAddress.allowance(msg.sender, address(this)));
  ...
  */
  ...
  function callMarketItem(uint256 itemId) public view returns(uint) {
    return idToMarketItem[itemId].saleCount;
  }
...
//line 311 
    ...
    {
      tokenAddress.transferFrom(address(this),msg.sender,price); 
    }
    if(idToMarketItem[itemId].saleCount==2)
    {
    tokenAddress.transferFrom( address(this),owners[itemId][0],(price*royalties/100*royalty_firstOwner/100));
    tokenAddress.transferFrom( address(this),owners[itemId][idToMarketItem[itemId].saleCount-1],(price*royalties/100*(100-royalty_firstOwner)/100));
    tokenAddress.transferFrom( address(this),owners[itemId][idToMarketItem[itemId].saleCount-1],(price*(100-royalties)/100));
    ...
    if(idToMarketItem[itemId].saleCount>2){
      ...
      for (uint i=1;i< idToMarketItem[itemId].saleCount-1; i++){ //for all indermediaries, commission =  (msg.value*r*b/i)
        tokenAddress.transferFrom(address(this),owners[itemId][i],(price*royalties/100*(100-royalty_firstOwner-royalty_lastOwner)/100)/(idToMarketItem[itemId].saleCount-2));
   //     owners[itemId][i].transfer((price*royalties/100*(100-royalty_firstOwner-royalty_lastOwner)/100)/(idToMarketItem[itemId].saleCount-2));
      ...
      tokenAddress.transferFrom( address(this),owners[itemId][idToMarketItem[itemId].saleCount-1],(price*royalties/100*royalty_lastOwner/100));
      tokenAddress.transferFrom( address(this),owners[itemId][idToMarketItem[itemId].saleCount-1],(price*(100-royalties)/100));
    ...
    idToMarketItem[itemId].isAuctionable = isAuctionSale;
  ...
  function resellItem(
    ...
    idToMarketItem[itemId].isAuctionable = isAuctionItem;
  ...
  */
  ...
  function fetchAuctionItemsDetails(uint256 itemId) public view returns (Auction memory) {
