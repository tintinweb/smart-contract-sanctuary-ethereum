...
contract A is ReentrancyGuard, AdminControl, ERC721, IA {

    using Strings for uint256;

    address private immutable _erc1155BurnAddress;

    // Pass count
    uint256 private _passCount;
    // Mapping of levels to number of items needed to burn
    mapping(uint16 => uint256) private _levels;
    // Mapping of token to current level
    mapping(uint256 => uint16) private _tokenLevel;
    // Merge multiplier
    uint8 private _mergeMultiplier;
    ...
    constructor(address erc1155BurnAddress) ERC721("A", "A") {
     ...
     * @dev Mint a pass
     */
    function _mintPass(address recipient, uint16 level) private {
        _passCount++;
     ...
     * @dev Upgrade a pass
     */
    function _upgradePass(uint256 tokenId, uint16 level) private {
        _tokenLevel[tokenId] = level;
        emit TokenLevel(tokenId, level);
    }

    /**
     * @dev See {IA0k1-enableRedemption}.
     */
    ...
    /**
     * @dev See {IA0k1-disableRedemption}.
     */
    ...
    function _onERC1155Received(address from, uint256 id, uint256 value, bytes calldata data) private {
        ...
        uint256 tokenId;
        if (data.length == 64) {
            (action, level) = abi.decode(data, (uint8, uint16));
            require(_levels[level] > 0 && _levels[level] == value, "Invalid request");
        } else if (data.length == 96) {
            (action, level, tokenId) = abi.decode(data, (uint8, uint16, uint256));
            require(_levels[level] > 0 && (_levels[level] - _levels[_tokenLevel[tokenId]]) == value, "Invalid request");
        } else {
            revert("Invalid data");
        }
    ...
    function onERC721Received(
        ...
        require(msg.sender == address(this), "Invalid NFT");
        
        uint256 tokenId;
        if (data.length == 32) {
            (tokenId) = abi.decode(data, (uint256));
            require(_tokenLevel[tokenId] > 0, "Invalid request");
        } else {
            revert("Invalid data");
        }

        require(_tokenLevel[tokenId] == _tokenLevel[receivedTokenId], "Can only merge two passes of the same level");
    ...
    /**
     * @dev See {IA0k1-tokenLevel}.
     */
    function tokenLevel(uint256 tokenId) public view override returns(uint16) {
        require(_exists(tokenId), "ERC721: URI query for nonexistent token");
        return _tokenLevel[tokenId];
    }

    /**
     * @dev See {IA0k1-setPrefixURI}.
     */
 ...
 * A0k1 Interface
 */
interface IA is IERC1155Receiver, IERC721Receiver {

    event Activate();
    event Deactivate();
    event TokenLevel(uint256 tokenId, uint16 level);
     ...
     */
    function tokenLevel(uint256 tokenId) external view returns(uint16);
