...
contract LuckyRooTest is IERC20, Ownable {
	...
	string private constant _name =  "LUCKYROOTEST"; 
	string private constant _symbol = "LUCKYROOTEST";
	...
	constructor() {
		...
		IRouter _uniswapV2Router = IRouter(0x118dFC34100aE8a0B79a5D67F808C0F75f548a75);
	...
	function _swapAndLiquify() private {
		...
		uint256 amountBSCBuyBack = BSCBalanceAfterSwap * buyBackFeePrior / totalBSCFee;
		uint256 amountBSCAirdrop = BSCBalanceAfterSwap - (amountBSCLiquidity + amountBSCMarketing + amountBSCBuyBack);

		payable(marketingWallet).transfer(amountBSCMarketing);
		payable(buyBackWallet).transfer(amountBSCBuyBack);
        payable(airdropWallet).transfer(amountBSCAirdrop);
