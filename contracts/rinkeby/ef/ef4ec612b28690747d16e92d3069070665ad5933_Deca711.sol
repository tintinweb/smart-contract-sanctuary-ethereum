...
contract Deca711 is VRFConsumerBaseV2, Pausable, Ownable, ReentrancyGuard {
    ...
    uint32 numWords = 1;
    uint256 housePoolDepositFeePercent = 5;
    uint256 maxBetPercentage = 1;
    uint256 minBet = 1;
    uint256 betAmount = 0;
    ...
    function getMaxBetPercentage() public view returns (uint256){
...
//        require(_betAmount <= maxBetPercentage / 100 * housePoolBalance.depositAmount, "maxBet");
    ...
    function fulfillRandomWords(
        ...
        uint256 payoutAppliedAmount = payout / 100 * betAmount;
        if (userWon) {
            _transfer(USDC_ADDRESS, msg.sender, payoutAppliedAmount);
            for (uint256 i = 0; i < userHouseBalances.length; i++) {
                uint256 userContribution = userHouseBalances[i].depositAmount / housePoolBalance.depositAmount;
                userHouseBalances[i].depositAmount -= (payoutAppliedAmount - betAmount + calculatedFee) * userContribution;
            }
            housePoolBalance.depositAmount -= payoutAppliedAmount - betAmount + calculatedFee;
        } else {
            for (uint256 i = 0; i < userHouseBalances.length; i++) {
                uint256 userContribution = userHouseBalances[i].depositAmount / housePoolBalance.depositAmount;
                userHouseBalances[i].depositAmount += (betAmount - calculatedFee) * userContribution;
            }
