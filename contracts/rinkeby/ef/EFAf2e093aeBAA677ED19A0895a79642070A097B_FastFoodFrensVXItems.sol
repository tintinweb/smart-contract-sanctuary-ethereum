...
abstract contract ERC1155Factory is ERC1155, Controllable {
  ...
  function addItem(
    ...
    Item memory item = Item(itemUri, itemName, uint32(items.length), 0, uint32(maxSupply), uint32(price), isPaused);
    items.push(item);
  }
  ...
  function addItems(
    ...
    if (
      (itemUri.length != itemName.length) ||
      (itemUri.length != maxSupply.length) ||
      (itemUri.length != price.length) ||
      (itemUri.length != isPaused.length)
    ) revert ERC1155_MismatchingArraysLength();
    for (uint256 i; i < itemUri.length; ++i) {
      addItem(itemUri[i], itemName[i], maxSupply[i], price[i], isPaused[i]);
    }
  }
  ...
  /// @notice edit item.name
  function editItem_name(uint256 id, string memory newName) external onlyOwner {
    if (!exists(id)) revert ERC1155_Error404();
