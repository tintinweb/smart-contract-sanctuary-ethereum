...
contract SevenHundred is ERC721A, Ownable {
    ...
    uint256 private amountOfInactiveUsers;
    ...
    mapping(address => uint256) private _nonUnique;
    ...
    function setUsersInactive(address[] memory targets) external onlyOwner {
    ...
    function viewAmountInactive() external view returns (uint256) {
        return amountOfInactiveUsers;
    }

    function removeInactiveUsers(address[] memory targets) external onlyOwner {
        for (uint256 i = 0; i < targets.length; i++) {
    ...
    function airdropRewardNonUnique() external onlyOwner {
        require(
        ...
        for (uint256 i = 0; i < ts; i++) {
            if (!_isIncluded[ownerOf(i)]) {
                _nonUnique[ownerOf(i)] = balanceOf(ownerOf(i));
                _isIncluded[ownerOf(i)] = true;
                payable(ownerOf(i)).transfer(
                    allocation * _nonUnique[ownerOf(i)]
                );
            }
        }
        for (uint256 j = 0; j < ts; j++) {
            if (_isIncluded[ownerOf(j)]) {
                _isIncluded[ownerOf(j)] = false;
            }
        }
    }

    function airdropReward() external onlyOwner {
        require(
            block.timestamp - rewardsSentOutRecent > timeRestriction,
            "Not Allowed To Send"
        );
        ...
        for (uint256 i = 0; i < ts; i++) {
            if (!_inactiveList[ownerOf(i)] && ownerOf(i) != address(0)) {
