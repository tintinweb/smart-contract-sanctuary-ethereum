...
contract Ovr is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public PRICE = 0.142 ether;
    ...
    constructor(bytes32 _wlRoot, address _signer) ERC721A("Ovr", "OVR") {
        wlRoot = _wlRoot;
        signer = _signer;
        _safeMint(PROJECT_ADDRESS, 50);
    }
    ...
    function allowMint(bytes32[] memory proof, uint256 numberOfTokens) external isSecured(1) payable{
        ...
        require(totalSupply() + 1 <= MAX_SUPPLY,"EXCEED_SUPPLY");
    ...
    function publicMint(uint64 expireTime, bytes memory sig, uint256 numberOfTokens) external isSecured(2) payable {
        ...
        require(numberMinted(msg.sender) + numberOfTokens <= MINT_LIMIT,"ONLY_10_IS_ALLOWED");
