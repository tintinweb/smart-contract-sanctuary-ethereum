...
constructor(address feeCollector, uint256 marketFeeCoin, uint256 marketFeeCENT) NFTM(feeCollector, marketFeeCENT) {
    ...
    function cancelSaleItem(address tokenAddr, uint256 tokenId) public nonReentrant {
    ...
    // function executeSaleItem(
    //     address tokenAddr,
    //     uint256 tokenId,
    //     bool useCoin
    // ) public payable nonReentrant{

    //     _executeSaleItem(tokenAddr,tokenId,useCoin);
...
contract UnloxToken is Ownable, ERC20Burnable, ERC20Pausable {


    constructor() public ERC20("Lox", "LOX") {
    ...
    function issueBonusCoin(address receiverAddr, uint256 amount) public onlyOwner {
        assert(amount > 0);
        _mint(receiverAddr, amount);
    }

    function useCoinFrom(address account, uint256 amount) public onlyOwner {

        //TODO: test negative value
...
abstract contract NFTMTokenMarketFee is NFTM {
    ...
    function buyNFTWithLox(
        ...
        _executeSaleItem(tokenAddr,tokenId,item.price,item.seller,item.creator,item.creatorFee,0);
...
abstract contract NFTMPromotion is NFTM {
    ...
    //TODO: Optimize memory for Promoter struct
    mapping(string => SellPromoter) private _sellPromoters;
    struct SellPromoter {
        address payable promoter;
        uint256 promoterCoin; // number of bonus coin
        uint256 sellerCoin;
    }
    ...
    struct BuyPromoter {
        address payable promoter;
        uint256 promoterFeeCENT; // 250 = 2.5% , 2500 = 25%
        uint256 promoterCoin; // number of bonus coin
        uint256 buyerCoin;
    }
    ...
    function addSellPromoter (string memory promoCode, address payable promoter, uint256 promoterCoin, uint256 sellerCoin)
        public onlyOwner
    {
        //TODO: make sure that promoter is not null and the fees are correct
    ...
    function removeSellPromoter (string memory promoCode)
        public onlyOwner
    {
        //TODO: check what is the new value  is it null or empty?
    ...
    {
        // return _promoterToJSON(_promoters[promoCode]);
    ...
    function createSaleItemWithPromoCode(
        ...
        // TODO: check that approve for all has been called

        _createSaleItemWithCreatorFee(tokenAddr, tokenId, msg.sender, price, creator, creatorFee);

        //TODO: Check address is not zero and is human
    ...
    function buyNFTWithPromoCode(
        ...
        //TODO: Calculate new income for promoter, market, buyer
        // promoter get more money an dtoken
        // market reduce money 
        // buyer get bonus coin
        // seller gets the same value

        

        BuyPromoter memory promotion = _buyPromoters[promoCode];

        //TODO: write test and uncomment this line
        // require(promotion.promoter != address(0), "Promoter address cannot be the zero address"); 
        require(promotion.promoterFeeCENT < getMarketFeeCENT(),"Promoter fee can not be more than the market fee");

        SaleItem memory item = _nftSaleItems[tokenAddr][tokenId];

        // calculate promoter's fee in CENT 
        // prepare the leftover market fee to the market fee holder account
        ...
        // issue bonus coins to promoter and buyer
...
abstract contract NFTMBrowsable is NFTM {
    ...
    function getSaleItemDetails(address tokenAddr, uint256 tokenId)
        ...
        returns (string memory)
    {
        //TODO: handle a case where the requested item is not on the market
...
abstract contract NFTM is ReentrancyGuard, Ownable{
    ...
    //TODO: Getter/setter ownable
    
    uint256 private _marketFeeCENT; 
    ...
    //TODO: Implement ways to get money out for seller, creator and the market
    mapping(address => uint256) internal _credits;

    //TODO: Optimize memory management of the SaleItem Struct
    struct SaleItem {
        address tokenAddr;
    ...
    //TODO: apply reentrancy guard to necessary functions    
    ...
    function _createSaleItemWithCreatorFee(
        ...
        //TODO: check that this market has the right to sell this item

        // add item into the _nftCollections
        ...
        // add item into _nftIDs
        uint256[] memory tmpIDs = _nftIDs[tokenAddr];

        bool isIDExist = false;
        for(uint i = 0 ; i < tmpIDs.length ; i++)
        {
            if(tmpIDs[i] == tokenId)
            {
                isIDExist = true;
        ...
        // add item into _sellerSaleItems
        _sellerSaleItems[msg.sender].push(_nftSaleItems[tokenAddr][tokenId]);

        emit SaleStatusChange(tokenAddr, tokenId, price);
    }

    function _removeItemFromMarket(address tokenAddr, uint256 tokenId) internal {

        // mapping(address => mapping(uint256 => SaleItem)) private _nftSaleItems;
        delete _nftSaleItems[tokenAddr][tokenId];

        // mapping(address => SaleItem[]) private _sellerSaleItems;
        SaleItem[] storage sellerItems = _sellerSaleItems[msg.sender];
        for(uint i = 0 ; i < sellerItems.length ; i++)
        {
            if(sellerItems[i].tokenId == tokenId)
            {
                for(uint j = i; j < sellerItems.length-1; j++){
        ...
        // mapping(address => uint256[]) private _nftIDs;
        ...
        if(ids.length == 0)
        {
            for(uint i = 0 ; i < _nftCollections.length ; i++)
            {
                if(_nftCollections[i] == tokenAddr)
                {
                    for(uint j = i; j < _nftCollections.length-1; j++){
    ...
    function buyNFT(
        ...
        _executeSaleItem(tokenAddr,tokenId,item.price,item.seller,item.creator,item.creatorFee,marketFee);
    ...
    function _transferRevenue(
        ...
        //TODO: handle a case where creator address is 0x0
        ...
        require(creator != address(0), "Market: Creator address is 0x00");
    ...
    function transferMarketTokenOwnership(address newOwner) external onlyOwner {
    ...
    function _saleItemArrayToJSON(SaleItem[] memory saleItems) internal pure returns (string memory) {
    ...
    function _saleItemToJSON(SaleItem memory saleItem) internal pure returns (string memory) {

        string memory returnText = "";

        returnText = string.concat(returnText,'{"tokenAddr":"0x');
        returnText = string.concat(returnText,_toAsciiString(saleItem.tokenAddr));
        ...
        returnText = string.concat(returnText,Strings.toString(saleItem.tokenId));
        returnText = string.concat(returnText,'","price":"');
        returnText = string.concat(returnText,Strings.toString(saleItem.price));
        returnText = string.concat(returnText,'"}');
    ...
    function _centToWEI(uint256 centValue) internal returns (uint256)
    {
        //TODO: convert marketFee to based on the CENT value
        return 10;
    }

    

    // function printSaleItems() public view returns (string memory) {
    //     string memory returnText = "";

    //     returnText = string.concat(returnText,"\n\nTesting _nftCollections");

    //     for (uint256 i = 0; i < _nftCollections.length; i++) {
    //         address nftAddress = _nftCollections[i];
    //         uint256[] memory nftKeys = _nftIDs[nftAddress];

    //         returnText = string.concat(returnText,"\n-----NFT Collection:");
    //         returnText = string.concat(returnText,_toAsciiString(nftAddress));
    //         returnText = string.concat(returnText,"-----\n");

    //         for (uint256 j = 0; j < nftKeys.length; j++) {
    //             uint256 tokenID = nftKeys[j];
    //             SaleItem memory saleItem = _nftSaleItems[nftAddress][tokenID];
    //             // returnText = string.concat(returnText,"2");
                
    //             returnText = string.concat(returnText,'{"tokenAddr":"0x');
    //             returnText = string.concat(returnText,_toAsciiString(saleItem.tokenAddr));
    //             returnText = string.concat(returnText,'","seller":"0x');
    //             returnText = string.concat(returnText,_toAsciiString(saleItem.seller));
    //             returnText = string.concat(returnText,'","tokenId":"');
    //             returnText = string.concat(returnText,Strings.toString(saleItem.tokenId));
    //             returnText = string.concat(returnText,'","seedPrices":"');
    //             returnText = string.concat(returnText,Strings.toString(saleItem.price));
    //             returnText = string.concat(returnText,'"}');
    ...
    //     SaleItem[] memory sellerItems = _sellerSaleItems[seller];

    //     returnText = string.concat(returnText,"\n-----Seller ADDRESS:");
    //     returnText = string.concat(returnText,_toAsciiString(seller));
    //     returnText = string.concat(returnText,"-----\n");

    //     for(uint j = 0 ; j < sellerItems.length ; j++)
    //     {
    //         SaleItem memory saleItem = sellerItems[j];
