 ...
 */
contract PoolyNFT is ERC721, Ownable {
  ...
  /// @notice Max number of NFTs that can be minted
  ...
  uint256 public constant NFT_PRICE = 1000000000000000000; // 75 ETH
  ...
  constructor(
    ...
    emit PoolyNFTInitialized(_name, _symbol, _startTimestamp, _endTimestamp, _owner);
  }
  ...
  function mintNFT(uint256 _numberOfTokens) external payable {
    ...
    require(totalSupply.add(_numberOfTokens) <= MAX_NFTS, "PTNFT/nfts-sold-out");
    require(_numberOfTokens <= MAX_MINT, "PTNFT/exceeds-max-mint");

    uint256 _amount = _numberOfTokens.mul(NFT_PRICE);
  ...
  /**
    ...
    * @dev This NFT collection only has one image, so we only return the base URI.
