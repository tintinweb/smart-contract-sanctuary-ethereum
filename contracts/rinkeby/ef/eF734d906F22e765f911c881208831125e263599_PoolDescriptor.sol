...
contract PoolDescriptor is Ownable, NFTDescriptor {
    ...
    function generateAttributesList(uint256 tokenId) public view returns (string memory) {
        string memory amountStr = (poolAddress.stakedBalance(tokenId)/ 10 ** 18).toString();
        string memory tokenStr = tokenId.toString();
        string memory editionStr;
        if (tokenId < 40 ) {
            editionStr = "First";
        } else {
            editionStr = "Open";
        ...
        return string(
            abi.encodePacked(
                '{"trait_type":"Staked Amount","value":', amountStr, '}'
                // '{"trait_type":"Token ID","value":', tokenStr, '},',
    ...
    function dataURI(uint256 tokenId) public view returns (string memory) {
        ...
        string memory remainderStr = ((amount / 10 ** 16) % 100).toString();
        string memory name = string(abi.encodePacked(amountStr, '.',remainderStr, tokenName));
        uint256 unclaimed = poolHelper.unclaimedRewards(address(poolAddress), IJellyPoolWrapper(address(poolAddress)).rewardsToken(),tokenId);
        string memory unclaimedStr = (unclaimed / 10 ** 18).toString();

        string memory description = string(abi.encodePacked(amountStr, '.',remainderStr, ' Staked JELLY/USDC SLP - ', unclaimedStr, ' Unclaimed $JELLY - ', tokenDescription));
