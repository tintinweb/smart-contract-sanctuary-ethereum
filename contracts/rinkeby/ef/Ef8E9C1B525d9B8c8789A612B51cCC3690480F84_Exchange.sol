...
contract Exchange is Ownable, ExchangeDomain {
    ...
    constructor(
        ...
        ExchangeOrdersHolder _ordersHolder, address payable _beneficiary
    ) {
    ...
    function exchange(
        ...
        /* SELL = accept, BUY = buy */
        transferWithFeesPossibility(order.key.sellAsset, amount, order.key.owner, buyer, feeSide == FeeSide.SELL, buyerFee, sellerFee, order.key.buyAsset); 
        transferWithFeesPossibility(order.key.buyAsset, paying, msg.sender, order.key.owner, feeSide == FeeSide.BUY, sellerFee, buyerFee, order.key.sellAsset);
        emitBuy(order, amount, buyer);
    }

    function validateEthTransfer(uint value, uint _buyerFee) internal view {
        uint256 buyerFeeValue = value.bp(_buyerFee);
    ...
    function transferWithFeesPossibility(Asset memory firstType, uint value, address from, address to, bool hasFee, uint256 _sellerFee, uint256 _buyerFee, Asset memory secondType) internal {
        if (!hasFee) {
            transfer(firstType, value, from, to);
        } else {
            transferWithFees(firstType, value, from, to, _sellerFee, _buyerFee, secondType);
        }
    }
    ...
    function transferWithFees(Asset memory firstType, uint value, address from, address to, uint256 _sellerFee, uint256 _buyerFee, Asset memory secondType) internal {
        uint restValue = transferFeeToBeneficiary(firstType, from, value, _sellerFee, _buyerFee);
        if (
    ...
    function transferFeeToBeneficiary(Asset memory asset, address from, uint total, uint _sellerFee, uint _buyerFee) internal returns (uint) {
        (uint restValue, uint sellerFeeValue) = subFeeInBp(total, total, _sellerFee);
        uint buyerFeeValue = total.bp(_buyerFee);
