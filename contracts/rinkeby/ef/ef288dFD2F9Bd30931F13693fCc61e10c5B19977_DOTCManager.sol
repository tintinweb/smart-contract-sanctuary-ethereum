...
contract DOTCManager is ERC1155Holder, IdOTC, AdminFunctions, ReentrancyGuard {
    ...
    mapping(address => NftOffer[]) internal takenNftOffersFromAddress;
    mapping(uint256 => NftOffer) internal allNftOffers;
    ...
    function setOffer(
        ...
        Offer memory _offer =
            Offer({
                ...
                unitPrice: _amountOut.mul(DECIMAL).div(_amountIn),
     ...
     */
    function takeOffer(uint256 offerId, uint256 _amount)
        public
        isSpecial(offerId)
        isAvailable(offerId)
        can_buy(offerId)
        ...
        returns (uint256 takenOderId)
    {
        ...
        require(_amount <= allOffers[offerId].amountOut, "Amount is greater than offer");
        require(IERC20(allOffers[offerId].tokenOutAddress).balanceOf(msg.sender) >= _amount, "Insufficient balance");
        uint256 amountToReceiveByTaker = _amount.mul(DECIMAL).div(allOffers[offerId].unitPrice);
        if (allOffers[offerId].offerType == OfferType.FULL) {
            require(_amount == allOffers[offerId].amountOut, "This is a Full Request");
        }
        if (_amount == allOffers[offerId].amountOut) {
            allOffers[offerId].amountOut = 0;
            allOffers[offerId].takenAmount = _amount;
            allOffers[offerId].fullyTaken = true;
            emit CompletedOffer(offerId);
        } else {
            allOffers[offerId].amountOut -= _amount;
            allOffers[offerId].takenAmount += _amount;
        }
        ...
        takerOrders[orderId] = Order(offerId, _amount, msg.sender, amountToReceiveByTaker);
        IEscrow(allOffers[offerId].escrowAddress).withdrawDeposit(offerId, orderId);
        payFee(allOffers[offerId].tokenOutAddress, _amount.mul(allOffers[offerId].offerFee).div(BPSNUMBER));
        safeTransferAsset(
        ...
        emit CreatedOrder(offerId, orderId, _amount, msg.sender, amountToReceiveByTaker);
        return orderId;
    }
    ...
    function cancelOffer(uint256 offerId) external can_cancel(offerId) nonReentrant returns (bool success) {
        ...
        uint256 _amountToSend = offer.amountOut.mul(DECIMAL).div(offer.unitPrice);
        require(_amountToSend > 0, "can not cancel");
        require(
            IEscrow(offer.escrowAddress).cancelDeposit(offerId, offer.tokenInAddress, offer.maker, _amountToSend),
        ...
        returns (uint256 offerId)
    {
        ...
        NftOffer storage offer = allNftOffers[currentOfferId];
        offer.nftAddress = _nftAddress;
        offer.tokenOutAddress = _tokenOutAddress;
        offer.nftIds = _nftIds;
        offer.offerPrice = _offerPrice;
        offer.nftAmounts = _nftAmounts;
        offer.expiryTime = block.timestamp + (_expiryInDays * 1 days);
        offer.specialAddress = _specialAddress != address(0) ? _specialAddress : address(0);
        offer.escrowAddress = escrow;
        offer.offerFee = feeAmount;
        IEscrow(escrow).setNFTDeposit(currentOfferId);
        IERC1155(_nftAddress).safeBatchTransferFrom(_msgSender(), escrow, _nftIds, _nftAmounts, bytes(""));
     ...
     *   @return offer NftOffer
     */
    function getNftOffer(uint256 nftOfferId) external view override returns (NftOffer memory offer) {
...
contract TokenListManager is ReentrancyGuard, AccessControl {
    using Counters for Counters.Counter;
...
interface IdOTC {
    ...
    */
    ...
    struct Offer {
        ...
        uint256 takenAmount; // amount taken for partial other. This is used in the OTCManager Contract
    ...
    struct Order {
        ...
        uint256 amountToReceive; // the amount the taker is to recieve
    }
    ...
    function getNftOffer(uint256 _nftOfferId) external view returns (NftOffer memory offer);
...
contract AdminFunctions is AccessControl {
    ...
    uint256 public feeAmount = 3000000000000000;
    uint256 internal constant BPSNUMBER = 1000000000000000000;
    ...
    function setEscrowAddress(address _address) public isAdmin returns (bool status) {
        escrow = _address;
        return true;
    }

    function setEscrowLinker() public isAdmin returns (bool status) {
    ...
    function freezeEscrow() public isAdmin returns (bool status) {
     ...
     *   @return status bool
     */
    function unFreezeEscrow() public isAdmin returns (bool status) {
    ...
    function setTokenListManagerAddress(address _contractAddress) public isAdmin returns (bool status) {
        tokenListManagerAddress = _contractAddress;
        return true;
    }

    function setPermissionAddress(address _permissionAddress) public isAdmin returns (bool status) {
        permissionAddress = _permissionAddress;
        return true;
    }

    function setFeeAddress(address _newFeeAddress) public isAdmin returns (bool status) {
        feeAddress = _newFeeAddress;
        return true;
    }

    function setFeeAmount(uint256 _feeAmount) public isAdmin returns (bool status) {
