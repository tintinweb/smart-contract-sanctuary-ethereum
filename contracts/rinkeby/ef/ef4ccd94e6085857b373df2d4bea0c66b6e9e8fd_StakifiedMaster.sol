...
contract StakifiedMaster is Ownable, ReentrancyGuard {

    StakifiedStake public stakePool;
    mapping (address => address) public stakePools;
    address[] public stakesArray;
    //bytes32 tokenLogo = 0xQmR3HPAUVe7uP6pQ2CoZiijgQmxJGDtK1x3Ti24WUDNC2U;
    ...
    function createStakePool(
            ...
            uint256 _initialRewardSupply,
            uint256[] memory _durationArray, 
            uint256[] memory _apyArray, 
            uint256[] memory _withdrawalFeeArray, 
            uint256[] memory _unstakePenaltyArray 
        ) public payable{

        IERC20 rewardToken = IERC20(_rewardTokenAddress);

        require((msg.value == 0.0001 ether),"Less amount provided");
        require(rewardToken.allowance(msg.sender,address(this)) >= _initialRewardSupply, "Insufficient reward token allowance");

        stakePool = new StakifiedStake(
            _createrAddress, 
            _stakeTokenAddress,
            _rewardTokenAddress,
            _initialRewardSupply, 
            _durationArray, 
            _apyArray, 
            _withdrawalFeeArray, 
            _unstakePenaltyArray
        );

        stakePools[msg.sender] = address(stakePool);
        rewardToken.transferFrom(msg.sender,address(stakePool),_initialRewardSupply);
        stakesArray.push(address(stakePool));
    }
}

contract StakifiedStake is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        initialRewardSupply = _initialRewardSupply;

        for(uint256 i = 0; i < _durationArray.length; i++){
            addStakePool(
                _durationArray[i],
                _apyArray[i],
                _withdrawalFeeArray[i],
                _unstakePenaltyArray[i]
            );
        }
        /*
        addStakePool(
        ...
        */
        ...
        transferOwnership(_createrAddress);
    }
    ...
    function unstake(uint256 _stakeId) nonReentrant unpaused external returns (bool){
        require(isStakeLocked(_stakeId) == false,"The stake is locked");
