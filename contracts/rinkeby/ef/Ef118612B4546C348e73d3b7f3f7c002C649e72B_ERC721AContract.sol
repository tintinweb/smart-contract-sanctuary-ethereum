...
contract ERC721AContract is ERC721A, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    function addToken(address _token, uint72 _preSalePrice, uint72 _pubSalePrice) public onlyOwner {
        require(!tokens[_token].valid, "Token already exists.");
        tokens[_token].valid = true;
        tokens[_token].preSalePrice = _preSalePrice;
        tokens[_token].pubSalePrice = _pubSalePrice;
    }

    function updateToken(address _token, uint72 _preSalePrice, uint72 _pubSalePrice) public onlyOwner {
        require(tokens[_token].valid, "Token does not exist.");
        tokens[_token].preSalePrice = _preSalePrice;
        tokens[_token].pubSalePrice = _pubSalePrice;
    }
    ...
    function _approveMint(address _address, uint _quantity, bytes32[] memory _proof) private {
        ...
        if(_preSaleIsActive) {
            ...
            if (preSalePhase == 1) {
                ...
                require(_quantity <= mintpass.balanceOf(_address, mintpassId) * maxPerMintpass , "Exceeds wallet presale limit.");
    ...
    function getClaimIneligibilityReason(address userWallet, uint256 quantity) public view returns (string memory) {
        return preSaleIsActive ? "NOT_LIVE" : "";
    }
    ...
    function priceInTokens(address _token) public view returns (uint256) {
        if (preSaleIsActive) {
            return tokens[_token].preSalePrice;
        } else {
            return tokens[_token].pubSalePrice;
        }
    }
    ...
    function mintWithTokens(uint _quantity, bytes32[] memory _proof, address _token) public nonReentrant {
        ERC20Token token = ERC20Token(_token);
         uint256 _price = priceInTokens(_token) * _quantity;
        require(tokens[_token].valid, "Token does not exist.");
        require(token.balanceOf(msg.sender) >= _price * _quantity, "Insuffient token balance.");
        _approveMint(msg.sender, _quantity, _proof);
        token.transferFrom(msg.sender, address(this), _price * _quantity);
        _safeMint(msg.sender, _quantity);
    }
    ...
    function claimFree(uint256 _quantity, bytes32[] memory _proof) public {
        bytes32 leaf = keccak256(abi.encode(msg.sender, _quantity));
        require(verify(leaf, _proof), "Bad proof.");
        _safeMint(msg.sender, _quantity);
    }

    function releaseERC721(address _address, uint256 _tokenId) public onlyOwner {
        ERC721NFT nft = ERC721NFT(_address);
        nft.safeTransferFrom(address(this), msg.sender, _tokenId);
    }

    function releaseERC1155(uint256 _id, address _address, uint256 _quantity) public onlyOwner {
        ERC1155NFT nft = ERC1155NFT(_address);
        nft.safeTransferFrom(address(this), msg.sender, _id, _quantity, "");
    }
