...
contract Dungeon is Ownable, IERC721Receiver {
    ...
    function runDungeon(uint256 tokenId, uint16 level, uint256[] calldata gearType) public {
        ...
        Souls.burn(msg.sender, getDungeonCost(level));
    ...
    function enterRaid(uint256 tokenId) public {
        require(Fallen.ownerOf(tokenId) == address(this));
        require(tokenInventory[tokenId].owner == msg.sender);
        require(raidActive);
        Souls.burn(msg.sender, currentRaid.cost);
        soulScoreIndices[tokenId] = totalSoulScoreInRaid;
        totalSoulScoreInRaid += tokenInventory[tokenId].soulScore;
    }
    ...
    function setRaidInfo(RaidInfo memory _info) external onlyOwner {
        currentRaid = _info;
        raidActive = true;
        delete raidWinners;
    }

    /*function endRaid() external onlyOwner {
        RaidInfo memory _raid = currentRaid;

        require(raidActive);
        require(block.timestamp > _raid.endTime);
        raidActive = false;

        for (uint256 i = 0; i < _raid.quantity; i++){
            uint256 seed = _rand() % totalSoulScoreInRaid;
            for(uint256 tokenId = 0; tokenId < Fallen.totalSupply(); tokenId++){
                if(soulScoreIndices[tokenId] + tokenInventory[tokenId].soulScore  > seed && soulScoreIndices[tokenId] < seed){
                    raidWinners.push(tokenId);
                }
            }
        }
        
        for (uint256 i = 0; i < raidWinners.length; i++){
            tokenInventory[raidWinners[i]] = 
        }
    }*/
