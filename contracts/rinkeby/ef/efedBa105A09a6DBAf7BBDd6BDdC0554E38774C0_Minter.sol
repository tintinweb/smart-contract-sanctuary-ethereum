...
contract Minter is Ownable, ReentrancyGuard {
    ...
    // ======== Cost =========
    uint256 public immutable tokenPriceETH_Discount;
    uint256 public immutable tokenPriceETH;
    uint256 public tokenPriceUSDC;
    uint256 public tokenPriceUSDC_Discount;
    
    event TokenPriceUSDCChanged(uint256 oldPrice, uint256 newPrice);
    event TokenPriceUSDCDiscountChanged(uint256 oldPrice, uint256 newPrice);

    // ======== Sale Status =========
    bool public saleIsActive = false;
    bool public saleUSDCIsActive = false;
    ...
    constructor(address contractAddress,
                ...
                uint256 _tokenPriceUSDC_Discount,
        ...
        tokenPriceETH_Discount = _tokenPriceETH_Discount;
        tokenPriceETH = _tokenPriceETH;
        tokenPriceUSDC_Discount = _tokenPriceUSDC_Discount;
        tokenPriceUSDC = _tokenPriceUSDC;

        setTokenPriceUSDC(_tokenPriceUSDC);
        setTokenPriceUSDCDiscount(_tokenPriceUSDC_Discount);
    }
    ...
    modifier isMaxMintsPerAddressExceeded(uint amount) {
        require(addressToMintCount[msg.sender] + amount <= maxMintsPerAddress, "Exceeds max mint per tx!");
        _;
    }

    /// @notice Set the token price in USDC
    /// @param _tokenPriceUSDC The USDC price
    function setTokenPriceUSDC(uint256 _tokenPriceUSDC) public onlyOwner {
      emit TokenPriceUSDCChanged(tokenPriceUSDC, _tokenPriceUSDC);
      tokenPriceUSDC = _tokenPriceUSDC;
    }

    /// @notice Set the discounted token price in USDC
    /// @param _tokenPriceUSDCDiscount The USDC price
    function setTokenPriceUSDCDiscount(uint256 _tokenPriceUSDCDiscount) public onlyOwner {
      emit TokenPriceUSDCDiscountChanged(tokenPriceUSDC_Discount, _tokenPriceUSDCDiscount);
      tokenPriceUSDC_Discount = _tokenPriceUSDCDiscount;
    }
    ...
    function transferFunds(uint256 qty, bool applyDiscount) private {
        ...
        uint256 priceUSDC = !applyDiscount ? tokenPriceUSDC : tokenPriceETH_Discount;
        ...
        if(msg.value == qty * priceETH) { 
            ...
            // pay with USDC
            require(saleUSDCIsActive, "USDC sale is not active!");

            tokenUSDC.safeTransferFrom(
              msg.sender,
              withdrawalAddress,
              qty * priceUSDC
            );
    ...
    function mintDiscount(bytes32[] calldata merkleProof, uint amount, uint quantity) public payable 
        ...
        nonReentrant {
            ...
            require(!discountClaimed[msg.sender], "Discount is already claimed by this wallet!");
            ...
            discountClaimed[msg.sender] = amount == addressToMintCount[msg.sender];
    }
    ...
    function isDiscounted(address _address, bytes32[] calldata merkleProof, uint256 amount) external view
        ...
        returns (bool) {            
            require(!discountClaimed[_address], "Discount is already claimed by this wallet");

            return true;
    }

    /// @notice Check if user has claimed all their discounted spots
    /// @param _address The discounted address
    function isDiscountClaimed(address _address) external view returns (bool) {
    ...
    /// @notice Toggle USDC sale status
    function flipSaleUSDCStatus() public onlyOwner {
        saleUSDCIsActive = !saleUSDCIsActive;
    }
    ...
    /// @param _newWithdrawalAddress The withdrawal address for buth ETH/USDC funds
