...
contract NFTTweetMarketplace is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(address => mapping(uint256 => uint256)) escrowContracts; // track contract add => nft id => escrow id
    mapping(address => mapping(uint256 => bool)) escrowContractsState; // track contract add => nft id => bool
        ...
        nonReentrant
    {
        ...
        // calculate fees
        uint256 _marketFee = calcPercentage(msg.value, marketFee);
        ...
        if (
            ...
            !escrowContractsState[sl.nftContract][sl.nftID]
        ) {
            // list for escrow
            ...
            escrowContracts[sl.nftContract][sl.nftID] = escrowCount;
            escrowContractsState[sl.nftContract][sl.nftID] = true;
     ...
     * @param _minSaleTime new maximum sale time
     */
    ...
    {
        require(
            escrowContractsState[_nftContract][_tokenID],
            "NFTM: no escrow"
        );
        EscrowListing storage el = escrowListings[
            escrowContracts[_nftContract][_tokenID]
        ];
        ...
        el.settled = true;
        delete escrowContractsState[_nftContract][_tokenID];
        delete escrowContracts[_nftContract][_tokenID];
     ...
     * @param basisPoints basis points in hundredes (3% = 300, 2% = 200, etc.)
     */
    function calcPercentage(uint256 amount, uint256 basisPoints)
        internal
        pure
        returns (uint256)
    {
