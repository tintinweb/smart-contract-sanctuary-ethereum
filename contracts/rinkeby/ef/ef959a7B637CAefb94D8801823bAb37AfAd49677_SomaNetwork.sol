...
contract SomaNetwork is ISomaNetwork, ProposedOwnableUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    event NetworkInitialized();
    event NetworkKeyUpdated(bytes32 indexed key, address indexed prevAddress, address indexed newAddress);
    event NetworkAccessApproved(address indexed account, address indexed approvedBy);
    event NetworkAccessRevoked(address indexed account, address indexed revokedBy);
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(ISomaNetwork).interfaceId ||
            interfaceId == type(IProposedOwnable).interfaceId ||
    ...
    function remove(address target) external override {
        ...
        if (AddressUpgradeable.isContract(target)) {
            try IERC165(target).supportsInterface(type(INetworkAccess).interfaceId) returns (bool) {
                ...
                INetworkAccess networkAccess = INetworkAccess(target);
                networkAccess.removedFromNetwork();
