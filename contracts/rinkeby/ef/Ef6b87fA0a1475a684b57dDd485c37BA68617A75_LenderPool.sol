...
contract LenderPool is ERC1155Upgradeable, ReentrancyGuardUpgradeable, IPooledCreditLineEnums, ILenderPool {
     ...
     * @param sharesHeld total shares of borrow token held by the pooled credit line, principal withdrawals not accounted for
    ...
    function lend(uint256 _id, uint256 _amount) external nonReentrant {
        require(VERIFICATION.isUser(msg.sender, pooledCLConstants[_id].lenderVerifier), 'LP:L1');
        require(block.timestamp < pooledCLConstants[_id].startTime, 'LP:L2');

        uint256 _totalLent = totalSupply[_id];
        uint256 _maxLent = pooledCLConstants[_id].borrowLimit;
        require(_maxLent > _totalLent, 'LP:L3');
    ...
    function start(uint256 _id) external override nonReentrant {
        ...
        uint256 _totalLent = totalSupply[_id];
        require(_totalLent >= pooledCLConstants[_id].minBorrowAmount, 'LP:S3');
    ...
    function terminate(uint256 _id, address _to) external override nonReentrant onlyPooledCreditLine {
        ...
        if (_collateralHeld != 0) {
            // transferring the collateral that might be transferred to Lender pool to distribute among lenders as part of liquidation
    ...
    function _updateInterestSharesToWithdraw(
        ...
        (uint256 _borrowerInterestForLender, uint256 _yieldInterestForLender) = _calculateLenderInterest(
            _id,
            _lender,
            _strategy,
            _borrowAsset,
        ...
        if (_borrowerInterestForLender != 0) {
        ...
        return _yieldInterestForLender.add(_borrowerInterestForLender);
    }

    /**
     * @notice Function used to get interest accrued by a lender in pooled credit line
     ...
     */
    function getLenderInterest(uint256 _id, address _lender) external returns (uint256) {
        address _strategy = pooledCLConstants[_id].borrowAssetStrategy;
        address _borrowAsset = pooledCLConstants[_id].borrowAsset;
    ...
    function _calculateLenderInterest(
        ...
        uint256 _borrowLimit
    ) private returns (uint256 _borrowerInterestForLender, uint256 _yieldInterestForLender) {
        uint256 _totalInterestInShares;
        {
            ...
            uint256 _notBorrowedInShares = IYield(_strategy).getSharesForTokens(_notBorrowed, _borrowAsset);
            _totalInterestInShares = _sharesHeld.sub(_notBorrowedInShares);
        }
        uint256 _borrowerInterestShares = pooledCLVariables[_id].borrowerInterestShares;
        _borrowerInterestForLender = (_borrowerInterestShares.mul(_lenderBalance).div(_borrowLimit)).sub(
            pooledCLVariables[_id].lenders[_lender].borrowerInterestSharesWithdrawn
        );

        {
            uint256 _borrowerInterestSharesAfterWithdrawal = _borrowerInterestShares.sub(
                pooledCLVariables[_id].borrowerInterestSharesWithdrawn
            );
            uint256 _totalYieldInterest = _totalInterestInShares.sub(_borrowerInterestSharesAfterWithdrawal).add(
                pooledCLVariables[_id].yieldInterestWithdrawnShares
            );
            _yieldInterestForLender = (_totalYieldInterest.mul(_lenderBalance).div(_borrowLimit)).sub(
    ...
    function _withdrawLiquidity(uint256 _id, bool _isLiquidationWithdrawn) private {
        ...
        if (_status == PooledCreditLineStatus.CANCELLED) {
            ...
            uint256 _interestWithdrawable = IYield(_strategy).getTokensForShares(_interestSharesWithdrawable, _borrowAsset);
    ...
    function liquidate(uint256 _id, bool _withdraw) external nonReentrant {
        ...
        if (_withdraw) {
            ...
            _withdrawLiquidatedCollateral(_id, _lendingShare);
        }
    }
     ...
     * @param _id identifier for the pooled credit line
     */
    function withdrawLiquidatedCollateral(uint256 _id) external nonReentrant {
        uint256 _lendingShare = balanceOf(msg.sender, _id);
        require(_lendingShare != 0, 'LP:WLC1');
        _withdrawLiquidatedCollateral(_id, _lendingShare);
    }

    function _withdrawLiquidatedCollateral(uint256 _id, uint256 _balance) private {
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i; i < ids.length; ++i) {
            uint256 id = ids[i];
            require(to == address(0) || VERIFICATION.isUser(to, pooledCLConstants[id].lenderVerifier), 'LP:IT2');
            ...
            if (from == address(0)) {
                ...
                require(supply >= amount, 'LP:IT3');
                totalSupply[id] = supply - amount;
            } else {
                require(pooledCLConstants[id].areTokensTransferable, 'LP:IT4');
            }
