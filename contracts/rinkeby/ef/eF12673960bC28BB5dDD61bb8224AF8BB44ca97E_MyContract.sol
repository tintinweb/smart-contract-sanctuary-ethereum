...
contract MyContract is ERC721AQueryable, ERC721ABurnable, Ownable, EIP712, IERC2981 {
    using Address for address;

    error NotHuman(address caller);
    error ExccededTotalSupply(uint256 totalSupply, uint256 max);
    ...
    uint256 constant public PUBLIC_MINT_PRICE = 0.015 ether;
    ...
    modifier commonCheck(uint256 amount) {
        ...
        if (totalSupply() > maxTokenCount) {
            revert ExccededTotalSupply(totalSupply(), maxTokenCount);
        }
    }
    ...
    ) ERC721A(name, name) EIP712(name, "1.0.0") {
        treasury4 = treasury4_;
        treasury6 = treasury6_;
        treasurySplitter = treasurySplitter_;
    ...
    function royaltyInfo(uint256 tokenId, uint256 salePrice) external override view returns (address receiver, uint256 royaltyAmount) {
        ...
        royaltyAmount = salePrice * 75 / 1000;
    }
