    ...
    ReentrancyGuard
{
    ...
    event UserUpdateReservePrice(
        address indexed user,
        uint256 oldValue,
        uint256 newValue,
        uint256 tokenCount
    );
    event UserUpdateAuctionLength(
        address indexed user,
        uint256 oldValue,
        uint256 newValue,
        uint256 tokenCount
    );
    event UserUpdateBidIncrement(
        address indexed user,
        uint256 oldValue,
        uint256 newValue,
        uint256 tokenCount
    ...
    function _updateReservePrice(uint256 _reservePrice, uint256 weight) internal {
        ...
        if ((oldChoice == 0)) {
            ...
            userReservePrice[msg.sender] = _reservePrice;
            uint256 userOldChoice = (oldChoice * weight * totalFNFT) /
                totalSupply();
            reservePriceTotal =
                reservePriceTotal -
                userOldChoice +
                weightedReservePrice;
        }
        require(totalVoted > 0, "There must be at least one voter");
        emit UserUpdateReservePrice(
            msg.sender,
            oldChoice,
            _reservePrice,
            weight
        ...
        internal
    {
        ...
        if ((oldChoice == 0) && (_auctionLength != 0)) {
            ...
            auctionLengthTotal =
                auctionLengthTotal -
                userOldChoice +
                weightedAuctionLength;
        }
        emit UserUpdateAuctionLength(
            msg.sender,
            oldChoice,
            _auctionLength,
            weight
