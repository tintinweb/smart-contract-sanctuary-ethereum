...
import "contracts/interfaces/ICryptosPool.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import "hardhat/console.sol";


contract CryptosVault is Ownable, Pausable {
    ...
    event NewCompoundBlock(uint256 lastCompoundBlock);
    ...
    function compound() external { // this function is not complete
        yield();
    }

    function yield() internal { // this function is not complete
        uint256 poolbalance = IERC20(token).balanceOf(pool);
        uint256 timedelta = deltaTime();
        int256 newbalance = continuousCompound(poolbalance, APR, timedelta);
        int256 interest = newbalance - int256(poolbalance);
        IERC20(token).safeTransfer(pool, uint256(interest));
        ICryptosPool(pool).setLastCompoundBlock(block.timestamp);
    }

    /**
        * @notice Sets the APR for the vault.
        * @param _APR The APR for the vault.
     */

    function setAPR(uint256 _APR) external onlyAdmin whenNotPaused { // set APR
    ...
    /**
        * @notice delta is the time difference in secounds frin the last compound block
    */

    function deltaTime() public view returns (uint256) {
        int YEAR_IN_SECONDS = 31536000;
 
        uint256 lastCompoundBlock = ICryptosPool(pool).getLastCompoundBlock();
        uint256 currentTime = block.timestamp;

        uint256 deltaSeconds = currentTime - lastCompoundBlock;
        int256 timeDelta = PRBMathSD59x18.toInt(
            PRBMathSD59x18.mul(
                PRBMathSD59x18.div(
                    PRBMathSD59x18.fromInt(int256(deltaSeconds)),
                    PRBMathSD59x18.fromInt(YEAR_IN_SECONDS)
                ),
                PRBMathSD59x18.pow(
                    PRBMathSD59x18.fromInt(10),
                    PRBMathSD59x18.fromInt(12)
                )
            )
        );

        return uint256(timeDelta);
    }

    function moveDecimal12(uint256 num) internal pure returns (int256) {
        return PRBMathSD59x18.mul(
            PRBMathSD59x18.fromInt(int256(num)),
            PRBMathSD59x18.inv(
                PRBMathSD59x18.pow(
                    PRBMathSD59x18.fromInt(10),
                    PRBMathSD59x18.fromInt(12)
    ...
    function continuousCompound(uint256 principal, uint256 _rate, uint256 _time) public view returns (int256) {
        // takes principal and returns principal + interest
        // 2.7182818284590452353602874713526624977572470936999595749669676277
        int256 e = moveDecimal12(2718281828459);
        int256 rate = moveDecimal12(_rate);
        int256 time = moveDecimal12(_time);
        int256 rateTime = PRBMathSD59x18.mul(
            rate,
            time
        );

        return PRBMathSD59x18.toInt(
            PRBMathSD59x18.mul(
                PRBMathSD59x18.pow(
                    e,
                    rateTime
                ),
                PRBMathSD59x18.fromInt(int256(principal))
            )
        );
    }
}
...
interface ICryptosPool {
    function lastCompoundBlock() external returns (uint256);

    function setLastCompoundBlock(uint256 _lastCompoundBlock) external;

    function getLastCompoundBlock() external view returns (uint256);
