...
contract TokenTBDAuction is Ownable {
    ...
    struct Bid {
        address wallet_bidder;
        address wallet_recipient;
        uint256 bidAmount;
        bool cancelled;
    }
    // Token ID => top bid
    ...
    function _placeBid(uint8 tokenId, address recipient) private notLocked {
        ...
        if (currentTopBid.bidAmount == 0) {
            ...
            payable(currentTopBid.wallet_bidder).transfer(
        ...
        emit BidPlaced(
            tokenId,
            latestBid.wallet_bidder,
            latestBid.wallet_recipient,
            latestBid.bidAmount
        );
    }
    ...
    function endAuction(uint8 tokenId) external {
        ...
        require(!currentTopBid.cancelled, "AUCTION_CANCELLED");
        require(
            msg.sender == currentTopBid.wallet_bidder || msg.sender == owner(),
        ...
        require(currentTopBid.wallet_recipient != address(0), "NO_WINNERS");
        tokenTBDK.mintPlatinum(tokenId, currentTopBid.wallet_recipient);

        emit AuctionEnded(
            tokenId,
            currentTopBid.wallet_bidder,
            currentTopBid.wallet_recipient,
            currentTopBid.bidAmount
        );
    }

    function cancelAuction(uint8 tokenId) external payable onlyPaperOrOwner {
        Bid memory currentTopBid = currentTopBids[tokenId];
        require(!currentTopBid.cancelled, "ALREADY_CANCELLED");
        require(currentTopBid.bidAmount != 0, "MUST_HAVE_BID");
        require(currentTopBid.wallet_bidder != address(0), "MUST_HAVE_BIDDER");
        require(
            paperWallets[currentTopBid.wallet_bidder] || msg.sender == owner(),
        ...
        payable(currentTopBid.wallet_bidder).transfer(currentTopBid.bidAmount);

        // Cancel bid/auction.
        currentTopBids[tokenId].cancelled = true;

        emit AuctionCancelled(tokenId, currentTopBid.wallet_bidder);
    }
