...
contract SlayToEarnToken is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function enableAutomaticBuybacks(IUniswapV2Router02 uniswapRouter, IERC20 usdcToken) public onlyOwner {
        ...
        require(balanceOf(address(this)) >= 1 ether, "The contract needs to have at least one SLAY2EARN token to test the buyback parameters.");

        _doAutomaticBuyback(1 ether);
    }
    ...
    */
    ...
    function _transfer(
        ...
        _doAutomaticBuyback(devTokens);

        _transferInternal(sender, recipient, receivedTokens);
    }

    function _doAutomaticBuyback(uint256 tokensToSell) internal {
        address[] memory tradingPath = new address[](2);
        tradingPath[0] = address(this);
        tradingPath[1] = _usdcToken;

        increaseAllowance(address(_uniswapRouter), tokensToSell);
