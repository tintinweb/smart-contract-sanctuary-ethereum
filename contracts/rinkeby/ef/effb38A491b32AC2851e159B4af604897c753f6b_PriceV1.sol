/**
   ...
   Oracle  Contract ==> Call StoreFeed
   
   Access Contract  ==> Get Implementation || Buy Access Time
...
interface IAccessControl {
    ...
    function timeBasedAccess() external view returns (bool);
    function buyAccessTime() external payable;
}

contract PriceV1 {
    using strings for *;

    IPriceOracle public priceOracle;      // remove public
    IAccessControl public accessControl;
    address public anchor;

    function setupAddress(address _priceOracle, address _accessControl, address _anchor) public {
    ...
    function storeFeed(string memory _feeds, bool flag) public virtual returns (string memory) {
        // require(msg.sender == anchor, "Only anchor contract can call this");
    ...
    function inspector() public view returns (address, address){
        return (msg.sender, tx.origin);
    }

    //######################################################################################
    function setCurrentPrice(string memory currency_pair, string memory price) public virtual {
        require(msg.sender == anchor, "Only anchor contract can call this");
        priceOracle.setCurrentPrice(currency_pair, price);
    }



    //######################################################################################
    function setWeeklyPrice(string memory currency_pair, string memory price) public virtual {
        require(msg.sender == anchor, "Only anchor contract can call this");
        priceOracle.setWeeklyPrice(currency_pair, price);
    }
    ...
    //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function getCurrentPrice(string memory currencyPair) external virtual returns(string memory, uint256){
        require(accessControl.hasRole(0xdf8b4c520ffe197c5343c6f5aec59570151ef9a492f2c624fd45ddde6135ec42, tx.origin) || accessControl.timeBasedAccess(), "Your group is not authorised to access price || No time access left");
        return priceOracle.getCurrentPrice(currencyPair);
    }

    //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function getWeeklyPrice(string memory currency_pair) external view virtual returns(string memory, uint256){
        require(accessControl.hasRole(0xdf8b4c520ffe197c5343c6f5aec59570151ef9a492f2c624fd45ddde6135ec42, tx.origin) || accessControl.timeBasedAccess(), "Your group is not authorised to access price || No time access left");
        return priceOracle.getWeeklyPrice(currency_pair);
    }
