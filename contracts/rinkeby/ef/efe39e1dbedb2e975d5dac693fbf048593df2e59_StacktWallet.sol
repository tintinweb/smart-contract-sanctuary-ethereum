    ...
    StacktGuardManager
{
    ...
    event RefundCalculation(uint256 indexed gas, uint256 indexed datagas, uint256 indexed gasprice); ///
    ...
    function execTransaction(
        ...
        /* -------------------------------- execution ------------------------------- */
        {
            ...
            uint256 payment = 0;
            if (refundGas) {
                // good for 1 transfer, bad for 20 transfers:
                // - gasUsed + PAYMENT_GAS_COSTS + MISC_GAS_COSTS + msg.data.length * DATA_BYTE_GAS_COSTS
                // - PAYMENT_GAS_COSTS: 8000
                // - MISC_GAS_COSTS: 3000
                // - DATA_BYTE_GAS_COSTS: 68
                payment = (gasUsed + PAYMENT_GAS_COSTS + MISC_GAS_COSTS) * tx.gasprice;
                emit RefundCalculation(gasUsed + PAYMENT_GAS_COSTS + MISC_GAS_COSTS + msg.data.length * DATA_BYTE_GAS_COSTS, msg.data.length * DATA_BYTE_GAS_COSTS, tx.gasprice); ///
