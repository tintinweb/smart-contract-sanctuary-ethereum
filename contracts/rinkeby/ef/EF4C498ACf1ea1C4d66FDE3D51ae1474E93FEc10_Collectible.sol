...
contract ERC721MULTI is Context {
    ...
    function transferFrom(
        ...
        bool isApprovedOrOwner = (_msgSender() == previousOwner ||
            ...
            approvements[id] == _msgSender());

        if (!isApprovedOrOwner) revert CallerNotOwnerNorApproved();
        if(from != previousOwner) revert TransferFromIncorrectAddress();
        if(to == address(0)) revert ZeroAddress();
        ...
        unchecked {
            ...
            if (ownings[nextTokenId] == address(0)) {
                // This will suffice for checking _exists(nextTokenId),
                // as a burned slot cannot contain the zero address.
                if (nextTokenId < state.tokenCounter) {
                    ownings[nextTokenId] = previousOwner;
                }
            }
        }
    ...
    function ownershipOf(uint256 tokenId) internal view returns (address)
    {
        ...
        uint256 _maxMintPerUser = state.maxMintsPerUser;
        uint256 lowestTokenToCheck;

        unchecked{
            if(currentId < _tokenCounter)
            {
                if (tokenId >= _maxMintPerUser)
                {
                  lowestTokenToCheck = _tokenCounter - _maxMintPerUser + 1;
                }
