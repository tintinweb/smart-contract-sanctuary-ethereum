...
contract BlocksRewarder is Ownable, ReentrancyGuard {
    ...
    function allocateRewards(address[] calldata _beneficiaries, uint256[] calldata _earnings) external onlyOwner {
        ...
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            ...
            totalPayoutsFor[_beneficiary] = _totalEarned + _totalReceived;
    ...
    function claim(address _recipient) external nonReentrant {
        require(_recipient == msg.sender , "Claimer must be beneficiary.");
        require(totalPayoutsFor[_recipient] > 0, "Balance must be > 0");

        uint256 _claim = totalPayoutsFor[_recipient];
        increaseClaimed(_claim);
        totalAllocated = totalAllocated - _claim;
        token.transfer(_recipient, _claim);
    }
