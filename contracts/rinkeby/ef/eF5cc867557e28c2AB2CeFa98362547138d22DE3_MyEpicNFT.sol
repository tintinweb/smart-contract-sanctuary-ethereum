...
contract MyEpicNFT is ERC721URIStorage {
  ...
  string[] firstWords = ["Yamada", "Tanaka", "Sato", "Iida", "Suzuki", "Convoi"];
  string[] secondWords = ["Andree", "Pokepoke", "Jagger", "Rocket", "Blue", "Thunder"];
  string[] thirdWords = ["Momotaro", "Oniko", "Gonzaburo", "Seijuro", "Umeko", "Yamato"];

  uint256 public MAX_TOKEN_SUPPLY = 50;
  event NewEpicNFTMinted(address sender, uint256 tokenId, uint256 tokenSupply);
  ...
  function makeAnEpicNFT() public {
    // 最大供給量より発行されている場合は「これ以上NFTを発行できません」エラーを出す
    require(_tokenIds.current() < MAX_TOKEN_SUPPLY, "No more can be issued NFT.");
...
//   // OpenZeppelin が tokenIds を簡単に追跡するために提供するライブラリを呼び出しています
//   // Countersライブラリで指定したデータ型Counters.Counterに対して、ライブラリで定義したメソッドを追加する
//   using Counters for Counters.Counter;

//   // _tokenIdsを初期化（_tokenIds = 0（初期値））
//   Counters.Counter private _tokenIds;
...
//      // msg.sender を使って NFT（newItemIdの付与されたNFT）を送信者に Mint します。
//     _safeMint(msg.sender, newItemId);

//     // NFT データを設定（「 NFT の一意の識別子newItemId」と「実際のデータURI」を紐付ける）します。
