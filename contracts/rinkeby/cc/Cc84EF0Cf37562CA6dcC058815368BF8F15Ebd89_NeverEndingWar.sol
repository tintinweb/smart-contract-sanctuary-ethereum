...
contract NeverEndingWar is Ownable, IERC721Receiver {
    ...
    // reference to the Champion and WoNToken contract
    Champion nft;
    WoNToken token;
    ...
    constructor(Champion _nft, WoNToken _token) {
        nft = _nft;
        token = _token;
    }
    ...
    function _claim(address account) internal {
        ...
        if (userTeamData[account].totalWoNToken > 0) {
            token.mint(account, userTeamData[account].totalWoNToken);
        }
    ...
    function updateTotalWon(address account) internal{
        ...
        for (uint i = 0; i < userTeamData[account].tokenIds.length; i++) {
            ...
            uint256 score = nft.getCombatPower(tokenId);
            totalScore += score;
        }
        totalScore += (totalScore * nft.getTeamChemistry(userTeamData[account].tokenIds))/100;
        earned = totalScore * ((block.timestamp - userTeamData[account].timestamp) / 1 days);
    ...
    function getClaimableWon(address account) public view returns(uint256){
        ...
        for (uint i = 0; i < userTeamData[account].tokenIds.length; i++) {
            uint24 tokenId = userTeamData[account].tokenIds[i];
            teamCp += nft.getCombatPower(tokenId);
        }
...
contract WoNToken is ERC20, ERC20Burnable, Ownable {

    mapping(address => bool) controllers;

    constructor() ERC20("WoN Token", "WoN") { }
    ...
    PaymentSplitter
{
    ...
    //WHITELIST//
    uint256 whitelistWalletLimit = 1;
    uint256 whitelistTxLimit = 1;
    //0.12 ETH IN WEI
    //uint256 whitelistPrice = 120000000000000000;
    //0.00012 for rinkeby
    uint256 whitelistPrice = 120000000000000;

    //PUBLIC//
    uint256 publicWalletLimit = 1000;
    uint256 publicTxLimit = 3;
    //0.16 ETH IN WEI
    //uint256 whitelistPrice = 160000000000000000;
    //0.00016 for rinkeby
    uint256 publicPrice = 0;

    //DUTCH AUCTION//
    bool public isDutchAuction = false;
    uint256 dutchAuctionStartTime = 0;
    uint256 dutchAuctionLength = 360 minutes;
    uint256 dutchAuctionInterval = 30 minutes;
    ...
    uint256[] private teamShares = [10,15,25,25,25];
    address[] private team = [
        0x5638AD0739Ad0dc401bCE961d7F2df00F6D9f37a, //10
        0xcFD3F36910d58DB62E6A89385175b86EB8E5d15a, //15
        0x04E950C558322f0425B60EDB5E527137F23C5e34, //25
        0x1104EC306797a1a9Dd3FAF8A1356122dC3cd1630, //25
        0xE2089B1E01363575aca97FD29A2453C4AaCe6A32  //25
    ];
    ...
    function setBeforeRevealURI(string memory _beforeRevealURI) external onlyOwner{
        beforeRevealURI = _beforeRevealURI;
    }
    ...
    returns (uint256)
    {
        ...
        if (block.timestamp - dutchAuctionStartTime >= dutchAuctionLength) {
            ...
            uint256 steps = (block.timestamp - dutchAuctionStartTime) /
            dutchAuctionInterval;
    ...
    function togglePause() onlyOwner external{
        paused = !paused;
    }
    
    function togglePresale() onlyOwner external{
        presaleStarted = !presaleStarted;
    }
    
    function setPublicSale(bool _isActive) onlyOwner external{
    ...
    function getOwnerTeamChemistry() external view returns(uint256){
        if (combatPowerAddress == address(0x0)){
            return 0;
        }
        return ICombatPower(combatPowerAddress).getTeamChemistry(INeverEndingWar(combatPowerAddress).getStakedTokenIds(msg.sender));
    }
