...
contract Senra is ERC721A, Ownable {
  ...
  bool public testClaimed;
  bool public testClaimed2;
  ...
  function mint(uint256 _mintAmount, bytes32[] memory proof) public payable {
    ...
    //require(_mintAmount + balanceOf(msg.sender) <= maxMintAmount, "Excedida a quantidade limite de mint por vez.");
    require(_mintAmount + _mintAmountPerWallet[msg.sender] +  _freeMintAmountPerWallet[msg.sender] <= maxMintAmount, "Excedida a quantidade limite de mint por vez.(Cod. 50001)");
    require(supply + _mintAmount <= maxSupply, "Quantidade limite de mint excedida.(Cod.5002)");
    require(maxMintAmount > _freeMintAmountPerWallet[msg.sender] + _mintAmountPerWallet[msg.sender], "Esta carteira mintou a quantidade limite de tokens permitidos por carteira.(Cod.5003)");
    ...
    delete _addressLastMintedTokenIds[msg.sender];
        ...
        whitelist = isWhitelisted(msg.sender, proof);

        //Public mint
        if(!whitelist){
          require(_mintAmount +_mintAmountPerWallet[msg.sender] <= _maxMintAmountAllowedPerWallet, "Quantidade limite de mint publico por carteira excedida. (Cod.5004)");
          ...
          testClaimed = whitelistClaimed[msg.sender];

          //Não usou a whitelist
          if(!whitelistClaimed[msg.sender]){
            require(_mintAmount + _freeMintAmountPerWallet[msg.sender] <= _maxFreeMintAmountAllowedPerWallet, "Quantidade limite de mint free por carteira excedida. (Cod.5005)");
            ...
            testClaimed2 = whitelistClaimed[msg.sender];
  ...
  function doMint(uint _mintAmount) internal{
    ...
    */
    function _mint(
        ...
        //Sequencial
        uint256 tokenId = ++qtdMintedToken;
        ...
        //    lastTokenId = tokenId; //TESTAR DESCOMENTAR
        //    _mintedNFTs[lastTokenId] = true;
        // }
