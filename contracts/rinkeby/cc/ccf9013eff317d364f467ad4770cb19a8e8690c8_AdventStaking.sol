...
contract AdventStaking is Ownable, IERC721Receiver {
  ...
  function addLegendaryIds(uint256[] memory tokenIds) public onlyOwner {
    for (uint256 i; i < tokenIds.length; i++) {
      LegendaryId[tokenIds[i]] = true;
      }
    }

    function addRareIds(uint256[] memory tokenIds) public onlyOwner {
    for (uint256 i; i < tokenIds.length; i++) {
      RareId[tokenIds[i]] = true;
      }
    }
