...
/**
 * @title ERC-721 Non-PUPWORLD Token Standard, optional metadata extension
...
/**
 * @dev Implementation of https://eips.ethereum.org/EIPS/eip-721[ERC721] Non-PUPWORLD Token Standard, including
...
/**
 * @title ERC-721 Non-PUPWORLD Token Standard, optional enumeration extension
 * @dev See https://eips.ethereum.org/EIPS/eip-721
 */
...
contract PUPWORLD is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant PUPWORLD_PUBLIC = 10000;
    uint256 public constant PUPWORLD_MAX = PUPWORLD_PUBLIC;
    ...
    Counters.Counter private _publicPUPWORLD;

    constructor() ERC721("PUPWORLD", "PUPWORLD") {
    ...
    {
        require(
            _publicPUPWORLD.current() < PUPWORLD_PUBLIC,
            "Purchase would exceed PUPWORLD_PUBLIC"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicPUPWORLD.current();

            if (_publicPUPWORLD.current() < PUPWORLD_PUBLIC) {
                _publicPUPWORLD.increment();
                _safeMint(to, tokenId);
            }
        }
    }
