...
// File contracts/Wacky.sol


pragma solidity ^0.8.7;

contract Wacky is ERC20, Ownable, ReentrancyGuard {
    ...
    // Mapping claim voucher nonces used for signatures
    mapping(bytes32 => bool) private _nonces;

    constructor(uint256 supply) ERC20("Wacky", "WACKY") {
        MAX_SUPPLY = supply;
    }

    /**
     * @dev Emitted when user claims tokens
     */
    event Claimed(uint256 indexed amount);

    /**
     * @dev Allows admins to burn on the users behalf
     */
    ...
    /**
     * @dev Claiming tokens earned externally
     *
     * Emits a {Claimed} event.
     */
    function claimBySignature(uint256 amount, uint256 nonce, bytes calldata signature) external nonReentrant {
        bytes32 voucher = bytes32(bytes20(_msgSender())) | bytes32(nonce);
        require(signatureAddress != address(0), "Signature address not defined");
        require(totalSupply() + amount <= MAX_SUPPLY, "Cap exceeded");
        require(_validateSignature(signature, abi.encodePacked(amount, nonce, _msgSender())), "Invalid signature");
        require(_nonces[voucher] == false, "Signature invalid or expired");
        _nonces[voucher] = true;
        _mint(_msgSender(), amount);
        emit Claimed(amount);
    }

    /**
     * @dev Checks if the `nonce` has already been used for the given `address`
     */
    function isClaimed(address to, uint256 nonce) external view returns (bool) {
        bytes32 voucher = bytes32(bytes20(to)) | bytes32(nonce);
        return _nonces[voucher];
    }

    /**
     * @dev Allows admins to mint on the users behalf
     */
    ...
    /**
     * @dev Sets the coins max supply that can be minted
     */
