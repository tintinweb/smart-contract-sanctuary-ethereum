...
contract Dummyy is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("Dummyy", "Dummyy"){}

    function whitelistMint(uint256 qty, bytes32[] calldata _merkleProof) external payable
    { 
        require(mintWhitelistEnabled, "Dummyy: Minting Whitelist Pause");
        if(walletWhitelist[msg.sender] < freeNFT) 
        {
           require(msg.value >= priceWhitelist * (qty - freeNFT),"Dummyy: Insufficient Eth Claim Free");
        }
        else
        {
           require(msg.value >= qty * priceWhitelist,"Dummyy: Insufficient Eth");
        }
        require(walletWhitelist[msg.sender] + qty <= maxPerWallet,"Dummyy: Max Per Wallet");
        require(qty <= maxPerTx, "Dummyy: Limit Per Transaction");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Dummyy: Not whitelisted");
        require(totalSupply() + qty <= maxSupply,"Dummyy: Soldout");
    ...
    {
        require(mintPublicEnabled, "Dummyy: Minting Public Pause");
        require(walletPublic[msg.sender] + qty <= maxPerWallet,"Dummyy: Max Per Wallet");
        require(qty <= maxPerTx, "Dummyy: Limit Per Transaction");
        require(totalSupply() + qty <= maxSupply,"Dummyy: Soldout");
        require(msg.value >= qty * pricePublic,"Dummyy: Insufficient Eth Normal");
