...
import "../interfaces/IERC721URIExtensible.sol";
import "../interfaces/IMintable.sol";
import "../interfaces/IAdmin.sol";
import "../interfaces/IERC721EnumerableExt.sol";
import "./interfaces/INanoCellCreator.sol";

contract NanoCell is
    IERC721URIExtensible,
    IERC721EnumerableExt,
    IMintable,
    IAdmin,
    Ownable,
    INanoCellCreator
{
    ...
    constructor() ERC721("NanoCell", "NLL") {
        super._addAdmin(msg.sender);
    }
    ...
    /**
     * @dev No need to check validity of token owner, because ERC721 checks it before burn
     */
    function burn(uint256 tokenId) public override {
        require(ownerOf(tokenId) == msg.sender, "You are not owner of NanoCell");
        super._burn(tokenId);
    }
    ...
    function mint(address to) payable external {
        ...
        super._mint(to, _nanoCells.current());
    }

    function create(address to) external override isAdmin returns (uint256 tokenId) {
        return _create(to);
    }

    function _create(address _to) internal returns (uint256 tokenId) {
        _nanoCells.increment();
        super._mint(_to, _nanoCells.current());
        tokenId = _nanoCells.current();
    }

    function createMultiple(address to, uint256 count) external isAdmin {
    ...
    function tokenURI(uint256 tokenId)
        public
        view
        override(ERC721, IERC721URIExtensible)
        returns (string memory)
    {
        return super.tokenURI(tokenId);
    }

    function setBaseURI(string calldata newUri) external override isAdmin {
...
import "../libs/AdvancedString.sol";

abstract contract IERC721URIExtensible is ERC721URIStorage {
    using AdvanceStrings for string;
    event ChangeBaseURI(string indexed uri, address indexed changer);
    string constant private EXTENSION = ".json";
    string private uri;

    function setBaseURI(string calldata newUri) external virtual;

    /**
     * @dev Returns tokenUri by tokenId as input with insert of
     * part of the necessary URI - .json extension
     */
    function tokenURI(uint256 tokenId)
        public
        view
        ...
        returns (string memory)
    {
        ...
        if (split[0].length() == _uri.length()) {
            return string(abi.encodePacked(_uri, EXTENSION));
        }
        return string(abi.encodePacked(split[0], EXTENSION, "?", split[1]));
    }

    /**
     * @dev Setter for base URI
     */
    function _setBaseURI(string calldata newUri) internal {
        uri = newUri;
...
abstract contract IERC721EnumerableExt is ERC721Enumerable {
    function totalOfOwner(address _owner)
        external
        view
