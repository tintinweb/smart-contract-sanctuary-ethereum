...
contract FenDAOMintPass is ERC1155Burnable, Ownable, Pausable, ReentrancyGuard {
    using ECDSA for bytes32;

    uint256[4] public MAX_SUPPLY = [
        7,   // Round Table
        25,  // Founder
        694, // OG
        2767 // Member
    ...
    function totalSupply() public view returns (uint256) {
        return supply[0] + supply[1] + supply[2] + supply[3];
    }
     ...
     */
    function airdrop(uint256 tokenId, address to) public lockable onlyOwner {
        supply[tokenId] += 1;

        require(supply[tokenId] <= MAX_SUPPLY[tokenId], "Exceeds max supply");
    ...
    function mint(
        ...
        require(
            signer == ECDSA.recover(ECDSA.toEthSignedMessageHash(keccak256(digest)), signature),
            "Invalid signature"
        );
