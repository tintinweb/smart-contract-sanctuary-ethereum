...
contract StakeMNTGToken is Ownable {
    ...
    function stake(address referrer, uint128 _amount, uint32 duration) external returns(uint32 stakedCounter) {//duration in seconds
        ...
        // this call will be done from frontend
        ...
        if (block.timestamp < rewardStartTime + rewardLifetime){
            ...
            directReferralReward[referrer] += mntgReward;//to sumup the rewards earned by referrer
            fixedRewardsAvailable -= mntgReward;
        }
    ...
    function calculateLevel2(address _referee, uint256 _amount) private {
        address level2referer = immediateReferral[immediateReferral[_referee]];
        if (level2referer != address(0)){
            token.safeTransfer(level2referer, _amount/100);
            teamReferralBenefits[level2referer] += _amount/100;
        }
     }

    function calculateLevel3(address _referee, uint256 _amount) private {
        address level3referer = immediateReferral[immediateReferral[immediateReferral[_referee]]];
        if (level3referer != address(0)){
            token.safeTransfer(level3referer, (_amount*50)/10000);//0.5
            teamReferralBenefits[level3referer] += (_amount*50)/10000;
        }
    }
    function calculateLevel4(address _referee, uint256 _amount) private {
        ...
        if (level4referer != address(0)){
            token.safeTransfer(level4referer, (_amount*25)/10000);//0.25
            teamReferralBenefits[level4referer] += (_amount*25)/10000;
        }
    }
    function calculateLevel5(address _referee, uint256 _amount) private {
        ...
        if (level5referer != address(0)){
            token.safeTransfer(level5referer, (_amount*10)/10000);//0.10
            teamReferralBenefits[level5referer] += (_amount*10)/10000;
        }
    }
    ...
    // this call will be done after after owner deposited reward from frontend by calling token contract
    ...
    // summed total amount staked by user
