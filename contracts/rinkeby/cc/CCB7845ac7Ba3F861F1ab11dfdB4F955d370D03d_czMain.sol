...
contract czMain is Ownable, Pausable, ReentrancyGuard {
    ...
    /** PUBLIC VARS */
    uint256 public MINT_PRICE_GENESIS = 0.13 ether;

    bool public PRE_SALE_STARTED;

    bool public PUBLIC_SALE_STARTED;
    uint16 public MAX_PUBLIC_SALE_MINTS = 2;
    ...
    modifier requireVariablesSet() {
        ...
        require(wallet3Address != address(0), "Main: Withdrawal address wallet3Address must be set");
        _;
    }

    /** PUBLIC FUNCTIONS */
    // TODO: Test it properly (payment, limits)
    ...
    // TODO: Test the pre-sale mint limit
    function mint(uint256 amount) external payable whenNotPaused nonReentrant onlyEOA {
        ...
        // TODO: This is not needed because it's checked in the ROAR contract already
        // uint16 tokensMinted = nftContract.tokensMinted();
        // uint256 maxTokens = nftContract.MAX_TOKENS();
        // require(tokensMinted + amount <= maxTokens, "Main: All tokens minted");
    ...
    function stakeManyGenesis(uint16[] memory tokenIds) external whenNotPaused nonReentrant onlyEOA {
        for(uint16 i = 0; i < tokenIds.length; i++) {
            ...
            require(nftContract.ownerOf(tokenId) == _msgSender(), "Main: You are not the owner of this nft");
    ...
    function claimManyGenesis(uint16[] memory tokenIds, bool unstake) external whenNotPaused nonReentrant onlyEOA {
        for(uint16 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            // require(getOwnerOfGenesis(tokenId) == _msgSender(), "Main: You are not the owner of this nft");
            require(nftContract.ownerOf(tokenId) == _msgSender(), "Main: You are not the owner of this nft");
            ...
            if (unstake) {    
                ...
                nftContract.refreshLock(tokenId, 1);
            }
    ...
    // TODO: Code this in such a way that rewards dial down the closer it gets to its "MAX_TOKENS" limitation (minted)
    ...
    // TODO: this should not be needed anymore as we can always use nftContract.ownerOf() as the NFT remains in the owner's wallet at all times while staked
    /*
    function getOwnerOfGenesis(uint256 tokenId) public view returns(address ownerOf) {
        if (nftContract.isTokenStaked(tokenId)) {
            iczNft.Stake memory stake = nftContract.getStake(tokenId);
            ownerOf = stake.owner;
        } else {
            ownerOf = nftContract.ownerOf(tokenId);
        }
        
        require(ownerOf != address(0), "Main: The owner cannot be address(0)");
    ...
    */
    ...
    // TODO: Code this as an airdrop
    function withdraw() external onlyOwner {
        uint256 totalAmount = address(this).balance;
        
        uint256 amountWallet1 = totalAmount * 25/100;
        uint256 amountWallet2 = totalAmount * 50/100;
        uint256 amountWallet3 = totalAmount - amountWallet1 - amountWallet2;
        ...
        (sent, ) = wallet3Address.call{value: amountWallet3}("");
        require(sent, "Main: Failed to send funds to wallet3Address");
    }

    function addToPresale(address addr, uint8 allowedToMint) external onlyOwner {
        _preSaleAddresses[addr] = allowedToMint;
    }

    function setWallet1Address(address addr) external onlyOwner {
        require(wallet1Address == address(0), "Main: You cannot change the address anymore");
        require(addr != address(0), "Main: Cannot be set to the zero address");
    ...
    function setWallet3Address(address addr) external onlyOwner {
        wallet3Address = addr;
    }
...
interface iczNft is IERC721Enumerable {
    ...
    function refreshLock(uint256 tokenId, uint8 lockType) external; // onlyAdmin
    ...
    function getAllStakedTokens(address owner) external view returns (uint256[] memory);
    function getAllBridgedTokens(address owner) external view returns (uint256[] memory);
    function getWalletOfOwner(address owner) external view returns (uint256[] memory);
}
