...
contract TheUkrainians is Ownable, ReentrancyGuard, ERC721A {
    ...
    constructor(address[] memory _team, uint8[] memory _allocation) ERC721A("The Ukrainians", "UANFT") {
        for (uint8 i = 0; i < _team.length; i++) {
            _safeMint(_team[i], _allocation[i]);
        }
    }
    ...
    function makeDonation() external onlyOwner nonReentrant {
        address UkraineWallet = 0x165CD37b4C644C2921454429E7F9358d18A45e14;
        payable(UkraineWallet).transfer(address(this).balance);
    }
