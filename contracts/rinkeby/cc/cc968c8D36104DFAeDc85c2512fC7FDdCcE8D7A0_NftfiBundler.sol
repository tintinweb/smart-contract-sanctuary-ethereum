 ...
 * @title NftfiBundler
 * @author NFTfi
 * @dev ERC998 Top-Down Composable Non-Fungible Token that supports ERC721, ERC1155 and ERC20 children.
 */
contract NftfiBundler is ERC9981155Extension, ERC998ERC20Extension, IBundleBuilder {
     ...
     */
    function buildBundle(BundleElements memory _bundleElements) external override returns (uint256) {
        ...
        for (uint256 i = 0; i < _bundleElements.erc721s.length; i++) {
            if (_bundleElements.erc721s[i].safeTransferable) {
                ...
                getChild(msg.sender, bundleId, _bundleElements.erc721s[i].tokenContract, _bundleElements.erc721s[i].id);
            }
        }

        for (uint256 i = 0; i < _bundleElements.erc20s.length; i++) {
            getERC20(msg.sender, bundleId, _bundleElements.erc20s[i].tokenContract, _bundleElements.erc20s[i].amount);
        }
    ...
    function addBundleElements(uint256 bundleId, BundleElements memory _bundleElements) external {
        // solhint-disable-previous-line no-empty-blocks
    }

    function removeBundleElements(uint256 bundleId, BundleElements memory _bundleElements) external {
        // solhint-disable-previous-line no-empty-blocks
    }

    function addAndRemoveBundleElements(
        uint256 bundleId,
        BundleElements memory _toAdd,
        BundleElements memory _toRemove
    ) external {
        // solhint-disable-previous-line no-empty-blocks
    }
...
abstract contract ERC9981155Extension is ERC998TopDown, IERC998ERC1155TopDown, IERC1155Receiver {
    ...
    function onERC1155Received(
        ...
        _validateAndReceive1155Child(_from, msg.sender, _id, _amount, _data);
...
abstract contract ERC998ERC20Extension is ERC998TopDown, IERC998ERC20TopDown, IERC998ERC20TopDownEnumerable {
    ...
    /**
     * @notice Transfer ERC20 tokens to another parent token
     * @param _fromTokenId The token to transfer from
     * @param _erc20Contract The ERC20 contract
     * @param _value The number of ERC20 tokens to transfer
     * @param _toTokenId The token to transfer to
     */
    function transferERC20ToBundle(
        ...
        _validateERC20Value(_value);
        _validateERC20Transfer(_fromTokenId);
        _removeERC20(_fromTokenId, _erc20Contract, _value);
        emit TransferERC20(_fromTokenId, address(this), _erc20Contract, _value);

        _receiveErc20Child(address(this), _toTokenId, _erc20Contract, _value);
    }

    /**
     * @notice Get ERC20 tokens from ERC20 contract using EIP-2612
     * @dev The ERC20 contract must support EIP-2612. User have to sign the approval off-chain
    ...
    function getERC20WithPermit(
        ...
        IERC20Permit(_erc20Contract).permit(_from, address(this), _value, _deadline, _v, _r, _s);
        getERC20(_from, _tokenId, _erc20Contract, _value);
    }
    ...
    function getERC20(
        ...
        //// TODO explore this more, I needed to diseble the check to make bundles work with signatures,
        //// but what are the security concerns? - @1up8192
        // Allow sender to deposit only their own funds for security concerns
...
interface IBundleBuilder {
     ...
     * @param _bundleElements - the lists of erc721-20-1155 tokens that are to be bundled
     */
    function buildBundle(BundleElements memory _bundleElements) external returns (uint256);
