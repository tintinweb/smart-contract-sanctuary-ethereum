...
contract ProposalSystem is Modifiers, Utils{
    ...
    function newProposal(uint _dealId, address _dealBuyer, address _dealSeller, 
                        uint256 _lastProposal, uint256 _proposalType, string memory _infOrTitle, uint256 _timeInDays, uint256 _subOrAddToken)
        ...
        if(_proposalType == 3){
            ...
            (bool flagAmountFee, uint256 _newAmount)= SafeMath.trySub(DS.getVar().deals[_dealId].amount, _subOrAddToken);
    ...
    function proposalChoose(uint _dealID, uint256 _lastProposal ,address _dealBuyer, address _dealSeller ,uint8 _choose) 
            openDeal(_dealID) isPartTaker(_dealID) external{
    ...
    function applyChanges(uint256 _dealId) openDeal(_dealId) isPartTaker(_dealId) internal{
        ...
        if (_type == 1){
            ...
            uint256 _addDays = deadlineCal(_historyUpdates.proposalsInfo[_lastUpdate].timeInDays, DS.getVar().defaultLifeTime);
        ...
        if(_type == 3){
            ...
            uint256 _discountAmount = (
                _historyUpdates.proposalsInfo[_lastUpdate].subOrAddToken * 10 ** DS.getVar().tokenDecimal[DS.getVar().deals[_dealId].coin]);
            ...
            if(!flagdiffgoods) revert();
            
            DS.getVar().deals[_dealId].goods = _diffGoods;
            DS.getVar().deals[_dealId].amount = _diffGoods;

            //refund to buyer the _diff
            (bool _successBuyer)= _token.transfer(DS.getVar().deals[_dealId].buyer, _diff);
