    ...
    ERC2981Upgradeable
{
    ...
    /// @dev Values:
    ///  1 = transfer while staked disabled
    /// !1 = transfer while staked enabled
    uint256 private _staked_transfer;
    ...
    /// @dev Enables transferring tokens while they are staked.
    modifier enableTransferWhileStaked() {
        _staked_transfer = 2;
    ...
    /// @inheritdoc ISerpenta
    function toggleStake(uint256[] calldata ids) external {
        if (!isStakingEnabled) revert StakingNotEnabled();

        uint256 len = ids.length;
        if (len == 0) revert StakingOrUnstakingNothing();
        ...
        for (uint256 i; i < len; i++) {
            ...
            uint64 lastStakedAt = ptr.lastStakedAt;
            ...
            if (lastStakedAt == 0) {
                ...
                ptr.lastStakedAt = timestamp;
                emit Stake(msg.sender, id, timestamp);
            } else {
                // unstake

                // equivalent to: ptr.lastStakedAt = 0;
                delete ptr.lastStakedAt;
                emit Unstake(msg.sender, id, timestamp, lastStakedAt);
            }
        }
    }

    /// @inheritdoc ISerpenta
    function isTokenStaked(uint256 id) public view returns (bool) {
        return getTokenInfo[id].lastStakedAt != 0;
    }

    /// @inheritdoc ISerpenta
    function stakedFor(uint256 id) public view returns (uint256) {
        uint256 lastStakedAt = getTokenInfo[id].lastStakedAt;
        return lastStakedAt == 0 ? 0 : block.timestamp - lastStakedAt;
    }

    /// @inheritdoc ISerpenta
    function transferFromWhileStaked(
        address from,
        address to,
        uint256 id
    ) external enableTransferWhileStaked {
        if (msg.sender != ownerOf(id)) revert TransferFromIncorrectOwner();
        transferFrom(from, to, id);
    }

    /// @inheritdoc ISerpenta
    function safeTransferFromWhileStaked(
        address from,
        address to,
        uint256 id
    ) external enableTransferWhileStaked {
    ...
    /// @dev Reverts if token is staked and not using special transfer function.
    ...
    function _beforeTokenTransfers(
        ...
        if (from != address(0))
            if (isTokenStaked(id))
                if (_staked_transfer == 1) revert TransferWhileStaked();
    }
}
    ...
    IERC721ABatchUpgradeable
{
    ...
    /// @dev Thrown when staking is not enabled.
    error StakingNotEnabled();

    /// @dev Thrown when transferring a token that is staked.
    error TransferWhileStaked();

    /// @dev Thrown when trying to stake or unstake nothing.
    error StakingOrUnstakingNothing();

    /* ------------------------------------------------------------------------------------------ */
    /*                                           EVENTS                                           */
    /* ------------------------------------------------------------------------------------------ */

    /// @dev Emitted when staking a token.
    /// @param user The address that staked the token
    /// @param id The ID that was unstaked
    /// @param timestamp When the token was staked
    event Stake(address indexed user, uint256 indexed id, uint64 timestamp);

    /// @dev Emitted when unstaking a token.
    /// @param user The address that unstaked the token
    /// @param id The token ID that was unstaked
    /// @param timestamp When the token was unstaked
    /// @param lastStakedAt When the token was last staked
    event Unstake(address indexed user, uint256 indexed id, uint64 timestamp, uint64 lastStakedAt);
    ...
    /// @notice Returns whether staking is enabled.
    ...
    /// @notice Stakes or unstakes `ids` depending on their previous status.
    /// @param ids The token IDs to stake or unstake
    function toggleStake(uint256[] calldata ids) external;

    /// @notice Returns whether a token is staked.
    /// @param id The token ID
    function isTokenStaked(uint256 id) external view returns (bool);

    /// @notice Returns how long a token has been staked for.
    /// @param id The token ID
    function stakedFor(uint256 id) external view returns (uint256);

    /// @notice Transfers `id` token from `from` to `to` while it is staked.
    ...
    /// @notice Safely transfers `id` token from `from` to `to` while it is staked.
    /// The caller must be the owner of `id`.
