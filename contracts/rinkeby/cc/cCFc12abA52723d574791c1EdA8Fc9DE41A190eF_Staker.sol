...
contract Staker {
  ...
  modifier deadlinePassed(bool requireDeadlinePassed) {
    ...
    if (requireDeadlinePassed) {
      require(timeRemaining <= 0, "Deadline gecilmasdedi");
    } else {
      require(timeRemaining > 0, "Deadline geciasdldi");
    }
    _;
  }
  ...
  modifier stakingNotCompleted() {
    ...
    require(!completed, "Staking periot tamamlandi");
    _;
  }
  ...
  function withdraw() public deadlinePassed(true) stakingNotCompleted {
      require(openForWithdraw, "cekemeasdzsin");

      uint256 userBalance = balances[msg.sender];

      require(userBalance > 0, "balance asdsifir");

      balances[msg.sender] = 0;

      (bool sent, ) = msg.sender.call{value: userBalance}("");

      require(sent, "gonderilmeasddi");
  }
