...
contract StorageOperator is Ownable {
    ...
    event BaseContract(address token);
    ...
    constructor(address _fundAddress, uint256 _storageCostRate) {
    ...
    /**
     * @notice Used to know the isUpkeepNeeded or not
     * @return  bool(true/false)
     */
    function _isUpkeepNeeded() internal view returns (bool) {
        uint256 timestamp = block.timestamp;
        uint256 day = DateTime.getDay(timestamp);
        uint256 dayDifference = DateTime.diffDays(
            lastPerformTimestamp,
            timestamp
        );
        return (day == performDate && dayDifference > 1);
    }
    ...
    /**
     * @notice The external method is used to set baseContract (token address) to access BWGToken contract by using IBWGToken interface.
     * Owner permission required
     * @param newBaseContract, The requested token address to set
     * Emits {BaseContract} event.
     */
    function setBaseContract(address newBaseContract) external onlyOwner {
        baseContract = newBaseContract;
        emit BaseContract(newBaseContract);
    }

    /**     
     * @return BWGtoken contract address
     */
    function getBaseContract() external view returns (address) {
        return baseContract;
    }
    ...
    function performUpkeep(
        address[] memory costableWallets
    ) external  {       
            ...
            ITokenTransferOperator tokenTransferOperator = ITokenTransferOperator(_defaltOperators[0]); 
            _deductStorageCost(token, tokenTransferOperator, costableWallets);           
    
    }


    /**
     * @notice The internal method called from chainlink keepers (performUpkeep) usually run on the first day of every month to deduct the storage cost.
     * transfer storage cost to fundAddress on behalf of token holder by using ITokenTransferOperator Interface
     * @param token, the BWGToken token contract        
     * @param tokenTransferOperator, the BWG token operator contract     
     * Emits {StorageCostSummery} events and also {Transfer} and {Sent} in every token transfer.
     */

    function _deductStorageCost(IERC777 token,  ITokenTransferOperator tokenTransferOperator , address[] memory _costableWallets) internal {   
 
        
        address _fundAddress = fundAddress;
        uint256 _totalStorageCost;
        uint256 _storageCostRate = storageCostRate;
 
        uint256 _totalWallet = _costableWallets.length;     
      
        uint256 batchLastIndex = _batchLastIndex(_totalWallet);
       
      
        for (uint256 index = lastIndex; index < batchLastIndex; index++) {
            address _balanceWallet = _costableWallets[index];
            uint256 _balance = token.balanceOf(_balanceWallet);

            // Costfree wallets and wallets with a balance under the threshold are not allowed to deduct tokens as costs.
            // if (
            //     !iBWGToken.isCostFreeWallet(_balanceWallet) &&
            //     _balance >= _threshold
            // ) {
                // calculating storage cost to deduct
                // dividing _storageCostRate by 10000 because _storageCostRate is already multipied by 100 
                // like default_storageCostRate is 66 incase of .66 (.66 %)
                uint256 _storageCost = _balance.mul(_storageCostRate).div(
                    uint256(10000)
                );

                // total storage cost for storage cost summery log
                _totalStorageCost = _totalStorageCost.add(_storageCost);

                // send token to fund address as storage cost by calling operatorSend method
    ...
    /**
     * @notice The internal method to get lastindex of current costduction batch    
     * @param totalWallet, the totalcostfree wallet  
     * @return batchLastIndex      
     */
    function _batchLastIndex(uint256 totalWallet) internal view returns ( uint256 batchLastIndex) {           
        if (totalWallet > (lastIndex + batchSize)){
            batchLastIndex = lastIndex + batchSize;            
        }else {            
            batchLastIndex = totalWallet;
        }
        return batchLastIndex;   
    }
