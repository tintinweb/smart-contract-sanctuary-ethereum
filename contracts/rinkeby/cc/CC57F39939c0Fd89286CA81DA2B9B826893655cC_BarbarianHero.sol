...
import "../base/HeroBase.sol";

contract BarbarianHero is HeroBase {
  ...
  string private constant _URI = "https://raw.githubusercontent.com/tetu-io/tetu-game-assets/master/heroes/barbarian.png";
...
abstract contract HeroBase is NftBase, IHero {
  ...
  /// @notice Version of the contract
  /// @dev Should be incremented when contract changed
  string public constant HERO_BASE_VERSION = "1.0.0";
  uint public constant KILL_PENALTY = 70;
  bytes32 internal constant _PAY_TOKEN_SLOT = bytes32(uint256(keccak256("eip1967.Hero.PayToken")) - 1);
  bytes32 internal constant _PAY_TOKEN_AMOUNT_SLOT = bytes32(uint256(keccak256("eip1967.Hero.PayTokenAmount")) - 1);
  bytes32 internal constant _INDEX_SLOT = bytes32(uint256(keccak256("eip1967.Hero.Index")) - 1);
  bytes32 internal constant _HERO_CLASS_SLOT = bytes32(uint256(keccak256("eip1967.Hero.HeroClass")) - 1);

  // ---- VARIABLES ----
  bool public override isHero;
  mapping(uint => TokenTreasury) internal _tokenTreasures;
  ...
  function __HeroBase_init(
    ...
    _PAY_TOKEN_SLOT.set(payToken_);
    _PAY_TOKEN_AMOUNT_SLOT.set(payTokenAmount_);
    _HERO_CLASS_SLOT.set(heroClass_);
    isHero = true;
  }
  ...
  function _beforeTokenTransfer(
    ...
    require(currentDungeon[tokenId] == address(0), "Hero in dungeon");

    // items should be taken off
    for (uint i = 1; i < uint(IItem.ItemType.SKILL);) {
      require(_statController().heroItemSlot(address(this), tokenId, i).token == address(0), "Item equipped");
    unchecked{++i;}
    }
  ...
  function tokenTreasures(uint tokenId) external view override returns (TokenTreasury memory) {
    return _tokenTreasures[tokenId];
  }

  function attributes(uint tokenId) external view override returns (IStatController.Attributes memory) {
    return _statController().heroAttributes(address(this), tokenId);
  }

  function stats(uint tokenId) external view override returns (IStatController.ChangeableStats memory) {
    return _statController().heroStats(address(this), tokenId);
  }

  function payToken() external view override returns (address) {
    return _PAY_TOKEN_SLOT.getAddress();
  }

  function payTokenAmount() external view override returns (uint) {
    return _PAY_TOKEN_AMOUNT_SLOT.getUint();
  }

  function heroClass() external view override returns (uint) {
    return _HERO_CLASS_SLOT.getUint();
  }

  function killRewardForOwner(uint tokenId) external view override returns (uint) {
    return _tokenTreasures[tokenId].amount * KILL_PENALTY / 100;
  }

  function isReadyToFight(uint tokenId) external view override returns (bool) {
    return lastFightTs[tokenId] + IController(_controller()).fightDelay() < block.timestamp;
  }

  function isAlive(uint tokenId) external view override returns (bool) {
    return _statController().isHeroAlive(address(this), tokenId);
  }
  ...
  function _specificURI(uint tokenId) internal view override returns (string memory) {
    uint level = _statController().heroStats(address(this), tokenId).level;
    if (level < 10) {
      return "";
    } else if (level < 25) {
      return _heroUriByStatus[1];
    } else if (level < 50) {
      return _heroUriByStatus[2];
    } else if (level < 65) {
      return _heroUriByStatus[3];
    } else if (level < 75) {
      return _heroUriByStatus[4];
    } else if (level < 85) {
      return _heroUriByStatus[5];
    } else if (level < 99) {
      return _heroUriByStatus[6];
    } else {
      return _heroUriByStatus[7];
    }
  }
  ...
  function create() external override returns (uint) {
    uint id = _INDEX_SLOT.getUint();
    _INDEX_SLOT.set(id + 1);
    _safeMint(msg.sender, id);

    address token = _PAY_TOKEN_SLOT.getAddress();
    uint amount = _PAY_TOKEN_AMOUNT_SLOT.getUint();

    _tokenTreasures[id] = TokenTreasury({
    id : id,
    token : token,
    amount : amount,
    ...
    _statController().initNewHero(address(this), id, _HERO_CLASS_SLOT.getUint());
  ...
  function takeOff(address item, uint itemId, uint heroId, uint itemSlot) external {
    onlyOwner(heroId);
    require(currentDungeon[heroId] == address(0), "Hero in dungeon");

    IItem(item).takeOff(itemId, address(this), heroId, itemSlot, msg.sender);
  }

  function kill(uint heroId) external override returns (IStatController.NftItem[] memory) {
    onlyDungeon();

    // take off all equipments and transfer to sender
    IStatController.NftItem[] memory drop = new IStatController.NftItem[](uint(IItem.ItemType.SKILL));
    uint dropCount;
    for (uint i = 1; i < uint(IItem.ItemType.SKILL);) {
      IStatController.NftItem memory nft = _statController().heroItemSlot(address(this), heroId, i);
      if (nft.token != address(0)) {
        IItem(nft.token).takeOff(nft.tokenId, address(this), heroId, i, msg.sender);
        drop[i] = nft;
      unchecked{++dropCount;}
      }
    unchecked{++i;}
    }
    IStatController.NftItem[] memory dropSorted = new IStatController.NftItem[](uint(dropCount));
    uint j;
    for (uint i; i < uint(IItem.ItemType.SKILL);) {
      if (drop[i].token != address(0)) {
        dropSorted[j] = drop[i];
      unchecked{++j;}
      }
    unchecked{++i;}
    ...
    TokenTreasury memory tokenTreasury = _tokenTreasures[heroId];

    // user or dungeon will receive only some percent of treasury (70% currently)
    uint toOwner = tokenTreasury.amount * KILL_PENALTY / 100;
    uint toTreasury = (tokenTreasury.amount - toOwner) / 2;
    uint toController = tokenTreasury.amount - toOwner - toTreasury;

    IERC20(tokenTreasury.token).safeTransfer(
      msg.sender,
      toOwner
    );
    IERC20(tokenTreasury.token).safeTransfer(
  ...
  function levelUp(uint tokenId, IStatController.CoreAttributes calldata change) external override {
    onlyOwner(tokenId);

    address token = _PAY_TOKEN_SLOT.getAddress();
    uint amount = _PAY_TOKEN_AMOUNT_SLOT.getUint();

    TokenTreasury memory treasury = _tokenTreasures[tokenId];

    _tokenTreasures[tokenId] = TokenTreasury({
    id : tokenId,
    token : token,
    amount : amount + treasury.amount,
    blockNumber : treasury.blockNumber,
    timestamp : treasury.timestamp
    });

    _statController().levelUp(
      address(this),
      tokenId,
      _HERO_CLASS_SLOT.getUint(),
      change
    );
