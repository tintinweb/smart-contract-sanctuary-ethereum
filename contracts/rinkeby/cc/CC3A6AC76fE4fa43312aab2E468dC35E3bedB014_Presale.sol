...
contract Presale is Ownable, ReentrancyGuard {
    ...
    // uint256 public totalDepositedUsdcBalance;

    uint256 public totalDepositedEthBalance;

    uint256 public depositRate = 32; // Token price. i.e. 0.9 USDC
    ...
    function deposit() public payable nonReentrant {
        ...
        uint256 satmTokenAmount = ethAmount.mul(10 ** 5).div(depositRate);
    ...
    //     require(_amount > 0, "USDC Amount is less than zero");
    //     require(presaleEnabled == true, "Presale: Presale is not available");

    //     uint256 satmTokenAmount = _amount.mul(10).div(depositRate);

    //     usdc.safeTransferFrom(msg.sender, masterWallet, _amount);
    //     satm.safeTransferFrom(masterWallet, msg.sender, satmTokenAmount);

    //     totalDepositedUsdcBalance = totalDepositedUsdcBalance + _amount;
    ...
    function updateMasterWalletAddress(address _newWalletAddr) public onlyOwner{
        require(address(_newWalletAddr) != address(0), "Master Wallet address should not be zero address");
    ...
    function withdraw() public onlyOwner {
        ...
        // send all Ether to the master wallet
        // master wallet can receive Ether since the address of owner is payable
