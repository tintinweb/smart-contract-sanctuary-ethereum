...
contract GMCafeMoo is Ownable, IERC165, IERC721, IERC721Metadata {
	...
	uint256 constant CLAIM_DELAY = 3 minutes;
	
	/*
	struct MooData { // 32 bytes
		address owner;    // 0xFFFFFFFFFFFFFFFFFFFFFFFF0000000000000000000000000000000000000000 << 0
		uint32 block0;    // 0xFFFFFFFFFFFFFFFF00000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF << 160
		uint32 transfers; // 0xFFFFFFFF00000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF << 192
		uint8 lockStyle;  // 0xFFFFFF00FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF << 224 
		uint8 tag;        // 0xFFFF00FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF << 232
		uint16 moo;       // 0x0000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF << 240
	}
	*/
	uint256 constant BLOCK_SHIFT = 160;
	uint256 constant TRANSFER_SHIFT = 192;	
	uint256 constant LOCK_SHIFT = 224;
	uint256 constant TAG_SHIFT = 232;
	uint256 constant MOO_SHIFT = 240;

	uint256 constant MOO_TAG_MASK = ~((1 << TAG_SHIFT) - 1);
	uint256 constant TAG_CLEAR = ~(0xFF << TAG_SHIFT); 
	uint256 constant LOCK_CLEAR = ~(0xFF << LOCK_SHIFT);
	uint256 constant BLOCK_CLEAR = ~(0xFFFFFFFF << BLOCK_SHIFT);

	uint256 constant TOGGLE_LOCKED = 1;
	uint256 constant SECURE_LOCKED = 2;

	struct Unlock { // 64 bytes
	...
	function setTag(uint256 moo, uint8 tag) onlyOwner public {
	...
	function setProvenanceHash(bytes32 hash) onlyOwner public {
		if (_provenanceHash != bytes32(0)) revert NotAllowed(); // can only be set once
	...
	function tokenURI(uint256 moo) public view returns (string memory uri) {
		...
		if (_tokenURLSearch) {
			...
			if (data != 0) {
				...
				dst = _appendIntHex(dst, data >> 128, 32); // + 32
				dst = _appendIntHex(dst, data, 32);        // + 32
			}
		}
		assembly {
	...
	function getHerd() public view returns (bytes32[] memory ret) {
		unchecked {
			...
			while (moo < HERD_SIZE) {
				uint256 data = _moos[++moo];
				if (uint160(data) == 0) continue;
				// replace block with delta
				data = (data & BLOCK_CLEAR) | ((block.number - uint32(data >> BLOCK_SHIFT)) << BLOCK_SHIFT); 
				assembly {
					ptr := add(ptr, 32)
					mstore(ptr, data)
				}
	...
	function getMoo(uint256 moo) public view returns (
			address owner, uint256 transfers, uint256 blockRecv, uint256 blocksHeld, 
			uint256 tag, uint256 lockStyle, uint256 unlockPrice
	) {
		uint256 data = _moos[moo];
		owner = address(uint160(data));
		if (owner != address(0)) {
			transfers = uint32(data >> TRANSFER_SHIFT);
			blockRecv = uint32(data >> BLOCK_SHIFT);
			unchecked {
				blocksHeld = block.number - blockRecv;
			}
			tag = uint8(data >> TAG_SHIFT);
			lockStyle = _lockStyle(data);
			if (lockStyle == SECURE_LOCKED) {
				unlockPrice = _unlocks[moo].price;
			}
		}
	}
	function moosOf(address owner) public view returns (uint256[] memory moos) {
		moos = new uint256[](HERD_SIZE);
		uint256 n;
		uint256 moo;
		while (moo < HERD_SIZE) {
			unchecked {
	...
	// locking
	function _lockStyle(uint256 data) private pure returns (uint256) {
		return uint8(data >> LOCK_SHIFT);
	}
	function isLocked(uint256 moo) public view returns (bool) {
		return _lockStyle(_moos[moo]) != 0;
	}
	function lockMoo(uint256 moo) public { 
		uint256 data = _moos[moo];		
		if (_lockStyle(data) != 0) revert InvalidLockState(); // already locked
		_requireApproval(address(uint160(data)), moo);
		_moos[moo] = data | (TOGGLE_LOCKED << LOCK_SHIFT);
		emit MooLocked(moo);
	}
	function lockMoo(uint256 moo, bytes32 hash, uint256 price) public {
		...
		if (_lockStyle(data) != 0) revert InvalidLockState(); // already locked
		_requireApproval(address(uint160(data)), moo);
		_moos[moo] = data | (SECURE_LOCKED << LOCK_SHIFT);
		_unlocks[moo] = Unlock({hash: hash, price: price});
		emit MooLocked(moo);
	}
	function unlockMoo(uint256 moo) payable public { 
		uint256 data = _moos[moo];
		uint256 lockStyle = _lockStyle(data);
		if (lockStyle == 0) revert InvalidLockState(); // not locked
		_requireApproval(address(uint160(data)), moo); 
		if (lockStyle == SECURE_LOCKED) { // secure locked => check price
			if (_unlocks[moo].price > msg.value) revert InvalidLockState(); // insufficient
			delete _unlocks[moo];
		}
		_moos[moo] = data & LOCK_CLEAR; // reset lock
		emit MooUnlocked(moo);
	}
	function unlockMoo(uint256 moo, string memory password) public {
		uint256 data = _moos[moo];	
		if (_lockStyle(data) != SECURE_LOCKED) revert InvalidLockState(); // not secure locked
		_requireApproval(address(uint160(data)), moo); 
		bytes32 hash = keccak256(bytes(password));
		if (_unlocks[moo].hash != hash) { // wrong password
	...
	function migrateMoo(uint256 token) public {
		if (OPENSEA.balanceOf(msg.sender, token) == 0) revert NotOpenSeaOwner(token);
		uint256 moo = _mooFromToken(token);
		if (uint160(_moos[moo]) != 0) revert AlreadyMigrated(moo);
	...
	function transferFrom(address from, address to, uint256 moo) public {
		...
		if (_lockStyle(data) != 0) revert InvalidLockState(); // moo is locked
		_requireApproval(from, moo);
		delete _tokenApprovals[moo]; // clear token approval
		unchecked {
			_moos[moo] = (data & MOO_TAG_MASK) // keep moo and tag
				| uint256(uint160(to)) // new owner
				| (block.number << BLOCK_SHIFT) // current block height
				| ((((data >> TRANSFER_SHIFT) + 1) & 0xFFFFFFFF) << TRANSFER_SHIFT); // increase transfer count
			_balances[from]--;
			_balances[to]++;
		}
	...
	function _mooFromToken(uint256 token) public pure returns (uint256) {
		uint256 index = (token >> 40) & 0xFFF; // see above
