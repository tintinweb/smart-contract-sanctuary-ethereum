...
contract DynamicTaxHandler is ITaxHandler, ExchangePoolProcessor {
    ...
    /// @notice Immutable list of blacklisted addresses.
    ...
    function getTax(
        ...
        uint256 poolBalanceBasisPoint = token.balanceOf(primaryPool) / 10000;
        ...
        if (_exchangePools.contains(benefactor)) {
            return _getBuyTaxBasisPoints(amount, poolBalanceBasisPoint);
        } else {
            return _getSellTaxBasisPoints(amount, poolBalanceBasisPoint);
        }
    }
    ...
    function setBuyTaxCheckpoints(
        ...
        for (uint256 i = 0; i < _buyTaxPoints; i++) {
            emit BuyTaxCheckpointRemoved(
                _buyTaxBasisPoints[i].threshold,
                _buyTaxBasisPoints[i].basisPoints
            );
        ...
        for (uint256 i = 0; i < thresholds.length; i++) {
            _buyTaxBasisPoints[i] = TaxCheckpoint({
                basisPoints: basisPoints[i],
                threshold: thresholds[i]
            });

            emit BuyTaxCheckpointAdded(
                _buyTaxBasisPoints[i].threshold,
                _buyTaxBasisPoints[i].basisPoints
            );
        }
     ...
     */
    function setSellTaxCheckpoints(
        uint256[] memory thresholds,
        uint256[] memory basisPoints
    ) external onlyOwner {
        require(
        ...
        for (uint256 i = 0; i < _sellTaxPoints; i++) {
            emit SellTaxCheckpointRemoved(
                _sellTaxBasisPoints[i].threshold,
                _sellTaxBasisPoints[i].basisPoints
            );
        ...
        for (uint256 i = 0; i < thresholds.length; i++) {
            _sellTaxBasisPoints[i] = TaxCheckpoint({
                basisPoints: basisPoints[i],
                threshold: thresholds[i]
            });

            emit SellTaxCheckpointAdded(
                _sellTaxBasisPoints[i].threshold,
                _sellTaxBasisPoints[i].basisPoints
            );
        }
    ...
    function _getBuyTaxBasisPoints(
        uint256 amount,
        uint256 poolBalance
    ) private view returns (uint256 taxBasisPoints) {
        taxBasisPoints = baseBuyTaxBasisPoints;
        uint256 basisPoints = amount * 100 / poolBalance;
    ...
    function _getSellTaxBasisPoints(
        uint256 amount,
        uint256 poolBalance
    ) private view returns (uint256 taxBasisPoints) {
