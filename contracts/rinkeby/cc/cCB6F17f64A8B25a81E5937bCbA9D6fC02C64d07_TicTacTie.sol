...
contract TicTacTie is ERC721 {
    uint256 _currentTokenId;
    uint256 _maxSupply = 70;

    uint256 _price = 0.001 ether;
    uint256 _tiesMaxSupply = 10000;
    uint256 constant _minTieTokenId = 500;
    uint256 _tieTokenId = _minTieTokenId;
    uint256 constant _victoriesToWin = 5;
    ...
    mapping(uint256 => uint256) public opponent;
    ...
    function mint(uint256 tokenId) public payable {
        ...
        require(balanceOf(msg.sender) == 0, "Address already has a board."); // Needed?
        require(tokenId > 0 && tokenId <= _maxSupply, "Token id out of range");
    ...
    function victoriesLeft(uint256 boardIndex) public view returns (uint256) {
        uint256 currentVictories = _countVictories(boards[boardIndex]);
        if (_victoriesToWin >= currentVictories) {
            return _victoriesToWin - currentVictories;
        } else {
            return 0;
        }
    }
    ...
    function challenge(uint256 board1Index, uint256 board2Index) public {
        // player has to be in a win state
        require(ownerOf(board1Index) == msg.sender, "You don't own this board");
        uint256 boardPlayer = boards[board1Index];
        uint256 boardOpponent = boards[board2Index];

        require(boardPlayer > 0, "You don't have any board yet");
        require(boardOpponent > 0, "No one owns that board.");

        require(_isWaiting(boardPlayer), "You can't challenge now");
        require(_isWaiting(boardOpponent), "Opponend cannot challenge now");

        boards[board1Index] =
            (boardPlayer & statusMask & lastActionBlockMask) |
            (playingFlag << statusPosition) |
            ((block.number - 1) << lastActionBlockPosition); // first turn to who challenges
        boards[board2Index] =
            (boardOpponent & statusMask & lastActionBlockMask) |
            (playingFlag << statusPosition) |
    ...
    function whoAbandoned(uint256 boardIndex) public view returns (uint256) {
        ...
        require(!_isWaiting(board), "Board not playing");
        require(!_isWaiting(boardOpponent), "Opponent board not playing");
    ...
    function endGame(uint256 boardIndex) public {
        ...
        uint256 opponentBoardIndex = opponent[boardIndex];
        require(opponentBoardIndex > 0, "Board doesn't have an opponent");

        uint256 boardOpponent = boards[opponentBoardIndex];
        require(
            _didPlayer1Abandon(boardOpponent, board),
            "Opponent did not abandon"
        );
    ...
    function _mintTie() internal {
        unchecked {
            _tieTokenId++;
        }
        super._safeMint(msg.sender, _tieTokenId);
    }
    ...
    function play(uint256 boardIndex, uint16 coordinate) public {
        require(ownerOf(boardIndex) == msg.sender, "You don't own this board");
        require(coordinate > 0 && coordinate <= 1 << 8, "Move outside board");

        uint256 boardPlayer = boards[boardIndex];
        uint256 opponentBoardIndex = opponent[boardIndex];
        require(opponentBoardIndex > 0, "You don't have an opponent");
    ...
    function getBoardSVG(uint256 tokenId) public view returns (bytes memory) {
        uint256 currentBoard = boards[tokenId];
        uint256 opponentTokenId = opponent[tokenId];
        uint256 opponentBoard = 0;
        if (opponentTokenId > 0) {
            opponentBoard = boards[opponentTokenId];
        }

        uint256 xBoard;
        uint256 oBoard;
        if (tokenId < opponentTokenId) {
            xBoard = currentBoard;
            oBoard = opponentBoard;
        } else {
            oBoard = currentBoard;
            xBoard = opponentBoard;
        }
        ...
        return
            Board.buildBoard(
                ...
                _boardState(xBoard),
                _boardState(oBoard),
                _countVictories(currentBoard)
            );
    }

    function getTieSVG(uint256 tokenId) public pure returns (string memory) {
        return string(Tie.buildTie(tokenId, 42));
    }

    function getAllBoardsSVG() public view returns (string[70] memory) {
        string[70] memory boardSVGs;
        for (uint256 i = 1; i <= _maxSupply; i++) {
            boardSVGs[i - 1] = string(getBoardSVG(i));
        }
        ...
        returns (string memory)
    {
        ...
        bytes memory image;
        if (tokenId > _minTieTokenId) {
            // it's a tie
            image = Tie.buildTie(tokenId, 42);
        } else {
            // it's a board
            image = getBoardSVG(tokenId);
        }
        bytes memory json = abi.encodePacked(
            '{"name":"',
            flags[tokenId],
            '","description":"","created_by":"Inner Space","image":"',
            image,
            '"}'
        );
...
library Tie {
    uint256 constant colorCombiMonotone = 0;
    uint256 constant colorCombiAdjacent = 1;
    uint256 constant colorCombiTriad = 2;
    uint256 constant colorCombiOpposite = 3;

    string constant svg1 =
        "<?xml version='1.0' encoding='iso-8859-1'?><!-- Generator: Adobe Illustrator 19.0.0, SVG Export Plug-In . SVG Version: 6.00 Build 0) --><svg version='1.1' id='Layer_1' width='600' height='600' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' x='0px' y='0px'viewBox='0 0 600 600'><defs><style type='text/css'><![CDATA[#tieStroke {stroke: ";
    string constant colorStroke = "hsl(0, 100%, 80%)";
    string constant svg2 = ";}#background {fill: ";
    string constant colorBg = "hsl(180, 100%, 10%)";
    string constant svg3 =
        ";}]]></style></defs><mask id='tieMask'><pathd='M 280 80 L 250 480 L 300 530 L 350 480 L 320 80 L 325 60 L 300 50 L 275 60 L 280 80 L 320 80'fill='white'stroke='red'stroke-width='6'stroke-linejoin='round'/></mask><rect width='100%' height='100%' id='background'/><pathd='M 280 80 L 250 480 L 300 530 L 350 480 L 320 80 L 325 60 L 300 50 L 275 60 L 280 80 L 320 80'fill='transparent'stroke='red'stroke-width='6'stroke-linejoin='round'id='tie'/><g mask='url(#tieMask)' ><rect x='250' y='100' height='15' fill='#8E008E' width='200' transform='rotate(20)'/><rect x='250' y='115' height='15' fill='#400098' width='200' transform='rotate(20)'/><rect x='250' y='130' height='15' fill='#00C0C0' width='200' transform='rotate(20)'/><rect x='250' y='145' height='15' fill='#008E00' width='200' transform='rotate(20)'/><rect x='250' y='160' height='15' fill='#FFFF00' width='200' transform='rotate(20)'/><rect x='250' y='175' height='15' fill='#FF8E00' width='200' transform='rotate(20)'/><rect x='250' y='190' height='15' fill='#FF0000' width='200' transform='rotate(20)'/></g><pathd='M 280 80 L 250 480 L 300 530 L 350 480 L 320 80 L 325 60 L 300 50 L 275 60 L 280 80 L 320 80'fill='transparent'stroke='red'stroke-width='10'stroke-linejoin='round'id='tieStroke'/></svg>";

    function buildTie(uint256 tokenId, uint256 randomness)
        public
        pure
        returns (bytes memory)
    {
        //uint256 baseHue = randomness
        return abi.encodePacked(svg1);
    }
}
