...
contract ClaimVoting is IClaimVoting, Initializable, AbstractDependant {
    ...
    function lockedBMIAmount(uint256 claimIndex) external view override returns (uint256) {
        ...
        returns (AllClaimInfo[] memory _allClaimsInfo)
    {
        ...
        for (uint256 i = offset; i < to; i++) {
            ...
            if (claimingRegistry.canClaimBeCalculatedByAnyone(index)) {
                _allClaimsInfo[i - offset].bmiCalculationReward = claimingRegistry
        ...
        returns (MyClaimInfo[] memory _myClaimsInfo)
    {
        ...
        for (uint256 i = offset; i < to; i++) {
            ...
            if (_myClaimsInfo[i - offset].finalVerdict == IClaimingRegistry.ClaimStatus.ACCEPTED) {
                ...
                _myClaimsInfo[i - offset].bmiCalculationReward = claimingRegistry
                    .getBMIRewardForCalculation(index);
            }
        }
    }
        ...
        returns (MyVoteInfo[] memory _myVotesInfo, VotesUpdatesInfo[] memory _myVotesUpdatesInfo)
    {
        ...
        for (uint256 i = offset; i < to; i++) {
            ...
            if (_isVoteExpired(voteIndex)) {
                _myVotesUpdatesInfo[i - offset].stakeChange = int256(
                ...
                if (
                    ...
                    (uint256 stblAmount, uint256 bmiAmount, uint256 newReputation) =
                    ...
                    (uint256 bmiAmount, uint256 newReputation) =
                    ...
                    (uint256 bmiPenaltyAmount, uint256 newReputation) =
