...
contract GristleKing is ERC721A, DAOable {
    ...
    function teamMint(uint64 quantity, bytes32[] calldata merkleProof) external {
        ...
        if (!MerkleProof.verify(merkleProof, merkleRoots[1], keccak256(abi.encodePacked(msg.sender)))) revert InvalidMerkleProof();
    ...
    function whitelistMint(uint64 quantity, bytes32[] calldata merkleProof) external payable {
        ...
        if (!MerkleProof.verify(merkleProof, merkleRoots[2], keccak256(abi.encodePacked(msg.sender)))) revert InvalidMerkleProof();
        if (msg.value < prices[2] * quantity) revert InvalidEthereumValue();
        internalMint(quantity);
    }

    function publicMint(uint64 quantity) external payable {
        if (mintingTier < 3) revert PublicMintingDisabled();
        if (msg.value < prices[3] * quantity) revert InvalidEthereumValue();
        internalMint(quantity);
    }

    function internalMint(uint64 quantity) internal {
    ...
    function setMerkleRoot(uint256 _tier, bytes32 _merkleRoot) external onlyPartner {
        merkleRoots[_tier] = _merkleRoot;
    }

    function setPrice(uint256 _tier, uint256 _price) external onlyPartner {
        if (mintingTier > 0) revert MintingInProgress();
        prices[_tier] = _price;
    }

    function setMintingTier(uint256 _tier) external onlyPartner {
        mintingTier = _tier;
    }
