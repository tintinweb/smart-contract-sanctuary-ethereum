...
contract ProjectFactory is Ownable, BlackHolePrevention {
    ...
    IRegistryConsumer   immutable   public TheRegistry = IRegistryConsumer(0x7B3c35D7D339A3A0f70D06b8852a35D674bEB1dC);
    ...
    struct ProjectStruct {
        string      _name;
        address     _owner;
        uint8       _tokenContracts;
        uint8       _saleContracts;
    }

    mapping(uint256 => ProjectStruct) public project;
    mapping(uint256 => address[]) public projectTokens;
    mapping(uint256 => address[]) public projectSales;

    uint256 public projectCount = 0;

    event NewProject(uint256 _projectCount, string _name);
    ...
    function LaunchProject(
        ...
        projectCount++;
        saleConfig.projectID = projectCount;
        tokenConfig.projectID = projectCount;

        // Launch new token contract
        address _newTokenAddress = TokenFactory.deploy(tokenConfig);
        ...
        TokenFactory.finishSetup(_newTokenAddress, _newSaleAddress, msg.sender);
        ...
        // index data for viewing
        // projectTokens[projectCount] = new address[]();
        projectTokens[projectCount].push(_newTokenAddress);

        // projectSales[projectCount] = new address[]();
        projectSales[projectCount].push(_newSaleAddress);

        project[projectCount] = ProjectStruct(
            name,
            msg.sender,
            uint8(projectTokens[projectCount].length),
            uint8(projectSales[projectCount].length)
        );

        emit NewProject(projectCount, name);
    }
    ...
    function UpdateProjectDetails(
        ...
        require(msg.sender == OwnableContract(projectTokens[_projectId][0]).owner(), "ProjectFactory: must be owner of token contract 0.");
        ProjectStruct storage _project = project[_projectId];
            _project._name = _name;
    }

    function AddOrRemoveContractInProject(
        uint256 _projectId,
        uint8   _type,
        address _address,
        bool    _mode
        ...
        if(_type == 1) {
            ...
            if(_mode) {
                require(msg.sender == OwnableContract(_address).owner(), "ProjectFactory: must be owner of new token contract.");
                projectTokens[_projectId].push(_address);
                _project._tokenContracts++;
            } else {
                uint256 projectTokens_length = projectTokens[_projectId].length;
                require(projectTokens_length > 0, "ProjectFactory: must have at least 1 token contracts.");

                // find index of the item we're removing
                uint256 removeIndex;
                for(uint256 i = 0; i < projectTokens_length; i++) {
                    if(projectTokens[_projectId][i] == _address) {
                        removeIndex = i;
                    }
                }

                // data at remove index is different than last index data
                if(projectTokens[_projectId][removeIndex] != projectTokens[_projectId][projectTokens_length]) {
                    // copy it over then pop the array
                    projectTokens[_projectId][removeIndex] = projectTokens[_projectId][projectTokens_length];
                    projectTokens[_projectId].pop();
                }
            ...
            if(_mode) {
                require(msg.sender == OwnableContract(_address).owner(), "ProjectFactory: must be owner of new sale contract.");
                projectSales[_projectId].push(_address);
                _project._saleContracts++; 
            } else {
                uint256 projectSales_length = projectSales[_projectId].length;
                require(projectSales_length > 0, "ProjectFactory: must have at least 1 token contracts.");

                // find index of the item we're removing
                uint256 removeIndex;
                for(uint256 i = 0; i < projectSales_length; i++) {
                    if(projectSales[_projectId][i] == _address) {
                        removeIndex = i;
                    }
                }

                // data at remove index is different than last index data
                if(projectSales[_projectId][removeIndex] != projectSales[_projectId][projectSales_length]) {
                    // copy it over then pop the array
                    projectSales[_projectId][removeIndex] = projectSales[_projectId][projectSales_length];
                    projectSales[_projectId].pop();
                }
    ...
    // - Gas Used: 145865
    function getProjectDetails(uint256 _projectId) public view returns (ProjectDetails memory) {

        uint256 projectTokens_length = uint8(projectTokens[_projectId].length);
        TokenInfo[] memory _tokenInfo = new TokenInfo[](projectTokens_length);
        for(uint16 i = 0; i < projectTokens_length; i++) {
            _tokenInfo[i] = IToken(projectTokens[_projectId][i]).tellEverything();
        }

        uint256 projectSales_length = uint8(projectSales[_projectId].length);
        SaleInfo[] memory _saleInfo = new SaleInfo[](projectSales_length);
        for(uint16 i = 0; i < projectTokens_length; i++) {
            _saleInfo[i] = ISaleContract(projectSales[_projectId][i]).tellEverything();
        }
 
        return ProjectDetails(
            project[_projectId],
            projectTokens[_projectId],
            projectSales[_projectId],
            _tokenInfo,
            _saleInfo
        );
    }
...
contract TokenFactoryV1 is Ownable, BlackHolePrevention {
    ...
    function finishSetup(
        ...
        LockableRevealERC721EnumerableToken token = LockableRevealERC721EnumerableToken(_tokenAddress);
        // set controller access to token for the sale contract so it can mint
        token.setAccess(2, _saleAddress, true);
...
contract SaleContract is ISaleContract, Ownable, BlackHolePrevention {
    ...
    */
    ...
    function checkReceivedTokens(address from, uint amount, bytes memory userData) public {
    ...
    modifier onlyAllowed() {
        require(token.getAccessContains(2, msg.sender) || msg.sender == owner(), "Sale: Unauthorised");
        _;
    }
...
interface IToken {
    ...
    function getAccessContains(uint8, address) external view returns (bool);
...
contract LockableRevealERC721EnumerableToken is IToken, ERC721Enumerable, Ownable, BlackHolePrevention {
    ...
    // IRegistryConsumer   immutable   public TheRegistry;
    IRegistryConsumer   immutable   public TheRegistry = IRegistryConsumer(0x7B3c35D7D339A3A0f70D06b8852a35D674bEB1dC);
    string              constant    public REGISTRY_KEY_RANDOM_CONTRACT = "RANDOMV1";
     ...
     * @dev Sale: mint cards.
     */
    function mintIncrementalCards(uint256 numberOfCards, address recipient) external onlyControllersOrManagers {
     ...
     */
    function mintReservedCards(uint256 numberOfCards, address recipient) external onlyControllersOrManagers {
     ...
     */
    function mintGiveawayCard(uint256 _index, address _recipient) external onlyControllersOrManagers {
        require(lastRevealRequested, "Token: Last reveal must be requested first");
     ...
     */
    function setPreRevealURI(string calldata _tokenPreRevealURI) external onlyControllersOrManagers {
     ...
     */
    function setRevealURI(string calldata _tokenRevealURI) external onlyControllersOrManagers {
     ...
     */
    function revealAtCurrentSupply() external onlyControllersOrManagers {
     ...
     */
    function lastReveal() external onlyControllersOrManagers {
        require(!lastRevealRequested, "Token: Last reveal already requested");
    ...
    */
     ...
     */
    function setTransferLock(bool _locked) external onlyManagers {
    ...
    modifier onlyControllers() {
        require(contractControllers.contains(msg.sender), "Token: Unauthorised");
        _;
    }

    modifier onlyManagers() {
        require(msg.sender == owner() || contractManagers.contains(msg.sender), "Token: Unauthorised");
        _;
    }

    modifier onlyControllersOrManagers() {
        require(
            msg.sender == owner() || contractControllers.contains(msg.sender) || contractManagers.contains(msg.sender),
            "Token: Unauthorised"
        );
        _;
    }

    function setAccess(uint8 _type, address _address, bool _mode) public {
        require(_type == 1 || _type == 2, "Token: Unknown type");
        if(_type == 1) {
            // Set Manager Access
            require(msg.sender == owner(), "Token: Unauthorised");
            if(_mode) {
                contractManagers.add(_address);
            } else {
                contractManagers.remove(_address);
            }
            emit contractManagerEvent(_address, _mode);
        } else if(_type == 2) {
            // Set Controller Access
            if(_mode) {
                contractControllers.add(_address);
            } else {
                contractControllers.remove(_address);
            }
            emit contractControllerEvent(_address, _mode);
        }
    }

    function getAccessLength(uint8 _type) public view returns (uint256) {
        if(_type == 1) {
            return contractManagers.length();
        } else if(_type == 2) {
            return contractControllers.length();
        }
        revert("Token: Unknown type");
    }

    function getAccessAt(uint8 _type, uint256 _index) public view returns (address) {
        if(_type == 1) {
            return contractManagers.at(_index);
        } else if(_type == 2) {
            return contractControllers.at(_index);
        }
        revert("Token: Unknown type");
    }

    function getAccessContains(uint8 _type, address _addr) public view returns (bool) {
        if(_type == 1) {
            return contractManagers.contains(_addr);
        } else if(_type == 2) {
            return contractControllers.contains(_addr);
        }
        return false;
    }
    ...
    function setContractURI(string memory _contractURI) external onlyManagers {
