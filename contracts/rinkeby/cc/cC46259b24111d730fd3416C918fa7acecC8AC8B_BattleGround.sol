...
contract BattleGround is ERC721URIStorage, VRFConsumerBase{
    ...
    event BoxPurchase(string indexed boxType);
    event NftPurchase(uint indexed token_0, uint addon_token_0, uint indexed token_1, uint addon_token_1, uint  indexed token_2, uint addon_token_2, address buyer);
    event ChangedProbabilityBox(uint indexed box, uint[4] indexed probability);
    event RequestNumberToToken(uint indexed token, bytes32 indexed requestID_1, bytes32 indexed requestID_2);
    event ReceivedRandomNumber(bytes32 indexed requestID, uint indexed randomNumber);
    event ExecutedRequest(Request indexed request);
    event EmittedToken(uint indexed token, uint randomNumberToCharacter, uint indexChar, uint randomNumberToAddon, uint scaledNumber, uint indexed addon, string indexed boxType);
        ...
        override
    {
        ...
        if(generatedAllNumbers == true)
        {
            ...
            emit EmittedToken(requests[index].tokenID, requests[index].randomNumberToCharacter, index_char,
                aux, requests[index].randomNumberToAddon, addons, convertBoxTypeToString(boxType));
