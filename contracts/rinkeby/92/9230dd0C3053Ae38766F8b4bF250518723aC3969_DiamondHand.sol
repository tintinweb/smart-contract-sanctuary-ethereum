...
contract DiamondHand is Ownable, IERC721Receiver, IERC1155Receiver {
    ...
    address public DIAMONDPASS = 0xCE16Fe9d5bA898BbF3759512Cf7AaA1187F126f4; ///Address of DiamondPass NFT. Owners can create Diamond-Hands for free
    ...
    * @return diamondID
    */
    function createDiamondHands(diamondHands memory _diamondHands, diamondStruct[] memory _diamondNFT, uint256 _releaseTime, uint256 _breakPrice) payable public returns (uint256) {
        ...
        if(ERC721Interface(DIAMONDPASS).balanceOf(msg.sender) == 0) {
            require(msg.value == PRICE, "Incorrect ETH amount to create a diamond-hand");
        }
        ...
        for(i = 0; i < diamondNFTs[_diamondHands.id].length; i++) {
            if(diamondNFTs[_diamondHands.id][i].tokenType == ERC721) {
                ...
                nftToAddressMap[keccak256(abi.encodePacked(diamondNFTs[_diamondHands.id][i].dapp, diamondNFTs[_diamondHands.id][i].tokenId[0]))] = msg.sender;
                ...
                //Map NFT's (contract address, token ID) to sender's address
                uint256 j;
                for (j = 0; j < diamondNFTs[_diamondHands.id][i].tokenId.length; j++){
                    nftToAddressMap[keccak256(abi.encodePacked(diamondNFTs[_diamondHands.id][i].dapp, diamondNFTs[_diamondHands.id][i].tokenId[j]))] = msg.sender;
                }
            }
    ...
    */
    function breakUnlock(uint _diamondId) payable public {
        ...
        require(msg.value == diamondHandOrder.breakPrice, "Incorrect ETH amount to unlock this NFT");
    ...
    */
    ...
    /**
    * @dev Lookup the owner address of a specific NFT deposited in Diamond-Hand. Can be used to redirect airdrops
    ...
    */
    ...
    /**
    * @dev Batch lookup the owners of specific NFTs deposited in Diamond-Hand. Can be used to redirect airdrops
    * @param contractAddress The address of the NFT
    ...
    */
    function setDiamondPassAddress(address _newAddress) public onlyOwner {
