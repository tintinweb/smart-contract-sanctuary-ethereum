...
contract GAT is ERC721, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721("GAT_V1", "GATV1") {
  ...
  modifier mintCompliance(string calldata _salt) {
  ...
  function mint(string calldata _salt, bytes calldata _token) external payable nonReentrant mintCompliance(_salt) {
    require(!paused, "Paused");
    require(!preSaleIsActive, "Pre-sale is active");
    require(cost <= msg.value, "Ether value wrong");    
  ...
  function mintPresale(string calldata _salt, bytes calldata _token) external payable nonReentrant mintCompliance(_salt) {
    require(!paused, "paused!");
