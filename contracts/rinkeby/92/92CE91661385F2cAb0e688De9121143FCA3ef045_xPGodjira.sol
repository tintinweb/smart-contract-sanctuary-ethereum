...
contract xPGodjira is ERC721A, Ownable, Pausable, PaymentSplitter {
    using SafeMath for uint256;

    uint256 constant public maxSupply = 333;
    uint256 constant public max = 3;
    ...
    constructor() ERC721A("xPGodjira", "xPGJ") PaymentSplitter(_shareholders, _shares) {
        _pause();
    }
    ...
    function mint(uint _amount) external payable whenNotPaused enoughSupply(_amount) {
        ...
        require(_numberMinted(msg.sender) + _amount <= max, "Minting would exceed address allowance");
