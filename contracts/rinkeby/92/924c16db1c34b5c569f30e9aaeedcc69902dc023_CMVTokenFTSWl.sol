...
contract CMVTokenFTSWl is ERC20, Ownable {
    ...
    uint256 public presaleCost = 0.005 ether; //cost1 for 1 * 10 ** decimals()

    uint8 public stage = 0; //0 - nothing, 1 - first presale round, 2 - token launched

    mapping(address => uint8) public userStatus; //0 - nothing, 1 - white

    constructor() ERC20("CMVTokenFTSWl", "CMVFTSWL") {}
    ...
    function buyOnPresaleWhiteListed() public payable {
        ...
        _intFTSMint(msg.sender, amount);
    }

    function _intFTSMint(address to, uint256 exactAmount) internal virtual {
        uint256 newSupply = totalSupply() + exactAmount;
        require(newSupply <= finalTotalSupply, "Final supply reached!");
        _mint(to, exactAmount);

    }

    function mint(address to, uint256 amount) public onlyOwner {
        _intFTSMint(to, amount * 10**decimals());
    }
