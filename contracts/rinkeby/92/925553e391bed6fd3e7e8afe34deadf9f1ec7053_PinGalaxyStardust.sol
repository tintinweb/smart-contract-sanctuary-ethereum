...
contract PinGalaxyStardust is ERC20, Ownable {
    ...
    uint256 private START_TIME_YIELD = block.timestamp - 30 days; // airdrop for current holders
    uint256 private END_TIME_YIELD = block.timestamp + 6 * 365 days; // 6 years
    uint256 private REWARD_PER_SECOND = 48620000000000;
    uint256 private INITIAL_SUPPLY = 420690696969696969696969; // 420690
    address private PINPLANETS_ADDRESS;
    address private PINGALHERO_ADDRESS;
    uint256 private REWARD_MULTIPLIER_PINPLANETS = 100;
    uint256 private REWARD_MULTIPLIER_PINGALHERO = 1;

    constructor(address pinPlanAddress)
        ERC20("Pin Galaxy Stardust", "PINGALSD")
    {
        PINPLANETS_ADDRESS = pinPlanAddress;
        _mint(msg.sender, INITIAL_SUPPLY);
    }

    function pinPlanetsOfOwner(address owner)
        external
        view
