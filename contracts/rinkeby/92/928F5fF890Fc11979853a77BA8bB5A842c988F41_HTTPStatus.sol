...
import "./HTTPRenderer.sol";


contract HTTPStatus is ERC721URIStorage, Pausable, Ownable, ReentrancyGuard   {                
    uint                                  public                version_;
    uint                                  public                upgrade_version_;
    uint                                  public                upgrade_;
    uint                                  public                mint_fee_   = 0.3 ether;               
    uint                                  public                upgrade_fee_   = 0.3 ether;  
    uint                                  private               _max_tokens_hold = 1;             
    address                               private               _render_address;
    mapping( uint => address )            public                reserved_;
    mapping( address => uint )            private               _reservedBalances;
    string                                private               _premintURI;
    

    constructor(address render_address) ERC721("HTTP Status","HTTP") Ownable() {
        _render_address = render_address;
        version_ = block.timestamp;
    }


    modifier onlyTokenOwner( uint _tokenId ) {        
        require( _msgSender() == ERC721.ownerOf(_tokenId),"OnlyTokenOwner");
        _;
    }  

    function pause_() external onlyOwner {
        _pause();
    }

    function unpause_() external onlyOwner {
        _unpause();
    }

    function setMintFee_(uint mint_fee) external onlyOwner whenPaused {      
        mint_fee_ = mint_fee;
    }    

    function setUpgradeFee_(uint up_fee) external onlyOwner whenPaused {      
        upgrade_fee_ = up_fee;
    }   

    function setPremintURI_(string calldata premint ) external onlyOwner {      
        _premintURI = premint;
    }   
    ...
    function setUpgradeDays_(uint daysduration) external onlyOwner  {
        upgrade_ = daysduration * 1 days;
    }
    function setUpgradeSeconds_(uint secondsduration) external onlyOwner  {
        upgrade_ = secondsduration * 1 seconds;
    }

    function upgradeSVG_(uint version, string calldata svg_header) external onlyOwner  {
        require(upgrade_ > 0 && block.timestamp >= version_ + upgrade_,"NOTYET");
        HTTPRenderer http = HTTPRenderer(_render_address);
        http.upgradeSVGHeader(version,svg_header);
        upgrade_version_ = block.timestamp;
    }

    function setRender_(address rendaddr)  external onlyOwner {
        _render_address = rendaddr;
    }
    ...
    // function getSVGImage(uint tokenId) external view onlyTokenOwner(tokenId)
    //                                                  whenNotPaused returns(string memory) {                                                     
    //     require(_exists(tokenId),"INVALIDTOKEN");            
    //     string memory owner = Strings.toHexString(uint256(uint160(ownerOf(tokenId))));
    //     HTTPRenderer http = HTTPRenderer(_render_address);
    //     return http.buildTokenURI(tokenId,owner);
    // }

    function reserveToken_(uint tokenId) external  {
        require(!_exists(tokenId),"TOKENMINTED");
        require(balanceOf(msg.sender)<_max_tokens_hold,"MAXED");
        HTTPRenderer http = HTTPRenderer(_render_address);        
        require(http.httpAllocated(tokenId),"NOTALLOCATED");        
        require(reserved_[tokenId]==address(0),"RESERVED");
        require(_reservedBalances[msg.sender]<_max_tokens_hold,"MAXRICHED");
        reserved_[tokenId] = msg.sender;
        _reservedBalances[msg.sender]+=1;
    }

    function mint_(uint tokenId) external whenNotPaused payable nonReentrant {
        require(msg.value>=mint_fee_,"MINTFEE");
        assert(_render_address!=address(0));
        require(reserved_[tokenId]==msg.sender || reserved_[tokenId]==address(0),"INVALIDRESERVE");
        HTTPRenderer http = HTTPRenderer(_render_address);        
        require(http.httpAllocated(tokenId),"NOTALLOCATED");
        require(balanceOf(msg.sender)<_max_tokens_hold,"MAXED");
        require(!_exists(tokenId),"TOKENMINTED");
        (bool sent,) = payable(owner()).call{value:msg.value}("");
        require(sent==true,"MINTFEEFAILED");
        _safeMint( _msgSender(), tokenId);    
        http.setVersion(tokenId,version_);
        _setTokenURI(tokenId, http.buildTokenURI(tokenId, true, string(abi.encodePacked(msg.sender))));              
    }

    function mintUnofficial_(uint tokenId, string calldata name) external whenNotPaused payable nonReentrant {
        require(msg.value>=mint_fee_,"MINTFEE");
        assert(_render_address!=address(0));
        require(!_exists(tokenId),"TOKENMINTED");
        (bool sent,) = payable(owner()).call{value:msg.value}("");
        ...
        HTTPRenderer http = HTTPRenderer(_render_address);       
        http.setVersion(tokenId,version_);
        http.setUnofficial(tokenId,name);
        _setTokenURI(tokenId, http.buildTokenURI(tokenId, false, string(abi.encodePacked(msg.sender))));              
    }

    function upgradeTokenArtwork_(uint tokenId) external whenNotPaused payable nonReentrant {
        require(_exists(tokenId),"NOTATOKEN");
        require(msg.sender==ownerOf(tokenId),"ONLYTOKENOWNER");
        HTTPRenderer http = HTTPRenderer(_render_address); 
        require(upgrade_version_ >= http.getVersion(tokenId) + upgrade_,"VERSIONFAILED");
        require(msg.value>=upgrade_fee_,"UPGRADEFEE");
        assert(_render_address!=address(0));
        
        (bool sent,) = payable(owner()).call{value:msg.value}("");
        require(sent==true,"UPGRADEFEEFAILED");     
        
        http.setVersion(tokenId,upgrade_version_);
        _setTokenURI(tokenId, http.buildTokenURI(tokenId, http.httpAllocated(tokenId), string(abi.encodePacked(msg.sender))));              
    }     
    ...
    function _transfer(
        ...
        HTTPRenderer http = HTTPRenderer(_render_address); 
        _setTokenURI(tokenId, http.buildTokenURI(tokenId, http.httpAllocated(tokenId), string(abi.encodePacked(to))));   
    }

    function allocatedTo(uint tokenId,address to) external onlyOwner {
