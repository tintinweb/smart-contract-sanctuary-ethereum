...
import "contracts/NFTArt.sol";


contract FroggyFarm is ERC721, Pausable, ERC721Burnable, NFTArt {
    ...
    mapping(uint256 => uint96) public tokenIdToAssets;
    mapping(uint96 => uint256) public AssetsToTokenId;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // should be this, but need to figure out converting tokenHash to uint96, or "packed" assets string memory _tokenURI = formatTokenURI(renderSvg(tokenIdToHash[tokenId]));
        string memory uriAsset = encodeSvg(tokenIdToAssets[tokenId]);
        string memory _tokenURI = formatTokenURI(uriAsset);
...
function startMint(address to, uint96 packedAssets) public {
        ...
        tokenIdToAssets[tokenId] = packedAssets;
        AssetsToTokenId[packedAssets] = tokenId;
...
contract NFTArt {
    string public constant SVG_HEADER = '<svg xmlns="http://www.w3.org/2000/svg" version="1.2" viewBox="0 0 24 24">';
...
bytes memory encoding,
    ...
    function renderSvg(uint96 packed)public view returns (string memory svg){
    ...
    function encodeSvg (uint96 packed) public view returns (string memory){
    ...
    function formatTokenURI(string memory svgstring) public pure returns (string memory) {
        return string(abi.encodePacked("data:application/json;base64,",Base64.encode(bytes(abi.encodePacked('{"name":"sweet green baby boy", "description":"love this little guy", "attributes":[], "image":"',svgstring,'"}')))));
    }

    function demoURI(uint96 packed)public view returns (string memory){
        return formatTokenURI(encodeSvg(packed));
    }
