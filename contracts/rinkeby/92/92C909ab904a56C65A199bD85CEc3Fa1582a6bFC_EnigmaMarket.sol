...
abstract contract BaseEnigmaNFT1155 is IRoyaltyAwareNFT, ERC1155BurnableUpgradeable, OwnableUpgradeable {
    ...
    ITransferGatekeeper public transferGatekeeper;
    ...
    /**
     * @notice Set a transferGatekeeper with the logic to allow transfer NFTs
     *
     * @param _transferGatekeeper The Transfer Gatekeeper contract address
     */
    function setTransferGatekeeper(ITransferGatekeeper _transferGatekeeper) external onlyOwner {
        transferGatekeeper = _transferGatekeeper;
    }
}
...
contract TradeV3 is ReentrancyGuard {
...
// solhint-disable-next-line max-line-length
string constant error_SendValueWithFallbackWithdraw_No_Funds_Available = "SendValueWithFallbackWithdraw_No_Funds_Available";
 ...
 */
abstract contract SendValueWithFallbackWithdraw is ReentrancyGuardUpgradeable, NFTMarketCore {
...
string constant error_NFTMarketCore_FETH_Address_Is_Not_A_Contract = "NFTMarketCore_FETH_Address_Is_Not_A_Contract";
string constant error_NFTMarketCore_Only_FETH_Can_Transfer_ETH = "NFTMarketCore_Only_FETH_Can_Transfer_ETH";
string constant error_NFTMarketCore_Seller_Not_Found = "NFTMarketCore_Seller_Not_Found";
...
abstract contract NFTMarketCore is Constants {
    ...
    /// @dev The FETH ERC-20 token for managing escrow and lockup.
    IFethMarket internal feth;

    function initializeNFTMarketCore(address _feth) internal {
        if (!_feth.isContract()) {
            revert(error_NFTMarketCore_FETH_Address_Is_Not_A_Contract);
        }
    ...
    receive() external payable {
        if (msg.sender != address(feth)) {
            revert(error_NFTMarketCore_Only_FETH_Can_Transfer_ETH);
        }
    }
    ...
    function _transferFromEscrow(
        ...
        if (authorizeSeller != address(0)) {
            revert(error_NFTMarketCore_Seller_Not_Found);
        }
...
abstract contract Constants {
    ...
    // The minimum increase of 10% required when making an offer or placing a bid.
    uint256 public constant MIN_PERCENT_INCREMENT_DENOMINATOR = BASIS_POINTS / 1000;

    // The gas limit to send ETH to a single recipient, enough for a contract with a simple receiver.
    uint256 public constant SEND_VALUE_GAS_LIMIT_SINGLE_RECIPIENT = 20000;
}
...
abstract contract TradeV4 is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function applyBuyerFee(uint256 _price, uint8 _buyerFeePermille)
        internal
        pure
        returns (uint256 amount, uint256 buyerFee)
    {
        if (_buyerFeePermille == 0) {
            buyerFee = 0;
            amount = _price;
        } else {
            buyerFee = _price.mul(_buyerFeePermille).div(1000);
            amount = _price.add(buyerFee);
        }
    }
...
string constant ReserveAuction_Not_Matching_Seller = "ReserveAuction_Not_Matching_Seller";
...
string constant ReserveAuction_Too_Much_Value_Provided = "ReserveAuction_Too_Much_Value_Provided";
 ...
 */
abstract contract NFTMarketReserveAuction is Constants, NFTMarketCore, SendValueWithFallbackWithdraw, NFTMarketAuction {
     ...
     */
    function _adminCancelReserveAuction(uint256 auctionId, string calldata reason) internal nonReentrant {
    ...
    function placeBidOf(uint256 auctionId, uint256 amount) internal nonReentrant {
        ...
        if (auction.amount == 0) {
            ...
            revert(ReserveAuction_Too_Much_Value_Provided);
        }
        ...
        onlyValidAuctionConfig(reservePrice)
    {
        ...
        if (auction.seller != msg.sender) {
            ...
            revert(ReserveAuction_Price_Already_Set);
        }
    ...
    NFTMarketReserveAuction
{
     ...
     * TODO: complete TradeV4 doc
     * @param _minDuration The min duration for auctions, in seconds.
     * @param _maxDuration The max duration for auctions, in seconds.
     * @param feth ERC-20 token for managing escrow and lockup.
     */
     ...
     */
    function _beforeAuctionStarted(address nftContract, uint256 tokenId) internal override(NFTMarketCore) {
    ...
    function _transferFromEscrowIfAvailable(
        ...
        uint256 tokenId,
        address recipient
    ) internal override(NFTMarketCore, NFTMarketReserveAuction) {
        super._transferFromEscrowIfAvailable(nftContract, tokenId, recipient);
    }
        ...
        if (auctionId != 0) {
            ...
            if (auction.endTime == 0) {
                ...
                if (authorizeSeller != address(0) && auction.seller != authorizeSeller) {
                    ...
                    revert(ReserveAuction_Not_Matching_Seller);
                }
    ...
    function _finalizeReserveAuction(uint256 auctionId, bool keepInEscrow) private {
        ...
        Fee memory fee = getFees(auction.amount, auction.nftContract, auction.tokenId);
        _distAuctionFunds(auction, fee);
    ...
    /**
     * @notice Distributes funds for a finalized auction, creator, platforma and seller.
     * @param auction The finalized auction to distribus fees from.
     * @param fee struct witht the details of the amount each party receives
     */
    function _distAuctionFunds(ReserveAuction memory auction, Fee memory fee) internal {
        ...
        if (auction.seller == fee.tokenCreator) {
        ...
        _sendValueWithFallbackWithdraw(auction.seller, toSeller, SEND_VALUE_GAS_LIMIT_SINGLE_RECIPIENT);
    }
     ...
     * @param reason The reason for the cancellation (a required field).
     */
    function adminCancelReserveAuction(uint256 auctionId, string calldata reason) external onlyOwner {
        super._adminCancelReserveAuction(auctionId, reason);
    }
}
...
abstract contract BaseEnigmaNFT721 is IRoyaltyAwareNFT, ERC721BurnableUpgradeable, OwnableUpgradeable {
    ...
    /**
     * @notice Set a transferGatekeeper with the logic to allow transfer NFTs
     * @param _transferGatekeeper The transferGatekeeper contract address
     */
    function setTransferGatekeeper(ITransferGatekeeper _transferGatekeeper) external onlyOwner {
        transferGatekeeper = _transferGatekeeper;
    }
