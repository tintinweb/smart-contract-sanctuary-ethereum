...
contract ManagedVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, IManagedVault {
    ...
    //  * PerfomanceFeeUpdated: Requires PricesSet. Sets to PerformanceFeeUpdated.
    enum State { PricesSet, PerfomanceFeeUpdated }

    /* ============ CONSTANTS ============ */
    
    int256 constant MAX_PERFORMANCE_FEE_BPS = 4000; // 40% in basis points
    int256 constant HUNDRED_PERCENT_BPS = 10000; // 100% in basis points
    ...
    uint256 updatePerformanceFeeMinBlocksSinceSetPrice;
    uint256 updatePerformanceFeeMinBlocksSinceLastUpdate;

    uint256 updatePerformanceFeeBlockNumber;
    uint256 updatePerformanceFeeLastVaultPrice;

    uint256 performanceFeeClaimFirstBlock;
    uint256 performanceFeeClaimBlockCooldown;
    uint256 performanceFeeClaimBlockWindowLength;
    ...
    function totalSupply() public view virtual override returns (uint256) {
        if (accumulatedPerformanceFee > 0) {
            return super.totalSupply() + uint(accumulatedPerformanceFee);
        } else {
    ...
    function mint(uint maxMints) external onlyOwner returns (uint mintCount) {
        require(state == State.PerfomanceFeeUpdated);
    ...
    function setPerformanceFeeBPS(int256 _performanceFeeBPS) external onlyOwner {
        require(_performanceFeeBPS <= MAX_PERFORMANCE_FEE_BPS);
        performanceFeeBPS = _performanceFeeBPS;
    }

    function setUpdatePerformanceFeeMinBlocksSinceSetPrice(uint256 blocks) external onlyOwner {
        updatePerformanceFeeMinBlocksSinceSetPrice = blocks;
    }

    function setUpdatePerformanceFeeMinBlocksSinceLastUpdate(uint256 blocks) external onlyOwner {
        updatePerformanceFeeMinBlocksSinceLastUpdate = blocks;
    }

    function setPerformanceFeeClaimFirstBlock(uint256 blocks) external onlyOwner {
        performanceFeeClaimFirstBlock = blocks;
    }

    function setPerformanceFeeClaimBlockCooldown(uint256 blocks) external onlyOwner {
        performanceFeeClaimBlockCooldown = blocks;
    }

    function setPerformanceFeeClaimBlockWindowLength(uint256 blocks) external onlyOwner {
        performanceFeeClaimBlockWindowLength = blocks;
    }

    function claimPerformanceFee(uint256 epoch, address claimAddress) external onlyOwner {
        require(performanceFeeClaimFirstBlock + performanceFeeClaimBlockCooldown * epoch <= block.number);
        require(block.number <= performanceFeeClaimFirstBlock + performanceFeeClaimBlockCooldown * epoch + performanceFeeClaimBlockWindowLength);
        
        if (0 < accumulatedPerformanceFee) {
            _mint(claimAddress, uint(accumulatedPerformanceFee));
    ...
    function updatePerformanceFee() external {
        ...
        int256 denominator = int(vaultPrice) * HUNDRED_PERCENT_BPS;
        ...
        state = State.PerfomanceFeeUpdated;
    }
