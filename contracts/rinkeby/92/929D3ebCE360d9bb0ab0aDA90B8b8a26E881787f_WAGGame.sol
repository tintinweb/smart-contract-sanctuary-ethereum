...
contract WAGGame is Ownable, Pausable, ERC721Enumerable {
    ...
    constructor(IBank _bank, ILAND _land, ITraits _traits, IWildAbduction _wagNFT, uint256 _maxTokens) ERC721("WAG Game", 'WGAME') {
        land = _land;
        wagNFT = _wagNFT;
        bank = _bank;
        _pause;
     ...
     function mint(uint256 amount, bool stake) external payable whenNotPaused nonReentrant {
        ...
        if (minted < paidTokens) {
            ...
            if (hasPublicSaleStarted) {
                ...
                if (amount == 1 && _whitelistAddresses[_msgSender()].freeMint) {
                    require(msg.value == 0);
        ...
        wagNFT.updateOriginAccess(tokenIds);
    ...
    function addToWhitelist(address[] calldata addressesToAdd, bool freeMint) public onlyOwner {
...
contract Bank is Ownable, IERC721Receiver, Pausable {
   ...
   */
    constructor(WildAbduction _game, LAND _land, address _rand) {
        game = _game;
        land = _land;
   ...
   */
    function _addCowboyToBank(address account, uint256 tokenId) internal whenNotPaused _updateEarnings {
        bank[tokenId] = Stake({
        owner : account,
   ...
   */
    function _claimCowboyFromBank(uint256 tokenId, bool unstake, bool cowboy) internal returns (uint256 owed) {
        ...
        if (totalLANDEarned < MAXIMUM_GLOBAL_LAND) {
            owed = (block.timestamp - stake.value) * DAILY_LAND_RATE / 1 days;
        } else if (stake.value > lastClaimTimestamp) {
            owed = 0;
            // $LAND production stopped already
        } else {
            owed = (lastClaimTimestamp - stake.value) * DAILY_LAND_RATE / 1 days;
            // stop earning additional $LAND if it's all been earned
        }
        if (unstake) {
            if (cowboy) {
                if (randomizer.random() & 1 == 1) {// 50% chance of all $LAND stolen
                _payAlienTax(owed);
                owed = 0;
                totalCowboyStaked -= 1;
                }
...
contract WildAbduction is IWildAbduction, ERC721Enumerable, Ownable, Pausable {
    ...
    constructor(uint256 _maxTokens, ITraits _traits, IRandomizer _randomizer) ERC721("WAG Game", 'WGAME') {
    ...
    */
    function generate(uint256 tokenId, uint256 seed) internal returns (CowboyAlien memory t) {
        ...
        if (t.isMutant && MUTANT_COUNT == 55 && minted <= PAID_TOKENS) {
            return generate(tokenId, randomizer.random());
        }
   ...
   */
    function selectTraits(uint256 seed) internal view returns (CowboyAlien memory t) {
        ...
        seed >>= 16;
        t.accessory = selectTrait(uint16(seed & 0xFFFF), 0 );

        seed >>= 16;
        t.hat = selectTrait(uint16(seed & 0xFFFF), 1 );

        seed >>= 16;
        t.top = selectTrait(uint16(seed & 0xFFFF), 2);

        seed >>= 16;
        t.pants = selectTrait(uint16(seed & 0xFFFF), 3);

        seed >>= 16;
        t.base = selectTrait(uint16(seed & 0xFFFF), 4 );
