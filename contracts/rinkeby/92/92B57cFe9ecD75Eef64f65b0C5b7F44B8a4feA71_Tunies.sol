...
contract Tunies is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public root_mint_amount = 1; // Mints allocated from allowlist mint for each allowlisted address
    uint256 public pub_mint_max_per_tx = 1; // Max mint per transaction for public mint
    uint256 public total_reserved = 26; // total reserved amount to be minted by team + community wallet
    ...
    string private unrevealedURI = "ipfs://QmRiKsteocTM9Kj3gZFW8ueixr6vBYVj3a2w4SS8UJunPC/"; // unrevealed URI for all tokens

    bool public is_allowlist_active; // Access modifier for allowlist mint function
    bool public is_public_mint_active; // Access modifier for public mint function
    bool public is_revealed; // Access modifier for unrevealed vs revealed URI

    mapping (address => uint256) reservations; // Mapping tracks reservation mints

    constructor (bytes32 _root, uint256 _root_mint_amount) ERC721A("Tunies NFT", "Tunies") {
        root = _root;
        root_mint_amount = _root_mint_amount;

        // allocations for team and community
        reservations[0xB44e88a848cE623577D057a3825Da0d6834aF305] = 25;  // tunies
        reservations[0x21357bdA562d6861E87bB7dBf850F6AAfE940919] = 1;  // BG
    }
    ...
    */
    function publicMint(uint256 _amt) external payable nonReentrant {
        require(totalSupply() + _amt <= MAX_SUPPLY - total_reserved, "Mint Amount Exceeds Total Allowed Mints");
        require(msg.sender == tx.origin, "Minting from Contract not Allowed");
        require(item_price_pub * _amt == msg.value,  "Incorrect Payment");
        require(is_public_mint_active, "Public Mint Not Active");
        require(_amt <= pub_mint_max_per_tx, "Requested Mint Amount Exceeds Limit Per Tx");
    ...
    */
    ...
    /*
      Getter function returns how many allowlist mints a specified _user has remaining for current merkle root
    ...
    */
    function getAllowedMintAmount(bytes32[] calldata _proof, address _user) public view returns (uint256 amount) {
        ...
        amount = MerkleProof.verify(_proof, root, leaf) ? (root_mint_amount - _getAux(_user)) : 0;
    }
    ...
    */
    function withdrawEth() public onlyOwner nonReentrant {
        ...
        require(payable(0xB44e88a848cE623577D057a3825Da0d6834aF305).send((total * 95) / 100)); // tunies
    }
