...
contract PoolToken is Ownable {
    address public tokenAddress = 0x661B3a9297E8bee53602cA3E8140ff0555f725ED;
    struct UserInfo {
    ...
    event UserInformation(address user, uint256 dailyReceived, uint256 startAt, uint256 maxTokenReceived);
    event ClaimedToken(address user, uint256 tokenRemained, uint256 lastClaimAt);
    event AdminsChanged(address indexed account, bool allowance);

    function setTime(uint256 periodTime) external onlyOwner{
    ...
    function transferAnyBEP20Token(address _to, uint256 _amount) private onlyAdmins returns (bool success){
    ...
    function addUserInfo(address customer, uint256 dailyReceived, uint256 maxTokenReceived) external onlyAdmins {
        ...
        emit UserInformation(customer, dailyReceived, startAt, maxTokenReceived);
    }
    ...
    function claimToken(address recepient) external  {
        require(recepient != owner(), "address invalid");
        uint maxTokenReceived = _userInfo[recepient].maxTokenReceived;
        require(_userInfo[recepient].totalReceived != maxTokenReceived, "you have received enough token");
        // calculate the days passed
        uint startAt = _userInfo[recepient].startAt;
        uint daysPassed = daysHavePassed(startAt);

        uint tokenAmount = _userInfo[recepient].dailyReceived;
        uint totalToken = tokenAmount * daysPassed;

        uint tokenRemained =maxTokenReceived - _userInfo[recepient].totalReceived;
        _userInfo[recepient].totalReceived += totalToken;
         

        uint ClaimAt = block.timestamp;
        ...
        if (_userInfo[recepient].totalReceived > maxTokenReceived) {
            ...
            transferAnyBEP20Token(recepient, tokenRemained);
            emit ClaimedToken(recepient, 0, ClaimAt);
        } else {
            tokenRemained = maxTokenReceived - _userInfo[recepient].totalReceived;
            transferAnyBEP20Token(recepient, totalToken);
            emit ClaimedToken(recepient, tokenRemained, ClaimAt);
        }
