...
contract Clokies is IClokies, Ownable, ERC721A {
    ...
    uint256 public timelistPrice = 0.05 ether;
    uint256 public publicPrice = 0.069 ether;

    uint256 public constant RESERVED_CLOKIES = 50;
    uint64 public constant TIMELIST_MINT_MAX = 1;
    uint64 public constant PUBLIC_MINT_MAX = 1;

    string private _baseTokenURI;
    address private _signer;

    address private constant artistAddress = 0xE22306CBa5eaB8B9Baa1284D5dB721a832562e02;
    address private constant devAddress = 0x1eFEF8b085C75b7974B710e85494088ff52b9bfF;
    address private constant founderAddress = 0xa6E5a9cFF78a15627Ff4474AfB3Bf2c92360c3F3;
    address private constant vaultAddress = 0x303CA0B8FB4046F56D35632679b5d9e01b399DD9;
    ...
    function timelistMint(bytes calldata signature) 
        external
        payable 
        checkState(SaleStates.TIMELIST)
    {
        if (msg.value != TIMELIST_MINT_MAX * timelistPrice) revert InvalidEtherAmount();
        if (_totalMinted() + TIMELIST_MINT_MAX > maxClokies) revert SupplyExceeded();
        ...
        _setAux(msg.sender, TIMELIST_MINT_MAX);

        _mint(msg.sender, TIMELIST_MINT_MAX);
        emit Minted(msg.sender, TIMELIST_MINT_MAX);
    }
    ...
    {
        if (msg.value != PUBLIC_MINT_MAX * publicPrice) revert InvalidEtherAmount();
        if (_numberMinted(msg.sender) - _getAux(msg.sender) != 0)
            revert WalletLimitExceeded();
        if (_totalMinted() + PUBLIC_MINT_MAX > maxClokies) revert SupplyExceeded();
        if (!_verifySignature(signature, "PUBLIC")) revert InvalidSignature();

        _mint(msg.sender, PUBLIC_MINT_MAX);

        emit Minted(msg.sender, PUBLIC_MINT_MAX);
    }
    ...
    /// @notice Contract URI retrieval for OS collection.
