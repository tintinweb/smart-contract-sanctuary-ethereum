...
contract DiversifyNFTSales is AccessControl {
    ...
    mapping(uint256 => uint256) public mintLimit; // keeps the mint limit
    mapping(uint256 => uint256) public minted; // number of tokens minted
    ...
    uint256 public tokenURICount; // token URI count
    ...
    event ChangeMintLimit(uint256 id, uint256 limit);
    ...
    function mint(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            // check that user is not minting blank NFT
            require(_ids[i] <= tokenURICount, "invalid NFT");

            // check that user cannot mint more than the limit
            require(
                minted[_ids[i]] + _amounts[i] <= mintLimit[_ids[i]],
                "limit reached"
            );

            // check if the contract received fee
            require(msg.value >= fee * _amounts[i], "underpriced");
            ...
            minted[_ids[i]] = minted[_ids[i]] + _amounts[i];
        }
    }
    ...
    /// @param _mintLimits Number of NFTs to be minted for the URI
    function addInitialURIs(
        string[] memory _tokenURIs,
        uint256[] memory _mintLimits
    ) external onlyOwner {
        require(
            _tokenURIs.length == _mintLimits.length,
            "length should be equal"
        );
        tokenURICount += 1;
        for (uint256 i = 0; i < _tokenURIs.length; i++) {
            tokenURIs[tokenURICount] = _tokenURIs[i];
            mintLimit[tokenURICount] = _mintLimits[i];
            emit ChangeMintLimit(tokenURICount, _mintLimits[i]);
        }
    }
    ...
    function changeMintLimit(uint256 _id, uint256 _mintLimit)
        external
        onlyOwner
    {
        mintLimit[_id] = _mintLimit;
        emit ChangeMintLimit(_id, _mintLimit);
    }
}
