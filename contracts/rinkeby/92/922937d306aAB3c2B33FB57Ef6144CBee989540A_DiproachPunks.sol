...
// File contracts/DiproachPunksDNA.sol



pragma solidity ^0.8.0;

contract DiproachPunksDNA {
...
// File contracts/DiproachPunks.sol
...
contract DiproachPunks is ERC721, ERC721Enumerable, DiproachPunksDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("DiproachPunks", "DPRPKS") {
    ...
    function mint() public 
    {
        ...
        uint256 counter = _idCounter.current();
        require(counter < maxSupply, "No quedan mas DiproachPunks");

        // wallet.transfer(msg.value);

        tokenDNA[counter] = deterministicPseudoRandomDNA(counter, msg.sender);
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "DiproachPunks #',
                tokenId.toString(),
                '", "description": "Diproach Punks son randomizados por Avataaars", "image": "',
                image,
                '"}'
            )
        );
