...
function getSubscriptionByCreatorAddress(address _creator, address _follower) public view  returns(address, address, uint, bool){
    ...
    function getContent(uint256 _contentID, bytes memory _signature) public view returns (string memory,
        string memory,
        uint256,
        string memory) {

        (string memory name, string memory description, uint256 contentType, string memory link) = content_ref.getContent(_contentID, _signature);
        return(
                        name,
                        description,
                        contentType,
    ...
    function getContractBalance(bytes memory _signature) public view returns(uint256){

        require(_signature.length == 65, "invalid signature length");
        bytes32 prefixedHashMessage = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked('balance'))));
        bytes32 r;
        bytes32 s;
        uint8 v;
        assembly {
    ...
    function getCreatorBalance(bytes memory _signature) public view returns(uint256){

        address signer = Signature.getSignature('balance', _signature);
...
contract Content {
    ...
    function getContent(uint256 _contentID, bytes memory _signature) public view returns (string memory,
        ...
        address signer = Signature.getSignature('hello', _signature);
