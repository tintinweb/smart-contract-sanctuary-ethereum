...
contract Exchange is ReentrancyGuard, Ownable, IERC1155Receiver {
    ...
    function _addTokenIdToOpenTrades(uint256 _tokenId) internal {
        sellIdOpenForTradeArray.push(_tokenId);
    }
    function _removeTokenIdFromNFTOpenForTrade(uint256 _nftTokenId) internal {
    ...
    function sell(uint256 _futureTokenId, uint256 _amount) public {
        ...
        _addTokenIdToOpenTrades(newSellId);
    ...
    function buy(uint256 _sellId, uint256 _amount) public {
        ...
        uint256 expireDate = superFuture.expiredOfToken(trade.futureTokenId);
        require(expireDate > block.timestamp, "could not buy expire futureToken");
        require(
    ...
    function redeemFromNFT(uint256 _futureTokenId, uint256 _amount) external {
        ...
        uint256 expire = superFuture.expiredOfToken(_futureTokenId);
