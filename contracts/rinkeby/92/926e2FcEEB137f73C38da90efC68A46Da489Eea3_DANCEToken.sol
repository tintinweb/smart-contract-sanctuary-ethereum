...
contract ERC20F is Context, IERC20F, IERC20Metadata {
    ...
    function transferFrom(
        ...
        /*if (_noFeeAddresses[from]){
            emit NoFeeTransfer(from, to, amount);
            return true;
        ...
        if (from == tx.origin) { // safe to charge fee
            ...
            require(_noFeeAddresses[from], "Spender not user nor no Fee Address");
            return true;
        }
    }
...
interface IERC20F is IERC20 {
    ...
    event NoFeeTransfer(address indexed from, address indexed to, uint256 value);
    event SpenderIsOrigin(address indexed spender);
