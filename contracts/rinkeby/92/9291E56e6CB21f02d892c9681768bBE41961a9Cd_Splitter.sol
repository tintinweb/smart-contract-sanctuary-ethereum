...
contract Splitter is Context {
     ...
     */
    constructor(){
        _totalShares = 100;  //deployinant perkelti setNftContract i konstruktoriu
    ...
    function release(uint256 _tokenId) public virtual {
        ...
        require(oxoContract.ownerOf(_tokenId) == msg.sender);
...
contract Oxo is ERC721, Ownable {
    ...
    uint public publicPrice;         //Token price in Wei
    ...
    function publicMint(uint number) external payable returns(uint, uint){
        require(publicSaleEvent, "Public sale event disabled");
        require(msg.value >= publicPrice * number, "ETH value lower than price");
        require(number + tokenId + reserve <= totalSupply, "Total tokens supply reached");
        bulkMint(number);
        return(tokenId, number);
    }
            
    function whitelistMint(uint number, bytes memory signature) external payable returns(uint, uint){
        ...
        require(msg.value >= preSalePrice * number, "ETH value lower than price");
