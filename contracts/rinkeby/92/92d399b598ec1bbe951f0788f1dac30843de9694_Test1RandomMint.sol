...
contract Test1RandomMint is ERC721OwnerEnumerable,ReentrancyGuard {
    ...
    //test base uri
    string public baseURI = 'ipfs://QmTR6N5reiEUkF4N7DkbJtSCpMHS2QjQS3VMsAxYjenDSL/';

    uint256 private maxSupply = 100;
    uint256 public mintPhaseSupply = 60;

    // elements last token indexs
    // this how the elements metadata order set on
    // 1- 1200 fire
    // 1200 - 2400 water
    // 2400 - 3600 air
    //3600 - 4800 death
    //4800 - 6000 life
    uint256 public fireElementLimit = 12; // 1- 1200
    uint256 public waterElementLimit = 24;// 1200 - 2400
    uint256 public airElementLimit = 36; // 2400 - 3600
    uint256 public deathElementLimit = 48;//3600 - 4800
    uint256 public lifeElementLimit = 60;//4800 - 6000
    ...
    bool public fireGatOpen = false;
    bool public waterGatOpen = false;
    bool public airGatOpen = false;
    bool public deathGatOpen = false;
    bool public lifeGatOpen = false;
    ...
    //mapping keep track of wallet mint
    mapping(address => bool) public claimElement;
    ...
    constructor(
        ...
        //set initial URI
        baseURI = _initialURI;
        //set start index and max index (max supply) 
        //this function called from ERC721OwnerEnumerable
  ...
  /**
    * Fire Element mint.
    * if you have two fire element your eligable to mint
    */
    function fireClaimMint() 
      external 
      noBots
    {
   // fire elements token are available between 1 - 1200 range
   //check holder has owned token 1 - 1200 within the range 
   //if holder has 2 fire elemets tokenid holder can claim one token
   //check already claimed the token
    require(!fireGatOpen, 'The Fire gate is Closed!');
    uint256 tokenId = totalSupply() + 1;
    require(claimElement[msg.sender],"Your already claimed the Token");
    uint256[] memory ownedTokenIds = tokensOfOwner(msg.sender);
    uint256 TokenCount = 0;
        for (uint256 i = 0; i < ownedTokenIds.length; i++ ){
            //1201 > 1200
            if(ownedTokenIds[i] < fireElementLimit){
                TokenCount ++;
            }
        }
        if(TokenCount >= 2) {
  ...
  /**
    * Water Element mint.
    * if you have two Water element your eligable to mint
    */
    function waterClaimMint() 
      external 
      noBots
    {
    // water elements token are available between 1200 - 2400 range
    //check holder has owned tokens 1200 - 2400 within the range 
    //if holder has 2 water elemets tokenid holder can claim one token
    //check already claimed the token
    require(!waterGatOpen, 'The Water gate is Closed!');
    uint256 tokenId = totalSupply() + 1;
    require(claimElement[msg.sender],"Your already claimed the Token");
    uint256[] memory ownedTokenIds = tokensOfOwner(msg.sender);
    uint256 TokenCount = 0;
        for (uint256 i = 0; i < ownedTokenIds.length; i++ ){
            //1201 > 1200
            if(ownedTokenIds[i] > fireElementLimit && ownedTokenIds[i] < waterElementLimit){
                TokenCount ++;
            }
        }
  ...
  /**
    * Air Element mint.
    * if you have two Air element your eligable to mint
    */
    function airClaimMint() 
      external 
      noBots
    {
    // air elements token are available between 2400 - 3600 range
    //check holder has owned tokens 2400 - 3600 within the range 
    //if holder has 2 air elemets tokenid holder can claim one token
    //check already claimed the token
    require(!airGatOpen, 'The Air gate is Closed!');
    uint256 tokenId = totalSupply() + 1;
    require(claimElement[msg.sender],"Your already claimed the Token");
    uint256[] memory ownedTokenIds = tokensOfOwner(msg.sender);
    uint256 TokenCount = 0;
        for (uint256 i = 0; i < ownedTokenIds.length; i++ ){
            //1201 > 1200
            if(ownedTokenIds[i] > waterElementLimit && ownedTokenIds[i] < airElementLimit){
                TokenCount ++;
            }
        }
  ...
  /**
    * Death Element mint.
    * if you have two Death element your eligable to mint
    */
    function deathClaimMint() 
      external 
      noBots
    {
    // death elements token are available between 3600 - 4800 range
    //check holder has owned tokens 3600 - 4800 within the range 
    //if holder has 2 death elemets tokenid holder can claim one token
    //check already claimed the token
    require(!deathGatOpen, 'The Death gate is Closed!');
    uint256 tokenId = totalSupply() + 1;
    require(claimElement[msg.sender],"Your already claimed the Token");
    uint256[] memory ownedTokenIds = tokensOfOwner(msg.sender);
    uint256 TokenCount = 0;
        for (uint256 i = 0; i < ownedTokenIds.length; i++ ){
            //1201 > 1200
            if(ownedTokenIds[i] > airElementLimit && ownedTokenIds[i] < deathElementLimit){
                TokenCount ++;
            }
        }
  ...
  /**
    * Life Element mint.
    * if you have two Life element your eligable to mint
    */
    function lifeClaimMint() 
      external 
      noBots
    {
    // life elements token are available between 4800 - 6000 range
    //check holder has owned tokens 4800 - 6000 within the range 
    //if holder has 2 life elemets tokenid holder can claim one token
    //check already claimed the token
    require(!lifeGatOpen, 'The Life gate is Closed!');
    uint256 tokenId = totalSupply() + 1;
    require(claimElement[msg.sender],"Your already claimed the Token");
    uint256[] memory ownedTokenIds = tokensOfOwner(msg.sender);
    uint256 TokenCount = 0;
        for (uint256 i = 0; i < ownedTokenIds.length; i++ ){
            //1201 > 1200
            if(ownedTokenIds[i] > deathElementLimit && ownedTokenIds[i] < lifeElementLimit){
                TokenCount ++;
            }
        }
  ...
  /**
   ...
   // chaneg the elements of last index of the collection
   // this how the elements metadata order 
     // 1- 1200 fire
     // 1200 - 2400 water
     // 2400 - 3600 air
     //3600 - 4800 death
    ...
    function setFireElementLimit(uint256 _amount) public onlyOwner {
        fireElementLimit = _amount;
    }

    function setWaterElementLimit(uint256 _amount) public onlyOwner {
        waterElementLimit = _amount;
    }

    function setAirElementLimit(uint256 _amount) public onlyOwner {
        airElementLimit = _amount;
    }

    function setDeathElementLimit(uint256 _amount) public onlyOwner {
        deathElementLimit = _amount;
    }

    function setLifeElementLimit(uint256 _amount) public onlyOwner {
        lifeElementLimit = _amount;
    }
  /**
