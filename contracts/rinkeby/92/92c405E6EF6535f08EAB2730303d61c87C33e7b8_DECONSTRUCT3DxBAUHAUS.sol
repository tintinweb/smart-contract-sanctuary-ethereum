...
contract DECONSTRUCT3DxBAUHAUS is ERC1155, Ownable, ERC1155Supply, IERC2981 {
    ...
    {
        if (ids.length == 1) {
            COLLECTION[ids[0]].allowlist[account] -= amounts[0];
            _mint(account, ids[0], amounts[0], "");
        } else{
            for (uint256 i = 0; i < ids.length; i++) {
                COLLECTION[ids[i]].allowlist[account] -= amounts[0];
            }
    ...
    function addAllowlist(uint256 _id, address[] memory _addresses, uint256 amount) public onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            Token storage token = COLLECTION[_id];
            require(token.id == _id, "TOKEN ID NOT FOUND");
            token.allowlist[_addresses[i]] = amount + 1;
        }
    }
    ...
    function deleteAllowlist(uint256 _id, address[] memory _addresses) public onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            Token storage token = COLLECTION[_id];
            require(token.id == _id, "TOKEN ID NOT FOUND");
            COLLECTION[_id].allowlist[_addresses[i]] = 1;
        }
    }
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (owner() != _address) {
                if (allowlist) {
                    ...
                    require(token.allowlist[_address] >= amounts[i] + 1, "ALLOWLIST FORBIDDEN");
