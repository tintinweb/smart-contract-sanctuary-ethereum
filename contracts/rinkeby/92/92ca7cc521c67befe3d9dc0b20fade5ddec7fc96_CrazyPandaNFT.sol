...
interface ManagerInterface {
    ...
    function pricePanda() external view returns (uint256);
    ...
    Ownable
{
    ...
    constructor(
        ...
        _name = name_;
        _symbol = symbol_;
        manager = ManagerInterface(manager_);
...
// File: contracts\CrazyPandaNFT.sol
...
contract CrazyPandaNFT is ERC721 {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.UintSet;

    event LayPanda(uint256 indexed tokenId, address buyer);
    ...
    mapping(uint256 => CrazyPanda) internal pandas;
    ...
    function pricePanda() public view returns (uint256) {
        return manager.pricePanda();
    }
    ...
    function evolve(
        ...
        CrazyPanda storage panda = pandas[_tokenId];
    ...
    function layPanda(address receiver) external
        onlySpawner {
        uint256 nextTokenId = _getNextTokenId();
        _mint(receiver, nextTokenId);

        pandas[nextTokenId] = CrazyPanda({
            dna: 0
        });

        emit LayPanda(nextTokenId, receiver);
    }
    ...
    function getPanda(uint256 _tokenId)
        public
        view
        returns (CrazyPanda memory)
    {
        return pandas[_tokenId];
    }

    function getRare(uint256 _tokenId) public view returns (uint256) {
        uint256 dna = getPanda(_tokenId).dna;
        if (dna == 0) return 0;
