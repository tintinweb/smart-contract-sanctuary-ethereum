...
contract TESTINGMEN is ERC721Enumerable, Ownable {
    ...
    function setPhase(string memory _phase) public onlyOwner{
        if (keccak256(abi.encodePacked("VVIP")) == keccak256(abi.encodePacked(_phase))){
            currentPhase = _phase;
            setCost(140000000000000000);
            setNftPerAddressLimit(4);
            setMaxSupply(500);
        } else if (keccak256(abi.encodePacked("WHITELIST")) == keccak256(abi.encodePacked(_phase))){
            currentPhase = _phase;
            setCost(180000000000000000);
            setOnlyWhitelisted(true);
            setMaxSupply(2500);
        } else if (keccak256(abi.encodePacked("PRIVATE")) == keccak256(abi.encodePacked(_phase))){
            currentPhase = _phase;
            setCost(220000000000000000);
            setNftPerAddressLimit(5);
            setOnlyWhitelisted(false);
            setMaxSupply(5000);
        } else if (keccak256(abi.encodePacked("PUBLIC")) == keccak256(abi.encodePacked(_phase))){
            currentPhase = _phase;
