...
contract AlphaDwarves is ERC721IM, MerkleWhitelist, Ownable, PayableGovernance,
Security, WhitelistMint, PublicMint {
    constructor() payable ERC721IM("AlphaDwarves", "ALPHADWARVES") {}
    ...
    */
    ...
    ////// Project Constraints //////
    uint256[] public phases = [550, 1100, 1650, 2200];
    uint256[] public phasePrice = [0.05 ether, 0.07 ether, 0.09 ether, 0.05 ether];
    uint256 public phase = 0;
    bool public phaseEnabled = false;
    ...
    ///// Interfaces /////
    isAD public sAD;
    function setsAD(address address_) external onlyOwner {
        sAD = isAD(address_);
    }
    modifier onlyStaker {
        require(msg.sender == address(sAD), "You are not staker!"); _;
    }
    ...
    function setPhaseMintPrice(uint256 phaseIndex, uint256 mintPrice_) external onlyOwner {
        phasePrice[phaseIndex] = mintPrice_;
    }

    function setPhaseActive(bool state_) external onlyOwner {
        phaseEnabled = state_;
    }
    ...
    function _mintAndStakeMany(address to_, uint256 amount_) internal {
        ...
        for (uint256 i = 0; i < amount_; i++) {
            ...
            emit Transfer(to_, address(sAD), _startId + i);
            sAD.mintStakedTokenAsAlphaDwarves(to_, _startId + i);
        }
        totalSupply += amount_;
    }

    ///// Magic Stake Code /////
    // Dwarf Staker / Unstaker -- The validation logic is handled by sAlphaDwarves
    ...
    function stakeDwarves(uint256[] calldata tokenIds_) external onlyStaker {
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            _stake(tokenIds_[i]);
            emit Transfer(ownerOf(tokenIds_[i]), address(sAD), tokenIds_[i]);
        }
    }
    function updateDwarves(uint256[] calldata tokenIds_) external onlyStaker {
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            _update(tokenIds_[i]);
        }
    }
    function unstakeDwarves(uint256[] calldata tokenIds_) external onlyStaker {
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            _unstake(tokenIds_[i]);
            emit Transfer(address(sAD), _ownerOf[tokenIds_[i]].owner, tokenIds_[i]);
        }
    }

    ///// Minting Functions /////
    function airdrop(address[] calldata tos_, uint256[] calldata amounts_,
    ...
    bool stakeOnMint_) public payable onlySender whitelistMintEnabled {
        ...
        if(totalSupply + amount_ >= phases[phase] && phaseEnabled)
        {
            phaseEnabled = false;
            phase += 1;
        }

        require(totalSupply + amount_ <= phases[phase],
            "Phase has been minted out!");        
        require(msg.value == phasePrice[phase] * amount_,
            "Invalid value sent!");
