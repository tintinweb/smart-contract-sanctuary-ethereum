...
contract LastPass is ERC721A, Ownable, ReentrancyGuard, PaymentSplitter {
    string private _currentBaseURI;

    uint32 public MAX_SUPPLY = 3_333;
    uint32 public constant MAX_FREE_SUPPLY = 50;
    uint32 public constant MAX_WL_SUPPLY = 5;
    ...
    uint256[] private _teamShares = [65, 35]; // 2 PEOPLE IN THE TEAM
    address[] private _team = [
        0xE0e9d95a41187F468aDcF3747E60D37D463433a3, // Admin Account gets 65% of the total revenue
        0x8220077AFb75a4094f446A66524dBd250593A4B7 // Test Account gets 35% of the total revenue
    ];
    constructor(string memory uri, string memory hidden, bytes32 merkleroot, address _proxyRegistryAddress) ERC721A("Testing Pass", "TPass") 
        ...
        ReentrancyGuard() // A modifier that can prevent reentrancy during certain functions
    {
        ...
        setBaseURI(uri);
        setNotRevealedURI(hidden);

    }
    function mint(uint256 count) external payable nonReentrant {
        require(!paused, "TestPass: Contract is paused");
    ...
    function freeMint(uint32 count) external nonReentrant {
        require(isLive, "Minting is not live yet.");
        require(!isWhitelistSale, "Minting is not live yet.");
        require(!isPublic, "Minting is not live yet.");
        require(!paused, "TestPass: Contract is paused");
        require(totalSupply() + count <= MAX_SUPPLY, "Sold Out!");
        require(
    ...
    function burnSupply(uint32 maxSupplyNew) external onlyOwner {
