...
contract Farming is ReentrancyGuard, Ownable {
    ...
    event Staked(address indexed from, uint256 amountLP);
    event Withdrawn(address indexed to, uint256 amountLP);
    event Claimed(address indexed to, uint256 amount);
    event Halving(uint256 amount);
    event Received(address indexed from, uint256 amount);

    IERC20 public kitsuneToken;
    ...
    constructor(address _kitsuneToken, address _pairAddress) public {
        kitsuneToken = IERC20(_kitsuneToken);
    ...
    function stake(uint256 _amount) external payable nonReentrant {
        ...
        IERC20(pairAddress).transferFrom(msg.sender, address(this), _amount);
    ...
    function withdraw() external nonReentrant {
        ...
        // Send Fee Amount to Dev Address
        uint256 burnAmount = amount.mul(burnFeeX100).div(10000);
        if (burnAmount > 0) {
            amount = amount.sub(burnAmount);
            IERC20(pairAddress).transfer(address(devAddress), burnAmount);
        }
        IERC20(pairAddress).transfer(msg.sender, amount);
    ...
    function claim() external nonReentrant {
        ...
        if (reward > 0) {
            ...
            // Send reward
            kitsuneToken.transfer(msg.sender, net);
            if (tax > 0) {
                // Burn taxed token
                kitsuneToken.transfer(BURN_ADDRESS, tax);
            }
    ...
    function setBurnFee(uint256 _burnFee) external onlyDev {
