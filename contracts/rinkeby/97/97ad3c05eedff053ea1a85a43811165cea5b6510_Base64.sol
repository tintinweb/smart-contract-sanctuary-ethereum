...
library Base64 {
  ...
  function removeTrailingZeros(uint256 _value) public pure returns (uint256) {
  ...
  function formatValue(uint256 _value, uint256 _decimals, string memory _symbol) public pure returns (string memory) {
    uint256 decimals = 10 ** _decimals;
    uint256 integral = _value / decimals;
    uint256 fractional = removeTrailingZeros(_value - integral * decimals);
    if(fractional == 0) {
      return toString(integral);
    } else {
      return string(abi.encodePacked(toString(integral), '.', toString(fractional), ' ', _symbol));
    }
  }
