 ...
 *     * 引き出し可能アカウントはsetTreasuryで指定できる
 *     * ベースURIを変更できる
 *     * exists() をpublic化
 *     * 販売をpause()で一時停止できる
 *     * 日付時間を堺にステージが推移する。各ステージの価格や許可リストは任意に設定できる。
 *     * 許可リストの実装にはMerkleTreeを採用
 */
contract BoredOscarMotorClub is
    Ownable,
    ReentrancyGuard,
    Pausable,
    ERC721A,
    ERC2981
{
    string public baseURI = "http://localhost:3000/metadata/";
    ...
    constructor() ERC721A("BoredOscarMotorClub", "BOMC") {
        _setDefaultRoyalty(msg.sender, 1000); // デプロイした人に10%
        ...
        whenNotPaused
    {
        ...
        if (stage.useList) {
            require(allowance(stageNum, msg.sender, merkleProof) > 0, "Requested quantity is over allowance for account");
        ...
        require(msg.value >= totalPriceWei, "Insufficient ETH sent.");

        _safeMint(msg.sender, quantity);
        _refundExcess(totalPriceWei);
    }

    /**
     * 購入ミントで払いすぎたETHを返還する
     */
    function _refundExcess(uint256 priceWei) private {
        if (msg.value > priceWei) {
            (bool success, ) = msg.sender.call{value: msg.value - priceWei}("");
    ...
    function setSaleStages(
        ...
        for (uint256 i = 0; i < priceWeiVals.length; i++) {
            if (i > 0) {
    ...
    function setAllowList(
        ...
        stage.maxPerAddress = maxPerAddress;
    }
    ...
    /**
     * ERC721AとERC2981両方から継承されるメソッドのため、明示的にオーバーライドが必要
     */
