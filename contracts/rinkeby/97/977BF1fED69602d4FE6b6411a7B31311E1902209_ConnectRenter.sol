...
import "./lib/contracts/access/roles/WhitelistedRole.sol";
import "./lib/contracts/token/ERC20/IERC20.sol";
import "./lib/contracts/token/ERC721/IERC721Full.sol";
import "./lib/contracts/token/ERC721/ERC721Full.sol";
import "./sablier/IERC1620.sol";

contract ConnectRenter is ERC721Full, WhitelistedRole {
    ...
    constructor(
        ...
        IERC1620 _stream
    ) ERC721Full("Rent My NFT", "RENFT") public {
        ...
        // approve the stream to pull whatever they want from ConnectRenter
        paymentToken.approve(address(stream), uint(- 1));
    }


    function updateTokenContract(address tokenContract_) public returns(address) {
        tokenContract = IERC721Full(tokenContract_);
    ...
    function getRemainingTimeLeftForLoan(uint _tokenId) public view returns (uint256) {
        uint256 streamId = tokenIdToStreamId[_tokenId];
        require(streamId > 0, "Must have a stream");
    ...
    function getRemainingStreamBalance(uint _tokenId) public view returns (uint256) {
        uint256 streamId = tokenIdToStreamId[_tokenId];
        require(streamId > 0, "Must have a stream");
