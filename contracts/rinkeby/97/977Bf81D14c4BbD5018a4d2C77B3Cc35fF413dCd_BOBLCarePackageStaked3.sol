...
contract BOBLCarePackageStaked3 is Ownable {
    ...
    // returns whether a token id has been claimed
    ...
    function claimCarePackage(address _owner, uint16[] calldata _tokenIds, bytes32[] memory _hashes, bytes[] memory _signatures) external {
        ...
        for (uint16 i = 0; i < _tokenIds.length; i++) {
            require((isClaimed(_tokenIds[i]) == false), "care package has already been claimed for this token id");
            require(_isOwner(_tokenIds[i], stakedIds), "only owners approved");
    ...
    // checks both the staking contract and ERC721 contract for token ownership
    function _isOwner(uint16 _tokenId, uint16[] memory _stakedIds) internal view returns (bool){
