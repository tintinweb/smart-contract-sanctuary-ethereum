...
contract HITODAMA is Ownable, ERC721A {
    ...
    mapping(uint256 => uint256) public _saveSoulAt;
    mapping(uint256 => uint256) public _prayCount;
    mapping(uint256 => uint256) public _offeringsCount;
    mapping(uint256 => uint256) public _lastPray;
    mapping(uint256 => uint256) public _lastOfferings;
    mapping(uint256 => uint256) private _lastSize;

    event Pray(uint256 indexed tokenId, address indexed prayer, uint256 indexed changedAt);
    event Offerings(uint256 indexed tokenId, address indexed offerer, uint256 indexed offerAt);
    ...
    function pray(uint256 tokenId) external {
        ...
        require(_lastSize[tokenId] == 0 || block.timestamp - _lastPray[tokenId] > 1 days, "Only Once!");
        _lastSize[tokenId] = getCurrentSize(tokenId);
        _lastPray[tokenId] = block.timestamp;
        _prayCount[tokenId]++;
