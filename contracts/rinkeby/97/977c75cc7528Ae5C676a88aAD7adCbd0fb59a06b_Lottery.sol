...
error Lottery__SpendNotApproved();
error Lottery__OnlyHuman();

/** @title A Sample Lottery Contract
 *  @author https://twitter.com/luxury_kate
 *  @notice Notice here
 *  @dev This implements Chainlink Keepers and VRF V2
 */
...
contract Lottery is VRFConsumerBaseV2, KeeperCompatible {
    ...
    IERC20 private immutable i_shibToken;
    ...
    constructor(
        ...
        i_shibToken = IERC20(_shibToken);
    }

    function enterLottery(uint256 _tokenAmount) external {
        if (_tokenAmount < s_entranceFee) {
        ...
        if (i_shibToken.allowance(msg.sender, address(this)) < _tokenAmount) {
            revert Lottery__SpendNotApproved();
        }
        ...
        i_shibToken.transferFrom(msg.sender, address(this), _tokenAmount);
        ...
        )
    {
        ...
        upkeepNeeded = (isOpened && timePassed && enoughPlayers && hasBalance);
    }
    ...
    function approveSpending(uint256 _tokenAmount) external {
        i_shibToken.approve(address(this), _tokenAmount);
    }
    ...
    function getShibTokenAddress() external view returns (address) {
        return address(i_shibToken);
    }
}
