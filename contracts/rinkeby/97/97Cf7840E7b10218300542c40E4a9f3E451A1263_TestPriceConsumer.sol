...
contract TestPriceConsumer {
    ...
    //Get the ETH needed to disburse the annuity payment (based on USD, assume 2 periods for now)
    function getPaymentEthAmount() public {
        //Get Needed USD amount by dividing USD value by number of periods (2 and 3 for testing)
        uint paymentUSD = usdValue / 2; 
        testEthPayment = paymentUSD / (getLastEthPriceUSD() * 10**10); 
    }
