...
contract CrowdNFT is ERC1155, Ownable, ERC1155Burnable, ERC1155Pausable, ERC1155Supply, ERC1155URIStorage, Lockable {
    ...
    function mint(uint256 projectId, uint256 id, uint256 amount, bytes memory data) public onlyOwner
    {
    ...
    function mintBatch(uint256 projectId, uint256[] memory ids, uint256[] memory amounts, bytes memory data) public onlyOwner
    {
    ...
    function burnBatch(
        ...
        if (account == owner()) {
            for (uint256 i = 0; i < ids.length; ++i) {
                ...
                tokenInfo.totalBurned = tokenInfo.totalBurned + values[i];
            }
        }
    }
