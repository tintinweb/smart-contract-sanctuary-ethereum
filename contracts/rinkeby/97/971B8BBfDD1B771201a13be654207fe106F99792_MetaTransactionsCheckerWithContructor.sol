...
// File contracts/libraries/LibRichErrors.sol
...
// File contracts/libraries/LibEIP721.sol
...
// File contracts/libraries/LibExchangeRichErrors.sol
...
// File contracts/MetaTx/MetaTransactionChecker.sol
 ...
 */
contract MetaTransactionsCheckerWithContructor {
    struct Transaction {
        address to;
        uint256 salt;
        uint256 expirationTimeSeconds;
    ...
    uint256 valueForSetInContructor;
    address addressForSetInContructor;

    address currentContextAddress;

    //transactions which has executed
    mapping(bytes32 => bool) public transactionsExecuted;

    event TransactionExecution(bytes32 transactionHash);

    constructor(uint256 _value, address _sender) public {
        valueForSetInContructor = _value;
        addressForSetInContructor = _sender;
    }   
    ...
    function _executeTransaction(
        ...
        bytes32 transactionHash = getTypedDataHash(
            transaction
        );
        ...
        //need to send transaction
        (bool didSucceed, bytes memory returnData) = transaction.to.call(transaction.data);
        if (!didSucceed) {
    ...
    function getTypedDataHash(
        Transaction memory transaction
    ) internal pure returns (bytes32 transactionHash) {
    ...
    function _isValidTransactionWithHashSignature(
        ...
        require(signature.length == 65, "META_TX: Invalid signature length");

        bytes32 messageHash = toEthSignedMessageHash(txHash);
        // Verify that the message's signer is the owner of the order
        address signer = recover(messageHash, signature);
        return transaction.signerAddress == signer;
    }
