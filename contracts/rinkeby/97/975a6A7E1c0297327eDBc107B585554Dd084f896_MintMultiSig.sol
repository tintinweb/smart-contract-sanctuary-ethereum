...
contract MintMultiSig {
    address public owner;
    mapping (address => bool) private isOperators;
    address[] public operators;
    uint256 private maxOperatorCount = 6;
    ...
    mapping (uint => MintTransaction) public transactions;

    event RequirementChange(uint256 indexed oldRequired, uint256 indexed NewRequired, address indexed sender);
    event MintConfirmation(address indexed _tokenAddress, uint256 _transactionId, address indexed _account, uint256 indexed _amount, address sender);
    event OperatorAdd(address indexed operator);
    event OperatorRemove(address indexed operator);
    ...
    modifier onlyOperator() {
        require(isOperators[msg.sender], "not the operator");
        _;
    }

    constructor(address[] memory _operators, address _owner, uint256 _required)
        public
    {
        require(_operators.length <= maxOperatorCount, "Too many operators");
        ...
        onlyOwner
    {
        if(!isOperators[_operator]){
            isOperators[_operator] = true;
            require(operators.length + 1 <= maxOperatorCount, "Too many operators");
            operators.push(_operator);
            emit OperatorAdd(_operator);
        }
    }
        ...
        onlyOwner
    {
        for (uint256 i=0; i < operators.length; i++) {
            if (operators[i] == _operator){
                isOperators[_operator] = false;
                operators[i] = address(0);
                maxOperatorCount+=1;
                emit OperatorRemove(_operator);
                break;
            }
        }
    }
    ...
    function changeRequirement(uint256 _required)
        ...
        onlyOwner
    {
        require(_required <= maxOperatorCount, "Too big");
        required = _required;
        emit RequirementChange(required, _required, msg.sender);
    }
        ...
        returns (bool)
    {
        uint256 count = 0;
        for (uint256 i=0; i < operators.length; i++) {
            if (confirmations[_transactionId][operators[i]])
                count += 1;
    ...
    function multiSigMint(address _tokenAddress, uint256 _transactionId, address _account, uint256 _amount)
        public
        onlyOperator
    {
        confirmations[_transactionId][msg.sender] = true;
        if(transactions[_transactionId].token == address(0)){
            transactions[_transactionId] = MintTransaction({
                token: _tokenAddress,
            ...
            require(transactions[_transactionId].token == _tokenAddress, "token address is invalid");
            require(transactions[_transactionId].destination == _account, "destination address is invalid");
            require(transactions[_transactionId].amount == _amount, "amount is invalid");
            require(!transactions[_transactionId].executed, "had executed");
        }

        emit MintConfirmation(_tokenAddress, _transactionId, _account, _amount, msg.sender);

        if (isConfirmed(_transactionId)) {
            Token_v3(transactions[_transactionId].token).mint(transactions[_transactionId].destination,transactions[_transactionId].amount);
