 ...
 */
contract VotingTokenDistributor is Pausable, ReentrancyGuard, Ownable {
    ...
    // Total amount claimed by user (in GLR)
    ...
    /**
     * @notice Constructor
     * @param _gallerToken address of the GLR token
     */
    ...
    function claim(uint256 amount, bytes32[] calldata merkleProof) external whenNotPaused nonReentrant {
        ...
        gallerToken.safeTransfer(msg.sender, adjustedAmount);
     ...
     */
    function withdrawAll(uint256 amount) external onlyOwner whenPaused {
        require(block.timestamp > (lastPausedTimestamp + BUFFER_ADMIN_WITHDRAW), "Owner: Too early to withdraw");
        gallerToken.safeTransfer(msg.sender, amount);
