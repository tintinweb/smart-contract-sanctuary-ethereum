...
import "./TokenRecoveryUpgradeable.sol";

import "./ILockDrop.sol";

contract LockDrop is ILockDrop, TokenRecoveryUpgradeable, GuardableUpgradeable {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;

    bytes32 public constant override GLOBAL_ADMIN_ROLE = keccak256('LockDrop.GLOBAL_ADMIN_ROLE');
    ...
    modifier onlyAdmin {
        ...
        require(hasRole(GLOBAL_ADMIN_ROLE, sender) || hasRole(LOCAL_ADMIN_ROLE, sender), 'LockDrop: ADMIN ONLY');
        _;
    }
    ...
    function initialize(
        ...
        _updateStartEndDates(_startsAt, _endsAt);
    }

    function updateStartEndDates(uint256 _startsAt, uint256 _endsAt) external override onlyAdmin {
        _updateStartEndDates(_startsAt, _endsAt);
    }
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(GuardableUpgradeable, AccessibleUpgradeable) returns (bool) {
        return super.supportsInterface(interfaceId);
    }

    function totalBalance(bytes32 poolId) external view override returns (uint256) {
        return _pools[poolId].totalBalance;
    }

    function balanceOf(bytes32 poolId, address account) external view override returns (uint256) {
        return _pools[poolId].balances[account];
    }
    ...
    function updatePool(
        ...
        requiredAccess = requiredAccess | GuardHelper.DEFAULT_ACCESS; // Minimum default access

        _pools[poolId].enabled = enabled;
        _pools[poolId].label = label;
        _pools[poolId].requiredAccess = requiredAccess;

        emit PoolUpdated(poolId, label, requiredAccess, enabled, _msgSender());
    }
    ...
    function moveDelegation(bytes32 fromPoolId, bytes32 toPoolId, uint256 amount) external override {
        Pool storage fromPool = _pools[fromPoolId];
        Pool storage toPool = _pools[toPoolId];
        address sender = _msgSender();

        require(fromPoolId != toPoolId, 'LockDrop: SAME POOLS');
        require(startsAt <= block.timestamp, 'LockDrop: NOT STARTED');
        require(block.timestamp < endsAt, 'LockDrop: COMPLETED');
        require(amount > 0, 'LockDrop: ZERO AMOUNT');
        require(fromPool.enabled, 'LockDrop: POOL DISABLED');
        require(toPool.enabled, 'LockDrop: POOL DISABLED');
        require(fromPool.balances[sender] >= amount, 'LockDrop: INSUFFICIENT BALANCE');
        require(SOMA.guard().check(sender, toPool.requiredAccess), 'LockDrop: NO ACCESS');
        require(SOMA.guard().check(sender, fromPool.requiredAccess), 'LockDrop: NO ACCESS');

        fromPool.balances[sender] -= amount;
        fromPool.totalBalance -= amount;
        toPool.balances[sender] += amount;
        toPool.totalBalance += amount;

        emit DelegationMoved(fromPoolId, toPoolId, amount, _msgSender());
    }

    function delegate(bytes32 poolId, uint256 amount) external override {
        ...
        toPool.balances[sender] += receivedAmount;
        toPool.totalBalance += receivedAmount;

        emit DelegationAdded(poolId, receivedAmount, _msgSender());
    }

    function _updateStartEndDates(uint256 _startsAt, uint256 _endsAt) private {
        emit StartEndDatesUpdated(startsAt, _startsAt, endsAt, _endsAt, _msgSender());

        require(_startsAt != 0, 'LockDrop: INVALID START DATE');
        require(_endsAt != 0, 'LockDrop: INVALID END DATE');

        if (startsAt != _startsAt) {
            require(startsAt > block.timestamp || startsAt == 0, 'LockDrop: STARTED');
            require(_startsAt > block.timestamp, 'LockDrop: INVALID START DATE');
            startsAt = _startsAt;
        }

        // Should it be able to end?
        
        if (endsAt != _endsAt) {
            require(endsAt > block.timestamp || endsAt == 0, 'LockDrop: COMPLETED');
            require(_endsAt > _startsAt && _endsAt > block.timestamp, 'LockDrop: INVALID END DATE');
            endsAt = _endsAt;
        }
    }
...
abstract contract TokenRecoveryUpgradeable is AccessibleUpgradeable {
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IERC20;
    ...
    bytes32 public constant TOKEN_RECOVERY_ROLE = keccak256('TokenRecovery.TOKEN_RECOVERY_ROLE');

    EnumerableSet.AddressSet private _disabledTokens;

    function recoverTokens(address asset, address to, uint256 amount) external onlyRole(TOKEN_RECOVERY_ROLE) {
        require(!_disabledTokens.contains(asset), 'TokenRecover: cannot recover this token.');
