...
contract YumiosWithRaribleRoyalties is Yumios, RoyaltiesV2Impl {
...
contract Yumios is ERC721PresetMinterPauserAutoId {

    string private constant NAME = "FIWORK-GREEN";

    string private constant SYMBOL = "FWR";

    uint256 private constant MAX_ITEMS = 30;
...
abstract contract ERC721PresetMinterPauserAutoId is Base, Context, ERC721Burnable, ERC721Pausable, IERC721Mintable {
    ...
    uint256 internal constant DEFAULT_START_TOKEN_ID = 1;
     ...
     *
     * For this function to work, the token ID assigned to a minted NFT should have 
     * been generated sequentially (with increments of 1), with the starting starting 
     * token ID being 1 (the function '_startTokenId()' must return 1)
     */
    function tokenOfOwnerByIndex(address owner, uint256 index) public virtual view returns (uint256) {
        require (index >= 0 && index < balanceOf(owner), "token index out of bounds");

        // get the total number tokens that have been minted so far
        uint256 totalMintedSoFar = _totalMinted();
        uint256 tokenIdsIndex;
        uint256 tokenId;
        address currentOwnershipAddr;

        // index overflow is impossible as the loop breaks when index 'i' > totalMintedSoFar.
        unchecked {
            /* loop through the token ID of each minted NFT. Take note 
            that token ID's are generated sequentially staring from 1 */
            for (uint256 i; i <= totalMintedSoFar; i++) {
                TokenOwnership memory ownership = _ownerships[i];
                if (ownership.addr != address(0)) {
                    currentOwnershipAddr = ownership.addr;
                }

                if (currentOwnershipAddr == owner) {
        ...
        // returns 0 if no token could be found because 1 is the starting token id 
        return tokenId;
    }
