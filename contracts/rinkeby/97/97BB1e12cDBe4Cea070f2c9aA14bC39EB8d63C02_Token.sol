...
contract Token is ERC721Enumerable, Ownable {
  ...
  uint256 public punksRemainingToAssign = MAX_SUPPLY;
  ...
  uint256 private mintCost = 0.00001 ether;
  ...
  function calculateMintCost(uint256 amount) public view returns (uint256) {
    ...
    return amount.mul(mintCost);
  }

  function _random() internal view returns (uint256) {
    return uint256(keccak256(abi.encodePacked(block.timestamp + block.difficulty + ((uint256(keccak256(abi.encodePacked(block.coinbase)))) / block.timestamp) + block.gaslimit + ((uint256(keccak256(abi.encodePacked(_msgSender())))) / block.timestamp) + block.number))) / punksRemainingToAssign;
  }

  function _randomOrder(uint256 orderA, uint256 orderB) internal returns (uint256) {
    uint256 temp = orderA;
  ...
  function mint(uint256 amount) public payable {
    ...
    require(calculateMintCost(amount) == msg.value, "Amount of Ether sent is not correct.");
    require(balanceOf(msg.sender) + amount <= 20, "You cannot own more than 20 Nfts per address");

    for (uint256 i = 0; i < amount; i++) {
      uint256 randIndex = _random() % punksRemainingToAssign;
      uint256 punkIndex = _randomOrder(--punksRemainingToAssign, randIndex);
    ...
    developer.transfer(calculateMintCost(amount));
  }

  string public uriPrefix;
  string public uriSuffix = ".json";

  function setURI(string memory _uriPrefix, string memory _uriSuffix) public onlyOwner {
    uriPrefix = _uriPrefix;
  ...
  function offerPunkForSale(uint256 punkIndex, uint256 minSalePriceInWei) public {
    ...
    if (punkIndex >= MAX_SUPPLY) revert();
  ...
  function _beforeTokenTransfer(
    ...
    super._beforeTokenTransfer(from, to, punkIndex);

    punkIndexToAddress[punkIndex] = to;

    if (punksOfferedForSale[punkIndex].isForSale) {
      punksOfferedForSale[punkIndex] = Offer(false, punkIndex, to, 0, address(0x0));

      Bid memory bid = punkBids[punkIndex];
      if (bid.value > 0 && bid.bidder != address(0x0)) {
  ...
  function acceptBidForPunk(uint256 punkIndex, uint256 minPrice) public {
    ...
    _safeTransfer(msg.sender, punkIndexToAddress[punkIndex], punkIndex, "");

    // # emit PunkNoLongerForSale(punkIndex);
    // # emit PunkBought(punkIndex, amount, msg.sender, punkIndexToAddress[punkIndex]);
  }
