...
contract ConnectFour {
    ...
    function initializeGame() external payable returns (uint256) {
        require(msg.value <= maxBetAmount && msg.value >= minBetAmount, "ConnectFour: bet amount must be between min and max");
    ...
    function startGame(uint256 _gameId) external payable {
        ...
        require(games[_gameId].player1 != msg.sender, "ConnectFour: can't start the same game you initialized");
        require(msg.value == games[_gameId].betAmount, "ConnectFour: bet amount needs to match betAmount");
    ...
    function playMove(uint256 _gameId, uint256 _col) external {
        ...
        require(game.isPlayer1Turn ? msg.sender == game.player1 : msg.sender == game.player2, "ConnectFour: it isn't your turn");

        for (uint256 row = 0; row <= 5; row++) {
            uint256 idx = boardIndex(_col, row);
            if (game.board[idx] == Disc.Empty) {
                game.board[idx] = game.isPlayer1Turn ? Disc.Player1 : Disc.Player2;
                game.isPlayer1Turn = !game.isPlayer1Turn;
                break;
            } else if (row == 5)  {
                revert("ConnectFour: the column is full");
            }
        }
    ...
    function claimReward(uint256 _gameId, address payable _recipient, uint256 _startingWinDiscCol, uint256 _startingWinDiscRow, WinningDirection _direction) external {
        Game storage game = games[_gameId];
        require(game.status == Status.Started, "ConnectFour: game hasn't started, doesn't exist, or is already over");
        require(game.player1 == msg.sender || game.player2 == msg.sender, "ConnectFour: you must be a player in the game to claim the reward");
        
        Disc disc = game.player1 == msg.sender ? Disc.Player1 : Disc.Player2;

        uint256 currCol = _startingWinDiscCol;
        uint256 currRow = _startingWinDiscRow;
        for (uint256 i = 0; i <= 3; i++) {
            uint256 idx = boardIndex(currCol, currRow);
            require(game.board[idx] == disc, "ConnectFour: discs must all be the same player's");

            if (_direction == WinningDirection.LeftDiagonal) {
                if (currCol == 0 && i == 3) {
                    break;
                } else if (currCol == 0) {
                    revert("ConnectFour: column in provided winning line is out of bounds");
                }
                currCol--;
                currRow++;
    ...
    function boardIndex(uint256 _col, uint256 _row) public pure returns (uint256) {
        require(_row <= 5 && _col <= 6, "ConnectFour: column must be < 6 and row must < 5");
        return (7 * _row) + _col;
    }
}
