...
contract MoonPotatoz is ERC721A, Ownable, ReentrancyGuard {
  ...
  function mint(uint256 _count) external payable {
    ...
    require(supply + _count <= max_Supply, 'No more');

    if (msg.sender != owner()) {
      bool isFree = ((supply + _count < maxFreeSupply + 1) &&
        (_mintedFreeAmount[msg.sender] + _count <= maxPerFree));
      if (isFree) {
        cost = 0;
        _mintedFreeAmount[msg.sender] += _count;
      }
      require(msg.value >= _count * needToUpdateCost(supply), 'Please send the exact amount.');
    }
