...
contract TheNFTIslands is ITheNFTIslands, ERC721A, Ownable, AccessControl {
    ...
    uint256 public pricePresaleStd = 0.001 ether;
    uint256 public pricePresaleLrg = 0.001 ether;
    uint256 public pricePresalePrm = 0.001 ether;

    // public sale price for each token types
    uint256 public pricePublicStd = 0.05 ether;
    uint256 public pricePublicLrg = 0.1 ether;
    uint256 public pricePublicPrm = 0.15 ether;

    // max supply for each token types
    uint256 public standardMaxSupply = 5000;
    uint256 public largeMaxSupply = 4000;
    uint256 public premiumMaxSupply = 1000;

    // current supply for each token types
    uint256 public standardCurrentSupply;
    uint256 public largeCurrentSupply;
    uint256 public premiumCurrentSupply;

    /*
     * Max mint per transaction per token type
     */
    uint256 public MAX_PUBLIC_MINT_STD = 2;
    uint256 public MAX_PUBLIC_MINT_LRG = 3;
    uint256 public MAX_PUBLIC_MINT_PRM = 4;
    uint256 public MAX_WHITELIST_MINT_STD = 2;
    uint256 public MAX_WHITELIST_MINT_LRG = 1;
    uint256 public MAX_WHITELIST_MINT_PRM = 1;
     ...
     */
    uint256 internal constant fixedPayment = 0.5 ether;
    uint256 internal fixedPaymentReleased;

    uint256 internal constant totalShares = 100;
    uint256 internal totalReleased;
    mapping(address => uint256) internal released;
    mapping(address => uint256) internal shares;
    address internal constant project = 0x2da217E7518AA40767aE73C2872bba10807aab9d;
    address internal constant shareHolder2 = 0x70997970C51812dc3A010C7d01b50e0d17dc79C8;
    address internal constant shareHolder3 = 0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC;
    address internal constant dev = 0x90F79bf6EB2c4f870365E785982E1f101E93b906;

    constructor() ERC721A("TNI", "TNI") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(SIGNER_ROLE, msg.sender);

        shares[project] = 100;
        shares[shareHolder2] = 0;
        shares[shareHolder3] = 0;
        shares[dev] = 0;
    ...
    function setStandardPresalePrice(uint256 newPriceInWei) external onlyOwner {
        pricePresaleStd = newPriceInWei;
    }
     ...
     *
     */
    function setLargePresalePrice(uint256 newPriceInWei) external onlyOwner {
        pricePresaleLrg = newPriceInWei;
    }
     ...
     *
     */
    function setPremiumPresalePrice(uint256 newPriceInWei) external onlyOwner {
        pricePresalePrm = newPriceInWei;
    }

    /*
     * change price of standard islands in presale
     *
     * @param newPriceInWei: price in wei
     *
     */
    function setStandardPublicPrice(uint256 newPriceInWei) external onlyOwner {
        pricePublicStd = newPriceInWei;
    }

    /*
     * change price of large islands in public
     *
     * @param newPriceInWei: price in wei
     *
     */
    function setLargePublicPrice(uint256 newPriceInWei) external onlyOwner {
        pricePublicLrg = newPriceInWei;
    }

    /*
     * change price of premium islands in public
     *
     * @param newPriceInWei: price in wei
     *
     */
    function setPremiumPublicPrice(uint256 newPriceInWei) external onlyOwner {
        pricePublicPrm = newPriceInWei;
    }

    /*
     * change max mint per transaction
     *
     * @param _maxPublicStd: max number of standard token per transaction in public sale
     * @param _maxPublicLrg: max number of large token per transaction in public sale 
     * @param _maxPublicPrm: max number of premium token per transaction in public sale 
     * @param _maxPresaleStd: max number of standard token per transaction in presale
     * @param _maxPresaleLrg: max number of large token per transaction in presale
     * @param _maxPresalePrm: max number of premium token per transaction in presale
     *
     */
    function setMaxPerTransaction(
      uint256 _maxPresaleStd, 
      uint256 _maxPresaleLrg,
      uint256 _maxPresalePrm,
      uint256 _maxPublicStd,
      uint256 _maxPublicLrg,
      uint256 _maxPublicPrm
    ) external onlyOwner {
        MAX_PUBLIC_MINT_STD = _maxPublicStd;
        MAX_PUBLIC_MINT_LRG = _maxPublicLrg;
        MAX_PUBLIC_MINT_PRM = _maxPublicPrm;
        MAX_WHITELIST_MINT_STD = _maxPresaleStd;
        MAX_WHITELIST_MINT_LRG = _maxPresaleLrg;
        MAX_WHITELIST_MINT_PRM = _maxPresalePrm;
    }

    /*
     * change merkleroot
     *
     * @param newMerkleRoot: merkle root from the whitelist merkle tree
     *
     */
     ...
     * Error messages:
     *  - I2: "Wrong price"
     *  - I3: "Public not started"
     *  - I5: "Max quantity per transaction reached"
     */
    function publicMint(
        uint256 quantityStd,
        uint256 quantityLrg,
        ...
        require(quantityStd <= MAX_PUBLIC_MINT_STD && quantityLrg <= MAX_PUBLIC_MINT_LRG && quantityPrm <= MAX_PUBLIC_MINT_PRM, "T5");
     ...
     *  - I2: "Wrong price"
     *  - I5: "Max quantity per transaction reached"
     *  - I6: "Presale not started"
     *  - I7: "Presale is closed, please mint in public"
     *  - I8: "Merkleroot has not been set yet"
     *  - I9: "You are not whitelisted"
     */
    function presaleMint(
        ...
        require(quantityStd <= MAX_WHITELIST_MINT_STD && quantityLrg <= MAX_WHITELIST_MINT_LRG && quantityPrm <= MAX_WHITELIST_MINT_PRM, "T5");

        require(merkleRoot != 0, "T8");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProofLibrary.verify(merkleProof, merkleRoot, leaf),
            "T9"
        );
     ...
     *
     * Error messages:
     *  - I4: "Collection's max supply reached"
     */
    ...
    function setBaseURI(string calldata baseURI) external onlyOwner {
        require(!isUriFreezed, "URI has been freezed");
     ...
     * Error messages:
     * - I11 : "No shares for this account"
     * - I12 : "No remaining payment"
     */
    function withdraw(address account) external {
        uint256 fixedDelta = 0;

        if (fixedPayment != fixedPaymentReleased) {
          fixedDelta = fixedPayment - fixedPaymentReleased;
        }

        require(shares[account] > 0, "T11");

        if (address(this).balance + totalReleased > fixedDelta) {
          uint256 totalReceived = address(this).balance + totalReleased - fixedDelta;
          ...
          if (account == dev) {
            payment += fixedDelta;
            fixedPaymentReleased += fixedDelta;
          }
          ...
          require(account == dev, "No funds to withdraw yet");
          uint256 payment = address(this).balance;
          fixedPaymentReleased += payment;
          payable(account).transfer(payment);
        }        
    }
