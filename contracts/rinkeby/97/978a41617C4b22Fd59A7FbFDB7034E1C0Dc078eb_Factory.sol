...
contract Factory {
	...
	function deploy(bytes32 _salt, string memory name, string memory symbol, string memory tokenURIPrefix) external returns(address addr) {
		addr = address(new RUSTNFTUserToken721{salt: _salt}(name, symbol, tokenURIPrefix));
		RUSTNFTUserToken721 token = RUSTNFTUserToken721(address(addr));
...
contract RUSTNFTUserToken721 is ERC721 {
    ...
    function createCollectible(string memory tokenURI, address[] memory royalty, uint256[] memory royaltyFee) external returns (uint256) {
