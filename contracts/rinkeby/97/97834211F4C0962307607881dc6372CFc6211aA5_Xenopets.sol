...
contract Xenopets is ERC721, Ownable, AccessControl {
    using Strings for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenSupply;
    Counters.Counter private _currentPlanetCounter;

    bytes32 public constant WHITE_LIST_ROLE = keccak256("WHITE_LIST_ROLE");

    uint256 public constant unit_cost = 0.07 ether;
    ...
    mapping(string => Counters.Counter) public planetTotalSupply;
    mapping(string => uint256) public planetSupplyOffset;
        ...
        ERC721(_name, _symbol)
    {
        ...
        planetSupplyOffset["Terra Zoi"] = 0;
        planetSupplyOffset["Xeros Sfaria"] = total_supply_per_planet;
        planetSupplyOffset["Pagos Prasinos"] = total_supply_per_planet * 2;
        planetSupplyOffset["Mykitas Thanatos"] = total_supply_per_planet * 3;
    }
    ...
    function mint(uint256 num) public payable saleNotPaused {
        ...
        uint256 supply = _currentPlanetCounter.current() + 0;
        require(
        ...
        require(
            ...
            "Xenopets: Ether sent is less than unit_cost * num"
        );
        for (uint256 i = 0; i < num; i++) {
            _tokenSupply.increment();
            _currentPlanetCounter.increment();
            planetTotalSupply[currentPlanet].increment();
            _safeMint(
                msg.sender,
                _currentPlanetCounter.current() +
                    planetSupplyOffset[currentPlanet]
            );
        }
    }
    ...
    function adminMint(uint256 num) public payable onlyOwner {
        require(num > 0, "Xenopets: amount must be at least 1");
    ...
    function withdraw(address payable receiver) external onlyOwner {
        require(receiver != address(0), "Xenopets: Cannot withdraw to null");
    ...
    function planetTokensMinted() public view returns (uint256) {
        return _currentPlanetCounter.current();
    }
        ...
        returns (uint256[] memory)
    {
        ...
        if (tokenCount == 0) {
            ...
            uint256 totalXenos = planetTotalSupply[_planet].current() +
                planetSupplyOffset[_planet];
            uint256 resultIndex = 0;

            uint256 xenoId;

            for (
                xenoId = 1 + planetSupplyOffset[_planet];
                xenoId <= totalXenos;
                xenoId++
            ) {
