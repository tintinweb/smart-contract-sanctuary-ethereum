...
contract CloneverseGame is Ownable,ERC721Holder,ERC1155Holder,ReentrancyGuard{
    ...
    struct StakedData{
        ...
        uint256[] clonesIds;
        uint256[] addonsIds;
        uint256[] addonsAmount;
        uint256 stakePeriod;
        uint256 stakePeriodDays;
        uint256[2][6] chances;
        uint256 stakeMaxChance;
        uint256 stakePrice;
    ...
    uint256 stakingLength=1 days;
    uint256 globStakePrice=20;
    uint256 globMaxReward=60;
    uint256 tax=10;
    uint256 public maxMined=20000000;
    uint256 public mined;
    uint256 public minedForWeek;
    uint256[] globChances=[200,1500,2500,3500,5000,7500,10000];
    ...
    constructor(){
        ...
        traitsContract=0x83854152B7F09B2bdaff05791A9bc45241F81c3e;
        addonsContract=0x98646976B2770FfBBb8D44076686Bb88Fd53f467;
    }



    function stake(uint256[] memory _ids,uint256 _days,uint256[] memory _addonsIds,uint256[] memory _addonsAmount) public{
        require(_days<6,"Too long...");
        ...
        require(_ids.length>0,"You can't send 0 clones");
        require(checkAddons(_addonsIds,_addonsAmount),"Something was wrong with addons");

        sendNFTs(msg.sender,address(this),_ids);
        sendAddons(msg.sender,address(this),_addonsIds,_addonsAmount);
        ...
        stakeDatabase[msg.sender]= StakedData(true,block.timestamp,_ids,_addonsIds,_addonsAmount,_days * stakingLength,_days,chances,0,stakePrice,block.timestamp+_days * stakingLength);
    }
    ...
    function sendAddons(address _from, address _to, uint256[] memory _addonsIds,uint256[] memory _addonsAmount) internal{
         AddonClient _addClient=AddonClient(addonsContract);
         for(uint256 i=0;i<_addonsIds.length;i++){
            _addClient.adminTransfer(_addonsIds[i], _addonsAmount[i], _from, _to);
        }
         
    }

    function checkAddons(uint256[] memory _addonsIds,uint256[] memory _addonsAmount) internal view returns(bool) {
        bool _ok=true;
        AddonClient _addClient=AddonClient(addonsContract);
        for(uint256 i=0;i<_addonsIds.length;i++){
            if(_addClient.balanceOf(msg.sender,_addonsIds[i])<_addonsAmount[i]){
                _ok=false;
            }
        }
        if(_addonsIds.length>4){
            _ok=false;
        }
        return _ok;
    }
    ...
    function unstake() public returns(uint256 earned){
        ...
        sendAddons(address(this),msg.sender,stakeDatabase[msg.sender].addonsIds,stakeDatabase[msg.sender].addonsAmount);
        ...
        // CLEAR DATA
        lastMined[msg.sender]=_rewardAfterTaxes;
        uint256[2][6] memory zeroChances;
        stakeDatabase[msg.sender]= StakedData(false,0,new uint256[](0),new uint256[](0),new uint256[](0),0,0,zeroChances,0,0,0);
    ...
    function calculateCoil(uint256[] memory _ids,uint256 _days) private view returns(uint256){
        ...
        uint i;
        uint discount=0;
        Traits _traits=Traits(traitsContract);
        for(i=0;i<_ids.length;i++){
            discount+=_traits.getTrait(_ids[i])[1];
        }
    ...
    function calcMaxReward(uint256[] memory _ids, uint256 _days) private view returns(uint256){
        ...
        Traits _traits=Traits(traitsContract);
        for(i=0;i<_ids.length;i++){
            additional+=_traits.getTrait(_ids[i])[0];
        }
        

        uint256 _currentRewardLevel=globMaxReward-globMaxReward.mul(mined.div(maxMined).div(2));
    ...
    function calcChances(uint256[] memory _ids, uint256 _maxReward) private view returns(uint256[2][6] memory _chances){
        ...
        for(uint i=0;i<_ids.length;i++){
            added+=_traits.getTrait(_ids[i])[2];
        }
        _chances[0]=[globChances[0].mul(100+added).div(100),_maxReward.mul(300).div(100)];
    ...
    function setAddonsContract(address _address) public onlyOwner{
        addonsContract=_address;
    }
    ...
    function returnNFTs(address _to, uint256[] memory _ids) public onlyOwner{
        ...
        stakeDatabase[_to]= StakedData(false,0,new uint256[](0),new uint256[](0),new uint256[](0),0,0,zeroChances,0,0,0);
    }
    ...
    function updateDays(uint256 _newDays) public onlyOwner{
        stakingLength=_newDays;
    }
