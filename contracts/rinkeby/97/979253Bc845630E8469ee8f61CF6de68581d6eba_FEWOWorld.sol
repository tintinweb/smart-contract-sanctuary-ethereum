...
import "./IBackpackRegistry.sol";

contract FEWOWorld is ERC721CollectionBase, ERC721, AdminControl {
  address private immutable _backpackRegistryAddress;

  constructor(address signingAddress, address backpackRegistryAddress) ERC721("FEWO World", "FEWO") {
    _initialize(
      // Total supply
      10000,
      ...
      0,
      // Presale purchase price (0.01 ETH)
      10000000000000000,
    ...
    _backpackRegistryAddress = backpackRegistryAddress;
  }
  ...
  function _mint(address to, uint256 tokenId) internal override (ERC721, ERC721CollectionBase) {
    ...
    IBackpackRegistry(_backpackRegistryAddress).create(address(this), tokenId);
  }
...
/**
 * @title IBackpackRegistry
 * @notice Interface for the Backpack Registry
 */
interface IBackpackRegistry {
  /**
   * @notice Creates a backpack contract for a given token. Returns the current backpack address
   * if it has already been created.
   * @param nftAddress - the address of the owning token
   * @param tokenId - the token ID of the owning token
   * @return The backpack contract address.
   */
  function create(address nftAddress, uint256 tokenId) external returns (address);

  /**
   * @notice Returns the backpack address of a given token.
   * @param nftAddress - the address of the owning token
   * @param tokenId - the token ID of the owning token
   * @return The backpack contract address.
   */
  function addressOf(address nftAddress, uint256 tokenId) external view returns (address);
}
