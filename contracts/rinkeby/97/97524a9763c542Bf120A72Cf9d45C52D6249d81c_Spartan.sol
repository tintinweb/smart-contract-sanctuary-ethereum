...
/**s
 * @title ERC1155 Token Contract for Spartan Unbreakable NFT pass
 * @notice This contract is meant to serve for both Semi and Non-fungible tokens as part of the Spartan Unbreakable NFT pass
...
contract Spartan is Ownable, ERC1155Pausable, ERC1155Supply, ERC1155Burnable, PaymentSplitter {
    ...
    string private constant _METADATA_EXTENTION = ".json";
    ...
    /// @notice Indicates the number of mints per user 
    mapping(address => uint256) public _mintsPerUser;

    /// @notice Contract Token name
    string public name;

    /// @notice Contract Token symbol
    string public symbol;

    /// @notice Max token Id that can be minted;
    uint8 public maxId;

    /// @notice Admin mint event to be emitted upon admin mint
    event AdminMinted
    (
        address indexed to,
        ...
        onlyOwner
    {
        ...
        (
            tokenId_<=maxId, 
            "SPTN: Can't change price of nonexistant token Id"
        );
        require
        (
    ...
    function mint(
        ...
        (
            tokenId <= maxId,
            "SPTN: Token id mismatch"
        );
        require(
            amount + totalSupply(tokenId) <= supplyPerTokenId[tokenId],
            "SPTN: Amount mismatch"
        );
        require(
            checkLimit(to, amount), "SPTN: Exceeded amount minted by user"
        );
        require
        (
            !mintId[mintId_],
            "SPTN: mint id already used"
        );
    ...
    function _initialAddTokens(
        ...
        (
            ...
            "SPTN: IDs/MintCost arity mismatch"
        );

        require
        (
            maxId_ + 1 == supplyPerTokenId_.length,
            "SPTN: IDs/Supply arity mismatch"
        );

        for (uint256 i = 0; i <= maxId_; i++) {
    ...
    {
        require(
            _mintsPerUser[minter]+amount <= mintingLimit,
