...
contract PB is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public PRICE = 0.008 ether; // 8000000000000000
    ...
    function mint(uint256 _amount) public payable mintCompliance(_amount) {
        if(freeToClaim(msg.sender) == 0 || FREE_MINTED >= FREE_SUPPLY) {
            require(msg.value >= PRICE * _amount, "Insufficient funds");
        }else{
            uint256 free = freeToClaim(msg.sender);
            if(_amount > free){
                require(msg.value >= PRICE * (_amount - free), "Insufficient funds");
            }
            FREE_MINTED += free;
        }
    ...
    function freeToClaim(address _owner) public view returns (uint256){
        if(numberMinted(_owner) >= FREE_PER_WALLET || FREE_MINTED >= FREE_SUPPLY){
            return 0;
        }
        uint256 claim = FREE_PER_WALLET - numberMinted(_owner);
        uint256 supply = FREE_SUPPLY - FREE_MINTED;
        if(claim >= supply){
            return supply;
        }
        return claim;
    ...
    function mintForAddress(uint256 _amount, address _address) public payable mintCompliance(_amount) onlyOwner {
        require(totalSupply() + _amount <= maxSupply, "Exceeds max supply");
    ...
    function saleState() external view returns (bool, uint256, uint256, uint256, uint256, uint256, uint256) {
        return (
            SALE_ACTIVE, FREE_SUPPLY, freeToClaim(msg.sender), PRICE, MAX_PER_TX, totalSupply(), maxSupply
        );
    }
