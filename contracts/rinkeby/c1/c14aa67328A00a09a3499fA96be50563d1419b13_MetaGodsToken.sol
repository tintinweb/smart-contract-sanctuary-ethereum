...
contract MetaGodsToken is Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable {

    address private signerAddress;

    uint private withdrawRequestLifetimeSeconds;

    uint256 public withdrawTaxFunds;
    ...
    event WithdrawalRequestFinished(address wallet, uint256 internalIdentifier);
    ...
    function signalWithdrawFinished(address wallet_, uint256 internalIdentifier_, bool successful) internal {
        handledRequests[internalIdentifier_] = successful;
        emit WithdrawalRequestFinished(wallet_, internalIdentifier_);
    }
    ...
    function _validateWithdrawSignature(
        uint256 internalIdentifier_, uint256 amount_, uint generationDate_, bytes calldata signature_
    ) internal view {
        bytes32 dataHash = keccak256(abi.encodePacked(internalIdentifier_, msg.sender, amount_, generationDate_));
    ...
    function withdraw(
        ...
        uint256 taxed = amount_ * withdrawTax / 100;
        withdrawTaxFunds += taxed;
    ...
    function withdrawTaxes(uint256 amount_) external onlyOwner {
        require(withdrawTaxFunds >= amount_);
        _mint(msg.sender, withdrawTaxFunds);
        withdrawTaxFunds -= amount_;
    }
    ...
    function setWithdrawRequestLifetimeSeconds(uint withdrawRequestLifetimeSeconds_) external onlyOwner {
