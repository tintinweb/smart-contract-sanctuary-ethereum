...
// File: DungeonsAndDragons.sol

contract DungeonsAndDragons is VRFConsumerBase, ERC721 {
    ...
    mapping(uint256 => bytes32) requestIdByTokenId;
    ...
    constructor(
        ...
        address _link,
        bytes32 _keyHash
    ) public VRFConsumerBase(_vrfCoordinator, _link) ERC721("Dungeons and Dragons", "DnD") {
        artist = _artist;
    ...
    function createCharacter(string memory name) external payable returns (uint256 tokenId) {
    ...
    function isCharacterPending(uint256 tokenId) public view returns (bool) {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        PendingCharacter storage pendingCharacter = pendingCharacterByRequestId[requestId];
        require(
            pendingCharacter.owner != address(0),
            "Token is not being created"
        );

        Race race = Race(randomness % 10);
        CharacterClass characterClass = CharacterClass(randomness % 10 ** 2);
        uint256 strength = randomness % 10 ** 3;
        uint256 constitution = randomness % 10 ** 4;
        uint256 dexterity = randomness % 10 ** 5;
        uint256 intelligence = randomness % 10 ** 6;
        uint256 wisdom = randomness % 10 ** 7;
        uint256 charisma = randomness % 10 ** 8;
