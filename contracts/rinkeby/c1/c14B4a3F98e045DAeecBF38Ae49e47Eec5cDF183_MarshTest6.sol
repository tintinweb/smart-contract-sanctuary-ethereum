...
contract MarshTest6 is ERC721AQueryable, Ownable, ReentrancyGuard {
    using Strings for string;

    uint256 public constant WHITELIST_LIMIT = 2;
    uint256 public constant MAX_SUPPLY = 6444;
    uint256 public giveawayTokens = 10; 
    uint256 public PRICE = 0;//10000000000000000; // 0.01 ETH  main launch will be at   -------------------------------------------check on pricing for whitelist and main mint
    uint256 public CLAIM_QUANTITY = 173;
    uint256 public FREE_QUANTITY = 10;
    ...
    // bool public presale = true;   // True means presale is active, false means presale is not active.
    ...
    uint256[8] private paymentShares;
    address[8] private paymentAddresses;

    constructor(address _lolTestAddress, address[8] memory _addresses, uint256[8] memory _shares) ERC721A("MarshTest6", "MT6") {
    ...
    function claimMint(uint256[] memory _tokenIds) external nonReentrant{
        ...
        for (uint256 i = 0; i < tokenIdsLength; i++) {
            ...
            require(LolTestCont.ownerOf(_tokenIds[i]) == msg.sender, "9");
        }
        require(msg.sender == tx.origin, "5");
        for (uint256 i = 0; i < tokenIdsLength; i++) {
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        uint256 pay8 = (balance * paymentShares[7])/sumShare;
        uint256 partnerTransfer = balance - (pay1 + pay2 + pay3 + pay4 + pay5 + pay6 + pay7 + pay8);
        ...
        Address.sendValue(payable(paymentAddresses[7]), pay8);
