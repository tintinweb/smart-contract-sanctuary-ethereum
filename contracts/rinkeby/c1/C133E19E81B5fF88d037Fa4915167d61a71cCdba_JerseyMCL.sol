...
contract JerseyMCL is ERC721, ERC721URIStorage, Ownable, SVG {
    ...
    function _getPlayerName() internal view returns (string memory){
        uint256 ind = 0;//_getNewIndex();
        if (ind > 0  && ind < 11) return players[0];
        if (ind > 10 && ind < 21) return players[1];
        if (ind > 20 && ind < 31) return players[2];
        if (ind > 30 && ind < 41) return players[3];
        if (ind > 40 && ind < 51) return players[4];
        if (ind > 50 && ind < 61) return players[5];
        if (ind > 60 && ind < 71) return players[6];
        if (ind > 70 && ind < 81) return players[7];
        if (ind > 80 && ind < 91) return players[8];
        return players[9];
    }
    ...
    function nftMint() public returns (uint){
        ...
        playerName[tokenId] = _getPlayerName();
