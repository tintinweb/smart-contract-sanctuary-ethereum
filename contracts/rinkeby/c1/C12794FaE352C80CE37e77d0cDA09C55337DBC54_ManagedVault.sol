...
contract ManagedVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, IManagedVault {
     ...
     * Emits {Minted} events.
     */
    function mint(uint256 maxPendingDeposits) external onlyOwner {
        uint256 mintedPendingDeposits = 0;

        // Mint for the native cryptocurrency (Ether for the Ethereum Network)
        while (!pendingDeposits.isEmpty() && mintedPendingDeposits < maxPendingDeposits) {
        ...
        for (uint256 i = 0; i < tokenDepositStates.length(); i++) {
            ...
            while (!pendingTokenDeposits[depositTokenAddress].isEmpty() && mintedPendingDeposits < maxPendingDeposits) {
    ...
    function totalPendingNativeDepositAmount(uint256 maxBlockNumber) external view returns (uint256 totalAmount) {
        for (uint256 i = 0; i < pendingDeposits.length(); i++) {
            PendingDeposit.Item memory pendingDeposit = pendingDeposits.peekIndex(i);
            if (pendingDeposit.blockNumber > block.number) {
                break;
            }
            totalAmount += pendingDeposit.amount;
        }
