...
abstract contract AConvexAutocompounder is Multicall {
   ...
   */
  function compound() external virtual isAuthorized(OLib.STRATEGIST_ROLE) {
    _claim();
...
contract ConvexAutocompounder is AConvexAutocompounder, BasePairLPStrategy {
  ...
  int128 public seniorCrvIndex;
  int128 public juniorCrvIndex;
    ...
    returns (uint256, uint256)
  {
    ...
    vault_.seniorExcess = (_totalSenior - _seniorMinIn) + _extraSenior;
    vault_.juniorExcess = (_totalJunior - _juniorMinIn) + _extraJunior;

    _splitAndDepositConvex(_vaultId);
    ...
    return (_seniorMinIn, _juniorMinIn);
  }

  function _splitAndDepositConvex(uint256 _vaultId) internal {
    uint256 total3CRVAmount = _mint3CRV(_vaultId);
    if (total3CRVAmount <= 10000) {
      return;
    }
  ...
  function _mint3CRV(uint256 _vaultId) internal returns (uint256) {
    Vault memory vault_ = vaults[_vaultId];
    uint256[3] memory amounts;
    amounts[0] = IERC20(DAI).balanceOf(address(this));
    amounts[1] = IERC20(USDC).balanceOf(address(this));
    amounts[2] = IERC20(USDT).balanceOf(address(this));

    // Check to make sure that the excess assets are not invested.
    if (seniorCrvIndex == 0) {
      amounts[0] -= vault_.seniorExcess;
    } else if (seniorCrvIndex == 1) {
      amounts[1] -= vault_.seniorExcess;
    } else if (seniorCrvIndex == 2) {
      amounts[2] -= vault_.seniorExcess;
    }

    if (juniorCrvIndex == 0) {
      amounts[0] -= vault_.juniorExcess;
    } else if (juniorCrvIndex == 1) {
      amounts[1] -= vault_.juniorExcess;
    } else if (juniorCrvIndex == 2) {
      amounts[2] -= vault_.juniorExcess;
    }
    ...
    return THREE_CRV_LP.balanceOf(address(this)) - balanceBefore;
  }

  function compound() external override isAuthorized(OLib.STRATEGIST_ROLE) {
    _claim();
    _swapRewardTokensToStableAsset();
    _splitAndDepositConvex(currentVaultId);
  }
    ...
    returns (uint256 seniorReceived, uint256 juniorReceived)
  {
    ...
    vals.amountOfLp = THREE_CRV_LP.balanceOf(address(this));

    // Log the balance of assets within the contract to avoid collisions with unclaimed assets
    vals.balSr = vault_.senior.balanceOf(address(this));
    vals.balJr = vault_.junior.balanceOf(address(this));

    _swapLpToStablesVault(0, 0, vals.amountOfLp);

    vals.seniorReceived = vault_.senior.balanceOf(address(this)) - vals.balSr;
    vals.juniorReceived = vault_.junior.balanceOf(address(this)) - vals.balJr;

    if (address(vault_.senior) != address(vault_.junior)) {
      if (vals.seniorReceived < _seniorExpected) {
        (vals.seniorReceived, vals.juniorReceived) = _swapForSr(
          _vaultId,
        ...
        if (vals.seniorReceived > _seniorExpected) {
          ...
          vals.juniorReceived += _swapExactIn(
            vals.seniorReceived - _seniorExpected,
            0,
            path
          );
        }
      ...
      vals.seniorReceived = vals.seniorReceived / 2;
      vals.juniorReceived = vals.juniorReceived / 2;

      if (_seniorExpected > vals.seniorReceived) {
        uint256 seniorNeeded = _seniorExpected - vals.seniorReceived;
        if (seniorNeeded > vals.juniorReceived) {
          vals.seniorReceived += vals.juniorReceived;
          vals.juniorReceived = 0;
        } else {
          vals.seniorReceived = _seniorExpected;
          vals.juniorReceived -= seniorNeeded;
        }
      } else if (vals.seniorReceived > _seniorExpected) {
        vals.juniorReceived += vals.seniorReceived - _seniorExpected;
        vals.seniorReceived = _seniorExpected;
      }
    }
    require(
      _seniorMinOut <= vals.seniorReceived &&
        _juniorMinOut <= vals.juniorReceived,
      "Too Much Slippage"
    );

    vault_.senior.ondoSafeIncreaseAllowance(
      msg.sender,
      vals.seniorReceived + vault_.seniorExcess
    );
    vault_.junior.ondoSafeIncreaseAllowance(
      msg.sender,
      vals.juniorReceived + vault_.juniorExcess
    );

    _burn(_vaultId, shares);
    emit Redeem(_vaultId);
    return (vals.seniorReceived, vals.juniorReceived);
  }
