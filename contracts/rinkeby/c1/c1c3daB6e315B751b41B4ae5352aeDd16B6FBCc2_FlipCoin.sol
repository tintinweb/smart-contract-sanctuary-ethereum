...
//TODO current chainlink addresses rinkeby
...
contract VRFv2Consumer is protected, VRFConsumerBaseV2 {
  ...
  uint256[] public s_randomWords_old = [0];
  uint256[] public s_randomWords = [0];
  ...
  function fulfillRandomWords(
    ...
    s_randomWords_old = s_randomWords;
...
contract FlipCoin is VRFv2Consumer, KeeperCompatibleInterface {
    ...
    constructor(address d) VRFv2Consumer(subscriptionId) {
        owner = msg.sender;
        is_auth[owner] = true;
    ...
    //Check if someone bet
    function checkUpkeep(bytes calldata checkData) external view override returns (bool upkeepNeeded, bytes memory performData) {
        require(s_randomWords_old[0] != s_randomWords[0], 'Random words not updated');

        //Check if there is one bet active, send his ID
        bool executePerform = false;
        uint pendingBetId = 0;

        for(uint i = last_id; i >= 0; i--){
            if(bet[i].active){
                pendingBetId = i;
                executePerform = true;
                break;
        ...
        return (executePerform, abi.encode(pendingBetId));
    }

    //If someone bet we get a random number and call win/lose
    function performUpkeep(bytes calldata performData) external override {
        //Get the bet ID and check if its active
        uint betId = abi.decode(performData, (uint));

        //If its active then we get the random number and end the bet with win/lose call
        require(bet[betId].active, 'Nope');

        //Check the result
        uint result = s_randomWords[0] % 2; //0 win, 1 loss

        if(result == 0){
            win(betId); //Remember to auth the CA
        }
        else{
            lose(betId);
        }

        s_randomWords_old[0] = s_randomWords[0];

        //This method is unnecessary, we could use fulfillRandomWords for this
