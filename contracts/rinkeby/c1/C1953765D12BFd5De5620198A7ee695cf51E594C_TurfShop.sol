...
contract TurfShop is ERC1155, ERC1155Supply, AccessControl {
  ...
  function checkBlockStatus(uint objectId) external view returns (bool, uint256, uint256, uint256) {
    if(block.number >= turfObjects[objectId].startingReference && block.number <= turfObjects[objectId].endReference){
      return (true, block.number, turfObjects[objectId].startingReference, turfObjects[objectId].endReference);
    } else {
      return (false, block.number, turfObjects[objectId].startingReference, turfObjects[objectId].endReference);
    }
  }
  ...
  //
  function mint(uint objectId, uint amount, bytes32[] memory merkleProof, bytes memory data) public payable checkReleaseStatus(objectId) {
