...
interface IChainRunnersRenderer {
    function tokenURI(uint256 tokenId, ChainRunnersTypes.ChainRunner memory runnerData)
        external
        view
...
contract ChainRunners is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 tokenId) internal {
        ...
        runner.dna = uint256(keccak256(abi.encodePacked(tokenId, msg.sender, block.difficulty, block.timestamp)));
