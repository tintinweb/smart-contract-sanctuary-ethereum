...
contract HappyKoalas is ERC721Enumerable, Ownable {
    ...
    bytes32 public              whitelistMerkleRoot = 0x392f5c74693fa0fba201b1afef3e504146440db7aee30137286ec19f6f45785e;
    uint256 public              MAX_SUPPLY = 6666;

    uint256 public constant     MAX_WL_MINT = 3;
    uint256 public constant     MAX_PER_PUBLIC_TX = 10;
    uint256 public constant     priceInWei  = 0.036 ether;
    ...
    {
        proxyRegistryAddress = _proxyRegistryAddress; // opensea wyvern (or rinkeby on testnet)
    }
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        if (projectState <= 1){ // 3D stage of the project
    ...
    } // remember to flip proxy state on opensea wyvern to allow for no approval fee


    // TODO*** : Change to Spart's Main Net preferred address. May need more koalas for helpers too
    ...
    function whitelistMint(uint256 count, bytes32[] calldata _merkleProof) public payable {
        require(projectState > 2, "Whitelist Sale is not active"); //this function can be called during public sale as well. this reduces failed tx's
        ...
        addressMintAmount[_msgSender()] += count; // count the total they have minted
    ...
    function publicMint(uint256 count) public payable {
        uint256 totalSupply = _owners.length;
        // doing this check first to save late-minters gas on failed tx's
