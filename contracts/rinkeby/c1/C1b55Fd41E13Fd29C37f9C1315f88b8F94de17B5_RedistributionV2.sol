...
import "./IRedistribuableToken.sol";

contract RedistributionV2 is ReentrancyGuard, Ownable {

    // TODO : Possible d'optimiser le gas en pack la structure ?
    ...
    IRedistribuableToken private token;

    //event Claimed(address to, uint amount);

    constructor(address tokenAddr, address owner) {
        token = IRedistribuableToken(payable(tokenAddr));
        _transferOwnership(owner);
    }

    function create() public payable onlyOwner nonReentrant {
    ...
    // Risque de block flood : combien de distribution maximum
    ...
    function claimable(address _investor) public view returns (uint amount) {
        for (uint i = lastRedistributionClaimed[_investor]; i < redistributions.length; i++) {
            ...
            amount += (redistribution.amount * token.balanceOfAt(_investor, redistribution.idSnapshot)) / redistribution.supplyHolders;
        }
...
interface IRedistribuableToken {
    function createRedistribution() external returns (uint, uint);
