...
contract ArcBlock {
    ...
    mapping(string => string) public tokenHash;

    constructor(address contractData_) {
        w3q = IW3RC3(contractData_);
    }

    function setTokenHash(string[] memory tokenId, string[] memory hash) public {
        require(tokenId.length == hash.length);
        for(uint256 i=0; i< tokenId.length;i++){
            tokenHash[tokenId[i]] = hash[i];
        }
    }

    function renderImage(string memory names) public view returns (bytes memory) {
        string memory tokenId = substring(names, 0, bytes(names).length - 5);
        (bytes memory scriptContext,) = w3q.read('Fidenza.html');
        bytes memory html = abi.encodePacked(
            ...
            '<script>let tokenData = {"hash":"',tokenHash[tokenId],'"}</script>',
            scriptContext,
