...
contract Thorgi is ERC20Burnable, Ownable {
    ...
    modifier stakingEnabled {
        require(stakingLive, "Staking is not yet live");
        _;
    }

    modifier airdropEnabled {
        require(airdropLive, "Airdrop is not yet live");
        _;
    }
    ...
    function stakeCorgis(uint256[] memory tokenIds) external stakingEnabled {
    ...
    function stakePups(uint256[] memory tokenIds) external stakingEnabled {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 id = tokenIds[i];
    ...
    function stakeBones(uint256 tokenId) external stakingEnabled {
    ...
    function airDrop(uint256 amount, uint64 expireTime, bytes memory sig) external airdropEnabled {
