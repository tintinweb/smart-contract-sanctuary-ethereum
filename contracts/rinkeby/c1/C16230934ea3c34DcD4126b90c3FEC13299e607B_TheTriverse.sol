...
contract TheTriverse is ERC721A, Ownable, ReentrancyGuard {
    
    using Strings for uint256;

    uint32 public MAXIMUM_SUPPLY = 10; //3897;   //3600 public & private  -- 297 free mint
    uint32 FREEMINT = 3; //297;
    uint32 PPMINT = 7; //3600;
    ...
    uint128 public PUBLIC_MINT_COST = 2 wei; 
    uint128 public ONE_MINT_COST = 4 wei;
    uint128 public TWO_MINT_COST = 6 wei;
    uint128 public THREE_MINT_COST = 8 wei;

    mapping (address => uint32) public WL_DB;

    uint8 paused = 2;
    uint8 publicMintpauser = 2;
    uint8 privateMintpauser = 2;
    uint8 freeMintpauser = 2;
    ...
    function freeMint(bytes32[] calldata _merkleProof,uint _amount) public {
        pauser();   
        FreeMintChecker();
        maxlimitchecker(_amount);
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        if(counterFeeMint + _amount > FREEMINT){
    ...
    function airdrop(address[] calldata addresses) public onlyOwner {
        require(addresses.length > 0, "Error : Need to airdrop at least 1 NFT");
        maxlimitchecker(addresses.length - 1);
    ...
    function enablePublicMint(uint8 _val) public onlyOwner {
        publicMintpauser = _val;    //1 -> true , 2 -> false
    }
    ...
    function setRoots(bytes32[] calldata _roots) public {
        if(_roots.length < 3) revert lengthMismatch();
    ...
    //on mainnet this function will remove
    function setSupply(uint32 a , uint32 b , uint32 c) public onlyOwner {
        MAXIMUM_SUPPLY = a; //3897;   //3600 public & private  -- 297 free mint
        FREEMINT = b; //297;
        PPMINT = c; //3600;
    }
