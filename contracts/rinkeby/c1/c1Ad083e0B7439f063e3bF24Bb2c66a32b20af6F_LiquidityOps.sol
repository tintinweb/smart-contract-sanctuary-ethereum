...
contract LiquidityOps is Ownable, Operators {
    ...
    IStaxLP public immutable xlpToken;
    ...
    function applyLiquidityAmounts(uint256 _liquidity) internal view returns (uint256 lockAmount, uint256 addLiquidityAmount) {
    ...
    function _lockInTranche(ITranche tranche, uint256 liquidity) internal returns (bytes32 kekId) {
        // Send the liquidity to the tranche
        lpToken.safeTransfer(address(tranche), liquidity);

        // If first time lock or the latest lock has expired - then create a new lock.
        // otherwise add to the existing active lock
        IFraxGauge.LockedStake[] memory lockedStakes = tranche.lockedStakes();
        ...
        if (lockedStakesLength == 0 || block.timestamp >= lockedStakes[lockedStakesLength - 1].ending_timestamp) {
            kekId = tranche.stakeLocked(liquidity, lockTime);
        } else {
            kekId = lockedStakes[lockedStakesLength - 1].kek_id;
            tranche.lockAdditional(kekId, liquidity);
        }
    }
    ...
    /** 
      ...
      *        to accept.
      */
    function _applyLiquidity(uint256 _liquidity, uint256 _minCurveTokenAmount, ITranche tranche) internal {
        uint256 balance = lpToken.balanceOf(address(this));
        if (balance < _liquidity) revert CommonEventsAndErrors.InsufficientTokens(address(lpToken), _liquidity, balance);
        ...
        if (lockAmount > 0) {
            bytes32 kekId = _lockInTranche(tranche, lockAmount);       
    ...
    function applyLiquidity(uint256 _liquidity, uint256 _minCurveTokenAmount) external onlyOwnerOrOperators {
    ...
    function applyLiquidityInTranche(uint256 _liquidity, uint256 _minCurveTokenAmount, address _trancheAddress) external onlyOwnerOrOperators {
    ...
    function withdrawAndRelockForTranche(address _trancheAddress, bytes32 _oldKekId) external onlyOwnerOrOperators {
        if (!tranches[_trancheAddress]) revert ITrancheRegistry.UnknownTranche(_trancheAddress);

        ITranche tranche = ITranche(_trancheAddress);
        uint256 withdrawnAmount = tranche.withdrawLocked(_oldKekId, address(this));

        if (withdrawnAmount > 0) {
            // As long as this tranche is still active, re-use it.
            // Otherwise use the most current tranche
            // (which might need creating/updating to a new tranche - based on volume/etc)
            if (tranche.disabled()) {
                if (_newTrancheRequired()) {
                    _createTranche(nextTrancheImplId, true);
                }
            ...
            // Re-lock
            bytes32 newKekId = _lockInTranche(tranche, withdrawnAmount);
            emit WithdrawAndReLock(_trancheAddress, _oldKekId, address(tranche), withdrawnAmount, newKekId);
        }
    }
    ...
    function harvestRewards() external {
        ...
        for (uint i=0; i<rewardTokens.length;) {
            ...
            uint256 feeAmount = (amount * feeRate.numerator) / feeRate.denominator;
    ...
    function withdrawLockedForTranche(address trancheAddress, bytes32 kek_id, address destination_address) external onlyOwnerOrOperators returns (uint256) {
        return ITranche(trancheAddress).withdrawLocked(kek_id, destination_address);
    }
