...
contract NFTContract is ERC721A, Ownable{
    ...
    uint256 public  MAX_PUBLIC_MINT = 1000;
    uint256 public  MAX_WHITELIST_MINT = 1000;
    uint256 public  MaxPerPublicWallet = 1000;
    uint256 public  MaxPerWhiteListWallet = 1000;
    uint256 public  PUBLIC_SALE_PRICE = .001 ether;
    uint256 public  WHITELIST_SALE_PRICE = .0001 ether;

    uint256 public MaxMintPhase1 = 200;
    uint256 public MaxMintPhase2 = 1000;
    uint256 public MaxMintPhase3 = 2000;
    uint256 public MaxMintPhase4 = MAX_SUPPLY;
    ...
    constructor() ERC721A("NFTContract", "NFT"){
    ...
    function mint(uint256 _quantity) external payable callerIsUser{
        if(whiteListSale && Phase1){
            require(whiteListSale, "NFTContract :: Minting is on Pause");
            require((totalSupply() + _quantity) <= MaxMintPhase1, "NFTContract :: Cannot mint beyond phase 1 supply");
            ...
            require(_quantity + balanceOf(msg.sender) <= MaxPerWhiteListWallet , "NFTContract :: You can not mint more than the maximum allowed per user.");
            require(whiteListedAddress[msg.sender], "NFTContract :: Sorry you are not white listed!");

            totalWhitelistMint[msg.sender] += _quantity;
            _safeMint(msg.sender, _quantity);
        }
        else if (publicSale && Phase2){
            require(publicSale, "NFTContract :: Not Yet Active.");
            require((totalSupply() + _quantity) <= MaxMintPhase2, "NFTContract :: Cannot mint beyond phase 2 supply");
            require((totalPublicMint[msg.sender] +_quantity) <= MAX_PUBLIC_MINT, "NFTContract :: Already minted 3 times!");
            require(msg.value >= (PUBLIC_SALE_PRICE * _quantity), "NFTContract :: Payment is below the price ");
            require(_quantity + balanceOf(msg.sender) <= MaxPerPublicWallet , "NFTContract :: You can not mint more than the maximum allowed per user.");


            totalPublicMint[msg.sender] += _quantity;
            _safeMint(msg.sender, _quantity);
        }
        else if (publicSale && Phase3){
            require(publicSale, "NFTContract :: Not Yet Active.");
            require((totalSupply() + _quantity) <= MaxMintPhase3, "NFTContract :: Beyond phase 3 Supply");
            require((totalPublicMint[msg.sender] +_quantity) <= MAX_PUBLIC_MINT, "NFTContract :: Already minted 3 times!");
            require(msg.value >= (PUBLIC_SALE_PRICE * _quantity), "NFTContract :: Payment is below the price ");
            require(_quantity + balanceOf(msg.sender) <= MaxPerPublicWallet , "NFTContract :: You can not mint more than the maximum allowed per user.");
        ...
        else if (publicSale && Phase4){
            require(publicSale, "NFTContract :: Not Yet Active.");
            require((totalSupply() + _quantity) <= MAX_SUPPLY, "NFTContract :: Beyond total Supply");
            require((totalPublicMint[msg.sender] +_quantity) <= MAX_PUBLIC_MINT, "NFTContract :: Already minted 3 times!");
            require(msg.value >= (PUBLIC_SALE_PRICE * _quantity), "NFTContract :: Payment is below the price ");
            require(_quantity + balanceOf(msg.sender) <= MaxPerPublicWallet , "NFTContract :: You can not mint more than the maximum allowed per user.");
            ...
            require(!pause, "NFTContract :: Minting is paused");
        }
    }
    ...
    function EnablePhase4() external onlyOwner{
        ...
        publicSale = true;
        pause = false;
        PUBLIC_SALE_PRICE = .03 ether;
    }

    //3% to utility/investors wallet
    function withdraw() external onlyOwner{
        uint256 withdrawAmount = address(this).balance * 5/100;
        payable(0x91CdA6045FC2946B528973f8C20b1f3C9297E57D).transfer(withdrawAmount);
