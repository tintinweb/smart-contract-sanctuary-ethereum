...
contract StrainFactory is StrainContract, StrainAdmin {
    ...
    /// @dev rarity score
    uint[4] public rarityScores = [0, 20, 50, 100];
        ...
        returns (uint256) 
    {
        ...
        uint _genes = 1000000000000000000000000000000 +
            randMod()*10000000000000000000000000000 +
            randMod()*100000000000000000000000000 +
            randMod()*1000000000000000000000000 +
            randMod()*10000000000000000000000 +
            randMod()*100000000000000000000 +
            randMod()*1000000000000000000 +
            randMod()*10000000000000000 +
            randMod()*100000000000000 +
            randMod()*1000000000000 +
            randMod()*10000000000 +
            randMod()*100000000 +
            randMod()*1000000 +
            randMod()*10000 +
        ...
        uint rarityScore = calculateRarityScore(_genes, stage);
        ...
        emit StrainCreated(msg.sender, newStrainId, block.timestamp);
    ...
    function genesToNumber(uint _dna) private pure returns (uint) {
        uint result = 0;
        uint96[15] memory counter = [
            1,
            100,
            10000,
            1000000,
    ...
    function calculateRarityScore(uint _dna, uint _stage)
        private
        view
        returns (uint)
    {
        uint number = genesToNumber(_dna);
        uint raritySum = rarityScores[_stage];
        uint finalScore = number + raritySum;
    ...
    function updateRarity(uint _strainId, uint _times)
        public
        validStrainId(_strainId)
        onlyStakingContract(msg.sender)
    {
        Strain storage strain = strains[_strainId];
        require(strain.stage < 3,"This is already a bud");
        strain.rarityScore += _times;
    }
        ...
        onlyStakingContract(msg.sender)
    {
        ...
        strain.stage += 1;
        strain.rarityScore = calculateRarityScore(
            strain.dna,
            strain.stage
        );

        emit StrainUpdated(msg.sender, _strainId, block.timestamp);
    }
}
...
contract StrainContract is AccessControl, IERC721 {
    ...
    function dnaToArray(uint _dna) internal pure returns (uint[15] memory) {
        ...
        )
    {
        ...
        uint[15] memory filteredData;
        Strain storage strain = strains[_strainId];
        uint[15] memory data = dnaToArray(strain.dna);
