...
contract GhostNFTv2 is ERC721A, Ownable {
    ...
    string public hiddenMetadataUri = "ipfs://QmY28aPHSF4NvnvWU1NwhF1CR1uYn4NsAVJ8JRSKjaM3tX";

    uint256 public allowlistCost = 0.0 ether;
    ...
    constructor() ERC721A("Los Ghostos v2", "LOSGHOSTOS") {}
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(
            ...
            "Max Amount Per Tx exceeded"
        );

        uint256 tokenCount = _numberMinted(_msgSender());
               
        require(
            tokenCount + _mintAmount <= maxMintAmountPerWallet,
    ...
    function claim(uint256 nTokens, address to)
        ...
        onlyOwner
    {
        require(nTokens <= (reserved - claimed), "That would exceed the max reserved.");

        require(totalSupply() + nTokens <= maxSupply, "Max supply exceeded");

        //reserved = reserved - nTokens;
        claimed = claimed + nTokens;

        _safeMint(to, nTokens);
    } 


    function mint(uint256 nTokens) public payable mintCompliance(nTokens) {
        require(state == ContractMintState.PUBLIC, "Public mint is disabled");
        require(msg.value >= publicCost * nTokens, "Insufficient funds");

        require(totalSupply() + (reserved - claimed) + nTokens <= maxSupply, "Max supply exceeded");
    ...
    function mintAllowList(
    ...
    ) public payable mintCompliance(nTokens) {
        require(
        ...
        require(
            msg.value >= allowlistCost * nTokens, 
            "Insufficient funds"
        );

        require(
            numberMinted(msg.sender) + nTokens <= allowance,
    ...
    function mintForAddress(uint256 nTokens, address _receiver)
        public
        onlyOwner
    {        
        require(totalSupply() + (reserved - claimed) + nTokens <= maxSupply, "Max supply exceeded");
    ...
    function withdraw(address destination) public onlyOwner {
        ...
        (success, ) = payable(destination).call{
            value: (100 * contractBalance) / 100
        }("");
