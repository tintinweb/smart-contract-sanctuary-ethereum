...
contract Casino is Ownable {
    ...
    modifier onlyCasinoGame() {
        bool isAddr = false;
        for (uint256 i = 0; i < casinoGameAddresses.length; i++) {
    ...
    function alreadyClaimedTokens(address _address)
        external
        view
        returns (bool)
    {
    ...
    function claimInitialTokens() external {
        ...
        require(
            freeTokensClaimed[msg.sender] == false,
    ...
    // the casino contract.
    function transferFrom(address _from, uint256 _amount)
        external
        onlyCasinoGame
    {
