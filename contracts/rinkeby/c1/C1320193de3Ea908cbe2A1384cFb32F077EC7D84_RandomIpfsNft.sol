...
contract RandomIpfsNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    ...
    // Events
    event NftRequested(uint256 indexed requestId, address indexed owner);
    event NftMinted(uint256 indexed tokenId, address indexed owner, Breed indexed breed);
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        /*
         * We will get value between 0-99
         * 0-10  - first one
         * 10-40  - second one
         * 40 - 100 - third one
         */
        ...
        s_tokenCounter++;
        emit NftMinted(newTokenId, dogOwner, dogBreed);
    }

    function requestNft() public payable returns (uint256 requestId) {
        if (msg.value < s_mintFee) {
            revert SmallMintFeeValue();
        }
        /**
         * bytes32 keyHash,
            uint64 subId,
    ...
    function _initializeContract(string[3] memory _tokenUrls) private {
        if (s_initialized) {
            revert AlreadyInitialized();
        }
        s_tokenUris = _tokenUrls;
        s_initialized = true;
    }
    ...
    function getBreedFromModdedRng(uint256 moddedRng) public pure returns (Breed) {
        ...
        uint8[3] memory changeArray = getChanceArray();

        for (uint8 i = 0; i < changeArray.length; i++) {
    ...
    function getDogTokenUriByIndex(uint256 index) external view returns (string memory) {
