...
contract WhiteList is IWhiteList, NFTSpaceXAccessControls {
  using SafeMath for uint256;

  mapping(address => uint256) override public points;
  uint256 public totalPoints;

  event PointsUpdated(address indexed account, uint256 oldPoints, uint256 newPoints);

  function initWhiteList(address _admin) public override {
  ...
  function setPoints(address[] memory _accounts, uint256[] memory _amounts) external override {
    require(hasAdminRole(_msgSender()) || hasOperatorRole(_msgSender()), "WL: not allowed");
    require(_accounts.length != 0, "WL: empty array");
    require(_accounts.length == _amounts.length, "WL: length not equal");
    uint256 totalPointsCache = totalPoints;
