...
contract PolicyClaimsFacet is EternalStorage, Controller, IDiamondFacet, IPolicyClaimsFacet, PolicyFacetBase, ReentrancyGuard {
    ...
    function getClaimStats() public view override returns (uint256 numClaims_, uint256 numPendingClaims_) {
        numClaims_ = dataUint256["claimsCount"];
        numPendingClaims_ = dataUint256["claimsPendingCount"];
    }

    function getClaimInfo(uint256 _claimIndex)
        public
        view
        override
        returns (
    ...
    function makeClaim(uint256 _trancheIndex, uint256 _amount) external override assertActiveState assertBelongsToEntityWithRole(msg.sender, ROLE_INSURED_PARTY) {
        ...
        // check amount
        require(_getTranchePendingClaimsAmount(_trancheIndex) + _amount <= dataUint256[string(abi.encodePacked(_trancheIndex, "balance"))], "claim too high");
    ...
    function disputeClaim(uint256 _claimIndex) external override assertBelongsToEntityWithRole(msg.sender, ROLE_UNDERWRITER) {
    ...
    function acknowledgeClaim(uint256 _claimIndex) external override assertBelongsToEntityWithRole(msg.sender, ROLE_UNDERWRITER) {
        require(0 < dataUint256[__i(_claimIndex, "claimAmount")], "invalid claim");
        uint256 state = dataUint256[__i(_claimIndex, "claimState")];
        require(state == CLAIM_STATE_CREATED, "in wrong state");
    ...
    function approveClaim(uint256 _claimIndex) external override assertBelongsToEntityWithRole(msg.sender, ROLE_CLAIMS_ADMIN) {
        // check claim
        require(0 < dataUint256[__i(_claimIndex, "claimAmount")], "invalid claim");
        uint256 state = dataUint256[__i(_claimIndex, "claimState")];
        require(state == CLAIM_STATE_CREATED, "in wrong state");
    ...
    function declineClaim(uint256 _claimIndex) external override assertBelongsToEntityWithRole(msg.sender, ROLE_CLAIMS_ADMIN) {
        // check claim
        require(0 < dataUint256[__i(_claimIndex, "claimAmount")], "invalid claim");
        uint256 state = dataUint256[__i(_claimIndex, "claimState")];
        require(state == CLAIM_STATE_CREATED, "in wrong state");
    ...
    function payClaim(uint256 _claimIndex) external override nonReentrant assertBelongsToEntityWithRole(msg.sender, ROLE_CLAIMS_ADMIN) {
        // check claim
        require(0 < dataUint256[__i(_claimIndex, "claimAmount")], "invalid claim");
        uint256 state = dataUint256[__i(_claimIndex, "claimState")];
        ...
        require(_tranchePaymentsAllMade(dataUint256[__i(_claimIndex, "claimTranche")]), "not possible until premiums are fully paid");

        // transfer
        _getTreasury().payClaim(dataAddress[__i(_claimIndex, "claimEntity")], dataUint256[__i(_claimIndex, "claimAmount")]);
...
abstract contract IPolicyClaimsFacet {
     ...
     */
    uint256 public constant CLAIM_STATE_CREATED = 0;
    /**
     * @dev State: The claim has been approved.
     */
    uint256 public constant CLAIM_STATE_APPROVED = 2;
    /**
     * @dev State: The claim has been declined.
     */
    uint256 public constant CLAIM_STATE_DECLINED = 3;
    /**
     * @dev State: The claim has been paid out.
     */
    uint256 public constant CLAIM_STATE_PAID = 5;
     ...
     */
    function makeClaim(uint256 _trancheIndex, uint256 _amount) external virtual;
     ...
     */
    function approveClaim(uint256 _claimIndex) external virtual;
     ...
     * @param _claimIndex Claim index.
     */
    function disputeClaim(uint256 _claimIndex) external virtual;
     ...
     * @param _claimIndex Claim index.
     */
    function acknowledgeClaim(uint256 _claimIndex) external virtual;
     ...
     * @param _claimIndex Claim index.
     */
    function declineClaim(uint256 _claimIndex) external virtual;
     ...
     * @param _claimIndex Claim index.
     */
    function payClaim(uint256 _claimIndex) external virtual;
     ...
     */
    function getClaimStats() public view virtual returns (uint256 numClaims_, uint256 numPendingClaims_);
