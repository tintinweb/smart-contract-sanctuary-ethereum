...
contract Vendor is Ownable {
  ...
  function buyTokens() public payable {
	// Validate the user can send eth
  ...
  function sellTokens(uint256 _amount) public {
		// Validate the user has enough tokens
		address user = msg.sender;
		uint256 userBalance = yourToken.balanceOf(user);
		require(userBalance >= _amount, "You don't have enough tokens");
		
		// Validate the vendor has ETH to send
		uint256 vendorBalance = address(this).balance;
		uint256 amountOfETH = _amount / tokensPerEth;
		require(vendorBalance >= amountOfETH, "Vendor does not have any ETH for the swap");
  
		// Send Tokens back to the contract(buyback)
		...
		emit SellTokens(user, _amount, amountOfETH);
  }
