...
contract RiggedRoll is Ownable {
    ...
    constructor(address payable diceGameAddress) {
        ...
        adress1=diceGameAddress;

    }

   function riggedRoll() external payable {
        require(address(this).balance >= .002 ether,"you dont have enough balnce");
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(abi.encodePacked(prevHash, adress1, diceGame.nonce));
        uint256 roll = uint256(hash) % 16;
        console.log("THE ROLL IS ",roll);
        assert(roll<=2);
   ...
   function withdraw(address _addr, uint256 _amount)public {
    require(_amount > 0, "You have no balance to withdraw!");
