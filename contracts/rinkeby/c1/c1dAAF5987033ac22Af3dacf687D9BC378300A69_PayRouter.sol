...
contract PayRouter{
    ...
    string public constant name = "PayRouter";
    address internal constant safeSwapAddress = 0x751A3F987DD8b08FB85dfFF163ab9C6982CE1790;
    SafeSwap public safeSwap = SafeSwap(0x751A3F987DD8b08FB85dfFF163ab9C6982CE1790);
    ...
    function Payment(uint256 _amount, address _to, address _tokenIn, address _tokenOut)external payable{
       require(_amount > 0, "amount == 0");
       if(_tokenIn == address(0)){
            // Call SwapETHForToken
            safeSwap.SwapETHForToken{value: msg.value}(_tokenOut, _to);
       }else if(_tokenOut == address(0)){
            // Call SwapTokenForETH
            TransferHelper.safeTransferFrom(_tokenIn, msg.sender, address(this), _amount);
            TransferHelper.safeApprove(_tokenIn, safeSwapAddress, _amount);
            safeSwap.SwapTokenForETH(_amount,_tokenIn,_to);
       }else{
            // Call SwapTokenForToken
            TransferHelper.safeTransferFrom(_tokenIn, msg.sender, address(this), _amount);
            TransferHelper.safeApprove(_tokenIn, safeSwapAddress, _amount);
            safeSwap.SwapTokenForToken(_tokenIn,_tokenOut,_amount,_to);
       }
    }
