...
// """
// Idea: on-chain clock
// Think classy looking clock that you can display on your iWatch
// Runs continuously
// Function call sets the start time. Input time and clock runs
// Default time is GM time
// Switch from analog to digital?
// I can make the watch parts with svg, but I'll need to do the movement with js I think. I found a couple examples
// marble background would look cool. Art Blocks saves js scripts for each project on the chain. If I get farther into this
// I can save the js for the clock start time to the blockchain, but that's not necessary. So for now, just focus on the svg

// Variables to randomize: turbulence, background color, text color
// no floats in solidity. I can add . to svg code and just put a number 0-9 behind it
...
contract Watch is ERC721, Ownable {
  ...
  string constant private _S1_TAG = '<TURBSEED1>';
  string constant private _MM_TAG = '<MARBLEMATRIX>';
  string constant private _BF1_TAG = '<BASEFREQ1>';
  string constant private _S2_TAG = '<TURBSEED2>';
  string constant private _CR_TAG = '<REDCOLOR>';
  string constant private _CG_TAG = '<GREENCOLOR>';
  string constant private _CB_TAG = '<BLUECOLOR>';
  string constant private _MC_TAG = '<MARBLECOLOR>';

  string[] private _imageParts;

  constructor() ERC721("WGMI Watch", "WGW") {
    _imageParts.push('data:image/svg+xml;utf8,');
    _imageParts.push('<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" id="watch" width="1000" height="1000" viewBox="-0.5 -0.5 1 1">');
    ...
    _imageParts.push('<filter id="marble2">');
        ...
        _imageParts.push(_CR_TAG);
        _imageParts.push(' 0 1 0 0 .');
        _imageParts.push(_CG_TAG);
        _imageParts.push(' 0 0 1 0 .');
        _imageParts.push(_CB_TAG);
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
      ...
      // make all the calculations here
      bytes32 predictableRandom = keccak256(abi.encodePacked( blockhash(block.number-1), msg.sender, address(this), tokenId ));
      uint256 s1 = uint8(predictableRandom[0]);
      uint256 s2 = uint8(predictableRandom[1]);
      uint256 bf1 = (uint8(predictableRandom[3]) % 64 + uint8(predictableRandom[4]) % 64 + uint8(predictableRandom[5]) % 64 + uint8(predictableRandom[6]) % 64) / 8 + 4;
      ...
      string memory _marbleMatrix = "0.83 0.83 0.83 0 0 0.69 0.69 0.69 0 0 0.22 0.22 0.22 0 0 0 1 0 1 0";
      string memory _marbleColor = "#d4af37";
      if (uint8(predictableRandom[13]) <= 11) {
        _marbleMatrix = "0.83 0.83 0.83 0 0 0.69 0.69 0.69 0 0 0.22 0.22 0.22 0 0 0 1 0 1 0";
        _marbleColor = "#d4af37";
      } else {
        _marbleMatrix = "0.86 0.86 0.86 0 0 0.89 0.89 0.89 0 0 0.92 0.92 0.92 0 0 0 1 0 1 0";
        _marbleColor = "#dbe4eb";
      }

      return string(abi.encodePacked('data:application/json;utf8,{"name":"WGMI Watch", "description":"SVG Watch", "image":"',
          _generateImage(s1, _marbleMatrix, bf1, s2, cr, cg, cb, _marbleColor),
          '"}'));

  }

  // Visibility is `public` to enable it being called by other contracts for composition.
  function _generateImage(uint256 s1, string memory _marbleMatrix, uint256 bf1, uint256 s2, uint256 cr, uint256 cg, uint256 cb, string memory _marbleColor) private view returns (string memory) {
      bytes memory byteString;
      for (uint i = 0; i < _imageParts.length; i++) {
          if (_checkTag(_imageParts[i], _S1_TAG)) {
              byteString = abi.encodePacked(byteString, uint2str(s1));
          } else if (_checkTag(_imageParts[i], _MM_TAG)) {
              byteString = abi.encodePacked(byteString, _marbleMatrix);
          } else if (_checkTag(_imageParts[i], _BF1_TAG)) {
              byteString = abi.encodePacked(byteString, uint2str(bf1));
          } else if (_checkTag(_imageParts[i], _S2_TAG)) {
              byteString = abi.encodePacked(byteString, uint2str(s2));
          } else if (_checkTag(_imageParts[i], _CR_TAG)) {
              byteString = abi.encodePacked(byteString, uint2str(cr));
          } else if (_checkTag(_imageParts[i], _CG_TAG)) {
              byteString = abi.encodePacked(byteString, uint2str(cg));
          } else if (_checkTag(_imageParts[i], _CB_TAG)) {
              byteString = abi.encodePacked(byteString, uint2str(cb));
          } else if (_checkTag(_imageParts[i], _MC_TAG)) {
              byteString = abi.encodePacked(byteString, _marbleColor);
          } else {
