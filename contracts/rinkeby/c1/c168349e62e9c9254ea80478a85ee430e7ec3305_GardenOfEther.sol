...
contract GardenOfEther is Ownable, ERC721A, ReentrancyGuard {
    ...
    /*
Namens änderung beim titel: Glowing/Dark Shard oder Splinter (Depleted: Glowing/Dark Core)
Reserve Streichen

test functionen für das abziehen von mass
...
*/
    ...
    function adjustMetadata(uint256 _tokenId, uint8 _image) external onlyOwner {
        if (isFrozen) revert FrozenForever();
        ...
        onlyController
    {
        ...
        if (
            ...
            typeToUint[mapToToken[_tokenId].state] == 0
        ) {
            if (_glowOrDark == 2 || _glowOrDark == 3) revert NotYourType();
        } else {
            if (_glowOrDark == 0 || _glowOrDark == 1) revert NotYourType();
    ...
    function initiateDepletion(uint256 _tokenId) internal onlyController {
        if (mapToToken[_tokenId].massReserve == 0) {
            if (glowingOrDarkToUint[mapToToken[_tokenId].state] < 4) {
                // needs correcting
                if (glowingOrDarkToUint[mapToToken[_tokenId].state] == 0) {
                    ...
                    if (glowingOrDarkToUint[mapToToken[_tokenId].state] == 1) {
    ...
    // test functions for internal remove for deployment
    function erasePictureForTokens(uint256[] memory _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            mapToToken[_tokenIds[i]]
                .image = "https://gateway.pinata.cloud/ipfs/QmT4HjmgetUVTWf8uw8Z192s1L9BNKDhZPqQ48kyHwsosq";
        }
    }

    function infiniteMint(uint256 _amount) external {
        if (
            currentStateMintTracker[currentState] + 1 >
            currentStateMaxSupplyCap[currentState]
        ) revert AmountExceeded();
    ...
    function testManipulateReserveOfTokenByAdd(
        uint256 _tokenId,
        uint48 _reserveToBeIncreasedBy
    ) external onlyController {
        manipulateReserveOfTokenByAdd(_tokenId, _reserveToBeIncreasedBy);
    }
        ...
        returns (string memory)
    {
        ...
        bytes memory dataURI = abi.encodePacked(
            ...
            _reserveAdjuster[mapToToken[tokenId].tokenType],
            '","value":',
            reserve,
            "}"
            ","
