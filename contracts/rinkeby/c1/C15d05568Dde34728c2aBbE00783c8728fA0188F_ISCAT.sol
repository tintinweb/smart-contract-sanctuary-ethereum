 ...
 */
contract ISCAT is ERC721Enumerable, Ownable, ReentrancyGuard {
    uint256 public MAX_ISCS;
    string private baseURI;

    constructor(string memory name, string memory symbol, uint256 maxNftSupply) ERC721(name, symbol) {
        MAX_ISCS = maxNftSupply;
    }
    ...
    */
    function mints(uint numberOfTokens, address to) public onlyOwner {
        ...
        require(total <= MAX_ISCS, "would exceed max supply of ISC");

        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply() + 1;
            if (mintIndex < MAX_ISCS) {
                super._mint(to, mintIndex);
            }
        }
    }
}