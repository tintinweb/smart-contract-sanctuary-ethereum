...
contract MyToken is ERC721A, ReentrancyGuard, AccessProtected {
    using Address for address;
    uint256 public fee = 10**17;
    uint256 startSale;
    uint256 lmtPerWallet = 3;
    ...
    function addAddressesToWhitelist(address[] memory _userAddresses)
        public
        onlyOwner
    {
    ...
    function mint(uint256 quantity) public payable {
        ...
        require(nftCount[msg.sender] <= lmtPerWallet, "Reached max Limit");
        ...
        if (
            startSale < block.timestamp && startSale + 86400 > block.timestamp
        ) {
