...
contract Merkle_TokenID  is Governance { //} is ERC721 {
  ...
  constructor() {
    ...
    function checkProof(uint256 tokenID, bytes32[] calldata _proof) 
      public
      view
      returns (bool)
    {
    ...
    function setMerkleProof(bytes32 _merkleRoot) public onlyGovernance {
        merkleRoot = _merkleRoot;
    }
    
    function mintAllowList(uint256 _tokenId, bytes32[] calldata _proof) external {
      require(checkProof(_tokenId, _proof), "You are not on the allowlist");
