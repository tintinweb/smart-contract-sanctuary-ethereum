...
contract DigiBitties is Ownable, ERC721B {
    using Strings for uint256;
    using SafeERC20 for IERC20;
    using ECDSA for bytes32;

    string private baseURI; // Private to reduce gas.
    mapping(address => uint64) private teamCredits; // Private to reduce gas.
    ...
    modifier belowMax(uint256 _quantity) {
        if (_currentIndex + _quantity > MAX_SUPPLY) revert QuantityExceedsMax();
        _;
    }
    ...
    constructor(
        ...
        uint128 _price
    ) ERC721B("DigiBitties - 1337 Collection", "DB1") {
     ...
     */
    function teamMint(uint64 _quantity) public belowMax(_quantity) {
     ...
     */
    function claim(bytes calldata _signature) public belowMax(CLAIM_REWARD) {
     ...
     */
    function mint(uint256 _quantity) public payable belowMax(_quantity) {
        if (msg.value != _quantity * price) revert IncorrectETH();
        _safeMint(_msgSender(), _quantity);
    }

    /**
     * @return The baseURI for assemblign the URI to the metadata.
     */
...
library ECDSA {
     ...
     * verification to be secure: it is possible to craft signatures that
     * recover to arbitrary addresses for non-hashed data. A safe way to ensure
     * this is by receiving a hash of the original message (bytes32(uint256(uint160(_msgSender())))which may otherwise
     * be too long), and then calling {toEthSignedMessageHash} on it.
     */
 ...
 * the Metadata and _not_ Enumerable extension. Built to optimize for lower gas during batch mints.
 *
 * ERC721B further builds on ERC721A by removing burning and Enumerable to save even more gas.
 * These changes also make it so an owner can have as well as the max token ID is 2^256 - 1 (max value of uint256).
 *
...
abstract contract ERC721B is Context, ERC165, IERC721, IERC721Metadata {
    ...
    /**
     * @dev This is not an override of {IERC721Enumerable-totalSupply}.
     */
    ...
    function ownerOf(uint256 tokenId) public view override returns (address) {
        ...
        unchecked {
            if (curr < _currentIndex) {
                address ownership = _owners[curr];
        ...
        unchecked {
            ...
            if (_owners[nextTokenId] == address(0)) {
                if (nextTokenId < _currentIndex) {
                    _owners[nextTokenId] = prevOwnership;
                }
            }
        }
