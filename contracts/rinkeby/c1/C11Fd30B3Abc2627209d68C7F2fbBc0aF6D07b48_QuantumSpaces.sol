    ...
    QuantumBlackListable
{
    ...
    function __QuantumSpaces_init_unchained(
        ...
        QuantumBlackListable.__setBlackListContractAddress(blackListAddress);
    }
    ...
    function setBlackListAddress(address blackListAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        QuantumBlackListable.__setBlackListContractAddress(blackListAddress);
...
// contracts that want to implement blackListing on specific methods (e.g. minting) can extended this contract, and use the modifier
contract QuantumBlackListable {
    address public blackListContractAddress;

    // owner checks to be performed in contracts which extend this one
    function __setBlackListContractAddress(address blAddress) internal {
        require(blAddress != address(0), "Invalid address");
        blackListContractAddress = blAddress;
    }

    modifier isNotBlackListed(address user) {
        QuantumBlackList qbl = QuantumBlackList(blackListContractAddress);

        if (blackListContractAddress == address(0)) {
            revert BlackListedAddressNotSet();
        }

        if (qbl.isBlackListed(user)) {
            revert BlackListedAddress(user);
        }
