...
contract MonsterLand is MultiStage {
  function initialize(
    ...
    setPrice(3, 0);
    updateMerkleTreeRoot(2, freelistMerkleTreeRoot_);
    // Waitlist
    updateBalanceLimit(4, 1);
    setPrice(4, 0.00999 ether);
    updateMerkleTreeRoot(2, waitlistMerkleTreeRoot_);
    // Public
    ...
    CustomPaymentSplitterUpgradeable
{
    ...
    {
        require(currentStage == stage_, "Current stage is not enabled");
    ...
    function publicMint(uint256 amount_)
        ...
        payable
    {
        require(currentStage == 1, "Current stage is not enabled");
        _increaseBalance(1, msg.sender, amount_);
        _callMint(msg.sender, amount_);
        _handlePayment(amount_ * price(1));
    }

    function setStage(uint8 currentStage_) external onlyAdmin {
