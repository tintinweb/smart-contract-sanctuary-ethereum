...
contract KenFT is ERC721, Owned {
    ...
    constructor(
        ...
        address _owner
    ) payable ERC721(_name, _symbol) Owned(_owner) {
        baseURI = _baseURI;
    }
    ...
    function burn(uint256 id) public {
        if (msg.sender != ownerOf(id)) revert NotTokenOwner();
        _burn(id);
        totalSupply--;
    }
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        ...
        return
            bytes(baseURI).length > 0
                ? string.concat(baseURI, string.concat(id.toString(), ".json"))
                : "";
    }
}
