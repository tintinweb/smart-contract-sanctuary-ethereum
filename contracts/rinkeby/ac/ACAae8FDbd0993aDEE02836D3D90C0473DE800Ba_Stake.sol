...
contract Stake {
    ...
    uint256 public constant CEO_FEE = 35;
    uint256 public constant DEV_FEE = 35;
    uint256 public constant MARK_FEE = 35;
    ...
    function payoutOf(address _addr) external view returns (uint256 value) {
        ...
        for (uint256 i = 0; i < player.deposits.length; i++) {
            ...
            if (from < to) {
                value +=
                    (dep.amount * (to.sub(from)) * tarif.percent) /
                    tarif.life_days /
                    8640000;
            }
        }
