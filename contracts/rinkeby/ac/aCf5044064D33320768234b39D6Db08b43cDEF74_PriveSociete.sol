...
contract PriveSociete is ERC721A, Ownable, ERC2981 {
    ...
    uint256 private constant INITIAL_SUPPLY = 499;
    ...
    function whitelist(address[] calldata _users) external onlyOwner {
        ...
        totalWhitelisted += _users.length;
        emit Whitelisted(_users);
    }

    function blacklist(address _user) external onlyOwner {
        if (
            userMints[_user] == MintStatus.NOT_WHITELISTED ||
            userMints[_user] == MintStatus.MINTED
    ...
    function mint() external {
        if (userMints[msg.sender] == MintStatus.NOT_WHITELISTED)
            revert NotWhitelisted();
        if (userMints[msg.sender] == MintStatus.MINTED) revert AlreadyMinted();

        userMints[msg.sender] = MintStatus.MINTED;
    ...
    function mintRemaining() external onlyOwner {
        _safeMint(msg.sender, MAX_SUPPLY - INITIAL_SUPPLY - totalWhitelisted);
    }
