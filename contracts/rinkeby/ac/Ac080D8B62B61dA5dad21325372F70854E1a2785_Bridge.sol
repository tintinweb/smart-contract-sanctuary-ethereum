...
contract Bridge {
    ...
    event FeeCollected(address indexed feeRecevier, string ticker, uint256 amount);
    ...
    function redeem(
        ...
        bytes32 hash_ = keccak256(abi.encodePacked(from, to, amount, feeAmount, ticker, chainFrom, chainTo, nonce));
        ...
        IToken(token).mint(to, amountOut);
        IToken(token).mint(feeRecevier, amount);

        emit Redeemed(from, to, amountOut, ticker, chainTo, chainFrom, nonce);
        emit FeeCollected(feeRecevier, ticker, feeAmount);
    }
