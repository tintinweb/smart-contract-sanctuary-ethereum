...
contract Geneticats is ERC721Enumerable, controllerPanel {
    ...
    function whiteListBuySignature(
        ...
        if (_ec_token_id == 0) {
            ...
            require(availableSales() >= _tokenCount, "sold out");
            require(
                msg.value >= _tokenCount * (whiteListPrice),
                "Price not met"
            );
    ...
    // REMOVE ////////////////////////////////////////////////////////////////////////////////////////
    function resetTime(uint8 _testMode) public {
        if (_testMode == 0) {
            discount_start = block.timestamp + 10 minutes;
            discount_end = discount_start + 30 minutes;
            sale_start = discount_end + 30 minutes;
            sale_end = sale_start + 30 minutes;
        } else if (_testMode == 1) {
            discount_start = block.timestamp;
            discount_end = discount_start + 30 minutes;
            sale_start = discount_end + 30 minutes;
            sale_end = sale_start + 30 minutes;
        } else if (_testMode == 2) {
            ...
            // Test reveal
            discount_end = block.timestamp;
            sale_end = block.timestamp;
            reveal_start = block.timestamp;
            offset = 0;
