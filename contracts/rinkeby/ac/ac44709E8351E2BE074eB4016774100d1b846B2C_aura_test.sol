...
contract aura_test is ERC1155Supply, Ownable  {
    bool public pausedSale = false;
    bool public isAllowListActive = false;
    uint private currentID = 1;
    uint constant MAX_AURA_PER_PURCHASE = 5; //verify for level
    uint[] public MAX_AURA = new uint256[](10);
    ...
    event Minted(uint256 _totalSupply); //check totalSupply
    string public name;
...
constructor(string memory uri, string memory _name, string memory _symbol, uint256 _StartSale, address _wDev) ERC1155(uri) {
    ...
    for (uint i = 1; i < 10; i++) {
        ...
        user._auto.push(true);
        user.status.push(false);
        _level[i].countpay = 0; //test
        _level[i].countid = 0;  //test not storage
    }
...
//Price
function getPrice(uint num) public view returns (uint256) {
        return price[num];
    }

//Set price
function setPrice(uint256[] calldata newprice) public onlyOwner {
        price = newprice;
    }
...
//Mint
    ...
    if (currentID == 1){
        if (balanceOf(refadd, currentID) > 0) {ref = true;}  //1155
        if (balanceOf(msg.sender, currentID) > 0) {newal = true;} //1155
    ...
    user.countref += 1;
    //uint256 lenref = user.myref.length;
    if (user.countref == 2) {
...
user = _users[_toLine];
user.status[_num - 1] = true; // test  - not to iD
if ((level.countid > 0)&&(level.countid % 3==0)) {
    ...
    if (user._auto[_num - 1]){
        ...
        user.status[_num - 1] = false; // test  - not to iD
    }
    }
level.countpay += 1;
}
...
function autoR (uint _num, bool select) external payable { //0..9
    ...
    uint _i;
    if (user.status[_num]){
        user._auto[_num] = !user._auto[_num]; //on-off auto
    }
    else {
        //chainVerify, = return i if i == _num => ok
        require(user.qalification, "Error! You are not qualified!");

        //uint chain = chainVerify(_num);
        ...
        require((_num + 1) == _i, "Error! There are no NFTs of previous levels in your chain!");
        if (select) {
         // payble localBalance   
        require(price[_num] <= user.balance, "There are not enough funds on your balance in the Aura system!");
        user.balance -= price[_num];
        user._auto[_num] = !user._auto[_num]; //on auto
        startLine(msg.sender, (_num + 1));
        }
        else {
        // payble new tx
...
//withdraw BNB for owner
   function withdrawAll() public payable onlyOwner {
    uint256 _per = (address(this).balance - reserve) / 100;
    if ((StartSale + 86400) < block.timestamp) { //2592000 for 30 days
