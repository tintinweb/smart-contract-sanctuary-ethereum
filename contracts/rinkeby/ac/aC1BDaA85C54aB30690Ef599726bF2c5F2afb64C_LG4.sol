...
contract LG4 is ERC1155, ERC1155Burnable,  Ownable, ReentrancyGuard {
    string private _name="lgnm";
    ...
    constructor(address signer, address teamSafe, address ProjectSafe,
    ...
    function DailyWinner(address winner) external onlyOwner {
        ...
        (sent,) = payable(winner).call{value: uint(( contractBalance * dayPercentage ) / 100) }("");
        (sent,) = payable(_teamSafe).call{value: uint(( contractBalance * teamPercetage ) / 100) }("");
        (sent,) = payable(_projectSafe).call{value: uint(( contractBalance * projectPercentage ) / 100) }("");
        _day  = currentToken;
    ...
    function zeroMint(bytes32[] calldata signature) public {
        //require(!zeroClaimed[msg.sender], "Already claimed");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(signature, _signerroot, leaf),
            "Invalid Wallet"
    ...
    function ownerzeroMint(address receiver, bytes32[] calldata signature) public {
        bytes32 leaf = keccak256(abi.encodePacked(receiver));
        require(
            MerkleProof.verify(signature,leaf,_signerroot),
            "Invalid Signature."
        );
        mintLG(receiver, 1);
    }
    ...
    function FreeMint(uint256 mintCount) external  {
	    require(
                 (_freeSaleIsActive),
                "Closed"
        );
