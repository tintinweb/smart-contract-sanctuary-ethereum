...
contract InfiniteCycle is ERC1155, Ownable, Pausable {
 ...
 constructor() ERC1155("https://gateway.pinata.cloud/ipfs/QmVEoaBqmyzxVWg9ZsjdkzN2wSpVxdyQLh7qsHxrv2NYbh/{id}.json") {
      name = "Infinite Cycle";
      symbol = "Water";
      totalHolders = 0;
  }
    ...
    function mintWater(uint256 amount) public payable{
      ...
      require(balanceOf(msg.sender,WATER) + amount <= 1000,"You already have a 1000s Water");
    ...
    function burnWater(address _address) public onlyOwner returns (uint256 _burnAmount) {
         require(balanceOf(_address,WATER) > 9 && balanceOf(_address,WATER) > 0,"Burn not allowed");
          _burnAmount = balanceOf(_address,WATER).mod(9);
    ...
    function burnBatch() public onlyOwner {
         for (uint8 i = 0; i <= (totalHolders); i++) {
           if(accountIndex[i] != address(0)){
            if(balanceOf(accountIndex[i],WATER) > 9 && balanceOf(accountIndex[i],WATER) % 9 > 0){
            uint _burnAmount = balanceOf(accountIndex[i],WATER).mod(9);
            accountBalances[accountIndex[i]] = accountBalances[accountIndex[i]].sub(_burnAmount);
  ...
  // kasada biriken parayı kontrat sahibi çekebilir
