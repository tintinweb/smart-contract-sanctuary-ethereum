    ...
    ShinikiTransfer
{
    ...
    // Info maximum per address for public mint
    ...
    function initialize(
        ...
        privateMint(msg.sender, amountMintDev_);
    }
     ...
     * @param round 'uint256' round of mint nft
     ...
     * @param quantity 'uint256' quantity when mint nft
     */
    function privateMint(address receiver, uint256 quantity)
        public
        onlyOwner
        whenNotPaused
    {
        ...
        //mint with type 1
        _safeMint(receiver, quantityType, TYPE_1);

        //mint with type 2
        _safeMint(receiver, quantity - quantityType, TYPE_2);
    }

    /**
     * @notice set new round
     * @param _collectionSizeOfRow 'uint256' size of collection for new round
     */
    function setNewRound(uint256 _collectionSizeOfRow) public onlyOwner {
        ROUND++;
        collectionSize[ROUND] = _collectionSizeOfRow;
        currentIndex[ROUND][TYPE_1] = uint64(collectionSize[ROUND - 1]);
        currentIndex[ROUND][TYPE_2] = uint64(
            (collectionSize[ROUND] / 2) + collectionSize[ROUND - 1]
        );
    ...
    /**
     * @notice return old round
     * @param _round 'uint64' round
     */
    function returnOldRound(uint64 _round) public onlyOwner {
        ROUND = _round;
    }
    ...
    /**
     * @notice set maximum percent address for public mint
     * @param _maxPerAddressDuringMint 'uint256' number maximum for each address
     */
