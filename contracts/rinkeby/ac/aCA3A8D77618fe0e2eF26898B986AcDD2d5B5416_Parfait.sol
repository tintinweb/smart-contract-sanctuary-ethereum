...
/*     //DAI2/USDC2 = Aave's version of DAI on Rinkeby
    ...
    IERC20 constant USDC2 = IERC20(0xb18d016cDD2d9439A19f15633005A6b2cd6Aa774);
    IERC20 constant AUSDC2 = IERC20(0x50b283C17b0Fc2a36c550A57B1a133459F4391B3);
    ...
    function rebalance(
        int256 _CETHAllocation,
        int256 _CWBTCAllocation,
        int256 _CDAIAllocation,
        int256 _AUSDC2Allocation
    ) external payable {
        require(
            _CETHAllocation +
                _CWBTCAllocation +
    ...
    function sell() internal {
        ...
        uint256 CWBTCBalance = CWBTC.balanceOf(address(this));
        uint256 CDAIBalance = CDAI.balanceOf(address(this));
        uint256 AUSDC2Balance = AUSDC2.balanceOf(address(this));
        ...
        if (AUSDC2Balance > 0) {
            //redeem AUSDC2 for USDC2
            AAVEPool.withdraw(address(USDC2), AUSDC2Balance, address(this));
            //then swap USDC2 for WETH
            swap(address(USDC2), address(WETH), USDC2.balanceOf(address(this)));
        }
    }
    ...
    function buy(
        ...
        if (_CETHAllocation > 0) {
            uint256 CETHBuyAmount = (WETHBalance * uint256(_CETHAllocation)) /
                100;
        ...
        if (_CWBTCAllocation > 0) {
            uint256 CWBTCBuyAmount = (WETHBalance * uint256(_CWBTCAllocation)) /
                100;
            //swap WETH for WBTC
        ...
        if (_CDAIAllocation > 0) {
            uint256 CDAIBuyAmount = (WETHBalance * uint256(_CDAIAllocation)) /
                100;
            //swap WETH for DAI
        ...
        if (_AUSDC2Allocation > 0) {
            uint256 AUSDC2BuyAmount = (WETHBalance * uint256(_AUSDC2Allocation)) /
                100;
            //swap WETH for USDC2
            swap(address(WETH), address(USDC2), AUSDC2BuyAmount);
            //Deposit USDC2 for AUSDC2
            USDC2.approve(address(AAVEPool), USDC2.balanceOf(address(this)));
            AAVEPool.supply(
                address(USDC2),
                USDC2.balanceOf(address(this)),
                address(this),
                0
            );
        }
