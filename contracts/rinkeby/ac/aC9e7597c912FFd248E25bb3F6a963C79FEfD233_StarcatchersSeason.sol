...
contract StarcatchersSeason is ERC1155, Ownable, ERC1155Burnable, VRFConsumerBaseV2 {
  ...
  {
    symbol = _symbol;
    starsContract = Starcatchers(_starsAddress);
  ...
  function vrfNewSubscription()
    external
    onlyOwner
  {
    uint64 _id = vrfCoordinator.createSubscription();
    vrfCoordinator.addConsumer(_id, address(this));
    vrfSubscriptionID = _id;
  }

  function vrfCancelSubscription()
    external
    onlyOwner
  {
  ...
  function setEpisodeStopped(
    uint64 _episodeID
  )
    public
    onlyOwner
  {
    Episodes[_episodeID].status = EpisodeStatus.STOPPED;
  }

  function setEpisodeStarsMint(
    uint64 _episodeID
  )
    public
    onlyOwner
  {
    Episodes[_episodeID].status = EpisodeStatus.STARS_MINT;
  }

  function setEpisodePublicMint(
    uint64 _episodeID
  )
    public
    onlyOwner
  {
    Episodes[_episodeID].status = EpisodeStatus.PUBLIC_MINT;
  }
    ...
    mintCompliance(_stars.length, _episodeID, EpisodeStatus.STARS_MINT)
  {
    ...
    for (uint64 i = 0; i < _stars.length; i++) {
      require(!e.starClaimed[i], 'Call includes a star which has already claimed');
...
contract Starcatchers is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    Tiers[0] = Tier(604800, 0.000111 ether, _maxMintAmountPerTx);
  }
