...
contract Vesting is AccessControl {
    ...
    function claim() external notStopped notBlacklisted {
        ...
        if (_checkBeneficiaryMarkup(msg.sender)) _beneficiaryMarkup[msg.sender] = true;
    ...
    function getAvailableAmountToClaim(address beneficiary) public view returns (uint256 amount, uint256 amountMarkup) {
        ...
        if (_beneficiaryMarkup[beneficiary]) {
            uint96 markupFee = sale.getMarkupFee();
            amountMarkup = (amount * markupFee) / PERCENTAGE;
        }
    }

    // Checks if beneficiary already bought marked up prices on IDO if not markup beneficiary depending on stake amount
    // Note: once staked amount is not enough always deduct incoming vesting amounts
    function _checkBeneficiaryMarkup(address beneficiary) private view returns (bool markup) {
        ...
        uint256 stakedAmount = staking.getStakedAmount(beneficiary);
        uint256 minStakedAmount = sale.getMinStakedAmount();
        if (!participantMarkup && !_beneficiaryMarkup[beneficiary])
