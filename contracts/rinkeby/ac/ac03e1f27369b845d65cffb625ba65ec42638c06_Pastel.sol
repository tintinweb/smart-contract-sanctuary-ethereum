...
contract Pastel is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    function gift(address _to, uint256 _mintAmount) public onlyOwner { // or public
        require(
            totalSupply() + _mintAmount <= maxSupply,
    ...
    // perhaps would bemore efficient with a merkle airdrop vs address in memory
    // is this airdrop pulling from collection or another, if so where is that being passed
    // is this just gifting but 1 to each address
    ...
    function setWithdrawAddresses(address[] calldata _w) external onlyOwner {
