...
contract Cave is Ownable, IERC721Receiver, Pausable {
  ...
  function random(uint256 seed) internal view returns (uint256) {
    ...
    function getTokenIds(address _user) public view returns( uint256 [] memory){
      ...
      uint256[] memory checker; // change on what you need to return
      uint j = 0;
      for(uint i = 0; i < 3333; i++){
          if(theCave[i].value == 0) {
            continue;
          }
          if(theCave[i].owner == _user){
              checker[j]=theCave[i].tokenId; //or whatever you want to do if it matches
              j++;
          }
      }
      return checker;    
