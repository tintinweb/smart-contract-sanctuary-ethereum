...
contract Decanraland_A3 is Ownable{
    ...
    // function removeSig(uint256 key) external onlyOwner {
    //    s.sig[key] = s.sig[s.sig.length-1];
    //     s.sig.pop();
    // }

    // function getSign(uint256 key) external view returns(bytes memory){
    //     return s.sig[key];
    // }
    ...
    function checkAllowed(address contractAddress) external view{
