...
contract PuppetParts is ERC721A, Ownable {
    ...
    */
    ...
    constructor (string memory _name, string memory _symbol, string memory _initPartsBaseUri) ERC721A(_name, _symbol) {
    ...
    function presaleMintTier1(bytes32[] calldata _proof, uint256 _quantity) external payable {
        require(presaleIsActive,                                   "Not Allow");
        require(_quantity > 0 && _quantity <= 6,                   "Incorrect quantity");
        require(priceByMintAmount[_quantity - 1] == msg.value,     "Ether value incorrect");
        require(totalSupply() + _quantity <= maxPartSupply,        "Quantity exceed total supply");

        bytes32 merkleTreeRoot = 0xb92677049f5545fb8c036fa6b66ed58c2e7c02124b3234d5a5210be5d3d79653;
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_proof, merkleTreeRoot, leaf),  "Not whitelisted");
        require(amountMinted[msg.sender] + _quantity <= 6,         "Overclaimed");

        amountMinted[msg.sender] += _quantity;
        _safeMint(msg.sender, _quantity);
    }

    function presaleMintTier2(bytes32[] calldata _proof, uint256 _quantity) external payable {
        require(presaleIsActive,                                   "Not Allow");
        require(_quantity > 0 && _quantity <= 6,                   "Incorrect quantity");
        require(priceByMintAmount[_quantity - 1] == msg.value,     "Ether value incorrect");
        require(totalSupply() + _quantity <= maxPartSupply,        "Quantity exceed total supply");
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_proof, merkleTreeRoot, leaf),  "Not whitelisted");
        require(amountMinted[msg.sender] + _quantity <= 12,         "Overclaimed");
