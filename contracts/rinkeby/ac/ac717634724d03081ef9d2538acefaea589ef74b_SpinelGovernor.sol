...
import "./club/TreasuryInit.sol";
import "./club/ClubInit.sol";
    ...
    OwnableUpgradeable
{
    ...
    mapping(address => ClubMeta) clubs;
    address clubGov;
    ...
    function setClubContract(address _clubGov, address _clubTreasury, address _votePower) public  {
        clubGov = _clubGov;
        clubTreasury = _clubTreasury;
        votePower = _votePower;
    }
    function createClub(ITreasury.ClubType clubType, address gnosis) public {
        require(clubs[msg.sender].owner == address(0), "existed");
        clubs[msg.sender].owner = msg.sender;
        clubs[msg.sender].voteToken = Clones.clone(votePower);
        clubs[msg.sender].treasury = Clones.clone(clubTreasury);
        TreasuryInit(clubs[msg.sender].treasury).initialize(owner(), 10, gnosis, IERC20(clubs[msg.sender].voteToken));
        clubs[msg.sender].gov = Clones.clone(clubGov);
        ClubInit(clubs[msg.sender].treasury).initialize(clubs[msg.sender].voteToken, clubs[msg.sender].treasury);
        ITreasury(clubs[msg.sender].treasury).setAdmin(clubs[msg.sender].gov);
        ITreasury(clubs[msg.sender].treasury).setClubType(clubType);
...
interface TreasuryInit {
    function initialize(address admin_, uint256 delay_, address _treasuryAddress, IERC20 _votePower) external;
}
