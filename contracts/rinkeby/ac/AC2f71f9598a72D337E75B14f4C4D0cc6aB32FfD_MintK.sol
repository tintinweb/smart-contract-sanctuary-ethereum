...
contract MintK is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public preSaleMintPrice = .01 ether;
    ...
    ) ERC721A("KashMint", "KMINT", maxBatchSize_, collectionSize_) {
    ...
    function preSaleMint() external payable callerIsUser {
        uint256 price = preSaleMintPrice;
        require(price != 0, "presale sale has not begun yet");
        require(presalelist[msg.sender] > 0, "not eligible for presale mint"); //check to see if you own one
        require(totalSupply() + 1 <= collectionSize, "reached max supply");
        presalelist[msg.sender]--;
        _safeMint(msg.sender, 1);
        tokenCounter++;
        //refundIfOver(price); //see if people want this line
    }

    function publicSaleMint(uint256 quantity) external payable callerIsUser {
        uint256 publicPrice = publicMintPrice;
        require(
