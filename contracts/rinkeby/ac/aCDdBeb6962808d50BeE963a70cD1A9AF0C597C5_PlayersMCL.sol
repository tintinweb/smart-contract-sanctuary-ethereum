...
contract PlayersMCL is ERC721, Ownable {
    ...
    constructor( ) ERC721("TestPlayersMCL", "MCL"){ //string memory _baseURI
        totalMinted = 0;
    ...
    function nftMint(uint256 _combId) public returns (uint){
        require( bytes(_combId.toString()).length >= layerOrder.length-1, "CombId error");    
    ...
    function generateFinalMetaJson(uint256 tokenId) internal view returns (string memory){
        ...
        string memory nftName = string(abi.encodePacked("TMCL Player #", tokenId.toString())) ;

        string memory json = Base64.encode(
            bytes(string(
                    abi.encodePacked(
                        '{"name": "',nftName, '",',
                        ' "description": "TMCL Players Collection",',
