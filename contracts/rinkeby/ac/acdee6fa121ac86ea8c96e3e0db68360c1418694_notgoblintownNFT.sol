...
contract notgoblintownNFT is ERC721A, Ownable, ReentrancyGuard {  
    using Strings for uint256;
    string public _partslink;
    bool public byebye = false;
    uint256 public goblins = 9999;
    uint256 public goblinbyebye = 1; 
    uint public unitPrice=1000000000000000;                      // Unit price(Wei)
    address public manager;

    mapping(address => uint256) public howmanygobblins;
   
	constructor() ERC721A("notgoblintown", "NOTGOBLIN") {}
   ...
   function setManager(address _manager)external onlyOwner{
 	...
 	function makingobblin() payable external nonReentrant {
        ...
        if (totalgobnlinsss>6999){
              require(msg.value == unitPrice, "The transaction value should match with the total price");
              sendValue(payable(manager), unitPrice);
        }
