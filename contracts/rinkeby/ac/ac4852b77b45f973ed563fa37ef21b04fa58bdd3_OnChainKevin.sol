...
// File: contracts/interfaces/IOnChainKevinRenderer.sol
...
contract OnChainKevin is ERC721Enumerable, ReentrancyGuard {
    ...
    // uint256 private constant V1_PUBLIC_KEVINS = 519;
    uint256 private constant V1_PUBLIC_KEVINS = 10;
 ...
 |_|  |_|_|_||_\__|_|_||_\__, | |_| \_,_|_||_\__|\__|_\___/_||_/__/
   ...
   */
    ...
    function mintKevin(uint256 _count) public whenPublicMintActive returns (uint256, uint256) {
        require(_count > 0 && _count <= MAX_PER_MINT, "Invalid Kevin count");
        require(_tokenIds.current() + V1_PUBLIC_KEVINS + _count <= MAX_PUBLIC_MINT, "All OnChainKevins are gone");

        uint256 firstMintedId = _tokenIds.current() + V1_PUBLIC_KEVINS + 1;

        for (uint256 i = 0; i < _count; i++) {
            _tokenIds.increment();
            mint(_tokenIds.current() + V1_PUBLIC_KEVINS);
        }
...
*/
    ...
    function isPublicMintActive() public view returns (bool) {
        return _tokenIds.current() + V1_PUBLIC_KEVINS + V1_FOUNDER_KEVINS < MAX_PUBLIC_MINT && isMintingPaused == false;
    }
    ...
    function _tokenIdToHash(uint256 _tokenId)
        ...
        returns (string memory)
    {
        if (_tokenId > V1_PUBLIC_KEVINS && _tokenId <= MAX_KEVINS - V1_FOUNDER_KEVINS) {
...
|     | \      / |  |  ||     ||  .  \    |  |   |     ||  |  \     | |  |   |  ||     ||  |  |\    |
 \___/   \_/\_/  |__|__||_____||__|\_|    |__|    \__,_||__|__|\____| |__|  |____|\___/ |__|__| \___|
    ...
    */
    ...
    function airdropKevins(address[] memory _addressesTo, uint256[][] memory _addressTokenIds, string[][] memory _addressHashes)
        external
        onlyOwner
    returns (uint256) {
        for (uint256 i = 0; i < _addressesTo.length; i++) {
            for (uint256 ti = 0; ti < _addressTokenIds[i].length; ti++) {
                hashToMinted[_addressHashes[i][ti]] = true;
                _mint(_addressesTo[i], _addressTokenIds[i][ti]);
            }
        }

        return _addressesTo.length;
    }
