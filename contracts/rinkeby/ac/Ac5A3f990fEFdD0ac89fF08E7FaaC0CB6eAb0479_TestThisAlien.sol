...
contract TestThisAlien is ERC721AQueryable, Ownable, ReentrancyGuard {

  using Strings for uint256;

  bytes32 private merkleRootOne;
  bytes32 private merkleRootTwo;
  bytes32 private merkleRootThree;
  bytes32 private merkleRootFour;
  bytes32 private merkleRootFive;
 ...
 function getMerkleeTreeOne() public view returns (bytes32) {
  ...
  function getMerkleeTreeFour() public view returns (bytes32) {
        return merkleRootFour;
    }

  function getMerkleeTreeFive() public view returns (bytes32) {
        return merkleRootFive;
    }
  ...
  function getMerkleFour(address sampleAddress, bytes32[] calldata _merkleProof) public view returns (bool){
        bytes32 leaf = keccak256(abi.encodePacked(sampleAddress));
        return MerkleProof.verify(_merkleProof, merkleRootFour, leaf);
    }
  function getMerkleFive(address sampleAddress, bytes32[] calldata _merkleProof) public view returns (bool){
        bytes32 leaf = keccak256(abi.encodePacked(sampleAddress));
        return MerkleProof.verify(_merkleProof, merkleRootFive, leaf);
    }
  ...
  function setMerkleRootFour(bytes32 _merkleRoot) public onlyOwner {
        merkleRootFour = _merkleRoot;
    }
  
  function setMerkleRootFive(bytes32 _merkleRoot) public onlyOwner {
        merkleRootFive = _merkleRoot;
    }
 /*
  * holder addresses with more than 5 Eligible NFTs 
  */
 ...
 /*
  * Free Mint - Requires User To Be In Free Mint Merkle Trees or added to holder addresses array
  */
  function holderMint(uint256 numberOfTokens, bytes32[] calldata _merkleProof) public nonReentrant() {
        ...
        require(totalSupply() + numberOfTokens <= maxSupply, "mint is sold out");
        ...
        if (validNumberOfTokensPerBuyerMap[msg.sender] == 0){
           ...
           } else if (MerkleProof.verify(_merkleProof, merkleRootFour, leaf)){
                validNumberOfTokensPerBuyerMap[msg.sender] = 4;
           } else if (MerkleProof.verify(_merkleProof, merkleRootFive, leaf)){
                validNumberOfTokensPerBuyerMap[msg.sender] = 5;
            }else{
                require(false, "your address is not approved");
            }
        }

        require(numberMinted(msg.sender) + numberOfTokens <= validNumberOfTokensPerBuyerMap[msg.sender], "you have reached your maximum number of free mints");
...
//WL Functions
  ...
  function getMerkleeTree() public view returns (bytes32) {
        return merkleRoot;
    } 
  
  function getMerkleRoot(address sampleAddress, bytes32[] calldata _merkleProof) public view returns (bool){
        bytes32 leaf = keccak256(abi.encodePacked(sampleAddress));
