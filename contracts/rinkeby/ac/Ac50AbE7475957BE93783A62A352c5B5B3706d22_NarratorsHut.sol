...
contract NarratorsHut is INarratorsHut, ERC721Hut, IERC2981, Ownable {
    ...
    constructor(address _metadataContractAddress, address _narratorAddress)
        ...
        canMintArtifact(tokenData)
    {
        ...
        _mint(
            ...
            tokenData.artifactId,
            tokenData.witchId,
            tokenData.preassignedAttributeIndex
        );
    }
        ...
        returns (string memory)
    {
        ...
        return
            INarratorsHutMetadata(metadataContractAddress).tokenURI(
                artifactId,
                preassignedAttributeIndex,
                tokenId,
                witchId
...
library Signatures {
    ...
    bytes constant TOKEN_DATA_TYPE_DEF =
        "TokenData(uint32 artifactId,uint32 preassignedAttributeIndex,uint32 witchId)";
...
struct TokenDataStorage {
    uint32 artifactId;
    uint32 witchId;
    uint32 preassignedAttributeIndex;
...
contract ERC721Hut is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _safeMint(
        ...
        _mint(to, tokenId, artifactId, witchId, preassignedAttributeIndex);
        require(
            _checkOnERC721Received(address(0), to, tokenId, _data),
            "ERC721: transfer to non ERC721Receiver implementer"
        );
    ...
    function _mint(
        ...
        _tokenDataStorage[tokenId]
            .preassignedAttributeIndex = preassignedAttributeIndex;
    ...
    /**
     * @dev Extracts the artifactId, preassignedAttributeIndex and counter
     * that are packed into the uint256 tokenId.
     * @return (uint64 artifactId, uint64 preassignedAttributeIndex, uint128 witchId)
     */
    ...
    {
        uint32 artifactId = _tokenDataStorage[tokenId].artifactId;
        uint32 preassignedAttributeIndex = _tokenDataStorage[tokenId]
            .preassignedAttributeIndex;
        uint32 witchId = _tokenDataStorage[tokenId].witchId;
        return (artifactId, preassignedAttributeIndex, witchId);
    }
...
struct TokenData {
    // Can safely use uint32 for these values since they can
    // comfortably fit within 2^32
    uint32 witchId;
    uint32 artifactId;
    uint32 preassignedAttributeIndex;
}
