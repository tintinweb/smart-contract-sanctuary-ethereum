...
contract CricStoxTreasury is Ownable {
    ...
    constructor(address adminWallet_, address factoryAddress_) {
    ...
    function userWithdraw(address token_, address user_, uint256 amount_) external onlyMaster {
        bool playerStoxs = ICricStoxFactory(factoryAddress).getPlayerStox(msg.sender);
        require(playerStoxs, "Can be called only by player stox contract.");
        IERC20 token = IERC20(token_);
