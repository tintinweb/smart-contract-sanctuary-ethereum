 ...
 * by @prabhu
 */
contract HoverchairContract is ERC1155, Ownable, ReentrancyGuard, AccessControl {
    ...
    constructor(
        ...
        )ERC1155("https://www.hoverchairstudios.com/")
        {
    ...
    function addToken1(bool _active, string memory _name, string memory _metadataUrl, uint256 _maxSupply, uint256 _mintWeiPrice) 
    public 
    onlyOwnerOrAdmin1 
    {
    ...
    function toggleTokenSaleStatus1(uint256 tokenId) 
    public
    onlyOwnerOrAdmin1 
    {
    ...
    modifier isTokenTransactionValid1(uint256 tokenId, uint256 amount, bool shouldBeActive) {
        MultiToken storage token = tokens[tokenId];
    ...
    modifier onlyOwnerOrAdmin1() {
        require(owner() == _msgSender() || hasRole(DEFAULT_ADMIN_ROLE,_msgSender()),"This action can be performed by Owner or Admin only");
        _;
    }

    function airdrop1(uint256 _tokenId, address[] calldata _list)
        public
    {
        batchAirdrop1(_tokenId,1,_list);
    }
    function batchAirdrop1(uint256 _tokenId, uint256 _tokenCount, address[] calldata _list)
        public
        isTokenTransactionValid1(_tokenId, _tokenCount*_list.length, false)
        onlyOwnerOrAdmin1
    {
        MultiToken storage token = tokens[_tokenId];
    ...
    virtual 
    isTokenTransactionValid1(id, amount, false)
    onlyOwnerOrAdmin1
    {
    ...
    nonReentrant
    isNotContract
    isTokenTransactionValid1(tokenId, amount, true)
    {
        MultiToken storage token = tokens[tokenId];
    ...
    function withdraw(uint256 amount) public virtual onlyOwnerOrAdmin1 {
         if (amount == 0) {
