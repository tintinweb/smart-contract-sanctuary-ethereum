...
contract NiftyWalls is Ownable, ERC721WithShuffledIDs {
    ...
    constructor(address _metadata, uint256 _tokenCount, uint256 _mintsPerDay, uint _initialMint) 
    ...
    function price() public view returns (uint256) {
        if ( (block.timestamp - mintPeriodStart) / (60*60*24) == 0) {
            ...
            newPrice = (mintedToday == mintsPerDay) ? _price+(_price/10) : newPrice ;
            newPrice = (mintedToday < 3) ? _price-(_price/10) : newPrice ;
            newPrice = (_price < minPrice) ? minPrice : newPrice ;
            return newPrice ;
        }
    }
    ...
    function _mint() private
    {
        ...
        updateMintsPerDay();
        require( mintsPerDay > mintedToday, "Maximum limit of mints per period reached." );
        mintedToday += 1 ;
        _safeMint(msg.sender);
    }
