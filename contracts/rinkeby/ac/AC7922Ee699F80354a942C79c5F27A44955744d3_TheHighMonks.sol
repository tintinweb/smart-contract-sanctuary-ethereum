...
contract TheHighMonks is ERC721A, Ownable, ReentrancyGuard {
    ...
    ) ERC721A("TheHighMonks", "THM", _maxBatchSize, _collectionSize) {
    ...
    function mintAllowList(uint8 numberOfTokens) external payable {
        ...
        require(
            ts + numberOfTokens <= MAX_SUPPLY - reserved,
    ...
    function reserve(uint256 n) public onlyOwner {
        uint256 ts = totalSupply();
        require(ts + n <= MAX_SUPPLY, "Reserve too many tokens");

        reserved = n;
    }

    function mintReserved(address to, uint256 n) public onlyOwner {
        require(n <= reserved, "Mint more than reserved");
        _safeMint(to, n);
        reserved -= n;
    }
