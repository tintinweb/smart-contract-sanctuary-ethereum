...
contract bemuerTestContractx is ERC721A, Ownable {
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable callerIsUser {
    ...
    require(MerkleProof.verify(_merkleProof, ogRoot, leaf), "user is not whitelisted");
    require(ogMintedBalance[msg.sender] + _mintAmount <= maxOgMint, "max NFT limit exceeded for this whitelist user");
    //
