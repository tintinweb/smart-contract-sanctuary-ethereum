...
contract AnimalChess {
    ...
    function deleteOwnerArr(address delOwner) private validAddress(delOwner){
        uint ownersArrLen = ownersArr.length;
        for(uint i = 0; i < ownersArrLen; i++) {
    ...
    function struggleAction(string memory roomID , address _addrA , uint[] memory _animalArrA, address _addrB , uint[] memory _animalArrB) public onlyOwners {
        ...
        address addrA = play_list[roomID].playA.player;
        address addrB = play_list[roomID].playB.player;
        require(addrA == _addrA || addrA == _addrB,"_addrA not Include playA");        
        require(addrB == _addrA || addrB == _addrB,"_addrB not Include playB");       
        if (addrA == _addrA){
            play_list[roomID].playA.aniArr = _animalArrA;
            play_list[roomID].playB.aniArr = _animalArrB;
        } else if (addrB == _addrA){
    ...
    function animalStruggle(string memory roomID) private {
        ...
        uint aniAArrLength = aniAArr.length;
        for (uint i = 0; i < aniAArrLength; i++) {
        ...
        uint winnerNum;
        winner finalResult = play_list[roomID].Final_Result;
        if (winA > winB) {
            finalResult = winner.A;
            winnerNum = 1;
        } else if (winB > winA) {
            finalResult = winner.B;
            winnerNum = 2;
        } else if (winA == winB) {
            finalResult = winner.none;
            winnerNum = 0;
        }
