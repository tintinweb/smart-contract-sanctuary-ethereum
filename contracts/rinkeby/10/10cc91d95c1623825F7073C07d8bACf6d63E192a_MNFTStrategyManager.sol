...
contract MNFTStrategyManager is OwnableUpgradeable, AccessControlEnumerableUpgradeable, IMNFTStrategyManager {
   ...
   function viewblacklistedStrategies() external view override returns (address[] memory) {
      uint256 length = blacklistedStrategies.length();
      address[] memory strategies = new address[](length);

      for (uint256 i = 0; i < length; i++) {
         strategies[i] = blacklistedStrategies.at(i);
      }
