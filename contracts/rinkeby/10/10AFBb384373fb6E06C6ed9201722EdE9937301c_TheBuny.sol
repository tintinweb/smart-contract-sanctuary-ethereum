...
abstract contract cidIPFS is ERC721 {
...
.
        return string(abi.encodePacked(
            _baseURI(), "/", tokenId.toString(), "/_metadata.json"
        ));
    }
*/
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return string(abi.encodePacked("https://buni.mypinata.cloud/ipfs/", cid));
    }
...
import "@1001-digital/erc721-extensions/contracts/cidIPFS.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/cryptography/ECDSA.sol";
import "@openzeppelin/contracts/finance/PaymentSplitter.sol";
import "@openzeppelin/contracts/utils/cryptography/SignatureChecker.sol";
import "./ERC2981PerToken.sol";


contract TheBuny is ERC721, ERC2981PerToken,
   cidIPFS,
   ReentrancyGuard,
   PaymentSplitter,
   Ownable  {
  ...
  constructor(string memory _cid, address[] memory payees, uint256[] memory shares) ERC721("The Buny", "BUNY")
...
function supportsInterface(bytes4 interfaceId)
    ...
    /// @param payees an array of payees for each token
    /// @param royaltyRecipients an array of payees for royalties (if royaltyValues[i] > 0)
    ...
    function mintBatch(
        ...
        uint256 tokenId = nextTokenId;
        require(
            payees.length == royaltyRecipients.length &&
                payees.length == royaltyValues.length,
            'ERC721: Arrays length mismatch'
        );

        for (uint256 i; i < payees.length; i++) {
            _safeMint(payees[i], tokenId, '');
    ...
    function mint (uint256 _quantity) public payable {
     ...
     require(_quantity > 0 && _quantity <= MAX_MINTS, "Minimum of 1, maximum of 10 tokens");
     uint256 price = MINT_PRICE;
     require(msg.value >= _quantity * price, "Insufficient Payment: Amount of Ether sent is not correct. ");
      _tokenId++;
    ...
    //IPFS
    // cidIPFS implementation
    function _baseURI()
        internal view override(ERC721, cidIPFS)
        returns (string memory)
    {
        return cidIPFS._baseURI();
    }

    // Use cidIPFS implementation
    function tokenURI(uint256 tokenId)
        public view override(ERC721, cidIPFS)
        returns (string memory)
    {
        return cidIPFS.tokenURI(tokenId);
    }
