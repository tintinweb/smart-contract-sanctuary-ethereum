...
contract RouterUpgradeable is Initializable, OnlyAddress {
    ...
    function init(IStorage storage_, address superAdmin_) external initializer {
        _storage = storage_;
        _storage.setSuperAdmin(superAdmin_);
...
library StorageGetters {
    ...
    function isSuperAdmin(IStorage storage_) internal view returns (address) {
        return storage_.read(StorageFields.SUPER_ADMIN()).toAddress();
    }
...
library StorageSetters {
    ...
    function setSuperAdmin(IStorage storage_, address admin) internal {
        return storage_.write(StorageFields.SUPER_ADMIN(), admin.toBytes32());
    }
...
library StorageFields {
    /// MAIN ADMIN
    function SUPER_ADMIN() internal pure returns (bytes32) {
        return keccak256(abi.encode('SUPER_ADMIN'));
    }
