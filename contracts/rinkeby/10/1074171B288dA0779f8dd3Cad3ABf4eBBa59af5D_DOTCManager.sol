...
contract DOTCManager is ERC1155Holder, IdOTC, AdminFunctions, ReentrancyGuard {
    ...
    event CreatedOrder(uint256 indexed orderId, Order order, uint256 availableAmountOut);
    ...
    function takeOffer(
        ...
        uint256 amountOut = allOffers[offerId].amountOut;
        address tokenOutAddress = allOffers[offerId].tokenOutAddress;
        uint256 realAmountOut = unstandardisedNumber(amountOut, tokenOutAddress);
        emit CreatedOrder(orderId, takerOrders[orderId], realAmountOut);
        return orderId;
    }
    ...
    {
        require(_amount > 0, "ZERO_AMOUNT");
        require(_amount == allNftOffers[nftOfferId].offerPrice, "AMOUNT_IS_TOO_BIG");
        require(
    ...
    function updateOffer(
        ...
        if (!isNFT) {
            ...
            require(allNftOffers[offerId].maker == msg.sender, "NOT_OWNER");
            if (newOffer != allNftOffers[offerId].offerPrice) {
                allNftOffers[offerId].offerPrice = newOffer;
                emit NftOfferUpdated(offerId, newOffer);
            }
            if (_expiryTimestamp != allNftOffers[offerId].expiryTime) {
                allNftOffers[offerId].expiryTime = _expiryTimestamp;
                emit UpdatedNftOfferExpiry(offerId, _expiryTimestamp);
            }
        }
        return true;
    }
    ...
    */
    modifier canBuyNftOffer(uint256 offerId) {
        require(isActiveNftOffer(offerId), "IN_ACTIVE_OFFER");
        _;
    }
    /**
