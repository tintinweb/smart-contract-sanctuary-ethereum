...
contract FloppyNftErc1155 is ERC1155Supply, Ownable {
    ...
    // replace this signer address with backend address
    ...
    */
    ...
    // check to prevent collectionId > 5
    ...
    */
        ...
        isMintLive(collectionId)
    {
        ...
        // check if collection is active, defaults to inactive
        if (!collection.active) revert collectionSaleNotActive();
        // ensure 1 wallet only gets to mint once
        if (collection.mintList[msg.sender]) revert alreadyMinted();
        // free mint, so no need for price check
        // always mints one token, so no need for to check for batch mint
        if (totalSupply(collectionId) > collection.maxSupply)
            revert maxSupplyReached();

        // add sender to mint list
    ...
    */
    ...
    // TODO: Change the addresses before deploying
    function withdraw() external onlyOwner {
        uint256 _balance = address(this).balance;
        payable(0x08a80B42f8499C1C3f9e352632fB87E38F5Cbc80).transfer(
            ((_balance * 5) / 100)
        );
        payable(0x27daDECD72f316C7fb94aE06588ee41b40a79c6b).transfer(
            ((_balance * 95) / 100)
        );
    }
}
