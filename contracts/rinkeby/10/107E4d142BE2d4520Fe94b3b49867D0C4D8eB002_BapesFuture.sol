...
contract BapesFuture is ERC721A, Ownable, AccessControl, ReentrancyGuard {
  ...
  function _mintCompliance(uint256 _mintAmount) private view {
    ...
    if (_mintAmount > maxPerTx) {
      revert MintAmountExeedsMaxPerTxLimit();
    }

    if (totalSupply() + _mintAmount > maxSupplyTotal) {
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    if (!_exists(tokenId)) {
      revert NoDataExistsForProvidedTokenId();
    }
  ...
  function pcMint(uint256 _mintAmount) external payable mintCompliance(_mintAmount) {
    if (!pcStarted) {
      revert ParentalCertificateSaleIsPaused();
    }

    if (msg.value < (pcPrice * _mintAmount)) {
      revert InsufficientBalanceToMint();
    }

    if (_mintAmount > pc.balanceOf(_msgSender())) {
      revert YouDoNotHaveEnoughPCsToMintSelectedAmountOfTokens();
    }
  ...
  function bgkMint(uint256 _mintAmount) external payable mintCompliance(_mintAmount) {
    if (!bgkStarted) {
      revert BapesGenesisKeySaleIsPaused();
    }

    if (msg.value < (bgkPrice * _mintAmount)) {
      revert InsufficientBalanceToMint();
    }

    if (_mintAmount > bgk.balanceOf(_msgSender())) {
      revert YouDoNotHaveEnoughBGKsToMintSelectedAmountOfTokens();
    }
    ...
    mintCompliance(_mintAmount)
  {
    ...
    if (!wlStarted) {
      revert PrivateSaleIsPaused();
    }

    if (msg.value < (wlPrice * _mintAmount)) {
      revert InsufficientBalanceToMint();
    }

    if ((minted + _mintAmount) > maxPerWallet) {
      revert SelectedNumberOfMintsWillExceedTheMaximumAmountOfAllowedPerWallet();
    }

    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));

    if (!MerkleProof.verify(_merkleProof, merkleRoot, leaf)) {
      revert InvalidProofThisWalletIsNotWhitelisted();
    }
  ...
  function publicMint(uint256 _mintAmount) external payable mintCompliance(_mintAmount) {
    if (!publicStarted) {
      revert PublicSaleIsPaused();
    }

    if (msg.value < (publicPrice * _mintAmount)) {
      revert InsufficientBalanceToMint();
    }
  ...
  function updateMaxSupplyTotal(uint256 _number) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    if (_number > maxSupply) {
      revert PublicSupplyCanNotExceedTotalDefined();
    }
