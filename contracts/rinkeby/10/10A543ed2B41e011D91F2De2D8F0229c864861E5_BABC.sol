...
contract BABC is ERC721A, BaseOpenSea, Ownable {
    bytes32 private _merkleRootWhitelist;
    bytes32 private _merkleRootOG;

    uint256 public constant maxSupply = 3456;

    uint256 public constant maxOGSupply = 200;
    uint256 public constant maxWhitelistSupply = 400;
    uint256 public constant maxFreeMintSupply = 400;

    uint256 private constant maxMintsFree = 1;
    uint256 private constant maxMintsPerOG = 1;
    uint256 private constant maxPerAddress = 8;

    uint256 public constant publicMintPrice = 0.022 ether;

    uint256 public whitelistMintCounter;
    uint256 public freeMintCounter;
    uint256 public OGMintCounter;

    uint256 public whitelistStartDate = 1645372800;
    uint256 public constant saleStartDate = 1643381100;
    string private baseUri =
        "https://gateway.pinata.cloud/ipfs/QmSm34McXaE2F2oAz3QnUP8kwNVnTZSQHvEHWawmLCbRAS/";
    string private baseExtension = ".json";
    bool revealed = false;

    mapping(address => bool) public OGMintedByAddress;
    mapping(address => bool) public freemintedbyaddress;
    mapping(address => uint16) public whitelistmintedByAddress;

    constructor(address openSeaProxyRegistry, bytes32 root)
        ERC721A("Bored Ape Bones Club", "BABC")
    {
    ...
    function isWhitelistOpen() public view returns (bool) {
        if (
            block.timestamp >= whitelistStartDate &&
            block.timestamp <= whitelistStartDate + 30 minutes
        ) {
            return true;
        } else {
            return false;
    ...
    function isOGOpen() public view returns (bool) {
        if (block.timestamp >= whitelistStartDate) {
            return true;
        } else {
            return false;
        }
    ...
    function setWhitelistStartDate(uint256 date) external onlyOwner {
    ...
    function _claimfree(uint256 amount) private {
        require(totalSupply() + amount + 1 <= maxSupply, "Max Supply reached");
        require(
            (amount > 0) && (amount <= maxPerAddress + 1),
            "8 items max each wallet reached"
        );
        require(msg.value >= publicMintPrice * amount, "Incorrect Price sent");
        freeMintCounter += 1;
        freemintedbyaddress[msg.sender] = true;
        _safeMint(msg.sender, amount + 1);
    }

    function _claimsale(uint256 amount) private {
    ...
    function saleMint(uint256 amount) external payable {
        ...
        freemintedbyaddress[msg.sender] != true &&
            freeMintCounter < maxFreeMintSupply
            ? _claimfree(amount)
            : _claimsale(amount);
    }
    ...
    function mintWhitelist(bytes32[] calldata proof, uint16 amount)
        ...
        onlyEOA
    {
        require(isWhitelistOpen(), "Whitelist Session Closed");
        require(
        ...
        require(
            whitelistmintedByAddress[msg.sender] + amount <= maxPerAddress,
            "Maximum Reserved supply already claimed"
        );
        require(totalSupply() + amount <= maxSupply, "Max Supply reached");
    ...
    function mintOG(bytes32[] calldata proof) external onlyEOA {
        require(isOGOpen(), "OG mint session not open/closed");
        require(verifyWhitelist(proof, _merkleRootOG), "OG Not whitelisted");
        require(
            OGMintCounter + maxMintsPerOG <= maxOGSupply,
            "OG Mint Stock Unavailable"
        );
        require(
            OGMintedByAddress[msg.sender] != true,
            "Reserved supply already claimed"
        );
        require(
            _numberMinted(msg.sender) + maxMintsPerOG <= maxPerAddress,
            "Max per address"
        );
        require(
            totalSupply() + maxMintsPerOG <= maxSupply,
            "Max Supply reached"
        );
        OGMintCounter += maxMintsPerOG;
        OGMintedByAddress[msg.sender] = true;
        _safeMint(msg.sender, maxMintsPerOG);
    }
    ...
    function setMerkleRootOG(bytes32 root) external onlyOwner {
        _merkleRootOG = root;
    }
