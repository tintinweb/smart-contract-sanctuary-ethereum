...
contract TokenFactory is NonblockingLzApp {
    ...
    constructor(address _lzEndpoint) NonblockingLzApp(_lzEndpoint)  {
        ...
        _owner = msg.sender;
        _baseAsset = IERC20(address(0xe6b8a5CF854791412c1f6EFC7CAf629f5Df1c747)); //0xe6b8a5CF854791412c1f6EFC7CAf629f5Df1c747 Mumbai
        // 0x1E77ad77925Ac0075CF61Fb76bA35D884985019d
    ...
    function mintToken(
        ...
        if (_chainSymbolToId[chainSymbol] == _contractChainId) {
            ...
            if (price > 0) {
                processMintPayment(price, msg.sender, omniNft.getCreator());
            }
        ...
        if (mintPrice > 0) {
            processMintPayment(mintPrice, msg.sender, creator);
        }
        uint16 v = 1;
        uint gas = 4900000;
        bytes memory adapterParams = abi.encodePacked(v, gas);
        (uint messageFee,) = lzEndpoint.estimateFees(mappedChainId, address(this), payload, false, adapterParams);
        require(msg.value >= messageFee, "sent gas < messageFee");

        lzEndpoint.send{value : msg.value}(
            mappedChainId,
            this.getTrustedRemote(mappedChainId), // destination address of OmniMint LzReceiver Contract
            payload,
            payable(msg.sender),
            address(0x0),
            adapterParams
    ...
    function _nonblockingLzReceive(
        ...
        require(this.isTrustedRemote(_srcChainId, _srcAddress), "not allowed with setTrustedRemote()");
    ...
    function processMintPayment(uint256 price, address spender, address creator) internal {
        uint256 usdcAllowance = _baseAsset.allowance(spender, address(this));
        require(usdcAllowance >= price, "Insufficient allowance");
        _baseAsset.transferFrom(spender, creator, price * 98 / 100);
        _baseAsset.transferFrom(spender, _treasury, price * 2 / 100);
    }
    ...
    function estimateFees(
        ...
        bytes memory payload = abi.encode(contractAddress, tokenURI, destinationAddress, mintPrice, creator);
        uint16 mappedChainId = uint16(_chainSymbolToId[chainSymbol]);
        uint16 v = 1;
        uint gas = 5000000;
        bytes memory adapterParams = abi.encodePacked(v, gas);
