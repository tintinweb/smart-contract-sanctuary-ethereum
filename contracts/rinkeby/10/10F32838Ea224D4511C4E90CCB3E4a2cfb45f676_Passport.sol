    ...
    ERC2771Recipient
{
    ...
    uint256 public limitPerWallet; // 0 is no limit
    uint256 public claimFee; // 0 is no fee

    event BaseUriUpdated(string uri);
    event LimitPerWalletUpdated(uint256 limitPerWallet);
    ...
    function claim() external payable returns (uint256 tokenId) {
        ...
        if (claimFee > 0) require(msg.value == claimFee, "eth amount incorrect");
    ...
    function setClaimFee(uint256 _claimFee) external onlyRole(MANAGER_ROLE) {
        claimFee = _claimFee;
    }
    ...
    function setLimitPerWallet(uint256 _limitPerWallet) external onlyRole(MANAGER_ROLE) {
        limitPerWallet = _limitPerWallet;

        emit LimitPerWalletUpdated(limitPerWallet);
    }
    ...
    function _mint(address to) internal returns (uint256) {
        ...
        if (limitPerWallet > 0) require(this.balanceOf(to) < limitPerWallet, "limit per wallet reached");
...
interface IPassageRegistry {
    ...
    event LoyaltyImplementationUpdated(uint256 version, address implementation);
    ...
    event PassportImplementationUpdated(uint256 version, address implementation);
    ...
    function _loyaltyImplementations(uint256) external view returns (address);

    function _passportImplementations(uint256) external view returns (address);
    ...
    function loyaltyLedgers(address) external view returns (bool managed);

    function passportLatestVersion() external view returns (uint256);

    function passports(address) external view returns (bool managed);
    ...
    function updateLoyaltyImplementation(uint256 version, address implementation) external;

    function updatePassportImplementation(uint256 version, address implementation) external;
