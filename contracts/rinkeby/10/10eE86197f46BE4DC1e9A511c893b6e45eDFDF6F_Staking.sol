...
contract Staking is Ownable {
    ...
    function setMythApr(uint[] memory _APR) public onlyOwner {
        APR=_APR;
    }
    
     function setMythPool(uint[] memory _poolValue) public onlyOwner {
        pools = _poolValue;
    }
    ...
    function getReward(address account,uint id) public view returns (uint256) {
         ...
         uint totalDays;
         uint finalAmount;
        if(Stakers[account][id].poolType ==poolTypes[Stakers[msg.sender][id].poolType]){
    ...
    function stake(uint256 poolType) public {
        ...
        if(poolType==poolTypes[poolType]){
            require(IkonStakersRecord<=2,"No More Ikon Stake");
            if(poolType==3){
               IkonStakersRecord+=1; 
            }
