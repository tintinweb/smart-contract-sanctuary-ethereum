...
contract BookNFT1155 is ERC1155, EIP712, Ownable, AccessControl, Voucher, ERC2981PerTokenRoyalties {
    ...
    function redeemToken(address redeemer, NFTVoucherERC1155 calldata voucher) public payable returns (uint256) {
        ...
        _mint(signer, voucher.tokenId, voucher.quantity, "");
        ...
        safeTransferFrom(signer, redeemer, voucher.tokenId, voucher.quantity, "");

        // send funds to the creator and platform owner
        _sendFunds(signer, msg.value);

        // save royalties
        _saveRoyalties(voucher.tokenId, signer, voucher.royaltyBasisPoints);

        emit TokenMintedAndSold(voucher.tokenId, signer, redeemer, msg.value);

        tokenSupply[voucher.tokenId] = voucher.quantity;
...
contract Voucher is EIP712 {
    ...
    function _verify(NFTVoucherERC1155 calldata voucher) internal view returns (address) {
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
            keccak256("NFTVoucherERC1155(uint256 tokenId,uint256 sellingPrice,uint256 quantity,uint16 royaltyBasisPoints,string tokenUri)"),
            voucher.tokenId,
