...
contract RMDtest is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x2f52b09bD82c0DDB28D7A1c3AEA12B7FAfefA761); // Marketing Address
    ...
    string private _name = "RMDEngine";
    ...
    uint256 private buyBackUpperLimit = 5 * 10**17;
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        /* Add liquidity */
        swapAndLiquify((contractTokenBalance*liquidityPercent)/_liquidityFee);
        /* Swapping Tokens for Marketing Funds */
        swapTokensForBNB(contractTokenBalance.sub((contractTokenBalance*liquidityPercent)/_liquidityFee));
        uint256 transferredBalance = address(this).balance.sub(initialBalance);

        /* Send to Marketing address */
        transferToAddressBNB(marketingAddress, transferredBalance.mul(marketingPercent).div(_liquidityFee.sub(liquidityPercent)));
    ...
    function changePercentage(uint256 _liquidityPercent,uint256 _buybackPercent,uint256 _marketingPercent) public onlyOwner {
    ...
    function transferToAddressBNB(address payable recipient, uint256 amount) public onlyOwner {
        recipient.transfer(amount);
    }
