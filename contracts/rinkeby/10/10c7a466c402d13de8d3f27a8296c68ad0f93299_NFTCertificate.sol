...
contract NFTCertificate is Initializable{
    ...
    function userTransfer(uint256 id, address from, address to) private {
        InterfaceNFTToken(NFTTokenAddress).removeCertificateList(from, InterfaceNFTToken(NFTTokenAddress).findCertificateIndex(from, id));
        if (InterfaceNFTToken(NFTTokenAddress).findCertificateIndex(to, id) < int(0)) {
            InterfaceNFTToken(NFTTokenAddress).addUserCertificate(to, id);
        }
    ...
    function issueCertificate(address _receiverAddress, Certificate memory _certificate, string memory _uri) public  {
        ...
        InterfaceNFTToken(NFTTokenAddress).addUserCertificate(_receiverAddress, _certificate.tokenId);
    ...
    function voidCertificate(address _target, string memory _id, uint256 _amount, string memory _reason) public  {
        ...
        InterfaceNFTToken(NFTTokenAddress).removeCertificateList(_target, InterfaceNFTToken(NFTTokenAddress).findCertificateIndex(_target, mapId[_id]));
...
interface InterfaceNFTToken {
    ...
    //badge
    function addUserBadge(address _user, uint256 _tokenId) external;
    function findBadgeIndex(address _receiver, uint256 _tokenId) external  view returns (int);
    function removeBadgeList(address _receiver, int _index) external;
    //certificate
    function addUserCertificate(address _user, uint256 _tokenId) external;
    function findCertificateIndex(address _receiver, uint256 _tokenId) external view returns (int);
    function removeCertificateList(address _receiver, int _index) external;
...
contract NFTToken is Initializable, ERC1155Upgradeable, ERC1155BurnableUpgradeable, AccessControlUpgradeable, InterfaceNFTToken {
    ...
    function addUserBadge(address _user, uint256 _tokenId) external override {
    ...
    function removeBadgeList(address _receiver, int _index) external override  {
    ...
    function addUserCertificate(address _user, uint256 _tokenId) external override {
    ...
    function removeCertificateList(address _receiver, int _index) external override  {
        require(_index >= 0, "not found index");
