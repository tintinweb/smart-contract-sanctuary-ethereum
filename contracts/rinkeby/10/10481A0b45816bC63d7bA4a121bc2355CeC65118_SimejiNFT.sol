...
contract SimejiNFT is ERC721Common, SimejiSeller {
    ...
    function whitelistBuy(uint256 requested, bytes32[] calldata signature_) external {
        require(verify(_msgSender(), signature_), "caller is not in whitelist");
        SimejiSeller._purchase(msg.sender, requested);
    }
...
abstract contract SimejiSeller is OwnerPausable, ReentrancyGuard {
    ...
    function setSellerConfig(SellerConfig memory config) public onlyOwner {
        require(
            ...
            "SimejiSeller: excessive free quota"
        );
    ...
    whenNotPaused
    {
        ...
        _handlePurchase(_msgSender(), remain);

        _totalSold.add(remain);
        reserve.add(remain);
    }
        ...
        whenNotPaused
    {
        ...
        require(config.startPoint != 0 && block.timestamp >= config.startPoint, "SimejiSeller: Not started");
        ...
        require(n > 0, "SimejiSeller: Sold out");
        require(tx.origin == _msgSender(), "SimejiSeller: Sender limit");
