...
contract WorkspaceV1 is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable, WorkspaceStorageV1{
    ...
    event SpaceCreated(address indexed _owner, uint256 _spaceID);
    ...
    function checkIn(uint256 _spaceID) external payable returns(bool){
       ...
       // require(users[msg.sender].hold == false,"Plz cancel your hold");
        ...
        require(msg.value == ((space.hourlyRate)*8), "Please send accurate Token");
        ...
        space.checkInUsers.push(msg.sender);
        space.userIndex[msg.sender] = space.checkInUsers.length - 1;
    ...
    function checkOut(uint256 _spaceID) external returns(bool){
        ...
        Space storage space = spaces[_spaceID]; 
        require(user.spaceID == space.spaceID);
        ...
        space.numActiveCheckIns -= user.numGuest + 1;
        uint index = space.userIndex[msg.sender];
        uint lastIndex = space.checkInUsers.length - 1;
        address lastCheckIn = space.checkInUsers[lastIndex];
        space.userIndex[lastCheckIn] = index;
        space.checkInUsers[index] = lastCheckIn;
        space.checkInUsers.pop();
        delete users[msg.sender];
        spaces[_spaceID].available = true;
       // emit TokenReturned(msg.sender , returnValue); 
        return true;
    }

    function activeCheckIns(uint _spaceID) external view returns(address[] memory){
       Space storage space = spaces[_spaceID]; 
       return space.checkInUsers;
    }
