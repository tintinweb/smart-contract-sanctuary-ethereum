    ...
    GovernorTimelockControl
{
    ...
    event GCProposae(bool status,string message, address account);
        ...
        GovernorTimelockControl(_timelock)
    {
        ...
        proposals = Proposals(proposalContract);
    }
    ...
    function castVoteWithReason(
        ...
        returns (uint256)
    {
        bytes32 group = proposals.getSingleProposalGroup(proposalId);
        address owner = proposals.getSingleProposalOwner(proposalId);
...
contract Proposals {
    ...
    struct Proposal {
        uint256 proposalID;
        string hash;
        uint8 typ; // 0 - blog, 1 - integration
        address owner;
        bytes32 groups;
    }
    ...
    //add single proposal
    function addProposal(uint256 proposalID,string memory _hash,uint8 _typ, address _owner, bytes32 _group)
    public
    onlyMember
    returns (bool) {
        if(!proposalsMap[proposalID]){
            proposals.push(Proposal(proposalID, _hash,_typ,_owner,_group));
    ...
    //change hash of single proposal
    ...
    public
    view
    returns (uint256, string memory,uint8,address,bytes32) {
        require(proposalsMap[proposalID],"Proposal not exist");
        for(uint i=0;i<proposals.length;i++){
            if(proposals[i].proposalID == proposalID)
            {
                return (proposals[i].proposalID, proposals[i].hash, proposals[i].typ, proposals[i].owner,proposals[i].groups) ;
            }
        }
    }
