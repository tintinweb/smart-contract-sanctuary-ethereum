...
contract CoreCollection is ERC721Enumerable, Ownable {
    ...
    ICoreMembership public membership;
    ...
    modifier onlyProducer() {
        require(
            membership.isProducer(msg.sender),
            "CoreCollection: Only Producer can operate"
        );
        _;
    }

    modifier onlyMember(uint256 mints) {
        require(
            membership.getUserAvailableMints(msg.sender) >= mints,
            "CoreCollection: Not available mint freely"
        );
        _;
    }
    ...
    function mint() private returns (uint256) {
        ...
        membership.decreaseUserAvailableMints(msg.sender);
    ...
    function initialize(
        ...
        membership = ICoreMembership(_coreMembership);

        initialized = true;
    }

    /**
     * @dev free mint for members
    ...
    function freeMint()
        ...
        returns (uint256)
    {
        require(totalSupply() < maxSupply, "CoreCollection: Over Max Supply");
        return mint();
    }

    /**
     * @dev free batch mint for members
     */
    function batchFreeMints(uint256 _mints)
        external
        onlyInitialized
        onlyMember(_mints)
    {
    ...
    /**
     * @dev paid mint for public users
     * @return { uint256 } minted token id
     */
    function paidMint() external onlyInitialized returns (uint256) {
        require(
            totalSupply() < getMaxSupply(),
            "CoreCollection: Over Max Supply"
        );

        collateral.transferFrom(msg.sender, address(this), mintFee);
        return mint();
    }
    ...
    /**
     * @dev get max supply for public users
     * @return { uint256 } un-minted token amount
     */
    function getMaxSupply() public view returns (uint256) {
        return
            maxSupply -
            uint256(membership.producerAvailableMints()) *
            uint256(membership.producers()) -
            uint256(membership.collaboratorAvailableMints()) *
            uint256(membership.collaborators());
    }
}
