...
contract LotteryC is VRFConsumerBaseV2, Ownable {
    // Lottery contract as a simple VRF consumer. This contract must be
    ...
    constructor(address _priceFeedAddress,
                ...
                address _link,
                uint64 _subID,
                bytes32 _keyHash) payable VRFConsumerBaseV2(_vrfCoordinator) {
    ...
    function enter() public payable {
        ...
        require(msg.value >= getEntranceFee(),
                'Insufficient amount to cover the entrance fee!');
        players.push(msg.sender);
    }

    function startLottery() public onlyOwner {
        require(lottery_state == LOTTERY_STATE.CLOSED,
                'Lottery still running, cannot start a new one at this time!');
    ...
    function fulfillRandomWords(uint256, /* requestId */
        ...
        require(lottery_state == LOTTERY_STATE.CALCULATING_WINNER,
                'Wrong timing!');
        require(randomWords.length > 0, 'Random number has not arrived yet...');
        s_randomWords = randomWords[0] % players.length;
        recentWinner = payable(players[s_randomWords]);
    ...
    function getEntranceFee() public view returns(uint256) {
        ...
        return (usdEntryFee * precision)/getPrice();
    }
