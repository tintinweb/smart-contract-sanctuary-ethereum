  ...
  PausableUpgradeable
{
    ...
    for (i = 0; i < tokenIds.length; i++) {
      ...
      reward +=
        _getStakingRate(
          tokenId,
          ranks[i],
          multiplerCheck == 22 ? multiplierRate : 1
        ) *
        isDeposited *
  ...
  function determineCubeType(uint256 tokenId) internal pure returns (uint256) {
    if(tokenId <= 1414) {
      return 1;
    } else if (tokenId <= 2828) {
      return 2;
    } else {
      return 3;
    }
  ...
  function _getStakingRate(
    ...
    return rate * (1 + _getWeight(_tokenId, _rank)) * extraRate;
  }
