...
contract AlkebulanTribeCollection is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) callerIsUser{
    ...
    //require(whitelistClaimed[_msgSender()] + _mintAmount <= 5, 'Max number of Tokens allowed Exceeded. Kindly reduce the amount of tokens to be acquired!');
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), 'Invalid proof!');

    whitelistClaimed[_msgSender()] = whitelistClaimed[_msgSender()] + _mintAmount;
    whitelistClaimed2[_msgSender()] = whitelistClaimed[_msgSender()] <= 5;
    whitelistClaimed3[_msgSender()] = whitelistClaimed[_msgSender()] > 5;
