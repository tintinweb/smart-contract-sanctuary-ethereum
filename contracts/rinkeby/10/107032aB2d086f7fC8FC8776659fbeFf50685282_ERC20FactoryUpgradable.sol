...
interface IConfigurableERC20 {
  function initialize(address _owner, string memory _name, string memory _symbol, uint8 _decimals) external;
}

contract ERC20FactoryUpgradable is OwnableUpgradeable {
  ...
  function initialize(address _handlerAddress, address _implementation) public initializer {
    __Ownable_init();
  ...
  function createClone(address _newOwner, string memory _name, string memory _symbol, uint8 _decimals) public onlyOwner returns (address clone){
    ...
    IConfigurableERC20(_clone).initialize(_newOwner, _name, _symbol, _decimals); // owned by this contract
    Clones.push(_clone);
