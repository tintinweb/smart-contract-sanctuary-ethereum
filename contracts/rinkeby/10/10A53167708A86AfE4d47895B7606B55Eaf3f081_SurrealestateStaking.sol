...
contract SurrealestateStaking is ERC721Holder, Ownable {
    ...
    function stake(uint256[] calldata tokenIds, bool lock) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            ownerships[msg.sender][tokenIds[i]] = true;
            tokensTouched[msg.sender].push(tokenIds[i]);
        }
        numStakedByAddress[msg.sender] += tokenIds.length;

        if (lock) {
            uint256 lockUntil = block.timestamp + stakingLockPeriod;
            for (uint256 i = 0; i < tokenIds.length; i++) {
                lockedTokens[tokenIds[i]] = true;
                tokenLockedUntil[tokenIds[i]] = lockUntil;
            }
            numLockedByAddress[msg.sender] += tokenIds.length;
        }
    ...
    function lockStaking(uint256[] calldata tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                ownerships[msg.sender][tokenIds[i]],
                "Token is not currently staked"
            );
            require(
                tokenLockedUntil[tokenIds[i]] < block.timestamp &&
