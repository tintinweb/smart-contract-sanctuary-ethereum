...
contract BorrowerOps is  OrumBase, Ownable, CheckContract, IBorrowerOps {
    ...
    function openVault(uint _maxFeePercentage, uint _USDCAmount, uint _colloETHamount , address _upperHint, address _lowerHint) external payable override {
        require(stakingPool.allowBorrow(), "BorrowerOps: utilisationRatio >= 90 or BufferPool dont have Enough USDC");
        require(_colloETHamount <= oethToken.balanceOf(msg.sender) , "BorrowerOps: insufficient oETH balance");
        require(stakingPool.getUSDCDeposits() >= _USDCAmount + (USDC_GAS_COMPENSATION), "BorrowerOps: SP balance is lower than requested borrow amount");
    ...
    function moveoETHGainToVaultFromStakingPool(address _borrower, uint depositorROSEGain, address _upperHint, address _lowerHint) external payable override {
    ...
    function withdrawUSDC(uint _maxFeePercentage, uint _USDCAmount, address _upperHint, address _lowerHint) external override {
        require(stakingPool.allowBorrow(), "BorrowerOps: utilisationRatio >= 90");
    ...
    */
    ...
    function _withdrawUSDC(IActivePool _activePool, IUSDCToken _usdcToken, address _account, uint _USDCAmount, uint _netDebtIncrease) internal {
        // IStakingPool stakingPoolChached = stakingPool;
        _activePool.increaseUSDCDebt(_netDebtIncrease);
        // _usdcToken.mint(_account, _USDCAmount);
        stakingPool.sendUSDCtoBorrower(_account, _USDCAmount);
    }
    ...
    function _repayUSDC(IActivePool _activePool, IUSDCToken _usdcToken, address _account, uint _USDC) internal {
        ...
        stakingPool.decreaseLentAmount(_USDC);
    }
    ...
    function _requireCallerIsStakingPool() internal view {
        require(msg.sender == stakingPoolAddress, "BorrowerOps: Caller is not Stability Pool");
    }
...
interface IBorrowerOps {
    ...
    function moveoETHGainToVaultFromStakingPool(address _borrower, uint depositorROSEGain, address _upperHint, address _lowerHint) external payable;
