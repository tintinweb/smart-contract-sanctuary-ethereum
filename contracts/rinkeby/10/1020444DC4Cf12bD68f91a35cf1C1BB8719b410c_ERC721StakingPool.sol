...
contract ERC721StakingPool is LockCycle, BaseStakingPool, ERC721Holder
{
	...
	function deposit(uint256[] calldata _tokenIdList, bool _claimRewards) external nonReentrant
	{
		if (_claimRewards) {
	...
	function withdraw(uint256[] calldata _tokenIdList, bool _claimRewards) external nonReentrant
	{
		if (_claimRewards) {
			_harvestAll(msg.sender);
		}
...
abstract contract BaseStakingPool is Ownable, ReentrancyGuard, Initializable
{
	...
	{
		_updatePool();
		uint256 _amount = freeBalance(_token);
