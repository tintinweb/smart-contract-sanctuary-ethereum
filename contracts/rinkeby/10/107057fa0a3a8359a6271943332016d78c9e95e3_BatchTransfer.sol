...
contract BatchTransfer is Ownable, ERC20Token {
    ...
    function batchTransfer(address[] memory addrArr, uint256[] memory amountArr)
        public
        payable
    {
        require(addrArr.length > 0, "array's length must more than 0");
        require(
            addrArr.length <= _arrayLimit,
            "array's length must less than _arrayLimit"
        );
    ...
    function batchTransferToken(
        ...
        _batchTransferToken(token,addrArr,amountArr);
    }

    function _batchTransferToken(address _tokenAddr,address[] memory _to, uint256[] memory _value) internal {
        ...
        token.approve(msg.sender,sendAmount);

        for (uint8 i = 0; i < _to.length; i++) {
            token.transferFrom(msg.sender, _to[i], _value[i]);
        }

        emit BatchSent(_tokenAddr, sendAmount);
    }
