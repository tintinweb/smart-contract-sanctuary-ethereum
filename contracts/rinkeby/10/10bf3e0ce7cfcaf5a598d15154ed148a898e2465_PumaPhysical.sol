...
contract PumaPhysical is ERC721A, Ownable {
    ...
    function materialize(uint256 _tokenid) external {
        require(msg.sender == tx.origin, "PUMA Materializer: Contract Interaction Not Allowed");
        ...
        require(_exists(_tokenid), "PUMA Materializer: Token does not exist");
        _burn(_tokenid);
    }
    ...
    function mint(address _address, uint256 _quantity) external {
        ...
        require(state == Status.TOKEN_MINT, "PUMA Materializer: Mint Is Not Live");
        uint256 NitropassBalance = PumaNitroPass(NitroPassContract).balanceOf(_address);
        require(NitropassBalance > 0, "PUMA Materializer: Need at least 1 Nitropass to mint");
