...
contract ERC1155Lockable is
     ...
     */
    function mint(address to, uint256 id, uint256 amount, bytes calldata uri_, bytes calldata key, 
        string[] calldata cdn_, address royaltyRecipient, uint256 royaltyValue) external virtual {
        _mint(to, id, amount, "");
        if (uri_.length > 0) {
