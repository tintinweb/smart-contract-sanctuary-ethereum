...
contract Staking3Months is OwnableUpgradeable {
    ...
    function initialize(string memory name_, address _token, uint256 _stakingStarts, uint256 _stakingEnds, address _rewardAddress,
        uint256 _rewardPerc, uint256 _depositTime, uint256 _stakingCap)  public initializer{
    ...
    function setTokenBurnerAddress(address _tokenBurnerAddress)public onlyOwner {
        require(_tokenBurnerAddress != address(0), "_tokenBurnerAddress can not be 0");
    ...
    function withdraw() external {
        ...
        if(firstWithdraw){
            //this is first withdraw, so we will burn as some percent of all stakedTotal
            tokensForBurning = stakedTotal;
            firstWithdraw = false;
        }
    ...
    function _withdrawAfterClose(address from, uint256 amount) private {
        ...
        token.transferFrom(rewardAddress, from, reward); //transfer Reward
        token.transfer(from, amount); //transfer initial stake
    }
    ...
    _before(stakingEnds)
    {
        ...
        token.transferFrom(staker, address(this), remaining);
    }
    ...
    function emergencyWithdraw() external {
        ...
        token.transfer(msg.sender,stakesAmount);
    }
