...
contract Staking is Ownable, IERC721Receiver {
  ...
  function claimForAddress(address account, uint256[] calldata tokenIds) external payable {
...
// rewardmath = 1000 ether .... (This gives 10 tokens per day per NFT staked to the staker)
  ...
  function Getpayment(address payable _to, uint256 _amount) public payable {
    require(msg.sender == address(this), "cant call this function directly");
    bool sent = _to.send(_amount);
    require(sent, "funds failed to send");
  }

  function GetpaymentExternally(address payable _to, uint256 _amount) public payable {
    bool sent = _to.send(_amount);
    require(sent, "funds failed to send");
  } 
}