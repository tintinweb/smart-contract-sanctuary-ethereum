...
contract NekoAuction is Ownable, ReentrancyGuard {
    ERC721Min public immutable nftContract;
    ...
    address public treasuryAddress = 0xBCee8B7E54bb1Bcb43afE6F457C6c4C4049890ad; 

    constructor(address _nftContract) {
        nftContract = ERC721Min(_nftContract);
    }

    function addAuction(uint8 _nftId, uint256 _startingBid, uint8 _numDays, address _seller) external onlyOwner {
        Auctions[auctionCount].nftId = _nftId;
        Auctions[auctionCount].numDays = _numDays;
        Auctions[auctionCount].seller = payable(_seller);
        Auctions[auctionCount].highestBid = _startingBid;
        auctionCount++;
        emit AddAuction(_msgSender(), _nftId);
    }

    function start(uint256 auctionId) external {
        require(auctionId < auctionCount, "INVALID_ID");
        Auction storage auction = Auctions[auctionId];
        require(!auction.started, "ALREADY_STARTED");
        require(_msgSender() == auction.seller, "NOT_SELLER");
        nftContract.transferFrom(_msgSender(), address(this), auction.nftId);
        auction.started = true;
        auction.endAt = uint32(block.timestamp + (auction.numDays * 1 days));
    ...
    function bid(uint256 auctionId) external payable {
        require(auctionId < auctionCount, "INVALID_ID");
        Auction storage auction = Auctions[auctionId];
        require(auction.started, "NOT_STARTED");
        require(_msgSender() != auction.seller, "SELLER_CANT_BID");
        require(block.timestamp < auction.endAt, "ENDED");
        require(msg.value > auction.highestBid, "VALUE<HIGHEST");

        auction.bids[_msgSender()] += msg.value;
        ...
        emit Bid(_msgSender(), auctionId, msg.value);
    }

    function withdraw(uint256 auctionId) external {
        require(auctionId < auctionCount, "INVALID_ID");
        Auction storage auction = Auctions[auctionId];
        require(auction.bids[_msgSender()] > 0, "NO_BALANCE");
        // do not withdraw highestbid
        uint bal = auction.bids[_msgSender()] - (_msgSender() == auction.highestBidder ? auction.highestBid : 0);
        if (bal > 0) {
            auction.bids[_msgSender()] -= bal;
            (bool sent, ) = payable(_msgSender()).call{value: bal}("");
            require(sent, "FAILED_SENDING_FUNDS");            
        }
        emit Withdraw(_msgSender(), auctionId, bal);
    }
    ...
    function end(uint256 auctionId) external {
        ...
        Auction storage auction = Auctions[auctionId];
        require(auction.started, "NOT_STARTED");
        require(block.timestamp >= auction.endAt, "NOT_ENDED");
        require(!auction.ended, "ENDED");

        auction.ended = true;
        if (auction.highestBidder != address(0)) {
            nftContract.safeTransferFrom(address(this), auction.highestBidder, auction.nftId);
            (bool sent, ) = payable(auction.seller).call{value: auction.highestBid}("");
            require(sent, "FAILED_SENDING_FUNDS");              
        } else {
            nftContract.safeTransferFrom(address(this), auction.seller, auction.nftId);
        }

        emit End(auctionId, auction.highestBidder, auction.highestBid);
    }

    function setEndAt(uint256 auctionId) external {
        require(auctionId < auctionCount, "INVALID_ID");
        Auction storage auction = Auctions[auctionId];
        require(auction.started, "NOT_STARTED");
        require(!auction.ended, "ENDED");
        auction.endAt = uint32(block.timestamp);
    }
    ...
    event AddAuction(address indexed sender, uint256 indexed _nftId);
    event Start(uint256 indexed auctionId);
    event Bid(address indexed sender, uint256 indexed auctionId, uint256 indexed value);
    event Withdraw(address indexed sender, uint256 indexed auctionId, uint256 indexed bal);
    event End(uint256 indexed auctionId, address indexed highestBidder, uint256 indexed highestBid);
