...
contract Lottery is VRFConsumerBase, Ownable {
    ...
    constructor(
        ...
        address _link,
        int256 _randomNumberFee,
        bytes32 _keyHash
    ) public VRFConsumerBase(_vrfCoordinator, _link) {
        usdEntryFee = 50 * (10**18);
        ethUsdPriceFeed = AggregatorV3Interface(_priceFeedAddress);
        lotteryState = LotteryState.CLOSED;
        randomNumberFee = _randomNumberFee;
        keyhash = _keyHash;
    }

    function enter() public payable {
        require(lotteryState == LotteryState.OPEN);
        require(int256(msg.value) >= getEntranceFee(), "Not enough ETH");
        players.push(msg.sender);
    }

    function getEntranceFee() public view returns (int256) {
        (, int256 price, , , ) = ethUsdPriceFeed.latestRoundData();
        int256 adjustedPrice = price * 10**10; // 18 decimals
