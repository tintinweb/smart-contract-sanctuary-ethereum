...
contract IuvoDao is Ownable, VRFConsumerBaseV2 {
  ...
  uint256 public percentTreasuryBuyerPool = 30; // 3%
  ...
  event SelectedEpochWinner(uint256 indexed requestId, uint256 indexed epoch);
  ...
  function getAllCurrentCharities() external view returns (address[] memory) {
    return charities[lastCharityChange];
  }
  ...
  function getEpochBuyerWinners(uint256 _epoch)
    external
    view
    returns (address[] memory)
  {
    return _vrfWinners[_epoch];
  }
    ...
    override
  {
    ...
    for (uint256 i = 0; i < randomWords.length; i++) {
      ...
      _vrfWinners[_epoch].push(voterToken.epochBuyers(_epoch, _winnerIdx));
      payable(_vrfWinners[_epoch][i]).call{ value: _amountETHPerWinner }('');
    }
    require(address(this).balance >= _balanceBefore - _amountETHTotal);
    emit SelectedEpochWinner(requestId, _epoch);
  }
  ...
  {
    return
      charitiesIndexed[lastCharityChange][_charity] ? _charity : address(0);
  }
  ...
  function setNumBuyerWinners(uint16 _winners) external onlyAuthorized {
    require(_winners <= 10, 'no more than 10 winners at a time');
    numBuyerWinners = _winners;
  }
