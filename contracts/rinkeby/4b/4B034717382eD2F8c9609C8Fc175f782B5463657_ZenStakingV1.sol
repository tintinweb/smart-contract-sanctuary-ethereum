...
contract ZenStakingV1 {
    ...
    function getClaimableAmount(uint tokenId) external view returns (uint256 claimAmount){
        StakedToken memory token = stakedTokens[uint16(tokenId)];
        return _getClaimableAmount(token);
    }
