...
import "./IAvatar.sol";
import "./Authenticable.sol";
import "openzeppelin-contracts/utils/Address.sol";
import "openzeppelin-contracts/security/Pausable.sol";
import "openzeppelin-contracts/utils/cryptography/draft-EIP712.sol";

contract Avatar is IAvatar, ERC721, Ownable, Pausable, Authenticable {
    ...
    // Event for setting signature address:
    event SetSignatureAddress(address newSignatureAddress);

    // Event for setting withdraw address:
    ...
    // Mapping from token id to configuration bit mapping
    mapping(uint256 => uint256) private _tokenIdToConfiguration;

    // Mapping from configuration bit mapping to token id
    mapping(uint256 => uint256) private _configurationToTokenId;

    // Mapping from address to boolean indicating if avatar is reserved.
    mapping(address => bool) private _reservations;
    ...
    function mintAvatar(
        ...
        MintSignature memory mintSignature = MintSignature({
            sender: msg.sender,
            signatureExpiration: signatureExpiration,
            price: price,
            desiredConfiguration: desiredConfiguration
        });
        bytes32 digest = hashContent(mintSignature);

        require(
            verifySignature(signatureAddress, digest, signature),
    ...
    function reserveAvatar(
        ...
        ReservationSignature
            memory reservationSignature = ReservationSignature({
                sender: msg.sender,
                signatureExpiration: signatureExpiration,
                price: price
            });
        bytes32 digest = hashContent(reservationSignature);
    ...
    function configurationByToken(uint256 token) public view returns (uint256) {
...
abstract contract Authenticable {
    using ECDSA for bytes32;
    ...
    bytes32 constant RESERVATION_TYPEHASH =
        keccak256(
            "ReservationSignature(address sender, uint256 signatureExpiration, uint256 price)"
        );

    bytes32 constant MINT_TYPEHASH =
        keccak256(
            "MintSignature(address sender, uint256 signatureExpiration, uint256 price, uint256 desiredConfiguration)"
        );

    bytes32 DOMAIN_SEPARATOR;

    function hashContent(EIP712Domain memory eip712Domain)
        internal
        pure
        returns (bytes32)
    {
    ...
    function hashContent(ReservationSignature memory reserveSignature)
        internal
        pure
        returns (bytes32)
    {
        return
            keccak256(
                abi.encode(
                    ...
                    reserveSignature.sender,
                    reserveSignature.signatureExpiration,
                    reserveSignature.price
                )
            );
    }

    function hashContent(MintSignature memory mintSignature)
        internal
        pure
        returns (bytes32)
    {
        return
            keccak256(
                abi.encode(
                    MINT_TYPEHASH,
                    mintSignature.sender,
                    mintSignature.signatureExpiration,
                    mintSignature.price,
                    mintSignature.desiredConfiguration
                )
            );
    }
