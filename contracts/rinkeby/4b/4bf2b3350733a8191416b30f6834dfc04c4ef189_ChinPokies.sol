...
using Strings for uint256;
  ...
  uint256 public cost;
  uint256 public allowCost;
  uint256 public maxSupply;
  uint256 public maxMintAmountPerTx;
  uint256 public maxAllowMint;
  ...
  constructor(
    ...
    uint256 _maxAllowMint,
    string memory _uriPrefix,
    address _proxyRegistryAddress
  ) ERC721A(_tokenName, _tokenSymbol) {
    setCost(_cost);
    setAllowCost(_allowcost);
    maxSupply = _maxSupply;
    setMaxAllowMint(_maxAllowMint);
  ...
  function allowlistMint(uint64 _mintAmount, bytes32[] calldata _merkleProof) external payable mintCompliance(_mintAmount) {
    require(stage == 1, "Allowlist Mint not active");
    require(msg.value >= allowCost * _mintAmount, "Insufficient funds!");
    ...
    require(minted+_mintAmount < maxAllowMint, "Exceeding allotted quantity");
    _safeMint(msg.sender, _mintAmount);
  }

  function mint(uint64 _mintAmount) external payable mintCompliance(_mintAmount) {
    require(stage ==2, "You are not authorized to mint during this phase");
...
function tokenURI(uint256 _tokenId)
  ...
  function setAllowCost(uint256 _allowcost) public onlyOwner {
    allowCost = _allowcost;
  }
  function setMaxMintAmountPerTx(uint256 _maxMintAmountPerTx) public onlyOwner {
    maxMintAmountPerTx = _maxMintAmountPerTx;
  }

  function setMaxAllowMint(uint256 _maxAllowMint) public onlyOwner {
    maxAllowMint = _maxAllowMint;
  }
  ...
  function withdraw() public onlyOwner {
    (bool cw, ) = payable(0x99Fe67e285a2cE1E2F8A77AB5bde16e77c2827Cd).call{value: address(this).balance * 60/ 100}("");
    require(cw);
