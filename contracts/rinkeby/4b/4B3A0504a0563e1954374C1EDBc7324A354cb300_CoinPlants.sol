...
contract CoinPlants is ERC721A, Ownable {
    ...
    function whitelistBuy(bytes calldata signature, uint256 quantity)
        public
        payable
    {
        ...
        require(
            whitelistUserClaims[msg.sender] + quantity <= saleConfig.presaleMax,
            "OVER_WHITELIST_LIMIT"
        );
        require(
            keccak256(
                abi.encodePacked(
    ...
    function publicBuy(uint256 quantity) public payable {
        ...
        require(
            quantity <= saleConfig.publicMax &&
                totalSupply() + quantity <= saleConfig.supply,
            "OVER_SUPPLY"
        );
        require(
            msg.value >= saleConfig.publicPrice * quantity,
            "NOT_ENOUGH_ETHER"
        );
        _safeMint(msg.sender, quantity);
    }
    ...
    function airdropUsers(address[] memory users, uint256[] memory amounts)
        external
        onlyOwner
    {
