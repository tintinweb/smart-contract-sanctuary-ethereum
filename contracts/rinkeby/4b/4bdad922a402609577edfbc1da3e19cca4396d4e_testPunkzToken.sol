...
contract testPunkzToken is ERC20, Ownable{
    ...
    //@Times
    uint public punkXMintingTime = 1644475967;
    uint tendayz = 1 minutes;
    uint tenseconds = 10;
    ...
    function lockInternal(uint[] memory tokenIds) internal {
        for(uint i=0;i<tokenIds.length;i++){
            ...
            require(block.timestamp - currentToken.lockTime > tendayz,"Already locked"); //Need to not be locked to lock
            currentToken.lockTime = block.timestamp;
            console.log("Here stored:-", currentToken.lockTime );
            tokenIds.length > 6 ? currentToken.lockedPieces = 6 : currentToken.lockedPieces = tokenIds.length ;
        }
    }

    //@notice This function is used to unstake the staked tokens. PS: tokens which are locked cannot be unstaked within tendayz of locking period
    ...
    function unstakeTokens(uint[] memory tokenIds) external {
        ...
        for (uint i; i< tokenIds.length; i++) {
            ...
            require(block.timestamp - currentToken.lockTime > tendayz,"Not unlocked"); //Needs to be unlocked
    ...
    function rewardFromMint(uint[] memory tokenIds) external{
        uint totalAmount;
        for(uint i=0;i<tokenIds.length;i++){
            require(tokenIds[i] < 2,"Reward only till 2");
    ...
    function getReward(uint tokenId) public view returns(uint){
        ...
        uint reward;
        if(block.timestamp - currentToken.lockTime <= tendayz){
            //tokenId still in lock period
            uint timeInDays = (block.timestamp - currentToken.lastClaim)/tenseconds;
            uint combo = currentToken.lockedPieces;
            reward = 1 ether*timeInDays*rewards[tokenId/2]*(10+multiplier[combo])/tenseconds - externallyClaimed[tokenId];
        }
        else{
            uint unlockTime = currentToken.lockTime + tendayz;
            if(block.timestamp > unlockTime && currentToken.lastClaim < unlockTime){
                //last claim before lock end but current time after
                uint timeLocked = (currentToken.lockTime + tendayz - currentToken.lastClaim)/tenseconds;
                uint timeUnlocked = (block.timestamp - (currentToken.lockTime + tendayz))/tenseconds;
                uint combo = currentToken.lockedPieces;
                reward = 1 ether*timeLocked*rewards[tokenId/2]*(10+multiplier[combo])/ tenseconds;
                reward += 1 ether*timeUnlocked*rewards[tokenId/2] - externallyClaimed[tokenId];
            }
            else{
