...
contract GoonzSchoolPhotos is ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function initializeToOwners(uint256 start_, uint256 end_) external {
        for (uint256 i = start_; i <= end_; i++) {
            emit Transfer(address(0), ownerOf(i), i);
        }
    }

    function initializeBulk(uint256 start_, uint256 end_) external {
        emit ConsecutiveTransfer(start_, end_, address(0), address(this));
    }
    
    function initializeBulkTo(uint256 start_, uint256 end_, address to_) external  {
