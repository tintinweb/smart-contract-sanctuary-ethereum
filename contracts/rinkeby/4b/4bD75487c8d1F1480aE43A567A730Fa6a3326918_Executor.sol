...
contract Executor is LiquityMath{
	...
	function adjustCreditLineLiquity(
		...
		// Assertions and relevant reverts are done within Liquity protocol
		// TODO Re-entrancy mitigation if needed
		...
		if (msg.value > 0) {
			...
			// Must be last to avoid re-entrancy attack
			// TODO consider consequences of re-entrancy attack
			// solhint-disable-next-line avoid-low-level-calls
	...
	/// @notice Enables ANY ADDRESS (calling and repaying) to partially repay the debt by the given amount of LUSD.
	...
	///		Having the impact on the Smart Account storage. Therefore no 3rd party contract besides Liquity is called.
	/* solhint-disable-next-line var-name-mixedcase */	
	function repayLUSDLiquity(uint256 _LUSDRequestedChange, address _LUSDFrom, address _upperHint, address _lowerHint, address _caller) external onlyProxy {
		// Debt decrease
		// Liquity requires to have LUSD on the msg.sender, i.e. on Smart Account proxy
