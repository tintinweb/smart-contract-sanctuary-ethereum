...
contract Drip2 is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function updateSupply(uint256 _redeemTime, uint256 _price) external onlyOwner {
        require(supply+3<= 12, "Total Supply Reached");
        drop += 1;
    ...
    // function updateParams(uint256 _drop, uint256 _redeemTime) external onlyOwner {
    ...
    function redeem(uint256 tokenId) external {
        ...
        if(tokenId<=3){
            ...
            require(block.timestamp<=redeemTime[uint256((tokenId-4)/3) + 1], "Redeem time finished");
            redeemed[tokenId] = msg.sender;
        }
    }
