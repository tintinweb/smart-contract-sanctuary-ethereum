...
contract Trees is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public price = 0.001 ether; // TEST VALUE
    ...
    {
        require(quantity <= batchLimit, "BATCH LIMIT: Quantity too high");
        require(isActive, "Contract paused");
        require(
            minted[msg.sender] + quantity <= maxMint,
            "MAX_MINT: Amount too high"
        );
    ...
    function setTeam(address _user, bool _state) external onlyOwner {
        require(team[_user] != _state, "Already set");
        team[_user] = _state;
    }

    function setMaxSupply(uint256 _amount) external onlyOwner {
        require(_amount >= totalSupply(), "Max supply too low");
        maxSupply = _amount;
    }
    ...
    function setBatchLimit(uint256 _amount) external onlyOwner {
        require(_amount > 0, "Limit can't be zero");
        batchLimit = _amount;
    }
    ...
    modifier verifyPrice(uint256 _quantity, uint256 _price) {
        require(msg.value >= _quantity * _price, "PRICE: Value too low");
        _;
    }
    ...
    modifier supplyLimit(uint256 _quantity) {
        require(
            ...
            "MAX_SUPPLY: Amount too high"
        );
        _;
    }

    modifier onlyTeam() {
        require(team[msg.sender], "Not team member");
        _;
    }
}
