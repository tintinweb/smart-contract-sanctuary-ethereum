...
contract TESTERC721A is Ownable, ERC721A, PaymentSplitter {
    ...
    uint256 public ogSalePrice = 0.007 ether;
    uint256 public wlSalePrice = 0.010 ether;
    uint256 public publicSalePrice = 0.013 ether;

    bytes32 public ogMerkleRoot;
    bytes32 public wlMerkleRoot;

    mapping(address => uint256) public amountNFTsPerWalletSale;
    uint256 public maxAmountMint = 10;
    ...
    ) ERC721A("TESTERC721A", "Test") PaymentSplitter(_team, _teamShares) {
    ...
    modifier callerIsUser() {
        require(
            ...
            "Error because the caller is another contract"
        );
        _;
    }

    function ogMint(uint256 _quantity, bytes32[] calldata _proof)
        external
        payable
        callerIsUser
    {
        uint256 price = ogSalePrice;
        require(price != 0, "Error because the price is 0");
        require(
            sellingStep == Step.OGSale,
            "Error because the OG Sale is not activated"
        );
        require(isOG(msg.sender, _proof), "Error because you are not an OG");
        require(
            amountNFTsPerWalletSale[msg.sender] + _quantity <= maxAmountMint,
            "Not possible to mint more of 10 NFT"
        );
        ...
        callerIsUser
    {
        ...
        require(price != 0, "Error because the price is 0");
        require(
            sellingStep == Step.WLSale,
            "Error because the Whitelist Sale is not activated"
        );
        require(
            isWhiteListed(msg.sender, _proof),
            "Error because the address is not whitelisted"
        );
        require(
            amountNFTsPerWalletSale[msg.sender] + _quantity <= MAX_SUPPLY,
            "Not possible to mint more of 1 NFT on the Whitelist Sale"
        );
        require(
            totalSupply() + _quantity <= MAX_SUPPLY,
            "Sold Out of the Whitelist Sale"
        );
        require(msg.value >= price * _quantity, "Not enough funds");
        amountNFTsPerWalletSale[msg.sender] += _quantity;
        _safeMint(msg.sender, _quantity);
    ...
    function isOG(address _account, bytes32[] calldata _proof)
        internal
        view
        returns (bool)
    {
