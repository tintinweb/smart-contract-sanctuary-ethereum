...
contract Minter is IMerkleMinter, Ownable {
    ...
    function hasClaimed(address user) public view override returns (uint8) {
        return claims[user];
    }

    function claim(bytes32[] calldata merkleProof) external override {
        require(!openMint && hasClaimed(msg.sender) < MAX_CLAIMS, 'Minter: Max minting reached');
...
interface IMerkleMinter {
    ...
    function hasClaimed(address user) external view returns (uint8);
