...
contract SportsFeedsAggregator is ChainlinkClient {
        ...
        returns (bool, GameCreate[] memory)
    {
        ...
        for (uint256 x = 0; x < games.length; x++) {
            rounds[_roundId].answer[x] = games[x];
        }
    ...
    // function getGamesCreated(bytes[] memory data)
    //     internal
    //     pure
    ...
    //         GameCreate memory game = abi.decode(data[i], (GameCreate));
    //         games[i] = game;
    //     }
    ...
    function triggerRound(bytes32 _requestId) internal {
        GameCreate[] memory g = getGamesCreated(_requestId);
        updateRoundAnswer(latestRoundId++, g);
    }
