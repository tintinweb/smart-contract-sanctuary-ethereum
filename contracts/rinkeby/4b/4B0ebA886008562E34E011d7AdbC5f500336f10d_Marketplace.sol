...
uint256 public sayed = 1;
    ...
    event NewBidOnAuction(uint256 index,address addressNFTCollection,uint256 auctionIndex, uint256 newBid,uint256 nftId);
    event NFTClaimed(     uint256 index,address addressNFTCollection,uint256 auctionIndex, uint256 nftId, address claimedBy);
    event TokensClaimed(     uint256 index,address addressNFTCollection,uint256 auctionIndex, uint256 nftId, address claimedBy);
    event Nftselltotopbidder(     uint256 index,address addressNFTCollection,uint256 auctionIndex, uint256 nftId, address claimedBy,uint256 price,bool sold);
    event NFTRefunded(     uint256 index,address addressNFTCollection,uint256 auctionIndex, uint256 nftId, address claimedBy);
    event BuyPriceSell(     uint256 index,address addressNFTCollection,address buyer,address seller,uint256 auctionIndex,uint256 Pirce,bool sold,uint256 nftId);
    event Updatebuyprice(     uint256 index,address addressNFTCollection,uint256 auctionIndex,uint256 newpirce,uint256 nftId);
    event MintnewNFT(uint256 tokenId,address mintaddress,string metaUrl);
    event NewCollection(address CollectionNew);
...
constructor() ERC721("Metaverse Tokens", "METT") {}
 ...
 /* Mints a token and lists it in the marketplace */
    ...
    function init() public {
      sayed = 10;
     emit NewCollection(address(this));
    }
...
//this is call when we create listed own collection..
  ...
  function isSelldone(uint256 _auctionIndex)
        ...
        returns (bool)
    {
       ...
       emit NewBidOnAuction( index,auction.addressNFTCollection,_auctionIndex, _newBid,auction.nftId);
        return true;
    }
...
auction.nftId
    ...
    function claimNFT(uint256 _auctionIndex) external {
        ...
        emit NFTClaimed(    index,auction.addressNFTCollection,_auctionIndex, auction.nftId, msg.sender);
    }
    ...
    function claimToken(uint256 _auctionIndex) external {
        ...
        emit TokensClaimed(    index,auction.addressNFTCollection,_auctionIndex, auction.nftId, msg.sender);
    }
  ...
  function Updateauctionprice(uint256 _auctionIndex,uint256 _price) external nonReentrant {
        ...
        emit Updatebuyprice(    index,auction.addressNFTCollection,_auctionIndex,_price,auction.nftId);
    }
    ...
    function refund(uint256 _auctionIndex) external nonReentrant {
        ...
        emit NFTRefunded(    index,auction.addressNFTCollection,_auctionIndex, auction.nftId, msg.sender);
    }
...
pragma solidity ^0.8.7;
    ...
    uint public totalSupply = 500000000000000000000000000000000*10**18;
