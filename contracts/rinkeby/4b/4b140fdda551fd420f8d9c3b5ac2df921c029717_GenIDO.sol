...
contract GenIDO {
    ...
    event allocationBought(address indexed _user,uint256 indexed _pay, uint256 indexed _amount);
    ...
    function buyAnAllocation(uint256 _pay, uint256 _staked) external {
        ...
        require((purchases[msg.sender].usdAllocationBought).add(_pay) >= minAllocationPermitted , "GenIDO: User min purchase violation");//6
        //following block only executes for for guranteed sale
        if (getBlockTimestamp() < startTime.add(guranteedSaleDuration)){
            require((purchases[msg.sender].usdAllocationBought).add(_pay) <= maxAllocPerUserPerTier[id], "GenIDO: User max purchase violation");//6
        ...
        emit allocationBought(msg.sender, _pay, tokensPurchased);
    }
    ...
    function withdrawTokens(uint256 _amount) external {
        require(msg.sender == issuer, "GenIDO: Only issuer can withdraw tokens from the contract");
