...
// File: contracts/Copy_GS_MP/asset/IAssetCommon.sol
...
// File: contracts/Copy_GS_MP/secondary/IFixedPriceSecondary.sol
...
// File: contracts/Copy_GS_MP/secondary/IBestOfferSecondary.sol
...
// File: contracts/Copy_GS_MP/secondary/IAuctionSecondary.sol
...
// File: @openzeppelin/contracts-upgradeable/interfaces/IERC721ReceiverUpgradeable.sol
...
// File: @openzeppelin/contracts-upgradeable/interfaces/IERC721Upgradeable.sol
...
// File: contracts/Copy_GS_MP/GS_Marketplace.sol
    ...
    OwnableUpgradeable
{
    ...
    function listFixedPriceItem(
        ...
        if (assetType == AssetType.ERC721) {
            ...
            // require(IERC165Upgradeable(asset).supportsInterface(_INTERFACE_ID_ERC2981),"Does not support royalty");
        ...
        ListInfo memory newInfo = ListInfo({
            _itemId: _fixedItemId,
            _nft: asset,
            _tokenId: tokenId,
            _seller: msg.sender
        });
        listInfo.push(newInfo);

        emit ListedFixedPriceItem(msg.sender, _fixedItemId);
    }

    function getListingPriceItemDetails(address _nft, uint128 _tokenId, address _seller) public view returns (ListInfo[] memory){
        uint256 resultCount;

        for(uint256 i=0; i<listInfo.length; i++){
            if(listInfo[i]._nft == _nft && listInfo[i]._tokenId == _tokenId && listInfo[i]._seller == _seller){
                resultCount++;
            }
        }

        ListInfo[] memory result = new ListInfo[](resultCount);
        uint256 j;
    ...
    function makeOffer(
        ...
        OfferInfo memory newInfo = OfferInfo({
            _itemId: _offerItemId,
            _nft: asset,
            _tokenId: tokenId
        });
        offerInfo.push(newInfo);

        emit OfferMade(msg.sender, _offerItemId);

    }

    function getOfferPriceItemDetails(address _nft, uint128 _tokenId) public view returns (OfferInfo[] memory){
        uint256 resultCount;

        for(uint256 i=0; i<offerInfo.length; i++){
            if(offerInfo[i]._nft == _nft && offerInfo[i]._tokenId == _tokenId ){
                resultCount++;
            }
        }

        OfferInfo[] memory result = new OfferInfo[](resultCount);
        uint256 j;

        for(uint256 i=0; i<offerInfo.length; i++){
            if (offerInfo[i]._nft == _nft && offerInfo[i]._tokenId == _tokenId){
                result[j] = offerInfo[i];
                j++;
            }
        }
    ...
    function cancelOffer(uint256 itemId) external override{
        ...
        (address buyer,
            AssetType assetType,
            Status status) = _decodeBuyerTypeStatus(item.buyer_type_status);
        (uint128 tokenId, uint128 amount) = _decodeTokenIdAmount(item.tokenId_amount);
    ...
    function acceptOffer(
        ...
        require(block.timestamp > offerTime && (block.timestamp < offerTime + offerDuration), "Offer is either not made or expired");

        require(_WETH.balanceOf(buyer) >= price, "Buyer is out of balance");
    ...
    function listAuction(
        ...
        AuctionInfo memory newInfo = AuctionInfo({
            _itemId: _auctionItemId,
            _nft: asset,
            _tokenId: tokenId,
            _seller: msg.sender
        });
        auctionInfo.push(newInfo);
    ...
    function makeBid(
        ...
        (address asset, uint96 minimumAuctionPrice) = _decodeAssetPrice(item.asset_price);
        ...
        require(bidPrice >= minimumAuctionPrice, "bid price should be greater than minimum bid amount set by seller");
        require(bidPrice > bidInfo[_auctionItmId][currentBidCounter - 1].bidPrice, "Bid price should be greater than top bidder");
        require(block.timestamp > auctionTime  && (block.timestamp < auctionTime + auctionDuration), "Auction is either not made or has expired");
        require(_WETH.allowance(msg.sender, address(this)) >= bidPrice, "Please approve bid price to WETH");
        ...
        bidInfo[_auctionItemId][currentBidCounter].bidPrice = bidPrice;
    ...
    //think if already one auction is running seller canot start another auction?? This method returns all itemIds in auction
    function getAuctionPriceItemDetails(address _nft, uint128 _tokenId, address _seller) public view returns (AuctionInfo[] memory){
        uint256 resultCount;

        for(uint256 i=0; i<auctionInfo.length; i++){
            if(auctionInfo[i]._nft == _nft && auctionInfo[i]._tokenId == _tokenId && auctionInfo[i]._seller == _seller){
                resultCount++;
            }
        }
    ...
    function finishAuction(
        ...
        _safePayment(bid*amount, buyer, seller, asset, tokenId);//*amount need to test?
    ...
    function _safePayment(
        ...
        // (address receiver, uint256 royalty) = IERC2981Upgradeable(address(asset)).royaltyInfo(tokenId, bid);
        // require(receiver != address(0), "Royalty receiver is a null address");
        // require(royalty > 0, "Royalty amount returned is zero");
        uint256 comission_ = (bid * _comission) / 100;

        // _WETH.transferFrom(buyer, seller, bid - royalty - comission_);
        _WETH.transferFrom(buyer, seller, bid - comission_);
    ...
    function _encodeTimeDuration(uint128 time, uint128 duration)
        internal
        pure
        returns (uint256 timeDuration)
    {
    ...
    function _encodeBuyerTypeStatus(address buyer, AssetType assetType)
        internal
        pure
        returns (uint256 buyerNumeratorTypeStatus)
    {
    ...
    function _decodeTimeDuration(uint256 timeDuration)
        internal
        pure
        returns (uint128 time, uint128 duration)
    {
    ...
    function _decodeBuyerTypeStatus(uint256 buyerNumeratorTypeStatus)
        internal
        pure
        returns (
            address buyer,
    ...
    function _onlyOwner(address _addr, address sender) internal pure {
        if (_addr != sender) {
            revert ("Only creator access!");
        }
    }

    function _onlyCreatedItem(Status status) internal pure {
        if (status != Status.CREATE) {
            revert ("Only item with created status can be processed!");
        }
    }
}