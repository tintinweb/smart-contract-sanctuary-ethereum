...
contract PGT34 is ERC721A, Ownable, ReentrancyGuard {
    // Qty
    ...
    struct Variables {
        ...
        uint64 PUBLIC_PRICE;
        uint64 AUCTION_START_PRICE;
        uint32 MAX_PER_MINT;
        uint32 AUCTION_PERIOD;
    }
    ...
    constructor(string memory baseURI) ERC721A("PlaygroundTestV34", "PlaygroundTestV34") {
    ...
    function presaleMint(uint256 _qty, uint256 _cid, bytes32[] calldata _merkleProof) external payable {
        ...
        require(wlPurchaseCount[msg.sender] + _qty <= variables.MAX_PER_MINT, "Max qty");
    ...
    function publicMint(uint256 _qty) external payable {
        ...
        require(totalSupply() + _qty <= SUPPLY, "No more supply");
        require(_qty > 0 && _qty <= variables.MAX_PER_MINT, "Too many qty");
    ...
    function auctionMint(uint256 _qty) external payable {
        require(variables.AUCTION_START_PRICE > 0, "Auction closed");
        require(totalSupply() + _qty <= SUPPLY, "No more supply");
        require(_qty > 0 && _qty <= variables.MAX_PER_MINT, "Too many qty");
        ...
        if(timeElapsed < variables.AUCTION_PERIOD) {
            uint256 deduction = AUCTION_DEDUCTION_RATE * timeElapsed;
            currentPrice = variables.AUCTION_START_PRICE - deduction;
        } else {
            currentPrice = variables.PUBLIC_PRICE;
        }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        ...
        for(uint256 i = 0; i < receivers.length; i++) {
            _safeMint(receivers[i], receivers.length);
        }
    }

    function setVariables(uint64 _presale, uint64 _public, uint64 _auction, uint32 _max, uint32 _period) external onlyOwner {
        variables = Variables(
            _presale,
            _public,
            _auction,
    ...
    function setAuctionVariable(uint256 _rate) external onlyOwner {
        AUCTION_DEDUCTION_RATE = _rate;
