...
contract ClaimingRegistry is IClaimingRegistry, Initializable, AbstractDependant {
    ...
    uint256 internal constant VOTING_DURATION = 3600;
    uint256 internal constant PRIVATE_CLAIM_DURATION = 7200;
    uint256 internal constant VIEW_VERDICT_DURATION = 1 days;
    uint256 public constant WITHDRAWAL_PERIOD = 4200;
    uint256 public constant READY_TO_WITHDRAW_PERIOD = 7200;
    ...
    mapping(uint256 => uint256) public override readyToWithdrawDate;
    //reward withdraw
    EnumerableSet.AddressSet internal _withdrawRewardRequestVoterList;
    mapping(address => RewardInfo) public override rewardWithdrawalInfo;
    ...
    {
        if (readyToWithdrawDate[index] == 0) {
            return WithdrawalStatus.NONE;
        }

        if (block.timestamp < readyToWithdrawDate[index]) {
            return WithdrawalStatus.PENDING;
        }

        if (block.timestamp >= readyToWithdrawDate[index].add(READY_TO_WITHDRAW_PERIOD)) {
    ...
    function submitClaim(
        ...
        ClaimStatus status = _myClaims[claimer].contains(index)
            ? claimStatus(index)
        ...
        returns (ClaimStatus)
    {
        ...
        bool newPolicyBought = policyRegistry.policyStartTime(claimer, policyBookAddress) >
            _allClaimsByIndexInfo[index].dateSubmitted;
    ...
    function _requestClaimWithdrawal(address claimer, uint256 index) internal {
        ...
        readyToWithdrawDate[index] = block.timestamp.add(WITHDRAWAL_PERIOD);

        emit WithdrawalRequested(
            claimer,
            _allClaimsByIndexInfo[index].claimRefund,
            readyToWithdrawDate[index]
        );
    }
        ...
        onlyClaimVoting
    {
        ...
        rewardWithdrawalInfo[voter] = RewardInfo(rewardAmount, _readyToWithdrawDate);

        emit WithdrawalRequested(voter, rewardAmount, _readyToWithdrawDate);
    }

    function withdrawClaim(uint256 index) public {
        address claimer = _allClaimsByIndexInfo[index].claimer;
        ...
        address policyBookAddress = _allClaimsByIndexInfo[index].policyBookAddress;
        IPolicyBook(policyBookAddress).withdrawClaim(
            msg.sender,
            _allClaimsByIndexInfo[index].claimRefund
        );

        _withdrawClaimRequestIndexList.remove(index);
        delete readyToWithdrawDate[index];

        emit ClaimWithdrawn(msg.sender, _allClaimsByIndexInfo[index].claimRefund);
    }
    ...
    function withdrawReward() public {
        ...
        uint256 rewardAmount = rewardWithdrawalInfo[msg.sender].rewardAmount;

        capitalPool.fundReward(msg.sender, rewardAmount);
