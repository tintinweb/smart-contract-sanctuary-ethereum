...
contract Amm is IAmm, AmmStorageV1, BlockContext, OwnerPausable, ReentrancyGuardUpgradeable {
    ...
    function initialize(InitializeParams memory params) external initializer {
        ...
        // Pack A and gamma:
        // shifted A + gamma
        uint256 A_gamma = PerpMath.shift(params.A, 128);
        A_gamma = PerpMath.bitwise_or(A_gamma, params.gamma);
        _globalInitParams.initialAGamma = A_gamma;
        _globalFutureParams.futureAGamma = A_gamma;

        _globalInitParams.adjustmentStep = params.adjustmentStep;
        _globalInitParams.maHalfTime = params.maHalfTime;
    ...
    function swap(
        ...
        // AM_OT: only Trader
        require(_trader == _msgSender(), "AM_OT");
        return _swap(i, j, dx, dyLimit, isExactInput, false);
    }

    function addLiquidity(uint256[N_COINS] memory amounts, uint256 minLiquidity)
        external
        override
        whenNotPaused
        nonReentrant
        returns (uint256)
    {
        ...
        if (al.old_D > 0) {
            mintedLiquidity = (_totalLiquidity * D) / al.old_D - _totalLiquidity;
        } else {
            mintedLiquidity = _getXcp(D); // making initial virtual price equal to 1
        }

        // AM_NM: nothing minted
        require(mintedLiquidity > 0, "AM_NM");

        _totalLiquidity += mintedLiquidity;
        if (al.old_D > 0) {
        ...
        // AM_TLRAL: too little received when add liquidity
        require(mintedLiquidity >= minLiquidity, "AM_TLRAL");

        emit AddLiquidity(_msgSender(), amounts, mintedLiquidity, _totalLiquidity);

        return mintedLiquidity;
    }

    function removeLiquidity(uint256 liquidity, uint256[N_COINS] memory minTokenAmounts)
        public
        override
        whenNotPaused
        nonReentrant
        returns (uint256[N_COINS] memory)
    {
        ...
        uint256[N_COINS] memory tokenAmounts = PerpMath.copy(_coinBalances);
        for (uint8 i = 0; i < N_COINS; i++) {
            uint256 tokenAmount = (tokenAmounts[i] * calcLiquidity) / total_supply;
            // AM_TLRRL: too little received when remove liquidity
            require(tokenAmount >= minTokenAmounts[i], "AM_TLRRL");
        ...
        emit RemoveLiquidity(_msgSender(), tokenAmounts, liquidity, _totalLiquidity);
    ...
    function getDy(
        ...
        uint256 priceScaleTemp = _priceScale * _coinPrecisions[1];
        uint256[N_COINS] memory xp = PerpMath.copy(_coinBalances);
    ...
    function calcTokenAmountsByLiquidity(uint256 liquidity) external view override returns (uint256[N_COINS] memory) {
        // Make rounding errors favoring other LPs a tiny bit
    ...
    function calcLiquidityByTokenAmounts(uint256[N_COINS] memory amounts) external view override returns (uint256) {
        uint256[2] memory A_gamma = _A_gamma();
        uint256[N_COINS] memory xp = _getXp();
        uint256 D0 = _globalD;

        if (_globalFutureParams.futureAGammaTime > 0) {
            D0 = AmmLib.newton_D(A_gamma[0], A_gamma[1], xp);
        }

        uint256[N_COINS] memory amountsp = [
            amounts[0] * _coinPrecisions[0],
            (amounts[1] * _priceScale * _coinPrecisions[1]) / PRECISION
        ];
        xp[0] += amountsp[0];
        xp[1] += amountsp[1];

        uint256 D1 = AmmLib.newton_D(A_gamma[0], A_gamma[1], xp);

        return (_totalLiquidity * D1) / D0 - _totalLiquidity;
    }
    ...
    //     return AmmLib.getTwapMarkPrice(interval, address(this));
    // }
    ...
    function _swap(
        ...
        sp.A_gamma = _A_gamma();
        sp.xp = PerpMath.copy(_coinBalances);
        ...
        if (isExactInput) {
            ...
            // exact input swap slippage
            require(dy >= dyLimit, "AM_EISS");
            ...
            // exact output swap slippage
            require(dy <= dyLimit, "AM_EOSS");
    ...
    function _tweakPrice(
        ...
        if (priceLastTimestampTemp < _blockTimestamp()) {
            // MA update required
            uint256 maHalfTimeTemp = _globalInitParams.maHalfTime;
        ...
        if (p_i > 0) {
            ...
            uint256[N_COINS] memory __xp = PerpMath.copy(_xp);
    ...
    function _repegging(
        ...
        // uint256 adjustmentStepTemp = PerpMath.max(_globalInitParams.adjustmentStep, norm / 10);
        uint256 adjustmentStepTemp = _globalInitParams.adjustmentStep;

        // if (norm > adjustmentStepTemp && old_virtual_price > 0) {
    ...
    function _A_gamma() internal view returns (uint256[2] memory) {
        ...
        uint256 gamma1 = PerpMath.bitwise_and(A_gamma_1, 2**128 - 1);
        uint256 A1 = PerpMath.shift(A_gamma_1, -128);

        if (_blockTimestamp() < t1) {
            // handle ramping up and down of A
            uint256 A_gamma_0 = _globalInitParams.initialAGamma;
            uint256 t0 = _globalInitParams.initialAGammaTime;

            // Less readable but more compact way of writing and converting to uint256
            // uint256 gamma0 = PerpMath.bitwise_and(A_gamma_0, 2**128-1)
            // uint256 A0 = PerpMath.shift(A_gamma_0, -128)
            // A1 = A0 + (A1 - A0) * (_blockTimestamp() - t0) / (t1 - t0)
            // gamma1 = gamma0 + (gamma1 - gamma0) * (_blockTimestamp() - t0) / (t1 - t0)
            t1 -= t0;
            t0 = _blockTimestamp() - t0;
            uint256 t2 = t1 - t0;
            A1 = (PerpMath.shift(A_gamma_0, -128) * t2 + A1 * t0) / t1;
            gamma1 = (PerpMath.bitwise_and(A_gamma_0, 2**128 - 1) * t2 + gamma1 * t0) / t1;
        }
    ...
    function setTrader(address traderArg) external onlyOwner {
        // AM_TNC: Trader is not contract
        require(traderArg.isContract(), "AM_TNC");
    ...
    function rampAGamma(
        ...
        require(_blockTimestamp() > _globalInitParams.initialAGammaTime + (MIN_RAMP_TIME - 1), "AM_CTTS");
        // AM_FTTS: future time is too small
        require(futureTime > _blockTimestamp() + (MIN_RAMP_TIME - 1), "AM_FTTS");

        uint256[2] memory A_gamma = _A_gamma();
        uint256 initialAGammaTemp = PerpMath.shift(A_gamma[0], 128);
        initialAGammaTemp = PerpMath.bitwise_or(initialAGammaTemp, A_gamma[1]);
        ...
        _globalInitParams.initialAGamma = initialAGammaTemp;
        _globalInitParams.initialAGammaTime = _blockTimestamp();

        uint256 futureAGammaTemp = PerpMath.shift(futureA, 128);
        futureAGammaTemp = PerpMath.bitwise_or(futureAGammaTemp, futureGamma);
    ...
    function stopRampAGamma() external onlyOwner {
        uint256[2] memory A_gamma = _A_gamma();
        uint256 currentAGamma = PerpMath.shift(A_gamma[0], 128);
        currentAGamma = PerpMath.bitwise_or(currentAGamma, A_gamma[1]);

        _globalInitParams.initialAGamma = currentAGamma;
    ...
    function commitNewParameters(uint256 newAdjustmentStep, uint256 newMAHalfTime) external onlyOwner {
        ...
        if (newAdjustmentStep > 10**18) {
            newAdjustmentStep = _globalInitParams.adjustmentStep;
        }
        ...
        if (newMAHalfTime < 7 * 86400) {
            ...
            newMAHalfTime = _globalInitParams.maHalfTime;
        }
    ...
    function applyNewParameters() external nonReentrant onlyOwner {
        ...
        _globalInitParams.adjustmentStep = adjustmentStepTemp;

        uint256 maHalfTimeTemp = _globalFutureParams.futureMAHalfTime;
        _globalInitParams.maHalfTime = maHalfTimeTemp;
...
abstract contract AmmStorageV1 {
    ...
    AmmLib.InitialParams internal _globalInitParams;
    AmmLib.FutureNewParams internal _globalFutureParams;
