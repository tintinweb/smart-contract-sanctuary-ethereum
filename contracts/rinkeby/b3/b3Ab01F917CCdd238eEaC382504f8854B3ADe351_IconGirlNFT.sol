...
interface Ifashion{
  function lockFashionItems(uint256 [] memory fashion_tokenIds) external;

  function unlockFashionItems(uint256 [] memory fashion_tokenIds) external;
}
contract IconGirlNFT is ERC721Enumerable, Ownable, Pausable {
  ...
  address public fashion_item = 0x56bFe872e85F7b18169059e6C8Ee4A3038638862;
  ...
  constructor () ERC721 ("IconGirl", "$IGT") {
    setBaseURI("https://testnet-api.icon.fashion/api/getTokenURI/icongirl/");
    pause(true);
  }
  ...
  function mintIconGirl(uint256 [] memory fashion_tokenIds) public payable saleIsOpen {
    Ifashion(fashion_item).lockFashionItems(fashion_tokenIds);
    _mintAnElement();
  ...
  function undressIconGirl(uint256 collectionId) public saleIsOpen {
    Ifashion(fashion_item).unlockFashionItems(burnedFashionItems[msg.sender][collectionId]);
    _burn(collectionId);
  }
