...
error KaijuMart_PaymentTokeNotAccepted();
...
contract KaijuMart is AccessControl, ReentrancyGuard {
    ...
    enum LotType { NONE, AUCTION, RAFFLE, DOORBUSTER }
    enum PaymentToken { RWASTE, SCALES, EITHER }
    ...
    constructor(
        ...
        IDoorbusterManager _doorbusterManager,
        IRaffleManager _raffleManager
    ) {
        ...
        doorbusterManager = _doorbusterManager;
    ...
    modifier validPayment(uint256 lotId, PaymentToken token) {
        ...
        if (acceptedPaymentToken != PaymentToken.EITHER && acceptedPaymentToken != token)
            revert KaijuMart_PaymentTokeNotAccepted();
        _;
    }

    function _manager(LotType lotType) internal view returns (address) {
        if (lotType == LotType.RAFFLE) {
            return address(raffleManager);
        } else if (lotType == LotType.DOORBUSTER) {
            return address(doorbusterManager);
        } else if (lotType == LotType.AUCTION) {
    ...
    function _charge(
        ...
        nonReentrant
    {
        if (token == PaymentToken.SCALES) SCALES.spend(account, lots[lotId].scalesPrice * amount);
        else RWASTE.burn(account, lots[lotId].rwastePrice * amount);
    }
    ...
    function _charge(
        ...
        nonReentrant
    {
        if (token == PaymentToken.SCALES) SCALES.spend(account, value);
        else RWASTE.burn(account, value);
    }
    ...
    function _refund(
        ...
        nonReentrant
    {
        if (token == PaymentToken.RWASTE) RWASTE.claimLaboratoryExperimentRewards(account, value);
        else if (token == PaymentToken.SCALES) SCALES.credit(account, value);
    }
        ...
        nonReentrant
    {
        ...
        if (settler != address(0)) {
            ...
            lots[lotId].settler = address(0);

            emit Settle(lotId, settler, _msgSender());
        }
    }
     ...
     */
    function getAuction(uint256 auctionId) public view returns (IAuctionManager.Auction memory) {
        return auctionManager.get(auctionId);
    }

    /**
     * @notice TODO
     * @param _auctionManager TODO
     */
        ...
        onlyRole(MANAGER_ROLE)
    {
        ...
        auctionManager.create(lotId, auction);
    }
        ...
        onlyKingz
    {
        (
            ...
            uint104 refundValue
        ) = auctionManager.bid(auctionId, value, _msgSender());

        PaymentToken token = lots[auctionId].paymentToken;
        _charge(_msgSender(), value, token);
        if (refundValue > 0) _refund(refundReceiver, refundValue, token);
    ...
    function settle(
        ...
        lotExists(auctionId)
    {
        if (!auctionManager.canSettle(auctionId, _msgSender())) revert KaijuMart_CannotSettleLot();

        _settle(auctionId, data);
    }
     ...
     */
    function getDoorbuster(uint256 doorbusterId) public view returns (IDoorbusterManager.Doorbuster memory) {
        return doorbusterManager.get(doorbusterId);
    }

    /**
     * @notice TODO
     * @param _doorbusterManager TODO
     */
    function setDoorbusterManager(
        IDoorbusterManager _doorbusterManager
    )
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
        ...
        onlyRole(MANAGER_ROLE)
    {
        ...
        doorbusterManager.create(lotId, supply);
    }
    ...
    function purchase(
        ...
        onlyKingz
    {
        _purchase(doorbusterId, amount, token, _msgSender());
    }
    ...
    {
        doorbusterManager.purchase(doorbusterId, amount);

        _charge(account, doorbusterId, amount, token);
     ...
     * @param raffleId TODO
     */
    function getRaffle(uint256 raffleId) public view returns (IRaffleManager.Raffle memory) {
        return raffleManager.get(raffleId);
    }

    /**
     * @notice TODO
     * @param _raffleManager TODO
     */
    function setRaffleManager(
        IRaffleManager _raffleManager
    )
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
        ...
        onlyRole(MANAGER_ROLE)
    {
        ...
        raffleManager.create(lotId, raffle);
    }
    ...
    function draw(
        ...
        onlyRole(MANAGER_ROLE)
    {
        raffleManager.draw(raffleId, vrf);
    }
    ...
    function enter(
        ...
        onlyKingz
    {
        raffleManager.enter(raffleId, amount);

        _charge(_msgSender(), raffleId, amount, token);
...
interface IManager {
    enum Token { RWASTE, SCALES, EITHER }
}
...
interface IAuctionManager is IManager {
    ...
    function canSettle(uint256 id, address sender) external view returns (bool);
    function create(uint256 id, CreateAuction calldata auction) external;
    function bid(uint256 id, uint104 amount, address sender) external returns (address, uint104);
}
...
interface IDoorbusterManager is IManager {
    struct Doorbuster {
        uint32 supply;
    }
...
interface IRaffleManager is IManager {
    struct CreateRaffle {
        uint64 scriptId;
        uint64 winners;
    }
