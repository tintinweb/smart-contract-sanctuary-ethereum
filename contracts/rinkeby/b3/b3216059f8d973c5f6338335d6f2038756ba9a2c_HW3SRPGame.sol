...
contract HW3SRPGame is usingOraclize { //Scissor Rock Paper
    ...
    event roundResult(address player, uint playerGuess, string randomDS, uint contractGuess, string result);
    ...
    function __callback(bytes32 _queryId, string _result, bytes _proof) public { 
        ...
        if (oraclize_randomDS_proofVerify__returnCode(_queryId, _result, _proof) != 0) {
            ...
            emit roundResult(queryIdToPlayer[_queryId], playerGuess, _result, contractGuess, ans);
        }
    }
    ...
    function guess(uint gesture) payable public {
        require(msg.value == 0.1 ether, "play game need 0.1 ether.");
