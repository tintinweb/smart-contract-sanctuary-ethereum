...
/// @title Fiscus Bond Depository V2
/// @author Zeus, Indigo
/// Review by: JeffX

contract FiscusBondDepositoryV2 is IBondDepository, NoteKeeper {
    /* ======== DEPENDENCIES ======== */
    ...
    constructor(
        IFiscusAuthority _authority,
        IERC20 _fisc,
        IgFISC _gfisc,
        IStaking _staking,
        ITreasury _treasury
    ) NoteKeeper(_authority, _fisc, _gfisc, _staking, _treasury) {
        // save gas for users by bulk approving stake() transactions
        // usdce = _usdce;
        _fisc.approve(address(_staking), 1e45);
    }
     ...
     * @return payout_     the amount of gFISC due
        ...
        )
    {
         ...
         * price = quote tokens : fisc (i.e. 42069 DAI : FISC)
         *
         * 1e18 = FISC decimals (9) + price decimals (9)
         */
         ...
         *
         * this is either the number of FISC that the market can sell
        ...
        // purchased, and how much FISC has been sold
    ...
    function _tune(uint256 _id, uint48 _time) internal {
        ...
        if (_time >= meta.lastTune + meta.tuneInterval) {
            ...
            // fisc decimals (9) + price decimals (9)
             ...
             * is 10,000 FISC. max payout would be 1,000 FISC (10,000 * 1 / 10).
             */
     ...
     * @param _market      [capacity (in FISC or quote), initial price / FISC (9 decimals), debt buffer (3 decimals)]
    ...
    function create(
         ...
         *
         * 1e18 = fisc decimals (9) + initial price decimals (9)
         */
        uint64 targetDebt = uint64(_booleans[0] ? ((_market[0] * 1e18) / _market[1]) / 10**decimals : _market[0]);
        
        // Giving discount
        // "Bonding pair discounts of 10% discount for FISC/USDCe and 7% discount on USDC.e alone"
        // if (address(_quoteToken) == usdce) {
        //     targetDebt = targetDebt * 9 / 10;
        // }

        /*
         * max payout is the amount of capacity that should be utilized in a deposit
         * interval. for example, if capacity is 1,000 FISC, there are 10 days to conclusion,
         * and the preferred deposit interval is 1 day, max payout would be 100 FISC.
         */
        ...
        emit CreateMarket(id_, address(fisc), address(_quoteToken), _market[1]);
    }
     ...
     * @param _id          ID of market
     * @return             price for market in FISC decimals
     *
     ...
     * @param _id          ID of market
     * @return             amount of FISC to be paid in FISC decimals
     *
     * @dev 1e18 = fisc decimals (9) + market price decimals (9)
     */
     ...
     * @param _id          ID of market
     * @return             current debt for market in FISC decimals
     */
     ...
     * @param _id          ID of market
     * @return             control variable for market in FISC decimals
     */
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
    ...
    IgFISC internal immutable gFISC;
    ...
    constructor(
        ...
        IStaking _staking,
        ITreasury _treasury
    ) FrontEndRewarder(_authority, _fisc) {
        gFISC = _gfisc;
        staking = _staking;
        treasury = _treasury;
    }
     ...
     * @param _payout      the amount of FISC due to the user
    ...
    function addNote(
        ...
        notes[_user].push(
            Note({
                payout: gFISC.balanceTo(_payout),
        ...
        // note that only the payout gets staked (front end rewards are in FISC)
     ...
     * @param _sendgFISC    send payout as gFISC or sFISC
     * @return payout_     sum of payout sent, in gFISC
     */
    function redeem(
        address _user,
        ...
        if (_sendgFISC) {
            gFISC.transfer(_user, payout_); // send payout as gFISC
        } else {
            staking.unwrap(_user, payout_); // unwrap and send payout as sFISC
        }
    }
     ...
     * @param _sendgFISC    send payout as gFISC or sFISC
     * @return             sum of payout sent, in gFISC
     */
    function redeemAll(address _user, bool _sendgFISC) external override returns (uint256) {
        return redeem(_user, indexesFor(_user), _sendgFISC);
    }
     ...
     * @return payout_     the payout due, in gFISC
...
interface IBondDepository {
    ...
    struct Market {
        ...
        bool capacityInQuote; // capacity limit is in payment token (true) or in FISC (false, default)
    ...
    struct Terms {
        ...
        uint64 maxDebt; // 9 decimal debt maximum in FISC
    }
...
abstract contract FrontEndRewarder is FiscusAccessControlled {
    /* ========= STATE VARIABLES ========== */
    ...
    IERC20 internal immutable fisc; // reward token

    constructor(IFiscusAuthority _authority, IERC20 _fisc) FiscusAccessControlled(_authority) {
        fisc = _fisc;
    }
    ...
    function getReward() external {
        ...
        fisc.transfer(msg.sender, reward);
    }
...
interface IgFISC is IERC20 {
    ...
    function migrate(address _staking, address _sFISC) external;
}
...
interface INoteKeeper {
    ...
    struct Note {
        uint256 payout; // gFISC remaining to be paid
    ...
    function redeemAll(address _user, bool _sendgFISC) external returns (uint256);
