...
contract DegenDAO is ERC20, Ownable {
  ...
  uint256 public maxAllowableGas = 300 gwei;
  ...
  constructor() ERC20('Degen DAO', 'DD') {
    ...
    maxTxnAmount = (totalSupply() * 25) / 10000; // 0.25% supply
  ...
  function _transfer(
    ...
    if (_isSwap) {
      if (block.timestamp == launchTime) {
        ...
        require(
          tx.gasprice <= maxAllowableGas,
          'must not exceed max allowable gas price'
        );
  ...
  function setMaxAllowableGas(uint256 _wei) external onlyOwner {
    require(_wei >= 50 gwei, 'cannot require less than 50 gwei');
    maxAllowableGas = _wei;
  }
