...
contract GluwaInvestmentPoolVault is SignerNonce, EIP712Upgradeable, VaultControl, GluwaInvestment {
    ...
    function withdrawUnstartedBalances(
    ...
    function withdrawBalancesFor(
        ...
        require(_token.transfer(ownerAddress, _withdrawBalances(balanceHashList, ownerAddress, fee)), 'GluwaInvestment: Unable to send amount to withdraw balance');
        return true;
    }

    function withdrawBalances(bytes32[] calldata balanceHashList) external returns (bool) {
        require(_token.transfer(_msgSender(), _withdrawBalances(balanceHashList, _msgSender(), 0)), 'GluwaInvestment: Unable to send amount to withdraw balance');
        return true;
    }
    ...
    function addPoolRepayment(
        ...
        require(
            amount + pool.totalRepayment <= pool.totalDeposit + _calculateTotalExpectedPoolWithdrawal(pool.interestRate, pool.tenor, pool.totalDeposit),
...
contract GluwaInvestment is Initializable, ContextUpgradeable {
    ...
    function _getUnstartedBalanceList(address owner) internal view returns (GluwaInvestmentModel.Balance[] memory) {
        ...
        for (uint256 i; i < balanceIds.length; ) {
            ...
            if (_getPoolState(temp.poolHash) == GluwaInvestmentModel.PoolState.Canceled && !_balancePrematureClosed[_balanceIndex.get(balanceIds[i])]) {
                balanceList[i] = temp;
            }
            unchecked {
                ++i;
    ...
    function _getMatureBalanceList(address owner) internal view returns (GluwaInvestmentModel.Balance[] memory) {
        ...
        for (uint256 i; i < balanceIds.length; ) {
            temp = _balanceStorage[_balanceIndex.get(balanceIds[i])];
            if (_getBalanceState(_balanceIndex.get(balanceIds[i])) == GluwaInvestmentModel.BalanceState.Mature && !_poolLocked[temp.poolHash]) {
                balanceList[i] = temp;
            }
            unchecked {
                ++i;
    ...
    function _createAccount(
        ...
        bytes32 identityHash,
        bytes32 poolHash
    ) internal returns (bool, bytes32, bytes32) {
    ...
    function _withdrawUnstartedBalances(
        ...
        for (uint256 i; i < balanceHashList.length; ) {
            ...
            require(
                _getPoolState(balance.poolHash) == GluwaInvestmentModel.PoolState.Canceled && balance.owner == ownerAddress_,
    ...
    function _matureBalance(bytes32 balanceHash, GluwaInvestmentModel.Balance storage balance) private returns (uint256) {
        require(
            _getBalanceState(balanceHash) == GluwaInvestmentModel.BalanceState.Mature && !_poolLocked[balance.poolHash],
        ...
        uint256 withdrawalAmount = ((balance.principal + _calculateYield(pool.interestRate, pool.tenor, balance.principal)) * pool.totalRepayment) /
            pool.totalDeposit - balance.totalWithdrawal;
