    ...
    ERC721HolderUpgradeable
{
    ...
    IAdventurerStaking public legacyContract;
    ...
    // Store of token availability for summoning
    mapping(uint256 => uint256) public _status;

    // Store of token renting status
    mapping(uint256 => uint256) _lockIn;

    uint256 constant SHARED_SUMMON_COST = 1000 ether; // 1000 CHRONOS
    uint256 constant SHARED_SUMMON_REWARD = 250 ether; // 250 CHRONOS
    uint256 constant SINGLE_SUMMON_COST = 750 ether;
    ...
    function initialize(
        ...
        require(_adminAddress != address(0), "admin cannot be 0x0");
    ...
    function claimChronos(address _address) public {
        uint256 unclaimed = chronosContract.getTotalUnclaimed(msg.sender);
        uint256 accrued = unclaimed;
        if (grantChronos == true) {
            accrued += getAccruedChronos(_address);
            lastUpdated[_address] = uint64(block.timestamp);
            _chronosReward[_address] = 0;
        }
    ...
    function stake(uint256 token) public override {
        if (_userStakes[msg.sender] > 0) {
            claimChronos(msg.sender);
        } else {
            lastUpdated[msg.sender] = uint64(block.timestamp);
        }
        _stake(token);
    }
    ...
    function unstake(uint256 token) public override {
        require(stakes[token].user != address(0), "tokenId not staked");
        require(stakes[token].user == msg.sender, "sender didn't stake token");
        require(block.timestamp > _lockIn[token], "token is currently locked");
        uint64 stakeLength = uint64(block.timestamp) - stakes[token].timeStaked;
        require(stakeLength > LOCK_IN, "token lock in hasn't passed");
        claimChronos(msg.sender);
    ...
    function groupUnstake(uint256[] memory tokens) external override {
        ...
        for (uint256 i = 0; i < tokens.length; ++i) {
            require(stakes[tokens[i]].user != address(0), "tokenId not staked");
            require(
                stakes[tokens[i]].user == msg.sender,
    ...
    function burnToken(uint256 token) public onlyRole(BURNER_ROLE) {
        require(stakes[token].user != address(0), "token must be staked");
    ...
    {
        require(stakes[token].user != address(0), "token not staked");
    ...
    function _summon(
        ...
        require(option <= uint8(type(SummonOption).max), "invalid option");
        require(
            (SummonOption(option) != SummonOption.HoldingUnclaimed) ||
                (getAccruedChronos(msg.sender) > SHARED_SUMMON_COST),
            "insufficient earned chronos"
        );
        SummonOption selectedOption = SummonOption(option);
        uint256 cost = shared ? SHARED_SUMMON_COST : SINGLE_SUMMON_COST;
        ...
        if (selectedOption == SummonOption.HoldingUnclaimed) {
            _chronosReward[msg.sender] = getAccruedChronos(msg.sender);
            lastUpdated[msg.sender] = block.timestamp;
            _chronosReward[msg.sender] -= cost;
            chronosContract.grantChronos(address(this), cost);
            adventurerContract.summon(_token1, _token2, true);
    ...
    function sharedSummon(
        ...
        require(
            stakes[token].user == msg.sender,
            "token must be staked by user"
        );
        require(
            BitMap.checkBit(_status[summoner], SUMMON_BIT),
            "token not available for summon"
        );
        _summon(token, summoner, option, true);
    }
    ...
    function summon(
        ...
        require(
            ...
            "tokens must be staked by user"
        );
        _summon(token1, token2, option, false);
    }

    function setSummoning(uint256 token, bool status) public {
        require(stakes[token].user == msg.sender, "user must own token");
        require(
            !status || (token <= MAX_GEN_COUNT),
            "token must be gen 1 to summon"
        );
        ...
        external
    {
        ...
        for (uint256 i = 0; i < tokens.length; ++i) {
            require(
                stakes[tokens[i]].user != address(0),
    ...
    function addLockInRole(address user) external onlyRole(ADMIN_ROLE) {
        require(user != address(0), "setter cannot be 0x0");
...
interface IAdventurerHolding is IERC721ReceiverUpgradeable {
  ...
  function viewStakes(address _address) external view returns (uint256[] memory);
  ...
  /// @return the list of tokenIds as uint256[] memory
  function viewStakesByStatus(uint8[] memory status) external view returns (uint256[] memory);
  ...
  function sharedSummon(uint256 token, uint256 summoner, uint8 option) external;
  ...
  /// @param option where to use the chronos from
  function summon(uint256 token1, uint256 token2, uint8 option) external;
  ...
  /// @notice Claims Chronos accrued by the Adventurer while staked.
  /// @param _address the address that's claiming Chronos
  function claimChronos(address _address) external;
