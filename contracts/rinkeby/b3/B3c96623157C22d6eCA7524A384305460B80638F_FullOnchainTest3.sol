...
contract FullOnchainTest3 is ERC721Enumerable, Ownable {

    bytes[10] private colors;
    string[10] private tests;
    string private description;
    string private parts11;
    ...
    function mint(address _to, uint256 _tokenId) public onlyOwner {
        require((_tokenId > 0 ) && (_tokenId <= 10000 ), "tokenId must be between 1 and 10000");
    ...
    function setColor(uint256 _colorId, bytes memory _string) public onlyOwner {
        require(colors[_colorId].length == 0, "Data can be set once");
        require((_colorId >= 0)&&(_colorId < 10), "tokenId must be between 0 and 9");
        colors[_colorId] = _string;
    }

    function setTest(uint256 _tokenId, string memory _string) public onlyOwner {
        tests[_tokenId] = _string;
    }

    function getTest(uint256 _tokenId) private view returns (string memory) {
        string memory output = tests[_tokenId];
        return output;
    }

    function setParts(string memory _string) public onlyOwner {
        require(bytes(parts11).length == 0, "Data can be set once");
        parts11 = _string;
    }

    function setDescription(string memory _string) public onlyOwner  {
        require(bytes(description).length == 0, "Data can be set once");
        description = _string;
    }

    function getColor(uint256 _tokenId, uint256  _colorId) private view returns (string memory) {
        bytes memory colormap;
        uint256 index;
        if(_tokenId>5000){
            colormap = colors[_colorId + 5];
            index = _tokenId - 5000;
        }else{
            colormap = colors[_colorId];
            index = _tokenId;
        }
        // 例えばcolormapにはカラーID：５の長い文字列が入っているそこから、RGBの１バイトずつを取ってきて、文字列に変換している。
        // 一見文字としては６文字だが、２つの文字で１バイトなので、カラーコードは３バイトからなっている。そのため、３倍している
        // RGBをそれぞれ16進数から文字列に変換し、それをつなげている
        // これにより、指定したToken IDのカラーコードを取ってきている
        string memory output = string(abi.encodePacked(toHexString(colormap[(index-1) * 3]),toHexString(colormap[(index-1) * 3 + 1]),toHexString(colormap[(index-1) * 3 + 2])));
        return output;
    }

    // 1バイトを文字に変換
    function toHexString(bytes1 value) internal pure returns (string memory) {
        bytes16 _HEX_SYMBOLS = "0123456789abcdef";
        // bytes1のvalueをuint8に変換したものをuint256に入れている
        uint256 v = uint8(value);
        bytes memory buffer = new bytes(2 );
        for (uint256 i = 2  + 1; i > 1; --i) {
            buffer[i - 2] = _HEX_SYMBOLS[v & 0xf];
            v >>= 4;
        }
    ...
    function tokenURI(uint256 _tokenId) public override view returns (string memory) {
        ...
        parts[1] = getColor(_tokenId, 1);
        parts[2] = ';}.cls-4{fill:#';
        parts[3] = getColor(_tokenId, 4);
        parts[4] = ';}.cls-1{fill:#';
        parts[5] = getColor(_tokenId, 3);
        parts[6] = ';}.cls-5{fill:#';
        parts[7] = getColor(_tokenId, 0);
        parts[8] = ';}.cls-3{fill:#';
        parts[9] = getTest(1);
        parts[10] = ';}.cls-2{fill:#3d3938;}</style></defs>';

        string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6], parts[7], parts[8]));
        output = string(abi.encodePacked(output, parts[9], parts[10], parts11));

        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "#', toString(_tokenId),' Original Otaku Coin', '", "description": "',description,'", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
