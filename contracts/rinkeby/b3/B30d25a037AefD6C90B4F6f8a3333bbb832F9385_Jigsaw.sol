...
contract Jigsaw is ERC721URIStorage {
    ...
    function getUris(uint start, uint finish, bool status) public view returns(string[] memory) {
        if (finish > total) {
            finish = total;
        }
        ...
        string[] memory uris;
        uint count;
        while (count < start + finish) {
            if (solutions[start].status == status) {
                uris[count] = solutions[start].uri;
                count++;
            }
        }
