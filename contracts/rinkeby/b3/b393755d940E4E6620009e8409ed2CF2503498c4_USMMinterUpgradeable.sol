...
contract USMMinterUpgradeable is BaseRelayRecipient, OwnableUpgradeable {
    ...
    // function enoughMintBuffer(uint _usmAmount) internal returns(bool){
    //     uint buffer = getCurrentMintBuffer();
    //     if(_usmAmount <= buffer){
    //         mintBuffer = buffer - _usmAmount;
    //         return true;
    //     }else{
    //         return false;
    //     }
    ...
    //     _mintBuffer = mintBuffer + (block.timestamp-lastMintTimestamp)*mintRate;
    ...
    function mintWithToken(IERC20 _token, uint _tokenAmount, address _to) internal returns(uint _usmAmount){
        ...
        // require(enoughMintBuffer(usm2mint), "not enough buffer for minting");
...
contract USMMinterUpgradeableProxy is TransparentUpgradeableProxy {
