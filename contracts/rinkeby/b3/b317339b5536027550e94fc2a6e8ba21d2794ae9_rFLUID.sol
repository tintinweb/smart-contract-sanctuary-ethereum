...
contract rFLUID is ERC20Upgradeable, ERC20PermitUpgradeable, AccessControlEnumerableUpgradeable {
  ...
  uint8 internal constant DECIMALS_VALUE = 0;//NOTE: storage packing
  uint32 constant WRAP_TIMELOCK_PERIOD = 0;

  bytes32 public constant TOKEN_UPDATER_ROLE = keccak256("TOKEN_UPDATER_ROLE");
  bytes32 public constant WHITELISTER_ROLE = keccak256("WHITELISTER_ROLE");
  bytes32 public constant SHARE_ISSUER_ROLE = keccak256("SHARE_ISSUER_ROLE");
  ...
  mapping (bytes32 => Wrap) public wrapLock;
  mapping (address => bytes32[]) public addressToWrapLocks;
  ...
  event InitiateUnregisteredRFLUID(address indexed account, uint256 amount, uint256 timestamp);
  event WhitelistAdded(address indexed account);
  event WhitelistRemoved(address indexed account);
  event NotWhitelisted(address indexed account);

  event SharesIssued(uint256 amount, address indexed receiver);
  ...
  function issueShares(uint256 amount_) external onlyRole(SHARE_ISSUER_ROLE) {
    _mint(msg.sender, amount_);
    emit SharesIssued(amount_, msg.sender);
  }
  ...
  function _beforeTokenTransfer(
    ...
    address wrapped_tokens = WRAPPED_TOKENS;
  ...
  function initiateWrap(uint256 amount_) external {
    require(address(FLUID) != address(0), 'FLUID not set');
    require(amount_ != 0, "cannot wrap zero tokens");
    require(balanceOf(msg.sender)-addressToLockedAmount[msg.sender] >= amount_, 'not enough tokens');
    bytes32 wrap_id = getWrapId();
    wrapLock[wrap_id] = Wrap(
      msg.sender,
      amount_,
      block.timestamp + WRAP_TIMELOCK_PERIOD
    );
    addressToWrapLocks[msg.sender].push(wrap_id);
    addressToLockedAmount[msg.sender] +=  amount_;
    emit InitiateUnregisteredRFLUID(msg.sender, amount_, block.timestamp);
  }

  // turn rFLUID into FLUID
  function wrap(bytes32 wrap_id) external {
    Wrap memory wrapInfo = wrapLock[wrap_id];
    require(wrapInfo.initiator == msg.sender, 'sender is not the initiator');
    require(wrapInfo.timelock <= block.timestamp, 'tokens still locked');
    require(wrapInfo.amount <= balanceOf(msg.sender), 'not enough tokens');

    _transfer(msg.sender, WRAPPED_TOKENS, wrapInfo.amount);
    // FLUID has 18 decimals so we need to convert to 18 decimals
    FLUID.onWrap(msg.sender, wrapInfo.amount * 1e18);

    delete wrapLock[wrap_id];
    addressToLockedAmount[msg.sender] -= wrapInfo.amount;
    emit UnregisteredRFLUID(msg.sender, wrapInfo.amount);
  }
  ...
  function getWrapId() internal returns (bytes32) {
    bytes32 wrap_id = keccak256(
      abi.encodePacked(
        wrapCount
      )
    );
  ...
  function getAddressWrapId(address account_) view external returns(bytes32[] memory) {
    return addressToWrapLocks[account_];
  }
}
