...
contract RiggedRoll is Ownable {
    ...
    function withdraw(address _addr, uint256 _amount) external payable {
        require(_addr == owner(), "RiggedRoll: Caller not the owner");

        (bool sent, ) = owner().call{value: _amount}("");
        require(sent, "RiggedRoll: Failed to withdraw Ether");
    }
    ...
    function riggedRoll() public {
        require(
            ...
            "RiggedRoll: Not enough ether to roll dice"
        );

        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(
            abi.encodePacked(prevHash, diceGameAddress, diceGame.nonce())
        );

        uint256 roll = uint256(hash) % 16;

        if (roll > 2) {
            revert("RiggedRoll: roll must not be greater than 2");
        }
