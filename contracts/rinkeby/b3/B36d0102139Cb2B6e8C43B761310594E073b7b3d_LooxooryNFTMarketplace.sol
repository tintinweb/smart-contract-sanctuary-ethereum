...
contract LooxooryNFTMarketplace is ReentrancyGuard,Ownable {
    ...
    constructor(address _nftContract,address _randomContract){
    ...
    function createCampaign(
      CreateCampaignStruct memory _campaignDTO
        ...
        for(uint256 i=0;i<_campaignDTO.nfts.length;i++){
          if(nftItems[_campaignDTO.nfts[i]].owner==address(this) && nftItems[_campaignDTO.nfts[i]].campaignID==0){
    ...
    function createMarketItem(
      CreateMarketItemStruct memory marketItem
        ...
        ILooxooryNft(nftContract).mint(address(this),marketItem.fractionAmount,marketItem.metadataUrl);
  ...
  function createMarketSale(
    ...
    uint256 amount
      ...
      if(nftItems[itemId].remainingAmount>amount){
    ...
    function listNftForAuction(uint256 nftID,uint256 bidEndTime) external onlyOwner{
        ...
        require(nftItems[nftID].owner==address(this),"Cannot list this nft");
  ...
  function claimBidItem(uint256 itemId) external nonReentrant {
    ...
    IERC1155(nftContract).safeTransferFrom(address(this), msg.sender, tokenId,nftItems[itemId].fractionAmount,"");
    nftItems[itemId].owner = payable(msg.sender);
    nftItems[itemId].sold = true;
