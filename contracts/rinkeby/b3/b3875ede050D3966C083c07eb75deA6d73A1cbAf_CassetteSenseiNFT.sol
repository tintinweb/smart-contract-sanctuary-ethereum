...
contract CassetteSenseiNFT is ERC721, Ownable, ERC721Pausable, ERC721Enumerable {
    ...
    mapping(uint256 => uint256) private tokenIdTrackerAtRound;
    ...
    constructor(
        ...
        rounds[0]._tokenIdTracker.increment();
        _tokenIdTracker.increment();

        rounds[0].exclusiveSaleStartTime = _startTime;
        rounds[0].publicSaleStartTime = _startTime + 2 hours;
        rounds[0].revealed = false;
        rounds[0].exclusiveSalePrice = 0.000000188 ether;
        rounds[0].publicSalePrice = 0.00000258 ether;
        rounds[0].notRevealedURI = _notRevealedURI;
    ...
    function startRound(
        ...
        emit StartRound(
            id,
            rounds[id]._tokenIdTracker.current(),
            rounds[id].maxSupply,
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        uint256 rid = tokensAtRound[tokenId];
        if (rounds[rid].revealed == false) {
            ...
            return
                string(abi.encodePacked(baseURI, Strings.toString(tokenIdTrackerAtRound[tokenId])));
        }
    }
    ...
    function redeemPublic() external payable whenNotPaused {
        ...
        require((rounds[rid]._tokenIdTracker.current() + 1) <= rounds[rid].maxSupply, "Sold out");
        require(_price <= msg.value, "You must send exactly the price of the NFT.");

        rounds[rid].claimed[msg.sender] = true;
        tokensAtRound[_tokenIdTracker.current()] = rid;
        tokenIdTrackerAtRound[_tokenIdTracker.current()] = rounds[rid]._tokenIdTracker.current();

        _safeMint(msg.sender, _tokenIdTracker.current());

        _tokenIdTracker.increment();
        rounds[rid]._tokenIdTracker.increment();
    ...
    function mintPrivate(uint256 qty) external onlyOwner {
        ...
        for (uint256 i = 0; i < qty; i++) {
            ...
            tokenIdTrackerAtRound[_tokenIdTracker.current()] = rounds[id]._tokenIdTracker.current();
