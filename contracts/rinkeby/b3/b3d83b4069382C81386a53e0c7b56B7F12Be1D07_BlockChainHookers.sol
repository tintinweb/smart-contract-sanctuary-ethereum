...
contract BlockChainHookers is ERC721A, Ownable {
    ...
    constructor()
        ERC721A("BlockChainHookers", "HOOKERS")
    {}
    ...
    function mint(uint256 quantity) public payable {
        ...
        if (msg.sender != owner()) {
            ...
            require(
                publicmint_claimed[msg.sender] + quantity <= publicMintLimit_pw,
                "Public Mint Limit Reached"
            );
            require(
                publicMintCount + quantity <= publicSaleLimit,
            ...
            if (publicmint_claimed[msg.sender] > 0) {
                ...
                require(
                    msg.value >= (publicSaleCost * (quantity - 1)),
                    "Not enough ether sent"
                );
    ...
    function whitelistMint(bytes32[] calldata _proof, uint256 quantity)
        public
        payable
    {
        require(startDate <= block.timestamp, "Minting Not Yet Started");
        ...
        require(
            whitelist_claimed[msg.sender] + quantity <= whitelistLimit_pw,
            "Limit Exceed"
        );
        require(msg.value >= whitelistCost * quantity, "insufficient funds");
        require(
            whitelistCount + quantity <= whitelistLimit,
    ...
    function setpublicMintLimit_pw(uint256 _publicMintLimit_pw)
        public
        onlyOwner
    {
