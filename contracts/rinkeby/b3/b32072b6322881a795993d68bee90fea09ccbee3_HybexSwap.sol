...
contract HybexSwap is Initializable, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  event AccountsWhitelisted(address[] indexed accounts, bool status);
  ...
  function swap(uint256 amount) external nonReentrant whenNotPaused {
    ...
    require(amount > 0, "Swap: Amount cant be zero");
    if(enableWhitelisting) require(whitelisted[msg.sender], "Swap: Account is not whitelisted");
  ...
  function updateEnableWhitelisting(bool enable) external onlyOwner {
    require(enableWhitelisting != enable , "Swap: Already in same status");
  ...
  function includeInWhitelist(address account) external onlyOwner {
    ...
    require(!whitelisted[account], "Swap: Account is already whitelisted");
    whitelisted[account] = true;
    emit AccountWhitelisted(account, true);
  }
    
  function excludeFromWhitelist(address account) external onlyOwner {
    require(account != address(0), "Swap: Account cant be zero address");
    require(whitelisted[account], "Swap: Account is not whitelisted");
    whitelisted[account] = false;
    emit AccountWhitelisted(account, false);
  }
  ...
  function includeAllInWhitelist(address[] memory accounts) external onlyOwner {
    ...
    emit AccountsWhitelisted(accounts, true);
  }
  ...
  function updateSwapToken(address token) external virtual onlyOwner whenNotPaused {
