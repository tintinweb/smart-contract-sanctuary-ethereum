...
contract MarsversMarket is EIP712MetaTransaction("MarsversMarket", "1"), ReentrancyGuard, Ownable, IMarsversMarket {
    ...
    function executeSell(
        uint256[3] memory uints, // sellId, price, deadline, nftId
        address[3] memory addrs, // seller, quoteToken, nftAddress
        bytes[2] memory sigs,
        bytes32 sellId
    ) external payable nonReentrant {
        require(uints[1] >= block.timestamp, "MarsMarket: This sale is expired.");
        ...
        bytes32 msgHash = keccak256(abi.encode(addrs[0], addrs[1], uints[0], uints[1], addrs[2], uints[2]));
        ...
        if (addrs[1] == address(1)) {
            require(msg.value == uints[0], "Insufficient fund");
            TransferHelper.safeTransferETH(addrs[0], uints[0]);
        } else {
            require(msg.value == 0, "ERC20 token sale");
            TransferHelper.safeTransferFrom(addrs[1], msg.sender, addrs[0], uints[0]);
        }

        IERC721 nftTokenContract = IERC721(addrs[2]);
        nftTokenContract.transferFrom(addrs[0], msg.sender, uints[2]);

        emit SellExecuted(sellId, addrs[0], msg.sender, addrs[2], uints[2], 1);
    }
    ...
    function executeOffer(
        uint256[4] memory uints, // offerId, offerPrice, saleDeadline, offerDeadline, nftId
        address[] memory addrs, // offerProvider, quoteToken, nftAddress
        bytes[2] memory sigs,
        bytes32 offerId
    ) external nonReentrant {
        Order memory orderOffer = Order(addrs[0], addrs[1], addrs[2], uints[0], uints[2], uints[3]);
        require(uints[1] >= block.timestamp || uints[1] == 0, "MarsMarket: This sale is expired");
