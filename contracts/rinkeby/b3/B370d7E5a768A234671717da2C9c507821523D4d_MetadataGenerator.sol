...
contract MetadataGenerator is OwnableUpgradeable {
    ...
    mapping (address => bool) public onlyMarketplaceMapping;
    ...
    modifier onlyMarketplace {
        require (onlyMarketplaceMapping[msg.sender], 'Error: Caller Is Not Allowed To Interact');
        _;
    }

    function addToController (address _controller) external onlyOwner {
        controllers[_controller] = true;
    }

    function setOnlyMarketplace(address _marketplace) external onlyOwner {
        onlyMarketplaceMapping[_marketplace] = true;
    }
    ...
    function createTraits(uint tokenId, uint randomNumber) external onlyController {
    ...
    function forceCreateTraits(uint tokenId, uint256[] memory traits) external onlyController{
    ...
    function addAttributeToSpecificTrait (uint tokenId, uint traitType, uint attributeId) external onlyMarketplace {
        uint[] memory traits = traitMapper[tokenId];
        traits[traitType] = attributeId;
        traitMapper[tokenId] = traits;
    }
