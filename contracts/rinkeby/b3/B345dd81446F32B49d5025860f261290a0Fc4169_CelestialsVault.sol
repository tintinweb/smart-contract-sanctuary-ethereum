...
contract CelestialsVault is  Ownable {
    ...
    function airdrop(address[] calldata accounts, uint[] calldata amounts, uint[] calldata tokenIds) external onlyOwner{
        if(accounts.length != amounts.length || amounts.length != tokenIds.length) revert ArraysDontMatch();
