...
contract PartyPals is Authorized, ERC721A, VerifySigner, PaymentSplitter {
  ...
  mapping(address => uint256) public addressQuantity;

  address[] private _splitters = [0x71665B951a9EB87c8e6A8EF29a266fA578a78654];
  uint256[] private _splits = [100];

  constructor(string memory _secret, address _signer)
    ERC721A("Party Pals", "PALS", maxPerAddress, maxSupply)
    PaymentSplitter(_splitters, _splits)
  {
    setSecret(_secret);
    setSigner(_signer);
  }
  ...
  function airdrop(address _address, uint256 _quantity) public onlyAuthorized {
    require(
  ...
  function mint(uint256 _quantity, bytes memory _signature)
    public
    callerIsUser
  {
    require(!paused, "mint: mint is paused");
    require(
      totalSupply() + _quantity <= collectionSize,
      "mint: invalid quantity"
    );
    require(verify(_quantity, _signature), "mint: invalid signature");
    require(
      addressQuantity[msg.sender] + _quantity <= maxPerAddress,
      "mint: exceeds user mint quantity"
    );

    addressQuantity[msg.sender] += _quantity;

    _safeMint(msg.sender, _quantity);
  }

  function setURI(string calldata _baseURI, string calldata _suffixURI)
    external
    onlyAuthorized
  {
    baseTokenURI = _baseURI;
    suffixTokenURI = _suffixURI;
  }

  function togglePause() external onlyAuthorized {
    paused = !paused;
  }
  ...
  {
    require(_exists(_tokenId), "tokenURI: token queried doesn't exist");
    return
      string(
        abi.encodePacked(baseTokenURI, _tokenId.toString(), suffixTokenURI)
      );
  }
}
...
contract VerifySigner is Authorized {
    ...
    */
    function verify( uint256 _quantity, bytes memory _signature ) internal view returns ( bool ) {
    ...
    */
    function createHash( string memory _quantity ) private view returns ( bytes32 ) {
        return keccak256( abi.encodePacked( address(this), msg.sender, _quantity, secret ) );
    }
