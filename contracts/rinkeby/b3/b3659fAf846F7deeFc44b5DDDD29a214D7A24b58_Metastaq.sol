  ...
  ReentrancyGuard
{
  ...
  modifier isOwnerOrHotWallet() {
    if (
      !(owner() == _msgSenderERC721A() || _msgSenderERC721A() == getHotWallet())
  ...
  function allowlistMint(
    ...
    if (!(_type == 'fiat' || _type == 'crypto')) revert InvalidPaymentType();
  ...
  function refundIfOver(uint256 price) private {
    ...
    if (msg.value > price) {
      (bool success, bytes memory returnError) = payable(_msgSenderERC721A())
