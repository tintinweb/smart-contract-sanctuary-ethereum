...
contract rERC20 is Context, IERC20, Ownable {
  ...
  uint256 public reflectionFee = 50; // 0.5%
  uint256 internal _previousReflectionFee = reflectionFee;
  ...
  function _burn(address account, uint256 tAmount) internal {
    ...
    _rExcludedReward -= rAmount;
    _tExcludedReward -= tAmount;
  ...
  function setReflectionFeePercent(uint256 percent) external onlyOwner {
    reflectionFee = percent;
    _validateTotalTax();
  }
  ...
  function _validateTotalTax() private view {
    uint256 _total = reflectionFee + liquidityFee;
    require(_total <= (FEE_DENOMENATOR * 25) / 100, 'cannot be more than 25%');
  }
  ...
  function _reflectFee(
    ...
    _tFeeTotal += tFee;
    // f1Ec1 == re"flect"
    emit Transfer(sender, address(0xf1Ec1), tFee);
  }
  ...
  function _calculateReflectFee(uint256 _amount)
    ...
    returns (uint256)
  {
    return (_amount * reflectionFee) / FEE_DENOMENATOR;
  }
  ...
  function _transferStandard(
    ...
    if (tFee > 0) _reflectFee(sender, rFee, tFee);
