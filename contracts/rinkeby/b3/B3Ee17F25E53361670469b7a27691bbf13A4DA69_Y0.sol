...
contract Y0 is ERC1155, Ownable {
  ...
  mapping (address => mapping (uint256 => bool)) private isRevealed;
  ...
  */
  ...
  function reveal(address claimer, uint256 _typeId) public onlyOwner {
    require(block.timestamp - mint_timestamp[claimer][_typeId] >= 1 minutes, 'NFT is revealed soon');
    isRevealed[claimer][_typeId] = true;
    setUri(claimer, _typeId);
  }

  function setUri(address claimer, uint256 _typeId) public view {
    if (isRevealed[claimer][_typeId])
      uri(_typeId);
    uri(1);
  }
}
