...
contract Jackpot is Ownable, ReentrancyGuard, VRFConsumerBase {
    using SafeERC20 for IERC20;

    IRJGToken public RJGTokenAddress;

    // 3 plan will be exsit
    uint256[3] public plan = [0.01 ether, 0.1 ether, 1 ether];
    //when not test TODO
    ...
    uint64 public constant prepareTime = 2 minutes;
    //when not test TODO
    //uint64 public constant prepareTime = 2 minutes;
    uint64 public constant playTime = 1 hours;
    //when not test TODO
    //uint64 public constant playTime = 1 hours;
    ...
    // arry of balance of players
    mapping(uint256 => mapping(address => uint256)) public gTokenBalance;
    ...
    // Game ID's to info
    mapping(uint256 => JackpotInfo) internal allGames_;
    ...
    event GameCloseded(uint256 PlayerNumber, uint256 GameID);
    ...
    // CONSTRUCTOR
    //-------------------------------------------------------------------------
    uint256 public gameID_;

    bytes32 public keyHash;
    uint256 public fee;
    uint256 public randomResult;

    constructor(IRJGToken _RJGToken)
        VRFConsumerBase(
    ...
    function deposit(uint256 _gameID) external payable {
        ...
        JackpotInfo memory _currentGameInfo = allGames_[_gameID];

        require(
            msg.value >= _currentGameInfo.selectedPlan,
            "You can only send certain amount of Ether"
        );

        playerOnGame[_gameID].push(msg.sender);
        gTokenBalance[_gameID][msg.sender] = 0;
        _currentGameInfo.playersOnGame++;
    ...
    function openGame(uint256 _gameID) external onlyOwner {
        ...
        if (uint64(block.timestamp) < _currentGameInfo.openingTimestamp) {
            emit GamePreparing(
                _currentGameInfo.selectedPlan,
                _currentGameInfo.startingTimestamp,
                _gameID
            );
        } else if (_currentGameInfo.playersOnGame < minRequiredPlayer) {
            _currentGameInfo.gameStatus = Status.Closed;
            _currentGameInfo.endTimestamp = uint64(block.timestamp);
            emit GameCloseded(_currentGameInfo.playersOnGame, _gameID);
        } else {
            _currentGameInfo.gameStatus = Status.Shipped;
            emit GameStarted(
                _currentGameInfo.playersOnGame,
                _currentGameInfo.openingTimestamp,
                _gameID
            );
        }
    }

    function _dailyGame(uint256 _gameID) private onlyOwner {
        require(
            allGames_[_gameID].gameStatus == Status.Shipped,
            "ERROR: game is not started"
        );
        JackpotInfo memory _currentGameInfo = allGames_[_gameID];
        uint8 _date = _currentGameInfo.dayPassed++;
        require(_currentGameInfo.dayPassed < playTime, "ERROR");
        require(
            _currentGameInfo.openingTimestamp + _date * (1 days) >
                block.timestamp &&
                _currentGameInfo.openingTimestamp + (_date - 1) * (1 days) <
                block.timestamp,
            "You can only update once in one day"
        );

        if (block.timestamp <= _currentGameInfo.endTimestamp) {
            uint256[] memory _tempbalances = getRandomData(
                _currentGameInfo.playersOnGame
            );
            uint256 _currentMaxBalance = 0;
            address _currentWinner = playerOnGame[_gameID][0];

            for (uint256 i = 0; i < _currentGameInfo.playersOnGame; i++) {
                ...
                if (
                    ...
                    _currentGameInfo.winner = _currentWinner;
                }
            }
        } else {
            _currentGameInfo.gameStatus == Status.Completed;
            _currentGameInfo.endTimestamp = uint64(block.timestamp);
    ...
    function claimReward(uint256 _gameID) external {
        ...
        JackpotInfo memory _currentGameInfo = allGames_[_gameID];
        require(
            _currentGameInfo.winner == _msgSender() &&
                Address.isContract(_msgSender()),
            "Your are the winner so pls get prize instead of token"
        );
        require(
            gTokenBalance[_gameID][msg.sender] > 0,
        ...
        RJGTokenAddress.mint(msg.sender, amount);
    ...
    function getPrize(uint256 _gameID) external nonReentrant {
        ...
        uint256 amount = plan[allGames_[_gameID].selectedPlan] *
