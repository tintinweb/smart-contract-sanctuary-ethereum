...
// TODO: Adapter contracts for PFPs and Bgs, map them.
...
contract PfpBg is Ownable, FeeLockable, DestroyLockable {
    ...
    function setBackground(string memory color, string memory image, string memory svg, address nftBgContract, uint256 nftBgTokenId) external payable {
        require(msg.value == feeAmount, "Value");
        addressToBackground[msg.sender].color = color;
        addressToBackground[msg.sender].image = image;
        addressToBackground[msg.sender].svg = svg;
    ...
    function getBgSvg(address pfpOwner) public view returns(string memory) {
        ...
        bytes memory image = bytes(background.image).length > 0 ? abi.encodePacked('<image y="0" height="640" x="0" width="640" href="', background.image, '"/>') : bytes("");
        ...
        return string(abi.encodePacked(color, image, svg, nft));
    }
