...
import "./LpStakerAccessControl.sol";

contract LpStaker is LPTokenWrapper,Ownable,LpAccessControl {
    ...
    constructor(IERC20 _uniPair,IShroomController _controller) LPTokenWrapper(_uniPair) Ownable(){
    ...
    function _updateIndexAndTime(uint256 _index,uint256 _time)internal{
        rewardIndex = _index;
        lastUpdateTimestamp =_time;
    }

    function updateEmissionPerSecond(uint256 _newEmission) public onlyRole(HANDLER_ROLE){
        _updateIndexAndTime(_getNewIndex(),block.timestamp);
        emissionPerSecond = _newEmission;
    }

    function _getNewIndex()internal view returns (uint){
        uint256 oldIndex = rewardIndex;
        uint256 currentTime = block.timestamp;
        uint256 newIndex;
        if (totalSupply() == 0){
            return oldIndex;
        }
        newIndex = oldIndex +(((currentTime - lastUpdateTimestamp) * emissionPerSecond)*(1e18))/totalSupply();
        return newIndex;
    }
    ...
    function _updateUserReward(address _user) internal{
        ...
        userRewards[_user] = _earned(_user,newIndex);
        _updateIndexAndTime(newIndex,block.timestamp);
    ...
    function earned(address _user)public view returns(uint256){
        uint256 index = _getNewIndex();
        return _earned(_user,index);
    }

    function _earned(address _user,uint256 _index) internal view returns (uint256){
        ...
        uint256 reward =(balanceOf(_user) * (_index - uIndex))/10**18 + userRewards[_user];
        return reward;
    }
    ...
    function claimReward() public whenNotPaused {
        ...
        uint256 newIndex = _getNewIndex();
        _updateIndexAndTime(newIndex,block.timestamp);
        uint256 reward = _earned(user,newIndex);
        if (reward > 0){
            userRewards[user] =0 ;
            userIndex[user] = newIndex;
            claimedRewards[user] = claimedRewards[user] + reward;
            controller.getShroom(reward,"lpStaker",user);
        }
    }
...
contract LPTokenWrapper {
    ...
    constructor(IERC20 pair_) public {
        pair = pair_;
    }
...
abstract contract LpAccessControl is AccessControl {
    ...
    constructor() {
        _setupRole(HANDLER_ROLE, defaultOwner);
