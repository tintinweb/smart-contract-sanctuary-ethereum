...
contract StakeContract {
    ...
    constructor() {
        owner = msg.sender;
        is_farmable[SquidGame] = false;
        pesa_reward = IERC20(SquidGame);
    }
    ...
    function unfarmTokens(uint id) public safe cooldown {
        ...
        pesa_reward.transfer(msg.sender, farming_unit[msg.sender][id].balance);
        ...
        if (has_revenue) {  
          ...
          pesa_reward.transfer(msg.sender, balance);
        }
  ...
  function set_reward_rate(uint rate) public authorized {
    ...
    function set_pesa(address token) public authorized {
        SquidGame = token;
        pesa_reward = IERC20(SquidGame);
    }  
