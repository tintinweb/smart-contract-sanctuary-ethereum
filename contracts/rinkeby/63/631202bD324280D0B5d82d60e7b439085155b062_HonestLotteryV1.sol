...
contract HonestLotteryV1 is Ownable,VRFConsumerBaseV2{
  ...
  // Array for storing requested random values.
  ...
  function startLottery() public onlyOwner {
    require(lottery_state == LOTTERY_STATE.CLOSED, "Cant start a new lottery yet!");//Don't start a new lottery if previous one didn't end.
  ...
  // Function for users to enter the lottery_state.
  // Transaction must include a ticket price.
  function enter() public payable{
    require(lottery_state == LOTTERY_STATE.OPEN);//only allow to enter if the lottery has started
  ...
  function fulfillRandomWords(
    ...
    require(lottery_state == LOTTERY_STATE.CALCULATING_WINNER,"Lottery in progress!");
 ...
 // Executed by the owner to keep the lottery running and random winners being generated.
 ...
 function getWinPoolUsd() public view returns(uint256){
   (,int256 price,,,)= ethUsdPriceFeed.latestRoundData();
   uint256 adjustedPrice = uint256(price)*1e10; //18 decimals
   return (address(this).balance/10)*9*adjustedPrice;//
 }
