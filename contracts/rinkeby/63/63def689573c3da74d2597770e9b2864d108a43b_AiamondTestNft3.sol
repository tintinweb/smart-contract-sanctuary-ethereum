...
contract AiamondTestNft3 is ERC1155, IERC2981, Ownable, ContextMixin, NativeMetaTransaction, ReentrancyGuard {    
    using SafeMath for uint256;
    ...
    // url folder id
    uint256 public constant Founder = 1;
    uint256 public constant Gold = 2;
    uint256 public constant Silver = 3;
    uint256 public constant Bronze = 4;

    address private _recipient;

    string public url = "ipfs://QmQaAC43a34g9jRjz5QH624qMi6GHuUpY85PTLRm6vtQAN/";
    ...
    constructor(
        ...
        for (uint256 i = 0; i < 15; i++) {
            create(
                ...
                1, // initialSupply
                1, // maxSupply
                i < 5 ? 0 ether : 25 ether, // price
                i < 5 ? 0 : 400, // royality,
                string.concat(url, Strings.toString(i < 5 ? Founder : Gold), "/"),
                ""
            );
        }
        ...
        create(
            ...
            0.05 ether, // price
            400, // royality,
            string.concat(url, Strings.toString(Bronze), "/"),
            ""
        );
    }

    function buySilver(uint256 id, uint256 amount)
        payable
        public
    {
        require(msg.value >= (amount * prices[id -1]), "Not enough ether sent");
        _checkMintReq(id, amount);
    ...
    function _checkMintReq(uint256 _id, uint256 _amount) private view
    {
        require(_exists(_id), "Token doesn't exist");
        require(_id > 0, "Token doesn't exist");
        require(tokenSupply[_id -1] + _amount <= maxSupplies[_id -1], "not enought supply left");
    }
    ...
    */
    ...
    function uri(uint256 _id) override public view returns (string memory) {
        if(bytes(customUri[_id]).length>0){
            return string(
                abi.encodePacked(
                    string(customUri[_id]),
                    _id,
                    ".json"
                )
            );
    ...
    */
     ...
     * @param _maxSupply max amount for this token
     * @param _salePrice for this token
     * @param _royality for this token
    ...
    function create(
        ...
        prices[_id] = _salePrice;
        maxSupplies[_id] = _maxSupply;
        royalities[_id] = _royality;
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view override
        returns (address receiver, uint256 royaltyAmount)
    {
        return (_recipient, (_salePrice * royalities[_tokenId-1]) / 10000);
    }
     ...
     * https://docs.opensea.io/docs/polygon-basic-integration#code-example-for-erc1155
    */
    ...
    */
    ...
    function contractURI() public view returns (string memory) {
        return url; // Contract-level metadata
    }
}