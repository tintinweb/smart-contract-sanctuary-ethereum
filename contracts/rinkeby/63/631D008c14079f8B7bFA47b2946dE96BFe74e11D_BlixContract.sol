...
contract BlixContract is NonblockingLzApp {
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal override {
        Proxy proxy = new Proxy(endpointAddress, _srcChainId, _srcAddress);
        emit CreatedProxy(_srcChainId, _srcAddress, address(proxy));
    }

    function createProxy(uint16 _dstChainId) public payable {
        _lzSend(_dstChainId, bytes("test"), payable(msg.sender), address(0x0), bytes(""));
    }
...
contract Proxy is NonblockingLzApp {
