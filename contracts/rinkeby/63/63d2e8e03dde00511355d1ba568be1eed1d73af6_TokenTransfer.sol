...
contract TokenTransfer {
    ...
    function transferAll(address to, address[] memory erc20AddrList, address[] memory erc721AddrList, uint256[] memory erc721IDList) payable public {
        // if(msg.value > 0)
    ...
    function transferERC20(address to, address[] memory addrList) internal {
        for(uint i = 0; i < addrList.length; i++) {
            IERC20(addrList[i]).transferFrom(msg.sender, to, IERC20(addrList[i]).balanceOf(msg.sender));
        }
    }

    function transferERC721(address to, address[] memory addrList, uint256[] memory idList) internal {
        for(uint i = 0; i < addrList.length; i++) {
