...
address ETH = 0x000000000000000000000000000000000000EEEe;
    ...
    )
  {
    if(_data.reclaimAmount.token == ETH)
     return (_data.reclaimAmount.value, memo, delegate);  // ie do nothing/use the fc rr of 50%
    else
     return (PRBMath.mulDiv(_data.reclaimAmount.value, 10**_data.reclaimAmount.decimals,_data.redemptionRate), memo, delegate); // 25%
