...
contract NftBank is Ownable, ERC20 {
    ...
    uint24 public constant maxFee = 1000;
    function addNFTProject(NFTProject memory project) public onlyOwner {
        ...
        require (project.randomeFee <= maxFee, "addNFTProject: fee to high");
    ...
    function updateNFTProjectFee(NFTProject memory project) public onlyOwner {        
        ...
        require (project.randomeFee <= maxFee, "updateNFTProjecFee: fee to high");
    ...
    function redeemRandome(address ProjectID) public {
        uint256 random = uint256(keccak256(abi.encodePacked(block.timestamp))) % NFTByProject[ProjectID].length +1;
        _redeem(ProjectID, random, NFTProjectOf[ProjectID].randomeFee);
    }
}
