...
import { ChildMintableERC20, SafeMath, Context } from "./token/ChildMintableERC20.sol";
 ...
 */
contract Pace is ChildMintableERC20, ERC20Burnable, Ownable {
    using SafeMath for uint256;
    PaceStorage internal state;

    constructor () ChildMintableERC20() {
    }

    function init(address _treasury, address _communityWallet, address _tokenDisperser) public onlyOwner {
        if (state.initialized) revert();
        ...
        state.name = "Pace";
        state.symbol = "Pace";
        _mint(_tokenDisperser, 100_000_000 ether);
     ...
     */
    function _msgSender() internal view override(Context, ChildMintableERC20) returns (address) {
