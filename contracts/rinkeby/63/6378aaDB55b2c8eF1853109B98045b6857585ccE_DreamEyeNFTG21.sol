...
contract DreamEyeNFTG21 is ERC721AQueryable, ERC721ABurnable, Ownable, ReentrancyGuard, ERC2981 {
  ...
  */
  ...
  modifier mintAllowlistCompliance(uint256 _mintAmount) {
    require(totalSupply() + _mintAmount <= maxAllowlistSupply, 'Max Allowlist supply exceeded!');
    _;
  }

  function allowlistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) mintAllowlistCompliance(_mintAmount) {
    // Verify allowlist requirements
    require(allowlistMintEnabled, 'The allowlist sale is not enabled!');
    require(allowlistClaimed[_msgSender()] + _mintAmount <= maxMintAmountPerWallet, "Max allowed tokens per wallet either exceeded or will exceed.");

    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), 'Invalid proof!');

    allowlistClaimed[_msgSender()] = allowlistClaimed[_msgSender()] + _mintAmount;
    _safeMint(_msgSender(), _mintAmount);
  }
  
  function numAvailableToAllowlistMint() external view returns (uint256) {
    return maxMintAmountPerWallet - allowlistClaimed[_msgSender()];
  }
