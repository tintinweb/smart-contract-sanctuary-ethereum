...
{
    /**
     * @dev Emitted when the token transfer status is changed by `sender`.
     */
    event TokenTransfer(address sender, bool state);
    ...
    uint256 private lastPoapId;
    bool public poapActive;

    mapping(address => uint256) private lastYearMinted;

    function initialize() external initializer {
        __ERC1155_init("https://ipfs.io/QmekEBXjaRn7kYKnnZEtFwW4K8fNfVKujRwwVWHTKMKGQU/");
        __AccessControl_init();
        __UUPSUpgradeable_init();
        _tokenTransfer = false;
        lastPoapId = 100;
        currentPoap = lastPoapId;
    ...
    function startNewPoap(bool active_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        currentPoap = lastPoapId + 1;
        poapActive = active_;
    }

    function isValidPoap(uint256 tokenId_) public view returns (bool) {
        return tokenId_ >= STARTING_POAP_ID && tokenId_ <= lastPoapId;
    }
        ...
        onlyRole(MINTER_ROLE)
    {
        ...
        require(
            receiver_ != address(0),
            "LuckyVR: cannot mint to zero address"
        );
        require(
            balanceOf(receiver_, currentPoap) == 0,
            "LuckyVR: receiver already has current POAP"
        );
    ...
    */
    function mint(
        ...
        uint256 startingYearId = years_ - (lastYearMinted[receiver_] + 1);
        uint256 tokenCount = startingYearId;
        uint256[] memory tokenIds;
        uint256[] memory quantities;
        if (withPoap_ && balanceOf(receiver_, currentPoap) == 0) {
            tokenIds = new uint256[](tokenCount);
            quantities = new uint256[](tokenCount);
            tokenIds[tokenCount - 1] = currentPoap;
            quantities[tokenCount - 1] = 1;
        } else {
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit TokenTransfer(msg.sender, _tokenTransfer);
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            ...
            "LuckyVR: token transfer not allowed"
        );
