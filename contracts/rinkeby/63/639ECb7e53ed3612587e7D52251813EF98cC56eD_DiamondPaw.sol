...
contract DiamondPaw is ERC721Enumerable, Ownable {
    ...
    address private admin = 0xA629E14908F5cE17F3AFA3BAF5F7318d06091362;

    string public constant CONTRACT_NAME = "Diamond Paw";
    bytes32 public constant DOMAIN_TYPEHASH = keccak256("EIP712Domain(string name,uint256 chainId,address verifyingContract)");
    bytes32 public constant MINT_TYPEHASH = keccak256("Mint(address user)");
     ...
     * Mint Token by owner
     */
    function reserveByOwner(address _to) external onlyOwner {
    ...
    * Mints tokens
    */
    function mint(address user, uint8 v, bytes32 r, bytes32 s) external {
        require(mintIsActive, "Sale must be active to mint");

        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes(CONTRACT_NAME)), getChainId(), address(this)));
        bytes32 structHash = keccak256(abi.encode(MINT_TYPEHASH, user));
