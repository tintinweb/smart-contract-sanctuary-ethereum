...
interface INFT {
    ...
    function getOwnerAddress(uint256 tokenId) external view returns (address payable);
}
...
contract collection {
    ...
    mapping(address => collectionDetail) public collectionMapping;
    ...
    function createCollection(string memory name, string memory symbol)
        ...
        returns (address)
    {
        NFT nftAddress = new NFT(msg.sender, name, symbol);
        collectionMapping[address(nftAddress)] = collectionDetail(
            name,
            symbol,
            msg.sender
        );
        contractAddress[msg.sender].push(address(nftAddress));
        // return collectionMapping[msg.sender];
    }

    function getCollectionName(address contractAddres)
        public
        view
        returns (string memory)
    {
        return collectionMapping[contractAddres].name;
    }

    function getCollectionSymbol(address contractAddres)
        public
        view
        returns (string memory)
    {
        return collectionMapping[contractAddres].symbol;
    }

    function getCollectionOwner(address contractAddres)
        public
        view
        returns (address)
    {
        return collectionMapping[contractAddres].owner;
    }

    function getNFTAddress() public view returns (address[] memory) {
        return contractAddress[msg.sender];
    }
}