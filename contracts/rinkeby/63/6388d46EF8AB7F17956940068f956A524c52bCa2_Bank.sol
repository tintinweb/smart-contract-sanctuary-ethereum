...
contract Bank {
    ...
    function deposit(uint amount) external {
        require (rewardPool1 > 0, "Please wait for the rewardPool to get filled");
    ...
    function withdraw() external {
        ...
        require(block.timestamp > _startTime + 2*_deferral, "Lock period has not yet passed");
        ...
        if (block.timestamp > _startTime + 2*_deferral && block.timestamp < _startTime + 3*_deferral)
        {
            phase = 1;
