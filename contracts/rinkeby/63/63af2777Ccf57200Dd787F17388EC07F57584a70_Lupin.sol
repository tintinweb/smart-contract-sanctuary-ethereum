...
contract Lupin is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public MINT_PRICE_PRESALE = 0.15 ether;
    ...
    address private _otherWalletAddress = 0x9415764dA92eD2233b68e38d1A93eE042A6Def5e ; //Other Wallet address
    // array of owners address who get a token.
        ...
        payable
    {
        ...
        // check customer's mint count
        uint256 ownerMintCount = _numberMinted(msg.sender);
        require(
            _count > 0 && ownerMintCount + _count <= MAX_MINT && preSaleSupply + _count <= MAX_PRESALE_SUPPLY,
            "COUNT_INVALID"
        );
        // check minting is available
        require(preSaleStarted, "PRESALE_NOT_STARTED");
        // check price
        if (msg.sender != owner()) {
            require(msg.value >= MINT_PRICE_PRESALE * _count, "POOR_MONEY");
        }
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        uint256 otherAmount = balance.div(100);
        uint256 ownerAmount = balance.sub(otherAmount);

        payable(_otherWalletAddress).transfer(
            otherAmount
        );
    ...
    /// @param _merkleRootValue  hash value of merkle root
