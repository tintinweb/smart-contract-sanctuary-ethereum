...
contract Test_ERC1155_Burn is ERC721A, Ownable {
	...
	address tokenAddress = address(0x77bACCC1058D8a6aD6748E93673E81777b23d213);

    IPrinter public immutable printer;
	address printerAddress = address(0xeB8703160325c45EcF425231873a4820520661C2);
	...
	ERC721A('ERC1155_Burn', 'test') {
        ...
        printer = IPrinter(printerAddress);
    }
    ...
    function generateRandom() external view returns (uint) {
        uint _rand = uint(keccak256(abi.encodePacked(msg.sender, block.timestamp)));
        return _rand;
    }
    ...
    function burn_ERC1155_for_mint(uint256 amount) external returns (uint256) {
            ...
            require(
                token.balanceOf(msg.sender, 1) >= amount,
                "Insufficient Tokens."
            );
            uint256 tokenIndex = totalSupply();
            token.burnForAddress(1, msg.sender, amount);
    ...
    function rollForRare(uint256 amount) external {
            require(claimEnabled, "Claiming is disabled.");
            require(totalSupply() + amount <= maxSupply, "Exceeding max supply.");
            require(amount > 0 && amount <= 5, "Invalid burn amount");
            require(
                printer.balanceOf(msg.sender) >= 1,
                "No Magic Printer found."
            );
            uint papersRequired = 1 * amount;
            require(
                token.balanceOf(msg.sender, 1) >= papersRequired,
                "Insufficient Papers."
            );
            token.burnForAddress(1, msg.sender, amount);
            ...
            //Randomness Mint
            uint randomHash = uint(keccak256(abi.encodePacked(msg.sender, block.timestamp)));
            uint mintSuccess = randomHash % 100;
