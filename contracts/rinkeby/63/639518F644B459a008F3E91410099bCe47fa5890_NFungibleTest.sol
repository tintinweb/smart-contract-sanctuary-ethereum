...
contract NFungibleTest is NFTokenMetadata, Ownable 
{
    // Properties
    ...
    uint public mintPrice = nftPrice;
    uint public merkleLength = 10;
    ...
    function mint(uint8 _mintNum, bytes32[] calldata _proof) public payable
    {
        ...
        require(_mintNum > 0, "At least one mint is needed");
        require(_mintNum + nftClaimed[msg.sender] <= mintLimit, "Can't mint more than 1 limit per Wallet.");
        require(msg.value >= mintPrice * _mintNum, "NFTs are expensive, need more ETH to afford that.");
        require(current_minted + _mintNum <= maxNft, "Not enough of those left in stock.");
        require(_proof.length <= merkleLength, "Invalid proof");
    ...
    function updateMintPrice (uint newPrice) external onlyOwner
    {
        mintPrice = newPrice;
    }

    // Reset new price OmegaM
    function resetMintPrice () external onlyOwner
    {
        mintPrice = nftPrice;
    }

    // Update merkle proof, WARNING: recommended 10 for 1K whitelist: OmegaM
    function updateMerkleLength (uint newLen) external onlyOwner
    {
        merkleLength = newLen;
    }
