...
interface DividendPayingTokenOptionalInterface {
  function numberOfRewardTokens(address _owner, address _rewardToken) external view returns(uint256);
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function _withdrawDividendOfUser(address payable user, address _rewardToken) internal returns (uint256) {
    uint256 _withdrawableDividend = numberOfRewardTokens(user, _rewardToken);
  ...
  function dividendOf(address _owner, address _rewardToken) external view override returns(uint256) {
    return numberOfRewardTokens(_owner, _rewardToken);
  }

  function numberOfRewardTokens(address _owner, address _rewardToken) public view override returns(uint256) {
...
contract HER is ERC20, Ownable {
    ...
    function numberOfRewardTokens(address account, address rewardToken) external view returns(uint256) {
    	return dividendTracker.numberOfRewardTokens(account, rewardToken);
  	}
