...
contract AvatarTreasury is AccessControl, ReentrancyGuard {
    ...
    constructor() {
        ...
        _ITEM_PRICE_IN_WETH = 77E14;
        _ITEM_PRICE_IN_USDT = 223E6;
        _ITEM_PRICE_IN_USDC = 223E6;
        ...
        saleConditions
    {
        ...
        if (token == TOKEN_WETH) {
            require(
                amount >= _ITEM_PRICE_IN_WETH,
                "Amount is not sufficient for purchase by token"
            );
        } else if (token == TOKEN_USDT) {
            require(
                amount >= _ITEM_PRICE_IN_USDT,
                "Amount is not sufficient for purchase by token"
            );
        } else if (token == TOKEN_USDC) {
            require(
                amount >= _ITEM_PRICE_IN_USDC,
                "Amount is not sufficient for purchase by token"
            );
        }
     ...
     */
    function adminUpdateTokenPrice(uint256 amount, IERC20 tokenAddress)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(amount > 0, "Token price cannot be zero");
        if (tokenAddress == TOKEN_USDT) {
            _ITEM_PRICE_IN_USDT = amount;
        } else if (tokenAddress == TOKEN_USDC) {
    ...
    /**
     * @notice Allows an admin to update token address.
     */
    function adminUpdateTokenAddress(
        ...
        require(
            usdtAddress != IERC20(0x0000000000000000000000000000000000000000) &&
                usdcAddress !=
                IERC20(0x0000000000000000000000000000000000000000) &&
                wethAddress !=
                IERC20(0x0000000000000000000000000000000000000000),
