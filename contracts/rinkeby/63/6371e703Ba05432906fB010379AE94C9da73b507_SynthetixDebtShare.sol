...
contract SynthetixDebtShare is Owned, MixinResolver, ISynthetixDebtShare {
    ...
    function balanceOfOnPeriod(address account, uint256 periodId)
        public
        view
        returns (uint256)
    {
        ...
        int256 i;
        for (
            i = int256(accountPeriodHistoryCount) - 1;
    ...
    function sharePercentOnPeriod(address account, uint256 periodId)
        public
        view
        returns (uint256)
    {
    ...
    function _increaseBalance(address account, uint256 amount) internal {
        ...
        if (accountBalanceCount == 0) {
            ...
            uint128 newAmount = uint128(
                uint256(balances[account][accountBalanceCount - 1].amount).add(
                    amount
                )
            );
    ...
    function _deductBalance(address account, uint256 amount) internal {
        ...
        uint128 newAmount = uint128(
            uint256(balances[account][accountBalanceCount - 1].amount).sub(
                amount
            )
        );
