...
contract BillyBoys is ERC721A, PaymentSplitter, Ownable {
    ...
    constructor(
        ...
        Metadata[] memory metadataValues, 
        ...
        for(uint i = 0; i < 200; i++) {
            tokenURIs[i + 7] = metadataValues[i];
            minted[metadataHashes[i]] = true;
        }
        _safeMint(msg.sender, 206);  
    }

    function create(
        uint16[8] calldata values
        ) external payable {
        //Don't mint if over supply
        require(totalSupply() + 1 <= maxSupply, "MAX_SUPPLY");
        //Fetch price for given traits and return price
        uint256 price = getPrice(values);
        //Check user has sent enough ETH
