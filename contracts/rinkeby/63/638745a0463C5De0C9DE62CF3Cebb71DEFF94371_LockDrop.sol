...
contract LockDrop is TokenRecoveryUpgradeable, GuardableUpgradeable {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;

    event DelegationAdded(bytes32 indexed poolId, uint256 amount, address indexed sender);
    event PoolEnabled(bytes32 indexed poolId, bytes32 requiredAccess, address indexed sender);
    event PoolDisabled(bytes32 indexed poolId, address indexed sender);
    ...
    bytes32 public constant GLOBAL_ADMIN_ROLE = keccak256('LockDrop.GLOBAL_ADMIN_ROLE');
    ...
    function initialize(IERC20 _asset) external initializer {
        address[] memory disabledAssets = new address[](1);
        disabledAssets[0] = address(_asset);

        __Guardable__init();
        __TokenRecovery__init(disabledAssets);
    ...
    function totalDelegators(bytes32 poolId) external view returns (uint256) {
        return _pools[poolId].delegators.length();
    }

    function scanDelegators(bytes32 poolId, uint256 startFrom, uint256 totalToTake) external view returns (address[] memory delegators) {
        Pool storage _pool = _pools[poolId];

        uint256 totalDelegators = _pool.delegators.length();
        uint256 length = startFrom + totalToTake <= totalDelegators ? totalToTake : totalDelegators - startFrom;

        delegators = new address[](length);
        for (uint i = 0; i < length; i++) {
            delegators[i] = _pools[poolId].delegators.at(i + startFrom);
        }
    }

    function enablePool(bytes32 poolId, bytes32 requiredAccess) external onlyAdmin {
        _pools[poolId].enabled = true;
        _pools[poolId].requiredAccess = requiredAccess | GuardHelper.DEFAULT_ACCESS; // Minimum default access
        emit PoolEnabled(poolId, requiredAccess, _msgSender());
    }

    function disablePool(bytes32 poolId) external onlyAdmin {
        require(_pools[poolId].enabled, 'LockDrop: POOL NOT ENABLED');
        _pools[poolId].enabled = false;
        emit PoolDisabled(poolId, _msgSender());
    }

    function sendFunds(address token, address to, uint256 amount) external onlyAdmin {
    ...
    function delegate(bytes32 poolId, uint256 amount) external {
        ...
        require(_pool.enabled, 'LockDrop: POOL DISABLED');
        require(amount > 0, 'LockDrop: ZERO AMOUNT');
        require(SOMA.guard().check(sender, _pool.requiredAccess));

        asset.safeTransferFrom(sender, address(this), amount);

        _pool.balances[sender] += amount;
        _pool.totalBalance += amount;
        _pool.delegators.add(sender);

        emit DelegationAdded(poolId, amount, _msgSender());
    }

    function forwardDelegation(bytes32 poolId, address account, address target) external returns (bytes memory) {
        bytes memory data = abi.encodeWithSignature(
            'updateDelegation(bytes32,address,uint256)',
            poolId,
            account,
            _pools[poolId].balances[account]
        );
