...
contract MultiChainGang is ERC721URIStorage, Operatorable, ReentrancyGuard {
    ...
    //setTokenUri on an item in q for tokenid will pop it out of q
    struct UpgradeInQ {
        uint256 _tokenID_;
        string[] inputBadges;
        string upgradeType;
    ...
    //Upgrade count for a token id - repeat of nftinfo get rid of this later
     ...
     */
    function setTokenURI(uint256 tokenId, string memory _tokenURI, uint256 qId) public onlyOperator {
        //create a check here that the tokenId is in the queue as an upgrade, otherwise it fails
        // require(upgradesInQ[qId] || "MultiChainGang: QUEUE_ID_NOT_VALID");
        // require(upgradesInQ[qId]._tokenID_ = tokenId || "MultiChainGang: WRONG_QUEUEID_FOR_TOKENID");
        super._setTokenURI(tokenId, _tokenURI);
        //after setting the tokenUri, then pop the tokenId from the upgradeQueue
        //actually set the isDeleted parameter to true
        // _popQId(qId);
     ...
     */
    function mintNFT(address recipient, string memory _tokenURI, string[] memory _badges, uint256 _upgradeCount, string memory _upgradeRank) public onlyOperator returns (uint256) {
    ...
    function upgradeNFT(uint256 onChainTokenId, uint256 offChainTokenId, string memory offChainChainName, string[] memory _badges, string memory _upgradeRank, uint256 _upgradeCount, string memory _upgradeType, address mintToAddress) public onlyOperator {
        //check mintToAddres is owner of onChainTokenId or this is done offchain by operator and then we need mintToAddress
        //could also have signature done by holder and passed in by operator in a metatransaction

        // require(ownerOf(onChainTokenId) = mintToAddress || "MultiChainGang: #MINT_ADDRESS_DOES_NOT_OWN_UPGRADE_INPUT");
        //check sender is owner of offChainTokenId [may need to be done offchain by operator before this transaction, making this an operatorOnly tx]
    ...
    //     /**
    //  * @dev Remove the given queue item from queueIds.
    //  *
    //  * @param _qId _qId to remove
    //  */
    // function _popQId(uint256 _qId) internal {
    //     uint256[] storage _queueIds = upgradesInQ;
    //     for (uint256 i = 0; i < _queueIds.length; i++) {
    //         if (_queueIds[i] == _qId) {
    //             if (i != _queueIds.length - 1) {
    //                 _queueIds[i] = _queueIds[_queueIds.length - 1];
    //             }
    //             _queueIds.pop();
    //             break;
    //         }
