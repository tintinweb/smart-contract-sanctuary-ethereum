...
contract OXYGEN is ERC721, Ownable {
	...
	constructor(string memory initUnrevealedUriPrefix) ERC721("OXYGEN", "O2") {
		paused = true;
		revealed = false;
		whitelistOnly = true;
	...
	function mint(uint256 amount) external payable {
		uint256 newSupply = _supply.current() + amount;

		require(amount > 0 && amount <= maxMintAmountPerTX, "OXYGEN: invalid mint amount");
		require(newSupply <= maxSupply, "OXYGEN: max token supply exceeded");
		require(!paused, "OXYGEN: minting is paused");

		if (whitelistOnly) {
			require(isAddressWhitelisted(_msgSender()), "OXYGEN: caller is not whitelisted");
			require(msg.value >= presaleCost * amount, "OXYGEN: insufficient ether");
        } else {
			require(msg.value >= cost * amount, "OXYGEN: insufficient ether");
		}
	...
	function airDrop(address[] calldata addresses, uint8[] calldata amounts) external onlyOwner {
		require(addresses.length == amounts.length, "OXYGEN: invalid function arguments");
		require(addresses.length > 0, "OXYGEN: invalid function arguments");
	...
	function setPresaleSupply(uint256 newAmount) external onlyOwner {
		require(newAmount < maxSupply, "OXYGEN: presale supply must be smaller than the total supply");
