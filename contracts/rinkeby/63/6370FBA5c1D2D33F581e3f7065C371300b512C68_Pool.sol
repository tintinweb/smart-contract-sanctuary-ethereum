...
import {PayLaterLogic} from "../libraries/logic/PayLaterLogic.sol";
...
contract Pool is ReentrancyGuard, VersionedInitializable, PoolStorage, IPool {
    ...
    function buyWithCredit(
        ...
        DataTypes.OrderInfo memory orderInfo = MarketplaceLogic.getAskOrderInfo(
            marketplaceId,
            data,
            weth
        );
        ...
        return
            PayLaterLogic.executeBuyWithCredit(
                _reserves,
                _reservesList,
                _usersConfig[onBehalfOf],
                _marketplaces,
                DataTypes.ExecuteBuyWithCreditParams({
    ...
    function acceptBidWithCredit(
        ...
        DataTypes.OrderInfo memory orderInfo = MarketplaceLogic.getBidOrderInfo(
            marketplaceId,
            data
        );
        require(orderInfo.taker == onBehalfOf, Errors.INVALID_ORDER_TAKER);
        return
            PayLaterLogic.executeAcceptBidWithCredit(
                _reserves,
                _reservesList,
                _usersConfig[orderInfo.maker],
                _marketplaces,
                DataTypes.ExecuteAcceptBidWithCreditParams({
                    marketplaceId: marketplaceId,
                    data: data,
                    payLater: payLater,
                    orderInfo: orderInfo,
    ...
    /// @inheritdoc IPool
    function setMarketplace(bytes32 marketplaceId, address marketplace)
        external
        virtual
        override
        onlyPoolAdmin
    {
        _marketplaces[marketplaceId] = marketplace;
    }
...
library PayLaterLogic {
    ...
    function executeBuyWithCredit(
        ...
        // 1. extract payLater portion from reserves
        // if it's ETH, then exchange WETH to ETH
        for (uint256 i = 0; i < params.payLater.tokens.length; i++) {
            DataTypes.ReserveData storage reserve;
            address token = params.payLater.tokens[i];
            uint256 amount = params.payLater.amounts[i];
            bool isETH = token == address(0);

            if (isETH) {
                reserve = reservesData[params.WETH];
            } else {
                reserve = reservesData[token];
            }
            ...
            IPToken(reserve.xTokenAddress).transferUnderlyingTo(agent, amount);

            if (isETH) {
                // No re-entrancy because it sent to our contract address
                IWETH(params.WETH).withdraw(amount);
                // Set to WETH for minting debt
                params.payLater.tokens[i] = params.WETH;
            }
        }

        // 2. transfer funds from taker to pool contract
        // and then approve payNow + payLater portion to marketplace contract
        // and call marketplace contract
        address marketplace = MarketplaceLogic.getMarketplace(
            marketplaces,
        ...
        for (uint256 i = 0; i < params.orderInfo.consideration.length; i++) {
            ...
            if (i < params.payLater.tokens.length) {
                require(
                    params.payLater.tokens[i] == item.token,
                    Errors.PAYLATER_ORDER_DO_NOT_MATCH
                );
                actualPayment -= params.payLater.amounts[i];
            }
            ...
            IERC20(item.token).approve(marketplace, 0);
        ...
        // TODO: verify msg.value
        MarketplaceLogic.matchAskWithTakerBid(
            marketplaces,
            params.marketplaceId,
            params.data,
            value
        ...
        // 3. supply NFT on behalf of taker and assume the pool
        // contract already gets the NFT
        //
        // if it's NToken we just ignore the supply
        //
        // if no credit has been used, then dont collaterize the NFT
        for (uint256 i = 0; i < params.orderInfo.offer.length; i++) {
            OfferItem memory item = params.orderInfo.offer[i];
            require(
                item.startAmount == item.endAmount && item.startAmount == 1,
                Errors.INVALID_MARKETPLACE_ORDER
            );
            require(
            ...
            SupplyLogic.executeSupplyERC721(
                ...
                DataTypes.ExecuteSupplyERC721Params({
                    asset: token,
                    tokenData: tokenData,
                    onBehalfOf: params.orderInfo.taker,
                    actualSpender: agent,
        ...
        // 4. Borrow (mint debt) on behalf of taker without extracting
        ...
        for (uint256 i = 0; i < params.payLater.tokens.length; i++) {
            BorrowLogic.executeBorrow(
                ...
                userConfig,
                DataTypes.ExecuteBorrowParams({
                    asset: params.payLater.tokens[i],
                    user: params.orderInfo.taker,
                    onBehalfOf: params.orderInfo.taker,
                    amount: params.payLater.amounts[i],
                    interestRateMode: DataTypes.InterestRateMode(
    ...
    function executeAcceptBidWithCredit(
        ...
        address agent = address(this);
        address maker = params.orderInfo.maker;
        ...
        for (uint256 i = 0; i < params.payLater.tokens.length; i++) {
            ...
            ValidationLogic.validateFlashloanSimple(reserve);
            IPToken(reserve.xTokenAddress).transferUnderlyingTo(maker, amount);
        }

        MarketplaceLogic.matchBidWithTakerAsk(
            marketplaces,
            params.marketplaceId,
            params.data
        );
...
contract PoolStorage {
    ...
    mapping(bytes32 => address) internal _marketplaces;
}
