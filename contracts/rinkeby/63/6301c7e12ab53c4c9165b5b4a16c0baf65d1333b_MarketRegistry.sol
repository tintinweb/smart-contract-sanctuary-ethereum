...
contract GenieSwap is Ownable, ReentrancyGuard {
    ...
    function _trade(
        ...
        uint256 len = _tradeDetails.marketId.length;
        for(uint256 i = 0; i < len; i++){
            // get market details
            //(address _proxy, bool _isLib, bool _isActive) = marketRegistry.markets(_tradeDetails[i].marketId);
            (address _proxy, bool _isLib, bool _isActive) = marketRegistry.markets(_tradeDetails.marketId[i]);
            // market should be active
            require(_isActive, "_trade: InActive Market");
            // execute trade
            (bool success, ) = _isLib
               ? _proxy.delegatecall(_tradeDetails.tradeData[i])
               : _proxy.call{value:_tradeDetails.value[i]}(_tradeDetails.tradeData[i]);
    ...
    function multiAssetSwap(
        uint256[] calldata marketId,
        uint256[] calldata value,
        bytes[] calldata tradeData
    ) payable external isOpenForTrades nonReentrant {

        uint256 len = marketId.length;
        require(value.length == len && tradeData.length == len,"LENGTH_MISMATCH");
        // collect fees
        _collectFee();

        // execute trades
        MarketRegistry.TradeDetails memory _tradeDetails = MarketRegistry.TradeDetails(marketId,value,tradeData);
        uint256 total;
        for(uint256 i = 0; i < len; i++){
            total = total + value[i];
        }
        require(total <= msg.value,"Not enough tokens to buy");
