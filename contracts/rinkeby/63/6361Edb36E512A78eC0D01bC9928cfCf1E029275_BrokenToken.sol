...
contract BrokenToken is ERC20, Ownable, ReentrancyGuard {
    ...
    function getPending(address account) internal view returns (uint256) {
        if (START == 0) {
            ...
            return
                (BrokenClub.balanceOf(account) *
                    BASE_RATE *
                    (block.timestamp -
                        (
                            lastClaimDatetime[account] > START
                                ? lastClaimDatetime[account]
                                : START
                        ))) / 1 days;
        }
    }
    ...
    function redeem(address from, uint256 amount) external onlyAllowed {
        //TODO: send the amount back to pool
        require(
            msg.sender == from || allowed[msg.sender],
            "Caller not allowed"
        );
