...
contract NFT is ERC721, EIP712Whitelist {
    ...
    uint256 public reserved = 77;
    uint256 public genesisPrice = 0.1 ether;
    uint256 public price = 0.077 ether;

    string private _baseTokenURI;
    address private _withdrawer;

    // TODO: Change bool to uint
    mapping(address => bool) private _freeMinter;

    constructor(string memory baseURI, address withdrawer, address signer) 
        ERC721("TestERC721", "ERC721") 
        EIP712Whitelist() 
    {
        ...
        requiresWhitelist(signature, nonce)
    {
        ...
        */
        ...
        require( _tokenIdCounter.current() + num <= maxSupply - reserved, "Exceeds maximum token supply" );
        ...
        returns (uint256)
    {
        ...
        * - The contract is constructed to optimize gas.
        */
        require(_freeMinter[msg.sender], "No free mint allowed for your address");
        require(_tokenIdCounter.current() < maxSupply - reserved, "Exceeds maximum token supply" );

        if (genesisPhase) {
            require(_tokenIdCounter.current() < genesisSupply, "Exceeds genesis token supply" );
        }

        _freeMinter[msg.sender] = false;
        return _mintOne(msg.sender);
    }
        ...
        returns (uint256)
    {
        ...
        require( _tokenIdCounter.current() < reserved, "Exceeds reserved tokens supply" );

        uint256 newTokenId = _mintOne(_to);
        reserved--;
    ...
    function setFreeMinter(address freeMinter)
        external
        onlyOwner
    {
        _freeMinter[freeMinter] = true;
    }
    ...
    function withdrawAll() 
        ...
        onlyOwner 
    {
        require(payable(_withdrawer).send(address(this).balance));
    }
}
