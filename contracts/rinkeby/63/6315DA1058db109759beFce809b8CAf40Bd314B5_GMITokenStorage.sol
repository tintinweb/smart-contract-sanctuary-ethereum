...
import { GMITokenAutoProxy } from './GMITokenAutoProxy.sol';
import { NonReentrant } from './NonReentrant.sol';

import { LibClaimAirdrop } from './libraries/LibClaimAirdrop.sol';
import { LibClaimListing } from './libraries/LibClaimListing.sol';
...
import { IGMITokenStorage } from './interfaces/IGMITokenStorage.sol';

contract GMITokenStorage is StorageBase, IGMITokenStorage {
    address private gmiFarmingProxy; // Farming implementation can mint
    ...
    address private airdropService; // Signs GMI airdrop rewards claims

    address private listingService; // Signs GMI listing rewards claims

    mapping(address => bool) submittedAirdropClaims;

    mapping(address => uint256) private lastClaimNonce;

    constructor(
        address _gmiFarmingProxy,
        address _airdropService,
        address _listingService,
        string memory _name,
        string memory _symbol,
        uint8 _decimals
    ) public {
        gmiFarmingProxy = _gmiFarmingProxy;
    ...
    function getGMIFarmingProxy() external view returns (address _gmiFarmingProxy) {
        _gmiFarmingProxy = gmiFarmingProxy;
    }
    ...
    function getLastClaimNonce(address _user) external view returns (uint256 _lastClaimNonce) {
        _lastClaimNonce = lastClaimNonce[_user];
    }
    ...
    function hasSubmittedAirdropClaim(address _user)
        external
        view
        returns (bool _hasSubmittedAirdropClaim)
    {
        _hasSubmittedAirdropClaim = submittedAirdropClaims[_user];
    }
    ...
    function setGMIFarmingProxy(address _gmiFarmingProxy) external requireOwner {
        gmiFarmingProxy = _gmiFarmingProxy;
    }
    ...
    function setListingService(address _listingService) external requireOwner {
        listingService = _listingService;
    }

    function setSubmittedAirdropClaim(address _user, bool _submittedAirdropClaim)
        external
        requireOwner
    {
        submittedAirdropClaims[_user] = _submittedAirdropClaim;
    }
...
contract GMIToken is NonReentrant, GMITokenAutoProxy, IGMIToken {
    // Data for migration
    //---------------------------------
    GMITokenStorage public gmiTokenStorage;
    //---------------------------------

    modifier onlyGMITokenOwner() {
        require(_callerAddress() == owner, 'GMIToken: FORBIDDEN');
        _;
    }

    modifier onlyGMITokenOwnerOrFarming() {
        require(
            _callerAddress() == owner || _callerAddress() == getGMIFarmingImpl(),
            'GMIToken: FORBIDDEN'
        );
        _;
    }
    ...
    constructor(
        address _proxy, // If set to address(0), GMITokenGovernedProxy will be deployed by GMITokenAutoProxy
        address _gmiFarmingProxy,
        address _airdropService,
        address _listingService,
        string memory _name,
        string memory _symbol,
        uint8 _decimals
    ) public GMITokenAutoProxy(_proxy, this) {
        // Deploy GMI Token storage
        gmiTokenStorage = new GMITokenStorage(
            _gmiFarmingProxy,
            _airdropService,
            _listingService,
            _name,
    ...
    function setSporkProxy(address payable _sporkProxy) public onlyGMITokenOwner {
        IGMITokenGovernedProxy(proxy).setSporkProxy(_sporkProxy);
    }
    ...
    function destroy(IGovernedContract _newImpl) external requireProxy {
        gmiTokenStorage.setOwner(_newImpl);
    ...
    // Getter function for GMI farming implementation contract
    //
    function getGMIFarmingImpl() public view returns (address _gmiFarmingImpl) {
        _gmiFarmingImpl = address(
            IGovernedProxy(address(uint160(gmiTokenStorage.getGMIFarmingProxy()))).impl()
        );
    }
    ...
    function name() external view returns (string memory _name) {
        _name = gmiTokenStorage.getName();
    }

    function symbol() external view returns (string memory _symbol) {
        _symbol = gmiTokenStorage.getSymbol();
    }

    function decimals() external view returns (uint8 _decimals) {
        _decimals = gmiTokenStorage.getDecimals();
    }

    function getGMIFarmingProxy() external view returns (address _gmiFarmingProxy) {
        _gmiFarmingProxy = gmiTokenStorage.getGMIFarmingProxy();
    }

    function getAirdropService() external view returns (address _airdropService) {
        _airdropService = gmiTokenStorage.getAirdropService();
    }
    ...
    function hasSubmittedAirdropClaim(address _recipient)
        ...
        returns (bool _hasSubmittedAirdropClaim)
    {
        _hasSubmittedAirdropClaim = gmiTokenStorage.hasSubmittedAirdropClaim(_recipient);
    }

    function getListingService() external view returns (address _listingService) {
        _listingService = gmiTokenStorage.getListingService();
    }

    function getLastClaimNonce(address _user) external view returns (uint256 _lastClaimNonce) {
        _lastClaimNonce = gmiTokenStorage.getLastClaimNonce(_user);
    }

    // Setter functions
    //
    function setName(string calldata _name) external onlyGMITokenOwner {
        gmiTokenStorage.setName(_name);
    }

    function setSymbol(string calldata _symbol) external onlyGMITokenOwner {
        gmiTokenStorage.setSymbol(_symbol);
    }

    function setDecimals(uint8 _decimals) external onlyGMITokenOwner {
        gmiTokenStorage.setDecimals(_decimals);
    }

    function setGMIFarmingProxy(address _gmiFarmingProxy) external onlyGMITokenOwner {
        gmiTokenStorage.setGMIFarmingProxy(_gmiFarmingProxy);
    }

    function setAirdropService(address _airdropService) external onlyGMITokenOwner {
        gmiTokenStorage.setAirdropService(_airdropService);
    }

    function setListingService(address _listingService) external onlyGMITokenOwner {
        gmiTokenStorage.setListingService(_listingService);
    }

    // Mint/burn functions
    //
    function mint(address recipient, uint256 amount) external onlyGMITokenOwnerOrFarming {
        _mint(recipient, amount);
    }

    function burn(address recipient, uint256 amount) external onlyGMITokenOwner {
        _burn(recipient, amount);
    }

    // GMI listing rewards claim function
    function claimListingRewards(
        ...
        // Get listing claim recipient
        address recipient = _callerAddress();
        // Make sure claim has not been processed yet
        require(
            lastClaimNonce == gmiTokenStorage.getLastClaimNonce(recipient),
            'GMIToken: invalid lastClaimNonce value'
        );
        // Check that lastClaimNonce < claimNonce
        require(
            lastClaimNonce < claimNonce,
            'GMIToken: claimNonce must be larger than lastClaimNonce'
        );
        // Validate listing claim
        LibClaimListing.validateClaim(
            recipient, // Listing claim recipient address
            claimAmount, // Listing rewards claim amount
            lastClaimNonce, // Recipient's last claim nonce
            claimNonce, // Recipient's current claim nonce
            listingServiceSignature, // Listing claim signature from listing service
            proxy, // Verifying contract address
            gmiTokenStorage.getListingService() // Listing service address
        );
        // Update recipient's last claim nonce to current claim nonce
        gmiTokenStorage.setLastClaimNonce(recipient, claimNonce);
        // Mint tokens
        _mint(recipient, claimAmount);
        // Emit ListingRewardsClaimed event
        IGMITokenGovernedProxy(proxy).emitListingRewardsClaimed(
            recipient,
            claimAmount,
            lastClaimNonce,
            claimNonce,
    ...
    // GMI airdrop claim function
    function claimAirdropRewards(uint256 claimAmount, bytes calldata airdropServiceSignature)
        external
        noReentry
    {
        // Get airdrop claim recipient
        address recipient = _callerAddress();
        // Verify that airdrop claim has not already been validated
        require(
            gmiTokenStorage.hasSubmittedAirdropClaim(recipient) == false,
            'GMIToken: airdrop claim has already been submitted'
        );
        // Validate airdrop claim
        LibClaimAirdrop.validateClaim(
            recipient, // Airdrop claim recipient address
            claimAmount, // Airdrop rewards claim amount
            airdropServiceSignature, // Airdrop claim signature from airdrop service
            proxy, // Verifying contract address
            gmiTokenStorage.getAirdropService() // Airdrop service address
        );
        // Register airdrop claim as submitted and validated
        gmiTokenStorage.setSubmittedAirdropClaim(recipient, true);
        // Mint tokens
        _mint(recipient, claimAmount);
        // Emit AirdropRewardsClaimed event
        IGMITokenGovernedProxy(proxy).emitAirdropRewardsClaimed(
            recipient,
            claimAmount,
...
library LibEIP712 {
    ...
    // so that users signatures remain valid when we upgrade the GMIToken contract
...
library LibClaimListing {
    bytes32 constant CLAIM_TYPEHASH =
        keccak256(
            'Claim(address recipient,uint256 amount,uint256 lastClaimNonce,uint256 claimNonce)'
        );
    ...
    function hashClaim(
        ...
        return keccak256(abi.encode(CLAIM_TYPEHASH, recipient, amount, lastClaimNonce, claimNonce));
    }
    ...
    function validateClaim(
        ...
        uint256 lastClaimNonce,
        uint256 claimNonce,
        bytes memory listingServiceSignature,
        address verifyingContractProxy,
        address listingService
    ) internal pure {
        // Generate EIP712 hashStruct of listingClaim
        bytes32 hashStruct = hashClaim(recipient, amount, lastClaimNonce, claimNonce);
        ...
        if (
            ...
            revert('LibClaimListing: EIP-712 listing service signature verification error');
        }
    }
}
...
library LibClaimAirdrop {
    bytes32 constant CLAIM_TYPEHASH = keccak256('Claim(address recipient,uint256 amount)');

    function hashClaim(address recipient, uint256 amount) internal pure returns (bytes32) {
        return keccak256(abi.encode(CLAIM_TYPEHASH, recipient, amount));
    }
    ...
    function validateClaim(
        ...
        bytes32 hashStruct = hashClaim(recipient, amount);
        // Verify claim EIP712 hashStruct signature
        if (
            LibSignature.recover(
                LibEIP712.hashEIP712Message(hashStruct, verifyingContractProxy),
...
interface IGMITokenStorage {
    ...
    function setSubmittedAirdropClaim(address _user, bool _submittedAirdropClaim) external;
...
import { GMITokenGovernedProxy } from './GMITokenGovernedProxy.sol';

contract GMITokenAutoProxy is GovernedERC20 {
    constructor(address _proxy, IGovernedContract _impl) public GovernedERC20(_proxy) {
        if (_proxy == address(0)) {
            _proxy = address(new GMITokenGovernedProxy(_impl));
        }
        proxy = _proxy;
    }
}
