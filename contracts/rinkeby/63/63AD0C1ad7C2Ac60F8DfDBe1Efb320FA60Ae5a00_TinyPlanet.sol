...
contract TinyPlanet is Ownable, ERC721A, ReentrancyGuard
{
    ...
    bool private _isPresaleOn = false;
    ...
    mapping (address => uint256) private whitelistMint;
    ...
    {
        _isPresaleOn = !_isPresaleOn;
    }
    
    function togglePublicPhase() external onlyOwner
    {
        _isPublicPhase = !_isPublicPhase;
    }
    ...
    function verifySignature(bytes memory _signature) internal view returns (uint256)
    {
    ...
    function buyPresalePlanet(bytes memory _signature, uint256 _count) external payable nonReentrant
    {
        require(_isPaused == false, "Mint phase is paused");
        require(_isPresaleOn == true, "Presale isn't active");
        uint256 signedResult = verifySignature(_signature);
        require(signedResult > 0, "You are Unauthorized");
        require(_count > 0, "You must mint at least 1 NFT");
        require(presaleMintList[msg.sender] < 5, "You minted your max presale quantity, come back on public sale for more!");
        require(_tokenCounter + _count < COLLECTION_SIZE, "Max supply has been reached");

        if(signedResult == 1)
        {
            require(msg.value == PUBLIC_PRICE * (_count - 1), "Incorrect Price");
        }
        else if(signedResult == 2)
        {
            require(msg.value == WL_PRICE + (PUBLIC_PRICE * (_count - 1)), "Incorrect Price");
        }
    ...
    function devMint(uint256 _count) external onlyOwner
    {
        ...
        for(uint i = 0; i < _count; i++)
        {
            generatePlanetData(abi.encodePacked(msg.sender, _tokenCounter, block.difficulty, block.timestamp, i), _tokenCounter);
            _tokenCounter++;
        }
        _safeMint(msg.sender, _count);
    }

    function buyPlanet(uint256 _count) external payable
    {
        require(_isPaused == false, "Mint phase is paused");
        require(_isPublicPhase == true, "Public sale isn't active");
        require(_count > 0, "You must mint at least 1 NFT");
        require(_count <= MAX_BATCH_MINT, "Too many NFTs for 1 tx");
        require(msg.value == PUBLIC_PRICE * _count, "Incorrect Price");
        require(_tokenCounter + _count < COLLECTION_SIZE, "Mint exceeds supply");
        require(quantityMinted[msg.sender] <= MAX_PER_WALLET, "You minted your max presale quantity, come back on public sale for more!");
        require(_tokenCounter + _count < COLLECTION_SIZE, "Max supply has been reached");

        quantityMinted[msg.sender] += _count;
        for(uint i = 0; i < _count; i++)
        {
            generatePlanetData(abi.encodePacked(msg.sender, _tokenCounter, block.timestamp, _count, i), _tokenCounter);
            _tokenCounter++;
        }
        _safeMint(msg.sender, _count);
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory)
    {
        ...
        if(_isRevealed)
        {
            string memory image = string(abi.encodePacked(_baseTokenURI, '/', uint2str(_tokenId + 1), '.png'));
            bytes memory data1 = abi.encodePacked('{ "description": "', planetsData[_tokenId].desc, '",', 
                                            ' "image": "', image, '",',
                                            ' "name": "', planetsData[_tokenId].name, '", "attributes": [',
                                            '{ "trait_type": "Life Conditions", "value": "', LifeConditions[planetsData[_tokenId].properties[0]] ,'" }, ',
                                            '{ "trait_type": "Metal Resources", "value": "', MetalsGasOrganic[planetsData[_tokenId].properties[1]] ,'" }, ',
                                            '{ "trait_type": "Gas Resources", "value": "', MetalsGasOrganic[planetsData[_tokenId].properties[2]] ,'" }, ');

            bytes memory data2 = abi.encodePacked(
                                            '{ "trait_type": "Organic Resources", "value": "', MetalsGasOrganic[planetsData[_tokenId].properties[3]] ,'" }, ',
                                            '{ "trait_type": "Building Difficulty", "value": "', BuildingDifficulty[planetsData[_tokenId].properties[4]] ,'" }, ',
                                            '{ "trait_type": "Ringed Planet", "value": "', RingedPlanet[planetsData[_tokenId].properties[5]] ,'" }, ',
                                            '{ "trait_type": "Planet Type", "value": "', PlanetTypes[planetsData[_tokenId].properties[6]] ,'" } ',
                                            ']}');
    ...
    function testSignerRecovery(bytes memory _signature) external view returns (address) 
    {
        return keccak256(
            abi.encodePacked(
    ...
    {
        return uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _offset, _tokenCounter, _expo)));
    }

    function generatePlanetData(bytes memory _offset, uint256 _id) internal
    {
        uint256 random = getrandomSeed(_offset, _id);

        planetsData[_id].name = string(abi.encodePacked('Tiny Planet #', uint2str(_tokenCounter)));
        planetsData[_id].properties[0] = getPercentage(random % 100);
        planetsData[_id].properties[1] = getPercentage((random / 100) % 100);
        planetsData[_id].properties[2] = getPercentage((random / 10000) % 100);
        planetsData[_id].properties[3] = getPercentage((random / 1000000) % 100);
        planetsData[_id].properties[4] = getPercentage((random / 100000000) % 100);
        planetsData[_id].properties[5] = 0;
        planetsData[_id].properties[6] = 0;
    }
    ...
    function updatePlanetTypes(uint256[] memory _tokens, uint8 _id) external onlyOwner
    {
        ...
        {
            planetsData[_tokens[i]].properties[6] = _id;
        }
    }
    ...
    function updatePlanetRings(uint256[] memory _tokens, uint8  _id) external onlyOwner
    {
        for(uint i = 0; i < _tokens.length; i++)
        {
            planetsData[_tokens[i]].properties[5] = _id;
        }
    }
}
