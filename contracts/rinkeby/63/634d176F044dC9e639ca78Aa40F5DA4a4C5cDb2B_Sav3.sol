...
contract Sav3 is Ownable, ERC721A, ReentrancyGuard, Pausable {
	...
	uint256 public reservedQuantity;

	uint256 public usedReservedQuantity;
	...
	function reservedMint(uint256 quantity, address addr) external onlyOwner {
		require(usedReservedQuantity + quantity <= reservedQuantity, "not enough reserved quantity");
		require(totalSupply() + quantity <= collectionSize, "reached max supply");

		_safeMint(addr, quantity);

		usedReservedQuantity += quantity;
	}
}
