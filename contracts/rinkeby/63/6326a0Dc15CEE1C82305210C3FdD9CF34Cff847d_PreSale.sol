...
contract PreSale is Crowdsale, AllowanceCrowdsale, CappedCrowdsale, TimedCrowdsale, PausableCrowdsale, PostDeliveryCrowdsale, Ownable {
    constructor(
        uint256 rate,            // rate, in TKNbits
        address payable wallet,  // wallet to send Ether
        IERC20 token,            // the token
        address tokenWallet,     // wallet that sends token
    ...
    function changeClosingTime(uint256 closingTime) public onlyOwner {
        _changeClosingTime(closingTime);
    }
}
...
contract TimedCrowdsale is Crowdsale {
    ...
    /**
     * Event for crowdsale closing time change
     * @param newClosingTime new closing time
     * @param prevClosingTime old closing time
     */
    event TimedCrowdsaleClosingTimeChanged(uint256 prevClosingTime, uint256 newClosingTime);
    ...
    /**
     * @dev Change closing time.
     * @param newClosingTime Crowdsale closing time
     */
    function _changeClosingTime(uint256 newClosingTime) internal {
        require(!hasClosed(), "TimedCrowdsale: already closed");
        // solhint-disable-next-line max-line-length
        require(newClosingTime > _openingTime, "TimedCrowdsale: new closing time is before current opening time");

        emit TimedCrowdsaleClosingTimeChanged(_closingTime, newClosingTime);
