...
contract R0N1World is ERC721Enumerable, Ownable, Pausable, ReentrancyGuard {
  ...
  bool public walletCapEnabled = true;
  ...
  uint256 public constant MAX_GIVEAWAY = 300;
  ...
  /****************************************************
    STORAGE
  /****************************************************/
  mapping(address => uint256) allowListTokensMintedByAddress;
  mapping(address => uint256) publicTokensMintedByAddress;
  ...
  function mintR0N1(uint256 _qty) public payable whenNotPaused nonReentrant {
    ...
    require(publicSaleEnabled, "Public sale not started yet");
    ...
    if (walletCapEnabled) {
      require(publicTokensMintedByAddress[msg.sender] + _qty <= MAX_SUPPLY, "Reached wallet cap");
      publicTokensMintedByAddress[msg.sender] += _qty;
    }
  ...
  function allowListMintR0N1(uint256 _qty, uint _freeMint, bytes32[] calldata _proof) public payable whenNotPaused nonReentrant {
    ...
    if (walletCapEnabled) {
      require(allowListTokensMintedByAddress[msg.sender] + _qty <= MAX_SUPPLY, "Reached wallet cap");
  ...
  function withdraw() public onlyOwner {
    PAYABLE_ADDRESS_1.call { value: (address(this).balance * 60) / 100 }("");
  ...
  function tokenURI(uint256 _tokenId) public view override returns (string memory) {
    ...
    return
      bytes(baseTokenURI).length > 0
        ? string(abi.encodePacked(baseTokenURI, _tokenId.toString()))
        : baseTokenURI;
  }
  ...
  function setWalletCapEnabled(bool _walletCapEnabled) public onlyOwner {
    walletCapEnabled = _walletCapEnabled;
  }
