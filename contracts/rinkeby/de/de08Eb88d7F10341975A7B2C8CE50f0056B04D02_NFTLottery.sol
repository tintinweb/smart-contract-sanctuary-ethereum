...
contract NFTLottery is VRFConsumerBaseV2, ERC721Holder {
  ...
  mapping(uint => address) public rewardAddress;
  mapping(uint => uint) public rewardId;

  // We have 2 ways of fulfilling randomness:

  // 1) sending an NFT to one address from list of addresses
  mapping(uint => address[]) public addressRecipients;

  // 2) sending an NFT to one holder from a list of NFT holders
  mapping(uint => address) public nftRecipientAddress;
  // Both start and end are inclusive indices
  mapping(uint => uint) public nftRecipientStart;
  mapping(uint => uint) public nftRecipientEnd;

 constructor(uint64 subscriptionId, address _vrfCoordinator, bytes32 _keyHash, address _link) VRFConsumerBaseV2(vrfCoordinator) {
    COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
    s_owner = msg.sender;
    s_subscriptionId = subscriptionId;
    // https://docs.chain.link/docs/get-a-random-number/
    // https://docs.chain.link/docs/vrf-contracts/#configurations
    vrfCoordinator = _vrfCoordinator; // 0x6168499c0cFfCaCD319c818142124B7A15E857ab (// Rinkeby coordinator.)
    keyHash = _keyHash; // 0xd89b2bf150e3b9e13446986e571fb9cab24b13cea0a43ea20a6049a85cc807cc (gas lane to use) 
    link =  LinkTokenInterface(_link); // 0x01be23585060835e02b77ef475b0cc51aa1e0709 (rinkeby)
  }

  // Assumes the subscription is funded sufficiently.
  function requestRandomWords() internal onlyOwner returns(uint) {
  ...
  function distributeToAddresses(uint256 fee, address[] calldata recipients, 
  address _rewardAddress, uint256 _rewardId) external {
  ...
  function fulfillRandomWords(uint requestId, uint256[] memory randomWords) internal override {
    ...
    if (addressRecipients[requestId].length == 0) {
      ...
      IERC721(rewardAddress[requestId]).transferFrom(
        address(this),
        IERC721(nftRecipientAddress[requestId]).ownerOf(randomWords[0] % (endIndex + 1 - startIndex) + startIndex),
        rewardId[requestId]
      );
    ...
    // Otherwise we pick a random address from the list
      ...
      IERC721(rewardAddress[requestId]).transferFrom(
        address(this),
        recipients[randomWords[0] % recipients.length],
        rewardId[requestId]
      );
