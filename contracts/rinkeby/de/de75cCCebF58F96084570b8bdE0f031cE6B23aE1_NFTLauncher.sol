...
interface SeapadPayments {
     function insertNewProject(address owner, uint256 projectId, uint[] calldata paymentMethods_, uint basePayment,  uint slippage) external payable;
    function getProjectPaymentMethod(uint256 projectid, uint paymentMethod) external view returns (uint paymentMethodID, address paymentMethodAddress, uint feePercentage, uint256 balance, bool valid);
    function getProjectPaymentMethods(uint256 projectid) external view returns(uint[] memory _paymentMethods, uint basePayment);
    function getPaymentMethod(uint paymentMethodId) external view returns (bool valid, address paymentMethodAddress, uint defaultFeePercentage,bool keep,uint256 currentSiteBal,uint256 totalSiteEarned,uint decimals);
    function getEstimatedETHforToken(address token, uint256 amountOfTokensToRecieve) external view returns (uint256);
    function buyLinkToken(address tokenToConvertFrom, uint256 fullPaymentAmount, uint256 amountToTokenToBuy, address linkTokenAddress, uint slippage)  external  payable   returns(uint256);
}
...
contract NFTLauncher is VRFConsumerBase, ChainlinkClient, Ownable {
    ...
    address public packBordersAddress =0x028418706852fdcA5D194d443813c7F6abe19d04;
    address public redeemablesAddress;

    uint256 projectAmount = 1;
    uint paymentMethodAmount = 1;
    ...
    mapping(uint => PaymentMethods) private paymentMethods;
    ...
    function purchasePack(uint256 projectid, uint paymentMethod, uint slippage) public payable {
        ...
        PacksBought storage pb = project.packsBought[packsBoughtId];
        uint256 tmpProjectID = projectid;
        // uint256 ;
        // address tmpPackAddress = project.packAddress;
       
        // if (project.takeFeeFromPackPrice) {

        if(project.whitelisted) {
            Whitelist memory whitelist = project.whitelist[tx.origin];
            require(whitelist.whitelisted, "Not whitelisted");
        }
        
        // require(projectP2.paymentMethod[paymentMethod].valid, "Invalid payment method");
        // PaymentMethod

        (, address paymentMethodAddress, , , bool valid) = seapadPayments.getProjectPaymentMethod(projectid, paymentMethod);
        require(valid, "Invalid payment method");
        // (uint[] memory _paymentMethods, uint basePayment) = seapadPayments.getProjectPaymentMethods(projectid);
        // (,,,bool keep,,,) = seapadPayments.getPaymentMethod(paymentMethod);
    
  
        uint256 pricePerPackInBaseToken = project.pricePerPack; //100 DAI
        //TRANSFER THE TOKENS TO US.
        IERC20 token = IERC20(paymentMethodAddress); //token of the payment method
        
        // uint256 prevBal = IERC20(paymetMethodAddress).token.balanceOf(address(this));
        //Transfer the tokens from the user to us.
        uint256 prevPrevBal = token.balanceOf(address(this));
        require(token.allowance(msg.sender, address(this)) >= pricePerPackInBaseToken,"Please allow contract to spend your tokens!") ;
        token.transferFrom(msg.sender, address(this), pricePerPackInBaseToken);
        ...
        token.approve(paymentAddress, pricePerPackInBaseToken);
        uint slippage2 = slippage; 
        SeapadPayments(paymentAddress).buyLinkToken(paymentMethodAddress, pricePerPackInBaseToken, fee, address(LINK), slippage2);
        // prevPrevBal = token.balanceOf(address(this)) - prevPrevBal; //remaining balance.
        // uint256 linkBal = IERC20(address(LINK)).balanceOf(address(this)); //link remaining balance.
    }
    ...
    function mintOpenedPack(uint256 projectid, uint256 packSoldID_) public {
        ...
        Token minter_ = Token(project.contractAddress);
    ...
    function updateWhitelist(address[] calldata addresses, uint256 projectId,  bool whitelisted) public {
    ...
    function createToken(
        string calldata _uriforcollection,
        uint256 amountToPayOutPerPack,
    ...
    function getProjectsCreated(address creator) public view returns(uint256[] memory projectsCreated) {
    ...
    function createTokenPart2(
        ...
        uint[] calldata paymentMethods2 = paymentMethods_;
        uint bp = basePayment;
        ...
        payments.insertNewProject{value: msg.value}(msg.sender, pid, paymentMethods2, bp, slippage2);
        makeChainLinkRequest(project.amountToPayOutPerPack, projects2.name,  project.packsForSale, pid, bid, pbg);
        emit ProjectCreatedP2(pid, bid, pbg,  project.pricePerPack, projects2.launchTime, projects2.isJson);
    }

    function makeChainLinkRequest(uint256 amountToPayOutPerPack, string memory projectName, uint totalPacks, uint256 projectId, uint borderId, string memory packBackgroundImageUrl) public returns (bytes32 requestID) {
    ...
    function updateProject(
        uint256 projectid,
        string calldata newstorefronturl,
        uint256 pricePerPack,
        bool whitelisted
    ) public {
        ...
        Projects storage project = projects[projectid];
        // ProjectsP2 storage projects2 = projectsP2[projectid];
        project.storefront = newstorefronturl;
        project.pricePerPack = pricePerPack;
