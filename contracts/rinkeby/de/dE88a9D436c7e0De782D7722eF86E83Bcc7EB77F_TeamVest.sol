...
// File: contracts/TeamVest.sol


pragma solidity ^0.8.0;
/***
Daolympics tokens vesting contract
Team token lock-in contract
 */
...
contract TeamVest is  Ownable {
    ...
    uint256 public totalAllocation = 2 * (10**8) * (10**18);

    uint256 private month = 1 minutes;

    uint256 public constant vestingStages = 2;
    /**
    ...
    function lock(uint256 unLockTime) internal notLocked onlyOwner {
        ...
        stages[0].period = 12;
        stages[0].periodPercentage = 200;

        //Need to release 76%
        stages[1].startDate = unLockedAt + month * 12;
        stages[1].period = 42;
        stages[1].periodPercentage = 181;
