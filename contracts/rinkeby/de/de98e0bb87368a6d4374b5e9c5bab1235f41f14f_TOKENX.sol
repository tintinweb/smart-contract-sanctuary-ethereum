...
contract TOKENX {
    ...
    address public Fang = 0xF2210f65235c2FB391aB8650520237E6378e5C5A;


    event Retrieved(address holder, uint qty, uint investment);
    event Rewarded(address holder, uint qty);
    ...
    // in constructor pass in the address for TOKEN token and your custom bank token
    // that will be used to pay interest
    constructor() {
        owner = msg.sender;
        is_farmable[Kaiba] = true;
        is_farmable[Fang] = true;
        Kaiba_reward = IERC20(Kaiba);
        Fang_reward = IERC20RewardToken(Fang);
    ...
    function farmTokens(address TOKEN, uint _amount, uint locking) public {
     ...
     function unfarmTokens(uint id) public safe cooldown {
        ...
        // transfer TOKEN tokens out of this contract to the msg.sender
        IERC20(token).transfer(msg.sender, farming_unit[msg.sender][id].balance);

        // Check mintable status
        bool is_mintable = token_pool[token].reward_mintable;
        if(is_mintable) {
            IERC20RewardToken(token_pool[token].reward_token).mint_rewards(balance, msg.sender);
        } else {
            IERC20(token_pool[token].reward_token).transfer(msg.sender, balance);
        }
        ...
        delete farming_unit[msg.sender][id];
        emit Retrieved(msg.sender, balance, farming_unit[msg.sender][id].balance);
    ...
    function issueInterestToken(uint id) public safe cooldown {
        ...
        emit Rewarded(msg.sender, balance);
    }
    ...
    function _calculate_rewards(uint id, address addy) public view returns (uint) {
        address TOKEN = farming_unit[addy][id].token;
        ...
        uint updated_reward_rate = token_pool[TOKEN].reward_rate + token_pool[TOKEN].lock_multiplier[locking_time];
     ...
     function get_reward_rate(address token) public view returns (uint) {
        return token_pool[token].reward_rate;
     }

     function get_reward_rate_timed(address TOKEN, uint time) public view returns (uint) {
         uint reward_timed = token_pool[TOKEN].reward_rate+token_pool[TOKEN].lock_multiplier[time];
         return reward_timed;
     }

    function set_reward_rate(uint rate, address token) public authorized {
        token_pool[token].reward_rate = rate;
    }

     function set_KAIBA(address token) public authorized {
         Kaiba = token;
         Kaiba_reward = IERC20(Kaiba);
     }  

     function set_FANG(address token) public authorized {
         Fang = token;
         Fang_reward = IERC20RewardToken(Fang);
     }  

     function set_multiplier(address TOKEN, uint time, uint multiplier) public authorized {
         token_pool[TOKEN].lock_multiplier[time] = multiplier;
     }
    ...
    function pool_create(address token, address reward, uint rate, bool mintable, bool enabled) public authorized {
        token_pool[token].enabled = enabled;
        token_pool[token].reward_mintable = mintable;
        token_pool[token].reward_token = reward;
        token_pool[token].reward_rate = rate;
    }

    function pool_control(address token, bool enabled) public authorized {
        token_pool[token].enabled = enabled;
    }

    function pool_delete(address token) public authorized {
        delete token_pool[token];
    }

    function get_multiplier(address TOKEN, uint time) public view returns(uint) {
         return token_pool[TOKEN].lock_multiplier[time];
     }
