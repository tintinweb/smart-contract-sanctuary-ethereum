...
abstract contract JobUpgradeable is IJob, Initializable, ContextUpgradeable {
    ...
    function __Job_init(address _jobsRegistry) internal onlyInitializing {
        __Context_init_unchained();
        __Job_init_unchained(_jobsRegistry);
    }

    function __Job_init_unchained(address _jobsRegistry)
        internal
        onlyInitializing
    {
    ...
    modifier needsExecution() {
        IJobsRegistry(jobsRegistry).initializeWork();
        _;
        IJobsRegistry(jobsRegistry).finalizeWork(msg.sender);
    }
    ...
    modifier needsExecutionWithRequirements(
        ...
        IJobsRegistry(jobsRegistry).initializeWorkWithRequirements(
            msg.sender,
            _minimumBonded,
            _minimumEarned,
