...
contract EventUnlockSchedule is BaseGovernanceWithUserUpgradable {
    
    uint256 constant EXP = 1e18;
    int256[2][] keyPoints;//[[condition1, price1],[condition2, price2]]
    uint lockStartTime;
    address tknAddrBase;
    address tknAddrQuote;
    uint256[] amounts;
    uint256[] amountsUnlocked;
    ...
    function initialize(ILock lock, bytes calldata data) public initializer {
        (
            int256[2][] memory _keyPoints, 
            uint256[] memory _amounts, 
            address _tknAddrBase, 
            address _tknAddrQuote, 
        ...
        __EventUnlockSchedule_init(lock, _keyPoints, _amounts, _tknAddrBase, _tknAddrQuote, _feedRegistry, governanceAddress);
    }

    function __EventUnlockSchedule_init(ILock _lock, int256[2][] memory _keyPoints, uint256[] memory _amounts, address _tknAddrBase, address _tknAddrQuote, address _feedRegistry, address governanceAddress) internal onlyInitializing {
        __BaseGovernanceWithUser_init(governanceAddress);
        __EventUnlockSchedule_init_unchained(_lock, _keyPoints, _amounts, _tknAddrBase, _tknAddrQuote, _feedRegistry);
    }

    function __EventUnlockSchedule_init_unchained(ILock _lock, int256[2][] memory _keyPoints, uint256[] memory _amounts, address _tknAddrBase, address _tknAddrQuote, address _feedRegistry) internal onlyInitializing {
        ...
        tknAddrBase = _tknAddrBase;
        tknAddrQuote = _tknAddrQuote;
    ...
    function unlockedAmount(uint256 _initialAmount) external view returns (uint256) {
        if(amountsUnlocked.length < 1) {
            return 0;
        }

        uint256 unlockedNormalized;
        for(uint256 i = 0; i < amountsUnlocked.length; i++) {
            unlockedNormalized += amountsUnlocked[i];
        }

        return _initialAmount * unlockedNormalized / EXP;
    }
    
    function lockStart() public view returns(uint) {
    ...
    function lockEnd() external view returns(uint) {
        return lockStartTime + 9 * 365 days;
    }
    ...
    //     for(uint i; i < _keyPoints.length - 1; i++) {
    ...
    function checkUpKeep(bytes calldata /*checkData*/) external view returns (bool upkeepNeeded, bytes memory performData) {
        ...
        if(keyPoints.length > 0){
            int price = _getPrice(tknAddrBase, tknAddrQuote);
            for(uint256 i = 0; i < keyPoints.length; i++) {
                upkeepNeeded = false;
                if(keyPoints[i][0] == 0){
                    if(price == keyPoints[i][1]){
                        upkeepNeeded = true;
                    }
                }
                if(keyPoints[i][0] == 1){
                ...
                if(upkeepNeeded){
                    performData = abi.encode(i, amounts[i]);
                }
            }
        }
    }
    function performUpKeep(bytes calldata performData) external {
        ...
        _updateAmountsUnlocked(_amount);
        _removeEventSchedule(_idx);
    }
    function _updateAmountsUnlocked(uint256 amount) internal {
        amountsUnlocked.push(amount);
    }
    function _removeEventSchedule(uint idx) internal {
        delete keyPoints[idx];
        delete amounts[idx];
    }
     ...
     */
    function _getPrice(address base, address quote) internal view returns (int) {
        (
            uint80 roundID, 
            int price,
            uint startedAt,
...
interface IDepositManager {
     ...
     * @param totalAmount Total amount of tokens to be locked for additional beneficiaries
     */
    function addDeposits(bytes calldata data, uint256 totalAmount) external;
     ...
     * @param addresses List of addresses of beneficiaries
     */
    function split(uint originId, uint[] memory splitParts, address[] memory addresses) external returns(uint[] memory);
