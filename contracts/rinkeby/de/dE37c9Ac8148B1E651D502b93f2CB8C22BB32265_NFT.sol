 ...
 */
contract NFT is IERC2981, ERC165Storage, Ownable, IERC1155MetadataURI, ERC1155, MinterRole {
    ...
    /// @dev character type -> layerId -> url
    mapping(uint256 => mapping(uint256 => string[])) private AssetsMapping;

    /**
     @notice Constructor
     */
    constructor(string memory _name, string memory _symbol, string memory _baseUri, address _royalty) ERC1155(_baseUri) {
        name = _name;
        symbol = _symbol;
        baseUri = _baseUri;
        royalty = _royalty;
    ...
    /**
     ...
     @param _characterType character Type
     @param _layerId layer Id of character
     @param _assetId asset Id of layer
    ...
    function mint(address _beneficiary, uint256 _characterType, uint256 _layerId, uint256 _assetId, uint256 _supply) 
        ...
        require(_beneficiary != address(0), "beneficiary adress should be not zero address");
        require(_characterType != 0, "characterType should be positive");
        require(_layerId != 0, "layerId should be positive");
        require(_assetId != 0, "assetId should be positive");
        require(_supply != 0, "Supply should be positive");

        uint tokenId = ((_characterType - 1) * 32 + (_layerId - 1)) * 32 + _assetId;

        // // Mint token and set token URI
        _mint(_beneficiary, tokenId, _supply, "");
    ...
    /**
     ...
     @dev Only senders with either the admin or mintor role can invoke this method
     @param _characterType character Type
     @param _layerIds layer Ids array of character
     @param _assetIds asset Ids array for each layer
     @return uint256 The token ID of the token that was minted
     */
    function mintCharacter(address _beneficiary, uint256 _characterType, uint256[] memory _layerIds, uint256[] memory _assetIds) 
        ...
        require(_layerIds.length == _assetIds.length , "layerIds's length should be the same as assetIds");

        require(msg.value >= mintCost, "NFT.mint: mint cost is not enough");

        uint[] memory tokenIds = new uint[](_layerIds.length);
        uint[] memory supplies = new uint[](_layerIds.length);
        for(uint i = 0; i < _layerIds.length; i++) {
            tokenIds[i] = (_characterType * 32 + _layerIds[i]) * 32 + _assetIds[i];
            supplies[i] = 1;
        }

        // // Mint token and set token URI
        _mintBatch(_beneficiary, tokenIds, supplies, "");

        return tokenIds;
    }

    /**
     * @dev See {ERC1155-royaltyInfo}.
     */
    ...
    function getAssetsMapping(uint256 characterType, uint256 layerId) external view returns (string[] memory){
        return AssetsMapping[characterType][layerId];
    }

    function addAssetsMapping(uint256 characterType, uint256 layerId, string calldata cosmeticUrl) external{
        return AssetsMapping[characterType][layerId].push(cosmeticUrl);
    }
}
