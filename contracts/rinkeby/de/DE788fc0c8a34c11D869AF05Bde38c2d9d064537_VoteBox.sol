...
contract VoteBox is IVoteBox, Ownable, EIP712VoteAllowlist, ReentrancyGuard {
    ...
    // Get poll option name
    function getPollOptionNames(uint256 _pid) external view override returns (string[] memory) {
        if (!_existsPoll(_pid)) revert NonExistPoll();
        uint256 optionCount = polls[_pid].optionCount;
        string[] memory names;
        uint256 index = 1;
        do {
            // option index starts from 1 to n
            names[index-1] = polls[_pid].options[index].name;
            index ++;
        }
...
interface IVoteBox {
    ...
    // Get all poll option names
    function getPollOptionNames(uint256 _pid) external view returns (string[] memory);
    // Get poll total vote counts
