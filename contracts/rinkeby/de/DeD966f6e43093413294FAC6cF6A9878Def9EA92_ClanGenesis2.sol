...
contract ClanGenesis2 is ERC721A, Ownable, AccessControl, ReentrancyGuard {
  ...
  bool private pcStarted = false;
  bool private bgkStarted = false;
  bool private wlStarted = false;
  ...
  constructor(
    ...
    address _pc,
    address _bgk
  ) ERC721A("Bapes Clan Genesis 2", "BAPE2") {
    pc = IBapesTokens(_pc);
    bgk = IBapesTokens(_bgk);
  ...
  modifier mintCompliance(uint256 _mintAmount, uint256 _totalAmount) {
    ...
    require(totalSupply() + _mintAmount <= _totalAmount, "Max supply exceeded.");
    _;
  }
  ...
  function pcMint(uint256 _mintAmount) external payable mintCompliance(_mintAmount, maxSupplyPrivate) {
    ...
    for (uint256 i = 0; i < length; i = increment(i)) {
      if (!tokensUsed[address(pc)][ownerTokens[i]]) {
        arr[j] = ownerTokens[i];
        j++;
      }
    }

    require(j >= _mintAmount, "You have already used all PCs to mint.");

    for (uint256 i = 0; i < _mintAmount; i = increment(i)) {
      tokensUsed[address(pc)][arr[i]] = true;
    }

    _safeMint(_msgSender(), _mintAmount);
  }

  function bgkMint(uint256 _mintAmount) external payable mintCompliance(_mintAmount, maxSupplyPrivate) {
    ...
    for (uint256 i = 0; i < length; i = increment(i)) {
      if (!tokensUsed[address(bgk)][ownerTokens[i]]) {
        arr[j] = ownerTokens[i];
        j++;
      }
    }

    require(j >= _mintAmount, "You have already used all BGKs to mint.");

    for (uint256 i = 0; i < _mintAmount; i = increment(i)) {
      tokensUsed[address(bgk)][arr[i]] = true;
    }
  ...
  function updateMaxSupplyPrivate(uint256 _number) external onlyRole(DEFAULT_ADMIN_ROLE) {
