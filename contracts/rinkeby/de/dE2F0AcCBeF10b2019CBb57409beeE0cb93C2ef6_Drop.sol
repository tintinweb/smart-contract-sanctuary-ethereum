...
interface IMedia {
  struct MediaData {
    ...
    string tokenURI;
    // A valid MYSTERY URI of the metadata associated with this token
    string metadataURI;
    // A valid Reveal URI of the metadata associated with this token
    string revealURI;
...
contract Drop is ERC721 , IDrop {
    ...
    // mint status
    // bool public eggsReavealed = false;
    ...
    constructor() ERC721("Base Egg Drop", "BED"){
        title = "Base Egg Drop";
        owner = msg.sender;
    }
    ...
    function setEgg(string memory name, uint256 price, uint256 supply, string memory _tokenURI, string memory metadataURI, string memory revealURI) public onlyOwner returns (Egg memory) {
        Egg memory egg;
        egg.name = name;
        egg.data = getMediaData(_tokenURI, metadataURI, revealURI);
    ...
    function updateMetadataURI(string memory name, string memory newMetadataURI) public onlyOwner {
        eggs[name].data.metadataURI = newMetadataURI;
    }

    function updateRevealURI(string memory name, string memory newRevealURI) public onlyOwner {
        eggs[name].data.revealURI = newRevealURI;
    }

    function updateTokenURI(string memory name, string memory newTokenURI) public onlyOwner {
        eggs[name].data.tokenURI = newTokenURI;
    }

    function updateEggPrice(string memory name, uint newPrice) public onlyOwner {
        eggs[name].price = newPrice;
    }

    function updateEggSupply(string memory name, uint newSupply) public onlyOwner {
        eggs[name].supply = newSupply;
    }
    ...
    function eggsMinted() public view returns (uint256) {
        return getEgg(baseEgg).minted;
    }

    function eggMatadataURI() public view returns (string memory) {
        return getEgg(baseEgg).data.metadataURI;
    }

    function eggTokenURI() public view returns (string memory) {
        return getEgg(baseEgg).data.tokenURI;
    }
    ...
    function totalWhitelistedAddresses() public view returns (uint) {
        return whitelistedCount.current();
    }

    function getWhitelistedAddresses() public view returns (address[] memory) {
       address[] memory ret = new address[](whitelistedCount.current());
        for (uint i = 0; i < whitelistedCount.current(); i++) {
            ret[i] = whitelisted[i];
        }
        return ret;
    }
    ...
    function AirdropNFTS(address[] memory addresses, uint256[] memory numAllowedToMint) airdropModifier(addresses, numAllowedToMint) public onlyOwner {
        require(mintStatus, "NOT_ALLOWED_TO_MINT");
        ...
        for (uint256 i = 0; i < addresses.length; i++){
                require(_whitelistedAllowToMint[addresses[i]] != 0, "Can not mint 0 token");
                for (uint256 j = 0; j < _whitelistedAllowToMint[addresses[i]]; j++){
                    require(addresses[i] != address(0), "An address is equal to 0x0");
                    require(egg.minted <= egg.supply, "STOCK_EXCEEDED");
                    egg.minted++;
                    _safeMint(addresses[i], egg.minted);
                }
    ...
    function getMediaData(string memory _tokenURI, string memory metadataURI, string memory revealURI) private pure returns (IMedia.MediaData memory) {
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(getEgg(baseEgg).data.revealURI, Strings.toString(tokenId), ".json")
            );
    }
