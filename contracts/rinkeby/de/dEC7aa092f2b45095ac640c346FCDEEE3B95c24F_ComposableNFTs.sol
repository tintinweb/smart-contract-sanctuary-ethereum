...
contract ComposableNFTs is ERC721Enumerable, Ownable {
  ...
  /// @notice Composable NFT will include default layers and optional "accessory" layers
  uint256 public numDefaultLayers;

  /// @notice only whitelisted users are able to mint during the 1st wave. Anyone can mint during the 2nd wave
  uint256 public mintWave = 1;

  /// @notice When NFT minting begins (upon contract deployment)
  uint256 public mintStartDate;

  /// @notice records which wave the NFT was minted during
  mapping (uint256 => uint256) waveDuringMint;
  ...
  /// @notice total of 40 special 1 of 1 NFTs
  uint256 public numSpecialMint;

  /// @notice Unique 1 of 1 NFTs will have JSON metadata and image file pre-uploaded to IPFS
  string public specialMintIpfsUri;

  /// @notice Records a pointer between the NFT ID and the unique Mint ID (which ranges from 1-40)
  mapping (uint256 => uint256) specialMintId;
  ...
  constructor(
    ...
    uint256 _numDefaultLayers,
    string memory _specialMintIpfsUri,
    address _whitelist
  ) ERC721(_collectionName, _symbol) {
    description = _description;
    numDefaultLayers = _numDefaultLayers;
    specialMintIpfsUri = _specialMintIpfsUri;
    whitelist= IWhitelist(_whitelist);
    mintStartDate = block.timestamp;
  }

  /// @notice External function callde by a user to mint an NFT - 1 at a time
  function mint() public payable onlyWhenNotPaused{

    require(msg.value == mintPrice);

    if(mintWave == 1){
      require(whitelist.whitelistedAddresses(msg.sender) == true, "You are not whitelisted");
      require(block.timestamp < mintStartDate + 1 weeks, "1st minting wave has ended");
    }
    require(tokenIds < maxTokenIds, "Exceeded maximum supply");
    
    if(block.timestamp < mintStartDate + 24 hours){
      require(mintsPerAddress[msg.sender] < 5, "Only 5 mints during first 24 hours");
    }
    mintsPerAddress[msg.sender] = mintsPerAddress[msg.sender] + 1;
    tokenIds += 1;
  ...
  /// @notice Determines whether an NFT should be minted as an unique 1 of 1 NFT; 40 of the mints will be these special 1 of 1 NFTs
  function checkIfSpecialMint() private view returns(bool){
    if(numSpecialMint == 40){
      return false;
    }

    // this guarantees that 40 unique 1 of 1 NFTs will always be minted when the number of mints reaches 10000
    if(tokenIds > 9960){
      return true;
    }
    ...
    pseudoRandomness = pseudoRandomness % maxTokenIds;

    // double the chance during the 1st half of the mints to generate a unique 1 of 1 NFT
    if(tokenIds <= (maxTokenIds / 2)){
      if(pseudoRandomness < 80){
        return true;
      }
      return false;
    }
    else{
      if(pseudoRandomness < 40){
        return true;
      }
      return false;
    }
  ...
  /// @notice Checks whether an alreadt minted NFT is an unique 1 of 1 NFT
  function isSpecialNft(uint256 tokenId) public view returns (bool){
    if(specialMintId[tokenId] != 0){
      return true;
    }
    else{
      return false;
  ...
  /// @notice Randomly generates each separate layer of the NFT's SVG upon mint
  /// @param tokenId ID of the NFT
  function mintItem(uint256 tokenId) private {

    bool shouldMintSpecialNft = checkIfSpecialMint();
    if(shouldMintSpecialNft == true){
      numSpecialMint = numSpecialMint + 1;
      specialMintId[tokenIds] = numSpecialMint;
    } else {
      uint256[] memory randomNumbers = generateRandomNumbers();

      uint256 hatArrayIndex = 5;
      uint256 helmetArrayIndex = 6;
      // selectOption logic must be semi-hardcoded due to customized if-then logic
      for(uint i=0; i < numDefaultLayers; i++){
        if(i != hatArrayIndex){
          if(i != helmetArrayIndex){
            layerContracts[i].selectOption(tokenId, randomNumbers[i], false);
          }
          // if helmet do nothing b/c helmet selection logic is coupled with hat selection logic
        }
        else{
          bool hatInsteadOfHelment = hatOrHelment(randomNumbers[hatArrayIndex]);
          if(hatInsteadOfHelment == true){
            layerContracts[hatArrayIndex].selectOption(tokenId, randomNumbers[hatArrayIndex], false);
            layerContracts[helmetArrayIndex].selectOption(tokenId, 0 , true);
          }
          else{
            layerContracts[hatArrayIndex].selectOption(tokenId, 0, true);
            layerContracts[helmetArrayIndex].selectOption(tokenId, randomNumbers[helmetArrayIndex], false);
          }
        }
      }
      
    }
    waveDuringMint[tokenIds]=mintWave;
  }
    ...
    returns (string memory)
  {
    ...
    if(!isRevealed){
      return string(abi.encodePacked(specialMintIpfsUri, Strings.toString(0), ".json"));
    }

    if(isSpecialNft(tokenId) == true){
      return string(abi.encodePacked(specialMintIpfsUri, Strings.toString(tokenId), ".json"));
    }

    string memory tokenName_ = string.concat("NFT Collection name #", Strings.toString(tokenId));
    ...
    for (uint i = 0; i < layerContracts.length; i++) {
      if(i == 0) {
        attributes = string.concat(
          layerContracts[i].getOptionMetadataByTokenId(tokenId)
        );
      } else {
        attributes = string.concat(
          attributes,
    ...
    returns (string memory)
  {
    ...
    for (uint i=0; i<layerContracts.length; i++) {
      render = string(abi.encodePacked(
        render,
        layerContracts[i].renderOptionByTokenId(tokenId)
      ));
    }
  ...
  /// @notice Custom logic when minting: can only get either a hat or a helment 
  function hatOrHelment(uint256 randomNum) internal pure returns(bool){
    if(randomNum % 2 == 0){
      return true;
    }
    return false;
  ...
  function addLayers (Layer[] memory layerAddresses) public onlyOwner {
    for(uint i=0; i<layerAddresses.length; i++){
      layerContracts.push(Layer(layerAddresses[i]));
    }
  }

  /// @notice Change from Wave 1 minting to Wave 2 minting. Can only be called once
  function incrementMintWave () public onlyOwner {
    require(mintWave == 1);
    require(block.timestamp > mintStartDate + 1 weeks);
    mintWave = mintWave + 1;
  }

  /// @notice Transfers ETH collected from minting to owner
  function collectMintFee() public onlyOwner{
    payable(msg.sender).transfer(address(this).balance);
  }

  /// @notice Fetches all of the individual Layer contracts' addresses
  function getAllLayerContracts() public view returns (Layer[] memory) {
