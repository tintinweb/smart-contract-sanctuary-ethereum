...
contract CKD02 is Auth {
    ...
    struct DaoSettings {
        ...
        uint32 minVotingDuration; //how many days will a proposal be open for vote submissions
        uint32 minTimeToProcess; //how many hours after vote is passed till proposal can be actioned
    }
    ...
    mapping(uint256 => string) public proposalFinalNote;

    event SettingsUnlockTimerStarted(uint256 requestedOn, uint256 unlocksOn);
    event SettingsLocked(uint256 lockedOn);
    event SettingsUpdated(address caToken, address caStaking, uint256 minStakingTimeToVote, uint256 minStakedTokensVote, uint32 minVotingDuration, uint32 minTimeToProcess);
    ...
    event ProposalFinalNote(uint256 proposalId, string note);
    ...
    function settingsChange(address caToken, address caStaking, uint256 minStakedTimeToVote, uint256 minStakedTokensVote, uint32 minVotingDuration, uint32 minTimeToProcess) external onlyOwner {
    ...
    function proposalSetFinalNote(uint256 proposalId, string memory note) external onlyAuthorizedProposer {
        require(uint8(proposal[proposalId].state) > 2, "Cannot set note until proposal is finalized" );
        proposalFinalNote[proposalId] = note;
        emit ProposalFinalNote(proposalId, note);
    }
