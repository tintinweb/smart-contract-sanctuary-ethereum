...
contract MarketplaceV3 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint16 private constant FEES_MULTIPLIER = 10000;
    ...
    uint16 public maxRoyaltyPercent;
    ...
    event ServiceFeeChanged(uint16 newFee);

    /// @notice When maximal royalty percent changed
    event MaxRoyaltyChanged(uint16 oldMaxRoyaltyPercent, uint16 newMaxRoyaltyPercent);
     ...
     */
    function setServiceFee(uint16 newServiceFee) external onlyOwner {
     ...
     */
    function setMaxRoyalty(uint16 _newMaxRoyaltyPercent) external onlyOwner {
        require(maxRoyaltyPercent != _newMaxRoyaltyPercent, "same amount");
        uint16 _oldMaxRoyaltyPercent = maxRoyaltyPercent;
    ...
    function setRoyalty(
        ...
        require(royaltyPercent <= uint256(maxRoyaltyPercent), "% is bigger than maxRoyaltyPercent");
    ...
    function _buy(
        ...
        uint256 _fee = (price * uint256(serviceFee)) / uint256(FEES_MULTIPLIER);
        ...
        if (_payRoyalty) {
            ...
            if (_royalty.percent > 0) {
                _royaltyPercent = (price * _royalty.percent) / uint256(FEES_MULTIPLIER);
