...
struct Upgrade{
    uint256 upgradeStartTime;   // Date when upgarde start
...
contract Ocean is Ownable, ReentrancyGuard, Pausable {
    ...
    /* PARAMETERS */
    // TODO: implement setter for parameters
    // Minimum diving time in hour
    uint256 public DIVING_TIME = 1;
    // Minimum upgrade time in hour
    ...
    */
    function upgrade(uint8[] calldata tokensId) public whenNotPaused nonReentrant{
        ...
        uint256 amountpearlNeeded = getTotalPearlNeededForUpgarde(tokensId);
    ...
    function getRewardBySubmarineLevel(uint256 level) public view returns(uint256){
        ...
        if(level != 1){
            for(uint i = 1; i < level; i++){
                reward = reward + (reward*SUBMARINE_REWARD_COEF)/100; 
            }
        }
        return reward/100;
    }
    ...
    */
    function getFuelByLevel(uint256 level) public view returns(uint256){
        ...
        uint256 fuel = FUEL_START * 100;
        if(level != 1){
            for(uint i = 1; i < level; i++){
                fuel = fuel * (100+FUEL_COEF)/100; 
            }
        }
        return fuel/100;
    }
    ...
    */
    ...
    /**
    * Return the $FUEL amount needed to upgarde all tokens of `tokensId` to next level
    * @param tokensId Array of token's id
    */
    function getTotalPearlNeededForUpgarde(uint8[] calldata tokensId) public view returns(uint256){
...
contract Pearl is ERC20, Ownable{
    ...
    // TODO: add MAX_SUPPLY value
    uint256 public constant SUPPLY_AMOUNT = 0;
...
contract NautilusNFT is ERC721Enumerable, Ownable, ReentrancyGuard, Pausable{
    ...
    // Free mint address
    mapping(address => uint256) public isFreeMint;
    ...
    function freeMint() public{
        ...
        require(tx.origin == msg.sender, "Contracts not allowed");
        require(presaleStart || saleStart,"Sale is not open");
        require(isFreeMint[msg.sender] > 0, "You are not freeminted");
        // TODO: modifier comptage des freemint
        require(transactionsForAddress[msg.sender].length <= isFreeMint[msg.sender],"Cannot buy more than free mint limit");
        require(totalSupply() + 1 <= MAX_SUPPLY,"All has been purchased");
    ...
    function addAllwhitelist(address[] memory _toWhitelist) public onlyOwner{
    ...
    function addFreeMint(address _toFreeMint, uint256 amount) public onlyOwner{
        isFreeMint[_toFreeMint] = amount;

        emit AddressFreeMint(_toFreeMint);
    }

    // TODO: écrire
    // function addAllFreeMint(address[] memory _toFreeMint) public onlyOwner{
    //     for(uint256 i = 0; i < _toFreeMint.length; i++){
    //         addFreeMint(_toFreeMint[i]);
    //     }
    // }
    ...
    // TODO: remove freemint and whitelist
