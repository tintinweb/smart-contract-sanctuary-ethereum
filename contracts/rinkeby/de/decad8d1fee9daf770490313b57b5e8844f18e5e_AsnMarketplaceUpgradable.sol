    ...
    EIP712Upgradeable
{
    ...
    function cancelAuction(uint256 collectableId) public {
        require(
    ...
    function _setBidder(
        ...
        auction[collectableId].deposit = deposit;
    }
    ...
    function startAuction(
        ...
        _setSalePrice(collectableId, salePrice);
        tokenSaleData[collectableId].collectableOwner = tokenOwner;
        tokenSaleData[collectableId].nTokens = ntokens;

        emit StartAuction(tokenOwner, collectableId, ntokens, startingPrice);
    }
    ...
    function placeBid(
        ...
        require(
            verifySignature(signature) == msg.sender,
        ...
        _setBidder(collectableId, payable(msg.sender), amount, msg.value);
    ...
    function endAuction(uint256 collectableId) public payable nonReentrant {
        ...
        tokenSaleData[collectableId].isComplete = true;
    ...
    function claimNft(uint256 collectableId) public payable nonReentrant {
        require(
            msg.sender == auction[collectableId].account,
            "Wrong bidder address"
        );

        require(
            tokenSaleData[collectableId].isComplete != true,
            "The Auction is not completed yet"
        );

        require(
            msg.value >=
                auction[collectableId].value - auction[collectableId].deposit,
            "The entered balance amount is wrong"
        );
    ...
    /* Signature Verification Method Starts - Method follows EIP-712 standard*/

    bytes32 private SIGNATURE_SALT;

    function setSignatureSalt(bytes32 salt) public virtual onlyAdmin {
        SIGNATURE_SALT = salt;
    }

    function getSignatureSalt() public view onlyAdmin returns (bytes32) {
    ...
    function _verify(bytes memory signature) internal view returns (address) {
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "signatureStruct(string name,string version,address contractAddress,bytes32 salt)"
                        ),
                        keccak256(bytes("AsnMarketplaceUpgradable")),
                        keccak256(bytes("1")),
                        address(this),
                        SIGNATURE_SALT
                    )
