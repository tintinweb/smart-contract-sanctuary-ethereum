...
contract pay_test is ERC1155Supply, Ownable  {
    ...
    uint public constant MAX_PAY = 10000;
    uint private PriceWL = 0.001 ether; //BNB
    uint private Price = 0.002 ether;   //BNB
    uint[] private reward = new uint[](3);
    ...
    mapping(address => address) private _layer; //L
    mapping(uint256 => address) private _listAll; //Mc
   // mapping(address => uint256) private _balance; 
    mapping(address => Users) private _users;
    

// Withdraw addresses
    address w1 = 0xd811459dad4e973dD905857CCFa57Ec3A71c5179;
    address w2 = 0xE098Ad7d2E174Cb343Eb2E8284ACB1eaD076D8C0;
    address wDev;
    
constructor(string memory uri, string memory _name, string memory _symbol, uint256 _StartSale, address _wDev) ERC1155(uri) {
    name = _name;
    symbol = _symbol;
...
//Mint
    ...
    bool ref = false;
    bool newal = false;
    address[] memory W = new address[](3); //!test
    Users storage user;
    if (balanceOf(refadd, TOKEN_ID) > 0) {ref = true;}  //1155
    if (balanceOf(msg.sender, TOKEN_ID) > 0) {newal = true;} //1155
    _mint(msg.sender, TOKEN_ID, numberOfTokens, "");
    emit Minted(totalSupply(TOKEN_ID));
    ...
    _listAll[count] = w1;
    count +=1;
    _layer[msg.sender] = w1;
    user = _users[w1];
    //uint256 uid = user.myref.length;
    //user.myref[user.numref] = msg.sender;
    user.indexref.push(count);
    user.numref += 1;
	...
	W[2] = _layer[W[1]];
	if (W[0] != address(0)) {
	//rewardStart(W[0], reward[0]);
        if (balanceOf(W[0], TOKEN_ID) > 0) {
        user = _users[W[0]];
        user.balance += (((Price * numberOfTokens) / 100) * reward[0]);
        reserve += (((Price * numberOfTokens) / 100) * reward[0]);
        }
    }
	if (W[1] != address(0)) {
	//rewardStart(W[1], reward[1]);
        if (balanceOf(W[1], TOKEN_ID) > 0) {
        user = _users[W[1]];
        user.balance += (((Price * numberOfTokens) / 100) * reward[1]);
        reserve += (((Price * numberOfTokens) / 100) * reward[1]);
        }
    }
	if (W[2] != address(0)) {
	//rewardStart(W[2], reward[2]);}
        if (balanceOf(W[2], TOKEN_ID) > 0) {
        user = _users[W[2]];
        user.balance += (((Price * numberOfTokens) / 100) * reward[2]);
        reserve += (((Price * numberOfTokens) / 100) * reward[2]);
        }
	}
    }
}
...
//View tree
function getTree(address myadd) public view returns (uint256[] memory) {
//  for (uint256 i = 0; i < count; i++) {
//            if (_layer[_listAll[i]] == myadd) {
//                 forkList[forkList.length] = _listAll[i];
//               }         
//       }  
                    
    return _users[myadd].indexref; 
 }

//withdraw BNB for owner
   function withdrawAll() public payable onlyOwner {
    uint256 _per = address(this).balance / 100;
    uint256 _part1 = _per * 90;
    uint256 _part2 = _per * 10;
