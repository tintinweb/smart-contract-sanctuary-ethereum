   ...
   // @dev Accumulated tokens per share
    ...
    function calculatePendingRewards(address user) public view returns (uint256) {
        if ((block.number > lastRewardBlock) && (totalAmountStaked != 0)) {
            ...
            uint256 tokenPerShare = ((stakers[user].amount * PRECISION_FACTOR )/ totalAmountStaked) *rewardsPerSecForStaking;

            return  (tokenPerShare + tokenRewardForStaking)/ PRECISION_FACTOR;
        } else {
            uint256 tokenPerShare = ((stakers[user].amount * PRECISION_FACTOR )/ stakers[user].amount) *rewardsPerSecForStaking;
