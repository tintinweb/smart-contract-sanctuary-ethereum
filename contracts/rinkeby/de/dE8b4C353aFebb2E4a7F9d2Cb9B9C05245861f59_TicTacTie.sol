...
contract TicTacTie is ERC721, IERC1155Receiver {
    ...
    uint256 immutable _turnBlockDuration; // = 6646;
    uint256 immutable _minTurnBlockDuration; // = 138;

    uint256 constant stateSize = 9;
    uint256 constant durationSize = 13;
    ...
    uint256 constant stateMask = 1152921504606846464;
    uint256 constant durationMask = 1152921504602653183;
    uint256 constant lastActionBlockMask = 1152780767122685951;
    uint256 constant victoriesMask = 1144055042840461311;
    uint256 constant tiesMask = 585467951558164479;
    uint256 constant statusMask = 576460752303423487;
    ...
    //mapping(address => uint256) public board;
    mapping(uint256 => uint256) opponent;

    // User mints a board
    // User selects an opponent board
    // Players can play
    // When player makes last winning move, game is over
    // When other player doesn't play, game is over
    // Winner can select another opponent
    // Players can play
    modifier onlyArtist() {
        require(_artist == _msgSender(), "E1");
        _;
    }
    ...
    constructor(
        uint256 turnBlockDuration,
        uint256 minTurnBlockDuration,
        string memory name,
        string memory symbol,
        address boardBuilder,
        uint256 maxBoardsSupply,
        uint256 maxTieSupply,
        uint256 price
    ) ERC721(name, symbol) {
        _turnBlockDuration = turnBlockDuration;
        _minTurnBlockDuration = minTurnBlockDuration;
        _artist = payable(msg.sender);
        _price = price;
        setSupply(maxBoardsSupply, maxTieSupply, boardBuilder);
    }

    function withdraw() external {
        uint256 balance = address(this).balance - _donations;
        _artist.transfer(balance);

        _charity.transfer(_donations);
        _donations = 0;
    }

    function setCharity(address payable charity) external onlyArtist {
        _charity = charity;
    }

    function setArtist(address payable artist) external onlyArtist {
        _artist = artist;
    }
    ...
    function setSupply(
        uint256 additionalBoardSupply,
        uint256 additionalTieSupply,
        address newBoardBuilder
    ) public onlyArtist {
        _maxBoardSupply += additionalBoardSupply;
        _boardBuilder = IBoardBuilder(newBoardBuilder);

        if (additionalTieSupply > 0) {
            _startingTieId += _tieSupply;
            _leftToMint += additionalTieSupply;
            _tieSupply += additionalTieSupply;
        }
    }
    ...
    function boardState(uint256 boardIndex) public view returns (uint256) {
        return _boardState(boards[boardIndex]);
    }

    function expiryBlock(uint256 boardIndex) public view returns (uint256) {
    ...
    function isBoardTurn(uint256 boardIndex) public view returns (bool) {
    ...
    function mintableTies(uint256 boardIndex) public view returns (uint256) {
        ...
        onlyBoardOwner(board1Index)
    {
        ...
        uint256 board1LastBlock;
        uint256 board2LastBlock;
        if (victories(board1Index) >= victories(board2Index)) {
            board1LastBlock = block.number - 1;
            board2LastBlock = block.number;
        } else {
            board1LastBlock = block.number;
            board2LastBlock = block.number - 1;
        }
        ...
        returns (bool)
    {
        ...
        return lastActionBlock1 < lastActionBlock2;
    }
