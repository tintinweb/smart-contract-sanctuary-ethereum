...
/// @dev Jacobo Lansac Nieto & Jorge Winter Pais de Brito
contract VinciStakingV1 is Ownable, Pausable {
    ...
    /// user tokens that are staked (earning APR), but are shceduled to be unstaked in next checkpoint
    ...
    /// checkpoints are postponed in multiples of 30 days. The checkpointMultiplier defines how many blocks of 30 days the current checkpoint has been postponed.
    ...
    /// user tier which is granted according to the tier thresholds in vinci. Tiers are re-evalyated in certain ocasions (unstake, relock, crossing a checkpoint)
    ...
    /// balance of vinci tokens collected via the penalizations happening when executing unstake() function
    ...
    event SuperstakerGranted(address user);
    event SuperstakerRemoved(address user);
    ...
    */
    ...
    @dev    Emits a {Staked} event.
            ...
            staking rewars, unstaking tokens and checkpoints need to be updated, so the corresponding modifers are used
    ...
    */
    ...
    @param _amount      amount of vinci tokens
    @param _poolId      unique id of the pool to stake to.
    @param _to          user recieving the stake
    */
    function stakeTo(
        uint _amount,
        uint _poolId,
    ...
    */
    ...
    @dev    the scheduled tickets to unstake cannot be directly unstake using unstake(). First they need to be cancelled with this function
            ...
            staking rewars, unstaking tokens and checkpoints need to be updated, so the corresponding modifers are used
    @notice allows a user to cancel the scheduled tickets to unstake. When doing this, there is no posibility of canceling a specific amount, but all scheduled amount is cancelled
    */
    ...
    /**
    @dev    lock is used due to interaction with extrnal contracts
            emits {Claimed}
            staking rewars, unstaking tokens and checkpoints need to be updated, so the corresponding modifers are used
    @notice user can claim tokens to be recieved in the wallet. Only the tokens that are under claimableBalance() are subject to be claimed
    */
    ...
    /**
    @dev    emits {Claimed} and {Staked} events
    staking rewars, unstaking tokens and checkpoints need to be updated, so the corresponding modifers are used
    @notice allows the user to claim the claimable tokens and stake them directly in the same transaction
    @param  _poolId unique id of the pool to stake
    */
    ...
    @notice user tier is reevaluated, allowing user to upgrade (or downgrade) at the cost of postponing the checkpoint the same number of monts as the current multiplier
    */
    function relock() external updateBalances(msg.sender) checkpointUpdated(msg.sender) {
        address sender = _msgSender();
        require(_existingUser(sender), 'Not authorized to run this function');
        uint userStaking = stakingBalance(sender);
        if (userStaking == 0) {
            _finishStakeholder(sender);
        } else {
            _setTier(sender, calculateTier(userStaking));
    ...
    */
    ...
    fund, and also updates if tokens that are currently unstaking have passed the unloc time, then these become claimable
    */
    function updateUserBalances(address _user) public returns (bool){
    ...
    @notice Inlcudes both the active staking and the scheduled tokens to be unstaked in next checkpoint
    ...
    */
    ...
    @notice includes multiple balances that were converted to claimable after crossing a checkpoint
            shceduled tokens after crossing a checkpoint and the rewards that became claimable
    @param  _user user to read balances from
    ...
    */
    ...
    function unstakingUnlockTime(address _user) public view returns (uint) {
    ...
    function readLastStakingRewardsUpdate(address _user) public view returns (uint) {
        return lastStakingRewardsUpdate[_user];
    }
    ...
    */
    function readTotalStaked() external view returns (uint){
        uint totalStaked = 0;
    ...
    */
    ...
    @param  _amount     amount of vinci tokens to be transfered to the contract
    */
    ...
    */
    ...
    /**
    @dev    The contract owner will execute this preiodically to follow the vinci price in usd
    ...
    */
    ...
    @param  _to     wallet to recieve the withdrawal
    */
    ...
    */
    ...
    @param   _to    wallet recieving the tokens
    */
