    ...
    ONFT721
{
    ...
    {
        uint16 mintedBefore = minted;
        uint availableMints = MAXMINT - mintedBefore;
        possibleToMint = uint8(availableMints >= _quantity ? _quantity : availableMints);

        assert(mintedBefore + possibleToMint <= MAXMINT);

        minted += possibleToMint;

        for (uint i = 0; i < possibleToMint;) {
            _mint(_to, (mintedBefore + 1) + i);
