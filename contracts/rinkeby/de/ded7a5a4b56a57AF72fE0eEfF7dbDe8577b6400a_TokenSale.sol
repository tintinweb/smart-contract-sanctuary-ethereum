...
contract TokenSale is ERC721 {
    ...
    // The service fee is the percentage of the sale price that is charged to the seller
    ...
    constructor(
        ...
        setServiceFeePercentage(_serviceFeePercentage);
        setPayoutAddress(payable(_msgSender()));
    }
    ...
    function setServiceFeePercentage(uint256 percentage) public onlyOwner {
        serviceFeePercentage = percentage;
    }

    function setPayoutAddress(address payable _address) public onlyOwner {
    ...
    function buyToken(uint256 tokenId, uint256 saleId) public {
        ...
        uint256 feeDeduction = (salePrice * serviceFeePercentage) / 10000;

        ft.transferFrom(
            _msgSender(),
            saleData.seller,
            salePrice - feeDeduction
        );
        ft.transferFrom(_msgSender(), payoutAddress, feeDeduction);
    ...
    function makeOffer(
        ...
        require(
            amount != offer.price,
            "Token sale: Same offer price as before"
        );
        offer.price = amount;
    ...
    function acceptOffer(uint256 tokenId, address offerFrom) public {
        ...
        uint256 feeDeduction = (offer.price * serviceFeePercentage) / 10000;

        ft.transferFrom(offerFrom, _msgSender(), offer.price - feeDeduction);
        // Service Fee deduction
        ft.transferFrom(offerFrom, payoutAddress, feeDeduction);
        transferFrom(_msgSender(), offerFrom, tokenId);
        offer.isAvailable = false;
        emit OfferAccepted(offerFrom, _msgSender(), tokenId, offer.price);
    }
