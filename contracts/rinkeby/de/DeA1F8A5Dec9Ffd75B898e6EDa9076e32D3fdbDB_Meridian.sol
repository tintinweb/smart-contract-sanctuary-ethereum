    ...
    ,INonDilutive
{
    ...
    function whitelistMint(uint256 quantity) public virtual payable {
        ...
        if(totalSupply() + quantity > 9999) revert MintExceedsMaxSupply();
        require(_numberMinted(msg.sender) + quantity < 7, "Max mint 3 per wallet"); //use 4
        _mint(msg.sender, quantity, '', true);
        //_minter(quantity, factionId);
    }
    ...
    function _minter(uint256 quantity, uint256 factionId) 
        internal 
    {
        ...
        if(factionId == 1){
            ...
            for(uint256 i; i < quantity; i++) {
                unrevealedURIMapping[totalSupply + i] = factionId;
            }
            ...
            revert("Invalid Faction ID");
        }
    }

    // Team mint
    function batchReserve(uint256[] memory quantities, uint256[] memory factionIds) 
        external 
        onlyOwner 
    {
        for (uint256 i = 0; i < quantities.length; i++) {
            _minter(quantities[i], factionIds[i]);
        }
    }
