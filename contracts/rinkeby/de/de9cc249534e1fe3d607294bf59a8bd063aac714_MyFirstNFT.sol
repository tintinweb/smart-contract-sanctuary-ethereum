...
contract MyFirstNFT is ERC721Enumerable, Ownable {
    ...
    string public baseURI = "https://gateway.pinata.cloud/ipfs/QmaKXQLmhmXHniRNXZeTzHUFZgSyRdRDijyLbLPjEUnw6K/"; // 這一行是 NFT 該去哪裡找你的 MetaData
    bool public paused = false; // 可以拿來暫停或者開啟 Mint
    uint256 public cost = 0.0001 ether; // Mint 價格
    uint256 public maxSupply = 5; // 只有五個 NFT
    uint256 public maxBalance = 1; // 每個用戶只有一個
    uint256 public maxMintAmount = 1; // 一次最多只能 Mint 一個

    constructor() ERC721("zoon_0841", "Test2") payable {
        // 剛開始我們先自己 Mint 一個
        safeMint(msg.sender, 8);
    }
