...
contract OUTSOURCE is ERC721AQueryable, Ownable, ReentrancyGuard, RoyaltiesV2Impl {
    using Strings for uint256;

    string public baseURI = "ipfs://QmT4roxEKzaGH7S1JFUqrZ2Mzw9qqtmvrQD6mqw1keGQme/";
    string public baseExtension = ".json";
    string public notRevealedUri = "ipfs://notRevealed/";
    uint256 public costPreSale = 0.01 ether;
    ...
    uint public saleStartTime = 1664102262;
    ...
    constructor() ERC721A("Outsource 2014", "OTS") {}
        ...
        payable
    {
        ...
        if (msg.sender != owner()) {
            ...
            if (currentState == 1) {
                ...
                uint256 ownerMintedCount = addressMintedBalancePreSale[msg.sender];
                require(
                    isPreSale(msg.sender, _merkleProof),
                    "user is not whitelisted"
                );
                require(
                ...
                require(
                    ownerMintedCount + _mintAmount <= maxMintAmountPreSale,
                    "max NFT per address exceeded"
                );
                require(
				...
				require(
				    currentTime() >= saleStartTime,
					"PreSale Sale has not started yet"
				);
    ...
    function isPreSale(address _user, bytes32[] calldata _merkleProof)
        public
        view
        returns (bool)
    {
        bytes32 leaf = keccak256(abi.encodePacked(_user));
        return MerkleProof.verify(_merkleProof, merkleRootPreSale, leaf);
    }
    ...
    function mintableAmountForUser(address _user) public view returns (uint256) {
        if (currentState == 1) {
            ...
            return maxMintAmountPreSale - addressMintedBalancePreSale[_user];
        } else if (currentState == 3) {
    ...
    function setPreSaleCost(uint256 _price) public onlyOwner {
        costPreSale = _price;
    }
    ...
    function setOnlyPreSale() public onlyOwner {
        currentState = 2;
    }
