...
contract Whirly is ERC721A, Ownable, ReentrancyGuard {
    ...
    string private unrevealedURI = "ipfs://QmWT2vYiutQZMQuqofUCwvaq1CBAGuEcHZKNhPfp9ruCEE";
    ...
    uint256 public total_reserved = 500;

    using Strings for uint256;

    constructor (bytes32 _root) ERC721A("Whirly NFT", "Whirly") {
        root = _root;

        // don't forget to update total_reserved
        reserved_mints[0x10b5B489E9b4d220Ab6e4a0E7276c54D5bf837cD] = 500;  // Whirly
    }
    ...
    function withdrawEth() public onlyOwner nonReentrant {
        ...
        require(payable(0x452A89F1316798fDdC9D03f9af38b0586F8142e5).send((total * 5) / 100)); // dev
        require(payable(0x10b5B489E9b4d220Ab6e4a0E7276c54D5bf837cD).send((total * 15) / 100)); // Whirly wallet
        require(payable(0x41e1c9116667Fcc9dd640287796fB5eBDB1DB70E).send((total * 20) / 100)); // kristian
        require(payable(0x5C2ce2d9eFAA4361aB129f77Bdad019A9a1b1cbe).send((total * 20) / 100)); // mike
        require(payable(0x6D9d741BC5Bca227070C43a23977E2FDE6B971e9).send((total * 20) / 100)); // logan
        require(payable(0x94Eb23cC87c4826DF76158151e0C3e94c18f02bB).send((total * 20) / 100)); // demi
    }
