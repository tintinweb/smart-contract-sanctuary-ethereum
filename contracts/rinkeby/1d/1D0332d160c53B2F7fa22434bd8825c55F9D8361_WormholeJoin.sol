...
contract WormholeJoin {
  ...
  function _mint(
    ...
    uint256 amtToTake = _min(pending, uint256(int256(line_) - debt_));

    uint256 fee = vatLive
      ? FeesLike(fees[wormholeGUID.sourceDomain]).getFee(wormholeGUID, line_, debt_, pending, amtToTake)
      : 0;
   ...
   * @return postFeeAmount The amount of DAI sent to the receiver after taking out fees
   **/
  function requestMint(WormholeGUID calldata wormholeGUID, uint256 maxFeePercentage)
    external
    auth
    returns (uint256 postFeeAmount)
  {
    ...
    return _mint(wormholeGUID, hashGUID, maxFeePercentage);
  }
   ...
   * @return postFeeAmount The amount of DAI sent to the receiver after taking out fees
   **/
  function mintPending(WormholeGUID calldata wormholeGUID, uint256 maxFeePercentage)
    external
    returns (uint256 postFeeAmount)
  {
    require(
      bytes32ToAddress(wormholeGUID.receiver) == msg.sender || bytes32ToAddress(wormholeGUID.operator) == msg.sender,
      'WormholeJoin/not-receiver-nor-operator'
    );
