    ...
    Pausable
{
    ...
    mapping (uint => string) public graffiti;
    ...
    function getSvg(uint tokenId) private view returns (string memory) {
        ...
        parts[0] = "<svg width='350px' height='350px' viewBox='0 0 350 350' role='img' xmlns='http://www.w3.org/2000/svg'><style>.a { fill: #0000; font-size: 18px; }</style><text x='10' y='10' class='a'>";
        parts[1] = graffiti[tokenId];
        parts[2] = "</text></svg>";
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "MY GRAFFITI", "description": "", "image_data": "data:image/svg+xml;base64,', Base64.encode(bytes(svgData)), '"}'))));
        return string(abi.encodePacked('data:application/json;base64,', json));
    }


    function mintGraffiti(string memory message) public whenNotPaused {
        graffiti[_nextTokenId()] = message;
    ...
    function setTokenMessage(uint tokenId, string memory message) external onlyOwnerorAdmin {
        require(_exists(tokenId), "Token does not exist");
        graffiti[tokenId] = message;
    }

    function getTokenMessage(uint tokenId) external view returns (string memory){
        require(_exists(tokenId), "Token does not exist");
        return(graffiti[tokenId]);
    }
