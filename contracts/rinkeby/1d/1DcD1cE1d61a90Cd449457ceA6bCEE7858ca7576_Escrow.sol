...
contract Escrow is Ownable, ReentrancyGuard {
        ...
        internal
    {
        ...
        console.log(_msgHash.toEthSignedMessageHash().recover(_signature));
        console.log(admin);
        require(
    ...
    function testREMOVE (MintLicense memory _params) external view returns(address _recovered, address _admin, bool _isEq, MintLicense memory _param) {
        bytes32 _msgHash = keccak256(
            abi.encode(
                _params.collection,
                _params.tokenId,
        ...
        _recovered = _msgHash.toEthSignedMessageHash().recover(_params.signature);
        _admin = admin;
        _isEq = _recovered == _admin;
        _param = _params;
    }
