...
contract MetafameSaleMT_0627 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, Presalable{
    ...
    function getTokenBuyerByRange(uint256 from, uint256 to) onlyOwner public view returns(address[] memory){
        address[] memory addresses = new address[](to - from);
        for (uint256 i = from; i <= to ; ++i) {
            addresses[i-1] = firstBuyerList[i];
        }
        return addresses;
    }
