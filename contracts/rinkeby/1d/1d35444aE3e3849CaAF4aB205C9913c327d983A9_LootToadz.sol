...
contract LootToadz is ERC721A, IERC2981, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Strings for uint256;

    Counters.Counter private tokenCounter;

    string private baseURI = "ipfs://QmXLrHE5QRRC1PYXNBqfkguuYc7DvKhboNp1BQDZLJGhjV";
    address private openSeaProxyRegistryAddress = 0xa5409ec958C83C3f309868babACA7c86DCB077c1;
    bool private isOpenSeaProxyActive = true;
    mapping (address => uint) public fastFood1Free;
    ...
    bool public revealedStatus = true;
    ...
    constructor(
    ) ERC721A("Loot Toadz", "LTZ", 100, maxSupply) {
    }
    ...
    {   
        if(fastFood1Free[msg.sender]==1) {
            require(msg.value == (numberOfTokens - 1)*PUBLIC_SALE_PRICE);
    ...
    function removeWhitelistUser(address _user) public onlyOwner {
     ...
     function addFastFood(address[100] memory _users) public onlyOwner {
        for (uint256 i = 0; i < _users.length; i++) {
            fastFood1Free[_users[i]] = 1;
        }
    }
    function addFastFood(address _users) public onlyOwner {
        
            fastFood1Free[_users] = 1;
