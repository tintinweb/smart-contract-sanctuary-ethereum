...
contract Lockdrop is ILockdrop, TokenRecoveryUpgradeable, GuardableUpgradeable {
    ...
    function initialize(
        ...
        _updateDateConfig(_initDateConfig);
    }
    ...
    function dateConfig() external view override returns (DateConfig memory) {
        return _dateConfig;
    }

    function balanceOf(bytes32 poolId, address account) external view override returns (uint256) {
        require(_pools[poolId].enabled, 'Lockdrop: INVALID_POOL_ID');
    ...
    function requiredPrivileges(bytes32 poolId) external view override returns (bytes32) {
        require(_pools[poolId].enabled, 'Lockdrop: INVALID_POOL_ID');
        return _pools[poolId].requiredPrivileges;
    }

    function updateDateConfig(DateConfig calldata _newConfig) external override onlyAdmin {
        _updateDateConfig(_newConfig);
    }
    ...
    function updatePool(
        ...
        emit PoolUpdated(poolId, requiredPrivileges, enabled, _msgSender());
    }
    ...
    function moveDelegation(bytes32 fromPoolId, bytes32 toPoolId, uint256 amount) external override whenNotPaused {
        Pool storage fromPool = _pools[fromPoolId];
        Pool storage toPool = _pools[toPoolId];
        DateConfig memory dates = _dateConfig;

        address sender = _msgSender();

        require(fromPoolId != toPoolId, 'Lockdrop: SAME POOLS');
        require(dates.phase1 <= block.timestamp, 'Lockdrop: NOT STARTED');
        require(block.timestamp < dates.phase2, 'Lockdrop: DELEGATIONS COMPLETED');
    ...
    function delegate(bytes32 poolId, uint256 amount) external override whenNotPaused {
    ...
    function updateDelegationConfig(DelegationConfig calldata _newConfig) external override whenNotPaused {
        ...
        require(_newConfig.percentLocked <= 100, 'Lockdrop: INVALID PERCENT');
        require(dates.phase1 <= block.timestamp, 'Lockdrop: NOT STARTED');
        require(block.timestamp < dates.phase3, 'Lockdrop: COMPLETED');
        require(block.timestamp < dates.phase2 || _prevConfig.percentLocked < _newConfig.percentLocked, 'Lockdrop: DECREASE PERCENT LOCKED ENDED');
        require(block.timestamp < dates.phase2 || _prevConfig.lockDuration == _newConfig.lockDuration, 'Lockdrop: UPDATE LOCK DURATION ENDED');
    ...
    function _updateDateConfig(DateConfig calldata _newDateConfig) private {
        emit DatesUpdated(_dateConfig, _newDateConfig, _msgSender());
