...
contract Collection is ERC721RoyaltyUpgradeable, OwnableUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;

    string public fURI;
    CountersUpgradeable.Counter private _tokenIdTracker;

    function initialize( string memory n,
        string memory s,
        uint96 num,
        string memory fungeURI) public initializer{
        __ERC721_init(n,s);
        __Ownable_init();
        _transferOwnership(tx.origin);
        __Context_init_unchained();
        _setDefaultRoyalty(tx.origin, num);
        fURI = fungeURI;
        _safeMint(tx.origin, _tokenIdTracker.current());
        _tokenIdTracker.increment();
    }

    function mint(uint96 num) public onlyOwner {
        _setTokenRoyalty(_tokenIdTracker.current(), _msgSender(), num);
        _safeMint(_msgSender(), _tokenIdTracker.current());
        _tokenIdTracker.increment();
    }

    function burn(uint256 tokenId) public onlyOwner{
        require(isApprovedOrOwner(owner(),tokenId), 'Should be Approved or Owner');
        _burn(tokenId);
    }
...
contract FungeProxy is Initializable {
    using AddressUpgradeable for address;
    using CountersUpgradeable for CountersUpgradeable.Counter;

    CountersUpgradeable.Counter public _collectionTracker;
    ...
    function createCollection(string memory name,
        string memory symbol,
        uint96 defaultRoyalty,
        string memory ipfsURI) public{
        Collection collection = new Collection();
        collection.initialize(name, symbol, defaultRoyalty, ipfsURI);
        collections[_collectionTracker.current()] = address(collection);
        _collectionTracker.increment();
    }
