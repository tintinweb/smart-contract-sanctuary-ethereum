...
contract SCSGStaking is ERC20, IERC721Receiver, ISCSGStaking, Ownable {
  ...
  uint256 launchTime;
  uint256 public timelockSeconds = 0 days;
  ...
  /**
   * @notice launch: start the staking pool
  ...
  function launch(
    ...
    require(
      _perBlockAmount > 0 && _perBlockAmount <= _rewardSupply,
      'per block amount must be more than 0 and less than supply'
    );

    // A locked date of '0' corresponds to being locked forever until the supply has expired and been rewarded to all stakers
    require(
  ...
  function stakeTokens(
    ...
    _staker.amountStakedBase = _staker.amountStaked.add(
  ...
  function unstakeTokens(uint256 _amountBoosted, bool _shouldHarvest) external {
    ...
    require(
      _amountBoosted <= _userBalance,
      'user can only unstake amount they have currently staked or less'
    );
    require(
      block.timestamp >= _staker.timeOriginallyStaked + timelockSeconds,
  ...
  function _sendRewardsToUser(address _user, uint256 _amount) internal {
    uint256 _totalToSend = _amount;
    require(
      _scsg.transfer(_user, _totalToSend),
