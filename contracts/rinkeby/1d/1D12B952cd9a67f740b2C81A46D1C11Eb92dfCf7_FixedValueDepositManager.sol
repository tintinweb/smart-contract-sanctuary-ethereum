...
import "./interfaces/IUnlockSchedule.sol";
  ...
  * @dev 
  */
contract FixedValueDepositManager is BaseGovernanceWithUserUpgradable {
     ...
     */
    function getProperties(uint256 id) public view returns(address beneficiary, uint256 additionTime, uint256 initial, uint256 claimed ) {
     ...
     */
    function addDeposits(bytes calldata data, uint256 totalAmount) external onlyLockContract returns(uint256 deposited){
        ...
        if(time > lockContract.lockStartTime()){
            uint256 claimablePart = schedule.unlockedAmount(1e36);
            for (uint256 i = 0; i < addresses.length; i++) {
                uint256 fullAmount = amounts[i] * 1e36 / (1e36 - claimablePart);
     ...
     */
    function split(uint256 originId, uint256[] memory splitParts, address[] memory addresses) external onlyLockContract returns(uint256[] memory){
        (, uint256 additionTime, uint256 initialAmount, uint256 amountClaimed) = getProperties(originId);
        ...
        _removeID(originId);
        for (uint256 i; i < amountBeneficiaries; i++) {
            uint256 _initialAmount = splitParts[i] * initialAmount / 1e18;
            uint256 _amountClaimed = splitParts[i] * amountClaimed / 1e18;
     ...
     */
    function updateClaimedAmount(uint256 id, uint256 amountToClaim) external onlyLockContract {
        require(
     ...
     */
    function transfer(address to, uint256 id) external onlyLockContract {
     ...
     */
    function verifyOwnership(address /*beneficiary*/, bytes calldata /*data*/) external pure returns(bool, uint) {
        return (false, 0);
    }
    ...
    function _checkSplit(
        ...
        uint256 total;
        for(uint256 i; i < splitParts.length; i ++) {
            total += splitParts[i];
        }
        if(total == 1e18){
            return true;
