...
abstract contract ERC721Gateway is IERC721Gateway, AnyCallApp {
    ...
    constructor(
        ...
        address token_
    ) AnyCallApp(anyCallProxy, anyCallExecutor) {
        setAdmin(msg.sender);
        token = token_;
    }
    ...
    function _anyExecute(uint256 fromChainID, bytes calldata data) internal override returns (bool success, bytes memory result) {
        (uint256 tokenId, , address receiver, , bytes memory extraMsg) = abi.decode(data, (uint256, address, address, uint256, bytes));
...
contract ERC721Gateway_MintBurn is ERC721Gateway {
    ...
    constructor(
        ...
        address anyCallExecutor,
        address token
    ) ERC721Gateway(anyCallProxy, anyCallExecutor, token) {}

    function _swapout(uint256 tokenId) internal virtual override returns (bool, bytes memory) {
    ...
    function _swapoutFallback(
        ...
        if (sender.isContract()) {
            try IGatewayClient(sender).notifySwapoutFallback(result, tokenId, swapoutSeq) returns (bool) {} catch {}
        }
        return result;
    }
}