    ...
    StacktGuardManager
{
    ...
    function execTransaction(
        ...
        /* -------------------------------- execution ------------------------------- */
        {
            ...
            // this makes it possible to use `estimateGas` on metamask without issues, as it searches for the minimum gas where the tx doesn't revert
            if (!success && gas == 0) {
                revert("GS013");
            }
            ...
            if (refundGas) {
                ...
                payable(tx.origin).transfer(payment);
                // bool refundSuccess = payable(tx.origin).send(payment);
