...
contract ERC4610 is Context, ERC165, IERC4610, IERC721Metadata {
     ...
     * Emits a {Transfer} event.
     */
    function _safeMint(address to, uint256 tokenId, uint256 amount) internal virtual {
     ...
     * Emits a {Transfer} event.
     */
    function _mint(address to, uint256 tokenId, uint256 amount) internal virtual {
        require(to != address(0), "ERC721: mint to the zero address");
        require(!_exists(tokenId), "ERC721: token already minted");
     ...
     * Emits a {Transfer} event.
     */
    function _burn(uint256 tokenId, uint256 amount) internal virtual {
        address owner = ownerOf(tokenId);
        console.log(owner);

        _beforeTokenTransfer(owner, address(0), tokenId);
        console.log(address(0), tokenId);
...
contract Wrapper is ERC4610, IWrapper, IERC721Receiver {
    ...
    function deposit(uint256 tokenId, uint256 amount) public override {
        ...
        emit Deposit(_msgSender(), tokenId, amount);
    }
...
/**
 * @dev OCT is the token that needs to be wrapped
 */
contract OCT is ERC721, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    function mint(string memory name, string memory _tokenURI, uint256 tokenId, uint256 amount) external {
        Data memory data = Data(name, amount);
        _data[tokenId] = data;
...
/**
 * @dev OCT is the WOCF after wrappering
 */
contract WOCT is Wrapper {

    constructor(address underlyingToken_, string memory name_, string memory symbol_) Wrapper(underlyingToken_,name_,symbol_) {
    }

    // add READ function to get ratity property value from OCNFT
    function rarity() external view returns (uint256) {
        return OCT(_underlyingToken).rarity();
    }

    // add READ function to get data property value from OCNFT
    function getData(uint256 tokenId) external view returns (string memory, uint256) {
        return OCT(_underlyingToken).getData(tokenId);
    }
...
interface IWrapper {
    ...
    event Deposit(address indexed caller, uint256 indexed tokenId, uint256 indexed amount);

    event Withdraw(address indexed caller, uint256 indexed tokenId, uint256 indexed amount);
