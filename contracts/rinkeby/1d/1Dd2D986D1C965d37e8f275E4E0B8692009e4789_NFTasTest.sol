...
contract NFTasTest is ERC721A, Ownable, ReentrancyGuard {
    ...
    bool internal bulkMintEnabled = false;
    ...
    constructor() payable ERC721A("NFTasTest", "NFTAS") {}
    ...
    function bulkMint(uint256 _qty) public payable nonReentrant {
        _checkPaused();
        _checkCanMint(_qty);
        if (owner() != msg.sender) {
            require(!paused, "Contract is paused!");
            require(!onlyWhitelisted, "Only available for whitelisted!");
        }
        _safeMint(msg.sender, _qty);
    }

    function giftToken(address _to) public onlyOwner {
        _checkCanMint(1);
        _safeMint(_to, 1);
    }
