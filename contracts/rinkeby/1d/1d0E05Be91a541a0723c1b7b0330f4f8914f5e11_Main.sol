...
contract Main is ERC721Enumerable, Ownable {
    ...
    uint256 public MAX_MINT_PER_PRESALE = 10;
    ...
    constructor(bytes32 _merkleRoot) ERC721("Crypcentra.com", "CRYP") {
    ...
    function mintForDev(uint amount) public onlyOwner {
    ...
    function airdropToDev(address[] calldata addresses, uint amount) public onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            address airdropAddress = addresses[i];
            for (uint j = 0; i < amount; j++) {
                uint mintIndex = totalSupply();
                _safeMint(airdropAddress, mintIndex);
                devMinted++;
            }
        }
    }
    ...
    function mintPresale(uint numberOfTokens, bytes32[] calldata proof) public payable {
        ...
        require(mintPerAddress[msg.sender] + numberOfTokens <= MAX_MINT_PER_PRESALE, "Can only mint 10 tokens at a time");
