...
// Method: LINEAR
        ...
        if(f.verifyFounder(_founder) == true){
            ...
            */
            ...
            for(uint i = 1; i <= _vestingMonths; i++){
                vestingDues[_vestId][_investor]._date[i] = _vestingStartDate + (i * _vestingMode * 1 minutes);
        ...
        for(uint i = 0; i < _investors.length; i++){
            ...
            */
            ...
            for(uint j = 1; j <= _vestingMonths; j++){
                vestingDues[_vestId][_investor]._date[j] = _vestingStartDate + (j * _vestingMode * 1 minutes);
    ...
    */
    ...
    // create an seperate array for date and fund [][]
                                                                                                          // due[] memory _dues
    function setNonLinearInstallments(address _founder, address _founderSmartContractAd, uint _vestId, address _investor,uint[] memory _date, uint[] memory _fund) public {
        require(msg.sender == _founder,"The connected wallet is not founder wallet");
        Founder f = Founder(_founderSmartContractAd);   // Instance from the founder smart contract. 
        if(f.verifyFounder(_founder) == true){
            uint duesAmount;
            for(uint i = 0; i < _date.length; i++){     // error with for loop status: resolved.
                vestingDues[_vestId][_investor]._date[i+1] = _date[i];
                vestingDues[_vestId][_investor]._status[i+1] = false;
                vestingDues[_vestId][_investor]._fund[i+1] = (_fund[i] * (10**18))/10000;  // added the 10 ** 18 condition here.
                duesAmount += vestingDues[_vestId][_investor]._fund[i+1];
            }
            installmentCount[_vestId][_investor] = _date.length;
