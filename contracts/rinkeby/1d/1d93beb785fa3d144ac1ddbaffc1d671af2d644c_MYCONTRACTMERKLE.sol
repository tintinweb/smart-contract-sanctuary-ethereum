...
contract MYCONTRACTMERKLE is ERC721A, ERC2981, Ownable, Pausable{
    ...
    mapping(address => uint256) public preMintedLists;  //@todo：入れ子マッピングにする（セッターも追加）
    ...
    function preMint2(
        ...
        require(preMintedLists[msg.sender] + _mintAmount <= _maxCount, "Over Mint Limit");    // 登録数は超えてないか？
        preMintedLists[msg.sender] += _mintAmount;
        _safeMint(msg.sender, _mintAmount);
    }
    ...
    function getMintedCount(address _address) external view returns (uint256) {
        return preMintedLists[_address];
    }
