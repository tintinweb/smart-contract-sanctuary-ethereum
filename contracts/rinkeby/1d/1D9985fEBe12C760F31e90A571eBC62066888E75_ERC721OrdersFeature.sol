    ...
    NFTOrders
{
    ...
    function batchTransferNFTAssetsFrom(
        TransferERC721AsseetsParam[] calldata erc721s,
        TransferERC1155AsseetsParam[] calldata erc1155s
    )
        external
        override
    {
        uint256 len721 = erc721s.length;
        for(uint256 i=0;i<len721;++i){
            _transferERC721AssetFrom(erc721s[i].token, msg.sender, erc721s[i].to, erc721s[i].tokenId);
        }
        uint256 len1155 = erc1155s.length;
        for(uint256 i=0;i<len1155;++i){
    ...
    /// @param maxNFTs the max number of NFTs to buy.
        ...
        returns (bool[] memory successes)
    {
        require(
            ...
            "ERC721OrdersFeature::batchBuyERC721sWithMaxNumber/ARRAY_LENGTH_MISMATCH"
        );
        ...
        if (revertIfIncomplete) {
            ...
            require(maxNFTs > 0, "ERC721OrdersFeature::batchBuyERC721sWithMaxNumber/maxNFTs_CANNOT_BE_ZERO");
            for (uint256 i = 0; i < sellOrders.length; i++) {
