...
contract SeeriousInvestoors is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(address _signer) ERC721A("SeeriousInvestoors", "SEERIOUS") {
        signer = _signer;
    }
    ...
    //Essential
    function publicMint(uint64 expireTime, bytes memory sig, uint256 numberofMints) external isSecured(1) payable {
        ...
        require(numberofMints + totalSupply() <= MAX_SUPPLY,"NOT_ENOUGH_SUPPLY");

        if(numberMinted(msg.sender) < 1) {
            require(msg.value == MINT_PRICE * (numberofMints - 1), "WRONG_ETH_VALUE");
        } else {
            require(msg.value == MINT_PRICE * numberofMints, "WRONG_ETH_VALUE");
        }
