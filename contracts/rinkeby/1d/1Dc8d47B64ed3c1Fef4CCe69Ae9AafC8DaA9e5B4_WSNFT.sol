...
abstract contract MerkleOGList {
    bytes32 internal _merkleOGRoot;
    function _setMerkleOGRoot(bytes32 merkleOGRoot_) internal virtual {
        _merkleOGRoot = merkleOGRoot_;
    }
    function isOGListed(address address_, bytes32[] memory proof_) public view returns (bool) {
        bytes32 _leaf = keccak256(abi.encodePacked(address_));
        for (uint256 i = 0; i < proof_.length; i++) {
            _leaf = _leaf < proof_[i] ? keccak256(abi.encodePacked(_leaf, proof_[i])) : keccak256(abi.encodePacked(proof_[i], _leaf));
        }
        return _leaf == _merkleOGRoot;
    }
}
...
abstract contract OGListMint {
    // Whitelist Minting
    bool internal _OGListMintEnabled; uint256 public _OGListMintTime;
    function _setOGListMint(bool bool_, uint256 time_) internal {
        _OGListMintEnabled = bool_; _OGListMintTime = time_; }
    modifier OGListMintEnabled {
        require(_OGListMintEnabled && _OGListMintTime <= block.timestamp, 
            "OGList Mint is not enabled yet!"); _; } 
    function OGListMintStatus() external view returns (bool) {
        return _OGListMintEnabled && _OGListMintTime <= block.timestamp; }
}


// ERC721_WS (ERC721I modified for WS) 

abstract contract ERC721_WS is ERC721I, Ownable, MerkleWhitelist, MerkleOGList, Security, PublicMint, WhitelistMint, OGListMint {

    constructor(string memory name_, string memory symbol_) ERC721I(name_, symbol_) {}

    // Ownable Functions for ERC721_WS //
    ...
    function setMerkleOGRoot(bytes32 merkleRoot_) external onlyOwner {
        _setMerkleOGRoot(merkleRoot_);
    }
    ...
    // OG Mint
    function setOGListMint(bool bool_, uint256 time_) external onlyOwner {
        _setOGListMint(bool_, time_);
    }
...
contract WSNFT is ERC721_WS, PayableGovernance {
    constructor() payable ERC721_WS("WS NFT", "WS") {}

    // Project Contraints
    uint256 public mintPrice = 0.0075 ether; // LAUNCH
    uint256 public maxTokens = 20; // LAUNCH

    uint256 public maxMintsPerWl = 1;
    uint256 public maxMintsPerOG = 2;
    mapping(address => uint256) public addressToWlMints;
    mapping(address => uint256) public addressToOGMints;
    ...
    function ogListMint(bytes32[] calldata proof_, uint256 amount_) external payable 
    onlySender OGListMintEnabled {
        require(isOGListed(msg.sender, proof_), 
            "You are not OGListed!");
        require(maxMintsPerOG >= addressToOGMints[msg.sender] + amount_,
            "Over Max Mints per TX or Not enough whitelist mints remaining for you!");
        require(msg.value == mintPrice * amount_,   
            "Invalid value sent!");
        
        // Add address to WL minted
        addressToOGMints[msg.sender] += amount_;
