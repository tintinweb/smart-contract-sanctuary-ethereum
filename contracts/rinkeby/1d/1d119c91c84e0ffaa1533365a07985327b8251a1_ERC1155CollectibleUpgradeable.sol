    ...
    ERC1155BurnableUpgradeable
{
    ...
    //current number of types (champion and item currently)
    ...
    // token Id of every token prefix (prefix is the season - tier - type)
    mapping(uint256 => uint256) public prefixToToken;
    ...
    function setSeason(uint256 _season) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_season > 0, "season must be greater than 0");
        season = _season;
    }

    function setTypes(uint256 _types) public onlyRole(DEFAULT_ADMIN_ROLE) {
        //32 bit max value
        uint256 typesMax = ~uint32(0);
        //shift left 10 bits to make space for 10 more max bits
        typesMax = typesMax << 10;
        //add 1023 to add 10 lower max bits
        typesMax += 1023;

        require(_types <= typesMax, "types limit reached");
        types = _types;
    }
    ...
    function setTiers(uint256 _tiers) public onlyRole(DEFAULT_ADMIN_ROLE) {
        tiers = _tiers;
    }
    ...
    ) public withinBounds(_season, _tier, _type) returns (uint256) {
    ...
    function create(
        ...
        //add the prefix to get complete token ID (NFT bit - season - tier - type - collectible ID)
        _id += prefix;
    ...
    function mintNFT(
        ...
        uint256 amount,
        bytes memory data
    ) public withinBounds(_season, _tier, _type) {
        require(id <= ~uint128(0), "invalid NFT ID");
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            if (_seasons[i] != 0 && _tiers[i] != 0 && _types[i] != 0) {
                ...
                isWithinBounds(_seasons[i], _tiers[i], _types[i]);
            }
            uint256 prefix = calculatePrefix(_seasons[i], _tiers[i], _types[i]);
            uint256 _id = ids[i] + prefix;
            require(_exists(_id), "NONEXISTENT_TOKEN");
            require(
                hasRole(MINTER_ROLE, msg.sender) || creators[_id] == msg.sender,
                "#mintBatch: ONLY_CREATOR_OR_APPROVED"
            );
    ...
    function mintRewards(
        ...
        //passing empty array of 5 elements to tell function to mint shards
        uint256[] memory _empty = new uint256[](5);
        mintBatch(_empty, _empty, _empty, ids, to, amounts, data);
    ...
    function _getNextTokenID(uint256 prefix) private returns (uint256) {
        require(
            (!isNonFungible(prefix) &&
                prefixToToken[prefix] + 1 <= ~uint256(0) - 1) ||
                (isNonFungible(prefix) &&
                    prefixToToken[prefix] + 1 <= uint256(~uint128(0))),
            "No more tokens can be created for this prefix"
        );
    ...
    function calculatePrefix(
        ...
        uint256 _tier,
        uint256 _type
    ) public pure returns (uint256 baseToken) {
        ...
        //construct prefix with season, tier and type
            ...
            //left shift _season by 128 bits (NFT ID) + 42 bits (type) + 42 bits(tier)
            _season = _season << (128 + 42 + 42);
            //left shift _tier by 128 bits (NFT ID) + 42 bits (type)
            _tier = _tier << (128 + 42);
            //left shift _type by 128 bits (NFT ID) + 42 bits (type)
            _type = _type << 128;
            //add it all to the base token
            baseToken = baseToken + _season + _tier + _type;
        }
    }
        ...
        )
    {
        ...
        if (isNonFungible(_id)) {
            _season = _id;
            _season = _season << 1;
            _season = _season >> (1 + 42 + 42 + 128);

            _tier = _id;
            _tier = _tier << (43 + 1);
            _tier = _tier >> (43 + 1 + 42 + 128);

            _type = _id;
            _type = _type << (42 + 43 + 1);
            _type = _type >> (42 + 43 + 1 + 128);

            _id = _id << 128;
            _id = _id >> 128;

            return (_season, _tier, _type, _id);
        } else {
            return (0, 0, 0, _id);
        }
    }
