...
contract BLXMRouter is Initializable, BLXMRewardProvider, IBLXMRouter {
    ...
    function _addLiquidity(
        ...
        require(blockNumberLast + 10 >= block.number, 'RATIO_EXPIRED');
...
contract BLXMRewardProvider is ReentrancyGuardUpgradeable, BLXMMultiOwnable, BLXMTreasuryManager, IBLXMRewardProvider {
    ...
    function updateRatio(address token, uint ratio) external override onlyOwner {
        getTreasury(token);
        Field storage fields = treasuryFields[token];
        emit UpdateRatio(msg.sender, token, fields.ratio = ratio, fields.blockNumberLast = block.number);
    }
