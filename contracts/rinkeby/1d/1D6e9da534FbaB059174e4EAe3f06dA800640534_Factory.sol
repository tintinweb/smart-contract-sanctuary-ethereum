...
import "./factoryChildContracts/Factory721.sol";
import "./interfaces/ITreasury.sol";
import "./marketERC721.sol";
...
contract Factory {
    ...
    constructor(NFTMarket _market, ITreasury _treasury) {
        market = _market;
        treasury = _treasury;
    }
    ...
    function createCollection(string memory name, string memory symbol) external {
        require(
            treasury.isOperator(msg.sender),
            "Not qualified to create collections"
        );
        Factory721 nft = new Factory721(market, name, symbol, msg.sender);
        emit ContractCreated(address(nft), msg.sender, 721, name, symbol);
    }
}
...
contract Factory721 is ERC721URIStorage {
    ...
    event Minted(address creator, uint256 tokenId, string indexedTokenUriPath);
    mapping(string => bool) public tokenURIExists;

    constructor(
        NFTMarket marketplaceAddress,
        string memory _name,
    ...
    modifier onlyOwner() {
        require(msg.sender==owner,"NFT: Function only callable by owner");
        _;
    }
    ...
    function mint(string memory tokenIPFSPath) public onlyOwner {
        _mint(msg.sender, tokenIPFSPath);
        emit Minted(msg.sender, tokenId.current(), tokenIPFSPath);
    }

    function mint(string memory tokenIPFSPath, uint256 price) public onlyOwner {
        uint256 token = _mint(msg.sender, tokenIPFSPath);
        market.createSale(token, address(this), price);
        emit Minted(msg.sender, tokenId.current(), tokenIPFSPath);
    }

    function _mint(address to, string memory tokenIPFSPath)
        internal
        returns (uint256)
    {
        require(
            !tokenURIExists[tokenIPFSPath],
            "NFT: Token with ipfs already exists"
        );
        tokenId.increment();
        _mint(to, tokenId.current());
        _setTokenURI(tokenId.current(), tokenIPFSPath);
        tokenURIExists[tokenIPFSPath] = true;
        return tokenId.current();
    }

    // Owner-only functions

    function changeMarketAddress(NFTMarket _market) public onlyOwner {
        market = _market;
    }
...
contract NFTMarket is ERC1155Holder {
    ...
    function createSale(
        ...
        IERC721(nftAddress).transferFrom(tx.origin, address(this), tokenId);
        ...
        emit MarketItemCreated(itemId, nftAddress, 721, tokenId, tx.origin, price, 1);
    }
    ...
    function createSale(
        ...
        emit MarketItemCreated(itemId, nftAddress, 1155, tokenId, tx.origin, price, amount);
    }

    function _buyToken(MarketItem memory sale, uint256 saleId) internal {
        if (sale.ercType == 721) {
            IERC721(sale.nftAddress).transferFrom(
                address(this),
                msg.sender,
                sale.tokenId
            );
    ...
    function distributeFunds(address payable seller, uint256 price) internal {
        uint256 commission = (price * getListingPrice() * 100) / 10000;
    ...
    function buyToken(uint256 itemId) public payable {
        ...
        distributeFunds(sale.seller, sale.price);
