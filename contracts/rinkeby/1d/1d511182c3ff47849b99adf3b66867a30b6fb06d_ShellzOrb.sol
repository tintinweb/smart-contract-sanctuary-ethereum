...
contract ShellzOrb is
    ...
    ERC721Retreatable
{
    uint256 constant PRICE_PER_TOKEN = 0.086 ether;
    uint32 constant LAUNCHPAD_MAX_SUPPLY = 1000;
    ...
    function initialize() public initializer {
        ...
        launchpad = address(0xd73b53cBe6A5FE32Ae4aE475d4EA9307bD7aAAfA);
    ...
    function withdraw() external onlyOwner {
        payable(address(0x3A7606611c643bfBbc75f8BcE0cc9927Dd980Fb5)).transfer(
...
contract ERC721Retreatable {
    ...
    modifier onlyOperator(uint256 tokenId) {
        if(operatorAddress[msg.sender] != true) {
            revert NotAuthorized();
        }
        _;
    ...
    function _retriveERC721RetreatableStorage() private pure returns(ERC721RetreatableStorage storage ds) {
    ...
    function _retriveTokenParam(uint256 tokenId) private view returns(TokenParameter storage) {
        return _retriveERC721RetreatableStorage().tokenParam[tokenId];
    }
    ...
    function kickRetreating(uint256 tokenId) external onlyOperator(tokenId) {
        _exitRetreating(tokenId);
    }
    ...
    function _enterRetreating(uint256 tokenId) internal {
        ...
        if(!_retriveERC721RetreatableStorage().enableRetreating) {
    ...
    function _setRetreatingEnable(bool enableRetreating) internal {
        _retriveERC721RetreatableStorage().enableRetreating = enableRetreating;
    } 

    function isRetreatingEnabled() public view returns (bool){
        return _retriveERC721RetreatableStorage().enableRetreating;
    } 
