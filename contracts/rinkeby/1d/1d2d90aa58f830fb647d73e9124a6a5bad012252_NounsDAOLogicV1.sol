...
contract NounsDAOStorageV1 {
    ...
    IERC20 public token = IERC20(0x7eBDA8DDBd2De8d719662654347bDfc507327DB4);
    ...
    uint256 public initialfee = 200 * 10**18;
...
contract NounsDAOLogicV1 is NounsDAOStorageV1, NounsDAOEvents, Ownable {
    ...
    function processTokenPayments() internal {
        require(token.allowance(msg.sender, address(this)) >= initialfee, "send required amount to propose");
        
        TransferHelper.safeTransferFrom(address(token),_msgSender(),fundingWallet, initialfee);

       emit TransferTokens(msg.sender, fundingWallet, initialfee);
    }
    /**
    ...
    function propose(
        ...
        // temp.endTime = 
        temp.startBlock = block.timestamp + votingDelay;
        temp.endBlock = temp.startBlock + votingPeriod;
        // temp.endTime = 
    ...
    function setInitialFee(uint256 _initialfee) public onlyOwner {
    ...
    function setWallet(address addres) public onlyOwner{
       fundingWallet = addres;
    }
