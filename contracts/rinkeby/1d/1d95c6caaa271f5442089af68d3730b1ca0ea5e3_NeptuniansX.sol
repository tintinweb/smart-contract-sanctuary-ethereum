...
// File: contracts/ERC721A/NftEncode.sol
 ...
 */
contract ERC721A is IERC721A, NftEncode {
    ...
    //Mapping from tokenId Minted
    mapping(string => uint256) nftMinted;

    //Mapping for token listed
    mapping(address => mapping(uint256 => Listing)) nftListings;

    //Mapping for token listed
    mapping(uint256 => string) nftTokenURI;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (bytes(nftTokenURI[tokenId]).length != 0) {
            string memory uri = nftTokenURI[tokenId];
            return uri;
        } else {
...
// File: contracts/ERC721A/NFT721A.sol
...
contract NeptuniansX is ERC721A, ERC2981, Ownable {
    ...
    function _setTokenURI(
        ...
        for (uint i = 0; i < tokenMinted.length; i++) {
            ...
            nftTokenURI[tokenId] = nftData[i].url;
        }
    }
