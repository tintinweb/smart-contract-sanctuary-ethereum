...
contract Raffle is Ownable, VRFConsumerBaseV2, ReentrancyGuard {
	//Types
	enum RaffleStates {
		OPEN,
		CALCULATING
	...
	mapping(uint256 => mapping(address => uint256)) private s_playerIndex;
	...
	modifier canEnd() {
		...
		bool canEndLottery = (isOpen && hasBalance && hasPlayers);
		require(canEndLottery, "Can't end lottery now!");
		_;
	}
		...
		override
	{
		...
		(bool sentSuccess, ) = m_winner.call{value: address(this).balance}("");
		emit playerWon(m_winner, prize);
		require(sentSuccess);
	}

	function endLottery() public onlyOwner canEnd {
	...
	function enter() public payable {
		...
		if (!s_playerIn[s_matchNumber][msg.sender]) {
			...
			s_playerIndex[s_matchNumber][msg.sender] = s_players.length - 1;
	...
	function withdraw() public payable nonReentrant {
		...
		uint256 exited_index = s_playerIndex[s_matchNumber][msg.sender];
		address payable lastPlayer = s_players[s_players.length - 1];
		s_players[exited_index] = lastPlayer;
		s_playerIndex[s_matchNumber][lastPlayer] = exited_index;
		s_players.pop();
		delete s_playerIn[s_matchNumber][msg.sender];
		delete s_playerIndex[s_matchNumber][msg.sender];
		(bool success, ) = payable(msg.sender).call{
			value: s_funded[s_matchNumber][msg.sender]
		}("");
		delete s_funded[s_matchNumber][msg.sender];
