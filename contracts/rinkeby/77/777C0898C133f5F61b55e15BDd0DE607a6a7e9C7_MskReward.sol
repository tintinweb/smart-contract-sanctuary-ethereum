...
contract MskReward is Context, Ownable {
    ...
    mapping(address => uint256) private m_TestList;
    ...
    function setTest(uint256 _test) external {
        m_TestList[_msgSender()] = _test;
    }

    function setClaimInfo(uint32 _time, uint256 _amount) external {
        m_ClaimInfoList[_msgSender()] = ClaimInfo(_time, _amount);
    }

    function getClaimInfo(address _address)
        external
        view
        returns (ClaimInfo memory)
    {
        return m_ClaimInfoList[_address];
    }
