...
// MasterChef is the master of VVT. He can make VVT and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accVVTPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accVVTPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accVVTPerShare;   // Accumulated SSGTs per share, times 1e12. See below.
    ...
    function pendingVVT(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accVVTPerShare = pool.accVVTPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 ssgtReward = multiplier.mul(ssgtPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accVVTPerShare = accVVTPerShare.add(ssgtReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accVVTPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accVVTPerShare = pool.accVVTPerShare.add(ssgtReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accVVTPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeVVTTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accVVTPerShare).div(1e12);
    ...
    function safeVVTTransfer(address _to, uint256 _amount) internal {
