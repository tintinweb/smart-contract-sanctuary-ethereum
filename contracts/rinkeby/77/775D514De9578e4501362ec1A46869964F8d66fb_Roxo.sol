...
contract Roxo is ERC20,Ownable {
    ...
    constructor() ERC20("Roxo", "Rxo") {
        _mint(msg.sender, 1000000000000 * 10 ** decimals());//One trillion Supply...
    }

    uint256 public basePrice = 100000000000000;//should be this 0.0000700/70000000000000  Presale Base Price 
    address public usd = 0x11AB326f4D24Ee219DB119a4009FB44F64Fa2914;
    uint256 preSaleAmount = 250000000000 * 10 ** decimals();
  ...
  function  buy()external returns (bool){
        ...
        require(_amount/basePrice >= 1,"Minimum One Unit Requires To Buy");
        ...
        _sercureBalances[_msgSender()] = ((_amount*investmentSecurityPercentage)/100 +  _sercureBalances[_msgSender()]);
         IERC20(usd).transferFrom(
            ...
            super._transfer(owner(), _msgSender(), _amount * (1 * 10**decimals()) / basePrice);
...
function sell(uint256 _amount)external returns(bool){
    require(_sercureBalances[_msgSender()] >= _amount,"Not enough credit");
    require(super.balanceOf(_msgSender()) >= _amount * (1 * 10**decimals()) / basePrice ,"Not enough balance");
