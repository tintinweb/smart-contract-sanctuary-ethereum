...
contract Shrimp is ERC721ABurnable, Ownable, ReentrancyGuard {
 ...
 function whitelistFreeAdoption(bytes32[] memory _merkleProof, uint adoptCount) validateAdoption(adoptCount) nonReentrant external payable {
     require(currentMode == ReleaseMode.OPEN, "It's not go time yet.");
 ...
 function generalFreeAdoption(uint adoptCount) validateAdoption(adoptCount) requireCorrectEth(adoptCount) nonReentrant external payable{
     require(currentMode == ReleaseMode.OPEN, "It's not go time yet.");
    
    if(_freeBabyRedeemed[msg.sender] != 1){ 
      if(freeClaimed < 500){
      // The amount we're charging (e.g, the amount we're validating) will depend on
       // if we detected a free mint match. If so we check for a lesser amount of eth.
         uint adoptionFeesToCharge = adoptCount;
         adoptionFeesToCharge = adoptionFeesToCharge;
         _freeBabyRedeemed[msg.sender] = 1;
