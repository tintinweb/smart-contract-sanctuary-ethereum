...
contract SneakerHeadsERC721A is Ownable, ERC721ACommon, PaymentSplitter {
    ...
    mapping(uint256 => uint256) private nestingTotal;
     ...
     /**
    @dev MUST only be modified by safeTransferWhileStacking(); if set to 2 then
    the _beforeTokenTransfer() block while stacking is disabled.
     */
    uint256 private stackingTransfer = 1;
    ...
    stacking.
    @dev Stacking is tied to a specific Sneaker Heads, not to the owner, so it doesn't
    reset upon sale.
    @return nesting Whether the Sneaker Heads is currently stacking. MAY be true with
    ...
    @return total Total period of time for which the Sneaker Heads has stacking across
    ...
    thus not resetting the stacking period.
     */
    function safeTransferWhileStacking(
        address from,
        address to,
        uint256 tokenId
    ) external {
    ...
    function _beforeTokenTransfers(
        ...
        for (uint256 end = tokenId + quantity; tokenId < end; ++tokenId) {
            require(
                stackingStarted[tokenId] == 0 || stackingTransfer == 2,
                "Sneaker Heads: stacking"
            );
        }
    }

    /**
    @dev Emitted when a Sneaker Heads begins stacking.
     */
    event Stacked(uint256 indexed tokenId);

    /**
    @dev Emitted when a Sneaker Heads stops stacking; either through standard means or
    by expulsion.
     */
    event UnStacked(uint256 indexed tokenId);

    /**
    @dev Emitted when a Sneaker Heads is expelled from the stack.
     */
    ...
    /**
    @notice Changes the Sneaker Heads stacking status.
    */
        ...
        onlyApprovedOrOwner(tokenId)
    {
        uint256 start = stackingStarted[tokenId];
        if (start == 0) {
            require(stackingOpen, "Sneaker Heads: stacking closed");
            stackingStarted[tokenId] = block.timestamp;
            emit Stacked(tokenId);
        } else {
            nestingTotal[tokenId] += block.timestamp - start;
            stackingStarted[tokenId] = 0;
            emit UnStacked(tokenId);
        }
    }

    /**
    @notice Changes the Sneaker Heads stacking status
    @dev Changes the Sneaker Heads stacking sheep (see @notice).
     */
    ...
    /**
    @notice Only owner ability to expel a Sneaker Heads from the stack.
    @dev As most sales listings use off-chain signatures it's impossible to
    detect someone who has stacked and then deliberately undercuts the floor
    ...
    Sneaker Heads.
     */
    function expelFromStack(uint256 tokenId) external onlyOwner {
        require(stackingStarted[tokenId] != 0, "Sneaker Heads: not stacked");
        nestingTotal[tokenId] += block.timestamp - stackingStarted[tokenId];
        stackingStarted[tokenId] = 0;
        emit UnStacked(tokenId);(tokenId);
