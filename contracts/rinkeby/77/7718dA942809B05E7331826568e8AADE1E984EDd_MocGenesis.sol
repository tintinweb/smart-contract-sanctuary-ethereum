...
contract MocGenesis is ERC721AQueryable, Ownable, MerkleProofWrapper {
    ...
    uint256 public maxAddrFreeSupply = 2;

    bool public pausedBuy = true;
    bool public isPreSale = false;

    mapping(address => uint256) public addressMintedCount;
    mapping(address => uint256) public addrFreeMintCount;
    ...
    // function getMaxAddrSupply() public view returns (uint256) {
    //     return maxAddrSupply;
    // }

    // function getMaxAddrFreeSupply() public view returns (uint256) {
    //     return maxAddrFreeSupply;
    // }

    // function getMaxReserve() public view onlyOwner returns (uint256) {
    //     return maxAdminReserve;
    // }
    ...
    function setMaxAddrFreeSupply(uint256 _maxAddrFreeSupply) public onlyOwner {
        require(_maxAddrFreeSupply>=1, "amount error");
        require(_maxAddrFreeSupply<=maxAddrSupply, "amount error");
        maxAddrFreeSupply = _maxAddrFreeSupply;
    }
    ...
    function setPausedBuy(bool _pausedBuy) public onlyOwner {
        pausedBuy = _pausedBuy;
    }
    ...
    function splitMintAmount(uint256 _amount) public view returns (uint256, uint256) {
        uint256 totalFreeMintLeft = (maxFreeSupply > _freeMintCount) ? (maxFreeSupply - _freeMintCount) : 0;
        uint256 addrFreeMintLeft = (maxAddrFreeSupply > addrFreeMintCount[msg.sender]) ? (maxAddrFreeSupply - addrFreeMintCount[msg.sender]) : 0;
        if (addrFreeMintLeft > totalFreeMintLeft) {
            addrFreeMintLeft = totalFreeMintLeft;
        }

        if (_amount > addrFreeMintLeft) {
            return (addrFreeMintLeft, _amount - addrFreeMintLeft);
        }else {
            return (_amount, 0);
        }
    }
    ...
    function buy(uint256 _amount) public payable {
        require(!pausedBuy, "sell is paused");
        require(_amount>=1, "sell amount error");
        require(_amount<=maxAddrSupply, "sell amount exceed");
        require((maxAdminReserve + _freeMintCount + _mintCount + _amount) <= maxTotalSupply, "not enough tokens for buyer");
        require((totalSupply() + _amount) <= maxTotalSupply, "not enough tokens");
        require(_numberMinted(msg.sender) + _amount <= maxAddrSupply, "exceed max personal supply");
        require(msg.sender == tx.origin, "mint from contract not allowed");

        // uint256 price = publicSalePrice * _amount;
        // if (_freeMintCount + _amount <= maxFreeSupply ) {
        //     price = 0;
        //     _freeMintCount += _amount;
        // } else if (_freeMintCount >= maxFreeSupply ) {
        //     price = publicSalePrice * _amount;
        //     _mintCount = _mintCount + _amount;
        // } else {
        //     price = publicSalePrice * (_freeMintCount + _amount - maxFreeSupply);
        //     _freeMintCount += (maxFreeSupply - _freeMintCount);
        //     _mintCount += (_freeMintCount + _amount - maxFreeSupply);
        // }

        uint256 freeMint;
        uint256 mint;
        (freeMint, mint) = splitMintAmount(_amount);
        if (freeMint > 0) {
            _freeMintCount += freeMint;
            addrFreeMintCount[msg.sender] += freeMint;
        }
        if (mint > 0) {
            _mintCount += mint;
        }
        
        uint256 price = publicSalePrice * mint;
    ...
    function adminBuy(uint256 _amount,bytes32[] memory proof) public {
    ...
    function adminMint(uint256 _amount, address _to) public onlyOwner {
        ...
        _reserveCount += _amount;
        // addressMintedCount[_to]+=_amount;
