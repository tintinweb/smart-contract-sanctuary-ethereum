...
contract MyToken is ERC721, ERC721Burnable, ReentrancyGuard, AdminPrivileges, ALSalePeriod, AdminPause, RoyaltiesConfig {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;

    uint256 public MAX_SUPPLY = 10000;
    uint256 public WALLET_CAP_AL = 5;
    uint256 public WALLET_CAP = 20;
    uint256 public PRICE_AL = 0.08 ether;
    ...
    constructor(string memory _unrevealedURI) ERC721("MyToken", "MTK") ALSalePeriod(24) {
    ...
    function setWalletCaps(uint256 walletCap, uint256 walletCapAL) public onlyAdmins {
        WALLET_CAP = walletCap;
        WALLET_CAP_AL = walletCapAL;
    }
    ...
    function setUnrevealedURI(string memory URI) public onlyAdmins {
        unrevealedURI = URI;
    }
    ...
    function allowlistPurchase(uint256 amount, bytes32[] calldata _merkleProof) public payable onlyDuringALPeriod nonReentrant {
        require(
            amount + mintClaimed[msg.sender] <= WALLET_CAP_AL,
            string(
                abi.encodePacked(
                    "Maximum tokens per wallet during allowlist phase is ",
                    Strings.toString(WALLET_CAP_AL)
                )
            )
        );
        ...
        purchase(PRICE_AL, amount);
    }

    function publicPurchase(uint256 amount) public payable onlyDuringPublicSale nonReentrant {
        require(
            amount + mintClaimed[msg.sender] <= WALLET_CAP,
            string(
                abi.encodePacked(
                    "Maximum tokens per wallet is ",
                    Strings.toString(WALLET_CAP)
                )
            )
        );
    ...
    function purchase(uint256 price, uint256 amount) internal whenNotPaused {
        require(amount > 0, "Must purchase more than 0 tokens");
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, RoyaltiesConfig) returns (bool) {
        return ERC721.supportsInterface(interfaceId) || RoyaltiesConfig.supportsInterface(interfaceId);
    }
}
