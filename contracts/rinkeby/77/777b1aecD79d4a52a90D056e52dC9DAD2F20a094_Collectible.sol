...
contract Collectible is ICollectible, AccessControl, ERC721Enumerable, ERC721Pausable, ERC721Burnable, Ownable, ReentrancyGuard {
    ...
    */
    function setMintPassToken(address _mintPassToken) external override onlyRole(CONTRACT_ADMIN_ROLE) {
        mintPassFactory = MintPassFactory(_mintPassToken); 
    }    
    ...
    */
    function setRedeemStart(uint256 passID, uint256 _windowOpen) external override onlyRole(CONTRACT_ADMIN_ROLE) {
    ...
    */
    function setRedeemClose(uint256 passID, uint256 _windowClose) external override onlyRole(CONTRACT_ADMIN_ROLE) {
    ...
    */
    function setMaxRedeemPerTxn(uint256 passID, uint256 _maxRedeemPerTxn) external override onlyRole(CONTRACT_ADMIN_ROLE) {
    ...
    */
    ...
    function fusion(
        ...
        require(_inputTokenIDs.length % fusionConfig.numTokensRequired == 0, "Wrong multiple of required token quantity sent");
        //check for ownership
        Collectible contract721 = Collectible(this); 
        for(uint256 i = 0; i < _inputTokenIDs.length; i++) {
            require(contract721.ownerOf(_inputTokenIDs[i]) == msg.sender, "Sender does not own this token");
            require(tokenTypes[_inputTokenIDs[i]] == fusionConfig.tokenIDRequired, "Wrong token type");
        }

        uint256 numToIssue = _inputTokenIDs.length.div(fusionConfig.numTokensRequired);
        ...
        for(uint256 i = 0; i < numToIssue; i++) {
            ...
            tokensMinted = string(abi.encodePacked(tokensMinted, generalCounter.current().toString(), ","));
    ...
    */
    ...
    //TODO: allow for direct purchase of a certain type of token
    function purchase(
        uint256 quantity,
        uint256 tokenType, 
        ...
        require(tokenType == saleConfiguration.tokenType, "Sale: Token type not on sale");
        if(saleConfiguration.requireSignature){
            require(ECDSA.recover(keccak256(abi.encodePacked(msg.sender, quantity,  tokenType)).toEthSignedMessageHash(), signature) == saleConfiguration.signerAddress, "Invalid Signature");
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        console.log("tokenIde", tokenId);
        console.log("token.name", token.name);
        console.log("token.description", token.description);
        console.log("token.image", token.image);
        console.log("token.url", token.url);
        console.log("tier", membershipTiers[tokenTypes[tokenId]].name);
      ...
      bytes(
        string(
          abi.encodePacked(
            ...
            '", "external_url": "',
            token.url,
            '", "attributes": [{"trait_type": "Membership Tier","value": "', membershipTiers[tokenTypes[tokenId]].name,
            '"}]}'
          )
        )
      )
    ...
    function contractURI() public view returns (string memory) {
     ...
     function isUniqueArray(uint256[] calldata _array) internal pure returns(bool){
