...
contract ClaimLegacy is Ownable, ReentrancyGuard {
        ...
        
    {
        ...
        firstGen = IWallet(0xd9145CCE52D386f254917e481eB44e9943F39138);
        secondGen = IWallet(0xD7ACd2a9FD159E69Bb102A1ca21C9a3e3A5F771B);
    ...
    function claim(uint256 quantity, uint256 tokenId, bytes32[] calldata proof) public payable {
        ...
        require(
            isValid(proof, keccak256(abi.encodePacked(msg.sender))), "not whitelisted"
        );
        require(minted[msg.sender] + quantity <= maxPerAddress, "mint per wallet limited already reached");
        ...
        uint256[] memory secondGenTokenId = secondGen.walletOfOwner(legacyWallet);

        
        secondGen.transferFrom(legacyWallet, msg.sender, secondGenTokenId[0]);
        count++;
        minted[msg.sender]++;
        }
