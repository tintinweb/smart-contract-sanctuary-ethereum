...
contract SystemContext is SystemContextSubscribable {
    ...
    constructor (CollectionRegistry collectionRegistry_, AccessControlList accessControlList_, uint16 chainId_) {
    ...
    function setCollectionRegistry(CollectionRegistry collectionRegistry_, bool triggerCallbacks) external onlyRole(accessControlList.SYSTEM_CONTEXT_DEFAULT_ADMIN_ROLE()) {
        if (triggerCallbacks) {
...
contract LayerZeroBridge is ILayerZeroReceiver {
    ...
    function handleCall(CallData memory callData, uint16 srcChainId, bytes memory fromAddress, uint64 nonce) internal returns (bytes memory) {
        (,address target,) = systemContext.getCollectionRegistry().getRecord(callData.collectionId);
    ...
    function handleMultiCall(MultiCallData memory multiCallData, uint16 srcChainId, bytes memory fromAddress, uint64 nonce) internal returns (bytes[] memory) {
    ...
    function handleDeploy(DeployData memory deployData, uint16 srcChainId, bytes memory fromAddress, uint64 nonce) internal returns (address) {
        address deployedContractAddr = systemContext.getContractFactory().createContractInstanceByHash(deployData.bytecodeHash, deployData.ctorParams, deployData.collectionId);
        emit ContractDeployed(srcChainId, fromAddress, nonce, deployedContractAddr);
        systemContext.getCollectionRegistry().registerCollection(deployData.collectionId, deployData.collectionName, address(this), deployedContractAddr);
        return deployedContractAddr;
    }

    function handleERC721Bridge(ERC721BridgeData memory callData, uint16 srcChainId, bytes memory fromAddress, uint64 nonce) internal returns (address)
    {
        bytes memory ctorParams = abi.encode(callData.name, callData.symbol, callData.collectionId, address(this), systemContext);
        (address newAddress) = systemContext.getContractFactory().createContractInstanceByHash(callData.bytecodeHash, ctorParams, callData.collectionId);
        if (isContract(newAddress)) {
            emit CallSuccess(srcChainId, fromAddress, nonce, address(systemContext.getContractFactory()), abi.encodePacked(newAddress), 0);
        } else {
            emit CallFailed(srcChainId, fromAddress, nonce, address(systemContext.getContractFactory()), 0);
        }
        return newAddress;
    }

    function handleUndefined(Data memory data, uint16 srcChainId, bytes memory fromAddress, uint64 nonce) internal {
    ...
    function _handleAll(
        ...
        if (data.operation == Operation.CALL) {
            ...
            handleCall(callData, _srcChainId, _fromAddress, _nonce);
        } else if (data.operation == Operation.DEPLOY) {
            DeployData memory deployData = abi.decode(data.rawData, (DeployData));
            handleDeploy(deployData, _srcChainId, _fromAddress, _nonce);
        } else if (data.operation == Operation.MULTI_CALL) {
            MultiCallData memory callData = abi.decode(data.rawData, (MultiCallData));
            handleMultiCall(callData, _srcChainId, _fromAddress, _nonce);
        } else if (data.operation == Operation.ERC721_BRIDGE) {
            ERC721BridgeData memory callData = abi.decode(data.rawData, (ERC721BridgeData));
            handleERC721Bridge(callData, _srcChainId, _fromAddress, _nonce);
        }
        else {
            handleUndefined(data, _srcChainId, _fromAddress, _nonce);
        }
    }
    ...
    function estimateLzReceiveGas(
        ...
        bytes memory _fromAddress,
        uint64 _nonce,
        bytes memory _payload) external returns(uint256) {
        uint startGas = gasleft();

        address(this).staticcall(abi.encodeWithSelector(this.handleAllEstimate.selector, _srcChainId, _fromAddress, _nonce, _payload));
    ...
    function _sendMessageWithValue(uint16 chainId_, bytes calldata bridge_, bytes memory buffer_, address refundAddress_, uint256 value_) internal {
    ...
    function _sendMessage(uint16 chainId_, bytes calldata bridge_, bytes memory buffer_, address refundAddress_) internal {
        _sendMessageWithValue(chainId_, bridge_, buffer_, refundAddress_, msg.value);
    }
        ...
        return abi.encode(
            Data({
                ...
                rawData: abi.encode(CallData({
                    ...
                    packedData: abi.encodeWithSignature("mintToWithUri(address,uint256,string)", owner_, mintId_, tokenUri_)
                }))
            })
        );
    }
    ...
    function mintOnTargetChain(
        ...
        _sendMessage(chainId_, bridge_, mintOnTargetChainEncode(collectionId_, owner_, mintId_, tokenUri_), refundAddress_);
    }

    function burnOnTargetChainEncode(bytes32 collectionId_, uint256 burnId_) public view returns(bytes memory) {
        return abi.encode(
            Data({
                operation: Operation.CALL,
                apiVersion: apiVersion,
    ...
    function burnOnTargetChain(
        ...
        _sendMessage(chainId_, bridge_, burnOnTargetChainEncode(collectionId_, burnId_), refundAddress_);
    }
    ...
    function callOnTargetChain(
        ...
        _sendMessage(chainId_, bridge_, callOnTargetChainEncode(collectionId_, callData_), refundAddress_);
    }
    ...
    function deployContractEncode(bytes32 bytecodeHash_, bytes32 collectionId_, bytes calldata ctorParams_, string calldata collectionName_) public view returns(bytes memory) {
        return abi.encode(
            Data({
    ...
    function deployMultipleContracts(
        ...
        bytes32 _collectionId = keccak256(abi.encodePacked(block.timestamp, msg.sender));
        for (uint256 i = 0; i < deploymentParams_.length; i++) {
            if(deploymentParams_[i].chainId == systemContext.chainId()){
                handleDeploy(
                    DeployData({bytecodeHash: bytecodeHash_,
                                 ctorParams: deploymentParams_[i].params,
                                 collectionId: _collectionId,
                                 collectionName: collectionName_})
                ,deploymentParams_[i].chainId, abi.encodePacked(address(this)), 0);
            }
            else {
                _sendMessageWithValue(
                    deploymentParams_[i].chainId,
                    deploymentParams_[i].bridgeAddress,
                    deployContractEncode(bytecodeHash_, _collectionId, deploymentParams_[i].params, collectionName_),
                    refundAddress_,
    ...
    function callMultipleOnTargetChainEncode(address[] calldata destinationContracts_, bytes[] calldata callData_) public view returns(bytes memory) {
        return abi.encode(
            Data({
                operation: Operation.MULTI_CALL,
                apiVersion: apiVersion,
                rawData: abi.encode(MultiCallData({destinationContracts : destinationContracts_, packedData : callData_}))
            })
        );
    }

    function callMultipleOnTargetChain(
        uint16 chainId_,
        bytes calldata bridge_,
        address[] calldata destinationContracts_,
        address refundAddress_,
        bytes[] calldata callData_
    ) public payable onlyRole(systemContext.getAccessControlList().BRIDGE_CALLER_ROLE()) {
        _sendMessage(chainId_, bridge_, callMultipleOnTargetChainEncode(destinationContracts_, callData_), refundAddress_);
    }

    function bridgeERC721Encode(
        string memory name_,
        string memory symbol_,
        address originalERC721Address_,
        bytes32 bytecodeHash_
    ) public view returns(bytes memory) {
        return abi.encode(
            Data({
                operation: Operation.ERC721_BRIDGE,
                apiVersion: apiVersion,
                rawData: abi.encode(ERC721BridgeData({
                    name : name_,
                    symbol : symbol_,
                    collectionId : keccak256(abi.encodePacked(originalERC721Address_)),
                    bytecodeHash : bytecodeHash_
                }))
            })
        );
    ...
    function bridgeERC721(
        ...
        require(IERC165(originalERC721Address_).supportsInterface(_INTERFACE_ID_ERC721_METADATA), "Token doesnt support ERC721Metadata");
        string memory name_ = IERC721Metadata(originalERC721Address_).name();
        require(bytes(name_).length > 0, "token name is empty");
        string memory symbol_ = IERC721Metadata(originalERC721Address_).symbol();
        require(bytes(symbol_).length > 0, "token symbol is empty");

        _sendMessage(chainId_, bridge_, bridgeERC721Encode(name_, symbol_, originalERC721Address_, bytecodeHash_), refundAddress_);
    }
}
...
contract ContractFactory {
    event NewContractRegistered(bytes32 indexed hash, string name);
    ...
    * @dev Registers whitelisted bytecode at its hash.
     ...
     */
    function registerContract(bytes memory bytecode, string memory name, uint16 version) external onlyRole(systemContext.getAccessControlList().CONTROL_LIST_ADMIN_ROLE()) {
        bytes32 hash = keccak256(bytecode);
        if (latestContractVersion[name] != 0) {
            ...
            require(version == 1, "Versioning of contracts should start from '1'");
            emit NewContractRegistered(hash, name);
            require(bytes(contractName[hash]).length == 0, string(
                    abi.encodePacked("Contract '", name, "' registered with name '", contractName[hash], "'")
                ));
        }
...
contract CollectionRegistry is ICollectionRegistry {
    ...
    */
    ...
    function getRecord(bytes32 collectionId) public view returns (address, address, string memory) {
        return (records[collectionId].owner, records[collectionId].addr, records[collectionId].name);
    }
}
...
contract ERC721Mintable is ERC721Enumerable, AccessControl {
    ...
    constructor(string memory name_, string memory symbol_, bytes32 collectionId_,
        address owner_, SystemContext system_) ERC721(name_, symbol_) {
        collectionId = collectionId_;
    ...
    */
    ...
    function moveTo(uint16 _l0ChainId, bytes calldata _destinationBridge, uint256 _tokenId) external payable {
        require(_isApprovedOrOwner(msg.sender, _tokenId), "ERC721: transfer caller is not owner nor approved");
        transferFrom(msg.sender, address(this), _tokenId);

        system.getBridge().mintOnTargetChain{value : msg.value}(_l0ChainId, _destinationBridge, collectionId, msg.sender, msg.sender, _tokenId, _tokenURIs[_tokenId]);
    }
...
interface ICollectionRegistry {
    ...
    event NewCollection(bytes32 indexed collectionId, string name, address owner, address addr);
    ...
    function registerCollection(bytes32 collectionId_, string calldata name_, address owner_, address collectionAddress_)  external;
