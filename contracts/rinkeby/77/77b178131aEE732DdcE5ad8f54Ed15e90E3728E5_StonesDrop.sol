...
contract StonesDrop is ReentrancyGuard, AccessControl {
    ...
    function _pickNft() internal returns (ICollectionV3, uint256) {
        uint16 position = uint16(_getRandom(tree[0]));
    ...
    function _getRandom(uint256 gamerange) internal view returns (uint256) {
        return
            1 +
            (uint256(
                keccak256(
