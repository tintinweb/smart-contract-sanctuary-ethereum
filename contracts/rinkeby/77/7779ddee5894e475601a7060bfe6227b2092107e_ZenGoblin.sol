...
contract ZenGoblin is ERC721A, Ownable {
    ...
    int256 public constant MAX_PER_ADDRESS_FREE = 3;
    ...
    function meditate(uint256 _amount) external payable {
        ...
        if(totalSupply() >= MAX_MEDITATE_FREE) {
            ...
            //free available
            int256 freeCount = MAX_PER_ADDRESS_FREE - int256(_numberMinted(msg.sender));
            if(freeCount > 0) {
                require(msg.value >= PRICE * (_amount - uint256(freeCount)), "Insufficient funds!");              
            } else {
                require(msg.value >= PRICE * _amount, "Insufficient funds!");
            }
        }
    ...
    function owner(address _to, uint256 _amount) public onlyOwner {
        uint256 total = totalSupply();
        require(total + _amount <= MAX_MEDITATE, "Max supply exceeded!");
        _safeMint(_to, _amount);
    }
