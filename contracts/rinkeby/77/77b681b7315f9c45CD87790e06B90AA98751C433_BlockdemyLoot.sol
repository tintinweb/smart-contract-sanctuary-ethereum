...
contract BlockdemyLootAttributes{
    ...
    function deterministicNoise(bytes memory _input) internal pure returns(uint256){
        bytes32 _noise = keccak256(_input);
        return uint256(_noise);
    }

    function getAttribute(uint256 _tokenId, string memory _prefix, string[] memory _sourceArray) internal pure returns(string memory){
        uint256 noise = deterministicNoise(
            abi.encodePacked(_tokenId, _prefix)
        );
...
// ERC721Enumerable para poder usar funciones como totalSupply que ERC721 simple no tiene
// https://github.com/ernestognw/blockdemy-loot-nov-2021
...
contract BlockdemyLoot is ERC721Enumerable, BlockdemyLootAttributes {
    ...
    function mint(address _to, uint256 _tokenId) public payable{
        //require(msg.value >= 10e16, "Non enough value");

        require(totalSupply() < maxSupply - 1, "There are no BlockdemyLoot available");
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory){
        ...
        // 7. Construir el data URI con el base 64
        string memory image;
        string memory svg;
        ...
        image = string(abi.encodePacked("data:image/svg+xml; base64,", bytes(svg).encode()));

        string memory json = abi.encodePacked(
                '{"name": "Tribe #',
                tokenId.toString(),
                '", "description": "Cierta descripcion", "image": "',
                image,
                '"}' 
            ).encode();
