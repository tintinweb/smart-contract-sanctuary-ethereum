...
contract ChineseRestaurant is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    event TokenBurn(address indexed from, uint256 value);
    event SetLiquidityFee(uint256 amount);
    event SetBonkFundFee(uint256 amount);
    event SetBurnFee(uint256 amount);
    
    string private _name = "Chinese  Restaurant";
    string private _symbol = "CHINA";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * 10**18;
    
    address payable public bonkFundAddress = payable(0x35C6Ecc1A9D64e55caC26c6e6BF1585F578E9A04);
    address public bonkFundWalletToken = 0x35C6Ecc1A9D64e55caC26c6e6BF1585F578E9A04;
    ...
    constructor () {
        ...
        _bonkFundFee = 3;
        _burnFee = 2;
        _totalFees = _liquidityFee.add(_bonkFundFee).add(_burnFee);
        _totalFeesToContract = _liquidityFee.add(_bonkFundFee);

        _liquifyThreshhold = 20000 * 10**18;
        _maxBalance = 20000 * 10**18;
    ...
    function setBonkFundAddress(address payable newBonkFundAddress) external onlyOwner() {
        bonkFundAddress = newBonkFundAddress;
    }

    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(newLiquidityFee.add(_burnFee).add(_bonkFundFee) <= _maxFees, "Fees are too high.");
    ...
    function setBonkFundFeePercent(uint256 newBonkFundFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFee.add(_burnFee).add(newBonkFundFee) <= _maxFees, "Fees are too high.");
        _bonkFundFee = newBonkFundFee;
        _totalFees = _liquidityFee.add(_bonkFundFee).add(_burnFee);
        _totalFeesToContract = _liquidityFee.add(_bonkFundFee);
        emit SetBonkFundFee(_bonkFundFee);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFee.add(newBurnFee).add(_bonkFundFee) <= _maxFees, "Fees are too high.");
        _burnFee = newBurnFee;
        _totalFees = _liquidityFee.add(_bonkFundFee).add(_burnFee);
    ...
    function setBonkFundWalletToken(address _bonkFundWalletToken) external onlyOwner(){
        bonkFundWalletToken = _bonkFundWalletToken;
    }
    ...
    function bonkFundFee() public view returns (uint256) {
        return _bonkFundFee;
    }
    ...
    function _transfer(
        ...
        if(
            ...
            _totalFeesToContract > 0 &&                         // LiquidityFee + BonkFundFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 bonkFundTokensToSell = balanceOf(address(this)).mul(_bonkFundFee).div(_totalFeesToContract);
        
        // Get collected Liquidity Fees 
        swapAndLiquify(liquidityTokensToSell);  

        // Get collected BonkFund Fees 
        swapAndSendToFee(bonkFundTokensToSell); 
    }
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        swapTokensForBonkFundToken(tokens);

        // Transfer sold Token to bonkFundWallet
        IERC20(bonkFundWalletToken).transfer(bonkFundAddress, IERC20(bonkFundWalletToken).balanceOf(address(this)));
    }
    ...
    function swapTokensForBonkFundToken(uint256 tokenAmount) private {

        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = bonkFundWalletToken;
