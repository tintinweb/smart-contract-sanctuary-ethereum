...
contract M3rgeMutants is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("M3", "M3rge Mutants",10,10000)
    {
    ...
    {
        require(MintStartEnabled , "M3 Info:  Minting Public Pause");
        require(qty <= maxMintPerTx, "M3 Info:  Limit Per Transaction");
        require(totalSupply() + qty <= maxSupply,"M3 Info:  Soldout");
        _safemint(qty);
    }
    ...
    function _safemint(uint256 qty) internal
    {
        if(WalletMint[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (qty - freeMint) * MintPrice,"M3 Info:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
        else
        {
           require(msg.value >= qty * MintPrice,"M3 Info:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
    }
