...
// File: contracts/customcontract/batoruNFT.sol
...
contract BatoruNFT is Ownable, ERC721A, ReentrancyGuard {
    ...
    // team mint
    uint256 public teamMintLimit = 20;
    uint256 public numberTeamMint;

    function teamMint(uint256 quantity) external onlyOwner nonReentrant {
        require(
            totalSupply() + 1 <= collectionSize,
            "Cannot mint more than maximum supply"
        );
        require(teamMintLimit >= numberTeamMint, "Team mint already limit");
        numberTeamMint++;
        _safeMint(msg.sender, quantity);
    }

    function teamMintRemaining() external onlyOwner nonReentrant {
    ...
    function presaleMint(bytes32[] calldata _merkleProof)
        ...
        nonReentrant
    {
        require(isPresaleActive, "Presale has not active yet");
        require(
            totalSupply() + 1 <= collectionSize,
            "Cannot mint more than maximum supply"
        );
        require(msg.value >= presalePrice, "Need to send more ETH");
        require(
            !presaleWhitelistClaimed[msg.sender],
            "This address already claim"
        );

        require(merkleRoot[0] != 0, "Cannot mint with empty root");
        ...
        nonReentrant
    {
        ...
        require(isSaleActive, "Public sale has not active yet");
        require(
    ...
    function setWithdrawAddress(address owner) external onlyOwner {
    ...
    function numberPresaleMinted(address owner) public view returns (uint256) {
