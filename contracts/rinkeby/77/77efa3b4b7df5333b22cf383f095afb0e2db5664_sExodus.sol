...
contract sExodus is IsEXO, ERC20Permit {
    ...
    function setgEXO(address _gEXO) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
    ...
    /**
        @notice increases rEXO supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sEXO of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sEXO
    ...
    // toG converts an sEXO balance to gEXO terms. gEXO is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gEXO.balanceTo(amount);
    }

    // fromG converts a gEXO balance to sEXO terms. sEXO is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gEXO.balanceFrom(amount);
    }

    // Staking contract holds excess sEXO
