...
import "./InvadersInterface.sol";
import "./InvadersData.sol";

/// @author nftboi and tr666
/// @title Invaders! P2E Game
contract InvadersGameplay is ReentrancyGuard {
  InvadersData data;

  constructor(address dataAddress) {
    data = InvadersData(dataAddress);
  }
  ...
  function pirate(
    ...
    data.pirate(shipId, planetId, msg.sender, slippageTolerance, quotedPrice);
  }

  function travel(uint256 shipId, uint256 planetId) public {
    data.travel(shipId, planetId, msg.sender);
  }

  function claim(uint256 shipId, bool risky) public {
    data.claim(shipId, msg.sender, risky);
  }

  function beginMining(uint256 shipId) public {
    data.beginMining(shipId, msg.sender);
  }

  function removePlanet(uint256 planetId) public {
    data.removePlanet(planetId, msg.sender);
  }

  function addPlanet(uint256 planetId) public {
    data.addPlanet(planetId, msg.sender);
  }

  function removeShip(uint256 shipId, bool risky) public {
    data.removeShip(shipId, msg.sender, risky);
  }

  function addShip(uint256 shipId, uint256 planetId) public {
    data.addShip(shipId, planetId, msg.sender);
  }
}
...
// interface InvadersInterface {
//   function stake(uint256 shipId, uint256 planetId) external;

//   function claim(uint256 stakeIdx) external;

//   function unstake(uint256 stakeIdx) external;

//   function travel(uint256 stakeIdx, uint256 planetId) external;

//   function pirate() external;

//   function fortify() external;

//   function fortifyAdvanced() external;

//   function increaseFortificationPosition() external;
// }
  ...
  Pausable
{
  ...
  function addShip(
    ...
    ships[shipId] = Ship({
      ...
      shipId: shipId,
      idx: len - 1,
      planetIdx: _addShipToPlanet(shipId, planetId),
      owner: owner
    });
  ...
  function _addShipToPlanet(uint256 shipId, uint256 planetId)
    internal
    returns (uint256)
  {
    shipsOnPlanet[planetId].push(shipId);
    uint256 planetIdx = shipsOnPlanet[planetId].length - 1;
    return planetIdx;
  }

  mapping(uint256 => uint256) public reentryLocation;

  function _removeShipFromPlanet(uint256 shipId) internal {
    ...
    whenNotPendingClaim(shipId)
  {
    ...
    ships[shipId].rewardDebt = rewardPerShare;
  ...
  function stopMining(
    ...
    whenNotPendingClaim(shipId)
  {
    require(ships[shipId].exists == true, "Ship does not exist");
    uint256 planetId = ships[shipId].planetId;
    require(ships[shipId].owner == sender, "Not ship owner");
    require(ships[shipId].mining == true, "Ship not mining");
    _claim(shipId, sender, risky);
    totalMiningPower -= calculateMiningPower(shipId, planetId);
    ships[shipId].mining = false;
    ships[shipId].rewardDebt = rewardPerShare;
    delete planets[planetId].miningShipId;
  }
  ...
  int256 public DELTA_CALC_NUMERATOR = 249;
  int256 public DELTA_CALC_DENOMINATOR = 250;
  int128 public DELTA_CALC_DIVIDED =
    int128(DELTA_CALC_NUMERATOR).divi(DELTA_CALC_DENOMINATOR);

  function updateDeltaCalc(int128 num, int128 denom) public onlyOwner {
  ...
  function _getRewardsForDelta(uint256 delta) internal view returns (uint256) {
    uint256 balance = (EL69.balanceOf(address(this)) -
      pirateEscrowBalance -
      totalRewards) / 10**18;
  ...
  function _setEpoch() internal returns (uint256) {
    ...
    if (lastSavedEpoch < epoch) {
      ...
      rewardPerShare += totalMiningPower > 0
        ? PRBMathUD60x18.div(totalRewardsAtEpoch, totalMiningPower)
        : 0;
  ...
  function migrate(address to) public onlyOwner {
    EL69.transfer(to, EL69.balanceOf(address(this)));
  }
    ...
    override
  {
    ...
    if (reqIdTypes[requestId] == 1) {
      ...
      if (claimReq.risky) {
        ...
        bool destruct = planetParams.geologicalInstability >
          (randomness % 100_000);
        if (destruct) {
        ...
        uint256 rand = PRBMathUD60x18.div((randomness % 100) + 1, 100);
        awardedAmount = PRBMathUD60x18.mul(claimReq.amount, rand);
        bonuses[claimReq.claimer] += awardedAmount;
      }

      if (awardedAmount > 0) {
        assert(awardedAmount <= totalRewards);
        totalRewards -= awardedAmount;
      }
    ...
    if (reqIdTypes[requestId] == 2) {
      ...
      uint256 x = PRBMathUD60x18.div(
        shipParams.groundWeapons,
        planetParams.groundDefenses
      );

      uint256 a = PRBMathUD60x18.mul(x, 0.24 * 10**18) - (0.01 * 10**18);
      uint256 b = (randomness % 100);
      bool succeeded = a >= PRBMathUD60x18.div(b, 100);
      address currentOwner = planets[pirateReq.planetId].owner;
      if (succeeded) {
        uint256 loss = PRBMathUD60x18.mul(pirateReq.price, 0.05 * 10**18);
        uint256 amount = pirateReq.price - loss;

        // Transfer amount to planet owner
        EL69.transfer(planets[pirateReq.planetId].owner, amount);
        ...
        uint256 loss = PRBMathUD60x18.mul(pirateReq.price, 0.005 * 10**18);
        uint256 amount = pirateReq.price - loss;
        ships[pirateReq.shipId].disabledUntil = block.timestamp + 1 days;
        EL69.transfer(pirateReq.pirate, amount);
      }

      pirateEscrowBalance -= pirateReq.price;
      ships[pirateReq.shipId].piratePending = false;
      emit Pirated(
        pirateReq.pirate,
        currentOwner,
        pirateReq.planetId,
  ...
  modifier whenNotPendingClaim(uint256 shipId) {
    require(
      !planets[ships[shipId].planetId].pendingClaim,
      "Pending claim. Please wait until claim completes"
    );
    _;
  }
  ...
  function _claim(
    ...
    address sender,
    bool risky
  ) internal whenNotPendingClaim(shipId) {
    require(ships[shipId].exists, "Ship does not exist");
    require(ships[shipId].owner == sender, "Sender not owner");
    uint256 rewardsForRange = calcRewards(shipId);
    if (rewardsForRange == 0) return;
  ...
  function calcRewards(uint256 shipId) public view returns (uint256) {
    uint256 currentEpoch = getCurrentEpoch();
    Ship memory ship = ships[shipId];
    require(ship.exists, "Ship does not exist");
    require(ship.mining, "Ship is not mining");
    ...
    if (currentEpoch > lastSavedEpoch) {
      ...
      return
        PRBMathUD60x18.mul(
          projectedRewardPerShare - ship.rewardDebt,
          miningPower
        );
    } else {
      return PRBMathUD60x18.mul(rewardPerShare - ship.rewardDebt, miningPower);
    }
  }
    ...
    returns (int256)
  {
    ...
    int256 x = int256(planetA.x) - int256(planetB.x);
    int256 y = int256(planetA.y) - int256(planetB.y);
    return sqrt((x * x) + (y * y));
  }
  ...
  function _chargeTravelCost(
    ...
    int256 distance = getDistance(from, to);
    uint256 price = uint256(distance) * PRICE_PER_DISTANCE;
  ...
  function travel(
    ...
    uint256 planetId,
    address sender
  ) public nonReentrant onlyGameContract whenNotPendingClaim(shipId) {
    Ship memory ship = ships[shipId];
    require(ship.exists == true, "Invalid ship ID");
    require(ship.owner == sender, "Sender not owner");
    require(ship.mining == false, "Stop mining to travel");
    ...
    _removeShipFromPlanet(ship.planetId);
    ships[shipId].planetId = planetId;
    ships[shipId].planetIdx = _addShipToPlanet(shipId, planetId);
  }
  ...
  function pirate(
    ...
    _checkShipDisabled(shipId);
    require(ship.owner == sender, "Ship not owned");
    require(ship.planetId == planetId, "Ship must be on planet");
    require(planets[planetId].owner != sender, "Planet Already owned");
    require(planets[planetId].exists, "Planet does not exist");
    // TODO: check this on planet contract
    require(FF.planetIdToForceField(planetId) == 0, "Planet has force field");
    uint256 priceToSteal = getStealPrice(shipId, planetId);
    uint256 a = priceToSteal > quotedPrice ? priceToSteal : quotedPrice;
    uint256 b = priceToSteal > quotedPrice ? quotedPrice : priceToSteal;
    assert(a > b);
    uint256 delta = a - b;
    require(
      slippageTolerance <= PRBMathUD60x18.div(delta, priceToSteal),
      "Price changed"
    );
