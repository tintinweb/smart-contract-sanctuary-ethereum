...
contract ERC721xyz is Context, ERC165, IERC721, IERC721Metadata {
    ...
    mapping(uint256 => uint256) public _uriMappings;
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        ...
        require( !_burnedTokens[tokenId], "ERC721xyz: burnt token" );
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(_uriMappings[tokenId] == 0)
        {
            while (true) {
                        
                        uint256 uriReturn = _uriMappings[counter];
                        if (uriReturn != 0) {
                            return _uri[uriReturn];
                        }
                        unchecked{
                            ++counter;
                        }            
            ...
            uint256 uriReturn = _uriMappings[tokenId];
            return _uri[uriReturn];
        }
