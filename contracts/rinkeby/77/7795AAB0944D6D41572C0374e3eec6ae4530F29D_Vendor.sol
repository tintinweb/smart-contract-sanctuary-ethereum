...
contract Vendor is Ownable {
  ...
  function buyTokens() public payable{
    uint256 toSend = msg.value * tokensPerEth;
    yourToken.transfer(msg.sender, toSend);
    emit BuyTokens(msg.sender, msg.value, toSend);
  }

  // ToDo: create a withdraw() function that lets the owner withdraw ETH
  function withdraw() public {
    require(msg.sender == address(this.owner()), "You are not the owner");
  ...
  function sellTokens(uint256 amount) public {
    ...
    (bool sent, bytes memory data) = payable(msg.sender).call{value: amount/tokensPerEth}("");
    require(sent, "Failed to send ether");
