...
contract WAYCFactoryExtendable is WAYCFactory {
    ...
    * @dev Emitted when `owner` enables `approved` to manage the `tokenId` token.
    ...
    function addOption(string memory optionMetadata, string memory _name, string memory _symbol, string memory _baseTokenURI, uint256 _maxSupply) public onlyOwner {
        ...
        newNFT.supply = _maxSupply;
        rollingSupply = rollingSupply + _maxSupply;
...
abstract contract WAYCFactory is FactoryERC721, Ownable {
    ...
    uint256 internal rollingSupply;
    ...
    function totalSupply() public view returns(uint256){
        return rollingSupply;
    }

    function balanceOf(address owner) external view returns (uint256 balance){
        return _msgSender() == owner ? rollingSupply : 0;
    }
