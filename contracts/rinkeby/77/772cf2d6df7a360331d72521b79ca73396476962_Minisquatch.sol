...
contract Minisquatch is ERC721Enumerable, Ownable {
  ...
  address public rewardPool = 0xddac44cf8959EA0F1d8BdceB2d278b40b6ba31B9;
  ...
  function canMint(uint _mintAmount) public view returns(bool){
    uint256 supply = totalSupply();
    return hasParents(msg.sender) && !paused && _mintAmount > 0 && _mintAmount <= maxMintAmount && supply + _mintAmount <= maxSupply ? true : false;
  }
  ...
  function hasParents(address sender) public view returns(bool){
      ...
      // require(ILand(landAddress).balanceOf(sender) > landNeeded, "You don't have enough land");
      // require(IMetasquatch(metasquatchAddress).balanceOf(sender) > squatchNeeded, "You don't have enough Metasquatches");
