...
UUPSUpgradeable
{
    ...
    function awardAdminItem(uint256[] memory tokenIds,uint16[] memory locationIds) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        require(tokenIds.length == locationIds.length,"require length error");
        uint256[] memory parts = new uint256[](0);
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            Product memory newItem = Product(locationIds[i], parts);
            _safeMint(_msgSender(), tokenIds[i]);
            _tokenIdDetailMapping[tokenIds[i]] = newItem;
    ...
    function setRivermen(address rivermenAddress) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "require admin permission");
        rivermen = IRiverMen(rivermenAddress);
    }
