 ...
 */
contract ExchangeCircuitBreaker is
    Owned,
    MixinSystemSettings,
    IExchangeCircuitBreaker
{
    ...
    function rateWithInvalid(bytes32 currencyKey)
        external
        view
        returns (uint256, bool)
    {
        (uint256 rate, bool invalid) = _exchangeRates().rateAndInvalid(
            currencyKey
        );
     ...
     */
    function rateWithBreakCircuit(bytes32 currencyKey)
        external
        returns (uint256 lastValidRate, bool circuitBroken)
    {
        // check system status
        if (systemStatus().systemSuspended()) {
            ...
            if (invalid || _isRateOutOfBounds(currencyKey, rate)) {
                ...
                require(
                    issuer().synths(currencyKey) != ISynth(0),
                    "No such synth"
                );
                systemStatus().suspendSynth(
                    currencyKey,
     ...
     */
    function resetLastExchangeRate(bytes32[] calldata currencyKeys)
        external
        onlyOwner
    {
        (uint256[] memory rates, bool anyRateInvalid) = _exchangeRates()
        ...
        for (uint256 i = 0; i < currencyKeys.length; i++) {
            emit LastRateOverriden(
                currencyKeys[i],
                _lastExchangeRate[currencyKeys[i]],
                rates[i]
            );
    ...
    function _isDeviationAboveThreshold(uint256 base, uint256 comparison)
        internal
        view
        returns (bool)
    {
     ...
     */
    function _isRateOutOfBounds(bytes32 currencyKey, uint256 currentRate)
        internal
        view
        returns (bool)
    {
        ...
        if (lastRateFromExchange > 0) {
            return
                _isDeviationAboveThreshold(lastRateFromExchange, currentRate);
        }

        // if no last exchange for this synth, then we need to look up last 3 rates (+1 for current rate)
        (uint256[] memory rates, ) = _exchangeRates()
        .ratesAndUpdatedTimeForCurrencyLastNRounds(currencyKey, 4, 0);
        ...
        for (uint256 i = 1; i < rates.length; i++) {
            ...
            if (
                rates[i] > 0 &&
                _isDeviationAboveThreshold(rates[i], currentRate)
            ) {
                return true;
            }
        }
