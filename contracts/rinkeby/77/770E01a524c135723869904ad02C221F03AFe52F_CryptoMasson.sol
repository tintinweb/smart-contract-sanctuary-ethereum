...
contract CryptoMasson is ERC721A, Ownable {
    ...
    function checkReferals(address user, uint256 startPoint)
        external
        view
        returns (refs[8] memory referalInfo)
    {
        refs[8] memory adrArray;
        uint256 amount = 8;
        ...
        if (adrToRefsInfo[user].length < startPoint + amount) {
            for (uint256 j; j < adrToRefsInfo[user].length; j++) {
                adrArray[count] = adrToRefsInfo[user][j];
                count++;
            }
        } else {
            for (uint256 i = startPoint; i < startPoint + amount; i++) {
                //address ref = adrToRefsInfo[user][i];
                if (i + 1 > adrToRefsInfo[user].length) {
                    return adrArray;
                }
