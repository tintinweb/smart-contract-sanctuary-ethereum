...
contract MetacommerceERC721A is Initializable, ERC721AUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        require(
            _uintData.length == 5,
            "Missing uint data (priceInWei, priceWhitelist, maxSupply, maxPerWallet, maxWhitelist)"
        );
        require(_boolData.length == 2, "Missing bool data (revealed, whitelistActive)");
        ...
        priceInWei = _uintData[0];
        priceWhitelist = _uintData[1];
        MAX_SUPPLY = _uintData[2];
        MAX_PER_WALLET = _uintData[3];
        MAX_WHITELIST = _uintData[4];
        isRevealed = _boolData[0];
        isWhitelistActive = _boolData[1];
        isPaused = true;
        MAX_PER_TX = 10;
    }
    ...
    function setWhitelistActive(bool _whitelistActive) external onlyOwner {
        require(!isFrozen, "Contract is frozen.");
        isWhitelistActive = _whitelistActive;
    }

    /**
     * Pauses users from minting from contract
     */
    ...
    function whitelistMint(
        ...
        require(isWhitelistActive, "Whitelist sale is over.");
        require(this.totalSupply() + amount <= MAX_SUPPLY, "Exceeds max supply.");
    ...
    function mint(uint256 amount) public payable callerIsUser {
        require(!isFrozen, "Contract is frozen.");
        require(!isPaused, "Contract is paused.");
        require(!isWhitelistActive, "Whitelist sale is not over yet.");
        require(this.totalSupply() + amount <= MAX_SUPPLY, "Exceeds max supply.");
    ...
    function _collectRoyalties() internal {
        (uint256 _protocolFee, address _protocolFeeRecipient) = cloneFactory.getProtocolFeeAndRecipient(address(this));
