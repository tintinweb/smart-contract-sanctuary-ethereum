...
contract Creature is ICreature, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    mapping(uint256 => Creature) private __creatures;

    /// @notice `Generator` contract address
    address public generatorAddress;
    /// @notice `ProtectionProgram` contract address
    address public protectionProgramAddress;
     ...
     */
    function setProtectionProgramAddress(address _protectionProgramAddress) external override onlyOwner {
        protectionProgramAddress = _protectionProgramAddress;
    }
    ...
    function addCreaturesInfo(
        ...
        for (uint256 _i; _i < _ids.length; _i++) {
            __addCreatureInfo(_ids[_i], _gens[_i], _tenures[_i]);
        }
    }
    ...
    function addCreaturesInfoAndStake(
        ...
        address _protectionProgramAddress = protectionProgramAddress;
        require(_protectionProgramAddress != address(0), "Creature: set PP address");

        for (uint256 _i; _i < _ids.length; _i++) {
            __addCreatureInfo(_ids[_i], _gens[_i], _tenures[_i]);
        }

        IProtectionProgram(_protectionProgramAddress).add(_ids);
    }
     ...
     * @param _id NFT ID
     */
    function getCreatureInfo(uint256 _id) external view override returns (uint256, uint256, CreatureType) {
        return (__creatures[_id].gen, __creatures[_id].tenure, __creatures[_id].creatureType);
    }

    function __addCreatureInfo(uint256 _id, uint8 _gen, uint8 _tenure) private {
        CreatureType _type;
        ...
        if (_tenure == 0) {
            ...
            _type = CreatureType.Humal;
            totalHumals++;
        }

        __creatures[_id] = Creature(_gen, _tenure, _type);
    }
