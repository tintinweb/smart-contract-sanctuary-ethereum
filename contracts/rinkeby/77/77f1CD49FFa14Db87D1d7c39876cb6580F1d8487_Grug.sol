...
contract Grug is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function oGMint(uint8 numberOfTokens, bytes32[] calldata merkleProof)
        external
        payable
        nonReentrant
        oGSaleActive
    ...
    {
        if (block.timestamp < startTime + OG_MINT_TIME) {
            require(
                MerkleProof.verify(
                    merkleProof,
                    oGMerkleRoot,
    ...
    function setIsPublicSaleActive(bool _value) external onlyOwner {
        isPublicSaleActive = _value;
    }
