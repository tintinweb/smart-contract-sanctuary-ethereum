...
contract BurgerRedemption is Ownable {
	...
	IWaitGame private waitGame;
	IERC1155 private erc1155;

	//amount of time that needs to pass for the burger to be served
	uint256 public timeToServe = 2592000;

	constructor() {
        setWaitGame(0xcee57d06369bBD76547A22603494e4983A9de50C);    
	}
    ...
    function setWaitGame(address newContract) public onlyOwner {
    	require(newContract != address(0), "Invalid Address!");
    	waitGame = IWaitGame(newContract);
    ...
    function fundsForBurger(uint burgerNumber) external {
    	require(erc1155.balanceOf(msg.sender, 1) > 0, "Not Burgers to Redeem!");
    	require(burgerIsReady(burgerNumber), "Burger not ready to be Redeemed!");
    	IWaitGame.Burger memory burger = waitGame.burgers(burgerNumber);
    	require(msg.sender == burger.buyer, "Not the Owner of this Burger!");

    	waitGame.redeemBurger(msg.sender);
    	_withdraw(payable(msg.sender), waitGame.rewardAmount());
    }
