...
contract Spin is VRFConsumerBaseV2 {
    ...
    function Reset() public {
        s_requestIdToRandomWords[s_requestAddressforId[msg.sender]][0] = 0;
        s_requestAddressforId[msg.sender] = 0;
    }

    function ReadThewords() public returns (uint256[10] memory) {
        uint256[10] memory thereturn;

        uint256 cpt = 0;
        while (
            cpt <
            s_requestIdToRandomWords[s_requestAddressforId[msg.sender]].length
        ) {
            thereturn[cpt] = s_requestIdToRandomWords[
