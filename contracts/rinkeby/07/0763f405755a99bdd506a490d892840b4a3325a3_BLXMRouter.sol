...
contract BLXMRouter is Initializable, BLXMRewardProvider, IBLXMRouter {
    ...
    function addRewards(address token, uint totalBlxmAmount, uint16 supplyDays) external onlyOwner override returns (uint amountPerHours) {
...
contract BLXMRewardProvider is ReentrancyGuardUpgradeable, BLXMMultiOwnable, BLXMTreasuryManager, IBLXMRewardProvider {
    ...
    struct Field {
        ...
        // days => session
        mapping(uint32 => uint16) daysToSession; // @Deprecated
    ...
    function updateRewardFactor(uint16 lockedDays, uint factor) public override onlyOwner returns (bool) {
        ...
        uint oldFactor = rewardFactor[lockedDays].add(10 ** decimals());
        rewardFactor[lockedDays] = factor.sub(10 ** decimals());
    ...
    function _addRewards(address token, uint totalAmount, uint16 supplyDays) internal nonReentrant returns (uint amountPerHours) {
    ...
    function syncStatistics(address token) external override {
        getTreasury(token);
        _syncStatistics(token);
    }
    ...
    function hoursToSession(address token, uint32 hourFromEpoch) public override view returns (uint16 session, uint amountPerHours) {
        Field storage field = treasuryFields[token];
        uint32 initialHour = field.initialHour;
        session = field.lastSession;
        if (session == 0 || hourFromEpoch < initialHour) {
            return (0, 0);
        }

        uint currentHour = hourFromEpoch - field.initialHour;

        while (session != 0) {
            Period storage period = field.periods[session];
            if (currentHour >= period.startHour && currentHour < period.endHour) {
                return (session, period.amountPerHours);
            }
            session--;
        }
    }
    ...
    function _arrangeFailedRewards(address token, uint rewardAmount) internal {
        ...
        (uint16 session,) =  hoursToSession(token, startHour);
        if (session == 0) {
    ...
    function _updateStatistics(address token, uint32 fromHour, uint32 toHour) internal {
        ...
        while (fromHour < toHour) {
            ...
            (,uint rewards) = hoursToSession(token, fromHour);
...
interface IBLXMRewardProvider {
    ...
    function hoursToSession(address token, uint32 hourFromEpoch) external view returns (uint16 session, uint amountPerHours);
...
contract BLXMTreasuryManager is BLXMMultiOwnable, IBLXMTreasuryManager {
    ...
    function updateRatioAdmin(address _ratioAdmin) external override onlyOwner {
        ...
        address oldRatioAdmin = ratioAdmin;
        ratioAdmin = _ratioAdmin;

        emit UpdateRatioAdmin(msg.sender, oldRatioAdmin, _ratioAdmin);
    }
    ...
    function _withdraw(address token, uint rewards, uint liquidity, address to) internal returns (uint amountBlxm, uint amountToken) {
        ...
        // and multiply more decimals to reduce the effects of the square root
        amountToken = liquidity.mul(10 ** 18) / Math.sqrt(_ratio * 10 ** 18);
...
interface IBLXMTreasuryManager {
    ...
    event UpdateRatioAdmin(address indexed sender, address oldRatioAdmin, address newRatioAdmin);
