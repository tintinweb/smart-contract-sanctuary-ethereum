...
contract DGN is ERC1155, Ownable {
    ...
    MintInformation private mintInformation;
    ...
    function mint(uint _id) public {
        ...
        require(_id == mintInfo[_id].id, "Mint must be active for specified ID");
        ...
        if (mintInfo[_id].status == MintStatus.Whitelist) {
            ...
            require(mintInfo[_id].status == MintStatus.Public, "Public mint must be active or you are not whitelisted.");
            require(addressData[msg.sender][_id] < tokenInfo[_id].publicMintLimit, "Public mint limit reached");
    ...
    Can only set supply of a token ONCE*/
    function _setTokenSupply(uint _id, uint _maxSupply) external onlyOwner {
        //IF you want to only set token ONCE: 
        //require(tokenInfo[_id].maxSupply == 0, "Cannot set a token that has already been set.");
    ...
    function _setPublicMintLimit(uint _id, uint _publicMintLimit) external onlyOwner {
        ...
        tokenInfo[_id].publicMintLimit = _publicMintLimit;
    }   
    ...
    function getTokenPublicMintLimit (uint _id) public view returns (uint) {
        return tokenInfo[_id].publicMintLimit;
    }
