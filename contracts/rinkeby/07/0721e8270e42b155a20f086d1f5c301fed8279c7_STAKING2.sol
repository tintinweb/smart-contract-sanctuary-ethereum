...
contract STAKING2 is Ownable {
    ...
    uint128 private _lockTimePeriod = 100;
    // coolTime period default 1 week
    // 60 * 60 * 24 * 7 =604800 sec
    uint128 private coolingTime = 60;
    ...
    function AmountUstaked(address addr) external view returns (uint256) {
        return AvailableForClaim[addr];
    }

    // work after coolperiod is over
    // user will claim amount after claim period is over
    ...
    function Claim() external {
        ...
        uint256 claim = AvailableForClaim[msg.sender];
        ...
        if (_stakeDetail[msg.sender].totalStaked == 0) {
            ...
            GETS(stakingToken).transferPrice(primaryWallet, msg.sender, claim);
        }
    ...
    //only owner can
    function setPrimaryWallet(address walletAddress) external onlyOwner {
        require(
            walletAddress != address(0x0),
            " Error: Can't be a zero Address"
        );
    ...
    function UserTotallockAmount(address addr) external view returns (uint256) {
        return _stakeDetail[addr].totalStaked;
    }

    //set cooling time only by owner
    function setCoolingTime(uint128 CoolingTime) external onlyOwner {
        coolingTime = CoolingTime;
    }

    //additional time after unlockTime token
    function getCoolingTime() external view returns (uint256) {
        return coolingTime;
    }

    //set cooling time only by owner
    function setlockTimePeriod(uint128 lockTimePeriod) external onlyOwner {
    ...
    function stakeTimeAmount(address addr)
        external
        view
    ...
    function unlockTime_(address addr) external view returns (uint256) {
        return unlockTime[addr];
    }

    // cooling time of totalamount staked by user for claim
    function coolTime_(address addr) external view returns (uint256) {
        return _coolingTime[addr];
    }

    // total unstaked amount by user
    function userTotalClaimed(address addr) external view returns (uint256) {
