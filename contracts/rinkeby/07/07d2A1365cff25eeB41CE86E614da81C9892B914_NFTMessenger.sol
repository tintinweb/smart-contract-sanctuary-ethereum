...
// import "../lz/interfaces/ILayerZeroUserApplicationConfig.sol";
// import "../lz/lzApp/LzApp.sol";

// contract NFTMessenger is LzApp {
...
//         address _dstAddress // destination address of PingPong contract
//     ) public payable returns(string memory) {
//         bytes memory payload = abi.encode("mintNFT", "hello", "world");

//         bytes memory remoteAndLocalAddresses = abi.encodePacked(_dstAddress, address(this));

//         endpoint.send{value: msg.value}(_dstChainId, remoteAndLocalAddresses, payload, payable(msg.sender), address(0x0), bytes(""));
...
//     function lzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint _nonce, bytes memory _payload) external {
...
contract NFTMessenger is NonblockingLzApp, Pausable {
    ...
    function MintNFTOnMsg(
        ...
        bytes memory payload = abi.encode("mintNFT");
