...
contract FpxMetaverse is ERC721A,Ownable,Pausable {
    ...
    function refundIfOver(uint256 price,uint8 minttype) private {
        uint256  needmsgvalue = publicSalePrice ;
        if (minttype==2) {
            needmsgvalue = allowlistSalePrice;
        }
        if (msg.value > price) {
