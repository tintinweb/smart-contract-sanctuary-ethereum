...
contract BreederStakingView {
    ...
    BreederLiquidityMiningManager
        public immutable breederLiquidityMiningManager;
    BreederTimeLockPool public immutable escrowPool;

    constructor(address _breederLiquidityMiningManager, address _escrowPool) {
        breederLiquidityMiningManager = BreederLiquidityMiningManager(
            _breederLiquidityMiningManager
        );
        escrowPool = BreederTimeLockPool(_escrowPool);
    }
    ...
    {
        uint256 rewardPerSecond = breederLiquidityMiningManager
            .rewardPerSecond();
        uint256 lastDistribution = breederLiquidityMiningManager
            .lastDistribution();
        uint256 pendingRewards = rewardPerSecond *
            (block.timestamp - lastDistribution);

        result.totalWeight = breederLiquidityMiningManager.totalWeight();

        BreederLiquidityMiningManager.Pool[]
            memory pools = breederLiquidityMiningManager.getPools();

        result.pools = new Pool[](pools.length);

        for (uint256 i = 0; i < pools.length; i++) {
            BreederTimeLockPool poolContract = BreederTimeLockPool(
            ...
            BreederTimeLockPool.Deposit[] memory deposits = poolContract
                .getDepositsOf(_account);

            for (uint256 j = 0; j < result.pools[i].deposits.length; j++) {
                BreederTimeLockPool.Deposit memory deposit = deposits[j];
                result.pools[i].deposits[j] = Deposit({
                    amount: deposit.amount,
                    start: deposit.start,
                    end: deposit.end,
                    multiplier: poolContract.getMultiplier(
        ...
        BreederTimeLockPool.Deposit[] memory deposits = escrowPool
            .getDepositsOf(_account);
        ...
        for (uint256 j = 0; j < result.escrowPool.deposits.length; j++) {
            ...
            result.escrowPool.deposits[j] = Deposit({
                ...
                multiplier: escrowPool.getMultiplier(
                    deposit.end - deposit.start
                )
            });
        }
...
contract BreederLiquidityMiningManager is TokenSaver {
    ...
    modifier onlyGov() {
        require(
            ...
            "BreederLiquidityMiningManager.onlyGov: permission denied"
        );
        _;
    }
    ...
    modifier onlyRewardDistributor() {
        require(
            ...
            "BreederLiquidityMiningManager.onlyRewardDistributor: permission denied"
        );
        _;
    }
    ...
    constructor(address _reward, address _rewardSource) {
        require(
            _reward != address(0),
            "BreederLiquidityMiningManager.constructor: reward token must be set"
        );
        require(
            _rewardSource != address(0),
            "BreederLiquidityMiningManager.constructor: rewardSource token must be set"
        );
    ...
    function addPool(address _poolContract, uint256 _weight) external onlyGov {
        ...
        require(
            ...
            "BreederLiquidityMiningManager.addPool: pool contract must be set"
        );
        require(
            !poolAdded[_poolContract],
            "BreederLiquidityMiningManager.addPool: Pool already added"
        );
        require(
            pools.length < MAX_POOL_COUNT,
            "BreederLiquidityMiningManager.addPool: Max amount of pools reached"
        );
        // add pool
        pools.push(
    ...
    function removePool(uint256 _poolId) external onlyGov {
        require(
            ...
            "BreederLiquidityMiningManager.removePool: Pool does not exist"
        );
        distributeRewards();
        ...
        onlyGov
    {
        require(
            _poolId < pools.length,
            "BreederLiquidityMiningManager.adjustWeight: Pool does not exist"
        );
        distributeRewards();
    ...
    function distributeRewards() public onlyRewardDistributor {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            uint256 poolRewardAmount = (totalRewardAmount * pool.weight) /
                totalWeight;
...
abstract contract AbstractRewards is IAbstractRewards {
     ...
     */
    function withdrawableRewardsOf(address _account)
        public
        view
        override
        returns (uint256)
     ...
     */
    function withdrawnRewardsOf(address _account)
        external
        view
        override
        returns (uint256)
     ...
     */
    function cumulativeRewardsOf(address _account)
        public
        view
        override
        returns (uint256)
