...
contract Gashapondo is ERC721, ERC721Burnable, Ownable, IGashapondo {
    ...
    modifier onlyAuthorOrAdmin(uint256 collectionId) {
        require(
            collections[collectionId].authorAddress == _msgSender() ||
                admins[_msgSender()],
        ...
        returns (uint256 tokenId)
    {
        ...
        if (
            numberOfTokensAfterMinted == collections[collectionId].totalTokens
        ) {
        ...
        returns (uint256[] memory tokenIds)
    {
        ...
        require(
            numberOfTokens > 0,
            "Number of tokens should be greater than 0"
        );
        require(
            collections[collectionId].minted + numberOfTokens <=
                collections[collectionId].totalTokens,
        ...
        // EFFECTS
        uint24 numberOfTokensAfterMinted = collections[collectionId].minted +
            numberOfTokens;
        if (
            numberOfTokensAfterMinted == collections[collectionId].totalTokens
        ) {
    ...
    {
        require(
            erc20Tokens.length == supported.length,
            "Array lengths don't match"
        );
        require(
            erc20Tokens.length == pricesInWei.length,
            "Array lengths don't match"
        );
        ...
        for (uint256 i = 0; i < erc20Tokens.length; i++) {
            ...
            collectionMintPrices[collectionId].acceptedErc20Tokens[
                token
            ] = supported[i];
            collectionMintPrices[collectionId].erc20TokenPricesInWei[
                    token
                ] = pricesInWei[i];
        }
    }
    ...
    {
        require(
            payees.length == percentages.length,
            "Array lengths don't match"
        );

        uint256 index = collectionPayments[collectionId]
            .numberOfAddtionalPayees;
        for (uint256 i = 0; i < payees.length; i++) {
            index += 1;
            collectionPayments[collectionId].additionalPayees[index] = payees[
                i
            ];
            collectionPayments[collectionId].additionalPayeePercentages[
                    index
                ] = percentages[i];
        }
    ...
    function removeLastPayees(uint256 collectionId, uint256 numberOfLastPayees)
        ...
        whenNotInState(CollectionState.FROZEN, collectionId)
    {
        uint256 numberOfAddtionalPayees = collectionPayments[collectionId]
            .numberOfAddtionalPayees;
        require(
            numberOfLastPayees <= numberOfAddtionalPayees,
            "Exceeds current number of addtional payees"
        );

        for (
            uint256 i = numberOfAddtionalPayees;
            i > numberOfAddtionalPayees - numberOfLastPayees;
            i--
        ) {
            delete collectionPayments[collectionId].additionalPayees[i];
            delete collectionPayments[collectionId].additionalPayeePercentages[
                i
            ];
        }
        collectionPayments[collectionId]
            .numberOfAddtionalPayees -= numberOfLastPayees;
    }
    ...
    function setCollectionMaxPurchasePerTx(
        uint256 collectionId,
        uint24 maxPurchasePerTx
    ) external onlyAdmin whenNotInState(CollectionState.FROZEN, collectionId) {
        ...
        onlyAuthor(collectionId)
    {
        CollectionState currentState = collections[collectionId].state;
        require(
            currentState == CollectionState.ACTIVE ||
                currentState == CollectionState.PAUSED,
    ...
    function getCollectionErc20TokenPrice(
        uint256 collectionId,
        address erc20Token
    ) external view override returns (uint256 priceInWei) {
        priceInWei = collectionMintPrices[collectionId].erc20TokenPricesInWei[
            erc20Token
        ];
    }
    ...
    function isErc20TokenPaymentSupported(
        ...
        supported = collectionMintPrices[collectionId].acceptedErc20Tokens[
            erc20Token
        ];
    }
    ...
    {
        numberOfAddtionalPayees = collectionPayments[collectionId]
            .numberOfAddtionalPayees;
    }
    ...
    {
        payees = new PayeeInfo[](
            collectionPayments[collectionId].numberOfAddtionalPayees
        );
        for (uint256 i = 0; i < payees.length; i++) {
            payees[i] = PayeeInfo({
                addr: collectionPayments[collectionId].additionalPayees[i],
                percentages: collectionPayments[collectionId]
                    .additionalPayeePercentages[i]
            });
        }
    }
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(collection.baseUri, Strings.toString(tokenId))
            );
    }
    ...
    function _mintToken(address to, uint256 collectionId)
        internal
        returns (uint256 tokenId)
    {
        // randomization by Thieu
    ...
    function _getCollectionId(uint256 tokenId)
        internal
        pure
        returns (uint256 collectionId)
    {
