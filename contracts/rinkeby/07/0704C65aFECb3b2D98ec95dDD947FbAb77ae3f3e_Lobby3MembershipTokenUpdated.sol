 ...
 *
 */
contract Lobby3MembershipTokenUpdated is
    ERC721A,
    Ownable
{
  ...
  uint256 public immutable goldMaxSupply;

  uint256 private _goldCounter;
  ...
  ) ERC721A("Lobby3 Membership Token", "L3") {
    _uri = baseURI;
  ...
  /**
    ...
    */
  function mintStandardETH(address to, uint256 quantity) external payable {
    require(isSaleActive, "Sale must be active");
    require(
      !Address.isContract(msg.sender),
      "Caller must not be a contract"
    );
    require ((STANDARD_ETH_MINT_PRICE * quantity) <= msg.value, "Eth value is incorrect");
  ...
  /**
    ...
    * @param to Address to mint token to.
    */
  function mintStandardETHBackup(address to, uint256 quantity) external onlyOwner{
    require(isSaleActive, "Sale must be active");
    require(quantity > 0, "Must mint at least 1 token");
  ...
  /**
    ...
    * @param to Address to mint token to.
    */
  function mintGoldETH(address to, uint256 quantity) external payable {
    require(isSaleActive, "Sale must be active");
    require(
      !Address.isContract(msg.sender),
      "Caller must not be a contract"
    );
    require ((GOLD_ETH_MINT_PRICE * quantity) <= msg.value, "Eth value is incorrect");
    require(quantity > 0, "Must mint at least 1 token");
    require(
      _goldCounter + quantity <= goldMaxSupply,
      "Token count exceeds gold limit"
    );

    _safeMint(to, quantity);
    _goldCounter+=quantity;
  }
  ...
  /**
    ...
    * @param to Address to mint token to.
    */
  function mintGoldETHBackup(address to, uint256 quantity) external onlyOwner{
    require(isSaleActive, "Sale must be active");
    require(quantity > 0, "Must mint at least 1 token");
