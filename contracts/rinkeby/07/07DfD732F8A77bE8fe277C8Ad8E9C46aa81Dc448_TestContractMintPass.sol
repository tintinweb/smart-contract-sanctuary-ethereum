...
contract TestContractMintPass is ERC721, Ownable {
    ...
    mapping(uint256 => bool) public expiredTokens;
     ...
     */
    function isFlaggedUsed(uint256 tokenId) external view returns (bool) {
        require(_exists(tokenId), "Nonexistent token");
        return usedTokens[tokenId];
    }

    /**
     * @dev Returns whether a token has expired
     */
    function isFlaggedExpired(uint256 tokenId) external view returns (bool) {
        require(_exists(tokenId), "Nonexistent token");
    ...
    /**
     * @dev Returns the amount of time (sec) a token has before it loses validity
     */
    ...
    /**
     * @dev Returns list of token that are out of time (Can be flagged Expired)
     */
    function outOfTimeTokens() external view returns (uint256[] memory) {
    ...
    /**
     * @dev Flags the token as expired; anyone can call this
     */
