...
contract RouterXYZ is Context, IERC20, Ownable {
    ...
    uint256 public maxWallet = _tTotal.div(3).mul(100) * 10** _decimals;
    ...
    constructor () payable {
        _balances[_msgSender()] = _tTotal.div(2);
        _balances[address(this)] = _tTotal.div(2);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!live){
            _tokenTransfer(from, to, amount, false, false, false);
        } else {
            ...
            //anti bot
            if (block.number <= activeTradingBlock + 1) {
            ...
            if(from == pair || isExchangeWallet[from]) {
                require(isMaxWalletExcluded[to] || holderBalance <= maxWallet, "Wallet cannot exceed max Wallet limit");
            }
    ...
    function goLive() external onlyOwner returns (bool) {
        ...
        _isExcludedFromRewards[address(pair)] = true;
