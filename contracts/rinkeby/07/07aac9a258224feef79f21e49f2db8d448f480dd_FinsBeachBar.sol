...
contract FinsBeachBar is ERC1155, ERC1155Supply, ERC1155Burnable, Ownable {
     ...
     * @dev Airdrop two tokens to each address in the calldata list,
     ...
     * @param _list address[] list of wallets to send 2 token to each.
     */
    function airdrop(uint256 _dropNumber, address[] calldata _list)
        external
        onlyOwner
    {
        // added this as a check to ensure the list length + existing supply don't exceed the max tokens that can be issued
        require(tokensMinted[_dropNumber] + (2 * _list.length) <= MAX_PER_ITEM, "No remaining supply");
        ...
        tokensMinted[_dropNumber] = tokensMinted[_dropNumber] + (2 * _list.length);
    }
    ...
    function burnToRedeem(uint256[] memory ids) public {
        // make sure redemption is active
        ...
        // keep track of which tokens were submitted
        uint256[] memory tokens = new uint256[](requiredToClaim);
        uint256[] memory values = new uint256[](requiredToClaim);

        // loop over each token. First, check to make sure the token is not already included in the list. 
        // once we confirm it's not, add it to the list
        ...
        _burnBatch(msg.sender, tokens, values);
