...
contract Trade is Initializable, OwnableUpgradeable {
    ...
    function initialize(address _buybackAdminAddress, uint256 _serviceValue)
        public
        initializer
    {
        __Ownable_init();
    ...
    bool private adminApproval = true;
    ...
    function calc(uint256 amount, uint256 _serviceValue)
        internal
        pure
    ...
    function _saleToken(
        ...
        if (
            ...
            (uint256 _adminfee, uint256 netamount) = calc(amount, serviceValue);
            require(
                approveValue >= _adminfee.add(netamount).div(10**Tokendecimals),
                "Insufficient Balance"
            );
            t.transferFrom(
                msg.sender,
