...
contract ToolStaking is Ownable, ERC1155Receiver {
    ...
    uint256 public constant RESTING_TIME = 21600;
    ...
    function withDrawTools(uint256[] calldata collectionIds) public {
        ...
        for (uint256 index = 0; index < length; index++) {
            ...
            require(amounts[index] > 0, "Amount must be more than or equal to 1");
    ...
    function calculateRewards(uint256[] memory collectionIds, address account)
        public
        returns (uint256)
    {
        uint256 totalClaimAbleAmount;
        for (uint256 index = 0; index < collectionIds.length; index++) {
            totalClaimAbleAmount += calculateReward(
                collectionIds[index],
                account
            );
        }
