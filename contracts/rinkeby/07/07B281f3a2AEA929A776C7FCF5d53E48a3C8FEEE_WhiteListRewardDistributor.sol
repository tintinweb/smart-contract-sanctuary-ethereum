...
import "../interfaces/IWhiteListRewardDistributor.sol";

contract WhiteListRewardDistributor is IShares, IWhiteListRewardDistributor, Pausable, ReentrancyGuard {
  ...
  uint16 public constant WHITELIST_REWARD_SHARES = 500;
  uint16 public constant TOTAL_SHARES = 10000;
  uint256 public constant BUFFER_ADMIN_WITHDRAW = 1 seconds;
  ...
  event UpdateWhiteListReward(uint256 indexed rewardRound);
  event TokenWithdrawnOwner(uint256 amount);

  modifier onlyAdmin() {
    require(nftspacexAccessControls.hasAdminRole(_msgSender()), "WRD: only admin");
    _;
  }
  ...
  function shares() public pure override returns (uint16) {
    return WHITELIST_REWARD_SHARES;
  }

  function maxBalanceNST() public view override returns (uint256) {
    return (nftspacexToken.maxTotalSupply() * WHITELIST_REWARD_SHARES) / TOTAL_SHARES;
  }
  ...
  function updateWhiteListRewards(bytes32 merkleRoot, uint256 newMaximumAmountPerUser) external onlyAdmin {
    require(!merkleRootUsed[merkleRoot], "WRD: Already used");
    ...
    emit UpdateWhiteListReward(currentRewardRound);
  }

  function pauseDistribution() external onlyAdmin whenNotPaused {
    lastPausedTimestamp = block.timestamp;
    _pause();
  }

  function unpauseDistribution() external onlyAdmin whenPaused {
    _unpause();
  }

  function withdrawTokenRewards(uint256 amount) external onlyAdmin whenPaused {
    require(block.timestamp > (lastPausedTimestamp + BUFFER_ADMIN_WITHDRAW), "WRD: still early");
    nftspacexToken.safeTransfer(_msgSender(), amount);

    emit TokenWithdrawnOwner(amount);
  }

  function claim(uint256 amount, bytes32[] memory merkleProof) external override whenNotPaused nonReentrant {
    require(!hasUserClaimedForRewardRound[currentRewardRound][_msgSender()], "WRD: Already claim");
    (bool claimStatus, uint256 adjustedAmount) = _canClaim(_msgSender(), amount, merkleProof);
    require(claimStatus, "WRD: invalid proof");
    require(maximumAmountPerUserInCurrentTree >= amount, "WRD: excceed max amount");

    hasUserClaimedForRewardRound[currentRewardRound][_msgSender()] = true;
    amountClaimedByUser[_msgSender()] += adjustedAmount;
    nftspacexToken.safeTransfer(_msgSender(), adjustedAmount);

    emit RewardsClaim(_msgSender(), currentRewardRound, adjustedAmount);
  }

  function canClaim(address user, uint256 amount, bytes32[] memory merkleProof) external view override returns (bool, uint256) {
    return _canClaim(user, amount, merkleProof);
  }

  function _canClaim(address user, uint256 amount, bytes32[] memory merkleProof) internal view returns (bool, uint256) {
...
interface IWhiteListRewardDistributor {
  function claim(uint256 amount, bytes32[] memory merkleProof) external;
  function canClaim(address user, uint256 amount, bytes32[] memory merkleProof) external returns (bool, uint256);
}
