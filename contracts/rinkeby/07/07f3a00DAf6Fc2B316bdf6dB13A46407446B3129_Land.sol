...
contract Land is ERC721Enumerable, Ownable, ReentrancyGuard, PaymentSplitter {
    using Strings for uint256;

    string private _baseTokenURI;
    string private _tokenURISuffix;

    uint256 public constant PRICE = 0.2 ether; // 0.1 ETH
    uint256 public constant MAX_SUPPLY = 6002; // real max supply is 3223 - saving gas for the minters

    bytes32 public allowlistMerkleRoot;

    bool public isPublic = false;
    bool public isPresale = false;
    address private yakuzaAddress;

    mapping(address => bool) public addressToMinted;
    mapping(uint256 => bool) public yakuzaMinted;

    constructor(
        address _yakuzaAddress,
        address[] memory _payees,
        uint256[] memory _shares
    ) ERC721("Land", "LND") PaymentSplitter(_payees, _shares) {
        yakuzaAddress = _yakuzaAddress;
    }
    ...
    function yakuzaMint(uint256[] memory tokenIds)
        external
        payable
        nonReentrant
    {
        ...
        require(supply + tokenIds.length < MAX_SUPPLY, "Max supply reached");
        require(PRICE * tokenIds.length <= msg.value, "Invalid funds provided");
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            require(
                IYakuza(yakuzaAddress).ownerOf(tokenIds[i]) == msg.sender,
                "not owner"
            );
            require(!yakuzaMinted[tokenIds[i]], "Already claimed");
            yakuzaMinted[tokenIds[i]] = true;
            _safeMint(msg.sender, supply++);
        }
    }
