...
contract merton {
    ...
    function cdf(int x) public view returns (int y) {
    ...
    function BS_CALL(int S, int K, int T, int r, int sigma) public view returns (int){
        int d1 = D1(S,K,r,sigma,T);
        int d2 = D2(d1,sigma,T);

        int C = (S * cdf(d1)) / 1e18 - K  * (-r*T/1e18).exp() * cdf(d2) / 1e36;
        return C;
    }



    function BS_PUT(int S, int K, int T, int r, int sigma) public view returns (int){
        int d1 = D1(S,K,r,sigma,T);
        int d2 = D2(d1,sigma,T);

        int C = K * (-r*T/1e18).exp() * cdf(-d2) / 1e36 - (S * cdf(-d1) / 1e18);
        return C;
    }


    function D1(int S,int K, int r, int sigma, int T) public pure returns (int){
        int d1 = ((S.div(K)).log2() + (r + (sigma**2/2 / 1e18))*T) / (sigma * T.sqrt() / 1e18);
        return d1;
    }


    function D2(int d1, int sigma, int T) public pure returns (int){
        int d2 = d1 - sigma * T.sqrt() / 1e18;
        return d2;
    }



    function merton_jump_call(  int S, 
                                int K, 
                                int T, 
                                int r, 
                                int sigma, 
                                ...
                                int v, 
                                int lam) 
    public view returns (int p) {
        p = 0;
        for (int k=0; k<5; k++){
            
            int r_k = RK(r,lam,m,k,T);
            
            int sigma_k = SIGMA_K(sigma,k,v,T);

            int k_fact = factorial[k];
            
            p += MJCnum(m,lam,T,k,k_fact) * BS_CALL(S,K,T,r_k,sigma_k) / 1e18;
    ...
    function RK(int r, int lam, int m, int k, int T) public pure returns (int) {
        int r_k = r - lam.mul(m - 1e18) + (k.mul(m.log2()).div(T));
        return r_k;
    }

    function SIGMA_K(int sigma, int k, int v, int T) public pure returns (int) {
        int sigma_k = (sigma**2/1e18 + (k * (v**2/1e18)).div(T)).sqrt();
        return sigma_k;
    }


    function test2(  int S, 
                                int K, 
                                int T, 
                                int r, 
                                int sigma, 
    ...
    // very bad
    function K_FACT(int n) public pure returns (int) {
        // n < 57
        // this needs to be in a lookup table for gas savings
        int range = n + 1;
        int factorial = 1;
    ...
    mapping(int => int) public factorial;

    function createlookup() public {
        for (int i=0;i<41;i++) {
            int fact = K_FACT(i);
            factorial[i] = fact;
        }
    ...
    function MJCnum(int m, int lam, int T, int i, int k_fact) public pure returns (int) {
        int num = ((-m*lam*T / 1e36).exp() * (m*lam*T / 1e36).pow(i) / 1e18) / k_fact;
        return num;
    }


    function test3(int m, int lam, int T, int i, int k_fact) public pure returns (int) {
        int num = ((-m*lam*T / 1e36).exp() * (m*lam*T / 1e36).pow(i) / 1e18) / k_fact;
        return num;
    }
