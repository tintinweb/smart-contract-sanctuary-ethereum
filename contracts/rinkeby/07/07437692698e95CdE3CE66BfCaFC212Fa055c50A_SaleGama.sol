...
contract SaleGama is Ownable, ReentrancyGuard {
    ...
    function mintWhitelist(bytes32[] calldata merkleProof) public payable nonReentrant {
        ...
        if(block.timestamp < startMint + 60){ //7200 - 2 first hours
            require(isWalletOnAlphaWhitelist(merkleProof, msg.sender) == true, "Invalid proof, your wallet isn't listed in any whitelist");
        }  else {
            if (block.timestamp > startMint + 60 && block.timestamp < startMint + 120) { //86400 - 24 hours
