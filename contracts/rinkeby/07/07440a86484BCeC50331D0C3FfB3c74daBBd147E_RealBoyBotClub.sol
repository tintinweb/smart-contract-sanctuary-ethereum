...
/*
Linktree - https://linktr.ee/RealBoyBotClub

Website - https://realboybotclub.com/

Discord - https://discord.gg/QtNCzn7kmn

Twitter - https://twitter.com/RealBoyBotClub

Instagram - https://www.instagram.com/realboybotclub/
*/
...
contract RealBoyBotClub is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public MAX_ELEMENTS = 5555;
    uint256 public preSalePrice1 = 4 ether;
    uint256 public preSalePrice2 = 4 ether;
    uint256 public publicSalePrice = 4 ether;
    ...
    event welcomeToRealBoyBotClub(uint256 indexed id);
    event NewPriceEvent(uint256 price);
    event NewMaxElement(uint256 max);

    constructor(string memory baseURI) ERC721("RealBoyBotClub", "RBBC") {
        setBaseURI(baseURI);
        preSaleStarted = true;
    }
    ...
    function mint(uint256 _tokenAmount) public payable saleIsOpen {
        ...
        // require(_tokenAmount <= 10, "Max limit");
    ...
    function _mintAnElement(address _to, uint256 _tokenId) private {
        ...
        emit welcomeToRealBoyBotClub(_tokenId);
    }

    function price(uint256 _count) public view returns (uint256) {
        uint256 _price = 0;
        if (publicSaleStarted) {
            _price = publicSalePrice;
        } else {
            if (_count <= 4) {
                _price = preSalePrice1;
            } else {
                _price = preSalePrice2;
            }
        }
    ...
    function setPreSalePrice1(uint256 _price) public onlyOwner {
        preSalePrice1 = _price;
        emit NewPriceEvent(preSalePrice1);
    }

    function setPreSalePrice2(uint256 _price) public onlyOwner {
        preSalePrice2 = _price;
        emit NewPriceEvent(preSalePrice2);
    }

    function setPublicSalePrice(uint256 _price) public onlyOwner {
        publicSalePrice = _price;
        emit NewPriceEvent(publicSalePrice);
    }
    ...
    function withdrawFor(address[] memory _addrs, uint256[] memory _values)
        public
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _addrs.length; i++) {
            ...
            value = _values[i];
            require(value <= 100);
            _widthdraw(addr, balance.mul(value).div(100));
        }
    }
