...
contract z4 is ERC721A, ERC2981ContractWideRoyalties, RrpRequesterV0, Ownable, ReentrancyGuard {
    ...
    event Winner(uint256 winTokenId);
    ...
    constructor(address _airnodeRrp) RrpRequesterV0(_airnodeRrp) ERC721A("Z4", "Z4") {}
    ...
    modifier callerIsUser() {
      require(tx.origin == msg.sender, "Minting cannot be executed by contracts");
      _;
    }
    ...
    function isWinner(uint256 tokenId) external view returns (bool) {
      return gti[tokenId].GW;
    }

    function setPU(uint256 _ppu) external onlyOwner() {
      ppu = _ppu;
    }
    ...
    function getPU() external view returns (uint256) {
      return ppu;
    }
    ...
    function mintIt(uint256 totalMints) payable callerIsUser public returns (bytes32) {
	require(mintingEnabled, "Minting is not currently enabled.");

	uint256 finalCostMulti = paidMintPrice * totalMints;

	if (totalMints == 3) {
	  finalCostMulti -= paidMintPrice;
	  totalMints = 4;
	} else if (totalMints == 6) {
	  finalCostMulti -= (paidMintPrice * 3);
	  totalMints = 6;
	}

	require(msg.value >= finalCostMulti, "Insufficient fees paid for mint.");
        ...
        bytes32 requestId = airnodeRrp.makeFullRequest(
          ...
          abi.encode(bytes32("1u"), bytes32("size"), totalMints)
        );
    ...
    function fulfillUint256Array(bytes32 requestId, bytes calldata data) external onlyAirnodeRrp {
        ...
        uint256 gwdfi = (qrngUint256[i] % 100);
    ...
    function giveGT(address gtWinner, uint256 gtCount) external onlyOwner {
	...
	for (uint i = 0; i<gtCount; i++) {
	  ...
	  uint256 newTokenId = _tokenIds.current() + 1;
          ...
          _safeMint(gtWinner, newTokenId);
	}
    }

    function getRStats(uint256 tokenId) public view returns (bool, uint256, uint256, uint256, uint256, uint256) {
        return (
            gti[tokenId].GW,
            gti[tokenId].GWDF,
            gti[tokenId].PPU,
