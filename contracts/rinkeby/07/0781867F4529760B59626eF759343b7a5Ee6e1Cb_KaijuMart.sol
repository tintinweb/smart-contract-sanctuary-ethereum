...
error KaijuMart_CannotSettleLot();
...
error KaijuMart_MustIncludeData();

interface IKaiju { function balanceOf(address) external view returns (uint256); }
interface IMutants { function balanceOf(address) external view returns (uint256); }
interface IScientists { function balanceOf(address) external view returns (uint256); }
interface IScales {
 ...
 */
contract KaijuMart is AccessControl, ReentrancyGuard {
    ...
    modifier onlyKingz() {
        if (
            kaijuContracts.scientists.balanceOf(_msgSender()) == 0
            && kaijuContracts.mutants.balanceOf(_msgSender()) == 0
            && kaijuContracts.kaiju.balanceOf(_msgSender()) == 0
        ) revert KaijuMart_MustBeAKing();
        _;
    }

    /**
     * @notice Modifier that ensures a lot ID is available and reserves it
    ...
    /**
     * @notice Modifier that ensures a lot accepts a specified payment token
     * @param lotId Unique identifier for a lot
     * @param token Payment token option
     */
    modifier validPayment(uint256 lotId, PaymentToken token) {
    ...
    /**
     * @notice Set external KaijuKingz contracts
    ...
    /**
     * @notice Set external manager contracts
    ...
    /**
     * @notice Charge a specified address a specified amount of tokens
     * @dev Payment defaults to $RWASTE if `EITHER` is specified
     * @param account Address to charge
     * @param token Type of tokens to charge
     * @param value Amount of tokens to charge
     */
    function _charge(
        address account,
        PaymentToken token,
    ...
    /**
     * @notice Refund a specified address a specified amount of tokens
    ...
    /**
     * @notice Settle a specified lot
     * @param lotId Lot to settle
     * @param account Address settling the lot
     * @param data Bytes sent to the settler contract
     */
    function _settle(
        uint256 lotId,
        address account,
        ...
        nonReentrant
    {
        address settler = lots[lotId].settler;

        if (settler != address(0)) {
            settler.functionCall(data);

            emit Settle(lotId, settler, account, data);
        }
    }
     ...
     * @notice TODO
     * @param auctionId TODO
     * @param lowestWinningBid TODO
     * @param tiebrokenWinners TODO
     */
    function close(
        uint256 auctionId,
    ...
    function bid(
        ...
        onlyKingz
    {
        uint104 newBid = managerContracts.auction.increaseBid(auctionId, value, _msgSender());
        ...
        emit Bid(auctionId, _msgSender(), newBid);
    }
     ...
     * @param auctionId TODO
     */
    function claimRefund(uint256 auctionId) external lotExists(auctionId) {
    ...
    /**
     * @notice TODO
     * @dev Do not need to check Kingz status because it was needed to create a bid
     * @param auctionId TODO
     * @param data TODO
     */
    function settle(
        ...
        lotExists(auctionId)
    {
        if (!managerContracts.auction.isWinner(auctionId, _msgSender())) revert KaijuMart_CannotSettleLot();

        _settle(auctionId, _msgSender(), data);
    ...
    function purchase(
        ...
        validPayment(doorbusterId, token)
        onlyKingz
    {
        if (lots[doorbusterId].settler != address(0)) revert KaijuMart_MustIncludeData();

        managerContracts.doorbuster.purchase(doorbusterId, amount, nonce, signature);

        _purchase(doorbusterId, amount, token);
    }
    ...
    function purchase(
        ...
        onlyKingz
    {
        managerContracts.doorbuster.purchase(doorbusterId, amount, nonce, data, signature);

        _purchase(doorbusterId, amount, token);

        _settle(doorbusterId, _msgSender(), data);
    }
