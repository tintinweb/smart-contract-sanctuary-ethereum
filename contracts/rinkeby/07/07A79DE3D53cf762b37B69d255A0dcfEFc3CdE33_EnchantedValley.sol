...
/**
 * @title EnchantedValley
 * EnchantedValley - Smart contract for EnchantedValley characters
 */
contract EnchantedValley is ERC721, Ownable {
    ...
    mapping(uint256 => string) private tribe_uris;
    ...
    constructor(
        ...
        tribe_uris[0] = "ipfs://IPFS_HASH_TRIBE_1/";
        tribe_uris[1] = "ipfs://IPFS_HASH_TRIBE_2/";
        tribe_uris[2] = "ipfs://IPFS_HASH_TRIBE_3/";
        tribe_uris[3] = "ipfs://IPFS_HASH_TRIBE_4/";
        tribe_uris[4] = "ipfs://IPFS_HASH_TRIBE_5/";
    ...
    */
    function claimArtifact(uint256[] calldata _artifacts) public {
        uint256 artifact_balance = ARTIFACT.balanceOf(msg.sender);
        require(artifact_balance > 0, "You don't own any artifact");
        uint256 j = 0;
        for (j = 0; j < _artifacts.length; j++) {
            address owner = ARTIFACT.ownerOf(_artifacts[j]);
            require(
                owner == msg.sender,
                "Trying to claim a token you don't own"
            );
            require(
                claimed_artifacts[_artifacts[j]] == false,
                "Artifact claimed yet"
            );
            uint256 reached_hardcap = totalSupply() + 1;
            uint256 reached_softcap = token_ids[selling_tribe] + 1;
            require(
                reached_hardcap <= HARD_CAP && reached_softcap <= SOFT_CAP,
                "Max amount reached."
            );
            claimed_artifacts[_artifacts[j]] = true;
            token_ids[selling_tribe]++;
            uint256 nextId = (2200 * selling_tribe) + token_ids[selling_tribe];
    ...
    */
    function buyNFT(bytes32[] calldata _merkleProof, uint256 _tribe)
        ...
        payable
    {
        require(sale_active && selling_tribe >= _tribe, "Sale is not active");
        bool canMint = false;
        ...
        require(canMint, "Sorry you can't mint right now");
        require(
            msg.value % minting_price == 0,
            "Amount should be a multiple of minting cost"
        );
        uint256 amount = msg.value / minting_price;
        require(amount >= 1, "Amount should be at least 1");
        require(amount <= maxAmount, "Amount must be less or equal to 3");
    ...
    */
    function fixSellingTribe(uint256 newTribe) public onlyOwner {
    ...
    /*
        This method will allow owner to start and stop the sale
    */
    function fixSaleState(bool newState) public onlyOwner {
    ...
    */
    function fixMaxAmount(uint256 newMax) public onlyOwner {
        MAX_AMOUNT = newMax;
    }
    ...
    */
    function fixArtifactAddress(address _artifactAddress) public onlyOwner {
        ARTIFACT = IARTIFACT(_artifactAddress);
    }
    ...
    */
    function returnTribeSupply(uint256 _tribe) public view returns (uint256) {
    ...
    */
    function withdrawEther() public onlyOwner {
        require(gnosis_vault != address(0), "Can't withdraw on black hole.");
        uint256 balance = address(this).balance;
        require(balance > 0, "Nothing to withdraw!");
        payable(gnosis_vault).transfer(balance);
    }
 ...
 */
interface IARTIFACT is IERC165 {
    /**
