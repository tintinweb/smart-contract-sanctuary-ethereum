...
contract FFTokenGenerator is AccessControl, Pausable, ReentrancyGuard {
    ...
    */
    ...
    function updateProject(FFProject calldata FFProject_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
        ...
        projects[FFProject_.addr] = FFProject_;
    ...
    */
    ...
    /** 
        Stopping disregards daysEnd and simply sets the `end` to a hard stop of now
    */
    ...
    */
    ...
    /** 
        #TODO decide if we need the function after the audits
		If we need to stop token generation, "end" can be set in the past (#TODO test that)
		The last claims entry in the mapping remains. Should we delete it too? (can't be done with "delete")
	*/
    function removeProject(address project_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        delete projects[project_];

        emit ProjectRemoved(msg.sender, project_);
    }
    ...
    */
    ...
    {
        _claimable = getClaimable(project_, id_);
    }

    function getClaimable(address project_, uint256 id_)
        internal
        view
        returns (uint256 _totalClaim)
    {
        FFProject memory _project = projects[project_];

        if (lastClaims[project_][id_] < _project.end) {
            _totalClaim = pendingYield(project_, id_);
        }
    }
        ...
        returns (uint256 _totalClaim)
    {
        for (uint256 i = 0; i < FFProjectIds_.length; i++) {
            ...
            for (uint256 j = 0; j < _ids.length; j++) {
                _totalClaim = _totalClaim.add(getClaimable(_project, _ids[j]));
            }
        }
    }
