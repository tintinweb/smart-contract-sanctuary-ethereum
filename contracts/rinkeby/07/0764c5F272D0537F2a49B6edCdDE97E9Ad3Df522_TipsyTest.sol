...
contract TipsyTest is ERC721A, Ownable {
    ...
    uint256 public MAX_WHITELIST_TOKENS = 2500;
    uint256 public MAX_WHITELIST_PER_WALLET = 12;
    uint256 public whitelistMintPrice = 0.004 ether;
    ...
    function whitelistMint(uint256 quantity, bytes32[] calldata merkleProof) external payable isValidMerkleProof(merkleProof, whitelistMerkleRoot) {
        ...
        bool isFree = (freemintedAmount + quantity <= NUM_FREE_MINTS) && (quantity <= MAX_FREE_PER_WALLET) && (freeMintCounts[msg.sender] + quantity <= MAX_FREE_PER_WALLET);

        if(!isFree) {
            require(msg.value >= (whitelistMintPrice * quantity), "Not enough ether sent");
        } else {
            freemintedAmount += quantity;
