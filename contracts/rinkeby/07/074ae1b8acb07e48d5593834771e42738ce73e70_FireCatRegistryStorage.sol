...
* @notice Ownable
*/
contract FireCatRegistryStorage is Ownable {
    using SafeMath for uint256;
    address public registryProxy;

    mapping (address => address) public userInviterMap;
    mapping (address => address[]) public inviterUsersMap;
    mapping (address => bool) public isRegistered;
    address[] public userArray;
    uint256 public totalUsers = 0;

    event SetRegistryProxy(address proxy_);
    event SetRegistered(address user_);

    modifier onlyProxy() {
        require(msg.sender == registryProxy, "FC:E05");
        _;
    }

    /**
    * @notice set registry proxy.
    * @dev Access: only owner.
    * @param proxy_ proxy_address.
    */
    function setRegistryProxy(address proxy_) external onlyOwner {
        registryProxy = proxy_;
        emit SetRegistryProxy(proxy_);
    }
    ...
    */
    function setRegistered(address user_) external onlyProxy {
        isRegistered[user_] = true;
        totalUsers = totalUsers.add(1);
        userArray.push(user_);
        emit SetRegistered(user_);
    }
    ...
    */
    function setUserInviterMap(address user_, address inviter_) external onlyProxy {
        userInviterMap[user_] = inviter_;
    }
    ...
    * @param inviter_ inviter_address
    */
    function setInviterUsersMap(address user_, address inviter_) external onlyProxy {
        inviterUsersMap[inviter_].push(user_);
    }
    ...
    */
    function getUsers(address inviter_) public view returns (address[] memory) {
        return inviterUsersMap[inviter_];
    }
