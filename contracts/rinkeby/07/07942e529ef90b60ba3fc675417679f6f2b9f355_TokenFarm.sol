...
contract TokenFarm is Ownable {
    ...
    function addPool (address _tokenAddress, address _rewardTokenAddress, uint256 _maxPoolSize, uint256 _maxContribution, uint256 _rewardNum, uint256 _rewardDen, uint256 _emergencyFees, uint256 _lockDays, bool _poolType, bool _poolActive) public onlyOwner {
    ...
    function getUserLockTime (uint256 _pid, address _user) public view returns (uint256) {
        return (userInfo[_pid][_user].stakingTime).add((poolInfo[_pid].lockDays).mul(1 minutes));
    }
    ...
    function claimableRewards (uint256 _pid, address _user) public view returns (uint256) {
        ...
        uint256 _refundValue = ((userInfo[_pid][_user].amount * poolInfo[_pid].rewardNum).mul(10 ** rewardToken.decimals())).div (poolInfo[_pid].rewardDen * 10 ** token.decimals());
        return _refundValue;
    }
