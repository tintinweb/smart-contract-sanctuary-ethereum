...
contract SoundMintEdition is ERC721ARoyalty, Ownable, ReentrancyGuard {
    ...
    onlyEOA
  {
    ...
    _safeTransferAPE(address(this), salePriceETH * qty);
    ...
    onlyEOA
  {
    ...
    // require(msg.value == salePriceETH * qty, "PreSale: Insufficient ETH");
     if(msg.value <= salePriceETH * qty) {
      require(IERC20(apeAddress).balanceOf(msg.sender) >= salePriceAPE * qty, "PublicSale: Insufficient ETH and APE");
      _safeTransferAPE(address(this), salePriceETH * qty);
  ...
  function withdraw() external onlyOwner {
    ...
    function _safeTransferAPE(address to, uint256 amount) internal returns (bool) {
        bool success = IERC20(apeAddress).transfer(to, amount);
        return success;
    }    
}
