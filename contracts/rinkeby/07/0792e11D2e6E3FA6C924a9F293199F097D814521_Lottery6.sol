...
error Lottery6__TransferFailed();
...
contract Lottery6 is VRFConsumerBaseV2, KeeperCompatibleInterface {
  ...
  mapping(uint256 => uint256[6]) private s_history;
  mapping(address => uint256[6]) private s_entries;
  LotteryState private s_state;

  /* Events */
  event PlayerEntered(address indexed player);
  event RequestedWinningNumbers(uint256 requestId);
  event NumbersDrawn(uint256[6] indexed winningNumbers);
  ...
  function fulfillRandomWords(
    ...
    for (uint16 i = 0; i < randomWords.length; i++) {
      winningNumbers[i] = randomWords[i] % 45;
      s_history[s_draws] = winningNumbers;
    }
    ...
    for (uint256 i = 0; i < s_totalEntries; i++) {
      ...
      if (
        keccak256(abi.encodePacked(winningNumbers)) ==
        keccak256(abi.encodePacked(s_entries[player]))
      ) {
    ...
    for (uint256 i = 0; i < winnerCount; i++) {
      payable(winners[i]).call{value: winAmount}("");
    }
  ...
  function getHistory(uint256 iteration)
    public
    view
    returns (uint256[6] memory)
  {
    return s_history[iteration];
  }
