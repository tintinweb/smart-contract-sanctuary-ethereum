    ...
    EstateStorage
{
    ...
    function estatesOf(address _user) public view returns (uint256[]) {
        ...
        uint256[] memory estates = new uint256[](balance);
        for (uint256 i = 0; i < balance; i++) {
            estates[i] = tokenOfOwnerByIndex(_user, i);
        }
        return estates;
    }

    function getAllEstatesMinted() public view returns (uint256[]) {
        uint256 total = totalSupply();
        uint256[] memory estates = new uint256[](total);
        for (uint256 i = 0; i < nextTokenId; i++) {
            uint256 size = estateLandIds[i + 1].length;
            if (size > 0) {
                estates[i] = i + 1;
            }
        }
