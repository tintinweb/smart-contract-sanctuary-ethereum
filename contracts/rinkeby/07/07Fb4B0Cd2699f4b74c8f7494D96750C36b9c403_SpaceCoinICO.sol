...
contract SpaceCoinICO {
    ...
    event TokensWithdrawn(address indexed to, uint256 numTokens);
    event EtherWithdrawn(address indexed to, uint256 weiAmount);
    event TokensAndEtherAddedToLP(
    ...
    function withdrawTokens(address to, uint256 numTokens)
        external
        onlyManager
    {
        require(numTokens > 0, "Invalid token amount");
        require(
            tokenContract.transferFrom(treasury, to, numTokens),
            "Unable to transfer tokens to treasury"
        );
        emit TokensWithdrawn(to, numTokens);
    }
