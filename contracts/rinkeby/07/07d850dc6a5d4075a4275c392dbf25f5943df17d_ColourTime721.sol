...
//    event MetadataUpdated(string newTokenUriBase);
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory){
            ...
            // parts[1] = svgData[tokenId][0];
            // parts[2] = '" dur="';
            // parts[3] = toString(svgSpeeds[tokenId][0]);
            // parts[4] = '" repeatCount="indefinite"></animate></rect><rect x="2" y="2" width="6" height="6"><animate id="innerbox" attributeName="fill" values="';
            // parts[5] = svgData[tokenId][1];
            // parts[6] = '" dur="';
            // parts[7] = toString(svgSpeeds[tokenId][1]);
            ...
            // string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Bag #', toString(tokenId), '", "description": "Loot is randomized adventurer gear generated and sto.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
            ...
            string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Colour Time ', toString(tokenId), '", "description": "Slowly shifting colours, records of time past. Jonathan Chomko, 2022", "image": "data:image/svg+xml;base64,',Base64.encode(bytes(output)),'"}'))));
            output = string(abi.encodePacked('data:application/json;base64,', json));
            return output;
    }
    ...
    function updateBaseURI(string memory givenTokenURIBase) external onlyOwner{
        _setBaseURI(givenTokenURIBase);
    }
    ...
    //Set data for tokens
    function setData(tokenData[] memory givenData )external {
    ...
    // function mintItem(string memory fore, string memory back, uint256 foreSpeed, uint256 backSpeed, string memory givenTitle) public onlyOwner returns (uint256) {
        ...
        // tokenDataMap[tokenCounter] = tokenData({ colours:[fore, back], speeds:[foreSpeed, backSpeed], title:givenTitle});
        _safeMint(msg.sender, givenTokenId);

        //do we want a max number of tokens?
        tokenCounter += 1;
