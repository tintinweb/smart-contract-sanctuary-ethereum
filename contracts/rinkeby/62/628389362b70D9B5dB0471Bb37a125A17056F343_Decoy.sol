...
contract Decoy is ERC721A, Ownable, ReentrancyGuard, VRFConsumerBaseV2 {
    ...
    uint256 private constant _availableForAllowlist = 1000;
    uint256 private constant _allowlistWalletLimit = 3;
    uint256 private _mintPrice = 30000000000000000; // 0.03 ETH
    ...
    modifier mintCompliance(uint256 _quantity) {
        ...
        if (msg.value != _quantity * _mintPrice)
            revert WrongAmount(msg.value, _quantity * _mintPrice);
        _;
    }
        ...
        mintCompliance(_quantity)
    {
        ...
        if (_allowlist[msg.sender] + _quantity > _allowlistWalletLimit)
    ...
    function withdrawFunds() external onlyOwner {
        (bool sent, ) = payable(_withdrawalAddress).call{value: address(this).balance}('');
