...
contract MarketplaceV2 is Ownable {
    ...
    /**
     * @dev Function matched transaction with user signatures bundle sell
     */
    function matchTransaction(
        ...
        require(
            paymentContract.balanceOf(_msgSender()) >= price,
            "Marketplace: buyer doesn't have enough token to buy this item"
        );
        require(
            paymentContract.allowance(_msgSender(), address(this)) >= price,
        ...
        if (transactionFeePerMillion.mul(price).div(1000000) > 0) {
            ...
            paymentContract.safeTransferFrom(_msgSender(), feeToAddress, transactionFeePerMillion.mul(price).div(1000000));
        ...
        for (uint i=0;i<tokenIds.length;i++ ){
            ...
            emitEvent(addresses, tokenIds[i],price);
        }
        usedSignatures[signature] = true;
        // emit sale event
        return true;
...
/**
     * @dev Function matched transaction with user signatures single sell
     */
    function matchTransaction(
        address[3] calldata addresses,
        uint256  tokenId,
        ...
        uint256 fee = transactionFeePerMillion.mul(price).div(10000);
        uint256 payToSellerAmount = price.sub(fee);
        ...
        usedSignatures[signature] = true;
        // emit sale event
        emitEvent(addresses, tokenId,price);
        return true;
    }
