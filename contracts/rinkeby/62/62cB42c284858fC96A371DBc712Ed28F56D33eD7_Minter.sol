...
/** 
 *  SourceUnit: c:\Users\ADMIN\Desktop\United-WorldEx-Contract\contracts\Minter.sol
*/
...
contract Minter is Ownable, ReentrancyGuard {
    ...
    function claimById(address uAssetAddress, bytes memory id) public nonReentrant {
        ...
        emit ClaimToken(msg.sender, uAssetAddress, id, tokenAmount, block.timestamp);
    }
    
    function claimAll(address[] memory uAssetAddresses, bytes[] memory ids) external nonReentrant {
        uint256 lockTime = IController(controllerAddress).lockTime();
        for(uint256 i = 0; i < ids.length; i++) {
            if(block.timestamp - updatedLockTime[ids[i]] > lockTime) {
                claimById(uAssetAddresses[i], ids[i]);
            }
        }
    }
    ...
    function borrow(address uAssetAddress, uint256 uAssetAmount, uint256 collateralAmount, bytes memory id) external nonReentrant {
        ...
        require(collateralBalances[id] == 0, "Cannot call borrow again with the same id");
        ...
        emit BorrowAsset(msg.sender, uAssetAddress, id, uAssetAmount, collateralAmount, block.timestamp);
    }
    ...
    function close(address uAssetAddress, bytes memory id) external nonReentrant {
        ...
        emit Close(msg.sender, uAssetAddress, id, uAssetAmount, collateralAmount, block.timestamp);
    }

    function short(address uAssetAddress, uint256 uAssetAmount, uint256 collateralAmount, bytes memory id) external nonReentrant {
        if(accounts[id] == address(0)) accounts[id] = msg.sender;
        ...
        }
        {
            ...
            emit Short(msg.sender, uAssetAddress, id, uAssetAmount, collateralAmount, block.timestamp);
        }
    }
    
    function editShort(address uAssetAddress, uint256 uAssetAmount, uint256 collateralAmount, bytes memory id) external nonReentrant {
        ...
        }
        {
            if(block.timestamp - updatedTime > ttl) {
                ...
                if(uAssetAmount < borrowBalances[id]) {
                    ...
                    address routerAddress = IController(controllerAddress).router();
                    IERC20(addr).safeApprove(routerAddress, diff);
                    uint[] memory amountOuts = IUniswapV2Router02(routerAddress).swapExactTokensForTokens(diff, 0, path, address(this), block.timestamp + 60);
                    lock(id, amountOuts[1]);
                }
            }
        }
    ...
    function liquidation(address userAddress, address uAssetAddress, uint256 uAssetAmount, bytes memory id) external nonReentrant {
        ...
        uint256 discountedCollateralValue = 
            (uAssetAmount * targetPrice) / (10 ** ERC20(uAssetAddress).decimals())
        ...
        if (discountedCollateralValue <= collateralBalances[id]) {
            ...
            emit Liquidation(msg.sender, userAddress, uAssetAddress, id, uAssetAmount, discountedCollateralValue, block.timestamp);
        } else {
            uint256 redundant = discountedCollateralValue - collateralBalances[id];
            uint256 refundUAssetAmount = redundant * (10**calculationDecimal - discountRate) / ((10**calculationDecimal) * targetPrice / (10 ** ERC20(uAssetAddress).decimals()));
            IERC20(uAssetAddress).safeTransferFrom(msg.sender, address(this), uAssetAmount - refundUAssetAmount);
            IERC20(collateralAddress).safeTransfer(msg.sender, collateralBalances[id]);
            borrowBalances[id] -= (uAssetAmount - refundUAssetAmount);
            collateralBalances[id] = 0;
            uint256 sub1 = uAssetAmount - refundUAssetAmount;
            uint256 sub2 = discountedCollateralValue - redundant;
            emit Liquidation(msg.sender, userAddress, uAssetAddress, id, sub1, sub2, block.timestamp);
        }
    }
