...
abstract contract OdysseyDatabase {
    ...
    // keccak256("whitelistMint(bytes32 merkleRoot,uint256 minPrice)");
    bytes32 public constant MERKLE_TREE_ROOT_TYPEHASH =
        0xc6a8334400691a3fa229cd09555c5d4451f04fa3f7739421b209d93463965d21;
    // Common Storage
    mapping(address => bytes32) public domainSeparator;
    mapping(address => uint256) public whitelistActive;
    mapping(address => address) public ownerOf;
    // ERC721 Storage
    mapping(address => mapping(address => uint256)) public whitelistClaimed721; // todo remove public. public is helping me for testing purposes atm
...
contract OdysseyLaunchPlatform is OdysseyDatabase, ReentrancyGuard {
    ...
    function mintERC721(
        ...
        address tokenContract = OdysseyTokenFactory(factory).getToken(
            name,
            symbol
        );

        require(
            whitelistClaimed721[tokenContract][msg.sender] == 0,
            "OdysseyLaunchPlatform: Already claimed"
        );
        // Check if user is already reserved + paid
        if (isReserved721[tokenContract][msg.sender] == 0) {
            require(
                cumulativeSupply721[tokenContract] <
                    maxSupply721[tokenContract],
            ...
            if (whitelistActive[tokenContract] == 1) {
                // Verify merkle root and minPrice signed by owner (all id's have same min price)

                bytes32 hash = keccak256(
                    abi.encode(MERKLE_TREE_ROOT_TYPEHASH, merkleRoot, minPrice)
                );
                //todo include name + symbol in sig verify
                Signature.verify(
                    hash,
                    ownerOf[tokenContract],
                    v,
                    r,
                    s,
                    domainSeparator[tokenContract]
                );
        ...
        // Update State
        whitelistClaimed721[tokenContract][msg.sender] = 1;
        OdysseyERC721(tokenContract).mint(
            msg.sender,
            mintedSupply721[tokenContract]++
        );
    }
    ...
    function reserveERC721(
        ...
        require(
            cumulativeSupply721[tokenContract] < maxSupply721[tokenContract],
            "OdysseyLaunchPlatform: Max Supply"
        );
        require(
            isReserved721[tokenContract][msg.sender] == 0,
        ...
        isReserved721[tokenContract][msg.sender] = 1;
...
contract OdysseyRouter is OdysseyDatabase, ReentrancyGuard {
    ...
    function create1155(
        ...
        for (; i < tokenIds.length; ++i) {
            maxSupply1155[token][tokenIds[i]] = maxSupply[i];
        }
    ...
    function create721(
        ...
        ownerOf[token] = msg.sender;
        maxSupply721[token] = maxSupply; // todo let 0 mean no cap
        whitelistActive[token] = whitelist ? 1 : 0;
        domainSeparator[token] = keccak256(
            abi.encode(
                // keccak256('EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)')
    ...
    function mintERC721(
        ...
        (bool success, ) = launchPlatform.delegatecall(
            abi.encodeWithSignature(
                "mintERC721(bytes32[],bytes32,uint256,string,string,uint8,bytes32,bytes32)",
                merkleProof,
                merkleRoot,
                minPrice,
                name,
    ...
    function reserveERC721(
        ...
        (bool success, ) = launchPlatform.delegatecall(
            abi.encodeWithSignature(
                "reserveERC721(bytes32[],bytes32,uint256,string,string,uint8,bytes32,bytes32)",
                merkleProof,
                merkleRoot,
                minPrice,
                name,
    ...
    function mintERC1155(
        ...
        (bool success, ) = launchPlatform.delegatecall(
            abi.encodeWithSignature(
                "mintERC1155(bytes32[],bytes32,uint256,uint256,address,uint8,bytes32,bytes32)",
                merkleProof,
                merkleRoot,
                minPrice,
                tokenId,
    ...
    function reserveERC1155(
        ...
        (bool success, ) = launchPlatform.delegatecall(
            abi.encodeWithSignature(
                "reserveERC1155(bytes32[],bytes32,uint256,uint256,address,uint8,bytes32,bytes32)",
                merkleProof,
                merkleRoot,
                minPrice,
                tokenId,
