...
contract AccessToken is ERC721 {
    ...
    function buyMembership(uint256 membershipId, string memory coverTitle) external payable {
        ...
        // set token URI
        tokenURI(nextTokenId, coverTitle, memberships[membershipId].coverId);
    ...
    function tokenURI(uint256 _tokenId, string memory coverTitle, uint256 _coverId)
    public
    view
    returns (string memory)
    {
        // If the token does not map to an memberships, it'll be 0.
        require(tokenToMembership[_tokenId] > 0, "Token has not been sold yet");
        // Concatenate the components, baseURI, membershipsId and tokenId, to create URI.

        bytes memory dataURI = abi.encodePacked(
            '{',
            '"name": ', coverTitle,' #', Strings.toString(_tokenId), '"',
        // Replace with extra ERC721 Metadata properties
            '"description": ', '" this is Creader accessToken for cover #', Strings.toString(_coverId), '"',
            '"Cover":', '{'
        '"id":', Strings.toString(_coverId),
            '"title":', coverTitle,
            '}'
            '}'
        );
    ...
    function checkOwnership(uint256 coverId, uint256 tokenId) public view returns (bool) {
        require(ownerOf(tokenId) == msg.sender, "you are not the owner of token");
        uint MembershipId = tokenToMembership[tokenId];

        return memberships[MembershipId].coverId == coverId;
    }
