...
contract MockTokenFaucet is Ownable {
    ...
    function mintERC721(
        ...
        for (uint256 count = 0; count < mintValue; count++) {
            mintToken.mint(mintValue, to);
        }
    }

    function mintPunk(address to, uint256 mintValue) public {
        if (address(cryptoPunks) == address(0)) return;
        uint256 nextPunkIndex = cryptoPunks.punksRemainingToAssign() - 1;
        uint256 mintPunkCount;

        for (uint256 index = 0; index < 10000; index++) {
            if (cryptoPunks.punkIndexToAddress(nextPunkIndex) == address(0)) {
                cryptoPunks.getPunk(nextPunkIndex);
                cryptoPunks.transferPunk(to, nextPunkIndex);
                mintPunkCount++;
            }
            if (mintPunkCount == mintValue) break;
            nextPunkIndex--;
        }
    }
    ...
    function mintPunks(address to) internal {
        ...
        for (uint256 index = 0; index < 10000; index++) {
            if (mintPunkCount == punksToken.mintValue) break;
            if (cryptoPunks.punkIndexToAddress(nextPunkIndex) == address(0)) {
                cryptoPunks.getPunk(nextPunkIndex);
                cryptoPunks.transferPunk(to, nextPunkIndex);
                mintPunkCount++;
