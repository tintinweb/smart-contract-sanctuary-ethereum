...
contract PleaseWork is ERC20("fucktoken", "FKTOKEN"), Ownable, IDragonToken, Pausable {
	event ClaimedFuck(address indexed _account, uint256 _reward);

    uint256 constant public DAILY_RATE = 1000 ether;
    uint256 constant public DAILY_LEGENDARY_BONUS = 3000 ether;
    ...
    function updateReward(address _from, address _to, uint256 _tokenId) external override {
        require(_from == address(0) || dragons.ownerOf(_tokenId) == _from, "NOT_OWNER_OF_DRAGON");
				require(_from == address(0) || genesis.ownerOf(_tokenId) == _from, "NOT_OWNER_OF_DRAGON");
        require(msg.sender == address(dragons), "ONLY_DRAGONS");
				require(msg.sender == address(genesis), "ONLY_DRAGONS");
    ...
    function _calculateRewards(address _account) internal view returns(uint256) {
        ...
        if (delta > 0) {
            /*
            uint256 pendingBasic = dragons.balanceOf(_account) * (DAILY_RATE * delta / 86400);
            ...
            */
            uint256 pendingBasic = dragons.balanceOf(_account) * (DAILY_RATE * delta / 8);
						uint256 pendingGenesis = genesis.balanceOf(_account) * (DAILY_RATE * delta / 8);
            uint256 bonus = _calculateBonus(_account) * delta / 86400;
    ...
    function _calculateBonus(address _account) internal view returns(uint256) {
        uint256 curBalance = dragons.balanceOf(_account);
        if (curBalance >= 20)
            return 400 ether;
        if (curBalance >= 10)
            return 150 ether;
