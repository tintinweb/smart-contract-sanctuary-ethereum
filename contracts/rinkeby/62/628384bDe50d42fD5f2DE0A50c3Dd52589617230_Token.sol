...
contract Token is ERC721A, Pausable, AccessControl {
    ...
    event ChangeMaxTokens(uint32 _maxTokens);
    ...
    function setMaxTokens(uint32 _maxTokens) external onlyRole(DEFAULT_ADMIN_ROLE) {
        maxTokens = _maxTokens;
        emit ChangeMaxTokens(_maxTokens);
    }    
    ...
    function mint(address _owner, uint256 _amount, uint256[] calldata _options) external onlyRole(MINTER_ROLE) {
        ...
        require(_amount <= maxTokens, "max amount exceeded");
