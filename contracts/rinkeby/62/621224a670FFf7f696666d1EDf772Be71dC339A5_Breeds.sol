 ...
 */
contract Breeds is ERC1155, Ownable {
    ...
    function createKittyGen0(uint256 _dna) public onlyOwner returns (uint256) {
        ...
        )
    {
        ...
        birthTime_ = uint256(kitties[_kittyId].birthTime);
        momId_ = uint256(kitties[_kittyId].momId);
        dadId_ = uint256(kitties[_kittyId].dadId);
        generation_ = uint256(kitties[_kittyId].generation);
    }
    ...
    {
        require(
            tokenOwner[_momId] == msg.sender &&
                tokenOwner[_dadId] == msg.sender,
        ...
        returns (uint256 newDna)
    {
        ...
        uint256[8] memory dnaArray;

        uint8 random = uint8(block.timestamp % 255);
        uint256 i;
        uint256 dnaArrayMaxIndex = 7;
    ...
    function transfer(address _to, uint256 _tokenId) external {
        require(
            ...
            "Sender should own the token before transferring."
        );
        require(_to != address(0), "Receiving address should be valid");
        require(
            _to != address(this),
            "Contract address cannot be the receiver"
        );
