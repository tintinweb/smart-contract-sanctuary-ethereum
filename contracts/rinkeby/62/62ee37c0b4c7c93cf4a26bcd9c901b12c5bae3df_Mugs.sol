...
import {ThirdwebContract} from "./lib/thirdweb/ThirdwebContract.sol";
    ...
    ThirdwebContract
{
    ...
    modifier onlyPublic() {
        require(saleState == SaleState.PUBLIC, "NOT PUBLIC");
        _;
    }

    modifier whenNotPaused() {
        require(saleState != SaleState.PAUSED, "PAUSED");
        _;
    }

    modifier canMint(uint256 quantity) {
        require(quantity + _nextTokenId() <= MAX_PUBLIC_MINTABLE, "MAX SUPPLY");
        _;
    }

    modifier includesCorrectPayment(uint256 quantity) {
        require(
            quantity * uint256(mintPrice) == msg.value,
            "INCORRECT PAYMENT"
        );
        _;
    }
    ...
    ) MaxMintable(name, symbol) BatchReveal(provenanceHash) {
    ...
    function mint(uint256 quantity)
        ...
        includesCorrectPayment(quantity)
    {
    ...
    function devMint(uint16 amount, address to) public onlyOwner {
        require(
            uint256(numDevMinted) + amount <= MAX_DEV_MINTABLE,
            "MAX DEV MINTED"
        );
        numDevMinted += amount;
        _safeMint(to, amount);
    ...
    function setMintPrice(uint64 newPrice) public onlyOwner {
        mintPrice = newPrice;
    }
    ...
    function setRoyaltyInfo(address recipient) public onlyOwner {
...
contract AllowList is Ownable {
    // uint256 public maxRedemptionsPerAddress;
...
contract BatchReveal is Ownable {
    using Strings for uint256;
    ...
    ///@dev reveal a batch of tokens by including a maxId (exclusive) and a URI all tokens starting at previous Reveal's maxId
    ...
    ///@dev update defaultURI, including in case of final reveal;
        ...
        if (!fullyRevealed) {
            ...
            for (uint256 i = 0; i < length; i++) {
                ...
                if (_tokenId < reveal.maxId) {
                    return string.concat(reveal.uri, _tokenId.toString());
                }
            }
        }
        return
            fullyRevealed
                ? string.concat(defaultURI, _tokenId.toString())
                : defaultURI;
    }
}
...
contract MaxMintable is ERC721A, Ownable {
    ...
    function _checkWalletcheckMaxMinted(uint256 _quantity) internal {}
}
...
contract ThirdwebContract is Ownable {
    ...
    function _contractDeployer() internal view returns (address) {
        if (address(this).code.length == 0) {
            try
                IContractDeployer(msg.sender).getContractDeployer(address(this))
...
interface IContractDeployer {
    ...
    function getContractDeployer(address _contract)
        external
        view
        returns (address);
}
