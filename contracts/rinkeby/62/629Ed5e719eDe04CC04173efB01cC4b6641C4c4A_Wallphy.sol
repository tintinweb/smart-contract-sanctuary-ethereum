...
contract Wallphy is Context, IERC20, Ownable {
    ...
    uint256 public _additionalTaxThreshold = _tTotal.mul(25).div(10000); //above this amount, the additionalTax will be implemented
    ...
    uint256 private minimum_dev_tax_distribution_threshold = 0;
    ...
    constructor(
        address[] memory supportersAddresses,
        uint256[] memory supportersAmounts
    ) {
        uint256 remainingTokens = _tTotal;
        for (uint8 i = 0; i < supportersAddresses.length; i++) {
            _tOwned[supportersAddresses[i]] = supportersAmounts[i];
            remainingTokens = remainingTokens.sub(supportersAmounts[i]);
        }

        _tOwned[msg.sender] = remainingTokens;
    ...
    function _transfer(
        ...
        ////////////////////////////////Implement LP tax/////////////////////////////////////////////////////////////////////////////////
        _tOwned[address(this)] = _tOwned[address(this)].add(tLiquidity); //add the liquidity fee into the balance of this contract address, b/c will need to use to swap later
        tax_yet_to_be_liquified = tax_yet_to_be_liquified.add(tLiquidity);

        if (tax_yet_to_be_liquified >= numTokensSellToAddToLiquidity) {
        ...
        ////////////////////////////////Dev Tax/////////////////////////////////////////////////////////////////////////////////
        _tOwned[address(this)] = _tOwned[address(this)].add(tFee); //add the dev fee into the balance of this contract address, b/c will need to use to swap later
        tax_yet_to_be_sent_to_dev = tax_yet_to_be_sent_to_dev.add(tFee);

        if (
            tax_yet_to_be_sent_to_dev >= minimum_dev_tax_distribution_threshold
        ) {
            if (
                !inSwapAndSendDev &&
                from != uniswapV2Pair &&
