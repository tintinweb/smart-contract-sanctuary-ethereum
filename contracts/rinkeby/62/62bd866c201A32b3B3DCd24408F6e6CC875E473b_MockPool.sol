    ...
    function changeAddress(address newAccount) public virtual {
        ...
        for (uint256 i = 0; i < _payees.length; i++) {
            if (_payees[i] == account) {
                _shares[newAccount] = _shares[_payees[i]];
                _shares[_payees[i]] = 0;
                _tokenReleased[newAccount] = _tokenReleased[_payees[i]];
                _tokenReleased[_payees[i]] = 0;
                _payees[i] = newAccount;
            } 
        }
    }
