...
contract Genesis is MultisigOwnable, ERC721A, ERC721ABurnable, ERC721AQueryable, PaymentSplitter {

    using Strings for uint;

    uint WLSaleDate = 1663599600;
    uint ALSaleDate = 1663600800;
    uint SaleDate = 1663602000;
    uint EndSaleDate = 1663603200;

    string public baseURI;
    string public unrevealedURI;

    uint private constant METADATAS_LENGTH = 33;
    uint private constant MAX_TEAM = 15;
    uint private constant MAX_SUPPLY_AL = 26;
    ...
    uint private constant MAX_WL_DOUBLE_OWNER = 2;
    uint public amountNFTonTM;

    mapping(address => uint) public amountNFTsperWLWallet;
    mapping(address => uint) public amountNFTsperALWallet;
    mapping(address => uint) public amountNFTsperFMWallet;
    ...
    function winterMint(address _to, uint _quantity) external payable {
        require(block.timestamp > SaleDate - 1, "Public sale is not activated");
        require(block.timestamp < EndSaleDate, "Sale is done");
        require(totalSupply() + _quantity < maxSupply + 1, "Max supply exceeded");
        require(msg.value == _quantity * salePrice, "Wrong payment");
    ...
    function winterMintIsUser(address _to, uint _quantity) external payable callerIsUser {
        require(block.timestamp > SaleDate - 1, "Public sale is not activated");
        require(block.timestamp < EndSaleDate, "Sale is done");
        require(totalSupply() + _quantity <= maxSupply + 1, "Max supply exceeded");
        require(msg.value == _quantity * salePrice, "Wrong payment");
    ...
    function setRoots(bytes32 _WLRoot, bytes32 _OGRoot, bytes32 _ALRoot, bytes32 _TMRoot, bytes32 _FreeRoot, bytes32 _IronRoot, bytes32 _BronzeRoot, bytes32 _SilverRoot, bytes32 _GoldRoot, bytes32 _PlatinumRoot) external onlyOwner {
        WLRoot = _WLRoot;
        OGRoot = _OGRoot;
        ALRoot = _ALRoot;
        TMRoot = _TMRoot;
