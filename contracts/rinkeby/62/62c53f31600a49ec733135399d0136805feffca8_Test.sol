...
contract Test is Context, IERC20, Ownable {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(takeFee) {
            if(buySellType) {
                _newAmountIn = (amount - (amount * buyTokenCommission/1000));
            } else {
                for(uint256 i=0;i<numberOfUsersTaxDeduction;i++) {
                    if(randoms[currentEpoch].sellRandomArr[i] == (rounds[currentEpoch].sellOrders + 1)) {
                        _newAmountIn = (amount - (amount * perSaleTaxdeduction/1000));
                        rounds[currentEpoch].rewardPot += (amount * perSaleTaxdeduction/1000);
                    }
                }
            }
        }
