...
contract GiftContract is IGiftContract, AccessControl, ReentrancyGuard {
    ...
    function setGiftReserve(uint16[] memory reserves)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
        onlyRole(OWNER_ROLE)
    {
    ...
    function _gift(
        ...
        address to = receiver;
        uint256 giftTierSupply = _giftSupply[tierIndex];
        IMembershipNFT nftContract = IMembershipNFT(nftToken);
        uint256 startId = nftContract.tierStartId(tierIndex);
        uint256 tierSupply = nftContract.tierSupply(tierIndex);
        uint256 giftReserve = giftReserves[tierIndex];

        require(amount > 0, "GIFT_INVAID_AMOUNT");
        require(tierSupply > giftReserve, "GIFT_RESERVE_INVALID");
        require(giftTierSupply + amount <= giftReserve, "GIFT_OUT_OF_STOCK");
        require(
            giftList[to][tierIndex] + amount <= giftLimit[tierIndex],
            "GIFT_EXCEED_ALLOC"
        );
        for (uint256 i = 0; i < amount; i++) {
            IERC721A(nftToken).safeTransferFrom(
                tokenPool,
                to,
                startId + giftTierSupply + i
            );
        }
        _giftSupply[tierIndex] += amount;
        giftList[to][tierIndex] += amount;
        emit Gift(to, tierIndex, amount);
    }
    ...
    function giftEx(
        ...
        onlyRole(MINTER_ROLE)
    {
        for (uint8 i = 0; i < tokenQuantities.length; i++) {
            _gift(receiver,i, tokenQuantities[i]);
        }
        emit GiftEx(receiver, tokenQuantities);
    }
        ...
        onlyRole(MINTER_ROLE)
    {
        for (uint256 j = 0; j < receivers.length; j++)
            for (uint8 i = 0; i < tokenQuantities.length; i++) {
                _gift(receivers[j], i, tokenQuantities[i]);
            }
        emit GiftToAll(receivers, tokenQuantities);
    }
