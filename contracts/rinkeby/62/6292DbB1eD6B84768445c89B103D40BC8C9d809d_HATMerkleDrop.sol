...
contract HATMerkleDrop {
  ...
  constructor(HATToken _hat, bytes32 _merkleRoot) {
    hat = _hat;
  ...
  function claim(address _to, uint256 _amount, bytes32[] calldata _proof) external {
    ...
    require(!hasClaimed[_to], "Account already claimed");
    require(hat.delegates(_to) != address(0), "Must delegate to claim");
...
// SPDX-License-Identifier: MIT
// Taken from https://github.com/hats-finance/hats-contracts/blob/develop/contracts/HATToken.sol
pragma solidity 0.8.6;
