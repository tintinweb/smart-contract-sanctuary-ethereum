...
/// @title A contract for TopWolves
/// @author Rob Park
/// @notice NFT Minting
contract TopWolves is ERC721, Ownable, ReentrancyGuard {
    ...
    // Team wallet
    address walletAddress = 0x1D36fcD710d5e663714849c756B82038e855cED5;
     ...
     * 1:   preSale - 24 hours (8 hours, 16 hours)
     * 2:   publicSale,
     * 3:   paused
     * 4:   not started
     */ 
    ...
    // Mint Price
    uint public mintPriceDiscount = 0.1 ether;
    ...
    uint8 private LIMIT1 = 1; // presale first 8 hours : can mint only 1
    uint8 private LIMIT3 = 3; // presale rest 16 hours : can mint only 3

    uint256 firstPresaleTime = 3600 * 8;
    uint256 secondPresaleTime = 3600 * 16;
/*
    uint256 firstPresaleTime = 60;
    uint256 secondPresaleTime = 120;
*/

    uint256 public presaleStartTime = block.timestamp;

    mapping (address => uint8) addressFirstPresaleCountMap;     // Up to 1
    mapping (address => uint8) addressSecondPresaleCountMap;    // Up to 3
    mapping (address => uint8) addressPublicSaleCountMap;       // Up to Unlimited
    constructor() ERC721("TopWolves", "TWOL") {
    ...
    function mintPresale(uint8 _mintCount, bytes32[] memory _proof) external payable nonReentrant returns (uint256) {
        ...
        require((       // Presale 1
                    ...
                    && (addressFirstPresaleCountMap[msg.sender] + _mintCount <= LIMIT1)
                ) || (  // Presale 2
                     (_mintCount <= LIMIT3)
                    && (block.timestamp > presaleStartTime + firstPresaleTime)
                    && (block.timestamp <= presaleStartTime + firstPresaleTime + secondPresaleTime)
                    && (addressSecondPresaleCountMap[msg.sender] + _mintCount <= LIMIT3)
                    && (msg.value == (mintPriceDiscount * _mintCount))
                ));
        ...
        if ((block.timestamp > presaleStartTime + firstPresaleTime) && (block.timestamp <= presaleStartTime + firstPresaleTime + secondPresaleTime))
            addressSecondPresaleCountMap[msg.sender] += _mintCount;
        else
            addressFirstPresaleCountMap[msg.sender] += _mintCount;
        ...
        emit Mint(msg.sender, 
                    ...
                    mintPrice,
                    _mintCount,
                    addressFirstPresaleCountMap[msg.sender],
                    addressSecondPresaleCountMap[msg.sender],
     ...
     * Presale
     * mintStep:    1
     * mintCount:   first 8 hours : 1, next 16 hours : 3
     */
    /*
    ...
    */
    ...
    function setMintStep(uint8 _mintStep) external onlyOwner returns (uint8) {
        ...
        mintStep = _mintStep;
        emit Setting(mintStep, mintPrice, mintPriceDiscount, totalLimit, LIMIT3, LIMIT1);
        return mintStep;
    }
    ...
    function getAccountStatus(address account) external view returns (uint8[] memory) {
        ...
        status[1] = addressSecondPresaleCountMap[msg.sender];
