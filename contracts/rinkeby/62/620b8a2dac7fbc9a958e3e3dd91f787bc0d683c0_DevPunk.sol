...
// File contracts/DevPunk.sol

//SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

contract DevPunk is ERC721, ERC721Enumerable, DevPunkADN {
    ...
    constructor(uint256 _maxSupply) ERC721("DevPunks", "DRP") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No hay mas DevPunks");
    ...
    //     returns (string memory)
    // {
    //     require(_exists(tokenId), "RC721 metadata no existe la URI");
    //     uint256 dna = tokenDNA[tokenId];
    //     string memory image = imagenByDNA(dna);
    //     string memory jsonURI = string(
    //         abi.encodePacked(
    //             '{"name": "DevPunks #',
    //             tokenId.toString(),
    //             '", "description":"Primer token loco de la vida", "image":',
    //             image,
    //             '"}'
    //         )
    //     );
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "DevPunk #',
                tokenId.toString(),
                '", "description": "DevPunk que cosa mas loca", "image": "',
                image,
                '"}'
            )
        );
    ...
    function imagenByDNA(uint256 _dna) public view returns (string memory) {
        string memory baseURI = _baseURI();
