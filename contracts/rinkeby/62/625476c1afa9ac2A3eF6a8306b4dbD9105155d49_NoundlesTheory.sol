...
contract NoundlesTheory is ERC721, Ownable {
  ...
  function _handleRainbowMinting(address _to, uint256 index) private {
    ...
    if (
      weaponsEnabled &&
      evilTokenId != 0 &&
      MigrationMapToMap[evilTokenId] != 0 &&
      BetaTheory.ownerOf(MigrationMapToMap[evilTokenId]) == ownerOf(evilTokenId)
    ) {
  ...
  function setSaleBlockandCSBandWep(
    uint256 _arg,
    bool _bool,
    bool _OCSB,
    bool _wep
    ...
    isRainbowMintingEnabled
  {
    ...
    require(
      _noundles + (mintCountCompanions + mintCountEvil + OG_TOTAL_SUPPLY) <=
        MAX_EVIL_NOUNDLES,
    ...
    isPhaseTwoStarted
  {
    ...
    require(
      (mintCountLandLow + mintCountLandMid + mintCountLandHigh) + __noundles <=
        MAX_FREE_LAND_MINTS,
    ...
    for (
      uint256 index;
      (index < holdingNoundles.length) && offset < _noundles;
      index += 1
    ) {
    ...
    returns (uint256[] memory)
  {
    ...
    for (
      uint256 i;
      (offset < __noundles) && i < localTotalSupply.current();
      i++
    ) {
  ...
  function setMintCost(uint256 _evilPrice, uint256 _landPrice)
    external
    onlyOwner
  {
  ...
  function setEvilPercentages(
    uint256 _evilPercent,
    uint256 _jailPercent,
    uint256 _stealPercent
  ) public onlyOwner {
  ...
  function setBaseMintPriceTiers(
    uint256 tier1,
    uint256 tier2,
    uint256 tier3
  ) public onlyOwner {
  ...
  function setContractMintBlock(bool _arg) public onlyOwner {
    overrideContractMintBlock = _arg;
  }
  ...
  function transferFrom(
    ...
    require(
      overrideCallbackSaleBlock || callbackSaleBlock[tokenId] == false,
