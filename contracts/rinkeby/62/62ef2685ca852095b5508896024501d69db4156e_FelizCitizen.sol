...
// File: contracts/FelizCitizens.sol
...
// Modifier 
  ...
  modifier isValidMerkleProof(bytes32[] storage merkleProof, bytes32 root) {
        require(
            MerkleProof.verify(
                merkleProof,
                root,
...
// Check presale validate
  function presaleValidations(
    ...
    uint256 _mintAmount,
    uint256 _supply
  ) internal isValidMerkleProof(whitelistProof, whitelistMerkleRoot) {
    uint256 actualCost;
    _supply <= 1100 
...
// SETTING parameter
  ...
  function setWhitelistMerkleProof(bytes32[] calldata _merkleProof) external onlyOwner {
        whitelistProof = _merkleProof;
  }
