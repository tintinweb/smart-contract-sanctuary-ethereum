 ...
 * [X] mint checks that the submitted Corner ids match the correct quadrants
 * [X] mint checks that msg.sender owns the Corners they want to merge (in their wallet)
 * [] A mint/merge of Corners burns them and emits an event with the info needed to make that Square
 *
 * # Hardhat (this contract)
 * [x] Deploy script written and depends on the v0/Corners contract.
 * [] Test script setup written (deploy a v0 contract, mint some NFTs to some accts)
 * [] Tests implemented ()
 *
 * # Website UI
 * [] Mergers need to call setApprovalForAll on the Corners contract with the
 *      Squares contract as the operator (to burn Corners during mint/merge)
 *
 * # Files, Metadata, Images
 * [] An "unrevealed Square" image needs to be created (by @nftsupply).
 * [] Pre-create 2500 JSON files w/ an "unrevealed Square" image for the default Square meta URIs.
 *
 * # Bots, Scripts, APIs
 * [] Some bot/script will watch for ethereum events to get the merge data
 *      (corner ids), and then make the Square image, publish it to ipfs,
 *      make a new JSON metadata file, publish that to IPFS as well,
 *      and update the tokenURI.
 * [] Better save all the images that get uploaded to IPFS perfectly, in case
 *      they get unpinned or something and a future run of the code generates a png
 *      with different bytes... too much code to check.
 // [] Watch for `URI(string, uint256 indexed)` to know when a Square has been revealed!
 *
 * # PFP Contract
 * [] IDEA!!! Hold the Corner NFTs until the tokenURI is written? Manual human
 *      approval of the generated image before burning the tokens and writing the
 *      URI. The human can edit the URI before approving. (This will cost us some
 *      gas but is worth it)
 */

contract SquareSupplyV1 is
Initializable,
ERC1155,
Ownable,
Pausable,
...
ERC1155URIStorage
{
    ...
    uint256 public maxSupply;
    //uint256 public squaresRemaining;
    Counters.Counter public squaresRemaining;

    string private baseURI;
    string private baseExtension;

    SquareSupplyV0 public ssCorners;
    // class vars }}}
    ...
    constructor(
        string memory _baseURI,
        SquareSupplyV0 _ssCorners
    ) ERC1155("ipfs://QmTz1aAoS6MXfHpGZpJ9YAGH5wrDsAteN8UHmkHMxVoNJk/") {
        name             = "Square Supply Squares";
        symbol           = "SQUARE";
        maxSupply        = 2500;

        squaresRemaining = Counters.Counter({_value: maxSupply});

        baseURI          = _baseURI;
        baseExtension    = ".json";

        ssCorners = _ssCorners;
    }
    // constructor }}}
    ...
    function setURI(uint256 _tokenId, string memory _tokenURI)
        ...
        onlyOwner
    {
        // TODO: watch for `URI(string value, uint256 indexed id)` to know when a Square has been revealed!
    ...
    function _requireEnoughSupply(uint256 _howMany) private view {
        ...
        console.log(squaresRemaining.current());
        require(_howMany <= squaresRemaining.current(), "fewer squares remain");
    }

    function _requireValidSquare(uint256 _bl, uint256 _tl, uint256 _tr, uint256 _br) private pure {
        require(_bl > 0    && _bl <= 2500,  "Invalid id: BL");
        require(_tl > 2500 && _tl <= 5000,  "Invalid id: TL");
        require(_tr > 5000 && _tr <= 7500,  "Invalid id: TR");
        require(_br > 7500 && _br <= 10000, "Invalid id: BR");
    }

    function _requireIdUnminted(uint256 _squareId) private view {
        require(totalSupply(_squareId) == 0, "id already minted");
    }

    function _requireOwnsCorners(address _account, uint256 _bl, uint256 _tl, uint256 _tr, uint256 _br) private view {
        address[] memory balAccounts = new address[](4);
        balAccounts[0] = _account;
        balAccounts[1] = _account;
        balAccounts[2] = _account;
        balAccounts[3] = _account;

        uint256[] memory balIds = new uint256[](4);
        balIds[0] = _bl;
        balIds[1] = _tl;
        balIds[2] = _tr;
        balIds[3] = _br;

        uint256[] memory balances = ssCorners.balanceOfBatch(balAccounts, balIds);
        require(balances[0] == 1, "Unowned id: BL");
        require(balances[1] == 1, "Unowned id: TL");
        require(balances[2] == 1, "Unowned id: TR");
        require(balances[3] == 1, "Unowned id: BR");
    }

    function _mintSquareSafe(uint256 _bl, uint256 _tl, uint256 _tr, uint256 _br) private {
        _requireValidSquare(_bl, _tl, _tr, _br);
        _requireOwnsCorners(msg.sender, _bl, _tl, _tr, _br);
        _requireEnoughSupply(1);

        uint256 newSquareId = mintedSquares() + 1;
        _requireIdUnminted(newSquareId);
        ...
        require(ssCorners.isApprovedForAll(msg.sender, address(this)), "approve v0 contract for v1");
        ssCorners.burnBatch(msg.sender, ids, amounts);

        emit Merge(msg.sender, uint16(newSquareId), uint16(_bl), uint16(_tl), uint16(_tr), uint16(_br));

        _mintSquare(newSquareId);
    }

    // Private mint single interface.
    function _mintSquare(uint256 _squareId) private {
        /*bytes memory mintData = abi.encodePacked(_bl, _tl, _tr, _br);*/
        /*_mint(msg.sender, newSquareId, 1, mintData);*/
        _mint(msg.sender, _squareId, 1, "0x00");
        squaresRemaining.decrement();
    }
    // PRIVATE mint logic }}}

    // PUBLIC mint logic {{{
    // The number of squares that have been minted so far.
    function mintedSquares() public view returns (uint256) {
        return maxSupply - squaresRemaining.current();
    }

    // Public mint single interface.
    function mintSquareFromCorners(uint256 _bl, uint256 _tl, uint256 _tr, uint256 _br) external {
        _mintSquareSafe(_bl, _tl, _tr, _br);
    }
    // PUBLIC mint logic }}}
}
