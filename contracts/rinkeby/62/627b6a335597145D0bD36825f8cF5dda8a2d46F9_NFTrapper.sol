...
contract NFTrapper is ERC721Enumerable, Ownable, ReentrancyGuard, PaymentSplitter {
  uint256 private constant MAX_SUPPLY = 600;
  bool private _isPublicSaleActive = false;
  string private _baseTokenURI;
  uint256 private PRICE = 0.35 ether;

  mapping(uint256 => uint256) public hhhNFTClaimed;
  bool public communityGrant = true;
  address internal _hhhAddress;
  ...
  ) ERC721(name, symbol) PaymentSplitter(payees,shares_) {
    _baseTokenURI = baseTokenURI;
    _hhhAddress = hhhAddress;
  }
  ...
  function minthWithHHHNFT(
    uint256 _hhhNFTokenID
  ) external nonReentrant() {
    require(communityGrant);
    require(
      _hhhNFTokenID >= 0 && _hhhNFTokenID < 10000,
      "Invalid HHH NFT ID"
    );
    require(
      hhhNFTClaimed[_hhhNFTokenID] == 0,
      "Already minted with this HHH NFT"
    );
    require(
      IERC721(_hhhAddress).ownerOf(_hhhNFTokenID) == msg.sender,
      "Not the owner of this HHH NFT"
    );
    require(
      totalSupply() + 1 <= MAX_SUPPLY,
      "Mint would exceed max supply."
    );
    hhhNFTClaimed[_hhhNFTokenID] = 1;
  ...
  function setHHHAddress(address hhhAddress) external onlyOwner {
    _hhhAddress = hhhAddress;
  }
