...
contract MyLeadStake is Owned {
    ...
    function stake(uint _amount) external onlyRegistered() whenActive() {
        ...
        uint remainder = (block.timestamp.sub(lastClock[msg.sender])).mod(60);
    ...
    //register address if onlyOwner
    function register(address newOwner) public onlyOwner {
        registered[newOwner] = true;
    }
}