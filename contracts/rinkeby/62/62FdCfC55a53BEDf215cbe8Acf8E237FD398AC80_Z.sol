...
contract Z is ERC721Omnibus, Royalties, Signable, Withdrawable, IERC2309 {    
    ...
    function getZValue(uint256 tokenId) public view returns (uint16) {        
        if(!exists(tokenId)) {
            return 0;
        } else {
            uint256 bin = 0;
    ...
    function mint(uint256[] calldata zValues) external {
        _requireOnlyValidSender();

        uint256 numValues = zValues.length;
        ...
        if(index % BITS_PER_VALUE != 0) {
           packedWord = packedZValues[packedZValues.length - 1];
           packedZValues.pop();
        }

        while(valueIx < numValues) {
           uint256 value = zValues[valueIx];
           require(value > 0 && value <= MAX_UINT_16, "Requires z of 1 to 65535");

           uint256 mod16 = index % BITS_PER_VALUE;
           if(mod16 == 0 && packedWord > 0) {
               packedZValues.push(packedWord);
               packedWord = 0;
           }
