...
contract MintableERC721 is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        _grantRole(UPGRADER_ROLE, operator_);
        ...
        payable
    {
        ...
        string memory message = string(abi.encodePacked(uri_, StringsUpgradeable.toString(id_), StringsUpgradeable.toString(price_)));
        address recovered = verify(getEthSignedHash(message), signature_);
        require(recovered == operator, "Signature mismatch");

        (bool sent, /* bytes memory data */) = owner.call{value: msg.value.div(100).mul(80)}("");
        require(sent, "Failed to send Ether");
