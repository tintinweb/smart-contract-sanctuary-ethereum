...
/// @title: Gucci Auction Collection
/// @author: niftykit.com
    ...
    BaseCollection
{
    ...
    function setAuction(
        ...
        uint256 reservePrice_,
        uint256 duration_
    ) external hasAuction(auctionId) onlyOwner {
        require(_auctions[auctionId].startedAt == 0, "Auction already started");

        _auctions[auctionId].tokenURI = tokenURI_;
        _auctions[auctionId].creator = creator_;
        _auctions[auctionId].reservePrice = reservePrice_;
        _auctions[auctionId].duration = duration_;
    }

    function setAuctionActive(uint256 auctionId, bool active_)
        external
        hasAuction(auctionId)
        onlyOwner
    {
        _auctions[auctionId].active = active_;
    }
    ...
    function placeBid(uint256 auctionId, uint256 amount)
        ...
        nonReentrant
    {
        require(_auctions[auctionId].active, "Auction not active");
        require(
            amount >= _auctions[auctionId].reservePrice,
            "Lower than reserve price"
        );
        require(
            _auctions[auctionId].startedAt == 0 ||
                block.timestamp <
                _auctions[auctionId].startedAt.add(
        ...
        require(
            amount >=
                _auctions[auctionId].amount.add(
                    _auctions[auctionId].amount.mul(minBidPercentage).div(100)
                ),
            "Lower than minimum bid amount"
        );

        // Start the auction when we receive the first bid
        if (_auctions[auctionId].startedAt == 0) {
            _auctions[auctionId].startedAt = block.timestamp;
        }

        // Return the previous bid if there is any
        if (_auctions[auctionId].bidder != address(0)) {
            Address.sendValue(
                payable(address(_auctions[auctionId].bidder)),
        ...
        if (
            _auctions[auctionId]
                .startedAt
                .add(_auctions[auctionId].duration)
                .sub(block.timestamp) < timeBuffer
        ) {
            uint256 prevDuration = _auctions[auctionId].duration;
            _auctions[auctionId].duration = prevDuration.add(
                timeBuffer.sub(
                    _auctions[auctionId].startedAt.add(prevDuration).sub(
                        block.timestamp
                    )
                )
            );
        ...
        emit AuctionBidPlaced(auctionId, _msgSender(), amount);
    }
    ...
    {
        require(_auctions[auctionId].active, "Auction not active");
        require(_auctions[auctionId].startedAt != 0, "Auction hasn't started");
        require(
            block.timestamp >=
                _auctions[auctionId].startedAt.add(
                    _auctions[auctionId].duration
        ...
        _safeMint(_auctions[auctionId].bidder, auctionId);
        _setTokenURI(auctionId, _auctions[auctionId].tokenURI);

        _niftyKit.addFees(_auctions[auctionId].amount);
        uint256 fees = _niftyKit.getFees(address(this));
        _niftyKit.addFeesClaimed(fees);

        Address.sendValue(payable(address(_niftyKit)), fees);
        Address.sendValue(
            payable(_auctions[auctionId].creator),
            _auctions[auctionId].amount - fees
        );
...
abstract contract BaseCollection is IAuctionCollection, Ownable {
