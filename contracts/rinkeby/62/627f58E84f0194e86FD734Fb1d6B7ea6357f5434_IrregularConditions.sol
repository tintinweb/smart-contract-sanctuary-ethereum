...
contract IrregularConditions is ERC721A, IERC2981, Ownable {
    ...
    event artMinted(address sentTo, uint256 amount, uint256 startIndex);
    ...
    function _mintArt( uint256 numArt, address mintTo, bool requirePayment) private {
        require(amountLeft() >= numArt, "Sold out.");
        
        if (requirePayment) {
            require( msg.value == numArt * mintPrice, "ETH amount not correct."
            );
        }

        uint256 startIndex = currentIndex;
        _safeMint(mintTo, numArt);
        emit artMinted(mintTo, numArt, startIndex);
    }
