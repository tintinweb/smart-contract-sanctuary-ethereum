...
contract GwPayment is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public depositTotalAmount;
    address public cSigner =
        address(0xec3338e874c12A2188e9F0Fa3c1350cFca8cF9fd);
    ...
    function deposit(uint256 amount) external onlyEOA {
        ...
        if (amount > 0) {
            ...
            depositTotalAmount[msg.sender] += amount;
        ...
        emit Deposit(msg.sender, ts, amount, orderID);
    }
    ...
    function withdraw(
        ...
        address user,
        uint256 amount
    ) external nonReentrant onlyEOA onlyOwner {
        ...
        if (amount > 0) {
            token.safeTransfer(address(user), amount);
        }
        emit Withdraw(user, amount);
    }

    function getDepositedAmount(address user) external view returns (uint256) {
        return depositTotalAmount[user];
    }

    function getDepositeList(address user)
        external
        view
    ...
    function getMessageHash(uint256 amount, address receiver)
        public
        view
        returns (bytes32)
    {
