...
/**
 * Mechancics of how a user can stake agency.
 */
contract AgencyStaking is Ownable {
    using ABDKMath64x64 for int128;
    
    AgencyERC20Token immutable public AGENCY; // The token being staked, for which AGENCY rewards are generated
    OGAgency immutable public OG_AGENCY; // Token used to redeem staked AGENCY
    ...
    constructor(
        ...
        // Each version of the staking contract needs it's own instance of OGAgency users can use to
        // claim back rewards
        OG_AGENCY = new OGAgency(); 
    ...
    /** Balance in AGENCY for a given amount of OG_AGENCY */
    function balance(uint256 amountOgAgency) public view returns(uint256) {
        return _overflowSafeMul1e18(
            ABDKMath64x64.divu(amountOgAgency, 1e18).mul(_accumulationFactorAt(currentEpoch()))
        );
    }
    ...
    function stakeFor(address _staker, uint256 _amountAgency) public returns(uint256 amountOgAgency) {
        require(_amountAgency > 0, "Cannot stake 0 tokens");

        _updateAccumulationFactor();

        // net past value/genesis value/OG Value for the agency you are putting in.
        amountOgAgency = _overflowSafeMul1e18(ABDKMath64x64.divu(_amountAgency, 1e18).div(accumulationFactor));

        SafeERC20.safeTransferFrom(AGENCY, msg.sender, address(this), _amountAgency);
        OG_AGENCY.mint(_staker, amountOgAgency);
        emit StakeCompleted(_staker, _amountAgency, 0);

        return amountOgAgency;
    }

    /** Stake agency */
    function stake(uint256 _amountAgency) external returns(uint256 amountOgAgency) {
        return stakeFor(msg.sender, _amountAgency);
    }

    /** Unstake agency */
    function unstake(uint256 _amountOgAgency) external {      
        require(OG_AGENCY.allowance(msg.sender, address(this)) >= _amountOgAgency, 'Insufficient OGAgency allowance. Cannot unstake');

        _updateAccumulationFactor();
        uint256 unstakeBalanceAgency = balance(_amountOgAgency);

        OG_AGENCY.burnFrom(msg.sender, _amountOgAgency);
        SafeERC20.safeIncreaseAllowance(AGENCY, address(EXIT_QUEUE), unstakeBalanceAgency);
        EXIT_QUEUE.join(msg.sender, unstakeBalanceAgency);

        emit UnstakeCompleted(msg.sender, _amountOgAgency);    
    }
...
contract AgencyERC20Token is ERC20, ERC20Burnable, Ownable, AccessControl {
    ...
    constructor() ERC20("Agency", "AGENCY") {
        _setupRole(DEFAULT_ADMIN_ROLE, 0xA8B1405e6d793B80F85836683b09786A8D16C7D1);
    }
 ...
 *
 * It mints and burns OGAgency as users stake/unstake
 */
contract OGAgency is ERC20, ERC20Burnable, Ownable {
    constructor() ERC20("OGAgency", "OG_AGENCY") {}
...
/**
 * How all exit of AGENCY rewards are managed.
 */
contract ExitQueue is Ownable {
    ...
    IERC20 public AGENCY;   // AGENCY

    // Limit of how much agency can exit per epoch
    uint256 public maxPerEpoch;

    // Limit of how much agency can exit per address per epoch
    ...
    function withdraw(uint256 epoch) external {
        ...
        SafeERC20.safeTransfer(AGENCY, msg.sender, amount);
