...
contract Vendor is Ownable {
    ...
    function withdraw() public onlyOwner {
        ...
        require(vendorBalance > 0, "Vendor does not have any ETH");
    ...
    function sellTokens(uint256 _amount) public payable {
        ...
        bool sent = yourToken.transferFrom(user, address(this), _amount);
        require(sent, "Failed to transfer tokens");

        (bool ethSent, ) = user.call{value: amountOfEth}("");
        require(ethSent, "Failed to send back eth");

        emit SellTokens(user, amountOfEth, _amount);
    }
}
