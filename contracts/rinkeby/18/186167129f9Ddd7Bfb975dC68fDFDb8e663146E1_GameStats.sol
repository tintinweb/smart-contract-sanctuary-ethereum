...
contract GameStats is Ownable, VRFConsumerBase, Pausable {
    ...
    uint256 redTotalLevel;
    uint256 blueTotalLevel;
    ...
    uint256 public TREE_HOUSE_STAKE = 1;
    uint256 public HOUSE_STAKE = 2;
    ...
    function setTokensData(
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            ...
            bytes32 tokenKey = _getTokenKey(collection_, tokenIds_[i]);
    ...
    function setStakedTokenData(
        ...
        uint256 blueLevelToAdd;
        uint256 redLevelToAdd;
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            bytes32 tokenKey = _getTokenKey(collection_, tokenIds_[i]);
            TokenData storage tokenData = tokenDataEncoded[tokenKey];
            ...
            if (!isMovingStance) {
                ...
                if (tokenData.isElite) {
                    if (tokenData.faction) {
                        ...
                        blueLevelToAdd += tokenData.level;
                    } else {
                        redElites.add(tokenKey);
                        redLevelToAdd += tokenData.level;
                    }
                }
            }
        ...
        if (!isMovingStance) {
            blueTotalLevel += blueLevelToAdd;
            redTotalLevel += redLevelToAdd;
        }

        delete blueLevelToAdd;
        delete redLevelToAdd;
    }
    ...
    function unsetStakedTokenData(
        ...
        uint256 blueLevelToSubtract;
        uint256 redLevelToSubtract;
            ...
            if(!isMovingStance) {
                ...
                if (tokenData.isElite) {
                    if (tokenData.faction) {
                        ...
                        blueLevelToSubtract += tokenData.level;
                    } else {
                        redElites.remove(tokenKey);
                        redLevelToSubtract += tokenData.level;
                    }
                }
            }
        ...
        if (!isMovingStance) {
            blueTotalLevel -= blueLevelToSubtract;
            redTotalLevel -= redLevelToSubtract;
        }

        delete blueLevelToSubtract;
        delete redLevelToSubtract;

    }


    function _getTokenKey(address collection_, uint256 tokenId_) internal pure returns (bytes32) {
    ...
    function getLevel(address collection_, uint256 tokenId_) public view returns (uint256) {
        return tokenDataEncoded[_getTokenKey(collection_, tokenId_)].level;
    }

    function getLevels(address collection_, uint256[] calldata tokenIds_) external view returns (uint256[] memory) {
    ...
    function getLevelBoosts(address collection_, uint256[] calldata tokenIds_) external view returns (uint256[] memory) {
        uint256[] memory levelBoosts = new uint256[](tokenIds_.length);

        for (uint256 i = 0; i < tokenIds_.length; ++i) {
            for (uint256 j = levelMilestones.length - 1; j >= 0; --j) {
                if (tokenDataEncoded[_getTokenKey(collection_, tokenIds_[i])].level >= levelMilestones[j]) {
    ...
    function getLevelBoost(address collection, uint256 tokenId) external view returns (uint256) {
        uint256 levelBoost;
        for (uint256 j = levelMilestones.length - 1; j >= 0; --j) {
            if (tokenDataEncoded[_getTokenKey(collection, tokenId)].level >= levelMilestones[j]) {
    ...
    function claimLevel(TokenSelection[] calldata tokensSelection_) public {
        for (uint256 i = 0; i < tokensSelection_.length; ++i) {
            for (uint256 j = 0; j < tokensSelection_[i].tokens.length; ++j) {
                require(
                    tokenDataEncoded[_getTokenKey(tokensSelection_[i].collectionAddress, tokensSelection_[i].tokens[j])].owner == msg.sender
                );
                if (
                    tokenDataEncoded[_getTokenKey(tokensSelection_[i].collectionAddress, tokensSelection_[i].tokens[j])].stakeType == TREE_HOUSE_STAKE
                    || tokenDataEncoded[_getTokenKey(tokensSelection_[i].collectionAddress, tokensSelection_[i].tokens[j])].stakeType == HOUSE_STAKE
                ) {
    ...
    function _claimLevelForToken(address collection_, uint256 tokenId_) internal {
        if (tokenDataEncoded[_getTokenKey(collection_, tokenId_)].level != LEVEL_CAP) {
            require(tokenDataEncoded[_getTokenKey(collection_, tokenId_)].stakeType != 0);
            uint256 levelYield = (block.timestamp - tokenDataEncoded[_getTokenKey(collection_, tokenId_)].levelEnrolDate) /
            (stakeTypeImpacts[tokenDataEncoded[_getTokenKey(collection_, tokenId_)].stakeType].boost * 1 minutes);


            if (tokenDataEncoded[_getTokenKey(collection_, tokenId_)].level + levelYield > LEVEL_CAP) {
                tokenDataEncoded[_getTokenKey(collection_, tokenId_)].level = LEVEL_CAP;
            } else {
                tokenDataEncoded[_getTokenKey(collection_, tokenId_)].level += levelYield;
            }

            delete levelYield;

            tokenDataEncoded[_getTokenKey(collection_, tokenId_)].levelEnrolDate = block.timestamp;
        }
    }
    ...
    function isUnStakeClaimSuccessful(
        ...
        uint256 risk = BASE_RISK * 100;

        for (uint256 j = levelMilestones.length - 1; j >= 0; --j) {
            if (tokenDataEncoded[_getTokenKey(collection_, tokenId)].level >= levelMilestones[j]) {
        ...
        if (didLose) {
            address winner = pickWinnerFromElites(!tokenDataEncoded[_getTokenKey(collection_, tokenId)].faction);
            if (winner != address(0)) {
    ...
    function _didLoseClaimAmount(uint256 risk_, uint256 tokenId_, uint256 amount_) internal view returns (bool) {
        return uint256(keccak256(abi.encodePacked(seed, tokenId_, amount_, tx.origin, blockhash(block.number - 1), block.timestamp))) % 100 < risk_;
    }

    function pickWinnerFromElites(bool faction_) public view returns (address) {
        if (faction_) {
            return _pickWinnerFromElitesBySide(blueElites, blueTotalLevel);
        } else {
            return _pickWinnerFromElitesBySide(redElites, redTotalLevel);
        }
    }
    ...
    function _pickWinnerFromElitesBySide(
        ...
        uint256 random = totalLevel_ - _getRandom(totalLevel_);
        for (uint256 i = 0; i < elites_.length(); i++) {
            if (random < tokenDataEncoded[elites_.at(i)].level) {
                return tokenDataEncoded[elites_.at(i)].owner;
            }
            random -= tokenDataEncoded[elites_.at(i)].level;
        }
    ...
    function _getRandom(uint256 totalLevel_) internal view returns (uint256) {
        return uint256(
            keccak256(
                abi.encodePacked(
                    seed,
    ...
    function upgradeHouseSize(uint256 tokenId_, uint256 upgrade_) external {
        ...
        require(tokenDataEncoded[_getTokenKey(contractsAddressesMap["treeHouse"], tokenId_)].level + upgrade_ <= HOUSE_CAP);
        require(tokenDataEncoded[_getTokenKey(contractsAddressesMap["treeHouse"], tokenId_)].owner == msg.sender);
        ...
        tokenDataEncoded[_getTokenKey(contractsAddressesMap["treeHouse"], tokenId_)].level += upgrade_;
    }

    function addLevel(address collection_, uint256 tokenId_, uint256 levelIncrease_) external authorised {
        tokenDataEncoded[_getTokenKey(collection_, tokenId_)].level += levelIncrease_;
    }
    ...
    function setEliteStatus(address collection_, uint256 tokenId_) external authorised {
        ...
        bytes32 tokenKey = _getTokenKey(collection_, tokenId_);
    ...
    function setHousesLevels(uint256[] calldata tokenIds_, uint256[] calldata levels_) external authorised {
        for (uint256 i = 0; i < tokenIds_.length; ++i) {
            tokenDataEncoded[_getTokenKey(contractsAddressesMap["treeHouse"], tokenIds_[i])].level = levels_[i];
        }
    }
