...
contract gamers is ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI) ERC721("pen island", "bbc") {
        setBaseURI(baseURI);
    ...
    function setPerWallet(uint newPerWallet) external onlyOwner {
        perWallet = newPerWallet;
    }

    function setWhitelistSale(bool status) external onlyOwner {
        whitelistSale = status;
    }

    function setReserveListSale(bool status) external onlyOwner {
        reserveListSale = status;
    }
    ...
    function whitelistMint(uint numTokens) public payable {
        ...
        require(numTokens == perWallet, "cant mint that many");
        require(price.mul(numTokens) == msg.value, "incorrect ether value");
        uint tokensMinted = _tokenIds.current();
        require(tokensMinted.add(numTokens) - 1 <= supply, "exceeds supply");
    ...
    function reserveListMint(uint numTokens) public payable {
