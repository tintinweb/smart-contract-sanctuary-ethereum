...
contract Vendor is Ownable {
  ...
  function withdraw(address payable _to) public payable onlyOwner {
    //(bool sent, bytes memory data) = _to.call{value: ETHbalances[_to]}("");
  ...
  //function sellTokens(uint256 theAmount, address payable _to) public {
  function sellTokens(uint256 theAmount ) public {
    yourToken.transferFrom(msg.sender, address(this), theAmount);
    address payable _to = payable(msg.sender);
    uint256 ethAmount = theAmount / tokensPerEth;
    bool sent = _to.send( ethAmount );

    require(sent, "Failed to send Ether");
    emit SellTokens(_to, ethAmount, theAmount);
  }
