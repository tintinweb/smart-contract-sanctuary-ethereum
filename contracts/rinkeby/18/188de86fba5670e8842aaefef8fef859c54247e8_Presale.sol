...
contract Presale is AccessControl, Initializable, OwnableUpgradeable {
    ...
    event AddedToReferralList(string indexed referralCode);
    event RemovedFromReferralList(string indexed referralCode);
    event RemoveReferralInfoLayer(uint256 discountLayer);
    ...
    function _buyWithHYFIToken(
        ...
        (
            ...
            HYFItokenPayment
        ) = mixedTokenPaymentCalculator(token, unitAmount, discount, referralCode);
        tokenInfo[token].tokenAddress.transferFrom(
            msg.sender,
            collectorWallet,
            stableCoinPaymentAmount
    ...
    function _buyWithCurrency(uint256 unitAmount, uint256 discount, string memory referralCode) internal {
        uint256 priceTotal = currencyPaymentCalculator(unitAmount, discount, referralCode);
        require(
    ...
    function addToReferralList(uint256 referralDiscount, string memory referralCode) public onlyOwner {
        _referralInfo[referralDiscount].push(referralCode);
        _referralDiscountAmount[referralCode] = referralDiscount;
        emit AddedToReferralList(referralCode);
    }

    function addMultipleToReferralList(uint256 referralDiscount, string[] memory referralCode)
        external
        onlyOwner
    {
        for (uint256 code = 0; code < referralCode.length; code++) {
            addToReferralList(referralDiscount, referralCode[code]);
        }
    }
    function removeFromReferralList(string memory referralCode) public onlyOwner {
        delete _referralDiscountAmount[referralCode];
        emit RemovedFromReferralList(referralCode);
    }

    function removeReferralInfoLayer(uint256 referralDiscount) external onlyOwner {
        for (uint256 code = 0; code < _referralInfo[referralDiscount].length; code++) {
            removeFromReferralList(_referralInfo[referralDiscount][code]);
        }
        delete _referralInfo[referralDiscount];
        emit RemoveReferralInfoLayer(referralDiscount);
    }
    ...
    function currencyPaymentCalculator(uint256 unitAmount, uint256 discount, string memory referralCode)
        public
        view
        returns (uint256 paymentAmount)
    {
        uint256 priceTotal = ((unitAmount * unitPrice * 10**8) *
            tokenInfo["ETH"].decimals) / 181988240000;
        priceTotal -= discountAmountCalculator(discount, priceTotal);
        if(_referralDiscountAmount[referralCode] != 0){
            priceTotal -= ((unitAmount * _referralDiscountAmount[referralCode] * 10**8) *
            tokenInfo["ETH"].decimals) / 181988240000;
        }
        return priceTotal;
    }
    ...
    function simpleTokenPaymentCalculator(
        ...
        priceTotal -= discountAmountCalculator(discount, priceTotal);
        if(_referralDiscountAmount[referralCode] != 0){
            priceTotal -= _referralDiscountAmount[referralCode] * tokenInfo[token].decimals;
        }
        return priceTotal;
    }
        ...
        returns (uint256 stableCoinPaymentAmount, uint256 HYFIPaymentAmount)
    {
        ...
        if(_referralDiscountAmount[referralCode] != 0){
            baseTokenPayment -= _referralDiscountAmount[referralCode] * tokenInfo[token].decimals;
        }
