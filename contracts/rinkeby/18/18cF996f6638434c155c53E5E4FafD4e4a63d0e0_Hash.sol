...
contract Hash is ERC721, Ownable {
    ...
    bytes private _password = "temp";

    Counters.Counter private _tokenIdCounter;
    
    uint public maxSupply = 9999;
    uint public tokenPrice = 0.0 ether;
    ...
    //@dev Owner Mint
    //@param _receiver Address To Recieve The Token
    ...
    //@dev Function that is used to extend/renew a tokens expiry date.
    ...
    //@dev Owner function that is used to extend/renew a tokens expiry date.
    //@param _tokenId The token ID to extend/renew.
    ...
    //@param _tokenId The token ID to extend/renew.
    function ownerRenewTokenLong(uint[] calldata _tokenIds) external onlyOwner {
        ...
        for (uint i=0; i<_tokenIds.length; i++) {
            ...
            if (block.timestamp > _currentexpirationTimes) {
                expirationTimes[_tokenIds[i]] = block.timestamp + 365 days;
            } else {
                expirationTimes[_tokenIds[i]] += 365 days;
            }
        }
    }

    //@dev Update Price
    //@param _updatedTokenPrice Example: 200000000000000000 is 0.2 Ether.
    ...
    //@dev Add Tokens to Max Supply; Is the stock of restocks.
    function addTokens(uint _newTokens) external onlyOwner {
        maxSupply += _newTokens;
    }

    //@dev Remove Tokens to Max Supply.
