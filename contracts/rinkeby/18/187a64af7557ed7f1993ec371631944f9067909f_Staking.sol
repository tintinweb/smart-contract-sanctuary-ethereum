...
contract Staking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, WhitelistChecker, ERC721EnumerableUpgradeable, IERC721ReceiverUpgradeable {
	...
	function _calculateStakeReward(uint256 _bagId) internal view returns (uint256) {
		...
		if(gen2TokenLen <= 1) {
			...
			total = (baseEarning * genesisRateMultiplier * period) * doubleRewardNumerator / doubleRewardDenominator;
		}
		else if(gen2TokenLen == 3) {
			total = (baseEarning * genesisRateMultiplier * period) * tripleRewardNumerator / tripleRewardDenominator;
		} 
	...
	function getGen2TokenIdsInBag(uint256 _bagId) public view returns (uint256[] memory){
		uint256 bagId = _bagId;
		//uint256[] memory noGen2;
		//noGen2 = [9999];
		//if (bags[bagId].gen2TokenIds.length > 0){
		uint256[] memory tokens = bags[bagId].gen2TokenIds;
		return tokens;
		//}
		//else{
		//	return noGen2;
	...
	function burnBag(uint256 bagId) external onlyOwner {
		require(bags[bagId].genTokenId == 0, "bag has genesis");
		require(bags[bagId].gen2TokenIds.length == 0, "bag has gen2");
		_burn(bagId);
		delete bags[bagId];
	}
	...
	function getUnclaimedBalance(uint256 bagId) public view returns (uint256) {
		uint256 amount = _calculateStakeReward(bagId);
		return amount;
	}

	function deleteBagInfo(uint256 bagId) external onlyOwner {
		delete bags[bagId];
	}
