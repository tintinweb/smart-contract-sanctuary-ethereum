...
contract MyFirstNFT is ERC721A, Ownable {
    ...
    uint128 public constant MAX_MINT_PER_TX = 404;
    uint256 mintPrice = 0.01 ether;

    constructor(string memory uri) ERC721A("NimNFT", "MNFT") {
        BASE_URI = uri;
    }

    function mint(uint8 quantity) public payable {
        require(quantity <= MAX_MINT_PER_TX, "User surpassed the maximal quantity possible");
        require(totalSupply() + quantity <= MAX_SUPPLY);
        require(mintPrice * quantity == msg.value);
