...
interface SellaTokenInterface {
    ...
    function approvePayment(address ownerAddress, address spender, uint amount) external returns (bool);
}
...
contract CompanyFee is Ownable {
    ...
    function splitPayment(address agentAddress, uint256 amount) public returns (bool) {
        ...
        uint256 payCompany = SafeMath.div(SafeMath.mul(amount, companyFee), 100);
    ...
    function approveClientPayment(uint256 amount) public returns (bool) {
        return SellaTokenContract.approvePayment(msg.sender, address(this), amount);
    }

    function getClientAllowance(address owner, address spender) public returns (uint256){
    ...
    function getCompanyAgents(address companyAddress) public view returns (address [] memory) {
        return companyAgents[companyAddress];
    }

    function getAgentCompany(address agentAddress) public view returns (address) {
