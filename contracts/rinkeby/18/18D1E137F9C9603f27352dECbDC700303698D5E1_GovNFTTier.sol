...
contract GovNFTTier is IGovNFTTier, OwnableUpgradeable, SuperAdminControl {
        ...
        returns (NFTTierData memory nftTierData)
    {
        ...
        if (nftTierLevels[nftTierLevelsKeys[0]].isTraditional) {
            maxLTVFromNFTTier = IGovTier(govTier)
                .getSingleTierData(
            ...
            maxLTVFromNFTTier = spTierLevels[
                nftTierLevels[nftTierLevelsKeys[0]].spTierId
            ].ltv;
            maxNFTTierAddress = nftTierLevelsKeys[0];
        }
        ...
        for (uint256 i = 1; i < nftTiersLength; i++) {
            ...
            if (currentLoanToValue >= maxLTVFromNFTTier && IERC721(nftTierLevelsKeys[i]).balanceOf(_wallet) > 0) {
