    ...
    ClearingHouseStorageV1
{
    ...
    function initialize(
        ...
        // liquidityProvider is not contract
        require(liquidityProviderArg.isContract(), "CH_OBNC");
        ...
        returns (RemoveLiquidityResponse memory)
    {
        ...
        int256 takerOpenNotional = IPositionMgmt(_positionMgmt).getTakerOpenNotional(trader, params.baseToken);
        uint256 markPriceAfter = IMarketTaker(_marketTaker).getTwapMarkPrice(params.baseToken, 0);

        emit PositionChanged(
            trader,
            params.baseToken,
            response.takerBase, // base
            response.takerQuote, // quote
            0,
    ...
    function liquidate(address trader, address baseToken) external override whenNotPaused nonReentrant {
        ...
        require(!IPositionMgmt(_positionMgmt).hasOrder(trader), "CH_CLWTISO");
        ...
        IMarketTaker.SwapResponse memory response = _closePosition(
            InternalClosePositionParams({ trader: trader, baseToken: baseToken, isLiquidation: true })
        );
    ...
    function cancelOpenOrder(address maker, address baseToken) external override whenNotPaused nonReentrant {
        ...
        int256 takerOpenNotional = IPositionMgmt(_positionMgmt).getTakerOpenNotional(maker, baseToken);
        uint256 markPriceAfter = IMarketTaker(_marketTaker).getTwapMarkPrice(baseToken, 0);
        emit PositionChanged(
            maker,
            baseToken,
            response.takerBase, // base
        ...
        returns (IMarketTaker.SwapResponse memory)
    {
        ...
        bool isExactInput = positionSize > 0;

        uint256 amountLimit = IMarketTaker(_marketTaker).getEstimateAmountLimit(
            params.baseToken,
            positionSize.abs(),
            true,
            isExactInput
...
interface IMarketTaker {
    ...
    function getEstimateAmountLimit(
        address baseToken,
        uint256 amount,
        bool isBaseToQuote,
        bool isExactInput
...
interface IClearingHouse {
    ...
    /// @param isExactInput (true: sell, false: buy)
    // Open Long : isBaseToQuote = true, isExactInput = false || isBaseToQuote = false, isExactInput = true
    // Open Short: isBaseToQuote = true, isExactInput = true  || isBaseToQuote = false, isExactInput = true
    // Close Long : isBaseToQuote = true, isExactInput = true
    // Close Short: isBaseToQuote = true, isExactInput = false
    /// @param amountLimit
    // isExactInput : the price cannot be less than this value after the swap
    // !isExactInput: the price cannot be greater than this value after the swap
