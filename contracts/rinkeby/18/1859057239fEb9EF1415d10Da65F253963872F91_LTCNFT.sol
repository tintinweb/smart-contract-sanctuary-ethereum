...
abstract contract ERC721 is IERC165, IERC721, IERC721Metadata, IERC1155Metadata_URI,  IERC721Enumerable{
    ...
    // All Enumeration
    uint[] _allTokens;
    mapping(uint => uint) _allTokensIndex; //mapping token to tokenId
    function _addTokenToAllEnumeration(uint tokenId) private{
        _allTokens.push(tokenId);
        _allTokensIndex[tokenId] = _allTokens.length - 1;
    }

    function _removeTokenToAllEnumeration(uint tokenId) private{
    ...
    mapping(uint => uint) _ownedTokenIndex; // mapping tokekId to index
...
contract LTCNFT is ERC721 {
    constructor() ERC721("LTCNFT Collectibles", "LTCNFT"){}
