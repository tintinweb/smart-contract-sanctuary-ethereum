...
contract DLS is ERC721, Ownable {
    string internal baseTokenURI =
        "https://gateway.pinata.cloud/ipfs/Qmc2rHWFDvf5ezTDVqdWXGtTWFQxciDumBwGBuiB6ekmL3/";
     ...
     */
    function calculatePrice(uint256 qty) internal view returns (uint256) {
        uint256 totalPrice = 0;
        for (uint256 i = 1; i <= qty; i++) {
            uint256 tokenId = nonce + i;
            if (tokenId <= presaleSupply) {
                totalPrice += presalePrice;
            } else {
                totalPrice +=
                    presalePrice +
                    (tokenId - presaleSupply) *
                    increasePrice;
            }
        }
        return totalPrice;
     ...
     */
    function doMint(uint256 qty) internal {
     ...
     */
    function allowlistPresale(uint256 qty) external payable {
        require(allowlistPresaleActive, "TRANSACTION: presale is not active");
        uint256 qtyAllowed = presaleWallets[msg.sender];
        require(
            qty <= qtyAllowed && qty >= 1,
            "TRANSACTION: you can't mint on presale"
        );
        require(
            qty + nonce <= presaleSupply,
            "SUPPLY: value exceeds presale supply"
        );
