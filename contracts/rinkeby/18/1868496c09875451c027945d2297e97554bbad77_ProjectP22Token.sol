...
contract ProjectP22Token is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC721AQueryableUpgradeable, VRFConsumerBaseV2 {
    ...
    function setProvenance(string memory _rang, string memory _provenance) external {
      provenanceRecord.push(ProvenanceRecord(_rang, _provenance));
    }

    function setProvenance(uint32 index, string memory _rang, string memory _provenance) external {
      provenanceRecord[index] = ProvenanceRecord(_rang, _provenance);
    }

    function getProvenance() external view returns(ProvenanceRecord[] memory) {
