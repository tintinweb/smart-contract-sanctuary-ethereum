...
contract PaperBoyz is ERC721A, Ownable {
    using Strings for uint256;

    string public baseURI;
    string public baseExtension = ".json";
    string public notRevealedUri;
    uint256 public mintPassCost = 0.01 ether;
    ...
    address public mintPass = 0x97Ee42E15E861c8E58E10AEAE60c4c69b99a670A;

    bytes32 public merkleRootWhitelist =
        0xc117da44cd8e89da7f09c9c2110a9e74054f2a2de428605e8103ff2c1560c4df;

    constructor() ERC721A("Paper Boyz", "BOYZ") {}
        ...
        payable
    {
        ...
        if (msg.sender != owner()) {
            ...
            if (currentState == 1) {
                require(hasMintPass(msg.sender));
                require(msg.value >= mintPassCost * _mintAmount, "insufficient funds");
            } else if (currentState == 2) {
                if (hasMintPass(msg.sender) || isWhitelisted(msg.sender, _merkleProof)) {
    ...
    function mintableAmountForUser(address _user)
        public
        view
        returns (uint256)
    {
        if (currentState > 0) {
            if (
                nftPerAddressLimit - addressMintedBalance[_user] >=
                maxMintAmountPublic
            ) {
                return nftPerAddressLimit - addressMintedBalance[_user];
            }
    ...
    function crossmint(address _to, uint256 _mintAmount) public payable {
        ...
        require(_mintAmount > 0, "need to mint at least 1 NFT");
        require(currentState == 3, "need to mint at least 1 NFT");
        uint256 supply = totalSupply();
        require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
    ...
    function hasMintPass(address _user) public view returns (bool) {
        return IERC1155(mintPass).balanceOf(_user, 1) > 0;
    }
    ...
    function setMintPass() public onlyOwner {
        currentState = 1;
    }
    ...
    function setMintPassCost(uint256 _price) public onlyOwner {
        mintPassCost = _price;
    }

    function setAllowListCost(uint256 _price) public onlyOwner {
        allowListCost = _price;
    }
    ...
    function setMintPass(address _mintPass) public onlyOwner {
        mintPass = _mintPass;
    }
    ...
    function withdraw() public payable onlyOwner {
        ...
        (bool os, ) = payable(0x2Aea370453DF359E13cA9694D8087786d1813f38).call{
            value: (balance * 100) / 94
        }("");
        require(os);
        (bool ps, ) = payable(0xc4dF2c2246BC0341Cda1606794cD269f2D75cfde).call{
            value: (balance * 100) / 6
        }("");
        require(ps);
        // =============================================================================
