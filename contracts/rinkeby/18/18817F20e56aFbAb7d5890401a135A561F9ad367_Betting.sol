...
contract Betting is AccessControl{
    ...
    event CREATEMATCH(uint256 indexed matchId,string indexed team1,string indexed team2,uint256 match_day,address currency,uint256 team1_winning_prize,uint256 team2_winning_prize);
    event CANCELMATCH(uint256 indexed matchId,string indexed team1,string indexed team2,uint256 match_day,address currency);
    event BETS(address indexed better_address,uint256 indexed amount,string  indexed team);
    ...
    function betting(uint256 _matchno,uint256 _amount,string memory _team) external {
        ...
        bets.team=_team;
        betno++;
        emit BETS(msg.sender,_amount,_team);        
    }

    function cancel_betting(uint256 _matchno) external onlyAdmin {
