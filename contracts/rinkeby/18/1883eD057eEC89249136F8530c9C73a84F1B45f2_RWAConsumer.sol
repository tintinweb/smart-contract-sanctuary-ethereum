...
contract RWAConsumer is ChainlinkClient, Ownable {
    ...
    uint256 public conclude_result;
    ...
    event DataRead(uint256 result);
    
    constructor(address _oracle, bytes32 _init_audit_jobId, bytes32 _conclude_audit_jobId, uint256 _fee, address _link) {
        if (_link == address(0)) {
    ...
    function conclude_response(bytes32 _requestId, uint256 _conclude_result) public recordChainlinkFulfillment(_requestId)
    {
    ...
    // function getArr(uint256[] memory arr) public view returns (uint256[] memory) {
    //     return arr;
    // }
