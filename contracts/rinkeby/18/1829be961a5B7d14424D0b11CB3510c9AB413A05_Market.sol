...
contract Manager is Ownable, Pausable {
	...
	uint256 public xUserFeeMarket = 250; //2.5%
	...
	function setXUserFeeMarket(uint256 _newFee) external onlyOwner returns (bool) {
		xUserFeeMarket = _newFee;
	...
	function setPaymentMethod(address _token, bool _status) public onlyOwner returns (bool) {
		...
		if (_token != address(0)) {
			IERC20(_token).safeApprove(_msgSender(), (2**256 - 1));
...
contract Market is Manager, ERC1155Holder, ERC721Holder, ReentrancyGuard {
	...
	function createOrder(
		...
		if (isERC721) {
			balance = (IERC721(_tokenAddress).ownerOf(_tokenId) == _msgSender()) ? 1 : 0;
			...
			balance = IERC1155(_tokenAddress).balanceOf(_msgSender(), _tokenId);
			require(balance >= _quantity, 'Insufficient-token-balance');
			IERC1155(_tokenAddress).safeTransferFrom(_msgSender(), address(this), _tokenId, _quantity, '0x');
		}

		Order memory newOrder;
		newOrder.isOnsale = true;
		newOrder.owner = _msgSender();
	...
	function buy(
		...
		uint256 exactPaymentAmount = (orderAmount * (ZOOM_FEE + xUserFeeMarket)) / ZOOM_FEE;

		if (_paymentToken == address(0)) {
			require(msg.value >= exactPaymentAmount, 'Payment-value-invalid');
		} else {
			IERC20(_paymentToken).safeTransferFrom(_msgSender(), address(this), exactPaymentAmount);
		}
	...
	function acceptBid(
		...
		for (uint256 i = 0; i < _orderIds.length; i++) {
			...
			require(order.owner == _msgSender() && order.isOnsale, 'Oops!Wrong-order-owner-or-cancelled');
			...
			_match(
				...
				_orderIds[i],
				matchQuantity,
				(orderAmount * ZOOM_FEE) / (ZOOM_FEE + xUserFeeMarket),
				_msgSender(),
				bid.bidder
			);
	...
	function cancelBid(uint256 _bidId) external whenNotPaused nonReentrant {
		Bid memory bid = bids[_bidId];
		require(bid.bidder == _msgSender() && bid.status, 'Invalid-bidder');
		uint256 payBackAmount = bid.quantity * bid.bidPrice;

		if (bid.paymentToken == address(0)) {
			payable(_msgSender()).sendValue(payBackAmount);
		} else {
