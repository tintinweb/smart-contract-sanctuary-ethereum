...
contract EliteApeHelper is Ownable {
    ...
    function setPriceAll(uint256 _mintPrice) external onlyOwner {
        for(uint i; i < nftContracts.length;i++) {
            IEliteApeNFTContract(nftContracts[i]).setPrice(_mintPrice);
        }  
    }    
    ...
    function editWindowsAll(uint256 _purchaseWindowOpens, uint256 _purchaseWindowOpensPublic) external onlyOwner {
        for(uint i; i < nftContracts.length;i++) {
            IEliteApeNFTContract(nftContracts[i]).editWindow(_purchaseWindowOpens, _purchaseWindowOpensPublic);
        }  
    }
...
interface IEliteApeNFTContract {
    function editWindow(uint256 _purchaseWindowOpens, uint256 _purchaseWindowOpensPublic) external;
