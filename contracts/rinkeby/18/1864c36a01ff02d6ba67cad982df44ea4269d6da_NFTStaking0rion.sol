...
contract NFT0rion is ERC721Enumerable, Ownable {
  using Strings for uint256;
...
contract Token0rion is ERC20, ERC20Burnable, ReentrancyGuard, Ownable {
...
contract NFTStaking0rion is Ownable, IERC721Receiver {
  ...
  mapping(uint256 => Stake) public vault; 
   ...
   constructor(NFT0rion _nft, Token0rion _token) { 
    nft = _nft;
    token = _token;
  }
...
// rewardmath = 1000 ether .... (This gives 10 tokens per day per NFT staked to the staker)
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      rewardmath = daily * (block.timestamp - stakedAt) / 86400 ;
  ...
  function setDaily(uint256 _newDaily) public onlyOwner {
    daily = _newDaily;
  }
