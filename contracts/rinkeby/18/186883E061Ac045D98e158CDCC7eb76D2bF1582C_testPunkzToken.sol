...
// File: testContracts/testpunkzNFT.sol
...
// File: testContracts/testPunkz.sol
...
//@title A ERC20-token minting and ERC721 staking contract used for Punkz NFT series
//@author Ace, Alfa, Anyx <0xytocin>
//@notice This contract is used for staking Punkz NFT's and mint Degen tokens according to the tokenomics mentioned in the whitepaper
//@dev All functions are currently working and having the exact value they should have. TESTS ARE YET TO BE DONE.


contract testPunkzToken is ERC20, Ownable{
    ...
    //@notice This takes in PUNKZ NFT's contract address and also sets the start-time to block.timestamp
    constructor(address punkzAddress) ERC20("DEGEN","DEGEN"){
        Punkz = testPunkX(punkzAddress);
        punkZStart = block.timestamp;
    }
    ...
    function stakeTokens (uint[] memory tokenIds,bool lock) external {
        for (uint i=0;i<tokenIds.length;i++) {
            require(Punkz.ownerOf(tokenIds[i]) == msg.sender,"Not Owner"); //Can't stake unowned tokens
            ...
            Punkz.safeTransferFrom(msg.sender,address(this),tokenIds[i]);
        }
        if(lock){
    ...
    function unstakeTokens(uint[] memory tokenIds) external {
        ...
        for (uint i; i< tokenIds.length; i++) {
            require(Punkz.ownerOf(tokenIds[i]) == address(this),"Token not staked"); //Needs to not be staked
            stake storage currentToken = stakedTokens[tokenIds[i]];
            ...
            Punkz.safeTransferFrom(address(this), msg.sender, tokenIds[i]);
        }
    }
    ...
    function rewardFromMint(uint[] memory tokenIds) external{
        require(block.timestamp - punkZStart > 5 minutes, "It's not birthday yet");
        uint totalAmount;
        for(uint i=0;i<tokenIds.length;i++){
            require(tokenIds[i] < 22,"Reward only till 22");
            require (Punkz.ownerOf(tokenIds[i])==msg.sender || stakedTokens[tokenIds[i]].tokenOwner == _msgSender());
    ...
    function getReward(uint tokenId) public view returns(uint){
        require(Punkz.ownerOf(tokenId) == address(this),"Token not staked"); //Needs to not be staked
