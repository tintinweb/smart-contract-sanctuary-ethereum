    ...
    Pausable
{
    ...
    function wrap(
        ...
        bytes memory payload = abi.encode(abi.encode(_erc721Address), abi.encode(msg.sender), _tokenId);
    ...
    function _nonblockingLzReceive(
        ...
        (bytes memory _erc721AddressBytes, bytes memory _toAddressBytes, uint _tokenId) = abi.decode(_payload, (bytes, bytes, uint));
        address _erc721Address;
        address _toAddress;

        assembly {
            _erc721Address := mload(add(_erc721AddressBytes, 20))
            _toAddress := mload(add(_toAddressBytes, 20))
        }
