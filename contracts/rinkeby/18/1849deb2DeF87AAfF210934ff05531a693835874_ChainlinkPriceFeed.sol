 ...
 * the owner.
 */
contract OwnableUpgrade is ContextUpgradeSafe {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "OwnableUpgrade: caller is not the owner");
        _;
    }
    ...
    function setOwner(address newOwner) public onlyOwner {
        require(newOwner != address(0), "OwnableUpgrade: zero address");
        require(newOwner != _owner, "OwnableUpgrade: same as original");
        require(newOwner != _candidate, "OwnableUpgrade: same as candidate");
        _candidate = newOwner;
    }
    ...
    function updateOwner() public {
        require(_candidate != address(0), "OwnableUpgrade: candidate is zero address");
        require(_candidate == _msgSender(), "OwnableUpgrade: not the new owner");
...
contract ChainlinkPriceFeed is IPriceFeed, OwnableUpgrade, BlockContext {
    using SafeMath for uint256;
