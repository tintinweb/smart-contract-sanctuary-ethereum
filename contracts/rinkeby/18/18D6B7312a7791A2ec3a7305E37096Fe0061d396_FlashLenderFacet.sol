...
import "../libraries/FlashLenderLib.sol";

/// @notice lets token act as a liquidity pool using the ether on deposit within it
contract FlashLenderFacet is Modifiers {

    using FlashLenderLib for FlashLenderContract;

    constructor() {
        FlashLenderContract storage ds = s().flashLenders[address(this)];
        ds.feePerMillion = 1000;
    }

    function _setWrappedToken(address token) internal {
        FlashLenderContract storage ds = s().flashLenders[address(this)];
        require(ds.wrappedToken == address(0), "wrapped token already set");
        ds.wrappedToken = token;
    }
    ...
    function maxFlashLoan(address tokenAddress)
        ...
        returns (uint256)
    {
        return FlashLenderLib.maxFlashLoan(address(this), tokenAddress);
    }
    ...
    {
        FlashLenderContract storage ds = s().flashLenders[address(this)];
        return ds.flashFee(token, amount);
    }
    ...
    function setFeePermillion(
        ...
        FlashLenderContract storage ds = s().flashLenders[address(this)];
        ds.feePerMillion = _feePermillion;
    }

    function getFeePermillion(
    ) external view returns (uint256) {
        FlashLenderContract storage ds = s().flashLenders[address(this)];
        return ds.feePerMillion;
    }
    ...
    function flashLoan(
        ...
        FlashLenderContract storage ds = s().flashLenders[address(this)];
        return ds.flashLoan(receiver, address(this), ds.wrappedToken, token, amount, data);
    }
...
library FlashLenderLib {

    /**
     * @dev The maximum flash loan amount - 90% of available funds
     */
    function maxFlashLoan(address contractAddress, address tokenAddress)
        internal
        view
        returns (uint256)
    {
        ...
        if (tokenAddress != address(0)) {
            try IERC20(tokenAddress).balanceOf(contractAddress) returns (
                uint256 balance
            ) {
                return balance;
            } catch {
    ...
    function flashFee(FlashLenderContract storage self, address token, uint256 amount)
        public
        view
        returns (uint256)
    {
        ...
        uint256 feeDiv = self.feePerMillion;        // if no default fee, set the fee to 1000 (0.1%)
        if (feeDiv == 0) {
            feeDiv = 1000;
        }
     ...
     */
    function flashLoan(
        FlashLenderContract storage self,
        IERC3156FlashBorrower receiver,
        ...
        uint256 fee = flashFee(self, token, amount);
        ...
        if (token == feeToken) {
            IWrappedToken(feeToken).unwrap(_allowance);
        }
