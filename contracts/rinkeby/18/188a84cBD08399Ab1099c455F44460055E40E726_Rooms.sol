...
contract Rooms is ERC721
 {
    ...
    uint256 public maxSupplyQueen = 17; 
    uint256 public maxSupplyTwin = 6;

    function maxSupply() public view returns (uint256) {
        return maxSupplyTwin + maxSupplyQueen;
    }

    function changeMaxSupply(uint256 newMaxSupply, bool isQueen) public {
        require(msg.sender == owner, "Only owner can change the max supply");
        if(isQueen) maxSupplyQueen = newMaxSupply; 
        else maxSupplyTwin = newMaxSupply;
    }
    ...
    // hardcode chainlink address
    address private immutable feed = 0x5e5703eB345A7C7e9bFdeD2B8F6d78657976CA19;
    ...
    // prices in dollars
    uint256 public immutable priceQueenInUSD = 630 * 1e8;   // Price for Queen rooms
    uint256 public immutable priceTwinInUSD = 666 * 1e8;     // Price for Twin rooms
    ...
    // hardcoded allowList (CNC token address)
    address private immutable allowList = 0x26284Da479A7a27093607F426C9097152D47E1a7; //TODO


    address private immutable owner;

    constructor() ERC721("Rooms by CNC", "ROOMS") {
        owner = msg.sender;
    }
    ...
    function mintTwin(uint256 amount) external payable {
        ...
        _mint(amount, 100); // this shift in tokenIDs ensures Twins get the right metadata
        totalSupplyTwin += amount;
    }

    function _mint(uint256 amount, uint256 shift) internal {
        require(
            amount <= 3,
            "Leave some rooms for the others ser"
        );

        for (uint256 i = 0; i < amount; i++) {
            _safeMint(msg.sender, totalSupply() + i + shift);
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual override(ERC721) onlyAllowlist(to){
        super._beforeTokenTransfer(from, to, tokenId);
    }

    modifier onlyAllowlist(address who) {
        require(
            allowList == address(0) ||
                ERC721(allowList).balanceOf(who) > 0,
            "Requires Crypto Nomads Club NFT"
        );
        _;
    }
