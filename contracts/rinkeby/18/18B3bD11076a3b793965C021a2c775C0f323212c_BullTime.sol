...
contract BullTime is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public constant PRESALE_LIMIT = 333;
    ...
    uint256 public constant GENESIS_PRICE = 0.18 ether;
    ...
    address private Faddr = 0x43640829F5E5F919052A673C651c013c9773e3cC; // 53.5%
    ...
    // --- gift
    function gift(uint256 quantity) external {
    ...
    function withdraw() external onlyOwner {
        payable(Kaddr).transfer(address(this).balance * 5 / 100);
        payable(Daddr).transfer(address(this).balance * 5 / 100);
        payable(Laddr).transfer(address(this).balance * 5 / 100);
        payable(Taddr).transfer(address(this).balance * 5 / 100);
        payable(DAaddr).transfer(address(this).balance * 5 / 100);

        payable(Zaddr).transfer(address(this).balance * 14 / 100);
        payable(Caddr).transfer(address(this).balance * 15 / 1000);
        payable(Iaddr).transfer(address(this).balance * 5 / 1000);
        payable(Aaddr).transfer(address(this).balance * 5 / 1000);
