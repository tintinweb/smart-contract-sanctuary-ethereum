...
// MasterChef was the master of king. He now governs over KINGS. He can make Kings and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    function pendingKing(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 kingReward = multiplier
                .mul(kingPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accKingPerShare = accKingPerShare.add(
                kingReward.mul(1e12).div(lpSupply)
            );
        }
        return
            user.amount.mul(accKingPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 kingReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        king.mint(devaddr, kingReward.div(devFundDivRate));
        king.mint(address(this), kingReward);
        pool.accKingPerShare = pool.accKingPerShare.add(
            kingReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accKingPerShare).div(1e12).sub(
            user.rewardDebt
        );
        safeKingTransfer(msg.sender, pending);
    ...
    function setKingPerBlock(uint256 _kingPerBlock) public onlyOwner {
        require(_kingPerBlock > 0, "!kingPerBlock-0");
