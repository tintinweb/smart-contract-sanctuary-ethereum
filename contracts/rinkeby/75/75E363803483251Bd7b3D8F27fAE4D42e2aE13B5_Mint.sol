...
import "./UnknownPortal.sol";

interface IMint {
    function mint(uint256 token_Id, uint256 amount) external payable;
    function isAddressMinted(address _address) external returns(uint256);
}
...
contract Mint is ERC721Enumerable, Ownable {
  ...
  mapping(address => uint256) public mintedCounterTokenId;
  constructor(
    string memory _name,
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    uint256 addressMinted = IMint(mintContract).isAddressMinted(msg.sender);
    require( addressMinted > 0 , 'need to buy 5 SFTs');
    if (msg.sender != owner()) {
...
contract UnknownPortal is ERC1155Supply, ERC1155Burnable, Ownable {
    ...
    function isAddressMinted (address _address) external view returns (uint256) {
            ...
            if ( mintedCounterTokenId[token_Id] == 5 )
                {
                    ...
                    addressMinted[msg.sender] = token_Id;
