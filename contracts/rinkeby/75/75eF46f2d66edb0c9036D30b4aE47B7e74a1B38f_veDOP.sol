...
contract veDOP is IERC721, IERC721Metadata {
    enum DepositType {
        DEPOSIT_FOR_TYPE,
        CREATE_LOCK_TYPE,
    ...
    string constant public name = "veDOP";
    string constant public symbol = "VEDOP";
    ...
    /// @param token_addr `DOP` token address
    constructor(
        address token_addr
    ) {
        token = token_addr;
    ...
    function deposit_for(uint _tokenId, uint _value) external nonreentrant {
        ...
        require(_locked.amount > 0, 'No found');
        require(_locked.end > block.timestamp, 'Withdraw');
    ...
    function _create_lock(uint _value, uint _lock_duration, address _to) internal returns (uint) {
        ...
        require(_value > 0); // dev: need non-zero value
        require(unlock_time > block.timestamp, 'no future time');
        require(unlock_time <= block.timestamp + MAXTIME, '4years max');
    ...
    function increase_amount(uint _tokenId, uint _value) external nonreentrant {
        ...
        require(_locked.amount > 0, 'Nofound');
        require(_locked.end > block.timestamp, 'Withdraw');
    ...
    function increase_unlock_time(uint _tokenId, uint _lock_duration) external nonreentrant {
        ...
        require(_locked.amount > 0, 'Nothing');
        require(unlock_time > _locked.end, 'Only duration');
        require(unlock_time <= block.timestamp + MAXTIME, '4years max');
    ...
    function withdraw(uint _tokenId) external nonreentrant {
        ...
        LockedBalance memory _locked = locked[_tokenId];
        require(block.timestamp >= _locked.end, "didn't expire");
    ...
    function tokenURI(uint _tokenId) external view returns (string memory) {
        require(idToOwner[_tokenId] != address(0), "Query");
        LockedBalance memory _locked = locked[_tokenId];
        return
        _tokenURI(
            _tokenId,
