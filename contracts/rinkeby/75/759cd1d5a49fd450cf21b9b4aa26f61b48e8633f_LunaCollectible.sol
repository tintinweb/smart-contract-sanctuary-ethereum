...
// File: contracts/interfaces/ILunaFactory.sol



pragma solidity ^0.8.4;

interface ILunaFactory {
  function isPrivate() external pure returns (bool);
  function defender() external pure returns (address);
...
// File: contracts/roles/LunaNFTAccessControls.sol
 ...
 */
contract LunaNFTAccessControls is AccessControl {
  /// @notice Role definitions
  bytes32 public constant PAYOUT_ROLE = 
  0x5612063648f807201ec1c6d4f999a34ed02c6a8b521b7b99e155a6823c568736; // keccak256("PAYOUT_ROLE")
  ...
  event PayoutRoleGranted(
    address indexed beneficiary,
    address indexed caller
  );

  event PayoutRoleRemoved(
    address indexed beneficiary,
    address indexed caller
  );
  ...
  /**
    ...
    * @return bool True if the account has the role or false if it does not
    */
  function hasPayoutRole(address _address) public view returns (bool) {
    return hasRole(PAYOUT_ROLE, _address);
  }
  ...
  modifier onlyAdmin () {
    require(hasAdminRole(_msgSender()), "Ownable: caller is not the admin");
    _;
  }

  modifier onlyPayouter () {
    require(hasPayoutRole(_msgSender()), "Ownable: caller is not the minter");
    _;
  }

  modifier onlyMinter () {
    require(hasMinterRole(_msgSender()), "Ownable: caller is not the minter");
    _;
  }
  ...
  /**
    ...
    * @param _address EOA or contract receiving the new role
    */
  function addPayoutRole(address _address) public {
    grantRole(PAYOUT_ROLE, _address);
    emit PayoutRoleGranted(_address, _msgSender());
  }
  ...
  /**
    ...
    * @param _address EOA or contract affected
    */
  function removePayoutRole(address _address) public {
    revokeRole(PAYOUT_ROLE, _address);
    emit PayoutRoleRemoved(_address, _msgSender());
  }
...
// File: contracts/LunaCollectible.sol
...
contract LunaCollectible is
    IERC2981,
    ERC165Storage,
    Ownable,
    ERC1155,
    ERC1155URIStorage,
    LunaNFTAccessControls
{
    using Strings for uint256;
    ...
    address payable public payouter;

    // factory contract
    address public factory;

    /// @notice Constructor Function
    /// @dev Imtegrate EIP712 signature for lazy minting
    ...
    constructor(
        ...
        tokenIndex = 0;
        addAdminRole(_msgSender());
        addMinterRole(_msgSender());
        payouter = _payouter;
        factory = _factory;
    ...
    function mint(
        ...
        require(
            ...
            "The mintprice is not enough"
        );

        if (ILunaFactory(factory).isPrivate()) {
            require(
                msg.sender == address(ILunaFactory(factory).defender()),
                "Private case: minter should be defender address"
            );
        }
        ...
        if (tokenIndex < _id) tokenIndex++;
    }

    /// @dev withdraw all ethers to payout wallet.
    function withdraw() public payable {
        (bool withdrawSuccess, ) = payable(payouter).call{
            value: address(this).balance
        }("");
        require(withdrawSuccess, "Failed to send platform fee");
    }
    ...
    function setMintPrice(uint256 _tokenId, uint256 _mintPrice)
        external
        onlyAdmin
    {
    ...
    function setPayouter(address _newPayouter) external onlyAdmin {
        payouter = payable(_newPayouter);
    }
    ...
    function setRoyaltyPercent(uint256 _royaltyPercent) external onlyAdmin {
