...
contract Obscurals is ERC721MT, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;

    //CONST
    uint256 public constant OB_MAX = 5555;
    ...
    string private _baseTokenURI = "https://meta-front-zeta.vercel.app/";
    uint256 private _price = 0.02 ether; // set to 0.04 after presale
    mapping(address => bool) _hasMinted;


    constructor(
        string memory name,
        string memory symbol) ERC721MT(name, symbol) {  }
   ...
   function mintTokens(address addy, uint256 quantity) private {
       _mint(addy, quantity);
    }
    ...
    function generalMintingRules(uint256 value, uint256 quantity) private view {
        ...
        require(totalSupply() < OB_MAX, "Sold Out");
        require(totalSupply() + quantity <= OB_MAX, "exceeds the public amount"); 
        require(value == getPrice(quantity), "wrong eth value");
    }

    function presalemint(address addy, uint256 numberOfTokens, bytes32[] calldata proof) external payable {
        require(isPreSaleActive, "PreSale is not active");
        require(isAllowed(msg.sender, proof), "Not on Presale");
        require(numberOfTokens < 6, "Max 5 mints during presale");
        require(!_hasMinted[msg.sender], "You already minted");
        generalMintingRules(msg.value, numberOfTokens);
        mintTokens(addy, numberOfTokens);
        _hasMinted[msg.sender] = true; 
    }

    function mint(address addy, uint256 numberOfTokens) external payable {
    ...
    function reserveTokens(uint256 quantity) public onlyOwner {
         require(totalSupply() + quantity <= OB_MAX, "this exceed the public amount");
        mintTokens(msg.sender, quantity);
    }
   ...
   function walletOfOwner(address address_)
    ...
    function togglePresaleActive() external onlyOwner {
     ...
     function withdraw() external onlyOwner { 
        uint256 _DevCut = address(this).balance * 15 / 100;
