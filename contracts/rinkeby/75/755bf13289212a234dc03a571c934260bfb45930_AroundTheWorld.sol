...
contract AroundTheWorld is ERC721Enumerable, Ownable
{
    using SafeMath for uint256;
    using Strings for uint256;

    uint public constant _TOTALSUPPLY = 50;
    uint public maxQuantity =10;
      mapping(address=>bool) public whiteListedAddress;
      uint256[29] randomId=[40, 33, 31, 34, 36, 32, 22, 23, 45, 47, 48, 49, 27, 30, 35, 38, 37, 26, 29, 44, 42, 43, 24, 41, 25, 50, 28, 46, 39];
   
    uint public reserve=21;
    uint randomIndex=0;
    uint256 public price = 0.04 ether; //WL  0.04 , 0.06  Public 
    uint256 public status = 0; // 0-pause, 1 - Whitelist,  2- Public
    

    // uint private tokenId=1;

    constructor(string memory baseURI) ERC721("Around The World", "ATW") {
        setBaseURI(baseURI);
        for (uint i = 1; i <=21; i++) {
    ...
    function mint(uint chosenAmount) public payable isSaleOpen {
        ...
        for (uint i = 0; i < chosenAmount; i++) {
            _safeMint(msg.sender, randomId[randomIndex]);
            randomIndex++;
        }
    ...
    function contractURI() public view returns (string memory) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Around The World", "description": " With only 9,770 ATW NFTs ever to be minted, they grant owners proof of ownership on the Ethereum Blockchain. This is not your average NFT, this is your ticket to travel the world", "seller_fee_basis_points": 500, "fee_recipient": "0xA5B6e0d6758299773AFA4F4c20e8Ed901707ae41"}'))));
