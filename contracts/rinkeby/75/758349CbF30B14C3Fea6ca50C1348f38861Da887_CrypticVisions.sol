...
contract CrypticVisions is ERC721, EIP712, Ownable {
	...
	uint256 public constant TOKENS_PER_PERSON_LIMIT = 5;
	uint256 public constant TOKENS_PER_TRAN_LIMIT = 5;
	uint256 public constant COLLECTION_SIZE = 10000;
	uint256 public constant MINT_PRICE = 0.08 ether;
	uint256 public constant PRESALE_TOKEN_PRICE = 0.1 ether;
	SaleStatus public saleStatus = SaleStatus.PAUSED;

	bool public canReveal = true;

	string private _delayedRevealURI;
	string private _baseUri;
	address private immutable _voucherSigner;
	mapping(address => uint256) private _mintedCount;

	constructor(
		string memory delayedRevealURI,
		string memory baseUri,
		address voucherSigner,
		uint256 setAsideCount
	) ERC721("CrypticVisions", "CRVS") EIP712("CrypticVisions", "1")  {
		_delayedRevealURI = delayedRevealURI;
		_baseUri = baseUri;
		_voucherSigner = voucherSigner;

		if (setAsideCount > 0) {
			_mintTokens(msg.sender, setAsideCount);
		}
	}
	...
	function redeem(
		...
		require(
			_verify(signer, _hash(voucher), signature),
			"Transaction is not authorized (invalid signature)"
		);
		require(saleStatus != SaleStatus.PAUSED, "Sale is inactive");

		if (saleStatus == SaleStatus.PRESALE) {
			require(
				voucher.whitelisted,
				"Presale is only open to whitelisted users"
			);
			require(
				msg.value >= voucher.numberOfTokens * PRESALE_TOKEN_PRICE,
				"Ether value sent is not sufficient"
			);
		} else {
			require(
				msg.value >= voucher.numberOfTokens * MINT_PRICE,
				"Ether value sent is not sufficient"
			);
		}

		require(
			_tokenIds.current() <= COLLECTION_SIZE,
			"All tokens have been minted"
		);
		require(
			_tokenIds.current() + voucher.numberOfTokens <= COLLECTION_SIZE,
			"Number of requested tokens will exceed collection size"
		);
		require(
			_mintedCount[voucher.redeemer] + voucher.numberOfTokens <=
				TOKENS_PER_PERSON_LIMIT,
			string(
				abi.encodePacked(
					"Max ",
					TOKENS_PER_PERSON_LIMIT.toString(),
					" token(s) per wallet"
				)
			)
		);
		...
		return
			_hashTypedDataV4(
				keccak256(
					abi.encode(
						keccak256(
							"NFTVoucher(address redeemer,bool whitelisted,uint numberOfTokens)"
						),
						voucher.redeemer,
						voucher.whitelisted,
