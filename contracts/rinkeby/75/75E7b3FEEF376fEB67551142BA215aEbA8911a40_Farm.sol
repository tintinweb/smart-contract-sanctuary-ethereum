...
contract Farm is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    address private APIConsAddress;
    address private HorseAddress;
    address private HorseGovAddress;
    uint256 private pyramidsMinted;
    uint256[] private land_gend;
    ...
    uint256 private maxStableLength;
    ...
    mapping(address => uint256[]) public wraps;
    ...
    function checkLandGend() public view virtual returns (uint256[] memory) {
        return land_gend;
    }
    ...
    function setMaxStableLength(uint256 num) public onlyOwner {
        maxStableLength = num;
    }
    ...
    function wrapLandtoFarmReq(
        ...
        require(
            ids.length <= maxStableLength,
            "FARM MUST BE LESS THAN or = to MAX SIZE"
        );
        Lander LandContract = Lander(LandAddress);
        for (uint256 i = 0; i < ids.length; i++) {
            require(
                LandContract.ownerOf(ids[i]) == msg.sender,
                "MUST OWN ALL SELECTED LAND TOKENS"
            );
        }
    ...
    function stableHorseFree(
        uint256 horseID,
        uint256 farmID,
        uint256 stableTerm
    ) public {
        stableHorse(horseID, farmID, stableTerm);
    }

    function stableHorsePayable(
        uint256 horseID,
        uint256 farmID,
        uint256 stableTerm
    ) public payable {
    ...
    function stableHorse(
        ...
        require(
            stableTerm > 0 && stableTerm <= maxStableLength,
            "STABLE TERM MUST BE 1-6"
        );
        require(
        ...
        for (uint256 i = 0; i < farms[farmID].length; i++) {
            require(
                farms[farmID][i] != horseID,
                "HORSE MUST NOT ALREADY BE STABLED IN THIS FARM"
            );
        }
        uint256 farmIDfrHorse = HorseContract.checkStable(horseID);
        uint256 farmExpDate = HorseContract.checkStableExp(horseID) * 30 days;
        if (
            ...
            require(
                ...
                "MUST OWN FARM TO STABLE HORSES IN PRIVATE FARM"
            );
            if (farmIDfrHorse == 0) {
                horses[farmID].push(horseID);
                HorseContract.setStable(horseID, farmID, stableTerm);
            } else {
            ...
            if (farmIDfrHorse == 0) {
                stableRequests[farmID].push(
                    StableRequest(horseID, msg.sender, stableTerm)
                );
            } else {
                if (block.timestamp < farmExpDate) {
                    require(
    ...
    function destableFree(uint256 horseID, uint256 farmID) public {
        destable(horseID, farmID);
    }
    ...
    function approveRequest(
        ...
        require(farms[farmID].length > 0, "FARM MUST BE >0");
        require(farms[farmID].length > 9, "FARM MUST BE PUBLIC");
        require(
            (farms[farmID].length - horses[farmID].length) > 0,
            "FARM MUST HAVE ENOUGH SLOTS"
        );
    ...
    function purchase(uint256 count) external payable whenNotPaused {
        ...
        require(isPublicSaleActive(), "CANNOT_MINT");

        _purchase(count, _price);
        emit Purchase(msg.sender, _price, count);
    }

    function earlyPurchase(uint256 count, bytes32[] calldata merkleProof)
        external
        payable
        whenNotPaused
    {
    ...
    function _purchase(uint256 count, uint256 price) private {
        require(price * count <= msg.value, "INSUFFICIENT_ETH_AMOUNT");
    ...
    function ensurePublicMintConditions(
        address to,
        uint256 count,
        uint256 maxPerAddress
    ) internal view {
    ...
    function isPreSaleActive() public view returns (bool) {
        return (_preSaleTime == 0 ||
            ((_preSaleTime < block.timestamp) &&
                (block.timestamp < _publicSaleTime)));
    }

    function onEarlyPurchaseList(address addr, bytes32[] calldata merkleProof)
        public
        view
        returns (bool)
    {
        require(_merkleRoot.length > 0, "PRESALE_MINT_LIST_UNSET");
