...
contract AviumNFT is Ownable, ERC721AURIStorage {
    ...
    */
    ...
    /**
     * @dev Set controller
     * @param _newController new controller
     */
    ...
    */
    ...
    /**
     @dev Get start time for mint NFT
    */
    ...
    */
     ...
     * @param tokenURIs the URI of tokens
    ...
    function mint(
        ...
        require(
            ...
            "AviumNFT: only owner or controller"
        );
        require(
            _currentIndex < totalMint, "AviumNFT: total mint has been exeeded"
        );
        require(
            block.timestamp > startTime, "AviumNFT: it haven't been time to start mint"
        );
        uint256 curr = _currentIndex;
        _safeMint(to, quantity, _data);
        for (uint256 i = 0; i < quantity; i++) {
            _setTokenURI(curr++, tokenURIs[i]);
        }
    }
...
contract ERC721AURIStorage is ERC721A {
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "ERC721AURIStorage: URI query for nonexistent token"
        );
