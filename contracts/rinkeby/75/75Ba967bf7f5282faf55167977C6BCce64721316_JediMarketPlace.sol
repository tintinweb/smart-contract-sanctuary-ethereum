...
contract JediMarketPlace is ReentrancyGuard, ERC721Holder, ERC1155Holder, Ownable {
    struct Item {
        address tokenOwner; 
        bool listed;
        uint256 price;
   ...
   /// @return true if item is listed
    function listItem(uint256 id, uint256 price) external returns(bool) {
        require(block.timestamp >= _lastTradeTime[msg.sender] + _cooldown, "Trade is on cooldown!");

        _erc721.safeTransferFrom(msg.sender, address(this), id);

        _lastTradeTime[msg.sender] = block.timestamp;
...
contract Jedi721 is ERC721URIStorage, AccessControl, Ownable {
    ...
    function toggleBlacklisted(address victim) external onlyOwner returns(bool) {
        _blackListed[victim] = !_blackListed[victim];

        return _blackListed[victim];
    }
