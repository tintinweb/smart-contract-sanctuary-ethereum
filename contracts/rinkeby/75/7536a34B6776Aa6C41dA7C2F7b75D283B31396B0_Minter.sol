...
contract Minter is Ownable, ERC721Enumerable {
    ...
    //remaining reserved to be minted
    uint256 public reserved;

    //remaining adopters to be minted
    uint256 adopterMints;

    //remaining adopters to be minted
    uint256 dipHolderMints;

    /** 
        base uri will be replace after hitting totoalSupply
        s3 bucket for temporary
    ...
    **/
    ...
    bytes32 public dipHolderRoot;
    bytes32 public earlyAdopterRoot;
    ...
    function mintIdagWithDip(uint256 numberOfTokens, bytes32[] calldata proofs)
        ...
        payable
    {
        require(verifyMerkleProof(msg.sender, proofs, dipHolderRoot), "Wrong Proof");
    ...
    function adopterMintIdag(uint256 numberOfTokens, bytes32[] calldata proofs)
        ...
        payable
    {
        require(verifyMerkleProof(msg.sender, proofs, earlyAdopterRoot),  "Wrong Proof");
        require(
        ...
        internal
    {
        ...
        uint256 userMints = _wlMintStatus[msg.sender];

        require(
            userMints + numberOfTokens <= maxWlMint,
            "Exceesd max limit"
        );
        require(
        ...
        _wlMintStatus[msg.sender] = userMints + numberOfTokens;
    ...
    function mintIDGAF(uint256 numberOfTokens) external payable {
        ...
        uint256 userMints = _mintStatus[msg.sender];
         require(numberOfTokens > 0, "Cant mint zero");
        require( userMints + numberOfTokens <= maxPublicMint,
        ...
        _mintStatus[msg.sender] = userMints + numberOfTokens;
        _mintIdag(msg.sender, numberOfTokens);
    }
