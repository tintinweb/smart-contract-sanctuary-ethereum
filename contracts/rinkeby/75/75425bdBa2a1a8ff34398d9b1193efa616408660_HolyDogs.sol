...
contract HolyDogs is Abstract_HolyDogs {
    ...
    function storeRemainingIds() public {
    ...
    function airDropToOwner(uint256 amount) external onlyOwner {
        ...
        for(uint i = amount; i>0; i--) {
            uint airDropTokenIds = remainingTokenIds[remainingTokenIds.length-1];
            _mint(msg.sender, airDropTokenIds ,NFT_AMOUNT, "");
            remainingTokenIds.pop();
            totalSupply++;
            emit NFTAirDrop(airDropTokenIds);
        }
    }
