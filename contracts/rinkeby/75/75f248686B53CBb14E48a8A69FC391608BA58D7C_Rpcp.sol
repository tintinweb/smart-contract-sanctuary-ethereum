...
contract Rpcp is ERC721A, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    uint public priceRaton = 0.0000002 ether;

    uint public priceCarcajou = 0.0000006 ether;

    uint public priceRenard = 0.000002 ether;

    uint public priceLoup = 0.000004 ether;

    uint public priceOurs = 0.000008 ether;
    ...
    constructor(string memory _theBaseURIRaton, string memory _notRevealedURIRaton, string memory _theBaseURICarcajou, string memory _notRevealedURICarcajou, string memory _theBaseURIRenard, string memory _notRevealedURIRenard, string memory _theBaseURILoup, string memory _notRevealedURILoup, string memory _theBaseURIOurs, string memory _notRevealedURIOurs) ERC721A("Rpcp", "RPCP") PaymentSplitter(_team, _teamShares) {
        transferOwnership(msg.sender);
        sellingStep = Steps.Before;
        baseURIRaton = _theBaseURIRaton;
        notRevealedURIRaton = _notRevealedURIRaton;
        baseURICarcajou = _theBaseURICarcajou;
        notRevealedURICarcajou = _notRevealedURICarcajou;
        baseURIRenard = _theBaseURIRenard;
        notRevealedURIRenard = _notRevealedURIRenard;
        baseURILoup = _theBaseURILoup;
        notRevealedURILoup = _notRevealedURILoup;
        baseURIOurs = _theBaseURIOurs;
    ...
    **/
    ...
    function gift(address _account, uint256 _quantity) external onlyOwner {
        uint numberNftSold = totalSupply();
        require(numberNftSold + _quantity <= MAX_SUPPLY_OURS, "Sold out");
