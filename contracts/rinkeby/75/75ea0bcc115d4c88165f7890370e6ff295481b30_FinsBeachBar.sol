...
contract FinsBeachBar is ERC1155, ERC1155Supply, ERC1155Burnable, Ownable {
        ...
        onlyOwner
    {
        ...
        require(_list.length + totalSupply(_dropNumber) <= MAX_PER_ITEM);
    ...
    function batchMint(
        ...
        // added this as a check to ensure the list length + existing supply don't exceed the max tokens that can be issued
        require(totalSupply(_tokenID)+ _quantity <= MAX_PER_ITEM);
    ...
    function burnToRedeem(address account, uint256[] memory ids) public {
        // ensure the person sending the message owns the tokens we're going to burn
        require(msg.sender == account);
        // make sure redemption is active
        require(redeemIsActive);
        // ensure that exactly ten items are submitted to burn
        require(ids.length == requiredToClaim);
        // added this as a check to ensure the list length + existing supply don't exceed the max tokens that can be issued
        require(totalSupply(claimTokenID) < MAX_PER_ITEM);
        ...
        for (uint256 i = 0; i < requiredToClaim; i++) {
            require(tokens[ids[i]] == 0);
            tokens[i] = ids[i];
            values[i] = 1;
        }

        _burnBatch(account, tokens, values);
        _mint(account, claimTokenID, 1, "");
