...
contract LRP is ERC721, Ownable {
  using Strings for uint256;
  ...
  constructor() ERC721("Locker Room Pass Genesis", "LRP") {
    setHiddenMetadataUri("ipfs://QmZnafm7avgvSgde7DukbRhRa2qUcYd75VMszSMmffTKiM/");
  }
...
//ensures minting security
  ...
  function mintReserved(uint256 _mintAmount) public onlyOwner{
      ...
      require(addressMintedBalance[msg.sender] + _mintAmount <= reserved);
      for (uint256 i = 1; i <= _mintAmount; i++) {
        addressMintedBalance[msg.sender]++;
      }
      _mintLoop(msg.sender, _mintAmount);
