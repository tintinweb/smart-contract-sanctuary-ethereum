...
contract TarvenNFT is ERC721, Ownable, ERC721Enumerable {
    ...
    uint256 public constant MAX_PRESALE = 800;
    ...
    // reverse token to dev with 400
    function reserveTokens(address _to, uint256 _reserveAmount)
        external
        onlyOwner
    {
        require(
            _reserveAmount > 0 && _countreserveTokens.add(_reserveAmount) <= devReserve,
    ...
    function mint(uint256 numberOfTokens) external payable {
        ...
        if(_presaleList[msg.sender]){
            require(balanceOf(msg.sender) <= 1, "Only whitelist address can be mint one more in public sale");
        }
        else {
            require(balanceOf(msg.sender) < 1, "Must be mint only one");
        }
    ...
    function presaleTarven(uint256 numberOfTokens) external payable {
        require(presaleIsActive, "Presale is not active");
        require(_presaleList[msg.sender], "You are not on the Presale List");
        require(
            totalSupply().add(numberOfTokens) <= MAX_PRESALE.add(devReserve),
    ...
    function setPresaleMaxMint(uint256 maxMintNFT) external onlyOwner {
        maxMint = maxMintNFT;
    }
