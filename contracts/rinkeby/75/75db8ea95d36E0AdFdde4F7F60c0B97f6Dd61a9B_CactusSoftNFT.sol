...
contract CactusSoftNFT is ERC721, ERC721Enumerable {
    ...
    // Убрал, потому что указывается в методе activate
    ...
    function deactivate() public onlyOwners {
        // Как идея, чтобы maxSupply() метод возвращал кол-во, которое сейчас есть
    ...
    function changeRoyaltyAddress(address newRoyaltyAddress) public onlyOwners {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // Генерировать URI основываясь на ID токена 
    ...
    {
        // Проверка на кол-во токенов у получателя
        require(balanceOf(_to) < maxPerAddress, 'Recipient has too much tokens.');
