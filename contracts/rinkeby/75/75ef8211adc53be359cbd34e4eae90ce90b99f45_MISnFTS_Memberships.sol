...
// File: MISNFTS_MEMBERSHIP_FINAL_.sol

//discuss optimization settings. higher runs -> less computation costs for external calls (500 runs-1000runs)

pragma solidity ^0.8.13;

/*
 __       __ ______  ______           ________ ________  ______  
|  \     /  \      \/      \         |        \        \/      \ 
| ▓▓\   /  ▓▓\▓▓▓▓▓▓  ▓▓▓▓▓▓\_______ | ▓▓▓▓▓▓▓▓\▓▓▓▓▓▓▓▓  ▓▓▓▓▓▓\
| ▓▓▓\ /  ▓▓▓ | ▓▓ | ▓▓___\▓▓       \| ▓▓__      | ▓▓  | ▓▓___\▓▓
| ▓▓▓▓\  ▓▓▓▓ | ▓▓  \▓▓    \| ▓▓▓▓▓▓▓\ ▓▓  \     | ▓▓   \▓▓    \ 
...
| ▓▓  \▓ | ▓▓   ▓▓ \\▓▓    ▓▓ ▓▓  | ▓▓ ▓▓        | ▓▓   \▓▓    ▓▓
 \▓▓      \▓▓\▓▓▓▓▓▓ \▓▓▓▓▓▓ \▓▓   \▓▓\▓▓         \▓▓    \▓▓▓▓▓▓ 
           ...
           MISnFTS | Membership Contract | September 22                                                                   
*/

contract MISnFTS_Memberships is ERC1155, EIP712Whitelisting, ERC2981ContractWideRoyalties, CantBeEvil(LicenseVersion.CBE_PR_HS){
  ...
  function randyMint(uint _tokenId) public payable randyWatch{
    require(tokenList[_tokenId].userRedeemed[msg.sender] == false, "User redeemed membership already.");
    require(tokenList[_tokenId].validation, "Token doesn't exist.");
    require(!tokenList[_tokenId].isPaused, "Minting is paused.");
    _mint(msg.sender, _tokenId, 1, "");
  ...
  function pushRandy(uint[] calldata _tokenIds) public onlyOwner {
    uint length = _tokenIds.length;
    for(uint i = 0; i < length; i++) randyTokens.push(_tokenIds[i]);
  }
  ...
  function withdrawFunds() public payable onlyOwner {
    ...
    (bool successA, ) = payable( --- ).call{value: (balance / 100) * 50}(""); //address 50%
    (bool successB, ) = payable( --- ).call{value: (balance / 100) * 25}(""); //address 25%
    (bool successC, ) = payable( --- ).call{value: (balance / 100) * 25}(""); //address 25%
