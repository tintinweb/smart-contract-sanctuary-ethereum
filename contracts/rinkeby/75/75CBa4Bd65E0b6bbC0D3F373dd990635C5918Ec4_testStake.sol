...
contract testStake is Ownable, Pausable, ReentrancyGuard {
    ...
    function calculateRewardQuantity(StakeShrooms memory stakedShrooms) internal view returns (uint256) {
        uint256 reward=0;
        uint256 duration = 10000 ether * (block.timestamp - stakedShrooms.start) / 86400;
        reward = duration/100;
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      tokenId = tokenIds[i];
      StakeShrooms memory stakedShrooms = stakedShrooms[tokenId];
      require(stakedShrooms.owner == account, "not an owner");
      rewardDuration = 10000 ether * (block.timestamp - stakedShrooms.start) / 86400;
      reward = rewardDuration / 100;
