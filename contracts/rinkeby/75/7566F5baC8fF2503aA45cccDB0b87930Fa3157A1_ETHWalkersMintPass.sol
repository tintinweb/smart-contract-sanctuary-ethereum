...
contract ETHWalkersMintPass is ERC721A, Ownable, ReentrancyGuard, Pausable {
    ...
    uint public allowListSale = 1656593590; // 6/15 at 11am PDT
    uint public publicSale = 1656593590; // 6/15 at 11pm PDT
    uint public endSale = 1656766390; // 6/16 at 11pm PDT
    ...
    function allowlistETHWalkerMintPass(uint8 numberOfTokens, uint nonce, bytes memory signature) external payable whenNotPaused {
        ...
        bytes32 userhash = keccak256(abi.encodePacked(_msgSender(),nonce));
        require(userhash.toEthSignedMessageHash().recover(signature) == whitelistSigner, "Invalid Signature");
