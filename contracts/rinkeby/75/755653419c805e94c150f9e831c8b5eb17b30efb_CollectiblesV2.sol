...
// File @openzeppelin/contracts/token/ERC721/[email protected]v4.3.2
...
// File contracts/interfaces/ITraded.sol
...
// File contracts/CollectiblesV2.sol
...
contract CollectiblesV2 is ERC721URIStorage, IERC2981, ITraded {
    ...
    /// @dev To store original owner for each token
    mapping(uint256 => address) private royaltyOwners;
    ...
    /// @dev To store the original deployer
    ...
    event RoyaltyRightsTransferred(
        address indexed _prevRoyaltyOwner,
        address indexed _newRoyaltyOwner,
        ...
        external
    {
        ...
        royaltyOwners[newItemId] = msg.sender;

        delete newItemId;
        delete tokenURI;
        delete _royaltyPercent;
    }
    ...
    /// @param _newRoyaltyOwner Address of the new royalty rights owner/receiver
    ...
    function transferRoyaltyRights(address _newRoyaltyOwner, uint256 _tokenId)
        external
    {
        require(
            msg.sender == royaltyOwners[_tokenId],
            "Only royalty owner can transfer the rights"
        );

        royaltyOwners[_tokenId] = _newRoyaltyOwner;
        emit RoyaltyRightsTransferred(msg.sender, _newRoyaltyOwner, _tokenId);
        delete _newRoyaltyOwner;
        delete _tokenId;
    }
    ...
    function getAllTokens() external view returns (tokenStruct[] memory) {
        ...
        for (uint256 i = 0; i < tokenIds.current(); i++) {
            ...
            allTokens[i].creator = royaltyOwners[index];
        }
