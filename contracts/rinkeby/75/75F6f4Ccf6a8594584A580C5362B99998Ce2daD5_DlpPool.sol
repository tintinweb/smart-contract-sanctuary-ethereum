...
contract DlpPool is Initializable, CorePool {
    ...
    mapping(address => uint256) veBalance;

    mapping(address => uint256) vpBalance;
    ...
    function addVEAmount(address _user, uint256 _amount) external nonReentrant {
        // checks if contract is paused
        _requireNotPaused();
        // expects caller to be a valid contract registered by the pool factory
    ...
    function reduceVEAmount(address _user, uint256 _amount)
        external
        nonReentrant
    {
        // checks if contract is paused
