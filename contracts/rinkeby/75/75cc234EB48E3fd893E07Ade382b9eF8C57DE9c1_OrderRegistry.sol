...
contract Order {
    ...
    function transferBalanceTo(address _to) external {
        ...
        uint256 balance = IERC20(tokenGive).balanceOf(address(this));
        IERC20(tokenGive).transfer(_to, balance);
    }
    ...
    function confirmBalance() external view returns (bool confirmation) {
        confirmation = IERC20(tokenGive).balanceOf(address(this)) >= amountGive ? true : false;
    }
    ...
    function returnDeposit() external {
        ...
        uint256 balance = IERC20(tokenGive).balanceOf(address(this));
        require(balance > 0);
        IERC20(tokenGive).transfer(user, balance);
    }

    function sendOrderAmount(address _to) external {
        require(msg.sender == registry, "UNAUTHORIZED");
        IERC20(tokenGive).transfer(_to, amountGive);
    }

    function getBalance() public view returns (uint256) {
        return IERC20(tokenGive).balanceOf(address(this));
    }
