...
/// @title Purchase Manager Contract for Lazy Cubs Collection
/// @author Akshat Mittal
contract PurchaseManager is Ownable, ContractSafe {
    using ECDSA for bytes32;

    IERC721 LazyLionsInterface;
    LazyDrinks LazyDrinksInterface;
    LazyCubs LazyCubsInterface;
     ...
     **
     **      0 -  9,999 - Public Sale
     ** 10,000 - 19,999 - Mutation: Young
     ** 20,000 - 29,999 - Mutation: Old
     ** 30,000 - 30,005 - Mutation: Special
     */
    ...
    constructor(
        ...
        signerAccount = _whitelist_signer;
    }
    ...
    uint256 public constant MAX_CUBS = 10000;

    uint256 public mintingLimitWhitelist = 1;
    uint256 public mintingLimitPublic = 1;

    uint256 public mintOpenWhitelistTimestamp = 0;
    uint256 public mintOpenPublicTimestamp = 0;
    uint256 public mintOpenMutationTimestamp = 0;

    mapping(address => uint256) public mintedDuringWhitelistSale;
    mapping(address => uint256) public mintedDuringPublicSale;

    function setMintingLimits(uint256 _whitelist, uint256 _public) public onlyOwner {
        mintingLimitWhitelist = _whitelist;
    ...
    function setOpenWhitelistTimestamp(uint256 _timestamp) public onlyOwner {
        mintOpenWhitelistTimestamp = _timestamp;
    }
    ...
    function isWhitelistSaleOpen() public view returns (bool) {
        return mintOpenWhitelistTimestamp == 0 ? false : (block.timestamp >= mintOpenWhitelistTimestamp);
    }
    ...
    function areCubsAvailableForSale() public view returns (bool) {
        return LazyCubsInterface.totalSupply() < MAX_CUBS;
    }
    ...
    uint256 public cubsReserved = 0;
    uint256 public constant MAX_RESERVED = 100;

    uint256 public whitelistMintingPrice = 1 ether;

    /*
     ** Dutch Auction Public Sale
     ** Starting Price: 1 ETH
     ** Ending Price: 0.01 ETH
     ** Total Duration: 32400 seconds (9 hours)
     ** Step: 900 seconds (15 minutes)
     ** Step: 0.0275 ETH ((1 ETH - 0.01 ETH)/(32400/900))
     ** Total Steps: 36 (32400/900)
     */
    uint256 public dutchAuctionStartingPrice = 1 ether;
    uint256 public dutchAuctionEndingPrice = 0.01 ether;
    uint256 public dutchAuctionDuration = 9 hours;
    uint256 public dutchAuctionStepDuration = 15 minutes;
    ...
    function reserveCubs(uint256 numberOfTokens) public onlyOwner {
        require(cubsReserved + numberOfTokens <= MAX_RESERVED, "Exceeds reservation limit");

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 nextMint = LazyCubsInterface.totalSupply();
            if (nextMint < MAX_CUBS) {
    ...
    function mintWhitelist(
        ...
        require(isWhitelistSaleOpen(), "Whitelist not open yet");
        require(!isPublicSaleOpen(), "Public Sale already open");
        require(msg.value >= (getCurrentPrice() * numberOfTokens), "Incorrect amount");
        require(mintedDuringWhitelistSale[msg.sender] + numberOfTokens <= mintingLimitWhitelist, "Exceeds limit");
        require(LazyCubsInterface.totalSupply() + numberOfTokens <= MAX_CUBS, "Purchase would exceed max supply");
        ...
        mintedDuringWhitelistSale[msg.sender] += numberOfTokens;
    }
    ...
    function mintPublic(uint256 numberOfTokens) public payable {
        ...
        require(LazyCubsInterface.totalSupply() + numberOfTokens <= MAX_CUBS, "Purchase would exceed max supply");
        require(!isContract(msg.sender) && isSentViaEOA(), "Must be sent via EOA"); // This is to prevent bypassing minting limits using contracts
    ...
    function isLionAlreadyMutated(uint256 lionId, uint256 drinkId) public view returns (bool, uint256) {
        ...
        if (drinkId == 0) {
            // Milk
            mutationId = lionId + 10000;
        } else if (drinkId == 1) {
            // Juice
            mutationId = lionId + 20000;
        } else if (drinkId == 2) {
            // Special
            if (specialMutations[lionId] != 0) {
                mutationId = specialMutations[lionId];
            } else {
                mutationId = 30000 + currentSpecialIndex;
            }
        } else {
    ...
    function mutate(uint256 lionId, uint256 drinkId) public returns (uint256) {
...
contract LazyDrinks is ERC1155, AccessControl, ERC1155Supply {
    ...
    string public name = "Lazy Drinks";
    string public symbol = "DRINKS";
