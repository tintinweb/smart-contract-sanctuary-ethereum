...
import "./erc721vf2/contracts/ERC721VF.sol";
import "./AccessControlVF.sol";
import "./RoyaltiesVF.sol";
import "@openzeppelin/contracts/utils/Context.sol";
import "@openzeppelin/contracts/utils/cryptography/ECDSA.sol";

contract ERC721VFToken2 is ERC721VF {
    using ECDSA for bytes32;

    string private baseURI;
    uint8 private maxBatchAmount;
    address private burnSigner;

    bool public mintingPermanentlyLocked = false;
    bool public isBurnActive = false;

    AccessControlVF controlContract;

    RoyaltiesVF royaltiesContract;

    constructor(
        string memory initialBaseUri,
        string memory name,
        string memory symbol,
        uint8 initialMaxBatchAmount,
        address controlContractAddress
    ) ERC721VF(name, symbol) {
        controlContract = AccessControlVF(controlContractAddress);
        setBaseURI(initialBaseUri);
        setMaxBatchAmount(initialMaxBatchAmount);
    }

    modifier onlyRole(bytes32 role) {
        controlContract.checkRole(role, _msgSender());
        _;
    }
    ...
    function setBaseURI(string memory _baseUri)
        public
        onlyRole(controlContract.getAdmin())
    {
        baseURI = _baseUri;
    }
    ...
    function setRoyaltiesContract(address royaltiesContractAddress)
        ...
        onlyRole(controlContract.getAdmin())
    {
        royaltiesContract = RoyaltiesVF(royaltiesContractAddress);
    }

    function setMaxBatchAmount(uint8 _maxBatchAmount)
        public
        onlyRole(controlContract.getAdmin())
    {
        maxBatchAmount = _maxBatchAmount;
    }

    function setBurnSigner(address signer)
        public
        onlyRole(controlContract.getAdmin())
    {
        burnSigner = signer;
    ...
    function toggleBurnActive() public onlyRole(controlContract.getAdmin()) {
    ...
    function batchAirdrop(
        ...
        uint256 startIndex
    ) external onlyRole(controlContract.getMinter()) notLocked {
        ...
        require(
            totalQuantity <= maxBatchAmount,
            "Batch is greater then max amount"
        );

        for (uint256 i; i < addresses.length; i++) {
            uint256 quantity = quantities[i];
            for (uint256 x; x < quantity; x++) {
                _mint(addresses[i], updatedIndex);
                indexTokenMap[currentIndex] = updatedIndex;
                updatedIndex++;
                if (updatedIndex > maxTokenId) {
                    maxTokenId = updatedIndex;
                }
            }
        }
        currentIndex += totalQuantity;
    }

    function mint(address to, uint256 tokenId)
        external
        onlyRole(controlContract.getSalesContract())
        notLocked
    {
        _mint(to, tokenId);
        indexTokenMap[currentIndex] = tokenId;
    ...
    function adminTransferFrom(
        ...
        address to,
        uint256 tokenId
    ) external virtual onlyRole(controlContract.getAdmin()) {
    ...
    ) external onlyRole(controlContract.getSalesContract()) burnActive {
        bytes32 messagehash = keccak256(abi.encodePacked(burnId, msg.sender));
        address signer = messagehash.toEthSignedMessageHash().recover(
            _signature
        );
        require(signer == burnSigner, "Invaild signer");
    ...
    {
        return royaltiesContract.royaltyInfo(address(this), value);
    }

    function withdrawMoney() external onlyRole(controlContract.getAdmin()) {
 ...
 * {ERC721Enumerable}.
 */
contract ERC721VF is Context, ERC165, IERC721, IERC721Metadata {
    ...
    uint256 internal maxTokenId = 1000000;
    ...
    function checkOwnerOf(uint256 tokenId)
        public
        view
        virtual
        returns (address)
    ...
    function _safeTransfer(
        ...
        _transfer(from, to, tokenId, approvalCheck);
        require(
        ...
        returns (uint256[] memory ownerTokens)
    {
        ...
        if (tokenCount == 0) {
            ...
            uint256 index;
            for (index = 0; index <= maxTokenId; index++) {
                if (checkOwnerOf(index) == _owner) {
...
contract AccessControlVF is Context, ERC165, ReentrancyGuard {
    struct RoleData {
    ...
    uint8 private currentMintIndex = 0;
    ...
    function getAdmin() external view virtual returns (bytes32) {
        return DEFAULT_ADMIN_ROLE;
    }

    function getTokenContract() external view virtual returns (bytes32) {
        return TOKEN_CONTRACT_ROLE;
    }

    function getSalesContract() external view virtual returns (bytes32) {
        return SALES_CONTRACT_ROLE;
    }

    function getMinter() external view virtual returns (bytes32) {
        return MINTER_ROLE;
    }
    ...
    function _selectNextMinter() internal returns (address payable) {
        address nextMinter = minterAddresses[currentMintIndex];
        if (currentMintIndex + 1 < minterAddresses.length) {
            currentMintIndex++;
        } else {
            currentMintIndex = 0;
        }
...
contract RoyaltiesVF is Context {
    struct RoyaltyInfo {
        address recipient;
        uint24 amount;
    }
    ...
    function _setRoyalties(
        ...
        contractRoyalties[contractAddress] = RoyaltyInfo(
            recipient,
            uint24(value)
        );
    }
    ...
    function setRoyalties(
        ...
        address recipient,
        uint256 value
    ) external onlyRole(controlContract.getAdmin()) {
        _setRoyalties(contractAddress, recipient, value);
    }

    function royaltyInfo(address contractAddress, uint256 value)
        external
        view
        returns (address receiver, uint256 royaltyAmount)
    {
        RoyaltyInfo memory royalties = contractRoyalties[contractAddress];
