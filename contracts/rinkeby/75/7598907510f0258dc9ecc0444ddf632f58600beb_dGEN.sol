...
contract dGEN is ERC721A, Ownable {
    ...
    modifier mintCompliance(uint8 _numberOfTokens) {
    require(_numberOfTokens < maxPerWallet + 1, "Slowdown dGen! You cannot mint more than 10 Tokens at once!");
    ...
    require(mintCountPerAddress[msg.sender] + _numberOfTokens <= maxPerWallet, "Wallet mints exceeded");
     mintCountPerAddress[msg.sender] += _numberOfTokens;
    require(tx.origin == msg.sender, "No contract mint");

    _;
}
     function mint(uint8 _numberOfTokens) external payable mintCompliance (_numberOfTokens){
      if(allowlistLive == true){
          require(_allowList[msg.sender] >= _numberOfTokens, "Slowdown dGEN!");
          _allowList[msg.sender] -= _numberOfTokens;

      } else {

          require(publicSaleLive == true, "Steady Lads");
      }
   ...
   function founderClaim(address _to, uint256 _amount) external onlyOwner() {
    ...
    function setMintSettings(bool isSaleLive) public onlyOwner {
    ...
    function setAllowList(address[] calldata addresses, uint8 _numberOfTokens) external onlyOwner {
    for (uint256 i; i < addresses.length;) {
        _allowList[addresses[i]] = _numberOfTokens;
        unchecked { ++i; }
    }
}
