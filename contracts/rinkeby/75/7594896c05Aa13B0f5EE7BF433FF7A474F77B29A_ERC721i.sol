...
contract ERC721i is ERC721Consumable {
  struct Fee {
  ...
  mapping (uint256 => uint256) private tokenIdPointers; // Points to metadata of NFT editions
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721Consumable) returns (bool) {
  ...
  function mint(uint256 _quantity, string memory _metadata, Fee[] memory _fees) external payable {
    uint256 nextTokenId = _nextTokenId();
    for (uint256 i = 0; i < _quantity; i++) {
      tokenIdPointers[nextTokenId + i] = nextTokenId;
    }

    metadata[nextTokenId] = _metadata;

    _safeMint(msg.sender, _quantity);
    _registerFees(nextTokenId, _fees);
  }

  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    uint256 tokenIdentifier = (tokenIdPointers[tokenId] > 0) ? tokenIdPointers[tokenId] : tokenId;
  ...
  function getFeeRecipients(uint256 id) public view returns (address payable[] memory) {
    uint256 tokenIdentifier = (tokenIdPointers[id] > 0) ? tokenIdPointers[id] : id;
