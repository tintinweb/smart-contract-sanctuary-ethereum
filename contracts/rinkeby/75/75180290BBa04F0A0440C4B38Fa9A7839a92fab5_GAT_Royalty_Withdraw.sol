...
contract GAT_Royalty_Withdraw is Ownable, ReentrancyGuard{
  ...
  modifier withdrawCompliance(string calldata _salt) {
    if (tx.origin != msg.sender)
      revert withdrawNoAllowed();
    if(usedMessages[_salt] == true)  
      revert saltAlreadyUsed();
    if(mintContractaddress.getMintAddresses(msg.sender).length > 0)
        revert notAuthorize();
    _;
  }

  function withdraw(uint amount, address payable destAddr, string calldata _salt, bytes calldata _token) external nonReentrant withdrawCompliance(_salt) {
