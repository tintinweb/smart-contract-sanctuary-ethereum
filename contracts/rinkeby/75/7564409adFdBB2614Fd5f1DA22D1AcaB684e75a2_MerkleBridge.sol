...
contract MerkleBridge is Ownable, MerkleProof {
    ...
    /// @param targetChainId target chainId
    function deposit(
        address to,
        uint256 amount,
        uint256 targetChainId
    ) external {
        require(allowedChainIds[targetChainId], 'chainId is not allowed');
        ...
        emit Deposit(block.timestamp, msg.sender, to, amount, CHAIN_ID, targetChainId);
    }
    ...
    /// @param originChainId chain id of target chain
    ...
    function withdraw(
        ...
        bytes32 root = roots[originChainId][epoch];
    ...
    /// @param targetChainId chain id of target chain
    ...
    function transactionHash(
        ...
        return
            keccak256(
                abi.encodePacked(from, to, amount, nonce, originChainId, targetChainId)
            );
    }
