...
import "../interfaces/IMiNFTContract.sol";
import "./MiNFTRegistry.sol";

contract MiNFTFactory is Ownable {
    MiNFTRegistry public immutable registry;
    ...
    constructor(address _registry) {
        registry = MiNFTRegistry(_registry);
    }
    ...
    function deployProxy(bytes32 _type, bytes memory _data)
        ...
        returns (address)
    {
        bytes32 salt = bytes32(registry.count(msg.sender));
    ...
    function deployProxyByImplementation(
        ...
        deployer[deployedProxy] = msg.sender;

        // add deployment to registry
        registry.add(msg.sender, deployedProxy);
...
contract MiNFTRegistry is Ownable {
    ...
    function add(address _deployer, address _proxy) external {
        require(
            owner() == msg.sender || _deployer == msg.sender,
