...
contract PayoutTreasury is Ownable, IPayoutTreasury {
   ...
   */
  function setPayoutEscrowAddress(address _PayoutEscrowAddress)
    external
    onlyOwner
  {
    PayoutEscrowAddress = _PayoutEscrowAddress;
  ...
  function createAndTransferPayout(
    ...
    require(
      address(PayoutEscrowAddress).isContract(),
      "PayoutEscrowAddress is not a contract address"
    );

    // transfer allocated USDC from sender to contract
    ...
    ClaimablePayout memory payout = ClaimablePayout(
      payee,
      totalUsdcAmount,
      tokensToExchange
    );
  ...
  /**
   * @dev Refunds excess USDC from PayoutTreasury
   */
...
interface IPayoutTreasury {
  ...
  // Transfers overallocated USDC back to caller
  function createAndTransferPayout(
    uint256 usdcAmount,
    address payee,
    TokenToExchange[] memory tokensToExchange
...
interface IPayoutEscrow {
  ...
  function claimPayout(IPayoutTreasury.ClaimablePayout calldata claimablePayout)
    external;
}
