...
contract DOTS is ERC721AQueryable, AccessControl {
    ...
    function mint(uint256 _evoNum, uint256 _numDots) external {
        ...
        for (uint i=0; i<_numDots; ) {
            uint256 _gene;
            uint256 _nonce;
            uint256 _rand = getRand(_tokenId);

            while (true) {
                _gene = normalize(_rand);
    ...
    function getRand(uint256 _tokenId) internal view returns (uint256 _rand) {
    ...
    function _getId(
        ...
        uint256 _cutoff,
        uint256 _options
    ) internal pure returns (uint256 _id) {
    ...
    function getSize() external view returns (uint32 size){
