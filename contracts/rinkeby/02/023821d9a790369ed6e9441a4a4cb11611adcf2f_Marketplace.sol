...
contract Marketplace is Ownable, AccessControl {
    ...
    mapping(string => address[]) public NameToAddresses;
    mapping(string => string[]) public NameToDiscordIds;
    ...
    function getAddressesFor(string memory name)
        public
        view
    ...
    function getDiscordIdsFor(string memory name)
        public
        view
        returns (string[] memory)
    {
        return NameToDiscordIds[name];
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        delete NameToAddresses[name];
        delete NameToDiscordIds[name];
    ...
    function buyListing(
        ...
        uint256 i = 0;
        uint256 numSpots = 0;
        address[] memory addresses = NameToAddresses[name];
        uint256 len = addresses.length;
        for (i; i < len; i++) {
            if (addresses[i] == alice) {
                numSpots++;
            }
        }
        ...
        NameToAddresses[name].push(alice);
        NameToDiscordIds[name].push(discordId);
    }
