...
contract VRFConsumerV2 is VRFConsumerBaseV2 {
    ...
    function configureNewAirdrop(string memory nameOfContest, uint32 winnersCount, address[] memory contestantAddressArray, uint256 dateOfAnnouncement, string memory imageURL, uint256 prizeWorth) external onlyOwner {
        address[] memory _winner;
        uint256 _randomSeed;
        AirDropCampaign memory campaign = AirDropCampaign(nameOfContest, winnersCount, contestantAddressArray, _winner, dateOfAnnouncement, false, imageURL, prizeWorth, _randomSeed, false);
        ...
        s_requestIdToContestIndex[requestId] = airdropCampaigns.length;
    }
    ...
    function drawContest(uint contestIndex) external onlyOwner {
        ...
        uint256[] memory tempWinners = shuffle(airdropCampaigns[contestIndex].contestants_addresses.length - 1, airdropCampaigns[contestIndex].randomSeed);
		for (uint i = 0; i < airdropCampaigns[contestIndex].numberOfWinners; i++) {
            airdropCampaigns[contestIndex].winners.push(airdropCampaigns[contestIndex].contestants_addresses[tempWinners[i]]);
        }   
    }
