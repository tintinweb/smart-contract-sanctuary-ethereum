...
contract UnicFactory is IUnicFactory, Initializable, OwnableUpgradeable {
    ...
    function initialize(address _feeToSetter, uint256 _feeDivisor, uint256 _uTokenSupply, address _unic) public initializer {
        __Ownable_init();
    ...
    function setAirdropCollections(address[] calldata _airdrops) onlyOwner external override {
        for (uint8 i = 0; i < _airdrops.length; i++) {
            isAirdropCollection[_airdrops[i]] = true;
        }
    }
