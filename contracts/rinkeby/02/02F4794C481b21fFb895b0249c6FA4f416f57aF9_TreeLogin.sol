...
contract TreeLogin is AccessControl {
    ...
    function deleteOwnershipLinks() external {
    ...
    //manage hotwallets
    function deleteHotWallet(address _hotWallet) internal {
        for (uint256 i = 0; i < hotWallets.length - 1; i++) {
            if (hotWallets[i] == _hotWallet) {
                hotWallets[i] = hotWallets[hotWallets.length - 1];
                hotWallets.pop();
            }
        }
    }

    function addHotWallet(address _hotWallet) internal {
        for (uint256 i = 0; i < hotWallets.length - 1; i++) {
            if (hotWallets[i] == _hotWallet) {
                return;
            }
        }
        hotWallets.push(_hotWallet);
    }

    function getHotWallets()
        external
        view
        onlyRole(MOD_ROLE)
        returns (address[] memory)
