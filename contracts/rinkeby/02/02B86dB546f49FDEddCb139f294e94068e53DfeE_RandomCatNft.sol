...
error RandomNft__NotEnoughFunds();

contract RandomCatNft is ERC721URIStorage, VRFConsumerBaseV2, Ownable {
    enum Breed {
        MIXED,
        PINK,
        ORANGE
    ...
    uint16 private constant REQ_CONFIRMATIONS = 5;
    ...
    event NftMinted(address minter, Breed catBreed);
    ...
    function requestNft() public payable returns (uint256) {
        if (msg.value < i_mintFee) revert RandomNft__NotEnoughFunds();
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        Breed cat = catBreed(moddedRng);
        address minter = s_requestIdToSender[requestId];

        _safeMint(minter, s_tokenCounter);
        _setTokenURI(newTokenId, s_tokenUris[uint256(cat)]);
        s_tokenCounter++;

        emit NftMinted(minter, cat);
    }

    function catBreed(uint256 moddedRng) public pure returns (Breed) {
