...
contract XCC is ERC721A, AuthorityControlled, ReentrancyGuard {
    ...
    function reserve(address recipient, uint256 amount) external onlyManager {
        ...
        onlyManager
    {
        ...
        for (uint256 i = 0; i < _addrList.length; i++) {
            ...
            if (0 >= SafeMath.sub(_latestPrice, SafeMath.mul(floorPrice, i))) {
                _latestPrice = _latestPrice;
            } else {
                _latestPrice = SafeMath.sub(
                    _latestPrice,
                    SafeMath.mul(floorPrice, i)
                );
            }
            if (have) {
                // exist already
            ...
            bidInfo.push(BidInfo({addr: msg.sender, amount: _latestPrice}));
            emit Bided(msg.sender, _latestPrice);
        }
    }
    ...
    function refundIfNoWin() external payable onlyManager {
        require(
            status == Status.Minted,
