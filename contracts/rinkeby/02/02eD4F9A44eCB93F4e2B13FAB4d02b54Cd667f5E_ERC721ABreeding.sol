...
contract ERC721ABreeding is ERC721A, Ownable {
    ...
    function getBreedable(address _address, uint256[] memory _tokenIds, uint256 _type) public view returns (uint256[] memory) {
        ParentContract parentContract = _type == 1 ? dadContract : momContract;
        bool hasBred = false;
        uint256 index = 0;
        uint balance = parentContract.balanceOf(_address);
        uint[] memory tokens = new uint[](balance);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            hasBred = _type == 1 ? isDad[i] : isMom[i];
            if (parentContract.ownerOf(_tokenIds[i]) == _address && !hasBred) {
