...
contract Metabergs is ERC721A, Ownable {
  ...
  bool public reservedMetabergs = false;
  ...
  // Reserve some metabergs for giveaways
  function reserveMetabergs() external onlyOwner {
    // Only allow one-time reservation of 40 tokens
    if (!reservedMetabergs) {
      _mintMetabergs(40);
      reservedMetabergs = true;
    }
  }

  // Internal mint function
  function _mintMetabergs(uint256 numberOfTokens) private {
    ...
    if (earlyAccessMode) {
      earlyAccessMinted[msg.sender] = earlyAccessMinted[msg.sender].add(
        numberOfTokens
      );
    }
  ...
  function mintMetabergs(
    ...
    require(
      msg.value == numberOfTokens.mul(salePrice),
    ...
    if (earlyAccessMode) {
      ...
      bytes32 node = keccak256(
        abi.encodePacked(index, account, whitelistedAmount)
      );
      require(
      ...
      require(
        earlyAccessMinted[msg.sender].add(numberOfTokens) <= whitelistedAmount,
