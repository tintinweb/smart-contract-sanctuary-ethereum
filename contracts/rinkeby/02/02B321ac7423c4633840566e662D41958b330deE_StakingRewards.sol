...
contract StakingRewards is ReentrancyGuard {
    ...
    enum PoolStatus { UNKNOWN, NOT_STARTED, STARTED, FINISHED }
    ...
    function getPoolStatus() external view returns (PoolStatus) {
        ...
        if (startTime > 0 && block.timestamp > startTime && block.timestamp < periodFinish) {
    ...
    function withdraw(uint256 amount) public nonReentrant checkStart updateReward(msg.sender) {
    ...
    function getReward() public nonReentrant checkStart updateReward(msg.sender) {
    ...
    function notifyRewardAmount(uint256 reward) external onlyGov updateReward(address(0)) {
        ...
        require(IERC20(rewardsToken).balanceOf(address(this)) >= reward, "Not enough rewards");
    ...
    function setStartTime(uint256 _startTime) external onlyGov {
    ...
    function toggleEmergencyWithdraw() external onlyGov {
