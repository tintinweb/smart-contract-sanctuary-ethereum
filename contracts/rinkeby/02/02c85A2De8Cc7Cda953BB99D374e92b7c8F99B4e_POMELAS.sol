...
contract POMELAS is ERC721A, Ownable, ReentrancyGuard {
  ...
  bool public paused = false; //change later
  bool public whitelistMintEnabled = true; //change later
  bool public revealed = false;

  address[] private _whitelistMinters;
  mapping(address => bool) isMinted;
  mapping(address => bool) isShareHolder;
  uint256 public ShareHolderCount = 0;
  ...
  mapping(uint256 => shareHolderStruct) public shareHolders;
  ...
  function getShareHolderRevenue() public view returns(uint256 _revenue) {
    require(isShareHolder[_msgSender()] == true, "This address is not a share holder");
    for (uint256 i = 0; i < ShareHolderCount; i++) {
      if (shareHolders[i].walletAddress == _msgSender()) return shareHolders[i].revenue;
    }
  }
  ...
  function revealImage(uint256 tokenId) public view returns(string memory) {
    string memory same = "same";
    string memory notSame = "not same";
    if (_msgSender() == _ownershipOf(tokenId).addr) {
      return same;
    } else {
      return notSame;
    }
