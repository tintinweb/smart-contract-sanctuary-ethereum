...
contract BLXMStaker is Initializable, BLXMRewardProvider, IBLXMStaker {
    ...
    function addRewards(uint256 totalBlxmAmount, uint16 supplyDays) external sync onlyOwner override returns (uint256 amountPerHours) {
    ...
    function stake(uint256 amount, address to, uint16 lockedDays) external override sync {
        _amountNotZero(amount);
        TransferHelper.safeTransferFrom(BLXM, msg.sender, getTreasury(), amount);
        _stake(to, amount, lockedDays);
    }

    function reStake(uint256 amount, address to, uint256 idx, uint16 lockedDays) external sync {
        _amountNotZero(amount);
        uint256 rewardAmount = _withdraw(address(this), amount, idx);

        uint256 reStakeAmount = amount + rewardAmount;
        TransferHelper.safeTransfer(BLXM, getTreasury(), reStakeAmount);
        _stake(to, reStakeAmount, lockedDays);
    }

    function withdraw(uint256 amount, address to, uint256 idx) external override sync returns (uint256 rewardAmount) {
        _amountNotZero(amount);
        rewardAmount = _withdraw(to, amount, idx);
    }

    function _amountNotZero(uint amount) internal pure {
...
contract BLXMRewardProvider is ReentrancyGuardUpgradeable, BLXMMultiOwnable, BLXMTreasuryManager, IBLXMRewardProvider {
    ...
    function _addRewards(uint totalAmount, uint16 supplyDays) internal nonReentrant returns (uint amountPerHours) {
    ...
    function _stake(address to, uint amount, uint16 lockedDays) internal nonReentrant {
    ...
    function _withdraw(address to, uint amount, uint idx) internal nonReentrant returns (uint rewardAmount) {
