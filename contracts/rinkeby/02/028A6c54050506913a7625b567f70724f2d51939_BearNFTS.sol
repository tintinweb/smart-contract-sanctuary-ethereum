...
contract BearNFTS is Ownable, ERC721A {
    ...
    mapping(uint256 => RewardOwnership) private _rewardOwnership;

    constructor() ERC721A("BearNFTS", "BearNFTS", 10, 10000) {}
        ...
        onlyOwner
    {
        for (uint256 index = 0; index < tokenIds.length; index++) {
            ...
            _rewardOwnership[tokenIds[index]] = RewardOwnership(
                accounts[index],
    ...
    function _rewardExits(uint256 tokenId) internal view returns (bool) {
        return _rewardOwnership[tokenId].addr != address(0);
    }

    function changeRestBulls(bool status) public onlyOwner {
        restBulls = status;
    }
    ...
    function _beforeTokenTransfers(
        ...
        for (uint256 end = tokenId + quantity; tokenId < end; ++tokenId) {
            require(restingStarted[tokenId] == 0, "Bull: resting");
        }
    }
    ...
    function toggleResting(uint256 tokenId) internal {
        ...
        uint256 start = restingStarted[tokenId];
        if (start == 0) {
            require(restBulls, "Bull: resting closed");
    ...
    IERC721Enumerable
{
        ...
        returns (uint256)
    {
        ...
        uint256 numMintedSoFar = collectionSize;
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        // return _ownerships[tokenId] < currentIndex;
    ...
    function _safeMint(
        ...
        _addressData[to] = AddressData(
            addressData.balance + uint128(tokenIds.length),
            addressData.numberMinted + uint128(tokenIds.length)
        );
