    ...
    IStaking
{
     ...
     */
    function initialize(address _erc20Token, address _erc721Token,address adminRole,uint256 _accPerShare)
        public
        initializer
    {
        require(_erc20Token != address(0), "MErn_DAO: initializing with zero address");
        require(_erc721Token != address(0), "MErn_DAO: initializing with zero address");
    ...
    /**
     * @dev setAccPErShare so that only owner can update accPerShare.
     */
    function setAccPerShare(uint256 amount) 
        public
        onlyOwner 
    {
        accPerShare=amount;
    ...
    function deposit(uint256[] calldata tokenIds) external {  
        ...
        for (uint256 i; i < tokenIds.length; i++) {
             require(
                !hasStaked(msg.sender, tokenIds[i]),
                "Mern_DAO: Given nft is already staked by user"
            );
            address owner=IERC721Upgradeable(ERC721_CONTRACT).ownerOf(tokenIds[i]);
            require(
                owner!=address(0) && owner!=msg.sender,
                "Mern_DAO: The owner of this token Id is different ");
            require(
                tokenIds[i]<ERC721EnumerableUpgradeable(ERC721_CONTRACT).totalSupply(),
                "Mern_DAO: This token id has not minted yet");
        ...
        uint256 userShare= tokenIds.length.mul(accPerShare).div(10000);
        rewardDebt[msg.sender] +=userShare;
        MyToken(ERC20_CONTRACT).mint(msg.sender,userShare);
    ...
    function withdraw(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            require(
                hasStaked(msg.sender, tokenIds[i]),
                "Mern_DAO:NFT has not staked yet ");
        ...
        uint256 userShare= tokenIds.length.mul(accPerShare).div(10000);
        rewardDebt[msg.sender] -=userShare;
        MyToken(ERC20_CONTRACT).burn(userShare);
    ...
    /**
     * @dev Get true or false that particular id has staked by given user
     *
     * @param account. User address to check token has staked or not.
     * @param tokenId. Token Id for that we checked
     */
    function hasStaked(
        address account, 
        uint256 tokenId
        ...
        returns(bool)
    {
        return _deposits[account].contains(tokenId);
    }
...
pragma solidity ^0.8.11;
interface IStaking{
    function hasStaked(address account,uint256 tokenId) external view returns (bool);
}
    ...
    UUPSUpgradeable 
{
        ...
        override
    {
        require(
            ...
            "Mern_Dao: Non-Transferable Token"
        );
