...
contract NFTL is Ownable, ERC721A, ReentrancyGuard {
    ...
    function presaleEligibility(address userAddress) public view returns(bool) {
        ...
        ownTokensCount = ownTokensCount + RektCats.balanceOf(userAddress);
        ownTokensCount = ownTokensCount + Nouns.balanceOf(userAddress);
        ownTokensCount = ownTokensCount + Cryptoadz.balanceOf(userAddress);
        ownTokensCount = ownTokensCount + Blitmap.balanceOf(userAddress);
        ownTokensCount = ownTokensCount + ChainRunners.balanceOf(userAddress);
        return ownTokensCount > 0;
    }
