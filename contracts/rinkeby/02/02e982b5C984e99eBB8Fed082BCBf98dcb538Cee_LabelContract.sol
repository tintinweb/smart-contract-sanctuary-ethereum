...
contract LabelContract {
    ...
    //later: protect against the case where contract changes 
    ...
    function getLabelData(address _address) public view returns (uint256) {
        uint256 retVal = labelInfo[_address].labelValues;
        return retVal; // Defaults to a LabelData with no real value (or proofs)
    }

    function getProofs(address _address) external view returns (string[] memory) {
        return labelInfo[_address].proofs;
    }
    ...
    function editLabelData(address _address, string calldata _label, bool _labelValue, string calldata _labelProof) public { 
        require(whiteListedAuditors[msg.sender], "Auditor/contributor not whitelisted.");
        changeLabelValue(_address, _label, _labelValue, _labelProof);
    }
    ...
    function getProof(address _address, string calldata _label) public view returns (string memory) { 
        uint256 labelIndex = labelIndices[_label];
        require(labelIndex < labelInfo[_address].proofs.length, "Label data does not exist.");
        string memory proof = labelInfo[_address].proofs[labelIndex];
