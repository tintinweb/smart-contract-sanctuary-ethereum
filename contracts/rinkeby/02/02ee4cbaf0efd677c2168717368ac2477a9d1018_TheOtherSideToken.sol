...
contract TheOtherSideToken is ERC20Burnable, Delegated {
    ...
    constructor() ERC20("The Other Side Token V7", "MOONZV7") {
    }
     ...
     */
    function whitelistMint(uint256 _mintableQty, uint _totalQty,bytes32[] calldata _merkleProof, bytes32 _leaf) public {

        //Three required conditions must be passed before public will be able to mint.
        //1. Leaf node must be equal after cryptographically has the qty+owner address
        //2. Minted qty + mintable qty should be less than or equal to total Qty
        //3. Merkle proof, root node and leaf node must be verified.
        require(keccak256(abi.encodePacked(convertQtyWithOwnerToStr(_mintableQty,msg.sender))) == _leaf,"Hashing of Qty+wallet doesn't match with leaf node");
