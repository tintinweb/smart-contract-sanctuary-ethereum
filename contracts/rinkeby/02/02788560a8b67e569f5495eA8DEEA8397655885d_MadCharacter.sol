...
contract MadCharacter is ERC721A, Ownable {
    ...
    uint256 constant MADPASSNORMAL = 1;
    uint256 constant MADPASSTEAM = 2;
    ...
    function mint(uint256[] memory amounts) external {
        uint256[] memory ids;
        ids[0] = MADPASSNORMAL;
        ids[1] = MADPASSTEAM;
        IMadPass(MADPASS).safeBatchTransferFrom(
            msg.sender,
            address(this),
            ids,
            amounts,
        ...
        IMadPass(MADPASS).burnBatch(ids, amounts);
        _safeMint(msg.sender, amounts[0] + amounts[1]);
    }
