...
contract TimeLockPool1 is BasePool, ITimeLockPool {
    ...
    // Allows a user to unstake a deposit, it can be a partial amount of the total deposited amount 
        ...
        (uint256 totalWithdrawable, uint256 lastTotalBeforeLessPerToken, uint256 lastAdminWithdrawalIndex, bool indexChanged) = _getWithdrawableAmount(_depositId);
        ...
        require(amountToSend > 0, "TimeLockPool.withdraw: insufficient amount to pay");
    ...
    function withdrawableDepositedAmount(uint256 _depositId) public view returns (uint256){
         (uint256 totalWithdrawable,,,) = _getWithdrawableAmount(_depositId);

        if (!withdrawalsPaused) return totalWithdrawable;

        uint256 pausedAmount = initialBalanceWhenPaused[pauseId][_msgSender()];
        ...
        return maxWithdrawableAmount - userWithdrawalsWhenPaused[pauseId][_msgSender()];
    }
    ...
    function _getWithdrawableAmount(uint256 _depositId) private view returns (uint256, uint256, uint256, bool){
        ...
        ITimeLockPool.Deposit memory userDeposit = depositsOf[_msgSender()][
            _depositId
        ];
...
abstract contract BasePool is IBasePool, AccessControlEnumerable {
    using SafeERC20 for IERC20;
