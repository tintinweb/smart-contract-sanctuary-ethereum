...
abstract contract Ownable is Context {
    ...
    contract RareGobz is ERC721A, Ownable {
    using Strings for uint256;
  ...
  constructor() ERC721A("RareGobz", "RB") {
  }

  
 
  function MAKEGOBZ(uint8 _mintAmount) external payable  {
   ...
   else
{
  ...
  function WLGOBZDROP(uint16 _mintAmount, address _receiver) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
