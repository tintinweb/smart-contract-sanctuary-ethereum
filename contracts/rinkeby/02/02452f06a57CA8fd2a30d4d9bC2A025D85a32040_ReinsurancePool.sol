...
contract ReinsurancePool is AbstractLeveragePortfolio, IReinsurancePool, OwnableUpgradeable {
    ...
    /// @notice add the 20% of premium from demand
    /// @dev access CapitalPool
    /// @param  premiumAmount uint256 the premium amount which is 20% of premium + portion of 80% (in PB case)
    function addPremium(uint256, uint256 premiumAmount) external override onlyCapitalPool {
    ...
    AbstractDependant
{
    ...
    function __LeveragePortfolio_init() internal onlyInitializing {
    ...
    /// @param  premiumAmount uint256 the premium amount which is a portion of 80% of the premium
    function addPremium(uint256 epochsNumber, uint256 premiumAmount) external virtual override;
        ...
        returns (address[] memory _coveragePools)
    {
        ...
        _coveragePools = new address[](to.trySub(offset));
    ...
    function _deployLeverageStableToCoveragePools(
        ...
        if (leveragePoolType == LeveragePortfolio.USERLEVERAGEPOOL) {
            _netMPL = totalLiquidity.mul(_policyBookFacade.userleveragedMPL()).uncheckedDiv(
                PERCENTAGE_100
            );
            ...
            _netMPL = totalLiquidity.mul(_policyBookFacade.reinsurancePoolMPL()).uncheckedDiv(
                PERCENTAGE_100
            );
        }
        _netMPLn = _netMPLn.add(
        ...
        if (deployedAmount > 0) {
            if (deployedAmount >= poolsVDeployedAmount[policyBookAddress]) {
                deployedAmount = deployedAmount.uncheckedSub(
