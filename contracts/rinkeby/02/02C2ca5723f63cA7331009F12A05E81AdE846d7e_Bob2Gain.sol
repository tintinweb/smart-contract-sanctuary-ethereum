...
contract Bob2Gain is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    uint256 public maxSupply = 4567;
    uint256 public devReserved = 167;
    uint256 public wlMaxMintAmountPerTx = 3;
    uint256 public maxMintAmountPerTx = 5;

    MintStatus public mintStatus = MintStatus.PAUSED;

    constructor()ERC721A("Bob2Gain", "B2G")
    {}
    ...
    function devClaim() external onlyOwner {
        require(totalSupply() + devReserved <= maxSupply, "Max supply exceeded!");
        _safeMint(_msgSender(), devReserved);
    }

    function whitelistMint(bytes32[] calldata _merkleProof, uint256 _qty)
        public
        payable
        wlMintCompliance(_qty)
        wlMintPriceCompliance(_qty)
        nonReentrant
    {
        require(
    ...
    function halfWithdraw() public onlyOwner nonReentrant {
        (bool os, ) = payable(owner()).call{value: address(this).balance /2}("");
        require(os);
    }
