...
contract TerminalV1_1 is Operatable, ITerminalV1_1, ITerminal, ReentrancyGuard, Ownable {
  ...
  */
  ...
  function tap(
    ...
    if (_leftoverTransferAmount > 0) IERC20(daliToken).transferFrom(address(this), _projectOwner, _leftoverTransferAmount);
    ...
    returns (uint256 amount)
  {
    ...
    if (amount > 0)
      ...
      IERC20(daliToken).transferFrom(address(this), _beneficiary, amount);
    ...
    for (uint256 _i = 0; _i < _mods.length; _i++) {
      ...
      if (_modCut > 0) {
        ...
        if (_mod.allocator != IModAllocator(address(0))) {
          ...
          IERC20(daliToken).transferFrom(address(this), _mod.beneficiary, _modCut);
        }
      }
