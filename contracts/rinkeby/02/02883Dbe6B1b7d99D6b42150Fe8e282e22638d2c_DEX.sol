...
contract DEX {
    ...
    IERC20 immutable token; //instantiates the imported contract
    ...
    function init(uint256 tokens) public payable returns (uint256) {
        ...
        if (!success) revert DEX__TokenTransferFailed();
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        if (msg.value == 0) revert DEX__NoEth();
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        if (tokenInput == 0) revert DEX__NoTokens();
        ethOutput = price(
            tokenInput,
            token.balanceOf(address(this)),
        ...
        if (!success) revert DEX__TokenTransferFailed();
        (success, ) = msg.sender.call{value: ethOutput}("");
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        ...
        uint256 ethReserve = address(this).balance - msg.value;
        tokensDeposited =
            ((msg.value * token.balanceOf(address(this))) / ethReserve) +
            1;
    ...
    {
        if (liquidity[msg.sender] < amount) revert DEX__NotEnoughLiquidity();
        ...
        if (!success) revert DEX__EthTransferFailed();
        success = token.transfer(msg.sender, token_amount);
        if (!success) revert DEX__TokenTransferFailed();
