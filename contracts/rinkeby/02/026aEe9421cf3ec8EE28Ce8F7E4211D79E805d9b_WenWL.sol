...
contract WenWL is ERC1155, Ownable{
    ...
    function mintRuby(bytes32[] memory proof) external isSecured(1) payable{
        ...
        require(1 + presaleMintCount <= RUBY_SUPPLY,"NOT_ENOUGH_PRESALE_SUPPLY");
        require(mintedRuby[msg.sender] + 1 <= RUBY_LIMIT,"MINTED_RUBY_ALREADY");
        require(balanceOf(msg.sender, RUBY) + 1 <= RUBY_LIMIT,"EXCEED_MINT_LIMIT");
        
        require(msg.value == RUBY_PRICE , "NOT_ENOUGH_ETH");
    ...
    function mintDiamond() external onlyOwner {
        require(balanceOf(msg.sender, DIAMOND) >= DIAMOND_SUPPLY,"EXCEED_MINT_LIMIT");
