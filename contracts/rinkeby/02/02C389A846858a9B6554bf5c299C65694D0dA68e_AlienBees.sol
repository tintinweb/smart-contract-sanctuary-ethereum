...
contract AlienBees is ERC721, Ownable, ReentrancyGuard, RandomlyAssigned{
    ...
    uint256 public constant mintPrice = 1000000000000000;//0.001 ETH//30000000000000000; //0.03 ETH
    uint256 public reservedNFTLimit;
    uint256 public HPLimit;
    uint256 public WhiteListedLimit;
    uint256 public reservedTokensMinted;

    string public baseURI;
    string public constant provenanceHash = "To be replaced with provenance hash";
    ...
    address public constant HPContractAddress = 0xFe58d9806aeba6115dc78080E34B631A7FA273E1;//update live contract address
    address public reservedWalletAddress; 
    mapping(address => bool) public whiteListedWallets;
    mapping(address => uint256) public mintedTokensByAddress;
    constructor(
        uint256 _totalSupply,
        uint256 _reservedNFTLimit,
        uint256 _publicLimit,
        uint256 _whiteListedLimit, 
        uint256 _HivePalsMintLimit,
        string memory _baseURI,
        string memory _placeholderURI,
        address _reservedNFTaddress,
        address[] memory _whiteListedAddresses
    ) ERC721("Alien Bees Club", "ABC") RandomlyAssigned(_totalSupply, 1){

        require(_totalSupply > 0, "The maximum supply value cannot be less than 1.");
        require(_reservedNFTLimit < _totalSupply, "You are not allowed to reserve all NFTs.");

        WhiteListedLimit    =   _whiteListedLimit;
        HPLimit             =   _HivePalsMintLimit;  
        publicLimit         =   _publicLimit;  
        reservedNFTLimit    =   _reservedNFTLimit;
        reservedWalletAddress = _reservedNFTaddress;
        baseURI             =   _baseURI;
        placeholderURI      =   _placeholderURI;

        for(uint32 walletLoop =0; walletLoop < _whiteListedAddresses.length; walletLoop++){
            whiteListedWallets[_whiteListedAddresses[walletLoop]] = true;
        }
    }
    ...
    function mintBee(uint256 quantity) public payable {
        ...
        console.log("totalMintLimit =>>>>", totalMintLimit);
    ...
    function updateBaseURIRevealStatus(bool _isRevealed) external onlyOwner{
    ...
    function updateReservedWalletLimit(uint256 newLimit) public onlyOwner{
        require(newLimit > 0, "Limit should be greater than 0");
        reservedNFTLimit = newLimit;
    }
