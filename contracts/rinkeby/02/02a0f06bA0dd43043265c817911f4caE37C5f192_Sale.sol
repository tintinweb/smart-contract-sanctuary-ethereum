...
contract Sale is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    event Bidded(address indexed buyer, uint256 indexed saleId, uint256 indexed tokenId, uint256 amount, uint256 percentAdminFee, uint256 adminFee, uint256 royaltyFee, uint256 valueForUser, uint256 residualPrice);
    ...
    function creates(ProjectInfo memory _projectInfo, SaleInput[] memory _saleInputs) external onlyProject returns(uint256) {
        ...
        for (uint256 i; i < _saleInputs.length; i++) {
            ...
            _sales[i] = _createSale(++id, _projectInfo, _saleInputs[i]);
        }
    ...
    function _createSale(uint256 saleId, ProjectInfo memory _project, SaleInput memory _saleInput) private returns (SaleInfo memory) {
        ...
        if (_project.isSingle) IOSB721(_project.token).safeTransferFrom(_msgSender(), address(this), _saleInput.tokenId);
        else IOSB1155(_project.token).safeTransferFrom(_msgSender(), address(this), _saleInput.tokenId, _saleInput.amount, "");
        return sale;
    }
    ...
    function _sharing(ProjectInfo memory _project, SaleInfo memory _sale, uint256 _amount, uint256 _price) private {
        ...
        if (_project.isCreatedByAdmin) {
            ...
            // admin fee
            supperAdminProfit = _getAdminFee(_price, approval.percent);
        ...
        emit Bidded(_msgSender(), _sale.id, _sale.tokenId, _amount, percentAdminFee, supperAdminProfit, royaltyProfit, sellerProfit, residualPrice);
    }
    ...
    function close(ProjectInfo memory _project, SaleInfo memory _sale, address[] memory _accountBuyers, bool _isGive) external onlyProject nonReentrant {
        for (uint256 i; i < _accountBuyers.length; i++) {
            if (bills[_sale.id][_accountBuyers[i]].account == _accountBuyers[i]) {
                Bill memory billInfo = bills[_sale.id][_accountBuyers[i]];
...
interface ISale {
    ...
    function creates(ProjectInfo memory _projectInfo, SaleInput[] memory _saleInputs) external returns(uint256);
}
