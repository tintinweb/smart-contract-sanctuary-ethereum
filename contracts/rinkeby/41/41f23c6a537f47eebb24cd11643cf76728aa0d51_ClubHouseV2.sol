...
contract ClubHouseV2 is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    event createdNft(address user, uint256 referenceId, uint256 tokenId, uint256 price, string dollar, uint256 sizeId);

     /**
     * @dev Emitted when token is purchased by user.
     */
    event purchaseNft(address previousOwner, address newOwner, uint256 tokenId, uint256 price, string dollar);
    ...
    function mint(MintOrder2 calldata _order, bytes memory _signature)
        external
        payable
        virtual
        nonReentrant
    ...
    function purchase(Order2 calldata _order, bytes memory _signature)
        external
        payable
        virtual
        nonReentrant
        returns (bool)
    {
        ...
        ClubHouseAddress.transferFrom(ownerOf(_order.id), address(this), _order.id);
        ClubHouseAddress.approve(msg.sender, _order.id);
        ClubHouseAddress.transferFrom(address(this), msg.sender, _order.id);
