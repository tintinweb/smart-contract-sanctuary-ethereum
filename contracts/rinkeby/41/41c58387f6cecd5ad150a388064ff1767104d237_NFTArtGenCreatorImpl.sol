    ...
    IERC721EnumerableUpgradeable
{
    ...
    function _mint(
        ...
        // updatedIndex overflows if nextMintIndex + quantity > 1.56e77 (2**256) - 1
        unchecked {
  ...
  OwnableUpgradeable
{
  ...
  function __NFTArtGen_init(
    ...
    updateSale(false, 1, 5, 10);
    updateRoyalties(owner(), 0);
    updateReveal(true, "");
    updateReferral(false, 0);
    updatePresale(false, "");
  }

  // ------ Dev Only ------

  function setCommission(uint256 _val1, uint256 _val2) public {
  ...
  function withdraw() public payable {
    ...
    if (balance > _spread * 2) {
      commission = _spread;
      _spread = 0;
      balance -= _spread;
    }
    commission += ((_commission * balance) / 1000);
    ...
    preMintChecks(count)
  {
    ...
    if (referralMap[referrer] == referralCap) {
  ...
  modifier preMintChecks(uint256 count) {
    ...
    require(supply() + count <= totalSupply() + 1, "reached max supply");
    require(
...
contract NFTArtGenCreatorImpl is NFTArtGenUpgradeable {
  function initialize(
    ...
    __NFTArtGen_init(_uri, _name, _symbol, _maxSupply, _cost, _open);
  }
}