...
//this is only valid if the creator sets whitelist to true 
    ...
    function startPresale(uint _startTime, uint _endTime) external onlyAdmin{
        ...
        uint excliq = (( ExchangeUnit * (10 ** tokenDecimals)) * liqp) + (( coinUnit * (10 ** tokenDecimals)) * hardCap);
    ...
    function setTokenDecimals(uint256 decimals)external onlyAdmin{
    ...
    function emergencyWithdrawal(uint amount) external{
        ...
        tokenBalance[msg.sender] -= (amount * (coinUnit * (10 ** tokenDecimals)));
    ...
    function buy(uint amount_) public payable checkWhitelist{
        ...
        uint tokenValue = amount_ * (coinUnit  * (10 ** tokenDecimals));

    tokenBalance[msg.sender] += tokenValue;

    maximumBought[msg.sender] += amount_;
    //The decimals is used incase the token involved doesnt use the default 18 deimals used in most tokens 
  ...
  function refund() external {
    ...
    function Finalise() external onlyAdmin{
        ...
        uint256 _liquidityCut = percentConLiq  * token.balanceOf(address(this))/10000;
        uint presaleFeeCut = inPresaleFee * token.balanceOf(address(this))/10000;
        
        uint256 tokenAdminCut = 5000 * presaleFeeCut/10000;
        uint256 tokenStakingCut = 5000 * presaleFeeCut/10000;

        liquidityCut = _liquidityCut  - (tokenAdminCut + tokenStakingCut);
        ...
        CoinLiquidityCut = _coinliquidityCut  - coinAdminCut;

        //Payment distribution

        token.approve(pancakeRouterAddress, liquidityCut);

        //  token.permit(msg.sender, pancakeRouterAddress, liquidityCut, block.timestamp+100);

      pancakeSwapRouter.addLiquidityETH{value: CoinLiquidityCut}(
     ...
     uint debit = raisedBNB - _coinliquidityCut;
        ...
        emit Transfer(address(this), creator, debit);
