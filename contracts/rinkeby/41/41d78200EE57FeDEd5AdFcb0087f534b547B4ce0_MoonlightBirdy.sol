...
contract MoonlightBirdy is ERC721A, Ownable, ReentrancyGuard {
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        ...
        require(_numberMinted(msg.sender) + _mintAmount <= 20, "Moonlight Birdy: Exceeds max mint per wallet!");
        _;
    }
    ...
    function freeMint(uint256 _mintAmount) public {
        require(!paused && isFreeMintOpen, "Moonlight Birdy: Free mint phase not open yet!");
        require(_mintAmount > 0, "Invalid mint amount!");
        require(totalSupply() + _mintAmount <= maxFreeMintSupply, "Moonlight Birdy: Exceeds max free mint supply!");
        require(_numberMinted(msg.sender) + _mintAmount <= maxFreeMintAmountPerAddr, "Moonlight Birdy: Exceeds max free mint per wallet!");
        _safeMint(_msgSender(), _mintAmount);
    }
    ...
    function setMaxFreeMintAmountPerAddr(uint256 _maxFreeMintAmountPerAddr) public onlyOwner {
