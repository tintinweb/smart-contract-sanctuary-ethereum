...
contract RoundContract is AccessControl {
  ...
  modifier inWhitelist() {
    if (_enableWhitelist) {
      require(whiteList[msg.sender], "Round: Account not in whitelist");
    }
    _;
  }
  ...
  function mintTokens(uint nTokens) public payable inWhitelist {
    require(msg.value >= info.mintPrice * nTokens || (isFreeRoundNow() && checkPayableWhitList(msg.sender)), "NFT round: Not enough funds for this round");
    ...
    require(maxPurchase() > userPurchase(msg.sender), "NFT round: you can't mint anymore");
    require(checkWhitelist(msg.sender), "NFT round: You're not in white list");
    require(masterContract.totalSupply() <= masterContract.maxSupply(), "NFT: too many token");
    require(nTokens <= masterContract.maxSupply() - masterContract.totalSupply(), "NFT:  There are not enough tokens left");
    if (isFreeRoundNow() && checkPayableWhitList(msg.sender)) {
    ...
    _userPurchase[msg.sender] += idxs.length;
    info.roundTotalSupply += uint16(idxs.length);

    masterContract.mint(idxs, msg.sender);
  }
}
...
interface IMaster {
  ...
  function mint(uint[] memory tokenIdxs, address from) external;
