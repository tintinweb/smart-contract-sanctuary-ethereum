...
contract Light is ERC721, ThreeChiefOfficersWithRoyalties {
    ...
    ) ERC721(name, symbol) ThreeChiefOfficersWithRoyalties(newChiefFinancialOfficer) {
    ...
    /// @param  dropID The identifier, or batch number, for the drop
    function abortDrop(uint64 dropID) onlyOperatingOfficer external {
    ...
    function setDropPhases(uint64 dropID, DropPhase[] calldata dropPhases) onlyOperatingOfficer external {
        Drop storage drop = drops[dropID];
    ...
    function mintChosen(
        ...
        for (uint256 index=0; index<tokenIDsInDrop.length; index++) {
            require(index < drop.quantityForSale, "Light: invalid token ID");
    ...
    ///                  drop
    function finalizeRandomDrop(uint64 dropID, string calldata password) onlyOperatingOfficer external {
        Drop storage drop = drops[dropID];
    ...
    /// @dev    Randomness is used from the owner's randomization secret as well as each buyer.
    /// @param  dropID The identifier, or batch number, for the drop
    function finalizeRandomDropAndIForgotThePassword(uint64 dropID) onlyOperatingOfficer external {
        Drop storage drop = drops[dropID];
        require(drop.passwordHash != bytes32(0), "Light: this drop does not have a password (anymore)");
        require(drop.quantitySold == drop.quantityForSale, "Light: this drop has not completed selling");
        _addEntropyBit(dropID, uint256(blockhash(block.number - 1)));
    ...
    function freezeMetadataForDrop(uint64 dropID, uint256 quantityToFreeze) external {
        ...
        require(drop.quantityFrozeMetadata < drop.quantityForSale, "Light: all metadata is already frozen");
        while (quantityToFreeze > 0 && drop.quantityFrozeMetadata < drop.quantityForSale) {
            uint256 tokenID = _assembleTokenID(dropID, drop.quantityFrozeMetadata);
            emit PermanentURI(tokenURI(tokenID), tokenID);
            drop.quantityFrozeMetadata++;
            quantityToFreeze--;
        }
    }
    ...
    function setRoyaltyAmount(uint256 newRoyaltyFraction) onlyOperatingOfficer external {
    ...
    function setUnrevealedTokenURI(string calldata newUnrevealedTokenURI) onlyOperatingOfficer external {
    ...
    function tokenURI(uint256 tokenID) public view override (ERC721) returns (string memory) {
        ...
        if (drop.accumulatedRandomness == 0) { // Not randomized
            return string.concat(drop.tokenURIBase, Strings.toString(tokenIDInDrop));
        }
        if (drop.passwordHash != bytes32(0)) { // Randomized but not revealed
...
abstract contract ThreeChiefOfficersWithRoyalties is IERC2981, ERC165 {
    ...
    constructor(address payable newFinancialOfficer) {
    ...
    /// @notice EIP5313 (DRAFT) implementation
