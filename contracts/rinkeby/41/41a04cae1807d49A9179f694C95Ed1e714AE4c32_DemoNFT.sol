...
contract DemoNFT is ExtendERC721, PaymentSplitter {
    ...
    constructor(string memory name, string memory symbol, address[] memory payees, uint256[] memory shares) ExtendERC721(name, symbol) PaymentSplitter(payees, shares) {
    }

    function calculatePrice(uint256 numTokens) public view returns (uint256) {
        require(hasSaleStarted == true, "Sale hasn't started");
        require(totalSupply().add(numTokens) < MAX_TOKENS, "No more tokens");
    ...
    function mint(uint256 numTokens) public payable {
        ...
        require(msg.value >= calculatePrice(numTokens).mul(numTokens), "Amount of Ether sent is not correct.");
    ...
    function magicMint(uint256 numTokens) external  onlyOwner {
        require(totalSupply().add(numTokens) <= MAX_TOKENS, "Exceeds maximum token supply.");
...
contract ExtendERC721 is ERC721Enumerable, IERC721Swappable, AccessControlEnumerable, Ownable {
    ...
    function burn(uint256 _tokenId) public override(IERC721Swappable) {
        require(
