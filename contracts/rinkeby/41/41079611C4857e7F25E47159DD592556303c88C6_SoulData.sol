...
contract SoulData is ISoulData, Adminable {
    ...
    function _svgText(string memory text, string memory fontSize, string memory x, string memory y, string memory rotate) 
        internal 
        pure 
        returns (string memory) 
    {
        if (bytes(text).length > 0) {
            ...
            return string(abi.encodePacked(
                "<text text-anchor='middle' x='",x,"' y='",y,"' fill='#8ecad8' font-family='custom' font-size='",fontSize,
                "' transform='rotate(",rotate,")'>",text,"</text>"
            ));
        }
