 ...
 */
contract ERC721W is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, VRFConsumerBaseV2 {
    using Address for address;
    ...
    struct Request {
        // Address of minter
        address to;
        // Quantity to be minted
        uint32 quantity;
    }

    // Compiler will pack the following 
    //_burnCounter into a single 256bit word.
    ...
    constructor(
        ...
        LINKTOKEN = LinkTokenInterface(linkContract_);
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view override returns (uint256) {
        ...
        uint256 numMintedSoFar = _mintedTokens.length;
        uint256 tokenIdsIdx;
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner = ERC721W.ownerOf(tokenId);
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        for (uint256 i = 0; i < _mintedTokens.length; i += 1) {
    ...
    function createNewSubscription() private {
        ...
        COORDINATOR.addConsumer(_subscriptionId, consumers[0]);
    }
    ...
    function fulfillRandomWords(
        ...
        address to = _requests[requestId].to;
        uint256 quantity = _requests[requestId].quantity;
        _addressData[to].balance += uint64(quantity);
        _addressData[to].numberMinted += uint64(quantity);
        uint256 index;
        uint256 tokenId;
        for (uint256 i; i < randomWords.length; i++) {
            index = randomWords[i] % _availableTokens.length;
            tokenId = _availableTokens[index];
...
contract ERC721WContract is ERC721W, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    modifier onlyFiatMinter() {
        require(msg.sender == fiatMinterAddress, "Not authorized to perform this action");
        _;
    }
    ...
    constructor(
        ...
        initialParameters.symbol, 
        initialParameters.linkContract,
        initialParameters.vrfCoordinator,
        initialParameters.requestConfirmations,
        initialParameters.callbackGasLimit,
        initialParameters.keyHash
    )
    ...
    function getClaimIneligibilityReason(address userWallet, uint256 quantity) public view returns (string memory) {
        if (totalSupply() + quantity <= maxSupply) return "NOT_ENOUGH_SUPPLY";
        if (!saleIsActive) return "NOT_LIVE";
        if (saleIsActive && !preSaleIsActive) return "";
    }
    ...
    function _approveMint(address _address, uint _quantity, bytes32[] memory _proof) private {
        ...
        if(_preSaleIsActive) {
            uint mintedAmount = hasMinted[_address] + _quantity;
            require(mintedAmount <= _maxPerWallet, "Exceeds per wallet presale limit.");
            if (preSalePhase == 1) {
                require(mintpass.balanceOf(_address, mintpassId) > 0, "You do not have a MintPass.");
                require(_quantity <= mintpass.balanceOf(_address, mintpassId), "Exceeds wallet presale limit.");
                if (burnMintPass) mintpass.burnForAddress(mintpassId, _quantity, _address);
            }
            if (preSalePhase == 2) {
            ...
            hasMinted[_address] = mintedAmount;
        } else {
            require(_quantity <= _maxPerTransaction, "Exceeds per transaction limit for public sale.");
        }
    }

    function mint(uint32 _quantity, bytes32[] memory _proof) public payable nonReentrant {
        require(price() * _quantity <= msg.value, "ETH sent is incorrect.");
        _approveMint(msg.sender, _quantity, _proof);
        _safeMint(msg.sender, _quantity);
    }

    function claimTo(address _address, uint32 _quantity) public payable nonReentrant onlyFiatMinter {
        require(fiatMinterAddress != address(0), "Fiat minter is undefined.");
        require(price() * _quantity <= msg.value, "ETH sent is incorrect.");
        _safeMint(_address, _quantity);
    }

    function claimFree(uint32 _quantity, bytes32[] memory _proof) public nonReentrant {
    ...
    function reserve(address _address, uint32 _quantity) public nonReentrant onlyOwner {
