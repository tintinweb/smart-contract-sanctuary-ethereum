...
contract VoteBox {
    ...
    // Min SHUB rate of totalSupply for creating a new proposal
    uint256 public constant MIN_PROPOSAL_RATE = 10**16; // 1%
    ...
    /**
     * @dev    Constructor
     * @param  shubAddress SHUB address
     */
    constructor(address shubAddress)
        public
    {
        shub = IERC20(shubAddress);
    ...
    function propose(string calldata link, uint256 beginBlock, uint256 endBlock)
        external
    {
        uint256 minProposalSHUB = shub.totalSupply().mul(MIN_PROPOSAL_RATE).div(10**18);
        require(shub.balanceOf(msg.sender) >= minProposalSHUB, "proposal privilege required");
