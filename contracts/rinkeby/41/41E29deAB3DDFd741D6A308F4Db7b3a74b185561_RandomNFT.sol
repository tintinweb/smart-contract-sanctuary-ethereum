...
error RandomNFT__TrasnfertError();
...
contract RandomNFT is ERC721URIStorage, VRFConsumerBaseV2 {
    ...
    string[3] private i_dogTokenUris;
    ...
    mapping(uint256 => address) private requestIdToNFTOwner;
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords)
        ...
        override
    {
        address nftOwner = requestIdToNFTOwner[requestId];
        _mint(nftOwner, s_tokenCounter);

        uint256 randomNumber = randomWords[0] % 100;
        Breed dogBreed = _getDogClass(randomNumber);

        string memory tokenURI = i_dogTokenUris[uint256(dogBreed)];
    ...
    function withdraw() public payable onlyOwner {
        ...
        if (!callSuccess) {
            revert RandomNFT__TrasnfertError();
        }
    }

    function _getDogClass(uint256 randomNumber) internal pure returns (Breed) {
        if (randomNumber < 10) {
            return Breed.PUG;
        }
