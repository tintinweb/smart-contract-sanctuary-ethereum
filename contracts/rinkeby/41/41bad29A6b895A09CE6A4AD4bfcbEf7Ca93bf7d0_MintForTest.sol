...
contract MintForTest is ERC721A, Ownable, VRFConsumerBase {
  enum SaleStatus {
    PAUSED,
    PRESALE,
    PUBLIC,
  ...
  // ------ Sale Settings
  uint256 private constant PRICE_MFT = 0.0006 ether;
  uint256 private constant MAX_MFT = 40;
  uint256 private constant DEPLOYER_RESERVED_MFT = 10;
  uint256 private constant PUBLIC_MFT_PER_TXN_LIMIT = 3;

  // Remaining public ㅡㄹㅆs (40-5-5-10-10=10)  여기 용도보고 수정 필요
  uint256 public publicMFTRemaining = 10;
  bytes32 public presaleMerkleRoot;
  bytes32 public premintWinnerMerkleRoot;
  address public publicMintSigner;

  mapping(address => bool) public presalePurchased;
  mapping(address => bool) public premintWinnerSalePurchased;
  ...
  ) ERC721A("Mint for test", "MFT") VRFConsumerBase(_vrfCoordinator, _linkAddress) {
    preRevealURI = _preRevealURI;
    withdrawalAddress = _withdrawalAddress;
    chainlinkKeyHash = _chainlinkKeyHash;
    chainlinkFee =  0.25 * 10 ** 18;
    _mint(tx.origin, DEPLOYER_RESERVED_MFT);
  }

  // ------ Prevention from minting off of Contract 다른사람이 실행 시키는거 방지하는거 
  ...
  // After reveal, return post-reveal URI with random token offset from Chainlink 리빌 후에는 랜덤으로 메타데이터 설정됨 = 1등으로 구매했어도 #1을 가지는게 아님 
  ...
  // ------ MERKLE ROOTS 수정 필요 
  function setMerkleRoots(bytes32 _presaleMerkleRoot, bytes32 _premintWinnerMerkleRoot) external onlyOwner {
    presaleMerkleRoot = _presaleMerkleRoot;
    premintWinnerMerkleRoot = _premintWinnerMerkleRoot;
  }

  function processMint(uint256 _quantity) internal {
    require(!revealed, "NO MINTS POSTREVEAL"); // 리빌된그림 민팅 방지 
    require(msg.value == PRICE_MFT * _quantity, "INCORRECT ETH SENT"); // 이상한 가격 구매 방지
    require(totalSupply() + _quantity <= MAX_MFT, "MAX CAP OF BOKI EXCEEDED"); // 수량 이상 구매 방지 
    _mint(msg.sender, _quantity);
  }

  //  ------ PRESALE PRESALE + PRESALE ALLOW LIST SALE
  function presalesSale(bytes32[] memory _proof) external payable callerIsUser {
    require(saleStatus == SaleStatus.PRESALE || saleStatus == SaleStatus.PREMINTWINNER, "PRESALE SALE NOT ACTIVE");
    require(
      MerkleProof.verify(_proof, presaleMerkleRoot, keccak256(abi.encodePacked(msg.sender))),
      "MINTER IS NOT ON PRESALE LIST"
    );
    if (saleStatus == SaleStatus.PRESALE) {
      require(!presalePurchased[msg.sender], " PRESALE TICKET ALREADY USED"); // 재구매 방지 
      presalePurchased[msg.sender] = true;
    } else {
      require(!premintWinnerSalePurchased[msg.sender], "PREMINT WINNER  TICKET ALREADY USED");
       premintWinnerSalePurchased[msg.sender] = true;
    }
  ...
  function allowlistSale(bytes32[] memory _proof) external payable callerIsUser {
    require(saleStatus == SaleStatus.PREMINTWINNER, "PREMINTWINNER MINTING IS NOT ACTIVE");
    require(
      MerkleProof.verify(_proof, premintWinnerMerkleRoot, keccak256(abi.encodePacked(msg.sender))),
      "MINTER IS NOT ON PREMINT WINNER"
    );
    require(!presalePurchased[msg.sender], "PREMINT TICKET ALREADY USED");
    presalePurchased[msg.sender] = true;
  ...
  function publicSale(
    ...
    require(saleStatus == SaleStatus.PUBLIC || saleStatus == SaleStatus.FINAL, "PUBLIC SALE IS NOT ON");
    require(saleStatus == SaleStatus.FINAL || publicMFTRemaining - _quantity >= 0, "PUBLIC CAP EXCEEDED"); // 수량 이상 민팅 방지 
    require(_quantity <= PUBLIC_MFT_PER_TXN_LIMIT, "QUANTITY SURPASSES PER-TXN LIMIT");  //개인 수량 초과 방지
    validateSignature(msg.sender, _nonce, _signature);

    publicMFTRemaining -= _quantity; // 산만큼 수량 제거 
