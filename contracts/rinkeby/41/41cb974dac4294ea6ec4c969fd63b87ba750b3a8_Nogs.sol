...
contract Nogs is ERC721A, Ownable {
    ...
    function internalMint(address receiver, uint256 quantity) private {
    ...
    function constructTokenId(uint256 tokenId, address minterAddress) public view returns (Structs.Nog memory) {
        ...
        if (backgroundOdds >= 68 && backgroundOdds < 75) { background = 1; }
        if (backgroundOdds >= 75 && backgroundOdds < 82) { background = 2; }
        if (backgroundOdds >= 82 && backgroundOdds < 91) { background = 3; }
        if (backgroundOdds >= 91 && backgroundOdds < 100) { background = 4; }

        return
            Structs.Nog({
                minterAddress: minterAddress,
                nogStyle: uint16(uint256(descriptor.getPseudorandomness(tokenId, 5))) % 5,
    ...
    function ownerMintForOthers(address receiver) external onlyOwner {
        uint256 supply = totalSupply();
        uint256 tokenId = supply + 1;
        internalMint(receiver, tokenId);
    }
