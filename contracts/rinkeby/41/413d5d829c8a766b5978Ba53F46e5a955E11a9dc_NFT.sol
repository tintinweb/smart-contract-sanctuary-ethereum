...
contract NFT is ERC721, PullPayment, Ownable {
    ...
    function getPrice(uint256 tokenId, uint256 count) public pure returns (uint256) {
        // uint256 tokenId = currentTokenId.current();
    ...
    // function multipleMintTo(address recipient, uint256 count) public payable
    // {
    //     require(mintingOpen == true, "Minting is not open right now!");
    ...
    function multipleMintToAdmin(address recipient, uint256 count) public onlyOwner {
        uint256 tokenId = currentTokenId.current();
        require(tokenId + count < TOTAL_SUPPLY, "Max supply reached");
