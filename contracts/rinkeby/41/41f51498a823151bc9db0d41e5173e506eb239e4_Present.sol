...
contract WriteSVG {
    ...
    function write(string memory text, string memory color, uint256 spacing, uint256 y) public view returns (string memory) {
        uint256 letterPos = 0;
        ...
        for (uint256 i = 0; i < bytes(text).length; i++) {
						...
						string memory normalized = string(firstCharByte);
