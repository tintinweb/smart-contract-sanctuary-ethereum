...
contract JellyRewarder is IJellyRewarder, IJellyContract {
    ...
    // PW: TODO This needs to set the staking address, needs to be redone, think about when we have multiple pools
    ...
    function setPoolPoints(address _poolAddress, uint256 _poolPoints) public override {
        ...
        if (tokenPoolToId[_poolAddress] == 0 && tokenPools[0] != _poolAddress ) {
            uint256 poolId = tokenPools.length;
            tokenPoolToId[_poolAddress] = poolId;
            tokenPools.push(_poolAddress);
            emit TokenPoolAdded(_poolAddress, poolId);
        ...
        rewardData.rewardPoints = rewardData.rewardPoints - _pool.poolPoints + BoringMath.to32(_poolPoints);

        /// PW: virtual check to avoid overflow
        ...
        returns(bool)
    {
        ...
        if (block.timestamp <= uint256(_pool.lastRewardTime) || startTime == 0 ) {
            return false;
        }

        /// @dev check that the rewards have started
        if(block.timestamp <= startTime) {
            poolData[_poolAddress].lastRewardTime = BoringMath.to48(block.timestamp);
            return false;
        }
        // PW: Is the above check not needed because of the check below?
        ...
        /// PW: This has the effect of reducing/delaying rewards if nothing is staked.
        if (m_net == 0) {
            _pool.lastRewardTime = BoringMath.to48(block.timestamp);
            return false;
        } else {
            uint256 rewards = poolRewards(_poolAddress, rewardsToken, uint256(_pool.lastRewardTime), block.timestamp);
            if ( rewards > 0 ) {
                // PW: Do we need to keep track of pool level rewards?
        ...
        /// @dev update weighs between pools ( only if dynamic weights )
        _updateWeights();
    ...
    /// We will move to a new period based rewards, replacing the above.
    /// This will help us adjust rewards day by day or weekly in the first few weeks of launch.
    /// Still need to update deploy and test scripts to input each period rewards. 
    /// PW: Check that pool points changing over time is ok, has no historical effects
    function poolRewards(address _poolAddress, address _rewardToken, uint256 _from, uint256 _to) public override view returns (uint256 rewards) {
        Rewards memory _rewardInfo = rewardData;
        uint256 poolPoints = uint256(poolData[_poolAddress].poolPoints);

        if (_to <= uint256(_rewardInfo.startTime)) {
            return 0;
        }
        if (_from < uint256(_rewardInfo.startTime)) {
            _from = uint256(_rewardInfo.startTime);
        }
        if (poolPoints == 0) {
            return 0;
        }
        uint256 startPeriod = diffDays(uint256(_rewardInfo.startTime), _from) / periodLength;
        uint256 endPeriod = diffDays(uint256(_rewardInfo.startTime), _to) / periodLength;

        // address tokenPool = tokenPools[_poolAddress];
        // address tokenPool = tokenPoolTemplate[_poolAddress];
        if (startPeriod == endPeriod) {
            return _rewardsFromPoints(periodRewardsPerSecond[startPeriod],
                                    _to - _from,
                                    poolPoints,
                                    _rewardInfo.rewardPoints);
        }
        /// @dev First count remainer of first week 
        /// @dev End of the first from period, minus the from timestamp gives the remaider
        uint256 pSeconds = periodInSeconds();
        uint256 initialRemander = uint256(_rewardInfo.startTime) + ((startPeriod+1) * (pSeconds)) - _from;

        rewards = _rewardsFromPoints(periodRewardsPerSecond[startPeriod],
                                    initialRemander,
                                    poolPoints,
                                    _rewardInfo.rewardPoints);
        ...
        uint256 finalRemander = _to - (endPeriod * pSeconds + uint256(_rewardInfo.startTime));
        rewards = rewards + _rewardsFromPoints(periodRewardsPerSecond[endPeriod],
                                    finalRemander,
                                    poolPoints,
                                    _rewardInfo.rewardPoints);
        return rewards;
    ...
    function _updateWeights(
        ...
        internal
    {
        // PW: This is blank for basic pools
    }
    ...
    function totalRewards(address _rewardsToken) public override view returns (uint256) {
        ...
        /// @dev add multiples of the week
        for (uint256 i = 0; i < tokenPools.length; i++) {
            poolRewardsCount += poolRewards(tokenPools[i], _rewardsToken, uint256(poolData[tokenPools[i]].lastRewardTime), block.timestamp);
        }
