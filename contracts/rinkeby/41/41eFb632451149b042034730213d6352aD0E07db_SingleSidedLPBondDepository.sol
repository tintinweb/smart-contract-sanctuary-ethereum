...
contract SingleSidedLPBondDepository is Ownable, ReentrancyGuard {
    ...
    function deposit(
        ...
        // FIXME can balance pool here, can add + or - 20% tokens
        uint _usdbAmount = _amount;
    ...
    function redeem(address _recipient, uint _amount, uint _amountMin) external nonReentrant returns (uint) {
        ...
        // withdraw dai
        (, uint _principleAmount) = exitPool(_amount, 1);
        require(_principleAmount >= _amountMin, "Slippage limit: more than amountMin");

        uint toReturnBack = 0;
        if (_principleAmount > info.payout) {
            toReturnBack = _principleAmount.sub(info.payout);
            _principleAmount = info.payout;
        }

        // put rest of dai back
        uint restOfLpTokenAmount = joinPool(toReturnBack, 0);
        // withdraw usdb
        (uint _usdbAmount,) = exitPool(restOfLpTokenAmount, 0);
