...
contract Administration {
  ...
  function deleteOffice(address _hgsBoxOfficeAddress) public onlyOwner {
  ...
  function getHGSNumber(address _owner) public view returns (uint64) {
    return registeredVehicles[_owner].HGSNumber;
  }

  function getHGSNumber() public view returns (uint64) {
    return registeredVehicles[msg.sender].HGSNumber;
  }

  function getName(address _owner) public view returns (string memory) {
    return registeredVehicles[_owner].name;
  }

  function getName() public view returns (string memory) {
    return registeredVehicles[msg.sender].name;
  }

  function getSurname(address _owner) public view returns (string memory) {
    return registeredVehicles[_owner].surname;
  }

  function getSurname() public view returns (string memory) {
    return registeredVehicles[msg.sender].surname;
  }

  function getVehicleType(address _owner) public view returns (uint8) {
    vehicleType class = registeredVehicles[_owner].vehicleClass;
    if (class == vehicleType.car) return 1;
    else if (class == vehicleType.bus) return 2;
    else return 3;
  }

  function getVehicleType() public view returns (uint8) {
    vehicleType class = registeredVehicles[msg.sender].vehicleClass;
    if (class == vehicleType.car) return 1;
    else if (class == vehicleType.bus) return 2;
    else return 3;
  }

  function getCrossingHistories(address _owner)
    public
    view
    returns (uint64[] memory)
  {
    return registeredVehicles[_owner].crossingHistories;
  }

  function getCrossingHistories() public view returns (uint64[] memory) {
    return registeredVehicles[msg.sender].crossingHistories;
  }
}
...
contract HGSBoxOffice {
  ...
  uint256 public immutable CAR_FEE_USD;
  uint256 public immutable MINIBUS_FEE_USD;
  uint256 public immutable BUS_FEE_USD;
  ...
  modifier notRecorded() {
    if (admin.getHGSNumber(msg.sender) == 0) revert HGSBoxOffice__NotRecorded();
    _;
  }

  modifier lessFee(uint8 vehicleClass, uint256 fee) {
    if (
      (vehicleClass == 1 && fee < CAR_FEE_USD) ||
      (vehicleClass == 2 && fee < MINIBUS_FEE_USD) ||
      (vehicleClass == 3 && fee < BUS_FEE_USD)
    ) {
  ...
  function crossing()
    ...
    notRecorded
    lessFee(
      admin.getVehicleType(msg.sender),
  ...
  function getVehicle(address _owner)
    ...
    returns (
      ...
      uint64[] memory CrossingHistories
    )
  {
    uint8 vehicleClass = admin.getVehicleType(_owner);
    if (vehicleClass == 1) {
      return (
        admin.getHGSNumber(_owner),
        admin.getName(_owner),
        admin.getSurname(_owner),
        "car",
        admin.getCrossingHistories(_owner)
      );
  ...
  {
    uint8 vehicleClass = admin.getVehicleType(msg.sender);
    if (vehicleClass == 1) {
      return (
        admin.getHGSNumber(msg.sender),
        admin.getName(msg.sender),
        admin.getSurname(msg.sender),
        "car",
        admin.getCrossingHistories(msg.sender)
      );
    } else if (vehicleClass == 2) {
      return (
        admin.getHGSNumber(msg.sender),
