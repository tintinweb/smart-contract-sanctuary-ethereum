...
contract ERC20DividendToken is ContextUpgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    function __ERC20DividendToken_init(string memory name_, string memory symbol_, uint256 totalSupply_, address routerV2_, address devAddress_, address buyBackAddress_, address treasuryAddress_) internal onlyInitializing {
        ...
        changeMaxWalletAmount(1_000_000 * (10**9)); // 1M
        changeMaxTxAmount(1_000_000 * (10**9)); // 1M

        _deployDividendTracker(routerV2_);

        _updateUniswapV2Router(routerV2_);
        updateGasForProcessing(300000);
    ...
    function _deployDividendTracker(address _uniswapV2Router) internal {
        ...
        dividendTracker.excludeFromDividends(_uniswapV2Router);
    ...
    function setSellFees(uint256 _rewardsFee, uint256 _buyBackFee, uint256 _treasuryFee) public onlyOwner {
        rewardsFee = _rewardsFee;
        buyBackFee = _buyBackFee;
        treasuryFee = _treasuryFee;

        totalSellFees = rewardsFee.add(buyBackFee).add(treasuryFee);
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "ERC20DividendToken: gasForProcessing must be between 200,000 and 500,000"
        );
    ...
    function _transfer(
        ...
        if (
            ...
            if (_liquidityPending > 0) {
                swapAndLiquify(_liquidityPending);
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from] && totalBuyFees > 0) { // BUY
                ...
                _liquidityPending += fees.mul(liquidityFee).div(totalBuyFees);
...
contract TokenDividendTracker is Ownable, DividendPayingToken {
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ERC20DividendToken contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "Dividend_Tracker: claimWait must be updated to between 1 second and 24 hours"
        );
        require(
...
contract BATL is Initializable, ERC20DividendToken {
    ...
    function initialize(address _routerAddress, address _devAddress, address _buyBackAddress, address _treasuryAddress) initializer public {
        __ERC20DividendToken_init("test", "test", 100_000_000_000, _routerAddress, _devAddress, _buyBackAddress, _treasuryAddress);
    }
}
