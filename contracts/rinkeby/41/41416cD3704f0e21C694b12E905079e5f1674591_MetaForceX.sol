...
contract MetaForceX is ERC721A, ERC2981, PaymentSplitter, Ownable {
  ...
  uint16 public remainingFreeMint = 1000;
  uint16 public freeMintPerWalletLimit = 2;
  ...
  event RemainingFreeMintUpdated(uint16 newLimit);
  ...
  function updateCollectionSize(uint16 _limit) external onlyOwner {
    collectionSize = _limit;
  ...
  function updateRemainingFreeMint(uint16 newRemainingFreeMint) external onlyOwner {
    remainingFreeMint = newRemainingFreeMint;
    emit RemainingFreeMintUpdated(newRemainingFreeMint);
  }
  ...
  function freemint(uint16 count) external {
    require(isFreeMintingOpened, "Free minting is closed");

    uint64 freemintCounter = _getAux(msg.sender) + count;
    require(freemintCounter <= freeMintPerWalletLimit, "Max wallet limit");
    _setAux(msg.sender, freemintCounter);

    remainingFreeMint -= count;
