...
contract testI is ERC721A, Ownable, ReentrancyGuard {
    ...
    mapping (address => uint256) public chosenBurned;
    ...
    function becomeChosen() external nonReentrant callerIsUser {
    ...
    function sacrifice(uint256[] calldata tokenIds) external nonReentrant callerIsUser{
        for(uint256 i; i < tokenIds.length;) {
            uint256 tokenId = tokenIds[i];
            _burn(tokenId);
            chosenBurned[msg.sender] += walletLimit;
            unchecked {
                i++;
            }
        }
    ...
    function ownerMint(uint256 amount) external onlyOwner {
        require(totalSupply() + amount <= walletLimit);
