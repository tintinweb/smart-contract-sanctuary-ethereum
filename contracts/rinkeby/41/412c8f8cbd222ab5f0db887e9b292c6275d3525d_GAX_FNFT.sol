...
contract GAX_FNFT is ERC721Upgradeable {
    ...
    function transferToken(
     ...
     function callBackTransferToken(
        ...
        emit ArtbitTransferred(_from, _to, _assetId, _artBits, _royaltyArtbits, _transferArtbits, _royaltyPercentage, _transferType);
    }
...
contract GAX_Artbits is Singleton, ERC20Upgradeable {
    ...
    function transferArtbits(
        ...
        if (
            ...
            recipient == gaxAdmin ||
            sender == gaxAdmin ||
            gaxInstance.isDexAddress(recipient) // add liquidity for dex cant count here

        ) {
            transferAmount = amount;            
            gaxInstance.callBackTransferToken(sender, recipient, nftId, amount, 0, transferAmount, 0, "whitelist");
        }
        // DEX transaction NFT_Asset owner receive royalty
            ...
            gaxInstance.callBackTransferToken(sender, recipient, nftId, amount, royalty_amt, transferAmount, drp , "DEX");
        } else {
           uint256 p2p = p2p_rp;  
           if(p2p_rp == 0) {p2p = gaxInstance.p2p_rp();}
            ...
            gaxInstance.callBackTransferToken(sender, recipient, nftId, amount, commission, transferAmount, p2p, "P2P");
        }
