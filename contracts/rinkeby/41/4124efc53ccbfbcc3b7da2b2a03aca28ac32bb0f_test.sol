...
contract test {
    ...
    function batchSearchTesting(uint256 amount) public returns (STRUCT[] memory) {
        STRUCT[] memory tmp = new STRUCT[](amount);
        
        tmp[0] = arrayOfStruct[0];
        tmp[1] = arrayOfStruct[1];
        tmp[2] = arrayOfStruct[2];
