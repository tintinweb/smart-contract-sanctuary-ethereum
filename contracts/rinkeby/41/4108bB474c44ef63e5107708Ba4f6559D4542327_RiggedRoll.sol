...
contract DiceGame {
    ...
    function rollTheDice() public payable {
        ...
        console.log("DG block number", block.number - 1);
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(this), nonce));
        console.log("DG nonce", nonce);
        console.log("DG address", address(this));


        uint256 roll = uint256(hash) % 16;

        console.log("DG Roll: ",roll);
...
contract RiggedRoll is Ownable {
    ...
    function riggedRoll() public {
        bytes32 prevHash = blockhash(block.number - 1);
        console.log("RR blocknumber", block.number - 1);

        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(diceGame), diceGame.nonce()));

        uint256 roll = uint256(hash) % 16;

        console.log("RR: THE ROLL IS ",roll);


        //require(roll >= 0 && rorll <= 2, "Bad roll. Don't continue");
