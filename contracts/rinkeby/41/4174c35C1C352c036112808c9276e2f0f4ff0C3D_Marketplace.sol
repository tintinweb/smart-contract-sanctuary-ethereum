...
contract Marketplace is Ownable, Pausable, MarketplaceStorage, IERC721Receiver {
     ...
     */
    constructor(uint256 _ownerCutPerMillion, address _tokenFTVAddress) {
        // Fee init
        setOwnerCutPerMillion(_ownerCutPerMillion);

        // Token address init
        setTokenAddress(_tokenFTVAddress);
    }
    ...
    function createOrder(
        ...
        _createOrder(nftAddress, assetId, priceInWei);
    }
    ...
    function _createOrder(
        ...
        emit PlaceOrder(orderId, assetOwner, nftAddress, assetId, priceInWei);
    }
    ...
    function _executeOrder(
        ...
        if (ownerCutPerMillion > 0) {
            ...
            IERC20(tokenFTVAdress).transferFrom(
                sender,
                address(this),
                saleShareAmount
            );
        ...
        )
    {
        ...
        if (_order.id != 0) return (_order.id, _order.seller, _order.price);
    }

    function withdraw() external onlyOwner {
        IERC20(tokenFTVAdress).transfer(owner(), IERC20(tokenFTVAdress).balanceOf(address(this)));
    }
