...
contract MyEpicNFT is ERC721URIStorage {
	...
	event NewEpicNFTMinted(address sender, uint256 tokenId);			// イベント定義
	...
	function makeAnEpicNFT() public {
		...
		emit NewEpicNFTMinted(msg.sender, newItemId);	// NewEpicNFTMintedイベントがemitされるごとに、コントラクトに書き込まれたデータをフロントに反映させる
		// コントラクトでイベントがemitされると、フロント(App.js)でその情報を受け取る
		// NewEpicNFTMintedイベントがemitされる際、フロントで使用するmsg.sender, newItemIdをフロントに送信している
