...
contract Payments is Ownable {
    ...
    address public chainLinkToken = address(0x01BE23585060835E02B77ef475b0Cc51aA1e0709);
    ...
    struct Projects {
        address owner;
        mapping(uint => PaymentMethod) paymentMethod;
        uint basePayment;
    ...
    function insertNewProject(address owner, uint256 projectId, uint[] calldata paymentMethods_, uint basePayment, uint slippage) public payable onlyParentContract {
        require(
            !projects[projectId].validProject,
        ...
        for (uint256 index = 0; index < paymentMethods_.length; index++) {
            uint paymentMethod = paymentMethods_[index];
            ...
            PaymentMethod storage pmm = projects[projectId].projectPaymentMethods[paymentMethod];
            pmm.balance = 0;
        ...
        uint256 MINETHNEEDED = getEstimatedETHforToken(chainLinkToken, chainLinkFee)[0];
        //slippage to eth amt.
        uint256 MINETHNEEDEDWITHSLIPPAGE = (MINETHNEEDED * slippage / 10000) + MINETHNEEDED;
        require(msg.value >= MINETHNEEDEDWITHSLIPPAGE , "need more eth to do transfer.");
        // function buyTokensWithETH2(uint256 amountEthToSend, uint256 minTokenAmountToRecieve, address tokenAddressToRecieve)
        uint256 balEfter = buyTokensWithETH2(MINETHNEEDEDWITHSLIPPAGE, chainLinkFee, chainLinkToken, msg.value);
        require(validBasePayment, "Base payment method is not valid1.");
        require(IERC20(chainLinkToken).balanceOf(address(this)) >= chainLinkFee, "Could not get enough chainlink!");
    ...
    function getPaymentMethod(uint paymentMethodId) public view returns (bool valid, 
        address paymentMethodAddress, //address 0x0 is valid as main payment method
        uint defaultFeePercentage,
        bool keep,
        uint256 currentSiteBal,
        ...
        return (paymentMethod.valid, paymentMethod.paymentMethodAddress, paymentMethod.defaultFeePercentage, paymentMethod.keep, paymentMethod.currentSiteBal, paymentMethod.totalSiteEarned, paymentMethod.decimals);
    }

    function getProjectPaymentMethod(uint256 projectid, uint paymentMethod) public view returns (uint paymentMethodID, address paymentMethodAddress, uint feePercentage, uint256 balance, bool valid) {
        Projects storage project = projects[projectid];
        PaymentMethod memory paymentMethodStruct = project.projectPaymentMethods[paymentMethod];
        return (paymentMethodStruct.paymentMethodID, paymentMethodStruct.paymentMethodAddress, paymentMethodStruct.feePercentage, paymentMethodStruct.balance, paymentMethodStruct.valid);
    }
    //get payment methods array of uints
    function getProjectPaymentMethods(uint256 projectid) public view returns(uint[] memory _paymentMethods, uint basePayment) {
        Projects storage project = projects[projectid];
    ...
    function editSitePaymentMethods(
        ...
        uint[] calldata fees) public onlyOwner {
        require(
                ...
                && paymentMethods_.length ==  fees.length, "Invalid lengths provided");
        for (uint index = 0; index < paymentMethods_.length; index++) {
                uint _paymentMethod = paymentMethods_[index];
    ...
    function swap1Token2AnotherThroughETH2(address tokensToSwapToMainToken, uint256 amountToSwap, uint slippage, address finalCurrencyToRecieve) public payable returns (uint256) {
        ...
        require(tokenToSwap.transferFrom(msg.sender, address(this), amountToSwap), "token transferFrom failed.");
        require(tokenToSwap.approve(address(uniswapV2Router), amountToSwap), 'approve failed.');
        uint256 MIN_RECIEVE_AMT_ETH = getEstimatedETHforToken(tokensToSwapToMainToken, amountToSwap)[0];
        uint256 slippageAmount = MIN_RECIEVE_AMT_ETH * slippage / 10000;
        uint256 balBeforeEth = address(this).balance;
        _uniswapV2Router.swapExactTokensForETH(amountToSwap, MIN_RECIEVE_AMT_ETH - slippageAmount, getPathToEth(tokensToSwapToMainToken), address(this), block.timestamp + 150);
    ...
    function buyTokensWithETH2(uint256 amountEthToSend, uint256 minTokenAmountToRecieve, address tokenAddressToRecieve, uint256 totalEthAmt) public returns(uint256) {
    ...
    function buyTokensWithETH(uint256 amountEthToSend, address tokensToRecieve, uint slippage) public {
        uint256 slippageAmount = amountEthToSend * slippage / 10000;
        uint256 MIN_RECIEVE_AMT_SECOND_CURRENCY = getEstimatedTokenRecieveForEth(tokensToRecieve, amountEthToSend)[1];
        _uniswapV2Router.swapExactETHForTokens{value: amountEthToSend}(
    ...
    function buyEthWithToken(uint256 tokenAmountToSpend, address tokenAddress, uint256 estimatedEthToRecieve) public returns(uint256 ETHRecieved) {
        // IERC20 token = IERC20(uniswapV2Router.WETH());
        IERC20(tokenAddress).approve(address(uniswapV2Router), tokenAmountToSpend);
    ...
    function buyLinkToken(address tokenToConvertFrom, uint256 fullPaymentAmount, uint256 amountToTokenToBuy, address linkTokenAddress, uint slippage)  public  payable  onlyParentContract returns(uint256) {
        ...
        require(token.transferFrom(msg.sender, address(this), fullPaymentAmount), "token transferFrom failed.");
        ...
        estimated = getEstimatedETHforToken(linkTokenAddress, amountToTokenToBuy + slippage2)[0]; //amount of ETH needed to get the chainlink token.
        ...
        tokensNeededForETHAmt = getEstimatedTokenForETH(tokenToConvertFrom, estimated)[0]; //amount of paymentMethodToken needed to get the eth.
        //
        string memory errorMessage = string(abi.encodePacked("Payment needs at least least: ", uint2str(tokensNeededForETHAmt), " tokens to buy eth!"));
        require(fullPaymentAmount >= tokensNeededForETHAmt, errorMessage);
      
        IERC20(tokenToConvertFrom).approve(address(uniswapV2Router), tokensNeededForETHAmt);
        // buyEthWithToken(tokensNeededForETHAmt, tokenToConvertFrom, (estimated - slippage2));
        uint256 balBefore = address(this).balance; 
        // approve uniswap
        _uniswapV2Router.swapExactTokensForETH(tokensNeededForETHAmt,estimated,getPathToEth(address(tokenToConvertFrom_)),address(this),block.timestamp + 150);
        ...
        _uniswapV2Router.swapExactETHForTokens{value:balBefore}(amountToTokenToBuy,getPathForToken(linkTokenAddress),address(this),block.timestamp + 150);
        IERC20(linkTokenAddress).transfer(parentAddress,  amountToTokenToBuy + slippage2); //transfer link token to parent contract.
    ...
    function getEstimatedTokenNeededToRecieveOtherToken(address inputToken, address outputToken, uint256 outputTokensAmountToGetPriceFor) public view returns (uint256[] memory) {
         return uniswapV2Router.getAmountsOut(outputTokensAmountToGetPriceFor, getPathToTokenFromToken(inputToken, outputToken));
    }
    // GetAmountOut() will return how much ETH you would get for a given TOKEN.
    function getEstimatedETHRecievedForToken(address token, uint256 tokenAmount) public view returns (uint256[] memory) {
    ...
    function getEstimatedTokenForETH(address token, uint256 amountOfETHToConvert) public view returns (uint256[] memory) {
        return uniswapV2Router.getAmountsIn(amountOfETHToConvert, getPathToEth(token));
    }
    function getEstimatedETHforToken(address token, uint256 amountOfTokensToRecieve) public view returns (uint256[] memory) {
        return uniswapV2Router.getAmountsIn(amountOfTokensToRecieve, getPathForToken(token));
    }
    function getEstimatedTokenRecieveForEth(address token, uint256 amountOfTokensToRecieve) public view returns (uint256[] memory) {
        return uniswapV2Router.getAmountsOut(amountOfTokensToRecieve, getPathForToken(token));
    }
    
    function getPathToTokenFromToken(address inputToken, address outputToken) private pure returns (address[] memory) {
    ...
    function getPathToEth(address token) private view returns (address[] memory) {
        address[] memory path = new address[](2);
...
contract Token is ERC1155, Ownable {
    ...
    // function claimNFT(uint256 id) public onlyNFTOwner {}
