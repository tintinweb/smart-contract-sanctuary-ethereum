...
error InvalidAirdropInput();
error TokenIdOutOfRange();

contract Airdrop is ERC1155Supply, AccessControl, ReentrancyGuard {
    bytes32 public constant AIRDROPPER_ROLE = keccak256("AIRDROPPER_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    uint256 public constant TYPE_COUNT = 4;

    ISpendable public Spendable;
    uint256[TYPE_COUNT] public scaleRewards = [
        500 ether,
        250 ether,
        100 ether,
        50 ether
    ...
    constructor(string memory uri, address spendable)
        ERC1155(uri)
    {
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());

        Spendable = ISpendable(spendable);
    }
     ...
     */
    function setURI(string memory uri) external lockable onlyRole(DEFAULT_ADMIN_ROLE) {
        _setURI(uri);
    }

    /**
     * @notice Set $SPENDABLE token address
     * @param spendable Address of $SPENDABLE token contract
     */
    function setSpendable(address spendable) external lockable onlyRole(DEFAULT_ADMIN_ROLE) {
        Spendable = ISpendable(spendable);
    }
    ...
    {
        if (receivers.length != amounts.length) revert InvalidAirdropInput();
        if (tokenId >= TYPE_COUNT) revert InvalidAirdropInput();
    ...
    /**
     * @notice Redeem rewards from airdropped tokens
     * @param ids Token IDs to redeem rewards for
     * @param amounts Amount of token IDs to redeem rewards for
     */
    function redeem(
        ...
        nonReentrant
    {
        ...
        for (uint256 i; i < ids.length; i++) {
            if (ids[i] >= TYPE_COUNT) revert TokenIdOutOfRange();

            totalRedeemed += amounts[i];
            rewards += scaleRewards[ids[i]] * amounts[i];
            _burn(_msgSender(), ids[i], amounts[i]);
        }

        _mint(_msgSender(), TYPE_COUNT, totalRedeemed, "");

        Spendable.credit(_msgSender(), rewards);
    }
