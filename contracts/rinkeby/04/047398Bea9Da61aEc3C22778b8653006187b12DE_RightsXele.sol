...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Governance {
    ...
    function _transfer(
        ...
        if( landNft != address(0x0) )
        {
            IERC4907 _land = IERC4907(landNft);
            _land.setUser(tokenId, to, 0);
        }        
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function _beforeTokenTransfer(
        ...
        {  
            if( to != address(0x0) ){
               require( tiles[tokenId].expire != 0, "Setting Lease blocktime is needed"); 
            }
...
contract RightsXele is ERC721Metadata {
    ...
    uint256 public LeaseMaxt = 3153600;   // Maximum 1 years
    ...
    event SetExpire(uint256 tokenId, uint64 expblock);

    constructor() ERC721("RightsXele", "RXE") { }
    ...
    function setExpiration(uint256 tokenId, uint64 expires) external 
    {
        require( tiles[tokenId].expire == 0, "the expired number has exists" );
        require( expires >  block.timestamp, "timestamp invalid" );
        require( expires <  block.timestamp+LeaseMaxt, "timestamp over max. limit" );
        require(
            _isApprovedOrOwner(_msgSender(), tokenId),
            "operator is not owner nor approved"
        );
        tiles[tokenId].expire = expires;

        IERC4907 _land = IERC4907(landNft);
        _land.setUser(tokenId, _msgSender(), expires);

        emit SetExpire(tokenId, expires);
    }
    ...
    function setLeaseMaxTimeStamp(uint64 maxtimestamp) public onlyGovernance {
        LeaseMaxt = maxtimestamp;
    }
