...
contract HolaMetaversoSpeaker is ERC721Enumerable, Ownable {
  using Strings for uint256;

  uint256 public constant MAX_SUPPLY = 50; //Todo: Verify this total
  ...
  mapping(address => uint256) public speakerTokensMinted;

  constructor() ERC721("HolaMetaversoSpeaker", "HOLASPKR") {
  }

  modifier callerIsUser() {
    require(tx.origin == msg.sender, "HolaMetaversoSpeaker; Cannot be called by another contract");
    _;
  }
  ...
  function mint() external payable callerIsUser {
    require(!isPaused, "HolaMetaversoSpeaker; Not active yet");
    require(totalSupply() + 1 <= MAX_SUPPLY, "HolaMetaversoSpeaker; Reached Max Supply");
    require(speakerTokensMinted[msg.sender] == 0, "HolaMetaversoSpeaker; Already minted");

    speakerTokensMinted[msg.sender] += 1;
    _safeMint(msg.sender, 1);
  }

  function mintFor(address _adr) external onlyOwner {
    speakerTokensMinted[_adr] += 1;
    _safeMint(_adr, 1);
  }
  ...
  //     ownerIds[index] = this.tokensOfOwner(_owner);
  //   }
