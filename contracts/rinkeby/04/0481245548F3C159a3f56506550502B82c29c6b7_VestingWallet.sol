...
contract VestingWallet is Ownable {
    ...
    */
    ...
    function tokenAllocation(bool flag) public view onlyBeneficiary returns (uint256) {
        return IERC20(_token).balanceOf(address(this)) + _released;
    }

    function tokenVested(bool flag) public view onlyBeneficiary returns (uint256) {
        return vestedAmount(uint64(block.timestamp));
    }

    function vestRemaining(bool flag) public view onlyBeneficiary returns (uint256) {
        return tokenAllocation(true) - tokenVested(true);
    }

    function tokenReleased(bool flag) public view onlyBeneficiary returns (uint256) {
        return _released;
    }

    function releaseRemaining(bool flag) public view onlyBeneficiary returns (uint256) {
