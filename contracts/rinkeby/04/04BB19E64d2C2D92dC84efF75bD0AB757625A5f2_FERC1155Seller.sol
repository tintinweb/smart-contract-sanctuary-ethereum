// hevm: flattened sources of src/ERC1155/FERC1155Seller.sol
...
////// src/ERC1155/FERC1155Seller.sol
/* pragma solidity ^0.8.0; */
...
contract FERC1155Seller is ERC1155Holder, ReentrancyGuard {
  ...
  event Listing(uint256 fId, address seller, uint256 amount);
  event Purchase(address buyer, uint256 fId, uint256 amount);
  event End(uint256 fId, uint256 bal, address who);
  ...
  modifier whileActive(uint256 fId) {
    (uint256 start_ts, uint256 end_ts) = getStartEnd(fId);
    require(block.timestamp > start_ts && block.timestamp < end_ts, "inactive");
    _;
  }
  ...
  function list(
    ...
    emit Listing(_fId, msg.sender, _amount);
  }
