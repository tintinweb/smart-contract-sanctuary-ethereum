...
/** 
 *  SourceUnit: c:\Users\Schmidt\Desktop\portfolio\swimgithub\mugstars_nft_contract\contracts\mugstars.sol
*/
 ...
 */
contract MugStars is ERC721Enumerable, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    // many MugStars remain that they can purchase during presale.
    ...
    /**
     * @param newPrice The new price for an individual token.
     * @dev Sets a new price for an individual token.
    ...
    /**
     * @param newMaxPurchase The new maximum number of a token that can be minted per transaction.
     * @dev Sets the new maximum number of tokens purchases per transaction.
     * @dev Can only be called by owner.
     */
     ...
     * @param numPresaleNFT Amount of presale NFT to give to the address.
     * @dev Can only be called by owner.
     */
    function addPresaleAddress(address presaleAddress, uint256 numPresaleNFT) public onlyOwner {
        require(presaleAddress != address(0), "NFT: Cannot add burn address to the presale.");
        
        presaleVouchers[presaleAddress] = numPresaleNFT;
    }
    ...
    /**
     * @param newBaseURI The base URI for the contract's NFTs.
     * @dev Can only be called by owner.
     */
    ...
    // Override empty _baseURI() function in ERC721.sol
    ...
    function mintNFT(uint256 numberOfTokens) public payable nonReentrant {
        require(saleStatus == SaleState.ACTIVE || saleStatus == SaleState.PRESALE, "NFT: Sale must be active or in presale mint NFT.");
        ...
        require(balanceOf(msg.sender) + numberOfTokens <= maxPurchase, "NFT: Please try to mint a lower amount of NFTs");
        require(totalSupply() + numberOfTokens <= MAX_Tokens, "NFT: Purchase would exceed max supply of NFTs.");
        require(Price * (numberOfTokens) <= msg.value, "NFT: Ether value sent is not correct.");
