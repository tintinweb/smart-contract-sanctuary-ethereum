...
contract NemoLand is Admin, ERC721Upgradeable {
    ...
    mapping(uint256 => uint256[]) public referralToIds;
    mapping(uint256 => uint256[]) public stateToLands;

    event mintWithReferral(uint256 indexed referral, uint256[] indexed lands);
    event StateMinted(uint256 indexed state, uint256[] indexed lands);
    ...
    function mintLand(
        ...
        require(
            isValidRequest(_signature, lands, _zoneId, referral),
            "Invalid sign"
        );
        ...
        for (counter = 0; counter < lands.length; counter++) {
            require(_exists(lands[counter]) == false, "Land already exists");
            require(lands[counter] <= MAX_SUPPLY, "More than MAX_SUPPLY");
        }

        uint256 ethRequired = zoneIdToPrice[_zoneId] * lands.length;
        // uint256 ethRequired = getSalePrice(_zoneId)  * lands.length;
        ...
        if (state > 0) {
            ...
            emit StateMinted(state, lands);
        } else {
            for (counter = 0; counter < lands.length; counter++) {
                _safeMint(msg.sender, lands[counter]);

                if (referral > 0) {
                    referralToIds[referral].push(lands[counter]);
                }
            }
            if (referral > 0) {
                emit mintWithReferral(referral, lands);
            }
        }
    }
