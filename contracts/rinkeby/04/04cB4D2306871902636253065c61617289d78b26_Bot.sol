...
contract Bot is ERC721, Managable, ERC721Enumerable {
    ...
    constructor(
        ...
        currentTokenId = _tokenOffset;
        tokenOffset = _tokenOffset;

        _addManager(msg.sender);
    }

    function mint(address _to) external onlyManager returns(uint256) {
    ...
    function mintTokenId(address _to, uint256 _tokenId) external onlyManager {
        require(!_exists(_tokenId), "token already exists");
        require(_tokenId <= tokenOffset, "not in token offset");
