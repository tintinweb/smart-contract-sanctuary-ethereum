...
contract Paper is ERC20Burnable, Ownable {
...
*/
    ...
    constructor() ERC20("Paper", "PAPER-DC") {}
    ...
    function unstakeAllProps() public {
        ...
        for (uint256 i = stakerToPropIds[msg.sender].length; i > 0; i--) {
            ...
            uint256 daysStaked = stakeTime / 3600; // / 86400;

            uint256 propReward = (daysStaked * pay_rate) - 5;

            //require (propReward > 0, "must have staked for at least a day to unstake");
    ...
    function unstakeAllBankers() public {
        ...
        for (uint256 i = stakerToBankerIds[msg.sender].length; i > 0; i--) {
            ...
            // loan period is in days so we convert to seconds, (60 * 60 * 24 = 86400)
            uint256 loanPeriodSeconds = loanPeriod * 3600; //* 86400;
            ...
            uint8 pay_rate = IDebtCity(debtCityAddress).getPayForBanker(bankerId);
            uint256 rewardDays = rewardTime / 3600; // / 86400;
    ...
    function unstakeByIds(uint256[] memory bankerIds) public {
        ...
        for (uint256 i = 0; i < bankerIds.length; i++) {
            ...
            // TODO might need to change to an if
    ...
    function claimByBankerId(uint256 bankerId) public {
        require(
            bankerIdToStaker[bankerId] == msg.sender,
            "Banker is not claimable by you!"
        );
    ...
    function claimByPropId(uint256 propId) public {
        require(
            propIdToStaker[propId] == msg.sender,
            "Property is not claimable by you!"
        );

        uint8 pay_rate = IProperty(propertyAddress).getPayForProperty(propId);
        uint256 claimTime = block.timestamp - propIdToTimeStamp[propId];
        uint256 daysStaked = claimTime / 3600; // / 86400;
        ...
        returns (uint256)
    {
        ...
        uint256 daysStaked = secondsStaked / 3600; // / 86400;
