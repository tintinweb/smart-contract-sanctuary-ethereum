...
pragma solidity ^0.8.11;
    contract MyNFTCampaignFactory {
        MyNFTCampaign[] public myNFTCampaign;
        function createNFTCampaign() public{
            MyNFTCampaign campaign = new MyNFTCampaign("name", "symbol", "a/", "b/", 0,10,100, msg.sender);
            myNFTCampaign.push(campaign);
        }
        function getDeployedNFTCampaigns() public view returns (MyNFTCampaign[] memory) {
            return myNFTCampaign;
        }
    }
...
contract MyNFTCampaign is ERC721Enumerable, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  constructor(string memory _name, string memory _symbol, string memory _initBaseURI, string memory _initNotRevealedUri, uint256 _cost, uint8 _maxMintAmount, uint16 _maxSupply, address _treasurerAddress) ERC721(_name, _symbol) {
  ...
  function mint(uint256 _mintAmount) external payable nonReentrant{
    ...
    require(totalSupply() + _mintAmount <= maxSupply, ERROR_MINT_FINISHED);
    require(msg.value >= cost * _mintAmount);

    uint256 baseSupply = totalSupply();
    for (uint256 i = 1; i <= _mintAmount; i++) {
      _safeMint(msg.sender, baseSupply + i);
    }
  }
