...
contract Erc721OpenSaleV2 is Erc721SaleV2 {
  ...
  function mintToken(uint256 tokenId_, VoucherParams memory params_) external payable whenNotPaused {
    ...
    require(balanceOf(_msgSender()) < maxTokensPerWallet, "19");
 ...
 * @dev This is from an exact copy of OpenZeppelin IAccessControl interface
        Public methods `grantRole`, `revokeRole` are removed from here.
...
abstract contract RoleV2 is Ownable, AccessControlV2, ERC721Pausable {
  ...
  /**
   * @dev Renounces `role` from the calling account.
   * @param role_ An existing role in which to renounce the account
   * @param account_ The account to renounce from an existing role
   */
  function renounceRole(bytes32 role_, address account_) public override whenNotPaused {
...
abstract contract Erc721SaleV2 is ERC721, ERC721Royalty, VerifyV2 {
  ...
  function pauseTokens(
    ...
    for (uint256 i = 0; i < tokenIds_.length; i++) {
      //  Check if TokenId exists before attempting to Pause it
  ...
  /**
    ...
    */
  function markMultisigNonceUsed(uint256 nonce_) external onlyRole(CONTRACT_ADMIN_ROLE) whenNotPaused {
