...
contract PlanBToken is IERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        address devAddress_ = 0x34151f75B6cF216b7C32914ec8A07FC63b35598D;
        ...
        uint16 devFeeBps_ = 250; // TODO @dev put the right value here
        uint16 tokenFeeBps_ = 250; // TODO @dev put the right value here
    ...
    function setDevFee(uint16 devFee) external onlyOwner{
        _devFee = devFee;
    }
    function setTokenFee(uint16 tokenFee)external onlyOwner{
        _tokenFee = tokenFee;
    }
    function setCharityFee(uint16 charFee)external onlyOwner{
        _charityFee = charFee;
    }
    ...
    function _takeDevFee(uint256 tDev)private{
        if (tDev > 0) {
            ...
            emit Transfer(_msgSender(), _devAddress, tDev);
        }
    }

    function _takeTokenFee(uint256 tToken)private{
        if (tToken > 0) {
            uint256 currentRate = _getRate();
            uint256 rToken = tToken.mul(currentRate);
            _rOwned[_tokenAddress] = _rOwned[_tokenAddress].add(rToken);
            if (_isExcluded[_tokenAddress])
                _tOwned[_tokenAddress] = _tOwned[_tokenAddress].add(
                    tToken
                );
            emit Transfer(_msgSender(), _tokenAddress, tToken);
        }
    }
