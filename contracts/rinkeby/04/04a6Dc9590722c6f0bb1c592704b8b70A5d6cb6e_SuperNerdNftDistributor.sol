...
contract SuperNerdNftDistributor is AccessControl {
    ...
    function whiteListMint(uint256 _num) public payable returns (bool) {
        ...
        require (block.timestamp >= WHITELIST_START_TIME && block.timestamp < WHITELIST_START_TIME + WHITELIST_END_TIME, "Time is not suitable for whiteList");
    ...
    function preSaleMint(uint256 _num) public payable returns (bool) {
        ...
        require (block.timestamp >= PRE_SALE_START_TIME && block.timestamp < PUBLIC_SALE_START_TIME, "Time is not suitable for presale");
        require(_num > 0, "need to mint at least 1 NFT");
    ...
    function airdropForAlphaUsers(address wallet, uint256 _mintCount, uint256 _canMintTotal, uint256 _timestamp, bytes memory _signature) public returns (bool) {
