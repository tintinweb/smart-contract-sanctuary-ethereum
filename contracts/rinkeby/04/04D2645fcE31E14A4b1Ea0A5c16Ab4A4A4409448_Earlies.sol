...
contract Earlies is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public constant MAX_MINT_PER_WALLET = 14;
  ...
  constructor() ERC721A("Earlies Society", "EA") {}
  ...
  function devMint(uint256 quantity) external onlyOwner {
    require(
      ...
      "Not enough NFTs left to mint the desired amount."
    );
    _safeMint(msg.sender, quantity);
  }
  ...
  function teamMint(TeamMint[] memory teamMintData) external onlyOwner {
    for (uint256 i = 0; i < teamMintData.length; i++) {
      require(totalSupply() + teamMintData[i].amount <= TOTAL_SUPPLY, "Not enough NFTs left to mint the desired amount.");
