...
contract CrowdFunding {
    ...
    */
    ...
    event readEvent(address ad, uint256 amount);

    

    function readData(uint256 campaignID, uint256 funderID) public returns(address ad, uint256 amount){
        require(campaignID < numCampaigns);
        Campaign storage c = campaigns[campaignID];

        require(funderID < c.numFunders);

        emit readEvent(c.funders[funderID].addr, c.funders[funderID].amount);
