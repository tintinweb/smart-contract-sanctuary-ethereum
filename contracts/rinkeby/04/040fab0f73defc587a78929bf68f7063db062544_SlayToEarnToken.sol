...
contract SlayToEarnToken is Context, Ownable, IERC20, IERC20Metadata {
    ...
    */
    ...
    function _transfer(
        ...
        if (!isGateKeeperPermanentlyDisabled()) {
            ...
            require(sentAmount <= _gateKeeperMaxTokenAmount, "Buy blocked by gatekeeper. Token limit per transaction exceeded. Refer to our Telegram for further information. This restriction will be lifted shortly.");

            // Only apply this for buys, not sells
            if (sender == _uniswapLpPair) {
                require(_lastBuyBlock[recipient] < block.number, "Buy blocked by gatekeeper. You can only buy once per block. Refer to our Telegram for further information. This restriction will be lifted shortly.");

                _lastBuyBlock[recipient] = block.number;
            }

            if (recipient != _uniswapLpPair) {
                // in case of sells, we do not ask the LP pair to have our launch witness
                require(getOwnedWitnessCountForAddress(recipient) >= 2, "Buy blocked by gatekeeper. Target wallet does not own enough of our NFTs. Refer to our Telegram for further information. This restriction will be lifted shortly.");
            }
        }

        uint256 taxReductionPercentage = _taxReductionPercentageMap[sender];
        if (taxReductionPercentage > 0) {
        ...
        if (!isAutomaticBuybackEnabled()) {
            if (devTokens > 0) {
                _transferInternal(sender, _devIncomeWallet, devTokens);
            }
        } else {
            if (devTokens > 0) {
