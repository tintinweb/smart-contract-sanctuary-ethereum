...
contract Pastel is ERC721A, Ownable {

  using Strings for uint256;
//   using Address for address;

  address public futureContract;
  address public PastelTreasury;
  ...
  constructor(
    ...
    ERC721A(_name, _symbol)
     {
        ...
        // initWithdrawAddresses = _withdrawAddresses;
    ...
    // perhaps would be more efficient with a merkle airdrop vs address in memory
    ...
    function setWithdrawAddresses(address[] memory _w) public onlyOwner {
    ...
    function withdraw() external onlyOwner {
        ...
        (bool s1, ) = payable(PastelTreasury).call{value: (amount * 31) / 100}("");
        (bool s2, ) = payable(Charity).call{value: (amount * 5) / 100}("");
        (bool s3, ) = payable(Artist).call{value: (amount * 30) / 100}("");
        (bool s4, ) = payable(Dev).call{value: (amount * 10) / 100}("");
        (bool s5, ) = payable(Comm).call{value: (amount * 15) / 100}("");
        (bool s6, ) = payable(Admin).call{value: (amount * 9) / 100}("");

        if (s1 && s2 && s3 && s4 && s5 && s6) return;
        // fallback to paying all to treasury
        (bool s7, ) = PastelTreasury.call{value: amount}("");
        require(s7, "Withdrawal failed");
    }
