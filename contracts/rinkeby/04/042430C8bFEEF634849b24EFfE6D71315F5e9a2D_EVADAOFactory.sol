...
contract EVADAOFactory is Ownable {
  EVADAO[] public addresses;
  event EVADAOContractCreated(EVADAO evaDaoContract);

  function createEvaDao(address _evaDQContract) external {
    EVADAO evaDaoContract = new EVADAO(_evaDQContract);

    evaDaoContract.transferOwnership(owner());

    addresses.push(evaDaoContract);
    emit EVADAOContractCreated(evaDaoContract);
  }

  function getEvaDaoContracts() external view returns (EVADAO[] memory) {
    return addresses;
  }
}
