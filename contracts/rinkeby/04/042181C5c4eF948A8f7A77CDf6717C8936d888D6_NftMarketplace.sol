...
contract NftMarketplace is ReentrancyGuard {
  ...
  // 10- If someone bids higher then the previous highest bidder, update the highest bidder. and also update the proceeds for the previous bidder with his bidding amount.
   function bidAuction(address nftAddress, uint256 tokenId) external payable isAuctioned(nftAddress, tokenId){
    Auction memory m_auction = s_auctions[nftAddress][tokenId];

    if(m_auction.bid > msg.value){
        revert NewBidMustBeHigher(m_auction.bid, msg.value);
    }
    ...
    s_auctions[nftAddress][tokenId].bid = msg.value;
    s_auctions[nftAddress][tokenId].bidder = msg.sender;

    //Sending the previous bidder the proceeds of the current bidder.
    // Problem - On first bid, the previous bidder is set to be the owner and he hasn't paid to the reserved price.
    if(nftOwner != m_auction.bidder){
       s_proceeds[m_auction.bidder] = s_proceeds[m_auction.bidder] + m_auction.bid;
    }
    emit AuctionBiddedSuccessfully(nftAddress, tokenId, msg.sender, msg.value);
