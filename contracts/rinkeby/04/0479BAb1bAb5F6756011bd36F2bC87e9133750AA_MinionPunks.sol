...
contract MinionPunks is ERC721A, Ownable {
    ...
    uint256 public MAX_TXN_FREE = 5;
    uint256 public constant FREE_SUPPLY = 500;
    uint256 public constant PAID_SUPPLY = 9500;
    uint256 public constant MAX_SUPPLY = FREE_SUPPLY + PAID_SUPPLY;

    constructor() ERC721A("MinionPunks", "MINION", MAX_TXN) {
        saleEnabled = false;
        price = 0.01 ether;
    }
    ...
    function freeMint(uint256 numOfTokens) external payable {
        require(saleEnabled, "Sale must be active.");
        require(
            totalSupply() + numOfTokens <= FREE_SUPPLY,
            "Exceed max supply"
        );
        require(numOfTokens <= MAX_TXN_FREE, "Cant mint more than 5");
        require(numOfTokens > 0, "Must mint at least 1 token");
