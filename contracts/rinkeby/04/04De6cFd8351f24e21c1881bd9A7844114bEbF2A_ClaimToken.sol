...
contract ClaimToken is OwnableUpgradeable, SuperAdminControl, IClaimToken {
    ...
    */
        ...
        onlySuperAdmin(govAdminRegistry, msg.sender) /** only super admin wallet can add sun tokens */
    {
        ...
        require(
            prevClaimTokenData.pegTokens.length ==
                _newClaimtokendata.pegTokens.length &&
                prevClaimTokenData.pegTokensPricePercentage.length ==
                _newClaimtokendata.pegTokensPricePercentage.length,
            "GCL: length mismatch"
        );
        require(
            approvedClaimTokens[_claimTokenAddress],
