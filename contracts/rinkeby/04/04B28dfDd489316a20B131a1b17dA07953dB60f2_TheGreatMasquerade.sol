...
contract TheGreatMasquerade is ERC721A, Ownable, ReentrancyGuard {
    ...
    bool private allowPublicSale = false;
    ...
    ) ERC721A('TheGreatMasquerade', 'TGM') {
    ...
    modifier saleIsOpen() {
        ...
        require(isSaleActive, 'Sales not open');
        _;
    }

    modifier publicSaleIsOpen() {
        require(totalSupply() <= MAX_SUPPLY, 'Soldout!');
        require(allowPublicSale, 'Public Sales not open');
        _;
    }
    ...
    function reserveNFTs(address to, uint256 quantity) public onlyOwner {
    ...
    function setAllowPublicSale(bool _allowPublicSale) public onlyOwner {
        allowPublicSale = _allowPublicSale;
    }
    ...
    function publicMint(uint256 _amountOfTokens)
        public
        payable
        publicSaleIsOpen
        callerIsUser
    {
        ...
        require(
            msg.value >= price(_amountOfTokens),
            'Ether value sent is not correct'
        );

        require(
            numberMinted(msg.sender) + _amountOfTokens <= MAX_PER_WALLET,
