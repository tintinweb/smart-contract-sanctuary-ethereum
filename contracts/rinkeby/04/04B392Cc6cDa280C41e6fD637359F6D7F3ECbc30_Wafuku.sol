...
contract Wafuku is ERC721A, Ownable, Pausable {
    ...
    constructor() ERC721A("Wafuku", "WFK") {
        _safeMint(0x2aa011A011e2fC4139402F0D6BCd0E86E3741fE2, 600); // 運営1
        _safeMint(0x40Af078ebE19F16d064D055881ee97E1c6D6be12, 600); // 運営2
        _safeMint(0x0306a06fE4Da5a280f4A78BeCD37F30B352EbA4a, 600); // 運営3
        _safeMint(0xf4895afF82ac25027D8fa5b292930746A497837E, 100); // wafuku
        _safeMint(0xD253e45c1f48bEb7dfe4678cB6d831C842b9E8B2, 100); // solo
        _safeMint(0x597B11F34edE259f459E25560c40e80D9a0dB53F, 50); // solo
        _safeMint(0xBBA254699Bc6C8CDEeAAb5DB75BeE22E4E990AB8, 50); // okuten
        _safeMint(0x05C4e1D4445dEb2a1D7F1db4e33909fBb5C599b1, 50); // yokodori
        _safeMint(0xb21D44ed67c495A4ABD644575c20c145cECB47e7, 50); // youhei
        _safeMint(0x924fCAceaE7187629F7974D2f570D5F6cDb20052, 50); // reon
        _safeMint(0x76E5A6c87a912c382B767BDF4053f4A290d2b9B3, 50); // munakata
        _pause();
    }
...
library Sales {
    struct Sale {
        ...
        uint64 mintCost; // セールごとに価格が変わる
        uint64 maxSupply; // セールごとにミントできる上限があるので
        uint8 hasRestriction; // 1ならWL / 1以外ならパブリック
        uint8 isBurnAndMint; // 1ならburnAndMint / 1以外ならmint
        bytes32 merkleRoot; // WL用(hasRestriction=1の場合のみ値が入る)
    }
