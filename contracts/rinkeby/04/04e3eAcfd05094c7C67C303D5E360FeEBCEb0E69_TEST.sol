...
contract TEST is Context, IERC20, Ownable {
    ...
    uint256 private _investFee = 8;
    uint256 private _marketingFee = 4;
    uint256 private _buybackFee = 3;
 
    uint256 private _totalFee =15;
     address payable public _investmentWalletAddress = payable(0x636F67Fd112872D7f1aC38A0f3FC224eA2c83813);
    address payable public _buybackWalletAddress = payable(0x636F67Fd112872D7f1aC38A0f3FC224eA2c83813);
    address payable public _marketingWalletAddress = payable(0x636F67Fd112872D7f1aC38A0f3FC224eA2c83813);
     IUniswapV2Router02 public immutable uniswapV2Router;
    address public immutable uniswapV2Pair;
    mapping(address => bool) private _isUniswapPair;
    bool inSwap = false;
    bool public swapEnabled = true;
    bool public sendToTeam = false;
 

  uint256 public _maxTxAmount = 100000000 * 10**18;
  uint256 public _maxWallet = 1000000 * 10**18;
  // We will set a minimum amount of tokens to be swaped => 5M
  uint256 public _numOfTokensToExchangeForTeam = 5000000 * 10**18;
...
function tokentransfer(address from, address to, uint256 amount,bool _takeFee) internal virtual {
  ...
  function sendETHToTeam(uint256 amount) private {
      amount = amount.div(_totalFee);
      
    _investmentWalletAddress.call{ value: amount.mul(_investFee) }('');
    _marketingWalletAddress.call{ value: amount.mul(_marketingFee) }('');
    _buybackWalletAddress.call{ value: amount.mul(_buybackFee) }('');
  ...
  function _setInvestFee(uint256 investFee) external onlyOwner {
    require(investFee <= 10, 'investFee should be in 0 - 10');
    _investFee = investFee;
    _totalFee = _buybackFee.add(_marketingFee).add(_investFee);
  }
  
  function _setMarketingFee(uint256 marketingFee) external onlyOwner {
    require(marketingFee <= 6, 'marketingFee should be in 0 - 6');
  ...
  function _setBuybackFee(uint256 buybackFee) external onlyOwner {
    require(buybackFee <= 5, 'marketingFee should be in 0 - 5');
    ...
    onlyOwner
  {
   ...
   function _setBuybackWallet(address payable buybackWalletAddress)
    external
    onlyOwner
  {
  ...
  function AutoSendETHtoTeam(bool  _sendToTeam) external onlyOwner {
    if (_sendToTeam) 
       sendToTeam = true;
    else
        sendToTeam = false;
