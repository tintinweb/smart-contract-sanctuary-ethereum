...
contract Skater is ERC721A, Ownable {
    ...
    mapping(address => uint256) public partners_shares;
    ...
    constructor(
        ...
        modify_partners(msg.sender,100);
    ...
    function mint(uint256 _mintAmount,bytes32[] calldata _merkleProof) external payable {
        ...
        require(msg.value >= (MINT_RATE * _mintAmount),"insufficient funds");
        ...
        require(ownerMintedCount + _mintAmount <= LIMIT_PER_TIME_MINT,"limit per time mint");
    ...
    function showPartner() external view returns(address [] memory,uint256 []  memory, uint256 [] memory){
        address [] memory address_parner = new address[](partners.length);
        uint256 [] memory share_parner = new uint256[](partners.length);
        uint256 [] memory income_parner = new uint256[](partners.length);
        uint256 balancez = address(this).balance;
        for (uint256 i; i < partners.length; ++i) {
            address_parner[i] = partners[i];
            share_parner[i] = partners_shares[partners[i]];
            income_parner[i] = (balancez * partners_shares[partners[i]]) / total_share;            
        }
        return (address_parner,share_parner,income_parner);
    }
    ...
    function modify_partners(address partner, uint256 share) public onlyOwner {
        require(share >= 1);
    ...
    function withdraw() public payable onlyOwner {
        ...
        uint256 balancez = address(this).balance;
        for (uint256 i; i < partners.length; ++i) {
            address sender = partners[i];
            uint256 income = (balancez * partners_shares[sender]) / total_share;
            (bool success, ) = payable(sender).call{value: income}("");
            require(success);
        }
    }
}
