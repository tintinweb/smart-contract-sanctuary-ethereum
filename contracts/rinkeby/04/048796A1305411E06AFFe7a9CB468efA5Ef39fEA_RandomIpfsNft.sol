...
contract RandomIpfsNft is ERC721URIStorage, VRFConsumerBaseV2 {
    ...
    ) ERC721("Random IFPS NFT", "RIN") VRFConsumerBaseV2(vrfCoordinatorV2) {
    ...
    function fulfillRandomWords(
        ...
        uint256 newTokedId = s_tokenCounter;
        s_tokenCounter = s_tokenCounter + 1;

        uint256 moddedRng = randomWords[0] % MAX_CHANCE_VALUE;
        uint256 breed = getBreedFromModdedRng(moddedRng);
        _safeMint(dogOwner,newTokedId);
        // set the tokenURI
        _setTokenURI(newTokedId, s_dogTokenUris[breed]);
    }
    ...
    function getBreedFromModdedRng(uint256 moddedRng) private pure returns (uint256){
