...
contract ExampleToken is ERC721SeaDrop {
    ...
    function _tokenJson(uint256 tokenId) internal view returns (string memory) {
        ...
        svg = string.concat(svg, "<svg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='xMinYMin meet' viewBox='0 0 350 350'>"
        "<style>"
        ...
        json = string.concat(json, '", "description": "This is a test token, for trying out cool things related to NFTs!'
        ' Please note that this token has no value or warranty of any kind.<br /><br />\\"The future belongs to those who believe in'
        ' the beauty of their dreams.\\"<br />-Eleanor Roosevelt", "image_data": "');
        json = string.concat(json, svg);
        json = string.concat(json, '", "attributes": [ {"trait_type": "Token ID", "value": "');
        json = string.concat(json, Strings.toString(tokenId));
        json = string.concat(json, '"}, {"trait_type": "Font", "value": "');
        json = string.concat(json, _font(tokenId));
        json = string.concat(json, '"}, {"trait_type": "Font size", "value": "');
     ...
     *         Starts at 200px and decrerases to a minimum of 10px.
     */
    function _fontSize(uint256 tokenId) internal pure returns (string memory) {
        // Don't return smaller than 10px
        if (tokenId >= 190) {
            return "10";
        }

        return Strings.toString(200 - tokenId);
    }
}
