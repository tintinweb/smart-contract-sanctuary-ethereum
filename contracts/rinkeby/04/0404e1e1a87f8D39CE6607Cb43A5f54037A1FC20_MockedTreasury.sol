...
contract MockedTreasury is ITreasury {
    ...
    function get_tokens(uint reward, uint _amountBlxm, uint _amountToken, address to) external override returns (uint sentToken, uint sentBlxm) {
        sentBlxm = _amountBlxm.add(reward);
        sentToken = _amountToken;
        IERC20(BLXM).transfer(to, sentBlxm);
        IERC20(token).transfer(to, sentToken);
