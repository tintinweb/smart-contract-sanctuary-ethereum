...
contract ERC721AMint is ERC721A, Ownable {
    ...
    function mintWithString(uint256 quantity, string memory tokenURI)
        external
        payable
    {
        tokenURI;
    ...
    function mintOne() external payable {
        require(saleActive, "Sale must be active to mint");
        require(
            accountMinted[msg.sender] + 1 <= maxMintPerAccount,
            "Sender is trying to mint more than allocated tokens"
        );
        require(msg.value >= cost, "Amount of ether is not enough");
        require(
            totalSupply() + 1 <= maxSupply,
            "Mint would exceed max supply of mints"
        );
        accountMinted[msg.sender] += 1;
        maxSupply += 1;
