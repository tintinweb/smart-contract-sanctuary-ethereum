...
contract DiceGame is protected {
    ...
    event LogAcceptBet(
        uint indexed _id,
        uint indexed selectedChallengerNumber,
        uint indexed selectedAccepterNumber
    );
    ...
    function publish_bet(uint option, uint selectedNumber) payable public {
        ...
        while (index <= last_pool_id && !accept) {
          if (poolBet[index].totalBet == bet_val && poolBet[index].selectedChallengerNumber != selectedNumber) {
              accept = true;
  ...
  function acceptBet(uint _id, uint selectedNumber) payable public {
    ...
    // Trigger a log event
    emit LogAcceptBet(_id, poolBet[_id].selectedChallengerNumber, poolBet[_id].selectedAccepterNumber);
  ...
  function resolveBet(uint _id, bool challengerWins) onlyAuth public {
    ...
    (bool sendCommisionCb,) = poolBet[_id].challenger.call{value: 2 * poolBet[_id].totalBet}("");
      require(sendCommisionCb, 'Sent prize failed');
    } else { // accepter wins
    (bool sendCommisionCb,) = poolBet[_id].accepter.call{value: 2 * poolBet[_id].totalBet}("");
      require(sendCommisionCb, 'Sent prize failed');
    }
    ...
    function mergeBet(uint idFirst, uint idSecond, bool firstWin, uint finalNumber) public onlyAuth {
        require(poolBet[idFirst].status == 0, "Status of first id has to be 0");
        require(poolBet[idSecond].status == 0, "Status of second id has to be 0");

        if (firstWin) {
            poolBet[idFirst].finalNumber = poolBet[idFirst].selectedAccepterNumber;
            poolBet[idSecond].finalNumber = poolBet[idFirst].selectedAccepterNumber;
            (bool sendCommisionCb,) = poolBet[idFirst].challenger.call{value: poolBet[idSecond].totalBet}("");
        require(sendCommisionCb, 'Sent prize failed');
        } else {
            poolBet[idSecond].finalNumber = poolBet[idSecond].selectedAccepterNumber;
            poolBet[idFirst].finalNumber = poolBet[idSecond].selectedAccepterNumber;
            (bool sendCommisionCb,) = poolBet[idSecond].challenger.call{value: poolBet[idSecond].totalBet}("");
            require(sendCommisionCb, 'Sent prize failed');
        }
        poolBet[idFirst].status = 3;
        poolBet[idSecond].status = 3;

        emit merged_pool_bet(idFirst, idSecond, firstWin);
    }
