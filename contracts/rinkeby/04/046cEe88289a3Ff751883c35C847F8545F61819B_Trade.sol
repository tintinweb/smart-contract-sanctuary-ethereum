...
contract Trade is AccessControl {
    ...
    event WhitelistSpotsAdded(
        address indexed user,
        uint256 quantity
    );
    event RevokedWhiteListAddress(address indexed user);
    event SpotsIncreased(
        address indexed user,
    ...
    */
    ...
    function grandWhitelistSpotsBatch(
        address[] memory _whitelistAddress,
        uint256[] memory quantity
    ) external onlyRole("ADMIN_ROLE") {
        require(
            _whitelistAddress.length == quantity.length,
            "Length missmatch"
        );

        for (uint256 i = 0; i < _whitelistAddress.length; ++i) {
            whitelist[_whitelistAddress[i]] += quantity[i]; // addition
            emit WhitelistSpotsAdded(_whitelistAddress[i], quantity[i]);
        }
        
    }

    function revokeWhitlistSpots(address _removeuser)
        external
        onlyRole("ADMIN_ROLE")
    {
        require(whitelist[_removeuser] > 0, "User dont have whitelistspots");
        whitelist[_removeuser] = 0;

        emit RevokedWhiteListAddress(_removeuser);
    }

    function userRemainingSpots(address _whitelistAddress)
        external
        view
        returns (uint256)
    {
    ...
    function increaseSpots(address _whitelistAddress, uint256 quantity)
        external
        onlyRole("ADMIN_ROLE")
    {
        whitelist[_whitelistAddress] += quantity;
        emit SpotsIncreased(_whitelistAddress, quantity);
    }
    ...
    */
    ...
    /**
        excuting the NFT order.
        @param order ordervalues(seller, buyer,...).
        @param ownerSign Sign value(v, r, f).
    */

    function lazyMint(Order calldata order, Sign calldata ownerSign)
        external
        returns (bool)
    {
        ...
        if (whitelist[order.buyer] > 0) {
            whitelist[order.buyer] -= 1;
        } else {
        ...
        emit LazyMint(order.seller, order.tokenId, order.qty, msg.sender);
        return true;
    }
