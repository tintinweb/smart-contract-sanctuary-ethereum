...
contract Fubao is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public reservedMintedAmount = 0;
    uint256 public publicAvailableAmount;
    uint256 public publicMintedAmount = 0;
    ...
    constructor(
        ...
        require(
            ...
            "collection size must be larger than reserved amount"
        );
        require(
    ...
    function mint(uint256 amount) external payable callerIsUser {
        ...
        emit Mint(_msgSender(), amount, publicPrice);
    }
    ...
    function setWhiteListConfig(
        ...
        require(
            ...
            "address list does not match amount list length"
        );
    ...
    function whiteListMint(uint256 amount) external payable callerIsUser {
        ...
        _safeMint(_msgSender(), amount);
        refundIfOver(whiteListPrice * amount);
    ...
    function airdropList(
        ...
        for (uint256 i = 0; i < addressList.length; i++) {
            airdrop(addressList[i], amountList[i]);
        }
    }
    ...
    event Mint(address user, uint256 amount, uint256 price);
    event WhiteListMint(address user, uint256 amount, uint256 price);
    event Airdrop(address target, uint256 amount);
}
