...
contract  RisingBaseballStars is ERC721, ERC721Enumerable, Ownable{
    ...
    // Prices // 
    uint256 public MVP_PRICE;
    uint256 public HOF_PRICE;
    uint256 public TC_PRICE;
    // Supply //
    uint256 public MVP_MAX_SUPPLY;
    uint256 public HOF_MAX_SUPPLY;
    uint256 public TC_MAX_SUPPLY;
    uint256 public MVP_WHITELIST_SUPPLY;
    uint256 public HOF_WHITELIST_SUPPLY;
    uint256 public TC_WHITELIST_SUPPLY;
    uint256 public MVP_RESERVE_SUPPLY;
    uint256 public HOF_RESERVE_SUPPLY;
    uint256 public TC_RESERVE_SUPPLY;
    ...
    constructor(address _payments) 
        ERC721("Rising Baseball Stars Testnet", "RBST") {
            // Whitelist Supply
            TC_WHITELIST_SUPPLY = 20;
            HOF_WHITELIST_SUPPLY = 30;
            MVP_WHITELIST_SUPPLY = 160;
            // Max Supply
            TC_MAX_SUPPLY = 10 + TC_WHITELIST_SUPPLY;             
            HOF_MAX_SUPPLY = 450 + HOF_WHITELIST_SUPPLY;
            MVP_MAX_SUPPLY = 1500 + MVP_WHITELIST_SUPPLY;
            // Reserve Supply
            TC_RESERVE_SUPPLY = 30;
            HOF_RESERVE_SUPPLY = 520;
            MVP_RESERVE_SUPPLY = 1420;
            // Prices
            TC_PRICE = 0.2 ether;
            HOF_PRICE = 0.1 ether;
            MVP_PRICE = 0.01 ether;
            // Payment Splitter
    ...
    // MINT FUNCTIONS //
    function mintPresaleMVP(address to, uint256 _quantity) public payable {
        require(msg.value >= MVP_PRICE*_quantity, "Not enough ether sent.");
        require(WHITELIST_ACTIVE, "Private mint is not yet live.");
        require(WHITELISTED[to], "This address is not whitelisted.");
        require(_MVPCounter.current() < MVP_WHITELIST_SUPPLY, "Whitelist is sold out for this member type.");
        for (uint i = 0; i < _quantity; i++) {
            _MVPCounter.increment();
            tokenCounter = _MVPCounter.current()+ TC_MAX_SUPPLY + TC_RESERVE_SUPPLY + HOF_MAX_SUPPLY + HOF_RESERVE_SUPPLY; 
    ...
    function mintPublicMVP(address to, uint256 _quantity) public payable {
        require(msg.value >= MVP_PRICE*_quantity, "Not enough ether sent.");
        ...
        for (uint i = 0; i < _quantity; i++) {
            _MVPCounter.increment();
            _safeMint(to, _MVPCounter.current() + TC_MAX_SUPPLY + TC_RESERVE_SUPPLY + HOF_MAX_SUPPLY + HOF_RESERVE_SUPPLY);
        }
    }
    function mintReserveMVP(address to, uint256 _quantity) public payable {
        require(msg.value >= MVP_PRICE*_quantity, "Not enough ether sent.");
    ...
    function mintPresaleHOF(address to, uint256 _quantity) public payable {
        require(msg.value >= HOF_PRICE*_quantity, "Not enough ether sent.");
        require(WHITELIST_ACTIVE, "Private mint is not yet live.");
        require(WHITELISTED[to], "This address is not whitelisted.");
        require(_HOFCounter.current() < HOF_WHITELIST_SUPPLY, "Whitelist is sold out for this member type.");
        for (uint i = 0; i < _quantity; i++) {
            _HOFCounter.increment();
            tokenCounter = _HOFCounter.current() + TC_MAX_SUPPLY + TC_RESERVE_SUPPLY;
            _safeMint(to, tokenCounter);
            _HOFCounter.increment();
            tokenCounter += 1;  
            _safeMint(to, tokenCounter);
    ...
    function mintPublicHOF(address to, uint256 _quantity) public payable {
        require(msg.value >= HOF_PRICE*_quantity, "Not enough ether sent.");
        require(PUBLIC_SALE_ACTIVE, "Public mint is not yet live.");
        require(_HOFCounter.current() < HOF_MAX_SUPPLY, "Public mint is sold out for this member type.");
        for (uint i = 0; i < _quantity; i++) {
            _HOFCounter.increment();
            _safeMint(to, _HOFCounter.current() + TC_MAX_SUPPLY + TC_RESERVE_SUPPLY);  
        }
    }
    function mintReserveHOF(address to, uint256 _quantity) public payable {
        require(msg.value >= HOF_PRICE*_quantity, "Not enough ether sent.");
        require(RESERVE_SALE_ACTIVE, "Reserve mint is not live.");
    ...
    function mintPresaleTC(address to, uint256 _quantity) public payable {
        require(msg.value >= TC_PRICE* _quantity, "Not enough ether sent.");
        require(WHITELIST_ACTIVE, "Private mint is not yet live.");
        require(WHITELISTED[to], "This address is not whitelisted.");
        require(_TCCounter.current() < TC_WHITELIST_SUPPLY, "Whitelist is sold out for this member type.");
        for (uint i = 0; i < _quantity; i++) {
            _TCCounter.increment();
            _safeMint(to, _TCCounter.current());
            _TCCounter.increment();
            _safeMint(to, _TCCounter.current());
        }
    }
    function mintPublicTC(address to, uint256 _quantity) public payable {
        require(msg.value >= TC_PRICE* _quantity, "Not enough ether sent.");
        require(PUBLIC_SALE_ACTIVE, "Public mint is not yet live.");
    ...
    function mintReserveTC(address to, uint256 _quantity) public payable {
        require(msg.value >= TC_PRICE* _quantity, "Not enough ether sent.");
        require(RESERVE_SALE_ACTIVE, "Reserve mint is not live.");
