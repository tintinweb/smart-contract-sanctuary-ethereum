...
contract HexArt is ERC721URIStorage, ERC2981, Ownable {
    ...
    event erc20AssetAdded(uint256 parentNFtId,address assetContract,uint256 balance);
    ...
    function removeNFTPayable(
        ...
        require(msg.value == (amountToRemoveAsset * _assetId.length), "Not enough funds sent");
        require(!isLockPeriodOver(_nftTokenId));
           ...
           (bool check, uint256 index) =  isErcAssetAttached(_nftTokenId,_assetContract);
            if(check) {
                if(_assetContract == _weth9){
                    ...
                    payable( _msgSender()).transfer(erc20AssetDetails[_nftTokenId][index].balance);
                }else{
