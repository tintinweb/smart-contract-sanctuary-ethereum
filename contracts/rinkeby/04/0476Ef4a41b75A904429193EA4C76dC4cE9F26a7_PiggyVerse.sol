...
interface P_D {
	...
	function viewLockedDiary(uint diaryId, string memory secret, address sender) external view returns (string memory);
...
contract PiggyVerse{
	...
	function PiggyDiaries_viewLockedDiary(uint diaryId, string memory secret) public view returns (string memory){
		return PiggyDiaries.viewLockedDiary(diaryId, secret, msg.sender);
	}
