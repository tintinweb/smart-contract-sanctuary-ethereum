...
contract TestNFT is ERC721Enumerable, Ownable {
    ...
    uint256 public allowPresaleMintingAfter = 0;
    bool public isPaused = false;
    bool public isRevealed = true;
    string private notRevealedUri;

    address public creator = owner();
    uint256 public royalityFee = 6; // % of mint cost
    ...
    constructor(
        ...
        if (_allowPresaleMint > block.timestamp) {
            allowPresaleMintingAfter = _allowPresaleMint - block.timestamp;
        }
    ...
    function mint(uint256 _mintAmount) public payable {
        if (isWhitelisted[msg.sender] == true) {  
            require(block.timestamp >= timeDeployed +  allowPresaleMintingAfter, "Presale mint not allowd yet");
        } else {
            require(block.timestamp >= timeDeployed +  allowMintingAfter, "Public mint not allowd yet");
        }

        require(balanceOf(msg.sender) <= 19, "20 NFT per account allowed");
        ...
        if (msg.sender != owner()) {
            ...
            // Pay royality to creator and remaining to deployer (owner) of contract
    ...
    function removeFromWhitelist(address removeAddress) public onlyOwner {
        isWhitelisted[removeAddress] = false;
    }
