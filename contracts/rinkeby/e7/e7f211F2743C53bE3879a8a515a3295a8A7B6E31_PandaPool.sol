...
contract PandaPool is Ownable, ReentrancyGuard, IERC721Receiver, VRFConsumerBaseV2, IPandaPool {
    ...
    event TestEvent(uint256 index, uint256 value);
    ...
    function reborn(uint256 _ownerTokenId, uint256 _amountFeeMax, uint256 _deadline, uint8 _gasLevel) payable external override ensure(_deadline)  nonReentrant poolOpen {
        ...
        emit TestEvent(index, targetIndex);
        _poolRemoveNft(index);
        _reborn(msg.sender,_ownerTokenId, targetIndex);
            


        emit PreReborn(requestId, msg.sender, _ownerTokenId, _price * 750 / 10000);
    }
    ...
    function buy(uint256 _amountBuyMax, uint256 _deadline, uint8 _gasLevel) payable external override ensure(_deadline)  nonReentrant poolOpen {
        ...
        IERC20 _pandaToken = IERC20(pandaToken);
        _pandaToken.safeTransferFrom(msg.sender,  owner(), _price * 375 / 10000);
        emit PreBuy(requestId, msg.sender, _deadline);
    }
    ...
    function fulfillRandomWords(
        ...
        uint256 targetIndex = _getIdByIndex(index);
        RequestInfo memory reqInfo = reqInfos[_requestId];
        uint256 funcType = reqInfo.reqFunc;
        if (funcType == REBRON_FUNC_TYPE) {
            _poolRemoveNft(index);
            _reborn(reqInfo.sender,reqInfo.ownerTokenId, targetIndex);
            
        } else {
            _poolRemoveNft(index);
            _buy(reqInfo.sender, targetIndex, reqInfo.amountFeeMax);
        }
    }

    /**
     * @dev internal reborn NFT.
     * @param _ownerTokenId The NFT Id you want to reborn
     * @param _targetTokenId  random target NFT Id
     */
    function _reborn(address sender, uint256 _ownerTokenId, uint256 _targetTokenId) internal poolOpen {
        IERC721 _pandaNFT = IERC721(pandaNFT);
    ...
    /**
     * @dev internal buy NFT, you will receive $PANDA.
     * @param _targetTokenId The NFT Id you want to buy
     * @param _amountBuyMax  Above this value you will not make a purchase
     */
    function _buy(address buyer, uint256 _targetTokenId, uint256 _amountBuyMax) internal poolOpen {
        IERC721 _pandaNFT = IERC721(pandaNFT);
        if (_pandaNFT.ownerOf(_targetTokenId) != address(this)) {
            revert NFTOwnerNotMatch(_targetTokenId,  address(this), _pandaNFT.ownerOf(_targetTokenId));
        }
    ...
    function _poolRemoveNft(uint256 _index) public {
        // poolNftMap.remove(_index, poolSize);
        require(_index < poolSize, "Array Index Out Of Bounds");
    ...
    function _poolPushNft(uint256 _value) public {
    ...
    function _getIdByIndex(uint256 _index) public returns(uint256) {
        // return poolNftMap.getValueByIndex(_index, poolSize, _index + 1);
        require(_index < poolSize, "Array Index Out Of Bounds");
        if(poolNftMap[_index] == 0) {
            poolNftMap[_index] = _index + 1;
