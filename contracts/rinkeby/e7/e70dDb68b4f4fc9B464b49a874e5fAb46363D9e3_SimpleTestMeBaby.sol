...
contract SimpleTestMeBaby is ERC721, Ownable {
    ...
    constructor(
        ...
        _maxTokensPerPurchase = 10;
        _reservedTokensNumber = 30;
        _price = 0.05 ether;
    }

    function Mint(uint256 quantityToMint) external payable {
        require(!pauseState, "Sale paused");
        require(publicSalesState, "Public sales are paused");
        require(totalSupply < MAX_SUPPLY, "Sold out");

        require(
            quantityToMint > 0 && quantityToMint <= _maxTokensPerPurchase,
            "Maximum is 10 babies that can be minted at once"
        );

        require(
            totalSupply + quantityToMint <= MAX_SUPPLY - _reservedTokensNumber,
            "Purchase would exceed the maximum Babies supply"
        );
        require(
            msg.value >= _price * quantityToMint,
    ...
    function SignedMint(
        ...
        require(!pauseState, "Sale is paused");
        require(quantityToMint > 0, "Quantity to mint must be greater 0");
        require(totalSupply < MAX_SUPPLY, "Sold out");

        if (freeMintEligible) {
            require(
                quantityToMint <= _reservedTokensNumber,
                "Exceeds reserved number of Baby supply"
            );
        } else {
            require(
                totalSupply + quantityToMint <=
                    MAX_SUPPLY - _reservedTokensNumber,
                "Purchase would exceed the maximum available Babies supply"
            );
        }
        ...
        require(signerAddress == owner(), "Not authorized to mint");
        require(!_isNonceUsed[nonce], "Nonce has been already used");

        if (freeMintEligible) {
            require(
                _freeClaimQtyMintedByFunguy[msg.sender] + quantityToMint <=
                    maxQtyForOneGuy,
                "Maximum amount for you has been exceeded"
            );
        } else {
            require(
    ...
    function setNumberOfReservedTokens(uint256 newNumber) external onlyOwner {
        require(
            newNumber >= 0,
            "New Reserved Tokens Number cannot be negative"
        );

        require(
            MAX_SUPPLY - (totalSupply + newNumber) >= 0,
            "New Reserved Tokens Number would exceed the maximum Babies supply"
        );
