...
// hevm: flattened sources of /nix/store/xnb72bvqa188nb41c35nsz3pw04mhhvc-geb-proxy-actions/dapp/geb-proxy-actions/src/GebProxyAuctionActions.sol
...
////// /nix/store/xnb72bvqa188nb41c35nsz3pw04mhhvc-geb-proxy-actions/dapp/geb-proxy-actions/src/GebProxyAuctionActions.sol
/// GebProxyActions.sol
...
abstract contract AccountingEngineLike_1 {
    ...
    function safeEngine() external virtual returns (SAFEEngineLike_4);
}
...
contract GebProxyDebtAuctionActions is Common, AuctionCommon {
    ...
    function startAndDecreaseSoldAmount(address coinJoin, address accountingEngineAddress, uint amountToBuy) public {
        ...
        SAFEEngineLike_4 safeEngine = SAFEEngineLike_4(CoinJoinLike_2(coinJoin).safeEngine());
    ...
    function settleAuction(address coinJoin, address auctionHouse, uint auctionId) public {
        ...
        claimProxyFunds(address(CoinJoinLike_2(coinJoin).systemCoin()));
    ...
    function exitCoin(address coinJoin, uint wad) public {
        // Allows adapter to access to proxy's COIN balance in the safeEngine
        if (CoinJoinLike_2(coinJoin).safeEngine().canModifySAFE(address(this), address(coinJoin)) == 0) {
            CoinJoinLike_2(coinJoin).safeEngine().approveSAFEModification(coinJoin);
        }
        CoinJoinLike_2(coinJoin).exit(msg.sender, wad);
    }

    /// @notice Exits full Coin balance to proxy owner
    /// @param coinJoin CoinJoin
    function exitAllCoin(address coinJoin) public {
        exitCoin(coinJoin, CoinJoinLike_2(coinJoin).safeEngine().coinBalance(address(this)) / RAY);
    }
}
...
contract GebProxySurplusAuctionActions is Common, AuctionCommon {
    ...
    function settleAuction(address coinJoin, address auctionHouse, uint auctionId) public {
        ...
        CoinJoinLike_2(coinJoin).exit(msg.sender, toWad(amountToBuy));
