...
contract Market is ReentrancyGuard{
    ...
    // 上架
    function createMarketItem( address nftContract, uint256 tokenId, uint256 price, uint256 deposit, uint256 validTime) public nonReentrant{
    ...
    // 租
   function rentMarketItem(address nftContract, uint256 itemId) public payable nonReentrant {
    ...
    function getDeposit(uint256 itemId) public payable{
        ...
        require(
            msg.sender == lender,
            "The owner of the NFT not match!"
        );
        // TODO: 時間檢查、未歸還檢查
        payable(msg.sender).transfer(deposit);
    }
    ...
    // 個人頁：上架商品
    function fetchMyMarketItems(address userAddr) public view returns (MarketItem[] memory) {
        uint256 itemCount = _itemIds.current();
        uint256 unsoldItemCount = _itemIds.current() - _itemsSold.current();
        uint256 currentIndex = 0;
    ...
    // 個人頁：租賃中商品
    function fetchMyRentingItems(address userAddr)public view returns (MarketItem[] memory) {
        uint256 itemCount = _itemIds.current();
        uint256 unsoldItemCount = _itemIds.current() - _itemsSold.current();
        uint256 currentIndex = 0;
