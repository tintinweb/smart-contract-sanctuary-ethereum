...
contract CuBeanz is ERC721A, Ownable, ReentrancyGuard {
    ...
    bool burnMintActive = true;
    
  
    uint burnRate = 10000 ether;
    uint public cubePrice = 22000 ether; 
   ...
   //@dev maps tokenIds to number of times they've attacked
    mapping(uint => uint8) public numAttacks;



      // @dev these mappings track how many one has minted on public and WL respectively
    mapping(address =>uint256) public rOneMapping;
    mapping(address => uint) public rTwoMapping;
    ...
    event ATTACK(uint indexed tokenId);
 
    function teamMint(address to ,uint256 amount) external onlyOwner  nonReentrant{
        if(teamMints + amount > maxTeamMints) revert MaxTeamMints();
        teamMints+= uint16(amount);
        _safeMint(to,amount);
    }
    function roundOneMint(uint256 amount) external  nonReentrant{
        ...
        if(rOneMapping[msg.sender] + amount > maxRoundOneMints) revert MaxMints();
         ...
         roundOneMints += uint16(amount);
        _safeMint(msg.sender,amount);
    }

    function roundTwoMint(uint amount) external  nonReentrant{
        if(!roundTwoLive) revert SaleNotStarted();
