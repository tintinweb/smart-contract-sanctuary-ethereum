...
contract Field is Ownable, IERC721Receiver, Pauseable {
    ...
    function addManyToFieldAndPack(address account, uint16[] calldata tokenIds) external nonReentrant {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (isPlayer(tokenIds[i])) {
                ...
                unstackDate[tokenIds[i]]=block.timestamp + 2 days;
            }
            else
             {
    ...
    function claimManyFromFieldAndPack(uint16[] calldata tokenIds, bool unstake) external nonReentrant whenNotPaused _updateEarnings {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            if (isPlayer(tokenIds[i])){
                ...
                uint16 goalIdx = getIndexGoal(msg.sender,tokenIds[i]);
   ...
   */
    ...
    return tab;
     ...
     function getIndexPlayer(address _address, uint16 _tokenId) public view returns (uint16) {
       ...
       function getIndexGoal(address _address, uint16 _tokenId) public view returns (uint16) {
    ...
    function deleteGoalEntry(address _address, uint16 _tokenIdx) internal {
