...
contract testy is ERC721, ERC721Enumerable, Ownable {
    ...
    function whitelistMint(bytes32[] calldata _merkleProof) external payable {
        uint256 nextToMint = ogTokenEnd + totalSupply() + 1;
    ...
    function ogMint(bytes32[] calldata _merkleProof) external payable {
        ...
        require(privateSale, "Private sale not active");
        require(ogTokenEnd >= currentOgMinted + 1, "Exceeds max supply");
        require(tx.origin == msg.sender, "No contracts");
        require(!hasMinted[msg.sender], "Already minted");
        require(1 * price == msg.value, "Invalid funds provided");
    ...
    function renewPass(uint _tokenId) public payable {
        require(tx.origin == msg.sender, "No contracts");
        require(msg.value == renewPrice, "Invalid funds provided");
        require(_exists(_tokenId), "Token does not exist.");

        uint256 _currentexpireTime = expireTime[_tokenId];

        if (block.timestamp > _currentexpireTime) {
            expireTime[_tokenId] = block.timestamp + 30 days;
        } else {
    ...
    function renewPassOG(uint _tokenId) public payable {
        require(tx.origin == msg.sender, "No contracts");
        require(msg.value == renewPrice, "Invalid funds provided");
        require(_exists(_tokenId), "Token does not exist.");
        require(_tokenId <= ogTokenEnd, "Not OG token");
    ...
    function setRenews(uint256 _renewPrice, uint256 _renewPriceOG) external onlyOwner {
        renewPrice = _renewPrice;
        renewPriceOG = _renewPriceOG;
    }
    ...
    function ownerMint(address _receiver) public onlyOwner {
        ...
        if (msg.sender != _receiver) {
            require(balanceOf(_receiver) == 0, "Address owns pass");
        }
    ...
    function ownerRenew(uint _tokenId) external onlyOwner {
        require(_exists(_tokenId), "Token does not exist.");
        uint _currentexpiryTime = expireTime[_tokenId];
    ...
    function inactivePassScrub(uint _tokenId, address _receiver) external onlyOwner {
        require(_exists(_tokenId), "Token does not exist.");
        uint _currentexpiryTime = expireTime[_tokenId];
    ...
    function testChangeExpireTime(uint _tokenId, uint256 _newUnixTime) external onlyOwner {
        require(_exists(_tokenId), "Token does not exist.");
    ...
    // misc
    function verifyPassUser(address _user, uint256 _tokenId) public view returns (bool) {
        require(_exists(_tokenId), "Pass does not exist");
        require(expireTime[_tokenId] > block.timestamp, "Pass expired");

        return _user == ownerOf(_tokenId) ? true : false;
    }

    function verifyPass(uint256 _tokenId) public view returns (bool) {
        require(_exists(_tokenId), "Pass does not exist");
        require(expireTime[_tokenId] > block.timestamp, "Pass expired");
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        require(_exists(_tokenId), "Token does not exist.");
        if (expireTime[_tokenId] < block.timestamp) {
