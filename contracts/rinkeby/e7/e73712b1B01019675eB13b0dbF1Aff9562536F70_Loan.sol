...
contract Loan {
  ...
  constructor( address _odonAddress,
  ...
  function init(uint256 _amount) public payable {
    ...
    require(
      IERC20(odonAddress).transferFrom(msg.sender, address(this), _amount),
 ...
 // count require odon amount by passing ether amount
  ...
  function loan(uint256 _amount, uint256 _duration, uint256 _type) public {
    if(_type == 1){
   require(_amount >= ethPerOdon, "loanEther: Not enough fund in order to loan");
    }
    if(_type == 2){
    require(_amount >= usdcPerOdon, "loanUsdc: Not enough fund in order to loan");
    }
    if(_type == 3){
    require(_amount >= usdtPerOdon, "loanUsdt: Not enough fund in order to loan");
    }
    if(_type == 4){
    require(_amount >= wbtcPerOdon, "loanWbtc: Not enough fund in order to loan");
    }
    require(checkEnoughLiquidity(_amount, _type), "loanEther: not enough liquidity");
    LoanRequest memory newLoan;
    newLoan.borrower = msg.sender;
    newLoan.loanAmount = _amount;
    newLoan.collateralAmount = collateralAmount(_amount, 1) * (10 ** 18);
    ...
    require(
      IERC20(odonAddress).transferFrom(msg.sender, address(this), newLoan.collateralAmount),
    ...
    if(_type == 2){
     require(
      IERC20(usdcAddress).transferFrom(address(this), msg.sender, _amount),
      "loanEther: Transfer usdc from contract to user failed"
    );
      usdcTotalLiquidity=usdcTotalLiquidity.sub(_amount);
      IERC20(usdcAddress).decreaseAllowance(address(this), _amount);
    }
    if(_type == 3){
    require(
      IERC20(usdtAddress).transferFrom(address(this), msg.sender, _amount),
      "loanEther: Transfer usdt from contract to user failed"
    );
      usdtTotalLiquidity=usdtTotalLiquidity.sub(_amount);
      IERC20(usdtAddress).decreaseAllowance(address(this), _amount);
    }
    if(_type == 4){
    require(
      IERC20(wbtcAddress).transferFrom(address(this), msg.sender, _amount),
      "loanEther: Transfer wbtc from contract to user failed"
    );
      wbtcTotalLiquidity=wbtcTotalLiquidity.sub(_amount);
      IERC20(wbtcAddress).decreaseAllowance(address(this), _amount);
    }
    IERC20(odonAddress).increaseAllowance(address(this), newLoan.collateralAmount);
  ...
  function lendToken(uint256 _amount, uint256 _type) public {
    require(IERC20(odonAddress).transferFrom(
    ...
    lendCount++;
    userLendsCount[msg.sender]++;
    IERC20(odonAddress).increaseAllowance(address(this), request.paybackAmountToken);
    emit NewLend(
      request.lender,
      request.lendAmountEther,
      request.lendAmountToken,
  ...
  function withdraw(uint256 _id) public {
    ...
    if(block.timestamp > req.timeCanGetInterest) {
      ...
      if(req.isLendEther) {
        ...
        if(req.mtype == 2){
        IERC20(usdcAddress).transferFrom(address(this), req.lender, req.paybackAmountToken);
        } else if(req.mtype == 3) {
        IERC20(usdtAddress).transferFrom(address(this), req.lender, req.paybackAmountToken);
        } else if(req.mtype == 4) {
        IERC20(wbtcAddress).transferFrom(address(this), req.lender, req.paybackAmountToken);
        }
      ...
      if(req.isLendEther) {
        ...
        if(req.mtype == 2){
        IERC20(usdcAddress).transferFrom(address(this), req.lender, req.lendAmountToken);
        }
       else if(req.mtype == 3){
        IERC20(usdtAddress).transferFrom(address(this), req.lender, req.lendAmountToken);
        }
       else if(req.mtype == 4){
        IERC20(wbtcAddress).transferFrom(address(this), req.lender, req.lendAmountToken);
        }
        // transfer token to lender
        emit Withdraw(
          false,
  ...
  function payback(uint256 _id) public payable {
    ...
    if(loanReq.isLoanEther){
      ...
      if(loanReq.mtype == 2){
        require(
        IERC20(usdcAddress).transferFrom( msg.sender,address(this), msg.value),
        "payback: Transfer collateral from contract to user failed"
        );
        IERC20(usdcAddress).increaseAllowance(address(this), msg.value);
        usdcTotalLiquidity = usdcTotalLiquidity.add(msg.value);
      }
      else if(loanReq.mtype == 3){
        require(
        IERC20(usdtAddress).transferFrom( msg.sender,address(this), msg.value),
        "payback: Transfer collateral from contract to user failed"
        );
        IERC20(usdtAddress).increaseAllowance(address(this), msg.value);
         usdtTotalLiquidity = usdtTotalLiquidity.add(msg.value);
      }
      else if(loanReq.mtype == 4){
        require(
        IERC20(wbtcAddress).transferFrom( msg.sender,address(this), msg.value),
        "payback: Transfer collateral from contract to user failed"
        );
        IERC20(wbtcAddress).increaseAllowance(address(this), msg.value);
         wbtcTotalLiquidity = wbtcTotalLiquidity.add(msg.value);
      }
    }
    require(
      IERC20(odonAddress).transferFrom(address(this), msg.sender, loanReq.collateralAmount),
      "payback: Transfer collateral from contract to user failed"
    );
    IERC20(odonAddress).decreaseAllowance(address(this), msg.value);
...
interface IERC20 {
    ...
    function decreaseAllowance(address spender, uint256 subedValue) external returns (bool);
    /**
