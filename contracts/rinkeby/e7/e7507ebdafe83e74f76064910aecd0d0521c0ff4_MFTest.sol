...
contract MFTest is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public maxPublicSupply = 5000;
    uint256 public mintPrice = 0.0 ether;
    uint256 public maxTotalMints = 3;
...

    {
        ...
        setBaseURI("");
        setNotRevealedURI("ipfs://bafkreihlygrru2thwu6f6kdlasz7nqsv3ejfpawjqehg7dw76xg2akglsy");
    }
    ...
    function teamMint(address to ,uint256 amount) external onlyOwner{
        ...
        //@dev Max mint for each wallet is encoded in the signer. 
    function publicMint(uint256 amount) external payable checkIfUser nonReentrant{

        uint256 supply = totalSupply();

        require(supply + amount <= maxPublicSupply,"Public Mint Sold Out!");
        require(publicMints[msg.sender] + amount <=maxTotalMints,"You've Maxed Out Your Mints");
    ...
    function setPublicSupply(uint256 newSupply) external onlyOwner {
        require(newSupply <= maxPublicSupply,"Can't Increase Supply");
        maxPublicSupply = newSupply;
    }
