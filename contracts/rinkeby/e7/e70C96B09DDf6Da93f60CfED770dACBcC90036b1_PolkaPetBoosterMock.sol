...
contract PolkaPetBooster is Ownable, ERC1155Holder, ReentrancyGuard, Pausable {
	...
	function openPack(uint256 id) external whenNotPaused nonReentrant {
		...
		fulfillOpenPack(msg.sender, randomness);
	}
	...
	function fulfillOpenPack(address reciepient, uint256 randomness) internal {
		...
		while(generated < packSize) {
			iterations++;
			uint256 randomSlot = expandOnce(randomness, iterations) % slots.length;
			if(generated < slots.length) {
				//need unique
				bool duplicated = false;
				for(uint i = 0;i < generated; i++) {
					if(slotIds[i].slot == randomSlot) {
						duplicated = true;
						break;
					}
				}
			...
			while(! found){
				...
				if(nftQty > 0) {
					...
					bool duplicated = false;
					for(uint i = 0;i < generated;i++) {
						if(slotIds[i].slot == randomSlot && slotIds[i].id == nft.ID) {
							duplicated = true;
							break;
						}
					}
	...
	function getRandomNumber() internal returns (uint256 requestId) {
		requestId = uint256(keccak256(abi.encodePacked(randomNonce)));
		randomNonce += 1;
	}
