...
contract CKS01 is Auth {
    ...
    uint256 private _stakedWalletsCount; 
    ...
    event SettingsUpdated(address caToken, address caTreasuryDAO, uint256 unstakeCooldownDuration, uint16 rewardsAPR, uint16 prematureClaimPenalty);
    ...
    constructor() Auth(msg.sender) {    
        ...
        stakingSettings.unstakeCooldownDuration = 14*60; //TODO 14 minutes in testnet, 14 days in mainnet
        ...
        stakingSettings.rewardsUpdatePeriod = 1; //1 means rewards recalculate every second (in reality every block where new block.timestamp is reached)
    }
    ...
    function settingsChange(address caToken, address caTreasuryDAO, uint256 unstakeCooldownDuration, uint16 rewardsAPR, uint16 prematureClaimPenalty, uint16 rewardsUpdatePeriod) external onlyOwner {
        ...
        stakingSettings.rewardsAPR = rewardsAPR; //set new APR, probably won't change but keep the ability to do so
        ...
        require(rewardsUpdatePeriod>0 && rewardsUpdatePeriod<=86400, "Invalid reward recalc period");
        stakingSettings.rewardsUpdatePeriod = rewardsUpdatePeriod;
        
        emit SettingsUpdated(caToken, caTreasuryDAO, unstakeCooldownDuration, rewardsAPR, prematureClaimPenalty);
    }
    ...
    function _lockRewards(address staker) internal {
        uint256 newRewardAmount  = _calculateRewards(staker);
    ...
    function getStakingInfo(address staker) external view returns (uint256 started, uint256 lastChanged, uint256 stakedTokens, uint256 rewardsAccrued, uint16 activeAPR, bool rewardsPausedByUnstake, uint256 lockedUntil ) {
        return (
            ...
            _stakingData[staker].rewardTokensOwned + _calculateRewards(staker),
            _stakingData[staker].activeAPR,
            _getRewardsPaused(staker),
            _stakingData[staker].unstakeLockedUntil
        );
    }
    ...
    function _increaseStaking(address staker, uint256 amount) internal {
        ...
        if (_stakingData[staker].firstSeen == 0) {
            //this wallet never staked before so set value and push it in the wallet list
            _stakingData[staker].firstSeen = block.timestamp;
    ...
    function unstakeTokens(uint256 amount) external {
        ...
        uint256 tokensAvailable = _stakingData[staker].stakedTokens + _stakingData[staker].rewardTokensOwned + _calculateRewards(staker);
