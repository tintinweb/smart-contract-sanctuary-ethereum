...
import { AddressData, AppStorage, TokenOwnership } from "../libraries/LibAppStorage.sol";
...
contract MintingFacet is IERC721 {
    ...
    function purchase(address targetAddress, uint256 quantity) external payable {
        ...
        if(quantity > s._maxAllowed)
            revert QuantityMoreThanAllowed();

        /// Revert if outside sale limit
        if(totalMinted() + quantity > s.saleLimit)
            revert QuantityMoreThanRemainingAvailable();

        /// Revert if incorrect amount sent
        if(msg.value != s.priceWEI * quantity)
            revert IncorrectAmountSent();

        _safeMint(targetAddress, quantity, '');
    }

    // Mint as a holder of an approved project
    function mintAsHolder(
        address targetAddress,
        uint256 freeQuantity,
        uint256 paidQuantity,
        address[] memory holderContracts,
        uint256[] memory tokenIds
    ) external payable{
        uint256 totalQuantity = freeQuantity + paidQuantity;
        /// Revert if quantity more than allowed
        if(paidQuantity > s._maxAllowed)
            revert QuantityMoreThanAllowed();

        /// Revert if outside sale limit
        if(totalMinted() + totalQuantity > s.saleLimit)
            revert QuantityMoreThanRemainingAvailable();

        /// Revert if incorrect amount sent
        if(msg.value != s.priceWEI * paidQuantity)
            revert IncorrectAmountSent();

        if(holderContracts.length != tokenIds.length)
            revert ParamArrayLengthsDoNotMatch();

        for (uint256 index = 0; index < freeQuantity; index++) {
            address holderContractAddress = holderContracts[index];
            uint256 tokenId = tokenIds[index];
            if(s._redeemedTokens[holderContractAddress][tokenId])
                revert TokenAlreadyRedeemed();
            if(!s._approvedProjects[holderContractAddress])
                revert NotApprovedProject();
            s._redeemedTokens[holderContractAddress][tokenId] = true;
        }
        _safeMint(targetAddress, totalQuantity, '');
        /// Delete variables
...
struct AppStorage {
    ...
    /// Mapping of holder contract redeemed token ids
    mapping(address => mapping(uint256 => bool)) _redeemedTokens;
    /// Mapping of approved projects
    mapping(address => bool) _approvedProjects;
...
contract Modifiers {
    ...
    modifier onlyEditor() {
        if(!s._editors[msg.sender])
            revert CallerIsNotEditor();
        _;
    }
