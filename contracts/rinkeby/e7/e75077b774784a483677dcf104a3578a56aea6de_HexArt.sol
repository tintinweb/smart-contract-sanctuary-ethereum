...
abstract contract ERC2981 is IERC2981, ERC165 {
    ...
    uint256 public amountToRemoveAsset = 36900000000000000;
...
contract HexArt is ERC721URIStorage, ERC2981, Ownable {
    HexArtMarket public marketplace;
    ...
    mapping(uint256 => NFTDetail[]) public nftAssetDetails;
    ...
    function setMarketplace(address _marketplace) external onlyOwner {
        marketplace = HexArtMarket(_marketplace);
    }
    ...
    function mint(
        ...
        assetLockPeriod[tokenId] = block.timestamp + (1800 * _lockperiodInDays);
    }

    /**
     * @dev Burns token of entered token id.
     */
    function burn(uint256 _tokenId) external isNFTOwner(_tokenId) {
    ...
    function addNFTAsset(
        ...
        uint256 _tokenId
    ) external isWhitelisted(_assetContract) isNFTOwner(_tokenId) {
      
        require(
            !marketplace.isListed(_tokenId),
            "Cannot add asset to a listed NFT"
        );
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            nftAssetDetails[_tokenId].push(
                NFTDetail({hsiTokenId: _tokenIds[i], assettype: _assetContract})
            );
        }
    }
    ...
    function removeNFTAsset(
        ...
        uint256 _assetId,
        address _assetContract
    ) public isNFTOwner(_nftTokenId) {
        require(
            block.timestamp >= assetLockPeriod[_nftTokenId],
            "Cannot remove stake during lock period"
        );
      
        require(
            !marketplace.isListed(_nftTokenId),
            "Cannot remove asset from a listed NFT"
        );
        NFTDetail[] memory array = getAssetsOfNFT(_nftTokenId);
        for (uint256 i = 0; i < array.length; i++) {
            if (
                array[i].hsiTokenId == _assetId &&
                array[i].assettype == _assetContract
            ) {
            
                require(removeAsset(array[i].assettype, _assetId), "Asset not removed successfully");
    ...
    function removeNFTPayable(
        ...
        uint256 _assetId,
        address _assetContract
    ) public payable  isNFTOwner(_nftTokenId){
        require(
            block.timestamp >= assetLockPeriod[_nftTokenId],
            "Cannot remove stake during lock period"
        );
    ...
    function removeAllNFTAssetsPayable(uint256 _nftTokenId) public payable isNFTOwner(_nftTokenId) {
        require(
            block.timestamp >= assetLockPeriod[_nftTokenId],
            "Cannot remove stake during lock period"
        );
        ...
        NFTDetail[] memory array = getAssetsOfNFT(_nftTokenId);
        for (uint256 i = 0; i < array.length; i++) {
require(removeAsset(array[i].assettype, array[i].hsiTokenId), "Asset not removed successfully");
            removeAssetToken(i, _nftTokenId);
        }
    }

    function removeAllNFTAssets(uint256 _nftTokenId) public isNFTOwner(_nftTokenId) {
         require(
                block.timestamp >= assetLockPeriod[_nftTokenId],
                "Cannot remove stake during lock period"
            );
   ...
   require(removeAsset(array[i].assettype, array[i].hsiTokenId), "Asset not removed successfully");
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public override isNFTOwner(tokenId){
    ...
    if(index < nftAssetDetails[_tokenId].length){
    ...
    function removeAsset(address _nftContract, uint256 _assetId) internal returns(bool){
