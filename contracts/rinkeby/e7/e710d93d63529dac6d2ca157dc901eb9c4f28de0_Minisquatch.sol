...
contract Minisquatch is ERC721Enumerable, Ownable {
  ...
  function mint() public payable {
    ...
    if (msg.sender != owner()) {
        ...
        try IMetasquatch(metasquatchAddress).safeTransferFrom(msg.sender, address(this), IMetasquatch(metasquatchAddress).walletOfOwner(msg.sender)[0], ""){
        }catch{
    ...
    if(IMetasquatch(metasquatchAddress).balanceOf(msg.sender) >= 4){
      for(uint i = 0; i < IMetasquatch(metasquatchAddress).balanceOf(msg.sender); i++){
