...
contract RIP is ERC721A, Payable {
    ...
    uint256 private maxSupply = 2500;
    ...
    // Presale
    uint256 public constant PRESALE_GETS = 5;
    ...
    modifier withinMintLimit(uint256 quantity) {
        require((_totalMinted() + quantity) <= maxSupply, "RIP: Exceeds available tokens");
        _;
    }
    ...
    /**
     * Update URI.
     * @param _uri The new static URI.
     */
    function setStaticURI(string memory _uri) external onlyOwner {
        staticURI = _uri;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString(), ".json")) : staticURI;
    }
