...
contract svgManip {
    ...
    function get_rand_rgb(string memory prefix, string memory tokenId, uint256 min, uint256 range) view internal returns (string[3] memory) {
        return [
            _get_rand_in_range_toStr(_random(abi.encodePacked(prefix, "RED RED RED RED", _seed_phrase, tokenId)), min, range),
            _get_rand_in_range_toStr(_random(abi.encodePacked("GREEN GREEN GREEN", prefix, _seed_phrase, tokenId)), min, range),
            _get_rand_in_range_toStr(_random(abi.encodePacked(_seed_phrase, "BLUE BLUE BLUE", tokenId, prefix)), min, range)
        ];
    }
