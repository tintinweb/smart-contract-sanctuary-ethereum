...
contract AuctionHouse is Ownable, ReentrancyGuard {
  ...
  // NONE: can be bought/sold directly without auction
  enum AuctionStatus {
    NONE,
    OPEN,
    SETTLED,
  ...
  constructor(
    ...
    uint256 _minimumBidSize,
    uint256 _minimumAuctionLiveness
  ) {
  ...
  function purchase(uint256 _tokenId) external payable {
    uint256 price = auctions[_tokenId].currentPrice;
    require(msg.value == price, 'Please submit the asking price in order to complete the purchase');
    require(auctions[_tokenId].status == AuctionStatus.NONE, 'This NFT has already sold');

    uint256 fee = msg.value.mul(commissionPercentage).div(1 ether);

    payable(address(this)).transfer(fee);
    payable(auctions[_tokenId].seller).transfer(msg.value.sub(fee));
    address contractAddress = auctions[_tokenId].contractAddress;
    IERC721(contractAddress).transferFrom(address(this), msg.sender, _tokenId);
    auctions[_tokenId].status = AuctionStatus.SETTLED;
  }

  /**
   * @dev Returns all auctions
   */
  function getAllAuctions() external view returns (Auction[] memory) {
    uint256 resultIndex = 0;
    Auction[] memory totalAuctions = new Auction[](totalAuctionCount.current());
    for (uint256 i = 1; i <= totalAuctionCount.current(); i++) {
      totalAuctions[resultIndex] = auctions[i];
      resultIndex++;
    }
   ...
   */
  function getOpenAuctions() external view returns (Auction[] memory) {
   ...
   */
  function placeBid(uint256 auctionId, uint256 bidPrice)
    external
    payable
    openAuction(auctionId)
    nonExpiredAuction(auctionId)
    nonReentrant
  {
    Auction storage auction = auctions[auctionId];
    require(
      bidPrice >= auction.currentPrice.add(minimumBidSize),
      'Bid has to exceed current price by the minimumBidSize or more'
    );
    require(
      msg.value == bidPrice.add(calculateCommission(bidPrice)),
      'Transaction value has to equal price + commission'
    );
  ...
  function setCommissionPercentage(uint256 _percentage) external onlyOwner {
    uint256 newComPerc = _percentage.mul(1 ether).div(100);
    commissionPercentage = newComPerc;
  }
