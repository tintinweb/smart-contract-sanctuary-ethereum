...
contract Referrals is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function addNewChildReferralToFather(
        ...
        emit AddNewChildReferralToFather(fatherReferral, childReferral, code);
    }

    function storageReferralDepositAdmin(
        address fatherReferral
    ) public payable onlyOwnerOrHelperAccount {
        assertOneYocto();
        require(
    ...
    function updateLevelReferralFather(
        ...
        bool isPresent;
        for (uint256 i = 0; i < keyListReferralLevelsAndPercents.length; i++) {
            if (keyListReferralLevelsAndPercents[i] == newLevel) {
                isPresent = true;
                break;
            }
        }
        ...
        onlyOwnerOrProxyRouterAccount
    {
        ...
        emit NewReferralPayout(fatherReferral, _childReferral, _amount, msg.value);
    }
