...
contract ProjectP22Token is OwnableUpgradeable, ReentrancyGuardUpgradeable, IERC721ReceiverUpgradeable, ERC721AUpgradeable, VRFConsumerBaseV2 {
    string public Provenance;

    bool public saleIsActive;

    uint256 public maxProjectP22;
    ...
    bool public goldenTicketRedeemActive;
    struct LinkRequest {
      uint32 tokenId;
      address userAddress;
    }
    ...
    uint32 public goldenTicketTokenCount;
    uint32 public goldenTicketTokenMinAvailableIndex;
    uint32 public goldenTicketTokenMaxAvailableIndex;
    ...
    address public goldenTicketTokenAddress;

    // ############################# constructor #############################
    function initialize() external initializer {
      __Ownable_init();
      __VRFConsumerBaseV2_init(chainLinkVrfCoordinator);
      ...
      chainLinkSubscriptionId = 2411;
      chainLinkVrfCoordinator = 0x6168499c0cFfCaCD319c818142124B7A15E857ab;
      chainLinkLinkAddress = 0x01BE23585060835E02B77ef475b0Cc51aA1e0709;
      chainLinkKeyHash = 0xd89b2bf150e3b9e13446986e571fb9cab24b13cea0a43ea20a6049a85cc807cc;
      chainLinkCallbackGasLimit = 400000;
      chainLinkRequestConfirmations = 3;
      numWords = 1;

      goldenTicketTokenCount = 0;
      goldenTicketTokenMinAvailableIndex = 0;
      goldenTicketTokenMaxAvailableIndex = 2;
      goldenTicketTokenAddress = 0xf1183A89c576aa07f5DBE5a13ca4B42855fb6545; // TODO Update before LIVE

      maxProjectP22 = 500000;
      saleIsActive = false;
    }
    ...
    function setGoldenTicketRedeemActive(bool isActive) external onlyOwner {
      require(isActive != goldenTicketRedeemActive, "same value already set");
      goldenTicketRedeemActive = isActive;
    }

    function withdrawGoldenTicketToken(address contractAddress, uint256 tokenId) external onlyOwner {
      Erc721Contract erc721ContractInstance = Erc721Contract(contractAddress);
      erc721ContractInstance.transferFrom(address(this), msg.sender, tokenId);
      uint32 index = goldenTicketTokenIndex[tokenId];
      if(goldenTicketTokens[index] == tokenId && isGoldenTicketTokenIndexPicked[index] == false) {
        isGoldenTicketTokenIndexPicked[index] = true;
      }
    }
    ...
    function Mint(uint numberOfTokens, address toAddress, uint tokenStartIndex, uint randomNumber) external {
      ...
      require(currentTotalSupply + numberOfTokens <= maxProjectP22, "Purchase would exceed max supply of ProjectP22s");
      _safeMint(toAddress, numberOfTokens, tokenStartIndex, randomNumber);
    }
    ...
    function RedeemGoldenTicket(uint32 tokenId) external {
      require(goldenTicketRedeemActive, "Golden ticket redeem is not active");
    ...
    enum GasFreeListingState {
        Active,
        Inactive
    }

    /// @notice The state of the pre-approval for a given owner
    mapping(address => GasFreeListingState) private gasFreeListingState;
        ...
        returns (bool)
    {
        ...
        return
            gasFreeListingState[owner] == GasFreeListingState.Active &&
        ...
        override
    {
        ...
        if (operator == OpenSeaGasFreeListing.proxyFor(owner)) {
            gasFreeListingState[owner] = approved ? GasFreeListingState.Active : GasFreeListingState.Inactive;
    ...
    function _beforeTokenTransfers(
        ...
        if (to == address(0) || gasFreeListingState[to] == GasFreeListingState.Inactive) {
            return;
        }
        ...
        if (operator == address(0)) {
            gasFreeListingState[to] = GasFreeListingState.Inactive;
            return;
        }
...
contract ERC721AUpgradeable is Initializable, ContextUpgradeable, ERC165Upgradeable, IERC721AUpgradeable {
    ...
    // VESTED FUNCTION: START || ADDED IN THIS CONTRACT TO SAVE GAS
    mapping(uint256 => uint256) private tokenImageMapping;
    mapping(uint256 => string) private slabbed;
    error QueryForNonexistentToken();

    function getSlabbedChar(uint randomNumber) internal pure returns(string memory) {
        uint256 moduloNumber = randomNumber % 10;
        if (moduloNumber > 5) {
        return "C";
        } else if (moduloNumber > 2) {
        return 'B';
        }
        return 'A';
    }

    function getTokenImageMapping(uint tokenId) external view returns(uint) {
        uint256 curr = tokenId;

        unchecked {
            if (_startTokenId() <= curr && curr < _currentIndex) {
                uint tokenImageIndex = tokenImageMapping[curr];
                if (tokenImageIndex > 0) {
                    return tokenImageIndex - 1;
                }
                while (true) {
                    curr--;
                    tokenImageIndex = tokenImageMapping[curr];
                    if (tokenImageIndex > 0) {
                        return tokenImageIndex + (tokenId - curr) - 1;
                    }
                }
            }
        }
    ...
    function getSlabbed(uint tokenId) external view returns(string memory) {
        uint256 curr = tokenId;

        unchecked {
            if (_startTokenId() <= curr && curr < _currentIndex) {
                string memory salb = slabbed[curr];
                if (!compareStrings(salb, "")) {
                    return salb;
                }
                while (true) {
                    curr--;
     ...
     */
    function _safeMint(address to, uint256 quantity, uint imageStartIndex, uint randomNumber) internal {
        _safeMint(to, quantity, '', imageStartIndex, randomNumber);
    }
    ...
    function _safeMint(
        ...
        unchecked {
            ...
            // VESTED CODE: START || Reason to add code here is to save gas
            tokenImageMapping[startTokenId] = imageStartIndex + 1;
            slabbed[startTokenId] = getSlabbedChar(randomNumber);
            // VESTED CODE: END
