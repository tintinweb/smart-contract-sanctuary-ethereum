 ...
 * Any changes/appends in storage variable needs to happen in VolmexRealizedVolVaultStorage.
 * VolmexRealizedVolVault shouldn't inherit from any other contract aside from VolmexVault,VolmexRealizedVolVaultStorage
 */
contract VolmexRealizedVolVault is VolmexVault, VolmexRealizedVolVaultStorage {
    ...
    function withdrawInstantly(uint256 amount) external nonReentrant {
        ...
        require(amount > 0, "VolmexRealizedVolVault: !amount");
        require(
        ...
        uint256 sharesToBeBurn = previewDeposit(amount);
     ...
     */
    function _closeShort(address oldVToken) private {
        uint256 lockedAmount = vaultState.lockedAmount;
        if (oldVToken != address(0)) {
    ...
    function redeemByMarketMaker(address vToken, uint256 shares) external {
        require(shares > 0, "VolmexRealizedVolVault: shares cannot be zero");
        require(
            vToken != address(0),
            "VolmexRealizedVolVault: address cannot be zero"
        );
        require(
            shares <= IVToken(vToken).balanceOf(msg.sender),
            "VolmexRealizedVolVault: Insufficient balance"
        );
        uint256 amountToBeWithdraw = (
            shares.div(10**18).mul(vTokenState.vTokenPnL)
        );

        emit VTokenRedeemed(msg.sender, amountToBeWithdraw);

        IVToken(vToken).burn(msg.sender, shares);
     ...
     * @param auction is the address of the auction contract
    ...
    function rollToNextVtoken(
        ...
        // TODO: Search; Is it required to approve the VToken for EasyAuction to start auction.
    ...
    IVolmexVault
{
    ...
    function setNewKeeper(address newKeeper) external onlyOwner {
        require(newKeeper != address(0), "VolmexVault: !newKeeper");
        keeper = newKeeper;
    }
    ...
    function setFeeRecipient(address newFeeRecipient) external onlyOwner {
        require(newFeeRecipient != address(0), "VolmexVault: !newFeeRecipient");
        require(
    ...
    function setCap(uint256 newCap) external onlyOwner {
        require(newCap > 0, "VolmexVault: !newCap");
