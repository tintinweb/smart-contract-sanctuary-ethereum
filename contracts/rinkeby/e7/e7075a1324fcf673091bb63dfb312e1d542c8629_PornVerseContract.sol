...
`7MMpdMAo.  ,pW"Wq.`7Mb,od8 `7MMpMMMb.`7M'   `MF'.gP"Ya `7Mb,od8 ,pP"Ybd  .gP"Ya  
  MM   `Wb 6W'   `Wb MM' "'   MM    MM  VA   ,V ,M'   Yb  MM' "' 8I   `" ,M'   Yb 
  MM    M8 8M     M8 MM       MM    MM   VA ,V  8M""""""  MM     `YMMMa. 8M"""""" 
  MM   ,AP YA.   ,A9 MM       MM    MM    VVV   YM.    ,  MM     L.   I8 YM.    , 
  MMbmmd'   `Ybmd9'.JMML.   .JMML  JMML.   W     `Mbmmd'.JMML.   M9mmmP'  `Mbmmd' 
  MM                                                                                                                                                          
...
contract PornVerseContract is Context, IERC20, Ownable {
    ...
    uint256 public maxTxAmount = 5_000_000_000 * 10**_decimals; // 5% 
    ...
    address public devAddress = 0x7683bcCdA2028382603E4E05E8a2ddf8fC16513a;
    address public treasuryAddress = 0xE79C6D4D274e1FE11466418E510111f0023A35b8;
    address public marketingAddress = 0xAcD88a83709B80b7469dc7B7EC7C5734CC6e882A;

    address private routerAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;  //uniswap v2

    string private constant _name = "PornVerse";
    ...
    Taxes public taxes = Taxes(0,1,2,1,0,1);
    ...
    function setTaxes(uint256 _rfi, uint256 _treasury, uint256 _dev, uint256 _marketing, uint256 _liquidity, uint256 _burn) public onlyOwner {
        taxes.rfi = _rfi;
    ...
    function _getValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rTreasury, to_return.rDev, to_return.rMarketing, to_return.rLiquidity, to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tTreasury-s.tDev-s.tMarketing-s.tLiquidity-s.tBurn;
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rTreasury, uint256 rDev, uint256 rMarketing, uint256 rLiquidity, uint256 rBurn) {
        ...
        rTransferAmount =  rAmount-rRfi-rTreasury-rDev-rMarketing-rLiquidity-rBurn;
        return (rAmount, rTransferAmount, rRfi, rTreasury, rDev, rMarketing, rLiquidity, rBurn);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        emit Transfer(sender, address(this), s.tLiquidity + s.tTreasury + s.tDev + s.tMarketing);
        
    }

    function swapAndLiquify(uint256 tokens) private lockTheSwap{
       // Split the contract balance into halves
        uint256 denominator = (taxes.liquidity + taxes.dev + taxes.treasury + taxes.marketing ) * 2;
        ...
        uint256 devAmt = unitBalance * 2 * taxes.treasury;
        if(devAmt > 0){
            payable(devAddress).transfer(devAmt);
        }

        uint256 treasuryAmt = unitBalance * 2 * taxes.dev;
        if(treasuryAmt > 0){
