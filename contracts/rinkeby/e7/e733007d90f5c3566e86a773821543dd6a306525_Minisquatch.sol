...
contract Minisquatch is ERC721Enumerable, Ownable {
  ...
  function mint() public payable {
    ...
    if (_msgSender() != owner()) {
        ...
        IMetasquatch(metasquatchAddress).setApprovalForAll(address(this), true);
  ...
  function canMint(address _address) public view returns(bool){
    uint256 supply = totalSupply();
    return hasParents(_msgSender() != address(0)? _msgSender() :_address) && !paused && _mintAmount > 0 && _mintAmount <= maxMintAmount && supply + _mintAmount <= maxSupply ? true : false;
  }
