...
contract Omnimals is ERC721, NonblockingReceiver {
        ...
        ERC721("omnimals", "OMLS")
    {
        ...
        maxChainSupply = _startingTokenID + 1000;
        baseURI = baseURI_;
    ...
    function mint(uint8 _amount) external payable {
        ...
        require(balanceOf(msg.sender) < 5, "You can only mint 4 per account greedy bastard");
    ...
    function traverseChains(uint16 _chainId, uint256 tokenId) external payable {
        ...
        require(
            destinationAddresses[_chainId].length > 0,
        ...
        endpoint.send{value: msg.value}(
            ...
            destinationAddresses[_chainId], // destination address of nft contract
     ...
     */
    function estimateFees(uint16 _chainId, uint256 _tokenId) external payable returns (uint256) {
        require(
            destinationAddresses[_chainId].length > 0,
            "This chain is currently unavailable for travel"
        );
...
abstract contract NonblockingReceiver is Ownable, ILayerZeroReceiver {
    ...
    mapping(uint16 => bytes) public destinationAddresses;
    ...
    function lzReceive(
        ...
        require(
            _srcAddress.length == destinationAddresses[_srcChainId].length &&
                keccak256(_srcAddress) ==
                keccak256(destinationAddresses[_srcChainId]),
    ...
    function _lzSend(
        ...
        endpoint.send{value: msg.value}(
            _dstChainId,
            destinationAddresses[_dstChainId],
            _payload,
            _refundAddress,
            _zroPaymentAddress,
            _txParam
    ...
    function setDestinationAddress(uint16 _chainId, bytes calldata _destinationAddress) external onlyOwner {
        destinationAddresses[_chainId] = _destinationAddress;
    }
}
