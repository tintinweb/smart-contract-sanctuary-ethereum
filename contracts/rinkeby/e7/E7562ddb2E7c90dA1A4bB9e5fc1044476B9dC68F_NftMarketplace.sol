...
error NftMarketplace__PriceMustBeOverZero();
error NftMarketplace__NotApproved();
error AlreadyListed(address nftAddress, uint256 tokenId);
error NotOwner();
error NotListed();
error PriceNotMeet(address nftAddress, uint256 tokenId, address owner);
...
contract NftMarketplace is ReentrancyGuard {
    ...
    event ItemCancled(address indexed nftAddress, address indexed owner, uint256 indexed tokenId);
    event ItemUpdated(
        address indexed nftAddress,
    ...
    4. `updateListing` : update a new Price
    5. `withdrawPrice` : withdraw the nft price
     */
    ...
    function listItem(
        ...
        if (price <= 0) {
            revert NftMarketplace__PriceMustBeOverZero();
        }
        // 1. send transfer
        // 2. give the marketplace approval to sell the nft for them
        IERC721 nft = IERC721(nftAddress);
        if (nft.getApproved(tokenId) != address(this)) {
            revert NftMarketplace__NotApproved();
        }
        s_listing[msg.sender][tokenId] = Listing(price, msg.sender);
        ...
        nonReentrant
    {
        ...
        if (msg.value < listing.price) {
            revert PriceNotMeet(nftAddress, tokenId, msg.sender);
        }
        // check how much money in which accounts
    ...
    {
        delete (s_listing[nftAddress][tokenId]);
        emit ItemCancled(nftAddress, msg.sender, tokenId);
    }
    ...
    function updateListing(
        ...
        emit ItemUpdated(nftAddress, msg.sender, tokenId, newPrice);
    }
