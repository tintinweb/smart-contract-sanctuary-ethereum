...
contract MetroVault is Ownable, IERC721Receiver {
    ...
    /** 
    //REMOVE BELOW
    function iterateStake() external onlyOwner {
        for (uint256 i = 0; i < survivor.totalSupply(); i++) {
            if (vault[i].tokenId == 0 && (msg.sender == survivor.ownerOf(i))) {
                survivor.transferFrom(msg.sender, address(this), i);
                vault[i] = Stake({
                    owner: msg.sender,
                    tokenId: uint24(i),
                    mintingWallet: msg.sender
                });
            }
        }
    }   //REMOVE ABOVE
