...
library LiquidateLogic {
    ...
    //TODO
    // for test use 60 seceond, we will use 1 day in mainnet
    uint constant internal LIQUIDATE_INTERVAL = 1200;
    //TODO
    ...
    function executeAuction(
        ...
        if (loanData.state == DataTypes.LoanState.Active) {
            ...
            vars.auctionEndTimestamp = loanData.bidStartTimestamp + (nftData.configuration.getAuctionDuration() * LIQUIDATE_INTERVAL);
    ...
    function executeRedeem(
        ...
        vars.redeemEndTimestamp = (loanData.bidStartTimestamp + nftData.configuration.getRedeemDuration() * LIQUIDATE_INTERVAL);
        ...
        if (loanData.bidderAddress != address(0)) {
            ...
            if (loanData.reserveAsset == address(WETH)) {
                ...
                payable(loanData.bidderAddress).transfer(loanData.bidPrice + vars.bidFine);
            } else {
                IERC20Upgradeable(loanData.reserveAsset).safeTransfer(loanData.bidderAddress, loanData.bidPrice + vars.bidFine);
            }
        }
    ...
    function executeLiquidate(
        ...
        if (vars.remainAmount > 0) {
            if(loanData.reserveAsset == address(WETH)){
                WETH.withdraw(vars.remainAmount);
                payable(loanData.borrower).transfer(vars.remainAmount);
            } else {
...
interface IBToken is IScaledBalanceToken, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    //todo
    event TransferAdjust(
        uint256 fromFTokenBal,
...
library ReserveLogic {
    ...
    function getNormalizedIncome(DataTypes.ReserveData storage reserve) internal view returns (uint256) {
        if (reserve.lastUpdateTimestamp == block.timestamp) {
            ...
            uint128 newBorrowIndex = getNewBorrowIndex(reserve, reserve.variableBorrowIndex);
    ...
    function updateState(DataTypes.ReserveData storage reserve) internal {
        ...
        _mintToTreasury(
            reserve,
            IDebtToken(reserve.debtTokenAddress).scaledTotalSupply(),
            previousVariableBorrowIndex,
    ...
    function _updateIndexes(
        ...
        reserve.variableBorrowIndex = getNewBorrowIndex(reserve, variableBorrowIndex);
        //solium-disable-next-line
    ...
    function getNewBorrowIndex(DataTypes.ReserveData storage reserve,
        uint256 variableBorrowIndex) internal view returns (uint128) {
        if (reserve.lastUpdateTimestamp == block.timestamp) {
            return reserve.variableBorrowIndex;
        }
