...
abstract contract AbstractRewards is IAbstractRewards {
    ...
    function _prepareCollect(address _account) internal returns (uint256) {
        require(
            _account != address(0),
            "AbstractRewards._prepareCollect: account cannot be zero address"
        );
    ...
    function _correctPointsForTransfer(
        ...
        require(
            _from != address(0),
            "AbstractRewards._correctPointsForTransfer: address cannot be zero address"
        );
        require(
            _to != address(0),
            "AbstractRewards._correctPointsForTransfer: address cannot be zero address"
        ...
        require(
            _shares != 0,
            "AbstractRewards._correctPointsForTransfer: shares cannot be zero"
        );
    ...
    function _correctPoints(address _account, int256 _shares) internal {
        require(
            _account != address(0),
            "AbstractRewards._correctPoints: account cannot be zero address"
        );
        require(
            _shares != 0,
            "AbstractRewards._correctPoints: shares cannot be zero"
        );
...
contract TimeLockPool is BasePool, ITimeLockPool {
        ...
        )
    {
        require(
            ...
            "TimeLockPool.constructor: max lock duration must be less than or equal to maximum lock duration"
        );
        require(
    ...
    function deposit(
        ...
        require(
            _receiver != address(0),
            "TimeLockPool.deposit: receiver cannot be zero address"
        );
    ...
    function withdraw(uint256 _depositId, address _receiver)
        external
        nonReentrant
    {
        require(
            _receiver != address(0),
            "TimeLockPool.withdraw: receiver cannot be zero address"
        );
        require(
