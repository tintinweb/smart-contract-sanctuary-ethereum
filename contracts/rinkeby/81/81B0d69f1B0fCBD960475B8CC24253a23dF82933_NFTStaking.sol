...
contract NFTStaking is Ownable, ReentrancyGuard {
  ...
  constructor(IERC20 _rewardsToken) {
    ...
    addVault(address(0xe6096DeeDCAe8e15D3113a13Db8ae079ed9465ab), "Singularity 0 Universe ANTHRO");
    addVault(address(0x960962ED55CBb4b8fCA35324938f6D956b417049), "Singularity 0 Universe HUMAN");
    addVault(address(0xd82c2f4dA460eA5708Fb8b02f223c7DcFE219005), "Singularity 0 Universe ANKOKU");
  }
  ...
  function stake(StakeItem[] calldata _stakeItems) external nonReentrant {
    ...
    for (uint256 k; k < length; ++k) {
      ...
      for (uint256 i; i < len; ++i) {
        require(
          IERC721(vaultInfo[_pid].nft).ownerOf(_tokenIds[i]) == _msgSender(),
          "Can't stake tokens you don't own!"
        );
        require(
          IERC721(vaultInfo[_pid].nft).isApprovedForAll(_msgSender(), address(this)),
          "Can't stake tokens without approved"
        );
        IERC721(vaultInfo[_pid].nft).transferFrom(_msgSender(), address(this), _tokenIds[i]);
  ...
  function withdraw(StakeItem[] calldata _stakeItems) external nonReentrant {
    ...
    for (uint256 k; k < length; ++k) {
      ...
      for (uint256 i; i < len; ++i) {
        ...
        IERC721(vaultInfo[_pid].nft).transferFrom(address(this), _msgSender(), _tokenIds[i]);
      }
      stakers[_msgSender()].amountStaked -= len;
      stakers[_msgSender()].timeOfLastUpdate = block.timestamp;
      if (stakers[_msgSender()].amountStaked == 0) {
