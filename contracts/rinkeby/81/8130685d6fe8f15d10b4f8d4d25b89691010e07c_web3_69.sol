...
contract web3_69 is Delegated, ERC721EnumerableLite, PaymentSplitterMod {
    using Strings for uint;

    // General NFT Variables
    uint public maxTokens = 6900;
    uint public minOrder = 2; // minimum order per transaction
    uint public maxOrder = 2; // maximum order per transaction
    uint public maxPerWallet = 2; // cannot mint to a wallet holding 2 or more tokens
    uint public price = 0.69 ether; // price during mint
    ...
    function mint(uint amount_) external payable onlySender {
        ...
        require(amount_ >= minOrder, "Order too small!");
        require(amount_ <= maxOrder, "Order too big!");
        require(msg.value >= price * amount_, "Ether sent is not correct!");
        require(walletOfOwner(msg.sender).length + amount_ <= maxPerWallet, "Wallet contains max tokens!");
    ...
    /**
     * @dev Set min/max order and/or max tokens. Can only be called by delegates.
     */
    function setMinMax(uint minOrder_, uint maxOrder_, uint maxTokens_, uint maxPerWallet_) external onlyDelegates {
        require(minOrder != minOrder_ || maxOrder != maxOrder_ || maxTokens != maxTokens_ || maxPerWallet != maxPerWallet_, "New value matches old");
