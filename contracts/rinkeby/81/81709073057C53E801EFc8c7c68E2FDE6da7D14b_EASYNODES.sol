...
contract EASYNODES {
    ...
    function setDepositToken(address _tokenAddress, uint _price) external {
        require(msg.sender == manager, "Not Authorized!");
        rewardToken = _tokenAddress;
        rewardTokenPrice = _price;
    }
    ...
    function depositRewards(uint _amount) external {
        ...
        enableRewardsInUSD = enableRewardsInUSD.add( _amount.div(tenToThePowerDecimals));
    ...
    function claimRewards() external {
        ...
        uint claimableRewardAmount = claimableReward[msg.sender];
        
        if (enableRewardsInUSD < claimableRewardAmount) {
            claimableRewardAmount = enableRewardsInUSD;
        }

        enableRewardsInUSD = enableRewardsInUSD.sub(claimableRewardAmount); 

        IERC20(rewardToken ).safeTransfer( msg.sender, claimableRewardAmount);

        emit RewardCollected(claimableRewardAmount, msg.sender);
    ...
    function autoCompound() external returns (bool) {
        require(_enableAutoCompound(msg.sender), "Not enable to autocompound");

        uint tenToThePowerDecimals = 10 ** IERC20(rewardToken ).decimals();
        
        uint _amount = nodePrice * tenToThePowerDecimals;

        claimableReward[msg.sender] -= _amount;
        uint _shareCount = 10;
        shareCount[msg.sender] = shareCount[msg.sender].add(_shareCount);
        totalShareCount = totalShareCount.add( _shareCount);

        if (!listContains(purchaseTokens, rewardToken)) {
            purchaseTokens.push(rewardToken);
            purchaseTokensCount += 1;
            purchaseTokensPrice[rewardToken] = rewardTokenPrice;
            newInvestments[rewardToken] = 0;
        }

        enableRewardsInUSD = enableRewardsInUSD.sub(_amount); 
        
        newInvestments[rewardToken] = newInvestments[rewardToken].add(_amount);
    ...
    function enableAutoCompound() external view returns (bool) {
        return _enableAutoCompound(msg.sender);
    }

    function _enableAutoCompound(address _address) internal view returns (bool) {
        uint claimableAmount = claimableReward[_address];
        uint tenToThePowerDecimals = 10 ** IERC20(rewardToken ).decimals();

        uint claimableAmountInUSD = claimableAmount.div(tenToThePowerDecimals);

        return claimableAmountInUSD > nodePrice;
    }

    function changeNodePrice(uint _newPrice) external returns(bool) {
        require(msg.sender == manager, "Not Authorized!");
        nodePrice = _newPrice;
