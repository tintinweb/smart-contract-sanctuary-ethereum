...
contract CrowdNFT is ERC1155, Ownable, ERC1155Burnable, ERC1155Pausable, ERC1155Supply, ERC1155URIStorage, Lockable {
    struct TokenInfo {
        uint256 totalMints;
        uint256 totalSells;
        uint256 totalBuyers;
    ...
    function mint(uint256 projectId, uint256 id, uint256 amount, string memory tokenURI, bytes memory data) public onlyOwner
    {
        ...
        _tokenInfos[id] = tokenInfo;
//        super._totalSupply[id] = amount;
    }
    ...
    function buy(uint256 id, uint256 amount) public payable
    {
        ...
        TokenInfo memory tokenInfo = _tokenInfos[id];

        tokenInfo.totalSells = tokenInfo.totalSells + amount;
        tokenInfo.totalBuyers = tokenInfo.totalBuyers + 1;
    }

    function getTokenInfo(uint256 id) public view returns (TokenInfo memory)
    {
        return _tokenInfos[id];
    }

    function mintBatch(uint256 projectId, uint256[] memory ids, uint256[] memory amounts, string[] memory tokenURIs, bytes memory data) public onlyOwner
    {
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            _tokenInfos[ids[i]] = tokenInfo;

//            super._totalSupply[ids[i]] = amounts[i];
        }
    }
