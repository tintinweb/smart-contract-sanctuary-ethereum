...
contract Punksabcde is ERC721Optimized, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721Optimized("Punksabcde", "PK") {
        setHiddenMetadataUri("https://gateway.pinata.cloud/ipfs/QmaweVnp39Drqpfwd59nR6LpDVgf4J8QkUK4kViBACG6zY");
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        if (revealed == false) {
            ...
            uint256 RTime = uint256(RevealTime);
            require(RTime != 0 && block.timestamp >= RTime,"haven't reached the reveal time");
    ...
    function WhiteListMint(uint256 numberOfTokens) external payable {
        ...
        require(!wlMintPaused, "WhiteList mint is paused!");
        require(numberOfTokens <= iswhitelist[msg.sender], "Exceeded max available to purchase");
    ...
    function FreeListMint(uint256 numberOfTokens) external payable {
        uint256 totalS = totalSupply();
        require(!freeMintPaused, "FreeList mint is paused!");
        require(numberOfTokens <= isfreelist[msg.sender], "Exceeded max available to purchase");
        require(totalS + numberOfTokens <= maxsupply, "Purchase would exceed max tokens");
    ...
    function togglewlMintPaused() external onlyOwner {
        wlMintPaused = !wlMintPaused;
    }
    ...
    function togglefreeMintPaused() external onlyOwner {
    ...
    function setRevealTime(uint32 timestamp) external onlyOwner {
    ...
    function _mintSingle(address mintAddress) private {
        uint256 totalS = totalSupply();
        if (totalS < maxsupply) {
            _safeMint(mintAddress, totalS);
            emit ERC721Minted(mintAddress, totalS);
        }
    }
