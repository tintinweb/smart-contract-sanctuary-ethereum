...
contract BinoGlyph is ERC721,Ownable,ContextMixin {
    ...
    function presaleMint(uint256 amount,bytes32[] calldata proof) payable public {
        ...
        require (PRE_SALE_MINT_BALANCE[_msgSender()] + amount <= MAX_MINT_PER_ADDRESS_PRESLAE ,"Maximum token per address reached on presale" );
        require (amount <= MAX_MINT_PER_TRANSACTION , "Cannot mint this much token on one transaction.");
        require (PRICE.mul(amount) >= msg.value, "Invalid Price.");
    ...
    function giftMint(uint256 amount, address _to_address) public {
        ...
        require (amount <= MAX_MINT_PER_TRANSACTION , "Cannot mint this much token on one transaction.");
        require (MINT_BALANCE[address(_to_address)]  + amount <= MAX_MINT_PER_ADDRESS ,"Maximum token per address reached!");
