...
contract Assets is  Pausable {
  ...
  struct Asset{
    ...
    function addAsset( address _token, address _oracle, uint256 _price) external onlyOwner {
        require ( whitelist[_token] == false, "Formation.Fi: Token is already exists");
        ...
        Asset memory _asset = Asset(_token, _oracle, _price);
       ...
       function removeAsset( address _token) external onlyManager {
        require ( whitelist[_token] == true, "Formation.Fi: Token doesn't exist");
        whitelist[_token] = false;
    }

    function updateOracle( address _token, address _oracle) external onlyManager {
        require ( whitelist[_token] == true, "Formation.Fi: Token doesn't exist");
        uint256 _index = indexAsset[_token];
        assets[_index].oracle = _oracle;
    }

    function updatePrice( address _token, uint256 _price) external onlyManager {
        require ( whitelist[_token] == true, "Formation.Fi: Token doesn't exist");
        require ( _price != 0, "Formation.Fi: price is zero");
        uint256 _index = indexAsset[_token];
         require (assets[_index].oracle == address(0));
    ...
    function getIndex( address _token) public view  returns (uint256) {
...
/**
 * @title Admin
 * @dev Implementation of Admin
 */

contract Admin is Pausable {
    ...
    uint256 public DEPOSIT_FEE_RATE = 50; // 
      uint256 public DEPOSIT_FEE_RATE_PARITY = 15; // 
    ...
    function set_stableToken(address _stableTokenAddress) external 
      onlyOwner{
          require(
            _stableTokenAddress != address(0),
            "Formation.Fi: _stable Token address is the zero address"
        );  
         stableToken = IERC20(_stableTokenAddress);
      }
   ...
   function mint(address _account, uint256 _amount) external onlyProxy {
    ...
    function updateDepositData( address _account,  uint256 _amount) internal {
        ...
        for (uint256 i = 0; i < _amountDeposit.length; i++) {
            ...
            amountDepositPerAddress[_account][k] = _newAmount;
            if (_newAmount==0){
               deleteDepositData(_account, k);
            }
            else {
             k = k+1;
            }
