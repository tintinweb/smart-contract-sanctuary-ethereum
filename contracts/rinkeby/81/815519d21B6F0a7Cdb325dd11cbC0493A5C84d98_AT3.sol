...
contract AT3 is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("AT3", "TA3", maxBatchSize_, collectionSize_) {
  ...
  // For marketing etc.
  function myMint(uint256 quantity) external onlyOwner {
    require(
  ...
  function partnerMint(uint256 quantity, address to) external onlyOwner {
    require(
      totalSupply() + quantity <= amountForAuctionAndDev,
      "too many already minted before partner mint"
    );
    require(
      quantity % maxBatchSize == 0,
      "can only mint a multiple of the maxBatchSize"
