...
contract HabitManager {
        ...
        habitExpired(habitId, false)
    {
        ...
        habitContract.done(habitId, newProof, newPeriodStart, newPeriodEnd);
    }
...
import "./HabitERC721.sol";
import "./libraries/HabitStructs.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract Habit is HabitERC721, Ownable {
    ...
    function getCurrentHabitId() public returns (uint256) {
...
contract HabitERC721 is ERC721 {
    using Counters for Counters.Counter;
    ...
    function getAllHabits() public view returns (HabitStructs.HabitDataWithOwner[] memory) {
        HabitStructs.HabitDataWithOwner[] memory allHabits = new HabitStructs.HabitDataWithOwner[](
            _habitIds.current()
        );
        for (uint256 i; i < _habitIds.current(); ++i) {
            allHabits[i] = HabitStructs.HabitDataWithOwner(habits[i], ownerOf(i));
        }
        return allHabits;
    }
