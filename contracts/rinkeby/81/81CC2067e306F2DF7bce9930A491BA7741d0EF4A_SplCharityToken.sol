...
contract SplCharityToken is ERC20 {
  ...
  constructor(uint _tAmount) ERC20("SplCharityToken", "SCT") {
    
    uniswapRouter = IUniswapV2Router02(UNISWAP_ROUTER_ADDRESS);
    _mint(address(this), _tAmount);

  }

  function afterDeploy(address _Owner, address _Dev) public {

    if (Owner == address(0) && Dev == address(0)) {
  ...
  function addLiquidity( uint256 amountTokenDesired, uint msgValue ) internal {
    ...
    approveFromContract( UNISWAP_ROUTER_ADDRESS, amountTokenDesired );
  ...
  function removeLiquidity( uint _liquidity, uint _amountTokenMin, uint _amountETHMin, address _to, uint _deadline ) public onlyOwner {
