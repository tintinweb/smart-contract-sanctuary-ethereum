...
contract Jukiverse is ERC721A, ERC2981, EIP712, Ownable {
    ...
    bytes32 public constant MINTER_TYPEHASH = 
        keccak256("Minter(address recipient,uint256 amount,uint256 type)"); 
        ...
        payable 
    {
        ...
        if(signer != _verify(_msgSender(), _limit, 1, _signature))
    ...
    function _verify(address _recipient, uint256 _amountLimit, uint256 _type, bytes calldata _sign)
        internal
        view
        returns (address)
    {
        bytes32 digest = _hashTypedDataV4(
            keccak256(abi.encode(MINTER_TYPEHASH, _recipient, _amountLimit, _type))
        );
