...
contract NFT721A is ERC721A, Ownable {
    ...
    function mint(
        ...
        require(hashMint(quantity, nonce) == hash, "Invalid hash!");
    ...
    function hashMint(uint256 quantity, string memory nonce)
    private
    view
    returns (bytes32)
    {
        bytes32 hash = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encodePacked(quantity, msg.sender, nonce, "bzuki_mint")
                )
            )
        );
        return hash;
