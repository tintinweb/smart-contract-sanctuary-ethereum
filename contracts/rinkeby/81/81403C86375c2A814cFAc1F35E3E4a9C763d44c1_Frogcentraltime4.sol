...
contract Frogcentraltime4 is ERC721A, Ownable, ReentrancyGuard {
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    if(bytes(currentPage.message).length == 0) {
      return bytes(phase0MetadataUri).length > 0
        ? string(abi.encodePacked(phase0MetadataUri, _tokenId.toString(), uriSuffix))
        : '';
    }

    if(bytes(currentPage.message).length == 1) {
      return bytes(phase1MetadataUri).length > 0
        ? string(abi.encodePacked(phase1MetadataUri, _tokenId.toString(), uriSuffix))
        : '';
    }

    if(bytes(currentPage.message).length == 2) {
      return bytes(phase2MetadataUri).length > 0
        ? string(abi.encodePacked(phase2MetadataUri, _tokenId.toString(), uriSuffix))
        : '';
    }
