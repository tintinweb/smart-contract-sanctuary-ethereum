...
interface IERC20 {
    ...
    /**
    * @dev Unlock the specified amount of balance.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
    ...
    /**
    * @dev Destroy the specified amount of balance.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
     * Emits a `burn` event.
...
contract ERC20 is IERC20 {
    ...
    uint256 private _destroyedAmount;


    //@dev Amount destroyed
    uint256 private _lockNum=1;


    /**
     * @dev See `IERC20._lockaddress`.
     */
    function setLockAddress(address _address) public returns (bool){
         require(_address!=address(0), "ERC20: empty account!");
         require(!isLockAddress(_address), "ERC20: address locked");
        _lockaddress[_address]=true;
        return true;
     ...
     * @dev See `IERC20._lockaddress`.
     */
    function setUnlockAddress(address _address) public returns(bool){
        require(_address!=address(0), "ERC20: empty account!");
        require(isLockAddress(_address), "ERC20: address unlocked");
        delete _lockaddress[_address];
        return true;
    }
    ...
    * @dev See `IERC20._destroyedAmount`.
    ...
    /**
    * @dev See `IERC20._lockNum`.
     */
    function lockNum() public view returns (uint) {
        return _lockNum;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        require(!isLockAddress(recipient), "ERC20: address locked");
     ...
     */
    function _mint(address account, uint256 amount, uint256 totalSupplyAmount) internal {
    ...
    * @dev Destroys `amount` tokens from `account`, reducing the
    ...
    function _burn(address account, uint256 value) internal {
        ...
        _initialSupply = _initialSupply.sub(value);
...
contract Reallink is ERC20 {
     ...
     */
    constructor(string memory name, string memory symbol, uint8 decimals, uint256 initialSupply, uint256 totalSupply, address payable feeReceiver, address tokenOwnerAddress) public payable {
        _name = name;
        _symbol = symbol;
        _decimals = decimals;
        // set tokenOwnerAddress as owner of all tokens
        _mint(tokenOwnerAddress, initialSupply , totalSupply);
