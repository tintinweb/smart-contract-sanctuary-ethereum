...
contract BountyHunters is ERC721A, Ownable{
    ...
    function whitelistMint(bytes32[] memory _merkleProof, uint256 _quantity) external payable callerIsUser{
        require(whiteListSale, "Bounty Hunters :: Minting is on Pause");
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "Bounty Hunters :: Cannot mint beyond max supply");
        require((totalWhitelistMint[msg.sender] + _quantity)  <= MAX_WHITELIST_MINT, "Bounty Hunters :: Cannot mint beyond whitelist max mint!");
        require(msg.value >= (WHITELIST_SALE_PRICE * _quantity), "Bounty Hunters :: Payment is below the price");
        //create leaf node
        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, sender), "Bounty Hunters :: You are not whitelisted");
