    ...
    OwnableUpgradeable 
{
    ...
    b. 
        If greatness > 45, ~2.18% chance then the output would have an unique part.
        If greatness > 40, ~10.87% chance then the output would have prefix, and suffix.
        If greatness > 35, ~21.74% chance then the output would have suffix only.
        If greatness > 30, ~32.61% chance then the output would have prefix only.
    ...
    */
    function ownerMintAnomura(address _address) external onlyOwner 
    {
        ...
        string memory generatedHeadPieces = hasHeadpiece == true? pluck(newTokenId, "HEADPIECE", data.headPieces, data.prefixes, data.suffixes, data.unique): "";
    ...
    */
    function mintAnomura(address _address) external onlyBowlContract(_address) returns (uint256 anomuraId)
    {
        ...
        string memory generatedHeadPieces = hasHeadpiece == true? pluck(anomuraId, "HEADPIECE", data.headPieces, data.prefixes, data.suffixes, data.unique): "";
