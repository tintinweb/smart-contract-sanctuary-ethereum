    ...
    UUPSUpgradeable
{
    ...
    DataTypes.Lottery public s_lottery;

    DataTypes.LotteryState public s_lotteryState;
    ...
    mapping(uint256 => mapping(address => bool)) s_hasEntered;
    ...
    function enteryLottery() public payable nonReentrant whenNotPaused {
        DataTypes.Lottery memory _lottery = s_lottery;

        if (_currentTime() < _lottery.endTime) {
            if (s_usdc.balanceOf(_msgSender()) < s_entryFee)
                revert Errors.SaturnaLotteryHub__NotEnoughUSDC();
            if (s_hasEntered[s_lotteryId.current()][_msgSender()])
                revert Errors.SaturnaLotteryHub__AlreadyEnteredThisLottery();

            if (!(_currentTime() <= _lottery.endTime - s_entryBuffer))
                revert Errors
                    .SaturnaLotteryHub__CannotEnterLotteryEnteryBufferExpired();

            s_usdc.transferFrom(_msgSender(), address(this), s_entryFee);

            s_participants[s_lotteryId.current()][
                _lottery.entries
            ] = _msgSender();
            _lottery.entries++;
            s_hasEntered[s_lotteryId.current()][_msgSender()] = true;

            emit Events.LotteryEntered(_msgSender(), _currentTime());
        } else {
            revert Errors.SaturnaLotteryHub__LotteryExipered();
        }
    }

    function settleCurrentAndCreateNewLottery()
        public
        nonReentrant
        whenNotPaused
    {
    ...
    function _createLottery() internal {
        ...
        emit Events.LotteryCreated(s_lotteryId.current());
    }
    ...
    function _settleLotteryAndDeclareWinner() internal {
        ...
        if (_lottery.startTime <= _lottery.endTime)
            revert Errors.SaturnaLotteryHub__LotteryHasNotEnded();

        uint256 randomNumber = s_rng.randomNumber(s_rngRequest.id);
        uint64 time = _currentTime();

        uint256 moduloRandomNumber = _lottery.entries % randomNumber;

        address winner = s_participants[s_lotteryId.current()][
            moduloRandomNumber
        ];

        _lottery.winner = payable(winner);

        emit Events.LotterySetteled(s_lotteryId.current());
        emit Events.WinnerDeclared(winner, time);
    }
    ...
    function _payWinnerAndHouse() internal {
        ...
        address winner = _lottery.winner;

        if (winner == address(0)) {
            revert Errors.SaturnaLotteryHub__AddressIsZero();
        }

        uint256 balance = s_usdc.balanceOf(address(this));
        uint256 commision = (balance * s_houseFeePercentage) / 100;

        if (balance == 0) {
            revert Errors.SaturnaLotteryHub__InsufficentUSDCInContract();
        }

        s_usdc.transferFrom(address(this), owner(), commision);
        s_usdc.transferFrom(
            address(this),
            winner,
            s_usdc.balanceOf(address(this))
        );

        emit Events.WinnerPaid(winner, balance - commision);
        emit Events.CommissionIssued(owner(), commision);
    }

    function _setRngService(RNGInterface _rng) internal {
        s_rng = _rng;
        emit Events.RNGUpdated(_rng);
    }
    ...
    function changeEntryFee(uint256 _entryFee) public whenNotPaused onlyOwner {
        s_entryFee = _entryFee;
        emit Events.EntryFeeChanged(_entryFee);
    }

    function changeDuration(uint256 _duration) public whenNotPaused onlyOwner {
        s_duration = _duration;
        emit Events.DurationChanged(_duration);
    }
        ...
        onlyOwner
    {
        if (s_isHouseFeesPercentageLocked)
            revert Errors.SaturnaLotteryHub__HouseFeePercentageLocked();
        s_houseFeePercentage = _houseFeePercentage;
        emit Events.HouseFeePercentageChanged(_houseFeePercentage);
    }
    ...
    function lockHouseFeePercentage() public whenNotPaused onlyOwner {
        ...
        emit Events.HouseFeePercentageLocked();
    }
...
/// @title Errors for Saturna contract
