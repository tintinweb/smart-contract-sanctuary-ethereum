...
contract EncryptedGods is Ownable, ERC721 {
  ...
  uint256 public totalWhitelistedWalletCount;
  uint256 public maxWhitelistedWalletCount = 1234;
  ...
  mapping(address => uint256) private _whitelistedAmount;

  //team addresses [WILL BE ADDED]
  address private deployer;
   

  constructor() ERC721("Encrypted Gods", "EG") { 
    deployer = msg.sender;
    reserve(111);//Reserve for team
  }
  
  modifier whitelisted {
    require(_whitelistedAmount[msg.sender] > 0, "Not on presale whitelist");
    _;
  }
  ...
  function addToWhitelist(address[] memory wallets) public onlyOwner {
    require(wallets.length + totalWhitelistedWalletCount <= maxWhitelistedWalletCount, "This would exceed max whitelisted wallet count");

    for(uint i = 0; i < wallets.length; i++) {
      _whitelistedAmount[wallets[i]] += 3;
    }
  }

  function checkWhitelistedAmount(address wallet) external view returns (uint256) {
    return _whitelistedAmount[wallet];
  }
  ...
  function mintPresale(uint numberOfTokens) external whitelisted payable {
    ...
    require(numberOfTokens <= _whitelistedAmount[msg.sender], "You don't have enough whitelisted tokens");
    require(msg.value >= mintPrice.mul(numberOfTokens), "Insufficient payment");


    mintInternal(msg.sender, numberOfTokens);
    _whitelistedAmount[msg.sender] -= numberOfTokens;
  }
  ...
  function tokenOfOwnerByIndex(address _wallet, uint256 index)public view returns(uint256) {
    require(index < balanceOf(_wallet), "This wallet doesn't have this many tokens");
    uint256[] memory tokensOfOwner = tokensOfWallet(_wallet);
    return tokensOfOwner[index];
  }
