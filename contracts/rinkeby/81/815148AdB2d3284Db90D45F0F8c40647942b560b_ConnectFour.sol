...
contract ConnectFour {
    ...
    function initializeGame() external payable returns (uint256) {
        // require function reverts if condition is not met, if statement with built in error handling
        require(
    ...
    function startGame(uint16 _gameId) external payable {
        ...
        require(
            ...
            "ConnectFour: can't start the game you initialized"
        );
        require(
        ...
        // now the game has started, and ConnectFour.playMove() can be called 
    ...
    function playMove(uint256 _gameId, uint256 _col) external {
        // requires
        require(games[_gameId].status == Status.Started, "ConnectFour: games does not exist or has not started");
        Game memory game = games[_gameId];
        require(game.isPlayer1Turn ? msg.sender == game.player1 : msg.sender == game.player2, "ConnectFour: incorrect player");
        ...
        for (uint256 row = 0; row <= 5; row++) {
            ...
            if (rowDisc == Disc.Empty) {
                games[_gameId].board[idx] = games[_gameId].isPlayer1Turn ? Disc.Player1 : Disc.Player2;
                games[_gameId].isPlayer1Turn = !games[_gameId].isPlayer1Turn;
                break;
            } else if ( row == 5) {
                // at this point we've iterated through all of the rows of the column and they've all been
                // non-empty, so this means the column is full, and this move is illegal
    ...
    address payable _recipient,
     uint256 _startingWinDiscCol,
      ...
      WinningDirection _direction
        ...
        Disc disc = games[_gameId].player1 == msg.sender ? Disc.Player1 : Disc.Player2;

        // verify that there is indeed a winning four in a four starting at these coordinates
        // and in the given direction
        uint256 currCol = _startingWinDiscCol;
        uint currRow = _startingWinDiscRow;

        for (uint256 i = 0; i < 4; i++) {
            // is the current index the correct disc?
            uint256 idx = boardIndex(currCol, currRow);
            require(games[_gameId].board[idx] == disc, "ConnectFour: four-in-a-row must only contain caller's discs");

            // if so, update the current index to the next disc in the line
    ...
    function boardIndex(uint256 _col, uint256 _row) public pure returns (uint256) {
        // requires 
        require(_col <= 6 && _row <= 5, "ConnectFour: row or column is out bounds");
