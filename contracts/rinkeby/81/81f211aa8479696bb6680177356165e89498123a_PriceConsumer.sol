    ...
    SuperAdminControl
{
    ...
    function getAltCoinPriceinStable(
        ...
        if (
            ...
            uint256 priceOfOneAltcoinTokeninStable = (uint256(collateralChainlinkUsd) * 10 ** stableDecimals) / uint256(priceFromChainLinkinStable);
            uint256 totalCollateralinStable = (_collateralAmount * priceOfOneAltcoinTokeninStable) / 10 ** collateralDecimals;
            collateralAmountinStable =
                collateralAmountinStable + totalCollateralinStable;
    ...
    function getStablePriceInAltcoin(
        ...
        uint256 stableAmountInCollateral;

        if (
            this.isChainlinFeedEnabled(_altCoin) &&
            this.isChainlinFeedEnabled(_stableCoin)
        ) {
            int256 collateralChainlinkUsd = this.getNetworkPriceFromChainlinkinUSD();
            (int256 priceFromChainLinkinStable, ) = this.getLatestUsdPriceFromChainlink(_stableCoin);
            ...
            uint256 priceofOneStableinAltcoin = (uint256(priceFromChainLinkinStable) * 10 ** collateralDecimals) / uint256(collateralChainlinkUsd);
            uint256 totalStableinCollateral = (_stableCoinAmount * priceofOneStableinAltcoin) / 10 ** stableDecimals;
            stableAmountInCollateral =
                stableAmountInCollateral + totalStableinCollateral;
            return stableAmountInCollateral;
        } else {
    ...
    function getStablePriceInETH(
        ...
        if (
            ...
            uint256 collateralDecimals = IERC20Metadata(this.WETHAddress()).decimals();
            uint256 priceofOneStableinAltcoin = (uint256(priceFromChainLinkinStable) * 10 ** collateralDecimals) / uint256(collateralChainlinkUsd);
            uint256 totalCollateralinStable = (_stableCoinAmount * priceofOneStableinAltcoin) / 10 ** stableDecimals;
            stableAmountInCollateral =
                stableAmountInCollateral + totalCollateralinStable;
            return stableAmountInCollateral;
        } else {

            stableAmountInCollateral = stableAmountInCollateral + (this.getETHPriceFromDex(this.WETHAddress(), _stableCoin, _stableCoinAmount));
            return stableAmountInCollateral;
        }
    }
