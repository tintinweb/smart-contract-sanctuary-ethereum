...
contract Factory is AccessControl {
    ...
    function createCollection(
        ...
        collecData.tokenFeed = _tokenFeed;

        Collection collection = new Collection(collecData);
    ...
    function addExternalAddresses(address _treasury)
        external
...
contract Collection is ERC1155, AccessControl {
    ...
    event SetAddresses(address token);
    ...
    IERC20 public token;
    AggregatorV3Interface public tokenFeed;
    ...
    constructor(CollectionData memory collecData) ERC1155(collecData.uri) {
        ...
        treasury = collecData.treasury;

        token = IERC20(collecData.token);
        tokenFeed = AggregatorV3Interface(collecData.tokenFeed);
    ...
    function setPriceRanges(PriceRange[] calldata _priceRange)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        delete priceRange;
        for (uint32 i = 0; i < _priceRange.length; i++) {
            priceRange.push(_priceRange[i]);
        }
    }

    function getPrice(uint256 tokenId) public view returns (uint256) {
        for (uint32 i = 0; i < priceRange.length; i++) {
            if (
                tokenId >= priceRange[i].startId &&
                tokenId < priceRange[i].endId
            ) {
                return priceRange[i].amount;
            }
        }
        require(false, "Token not for sale");
        return 0;
    }
    ...
    function getCardPrice(uint256 tokenId) public view returns (uint256) {
        uint256 tokenPrice = getTokenPrice();
        uint256 amount = getPrice(tokenId);
        uint256 result = (amount * (1e44)) / (tokenPrice * (1e18));

        return result;
    }

    function _withToken(address buyer, uint256 tokenId) private {
        uint256 price = getCardPrice(tokenId);
        require(
            token.balanceOf(buyer) >= price,
            "Insufficient funds: Cannot buy this NFT"
        );
        token.transferFrom(buyer, treasury, price);
