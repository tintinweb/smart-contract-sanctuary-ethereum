...
contract TwitterscanPass is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, AccessControlUpgradeable,IERC721R {
    ...
    //uint internal constant publicMax=5;
    ...
    function setTest(uint[] calldata para) external onlyRole(DEFAULT_ADMIN_ROLE){  //only test contract
            collectionSize = para[0];
            conf.wlSize = para[1];
            conf.refundStartId = para[2];
            conf.wlPrice = para[3];
            conf.publicPrice = para[4];            
    }


    function setConf(Conf calldata conf_) external onlyRole(DEFAULT_ADMIN_ROLE){  //only test contract
            conf = conf_;
    }
   ...
   function publicMint (uint vol) public whenNotPaused payable{
        ...
        require((claimed+vol) <= 5, "Address claim max <= 5");
        require(totalSupply() + vol <= collectionSize, "reached max supply");
    ...
    function canBeRefunded(address refunder,uint256 tokenId) public view returns (bool) {
        return
            ...
            isRefundGuaranteeActive() && enableRefund[refunder][tokenId];
    }
