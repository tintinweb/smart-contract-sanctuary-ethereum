...
contract ExcitedChimpsParadiseClub is ERC721A, Ownable {
    ...
    bool public revealed;                               // flag for revealing hidden NFTs
    string public hiddenMetadataUri;                    // for unrevealed NFTs
    mapping(address => uint256) public mintedWallets;   // Keeping track of No. of NFTs per wallet

    constructor() ERC721A("Excited Chimps Paradise Club", "ECPC") {
       mintPrice = 0.04 ether;
       maxSupply = 5555;
       maxPerWallet = 3;
       baseURI = "ipfs://CID/"; 
       hiddenMetadataUri = "ipfs://QmUuQvmoLh7brxLZu7VN5dhQ7jqRZSxa78ov5QaEUEGWAG/" ;
    }
    ...
    if (revealed == false) {
      return string(abi.encodePacked(hiddenMetadataUri,"hidden_metadata.json"));
    }
    ...
    function setHiddenMetadataUri(string memory _hiddenMetadataUri) public onlyOwner {
        bytes memory uri = bytes(_hiddenMetadataUri);
        if(uri.length > 0) {
