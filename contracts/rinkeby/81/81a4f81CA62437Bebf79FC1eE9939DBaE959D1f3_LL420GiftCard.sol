...
contract LL420GiftCard is ERC1155Burnable, ReentrancyGuard, Ownable, Pausable {
    using SafeMath for uint256;
    using ECDSA for bytes32;

    uint256 public constant MINT_LIMIT = 1;
    uint256 public constant ITEM_TYPES = 4;
    string public constant NAME = "LOOK LABS Gift Card";
    string public constant SYMBOL = "LGIFT";

    uint64[4] public supplyCaps = [200, 65, 22, 10];
    ...
    function mint(
        ...
        require(supplies[_id] < supplyCaps[_id], "Reached max supply");
        require(timestamps[_to] != _timestamp, "Already minted");

        supplies[_id] += MINT_LIMIT;
        timestamps[_to] = _timestamp;
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        require(_id < ITEM_TYPES, "URI requested for invalid token type");
    ...
    function _verify(
        ...
        bytes32 signedHash = keccak256(abi.encodePacked(_to, keccak256("GiftCard"), _id, MINT_LIMIT, _timestamp));
    ...
    function isMinted(
        ...
        if (!_verify(_to, _id, _timestamp, _signature)) return false;
        if (timestamps[_to] == _timestamp) return true;
     ...
     */
    function setSupply(uint64[] memory _caps) external onlyOwner {
        for (uint256 i; i < ITEM_TYPES; i++) {
