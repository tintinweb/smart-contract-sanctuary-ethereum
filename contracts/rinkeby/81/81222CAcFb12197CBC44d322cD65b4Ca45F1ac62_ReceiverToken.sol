...
contract ReceiverToken is ERC1155, Ownable {
  ...
  /* ========== EVENTS ========== */
  event ReceiverInitialMint(address indexed from, address to, uint tokenId, uint timestamp, string keyword);
  event ReceiverMint(address indexed to, uint tokenId, uint timestamp);
  ...
  function mintReceiver(uint256 tokenId) external {
    require(_exists[tokenId], "token does not exist, mint sender token first");

    _mint(msg.sender, tokenId, 1, new bytes(0));
    emit ReceiverMint(msg.sender, tokenId, block.timestamp);
    // ERC1155 also emits TransferSingle(operator, address(0), to, id, amount);
    tokenSupply[tokenId]++;
  }
  ...
  function initialMintReceiver(address to, uint256 tokenId, string memory _keyword) external onlySenderContract {
    ...
    emit ReceiverInitialMint(msg.sender, to, tokenId, block.timestamp, _keyword);
  }
