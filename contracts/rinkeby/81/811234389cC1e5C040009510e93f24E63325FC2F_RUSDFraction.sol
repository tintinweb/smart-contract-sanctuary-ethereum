...
contract RUSDFraction is ERC20Detailed, Ownable {
    ...
    function _updateRewardsPerToken() internal {
        ...
        if (totalSupply_ != 0) rewardsPerToken_.accumulated = (rewardsPerToken_.accumulated + 1e9 * unaccountedTime * rewardsPerToken_.rate / totalSupply_).u128(); // The rewards per token are scaled up for precision
    ...
    function _updateUserRewards(address user) internal returns (uint128) {
        ...
        userRewards_.accumulated = (userRewards_.accumulated + _fracBalances[user] * (rewardsPerToken_.accumulated - userRewards_.checkpoint) / 1e9).u128(); // We must scale down the rewards by the precision factor
