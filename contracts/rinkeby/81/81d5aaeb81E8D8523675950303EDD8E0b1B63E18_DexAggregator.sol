...
contract DexAggregator {
  ...
  function uniswapv3rate(
    ...
    address pool = IUniswapV3Factory(univ3factory).getPool(
      tokenIn,
      tokenOut,
      500
    );
    ...
    if (pool == address(0) || checkAmount(tokenIn, pool) < amountIn) {
      pool = IUniswapV3Factory(univ3factory).getPool(tokenIn, tokenOut, 3000);
      fees = 3000;
    }

    if (pool == address(0) || checkAmount(tokenIn, pool) < amountIn) {
      pool = IUniswapV3Factory(univ3factory).getPool(tokenIn, tokenOut, 10000);
      fees = 10000;
    }

    require(pool != address(0), "pool doesn't exist");

    if (checkAmount(tokenIn, pool) < amountIn) {
      return (0);
    }
  ...
  function uniswapv2rate(
    ...
    address pool = IUniswapV2Factory(univ2factory).getPair(tokenIn, tokenOut);
  ...
  function sushiswaprate(
    ...
    address pool = IUniswapV2Factory(sushifactory).getPair(tokenIn, tokenOut);
    if (checkAmount(tokenIn, pool) < amountIn) {
      return 0;
    }
  ...
  function getRates(
    ...
    require(flag == 1 || flag == 2, "Flag value incorrect");

    uint256[] memory prices = new uint256[](3);

    (prices[0]) = uniswapv3rate(
      tokenIn,
      tokenOut,
      uint128(amountIn),
      uint32(2)
    );
    (prices[1]) = uniswapv2rate(tokenIn, tokenOut, amountIn);
    (prices[2]) = sushiswaprate(tokenIn, tokenOut, amountIn);
  ...
  function swapuniv3(
    ...
    TransferHelper.safeApprove(tokenIn, univ3router, amountIn);
  ...
  function bestrateswap(
    ...
    uint8 flag;
    //getting platform flag from the getRates function

    (, flag) = getRates(tokenIn, tokenOut, amountIn, 2);

    if (flag == 1) {
      swapuniv3(tokenIn, tokenOut, amountIn, amountOutMin, to);

      balance1 = checkAmount(tokenOut, to);
      return balance1 - balance0;
    } else {
      swapuniv2andsushi(tokenIn, tokenOut, amountIn, amountOutMin, to, flag);
