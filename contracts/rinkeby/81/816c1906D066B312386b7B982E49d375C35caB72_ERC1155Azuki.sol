 ...
 */
contract ERC1155YuzuInternal is Context, AccessControl, ERC1155Supply {
...
contract ERC1155Marketplace is ERC1155YuzuInternal, ReentrancyGuard {
    ...
    {
        require(_admin != address(0), "Azuki: admin should not be zero address");
        require(_owner != address(0), "Azuki: owner should not be zero address");
        admin = _admin;
        contractOwner = _owner;
        require(_commissionRate<=100, "ERC1155Azuki: Commission rate has to be between 0 and 100");
    ...
    function sell(uint256 tokenId, uint256 qty, uint256 price, address wallet) public {
    ...
    function buy(uint256 tokenId, uint256 index, uint256 qty) payable public nonReentrant {

        // check for a valid index position
        require(index < openSales[tokenId].length, "ERC1155Azuki: wrong index");

        // Cannot buy more items than available in the offer
        require(openSales[tokenId][index].qty >= qty, "ERC1155Azuki: not enough items for sale");

        // Cannot buy more items than the amount the owner have
        require(balanceOf(openSales[tokenId][index].user, tokenId) >= qty, "ERC1155Azuki: the seller doesn't have enough items for sale");
    ...
    function cancelSale(uint256 tokenId, uint256 index) external {
        ...
        require(openSales[tokenId][index].user == _msgSender(), "ERC1155Azuki: only the original seller can cancel his sales");
    ...
    function cancelBid(uint256 tokenId, uint256 index) external nonReentrant {
        ...
        // check for a valid index position
        require(index < openOffers[tokenId].length, "ERC1155Azuki: wrong index");

        // Only the original bidder can cancel his bids
        require(openOffers[tokenId][index].user == _msgSender(), "ERC1155Azuki: only the original bidder can cancel his bids");
    ...
    function acceptBid(uint256 tokenId, uint256 index) external nonReentrant {
        ...
        // validate that the user has enough qty for the bid, accepts all or nothing
        require(balanceOf(_msgSender(), tokenId) >= openOffers[tokenId][index].qty, "Azuki: you do not have enough tokens to sell");
        ...
        // we need to call a transferFrom from this contract, which is the one with permission to sell the NFT
        callOptionalReturn(this, abi.encodeWithSelector(this.safeTransferFrom.selector, _msgSender(), openOffers[tokenId][index].user, tokenId, openOffers[tokenId][index].qty, "0x"));
         ...
         */ 
        distributeFunds(openOffers[tokenId][index].qty * openOffers[tokenId][index].price, openOffers[tokenId][index].user, _msgSender(), tokenId);
    ...
    function updateAdmin(address _admin, uint256 _commissionRate, uint256 _royaltiesCommissionRate, bool _anyoneCanMint) external {
        require(_admin != address(0), "Azuki: admin should not be zero address");
        require(_msgSender()==contractOwner, "Only contract owner can do this");
    ...
    // update owner
    function updateOwner(address _contractOwner) external {

        require(_contractOwner != address(0), "Azuki: owner should not be zero address");
        require(_msgSender()==contractOwner, "Only contract owner can do this");

        // grant roles to new owner
        _setupRole(DEFAULT_ADMIN_ROLE, _contractOwner);
        _setupRole(MINTER_ROLE, _contractOwner);

        // revoke roles of old owner
        revokeRole(MINTER_ROLE, contractOwner);
...
contract ERC1155Azuki is ERC1155Marketplace, EIP712 {
    ...
    mapping(uint256 => uint256) vouchersMintedQty;

    // minted NFT ID for each voucher ID
    mapping(uint256 => uint256) vouchersNFTIds;

    event LazyMint(uint256 indexed tokenId, address indexed from, address indexed to, uint256 value);

    /// @notice Redeems an NFTVoucher for an actual NFT, creating it in the process.
    function redeem(
        uint256 id, uint256 nftPostId, uint256 qty, uint256 price, uint256 collectionId, uint256 chainId, address author, uint256 salt, bytes memory signature, uint256 qtyToMint
    ) public payable nonReentrant {

        require(chainId == getChainId(), "ERC1155Azuki: Wrong chain");
        require(vouchersMintedQty[id] + qtyToMint <= qty, "ERC1155Azuki: the qty exceeds the amount available for minting");
        ...
        verify( id, nftPostId, qty, price, collectionId, chainId, author, salt, address(this), signature, contractOwner);

        // allow to mint less qty of the amounts in voucher, until all qtys are minted
        // first time minting? do autoMint then
        // the temporary owner of the new items is the contract itself
        if (vouchersNFTIds[id]==0) { 
            
            vouchersNFTIds[id] = autoMint(_msgSender(), qtyToMint);
        ...
        // already minted, increase qty and assign to user
        } else { 
            mint(_msgSender(), vouchersNFTIds[id], qtyToMint, '0x');
        }

        // distribute payments
        distributeFunds(qtyToMint * price, _msgSender(), contractOwner, vouchersNFTIds[id]);

        // Inform the Lazy Mint
        emit LazyMint(vouchersNFTIds[id], _msgSender(), contractOwner, qtyToMint * price);
    ...
    function verify(
        ...
        require(recoveredSigner == signer, "ERC1155Azuki: wrong signature");
    }
