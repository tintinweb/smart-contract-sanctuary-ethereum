...
interface IPoolFactory {
    ...
    struct User {
        ...
        function getPoolInfo(address _poolCreater) external view returns (address, address, address , uint256);
...
/////// FUNCTIONS FOR POOL INCENTIVE PROGRAM CREATORS ////////
    ...
    function getPoolInfo(address info) external view  returns (address, address, address , uint256) {
    ...
    function createIncentive(address _pool, uint256 _startTime, uint256 _endTime,address _refundee , uint256 _reward) external {
        ...
        (address token1, address token2 , address poolCreater, uint256 fee) = factory.getPoolInfo(_pool);
...
////// FOR NORMAL USERS //////
    ...
    function _stakeToken(uint256 _key, uint256 _liquidityId) private {
        ...
        (address token1, address token2 , address poolCreater, uint256 fee) = factory.getPoolInfo(key.pool);
        ...
        stake.liquidity = amount;

        emit TokenStaked(_liquidityId, incentiveId, amount);
