...
contract gNR is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    function initialize(IERC20 nr_) public initializer {
        ...
        // multipleToLockup[1] = 150;
        // multipleToLockup[2] = 200;
        // multipleToLockup[3] = 250;
        // multipleToLockup[4] = 300;
    }
    ...
    function massUnlockAll(address to) public {
        ...
        nr.safeTransfer(to, totalNRAmount);
    ...
    function instantUnlockWithPenalty(
        ...
        for (; i < endIdx; ++i) {
            ...
            if (unlockedAmount.add(nrAmount) <= amount) {
                ...
                nrAmount = nrAmount.sub(lockedList[i].nrAmount);
                afterLockupDay = _getDay() - lockedList[i].startTime;
                unlockedAmount = unlockedAmount.add(nrAmount);
                transferAmount = transferAmount.add(nrAmount * afterLockupDay / lockedList[i].endDay);
                burnedAmount = burnedAmount.add(nrAmount.sub(transferAmount));
                console.log(unlockedAmount,amount,nrAmount,lockedList[i].nrAmount);
                break;
            }
        }
        ...
        returns (uint256)
    {
        require(
            ...
            "gNR::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "gNR::_writeCheckpoint: block number exceeds 32 bits"
        );
