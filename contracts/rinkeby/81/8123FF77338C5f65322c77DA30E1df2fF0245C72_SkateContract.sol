 ...
 * @title SkateContract
 */
contract SkateContract is ERC721Enumerable, Ownable, ReentrancyGuardUpgradeable {
    ...
    event AuctionBid(uint256 indexed gnarId, address sender, uint256 value, uint256 timestamp);
    event AuctionSettled(uint256 indexed gnarId, address winner, uint256 amount, uint256 timestamp);
    ...
    constructor(
        ...
        require(
            _skate != address(0) && _dao != address(0) && _descriptor != address(0) && _seeder != address(0),
            "ZERO ADDRESS"
        );
        skate = _skate;
        dao = _dao;
     ...
     */
    function setSkateDaoAddresses(address _skate, address _dao) external onlyOwner {
    ...
    function mint() internal returns (uint256) {
        IGnarSeeder.Seed memory _seed = seeder.generateSeed(currentGnarId, descriptor);
    ...
    function createBid(
        ...
        require(_skatePercent + _daoPercent == 100, "Sum of percents is not 100");
        ...
        if (lastBidder != address(0)) {
            require(_safeTransferETH(lastBidder, _auction.amount), "ETH transfer failed");
        }
    ...
    function _settleAuction() internal {
        ...
        if (_auction.amount > 0) {
            require(_safeTransferETH(skate, (_auction.amount * _auction.skatePercent) / 100), "ETH transfer failed");
            require(_safeTransferETH(dao, (_auction.amount * _auction.daoPercent) / 100), "ETH transfer failed");
        }
    ...
    function setAuctionPeriodBlocks(uint16 _auctionPeriodBlocks) external onlyOwner {
...
contract GNARSeeder is IGnarSeeder {
    function generateSeed(uint256 gnarId, IGnarDescriptor descriptor) external view override returns (Seed memory) {
...
contract GNARDescriptor is IGnarDescriptor, Ownable {
     ...
     */
    function tokenURI(uint256 tokenId, IGnarSeeder.Seed memory seed) external view override returns (string memory) {
        if (isDataURIEnabled) {
     ...
     */
    function dataURI(uint256 tokenId, IGnarSeeder.Seed memory seed) public view override returns (string memory) {
     ...
     */
    function generateSVGImage(IGnarSeeder.Seed memory seed) external view override returns (string memory) {
     ...
     */
    function _getPartsForSeed(IGnarSeeder.Seed memory seed) internal view returns (bytes[] memory) {
...
interface IGnarSeeder {
    ...
    function generateSeed(uint256 gnarId, IGnarDescriptor descriptor) external view returns (Seed memory);
}
...
interface IGnarDescriptor {
    ...
    function tokenURI(uint256 tokenId, IGnarSeeder.Seed memory seed) external view returns (string memory);

    function dataURI(uint256 tokenId, IGnarSeeder.Seed memory seed) external view returns (string memory);
    ...
    function generateSVGImage(IGnarSeeder.Seed memory seed) external view returns (string memory);
}
