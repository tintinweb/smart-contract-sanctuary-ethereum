...
contract Trade {
    ...
    // function getSigner(bytes32 hash, bytes memory sign) internal pure returns(address) {
    //     (bytes32 r, bytes32 s, uint8 v) = splitSignature(sign);
    //     return ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", hash)),v,r,s );
    // }

    function verifySellerSign(address seller, uint256 tokenId, uint amount, address paymentAssetAddress, address assetAddress, bytes memory sign) public pure {
        ...
        require(seller == recoverSigner(ethSignedMessageHash, sign), "seller sign verification failed");
    }

    function verifyBuyerSign(address buyer, uint256 tokenId, uint amount, address paymentAssetAddress, address assetAddress, uint qty, bytes memory sign) public pure {
        bytes32 hash = keccak256(abi.encodePacked(assetAddress,tokenId,paymentAssetAddress,amount,qty));
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(hash);
        
        require(buyer == recoverSigner(ethSignedMessageHash, sign), "buyer sign verification failed");
    }
        ...
        )
    {
      ...
      function getFees(uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId) public view returns(Fee memory){
        ...
        uint temp = paymentAmt.mul(1000);
        uint price =temp.div((1000+buyerFeePermille));
    ...
    function buyAsset(Order memory order, bytes memory sign) public returns(bool) {
        ...
        verifySellerSign(order.seller, order.tokenId, order.amount, order.erc20Address, order.nftAddress, sign);
    ...
    function executeBid(Order memory order, bytes memory sign) public returns(bool) {
        Fee memory fee = getFees(order.amount, order.nftType, order.nftAddress, order.tokenId);
