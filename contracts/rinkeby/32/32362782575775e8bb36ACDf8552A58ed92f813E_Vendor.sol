...
contract Vendor is Ownable {
    ...
    event SellTokens(address seller, uint256 amount, uint256 payback);
    ...
    function withdraw() external onlyOwner {
        ...
        if (!success) revert Vendor__TransactionFailed();
    }
    ...
    function sellTokens(uint256 amount) external {
        ...
        uint256 payback = amount / tokensPerEth;
        (bool success, ) = payable(msg.sender).call{
            value: amount / tokensPerEth
        }("");
        if (!success) revert Vendor__TransactionFailed();

        emit SellTokens(msg.sender, amount, payback);
    }
}
