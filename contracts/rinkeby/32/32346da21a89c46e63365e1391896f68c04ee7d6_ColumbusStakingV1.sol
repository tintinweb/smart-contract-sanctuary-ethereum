...
contract ColumbusStakingV1 is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMathUpgradeable for uint256;
    string public constant name = "Columbus - Staking";

//  Token which user stakes
    IERC20Upgradeable public stakeToken;
//  User gets reward in this token
    IERC20Upgradeable public rewardToken;
//  Amount of token given out during an interval
    uint256 public rewardTokenEmissionRate;
//  Timestamp from when staking begins
    uint256 public startTime;
//  Timestamp at which staking ends
    uint256 public endTime;
//  emission interval eg., Day, Months, Years, etc.,
    uint256 public emissionInterval;
//  addresses who are staked here
    address[] public stakers;
//  Staking start time
    mapping(address => uint256) public stakingStartTime;
//  Staked balance of addresses
    mapping(address => uint) public stakedBalance;
//  check is account staked
    mapping(address => bool) public hasStaked;
//  check is account currently staking
    mapping(address => bool) public isStaking;
//  total amount staked on contract
    ...
    function initialize(IERC20Upgradeable _stakeToken, IERC20Upgradeable _rewardToken, uint256 _rewardTokenEmissionRate, uint256 _startTime, uint256 _emissionInterval) public virtual initializer {
    ...
    function getCompletedEmissionInterval() public view returns(uint256){
        return (startTime - block.timestamp) / emissionInterval;
    }

    function getNextEmissionStartTime() external view returns(uint256){
        return ((startTime + (getCompletedEmissionInterval() * emissionInterval)) + emissionInterval);
    }
    ...
    */
    ...
    function stakeTokens(uint256 _amount) external virtual whenNotPaused {
        _beforeStakeTokens(_amount);
    ...
    function _beforeStakeTokens(uint256 _amount) internal virtual {
        require(block.timestamp >= startTime, "STAKING: Start Block has not reached");
        require(block.timestamp <= endTime, "STACKING: Has Ended");
        require(_amount > 0, "STAKING: Balance cannot be 0");
        require(stakeToken.balanceOf(msg.sender) >= _amount, "STAKING: Insufficient stake token balance");
    }
    ...
    */
    function calculateReward(address _rewardAddress) public view returns(uint256, uint256){
		...
		if(balances > 0){
		    ...
		    */
            //consider day
            uint256 timeFactor = timeDifferences / emissionInterval;
            uint256 userPercentage = (stakedBalance[_rewardAddress] / _totalStakedAmount) * 100;
            rewards = (userPercentage * (rewardTokenEmissionRate * timeFactor)) / 100;
		}
    ...
    */
    function claimMyReward() external nonReentrant whenNotPaused {
        ...
        require(timeDifferences/emissionInterval >= 1, "STAKING: Can be claimed only after the interval");
        ...
        stakingStartTime[msg.sender] = block.timestamp + emissionInterval;
	}
