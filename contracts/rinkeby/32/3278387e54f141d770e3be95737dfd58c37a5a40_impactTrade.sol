...
contract impactTrade is Initializable, OwnableUpgradeable {
    ...
    function saleToken(
        ...
        if (ids[3] == 721) {
            ...
            if(IERC1155Upgradeable(_conAddr).balanceOf(
                    from,
                    ids[0]
                ) == 0 ) {
                    delete order_place[from][ids[0]];
    ...
    function _saleToken(
        ...
        // (ldatas[0],ldatas[1],ldatas[2],ldatas[3],ldatas[4],ldatas[5],ldatas[6]) = (1,2,3,4,5,6,7);
    ...
    function _acceptBId(
        ...
        //     ldatas[2] >= ldatas[6].div(10**ldatas[1]) &&
        //         ldatas[6] > 0,
        //     "Insufficient Balance in approve"
        // );
        (ldatas[3], ldatas[4], ldatas[5]) = calc(
            ids[1],
            ldatas[0],
    ...
    function getSplitRoyalty(uint256 tokenId) public view returns(address[] memory, uint256[] memory, uint256, uint256) {
        return (cRoyaltyDetails[tokenId].spliters,cRoyaltyDetails[tokenId].splitpercentage,cRoyaltyDetails[tokenId].colletionRoyalty,cRoyaltyDetails[tokenId].tokenId);
    }
