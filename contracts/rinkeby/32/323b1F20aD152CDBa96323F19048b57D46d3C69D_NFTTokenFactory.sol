...
contract NFTTokenFactory is Ownable {
    NFTToken[] public tokens; //an array that contains different NFTToken tokens deployed
    mapping(uint256 => address) public indexToContract; //index to contract address mapping
    mapping(uint256 => address) public indexToOwner; //index to NFTToken owner address

    event NFTTokenCreated(address owner, address tokenContract); //emitted when NFTToken token is deployed
    event NFTTokenMinted(address owner, address tokenContract, uint256 amount); //emmited when NFTToken token is minted
    ...
    /*
    deployNFTToken - deploys a NFTToken token with given parameters - returns deployed address

    _contractName - name of our NFTToken token
    _uri - URI resolving to our hosted metadata
    _ids - IDs the NFTToken token should contain
    _name - Names each ID should map to. Case-sensitive.
    */
    function deployNFTToken(
        string memory _contractName,
        string memory _uri,
        uint256[] memory _ids,
        string[] memory _names
    ) public returns (address) {
        NFTToken t = new NFTToken(_contractName, _uri, _names, _ids);
        tokens.push(t);
        indexToContract[tokens.length - 1] = address(t);
        indexToOwner[tokens.length - 1] = msg.sender;
        emit NFTTokenCreated(msg.sender, address(t));
        return address(t);
    }
    ...
    */
    function getCountNFTTokenbyIndex(uint256 _index, uint256 _id) public view returns (uint256 amount) {
        return tokens[_index].balanceOf(indexToOwner[_index], _id);
    }

    function getCountNFTTokenbyName(uint256 _index, string calldata _name) public view returns (uint256 amount) {
    ...
    function getNFTTokenbyIndexAndId(uint256 _index, uint256 _id)
        public
        view
        returns (
            address _contract,
...
contract NFTToken is ERC1155, Ownable {
    ...
    */
    function uri(uint256 _tokenid) public view override returns (string memory) {
        return string(abi.encodePacked(uriBase, "/", Strings.toString(_tokenid), "/metadata.json"));
    }
