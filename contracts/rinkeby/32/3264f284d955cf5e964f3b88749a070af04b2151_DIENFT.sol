...
contract DIENFT is ERC721A, Ownable, ReentrancyGuard {

    using Strings for uint256;

    address RESURRECTIONCONTRACT;
  ...
  /**
    ...
    modifier mintLimitCompliance(uint256 _mintAmount) {
        ...
        require( totalMintCount() + _mintAmount <= mintSupply , "Max supply Exceeded"); 
       _;
    } 
  ...
  /**
    ...
    */
    ...
    function mintExternal(address _address, uint256 _mintAmount) external {
        require( msg.sender == RESURRECTIONCONTRACT,
            "Sorry you dont have permission to mint form your contract");
        _safeMint(_address, _mintAmount);
    }

    //This function will be used to extend the project with more capabilities 
    function setResurrectionContractAddress(address _rAddress) public onlyOwner {
        RESURRECTIONCONTRACT = _rAddress;
    }


  /**
    * Mint tokens during the presale(Whitelist).
    ...
    */
        ...
        mintLimitCompliance(_mintAmount)
    {
        ...
        //check if wallet has minted the free token
        //else then pay for mint token price 
       if(addressMintedBalance[msg.sender] > freeMintLimit){
           require(msg.value >= whitelistCost * _mintAmount, "Insuffient funds");
         }else{
             uint256 a = freeMintLimit - addressMintedBalance[msg.sender];               
            if(_mintAmount > a){
               uint256 b = _mintAmount - a; 
               require(msg.value >= whitelistCost * b, "Insuffient funds");
             }
        }
      ...
      mintLimitCompliance(_mintAmount)
    {
       ...
       if(addressMintedBalance[msg.sender] > freeMintLimit){
           require(msg.value >= cost * _mintAmount, "Insuffient funds");
            ...
            if(_mintAmount > a){
               uint256 b = _mintAmount - a; 
               require(msg.value >= cost * b, "Insuffient funds");
             }
        }
        _mintLoop(msg.sender, _mintAmount);
        addressMintedBalance[msg.sender] += _mintAmount;
  ...
  /**
    ...
    * Returns the owner token ids owner owned.
    */
    function walletOfOwnerTokens(address _owner)
      public
      view
      returns (uint256[] memory)
    {
      ...
      while (ownedTokenIndex < ownerTokenCount && currentTokenId <= mintSupply) {
    ...
    * @param _tokenIds token ids owner owned.
    * Returns the boolean if token owned by the owner.
    */

    function isOwnerOf(address _account, uint256[] calldata _tokenIds)
      external
      view
      returns (bool)
    {
        for (uint256 i; i < _tokenIds.length; ++i) {
            if (ownerOf(_tokenIds[i]) != _account) return false;
        }
  ...
  /**
    * Update sales free mint limit per wallet.
    * @param _freeMintLimit token limit.
    */
