...
contract Blast is Ownable , VRFConsumerBase{
    ...
    function enter(uint16 _ratio) payable public{
    ...
    function blastPayment() internal onlyOwner{
        require(blast_status == BLAST_STATUS.CALCULATING , "blast is not ended!!!");
        for (uint i=0 ; i<players.length ; i++){
            if (randomRatio > 0){
                if (players[i].ratio <= randomRatio){
                    players[i].account.transfer(players[i].money * players[i].ratio / 100);
                    winnerCounter++;
                }
            }
        }
