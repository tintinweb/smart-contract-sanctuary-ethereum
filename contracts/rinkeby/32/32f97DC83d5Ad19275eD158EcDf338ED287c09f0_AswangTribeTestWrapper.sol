...
import "../AswangTribe.sol";

contract AswangTribeTestWrapper is AswangTribe {
    constructor(
        address signer,
...
contract AswangTribe is ERC721ABase {
    ...
    event HatchEgg(uint256 indexed tokenId);
    ...
    uint256[7] public rankDurations;
        ...
        )
    {
        ...
        rankDurations = [30 days, 90 days, 180 days, 365 days, 1095 days, 12045 days, 121545 days];
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (tokenId < MAX_MINTABLE) {
            metadataBase = revealed
                ? string(abi.encodePacked(genesisURI, _toString(rank(tokenId)), "/"))
                : metadata.placeholderURI;
        } else {
            metadataBase = manananggalURI;
        }

        return string(abi.encodePacked(metadataBase, _toString(tokenId)));
    }
    ...
    function rank(uint256 tokenId) public view returns (uint256) {
        if (prayingStartedAt[tokenId] == 0) return 0;

        uint256 durationPraying = block.timestamp - prayingStartedAt[tokenId];
        for (uint256 i; i < rankDurations.length; ++i) {
            if (rankDurations[i] > durationPraying) return i;
        }

        return rankDurations.length;
    }
    ...
    /**
     * @inheritdoc ERC721ABase
     * @dev Provenance hash is superfluous given the need for known clans pre-reveal
     */
    function flipRevealed() public override lockable onlyOwner {
        revealed = !revealed;
    }
     ...
     */
    function setPraying(uint256[] calldata tokenIds, bool pray) public {
        if (pray && !prayingEnabled) revert AswangTribe_PrayingDisabled();
        ...
        for (uint256 i; i < tokenIds.length;) {
            ...
            prayingStartedAt[tokenId] = timestamp;

            if (pray) emit StartPraying(tokenId);
            else emit StopPraying(tokenId);
    ...
    function summon(uint256 tokenId) public {
        ...
        if (ownerOf(tokenId) != _msgSender()) revert AswangTribe_SenderNotTokenOwner();
        if (prayingStartedAt[tokenId] == 0) revert AswangTribe_TokenNotPraying();
        if (_totalMinted() == MAX_SUPPLY) revert ERC721ABase_InsufficientSupply();

        Dugo.burn(_msgSender(), SUMMONING_COST);

        emit Summon(_nextTokenId());

        _mint(_msgSender(), 1);
    }

    /**
     * @notice Hatch a Manananggal egg, revealing traits
     * @param tokenId Manananggal egg to hatch
     */
    function hatch(uint256 tokenId) public {
        if (tokenId < MAX_MINTABLE) revert AswangTribe_InvalidTokenId();
        if (ownerOf(tokenId) != _msgSender()) revert AswangTribe_SenderNotTokenOwner();
        if (hatched[tokenId]) revert AswangTribe_EggAlreadyHatched();
    ...
    function transferFrom(address from, address to, uint256 tokenId)
        ...
        override(ERC721A)
    {
        if (prayingStartedAt[tokenId] != 0) revert AswangTribe_TokenPraying();
