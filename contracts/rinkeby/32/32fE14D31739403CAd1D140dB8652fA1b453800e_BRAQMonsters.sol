...
contract BRAQMonsters is ERC721A, Ownable, ReentrancyGuard {
    ...
    function whitelistMint(uint256 _mintAmount, uint8 _maxAllowed, bytes32[] calldata _merkleProof) public mintCompliance(_mintAmount) {
            ...
            uint256 whiteListedMintAmt = whitelistedAmt[msg.sender]; 
            require(whiteListedMintAmt + _mintAmount <= _maxAllowed, "Purchase exceeds max whitelisted count."); 
    ...
    function publicMint(uint256 _mintAmount) public mintCompliance(_mintAmount) {
        require(!whiteListSale && publicSale, "Public not Active currently");
        uint256 publicMintAmt = publicAmt[msg.sender];  
        require(publicMintAmt + _mintAmount <= maxPublic, "You have minted the max you are allowed in public sale");
        publicAmt[msg.sender]++;  
        _safeMint(msg.sender, _mintAmount); 
    }

    function mintReserved(address _to, uint256 _mintAmount) external mintCompliance(_mintAmount) onlyOwner {
        require(publicAmt[msg.sender] + _mintAmount <= reservedMonsters, "All reserved Monsters have not been minted"); 
