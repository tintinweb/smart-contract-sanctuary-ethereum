...
contract ExchangePool is AccessControl, ReentrancyGuard
{
    ...
    uint256 public limitedUSDTOrUSDCAmountForPartner = 8.37e7;
    uint256 public level1Amount = 1.23e9;
    uint256 public level2Amount = 5.967e9;
    ...
    )
    {
       ...
       partnersList[0].partnersTokenAmount = 9.3e7;  //1.Winnamore Syndicates 
       partnersList[1].partnersTokenAmount = 8.37e7;  //2.Jonathan 
       partnersList[2].partnersTokenAmount = 1e7;  //3.	PCS 
       partnersList[3].partnersTokenAmount = 1e6;   //4.	JBR 
       _setupRole(Owner_ROLE, mainOwner);
       _setpartnersAddress(_partnerAddress); 
    }

    function _partnerAirdrop() 
      external
      nonReentrant
      onlyOwner
    {
        ...
        require(LEVA.allowance(mainOwner,address(this)) >= _amount,"allowance is not enough for LEVA");
        LEVA.transferFrom(mainOwner,address(this),_amount);
        LEVA.transfer(partnerContract,_amount);
       ...
       onlyOwner 
    {
        ...
        LEVA.transfer(partnersList[_index].listOfPartnersAddress,_amount);
    }
       ...
       nonReentrant
    {
        ...
        LEVA.transferFrom(mainOwner,address(this),tokenAmount); 
        LEVA.transfer(msg.sender,tokenAmount);
        transactionRecords[mainOwner].amountOfLEVAReturn = transactionRecords[mainOwner].amountOfLEVAReturn.
                                                                                           add(tokenAmount);
    ...
    function swapTokenToUSDCOrUSDT(uint256 _amount,address _swapTokenAddress)
      ...
      nonReentrant
    {
        require(LEVA.allowance(msg.sender,address(this)) >= _amount,"allowance is not enough");
        ...
        LEVA.transferFrom(msg.sender,address(this),_amount);  
        LEVA.transfer(mainOwner,_amount);
    }
       ...
       internal
    {
        ...
        uint256 _partner = transactionRecords[partnerContract].amountOfUSDTOrUSDCRecieved;

        if((_partner+_mainOwner+_amount)<=
            (level1Amount.mul(decimal)))
        {
            _transferringUSDCOrUSDT(_amount,level1percentage,_swapTokenAddress);             
        }
        else if((_partner+_mainOwner)<(level1Amount.mul(decimal)) && (_partner+_mainOwner+_amount)>
            (level1Amount.mul(decimal)))
        {
           uint256 _levelTwoAmount = (_partner + _mainOwner + _amount) - level1Amount;
        ...
        else if((_partner+_mainOwner+_amount)<=(level2Amount.mul(decimal)))
        {
            _transferringUSDCOrUSDT(_amount,level2percentage,_swapTokenAddress);
        }
        else if((_partner+_mainOwner)<(level2Amount.mul(decimal)) && (_partner+_mainOwner+_amount)>
            (level2Amount.mul(decimal)))
        {
            uint256 _levellastAmount = (_partner + _mainOwner + _amount) - level2Amount;
    ...
    ) internal
    {
        ...
        partner.distributingUSDTOrUSDC(calculatingPartnerAmount,_swapTokenAddress,exchangeRate);
    ...
    function _returnLEVAFromPartners(uint256 _amount)
       internal
    {
        uint256 tokenAmount = tokenPriceCalculation(_amount);
        require(LEVA.balanceOf(address(this)) >= tokenAmount,"contract don't have enough LEVA token");
        ...
        transactionRecords[partnerContract].amountOfLEVAReturn = transactionRecords[partnerContract].
