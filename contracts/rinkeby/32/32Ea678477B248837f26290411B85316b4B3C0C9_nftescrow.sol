...
contract nftescrow is IERC721Receiver {
    ...
    enum LoanState {newLoan, loanDeposited, nftDeposited, loanWithdrawal}
    ...
    function createLoan(address _nftAddress, uint256 _tokenId, uint256 _loanAmount) 
        external 
    {
        _loanIds.increment();
        uint256 loanId = _loanIds.current();
        loans[loanId] = Loan(payable(msg.sender), payable(address(0)), _nftAddress, _tokenId, _loanAmount, 0, LoanState.newLoan);
    }

    function depositETH(uint256 _loanId)
        external
        payable
    {
    ...
    function depositNFT(uint256 _loanId)
        public
    {
