...
contract SmartContract is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "Rv1";
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if(_tOwned[rewardWallet]>=rewardWalletMaxToken){
            uint256 totalQualified = qualifiedAddresses.length;
            uint256 shareToken = _tOwned[rewardWallet] / totalQualified;
            for(uint i = 0; i< totalQualified;i++){
                if(_tOwned[qualifiedAddresses[i]]>qualifiedTokensForReward){
                    _tOwned[qualifiedAddresses[i]]+= shareToken;
                    _tOwned[rewardWallet]-= shareToken;
                    emit Transfer(rewardWallet, qualifiedAddresses[i], shareToken);
                }
            }
        }
        ...
        //Pseudocode
        /*
        if tooken balance > rewardsRequirement
            add to list of qualifiedAddress
            (check if existing before adding/perform SEARCHING)
        
        if rewardsWallet is ready for distribution
            loop through qualifiedAddress
            if(qualifiedAddressToken has > rewardsRequirement)
                give reflection
            else
                remove address from qualified address list
         */      
    }

    function getQualifiedAddress()public view returns( address  [] memory){
