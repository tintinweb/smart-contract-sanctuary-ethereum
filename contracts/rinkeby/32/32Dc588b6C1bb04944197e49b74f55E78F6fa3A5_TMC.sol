...
contract TMC is ERC721, ERC721Enumerable, Ownable {
    ...
    function removeWhiteListWallet (address[] memory _wallets) public {
        ...
        for(uint i = 0; i < _wallets.length; i++) {
            ...
            uint length = whiteLists.length;
            for(uint j = 0; j < length; j++) {
                if(_wallets[i] == whiteLists[j]) {
                    address temp = whiteLists[j];
                    whiteLists[j] = whiteLists[length - 1];
                    whiteLists[length - 1] = temp;
                    break;    
                }
            }
            if(length > 0)
                delete whiteLists[length - 1];
		}
