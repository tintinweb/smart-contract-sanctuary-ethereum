...
contract AiCloneX is ERC721, Ownable {
  ...
  function changeMaxMintAmountPerTx(uint256 _supply) public view returns (uint256 _maxMintAmountPerTx){
      if(_supply < 10) {
          return 3;
      }
  ...
  function changeCost(uint256 _supply) internal view returns (uint256 _cost){
      if(_supply < 10) {
          return 0 ether;
      }
      if(_supply < 20) {
