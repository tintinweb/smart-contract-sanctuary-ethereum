...
contract NOGOBLIN is ERC721A, Ownable {
    enum Status {
        Waiting,
        Started,
        Finished
    ...
    constructor(string memory initBaseURI) ERC721A("NoGoblin", "NOGOBLIN") {
        baseURI = initBaseURI;
    }
    ...
    function mint(uint256 quantity) external payable {
        ...
        if (INSTANT_FREE_MINTED < FREE_MINT_SUPPLY) {
            uint256 freeMintAmont = (numberMinted(msg.sender) <
            ...
            refundIfOver(
                PUBLIC_PRICE *
                    (
                        (quantity <= freeMintAmont)
                            ? 0
                            : (quantity - freeMintAmont)
                    )
            );
            INSTANT_FREE_MINTED += freeMintAmont;
            emit FreeMinted(
                freeMintAmont,
                INSTANT_FREE_MINTED,
                quantity,
