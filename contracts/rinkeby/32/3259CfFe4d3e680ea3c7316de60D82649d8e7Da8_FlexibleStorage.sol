...
contract ContractStorage {
    ...
    function _memoizeHash(bytes32 contractName) internal returns (bytes32) {
        ...
        if (hashKey == bytes32(0)) {
            ...
            hashKey = keccak256(
                abi.encodePacked(msg.sender, contractName, block.number)
            );
    ...
    function migrateContractKey(
        ...
        require(
            hashes[fromContractName] != bytes32(0),
...
contract FlexibleStorage is ContractStorage, IFlexibleStorage {
        ...
        returns (uint256[] memory)
    {
        ...
        mapping(bytes32 => uint256) storage data = uintStorage[
            hashes[contractName]
        ];
        ...
        returns (int256[] memory)
    {
        ...
        mapping(bytes32 => int256) storage data = intStorage[
            hashes[contractName]
        ];
        for (uint256 i = 0; i < records.length; i++) {
            results[i] = data[records[i]];
        ...
        returns (address[] memory)
    {
        ...
        mapping(bytes32 => address) storage data = addressStorage[
            hashes[contractName]
        ];
        for (uint256 i = 0; i < records.length; i++) {
            results[i] = data[records[i]];
        ...
        returns (bool[] memory)
    {
        ...
        mapping(bytes32 => bool) storage data = boolStorage[
            hashes[contractName]
        ];
        for (uint256 i = 0; i < records.length; i++) {
            results[i] = data[records[i]];
        ...
        returns (bytes32[] memory)
    {
        ...
        mapping(bytes32 => bytes32) storage data = bytes32Storage[
            hashes[contractName]
        ];
        for (uint256 i = 0; i < records.length; i++) {
            results[i] = data[records[i]];
