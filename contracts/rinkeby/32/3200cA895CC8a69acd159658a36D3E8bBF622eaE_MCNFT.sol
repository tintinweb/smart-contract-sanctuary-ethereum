...
contract MCNFT is ERC721A, VRFConsumerBase, ReentrancyGuard, EIP712Whitelisting, ERC2981ContractWideRoyalties {
    ...
    uint private maxMintPerAddress = 2;
    uint256 public revealBlock = 0;
    uint256 public seed = 0;
    uint256 public maxSupply = 10000;
    uint256 public price = 0.00098 ether; // 0.98 ether;
    ...
    constructor(string memory _baseURI, string memory _unrevealedURI, bytes32 _keyHash, address _linkToken, address _linkCoordinator, uint256 _maxSupply, address _treasury, address _reserve)
    ...
    function reserveAirdrop(uint256 amount) external onlyOwner
    {
        ...
        returns (bool)
    {
        ...
        require(price.mul(amount) <= msg.value, "Amount received is not correct");
        require(addressMintCount[_msgSender()] < maxMintPerAddress, "You cannot mint more.");

        if(salePhase == SalePhase.Private) {
            // require(isEIP712WhiteListed(signature), "Not whitelisted."); // whitelist phase then public phase
