...
contract PixelsMetaverseLandETH is PixelsMetaverseLand {
    address private _pawnshop;

    constructor(
        address _endpoint,
        address pawnshop,
        uint256[] memory ids
    ) payable PixelsMetaverseLand(_endpoint) {
        ...
        _pawnshop = payable(pawnshop);
    }

    function getPawnshop() public view returns (address) {
        return _pawnshop;
    }

    function mint(address to, uint256[] memory ids) external payable lock {
        uint256 len = ids.length;
        require(len <= 9, "You can only issue a maximum of nine at a time");
        uint256 total;
        unchecked {
            _id += len;
            total = _price * len;
        ...
        (bool success, ) = _pawnshop.call{value: msg.value}(new bytes(0));
...
contract PixelsMetaverseLand is ERC721, NonblockingLzApp {
    uint256 internal _price = 1;
    uint256 internal _id;

    event CrossChain(
        uint16 _chainId,
        bytes _contractAddress,
        address _receiver,
        uint256[] _ids,
        bool _isReceive
    );
    ...
    constructor(address _endpoint)
        payable
        ERC721("PixelsMetaverseLand", "PML")
    ...
    function crossChain(uint16 _dstChainId, uint256[] memory ids)
        external
        payable
        lock
    {
        ...
        bytes memory payload = abi.encode(msg.sender, ids);
        bytes memory _dstContract = trustedRemoteLookup[_dstChainId];
        require(
            _dstContract.length != 0,
        ...
        emit CrossChain(_dstChainId, _dstContract, msg.sender, ids, false);
    }

    function _nonblockingLzReceive(
        uint16 _srcChainId,
        bytes memory _srcContractAddress,
        uint64,
        bytes memory _payload
    ) internal override {
        (address receiver, uint256[] memory ids) = abi.decode(
            _payload,
            (address, uint256[])
        );
        ...
        _id = total;
        emit CrossChain(_srcChainId, _srcContractAddress, receiver, ids, true);
    }

    function retryCallCrossChain(
        uint16 _srcChainId,
        uint256[] memory ids,
        uint64 _nonce,
        address _receiver
    ) external {
        bytes memory payload = abi.encode(_receiver, ids);
        bytes memory _srcAddress = trustedRemoteLookup[_srcChainId];
        retryMessage(_srcChainId, _srcAddress, _nonce, payload);
    }

    function estimateFees(uint16 _dstChainId, uint256[] memory ids)
        external
        view
        returns (uint256 fee)
    {
