...
contract OmniFoods is ERC721, NonblockingReceiver, ILayerZeroUserApplicationConfig {
    ...
    function setPublicSaleState(bool _value) external onlyOwner {
    ...
    function mintDev(uint256 numTokens) public onlyOwner {
    ...
    function mintTokensPublicSale(uint8 numTokens) external payable {
        require(numTokens > 0, "Number of tokens cannot be lower than, or equal to 0");
    ...
    function addPresaleAddresses(uint numberOfTokens, address[] calldata addresses) external onlyOwner {
        ...
        require(numberOfTokens <= maxMintAmountPerTx, "One presale address can only mint maxMintAmountPerTx tokens maximum");
