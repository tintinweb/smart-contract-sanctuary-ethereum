...
contract SimpleCurveOracle is IOracle {
    ...
    function getDataParameter(ICurvePairs pair, uint128 iCollateral, uint128 iAsset) public view returns (bytes memory) {
        address asset = pair.coins(iAsset);
        uint8 decimals = IERC20Extended(asset).decimals();
        uint256 dx = 10**uint256(decimals);
        if (EXCHANGE_RATE_PRECISION < dx) {
            dx = EXCHANGE_RATE_PRECISION;
        }
        return abi.encode(pair, iAsset, iCollateral, dx);
    }
    ...
    function peek(bytes calldata data) public view override returns (bool, uint256) {
        (ICurvePairs pair, int128 i, int128 j, uint256 dx) = abi.decode(data, (ICurvePairs, int128, int128, uint256));
        uint256 dy = pair.get_dy(i, j, dx);
        dy = dy * (EXCHANGE_RATE_PRECISION / dx); // Convert to the correct precision
        return (true, dy);
    }
