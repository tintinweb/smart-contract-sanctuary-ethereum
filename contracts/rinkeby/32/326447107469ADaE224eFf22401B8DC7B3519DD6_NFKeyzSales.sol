...
contract NFKeyzSales is AccessControl {
    address public nfkeyz;
    ...
    function mint(
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            ...
            // mint the NFT
            INFKeyz(nfkeyz).mint(_user, tokenURIs[minted]);
    ...
    function addInitialURIs(string[] memory _tokenURIs) external onlyAdmin {
    ...
    function changeMintLimit(uint256 _mintLimit) external onlyAdmin {
        mintLimit = _mintLimit;
    }

    /// @notice Set New Whitelist
    /// @param _merkleRootWL New Whitelist
    function setMerkleRootWL(bytes32 _merkleRootWL) external onlyAdmin {
