 ...
 * @dev
 */
contract EscrowLock is SecuredAddressLock, ERC165  {

    string public metadataURI = "https://revest.mypinata.cloud/ipfs/QmPkvkhS9gN8mM2TfMDjF3sSGfx4RowENtXHE93jK6Daei";

    mapping (uint => AdminLock) public locks;

    bytes32 public constant UNLOCK_PREFIX = 'UNLOCK_PREFIX';
    bytes32 public constant HARDLOCK_PREFIX = 'HARDLOCK_PREFIX';

    // Perfect 32-byte object, ideal for close-packing
    struct AdminLock {
        uint80 endTime; // 10 bytes
        address admin;  // 20 bytes
        bool hardlock;  // 1 byte
    ...
    function createLock(uint fnftId, uint, bytes memory arguments) external override onlyRevestController {
        uint80 endTime;
        address admin;
        (endTime, admin) = abi.decode(arguments, (uint80, address));

        // Check that we aren't creating a lock in the past
        require(block.timestamp < endTime, 'E002');

        AdminLock memory adminLock = AdminLock(endTime, admin, false, false);
        locks[fnftId] = adminLock;
    }

    function updateLock(uint fnftId, uint, bytes memory args) external override {
        (bytes32 prefix, bool setToState) = abi.decode(args, (bytes32, bool));
        require(msg.sender == locks[fnftId].admin, '!AUTH'); 
        if(prefix == UNLOCK_PREFIX) {
            locks[fnftId].unlocked = setToState;
        } else if(prefix == HARDLOCK_PREFIX) {
            locks[fnftId].hardlock = setToState;
        }
    }

    function setFNFTUnlockState(uint fnftId, bool state) external {
        require(locks[fnftId].admin == _msgSender(),'!AUTH');
        _setUnlockState(fnftId, state);
    }

    function batchSetFNFTUnlockState(uint[] memory fnftIds, bool state) external {
        for(uint i; i < fnftIds.length; i++) {
            uint fnftId = fnftIds[i];
            require(locks[fnftId].admin == _msgSender(),'!AUTH');
            _setUnlockState(fnftId, state);
        }
    }

    function setFNFTHardlockState(uint fnftId, bool state) external {
        require(locks[fnftId].admin == _msgSender(),'!AUTH');
        _setHardlockState(fnftId, state);
    }

    function batchSetFNFTHardlockState(uint[] memory fnftIds, bool state) external {
        for(uint i; i < fnftIds.length; i++) {
            uint fnftId = fnftIds[i];
            require(locks[fnftId].admin == _msgSender(),'!AUTH');
            _setHardlockState(fnftId, state);
    ...
    function getDisplayValues(uint fnftId, uint) external view override returns (bytes memory) {
        uint endTime = locks[fnftId].endTime;
        address admin = locks[fnftId].admin;
        bool canUnlock = admin == _msgSender();
        return abi.encode(endTime, admin, locks[fnftId].hardlock, canUnlock);
    }

    function isUnlockable(uint fnftId, uint) public view override returns (bool) {
        return !locks[fnftId].hardlock && (locks[fnftId].unlocked || block.timestamp > locks[fnftId].endTime);
    }
    ...
    function _setUnlockState(uint fnftId, bool state) private {
        locks[fnftId].unlocked = state;
    }

    function _setHardlockState(uint fnftId, bool state) private {
        locks[fnftId].hardlock = state;
    }
