 ...
 */
contract ERC721DutchAuctionMatcherModule is ISubModule {
    error ReentryError();
    error MaxTokenAmountExceededError();
    error ETHNotSentError();

    event DutchOrderFilled(address indexed maker, uint256 indexed orderId, bool indexed orderType);

    address internal addressLock;
    address public immutable WETH;
    IERC721Module public immutable ERC721ModuleContract;
    address public immutable dutchAuctionModule;
     ...
     * @param _dutchAuctionModule Address of the dutchAuctionModule contract.
     */
    constructor(
        address _WETH,
        address _ERC721Module,
        address _dutchAuctionModule
    ) {
        WETH = _WETH;
        ERC721ModuleContract = IERC721Module(payable(_ERC721Module));
        dutchAuctionModule = _dutchAuctionModule;
    }

    /**
     * @notice Matches dutch orders.
     * @dev Matches a provided dutch order with its counterpart created during the execution of the function. Depending on the characteristics of the order, the matching is done with different ERC721Module functions. Emits one detailed event of the sell order.
     * @param _order Dutch order struct to be matched/filled.
     * @param _maxTokenAmount Max price (in amount of tokens) the buyer is willing to pay.
     */
    function matchOrder(LibDutchOrder.SignedOrder calldata _order, uint256 _maxTokenAmount) external payable {
        LibOrder.VerifiedOrder memory signedSellOrder = LibDutchOrder.getSignedOrder(_order, dutchAuctionModule);
        if (signedSellOrder.order.tokenAmount > _maxTokenAmount) {
            revert MaxTokenAmountExceededError();
        }

        // If sellOrder expects ETH or WETH and msg.value != 0
        if (
            (signedSellOrder.order.token == address(0) || signedSellOrder.order.token == address(WETH)) &&
            msg.value != 0
        ) {
            ERC721ModuleContract.fillSell{ value: signedSellOrder.order.tokenAmount }(signedSellOrder, msg.sender);
            if (msg.value > signedSellOrder.order.tokenAmount) {
                payable(msg.sender).transfer(msg.value - signedSellOrder.order.tokenAmount);
            }
            emit DutchOrderFilled(
                signedSellOrder.order.maker,
                signedSellOrder.order.orderId,
                signedSellOrder.order.orderType
            );
            return;
        }

        // If sellOrder expects ETH and msg.value == 0
        if (signedSellOrder.order.token == address(0) && msg.value == 0) {
            revert ETHNotSentError();
        }

        // Otherwise match it using a generated buy order
        LibOrder.VerifiedOrder memory signedBuyOrder;

        signedBuyOrder.order.maker = msg.sender;
        signedBuyOrder.order.taker = address(0);
        signedBuyOrder.order.nft = signedSellOrder.order.nft;
        signedBuyOrder.order.token = signedSellOrder.order.token;
        signedBuyOrder.order.orderId = signedSellOrder.order.orderId;
        signedBuyOrder.order.nftId = signedSellOrder.order.nftId;
        signedBuyOrder.order.tokenAmount = signedSellOrder.order.tokenAmount;
        signedBuyOrder.order.validTill = signedSellOrder.order.validTill;
        signedBuyOrder.order.orderType = LibOrder.BUY_TYPE;
        signedBuyOrder.order.feeFraction = signedSellOrder.order.feeFraction;
        signedBuyOrder.verificationMethod = signedSellOrder.verificationMethod;
        signedBuyOrder.signature = abi.encode(address(this), bytes(""));

        LibOrder.VerifiedOrder[] memory orders = new LibOrder.VerifiedOrder[](2);
        orders[0] = signedBuyOrder;
        orders[1] = signedSellOrder;
        // Keep hash of order in storage to be able to later verify it, also functions as a reentry protection
        if (orderHash != bytes32(0)) {
            revert ReentryError();
        }
        orderHash = keccak256(abi.encode(signedBuyOrder.order));
        ERC721ModuleContract.batchMatchOrders(orders, true);
        orderHash = bytes32(0);
        emit DutchOrderFilled(
            signedSellOrder.order.maker,
            signedSellOrder.order.orderId,
    ...
    /**
     * @notice Validates an order with its saved hash.
     * @dev A security check that uses a stored hash to check if the input order has been stored previously.
     * @param _order Order struct to be validated.
     * @param _data Encoded parameters of the order.
     * @return Whether or not the order hash is stored.
     */
    function validateAndHandleOrder(LibOrder.Order calldata _order, bytes calldata _data) external returns (bool) {
        // check if saved hash matches order
        return orderHash == keccak256(abi.encode(_order));
    }
}
