...
contract Fancy721 is ERC721Enumerable, Ownable, AccessControlEnumerable {
    ...
    // function bulkMint(address[] calldata _to, uint256[] calldata _referenceTokenIds) public onlyRole(MINTER_ROLE) {
    //     require(_to.length == _referenceTokenIds.length, "bulkMint: arrays must match in length");
    //     for(uint256 i = 0; i < _referenceTokenIds.length; i++){
    //         require(referenceContract.ownerOf(_referenceTokenIds[i]) == _to[i], "bulkMint: destination address must own bored ape token");
    //         require(!tokenClaimed[_referenceTokenIds[i]], "bulkMint: ape token already used for claim");
    //         _safeMint(_to[i], _referenceTokenIds[i]);
    //         tokenClaimed[_referenceTokenIds[i]] = true;
    //     }
    // }
