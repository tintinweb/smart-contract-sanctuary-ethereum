...
contract TestNFT is Ownable, ERC721A, ReentrancyGuard {
    // // metadata URI
    ...
    constructor() ERC721A("Test NFT", "TT", MAX_BATCH_SIZE, MAX_TOKENS) {
    }
    ...
    // For team members.
    function teamMint(uint256 _quantity) external payable callerIsUser isMintable(_quantity) {
    ...
    // For whitelist.
    function whitelistMint(uint _quantity) external payable callerIsUser isMintable(_quantity) {
    ...
    function publicSaleMint(uint256 _quantity) external payable callerIsUser isMintable(_quantity) {
        require(
    ...
    // For airdrop.
    function airdrop(address[] memory _recipients, uint[] memory _amounts) external onlyOwner {
        require(_recipients.length == _amounts.length, "Wrong value");
        uint quantity = 0;
        for(uint i = 0; i < _amounts.length; i++) {
          quantity += _amounts[i];
        }
        require(totalSupply() + quantity <= MAX_TOKENS - RESERVED_TOKENS, 
            "Would exceed max supply");

        for(uint i = 0; i < _recipients.length; i++) {
            require(
                userTotalMinted[_recipients[i]] + _amounts[i] <= TOTAL_LIMIT_PER_ACCOUNT,
                "Can not mint more than the total limit per account"
            );
            _safeMint(_recipients[i], _amounts[i]);
            if ( teams[_recipients[i]] > 0)
                teamMinted[_recipients[i]] += _amounts[i];
            else if ( whitelists[_recipients[i]] > 0)
                whitelistMinted[_recipients[i]] += _amounts[i];
            userTotalMinted[_recipients[i]] += _amounts[i];
        }
    }
