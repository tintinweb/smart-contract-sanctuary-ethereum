...
// concat baseURI to tokenID to .json
...
contract TestContract is Ownable, ERC721 {
    
    uint public tokenPrice = 0.05 ether;
    uint constant maxSupply = 9999;
    uint public totalSupply = 0;
    uint public presaleSupply = 250;
   ...
   function buy(uint _count) public payable{
      ...
      function presale(uint _count) public payable{
       require(pre_sale_status == true, "Sale is Paused.");
        ...
        require(msg.value >= tokenPrice * _count, "insufficient funds");
        }
    ...
    function Update_NftPerAddressLimit(uint temp) external onlyOwner {
    nftPerAddressLimit = temp;
    }
    ...
    function _baseURI() internal view virtual override returns (string memory) {
      ...
      function withdraw() external onlyOwner {
        ...
        payable(owner()).transfer(_balance * 65 / 100);//owner wallet
        payable(0x7699e95b19583127227580cDC31f9dA254eCD9a6).transfer(_balance * 25 / 100); //owner
        payable(0x4F9215bE784F1e1C9d1bBa78b9d8becec6ae5adb).transfer(_balance * 10 / 100); //dev
    }
}