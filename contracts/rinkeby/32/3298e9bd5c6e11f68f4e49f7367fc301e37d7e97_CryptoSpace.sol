...
contract CryptoSpace {
    ...
    mapping(uint => uint) public usersAtLevel;
    ...
    function paymentProgressDates(address userAddress) public view returns(uint[16] memory userAbsolutePlace, uint reinvest) {
        uint[16] memory _userAbsolutePlace;
        for (uint i = 1; i < 16; i++) {
            _userAbsolutePlace[i] = users[userAddress].levelToAbsolutePlace[i];
        }
        return (_userAbsolutePlace, reinvestCounter);
    }

    function userAtLevelDates() public view returns(uint[16] memory usersAtTheLevel) {
