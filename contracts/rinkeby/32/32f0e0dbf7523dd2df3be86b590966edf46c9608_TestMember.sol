...
contract TestMember is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    
    IERC20 private constant testTokens = IERC20(0xb11fc9C9D7F683f87C16728E64DF6b0e0e633608);

    mapping(address => TokenTimelock) private memberships;
    mapping(address => uint256)private memberType;
    ...
    function ModifyPrice(uint256 index, uint256 value) external onlyOwner {
        prices[index] = value;
    }
    ...
    function ModifyReleaseTime(address target, uint256 time) external onlyOwner {
        require(address(memberships[target]) != address(0), "Empty Vault");
        memberships[target].overwriteReleaseTime(time);
    }

    function CheckTokenRequirements(uint256 membershipType) internal view returns (bool) {
        require(prices[membershipType] <= testTokens.balanceOf(address(_msgSender())), "Insufficient FYN");
        require(prices[membershipType] <= testTokens.allowance(address(_msgSender()), address(this)), "Not enough allowance");
    ...
    function CreateAndSignupMembership(uint256 membershipType) external nonReentrant {
        require(prices[membershipType] > 0, "No valid pricing for membership index");
        require(address(memberships[_msgSender()]) == address(0), "A vault is already created");
        require(memberType[_msgSender()] == 0, "Already have a membership");
        
        TokenTimelock newVault = new TokenTimelock(testTokens, _msgSender(), block.timestamp + LOCKUP_DURATION, address(this));
        testTokens.safeTransferFrom(_msgSender(), address(newVault), prices[membershipType]);

        memberships[_msgSender()] = newVault;
        memberType[_msgSender()] = membershipType;
    }

    function GetMembershipStatus(address target) external view returns(uint256) {
        return (memberType[target]);
    }

    function GrantMembershipStatus(address target, uint256 status) external onlyOwner{
        memberType[target] = status;
    }

    //Vault

    function GetVaultAddress(address wallet) external view returns(address) {
        return address(memberships[wallet]);
    }

    function GetVaultAddress2() external view returns(address) {
        return address(memberships[_msgSender()]);
    }

    function GetVaultBalance(address wallet) external view returns(uint256) {
        return testTokens.balanceOf(address(memberships[wallet]));
    }

    function GetVaultReleaseTime() external view returns(uint256) {
        require(address(memberships[_msgSender()]) != address(0), "No vault was found");
        return memberships[_msgSender()].releaseTime();
    }

    function WalletVaultReleaseTime(address wallet) external view onlyOwner returns(uint256) {
        require(address(memberships[wallet]) != address(0), "No vault was found");
        return memberships[wallet].releaseTime();
    }

    function RedeemVault() external nonReentrant {
        require(address(memberships[_msgSender()]) != address(0), "No vault was found");
        memberships[_msgSender()].release();
    }
}