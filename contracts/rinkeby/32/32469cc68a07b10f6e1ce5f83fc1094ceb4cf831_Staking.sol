    ...
    function calculateReward(uint256 bundleId) public view returns(uint256) {
        if (ownerOf(bundleId) == address(this)) {
            ...
            for(currentIndex; currentIndex < baseRewardsIndex - 1; currentIndex++)   {
				...
				totalRewards = totalRewards.add(((nextTimestamp - previousTimestamp).div(180)).mul(baseRewardsData[currentIndex].rewardsPerSecond));
                previousTimestamp = nextTimestamp;
            }
        
            //Add rewards for current index
            totalRewards = totalRewards.add(((block.timestamp - previousTimestamp).div(180)).mul(baseRewardsData[currentIndex].rewardsPerSecond));
            return totalRewards;
        }
