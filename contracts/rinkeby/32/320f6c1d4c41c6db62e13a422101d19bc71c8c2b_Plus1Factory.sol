...
contract Plus1Factory {
    ...
    constructor() {
        impl = new Plus1();
        impl.init(address(this), "", "", ITokenOwner(address(0)), 0, 1, "");
    }
    ...
    /// @param startId The starting ID of the `original` contract.
    /// @param limit The number of Plus-1s that can be created per original token
    ...
    function createPlus1(
        ...
        p1.init(msg.sender, name, symbol, original, startId, limit, baseURI);
...
/// @notice Let owners of an original ERC-721 mint plus1 versions up to a fixed limit.
/// The original has the abilithy to transfer and burn these plus1 versions.
...
contract Plus1 is ERC721, Initializable, Owned {
    ...
    {
        return string.concat(_baseURI, originalTokenId(id).toString());
    }
    ...
    function mint(uint256 id, address to) external {
        ...
        uint256 originalId = originalTokenId(id);
        if (original.ownerOf(originalId) != msg.sender) {
    ...
    function transferFrom(
        ...
        if (
            !(msg.sender == from ||
                msg.sender == original.ownerOf(originalTokenId(id)) ||
    ...
    function originalTokenId(uint256 id) public view virtual returns (uint256) {
        if (id < startId) {
            revert IdOutOfBounds();
        }

        uint256 offsetId = id - startId;
        return (offsetId - (offsetId % limit)) / limit + startId;
    }

    function plus1IdAtIndex(uint256 originalId, uint256 index)
        external
        view
        virtual
        returns (uint256)
    {
        if (originalId < startId) {
            revert IdOutOfBounds();
        }
        if (index >= limit) {
            revert IndexOutOfBounds();
        }
        uint256 offsetTokenId = originalId - startId;
        uint256 offsetBaseId = offsetTokenId * limit;
        uint256 offsetId = offsetBaseId + index;
        return offsetId + startId;
    }
