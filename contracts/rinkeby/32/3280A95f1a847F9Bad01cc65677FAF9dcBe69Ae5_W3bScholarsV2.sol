...
contract W3bScholarsV2 is ERC721A, IERC2981, Ownable, ReentrancyGuard {
    ...
    function mintGenesis(uint256[] calldata tokenIndexes) external {
        ...
        require(!isSaleFinalized, "Sale is already finalized");
        require(
            totalSupply() + tokenIndexes.length <= MAX_SUPPLY,
            "Purchase would exceed max supply"
        );
        ...
        for (uint256 i = 0; i < tokenIndexes.length; i++) {
            require(
                tokenIndexes[i] > 0 && tokenIndexes[i] < 22,
                "Token number is invalid"
            );
            require(
                scholarContract.ownerOf(tokenIndexes[i]) == msg.sender,
                "Token is not owned by sender"
            );
            // what happens if token number provided goes outside the bounds of array?
            require(
        ...
        // record them as minted
        for (uint256 i = 0; i < tokenIndexes.length; i++) {
            genesisMinted[tokenIndexes[i] - 1] = true;
        }

        _safeMint(msg.sender, tokenIndexes.length);
    }
    ...
    function withdraw() public onlyOwner {
        require(isSaleFinalized, "Sale must be finalized");
        uint256 ts = totalSupply();
        require(ts > 0, "Total supply must be non-zero");

        // determine proceeds from the mint
        uint256 contractBalance = address(this).balance;
        uint256 ownerProceeds = (contractBalance * OWNER_EQUITY_BASIS) / 10000; // check for overflow
        uint256 minterProceeds = ((contractBalance * MINTER_EQUITY_BASIS) /
            10000) / ts; // check for overflow

        // send out 75% to beneficiary
        payable(beneficiary).transfer(ownerProceeds);

        // send out 25% split between minters
        address tokenOwner;
        for (uint256 i = 0; i < ts; i++) {
            tokenOwner = ownerOf(i);
            payable(tokenOwner).transfer(minterProceeds);
        }

        emit ProceedsWithdrawn(contractBalance);
    }
