...
contract TheBearClan is ERC721A, Ownable, ReentrancyGuard {
    ...
    function preSaleMint(uint256 _amount) external payable nonReentrant{
        require(preSaleActive, "BC Pre Sale is not Active");
        require(nftMinted[msg.sender].add(_amount) <= maxPreSale, "BC Maximum Pre Sale Minting Limit Reached");
        mint(_amount, true);
    }

    function publicSaleMint(uint256 _amount) external payable nonReentrant {
        require(publicSaleActive, "BC Public Sale is not Active");
        require(nftMinted[msg.sender].add(_amount) <= maxPublicSale, "BC Maximum Minting Limit Reached");
        mint(_amount, false);
    }

    function mint(uint256 amount,bool state) internal {
        require(!paused, "BC Minting is Paused");
        require(totalSupply().add(amount) <= maxSupply, "BC Maximum Supply Reached");
        if(state){
            require(preSalePrice*amount <= msg.value, "BC ETH Value Sent for Pre Sale is not enough");
        }
        else{
            require(publicSalePrice*amount <= msg.value, "BC ETH Value Sent for Public Sale is not enough");
        }
    ...
    function airDrop(address[] memory _address) external onlyOwner {
        require(totalSupply().add(_address.length) <= maxSupply, "BC Maximum Supply Reached");
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        require(_exists(_tokenId), "BC URI For Token Non-existent");
        if(!revealed){
