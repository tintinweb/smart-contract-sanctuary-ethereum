...
contract RewardPool is ReentrancyGuard, Ownable, Pausable, ERC20 {
    using Math for uint256;

    uint256 public epochDuration;
    uint256 public startTime;
    uint256 public rewardsBalance;
    address public slotMachine;

    uint256 MINIMUM_LIQUIDITY = 1000;

    modifier onlySlotMachine() {
        require(
            _msgSender() == slotMachine,
    ...
    function addLiquidity() external payable returns (uint256 liquidity) {
        ...
        uint256 _reserve = rewardsBalance;
    ...
    function addFunds(uint256 amount) external payable onlySlotMachine {}

    function awardWinner(uint256 amount) external onlySlotMachine {}

    function endEpoch() external onlyOwner {}

    function epochEndAt() internal view returns (uint256) {
        return startTime + epochDuration;
    }

    function hasEpochEnded() internal view returns (bool) {
