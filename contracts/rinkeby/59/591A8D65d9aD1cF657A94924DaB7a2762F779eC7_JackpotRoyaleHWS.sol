...
interface HWSContract {
    function balanceOf(address owner) external view returns (uint256 balance);
}

contract JackpotRoyaleHWS is ERC1155, Ownable {
    string private baseUri; // Pinata URL
    string public name; // Collection Name
    string public symbol; // Collection symbol
    uint256 private unitPrice = 0.001 ether;

    uint256 private goldMember = 5;
    uint256 private silverMember = 3;
    uint256 private normalMember = 2;

    address private hwsAddress;
    ...
    // Modifiers
    modifier beforeMint(uint256 _tokenId) {
        uint256 _hustlerBalance = balanceOfHustlers(msg.sender);
        require(_hustlerBalance > 0, "Not holder of hustler");
        require(
        ...
        require(_tokenId >= 1 && _tokenId <= maxSupply, "Invalid token id");
        uint256 _tokenAllowed = normalMember;
        if (_hustlerBalance >= 5) {
            _tokenAllowed = goldMember;
        } else if (_hustlerBalance >= 3) {
            _tokenAllowed = silverMember;
        }

        require(
            walletBalance[msg.sender] + 1 <= _tokenAllowed,
            "Minting limit exceeded"
        );
        require(msg.value >= getUnitPrice(), "Not enough ETH sent");
        _;
    }
    ...
    function mint_token(uint256 _tokenId) public payable beforeMint(_tokenId) {
        if (mintedTokenIds.length + 1 == maxSupply) {
            CurrentMintingStatus = MintingStatus.Close;
        ...
        emit OnMintToken(mintedTokenIds);
    }

    function getHwsAddress() public view returns (address) {
        return hwsAddress;
    }

    function setHwsAddress(address _address) public onlyOwner {
        hwsAddress = _address;
    }

    function balanceOfHustlers(address _address) public view returns (uint256) {
        return HWSContract(hwsAddress).balanceOf(_address);
    }
