...
// Name of the marketplace
    ...
    //Index of Lottery 
    uint256 public Participatorindex = 0;
...
//bidder details
 struct Bidder {
    ...
    //bidder details
 struct LotteryParticipator {
        uint256 index;
        uint256 lotteryIndex;
        address participatorAddress;
    ...
    //Structure to define Lottery properties 
    struct Lottery {
        uint256 index; // Auction Index
        address addressNFTCollection; // Address of the ERC721 NFT Collection contract
        address addressPaymentToken; // Address of the ERC20 Payment Token contract
        uint256 nftId; // NFT Id
        address creator; // Creator of the Lottery Sale
        uint RoyaltyP;   //Royalty P 
        address CollectionOwnerAddress;
        uint256 LotteryAmount; // Lottery price of this sale 
        uint256 endLotteryDate; // Timestamp for the end day&time of the auction
        uint256 LotteryParticipantCount; // Number of lottery entered in Lottery Auction 
    }

    //Array for all bidders
    Bidder[] private Allbidders;
    //Lottery Participator 
    LotteryParticipator[] private AllLotteryParticipator ;
    //Array for Fixed Price
    FixedPriceSell[] private AllFixedPriceSell;
    //Array for All Lotteries
...
//get auction details
function getAuctionDetails(uint256 _auctionIndex) public view returns (uint, address , address ,uint , address , address , uint ,uint ,uint){

  Auction memory Auctions = allAuctions[_auctionIndex];
  return (Auctions.index, Auctions.addressNFTCollection, Auctions.addressPaymentToken, Auctions.nftId, Auctions.creator, Auctions.currentBidOwner, Auctions.currentBidPrice, Auctions.endAuction, Auctions.bidCount);
}
...
/**
    ...
    function RewardNFT(uint256 _auctionIndex) external {
        ...
        uint256 ownerCut = (auction.currentBidPrice * Platformpercentage) / 100;
        //owner of this contract
        address owner = owner();
        //Give royalties to NPort (10%)
        uint256 nportCut = (auction.currentBidPrice * 10) / 100;
        //Balance remained
        uint balanceforSeller = (auction.currentBidPrice -  ownerCut - nportCut);
        //Get ERC20 Payment token contract
        
        require(
            paymentToken.transferFrom(auction.currentBidOwner,auction.CollectionOwnerAddress, nportCut),
            "Tranfer of token failed"
        );
        
        require(
            paymentToken.transferFrom(auction.currentBidOwner,owner, ownerCut),
            "Tranfer of token failed"
        );
        ...
        require(
            paymentToken.transferFrom(auction.currentBidOwner,auction.creator, balanceforSeller)
        );
...
function getFixPriceDetails(uint256 _fixIndex) public view returns (uint , uint , address ,address ,address , uint ,  FixSaleStatus ){

  FixedPriceSell memory Sales = AllFixedPriceSell[_fixIndex];
  return (Sales.index,Sales.TokenId,Sales.CollectionAddress , Sales.CollectionOwnerAddress,Sales.NFTOwnerAddress,Sales.Price,Sales.Status);
}




//Lottery Section begins s
 function createLottery(
        ...
        require(Royalty <= 10 , "Max 10% royalty allowed.");
        // Check if the endAuction time is valid
        require(_endLottery > block.timestamp, "Invalid end date for auction");

        // Check if the initial bid price is > 0
        require(_lotteryAmount > 0, "Invalid Lottery price");
    ...
    function isLotteryOpen(uint256 _lotteryIndex) public view returns (bool) {
        Lottery storage lottery = allLotteries[_lotteryIndex];
        if (block.timestamp >= lottery.endLotteryDate) return false;
        return true;
    }

    

    function getLotteryParticipantCount(uint256 _lotteryIndex)
        public
        view
        returns (uint256)
    {
        require(_lotteryIndex < allLotteries.length, "Invalid lottery index");
        return allLotteries[_lotteryIndex].LotteryParticipantCount;
    }

    
    function ParticipateinLottery(uint256 _lotteryIndex, uint256 _newLotteryAmount)
        external
        returns (bool)
    {
       require(_lotteryIndex < allLotteries.length, "Invalid lottery index");
        ...
        // check if auction is still open
        require(isOpen(_lotteryIndex), "Lottery is not open");

        // check if new bid price is higher than the current one
        require(
            _newLotteryAmount >= lottery.LotteryAmount,
            "must be same as lottery price"
        );

        // check if new bider is not the owner
        require(
            msg.sender != lottery.creator,
            "Creator of the lottery cannot participate in lottery"
        );
        
          LotteryParticipator memory participator = LotteryParticipator({
            index : Participatorindex,
            lotteryIndex : _lotteryIndex,
            participatorAddress : msg.sender,
            lotteryAmount : _newLotteryAmount
        });
        
        AllLotteryParticipator.push(participator);
        Participatorindex ++ ;
        // get ERC20 token contract
        IERC20 paymentToken = IERC20(lottery.addressPaymentToken);

        // new bid is better than current bid!

        // Transfer
        require(
            paymentToken.transfer(address(this), _newLotteryAmount),
            "Tranfer of token failed"
        );
        
        // update lottery info
        
        lottery.LotteryParticipantCount++;
...
//get all bids
function AllLotteries(uint256 _auctionIndex) public view returns (LotteryParticipator[] memory) {
        LotteryParticipator[] memory subBids = new LotteryParticipator[](AllLotteryParticipator.length);
        uint count = 0;
        for (uint i=0; i<AllLotteryParticipator.length ; i++)
        {
            if (AllLotteryParticipator[i].lotteryIndex == _auctionIndex)
            {
                subBids[count] = AllLotteryParticipator[i];
                count++;
            }
        }
        return subBids; 
...
//get bid details
function getParticipantDetails(uint Index) public view returns (uint , uint , address , uint){

    LotteryParticipator memory parti = AllLotteryParticipator[Index];
    return (parti.index ,parti.lotteryIndex , parti.participatorAddress , parti.lotteryAmount);
}
//get auction details
function getLotteryDetails(uint256 _auctionIndex) public view returns (uint, address , address ,uint , address  , uint ,uint ,uint){

  Lottery memory lottery = allLotteries[_auctionIndex];
  return (lottery.index, lottery.addressNFTCollection, lottery.addressPaymentToken, lottery.nftId, lottery.creator, lottery.LotteryAmount, lottery.endLotteryDate, lottery.LotteryParticipantCount);
}



    function RewardLotteryNFT(uint256 _lotteryIndex , uint256 _lotteryParticipatorIndex) external {
        require(_lotteryIndex < allLotteries.length, "Invalid lottery index");

        // Check if the auction is closed
        require(!isLotteryOpen(_lotteryIndex), "Lottery is still open");
        // Get Lottery Details
        Lottery storage lottery = allLotteries[_lotteryIndex];
        require(
        ...
        //Get Lottery Participant details 
        LotteryParticipator storage lotteryParticipator = AllLotteryParticipator[_lotteryParticipatorIndex];
        // Check if the caller is the winner of the auction
        uint FinalAmount = 0;

        for (uint i=0; i<AllLotteryParticipator.length ; i++)
        {
            if (AllLotteryParticipator[i].lotteryIndex == _lotteryIndex)
            {
                FinalAmount += AllLotteryParticipator[i].lotteryAmount;
            }
        }

        uint256 Platformpercentage = 5;
        //Give royalties to original contract owner
        uint256 ownerCut = (FinalAmount * Platformpercentage) / 100;
        //owner of this contract
        address owner = owner();
        //Give royalties to NPort (10%)
        uint256 nportCut = (FinalAmount * 10) / 100;
        //Balance remained
        uint balanceforSeller = (FinalAmount -  ownerCut - nportCut);
        //Get ERC20 Payment token contract
        IERC20 paymentToken = IERC20(lottery.addressPaymentToken);
        
        
        require(
            paymentToken.transfer(lottery.CollectionOwnerAddress, nportCut),
            "Tranfer of token failed"
        );
        
        require(
            paymentToken.transfer(owner, ownerCut),
            "Tranfer of token failed"
        );

        // contract to the lottery creator address
        require(
            paymentToken.transfer(lottery.creator, balanceforSeller),
            "Tranfer of token failed"
        );

        // Get NFT collection contract
        IERC721 nftCollection = IERC721(
            lottery.addressNFTCollection
        );
        ...
        // to the winner address
            nftCollection.transferFrom(
                address(this),
                lotteryParticipator.participatorAddress,
                lottery.nftId
            );
