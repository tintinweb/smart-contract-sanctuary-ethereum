...
contract NFT_Auction is ERC721URIStorage, ReentrancyGuard {
    ...
    address[] internal addressTeam;

    uint256[] internal sharesTeam;
    ...
    error Error_HighestBidderCantWithdraw();
    ...
    constructor(
        ...
        uint256[] memory _sharesTeam
    ) ERC721(name, symbol) {
        require(
            _addressTeam.length == _sharesTeam.length,
            "PaymentSplitter: payees and shares length mismatch"
        );
        require(_addressTeam.length > 0, "PaymentSplitter: no payees");
        AUCTION_DURATION = AUCTION_DURATION_;
        MIN_PRICE = MIN_PRICE_;
        MIN_BID = MIN_BID_;
        owner = msg.sender;
        addressTeam = _addressTeam;
        sharesTeam = _sharesTeam;
    ...
    function claimRefund() external {
        uint256 refund = getAmountToRefund(msg.sender);
        require(refund != 0);
        if (msg.sender == bidder) {
            revert Error_HighestBidderCantWithdraw();
        }
    ...
    function getAmountToRefund(address recipient)
        public
        view
        returns (uint256)
    {
    ...
    function _payTeam() private {
        for (uint256 n = 0; n < addressTeam.length; n++) {
            address recipient = addressTeam[n];
            uint256 payment = (bidPrice * sharesTeam[n]) / 100;
            Address.sendValue(payable(recipient), payment);
        }
    }

    function _refundWinner() private {
        uint256 remaining = refunds[bidder] - bidPrice;
        if (remaining != 0) {
            refunds[bidder] = 0;
            Address.sendValue(payable(bidder), remaining);
        }
    }
}
