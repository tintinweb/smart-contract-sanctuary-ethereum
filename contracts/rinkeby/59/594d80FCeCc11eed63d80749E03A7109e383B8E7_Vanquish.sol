...
contract Vanquish is IVanquish {
    ...
    function spawn(int256 x, int256 y) external {
        ...
        if (getLand(x, y) != 0) revert NotSpawnLandError();
    ...
    function claim(
        ...
        statsOf[msg.sender].availableResources += getLand(x, y);
        map[x][y].owner = msg.sender;
    ...
    function getLand(int256 x, int256 y) public pure returns (uint256) {
