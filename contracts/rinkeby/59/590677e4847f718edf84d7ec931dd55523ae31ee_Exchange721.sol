...
pragma solidity ^0.8.1;
contract Exchange721 is ERC721HolderUpgradeable, OwnerOperator {
    ...
    function getCreatorOf(address _tokenAddress) external view returns(address) {
        return _creatorOf[_tokenAddress];
    }
    ...
    function sell(
        ...
        require(
            msg.sender == IERC721Upgradeable(_tokenAddress).ownerOf(_tokenId),
    ...
    function buy(
        ...
        require(
            IERC20Upgradeable(orders[_orderId].currency).balanceOf(msg.sender) >=
    ...
    function acceptOffer(
        ...
        require(
            IERC721Upgradeable(_tokenAddress).ownerOf(_tokenId) == msg.sender,
            "You are not the owner of NFT"
        );
    ...
    function setCreatorAddress(
        ...
        _creatorOf[_tokenAddress] = _creator;
    }
...
function bidOverPrice(
    ...
    ,bool payCreatorFee
        ...
        require(
            IERC20Upgradeable(orders[_orderId].currency).balanceOf(msg.sender)>= _price,
        ...
        IERC20Upgradeable(orders[_orderId].currency).transferFrom(
            msg.sender,
            orders[_orderId].owner,
            _price.sub(adminFee).sub(creatorFee)
        );
    ...
    function bidOverTime(
    uint256 _orderId
    ,UserBid memory _userbid
    ,bool payAdminFee
    ,bool payCreatorFee
    )external onlyOperator
        returns ( uint256 )
    {
        address winner=_userbid.userAddress;
        uint256 finalPrice=_userbid.price;
        uint256 winnerBidId=_userbid.bidId;
        ...
        uint256 creatorFee;
        if (payAdminFee && _adminFee != 0) {
            adminFee = finalPrice.div(100).mul(_adminFee);
            TransferHelper.safeTransferFrom(
                orders[_orderId].currency,
                winner,
                _adminAddress,
        ...
        if (payCreatorFee && _creatorFee != 0) {
            creatorFee = finalPrice.div(100).mul(_creatorFee);
            TransferHelper.safeTransferFrom(
                orders[_orderId].currency,
                winner,
                _creatorOf[order.tokenAddress],
                creatorFee
            );
        }
        ...
        IERC20Upgradeable(orders[_orderId].currency).transferFrom(
            winner,
            order.owner,
            finalPrice.sub(adminFee).sub(creatorFee)
        );
