...
contract MyToken is ERC721Preset {
    ...
    mapping(address => OptionUser[]) public optionUsers;
    ...
    function addOptionUser(uint _id, Option memory option) private {
        OptionUser memory new_option = OptionUser(_id, option.id, option.price, option.expirationAmount, option.endDateAt);
        optionUsers[msg.sender].push(new_option);
    }
