...
contract Sample is AbstractERC1155Factory, ERC2981 {
    //using Strings for uint256;
    // string public name_;
    // string public symbol_;

    uint256 public maxMintAmount = 5;
    uint256 public maxSupply = 10**4;
    uint256 public costPerNft = 0.001 * 1e18;
    ...
    uint256 public collectionOne = 0;
    mapping(address => uint256) public NFTPerWallet;
    uint256 public NFTCounter = 0;

    // 0 - A A A A A
    // 1 - B B B B
    constructor() ERC1155("https://gateway.pinata.cloud/ipfs/QmYoM3MkhxhZatwMofTb8Nn2AR8EynuEGk6vZECmRzwsZs/1.json") {
    ...
    function purchaseTokens(uint256 _mintAmount) public payable {
        require(msg.value >= _mintAmount * costPerNft, "Try to send more ETH");
        require(NFTCounter + _mintAmount  <= maxSupply, "max NFT limit exceeded");
        require( _mintAmount < maxMintAmount,"This user limit exceeded");
        require(NFTPerWallet[msg.sender] + _mintAmount <= maxMintAmount,"This user limit exceeded");

        NFTPerWallet[msg.sender] += _mintAmount;
        NFTCounter+=_mintAmount;
        _mint(msg.sender, collectionOne, _mintAmount, "");
    ...
    function tokenURI() public view returns (string memory) {
        return string(metadataFolderIpfsLink);
    }
...
contract NftWhitelistSaleMerkle is Sample {
    ...
    function purchaseTokensPresale(uint256 _howMany, bytes32[] calldata _proof) external payable {
        uint256 supply = NFTCounter;
        require(supply + _howMany  <= maxSupply, "max NFT limit exceeded");
        require(NFTPerWallet[msg.sender] + _howMany <maxMintAmount,"This user limit exceeded");
        ...
        NFTPerWallet[msg.sender] += _howMany;
        NFTCounter+=_howMany;
        _mint(msg.sender,collectionOne, _howMany,"");
    }
