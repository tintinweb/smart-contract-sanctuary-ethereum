...
contract RedeemableCollection is
    BaseCollection,
    ...
    ERC721BurnableUpgradeable
{
    ...
    string private _collectionType = "redeemable";

    modifier onlyRedeemable(
        uint256 redeemableId,
        uint256 numberOfRedeemables,
        bytes calldata signature
    ) {
        Redeemable memory redeemable = _redeemables[redeemableId];
        require(redeemable.active, "Not valid: 1");
        require(
            redeemable.price.mul(numberOfRedeemables) <= msg.value,
            "Value incorrect"
        );
        require(
            _redeemedByWallet[redeemableId][_msgSender()].add(numberOfRedeemables) <=
                redeemable.maxPerWallet,
            "Exceeded max: 1"
        );
        require(
            redeemable.redeemedCount.add(numberOfRedeemables) <=
    ...
    function redeem(
        ...
        uint256 numberOfRedeemables,
        bytes calldata signature
    ) public payable onlyRedeemable(redeemableId, numberOfRedeemables, signature) {
        Redeemable memory redeemable = _redeemables[redeemableId];
        require(redeemable.merkleRoot == "", "Not valid: 3");

        _redeem(redeemableId, numberOfRedeemables);
    }
    ...
    function _redeem(uint256 redeemableId, uint256 numberOfRedeemables) internal {
        Redeemable memory redeemable = _redeemables[redeemableId];
        if (msg.value > 0) {
        ...
        _redeemedByWallet[redeemableId][_msgSender()] = _redeemedByWallet[
            redeemableId
        ][_msgSender()].add(numberOfRedeemables);

        for (uint256 i = 0; i < numberOfRedeemables; i++) {
            _mint(_msgSender(), redeemable.tokenURI);
        }

        emit TokenRedeemed(_msgSender(), redeemableId, numberOfRedeemables);
    }
