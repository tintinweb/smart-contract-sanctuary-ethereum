    ...
    TokenRescuer
{
    ...
    modifier onlyWithCorrectPayment(uint256 _price) {
        if (msg.value != _price) revert IncorrectPaymentAmount();
        _;
    }
    ...
    /**
     * @notice Mints a single token to the caller if the signature is valid.
    ...
    function mintPublic(
        ...
        onlyWithCorrectPayment(pricePublic)
    ...
    /**
     * @notice Mints internal `_tokenID` to `_to`, emits actual token ID.
     */
    function _mint(
        address _to,
        uint256 _tokenID
    ...
    /**
     * @notice Mints one token to the caller, emits actual token ID.
     */
    function _mintOne()
        internal
    {
