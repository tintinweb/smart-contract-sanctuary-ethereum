...
contract Eye is IEye, ERC721A, IERC2981, Ownable, ReentrancyGuard {
    ...
    uint16 private immutable _teamReserved;
    ...
    constructor(
        uint256 maxSupply_,
        uint16 teamReserved_,
        uint256 mintCost_
    ) ERC721A("Eye", "Eye") {
        _teamReserved = teamReserved_;
    ...
    function addStory(uint256 storyId, string calldata librariumId) external onlyOwner {
    ...
    function removeStory(uint256 storyId) external onlyOwner {
        stories[storyId] = "";
        for(uint i = 0; i < storyList.length; i++) {
    ...
    function mint()
        ...
        payable
        nonReentrant
        onlyIfPaymentAmountValid(mintCost)
    {
        _safeMint(msg.sender, 1);
    }
