...
contract DVDA is ERC721AQueryable, Ownable, ReentrancyGuard {    
    ...
    //WHITELIST MINT (FREE)
  function whitelistMint(uint256 amount, bytes32[] memory proof) external nonReentrant {
    ...
    require(amount > 0 && amount <= maxMintAmountPerAddress, "Invalid mint amount!");
    ...
    function PublicMint(uint256 amount) external nonReentrant { 
        ...
        require(_minted + amount <= MAX_SUPPLY, "Exceed max supply");
        require(amount > 0 && amount <= maxMintAmountPerAddress,"Must mint between the min and max.");
