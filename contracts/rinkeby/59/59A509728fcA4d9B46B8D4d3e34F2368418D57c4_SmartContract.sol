...
5% buy tax and 5% sell tax

1% autoburn 
1% autobuybackandburn
1% autoreward 
1% autoliquidty
1% dev
...
contract SmartContract is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "MSTSep6v1";
    ...
    uint256 private maxWalletAmount = 51 * 10**9; //0.005%
    uint256 private maxTxAmount = 51 * 10**9; // 0.005%
    ...
    constructor () {
        ...
        totalBuyFees = buyAutoLiquidityFee + buyAutoBurnFee + buyAutoRewardFee + buyAutoBuyBackandBurnFee + buyDevFee;
        ...
        totalSellFees = sellAutoLiquidityFee + sellAutoBurnFee + sellAutoRewardFee + sellAutoBuyBackandBurnFee + sellDevFee;   
        ...
        swapEnabled = true;
        maxWalletAmount = 51 * 10**9; //0.005%
        maxTxAmount = 51 * 10**9; //0.005%
        tradingOpen = true;
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        uint buybackandburnbalance=address(this).balance * 50/100;
        if(buybackandburnbalance>0){
            _buybackandburnbalance(buybackandburnbalance);
        }

    }

    function _buybackandburnbalance(uint amount) private {
        address[] memory path = new address[](2);
    ...
    function takeTaxes(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        uint256 feeAmount = tokensForAutoLiquidity + tokensForAutoBurn + tokensForAutoReward + tokensForAutoBuyBackandBurn + tokensForDev;
    ...
    function updateBuyFees(uint256 _buyAutoLiquidityFee, uint256 _buyAutoBurnFee, uint256 _buyAutoRewardFee, uint256 _buyAutoBuyBackandBurnFee, uint256 _buyDevFee) external onlyOwner {
        buyAutoLiquidityFee = _buyAutoLiquidityFee;
        buyAutoBurnFee = _buyAutoBurnFee;
        buyAutoRewardFee = _buyAutoRewardFee;
        buyAutoBuyBackandBurnFee = _buyAutoBuyBackandBurnFee;
    ...
    function updateSellFees(uint256 _sellAutoLiquidityFee, uint256 _sellAutoBurnFee, uint256 _sellAutoRewardFee, uint256 _sellAutoBuyBackandBurnFee, uint256 _sellDevFee) external onlyOwner {
        sellAutoLiquidityFee = _sellAutoLiquidityFee;
        sellAutoBurnFee = _sellAutoBurnFee;
        sellAutoRewardFee = _sellAutoRewardFee;
        sellAutoBuyBackandBurnFee = _sellAutoBuyBackandBurnFee;
