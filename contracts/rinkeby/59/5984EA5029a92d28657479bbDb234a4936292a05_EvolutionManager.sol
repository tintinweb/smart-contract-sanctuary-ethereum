...
contract EvolutionManager is Context, Ownable, IERC721Receiver {
    ...
    //////////////////
    function evolveMonkeys(Animal[] memory _monkeys) public payable {
        require(
            _monkeys.length == 4,
            "Exactly 4 monkeys required for evolution"
        );
        uint256 totalRarity;
        for (uint256 i; i < _monkeys.length; i++) {
            Animal memory _monkey = _monkeys[i];
            require(
                isOwnedBySender(_monkey.tokenId, _monkey.evolution, msg.sender),
                "monkey not owned by sender"
            );
            require(verifyRarity(_monkey), "rarity data submitted not correct");
            totalRarity += _monkey.C;
        }
        uint256 avgRarity = totalRarity / _monkeys.length;
        RarityGroup rg = roundRarity(avgRarity, 0);

        // require(msg.value >= monkeyEvoPrice, "invalid price");

        for (uint256 i; i < _monkeys.length; i++) {
            Animal memory _monkey = _monkeys[i];
            NFTs[_monkey.evolution].burn(_monkey.tokenId);
        }
        
        // Galactic Gorillas
        NFTs[1].mint(_msgSender(), uint(rg));
    }

      //////////////////
    function evolveGalacticGorrilas(Animal[] calldata _ggorillas) public payable {
        require(
            _ggorillas.length == 4,
        ...
        // Galactic Gorillas
        NFTs[2].mint(_msgSender(), uint(rg));
    }

          //////////////////
    function evolveAlienGorrilas(Animal[] calldata _ggorillas) external payable {
        require(
            _ggorillas.length == 2,
        ...
        // Eternal Yeti
        NFTs[3].mint(_msgSender(), uint(rg));
    }
