...
// File: contracts/MultiverseMetaLand.sol
...
contract MultiverseMetaLand is ERC721A, ERC721ABurnable, ERC2981, Ownable {
  using Counters for Counters.Counter;

  uint256 public MAX_MINTS = 10;
  uint256 public MAX_Parcel = 10000;

  Counters.Counter private supply1x1;
  Counters.Counter private supply2x2;
  Counters.Counter private supply3x3;
  ...
  string public baseURI = "ipfs://QmUfWksPBpzxbXdunw53DdseC11JDNQXJrBjEAdLVA8D3A/";
  string public contractURI = "ipfs://QmUfWksPBpzxbXdunw53DdseC11JDNQXJrBjEAdLVA8D3A/contract.json";

  // Calculated from `merkle_tree.js`
  bytes32 public merkleRoot = 0x4f843c8d23c8b94c53a40b0315743d22c1e06fa8a3b95bb549b8afe6bd8ada2c;

  //Events
  event Mint(
        uint256 indexed quantity,
        uint256 indexed landSize,
        uint256 indexed startingTokenId,
        address sender
  );
  event Burn(
  ...
  constructor() ERC721A("Multiverse Meta Land", "MML") {
  ...
  modifier mintCompliance(uint256 _quantity, uint256 _landSize) {
    ...
    require((supply1x1.current() + supply2x2.current() * 4 + supply3x3.current() * 9) <= MAX_Parcel, "Not enough parcels left");
  ...
  function mint(uint256 _quantity, uint256 _landSize) external payable mintCompliance(_quantity, _landSize){
    ...
    emit Mint(_quantity, _landSize, _currentIndex - _quantity, msg.sender);
  }
  ...
  function whitelistMint(bytes32[] calldata _merkleProof, uint256 _quantity, uint256 _mintRate) external payable mintCompliance(_quantity, _mintRate) {
    ...
    _safeMint(msg.sender, _quantity);
    emit Mint(_quantity, _mintRate, _currentIndex - _quantity, msg.sender);
  }
  ...
  function setMaxMint(uint256 _number) public onlyOwner {
    MAX_MINTS = _number;
  }
