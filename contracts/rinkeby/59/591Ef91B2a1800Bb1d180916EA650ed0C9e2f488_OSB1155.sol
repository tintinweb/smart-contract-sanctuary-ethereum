...
contract OSB1155 is ERC1155, Ownable {
    ...
    event SetController(address indexed oldAccount, address indexed newAccount);
    event RemoveController(address indexed oldAccount);

    modifier onlyOwnerOrController() {
        require(_msgSender() == owner() || _msgSender() == controller, "caller is not the owner or controller");
        _;
    }

    constructor(address _owner, string memory _name, string memory _symbol) ERC1155("") {
        name   = _name;
        symbol = _symbol;
    ...
    function setController(address _account) external onlyOwner {
        require(_account != address(0), "account is zero address");
        address oldController = controller;
        controller = _account;
        emit SetController(oldController, _account);
    }

    function removeController() external onlyOwner {
        require(controller != address(0), "the contract does not have controller");
        address oldController = controller;
        controller = address(0);
        emit RemoveController(oldController);
    }
}
...
contract $OSB1155 is OSB1155 {
    constructor(address _owner, string memory _name, string memory _symbol) OSB1155(_owner, _name, _symbol) {}
