...
contract RUSDFraction is ERC20Detailed, Ownable {
    ...
    uint256 public gaslessClaimPeriod= 86400; // Gasless Claim once every 1 day
    ...
    uint256 constant MAX_REWARD_POOL = 130;
    ...
    function setRewards(
        ...
        require(
            block.timestamp.u32() < rewardsPeriod[_rewardToken].start ||
                block.timestamp.u32() > rewardsPeriod[_rewardToken].end,
            "Ongoing rewards"
        );
        require(
        ...
        rewardsPeriod[_rewardToken].start = uint32(block.timestamp) + start;
        rewardsPeriod[_rewardToken].end = uint32(block.timestamp) + end;

        // If setting up a new rewards program, the rewardsPerToken.accumulated is used and built upon
        // New rewards start accumulating from the new rewards program start
        // Any unaccounted rewards from last program can still be added to the user rewards
        // Any unclaimed rewards can still be claimed
        rewardsTokenDetail[_rewardToken].lastUpdated =
    ...
    function addReward(
        ...
        require(
            rewardStatus[_rewardToken] != RewardStatus.APPROVED,
            "Already exists in reward pool"
        );
        require(msg.value > 0, "Must be greater than");
    ...
    function approveReward(IERC20 _rewardToken) public {
        require(
            rewardStatus[_rewardToken] == RewardStatus.PENDING,
            "token is either approved or rejected"
        );
        address tokenOwner = tempRewardDetails[_rewardToken].tokenOwner;

        // set reward period with rate
        rewardsPeriod[_rewardToken].start =
            uint32(block.timestamp) +
            tempRewardDetails[_rewardToken].start;
        rewardsPeriod[_rewardToken].end =
            uint32(block.timestamp) +
            tempRewardDetails[_rewardToken].end;
    ...
    function rejectToken() external onlyOwner {}
    ...
    function gaslessClaim(
        ...
        require(block.timestamp>=gaslessClaimTimestamp[_to].add(gaslessClaimPeriod), "Cannot reclaim before 1 day");
        require(verify(_to, _messageHash, signature), "signature is not matching");
        gaslessClaimTimestamp[_to]=block.timestamp;
        claim(_to, _rewardTokens);
    }

    /// @dev Claim all rewards from caller into a given address
    function claim(address _to, IERC20[] memory _rewardTokens)
        public
    {
        _updateRewardsPerToken();
        _updateUserRewards(msg.sender);
