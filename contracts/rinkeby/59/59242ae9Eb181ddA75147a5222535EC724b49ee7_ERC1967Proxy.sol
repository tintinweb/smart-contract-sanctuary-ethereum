    ...
    IPassageRegistry
{
    ...
    function upgradePassport(uint256 version, address passportAddress) external returns (uint256 newVersion) {
        ...
        require(passport.hasUpgraderRole(_msgSender()), "not upgrader role");

        uint256 currentVersion = passport.passportVersion();
        require(version == currentVersion + 1, "only +1 increment allowed");
        require(version <= passportLatestVersion(), "version not found");
    ...
    function upgradeLoyalty(uint256 version, address loyaltyAddress) external returns (uint256 newVersion) {
        ...
        require(ll.hasUpgraderRole(_msgSender()), "not upgrader role");
        uint256 currentVersion = ll.loyaltyLedgerVersion();

        require(version == currentVersion + 1, "only +1 increment allowed");
        require(version <= loyaltyLatestVersion(), "version not found");
...
import "../PassageRegistry.sol";

contract TestPassageRegistryUpgraded is PassageRegistry {
...
import "../LoyaltyLedger.sol";

contract TestLoyaltyLedgerUpgraded is LoyaltyLedger {
    function testAddedFunction() external pure returns (string memory) {
        return "passage!";
    }

    function loyaltyLedgerVersion() public pure override returns (uint256) {
        return 1;
    }
}