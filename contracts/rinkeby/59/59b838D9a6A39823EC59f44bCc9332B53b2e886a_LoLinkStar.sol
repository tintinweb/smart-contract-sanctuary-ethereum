...
contract LoLinkStar is ERC721A, Ownable, ReentrancyGuard {
  ...
  function updateNombre( uint _index, string calldata _nombre) public {
    require(ownerOf(_index) == _msgSender(), 'Not Own');
    Info storage info = infos[_index];
    info.nombre = _nombre;
  }
  function updateVoto(uint _index, uint256 _voto) public {
    require(ownerOf(_index) == _msgSender(), 'Not Own');
    Info storage info = infos[_index];
    info.voto = _voto;
  }
