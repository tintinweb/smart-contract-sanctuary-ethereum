...
contract SuperSpaceDefenders is ERC721A, Ownable {  
    ...
    uint256 public dynamicPreSaleLimit = 69;
    ...
    function mintPresale(uint256 _amount, bytes32[] calldata _merkleProof, bytes32[] calldata _earlyMerkleProof) public payable {
        ...
        if(earlySaleOver) {
            if(isWhiteListed) {
                ...
                // safty check to avoid underflow
    ...
    function withdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        require(payable(a1).send(percent * 80)); // 80% for MARCO
        require(payable(a2).send(percent * 20)); // 20% for NFT Forge
    }
}
