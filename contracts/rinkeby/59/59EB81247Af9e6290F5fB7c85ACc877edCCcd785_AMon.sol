...
contract AMon is ERC721A, Ownable {
    enum Status {
        Pending,
        EarlyBirdSale, 
        EarlyBirdSale2,
    ...
    uint256 public currentMintLimit = 0;
    ...
    uint256 public immutable earlybirdTotal = 3;
    uint256 public immutable earlybird2Total = 2;
    uint256 public immutable pioneerTotal = 2;
    ...
    event currentMintLimitChanged(uint256 currentMintLimit);
    ...
    function earlybirdMint(uint256 amount, bytes32[] calldata proof)
        ...
        callerIsUser
    {
        require(status == Status.EarlyBirdSale || status == Status.EarlyBirdSale2 || status == Status.PioneerSale, "EarlyBirdSale is not active.");
        require(
        ...
        require(
            totalSupply() + amount <= currentMintLimit,
    ...
    */
    ...
    function setStatus(Status _status) external onlyOwner {
        ...
        if (status == Status.EarlyBirdSale) {
            PRICE = 0;
            currentMintLimit = currentMintLimit + earlybirdTotal;
        } 
        else if (status == Status.EarlyBirdSale2) {
            currentMintLimit = currentMintLimit + earlybird2Total;
            PRICE = 0.01 ether; 
        }
        else if (status == Status.PioneerSale) {
            currentMintLimit = currentMintLimit + pioneerTotal;
            PRICE = 0.03 ether;
        }
        ...
        emit currentMintLimitChanged(currentMintLimit);
    }
