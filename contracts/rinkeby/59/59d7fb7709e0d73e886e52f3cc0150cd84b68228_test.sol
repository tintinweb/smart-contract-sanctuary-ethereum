...
contract test is ERC721A, Ownable {
    ...
    string public rufundTokenURI;
    string public baseTokenURI;
    address public withdrawAddress;
    bool lucky;

    uint256 public price = 0.01 * 10 ** 18;
    uint256 public price2 =0.02 * 10 ** 18;
    ...
    constructor(string memory _prizeTokenURI, string memory _rufundTokenURI, string memory _baseTokenURI)  
     ERC721A("test", "TT")  {
        prizeTokenURI = _prizeTokenURI;
        rufundTokenURI = _rufundTokenURI;
        baseTokenURI = _baseTokenURI;
    }


    function _getRandom() private returns(bool) {
        uint256 random = uint256(keccak256(abi.encodePacked(block.difficulty, block.number, block.gaslimit, block.timestamp)));
    ...
    function refund(uint256[] calldata tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(tokenId_luckys[tokenId],"Not lucky");
            hasRefunded[tokenId] = true;
        }
        uint256 refundAmount = tokenIds.length * price2;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
     require(_exists(tokenId), 'ERC721Metadata: URI query for nonexistent token');
      if(tokenId_luckys[tokenId]) {
            if (!hasRefunded[tokenId]) {return rufundTokenURI;} //已兑奖图片
            else {return prizeTokenURI;}} //中奖图片
      else {return baseTokenURI;}//未中奖图片
    }
