...
contract ZenGoblin is ERC721A, Ownable {
    ...
    uint256 public constant MAX_PER_ADDRESS_FREE = 3;
    uint256 public constant MAX_PER_ADDRESS = 10;
    uint256 public constant MAX_MEDITATE_TEAM = 50;
    uint256 public constant MAX_MEDITATE_FREE = 1000;
    uint256 public MAX_MEDITATE = 5000;
    uint256 public PRICE = 0.007 ether;

    bool public paused = false;
    bool public gogo = false;
    mapping(address => uint256) private _teamMinted;

    constructor() ERC721A("ZenGoblin", "ZENGOB") {}

    function subUnchecked() view private returns(uint256) {
        unchecked { return MAX_PER_ADDRESS_FREE - _numberMinted(msg.sender) ;}
    }
    ...
    function meditate(uint256 _amount) external payable {
        ...
        if(totalSupply() >= MAX_MEDITATE_FREE) {
            ...
            //free available
            uint256 freeCount = subUnchecked();
            if(freeCount < MAX_PER_ADDRESS_FREE) {
                require(msg.value >= PRICE * (_amount - freeCount), "Insufficient funds!");              
            } else {
                require(msg.value >= PRICE * _amount, "Insufficient funds!");
            }
        }
    ...
    function team(address _to, uint256 _amount) public onlyOwner {
        uint256 total = totalSupply();
        require(total + _amount <= MAX_MEDITATE, "Max supply exceeded!");
	    require(_teamMinted[msg.sender] + _amount <= MAX_MEDITATE_TEAM, "Max team mint exceeded!");
        _safeMint(_to, _amount);
        _teamMinted[msg.sender] += _amount;
    }
