...
contract StrikeBoostFarm is Ownable, ReentrancyGuard {
    ...
    function boostManyNfts(uint256 _pid, uint[] calldata _tokenIds) external nonReentrant {
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount + user.pendingAmount > 0, "no stake tokens");
        require(_tokenIds.length > 0, "no id take");
        require(user.boostFactors.length + _tokenIds.length <= maximumBoostCount);
        PoolInfo storage pool = poolInfo[_pid];
        if (user.boostFactors.length == 0) {
            boostedUsers[_pid].push(msg.sender);
        }
