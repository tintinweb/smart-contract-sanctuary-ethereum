...
contract XillionIDOMaster is XillionIDOCommon {
    ...
    function joinOrRecalculateAllocation() external whenNotPaused {
        ...
        for (uint256 i; i < stakesLength; i++) {
         ...
         StakingStatus stakingStatus_ = StakingStatus(_stakingContract.getUserStakeStatus(msg.sender));

         uint256 allocationTierMultiplier;

         if(stakingStatus_ == StakingStatus.None){
             allocationTierMultiplier = 100;
         }else if (stakingStatus_ == StakingStatus.Base){
            allocationTierMultiplier = 200;
         }else if (stakingStatus_ == StakingStatus.Advanced){
            allocationTierMultiplier = 300;
         }else if (stakingStatus_ == StakingStatus.Epic){
             allocationTierMultiplier = 400;
         }
        ...
        // This will be calculated on staking in future updates
        // uint256 allocationTierMultiplier = 100;
        ...
        if(BEP20IdoTokenAddress != address(0)){
            uint256 maxInvestmentBEP20AmountPerInvestorInChainCurrency = maxInvestmentBEP20AmountPerInvestor * xilToChainCurrencyAllocationSizeRatio / (10 ** getXilToChainCurrencyAllocationSizeRatioDecimals());
            
            uint256 minMaxInvestment = maxInvestmentAmountPerInvestor == 0 ? maxInvestmentBEP20AmountPerInvestorInChainCurrency : Math.min(maxInvestmentAmountPerInvestor, maxInvestmentBEP20AmountPerInvestorInChainCurrency);

            allocationSize = Math.min(allocationSize, minMaxInvestment);
        }else{
        ...
        if (allocationSize > _stakingAllocationSizeList[_msgSender()]) {
            ...
            uint256 currentlyAllocatedAmount = _stakingAllocationSizeList[_msgSender()];
            if (allocationSize > currentlyAllocatedAmount) {
                ...
                console.log("TotalAllocatedAmountUpdated", totalAllocatedAmount);
            }
    ...
    function invest() external payable whenNotPaused {
        ...
        // console.log("idoValueCap", idoValueCap);
        totalInvestedAmount += newInvestment;
        //  console.log("totalINvest", totalInvestedAmount);
    ...
    function investBEP20(uint256 _tokenAmount) external whenNotPaused {
        ...
        uint256 allocationSize = _stakingAllocationSizeList[_msgSender()];
    ...
    function finish() external nonReentrant {
        ...
        uint256 totalInvestedAmountOfIDOInChainCurrency = totalInvestedAmount;
        uint256 totalIdoValueCapInChainCurrency = idoValueCap;
        bool isSuccessfullyFunded = true;
        
            // check if we reached the success threshold for ERC-20 invested tokens
        if (BEP20IdoTokenAddress != address(0) && BEP20IdoValueCap > 0) {
                console.log("inBEP20Investment");
                if (totalInvestedAmountInBEP20 < BEP20IdoValueCap * minInvestmentPercentOfBEP20ForIDOCompletion / 100) {
                    ...
                    if (BEP20IdoTokenAddress == xillTokenAddress) {
                        tokenInvestendInChainCurrency = xilToChainCurrencyAllocationSizeRatio * totalInvestedAmountInBEP20 / 10 ** getXilToChainCurrencyAllocationSizeRatioDecimals();
                        tokenIdoValueCapInChainCurrency = xilToChainCurrencyAllocationSizeRatio * BEP20IdoValueCap / 10 ** getXilToChainCurrencyAllocationSizeRatioDecimals();
                    } else {
                        tokenInvestendInChainCurrency = busdToChainCurrencyAllocationSizeRatio * totalInvestedAmountInBEP20 / 10 ** getXilToChainCurrencyAllocationSizeRatioDecimals();
                        tokenIdoValueCapInChainCurrency = busdToChainCurrencyAllocationSizeRatio * BEP20IdoValueCap / 10 ** getXilToChainCurrencyAllocationSizeRatioDecimals();
                    }
                    totalInvestedAmountOfIDOInChainCurrency += tokenInvestendInChainCurrency;
                    totalIdoValueCapInChainCurrency += tokenIdoValueCapInChainCurrency;
                }
                require(isSuccessfullyFunded, "Not all ERC20 tokens reached success threshold");
        }

        // Check if we reached the success threshold of chain currency
        if(idoValueCap > 0){
            
            isSuccessfullyFunded = totalInvestedAmount >= idoValueCap * minInvestmentPercentForIDOCompletion / 100;
            console.log("inBNBInvestment");
        }

        console.log("isSuccessAfterBEP20ANDBNB", isSuccessfullyFunded);
        ...
        if (isSuccessfullyFunded) {
            ...
            uint256 finalInvestorsInBEP20Amount = Math.min(_invesotrsOfBEP20.length, MAX_INVESTORS);

            for (uint256 i; i < finalInvestorsAmount; i++) {

                // calculate amount & release date for investor
                tokensToDistribute = investorTokens * _investments[_investors[i]] / totalInvestedAmountOfIDOInChainCurrency;
            ...
            for (uint256 i; i < finalInvestorsInBEP20Amount; i++) {

                uint256 tokenToChainCurrency;
                if(BEP20IdoTokenAddress == xillTokenAddress){
                        tokenToChainCurrency = factory.xilToChainCurrencyAllocationSizeRatio();
                }else{
                        tokenToChainCurrency = factory.busdToChainCurrencyAllocationSizeRatio();
                }
                
                uint256 investmentsPerWalletInChainCurrency = _investmentsInBEP20[_invesotrsOfBEP20[i]] * tokenToChainCurrency / 10 ** getXilToChainCurrencyAllocationSizeRatioDecimals();

                // calculate amount & release date for investor
                uint256 tokensToDistributeBEP20 = investorTokens * investmentsPerWalletInChainCurrency / totalInvestedAmountOfIDOInChainCurrency;
                // add shares for investor
                poolTokenShares[_invesotrsOfBEP20[i]] += tokensToDistributeBEP20;
                // increment total shares
                totalShares += tokensToDistribute;
    ...
    function claimPoolTokens() external nonReentrant {
        ...
        // console.log("amountVested",amountVested);
        // console.log("amountVestedSender",_msgSender());
        ...
        uint256 newTotalReleased = totalReleased + amountVested;
        totalReleased = newTotalReleased;

        uint256 idoCurrencyBalance = address(this).balance;
        uint256 idoPoolTokenBalance = _poolToken.balanceOf(address(this));

        // Define BEP20Token info if there is BEP20 currency in IDO
        uint256 idoBEP20Balance;
        if(BEP20IdoTokenAddress != address(0) && BEP20IdoValueCap > 0){
           IERC20 BEP20Token = IERC20(BEP20IdoTokenAddress);
           idoBEP20Balance = BEP20Token.balanceOf(address(this));
        }

        if(idoCurrencyBalance == 0 && idoBEP20Balance == 0  && idoPoolTokenBalance == 0){
            _state = IDOState.Closed;
            emit IDOStateUpdated(IDOState.Closed);
        }
    ...
    function claimRefundedInvestment() external nonReentrant {
        ...
        uint256 idoCurrencyBalance;

        uint256 idoBEP20Balance;

        if (amount > 0) {
            idoCurrencyBalance = address(this).balance - amount;
        ...
        if (tokenAmount > 0) {
            // refund BEP20 tokens
            IERC20 BEP20Token = IERC20(BEP20IdoTokenAddress);
            idoBEP20Balance = BEP20Token.balanceOf(address(this)) - tokenAmount;
            BEP20Token.transfer(_msgSender(), tokenAmount);
        ...
        if(idoCurrencyBalance == 0 && idoBEP20Balance == 0){
            _state = IDOState.Closed;
            emit IDOStateUpdated(IDOState.Closed);
        }
    ...
    function claimTotalInvestedAmount() external nonReentrant {
        ...
        require(_msgSender() == _curatorWalletAddress, "Invalid address");

        uint256 idoPoolTokenBalance = _poolToken.balanceOf(address(this));

         if(idoPoolTokenBalance == 0){
            _state = IDOState.Closed;
            emit IDOStateUpdated(IDOState.Closed);
        }

        Address.sendValue(_curatorWalletAddress, totalInvestedAmount);

        // loop through all ERC20 invested tokens and transfer it to Chain Currency recipient
        IERC20 tokenInstance = IERC20(BEP20IdoTokenAddress);
        tokenInstance.transfer(_curatorWalletAddress, totalInvestedAmountInBEP20);
     ...
     */
    function setPoolTokenSharePercentages(XillionIDOStructs.PoolTokenSharePercentage calldata poolTokenSharePercentages_) external {
        onlyModeroratorOrFactory(msg.sender);
     ...
     */
    function setWalletAddresses(XillionIDOStructs.WalletAddressV2 calldata walletAddresses_) external {
        onlyModeroratorOrFactory(msg.sender);
        onlyBeforeSwapStartDate();
        ...
        _curatorWalletAddress = payable(walletAddresses_.curator);
     ...
     */
    function setCuratorAddress(address walletAddress_) external {
        onlyModerator(msg.sender);
        onlyBeforeSwapStartDate();

        require(walletAddress_ != address(0), "Invalid address");

        _curatorWalletAddress = payable(walletAddress_);
     ...
     */
    function setAllocationStartDate(uint256 allocationStartDate_) external {
        onlyModeroratorOrFactory(msg.sender);
        notInClaimState();
     ...
     */
    function setAllocationEndDate(uint256 allocationEndDate_) external {
        onlyModeroratorOrFactory(msg.sender);
        notInClaimState();
     ...
     */
    function setSwapStartDate(uint256 swapStartDate_) external {
        onlyModeroratorOrFactory(msg.sender);
     ...
     */
    function setSwapEndDate(uint256 swapEndDate_) external {
        onlyModeroratorOrFactory(msg.sender);
        notInClaimState();
     ...
     */
    function setInvestmentDetails(XillionIDOStructs.InvestmentDetails calldata investmentDetails_) external {
        onlyModeroratorOrFactory(msg.sender);
     ...
     /**
     * @notice Sets the investment details
     * @param investmentBEP20Details_ New investment details
     */
    function setInvestmentBEP20Details(XillionIDOStructs.BEP20IDOTokenDetails calldata investmentBEP20Details_) external{
        onlyModerator(msg.sender);
        require(investmentBEP20Details_.minInvestmentPercentOfBEP20ForIDOCompletion <= 100,
            "Invalid percentages");

        maxInvestmentBEP20AmountPerInvestor = investmentBEP20Details_.maxInvestmentBEP20AmountPerInvestor;
        minInvestmentPercentOfBEP20ForIDOCompletion = investmentBEP20Details_.minInvestmentPercentOfBEP20ForIDOCompletion;

        BEP20IdoTokenAddress = investmentBEP20Details_.BEP20IdoTokenAddress;
        BEP20IdoValueCap = investmentBEP20Details_.BEP20IdoValueCap;


        emit InvestmentBEP20DetailsUpdated(
            investmentBEP20Details_.maxInvestmentBEP20AmountPerInvestor,
            investmentBEP20Details_.BEP20IdoTokenAddress,
            investmentBEP20Details_.BEP20IdoValueCap,
            investmentBEP20Details_.minInvestmentPercentOfBEP20ForIDOCompletion
        );
    }
    /**
     ...
     * @param userAddress_ Address of the manually whitelisted account
     */
    function onlyModeroratorOrFactory(address userAddress_) internal returns(bool){
        require(factory.checkIsUserModerator(userAddress_) || userAddress_ == address(factory), "Caller is not available to edit");
    }
