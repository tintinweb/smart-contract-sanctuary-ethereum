    ...
    ReentrancyGuard
{
    ...
    mapping(uint256 => bool) internal tokenMinted;
    ...
    function setInventoryTypeConfig(
        ...
        require(currentIndex < MAX_TOKEN_SUPPLY, "All items have been minted");
    ...
    function submitOrder(string memory orderId, string memory inventoryType)
        public
        payable
        nonReentrant
    {
        ...
        address nextMinter = _selectNextMinter();
        (bool sent, bytes memory data) = payable(nextMinter).call{
            value: msg.value
        }("");
        require(sent, "Payment failed");

        emit OrderSubmitted(orderId, msg.sender, msg.value, nextMinter, data);
    }

    function _selectNextMinter() internal view returns (address) {
        address nextMinter;
        ...
        for (uint256 index = 0; index < minterAddresses.length; index++) {
            ...
            if (index == 0) {
                ...
                lowestMintCount = addressMintCount[nextMinter];
            } else if (
                addressMintCount[minterAddresses[index]] < lowestMintCount
            ) {
                nextMinter = tempAddress;
                lowestMintCount = addressMintCount[nextMinter];
            }
    ...
    function mintBatch(
        ...
        require(
            ...
            "Not enough tokens left to mint batch"
        );
        require(
            inventoryTypeConfig[inventoryType].isActive,
            "Inventory type is not active"
        ...
        require(
            ...
            "Not enough left for this inventory type to mint batch"
        );
        ...
        for (index = 0; index < tokenIds.length; index++) {
            ...
            require(
                ...
                "Token id is not part of this inventory"
            );
            require(
            ...
            require(!tokenMinted[tokenId], "Token id is already minted");

            _safeMint(buyer, tokenId);
            indexTokenMap[updatedIndex] = tokenId;
    ...
    function freezeTokenURI(uint256 id) public onlyRole(DEFAULT_ADMIN_ROLE) {
        emit PermanentURI(baseUri, id);
    }

    function freezeTokenURIBatch(uint256[] memory ids)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
