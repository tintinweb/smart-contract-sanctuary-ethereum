...
contract PhantomAirdropERC721A is Initializable, ERC721AUpgradeable, OwnableUpgradeable {
    ...
    uint256 private constant BITPOS_AUX_HAS_WRITTEN = 1 << 255;
    ...
    function getLocalStorageBalance(address owner) public view virtual returns (uint256) {
        return ERC721AStorage.layout()._packedAddressData[owner] & BITMASK_ADDRESS_DATA_ENTRY;
    }

    function balanceOf(address owner) public view virtual override(ERC721AUpgradeable) returns (uint256) {
        uint256 balance = 0;
        for (uint256 i = _startTokenId(); i <= ERC721AStorage.layout()._currentIndex; i++) {
            address q = address(uint160(_packedOwnershipOf(i)));
            if (q != address(0)) {
    ...
    function ownerOf(uint256 tokenId) public view virtual override(ERC721AUpgradeable) returns (address) {
        require(
            ...
            "ERC721: approved query for nonexistent token1"
        );
    ...
    function transferFrom(
        ...
        //console.log("prevOwnership:", prevOwnership);

        uint256 prevOwnershipPacked = uint256(uint160(ownerOf(tokenId)));
        //console.log("prevOwnershipPacked:", prevOwnership);
        //console.log("result", address(uint160(prevOwnershipPacked)));
        ...
        unchecked {
            ...
            //console.log("Old Token ID Address", address(uint160(ERC721AStorage.layout()._packedOwnerships[tokenId])));
     ...
     */
    function getApproved(uint256 tokenId) public view override(ERC721AUpgradeable) returns (address) {
     ...
     */
    function approve(address to, uint256 tokenId) public override(ERC721AUpgradeable) {
