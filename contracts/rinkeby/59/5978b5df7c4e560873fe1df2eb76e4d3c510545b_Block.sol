...
contract Block is ERC721, Ownable{
    ...
    constructor(address _Directpurchase,address _plateformFee,address _admin,string[] memory countryName,uint256[] memory _Totalblock) ERC721("AOJUS", "OJS"){
        ...
        usd = usd + 100;
        aafterchangerate = aafterchangerate + 30 ;
    ...
    function plentblockMint(int256 x,int256 y,string memory _countryname) private {
        ...
        _mint(_msgSender(), id);
        emit onMint(id, x, y, msg.sender,usd,usd);
    ...
    function multibuyparsel(uint256 _nonce ,bytes memory signature,int256[] memory _x,int256[] memory _y,string memory _countryname)public payable returns(bool) {
        ...
        payable(Directpurchase).transfer(msg.value);
        emit mainevent(msg.sender,msg.value);
