...
interface ICycleVault {
    ...
    function getETHquoteForLPamount(uint256 amountLP)
        external
        view
        returns (uint256);
}
...
contract DistributorV5 is Ownable {
    ...
    address[] public swapPath = [CYCLE, WETH];
    ...
    function addRewardData(address StakingRewards) external onlyOwner {
        ...
        for (uint256 i; i < rewards.length; i++) {
            require(
                rewards[i].StakingRewards != StakingRewards,
                "Distributor: Reward contract has already been added"
            );
        }
    ...
    function updateRewardWeight(address StakingRewards, uint256 weight)
        external
        onlyOwner
    {
        for (uint256 i; i < rewards.length; i++) {
     ...
     */
    function setCoreRewardsBasisPoints(uint256 _coreRewardsBasisPoints)
        external
        onlyOwner
    {
        require(
            _coreRewardsBasisPoints < BP_DIVISOR,
            "DistributorV5: Basis Points out of bounds"
        );
        coreRewardsBasisPoints = _coreRewardsBasisPoints;
        emit CoreRewardsBasisPointsUpdated(coreRewardsBasisPoints);
    }
     ...
     */
    function getRewardWeight(address StakingRewards)
        external
        view
        returns (uint256)
    {
    ...
    function getRewardIndex(address StakingRewards)
        external
        view
        returns (uint256)
    {
    ...
    function getCoreRewardsTVL() public view returns (uint256) {
        ...
        (uint256 reservesWETH, ) = PangolinLibrary.getReserves(
            Factory,
            WETH,
            CYCLE
        );
        uint256 amountETH = reservesWETH.mul(amountCoreLPstaked).div(
            totalSupplyCoreLP
        );
        // ETH + CYCLE
        return amountETH.mul(2);
    }
    ...
    function getVaultTVL(address StakingRewards) public view returns (uint256) {
        ...
        return ICycleVault(Vault).getETHquoteForLPamount(totalLPinVault);
    }

    /**
     * @dev Call fee kickback logic, sending {distributionCost} in ETH to caller
     */
    function _processKickback(address caller) internal {
        (uint256 reservesWETH, uint256 reservesCYCLE) = PangolinLibrary
            .getReserves(Factory, WETH, CYCLE);
        uint256 amountCYCLEtoSwap = PangolinLibrary.getAmountIn(
            distributionCost,
            reservesCYCLE,
            reservesWETH
        );
        ...
        (bool success, ) = caller.call{value: balanceWETH}("");
        require(success, "DistributorV5: Unable to transfer ETH");
    }

    /**
     * @dev Distribution function
     *
     * Normalizes output based on correlated ETH TVL of Vaults, applying custom weighting
    ...
    function distribute(address caller) external onlyProxy {
        ...
        require(
            ...
            "DistributorV5: No CYCLE to distribute"
        );
        ...
        if (processorBalance < emissionAmount) {
            IERC20(CYCLE).safeTransfer(
                Processor,
                emissionAmount.sub(processorBalance)
            );
        }
        ...
        uint256 amountForCoreRewards = amountToDistribute
            .mul(coreRewardsBasisPoints)
            .div(BP_DIVISOR);
        ...
        NormalizedRewardData[]
            memory normalizedRewards = new NormalizedRewardData[](
                rewards.length
            );
        ...
        for (uint256 i; i < normalizedRewards.length; i++) {
            ...
            if (i == normalizedRewards.length - 1) {
                ...
                amountToSend = amountToDistribute
                    .mul(normalizedRewards[i].normalizedTVL)
                    .div(totalNormalizedTVL);
            }
