...
abstract contract TokenVotes is
    Initializable,
    ERC721Upgradeable,
    VotesUpgradeable
{
    ...
    TokenVotes
{
    ...
    function __Token_init(
        ...
        OwnableUpgradeable.transferOwnership(address(this));
    ...
    function burn(uint256 tokenId) public {
        require(
            ...
            "ERC721Upgradeable: caller is not owner nor approved"
        );
        _burn(tokenId);
    }
    ...
    function _afterTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal override(ERC721Upgradeable, TokenVotes) {
        super._afterTokenTransfer(from, to, tokenId);
    }
...
contract Factory is Context, ERC165, AccessControlEnumerable {
  ...
  constructor() {
    ...
    implementationAddress = address(new Token());
  ...
  function deploy721Contract( string calldata _name, string calldata _symbol, string calldata _baseURI, uint256 _mintPrice, uint256 _maxSupply, uint256 _launchDate, address _payout) external returns (address) {
    ...
    BeaconProxy proxy = new BeaconProxy(
      ...
      abi.encodeWithSelector(
        Token(address(0x0)).initialize.selector,
        _name,
        _symbol,
        _baseURI,
        _mintPrice,
    ...
    Token(address(proxy)).transferOwnership(msg.sender);
