...
Ownable
{
  ...
  uint256 public maxTokensAddress = 10;
  ...
  //TODO settare new merkle
  bytes32 public merkleRoot = 0x7f9947af1470e7df017d480516fb72e1b515240c75cf5afacfd79d475f309f35;

  constructor(
    string memory _baseUri
  ) ERC721("Kromin NFT", "KRMN") {
    baseUri = _baseUri;
  }
  ...
  function setMaxTokensAddress(uint256 _maxTokensAddress) external onlyOwner {
    maxTokensAddress = _maxTokensAddress;
  }

  function _mintConsecutive(uint256 nTokens, address to) internal {
    require(tokensCounter.current() + nTokens <= maxSupply, "Not enough Tokens left.");
    claimToken(nTokens);
  ...
  modifier whenisActive() {
    require(_isActive(), "Minting not started");
    _;
  }

  modifier onlyWhitelisted(bytes32[] calldata proof) {
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(proof, merkleRoot, leaf), "Your address is not whitelisted");
    _;
  }

  function mint(uint256 nTokens) external payable nonReentrant whenisActive {
    require(nTokens <= maxTokensTx, "You cannot mint more than maxTokensTx tokens at once!");
    require(nTokens * price <= msg.value, "Inconsistent amount sent!");

    _mintConsecutive(nTokens, msg.sender);
  }

  function mintWhitelist(bytes32[] calldata proof) external onlyWhitelisted(proof) nonReentrant {
    _mintConsecutive(1, msg.sender);
  }
  ...
  function claimToken(uint256 count) internal {
    uint256 claimed = addressTokens[msg.sender];
    require(claimed + count <= maxTokensAddress, "Can't mint more than allocated");
    addressTokens[msg.sender] = claimed + count;
  }

  function _isActive() public view returns (bool) {
    return isActive;
  }
  ...
  function isApprovedForAll(address owner, address operator) public view override returns (bool){
    ...
    //TODO: controllare che questo sia effettivamente il proxy di opensea
