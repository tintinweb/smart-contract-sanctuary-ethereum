...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // HELLO1.json
    // link/1.json
    // https:/1.json - {imageUrl: "sample image link"}
...
contract TimeFrogs is
    ERC721Enumerable,
    Ownable,
    Multisig(msg.sender), // makes the ownder the signatory 
    ERC2981PerTokenRoyalties
{
    ...
    uint256 public lotteryIntervalDays = 1;
    uint256 private epochDay = 0;
    uint256 public cost = 0.0001 ether; // bruh? 
  ...
  */
    ...
    */
    function selectWinners(uint256 noOfWinners) public payable onlySignatories {
        ...
        uint256 epochNow = block.timestamp; // time right now
        uint256 nextLotteryDate = lotteryDates[lotteryDates.length - 1];

        require(
            epochNow >= nextLotteryDate,
            "ERROR: Cannot draw yet, too early"
        ); // if the time now is greater than the next lottery date then we can draw a winner
    ...
    */
    function selectAWinner(
        uint256 it, // iterator value = 0 
        uint256 epochNow, // what is this
        address sender,
        uint256 lotteryDate, // heh
        address randomAddr, // sends the sender's address 
        uint256 randomNo // 0 
    ) internal {
        ...
        // iteration less than 5 and has already won -> goes for a new guess
        // else more than 5 and has already won -> returns empty -- what are the chances we are hitting here
        // else ( less than 5 and hasn't won yet -> winner selection )


        if ((it < 5) && alreadyWon) { //4 and true -> Go again
            uint256 newIt = it + 1;
            return
                selectAWinner(
                    newIt,
    ...
    */
    function castVote(string memory option) public {
        ...
        /*
            if I vote on one poll, it won't let me vote on other polls. 
            try this change in the future.
        */ 

        // require((votedAddresses[msg.sender] != pollName), "Error: You have already voted in this poll!");
        require(
        ...
        uint256 noVotes = balanceOf(msg.sender); // checks if the voter has minted an NFT

        require(
            noVotes > 0,
            "ERROR: You have no voting rights. Get yourself a frog my fren!"
        );
        votes[option] += noVotes; // if you own themax amount of token, you decide the voting polls. 
