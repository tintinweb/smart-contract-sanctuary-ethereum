...
contract Universal is ONFT1155 {
    
    string public name = "Universal";
    string public symbol = "UN";
    ...
    constructor(address _layerZeroEndpoint, address _oldContract, address royaltyAddress, uint256 royaltyPercentage) ONFT1155("test", _layerZeroEndpoint) {
    ...
    function remint(uint256[] memory tokenIds) public {
        uint256[] memory amounts = new uint256[](tokenIds.length);
        unchecked {
            for(uint256 i; i < tokenIds.length; i++) {
                uint256 currentId = tokenIds[i];
                require(oldContract.ownerOf(currentId) == msg.sender);
                amounts[i] = 1;
...
interface IONFT1155 {
    ...
    function estimateSendBatchFee(uint16 _dstChainId, uint[] memory _tokenIds) external view returns (uint nativeFee, uint zroFee);
}
...
contract ONFT1155 is IONFT1155, NonblockingLzApp, ERC1155 {
    ...
    function estimateSendBatchFee(
        ...
        uint16 version = 1;
        uint256 gas = 50000 + (55000 * _tokenIds.length);
        bytes memory _adapterParams = abi.encodePacked(version, gas);
