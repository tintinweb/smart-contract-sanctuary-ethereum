...
contract LittleExplorer is Ownable, ERC721A {
    ...
    uint16 public reservedMintAmount = 100;
    uint16 public presaleMintAmount = 6000;
    uint256 public price = 0.050 ether;
    uint16 public supply = 8000;

    constructor(
        bytes32 _presaleMerkleRoot, 
        uint256 _presaleTime,
        uint256 _publicTime
    ) ERC721A("Little Explorer", "LER", supply) {
    ...
    function presaleMint(bytes32[] calldata merkleProof, uint16 numToMint, uint16 mintCap) 
    payable public 
    isMintValid(numToMint, mintCap) 
    isPriceValid(numToMint) 
    isMintLive(presaleTime) 
    {
        require(totalSupply() + numToMint < presaleMintAmount + 1, "Sold Out");
    ...
    function devMint(
        address to,
        uint16 numToMint
    ) payable public onlyOwner isMintValid(numToMint, reservedMintAmount) {
