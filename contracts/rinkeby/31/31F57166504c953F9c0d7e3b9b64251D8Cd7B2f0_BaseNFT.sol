...
contract BaseNFT is ERC721A , Ownable, PaymentSplitter, Pausable,  ReentrancyGuard {
    ...
    uint256 public maxMint = 9000;
    bool public presaleActive = false;
    bool public publicSaleActive = false;

    //presale settings
    //TODO: replace with merkle root whitelist system
    mapping(address => uint256) public presaleWhitelist;

    //share settings
    address[] private addressList = [
    //founders wallet
    0xE12Ab4ced7d147fe6cdD02C5bEbcbdf6D84694B1,
    //community wallet
    0x5c0404F8bBdB8c143580D5E910921E572Cbc9ce8
    ];

    uint[] private shareList = [90, 10];

    constructor()ERC721A("BaseNFT", "BNFT") PaymentSplitter(addressList, shareList){
    }
    ...
    function mint(uint256 _mintAmount) public payable nonReentrant{
        ...
        require(totalSupply() + _mintAmount <= maxSupply, "Cant go over supply" );
    ...
    function gift(uint256[] calldata _mintAmount, address[] calldata recipient) external onlyOwner{
        require(_mintAmount.length == recipient.length, "Provide equal mintAmount and recipients" );
        for(uint i = 0; i < recipient.length; ++i){
            require(totalSupply() + _mintAmount[i] <= maxSupply, "Cant go over supply");
