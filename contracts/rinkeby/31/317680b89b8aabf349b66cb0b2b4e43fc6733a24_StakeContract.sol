   ...
   function withdrawInAmd(uint256 count) public {
    ...
    function extendStake(uint256 count,uint256 newplan) public {
       ...
       require(user.stakerecord[count].plan < newplan, "Can not extend to lower plan");
        ...
        calculateNewReward(msg.sender,user.stakerecord[count].amount, newDays, user.stakerecord[count].plan);
        ...
        user.stakerecord[count].bonus = (user.stakerecord[count].amount.mul(percentages[newplan]).div(percentDivider) + user.stakerecord[count].beforeExtendBonus);
    ...
    function calculateNewReward(address userAddress, uint256 amount, uint256 _newDuration, uint256 _oldPlan) public {
           ...
           User storage user = users[userAddress];
            ...
            user.stakerecord[user.stakecount].beforeExtendBonus = amount.mul(newPercentage).div(percentDivider);
    }
