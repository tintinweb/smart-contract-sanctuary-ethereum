...
contract GutterArt is ERC1155Burnable, Ownable, ReentrancyGuard {
	...
	constructor(address _signerAddress, address _registry) ERC1155(_baseTokenURI) {
		minter = msg.sender;
    ...
    function mint(
		...
		require(allowedIds[nftID], "id is not allowed to mint");
		require(matchAddresSigner(hash, signature), "wrong signer");
		require(hashMint(msg.sender, to, nftID, quantity) == hash, "wrong hash");
		require(to != address(0), "mint to 0 address");
		require(
			to == msg.sender || registry.isPairValid(to, msg.sender), 
			"can't mint to this address"
		);
	...
	function mintWithGang(
		...
		gangToken.safeTransferFrom(msg.sender, address(this), price);
    ...
    function auctionMint(
        ...
        require(dutchAuctions[nftID].currency == 0, "wrong currency");
    ...
    function auctionMintGang(
        ...
        require(dutchAuctions[nftID].startPrice != 0, "auction doesn't exist");
        require(dutchAuctions[nftID].currency == 1, "wrong currency");
        require(block.timestamp >= dutchAuctions[nftID].startDate, "not started yet");

        uint256 costToMint = getAuctionPrice(nftID) * quantity;
		gangToken.safeTransferFrom(msg.sender, address(this), costToMint);
		userHasMinted[nftID][to] = true;
	...
	function hashMint(
		...
		return ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(sender, to, id, qty)));
	}
