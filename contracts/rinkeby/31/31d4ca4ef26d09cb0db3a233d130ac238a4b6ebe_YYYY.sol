...
contract YYYY is Ownable, ERC721I, Withdraw, Security {
	...
	constructor() ERC721I("Y Y Y Y", "YYYY") {
	...
	function freeMint(uint256 quantity) external onlySender mintConstraints(quantity) {
		require(totalSupply < maxFreeMints, "No more free mints");
		_mintLoop(msg.sender, quantity);
	}

	function mint(uint256 quantity) external payable onlySender mintConstraints(quantity) {
		require(msg.value >= cost * quantity, "Not correct ether");
		_mintLoop(msg.sender, quantity);
	}
