...
contract PrimaryLendingPlatformProxyAdmin is ProxyAdmin {
    uint256 public constant minimumDelayPeriod = 0.01 days;
    ...
    constructor() {
        delayPeriod = 0.01 days;
    }

    function setDelayPeriod(uint256 _delayPeriod) public onlyOwner {
        require(
            minimumDelayPeriod <= _delayPeriod,
            "PrimaryLendingPlatformProxyAdmin: too low delayPeriod"
        );
        ...
        onlyOwner
    {
        ...
        if (_upgrade.appendTimestamp == 0) {
            revert(
                "PrimaryLendingPlatformProxyAdmin: Call first appendUpgrade(...)"
            );
        }
        require(
            _upgrade.appendTimestamp + _upgrade.delayPeriod <= block.timestamp
        );
