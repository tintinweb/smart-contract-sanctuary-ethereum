...
contract OnChainMachines is Reflect {
    ...
    // Mint price without whitelist
    uint256 public tokenPriceSale;

    // Mint price with whitelist
    ...
    // Status of presale
    bool public tokenPresale;

    // Withdrawable amount of minting revenue
    uint256 public tokenRevenue;

    // Resource value for each token, token => Resource struct
    mapping (uint256 => Resource) public tokenResources;

    // Whitelisted amout for each address, address => amount
    mapping(address => uint16) public tokenWhitelist;

    /**
     * Set variables to contract
     */
    constructor(address metadata, uint256 priceSale, uint256 pricePresale, uint16 supply, uint16 maximum) {
        tokenMetadata = OnChainMachinesMetadata(metadata);
        tokenPriceSale = priceSale;
        tokenPricePresale = pricePresale;
        tokenSupply = supply;
    ...
    /**
     * Set sale of contract
     */   
    function setTokenSale(bool status) external onlyOwner {
        tokenSale = status;
    }

    /**
     * Set presale of contract
     */   
    function setTokenPresale(bool status) external onlyOwner {
        tokenPresale = status;
    }
     ...
     */  
    function setTokenWhitelist(address[] calldata addresses, uint16 amount) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            tokenWhitelist[addresses[i]] = amount;
        }
    }

    /**
     * Call withdraw balance of contract
     */   
    function callWithdraw() external onlyOwner {
        payable(owner()).transfer(tokenRevenue);
    ...
    function mintToken(uint16 amount) external payable {
        require(tokenSale == true, "token.sale");
        require(tokenMaximum >= amount, "token.maximum");
        require(tokenSupply >= totalSupply() + amount, "token.supply");

        require(tokenPresale == false || tokenWhitelist[msg.sender] >= amount, "token.presale");
        require(tokenPresale == true && tokenPricePresale * amount <= msg.value || tokenPriceSale * amount <= msg.value, "token.price");

        if (tokenWhitelist[msg.sender] >= amount) tokenWhitelist[msg.sender] -= amount;
