    ...
    Signature
{
    ...
    function stableFinanceUserPendingWithdrawalsData(
        uint256 _poolId,
        address _account
    ) external view returns (StableFinancePendingWithdrawal memory) {
    ...
    function stableFinanceAddEarlyWhitelist(
        uint256 _poolId,
        ...
        notInBlackList(msg.sender)
    {
        ...
        uint256[] memory reward = new uint256[](
            stakingData.pendingReward.length
        );
        ...
        if (totalReward.length > 0) {
            for (uint256 i; i < totalReward.length; i++) {
                ...
                reward[i] = (percentage * totalReward[i]) / HUNDRED_PERCENT;
                stakingData.pendingReward[i] = totalReward[i] - reward[i];
                pending.reward[i] += reward[i];
            }
        }
        ...
        pool.stableFinanceAcceptedToken.safeTransfer(
    ...
    function stableFinanceUpdatePool(
        ...
        require(pool.startJoinTime > block.timestamp);
    ...
    function stableFinanceSetRewardDistributor(
        ...
        require(_stableFinanceRewardDistributor != address(0));
        ...
        notInBlackList(msg.sender)
    {
        ...
        require(
            pending.earlyWithdrawTime + addPoolInfo.delayDuration <=
                block.timestamp,
    ...
    ) external whenNotPaused nonReentrant notInBlackList(msg.sender) {
        require(
            _poolId < stableFinancePoolInfo.length,
            "StakingPool: not exist"
        );
    ...
    function linearAssignStakedData(
        ...
        // LinearStakingData
        stableFinanceStakingData[_poolId][_to] = stableFinanceStakingData[
            _poolId
        ][_from];
    ...
    function _claimReward(uint256 _poolId, uint256[] memory _amount) private {
        address account = msg.sender;
        ...
        require(stableFinanceRewardDistributor != address(0));

        require(stakingData.pendingReward.length == _amount.length);
    ...
    function stableFinanceClaimReward(
        ...
        uint256[] memory emptyArray = new uint256[](
            stakingData.pendingReward.length
        );
    ...
    function _stableFinanceDeposit(
        ...
        require(block.timestamp <= pool.endJoinTime, "Staking: already closed");
