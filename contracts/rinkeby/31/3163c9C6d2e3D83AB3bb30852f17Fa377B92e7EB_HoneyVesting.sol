...
import "./HoneyTestToken.sol";
import "./IHoneyJarsNFT.sol";
import "./IFancyBears.sol";
import "./tag.sol";

contract HoneyVesting is Ownable, AccessControlEnumerable, VestingCalculator {

    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IFancyBears public immutable fancyBearsContract;
    IHoneyJarsNFT public immutable honeyJarsContract;
    HoneyTestToken public immutable honeyContract;

    uint256 public immutable honeyPerBear;
    uint256 public immutable honeyPerJar;
    ...
    event HoneyReleasedFromBear(uint256 indexed _tokenId, uint256 _releasable);
    event HoneyReleasedFromJar(uint256 indexed _tokenId, uint256 _releasable);
    event HoneySpentFromBear(uint256 indexed _tokenId, uint256 _amount);
    event HoneySpentFromJar(uint256 indexed _tokenId, uint256 _amount);

    constructor(
        IFancyBears _fancyBearsContractAddress, 
        IHoneyJarsNFT _honeyJarsContractAddress,
        HoneyTestToken _honeyContractAddress,
    ...
    function claimHoneyFromBear(uint256 _tokenId) public {
        ...
        (bool overflowFlag, uint256 releasable) = vestingScheduleForBear(block.timestamp).trySub(honeyReleasedByBearId[_tokenId].add(honeySpentByBearId[_tokenId]));
        ...
        emit HoneyReleasedFromBear(_tokenId, releasable);
        SafeERC20.safeTransfer(honeyContract, msg.sender, releasable);
    }
    ...
    function claimHoneyFromJar(uint256 _tokenId) public {
        ...
        (bool overflowFlag, uint256 releasable) = vestingScheduleForJar(block.timestamp).trySub(honeyReleasedByJarId[_tokenId].add(honeySpentByJarId[_tokenId]));
        ...
        emit HoneyReleasedFromJar(_tokenId, releasable);
        SafeERC20.safeTransfer(honeyContract, msg.sender, releasable);
    }

    function spendHoneyInBear(uint256 _tokenId, uint256 _amount) external onlyRole(HONEY_SPENDER_ROLE) {
        require(
        ...
        //what happens to honey tokens in contract?

        emit HoneySpentFromBear(_tokenId, _amount);
    }

    function spendHoneyInJar(uint256 _tokenId, uint256 _amount) external onlyRole(HONEY_SPENDER_ROLE) {
        require(
        ...
        emit HoneySpentFromJar(_tokenId, _amount);
    }
        ...
        onlyRole(HONEY_SPENDER_ROLE)
    {
        ...
        // returnHoneyToHive(totalHoneySpent);

    }

    function getVestingAmountRemainingInBearsByWallet(address _address) public view returns (uint256[] memory) {
    ...
    function getVestingAmountRemainingInJarsByWallet(address _address) public view returns (uint256[] memory) {
    ...
    function returnHoneyToHive(uint256 _amount) private view {
        require(_amount <= honeyContract.balanceOf(address(this)), "returnHoneyToHive: not enough honey remaining in vesting contract");
        // SafeERC20.safeTransfer(honeyContract, liquidityPool, releasable);
        // SafeERC20.safeTransfer(honeyContract, teamWallet, releasable);
        // SafeERC20.safeTransfer(honeyContract, farmingPool, releasable);
        // honey.b
    }
    ...
    function setCliffStart() public override(VestingCalculator) onlyOwner() {
        super.setCliffStart();
    }
...
interface IHoneyJarsNFT is IERC721, IERC721Enumerable {
...
contract VestingCalculator {
    ...
    function calculateRewardPeriod(uint256 _timestamp) public view returns (uint256){
        ...
        return (adjustedStartTime - adjustedStartTime.mod(rewardPeriod)).div(rewardPeriod);
    }
