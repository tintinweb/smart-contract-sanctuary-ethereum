...
contract MutantPudgies is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint constant public SALE_OFFSET = 6670;
    ...
    address public immutable PAFC;
    address public immutable PS;
    ...
    constructor(address _pafc, address _ps, string memory _baseTokenURI) ERC721("MutantPudgies", "MPAFC") {
    ...
    modifier isSecure(uint _amount) {
        require(_amount <= maxTX, "MutantPudgies: You can't buy more than 100 pudgies at once!");
        require(msg.sender == tx.origin, "Must use EOA");
        require(msg.value >= mintPrice.mul(_amount), "MutantPudgies: You don't have enough ETH to mint your PudgyApes!");
        require(maxSupplyPublic.sub(reservedMutants) >= _publicSale.current().add(_amount), "MutantPudgies: Minting would exceed max supply!");
        _;
    }

    function mutate(uint _id, uint _serumType) external nonReentrant {
        require(mutationActive, "MutantPudgies: Mutation didn't start yet!");
        require(IERC721(PAFC).ownerOf(_id) == msg.sender, "MutantPudgies: You don't own the pudgy you are trying to mutate!");
        require(PudgySerumsInterface(PS).balanceOf(msg.sender, _serumType) >= 1, "MutantPudgies: You don't have any serums of this type!");
        require(!hasBeenMutatedWith(_id, _serumType), "MutantPudgies: You already have mutated this pudgy with this serum type!");
        uint id;
        if(_serumType == 3) {
    ...
    function buyMutant(uint _amount) external payable isSecure(_amount) {
        require(mainsaleActive, "MutantPudgies: Mainsale didn't start yet!");
        _mintMultiple(_amount);
    }

    function presaleMint(uint _amount, bytes memory _signature) external payable isSecure(_amount) {
        require(presaleActive, "MutantPudgies: Presale didn't start yet!");
        require(isWhitelisted(_signature), "MutantPudgies: You aren't whitelisted!");
        _mintMultiple(_amount);
    }
    ...
    function getTokenId(uint _id, uint _serumType) public pure returns (uint) {
        require(_serumType != 3, "MutantPudgies: Can't calculate M3 ids!");
        if(_serumType == 2) {
    ...
    function toggleMutation() external onlyOwner {
