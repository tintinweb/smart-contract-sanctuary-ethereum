...
contract help is ERC721A, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "help - The Contract is Paused");
        require(
            tokenCount + _amount <= maxSupply,
            "help - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "help - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "help - max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "help - insufficient ethers"
            );
        }
