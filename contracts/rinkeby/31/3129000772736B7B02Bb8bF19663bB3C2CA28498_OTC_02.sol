...
contract OTC_02 {
    ...
    mapping(uint256 => uint256) public roomNumberTimestamp;
    ...
    function zeroing(uint256 _roomNumber) internal {
        ...
        roomNumberTimestamp[_roomNumber] = 0;
    }
...
// часть контракта с мнгновенными сделками
    ...
    function storeETH(address _recipient, uint256 _roomNumber) payable public {
        ...
        roomNumberTimestamp[_roomNumber] = block.timestamp;
    }
    ...
    function delayFromSender(uint256 _roomNumber) external {
        ...
        require(roomNumberTimestamp[_roomNumber] + 300 >= block.timestamp);
    ...
    function refund(uint256 _roomNumber) external onlyAdmin {
        ...
        // это проверенная строка, которая должна компенсировать комиссию админу от возврата средств по сделке
        // ее надо вставить везде, где админ рассылает средства своей транзакцией
...
//-----------------------------------------------------------------------------------------------------------
// часть контракта со сделками с удержанием средств (функции начинаются на "Y")


    mapping(uint256 => uint256) public timeForDelay;


    // сюда передается окончательный timestamp (его в js на сайте надо генерить)
    // и создается комната
    function YstoreETH(address _recipient, uint256 _roomNumber, uint256 _timeForDelay) payable public {
        require(roomNumberSender[_roomNumber] == 0x0000000000000000000000000000000000000000, "there is a deal");
        require(roomNumberRecipient[_roomNumber] == 0x0000000000000000000000000000000000000000, "there is a deal");
        require(roomNumberIsPayed[_roomNumber] == false, "there is a deal");
        require(roomNumberValue[_roomNumber] == 0, "there is a deal");

        timeForDelay[_roomNumber] = _timeForDelay;
    ...
    // вывод средств с удержанием средств
    function YwithdrawWithDelay(uint256 _roomNumber) external {
        require(msg.sender == roomNumberRecipient[_roomNumber]);
        
        require(roomNumberTimestamp[_roomNumber] + timeForDelay[_roomNumber] >= block.timestamp);
