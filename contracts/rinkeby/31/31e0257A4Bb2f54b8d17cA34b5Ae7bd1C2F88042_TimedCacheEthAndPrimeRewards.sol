...
contract PrimeRewards is Ownable, ERC1155Holder {
    ...
    function setPrimePerSecond(
        ...
        PRIME.transferFrom(msg.sender, address(this), _primeAmount);
    ...
    function claimPrime(uint256 pid) public {
        ...
        uint256 _pendingPrime = uint256(
            accumulatedPrime - _deposit.rewardDebt
        ) / primeAmountPerSecondPrecision;
...
contract TimedCacheEthAndPrimeRewards is PrimeRewards, ReentrancyGuard {
    ...
    /// @param pids Pool IDs of all to be claimed
    function claimPoolsPrimeAndEth(uint256[] calldata pids) external {
        for (uint256 i = 0; i < pids.length; ++i) {
    ...
    function withdrawAndClaimEth(uint256 pid, uint256 amount) external {
        claimEth(pid);
        withdraw(pid, amount);
    }
    ...
    /// @param pid Token id to withdraw.
    function withdrawAndClaimPrimeAndEth(uint256 pid, uint256 amount) external {
        claimEth(pid);
