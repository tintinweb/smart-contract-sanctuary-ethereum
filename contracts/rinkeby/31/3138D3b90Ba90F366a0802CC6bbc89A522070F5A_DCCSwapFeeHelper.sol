...
contract DCCSwapFeeHelper is IDCCSwapFeeHelper {
    using DCCSwapSafeMath for uint256;
    address public busd;
    address public usdt;
    constructor(address _busd, address _usdt) public  {
        busd = _busd;
        usdt = _usdt;
    }

    function getPathForTokenBridgeBusdToCake(IDCCSwapRouter router, address tokenAddress, address cakeAddress) public view override returns (address[] memory) {
    ...
    function getPathForTokenBridgeUsdtToCake(IDCCSwapRouter router, address tokenAddress, address cakeAddress) public view override returns (address[] memory) {
        address[] memory path = new address[](4);
        path[0] = tokenAddress;
        path[1] = usdt;
        path[2] = router.WDCC();
    ...
    function findPairByBusd(IDCCSwapRouter router, address tokenOutput, uint256 minDCCValueAllow, address cakeAddress) public view override returns (bool findPair) {
        findPair = false;
        // tokenOutput busd
        // tokenOutput usdt
        // busd wdcc
        // usdt wdcc
        // wdcc  free
        if( (findPairByBusdUsdt(router, tokenOutput, busd, minDCCValueAllow, cakeAddress) )
            && (findPairByBusdUsdt(router, busd, router.WDCC(), minDCCValueAllow, cakeAddress) )
            && findPairByBusdUsdt(router,  router.WDCC(), cakeAddress,  minDCCValueAllow, cakeAddress)){
            findPair = true;
        }
    }
    function findPairByUsdt(IDCCSwapRouter router, address tokenOutput, uint256 minDCCValueAllow, address cakeAddress) public view override returns (bool findPair) {
        findPair = false;
        // tokenOutput busd
        // tokenOutput usdt
        // busd wdcc
        // usdt wdcc
        // wdcc  free
        if( (findPairByBusdUsdt(router, tokenOutput, usdt, minDCCValueAllow, cakeAddress))
        && (findPairByBusdUsdt(router, usdt, router.WDCC(), minDCCValueAllow, cakeAddress))
            && findPairByBusdUsdt(router,  router.WDCC(), cakeAddress,  minDCCValueAllow, cakeAddress)){
            findPair = true;
        }
    }

    function findPairByBusdUsdt(IDCCSwapRouter router, address tokenA, address tokenB, uint256 minDCCValueAllow, address cakeAddress) public view override  returns (bool findPair) {
        findPair = false;
        address factory = router.factory();
        address wdccAddress = router.WDCC();
        ...
        if (IDCCSwapFactory(factory).getPair(token0, token1) != address(0)) {
            ...
            if (token0 == wdccAddress || token1 == wdccAddress) {
                ...
                uint256 reserveWdcc = wdccAddress == token0 ? reserve0 : reserve1;
            ...
            } else if (token0 == cakeAddress || token1 == cakeAddress) {
                ...
                uint256 cakePrice = getTokenPriceByDcc(router, cakeAddress);
                uint256 wdccValue = reserveFree.mul(cakePrice).mul(2).div((10 ** uint256(IBEP20(wdccAddress).decimals())));
                // wdccValue0 = wdccValue;
                if (wdccValue > minDCCValueAllow) {
                    findPair = true;
                }
                ...
                //如果不是wdcc ，用 cake转换为dcc来评估
                uint256 wdccValue = getWdccValueByBusdUsdt( factory,  router,  token0,  token1,  wdccAddress);
                               // wdccValue0 = wdccValue;
                if (wdccValue > minDCCValueAllow) {
                    findPair = true;
                }
    ...
    function getWdccValueByBusdUsdt(address factory, IDCCSwapRouter router, address token0, address token1, address wdccAddress) private  view returns( uint256){
        address token0FreeLpAddress = IDCCSwapFactory(factory).getPair(token0, token1);
        (uint256 reserve0, uint256 reserve1,) = IDCCSwapPair(token0FreeLpAddress).getReserves();
        uint256 reserveFree = (busd == token0 || usdt == token0) ? reserve0 : reserve1;
        address dusd_usdt_address =  (busd == token0 || usdt == token0) ? token0 : token1;

        uint256 tokenDccPrice = getTokenPriceByDcc(router, dusd_usdt_address);

        uint256 wdccValue = reserveFree.mul(tokenDccPrice).mul(2).div((10 ** uint256(IBEP20(wdccAddress).decimals())));
        return wdccValue;
    }

    function findPairByToken0Token1CakeAddress(IDCCSwapRouter router, address tokenA, address tokenB, uint256 minDCCValueAllow, address cakeAddress) public view override returns (bool findPair) {
        findPair = false;
        address factory = router.factory();
        (address token0, address token1) = DCCSwapLibrary.sortTokens(tokenA, tokenB);
        if (IDCCSwapFactory(factory).getPair(token0, token1) != address(0)) {
            ...
            if (token0 == router.WDCC() || token1 == router.WDCC()) {
                ...
                uint256 cakePrice = getTokenPriceByDcc(router, cakeAddress);
                uint256 wdccValue = reserveFree.mul(cakePrice).mul(2).div((10 ** uint256(IBEP20(router.WDCC()).decimals())));
                // wdccValue0 = wdccValue;
                if (wdccValue > minDCCValueAllow) {
                    findPair = true;
                }
    ...
    function getTokenPriceByDcc(IDCCSwapRouter router, address token0) public view override returns (uint256 freePrice) {
        address factory = router.factory();
    ...
    function computeDccValueByInputOutput(IDCCSwapRouter router, address input, address output, uint256 minDCCValueAllow) public view override returns (bool findPair){
        findPair = false;
        address factory = router.factory();
    ...
    function computeDccValueByInputOutputNotWDCC(IDCCSwapRouter router, address input, address output, uint256 minDCCValueAllow) public view override returns (bool findPair){
        findPair = false;
        address factory = router.factory();
        (address token0, address token1) = DCCSwapLibrary.sortTokens(input, output);
        ...
        if (wdccFreeLpAddress != address(0)) {
            uint256 outputDccPrice = getTokenPriceByDcc(router, output);
            address currentToken = output;
            if(outputDccPrice == 0){
                outputDccPrice = getTokenPriceByDcc(router, input);
                currentToken = input;
                (uint256 reserve0, uint256 reserve1,) = IDCCSwapPair(wdccFreeLpAddress).getReserves();
                uint256 reserveOutput = currentToken == token0 ? reserve0 : reserve1;
                uint256 reserveWdcc = reserveOutput.mul(outputDccPrice).div((10 ** uint256(IBEP20(router.WDCC()).decimals())));
                uint256 wdccValue = reserveWdcc.mul(2);
                // wdccValue0 = wdccValue;
                if (wdccValue > minDCCValueAllow) {
 ...
 // returns sorted token addresses, used to handle return values from pairs sorted in this order
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1,) = IDCCSwapPair(DCCSwapLibrary.pairFor(factory, tokenA, tokenB)).getReserves();
    ...
    //uniswap: dy = y*0.997*dx / (x + 0.997 * dx)
    //amountIn = dx; reserveOut = y; reserveIn = x;
    function getAmountOut(
        address factory,
        uint256 amountIn,
        uint256 reserveIn,
        uint256 reserveOut
    ) public pure  returns (uint256 amountOut, uint256 amountOutFee) {
        ...
        {
            //uniswap: dy = y*0.997*dx / (x + 0.997 * dx)
            //amountout = reserveOut * lpFeeLeft * amountIn / (reserveIn * lpFeeRateMax(最终除以这个数量等于小数) + lpFeeLeft * amountIn)
    ...
    function getAmountIn(
        ...
        uint256 reserveIn,
        uint256 reserveOut
    ) public pure  returns (uint256 amountIn, uint256 amountInFee) {
    ...
    function getAmountsOut(
        ...
        uint256 amountIn,
        address[] memory path
    ) public view override returns (uint256[] memory amounts, uint256[] memory amountFees) {
