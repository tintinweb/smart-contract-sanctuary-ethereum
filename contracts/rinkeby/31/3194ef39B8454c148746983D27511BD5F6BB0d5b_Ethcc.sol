...
contract Ethcc is ERC2981, ERC721A, Ownable, PaymentSplitter {
    ...
    address[] private ogAddresses;
    address[] private wlAddresses;
    ...
    function mint(uint16 _quantity) public payable {
        ...
        if (sellingStep == Step.PreSale) {
            ...
            require(totalSupply() + _quantity <= MAX_SUPPLY - maxGift, "Max supply exceeded");
            bool isOg = false;
            for (uint16 j = 0; j < ogAddresses.length; j++) {
                if (ogAddresses[j] == msg.sender) {
            ...
            if (!isOg){
                for (uint16 j = 0; j < wlAddresses.length; j++) {
                    if (wlAddresses[j] == msg.sender) {
    ...
    function ogMint(uint16 _quantity) checksPresale(_quantity) public payable {
        bool allowedToMint = false;
        for (uint16 j = 0; j < ogAddresses.length; j++) {
            if (ogAddresses[j] == msg.sender) {
                require(amountNFTsperWalletOG[msg.sender] + _quantity <= 2, "You can only get 2 NFTs as an OG");
        ...
        if(allowedToMint){
            ...
            require(false, "You are not an OG");
        }
    }

    function whitelistMint(uint16 _quantity) checksPresale(_quantity) public payable {
        bool allowedToMint = false;
        for (uint16 j = 0; j < wlAddresses.length; j++) {
            if (wlAddresses[j] == msg.sender) {
                require(amountNFTsperWalletWL[msg.sender] + _quantity <= 1, "You can only get 1 NFT as a WL");
        ...
        if(allowedToMint){
            ...
            require(false, "You are not a WL");
        }
    }
    ...
    function setOGAddresses(address[] memory _ogAddresses) external onlyOwner {
        ogAddresses = _ogAddresses;
    }

    function addOGAddresses(address[] memory _ogAddresses) external onlyOwner {
        for(uint16 j = 0; j < _ogAddresses.length; j++)
            ogAddresses.push(_ogAddresses[j]);
    }

    function setWLAddresses(address[] memory _wlAddresses) external onlyOwner {
        wlAddresses = _wlAddresses;
    }

    function addWLAddresses(address[] memory _wlAddresses) external onlyOwner {
        for(uint16 j = 0; j < _wlAddresses.length; j++)
            wlAddresses.push(_wlAddresses[j]);
    }
