...
contract WoofGangWithRandom is Abstract_WoofGang {
    ...
    uint256[] public randomArr;

    uint256[] public pendingIds = _pendingIds;

    // Declaring this variable for Not Minted Tokens
    ...
    constructor(
        ...
        uint256 _mintPrice,
        uint256[MAX_SUPPLY] memory _random
    ...
    function mint(uint256 quantity) external override payable whenNotPaused nonReentrant
    {
        ...
        require(block.timestamp <= mintEndTime , "WoofGang: Mint process is ended");
        require((msg.value)*quantity >= mintPrice, "WoofGang: Insufficient ETH supplied");
    ...
    /// @dev This function is used for storing token ids which is not minted

    function remainingIds() public 
    {
    ...
    function mintRemainingToOwner(uint256 amount) external override onlyOwner whenNotPaused 
    {
        ...
        if(flag && (block.timestamp > mintEndTime)){
            remainingIds();
            flag = false;
        }

        for(uint i = amount; i > 0; i--) {
            uint mintTokenIds = remainingTokenIds[remainingTokenIds.length];
