...
contract Staking is OwnableAndWhitelistble {
    ...
    uint256 public constant stakePeriod = 48 hours;
    ...
    function harvest() public returns (uint256 value, uint256 toDividends) {
        ...
        require(
            mainToken.balanceOf(address(this)) >= (value_ + toDividends_),
            'Contract doesnt have enough DES'
        );
    ...
    function harvested(address who_)
        public
        view
        returns (uint256 value_, uint256 toDividends_)
    {
        ...
        uint256 reward_ = (stake_.amount * timePassed_ * (rate + stake_.boost)) /
            (100 * year);
    ...
    function unstake() external {
        require(stakes[msg.sender].startStaking != 0, 'You dont have stake');
        require(
            (block.timestamp - stakes[msg.sender].startStaking) >= stakePeriod,
            'Time does not pass'
        );
        ...
        require(
            mainToken.balanceOf(address(this)) >= amount_,
            'Contract doesnt have enough DES'
        );
