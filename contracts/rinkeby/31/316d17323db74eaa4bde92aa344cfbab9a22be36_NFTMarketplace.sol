...
contract NFTMarketplace is ReentrancyGuard, Ownable {
    ...
    function listItem(
        ...
        require(startingPrice > 0 && endingPrice > 0, "Price must be greater than 0");
        require(IERC721(nft).ownerOf(tokenId) == msg.sender, "Sender must be the token owner");
        require(IERC721(nft).getApproved(tokenId) == address(this), "NFT must be approved");
    ...
    function buyItem(uint256 itemId, uint256 payment) public nonReentrant {
        require(itemId <= _itemCounter.current(), "id must <= item count");
        require(items[itemId].state == State.Listed, "item must be in the listed state");
        require(IERC721(items[itemId].nft).getApproved(items[itemId].tokenId) == address(this), "NFT must be approved to market");
    ...
    function getItemListByState(State state) public view returns (Item[] memory) {
        uint itemCount = 0;
