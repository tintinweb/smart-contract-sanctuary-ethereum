...
contract SteezyGenesis is ERC721Burnable, Ownable {
    ...
    uint16 constant public MAX_SUPPLY = 443;
    ...
    function reserveTokens(uint16 _amount) external onlyOwner {
        require(counter + _amount <= MAX_SUPPLY, "Would exceed max number of tokens");
    ...
    function mint(bytes32[] calldata proof) external payable {
        ...
        require(counter + 1 <= MAX_SUPPLY - 1, "Maximum tokens have been minted");
        require(!addressToMinted[msg.sender], "Wallet has minted already");
