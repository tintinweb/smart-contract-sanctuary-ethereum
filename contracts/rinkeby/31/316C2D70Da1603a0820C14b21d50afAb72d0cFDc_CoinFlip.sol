...
contract CoinFlip is VRFConsumerBaseV2 {
    ...
    event Result(uint256 id, uint256 bet, uint256 randomSeed, uint256 amount, address player, uint256 winAmount, uint256 s_randomWords,uint256 time);
    ...
    function weiInusd() public view returns (uint) {
        int  priceFeed = ethInUsd();
        int weiUsd = 10**26/priceFeed;
    ...
    function verdict(uint256 random) public payable onlyVFRC {
        ...
        //    payPlayer(games[i].player,winAmount);
    ...
    // function payPlayer(address payable recipient,uint256 winMoney) external payable  {
    //     recipient.transfer(winMoney);
    // }
    function payPlayer(address payable recipient,uint256 randomNumber) public payable onlyPlayer{
       //check bets from latest betting round, one by one
        for(uint256 i=lastGameId; i<gameId; i++){
        //reset winAmount for current user
        uint256 winAmount = 0;
        ...
        //if user wins, then receives 2x of their betting amount
        if((randomNumber>=half && games[i].bet==1) || (randomNumber<half && games[i].bet==0)){
           winAmount = games[i].amount*2;
        //    payPlayer(games[i].player,winAmount);
           lastWinAmount=winAmount;
           require(recipient == games[i].player, 'Error, ');
           require(address(this).balance>=lastWinAmount, 'Error, ');
           (bool sent, bytes memory data) = recipient.call{value: lastWinAmount}("");
           require(sent, "Failed to send Ether");
        //    recipient.transfer(lastWinAmount);
           // games[i].player.transfer(winAmount);
        }
