  ...
  VRFConsumerBaseV2
{
  ...
  mapping(address => mapping(uint256 => uint256))
    public userPlayThroughPerMonth;
  mapping(address => uint256) public userPlayThroughClaimed;
  ...
  uint256 public sellTaxUnwageredMultiplier = 2; // init 8% (4% * 2)
  ...
  constructor(
    ...
    address _linkToken,
    bytes32 _keyHash
  ) ERC20('smolting inu', 'SMOL') VRFConsumerBaseV2(_vrfCoordinator) {
  ...
  function flipCoin(uint16 _percent, bool _isHeads) external payable {
    ...
    _addPlayThrough(msg.sender, _finalWagerAmount);
    _payServiceFee();
  ...
  function addPlayThrough(address _wallet, uint256 _amountWagered)
    external
    override
    onlyGame
  {
    _addPlayThrough(_wallet, _amountWagered);
    emit AddPlayThrough(_wallet, _amountWagered);
  }

  function _addPlayThrough(address _wallet, uint256 _amount) internal {
    totalPlayThrough += _amount;
    userPlayThrough[_wallet] += _amount;
    userPlayThroughPerMonth[_wallet][getMonth()] += _amount;
  }
  ...
  function getPlayThroughThisMonth(address _wallet)
    ...
    returns (uint256)
  {
    return userPlayThroughPerMonth[_wallet][getMonth()];
  }

  function claimRakeBack(uint256 _month) external {
    _month = _month == 0 ? getMonth() : _month;
    uint256 _playThrough = userPlayThroughPerMonth[msg.sender][_month];
    require(
    ...
    _mint(
      msg.sender,
      (_playThrough * rakeBackPercentage) / PERCENT_DENOMENATOR
    );
  }
  ...
  // starts at 1 and increments forever every month after launch
  function getMonth() public view returns (uint256) {
    uint256 secondsSinceLaunch = block.timestamp - launchTime;
    return 1 + (secondsSinceLaunch / 30 days);
  }
