...
contract Serpent is ERC721Enumerable, Ownable {
    ...
    string public uriBase =
        "ipfs://bafybeiex4fbtwhs4vroknjjnup2542ugcp4ftclmlg4dn3dghcqpr5gtsa/";
    ...
    constructor(address _marketAddress) ERC721("Serpent NFT Game", "SRPNT") {
    ...
    function getOwnerTokens() public view returns (SerpentAttributes[] memory) {
        ...
        SerpentAttributes[] memory ownedTokens = new SerpentAttributes[](
            ownerBalance
        );

        for (uint256 i = 0; i < ownerBalance; i++) {
            uint256 _tokenId = tokenOfOwnerByIndex(msg.sender, i);
            SerpentAttributes memory serpentToken = tokenToSerpentAttributes[
                _tokenId
            ];
    ...
    function performUpkeep(
        ...
        for (uint256 i = 0; i <= supply.current(); i++) {
            if (
                tokenToSerpentAttributes[i].hp > 0 &&
