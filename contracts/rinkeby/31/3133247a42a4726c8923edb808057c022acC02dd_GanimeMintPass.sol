...
contract GanimeMintPass is ERC1155 {
    ...
    string public name = "Ganime Free Mint Pass";
    string public symbol = "GMP";

    uint public MINT_PRICE = 0.04 ether;
    uint public MAX_SUPPLY = 1000;

    uint public MAX_MINT_PER_WALLET = 2;
    uint public MAX_MINT_PER_TX = 2;
    ...
    function uri(uint256) override public view returns (string memory) {
        return "ipfs://bafkreihzpfu4pu6yhlvohdccghgffzfejg43flilpphdoapcydjdbttuhy";
    }
    ...
    function whiteListMint(uint amount, bytes32[] calldata merkleProof) external payable {
        ...
        require(amount <= MAX_MINT_PER_TX, "Mint amount limit");
        require(balanceOf(msg.sender, 0) + amount <= MAX_MINT_PER_WALLET, "Wallet amount limit");
    ...
    function setMaxMintPerTx(uint max_mint_per_tx) external onlyOwner {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "Only owner, deear");
        _;
    }
}
