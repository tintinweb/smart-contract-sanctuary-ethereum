...
contract RektCollective is ERC721A, EIP712, Ownable{
    ...
    //event 
    event FreeMint(uint256 tokenId, address wallet);
    event WhiteListMint(uint256 tokenId, address wallet);
    ...
    function mint(bytes calldata _signature, uint256 _nftIndex, uint256 _quantity) external payable {
      require(totalSupply() < MAX_SUPPLY, "NFT: Cannot mint more!");
      if(msg.value == 0){
    ...
    function safeMint(uint256 _quantity) internal{
      require(msg.value >= mintFee * _quantity, "NFT: Not enough Minting Fee!");
