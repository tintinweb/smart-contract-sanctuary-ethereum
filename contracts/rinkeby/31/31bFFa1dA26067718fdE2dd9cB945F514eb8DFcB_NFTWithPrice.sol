...
contract NFTWithPrice is ERC1155 {
  ...
  // 定义支持的 Token ID 和初始值
  uint256 public constant TATTOO = 0;

  // 这个可以调节允许铸造多少个 NFT
  mapping(uint256 => uint256) tokensRemaining;

  // 这里的入参是 NFT 的元数据
  // ERC1155() 这里在生产环境要用动态 ID
  // https://docs.openzeppelin.com/contracts/3.x/erc1155
  constructor(string memory initMessage)
    ERC1155(
      'https://gateway.pinata.cloud/ipfs/QmYRNaePygQRRGNRu34fg5cR4FswKE1QkN89mqKcTHRrCb'
    )
  {
    message = initMessage;

    // 在这里限制最多铸造多少个 TATTOO NFT
    tokensRemaining[TATTOO] = 1;
  }

  // 铸造 NFT
  // tokenId - 多个 NFT 需要指定是哪个
  // amount - 要铸造的数量
  function mint(
    uint256 tokenId,
    uint256 amount,
    ...
    // 不乘以 1 会报错，无法单纯靠变量来完成计算
    require(msg.value >= amount * PRICE * 1 ether);

    // 查询是否有库存名额，允许继续铸造
    // 但这样直接计算不安全，容易堆栈溢出
    // require(tokensRemaining[tokenId] - amount >= 0);

    // 配合 SafeMath 安全数学的方法去计算
    ...
    // 实现付费，并且 msg.sender 会成为 NFT 的所有者
