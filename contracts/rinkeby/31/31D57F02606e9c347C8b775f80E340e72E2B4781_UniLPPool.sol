...
import {VeEarn} from "../lib/VeEarn.sol";
    ...
    Timestamp
{
        ...
        nonReentrant
    {
        ...
        IDLPPool(dlpPool).addVEAmount(msg.sender, _value);
    ...
    function unstake(uint256 _stakeId, uint256 _value) external virtual {
        ...
        address dlpPool = _factory.getPoolAddress(_dlp);
        IDLPPool(dlpPool).reduceVEAmount(msg.sender, _value);
    ...
    function unstakeMultiple(
        ...
        address dlpPool = _factory.getPoolAddress(_dlp);
        IDLPPool(dlpPool).reduceVEAmount(msg.sender, valueToUnstake);
    ...
    function _sync() internal virtual {
        ...
        uint256 dlpPerSecond = factory_.dlpPerSecond();

        // calculate the reward
        uint256 dlpReward = (secondsPassed * dlpPerSecond * weight) /
    ...
    function _yieldStake(
        ...
        address dlpPool = _factory.getPoolAddress(_dlp);
        IDLPPool(dlpPool).addVEAmount(msg.sender, pendingYieldToClaim);
        // emit an events
        emit LogStake(
            msg.sender,
            msg.sender,
    ...
    function _additional() internal virtual {
        ...
        // calculate the reward
        uint256 dlpReward = secondsPassed * dlpAdditionalPerSecond;
...
library VeEarn {
    uint256 internal constant BASE_MULTIPLY = 1e8;
    ...
    function veEarnd(uint256 _amount, uint256 _lockTime)
        internal
        pure
        returns (uint256)
    {
        return
            ((_lockTime * BASE_MULTIPLY) / BASE_TIME + BASE_MULTIPLY) * _amount;
    }
}
...
interface IDLPPool is ICorePool {
    ...
    function addVEAmount(address _staker, uint256 _value) external;

    function reduceVEAmount(address _staker, uint256 _value) external;
}
