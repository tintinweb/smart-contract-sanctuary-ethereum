...
contract EraNinjasNFT is ERC721, Ownable{
    
    uint256 public mintPrice; // mint ucreti
    uint256 public totalSupply; // satilmis adet sayisini tutacak
    uint256 public maxSupply; // ne kadar nft olacak
    uint8 public maxMint;
    bool public isMintEnabled;
    string internal baseTokenUri; 
    mapping(address => uint8) public walletMints; // walletMints['cüzdan adresi'] = 2

    constructor() payable ERC721('EraNinjas','ERA'){
    ...
    function withdraw() external onlyOwner{
        ...
        require(success, 'withdraw olmadi');
    }

    function mint(uint8 quantity_) public payable{
        require(isMintEnabled,'Mint baslamadi');
        // 0.02 ether == 2 * 0.01        
        require(msg.value == quantity_ * mintPrice, 'mint ucreti yetersiz');

        // 1000 +2 tane sattim 2 tane
