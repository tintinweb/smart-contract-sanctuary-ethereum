...
contract Passport is ERC721BurnableUpgradeable, PassageAccess, UUPSUpgradeable, ERC2771Recipient, Ownable, IPassport {
    ...
    function claimClaimlist(
        ...
        if (claimlistClaimFee > 0) require(msg.value == claimlistClaimFee * _claimAmount, "T8");
    ...
    function setClaimlistClaimFee(uint256 _claimFee) external onlyRole(MANAGER_ROLE) {
    ...
    function setClaimlistClaimEnabled(bool _claimEnabled) external onlyRole(MANAGER_ROLE) {
    ...
    function setClaimlistRoot(bytes32 _claimlistRoot) external onlyRole(MANAGER_ROLE) {
    ...
    /// @param _claimlistRoot Merkle tree root
    function setClaimlistOptions(uint256 _claimFee, bytes32 _claimlistRoot) external onlyRole(MANAGER_ROLE) {
        claimlistClaimFee = _claimFee;
        claimlistRoot = _claimlistRoot;
    }
