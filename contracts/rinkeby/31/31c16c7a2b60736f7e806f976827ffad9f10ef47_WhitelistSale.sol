...
// FILE 14: WhitelistSale.sol

pragma solidity ^0.8.0;

contract WhitelistSale is ERC721A, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  constructor() ERC721A('WhitelistSale', 'WL') {
  ...
  function whitelistMint(uint256 _count) external payable {
    uint256 supply = totalSupply();
    require(saleState == SaleState.WHITELIST_SALE, 'WHITELIST_SALE');
    require(_count > 0, 'Minimum 1 NFT has to be minted per transaction');
    require(_count <= maxPerTx, 'Max per TX reached.');
    require(supply + _count <= maxSupply, 'No more');
    ...
    if (msg.sender != owner()) {
      ...
      require(msg.value >= _count * 0, 'Please send the exact amount.');
    }
