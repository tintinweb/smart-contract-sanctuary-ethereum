...
// EXTERNAL
    ...
    /**
     * @dev Mints 'quantity' tokens to 'to' from allowlist.
     *
     * Requires allows(msg.sender) >= quantity.
     */
    function mint(address to, uint8 quantity) external payable whenNotPaused hasSupply(quantity) paidEnough(quantity) {
        if (_allows[_msgSender()] < quantity) {
            revert("Insufficient allows");
        }
        _allows[_msgSender()] -= quantity;
        _safeMint(to, quantity);
    }

    /**
     * @dev Mints 'quantity' tokens to 'to' from merkle drop.
     *
    ...
    function mintMerkle(
        ...
        uint256 claims = _merkleClaims[_merkleCounter][_msgSender()] + quantity;
        if (claims > _merkleClaimLimit) {
            revert("Insufficient allows.");
        }
        _merkleClaims[_merkleCounter][_msgSender()] = claims;
        _safeMint(to, quantity);
    }
...
// EXTERNAL VIEW
    ...
    function royaltyInfo(
        ...
        uint256 royaltyAmount = (_salePrice * _royalty) / ROYALTY_MAX;
...
// PUBLIC
    ...
    /**
     * @dev Sets the royaly percentage for IERC2981 standard. 
     * royaly % = feeNumerator / feeDenominator (100)
     */
    function setRoyalty(uint8 royalty) public onlyOwnerAndAdmins {
        if (_royalty > ROYALTY_MAX) {
