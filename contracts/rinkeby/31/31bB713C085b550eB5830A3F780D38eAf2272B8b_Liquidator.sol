...
contract Liquidator is LiquidatorBase, SuperAdminControl {
    ...
    function _liquidateCollateralAutoSellOn(uint256 _loanId, uint256 _typeOfSwap, bytes[] calldata _swapData) internal {
        ...
        require(callDataLength == lengthCollaterals, "swap call data and collateral length mismatch");
        ...
        if(_typeOfSwap == 0) {
            ...
            IUniswapSwapInterface swapInterface = IUniswapSwapInterface(
                IPriceConsumer(govPriceConsumer).getSwapInterface(
                    loanDetails.stakedCollateralTokens[i]
                )
            );
            IERC20Upgradeable(loanDetails.stakedCollateralTokens[i]).approve(
     ...
     */
    function approveCollateralToOneInch(address[] memory _collateralTokens, uint256[] memory _amounts) external onlyLiquidatorRole(msg.sender) {

        uint256 lengthCollaterals = _collateralTokens.length;
        require(lengthCollaterals == _amounts.length, "collateral and amount length mismatch");
        address oneInchAggregator = marketRegistry.getOneInchAggregator();
        for(uint256 i = 0; i < lengthCollaterals; i++) {
            IERC20(_collateralTokens[i]).approve(oneInchAggregator, _amounts[i]);
        }
    }
    ...
    @param _typeOfSwap is only used for testing local, will remove later,
    @param _swapData is the data getting from the 1inch swap api after approving token from smart contract
     */
    function liquidateLoan(uint256 _loanId, uint256 _typeOfSwap, bytes[] calldata _swapData)
        external
        override
        onlyLiquidatorRole(msg.sender)
    {
        ...
        if (lenderDetails.autoSell) {
            _liquidateCollateralAutoSellOn(_loanId, _typeOfSwap, _swapData);
        } else {
...
interface ILiquidator {
    ...
    function liquidateLoan(uint256 _loanId, uint256 _typeOfSwap, bytes[] calldata _swapData) external;
