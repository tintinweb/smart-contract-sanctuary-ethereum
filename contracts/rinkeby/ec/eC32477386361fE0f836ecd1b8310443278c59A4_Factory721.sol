...
contract Factory721 {
	...
	function deploy(bytes32 _salt, string memory name, string memory symbol, string memory tokenURIPrefix) external returns(address addr) {
		addr = address(new GDEXUserToken721{salt: _salt}(name, symbol, tokenURIPrefix));
		GDEXUserToken721 token = GDEXUserToken721(address(addr));
...
contract GDEXUserToken721 is ERC721 {
