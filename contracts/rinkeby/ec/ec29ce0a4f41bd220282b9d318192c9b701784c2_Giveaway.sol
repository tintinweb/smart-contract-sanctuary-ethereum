...
contract Giveaway is ReentrancyGuard, Maps {
    ...
    function stake(uint256 _tokenId, IERC721 nftCollection, uint256 _guildId, uint256 _spots) external nonReentrant {
        ...
        require(!contestExists[_guildId], "Each guild can have only one giveaway at a time");
        ...
        contest[_guildId].initialNumber = uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,  
        msg.sender))) % _spots;
    ...
    function claimReward(uint256 _guildId) external nonReentrant {
        ...
        require(contest[_guildId].Participates.length >= contest[_guildId].TotalEntries, "Giveaway is not finished");
        uint256 winnerIndex = rand(_guildId, contest[_guildId].Participates.length);
        address winner = contest[_guildId].Participates[winnerIndex];
        contest[_guildId].StakedCollection.transferFrom(address(this), winner, contest[_guildId].StakedTokenId);
        delete contest[_guildId];
    }

    function totalEntriesOfContest(uint256 _guildId) public view returns(uint256) {
    ...
    function rand(uint256 _guildId, uint256 _spots) private returns(uint) {
        return uint(keccak256(abi.encodePacked(contest[_guildId].initialNumber++))) % _spots;
    }
}