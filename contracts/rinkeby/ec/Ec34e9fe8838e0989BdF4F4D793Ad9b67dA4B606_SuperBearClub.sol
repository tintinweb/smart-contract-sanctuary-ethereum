...
contract SuperBearClub is ERC721A, Ownable, ReentrancyGuard {
    ...
    bytes32 public modRoot;
    bytes32 public whitelistRoot;
    bytes32 public teamRoot;
    bytes32 public specialRoot;

    mapping(address => bool) private modMinted;
    mapping(address => bool) private whitelistMinted;
    mapping(address => bool) private teamMinted;
    mapping(address => bool) private specialMinted;

    uint64 public modMintCounts;
    uint64 public whitelistMintCounts;
    uint64 public teamMintCounts;
    uint64 public specialMintCounts;
    uint64 public giftedMintCounts;

    uint256 constant MOD_INDEX = 0;
    uint256 constant WHITELIST_INDEX = 1;
    uint256 constant TEAM_INDEX = 2;
    uint256 constant SPECIAL_INDEX = 3;
    uint256 public constant MD_MINT_MAX_PER_WALLET = 10;
    uint256 public constant WL_MINT_MAX_PER_WALLET = 2;
    uint256 public constant TE_MINT_MAX_PER_WALLET = 50;
    uint256 public constant SP_MINT_MAX_PER_WALLET = 20;
    ...
    string private baseTokenURI = "ipfs://QmUnpgvQ8Q29mCscbQeF6eL6YU8gvr2QktGsTHb13pUe4W/";

    address scoresContractAddress;
    ...
    modifier preSaleActive(){
        require(!isPublicSaleActive && (isModActive || isWlActive || isTeamActive || isSpecialActive),"Presale is not open");
        _;
    }
    ...
    function setModMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        modRoot = merkleRoot;
    }
    ...
    function setSpecialMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        specialRoot = merkleRoot;
    }

    function setAllMerkleRoot(bytes32[4] calldata merkleRoots) external onlyOwner {
        modRoot = merkleRoots[MOD_INDEX];
        whitelistRoot = merkleRoots[WHITELIST_INDEX];
        teamRoot = merkleRoots[TEAM_INDEX];
        specialRoot = merkleRoots[SPECIAL_INDEX];
    }

    function setModSaleActive(bool isActive) external onlyOwner{
        require(modRoot.length != 0,"mod root not assigned");
        isModActive = isActive;
    }

    function setWhiteListSaleActive(bool isActive) external onlyOwner{
        require(whitelistRoot.length != 0,"whitelist root not assigned");
        isWlActive = isActive;
    }
    ...
    function setSpecialSaleActive(bool isActive) external onlyOwner{
        require(specialRoot.length != 0,"special root not assigned");
    ...
    function getCurrentStage() public view returns (Stage){
        ...
        }else if(isModActive || isWlActive || isTeamActive || isSpecialActive){
    ...
    function getCanFreeMintCount(bytes32[][4] calldata merkleProofs) public view returns (uint256[] memory )
    {
        ...
        if(merkleProofs[MOD_INDEX].length != 0){
            require(modRoot.length != 0,"mod root not assigned");
            require(MerkleProof.verify(merkleProofs[MOD_INDEX],modRoot,keccak256(abi.encodePacked(msg.sender))),"MerkleProof: Invalid mod proof.");
            if(isModActive && !modMinted[msg.sender]){
                counts[MOD_INDEX] = MD_MINT_MAX_PER_WALLET;
            }
        }
        ...
        if(merkleProofs[SPECIAL_INDEX].length != 0){
            require(specialRoot.length != 0,"special root not assigned");
            require(MerkleProof.verify(merkleProofs[SPECIAL_INDEX],specialRoot,keccak256(abi.encodePacked(msg.sender))),"MerkleProof: Invalid special proof.");
            if(isSpecialActive && !specialMinted[msg.sender]){
                counts[SPECIAL_INDEX] = SP_MINT_MAX_PER_WALLET;
            }
        }
    ...
    preSaleActive
    {
        ...
        if(amountsToBuy[MOD_INDEX] > 0){
            modMinted[msg.sender] = true;
            modMintCounts += uint64(amountsToBuy[MOD_INDEX]);
        }
        ...
        if(amountsToBuy[SPECIAL_INDEX] > 0 ){
            specialMinted[msg.sender] = true;
            specialMintCounts += uint64(amountsToBuy[SPECIAL_INDEX]);
        }
