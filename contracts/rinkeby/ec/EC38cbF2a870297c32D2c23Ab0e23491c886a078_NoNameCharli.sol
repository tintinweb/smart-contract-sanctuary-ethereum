...
contract NoNameCharli is Ownable, ReentrancyGuard, ERC721ACommon, ERC2981, PaymentSplitter {
    ...
    string public notRevealedUri = 'ipfs://asdfasdf/hidden.json';
    ...
    address[] private _team = [
        0x73F6E17f5C8AdEd929020859942cA5E014a8e8ef // Yangha's "Receiver" Account
    ];

    constructor(string memory uri, bytes32 merkleroot, address _proxyRegistryAddress, address payable royaltyReceiver)
        ERC721ACommon('No Name Charli', 'NNC')
    ...
    function presaleMint(address account, uint256 _amount, bytes32[] calldata _proof) 
        ...
        require(msg.sender == account, 'NoNameCharli: Not allowed.');
        require(presaleM, 'NoNameCharli: PreSale is OFF.');
        require(!isPaused, 'NoNameCharli: Contract is paused.');
        require(_amount <= presaleAmountLimit, "NoNameCharli: You are exceeding the max amount of tokens mintable during presale (3).");
        require(_presaleClaimed[msg.sender] + _amount <= presaleAmountLimit, "NoNameCharli: You are exceeding the max amount of tokens mintable during presale (3).");

        uint256 current = _tokenIds.current();

        require(current + _amount <= maxSupply, 'NoNameCharli: max supply exceeded');
        require(_price * _amount <= msg.value, 'NoNameCharli: Not enough ethers sent');
    ...
    function publicSaleMint(uint256 _amount) external payable onlyAccounts {
        require(publicM, 'NoNameCharli: PublicSale is OFF');
        require(!isPaused, 'NoNameCharli: Contract is isPaused');
        require(_amount > 0, 'NoNameCharli: zero amount');
