...
contract NativeTokenHandler is IDepositExecute, HandlerHelpers, Settleable {
    ...
    function deposit(
        ...
        require(tokenAddress == address(0x1), "invalid native token address");
    ...
    /// - {tokenAddress} must be 0x01.
    /// - {tokenAddress} must be allowed.
    function _settle(address tokenAddress, KeyValuePair[] memory entries)
        internal
        virtual
        override
    {
        ...
        for (uint256 i = 0; i < entries.length; i++) {
            address to = abi.decode(entries[i].key, (address));
...
abstract contract Settleable {
    ...
    function settle(
        ...
        BatchedStateChanges memory batchedStateChanges = abi.decode(
            data,
            (BatchedStateChanges)
        );
        require(
            _executeBatchNonce[nonceAndID] == batchedStateChanges.batchIndex,
