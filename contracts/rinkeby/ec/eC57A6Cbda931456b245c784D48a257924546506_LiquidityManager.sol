...
contract LiquidityManager is Ownable {
    SentinelToken immutable public token;
    IUniswapV2Router02 immutable public router;
    IWETH immutable public WETH;
    IUniswapV2Pair public pair;
    address immutable public ILOContract;

    constructor (SentinelToken _token, address _ILOContract, IUniswapV2Router02 _router, IWETH _WETH) public {
        token = _token;
    ...
    function createPool() external payable returns (uint amountToken, uint amountETH, uint liquidity) {
        require(msg.sender == ILOContract || msg.sender == owner());
        uint tokenAmount = token.balanceOf(address(this));
        IUniswapV2Factory factory = IUniswapV2Factory(router.factory());
        require(factory.getPair(address(WETH), address(token)) == address(0), "pool exists");
        uint balance = msg.value;
        token.approve(address(router), tokenAmount);
        (amountToken, amountETH, liquidity) = 
            router.addLiquidityETH{value: balance}(address(token), tokenAmount, tokenAmount, balance, address(this), block.timestamp);
        ...
        emit PoolCreated(address(pair), tokenAmount, balance);
    }
    ...
    function getBalancesInLP() public view returns (uint tokenBalance, uint ethBalance, uint ownedLPSupply) {
        ...
        uint pairEthBalance = WETH.balanceOf(address(pair));
...
contract SentinelToken is ERC20 {
    constructor(
        string memory name,
