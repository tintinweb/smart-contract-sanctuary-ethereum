...
contract Eighty80_l1 is ERC721Enumerable, Ownable, Pausable, NonblockingLzApp {
    ...
    constructor (address _migrator) ERC721('Eighty80TestNFT','8080') {
        __nonblockingLzApp_init(_migrator);
        maxSupply = 560;
        gas = 300000;
        individualCap = 10;
    }
    ...
    function transferTokensToL2(uint[] memory tokenIds, bytes memory userAddress, bytes memory l2Contract,  uint16 destinationChainId) external payable{
        burnTokens(tokenIds);
        bytes memory payload = abi.encode(userAddress,tokenIds);
        uint16 version = 1;
        uint _gas = gas*tokenIds.length;
        newGas = _gas;
        bytes memory adaptorParams = abi.encodePacked(version, _gas);
        (uint messageFees, ) = lzEndpoint.estimateFees(destinationChainId,address(this),payload,false,adaptorParams);
...
//    constructor(address _endpoint) LzApp(_endpoint) {}
    ...
    function __nonblockingLzApp_init(address _endpoint) internal {
        require (!isInitialised,'Error: Contract Already Initialised');
        isInitialised = true;
...
//    }
    ...
    function __lzApp_init (address _endpoint) internal {
        require (!isInitialisedOnLzApp,'Error: Contract Already Initialised');
        isInitialisedOnLzApp = true;
        lzEndpoint = ILayerZeroEndpoint(_endpoint);
    }
