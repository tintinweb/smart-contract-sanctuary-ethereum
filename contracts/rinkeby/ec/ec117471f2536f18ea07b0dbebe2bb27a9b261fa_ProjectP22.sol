...
contract ProjectP22 is ERC1155Upgradeable, OwnableUpgradeable {
    ...
    // Revenue
    uint32 public currentRevenueYear;
    mapping(uint32 => uint256) public totalRevenue;
    struct RevenueShares { 
        uint32 ownerShare;
        uint32 creatorShare;
        uint32 tokenMembersShare;
    }
    RevenueShares public revenueShares;
    mapping(uint32 => mapping(uint256 => uint256)) public perTokenClaimedAmount;
    mapping(uint32 => uint256) public ownerClaimedAmount;
    mapping(uint32 => uint256) public creatorClaimedAmount;
    ...
    function resetRevenueShare(uint32 _revenueYear) external onlyOwner {
        currentRevenueYear = _revenueYear;
        transferFund(msg.sender, address(this).balance);
    }
    ...
    mapping(address => uint32) public maxCardSupply;
    mapping(uint32 => mapping(uint32 => bool)) public isCardPicked;
    ...
    modifier publicRevenueClaim {
        require(420000000000000000000 > totalRevenue[currentRevenueYear], "total revenue must be greater than 420 eth");
        _;
   }

   modifier activePack(uint32 packId) {
        ...
        returns (uint256)
    {
        ...
        for (
            j = randomNumber;
            j <= maxAvailableIndex[packId];
            j = j + 1
        ) {
            if (isCardPicked[packId][uint32(j)] != true) {
                return j;
            }
        }
    ...
    function nowOpenPack(
        ...
        isCardPicked[packId][uint32(newImageIndex)] = true;
    ...
    function addNewPack(
        ...
        PackDetails[newPackId] = PackDetail(true, _packName, _packType, _packDescription, _numOfTokensInPerPack, _maxPackSupply, 
                maxCardSupply[_tokenContractAddress], maxCardSupply[_tokenContractAddress] + (_numOfTokensInPerPack * _maxPackSupply) - 1,'', _tokenContractAddress);
        
        maxAvailableIndex[newPackId] = _maxPackSupply - 1;
        maxCardSupply[_tokenContractAddress] = PackDetails[newPackId].endCardId + 1;
    }

    function setRevenueSharingContractAddress(address _revenueSharingContractAddress) external onlyOwner {
    ...
    function startPackSale(
        ...
        require(packSaleData[packId].SaleIsActive == false, "Sale already begun");
        packSaleData[packId] = SaleData(true, _saleDuration, block.timestamp, _packStartPrice, _packMinPrice);
        numOfDropPointsInPack[packId] = IVestedUtils(VestedUtilsAddress).getNumOfDropPoints(_packStartPrice);
    }

    function pausePackSale(uint32 packId) external onlyOwner activePack(packId) {
        require(packSaleData[packId].SaleIsActive == true, "Sale already paused");
        packSaleData[packId].SaleIsActive = false;
    }

    function setPackDetails(uint32 packId, string memory newPackName, string memory description, string memory newPackType, uint128 _packMinPrice)
        external
        onlyOwner
        activePack(packId)
    {
    ...
    function getRevenueShare(uint32 share) internal view returns(uint256) {
        return (totalRevenue[currentRevenueYear] * share) / 100;
    }
    ...
    function claimOwnerShare() external onlyOwner {
        uint256 claimValue = getRevenueShare(revenueShares.ownerShare) - ownerClaimedAmount[currentRevenueYear];
        transferFund(msg.sender, claimValue);
        ownerClaimedAmount[currentRevenueYear] += claimValue;
    }

    function setVestedUtilsProvider(
        address _VestedUtilsAddress
    ) external onlyOwner {
    ...
    function setRevenueShares(uint32 ownerShare, uint32 creatorShare, uint32 tokenMembersShare) external onlyOwner {
        require(totalRevenue[currentRevenueYear] == 0, "You can't change revenue share now");
        require((ownerShare + creatorShare + tokenMembersShare) == 100, "The total of revenue share must be 100");
        revenueShares = RevenueShares(ownerShare, creatorShare, tokenMembersShare);
    }
    ...
    function getRemainingSaleTime(uint32 packId) public view returns (uint256) {
        ...
        return
            (packSaleData[packId].saleStartTime + packSaleData[packId].saleDuration) -
            block.timestamp;
    }

    function claimToadzFreePack(uint32 toadzTokenId, uint32 packId) external activePack(packId) {
        require(
    ...
    function claimToadzAllFreePacks(uint32 packId) external activePack(packId) {
    ...
    function MintPrice(uint32 packId) public view activePack(packId) returns (uint256) {
        ...
        if (elapsed >= packSaleData[packId].saleDuration) {
            ...
            uint256 currentPrice = IVestedUtils(VestedUtilsAddress).getDroppedPrice(
                ((numOfDropPointsInPack[packId] *
                    getRemainingSaleTime(packId)) / packSaleData[packId].saleDuration) +
                    1
            );
            return
     ...
     */
    function OpenPack(uint32 packId) external activePack(packId) {
        require(
            balanceOf(msg.sender, packId) > 0,
            "You don't have any pack in your account"
        );

        uint256 randomNumberRes = IVestedUtils(
            VestedUtilsAddress
        ).getRandomNumber(
                msg.sender,
    ...
    @notice Mint pack
     */
    function Mint(uint32 count, uint32 packId) external payable activePack(packId) {
        require(packSaleData[packId].SaleIsActive, "Public sale is not active.");
        require(
        ...
        totalRevenue[currentRevenueYear] += msg.value;
    }

    function uri(uint256 id) public view activePack(uint32(id)) override returns (string memory) {
        return
    ...
    {
        return
            (getRevenueShare(revenueShares.tokenMembersShare) / tokenRevenueSharingMembers) -
            perTokenClaimedAmount[currentRevenueYear][tokenId];
    }
    ...
    function unclaimedOwnerShare() external view returns (uint256) {
        return getRevenueShare(revenueShares.ownerShare) - ownerClaimedAmount[currentRevenueYear];
    }
    ...
    function unclaimedCreatorShare() external view returns (uint256) {
        return getRevenueShare(revenueShares.creatorShare) - creatorClaimedAmount[currentRevenueYear];
    }
    
    /**
    @notice Claim individual token share by OG Cryptoadz tokenID
     */
    function claimTokenShareByToken(uint256 tokenId) external publicRevenueClaim {
        require(
            revenueSharingContract.ownerOf(tokenId) == msg.sender,
            "Your wallet does not own this token!"
        );
        uint256 claimValue = (getRevenueShare(revenueShares.tokenMembersShare) / tokenRevenueSharingMembers) - perTokenClaimedAmount[currentRevenueYear][tokenId];
        transferFund(msg.sender, claimValue);
        perTokenClaimedAmount[currentRevenueYear][tokenId] += claimValue;
    }

    /**
    @notice Claim creator's share 
     */
    function claimCreatorShare() external publicRevenueClaim {
        require(
            revenueSharingCreatorAddress == msg.sender,
            "you can't claim creator share"
        );
        uint256 claimValue = getRevenueShare(revenueShares.creatorShare) - creatorClaimedAmount[currentRevenueYear];
        transferFund(msg.sender, claimValue);
        creatorClaimedAmount[currentRevenueYear] += claimValue;
    }

    /**
    @notice Claim all unclaimed amount for all of your OG Cryptoadz tokenIDs
     */
    function claimTokenShare() external publicRevenueClaim {
        uint256 tokenCount = revenueSharingContract.balanceOf(msg.sender);
        require(tokenCount > 0, "you don't own any token");
        uint256 index;
        uint256 alreadyClaimedAmount = 0;
        uint256 perTokenClaimableAmount = (getRevenueShare(revenueShares.tokenMembersShare) / tokenRevenueSharingMembers);
        for (index = 0; index < tokenCount; index++) {
            uint256 tokenId = revenueSharingContract.tokenOfOwnerByIndex(
                msg.sender,
                index
            );
            alreadyClaimedAmount += perTokenClaimedAmount[currentRevenueYear][tokenId];
            perTokenClaimedAmount[currentRevenueYear][tokenId] = perTokenClaimableAmount;
        }
...
interface TokenContract { function Mint(uint256 numberOfTokens,address toAddress,uint256 tokenStartIndex,uint256 randomNumber) external; }
