...
contract StakeNFT {
    ...
    constructor(ERC1155Token _nft) {
        //parentNFT = 0x72af1D64c281727cdd818c921cd42c7Fb539E879; // Change it to your NFT contract addr
        parentNFT = _nft;
    }

    function stake(uint256 _tokenId, uint256 _amount) public {
        //we should verify if the user has that amount of NFT token

        //this contract has to be approved, so the parent contract allows its address to make operations
        parentNFT.safeTransferFrom(msg.sender, address(this), _tokenId, _amount, "0x00");

        Staker storage staker = stakers[msg.sender];

        if (staker.stakedTokens[_tokenId].amount > 0) {
            //should add logic to calculate the reward from old amount and start counting for new amount
    ...
    function unstake(uint256 _tokenId, uint256 _amount) public {
        ...
        //stakingTime[stakeKey] += (block.timestamp - stakes[stakeKey].timestamp);
    }

    function getStakeValue(address _owner, uint256 _tokenId) public view returns (string memory) {
        Staker storage staker = stakers[_owner];
        return Strings.toString(staker.stakedTokens[_tokenId].amount);
    }
