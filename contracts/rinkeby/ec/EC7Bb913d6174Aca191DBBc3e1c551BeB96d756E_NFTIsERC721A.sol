...
contract NFTIsERC721A is ERC721A, ERC721AQueryable, Ownable, PaymentSplitter {
    ...
    **/
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable whenNotPaused {
        require(getStep() == Step.WhitelistSale, "Whitelist Sale is not activated");
        require(isWhitelisted(_account, _proof), "Not whitelisted");
        require(amountNFTperWalletWhitelistSale[msg.sender] + _quantity <= MAX_PER_ADDRESS_DURING_WHITELIST_MINT, "You can only mint one NFT during the Whitelist Sale");
    ...
    **/
    function publicMint(address _account, uint _quantity) external payable whenNotPaused {
        ...
        require(amountNFTperWalletPublicSale[msg.sender] + _quantity <= MAX_PER_ADDRESS_DURING_PUBLIC_MINT, "You can only mint 3 NFTs during the Public Sale");
        require(totalSupply() + _quantity <= MAX_SUPPLY_MINUS_GIFT, "Max supply exceeded");
        require(msg.value >= PRICE_PUBLIC_MINT * _quantity, "Not enought funds");
    ...
    **/
    function getStep() public view returns(Step actualStep) {
        ...
        if(block.timestamp >= saleStartTime && block.timestamp < saleStartTime + 24 hours) {
