...
contract CustomERC721A is ERC721A, Ownable, RoyaltiesV2Impl, ReentrancyGuard {
    struct Winner {
        address wallet;
        uint256 position;
        uint256 tokenId;
    ...
    address payable constant _ROYALTIES_WALLET =
        payable(0x11573fa8936eBaB7e80FfBA8513796C83C7A24B9);
    uint96 constant _ROYALTIES_PERCENTAGE = 1000;

    uint256 constant _MINT_PRICE = 0.25 ether;
    uint256 constant _WHITELIST_MINT_PRICE = 0.25 ether;

    uint256 constant _MAX_SUPPLY = 10000;
    uint256 constant _WHITELIST_MAX_SUPPLY = 2000;

    uint256 constant _MAX_TOKENS_PER_TX = 10;
    uint256 constant _MAX_TOKENS_PER_WHITELIST_TX = 2;
    uint256 constant _MAX_TOKENS_PER_WHITELIST_USER = 2;
    ...
    constructor() ERC721A('HungryEyesV3Test', 'HEV3NFT') {}
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(
            _mintAmount > 0 && _mintAmount <= _MAX_TOKENS_PER_TX,
            'Maximum allowed tokens per transaction exceeded'
        );

        require(
            totalMinted() + _mintAmount <= _MAX_SUPPLY,
    ...
    modifier whitelistCompliance(uint256 _mintAmount) {
        require(
            _mintAmount > 0 && _mintAmount <= _MAX_TOKENS_PER_WHITELIST_TX,
            'Maximum allowed tokens per transaction exceeded'
        );

        require(
            totalMinted() + _mintAmount <= _WHITELIST_MAX_SUPPLY,
    ...
    function whitelistMint(uint256 _mintAmount)
        ...
        whitelistOpen
        onlyWhitelisted
        whitelistCompliance(_mintAmount)
    {
        require(!_paused, 'The contract is paused');
        require(
            balanceOf(msg.sender) + _mintAmount <=
                _MAX_TOKENS_PER_WHITELIST_USER,
            'You have exceeded the number of tokens per whitelist user'
        );

        require(
            msg.value >= (_WHITELIST_MINT_PRICE * _mintAmount),
            'Insufficient funds'
        );
    ...
    function raffle(uint256 _winnerAmount) public onlyOwner returns (uint256) {
        uint256[] memory _range = _rangeArray(totalMinted());
        ...
        for (uint256 i = 0; i < _winnerAmount; i++) {
            ...
            while (_shuffled[random] == 0) {
                random > totalMinted() / 2 ? random-- : random++;
            }
    ...
    function _getLastRaffleTimestamp() internal view returns (uint256) {
        return _last_raffle_timestamp;
    }

    function getLatestWinners() public view returns (Winner[] memory) {
        return _raffle_winners[_getLastRaffleTimestamp()];
    }
