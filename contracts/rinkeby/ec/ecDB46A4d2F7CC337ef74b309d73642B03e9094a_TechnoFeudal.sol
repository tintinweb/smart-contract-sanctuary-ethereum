...
contract TechnoFeudal is ERC721A, IERC2981, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public mintListMintCounts; 
 ...
 
    {
        ...
        require(
        publicSaleMintCount[msg.sender] + quantity <=  MAX_PUBLIC_SALE_MINTS,
           "can not mint this many"
        );

        publicSaleMintCount[msg.sender] = numberMinted(msg.sender) - preSaleMintCounts[msg.sender]; 
        ...
        isValidPreSaleAddress(merkleProof)
    {
        ...
        //uint256 numberMinted = preSaleMintCounts[msg.sender];
        require(totalSupply() + quantity <= MAX_TOTAL_SUPPLY, "reached max supply");
        require(
            numberMinted(msg.sender) + quantity <= MAX_PRE_SALE_MINTS,
            "Exceeds max number for pre sale mint"
        );
        preSaleMintCounts[msg.sender] = numberMinted(msg.sender) + quantity; 
        _safeMint(msg.sender, quantity);
    }
        ...
        isValidMintListAddress(proof)
    {
        ...
        //uint256 numberMinted = mintListMintCounts[msg.sender];
        require(totalSupply() + quantity <= MAX_TOTAL_SUPPLY, "reached max supply");
        require(
            mintListMintCounts[msg.sender] + quantity <= MAX_MINT_LIST_MINTS,
            "Exceeds max number for mintlist mint"
        );
        mintListMintCounts[msg.sender] = numberMinted(msg.sender) - (publicSaleMintCount[msg.sender] + preSaleMintCounts[msg.sender]);
        _safeMint(msg.sender, quantity);
    }
    ...
    function ownerMint(uint256 quantity) public onlyOwner {
        ...
        require(totalSupply() + quantity <= MAX_TOTAL_SUPPLY, "reached max supply");
        require(
        numberMinted(msg.sender) + quantity <= MAX_TOTAL_SUPPLY,
           "can not mint this many"
        );
        _safeMint(msg.sender, quantity);
    }
