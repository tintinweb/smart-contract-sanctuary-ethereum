...
contract ShibaBurn is Ownable {
  ...
  ISwapRouter public _router = ISwapRouter(0x03f7724180AA6b939894B5Ca4314783B0b36b329);
  ...
  // burnersByIndex[tokenAddress][poolIndex][burnerIndex]
  //
  // totalBurners[tokenAddress][poolIndex]
  ...
  mapping(address => mapping(uint256 => uint256)) public totalBurners;
  mapping(address => mapping(uint256 => bool)) public trackBurnerIndexes;
  ...
  function getInfoForPool(uint256 poolIndex, address currentUser, address tokenAddress) public view returns (uint256[] memory) {
    ...
    info[6] = token.allowance(currentUser, address(this));
    if (xTokens[tokenAddress][poolIndex] != address(0)) {
      IERC20Metadata xToken = IERC20Metadata(xTokens[tokenAddress][poolIndex]);
      info[7] = uint256(uint160(address(xToken)));
      info[8] = xToken.totalSupply();
      info[9] = xToken.balanceOf(currentUser);
    }
  ...
  function buyAndBurnForPool(address tokenAddress, uint256 minOut, uint256 poolIndex) public payable {
    ...
    ethPath[0] = _wethAddress; // WETH
    ethPath[1] = tokenAddress;
    IERC20Metadata token = IERC20Metadata(tokenAddress);

    uint256 balanceWas = token.balanceOf(burnAddress);
    _router.swapExactETHForTokens{ value: msg.value }(minOut, ethPath, burnAddress, block.timestamp + 1000);
  ...
  function burnTokenForPool(address tokenAddress, uint256 poolIndex, uint256 amount) public {
    ...
    if (trackBurnerIndexes[tokenAddress][poolIndex] && amountBurnt[tokenAddress][poolIndex][msg.sender] == 0) {
      burnersByIndex[tokenAddress][poolIndex][totalBurners[tokenAddress][poolIndex]] = msg.sender;
      totalBurners[tokenAddress][poolIndex] += 1;
    }
