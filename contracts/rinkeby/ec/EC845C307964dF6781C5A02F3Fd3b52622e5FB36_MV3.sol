...
contract MV3 is ERC721A, Ownable {
  ...
  constructor(uint256 collectionSize_) ERC721A("Test", "Test") {
    ...
    function startEvent(uint256[] memory list) external onlyOwner returns (uint256[] memory) {
      ...
      hashedList = keccak256(abi.encode(list));
      return list;
    }

    function validateList(uint256[] memory list) external view returns (bool) {
      if (keccak256(abi.encode(list))==hashedList) {
        return true;
      }
      return false;
    }
    ...
    function auctionMint(uint256 quantity) external payable callerIsUser {
      ...
      uint256 totalCost = getAuctionPrice(publicSaleStartTime) * quantity;
    ...
    function mintFromSignature(bytes memory sig, int quantity, int tier) external payable returns (bool) {
      uint32 q = uint32(int32(quantity));
        ...
        //If whitelist sale will end before auction, set a max whitelist size for it
        require(totalSupply() + 1 <= 4950, "reached max supply");
        
        bytes memory hash = abi.encodePacked(toAsciiString(msg.sender), uint2str(q), uint2str(uint256(tier)), uint2str(totalSupply()));
    ...
    function setServerAddress(address server) public onlyOwner {
    ...
    function withdraw() external onlyOwner {
      ...
      address payable address1 = payable(0xBAFAe2c3377812A2983975B04fAFAcB2006FFaCc);//Test wallets, need to set correct wallets with correct percentages
      address payable address2 = payable(0x86d84DC1717BB44ED5470B180c5A663DF4C5c3E9);
      address payable address3 = payable(0x517Cde6c5acA0BA5dddBAaafFC4D8824c93e5c59);
      address1.transfer(onePercent*50);
      address2.transfer(onePercent*25);
      address3.transfer(onePercent*25);
    }
}