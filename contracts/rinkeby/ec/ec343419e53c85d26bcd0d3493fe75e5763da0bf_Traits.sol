...
contract Traits is Ownable, ITraits {
    ...
    string mutantBody;
    mapping(uint256 => string) cowboyBody;
    mapping(uint256 => string) alienBody;
    ...
    // need to include all 9 bodies and rotate
    function uploadBodies(string[] calldata _cowboy, string[] calldata _alien, string calldata _mutant)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _cowboy.length; i++) {
            cowboyBody[i] = _cowboy[i];
        }
        for (uint256 i = 0; i < _alien.length; i++) {
            alienBody[i] = _cowboy[i];
        }
        mutantBody = _mutant;
    }
