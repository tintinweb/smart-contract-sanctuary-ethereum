...
contract ERC721AFacet is ERC721A {
    ...
    function mint(
        ...
        require(
            !s.allowListEnabled,
            "AllowList is not enabled. Use regular mint function."
        );
        require(
    ...
    function mint(address to, uint256 quantity) public payable whenNotPaused {
        require(
            s.allowListEnabled,
            "Allow list is enabled and no proof provided"
        );
    ...
    function quantityCheck(address to, uint256 quantity) internal view {
        ...
        unchecked {
            ...
            require(
                (s.currentIndex + quantity) < s._maxSupply,
    ...
    function priceCheck() internal view {
        require(msg.value >= s._price, "Incorrect value sent to mint");
    }
    ...
    function setMaxMintPerTx(uint256 _maxMintPerTx) public onlyOwner {
        s._maxMintPerTx = _maxMintPerTx;
    }

    function setMaxSupply(uint256 _maxSupply) public onlyOwner {
        s._maxSupply = _maxSupply;
    }
...
abstract contract ERC721A is IERC721A, Pausable {
    ...
    function _numberMinted(address owner) internal view returns (uint256) {
        return
            (s._packedAddressData[owner] >> _BITPOS_NUMBER_MINTED) &
    ...
    function _numberBurned(address owner) internal view returns (uint256) {
        return
            (s._packedAddressData[owner] >> _BITPOS_NUMBER_BURNED) &
            _BITMASK_ADDRESS_DATA_ENTRY;
    }
        ...
        unchecked {
            if (_startTokenId() <= curr)
                if (curr < s.currentIndex) {
                    uint256 packed = s._packedOwnerships[curr];
                    // If not burned.
                    if (packed & _BITMASK_BURNED == 0) {
                        ...
                        while (packed == 0) {
                            packed = s._packedOwnerships[--curr];
                        }
                        return packed;
                    }
                }
    ...
    function _exists(uint256 tokenId) internal view virtual returns (bool) {
        return
            ...
            s._packedOwnerships[tokenId] & _BITMASK_BURNED == 0; // and not burned.
    }
    ...
    {
        TokenApprovalRef storage tokenApproval = s._tokenApprovals[tokenId];
        // The following is equivalent to `approvedAddress = s._tokenApprovals[tokenId].value`.
        assembly {
    ...
    function transferFrom(
        ...
        assembly {
            if approvedAddress {
                // This is equivalent to `delete s._tokenApprovals[tokenId]`.
    ...
    function _mint(address to, uint256 quantity) internal virtual {
        ...
        unchecked {
            ...
            s._packedOwnerships[startTokenId] = _packOwnershipData(
                to,
...
library AllowList {
        ...
        returns (bool)
    {
        ...
        require(
            MerkleProof.verify(_merkleProof, s._allowListRoot, leaf),
            "Incorrect proof"
        );
        return true;
    }
