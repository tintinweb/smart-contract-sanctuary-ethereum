...
contract VerumWorld is AbstractERC1155Factory {
    ...
    function pauseArtistCollection(string memory collectionID, bool pauseBoolean) external override onlyOwner whenNotPaused {
        ...
        require(artistCollection.isPaused != pauseBoolean , "VerumWorld: It is already in the same state");
    ...
    function updateMaintainerAddress(string memory collectionID, address newMaintainerAddress) external onlyAdmins(collectionID) whenNotPaused {
        ...
        ArtistCollection storage artistCollection = artistCollections[collectionID];
        require(artistCollection.maintainerAddress != newMaintainerAddress,"VerumWorld: Maintainer address must be different from old maintainer address");
        require(artistCollection.isPaused != true , "VerumWorld: Artist is not active with verum world");
    ...
    function burnAndMint(string memory collectionID, string memory avatarUri) payable external whenNotPaused nonReentrant {
        ArtistCollection memory artistCollection = artistCollections[collectionID];
        require(artistCollection.isPaused != true , "VerumWorld: Artist is not active with verum world");
        ...
        iartistcol.mint{value:msg.value}(_msgSender(),avatarUri); 
    }
...
interface IArtistCollectibles {
     ...
     * @dev Mint a new NFT 
     */
    function mint(address account, string memory avatarURI) external payable;
