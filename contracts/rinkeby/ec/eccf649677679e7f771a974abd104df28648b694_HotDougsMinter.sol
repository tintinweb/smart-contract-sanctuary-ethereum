...
contract HotDougsMinter is Ownable, ReentrancyGuard, ERC721TokenReceiver {
    ...
    /// @notice price of Dougs minted through Minter contract
    uint256 public newDougPrice = .025 ether;
    ...
    function setNewDougPrice(uint256 _newDougPrice) external onlyOwner {
        newDougPrice = _newDougPrice;
    }
    ...
    uint256 public lastFreeDougTokenId = 30;
    ...
    modifier canMintDougs(uint256 numberOfTokens) {
        require(
            dougTotalSupply() + numberOfTokens <= dougsMax,
            "Not enough Dougs remaining to mint"
        );
        _;
    }
    ...
    function setDougContractAddress(address _addressDoug) external onlyOwner {
        dougContractAddress = _addressDoug;
    }
        ...
        returns (uint256[] memory)
    {
        ...
        uint256[] memory _tokensOfOwner = new uint256[](
            _hotDougs.balanceOf(_owner)
        );
        uint256 i;

        for (i = 0; i < _hotDougs.balanceOf(_owner); i++) {
    ...
    function sendFreeDougs() external {
        uint256[] memory addressTokenIds = getTokenIds(tx.origin);
        require(addressTokenIds.length > 0, "No tokens owned!");

        for (uint256 i = 0; i < addressTokenIds.length; i++) {
            if (
                claimedDougs[addressTokenIds[i]] == 0 &&
                addressTokenIds[i] <= lastFreeDougTokenId
            ) {
                // add tokenId to list of claimedDougs
                claimedDougs[addressTokenIds[i]] = 1;

                mintAndTransferDoug(1);
            }
        }
    }

    function mintAndTransferDoug(uint256 _numberOfTokens)
        internal
    ...
    function mintDougs(uint256 numberToMint)
        ...
        isCorrectPayment(newDougPrice, numberToMint)
        canMintDougs(numberToMint)
        nonReentrant
    {
        mintAndTransferDoug(numberToMint);
    }

    /// @notice allow the owner to update the maximum tokenId redeemable for a free Doug
    function setLastFreeDougTokenId(uint256 _tokenId) external onlyOwner {
        lastFreeDougTokenId = _tokenId;
    }
    ...
    function ownerMint(uint256 numberOfTokens)
        ...
        canMintDougs(numberOfTokens)
    {
        mintAndTransferDoug(numberOfTokens);
    }
