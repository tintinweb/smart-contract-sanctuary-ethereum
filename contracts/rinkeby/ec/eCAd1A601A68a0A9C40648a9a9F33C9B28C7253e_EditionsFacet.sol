...
contract EditionsFacet is Pausable {
    ...
    function mintEdition(
    ...
    ) public payable whenNotPaused validEdition(_editionIndex) {
        ...
        unchecked {
            if (
                ...
                (_edition.totalSupply + _quantity) > _edition.maxSupply
            ) {
        ...
        if (_edition.price > 0) {
            require(msg.value >= _edition.price, "Insufficient funds");
        }

        // Set token edition
        // Next token ID is s.currentIndex;
        s.tokenEdition[s.currentIndex] = _editionIndex;

        // Mint the token
        _mintApproved(_to, _quantity);
    ...
    function quantityCheck(address to, uint256 quantity) private view {
        unchecked {
            if ((s.currentIndex + quantity) > maxSupplyEdition())
                revert ExceedsMaxSupply();

            if (
                ERC721ALib._numberMinted(to) + quantity >
                maxMintPerAddressEdition()
            ) revert ExceedsMaxMintPerAddress();

            if (quantity > maxMintPerTxEdition()) revert ExceedsMaxMintPerTx();
        }
    }
    ...
    function priceCheck(uint256 quantity) private {
        if (msg.value < (quantity * priceEdition())) revert InvalidValueSent();
    }

    // =============================================================
    //                        Getters
    // =============================================================
    function maxMintPerTxEdition() public view returns (uint256) {
        return s.maxMintPerTx == 0 ? MAX_UINT256 : s.maxMintPerTx;
    }

    function maxMintPerAddressEdition() public view returns (uint256) {
        return s.maxMintPerAddress == 0 ? MAX_UINT64 : s.maxMintPerAddress;
    }

    function maxSupplyEdition() public view returns (uint256) {
        return s.maxSupply == 0 ? MAX_UINT256 : s.maxSupply;
    }

    function priceEdition() public view returns (uint256) {
        return s.isPriceUSD ? ERC721ALib.convertUSDtoWei(s.price) : s.price;
    }

    function editionsEnabled() public view returns (bool) {
        return s.editionsEnabled;
    }
    ...
    //                        Setters
    // =============================================================
    function setPriceEdition(uint256 _editionIndex, uint256 _price)
        public
        onlyOwner
        validEdition(_editionIndex)
    {
