...
contract EthLlamasNFT is ERC721, Pausable, Ownable, ReentrancyGuard {
    ...
    uint256 public constant totalReserved = 100;
    // _reserved is the total number of claimed reserved EthLlamas
    ...
    // ----- Owner and DAO wallets -----
    address private _ownerWallet = 0x268800C4Cb54a5Fef148Be805bac88ffcf519DE3;
    address private _communityDAOWallet;
    ...
    function safeMint(address _to, uint256 _mintAmount) external virtual whenNotPaused payable nonReentrant {
        ...
        require(_tokenSupply.current() + _mintAmount <= maxSupply, "Current tokenSupply and mintAmount needs to be lessthan or equal to maxSupply.");
        require(msg.value >= _cost * _mintAmount, "Not enough ETH sent, check price.");
        _internalMint(_to, _mintAmount);
    }
    ...
    function getEthLlamas()public view returns(uint256 [] memory){
        return ethLlamas;
    }
    ...
    function setDAOWallet(address _newDAOWallet) external onlyOwner {
        _communityDAOWallet = _newDAOWallet;
    }
    
    function withdraw() external onlyOwner {
        require(_communityDAOWallet != address(0), 'Community DAO wallet must be set.');
        uint256 _balance = address(this).balance;
        uint256 _split = _balance.mul(30).div(100);
        require(payable(_ownerWallet).send(_split));
        require(payable(_communityDAOWallet).send(_balance.sub(_split)));
    }

    function claimReserved(address _to, uint256 _amount) external whenNotPaused onlyOwner {
        require(_amount <= totalReserved, "That would exceed the max reserved.");
        _internalMint(_to, _amount);
        _reserved -= _amount;
    }
