...
contract Fanverse is ERC1155URIStorageUpgradeable, ERC2981Upgradeable, EIP712Upgradeable{
    ...
    mapping(uint256 => bool) public counterstatus;   // for verifying if the voucher has NFTs greater than zero
    mapping(uint256 => uint256) public nftamountleft; // for checking the amount of nft copiesleft  
...
*/
    ...
    function _hashMinter(FNVvoucher.mintVoucher memory voucher) public view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256(
                "mintVoucher(uint160 nftAddress,uint160 nftOwner,uint256 tokenId,uint256 countervalue,uint256 amount,string tokenUri,uint160 royaltyKeeper,uint96 royaltyFees)"
            ),
            voucher.nftAddress,
            voucher.nftOwner,
            voucher.tokenId,
...
*/
    ...
    function buyFixBuy(FNVvoucher.mintVoucher memory mintvoucher, FNVvoucher.priListing memory _voucher, address redeemer, uint256 nftamount) public payable{
        ...
        require(counterstatus[_voucher.countervalue] == false && counterstatus[mintvoucher.countervalue] == false, "Voucher has expired");
        require(mintvoucher.countervalue != _voucher.countervalue, "Counters can't be same");
        ...
        if(nftamountleft[_voucher.countervalue] == 0){
            nftamountleft[_voucher.countervalue] = _voucher.amount;
        }

        if(nftamountleft[mintvoucher.countervalue] == 0){
            nftamountleft[mintvoucher.countervalue] = mintvoucher.amount;
        }

        require(nftamount <= _voucher.amount , "Not listed enough NFTs");
        require(nftamountleft[mintvoucher.countervalue] >= nftamount, "Owner doesn't have enough NFT balance");
        ...
        nftamountleft[_voucher.countervalue] -= nftamount;
        nftamountleft[mintvoucher.countervalue] -= nftamount;

        if(nftamountleft[_voucher.countervalue] == 0){
            counterstatus[_voucher.countervalue] = true;
        }
        if(nftamountleft[mintvoucher.countervalue] == 0){
            counterstatus[mintvoucher.countervalue] = true;
