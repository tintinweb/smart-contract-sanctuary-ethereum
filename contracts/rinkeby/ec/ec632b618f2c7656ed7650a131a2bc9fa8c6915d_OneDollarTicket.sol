...
// File: contracts/Lottery Contracts/OneDollarLottery.sol
...
contract OneDollarTicket is VRFConsumerBase, ReentrancyGuard, Ownable, Pausable {
    ...
    uint ticketcount;
    uint ticketprice = 0.00040 ether;
    ...
    // receive function is called when ether is sent to contract
    ...
    // this will call the pause function from pausable.sol
    // pausing the specified functions
    function PauseContract() public {
        _pause();
    }

    // this will call the unpause function from pausable.sol
    // unpausing the specified functions
    function UnpauseContract() public {
        _unpause();
    }

    // Allows the owner of the contract to transfer ownership to another 
    function transferOwnership(address newOwner) whenPaused() public virtual onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        emit OwnershipTransferred(owner, newOwner);
        owner = newOwner;
    }

    // displays the current onwer of the contract
    ...
    function pickWinner() whenPaused() public {
        getRandomNumber();
    }
    ...
    function payWinner() whenPaused() public onlyOwner nonReentrant {
        ...
        players[index].transfer( (address(this).balance / 1000) * 100 );
        emit TransferSent(msg.sender, players[index], address(this).balance);
