...
contract MerkleTest is Ownable {
    ...
    function verify(bytes32[] calldata proof, address sender) public view returns (bool, bytes32) {
        bytes32 leaf = keccak256(abi.encodePacked(keccak256(abi.encodePacked(sender))));
        return (MerkleProof.verify(proof, root, leaf), leaf);
    }

    function verifyLeaf(bytes32[] calldata proof, bytes32 leaf) public view returns (bool) {
