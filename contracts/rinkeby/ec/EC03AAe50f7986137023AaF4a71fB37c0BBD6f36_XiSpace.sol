...
contract XiSpace is AccessControl {
    ...
    uint256 public constant PIXEL_X_PRICE = BETA_RHO_SUPPLY / MAX_X;
    uint256 public constant PIXEL_Y_PRICE = BETA_RHO_SUPPLY / MAX_Y;
    uint256 public constant SECOND_PRICE = 10**18;

    address public treasury = 0x35F67c1D929E106FDfF8D1A55226AFe15c34dbE2; //TODO: Get treasury address
    ...
    */
    function submit(uint16 x, uint16 y, uint16 width, uint16 height, uint256 time, uint256 duration, bytes32 sha, uint256 computedKappaAmount, uint256 computedGammaAmount) external {
        ...
        receipts[bookingsCount] = Receipt(computedKappaAmount, computedGammaAmount);
        emit SUBMISSION(bookingsCount, msg.sender);
        bookingsCount++;
        // Transfer the tokens from the user
        betaToken.transferFrom(msg.sender, address(this), PIXEL_X_PRICE * width);
        rhoToken.transferFrom(msg.sender, address(this), PIXEL_Y_PRICE * height);
        ...
        xiToken.transferFrom(msg.sender, address(this), SECOND_PRICE * duration);
    }
    ...
    */
     ...
     * In case of rejection, tokens are refunded, in case of acceptance the tokens are sent to treasury
     * @param id ID of the submission to validate
     * @param accept True to accept the submission, false to reject it
    */
    function validate(uint256 id, bool accept) external onlyRole(VALIDATOR_ROLE) {
        if(accept) {
            // Transfer the tokens to the treasury
            _moveTokens(id, treasury);
            // no longer needed since can't be reimbursed after validation
            delete receipts[id];
    ...
    */
    function _moveTokens(uint256 id, address destination) internal {
        Booking memory submission = bookings[id];
        Receipt memory receipt = receipts[id];
        betaToken.transfer(destination, PIXEL_X_PRICE * submission.width);
        rhoToken.transfer(destination, PIXEL_Y_PRICE * submission.height);
        kappaToken.transfer(destination, receipt.kappaAmount);
        gammaToken.transfer(destination, receipt.gammaAmount);
        xiToken.transfer(destination, SECOND_PRICE * submission.duration);
    }
}
