...
contract StakingContract {
    ...
    function JoinBattle(uint256 _amount, uint256 _battleId, address joinerReferalPerson) public {
        ...
        uint256 public losertokenAmount;
        uint256 public winnertokenAmount ;

        uint256 public totalMinutes;

        uint256 public loserGenReward ;
        uint256 public winnerGenReward ;

        uint256 public riskDeductionFromLoser;
 
        uint256 public loserFinalGenReward ;

        uint256 public winnerAreenaReward ;

        uint256 public sendWinnerGenReward ;
        uint256 public sendLoserGenReward ;
    ...
    function LeaveBattle(uint256 count) public {
       ...
       Battle storage battle = battles[count];
        ...
        if(!battle.joined){
            ...
            losertokenAmount = players[battle.loser].battleRecord[players[battle.loser].battleCount].amount;
             winnertokenAmount = players[battle.winner].battleRecord[players[battle.winner].battleCount].amount;

             totalMinutes =  calculateTotalMinutes(block.timestamp, battle.startingTime);
 
             loserGenReward = calculateRewardInGen(losertokenAmount, totalMinutes);
             winnerGenReward = calculateRewardInGen(winnertokenAmount, totalMinutes);
     
             riskDeductionFromLoser = calculateRiskPercentage(loserGenReward, battle.riskPercentage);
             
             loserFinalGenReward = loserGenReward - riskDeductionFromLoser;

             winnerAreenaReward = calculateRewardInAreena(winnertokenAmount, totalMinutes);
 
             sendWinnerGenReward =  winnerGenReward + riskDeductionFromLoser + winnertokenAmount;
             sendLoserGenReward =  losertokenAmount + loserFinalGenReward;
    ...
    function calculateRewardInGen(uint256 _amount, uint256 _totalMinutes) public view returns(uint256){
        ...
        uint256 value =  ((_amount.mul(_initialPercentage)).div(100000000000));
        return value;
    }
    ...
    function setGenRewardPercentage(uint256 _percentage) external  onlyOwner {
        genRewardPercentage = _percentage.mul(1e9);
    }

    function getGenRewardPercentage() external returns(uint256) {
        genRewardPercentage = genRewardPercentage.div(1e9);
        return genRewardPercentage;
    }
