...
contract NftArtForUkraine is ERC1155, Ownable {

    uint256 public minPrice = 10**17; //0.1ETH
    ...
    function newArt(uint256 count) external onlyOwner returns (uint256[] memory) {
        require(count > 0, "No new art to add");
        uint256[] memory result = new uint256[](count);
        for(uint256 i=0; i<count; i++) {
            uint256 _id = idList.length;
            idList.push(_id);
            idExists[_id] = true;
            result[i] = _id;
    ...
    function mint(uint256 id, uint256 amount) payable external {
        require(idExists[id], "Provided ID does not exist");
    ...
    function mintBatch(
        ...
        for(uint256 i=0; i<amounts.length; i++) {
            ...
            require(idExists[ids[i]], "Provided ID does not exist");
        }
    ...
    function forward(address to, uint256 amount) external onlyOwner {
