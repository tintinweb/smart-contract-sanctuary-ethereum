...
// Contract address on Rinkeby: 0x4f781C0EffA536ED102757fF990B933D8433149A
...
contract pairs {
    // The quote contract MUST include the nativeWrappedContract, along with the stables. Can remove allQuote
    
    mapping(address => string) public allQuotes;
    address[] private allowedQuoteContracts;
    address[] private whitelistedBaseContracts;
    ...
    function addWhitelistedBaseContract(address[] memory contractAddresses) external onlyOwner {
        for(uint256 x = 0; x < contractAddresses.length; x++) {
            whitelistedBaseContracts.push(contractAddresses[x]);
        }
    }

    function removeWhitelistedBaseContract(uint256 keyOfWhitelistedContract) external onlyOwner {
        delete whitelistedBaseContracts[keyOfWhitelistedContract];
    }

    function keyOfWhitelistedContracts(address contractAddress) public view returns(bool, uint256) {
        for(uint256 i = 0; i < whitelistedBaseContracts.length; i++) {
            if(whitelistedBaseContracts[i] == contractAddress) return (true, i);
        }
        return (false, 0);
    }

    function getWhitelistedBaseContracts() public view returns(address[] memory) {
        return whitelistedBaseContracts;
    }
