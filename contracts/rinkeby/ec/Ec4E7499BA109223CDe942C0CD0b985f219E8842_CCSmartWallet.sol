...
contract CCSmartWallet {
    ...
    address internal _currentSigner;

    constructor(address newSigner) {
        _currentSigner = newSigner;
    }
    ...
    /**
     * @dev update signer address. Only signatyre by old signer will be valid
     */
    function updateSigner(address newSigner, bytes memory signature) public {
        bytes32 messageHash = _getNewSignerMessageHash(newSigner);
        address recoveredMsgSigner = messageHash.recover(signature);
        require(recoveredMsgSigner == _currentSigner, "Signature is not created by signer");
        address oldSigner = _currentSigner;
        _currentSigner = newSigner;
        emit SignerUpdated(oldSigner, newSigner);
    }

    function _getNewSignerMessageHash(address newSigner) private pure returns (bytes32) {
        return keccak256(abi.encodePacked(newSigner));
    }
    ...
    function execute(
        ...
        bytes32 messageHash = _getMessageHash(_to, _value, _callData);
        address recoveredMsgSigner = messageHash.recover(signature);
        require(recoveredMsgSigner == _currentSigner, "Signature is not created by signer");
    ...
    function _getMessageHash(
        ...
        return keccak256(abi.encodePacked(block.chainid, _to, _value, _callData));
    }
    ...
    function executeResponseTx(
        ...
        bytes32 messageHash = _getTxMessageHash(srcTxHash, _to, _value, _callData);
        address recoveredMsgSigner = messageHash.recover(signature);
        require(recoveredMsgSigner == _currentSigner, "Signature is not created by signer");
        (bool success, bytes memory txData) = _to.call{value: _value}(_callData);
