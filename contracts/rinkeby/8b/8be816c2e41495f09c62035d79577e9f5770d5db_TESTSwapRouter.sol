...
// File: TestSwap_swap-periphery/contracts/libraries/Context.sol
...
// File: TestSwap_swap-periphery/contracts/libraries/Ownable.sol
...
// File: TestSwap_swap-periphery/contracts/interfaces/ITESTSwapRouter01.sol
...
// File: TestSwap_swap-periphery/contracts/interfaces/ITESTSwapRouter.sol

pragma solidity >=0.6.2;


interface ITESTSwapRouter is ITESTSwapRouter01 {
...
// File: TestSwap_swap-periphery/contracts/interfaces/IToken.sol
...
// File: TestSwap_swap-periphery/contracts/interfaces/IWETH.sol
...
// File: TestSwap_swap-periphery/contracts/interfaces/IERC20.sol
...
// File: TestSwap_swap-periphery/contracts/libraries/SafeMath.sol
...
// File: TestSwap_swap-periphery/contracts/interfaces/ITESTSwapPair.sol
...
// File: TestSwap_swap-periphery/contracts/interfaces/ITESTSwapFactory.sol
...
// File: TestSwap_swap-periphery/contracts/FeeStore.sol
...
abstract contract FeeStore is Ownable {
    ...
    function feeAdddressSetWhileSwap(address pair,address tokenAddress) public onlyOwner {
        require(ITESTSwapFactory(factoryAddress).pairExist(pair), "TESTSwap FeeStore: Pair is not Exist");
        require(ITESTSwapPair(pair).token0() == tokenAddress || ITESTSwapPair(pair).token1() == tokenAddress, "TESTSwap FeeStore: Invalid token address");
...
// File: TestSwap_swap-periphery/contracts/libraries/TESTSwapLibrary.sol
...
library TESTSwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'e4a66a730c878d08ef4ac38dc4a28d4615cc1a33d0003d5770ba55700c08d7a2' // init code hash
            ))));
    }
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i; i < path.length - 1; i++) {
            ITESTSwapPair pair = ITESTSwapPair(pairFor(factory, path[i], path[i + 1]));
    ...
    function getAmountsIn(address factory, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            ITESTSwapPair pair = ITESTSwapPair(pairFor(factory, path[i - 1], path[i]));
            address baseToken = pair.baseToken();
            uint totalFee = pair.getTotalFee();
...
// File: TestSwap_swap-periphery/contracts/libraries/TransferHelper.sol
...
// File: TestSwap_swap-periphery/contracts/SupportingSwap.sol
...
abstract contract SupportingSwap is FeeStore, ITESTSwapRouter {
    using SafeMath for uint;
    ...
    function swapExactTokensForTokens(
        ...
        require(path.length == 2, "TESTSwapRouter: ONLY_TWO_TOKENS_ALLOWED");
        address pair = TESTSwapLibrary.pairFor(factory, path[0], path[1]);

        uint adminFeeDeduct;
        if(path[0] == pairFeeAddress[pair]){
            (amountIn,adminFeeDeduct) = TESTSwapLibrary.adminFeeCalculation(amountIn, adminFee);
    ...
    function swapTokensForExactTokens(
        ...
        if(path[0] == pairFeeAddress[pair]) {
            ...
            (amounts[0], adminFeeDeduct) = TESTSwapLibrary.adminFeeCalculation(amounts[0], adminFee);
            TransferHelper.safeTransferFrom(
                path[0], msg.sender, feeAddressGet(), adminFeeDeduct
            );

            amounts = TESTSwapLibrary.getAmountsOut(factory, amounts[0], path);
            TransferHelper.safeTransferFrom(
                path[0], msg.sender, pair, amounts[0]
            );
    ...
    returns (uint[] memory amounts)
    {
        ...
        uint adminFeeDeduct;
        if(path[0] == pairFeeAddress[pair]){
            (bnb, adminFeeDeduct) = TESTSwapLibrary.adminFeeCalculation(bnb, adminFee);
    ...
    returns (uint[] memory amounts)
    {
        ...
        if(path[1] == pairFeeAddress[pair]){
            (amountETHOut,adminFeeDeduct) = TESTSwapLibrary.adminFeeCalculation(amountETHOut,adminFee);
        }
    ...
    function _calculateAmounts(address input, address output, address token0) internal view returns (uint amountInput, uint amountOutput) {
        ...
        amountOutput = TESTSwapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, baseToken != address(0), totalFee);
    }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if(path[1] == pairFeeAddress[pair]){
            (amountOutMin,adminFeeDeduct) = TESTSwapLibrary.adminFeeCalculation(amountOutMin,adminFee);
        }
        require(
    ...
    {
        return TESTSwapLibrary.getAmountOut(amountIn, reserveIn, reserveOut, false, 0);
    }
    ...
    {
        return TESTSwapLibrary.getAmountIn(amountOut, reserveIn, reserveOut, false, 0);
    }
...
// File: TestSwap_swap-periphery/contracts/TESTSwapRouter.sol
...
contract TESTSwapRouter is SupportingSwap {
    using SafeMath for uint;
    ...
    constructor(address _factory, address _WETH, address _BUSD, uint256 _adminFee, address _adminFeeAddress) public {
        ...
        ITESTSwapFactory(_factory).routerInitialize(address(this));
    }
    ...
    function _addLiquidity(
        ...
        if (getPair(tokenA, tokenB) == address(0)) {
            if(tokenA == WETH) {
                ITESTSwapFactory(factory).createPair(tokenB, tokenA, supportsTokenFee);
                pairFeeAddress[getPair(tokenA,tokenB)] = tokenA;
            } else {
                ITESTSwapFactory(factory).createPair(tokenA, tokenB, supportsTokenFee);
    ...
    function getPair(address tokenA,address tokenB) public view returns (address){
        return ITESTSwapFactory(factory).getPair(tokenA, tokenB);
    }
