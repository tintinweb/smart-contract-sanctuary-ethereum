...
import "@manifoldxyz/creator-core-solidity/contracts/ERC721CreatorUpgradeable.sol"; // need I version of this, only need initialize and registerExtension

contract NFTFactory {
    address public manifoldCreatorContract;

    event ContractCreated(address creator, address manifoldCreator);

    constructor(
        address _manifoldCreatorContract
    ) {
        manifoldCreatorContract = _manifoldCreatorContract;
    }

    function createCloneAndRegister(
        string memory _name,
        string memory _symbol,
        address _mintSongsOneExtAddress,
        string memory _baseUri) public {

        address manifoldCreator = Clones.clone(manifoldCreatorContract);
        ERC721CreatorUpgradeable(manifoldCreator).initialize(_name, _symbol);

        // While we are the owner we can register an extension
        ERC721CreatorUpgradeable(manifoldCreator).registerExtension(_mintSongsOneExtAddress, _baseUri);

        // Now we can transfer owner of the contract to the creator
        ERC721CreatorUpgradeable(manifoldCreator).transferOwnership(msg.sender);

        emit ContractCreated(msg.sender, manifoldCreator);
    }
}
