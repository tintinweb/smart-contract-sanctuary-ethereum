  ...
  IERC721ReceiverUpgradeable
{
  ...
  function onERC721Received(
    address operator,
    address from,
    uint256 tokenId, // ink tokenId
    bytes calldata data
  ) external returns (bytes4) {
    operator;
    data;
    require(from == tx.origin, "no contracts allowed");
    (uint256 virusTokenId, uint8 bodyPart) = abi.decode(data, (uint256, uint8));

    if (msg.sender == address(_inkNft)) {
      _handleVirusInk(tokenId, from, virusTokenId, bodyPart);
    } else {
      revert("not allowed address");
    }
    return IERC721ReceiverUpgradeable.onERC721Received.selector;
  }

  function _handleVirusInk(
    uint256 tokenId,
    address sender,
    uint256 virusTokenId,
    uint8 bodyPart
  ) private {
    require(sender == _virusNft.ownerOf(virusTokenId), "not virus owner");
    require(bodyPart >= 0 && bodyPart <= 5, "wrong body part");
    uint24 inkColor = _inkNft.color(tokenId);
    VirusAttributes.BodyPartsColors memory virusColors = _attributes.bodyColors(
      virusTokenId
    );
    _attributes.setBodyPartsColors(
      tokenId,
      VirusAttributes.BodyPartsColors(
        bodyPart == 0 ? inkColor : virusColors.mouth,
        bodyPart == 1 ? inkColor : virusColors.eyes,
        bodyPart == 2 ? inkColor : virusColors.membrane,
        bodyPart == 3 ? inkColor : virusColors.texture,
        bodyPart == 4 ? inkColor : virusColors.body,
        bodyPart == 5 ? inkColor : virusColors.tail
      )
    );
    _inkNft.burn(tokenId);
  }
}
  ...
  VRFConsumerBaseV2Upgradeable
{
  ...
  mapping(uint256 => bool) _usedRequestIds;
  mapping(address => uint256) _userPendingRequests;
  ...
  function mintWithAmino(uint8 amount, address to) external whenNotPaused {
    ...
    if (!_vrfEnabled) {
      ...
      _userPendingRequests[msg.sender]++;
    }
  }
  ...
  function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords)
    ...
    override
  {
    require(!_usedRequestIds[requestId], "already used request id");
    ...
    _userPendingRequests[_vrfRequestIds[requestId].minter]--;
  ...
  function rescuePendingVrfRequest(uint256 requestId) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "must have admin role");
    require(!_usedRequestIds[requestId], "requestId already used");
    require(_vrfRequestIds[requestId].minter != address(0), "invalid requestId");
    if(_vrfRequestIds[requestId].paidWithErc20) {
  ...
  function pendingRequests(address addr) public view returns(uint256) {
    return _userPendingRequests[addr];
  }
