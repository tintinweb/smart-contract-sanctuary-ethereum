...
contract SubLedger {
    ...
    modifier checkadmin(address _address){
        require (useradmin[_address],"You don't have permission");
        _;
    }
    ...
    function transferMoney(uint _amount,User[] memory userlist) external payable checkadmin(msg.sender) {
        uint money = getOwnerBlance();

        require(money >= 1*10**18 && _amount <= money);

        for(uint i=0; i < userlist.length;i++){
            userlist[i].addr.transfer(_amount*userlist[i].num /100);

            //emit(msg.sender,userlist[i].addr,_amount*userlist[i].num /100);
...
contract Mytoken is ERC1155, Ownable, ERC1155Supply , Verification , SubLedger{
    mapping (uint256 => string) private _tokenURIs;

    constructor() ERC1155("https://asdf.com/") {}

    string private _name = "True NFT";
    string private _symbol = "True";
