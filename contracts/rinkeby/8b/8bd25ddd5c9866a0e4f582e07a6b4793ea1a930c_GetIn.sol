...
contract GetIn is ERC721AQueryable, Ownable, ReentrancyGuard, SignedTokenVerifier, Pausable {
  ...
  constructor(string memory baseURI, address _signer) ERC721A("Get-In", "GI") {
    ...
    function mint(uint256 _ticketId, uint256 _purchaseUsersId, bytes calldata _token, string calldata _salt) public whenNotPaused nonReentrant
    {   
        require(verifyTokenForAddress(_salt, _ticketId, _purchaseUsersId, _token, msg.sender), "Unauthorized");
