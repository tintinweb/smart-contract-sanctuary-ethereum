...
contract MATICALPHA {
	...
	uint256 constant public TIME_STEP = 5; //30 seconds for testing
	...
	function _reinvest( uint256 _plan, address payable sender, uint256 depositID) private {
		...
		require(depositID < user.deposits.length);
		...
		uint256 userPlanBalance = getPlanBalanceByDeposit(sender, depositID);
		uint256 userHoldBalance = getHoldBalanceByDeposit(sender, depositID);
		uint256 totalDepositBalance = userPlanBalance.add(userHoldBalance);
        ...
        totalUsers = totalUsers.add(1);
		emit NewDeposit(sender, uplan, percent, totalDepositBalance, _profit, block.timestamp, _finish);
	}
