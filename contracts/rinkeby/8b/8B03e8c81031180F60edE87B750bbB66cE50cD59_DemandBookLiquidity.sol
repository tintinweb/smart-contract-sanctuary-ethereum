...
contract DemandBookLiquidity is IDemandBookLiquidity, Initializable, AbstractDependant {
    ...
    mapping(address => mapping(uint256 => ProviderInfo)) internal _providerInfo; // provider -> demandIndex -> ProviderInfo

    // liquidity withdraw
    EnumerableSet.AddressSet internal _withdrawRequestProvidersList; // list of provider who requested
    mapping(address => EnumerableSet.UintSet) private _providerWithdrawlRequestsDemandsList; // provider -> demandIndexes
    ...
    function addLiquidity(uint256 demandIndex, uint256 _liquidityAmount) external override {
        ...
        _providerInfo[msg.sender][demandIndex].liquidityAmount = _providerInfo[msg.sender][
            demandIndex
        ]
            .liquidityAmount
    ...
    ) external override onlyDemandBook {
        // transfer from DemandBookLiquidity to CapitalPool : liquidityAmount
        uint256 stblLiquidityAmount =
            DecimalsConverter.convertFrom18(_liquidityAmount, stblDecimals);
        capitalPool.addDemandProvidersHardSTBL(demandIndex, stblLiquidityAmount);

        // transfer from DemandBookLiquidity to CapitalPool : reinsurancePoolAmount
        uint256 stblReinsurancePoolAmount =
            DecimalsConverter.convertFrom18(_reinsurancePoolAmount, stblDecimals);
        capitalPool.addDemandHoldersHardSTBL(stblReinsurancePoolAmount);

        uint256 stblAmount = stblLiquidityAmount.add(stblReinsurancePoolAmount);
        stablecoin.safeTransfer(address(capitalPool), stblAmount);
    }
    ...
    function requestLiquidityWithdrawal(uint256 demandIndex) external override {
        ...
        require(
            status == WithdrawalStatus.NONE || status == WithdrawalStatus.EXPIRED,
            "DBL: The withdrawal is already requested"
        );
        uint256 liqudityToWithdraw = _providerInfo[msg.sender][demandIndex].liquidityAmount;
    ...
    function _requestLiquidityWithdrawal(
        ...
        uint256 readyToWithdrawDate = block.timestamp.add(capitalPool.getWithdrawPeriod());
        ...
        returns (WithdrawalStatus)
    {
        ...
        if (
            block.timestamp >=
            liquidityWithdrawalInfo[demandIndex][_provider].readyToWithdrawDate.add(
        ...
        returns (uint256 totalLiquidityAmount)
    {
        ...
        for (uint256 i = 0; i < _limit; i++) {
            ...
            totalLiquidityAmount = totalLiquidityAmount.add(
                _getAllPendingWithdrawalRequestByProvider(
                    provider,
                    _getProviderWithdrawlRequestsDemandsListCount(provider)
                )
            );
        }
    }

    function _getProviderWithdrawlRequestsDemandsListCount(address _provider)
        internal
        view
        returns (uint256)
    {
    ...
    /// @dev use it with _getProviderWithdrawlRequestsDemandsListCount
    function _getAllPendingWithdrawalRequestByProvider(address _provider, uint256 _limit)
        internal
        view
        returns (uint256 totalLiquidityAmount)
    {
        ...
        for (uint256 j = 0; j < _limit; j++) {
            ...
            if (
                currentStatus == WithdrawalStatus.NONE || currentStatus == WithdrawalStatus.EXPIRED
            ) {
                continue;
            }
    ...
    function withdrawLiquidity(uint256 demandIndex) external override {
        ...
        require(_providerInfo[msg.sender][demandIndex].liquidityAmount > 0, "DBL: No liquidity");
    ...
    function _withdrawLiquidityClosed(uint256 demandIndex, address _provider) internal {
        ...
        if (!_providerInfo[_provider][demandIndex].isRewardWithdrawn) {
            rewardToWithdraw = _getReward(demandIndex);
        }

        // liquidity withdraw
        uint256 liquidityToWithdraw = _providerInfo[_provider][demandIndex].liquidityAmount;
        _providerInfo[_provider][demandIndex].liquidityAmount = 0;
    ...
    function _withdrawLiquidityEnded(uint256 demandIndex, address _provider) internal {
        ...
        uint256 _actualAmount =
            capitalPool.withdrawDemandLiquidity(demandIndex, _provider, liquidityAmountConverted);
        ...
        if (liquidityAmountConverted == 0) {
            ...
            _providerInfo[_provider][demandIndex].liquidityAmount = liquidityWithdrawalInfo[
                demandIndex
            ][_provider]
                .liquidityAmount;
        ...
        returns (uint256 rewardAmountUser)
    {
        ...
        if (depositedLiquidity > 0) {
            rewardAmountUser = _providerInfo[_provider][demandIndex]
                .liquidityAmount
                .mul(rewardAmount)
                .div(depositedLiquidity);
        }
    }
        ...
        returns (uint256)
    {
        ...
        uint256 expectedRewardAmount =
            premiumAmount.mul(ACTIVE_REWARD_PERCENTAGE).div(PERCENTAGE_100);

        return _liquidityAmount.mul(expectedRewardAmount).div(coverageAmount);
    }

    function getReward(uint256[] memory demandIndexes) external override {
        uint256 rewardToWithdraw;
        for (uint256 i = 0; i < demandIndexes.length; i++) {
            ...
            require(
                _providerInfo[msg.sender][demandIndex].liquidityAmount > 0,
    ...
    function _getReward(uint256 demandIndex) internal returns (uint256 rewardToWithdraw) {
        if (_providerInfo[msg.sender][demandIndex].isRewardWithdrawn == false) {
            rewardToWithdraw = calculateReward(demandIndex, msg.sender);
            _providerInfo[msg.sender][demandIndex].isRewardWithdrawn = true;
        }
    }
...
interface IDemandBook {
    struct DemandInfo {
        ...
        uint256 currentLiquidity; // liquidity taking into account the withdrawals
        uint256 premiumAmount; // premiumAmount = rewardAmount + reinsurancePoolAmount
        bool isPremiumWithdrawn;
        uint256 reinsurancePoolAmount;
        uint256 epochAmount;
    ...
    function countCreatedDemands(address user) external view returns (uint256);

    function countInvestedDemands(address user) external view returns (uint256);
