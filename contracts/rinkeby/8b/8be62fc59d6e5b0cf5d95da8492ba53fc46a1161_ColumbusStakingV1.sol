...
//  total amount staked on contract
    ...
    */
    function calculateReward(address _rewardAddress) public view returns(uint256, uint256, uint256, uint256){
		...
		if(balances > 0){
		    ...
		    */
            //consider day
            timeFactor = timeDifferences / emissionInterval;
            userPercentage = ((stakedBalance[_rewardAddress] * 100) / _totalStakedAmount);
            rewards = (userPercentage * (rewardTokenEmissionRate * timeFactor)) / 100;
		}
		return (rewards, timeDifferences, timeFactor, userPercentage);
    }
    ...
    */
    function claimMyReward() external nonReentrant whenNotPaused {
        ...
        (uint256 reward, uint256 timeDifferences, uint256 timeFactor, uint256 userPercentage) = calculateReward(msg.sender);
