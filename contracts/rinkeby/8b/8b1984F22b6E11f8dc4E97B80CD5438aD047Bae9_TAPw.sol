...
contract TAPw is ERC20, AccessControl, ERC20Burnable {
  ...
  modifier onlyWithinLimits(uint256 fee, uint256 feeDecimals) {
    require(fee > 0, "The fee must be greater than 0");
    require((10 * fee) <= 25 * 10**feeDecimals, "The fee may not exceed 2.5%");
    _;
  }
  ...
  function setFoundationFee(uint256 fee, uint256 feeDecimals)
    public
    onlyAdmin
    onlyWithinLimits(fee, feeDecimals)
  {
    foundationFee = fee;
  ...
  function setVaultFee(uint256 fee, uint256 feeDecimals)
    public
    onlyAdmin
    onlyWithinLimits(fee, feeDecimals)
  {
