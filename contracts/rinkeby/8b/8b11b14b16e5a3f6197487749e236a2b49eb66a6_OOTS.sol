...
contract OOTS is ERC721A, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "OOTS - The Contract is Paused");
        require(
            totalSupply() + _amount <= maxSupply,
            "OOTS - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "OOTS - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "OOTS - max NFT per address exceeded"
            );
            if (ownerMintedCount + _amount <= 1) {
                require(msg.value >= 0 * _amount, "OOTS - insufficient ethers");
            } else {
                require(
                    msg.value >= cost * _amount,
                    "OOTS - insufficient ethers"
                );
            }
        }
