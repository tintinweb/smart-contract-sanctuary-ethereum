...
contract SwapFactory {
    ...
    function createSwap(
        ...
        Swap swap = new Swap();
        TransferHelper.safeTransferFrom(_chainlinkToken, msg.sender, address(swap), _chainlinkDepositAmount);
...
contract Swap is Initializable, OwnableUpgradeable, ChainlinkClientUpgradeable {
    ...
    function swap(uint256 _amountIn, address _from, address _to) external {
        ...
        if (_from == tokenA) {
            ...
            uint256 amountB = amountA * lastPriceFeed[SELL_INDEX] / (10**8);
            ...
            uint256 amountA = amountB * (10**8) / lastPriceFeed[BUY_INDEX];
    ...
    function addLiquidity(uint256 amountA, uint256 amountB) external onlyOwner {
        require((amountA * rate[BUY_INDEX])/10**8 == amountB, "amountA should be equal with amountB");
        TransferHelper.safeTransferFrom(tokenA, msg.sender, address(this), amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, address(this), amountB);
        reserveA = reserveA + amountA;
        reserveB = reserveB + amountB;
   ...
   function setTradeFee(uint256 _newTradeFee) external onlyOwner {
    ...
    function getRate(uint256 _rateIndex) public view returns (uint256) {
