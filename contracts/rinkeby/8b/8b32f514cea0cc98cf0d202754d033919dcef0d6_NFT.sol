...
contract NFT is ERC721, Ownable {
    ...
    bool public frozenSupply;
    uint256 public issued;

    string public tokenBaseURI;
    string public tokenURIExt;
    ...
    function mint(
        ...
        require(
            isAllowlistAddress[_msgSender()], 
            "Address is not allowlisted"
        );
        _mint(_msgSender());
        isAllowlistAddress[_msgSender()] = false;
    }
    ...
    function mintOwner(
        ...
        require(
            issued + _addresses.length <= maxSupply, 
            "Minting limit reached"
        );
        for (uint256 i = 0; i < _addresses.length; i++) {
            _safeMint(_addresses[i], issued + i);
        }
        issued += _addresses.length;
    }
    ...
    function removeAllowlistAddresses(
        address[] calldata wAddresses
    ) public onlyOwner {
        for (uint i = 0; i < wAddresses.length; i++) {
            isAllowlistAddress[wAddresses[i]] = false;
        }
    }

    function setURI(
        string calldata _baseURI,
        string calldata _URIExt
    ) external onlyOwner {
        require(
            !frozen, 
            "Contract is frozen"
