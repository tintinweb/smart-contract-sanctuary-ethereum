...
contract NftPuzzle is ERC721A, Ownable, ReentrancyGuard {
    ...
    function makePuzzleByKuma(uint256 puzzles) external payable
    {
        ...
        uint256 qtyFreeMint = isTryingToMintAOneFreeMint ? 1 : 0;

        if (canMintAFree && !isTryingToMintAOneFreeMint) {
            require(msg.value >= (KUMA_HOLDERS_PRICE * (puzzles - availableFreeKumas)), "Error");

            uint256 total = KUMA_HOLDERS_PRICE * puzzles;
            uint256 totalWithMsgValue = msg.value * puzzles;
        ...
        if (itsAPaidMint) {
            require(msg.value >= (KUMA_HOLDERS_PRICE * puzzles) , "Incorrect ETH value sent");
        }
