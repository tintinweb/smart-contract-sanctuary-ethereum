...
contract CanvasN is ERC721 {
    ...
    function withDrawBalance(uint256 _ownerPer) public payable onlyOwner {
        ...
        uint256 _adminPer = 100 - _ownerPer;

        (bool success1, ) = payable(owner).call{value: (balance / 100) * _ownerPer}("");
        require(success1, "Failed to send to Wallet Owner");

        (bool success2, ) = payable(admin).call{value: (balance / 100) * _adminPer}("");
