    ...
    ReentrancyGuard
{
    ...
    {
        require(_weightLoss > 0 && _weightLoss <= tradeCoin[_tokenId].weight, 
            "Invalid Weightloss"
        );
        ...
        uint256 newWeight = tradeCoin[_tokenId].weight - _weightLoss;
        ...
        nonReentrant
    {
        // When not paying in Fiat pay but in Eth
        if (!_payInFiat) {
            require(_paymentInWei >= msg.value, "Can't be less than requested");
        ...
        onlyInformationHandlerOrAdmin
    {
        require(_tokenIds.length == _rootHash.length, 
            "The amount of TokenIds should update the same amount of rootHashes"
        );
    ...
    function burn(
        ...
        require(
            _documents.docHash.length == _documents.docType.length && 
                (_documents.docHash.length <= 2 || _documents.docType.length <= 2), 
            "Wrong amount of docTypes or docHashes"
        );
