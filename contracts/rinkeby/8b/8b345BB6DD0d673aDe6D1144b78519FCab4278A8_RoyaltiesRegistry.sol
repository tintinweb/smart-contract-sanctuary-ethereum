...
contract RoyaltiesRegistry is ERC165, OwnableUpgradeable, IRoyaltiesRegistry {
     ...
     *                               MODIFIERS
     * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    modifier isMinter(address collectionAddress) {
        if (!_hasMinterRole(collectionAddress)) {
            revert NotApproved();
        }
        _;
    }

    modifier isCollectionAdmin(address collectionAddress) {
        if (!_hasCollectionAdminRole(collectionAddress)) {
            revert NotApproved();
        }
        _;
    }
    ...
    /// @dev Returns whether `caller` is the minter of the `collectionContract` token.
    ...
    ) public isCollectionAdmin(collectionAddress) {
    ...
    function registerEditionPrimaryRoyaltyReceivers(
        ...
        uint256 tokenId,
        RoyaltyReceiver[] memory royaltyReceivers
    ) public isMinter(collectionAddress) {
        RoyaltiesRegistryState storage state = _getRoyaltiesRegistryState();
    ...
    function primaryRoyaltyInfo(address collectionAddress, uint256 tokenId)
        external
        view
        isCollectionAdmin(collectionAddress)
