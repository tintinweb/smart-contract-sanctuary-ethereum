...
contract Sale is Initializable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    modifier isValidSale(uint256 id) {
        require(id > 0 && id <= lastedId, "Invalid sale id");
        _;
    }
    ...
    function createRaiseSingle(uint256 _projectId, uint256[] memory _tokenIds, uint256[] memory _raisePrices) external payable onlyManagerWithProjectId(_projectId) {
        ...
        for (uint256 i; i < _tokenIds.length; i++) {
            require(_raisePrices[i] > 0, "create: invalid price");
    ...
    function createDutchSingle(uint256 _projectId, uint256[] memory _tokenIds, uint256[] memory _maxPrices, uint256[] memory _minPrices, uint256[] memory _priceDecrementAmts) external onlyManagerWithProjectId(_projectId) {
        ...
        for (uint256 i; i < _tokenIds.length; i++) {
            require(_maxPrices[i] > _minPrices[i] && _minPrices[i] > 0, "create: invalid price");
            require(_priceDecrementAmts[i] > 0, "create: invalid Price Decrement Amt");
            SaleInfo storage sale = sales[++_id];
            sale.id           = _id;
            sale.projectId    = _projectId;
            sale.token        = _project.token;
    ...
    function bidSingle(uint256 _saleId) external payable onlyWinner(_saleId) isValidSale(_saleId) {
    ...
    function bidMulti(uint256 _saleId, uint256 _amount) external payable onlyWinner(_saleId) isValidSale(_saleId) {
    ...
    function _sharing(ProjectInfo memory _project, SaleInfo memory _sale, uint256 _amount) private {
        ...
        if (_project.isCreatedByAdmin) {
            ...
            remainingProfit = msg.value.sub(adminFee);
            if (royaltyFee > remainingProfit) royaltyFeeAble = remainingProfit;

            remainingProfit = remainingProfit.sub(royaltyFeeAble);            
        }

        // Transfer fee and profit
        if (royaltyFeeAble > 0) {
            payable(royaltyReceiver).transfer(royaltyFeeAble);
        }
        ...
        if (remainingProfit > 0) {
            payable(project.getManager(_project.id)).transfer(remainingProfit);
        }
    ...
    function close(uint256 _projectId, uint256[] memory _saleIds) external onlyManagerWithProjectId(_projectId) {
