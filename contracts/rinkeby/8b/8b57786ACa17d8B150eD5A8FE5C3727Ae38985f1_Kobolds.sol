...
contract Kobolds is ERC721A,Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("Kobolds","KBT",maxPerWallet,maxWLmint) {
    }

    // public function  
    function freeListMint() external payable mintCompliance(1){
    ...
    function OGMint(bytes32[] memory proof) 
            external 
            payable 
            isValidMerkleProof(proof,_whiteListRoot) {
        require(curOpenState,"not open yet!");
    ...
    function canWLMint(bytes32[] calldata merkleProof) view external returns (bool res){
        require(_whiteListRoot != "", "root not set");
        if (MerkleProof.verify(merkleProof,_whiteListRoot,keccak256(abi.encodePacked(msg.sender))) && ogMinted[msg.sender] == false)
        {
            return true;
        }
        return false;
