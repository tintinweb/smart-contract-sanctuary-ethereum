...
contract UnipilotFarm is IUnipilotFarm, ReentrancyGuard {
        ...
        isLimitActive
    {
        ...
        require(IERC20(_pool).balanceOf(caller) >= _amount, "NEB");
    ...
    function unstakeLp(address _pool, uint256 _amount) external override {
        ...
        address caller = msg.sender;
        PoolInfo storage poolState = poolInfo[_pool];
        UserInfo storage userState = userInfo[_pool][caller];
        require(
        ...
        onlyGovernance
    {
        for (uint256 i = 0; i < _pools.length; i++) {
            ...
            bool status = false;
            poolWhitelist[_pools[i]] = status;
            emit PoolWhitelistStatus(_pools[i], status);
        }
    }
    ...
    function toggleBooster(address _pool, address _user)
        external
        override
        onlyStake
    {
        ...
        )
    {
        ...
        if (userState.boosterActive) {
            uint256 boostMultiplier = IUnipilotStake(stakeContract)
                .getBoostMultiplier(msg.sender, userState.pool);
            uint256 boostedReward = (reward.mul(boostMultiplier)).div(1e18);
...
interface IUnipilotFarm {
    ...
    function blacklistPools(address[] memory _pool) external;
    ...
    function toggleBooster(address _pool, address _user) external;
}
...
interface IUnipilotStake {
    function getBoostMultiplier(address userAddress, address poolAddress)
        external
        view
        returns (uint256);
