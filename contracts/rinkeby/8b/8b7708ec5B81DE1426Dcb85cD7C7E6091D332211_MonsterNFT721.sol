    ...
    AccessControl
{
    ...
    function updateMintingfee(uint256 _mintingFee) external  onlyRole(ADMIN_ROLE) {
    ...
    function batchMint(    
        ...
        require(mintedCount[msg.sender] < 3, "Minting limit exceeded");
        require((mintingFee * qty) == msg.value, "Insufficiet minting fee");
        for(uint256 i = 0; i < qty ; i++) {
            require(mintedCount[msg.sender] < 3, "Invalid Quantity");
        ...
        returns (string memory)
    {
        ...
        if(_exists(tokenId) && isRevealed) {
            return super.tokenURI(tokenId);
        } 
        else {
            return "https://gateway.pinata.cloud/ipfs/QmZwzaoPbGbQ4BFFF1ceweHaysVGL3BJaR8WUZs3WzpE6f";
        }
    ...
    function _beforeTokenTransfer(
        ...
        if(_tokenIdTracker.current() == 9999) {
            isRevealed = true;
        }
