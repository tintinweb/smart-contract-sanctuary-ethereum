...
contract StarXCelebrity is Ownable, IStarXCelebrity {
    //---------------------------------- v1 - contract events - start ------------------------------------
    event CelebContractCreated(address indexed entity);
    event AssetContractCreated(
        address indexed createdBy,
        address indexed celebAddress,
    ...
    // TODO: fn to update this
    address private m_starXAddressRegistry;

    bool private m_ownershipTransferredAtDeployment;

    uint256 private m_deployedSecurityCount;
    mapping(uint256 => address) private m_securityLookup;
    mapping(address => uint256) private m_securityLookupRev;
    ...
    modifier modOnlyApproved() {
        ...
        require(
            isOwner || isStarXAdmin,
            "caller must be either contract owner or StarXAdmin"
        );
        _;
    }
    ...
    // TODO: fn doc
    constructor(address _entityAddress, address _starXAddressRegistry) {
        // transfer ownership of this contract to specified "entity"
        _transferOwnership(_entityAddress);

        // set flag to indicate that one-time ownership transfer has transpired
        m_ownershipTransferredAtDeployment = true;

        // set starting address of contract = StarXAddressRegistry
        m_starXAddressRegistry = _starXAddressRegistry;

        // emit event
        emit CelebContractCreated(_entityAddress);
    }

    //    function initialize(address _entityAddress, address _starXAddressRegistry)
    //        public
    //        initializer
    //    {
    //        // TODO: relax this for now to get a deployment out the door
    //        //        // checks: ensure that contract is deployed by approved StarXAdmin
    //        //        require(
    //        //            IStarXAddressRegistry(_starXAddressRegistry).adminNftContracts(
    //        //                _msgSender()
    //        //            ),
    //        //            "contract can only be deployed by registered StarXAdmin"
    //        //        );
    //
    //        // init parent contract
    ...
    function _transferOwnership(address newOwner) internal override {
        require(
            !m_ownershipTransferredAtDeployment,
            "ownership once at deployment and cannot be transferred thereafter"
        );
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return interfaceId == StarXSecurityLib.INTERFACE_ID_IStarXCelebrity;
    }

    // TODO: fn doc
    // TODO: reinstate mod = modOnlyApproved
    function deployFinancialSecurityAssetContract(
        IRequest.DeployAssetContractRequest calldata _req
    ) external virtual override returns (address) {
        //        console.log("IN[deployFinancialSecurityAssetContract]");

        //        // TODO: DEBUG:
        //        string memory arg1 = _name;
        //        string memory arg2 = _symbol;
        //        string memory arg3 = _description;
        //        string memory uri = "some uri";

        // init new contract
        console.log("init new asset...");
        console.log("   _msgSender(): %s", _msgSender());
        console.log("   _name: %s", _req.assetName);
        //        console.log("   _symbol: %s", arg2);
        //        console.log("   _description: %s", arg3);
        //        console.log("   _starXAddressRegistry: %s", m_starXAddressRegistry);
        //        console.log("   _custodyWallet: %s", _custodyWallet);
        //        console.log("   uri: %s", uri);
        StarXFinancialSecurityAsset security = new StarXFinancialSecurityAsset(
            _req,
            m_starXAddressRegistry
        );
        console.log("init new asset...DONE");

        // increment count of deployed contracts
        m_deployedSecurityCount++;
        //        console.log("new asset: %s", address(security));
        //        console.log("new asset count: %d", m_deployedSecurityCount);

        // add newly-deployed contract to local lookup
        m_securityLookup[m_deployedSecurityCount] = address(security);
        m_securityLookupRev[address(security)] = m_deployedSecurityCount;

        // TODO: do away with this event?
        // emit event
        emit AssetContractCreated(
            _msgSender(),
            owner(),
            address(security),
            _req.assetName,
    ...
    function mintNewSecurity(
        ...
        // check that specified 1155 contract address belongs to this entity
        require(
            m_securityLookupRev[_securityContract] > 0,
            "DNE: specified contract address of StarX1155Tradable"
        );
...
contract StarXFinancialSecurityAsset is ERC1155Burnable {
    ...
    // TODO: doc
    event AddressRegistryContractAddressUpdated(
        address indexed newAddress,
        string indexed assetContractName
    );
    ...
    // TODO: getter/updater for relevant fields?
    ...
    // custody wallet(s)
    uint256 private m_custodyWalletCount;
    mapping(uint256 => address) private m_custodyWallets;
    ...
    modifier modTokenIdExists(uint256 _tokenId) {
        require(_tokenExists(_tokenId), "DNE: token id");
        _;
    }
    ...
    modifier modIsCelebDeployer() {
        require(
            _msgSender() == m_deployingStarXCelebrity,
            "caller is not StarXCelebrity contract which deployed this one"
        );
        _;
    }

    modifier modOnlyAdmin() {
        require(_isStarXAdmin(_msgSender()), "caller must be StarXAdmin");
        _;
    }

    modifier modOnlyApproved() {
        // caller = deployer?
        bool isDeployer = _msgSender() == m_deployingStarXCelebrity;

        // caller = registered StarXAdmin?
        bool isStarXAdmin = !isDeployer && _isStarXAdmin(_msgSender());
        // halt function execution if neither of those conditions are true
        require(
            isDeployer || isStarXAdmin,
    ...
    constructor(
        IRequest.DeployAssetContractRequest memory _req,
        address _starXAddressRegistry
    ) ERC1155("") {
        // TODO: omit for now for the sake of achieving a rinkeby deployment
        //        require(
        //            StarXSecurityLib.isContractStarXCelebrity(_msgSender()),
        //            "contract deployer must be of contract type: StarXCelebrity"
        //        );
        console.log("IN[Asset:Ctor]");
        console.log("   _msgSender(): %s", _msgSender());
        //        console.log("   _name: %s", _name);
        //        console.log("   _symbol: %s", _symbol);
        //        console.log("   _description: %s", _description);
        //        console.log("   _starXAddressRegistry: %s", _starXAddressRegistry);
        //        console.log("   _custodyWallet: %s", _custodyWallet);
    ...
    function _tokenExists(uint256 _tokenId) private view returns (bool) {
        ...
        return m_tokenToFinancialSecurityAsset[_tokenId].supply > 0;
    }

    function _isStarXAdmin(address _caller) private view returns (bool) {
        return
            IStarXAddressRegistry(m_starXAddressRegistry).adminNftContracts(
                _caller
            );
        ...
        modOnlyAdmin
    {
        require(
            ...
            "invalid address arg for StarXAddressRegistry"
        );
        m_starXAddressRegistry = _newAddress;

        // emit event
        emit AddressRegistryContractAddressUpdated(_newAddress, m_name);
    }
    ...
    function updateListingStatus(uint256 _tokenId, bool _status)
        ...
        modOnlyApproved
    {
        bool oldStatus = m_tokenToFinancialSecurityAsset[_tokenId].isListed;
        m_tokenToFinancialSecurityAsset[_tokenId].isListed = _status;

        // emit event
        emit UpdateListingStatus(_tokenId, oldStatus, _status, m_name);
    }

    function _getTokenSupply(uint256 _tokenId) private view returns (uint256) {
        address tokenWallet = m_tokenToCustodyWallet[_tokenId];
        require(
            tokenWallet != address(0),
            "no custody wallet exists for that token"
        );
        return balanceOf(tokenWallet, _tokenId);
    }
        ...
        returns (uint256)
    {
        ...
        m_tokenToFinancialSecurityAsset[m_tokenId] = AssetInfo({
            ...
            hashedInvestmentContractUri: _req.hashedInvestmentContractUri
        });
        ...
        // TODO: do away with this event? not picked up by admin call
        // emit event
    ...
    {
        bytes32 tokenUri = m_tokenToFinancialSecurityAsset[_tokenId]
            .hashedInvestmentContractUri;
