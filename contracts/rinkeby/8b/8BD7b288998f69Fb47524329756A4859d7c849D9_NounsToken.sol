...
contract NounsToken is ERC721Tradable, INounsToken, ReentrancyGuard {
    ...
    /**
     * override the center part of card with a third-party nft specified by its
     * contract address and token id
     */
    function setOverrideNft(uint256 tokenId, address override_contract, uint256 override_token_id) external override {
        require(_exists(tokenId), 'NounsToken: URI query for nonexistent token');
        require(_msgSender() == ownerOf(tokenId), 'Only owner can update the card');

        // todo: verify the override_contract is one of the supported list
        seeds[tokenId].override_contract = override_contract;
        seeds[tokenId].override_token_id = override_token_id;
...
interface INounsToken is IERC721 {
    ...
    function setOverrideNft(uint256 tokenId, address override_contract, uint256 override_token_id) external;
}
