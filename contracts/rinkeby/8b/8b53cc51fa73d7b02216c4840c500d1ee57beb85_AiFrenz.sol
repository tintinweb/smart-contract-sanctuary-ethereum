...
contract AiFrenz is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  mapping(address => uint) public tokensMintedByAddress;
    ...
    constructor(string memory _theBaseURI, string memory _notRevealedURI) ERC721("Aifrenz", "Aifrenz"){
    ...
    **/
    ...
    function setUpWhitelistFreemint() external onlyOwner {
  ...
  function WhitelistFreemint(uint256 _ammount) external payable nonReentrant {
        ...
        require(sellingStep == Steps.Freemint, "Sorry, sale has not started yet.");
        ...
        require(numberNftSold + _ammount <= FREE_SUPPLY, "Freeminting is almost done and we don't have enought NFTs left.");
        ...
        if(numberNftSold + _ammount == FREE_SUPPLY) {
    ...
    function PresaleMint(uint256 _ammount) external payable nonReentrant {
        ...
        require(_ammount <= max_mint_allowed, "You can't mint more than 3 tokens");
        //If the user try to mint any non-existent token
        require(numberNftSold + FREE_SUPPLY + _ammount <= MAX_SUPPLY, "Sale is almost done and we don't have enought NFTs left.");
        //Add the ammount of NFTs minted by the user to the total he minted
        nftsPerWallet[msg.sender] += _ammount;
        //If this account minted the last NFTs available
        if(numberNftSold + FREE_SUPPLY + _ammount <= MAX_SUPPLY) {
            sellingStep = Steps.SoldOut;   
        }
        //Minting all the account NFTs
        for(uint i = 1 ; i <= _ammount ; i++) {
