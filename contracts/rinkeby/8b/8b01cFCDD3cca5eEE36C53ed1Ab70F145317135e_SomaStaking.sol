...
contract SomaStaking is ISomaStaking, ReentrancyGuardUpgradeable, TokenRecoveryUpgradeable, GuardableUpgradeable {
    ...
    mapping(address => uint256)     private _adminRewards;
    ...
    function adminRewards(address _asset) external override view returns (uint256) {
        return _adminRewards[_asset];
    }

    function userDebt(address _account, address _asset) external override view returns (uint256) {
    ...
    function userClaimable(address _account, address _asset) external override view returns (uint256) {
    ...
    function userStake(address _account) external override view returns (uint256) {
    ...
    function requestClaim(address _asset) external override nonReentrant {
        address _sender = _msgSender();
        UserInfo storage _user = _users[_sender];
        uint256 _userStake = _user.stake;
    ...
    function claim(address _asset) external override nonReentrant {
        address _sender = _msgSender();
        UserInfo storage _user = _users[_sender];
        ...
        require(block.timestamp - request.timestamp >= config_.claimDuration, "Staking: INSUFFICIENT_TIME");

        _user.claimable[_asset] -= request.amount;
        IERC20(_asset).safeTransfer(_sender, request.amount);

        emit Claimed(_sender, request.amount);
    }

    function immediateClaim(address _asset) external override nonReentrant {
        require(_rewardTokens.contains(_asset), "Staking: INVALID_ASSET");
        ...
        _update(_totalStake);
        _syncUser(_user, _userStake, _userStake); // TODO maybe refactor this to be one input for _userStake ?

        uint256 _claimable = _user.claimable[_asset];
        require(_claimable > 0, "Staking: NO_REWARDS");

        uint256 _adminFee = _claimable.applyPercent(config_.earlyClaimFee);

        _adminRewards[_asset] += _adminFee;
        IERC20(_asset).safeTransfer(_sender, _claimable - _adminFee);

        delete _user.claimable[_asset];
        delete _user.claimRequest[_asset];

        emit ClaimedImmediate(_claimable, _adminFee, _claimable - _adminFee, _msgSender());
    }
    ...
    function stake(uint256 _amount) external override nonReentrant {
        ...
        _totalStake = totalStake_ + _amount;

        emit Staked(_sender, _amount);
    }

    function claimAdmin(address _asset, address _to) external override onlyAdmin {
        uint256 amount = _adminRewards[_asset];
        require(amount > 0, "Staking: NOTHING_TO_CLAIM");
        delete _adminRewards[_asset];
    ...
    function unstake() external override nonReentrant {
        ...
        TimedRequest memory request = _user.unstakeRequest;
        StakingConfig memory config_ = _config;
        ...
        require(request.requested, "Staking: INVALID_REQUEST");
        require(block.timestamp - request.timestamp >= config_.unstakeDuration, "Staking: INSUFFICIENT_TIME");
        // If unstakeTimestamp is larger than 0 but less than the current strategy wait time, fail.

        _update(totalStake_);
        _syncUser(_user, _userStake, _userStake - request.amount);

        _totalStake = totalStake_ - request.amount;
        IERC20(_stakingToken).safeTransfer(_sender, request.amount);

        emit Unstaked(_sender, request.amount);
    }

    function immediateUnstake(uint256 _amount) external override nonReentrant {
        address _sender = _msgSender();
        UserInfo storage _user = _users[_sender];
        uint256 _userStake = _user.stake;
        uint256 totalStake_ = _totalStake;
    ...
    function createStrategy(
        ...
        require(_rewardToken != address(0), 'SomaStaking: INVALID_TOKEN');
        ...
        emit StrategyAdded(_rewardToken, _strategy.rewardsLocked, _startDate, _endDate, _msgSender());
    }
...
interface ISomaStaking {
    ...
    event Claimed(address indexed user, uint256 amount); // TODO change to sender
    event ClaimedImmediate(uint256 claimable, uint256 fee, uint256 received, address indexed sender);
    event StrategyAdded(address indexed rewardToken,  uint256 amount, uint256 startDate, uint256 endDate, address indexed sender);
    ...
    function userDebt(address _account, address _asset) external view returns (uint256);
    function userClaimable(address _account, address _asset) external view returns (uint256);
    ...
    function immediateUnstake(uint256 _amount) external;

    function requestClaim(address _asset) external;
    function claim(address _asset) external;
    function immediateClaim(address _asset) external;

    function adminRewards(address _asset) external view returns (uint256);
    function stake(uint256 _amount) external;

    function claimAdmin(address _asset, address _to) external;
    function createStrategy(
        uint256 _startDate,
        uint256 _endDate,
        address _rewardToken,
