...
contract ParallelReceiverPdiiiCorePack is VRFConsumerBaseV2, Ownable {
    ...
    function initializeAvailablePackIds2() public onlyOwner {
        ...
        for (uint16 i = 700; i < 1310; i++) {
            availablePackIds.push(i);
        }
    }

    function initializeAvailablePackIds3() public onlyOwner {
        // TODO: Update to match the new total
        require(availablePackIds.length == 1310, "cant re-initialize 3");
        for (uint16 i = 1310; i < 1737; i++) {
            availablePackIds.push(i);
        }
    }
