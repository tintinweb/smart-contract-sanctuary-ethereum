    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(bytes => bool) public checkSignature;
    ...
    event abc(bytes32 hashv1, bytes32 hashv2, bytes32 hashv3);
    ...
    function initialize() public initializer {
        ...
        baseUri = "https://s3.amazonaws.com/assets.thearchivesmint.xyz/nft-uri-dev/";
        baseExtension = "/token-uri.json";

        margin = 10;
        merchantWalletAddress = payable(
    ...
    /**
     * @dev updates the cross mint address
     *
     * @param _address cross mint address
     *
     * Requirements:
     * - only owner can update value.
     */

    function updateCrossMintaddress(address _address)
        external
        virtual
        onlyOwner
    {
        ...
        returns (bool)
    {
        ...
        tokenCounter += 1;
        checkSignature[_signature] = true;
    ...
    function mintTo(
        ...
        bytes32 validString = createHash(msg.value);
        ...
        require(
            status,
            "$GenesisV1: cannot purchase the token OR incorrect price"
        );
        tokenCounter += 1;
        ...
        emit createdCrossmintNft(to, id, userId, tokenCounter, msg.value, dollarPrice, size_id);
     ...
     * @param _dollarPrice price in dollar
    ...
    function purchaseTo(
        ...
        uint256 userId,
        uint256 _tokenId,
        string memory _dollarPrice,
        bytes32 _messageHash,
        bytes memory _signature,
        bytes memory _crossmint_sign
        ...
        require(
            SignatureCheckerUpgradeable.isValidSignatureNow(
                ...
                _messageHash,
                _signature
            ) && !checkSignature[_signature] && status,
            "ClubHouseV1: Not allowed to purchase OR Price is incorrect"
        ); // for verification
        ...
        ERC721Upgradeable(address(this)).approve(to, _tokenId);
        ERC721Upgradeable(address(this)).transferFrom(
            address(this),
            to,
            _tokenId
    ...
    function createHash(uint256 value) public pure returns(bytes32){
        bytes32 hashV1 = keccak256(abi.encodePacked(value));
        return hashV1;
    }
}
