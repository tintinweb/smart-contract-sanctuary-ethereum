...
contract MATICALPHA {
	...
	function _reinvest( uint8 _plan, address payable sender, uint256 value) private {
		...
		for(uint256 i=0; i<user.deposits.length; i++) {
			if(user.deposits[i].plan == _plan && user.deposits[i].finish > block.timestamp && user.deposits[i].withdrawn == false) {
	...
	function withdrawall(uint _type) public {
		...
		for(uint8 i=0;i<users[msg.sender].deposits.length;i++) {
			if(users[msg.sender].deposits[i].withdrawn == false) {
				...
				if(_type==1) {
					_sbalance += plans[users[msg.sender].deposits[i].plan].stakePercent.mul(users[msg.sender].deposits[i].amount).div(10000)*numberOfDaysPassed;
					users[msg.sender].deposits[i].stackedbalance = 0;
				}
				if(users[msg.sender].deposits[i].elegible){
					if(_type==2) {
						_hbalance += plans[users[msg.sender].deposits[i].plan].holderPercent.mul(users[msg.sender].deposits[i].amount).div(10000)*numberOfDaysPassed;
						users[msg.sender].deposits[i].holdbalance = 0;
					}
				}
				if(users[msg.sender].deposits[i].start>=block.timestamp&&users[msg.sender].deposits[i].finish<block.timestamp){
	...
	function withdraw(uint8 _plan, uint8 _type) public {
		...
		for(uint256 i=0;i<users[msg.sender].deposits.length;i++){
			if(users[msg.sender].deposits[i].plan==_plan) {
				if(users[msg.sender].deposits[i].withdrawn == false) {
					...
					stackedamount = plans[users[msg.sender].deposits[i].plan].stakePercent.mul(users[msg.sender].deposits[i].amount).div(10000)*numberOfDaysPassed;
					holdamount = plans[users[msg.sender].deposits[i].plan].holderPercent.mul(users[msg.sender].deposits[i].amount).div(10000)*numberOfDaysPassed;
					depositindex = i;
					break;
				}
			}
		...
		if(_type == 1){ // 1 for withdraw only stacked balance
	...
	function getResult(uint8 plan, uint256 deposit) public view returns (uint256 percent, uint256 profit, uint256 finish) {
		...
		profit = deposit + deposit.mul(percent).mul(plans[plan].time).div(10000);
	...
	function getUserPlanBalance(address userAddress, uint256 _plan) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			if (users[msg.sender].deposits[i].plan == _plan && users[msg.sender].deposits[i].withdrawn == false) {
				uint256 planTime = plans[users[msg.sender].deposits[i].plan].time; //plan time in days
				uint256 numberOfDaysPassed = (block.timestamp - planTime).div(TIME_STEP);
				planBalance = planBalance.add(plans[user.deposits[i].plan].stakePercent.mul(user.deposits[i].amount).div(10000)*numberOfDaysPassed);
				break;
