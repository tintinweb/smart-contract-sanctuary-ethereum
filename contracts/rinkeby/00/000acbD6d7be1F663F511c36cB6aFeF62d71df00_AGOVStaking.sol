...
contract AGOVStaking is Initializable, OwnableUpgradeable, PausableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using ECDSAUpgradeable for bytes32;

    /// @dev keccak256("AGOV TOKEN STAKING")
    uint256 public constant NAME_HASH = 0x58c19294e979156e03eae325be3cd509f3fe88e150c25f1b1bf59ca82e9cc4e5;
    /// @dev address of AGOV token
    IERC20Upgradeable public agovToken;
    /// @dev address of treasury wallet
    address public treasury;
    /// @dev address of validator
    address public validator;
    /// @dev current running round
    ...
    event Initialize(address _agov, address _treasury, address _validator);
    event Deposit(
    ...
    /**
     * @dev initialize contract with AGOV token and treasury wallet addressess
    ...
    function initialize(
        ...
        agovToken = IERC20Upgradeable(_agovToken);
        treasury = _treasury;
        validator = _validator;

        _pause();

        emit Initialize(_agovToken, _treasury, _validator);
    }

    /**
     * @dev deposits AGOV tokens and transfers it to treasury
     *
     * @param user address of participated user
     * @param round migration round
     * @param amount AGOV token amount
     * @param lock lock up period
     * @param timestamp timestamp to verify the signature
     * @param signature signature is generated from off-chain backend
     */
    function deposit(
        address user,
        uint256 round,
        ...
        // validate signature
        if (!_verify(user, round, amount, lock, timestamp, signature)) revert FailedVerify();

        // transfer agov token to treasury wallet
        agovToken.safeTransferFrom(_msgSender(), treasury, amount);

        emit Deposit(_msgSender(), round, amount, lock, timestamp, signature);
    }
     ...
     * @param _amount AGOV token amount
    ...
    function _verify(
        ...
        bytes32 signedHash = keccak256(abi.encodePacked(_who, NAME_HASH, _round, _amount, _lock, _timestamp));
    ...
    /**
     * @dev owner can update validator address
     *
