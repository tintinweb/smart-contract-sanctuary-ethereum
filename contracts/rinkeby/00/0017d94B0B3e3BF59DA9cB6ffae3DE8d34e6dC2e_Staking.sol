...
contract Staking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
   ...
   // @dev Accumulated tokens per share
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        emit Deposit(_msgSender(), _amount, stakers[_msgSender()].reward);
    }
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        totalAmountStaked = totalAmountStaked + stakers[_msgSender()].reward  - amount;
    ...
    function calculatePendingRewards(address user) public view returns (uint256) {
        if ((block.number > lastRewardBlock) && (totalAmountStaked != 0)) {
            ...
            uint256 tokenPerShare = accTokenPerShare + (stakers[user].amount * PRECISION_FACTOR) / totalAmountStaked;

            return (tokenRewardForStaking * tokenPerShare) / PRECISION_FACTOR;
        } else {
    ...
    function _updatePool() internal {
        ...
        // allocate tokens only if token rewards for staking are not null
         uint256 tokenRewardForStaking = (block.timestamp - start_time) * rewardsPerSecForStaking;
