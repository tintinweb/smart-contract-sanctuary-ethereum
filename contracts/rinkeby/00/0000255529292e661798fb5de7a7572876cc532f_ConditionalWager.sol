...
contract ConditionalWager is BartrrBase {
    ...
    function createWager(
        ...
        require(wagerTokens[_wagerToken], "Token not allowed to be wagered on");
        ...
        if (_paymentToken == address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)) { // ETH
            ...
            if (_userB == address(0)) {
                feeUserA = _calculateFee(_amountUserA, _paymentToken); // fee collected immediately for p2m
    ...
    function fillWager(uint256 _wagerId) external payable nonReentrant {
        ...
        require(!refundableToken[wager.wagerToken], "Kill Switch activated");
        ...
        if (wager.userB != address(0)) { // p2p
            ...
            if (wager.paymentToken == address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)) { // ETH
                ...
                uint256 feeUserA = _calculateFee(wager.amountUserA, wager.paymentToken); // fee collected immediately for p2m
                wager.amountUserA = wager.amountUserA - feeUserA;

                uint256 feeUserB = _calculateFee(wager.amountUserB, wager.paymentToken); // fee collected immediately for p2m
    ...
    /// @notice Refunds a wager when the "kill switch" is activated for its wagerToken
    /// @param _wagerId id of the wager
    /// @dev Does not refund any collected fee
    function refundWager(uint256 _wagerId) external nonReentrant {
        Wager memory wager = wagers[_wagerId];
        require(refundableToken[wager.wagerToken], "wager is not refundable");

        require(!wager.isClosed, "Wager has already been closed");
        if (wager.isFilled) {
            require(endTimes[_wagerId] > block.timestamp || refundUserA[_wagerId] || refundUserB[_wagerId], "wager must be redeemed");
            if (msg.sender == wager.userB) {
        ...
        if (refundUserA[_wagerId] && refundUserB[_wagerId]) {
            wagers[_wagerId].isClosed = true;
        }
    }

    /// @notice Redeems a wager and sends funds to the winner
    /// @param _wagerId id of the wager
    function redeem(uint256 _wagerId) external nonReentrant {
        Wager memory wager = wagers[_wagerId];
        require(!wager.isClosed, "Wager has already been closed");
        require(!refundUserA[_wagerId] && !refundUserB[_wagerId], "Wager has been partially refunded");
...
contract BartrrBase is Ownable, RoundIdFetcher, ReentrancyGuard {
    ...
    mapping(address => bool) public refundableToken; // Marked true by the "kill switch"
    ...
    /// @param _update true if turning on kill switch, false if turning it off
    function markTokenRefundable(address _token, bool _update) external onlyOwner {
        refundableToken[_token] = _update;
    }
