...
// File contracts/interfaces/IL1TokenGateway.sol
...
// File contracts/interfaces/IInbox.sol
...
// File contracts/interfaces/IOutbox.sol
...
// File contracts/libraries/L1CrossDomain.sol
...
library L1CrossDomain {
    ...
    function sendCrossDomainTx(
        ...
        address recipient,
        bytes memory data,
        CrossDomainTxOptions memory txOptions
    ) internal returns (uint256) {
        return
            IInbox(inbox).createRetryableTicket{value: msg.value}(
                recipient,
                txOptions.callValue,
                txOptions.maxSubmissionCost,
                txOptions.refundAddress,
                txOptions.refundAddress,
                txOptions.maxGas,
                txOptions.gasPriceBid,
                data
            );
    }

    function isCrossDomainSender(address inbox, address account)
        internal
        view
        returns (bool)
    {
        ...
        return l2ToL1Sender == account;
    }
}


// File contracts/libraries/L1OutboundDataParser.sol
...
// File contracts/gateways/L1TokensGateway.sol
...
contract L1TokensGateway is InterchainTokensGateway, IL1TokenGateway {
        ...
        returns (bytes memory res)
    {
        ...
        res = abi.encode(
            L1CrossDomain.sendCrossDomainTx(
                inbox,
                counterpartGateway,
                getOutboundCalldata(l1Token, from, to, amount),
                L1CrossDomain.CrossDomainTxOptions({
                    maxGas: maxGas,
                    callValue: 0,
    ...
    modifier onlyCounterpartGateway() override {
        if (!L1CrossDomain.isCrossDomainSender(inbox, counterpartGateway)) {
    ...
    function _onFinalizeInboundTransfer(
        ...
        address to,
        uint256 amount
    ) internal override whenWithdrawalsEnabled {
