...
contract NoundleTheory is ERC721, ERC721Enumerable, Ownable {
  ...
  InterfaceNoundles public Originals;
  ...
  modifier isPhaseOneStarted() {
    require(
      block.timestamp >= saleOneTime && saleOneTime > 0,
      "Phase One hasn't started"
    );
    _;
  }
  ...
  modifier isPhaseTwoStarted() {
    require(
      block.timestamp >= saleTwoTime && saleTwoTime > 0,
      "Phase Two hasn't started"
    );
    _;
  }
  ...
  function reserveNoundles(uint256 _amount, uint8 _type) public onlyOwner {
    ...
    if (_type == 0) {
      require(
        reservedComp + _amount <= MAX_RESERVED_COMP,
        'Cannot reserve more companions!'
      );
    } else if (_type == 1) {
      require(
        reservedEvil + _amount <= MAX_RESERVED_EVIL,
        'Cannot reserve more evil noundles!'
      );
    } else {
      require(
        reservedLand + _amount <= MAX_RESERVED_LAND,
        'Cannot reserve more land!'
      );
    }
  ...
  function claimEvilNoundle() public payable isPhaseOneStarted {
    ...
    // Verify request.
    require(
      freeCompanionMint + 1 <= MAX_FREE_COMPANION_MINTS,
      'We ran out of evil noundles :('
    );
    require(evilNoundleAllowed[msg.sender], 'You are not on whitelist');
    require(
      evilNoundleMinted[msg.sender] == false,
      'You already minted your free noundle.'
    );

    // Make sure that the wallet is holding at least 1 noundle.
    require(
      Originals.getNoundlesFromWallet(msg.sender).length > 0,
      'You must hold at least one Noundle to mint'
    );
  ...
  function mintHolderNoundles() public payable isPhaseOneStarted {
    ...
    require(
      freeCompanionMinted[msg.sender] == false,
      'You already minted your free companion.'
    );
  ...
  function mintNoundles(uint256 _noundles) public payable isPhaseOneStarted {
    ...
    require(
      _noundles > 0 && _noundles <= 5,
      "Your amount needs to be greater then 0 and can't exceed 5"
    );
    require(
      _noundles * publicMintCompanionPriceEth <= msg.value,
      'Ser we need more money for your noundles'
    );
    require(
      _noundles + __noundles <= MAX_COMPANION_NOUNDLES,
      'We ran out of noundles! Try minting with less!'
    );
    require(
      alreadyPublicMinted[msg.sender] == false || overrideAlreadyPublicMinted,
      'You cannot mint twice'
    );
  ...
  function mintNoundlesWithRainbows(uint256 _noundles)
    public
    payable
    isPhaseOneStarted
  {
  ...
  function mintWithRainbows(uint256 _noundles)
    public
    payable
    isRainbowMintingEnabled
  {
    uint256 __noundles = totalSupply();

    require(
      _noundles > 0 && _noundles <= 10,
      "Your amount needs to be greater then 0 and can't exceed 10"
    );
    require(
      _noundles + (mintCountCompanions + mintCountEvil) <= MAX_EVIL_NOUNDLES,
      'We ran out of noundles! Try minting with less!'
    );
  ...
  function mintHolderLandNoundles(uint256 _noundles)
    public
    payable
    isPhaseTwoStarted
  {
    ...
    require(
      _noundles + __noundles <= MAX_FREE_LAND_MINTS,
      'We ran out of land! Try minting with less!'
    );

    // The noundles that the sender is holding.
    uint256[] memory holdingNoundles = Originals.getNoundlesFromWallet(
      msg.sender
    );
    ...
    for (
      uint256 index;
      (index < holdingNoundles.length) && index < _noundles;
      index += 1
    ) {
      ...
      if (
        percentChance(
          __noundles + offset,
          (percentLowLand + percentMidLand + percentHighLand),
          percentHighLand
        )
      ) {
        _type = 4;
  ...
  function getOutOfJailByTokenId(uint256 _tokenId)
    public
    payable
    isRainbowMintingEnabled
  {
  ...
  function getRandomEvilNoundle(uint256 index, uint256 depth)
    internal
    returns (uint256)
  {
    uint256 selectedIndex = RandomNumber.getRandomNumber(index) %
      evilList.length;
  ...
  function getNoundlesFromWallet(address _noundles)
    external
    view
  ...
  // Most wanted.
  function getMostWanted() external view returns (address[] memory) {
    return mostWantedMembers;
  }

  function getMostWantedValue(address _arg) external view returns (uint256) {
    return mostWantedValues[_arg];
  }
  ...
  function getTypeByTokenId(uint256 _tokenId) external view returns (uint8) {
    return noundleType[_tokenId];
  }

  function getTypeByTokenIds(uint256[] memory _tokenId)
    external
    view
  ...
  function getFreeMintUsedByTokenId(uint256 _tokenId)
    external
    view
    returns (bool)
  {
    return alreadyMintedNoundles[_tokenId];
  }

  function getFreeLandMintUsedByTokenId(uint256 _tokenId)
    external
    view
    returns (bool)
  {
    return alreadyMintedLandNoundles[_tokenId];
  }

  function getJailStatus(uint256 _tokenId) external view returns (uint256) {
    return jailHouse[_tokenId];
  }

  function getJailStatusBool(uint256 _tokenId) public view returns (bool) {
    return (jailHouse[_tokenId] + jailLength > block.timestamp);
  }

  // Contract Setters (pretty standard :))
  function setBaseURI(string memory arg) public onlyOwner {
    baseURI = arg;
  }
  ...
  function setRandomNumberGenerator(address _arg) public onlyOwner {
    RandomNumber = InterfaceRandomNumber(_arg);
  }

  function setPhaseOneSaleTime(uint256 _arg) public onlyOwner {
    saleOneTime = _arg;
  }

  function setPhaseTwoSaleTime(uint256 _arg) public onlyOwner {
    saleTwoTime = _arg;
  }

  function setMintPriceEth(uint256 _arg) public onlyOwner {
    publicMintCompanionPriceEth = _arg;
  }

  function setMintPriceRain(uint256 _arg) public onlyOwner {
    publicMintCompanionPriceRainbow = _arg;
  }

  function setEvilMintCostRainbows(uint256 _arg) external onlyOwner {
    evilMintPriceRainbow = _arg;
  }

  function setLandMintCostRainbows(uint256 _arg) external onlyOwner {
    landMintPriceRainbow = _arg;
  }

  function setOverrideAlreadyPublicMinted(bool _arg) external onlyOwner {
    overrideAlreadyPublicMinted = _arg;
  }

  // Noundle Theory Setters (incase we need to balance some things out)
  function setLowLandPercent(uint256 _amount) public onlyOwner {
    percentLowLand = _amount;
  }

  function setMidLandPercent(uint256 _amount) public onlyOwner {
    percentMidLand = _amount;
  }

  function setHighLandPercent(uint256 _amount) public onlyOwner {
    percentHighLand = _amount;
  }

  function setEvilPercent(uint256 _amount) public onlyOwner {
    percentEvil = _amount;
  }

  function setJailPercent(uint256 _amount) public onlyOwner {
    percentJail = _amount;
  }

  function setStealPercent(uint256 _amount) public onlyOwner {
    percentSteal = _amount;
  }

  function setJailTime(uint256 _amount) external onlyOwner {
    jailLength = _amount;
  }

  function setGetOutOfJailCost(uint256 _amount) external onlyOwner {
    getOutOfJail = _amount;
  }

  function setJailTimeForEvil(uint256 _tokenId, uint256 _amount)
    external
    onlyOwner
  {
    jailHouse[_tokenId] = _amount;
  }

  // Contract Setters for the Genesis Contract
  function setGenesisAddress(address _genesis) external onlyOwner {
    Originals = InterfaceNoundles(_genesis);
  }

  // Contract Setters for the Rainbows Contract
  function setRainbowMintStatus(bool _arg) public onlyOwner {
    rainbowMintingEnabled = _arg;
  }

  function setBaseMintPriceTier1(uint256 _arg) public onlyOwner {
    rBaseMintPriceTier1 = _arg;
  }

  function setBaseMintPriceTier2(uint256 _arg) public onlyOwner {
    rBaseMintPriceTier2 = _arg;
  }

  function setBaseMintPriceTier3(uint256 _arg) public onlyOwner {
    rBaseMintPriceTier3 = _arg;
  }

  function setTier2Start(uint256 _arg) public onlyOwner {
    tier2Start = _arg;
  }

  function setTier3Start(uint256 _arg) public onlyOwner {
    tier3Start = _arg;
  }

  function setRainbowAddress(address _rainbow) external onlyOwner {
    ...
    returns (string memory)
  {
    ...
    return
      string(
        abi.encodePacked(
          base,
          uint2str(noundleType[tokenId]),
          '/',
          uint2str(noundleOffsetCount[tokenId])
        )
      );
  }
...
interface InterfaceEvilNoundles {
  ...
  function getRandomEvilNoundleExternal(uint256 index)
    external
    returns (address);
...
interface InterfaceRandomNumber {
  function getRandomNumber(uint256 arg) external returns (uint256);
