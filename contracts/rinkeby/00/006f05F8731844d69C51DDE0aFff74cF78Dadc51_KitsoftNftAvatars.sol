...
contract KitsoftNftAvatars is ERC721 {
    ...
    mapping(uint256 => address) private _avatarAddressList;
    ...
    {
        require(
            bytes(_tokenUriList[tokenId]).length != 0,
            "Wrong token ID (Avatar not found)."
        );
        require(msg.value >= _minMintPrice, "Not enough funds.");
        _safeMint(msg.sender, tokenId);
        payable(_avatarAddressList[tokenId]).transfer(msg.value);
        return tokenId;
    }

    function addAvatar(string memory tokenMetaUri, address avatarAddress)
        public
        onlyAdmin
    {
        require(avatarAddress != address(0), "Avatar owner should be defined.");
        uint256 tokenId = _nextTokenId;
        _tokenUriList[tokenId] = tokenMetaUri;
        _avatarAddressList[tokenId] = avatarAddress;
        _nextTokenId += 1;
    }
