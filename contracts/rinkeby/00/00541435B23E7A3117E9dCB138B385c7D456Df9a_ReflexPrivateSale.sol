...
abstract contract Crowdsale is Initializable {
    ...
    function buyTokens(address _beneficiary) internal {
        ...
        require(
            weiRaised <= params.hardcap,
            "Buy Token: Sale reached hardcap."
        );
        ...
        virtual
    {
        ...
        require(
            userTempBnbBalance >= params.minimumBuyLimit,
            "Min contribution not reached"
        );
        require(
            userTempBnbBalance <= params.maximumBuyLimit,
     ...
     */
    function _changeToken(IERC20MetadataUpgradeable newToken) internal virtual {
        token = newToken;
    }
}
    ...
    AccessControlUpgradeable
{
    ...
    function setParams(
        uint256 _minimumBuyLimit,
        uint256 _maximumBuyLimit,
        uint256 _softcap,
        uint256 _hardcap
    ) external whenNotPaused onlyRole(LAUNCHPAD_ROLE) {
        require(
            _maximumBuyLimit >= _minimumBuyLimit,
            "Set Params: Maximum buy must be greater than minimum buy."
        );
        require(
            _hardcap >= _softcap,
            "Set Params: Hardcap must be greater than softcap."
        );
        uint256 getCapPercentage = calculateCapPercentage(_hardcap);
        require(
            _softcap >= getCapPercentage,
            "Set Params: Soft cap must be greater than or equal to 50% of the hard cap."
        );
        _grantRole(DEFAULT_ADMIN_ROLE, owner());
        _grantRole(LAUNCHPAD_ROLE, owner());
        params = IntializeParams(
            _minimumBuyLimit,
            _maximumBuyLimit,
            _softcap,
            _hardcap
    ...
    function calculateCapPercentage(uint256 _hardcap)
        internal
        view
        returns (uint256)
    {
    ...
    function updateCapPercentage(uint256 _capPercentage) external onlyOwner {
        require(
            _capPercentage >= 50,
            "Cap Percentage: Cap percentage must be greater than 50%."
        );
        ...
        whenNotPaused
    {
        if (enableWhitelisting) {
            require(
                ...
                "Buy Token: Address need to whitelist"
            );
        }
    ...
    function finalization() internal virtual override {
        require(
            weiRaised >= params.softcap,
            "Finalization: Softcap is not reached"
        );
        ...
        require(
            ...
            "Finalize: Insufficient token balance"
        );
        ...
        uint256 refundBalance = tokenBalanceInContract -
            tokensSold -
            tokensForReflexOwner;
        if (refundType == 1) {
    ...
    function changeToken(IERC20MetadataUpgradeable newToken)
        external
        virtual
        onlyOwner
        onlyWhileOpen
  ...
  */
    function userWithdrawContributionWhileSoftcapNotReached()
        external
        whenNotPaused
    {
        require(hasClosed(), "Crowdsale is not yet closed");
        require(
            weiRaised <= params.softcap,
            "Withdraw Contribution: Softcap is reached"
        );
        require(
            userBnbBalance[msg.sender] > 0,
            "Withdraw Contribution: No contribution"
        );
  ...
  */
    function userWithdrawContribution() external whenNotPaused {
        ...
        if (isPoolCancelled) {
            ...
            uint256 deductPenality = (userBnbBalance[msg.sender] *
                penalityFee) / 10**2;
            address payable _owner = payable(owner());
            _owner.transfer(deductPenality);
            uint256 transferrableBnb = userBnbBalance[msg.sender] -
                deductPenality;
            address payable _user = payable(msg.sender);
  ...
  */
    function claimTokens(address account) external whenNotPaused {
        require(
            account != address(0),
            "Claim tokens: Account cant be zero address"
        );
        require(
            userTokenBalance[account] > 0,
            "Claim tokens: Amount cant be zero"
        );
   ...
   */
    function updatePenalityFee(uint256 _penalityFee)
        external
        onlyOwner
        whenNotPaused
    {
   ...
   */
    function updateDeadWallet(address _deadWallet) external onlyOwner {
        require(
            _deadWallet != address(0),
            "Dead wallet: Address can't be zero address"
        );
