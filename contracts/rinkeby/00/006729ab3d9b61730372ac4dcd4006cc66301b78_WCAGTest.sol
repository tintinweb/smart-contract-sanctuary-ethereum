...
//author:   pamuk.eth
//version:  0.2.0 - alpha - only suitable for testnet deployment 
...
contract WCAGTest is ERC721Enumerable,ERC2981Collection  {
    ...
    //add Artwork
    event ArtworkAdded( string _name, uint256  _ArtworkId,uint256  _Limit);
    ...
    //next artwork ID is a variation of version and artwork local in this contract
    ...
    struct Artwork {
        string name;
        string artworkBaseIpfsURI;
        uint256 minted;
        uint256 count;
        uint256 artworkLimit;
        bool locked;
        address royaltyReceiver;
        uint256 royaltiesInBP;
        uint256 artworkPrice;
        bool presale;
        uint256[] available_list;
    }
    ...
    mapping(address => bool) public isGalleryMinter;
    ...
    uint256 public defaultRoyaltiesInBP = 1000;
    uint256 public MINT_HARD_LIMIT=10;
    address public defaultRoyaltyReceiver;
    uint256 public WHITELIST_PER_ACCOUNT_LIMIT=5;
    uint256 public currentMappingVersion;
    uint256 public DAY_LIMIT=90;
    //𝗠𝗢𝗗𝗜𝗙𝗜𝗘𝗥𝗦
    ...
    {
        // checks the lock status of the artwork

        require(!artworks[_artworkId].locked, "E2");
        _;
    }
    ...
    {
        // checks if the msg sender is operator or not
        require(isOperator[msg.sender], "E4");
        _;
    }

    modifier onlyGalleryMinter() 
    {
        // checks if the msg sender is operator or not
        require(isGalleryMinter[msg.sender], "E4");
        _;
    }

    modifier onlyGateKeeperOne() 
    {
        require(gateKeeperOneAppointed, "E5.1!");

        // checks if the msg sender is gatekeeper or not
        require(msg.sender == gateKeeperOne, "E5.2!");
        _;
    }
    ...
    {
        // checks if the msg sender is gatekeeper or not
        require(gateKeeperTwoAppointed, "E6.1!");

        require(msg.sender == gateKeeperTwo, "E6.2!");
        _;
    }
    ...
    {
        // checks if the msg sender is gatekeeper or not
        require((msg.sender == gateKeeperOne) || (msg.sender == gateKeeperTwo) , "E21!");
        require(gateKeeperTwoAppointed, "E6.1!");
        require(gateKeeperOneAppointed, "E5.1!");
    ...
    constructor(string memory _tokenName, string memory _tokenSymbol, bytes32 merkleroot) ERC721(_tokenName, _tokenSymbol)  
    {
        ...
        isGalleryMinter[msg.sender]=true;
        root = merkleroot;
    ...
    function changeAdmin(address _address)  onlyAdmin public
    {
        ...
        isOperator[ex_admin] = false;

        // remember to add ex_admin as Operator , if we require exadmin to carry out priviliged work .
    }
    ...
    function addGalleryMinter(address _address)  onlyAdmin public
    {
        isGalleryMinter[_address] = true;
    }

    function removeGalleryMinter(address _address)  onlyAdmin public
    {
        isGalleryMinter[_address] = false;
    }
  ...
  function gateKeeperOneToggleAdminChange(bool toggle_bool)  onlyGateKeeperOne public
    ...
    function gateKeeperAdminOverride(address _address)  onlyGateKeeper public
    {
        ...
        require(gateKeeperTwoChangeAdmin,"E22.1!");
        require(gateKeeperOneChangeAdmin,"E22.2!");
    ...
    function addMultipleArtwork(string[] memory _artworkNames,uint256[] memory _artworkLimits,uint256[] memory _prices,string[] memory _artworkBaseIpfsURIs,bool[] memory _presales,uint256 len_check) onlyOperator public
    {

        uint256 q1=_artworkNames.length;
        ...
        require(q1==len_check,"E1");
        require(q2==len_check,"E2");
        require(q3==len_check,"E3");
        require(q4==len_check,"E4");
        require(q5==len_check,"E5");


        for (uint i = 0; i < len_check; i++) 
        {
            addArtwork(_artworkNames[i], _artworkLimits[i],_prices[i], _artworkBaseIpfsURIs[i],_presales[i]);

        }

    }

    function addArtwork(string memory _artworkName, uint256 _artworkLimit,uint256 _price, string memory _artworkBaseIpfsURI,bool presale)  onlyOperator public
    {
        uint256 artworkId = nextartworkId;
        require(artworkCheck[artworkId]==false,"E1");

        artworks[artworkId].name = _artworkName;
        ...
        for (uint i = 0; i < _artworkLimit; i++)
        {
            artworks[artworkId].available_list[i]=i+1;
            
        } 

        emit ArtworkAdded( _artworkName, artworkId,_artworkLimit);
        nextartworkId = nextartworkId.add(1); 
    ...
    function updateMultipleArtworkLock(uint256[] calldata _artworkIds,bool[] calldata lock_bools, uint256 check_len) onlyOperator public
    {
        uint256 q1=lock_bools.length;
        uint256 q2=_artworkIds.length;
        
        require(q1==check_len,"E1");
        require(q2==check_len,"E2");

        for (uint i = 0; i < check_len; i++) 
        {
            updateArtworkLock(_artworkIds[i], lock_bools[i]);

        }
    }
    
    function updateArtworkLock(uint256 _artworkId,bool lock_bool)  onlyOperator  public
    {
    ...
    function updateMultipleArtworkPresale(uint256[] calldata _artworkIds,bool[] calldata _preSales, uint256 check_len) onlyOperator public
    {
        uint256 q1=_preSales.length;
        uint256 q2=_artworkIds.length;
        ...
        for (uint i = 0; i < check_len; i++) 
        {
            updateArtworkPresale(_artworkIds[i], _preSales[i]);
    ...
    function updateMultipleRoyaltiesInBP(uint256[] calldata _artworkIds,uint256[] calldata BP_Royalties, uint256 check_len) onlyOperator public
    {
        uint256 q1=BP_Royalties.length;
        uint256 q2=_artworkIds.length;
        ...
        for (uint i = 0; i < check_len; i++) 
        {
            changeArtworkRoyaltiesInBP(_artworkIds[i], BP_Royalties[i]);
    ...
    function updateMultipleRoyaltyReceiver(uint256[] calldata _artworkIds,address[] calldata _royaltyReceivers, uint256 check_len) onlyOperator public
    {
        uint256 q1=_royaltyReceivers.length;
        uint256 q2=_artworkIds.length;
        ...
        for (uint i = 0; i < check_len; i++) 
        {
            changeArtworkRoyaltyReceiver(_artworkIds[i], _royaltyReceivers[i]);
    ...
    function galleryMint(address _to, uint256 _artworkId, bool _freeze,uint256 quantity,bool credit_card_sale) onlyGalleryMinter external returns (uint256[] memory) 
    {
        // if the sale is done via credit card, we reserve right to burn for 90days

        require(artworkCheck[_artworkId],"E1!");
        require(artworks[_artworkId].minted.add(quantity) <= artworks[_artworkId].artworkLimit, "E2!");
        require(quantity<=MINT_HARD_LIMIT, "E3!");
        require(!artworks[_artworkId].locked , "E4!");
        require(gateKeeperOneAllowMinting, "E5!");
        require(gateKeeperTwoAllowMinting, "E6!");
        require(gateKeeperOneAppointed, "E7!");
        require(gateKeeperTwoAppointed, "E8!");
    ...
    function publicSaleMint(uint256 _artworkId, bool _freeze,  uint256 quantity) public payable onlyAccounts returns (uint256[] memory) 
    {
        require(artworkCheck[_artworkId],"E1!");
        require(msg.value >=  artworks[_artworkId].artworkPrice.mul(quantity), "E2!");
        require(artworks[_artworkId].artworkPrice>0, "E3!");
        require(quantity<=MINT_HARD_LIMIT, "E24!");
        require(artworks[_artworkId].minted.add(quantity) <= artworks[_artworkId].artworkLimit, "E5!");
        require(!artworks[_artworkId].locked , "E6!");
        require(gateKeeperOneAllowMinting, "E7!");
        require(gateKeeperTwoAllowMinting, "E8!");
        require(gateKeeperOneAppointed, "E9!");
        require(gateKeeperTwoAppointed, "E10!");
        require(artworks[_artworkId].presale==false, "E11!");
    ...
    function preSaleMint(uint256 _artworkId, bool _freeze, uint256 quantity, bytes32[] calldata proof) public payable onlyAccounts returns (uint256[] memory) 
    {
        require(artworkCheck[_artworkId],"E1!");
        require(msg.value >=  artworks[_artworkId].artworkPrice.mul(quantity), "E2!");
        require(artworks[_artworkId].artworkPrice>0, "E3!");
        require(quantity<=MINT_HARD_LIMIT, "E4!");
        require(artworks[_artworkId].minted.add(quantity) <= artworks[_artworkId].artworkLimit, "E5!");
        require(!artworks[_artworkId].locked , "E6!");
        require(gateKeeperOneAllowMinting, "E7!");
        require(gateKeeperTwoAllowMinting, "E8!");
        require(gateKeeperOneAppointed, "E9!");
        require(gateKeeperTwoAppointed, "E10!");
        require(artworks[_artworkId].presale==true, "E11!");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(proof, root, leaf),"E12");
        uint256 limit=getWhiteListedLimit(msg.sender);
        uint256 new_limit=limit.add(quantity);
        require(new_limit<=WHITELIST_PER_ACCOUNT_LIMIT,"E13");
    ...
    function getNextAvailableExtId(uint256 _artworkId) internal  returns (bool found,uint256 ext_id) 
    {
        ...
        found=false;
        for (uint i = 0; i < avails.length; i++) 
        {
    ...
    function _mintToken(address _to, uint256 _artworkId, bool _freeze) internal returns (uint256 _tokenId) 
    {
        ...
        artworks[_artworkId].minted = artworks[_artworkId].minted.add(1);
        artworks[_artworkId].count = artworks[_artworkId].count.add(1);

        uint256 tokenIdToBe = (_artworkId * ONE_MILLION) + artworks[_artworkId].count;
        require(artworks[_artworkId].count<ONE_MILLION,"E9.2");
    ...
    {
        require(gateKeeperOneAllowMinting, "E1!");
        require(gateKeeperTwoAllowMinting, "E2!");
        require(gateKeeperOneAppointed, "E3!");
        require(gateKeeperTwoAppointed, "E4!");
        uint256 q1=_tokenIds.length;
        
        require(q1==check_len,"E5");
        for (uint i = 0; i < q1; i++) 
        {
    ...
    {
        require(_exists(_tokenId), "E1.1");
        address token_owner = ERC721.ownerOf(_tokenId);
        require(msg_sender==token_owner ,"E2.1");
        uint256 _artworkId=tokenIdToArtworkId[_tokenId];
        require(_artworkId!=0,"E3.1");

        //get the hash of the token and equate it to a value other than 0
        uint256 ipfsHash=tokenIdToIpfsHash[_tokenId];
        artworks[_artworkId].minted = artworks[_artworkId].minted.sub(1);
        tokenIdToArtworkId[_tokenId] = 0;
        tokenIdToIpfsHash[_tokenId]=0;
        artworks[_artworkId].available_list[ipfsHash.sub(1)]=ipfsHash; //burned ID is available for recycling
    ...
    function _creditCardBurn(uint256 _tokenId) internal 
    {
        ...
        require(_exists(_tokenId), "E1.1");
        require(paidWithCard[_tokenId], "E2.1");
        require((block.timestamp).sub(paidDate[_tokenId])<=DAY_LIMIT.mul(86400),"E3.1");
        require(burnDisabled[_tokenId]==false,"E4.1");
        uint256 ipfsHash=tokenIdToIpfsHash[_tokenId];

        uint256 _artworkId=tokenIdToArtworkId[_tokenId];
        require(_artworkId!=0,"E5.1");
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view override returns (address receiver, uint256 royaltyAmount) 
    {
        ...
        uint256 _royalties = _salePrice.mul(_royaltiesinBPartwork).div(10000);
    ...
    function artworkTokenInfo(uint256 _artworkId) view public returns (uint256 minted, uint256 artworkLimit ,  bool locked,uint256 price,uint256 count,string memory artworkName,string memory artworkBaseIpfsURI) 
    {
        ...
        artworkName = artworks[_artworkId].name;
    ...
    function showWhiteListLimit(uint256 _artworkId) public view returns (bool)
    {
        return artworkCheck[_artworkId];
    }
    ...
    function showCCSecondsLeft(uint256 _tokenId) public view returns (uint)
    {
        return (block.timestamp).sub(paidDate[_tokenId]);
    }

    function showNextAvailableExtId(uint256 _artworkId) public view  returns (bool found,uint256 ext_id) 
    {
        //loop the avialble
        //if it is not 0, break and return 
        // if 0
