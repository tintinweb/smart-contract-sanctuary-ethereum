...
contract Journey is ERC721Holder, Ownable, ReentrancyGuard, Pausable {
    ...
    function setGracePeriod(uint256 _gracePeriod) external onlyOwner {
        gracePeriod = _gracePeriod; // converted into decimals
    }

    function setGenesis() external onlyOwner {
        genesis = block.timestamp; // converted into decimals
    }

    function lordsIssuance(uint256 _new) external onlyOwner {
        lordsPerRealm = _new; // converted into decimals
    }
    ...
    function _epochNum() internal view returns (uint256) {
        if (finalAge != 0) {
            ...
            return 0;
        } else if ((block.timestamp - genesis) / (epoch * 3600) == 0) {
            return 1;
        } else {
            return (block.timestamp - genesis) / (epoch * 3600) - 1;
        }
        // return 10;
    }
