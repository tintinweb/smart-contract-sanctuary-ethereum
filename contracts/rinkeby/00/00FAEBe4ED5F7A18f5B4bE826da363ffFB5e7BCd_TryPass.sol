...
contract TryPass is ERC721("Try Pass", "VOP"), ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    // 9 hours = 32400 seconds, so price decrease (1 - 0.1) * 10**18 / 32400 = 27777777777777 wei
    uint256 public discountRate = 27777777777777 wei; 
    ...
    uint256 public privateMintLimit = 6300;
    uint256 public publicSaleMintLimit = 2000;
    ...
    function airdrop(address _target, uint _whitelistIndex) external onlyOwner {
        _safeMint(_target, totalSupply() + 1);
        addressMinted[_target] = true;
        updateMintMaps(totalSupply(), _whitelistIndex);
    }

    function canMint(bytes32[] calldata _merkleProof, address _address) public view returns (bool, string memory, uint) {

        require(totalSupply().add(1) <= TOTAL_NFT, "Can't mint more than 10000");
    ...
    function withdraw() public onlyOwner {
        payable(0x345A54d6b412EDbFF74f21F524bB6119c9292E2E).transfer(address(this).balance);
    }
    ...
    function isActive() public view returns (bool){
        return tryMintActive || oldMintActive || privateMintActive || publicSaleActive;
    }
