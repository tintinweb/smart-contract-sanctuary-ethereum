...
import "../deployers/ILPTokenDeployer.sol";
import "../tokens/ILPToken.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract LendingPoolInitializer is ILendingPoolInitializer, Ownable {
  address public copraGlobal;
  address public copTokenAddress;
  address public lendingPoolRegistry;
  address public lpTokenDeployer;
  address public loanNFT;

  event LendingPoolInitialized(address _jCopToken, address _sCopToken, address _lendingPool);

  constructor(
    address _copraGlobal,
    address _copTokenAddress,
    address _loanNFT,
    address _lendingPoolRegistry,
    address _lpTokenDeployer
  ) {
    copraGlobal = _copraGlobal;
    copTokenAddress = _copTokenAddress;
    loanNFT = _loanNFT;
    lendingPoolRegistry = _lendingPoolRegistry;
    lpTokenDeployer = _lpTokenDeployer;
  }
  ...
  function initPool(
    ...
    require(msg.sender == lendingPoolRegistry, "sender not lp registry");
    address jCopToken = (ILPTokenDeployer(lpTokenDeployer)).deploy(jCopName, jCopSymbol, _lpAddress);
    address sCopToken = (ILPTokenDeployer(lpTokenDeployer)).deploy(sCopName, sCopSymbol, _lpAddress);
    (ILendingPool(_lpAddress)).init(jCopToken, sCopToken, copraGlobal, copTokenAddress, loanNFT);
    emit LendingPoolInitialized(jCopToken, sCopToken, _lpAddress);
    return (jCopToken, sCopToken);
  }
}
