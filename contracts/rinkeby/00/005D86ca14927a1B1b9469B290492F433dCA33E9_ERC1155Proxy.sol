...
contract SafeMath {
    function safeMul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        uint256 c = a * b;
        require(c / a == b, "UINT256_OVERFLOW");
        return c;
    }
    ...
    function max64(uint64 a, uint64 b) internal pure returns (uint256) {
        return a >= b ? a : b;
    }

    function min64(uint64 a, uint64 b) internal pure returns (uint256) {
        return a < b ? a : b;
    }
...
contract ERC1155Proxy is MixinAuthorizable, SafeMath, IAssetProxy {
    using LibBytes for bytes;

    // Id of this proxy.
    bytes4 internal constant PROXY_ID =
        bytes4(keccak256("ERC1155Assets(address,uint256[],uint256[],bytes)"));
