...
contract AkiStory is ERC721A, Ownable {
  ...
  function mintFree(uint8 mintAmount, address _account, bytes32[] calldata _proof) public payable {
    require(sellingStep == Steps.FreeMint, "FreeMint has not started");
    require(totalSupply().add(mintAmount) <= MaxToken, "Sold Out");
    require(isFreeMintWhitelisted(_account, _proof), "Not Whitelisted");
  ...
  function isFreeMintWhitelisted(address account, bytes32[] calldata proof) internal view returns(bool) {
    return _verifyFreeMint(_leaf(account), proof);
  }
  
  function _verifyFreeMint(bytes32 leaf, bytes32[] memory proof) internal view returns(bool) {
    return MerkleProof.verify(proof, merkleRootFreeMint, leaf);
  }

  function changeMerkleRootFreeMint(bytes32 _newmerkleRootFreeMint) external onlyOwner {
    merkleRootFreeMint = _newmerkleRootFreeMint;
  }
