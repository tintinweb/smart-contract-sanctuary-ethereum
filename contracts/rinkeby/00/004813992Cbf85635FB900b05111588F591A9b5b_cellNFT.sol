...
interface iSVGManip {
    ...
    function get_cell_core_color(string memory tokenId) external view returns (bool);
...
contract cellNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(address _iSVGManip_addy) ERC721A("cellNFT", "CL") {
        ...
        _seed_phrase = iSVGManip(SVGManip_addy)._get_seed_phrase();
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        output.svg_data_bytes = SVGManip.add_gradients(tokenId_str, output.svg_data_bytes, cell.eukaryote, cell._type);
        output.svg_data_bytes = SVGManip.add_filters(tokenId_str, output.svg_data_bytes, cell.eukaryote, cell._type, false);
        if (cell.eukaryote == 1) {
            output.svg_data_bytes = SVGManip.add_filters(tokenId_str, output.svg_data_bytes, cell.eukaryote, cell._type, true);
        }
        ...
        output.svg_data_bytes = SVGManip.add_cell_body(tokenId_str, output.svg_data_bytes, cell.eukaryote, cell._type);

        // Nucleus
        if (cell.eukaryote == 1) {
            output.svg_data_bytes = SVGManip.add_nucleus(tokenId_str, output.svg_data_bytes, cell._type);
        }

        output.svg_data_bytes = SVGManip.add_spooky_overlay(tokenId_str, output.svg_data_bytes, cell.eukaryote, cell._type);
        ...
        output.json = (SVGManip.get_cell_core_color(tokenId_str) == true) ? SVGManip.add_trait(output.json, "Core Color", "Dark"): output.json = SVGManip.add_trait(output.json, "Core Color", "Light");

        if (cell.eukaryote == 1) {
            output.json = SVGManip.add_trait(output.json, "Nucleus?", "YES");
            output.json = SVGManip.add_trait(output.json, "Cell Type", cell_types[cell._type]);
        } else {
            output.json = SVGManip.add_trait(output.json, "Nucleus?", "no");
        }

        if (tokenId + 1 > TOKENS_MINTED) {
            output.json = SVGManip.add_trait(output.json, "Found yet?", "No");
            output.json = SVGManip.add_trait(output.json, "In a better place", "No");
        } else {
            output.json = SVGManip.add_trait(output.json, "Found yet?", "Yes");
            output.json = (ownerOf(tokenId) == address(0)) ? SVGManip.add_trait(output.json, "In a better place", "Yes") : SVGManip.add_trait(output.json, "In a better place", "No");
        }
...
contract svgManip {
    ...
    function _get_muscle_path() pure internal returns (bytes memory) {
        return abi.encodePacked('\n<path d="M 310 151 C 377 162 378 182 310 192 C 243 201 108 200 41 189 C -26 179 -26 158 40 149 C 107 139 242 140 310 151"/>');
    }
    ...
    function _get_cell_core_color(string memory tokenId) internal view returns (bool){
        return _random(abi.encodePacked(_seed_phrase, "CELL CORE COLOR", tokenId)) % 10 == 9;
    }
    ...
    function add_gradients(string memory tokenId, bytes memory output, uint64 eukaryote, uint256 _type) view external returns (bytes memory) {
        output = abi.encodePacked(output, '\n<defs>\n<linearGradient id="line_grad" spreadMethod="pad" x1="1" x2="0" y1="0" y2="0">',
            ...
            _get_cell_core_color(tokenId) ? '69, 69, 69' : '255, 255, 255', 
