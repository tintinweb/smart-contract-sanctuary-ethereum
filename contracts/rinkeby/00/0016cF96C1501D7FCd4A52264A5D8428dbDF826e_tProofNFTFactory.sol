...
import "./tProofNFTTokenUriGenerator.sol";

contract tProofNFTFactory is ERC721A, Ownable, Pausable {
    ...
    uint256 constant MAX_MINT_PER_TX = 10000;
    ...
    // address
    address tProofRouter;

    // contract
    tProofNFTTokenUriGenerator tokenUriGeneratorContract;
    ...
    event TokenUriGeneratorChanged(address _newAddress);
    event tProofRouterChanged(address _newAddress);


    constructor() ERC721A("tProof Certifications", "TPROOF", MAX_MINT_PER_TX, MAX_SUPPLY) { }
    ...
    **/
    ...
    external whenNotPaused() {
        ...
        require(_hash.length <= MAX_MINT_PER_TX, "Max mint per transaction reached");
        require(msg.sender == tProofRouter, "Only router can call");  // TODO Add roles also in this contract. Remove tProofRouter, setter and change event

        uint totalSupply = totalSupply();
        _safeMint(_to, _hash.length);

        // stores hash and title information
        for(uint i = 0; i < _hash.length; ++i) {
            data[totalSupply+i].hash = _hash[i];
            if (!tProofGeneralLibrary.isStringEmpty(_title[i]))
                data[totalSupply+i].title = _title[i];
            emit ProofMinted(totalSupply+i, _hash[i], block.timestamp);
        }
    }
    ...
    **/
    function updateTitle(uint[] calldata _nftNum, string[] calldata _title) external {
        require(_nftNum.length == _title.length, "Arrays must have same length");
        for (uint i = 0;  i < _nftNum.length; ++i) {
            require(ownerOf(_nftNum[i]) == msg.sender, "Only NFT owner can change title");
    ...
    **/
    function setDescription(uint[] calldata _nftNum, string[] calldata _description) external {
        require(_nftNum.length == _description.length, "Arrays must have same length");
        for (uint i = 0;  i < _nftNum.length; ++i) {
            require(ownerOf(_nftNum[i]) == msg.sender, "Only NFT owner can change description");
            description[ _nftNum[i] ] = _description[i];
            emit DescriptionEdited(_nftNum[i], _description[i]);
        }
    }

    // TODO Implement an interface for tokenUriGeneratorContract
    /**
    ...
    // TODO put a placeholder to show here starts the only oenwer section
     ...
     */
    function setTokenUriGenerator(address _newAddress) public onlyOwner {
        tokenUriGeneratorContract = tProofNFTTokenUriGenerator(_newAddress);
        emit TokenUriGeneratorChanged(_newAddress);
    }
    ...
    /**
     * @notice Record the router address
     * @param _newAddress address of the router
     */
    function setRouterAddress (address _newAddress) public onlyOwner {
        tProofRouter = _newAddress;
        emit tProofRouterChanged(_newAddress);
    }
...
contract tProofNFTTokenUriGenerator {
    ...
    **/
    function getTokenUri(uint256 _tokenId) public view returns (string memory) {
        require(nftContract._exists(_tokenId), 'URI query for nonexistent token');

        (bytes32 hash, string memory title) = nftContract.data(_tokenId);
        ...
        string memory description = nftContract.description(_tokenId);
        if (tProofGeneralLibrary.isStringEmpty(description))
            description = "Protected by tProof technology";  // TODO think about the claim
        return
        string.concat(
            ...
            Base64.encode(
                bytes(
                    string.concat(
                        ...
                        ' "image": "https://arweave.net/image-to-file",', // TODO here place a static image that is picked from Arweave
                        ' "attributes":', tokenIdToAttributes(_tokenId, hashStr), ",",
     ...
     */
    function tokenIdToAttributes(uint _tokenId, string memory _hash) public view returns (string memory)
    {
        string memory attributesString;
        (bool verified, uint64 certificationPendingValidUntil, uint16 storageType,
            uint32 mimeType, bool verificationInProgress) = hashRegistry.hashGeneralDetails(_tokenId);
        ...
        attributesString = string.concat(
            '[',
                '{"trait_type":"Verified","value":"', verifiedStr ,'"},',  // TODO place "False", "True", "Pending"
...
import "@openzeppelin/contracts/access/AccessControl.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "./tProofUrlVerifierRouter.sol";
import "./tProofNFTFactory.sol";
import "./interfaces/ItProofHashRegistryStorageTypeInterface.sol";
import "hardhat/console.sol";

contract tProofHashRegistry is Ownable, AccessControl {
    ...
    struct HashGeneralDetails {
        ...
        // TODO still 17 bytes left of space
    }
    ...
    // address
    tProofUrlVerifierRouter urlVerifierRouterContract;
    tProofNFTFactory NFTFactory;

    // bytes
    bytes32 public constant MARK_HASH_VERIFICATION_PREPAID_ROLE = keccak256("MARK_HASH_VERIFICATION_PREPAID_ROLE");
    bytes32 public constant DELEGATE_VERIFIER_ROLE = keccak256("DELEGATE_VERIFIER_ROLE");
    bytes32 public constant IS_ROUTER_ROLE = keccak256("IS_ROUTER_ROLE");
    bytes32 public constant IS_URL_VERIFIER_ROUTER = keccak256("IS_URL_VERIFIER_ROUTER");
    ...
    **/
    ...
    external
    onlyRole(IS_ROUTER_ROLE) {
        require(_nft.length == _url.length, "nfts and urls != length");
        require(_url.length == _storageType.length, "url and st != length");
        require(_storageType.length == _mimeType.length, "st and mt != length");

        for (uint i=0; i< _nft.length; ++i) {
            // check that the caller is either the owner, or the allowed third party
            if (NFTFactory.ownerOf(_nft[i]) != caller) {
            ...
            // if file is marked as verified, means we need to re-verify it for some reason. Set it to false
            if (hgd.verified)
            ...
            emit CertificationStarted(_nft[i], _storageType[i], _url[i]);  // TODO check if it's better to place all the emits at the end or not
        }
    ...
    **/
    function fulfillCertification(uint[] calldata _nft, bytes32[] calldata _hashEvalFromUrl) external onlyRole(IS_URL_VERIFIER_ROUTER) {
        for (uint i=0; i<_nft.length; ++i) {
    ...
    * @param _nft the nft to add the pre-paid flag
    ...
    **/
    function markHashVerificationPrepaid (uint _nft, uint16 _storageType, uint64 _until)
                                            external onlyRole(MARK_HASH_VERIFICATION_PREPAID_ROLE) {
    ...
    **/
    function delegateStartCertificationCall (uint _nft, address _delegateTo, address _from) public {
        require(NFTFactory.ownerOf(_nft) == msg.sender || hasRole(DELEGATE_VERIFIER_ROLE, msg.sender), "Not the owner");
        if (NFTFactory.ownerOf(_nft) == msg.sender)
            require(msg.sender == _from, "Don't fake the caller");
        delegatedStartCertification[_nft] = _delegateTo;
        emit DelegateStartCertification(_from, _nft, _delegateTo);
    }

    // TODO place a graphic representation of the section, to show that from here down it's onlyOwner function
    ...
    **/
    function addStorageTypeManager(address _contractAddress, string calldata _name) external onlyOwner {
    ...
    **/
    function disableOneStorageTypeManager (uint16 _position) external onlyOwner {
    ...
    **/
    function addMimeType (string[] calldata _mimeTypes) public onlyOwner {
    ...
    **/
    function setUrlVerifierRouter (address _newAddress) public onlyOwner {
        urlVerifierRouterContract = tProofUrlVerifierRouter(_newAddress);
...
contract tProofUrlVerifierRouter is ChainlinkClient, ConfirmedOwner {
    ...
    // address
    address private constant oracle = 0xA4362baBDDcD9071fF1270C3c16E0Ea1b0057284;
    ...
    constructor( address _hashRegistryContractAdd, string memory _jobId) ConfirmedOwner(msg.sender) {
    ...
    /**
      * @notice Prepares the call for Chainlink Oracle
    ...
    // TODO insert an onlyOwner section markup
    /**
    ...
    function cancelRequest(
        ...
        cancelChainlinkRequest(_requestId, _payment, _callbackFunctionId, _expiration);  // TODO search the docs and understand the meaning of those params
    }
    ...
    // TODO intert an private section markup
