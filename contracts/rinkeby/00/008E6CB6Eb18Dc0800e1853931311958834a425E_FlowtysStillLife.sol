...
contract FlowtysStillLife is ERC721, Ownable {
  using Strings for uint256;
  bytes32 public constant MINTER_ROLE = keccak256("MINTER");

  bool public saleActive = false;
  uint256 public TOKEN_PRICE_REALTIME = 1 ether;
  uint256 public TOKEN_PRICE_SLOWED_DOWN = 0.1 ether;
  ...
  mapping(TokenType => uint256) private _ageBlocksPeriod;
  // The threshold between metadata changes, when we need to show a different image
  mapping(TokenType => uint256) private _ageBlocksThreshold;
  ...
  function setAging(TokenType tokenType, uint256 period, uint256 threshold) external onlyOwner {
    _ageBlocksPeriod[tokenType] = period;
    _ageBlocksThreshold[tokenType] = threshold;
  }
  ...
  // 103 ... 202 => RealTime2
  // 203 ... 302 => RealTime3
  function mintRealTimeTo(uint256 qty, TokenType tokenType)
    external 
    payable
    callerIsUser
    saleIsActive
  ...
  function mintSlowedDownTo(uint256 qty, TokenType tokenType)
    external
    payable
    callerIsUser
    saleIsActive
  {
    ...
    require(_supply[tokenType] + qty <= 1000, "Purchase would exceed max RealTime tokens");
    ...
    returns (uint256)
  {
    ...
    if (_ages[tokenId] > 0) {
      uint256 currentAge = (uint((block.number - _ages[tokenId]) % _ageBlocksPeriod[tokenType]) / _ageBlocksThreshold[tokenType]);
      return currentAge;
    }
    return 0;
  }
  ...
  */
  //---------------------------------------------------------------------------------
  // We build URL is a following way: _baseTokenURI + tokenId + AGE (0...period)
  // e.g. fpr tokenId 0 and age 0 : ipfs://hash_of_ipfs/0/0
  // e.g. fpr tokenId 3 and age 10 : ipfs://hash_of_ipfs/3/10
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    string memory baseURI = super.tokenURI(tokenId);
    return bytes(_baseTokenURI).length > 0 ? 
          string(
            abi.encodePacked(abi.encodePacked(baseURI, "/"), getAge(tokenId).toString())) 
          : "";  }
