...
contract Akyllers is ERC721A, Ownable, ReentrancyGuard , ERC721ABurnable {
  ...
  function validatePrice(uint256 value, uint256 _mintAmount) public view{
    require(value>0, " Incorrect amount");
    require(value == cost * _mintAmount, 'Incorrect amount ');
  }


  
  function validateMerkle(address sender, bytes32[] calldata _merkleProof) public view{
    require(whitelistMintEnabled, "Not in presale phase"); 
    require(MerkleProof.verify(_merkleProof, merkleRoot, keccak256(abi.encodePacked(sender))), 'Invalid proof!');
  }
  ...
  function validateWhitelistState() public view{
    require(whitelistMintEnabled, "Not in whitelist phase"); 
  }
  function validatePublicSaleState() public view {
    require(!whitelistMintEnabled, "Not in public sale state");
  }


  function validatePresaleMint(address sender, uint256 _mintAmount, bytes32[] calldata _merkleProof) public view{
    validateAmount(_mintAmount, supplyLimit);
     // Verify whitelist requirements dont allow mints when public sale starts
    validateMerkle(sender, _merkleProof);
    //verify not that minting is not paused
    validateNotPaused();
    //verify that the smart contract in the whitelist phase
    validateWhitelistState();
    //verify that the supply is not exceeded by address in specifc whitelist phase
    
 }
    function validateAddressLimit(address sender, uint256 amount) public view {
       _getNumberMintedInWhitelist(sender) ; // found in the ERC721A contract in place of getaux
       require(maxperaddress >= ( (balanceOf(sender) + amount) - _getNumberMintedInWhitelist(sender) ) , " wallet limit exceeded"); 
     }
 function validatePublicMint(uint256 _mintAmount) public payable{
    validatePublicSaleState();
    validateNotPaused();
    validateAmount(_mintAmount, maxSupply);
    validateAddressLimit( msg.sender, _mintAmount);
    validatePrice(msg.value,_mintAmount);
 }
 function validateAndUpdateAddressSupplyInWhitelist(address sender, uint256 _mintAmount, uint256 addressWlT, uint256 whitelistT) public {
    uint256 value = addressWlT;
  ...
  function Presalemint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable nonReentrant {
    //validate mint 
    validatePrice(msg.value,_mintAmount);
    validatePresaleMint(msg.sender, _mintAmount,_merkleProof);
    //change the phase 
    validateAndUpdateAddressSupplyInWhitelist(msg.sender,_mintAmount, addresstxs[msg.sender], whitelistTag);
    //minting 
    _increaseNumberMintedInWhitelist(msg.sender, uint64(_mintAmount)); //increases the wallet supply of the whitelist address, function is found in ERC721A
...
//Normal minting allows minting on public sale satisfyign the necessary conditions
  function mint(uint256 _mintAmount) public payable  nonReentrant {
    validatePublicMint(_mintAmount);
    //minting
    _safeMint(msg.sender, _mintAmount);
  } 
...
// SETTERS DONE BY OWNER ONLY
 ...
 function setWhitelistPhase(bytes32 _merkleRoot, uint256 _maxperaddress, uint256 _supplyLimit) external onlyOwner {
    require(_supplyLimit<=maxSupply);
	merkleRoot = _merkleRoot;           
	maxperaddress = _maxperaddress;
	whitelistTag +=1; // set a new phase on every call
	supplyLimit = _supplyLimit; // set the total supply limit on the whitelist phase
	paused = true;
  }

  
 function setPublicSalePhase(uint256 _maxperaddress) external onlyOwner {
