...
contract TheOtherSideToken is ERC20Burnable, Delegated {
    ...
    constructor() ERC20("The Other Side Token Staging v.8", "MOONZV8") {
    }
    ...
    function setMerkleRoot(bytes32 _merkleRoot) external onlyDelegates {
        require(merkleRoot != _merkleRoot,"Merkle root is the same as the previous value");
    ...
    function bulkPublicMint(address[] memory to_, uint256[] memory amounts_) external onlyDelegates {
        require(to_.length == amounts_.length, "To address and Amounts length Mismatch!");
    ...
    function whitelistMint(uint256 _mintableQty, uint256 _totalQty,bytes32[] calldata _merkleProof, bytes32 _leaf) public {
        ...
        require(keccak256(abi.encodePacked(convertQtyWithOwnerToStr(_totalQty,msg.sender))) == _leaf,"Hashing of Qty+wallet doesn't match with leaf node");
        require((WhitelistMintClaimed[msg.sender].mintedQty + _mintableQty) <= _totalQty,"mintedQty + _mintableQty must be less than or equal to _totalQty");
        require( MerkleProof.verify(_merkleProof,merkleRoot, _leaf),"Invalid Merkle Proof.");
    ...
    function setTreasuryAddress(address _treasuryAddress) external onlyDelegates {
        require(treasuryAddress != _treasuryAddress, "new treasury address is the same as the new address ");
