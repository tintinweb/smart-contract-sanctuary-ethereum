...
contract Sarutama is ERC721A, Ownable {
    ...
    constructor() ERC721A("Sarutaa", "SARU", 2) {
    }
    ...
    //og only mint
    function ogMint(uint256 tokens, bytes32[] calldata merkleProof) external payable {
        ...
        require( _OGClaimed[_msgSender()] + tokens <= maxMint, "SARU: Max amount of SARUs have been minted by this wallet");
        require(tokens <= maxOGMint, "SARU: Exceeded allowed max mint limit");
        require(tokens > 0, "SARU: Please mint at least 1 SARU");
        require(price * tokens == msg.value, "SARU: Not enough ETH");

        _safeMint(_msgSender(), tokens);
         _OGClaimed[_msgSender()] += tokens;
    }
    ...
    function whitelistMint(uint256 tokens, bytes32[] calldata merkleProof) external payable {
        require(whitelistSale, "SARU: You can not mint right now");
        require(MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "SARU: Please wait to mint on public sale");
        require(_whitelistClaimed[_msgSender()] + tokens <= maxMint, "SARU: Max amount of SARUs have been minted by this wallet");
