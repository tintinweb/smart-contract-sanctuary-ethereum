...
{
    // new
    uint256 private MAX_QTY_SALES = 3333;

    uint256 public cost = 2 * 10**17; // default price 0.2 ether

    bool public activeDropping = false;
    ...
    string baseURI =
        "https://sqtokens.s3.amazonaws.com/sqi_meta.json";
    ...
    // Only SQC contract
    function dropMint(address _recipient, uint256 _mintsAmount)
        external
        whenNotPaused
        nonReentrant
    {
        require(activeDropping == true, "dropMint inactive");

        require(SQVContractAddr != address(0), "SQV address not configured");

        require(
            msg.sender == address(SQVContractAddr),
            "Only SQV contract allowed"
        );
        ...
        emit NewSQIMinted(address(this), _recipient, generatedIds);
    }
    ...
    function activateDropping() external onlyOwner {
        activeDropping = true;
    }

    function deactivateDropping() external onlyOwner {
        activeDropping = false;
    }
}
