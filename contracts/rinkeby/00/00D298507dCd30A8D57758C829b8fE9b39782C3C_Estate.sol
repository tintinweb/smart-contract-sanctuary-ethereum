...
interface IStaking {
	function stakeMint(uint64 tokenId, address user) external;
...
contract Estate is ERC721ABurnable, Ownable, ReentrancyGuard {
	...
	// Generations breakdown assuming supply is not reduced
	// Gen 1 = 10000
	// Gen 2 = 5000
	// Gen 3 = 2500
	// Gen 4 = 1250	
	...
	uint256 public immutable maxTeam = 100;
	uint256 public immutable maxWL = 1000;
	// 50% of the mint goes to team + WL sale, remaining 50% goes to public sale
	...
	function privateMint(
		...
		if (_option == 0){
			...
			require(_totalMinted() + _vol <= maxTeam,"Purchase would exceed max supply of Genesis Estates reserved for team");					
		} else {
		...
		require(numberOfWLMintsOnAddress[msg.sender] + _vol <= ADDRESS_WL_MAX_MINTS , "You have already minted your max WL amount");
		require(
			_isvalidsig(
				keccak256(
					abi.encodePacked(
		...
		numberOfWLMintsOnAddress[msg.sender] += _vol;
		if (_mintToStake){
			// mint directly to the staking contract and assign token to actual owner
			_safeMint(address(equityContract),_vol);
			stakingContract.stakeMint(uint64(_currentIndex), msg.sender);
		} else {
		...
		callerIsUser
	{
		...
		require(_totalMinted() + _vol <= maxGenesis,"Purchase would exceed max supply of Genesis Estates");
		require(_totalMinted() + _vol <= maxBatch*mintingPhase,"Purchase would exceed max supply of Estates in this phase.");	
		require(numberMinted(msg.sender) + _vol < ADDRESS_MAX_MINTS, "Amount to be minted would exceed maximum per address.");
		require(PublicPrice * _vol == msg.value,"Eth amount sent is not enough.");
		if (_mintToStake){
			_safeMint(address(equityContract),_vol);
			stakingContract.stakeMint(uint64(_currentIndex-1), msg.sender);
		} else {
			_safeMint(msg.sender, _vol);
		}
	}
	...
	// Gen 1 = 1-10000
	// Gen 2 = 10001-15000 (costs 180 to create)
	// Gen 3 = 15001-17500 (costs 360 to create)
	// Gen 4 = 17501-18750 (costs 720 to create)
    ...
    function groupStake(uint64[] memory tokenIds) external {
        for (uint64 i = 0; i < tokenIds.length; ++i) {
            stakingContract.setStake(tokenIds[i], msg.sender);
