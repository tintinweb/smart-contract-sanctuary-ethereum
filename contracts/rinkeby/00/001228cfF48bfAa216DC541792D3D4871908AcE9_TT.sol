...
contract TT is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    // Please consider that information below is only for example purposes
    string public baseURI;
    string public hiddenURI;
    bytes32 public whitelistMerkleRoot;
    
    uint256 public whitelistCost = 0.099 ether;
    uint256 public publicCost = 0.0125 ether;
    ...
    mapping(address => uint256) adrressAlreadyMinted;

    constructor() ERC721A("TT", "TT") {}

    function mint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable nonReentrant {
        require(!paused, "Contract is paused!");
        require(adrressAlreadyMinted[msg.sender] + _mintAmount < maxMintPerAddress + 1, "Max NFT per address exceeded!");
        ...
        adrressAlreadyMinted[msg.sender] += _mintAmount;
