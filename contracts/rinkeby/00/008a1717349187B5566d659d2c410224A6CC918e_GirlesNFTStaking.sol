...
// File: contracts/girlesNFTStaking.sol
...
contract GirlesNFTStaking is Ownable, ReentrancyGuard {
    ...
    function setEmissionRate(uint8 _pid, uint256 _rate) external onlyOwner {
        poolInfo[_pid].emissionRate = _rate;
    }
    ...
    function emergencyWithdrawAll(uint8 _pid) external {
        (uint256[] memory tokenIds,) = activeTokenIdsOf(_pid, msg.sender);

        emergencyWithdraw(_pid, tokenIds);
    }
    ...
    function claimAll(uint8 _pid) public nonReentrant{
        ...
        claimReward(_pid, tokenIds);
    }

    function getAllRewards(uint8 _pid, address _owner) public view returns (uint256) {
        (uint256[] memory tokenIds, ) = activeTokenIdsOf(_pid, _owner);

        uint256 totalRewards = 0;

        for (uint256 i = 0; i < tokenIds.length; i++) {
            totalRewards += (block.timestamp - Stakes[_pid][tokenIds[i]].lastClaimedBlockTime) * poolInfo[_pid].emissionRate;
        }
    ...
    function getRewardsByTokenId(uint8 _pid, uint256 _tokenId) 
        ...
        require(Stakes[_pid][_tokenId].isActive, "Not staked");
    ...
    function activeTokenIdsOf(uint8 _pid, address _owner) 
        ...
        uint256 tokenCount = ActiveStakes[_pid][_owner];
        if (tokenCount == 0) {
            // Return an empty array
            return (new uint256[](0), new string[](0));
        } else {
            uint256[] memory tokenIds = new uint256[](tokenCount);
            string[] memory tokenUrls = new string[](tokenCount);
            uint256 index;
            for (index = 0; index < tokenCount; index++) {
                tokenIds[index] = stakedTokenIdByIndex(_pid, _owner, index);
                tokenUrls[index] = ERC721(poolInfo[_pid].nftContract).tokenURI(tokenIds[index]);
            }
    ...
    function getStakes(uint8 _pid, uint256[] memory _tokenIds) 
        ...
        Stake[] memory stakes = new Stake[](_tokenIds.length);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            stakes[i] = Stakes[_pid][_tokenIds[i]];
        }
        return stakes;
    }
    ...
    function stakedTokenIdByIndex(uint8 _pid, address _owner, uint256 _idx) 
        ...
        return CurrentStakedTokens[_pid][_owner].at(_idx);
    }
}