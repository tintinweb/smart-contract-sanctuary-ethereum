...
contract ConditionalWager is BartrrBase {
    ...
    struct Wager {
        ...
        uint256 feeUserA; // fee userA paid
    ...
    function fillWager(uint256 _wagerId) external payable nonReentrant {
        ...
        if (wager.userB != address(0)) { // p2p
            if (wager.paymentToken == address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)) { // ETH
                ...
                wager.feeUserA = _calculateFee(wager.amountUserA, wager.paymentToken); // fee collected immediately for p2m
                IERC20(wager.paymentToken).safeTransfer(
                    feeAddress,
                    wager.feeUserA
                );

                wager.amountUserA = wager.amountUserA - wager.feeUserA;
    ...
    /// @param _wagerId id of the wager
    function cancelWager(uint256 _wagerId) external {
        Wager memory wager = wagers[_wagerId];
        ...
        if (wager.userB == address(0)) {
            ...
            // Refund the fee
            uint256 amount = (wager.amountUserA + wager.feeUserA);
            if (wager.paymentToken == address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)) {
                _transfer(payable(wager.userA), amount);
            } else {
                IERC20(wager.paymentToken).safeTransfer(wager.userA, amount);
            }
        }
        emit WagerCancelled(_wagerId, wager.userA);
    }

    /// @notice Refunds a wager when the "kill switch" is activated for its wagerToken
    /// @param _wagerId id of the wager
    /// @dev Does not refund any collected fee
    function refundWager(uint256 _wagerId) external {
        Wager memory wager = wagers[_wagerId];
        require(refundableToken[wager.wagerToken], "wager is not refundable");

        require(!wager.isClosed, "Wager has already been closed");
        if (wager.isFilled) {
            require(endTimes[_wagerId] > block.timestamp, "wager already complete");
        }
    ...
    /// @param _wagerId id of the wager
    function redeem(uint256 _wagerId) external {
        Wager memory wager = wagers[_wagerId];
        require(!wager.isClosed, "Wager has already been closed");
        ...
        emit WagerRedeemed(_wagerId, winner, winningSum);
    }
...
contract BartrrBase is Ownable, RoundIdFetcher, ReentrancyGuard {
    ...
    uint256 public immutable MIN_WAGER_DURATION = 1 days;
    uint256 public idCounter; // Counter for the wager id
    bool public isInitialized;

    event WagerCancelled(uint256 indexed wagerId, address indexed userA);
    ...
    function killSwitch(address _token, bool _update) external onlyOwner {
        refundableToken[_token] = _update;
    }

    /// @param _feeAddress address of the fee recipient
    function init(address _feeAddress) external onlyOwner {
