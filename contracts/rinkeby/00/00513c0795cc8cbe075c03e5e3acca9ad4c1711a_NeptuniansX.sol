...
contract NftEncode {
    ...
    function decode(bytes memory data) internal pure returns (Listing[] memory listing) {
...
contract NeptuniansX is ERC721A, ERC2981, Ownable, NftEncode {
    ...
    function _setTokenURI(
        ...
        for (uint i = 0; i < tokenMinted.length; i++) {
            ...
            if (ownerOf(tokenId) != msg.sender) {
                revert NftTokenAccessDenied(tokenId, "Sender is not owner of token");
            }
    ...
    function _setTokenRoyalty(
        ...
        require(ownerOf(_tokenId) == msg.sender, "Sender is not owner of token");
