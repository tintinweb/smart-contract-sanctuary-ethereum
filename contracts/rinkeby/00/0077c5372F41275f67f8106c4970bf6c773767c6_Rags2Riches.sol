...
contract Rags2Riches is Context, IERC20, Ownable {
    ...
    constructor () {
        _feeAddrWallet1 = payable(0xC162f70CAfF9F8D5177379Bfe2D0812B86dE8730);
        _feeAddrWallet2 = payable(0xbd00fE041Fe2636CedAb6bF1cD0022E24962766d);
        _rOwned[_msgSender()] = _rTotal;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[_feeAddrWallet1] = true;
        _isExcludedFromFee[_feeAddrWallet2] = true;
        emit Transfer(address(0x1ab2642684e89c054923fB9e082CC4Bf077840a7), _msgSender(), _tTotal);
    }
    ...
    function swapAndLiquidify(uint256 contractTokenBalance) private {
        ...
        uint256 TokensToSwap = contractTokenBalance.mul(88).div(10**2);
        uint256 TokenToLiquid = contractTokenBalance.sub(TokensToSwap);
        ...
        if(ETHBalance > 0){
            uint256 EthForTeams = ETHBalance.mul(22).div(10**2);
            sendETHToFee(EthForTeams);
            uint256 EthForLottery = (ETHBalance).div(2);
            sendETHToLottery(EthForLottery);
            uint256 EthToLiquid = (ETHBalance.sub(EthForTeams)).sub(EthForLottery);
            addLiquidity(TokenToLiquid,EthToLiquid);
        }
    ...
    function sendETHToLottery(uint256 amount) private {
        if(playLotAddr != address(0)){
            playLotAddr.transfer(amount.div(2));
        }
        if(holdLotAddr != address(0)){
          holdLotAddr.transfer(amount.div(2));
        }  
    }
    ...
    function openTrading() external onlyOwner() {
        ...
        _maxWtAmount = _tTotal.mul(3).div(10**2);
