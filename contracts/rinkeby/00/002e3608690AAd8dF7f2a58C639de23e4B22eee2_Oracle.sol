...
contract Oracle is IOracle, IncreOwnable {
    ...
    /// @notice A safety mechanism, to be used only if the chainlink gets compromised or stops working
    ...
    function setHeartBeat(address asset, uint24 newHeartBeat) external override onlyGovernance {
        if (address(assetToOracles[asset].aggregator) == address(0)) revert Oracle_UnsupportedAsset();
    ...
    /// @dev Pass the erc4626Shares to account for slippage in the ERC4626 contract
    /// @param asset Vault asset address
    /// @param erc4626Shares Vault asset balance. 18 decimals
    /// @return price Price of the collateral asset. 18 decimals
    function getPrice(address asset, int256 erc4626Shares) external view override returns (int256 price) {
        if (address(assetToOracles[asset].aggregator) == address(0)) revert Oracle_UnsupportedAsset();
        ...
        if (assetOracle.isVaultAsset) {
            ...
            uint256 colBalance = LibReserve.wadToToken(
                IERC20Metadata(asset).decimals(),
                erc4626Shares.abs().toUint256()
            );
            ...
            assetBalanceWeiPerUnit = assetBalanceWei.wadDiv(erc4626Shares.abs().toUint256());
        }
