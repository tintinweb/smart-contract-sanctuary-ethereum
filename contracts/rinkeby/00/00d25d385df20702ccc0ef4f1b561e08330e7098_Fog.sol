...
contract Fog is ERC721A, Ownable, ReentrancyGuard {
    ...
    function startAuction(uint256 duration, uint256 startPrice, uint256 endPrice) external onlyOwner {
        ...
        status = true;
        emit saleStart(duration, SALE_START_TIME);
    }
    ...
    function mint(uint256 _numberofTokens) external payable nonReentrant {
        ...
        require(status, "Auction is not live");
        require(_numberofTokens > 0 ,"You must mint at least 1 token");
        ...
        require(ts + _numberofTokens <= MAX_SUPPLY, "The collection is Sold Out");
    ...
    function airdropBatch(address[] calldata addr) external onlyOwner {
        uint256 ts = totalSupply();
        require(ts + addr.length <= MAX_SUPPLY, "The collection is Sold Out");
    ...
    function getPrice() public view returns(uint256) {
        ...
        if (elapsed >= SALE_START_TIME) {
            return END_PRICE;
        } else {
            int256 tPrice = int256(START_PRICE) +
                ((int256(END_PRICE) -
                        int256(START_PRICE)) /
                    int256(SALE_START_TIME)) *
                int256(elapsed);
    ...
    function getElapsedTime() internal view returns(uint256) {
        return SALE_START_TIME > 0 ? block.timestamp - SALE_START_TIME : 0;
    }

    /// @notice get remaining time of dutch auction
    function getRemainingTime() external view returns(uint256) {
        if (SALE_START_TIME == 0) {
            return 16200; //returns 4.5 hours, only shows when sale is not live
        }
        if (getElapsedTime() >= SALE_DURATION) {
            return 0;
        }
        return (SALE_START_TIME + SALE_DURATION) - block.timestamp;
    }
