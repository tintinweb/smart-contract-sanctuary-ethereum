...
/// @title El Salvador Coffee Laborer Verification
/// @author Joss Duff
/// @notice This contract allows for coffee laborers in El Salvador to check into 
/// work at a farm by a foreman and get paid by the farm they work at.
/// @dev This is version 1, will be optimized further by September 2022.
contract coffee is Ownable{
    ...
    /// @param _farmAddress is the wallet address that is to be indicated as a farm.
    ...
    /// @notice For determining if an address is a farm's address.
    /// @param _maybeFarm is the address to be checked.
    /// @return bool true if the address is a farm, false otherwise.
    function isAddressFarm(address _maybeFarm) public view returns(bool){
        return isFarm[_maybeFarm];
    }


    // Mapping for keeping track of foreman addresses.
    mapping(address => bool) isForeman;


    /// @notice Is to be called by a farm address to create a new foreman for that farm.
    /// @param _foremanAddress is the address to be indicated as a foreman.
    ...
    /// @notice For determining if an address is a foreman's address.
    /// @param _maybeForeman is the address to be checked.
    /// @return bool true if the address is a foreman, false otherwise.
...
*/
    ...
    /// @notice The farm will call this function with the address of each worker
    /// to pay and also the amount of days to pay them for as msg.value.
    ...
    /// @dev gas costs can be optimized by replacing require statements with
    /// custom solidity errors.
    /// @param _worker is the address of the worker to be paid msg.value ether.
    function payWorker(address _worker) public payable onlyFarm {
        ...
        // payable.send is unsafe.  Use a direct call to transfer Eth
        (bool success, ) = payable(_worker).call{value: _amount}("");
    ...
    /// @notice Foreman calls checkIn with the worker address and the date to 
    /// indicate a worker worked on that date. 
    /// Emits the workerCheckedIn event so the frontend can handle sorting 
    /// which days the worker worked for, and which days the worker is unpaid for.
    /// @dev using a custom date instead of solidity timestamps in order to handle
    /// the case in which a foreman forgot to check in a worker on a certain day.
    /// @param _workerAddress is the address of the worker who worked on a day.
    /// @param _date is passed in by the foreman's frontend.
    ...
    /// @notice Getter function to return the string array in the daysCheckedIn mapping
    /// @return array of strings of the days which a worker was checked in
