...
contract v2 is Ownable, PaymentSplitter, ERC721A, ReentrancyGuard {

    // Supply
    uint256 public constant RARES = 6; // 6 1of1s
    uint256 public constant HOLDERS = 6800; // from snapshot
    ...
    constructor(
      ...
      ERC721A("alienfrensv2", "alienfrensv2", maxMint, MAX_TOKENS)
    ...
    function rareMint(address to) external onlyOwner {
      require(totalSupply() + 1 <= RARES, "Minting would exceed rares supply");
      _safeMint(to, 1);
    }
    ...
    function IncubatorMint() external nonReentrant callerIsUser {
      ...
      // overflow check
      require(totalSupply() + 1 <= MAX_TOKENS, "Incubator minting would exceed max supply");
      // must have atleast 1 Incubator
      require(IAlienFrensIncubator(IncubatorContract).balanceOf(msg.sender, IncubatorId) > 0, "You do not have enough Incubator");

      // burn Incubator
      IAlienFrensIncubator(IncubatorContract).burnIncubatorForAddress(msg.sender);
    ...
    function preMint(bytes32[] calldata proof) external payable nonReentrant callerIsUser {
      // activation check
      require(preMintActive, "Pre minting is not active");
      require(nonIncubatorMints + 1 <= HOLDERS, "Mint would exceed max supply");
      require(_preSaleListCounter[msg.sender] + 1 <= maxMint, "Only 1 mint per wallet");
      require(PRICE * 1 == msg.value, "Incorrect funds");

      // check proof
      bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
      require(MerkleProof.verify(proof, preRoot, leaf), "Invalid MerkleProof");
    ...
    // @dev preSale() only mints max 1 token to holder
    function publicMint(bytes32[] calldata proof) external payable nonReentrant callerIsUser {
      // activation check
      require(pubMintActive, "Public minting is not active");
      require(nonIncubatorMints + 1 <= HOLDERS, "Mint would exceed holder supply");
      require(_pubSaleListCounter[msg.sender] + 1 <= maxMint, "Only 1 mint per wallet");
      require(PRICE * 1 == msg.value, "Incorrect funds");

      // check proof
      bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
      require(MerkleProof.verify(proof, pubRoot, leaf), "Invalid MerkleProof");
