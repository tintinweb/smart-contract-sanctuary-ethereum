...
//@notice This contract is used for staking Punkz NFT's and mint Degenz tokens according to the tokenomics mentioned in the whitepaper
  ...
  //@getter
    ...
    function lockInternal(uint[] memory tokenIds) internal {
        for(uint i=0;i<tokenIds.length;i++){
            ...
            require(block.timestamp - currentToken.lockTime > 10 days,"Already locked"); //Need to not be locked to lock
    ...
    //@notice This function is used to unstake the staked tokens. PS: tokens which are locked cannot be unstaked within 10 days of locking period
    ...
    function unstakeTokens(uint[] memory tokenIds) external {
        ...
        for (uint i; i< tokenIds.length; i++) {
            ...
            require(block.timestamp - currentToken.lockTime > 10 days,"Not unlocked"); //Needs to be unlocked
    ...
    function rewardFromMint(uint[] memory tokenIds) external{
        ...
        for(uint i=0;i<tokenIds.length;i++){
            ...
            require(stakedTokens[tokenIds[i]].tokenOwner == msg.sender,"Not token owner");
    ...
    //@notice This function used be used by the front-end to fetch the unclaimed amount value calculation
    ...
    function getReward(uint tokenId) public view returns(uint){
            ...
            uint reward;
            if(block.timestamp - currentToken.lockTime <= 10 days){
                ...
                uint unlockTime = currentToken.lockTime + 10 days;
                if(block.timestamp > unlockTime && currentToken.lastClaim < unlockTime){
                    //last claim before lock end but current time after
                    uint timeLocked = (currentToken.lockTime + 10 days - currentToken.lastClaim)/1 days;
                    uint timeUnlocked = (block.timestamp - (currentToken.lockTime + 10 days))/1 days;
                    uint combo = currentToken.lockedPieces;
                    console.log('Combo',combo);
                    reward = 1 ether*timeLocked*rewards[tokenId/2222]*(10+multiplier[combo])/10;
                    reward += 1 ether*timeUnlocked*rewards[tokenId/2222] - externallyClaimed[tokenId];
                }
                else{
    ...
    // function rewardFromMint(uint[] memory tokenIds) external{
    //     uint totalAmount;
    //     require (lockUser[msg.sender] == false, 'Still Locked!');
    //     for(uint i=0;i<tokenIds.length;i++){
    //          require (block.timestamp - startTime < 3 days, "Reward Date Over");
    ...
    //         lastClaim[tokenIds[i]] = block.timestamp;
    //     }
    //     if(block.timestamp - lockUser[msg.sender] > 7 days){}
    ...
    //     uint totalAmount;
    //     uint combo;
    //     require (!lockUser[msg.sender],"Use ClaimLockedValue Function");
    //     for(uint i=0;i<tokenIds.length;i++){
    //         require(tokenOwner[tokenIds[i]]==msg.sender,"Not token owner");
    //         uint timeInDays = (block.timestamp - lastClaim[tokenIds[i]])/ 1 days;
    //         if(timeInDays >= 1) {
    ...
    // // daily claim from locked nft where people will be getting 1.5x more than there NFT returns normally
    // function claimLockedValue() external {
    //     uint totalAmount;
    //     uint combo;
    //     require (lockUser[msg.sender], 'Lock Time Over');
    //     for(uint i=0;i<addressToTotalToken[msg.sender].length;i++){
    //         require(tokenOwner[addressToTotalToken[msg.sender][i]]==msg.sender,"Not token owner");
    //         uint timeInDays = (block.timestamp - lockClaim[addressToTotalToken[msg.sender][i]])/ 1 days;
    //         if(timeInDays >= 1) {
    //             combo += 1;
    //         }
    //         totalAmount += timeInDays*rewards[(addressToTotalToken[msg.sender][i]-1)/2222];
    //         lockClaim[addressToTotalToken[msg.sender][i]] = block.timestamp;
    //     }
    //     require(totalAmount > 0,"Nothing to transfer");
    //     if (combo>6) combo = 6;
    ...
    // function lockFunction() external {
    //     require (block.time - lockUser[msg.sender] > 7 days,'Lock Not Over');
    //     uint combo;
    //     uint totalAmount;
    //     lockUser[msg.sender] = block.timestamp;
    //     for (uint i;i<addressToTotalToken[msg.sender].length;i++) {
    //         uint timeInDays = (block.timestamp - lockClaim[addressToTotalToken[msg.sender][i]])/ 1 days;
    //         if(timeInDays >= 1) {
    //             combo += 1;
    //         }
    //         totalAmount += timeInDays*rewards[(addressToTotalToken[msg.sender][i]-1)/2222];
    //         lockClaim[addressToTotalToken[msg.sender][i]] = block.timestamp;
    //         // lastClaim[addressToTotalToken[msg.sender][i]] = block.timestamp + 7 days;
    //         }
    //         if (combo>6) combo = 6;
    //         _mint(msg.sender,(totalAmount* 1 ether)*(10+multiplier[combo])/10);
    // }