...
contract SingleUserCollectionFactory is Ownable, SingleUserCollection {
  address immutable implementationContract;
  // Holders[] public holders;
  // event NewSingleUserCollectionCreated(address indexed implementationContract, address proxy);
...
/*   constructor (address _implementationContract) {
  ...
  constructor() {
    implementationContract = address(new SingleUserCollection());
  }

  function createSingleUserCollectionProxy(string memory name, string memory symbol, string memory contractURI, string memory tokenURIPrefix) external returns (address) {
    address clone = Clones.clone(implementationContract);
    SingleUserCollection(clone).__SingleUserCollection_init_unchained(name, symbol, contractURI, tokenURIPrefix);
    return clone;
  }
  
/*   function getProxySingleUserCollectionAddress() external view returns (address) {
    require(implementationContract != address(0), "implementationContract must be set");
    return implementationContract.predictDeterministicAddress(salt);
  } */

/*   function createSingleUserCollectionProxy() external payable returns(address) {
        return implementationContract.clone();
  } */

/*   create
  clone */

/*   function createSingleUserCollection(bytes memory _data) public returns (address){
