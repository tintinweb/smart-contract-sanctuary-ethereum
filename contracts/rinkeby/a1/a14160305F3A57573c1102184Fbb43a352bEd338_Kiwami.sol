...
contract Kiwami is Ownable, ERC721A, MerkleWhitelist {
    ...
    uint256 public DA_STARTING_TIMESTAMP = 1647926604;
    ...
    uint256 public publicWLStartTime = 1648026684;
    uint256 public miceWLStartTime = 1648113084;
    ...
    */
    ...
    function withdrawInitialFunds() public onlyOwner {
        ...
        require(
            block.timestamp >= DA_STARTING_TIMESTAMP + 86400,
            "Must wait until 1 day after DA starts."
        );

        uint256 DAFunds = DA_QUANTITY * DA_FINAL_PRICE;
        uint256 publicWLFunds = PUBLIC_WL_MINTED * DA_FINAL_PRICE;

        uint256 initialFunds = DAFunds + publicWLFunds;

        INITIAL_FUNDS_WITHDRAWN = true;

        payable(TEAM_USA_ADDRESS).transfer((initialFunds / 100) * 15);
        payable(TEAM_JAPAN_ADDRESS).transfer((initialFunds / 100) * 35);
        payable(MULTISIG_ADDRESS).transfer((initialFunds / 100) * 50);
    }

    function withdrawFinalFunds() public onlyOwner {
        //Require this is 2 weeks after DA Start.
        require(block.timestamp >= DA_STARTING_TIMESTAMP + 1209600);

        uint256 finalFunds = address(this).balance;

        payable(TEAM_USA_ADDRESS).transfer((finalFunds / 100) * 15);
        payable(TEAM_JAPAN_ADDRESS).transfer((finalFunds / 100) * 35);
        payable(MULTISIG_ADDRESS).transfer((finalFunds / 100) * 50);
    }
}
