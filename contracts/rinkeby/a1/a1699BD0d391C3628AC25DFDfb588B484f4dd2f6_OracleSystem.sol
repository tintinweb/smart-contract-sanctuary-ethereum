...
contract OracleSystem is Modifiers, Utils{
    using SafeERC20 for IERC20;
    IERC20 _token;


    function _oracleDealEval(uint256 _dealID) onlyOracle external{
        uint256 _dealDeadLine = DS.getVar().deals[_dealID].deadline;
        uint256 _timeNow = block.timestamp;

        require(_dealDeadLine < _timeNow,"deadLine no reached");
        require(DS.getVar().deals[_dealID].status == 1, "Deal must be OPEN status" );
        require(DS.getVar().deals[_dealID].goods > 0, "Must have tokens in Deal");

        uint8 _buyer = DS.getVar().acceptance[_dealID].buyerChoose;
        uint8 _seller = DS.getVar().acceptance[_dealID].sellerChoose;

        //For Refund to Buyer
        if(_buyer > 0 && _seller == 0){
            oracleForceRefund(_dealID);
            return;
        }

        if(_buyer == 2 && _seller == 2){
            oracleForceRefund(_dealID);
            return;
        }
        //For Pay to Seller
        if(_buyer == 0 && _seller > 0){
            oracleForcePay(_dealID); 
            return;
        }

        if(_buyer == 1 && _seller == 1){
            oracleForcePay(_dealID); 
            return;
        }
    }

    function oracleForceRefund(uint256 _dealID) onlyOracle internal returns(bool){
        uint256 _dealDeadLine = DS.getVar().deals[_dealID].deadline;
        uint256 _timeNow = block.timestamp;
        ...
        //closing the Deal 
        DS.getVar().deals[_dealID].status = 4; //OracleForce
        ...
        if(!flagPenalty) revert("Error in _newPenalty");
        ...
        if(!_success) revert("Error in _success");
       
        (bool _successBuyer)= _token.transfer(DS.getVar().deals[_dealID].buyer, _newamount);
        if(!_successBuyer) revert("Error in _successBuyer");

        return(true);

    }

    function oracleForcePay(uint256 _dealID) onlyOracle internal returns(bool){
        // TODO> Agregar anti Reentry Guard
        _token = IERC20 (DS.getVar().tokens[DS.getVar().deals[_dealID].coin]);
        uint256 _newPenalty = (DS.getVar().defaultPenalty * 10 ** DS.getVar().tokenDecimal[DS.getVar().deals[_dealID].coin]);
        uint256 _fee = (feeCalculation(DS.getVar().deals[_dealID].amount,  DS.getVar().defaultFee)) + _newPenalty;
        ...
        if(!flagAmountFee) revert("Error in flagAmountFee");
        ...
        if(!_successSeller) revert("Error in _successSeller");
