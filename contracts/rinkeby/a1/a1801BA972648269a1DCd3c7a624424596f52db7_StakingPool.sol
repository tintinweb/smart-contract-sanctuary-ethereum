...
contract StakingPool is IStakingPool, Ownable, OrumBase, CheckContract{
    ...
    onlyOwner
    {
        ...
        checkContract(_rewardsPoolAddress);
        checkContract(_oUSDCToken);
        ...
        emit OETHTokenChanged(_oETHToken);
        emit VaultManagerAddressChanged(_vaultManagerAddress);
        emit CommunityIssuanceAddressChanged(_communityIssuanceAddress);
        emit RewardsPoolAddressChanged(_rewardsPoolAddress);
        emit OUSDCTokenChanged(_oUSDCToken);
        emit BufferPoolAddress(_bufferPoolAddress);
    }

     function setBuffer(uint _buffer, uint _staking) external onlyOwner {
        require((_buffer + _staking) == 100, "Staking Pool: Buffer ratios provided is invalid");
    ...
    function withdrawFromStakingPool(uint _amount) external override {
        uint oUSDCAmount = convertUSDCToOUSDC(_amount);
        require(oUSDCToken.balanceOf(msg.sender) >= oUSDCAmount, "StakingPool: Insufficient oUSDC balance");
        ...
        require(usdc_token.balanceOf(address(bufferPool)) >= _amount, "bufferPool insufficient USDC balance");
    ...
    function provideToStakingPool(uint256 _amount) external override {
        ...
        require(allow, "StakingPool: Stable coin deposit not allowed");
        require(usdc_token.balanceOf(msg.sender) >= _amount, "StakingPol: Insufficient USDC balance");
        ...
        bool sentToStakingInterface = _sendUSDCtoStakingPool(msg.sender, externalStakingAddress, stakingAmount);
        require(sentToStakingInterface, "StakingPool : transfer to StakingInterface failed");
        bool sentToBufferPool = _sendUSDCtoStakingPool(msg.sender, address(bufferPool), bufferAmount);
        require(sentToBufferPool, "StakingPool : transfer to BufferPool failed");
    ...
    function sendUSDCtoBorrower(
        ...
        if (_amount > 0) {
            USDC_Lent += _amount;
            bool success = bufferPool.sendUSDC(_to, _amount);
            require(success, "Staking pool: USDC transfer failed from BufferPool");
    ...
    function _requireCallerIsVaultManager() internal view {
        require(
            ...
            "StakingPool: Caller is not VaultManager"
        );
    }

    // Utilisation ratio setter functions
    function setBorrowingUtilisationRatio(uint _ratio) external onlyOwner {
        require(_ratio >= 0 && _ratio <= 100, "StakingPool: Borrowing utilisation ratio change invalid");
        borrowingUTR = _ratio;
    }

    function setDepositUtilisationRatio(uint _ratio) external onlyOwner {
        require(_ratio >= 0 && _ratio <= 100, "StakingPool: Deposit utilisation ratio change invalid");
        depositorUTR = _ratio;
    }
    ...
    function _requireCallerIsVaultManagerOrRewardsPool() internal view {
        require(
            ...
            "StakingPool: Caller is not VaultManager or RewardsPool"
        );
    }
    ...
    */
    ...
    function _requireCallerIsBorrowerOps() internal view {
        require(
            ...
            "StakingPool: Caller is not BorrowerOps"
        );
    }
...
interface ICommunityIssuance { 
    ...
    event StakingPoolAddressSet(address _stakingPoolAddress);
    event TotalOrumIssuedUpdated(uint _totalOrumIssued);

    // --- Functions ---

    function setAddresses(address _orumTokenAddress, address _stakingPoolAddress) external;
