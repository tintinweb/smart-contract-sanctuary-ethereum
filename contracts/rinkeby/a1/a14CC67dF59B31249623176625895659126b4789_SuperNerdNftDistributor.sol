...
contract SuperNerdNftDistributor is AccessControl {
    ...
    function setWhitelistSaleStartTime(uint256 _whitelistStartTime) public onlyAdmin {
    ...
    function setPublicSaleStartTime(uint256 _publicSaleStartTime) public onlyAdmin {
        PUBLIC_SALE_START_TIME = _publicSaleStartTime;
    }

    function setAlphaHolderStartTime(uint256 _alphaHoldersSaleStartTime) public  onlyAdmin{
    ...
    function setPublicSaleMintLimit(uint256 _publicSaleMintLimit) public onlyAdmin {
        publicSaleMintLimit = _publicSaleMintLimit;
    }

    function setWhiteListMintLimit(uint256 _whiteListMintLimit) public onlyAdmin {
    ...
    function whiteListMint(uint256 _num) public payable returns (bool) {
        ...
        require (block.timestamp >= WHITELIST_START_TIME && block.timestamp < WHITELIST_START_TIME + 3 hours, "Time is not suitable for whiteList");
    ...
    function preSaleMint(uint256 _num) public payable returns (bool) {
        ...
        require (block.timestamp >= WHITELIST_START_TIME + 3 hours && block.timestamp < PUBLIC_SALE_START_TIME, "Time is not suitable for presale");
        require(_num > 0, "need to mint at least 1 NFT");
    ...
    function publicSaleMint(uint256 _num) public payable returns (bool) {
        ...
        require (block.timestamp >= PUBLIC_SALE_START_TIME, "Time is not suitable for Public Sale");
    ...
    function airdropForAlphaUsers(address wallet, uint256 _mintCount, uint256 _canMintTotal, bytes memory _signature) public returns (bool) {
        ...
        address signerOwner = signatureWallet(wallet, _mintCount, _canMintTotal, _signature);
        require(hasRole(MINT_SIGNATURE, signerOwner), "SUPERNERDNFT: Not authorized to mint");
    ...
    function signatureWallet(address wallet, uint256 _mintCount, uint256 _canMintTotal, bytes memory _signature) public pure returns (address){
        return ECDSA.recover(keccak256(abi.encode(wallet, _mintCount, _canMintTotal)), _signature);
    }
    ...
    function removeFromwhiteListUsers(address[] calldata entries) public  onlyAdmin{
