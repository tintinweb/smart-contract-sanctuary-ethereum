...
contract DegenToken is ERC20, ERC20Burnable, Pausable, Ownable, ERC20Permit, ReentrancyGuard {
	...
	event ClaimDegensPerTChanged(address indexed token, uint256 degens);
	...
	function addNewContractForClaims(
		...
		newC.addr = _addr;
		newC.active = _active;
		newC.degensPerToken = _degensPerToken;

		if (_log) {
			// set for new token contracts to be picked up by subgraph
			emit NewTokenAdded(_addr);
		}
	}
	...
	function setClaimContractDegensPerToken(address _addr, uint256 _degens) public onlyOwner {
		for (uint256 i = 0; i < claims.length; i++) {
			if (claims[i].addr == _addr) {
				claims[i].degensPerToken = _degens;
				emit ClaimDegensPerTChanged(_addr, _degens);
				return;
			}
		}
		revert("contract not found");
	...
	function isTokenClaimed(address _addr, uint256 _tokenID) external view returns (bool) {
		for (uint256 i = 0; i < claims.length; i++) {
			if (claims[i].addr == _addr) {
				return BitMaps.get(claims[i].tokens, _tokenID);
			}
		}
	...
	function getClaimsDegensPerToken(address _addr) public view returns (uint256) {
		for (uint256 i = 0; i < claims.length; i++) {
			if (claims[i].addr == _addr) {
				return claims[i].degensPerToken;
			}
		}
		revert("contract not found");
	}

	function getClaimsActive(address _addr) public view returns (bool) {
		for (uint256 i = 0; i < claims.length; i++) {
			if (claims[i].addr == _addr) {
				return claims[i].active;
			}
		}
		revert("contract not found");
	}
	...
	function claim(UserClaim[] calldata _toClaim) external nonReentrant whenNotPaused {
		...
		IERC20(this).transfer(msg.sender, claimableTokensNum * (10**decimals()));
