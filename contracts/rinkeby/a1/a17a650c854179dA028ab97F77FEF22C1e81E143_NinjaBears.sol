...
contract NinjaBears is Ownable, ERC721A, ReentrancyGuard {
    ...
    // metadata URI
    string private _baseTokenURI = "ipfs://bafybeidjr4wmw2ohn3uc6jeur2gudbzx4ie332vnxmt4heoz4ekvvoviba/bears-meta/";

    // screensaver metadata

    string private _screensaverURI = 'ipfs://bafkreid6rhzqtei2ceiyi4frmnawcmxi7qzdo5ptcid6mvbxhv66tcujqa';
    ...
    uint256 public preSalesPrice = 0.08 ether;
    ...
    uint256 public preSaleRelease = 1648893000;
  ...
  function triggerReveal() public onlyOwner {
  ...
  function buyTokensOnPresale(uint256 tokensNumber, bytes calldata signature) public payable {
      ...
      require(block.timestamp >= preSaleRelease, "Purchase is not available now");
      require(tokensNumber <= maxPerAddressDuringMint, "You cannot purchase more than 5 tokens at once");
      require(_mintedTokens[msg.sender].add(tokensNumber) <= maxPerAddressDuringMint, 'You cannot purchase more then 5 tokens on Presale');
  ...
  function buyTokens(uint256 tokensNumber) callerIsUser public payable {
        require(publicSaleActive, "Sale is closed at this moment");
        require(tokensNumber <= maxPerAddressDuringMint, "You cannot purchase more than 1 tokens at once");
        require(_mintedTokens[msg.sender].add(tokensNumber) <= maxPerAddressDuringMint, 'You cannot purchase more then 5 tokens');
  ...
  function setPreSalesRelease(uint256 _releaseTime) public onlyOwner {
    require(_releaseTime > block.timestamp, "Release time must be greater than last mined block timestamp");
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory revealedURI = bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString(), _extension)) : "";
        return isRevealed ? revealedURI : _screensaverURI;
    }
