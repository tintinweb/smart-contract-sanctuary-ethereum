...
contract COSMIQS is ERC721A, Ownable, PaymentSplitter  {
    ...
    mapping(address => bool) private onWhitelist;
    ...
    function mintCosmiq(uint256 _tokenAmount) external payable {
        ...
        if (msg.sender != owner()) {
          ...
          if(hasWhitelist == true) {
                ...
                require(amountMinted[_msgSender()] + _tokenAmount <= whitelistMaxMint, "Already reached max whitelist mint!");
            } 
        require(msg.value == mintPrice * _tokenAmount, "Wrong ETH amount!");
        }

        _safeMint(_msgSender(), _tokenAmount);
        amountMinted[_msgSender()] += _tokenAmount;
    }
