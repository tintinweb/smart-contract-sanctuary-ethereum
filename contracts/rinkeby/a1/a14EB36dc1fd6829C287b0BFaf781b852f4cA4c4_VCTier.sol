...
contract VCTier is IVCTier, OwnableUpgradeable, SuperAdminControl {
        ...
        returns (VCNFTTier memory)
    {
        ...
        uint256 maxLTVFromNFTTier = IGovTier(govTier)
            .getSingleTierData(vcNftTiers[vcTiersKeys[0]].traditionalTier)
            .loantoValue;
        address maxVCTierAddress = vcTiersKeys[0];

        for (uint256 i = 1; i < vcTierlength; i++) {
            //user owns nft balannce
            uint256 tierLoantoValue;

            tierLoantoValue = IGovTier(govTier)
                .getSingleTierData(vcNftTiers[vcTiersKeys[i]].traditionalTier)
                .loantoValue;
        ...
        if (IERC721(maxVCTierAddress).balanceOf(_wallet) > 0) {
