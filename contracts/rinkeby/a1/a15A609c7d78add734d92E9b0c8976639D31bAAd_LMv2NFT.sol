...
/// @title LMv2NFT
/// @notice A contract for secondary version living modules in the starlink ecosystem
contract LMv2NFT is ERC721("Living Module Version 2", "LMV2"), Ownable, IERC721Receiver {

    /// @notice event emitted when token URI is updated
    event LMv2TokenUriUpdated(
        uint256 indexed _tokenId,
        string _tokenUri
    );

    /// @notice event emitted when creator is updated
    event LMv2CreatorUpdated(
        uint256 indexed _tokenId,
        address _creator
    );
    ...
    function mint(address _beneficiary, address _creator, string memory _tokenUri) external onlyOwner returns (uint256) {
        ...
        emit LMv2TokenUriUpdated(tokenId, _tokenUri);
        emit LMv2CreatorUpdated(tokenId, _creator);
        return tokenId;
    }    

    /**
     @notice Unmigrate LMv2 NFT to the old STLM contract
    ...
    function batchMint(address[] calldata _owners, address[] calldata _creators, string[] calldata _uris) external onlyOwner {
        ...
        for(uint256 i=0; i<_uris.length; i++) {
            ...
            emit LMv2TokenUriUpdated(tokenId, _uris[i]);
            emit LMv2CreatorUpdated(tokenId, creators[tokenId]);
        }
    }
    ...
    function setTokenURI(uint256 _tokenId, string calldata _tokenUri) external onlyOwner {
        ...
        emit LMv2TokenUriUpdated(_tokenId, _tokenUri);
    }
    ...
    function batchSetTokenURI(uint256[] memory _tokenIds, string[] calldata _tokenUris) external onlyOwner {
        ...
        for( uint256 i; i< _tokenIds.length; i++){
            ...
            emit LMv2TokenUriUpdated(_tokenIds[i], _tokenUris[i]);
        }
    }
    ...
    function batchSetCreator(uint256[] memory _tokenIds, address[] calldata _creators) external onlyOwner {
        ...
        for( uint256 i; i< _tokenIds.length; i++){
            ...
            emit LMv2CreatorUpdated(_tokenIds[i], creators[_tokenIds[i]]);
        }
    }
