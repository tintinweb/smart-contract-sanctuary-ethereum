...
//        emit dieRolled(8, DiceUtilities.dieRoll(8));
        ...
        uint8 styleRoll = uint8(DiceUtilities.dieRoll(100)) + 1;
        string memory style = DiceUtilities.rollToStyle(styleRoll);
...
//            addedUp += uint8(uint256(keccak256(abi.encodePacked(randomValue, i, tx.gasprice))) % die + 1);
    ...
    function rollToStyle(uint roll) internal pure returns (string memory) {
        if      (roll <= 4) return "0core";
        else if (roll <= 6) return "Acid panda";
        else if (roll <= 8) return "Beastie";
        else if (roll <= 12) return "Bitcrusher";
        else if (roll <= 14) return "Bloodsport";
        else if (roll <= 17) return "Cadavercore";
        else                 return "Codefolk";
    }
}