...
/** 
 *  SourceUnit: c:\Users\ADMIN\Desktop\United-WorldEx-Contract\contracts\LimitOffer.sol
*/
...
contract LimitOffer is Ownable, ReentrancyGuard {
    ...
    function buyNow(bytes memory id) external nonReentrant onlyAdmin {
        ...
        IERC20(controllerAddress).safeApprove(routerAddress, collateralAmount);
        uint[] memory amountOuts = IUniswapV2Router02(routerAddress).swapExactTokensForTokens(collateralAmount, 0, path, user, block.timestamp + 60);

        order.offerCollateralAmount = 0;
        order.offerUAssetAmount = 0;

        emit Offer(msg.sender, uAssetAddress, id, amountOuts[1], collateralAmount, block.timestamp);
    }

    function sellNow(bytes memory id) external nonReentrant onlyAdmin {
        uint256 royaltyFee = IController(controllerAddress).royaltyFeeRatio();
