...
contract Trade {
    ...
    event TestEmitAddr(address indexed val);
    ...
    function getFees(uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId) public view returns(Fee memory){
        ...
        uint256 price = paymentAmt.sub(platformFee);
        uint256 assetFee = price.sub(royaltyFee);
    ...
    function executeBidEmit(Order memory order) public returns(bool) {
        ...
        emit TestEmitUnit(fee.platformFee);
        emit TestEmitUnit(fee.assetFee);
        emit TestEmitUnit(fee.royaltyFee);
        emit TestEmitUnit(fee.price);
        emit TestEmitAddr(fee.tokenCreator);

        if(order.nftType == BuyingAssetType.ERC721) {
            emit TestEmitStr('721 1111');
        }
        else if(order.nftType == BuyingAssetType.ERC1155) {
            emit TestEmitStr('1155 1111');
        }
        else {
            emit TestEmitStr('last 1111');
        }

        require((order.amount >= order.unitPrice * order.qty), "Paid invalid amount");
        return true;
    }
