...
abstract contract NFTSale is Ownable {
    ...
    /// @dev We keep supplies as a struct (an a 3-elem array on constructor) purely to get around
    /// the limit of 16 local variables in solidity
    struct Supplies {
        uint16 pub;
        uint16 treasury;
    }
    ...
    uint64 public immutable startPublic;
    uint64 public immutable startWhitelist;
    ...
    mapping(address => bool) public whitelistRedeems;
    ...
    /// @param _supplies Supplies for [whitelist, public, treasury]
    ...
    constructor(
        ...
        supplies.pub = _supplies[0];
        supplies.treasury = _supplies[1];
    ...
    modifier onlyDuringWhitelistSale() {
        if (
            supplies.pub == 0 || _outsideBounds(startWhitelist, startPublic - 1)
        ) {
    ...
    modifier onlyDuringPublicSale() {
        if ((supplies.pub == 0) || startPublic > block.timestamp) {
    ...
    modifier useWhitelist(bytes32[] calldata _proof) {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        if (!MerkleProof.verify(_proof, whitelistMerkleRoot, leaf)) {
            revert NotWhitelisted();
        }

        if (whitelistRedeems[msg.sender]) {
            revert AlreadyRedeemed();
        }

        whitelistRedeems[msg.sender] = true;
        _;
    }
    ...
    function mintWhitelist(bytes32[] calldata _proof, uint16 _quantity)
        ...
        ensureETHAmount(priceWhitelist, _quantity) // TODO test this
        useWhitelist(_proof)
    {
        // TODO test this with more than 1 mint
        if (_quantity > whitelistMax) {
            revert AccountMaxExceeded();
        }

        supplies.pub -= _quantity;
    ...
    /// Mints a given quantity from the treasury supply
    /// @param _to Owner of the newly minted items
    /// @param _quantity How many items to mint (capped by {supplies.treasury})
    function mintTreasury(address _to, uint16 _quantity) external onlyOwner {
        supplies.treasury -= _quantity;
        _mintFromSale(_to, _quantity);
    }

    /// Withdraws all ETH in the contract to the owner wallet
    /// @dev only callable by the contract owner
    /// TODO should we prevent withdraws before the end period?
    ...
    // TODO test this one
    function transferERC20Token(IERC20 token, uint256 amount)
        external
        onlyOwner
    {
    ...
    function _mintFromSale(address to, uint256 quantity) internal virtual;
}
...
abstract contract RandomnessBatches {
    ...
    /// @param _batches How many batches to split max supply into
    constructor(uint128 _maxSupply, uint128 _batches) {
        maxSupply = _maxSupply;
        batchSize = _maxSupply / _batches;
        randomness = new bytes32[](_batches);
    ...
    function _rngTryReveal(uint256 _newSupply) internal {
        uint256 idx = (uint128(_newSupply) / batchSize);
    ...
    function shuffleID(uint256 _id) public view returns (bool, uint256) {
        uint256 idx = (uint128(_id) / batchSize);
        uint256 rand = uint256(randomness[idx]);

        if (rand == 0) {
            return (false, 0);
        } else {
            uint256 batchOffset = idx * batchSize;
            uint256 shuffled = ((_id + rand) % batchSize);

            return (true, batchOffset + shuffled);
        }
    }

    /// Internal function to reveal a batch, either once its filled,
    /// or by force from an owner-only call
    function _rngReveal(uint256 _batchIdx) internal {
        if (randomness[_batchIdx] == bytes32(0)) {
            randomness[_batchIdx] = rng;
        }
    }
}
...
contract WassieverseNFT is NFTSale, RandomnessBatches, ERC721A {
    using Strings for uint256;
    ...
    /// @param _whitelistMerkleRoot merkle root used to authenticate whitelisted mints
    /// @param _revealBatches Number of batches to reveal separately
    ...
    constructor(
        ...
        RandomnessBatches(_supplies[0] + _supplies[1], _revealBatches)
    ...
    /// Checks if a new batch can be revealed
    /// @dev We declare this here instead of inside {RandomnessBatches} due to the need to access {publicSupply}
    modifier rngTryReveal() {
        _;
        _rngTryReveal(totalSupply());
    }
    ...
    {
        (bool revealed, uint256 shuffledID) = shuffleID(id);
        if (!revealed) {
            return unrevealedURI;
        } else {
            return
                string(
                    abi.encodePacked(baseURI, shuffledID.toString(), ".json")
                );
        }
    }

    /// Reveals each individual item
    /// @notice Can only be called by the owner, or by anyone if all items have already been sold
    function forceReveal(uint256 _batchIdx) external onlyOwner {
