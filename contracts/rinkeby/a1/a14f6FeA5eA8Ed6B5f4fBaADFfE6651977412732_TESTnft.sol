...
contract TESTnft is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public finalcurrentMaxSupply = 0;
...
function resetFinalMaxSupply(uint256 _supply) public onlyOwner {
...
/// @dev Enables minting in stages if hasn't been done already
  ...
  function setCurrentMintcurrentMaxSupply(uint256 _supply) public onlyOwner {
    if (finalcurrentMaxSupply == 0) {
        finalcurrentMaxSupply = currentMaxSupply;
    }
    require(_supply <= finalcurrentMaxSupply && _supply >= totalSupply(), 'New supply must be >= currently minted and <= finalcurrentMaxSupply');
