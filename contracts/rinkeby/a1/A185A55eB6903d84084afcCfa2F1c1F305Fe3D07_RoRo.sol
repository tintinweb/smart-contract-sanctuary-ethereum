...
contract RoRo is ERC721A, Ownable {
	...
	function allowFreeMint() external view returns (bool) {
		return  FREE_LIMIT - freeMinted - 2 >= 0;
	}
	...
	function mint(uint count) external payable {
        ...
        if(_roroWhisperCount[msg.sender] < 2) {
			payForCount -= (2 - _roroWhisperCount[msg.sender]);
			if (payForCount < 0) payForCount = 0;
			freeMinted += (count - payForCount);
        }
