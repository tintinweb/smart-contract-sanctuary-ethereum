...
contract Dojo is IDojo, ERC165, ReentrancyGuard, AdminControl {
    ...
    string constant public override symbol = "DOJO";
    ...
    function exitDojo(
        ...
        _validateSignature(dojoIds, xpList, message, signature, nonce);

        emit CollectedXP(msg.sender, dojoIds, xpList);
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            require(xpList[i] <= daysElapsed * _maxDailyXP, "Exceeds daily limit");

            _owners[id].ownerAddress = address(0);
            _balances[msg.sender] -= 1;
            _xp[id].lastUpdateTime = 0;
            _xp[id].value += xpList[i];
    ...
    function returnLostChimp(
        ...
        if (isGenesisChimp) {
            IERC721(_chimpersGenesis).safeTransferFrom(address(this), to, tokenId);
        } else {
            IERC721(_chimpersGenerative).safeTransferFrom(address(this), to, tokenId);
        }
    }
...
interface IDojo is IERC721, IERC721Metadata, IERC721Receiver {
     ...
     * Requirements:
     * 
     * - The caller must own the staked chimp with the provided `tokenId`
     * - `quest` must point to an active and approved quest contract
     */
