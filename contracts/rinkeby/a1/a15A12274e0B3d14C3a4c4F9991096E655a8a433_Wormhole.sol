...
contract Wormhole is IWormhole, ERC1155Holder, Ownable, Pausable, ReentrancyGuard {
  ...
  uint256 public constant TICKET_CARD_PRICE = 50;
  ...
  function balanceOfBox(uint256 boxId) public view override returns (uint256) {
    return nftspacexBox.balanceOf(address(this), boxId);
  }

  function balanceOfItem(uint256 itemId) public view override returns (uint256) {
    return nftspacexCreatureAccessory.balanceOf(address(this), itemId);
  }

  function balanceOfTicketCard(uint256 cardId) public view override returns (uint256) {
    return nftspacexTicketCard.balanceOf(address(this), cardId);
  }
  ...
  function burnExpiredTC(uint256 id) public onlyAdmin {
    nftspacexTicketCard.burn(address(this), id, balanceOfTicketCard(id));
  }

  function buyTicketCard(uint256 amount) public override whenNotPaused nonReentrant {
    require(canBuyTicketCard(amount), "Wormhole: insufficient fund");
    nftspacexToken.burnFrom(_msgSender(), amount * TICKET_CARD_PRICE * BASE);
    nftspacexTicketCard.safeTransferFrom(address(this), _msgSender(), nftspacexTicketCard.currentIdSupported(), amount, "");
    emit BuyTicketCard(_msgSender(), nftspacexTicketCard.currentIdSupported(), amount);
  }

  function canBuyTicketCard(uint256 amount) public view override returns (bool) {
    return (
      nftspacexTicketCard.isIdExpired(nftspacexTicketCard.currentIdSupported()) &&
      nftspacexToken.balanceOf(_msgSender()) >= amount * TICKET_CARD_PRICE * BASE &&
      balanceOfTicketCard(nftspacexTicketCard.currentIdSupported()) >= amount
    );
  }

  function buyBox(uint256 optionId, uint256 amount) public override whenNotPaused nonReentrant {
    require(canBuyBox(optionId, amount), "Wormhole: inssuficient fund");
  ...
  function canBuyBox(uint256 optionId, uint256 amount) public view override returns (bool) {
    return (
      nftspacexToken.balanceOf(_msgSender()) >= amount * BOX_PRICE[optionId] * BASE &&
      balanceOfBox(optionId) >= amount
    );
  }
...
interface IWormhole {
  event BuyTicketCard(address indexed buyer, uint256 indexed cardId, uint256 amount);
  ...
  function balanceOfTicketCard(uint256 cardId) external view returns (uint256);
  function canBuyTicketCard(uint256 amount) external view returns (bool);
  function canBuyBox(uint256 optionId, uint256 amount) external view returns (bool);
  function canUpgrade(address user, uint256 toOptionId, uint256 amount) external view returns (bool);
  function buyTicketCard(uint256 amount) external;
