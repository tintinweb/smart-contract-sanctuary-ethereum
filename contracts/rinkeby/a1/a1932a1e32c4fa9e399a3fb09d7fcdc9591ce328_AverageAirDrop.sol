  ...
  constructor(
    ...
    function claimPFPs() external nonReentrant {
        ...
        for (uint256 i; i < _avgcreatures.balanceOf(msg.sender); i++) {
            if (!isClaimed(_avgcreatures.walletOfOwner(msg.sender)[i])) {
                _setClaimed(_avgcreatures.walletOfOwner(msg.sender)[i]);
                _safeMint(msg.sender, _avgcreatures.walletOfOwner(msg.sender)[i]);
                numTokensClaimed++;
            }
        }
