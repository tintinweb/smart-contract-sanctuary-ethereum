...
import "./token/ChildMintableERC20.sol";
...
contract Pace is ChildMintableERC20, ERC20Burnable, Ownable {
    ...
    constructor () ChildMintableERC20("Pace", "Pace", _msgSender()) {
        s.taxPercentage = 100;
        s.taxCount = 3;
        s.burnOnTransfer = true;
    }

    function init(address _treasury, address _communityWallet, address _tokenDisperser) public  onlyOwner {
        s.treasury = _treasury;
        s.communityWallet = _communityWallet;
        s.tokenDisperser = _tokenDisperser;
        s.excludedFromTax[_treasury] = true;
        s.excludedFromTax[_communityWallet] = true;
        s.excludedFromTax[_tokenDisperser] = true;
    ...
    function addExcludedAccount(address account) public onlyOwner {
        s.excludedFromTax[account] = true;
    }

    function removeExcludedAccount(address account) public onlyOwner {
        s.excludedFromTax[account] = false;
    }

    /**
     *  @dev Toggles burning on and off.
     *  @notice This adjusts the tax from 3% to 2% total. This is because burn accounts for 1/3 of the total tax.
     */
    function toggleBurn() public onlyOwner {
        ...
        s.burnOnTransfer = false;
        s.taxCount = 3;
        return;
    }

    /**
     *  @dev _transfer() override. Before transferring, applies 3% tax to tokens.
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        unchecked {
            s.balances[from] = fromBalance - amountAfterTax;
        }

        s.balances[to] += amountAfterTax;
    ...
    function _transferFees(address from, address to, uint256 amount) internal virtual returns (uint256) {
        if (s.excludedFromTax[to]) return amount;
        if (s.excludedFromTax[from]) return amount;

        uint256 taxAmount = amount.mul(s.taxPercentage).div(10000);
        _transfer(from, s.treasury, taxAmount);
        _transfer(from, s.communityWallet, taxAmount);
        if (s.burnOnTransfer) _burn(from, taxAmount);
        return amount.sub(taxAmount.mul(s.taxCount));
    }
}
