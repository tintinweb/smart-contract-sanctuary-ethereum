...
import "./civiumshraSigner.sol";

contract CiviShraMint is
    ERC721AUpgradeable,
    civiumshraSigner,
    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        __civiumshraSigner_init(domain,version);
        ...
        whiteListPriceForEach = 0.097 ether; 
        publicMintPriceForEach = 0.097 ether; 
    }
    ...
    function whiteListMint(Locker memory _whitelist, uint256 _amount)
        external
        payable
        nonReentrant
    {
        ...
        require(
             block.timestamp - _whitelist.nonce < 600, "signature Expired" 
        );
        require(
        ...
        require(!nonceUsed[msg.sender][_whitelist.nonce], "nonce already used");
        nonceUsed[msg.sender][_whitelist.nonce] = true;
    ...
    function publicMint(uint256 _amount) external payable onlyEOA nonReentrant {
        require((whiteListEndTime <= block.timestamp ||
                    whiteListMinted == whiteListCap),
            "Allow List Mint Period Not over"
        );
        require(
        ...
        require(totalSupply() + _amount <= MAX_SUPPLY - OwnerCap, "Public Mint all sold");
        require(
...
///@author Anyx
contract civiumshraSigner is EIP712Upgradeable{

    string private SIGNING_DOMAIN; //= "civiumshra";
    ...
    function __civiumshraSigner_init(string memory domain, string memory version) internal initializer {
