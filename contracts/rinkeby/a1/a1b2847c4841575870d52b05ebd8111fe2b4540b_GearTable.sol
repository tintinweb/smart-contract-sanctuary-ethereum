...
contract GearTable is Ownable {
    ...
    function getName(uint16 tier, uint256 seed, string memory gearName) internal view returns (string memory newPrefix) {
        if(tier < 3){
            return string(abi.encodePacked(prefixOne[seed % prefixOne.length], gearName)); 
        }
        if(tier < 5){
            return string(abi.encodePacked(prefixTwo[seed % prefixTwo.length], gearName)); 
        } else {
            return  string(abi.encodePacked(prefixThree[seed % prefixThree.length], gearName, suffixes[seed % suffixes.length])); 
        }
    }
    function getShirt(uint16 level) internal returns(Inventory.Gear memory) {
        uint256 seed = _rand(1);
        uint16 tier = getTier(level, seed);
        return Inventory.Gear(getName(tier, seed, shirt[seed % shirt.length]), tier, 1, GearMapping[tier].minSoulScore + uint16(seed % GearMapping[tier].range));
    }

    function getPants(uint16 level) internal returns(Inventory.Gear memory) {
        uint256 seed = _rand(2);
        uint16 tier = getTier(level, seed);
        return Inventory.Gear(getName(tier, seed, pants[seed % pants.length]), tier, 1, GearMapping[tier].minSoulScore + uint16(seed % GearMapping[tier].range));
    }

    function getHead(uint16 level) internal returns(Inventory.Gear memory) {
        uint256 seed = _rand(3);
        uint16 tier = getTier(level, seed);
        return Inventory.Gear(getName(tier, seed, head[seed % head.length]), tier, 1, GearMapping[tier].minSoulScore + uint16(seed % GearMapping[tier].range));
    }

    function getFeet(uint16 level) internal returns(Inventory.Gear memory) {
        uint256 seed = _rand(4);
        uint16 tier = getTier(level, seed);
        return Inventory.Gear(getName(tier, seed, feet[seed % feet.length]), tier, 1, GearMapping[tier].minSoulScore + uint16(seed % GearMapping[tier].range));
    }

    function getChest(uint16 level) internal returns(Inventory.Gear memory) {
        uint256 seed = _rand(5);
        uint16 tier =  getTier(level, seed);
        return Inventory.Gear(getName(tier, seed, chest[seed % chest.length]), tier, 1, GearMapping[tier].minSoulScore + uint16(seed % GearMapping[tier].range));
    }

    function getShoulders(uint16 level) internal returns(Inventory.Gear memory) {
        uint256 seed = _rand(6);
        uint16 tier =  getTier(level, seed);
        return Inventory.Gear(getName(tier, seed, shoulders[seed % shoulders.length]), tier, 1, GearMapping[tier].minSoulScore + uint16(seed % GearMapping[tier].range));
    }

    function getRing(uint16 level) internal returns(Inventory.Gear memory) {
        uint256 seed = _rand(7);
        uint16 tier =  getTier(level, seed);
        return Inventory.Gear(getName(tier, seed, ring[seed % ring.length]), tier, 1, GearMapping[tier].minSoulScore + uint16(seed % GearMapping[tier].range));
    }

    function getMainhand(uint16 level) internal returns(Inventory.Gear memory) {      
        uint256 seed = _rand(8);
        uint16 tier =  getTier(level, seed);
        return Inventory.Gear(getName(tier, seed, mainhand[seed % mainhand.length]), tier, 1, GearMapping[tier].minSoulScore + uint16(seed % GearMapping[tier].range));
    }

    function getOffhand(uint16 level) internal returns(Inventory.Gear memory) {
        uint256 seed = _rand(9);
        uint16 tier =  getTier(level, seed);
        return Inventory.Gear(getName(tier, seed, offhand[seed % offhand.length]), tier, 1, GearMapping[tier].minSoulScore + uint16(seed % GearMapping[tier].range));
    }

    function getArtifact(uint16 level) internal returns(Inventory.Gear memory) {
        uint256 seed = _rand(10);
        uint16 tier = getTier(level, seed);
        return Inventory.Gear(getName(tier, seed, artifact[seed % artifact.length]), tier, 1, GearMapping[tier].minSoulScore + uint16(seed % GearMapping[tier].range) );
    }
