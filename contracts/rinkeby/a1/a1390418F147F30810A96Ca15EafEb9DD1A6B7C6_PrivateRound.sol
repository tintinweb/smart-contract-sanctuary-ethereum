...
contract PrivateRound is AccessControl {
    ...
    * @param _MLPToken address of mlp token contract
    * @param _rate rate value
    * @param _totalAmount total amount of tokens
    * @param _minOrder min token amount in order
    * @param _maxOrder max token amount in order
    * @param _usdtReceiver address of usdt token receiver
    * @param unlockDates dates for locks
    * @param percents percents for locks
    *
    */
    constructor (
        address _USDTToken,
        ...
        MLPToken = _MLPToken;
        rate = _rate;
        mlpTotalAmount = _totalAmount;
        usdtReceiver = _usdtReceiver;
        require(unlockDates.length == percents.length, 'percents & unlockDates must be same size');
        for (uint256 index = 0; index < unlockDates.length; index++) {
            locks.push(Lock({percent: percents[index], unlockDate: unlockDates[index]}));
        }
    }

    /** @dev swap usdt to mlp token
    *
    * @param _amount amount of usdt token swapped
    *
    */
    function swap(uint256 _amount) external {
        require (_amount > minOrder && _amount < maxOrder, "Order does not match the private round volume limits");
        uint256 mlpAmount = _amount * ratesPrecision / rate;
        require(mlpTotalAmount - mlpAmount >= 0, "claimed amount exceed total amount");//
        ERC20(USDTAddress).safeTransferFrom(msg.sender, usdtReceiver, _amount);
        UserInfo storage user = users[msg.sender];
        usdtReceived += _amount;
        user.amount = user.amount + mlpAmount;
        mlpTotalAmount = mlpTotalAmount - mlpAmount;
        emit TokenExchanged(msg.sender, _amount, mlpAmount);
    }

    /** @dev claims all available stakes
    *
    */
    function claim() external {
        ...
        user.claimed = user.claimed + availableAmount;
        ERC20(MLPToken).safeTransfer(msg.sender, availableAmount);
    ...
    function calcAvailableAmount(UserInfo storage user)
        private
        view
        ...
        availableToken = (
        user.amount * availablePercent / 1000
        );

        if (availableToken >= user.claimed) {
            availableToken = availableToken - user.claimed;
        } else {
            availableToken = 0;
        }
    ...
    */
    ...
    /** @dev migrate users to this contract
    *
    * @param _users array of users
    * @param _amounts amount tokens for user
    ...
    */
    function migrateUsers(
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            require(mlpTotalAmount - _amounts[i] >= 0,  "Not enough tokens on round balance");
            UserInfo storage user = users[_users[i]];
            user.amount = _amounts[i];
            mlpTotalAmount -= _amounts[i];
            user.claimed = _claimed[i];
        }
        emit UserMigrated(_users, _amounts, _claimed);
    }

    /** @dev emergency transfer mlp tokens from round
    *
    */
    function transferMlpToken(address receiver, uint256 amount) external{
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
        ERC20(MLPToken).safeTransfer(receiver, amount);
    }
}
