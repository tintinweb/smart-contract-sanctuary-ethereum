...
contract ShufflerDescriptor is IShufflerDescriptor, Ownable {
     ...
     * @dev This function can only be called by the owner when not locked.
     */
    function addManyCorners(bytes[] calldata _corners, string[] calldata _cornerNames) external override onlyOwner whenPartsNotLocked {
        for (uint256 i = 0; i < _corners.length; i++) {
            _addCorner(_corners[i], _cornerNames[i]);
        }
    }
     ...
     * @notice Add a corner.
     */
    function _addCorner(bytes calldata _corner, string calldata _cornerName) internal {
        corners.push(_corner);
        cornerNames.push(_cornerName);
    }
    ...
    function _getPartNamesForSeed(IShufflerToken.Seed memory seed) internal view returns (string[] memory) {
        string[] memory _partNames = new string[](4);
        _partNames[0] = cardNames[seed.card];
        _partNames[1] = sideNames[seed.side];
        _partNames[2] = cornerNames[seed.corner];
        _partNames[3] = centerNames[seed.center];
        return _partNames;
    }
...
interface IShufflerDescriptor {
    ...
    function addManyCorners(bytes[] calldata corners, string[] calldata  cornerNames) external;
        ...
        returns (string memory traitDesc)
    {
        return string(
            abi.encodePacked(
                ...
                '"},{"trait_type": "Rank & Suites", "value": "', traitNames[2],
                '"},{"trait_type": "Theme", "value": "', traitNames[3],
                '"}]'
            )
        );
    }
