...
contract AGOVLPStaking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function withdraw(uint256 depositId) external whenNotPaused nonReentrant {
        ...
        if (mpwr.balanceOf(address(this)) > 0) {
            mpwr.transfer(_msgSender(), reward);
        }
     ...
     */
    function balancesOf() external view returns (uint256 agovAmount, uint256 mpwrAmount, uint256 wethAmount) {
    ...
    function _addLLCReward(
        ...
        uint256 llcRewardAmount = amountPerLock * (ethAmount / MIN_ETH_AMOUNT);
