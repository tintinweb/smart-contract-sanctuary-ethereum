...
contract CapitalPool is ICapitalPool, OwnableUpgradeable, AbstractDependant {
    ...
    // demand vStable balances updated by(validation, withdraw liq)
    mapping(uint256 => uint256) public regularDemandBalance;
    ...
    modifier onlyReinsurancePool() {
        require(
            ...
            "CP: Caller is not a reinsurance pool contract"
        );
        _;
    }
    ...
    function addDemandHoldersHardSTBL(uint256 _stblReinsurancePoolAmount)
        external
        override
        onlyDemandBookLiquidity
        broadcastBalancing
    {
        reinsurancePoolBalance = reinsurancePoolBalance.add(_stblReinsurancePoolAmount);
        reinsurancePool.addPremium(
            0,
            DecimalsConverter.convertTo18(_stblReinsurancePoolAmount, stblDecimals)
        );

        hardUsdtAccumulatedBalance = hardUsdtAccumulatedBalance.add(_stblReinsurancePoolAmount);
        virtualUsdtAccumulatedBalance = virtualUsdtAccumulatedBalance.add(
            _stblReinsurancePoolAmount
        );
    }
    ...
    /// @dev emits PoolBalancedUpdated event
    /// @param _stblAmount amount hardSTBL ingressed into the system
    function addDemandProvidersHardSTBL(uint256 demandIndex, uint256 _stblAmount)
        external
        override
        onlyDemandBookLiquidity
        broadcastBalancing
    {
        regularDemandBalance[demandIndex] = regularDemandBalance[demandIndex].add(_stblAmount);
        hardUsdtAccumulatedBalance = hardUsdtAccumulatedBalance.add(_stblAmount);
        virtualUsdtAccumulatedBalance = virtualUsdtAccumulatedBalance.add(_stblAmount);
    }
    ...
    function rebalanceLiquidityCushion() public override broadcastBalancing onlyMaintainer {
        ...
        uint256 _pendingWithdrawalDemandAmount =
        ...
        uint256 _requiredLiquidity =
            _pendingWithdrawlAmount.add(_pendingClaimAmount).add(_pendingRewardAmount).add(
    ...
    function _updatePoolLiquidity(
        ...
        if (_lostPercentage > 0) {
            ...
            _lostAmount = _currentLiquidity.mul(_lostPercentage).uncheckedDiv(PERCENTAGE_100);
        }
    ...
    function _dispatchLiquidities(address _policyBookAddress, uint256 _claimAmount) internal {
        ...
        uint256 reinsuranceContribution =
            (policyBookFacade.LUreinsurnacePool().add(policyBookFacade.VUreinsurnacePool()))
                .mul(PERCENTAGE_100)
                .tryDiv(totalPoolLiquidity);
        uint256 reinsuranceLoss =
            _claimAmount.mul(reinsuranceContribution).uncheckedDiv(PERCENTAGE_100);
        ...
        returns (uint256 _actualAmount)
    {
        ...
        regularDemandBalance[demandIndex] = regularDemandBalance[demandIndex].sub(_actualAmount);
    }
        ...
        returns (uint256 _actualAmount)
    {
        ...
        if (!deployFundsToDefi) {
            ...
            hardUsdtAccumulatedBalance = hardUsdtAccumulatedBalance.uncheckedSub(_stblAmount);
            ...
            if (_stblAmount > liquidityCushionBalance) {
                uint256 _diffAmount = _stblAmount.uncheckedSub(liquidityCushionBalance);
                if (hardUsdtAccumulatedBalance >= _diffAmount) {
                    hardUsdtAccumulatedBalance = hardUsdtAccumulatedBalance.uncheckedSub(
                        _diffAmount
                    );
    ...
    {
        return (
            factors.premiumPerDeployment.mul(factors.vStblDeployedByRP).uncheckedDiv(PRECISION)
        );
    }
    ...
    function _calcUserLeveragePoolPremium(PremiumFactors memory factors) internal {
        ...
        for (uint256 i = 0; i < _userLeverageArr.length; i = uncheckedInc(i)) {
            ...
            leveragePoolBalance[_userLeverageArr[i]] = leveragePoolBalance[_userLeverageArr[i]]
                .add(premium);
            ILeveragePortfolio(_userLeverageArr[i]).addPremium(
...
interface IDemandBookLiquidity {
    ...
    function transferFundsAtValidation(
        uint256 demandIndex,
        uint256 _liquidityAmount,
        uint256 _reinsurancePoolAmount
    ) external;
