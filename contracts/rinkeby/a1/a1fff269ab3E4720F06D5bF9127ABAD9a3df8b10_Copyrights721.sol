...
contract Copyrights721 is IERC721Metadata, Ownable, ERC721URIStorage, IERC721Receiver {
  ...
  mapping(address => bool) creatorApproval; //this will hold the address of the content creator and a 0 or 1 for confirmed
  mapping(uint => NFTs) public ContentTokens; //this will hold the Content information, so user inpus 1 and they get the original Contract address and id
  mapping(uint => RightsSale) public SoldRights; //this is where buyers cand go check their rights holdings and this is also how each rights sale is tracked
  mapping(uint => string) public RightsType; //this is a list of the rights id and which right each id represents eg 1=DistributionRight
  mapping(address => mapping(uint=>uint)) CoCreatorPercentage;//this is a mapping of colllaborators' addresses to the content they worked on and the percentage of the sale price they should get
  mapping(uint => mapping(uint => uint256)) public PriceOfRight; //this maps the price of each right of each content token
  mapping(address => bool) ApprovedBuyer; //this maps a list of addresses to boolean values to represent approved buyers
  mapping(uint => mapping(uint => bool)) public AvailableRights; // this is a mapping representing which type of right is for sale for each content token
  mapping(uint => mapping(uint => uint)) public RightsTokensSupply; //this keeps track of the available supply of each right
  mapping(uint => mapping(uint => uint)) ExercisedRights; //keeps count of the amount of times a certain right has been exercised
  mapping(address => uint256) public UsersCreditBalance; //this keeps track of the added and remaining credit
  mapping(uint => mapping(uint => uint256)) public PriceOfRightUsage; //this is where we store the price for the usage of a right
  mapping (uint256 => uint256) public RightForSale; //this is how a current owner indicates that his right is for sale and what the price is
  mapping (uint => RoyaltyTokens) public RoyaltyTokenPay; //Get info about royalty tokens - who holds them and how much should they get paid
  mapping(uint => address) ContentCreatorMapping;
  ...
  constructor() payable ERC721("Copyrights721", "ECF") {}
  ...
  function WithdrawCredit() public payable{
    require(UsersCreditBalance[msg.sender]>= msg.value, "10");//"Insufficient Funds");
    UsersCreditBalance[msg.sender]-=msg.value;
    (bool sent,)=msg.sender.call{value: msg.value}("Funds sent.");
    require(sent, "11");//"Failed to complete transaction.");
  ...
  function SetPrice(uint _contentid, uint _rightsid, uint256 _price, uint256 _usageprice) external onlyContentCreator {
    PriceOfRight[_contentid][_rightsid]=_price;
    PriceOfRightUsage[_contentid][_rightsid]=_usageprice;
  ...
  function setAprovedBuyer(address _operator, bool _approved) external onlyContentCreator {
  ...
  function exerciseRights(uint _uniqueRightsID, uint _AmountOfTimes) external {
    ...
    require(UsersCreditBalance[msg.sender]-(PriceOfRightUsage[SoldRights[_uniqueRightsID]._ContentID][SoldRights[_uniqueRightsID]._RightID]*_AmountOfTimes)>=0, "4");//"Add more credit in order to be able to exercise this right." );
    UsersCreditBalance[msg.sender]-=(PriceOfRightUsage[SoldRights[_uniqueRightsID]._ContentID][SoldRights[_uniqueRightsID]._RightID]*_AmountOfTimes);
    ...
    _ComputeRoyalties(SoldRights[_uniqueRightsID]._ContentID, PriceOfRightUsage[SoldRights[_uniqueRightsID]._ContentID][SoldRights[_uniqueRightsID]._RightID]*_AmountOfTimes);
  }

}

/*
  function GetPriceOfRight(uint _contentid, uint _rightid) public view returns(uint256, uint256) {
    return (PriceOfRight[_contentid][_rightid],PriceOfRightUsage[_contentid][_rightid]);
  }

  function GetCoCreatorPrecentagePay (address _cocreator, uint _contentid) public view returns(uint) {
    return CoCreatorPercentage[_cocreator][_contentid];
  }

  function CheckRightsAvailability(uint _contentid, uint _rightid) public view returns(bool) {
    return AvailableRights[_contentid][_rightid];
  }
  //function getApproved(uint256) public pure override(ERC721, IERC721) returns(address) { revert("disabled"); }
  //function isApprovedForAll(address, address) public pure override(ERC721, IERC721) returns(bool) { revert("disabled"); }
  function checkRemainingSupplyOfSpecificRight(uint _contentid, uint _rightsid) public view returns(uint) {
    return RightsTokensSupply[_contentid][_rightsid];
  }
  function transferRight(address _from, address _to, uint256 _UniqueRightID) external onlyApprovedBuyer {
    require(ApprovedBuyer[_to]==true, "1");//"The address you are attempting to transfer your token to is not an approved buyer.");
    address owner = ownerOf(_UniqueRightID);
    require ( owner == _from, "2");
    _safeTransfer(_from,_to,_UniqueRightID, "");
    emit RightsTransferred(_from, _to, _UniqueRightID, SoldRights[_UniqueRightID]._ContentID, SoldRights[_UniqueRightID]._RightID);
  } 
*/
