...
contract PuffyPlant is ERC721A, Ownable, ReentrancyGuard, PaymentSplitter {
  ...
  address[] public _payees = [
    ...
    0x6aa258e25E9Fe21FD3b51C7CaAd2cE138252B0ea
  ];
  ...
  {
    if (_mintedTokens.current() <= 10 && preMint) {
      bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
      require(
        MerkleProof.verify(_merkleProof, mintMerkleRoot, leaf),
        "Invalid proof"
      );
    ...
    } else if (_mintedTokens.current() > 11 && mintWhiteList) {
  ...
  function setMintWhitelist(bool _state) public onlyOwner {
    mintWhiteList = _state;
  }
