...
contract LOW1 is ERC721, Ownable {
  using Strings for uint256;
  ...
  bool public dutchAuctionStarted = true; // boolean for dutch auction
  ...
  //SMC DUTCH AUCTION
   ...
   function setDutchAuction(bool _state) public onlyOwner {
    ...
    function getCurrentPrice() public view returns (uint256) {
      return cost + ((dutchDuration - getTimeElapsed()) * dutchPriceAdditional / dutchDuration);
    }    

  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    require(!paused, "the contract is paused");
    require(dutchAuctionStarted && block.timestamp >= dutchStartTime, "the Auction has not started yet");
