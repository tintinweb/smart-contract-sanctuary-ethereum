...
contract NFTStaking is Context, Ownable, IERC721Receiver, ERC165, ReentrancyGuard {
    ...
    function allowStaking(address vesting) public onlyOwner {
        require(!isStakingAllowed, "NFTStaking: staking is already allowed");
    ...
    function setRewardForLastPeriod(uint256 amount) public onlyOwner {
        ...
        require(currentPeriod > 0, "NFTStaking: to early for reward");
    ...
    function setBonusForLastPeriod(uint256 amount) public onlyOwner {
        ...
        uint256 currentPeriod = (block.timestamp - allPeriods[0]) / quarter;
        require(currentPeriod > 0, "NFTStaking: to early for bonus");
    ...
    function getCurrentDayOfQuarter() public view returns (uint256) {
        ...
        uint256 day = ((block.timestamp - allPeriods[currentQuarter]) / (1 days)) + 1;
        return day;
    }
    ...
    function claimableRewards() public view returns (uint256) {
        return _calcAccruedRewards(_msgSender());
    }

    function estimatedRewards() public view returns (uint256) {
        return _calcEstimatedRewards(_msgSender());
    }
    ...
    function stake(uint256 tokenId, uint256 pool) public nonReentrant {
        require(isStakingAllowed, "NFTStaking: staking is not allowed");
        require(nftForStaking.getApproved(tokenId) == address(this), "NFTStaking: transfer not allowed");
        require(pool != 0 && pool < 4, "NFTStaking: wrong pool");
        require(_isPoolEnabled[pool - 1] != 0, "NFTStaking: pool is disabled");
    ...
    function _addStake(address staker, uint256 tokenId, uint256 pool) private {
        ...
        uint256 startDay = ((block.timestamp - allPeriods[currentPeriod]) / (1 days)) + 1;
    ...
    function _addShare(uint256 tokenPower, uint256 currentPeriod, uint256 startDay, uint256 pool) private {
        for (uint256 i = currentPeriod; i < 24; i++) {
            if (i == currentPeriod) {
                _allSharesPowerPerStakingPool[pool - 1][i] = _allSharesPowerPerStakingPool[pool - 1][i] + (tokenPower * tokenMultipliers[0] * (91 - startDay));
            }
            else {
                _allSharesPowerPerStakingPool[pool - 1][i] = _allSharesPowerPerStakingPool[pool - 1][i] + (tokenPower * ((tokenMultipliers[i - currentPeriod - 1] * startDay) + (tokenMultipliers[i - currentPeriod] * (91 - startDay))));
            }
        }
    }
    ...
    function _removeStake(address staker, uint256 tokenId) private {       
        ...
        uint256 startDay = _stakes[tokenId][4];
        uint256 tokenPower = _stakes[tokenId][5];
        uint256 endDay = ((block.timestamp - allPeriods[currentPeriod]) / (1 days)) + 1;
        uint256 i;
            ...
            if (pool > 0) {  
                if (duration >= 1) {      
                    for (i = currentPeriod; i < 24; i++) {
                        _allSharesPowerPerStakingPool[pool - 1][i] = _allSharesPowerPerStakingPool[pool - 1][i] - (tokenPower * ((tokenMultipliers[i - startPeriod - 1] * startDay) + (tokenMultipliers[i - startPeriod] * (91 - startDay))));
                        if (i == currentPeriod) {
                            if (endDay < startDay) {
                ...
                if (pool > 0) { 
                    if (currentPeriod >= nextClaim && duration >= 1) {
                        for (j = nextClaim - 1; j < currentPeriod; j++) {               
                            ...
                            if (j == startPeriod) {
                                sum = sum + ((tokenPower * tokenMultipliers[0] * (91 - startDay) * periodPoolsRewards[pool - 1][j]) / _allSharesPowerPerStakingPool[pool - 1][j]);
                            }
                            else {
                                sum = sum + (((tokenPower * ((tokenMultipliers[delta - 1] * startDay) + (tokenMultipliers[delta] * (91 - startDay)))) * periodPoolsRewards[pool - 1][j]) / _allSharesPowerPerStakingPool[pool - 1][j]);
                            }
                        }
                    }            
                    if (currentPeriod >= nextBonus && duration >= 1) {
                        for (j = nextBonus - 1; j < currentPeriod; j++) {               
                            delta = (allPeriods[j + 1] - startTime) / quarter;                    
                            if (j == startPeriod) {
                                sum = sum + ((tokenPower * tokenMultipliers[0] * (91 - startDay) * periodPoolsBonuses[pool - 1][j]) / _allSharesPowerPerStakingPool[pool - 1][j]);
                            }
                            else {
                                sum = sum + (((tokenPower * ((tokenMultipliers[delta - 1] * startDay) + (tokenMultipliers[delta] * (91 - startDay)))) * periodPoolsBonuses[pool - 1][j]) / _allSharesPowerPerStakingPool[pool - 1][j]);
                            }
                        }
                    } 
                }
    ...
    function _calcEstimatedRewards(address staker) private view returns (uint256) {
        uint256 currentPeriod = (block.timestamp - allPeriods[0]) / quarter;
        uint256 currentDay = ((block.timestamp - allPeriods[currentPeriod]) / (1 days)) + 1;
        uint256 sum = _calcAccruedRewards(staker);
        uint256 i;        
        for (i = 0; i < stakedNfts[staker].length; i++) {
            uint256 tokenId = stakedNfts[staker][i];
            uint256 startTime = _stakes[tokenId][2];
            ...
            uint256 startDay = _stakes[tokenId][4];
            uint256 tokenPower = _stakes[tokenId][5];
            uint256 delta = (allPeriods[currentPeriod + 1] - startTime) / quarter;
                ...
                if (pool > 0) {
                    if (periodPoolsEstimatedRewards[pool - 1][currentPeriod] > 0) {
                        if (currentPeriod == startPeriod) {
                            sum = sum + ((tokenPower * tokenMultipliers[0] * (currentDay - startDay) * periodPoolsEstimatedRewards[pool - 1][currentPeriod]) / _allSharesPowerPerStakingPool[pool - 1][currentPeriod]);
                        }
                        else {
                            if (currentDay >= startDay) {
                                sum = sum + (((tokenPower * ((tokenMultipliers[delta - 1] * startDay) + (tokenMultipliers[delta] * (currentDay - startDay)))) * periodPoolsEstimatedRewards[pool - 1][currentPeriod]) / _allSharesPowerPerStakingPool[pool - 1][currentPeriod]);
                            }
                            else {
                                sum = sum + ((tokenPower * tokenMultipliers[delta - 1] * currentDay * periodPoolsEstimatedRewards[pool - 1][currentPeriod]) / _allSharesPowerPerStakingPool[pool - 1][currentPeriod]);
                            }
                        }
                    }
                    else {
                        if (currentPeriod > 0) {
                            if (currentDay >= startDay) {
                                sum = sum + (((tokenPower * ((tokenMultipliers[delta - 1] * startDay) + (tokenMultipliers[delta] * (currentDay - startDay)))) * periodPoolsEstimatedRewards[pool - 1][currentPeriod - 1]) / _allSharesPowerPerStakingPool[pool - 1][currentPeriod]);
                            }
                            else {
                                sum = sum + ((tokenPower * tokenMultipliers[delta - 1] * currentDay * periodPoolsEstimatedRewards[pool - 1][currentPeriod - 1]) / _allSharesPowerPerStakingPool[pool - 1][currentPeriod]);
                            }
                        }
                    }
                }
                ...
                for (uint256 p = 8; p < 11; p++) {
                    ...
                    if (pool > 0) {
                        if (periodPoolsEstimatedRewards[pool - 1][currentPeriod] > 0) {
                            ...
                            if (endDay >= startDay) {
                                sum = sum + (((tokenPower * ((tokenMultipliers[delta - 1] * startDay) + (tokenMultipliers[delta] * (endDay - startDay)))) * periodPoolsEstimatedRewards[pool - 1][currentPeriod - 1]) / _allSharesPowerPerStakingPool[pool - 1][currentPeriod]);
                            }
                            else {
                                sum = sum + (tokenPower * tokenMultipliers[delta - 1] * endDay * periodPoolsEstimatedRewards[pool - 1][currentPeriod - 1] / _allSharesPowerPerStakingPool[pool - 1][currentPeriod]);
                            }
                        } 
                    }
                }
