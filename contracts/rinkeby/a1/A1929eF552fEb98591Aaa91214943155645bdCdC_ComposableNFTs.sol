...
contract ComposableNFTs is ERC721Enumerable, Ownable {
  ...
  mapping (uint256 => mapping(Layer => OptionSelection)) nfts;
  ...
  constructor() ERC721("NFTCollectionName", "SYMBOL") {
  }
  
  // starts a presalt for the whitelisted addressed
  ...
  function mintItem(uint256 tokenId) private {
    for(uint i = 0; i<layerContracts.length; i++) {
      uint256 len = layerContracts[i].getOptionsLength();
      uint256 optionNumber = randomNumber(tokenId) % len;

      // Todo function should return two colors. []string
      string memory colors = layerContracts[i].getOptionDefaultColorById(optionNumber);

      nfts[tokenId][layerContracts[i]].optionNumber = optionNumber;
      nfts[tokenId][layerContracts[i]].color1Hex = colors;
      nfts[tokenId][layerContracts[i]].color2Hex = colors;
    }
  }
    ...
    returns (string memory)
  {
    ...
    string memory tokenName_ = string.concat(Strings.toString(tokenId), "NFT Collection name");
    ...
    for (uint i=0; i<layerContracts.length; i++) {
      if(i == 0) {
        attributes = string.concat(
          layerContracts[i].getOptionMetaById(nfts[tokenId][layerContracts[i]].optionNumber)
        );
      } else {
        attributes = string.concat(
          attributes,
    ...
    returns (string memory)
  {
    ...
    for (uint i=0; i<layerContracts.length; i++) {
      render = string(abi.encodePacked(
        render,
        layerContracts[i].renderOptionById(
          nfts[tokenId][layerContracts[i]].optionNumber,
          nfts[tokenId][layerContracts[i]].color1Hex,
          nfts[tokenId][layerContracts[i]].color2Hex
        )
      ));
    }
  ...
  // TODO: Change public
  function randomNumber(uint256 seed)
    public
    view
    returns (uint8)
  {
    ...
    // return bytes2(predictableRandom[0]);
    return uint8(predictableRandom[5]);
    // ( bytes3(predictableRandom[2]) >> 16 
    // return predictableRandom;
  }

  function addLayers (address[] memory layerAddresses) public {
    for(uint i=0; i<layerAddresses.length; i++){
      layerContracts.push(Layer(layerAddresses[i]));
    }
  }

  function setOptionColor(uint tokenId, Layer layerContractAddress, string memory colorHex)
    public
  {
    // Should be nft owner
    // should have option number
    nfts[tokenId][layerContractAddress].color1Hex = colorHex;
  }
