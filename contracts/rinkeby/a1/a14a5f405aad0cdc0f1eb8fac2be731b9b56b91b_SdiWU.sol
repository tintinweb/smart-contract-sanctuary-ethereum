...
contract SdiWU is ERC721Enumerable, Ownable,Pausable {
    using Strings for uint256;
    uint256 ThronePassSale = 1;
    //NONE = 0
    //GOLD = 1
    //SILVER = 2
    ...
    uint256 public maxGenCount = 10000; 
    ...
    mapping(address => uint256) public SilverMinted;
  ...
  constructor() ERC721("SdiWU", "SCWU") {
    setNotRevealedURI("ipfs://notrevealeduri/comming-soon.json");
    setBaseURI("ipfs://revealeduri/");
    defineTeams();
  }
  ...
  function mint(uint256 _noOfTokens) public payable {
    ...
    require(ThronePassSale==0,"Currently Throne Pass sale started.");
  ...
  function Throne_Mint(uint256 _noOfTokens) public payable {
        uint256 supply = totalSupply();
        require(ThronePassSale > 0 ,"Sale has not started yet");
        require(_noOfTokens > 0);

        for (uint256 i = 1; i <= _noOfTokens; i++) {
            _safeMint(msg.sender, supply+i);
            if (ThronePassSale==1){
              GoldMinted[msg.sender]=GoldMinted[msg.sender]+1;
            }else if(ThronePassSale==2){
              SilverMinted[msg.sender]=SilverMinted[msg.sender]+1;
            }
            emit mintLog (supply+i, msg.sender,ThronePassSale);
        }
   
    }

     
  function setThronePassSale(uint8 _type) public onlyOwner {
        require(_type<=2,"Invalid value.");
        ThronePassSale = _type;
   }
   
   function getThronePassSale() public view returns (uint256) {
        return ThronePassSale;
    }
