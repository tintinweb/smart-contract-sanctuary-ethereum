...
contract HitchHikerOfStar is ERC721A, Ownable {
    ...
    uint256 public constant decayFactor = 0.1 ether;
    uint256 public constant BLOCKS_PER_HALF_HOUR = 150;
    ...
    function freeMint(
        ...
        if (
            (asset == cryptoPunksAddress &&
                ICryptoPunks(asset).punkIndexToAddress(tokenId) == to) ||
            IERC721Like(asset).ownerOf(tokenId) == to
        ) {
            freeMinted += 1;
            _safeMint(to, 1);
        }
    ...
    function getDutchMintPrice() public view returns (uint256 mintPrice) {
        uint256 declinedPrice = ((block.number - dutchAuctionStartBlockNumber) /
            BLOCKS_PER_HALF_HOUR) * decayFactor;
