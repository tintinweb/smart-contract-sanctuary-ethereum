...
abstract contract DHVessel is ERC721, ERC721Burnable {
    function tokenToTokenType(uint tokenId) public virtual view returns (uint256);
}

contract DHHaloHeads is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    // IERC721 _dhVessels = IERC721Enumerable(0x5DdB2Ca013EbE1C0f2f9aB8ad3D3d4BccdB66b35);
    DHVessel _dhVessels = DHVessel(0xc46b2AFc20afbB25705dA74Cc972B8904709842E); //rinkeby

    mapping(uint256 => uint256) public tokenToTokenType;

    event Evolve(uint256 dhVesselsTokenId, uint256 newTokenId, address owner);

    constructor() ERC721("DeadHeads HaloHeads", "HaloHead") {}
    ...
    function evolve(uint256 dhVesselTokenId) public {
        require(
            _dhVessels.ownerOf(dhVesselTokenId) == msg.sender,
            "sender not own vessel token"
        );
    ...
    function batchEvolve(uint256[] calldata dhVesselTokenIds) external {
        for (uint i = 0; i < dhVesselTokenIds.length; i++) {
            evolve(dhVesselTokenIds[i]);
        }
    }
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    tokenId.toString(),
                    "?t=",
                    tokenToTokenType[tokenId].toString()
                )
            );
    }
