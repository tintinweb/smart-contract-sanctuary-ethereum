    ...
    Withdrawable
{
    ...
    uint256 public WELCOME_MINT_MAX_QTY = 458;
    uint256 public SALES_MAX_QTY = TOTAL_MAX_QTY - GIFT_MAX_QTY - WELCOME_MINT_MAX_QTY;
    uint256 public constant MAX_QTY_PER_MINTER = 7;
    uint256 public constant MAX_WELCOME_QTY_PER_MINTER = 1;

    uint256 public PUBLIC_SALES_PRICE = 0;
    uint256 public WELCOME_SALES_PRICE = 0;

    mapping(address => uint256) public publicSalesMinterToTokenQty;
    mapping(address => uint256) public welcomeSalesMinterToTokenQty;

    uint256 public publicSalesMintedQty = 0;
    uint256 public welcomeSalesMintedQty = 0;
    uint256 public giftedQty = 0;

    bool public isSalesPaused = true; 
    bool public isWelcomeMintPaused = true; 

    constructor() ERC721r("Bad", "BU" , TOTAL_MAX_QTY)   {}

    function setWelcomeMintMaxQuantity(uint256 _mintQty) external onlyOwner {
        WELCOME_MINT_MAX_QTY = _mintQty;
    }

    function setPublicSalesPrice(uint256 pricenInWei) external onlyOwner {
        PUBLIC_SALES_PRICE = pricenInWei * 1000000000000000000;
    }

     function setWelcomeSalesPrice(uint256 pricenInWei) external onlyOwner {
        WELCOME_SALES_PRICE = pricenInWei * 1000000000000000000;
    }
    ...
    function setWelcomeMintState(bool _state) external onlyOwner {
        isWelcomeMintPaused = !_state;
    }
         ...
         
    {
        require(
            !isWelcomeMintPaused,
            "Welcome mint is paused"
        );
         

         require(
            welcomeSalesMintedQty + 1 <= WELCOME_MINT_MAX_QTY,
            "Exceed Welcome mint max limit"
        );
        require(
            welcomeSalesMinterToTokenQty[msg.sender] + 1 <=
                MAX_WELCOME_QTY_PER_MINTER,
            "Exceed max mint per minter"
        );
        
        require(tx.origin == msg.sender, "Contracts not allowed");
        require(msg.value >= WELCOME_SALES_PRICE, "Insufficient funds");

        welcomeSalesMinterToTokenQty[msg.sender] += 1;
        welcomeSalesMintedQty += 1;

        
        _mintRandom(msg.sender, 1);
    }

 function initialMint(uint256 _mintQty)
        external
        onlyOwner         
    {
         
        require(
            welcomeSalesMintedQty + _mintQty <= WELCOME_MINT_MAX_QTY,
            "Exceed Welcome mint max limit"
        );
        
        welcomeSalesMinterToTokenQty[msg.sender] += _mintQty;
        welcomeSalesMintedQty += _mintQty;
