...
import './TresFaceBase.sol';
import './TresFaceSplits.sol';

/**
 * @title TresFace
 */
contract TresFace is TresFaceSplits, TresFaceBase {
    constructor()
        TresFaceBase(
            'TresFace',
            'TRF',
            'https://nftculture.mypinata.cloud/ipfs/', // IMPORTANT: IPFS Gateway URI only.
            addresses,
            splits,
            0.069 ether,
            0.069 ether,
    ...
    {
        // Implementation version: v1.0.0-2

        // For rinkeby only, debug roots.
        _setMerkleRoots(
            0xe6eed1f8ae4d83e0ea46ab00dc0e52296f905cd0a78a42910c74f3f84ef28cfc,
        ...
        // TODO: Assign these to real values.
        string[] memory natureFragments = new string[](4);
        natureFragments[0] = 'QmSrhqSRe6Wgsxu35yjLMQMXnCR8A3Km2Ed9zwFxQbXyFf/nature1';
        natureFragments[1] = 'QmSrhqSRe6Wgsxu35yjLMQMXnCR8A3Km2Ed9zwFxQbXyFf/nature2';
        natureFragments[2] = 'QmSrhqSRe6Wgsxu35yjLMQMXnCR8A3Km2Ed9zwFxQbXyFf/nature3';
        natureFragments[3] = 'QmSrhqSRe6Wgsxu35yjLMQMXnCR8A3Km2Ed9zwFxQbXyFf/nature4';
 ...
 */
abstract contract TresFaceBase is
    ERC721ABurnable,
    Ownable,
    ...
    MerkleLeaves
{
    ...
    uint256 private constant MAX_PRESALE_BATCH_SIZE = 10;
    uint256 private constant PRESALE_PURCHASE_LIMIT = 10;
    uint256 private constant MAX_PUBLIC_BATCH_SIZE = 10;
    ...
    // Uri fragments. No slashes needed.
    mapping(uint256 => string) private _natureUriFragments;
    mapping(uint256 => string) private _userUriFragments;
    ...
    function presalePricePerNft() external view returns (uint256) {
    ...
    function setNatureFragments(uint256[] memory __natureIds, string[] memory __natureUri) external onlyOwner {
        _setNatureFragments(__natureIds, __natureUri);
    }

    function restoreUserFragment(uint256 tokenId, string memory userUri) external onlyOwner {
        // This is here in case of emergency.
        _setBindToUser(tokenId, userUri);
    }

    function restoreToBlank(uint256 tokenId, uint256 natureId) external onlyOwner {
        // This is here in case of emergency.
        _initializeOwnershipAt(tokenId);
        _setNature(tokenId, natureId);
    }

    function setMerkleRoots(bytes32 __claimRoot, bytes32 __presaleRoot) external onlyOwner {
        _setMerkleRoots(__claimRoot, __presaleRoot);
    }

    function _setMerkleRoots(bytes32 __claimRoot, bytes32 __presaleRoot) internal {
        if (__claimRoot != 0) {
    ...
    function getNextClaimIndex(address wallet) external view returns (uint256) {
        // This assumes the claim phase is the first phase.
    ...
    function getUserUriFragment(uint256 tokenId) external view returns (string memory) {
     ...
     */
    function mintSingleToken(uint256 selectedNature) external payable nonReentrant onlyUsers isPublicMinting {
        if (msg.value != publicMintPricePerNft) revert InvalidPublicMintPayment();

        _internalMintTokens(msg.sender, _totalMinted(), 1, selectedNature);
    }

    function bindToUser(uint256 tokenId, string memory userUri) external {
        require(_exists(tokenId), 'No token');
        ...
        // TODO: Need to implement ECDSA Scheme.
        // require(1 == 1, 'Method not implemented');
    }
    ...
    function _internalMintTokens(
        ...
        _setNature(nextToken, selectedNature);
        if (count > 1) {
            // This might end up being too expensive, but it does work, and since everything is best case
            // runtime, this might still be better than using a map.
            for (uint256 nextTokenIdx = nextToken + 1; nextTokenIdx < nextToken + count; nextTokenIdx++) {
                _initializeOwnershipAt(nextTokenIdx);
                _setNature(nextTokenIdx, selectedNature);
            }
        }
    }
    ...
    function _presaleTokens(
        ...
        if (!_presaleRoot._checkLeaf(proof, _generateLeaf(minter))) revert ProofInvalidClaim();

        _internalMintTokens(minter, _totalMinted(), count, selectedNature);
    }

    function _setNatureFragments(uint256[] memory __natureIds, string[] memory __natureUri) internal {
        require(__natureIds.length == __natureUri.length, 'Unmatched arrays');

        for (uint256 idx = 0; idx < __natureIds.length; idx++) {
            _natureUriFragments[__natureIds[idx]] = __natureUri[idx];
        }
    }

    function _setBindToUser(uint256 tokenId, string memory userUri) internal {
        _userUriFragments[tokenId] = userUri;
    }

    function _setNature(uint256 tokenId, uint256 selectedNature) internal {
        if (selectedNature < NATURE_BASE_VAL + NATURE_MIN || selectedNature > NATURE_BASE_VAL + NATURE_MAX)
            revert InvalidSelectedNature();

        _setExtraDataAt(tokenId, uint24(selectedNature));
    }
    ...
    function _isBoundToUser(uint256 tokenId) internal view returns (bool) {
        if (bytes(_userUriFragments[tokenId]).length == 0) {
            return false;
        }
...
contract TresFaceSplits {
    address[] internal addresses = [
        0x000000000000000000000000000000000000dEaD // TODO: Set TresFace Wallet(s)
    ];

    uint256[] internal splits = [100]; // TODO: Set TresFace Split(s)
}
