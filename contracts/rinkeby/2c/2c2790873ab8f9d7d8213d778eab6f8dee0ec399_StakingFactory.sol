...
interface cmnReferral {
    function payReferral(address, uint, uint) external returns(bool);
...
contract Stake {
    ...
    enum UserActions{DEPOSIT, REFER, HARVEST}
    ...
    uint256 public minAmount = 10000000000;
    ...
    function updateAccount(address account, bool claim) private {
        ...
        if(claim){
            ...
            if(pendingDivs > 0){
                pendingReward[account] = pendingReward[account].add(pendingDivs);
            }
        }
    ...
    function deposit(uint256 amountToStake) public {
        ...
        require(!isBlackListForDeposit[msg.sender], "User is blacklisted");
        require(
        ...
        if (
            ...
            uint256 referralAmount = amountToStake.mul(referralFeeRate).div(
                1e4
            );
            ...
            require(
                Token(depositToken).transfer(myReferralAddredd, referralAmount),
    ...
    function depositWithReferral(uint256 amountToStake, bytes3 referralCode)
        public
    {
        ...
        if (
            ...
            require(!isBlackListForRefer[msg.sender], "User is blacklisted");
            alreadyReferral[msg.sender] = true;
            myReferralAddresses[msg.sender] = referral;

            cmnReferral(referralAddress).setUserReferral(msg.sender, referral);

            cmnReferral(referralAddress).setReferralAddressesOfUsers(msg.sender, referral);

            cmnReferral(referralAddress).payReferral(msg.sender, 0, amountToStake);
            ...
            // );
            // require(
            //     Token(depositToken).transfer(referral, referralAmount),
            //     "Could not transfer referral amount"
            // );
        }
    ...
    function depositUSDT(uint256 amountToStake) public {
        require(now <= poolExpiryTime, "Pool is expired");
        require(now <= poolOpenTill, "Pool is closed");
        require(amountToStake > 0, "Cannot deposit 0 Tokens");
        require(!isBlackListForDeposit[msg.sender], "User is blacklisted");
        ...
        depositedTokens[msg.sender] = depositedTokens[msg.sender].add(
            amountToStake.div(cmnRate).div(100)
        );

        totalStaked = totalStaked.add(amountToStake.div(cmnRate).div(100));
        cmnStakePrice[msg.sender] = cmnRate;
    ...
    function depositUSDTWithReferral(uint256 amountToStake, bytes3 referralCode) public {
        // require(depositedTokens[referral] > 0, "Referral address does not have any deposit");
        require(now <= poolExpiryTime, "Pool is expired");
        require(now <= poolOpenTill, "Pool is closed");
        require(amountToStake > 0, "Cannot deposit 0 Tokens");
        ...
        if (
            ...
            cmnReferral(referralAddress).payReferral(msg.sender, 0, amountToStake.div(cmnRate).div(100));
