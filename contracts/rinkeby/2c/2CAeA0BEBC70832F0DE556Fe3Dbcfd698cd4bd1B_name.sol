...
contract name is GMERC721A {
    ...
    address[]  _receivers = [0xCcEbE44D6cba1F790F36be099Eb4bA5899650005];
    uint256[] _basisPoints = [1];
...
contract GMERC721A is ERC721A, Ownable {
    ...
    function presale(uint64 qty, bytes calldata signature) external payable {
        ...
        if (_currentIndex + (qty - 1) > preSaleInfo.presaleSupply) revert SoldOut();
        if (msg.value != preSaleInfo.presalePrice * qty) revert InvalidPrice();
        uint64 preSaleMint = _getAux(msg.sender) + qty;
