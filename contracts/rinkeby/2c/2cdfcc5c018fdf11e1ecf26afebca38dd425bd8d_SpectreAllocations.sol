    ...
    SignatureVerifier_V2
{
    ...
    IERC721Upgradeable Spectre;
    IERC20 USDC;

    function initialize(address _spectre) external initializer {
        __Ownable_init();
        __Pausable_init();
        __ReentrancyGuard_init();
        Spectre = IERC721Upgradeable(_spectre);
    }

    struct projectConfig {
        uint256 maxAllocations;
        uint256 maxAllocationsPerToken;
    ...
    mapping(bytes32 => mapping(uint256 => uint256)) public investmentPerToken;
    ...
    */
    function addInvestmentToProject(
        ...
        require(
            Spectre.ownerOf(tokenId) == msg.sender,
        ...
        */
        ...
        /*  check that the amount to invest is still within the limits 
            allowed to this specific token by the project.
        */
        uint256 tokenInvestment = investmentPerToken[projectName][tokenId];
        require(
            tokenInvestment + amount <= project.maxAllocationsPerToken,
            "you reached max allocations"
        );
        USDC.approve(address(this), amount);
        USDC.transferFrom(msg.sender, address(this), amount);
        //require(msg.value >= amount, "amount sent not enough");
        // add the amount to the user's portfolio
        investmentPerUser[projectName][msg.sender] += amount;
        // add the amount to the token's balance
        investmentPerToken[projectName][tokenId] += amount;
        // add the amount to the total collected balance by the project.
        projectsLedger[projectName].totalCollected += amount;

        delete project;
        delete tokenInvestment;
    }

    function addProject(
        bytes32 projectName,
        uint256 _maxAllocations,
        uint256 _maxAllocationsPerToken,
        uint256 _endDate,
        address _signer
    ) external onlyOwner {
        projectsLedger[projectName] = projectConfig({
            maxAllocations: _maxAllocations,
            maxAllocationsPerToken: _maxAllocationsPerToken,
            totalCollected: 0,
            endDate: _endDate,
            signer: _signer,
            paused: false
    ...
    function editProjectMaxAllocationsPerToken(
        bytes32 projectName,
        uint256 _maxAllocationsPerToken
    ) external onlyOwner {
        require(projectExist[projectName], "No such project");
