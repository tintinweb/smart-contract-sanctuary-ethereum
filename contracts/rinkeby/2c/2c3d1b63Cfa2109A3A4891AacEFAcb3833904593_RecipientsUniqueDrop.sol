...
import "../ipfs/ERC721IPFS.sol";

/// @title Drop (Mint) unique content to recipients
/// @author Amit Molek
/// @notice This contract provides the ability to drop (mint) content and
/// send it to given recipients
contract RecipientsUniqueDrop is ERC721IPFS, Ownable, ReentrancyGuard {
    ...
    /// @notice Drop had occured
    /// @dev tokenIdBegin and tokenIdEnd indicates a range for minted ids
    /// with range of [tokenIdBegin, tokenIdEnd)
    /// tokenIdBegin (inclusive)
    /// tokenIdEnd (exclusive)
    event Dropped(
        uint256 dropId,
        uint256 tokenIdBegin,
        uint256 tokenIdEnd,
    ...
    ) ERC721IPFS(name_, symbol_) Ownable() {
    ...
    function _mintDrop(address[] memory recipients, uint256[] memory uris)
        private
    {
        uint256 tokenIdBegin = _tokenIdCounter.current();
        uint256 tokenId;
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            // Store token's uri
            setIpfsUri(tokenId, uris[i]);
        }

        // Get the ids range end
        tokenId = _tokenIdCounter.current();

        // Store drop
        uint256 dropId = _dropIdCounter.current();
        _drops[dropId].from = msg.sender;
        _drops[dropId].tokenIdBegin = tokenIdBegin;
        _drops[dropId].tokenIdEnd = tokenId;
        _dropIdCounter.increment();

        // Signal that a drop has occured
        emit Dropped(dropId, tokenIdBegin, tokenId, msg.sender, recipients);
    }
    ...
    /// @param uris Hex URI array of the IPFS content, such as:
    /// 0x0155171c7a5638f61fc6cb5fe79746800653ba213355b9569ab13daf64efb25e, ...
    function drop(address[] memory recipients, uint256[] memory uris)
        external
        virtual
        nonReentrant
    {
        ...
        if (recipients.length != uris.length) {
            revert InvalidArgument("recipients, uris length mismatch", "");
        }

        // Mint dorp to recipients
        _mintDrop(recipients, uris);
    }

    /// @notice Returns how many Drop created so far
    /// @return count The count of how many drops created so far
    ...
    function fetch(uint256 dropId) external view virtual returns (Drop memory) {
...
import "./IPFSEconomicalUri.sol";

/// @title IPFS support for ERC721
/// @author Amit Molek
contract ERC721IPFS is ERC721, IPFSEconomicalUri {
...
///     0x0155171c7a5638f61fc6cb5fe79746800653ba213355b9569ab13daf64efb25e
///
abstract contract IPFSEconomicalUri is IIPFSUri {
    ...
    function setIpfsUri(uint256 tokenid, uint256 uri) public override {
        _uris[tokenid] = uri;
    }
