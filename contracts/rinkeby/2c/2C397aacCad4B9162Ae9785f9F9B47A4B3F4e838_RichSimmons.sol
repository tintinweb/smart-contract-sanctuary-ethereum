...
contract RichSimmons is ERC721A, Ownable, ReentrancyGuard {
    ...
    address public BGAddress = payable(0x00000000000);
    address public RSAddress = payable(0x00000000000);
    
    constructor(string memory _initBaseURI, string memory _initNotRevealedURI, bytes32 _root) ERC721A("richsimmons", "RS") {
    ...
    modifier doMint(uint8 quantity) {
        _mint(msg.sender, quantity);     
        mintCount += quantity; 
        _;
    }
    
    function devMint(uint8 quantity) external onlyOwner doMint(quantity) {
        require(totalSupply() + quantity < maxSupply, "[Supply Error] Not enough left for this mint amount");
    }
    
    function privateMint(bytes32[] memory proof, uint8 quantity) external payable nonReentrant whenPrivateMint doMint(quantity) {
    ...
    function mint(uint8 quantity) external payable nonReentrant whenPublicMint doMint(quantity) {
        require(msg.value >= cost * quantity, "[Value Error] Not enough funds supplied for mint");
        require(totalSupply() + quantity < maxSupply, "[Supply Error] Not enough left for this mint amount"); 
