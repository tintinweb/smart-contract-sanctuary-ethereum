...
contract RoRo is ERC721A, Ownable {
	...
	uint public constant RORO_LIMIT = 15;
	...
	string public prerevealURL = 'ipfs://QmWbv7bNyEvcrpX99yzpVHjWcBibCe97gTYJB5Xnmocgmh/hidden.json';
	...
	function allowFreeMint() external view returns (bool) {
		return  FREE_LIMIT - freeMinted - 1 >= 0;
	}
	...
	function mint(uint count) external payable {
        ...
        if(_roroWhisperCount[msg.sender] == 0 && FREE_LIMIT - freeMinted - 1 >= 0) {
			payForCount--;
			freeMinted ++;
        }
	...
	// function freeMint(uint count) external {
	// 	require(!isPaused, 'RoRo: Sales are off');
	// 	require(_totalMinted() + count <= MAX_SUPPLY,'RoRo: Over limit');

	// 	require(_roroWhisperCount[msg.sender] + count <= 2,'RoRo: Wallet Over limit');
		
	// 	require(freeMinted + count <= FREE_LIMIT,'RoRo: Free Mint Over limit');

	// 	_roroWhisperCount[msg.sender]  = _roroWhisperCount[msg.sender] + count;
	// 	freeMinted += count;
	// 	_safeMint(msg.sender, count);
	// }
}