...
contract CommonERC721 is ERC721, Ownable, IERC2981 {
  ...
  constructor(string memory name, string memory symbol, uint16 maxSupply, uint256 _mintRate, string memory _contractURI) ERC721(name, symbol) {
...
contract CryptoHomiesGenesis is CommonERC721, EIP712 {
  ...
  mapping (uint16 => bool) remdeemedVouchers;
  

  CryptoHomiesCommon public commonContract;

  constructor(string memory _contractURI, string memory _commonContractURI, address _validSigner) CommonERC721("CryptoHomiesGenesis", "CHG", 4848, 0.1 ether, _contractURI) EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) {
  ...
  function redeem(Voucher calldata _voucher) public payable {
    ...
    require(_signer != address(0), "Unable to recover signer from signature");
    require(_signer == validSigner, "Invalid Voucher");
    require(_redeemer == _voucher.recipient, "This Voucher doesn't belong to you");
    require(!remdeemedVouchers[_voucher.id], "This Voucher was already redeemed");
    _mint();
    remdeemedVouchers[_voucher.id] = true;
  }
  ...
  function _mint() internal {
    ...
    require(!mintedUsers[currentStage][msg.sender], "User has minted already");
  ...
  function _hash(Voucher calldata pass) internal view returns (bytes32) {
    return  _hashTypedDataV4(keccak256(abi.encode(keccak256("Voucher(uint16 id,address recipient)"), pass.id, pass.recipient)));
  }
...
contract CryptoHomiesCommon is CommonERC721 {
  ...
  constructor(address owner, string memory _contractURI) CommonERC721("CryptoHomiesCommon", "CHC", 20544, 0.1 ether, _contractURI) {
