...
contract MigrationHelper {
	...
	address owner;
	address oldTokenAddress; //memeone
	address public newTokenAddress;

	// uint256 decimalsOldToken = 18;
	// uint256 decimalsNewToken = 9;

	uint256 public divider = 1;
	...
	function setOldTokenAddress(address oldTokenAddress_) external  {
		require(oldTokenAddress_ != address(0));
		oldTokenAddress = oldTokenAddress_;
	}

	function setDivider(uint256 divider_) external  {
		require(divider_ != 0);
		divider = divider_;
	}

	function setNewTokenAddress(address newTokenAddress_) external  {
		require(newTokenAddress_ != address(0));
		newTokenAddress = newTokenAddress_;
	}

	function claim(uint256 balance) public {
		require(IERC20Metadata(oldTokenAddress).balanceOf(msg.sender) > 0, "You don't have old token");
		require(IERC20Metadata(oldTokenAddress).balanceOf(msg.sender) >= balance, "You don't have old token");
	...
	function setNewAmountToReceive(uint256 _balance) private {
		...
		newAmountToReceive = _balance.div(10**decimalsOldToken).mul(10**decimalsNewToken).div(divider);
	}
