...
contract BookListMarketplaceNFT721 is Ownable, ReentrancyGuard {
    ...
    event UpdateNftContractAddress(address nftContractAddress);
    ...
    function cancelListing(uint256 _tokenId) external {
        require(nftContract.isApprovedForAll(_msgSender(), address(this)), "Token transfer not approved");
    ...
    function buyItem(uint256 _tokenId, address _seller, uint256 _pricePerItem, bytes memory _signature) external payable nonReentrant {
        bytes32 messageHash = keccak256(abi.encodePacked(_tokenId, _seller, _pricePerItem));
        require(ECDSA.recover(ECDSA.toEthSignedMessageHash(messageHash), _signature) == getSignerAddress(), "Invalid signature");

        require(IERC721(nftContract).isApprovedForAll(_seller, address(this)), "Item not approved");
        ...
        IERC721(nftContract).safeTransferFrom(_seller, _msgSender(), _tokenId);
        
        // Broadcast the sale
        emit ItemSold(_tokenId, _seller, _msgSender(), msg.value);
    }
    ...
    /// @notice Method for updating nftContractAddress address
    /// @dev Only admin
    /// @param _nftContractAddress payable address the address to sends the funds to
