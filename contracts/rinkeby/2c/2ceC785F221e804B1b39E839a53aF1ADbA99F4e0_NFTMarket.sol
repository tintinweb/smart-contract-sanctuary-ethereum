...
contract NFTMarket is Ownable, PaymentSplitter, ReentrancyGuard {
...
// Storage for treasury wallets.
 ...
 struct MarketItem {
  ...
  constructor(address[] memory treasuryWallets, uint256[] memory treasuryShares)
      PaymentSplitter(treasuryWallets, treasuryShares) {
        _treasuryWallets = treasuryWallets;
      }    

 // Updates the fee percentage of the contract 
    function updateFeePercentage(uint _feePercentage) public onlyOwner {
  ...
  function sellItemAndTransferOwnership(
    ...
    idToMarketItem[itemId].seller.transfer(msg.value - (msg.value / (100/feePercentage)));
  ...
  function removeItemFromMarket(
    ...
    address nftContract = idToMarketItem[itemId].nftContract;
    uint tokenId = idToMarketItem[itemId].tokenId;
    require(msg.sender == idToMarketItem[itemId].seller, "Only the owner of this NFT can remove it.");
    idToMarketItem[itemId].forSale = false;
