...
contract StakeborgNFTicket is ERC721URIStorage, Ownable {
    ...
    uint256 public mintPrice = 100000000000000; // 0.0001 Ether, because of the greedy testnet faucets

    mapping(address => uint256[]) public holderTokenIDsMap;
    mapping(address => bool) public checkInsMap;

    constructor() ERC721("StakeBorg NFTicket", "NFTICKET") {
        currentId.increment();
        console.log(currentId.current());
    }

    function checkIn(address checkInAddress) public {
        checkInsMap[checkInAddress] = true;
        uint256 tokenId = holderTokenIDsMap[checkInAddress][0];

        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{ "name": "StakeBorg Talks CHECK-IN #',
                        Strings.toString(tokenId),
                        '", "description": "The StakeBorg Talks NFTicket", ',
    ...
    function mint() public payable {
        ...
        holderTokenIDsMap[msg.sender].push(currentId.current());
        currentId.increment();
    ...
    function confirmOwnership(address checkInAddress)
        public
        view
        returns (bool)
    {
        return holderTokenIDsMap[checkInAddress].length > 0;
    }
}
