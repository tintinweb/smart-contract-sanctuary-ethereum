...
contract INUBISNFT is ERC721Enumerable, Ownable{
    using SafeMath for uint256;
    ...
    uint256 public whitelistPriceEth = 0.015 ether;
    uint256 public mintPriceEth = 0.075 ether;

    IERC20 public InubisToken;
    ...
    constructor(IERC20 _InubisToken) ERC721("InubisNFT", "InubisNFTSYM") {
        _baseTokenURI = "";
        InubisToken = _InubisToken;
    }
    ...
    function setAllowList(address[] calldata addresses, uint8 numAllowedToMint) external onlyOwner {
     ...
     function ChangeMintStatus(bool newValue) public onlyOwner {
        MintEnabled = newValue;
    }
     function setPublicSaleStatus(bool newState) public onlyOwner {
        PublicSaleOpen = newState;
    }

    //only owner
    function setWhiteListPrice(uint256 _newCost) public onlyOwner() {
        whitelistPriceEth=_newCost;
    }
     function setPublicPrice(uint256 _newpublicprice) public onlyOwner() {
        mintPriceEth=_newpublicprice;
    }
    ...
    function mintAllowList(uint8 numberOfTokens) public payable {
        require(MintEnabled, "Minting  has not Started");
        require(!PublicSaleOpen, "Sale is for Public");
        // require(numberOfTokens <= _allowList[msg.sender], "Exceeded max available to purchase");

        // _allowList[msg.sender] -= numberOfTokens;
        require(numberOfTokens>0, "NFT: Invalid amount to mint.");
        require(msg.value==numberOfTokens*whitelistPriceEth , "NFT: Invalid value.");
    ...
    function mint_avatars_for_eth(uint256 _amount) public payable returns (uint256[] memory minted)  {
        require(MintEnabled, "Minting  has not Started");
        require(PublicSaleOpen, "Sale must be active to mint tokens");
    ...
    function getTokensForEth(uint _amountETH) public view returns(uint256){
        ...
        IUniswapV2Pair pair = IUniswapV2Pair(0x74468ef01f5E7dFe08F8cdc01a744a5469b049FE);//Test UNISWAP
    ...
    function mint_avatars_for_inubis(uint256 _amount) public payable returns (uint256[] memory minted)  {
        require(MintEnabled, "Minting  has not Started");
        require(PublicSaleOpen, "Sale must be active to mint tokens");
        require(_tokenIds.current() < phaseEndsAt, "Minting Phase Ended");
        ...
        require(InubisToken.balanceOf(msg.sender)>=numTokensNeeded, "Insufficient token balance.");
        InubisToken.safeTransferFrom(msg.sender, address(this) , numTokensNeeded*10e9);
    ...
    function withdraw_all() public onlyClevel {
        ...
        uint256 tokenBalance = InubisToken.balanceOf(address(this));
        if (tokenBalance>0){
            InubisToken.safeTransfer(owner(), tokenBalance);
        }
