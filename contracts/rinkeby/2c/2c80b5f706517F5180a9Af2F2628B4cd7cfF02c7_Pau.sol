...
import "./ERC721A-modified.sol";

contract Pau is Ownable, ERC721A, ReentrancyGuard {

    string public baseURI;

    uint256 public teamPau = 50;

    uint256 public constant price = 0.02 ether;

    uint8 public maxPublicMint = 3;
    uint8 public maxMintPerAccount = 3;

    uint256 public maxPauSupply = 100;
    ...
    mapping (address => bool) public mintedDPforPublic;
    mapping(address => uint256) addressBlockBought;

    constructor(
    )
    ERC721A("Pau", "PAU", 50, 100)  {
    }
    ...
    function mintPublic(
        ...
        require(totalSupply() + maxPublicMint <= maxPauSupply, "EXCEEDS_MAX_SUPPLY" );
        require(!mintedDPforPublic[msg.sender], "EXCEEDS_MAX_PUBLIC_MINT" );

        addressBlockBought[msg.sender] = block.timestamp;
        mintedDPforPublic[msg.sender] = true;
        _safeMint(msg.sender, numberOfTokens);
    }

    /**
     * Mint Pau for the Team
     */
    function mintPauForTeam(uint256 numberOfTokens) external onlyOwner {
        require(teamPau > 0, "NFTS_FOR_THE_TEAM_HAS_BEEN_MINTED");
        require(numberOfTokens <= teamPau, "EXCEEDS_MAX_MINT_FOR_TEAM");

        teamPau -= numberOfTokens;
        _safeMint(msg.sender, numberOfTokens);
    }

    /**
     * Returns Pau of the Caller
     */
    ...
    function setMaxPublicMint(uint8 _maxPublicMint) external onlyOwner {
...
IERC721Enumerable
{
   ...
   */
    ...
    returns (string memory)
    {
        ...
        string memory hiddenURI = "ipfs://QmZ3qZCtRVaAkfmML956AhQMXjy8kisUfnfqxdYkM1xEsi/hidden.json";
        return
