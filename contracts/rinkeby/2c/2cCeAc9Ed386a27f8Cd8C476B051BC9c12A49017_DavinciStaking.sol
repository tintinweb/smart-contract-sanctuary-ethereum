...
contract DavinciStaking is Ownable, ERC721Holder, ERC1155Receiver, ERC1155Holder {
    ...
    function stake(address nftAddress, uint256 tokenId, uint256 supply, AssetType nftType, poolType level, Sign memory sign) external returns(bool){
        require(!usedNonce[sign.nonce], "Invalid Nonce");
        usedNonce[sign.nonce] = true;
        verifySign(uint256(level), msg.sender, sign);
        uint256 endTime = block.timestamp + pools[level].stakeDuration;
        users[lastStakeId] = UserDetail(level, msg.sender, nftType, nftAddress, tokenId, supply, block.timestamp, endTime, true);
    ...
    function unStake(uint256 stakeId, Sign memory sign) external returns(bool) {
        ...
        uint256 rewardAmount = users[stakeId].nftType == AssetType.ERC1155
                            ? users[stakeId].supply * pools[users[stakeId].level].rewardToken
                            : pools[users[stakeId].level].rewardToken;
    ...
    function setPoolDetails(uint256 level, uint256 duration, uint256 amount) external onlyOwner returns(bool) {
        require(level <= 3, "Invaid Pool");
        pools[poolType(level)] = PoolDetails(duration, amount * 10 ** decimals); 
        emit PoolUpdated(level, duration, amount);
        return true;
    }
    ...
    function verifySign(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(this, level, caller, sign.nonce)
        );
        require(
            signer ==
                ecrecover(
