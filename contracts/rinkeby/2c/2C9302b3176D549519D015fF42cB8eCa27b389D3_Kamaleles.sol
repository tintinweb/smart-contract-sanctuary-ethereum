...
import "./YieldKamaleles.sol";

contract Kamaleles is ERC721A, Ownable {
    ...
    bool public allowListIsActive;

    mapping(address => uint256) public addressMintedBalance;
    mapping(address => uint256) public balanceOG;

    YieldKamaleles public yieldToken;

    constructor(string memory _initBaseURI) ERC721A("Kamaleles", "KML") {
        ...
        mintCompliance(_mintAmount)
    {
        ...
        require(_mintAmount <= maxPublicMints, "max amount");
    ...
    function allowlistMint(bytes32[] calldata _merkleProof, uint256 _mintAmount)
        external
        payable
        mintCompliance(_mintAmount)
    {
    ...
    function giveAway(uint256 _mintAmount, address _receiver) public onlyOwner {
        require(_totalMinted() + _mintAmount <= MAX_SUPPLY, "max supply");
    ...
    function setYieldToken(address _yield) external onlyOwner {
        yieldToken = YieldKamaleles(_yield);
    }
...
interface IKamaleles {
    function balanceOG(address _user) external view returns (uint256);
}

contract YieldKamaleles is ERC20("Trippis", "TRIPS") {
    using SafeMath for uint256;

    uint256 public constant BASE_RATE = 10 ether;
    uint256 public constant INITIAL_ISSUANCE = 300 ether;
    ...
    IKamaleles public kamalelesContract;

    event RewardPaid(address indexed user, uint256 reward);

    constructor(address _kamaleles) {
        kamalelesContract = IKamaleles(_kamaleles);
    }
    ...
    function updateRewardOnMint(address _user, uint256 _amount) external {
        require(msg.sender == address(kamalelesContract), "Can't call this");
    ...
    function updateReward(
        ...
        require(msg.sender == address(kamalelesContract));
        if (_tokenId < 1001) {
            uint256 time = min(block.timestamp, END);
            uint256 timerFrom = lastUpdate[_from];
            if (timerFrom > 0)
                rewards[_from] += kamalelesContract
                    .balanceOG(_from)
    ...
    function getTotalClaimable(address _user) external view returns (uint256) {
        uint256 time = min(block.timestamp, END);
        uint256 pending = kamalelesContract
            .balanceOG(_user)
