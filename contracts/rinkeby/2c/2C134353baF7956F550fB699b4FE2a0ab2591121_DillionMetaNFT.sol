...
import "@klaytn/contracts/KIP/token/KIP17/extensions/KIP17Royalty.sol";
import "@klaytn/contracts/KIP/token/KIP7/IKIP7.sol";
import "@klaytn/contracts/KIP/token/KIP7/KIP7.sol";
import "@klaytn/contracts/utils/cryptography/MerkleProof.sol";
    ...
    KIP17Pausable
{
    ...
    // Events
    event SettingMintStartBlock(uint256 mintStartBlock); // Emit when setting mint start block successfully
    event SettingSaleAmount(uint256 saleAmount); // Emit when setting sale amount successfully
    event SettingPublicMintStatus(bool isEnabled); // Emit when setting public mint status successfully
    event SettingPublicMintInfo(uint256 mintPrice, uint256 maxAmount); // Emit when setting public mint info successfully
    event SetWhitelistMintStatus(bool isEnabled); // Emit when setting whitelist mint status successfully
    event SetWhitelistMintInfo(uint256 price, uint256 maxAmount); // Emit when setting whitelist mint info successfully
    ...
    event SettingAirdropStatus(bool isEnabled); // Emit when setting airdrop status successfully
    ...
    modifier isAfterBlockStart() {
        require(block.number >= _blockStartMining, "Not yet started");
        _;
    }
    modifier isNotExceedAmount(uint256 amount) {
        uint256 currentTokenId = _tokenIdTracker.current();
        require(currentTokenId + amount <= _saleAmount, "Exceed max amount");
        _;
    }
     ...
     *
     * Set name, symbol and baseURI for token
     * The deployer is set admin, minter, pauser role.
     ...
     *
     * Requirements:
     * - Must be called by minter wallet.
     *
     ...
     *
     * - Must be called by minter wallet.
     * - New status must be different current status
     *
     ...
     *
     * - Must be called by minter wallet.
     * - New information must be different current information
     * (at least one of the two parameters has a value different from the current value of the corresponding parameter,
     * it is considered valid.)
     * - Max amount allow for each mint must be greater than 0
     *
     ...
     *
     * - Must be called by minter wallet.
     * - New merkle root must be different current merkle root
     *
    ...
    function registerWhitelistMint(bytes32 merkleRoot)
        ...
        onlyRole(MINTER_ROLE)
    {
        require(merkleRoot != _merkleRoot, "Same with current merkle root");
        ...
        payable
    {
        ...
        require(block.number >= _blockStartMining, "Early to mint");
        ...
        require(
            _tokenIdTracker.current() + amount <= _saleAmount,
            "Exceed sale amount"
        );
    ...
    function setAntiBotBlock(uint256 numOfBlock)
        ...
        onlyRole(MINTER_ROLE)
    {
        require(numOfBlock > 0, "Anti bot interval mus be greater than 0");
     ...
     * Requirements:
     *
     * - Must be public mint enabled.
     * - Must be caller after allow block start mint
        ...
        isNotExceedAmount(amount)
    {
        ...
        require(
            _lastMintBlockNumber[msg.sender] + _antiBotBlock < block.number,
        ...
        _lastMintBlockNumber[msg.sender] = block.number;
    }
     ...
     * - New sale amount must be greater than tokenIdTracker
     *
    ...
    * @dev Get status of airdrop function
    ...
    function setKIP7Token(address tokenAddress) external onlyRole(MINTER_ROLE) {
        require(
            KIP7(tokenAddress).supportsInterface(type(IKIP7).interfaceId),
            "New token's type must be KIP7"
        );
        ...
        require(tokenAddress.isContract(), "Address must be a smart contract");

        if (_changeNameTokenPayable != address(0)) {
            IKIP7 _kip7TokenCurrentSetted = IKIP7(_changeNameTokenPayable);
            uint256 balanceOfKIP7TokenInNFT = _kip7TokenCurrentSetted.balanceOf(
                address(this)
            );
            if (balanceOfKIP7TokenInNFT > 0) {
                _kip7TokenCurrentSetted.transfer(
                    _adminWallet,
                    balanceOfKIP7TokenInNFT
                );
            }
        }
    ...
    function validateName(string memory name) external view returns (bool) {}
