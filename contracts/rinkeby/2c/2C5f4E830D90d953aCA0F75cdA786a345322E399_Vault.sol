...
contract Vault {
    ...
    function getBalanceMapping(address _address) public view returns(string[] memory, uint256[] memory) {
        string[] memory nameLists = new string[](name[_address].length);
        uint256[] memory balance = new uint256[](name[_address].length);

        for (uint256 i = 0; i < name[_address].length; i++) {
            nameLists[i] = name[_address][i];
            balance[i] = accountBalance[msg.sender][name[_address][i]];
        }

        return (nameLists, balance);
    }

    function getListOfName(address _address) public view returns(string[] memory) {
        string[] memory nameLists = new string[](name[_address].length);
