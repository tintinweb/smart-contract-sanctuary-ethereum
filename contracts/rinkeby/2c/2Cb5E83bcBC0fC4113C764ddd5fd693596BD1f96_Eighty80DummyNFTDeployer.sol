...
import {Dummy} from "../Mocks/dummyNFt.sol";
contract Eighty80DummyNFTDeployer {


    uint public numberOfContractsDeployed;

    function deployNewProxy (string memory tokenURI)
    external {
        bytes memory bytecode = type(Dummy).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(msg.sender,block.timestamp));
        address newDeployedProxy;
        assembly {
            newDeployedProxy := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        Dummy(payable(newDeployedProxy)).initialize(string(abi.encode("0xy",numberOfContractsDeployed)),string(abi.encode("0xy",numberOfContractsDeployed)),tokenURI);
        tokenTracker[tokenURI] = newDeployedProxy;
        numberOfContractsDeployed ++;
    }

    mapping (string => address) public tokenTracker;
}
