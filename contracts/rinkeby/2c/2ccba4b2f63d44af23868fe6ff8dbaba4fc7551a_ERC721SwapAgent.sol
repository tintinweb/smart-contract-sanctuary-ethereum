    ...
    ERC721Holder
{
    ...
    mapping(address => mapping(uint256 => address)) public supposedRecipients;
    ...
    function swap(
        ...
        require(tokenIds.length > 0, "No tokens are provided");
        require(msg.value >= swapFee[dstChainId] * tokenIds.length, "Insufficient fee provided");
        if (msg.value != 0) {
        ...
        if (dstTokenAddr != address(0x0)) {
            ...
            for(uint256 i = 0; i < tokenIds.length; i++){
                require(supposedRecipients[tokenAddr][tokenIds[i]] == recipient, "Incorrect Recipient");
    ...
    function fill(
        ...
        if (bridgedTokenAddr != address(0x0)) {
            ...
            for (uint256 i = 0; i < tokenIds.length; i++){
                ...
                supposedRecipients[bridgedTokenAddr][tokenIds[i]] = sender;
            }
