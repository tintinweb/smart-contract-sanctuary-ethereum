...
abstract contract CCVault {
	function stakingInfo( address tokenOwner ) public virtual view returns ( uint256 lastUpdate, uint256 key, uint256 tier1, uint256 tier2, uint256 tier3, uint256 degen, uint256 partner, uint256 rewardsEarned );
}
...
contract Degen is NFTFree {
	...
	mapping( address => uint256 ) private _privateMints;
	mapping( address => uint256 ) private _partnerMints;

	CCVault private _vault;
	Partner private _tab;
	Partner private _fmc;
	...
	// **************************************
		function mintPartner() public isPartnerSale {
			...
			if ( _partnerMints[ _account_ ] == MINTS_PER_PARTNER ) {
			...
			unchecked {
				_partnerMints[ _account_ ] = MINTS_PER_PARTNER;
			}
		...
		function mintPrivate( uint256 qty_ ) public validateAmount( qty_ ) isPrivateOrPartnerSale {
			address _account_ = _msgSender();
			if ( _privateMints[ _account_ ] == MINTS_PER_KEY ) {
				revert NFT_ALLOCATION_CONSUMED( _account_ );
			}
