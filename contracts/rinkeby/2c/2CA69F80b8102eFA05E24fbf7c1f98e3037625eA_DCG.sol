...
contract DCG is Ownable(), ERC721("DCG Mastermind","DCG") {
    ...
    constructor()  {
        ...
        _uri = "https://ipfs.io/ipfs/QmZNqu3DXqmc4TToM2VkmAmMMnf8Ce5Ga2uAWy1P6RLZoY/";
    ...
    function buyDCGNFT(uint256 _id) payable public {
        ...
        if(!whitelist[msg.sender]){
            require(msg.value >= 0.1 ether && msg.value <= 5 ether, "ERROR: Wrong price range");
        }
    ...
    function addWhitelistAddress(address[] memory addresses) public onlyOwner{
        for(uint256 i=0; i<addresses.length; i++){
            whitelist[addresses[i]] = true;
        }
    }

    function removeWhitelistAddress(address[] memory addresses) public onlyOwner{
        for(uint256 i=0; i<addresses.length; i++){
