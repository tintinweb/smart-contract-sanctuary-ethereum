...
import "./utils/DissrupFees.sol";

contract DissrupMarketplace is Initializable,ContextUpgradeable,ERC165Upgradeable,AccessControlEnumerableUpgradeable, Auction, Proposal, DirectSale, DissrupFees {
    ...
    uint256 constant SALE_TYPE_SIZE = 3;
    ...
    mapping(address => mapping(uint256 => bool)) public assetsHistory;
    ...
    event RevealedChanged(uint256 listingId,bool isRevealed);
    event AddOrRemoveContractToMarketplace(address contractAddress,bool addOrRemove);

//LOGIC
    function initialize(address _dissrupAssetAddress, address _dissrupPayoutAddress) public initializer {
        __Context_init_unchained();
        __ERC165_init_unchained();
        currentListingId++;
        _tempDissrupAssetAddress = _dissrupAssetAddress;
        setDissrupPayoutAddress(_dissrupPayoutAddress);
        approvedContracts[_dissrupAssetAddress] = true;
    ...
    function transferAsset(uint256 _listingId, address _from, address _to, uint256 _amount) private {
        ...
        if (listing.contractTokenType == ContractTokenType.ERC721) {
            ...
            if(!erc721.isApprovedForAll(_msgSender(),address(this))){
                //require(erc721.approved());
            }
    ...
    function transferListingToBuyer(uint256 _listingId, uint256 _amount, address _buyer) private {
        ...
        if (listing.amount == 0) {
            ...
            assetsHistory[listing.contractAddress][listing.tokenId] = true;
    ...
    function list(
        ...
        require(approvedContracts[_contractAddress],"Contract not approved by dissrup!");
        require(_amount > 0,"amount is zero!");
         require(_price >= MIN_PRICE || _saleType == SaleType.Proposal, "price is too low to sell"); // proposal have no price
        require(_saleType <= SaleType.Auction,"Not valid sale type");
        ...
        bool isSecondaryList = assetsHistory[_contractAddress][_tokenId];
        //address[] memory payees,  uint256[] memory shares
        DissrupFees.CreateRoyaltiesWithDissrupFee(_contractAddress,_tokenId,currentListingId, _msgSender(), isSecondaryList);
        currentListingId++;
        return true;
    }
    ...
    function buyDirectSale(uint256 _listingId, uint256 _amount) public payable returns (bool) {
        ...
        require(msg.value >= total, "not enough ether to buy");
        ...
        DissrupFees.finishSale(_msgSender(),_listingId,total);
    ...
    function addOrRemoveContractToMarketplace(address _contractAddress, bool addOrRemove) public returns(bool) {
    // check role YAMBO
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Only admin can change");
    approvedContracts[_contractAddress] = addOrRemove;
        emit AddOrRemoveContractToMarketplace(_contractAddress,addOrRemove);
        return addOrRemove;
    }
    function setRevealed(uint256 _listingId, bool _isRevealed) public returns(bool){
        Listings storage listing = listings[_listingId];
        require (listing.initialized, "listing doesnt exist");
        require(listing.seller == _msgSender(),"Must be seller to set Revealed");
        listings[_listingId].isRevealed = _isRevealed;
        emit RevealedChanged(_listingId,_isRevealed);
        return true;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override ( ERC165Upgradeable, AccessControlEnumerableUpgradeable,DissrupFees ) returns (bool){
...
contract Auction is ContextUpgradeable {
    ...
    function getMinBidAmount(uint256 _listingId) private view returns (uint256) {
        Auction memory auction = auctions[_listingId];
...
contract DissrupFees is ContextUpgradeable,AccessControlEnumerableUpgradeable {
    using ERC165CheckerUpgradeable for address;

    bytes4 public constant IID_IROYALTIES = type(IRoyalties).interfaceId;

    uint256 constant MANIFOLD_OFFSET = 100;
    struct Royalty{
        address payable payee;
        uint256 share;
    }
    mapping(uint256 => Royalty[]) public saleToRoyalties;
    address dissrupAddress;
    event FinishSale(uint256 listingId);
    event Royalties();
    function setDissrupPayoutAddress(address _dissrupPayoutAddress) public {
        //require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Only admin can set");
        dissrupAddress = _dissrupPayoutAddress;
    }

    function CreateRoyaltiesWithDissrupFee(address _contractAddress,uint256 _tokenId, uint256 _listingId,address creator, bool isSecondaryList) internal returns(bool){
        uint256 allShares = 100;
        uint256 dissrupCut = (isSecondaryList) ? 15 : 5;
        saleToRoyalties[_listingId].push(Royalty({payee: payable(dissrupAddress), share: dissrupCut}));
        allShares -= dissrupCut;
    if(!isSecondaryList){
            uint256 creatorCut = 10;
            saleToRoyalties[_listingId].push(Royalty({payee: payable(creator), share: creatorCut}));
            allShares -= creatorCut;
    }
        allShares -= getRoyaltiesFromIRoyaltiesIfExist(_contractAddress,_tokenId,_listingId);
        require(allShares >= 0,"Shares make more then 100%");
        return true;
    }

    function getRoyaltiesFromIRoyaltiesIfExist(address _contractAddress, uint256 _tokenId,uint256 _listingId) private returns (uint256){
        uint256 sharesSum = 0;
        if(_contractAddress.supportsInterface(IID_IROYALTIES)){
        IRoyalties manifoldRoyalties = IRoyalties(_contractAddress);
        (address payable [] memory _payees, uint256 [] memory _shares) = manifoldRoyalties.getRoyalties(_tokenId);
            for(uint256 i = 0; i < _payees.length; i++){
                uint256 share = _shares[i] / MANIFOLD_OFFSET;
                saleToRoyalties[_listingId].push(Royalty({payee: _payees[i], share: share }));
                sharesSum += share;
            }
        }
        return sharesSum;
    ...
    function finishSale(address seller,uint256 _listingId,uint256 _price) internal returns (bool){
        uint256 oneShare = _price / 100;
        uint256 left = _price;
        for(uint256 i = 0; i < saleToRoyalties[_listingId].length; i++){
            Royalty memory royalty = saleToRoyalties[_listingId][i];
            uint256 payout = royalty.share * oneShare;
            payable(royalty.payee).transfer(payout);
            left -= payout;
        }
        payable(seller).transfer(left);
        emit FinishSale(_listingId);
        return true;
    }
