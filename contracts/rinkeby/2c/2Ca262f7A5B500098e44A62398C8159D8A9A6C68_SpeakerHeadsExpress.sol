...
/*
 * MultiToken SpeakerHeads contract for managing token drops
 *
 * by @prabhu
 */
contract SpeakerHeadsExpress is ERC1155, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => EnumerableSet.AddressSet) private  tokenWhiteList;
    ...
    function addToken(bool _active, bool _restrictSale,string memory _name, string memory _metadataUrl, uint256 _maxSupply, uint256 _mintWeiPrice) 
    public
    onlyOwner
    {
        ...
        EnumerableSet.AddressSet storage tokenWL = tokenWhiteList[tokenCounter.current()];
    ...
    isTokenTransactionValid(tokenId, amount, true)
    {
        ...
        if (token.restrictSale) {
            require( verifyWhitelist(msg.sender, tokenId) , "Not Whitelisted");
        }
    ...
    function verifyWhitelist(address sender, uint256 tokenIndex) internal view returns (bool) {
        EnumerableSet.AddressSet storage tokenWL = tokenWhiteList[tokenIndex];
        return tokenWL.contains(sender);
    }
    function revokeFromAllowList(uint256 tokenId, address revokeAddress) public onlyOwner {
        EnumerableSet.AddressSet storage tokenWL = tokenWhiteList[tokenId];
        tokenWL.remove(revokeAddress);
    }
    function appendToAllowList(uint256 tokenId, address[] memory appendList) public onlyOwner {
        EnumerableSet.AddressSet storage tokenWL = tokenWhiteList[tokenId];
        for (uint i = 0; i < appendList.length - 1; i++) {
            tokenWL.add(appendList[i]);
        }
    }
  
    function getTokenWhiteListCount(uint256 tokenId) external view returns (uint256) {
        return tokenWhiteList[tokenId].length();
    }

    function getTokenWhiteList (uint256 tokenId) external view returns (address[] memory) {
        return tokenWhiteList[tokenId].values();
    }
}
