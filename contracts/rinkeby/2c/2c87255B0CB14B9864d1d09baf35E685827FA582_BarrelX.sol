...
contract BarrelX is JupiterNFT, MintOptions,  MintPayable, IBarrelX, AllowList{
    ...
    function mintBatchBarrel (uint8 _option, uint256 _amount) payable external override{
        for (uint8 i=0; i < _amount; i++){
            ...
            require(options[_option].remaining > 0, 'No barrels remaining for option');
            // non operator validations
            require(msg.value >= options[_option].price * _amount, 'Not enough ETH sent, check price!');
            require(balanceOf(msg.sender) <= 10, 'Max barrels per wallet exceeded');
            ...
            emit MintBarrel(msg.sender, _option,currentTokenId);
        }
    }
...
abstract contract MintOptions is JupiterNFT {
    ...
    // Defines a single mint option
    // price the cost in wei this option has
    // remaining how many slots are available to mint with this option
    struct Option {
        uint256 price;
        uint256 remaining;
        bool enabled;
    ...
    constructor (){
        // option 1 - 3 month all mashBills
        // mashbills are 
        // 1 64C/24R/12M
        // 2 64C/24W/12M
        // 3 70C/21R/09M
    ...
    function setOption(uint8 _optionNumber, uint256 _newPrice, uint256 _newRemaining, bool _enabled) public {
        require(operators[msg.sender], "only operators");
        options[_optionNumber] = Option(_newPrice, _newRemaining, _enabled);
    }

    function getOptionPrice(uint8 _optionNumber) public view returns(uint256){
        return options[_optionNumber].price;
    }

    function getOptionRemaining(uint8 _optionNumber) public view returns(uint256){
        return options[_optionNumber].remaining;
    }

    function getOptionEnabled(uint8 _optionNumber) public view returns(bool){
        return options[_optionNumber].enabled;
    }
