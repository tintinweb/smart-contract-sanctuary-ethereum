...
contract MoonDAOWinner is Ownable,VRFConsumerBaseV2 {
  ...
  mapping(uint256 => uint) public winnerList;
  ...
  uint16 public len =uint16(NFTBlacklist.length);
  ...
  function reMapping(
    ...
    if(rolledNum<NFTBlacklist[0]){
      backnum= rolledNum;
    }else if (rolledNum+len > NFTBlacklist[len-1]){
      backnum= rolledNum + len;
    }else {
  ...
  function mappingDiceToNFT() external onlyOwner {
    for(uint i=0;i<requestCounter;i++){    //120万操作数1个值，1760万11个值。
      winnerNFTList[i] = reMapping(uint16(winnerList[i]));
    }
  }
  ...
  function reMappingAList() external onlyOwner{  
    ...
    uint16 reci=1;
    for(uint index=0;index<requestCounter;index++){
      uint16 rolledNum = b[index];  
      if(rolledNum<NFTBlacklist[0]){
        winnerNFTList[index]= rolledNum;      
      }else if (rolledNum+len > NFTBlacklist[len-1]){
        winnerNFTList[index]= rolledNum + len;
      }else {
        for(uint16 i=reci; i<len-1; i++){
          if(rolledNum+i>NFTBlacklist[i-1] && rolledNum+i<NFTBlacklist[i]){
            winnerNFTList[index] = rolledNum+i;
            reci=i;
            break;
          } 
        } 
  ...
  function insertionSort(uint16[] memory a) public pure returns(uint16[] memory) {
...
// 0x4CDf24BedD6dBc8aB6bb675E2ACe1f8073591515 rinkeby
