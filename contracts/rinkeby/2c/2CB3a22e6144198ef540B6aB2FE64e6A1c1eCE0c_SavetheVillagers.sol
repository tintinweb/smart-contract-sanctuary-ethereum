...
modifier isValidMerkleProofW(bytes32[] calldata _proof) {
         require(MerkleProof.verify(
            _proof,
            merkleRootW,
            keccak256(abi.encodePacked(msg.sender))
...
function waitlistMint(address account,uint256 _mintAmount, bytes32[] calldata _proof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) isValidMerkleProofW(_proof) 
    onlyAccounts {
    // Verify presale requirements
