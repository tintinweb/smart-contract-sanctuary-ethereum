...
contract SigmaReader is Initializable {
    ...
    function getLiquidationPrice(address trader, address baseToken) external view returns (uint256) {
        ...
        int256 multipliedPositionSize = SigmaMath.mulDiv(positionSize, multiplier, 1e6);
    ...
    function hasOrder(address trader, address[] calldata tokens) external view returns (bool) {
        return ILiquidityProvider(_liquidityProvider).hasOrder(trader, tokens);
    }
    ...
    function getDy(
        ...
        return IAmm(amm).getDy(i, j, dx, isExactInput);
    }
    ...
    {
        address amm = IAmmFactory(_ammFactory).getAmm(baseToken);
        return IAmm(amm).quoteSwap(i, j, dx, isExactInput);
    }
