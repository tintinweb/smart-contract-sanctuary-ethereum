    ...
    GuardManager
{
    ...
    /*address private daoFactory;

    modifier onlySubscribedDAO{
        //require(IDAOFactory(daoFactory).containsDao(msg.sender), "GS: caller is not registered dao");
        require(IDAOFactory(daoFactory).subscriptions(msg.sender) >= block.timestamp, "GS: subscription expired");
        _;
    }*/

    address public constant daoFactory = 0x52d1e8356849F85d5c7a1cf71A00cd6ba4a363c0;
    ...
    function setup(
        ...
        // добавляем подписку - вызвали addDAO() с Factory.sol
        IDAOFactory(daoFactory).addDAO(address(this));
    ...
    ) public payable virtual /*onlySubscribedDAO*/ returns (bool success) {
        bytes32 txHash;

        require(IDAOFactory(daoFactory).subscriptions(address(this)) >= block.timestamp, "GS: subscription expired");
