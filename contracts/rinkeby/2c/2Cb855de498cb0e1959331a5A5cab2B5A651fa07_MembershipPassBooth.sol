...
import {IMembershipPass, IUriStore} from "./interfaces/IMembershipPass.sol";
...
contract MembershipPass is IMembershipPass, ERC1155, Ownable {
    ...
    // Uri store utils to store the global basic uris
    IUriStore public override uriStore;
    ...
    function uri(uint256 _tokenId) public view override returns (string memory) {
        string memory _baseURI = uriStore.baseURI();
    ...
    function contractURI() public view returns (string memory) {
        string memory _baseContractURI = uriStore.baseContractURI();
    ...
    constructor(
        ...
        if (_uriStore == IUriStore(address(0))) revert TierUnknow();
        uint256 _tier = 0;
...
import {MembershipPass, IMembershipPass, IUriStore} from "./MembershipPass.sol";
...
contract MembershipPassBooth is IMembershipPassBooth, TerminalUtility {
    ...
    /*╔═════════════════════════╗
      ...
      ╚═════════════════════════╝*/
    constructor(ITerminalDirectory _terminalDirectory, IUriStore _uriStore)
    ...
    function issue(
        ...
        emit Issue(_projectId, _membershipPass, _tierFees, _tierCapacities);
    }
