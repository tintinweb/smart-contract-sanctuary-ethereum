...
contract SomaStaking is ISomaStaking, ReentrancyGuardUpgradeable, TokenRecoveryUpgradeable, GuardableUpgradeable {
    ...
    modifier onlyAdmin() {
        ...
        require(hasRole(GLOBAL_ADMIN_ROLE, _sender) || hasRole(LOCAL_ADMIN_ROLE, _sender), 'SomaStaking: ADMIN_ONLY');
        _;
    }
    ...
    function claimRequest(address _account, address _asset) external override view returns (TimedRequest memory) {
        return _users[_account].claimRequest[_asset];
    }

    function unstakeRequest(address _account) external override view returns (TimedRequest memory) {
        return _users[_account].unstakeRequest;
    }

    function initialize(address stakingToken, address[] memory rewardTokens) external override initializer {
    ...
    function currentTPS(address token) public override view returns (uint256 tps_) {
        ...
        while (curId != 0) {
            ...
            if (block.timestamp < _strategy.startDate) {
                break;
            }

            uint256 rewardsUnlocked = SomaStakingLibrary.rewardsUnlocked(_strategy, block.timestamp);
            tps_ += SomaStakingLibrary.rewardsToTPS(totalStake_, rewardsUnlocked);
        }
    }

    function requestUnstake(uint256 _amount) external override nonReentrant {
        address _sender = _msgSender();
        UserInfo storage _user = _users[_sender];
        uint256 _userStake = _user.stake;
        uint256 totalStake_ = _totalStaked;
        TimedRequest memory _unstakeRequest = _user.unstakeRequest;
        uint256 _totalUserStake = _userStake + _unstakeRequest.amount;

        require(_totalUserStake >= _amount, "Staking: INSUFFICIENT_STAKE");
        ...
        // this unstake request amount
        _syncUser(_user, _userStake, _totalUserStake - _amount);

        // adjust the total stake balances
        unchecked {
            _totalStaked = totalStake_ + _unstakeRequest.amount - _amount;
            _totalPendingUnstake = _totalPendingUnstake + _amount - _unstakeRequest.amount;
        }

        _user.unstakeRequest = TimedRequest({
            requested: true,
            timestamp: block.timestamp.toUint64(),
            amount: _amount.toUint128()
        });

        emit UnstakeRequested(_amount, _sender);
    }

    function cancelUnstakeRequest() external override nonReentrant {
        address _sender = _msgSender();
        UserInfo storage _user = _users[_sender];
        uint256 _userStake = _user.stake;
        uint256 totalStake_ = _totalStaked;

        TimedRequest memory _request = _user.unstakeRequest;

        require(_request.requested, 'Staking: INVALID_REQUEST');

        _update(totalStake_);
        // return the stake back to the user
        _syncUser(_user, _userStake, _userStake + _request.amount);

        unchecked {
            _totalStaked = totalStake_ + _request.amount;
            _totalPendingUnstake -= _request.amount;
        }

        delete _user.unstakeRequest;

        emit UnstakeRequestCancelled(_request, _sender);
    }

    function requestClaim(address[] calldata _assets) external override nonReentrant {
        address _sender = _msgSender();
        UserInfo storage _user = _users[_sender];
        uint256 _userStake = _user.stake;
        ...
        for (uint i = 0; i < _assets.length; ++i) {
            _requestClaim(_user, _sender, _assets[i]);
        }
    }

    function claim(address[] calldata _assets) external override nonReentrant {
        address _sender = _msgSender();
        UserInfo storage _user = _users[_sender];
    ...
    function adminClaim(address _asset, address _to) external override onlyAdmin {
        uint256 amount = _adminClaimable[_asset];
        require(amount > 0, "Staking: NOTHING_TO_CLAIM");
        delete _adminClaimable[_asset];
        IERC20(_asset).safeTransfer(_to, amount);
        emit AdminClaimed(_asset, amount, _to, _msgSender());
    }
    ...
    function unstake() external override nonReentrant {
        ...
        // If user has not flagged their intention to unstake, fail.
        require(_request.requested, "Staking: INVALID_REQUEST");
        // If unstakeTimestamp is larger than 0 but less than the current strategy wait time, fail.
        require(block.timestamp - _request.timestamp >= config_.unstakeDuration, "Staking: INSUFFICIENT_TIME");

        _totalPendingUnstake -= _request.amount;
        IERC20(_stakingToken).safeTransfer(_sender, _request.amount);

        emit Unstaked(_request.amount, _sender);
    }
    ...
    function unstakeImmediate(uint256 _amount) external override nonReentrant {
        ...
        require(_totalUserStake >= _amount, "Staking: INSUFFICIENT_BALANCE");
        require(_amount > 0, "Staking: INVALID_AMOUNT");
        ...
        if (_unstakeRequest.requested) delete _user.unstakeRequest;
    ...
    function createStrategy(
        ...
        require(_startDate <= type(uint48).max, 'SomaStaking: INVALID_START_DATE');
        ...
        Strategy memory _strategy = Strategy({
            ...
            rewardsLocked: SafeERC20Balance.safeTransferFrom(
                IERC20(_rewardToken),
                _msgSender(),
                address(this),
                _rewardAmount
    ...
    function updateConfig(
        ...
        Percent.validatePercent(_earlyUnstakeFee);
        Percent.validatePercent(_earlyClaimFee);
    ...
    function _claim(
        ...
        TimedRequest memory request = _user.claimRequest[_asset];
        delete _user.claimRequest[_asset];

        require(_rewardTokens.contains(_asset), 'Staking: INVALID_ASSET');
        require(request.requested, "Staking: INVALID_REQUEST");
        require(block.timestamp - request.timestamp >= _claimDuration, "Staking: INSUFFICIENT_TIME");

        _user.claimable[_asset] -= request.amount;

        IERC20(_asset).safeTransfer(_sender, request.amount);

        emit Claimed(_asset, request.amount, _sender);
    }
    ...
    function _requestClaim(
        ...
        _user.claimRequest[_asset] = TimedRequest({
            requested: true,
            timestamp: block.timestamp.toUint64(),
            amount: _rewards.toUint128()
        });

        emit ClaimRequested(_asset, _rewards, _sender);
    }
}
...
library SomaStakingLibrary {
    ...
    function rewardsToTPS(uint256 totalStake, uint256 _rewardsUnlocked) internal pure returns (uint256 tps) {
        return totalStake == 0
            ? 0
            : (_rewardsUnlocked * PRECISION_FACTOR) / totalStake;
    }
...
interface ISomaStaking {
    ...
    event UnstakeRequested(uint256 amount, address indexed sender);
    event UnstakedImmediate(uint256 amount, uint256 fee, address indexed sender);
    event UnstakeRequestCancelled(TimedRequest request, address indexed sender);
    event Claimed(address indexed asset, uint256 amount, address indexed sender);
    event ClaimRequested(address indexed asset, uint256 amount, address indexed sender);
    ...
    struct UserInfo {
        ...
        TimedRequest unstakeRequest;

        mapping(address => TimedRequest) claimRequest;
    ...
    function claimRequest(address _account, address _asset) external view returns (TimedRequest memory);
    function unstakeRequest(address _account) external view returns (TimedRequest memory);
    ...
    function requestClaim(address[] calldata _assets) external;
    function claim(address[] calldata _assets) external;
    ...
    // TODO should to be an address input to the contract?
