...
contract RPFPaperStorage {
    mapping(uint256 => bool) public writtenRPFPaper;
...
contract RPFPaper is IRPFPaper, RPFPaperStorage, Ownable, EIP712, ERC721A {

    using SafeMath for uint256;
	using Strings for uint256;

    constructor()
    EIP712("RPFPaper", "1.0.0")
    ERC721A("RPFPaper", "RPFP")
    {
        writeEnable = false;
    ...
    modifier paperWritten(uint256 tokenId) {
        require(writtenRPFPaper[tokenId] == false, "PP_WRITTEN");
        _;
    }
    ...
    function mintGiveawayPaper(
        ...
        onlyOwner
    {   
        require(totalSupply().add(quantity) <= MAX_RPFPAPER, "EXCEED_MAX_RPFPAPER.");
        ...
        returns (bool[] memory)
    {
        ...
        if (tokenCount == 0) {
            ...
            for (index = 0; index < tokenCount; index++) {
                ...
                result[index] = writtenRPFPaper[token];
            }
            return result;
        }
    }
