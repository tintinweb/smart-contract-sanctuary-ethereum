...
contract Rising is ERC20, Ownable {
    ...
    function excludeFromLimit(address account, bool excluded) public onlyOwner {
        require(isExcludedFromLimit[account] != excluded, "RISING: Account is already the value of 'excluded'");
    ...
    function excludeMultipleAccountsFromLimit(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            isExcludedFromLimit[accounts[i]] = excluded;
        }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "RISING: Automated market maker pair is already set to that value");
    ...
    function handleLimit(address from) internal {
        if (isLimitedUser(from)) {
            if (block.timestamp > _limitedUsers[from].limitStartTime.add(timeLock)) {
                ...
                revert("RISING: Currently blocked from selling. Please try again after ~24 hours");
            }
        }
    }
    ...
    function _transfer(address from,address to,uint256 amount) internal override {
        ...
        if (_isSell(from, to) && !isExcludedFromLimit[from]) {
            handleLimit(from);
        }

        if (_isBuy(from) && !isExcludedFromLimit[to]) {
    ...
    function _validateTime(address sender,address recipient,bool takeFee) private {    // Excluded addresses don't have time limits
        if (takeFee) {
            if (_isBuy(sender)) {
