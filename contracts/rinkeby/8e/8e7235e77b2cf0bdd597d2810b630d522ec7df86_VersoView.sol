...
contract VersoView is ERC721Enumerable, Ownable{
    using Strings for uint256;
    using SafeMath for uint;
    
    string public baseURI;

    uint256 public constant TOTAL_SUPPLY = 210;
            
    bool public saleActivated;

    constructor()
        ERC721("Versoview", "VVT")
    {
    }
    ...
    function mintNFT(address[] calldata destinations) public onlyOwner{

        require(totalSupply() < TOTAL_SUPPLY, "assignNFT: sale has already ended");
        require(totalSupply().add(destinations.length) <= TOTAL_SUPPLY, "assignNFT: sale has already ended");
