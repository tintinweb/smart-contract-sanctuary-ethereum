...
contract PepeRoyaleV1 is ERC721A, Ownable {

    event FightResults(uint winner, uint loser);
    ...
    // Game Data
	mapping(uint => bool) public playerStatuses; // True-Alive False-Dead
    mapping(uint => uint) public playerKills; 
    uint public livingPlayers = 0;
    bool public gameStarted = false;

    // Name
	constructor()
	ERC721A('PepeRoyale', 'PEPE') {
    }

	function _initalizePlayer(uint ID) internal {
		playerStatuses[ID] = true;
        playerKills[ID] = 0;
        livingPlayers++;
        if (livingPlayers >= maxSupply) gameStarted = true;
	}
    ...
    /*function getTest() public view returns (uint[] memory, uint[] memory) {
        uint[] memory _test1;
        _test1 = [0, 1, 2];
        uint[] memory _test2;
        _test2 = [0, 1, 2];
    ...
    function getPlayers(address addr) public view returns (address[] memory _players) {
        mapping(uint => Player) memory _tempPlayers;
        uint playersFound = 0;
        for (uint i = 1; i <= totalSupply(); i++) {
            if (addr == ownerOf(i)) {
                players[playersFound] = Player({
                    id: i,
                    status: playerStatuses[i] ? "true" : "false",
                    kills: playerKills[i]
                });
                playersFound++;
            }
    ...
    /*function getPlayers(address addr) public view returns (PlayerList memory _playerList) {
        Player[] memory players = new Player[](6);
        uint playersFound = 0;
        for (uint i = 1; i <= totalSupply(); i++) {
            if (addr == ownerOf(i)) {
                players[playersFound] = Player({
    ...
    function findPlayers(address addr) external view returns (uint[] memory) {
        uint[] memory playerIds = new uint[](0);
        uint playersFound = 0;
        for (uint i = 1; i <= totalSupply(); i++) {
            if (addr == ownerOf(i)) {
                playerIds[0] = i;
    ...
    function playerStats(uint id) external view returns (string memory, uint) {
        return (playerStatuses[id] ? "true" : "false", playerKills[id]);
    }
	...
	function airdrop(address to, uint count) external onlyOwner {
        ...
        // Initialize Newly Minted Player(s)
        for (uint i = 0; i < count; i++) {
            _initalizePlayer(totalSupply() + 1 + i);
        }
    ...
    function PUBLIC_MINT(uint count) external payable {
        ...
        // Update Newly Minted Player Statuses to "Alive"
        for (uint i = 0; i < count; i++) {
            _initalizePlayer(totalSupply() + 1 + i);
        }
    ...
    function fightPlayers(uint256 from, uint256 to) public {
        require(gameStarted, "Cannot fight. All players have not been minted yet.");
        require(msg.sender == ownerOf(from), "Cannot fight using an unowned player.");
        require((playerStatuses[from] == true) && (playerStatuses[to] == true), "Both players must be alive.");
        
        // Generate Fight Sequence
        uint fromOdds = playerKills[from] + 1;
        uint toOdds = playerKills[to] + 1;

        uint randomHash = uint(keccak256(abi.encodePacked(msg.sender, block.timestamp)));
        bool winner = (randomHash % (fromOdds + toOdds)) < fromOdds;
        ...
        if (winner) {
            //_burn(to); // BURN loser
            playerStatuses[to] = false; // Kill loser
            playerKills[from]++; // Increase winner's kills by 1
            livingPlayers--;
            winningID = from;
            losingID = to;
        } else {
            //_burn(from); // BURN loser
            playerStatuses[from] = false; // Kill loser
            playerKills[to]++; // Increase winner's kills by 1
            livingPlayers--;
            winningID = to;
            losingID = from;
        }

        // Check if this is the last player, if so then declare the winner
        if (livingPlayers == 1) {
            // Winner stuff
            uint256 balance = address(this).balance;
            address winnerAddr;
            if (winner) {
                winnerAddr = ownerOf(from);
            } else { 
                winnerAddr = ownerOf(to);
            }

            uint _payable1 = balance * 75 / 100;
            payable(winnerAddr).transfer(_payable1);

            uint _payable2 = balance * 25 / 100;
            payable(owner()).transfer(_payable2);
        }

        //return returnString;
        emit FightResults(winningID, losingID); // Return winner results
    }
}