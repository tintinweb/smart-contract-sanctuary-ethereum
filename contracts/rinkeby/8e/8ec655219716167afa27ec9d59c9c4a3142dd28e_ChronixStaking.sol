...
contract ChronixStaking is Initializable, ReentrancyGuardUpgradeable, ContextUpgradeable {
    ...
    function getPotTax(address staker) public view returns(uint) {
        ...
        if(address(PushersNFT) != address(0) && user.stakedChronix.length !=0 || ERC721A(PushersNFT).balanceOf(staker) !=0){
    ...
    function ifds() public view returns(bool){
      return(address(PushersNFT) == address(0));
    }
}
