...
error FWBHoldersMintLimitReached();
...
error FWBRequiredBalanceNotMet();
...
contract TaikaFWB is ERC721Royalty, Ownable, ReentrancyGuard {
  ...
  uint256 public mintAllocationFWB = 100;
  uint256 public mintAllocationAllowlist = 100;
  uint256 public mintAllocationPublic = 200;

  mapping(SaleState => uint256) public cumulativeAllocationMap;
  mapping(address => uint256) public mintsPerAddress;

  uint256 public mintPriceFWB = 0.05 ether;
  uint256 public mintPriceAllowlist = 0.08 ether;
  uint256 public mintPricePublic = 0.08 ether;

  uint256 public minimumFWBRequired = 5;

  address public allowlistSigner = 0x6Bb3a9cdCa3CF8341513230582b793828C9c2298;
  ...
  modifier isSaleValid(SaleState targetState, address receiver) {
    ...
    if (numberOfMintedTokens >= cumulativeAllocationMap[targetState]) {
      if (targetState == SaleState.FWB) {
        revert FWBHoldersMintLimitReached();
      }
  ...
  modifier holdsMinimumFWBTokens() {
    if (fwbBalanceOfAddress(msg.sender) < minimumFWBRequired * (1 ether)) {
      revert FWBRequiredBalanceNotMet();
    }
  ...
  function updateCumulativeAllocationMap() private {
    cumulativeAllocationMap[SaleState.FWB] = mintAllocationFWB;
  ...
  function unlockSaleToFWBHolders() public onlyOwner {
    saleState = SaleState.FWB;
  }
  ...
  function setMintAllocationFWB(uint256 newFWBAllocation)
    public
    onlyOwner
    isAllocationSafe(newFWBAllocation, mintAllocationFWB)
  {
    ...
    mintAllocationFWB = newFWBAllocation;
    updateCumulativeAllocationMap();
  }
   ...
   */
  function setMintPriceFWB(uint256 newPrice) public onlyOwner {
    mintPriceFWB = newPrice;
  }
   ...
   */
  function setAllowlistSigner(address newSignerAddress) public onlyOwner {
    allowlistSigner = newSignerAddress;
  }
  ...
  function mintFWB(bytes memory signature)
    public
    payable
    isSaleValid(SaleState.FWB, msg.sender)
    isValueValid(mintPriceFWB)
    holdsMinimumFWBTokens
  {
    if (getSigner(signature) != allowlistSigner) {
