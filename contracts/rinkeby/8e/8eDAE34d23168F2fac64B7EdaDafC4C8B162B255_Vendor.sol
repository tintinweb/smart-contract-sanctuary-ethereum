...
contract Vendor is Ownable {
    ...
    function sellTokens(uint256 tokenToSell) public {
        require(tokenToSell > 0, "Failed to sell, must be grater than 0");

        uint256 userBalance = yourToken.balanceOf(msg.sender);
        require(
            userBalance >= tokenToSell,
            "User has insufficient amount of tokens to sell"
        );

        uint256 ethAmountToTransefer = tokenToSell / tokensPerEth;
        uint256 ownerBalance = address(this).balance;
        require(
            ownerBalance >= ethAmountToTransefer,
            "Owner has insufficient balance to transfer"
        );
        yourToken.transferFrom(msg.sender, address(this), tokenToSell);

        (bool sent, ) = msg.sender.call{value: ethAmountToTransefer}("");
