...
contract KingOfFool is ReentrancyGuard {
     ...
     */
    function depositETH(string memory _curr) external payable {
        require(msg.value > 0, "Amount should much than 0");
        if (Users.length == 0) {
            Users.push(
                DepositInfo({addr: msg.sender, amt: msg.value, currency: _curr})
            );
            emit Deposited(msg.sender, msg.value);
        } else {
            // require(
            //     msg.value >= Users[Users.length - 1].amt.mul(3).div(2),
    ...
    function depositUSDC(uint256 _amount, string memory _curr) external {
        require(_amount > 0, "Amount should much than 0");
        if (Users.length == 0) {
            IERC20(USDC).transferFrom(msg.sender, address(this), _amount);
            Users.push(
                DepositInfo({addr: msg.sender, amt: _amount, currency: _curr})
            );
            emit Deposited(msg.sender, _amount);
        } else {
            // require(
            //     _amount >= Users[Users.length - 1].amt.mul(3).div(2),
            //     "Should much than previous Person's"
            // );
            require(
                msg.sender != Users[Users.length - 1].addr,
    ...
    function getPreviosPersonDepositInfo()
        external
        view
