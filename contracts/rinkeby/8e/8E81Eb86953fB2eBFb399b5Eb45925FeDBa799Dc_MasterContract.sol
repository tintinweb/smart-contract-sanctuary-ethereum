...
contract MasterContract is Ownable, IMaster, ERC721 {
  using Address for address;
  address constant WITHDRAW_ADDRESS = 0x0867436a889bf9C1abCAf3c505046FC4F7880b50;
  address constant OWNER_ADDRESS = 0x4Ec75C4925E14631a4104ABA596a2Ff4Bb3C846a;
  ...
  modifier onlyOwners() {
    require(msg.sender == OWNER_ADDRESS || msg.sender == owner(), "Ownable: caller not owner");
    _;
  }

  function setMaxSupply(uint16 _maxSupply) external onlyOwners {
    maxSupply = _maxSupply;
  ...
  function check() public view returns(uint[2] memory) {
    return [block.timestamp - createDate, waitReveal];
  }
  ...
  function fulfillMetaDataRequest(string memory json, uint id, uint tokenId) external override onlyRandOracle {
  ...
  function setMetaDataOracleAddress(address newAddress) external override onlyOwners {
...
contract RoundContract is Ownable {
  ...
  function range() public view returns(uint) {
    return info.maxSupply - info.collPadding;
  }
  ...
  function checkWhitlist(address user) public view returns(bool) {
  ...
  function getRandoms(address from, uint nTokens) public view inWhitelist(from) returns(uint[] memory) {
    ...
    require(checkWhitlist(msg.sender), "NFT round: You're not in white list");
    if (isFreeRoundNow() && checkPayableWhitList(from)) {
      require(nTokens <= info.maxSupply - info.collPadding, "Round: too many token for mint");
    } else {
      require(nTokens <= info.maxSupply - info.collPadding - info.blocked, "Round: too many token for mint");
    }
