...
// File: contracts/AtevadaTokenAccessControl.sol
...
contract AtevadaToken is ERC20, AtevadaTokenAccessControl{
	...
	function mint(address _to, uint256 amount) requireContractOwner external {
	...
	function updateRewardOnMint(address _user, uint256 _amount) requireIsOperational requireIsCallerAuthorized external {
		...
		if (timerUser > 0)
			rewards[_user] = rewards[_user].add(atevadaContract.balanceOR(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(30)
	...
	function updateReward(address _from, address _to) requireIsOperational requireIsCallerAuthorized external {
		// require(msg.sender == address(atevadaContract));
			uint256 time = min(block.timestamp, END);
			uint256 timerFrom = lastUpdate[_from];
			if (timerFrom > 0)
				rewards[_from] += atevadaContract.balanceOR(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(30);
			if (timerFrom != END)
				lastUpdate[_from] = time;
			if (_to != address(0)) {
				uint256 timerTo = lastUpdate[_to];
				if (timerTo > 0)
					rewards[_to] += atevadaContract.balanceOR(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(30);
				if (timerTo != END)
					lastUpdate[_to] = time;
			}
	}
	...
	function getTotalClaimable(address _user) external view returns(uint256) {
		uint256 time = min(block.timestamp, END);
		uint256 pending = atevadaContract.balanceOR(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(30); // 86400 1 day in seconds
		return rewards[_user] + pending;
	}

    function setIAtevada(address _atevada) requireContractOwner external {
	    atevadaContract = IATVD(_atevada);
	}
}
...
contract Atevada is ERC721A, ReentrancyGuard, Ownable {
    ...
    uint256 public constant DISCOUNT_PRICE = 0.24 ether; 

    uint256 public immutable maxPerAddressDuringMint;
    uint256 public constant AMOUNT_FOR_DEVS = 15; // 5 dev + 10 for events and marketing
    uint256 public constant FREE_MINT_SUPPLY = 55; 
    uint256 public freeMintCount;

    mapping(address => uint256) public whitelistedList;
    mapping(address => uint256) public discountList;
    mapping(address => uint256) public addressMintedList;
    // mapping(address => uint256) public freeMintList;
    mapping(address => bool ) public freeMintClaimed;
    bytes32 public FreeMintRoot = 0x4140db65186c93e074a38f6d4739601264c9f47f3bd335f66a2719c0b3355549;
    ...
    function addWhitelistUsers(address[] memory _users) external onlyOwner {
        for ( uint256 i = 0; i < _users.length; i++) {
            whitelistedList[_users[i]] = 1;
        }
    }

    function addDiscountList(address[] memory _users) external onlyOwner {
        for ( uint256 i = 0; i < _users.length; i++) {
            discountList[_users[i]] = 1;
        }
    }

    function whitelistSaleMint(uint256 _quantity) 
    callerIsUser 
    ...
    external payable {
        ...
        require(totalSupply() + _quantity <= collectionSize, "Purchase would reached max supply");

        if(discountList[msg.sender] == 1){
            require(msg.value >= DISCOUNT_PRICE * _quantity, "insufficient funds");
            _safeMint(msg.sender, _quantity);
            refundIfOver(DISCOUNT_PRICE * _quantity);
        }else {
            require(msg.value >= PRICE * _quantity, "insufficient funds");
            _safeMint(msg.sender, _quantity);
            refundIfOver(PRICE * _quantity);
        }
        addressMintedList[msg.sender] = 1;
    ...
    external payable {
        require(_quantity > 0, "Minimum mint amount is 1");
        require(_quantity == 1 || _quantity == 3 || _quantity == 5, "Can only mint using the provided options");
        // require(_quantity <= maxPerAddressDuringMint, "Mint amount exceed the maximum amount per mint");
        require(totalSupply() + _quantity <= collectionSize, "Purchase would reached max supply");
