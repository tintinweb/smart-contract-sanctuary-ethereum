...
contract ClaimNeuromod is Ownable, ReentrancyGuard {
    ...
    /**
     * @notice this is 4 because the first 4 are reserved for the Vault
     */
    uint256 public currentId = 4;
    ...
    function mintToVault(uint16[] memory mintedToVault) external onlyOwner {
        neuromod.mintBatch(vault, mintedToVault);
    }
    ...
    function claim(
        ...
        bytes32 node = keccak256(abi.encodePacked(msg.sender, _type));

        if (node != _leaf || !MerkleProof.verify(_merkleProof, merkleRoot, _leaf)) revert InvalidProof();
