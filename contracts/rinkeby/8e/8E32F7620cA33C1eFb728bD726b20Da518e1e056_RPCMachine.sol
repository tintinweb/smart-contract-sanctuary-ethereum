...
contract RPCMachine is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    uint256 constant saltGas = 179_980;
    address public saltSiger;
    uint256 public blockInterVal = 250;


    event Redeem(address indexed recipient, uint256[] ownerTokenIds, uint256 receiveAmount);
    ...
    event SetBlockInterVal(uint256 _interval);
    event UpdatePandaPool(uint256 updateMinPoolNftCount);
    event CommitRebron(address indexed sender,  uint64 block,uint256[] ownerTokenId, RequestInfo info, uint256 fee);
    event CommitBuy(address indexed sender, uint64 block, uint256 size, RequestInfo info, uint256 fee);
    event BuyRevealHash(address indexed sender, bytes32 revealHash, uint256[] indexs, uint256[] targetIndexs);
    event RebornRevealHash(address indexed sender, bytes32 revealHash, uint256[] indexs, uint256[] targetIndexs);
    ...
    function initNftMapping(uint256 startIdx, uint256 endIdx, uint256 baseIndex) external onlyOwner {
        for(uint256 i = startIdx; i < endIdx; i++) {
            poolNftMap[i] = i + baseIndex;
        }
    }

    function changeNftMapping(uint256 index, uint256 tokenId) external onlyOwner {
        poolNftMap[index] = tokenId;
    }

    function setSaltSiger(address _siger) external onlyOwner {
        saltSiger = _siger;
    }
...
//############################ Liquid Functions Start #####################
    ...
    // function C_buy(uint256 _amountBuyMax, uint256 _deadline, bytes32 _dataHash， bytes calldata signature) payable external ensure(_deadline)  nonReentrant crOpen callerIsUser {
        
    //     if(msg.value < fixedGasPrice * saltGas) {
    //         revert NoEnoughCallbackGas(msg.value, gasPrice * buyGas);
    //     }
    //     if (msg.value > fixedGasPrice * saltGas) {
    //         payable(msg.sender).transfer(msg.value - fixedGasPrice * saltGas);
    //     } 
    ...
    //                     )
    //                 ).recover(signature);
    //     if (saltSiger != recoverAds) {
    //         revert SignerAddressMismatch(saltSiger, recoverAds);
    //     }
    ...
    //     if (poolFeeValue != 0) {
    //         _pandaToken.safeTransferFrom(msg.sender, address(this), _price * (FEE_BASE + poolFeeValue) / FEE_BASE);
    //     }
        
    //     if (treasuryFeeValue != 0) {
    //         _pandaToken.safeTransferFrom(msg.sender, owner(), _price * treasuryFeeValue / FEE_BASE);
    //     }
    ...
    // function R_buy(bytes32 revealHash) external  nonReentrant callerIsUser {
    //     uint256 _requestId = buyMap[msg.sender];
    //     if (_requestId == 0) {
    //         revert RequestInvalid(msg.sender, _requestId, 0);
    //     }
    ...
    // function Save_buy(address user) external onlyOwner  {
    //     buyCommits[user].revealed=true;
    //     buyMap[user] = 0;
    //     bytes32 blockHash = blockhash(buyCommits[user].block);
    //     uint256 index = uint256(keccak256(abi.encodePacked(blockHash))) % poolSize;
    //     uint256 targetIndex = _getIdByIndex(index);
    //     IERC721 _pandaNFT = IERC721(pandaNFT);
    //     if (_pandaNFT.ownerOf(targetIndex) != address(this)) {
    //         revert NFTOwnerNotMatch(targetIndex,  address(this), _pandaNFT.ownerOf(targetIndex));
    //     }
    //     _pandaNFT.safeTransferFrom(address(this), user, targetIndex, "");
    //     _poolRemoveNft(index);
    //     emit BuyRevealHash(user, revealHash, index, targetIndex);
    // }

    // function C_reborn(uint256 _ownerTokenId, uint256 _amountFeeMax, uint256 _deadline, bytes32 _dataHash) payable external ensure(_deadline)  nonReentrant crOpen callerIsUser {
    ...
    //     if (poolFeeValue != 0) {
    //         _pandaToken.safeTransferFrom(msg.sender, address(this), _price * poolFeeValue / FEE_BASE);
    //     }
    ...
    //     emit CommitRebron(msg.sender, buyCommits[msg.sender].commit, buyCommits[msg.sender].block, _ownerTokenId, _price * feeValue / FEE_BASE, _deadline);
    // }

    // function R_reborn(bytes32 revealHash) external  nonReentrant callerIsUser {
    //     uint256 _requestId = rebornMap[msg.sender];
    //     if (_requestId == 0) {
    //         revert RequestInvalid(msg.sender, _requestId, 0);
    //     }
    ...
    // function Save_reborn(address user) external onlyOwner  {
    //     rebornCommits[user].revealed=true;
    //     rebornMap[user] = 0;
    //     bytes32 blockHash = blockhash(rebornCommits[user].block);
    //     uint256 index = uint256(keccak256(abi.encodePacked(blockHash))) % poolSize;
    //     uint256 targetIndex = _getIdByIndex(index);
    //     IERC721 _pandaNFT = IERC721(pandaNFT);
    //     if (_pandaNFT.ownerOf(targetIndex) != address(this)) {
     ...
     * @param _ownerTokenIds The NFT Id you want to redeem
     * @param _amountRedeemMin  Less than this value you will not redeem
     * @param _deadline deadline
     */
    function Mul_redeem(uint256[] calldata _ownerTokenIds, uint256 _amountRedeemMin, uint256 _deadline) external ensure(_deadline) nonReentrant poolOpen {
        uint256 size = _ownerTokenIds.length;
        uint256 _price = _currentPrice();
        uint256 feeValue = size * _price * (FEE_BASE - poolFeeValue - treasuryFeeValue) / FEE_BASE;
        if (_amountRedeemMin > feeValue) {
            revert MinReceiveNotMatch(_amountRedeemMin, feeValue);
        }
        IERC721 _pandaNFT = IERC721(pandaNFT);
        uint256 _ownerTokenId;
        for (uint256 i = 0; i < size; i++) {
            _ownerTokenId = _ownerTokenIds[i];
            if (_pandaNFT.ownerOf(_ownerTokenId) != msg.sender) {
                revert NFTOwnerNotMatch(_ownerTokenId,  msg.sender, _pandaNFT.ownerOf(_ownerTokenId));
            }
        ...
        emit Redeem(msg.sender, _ownerTokenIds, feeValue);
    }
...
//########################### Liquid Functions End ####################
    ...
    function MulC_buy(RequestInfo calldata info, uint256 size) payable external ensure(info.deadline)  nonReentrant crOpen callerIsUser {
    
        uint256 needGas = fixedGasPrice * saltGas * size;
        if(msg.value < needGas) {
            revert NoEnoughCallbackGas(msg.value, needGas);
        }
        if (msg.value > needGas) {
            payable(msg.sender).transfer(msg.value - needGas);
        } 
        address recoverAds = keccak256(
                        abi.encodePacked(
                            "\x19Ethereum Signed Message:\n32",
        ...
        buyCommits[msg.sender].commit = info.dataHash;
        buyCommits[msg.sender].block = uint64(block.number);
        buyCommits[msg.sender].revealed = false;
        buyCommits[msg.sender].amountBuyMax = info.amountFeeMax;
        buyCommits[msg.sender].size = size;

        uint256 feeValue = poolFeeValue + treasuryFeeValue;
        uint256 _price = _currentPrice();
        uint256 buyValue = _price * size * (FEE_BASE + feeValue) / FEE_BASE;
        if (info.amountFeeMax < buyValue) {
            revert MaxBuyNotMatch(info.amountFeeMax, buyValue);
        }
        IERC20 _pandaToken = IERC20(pandaToken);
        if (_pandaToken.balanceOf(msg.sender) < buyValue) {
            revert PandaNotEnough(msg.sender, _pandaToken.balanceOf(msg.sender), buyValue);
        }

        if (poolFeeValue != 0) {
            _pandaToken.safeTransferFrom(msg.sender, address(this), _price * size *(FEE_BASE + poolFeeValue) / FEE_BASE);
        }
        
        if (treasuryFeeValue != 0) {
            _pandaToken.safeTransferFrom(msg.sender, owner(), _price * size * treasuryFeeValue / FEE_BASE);
        }

        IERC721 _pandaNFT = IERC721(pandaNFT);
        if(_pandaNFT.balanceOf(address(this)) - size <= minPoolNFTCount) {
            revert PoolNotEnoughNFT(_pandaNFT.balanceOf(address(this)), minPoolNFTCount);
        }

        buyMap[msg.sender] = 1;
        emit CommitBuy(msg.sender, uint64(block.number), size, info, buyValue);
    }

    function MulR_buy(bytes32 revealHash) external  nonReentrant callerIsUser {
        uint256 _requestId = buyMap[msg.sender];
        uint256 size = buyCommits[msg.sender].size;
        ...
        uint64 commitBlock = buyCommits[msg.sender].block;
        bytes32 commit = buyCommits[msg.sender].commit;

        if (getHash(revealHash) != commit) {
            revert RevealedNotMatchError(getHash(revealHash), commit);
        }

        if (block.number <= commitBlock) {
            revert RevealHappenEarly(uint64(block.number), commitBlock);
        }
        
        if (block.number > commitBlock + blockInterVal) {
            revert RevealTooLate(block.number, commitBlock + blockInterVal);
        }
        ...
        //hash that with their reveal that so miner shouldn't know and mod it with some max number you want
        uint256 index = uint256(keccak256(abi.encodePacked(blockHash,revealHash))) % poolSize;
        uint256 random = uint256(keccak256(abi.encodePacked(index))) % poolSize;
        uint256[] memory idxs = new uint256[](size);
        uint256[] memory tokenIds = new uint256[](size);
        uint256 targetIndex;
        for (uint i = 0; i < size; i++) {
            index = (index + random) % poolSize;
            targetIndex = _getIdByIndex(index);
            idxs[i] = index;
            tokenIds[i] = targetIndex;
            if (_pandaNFT.ownerOf(targetIndex) != address(this)) {
                revert NFTOwnerNotMatch(targetIndex,  address(this), _pandaNFT.ownerOf(targetIndex));
            }
            _pandaNFT.safeTransferFrom(address(this), msg.sender, targetIndex, "");
        ...
        buyMap[msg.sender] = 0;
        emit BuyRevealHash(msg.sender, revealHash, idxs, tokenIds);
    }
    ...
    function Save_buy(address user) external onlyOwner  {
        ...
        buyMap[user] = 0;
        bytes32 blockHash = blockhash(buyCommits[user].block);
        uint256 size = buyCommits[user].size;
        ...
        emit BuyRevealHash(user, blockHash, idxs, tokenIds);
    }

    function MulC_reborn(uint256[] calldata _ownerTokenIds, RequestInfo calldata info) payable external ensure(info.deadline)  nonReentrant crOpen callerIsUser {
        uint256 randomGas = fixedGasPrice * saltGas * _ownerTokenIds.length;
        if(msg.value < randomGas) {
            revert NoEnoughCallbackGas(msg.value, randomGas);
        }
        if (msg.value > randomGas) {
            payable(msg.sender).transfer(msg.value - randomGas);
        } 
        address recoverAds = keccak256(
                        abi.encodePacked(
                            "\x19Ethereum Signed Message:\n32",
        ...
        IERC20 _pandaToken = IERC20(pandaToken); 
        uint256 _price = _currentPrice();
        uint256 feeValue = (poolFeeValue + treasuryFeeValue) * _price * _ownerTokenIds.length / FEE_BASE;
        if (info.amountFeeMax < feeValue) {
            revert MaxFeeNotMatch(info.amountFeeMax, feeValue);
        }
        if (_pandaToken.balanceOf(msg.sender) < feeValue) {
            revert PandaNotEnough(msg.sender, _pandaToken.balanceOf(msg.sender), feeValue);
        }

        if (poolFeeValue != 0) {
            _pandaToken.safeTransferFrom(msg.sender, address(this), _ownerTokenIds.length * _price * poolFeeValue / FEE_BASE);
        }

        if (treasuryFeeValue  != 0) {
            _pandaToken.safeTransferFrom(msg.sender, owner(), _ownerTokenIds.length * _ownerTokenIds.length * treasuryFeeValue / FEE_BASE);
        }
        ...
        for (uint256 i = 0; i < _ownerTokenIds.length; i++) {
            if (_pandaNFT.ownerOf(_ownerTokenIds[i]) != msg.sender) {
                revert NFTOwnerNotMatch(_ownerTokenIds[i], msg.sender, _pandaNFT.ownerOf(_ownerTokenIds[i]));
            }

            _pandaNFT.safeTransferFrom(msg.sender, address(this), _ownerTokenIds[i], "");
            _poolPushNft(_ownerTokenIds[i]);
        }
        

        rebornCommits[msg.sender].commit = info.dataHash;
        rebornCommits[msg.sender].block = uint64(block.number);
        rebornCommits[msg.sender].revealed = false;
        rebornCommits[msg.sender].amountFeeMax = info.amountFeeMax;
        rebornCommits[msg.sender].size = _ownerTokenIds.length;
        rebornMap[msg.sender] = 1;
        emit CommitRebron(msg.sender,  uint64(block.number), _ownerTokenIds, info, feeValue);
    }

    function MulR_reborn(bytes32 revealHash) external  nonReentrant callerIsUser {
        uint256 size = rebornCommits[msg.sender].size;
        uint256 _requestId = rebornMap[msg.sender];
        if (_requestId == 0) {
            revert RequestInvalid(msg.sender, _requestId, 0);
        }
        ...
        rebornCommits[msg.sender].revealed=true;
        uint64 commitBlock = rebornCommits[msg.sender].block;
        bytes32 commit = rebornCommits[msg.sender].commit;
        if (getHash(revealHash) != commit) {
            revert RevealedNotMatchError(getHash(revealHash), commit);
        }

        if (block.number <= commitBlock) {
            revert RevealHappenEarly(block.number, commitBlock);
        }
        ...
        rebornMap[msg.sender] = 0;
        emit RebornRevealHash(msg.sender, revealHash, idxs, tokenIds);
    }
    ...
    function Save_reborn(address user) external onlyOwner  {
        ...
        emit RebornRevealHash(user, blockHash, idxs, tokenIds);
    }
//########################### Pool Utils Functions Start ####################

    function _poolSize() public view returns(uint256) {
        return poolSize;
    }

    function _minPoolNFTCount() public view returns(uint256) {
        return minPoolNFTCount;
    }

    function _poolRemoveNft(uint256 _index) public onlyAdmin {
        if (_index >= poolSize) {
    ...
    function _poolPushNft(uint256 _value) public onlyAdmin {
     ...
     */
    function _currentPrice() public view returns(uint256 _price) {
...
//########################### Pool Utils Functions End ####################
    ...
    function setBlockInterVal(uint256 _interval) external onlyOwner {
        blockInterVal = _interval;
        emit SetBlockInterVal(_interval);
    }
    ...
    function recover(bytes32 _ethSignedMessageHash, bytes memory signature) public view returns (address){
        return _ethSignedMessageHash.recover(signature);
    }
