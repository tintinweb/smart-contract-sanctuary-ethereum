...
contract AveragePFPsTESTGAS8 is ERC721, Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    function _mintLoop(address _receiver, uint256[] memory _mintArray) internal {
      uint256 i = 0;
      uint256 runX = _avgcreatures.balanceOf(msg.sender);
      for (i; i < runX; i++) {
        uint256 currentToken = _mintArray[i];
        if (!isClaimed(currentToken)) {
          _setClaimed(currentToken);
          supply.increment();
          _safeMint(_receiver, currentToken);
        }
      }
    }
