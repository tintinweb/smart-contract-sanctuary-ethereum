...
/* TODOs:
    * Allow only 3 such traits per WALLET ADDRESS
    * Disallow selling these until we turn it ON
*/
...
contract czSpecialTraits is iczSpecialEditionTraits, ERC721Enumerable, IERC2981, Ownable, Pausable, ReentrancyGuard {
    ...
    constructor() ERC721("CyberZillaz Special Edition Traits", "CZ-SET") {
        ...
        uint16 traitId = 1;
        traits[traitId] = Trait({ traidId: traitId, price: 0.1 ether, maxMint: 10, minted: 0 });
        traitId = 2;
        traits[traitId] = Trait({ traidId: traitId, price: 0.2 ether, maxMint: 10, minted: 0 });
        traitId = 3;
        traits[traitId] = Trait({ traidId: traitId, price: 0.15 ether, maxMint: 10, minted: 0 });
        traitId = 4;
        traits[traitId] = Trait({ traidId: traitId, price: 0.09 ether, maxMint: 10, minted: 0 });
    }
    ...
    uint256 private _royaltyPermille = 69;
    ...
    // TODO: Test if this can ONLY be called by the owner of the NFT to be sure
    function burn(uint16 tokenId) external override whenNotPaused nonReentrant {
        // TODO: Test this well! Admins and Owners can burn! No one else.
        if (!_admins[_msgSender()]) {
            require(ownerOf(tokenId) == _msgSender(), "SET: Not allowed");
        }
        
        Token memory _token = _getToken(tokenId);
        Trait storage _trait = _getTraitStorage(_token.traitId);
    ...
    function royaltyInfo(uint256 tokenId, uint256 salePrice) external view override returns (address receiver, uint256 royaltyAmount){
        return (owner(), salePrice * _royaltyPermille/1000);
    }
    ...
    function _getTokenStorage(uint16 tokenId) private view returns (Token storage token) {
        return tokens[tokenId];
    }
