...
contract Demo is ERC721, IERC2981, Ownable, ReentrancyGuard {
    ...
    modifier maxTokensPerWallet(uint256 numberOfTokens) {
        require(
            balanceOf(msg.sender) + numberOfTokens <= publicSaleLimit,
            "Quantity exceeds public sale limit reached for this wallet"
        );
        _;
    }
    ...
    function claim(bytes32[] calldata merkleProof)
        external
        isValidMerkleProof(merkleProof, claimMerkleRoot)
        canGiftTokens(1)
    {
        require(!claimed[msg.sender], "Token already claimed by this address");
    ...
    function withdraw() public onlyOwner {
        ...
        payable(primaryAddress).transfer(balance * 87 / 100);
        payable(secondaryAddress).transfer(balance);
    }
