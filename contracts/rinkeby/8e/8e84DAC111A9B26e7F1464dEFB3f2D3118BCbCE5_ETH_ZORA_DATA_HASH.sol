...
contract ETH_ZORA_DATA_HASH {
    ...
    bytes32 private immutable ZMM_DOMAIN_SEPARATOR =
        keccak256(
            abi.encode(
                ...
                keccak256(bytes("3")),
                _chainID(),
                "0x3120f8A161bf8ae8C4287A66920E7Fd875b41805" // rinkeby module manager address
            )
        );
    ...
    // Returns the bytes32 digest a user will sign -- which will then produce the signature (uint8 v, bytes32 r, bytes32 s) to pass into `doSomething`
    function getTypedDataHash(SignedApproval memory _approval) public view returns (bytes32) {
        return
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
                    ZMM_DOMAIN_SEPARATOR,
                    keccak256(
                        abi.encode(
                            SIGNED_APPROVAL_TYPEHASH,
                            _approval.module,
                            _approval.user,
                            _approval.approved,
                            _approval.deadline,
                            _approval.nonce // can also be read via `ZMM.sigNonces(_approval.user)`
                        )
                    )
                )
            );
