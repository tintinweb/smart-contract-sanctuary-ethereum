...
// File: n_0503/interface/strings.sol
...
// File: n_0503/interface/UserInterface.sol
...
contract interface_0528 is ERC721{
    using strings for *;
...
//metadata
    ...
    mapping(uint256 => mapping(uint256 => string)) uintTo_StringNumToStringLayerName;
    ...
    m_0528 mainContract = m_0528(0xA4369F6909156726dcfe41a17dF63899Eb0e0932);
    
    constructor() ERC721("interface_0528" , "interface_0528" ) {
    }
    //区切り文字の指定
    ...
    function addLayers(string memory _layerInfo)external onlyOwner{
        ...
        for (uint i = 0; i < parts.length ; i++) {
            ...
            uintTo_StringNumToStringLayerName[str2uint(layer)][layerCount[str2uint(layer)]] = parts[i].split( "/".toSlice() ).toString();
            layerCount[str2uint(layer)]++;
        }
    }
    ...
    function getLayersInfo(uint256 _layer)public view returns(string memory,uint){
        ...
        for(uint i = 0;i<layerCount[_layer];i++){
            out = out.toSlice().concat(uint2str(i).toSlice()).toSlice().concat(":".toSlice()).toSlice().concat(uintTo_StringNumToStringLayerName[_layer][i].toSlice()).toSlice().concat("|".toSlice()) ;
    ...
    function sendMint(string memory _layerInfo,string memory _onlyLayerInfo)public payable returns(string memory){
        //m_ mainContract = m_(0x2711334f66F9F075028727793EA17c6fFe5FAF7C);
        require(mintedCount < limit);
        require(mainContract.getIsExist(_onlyLayerInfo));
        ...
        require(mintable);
        require(missList[_msgSender()] < 0);
        require(msg.value == in_price);
        
        //24519134421 区切り文字
        string[] memory parts = new string[](_layerInfo.toSlice().count(delimSTR.toSlice()) +1 );
        //layer 1~ 8 + imageipfs= 9
        require(parts.length == 9);

        for (uint i = 0; i < 9 ; i++) {
            parts[i] = _layerInfo.toSlice().split( delimSTR.toSlice() ).toString();
            if(strings.equals( uintTo_StringNumToStringLayerName[i][str2uint(parts[i])].toSlice() ,"".toSlice() ) && i < 8){
                missList[_msgSender()]++;
                return "mint Failed by input miss";
            }
        }
                // require( !strings.equals( uintTo_StringNumToStringLayerName[0][str2uint(parts[0])].toSlice() ,"".toSlice() ) );
                // require( !strings.equals( uintTo_StringNumToStringLayerName[1][str2uint(parts[1])].toSlice() ,"".toSlice() ) );
                // require( !strings.equals( uintTo_StringNumToStringLayerName[2][str2uint(parts[2])].toSlice() ,"".toSlice() ) );
                // require( !strings.equals( uintTo_StringNumToStringLayerName[3][str2uint(parts[3])].toSlice() ,"".toSlice() ) );
                // require( !strings.equals( uintTo_StringNumToStringLayerName[4][str2uint(parts[4])].toSlice() ,"".toSlice() ) );
                // require( !strings.equals( uintTo_StringNumToStringLayerName[5][str2uint(parts[5])].toSlice() ,"".toSlice() ) );
                // require( !strings.equals( uintTo_StringNumToStringLayerName[6][str2uint(parts[6])].toSlice() ,"".toSlice() ) );
                // require( !strings.equals( uintTo_StringNumToStringLayerName[7][str2uint(parts[7])].toSlice() ,"".toSlice() ) );
        
        //mintを呼び出す
        //mintできたかどうかをboolで返す
        bool minted = mainContract._inMint(_msgSender(),parts[8],parts[0],parts[1],parts[2],parts[3],parts[4],parts[5],parts[6],parts[7]);
        if(minted && allowlistMintStarted && allowlist[_msgSender()].remaining > 0 ){
