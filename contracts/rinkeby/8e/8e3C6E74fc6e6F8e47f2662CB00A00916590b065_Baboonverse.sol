...
contract Baboonverse is ERC721A, Ownable, VRFConsumerBase, ReentrancyGuard {
    ...
    string public constant PROVENANCE_HASH_RARITY = "63b3ac3d0dd86ffc4b5467eda8778b4d7b62aa93c5979fef365ae3afdd67ad2f"; 

    // Price
    uint256 public constant MINT_PRICE = 0.08 ether;
    uint256 public constant WHITELIST_MINT_PRICE = 0.068 ether;

    // Supply
    uint256 public constant INITIAL_SUPPLY = 5888;
    uint256 public constant MINIMUM_SUPPLY = 1000;
    ...
    address public multiSigTreasuryWallet;
    ...
    {
        // Wallet for Roadmap
        multiSigTreasuryWallet = treasuryWallet_;

        // Project Variables
        merkleRoot = merkleRoot_;
        baseTokenURI = "https://baboonverse-unrevealed.s3.amazonaws.com/metadata/";
        
        // Whitelist
        maxPerWalletWhitelist = 3;
        maxPerTxnWhitelist = 3;
    ...
    function devTeamMint(uint256 quantity_, bytes32[] memory proof_) external isUser isWhitelistMint isSaleActive {
        ...
        require(totalSupply() < maxSupply, "Sold out");
        require(numberMinted(msg.sender) <= 3, "Max per wallet reached");
        require(quantity_ <= 3, "Max per txn exceeded");
        require(maxSupply >= totalSupply() + quantity_, "Max supply exceeded");
        
        _safeMint(msg.sender, quantity_);
    }
    function baboonfrensMint(bytes32[] memory proof_) external isUser isPublicMint isSaleActive {
        uint256 maxSupply = currentMaxSupply();
    ...
    modifier isSaleActive {
        require(saleActive == true && block.timestamp >= saleStartTime, "Sale not active"); _; }
    ...
    function emergencyPauseSale() external onlyOwner {
        ...
        uint256 alreadyElapsedTime = block.timestamp - saleStartTime + prevElapsedTime;
        uint256 alreadyBurnedSupply = INITIAL_SUPPLY - currentMaxSupply();
        prevElapsedTime = prevElapsedTime + alreadyElapsedTime;
        prevBurnedSupply = prevBurnedSupply + alreadyBurnedSupply;
    ...
    function currentMaxSupply() public view returns (uint256) {
        ...
        // Initial after paused
        if (saleActive == true && block.timestamp <= saleStartTime){
            return INITIAL_SUPPLY - prevBurnedSupply;
        }

        uint256 timeElapsed = block.timestamp - saleStartTime + prevElapsedTime;

        if (timeElapsed > 60 minutes) {
            uint256 timeLeft = timeElapsed - 60 minutes;

            uint256 alreadyDecreased = 60 minutes * 802222 * 1e12;
            uint256 leftSupply = INITIAL_SUPPLY - alreadyDecreased;

            uint256 maximumDecrease = (leftSupply - MINIMUM_SUPPLY) * 1e18;
            
            uint256 fractionTimeLeft = timeLeft * 1e18 / 47 hours;

            uint256 decreasedAmount = (leftSupply - MINIMUM_SUPPLY) * fractionTimeLeft;

            if (decreasedAmount > maximumDecrease) {
                return MINIMUM_SUPPLY;
            } else {
                return (leftSupply - (decreasedAmount / 1e18));
            }
        }
        else {
            uint256 decreasedAmount = timeElapsed * 802222 * 1e12;
    ...
    function withdrawMoney() external onlyOwner {
        uint256 balanceForTreasury = address(this).balance * 300 / 1000; // 30% for Treasury
        uint256 remainingBalance = address(this).balance - balanceForTreasury;

        withdraw(payable(multiSigTreasuryWallet), balanceForTreasury);
        withdraw(payable(msg.sender), remainingBalance);
    }
