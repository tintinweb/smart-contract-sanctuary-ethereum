...
contract MetaMiniYouthClubNFT is ERC721Enumerable, Ownable, ReentrancyGuard{
    ...
    */
    uint256 constant MAX_GIVEAWAY = 600;
    uint256 constant MAX_ELEMENTS = 6400;
    ...
    mapping(address => uint) private _pubsaleCountList;

    // giveaway list
    address[] giveawayList;
    
    // keep track of those on whitelist who have  their NFT
    ...
    */
    ...
    modifier canPresaleMint(uint256 numberOfTokens) {
        require(
            _presaleCountList[msg.sender] + numberOfTokens <= PRESALE_MAX_PUBLIC_COUNT,
    ...
    modifier canPubsaleMint(uint256 numberOfTokens) {
        require(
            _pubsaleCountList[msg.sender] + numberOfTokens <= PUBSALE_MAX_PUBLIC_COUNT,
            "Overflow max presale minting tokens"
        );
        _;
    }
    ...
    */
        ...
        nonReentrant
    {
        ...
        require(super.totalSupply() + mintCount <= MAX_ELEMENTS, "Overflow Max Supply");
    ...
    */
    function mintGift(
        ...
        nonReentrant
    {
      require(_giftTokenCount + 1 <= MAX_GIVEAWAY, "Overflow max gift token");
      _giftTokenCount++;
      _tokenIdTracker.increment();
      _mint(msg.sender, _tokenIdTracker.current());
    ...
    function ownerMint() public onlyOwner {
        require(super.totalSupply() + 1 < MAX_GIVEAWAY + MAX_ELEMENTS, "Maximum supply reached.");
        _tokenIdTracker.increment();
        _mint(msg.sender, _tokenIdTracker.current());
    }
    ...
    */
    ...
    function isClaimAvailable(bytes32[] calldata merkleProof) public view returns(uint256) {
        if 
        ( 
            MerkleProof.verify(
                merkleProof,
