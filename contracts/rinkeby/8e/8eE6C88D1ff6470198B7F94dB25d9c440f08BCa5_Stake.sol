...
contract Stake is Initializable, ContextUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address _depositTokens, address _rewardToken, uint256 _rewardInterval) public initializer(
    ) {
    ...
    function getPendingDivs(address _holder) public view returns (uint256) {
        ...
        if(stakedAmount > 100 ether){
            rate = rate.add(200);
        }else if(stakedAmount > 500 ether){
            rate = rate.add(500);
        }else if(stakedAmount > 1000 ether){
            rate = rate.add(1000);
        }
