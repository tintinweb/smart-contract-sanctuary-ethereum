...
contract Hook {
    ...
    uint16 public constant size = 60; 

    address public lock;
    uint[size*size] public tokenIds; // based on position yield token id
    // positions:   0    1     2     3     4   
    // tokenId:     56   54    0     9     121

    uint[size*size] public positions; // based on token id yield position

    // store whatever was already minted
    uint[] public alreadyMinted;

    function already() public view returns (uint[] memory) {
    ...
    function onKeyPurchase(
      ...
      // Assign mapping or fail!
      if (msg.sender != lock) {
        revert ONLY_LOCK();
      }
      
      IPublicLockV10 lockContract = IPublicLockV10(lock);
      uint tokenId = lockContract.totalSupply();
      uint position = toUint256(data);
      if (tokenIds[position] > 0) {
        revert ALREADY_TAKEN();
      }
      tokenIds[position] = tokenId;
      positions[tokenId-1] = position;
      alreadyMinted.push(tokenId);
    }

    /**
     * Helper function to yield coordinates for position
     */
    function coordinate(uint position) public view returns (uint, uint) {
      return (position/size, position % size);
    }

    /**
     * Helper function to yield coordinates for token id
     */
    function coordinateForTokenId(uint tokenId) public view returns (uint, uint) {
      uint position = positions[tokenId-1];
      return (position/size, position % size);
    }

    /**
     * Helper function to build svg
     */
    function buildImageSvg(uint x, uint y) internal view returns (bytes memory) {
      IPublicLockV10 lockContract = IPublicLockV10(lock);
      bool[9] memory neighbors;
      ...
      if (x == 0 || y == 0) {
        ...
        uint p = (x-1) * size + y - 1;
        neighbors[0] = lockContract.isValidKey(tokenIds[p]);
      }
      ...
      if (x == 0) {
        ...
        neighbors[1] = lockContract.isValidKey(tokenIds[(x-1) * size + y]);
      }
      ...
      if (x == 0 || y == size) {
        ...
        neighbors[2] = lockContract.isValidKey(tokenIds[(x-1) * size + y + 1]);
      }
      ...
      if (y == 0) {
        ...
        neighbors[3] = lockContract.isValidKey(tokenIds[x * size + y - 1]);
      }

      // c
      neighbors[4] = lockContract.isValidKey(tokenIds[(x) * size + y]);

      // w
      if (y == size) {
        neighbors[5] = false;
      } else {
        neighbors[5] = lockContract.isValidKey(tokenIds[x * size + y + 1]);
      }
      ...
      if (x == size || y == 0) {
        ...
        neighbors[6] = lockContract.isValidKey(tokenIds[(x + 1) * size + y - 1]);
      }
      ...
      if (x == size) {
        ...
        neighbors[7] = lockContract.isValidKey(tokenIds[(x + 1) * size + y]);
      }
      ...
      if (x == size || y == size) {
        ...
        neighbors[8] = lockContract.isValidKey(tokenIds[(x + 1) * size + y + 1]);
      }


      return  abi.encodePacked(
        '<svg viewBox=\"0 0 30 30\" xmlns=\"http://www.w3.org/2000/svg\">',
        '<rect width=\"10\" height=\"10\" x=\"0\" y=\"0\" fill=\"', neighbors[0] ? 'black' : 'WhiteSmoke' ,'\" />',
        '<rect width=\"10\" height=\"10\" x=\"10\" y=\"0\" fill=\"', neighbors[1] ? 'black' : 'WhiteSmoke' ,'\" />',
        '<rect width=\"10\" height=\"10\" x=\"20\" y=\"0\" fill=\"', neighbors[2]  ? 'black' : 'WhiteSmoke' ,'\" />',
        '<rect width=\"10\" height=\"10\" x=\"0\" y=\"10\" fill=\"', neighbors[3] ? 'black' : 'WhiteSmoke' ,'\" />',
        '<rect width=\"10\" height=\"10\" x=\"10\" y=\"10\" fill=\"', neighbors[4] ? 'black' : 'WhiteSmoke' ,'\" />',
        '<rect width=\"10\" height=\"10\" x=\"20\" y=\"10\" fill=\"', neighbors[5] ? 'black' : 'WhiteSmoke' ,'\" />',
        '<rect width=\"10\" height=\"10\" x=\"0\" y=\"20\" fill=\"', neighbors[6]  ? 'black' : 'WhiteSmoke' ,'\" />',
        '<rect width=\"10\" height=\"10\" x=\"10\" y=\"20\" fill=\"', neighbors[7] ? 'black' : 'WhiteSmoke' ,'\" />',
        '<rect width=\"10\" height=\"10\" x=\"20\" y=\"20\" fill=\"', neighbors[8]  ? 'black' : 'WhiteSmoke' ,'\" />',
        '</svg>'
      );
    ...
    /**
     * Building the global SVG
     */
    function gameOfLife() public view returns (bytes memory) {
      return abi.encodePacked(
        'function view(e,t,l,o=0,i=0){let st=false,rs=0;document.onclick=()=> st=true;const n=20,r=requestAnimationFrame,f="setAttribute",c=document.getElementById("svg");c[f]("viewBox",[0,0,e*n+1,t*n+1]),c[f]("stroke","lightgray"),c.style.display="block";let s,a=new Array(e*t).fill(0),d=a.slice();for(let o=0;o<t;++o)for(let t=0;t<e;++t)a[o*e+t]=l[o*e+t];for(let l=0;l<t;++l)for(let t=0;t<e;++t){const e=c.appendChild(document.createElementNS("http://www.w3.org/2000/svg","rect"));e[f]("width",n),e[f]("height",n),e[f]("x",t*n),e[f]("y",l*n),e.onclick=()=>window.gol && window.gol(l*64+t)}let w=0,h=0;const u=()=>{if(rs>0 && !st) return setTimeout(u, 100);rs+=1;if(s=!1,o||i){let l=Date.now();r((function s(){const a=(Date.now()-l)/100;for(let l=0,r=0;l<t;++l){const s=(l+h+i*a)%t;for(let l=0;l<e;++l,++r){const i=(l+w+o*a)%e,d=c.childNodes[r];d[f]("x",(i<0?i+e:i)*n),d[f]("y",(s<0?s+t:s)*n)}}a<1&&r(s)}))}w+=o,h+=i;for(let l=0;l<t;++l)for(let n=0;n<e;++n){let r=l*e+n,w=a[r],h=-w;for(let r=-1;r<=1;++r){let f=l+r;if(i)0>f?f=t-1:f>=t&&(f=0);else{if(0>f)continue;if(f>=t)break}for(let t=-1;t<=1;++t){let l=n+t;if(o)0>l?l=e-1:l>=e&&(l=0);else{if(0>l)continue;if(l>=e)break}h+=a[f*e+l]}}let u=+(1===w?2<=h&&h<=3:3===h);w!==u&&(s=!0),d[l*e+n]=u,c.childNodes[r][f]("fill",w?"black":"white")}[a,d]=[d,a],s&&setTimeout(u,100)};u()}',
        'view(32,32,',getAllPositions(),'.map(c => BigInt(c).toString(2).padStart(256, "0").split("").reverse()).flat().map(d => parseInt(d)))');
    }
    ...
    function tokenURI(
        ...
        address owner, //owner,
        uint256 tokenId,
        uint256 //  a cool trick could be to render based on how far the expiration of the key is!
      ...
      bool isAlive = lockContract.isValidKey(tokenId);

      (uint x, uint y) = coordinateForTokenId(tokenId);
    
      bytes memory svg = buildImageSvg(x,y);

      bytes memory image = abi.encodePacked(
          "data:image/svg+xml;base64,",
          Base64.encode(svg));

      bytes memory js = gameOfLife();

      bytes memory animation_url = 
        abi.encodePacked(
          "data:text/html;base64,",Base64.encode(
            abi.encodePacked('<html><svg id="svg" xmlns="http://www.w3.org/2000/svg" style="position: absolute; top: 0; left: 0"></svg><script>', js, '</script>')
          )
        );
      ...
      bytes memory json = 
        abi.encodePacked(
          '{', 
            '"name": "Game of Life Cell ', Strings.toString(x), ',', Strings.toString(y),'",',
            '"description": "Game of Life is a multiplayer game where players can enable or disable cells to change the initial state of a 32x32 grid in Conway\'s Game of Life.",'
            '"external_url": "TODO",', // IPFS? 
            '"image": "', image, '", ',
            '"animation_url": "', animation_url, '", ',
            '"attributes": [',
              '{"trait_type": "alive", "value": ', isAlive ? 'true' : 'false','},'
              '{"trait_type": "x", "value": ', Strings.toString(x), '},',  
              '{"trait_type": "y", "value": ', Strings.toString(y),'}',
            ']',
          '}'
        );
    ...
    // The function that retrieves the state of all cells!
    function getAllPositions() public view returns (string memory) {
      IPublicLockV10 lockContract = IPublicLockV10(lock);
      
      // stringified using DynamicBuffer
      (, bytes memory coords) = DynamicBuffer.allocate(1000);
      DynamicBuffer.appendBytes(coords, bytes(
        abi.encodePacked('[')
      ));
      ...
      for (uint i = 0; i < size * size; i++) {
        if(lockContract.isValidKey(tokenIds[i])) {
          vect = vect | ONE << i % 256;
        }
        if (i % 256 == 0) {
          DynamicBuffer.appendBytes(coords, 
            abi.encodePacked(
              '"',
              Strings.toString(vect),
              '"',
              i > (size - 1)*size ? '' : ',' 
            )
          );
      ...
      DynamicBuffer.appendBytes(coords, bytes(abi.encodePacked(']')));
