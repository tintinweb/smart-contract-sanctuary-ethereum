...
contract MyLastNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public constant RESERVED_SUPPLY = 9; // 942;
    uint256 public MAX_SUPPLY = 19; //6942;
    uint256 public constant MAX_PER_TX = 10;
    uint256 public freeTokensMinted = 0;
    bytes32 public freeMintMerkleRoot;
    bytes32 public almostFreeMintMerkleRoot;
    ...
    constructor() ERC721("MY LAST NFT", "MLN") {}

    function mint(uint256 count) external payable nonReentrant {
        require(isPublic, "Wait for public sale");
        require(count <= MAX_PER_TX, "Max per transaction is 10");
        uint256 supply = _owners.length;
        require(supply + count < MAX_SUPPLY + 2, "Max supply reached");
    ...
    function presaleMint(
        ...
        if (rootType == 1) allowed = _verify(leaf, proof, freeMintMerkleRoot);
        else allowed = _verify(leaf, proof, almostFreeMintMerkleRoot);

        require(allowed, "Invalid proof");
        require(
            addressToMinted[msg.sender] + count <= allowance,
            "Exceeds your allowance"
        );

        bool hasFreeMint = rootType == 1 && addressToMinted[msg.sender] == 0;
        if (hasFreeMint) freeTokensMinted++;

        addressToMinted[msg.sender] += count;
        uint256 supply = _owners.length;
        uint256 maxSupply = hasFreeMint
            ? MAX_SUPPLY - (RESERVED_SUPPLY - freeTokensMinted)
            : MAX_SUPPLY - RESERVED_SUPPLY;

        if (hasFreeMint && supply + 1 < maxSupply + 2)
            require(supply + count < maxSupply + 2, "Only free mints left");
        else require(supply + count < maxSupply + 2, "Max supply reached");
    ...
    function setAlmostFreeMintMerkleRoot(bytes32 _merkleRoot)
        external
        onlyOwner
    {
        almostFreeMintMerkleRoot = _merkleRoot;
    }
    ...
    function burn(uint256 tokenId) external {
        require(
            (!isPublic && !isPresale) && isBurningAllowed,
            "Burning not authorized"
        );
