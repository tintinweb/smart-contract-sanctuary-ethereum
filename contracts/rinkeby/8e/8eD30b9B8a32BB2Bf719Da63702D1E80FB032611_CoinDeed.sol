...
contract CoinDeed is ICoinDeed, AccessControl {
    ...
    function executeRiskMitigation() external payable override onlyInState(DeedState.OPEN) {
        uint8 currentLeverage = riskMitigationTriggered ? riskMitigation.leverage : deedParameters.leverage;
        require(currentLeverage > 1, "BAD_RISK_MITIGATION_TRIGGER");
        ...
        (uint256 sellAmount, uint256 buyAmount) = coinDeedAddressesProvider.checkRiskMitigationAndGetSellAmount(
            pair, deedParameters, riskMitigation, riskMitigationTriggered);

        // Calculate liquidator discount
        uint256 maxDiscountPercentage = ICoinDeedFactory(coinDeedAddressesProvider.coinDeedFactory())
            .liquidationBonus() / BASE_DENOMINATOR;
        uint256 discount;

        if (!riskMitigationTriggered) {
            discount = buyAmount * maxDiscountPercentage;
        }
        // Caculate second liquidation time discount
            ...
            if (buyAmount > totalBorrowAmount) {
                ...
                discount = buyAmount * discountPercentage;
            }
            else {
                discount = 0;
            }
        ...
        if(pair.tokenA == address(0x00)) {
            ...
            if (pair.tokenA == USDT_ADDRESS) {
                IERC20(pair.tokenA).safeApprove(address(this), 0);
            }
            IERC20(pair.tokenA).safeApprove(address(this), buyAmount - discount);
        ...
        coinDeedAddressesProvider.withdrawFromPool(pair.tokenB, sellAmount);
