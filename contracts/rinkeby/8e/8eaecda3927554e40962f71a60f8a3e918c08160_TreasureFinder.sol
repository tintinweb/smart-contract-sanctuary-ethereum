...
contract TreasureFinder is BoringOwnable {
    ...
    function hideTokens(uint amounfFLAME) internal {
        ...
        uint amountLEASHtoHide = _swap(weth, dona, ethForDona, hideDona);
    ...
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            // eg. BONE - ETH
            flameOut = _toBONE(token1, amount1).add(amount0);
        } else if (token1 == flame) {
            // eg. USDT - BONE
            flameOut = _toBONE(token0, amount0).add(amount1);
        } else if (token0 == weth) {
            // eg. ETH - USDC
            flameOut = _toBONE(
                weth,
