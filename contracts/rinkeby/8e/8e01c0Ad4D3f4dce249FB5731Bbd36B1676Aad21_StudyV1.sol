...
interface Asagi{
    ...
    function balanceOf(address _address) external view returns (uint256 holds);
    function ownerOf(uint256 id) external view returns (address _address);
}

contract StudyV1 is ERC721Enumerable,ERC2981,Ownable,Pausable,ReentrancyGuard{
    using Strings for uint256;
    string baseURI = "";

    bool public mintStart = false;
    uint256 constant maxSupply = 10;
    uint256 constant mintLimit = 3;
  ...
  function stake() public{
    ...
    for (uint256 i; i < at_Contract.walletOfOwner(msg.sender).length; i++) {
      timeStampMap[i]=block.timestamp;
      stampOwnerMap[i]=msg.sender;
    }
  }
