...
// File contracts/CrazyNFTs_DNA.sol

 

pragma solidity ^0.8.0;

contract CrazyNFTs_DNA {
...
contract CrazyNFTs is ERC721, ERC721Enumerable, CrazyNFTs_DNA {
    ...
    constructor(uint256 _maxSupply) ERC721("CrazyNFTs", "CNF") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No CrazyNFTs left :(");
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "Crazy NFTs #',
                tokenId.toString(),
                '", "description": "Crazy NFTs are randomized Avataaars stored on chain to teach DApp development on Platzi", "image": "',
                image,
                '"}'
            )
        );
