...
contract METAVestingICO is Ownable {
    ...
    uint minimumXDC = 100000000000000000;
    uint maximumXDC = 10000000000000000000;
...
function buy() public payable icoShouldBeStarted icoShouldnotBeEnded {
        ...
        Investor memory users;
        users.account=msg.sender;
        users.amount=tokensToBuy;
        users.investorType=3;
        vestingDetails memory vesting;
        vesting.investorType =users.investorType;
        vesting.totalBalance =users.amount;
        uint256 _amount=users.amount;
        availableBalance[users.account]+=_amount;
        vesting.initialToBeClaimed = (initialAmountReleased[users.investorType] *_amount) / 1000;
        vesting.intermediateToBeClaimed = (intermediateAmountReleased[users.investorType] * _amount)/ 1000;
        vesting.linearToBeClaimed = (linearVestingAmountReleased[users.investorType] * _amount ) / 1000;
        uintPerDay[users.account]= vesting.linearToBeClaimed/(linearVestingTimePeriod[users.investorType]/60);
        Investors[users.account] = vesting;
        emit InvestorAddress(users.account,_amount,users.investorType);
