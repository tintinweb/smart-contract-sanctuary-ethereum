...
contract NFTVendor2 is Ownable {
	...
	constructor(address tokenAddress, address silverAddress, address goldAddress, address holdingAddress, address authWallet) {
	...
	function _buy(address account, uint256 amount, uint8 v, bytes32 r, bytes32 s, uint256 timestamp) internal {
		...
		require(nextID + amount - 1 <= lastID, "Not enough NFTs available to purchase.");
		if (whitelistEnabled) {
		...
		for (uint256 i = 0; i < amount; i++)
			token.safeMint(_msgSender(), nextID + i);
		nextID += amount;
	}
	...
	function setHoldingAddress(address holdingAddress) public onlyOwner {
		holding = holdingAddress;
	}
	...
	function setLastID(uint256 id) public onlyOwner {
		lastID = id;
	}
}
