...
contract Marketplace is Whitelist {
    ...
    /// @param _receiver Address of the original owner to receive royalty
    ...
    /// @param _newOwner Address of the new owner
    /// @custom:modifier Only existing owner can change the address
    ...
    /// @custom:modifier Only existing owner can change the gross pay
    ...
    /// @custom:modifier Only existing owner can transfer funds
    function transferEthToOwner(uint256 _amount) external onlyOwner {
        ...
        amountShouldBeAskingPrice(_contractAddress, _tokenId)
    {
        ...
        if (IERC721(_contractAddress).supportsInterface(0x2a55205a)) {
            (receiver, royaltyAmount) = IERC2981(_contractAddress).royaltyInfo(
                _tokenId,
                msg.value
            );
            if (receiver == tokenOwner || royaltyAmount == 0) {
                receiver = address(0);
                royaltyAmount = 0;
            } else {
        ...
        uint256 grossAmount = (msg.value * grossPay) / 10000;
        uint256 commissionAmount = msg.value - grossAmount;
        uint256 netAmount = grossAmount - royaltyAmount;

        require(
            msg.value >= (netAmount + royaltyAmount + commissionAmount),
