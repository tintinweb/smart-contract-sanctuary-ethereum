...
/* TODO:
 * - Chainlink request handler
 */
contract Drop is ERC721, ERC721Enumerable, VRFConsumerBaseV2, ReentrancyGuard {
    ...
    function randomMetadata(uint vrfNumber, uint tokenId) private {
        foundersArts[tokenId-1] = vrfNumber % 10;
    ...
    function randomMonthlyTrait(uint vrfNumber, uint tokenId) private {
        for (uint i = 0; i < metadata.traitsChance.length; i++) {
            if (uint(keccak256(
                    abi.encodePacked(block.difficulty, block.timestamp, vrfNumber, tokenId, i
                ))) % 10000 < metadata.traitsChance[i]) {
    ...
    function randomizeMonthlyDynamicTrait() public {
        require(msg.sender == deployer);
    ...
    function fulfillRandomWords(uint256, uint256[] memory randomWords) internal override {
        metadataRandomizedCounter.add(1);
        for (uint i = 0; i < tokenIdCounter; i++) {
            randomMonthlyTrait(randomWords[0], i);
        }
        metadataRandomizedCounter.add(1);
    }
    ...
    function fulfillVRFMock() public {
        ...
        for (uint i = 0; i < tokenIdCounter; i++) {
            randomMonthlyTrait(randomWord, i);
        }
    }

    function mint(uint16[] calldata traits, string memory uri) public payable nonReentrant isWhitelisted(msg.sender) {
