...
contract ShitTestsList is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  modifier mintPriceCompliance(uint256 _mintAmount) {
    require(msg.value >= updateMintCost(totalSupply()) * _mintAmount, 'Insufficient funds!');
    _;
  }
  ...
  function updateMintCost(uint256 _supply) internal view returns (uint256 _cost) {
    if (_supply <= 2){
      return 0.01 ether;
    }
    if (_supply <= 4){
