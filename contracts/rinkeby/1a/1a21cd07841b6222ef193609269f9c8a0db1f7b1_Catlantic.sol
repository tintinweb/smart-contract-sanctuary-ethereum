...
contract Catlantic is ERC721, Ownable, ReentrancyGuard {
    ...
    constructor(uint256 maxTokens_) ERC721("Catlantic","Cat") {
    ...
    function mintA(address _to) public onlyOwner {
        require(tokensAvailable() >= 1, "Max tokens reached");
        _mint(_to);
    }
    ...
    function mint() external payable {
        ...
        require(tokensAvailable() > 0, "Max tokens reached");
        _mint(msg.sender);
    }
    ...
    function mintS(uint256 count, string memory salt, bytes memory signature) external payable {
    ...
    function mintS(uint256 count, string memory uri, string memory salt, bytes memory signature) external payable {
