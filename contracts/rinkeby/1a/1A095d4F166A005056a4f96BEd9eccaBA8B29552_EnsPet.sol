...
contract EnsPet is ERC721Enumerable, Ownable {
    ...
    constructor(
        ...
        cOwner = _cOwner;
        wWallet = _wWallet;
         _prices = [0.06 ether, 0.09 ether, 0.12 ether, 0.15 ether, 0.18 ether];
        _priceSteps = [
            1, 
            3 ,//201
            5 //,501
    ...
    function getPriceForId(uint256 tokenId) internal view virtual returns (uint256) {
        for (uint8 i = 0; i < _priceSteps.length; i++) {
    ...
    function setNode(uint256 _tokenId,bytes32 node) public authorised(node) {
        require(ownerOf(_tokenId)==msg.sender,"Not the owner of the token");
         records[_tokenId]=node;
        emit SetNode(_tokenId, node);
    }
    function n_exists(bytes32 node) public view virtual returns (bool){
        for (uint8 ti = 1; ti < totalSupply() + 1; ti++) {
            if(records[ti]==node && ownerOf(ti)==msg.sender){
                return true;
            }
        }
        return false;
