...
contract AotaverseFF is ERC721, Ownable, ReentrancyGuard {
    using Strings for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private supply;

    string public uriPrefix = ""; //ipfs://-CID-/  ///OKAY
    string public hiddenMetadataUri;

    uint256 private cost = 0.01 ether; 
    uint256 public maxSupply = 25; //6666         ///OKAY
    uint256 public softCap = 18; //6397           ///OKAY
    ...
    address public immutable proxyRegistryAddress = address(0xF57B2c51dED3A29e6891aba85459d600256Cf317); ///OKAY
    //Rinkeby: 0xF57B2c51dED3A29e6891aba85459d600256Cf317
    //Mainnet: 0xa5409ec958C83C3f309868babACA7c86DCB077c1

    address private constant withdrawTo = 0x6701bc20F2810ecF1ec5614A10e76a8E2B38f565; //funds reserve address ALSO owner ///OKAY

    constructor() ERC721("AotaverseFF", "AOTAFF") ReentrancyGuard() {
        setHiddenMetadataUri("ipfs://QmYYZ4mnWi5Zg2g4iN2P4KvGD2xP8rZxBhspyVrqVTXzER/blind.json"); //blind box
        supply.increment();
    ...
    function mintForAddress(uint256 _mintAmount, address _receiver) external onlyOwner nonReentrant { //ONLY USED TO DISTRIBUTE TO INVESTORS
    ...
    //VIEWS
    function walletOfOwner(address _owner) external view returns (uint256[] memory) { //pass
    ...
    function totalSupply() external view returns (uint256) { //pass
        return supply.current();
    }

    function getMode() external view returns (uint256) { //pass
        return mode;
    }

    function getCost() external view returns (uint256) { //pass
        return cost;
    }

    //ONLY OWNER SET

    function setMaxSupply(uint256 _MS) external onlyOwner { //pass
        require(_MS > maxSupply, "New MS below previous MS");
        maxSupply = _MS;
    }

    function setSoftCap(uint256 _SC) external onlyOwner { //pass
        require(_SC > softCap, "New SC below previous SC");
        softCap = _SC;
    }

    function togglemode() external onlyOwner { //pass
        if(mode == 1) {
            mode = 2;
            cost = 0.02 ether;
            merkleRoot = 0x680c084b02b8bc91dd3a2e3e741ad966947d0e239dbd58fe00193e5c2beeff61;
        }
        else if(mode == 2) {
            mode = 3;
            cost = 0.03 ether;
    ...
    function setRevealed(bool _state) external onlyOwner { //pass
        revealed = _state;
    }

    function setCost(uint256 _newCost) external onlyOwner { //pass wei   //pass
        require(_newCost > 0);
        cost = _newCost;
    }
    ...
    function setUriPrefix(string memory _uriPrefix) external onlyOwner { //ipfs://-CID-/    //pass
        uriPrefix = _uriPrefix;
    }

    function setPaused(bool _state) external onlyOwner { //pass
        paused = _state;
    }
    ...
    function withdraw() external payable onlyOwner { //pass
