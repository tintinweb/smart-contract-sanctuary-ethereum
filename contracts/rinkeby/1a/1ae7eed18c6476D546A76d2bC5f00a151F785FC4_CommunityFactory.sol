...
contract CommunityFactory {
  ...
  function createCommunity() public {
    ...
    Community community = new Community(msg.sender, manager);
...
contract Community {
  ...
  constructor(address ownerAddress, address managerAddress) {
    manager = managerAddress;
    owner = ownerAddress;
  }

  function createCommunityCurrency(address tokenAddr) public {
  ...
  function createWithdrawalRequest(uint256 amount) public {
  ...
  function getWithdrawalRequest(address memberAddress) public view returns (uint256) {
  ...
  function removeMyAddress() public {
  ...
  function approveWithdrawalRequest(address memberAddress, uint256 amount) public {
    ...
    require(requestAmount == amount, 'Invalid request meh.. try again');
  ...
  function bulkRequestApproval(WithdrawalRequest[] memory requests) public {
    require(msg.sender == owner, 'Only owner can approve requests');

    for (uint i = 0; i < requests.length; i++) {
      uint256 requestAmount = withdrawalRequests[requests[i].toAddress];

      require(requestAmount == requests[i].amount, 'Invalid request meh.. try again');
      require(requestAmount <= balance, 'Insufficient balance, please top up and try again.');
  ...
  function rejectWithdrawalRequest(address memberAddress) public {
  ...
  function withdraw(uint256 amount, address payable destAddress) private {
    ...
    IERC20(tokenAddress).transfer(destAddress, amount);
