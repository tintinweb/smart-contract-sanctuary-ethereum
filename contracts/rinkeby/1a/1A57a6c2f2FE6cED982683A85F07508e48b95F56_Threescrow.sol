...
contract Threescrow is NFT, EscrowFactory, ReentrancyGuard {
    ...
    function _pay(address _escrowWalletAddress) internal {
        if (msg.value > 0) {
            (bool success, ) = payable(_escrowWalletAddress).call{
                value: msg.value
            }("");
            require(success, "escrow payment failed");
        }
    }
        ...
        returns (uint256 _tokenId)
    {
        ...
        address[] memory participants = Escrow(payable(_escrowWalletAddress))
            .getParticipants();
 ...
 */
abstract contract NFT is ERC721, Payable {
    constructor(
        string memory name_,
        string memory symbol_,
        string memory contractURI_
...
contract EscrowFactory is Ownable {
    ...
    function _createEscrow(
        ...
        IEscrow(payable(proxy)).initialize(
            _participants,
            _judge,
