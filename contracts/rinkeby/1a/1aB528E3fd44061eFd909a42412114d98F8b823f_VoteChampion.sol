...
contract VoteChampion is Pausable, Ownable {
    ...
    struct User {
        ...
        uint totalVote;
        uint[] rankVoted;
        mapping(uint => uint) rankVotedAmount; // rank => amount
    ...
    function getUser(address _user) public view returns(uint _totalVote, uint[] memory _rankVoted, uint[] memory _rankVotedAmounts) {
        _totalVote = users[_user].totalVote;
        _rankVoted = users[_user].rankVoted;
        _rankVotedAmounts = new uint[](3);
        _rankVotedAmounts[0] = users[_user].rankVotedAmount[1];
        _rankVotedAmounts[1] = users[_user].rankVotedAmount[2];
        _rankVotedAmounts[2] = users[_user].rankVotedAmount[3];
    }
    function getTop3Rank() public view returns(Rank[] memory) {
    ...
    function getTop3RankAndUser3Rank(address _user) external view returns(Rank[] memory _top3Rank, uint _totalVote, uint[] memory _rankVoted, uint[] memory _rankVotedAmounts) {
        _top3Rank = getTop3Rank();
        (_totalVote, _rankVoted, _rankVotedAmounts) = getUser(_user);
    }
    ...
    function vote(uint _rank, uint _teamIndex, uint amount) external whenNotPaused {
        ...
        users[_msgSender()].rankVotedAmount[_rank] += amount;
    ...
    function unVote(uint _rank, uint _teamIndex, uint amount) external whenNotPaused {
        ...
        voteToken.transfer(_msgSender(), amount - _fee);
        users[_msgSender()].totalVote -= amount;
        users[_msgSender()].rankVoted[_rank] -= amount;
        users[_msgSender()].voted[_teamIndex][_rank].totalVote -= amount;
