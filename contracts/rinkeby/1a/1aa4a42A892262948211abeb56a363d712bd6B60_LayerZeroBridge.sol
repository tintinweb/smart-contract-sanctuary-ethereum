...
contract LayerZeroBridge is ILayerZeroReceiver {
    ...
    function estimateLzReceiveGas(
        ...
        bytes memory _fromAddress,
        uint64 _nonce,
        bytes memory _payload) external {
        uint startGas = gasleft();

        address(this).call(abi.encodeWithSelector(this.handleAllEstimate.selector, _srcChainId, _fromAddress, _nonce, _payload));

        require(false, uint2str(startGas - gasleft()));
    }
