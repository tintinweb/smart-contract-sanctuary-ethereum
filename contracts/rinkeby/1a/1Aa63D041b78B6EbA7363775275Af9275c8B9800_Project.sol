...
contract Project is ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event Publish(uint256 indexed projectId, address indexed token, string name, string symbol, uint256[] saleIds);
    ...
    function publish(ProjectInput memory _projectInput, SaleInput[] memory _saleInputs) external payable {
        require(msg.value == createProjectFee, "Invalid createProjectFee");
        ...
        if (isCreateNewToken) {
            token = (osbFactory.create(_projectInput.isSingle, _projectInput.baseUri, _projectInput.tokenName, _projectInput.tokenSymbol, _saleInputs.length, _projectInput.royaltyReceiver, _projectInput.royaltyFeeNumerator)).token;
        ...
        project.id = lastId;
        project.isCreatedByAdmin = setting.isSuperAdmin(_msgSender()) || setting.isAdmin(_msgSender());
        project.manager          = _msgSender();
        project.token            = token;
        project.isSingle         = _projectInput.isSingle;
        ...
        if (operationBalance < opFundLimit) {
            require(msg.value >= opFundLimit - operationBalance, "Not enough value");
            Helper.safeTransferNative(opFundReceiver, opFundLimit - operationBalance);
        }

        emit Publish(project.id, project.token, _projectInput.tokenName, _projectInput.tokenSymbol, sale.getSaleIdsOfProject(project.id));
    }
...
interface IOSBFactory {
    function create(bool _isSingle, string memory _baseUri, string memory _name, string memory _symbol, uint256 _maxBatch, address _royaltyReceiver, uint96 _royaltyFeeNumerator) external returns(TokenInfo memory);
}
