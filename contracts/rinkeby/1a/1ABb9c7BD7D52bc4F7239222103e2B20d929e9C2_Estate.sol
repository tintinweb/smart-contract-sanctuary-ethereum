...
contract Estate is ERC721ABurnable, Ownable, ReentrancyGuard {
	...
	uint public ADDRESS_WL_MAX_MINTS = 2;
	// mappings of batch - address - number minted
	mapping (uint256 => mapping (address => uint256)) numberOfTeamMintsOnAddress;	
	mapping (uint256 => mapping (address => uint256)) numberOfWLMintsOnAddress;
	mapping (uint256 => mapping (address => uint256)) numberOfSaleMintsOnAddress;
	...
	// option 0 = team
	// option 1 = WL
	// privateMint is used both for team mints WL mints.
	function privateMint(
		address _contractAddress, // address of this contract
		uint256 _vol, //how many to mint
		uint256 _option, // whether i am Team '0' or Whitelist '1'
		bool _mintToStake, // whether to stake immediately during minting
		bytes memory _signature
	) external payable callerIsUser {
		if (_option == 0){
			// is a free mint by team
			require(teamSaleActive, "Team sale must be active to mint");				
			require(numberOfTeamMintsOnAddress[mintingPhase][msg.sender] + _vol <= ADDRESS_WL_MAX_MINTS , "You have already minted your max WL amount");
			numberOfTeamMintsOnAddress[mintingPhase][msg.sender] += _vol;
		} else {
			...
			require(numberOfWLMintsOnAddress[mintingPhase][msg.sender] + _vol <= ADDRESS_WL_MAX_MINTS , "You have already minted your max WL amount");
		...
		callerIsUser
	{
		...
		require(_totalMinted() + _vol <= maxGenesis,"Purchase would exceed max supply of Genesis Estates");
		require(_totalMinted() + _vol <= maxBatch*mintingPhase,"Purchase would exceed max supply of Estates in this phase.");	
		require(numberOfSaleMintsOnAddress[mintingPhase][msg.sender] + numberOfWLMintsOnAddress[mintingPhase][msg.sender] + numberOfTeamMintsOnAddress[mintingPhase][msg.sender] + _vol < ADDRESS_MAX_MINTS, "Amount to be minted would exceed maximum per address.");
		require(PublicPrice * _vol == msg.value,"Eth amount sent is not enough.");
		numberOfSaleMintsOnAddress[mintingPhase][msg.sender] += _vol;
		if (_mintToStake){
			_safeMint(address(stakingContract),_vol);
			stakingContract.stakeMint(_currentIndex-_vol, msg.sender, _vol);
		} else {
	...
	// view sale states
	function getSaleState() public view returns (bool, bool, bool, uint256) {
		return (teamSaleActive, WLSaleActive, publicSaleActive, mintingPhase);
	}

	// SALE OWNER FUNCTIONS

	// set sale states
	function togglePrivateSale(uint256 saleType, bool state) external onlyOwner {
		if (saleType == 0) {
			teamSaleActive = state;
		} else {
			WLSaleActive = state;
		}
	}
	...
	function setWLMaxMints(uint256 newWLMaxMints) external onlyOwner {
		ADDRESS_WL_MAX_MINTS = newWLMaxMints;
	}
