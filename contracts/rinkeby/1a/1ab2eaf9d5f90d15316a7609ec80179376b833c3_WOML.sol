 ...
 */
contract WOML is ERC721, Ownable, PaymentSplitter {
    ...
    mapping (address => bool) mintedSupremeLand;
    
    uint256 public tokenPriceBasicLand = .00 ether;
    ...
    uint256 public devTokenCount = 7000;

    string private _contractURI;
    
    uint256 public maxMintsPerTx = 10;
    
    constructor() public ERC721("WOML", "WOMLand") {}
    ...
    function mintExtraSpecial(uint256 quantity) external onlyOwner {
    ...
    function mintSupremeLand() external payable {
        require(totalSupplySupremeLand() + 1 <= 1000, "minting this many would exceed supply");
        require(msg.value >= tokenPriceSupremeLand, "not enough ether sent");
        require(openMint, "mint not open");
        require(!onlyWhitelisted, "not open to the public yet");
        require(!mintedSupremeLand[msg.sender], "can only mint 1 penthouse per wallet");
        require(msg.sender == tx.origin, "no contracts please");
        safeMintSupremeLand(msg.sender);
    }
    function mintSupremeLandWhitelist() external payable {
        require(totalSupplySupremeLand() + 1 <= 1000, "minting this would exceed supply");
        require(msg.value >= tokenPriceSupremeLand, "not enough ether sent");
        require(openMint, "mint not open");
