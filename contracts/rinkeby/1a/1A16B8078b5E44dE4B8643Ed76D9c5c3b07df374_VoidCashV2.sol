...
import "github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.7.3/contracts/access/Ownable.sol";
...
contract VoidCashV2 is ERC20, Ownable {
    ...
    bool public swapV1TokenEnabled = false;
    ...
    // swapV1ForV2
    function swapV1TokenForV2Token(uint256 amount) external {

        require(swapV1TokenEnabled, "Swapping currently not enabled.");

        // check supply
        uint256 _totalSupply = totalSupply();
        require(_totalSupply + amount <= maxSupply, "Max exceeded.");

        // burn V1 tokens
        IERC20(v1ContractAddress).transferFrom(msg.sender, burnAddress, amount);
    ...
    function setSwapTokensAtAmount(uint256 newAmount) external onlyOwner {
        uint256 _totalSupply = totalSupply();
        require(newAmount >= _totalSupply / 100000, "Swap amount cannot be lower than 0.001% total supply.");
    ...
    function setEnableSwapV1Token(bool enabled) external onlyOwner {
        swapV1TokenEnabled = enabled;
    }
