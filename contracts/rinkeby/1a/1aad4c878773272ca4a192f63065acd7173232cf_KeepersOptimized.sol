...
error ExceedingMaxPublicSupply();
    ...
    AMultiFounderslistMerkle
{
    ...
    uint256 public constant MAX_RESERVED_SUPPLY = 200; // Number of NFTs that have been reserved.
    uint256 public constant MAX_ALLOWLIST_SUPPLY = 500; // Number of NFTs on the allowlist.
    uint256 public constant MAX_FOUNDERSLIST_SUPPLY = 300; // Number of NFTs on the Founders List.
    uint256 public constant MAX_PUBLIC_SUPPLY = 9000; // Number of NFTs for the public sale.
    ...
    //uint256 public constant LIST_PRICE = 0.125 ether;

    // TODO - These are just to keep testing costs down
    uint256 public constant PUBLIC_PRICE = 0.00015 ether;
    uint256 public constant LIST_PRICE = 0.000125 ether;
    ...
    // TODO - SIGNATURES add signer here?
    ...
    function mintReserved(address _to, uint256 _amount) external onlyOwner {
        uint256 _curMintedReservedSupply = mintedReservedSupply + _amount;
        if (_curMintedReservedSupply > MAX_RESERVED_SUPPLY) revert ExceedingReservedSupply();
    ...
    function mintFounderslist(
        ...
        whenNotPaused
    {
        uint256 mintedEntitlements = _addressDatas[msg.sender].foundersListMintedAmount + _amount;
        if (mintedEntitlements > _entitlementAmount) revert ExceedingFoundersListEntitlements();
        _addressDatas[msg.sender].foundersListMintedAmount = uint16(mintedEntitlements);

        uint256 _curMintedFoundersListSupply = mintedFounderslistSupply + _amount;
        if (_curMintedFoundersListSupply > MAX_FOUNDERSLIST_SUPPLY) revert ExceedingFoundersSupply();
    ...
    function mintAllowlist(bytes32[] calldata _merkleProof)
        ...
        whenNotPaused
    {
        if (msg.value < LIST_PRICE) revert InsufficientETH();
        ...
        uint256 _curMintedAllowlistSupply = mintedAllowlistSupply + 1;
        if (_curMintedAllowlistSupply > MAX_ALLOWLIST_SUPPLY) revert ExceedingAllowlistSupply();
        mintedAllowlistSupply = _curMintedAllowlistSupply;

        refundOverpay(LIST_PRICE);
    ...
    //function mintPublic(uint256 _amount, bytes32 _nonce, bytes32 _hash, bytes calldata _signature)
        external
        payable
        notContract
        whenNotPaused
    {
        ...
        if (signerAddress != keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                //bytes32(uint256(uint160(msg.sender)))
                _hash
            )
        ).recover(_signature)) revert SignatureMismatch();
        ...
        */
        ...
        uint256 _curmintedPublicSupply = mintedPublicSupply + _amount;
        if (_curmintedPublicSupply > MAX_PUBLIC_SUPPLY) revert ExceedingMaxPublicSupply();
        mintedPublicSupply = _curmintedPublicSupply;

        uint256 totalPrice = PUBLIC_PRICE * _amount;
        if (totalPrice > msg.value) revert InsufficientETH();

        uint256 newTotalMintedForAddress = _addressDatas[msg.sender].publicMintedAmount + _amount;
        if (newTotalMintedForAddress > MAX_MINT_PER_ADDRESS) revert MaxMintPerAddressExceeded();
        _addressDatas[msg.sender].publicMintedAmount = uint32(newTotalMintedForAddress);

        refundOverpay(PUBLIC_PRICE);
        ...
        emit Buy(msg.sender, PUBLIC_PRICE, _amount);
    }

    // Owner-only mint functionality to "Airdrop" mints to specific users
        // This also allows the owners to mint unsold assets despite auction/AL/FL status
        // Note: These will likely end up hidden on OpenSea
    function mintToUser(uint256 _quantity, address _receiver) external payable onlyOwner {
        if ((_totalSupply + _quantity) > MAX_SUPPLY) revert ExceedingMaxSupply();

        _batchMint(_receiver, _quantity, false, "");
    }
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        if (revealed) {
            ...
            return
                bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, shiftedTokenId.toString())) : "";
        }
        else {
            return hiddenURI;
        }
    ...
    //function signatureTest(address _signer, bytes32 _nonce, bytes32 _hash, bytes calldata _signature) public returns (bool) {
        function signatureTest(address _signer, bytes32 _nonce, bytes32 _hash, uint8 v, bytes32 r, bytes32 s) public returns (bool) {
        if (nonceUsed[_nonce]) revert NonceAlreadyUsed();
        ...
        */
        ...
        if (_signer != ecrecover(messageDigest, v, r, s)) revert SignatureMismatch();
    ...
    */
    ...
    function _batchMint(
        ...
        unchecked {
            ...
            _addressDatas[to].balance += uint32(amount);
