...
contract Punk is ERC721A, Ownable, ReentrancyGuard {
...
function publicSaleMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
      ...
      if(WalletMint[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (_mintAmount - freeMint) * cost,"Claim Free NFT");
            WalletMint[msg.sender] += _mintAmount;
           _safeMint(msg.sender, _mintAmount);
        }
        else
        {
           require(msg.value >= _mintAmount * cost,"Fund not enough");
            WalletMint[msg.sender] += _mintAmount;
           _safeMint(msg.sender, _mintAmount);
        }
}
