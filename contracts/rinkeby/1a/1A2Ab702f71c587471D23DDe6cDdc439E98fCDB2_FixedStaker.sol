...
error CannotDepositUnownedItem();
error CannotDepositTimeLockedItem();
  ...
  Ownable, ReentrancyGuard
{
  ...
  */
  ...
  /**
    An event tracking a deposit of the specified `tokenIds` into a specific pool
    of this staker.

    @param timestamp The block timestamp when this event was emitted.
    @param caller The caller who triggered the claim.
    @param poolId The ID of the pool that tokens were deposited into.
    @param item The address of the item smart contract with the token IDs
      specified in `tokenIds`.
    @param tokenIds The IDs of the tokens deposited into this pool.
  */
  event Deposit (
    uint256 timestamp,
    address indexed caller,
  ...
  */
  function setPool (
    ...
    pools[_id] = Pool({
      item: _item,
      lockedTokensPerSecond: _lockedTokensPerSecond,
      unlockedTokensPerSecond: _unlockedTokensPerSecond,
  ...
  */
  function claim (
    ...
    for (uint256 poolIndex = 0; poolIndex < _poolIds.length; poolIndex += 1) {
      ...
      /*
        ...
        the caller has deposited assets, transfer any accrued balance to them.
      */
      for (uint256 i = 0; i < position.stakedItems.length; i++) {
        ...
        */
        ...
        position.tokenPaid += unclaimedReward;

        // Update the total amount of token disbursed by this contract.
        totalTokenDisbursed += unclaimedReward;

        // Transfer the unclaimed reward to the user.
        totalClaimAmount += unclaimedReward;
  ...
  */
  function deposit (
    ...
    console.log("Calling deposit(", _poolId, _tokenIds.length,")");
    ...
    for (uint256 i = 0; i < _tokenIds.length; i += 1) {
      ...
      */
      ...
      /*
        ...
        duration. This prevents callers from double-dipping by depositing the
        same item to multiple pools.
      */
      if (!itemUnlocked) {
        revert CannotDepositTimeLockedItem();
      }
      ...
      /*
        ...
        we must reset storage regarding the status of the item being deposited.
      */
      ...
      */
      ...
      console.log("sender", _msgSender(), "positions.length", position.stakedItems.length);
      /*

         TODO: ask Tim what this is for

      for (
        uint256 tokenIndex = 0;
        tokenIndex < _tokenIds.length;
        tokenIndex += 1
      ) {
        uint256 candidateTokenId = _tokenIds[tokenIndex];
        */
        ...
        console.log("Testing for newly-staked status: alreadyStaked", alreadyStaked, "tokenId:", tokenId);
        if (!alreadyStaked) {
          position.stakedItems.push(tokenId);
        }
     // }
     
      console.log("Positions.length after deposit/check:", position.stakedItems.length);

      /*
        If the item being deposited is not already transfer-locked, then lock
        any transfers of the item that has been deposited.
      */
    ...
    // Emit an event.
    emit Deposit(block.timestamp, _msgSender(), _poolId, pool.item, _tokenIds);
  }
  ...
  */
  function withdraw (
    ...
    for (uint256 i = 0; i < _tokenIds.length; i += 1) {
      ...
      */
      ...
      /*
        Update the caller's staked items to remove each item ID from the
        caller's array of staked token IDs.
      */
      for (
        uint256 tokenIndex = 0;
        tokenIndex < _tokenIds.length;
  ...
  function getPosition(uint256 _id, address _addr) public view returns (uint256[] memory, uint256){
    console.log("getPosition(", _id);
    console.log(",", _addr,")");

    Position memory p = positions[_id][_addr];
    for (
      uint256 stakedIndex = 0;
      stakedIndex < p.stakedItems.length;
      stakedIndex += 1
    ) {
      console.log("=>", p.stakedItems[stakedIndex]);
    } 

    console.log("); length=", p.stakedItems.length, "paid=", p.tokenPaid);
