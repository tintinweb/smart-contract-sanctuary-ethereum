...
contract Opers is Ownable {
    ...
    struct Doctor{
        ...
        string doc_name;
        string doc_email;
        uint256 doc_phone_number;
        string doc_desc;
        string doc_speciality;
        string clinic_name;
        string clinic_loc_link;
    ...
    function addUser(address _account_address, string memory _pbk, string memory pvtk ,string memory _name, string memory _email, uint256 _phone_number, uint256 _age, string memory _gender) public onlyOwner{

        User memory new_user = User(user_id_counter, _account_address, _email, _phone_number, _name, _age, _gender);
    ...
    function takeAppointment(uint256 _user_id, uint256 _doctor_id, string memory _appointment_date, string memory _appointment_time, string 
    ...
    memory _receiver_encryption_key, string[] memory _document_links) public{
        ...
        Appointment memory new_appointment = Appointment(appointment_id_counter, _user_id, _doctor_id, _appointment_date, _appointment_time, _sender_encryption_key, _receiver_encryption_key, "Pending");
    ...
    function addDoctor(address _account_address, string memory _pbk, string memory pvtk ,string memory _name, string memory _email, uint256 _phone, string memory _desc, string memory _speciality, string memory _clinic_name, string memory _clinic_loc_link, string memory _clinic_address, string memory _profile_photo) public onlyOwner{
        
        Doctor memory new_doctor = Doctor(doc_id_counter, _account_address, _name, _email, _phone, _desc, _speciality, _clinic_name, _clinic_loc_link, _clinic_address, _profile_photo);
    ...
    function getUserAllAppointments(uint256 _user_id) public view returns(Appointment[] memory) {
    ...
    function getDocAllAppointments(uint256 _doctor_id) public view returns(Appointment[] memory) {
    ...
    function getUserSpecificAppointment(uint256 _doctor_id, uint256 _user_id) public view returns(Appointment[] memory) {
    ...
    function getDoctorSpecificAppointment(uint256 _doctor_id, uint256 _user_id) public view returns(Appointment[] memory) {
    ...
    function getAppointmentDocs(uint256 _appointment_id) public view returns(string[] memory){
        return appointment_to_documents[_appointment_id];
    }

    function getAppointmentReports(uint256 _appointment_id) public view returns(string[] memory){
        return appointment_to_reports[_appointment_id];
    }

    function getDocNotes(uint256 _doctor_id, uint256 _user_id) public view returns(string[] memory){
        return doc_to_sidenotes[_doctor_id][_user_id];
    }

    function getAllDoctors() public view returns(Doctor[] memory){
        Doctor[] memory all_docs = new Doctor[](doc_id_counter-1706200);
        for (uint256 index = 0; index < doc_id_counter-1706200; index++) {
            all_docs[index] = id_to_doc[1706200+index];
        }
        return all_docs;
    }
