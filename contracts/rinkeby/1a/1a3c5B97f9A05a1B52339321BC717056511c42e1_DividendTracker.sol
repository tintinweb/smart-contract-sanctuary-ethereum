...
contract Goblin is Context, IERC20, Ownable {
    ...
    uint256 public ethPriceToSwap = 200000000000000000; //.3 ETH
    
    //uint256 public _maxWalletAmount = 3000000000000000 * 10** _decimals;
    uint256 public _maxWalletAmount = 100000000000000000 * 10** _decimals;
    
    address public marketingAddress = 0x80b24dbbd83D2267a61Dea69af20039d0aE2b2f3;
    ...
    function createV2Pair() private {// external onlyOwner {
...
contract DividendTracker is IERC20, Context, Ownable {
    ...
    function setTokenBalance(address account) external {
        uint256 balance = goblinToken.balanceOf(account);
        if(!goblinToken.isExcludedFromRewards(account)) {
    ...
    function setERC20Contract(address contractAddr) external onlyOwner {
        goblinToken = Goblin(payable(contractAddr));
    ...
    function distributeDividends() public payable {
        ...
        buyTokens(msg.value, USDCAddress);
        uint256 newBalance = IERC20(USDCAddress).balanceOf(address(this)).sub(initialBalance);
        if (newBalance > 0) {
