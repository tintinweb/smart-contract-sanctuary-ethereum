...
contract NFTToken is ERC721, ERC721URIStorage, Ownable {
    ...
    bool public canRewardMinters = false;
    string public placeholder = "QmTqs6hzUEjUbNrUjJg9cb5ECPLQAosNECVrFAx4vat2Pp";

    string[] public NFT_Hashes;

    constructor() ERC721("NFTToken2", "NTK2") {
    }
    ...
    /**new */
    function getOwnerToken(address owner) public view returns (uint256[] memory) {
        return _token_owners[owner];
    }

    // function changePlaceHolder(string memory  new_placeholder) public onlyOwner  {
    ...
    function getNFTsCount()
        ...
        returns(uint256)
    {
        return NFT_Hashes.length;
    }

    function addNFTHashes(string[] memory hashes) public onlyOwner {
        for (uint256 i = 0; i < hashes.length; i++) {
            NFT_Hashes.push(hashes[i]);
        }
    }

    function payToMint(address recipient) public payable returns (uint256) {
        require(
            msg.sender == address(this) || msg.value >= 0.0001 ether,
            "Need to pay up!"
        );
        uint256 newItemId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        // =>map token id to owner
        _token_owners[recipient].push(newItemId);
    ...
    function rewardAllMinters() public onlyOwner {
        require(canRewardMinters == false, "can only one time to reward all minters");
        canRewardMinters = true;
        for (uint256 i = 0; i < _tokenIdCounter.current(); i++) {
            // =>generate random number
            uint8 rand = uint8(_random(0, NFT_Hashes.length, i));
            // =>set random token id image
            _setTokenURI(i, NFT_Hashes[rand]);
        }
    }


    function _random(uint256 min, uint256 max, uint256 nounce) internal view returns (uint256) {
        uint256 randomnumber = uint256(
            keccak256(abi.encodePacked(nounce, blockhash(block.number-1)))
        ) % max;
