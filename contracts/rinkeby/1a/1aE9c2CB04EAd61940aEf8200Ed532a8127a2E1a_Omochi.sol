...
interface OmochiItemInterface {
  function mint(address to, uint256 tokenId, uint256 head, uint256 body, uint256 foot) external;
}
contract Omochi is Ownable, ERC721AQueryable {
  struct Equipment {
    uint256 head;
    uint256 body;
    uint256 foot;
  ...
  address _itemAddr;
  OmochiItemInterface OmochiItemContract = OmochiItemInterface(_itemAddr);

  constructor() ERC721A("Omochi", "XENO") {
  }
  ...
  function formatTokenURI(string memory _imageURI, uint256 tokenId) internal pure returns (string memory){
    string memory baseURL = "data:application/json;base64,";
    string memory color = 'blue';
    string memory face = 'normal';
    string memory style = 'normal';
