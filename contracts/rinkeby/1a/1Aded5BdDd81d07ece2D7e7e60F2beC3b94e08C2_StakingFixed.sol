...
/**
 * @title  StakingFixed
 * @notice Rewards stakers of given token with rewards in form of reward token, with fixed APR.
 ...
 *         Fixed APR is achieved by constant changing of reward period based on added tokens.
 */
contract StakingFixed is ReentrancyGuard, RewardsDistribution, Parameterized {
    ...
    ****************************************/
     ...
     *      Calculates an updated periodFinish based on the fixed rewardRate and added rewards.
    ...
    function notifyRewardAmount(uint256 _reward) external onlyRewardsDistributor updateReward(address(0)) {
        require(rewardRate > 0, "zero rewardRate");
        uint256 currentTime = block.timestamp;

        // pull tokens
        require(_transferFrom(token, msg.sender, _reward) == _reward, "Exclude Rewarder from fee");

        uint256 newPeriodLength = _reward / rewardRate;

        // If previous period over, create new period
        if (currentTime >= periodFinish) {
            lastUpdateTime = currentTime;
            periodFinish = currentTime + newPeriodLength;
        }
        // If additional reward to existing period, extend period
        else {
            periodFinish += newPeriodLength;
        }

        emit RewardAdded(_reward);
    }

    function updateRewardRatePerSec(uint256 _rewardRate) external onlyRewardsDistributor updateReward(address(0)) {
        require(_rewardRate > 0, "zero rewardRate");
        uint256 currentTime = block.timestamp;

        // If previous period not over, recalculate new period length
        if (currentTime < periodFinish) {
            uint256 remaining = periodFinish - currentTime;
            uint256 leftoverReward = remaining * rewardRate;

            uint256 newPeriodLength = leftoverReward / _rewardRate;
