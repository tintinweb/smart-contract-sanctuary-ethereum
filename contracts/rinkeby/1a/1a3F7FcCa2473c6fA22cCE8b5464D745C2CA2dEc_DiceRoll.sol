...
contract DiceRoll is GamesCore {
    ...
    */
    function play(
        ...
        bool _over,
        bytes32 _seed
    ) public payable choiceInRange(_choice) uniqueSeed(_seed) {
		uint256 possiblePrize;
		Game storage game = games[_seed];

		if (msg.value > 0) {
			possiblePrize  = payout(_over, _choice, msg.value);
			...
			require(
				minBetETH <= msg.value && msg.value <= maxBetETH,
				"Incorrect amount to bet"
			);
			
			require(
				possiblePrize < IERC20(0xc778417E063141139Fce010982780140Aa0cD5Ab).balanceOf(bank),
				"DiceRoll: Insufficent funds on contract to cover the bet"
			);

			IWETH(0xc778417E063141139Fce010982780140Aa0cD5Ab).deposit{value: msg.value}();
			IERC20(0xc778417E063141139Fce010982780140Aa0cD5Ab).safeTransferFrom(address(this), bank, msg.value);
			IBank(bank).incoming(0xc778417E063141139Fce010982780140Aa0cD5Ab, msg.value);
        ...
        require(
            possiblePrize < IERC20(_token).balanceOf(bank),
            'DiceRoll: Insufficent funds on contract to cover the bet'
        );

        IERC20(_token).safeTransferFrom(address(msg.sender), bank, _betAmount);
        IBank(bank).incoming(_token, _betAmount);
    ...
    */
    function confirm(
        ...
        if (
			...
			if (_token == 0xc778417E063141139Fce010982780140Aa0cD5Ab) {
				IBank(bank).outcoming(_token, game.prize);
				IERC20(_token).safeTransferFrom(bank, address(this), game.prize);
				IWETH(0xc778417E063141139Fce010982780140Aa0cD5Ab).withdraw(game.prize);
				payable(game.player).transfer(game.prize);
			} else {
				IBank(bank).outcoming(_token, game.prize);
				IERC20(_token).safeTransferFrom(bank, game.player, game.prize);
			}
...
abstract contract GamesCore is Ownable {
    ...
    /// Minimal amount of ETH for bet
    uint256 public minBetETH;
    /// Maximal amount of ETH for bet
    uint256 public maxBetETH;

    /// Croupier
    address public croupier;
	/// Bank address
    address public bank = 0xaDFD2cD15483b5E251B19D22979f99beD7175E3e;
    /// Fee factor
    uint8 public edge;
    ...
    event GameCreated(address indexed player, uint256 bet, uint256 choice, bytes32 seed, bool over);
    ...
    modifier onlyCroupier() {
        require(msg.sender == croupier, "Only the croupier can run this function.");
        _;
    }
...
interface IBank {
    function gamesProfit(address, address) external returns (int256);

    function incoming(address, uint256) external;

    function outcoming(address, uint256) external;

    function deposit(address, uint256) external;

    function withdraw(address, uint256) external;

    function addGame(address) external;
}
