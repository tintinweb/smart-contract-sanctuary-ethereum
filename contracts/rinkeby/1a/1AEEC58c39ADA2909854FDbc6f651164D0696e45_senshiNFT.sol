...
contract senshiNFT is ERC721Pausable, Ownable  {
    ...
    function mint(uint256 nftAmount) external whenNotPaused payable {
        ...
        require(currentSupply() + nftAmount <= _mintingLimit , "max limit reached");
        require(msg.value == _mintingPrice * nftAmount, "check price per nft");
    ...
    function addWhitelistUser(address[] memory addresses) external onlyOwner {
        for(uint indx = 0; indx < addresses.length; indx++) {
            _isWhitelisted[addresses[indx]] = true;
        }
    }
    ...
    function updatePriceAndSupply(uint256 newPrice, uint updatedSupply) external onlyOwner {
        require(currentSupply() + updatedSupply <= _maxSupply, "cannot be more than maxsupply");
    ...
    function remainingNftInCurrentPhase() external view returns(uint256) {
        return _mintingLimit - currentSupply();
    }
}
