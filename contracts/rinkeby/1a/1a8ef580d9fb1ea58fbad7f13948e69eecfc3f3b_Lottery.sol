...
contract Lottery {
    ...
    uint256 public potFull = 0.02 ether;
    ...
    modifier isPotFull() {
        require(address(this).balance == potFull, "Pot is not full");
        _;
    }

    function enter() public payable {
        if (address(this).balance == potFull) {
            pickWinner();
        }
        require(address(this).balance != potFull, "Pot is full");
