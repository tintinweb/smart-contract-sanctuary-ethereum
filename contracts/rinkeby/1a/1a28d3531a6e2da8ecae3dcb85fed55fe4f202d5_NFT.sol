...
contract NFT is ERC721A, ERC2981, Ownable {
    ...
    ) ERC721A("SPAGHETTI DAO - GENESIS", "SPAG") {
        token = IERC20(tokenAddress_);
        baseTokenURI = baseTokenURI_;
        revealTokenURI = revealTokenURI_;
    ...
    function mint(
        ...
        if (quantity > MAX_MINT) revert InvalidMintQuantity();
        if (price * quantity > msg.value) revert InvalidValue();
        _mint(msg.sender, quantity);
        token.transfer(msg.sender, TOKEN_REWARD * quantity);
    }
    ...
    function tokenURI(
        ...
        return revealed ? string(abi.encodePacked(baseTokenURI, tokenId.toString(), '.json')) : revealTokenURI;
    }
