...
interface HexArt{
       function royaltyInfo(uint256 _tokenId, uint256 _salePrice)
        ...
        returns (address, uint256);
         function transferNFT(uint256 _tokenId, address _from,address _to) external returns(bool);
...
struct ItemForSale {
    ...
    function buyItem(uint256 id)
        ...
        IsForSale(id){
            ...
            require(hexArtAddress.transferNFT(itemsForSale[id].tokenId,itemsForSale[id].seller,msg.sender),"NFT purchase failed");
             (address  _receiver,uint256 _royaltyAmount)=hexArtAddress.royaltyInfo(itemsForSale[id].tokenId,sellerShare);
             ...
             itemsForSale[id].seller.transfer(_sellerShare);
              ...
              require(feeCollectorAddress.manageHexArtFees(addShare));
