...
contract ScotchNFT is ScotchBeneficiary, ERC721URIStorage {
  ...
  function createAutoNFTs(uint count, string memory baseURI, string memory extension, uint minDigits) public payable

  {
...

    {
      ...
      while (bytes(name).length < minDigits)

        name = string.concat("0", name);



      _setTokenURI(id,  string.concat(baseURI, name, extension));
