    ...
    ReentrancyGuard
{
    ...
    string public notRevealedUri = "ipfs://QmZ4E43Co8edjLnVaMEyrmKf5oXnwSeTiDBfdvvZuHXetW/hidden.json";
    ...
    uint256 public publicsaleAmountLimit =5;
    ...
    constructor(string memory uri, bytes32 merkleroot)
        ERC721A("SavetheVillagers", "STV")
        ReentrancyGuard() // A modifier that can prevent reentrancy during certain functions
    {
        root = merkleroot;
        _setDefaultRoyalty(0x07fF610fA2C583FcE13B2c898a37002f9cf661c3, 1000);
        setBaseURI(uri);
    }
    ...
    onlyAccounts
    {
        ...
        require(_amount <= presaleAmountLimit,      "CryptoPunks: You can't mint so much tokens");
        require(_presaleClaimed[msg.sender] + _amount <= presaleAmountLimit,  "CryptoPunks: You can't mint so much tokens");
        uint current = _tokenIds.current();
        require(current + _amount <= maxSupply,"CryptoPunks: max supply exceeded");
        require(price * _amount <= msg.value,"CryptoPunks: Not enough ethers sent");  
    ...
    onlyAccounts
    {
        ...
        require(!paused, "CryptoPunks: Contract is paused");
        require(_amount <= publicsaleAmountLimit,      "CryptoPunks: You can't mint so much tokens");
        require(_publicClaimed[msg.sender] + _amount <= publicsaleAmountLimit,  "CryptoPunks: You can't mint so much tokens");
    ...
    function setpublicsaleAmountLimit(uint256 _publicsaleAmountLimit) public onlyOwner {
