...
contract SkyVerseStaking is ReentrancyGuardUpgradeable, OwnableUpgradeable, IERC721ReceiverUpgradeable, SkySigner {
    ...
    ///@dev Stake SKY NFTs.
    function stakeNFTs(uint[] memory _tokenIds) external {
        ...
        for(uint i = 0; i < length; i++) {
            ...
            tokenInfo[_tokenIds[i]] = stakeInfo(_msgSender(), block.timestamp, userStaked[_msgSender()].length);
    ...
    function claimRewards(uint[] memory _tokenIds) public {
        uint amount;
        uint length = _tokenIds.length;
        for(uint i = 0; i < length; i++){
            require(tokenInfo[_tokenIds[i]].owner == _msgSender(), "Sender not Owner");
    ...
    function getRewards(uint _tokenId) public view returns(uint){
        ...
        uint bonusAdded = bonusReward(info.lastClaim, collected);
        collected += bonusAdded;
        return collected;
    }

    function bonusReward(uint _lastClaim, uint _collected) private view returns(uint){
        uint bonus = 0;
        uint time = block.timestamp - _lastClaim;
    ...
    ///@dev Private and Internal functions.
