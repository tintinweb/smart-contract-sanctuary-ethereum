...
contract CCCMintContract is ERC721A, Ownable {
    ...
    function devWalletOfOwner(address _owner) public view returns(uint256[] memory) {
        uint256 ownerTokenCount = balanceOf(_owner);
        require(ownerTokenCount>0,"User Doesen't Own Any NFT's");
        uint256[] memory ownedTokenIds = new uint256[](ownerTokenCount);

        for(uint256 i=0; i<totalSupply(); i++){
            if(_owner == ownerOf(i)){
                ownedTokenIds[i] = i;
                // if(ownedTokenIds.length == ownerTokenCount){
                //     return ownedTokenIds;
                // }
            }
        }
    ...
    function testDevReturn() public view returns(uint256[] memory){
        // uint256 test[] = uint256[1,2,3];
