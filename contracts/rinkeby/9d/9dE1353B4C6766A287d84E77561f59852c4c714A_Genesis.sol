...
contract Genesis is Ownable, ERC721A, ReentrancyGuard, Pausable {
    ...
    mapping(address => uint8) public publicMintedAmount;
    ...
    function mint(uint8 amount, uint8 step) external payable nonReentrant whenNotPaused isNotContract {
        ...
        if (step == FREE_MINT_STEP) {
            _mintForFreeMintStep(amount);
        } else if (step == MINT_LIST_STEP) {
            _mintForMintListStep(amount);
        } else if (step == WHITE_LIST_STEP) {
            _mintForWhiteListStep(amount);
        } else if (step == PUBLIC_STEP) {
            _mintForPublicStep(amount);
        }

        _refundIfOver(_price);
    }
    ...
    function _mintForFreeMintStep(uint8 amount) private {
        require(freeMintAddress[msg.sender] - amount >= 0, "Don't have mint authority");
        freeMintAddress[msg.sender] -= amount;
        _safeMint(msg.sender, amount);
    }

    function _mintForMintListStep(uint8 amount) private {
        require(mintListAddress[msg.sender] - amount >= 0, "Don't have mint authority");
        mintListAddress[msg.sender] -= amount;
        _safeMint(msg.sender, amount);
    }

    function _mintForWhiteListStep(uint8 amount) private {
        require(whiteListAddress[msg.sender] - amount >= 0, "Don't have mint authority");
        whiteListAddress[msg.sender] -= amount;
        _safeMint(msg.sender, amount);
    }

    function _mintForPublicStep(uint8 amount) private {
        require(publicMintedAmount[msg.sender] + amount <= publicSaleAmountPerAddress, "Exceed mintable amount");
        publicMintedAmount[msg.sender] += amount;
        _safeMint(msg.sender, amount);
    }

    function _checkMintStepValid(uint8 step) internal virtual returns (bool) {
        return step >= FREE_MINT_STEP && step <= PUBLIC_STEP;
    }
    ...
    function setMintAuthority(
        ...
        if (step == FREE_MINT_STEP) {
            ...
            revert("The public does not require additional permissions.");
        }
    }
    ...
    function setPublicSaleAmountPerAddress(uint256 _amountPerSale) external onlyOwner {
        publicSaleAmountPerAddress = _amountPerSale;
    }
