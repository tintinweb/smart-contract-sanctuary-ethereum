...
contract MagicFolk is ERC165, ERC721, ERC721Enumerable, ERC721Holder,
                      CommonConstants, IERC1155Receiver, 
                      ReentrancyGuard, SigVer, AccessControl {
    ...
    event Equipped(uint256 indexed _tokenId, uint256 indexed _itemId, ItemType _itemType);
    event Unequipped(uint256 indexed _tokenId, uint256 indexed _itemId, ItemType _itemType);
    ...
    string _URI = "https://api.magicfolk.io/"; 
    ...
    function setMagicFolkGemsContract(
        address magicFolkGems
    ...
    function safeMintPublic(
        ...
        require(
            _verifyMsg(to, 0, msgHash, signature, _signer) 
    ...
    function tokensOfOwner(address owner) public view returns (uint256[] memory) {
        ...
        uint256[] memory ret = new uint[](totalOwned);
    ...
    /**
        @notice Owner of token does not receive any unclaimed GEMS if they've
        been forced to unstake. 
        @dev For emergencies. 
     */
    ...
    //     for (uint256 i = 0; i < tokenIds.length; i++) {
    //         require(_isTokenStaked(tokenIds[i]), "NOT_STAKED");
    //         _lastClaims[tokenIds[i]] = 0;
    //     }
    // }
    ...
    function _unstakeToken(address user, uint256 tokenId) internal {
        ...
        // Have to choose between claiming and unstaking 
        // (must wait 24hrs after claiming to unstake)
        // MAGIC_FOLK_GEMS.mint(_msgSender(), allocation);
...
contract MagicFolkGems is ERC20, AccessControl {
    ...
    function setMagicFolkItemAddress(
        address newAddress,
        ItemType itemType
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        address oldAddress;
...
contract MagicFolkItems is ERC1155, ERC1155Holder, SigVer,
                           AccessControl, Pausable, ERC1155Supply, ReentrancyGuard {
    using ECDSA for bytes32;
    using Counters for Counters.Counter;
    ...
    function setMagicFolkGemsAddress(address magicFolkGems) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _magicFolkGems = MagicFolkGems(magicFolkGems);
    }

    function setMagicFolkContractAddress(address magicFolk) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function setPrice(uint256 itemId, uint256 price) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    /**
        ...
        is used to set the stats for this item. 
    ...
    /**
        @dev Can view this as a "createItemType" function. For non-collab
             items only.
        @param nullAddress must be null address
        @param itemId The id for the *new* item you're creating
        @param amount The initial supply/stock for the new item
        @param data not used, can just pass an empty string
    ...
    function mint(
        ...
        require(nullAddress == address(0), "COLLAB_MINT?");
        _mint(address(this), itemId, amount, data);
    }

    /**
        @dev Overload of `mint` for collab items
        @param nftContract the contract for the collab NFT
        @param allowancePerNFT How many items can be minted for each NFT someone
                               holds. Will usually be 1. If stock runs out, then 
                               they won't be able to buy more even if they've not
                               used their allocation. 
        @param itemId id for the new item. 
        @param amount initial supply/stock
        @param data not used, can just pass an empty string
     */
    function mint(
        address nftContract,
