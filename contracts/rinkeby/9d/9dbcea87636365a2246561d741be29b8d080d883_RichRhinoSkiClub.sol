...
/// @title Rich Rhino Ski Club

contract RichRhinoSkiClub is ERC721A, Ownable {
    ...
    constructor(string memory _NotRevealedUri) ERC721A("Rich Rhino Ski Club", "RRSC", 20) {        
    ...
    function addToWhitelist(address _address) public onlyOwner {
        whitelist[_address] = WhitelistEntry(true, presaleMaxPerWallet);
    }       

    function initPresaleWhitelist(address [] memory addr) public onlyOwner {
        for (uint i = 0; i < addr.length; i++) {
            whitelist[addr[i]] = WhitelistEntry(true, presaleMaxPerWallet);
        }
    }
    ...
    function mintPresale(uint256 num) external payable {        
        ...
        require(totalSupply() + num <= presaleSupply, 'Exceeded pre-sale supply');
    ...
    function mint(uint256 num) external payable {        
        ...
        require(totalSupply() + num <= MAX_SUPPLY - reserved, 'Exceeded total supply');
