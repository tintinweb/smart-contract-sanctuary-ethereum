...
contract WenPass is ERC1155, Ownable {
    ...
    function createPass(
        ...
        require(launchpass.ownerOf(_launchpassId) == msg.sender,  "You do not own this LaunchPass.");
        require(passes[tokenId].valid == false, "token _id already exists");
        passes[tokenId].valid = true;
        passes[tokenId].launchpassId = _launchpassId;
    ...
    function burnForAddress(uint256 _id, uint256 _quantity, address _address)
        external
    {
        MintingContract mintingContract = MintingContract(msg.sender);
        require(passes[_id].launchpassId == mintingContract.launchpassId(), "Invalid burner address");
    ...
    {
        require(passes[_id].valid, "URI requested for invalid type");
        return
