...
contract StipendVault is Ownable{
    ...
    function deposit(uint256 _poolId, uint256 _amount) external {
        ...
        if(accounting > 0 && accounting > staker.weight){
            uint256 weightIncrease = staker.weight - accounting;
            pool.totalWeight += weightIncrease;
        }
        else if (accounting < staker.weight){ //edge case, account for incremental deposit that reduces individual weight (due to tier increase)
            pool.totalWeight += accounting - staker.weight;
        }
