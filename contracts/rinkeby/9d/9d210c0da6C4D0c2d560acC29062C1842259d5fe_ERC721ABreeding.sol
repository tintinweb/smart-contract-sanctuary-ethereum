...
contract ERC721ABreeding is ERC721A, Ownable {
    ...
    function getBreedable(address _address, uint[] memory _tokenIds, uint _type) public view returns (bool[] memory) {
        ParentContract parentContract = _type == 1 ? dadContract : momContract;
        bool hasBred = false;
        bool[] memory tokens = new bool[](_tokenIds.length);
        for (uint i = 0; i < _tokenIds.length; i++) {
            hasBred = _type == 1 ? isDad[_tokenIds[i]] : isMom[_tokenIds[i]];
            tokens[i] = parentContract.ownerOf(_tokenIds[i]) == _address && !hasBred;
        }
        return tokens;
    }
    ...
    function breed(uint16[] memory dadIds, uint16[] memory momIds) public {
        ...
        for (uint16 i = 0; i < _quantity; i++) {
            ...
            parents[_currentIndex].birthday = block.timestamp;
            _currentIndex++;
        }
