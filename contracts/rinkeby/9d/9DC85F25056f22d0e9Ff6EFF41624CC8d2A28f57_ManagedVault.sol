 ...
 * @author Teragon
 * 
 * @dev Consider using an array of PendingDeposit structs for pending depoists.
 */
...
contract ManagedVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, IManagedVault {
    ...
    /* ============ State Variables ============ */
    // The minimum deposit amount for the native cryptocurrency (Ether for the Ethereum network)
    uint256 public minimumDepositAmount;
    // The minimum deposit amounts for tokens
    mapping(IERC20 => uint256) public minimumTokenDepositAmounts;

    // Pending deposits for the native cryptocurrency (Ether for the Ethereum network)
    PendingDeposit.Queue pendingDeposits;
    ...
    // The block number of the last setVaultPrice function call
    uint256 public vaultPriceBlockNumber;
     ...
     * If set to false, the native cryptocurrency won't be accepted by the {deposit} function.
     *
     * Emits a {SetNativeDepositState} event if a whitelist state is changed for a token.
     */
     ...
     *
     * If state is true, the token is allowed for deposits. Otherwise deposit function is reverted.
     *
     * Emits a {SetTokenDepositState} event if a whitelist state is changed for a token.
     */
     ...
     *
     * Afterwards, the pending deposits up until the blockNumber could be minted.
     *
     * The native price must be set regardless of its deposit status. The token prices should be set
     * in the order 
     *
     * Requires to have the mint function being called with a true return value since the last
     * setVaultPrice call.
     *
     * Emits {SetVaultPrice}, {SetNativePrice}, and {SetTokenPrice} events when these values are updated.
     */
    function setPrices(uint256 _blockNumber, uint256 _vaultPrice, uint256 _nativePrice, IERC20[] calldata _tokenAddresses, uint256[] calldata _tokenPrices) external onlyOwner {
        // Set the block number
        require(_blockNumber < block.number);
        vaultPriceBlockNumber = _blockNumber;

        // Set the vault price
        if (vaultPrice != _vaultPrice) {
            vaultPrice = _vaultPrice;
            emit SetVaultPrice(_blockNumber, _vaultPrice);
        }

        // Set the native price
        if (nativePrice != _nativePrice) {
            nativePrice = _nativePrice;
            emit SetNativePrice(_nativePrice);
        }
        ...
        for (uint256 i = 0; i < tokenDepositStates.length(); i++) {
            ...
            require(depositTokenAddress == _tokenAddresses[i]);
            if (tokenPrices[depositTokenAddress] != _tokenPrices[i]) {
                tokenPrices[depositTokenAddress] = _tokenPrices[i];
                emit SetTokenPrice(depositTokenAddress, _tokenPrices[i]);
            }
        }
    }
    ...
    /**
     * @notice Mints tokens for new deposits since the last mint.
     *
     * Requires to have the setVaultPrice function being called since the last mint call.
     * Resets the pending deposit amounts, and adds corresponding amount of balances to depositors.
     *
     * Returns whether all pending deposits are minted or not. The function must be called repeadetely
     * until it returns true. Only after a true return value, setVaultPrice could be called again.
     *
     * Emits {Minted} events.
     */
    function mint(uint256 maxMints) external onlyOwner {
        uint256 mintedPendingDeposits = 0;

        // Mint for the native cryptocurrency (Ether for the Ethereum Network)
        if (nativeDepositState) {
            while (!pendingDeposits.isEmpty() && mintedPendingDeposits < maxMints) {
                PendingDeposit.Item memory nextPendingDeposit = pendingDeposits.peek();
                if (nextPendingDeposit.blockNumber > vaultPriceBlockNumber) {
                    break;
                }

                pendingDeposits.dequeue();
                uint256 vaultTokenAmount = nextPendingDeposit.amount * nativePrice / vaultPrice;
                _mint(nextPendingDeposit.depositor, vaultTokenAmount);
                mintedPendingDeposits++;
        ...
        for (uint256 i = 0; i < tokenDepositStates.length(); i++) {
            ...
            while (!pendingTokenDeposits[depositTokenAddress].isEmpty() && mintedPendingDeposits < maxMints) {
                ...
                uint256 vaultTokenAmount = nextPendingDeposit.amount * tokenPrice / vaultPrice;
                _mint(nextPendingDeposit.depositor, vaultTokenAmount);
                mintedPendingDeposits++;
                emit Minted(nextPendingDeposit.depositor, vaultTokenAmount);
            }
     ...
     */
    function invoke(address target, bytes calldata data, uint256 value) external onlyOwner returns (bytes memory) {
        bytes memory returnValue = target.functionCallWithValue(data, value);
     ...
     *
     * msg.value must be equal to or greater than minimumDepositAmount.
     *
    ...
    function depositNative() payable external {
        ...
        require(nativeDepositState);
        require(amount >= nativeMinimumDepositAmount);
        ...
        pendingDeposits.enqueue(pendingDeposit);
    ...
    function depositToken(IERC20 tokenAddress, uint256 amount) external {
        ...
        require(amount >= tokenMinimumDepositAmounts[tokenAddress]);
     ...
     */
    function depositTokens() external view returns (address[] memory tokenAddresses) {
        return tokenDepositStates.values();
    }


    /**
     * @notice Returns the number of pending mints and the total number of vault tokens.
     * Only considers the blocks at or before vaultPriceBlockNumber.
    ...
    function pendingMintsCountAndAmount(uint256 maxMints) external view returns (uint256 count, uint256 amount) {
        ...
        if (nativeDepositState) {
            for (uint256 i = 0; i < pendingDeposits.length(); i++) {
                ...
                PendingDeposit.Item memory pendingDeposit = pendingDeposits.peekIndex(i);
                if (pendingDeposit.blockNumber > vaultPriceBlockNumber) {
                    break;
                }
                count++;
    ...
    /**
     * @notice Returns the remaining number of deposit native cryptocurrency (Ether for the Ethereum network) amount.
     */
    ...
    /**
     * @notice Returns the remaining number of deposit amount for the given token address.
     */
    ...
    /**
     * @notice Returns the total remaining deposit amount for the given token address up to a block number.
     */
    ...
    /* ============ Public Functions ============ */
    function hasPendingMints() public view returns (bool) {
        if (nativeDepositState) {
            if (!pendingDeposits.isEmpty()) {
                if ( pendingDeposits.peek().blockNumber <= vaultPriceBlockNumber) {
                    return true;
                }
            }
        }
        ...
        for (uint256 i = 0; i < tokenDepositStates.length(); i++) {
            IERC20 depositTokenAddress = IERC20(tokenDepositStates.at(i));
            if ( !pendingTokenDeposits[depositTokenAddress].isEmpty() ) {
                if ( pendingTokenDeposits[depositTokenAddress].peek().blockNumber <= vaultPriceBlockNumber) {
                    return true;
                }
            }
        }
 ...
 * @author Teragon
 * 
 * @dev Consider using an array of PendingDeposit structs for pending deposits.
 */
...
interface IManagedVault {
     ...
     */
    event SetNativePrice(uint256 indexed price);

    /**
     * @notice Emitted when the manager sets the price of a deposit token for the deposits. 
     */
    event SetTokenPrice(IERC20 indexed tokenAddress, uint256 indexed price);
     ...
     * Emits {Minted} events.
     */
    function mint(uint256 maxIterations) external; //onlyOwner
     ...
     * @param value                  Quantity of Ether to provide the call (typically 0)
     * @param data                   Encoded function selector and arguments
     * @return ret                  Bytes encoded return value
     */
    function invoke(address target, bytes calldata data, uint256 value) external returns (bytes memory ret); //onlyOwner
     ...
     * @notice Returns the remaining number of deposit native cryptocurrency (Ether for the Ethereum network) amount.
     *
     * This value changes when {deposit} is called by depositor or {mint} is called by the owner.
     */
    function pendingNativeDepositAmount(address depositor) external view returns (uint256);

    /**
     * @notice Returns the remaining number of deposit amount for the given token address.
     *
     * This value changes when {depositToken} is called by depositor or {mint} is called by the owner.
     */
    function pendingTokenDepositAmount(address depositor, IERC20 tokenAddress) external view returns (uint256);
}
