...
contract Vesting is Ownable,ReentrancyGuard {
    ...
    uint[7] public  lockEnd=[0,seedLockEndDate,privateLockEndDate,publicLockEndDate,launchpadLockEndDate,marketdevelopmentLockEndDate,airdropcampaignLockEndDate];
    ...
    */
    ...
    function addInvestorDetails(Investor[] memory investorArray) public onlyOwner {
        for(uint16 i = 0; i < investorArray.length; i++) {
         if(isremoved){
                ...
                require(!isSameInvestor[investorArray[i].account][investorArray[i].saleType],"Investor Exit");
                isSameInvestor[investorArray[i].account][investorArray[i].saleType]=true;
            }
            ...
            if(saleType == 3) {
                ...
                investor.tokensPerUnit = ((investor.totalBalance)-(investor.initialAmount))/(1095);
            }
            ...
            if(saleType == 5){
                ...
                investor.initialAmount = (investor.totalBalance * 0)/(100);
                investor.tokensPerUnit = ((investor.totalBalance)-(investor.initialAmount))/(730);
            }
            if(saleType == 6){
                investor.reminingUnitsToVest = 180;
                investor.initialAmount = (investor.totalBalance * 0)/(100);
                investor.tokensPerUnit = ((investor.totalBalance)-(investor.initialAmount))/(180);
    ...
    function withdrawTokens() public   nonReentrant setStart {
        ...
        lockEnd=[0,seedLockEndDate,privateLockEndDate,publicLockEndDate,launchpadLockEndDate,marketdevelopmentLockEndDate,airdropcampaignLockEndDate];           
    ...
    function setDates(uint256 StartDate,bool _isStart) public onlyOwner{
        ...
        publicLockEndDate = publicStartDate + 4  minutes;
        ...
        publicVestingEndDate = publicLockEndDate + 1095 minutes;
    ...
    function removeInvestor(address  _addr) external onlyOwner{
        isremoved=true;
    ...
    function getAvailableBalance(address _addr) external view returns(uint256, uint256, uint256){
           if(Investors[_addr].isInitialAmountClaimed || Investors[_addr].investorType == 3 || Investors[_addr].investorType == 5 || Investors[_addr].investorType == 6 ){
            ...
            uint hello= day;
            uint timeDifference;
            uint lockDatemarket = marketdevelopmentLockEndDate;
            uint lockDatepublic = publicLockEndDate;
            
            if(Investors[_addr].investorType == 3 ){
                   if(Investors[_addr].lastVestedTime == 0) {
                if(block.timestamp<lockDatepublic) return(0,0,0);
            if(lockDatepublic + day> 0)return (((block.timestamp-publicLockEndDate)/day) *Investors[_addr].tokensPerUnit,0,0);//, "Active lockdate was zero");
            timeDifference = (block.timestamp) -(lockDatepublic);
            }
            else{ 
                 timeDifference = (block.timestamp) - (Investors[_addr].lastVestedTime);
               }

            }else if(Investors[_addr].investorType == 5){
                   if(Investors[_addr].lastVestedTime == 0) {
                if(block.timestamp<lockDatemarket) return(0,0,0);
            if(lockDatemarket + day> 0)return (((block.timestamp-marketdevelopmentLockEndDate)/day) *Investors[_addr].tokensPerUnit,0,0);//, "Active lockdate was zero");
            timeDifference = (block.timestamp) -(lockDatemarket);
            }
            else{ 
                 timeDifference = (block.timestamp) - (Investors[_addr].lastVestedTime);
               }
