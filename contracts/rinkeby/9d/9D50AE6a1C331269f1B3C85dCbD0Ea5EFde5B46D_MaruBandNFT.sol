  ...
  MerkleProofAllowlisting
{
  constructor(string memory name, string memory symbol) ERC721A(name, symbol) {
    console.log("MaruBandNFT contract created");
  ...
  // FIXME made it very cheap
  uint256 public publicSalePrice = 100000000000000; //0.0001 ETH
  uint256 public preSalePrice = 80000000000000; //0.00008 ETH
  uint256 private collectionSize = 2000;
  uint256 private maxDevMint = 100;
  uint256 private maxBatchSize = 5;
  ...
  // FIXME let anyone set the baseURI
  ...
  // XXX do we actually need this or is ownerOf sufficient?
  ...
  function publicSaleMint(uint256 quantity)
    ...
    isSufficientPayment(publicSalePrice * quantity)
  {
    require(quantity <= 4, "cannot mint more than 4 at once in public sale");
  ...
  function preSaleMint(uint256 quantity, bytes32[] calldata _merkleProof)
    ...
    isSufficientPayment(preSalePrice * quantity)
  {
    require(totalSupply() + quantity <= collectionSize, "reached max supply");

    require(
      numberMinted(msg.sender) + quantity <= 2,
      "can not mint more than 2 per wallet in pre sale"
    );

    // TODO check the user is actually allowlisted
  ...
  function devMint(uint256 quantity) external onlyOwner {
    require(
      totalSupply() + quantity <= maxDevMint,
    ...
    emit DevMint(msg.sender, quantity, indexBeforeMint);
  }
  ...
  // XXX check with Fabiano if this is the correct way to do this https://github.com/OpenZeppelin/openzeppelin-contracts/issues/3221
  ...
  modifier isSufficientPayment(uint256 totalPrice) {
...
contract MerkleProofAllowlisting is Ownable {
    ...
    modifier requiresMerkleProofAllowlist(bytes32[] calldata _merkleProof) {
        require(merkleRoot != 0, "allowlist not enabled");
...
contract SalesTimestamps is Ownable {
  ...
  modifier isInValidPreSalePeriod() {
    require(
      block.timestamp >= preSaleStartTimeInSeconds,
      "pre sale has not started yet"
    );
    require(block.timestamp <= preSaleEndTimeInSeconds, "pre sale has ended");
    _;
  }
  /**
  ...
  modifier isInValidPublicSalePeriod() {
    require(
      block.timestamp >= publicSaleStartTimeInSeconds,
      "public sale has not started yet"
    );
    require(
      block.timestamp <= publicSaleEndTimeInSeconds,
