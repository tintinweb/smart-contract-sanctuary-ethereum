...
contract NeighboursToken is ERC721, Ownable {
    ...
    mapping(uint => bool) private _tokenIsImageSet;
    ...
    function defaultURI() view external returns (string memory) {
        return _defaultURI;
    }

    function setDefaultURI(string memory defaultURIParam) external onlyCOO {
        _defaultURI = defaultURIParam;
    }
    ...
    function setBaseURI(string memory baseURIParam) external onlyCOO {
        _baseTokenURI = baseURIParam;
    }
    ...
    function _mintNewToken(address to, uint imageId, bool isCheck) internal {
        ...
        emit TokenBought(_msgSender(), to, tokenId, imageId);
    }

    function safeMint(address to, uint imageId) external onlyOwner {
        _mintNewToken(to, imageId, false);
    }
