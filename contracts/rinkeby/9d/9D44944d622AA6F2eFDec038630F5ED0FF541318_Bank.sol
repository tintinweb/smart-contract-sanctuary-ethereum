...
library EtherConverter {
    function toWei(uint amountInEther) internal pure returns (uint) {
...
contract Bank is Ownable, Pausable {
    ...
    function withdraw(uint _amount) external whenNotPaused {
        require(balances[msg.sender] >= _amount * 1 ether, "Not enough");
        uint amountInWei = _amount.toWei();
        balances[msg.sender] -= amountInWei;
        totalBalance -= amountInWei;
