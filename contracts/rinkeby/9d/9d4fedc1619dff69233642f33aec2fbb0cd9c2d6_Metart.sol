...
contract Metart is ERC721,Ownable{
   ...
   // uint256 public startingIndex;
    //nft价格  80000000000000000; //0.08 ETH
    uint256 public constant nftPrice = 20000000000000; //0.00002 ETH
    //单次铸币最大数
    uint public constant maxMintCoinsSingle = 20;
    uint256 public constant Max_Number = 8088;
    ...
    //公开发售的起始编号 
    uint256 public constant startOvertSale = 889;
   ...
   // uint256 public REVEAL_TIMESTAMP;
    ...
    event eventSetWhiteList(address _address);

    event eventSetBlackList(address _address);

    event eventRemoveWhiteList(address _address);

    event eventRemoveBlackList(address _address);
    ...
    function setWhiteList(address _address)  public onlyOwner{
        ...
        emit eventSetWhiteList(_address);
    }
    ...
    */
    function mintMetart(uint numberOfTokens) public payable {
        ...
        require(totalSupply().sub(indexReserveSale).add(startOvertSale).add(numberOfTokens) <= Max_Number, "Purchase would exceed max supply of Tokens");
        require(nftPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
         //889-8088 可以自由售卖         
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply().sub(indexReserveSale).add(startOvertSale);//.add(numberOfTokens) ;
