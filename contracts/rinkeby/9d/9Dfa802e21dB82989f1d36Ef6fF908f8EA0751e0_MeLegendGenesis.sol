...
//
contract MeLegendGenesis is ERC721Enumerable, Ownable, PaymentSplitter {
  ...
  constructor() ERC721("(m)E-Legend", "MEL") PaymentSplitter(_team, _teamShares) {}
  ...
  function mint(uint256 _amount, bytes32[] memory proof) external payable {
    ...
    require(totalMinted + _amount <= maxMintSupply, "max supply exceeded");
    require(_price * _amount <= msg.value, "value sent is not correct");
    require(_presaleClaimed[msg.sender] + _amount <= presaleMintLimit, "can't mint such a amount");
  ...
  function gift(address[] calldata _addresses) external onlyOwner {
    require(totalGifted + _addresses.length <= maxGiftSupply, "max gift supply exceeded");
  ...
  function gift(address _address, uint256 _amount) external onlyOwner {
    require(totalGifted + _amount <= maxGiftSupply, "max gift supply exceeded");
