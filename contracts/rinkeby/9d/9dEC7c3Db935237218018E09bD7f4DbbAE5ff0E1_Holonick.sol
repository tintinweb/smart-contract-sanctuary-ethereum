...
contract Holonick is AdminControl, ICreatorExtensionTokenURI, ILazyDelivery, ILazyDeliveryMetadata {
    ...
    address private _discountAddress;
    uint private _discountTokenId;
    ...
    function setDiscountListing(uint listingId, address marketplace, address discountAddress, uint discountTokenId) public adminRequired {
        _discountListingId = listingId;
        _discountMarketplace = marketplace;
        _discountAddress = discountAddress;
        _discountTokenId = discountTokenId;
    }
    ...
    function deliver(address, uint256 listingId, uint256 assetId, address to, uint256, uint256 index) external override returns(uint256) {
        ...
        if (listingId == _discountListingId) {
            require(IERC1155(_discountAddress).balanceOf(to, _discountTokenId) > 0 ||
            IERC721(_ashCCCore).balanceOf(to) > 0, "Must hold ASH Aeternum or ASH CC to mint.");
        }
