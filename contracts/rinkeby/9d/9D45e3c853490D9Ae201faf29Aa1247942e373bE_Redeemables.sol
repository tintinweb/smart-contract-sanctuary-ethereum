...
contract Redeemables is ERC1155, Ownable {
    //variables
    ...
    modifier onlyParentContact() {
        ...
        require(
            ...
            "Only parent contract can call this (Redeemables.sol)."
        );
    }
    ...
    {
        ValidContractsItemsIn memory validItemTypesIn = projects[
            contractAddress
        ];
        ...
        for (
            uint256 i = 0;
            i < validItemTypesIn.validItemTypesIDs.length;
            i++
        ) {
    ...
    function addRedeemableItemForProject(
        uint256[] calldata itemIDs, //this is what we want to have from redeemables contract.
        uint256 projectId // this is the projectid from maincontract we want to add extra items for.
    ) public onlyParentContact {
        uint256[] memory toPush = new uint256[](itemIDs.length); //amount of items to add
        MainContract mainContract = MainContract(parent);
        (uint256[] memory projectItems, address projectAddress) = mainContract
            .getProjectItems(projectId);
        ValidContractsItemsIn storage contractItems = projects[projectAddress];
        
        uint256 lastItemID = projectItems[projectItems.length - 1];
        uint256[] memory tempArr;
    
        for (uint256 index = 0; index < itemIDs.length; index++) {
            RededeemableItemType storage itemType = validItemTypes[
                itemIDs[index]
            ];
            require(itemType.enabled, "Item is not enabled/valid");

            //need to get items for the NFT and add on top of it.

            //add on top of last projectItemIDS in the array.



            if(tempArr.length == 0) {
                tempArr[index] = lastItemID + 1;
            } else {
                tempArr[index] = tempArr[index - 1] + 1;
            }

            // uint256 newItemID = lastItemID + 1;

            contractItems.validItemTypesIDs.push(tempArr[index - 1]);
            contractItems.validItemTypes.push(itemIDs[index]);


        }
        mainContract.addRedeemablesToProject(projectId, toPush);

    }

    function mintForUser(address owner, uint256 idToMint)
        public
        onlyParentContact
    {
        //get the item type
        //get the amount of items to mint
        //mint the items
        //add the items to the project
        _mint(owner, idToMint, 1, "");
    }
        ...
        returns (string memory)
    {
        ...
        RededeemableItemType memory item = validItemTypes[_tokenId];
        require(item.enabled, "Item is not enabled/valid");
        return item.uri;
    }
    ...
    constructor(address parentAddress, string memory baseURI) ERC1155(baseURI) {
