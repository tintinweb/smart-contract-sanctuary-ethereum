...
contract MetroMiniBlock is ERC721ExtraStorage, Controllable, IMetroBlockInfo {
    ...
    uint256 public constant MAX_MET_AND_TRADE_BLOCKS = MAX_MET_BLOCKS + MAX_TRADE_BLOCKS;
    ...
    uint256 public constant MAX_GENESIS_BLOCK_ID = 10_000;
    ...
    address public metroPassAddress;
    ...
    constructor(
      ...
      address _genesisTreasuryAddress,
      address _metroPassAddress) ERC721ExtraStorage("Metroverse Mini City Block", "METROMINIBLOCK")
    {
    ...
    function setMetroPassAddress(address _metroPassAddress) external onlyOwner {
        metroPassAddress = _metroPassAddress;
    }
    ...
    function setAccountExtraOf(address owner, uint224 extra) public onlyController returns (AccountInfo memory) {
        return _setAccountExtraOf(owner, extra);
    }

    function whitelistMinted(address owner) public view returns (uint256) {
        return accountExtraOf(owner);
    }

    function setTimestampOf(uint256 tokenId, uint40 timestamp) public onlyController returns (uint40 oldTimestamp) {
        return _setTimestampOf(tokenId, timestamp);
    }

    function setTokenExtraOf(uint256 tokenId, uint56 extra) public onlyController returns (uint56 oldExtra) {
        return _setTokenExtraOf(tokenId, extra);
    }

    function setTimestampAndExtraOf(uint256 tokenId, uint40 timestamp, uint56 extra) public onlyController returns (TokenInfo memory) {
        return _setTimestampAndExtraOf(tokenId, timestamp, extra);
    }
    ...
    function currentMetPrice() public view returns (uint256) {
        ...
        uint256 lastPrice = uint256(lastMetMintPrice) * 1 ether;
    ...
    function metMint(uint256 amount) public {
        ...
        require(metMinted + tradeMinted <= MAX_MET_AND_TRADE_BLOCKS, "Exceeded max MET block count");

        uint256 currentPrice = currentMetPrice();
        uint256 newMetMintPrice = currentPrice / 1 ether;
        if (newMetMintPrice < lastMetMintPrice) {
            lastMetMintPrice = uint32(newMetMintPrice);
        }
    ...
    function tradeMint(uint256[] calldata stakedTokenIds, uint256[] calldata unstakedTokenIds) public {
        ...
        require(tradeMinted + metMinted <= MAX_MET_AND_TRADE_BLOCKS, "Exceeded max trade block count");
        
        require(genesisTreasuryAddress != address(0), "No treasury available");
        ...
        if (stakedTokenIds.length > 0) {
            unchecked {
                for (uint256 i = 0; i < stakedTokenIds.length; ++i) {
                    require(stakedTokenIds[i] <= MAX_GENESIS_BLOCK_ID, "Only genesis blocks allowed");
                }
            }
    ...
    function _mintBlocks(uint256 amount, uint224 extra) internal {
    ...
    {
        return tokensOfOwnerInRange(owner, BLOCK_ID_OFFSET + 1, BLOCK_ID_OFFSET + totalMinted());
    }

    /// Returns tokens of user, should never be used inside of transaction because of high gas fee.
    function tokensOfOwnerInRange(address owner, uint256 start, uint256 stop) public view returns (uint256[] memory) {
...
contract ERC721ExtraStorage is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _setAccountExtraOf(address owner, uint224 extra) internal returns (AccountInfo memory oldAccountInfo) {
        require(owner != address(0), "ERC721: extra query for the zero address");
    ...
    function _setTimestampOf(uint256 tokenId, uint40 timestamp) internal returns (uint40 oldTimestamp) {
        TokenInfo storage info = _owners[tokenId];
        oldTimestamp = info.timestamp;
        info.timestamp = timestamp;
    }

    function _setTokenExtraOf(uint256 tokenId, uint56 extra) internal returns (uint56 oldExtra) {
        TokenInfo storage info = _owners[tokenId];
        oldExtra = info.extra;
        info.extra = extra;
    }

    function _setTimestampAndExtraOf(uint256 tokenId, uint40 timestamp, uint56 extra) internal returns (TokenInfo memory result) {
        TokenInfo storage info = _owners[tokenId];
        result = info;
        info.timestamp = timestamp;
        info.extra = extra;
