...
contract ROY is Context, IERC20, Ownable {  //&
 ...
 /* ======================================== CONSTRUCTOR ======================================== */
    ...
    function createLiquidityPool(uint256 ethAmount) external onlyOwner returns(bool){
        require(!launched, "Contract is already launched.");
        require(lpPair == address(0), "Liquidity pool is already alive.");
        require(ethAmount > 0, "ETH amount provided must be > 0");
        require(owner().balance >= ethAmount, "Owner does not have enough ETH");
        ...
        //add eth to contract
        bool success;
        (success,) = address(msg.sender).call{value: ethAmount}("");
