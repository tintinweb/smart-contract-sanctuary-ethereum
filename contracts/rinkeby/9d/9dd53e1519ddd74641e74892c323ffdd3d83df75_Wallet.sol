...
contract Wallet is Ownable  {
    ...
    function encode1(string memory str) external onlyOwner view returns (bytes memory) {
        bytes memory callbytes = abi.encodeWithSignature(str);

        return callbytes;
    }

    function encode2(string memory str) external onlyOwner view returns (bytes memory) {
