    ...
    override
  {
    ...
    if (reqIdTypes[requestId] == 1) {
      ...
      if (claimReq.risky) {
        ...
        uint256 a = planetParams.geologicalInstability;
        uint256 b = 1000;
        uint256 c = a - b;
        uint256 d = PRBMathUD60x18.div(c, 3000);
        uint256 e = PRBMathUD60x18.mul(d, 100);
        // TODO: Fix this equation
        bool destruct = e >= (randomness % 100) + 1;
        if (destruct) {
    ...
    if (reqIdTypes[requestId] == 2) {
      ...
      uint256 x;
      if (planetParams.groundDefenses != 0 && shipParams.groundWeapons != 0) {
        //  (weapons/defenses)*0.24 - 0.01;
        x = PRBMathUD60x18.div(
      ...
      if (succeeded) {
        ...
        EL69.transfer(pirateReq.pirate, pirateReq.price);
