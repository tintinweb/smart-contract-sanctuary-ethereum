...
contract superBLOX is ERC721AS, Ownable, Pausable, ReentrancyGuard {
  ...
  // should this max be constat?
  uint256 public constant teamVault = 100;
  ...
  ) ERC721AS("superTEST", "SB", maxBatchSize_,collectionSize_ ) {
  ...
  // REMEMBER TO UPDATE THE SALE TIMES AND NAME
  // uint256 public vipSaleStartTime = 1645063200000;  //16 Feb 6 PM PST
  uint256 public vipSaleStartTime = block.timestamp;  
...
// TODO check for vip list and max quantity 
...
// Frens should mint here, then mint public sale
  ...
  onlyOwner {
    require(
      balanceOf(owner()) + quantity <= teamVault,
      "too many minted already"
    );
    require(
