...
contract GGNFT is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function mint(address to, uint256 rarity) public adminOrOwner {
        require(rarity >= 0 && rarity < 5, "Rarity must be between 0-4");
        if (rarity == 0) {
            _mintRandom(to, commonCount, totalCommon, 0);
        } else if (rarity == 1) {
            _mintRandom(to, uncommonCount, totalUncommon, totalCommon);
        } else if (rarity == 2) {
            _mintRandom(to, rareCount, totalRare, totalCommon + totalUncommon);
        } else if (rarity == 3) {
            _mintRandom(to, superRareCount, totalSuperRare, totalCommon + totalUncommon + totalRare);
        } else if (rarity == 4) {
            _mintRandom(to, legendaryCount, totalLegendary, totalCommon + totalUncommon + totalRare + totalSuperRare);
        }
    }
