  ...
  PausableUpgradeable
{
  ...
  event VirusCreated(uint256 tokenId, address to, VirusAttributes.Attributes attributes );
  ...
  function mintWithAmino(uint256 amount, address to) external whenNotPaused {
    ...
    for (uint256 i = 0; i < amount; i++) {
      ...
      VirusAttributes.Attributes memory attr = _attributes.randomizeAttributes(_tokenIdTracker.current());
      emit VirusCreated(_tokenIdTracker.current(), to, attr);
      _tokenIdTracker.increment();
    }
  }

  function mintWithAminoERC20(uint256 amount, address to) external whenNotPaused {
    require(tx.origin == msg.sender, "no contracts");
