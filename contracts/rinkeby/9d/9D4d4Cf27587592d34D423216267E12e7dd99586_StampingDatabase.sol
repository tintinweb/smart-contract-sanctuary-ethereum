...
contract StampingDatabase is IStampingDatabase {
    ...
    /// @inheritdoc IStampingDatabase
    bytes32 public immutable override title;

    /// @inheritdoc IStampingDatabase
    mapping(address => IStampingDatabase.StampInfo) public override tokens;

    /// @inheritdoc IStampingDatabase
    mapping(address => mapping(uint256 => bytes32)) public override data;

    constructor() {
        (hub, title) = IStampingDatabaseDeployer(msg.sender).parameters();
    }

    /// @dev Check the ownership of erc721 token
    /// @dev This function is gas optimized to avoid a redundant extcodesize check in addition to the returndatasize
    /// check
    function ownerOf(address token, uint256 tokenId) private view returns (address) {
    ...
    /// @inheritdoc IStampingDatabase
    function addToken(address token, bytes calldata traits, bool editable, bool onlyOwner) external override {
        require(!tokens[token].activated, "This category already exist");
        tokens[token] = IStampingDatabase.StampInfo(traits, editable, onlyOwner, true);
        emit CategoryAdded(token, traits, editable, onlyOwner, msg.sender);
    }

    /// @inheritdoc IStampingDatabase
    function stamp(address token, uint256 tokenId, bytes32 trait) external override {
        IStampingDatabase.StampInfo memory category = tokens[token];
        ...
        if (!category.editable) {
            require(data[token][tokenId] == 0, "This tokenId already stamped");
        }

        if (category.onlyOwner) {
            require(ownerOf(token, tokenId) == msg.sender, "Caller is not the owner");
        }
        
        data[token][tokenId] = trait;
        emit TokenStamped(token, tokenId, trait, msg.sender);
    }
...
/// @notice A Stamping database facilitates data storage for each of token
interface IStampingDatabase {
    ...
    /// @notice The title of database
    /// @return title The title of database
    function title() external view returns (bytes32);

    /// @notice The token info
    /// @param token The token address
    ...
    function tokens(address token) external view returns (bytes memory traits, bool editable, bool onlyOwner, bool activated);

    /// @notice The content stored in database
    /// @param token The token address
    /// @param tokenId The id of stamped token
    /// @return value The value stored in the position
    function data(address token, uint256 tokenId) external view returns (bytes32);

    /// @notice Add a new token to be stamped
    /// @dev Token needed to be added before stamping
    /// @param token The token address
    /// @param traits The trait list that can be use for stamping (can be NULL to allow any trait)
    /// @param editable The status to determine whether the trait can be change later or not
    /// @param onlyOwner The status to determine whether the stamper needed to be token owner or not
    function addToken(address token, bytes calldata traits, bool editable, bool onlyOwner) external;

    /// @notice Stamp the data between trait and token
    /// @param token The token address
    /// @param tokenId The specific token id owned by sender
    /// @param trait The trait to be stamped with token
    function stamp(address token, uint256 tokenId, bytes32 trait) external;
}
...
interface IStampingHub {
    ...
    /// @param title The title of database
...
interface IStampingDatabaseDeployer {
    ...
    /// Returns title The title of database
    function parameters()
        external
        view
        returns (
