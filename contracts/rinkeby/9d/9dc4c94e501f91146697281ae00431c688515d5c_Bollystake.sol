...
contract Bollystake is BSERC20("sBOLLY Staked Bollycoin", "sBOLLY"),Ownable{
    ...
    uint256 private constant _TIMELOCK_730 = 73 minutes;
     ...
     mapping(address => Locked[7]) internal userStake;
    ...
    function getAllUserStakes() external view returns (address [] memory, uint256 [7][] memory,uint256 [7][] memory)
   {
       ...
       address [] memory user_address = new address[](stakeholders.length);
       for (uint256 s = 0; s < stakeholders.length; s += 1){
           all_user_stakes[s] = [ userStake[stakeholders[s]][0].lockedAmount, userStake[stakeholders[s]][1].lockedAmount, userStake[stakeholders[s]][2].lockedAmount, userStake[stakeholders[s]][3].lockedAmount, userStake[stakeholders[s]][4].lockedAmount, userStake[stakeholders[s]][5].lockedAmount, userStake[stakeholders[s]][6].lockedAmount];
           user_address[s]= stakeholders[s];
           for(uint k=0;k<7;k+=1){
               if( userStake[stakeholders[s]][k].expire > block.timestamp ) {
                 all_user_eligible_stakes[s][k]= userStake[stakeholders[s]][k].lockedAmount;
               } else {
   ...
   {
       Locked[7] storage user_stake_info = userStake[_stakeholder];
        uint256 expired_amount=0;
        for (uint256 s = 0; s < 7; s += 1){
             if (user_stake_info[s].expire < block.timestamp) {
                 expired_amount = expired_amount.add(user_stake_info[s].lockedAmount);
             }
        }
    ...
    function enterStake(uint256 _amount ,uint256 _days) external {
       ...
       stakes[msg.sender] = stakes[msg.sender].add(_amount);
        ...
        Locked[7] storage user_stake_info = userStake[msg.sender];
        if (_days ==30) {
        user_stake_info[0].expire = block.timestamp + _TIMELOCK_30;
        user_stake_info[0].lockedAmount = user_stake_info[0].lockedAmount.add(_amount);
        } else if (_days ==60) {
        user_stake_info[1].expire = block.timestamp + _TIMELOCK_60;
        user_stake_info[1].lockedAmount = user_stake_info[1].lockedAmount.add(_amount);
        } else if (_days ==90) {
        user_stake_info[2].expire = block.timestamp + _TIMELOCK_90;
        user_stake_info[2].lockedAmount = user_stake_info[2].lockedAmount.add(_amount);
        } else if (_days ==180) {
        user_stake_info[3].expire = block.timestamp + _TIMELOCK_180;
        user_stake_info[3].lockedAmount = user_stake_info[3].lockedAmount.add(_amount);
        } else if (_days ==365) {
        user_stake_info[4].expire = block.timestamp + _TIMELOCK_365;
        user_stake_info[4].lockedAmount = user_stake_info[4].lockedAmount.add(_amount);
        } else if (_days ==540) {
        user_stake_info[5].expire = block.timestamp + _TIMELOCK_540;
        user_stake_info[5].lockedAmount = user_stake_info[5].lockedAmount.add(_amount);
        } else if (_days ==730) {
        user_stake_info[6].expire = block.timestamp + _TIMELOCK_730;
        user_stake_info[6].lockedAmount = user_stake_info[6].lockedAmount.add(_amount);
        }
    ...
    function relockStake(uint256 _days) external {
        ...
        for (uint256 s = 0; s < 7; s += 1){
             if (user_stake_info[s].expire < block.timestamp) {
                 user_stake_info[s].lockedAmount = 0;
             }
        }    
        if (_days ==30) {    
        user_stake_info[0].expire = block.timestamp + _TIMELOCK_30;
        user_stake_info[0].lockedAmount = expired_amount;
        } else if (_days ==60) {
        user_stake_info[1].expire = block.timestamp + _TIMELOCK_60;
        user_stake_info[1].lockedAmount = expired_amount;
        } else if (_days ==90) {
        user_stake_info[2].expire = block.timestamp + _TIMELOCK_90;
        user_stake_info[2].lockedAmount = expired_amount;
        } else if (_days ==180) {
        user_stake_info[3].expire = block.timestamp + _TIMELOCK_180;
        user_stake_info[3].lockedAmount = expired_amount;
        } else if (_days ==365) {
        user_stake_info[4].expire = block.timestamp + _TIMELOCK_365;
        user_stake_info[4].lockedAmount = expired_amount;
        } else if (_days ==540) {
        user_stake_info[5].expire = block.timestamp + _TIMELOCK_540;
        user_stake_info[5].lockedAmount = expired_amount;
        } else if (_days ==730) {
        user_stake_info[6].expire = block.timestamp + _TIMELOCK_730;
        user_stake_info[6].lockedAmount = expired_amount;
        }
    ...
    function emergencyRemoveStake(address _user) external onlyOwner{
        Locked[7] storage user_stake_info = userStake[_user];
        uint256 lockedAmount = stakeOf(_user);
        require( (lockedAmount>0) ,"Please wait until some stakes are expired");
        _burn(_user,lockedAmount);
         for (uint256 s = 0; s < 7; s += 1){
                 user_stake_info[s].lockedAmount = 0;
        }
         stakes[_user] = stakes[_user].sub(lockedAmount);
       if(stakes[_user] == 0) removeStakeholder(_user);
        bool transfer_done = BOLLY.transfer(_user, lockedAmount);
        require(transfer_done,"BOLLY transfer failed");
        emit RemoveStake(lockedAmount,_user);
    }
