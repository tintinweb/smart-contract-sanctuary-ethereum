...
contract Trollz is ERC721A, Ownable, ReentrancyGuard{
    ...
    uint public price = .0001 ether;
    uint public maxFreeMints = 5;
    uint public maxPaidMints = 10;
    ...
    //False on mainnet
    enum SaleStatus  {INACTIVE,FREE,PAID}
    SaleStatus public saleStatus = SaleStatus.FREE; //todo: change on mainnet
    ...
    function freeMint(uint amount) external {
        if(saleStatus != SaleStatus.FREE) revert SaleNotStarted();
        if(totalSupply() + amount > maxSupply) revert SoldOut();
        if(tokenMints[msg.sender][1] + amount > maxFreeMints) revert MaxMints();
        tokenMints[msg.sender][1] += amount;

        _mint(msg.sender,amount);
    }
    function publicMint(uint amount) external payable {
        if(saleStatus != SaleStatus.PAID) revert SaleNotStarted();
        if(msg.value < amount * price) revert Underpriced();
        if(totalSupply() + amount > maxSupply) revert SoldOut();
        if(tokenMints[msg.sender][2] + amount > maxFreeMints) revert MaxMints();
        tokenMints[msg.sender][2] += amount;
    ...
    function setFreeOn() external onlyOwner {
        saleStatus = SaleStatus.FREE;
    }
    function setPaidOn() external onlyOwner {
