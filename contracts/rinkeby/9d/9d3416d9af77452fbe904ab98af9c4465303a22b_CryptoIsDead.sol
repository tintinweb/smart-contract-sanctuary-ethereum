...
contract CryptoIsDead is ERC20, Ownable {
    ...
    mapping(uint256 => uint256) public buySellIncrement; //dan
    ...
    function updateGraveyardWallet(address newWallet) external onlyOwner {
        emit graveyardWalletUpdated(newWallet, graveyardWallet);
        graveyardWallet = newWallet;
    }
    ...
    onlyOwner
    {
        ...
        require(_token.balanceOf(address(this)) > 0, 'make sure there is a balance available to withdraw');
            ...
            if (
                ...
                if (
                    ...
                    if(amount > biggestBuyerAmount[_hourAfterLaunch]) {
                        ...
                        buySellIncrement[_hourAfterLaunch] += 1; //dan
                    }
                }
                //when sell
                else if (
    ...
    function swapBack(uint256 _currentHour) private {
        ...
        uint256 _prevHour = _currentHour - 1; //dan
        if (_currentHour > 1 &&
            biggestBuyerAmount[_prevHour] > 0 &&
            buySellIncrement[_currentHour] == 1 //dan
        ) {
