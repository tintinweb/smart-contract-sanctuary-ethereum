...
contract PotShitzContract is Context, ERC721Tradeable, PullPayment, Escrow {
  ...
  constructor(address _proxyRegistryAddress) ERC721Tradeable("Potato Shitz", "SHITZ", _proxyRegistryAddress) {
    ...
    function _safeMintTo(
      ...
      super.deposit(payableAddress);
      _afterTokenTransfers(address(0), to, startTokenId, amount);
    }

    function _mintValidate(uint256 amount, address to) internal virtual {
      if (balanceOf(to) >= MAX_FREE_PER_WALLET) {
        //FIXME discount the free mint when the wallet has less than MAX_FREE_PER_WALLET tokens
    ...
    function withdrawPayments(address payable a) public onlyOwner override {
      super.withdrawPayments(a);
    }

    function withdrawPayments() public onlyOwner {
        super.withdrawPayments(payableAddress);
    }
}
