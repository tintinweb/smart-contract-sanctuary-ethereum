...
contract Fracture is ERC721A, EIP2981AllToken, Ownable {
    using Strings for uint256;

    bool public isEditioned;
    uint256 public immutable maxEditions;
    uint256 public numberOfEditions;
    ...
    /**
    *   @param _name is the name of the contract
    ...
    *   @param _maxEditions is the number of maximum editions
    */
    ...
    *   @dev requires owner or admin
    *   @dev requires that the next token to be minted is token 0
    ...
    *   @dev requires a number of editions less than or equal to maxEditions
    *   @dev requires isEditioned to be false
    *   @param _numOfEditions is the number of editions to make. Can be set between 1 and maxEditions. This number is the total number of pieces that will live on this contract
    */
    function createEditions(uint256 _numOfEditions) external {
        require(msg.sender == ERC721A.ownerOf(1), "Caller is not owner of token 1");
        require(_numOfEditions <= maxEditions, "Cannot set number of editions above max");
        require(!isEditioned, "Already is editioned");

        isEditioned = true;
        numberOfEditions = _numOfEditions;
        if (_numOfEditions > 1) {
            _mint(msg.sender, _numOfEditions - 1);
        }
    }
    ...
    */
    function tokenURI(uint256 tokenId) override public view returns(string memory) {
        ...
        string memory name = "Fracture";
        string memory attr = "[]";
        if (isEditioned && numberOfEditions > 1) {
            name = string(abi.encodePacked(name, ' #', tokenId.toString(), '/', numberOfEditions.toString()));
