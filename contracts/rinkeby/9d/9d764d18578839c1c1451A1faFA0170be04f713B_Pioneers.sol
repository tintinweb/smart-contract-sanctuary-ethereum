...
contract Pioneers is IERC721, IERC721Metadata{ 
    ...
    function stage() public view returns(Stage){ // TODO: Enable block.timestamp check
        ...
        }else if(totalSupply() > ((3*tokenIndexToWagon.length)/4) && totalSupply() <= tokenIndexToWagon.length){
             return Stage.Journey1;
        }else if(totalSupply() > tokenIndexToWagon.length/2 && totalSupply() <= ((3*tokenIndexToWagon.length)/4)){
