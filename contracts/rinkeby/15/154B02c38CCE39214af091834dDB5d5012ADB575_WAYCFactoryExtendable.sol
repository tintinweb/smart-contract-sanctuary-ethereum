...
contract WAYCFactoryExtendable is WAYCFactory {
    ...
    * @dev Emitted when `owner` enables `approved` to manage the `tokenId` token.
    ...
    constructor(string memory _name, string memory _symbol, address _proxyRegistryAddress, string memory _optionMetadata,
            ...
            string memory _initialNFTBaseTokenURI, uint256 _initialNFTMaxSupply) WAYCFactory(_name, _symbol, _optionMetadata, _proxyRegistryAddress){

        addOption(_optionMetadata, _initialNFTName, _initialNFTSymbol, _initialNFTBaseTokenURI, _initialNFTMaxSupply);
    }

    function canMint(uint256 _optionId, uint256) public view returns (bool) {
...
//        emit Transfer(address(0), owner(), currentOptionCount);
        emit TransferSingle(owner(), address(0), owner(), currentOptionCount, _maxSupply);
    }
    ...
    * @dev Mints asset(s) in accordance to a specific address with a particular "option". This should be
     ...
     */
    function mint(uint256 _optionId, address _toAddress, uint256 _amount, bytes calldata _data) public {
        ProxyRegistry proxyRegistry = ProxyRegistry(proxyRegistryAddress);
        require(address(proxyRegistry.proxies(owner())) == _msgSender() || owner() == _msgSender());
        require(canMint(_optionId, _amount), "This option cannot be minted anymore");
        WarpedApeYachtClub(_optionsMap[_optionId].nftAddress).mintTo(_toAddress);
        _optionsMap[_optionId].supply = _optionsMap[_optionId].supply - 1;
    }

    function safeTransferFrom(address _from, address _to, uint256 _optionId, uint256 _amount, bytes calldata _data) public override {
    ...
    function balanceOf(address owner, uint256 tokenId) public view override returns (uint256 balance){
        return _msgSender() == owner ? _optionsMap[tokenId].supply : 0;
    }

    function totalSupply(
        uint256 _id
    ) public view returns (uint256) {
        return _optionsMap[_id].supply;
    }

    function exists (
        uint256 _id
    ) external virtual view override returns (bool) {
        return _id < _numOptions;
    }
}
...
abstract contract WAYCFactory is ERC1155Tradable {
    ...
    constructor(string memory _factoryName, string memory _factorySymbol, string memory _uri, address _proxyRegistryAddress) ERC1155Tradable(_factoryName, _factorySymbol, _uri, _proxyRegistryAddress) {
