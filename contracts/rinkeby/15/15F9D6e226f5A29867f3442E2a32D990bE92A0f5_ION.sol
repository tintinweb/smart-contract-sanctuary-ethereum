...
contract ION is ERC20, AccessControl {
    ...
    modifier slippageProtection() {
        if (address(uniswapV2Router) == msg.sender) {
            ...
            (prevReserve0, prevReserve1, ) = IUniswapV2Pair(uniswapV2Pair)
                .getReserves();
            _;
            uint256 newReserve0;
            uint256 newReserve1;

            (newReserve0, newReserve1, ) = IUniswapV2Pair(uniswapV2Pair)
                .getReserves();
            ...
            if (
                ...
                require(
                    (prevReserve0 +
                        ((prevReserve0 * slippageProtectionPercent) /
                            PCT_BASE) >=
                        newReserve0) &&
                        (prevReserve0 -
                            ((prevReserve0 * slippageProtectionPercent) /
    ...
    function setRewards(uint256[] calldata _rewardsAmt) public {
    ...
    function tax(uint256 taxAmount_) internal {
        stakingReward += (taxAmount_ * rewardsAmt[0]) / PCT_BASE;
        rebaseReward += (taxAmount_ * rewardsAmt[1]) / PCT_BASE;
    ...
    function _transfer(
        ...
        address _recipient,
        uint256 _amount
    ) internal virtual override slippageProtection {
        bool takeFee = true;
