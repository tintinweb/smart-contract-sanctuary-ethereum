...
library SaitamaskV1Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'5fa930bcb016556586d543fac944fb5999a07f1bb40d526e11aca780f9c41a9d' // init code hash
            ))));
    }
...
contract SaitamaskV1Router02 is ISaitamaskV1Router02 {
    ...
    function addLiquidity(
        ...
        liquidity = ISaitamaskV1Pair(pair).mint(to);
    }
    function addLiquidityETH(
        address token,
        uint amountTokenDesired,
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = ISaitamaskV1Pair(pair).burn(to);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to, uint feeMultiplier) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            ISaitamaskV1Pair(SaitamaskV1Library.pairFor(factory, input, output)).swap(
