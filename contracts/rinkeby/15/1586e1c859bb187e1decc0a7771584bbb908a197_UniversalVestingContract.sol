...
//        setPauseStatus(true);
    ...
    function addMinter(Investor[] memory vest) external onlyOwner {
        for (uint i = 0;i < vest.length;i++) {
            ...
            vesting.totalBalance = vest[i].amount *10**18;
            vesting.initialToBeClaimed = (initialAmountReleased[vest[i].investorType] * vesting.totalBalance) / 1000;
            vesting.intermediateToBeClaimed = (intermediateAmountReleased[vest[i].investorType] * vesting.totalBalance)/ 1000;
            vesting.linearToBeClaimed = (linearVestingAmountReleased[vest[i].investorType] * vesting.totalBalance ) / 1000;
