...
import './IMy721Contract.sol';
import './IMy1155Contract.sol';
...
contract MyMarket {
  ...
  IMy721Contract my721Contract;

  IMy1155Contract my1155Contract;
  ...
  constructor(uint _saleTime, uint _auctionTime, uint256 _bidStep,
    ...
    my721Contract = IMy721Contract(_721Contract);
    my1155Contract = IMy1155Contract(_1155Contract);
  ...
  function createItem(string memory _tokenURI) public {
    ...
    my721Contract.mint(msg.sender,
      newNftTokenId , _tokenURI);
  ...
  function listItem(uint256 _tokenId, uint256 _price) public  {
    ...
    TokenOnTrade memory tk = TokenOnTrade(
      {
        tokenId: _tokenId,
        owner: msg.sender,
        onSale: true,
    ...
    _tradeIds.increment();
    onTrade[_tradeIds.current()] = tk;
    emit ListedOnSale(_tokenId, _price, 1);
  }
  ...
  function listItem(uint256 _tokenId, uint256 _amount, uint256 _price) public {
    ...
    _tradeIds.increment();
    onTrade[_tradeIds.current()] = tk;
    emit ListedOnSale(_tokenId, _price, _amount);
  }
  ...
  function buyItem(uint256 _tradeId) public {
    ...
    SafeERC20.safeTransferFrom(currencyTokens, msg.sender, tk.owner, tk.price);
    tk.owner = msg.sender;
    tk.onSale = false;
    tk.amountOnTrade = 0;
  ...
  function buyItem(uint256 _tradeId, uint256 _amount) public {
    ...
    SafeERC20.safeTransferFrom(currencyTokens, msg.sender, tk.owner, tk.price * _amount);
  ...
  function listItemOnAuction(uint256 _tokenId, uint256 _minPrice) public {
    ...
    _tradeIds.increment();
    onTrade[_tradeIds.current()] = tk;
    emit ListedOnAuction(_tokenId, _minPrice, 1);
  }
  ...
  function listItemOnAuction(uint256 _tokenId, uint256 _amount, uint256 _minPrice) public {
    ...
    _tradeIds.increment();
    onTrade[_tradeIds.current()] = tk;
    emit ListedOnAuction(_tokenId, _minPrice, _amount);
  }
  ...
  function makeBid(uint256 _tradeId, uint256 _bid) public {
    ...
    require(_bid > tk.price + bidStep, "A bid should be greater than minPrice plus bid step");
    if (tk.lastBidder != address(0)) {
      SafeERC20.safeTransferFrom(currencyTokens, address(this), tk.lastBidder,
        tk.price);
    }
    tk.price = _bid;
    SafeERC20.safeTransferFrom(currencyTokens, msg.sender, address(this),
      tk.price);
    tk.biddersCount += 1;
  ...
  function finishAuction(uint256 _tradeId) public {
    ...
    if ((msg.sender == tk.owner) || ((tk.biddersCount < 2) &&
      ...
      tk.onAuction = false;
      SafeERC20.safeTransferFrom(currencyTokens, address(this), tk.owner,
        tk.price);
      emit Sold(tk.tokenId, tk.amountOnTrade, msg.sender);
    }
  }
}
...
interface IMy721Contract is IERC721 {

  event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);

  function mint(address _receiver, uint256 _tokenId,
...
interface IMy1155Contract is IERC1155{
