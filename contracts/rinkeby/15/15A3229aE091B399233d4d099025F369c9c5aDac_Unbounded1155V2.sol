...
contract Unbounded1155V2 is ERC1155, NonblockingLzApp, IERC1155Receiver, IONFT1155 {
  ...
  /**
    ...
    *@param minId: Minimum mintable Token Id of the NFT
    *@param maxId: Maximum mintable Token Id of the NFT
    *@param uri: Template URI for the NFT
    *@param mintedCount: Minted Count of the NFT
  ...
  struct Metaverse {
    ...
    uint256 id;
    string name;
    string metaverseTemplateMetadata;
  }
  ...
  //Base URI address("https://metada-uri.s3.eu-central-1.amazonaws.com/")
  ...
  uint256 public itemBaseCountId;
  ...
  //Stores NFT's Metaverse URI //NFT Token ID => Metaver ID => Metaverse URI
  mapping(uint256 => mapping(uint256 => string)) public nftIdMetaverseUri;

  //Stores NFT Template Information//NFT Base ID => BaseUriMinMax Struct
  mapping(uint256 => BaseUriMinMax) public itemNFTIdRangeAndUri;
  ...
  */
  ...
  constructor(string memory uri_, address _lzEndpoint) ERC1155(uri_) NonblockingLzApp(_lzEndpoint) {
    admin = msg.sender;
    // baseURI = "https://metada-uri.s3.eu-central-1.amazonaws.com/";
  }
   ...
   *@param _metaverseTemplateMetadata: Metaverse Metadata
   */
  function registerMetaverse(
    address _wallet,
    string memory _name,
    string memory _metaverseTemplateMetadata
  ) external OnlyAdmin {
    require(registeredMetaverses[_wallet].wallet == address(0), "This Metaverse has already registered!");
    metaverseIds++;
    registeredMetaverses[_wallet].wallet = _wallet;
    registeredMetaverses[_wallet].id = metaverseIds;
    registeredMetaverses[_wallet].name = _name;
    registeredMetaverses[_wallet].metaverseTemplateMetadata = _metaverseTemplateMetadata;

    emit MetaverseRegistered(_wallet, metaverseIds, _name, _metaverseTemplateMetadata);
  }
  ...
  function createNewItem(
    ...
    itemBaseCountId++;
    storeBaseUri(itemBaseCountId, _minId, _maxId, _uri);

    emit NewItemtemplateCreated(itemBaseCountId, _minId, _maxId, _uri);
  }
   ...
   */
  function createNewItemBatch(
    uint256[] memory _minIds,
    uint256[] memory _maxIds,
    string[] memory _uris
  ) external OnlyAdmin {
    for (uint256 i = 0; i < _minIds.length; i++) {
      itemBaseCountId++;
      storeBaseUri(itemBaseCountId, _minIds[i], _maxIds[i], _uris[i]);
    }
  }
   ...
   */
  function addMetaverseURIToNFTId(
    uint256 _baseId,
    uint256 _tokenId,
    ...
    //nftIdMetaverseUri[_tokenId][_metaverseId] = itemNFTIdRangeAndUri[_baseId].uri;
    nftIdMetaverseUri[_tokenId][_metaverseId] = registeredMetaverses[_metaverseWallet].metaverseTemplateMetadata;
    emit MetaverseURIAdded(_baseId, _tokenId, _metaverseId, registeredMetaverses[_metaverseWallet].metaverseTemplateMetadata);
  }
   ...
   */
  function updateNFTIdMetaverseUri(
    ...
    require(registeredMetaverses[_metaverseWallet].id == _metaverseId, "Not authorized!");
    require(registeredMetaverses[_metaverseWallet].wallet != address(0), "Unregistered Metaverse");
    //require(msg.sender == _metaverseWallet, "Wallet mismatch!");
    bool isVerified = verify(_signature, _metaverseWallet, _metaverseId, _tokenId, _newURI, _salt);
    if (isVerified) {
      nftIdMetaverseUri[_tokenId][_metaverseId] = _newURI;
  ...
  function mintNft(uint256 _baseId) external {
    require(_baseId != 0, "Invalid ID!");
    require(
      itemNFTIdRangeAndUri[_baseId].mintedCount < (itemNFTIdRangeAndUri[_baseId].maxId - itemNFTIdRangeAndUri[_baseId].minId),
      "Can't mint anymore please talk to admins!"
    );
    uint256 mintId = itemNFTIdRangeAndUri[_baseId].minId + itemNFTIdRangeAndUri[_baseId].mintedCount;
    _mint(msg.sender, mintId, 1, "0x00");
    tokenURI[mintId] = itemNFTIdRangeAndUri[_baseId].uri;
    itemNFTIdRangeAndUri[_baseId].mintedCount += 1;
  ...
  function mintAfterTransfer(
    ...
    _mint(_toAddress, _mintId, 1, "0x00");
    tokenURI[_mintId] = itemNFTIdRangeAndUri[_baseId].uri;
  ...
  function checkItemBaseId(uint256 _tokenId) external view returns (uint256 _baseID) {
    for (uint256 i = 1; i < (itemBaseCountId + 1); i++) {
      if (itemNFTIdRangeAndUri[i].minId <= _tokenId && _tokenId <= itemNFTIdRangeAndUri[i].maxId) {
        return itemNFTIdRangeAndUri[i].baseID;
      }
    }
    return 0;
  }
  ...
  function storeBaseUri(
    ...
    //min and max id should be checked somehow with before min and max id!
    itemNFTIdRangeAndUri[_baseId].baseID = _baseId;
    itemNFTIdRangeAndUri[_baseId].minId = _minId;
    itemNFTIdRangeAndUri[_baseId].maxId = _maxId;
    itemNFTIdRangeAndUri[_baseId].uri = _uri;
    /*     itemNFTIdRangeAndUri[_baseId].mintedCount = _minId; */
  }
...
interface IONFT1155 {
    ...
    function send(uint16 _dstChainId, bytes calldata _toAddress, uint _tokenId, uint _baseId, uint amount, address payable _refundAddress, address _zroPaymentAddress, bytes calldata _adapterParam) external payable;
