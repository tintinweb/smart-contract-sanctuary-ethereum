...
contract Paradigm is Ownable, ERC721A, ERC2981, PaymentSplitter {
    enum SaleStatus {
        Inactive,
        Genesis,
        Public,
    ...
    uint256 constant public WHITELIST_PRICE = 0.0025 ether;
    uint256 constant public PUBLIC_PRICE = 0.005 ether;
    uint256 constant public PUBLIC_SPOTS = 4000;
    ...
    constructor(address[] memory _devs, uint256[] memory _shares) ERC721A("Forgotten Ethereal Worlds Paradigm", "PRDM") PaymentSplitter(_devs, _shares) {
    ...
    function genesisMint(uint256 _proofMints, uint256 _mints, bytes32[] calldata _proof) external payable {
        require(saleStatus == SaleStatus.Genesis, "Paradigm: Genesis mint isn't active");
        require(totalSupply() + _mints <= MAX_WORLDS, "Paradigm: Exceeds Max Supply");
        require(genesisMints[msg.sender] + _mints <= _proofMints, "Paradigm: Already Minted");

        if(_proofMints >= 2 && genesisMints[msg.sender] == 0) {
            require(msg.value >= (_mints - 1) * WHITELIST_PRICE, "Paradigm: Insufficient funds");
        }
        else {
            require(msg.value >= _mints * WHITELIST_PRICE, "Paradigm: Insufficient funds");
        }
        ...
        genesisMints[msg.sender] += _mints;
    ...
    function publicMint(uint256 _mints) external payable {
        ...
        require(msg.value >= PUBLIC_PRICE * _mints, "Paradigm: Insufficient funds");
    ...
    function whitelistMint(uint256 _proofMints, uint256 _mints, bytes32[] calldata _proof) external payable {
        ...
        // if (msg.value != whitelistPrice * _mints) revert PaymentNotCorrect();
        require(msg.value == _mints * WHITELIST_PRICE, "Paradigm: Insufficient funds");
    ...
    function finalMint(uint256 _mints) external payable {
    ...
    function mintRemaining() external onlyOwner {
        _safeMint(payee(0), MAX_WORLDS - totalSupply());
    }
    ...
    function setMerkleMint(SaleStatus _newStatus, bytes32 _newRoot) external onlyOwner {
        saleStatus = _newStatus;
