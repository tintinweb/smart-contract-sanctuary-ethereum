...
contract Swappable is Ownable {
    ...
    function createSwap(
        ...
        for (uint i=0; i<_fromContracts.length; i++) {
            if (_fromStandards[i] == Standard.ERC_1155) {
                require (_fromAmount[i] > 0, 'from amount must be greater than 0 erc1155');
                ...
                require (_fromAmount[i] > 0, 'from amount must be greater than 0 erc20');
        ...
        for (uint i=0; i<_toContracts.length; i++) {
            if (_toStandards[i] == Standard.ERC_1155) {
                require (_toAmount[i] > 0, 'to amount must be greater than 0 erc1155');
                ...
                require (_toAmount[i] > 0, 'to amount must be greater than 0 erc20');
    ...
    /// Execute the swap
    function executeSwap(uint256 swapId, address payable recipient) public payable {
        ...
        for (uint i=0; i<swap._to.length; i++) {
            ...
            if (token._standard == Standard.ERC_1155) {
                ...
                // If eth that is passed into the method is greater than the ERC2
                erc20TransferFrom(
                    token._contract,
                    swap._toAddress,
                    swap._fromAddress,
                    token._amount);
            } else if (token._standard == Standard.ETH) {
                // Transfer any eth to the recipient address
                require (msg.value >= token._amount, "The eth standard must have the equivalent eth as part of the transaction");
                if (msg.value > 0) {
                    require(recipient == swap._fromAddress, "The recipient account must be equal to the from address");
                    recipient.transfer(token._amount);
                }
            } else {
                require(false, "Unexpected token in the to list");
            }
        }
        ...
        for (uint i=0; i<swap._from.length; i++) {
            ...
            if (token._standard == Standard.ERC_1155) {
                ...
                require(false, "Unexpected token in the from list");
            }
        }
