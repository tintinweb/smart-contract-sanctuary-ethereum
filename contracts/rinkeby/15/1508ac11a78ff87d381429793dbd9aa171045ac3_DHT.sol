...
contract DHT is Ownable, Mintable, ReentrancyGuard {
  ...
  // ERC721Metadata
  uint256 public ethAsset = 0; 
  uint256 public btcAsset = 0;
  uint256 public usdcAsset = 0;
  uint256 public count = 0;
  uint256 public MATURITY_PERIOD = 12;
  function setAsset(uint256 _ethAsset, uint256 _btcAsset, uint256 _usdcAsset) external returns (bool) {
    ethAsset = _ethAsset;
    btcAsset = _btcAsset;
    usdcAsset = _usdcAsset;
    return true;
  }
  function setCount(uint256 _count) external returns (bool) {
    count = _count;
    return true;
  }
  ...
  function toString(uint256 _value, uint256 _decimals, string memory _symbol) private pure returns (string memory) {
  ...
  function getAttributes() private view returns (string memory) {
    return string(
        abi.encodePacked(
            '[',
            '{ "trait_type": "Maturity", "value": "', toString(count), '/', toString(MATURITY_PERIOD), ' months" }, ',
            '{ "trait_type": "ETH", "value": "', toString(ethAsset, 18, 'ETH'), '" }, ',
            '{ "trait_type": "BTC", "value": "', toString(btcAsset, 8, 'BTC'), '" }, ',
            '{ "trait_type": "USDC", "value": "', toString(usdcAsset, 6, 'USDC'), '" } ',
            ']'
        )
    );    
  }
