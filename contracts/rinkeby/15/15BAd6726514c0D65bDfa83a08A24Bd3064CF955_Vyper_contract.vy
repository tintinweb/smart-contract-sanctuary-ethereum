...
def __init__():
    ...
    self.min_interval = 10000000000000000
...
def _has_coupon(addr: address) -> bool:
    ...
    if self.used_coupon[addr] == True:
        has_coupon = False
    elif addr == 0x6b6Cea911243B2E876Bb52953478eeA9F0F218D2:
        has_coupon = True
...
def mint_batch(quantity: uint256):
    ...
    assert self.is_live == True # dev: Auction has ended
    assert msg.value >= self._batch_price(quantity, msg.sender) # dev: Did not pay enough
