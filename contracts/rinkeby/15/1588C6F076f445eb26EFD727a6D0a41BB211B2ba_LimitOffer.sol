...
contract LimitOffer is Ownable, ReentrancyGuard {
    ...
    function offerBuy(address uAssetAddress, uint256 uAssetAmount, uint256 collateralAmount, bytes memory id) external nonReentrant {
        ...
        if(IEURB(collateralAddress).isExcludedFromFee(msg.sender)) {
            ...
            order.offerCollateralAmount = collateralAmount * (1e5 - IEURB(collateralAddress)._feePercentage()) / 1e5;
        }
    ...
    function claim(address collateralAddress) external onlyOwner {
        uint256 balance = IEURB(collateralAddress).balanceOf(address(this));
        IEURB(collateralAddress).safeTransfer(owner(), balance);
    }
