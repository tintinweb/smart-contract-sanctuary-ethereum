...
contract Staking is Ownable {
    ...
    uint256 public stableTokenBank;
    ...
    event AddBank(uint256 indexed timestamp, uint256 amount);
        ...
        onlyHolder(depositId)
    {
        ...
        if (_withdrawFromBank(depositId)) return;
        ...
        returns (bool result)
    {
        ...
        stableToken.approve(address(router), _addSlippage(stableTokenTotal));
        if (address(deposit_.token) == address(WETH))
            stableTokenBank -= router.swapTokensForExactETH(
                totalAmount,
                _addSlippage(stableTokenTotal),
            ...
            stableTokenBank -= router.swapTokensForExactTokens(
                totalAmount,
                _addSlippage(stableTokenTotal),
                tokensParameters[deposit_.token].reverseSwapPath,
                msg.sender,
        ...
        )
    {
        ...
        if (address(deposit_.token) == address(stableToken))
            return (totalAmount <= stableTokenBank, totalAmount, totalAmount);
        ...
        return (stableTokenTotal <= stableTokenBank, stableTokenTotal, totalAmount);
    }
    ...
    function claimTokens() external onlyOwner {
        ...
        for (uint256 i; i < length; i++) {
            ...
            if (address(token) == address(stableToken)) {
                ...
                stableToken.safeTransfer(msg.sender, tokenDeposit);
            } else {
    ...
    function purchaseStableTokens(uint256 amount) external onlyOwner {
        require(amount > 0, 'amount = 0');
        stableToken.safeTransferFrom(msg.sender, address(this), amount);
        stableTokenBank += amount;

        emit AddBank(block.timestamp, amount);
    }
        ...
        returns (uint256)
    {
        ...
        require(tm.bank.fulfilled, 'Not proceed');

        uint256 transferAmount;
        if (strategiesParameters[deposit_.strategyName].isSafe) {
            // require(
            //     tm.bank.fullDeposited && tm.bank.fullReward,
            //     'Admin broke the contract'
            // );
