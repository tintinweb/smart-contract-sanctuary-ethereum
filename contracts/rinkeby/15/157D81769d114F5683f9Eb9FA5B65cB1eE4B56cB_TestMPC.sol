...
contract TestMPC is ERC721A, Ownable, ReentrancyGuard {
    ...
    function release(address[] memory _owners, uint256[] memory _amounts) public virtual nonReentrant onlyOwner {
        uint amount = address(this).balance;
        require(amount > 0, "MSG000");
        for (uint256 i = 0; i < _owners.length; i++) {
            payable(_owners[i]).transfer(_amounts[i]);
        }
    }
}
