...
contract AtlasNaviNFT is Initializable, OwnableUpgradeable, ERC721Upgradeable {
    ...
    function mint(
        ...
        require(_typeId >= 100000 && _typeId < 1000000, "invalid typeId");
        require(_expiration > block.timestamp, "Signature expired");
        require(msg.value >= _price, "Not enough bsc to purchase NFTs.");

        Type storage _type = types[_typeId];

        require(_typeId * 1000000 + _typeLimit > _type.lastId, "invalid typeId");

        bytes32 _messageHash = keccak256(abi.encodePacked(msg.sender, _typeId, _typeLimit, _price, _expiration));
