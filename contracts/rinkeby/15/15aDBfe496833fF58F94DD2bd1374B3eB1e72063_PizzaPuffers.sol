...
contract PizzaPuffers is ERC721A, Ownable, ReentrancyGuard {
    ...
    // writer: contract
    // reader: contract
    uint16 private __public_free_claimed;
    ...
    function mint_wl(uint256 quantity, bytes32[] memory proof) internal {
        ...
        // free eligibility
        if ((__has_minted_free[msg.sender] == false) && ((__wl_free_claimed + 1) <= MAX_WL_FREE_TOKENS)) {
            // global free check
            require((__public_free_claimed + __og_free_claimed + __wl_free_claimed + 1) <= MAX_TOTAL_FREE_TOKENS, "over free total.");
    ...
    function mint_public(uint256 quantity) internal {
        ...
        // free eligibility, could roll over to public
        if ((__has_minted_free[msg.sender] == false) && ((__public_free_claimed + __og_free_claimed + __wl_free_claimed + 1) <= MAX_WL_FREE_TOKENS)) {

            unchecked{
                __public_free_claimed++;
            }
            __has_minted_free[msg.sender] = true;

            // refund one tokens value
            (bool sent, ) = payable(msg.sender).call{value: MINT_PRICE_PUBLIC}("");
            require(sent, "!refund");
        }
