...
contract HatchMusic2 is ERC1155, Ownable, ReentrancyGuard {
  ...
  modifier maxAmmount(uint256 ammount) {
    require(ammount <= maxPerTransaction);
    _;
  }
  ...
  ) public payable nonReentrant maxAmmount(_trackIds.length) {
    require(areSalesActive, "Sale isn't active");
    uint256 totalCost = 0;
    for (uint256 i = 0; i < _trackIds.length; ++i) {
      require(existingTrackId[_trackIds[i]], "This track id doesn't exist");
      totalCost += track[_trackIds[i]].trackCost * _amounts[i];
    }
    require((msg.value / 10000) * 10000 >= totalCost, "Wrong amount of ETH sent");
    for (uint256 i = 0; i < _trackIds.length; ++i) {
      uint256 trackCost = track[_trackIds[i]].trackCost;
      uint256 trackNetCost = trackCost - (trackCost * hatchMintingFee / 10000);
      for (uint256 j = 0; j < track[_trackIds[i]].artistsWallets.length; ++j) {        
        uint256 artistsShare = trackNetCost * track[_trackIds[i]].artistsShares[j] / 10000;
        artistBalance[track[_trackIds[i]].artistsWallets[j]] += artistsShare;       
      }            
    }
    hatchBalance = totalCost * hatchMintingFee / 10000;  
    _mintBatch(_to, _trackIds, _amounts, "");    
  }
  ...
  function createTrackPack(
    ...
    uint256 _rootId
      ...
      require(isCreateTrackPackActive, "Create track packs is closed");
      require(_artistsWallets.length <= maxArtistsPerTrack, "Too many artists");
      require(_artistsWallets.length == _artistsShares.length, "Wallets and shares format is not correct");
      bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
      require(MerkleProof.verify(_merkleProof, roots[_rootId], leaf), "Invalid Merkle Proof.");
      createTrackInternal(
        _packName,
        _trackNames,    
        _trackAminWallet,    
  ...
  function createTrackInternal(
    ...
    string[][] memory _artistsNames
      ...
      for (uint256 i = 0; i < _costs.length; ++i) {      
        require(_costs[i] >= minCreateTrackCost, "Track price is too low");
      ...
      for (uint256 i = 0; i < _trackNames.length; ++i) {
        ...
        track[currentTrackId].id = currentTrackId;
        track[currentTrackId].packId = currentPackId;
        track[currentTrackId].packName = _packName;
        track[currentTrackId].trackName = _trackNames[i];   
        setURI(currentTrackId, _uris[i]);
        track[currentTrackId].fullTrack = _fullTracks[i];     
        track[currentTrackId].trackCost = _costs[i];     
        track[currentTrackId].trackAdminWallet = _trackAminWallet; 
        track[currentTrackId].artistsWallets = _artistsWallets[i]; 
        track[currentTrackId].artistsShares = _artistsShares[i]; 
        track[currentTrackId].artistsNames = _artistsNames[i]; 
        existingTrackId[currentTrackId] = true; 
        emit TracksCreated(track[currentTrackId], currentPackId, _trackAminWallet);       
      }
      existingPackId[currentPackId] = true;  
      emit PacksCreated(_packName, currentPackId, _trackAminWallet);
  }
  ...
  function updateTrack(
    ...
    string[] memory _artistsNames
      ...
      require(track[id].trackAdminWallet == msg.sender, "Not your tracks");
      track[id].trackName = _trackName;      
      setURI(track[id].id, _uri);
      track[id].fullTrack = _fullTrack;
      track[id].trackCost = _cost;
      track[id].artistsWallets = _artistsWallets;
      track[id].artistsShares = _artistsShares;
      track[id].artistsNames = _artistsNames;
  ...
  function featurePack(
    ...
    require(isFeaturePackActive, "Feature collections is not active");
    require(existingPackId[_packId], "This collection doesn't exist");
    require(!featuredPackIds[_packId], "This collection is already featured");
    require(msg.value >= featurePackPrice, "Invalid ammount sent");
    featuredPackIds[_packId] = true;    
    hatchBalance = msg.value;
    emit NewFeaturedPacks(_packId, msg.value);
  }

  function getTrack(uint _id) public view returns (Track memory) {
    require(balanceOf(msg.sender, _id) > 0, "You don't own this track");
    return track[_id];
  }
  
  function getArtistBalance() public view returns (uint256) {
    return artistBalance[msg.sender];
  }
  
  function getHatchBalance() public view onlyOwner returns (uint256) {
    return hatchBalance;
  }

  function uri(uint _id) public override view returns (string memory) {
    return track[_id].trackUri;
  }

  function setURI(uint _id, string memory _uri) internal {
    track[_id].trackUri = _uri;
    emit URI(_uri, _id);
  }
  ...
  function setMinCreateTrackCost(uint256  _minCreateTrackCost) public onlyOwner {
    minCreateTrackCost = _minCreateTrackCost;
  }

  function setMaxArtistsPerTrack(uint256  _maxArtistsPerTrack) public onlyOwner {
    maxArtistsPerTrack = _maxArtistsPerTrack;
  }
  ...
  function withdrawForArtist() public nonReentrant {
    require(artistBalance[msg.sender] > 0, "You balance is 0");
    sendEth(msg.sender, artistBalance[msg.sender]);
    artistBalance[msg.sender] = 0;
  }
  
  function withdrawForHatch() public onlyOwner nonReentrant {
    require(hatchBalance > 0, "You balance is 0");
    sendEth(hatchWallet, hatchBalance);
    hatchBalance = 0;
  }
}
