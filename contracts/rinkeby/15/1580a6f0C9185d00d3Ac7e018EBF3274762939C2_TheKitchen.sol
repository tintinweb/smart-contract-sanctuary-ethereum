...
contract TheKitchen is ERC1155Supply, Ownable, ReentrancyGuard {
    ...
    modifier maxPerWalletCheck(uint256 _itemType, uint256 _quantity) {
        require(
            itemsMinted[msg.sender][_itemType] + _quantity <=
                itemTypes[_itemType].maxPerWallet,
    ...
    function setItemValid(uint256 _itemType, bool _valid)
        external
        onlyOwner
    {
    ...
    function setmaxPerWallet(uint256 _maxPerWallet, uint256 _itemType)
        external
        onlyOwner
        itemTypeCheck(_itemType)
    {
        itemTypes[_itemType].maxPerWallet = _maxPerWallet;
    }

    function getmaxPerWallet(uint256 _itemType)
        public
        view
        itemTypeCheck(_itemType)
        returns (uint256)
    {
        return itemTypes[_itemType].maxPerWallet;
    }
    ...
    function mintSale(uint256 _itemType, uint256 _quantity)
        ...
        payable
        itemTypeCheck(_itemType)
        maxPerWalletCheck(_itemType, _quantity)
        nonReentrant
    {
        require(
    ...
    function airdrop(uint256 _itemType, uint256 _quantity, address[] calldata _airdropAddresses)
        external
        onlyOwner
        itemTypeCheck(_itemType)
        nonReentrant
    {
         for (uint i = 0; i < _airdropAddresses.length; i++) {
            ...
            require(itemsMinted[_airdropAddresses[i]][_itemType] + _quantity <= itemTypes[_itemType].maxPerWallet, "User is trying to mint more than allocated.");
            require(totalSupply(_itemType) + _quantity <= itemTypes[_itemType].itemSupply, "User is trying to mint more than total supply.");
            itemsMinted[_airdropAddresses[i]][_itemType] += _quantity;
            _mint(_airdropAddresses[i], _itemType, _quantity, "");
         }
    }
