    ...
    ReentrancyGuard
{
    ...
    modifier notContract() {
        require(!_isContract(msg.sender), "BM: contract not allowed");
        require(msg.sender == tx.origin, "BM: proxy not allowed");
        _;
    }
    ...
    function publicMint(uint16 qty)
        ...
        nonReentrant
    {
        require(publicSaleConfig.startTime != 0, "BM: expired or ended");
        require(
            block.timestamp >= publicSaleConfig.startTime,
            "BM: sale not started"
        );
        require(qty <= maxQtyPerBatchMint, "BM: invalid max qty");
        require(
            _totalMinted() + qty <= collectionSize,
            "BM: reached max supply"
        );
        require(
            qty * publicSaleConfig.price == msg.value,
            "BM: invalid pay amount"
        );
    ...
    function whitelistMint(uint16 qty, bytes32[] calldata proof)
        ...
        nonReentrant
    {
        require(whitelistSaleConfig.startTime != 0, "BM: WL expired or ended");
        require(
            block.timestamp >= whitelistSaleConfig.startTime,
            "BM: WL sale not started"
        );
        require(verifyMerkleProof(msg.sender, proof), "BM: invalid WL proof");
        require(qty <= whitelistSaleConfig.maxPerAddress, "BM: invalid qty");
        require(
            _totalMinted() + qty <= collectionSize,
            "BM: reached max supply"
        );
    ...
    {
        require(_exists(tokenId), "BM: nonexistent token");
