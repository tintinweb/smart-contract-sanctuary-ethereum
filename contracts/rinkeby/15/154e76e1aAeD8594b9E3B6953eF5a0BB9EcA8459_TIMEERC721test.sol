...
contract TIMEERC721test is Ownable, ERC721A, Pausable, AccessControl, ReentrancyGuard {
    // Type Declarations
    // Variables
    uint256 public immutable maxSupply;
    uint256 private constant MAX_BATCH_SIZE = 5;
    uint256 public constant ALLOW_LIST_ALLOWED_QUANTITY = 4;
    uint256 public constant PUBLIC_ALLOWED_QUANTITY = 2;
    ...
    mapping(address => uint256) public amountNFTsPerAllowlistSale;
    ...
    ) external payable whenNotPaused _notContract nonReentrant {
        require(
            amountNFTsPerAllowlistSale[msg.sender] + _quantity <= ALLOW_LIST_ALLOWED_QUANTITY,
            "You can only get 4 NFT on the Whitelist Sale"
        );
        require(totalSupply() + _quantity <= maxSupply, "Max supply exceed");
        uint256 totalPrice = allowlistSalePrice * _quantity;
        require(msg.value >= totalPrice, "Not enough funds");
        require(isWhiteListed(_proof, merkleRoot, msg.sender), "Wallet not Whitelisted");
        amountNFTsPerAllowlistSale[msg.sender] += _quantity;
    ...
    function publicSaleMint(uint256 _quantity)
        ...
        nonReentrant
    {
        require(amountNFTsPerPublicSale[msg.sender] + _quantity <= PUBLIC_ALLOWED_QUANTITY, "You can only get 2 NFT on the Public Sale");
        require(totalSupply() + _quantity <= maxSupply, "Max supply exceed");
    ...
    function supportsInterface(bytes4 interfaceId) public view override(AccessControl, ERC721A) returns (bool) {
        return interfaceId == type(IERC721A).interfaceId || super.supportsInterface(interfaceId);
    }
