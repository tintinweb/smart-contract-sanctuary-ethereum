...
contract Marketplace {
    ...
    function createItem(address to, uint256 tokenId, uint256 amount) external {
        require(amount != 0, "Amount cannot be zero");
        require(nfts[tokenId].owner == address(0), "Token with such id exists");
        if (amount == 1)
            mintERC721(to, tokenId);
        else mintERC1155(to, amount, tokenId);
        nfts[tokenId] = NFT(
            {
                owner: to,
                amount: amount,
    ...
    function mintERC721(address to, uint256 tokenId) internal {
        erc721.mintTo(to, tokenId);
    }

    function mintERC1155(address to, uint256 amount, uint256 tokenId) internal {
        erc1155.mintTo(to, amount, tokenId);
    }

    function listItem(uint256 tokenId, uint256 price) external {
        require(nfts[tokenId].sold == false && nfts[tokenId].forSale == false, "Already sold or listed");
        nfts[tokenId].forSale = true;
        nfts[tokenId].price = price;
    }

    function cancel(uint256 tokenId) external {
        require(nfts[tokenId].sold == false && nfts[tokenId].forSale == true, "Sold or not listed");
