...
contract OMGKirby is ERC721, Ownable {
  ...
  uint public claimTimeLock = claimStartTime + 2 hours;
  uint public whitelistStartTime;
  uint public whitelistTimeLock = whitelistStartTime + 2 hours;
  ...
  function claimMint(uint256[] memory _ids) external {
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      if(token.ownerOf(_ids[i]) != msg.sender) revert NotTokenOwner();
    }
