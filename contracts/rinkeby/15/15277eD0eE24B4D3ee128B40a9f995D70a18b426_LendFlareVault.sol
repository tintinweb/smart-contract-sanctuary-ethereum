    ...
    ILendFlareVault
{
    ...
    function borrowForDeposit(
        ...
        ILendingMarket.UserLending memory userLending = ILendingMarket(
            lendingMarket
        ).getUserLastLending(address(this));

        UserInfo storage _userInfo = userInfo[_pid][msg.sender];
        _userInfo.lendingIndex++;

        userLendings[_pid][msg.sender][_userInfo.lendingIndex] = userLending
            .lendingId;

        lendings[userLending.lendingId] = Lending({
            pid: _pid,
            user: msg.sender,
