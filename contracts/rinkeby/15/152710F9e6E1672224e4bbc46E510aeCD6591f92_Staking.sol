...
contract Staking is IStaking, ERC721, Ownable, BalanceNames {
    address public nend;
    address public commissionPool;
    Vault public lendingPool;
    mapping(address => bool) public activeStakeTokens;
    mapping(address => bool) public addedStakeTokens;
    ...
    mapping(address => uint256) internal epochIfpTokenToAmount;
    ...
    constructor(
        ...
        // Add native token
        addedStakeTokens[address(0)] = true;
        activeStakeTokens[address(0)] = true;
        stakeTokens.push(address(0));
        stakeTokenCount++;

        // Add nend
        addedStakeTokens[_nend] = true;
    ...
    function deposit(
        ...
        _deposit(
            _stakeToken,
            _amount,
            rewardAllocations[_durationId],
            uint48(block.timestamp + stakeDurations[_durationId]),
            msg.sender,
            false
        );
    }
    ...
    function _deposit(
        ...
        totalRewardAllocation[_stakeToken] += _rewardAllocation;
    ...
    function distributeRewards() public override {
        for (uint256 i = 0; i < stakeTokens.length; i++) {
            epochIfpTokenToAmount[stakeTokens[i]] = lendingPool.getNamedBalance(
                interestForPoolBalanceName,
                stakeTokens[i]
            );
        }

        uint256 tradeCommission = lendingPool.getNamedBalance(
            tradeCommissionBalanceName,
            nend
        );

        uint256 inflationReward = IERC20(nend).balanceOf(address(this));
        uint256 initialEscrowCount = escrows.length;

        uint256[] memory stakeIds = new uint256[](stakeCount);
        uint256[] memory claimedStakeToken = new uint256[](stakeCount);
        uint256[] memory claimedNend = new uint256[](stakeCount);
        uint256 idx;
        ...
        for (uint256 i = 0; i < stakes.length; i++) {
            ...
            uint256 depositPortion = (stake.amountStaked * 10000) /
                totalAmountStaked[stake.stakeToken];
            uint256 rewardAllocationPortion = (stake.rewardAllocation * 10000) /
                totalRewardAllocation[stake.stakeToken];
            uint256 portion = (depositPortion + rewardAllocationPortion) / 2;

            uint256 stakeTokenReward = (epochIfpTokenToAmount[
                stake.stakeToken
            ] * portion) / 10000;

            if (stakeTokenReward > 0) {
                ...
                stake.claimedStakeToken += stakeTokenReward;
            }

            uint256 nendReward = (tradeCommission * portion) /
                10000 /
                stakeTokenCount;

            if (nendReward > 0) {
                _lendingPoolTransfer(nend, stake.staker, nendReward);
                lendingPool.namedBalanceSpend(
                    tradeCommissionBalanceName,
                    nend,
                    nendReward
                );
                stake.claimedNend += nendReward;
            }

            uint256 escrowReward = (inflationReward * portion) /
                10000 /
                stakeTokenCount;

            if (escrowReward > 0 && !stake.eabBound) {
                uint48 claimAfter = uint48(block.timestamp + 30 weeks);
            ...
            stakeIds[idx] = i;
            claimedStakeToken[idx] = stake.claimedStakeToken;
            claimedNend[idx] = stake.claimedNend;
            idx++;
        }

        for (uint256 i = initialEscrowCount; i < escrows.length; i++) {
            Escrow storage escrow = escrows[i];

            uint256 childStakeId = _deposit(
                nend,
                escrow.amount,
                30,
                escrow.claimAfter,
            ...
            escrow.childStakeId = childStakeId;

            emit Escrowed(
                escrow.parentStakeId,
                childStakeId,
                i,
                escrow.amount,
                escrow.claimAfter,
        ...
        emit RewardsClaimed(stakeIds, claimedStakeToken, claimedNend);
    }

    function issueEAB(uint256 escrowId) external {
        Escrow storage escrow = escrows[escrowId];
        require(escrow.owner == msg.sender, "Not escrow owner");
        require(!escrow.eabIssued, "EAB already issued");
        require(!escrow.canceled, "Escrow canceled");

        escrow.eabIssued = true;
        _mint(msg.sender, escrowId + 1);

        emit EABIssued(escrowId);
    }

    function exchangeEAB(uint256 _escrowId) external override {
        require(ownerOf(_escrowId + 1) == msg.sender, "Not EAB owner");
        Escrow storage _escrow = escrows[_escrowId];
        require(
            _escrow.claimAfter <= block.timestamp,
            "Too soon to exchange EAB"
        );

        uint256 amount = _escrow.amount;

        _escrow.claimed = true;

        if (!stakes[_escrow.childStakeId].unstaked) {
            _unstake(_escrow.childStakeId);
        }
    ...
    function unstake(uint256 _stakeId) external override {
        ...
        require(stake.staker == msg.sender, "Not own stake");
        require(!stake.unstaked, "Already unstaked");
        require(!stake.eabBound, "Bound to EAB");

        if (stake.stakeToken != address(0)) {
            require(
                IERC20(stake.stakeToken).balanceOf(address(lendingPool)) >=
                    stake.amountStaked,
                "Insufficient pool balance"
            );
        } else {
            require(
                address(lendingPool).balance >= stake.amountStaked,
                "Insufficient pool balance"
            );
        }

        _lendingPoolTransfer(stake.stakeToken, msg.sender, stake.amountStaked);

        for (uint256 i = 0; i < escrows.length; i++) {
            Escrow storage escrow = escrows[i];
            if (escrow.owner == msg.sender && !escrow.claimed) {
                ...
                if (!stakes[escrow.childStakeId].unstaked) {
                    _unstake(escrow.childStakeId);
                }

                _lendingPoolTransfer(nend, commissionPool, escrows[i].amount);

                emit EscrowCanceled(i);
            }
        }
    ...
    function autoUnstake() external override onlyOwner {
        if (stakeCount == 0) {
            return;
        }
        for (uint256 i = 0; i < stakes.length; i++) {
            Stake memory stake = stakes[i];
            if (stake.unstaked || stake.end > block.timestamp) {
                continue;
            }
            ...
            if (!stake.eabBound) {
                ...
                if (
                    (isNativeCoin &&
                        address(lendingPool).balance < stake.amountStaked) ||
                    (!isNativeCoin &&
                        IERC20(stake.stakeToken).balanceOf(
                            address(lendingPool)
                        ) <
                        stake.amountStaked)
                ) {
    ...
    function _unstake(uint256 stakeId) internal {
        ...
        stake.unstaked = true;
        stakeCount--;
        totalAmountStaked[stake.stakeToken] -= stake.amountStaked;
        totalRewardAllocation[stake.stakeToken] -= stake.rewardAllocation;
    ...
    function _afterTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            Escrow storage _escrow = escrows[tokenId - 1];
            ...
            if (block.timestamp < _escrow.claimAfter) {
                _escrow.childStakeId = _deposit(
                    nend,
                    _escrow.amount,
                    30,
                    _escrow.claimAfter,
            ...
            emit EscrowOwnerChanged(tokenId, from, to, _escrow.childStakeId);
        }
    }
    ...
    modifier onlyNEND() {
        require(msg.sender == nend, "Only Nend");
        _;
    }
}
