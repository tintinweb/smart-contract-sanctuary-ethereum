...
// File: contracts/ThTest6.sol

pragma solidity >=0.7.0 <0.9.0;


contract ThTest6 is ERC721A {
  using Strings for uint256;
  ...
  constructor() ERC721A("Th Test 6", "THT6") {
  ...
  modifier mintCompliance(uint256 _mintAmount) {
   if (msg.sender != owner()) {
    ...
    if (msg.sender != owner()) {
    require (ownerMintedCount + _mintAmount <= maxMintAmountPerWallet, "max per address exceeded!");
    }
    _;
  }
  ...
  function mintWhitelist(uint256 _mintAmount, address _receiver) public mintCompliance(_mintAmount) onlyOwner {
