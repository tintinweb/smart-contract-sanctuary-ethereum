...
constructor(){}
    ...
    function register_subdomain(string memory domain, string memory subdomain) external payable {
        ...
        ensusers[enssubdomains_count] = ensdata(subdomain, domain, msg.sender, "Under process",msg.value);
    ...
    function update_subdomain(uint256 index, string memory domain) public {
    ...
    function cancel_subdomain(uint256 index, string memory domain) public {
        for (uint i=0; i< ensdomains_count; i++) {
            if(keccak256(bytes(ensdomains[i].domain)) == keccak256(bytes(string(abi.encodePacked(domain)))) ){
                if(ensdomains[i].ownner == msg.sender ){
                    ensusers[index].status =  "Cancel";
                }
            }
        }
    }
