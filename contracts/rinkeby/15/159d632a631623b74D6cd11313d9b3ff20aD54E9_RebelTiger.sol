 ...
 * @notice      An ERC721 extension with added functionality for the desired minting scenarios.
 ...
 *   \/_/ /_/  \/_/  \/_____/
 */
contract RebelTiger is ERC721, Ownable {
    using Counters for Counters.Counter;

    // Sale related constants
    uint256 constant MAX_SUPPLY_SHIFTED = 51; // total number of purchasable tigers is 7700
    uint256 constant NUM_TIGERS_SHIFTED = 61; // total number of tigers is 7777 including gifts
    uint256 constant WL_PRICE = 0.066 ether;
    uint256 constant PRICE = 0.077 ether;
    uint256 constant WL_CAP_SHIFTED = 6; // a whitelisted address can mint 5 NFTs at maximum
    uint256 constant MINT_CAP_SHIFTED = 21; // one public sale transaction can mint 20 NFTs at maximum

    // Sale States
    uint256 public saleState = 0; // { 0: INACTIVE, 1: PRE_SALE, 2: PUBLIC_SALE }

    // Merkle root for off-chain whitelist verification
    bytes32 public merkleRoot;

    // Mapping from whitelist addresses to their mint counts
    ...
    constructor() ERC721("RebelTiger", "RTC") {
     ...
     * @dev     Uses Merkle Tree verification to store whitelisted addresses off-chain so as to reduce gas.
     * @param amount Amount of NFTs attempted to be minted in the transaction.
     * @param merkleProof Merkle proof provided by the server to verify whether the sender is whitelisted.
     */
    function mintPreSale(uint256 amount, bytes32[] calldata merkleProof) external payable {
        ...
        require(msg.value >= amount * WL_PRICE, "RTC: Insufficient funds.");

        uint256 currentId = idTracker.current();
        require(currentId + amount - 1 < MAX_SUPPLY_SHIFTED, "RTC: Purchasable NFTs are all minted.");

        require(whitelistMints[msg.sender] + amount <  WL_CAP_SHIFTED, "RTC: Transaction exceeds whitelist mint cap.");
    ...
    function mintPublicSale(uint256 amount) external payable {
        ...
        require(amount < MINT_CAP_SHIFTED, "RTC: Amount exceeds mint cap.");
        require(msg.value >= amount * PRICE, "RTC: Insufficient funds.");
    ...
    function mintGifts() external onlyOwner {
        ...
        require(currentId < NUM_TIGERS_SHIFTED, "RTC: All tigers minted.");
    ...
    // Only owner function to set sale state
    ...
    // Only owner setter for the Merkle Root used in whitelist validation
