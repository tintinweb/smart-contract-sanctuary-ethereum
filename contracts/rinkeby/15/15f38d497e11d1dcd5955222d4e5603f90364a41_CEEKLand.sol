...
contract ERC721Mochi is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControlEnumerable, ERC721Burnable {
    ...
    function autoUngroup(uint256 tokenId, string memory groupId) public onlyUngroup {
		uint id;
		uint i;
		if (tokenId == 0) {
			tokenId = getTokenIdByCoordsOrGroup(groupId,0,0);
		...
		require((owner==msg.sender || contract_gate==msg.sender), "ERC721: token group owner can only ungroup token");
    ...
    function autoMint(string memory _tokenURI, address to, uint256 coord_x, uint256 coord_y, string memory groupId) public onlyMinter {
        uint id;
        do {
          _tokenIdCounter.increment();
          id = _tokenIdCounter.current();
	...
	function autoMintCoordsOrGroup(string memory _baseURI, string[] memory _tokenURI, address to, uint256[] memory coord_x, uint256[] memory coord_y, string[] memory groupId) public onlyMinter {
        uint id;
		uint i;
	...
	function burnCoordsOrGroup(string[] memory groupId, uint256[] memory coord_x, uint256[] memory coord_y) public {
		uint i;
		uint256 tokenId;
		
		for (i=0; i<groupId.length; i++) {
			tokenId = getTokenIdByCoordsOrGroup(groupId[i], 0, 0);
			burn(tokenId);
			delete _tokenGroup[groupId[i]];
		}

		for (i=0; i<coord_x.length && i<coord_y.length; i++) {
			tokenId = getTokenIdByCoordsOrGroup("0", coord_x[i], coord_y[i]);
			burn(tokenId);
			delete _tokenCoords[string(abi.encodePacked(coord_x[i],'_',coord_y[i]))];
		}
	}
...
contract CEEKLand is ERC721Mochi, ReentrancyGuard {
    ...
    function canSell(uint256 tokenId) public view returns (bool) {
        return ((ownerOf(tokenId)==msg.sender || contract_gate==msg.sender) && !auctions[tokenId].open);
    }
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet) public {
        ...
        // onlyOwner
        require((ownerOf(tokenId)==msg.sender || contract_gate==msg.sender), "CEEKLand: Only owner can sell this item");

        // cannot set a price if auction is activated
        require(!auctions[tokenId].open, "CEEKLand: Cannot sell an item which has an active auction");
    ...
    function sellCEEK(uint256 tokenId, uint256 price) public {
        // onlyOwner
        require((ownerOf(tokenId)==msg.sender || contract_gate==msg.sender), "CEEKLand: Only owner can sell this item");
    ...
    function buy(uint256 tokenId) public nonReentrant {
        ...
        // is on sale
        require(!auctions[tokenId].open && sellBidPrice[tokenId]>0, "CEEKLand: The collectible is not for sale");

        // transfer ownership
        address owner = ownerOf(tokenId);

        require(msg.sender!=owner, "CEEKLand: The seller cannot buy his own collectible");
    ...
    function canAuction(uint256 tokenId) public view returns (bool) {
        return ((ownerOf(tokenId)==msg.sender || contract_gate==msg.sender) && !auctions[tokenId].open && sellBidPrice[tokenId]==0);
    }
    ...
    function createAuction(uint256 tokenId, uint _closingTime, address payable _beneficiary, uint256 _reservePrice) public {
        ...
        require(sellBidPrice[tokenId]==0, "CEEKLand: The selected NFT is open for sale, cannot be auctioned");
        require(!auctions[tokenId].open, "CEEKLand: The selected NFT already has an auction");
        require((ownerOf(tokenId)==msg.sender || contract_gate==msg.sender), "CEEKLand: Only owner can auction this item");
	...
	function cancelAuction(uint256 tokenId) public {
	    ...
	    require((ownerOf(tokenId)==msg.sender || contract_gate==msg.sender), "CEEKLand: Only owner can cancel auction this item");
		require(auctions[tokenId].open, "CEEKLand: The selected NFT has not opened auction");
    ...
    function bid(uint256 tokenId, uint256 bid_value) public nonReentrant {
        ...
        address owner = ownerOf(tokenId);
        require(msg.sender!=owner, "CEEKLand: The owner cannot bid his own collectible");
