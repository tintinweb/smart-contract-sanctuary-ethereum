...
abstract contract ERC721URIStorage is ERC721 {
    ...
    function calcEmotion(uint256 tokenId) public view returns (string memory) {
        ...
        //poked emotion
        if (getMonth(block.timestamp) == getMonth(pokeDay) && getDay(block.timestamp) == getDay(pokeDay)){
            return (pokeEmotion);
        }
        ...
        if (isTodayBirthday == true){
            if (getMonth(block.timestamp) == getMonth(_birthday[tokenId]) && getDay(block.timestamp) == getDay(_birthday[tokenId])){
                return "celebrating";
            }
            else {
                return "celebrated";
    ...
    function isBirthday() public view virtual returns (bool) {
        for(uint z=0; z<77; z++){
            //require(_exists(z));
            if (getMonth(block.timestamp) == getMonth(_birthday[z]) && getDay(block.timestamp) == getDay(_birthday[z])){
                return true;
            } 
         }
        return false;
    ...
    function strIndex2num(string memory numString, uint256 index) public pure returns(uint) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        jsonSlot.artData = art.makeArt(_faces[tokenId], _colors[tokenId], _eyeDistance[tokenId], _mouthSize[tokenId], _speed[tokenId], _blur[tokenId]);
        jsonSlot.json = concatenate(unicode'{"description":"🟢 512 generative Soft Face NFTs that interact on chain 🟣 Project by Damjanski 2022 🟡","external_url":"http://something.xyz", "name": "Soft Faces ◑◑ ', uint2str(tokenId));
        jsonSlot.json = concatenate(jsonSlot.json, '", "image": "data:image/svg+xml;base64,');
        jsonSlot.json = concatenate(jsonSlot.json, Base64.encode(bytes(jsonSlot.artData)));
        jsonSlot.json = concatenate(jsonSlot.json, '","attributes":[{"trait_type":"Mood","value":"');
        jsonSlot.json = concatenate(jsonSlot.json, _faces[tokenId]);
        jsonSlot.json = concatenate(jsonSlot.json, '"},{"trait_type":"Color_1","value":"');
        jsonSlot.json = concatenate(jsonSlot.json, (jsonSlot.colorNameArray[strIndex2num(jsonSlot.colorValues, 0)]));
        jsonSlot.json = concatenate(jsonSlot.json, '"},{"trait_type":"Color_2","value":"');
        jsonSlot.json = concatenate(jsonSlot.json, (jsonSlot.colorNameArray[strIndex2num(jsonSlot.colorValues, 1)]));
        jsonSlot.json = concatenate(jsonSlot.json, '"},{"trait_type":"Color_3","value":"');
        jsonSlot.json = concatenate(jsonSlot.json, (jsonSlot.colorNameArray[strIndex2num(jsonSlot.colorValues, 2)]));
        jsonSlot.json = concatenate(jsonSlot.json, '"},{"trait_type":"Color_4","value":"');
        jsonSlot.json = concatenate(jsonSlot.json, (jsonSlot.colorNameArray[strIndex2num(jsonSlot.colorValues, 3)]));
        jsonSlot.json = concatenate(jsonSlot.json, '"},{"trait_type":"Color_5","value":"');
        jsonSlot.json = concatenate(jsonSlot.json, (jsonSlot.colorNameArray[strIndex2num(jsonSlot.colorValues, 4)]));
        jsonSlot.json = concatenate(jsonSlot.json, '"},{"trait_type":"Animation","value":"');
        jsonSlot.json = concatenate(jsonSlot.json, jsonSlot.speedArrayName[_speed[tokenId]]);
        jsonSlot.json = concatenate(jsonSlot.json, '"},{"trait_type":"Blur","value":"Level ');
        jsonSlot.json = concatenate(jsonSlot.json, uint2str(_blur[tokenId]+1));
        jsonSlot.json = concatenate(jsonSlot.json, '"}]}');

        return string(abi.encodePacked(_baseURI(), Base64.encode(bytes(string(jsonSlot.json)))));
    }
    ...
    function _setColors(uint256 tokenId, string memory _colorCode) internal virtual {
        require(_exists(tokenId), "ERC721URIStorage: URI set of nonexistent token");
        _colors[tokenId] = _colorCode;
    }

    function _setEyeDistance(uint256 tokenId, string memory _eyeDis) internal virtual {
        require(_exists(tokenId), "ERC721URIStorage: URI set of nonexistent token");
...
constructor(address initialRoyaltiesReceiver) ERC721("SOFT_FACES", "SFTFCS") {
    ...
    function mint(string memory emotion, string memory faceColors, string memory eyeDist, uint256 mouthSize, uint256 faceSpeed, uint256 faceBlur) external payable {
