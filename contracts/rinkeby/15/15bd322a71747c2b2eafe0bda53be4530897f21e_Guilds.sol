...
abstract contract ERC1155Supply is ERC1155 {
    ...
    function indexOfAddress(address[] memory arr, address searchFor) internal pure returns (uint256) {
    for (uint256 i = 0; i < arr.length; i++) {
      if (arr[i] == searchFor) {
        return i;
      }
    ...
    revert("Not Found2");
     ...
     function indexOfUint256(uint256[] memory arr, uint256 searchFor) internal pure returns (uint256) {
    for (uint256 i = 0; i < arr.length; i++) {
      if (arr[i] == searchFor) {
        return i;
      }
...
contract Guilds is ERC1155Supply {
    ...
    // mapping(address => uint256) public ModIdByAddress;
    ...
    // mapping(address => Guild) GuildsByAddress;
    ...
    uint256 public minimumMintRate = 10000000000000000;
    ...
    function createNewGuild(uint256 _amount, string memory _uri, uint256 _guildMintRate, string memory _name, string memory _desc)
        public
        payable
    {
        ...
        setNewGuild(_id, msg.sender, _guildMintRate, _name, _desc);
    ...
    function setNewGuild(uint256 _id, address _initiator, uint256 _guildMintRate, string memory _name, string memory _desc) private {
        AllGuilds[_id].TokenId = _id;
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            // Burn token
            if (address(0) == to) {
                // Burn the total supply of the guild:
                if(totalSupply(ids[i]) == amounts[i]) {
                _totalGuilds -= 1;
                delete AllGuilds[ids[i]];
                }
            }
            ...
            if (_tokensLeft == amounts[i]) {
               ...
               delete AllGuilds[ids[i]].GuildMembers[indexOfAddress(AllGuilds[ids[i]].GuildMembers, from)];
                ...
                if (indexOfUint256(GuildsByAddress[from], ids[i]) >= 0) {
            ...
            if (address(0) != to && AllGuilds[ids[i]].Admin != to) {
            // If guild exists:
            if (exists(ids[i])) {
            // Add new member:
    ...
    // MINT MADE BY MODERATOR REFERAL LINK OR THE MOD HIMSELF:
    function modMint(address _modAddress, uint8 _amount, uint256 _id) public payable {
        uint256 totalRate;
        // address _modAddress = ModAddressById[_modId];
        address _adminAddresss = tokenAdmin[_id];
        uint8 _modMintLimit = ModMintLimit[_id][_modAddress];
        require(_modMintLimit >= _amount, "Not enough spots.");
    ...
    function assignModerator(
        ...
        // ModIdByAddress[_account] = _lastModId;
        ModMintRate[_id][_account] = _mintRate;
        ModMintLimit[_id][_account] = _mintLimit;
        AllGuilds[_id].GuildMods.push(_account);
    }

    function adminSetModMintLimit(uint256 _id, address _account, uint8 _mintLimit) private onlyRole(ADMIN, _id) {
        _setModMintLimit(_id, _account, _mintLimit);
    }

    function _setModMintLimit(uint256 _id, address _account, uint8 _mintLimit) private {
        ModMintLimit[_id][_account] = _mintLimit;
    }
    ...
    function setNewTokenUri(uint256 _id, string memory _uri) public onlyOwner {
        tokenURI[_id] = _uri;
        emit URI(_uri, _id);
    }
