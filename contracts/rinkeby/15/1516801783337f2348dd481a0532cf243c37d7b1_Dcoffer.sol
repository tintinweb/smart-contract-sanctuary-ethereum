...
contract ERC721A is Context, ERC165, IERC721A, IDCF {
    ...
    */   
    ...
    function updateBlockScore(address from, address to)private{
        uint256 ScoreA = BlockScore(from);
        uint256 ScoreB = BlockScore(to);
        _BlockScore[from][2]=_BlockScore[from][2].add(ScoreA);
        _BlockScore[to][2]=_BlockScore[to][2].add(ScoreB);
        _BlockScore[from][1] = block.number;
        _BlockScore[to][1] = block.number;
    }
    ...
    function _burn(uint256 tokenId, bool approvalCheck) internal virtual {
        updateBlockScore(msg.sender,address(0));
        TokenOwnership memory prevOwnership = _ownershipOf(tokenId);
...
contract Dcoffer is ERC721A {
  ...
  function setInfo(uint paramA, uint paramB, uint paramC)external onlyOwner{
