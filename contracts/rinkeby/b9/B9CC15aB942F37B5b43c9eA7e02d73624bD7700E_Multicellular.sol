...
contract Multicellular is  Ownable, ERC721A, ReentrancyGuard {
    ...
    function tokenURI(uint tokenId_) public view override returns (string memory){
        ...
        if (wormActive[tokenId_]){
            ...
            if (wormLevel[tokenId_] > 0 )
                return string(abi.encodePacked(baseDeadTokenURI, Strings.toString(tokenId_)));
            else{
                return string(abi.encodePacked(baseHatchTokenURI, Strings.toString(tokenId_)));
            }
        }
    ...
    function sacrifice(uint256 sacrifice_, uint256 predator_) public {
        ...
        wormLevel[sacrifice_] = wormLevel[predator_] - wormLevel[sacrifice_];
        burnCounter++;
