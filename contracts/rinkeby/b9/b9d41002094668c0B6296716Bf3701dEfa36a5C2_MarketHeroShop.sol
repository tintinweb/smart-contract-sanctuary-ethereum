...
contract MarketHeroShop is Initializable, IOwnable {
    ...
    // address of Contract MarketHeroUtils
    ...
    function initialize(
        ...
        setHeroPrice(0,5);// hamster
        setHeroPrice(1,150);// bull
        setHeroPrice(2,150);// bear
        setHeroPrice(3,300);// whale
        setUpgradePrices(0,5,5);     // Hamster
        setUpgradePrices(1,100,100); // Bull
        setUpgradePrices(2,100,100); // Bear
        setUpgradePrices(3,150,150);// Whale
...
//  ************* MODIFIERS ********************************
    modifier onlyAdmin{
        require(MarketHeroController(marketHeroControllerAddress).hasRole(MarketHeroController(marketHeroControllerAddress).DEFAULT_ADMIN_ROLE(),msg.sender),"MarketHeroShop: caller is not an admin ");
        _;
    }

    modifier whenPaused{
        require(MarketHeroController(marketHeroControllerAddress).paused(),"MarketHeroShop is not paused ");
        _;
    }
    modifier whenNotPaused{
        require(!MarketHeroController(marketHeroControllerAddress).paused(),"MarketHeroShop is paused");
        _;
    }
//  ************* VIEW FUNCTIONS ********************************
    ...
    function getHeroPrice(uint8 _animaType) external view returns(uint256){
        return heroPrices[_animaType];
    }

    function getBurnAmount(uint8 _animalTypeBurn, uint8 _animalTypeClaim) external view returns(uint16){
        return animalBurnAmount[_animalTypeBurn][_animalTypeClaim];
    } 
//  ************* ADMIN FUNCTIONS ********************************
     ...
     * @notice Function sets contract address of MarketHeroToken
      */
    function setTokenMHT(address _tokenMHT) public onlyAdmin whenNotPaused{
        tokenMHT = _tokenMHT;
    }
    /**
    ...
    function setUpgradePrices(
        ...
        heroSkillUpgradePrices[_animalType].level1Price = _level1Price;
        heroSkillUpgradePrices[_animalType].interval = _interval;
    }
     ...
     * @notice Function sets contract address of MarketHeroAnimal contract
      */
    function setMarketHeroAnimalContract(address _marketHeroAnimalContract) public onlyAdmin whenNotPaused{
     ...
     * @param _amount number of tokens to create
      */
    function createHeroes(uint8 _animalType, uint256 _amount) public onlyAdmin whenNotPaused{
...
//  ************* PRIVATE FUNCTIONS ********************************
    ...
    function _createHeroes(uint8 _animalType, uint256 _amount) private {
        for (uint256 i =0; i<_amount; i++ ){
            MarketHeroAnimal(marketHeroAnimalContract).createHero(_animalType, msg.sender);
            (uint8 _speed,
             uint8 _lifesteal,
             uint8 _endurence,
             uint8 _fund,
             uint8 _level) = MarketHeroUtils(marketHeroUtilsAddress).multipleCase(_animalType, MarketHeroAnimal(marketHeroAnimalContract).lastMintedTokenID());
            MarketHeroAnimal(marketHeroAnimalContract).renewHeroParameters(
                MarketHeroAnimal(marketHeroAnimalContract).lastMintedTokenID(),
                _animalType,
                _speed,
                _lifesteal,
                _endurence,
...
//  ************* USER FUNCTIONS ********************************
     ...
     * @param _animalTypeClaim type of token to receive
      */
    function burnHeroesForReward(uint256[] memory heroes,uint8 _animalTypeBurn, uint8 _animalTypeClaim) public whenNotPaused{
        ...
        require(animalBurnAmount[_animalTypeBurn][_animalTypeClaim]>=heroes.length,"MarketHeroShop: not enough tokens selected");
    ...
    /**
     * @notice Function to mint hero for money 
     * @param _animalType type of Hero
      */
    function buyHero(uint8 _animalType) public whenNotPaused{
        IERC20Upgradeable(tokenMHT).safeTransferFrom(msg.sender, address(this), heroPrices[_animalType]);
        MarketHeroAnimal(marketHeroAnimalContract).createHero(_animalType, msg.sender);
        (uint8 _speed,
         uint8 _lifesteal,
         uint8 _endurence,
     ...
     * @param _trait characteristic to upgrade
      */
    function upgradeSpecificTrait(uint256 _tokenID, uint8 _trait) public whenNotPaused{
        // require((_msgSender()== ),"");
        uint8 _traitLevel;
        uint8 _heroMaxLevel;
        ...
        require(_level<_heroMaxLevel, "MarketHeroShop: Hero is already MAX level");
        ...
        // require ((IERC20Upgradeable(tokenMHT).balanceOf(msg.sender)>=(heroSkillUpgradePrices[_animalType].level1Price + _traitLevel* heroSkillUpgradePrices[_animalType].interval)),"MarketHeroShop: not enough money for upgrade this trait");
        // IERC20Upgradeable(tokenMHT).safeApprove(address(this),(_upgradePrice));
        IERC20Upgradeable(tokenMHT).safeTransferFrom(msg.sender, address(this),(heroSkillUpgradePrices[_animalType].level1Price + _traitLevel* heroSkillUpgradePrices[_animalType].interval));
