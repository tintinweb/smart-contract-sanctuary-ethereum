...
contract STANDBY is TOKEN {
    ...
    mapping(uint256 => bool) public standbyStatus;
    ...
    function _beforeTokenTransfers(
        ...
        require(
            standbyStatus[startTokenId] == false,
            "Chimera in standby state cannot be sent!"
        );

        super._beforeTokenTransfers(from, to, startTokenId, quantity);
    }

    function toggleStandby(uint256 tokenId) internal {
        ...
        if (standbyStatus[tokenId]) {
            standbyStatus[tokenId] = false;
        } else {
            standbyStatus[tokenId] = true;
        }
    }

    function toggleStandby(uint256[] calldata tokenIds) external {
        require(standbyStart, "This feature is not yet available.");
        require(
            tokenIds.length < 101,
            "The number of IDs that can be entered is limited to 100."
        );

        uint256 n = tokenIds.length;
        for (uint256 i = 0; i < n; ++i) {
            toggleStandby(tokenIds[i]);
        }
    }
...
contract TOKEN is ERC721AQueryable, ERC2981, Ownable, ReentrancyGuard {
    ...
    uint256 public prePrice = 0.0003 ether;
    uint256 public pubPrice = 0.0005 ether;
    ...
    string public notRevealedURI =
        "https://pna-generative.s3.ap-northeast-1.amazonaws.com/chimerative/contract_test/notrevealed.json";
    ...
    function pubMint(uint256 _quantity) public payable nonReentrant {
        ...
        require(_quantity <= mintLimit, "Mint amount over");
        require(
    ...
    function withdrawRevenueShare() external onlyOwner {
        ...
        address own = payable(owner());
        address cre = payable(0x87d2D88ac1444Acb76f7017E70027C0F4B06D8C5);
        address dev = payable(0x87d2D88ac1444Acb76f7017E70027C0F4B06D8C5);
        address mer = payable(0x87d2D88ac1444Acb76f7017E70027C0F4B06D8C5);

        bool success;

        (success, ) = own.call{value: ((sendAmount * 5000) / 10000)}("");
        require(success, "Failed to withdraw Ether");

        (success, ) = cre.call{value: ((sendAmount * 3000) / 10000)}("");
        require(success, "Failed to withdraw Ether");

        (success, ) = dev.call{value: ((sendAmount * 1000) / 10000)}("");
        require(success, "Failed to withdraw Ether");

        (success, ) = mer.call{value: ((sendAmount * 1000) / 10000)}("");
        require(success, "Failed to withdraw Ether");
    }
