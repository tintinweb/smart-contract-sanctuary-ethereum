...
// The ERC-20 Rewards Token.
interface IDeal {
    function mintToAddress(address toAddress, uint256 numTokensWei) external;
}
...
contract SurrealestateStaking is ERC721Holder, Ownable {
    ...
    constructor() public {
        address _surrealestateContract;
        address _dealContract;
        uint _stakingLockPeriod;
        assembly {
            switch chainid()
            case 1 {
                // mainnet
                _surrealestateContract := 0x6F3185B51A42E03a4F0eaAf37604DDd499EF9b12
                // TODO: SET THE CORRECT MAINNET CONTRACT.
                _dealContract := 0x8fEe2A25F23C0e711064f3d31F84D703A86E7c2C
                _stakingLockPeriod := 5184000 // 60 days in seconds.
            }
            case 4 {
                // rinkeby
                _surrealestateContract := 0xfa70b452397882b4677740976cd7ce20b8cd6576
                _dealContract := 0x8fEe2A25F23C0e711064f3d31F84D703A86E7c2C
                _stakingLockPeriod := 600 // 10 minutes in seconds.
            }
        }
        stakingLockPeriod = _stakingLockPeriod;
        surrealestateContractAddress = address(_surrealestateContract);
        surrealestates = IPPASurrealestates(surrealestateContractAddress);
        deal = IDeal(address(_dealContract));
    }
    ...
    mapping(address => uint256) numStakedByAddress;
    ...
    // Muliplier for this user.
    mapping(address => uint256) stakingMultiplier;
    // (TokenID => Timestamp))
    mapping(uint256 => uint256) tokenLockedUntil;
    ...
    function calculateTokensEarned(address addr) public view returns (uint256) {
        ...
        uint256 tokensEarnedAfterLastRefresh = secondsStakedSinceLastRefresh *
            numStakedByAddress[addr] *
            stakingMultiplier[addr];
        return
    ...
    function _unstakeSingle(uint256 tokenId) internal {
        require(
            tokenLockedUntil[tokenId] < block.timestamp,
            "Token is locked and cannot be unstaked yet"
        );
    ...
    /**
     * Claim the DEAL tokens the user has earned.
     */
    function claimDealTokens() public {
        refreshTokensEarned(msg.sender);
        deal.mintToAddress(
            msg.sender,
            _tokensEarnedBeforeLastRefresh[msg.sender]
        );
        _tokensEarnedBeforeLastRefresh[msg.sender] = 0; // Reset rewards back to 0.
    }

    function setStakingLockPeriod(uint256 newPeriod) public onlyOwner {
