...
contract Stamps is
  ...
  UUPSUpgradeable
{
  string public constant name = "Stamps";
  ...
  function recover(
    ...
    bytes32 message = keccak256(
      abi.encodePacked(msg.sender, tokenId, validUntil, ids, amounts)
    );
  ...
  function mintBatch(
    ...
    (uint256 tokenId, uint256 validUntil, bytes memory signature) = abi.decode(
      data,
      (uint256, uint256, bytes)
    );
    require(!_usedHash[signature], "Used signature");
    require(block.timestamp < validUntil, "signature expired");
    require(recover(tokenId, validUntil, ids, amounts, signature));
