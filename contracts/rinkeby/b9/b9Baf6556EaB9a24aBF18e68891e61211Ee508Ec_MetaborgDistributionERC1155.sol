...
contract MetaborgDistributionERC1155 is ERC1155Upgradeable {
    ...
    uint private mangaDistributionIndex;
    ...
    event withdrawOwnerBalanceEvent(address indexed owner, uint amount);
    ...
    function createMangaDistribution(uint _price, uint _diamondSupply, uint _goldSupply, uint _originalSupply, address[] memory _allowedAddressToMint, uint[] memory _quantityList, uint _ERC1155DiamondID, uint _ERC1155GoldID, uint _ERC1155OriginalID) public onlyOwner returns(uint){
    ...
    function extendDistribution(uint _mangaDistributionID, uint _diamondExtension, uint _goldExtension, uint _originalExtension, address[] memory _allowedAddressToMint, uint[] memory _quantityList) public onlyOwner returns(bool){
        uint diamondSupply = mangaDistribution[_mangaDistributionID].diamondSupply;
        uint goldSupply = mangaDistribution[_mangaDistributionID].goldSupply;
        uint originalSupply = mangaDistribution[_mangaDistributionID].originalSupply;
        require(diamondSupply.add(goldSupply).add(originalSupply) > uint(0), "DELETED_OR_NOT_INITIALIZED_DISTRIBUTION");
        require(_diamondExtension.add(_goldExtension).add(_originalExtension) > uint(0), "CANT_SET_NULL_DISTRIBUTION");
        require(_allowedAddressToMint.length == _quantityList.length, "DATA_LENGTH_DISMATCH");
        uint sumQuantity = uint(0);
        for(uint quantityIndex = uint(0); quantityIndex < _quantityList.length; quantityIndex++){
            sumQuantity = sumQuantity.add(_quantityList[quantityIndex]);
        }
        require(sumQuantity == _diamondExtension.add(_goldExtension).add(_originalExtension), "QUANTITY_LENGTH_DISMATCH");
        mangaDistribution[_mangaDistributionID].diamondSupply = diamondSupply.add(_diamondExtension);
        mangaDistribution[_mangaDistributionID].goldSupply = goldSupply.add(_goldExtension);
        mangaDistribution[_mangaDistributionID].originalSupply = originalSupply.add(_originalExtension);
        uint actualQuantityForAddress;
        for(uint index = uint(0); index < _allowedAddressToMint.length; index++){
            actualQuantityForAddress = mangaDistribution[_mangaDistributionID].allowedAddressToMint[_allowedAddressToMint[index]];
            mangaDistribution[_mangaDistributionID].allowedAddressToMint[_allowedAddressToMint[index]] = actualQuantityForAddress.add(_quantityList[index]);
        }
        emit extendDistributionEvent(_mangaDistributionID, msg.sender, _diamondExtension, _goldExtension, _originalExtension);
        return true;
    }
}
