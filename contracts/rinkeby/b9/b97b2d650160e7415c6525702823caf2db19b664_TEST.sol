...
// File: Лимит на публичный минт!!!_flat.sol
...
// File: contracts/Лимит на публичный минт!!!.sol
...
contract TEST is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public publicCost = 8700000000000000; // 0.0087
    //uint256 public whitelistCost = 0.003 ether;
    uint256 public publicSupply = 7500;
    uint256 public maxTotalSupply = 8000;
    uint256 public WLAmount = 1;
    ...
    function mintWL(bytes32[] memory proof) external nonReentrant {
        require(WLSale, "WLmint hasn't commenced yet.");
        require(MerkleProof.verify(proof, rootWL, keccak256(abi.encodePacked(msg.sender))), "You're not eligible.");
        require(freeMint[msg.sender] < 1, "You've already minted one Free NFT.");
        require(totalSupply() + WLAmount <= maxTotalSupply, "There's not enough supply left.");
        freeMint[msg.sender] += WLAmount;
        _safeMint(msg.sender, WLAmount);
    ...
    function mintPublic(uint256 _amount) external payable nonReentrant {
        require(publicSale, "Mint hasn't commenced yet.");
        require(_amount <= maxPerTXN && _amount > 0, "You can mint a maximum of 5 per transaction.");
        require(_amount + totalSupply() <= publicSupply, "There's not enough supply left.");
        require(msg.value >= publicCost * _amount, "Incorrect funds");
        _safeMint(msg.sender, _amount);
    }
    ...
    function setMaxTotalSupply(uint256 _tsupply) external onlyOwner {
        maxTotalSupply = _tsupply;
    }
    
    function setPublicSupply(uint256 _psupply) external onlyOwner {
        publicSupply = _psupply;
    }
    ...
    function StartStopPublicSale(bool _psail) external onlyOwner {
        publicSale = _psail;
    }
    
    function StartStopWLSale(bool _wlsail) external onlyOwner {
        WLSale = _wlsail;
    }
