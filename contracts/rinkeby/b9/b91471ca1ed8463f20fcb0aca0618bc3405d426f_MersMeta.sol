...
contract MersMeta is Initializable, ERC1155Upgradeable, OwnableUpgradeable, PaymentSplitterUpgradeable, ReentrancyGuardUpgradeable, IMersMeta {
    ...
    function setTokenIDPrice (uint32 id, uint256 mintValue) external override onlyOwner
    {
    ...
    function mintToken (uint32 id, uint256 amount) external override payable nonReentrant
    {
        require(mintPrice[id] != 0 , "MersMeta: Cannot mint token as token ID is not active yet!");
        require(msg.value >= (mintPrice[id])*(amount), "MersMeta: Insufficient ETH supplied.");
    ...
    function airDrop (address[] memory recipient, uint256[] memory amount , uint256 id) external override onlyOwner 
    {
    ...
    function setURI (string memory seturi) external override onlyOwner
    {
        baseUri = seturi;
        emit SetUri(seturi);
    }
    ...
    function uri(uint id) public view virtual override(IMersMeta,ERC1155Upgradeable) returns (string memory) 
    {
