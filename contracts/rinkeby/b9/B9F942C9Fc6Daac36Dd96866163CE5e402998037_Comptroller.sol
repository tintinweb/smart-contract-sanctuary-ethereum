...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     *          account shortfall below collateral requirements)
     */
    function getAccountLiquidityInternal(address account) public returns (Error, uint, uint) {
    ...
    //  *          hypothetical account shortfall below collateral requirements)
    //  */
    event Event(uint so,uint so2,uint so3);
    event Event1(uint so, Exp so2 , uint so3);
    function getHypotheticalAccountLiquidityInternal(
        address account,
        CToken cTokenModify,
        uint redeemTokens,
        // uint borrowAmount) internal view returns (Error, uint, uint) {
        uint borrowAmount) public payable returns (Error, uint, uint) {
        emit Event(0,1,1);
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            emit Event1( vars.oraclePriceMantissa,vars.tokensToDenom,1);
        ...
        emit Event(1,1,1);
        emit Event(vars.sumCollateral,vars.sumBorrowPlusEffects,1);
