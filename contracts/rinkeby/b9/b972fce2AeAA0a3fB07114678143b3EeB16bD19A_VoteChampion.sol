...
contract VoteChampion is Pausable, Ownable {
    ...
    struct User {
        ...
        uint totalVote;
        uint[] rankVoted;
        mapping(uint => Rank) voted;
    }

    string[] public teams;
    mapping(uint => Rank) public ranks; // 0 => rank 1; 1 => rank 2 ...
    ...
    constructor(string[] memory _teams, IERC20 _voteToken) {
        teams = _teams;
    ...
    function userAllocate(address user, uint _rank, uint _teamIndex) public view returns(uint) {
        return 100 * users[user].voted[_rank].totalVote / userVoteRanks[_teamIndex][_rank];
    }
    ...
    function claimWin(uint _rank) external whenNotPaused {
        ...
        require(u.voted[_rank].teamIndex == r.teamIndex, 'VoteChampion::claimWin: Not Vote');
        require(u.voted[_rank].totalVote > 0, 'VoteChampion::claimWin: Not Vote amount');
    ...
    function vote(uint _rank, uint _teamIndex, uint amount) external whenNotPaused {
        ...
        users[_msgSender()].voted[_rank].teamIndex = _teamIndex;
        users[_msgSender()].voted[_rank].totalVote += amount;
        if(users[_msgSender()].voted[_rank].totalVote == 0) users[_msgSender()].rankVoted.push(_rank);
    ...
    function unVote(uint _rank, uint _teamIndex, uint amount) external whenNotPaused {
        ...
        require(users[_msgSender()].voted[_rank].totalVote >= amount, 'VoteChampion::unVote: Invalid Amount');
