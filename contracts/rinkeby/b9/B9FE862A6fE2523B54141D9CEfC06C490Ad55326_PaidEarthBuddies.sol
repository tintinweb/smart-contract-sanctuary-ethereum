...
contract PaidEarthBuddies is ERC721A, Ownable {
    // ============ State Variables ============
    ...
    uint256 public priceAfterFirstMint = 0.1 ether;
    bytes32 public root;
    ...
    constructor(string memory __name, string memory __symbol, string memory baseURI, bytes32 __root) ERC721A(__name, __symbol) {
        _name = __name;
        _symbol = __symbol;
    ...
    function mint(uint256 quantity, bytes32[] memory proof) external payable onlyWhenNotPaused callerIsUser {
        ...
        if (balanceOf(msg.sender) > 0 || quantity > 1) {
            if (balanceOf(msg.sender) > 0) {
                require(msg.value >= priceAfterFirstMint * quantity);
            } else {
                require(msg.value >= priceAfterFirstMint * (quantity - 1));
            }
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, _toString(tokenId))) : hiddenMetadataUri;
    }
    ...
    function setPriceAfterFirstMint(uint256 price) external onlyOwner {
        priceAfterFirstMint = price;
    }
