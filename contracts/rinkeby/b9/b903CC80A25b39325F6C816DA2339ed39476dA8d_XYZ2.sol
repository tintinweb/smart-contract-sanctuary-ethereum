...
contract XYZ2 is ERC721, Ownable {
    using Strings for uint256;
    ...
    {
        require(_wl[msg.sender].canMint == true, 
            "Not on whitelist. Or already minted.");
        require(msg.value >= _wl[msg.sender].fee * kt.length, 
            "Insufficient fee");
        require(_wl[msg.sender].mintLength < maxMintLength, 
            "You're trying to mint too many tokens at once");

        for (uint256 i = 0; i < kt.length; i++) {
            require(isTeActive(getTeFromTokenId(kt[i])), "TE not active.");
            _safeMint(msg.sender, kt[i]);
            _wl[msg.sender].canMint = false;
        }
    }
