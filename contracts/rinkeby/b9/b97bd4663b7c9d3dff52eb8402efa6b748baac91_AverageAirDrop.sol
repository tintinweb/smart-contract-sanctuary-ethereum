...
contract AverageAirDrop is ERC721, Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    function claimPFPs() external nonReentrant {
        ...
        // }
        uint256 cap;
        if(_avgcreatures.balanceOf(msg.sender) - balanceOf(msg.sender) > 20){
          cap = 20;
        } else {
          cap = _avgcreatures.balanceOf(msg.sender) - balanceOf(msg.sender) - 1;
        }
        uint256[] memory walletTokenIDs = _avgcreatures.walletOfOwner(msg.sender);
        uint256 x = 0;
        uint256 i = 0;
        while(i  < cap){
          if (!isClaimed(walletTokenIDs[x])) {
              _safeMint(msg.sender, walletTokenIDs[x]);
              _setClaimed(walletTokenIDs[x]);
              i++;
              x++;
          } 
          x++;
