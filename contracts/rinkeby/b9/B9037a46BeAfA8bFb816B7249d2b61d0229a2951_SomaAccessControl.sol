...
contract SomaAccessControl is ISomaAccessControl, SomaContractUpgradeable, AccessControlUpgradeable {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.Bytes32Set;
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    string public constant name = 'SomaAccessControl';

    mapping (address => EnumerableSetUpgradeable.Bytes32Set) private _accountRoles;
    mapping (bytes32 => EnumerableSetUpgradeable.AddressSet) private _roleAccounts;
    ...
    public view virtual
    override(AccessControlUpgradeable, SomaContractUpgradeable)
    returns (bool) {
        return interfaceId == type(ISomaAccessControl).interfaceId ||
            interfaceId == type(IAccessControlUpgradeable).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    function rolesOf(address account) public view override returns (bytes32[] memory) {
        return _accountRoles[account]._inner._values;
    }

    function accountsOf(bytes32 role) external view override returns (address[] memory) {
        uint256 length = _roleAccounts[role].length();
        address[] memory accounts = new address[](length);

        for (uint i = 0; i < length; i++) {
            accounts[i] = _roleAccounts[role].at(i);
        }
    ...
    function revokeRole(bytes32 role, address account) public virtual override whenNotPaused {
        ...
        _roleAccounts[role].remove(account);
    }

    function revokeRoles(bytes32[] memory roles, address target) external override whenNotPaused {
        for (uint i = 0; i < roles.length; i++) {
            revokeRole(roles[i], target);
        }
    }

    function grantRoles(bytes32[] memory roles, address target) external override whenNotPaused {
        for (uint i = 0; i < roles.length; i++) {
            grantRole(roles[i], target);
        }
    }

    function setRoleAdmin(bytes32 role, bytes32 adminRole) external override whenNotPaused onlyRole(getRoleAdmin(getRoleAdmin(role))) {
