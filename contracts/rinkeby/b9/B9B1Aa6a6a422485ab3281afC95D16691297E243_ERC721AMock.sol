...
contract ERC721AMock is ERC721A, IERC2981, Pausable, Ownable, ReentrancyGuard {
    using ECDSA for bytes32;

    uint256 public constant PRICE_PER_TOKEN = 0.045 ether;
    uint256 public constant MAX_BUY_PER_ADDRESS = 5;
    uint256 public constant INIT_ROYALTYPERCENT = 5;
    ...
    constructor(uint256 _maxSupply) ERC721A("LuckyYearOfTiger", "LYOT") {
        maxSupply = _maxSupply;
        royaltyAddress = owner();
        royaltyPercent = INIT_ROYALTYPERCENT;
    }
    ...
    function verify(bytes32[] memory _proof) internal view returns (bool) {
    ...
    function whiteListMint(bytes32[] memory _proof) external nonReentrant {
        require(!freeMintList[msg.sender], "Already mint free NFT");

        require(verify(_proof), "Invalid merkle proof");

        require(totalSupply() <= maxSupply, "Exceed max token supply");

        freeMintList[msg.sender] = true;

        _safeMint(msg.sender, FREE_MINT_AMOUNT);
    }

    function publicMint(uint256 amount) external payable nonReentrant {
        require(isPublicSaleStart, "Public sale not start");
        require(
            numberMinted(msg.sender) + amount <= MAX_BUY_PER_ADDRESS,
            "Exceed max buy per address"
        );
        require(totalSupply() + amount <= maxSupply, "Exceed max token supply");
    ...
    function setPublicSaleStart() external onlyOwner {
        require(!isPublicSaleStart, "Already set");
        isPublicSaleStart = true;
    }
