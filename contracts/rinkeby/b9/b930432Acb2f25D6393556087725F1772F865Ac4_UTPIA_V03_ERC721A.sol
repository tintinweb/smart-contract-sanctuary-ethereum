...
contract UTPIA_V03_ERC721A is Ownable, ERC721A, ReentrancyGuard {

    string public UTOPIAS_PROVENANCE;
    ...
    uint256 public crewMintAmount = 3;
    string public baseExtension = ".json";

    enum Sale{NON, WHITELIST, SALE}
    Sale public sale = Sale.WHITELIST;

    mapping(address => uint256) public walletMints;

    bytes32 public root;
    string public notRevealedUri = "ipfs://Qmejqa2gv1euBtsfmQce3xFNrjLfKUTnf9f7uecnw4otMs/hidden.json";
    string public revealedBaseURI = "ipfs://QmPJdPHCXCdWt3gGCFKqXFCtSW2oJ3jNkMFKbX5bTatgR1/";


    constructor() ERC721A("Utp_V04_ERC721A", "UT") {
        setBaseURI(revealedBaseURI);
        setNotRevealedURI(notRevealedUri);
        airdropUtopias(crewMintAmount, msg.sender);
    }
    ...
    function mintUtopias(uint256 _mintAmount) public payable callerIsUser  {
        ...
        if (msg.sender != owner()) {
            ...
            if (sale == Sale.SALE) {
                require(walletMints[msg.sender] + _mintAmount <= maxMintAmount, "Exceeds max per wallet");
    ...
    function mintUtopiasWhitelist(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable callerIsUser  {
        uint256 supply = totalSupply();
        uint256 actualCost = cost;
        require(!paused);
        require(_mintAmount > 0, "mint amount > 0");
    ...
    function airdropUtopias(uint256 _mintAmount, address destination) public onlyOwner  {
        uint256 supply = totalSupply();
        require(_mintAmount > 0, "mint amount > 0");
        require(_mintAmount <= maxMintAmount, "mint amount < batch");
        require(supply + _mintAmount <= collectionSize, "max NFT limit exceeded");
