...
import "../Interfaces/ThirdPartyAccountManagerInterface.sol";
...
contract ThirdPartyAccountManagerContract is AccessControl, ThirdPartyAccountManagerInterface { 
    
    string public constant VersionString = "ThirdPartyAccountManagerContract v1.0";
    ...
    function addAccount(address accountAddress_) onlyRole(MODERATOR_ROLE) public returns (bool) {
        ...
        ThirdPartyAccountInterface accountInstance = ThirdPartyAccountInterface(accountAddress_);
        ...
        ThirdPartyAccountRegistryInterface(getThirdPartyAccountRegistryAddress()).addAccount(accountAddress_, accountInstance.getThirdPartyOwnerAddress());
        return true;
    }
    function enable(address accountAddress_) onlyRole(MODERATOR_ROLE) public override returns(bool) {
        require(_companyRegistryInstance.isEntityEnabled(getCompanyId(), getEntityId(), address(this)),"This smart contract is disabled!");
        require(ThirdPartyAccountRegistryInterface(getThirdPartyAccountRegistryAddress()).getIsAccountRegistered(accountAddress_), "Account is not part of this company!");
        
        ThirdPartyAccountInterface(accountAddress_).enableAccount();
        return true;
    }
    function disable(address accountAddress_) onlyRole(MODERATOR_ROLE) public override returns(bool) {
        require(_companyRegistryInstance.isEntityEnabled(getCompanyId(), getEntityId(), address(this)),"This smart contract is disabled!");
        require(ThirdPartyAccountRegistryInterface(getThirdPartyAccountRegistryAddress()).getIsAccountRegistered(accountAddress_), "Account is not part of this company!");
        
        ThirdPartyAccountInterface(accountAddress_).disableAccount();
        return true;
    }
    function purgeAccount(address accountAddress_) onlyRole(MODERATOR_ROLE) public override returns (bool) {
        require(_companyRegistryInstance.isEntityEnabled(getCompanyId(), getEntityId(), address(this)),"This smart contract is disabled!");
        require(ThirdPartyAccountRegistryInterface(getThirdPartyAccountRegistryAddress()).getIsAccountRegistered(accountAddress_), "Account is not part of this company!");
        require(!TokenRegistryInterface(getTokenRegistryAddress()).getHoldsTokens(accountAddress_), "Address token balance!");
        
        address thirdPartyOwnerAddress = ThirdPartyAccountInterface(accountAddress_).getThirdPartyOwnerAddress();
        ThirdPartyAccountRegistryInterface(getThirdPartyAccountRegistryAddress()).removeAccount(accountAddress_, thirdPartyOwnerAddress);
        ThirdPartyAccountInterface(accountAddress_).purgeAccount();
        IdentityRegistryInterface(getIdentityRegistryAddress()).whitelistContractsAddress(accountAddress_, true);
        return true;
    }
    
    function changeThirdPartyOwner(address accountAddress_, address thirdPartyOwner_) onlyRole(MODERATOR_ROLE) public override returns(bool) {
        require(_companyRegistryInstance.isEntityEnabled(getCompanyId(), getEntityId(), address(this)),"This smart contract is disabled!");
        require(ThirdPartyAccountRegistryInterface(getThirdPartyAccountRegistryAddress()).getIsAccountRegistered(accountAddress_), "Account is not part of this company!");

        ThirdPartyAccountInterface accountInstance = ThirdPartyAccountInterface(accountAddress_);
        accountInstance.setThirdPartyOwnerAddress(thirdPartyOwner_);
        ThirdPartyAccountRegistryInterface(getThirdPartyAccountRegistryAddress()).removeAccountsParticipant(accountInstance.getThirdPartyOwnerAddress(), accountAddress_);
        ThirdPartyAccountRegistryInterface(getThirdPartyAccountRegistryAddress()).addAccountsParticipant(thirdPartyOwner_, accountAddress_);
        return true;
    }
    function switchAccounts(address firstAccountAddress_, address secondAccountAddress_) onlyRole(MODERATOR_ROLE) public override returns (bool) {
        require(_companyRegistryInstance.isEntityEnabled(getCompanyId(), getEntityId(), address(this)),"This smart contract is disabled!");
        ThirdPartyAccountRegistryInterface(getThirdPartyAccountRegistryAddress()).switchAccounts(firstAccountAddress_, secondAccountAddress_);
        return true;
    }
    ...
    function getThirdPartyAccountRegistryAddress() public view override returns (address) {
        return _companyRegistryInstance.getCompanyEntity(_companyId, 7);
    }
    function getTokenRegistryAddress() public view override returns (address) {
        return _companyRegistryInstance.getCompanyEntity(_companyId, 3);
    }
...
interface ThirdPartyAccountManagerInterface is IAccessControl {
    
    // ===============================
    // ==== MODERATOR_ROLE REGION ====
    // ===============================
    function addAccount(address thirdPartyOwner_) external returns (bool);
    function enable(address accountAddress_) external returns(bool);
    function disable(address accountAddress_) external returns(bool);
    function purgeAccount(address accountAddress_) external returns (bool);
    
    function changeThirdPartyOwner(address accountAddress_, address thirdPartyOwner_) external returns(bool);
    ...
    function getThirdPartyAccountRegistryAddress() external view returns (address);
