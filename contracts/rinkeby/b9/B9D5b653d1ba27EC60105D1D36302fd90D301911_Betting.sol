...
contract Betting is AccessControl{
    ...
    event WinningAmount(uint256 indexed matchId,address better_address,uint256  amount,uint256 winamount);
    constructor() public {
    ...
    function betting_on_match(uint256 _matchno,uint256 _amount,string memory _team) external {
        ...
        ERC20 currency=ERC20(matchs.currency_address);
        ...
        uint256 decimal= currency.decimals();
        currency.transferFrom(msg.sender,address(this),_amount*10**decimal);
        ...
        for (uint256 i=0; i < total_bet[_matchno].length; i++){
            ...
            if(keccak256(abi.encodePacked(bets.team)) == keccak256(abi.encodePacked(_winner))){
                if(keccak256(abi.encodePacked(matchs.team1)) == keccak256(abi.encodePacked(_winner))){
                    ...
                    currency.transfer(bets.better_address,winner_amount*10**decimal);
                    emit WinningAmount(matchs.matchId, bets.better_address,bets.amount,winner_amount);
                }
