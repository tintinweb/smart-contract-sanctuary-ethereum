...
contract Minter is Ownable, ReentrancyGuard, IMinter {
    ...
    constructor(address erc721Core) {
    ...
    function purchase(
        ...
        require(gasPriceInWei > 0, "Wrong price configuration");
        ...
        bool paymentSuccess;
        IGashapondo.PaymentMetadata memory paymentMetadata = erc721CoreContract.getCollectionPaymentMetadata(
            collectionId
        );
        // pay to platform
        uint256 amountSentToPlatform = (amountRequired * paymentMetadata.platformPrimarySalesPercentage) / 100;
        (paymentSuccess, ) = paymentMetadata.platformPrimarySalesAddress.call{value: amountSentToPlatform}("");
        require(paymentSuccess, "Payment to platform failed");
        // pay to addtional payees
        uint256 amountSentToAuthorAndAddtionalPayees = amountRequired - amountSentToPlatform;
        uint256 amountSentToAddtionalPayees;
        if (paymentMetadata.payees.length > 0) {
            for (uint256 i = 0; i < paymentMetadata.payees.length; i++) {
                uint256 amountSentToCurrentPayee = (amountSentToAuthorAndAddtionalPayees *
                    paymentMetadata.payees[i].percentage) / 100;
                (paymentSuccess, ) = paymentMetadata.payees[i].addr.call{value: amountSentToCurrentPayee}("");
                require(paymentSuccess, "Payment to addtional payee failed");
                amountSentToAddtionalPayees += amountSentToCurrentPayee;
            }
        }
        // pay to author
        uint256 amountSentToAuthor = amountSentToAuthorAndAddtionalPayees - amountSentToAddtionalPayees;
        (paymentSuccess, ) = paymentMetadata.authorAddress.call{value: amountSentToAuthor}("");
        require(paymentSuccess, "Payment to author failed");
    }
    ...
    function purchaseWithErc20(
        ...
        // CHECKS
        bool isErc20TokenPaymentSupported = erc721CoreContract.isErc20TokenPaymentSupported(collectionId, erc20Token);
        require(isErc20TokenPaymentSupported, "Erc20 token is not supported");

        uint256 erc20TokenPriceInWei = erc721CoreContract.getCollectionErc20TokenPrice(collectionId, erc20Token);
        require(erc20TokenPriceInWei > 0, "Wrong price configuration");
        ...
        // pay to platform
        uint256 amountSentToPlatform = (amountRequired * paymentMetadata.platformPrimarySalesPercentage) / 100;
        erc20Contract.transferFrom(_msgSender(), paymentMetadata.platformPrimarySalesAddress, amountSentToPlatform);
        // pay to addtional payees
        uint256 amountSentToAuthorAndAddtionalPayees = amountRequired - amountSentToPlatform;
        uint256 amountSentToAddtionalPayees;
        // if (paymentMetadata.payees.length > 0){
        //     for (uint256 i = 0; i < paymentMetadata.payees.length; i++) {
        //         uint256 amountSentToCurrentPayee = amountSentToAuthorAndAddtionalPayees * paymentMetadata.payees[i].percentage / 100;
        //         erc20Contract.transferFrom(_msgSender(), paymentMetadata.payees[i].addr, amountSentToCurrentPayee);
        //         amountSentToAddtionalPayees += amountSentToCurrentPayee;
        //     }
        // }
        // pay to author
        uint256 amountSentToAuthor = amountSentToAuthorAndAddtionalPayees - amountSentToAddtionalPayees;
        erc20Contract.transferFrom(_msgSender(), paymentMetadata.authorAddress, amountSentToAuthor);
    }
