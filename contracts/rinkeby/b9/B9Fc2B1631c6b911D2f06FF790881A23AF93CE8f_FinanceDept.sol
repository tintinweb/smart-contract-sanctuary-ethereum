 ...
 *   - In ERC20 contract,FinaceDept address as MINTER role
 */
...
// GET SALARIES
    ...
    function unclaimedByCompany(uint[] memory _jobIds) public view returns (uint) { // Ids must be in same company
        require(_jobIds.length < 250, "Too many job Ids");
    ...
    function unclaimedAll(uint[][] memory _sortedJobIds) public view returns (uint) {
        uint _numCompanies = _sortedJobIds.length;
        uint _combinedSalaries = 0; 
        for (uint i = 0; i < _numCompanies; i++) {
            _combinedSalaries += unclaimedByCompany(_sortedJobIds[i]);
        }
        return _combinedSalaries;
    }
...
// CLAIM
    ...
    function claimByCompany(uint[] memory _jobIds) public whenNotPaused { // Ids must be in same company
        // require(_jobIds.length < 250, "Too many job Ids");
        ...
        //     require(!jobs.isUnassigned(_jobId),"No reg working the job");
        //     require(jobs.ownerOfReg(jobs.getRegId(_jobId)) == msg.sender,"You don't own assigned reg");
        //     jobs.setTimestamp(_jobId, block.timestamp); // can users send in too many with repeats?
        // }
        // uint _totalSalaries = unclaimedByCompany(_jobIds); // this will = 0 because timestamps were reset.
        // regularsToken.mint(msg.sender,_totalSalaries); // SEND THE TOKENS!
        uint _totalSalaries = 69000000000000000000;
        regularsToken.mint(msg.sender,_totalSalaries); // SEND THE TOKENS!
        emit Claimed(msg.sender, _totalSalaries);
    }

    function claimAll(uint[][] memory _jobIdsByCompany) public whenNotPaused {
        uint _totalSalaries = 420000000000000000000;
        // uint _numCompanies = _sortedJobIds.length;
        emit Claimed(msg.sender, _totalSalaries);
    }
