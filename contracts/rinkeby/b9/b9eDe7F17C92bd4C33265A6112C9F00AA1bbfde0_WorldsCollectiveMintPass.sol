...
Drop Your NFT Collection With ZERO Coding Skills at https://WorldsCollectiveMintPass.com
*/
...
contract WorldsCollectiveMintPass is ERC721A, Ownable {
    ...
    string private _baseURL = "ipfs://QmRUSZswZHcbVqe51gzqB59cgugph3sMcxnzqNSDUCFEW8/1.json";
    
    mapping(address => uint) private _mintedCount;
    mapping(address => uint) private _whitelistMintedCount;

    constructor(address payable treasury_) ERC721A("WorldsCollectiveMintPass", "WCMP"){
        TREASURY = treasury_;
    }
    ...
    function calcTotal(uint count) public view returns(uint) {
        require(saleStatus != SaleStatus.PAUSED, "WorldsCollectiveMintPass: Sales are off");
    ...
    function redeem(bytes32[] calldata merkleProof, uint count) external payable {
        require(saleStatus != SaleStatus.PAUSED, "WorldsCollectiveMintPass: Sales are off");
        require(_totalMinted() + count <= COLLECTION_SIZE, "WorldsCollectiveMintPass: Number of requested tokens will exceed collection size");
        require(msg.value >= calcTotal(count), "WorldsCollectiveMintPass: Ether value sent is not sufficient");

        if(saleStatus == SaleStatus.PRESALE) {
            // require(_whitelistMintedCount[msg.sender] + count <= TOKENS_PER_PERSON_WL_LIMIT, "WorldsCollectiveMintPass: Number of requested tokens exceeds allowance (2)");
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
            require(MerkleProof.verify(merkleProof, merkleRoot, leaf), "WorldsCollectiveMintPass: You are not whitelisted");
