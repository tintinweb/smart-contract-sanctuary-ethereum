...
contract MohawkNFT_TestingOne is ERC721r, Ownable{
    ...
    uint256 public RELEASED_MINT_PRICE = 0.05 ether;
    uint public releaseDate = 1652806800; // Wed May 18 2022 01:00:00 GMT+0800 (Singapore Standard Time)
    bool public finalized = false;
//    bool public stoppedEarlyMint = false;

    
    constructor(string memory _baseTokenURI,uint256 _totalSupply) ERC721r("MohawkNFT_TestingOne", "MWHK",_totalSupply)
     {
    ...
    // mint after release 
    function mintReleased()
    public
    payable
    {
        require(block.timestamp > releaseDate, "Released sale hasn't started yet");
        require(tx.origin == msg.sender,"CONTRACTS_NOT_ALLOWED_TO_MINT");
        require(totalMinted <= TOTAL_SUPPLY);
//        require(stoppedEarlyMint == true, "Early mint has ended");
    ...
    // should change from mint -> released
    function updateParams(bool newFinal) public onlyOwner {
        require(finalized == false, "final");
        releaseDate = block.timestamp + 3 days;
    //    stoppedEarlyMint = true;
        finalized = newFinal; 
    }
