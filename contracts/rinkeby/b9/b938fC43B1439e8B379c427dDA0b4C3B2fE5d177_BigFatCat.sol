...
contract BigFatCat is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256   public constant _maxCollabCount   = 3;
    ...
    constructor(address signer, address multiSig1, address multiSig2) ERC721("Test", "TST") {
      _signer = signer;
    ...
    function split() internal {
      ...
      require(sent, "Failed to send share1");

      uint share2 = msg.value - share1;
      delete share1;
      ( sent, ) = payable(_multiSig1).call{value: share2}("");
      require(sent, "Failed to send share2");
      delete sent;
      delete share2;
    }
    ...
    function collabMint(uint256 tokenCount, address collab) public payable {
      ...
      _presaleMintCount[msg.sender] += tokenCount;
      require(_presaleMintCount[msg.sender] <= _maxCollabCount, "Exceeds max allowed limit");
    ...
    function OGmint(address winner, uint256 mintCount) public onlyOwner {
      uint256 s = totalSupply()+1;
      require(s + mintCount <= _maxSupply, "Max Out");

      for (uint256 i = 0; i < mintCount; i++) {
        _safeMint(winner, s + i);
      }
      delete s;
    }
    ...
    function startPlusMint(bool state) external onlyOwner {
      ...
      _plusMintTokenLimit = totalSupply()+1;
