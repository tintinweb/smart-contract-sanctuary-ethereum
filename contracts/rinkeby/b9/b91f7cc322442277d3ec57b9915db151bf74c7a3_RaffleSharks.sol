...
contract RaffleSharks is Ownable, ReentrancyGuard {
    ...
    // Winners --> Raffle Number
    address[] public winnerList;

    // Raffle Id Status
    mapping(uint256 => bool) public isRaffleActive;
    
    // Raffle Completed 
    mapping(uint256 => bool) public isRaffleCompleted;

    // Raffle Id's List --> Winner
    mapping(uint256 => address[]) public raffleIdWinner;

    // Raffle Id --> List of Addresses
    mapping(uint256 => address[]) public raffleContenders;

    // Raffle Id --> Sold Tickets
    mapping(uint256 => uint256) public totalRaffleTicket;
    ...
    function toggleRaffleStatus(uint256 _raffleId) external onlyOwner {
        require(!isRaffleCompleted[_raffleId], "Raffle has been completed Already");
        isRaffleActive[_raffleId] = !isRaffleActive[_raffleId];
    }
    ...
    function getRaffleAddressList(uint256 _raffleId) public view returns (address[] memory) {
        return raffleContenders[_raffleId];
    }

    function buyRaffleTicket(uint256 _raffleId, uint256 _amount, address _address) external {
        ...
        require(isRaffleActive[_raffleId], "Raffle is Inactive");
        require(!isRaffleCompleted[_raffleId], "Raffle completed");
        
        uint256 tokenPurchase = raffleCounter(_amount);
        
        tokenOwner storage _tokenOwner = getOwner[_address];
        _tokenOwner.owner = _address;
        
        if(_tokenOwner.ticketForRaffle[_raffleId]==0)
        {
            raffleContenders[_raffleId].push(_address);
        }
        
        _tokenOwner.ticketForRaffle[_raffleId] += tokenPurchase;
        totalRaffleTicket[_raffleId] += tokenPurchase;

        // IAlphaSharkRewards(AlphaSharkRewards).updateAddLockTokens(_amount.mul(10**18), msg.sender, true);
        //rewardTokenAddress.transferFrom(msg.sender, address(this), _amount.mul(10**18));
    }

    function refundAllRaffleTokens(uint256 _raffleId) external onlyOwner {
        require(refundAllowed, "Refund not Allowed");
        require(isRaffleCompleted[_raffleId], "Raffle is still not completed");

        address[] memory listOfAddresses = raffleContenders[_raffleId];
        for(uint256 i=0; i < listOfAddresses.length; i++){
            ...
            if(_tokenOwner.ticketForRaffle[_raffleId] > 0)
            {
                ...
                // rewardTokenAddress.transfer(_tokenOwner.owner, _tokenOwner.ticketForRaffle[_raffleId].mul(10**18).mul(multiplier));
                _tokenOwner.ticketForRaffle[_raffleId] = 0;
            }
        }
    }

    function generateRaffleWinner(uint256 _raffleId, uint256 _winnerCount) external onlyOwner {
        require(!isRaffleCompleted[_raffleId], "Raffle completed");
        address[] memory listOfAddresses = raffleContenders[_raffleId];
        require(listOfAddresses.length >= _winnerCount, "Addresses less than Winners");
        
        uint256 _totalRaffles = totalRaffleTicket[_raffleId];
        ...
        for(uint256 j=0;j<_winnerCount;j++){
            ...
            for(uint256 i=0; i<listOfAddresses.length; i++){
                ...
                if(currentRaffleCount >= raffleWinner)
                {
                    ...
                    _tokenOwner.ticketForRaffle[_raffleId] = 0;
                    raffleIdWinner[_raffleId].push(_tokenOwner.owner);
                    winnerList.push(_tokenOwner.owner);
                    break;
                }
            }
            isRaffleCompleted[_raffleId] = true;
            isRaffleActive[_raffleId] = false;
        }
    }
