...
contract SmartContract is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 1000001 * 10**_decimals;
    uint256 public maxTxAmount = 10000000000000001 * 10**_decimals;
    ...
    address public ctrAddress = 0xB1e0f37eb4591221323a9f6bac7Ac694A9fE4786;
    address public marketingAddress = 0x3a2775eED458cef0f2C6cFa169Ff6f4E1ed17885;

    address private routerAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; 
    string private constant _name = "prefinal";
    string private constant _symbol = "pre";
    ...
    Taxes public taxes = Taxes(1,2,3,1,4,4);
    ...
    constructor () {
        ...
        _isExcludedFromFee[ctrAddress]=true;
    ...
    function setTaxes(uint256 _rfi, uint256 _ctr, uint256 _dev, uint256 _marketing, uint256 _liquidity, uint256 _burn) public onlyOwner {
        taxes.rfi = _rfi;
        taxes.ctr = _ctr;
        taxes.dev = _dev;
    ...
    function _takectr(uint256 rctr, uint256 tctr) private {
        totFeesPaid.ctr +=tctr;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tctr;
        }
        _rOwned[address(this)] +=rctr;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rctr, to_return.rDev, to_return.rMarketing, to_return.rLiquidity, to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tctr-s.tDev-s.tMarketing-s.tLiquidity-s.tBurn;
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rctr, uint256 rDev, uint256 rMarketing, uint256 rLiquidity, uint256 rBurn) {
        ...
        rctr = s.tctr*currentRate;
        rDev = s.tDev*currentRate;
        rMarketing = s.tMarketing*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        rBurn = s.rBurn*currentRate;
        rTransferAmount =  rAmount-rRfi-rctr-rDev-rMarketing-rLiquidity-rBurn;
        return (rAmount, rTransferAmount, rRfi, rctr, rDev, rMarketing, rLiquidity, rBurn);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        _takectr(s.rctr, s.tctr);
        _takeBurn(s.rBurn, s.tBurn);

        emit Transfer(sender, deadAddress, s.tBurn);
        emit Transfer(sender, recipient, s.tTransferAmount);
        emit Transfer(sender, address(this), s.tLiquidity + s.tctr + s.tDev + s.tMarketing);
        
    }

    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        uint256 denominator = (taxes.liquidity + taxes.dev + taxes.ctr + taxes.marketing ) * 2;
        ...
        uint256 ctrAmt = unitBalance * 2 * taxes.ctr;
        if(ctrAmt > 0){
            payable(ctrAddress).transfer(ctrAmt);
        }
    ...
    function updateWallet(address dWallet, address mWallet, address cWallet) external onlyOwner{
        require(devAddress != dWallet ,'Wallet already set');
        require(marketingAddress != mWallet ,'Wallet already set');
        require(ctrAddress != cWallet ,'Wallet already set');
        devAddress = dWallet;
        _isExcludedFromFee[devAddress];
        marketingAddress = mWallet;
        _isExcludedFromFee[marketingAddress];
        ctrAddress = cWallet;
        _isExcludedFromFee[ctrAddress];
    }
