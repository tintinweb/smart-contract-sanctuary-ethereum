...
contract RoyaltiesRegistry is OwnableUpgradeable {
    ...
    function getRoyaltiesUserOf(address _contract) external view returns(address) {
     ...
     function checkOwner(address _contract) public view {
        if ((owner() != _msgSender()) && (OwnableUpgradeable(_contract).owner() != _msgSender())) {
            revert("Token owner not detected");
        }
    }

    function setRoyaltiesOfContract(address _contract, uint256 _amount,address _user) public {
    ...
    function changeRoyaltiesOfContract(address _contract, uint256 _newAmount) public {
        checkOwner(_contract);
        getRoyaltiesOfCollection[_contract].percent1000 = _newAmount;
        emit ChangeRoyaltiesForContract(_contract, _newAmount,msg.sender);
    }

    function changeRoyaltiesOwnerOfContract(address _contract,address _user) public {
        checkOwner(_contract);
