...
/**
 * @title JiggleOs contract
 * @author Jiggle Labs
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 * @notice Generate 4294967295 free JiggleOs with the option to mint 21000
 * @notice Yes, Virginia. The JiggleOs project is 100 percent on-chain
 *
 *         gg                                                     _,gggggg,_            
 ...
 *      ,dP"'   Yb             ,d8I'       ,d8I'                                        
 *     ,8'      I8           ,dP'8I      ,dP'8I   This contract contains over 4 billion
 *    ,8'       I8          ,8"  8I     ,8"  8I   indestructible pieces of Ethereum art.
 *    I8,      ,8'          I8   8I     I8   8I   Function drawFreeSVG returns free art.
 *    `Y8,___,d8'           `8, ,8I     `8, ,8I   No gas fees. Minting is not required.
 *      "Y888P"              `Y8P"       `Y8P"                                          
 */
library Base64 {
...
contract JiggleOs is ERC721, ReentrancyGuard, Ownable {
    ...
    mapping (uint => string) public idToTitle;
    mapping (uint => address) public idToSeeder;
    
    constructor() ERC721("JiggleOs", "JGLO") {
        _nextTokenId.increment();
        base_external_url = "https://jiggleos.com/";
        //@dev alternate url: jiggleos.crypto
        }

     //@notice This function mints 21000 unique JiggleOs for 0.1 ether each
     //@notice The seed number must be unique, between 1 and 4294967295
     //@notice The title can be anything between 1 and 32 characters
     //@notice Once minted, the seed and title are unchangeable. Choose wisely
     //@notice The minter, who provides the seed and title, is the progenitor
    function mint(uint32 seed, string memory title) public payable nonReentrant {
        uint id = _nextTokenId.current();
  
        require(titleLength(title) == true, "Title must be between 1-32 characters");
        require(id < 21001, "Sold out");
    ...
    //@notice The seed number must be unique between 1 and 4294967295				
	function drawFreeSVG(uint32 _seed) public pure returns(string memory) {
	...
	function rollMetadata(uint256 _tokenId) internal view returns(string memory) {
		string memory description = " - Generative Ethereum Art";
		string memory progenitor = Strings.toHexString(uint256(uint160(idToSeeder[_tokenId])), 20);
	...
	function remainingSupply() public view returns (uint256) {
		return 21000 - _nextTokenId.current() + 1;
		}
									
	function titleLength(string memory str) internal pure returns (bool){
