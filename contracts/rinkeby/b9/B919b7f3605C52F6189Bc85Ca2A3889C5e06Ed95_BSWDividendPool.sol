...
import "./BatStarWarrior.sol";

contract BSWDividendPool is Ownable, IERC721Receiver {
    ...
    constructor(address _tokenContract){
        BSWNFT = BatStarWarrior(_tokenContract);
    }
    ...
    function stake(uint256 _tokenId) public {
        PoolInfo storage pool = getPoolInfo[cycle];
        require(block.number > pool.startBlock && block.number < pool.endBlock, "pool is not open");
        BSWNFT.safeTransferFrom(msg.sender, address(this), _tokenId);
        uint256 power = 1;
    ...
    function unStake(uint256 _tokenId) public {
        ...
        BSWNFT.safeTransferFrom(address(this), msg.sender, _tokenId);
        pool.count -= 1;
        addressInfo.count -= 1;
        addressInfo.stakePower -= power;
        pool.totalPower -= power;
    ...
    function stakeList(uint256[] memory _tokenIds) public {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            BSWNFT.safeTransferFrom(msg.sender, address(this), _tokenIds[i]);
            uint256 power = 1;
            pool.count += 1;
            addressInfo.count += 1;
            addressInfo.stakePower += power;
    ...
    function unStakeList(uint256[] memory _tokenIds) public {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            BSWNFT.safeTransferFrom(address(this), msg.sender, _tokenIds[i]);
            pool.count -= 1;
            addressInfo.count -= 1;
            addressInfo.stakePower -= power;
            pool.totalPower -= power;
    ...
    function addCycle(uint256 _startBlock, uint256 _endBlock) public onlyOwner {
        ...
        cycle++;
        getPoolInfo[cycle] = PoolInfo(pool.count, _startBlock, _endBlock, 0, pool.totalPower, 0);
    }
    ...
    function fullAddressInfo(address _address) external view returns (UserInfoView memory) {
        ...
        uint256[] memory tokenIds = getNfts(_address);
        return
            UserInfoView(
                addressInfo.count,
    ...
    function getNfts(address _address) public view returns (uint256[] memory) {
        AddressInfo storage addressInfo = addressToInfo[_address];
