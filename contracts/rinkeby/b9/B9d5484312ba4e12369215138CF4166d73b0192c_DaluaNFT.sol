...
contract DaluaNFT is ERC721, ERC721URIStorage, Ownable {
    address wethAddress;
    ...
    function acceptBid(uint256 tokenId, address newOwner, address owner, bytes32 hash, bytes memory signature, uint payThrough, uint256 amount) external payable onlyOwner {
        address signer = hash.recover(signature);
        // require(signer == msg.sender, "Invalid User signature");
        require(bidsOffers[tokenId][newOwner] > 0, "User did not palce bid");
        require(amount == bidsOffers[tokenId][newOwner], "Invalid amount");
