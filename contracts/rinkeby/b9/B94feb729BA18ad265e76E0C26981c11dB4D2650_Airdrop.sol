...
/* @Ken: This contract needs a new name please advise. */
contract Airdrop is ERC721A, Ownable {
    using Counters for Counters.Counter;  
    
    /* No proxy used in this contract as per the client's request */
    constructor(string memory name, string memory symbol, string memory baseTokenURI, uint32 maxTokenSupply)
      ERC721A(name, symbol) {
    ...
    function contractURI() public view returns (string memory) {
      ...
      * @param uris: a list of URIs for the metadata associated with the 
      ...
      * @dev The order and length of all lists must be equal for this function to succeed. */
      ...
      function airDropToken(address[] calldata receivers, string[] calldata  uris) external onlyOwner  {
    ...
    */
    ...
    /* Function for owner of a token to freeze metadata. */
    function freezeYourMetadata(uint256 tokenId, bool freeze) external {
     ...
     /* Below are setter functions for the contract owner in regards to the token URI */
     
     /* Contract owner can set token metadata to be frozen or not. */
     function freezeMetadata(uint256 tokenId, bool freeze) external onlyOwner {
            require(_contractFrozen != true, "Contract_Frozen");
            require(_exists(tokenId), "INVALID_TOKENID");
             _frozenURI[tokenId] = freeze;
            emit permanentURI(_tokenURI[tokenId], tokenId, freeze, msg.sender);
    ...
    function setMintingtoClosed(bool val) external onlyOwner {
        require(closeMinting == false, "ALREADY_CLOSED");
        closeMinting = val;
        emit mintingClosed(val);
    }
