    ...
    LootItems
{
    ...
    constructor()
        ERC1155(
            "https://nft-test-alex.s3.ap-southeast-2.amazonaws.com/NFTTest/metadata/{id}.json"
        )
    {}
    ...
    // *                    PAUSE
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    function setPause() public onlyRole(Roles.ROLE_PAUSER) {
        _pause();
    }

    function unpause() public onlyRole(Roles.ROLE_PAUSER) {
        _unpause();
    }
    ...
    function mintDailyStore(
        ...
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        if (!Settings.publicSaleActive) revert ErrorCodes.PublicSaleInactive();
    ...
    function mintClaim(
        ...
        if ((isOwner == false) && (Settings.requireWhitelist == true)) {
            // Verify Signature
            if (
                !verify(
    ...
    function CheckRequires(
        ...
        for (uint256 index = 0; index < tokenIndices.length; index++) {
            ...
            LootItems.LootItem memory SourceData = LootItems.getLootItem(
                tokenID
            );
            ...
            TotalCost =
                ...
                (
                    SourceData.IsOnSale
                        ? SourceData.WeiSaleCost
                        : SourceData.WeiCost * mintAmount
                );
            ...
            if (shouldCheckCost && msg.value < TotalCost)
                revert ErrorCodes.InsufficientFunds(TotalCost, msg.value);
        }
    }
    ...
    function setSignerAddress(address newSignerAddress)
        public
        onlyRole(Roles.ROLE_ADMIN)
    {
        Settings.signerAddress = newSignerAddress;
    }
...
contract SignatureVerify
{
    ...
    // Signature to compare to
        ...
        bytes32 messageHash = getMessageHash(to, tokenIndices, mintAmounts, networkName, contractAddress, nonce);//, epochTime);
...
contract LootItems is AccessControl, Roles, Settings
{
    ...
    */
    function addLootItem(
        ...
        uint saleCost,
        bool isOnSale ) public onlyRole(Roles.ROLE_ITEM_SETTER)
    {
    ...
    string memory newURI, 
    uint newSaleCost,
    bool isOnSale) public onlyRole(Roles.ROLE_ADMIN)
    {
        if(false == lootItemList[tokenIndex].exists) revert ErrorCodes.LootItem_ItemDoesntExist();
        lootItemList[tokenIndex] = LootItem(newCost, newTotalSupply, newMaxMintPerUser, newURI, newSaleCost, isOnSale, true);
    }
    ...
    */
    ...
    // *                    DAILY STORE
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    function setDailyStore(uint[] calldata newDailyStore) public onlyRole(Roles.ROLE_ADMIN)
    {
...
contract Roles is AccessControl {
    ...
    bytes32 internal constant ROLE_ROLE_SETTER    = keccak256("ROLE_ROLE_SETTER");
    bytes32 internal constant ROLE_ITEM_SETTER    = keccak256("ROLE_ITEM_SETTER");

    bytes32 internal constant ROLE_PAUSER         = keccak256("ROLE_PAUSER");
    bytes32 internal constant ROLE_SET_URI        = keccak256("ROLE_SET_URI");
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

    // TODO
    // Revisit roles to ensure that we have certain roles for support and certain roles for server so we can control different aspects of the contract
    // Possibly auto assign roles in constructor for support and server wallet
    // Look into gnosis safe
...
{
    string public baseURI = "https://nft-test-alex.s3.ap-southeast-2.amazonaws.com/NFTTest/metadata/";
    bool requireWhitelist = true;
    address signerAddress = 0x417bA3FaC3F0ED2B1c5424658DD549f742A6c196;
    bool publicSaleActive = false;

    function setPublicSaleActive(bool _Active) public onlyRole(Roles.ROLE_ADMIN) {
...
library ErrorCodes
{
    ...
    error InsufficientFunds(uint Expected, uint Actual);
    error MaxAmountMinted(uint Expected, uint Actual);
