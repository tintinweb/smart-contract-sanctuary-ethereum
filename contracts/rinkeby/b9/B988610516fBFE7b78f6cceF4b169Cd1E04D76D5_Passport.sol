    ...
    IPassport
{
    ...
    function claim(uint256 _amount) external payable {
        ...
        require(_amount <= claimAmount, "amount exceeds claim limit");
        if (claimFee > 0) require(msg.value == claimFee * _amount, "fee amount incorrect");
        for (uint256 i = 0; i < _amount; i++) {
            _mint(_msgSender());
        }
    }

    /// @notice Mint token(s) to caller if they are on the supplied whitelist
    /// @dev Must first set whitelist root, enable claim, & set fee (if desired). Merkle tree can be generated with the Javascript library "merkletreejs", the hashing algorithm should be keccak256 and pair sorting should be enabled. Leaf is abi encodePacked address & amount
    ...
    function claimWhitelist(
        ...
        require(whitelistClaimEnabled, "whitelist claim disabled");
        if (whitelistClaimFee > 0) require(msg.value == whitelistClaimFee * _claimAmount, "fee amount incorrect");
        ...
        require(!whitelistClaimed[_msgSender()], "already whitelist claimed");
    ...
    function setMaxSupply(uint256 _maxSupply) external onlyRole(MANAGER_ROLE) {
        require(_maxSupply >= this.totalSupply(), "max < total supply");
        maxSupply = _maxSupply;
    ...
    /// @param _claimEnabled If the whitelist claim is enabled
    function setWhitelistClaimEnabled(bool _claimEnabled) external onlyRole(MANAGER_ROLE) {
        whitelistClaimEnabled = _claimEnabled;
    }

    /// @notice Allows manager to set the merkle tree root for the whitelist
    ...
    /// @notice Allows manager to set the whitelist claim fee & merkle root
    /// @dev Merkle tree can be generated with the Javascript library "merkletreejs", the hashing algorithm should be keccak256 and pair sorting should be enabled. Leaf is abi encodePacked address & amount
    /// @param _claimFee The claim fee in wei for the whitelist
    /// @param _whitelistRoot Merkle tree root
    ...
    function _mint(address to) internal returns (uint256) {
        if (maxSupply > 0) require(_tokenIdCounter.current() < maxSupply, "max supply reached");
    ...
    function _beforeTokenTransfer(
        ...
        require((from == address(0) || ((to == address(0)) || transferEnabled)), "transfer disabled");
