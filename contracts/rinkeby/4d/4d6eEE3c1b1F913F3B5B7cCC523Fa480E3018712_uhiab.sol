...
contract uhiab is Ownable, ERC721A, ReentrancyGuard {

  event xxx(uint256 difference);
  ...
  ) ERC721A("uhiab", "uhiab", maxBatchSize_, collectionSize_) {
  }
  ...
  modifier afterClosed() {
    require(saleConfig.isFinished == true , "Sale is not finished.");
    _;
  }

  function presaleMint(uint256 quantity) external payable callerIsUser onlyWhiteList{
    ...
    emit xxx(block.timestamp - _saleStartTime);
    // require(block.timestamp >= _saleStartTime, "presale has not started yet");
    // require(block.timestamp <= _publicsaleStartTime, "presale is finished");
  ...
  function setSaleInfo(uint32 timestamp, uint32 presalePeriod, uint64 _presalePrice, uint64 _publicsalePrice, uint64 _publicsaleMaxMint) external onlyOwner {
  ...
  function airdrop(address[] memory airdropAddress, uint256 numberOfTokens) external onlyOwner {
    uint256 quantity = airdropAddress.length * numberOfTokens;
    require(quantity <= reserved, "Out of reserved amount.");
    require(totalSupply() + quantity <= collectionSize, "reached max supply");

    reserved -= quantity;
    for (uint256 k = 0; k < airdropAddress.length; k++) {
      _safeMint(airdropAddress[k], numberOfTokens);
    }
  }
