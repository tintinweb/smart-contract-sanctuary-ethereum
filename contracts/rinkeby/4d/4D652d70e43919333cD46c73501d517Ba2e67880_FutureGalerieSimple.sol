...
/*
	NOTES:
		- There is no plans for claming/minting, the owner plan to airdrop 203
		  (mint and transfer) all at the initial level (member)
		- Owners wish to update the metadata associated with a wallet address 
		  manually (simply check if given address owns an NFT - update all?)
	...
	QUESTIONS:
		X What is the payoutAddress? 0xa34da04b871A7dD50564487822df56766e442Fa8
		X What is the owner address? 0xb8323B4B2FBbFa55D6697AdABC18725AA9245Ba3
		X What is the deployer private key? ->in .env file
		X What royalty percentage do you want coded in (even tho some platforms limit this)? 10%
		X Are there infinite tokens? Should I set a max of 1000 so you can do eight 100 new
		  "drops"? 500?
		X Animation preview image
		X How will minting / purhcasing work in the future? Built-in ways to purchase
*/
...
contract FutureGalerieSimple is Satoshigoat {
	...
	string [NUM_LEVELS] _levelHashes = ["QmRJfz8yiWPx4kKnEbDDBsboYDYvHYdjygQsBb1e2q9poV",
						  	  		    "QmaaFeKMVPYMrdmnrZn8UUsT7KzN9LydRMrhwPLuZdb1g1",
						  	   		    "QmcwQsr3KKXKtdjCm9aKhUoVo2yZcPB1Yi1waiVaQmWpms",
						  	   			"QmSshDzvDUZ5aLean5vwD9U1nTZ5B2sKerPLmuDvUkNe9P"];
	// -----------
	// RESTRICTORS
	// -----------
	...
	{
		_contractURI = "ipfs://QmXJP2uetmRWdzBM8Nfikw5mwHEhrDck1b5aGa53FjVReF";
	...
	(
		...
		uint addressArrayLength
	) 
		external 
		isSquad
		nonReentrant
		enoughSupply(addressArrayLength)
	{
		//NOTE: if address[] is calldata i may be able to recopy in here and still do it that way
		if (addressArrayLength != airdropAddressesArray.length)
			revert DataError("length of array doesn't match addressArrayLength");
		unchecked {
			uint currentIdx = _totalMinted();
			uint lastIdx = currentIdx + addressArrayLength;

			uint i;
			for (i = currentIdx; i < lastIdx; i++) {
				_safeMint(airdropAddressesArray[i], 1);
				setStorageTokenIDHash(i, _levelHashes[0]);
			}
		}
	}
