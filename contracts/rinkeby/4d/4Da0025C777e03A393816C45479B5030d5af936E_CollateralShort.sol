...
contract Collateral is ICollateralLoan, Owned, MixinSystemSettings {
    ...
    function liquidationAmount(uint256 id)
        public
        view
        returns (uint256 liqAmount)
    {
        Loan memory loan = loans[id];
        return
    ...
    function _withdraw(uint256 id, uint256 amount)
        internal
        rateIsValid
        issuanceIsActive
        ...
        returns (uint256 collateralLiquidated)
    {
        ...
        uint256 liqAmount = _collateralUtil().liquidationAmount(
            loan,
            minCratio,
            collateralKey
        );
    ...
    function _draw(uint256 id, uint256 amount)
        internal
        rateIsValid
        issuanceIsActive
        returns (uint256, uint256)
    ...
    function _accrueInterest(Loan storage loan) internal {
        (uint256 differential, uint256 newIndex) = manager.accrueInterest(
            loan.interestIndex,
            loan.currency,
            loan.short
        );
        ...
        uint256 interest = loan.interestIndex == 0
            ? 0
    ...
    function _processPayment(Loan storage loan, uint256 payment) internal {
        ...
        if (loan.accruedInterest > 0) {
            uint256 interestPaid = payment > loan.accruedInterest
    ...
    function _getLoanAndAccrueInterest(uint256 id, address owner)
        internal
...
contract CollateralShort is Collateral {
    ...
    function open(
        ...
        IERC20(address(_synthsUSD())).transferFrom(
            msg.sender,
            address(this),
            collateral
        );
    ...
    function deposit(
        ...
        require(
            amount <=
                IERC20(address(_synthsUSD())).allowance(
                    msg.sender,
                    address(this)
                ),
            "Allowance too low"
    ...
    function liquidate(
        ...
        IERC20(address(_synthsUSD())).transfer(
            msg.sender,
            collateralLiquidated
        );
    }
        ...
        returns (uint256 amount, uint256 collateral)
    {
        ...
        require(
            payment <= loan.amount.add(loan.accruedInterest),
            "Payment too high"
        );
        _processPayment(loan, payment);
        ...
        (uint256 expectedAmount, uint256 exchangeFee, ) = _exchanger()
        .getAmountsForExchange(payment, loan.currency, sUSD);
    ...
    function _closeWithCollateral(address borrower, uint256 id)
        internal
        returns (uint256 amount, uint256 collateral)
    {
        // 0. Get the loan to repay and accrue interest.
        ...
        (amount, collateral) = _repayWithCollateral(
            borrower,
            id,
            amountToRepay
        );
