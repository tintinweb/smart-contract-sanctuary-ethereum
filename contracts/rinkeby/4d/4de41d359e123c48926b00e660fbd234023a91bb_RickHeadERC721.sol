...
contract RickHeadERC721 is ERC721, Pausable, Ownable {
    ...
    uint64 private maxTeamMintsPerAddress = 1;
    uint64 private maxPrivateMints = 1000;
    uint64 private totalPrivateMints = 0;
    uint64 public mintPrice = 0.15 ether;
    ...
    address private _adminSigner = 0xf38932D94BC4b018924b184c986F9268453fb028;
    ...
    function setMaxTeamMintsPerAddress(uint64 max)
    external
    onlyOwner {
        maxTeamMintsPerAddress = max;
    }
    ...
    whenNotPaused {
        ...
        if (phase == SalePhase.Private) {
            ...
            require(
                ...
                "Max whitelisted mint limit has been reached"
            );
            require(
                totalPrivateMints + amount <= maxPrivateMints,
                "Max supply to mint limit has been reached for the whitelist phase"
            );
            ...
            require(
                ...
                "you can only mint 1 NFTs during Presale"
            );
        }
    ...
    whenNotPaused{
        require(phase != SalePhase.Locked, "No sale phase is open");
        require(phase != SalePhase.Public, "No sale phase is open");

        uint256 currentSupply = _tokenId.current();
        bytes32 digest = keccak256(
            abi.encode(CouponType.Team, msg.sender)
        );
        require(_isVerifiedCoupon(digest, coupon), "Invalid coupon");
        require(
            TeamMintsCount[msg.sender] <= maxTeamMintsPerAddress,
            "Each member can mint only one nft"
        );
        require(
    ...
    function freeMints(address user) public view returns(uint256) {
        if (maxTeamMintsPerAddress >= TeamMintsCount[user]) {
            return maxTeamMintsPerAddress - TeamMintsCount[user];
        }
        return 0;
    }
