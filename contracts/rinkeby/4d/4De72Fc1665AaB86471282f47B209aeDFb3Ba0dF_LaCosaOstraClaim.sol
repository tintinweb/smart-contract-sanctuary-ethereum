...
contract LaCosaOstraClaim {
    ...
    */
    function claim(uint256[] calldata _tokenIds) external payable  {
        require(claimOpen, "Claim is closed");
        uint256 totalClaimed = 0;
        for(uint256 i = 0; i < _tokenIds.length; i++){
            if(laCosaOstra.ownerOf(_tokenIds[i]) == msg.sender && !claimedTokens[_tokenIds[i]]) {
                totalClaimed += 1;
                claimedTokens[_tokenIds[i]] = true;
            }
        }
