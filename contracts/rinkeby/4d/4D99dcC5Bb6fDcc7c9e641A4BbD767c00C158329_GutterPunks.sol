...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _transfer(
        ...
        for (uint256 i = 0; i < airdrops.length; i++) {
            ...
            adt.parentTokenTransferred(from, tokenId);
        }
...
contract GutterPunks is ERC721A, Ownable {
    ...
    event SetRoyaltyInfo(
        ...
        uint256 royaltyAmountNumerator
    );
    ...
    string internal _placeholderURI = "https://api.gutterpunks.xyz/metadata/1";
    ...
    constructor() ERC721A("Gutter Punks", "GP") {}
    ...
    function finalize() external onlyOwner notFinalized {
        require(_isRevealed, "Must be revealed to finalize");
        _isFinalized = true;
        emit Finalized();
    }
    ...
    function setPlaceholderURI(string calldata placeholderURI)
        external
        onlyOwner
    {
    ...
    function mintReservedTokens(address recipient, uint256 numToMint)
        external
        onlyOwner
    {
        require(
        ...
        _mint(recipient, numToMint, "", true);
    }
    ...
    function mintPresale(
        ...
        bytes32 leaf = keccak256(
            abi.encodePacked(msg.sender, maxMints, voucherAmount)
        );
        ...
        if (presaleExtrasNeeded > 0) {
            _presaleExtras -= presaleExtrasNeeded;
        }
    ...
    function royaltyInfo(
        ...
        return (
            _royaltyRecipient,
            (salePrice * _royaltyAmountNumerator) / ROYALTY_AMOUNT_DENOMINATOR
        );
    }
    ...
    function _mintInner(uint256 numToMint, uint256 voucherAmount) internal {
        ...
        _mint(msg.sender, numToMint, "", true);
    }
...
abstract contract AirdropToken {
    function parentTokenTransferred(address from, uint256 tokenId)
        public
        virtual;
}