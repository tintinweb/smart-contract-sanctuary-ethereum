...
contract NarutoPanels is ERC721URIStorage, VRFConsumerBaseV2, Ownable {
    ...
    constructor(
        ...
        console.log("This is my NFT contract. Datebayo!");
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        if (index == 0) {
            tokenUri = i_sadTokenUrisClassS[
                uint256(NftClassS(moddedRng % (uint256(NftClassS.LAST) + 1)))
            ];
        } else if (index == 1) {
            tokenUri = i_sadTokenUrisClassA[
                uint256(NftClassA(moddedRng % (uint256(NftClassA.LAST) + 1)))
            ];
        } else {
            tokenUri = i_sadTokenUrisClassB[
                uint256(NftClassB(moddedRng % (uint256(NftClassB.LAST) + 1)))
            ];
        }
