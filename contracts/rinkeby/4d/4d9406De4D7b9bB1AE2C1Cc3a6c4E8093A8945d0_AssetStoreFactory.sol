    ...
    ReentrancyGuardUpgradeable
{
    ...
    // IMembership Contract Address
    address private IMembershipFactoryAddress;

    // Storing all AssetStore Contract Addresses
    address[] private AssetStoreContractAddresses;

    // Mapping User to a Specific Contract Address
    mapping(address => address) private UserToAssetStoreContract;

    event AssetStoreCreated(address _user, address assetStoreAddress, string uid);
    ...
    * @dev checkAssetContract
    ...
    // Function to deployAssetStore for each user
    function deployAssetStore() external {
        address _userAddress = UserToAssetStoreContract[msg.sender];
        require(
            _userAddress == address(0),
            "User already has an AssetStore contract deployed"
        );
        IMembershipFactory _membershipFactory = IMembershipFactory(
            IMembershipFactoryAddress
        );
        require(
            _membershipFactory.getUserMembershipAddress(msg.sender) !=
                address(0),
            "User does not have a membership contract deployed"
        );
        AssetsStore assetStore = new AssetsStore();
        address _membershipAddress = _membershipFactory
            .getUserMembershipAddress(msg.sender);
        ...
        UserToAssetStoreContract[msg.sender] = address(assetStore);
        emit AssetStoreCreated(msg.sender, address(assetStore), IMember(IMemberAddress).getUID(msg.sender));
    }

    // Function to return assetStore Address of a specific user
    function getAssetStoreAddress() external view returns (address) {
        return UserToAssetStoreContract[msg.sender];
    }

    //Function set IMembershipFactoryAddress
    function setIMembershipAddress(address _Membership) external onlyOwner {
        IMembershipFactoryAddress = _Membership;
    }

    // Function to get IMembershipFactoryAddress
    function getIMembershipAddress() external view onlyOwner returns (address) {
        return IMembershipFactoryAddress;
    }
    ...
    function setIMember(address _addr) external onlyOwner {
        for (uint256 i = 0; i < AssetStoreContractAddresses.length; i++) {
            IAssetStore(AssetStoreContractAddresses[i]).setIMember(_addr);
        }
        IMemberAddress = _addr;
    }

    // Set Approval Active for a Specific UID
    function setApprovalActiveforUID() external onlyOwner {
        string memory _userUID = IMember(IMemberAddress).getUID(msg.sender);
        require(bytes(_userUID).length > 0, "UID is not found");
        IAssetStore(UserToAssetStoreContract[msg.sender]).setApprovalActive(
            _userUID
        );
    }
}
    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier checkIfMemberIsUser(address _user) {
        MembershipStruct memory membership = membershipInfoOfAddress[_user];
        require(
            membership.user != address(0),
    ...
    // Function to return membership information of the user
    function getMembership() external view returns (MembershipStruct memory) {
        return membershipInfoOfAddress[msg.sender];
    }

    // Function to check of membership is active for the user
    function checkIfMembershipActive(address _user)
        public
        view
        checkIfMemberIsUser(_user)
        returns (bool status)
    {
        MembershipStruct storage _membership = membershipInfoOfAddress[_user];
        if (block.timestamp >= _membership.membershipEnded) {
            status = false;
        } else {
            status = true;
        }
    ...
    // Function to set new member address
    function setMemberAddress(address _newMemberAddress) public {
        require(
            msg.sender == IMembershipFactoryAddress,
            "Only the factory can change the member address"
        );
        IMemberAddress = _newMemberAddress;
    }

    // Function to get new member address
    function getMemberAddress() public view returns (address) {
        return IMemberAddress;
    }

    function redeemUpdate(address _user) public {
        checkIfMembershipActive(_user);
        MembershipStruct storage _membership = membershipInfoOfAddress[_user];
        _membership.updatesPerYear = _membership.updatesPerYear - 1;
    ...
    ReentrancyGuardUpgradeable
{
    ...
    // IMembership Contract Address
    address private IAssetStoreFactoryAddress;
     ...
     * @dev initialize - Initializes the function for Ownable and Reentrancy.
     */
    function initialize(address memberAddress, address _membershipAddress, address _assetStoreFactory) public initializer {
        __Context_init_unchained();
        __Ownable_init();
        __ReentrancyGuard_init();
        IMemberAddress = memberAddress;
        IMembershipAddress = _membershipAddress;
        IAssetStoreFactoryAddress = _assetStoreFactory;
        _approvalId = 0;
    }
    ...
    function storeAssetsAndBackUpApprovals(
        ...
        checkUserHasMembership(msg.sender);
        address[] memory _backUpWallets = IMember(IMemberAddress)
            .getBackupWallets(uid);
    ...
    function storeAssetsApprovals(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            IAssetStoreFactory(IAssetStoreFactoryAddress).checkAssetContract(
                contractAddress,
                tokenType
            );
            require(
        ...
        IMembership(IMembershipAddress).redeemUpdate(msg.sender);
    }
    ...
    function editApproval(
        ...
        for (uint256 i = 0; i < approval_.length; i++) {
            if (approval_[i].approvalId == approvalId) {
                ...
                IAssetStoreFactory(IAssetStoreFactoryAddress)
    ...
    // Function to check if user has membership
    function checkUserHasMembership(address _user) public view {
        IMembership _membership = IMembership(IMembershipAddress);
        bool _MembershipActive = _membership.checkIfMembershipActive(_user);
        if (_MembershipActive == false) {
            revert("User does not have a membership");
        }
    }

    //Function set IMembershipAddress
    function setIMembershipAddress(address _Membership) external onlyOwner {
        IMembershipAddress = _Membership;
    }

    // Function to get IMembershipAddress
    function getIMembershipAddress() external view onlyOwner returns (address) {
        return IMembershipAddress;
    }

    //Function set IAssetStoreFactoryAddress
    function setAssetStoreFactoryAddress(address _assetStore)
        external
        onlyOwner
    {
        IAssetStoreFactoryAddress = _assetStore;
    }

    // Function to get IAssetStoreFactoryAddress
    function getAssetStoreFactoryAddress()
        external
        view
        onlyOwner
        returns (address)
    {
        return IAssetStoreFactoryAddress;
    }
}
...
interface IMember {
    ...
    */
    ...
    function deleteAllBackUp() external;
     ...
     // Function to check if user has membership 
    function checkUserHasMembership(address _userAddress) view external ;


    //Function set IMembershipAddress
    function setIMembershipAddress(address _user, address _Membership, address _factoryAddress) external;

    // Function to get IMembershipAddress
    function getIMembershipAddress() view external  returns(address);
}
...
interface IMembership {
    // Function to check of membership is active for the user
    function checkIfMembershipActive(address _user) external view returns (bool);
    ...
    // Function to set new member address
    function setMemberAddress(address _newMemberAddress) external;


    function getMemberAddress() view external returns(address);

    function getMembership() external view returns (MembershipStruct memory);


    function redeemUpdate(address _user) external;
...
interface IMembershipFactory {
    ...
    function transferToPool() payable external;

     // Function to return users membership contract address
    function getUserMembershipAddress(address _user) view external returns(address);
}
