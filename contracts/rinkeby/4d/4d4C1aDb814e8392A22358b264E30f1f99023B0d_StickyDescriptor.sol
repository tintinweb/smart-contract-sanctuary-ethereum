...
contract StickyDescriptor is  IJellyContract, NFTDescriptor {
    ...
    function dataURI(uint256 tokenId) public view returns (string memory) {
        string memory name = string(abi.encodePacked('lock #', tokenId.toString()));
    ...
    function _tokenImage(uint _tokenId, string memory _balanceOf, uint _locked_end, string memory _value) internal pure returns (string memory output) {
        ...
        output = string(abi.encodePacked(output, "token id ", _tokenId.toString(), '</text><text x="10" y="40" class="base">'));
        output = string(abi.encodePacked(output, "ve balance ", _balanceOf, '</text><text x="10" y="60" class="base">'));
        output = string(abi.encodePacked(output, "lock end date ", timestampToString(_locked_end), '</text><text x="10" y="80" class="base">'));
        output = string(abi.encodePacked(output, "staked ", _value, '</text><text x="150" y="330" class="base">refresh metadata for latest details</text></svg>'));
    }
