...
contract City is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    event CrimeCommitted(uint256 tokenId, bool successful, uint256 location, uint256 reward);
    event SpentBills(uint256 tokenId, uint256 amount, uint256 result);
    ...
    IThugCityNFT public citizen = IThugCityNFT(0xD80809eEeB5D9fb25e033f97252741434259a192);

    //**HARDCODE**: Initialize bills CHANGE TO PARAM FOR MAINNET
    IBills public bills = IBills(0xfa0ff1339d8539f04421C9946DDF7776719736a8);

    //**HARDCODE**: Initialize leaderboard CHANGE TO PARAM FOR MAINNET
    ILeaderboard public leaderboard = ILeaderboard(0x7EC3e13bfB1EabFdCdfd9f2D06e50EA470101cC8);
    ...
    function crime(uint256 tokenId, uint256 location) external whenNotPaused returns (bool successful) {
        require(!citizen.isCop(tokenId));
        ...
        if(location == 4){
            if(successful && reward == 1){
                leaderboard.addCash(tokenId);
            }else if(successful && reward == 2){
                leaderboard.addCashBag(tokenId);
            }else if(!successful){
                if (bankStakedCops.length != 0) {
                    uint256 randCop = getRandomNumber(tokenId, bankStakedCops.length);

                    if (reward == 2) {
                        leaderboard.addCashBag(bankStakedCops[randCop]);
                    } else {
                        leaderboard.addCash(bankStakedCops[randCop]);
                    }
                }
            }
        }else if(location == 3){
            if(successful && reward == 1){
                ...
                if (casinoStakedCops.length != 0) {
                    uint256 randCop = getRandomNumber(tokenId,casinoStakedCops.length);
                    
                    if (reward == 2) {
                        leaderboard.addChipStacks(casinoStakedCops[randCop]);
                    } else {
                        leaderboard.addChips(casinoStakedCops[randCop]);
                    }
                }
            }
        }else if(location == 2){
            if(successful && reward == 1){
                ...
                if (prisonStakedCops.length != 0) {
                    uint256 randCop = getRandomNumber(tokenId,prisonStakedCops.length);
                    
                    if (reward == 2) {
                        leaderboard.addGangLeaders(prisonStakedCops[randCop]);
                    } else {
                        leaderboard.addStreetThugs(prisonStakedCops[randCop]);
                    }
                }
            }
        }else if(location == 1){
            if(successful && reward == 1){
                ...
                if (dealershipStakedCops.length != 0) {
                    uint256 randCop = getRandomNumber(tokenId,dealershipStakedCops.length);
                    if (reward == 2) {
                        leaderboard.addSuperCar(dealershipStakedCops[randCop]);
                    } else {
                        leaderboard.addSportsCar(dealershipStakedCops[randCop]);
                    }
                }
            }
        }
        emit CrimeCommitted(tokenId, successful, location, reward);
    }
    ...
    // ADD FUNCTIONALITY TO USE WALLET FUNDS
