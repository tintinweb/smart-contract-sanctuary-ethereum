...
contract ExodiaFrostEmblem is ERC721A, Ownable {
    ...
    /**
     * @dev Allows Exodia Elite NFT holders to mint 2 for free
     *
     * Requirements:
     *
     ...
     * - If user wants to mint more than 2, he will get 2 free mints and pay for the rest
     */
    function mintElite(uint256 quantity, bytes32[] calldata proof) external payable {
        ...
        if (_numberMinted(msg.sender) + quantity > 2) {
            require(msg.value >= (mintPrice * quantity) - (mintPrice * 2), 'Value');
        }
    ...
    /**
     * @dev Allows holders of both the Fire Emblem NFT & the Thunder Emblem NFT to mint 1 for free
     *
     * Requirements:
     *
     * - Presale must be active
     ...
     * - Total number minted cannot be above max mint supply
     * - Total user amount minted cannot be above max user mint amount
     * - If user wants to mint more than 1, he will get 1 free mint and pay for the rest
     */
    function mintEmblems(uint256 quantity, bytes32[] calldata proof) external payable {
        ...
        if (_numberMinted(msg.sender) + quantity > 1) {
            require(msg.value >= (mintPrice * quantity) - mintPrice, 'Value');
        }
