...
contract Converter is IConverter, Ownable {
    ...
    function _tokenPath(address tokenIn_, address tokenOut_) internal view returns (address[] memory) {
        require(tokenIn_ != tokenOut_, "Converter: Tokens cannot be the same");
    ...
    function swapMaxTokenOut(
        ...
        IERC20(tokenIn_).safeTransferFrom(_msgSender(), address(this), amountIn_);
        IERC20(tokenIn_).safeApprove(router, amountIn_);
    ...
    function swapMaxEthIn(address tokenOut_) external payable override returns (uint256) {
        (bool isWeth, address[] memory path) = _ethInPath(tokenOut_);

        if (isWeth) {
            address weth = UniswapV2Router02(router).WETH();
            WETH(weth).deposit{value: msg.value}();
            IERC20(weth).safeTransfer(_msgSender(), msg.value);
    ...
    function swapMaxEthOut(address tokenIn_, uint256 amountIn_) external override returns (uint256) {
        ...
        if (isWeth) {
            ...
            require(success, "Converter: Failed to transfer ETH to caller. Make sure contract accepts payment");
    ...
    function maxAmountEthOut(address tokenIn_, uint256 amountIn_) external view override returns (uint256) {
        ...
        if (isWeth) return amountIn_;
        else return UniswapV2Router02(router).getAmountsOut(amountIn_, path)[path.length - 1];
    }
    ...
    function minAmountTokenInEthOut(address tokenIn_, uint256 amountOut_) external view override returns (uint256) {
        (bool isWeth, address[] memory path) = _ethInPath(tokenIn_);

        if (isWeth) return amountOut_;
        else return UniswapV2Router02(router).getAmountsIn(amountOut_, path)[0];
    }
