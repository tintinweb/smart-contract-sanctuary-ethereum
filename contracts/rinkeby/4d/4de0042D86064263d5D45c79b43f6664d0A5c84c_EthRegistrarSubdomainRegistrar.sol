...
contract EthRegistrarSubdomainRegistrar {
    ...
    event NewRegistration(bytes32 label, bytes32 subdomain, address owner, address reserver, uint256 price);
    ...
    function register(bytes32 label, string calldata subdomain, address resolver) external not_stopped owner_only(label) {
        ...
        emit NewRegistration(label, subdomainLabel, msg.sender, subdomainOwner, domain.price[reserve.subscription]);
    }
