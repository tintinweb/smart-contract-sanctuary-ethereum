...
contract NROSale is Ownable, ReentrancyGuard {
    ...
    // set time
    uint64 public immutable whitelistStartTime = 1655881200;
    uint64 public immutable whitelistEndTime = 1655882100;

    uint64 public immutable publicStartTime = 1655882100;
    uint64 public immutable publicEndTime = 1655883000;

    uint64 public immutable withdrawTime = 1655884200;
    ...
    mapping(address => bool) usersParticipated;
        ...
        nonReentrant
    {
        ...
        bool isParticipated = usersParticipated[msg.sender];
        if (isParticipated == false) {
            usersParticipated[msg.sender] == true;
            ++usersCount;
    ...
    function getUsers(uint256 start, uint256 end)
        public
        view
        returns (address[] memory)
    {
        address[] memory array = new address[](end - start + 1);
        uint256 j = 0;
