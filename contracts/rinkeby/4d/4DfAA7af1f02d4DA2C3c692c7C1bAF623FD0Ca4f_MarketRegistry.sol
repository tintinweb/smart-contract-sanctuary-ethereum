    ...
    TellerASResolver
{
     ...
     * @param _lenderAddress The address of the lender to add to the market.
    ...
    function attestLender(
        ...
        markets[_marketId].lenderAttestationIds[_lenderAddress] = uuid;
        // Add lender address to market set
        markets[_marketId].verifiedLendersForMarket.add(_lenderAddress);

        emit LenderAttestation(_marketId, _lenderAddress);
    }

    /**
     * @notice Removes a lender from a market via delegated revocation.
     * @dev The signature must match that of the market owner.
     * @param _marketId The market ID to remove the lender from.
     * @param _lenderAddress The address of the lender to remove from the market.
     * @param _v Signature value
     * @param _r Signature value
     * @param _s Signature value
     */
    function revokeLender(
        ...
        // Get stored lender attestation ID for the market ID
        bytes32 uuid = markets[_marketId].lenderAttestationIds[_lenderAddress];
        // Revoke lender attestation
        _revoke(_marketId, uuid, _v, _r, _s);
        // Remove lender address from market set
        markets[_marketId].verifiedLendersForMarket.remove(_lenderAddress);

        emit LenderRevocation(_marketId, _lenderAddress);
    }
     ...
     * @param _borrowerAddress The address of the borrower to add to the market.
     * @param _v Signature value
     * @param _r Signature value
     * @param _s Signature value
     */
    function attestBorrower(
        ...
        bytes32 _r,
        bytes32 _s
    ) external withAttestingSchema(borrowerAttestationSchemaId) {
        // Submit attestation for borrower to join a market (attestation must be signed by market owner)
        bytes32 uuid = _attest(
            _marketId,
            _borrowerAddress,
            _expirationTime,
        ...
        // Store the borrower attestation ID for the market ID
        markets[_marketId].borrowerAttestationIds[_borrowerAddress] = uuid;
        // Add borrower address to market set
        markets[_marketId].verifiedBorrowersForMarket.add(_borrowerAddress);

        emit BorrowerAttestation(_marketId, _borrowerAddress);
    }
     ...
     * @param _borrowerAddress The address of the borrower to remove from the market.
     * @param _v Signature value
     * @param _r Signature value
     * @param _s Signature value
     */
    function revokeBorrower(
        ...
        // Get stored borrower attestation ID for the market ID
        bytes32 uuid = markets[_marketId].borrowerAttestationIds[
            _borrowerAddress
        ];
        // Revoke borrower attestation
        _revoke(_marketId, uuid, _v, _r, _s);
        // Remove borrower address from market set
        markets[_marketId].verifiedBorrowersForMarket.remove(_borrowerAddress);

        emit BorrowerRevocation(_marketId, _borrowerAddress);
    }
    ...
    /**
     * @notice Attests a stakeholder (borrower/lender) for a market.
    ...
    /**
     * @notice Revokes a stakeholder (borrower/lender) for a market.
     * @param _marketId The market ID to add a stakeholder to.
     * @param _uuid The stored stakeholder attestation ID to revoke for the market ID
     * @param _v Signature value
     * @param _r Signature value
     * @param _s Signature value
     */
    function _revoke(
        ...
        address attestor = markets[_marketId].owner;
        tellerAS.revokeByDelegation(_uuid, attestor, _v, _r, _s);
    }
