...
contract TimeLockPool1 is BasePool, ITimeLockPool {
    ...
    uint256 public constant MIN_LOCK_DURATION = 0; // REVERT THIS TO 7 DAYS
    ...
    function deposit(uint256 _amount, uint256 _duration, address _receiver) external override updateReward(_receiver) {
        ...
        depositsOf[_receiver].push(
            ITimeLockPool.Deposit({
                kek_id: keccak256(abi.encodePacked(_receiver, block.timestamp, _amount)),
                amount: _amount,
                multiplier: getMultiplier(duration),
