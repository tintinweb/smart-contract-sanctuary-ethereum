...
contract test is ERC721A, Ownable, ReentrancyGuard {
	...
	uint256 public maxTeamReserve = 1;
    uint256 public chanceFreeMintsAvailable = 25;
	...
	uint256 public cost = 0.01 ether;//easier for testing purposes will change once price TBA
    uint256 public whitelistCost = 0.01 ether;//easier for testing purposes will change once price TBA

  constructor(string memory baseURI) ERC721A(tokenName, tokenSymbol) {
  ...
  function mint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable nonReentrant{
    ...
    uint256 refund = chanceFreeMintsAvailable > 0 && _mintAmount > 0 && isFreeMint()
            ? _mintAmount * cost
            : 0;

        if (refund > 0) {
            chanceFreeMintsAvailable = chanceFreeMintsAvailable - _mintAmount;
        }
  ...
  function setChanceFreeMintsAvailable(uint256 _chanceFreeMintsAvailable) public onlyOwner {
