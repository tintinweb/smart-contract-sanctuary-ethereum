...
abstract contract EIP712 {
    ...
    function _domainSeparatorV4() internal view returns (bytes32) {
        if (address(this) == _CACHED_THIS && 4 == _CACHED_CHAIN_ID) {
    ...
    function _buildDomainSeparator(
        ...
        return keccak256(abi.encode(typeHash, nameHash, versionHash, 4, address(this)));
    }
...
contract testGameLogic is  Ownable, IERC721Receiver, rarityCheck, Initializable, Pausable {
    ...
    // owedExp * 100
    uint[] yetiMultiplier = [100, 120, 150]; // new introduced
    uint[] levelCost = [0, 48, 210, 512, 980, 2100, 3430, 4760, 6379, 8313];//new
    uint[] levelExp = [0, 50, 200, 450, 800, 1600, 2450, 3200, 4050, 5000];//new
    ...
    function init(address _yeti, address _frxst, address _treasury, address _rs,address _vrf, address _designatedSigner)
        virtual
        public
        onlyOwner 
    {
    ...
    //------------------------------
    function newFightingHonour(uint level) internal returns(uint){
        uint FIGHTING_STOLEN_RISK_PERCENTAGE = 100;
        FIGHTING_STOLEN_RISK_PERCENTAGE -= 5*(level-1);
        return FIGHTING_STOLEN_RISK_PERCENTAGE;
    }

    function newHuntingInjuryRisk(uint level) internal returns(uint){
        uint HUNTING_INJURY_RISK_PERCENTAGE = 500;
        HUNTING_INJURY_RISK_PERCENTAGE -= 25*(level-1);
        return HUNTING_INJURY_RISK_PERCENTAGE;
    }

    function newGatheringTax(uint level) internal returns(uint){
        uint GATHERING_TAX_RISK_PERCENTAGE = 50;
        GATHERING_TAX_RISK_PERCENTAGE -= 2*(level-1);
        return GATHERING_TAX_RISK_PERCENTAGE;
    }
    //--------------------------------
    ...
    function levelup(uint tokenId) external whenNotPaused { //whenNotPaused
        ...
        console.log(levels[tokenId]);
        console.log(experience[tokenId]);
        experience[tokenId] -= levelExp[levels[tokenId]]; // [9]
        levels[tokenId] += 1;
    }
...
//    }
    ...
    function heal_cost(uint tokenId) internal returns (uint) {
        return 2*150* yetiMultiplier[tokenRarity[tokenId]] ; // new imp
    }
    ...
    function _claimYetiFromPalace(uint tokenId, bool unstake) internal returns (uint owedFrxst) {
        ...
        if (unstake) {
            //todo change this to 1 days in main-net 10 => 1 days
            uint mod = (block.timestamp - stake.value) / 5 minutes;
            owedFrxst =  (hourly * mod); // changed
            //todo change to 10 to 1 days 10 => 1 days
            uint owedExp = ((block.timestamp - stake.stakeTime)/5 minutes)* exprates[c][stake.activityId];
            if (stake.activityId == 0) {
                ...
                if (rand % 100 < newGatheringTax(levels[tokenId]))
                {
                    uint amountToPay = newGatheringTax(levels[tokenId]); // 45%
...
//                if (rand % 1000 > 0) {
                    console.log("OwedExp Expected",owedExp);
                    owedExp = owedExp/2;
                    console.log("OwedExp Received",owedExp);
                    if (experience[tokenId] < levelExp[levels[tokenId]]) {
                        if(owedExp > levelExp[levels[tokenId]]) {
                            owedExp = levelExp[levels[tokenId]] - experience[tokenId];
                        }
    ...
    function _claimYetiFromFighting(uint tokenId, bool unstake) internal returns (uint owedFrxst) {
        ...
        if (unstake) {
            ...
            uint owedExp = ((block.timestamp - stake.stakeTime)/5 minutes)* exprates[tokenRarity[tokenId]][stake.activityId];
            initGame storage init = randomIndex[tokenId];
            uint rand = RandomnessEngine.initiateRandomness(tokenId,init.randomValue);
            require (rand > 0,"Randomness not received");
            ...
            if (rand % 1000 < newFightingHonour(levels[tokenId])) { //changes made here
                init.randomValue = RandomnessEngine.getCurrentIndex();
                console.log("In Losing Honour");
            ...
            delete fighterIndices[tokenId]; // Delete old mappin
