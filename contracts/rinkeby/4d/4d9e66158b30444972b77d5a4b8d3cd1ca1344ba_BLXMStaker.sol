...
contract BLXMRewardProvider is ReentrancyGuardUpgradeable, BLXMMultiOwnable, BLXMTreasuryManager, IBLXMRewardProvider {
    ...
    function hoursToSession(uint32 hourFromEpoch) public override view returns (uint16 session, uint amountPerHours) {
        uint32 initialHour = treasuryFields.initialHour;
        session = treasuryFields.lastSession;
        if (session == 0 || hourFromEpoch < initialHour) {
            return (0, 0);
        }

        uint currentHour = hourFromEpoch - treasuryFields.initialHour;
    ...
    function _arrangeFailedRewards(uint rewardAmount) internal {
        ...
        (uint16 session,) = hoursToSession(currentHour);
        if (session == 0) {
    ...
    function _updateStatistics(uint32 fromHour, uint32 toHour) internal {
        ...
        while (fromHour < toHour) {
            ...
            (,uint rewards) = hoursToSession(fromHour);
...
interface IBLXMRewardProvider {
    ...
    function hoursToSession(uint32 hourFromEpoch) external view returns (uint16 session, uint amountPerHours);
