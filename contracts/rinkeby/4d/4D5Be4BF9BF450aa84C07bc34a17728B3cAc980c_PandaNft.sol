...
contract PandaNft is ERC721, Ownable {
    ...
    // presale whitelist merkle tree root
    ...
    // presale mint
    function presaleMint(address account, bytes32[] memory proof) public {
        ...
        if (balanceOf(account) >= maxPerAccount) {
            revert PandaNft__MaxPerAccountReached();
        }

        if (!inWhitelist(proof, whitelistRoot, account)) {
