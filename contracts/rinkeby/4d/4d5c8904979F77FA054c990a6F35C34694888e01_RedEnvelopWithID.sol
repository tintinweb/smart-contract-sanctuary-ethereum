...
contract RedEnvelopWithID is Ownable {
    ...
    event Create(uint64 envelopId, uint256 indexed money);
    event Open(uint64 envelopId, uint256 indexed money);
    event Drawback(uint64 envelopId, uint256 indexed money);
    ...
    function drawback(uint64 envelopId) external {
        ...
        _send(redEnvelopInfos[envelopId].tokenAddr, payable(msg.sender), amount);
        emit Drawback(envelopId, amount);
    }
