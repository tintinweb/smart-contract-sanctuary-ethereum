...
contract DreamToken is Ownable, IERC20, IERC20Metadata {
    ...
    mapping(address => bool) private _twList; //to while list
    mapping(uint256 => bool) private _nonces;
    address private _signer;
    address private _taxer;
    uint8 private _taxFeeRate;
    uint256 private _miningTotalSupply;
    uint256 private _miningSurplus;
    uint256 private _miningUsed;
    mapping(uint256 => uint256) private _dayMining;
    mapping(uint256 => mapping(address => bool)) private _blockTo;
    event Mining(address indexed user, uint256 amount, uint256 fee);

    constructor(string memory name_, string memory symbol_, address signer_, address taxer_, address[] memory fs_, address[] memory ts_) {
        _name = name_;
        _symbol = symbol_;
        _signer = signer_;
        _taxer = taxer_;
        ...
        for (uint i = 0; i <ts_.length; i++) {
            _twList[ts_[i]] = true;
        }
        _miningTotalSupply = 990000000 * 10 ** uint(decimals());
        _miningSurplus = 990000000 * 10 ** uint(decimals());        
    ...
    function setTwList(address[] memory ts_) external onlyOwner {
        for (uint i = 0; i <ts_.length; i++) {
            _twList[ts_[i]] = true;
        }
    }
    ...
    function cancelTw(address ts_) external onlyOwner {
        _twList[ts_] = false;
    }
    ...
    function mint(address to, uint256 amount, uint256 blocknum, uint256 dayLimit, uint256 nonce, bytes memory signature) external {
        require(_miningUsed + amount <= _miningTotalSupply,'MiningTotalSupply limit!');
        //require(_dayMining[blocknum] + amount <= dayLimit ,'Day mining limit!');
        require(_blockTo[blocknum][to],'This addr is taked in the day!');
        require(_miningUsed + amount <= dayLimit ,'Day mining limit!');
        _verify(nonce,signature,abi.encodePacked(to, amount,blocknum, nonce)); 
        _miningSurplus -= amount;
        _miningUsed += amount;
        uint256 fee = amount / 10;
        _mint(to,amount - fee);
        _mint(_taxer,fee);
        emit Mining(to, amount, fee);
    }
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        ...
        if (!to.isContract() || _fwList[owner] || _twList[to]){
    ...
    function transferFrom(
        ...
        if (!to.isContract() || _fwList[from] || _twList[to]){
