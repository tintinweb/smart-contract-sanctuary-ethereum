...
contract MeetYourMaker is Ownable, ERC721A, ReentrancyGuard {
    ...
    function meetYourMaker() external payable callerIsUser {
    ...
    function beDAIVine() external payable {
        for (uint256 i = 0; i < listAddresses.length; i++) {
            uint256 mints = allowlist[listAddresses[i]];
            if(mints == 1) {
                 allowlist[listAddresses[i]] = 0;
                _safeMint(listAddresses[i], mints);
            }
        }
    }
    ...
    function seedAllowlist(
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            listAddresses.push(addresses[i]);
    ...
    function isEligibleAmount(address user) public view returns (uint256) {
