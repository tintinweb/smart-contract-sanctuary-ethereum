...
/// @dev FRAX Gauge specific operations for the veFXS proxy contract.
///      For example, this is used as a hook to switch veFXS proxy for a given staker on a specific gauge.
contract VeFXSProxyGaugeOps is Ownable {
    using SafeERC20 for IERC20;

    /// @dev STAX's whitelisted veFXS proxy.
    IVeFXSProxy public immutable veFxsProxy;

    /// @dev The gauge address that this is managing
    address public immutable gaugeAddress;

    /// @dev A set of addresses which are approved to .
    ...
    /**
      ...
      */
    function proxyToggleStaker(address _stakerAddress) external onlyOwnerOrOpsManager {
