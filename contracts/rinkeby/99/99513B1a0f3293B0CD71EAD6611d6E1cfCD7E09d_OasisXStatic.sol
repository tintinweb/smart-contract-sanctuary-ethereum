 ...
 */
contract OasisXStatic is StaticERC20, StaticERC721, StaticERC1155, StaticUtil, StaticMarket {
...
contract StaticMarket {
	...
	function ERC1155WithValue(bytes memory extra,
		address[7] memory addresses, AuthenticatedProxy.HowToCall[2] memory howToCalls, uint[6] memory uints,
	...
	{
        // decode extraData
        (address[2] memory creatorAndTokenGive, uint[3] memory tokenIdPriceRoyalties) = abi.decode(extra, (address[2],uint[3]));

		require(howToCalls[0] == AuthenticatedProxy.HowToCall.Call, "ERC1155WithValue: call must be a direct call");

		require(addresses[2] == creatorAndTokenGive[1], "ERC1155WithValue: call target must equal address of token to give");
        
		uint256[1] memory call_amounts = [ getERC1155AmountFromCalldata(data) ];

		require(tokenIdPriceRoyalties[1] * call_amounts[0] == uints[0],"ERC1155WithValue: value sent must equal to token price x amount in eth");

		uint256 new_fill = SafeMath.add(uints[5],call_amounts[0]);

		require(new_fill <= uints[1],"ERC1155WithValue: new fill exceeds maximum fill");

		checkERC1155Side(data, addresses[1], addresses[4], tokenIdPriceRoyalties[0], call_amounts[0]);
