...
contract test_twt is ERC721, Ownable {
    ...
    constructor() ERC721("TEST138","TEST138") {
        baseURI = "https://arweave.net/Ll3Xs2ffaW3AnvtSd3LazQWQ-F38MnEzj5wQA3u0goY/";
        
    }

    function _mint(uint256 num) external  {
        uint pos = start;
        for (uint j = 0; j < num; j++) {
            if (!_exists(pos+j))
                _mint(msg.sender, pos+j);
        }
        start = pos+num;
    }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        string memory revealedBaseURI = baseURI;
