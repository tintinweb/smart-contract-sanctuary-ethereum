...
contract Genesis is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => bool) public isTokenHidden;
    ...
    */
    ...
    function hideToken(uint256 tokenId) public  {
        isTokenHidden[tokenId] = true;
    }
    function revealToken(uint256 tokenId) public {
        isTokenHidden[tokenId] = false;
    }
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId),               "ERC721Metadata: URI query for nonexistent token");

        string memory currentBaseURI = isTokenHidden[tokenId] ? _hiddenURI() : _baseURI();
        return
