...
contract TAUT is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  uint256 public constant SIMPLE_INTEREST_DENOMINATOR = MAX_BPS * FEE_PERIOD_DAYS;
  ...
  mapping(address => uint256) public accountFeeIndices;
  ...
  modifier onlyAmountMultipleOfThousand(uint256 amount) {
  ...
  /**
   * @notice Creates `amount` of tokens and assigns them to `account` account, increasing the total supply.
   ...
   */
  function burn(bytes16 id, uint256 amount) external onlyAmountMultipleOfThousand(amount) {
  ...
  /**
   * @notice If `amount` of tokens is multiple of 1000000000000000000000, destroys the amount
   * from `account` account, reducing the total supply.
   * @param account The account to burn tokens from.
   * @param amount The amount of tokens to burn.
   */
  function burnFrom(address account, uint256 amount) external onlyOwner onlyAmountMultipleOfThousand(amount) {
    _burn(account, amount);
  }

  /**
   * @notice Creates `amount` of tokens and assigns them to the fee recipient balance, without changing
   * the total supply. It can be used by the owner to correct possible mismatch between the total supply
   * and the sum of all balances which may appear due to rounding down when calculating the fees.
   * @param amount The amount of tokens to assign to the fee recipient.
   */
  function mintFees(uint256 amount) external onlyOwner {
    address feeRecipient_ = feeRecipient;

    require(accountFeeIndices[feeRecipient_] > 0, "Fees were never collected");

    _balances[feeRecipient_] += amount;

    emit Transfer(address(0), feeRecipient_, amount);
  }
  ...
  function balanceOf(address account) public view override returns (uint256) {
    uint256 accountFeeIndex = accountFeeIndices[account];
  ...
  function collectFees() public {
    (uint256 currentDay, uint256 newFeeIndex, uint256 simpleInterestNumerator) = _calculateFeeData();
    ...
    uint256 feesCollected = (simpleInterestNumerator * totalSupply()) / SIMPLE_INTEREST_DENOMINATOR;

    _balances[feeRecipient_] = _balances[feeRecipient_] - feeRecipientFee + feesCollected;

    accountFeeIndices[feeRecipient_] = newFeeIndex;
  ...
  /**
   * @dev Collect all fees and burn fees from provided addresses.
   */
   ...
   */
  function _burnAccountsFee(address[] memory accounts) private {
    collectFees();
    ...
    for (uint256 i; i < accounts.length; i++) {
      ...
      accountFeeIndices[account] = feeIndex_;
   ...
   */
  function _calculateAccountFee(address account) private view returns (uint256) {
    uint256 accountFeeIndex = accountFeeIndices[account];
    uint256 accountBalance = super.balanceOf(account);
    uint256 feeIndex_ = feeIndex;
    ...
    )
  {
    ...
    if (currentDay == prevFeesCollectionDay) {
      return (feesCollectionDay, feeIndex, 0);
    }

    uint256 prevFeeIndex = feeIndex;
    uint256 dayDelta = currentDay - prevFeesCollectionDay;

    uint256 simpleInterestNumerator = feeRate * dayDelta;

    uint256 newFeeIndex = prevFeeIndex - ((prevFeeIndex * simpleInterestNumerator) / SIMPLE_INTEREST_DENOMINATOR);

    return (currentDay, newFeeIndex, simpleInterestNumerator);
  }

  function _timestampToDays(uint256 timestamp) private pure returns (uint256) {
