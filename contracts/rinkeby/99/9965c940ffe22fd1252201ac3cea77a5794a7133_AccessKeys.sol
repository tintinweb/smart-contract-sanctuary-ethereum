...
contract AccessKeys is ERC1155Supply, Ownable
{
    bool public saleIsActive = false;
    bool public presaleIsActive = false;
    uint public activeBadgeId = 1;
    // uint public maxPerTransaction = 6;
    uint public maxPerWallet = 3;
    uint public maxPerPresaleWallet = 3;
    uint public maxSupply = 888;
    uint public constant NUMBER_RESERVED_TOKENS = 44;
    uint256 public constant PRICE = 0.5 ether; // 0.5
    uint256 public constant PRESALE_PRICE = 0.15 ether; // 0.15
    ...
    constructor() ERC1155("https://ipfs.io/ipfs/QmXpXSLe4vhuZPovhVoPvgCf9Yu6fBdZo3PiGkM2NGW3ep/1.json") {
        whitelist[address(0xA4e28b2211d17f6e810BD5f32fC46e14670D7F78)] = true;
        whitelist[address(0x344f29c8333513615E921A8f8B66FBa8e50c5289)] = true;
    }
    ...
    function mintTokenPresale(uint256 amount) external payable
    {
        ...
        // require(amount > 0 && amount <= maxPerTransaction, "Max per transaction reached, sale not allowed");
        require(balanceOf(msg.sender, activeBadgeId) + amount <= maxPerPresaleWallet, "Limit per wallet reached with this amount, sale not allowed");
        require(totalSupply(activeBadgeId) + amount <= maxSupply - (NUMBER_RESERVED_TOKENS - reservedTokensMinted), "Purchase would exceed max supply");
        require(msg.value >= PRESALE_PRICE * amount, "Not enough ETH for transaction");
    ...
    //, uint _maxPerTransaction
    function changeSaleDetails(uint _activeBadgeId, uint _maxPerWallet, uint _maxSupply) external onlyOwner
    {
