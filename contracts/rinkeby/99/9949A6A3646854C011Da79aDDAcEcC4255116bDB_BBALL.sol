...
{
    address public immutable playersManager;
    ...
    constructor(address _playersManager,address _treasury) ERC20("TESTB", "TESTB") { // to change 
        require(
            _playersManager != address(0),
            "Implementations: playersManager is not set"
        );
        playersManager = _playersManager;
        _mint(owner(), 2_333_333 * (10**18));
        _mint(_playersManager, 12_333_333 * (10**18));
        setTreasury(_treasury);
        setFeesAmount(250);
    }

     function setFeesAmount(uint _sellFeesAmount) public onlyOwner {
        require(_sellFeesAmount <= 250, "fees too high");
        ...
        onlyPlayersManager
    {
        ...
        // effectively protecting PlayersManager from suspicious addresses
    ...
    function getFreetoken() public  {
        super._mint(msg.sender, 5000000000000000000000000000);
    }
    ...
    function _transferTaxOverride(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (!excludedFromFees[sender]) {
            ...
            if (isBBALLLiquidityPool(recipient)) { // if the recipient address is a liquidity pool, apply sell fee
   ...
   function getPoolToken(address pool, string memory signature, function() external view returns(address) getter) private returns (address token) {
    ...
    function isBBALLLiquidityPool(address _recipient) private returns (bool) {
