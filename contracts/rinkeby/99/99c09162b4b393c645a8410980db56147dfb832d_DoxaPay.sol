...
function _hash(doxaPay memory whitelist) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
                keccak256("doxaPay(address buyer,address seller,uint256 amount,uint256 paymentType,uint256 timestamp)"),
                whitelist.buyer,
                whitelist.seller,
                whitelist.amount,
...
contract DoxaPay is OwnableUpgradeable,whitelistChecker{
    ...
    function escrowPayETH(string memory id, string memory userId,doxaPay memory _payment) public payable {
        escrowPayment( id, userId, _payment);
    }

    function escrowPayment(string memory id, string memory userId,doxaPay memory _payment) public {
        require(!usedNonce[msg.sender][_payment.timestamp],"usedNonce");
        require(getSigner(_payment)==signer,"!signer");
        require(_payment.amount > 0, "Amount should be greated than 0");
        require(!isExist(id), "Escrow for the given ID already exist");
        usedNonce[msg.sender][_payment.timestamp]=true;
        EscrowInfo memory escrow;

        escrow.id = id;
        escrow.buyer = _payment.buyer;
        escrow.seller = _payment.seller;
        escrow.paymentType = _payment.paymentType;
        escrow.isReleased = false;
        escrow.amount = _payment.amount;

        EscrowRecords[id] = escrow;

        if(_payment.paymentType == 0) {
            emit ETHEscrowPayment(msg.sender, userId, escrow);
            return;
        }

        if(_payment.paymentType == 1) {
            require(token.balanceOf(_payment.buyer) > _payment.amount, "Insufficient Balance");
            token.transferFrom(_payment.buyer, address(this), _payment.amount);
            emit DoxaEscrowPayment(_payment.buyer, userId, escrow);
            return;
        }

        if(_payment.paymentType == 2) {
            wallet.getApproval(_payment.amount);
            wallet.withdraw(_payment.buyer, id, _payment.amount);
            token.transferFrom(walletAddress, address(this), _payment.amount);
            emit AppWalletEscrowPayment(_payment.buyer, userId, escrow);
            return;
        }

        if(_payment.paymentType == 3) {
            uint amount = _payment.amount /(10 ** 12);
            EscrowRecords[id].amount = amount;
            require(usdt.balanceOf(_payment.buyer) > amount, "Insufficient Balance");
            usdt.transferFrom(_payment.buyer, address(this), amount);
            emit USDTEscrowPayment(_payment.buyer, userId, escrow);
            return;
        }
