...
contract PiggyBreaker is VRFConsumerBaseV2, IERC721Receiver {
    ...
    function sendETHReward(uint16 piggyId, uint256 randomWord) private {
      uint256 rewardPercent = rarityPrizes[piggies[piggyId].rarity][normalizer(randomWord % 100 + 1)] / (remainingPiggies * 1000);
      if (rewardPercent > 1) {
        rewardPercent = 1;
      }

      // Send ETH to holder
      payable(piggies[piggyId].holder).transfer(address(this).balance * rewardPercent);
    }
