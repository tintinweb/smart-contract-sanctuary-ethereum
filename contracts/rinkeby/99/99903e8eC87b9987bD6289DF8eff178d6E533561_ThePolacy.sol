...
contract ThePolacy is Ownable, ERC721A("ThePolacy", "TPL") {
    ...
    function isMintingPhaseWL() public view returns(bool) {
        return block.timestamp < deploymentTime + DURATION_WHITELIST && _totalMinted() < SUPPLY_TEAM + SUPPLY_WHITELIST;
    }

    function isMintingPhaseFreePublic() public view returns(bool) {
        return !isMintingPhaseWL() && block.timestamp < deploymentTime + DURATION_WHITELIST + DURATION_PUBLIC_FREE_MINT && _totalMinted() < SUPPLY_TEAM + SUPPLY_WHITELIST + SUPPLY_PUBLIC_FREE_MINT;
    }
