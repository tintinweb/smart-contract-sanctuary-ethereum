...
contract SolarSystem is ERC721Tradable {
    ...
    function mean_anomaly(uint planet) public view returns (uint) {
        ...
        uint E    = eccentric_anomaly_from_mean_anomaly(M,e) * 360 * 1000 / TWO_PI;
        uint O    = T*SECONDS*((180 + deg + E)*100/360)/100;
        return O;
    }

     function mean_anomaly_test(uint planet) public view returns (uint) {
        uint e    = uint(VALUES[planet]["ecc"]);
        uint T    = uint(VALUES[planet]["per"]);
        uint deg  = uint(VALUES[planet]["deg"]);
        int  t0   = VALUES[planet]["start"];
    ...
    function baseTokenURI() override public pure returns (string memory) {
        return "https://yazo.fun/";
    }

    function tokenURI(uint256 _tokenId) override public view returns (string memory) {
         return formatTokenURI(getTOKEN(_tokenId)); 
    }
        ...
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            '{"name":"',"Solar System"'", "image":"',imageURI,'", "animation_url":"',imageURI,'"}'
                        )
                    )
                )
            )
   ...
   function contractURI() public pure returns (string memory) {
    ...
    function getTOKEN(uint256 _tokenId) public view returns (string memory) {
       ...
       for (uint i=_tokenId-1; i<FIND_REPLACE.length; i++) {
          string memory offset = Strings.toString(mean_anomaly(i));
          for (uint it=0; it < OCCURS[i]; it++) {
    ...
    function premine(uint256 token) external onlyOwner {
