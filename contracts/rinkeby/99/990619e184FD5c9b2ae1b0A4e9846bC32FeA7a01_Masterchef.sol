...
// TopDog is the master of Alpha. He can make Alpha and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once ALPHA is sufficiently
...
contract Masterchef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accAlphaPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accAlphaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accAlphaPerShare; // Accumulated ALPHAs per share, times 1e12. See below.
    }

    // The Alpha TOKEN!
    AlphaToken public alpha;
    // The Alpha Token Locker contract
    AlphaLocker public alphaLocker;
    // Dev address.
    address public devAlphaDistributor;

    address public tAlphaAlphaDistributor;
    address public xEdanAlphaDistributor;
    address public xOmegaAlphaDistributor;

    uint256 public devPercent;
    uint256 public tAlphaPercent;
    uint256 public xEdanPercent;
    uint256 public xOmegaPercent;

    // Block number when bonus ALPHA period ends.
    uint256 public bonusEndBlock;
    // ALPHA tokens created per block.
    uint256 public alphaPerBlock;
    // Bonus muliplier for early alpha makers.
    ...
    // The block number when ALPHA mining starts.
    ...
    constructor(
        ...
        address _xEdanAlphaDistributor,
        address _xOmegaAlphaDistributor,
        uint256 _alphaPerBlock,
        uint256 _startBlock,
        ...
        alphaPerBlock = _alphaPerBlock;
        bonusEndBlock = _bonusEndBlock;
        startBlock = _startBlock;

        tAlphaAlphaDistributor = _tAlphaAlphaDistributor;
        xEdanAlphaDistributor = _xEdanAlphaDistributor;
    ...
    function updateRewardPerBlock(uint256 _perBlock) public onlyOwner {
        massUpdatePools();
        alphaPerBlock = _perBlock;
        emit RewardPerBlock(msg.sender, _perBlock);
    }

    // Update the given pool's ALPHA allocation point. Can only be called by the owner.
    ...
    function pendingAlpha(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accAlphaPerShare = pool.accAlphaPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 alphaReward = multiplier.mul(alphaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accAlphaPerShare = accAlphaPerShare.add(alphaReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accAlphaPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 devAlphaReward = alphaReward.mul(devPercent).div(100); // devPercent rewards to dev address
        alpha.mint(devAlphaDistributor, devAlphaReward.mul(devRewardMintPercent).div(100)); // partial devPercent rewards to dev address

        alpha.mint(address(alphaLocker), devAlphaReward.sub(devAlphaReward.mul(devRewardMintPercent).div(100))); // rest devPercent rewards locked to alpha token contract
        alphaLocker.lock(devAlphaDistributor, devAlphaReward.sub(devAlphaReward.mul(devRewardMintPercent).div(100)), true);

        alpha.mint(tAlphaAlphaDistributor, alphaReward.mul(tAlphaPercent).div(100)); // tAlphaPercent rewards to tAlphaAlphaDistributor address
        alpha.mint(xEdanAlphaDistributor, alphaReward.mul(xEdanPercent).div(100)); // xEdanPercent rewards to xEdanAlphaDistributor address
        alpha.mint(xOmegaAlphaDistributor, alphaReward.mul(xOmegaPercent).div(100)); // xOmegaPercent rewards to xOmegaAlphaDistributor address
        alpha.mint(address(this), alphaReward);

        pool.accAlphaPerShare = pool.accAlphaPerShare.add(alphaReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to TopDog for ALPHA allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accAlphaPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                // safeAlphaTransfer(msg.sender, pending);
                uint256 sendAmount = pending.mul(rewardMintPercent).div(100);
                safeAlphaTransfer(msg.sender, sendAmount);
                safeAlphaTransfer(address(alphaLocker), pending.sub(sendAmount)); // Rest amount sent to Alpha token contract
        ...
        user.rewardDebt = user.amount.mul(pool.accAlphaPerShare).div(1e12);
    ...
    // Safe alpha transfer function, just in case if rounding error causes pool to not have enough ALPHAs.
    function safeAlphaTransfer(address _to, uint256 _amount) internal {
        uint256 alphaBal = alpha.balanceOf(address(this));
        if (_amount > alphaBal) {
            alpha.transfer(_to, alphaBal);
        } else {
    ...
    // Update dev alpha distributor address by the owner.
    function devAlphaDistributorUpdate(address _devAlphaDistributor) public onlyOwner {
        devAlphaDistributor = _devAlphaDistributor;
    }
    ...
    // Call emergency withdraw to transfer alpha tokens to any other address, onlyOwner function
    function callEmergencyWithdraw(address _to) public onlyOwner{
        alphaLocker.emergencyWithdrawOwner(_to);
    }

    // Update tAlphaAlphaDistributor address by the owner.
    function tAlphaAlphaDistributorUpdate(address _tAlphaAlphaDistributor) public onlyOwner {
        tAlphaAlphaDistributor = _tAlphaAlphaDistributor;
        emit SetAddress("tAlpha-AlphaDistributor", msg.sender, _tAlphaAlphaDistributor);
    }

    // Update xEdanAlphaDistributor address by the owner.
    function xEdanAlphaDistributorUpdate(address _xEdanAlphaDistributor) public onlyOwner {
        xEdanAlphaDistributor = _xEdanAlphaDistributor;
        emit SetAddress("xEdan-AlphaDistributor", msg.sender, _xEdanAlphaDistributor);
    }
    ...
    function xOmegaAlphaDistributorUpdate(address _xOmegaAlphaDistributor) public onlyOwner {
        xOmegaAlphaDistributor = _xOmegaAlphaDistributor;
        emit SetAddress("xOmega-AlphaDistributor", msg.sender, _xOmegaAlphaDistributor);
    }

    // Update devPercent by the owner.
    function devPercentUpdate(uint _devPercent) public onlyOwner {
        require(_devPercent <= 10, "masterChef: Percent too high");
    ...
    function tAlphaPercentUpdate(uint _tAlphaPercent) public onlyOwner {
        ...
        emit SetPercent("tAlpha share", msg.sender, _tAlphaPercent);
    }

    // Update xEdanPercent by the owner.
    function xEdanPercentUpdate(uint _xEdanPercent) public onlyOwner {
        xEdanPercent = _xEdanPercent;
        emit SetPercent("xEdan share", msg.sender, _xEdanPercent);
    }
    ...
    function xOmegaPercentUpdate(uint _xOmegaPercent) public onlyOwner {
        ...
        emit SetPercent("xOmega share", msg.sender, _xOmegaPercent);
    }
}
...
// Alpha with Governance.
contract AlphaToken is ERC20("Alpha", "ALPHA"), Ownable {
    using SafeMath for uint256;
    ...
    */
        ...
        external
    {
        ...
        require(block.timestamp <= expiry, "Alpha::delegateBySig: signature expired");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying Alpha (not scaled);
        ...
        internal
    {
        ...
        if (nCheckpoints > 0 && checkpoints[delegatee][nCheckpoints - 1].fromBlock == blockNumber) {
            ...
            numCheckpoints[delegatee] = safe32(nCheckpoints + 1, "Alpha::_writeCheckpoint: new checkpoint exceeds 32 bits");
        }
