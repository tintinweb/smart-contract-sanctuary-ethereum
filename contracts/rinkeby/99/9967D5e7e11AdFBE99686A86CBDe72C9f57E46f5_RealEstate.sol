...
contract RealEstate is Pausable, Ownable {
    ...
    **************************************************/
    ...
    function getPropertiesDetals(uint256 _propertyId) public view returns(
    ...
    **************************************************/
    ...
    function buyTicketToken (
        ...
        uint256 _propertyId, address buyeAddress, string memory fullName, string memory phoneNumber,
        uint _USDT
        
        ) public {

        require(TICKET_TOKEN.ownerOf(_propertyId) == address(this));
        require(PROPERTY_TOKEN.ownerOf(_propertyId) == address(this));
        require(isOnSell[_propertyId]);

        buyerInfo[_propertyId] = _buyerInfo(
            buyeAddress, fullName, phoneNumber
        );

        uint _msgValue = _USDT;
        require(TicketTokenPrice == _msgValue);

        USDT.transferFrom(_msgSender(), agency(), _msgValue.mul(80).div(100));
        USDT.transferFrom(_msgSender(), properties[_propertyId].currentOwner, _msgValue.mul(20).div(100));
