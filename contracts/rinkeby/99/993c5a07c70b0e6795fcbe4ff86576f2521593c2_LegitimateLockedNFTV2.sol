...
contract LegitimateLockedNFTV2 is ERC721, ERC721Enumerable, Ownable {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override(ERC721) returns (string memory) {
        ...
        string memory baseTokenUri = bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, _tokenId.toString())) : "";
        if (tokenLock[_tokenId]) {
    ...
    function setTokenLock(uint256 _tokenId, bool _locked) public onlyOwner {
