...
contract PaceFacet is ChildMintableERC20 {
    ...
    /**
     *  @dev _msgSender override. This is required by ChildMintableERC20 
     *  @return sender - The account which a call is received from.
     */
     ...
     */
    function addExcludedAccount(address account) public {
     ...
     *  @param account - The account to exclude
     */
    function removeExcludedAccount(address account) public {
     ...
     * Requirements:
     *
     * - `spender` cannot be the zero addresstate.
     */
    function increaseAllowance(address spender, uint256 addedValue) public override virtual returns (bool) {
        address owner = _msgSender();
        _approve(owner, spender, state.allowances[owner][spender] + addedValue);
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public override virtual returns (bool) {
        address owner = _msgSender();
        uint256 currentAllowance = state.allowances[owner][spender];
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        unchecked {
            state.balances[from] = fromBalance - amountAfterTax;
        }

        state.balances[to] += amountAfterTax;

        emit Transfer(from, to, amountAfterTax);
    }

    /**
     *  @dev _mint() override. Mints tokens using diamond storage pointer.
     ...
     * total supply.
     *
     * Emits a {Transfer} event with `to` set to the zero addresstate.
     *
     * Requirements:
     *
     * - `account` cannot be the zero addresstate.
     * - `account` must have at least `amount` tokenstate.
     */
    function _burn(address account, uint256 amount) internal virtual override {
        require(account != address(0), "ERC20: burn from the zero address");
        uint256 accountBalance = state.balances[account];

        require(accountBalance >= amount, "ERC20: burn amount exceeds balance");

        unchecked {
            state.balances[account] = accountBalance - amount;
        }
    ...
    ContextMixin
{
    ...
    function _msgSender() internal override virtual view returns (address sender) {
