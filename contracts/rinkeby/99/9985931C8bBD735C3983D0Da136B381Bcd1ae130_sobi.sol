...
//       name.pop();
//     }
//     //the length of the string array cannot be called. it is executable in uint but not in string so this is tnhe simple way of retruning all string array 
//     function length()public view returns(uint){
//         return name.length;
//     }
...
//     }
//     address payable aa=payable(0x3d33C01bCC36ac6A8f872599A9c9351c11Ef07E7);
//     function send(uint am)public payable{
//           stakingToken.transfer(aa,am);
//     }
...
//The interface could only be used for particular address of any other contract when the same function which is in the interface is over rided in the contract so when we save the address of the contract and call the interface functions it actually calls the contracts overrides functions.
// interface IERC20 {
//   /**
...
//     function set(address a,address b,uint amount)public {
...
//kecchak256 is the hashing algorithm; there are several hashing algorith out there but here we shall discuss keccak256
// contract sobi{
...
//  mapping(uint => mapping(string => whiteListing)) public student;
//  mapping(whiteListing => address) public student1;
 mapping(uint => whiteListing) public student2;


function set()public{
  student2[1]=whiteListing(21,"usman");
  student2[2]=whiteListing(22,"sobi");
  student2[3]=whiteListing({age:23,name:"ubaid"});
}

function get()public view returns(uint){
    return student2[2].age;
}
function getName()public view returns(string memory ){
    return student2[2].name;
}
}
