...
contract ERC20 is Context, IERC20 {
    ...
    /* ERC20 constants */
    string public constant name = "EQUITY";
    string public constant symbol = "EQUITY";
    uint8 public constant decimals = 8;

    // allocating 10 tokens for promotion, airdrop, liquidity and dev share
    uint256 private _totalSupply = 0 * (10 ** uint256(decimals));
...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    event NewCarryOverPercent(uint indexed newCarryOverPercent);

    /* Flush address */
    address payable public FLUSH_ADDR;
    uint8 internal LAST_FLUSHED_DAY = 0;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_DIV = 10 ** uint256(decimals); // 1e8

    /* Time of contract launch (ex: 1603843200 -> 2020-28-10T00:00:00Z) */
    uint256 public LAUNCH_TIME;

    /* Start of claim phase */
    uint256 internal constant PRE_CLAIM_DAYS = 1;
    /* reduce amount of tokens to 2500000 */
    uint256 internal constant CLAIM_STARTING_AMOUNT = 2500000 * (10 ** uint256(decimals));
    /* reduce amount of tokens to 1000000 */
    uint256 internal constant CLAIM_LOWEST_AMOUNT = 1000000 * (10 ** uint256(decimals));
    ...
    uint256 internal constant MAX_STAKE_DAYS = 180; // 180 days Max Stake
    ...
    uint8 public carryOverPercent = 30; // range: 0 - 100
    ...
    modifier isDAO(address _account) {
        require(dao == _account, "Restricted Access! only DAO can access.");
        _;
    }

    constructor(address _flushAddr, address tokenAddress) public {
        dao =  msg.sender;
        payableToken = tokenAddress;
        FLUSH_ADDR = address(uint160(_flushAddr));
    }

    function launch() external isDAO(msg.sender) {
        require(isLaunched == false, 'EQUITY: ALREADY LAUNCHED');
        LAUNCH_TIME = block.timestamp  / 1  days * 1  days;
        isLaunched = true;
    }

    /**
     * @dev Internal function to receive tokens from the user
     * @param _amount token amount that is to be received from user
     */
    function receiveERC20Token(uint256 _amount) internal {
        IERC20 token = IERC20(payableToken);
        
        // transfers tokens that belong to your user to this contract
        token.transferFrom(msg.sender, address(this), _amount);
    }

    function setDAO(address _daoAddress) external isDAO(msg.sender) { // for the very first call DAO will be admin
        dao = _daoAddress;
    }

    function setCarryOverPercent(uint8 _percent) external isDAO(msg.sender) {
        require(_percent >= 0, "setCarryOverPercent: Limit must equal or greater than 0.");
        require(_percent <= 100, "setCarryOverPercent: Limit must equal or less than 100.");
        carryOverPercent = _percent;

        emit NewCarryOverPercent(_percent);
    }

    function incrementDay() external {
        LAUNCH_TIME = LAUNCH_TIME - 1 days;
    }
        ...
        external
    {
        ...
        require(g._currentDay >= CLAIM_PHASE_START_DAY, "EQUITY: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "EQUITY: beforeDay cannot be in the future");
    ...
    function dailyDataRange(uint256 beginDay, uint256 endDay)
        external
        view
        returns (uint256[] memory _dayStakeSharesTotal, uint256[] memory _dayPayoutTotal, uint256[] memory _dayDividends, uint256[] memory _dayCarryOver)
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "EQUITY: range invalid");
        ...
        _dayCarryOver = new uint256[](endDay - beginDay);
        ...
        do {
            ...
            _dayCarryOver[dst] = dailyData[src].dayCarryOver;
            dst++;
        } while (++src < endDay);

        return (_dayStakeSharesTotal, _dayPayoutTotal, _dayDividends, _dayCarryOver);
    }
        ...
        returns (uint256)
    {
        if (block.timestamp < LAUNCH_TIME){
            ...
            return (block.timestamp - LAUNCH_TIME) / 1 days + 1; // to start day index at 1
        }
    }
    ...
    function _stakeLoad(StakeStore storage stRef, StakeCache memory st)
        internal
        view
    {
        ...
        private
    {
        ...
        uint256 balance = xfLobby[day];
        if (0 < day) {
            balance += dailyData[day-1].dayCarryOver;
        }
        dailyData[day].dayCarryOver = balance.mul(carryOverPercent).div(100);
        dailyData[day].dayDividends = balance.sub(dailyData[day].dayCarryOver);
    }
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "EQUITY: newStakedDays lower than minimum");
        ...
        /* Move the staked tokens to the contract */
        _transfer(msg.sender, address(this), newStakedSuns);
     ...
     */
    function stakeEnd(uint40 stakeIdParam)
        external
    {
        GlobalsCache memory g;
        GlobalsCache memory gSnapshot;
        ...
        require(stakeListRef.length != 0, "EQUITY: Empty stake list");

        int256 stakeIndex = -1;
        for (uint8 i = 0; i < stakeListRef.length; i++) {
            if (stakeListRef[i].stakeId == stakeIdParam) {
                stakeIndex = i;
                break;
            }
        }
        require(0 <= stakeIndex, "EQUITY: stakeIdParam invalid");

        /* Get stake copy */
        StakeCache memory st;
        _stakeLoad(stakeListRef[uint256(stakeIndex)], st);
        ...
        if (g._currentDay >= st._lockedDay) {
            ...
            if (0 < dividends) {
                IERC20 token = IERC20(payableToken);        
                // transfers USDT from contract to the specified address
        ...
        if (stakeReturn != 0) {
            _transfer(address(this), msg.sender, st._stakedSuns);
            uint256 reward = stakeReturn - st._stakedSuns;
            if (0 < reward) {
        ...
        _stakeRemove(stakeListRef, uint256(stakeIndex));
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "EQUITY: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "EQUITY: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M EQUITY of stake.
...
contract TransformableToken is StakeableToken {
     ...
     */
    function xfLobbyEnter(address referrerAddr, uint256 value)
        external
    {
        receiveERC20Token(value);
        _xfLobbyEnter(msg.sender, referrerAddr, value);
    }

    function xfLobbyEnterEx(address memberAddr, address referrerAddr, uint256 value)
        external
    {
        receiveERC20Token(value);
        _xfLobbyEnter(memberAddr, referrerAddr, value);
    }

    // This function will be called in EquitusCommunity
    function _xfLobbyEnter(address memberAddr, address referrerAddr, uint256 value)
        internal
    {
        uint256 enterDay = _currentDay();
	    require(enterDay >= CLAIM_PHASE_START_DAY, "EQUITY: Auction has not begun yet");

        uint256 rawAmount = value;
        require(rawAmount != 0, "EQUITY: Amount required");

        XfLobbyQueueStore storage qRef = xfLobbyMembers[enterDay][memberAddr];
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "EQUITY: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "EQUITY: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "EQUITY: count invalid");
        }
        ...
        do {
            ...
            if (referrerAddr == address(0) || referrerAddr == memberAddr) {
                /* No referrer or Self-referred */
                _emitXfLobbyExit(enterDay, headIndex, memberAddr, xfAmount, referrerAddr);
            } else {
                /* Referrer bonus of 10% to referrer */
     ...
     * @param count Number of queued-enters to exit (optional; 0 for all)
     */
    function xfLobbyExitPreview(uint256 enterDay, uint256 count)
        external view returns (uint256 equityToReceive, uint256 equityParBnB)
    {
        require(enterDay < _currentDay(), "EQUITY: Round is not complete");
        ...
        //     _mint(msg.sender, totalXfAmount);
        // }
        return (totalXfAmount, waasLobby / _xfLobby);
    }
        ...
        external
    {
        ...
        require(IERC20(payableToken).balanceOf(address(this)) != 0, "EQUITY: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "EQUITY: Invalid day");

        _dailyDataUpdateAuto(g);

        IERC20(payableToken).transfer(FLUSH_ADDR, (dailyData[LAST_FLUSHED_DAY].dayDividends * 5) / 100);
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "EQUITY: Param invalid");
...
contract EQUITY is TransformableToken {
    constructor(address _flushAddr, address payableToken) GlobalsAndUtility(_flushAddr, payableToken) public
    {
