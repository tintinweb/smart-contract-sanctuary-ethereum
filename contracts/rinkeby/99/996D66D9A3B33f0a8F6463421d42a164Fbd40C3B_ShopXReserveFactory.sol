 ...
 */
contract ShopXReserveFactory is AccessControl {
  ...
  */
  ...
  function setShopxFee (uint256 _shopxFee) onlyRole(SHOPX_ADMIN)  external {
    shopxFee = _shopxFee;
  }

  function setShopxAddress (address _shopxAddress) onlyRole(SHOPX_ADMIN)  external{
    shopxAddress = _shopxAddress;
    emit ShopxAddressUpdate(_shopxAddress);
  }

  function setShopxAdmins (address[] memory _shopxAdmins) onlyRole(SHOPX_ADMIN)  external{
    delete shopxAdmins;
    for (uint i = 0; i < _shopxAdmins.length; i++) {
      shopxAdmins.push(_shopxAdmins[i]);
      _setupRole(SHOPX_ADMIN, _shopxAdmins[i]);
