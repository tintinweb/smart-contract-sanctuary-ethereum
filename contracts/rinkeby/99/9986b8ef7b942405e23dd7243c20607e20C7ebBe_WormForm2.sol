...
import "./utils/upgradeable/StringsUpgradeable.sol";
import "./utils/upgradeable/ReentrancyGuardUpgradeable.sol";

contract WormForm2 is ERC721Upgradeable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    //     stakingTime4 = 60 * 60 * 24 * 24;
    //     stakingTime3 = 60 * 60 * 24 * 36;
    //     stakingTime2 = 60 * 60 * 24 * 48;
    //     index = 1001;
    ...
    function setIdsAsLimited(uint256[] memory _limitedIds) external virtual onlyOwner {
        ...
        for (i; i < _limitedIds.length; i++) {
            ...
            emit IdWasSetAsLimited(_limitedIds[i]);
        }
     ...
     */
    function purchaseTicket() external payable virtual whenNotPaused nonReentrant{
        require(!revealed, "Ticket purchasing is only available before tokens were revealed.");
        require(ids.length > 0, "All tokens were minted.");
        require(
            balanceOf(_msgSender()) + 
            ERC721Upgradeable(oldNFTContract).balanceOf(_msgSender()) +
            ticketsDealt[_msgSender()] < 2,
            "You can only get 2 NFTs."
        );
        require(msg.value >= price, "Not enought ETH provided to pay for mint.");

        ticketsDealt[_msgSender()]++;
    ...
    /**
     * @dev Function to start staking. Available only to owner.
     */
    function startStaking() external virtual onlyOwner {
        require(!stakingStarted, "Staking is already started.");
        stakingStarted = true;
    }
    ...
    /**
     * @dev Function to set new URI of the real metadata.
     * @param _uri New URI string.
     */
     ...
     */
    function stakeToBreed(uint256 _ATokenId, uint256 _BTokenId) external virtual whenNotPaused {
        ...
        emit UserStaked(_msgSender(), _ATokenId, _ATokenId, limitedTokenStaked);
    }
    ...
    function hatchCocoon(uint256 _tokenId, string memory _uri, bytes32 _hash, bytes memory _signature) external virtual whenNotPaused {
        require(_tokenId >= 1001, "IDs lower that 1001 are not cocoons.");
