...
contract Wormhole is Ownable {
    ...
    function _getEventReward(uint64 id) internal view returns (uint64) {
        ...
        require(_getIsEventUnclaimed(id, msg.sender), "already claimed");

        uint256[] memory eventBabies = ownerBabiesByEvent[msg.sender][id];
        uint256[] memory eventGenesis = ownerGenesisByEvent[msg.sender][id];
        (uint256 babiesBaseRewardProbability, uint256 babiesAdditionalRewardProbability) = _eventRewardProbabilities(
            id,
            false
        );
        (uint256 genesisBaseRewardProbability, uint256 genesisAdditionalRewardProbability) = _eventRewardProbabilities(
            id,
            true
        );
        ...
        for (uint256 index = 0; index < eventBabies.length; index++) {
            uint256 babyId = eventBabies[index];
            if (_ownerOfToken(babyId, babiesAddress) != msg.sender) {
                continue;
            }
            if (_rand(salvageEvent.rngSeed, 10000 + babyId, id, 1) < babiesBaseRewardProbability) {
                rewardCount++;
            }
            if (_rand(salvageEvent.rngSeed, 10000 + babyId, id, 2) < babiesAdditionalRewardProbability) {
                rewardCount++;
            }
        }

        for (uint256 index = 0; index < eventGenesis.length; index++) {
            uint256 genesisId = eventGenesis[index];
            if (_ownerOfToken(genesisId, babiesAddress) != msg.sender) continue;

            if (_rand(salvageEvent.rngSeed, genesisId, id, 1) < genesisBaseRewardProbability) {
                rewardCount++;
            }
            if (_rand(salvageEvent.rngSeed, genesisId, id, 2) < genesisAdditionalRewardProbability) {
                rewardCount++;
            }
        }
        return rewardCount;
