...
contract CuteLittleLamp is ERC721Tradable, ReentrancyGuard {
    ...
    uint256 private constant _numTier = 5;
    uint256 private _reservedMinted;
    mapping (address => uint256) public totalMints;
    address private _signer = 0xC2646c3a9476A1b265782C96CE4af035db6291Ae;  
    ...
    ) ERC721Tradable("Little Lamp", "CLL", _proxyRegistryAddress) {
    ...
    modifier isSaleOpen {
        require(totalSupply() < maxSupply, "Sorry, All CuteLittleLamp are sold.");
        _;
    }
    ...
    function toggleWhitelistPause() external onlyOwner {
    ...
    function mintNFT(
        ...
        require( whitelistPaused == false, "Whitelist NFT Sale is not active at the moment.");
    ...
    function mintNFT(uint256 numTokens) external payable nonReentrant isSaleOpen {
        require( salePaused == false, "Public NFT Sale is not active at the moment.");
        require( totalSupply() + numTokens <= maxSupply, "Exceeds maximum NFT supply.");
        require( numTokens > 0 && numTokens <= 10, "You must mint between 1 to 10 tokens.");
        require( totalMints[msg.sender] + numTokens <= maxAllowedMints, "Address exceeds max allowed mints.");
        require( currentPrice().mul(numTokens) == msg.value, "Incorrect ether value sent. Please check again.");
    ...
    function currentPrice() public view returns (uint256) {
        ...
        uint256 tier1 = maxSupply / _numTier;
        uint256 tier2 = tier1*2;
        uint256 tier3 = tier1*3;
        uint256 tier4 = tier1*4;
    ...
    function mintReservedNFT(address[] calldata recipients, uint256 numTokens) external onlyOwner isSaleOpen {
        ...
        require( _reservedMinted + recipients.length * numTokens <= reservedSupply, "Exceeds maximum reserved supply.");
