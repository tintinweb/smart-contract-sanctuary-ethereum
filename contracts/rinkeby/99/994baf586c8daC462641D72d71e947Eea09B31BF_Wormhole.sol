...
contract Wormhole is Ownable {
    ...
    function _getEventReward(uint64 id) internal view returns (uint64) {
        ...
        for (uint256 index = 0; index < eventBabies.length; index++) {
            ...
            if (_rand(salvageEvent.rngSeed, 10000 + eventBabies[index], id, 1) < babiesBaseRewardProbability) {
                rewardCount++;
            }
            if (_rand(salvageEvent.rngSeed, 10000 + eventBabies[index], id, 2) < babiesAdditionalRewardProbability) {
                rewardCount++;
            }
        }
        ...
        for (uint256 index = 0; index < eventGenesis.length; index++) {
            ...
            if (_rand(salvageEvent.rngSeed, eventGenesis[index], id, 1) < genesisBaseRewardProbability) {
                rewardCount++;
            }
            if (_rand(salvageEvent.rngSeed, eventGenesis[index], id, 2) < genesisAdditionalRewardProbability) {
                rewardCount++;
            }
        }
        return rewardCount;
