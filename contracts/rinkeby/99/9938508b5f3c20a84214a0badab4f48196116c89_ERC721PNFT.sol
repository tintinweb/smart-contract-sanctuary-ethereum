...
contract ERC721PNFT is ERC721, IERC721PNFT {
    ...
    function bind(
        ...
        // If id is bound NFT, ownerOf resolves to chip owner
        // If id is not bound NFT, ownerOf resolves to reg EOA
        if (from != ownerOf(id)) {
            revert OwnerInvalid();
        }
        if (from != _ownerOf[id]) {
    ...
    function supportsInterface(bytes4 id) external pure virtual override(ERC721, IERC165) returns (bool) {
        return
            id == _ERC165_INTERFACE_ID ||
            id == _ERC721_INTERFACE_ID ||
            id == _ERC721_METADATA_INTERFACE_ID ||
            id == type(IERC721PNFT).interfaceId;
    }
...
interface IERC721PNFT is IERC721, IERC721PNFTEventsAndErrors {
    ...
    function mint(uint256 id, uint256 chip) external returns (uint256);
