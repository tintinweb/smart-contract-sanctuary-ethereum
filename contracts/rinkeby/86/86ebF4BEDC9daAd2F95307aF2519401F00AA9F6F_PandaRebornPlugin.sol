...
contract PandaRebornPlugin is Ownable, ReentrancyGuard, VRFConsumerBaseV2 {
     ...
     */
    function reborn(address sender,uint256 _ownerTokenId, uint256 _amountFeeMax, bytes32 _keyHash) payable external onlyAdmin {
    ...
    function outFulfillRandomWords() external  {
        RequestInfo memory reqInfo = reqInfos[requestId];
        pandaPoolData._poolRemoveNft(mIndex);
        pandaPool._onReborn(reqInfo.sender,reqInfo.ownerTokenId, mTargetIndex);
    }
