...
contract CryptoFrenzCollection is ERC721, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;

    // using counters instead of ERC721Enumerable for gas consumption optimization
    using Counters for Counters.Counter;
    Counters.Counter private _totalSupply; 

    uint256 public mintPrice = 30000000000000000; //0.03 ETH CHECK BEFORE DEPLOYING TO MAINNET
   ...
   function recoverSigner(bytes memory signature) public view returns (address) {
    ...
    function setWhitelistKey(address newKey) public onlyOwner {
        whitelistKey = newKey;
    }
   ...
   */
    ...
    */
    function freezeMetadata() public onlyOwner returns (bool) {
        require(_metadataIsFreezed == false, "You can't unfreeze metadata");
        return _metadataIsFreezed = !_metadataIsFreezed;
    }

    /**
    * Will be used to freeze contract sale state to lock token supply
    */
    function freezeSaleState() public onlyOwner returns (bool) {
        require(_saleStateIsFreezed == false, "You can't unfreeze sale state");
        return _saleStateIsFreezed = !_saleStateIsFreezed;
    }

    /**
    * Set Whitelist state to active if paused, and stops it if active
    */
    function flipWhitelistState() public onlyOwner returns (bool) {
        return whitelistIsActive = !whitelistIsActive;
    }

    /**
    * Mints CryptoFrensCollection Cards for whitelisted addresses
    */
    function mintFromWhitelist(uint256 numberOfTokens, bytes memory signature) public payable {
        require(whitelistIsActive, "Whitelist is not active");
        require(numberOfTokens <= 3, "Can only mint 3 tokens at a time");
        require(recoverSigner(signature) == whitelistKey, "Address is not whitelisted");
        require(!signatureUsed[signature], "Signature has already been used.");
        require(_totalSupply.current() + numberOfTokens <= 1420, "Purchase would exceed max tokens");
    ...
    /**
    * Mints CryptoFrensCollection Cards
    */
    function mintCard(uint256 numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint cards");
        require(numberOfTokens <= 5, "Can only mint 5 tokens at a time");
        require(_totalSupply.current() + numberOfTokens <= 1420,
        "Purchase would exceed max supply of Cards"
        );
        require(mintPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
    ...
    */
    function mintReservedCardTo(address to, uint256 numberOfTokens) public onlyOwner {
        if (to == address(0)) {
            to = msg.sender;
        }
    ...
    */
    function updateSalePrice(uint256 newValue) public onlyOwner {
        mintPrice = newValue;
    }
}