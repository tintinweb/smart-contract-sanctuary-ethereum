...
contract ScalesStaking is IScales, ERC20, Ownable, Pausable, ReentrancyGuard {
  IERC721 public immutable KAIJU;

  uint256 constant public BASE_RATE = 5 ether;
  uint256 constant public MAX_PER_TX = 25;
  uint256 constant public TOTAL_GENESIS = 3333;
  ...
  modifier onlyAllowed() {
    require(allowed[_msgSender()], "Caller not allowed");
    _;
  }
   ...
   */
  function getClaimable(address account) public view override returns (uint256) {
  ...
  /**
   * @notice Claim available $SCALES
   */
  function claim() public whenNotPaused nonReentrant {
    require(getClaimable(_msgSender()) > 0, "Nothing to claim");

    _mint(_msgSender(), getClaimable(_msgSender()));
  ...
  function stake(uint256[] calldata tokenIds) external whenNotPaused nonReentrant {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      require(current >= TOTAL_GENESIS, "Only babies can be staked");
  ...
  /**
   * @notice Set permissions for utility contracts
   ...
   */
  function spend(address from, uint256 amount) public override onlyAllowed {
    require(amount >= getClaimable(from), "Insufficient funds");
   ...
   */
  function credit(address to, uint256 amount) public override onlyAllowed {
    stash[to] += amount;
  }
