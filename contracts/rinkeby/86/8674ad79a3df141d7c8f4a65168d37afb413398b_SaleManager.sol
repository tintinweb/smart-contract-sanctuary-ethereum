...
// File: contracts/LidyERC721_SaleType4.sol
...
contract LidyERC721_SaleType4 is ERC721, ERC721Enumerable, Pausable, Ownable, ERC2981 {
    ...
    function isContractFull() public view returns(bool){
    	if(MINT_CAP > _tokenIdCounter.current()){
	    return false;
        }
    	else{
    	    return true;
    ...
    function safeMint(string memory tokenURI_) public onlyMinter whenNotPaused{
	...
	require(tokenId < MINT_CAP, "THE CONTRACT IS FULL");
        ...
        MetadataCIDofNFTs[tokenId] = tokenURI_;
    }
...
contract Sale is ReentrancyGuard{
    ...
    address public lidyversetest = 0x0c4F4De5550697bF86B16E5bA1C83D2A72F87D35;
...
/*
    ...
    function placeBid(bytes32[] calldata proof) payable external nonReentrant returns(bool){
    ...
    function getRefund() external nonReentrant returns(bool){
    ...
    function withdrawFunds() external nonReentrant returns(bool){
        require(getSaleState() == SaleState.ENDED || getSaleState() == SaleState.DIRECT_BUY);
        require(msg.sender == creator);
        (address receiver_, uint256 royaltyAmount) = LidyERC721_SaleType4(nftAddress).royaltyInfo(tokenId,maxBid);
        ...
        payable(lidyversetest).transfer((maxBid * lidyverseShare) / 100);
    ...
    function cancelSale() external nonReentrant returns(bool){
        require(msg.sender == creator);
        require(getSaleState() == SaleState.OPEN);
        require(maxBid == 0);
        isCancelled = true;
...
// File: contracts/salemanagertest.sol
...
contract SaleManager is ReentrancyGuard, Ownable {
    ...
    function createListing(uint _duration, uint _minIncrement, uint _directBuyPrice,uint _startPrice,address _nftAddress,uint _tokenId,uint _saleType, bool whitelistSale)  external nonReentrant returns (bool) {
