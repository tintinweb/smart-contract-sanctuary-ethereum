...
contract Seapad is VRFConsumerBaseV2, ChainlinkClient {
    ...
    function purchasePack(
        ...
        uint256 paymentMethod,
        uint256 slippage
    ) public payable returns(uint256 packId) {
        ...
        //     pricePerPackInChoosenPaymentMethodAfterSavedFeeRemoved
        // );
        // uint256 tokensSpentFromOriginalBalanceToGetChainlinkToken = Payments
        //     .buyLinkToken(
        ...
        // (
        //     uint256 amountSentToOwner,
        //     address ownerPaymentMethod
        // ) = splitRemainingFunds(
        //         pb.projectID,
        //         _paymentMethod,
        //         feeAmountRemainingForSeapadFromOriginalPackPrice,
        ...
        returns (uint256)
    {
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    "Top up required: reserved link balance required: ",
        ...
        override
    {
        ...
        (uint96 bal, , , ) = COORDINATOR.getSubscription(pb.subscriptionID);
        projectP2.LINK_TO_REDEEM += bal;
        COORDINATOR.cancelSubscription(pb.subscriptionID, address(this));
        emit OpenedPack(
            pbr.projectID,
            pbr.packSoldID,
            pb.wonItems,
    ...
    function topUp(
        ...
        Projects storage project = projects[projectId];
        PacksBought storage pb = project.packsBought[packSoldID];
        require(pb.wonItems.length == 0, "1"); //Cannot top up because subscription is cancelled already.
        pb.reservedLINK += amount;
    }

    function withrawLink(uint256 projectID) public {
        isValidCaller(projectID);
        ProjectsP2 storage projectp2 = projectsP2[projectID];
        LinkTokenInterface(chainlinkTokenAddress()).transfer(
            msg.sender,
            projectp2.LINK_TO_REDEEM
        );
        projectp2.LINK_TO_REDEEM = 0;
    }
