...
contract Artlab is ERC721A, Ownable {    
    ...
    //address private amSignerAddress;
    ...
    constructor( 
        ...
        //address _amSignerAddress,
        address payable _wallet
    ) ERC721A("Testlab2", "TLAB") {        
        setUriPrefix(_uriPrefix);
        setSignerAddresses(_ehSignerAddress);
        setWallet(_wallet);
        setCost(0.05 ether);
        console.log("Artlab contract deploying completed");
    }
    ...
    function setSignerAddresses(address _ehSignerAddress) public onlyOwner {
        ...
        //amSignerAddress = _amSignerAddress;
    }
    ...
    function whitelistMint(uint256 _mintAmount, uint _maxfree, uint256 _id, bytes memory _sig) public payable {
        ...
        console.log("minting:",_mintAmount, _id);        
        ...
        if(_priceMultiplier > 0) {
            ...
            console.log("amount, price multiplier", _mintAmount, priceMultiplier);
    ...
    /*
    function whitelistMint(uint256 _mintAmount, bool _ehrole, uint256 _id, bytes memory _sig) public payable {
        uint256 expMinted = alreadyExpMinted(msg.sender);
        address signerAddress = ehSignerAddress;
        console.log("minting:",_mintAmount, _id);
        if(!_ehrole) {
            signerAddress = amSignerAddress;
        }
        require(isSaleActive, "Sale is not active");     
        require(_mintAmount <= maxTxMintAmount, "Exceeds maximum tokens you can purchase in a single transaction!");
        require(_mintAmount > 0 && totalSupply() + _mintAmount <= maxSupply, "Invalid mint amount!");
        require(expMinted + _mintAmount <=  maxMintAmount, "Max mint amount exceeded!");
        require(keccak256(abi.encodePacked(msg.sender, _id))
                .toEthSignedMessageHash()
                .recover(_sig) == signerAddress, "Signature is not valid!");        
        if(!_ehrole || _mintAmount > 1 || expMinted > 0) {
            uint256 priceMultiplier = _mintAmount;
            if(_ehrole && expMinted == 0) {
                priceMultiplier--;
            }
            console.log("amount, price multiplier", _mintAmount, priceMultiplier);
            //require(msg.value >= cost * priceMultiplier, "Ether value sent is not correct!");
    ...
    */
    function ownerMint(address _to, uint256 _mintAmount) public onlyOwner {                                    
        console.log("call ownerMint:", this.name());
        require(_mintAmount > 0 && totalSupply() + _mintAmount <= maxSupply, "Invalid mint amount!");
        _safeMint(_to, _mintAmount);
        console.log("call _safeMint with params:", _mintAmount);
    }
}
