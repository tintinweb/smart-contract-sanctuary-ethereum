...
contract NftTest is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public maxTotalMintWhitelist = 10;
    uint256 public maxMintPerWalletWhite = 5;
    uint256 public maxMintPerWalletPublic = 3;    
    uint256 public mintPricePublic = 0.02 ether;
    ...
    function WhiteMint(address minterWallet) public payable { 
        require(whiteSaleActive,"Whitelist mint disabled");     
        require(isWhiteListed[minterWallet], "You need to be whitelisted");             
        require(msg.value >= mintPriceWhitelist, "insuficient fonds");
        require(balanceOf(msg.sender) < maxMintPerWalletWhite, "Max mint per wallet reached");
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId < maxSupply, "cant mint monre, limit reached");
     ...
     function PublicMint() public payable{
        require(publicSaleActive,"Public mint disabled");             
        require(msg.value >= mintPricePublic, "insuficient fonds");        
        require(balanceOf(msg.sender) < maxMintPerWalletPublic, "Max mint per wallet reached");
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId < maxSupply, "cant mint monre, limit reached");
        _tokenIdCounter.increment();
        _safeMint(msg.sender, _tokenIdCounter.current());
    ...
    function addAddressTowhitelistAddress(address[] memory _addresses) external onlyOwner {
    ...
    function setWhiteSaleActive(bool isActive_) external onlyOwner {
        whiteSaleActive = isActive_;
        emit SetWhiteSaleActive(isActive_, block.timestamp);
    }

    function setPublicSaleActive(bool isActive_) external onlyOwner {
