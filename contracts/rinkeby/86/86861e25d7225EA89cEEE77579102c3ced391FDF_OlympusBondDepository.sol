...
contract OlympusBondDepository is Ownable {
    ...
    struct Bond {
        ...
        uint vesting; // timedelta left to vest
    ...
    struct Adjust {
        ...
        uint rate; // increment
        uint target; // BCV when adjustment INISHED
        uint buffer; // minimum length (in ms) between adjustments
        uint lastTime; // last time when last adjustment made
    }
    ...
    function redeem( address _recipient, bool _stake ) external returns ( uint ) {        
        ...
        uint percentVested = percentVestedFor( _recipient ); // (timedelta since last interaction / vesting term remaining)
    ...
    function adjust() internal {
        uint256 timestampCanAdjust = adjustment.lastTime.add( adjustment.buffer );
    ...
    function percentVestedFor( address _depositor ) public view returns ( uint percentVested_ ) {
        ...
        uint timestampSinceLast = block.timestamp.sub( bond.lastTime );
