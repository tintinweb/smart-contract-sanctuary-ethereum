...
contract EventUnlockSchedule is BaseGovernanceWithUserUpgradable, KeeperCompatibleInterface {
    ...
    function initialize(ILock lock, bytes calldata data) public initializer {
        ...
        __EventUnlockSchedule_init(lock, _conditions, _offsetPrices, _amounts, /*_tknAddrBase, _tknAddrQuote,*/ _feedAddress, governanceAddress);
    }

    function __EventUnlockSchedule_init(ILock _lock, uint256[] memory _conditions, int256[] memory _offsetPrices, uint256[] memory _amounts, /*address _tknAddrBase, address _tknAddrQuote,*/ address _feedAddress, address governanceAddress) internal onlyInitializing {
        __BaseGovernanceWithUser_init(governanceAddress);
        __EventUnlockSchedule_init_unchained(_lock, _conditions, _offsetPrices, _amounts, /*_tknAddrBase, _tknAddrQuote,*/ _feedAddress);
    }

    function __EventUnlockSchedule_init_unchained(ILock _lock, uint256[] memory _conditions, int256[] memory _offsetPrices, uint256[] memory _amounts, /*address _tknAddrBase, address _tknAddrQuote,*/ address _feedAddress) internal onlyInitializing {
    ...
    function lockEnd() external view returns(uint) {
        return lockStartTime + 2 * 365 days;
    }
    ...
    function checkUpkeep(bytes calldata /*checkData*/) external override view returns (bool upkeepNeeded, bytes memory performData) {
        ...
        if(offsetPrices.length > 0){
            int price = _getPrice();
            for(uint i = 0; i < offsetPrices.length; i++) {
                upkeepNeeded = false;
                if(conditions[i] == 0){
    ...
    function whatFeedAddress() public view returns (address) {
        return feedAddr;
    }
    function getFeedSource() public view returns (AggregatorV3Interface) {
        return priceFeed;
    }
