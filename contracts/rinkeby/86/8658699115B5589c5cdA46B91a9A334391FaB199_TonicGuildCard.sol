...
contract TonicGuildCard is Initializable,ERC721Upgradeable,ERC721RoyaltyUpgradeable,ERC721EnumerableUpgradeable,PausableUpgradeable,OwnableUpgradeable {
    ...
    function adminMint(address to) public onlyOwner {
        require(totalSupply() + 1 <= maxSupply, "Purchase would exceed max allowed tokens");
        _mintAndIncrement(to);
    }

    function adminMintBulk(address to, uint8 quantity) public onlyOwner {
        require(totalSupply() + quantity <= maxSupply, "Purchase would exceed max allowed tokens");
    ...
    function allowListMint(uint8 quantity) external payable {
        ...
        require(quantity <= _allowList[msg.sender], "Exceeded quanity allowed to purchase");
        require(totalSupply() + quantity <= maxSupply, "Purchase would exceed maximum token supply");
    ...
    function quantityAllowed(address buyer) public view returns (uint8) {
