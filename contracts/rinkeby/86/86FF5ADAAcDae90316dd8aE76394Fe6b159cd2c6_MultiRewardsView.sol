...
contract MultiRewardsView {
    ...
    constructor(address _liquidityMiningManager, address _escrowPool) {
        liquidityMiningManager = MultiRewardsLiquidityMiningManager(
            _liquidityMiningManager
        );
        ...
        returns (Data memory result)
    {
        ...
        MultiRewardsLiquidityMiningManager.Pool[]
            memory pools = liquidityMiningManager.getPools();

        result.pools = new Pool[](pools.length);

        for (uint256 i = 0; i < pools.length; i++) {
            MultiRewardsTimeLockPool poolContract = MultiRewardsTimeLockPool(
            ...
            MultiRewardsTimeLockPool.Deposit[] memory deposits = poolContract
        ...
        TimeLockPool.Deposit[] memory deposits = escrowPool.getDepositsOf(
            _account
        );
...
abstract contract MultiRewardsBasePool is
    ERC20Votes,
    ...
    ReentrancyGuard
{
    ...
    constructor(
        ...
        ERC20Permit(_name)
        ERC20(_name, _symbol)
        AbstractMultiRewards(balanceOf, totalSupply)
    {
        require(
            _depositToken != address(0),
            "MultiRewardsBasePool.constructor: Deposit token must be set"
        );
        require(
            _rewardTokens.length == _escrowPools.length,
            "MultiRewardsBasePool.constructor: reward tokens and escrow pools length mismatch"
        );
        require(
            _rewardTokens.length == _escrowPortions.length,
            "MultiRewardsBasePool.constructor: reward tokens and escrow portions length mismatch"
        );
        require(
            _rewardTokens.length == _escrowDurations.length,
            "MultiRewardsBasePool.constructor: reward tokens and escrow durations length mismatch"
        );
        ...
        for (uint256 i = 0; i < _rewardTokens.length; i++) {
            ...
            require(
                ...
                "MultiRewardsBasePool.constructor: reward token cannot be zero address"
            );
            ...
            require(
                escrowPortion <= 1e18,
                "MultiRewardsBasePool.constructor: Cannot escrow more than 100%"
            );
    ...
    modifier onlyAdmin() {
        require(
            ...
            "MultiRewardsBasePool: only admin"
        );
        _;
    }
    ...
    function addRewardToken(
        ...
        require(
            _reward != address(0),
            "MultiRewardsBasePool.addRewardToken: reward token cannot be zero address"
        );
        require(
            _escrowPortion <= 1e18,
            "MultiRewardsBasePool.addRewardToken: Cannot escrow more than 100%"
        );
    ...
    function updateRewardToken(
        ...
        require(
            rewardTokensList[_reward],
            "MultiRewardsBasePool.updateRewardToken: reward token not in the list"
        );
        require(
            _reward != address(0),
            "MultiRewardsBasePool.updateRewardToken: reward token cannot be zero address"
        );
        require(
            _escrowPortion <= 1e18,
            "MultiRewardsBasePool.updateRewardToken: Cannot escrow more than 100%"
        );
    ...
    function distributeRewards(address _reward, uint256 _amount)
        external
        override
        nonReentrant
    {
    ...
    function claimRewards(address _reward, address _receiver) public {
        ...
        uint256 escrowedRewardAmount = (rewardAmount *
            escrowPortions[_reward]) / 1e18;
        uint256 nonEscrowedRewardAmount = rewardAmount - escrowedRewardAmount;
...
abstract contract AbstractMultiRewards is IAbstractMultiRewards {
     ...
     * @return A uint256 representing the rewards `account` can withdraw
     */
    function withdrawableRewardsOf(address _reward, address _account)
        public
        view
        override
        returns (uint256)
    {
        return
            cumulativeRewardsOf(_reward, _account) -
            withdrawnRewards[_reward][_account];
    }
     ...
     * @return The amount of rewards that `account` has withdrawn.
     */
    function withdrawnRewardsOf(address _reward, address _account)
        public
        view
        override
        returns (uint256)
     ...
     * @return The amount of rewards that `account` has earned in total.
     */
    function cumulativeRewardsOf(address _reward, address _account)
        public
        view
        override
        returns (uint256)
    {
        return
            ((pointsPerShare[_reward] * getSharesOf(_account)).toInt256() +
                pointsCorrection[_reward][_account]).toUint256() /
            POINTS_MULTIPLIER;
    }
    ...
    function _distributeRewards(address _reward, uint256 _amount) internal {
        require(
            _reward != address(0),
            "AbstractRewards._distributeRewards: reward cannot be zero address"
        );
     ...
     * greater than 0.
     */
    function _prepareCollect(address _reward, address _account)
        internal
        returns (uint256)
    {
        require(
            _reward != address(0),
            "AbstractRewards._prepareCollect: reward cannot be zero address"
        );
        require(
            _account != address(0),
            "AbstractRewards._prepareCollect: account cannot be zero address"
        ...
        uint256 _withdrawableDividend = withdrawableRewardsOf(
            _reward,
            _account
        );
        if (_withdrawableDividend > 0) {
    ...
    function _correctPointsForTransfer(
        ...
        require(
            _reward != address(0),
            "AbstractRewards._correctPointsForTransfer: reward address cannot be zero address"
        );
        require(
            _from != address(0),
            "AbstractRewards._correctPointsForTransfer: from address cannot be zero address"
        );
        require(
            _to != address(0),
            "AbstractRewards._correctPointsForTransfer: to address cannot be zero address"
        );
        require(
            _shares != 0,
            "AbstractRewards._correctPointsForTransfer: shares cannot be zero"
        ...
        pointsCorrection[_reward][_from] =
            pointsCorrection[_reward][_from] +
            _magCorrection;
        pointsCorrection[_reward][_to] =
            pointsCorrection[_reward][_to] -
            _magCorrection;

        emit PointsCorrectionUpdated(
            _reward,
            _from,
            pointsCorrection[_reward][_from]
        );
    ...
    function _correctPoints(
        ...
        require(
            _reward != address(0),
            "AbstractRewards._correctPoints: reward cannot be zero address"
        );
        require(
            _account != address(0),
            "AbstractRewards._correctPoints: account cannot be zero address"
        ...
        pointsCorrection[_reward][_account] =
            pointsCorrection[_reward][_account] +
            (_shares * (pointsPerShare[_reward].toInt256()));
        emit PointsCorrectionUpdated(
            _reward,
            _account,
            pointsCorrection[_reward][_account]
...
interface IAbstractMultiRewards {
     ...
     * @return A uint256 representing the rewards `account` can withdraw
     */
    function withdrawableRewardsOf(address reward, address account)
        external
        view
        returns (uint256);
     ...
     * @return The amount of funds that `account` has withdrawn.
     */
    function withdrawnRewardsOf(address reward, address account)
        external
        view
        returns (uint256);
     ...
     * @return The amount of funds that `account` has earned in total.
     */
    function cumulativeRewardsOf(address reward, address account)
        external
        view
        returns (uint256);
