...
contract ALIEN is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public resMintMaxSupply = 2000;
    uint256 public foundersReserveSize = 1800;
    uint256 public maxFightersPerWallet = 3;
    uint256 public maxFoundersPerWallet = 6;
    uint256 public maxFightersPerTx = 3;

    bool public reservationMintPaused = true;
    bool public foundersMintPaused = true;
    ...
    // ===== Constructor =====
    constructor() ERC721A(" Test Contract ", "TES") {}
    ...
    function resMint(address _address, bytes32[] memory proof) external payable onlySender nonReentrant {
        ...
        require(resMintMaxSupply >= amount, "Reservation mint is sold out");

        resMintMaxSupply = resMintMaxSupply - amount;
    ...
    // ===== Founders Mint ===== //
    function foundersMint(address _address, bytes32[] memory proof) external payable onlySender nonReentrant {
        ...
        founderslistMintedAmount[_address] += amount;
    ...
    function _getMintAmount(uint256 value) internal view returns (uint256) {
        ...
        require(
            (totalSupply() + amount) <= MAX_SUPPLY - foundersReserveSize,
            "Sold out!"
        );
        return amount;
    }
    ...
    // ===== Setter (owner only) =====
    function setReservedSize(uint256 _foundersReserveSize) external onlyOwner {
        foundersReserveSize = _foundersReserveSize;
    }
    ...
    function setReservationMintMaxSupply(uint256 _resMintMaxSupply)
        external
        onlyOwner
    {
        resMintMaxSupply = _resMintMaxSupply;
    }
