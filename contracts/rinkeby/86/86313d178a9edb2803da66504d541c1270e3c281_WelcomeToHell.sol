...
contract WelcomeToHell is ERC721A, Ownable {
    using Strings for uint256;

    uint256 public constant TOTAL_MAX_SUPPLY = 650;

    uint256 public  MAX_PUBLIC_PER_TX = 2;
    uint256 public  MAX_PUBLIC_MINT_PER_WALLET = 4;

    uint256 public token_price = 0.0066 ether;
    bool public publicSaleActive;

    string private _baseTokenURI;

    constructor() ERC721A("WelcomeToHell", "WTH") {
    }
    ...
    function withdrawFunds() external onlyOwner {
        uint256 artist = address(this).balance * 75 / 100;
        uint256 developer = address(this).balance * 25 / 100;

        (bool artSuccess, ) = payable(0x4B2022c549A56D5f3c0522190E3E821092c7B529).call{value: artist}('');
        require(artSuccess);

        (bool devSuccess, ) = payable(0xe720a7D54F5eE7F525c8BAEdBBC0C7c6D6D67aB6).call{value: developer}('');
        require(devSuccess);
    }
