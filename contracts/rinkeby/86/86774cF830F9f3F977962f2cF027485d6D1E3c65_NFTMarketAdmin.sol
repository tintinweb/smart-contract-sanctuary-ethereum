...
contract NFTMarketAdmin is ReentrancyGuard, AccessControl, ERC721Holder {
    ...
    function listTradeItem(
        ...
        uint256 tokenId,
        uint256 price
    ) external nonReentrant onlyRole(SELLER_ROLE) {
    ...
    function cancelTrade(uint256 tradeId) external nonReentrant {
        ...
        if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender) && tradeItem.seller != msg.sender) revert CallerIsNotAllowed();
        if (tradeItem.state != TradeState.ON_SALE)
            revert WrongTradeState({ state: tradeItem.state, requiredState: TradeState.ON_SALE });
    ...
    function cancelAuction(uint256 auctionId) external nonReentrant {
        ...
        if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender) && auctionItem.seller != msg.sender) revert CallerIsNotAllowed();
        if (auctionItem.state != AuctionState.ON_AUCTION)
            revert WrongAuctionState({ state: auctionItem.state, requiredState: AuctionState.ON_AUCTION });
