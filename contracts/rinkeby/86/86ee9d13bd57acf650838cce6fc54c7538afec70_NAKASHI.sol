...
/*
=> [Name]
* The Beginning (Rare #075)

=> [Traits]
* NFT Language: English/Spanish
* Manga Language: Spanish
* Edition: Genesis
* Chapter N°: 1
* Manga status: Burnt/Vault (seal 10/10 OR open 10/10 OR seal 7/10)
* Grading: 1
*/
...
contract NAKASHI is ERC721URIStorage, RoyaltiesV2Impl {
    using Address for address;
    ...
    uint256 public maxMintAmount = 3; // see  this
    ...
    constructor(string memory _initBaseURI) ERC721("Nakashi Manga", "NM") {
        ...
        setUnityDetails("common", 225); // Black
        setUnityDetails("uncommon", 150); // Purple
        setUnityDetails("rare", 75); // White
        setUnityDetails("epic", 35); // Red
        setUnityDetails("legendary", 15); // Yellow
    }

    event SetUnityDetails(string _unityName, uint16 _maxSupplyOfNFT);
    ...
    event NFTMintedCount(address minter, uint256 count, string unityName);
    event BatchLockNFT(uint256[] tokenIds);
    event BatchUnlockNFT(uint256[] tokenIds);
    modifier onlyOwner() {
    ...
    modifier check(string memory unityName, uint256 numberOfNFT) {
        PackDetail memory packDetail = unityDetails[unityName];
        uint256 currentBalance = IERC721(address(this)).balanceOf(_msgSender());
        uint256 canMintMax = maxMintAmount - currentBalance;

        require(
            numberOfNFT <= canMintMax,
            "Check: Cannot mint more than 3 NFT's"
        );
        require(
    ...
    function currentNFTNumber() external view returns (uint256) {
        return _tokenIds.current();
    }

    function mint(string memory unityName, uint256 numberOfNFT)
        public
        payable
        //   string memory metadataURL
        check(unityName, numberOfNFT)
    {
        for (uint8 i; i < numberOfNFT; i++) {
            _tokenIds.increment();
        ...
        emit NFTMintedCount(_msgSender(), numberOfNFT, unityName);
    }
    ...
    function batchLockNFT(uint256[] memory tokenIds) public onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            locked[tokenIds[i]] = true;
        }
        emit BatchLockNFT(tokenIds);
    }

    function batchUnlockNFT(uint256[] memory tokenIds) public onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            locked[tokenIds[i]] = false;
        }
        emit BatchUnlockNFT(tokenIds);
    }

    function setUnityDetails(string memory unityName, uint16 _maxSupplyOfNFT)
        public
        onlyOwner
    {
        require(
        ...
        emit SetUnityDetails(unityName, _maxSupplyOfNFT);
    }
