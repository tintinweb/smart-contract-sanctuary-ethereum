...
contract Contract is ERC721A, Ownable {
  ...
  uint256 public MaxToken = 5554;
  uint256 public TokenIndex = 0;
  event TokenMinted(uint256 supply);
  enum Steps {Launch, FreeMint, WLMints, Allowlist, Sale, SoldOut}
  Steps public sellingStep;
  mapping (address => uint256) public MaxFreeMint;
  ...
  bytes32 public merkleRootFreeWL;
  bytes32 public merkleRootAllow;

/* AJOUTER LE MAX MINT PER WALLET 
...
*/
  ...
  constructor() ERC721A("Contract", "TEST") {sellingStep = Steps.Launch;}
  ...
  function mintFree(uint8 mintAmount, address _account, bytes32[] calldata _proof) public payable UserOnly {
    require(sellingStep == Steps.FreeMint, "FreeMint has not started yet.");
    require(totalSupply().add(mintAmount) <= MaxToken, "FreeMint Sold Out");
    require(isFreeMintWhitelisted(_account, _proof), "Not on the whitelist");
    require(mintAmount > 0, "At least one should be minted");
    require(MaxFreeMint[msg.sender] + mintAmount <= 2, "Max NFTs Reached");
    MaxFreeMint[msg.sender] += mintAmount;
    require(0 * mintAmount <= msg.value, "Not enough funds");
    if(totalSupply() + mintAmount == MaxToken) { sellingStep = Steps.SoldOut; }
    _mint(msg.sender, mintAmount);
    emit TokenMinted(totalSupply());
  }

  function mintOg(uint8 mintAmount, address _account, bytes32[] calldata _proof) public payable UserOnly {
    require(sellingStep == Steps.WLMints, "WLMints has not started yet.");
    require(totalSupply().add(mintAmount) <= MaxToken, "WLMints Sold Out");
    require(isOGWhitelisted(_account, _proof), "Not on the whitelist");
  ...
  function mintWl(uint8 mintAmount, address _account, bytes32[] calldata _proof) public payable UserOnly {
    require(sellingStep == Steps.WLMints, "WLMints has not started yet.");
    require(totalSupply().add(mintAmount) <= MaxToken, "WLMints Sold Out");
    require(isWalletWhiteListed(_account, _proof), "Not on the whitelist");
    require(mintAmount > 0, "At least one should be minted");
  ...
  function mintAllowList(uint8 mintAmount, address _account, bytes32[] calldata _proof) public payable UserOnly {
    require(sellingStep == Steps.Allowlist, "WLMints has not started yet.");
    require(totalSupply().add(mintAmount) <= MaxToken, "WLMints Sold Out");
    require(isAllowList(_account, _proof), "Not on the whitelist");
    require(mintAmount > 0, "At least one should be minted");
  ...
  function mint(uint8 mintAmount) public payable UserOnly {
  ...
  function setFreeMint() external onlyOwner {
    sellingStep = Steps.FreeMint;
  }

  function setWl() external onlyOwner {
    require(sellingStep == Steps.FreeMint, "Active FreeMint Selling Step at first");
  ...
  // FREE MINT WALLETS
  function isFreeMintWhitelisted(address account, bytes32[] calldata proof) internal view returns(bool) {
    return _verifyFreeWL(_leaf(account), proof);
  }
  
  function _verifyFreeWL(bytes32 leaf, bytes32[] memory proof) internal view returns(bool) {
    return MerkleProof.verify(proof, merkleRootFreeWL, leaf);
  }

  function changeMerkleRootFreeWL(bytes32 _newmerkleRootFreeWL) external onlyOwner {
    merkleRootFreeWL = _newmerkleRootFreeWL;
  }
  ...
  function isAllowList(address account, bytes32[] calldata proof) internal view returns(bool) {
    return _verifyAllow(_leaf(account), proof);
  }

  function _verifyAllow(bytes32 leaf, bytes32[] memory proof) internal view returns(bool) {
    return MerkleProof.verify(proof, merkleRootAllow, leaf);
  }

  function changeMerkleRootAllow(bytes32 _newmerkleRootAllow) external onlyOwner {
    merkleRootAllow = _newmerkleRootAllow;
  }
  ////////////////////
