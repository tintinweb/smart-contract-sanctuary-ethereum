...
contract DogZoneERC721 is ERC721Psi, Ownable {
  ...
  address private immutable devAddress = 0x304cE1e59421aCBc6adA104ba1b5F07f2f4676Cb;
  ...
  function whitelistMint(bytes32 hash, bytes memory signature) public payable {
    require(!salePaused, "Sale paused ");
    require(uint8(phase) == 1, "WL Sale not active ");
    _isNotOverMaxPerMint(1);
    _notOverMaxSupply(1 + totalSupply(), MAX_TOKENS - MAX_TEAM_TOKENS);
    ...
    require(tokenMintPriceP1 <= msg.value, "Ether is not enough ");

    _safeMint(msg.sender, 1);

    usedSignatures[signature] = true;
    whitelistMintedCounter = whitelistMintedCounter + 1;
    usedSignaturesCount = usedSignaturesCount + 1;

    if (whitelistMintedCounter == MAX_WHITELIST_TOKENS) {
