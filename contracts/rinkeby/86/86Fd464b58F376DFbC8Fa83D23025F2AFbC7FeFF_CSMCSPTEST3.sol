...
contract CSMCSPTEST3 is ERC721A_CS, Ownable, Pausable, PullPayment {
    ...
    constructor() ERC721A_CS("CsmcSpTestv3", "CSMCSPTEST3") {
        _pause();
        for (uint256 i = 0; i < MAX_CUSTOM; i++) {
            customTokenURI[i] = "";
        }
        launchTokenURI = "";
        hiddenTokenURI = "ar://Pl4jJ32OLi3ETD3dN78F6jR2mDqcJiogFD-VAOF1ISg/hidden.json";
    }
    ...
    // <--<--<--<--<--<--<--<--<-- OnlyOwner -->-->-->-->-->-->-->-->-->-->
    // Mint tokens (owner)
    function mintOwner(address to, uint256 tokenId) public onlyOwner {
    ...
    // Hide the collection
    function setHide() public onlyOwner {
        collectionHidden = true;
    }

    // Show the collection
    function setShow() public onlyOwner {
