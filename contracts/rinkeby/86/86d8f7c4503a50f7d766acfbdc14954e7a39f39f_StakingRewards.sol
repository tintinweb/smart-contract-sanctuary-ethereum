...
contract RewardPool is Ownable {
    ...
    function setBCXAddress(address _bcx) external onlyOwner {
        require(_bcx != address(0), "Zero Address");
        bcx = _bcx;
    }

    function rewardTo(address _account, uint256 _rewardAmount) external onlyManager {
        require(IERC20(bcx).balanceOf(address(this)) > _rewardAmount, "Insufficient Balance");
        IERC20(bcx).transfer(_account, _rewardAmount);
    }

    function withdrawToken(address token, address _account) external onlyManager {
