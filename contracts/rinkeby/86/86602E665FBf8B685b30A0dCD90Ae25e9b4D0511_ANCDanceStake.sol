...
contract ANCDanceStake is ANCStake{
    ...
    function stake(uint32 coins, uint16 stakingDuration) external stakingGate(stakingDuration){
        //require(amount % _tokensPerShare == 0, "Can only stake tokens in increments");
        uint256 amount = uint256(coins) * TOKENS_PER_COIN;
        uint256 allowance = _dance.allowance(msg.sender, address(this));
        require(allowance >= amount, "Check the token allowance");
        uint256 currentWeek = timestamp2week(block.timestamp);
        _dance.transferFrom(msg.sender, address(this), amount);
    ...
    function unstake(uint256 id) external {
        ...
        SafeERC20.safeTransfer(_dance, msg.sender, payout);
    }
    ...
    function unstakeAll() external {
        ...
        // weird logic required because array length changes in unstake
        uint256 i = 0;
...
abstract contract ANCStake is Ownable{
    ...
    modifier stakingGate(uint32 duration){
        require(_stakingStart > 0, "Staking has not started");
        require(duration >= 1, "Minimum staking period 1 week");
        require(duration <= MAX_STAKING_DURATION, "Maximum staking period 1 year");
        _;
    }

    function getStakedIDs(address) virtual public view returns(uint256[] memory);

    function getNumStaked(address) virtual public view returns(uint256);
    ...
    function reserveAndGetTokens() internal returns(uint256){
        //console.log("balance:", _dance.balanceOf(address(this)));
        uint256 newReserved = (_dance.balanceOf(address(this)) - _reservedTokens) * tenthPercentPerWeek / 1000;
        ...
        for (uint256 week = lastUnreservedWeek; week < currentWeek; week++) {
            if(week & 1 == 0){
                if (_weeklyInfo[week].totalSharesEvenWeek > 0) {
                    _weeklyInfo[week].tokensEvenWeek = uint96(reserveAndGetTokens());
                    //console.log("tokens for week", week, _weeklyInfo[week].tokensEvenWeek);
                }
            } else {
                if (_weeklyInfo[week-1].totalSharesOddWeek > 0) {
                    _weeklyInfo[week-1].tokensOddWeek = uint96(reserveAndGetTokens());
    ...
    function setRewardToken(address tokenAddress) external onlyOwner {
        _dance = IERC20(tokenAddress);
    }

    function setStakingStart() external onlyOwner {
        require(_stakingStart == 0, "Staking has already started.");
    ...
    function getBasePayoutForWeek(uint256 week) internal view returns(uint256){
        if(week & 1 == 0){
    ...
    function timestamp2week (uint256 timestamp) internal view returns(uint256) {
        return ((timestamp - _stakingStart) / ONE_WEEK)+1;
    }
    ...
    function getAvailableFunds() public view returns(uint256){
        return _dance.balanceOf(address(this)) - _reservedTokens;
    }
}
