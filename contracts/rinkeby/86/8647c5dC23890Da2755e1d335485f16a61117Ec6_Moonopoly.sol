...
contract Moonopoly is ERC1155, Ownable, Pausable, ReentrancyGuard
{
    ...
    function _drawRandomCard() internal virtual returns (uint256 cardID) 
    {
        ...
        for (uint256 i = 1; i < _UNIQUE_CARDS; i++) 
        {
            ...
            if (value < probability && _canMint(i)) { return i; }
            else { value -= probability; }
        }

        // If Pseudorandom card is not valid, draw from top of the stack
        return _drawAvailableCard();
    }

     function _drawAvailableCard() internal virtual returns (uint256 ID) 
     {
