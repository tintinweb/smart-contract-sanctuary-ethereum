...
contract VotingContract is ERC721 {
  ...
  constructor() ERC721("MyNFT", "MTK") {
    ...
    mapping (address => uint256) public CandidateNoOfVoting ;
 ...
 modifier OnlyOwner{
    ...
    address []  public Remaing_Candidate;
...
function addCandidate(address []  memory _candidate) public {
...
function place_Vote(uint _place) public {
 ...
 if(_place == 1){
    ...
    for(uint i = 0 ; i<AllCandidate.length ; i++){
     ...
     for(uint i = 0 ; i<AllCandidate.length ; i++){
        if(AllCandidate[i] != Winner){
            Remaing_Candidate.push(AllCandidate[i]);
 ...
 function FinishVoting() public {
    ...
    function Reward_(uint voter_index) internal {
        Reward.transfer(AllVoter[voter_index] , 1000);
