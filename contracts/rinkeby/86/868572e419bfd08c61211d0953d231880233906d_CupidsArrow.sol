...
// File: contracts/cupids_arrow.sol
...
contract CupidsArrow is ERC721("CupidsArrow", "CSARROW"), ERC721Enumerable, Ownable {
    ...
    uint256 public mintLimitPerTx = 5;
    uint256 public mintLimitPerWallet = 20;
    ...
    uint256 public partnerMintAmount = 0;
    ...
    function setMintWhitelistActiveActive(bool _isActive) external onlyOwner {
        mintWhitelistActive = _isActive;
    }
    ...
    function canMint(address _address) public view returns (bool, string memory) {
        if (addressMintedCount[_address] >= mintLimitPerWallet) {
            return (false, "You have reach the limit of allowed mints per wallet.");
        }
        return (true, "");
    }

    function withdraw(address _address, uint256 percentage) public onlyOwner {
        require(percentage > 0 && percentage <= 100 ,"invalid percentage");
        uint256 balance = address(this).balance;
        uint256 amount1 = balance * percentage / 100;
        payable(_address).transfer(amount1);
        withdrawn = true;
    }
    ...
    function updateMintLimitPerTx(uint256 _newLimit) public onlyOwner {
        mintLimitPerTx = _newLimit;
    }

    function updateMintLimitPerWallet(uint256 _newLimit) public onlyOwner {
        mintLimitPerWallet = _newLimit;
    }
    ...
    function mintWhitelistNFT(uint256 _numOfTokens) public payable {
        require(mintWhitelistActive, "Mint Whitelist Not active");
        require(whitelist[msg.sender], "Not whitelisted");
        require(_numOfTokens <= 1, "Can't mint more than 1 NFT");
        require(addressMintedCount[msg.sender] + _numOfTokens <= mintLimitPerWallet, "Can't mint more than limit per wallet");
        ...
        addressMintedCount[msg.sender] +=_numOfTokens;
    }

    function mintNFT(uint256 _numOfTokens) public payable {
        require(mintActive, "Not active");
        require(_numOfTokens <= mintLimitPerWallet, "Can't mint more than limit per tx");
        require(addressMintedCount[msg.sender] + _numOfTokens <= mintLimitPerWallet, "Can't mint more than limit per wallet");
        require(mintPrice.mul(_numOfTokens) <= msg.value, "Insufficient payable value");
        require(totalSupply().add(_numOfTokens).add(partnerMintAmount) <= TOTAL_NFT, "Can't mint more than 10000");
        (bool success, string memory reason) = canMint(msg.sender);
    ...
    function partnersMintMultiple(address[] memory _to) public {
        require(mintWhitelistActive, "cannot mint yet");
