...
contract ZeeverseTicket is ERC721A, Ownable {
    ...
    uint256 public SALE_TIME_START; // todo
    uint256 public SALE_TIME_END; // todo
    ...
    constructor(
        ...
        baseURI = _uri;
        WETH = _weth;
        LvInfo[LevelType.Legend] = infoLegend;
        LvInfo[LevelType.Epic] = infoEpic;
        LvInfo[LevelType.Rare] = infoRare;
    }

    function mint(TokenStatus memory tokenStatus) external {
        // Gas optimization
        uint256 _num = 1;
        ...
        returns (string memory)
    {
        ...
        //     jungle: 2 => 9
        uint256 uriNum = 
            3 * uint256(tokenStatus.levelType) +
            uint256(tokenStatus.attrType) +
            1;

        return
            bytes(_base).length != 0
                ? string(abi.encodePacked(_base, uriNum.toString()))
                : "";
    }
    ...
    function setSaleStatus(uint256 _SALE_TIME_START, uint256 _SALE_TIME_END)
        external
        onlyOwner
    {
        SALE_TIME_START = _SALE_TIME_START;
