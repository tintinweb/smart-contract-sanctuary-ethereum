...
contract Koin is ERC721A, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "Koin - The Contract is Paused");
        require(
            totalSupply() + _amount <= maxSupply,
            "Koin - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(msg.value >= cost * _amount, "Koin - insufficient ethers");
        }
