...
error SaleClosed();
error ExceedsWhitelistMax();
error ExceedsPublicMax();
error ExceedsWLUserAllowance();
error ExceedsTokensPerUser();
...
contract NTTwo is ERC721A, Ownable, ReentrancyGuard {
  constructor() ERC721A("NTP2", "NTT2") {}

    using Strings for uint256;
    using ECDSA for bytes32;

    uint256 public NTT_PUBLIC_PER_USER = 2;
    uint256 public  NTT_WHITELIST = 25; //3344
    uint256 public  NTT_GIFT = 5; //100
    uint256 public  NTT_PUBLIC = 25; //1000
    uint256 public constant NTT_MAX_SUPPLY = 55; // 4444
    uint256 public NTT_WHITELIST_PRICE = 0.00008 ether;
    uint256 public NTT_PUBLIC_PRICE = 0.00009 ether;

    string private _finalProvenanceHash;
    string private _baseTokenURI = "ipfs://QmWz3xM3yACJZ3imE4iYdrWSiA3QWNqyEXyms5MqWUm7wc/";
    address private _ownerAddress = 0x696B88Ee6EEC155Eca37A12BC038014b81D4D05C;     // gnosis-safe address
    address private _signerAddress = 0x8b3A8f944F15f17EDE5a25fa63867b27e2A9fA2c;

    uint64 public giftingAmount;
    uint64 public publicAmountMinted;
    uint64 public whitelistAmountMinted;

    mapping(address => uint64) public publicMintedCount;
    ...
    //TODO - set back to private function 
    function hashTx(address sender, uint256 tokenLimit) pure public returns(bytes32) {
    ...
    liveSale callerIsUser tokenAmountValid(tokenQuantity) {
        ...
        if(whitelistAmountMinted + tokenQuantity > NTT_WHITELIST) revert ExceedsWhitelistMax();
        if(msg.value < NTT_WHITELIST_PRICE * tokenQuantity) revert InsufficientEth();

        uint64 mintedCount = _getAux(msg.sender);
        if(mintedCount + tokenQuantity > tokenLimit) revert ExceedsWLUserAllowance(); //TODO error should be more informative
    ...
    function publicMint(uint64 tokenQuantity) external payable liveSale callerIsUser tokenAmountValid(tokenQuantity) {
        ...
        if(publicAmountMinted + tokenQuantity > NTT_PUBLIC) revert ExceedsPublicMax();
        if(msg.value < NTT_PUBLIC_PRICE * tokenQuantity) revert InsufficientEth();

        uint64 mintedCount = publicMintedCount[msg.sender];
        if(mintedCount + tokenQuantity > NTT_PUBLIC_PER_USER) revert ExceedsTokensPerUser();

        _safeMint(msg.sender, tokenQuantity);        
        publicMintedCount[msg.sender] = mintedCount + tokenQuantity;
    ...
    function gift(address[] calldata winners) external onlyOwner {
        ...
        if(giftingAmount + winners.length > NTT_GIFT) revert ExceedsGiftsMax(); 

        for (uint256 i = 0; i < winners.length; i++) {
            _safeMint(winners[i], 1);
        }

        giftingAmount += uint64(winners.length);
    }
    ...
    function setNttPublicUserLimit(uint256 _limit) public onlyOwner {
    ...
    function setWhitelistPrice(uint256 _salePrice) public onlyOwner {
        NTT_WHITELIST_PRICE = _salePrice;
    }
  
    function reallocateTokens(uint256 whitelistAmount, uint256 publicAmount, uint256 giftAmount) public onlyOwner{
    ...
    function getWLMintedAmount(address user) public view returns (uint256) {
        return _getAux(user);
    }

    function getPublicMintedAmount(address user) public view returns (uint256) {
        return publicMintedCount[user];
    }
