...
contract CanvasN is ERC721 {
  ...
  bool public lockNft = false; // lockNft
  ...
  function getYesOrNoWhiteList(address _user) public view returns (bool) {
  ...
  function publicMint(uint256 _requestedAmount) public payable {
    ...
    require(
      nowTokenIds + _requestedAmount <= maxSupply + 1,
      'Exceed max amount'
    ); // 민팅 총 갯수
    require(
      _requestedAmount > 0 && _requestedAmount <= maxMintAmount,
      'Too many requests or zero request'
    ); // 최대 구매량 (한번의 tx)
    require(msg.value == price * _requestedAmount, 'Not enough eth'); // 이더 잔고부
    require(
      balanceOf(msg.sender) + _requestedAmount <= maxMintPerSale,
      'Exceed max amount per person'
    );

    for (uint256 i = 1; i <= _requestedAmount; i++) {
      _safeMint(msg.sender, nowTokenIds + i);
      _tokenIds.increment();
    }
  }
  ...
  function whiteListSale(uint256 _requestedAmount) public payable {
    uint256 nowTokenIds = _tokenIds.current();
    require(
      getYesOrNoWhiteList(msg.sender),
      'this address is not whitelist user'
    );
    require(publicMintEnabled, 'The public sale is not enabled!'); // 민팅 가능 여부
    require(
      nowTokenIds + _requestedAmount <= maxSupply + 1,
    ...
    if (!revealed) {
      ...
      return
        bytes(notRevealedUri).length > 0
          ? string(
            abi.encodePacked(notRevealedUri, _tokenId.toString(), baseExtension)
          )
          : '';
    }
  ...
  function reveal(bool _type) public onlyAdminAndOwner {
    revealed = _type;
  }

  function setLockNft(bool _type) public onlyAdminAndOwner {
    lockNft = _type;
  }

  function setSaleState(bool _type) public onlyAdminAndOwner {
    publicMintEnabled = _type;
  }

  function setPrice(uint256 _newPrice) public onlyAdminAndOwner {
  ...
  function setBaseURI(string memory _newBaseURI) public onlyAdminAndOwner {
    baseURI = _newBaseURI;
  }
  ...
  //whitelist
  function whitelistUser(address _user) public onlyAdminAndOwner {
    whiteListed[_user] = true;
  }

  function removeWhitelistUser(address _user) public onlyAdminAndOwner {
    whiteListed[_user] = false;
  }

  function setAll(
    string memory _allBaseURI,
    ...
    uint256 _maxMintPerSale
  ) public onlyAdminAndOwner {
    baseURI = _allBaseURI;
    notRevealedUri = _notRevealedURI;
    revealed = _reveal;
    lockNft = _lock;
    publicMintEnabled = mintState;
    price = _price;
    maxSupply = _maxSupply;
