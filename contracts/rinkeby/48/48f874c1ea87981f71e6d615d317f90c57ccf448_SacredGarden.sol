...
contract SacredGarden is ERC721ABurnable, ERC2981, Ownable, Pausable {
    struct StageConfig {
        bool set; // Is stage configured
        uint8 maxPerAddress; // Max mints per address
        uint240 price; // Price per mint
    ...
    function allowlistMint(
        ...
        validate(sc, _currentStage, quantity, to);

        require(sc.root != 0, "not allowlist mint");
        require(_verify(_leaf(to), sc.root, proof), "bad merkle proof");

        stageMinted[_currentStage][to] += quantity;
        _safeMint(to, quantity);
    }
    ...
    function validate(
        ...
        require(
            stageMinted[stage][to] + quantity <= sc.maxPerAddress,
            "over limit"
        );
    }
