...
contract SkinwalletNFT is Context, Ownable, ERC721 {
  ...
  function buyToken(uint8 _amount) public payable {
    ...
    require(balanceOf(msg.sender) + _amount <= LIMIT_TOKENS_PER_WALLET_PUBLIC, "mint: limit tokens for this wallet reached");
  ...
  function withdraw(address payable receiver, uint256 amount) public payable onlyOwner {
  ...
  function createGiveawayTokens(
    ...
    uint256 _currentSupply = _nextTokenId - 1;
    require(
      balanceOf(msg.sender) + tokensAmount <= LIMIT_TOKENS_FOR_GIVEAWAYS,
