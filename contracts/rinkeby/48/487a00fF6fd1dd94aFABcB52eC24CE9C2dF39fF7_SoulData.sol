...
contract SoulData is ISoulData, Adminable {
    ...
    // Token image key
    string public tokenImageKey = "svgHead";

    // TokenURI Render mode
    // 0 : data:application/json;base64,
    // 1 : data:application/json;utf8,
    uint256 internal _tokenURIMode;

    // Trait type index sequence coding mode
    // 0 : Braille unicode
    // 1 : Alphabet code, A, B, C, ..., Z, AA, AB ...
    uint256 public _traitTypeSequenceMode;

    // Add subffix to all badge trait types
    string internal _traitTypeSuffix;
    ...
    function setTraitSeqCoding(uint256 _mode) external onlyAdmin {
        _traitTypeSequenceMode = _mode;
    }

    // Update suffix for badge trait type name
    function setTraitTypeSuffix(string memory _suffix) external onlyAdmin {
        _traitTypeSuffix = _suffix;
    }
        ...
        returns (string memory) 
    {
        return string(abi.encodePacked('"attributes":[',
            '{"trait_type":"Creation Time","value":"', toString(_creationTime), '"},',
            _badgeTraits,
    ...
    /**
     * @dev See {ISoulData-getSoulBadgeVoteScore}.
     */
    function getSoulBadgeVoteScore(address _badgeContract) external view returns (uint256) {
        try IScore(_badgeContract).voteScore() returns (uint256 score) {
            return score;
        } catch (bytes memory) {
        ...
        if (isImplementing(_badgeContract, 0x80ac58cd)) {
            ...
            if (getERC721Balance(_badgeContract, _soulTokenOwner) > 0) {
                if (traitValueLength != 0) {
                    traitName = string(abi.encodePacked("ERC721 NFT ", traitSeqCode, _traitTypeSuffix));
            ...
            if (getSoulBadgeBalanceForSoul(_soulContract, _soulTokenId, _badgeContract) > 0) {
                if (traitValueLength != 0) {
                    traitName = string(abi.encodePacked("Soul Badge ", traitSeqCode, _traitTypeSuffix));
                    output = string(abi.encodePacked(output, '{"trait_type":"',traitName,'","value":"',traitValue, '"},'));
                }
            }    
        }
    ...
    /**
     * @dev See {ISoulData-getTokenBalance}.
     */
    ...
    function isValidTokenCustomName(string calldata name) external pure returns (bool) {
        ...
        for (uint256 i; i < data.length; i++) {
            char = uint8(data[i]);
            // accept A-Z:(64-90), a-z:(97-122), space(32 or 255)
...
interface ISoulData {
    ...
    /**
     * @dev Get token metadta attributes
     */
     ...
     */
    function getSoulBadgeVoteScore(address badgeContract) external view returns (uint256);
    ...
    /**
     * @dev Returns whether custom name string contains all valid characters
     *      We only accept [a-z], [A-Z], [space] characters.
     */
    function isValidTokenCustomName(string calldata name) external pure returns (bool);
...
interface IScore {
     ...
     */
    function voteScore() external view returns (uint256);
