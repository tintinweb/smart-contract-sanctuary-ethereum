...
CashMachine Decentralized Finance

u'all are not ready for this

Tax for Buying/Selling: 15%
...
contract CMAC is Ownable, IERC20 {
    address UNISWAPROUTER = address(0xc2fe2f945c5CAF788163d68B3696e32c50063e2F);
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string private _name = "Cash Machine";
    string private _symbol = "CMAC";
    ...
    address payable private PrivateSaleWallet;
    ...
    address[] private AdvWallets;  
    address[] private TeamWallets;  

    constructor(
        address payable _privateSaleWallet,
        address payable _preSaleWallet,
        address payable _publicSaleWallet,
        address payable _liquidityWallet,
        address payable _marketingWallet,
        address payable _researchWallet,
        address[] memory _advisorAddress,
        ...
        AdvWallets = _advisorAddress;
        TeamWallets = _teamAddress;
        ...
        _mint(PrivateSaleWallet, 300000000 * (10**18));
        _mint(PreSaleWallet,     100000000 * (10**18));
        _mint(PublicSaleWallet,  50000000 * (10**18));
        _mint(LiquidityWallet,   50000000 * (10**18));
        _mint(MarketingWallet,   100000000 * (10**18));
        _mint(ResearchWallet,    100000000 * (10**18));
        _mint(TeamWallets[0],     200000000 / uint256(3) * (10**18));
        _mint(TeamWallets[1],     200000000 / uint256(3) * (10**18));
        _mint(TeamWallets[2],     200000000 * (10**18) - balanceOf(TeamWallets[0]) - balanceOf(TeamWallets[1]));         
        _mint(AdvWallets[0],      100000000 / uint256(3) * (10**18));
        _mint(AdvWallets[1],      100000000 / uint256(3) * (10**18));
        _mint(AdvWallets[2],      100000000 * (10**18) - balanceOf(AdvWallets[0]) - balanceOf(AdvWallets[1]));         
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool)
    {
        ...
        require(
            ...
            "Cash Machine: decreased allowance below zero"
        );
    ...
    function transferFrom(
        ...
        require(
            ...
            "Cash Machine: transfer amount exceeds allowance"
        );
    ...
    function _transfer(
        ...
        require(
            ...
            "Cash Machine: transfer amount exceeds balance"
        );
    ...
    function _executeTransfer(
        ...
        require(sender != address(0), "Cash Machine: transfer from the zero address");
        require(recipient != address(0), "Cash Machine: transfer to the zero address");
        uint256 senderBalance = _balances[sender];
        require(
            senderBalance >= amount,
            "Cash Machine: transfer amount exceeds balance"
    ...
    function _approve(
        ...
        require(owner != address(0), "Cash Machine: approve from the zero address");
        require(spender != address(0), "Cash Machine: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "Cash Machine: mint to the zero address");
        _totalSupply += amount;
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "Cash Machine: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "Cash Machine: burn amount exceeds balance");
    ...
    function _executeSwap(uint256 tokens, uint256 native) private {
        ...
        if (address(LiquidityWallet) != address(0)) {
        ...
        if (nativeMarketing > 0) {
            payable(LiquidityWallet).transfer(nativeMarketing);
        }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "Cash Machine: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    {
        require(pair != uniswapV2Pair, "Cash Machine: DEX pair can not be removed");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "Cash Machine: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "Cash Machine: the router is already set to the new address"
        );
    ...
    function compound() public {
        require(compoundingEnabled, "Cash Machine: compounding is not enabled");
    ...
    function withdrawnDividendOf(address account) public view
        returns (uint256)
    {
        return dividendTracker.withdrawnDividendOf(account);
    }

    function accumulativeDividendOf(address account) public view
        returns (uint256)
    {
        return dividendTracker.accumulativeDividendOf(account);
    }

    function getAccountInfo(address account) public view
        returns (
            address,
            uint256,
            uint256,
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "CMAC_DividendTracker";
    string private _symbol = "CMAC_DividendTracker";
    ...
    function excludeFromDividends(address account, bool excluded) external onlyOwner
    {
        require(
            ...
            "CMAC_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function isExcludedFromDividends(address account) public view
        returns (bool)
    {
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "CMAC_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "CMAC_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "CMAC_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("CMAC_DividendTracker: method not implemented");
    }
