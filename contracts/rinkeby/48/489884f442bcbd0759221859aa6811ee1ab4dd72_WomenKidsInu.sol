...
contract WomenKidsInuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("WomenKidsInu_Dividend_Tracker", "WomenKidsInu_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "WomenKidsInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "WomenKidsInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WomenKidsInu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "WomenKidsInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "WomenKidsInu_Dividend_Tracker: Cannot update claimWait to same value");
...
contract WomenKidsInu is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    WomenKidsInuDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 40000000 * (10**18); // 4%
    uint256 public maxWalletToken = 40000000 * (10**18); // 4%
    uint256 public swapTokensAtAmount = 500 * (10**18); 

    uint256 private ETHRewardsFee;
    uint256 public liquidityFee;
    uint256 public totalFees;
    uint256 private extraFeeOnSell;
    ...
    function setFee(uint256 _liquidityFee, uint256 _marketingFee) public onlyOwner {
        ...
        totalFees = liquidityFee.add(marketingFee); // total fee transfer and buy
    }
    ...
    constructor() ERC20("WomenKids Inu", "WKI") {
        
        ETHRewardsFee = 0;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra sell fee
        marketingFee = 7;
        marketingWallet = payable(0xB899dd2822b2fc60403ad8fFb34914d710823f3B);

    
        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new WomenKidsInuDividendTracker();
        ...
        */
        _mint(owner(), 1_000_000_000 * (10**18)); // 1 Billion Tokens
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "WomenKidsInu: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "WomenKidsInu: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WomenKidsInu: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "WomenKidsInu: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WomenKidsInu: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WomenKidsInu: Cannot update gasForProcessing to same value");
