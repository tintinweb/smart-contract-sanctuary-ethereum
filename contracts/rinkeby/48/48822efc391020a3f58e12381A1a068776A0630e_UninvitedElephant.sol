...
[email protected][email protected]@[email protected]
*/
contract UninvitedElephant is ERC721Enumerable, Ownable, ReentrancyGuard, NonblockingLzApp {
    ...
    modifier mintCompliance(uint8 _mintAmount) {
        ...
        require(
            nextMintId + _mintAmount <= maxMintId + 1, 
    ...
    function whitelistMint(uint8 _mintAmount, bytes32[] calldata _merkleProof)
     ...
     mintCompliance(_mintAmount) {
        require(whitelistMintEnabled, "UninvitedElephant: The whitelist sale is not enabled");
        require(
            proofWhitelist(_merkleProof),
            "UninvitedElephant: Address not in the whitelist"
        );
        require(
            whitelistClaimed[_msgSender()] + _mintAmount <= maxMintAmountPerTx ,
            "UninvitedElephant: Address already claimed all"
        );
    ...
    function send(uint16 _dstChainId, bytes calldata _toAddress, uint _tokenId) external payable {
        require(_exists(_tokenId), "UninvitedElephant: URI query for nonexistent token");
