...
contract Skinniez is ERC721A, ERC2981, Ownable, ReentrancyGuard {

  using Strings for uint256;

  uint256 constant MAX_SUPPLY = 5000;
  uint256 constant MAX_MINTS_PER_ADDR = 5;
  ...
  uint256 public royaltyFraction = 400;
  ...
  constructor(string memory _initialBaseURI) ERC721A("Skinniez", "SKNZ") {
  ...
  function mintRequires(uint256 amount, address sender) internal {
    require(isActive, "Sale is closed");
    require(amount <= MAX_MINTS_PER_ADDR - _numberMinted(sender), "Insufficient mints left");
    require(msg.value == price * amount, "Incorrect payable amount");

    return;
  }

  function _verify(bytes32[] calldata _merkleProof) internal view {
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), 'invalid proof');

  }

  function mint(uint256 amount, bytes32[] calldata _merkleProof) public payable nonReentrant {
    address sender = _msgSender();

    mintRequires(amount, sender);
    
    if(isPresale) {
        _verify(_merkleProof);
    }
  ...
  function royaltyInfo(uint256 tokenId, uint256 salePrice) external view override returns (address receiver, uint256 royaltyAmount) {
      tokenId;
      uint256 _royaltyAmount = (salePrice * royaltyFraction) / 10000;
