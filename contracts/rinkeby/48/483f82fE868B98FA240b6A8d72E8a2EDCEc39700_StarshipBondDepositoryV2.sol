...
/// @title Starship Bond Depository V2
/// @author Zeus, Indigo
/// Review by: JeffX

contract StarshipBondDepositoryV2 is IBondDepository, NoteKeeper {
    /* ======== DEPENDENCIES ======== */
    ...
    function setCrowdsale ( address _contract ) external onlyGovernor {
      crowdsale = _contract;    
    }

    function crowdsalePurchase ( uint256 _payout ) external returns (uint256 rewards) {
      require(msg.sender == crowdsale, "called from invalid address");
      
      uint256 rewards = _giveRewards(_payout);
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
    ...
    ISUPERNOVA internal immutable gOHM;
    ...
    constructor(
        ...
        IStaking _staking,
        ITreasury _treasury
    ) FrontEndRewarder(_authority, _ohm, _treasury) {
        gOHM = _gohm;
        staking = _staking;
        treasury = _treasury;
    }
...
abstract contract FrontEndRewarder is StarshipAccessControlled {
    /* ========= STATE VARIABLES ========== */

    uint256 public daoRewardRate; // % reward for dao (3 decimals: 100 = 1%)
    uint256 public refRewardRate; // % reward for referrer (3 decimals: 100 = 1%)
    mapping(address => uint256) private rewards; // front end operator rewards

    IERC20 internal immutable ohm; // reward token
    
    ITreasury internal t;    
    
    address public operator = 0x0000000000000000000000000000000000000000;
//    address internal immutable rock; //operator address

    constructor(IStarshipAuthority _authority, IERC20 _ohm, ITreasury _treasury) StarshipAccessControlled(_authority) {
        t = _treasury;
        ohm = _ohm;
//        rock = 0x3305D042BB7a0f9D21a9fEb0489397CbEBD48ac2;
    }

    /* ========= EXTERNAL FUNCTIONS ========== */

    function migrateRewards(address _contract) external onlyGovernor {
      IMigrator migrator = IMigrator(_contract);
      (address _add, uint256 _amount) = migrator.getRewards();
      rewards[_add] = _amount;
    }

    // mint and pay rewards
    function getReward(uint256 _pct) external {
        require(rewards[msg.sender] > 0, "no rewards to get" );
        uint256 reward = ((rewards[msg.sender] * _pct) / 100);

        t.mint(address(this), reward);
        rewards[msg.sender] -= reward;
        ohm.transfer(msg.sender,reward);        

    }
    
    // pay rewards to operator address
    function getOperatorReward() external {
        uint256 reward = rewards[operator];

        rewards[operator] = 0;
        ohm.transfer(operator, reward);
    }
     ...
     */
    function _giveRewards(uint256 _payout) internal returns (uint256) {
        // calculate rewards paid to the DAO 
        uint256 toDAO = (_payout * daoRewardRate) / 1e4;         
        rewards[operator] += toDAO;
     ...
     */
    function setOperatorRewards(address _operator, uint256 _rate) external onlyGovernor {
        operator = _operator;
        daoRewardRate = _rate;
    }
    /**
     * @notice view outstanding rewards
     */
    function viewRewards(address _add) external view onlyGovernor returns (uint256) {
        uint reward = rewards[_add];
        return reward;
    }
...
interface IMigrator {
    function getRewards() external view returns (address, uint256);
}
