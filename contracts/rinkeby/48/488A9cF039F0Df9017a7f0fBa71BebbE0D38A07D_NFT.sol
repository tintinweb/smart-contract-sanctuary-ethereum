...
contract NFT is ERC721, Ownable {
    ...
    // This function is called by the contract owner to mint a new token.
    function mintTo(address recipient) public onlyOwner {
        require(currentTokenId.current() < maxTokenId);
        currentTokenId.increment();
        uint256 newItemId = currentTokenId.current();
        _safeMint(recipient, newItemId);
    }

    // This function is used by the contract owner to batch mint multiple tokens to the same address
    function batchMint(address recipient, uint256 count) public onlyOwner {
        require(currentTokenId.current() + count < maxTokenId);
    ...
    function batchMint(address[] memory recipients) public onlyOwner {
        require(currentTokenId.current() + recipients.length < maxTokenId);
        for (uint256 i = 0; i < recipients.length; i++) {
            mintTo(recipients[i]);
        }
    }

    // This function is used to batch transfer tokens to multiple recipients.
    ...
    // This function is used to batch approve token transfers.
