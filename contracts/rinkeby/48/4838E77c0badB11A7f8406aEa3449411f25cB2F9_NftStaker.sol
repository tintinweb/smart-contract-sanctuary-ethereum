...
contract NftStaker is IERC721Receiver, ReentrancyGuard {
    ...
    uint256 tier1Reward = 13000000000000000000;
    uint256 tier2Reward = 12000000000000000000;
    uint256 tier3Reward = 11000000000000000000;
    uint256 tier4Reward = 14000000000000000000;

    uint256 minNoOfDays = 60;
    ...
    modifier requireTimeElapsed(uint256 tokenId) {
        ...
        require(
            _getDays(receipt[tokenId].timestamp) >= 5,
            "requireTimeElapsed: Can not stake/unStake/harvest within a day"
        );
        _;
    }

    function setTier1Reward(uint256 _tier1Reward) external onlyDao {
        tier1Reward = _tier1Reward;
    }

    function setTier2Reward(uint256 _tier2Reward) external onlyDao {
        tier2Reward = _tier2Reward;
    }

    function setTier3Reward(uint256 _tier3Reward) external onlyDao {
        tier3Reward = _tier3Reward;
    }

    function setTier4Reward(uint256 _tier4Reward) external onlyDao {
        tier4Reward = _tier4Reward;
    }

    function setMinNoOfDays(uint256 _minNoOfDays) external onlyDao {
        minNoOfDays = _minNoOfDays;
    }
    ...
    function _getDays(uint256 _timestamp) internal view returns (uint256) {
        return (block.timestamp.sub(_timestamp)).div(minNoOfDays);
    }

    function _getNftTier(uint256 tokenId) internal view returns (uint256) {
        if (tokenId >= 1 && tokenId <= 11) {
            return tier1Reward;
        } else if (tokenId >= 12 && tokenId <= 22) {
            return tier2Reward;
        } else if (tokenId >= 23 && tokenId <= 33) {
            return tier3Reward;
        } else if (tokenId >= 34 && tokenId <= 36) {
            return tier4Reward;
        }
        return 0;
    }
    ...
    function _payoutStake(uint256 tokenId) internal {
        ...
        uint256 payout = timeStaked.mul(_getNftTier(tokenId));
    ...
    function getCurrentStakeEarned(uint256 tokenId) external view  returns (uint256) { 
        ...
        return _getDays(receipt[tokenId].timestamp).mul(_getNftTier(tokenId));
    }
