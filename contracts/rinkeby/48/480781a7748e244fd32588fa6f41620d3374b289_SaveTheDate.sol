...
contract SaveTheDate is ERC721, Ownable, ReentrancyGuard {
     ...
     * @return Token ID.
     */
    function mintPesaleDate(DateToMint calldata tokenDate, bytes32[] calldata merkleProof) external payable ensureAvailability nonReentrant returns (uint256) {
        require(suspendedSale == false, "The sale is suspended");
        require(salesPhase == SalesPhase.Presale, "Pre-sale is not active");
     ...
     * @return Token ID.
     */
    function mintPublicDate(DateToMint calldata tokenDate) external payable ensureAvailability nonReentrant returns (uint256) {
        require(suspendedSale == false, "The sale is suspended");
        require(salesPhase == SalesPhase.Public, "Sale is not active");
        
        (, bool _communityExempt, uint256 _communityIndex, uint256 _redeemIndex) = _verifyCommunityClaim(_msgSender());
