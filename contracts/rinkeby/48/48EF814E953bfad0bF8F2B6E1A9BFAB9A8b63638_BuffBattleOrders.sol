...
import "../../base/BuffItemBase.sol";


contract BuffBattleOrders is BuffItemBase {
  ...
  string internal constant _NAME = "Battle Orders";
  string internal constant _SYMBOL = "BATTLE_ORDERS";
  uint private constant _LEVEL = 1;
  IItem.ItemType private constant _TYPE = IItem.ItemType.SKILL;
  string private constant _URI = "https://raw.githubusercontent.com/tetu-io/tetu-game-assets/master/skills/1/battle_orders.png";
  ...
  function _generateBuffAttributes(uint tokenId) internal override returns (IStatController.Attributes memory, uint manaConsume) {
    uint random = _oracle().getRandomNumber(1e18);
    itemRarity[tokenId] = uint(_calculateRarity(random));
    ...
    defense : 0,
    blockRating : 0,
    life : _randomAttribute(20, 30, 1e18, random),
    mana : 0,
    fireResistance : 0,
    coldResistance : 0,
    lightningResistance : 0
...
abstract contract BuffItemBase is ItemBase, IBuffItem {
  ...
  mapping(uint => IStatController.Attributes) internal _buffItemAttributes;
  mapping(uint => uint) internal _buffItemMana;

  // ---- INITIALIZER ----

  function __BuffItemBase_init(
    string memory name_,
  ...
  external view override returns (IStatController.Attributes memory, uint) {
    return (_buffItemAttributes[tokenId], _buffItemMana[tokenId]);
  }
  ...
  function _afterMint(uint tokenId) internal override {
    ...
    (_buffItemAttributes[tokenId], _buffItemMana[tokenId]) = _generateBuffAttributes(tokenId);
  }

  function _additionalAugment(uint tokenId) internal override {
    IStatController.Attributes memory current = _buffItemAttributes[tokenId];

    _buffItemAttributes[tokenId] = IStatController.Attributes({
    ...
    _buffItemMana[tokenId] = _augmentAttribute(_buffItemMana[tokenId]);
  }

  function _generateBuffAttributes(uint tokenId) internal virtual returns (IStatController.Attributes memory, uint manaConsume);
