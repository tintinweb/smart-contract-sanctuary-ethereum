...
pragma solidity ^0.8.0;
    ...
    import "./brDAI.sol";
//Inherit from the ERC20 minting fucker contract


contract brrDAI is brDAI {
    using SafeMath for uint256;
    using SafeERC20 for IERC20; 



    //Identify this contract's own address - deprecated
    address public babydaddy;
    ...
    constructor() {
      // Initialize the babydaddy address to BE this contract. - deprecated
      //  babydaddy = address(this); - deprecated

        //We will change this address to an actual charities address
        charity = address(this);
        //emit Paternity_Test(address(0), babydaddy); - deprecated 
    }

    //Now we have this modifer to make sure functions can only be called by this contract's address - deprecated 
   // modifier onlyBbyDdy() {
    //    require(msg.sender == babydaddy, "get a paternity test bitch");
     //   _;
  //  }
   


    /// Withdraw `amount` of `token` to address `to`. Only callable by this contract's owner - deprecated
    //We are hardcoding this contract to only be able to withdraw to a specific charity 
    //Fuck you and your reee never hardcode shit, bullshit.  - deprecated, except the insults, those remain
    
   // function withdrawToCharity(address token, uint256 amount) public onlyOwner { - deprecated 
        //this transfer this specific token, to that specific address, in that specific amount
   //     IERC20(token).safeTransfer(charity, amount);
    //    emit Withdrawal(token, amount, charity);
   // }
...
contract brDAI is ERC20, Ownable{
    ...
    function _brDAI(address receiver, uint256 amount) internal { 
