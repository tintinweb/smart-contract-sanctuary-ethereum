...
contract MetaGodsStaking is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    bool public enableClaimResultEvents;
    ...
    event ClaimResult(address collectionAddress, uint256 tokenId, uint256 amount);

    event ClaimResultWithRisk(address collectionAddress, uint256 tokenId, uint256 amount, bool didWin);
    ...
    function toggleClaimResultEvents() external onlyOwner {
        enableClaimResultEvents = !enableClaimResultEvents;
    }
    ...
    function getYield(
        ...
        uint256 yield;
        bool _ignored;
        (yield, _ignored, _ignored) = calculateYield(collection_, tokenId_, applyRisk_);
    ...
    function getYieldAndEmit(
        ...
        bool appliedRisk;
        bool didWin;
        (yield, appliedRisk, didWin) = calculateYield(collection_, tokenId_, applyRisk_);

        if(enableClaimResultEvents) {
            if(appliedRisk) {
                emit ClaimResultWithRisk(collection_, tokenId_, yield, didWin);
            } else {
                emit ClaimResult(collection_, tokenId_, yield);
            }
        }
    ...
    function calculateYield(
        ...
        bool shouldApplyRisk = applyRisk_ && (gainLossBoost.gain > 0 || gainLossBoost.loss > 0);
        bool didWin;
        if(shouldApplyRisk) {
            didWin = _didWin(tokenId_);
            if(didWin) {
        ...
        return (yield, shouldApplyRisk, didWin);
    }
    ...
    function _getYieldAndUpdateDate(
        ...
        uint256 yield = getYieldAndEmit(collection_, tokenId_, true);
    ...
    function _exitStakeStanceMany(
        ...
        for (uint256 i = 0; i < tokenIds_.length; ++i) {
            ...
            yieldedAmount += getYieldAndEmit(collection_, tokenIds_[i], true);
