...
contract PixelApe is ERC721Tradable {
    ...
    mapping(uint256 => bool) internal pixelApesClaimed;
    ...
    function mint(address _to, uint256[] memory _tokenIds) public payable {
        ...
        require(_totalPublicSupply.current() + _tokenIds.length <= maxPublicSupply, "Max limit");
        for (uint256 i=0; i<_tokenIds.length; i++) {
            uint256 _tokenId = _tokenIds[i];
            require(canClaim(_tokenId) && Ape.ownerOf(_tokenId) == _msgSender(), "Bad owner!");
            pixelApesClaimed[_tokenId] = true;
            _mintPixelApe(_to, _tokenId);
        }
    }

    function canClaim(uint256 _tokenId) public view returns(bool) {
        return !pixelApesClaimed[_tokenId];
    }

    function _mintPixelApe(address _to, uint _tokenId) private {
        require(_totalPublicSupply.current() + 1 <= maxPublicSupply, "Max supply reached");
