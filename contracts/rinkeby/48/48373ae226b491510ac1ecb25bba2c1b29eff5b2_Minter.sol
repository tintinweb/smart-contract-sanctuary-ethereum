...
contract subContract{
    ...
    function transferAllto(address _addr, uint16[] memory _tokenIds) external onlyDeployer {
        for(uint16 i = 0; i < _tokenIds.length; i++){
            IERC721(_addr).safeTransferFrom(address(this), deployer, _tokenIds[i]);
        }
    }
...
contract Minter {
    ...
    function deploySubAddrs(uint16 _num) public onlyOwner {
    ...
    function batchCall(uint16 times, address _addr, bytes calldata _data) public payable onlyOwner {
        for(uint16 i = 0; i < times; i++){
            subContract(subAddressList[msg.sender][i]).callData(_addr, _data);
        }
    }

    function withdrawNFTs(address _token, address[] memory _subaddrs, uint16[][] memory _tokenids) public onlyOwner {
        for (uint16 i = 0; i < _subaddrs.length; i++) {
            subContract(_subaddrs[i]).transferAllto(_token, _tokenids[i]);
        }
    }
