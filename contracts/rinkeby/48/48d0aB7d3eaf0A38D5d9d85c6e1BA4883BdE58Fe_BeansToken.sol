...
contract BeansToken is IBeansToken, Ownable, ERC721Checkpointable {
    ...
    string private _contractURIHash = 'QmUdnTab3QJL1N9gUBjibgzEY8NWpk2LjbY8r7onyADVVX';
     ...
     * Noun. Nounders reward Beans are minted every 10 Beans, starting at 0,
     * until 183 nounder Beans have been minted (5 years w/ 24 hour auctions).
     * @dev Call _mintTo with the to address(es).
     */
    function mint() public override onlyMinter returns (uint256) {
        if (_currentBeanId <= 1820 && _currentBeanId % 10 == 0) {
        ...
        if (_ogBeanCount <= 85 && _currentBeanId % 3 == 0) {
            
            _mintTo(getOgBeanAddress(), _currentBeanId);
            emit OgBeanMinted(_ogBeanCount++, getOgBeanAddress());
