...
contract Wildp is ERC721, Ownable, ReentrancyGuard, ERC721Enumerable {
    ...
    function airdropTeam(uint256 quantity, address address_to) 
        ...
        onlyOwner
        {
        require(GENERAL_TOKEN - 50 + quantity <= MAX_SUPPLY, "too many already minted before team mint");
        unchecked {
            teamAmountMinted += quantity;
        }
        _mintCats(address_to, quantity);
    }

    function reserve(uint256 quantity, Sig memory MintRSV) 
        external 
        nonReentrant
        onlyOwner
        {
        ...
        require(
            verifySigner(
                platformAddress,
                messageHash(abi.encodePacked(msg.sender, quantity)),
                MintRSV
            ),
    ...
    function _mintCats(address _receiver, uint256 quantity) internal {
        for (uint256 i = 0; i < quantity; i++) {
            uint256 _tokenID = GENERAL_TOKEN + 1;
            _safeMint(_receiver, _tokenID);
            _mintedDate[_tokenID] = block.timestamp;
            emit Minted(
                _mintedDate[_tokenID],
                _tokenID,
                _receiver
            );
