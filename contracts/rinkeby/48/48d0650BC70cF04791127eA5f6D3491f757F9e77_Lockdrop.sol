...
contract Lockdrop is ILockdrop, TokenRecoveryUpgradeable, GuardableUpgradeable {
    ...
    function updatePool(
        ...
        requiredPrivileges = requiredPrivileges | GuardHelper.DEFAULT_REQUIRED; // Minimum required privileges

        _pools[poolId].enabled = enabled;
        _pools[poolId].label = label;
        _pools[poolId].requiredPrivileges = requiredPrivileges;

        emit PoolUpdated(poolId, label, requiredPrivileges, enabled, _msgSender());
    }
    ...
    function moveDelegation(bytes32 fromPoolId, bytes32 toPoolId, uint256 amount) external override {
        ...
        require(SOMA.guard().check(sender, toPool.requiredPrivileges), 'Lockdrop: NO PRIVILEGES');
        require(SOMA.guard().check(sender, fromPool.requiredPrivileges), 'Lockdrop: NO PRIVILEGES');
...
interface ILockdrop {
    ...
    event PoolUpdated(bytes32 indexed poolId, string label, bytes32 requiredPrivileges, bool enabled, address indexed sender);
