...
contract CoolSkullClub is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public WL_MINT_PRICE = 0.0666 ether;
    ...
    string private HIDDEN_URI = 'ipfs://QmUW9Yr5kGGYW4jQCWAnj9PS9ZFMJicuzu9eiWBzeRFhb5/metadata.json'; // Start of hidden meta data uri
    ...
    ProjectStatus public pStatus;

    constructor() ERC721A("COOL SKULL CLUB", "CSKC") { pStatus = ProjectStatus.Before; }

    modifier mintChecked(uint256 _mintAmount) {
        require(_mintAmount > 0 && _mintAmount < MAX_PER_WALLET , "COOL SKULL CLUB : Invalid mint amount!");
        require(totalSupply() + _mintAmount < MAX_SUPPLY, "COOL SKULL CLUB : Reached the max supply!");
        require(walletMint[msg.sender] + _mintAmount <= MAX_PER_WALLET, string(abi.encodePacked("COOL SKULL CLUB: You cannot mint more than ", MAX_PER_WALLET.toString(), " tokens!")));
        require(msg.value >= MINT_PRICE * _mintAmount, "COOL SKULL CLUB : Not enough balance!");
        _;
    }

    modifier wlChecked(uint256 _mintAmount) {
        require(_mintAmount > 0 && _mintAmount < MAX_PER_WALLET , "COOL SKULL CLUB : Invalid mint amount!");
        require(totalSupply() + _mintAmount < MAX_SUPPLY, "COOL SKULL CLUB : Reached the max supply!");
        require(walletWhitelist[msg.sender] + _mintAmount <= MAX_PER_WALLET, string(abi.encodePacked("COOL SKULL CLUB: You cannot mint more than ", MAX_PER_WALLET.toString(), " tokens!")));
        require(msg.value >= WL_MINT_PRICE * _mintAmount, "COOL SKULL CLUB : Not enough balance!");
        _;
    }

    function publicMint(uint256 _mintAmount) public payable mintChecked(_mintAmount) {
        ...
        require(pStatus != ProjectStatus.SoldOut, "COOL SKULL CLUB : SOLD OUT!");
    ...
    function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof)
        public payable wlChecked(_mintAmount) {
            bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
            require(!PAUSED, "COOL SKULL CLUB : This contract is paused!");
            require(pStatus == ProjectStatus.Whitelist, "COOL SKULL CLUB : Whitelist not enabled!");
            require(pStatus != ProjectStatus.SoldOut, "COOL SKULL CLUB : SOLD OUT!");
            require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "COOL SKULL CLUB : Address is not whitelisted!");
             _safeMint(_msgSender(),_mintAmount);
            walletWhitelist[msg.sender] += _mintAmount;
