...
contract EMPWR is ERC721A, Ownable, ReentrancyGuard 
{
    ...
    function setBaseURI(string memory _baseTokenURI) external onlyOwner { baseURI = _baseTokenURI; }
    ...
    function batchWhitelist(address[] calldata _users, uint256[] calldata _holdings) external onlyOwner 
    {
        ...
        for(uint256 i=0; i < size; i++)
        {
            ...
            whitelist[user] = _holdings[i];
        }
    }

    function addToWhiteList(address _addr, uint256 _amount) external onlyOwner { whitelist[_addr] = _amount; }

    function checkWhitelist(address _addr) public view returns(uint256) { return whitelist[_addr]; }
    ...
    function publicMint() public payable nonReentrant 
    {
        ...
        // ERC721A wants the amount to mint?
