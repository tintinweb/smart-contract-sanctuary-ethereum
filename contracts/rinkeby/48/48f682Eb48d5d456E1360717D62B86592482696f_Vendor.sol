...
contract Vendor is Ownable {
  ...
  function buyTokens() public payable{
    ...
    require (vendorBalance >= tokens, "Vendor doesn't have enough tokens to give out");
  ...
  function sellTokens(uint256 amount) public {
    //Validate caller has enough tokens
    uint256 tokenBalance = yourToken.balanceOf(msg.sender);
    // require (tokenBalance > amount, "Caller doesn't have enough tokens to sell");

    //Validate vendor has enough eth
    uint256 ethBalance = address(this).balance;
    uint256 ethToSend = amount/tokensPerEth;
    // require (ethBalance > ethToSend, "Vendor doesn't have enough eth to send");

    yourToken.transferFrom(msg.sender, address(this), amount);

    address(this).call{value: ethToSend}("");
