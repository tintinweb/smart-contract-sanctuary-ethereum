...
contract SomaERC777Wrapper is SomaERC777 {
    ...
    bytes32 public immutable LOCAL_RECOVER_ROLE;

    constructor(address networkAddress)
    SomaERC777(networkAddress) {
        LOCAL_RECOVER_ROLE = keccak256(abi.encodePacked(
            address(this), block.timestamp, 'SomaERC777Wrapper:local-recover-role'
        ));
    }

    function initialize(IERC20 underlyingToken, address[] memory defaultOperators) external {
...
contract SomaERC777 is ERC777Upgradeable, SecurityToken {
    ...
    bytes32 public immutable LOCAL_ADMIN_OPERATOR_ROLE;

    constructor(address networkAddress) Guardable(networkAddress) {
        LOCAL_ADMIN_OPERATOR_ROLE = keccak256(abi.encodePacked(
            address(this), _msgSender(), block.timestamp, 'SomaERC777:local-admin-operator'
        ));
    }
