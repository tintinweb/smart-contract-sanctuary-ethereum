...
contract ERC721AMock is ERC721A, EIP712Whitelisting, VRFv2Consumer {
    ...
    //     uint256 quantity,
    //     bytes memory _data
    // ) public requiresWhitelist(signature) {
    //     _safeMint(to, quantity, _data);
    // }

    // function mint(address to, uint256 quantity) public requiresWhitelist(signature) {
    //     _mint(to, quantity);
    // }
    ...
    function tokenURI(uint256 tokenId) public view virtual returns (string memory) {
        ...
        if (REVEALED) return string(abi.encodePacked(_baseURI, _toString(tokenId))); 
        return string(abi.encodePacked(_placeholderURI));
    }

    function reveal(bool status, string memory uri, string memory factor) external onlyOwner {
        REVEALED = status;
        setBaseURI(uri);
    }
    ...
    function getRandomWord() public view returns (uint256[] memory) {
        return _randomWords();
    }
}
...
contract VRFv2Consumer is VRFConsumerBaseV2, Ownable {
  ...
  function _randomWords() internal view virtual returns (uint256[] memory) {
