...
contract ArtcryptionMarketplace is Initializable, AccessControlUpgradeable {
    ...
    modifier onlyOwner(
        ...
        if (tokenAddress == address(0)) {
            ...
            require(
                tokenContract.balanceOf(_msgSender(), tokenId) > 0,
    ...
    function _registerItemForSale(
        ...
        counter ++;
        saleId = (lazyMint ||
            saleIdByAsset[_msgSender()][assetData.tokenAddress][assetData.tokenId] == 0)
            ? counter
            : saleIdByAsset[_msgSender()][assetData.tokenAddress][assetData.tokenId];
        ...
        if (!lazyMint) {
            saleIdByAsset[_msgSender()][assetData.tokenAddress][assetData.tokenId] = saleId;
        }
    }
    ...
    function _executeSale(AssetToken memory _asset, uint256 _saleId) internal {
        ...
        if (saleData[_saleId].lazyMint) {
            if(saleData[_saleId].assetToken.assetType == TokenTypes.ERC721){
                ...
                tokenId = multiNFT.create(_msgSender(), saleData[_saleId].assetToken.quantity, _asset.uri, bytes(""));
            }
