...
contract Minter is Ownable, Initializable, ReentrancyGuard {
    ...
    mapping(bytes => Data) public datas;
    ...
    function closeBorrow(address uAssetAddress, bytes memory id) external nonReentrant {
        ...
        emit CloseBorrow(msg.sender, uAssetAddress, id, uAssetAmount, collateralAmount, block.timestamp);
    }
    
    function shortBorrow(address uAssetAddress, uint256 uAssetAmount, uint256 collateralAmount, bytes memory id) external nonReentrant {
        address collateralAddress = IController(controllerAddress).collateralForToken(uAssetAddress);
        require(collateralAddress != address(0), "Collateral address is 0");
        {
            uint256 ttl = IController(controllerAddress).ttl();
        ...
        {
            editBorrow(uAssetAddress, uAssetAmount, collateralAmount, id);
        }
        {
