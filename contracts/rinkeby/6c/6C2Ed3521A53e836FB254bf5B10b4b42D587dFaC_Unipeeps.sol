...
import {UnipeepsSVG} from './UnipeepsSVG.sol';
...
contract Unipeeps is IUnipeeps, ERC721, Ownable {
    ...
    /// @notice The number of recipients should equal the number of peeps minted
    error InvalidAmountOfMintAddresses();

    /// @notice Thrown if a peep with the given ID already exists
    error PeepAlreadyExists(uint256 id);

    /// @notice The contract responsible for generating the SVG
    UnipeepsSVG public immutable svgContract;
    ...
    function newPeeps(Peep[] memory _peeps, address[] memory recipients) external onlyOwner {
      if (_peeps.length != recipients.length) revert InvalidAmountOfMintAddresses();
      unchecked {
        for (uint256 i = 0; i < _peeps.length; i++) {
            _mintPeep(i + 1, _peeps[i], recipients[i]);
        }
      }
    }

    /// @dev Shared with newPeep and constructor
    function _mintPeep(
        uint256 id,
        Peep memory peep,
        address to
    ) private {
        if (_exists(id)) revert PeepAlreadyExists(id);
    ...
    function _getSVG(uint256 tokenId) internal view returns (string memory) {
        return string(svgContract.interpolateSVG(tokenId, peeps[tokenId], totalSupply));
    }
