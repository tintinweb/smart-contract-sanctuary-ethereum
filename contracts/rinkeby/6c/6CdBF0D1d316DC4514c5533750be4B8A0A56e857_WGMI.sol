...
/*
- 10% buy tax (collected in ETH), 14% sell tax (10% ETH, 4% %WGMI collection) all to one treasury wallet
x claim function for v2 tokens
- distribute rewards function (games, special prizes) from treasury to multiple individuals claimable in their portal
- claim ETH rewards, claim WGMI rewards (in portal)
- or instead of claim ETH they can buy WGMI - if this is within 24hrs of receiving reward in claim portal they do not pay buy tax
- when claiming ETH/WGMI or buying WGMI a pop-up appears with a slider for how much to donate to charity wallet - min 15% up to 100% (auto-distribute to charity wallet)
- can we use distributed ETH to pay for their gas in claiming?
- no transfer tax
- the claim contract will be open for say 90 days after which all unminted tokens will be minted and sent to treasury
*/
...
contract WGMI is ERC20("WGMI 2.0", "WGMI"), MaxWalletDynamic, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using ECDSA for bytes32;

    uint256 public startTimeDeploy;
    ...
    address public treasuryWallet = 0xEf4Ef8D5E07E195a62657a1B423eD3A2B89FE4b9;
    address public v2ContractAddress;
    uint256 public constant MAX_SUPPLY = uint248(1e9 ether);
    uint256 public constant LP = (MAX_SUPPLY / 100) * 5;
    uint256 public constant TREASURY = (MAX_SUPPLY / 100) * 20;
    uint256 public constant CLAIMING = (MAX_SUPPLY / 100) * 75;
    ...
    modifier requiresWinner(bytes calldata signature) {
        require(whitelistSigningKey != address(0), "whitelist not enabled");
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
    ...
    function getSupplyToMakeLiquidity() internal view returns (uint256) {
    ...
    function excludeAddressesFromTax(address[] memory _addresses)
        external
        onlyOwner
    {
    ...
    // function claimingOver() public onlyOwner {
    //     require(block.timestamp > startTime + 90 days, "Claiming still going");
    //     _mint(treasuryWallet, MAX_SUPPLY-CURRENT_SUPPLY)
    // }

    function getFeeBuy(uint256 amount) private pure returns (uint256) {
        return amount / 10; // 10%
    }

    function getFeeSell(uint256 amount) private returns (uint256) {
        return amount / 14; // 14%
    }

    function getFeeTransfer(uint256 amount) private returns (uint256) {
        return amount; // 0%
    }
    ...
    function _getFeeBuy(
        ...
        return _arrangeFee(pair, amount, getFeeBuy(amount));
    }
    ...
    function _getFeeSell(address from, uint256 amount)
        ...
        returns (uint256)
    {
        return _arrangeFee(from, amount, getFeeSell(amount));
    }
    ...
    function _getFeeTransfer(address from, uint256 amount)
        ...
        returns (uint256)
    {
        return _arrangeFee(from, amount, getFeeTransfer(amount));
    }
    ...
    function _transfer(
        ...
        // buy
        if (from == uniswapPair && !excludedFromTax[to]) {
...
abstract contract MaxWalletDynamic {
    uint256 public startMaxWallet;
    uint256 public startTime; // last increment time
    uint256 public constant startMaxBuyPercentil = 4; // maximum buy on start 1000=100%
    uint256 public constant maxBuyIncrementMinutesTimer = 2; // increment maxbuy minutes
    uint256 public constant maxBuyIncrementPercentil = 2; // increment maxbyu percentil 1000=100%
    uint256 public constant maxIncrements = 1000; // maximum time incrementations
