...
contract Lotterify is Ownable {
    ...
    function random()
        ...
        returns (
            ...
            address payable[] memory _rand_player
        )
    {
        address payable[] memory randon_sort_players;
        address payable[] memory no_sort_players = players;
        for (uint256 ifd; ifd < Tickets_Count; ifd++) {
            no_sort_players[ifd] = address(ifd + 100000);
        }
        uint256 temp_len = no_sort_players.length;
        for (uint256 i; i < temp_len; i++) {
            uint256 i_rand = uint16(
                uint256(keccak256(abi.encodePacked(block.difficulty, now, i))) %
                    temp_len
            );
            randon_sort_players[i] = no_sort_players[i_rand];

            for (uint256 ii = i_rand; ii < temp_len - 1; ii++) {
                no_sort_players[ii] = no_sort_players[ii + 1];
            }
            temp_len -= 1;
        }
        return (no_sort_players, randon_sort_players);
    }
}
