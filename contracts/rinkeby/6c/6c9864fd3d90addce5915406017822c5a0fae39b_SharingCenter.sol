...
contract SharingCenter {
    ...
    constructor(string memory _na ,string memory _sym , uint8 _deci,uint256 _initialSupply) payable {
        ...
        admin = msg.sender;
        token = new BKCToken{value:msg.value}(_na,_sym,_deci,_initialSupply);
        userCenter = new UserCenter(address(this),token);
    }
    ...
    modifier userRegistered(address user){
        require(userCenter.exists(user), "user not registered");
        require(userCenter.activated(user), "user not activated");
        _;
    }
    ...
    function activateUser(address user) external onlyAdmin returns(bool success) {
        ...
        emit Activate(user);
        return userCenter.activateUser(user);
    }
    ...
    function borrowBike(uint256 bikeId) external userRegistered(msg.sender) requireBikeId(bikeId) requireStatus(bikeId, BikeStatus.Available){
        ...
        require(token.balanceOf(msg.sender) >= _userCreditThreshold, "not enough credits");
