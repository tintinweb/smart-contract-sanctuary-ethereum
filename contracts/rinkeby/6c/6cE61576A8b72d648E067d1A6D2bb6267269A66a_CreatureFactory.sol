...
*/
  ...
  uint256 public base = 0.3 ether;
  uint256 public white = 0.3 ether;
  uint256 public special = 0.3 ether;
  uint256 public boox = 0.3 ether;
  uint256 MULTI_OPTION = 8;  
  /* 
  imposta l'utente per ricevere il reward
  usertopay.push(payable(_uRev));
 
  if(whitelisted[msg.sender] == True) {
	require(msg.value >= white * _mintAmount);
  }
  */
    ...
    function name() override external pure returns (string memory) {
        return "OpenSeaFruity Item Sale";
    }
  ...
  function user_mint(uint256 _optionId, address _toAddress, uint256 _mintAmount) public payable {
    ...
    require(_mintAmount > 0);
	 ...
	 if(whitelisted[msg.sender] == true) {
		...
		if (_optionId == MULTI_OPTION) {
			require(msg.value >= special * _mintAmount);
		} else if (_optionId == LOOTBOX_OPTION) {
			require(msg.value >= boox * _mintAmount);
		} else {
			require(msg.value >= base * _mintAmount);
		}
	 }
    ...
    } else if (_optionId == MULTI_OPTION) {
  ...
  */
  ...
  function withdraw() public payable onlyOwner {
	  ...
	  //Attribuisce perc ai possessori dei token rari
      uint256 bps500 = address(this).balance / 10000 * 500;
      ...
      // The Fruity Tribe Metaverse
      (bool tftm, ) = payable(0x5B38Da6a701c568545dCfcB03FcB875f56beddC4).call{value: bps4800}("");
      require(tftm);
