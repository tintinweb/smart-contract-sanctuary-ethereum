...
contract Yard is ERC20, Authorizable {
    ...
    function _upsertFishStaking(
        ...
        totalFishStaked = totalFishStaked - previousFish + amount;
    ...
    function claimableView(address user) public view returns (uint256) {
        FishStake memory fish = fishStakeHolders[user];
        require(fish.user != address(0), "NOT STAKED");
    ...
    function swapFishForFeed(uint256 fishAmt) external {
        ...
        // supply feed...
        _mint(msg.sender, fishAmt * FEED_SWAP_FACTOR);
        emit FishSwap(msg.sender, fishAmt * FEED_SWAP_FACTOR);
    }
    ...
    function _mintYard(address sender, uint256 yardAmount) internal {
        ...
        _mint(sender, yardAmount);
        emit Minted(sender, yardAmount);
    }
    ...
    function rebalanceStakingPool(uint256 from, uint256 to) external  {
        ...
        for (uint256 i = from; i <= to; i++) {
            ...
            _upsertFishStaking(holderAddress, fish.amount);
        }
    }
}
...
contract Fish is ERC20, Authorizable {
    ...
    function claimableView(uint256 tokenId) public view returns (uint256) {
        ...
        if (captain.ton > 0) {
            uint256 fishPerDay = ((FISH_PER_DAY_PER_TON * (captain.ton / 1)) +
                BASE_HOLDER_FISH);
     ...
     */
     function levelUpChikn(uint256 tid) external {
        StakedCaptainObj memory captain = stakedCaptain[tid];
        require(captain.ton > 0, "NOT STAKED");
    ...
    function _burnFish(address sender, uint256 fishAmount) internal {
        ...
        _burn(sender, fishAmount);
        emit Burned(sender, fishAmount);
    }
     ...
     */
     function skipCoolingOff(uint256 tokenId, uint256 fishAmt) external {
        ...
        require( walletBalance >= fishAmt, "NOT ENOUGH FISH IN WALLET");

        // check: provided egg amount is enough to skip this level
        require(fishAmt >= checkSkipCoolingOffAmt(captain.ton), "NOT ENOUGH FISH TO SKIP");
        ...
        onlyAuthorized
    {
        ...
        captain.eatenAmount = uint48(yardAmount / 1e18) + captain.eatenAmount;
        stakedCaptain[tokenId] = captain;
    }
    ...
    function mintFish(address sender, uint256 amount) external onlyAuthorized {
        _mint(sender, amount);
