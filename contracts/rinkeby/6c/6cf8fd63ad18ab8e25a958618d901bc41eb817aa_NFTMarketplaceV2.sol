...
contract NFTMarketplaceV2 is NFTMarketplace, PausableUpgradeable, EIP712 {
    ...
    function buyItem(address tokenAddress, uint tokenId) public payable nonReentrant whenNotPaused override {
    ...
    function matchOrders(
    ...
    ) external payable nonReentrant whenNotPaused onlyRole(MARKETPLACE_WALLET) {
        ...
        if (takerOrder.deadline > block.timestamp || makerOrder.deadline > block.timestamp)
        {
        ...
        if (
            _hashTypedDataV4(LibOrder.hash(makerOrder)).recover(makerSignature) != makerOrder.maker ||
            _hashTypedDataV4(LibOrder.hash(takerOrder)).recover(takerSignature) != takerOrder.maker
        ) {
            revert Unauthorized();
        }

        (address nftAddress, uint tokenId) = abi.decode(makerOrder.makeAsset.data, (address, uint));
        ...
        emit Match(makerOrder, takerOrder);
    }
    ...
    function _transferERC20Tokens(
        ...
        (address tokenAddress, uint takerPrice) = abi.decode(takerOrder.makeAsset.data, (address, uint));

        // protocolFee * 2, is because in auctions we are taking the whole amount in one transaction (from buyer)
        uint theosFee = _calculateFee(takerPrice, protocolFee * 2);

        IERC20 erc20 = IERC20(tokenAddress);
        erc20.transferFrom(takerOrder.maker, theosProtocolAddress, theosFee);

        // set new price for calculations
        uint askPrice = takerPrice - theosFee;
        ...
        if (nftSupportsRoyalties) {
            ...
            if (royaltiesPercentage > 0) {
                ...
                uint royaltyValue = _calculateFee(askPrice, royaltiesPercentage);
                // transfer ERC20 token to whoever has the royalties
                erc20.transferFrom(takerOrder.maker, royalties[0].account, royaltyValue);
                // transfer ERC20 token to seller
                erc20.transferFrom(takerOrder.maker, makerOrder.maker, askPrice - royaltyValue);
            } else {
                // only pay seller
                erc20.transferFrom(takerOrder.maker, makerOrder.maker, askPrice);
            }
        } else {
            erc20.transferFrom(takerOrder.maker, makerOrder.maker, askPrice);
        }
...
interface INFTMarketplace {
    ...
    event Match(LibOrder.Order makerOrder, LibOrder.Order takerOrder);
