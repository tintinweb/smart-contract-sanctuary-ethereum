...
contract SmolFrens is Ownable, ERC721, NonblockingReceiver {
    ...
    function Whitelist(bytes32[] calldata _merkleProof, uint amount) external payable {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));

        require(status == 1, 'Whitelist Mint Not Live');
        require(amount < 11, 'You can only mint 10 or less!');
        require(nextTokenId + amount <= ETH_MINT, "Supply is capped!");
        require(tx.origin == msg.sender, 'Contract Denied');
        require(msg.value >= price * amount, 'Ether Amount Insufficient');
