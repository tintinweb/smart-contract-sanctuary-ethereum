...
contract MandoxStake is Owned {
    ...
    //MANDOX contract address
    address public mandox;
    ...
    uint public registrationTax;                   // 1000 = 1000/ 10000 = 10000
    //daily return of investment in percentage
    uint public dailyROI;                         //100 = 1% 2-0.02%
     ...
     * transfers MANDOX from sender's address into the smart contract
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(mandox).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        ...
        require(IERC20(mandox).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
     ...
     * transfers the sum of the remainder, stake rewards, referral rewards, and current earnings to the sender 
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(mandox).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(mandox).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient MANDOX balance in pool');
        ...
        IERC20(mandox).transfer(msg.sender, totalReward);
        //emit event
    ...
    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(mandox).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        //makes sure _amount is not more than required balance
        require((IERC20(mandox).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient MANDOX balance in pool');
        //transfers _amount to _address
        IERC20(mandox).transfer(msg.sender, _amount);
        //emit event
