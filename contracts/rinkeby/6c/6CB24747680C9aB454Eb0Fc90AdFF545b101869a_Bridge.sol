...
contract Bridge is AccessControl {
    ...
    function swap(address tokenAddress, address recipient, uint256 chainId, uint256 value) public {
        require(chainIds[chainId] == true, "Chain is not supported");
        string memory tokenName = IMyERC20(tokenAddress).name();
        require(tokens[tokenName] != address(0), "Token not added");
        IMyERC20(tokenAddress).burn(msg.sender, value);
