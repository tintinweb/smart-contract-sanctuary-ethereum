...
contract Contract is ERC721A, Ownable {
  ...
  uint256 public MaxToken = 500;
  uint256 public TokenIndex = 0;
  event TokenMinted(uint256 supply);
  enum Steps {Launch, FreeMint, WLMints, Sale, SoldOut}
  Steps public sellingStep;

  bytes32 public merkleRoot; //To check the addresses in the whitelist
...
///////////// Tout les mints | Mint Free -> Mint OG & Mint WL "normal" -> Public Sale

  function mintFree(uint8 mintAmount) public payable UserOnly {
    ...
    require(mintAmount <= 5, "You can not mint more than 5 NFT at the same time");
  ...
  function mintOG(uint8 mintAmount, address _account, bytes32[] calldata _proof) public payable UserOnly {
    ...
    require(mintAmount <= 10, "You can not mint more than 10 NFT at the same time");
    require(mintAmount > 0, "At least one should be minted");
  ...
  function mintPublic(uint8 mintAmount) public payable UserOnly {
  ...
  function setupWl() external onlyOwner {
...
/////////////
  ...
  function withdraw() public payable onlyOwner {
    (bool team, ) = payable(0x72AaA9AbbB639Da512C7f0d83E20d2B4c243ecDb).call{value: address(this).balance}("");
    require(team);
  }
