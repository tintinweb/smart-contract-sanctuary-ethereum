...
contract Market is ERC721, ERC721Burnable, Ownable, ReentrancyGuard, Pausable, IMarket {
    ...
    modifier onlyCollectionOwner(address _vaultAddress) {
        ...
        require(msg.sender == IVault(_vaultAddress).collectionOwner(), "onlyCollectionOwner");
        _;
    }
    ...
    function mintAndCreateList(
        ...
        require(_lockDuration == 0 || _maxRentalDuration <= _lockDuration, "maxRent.Dur>lockDur");
        require(_minRentalDuration <= _maxRentalDuration, "minRentalDur>maxRentalDur");

        require(vaultWhiteList[_vaultAddress], "Notwhitelisted");
        require(IVault(_vaultAddress).tokenIdAllowed(_tokenId), "NotAllowedId");
        require(IVault(_vaultAddress).minPrices(_paymentToken) <= _dailyRentalPrice, "dailyRent.Price<min");
        require(IVault(_vaultAddress).minDuration() <= _minRentalDuration * 1 days, "minRent.Dur<minDur");
        require(IVault(_vaultAddress).maxDuration() >= _maxRentalDuration * 1 days, "maxRent.Dur>maxDur");

        require(_isPaymentTokenAllowed(_vaultAddress, _paymentToken), "NotAllowedToken");
    ...
    function _claimFee(uint256 _lockId) internal {
    ...
    function claimProtocolFee() external onlyProtocolAdmin {
        for (uint256 i = 0; i < paymentTokens.length; i++) {
            ...
            if (paymentTokens[i] == address(0)) {
                ...
                IERC20(paymentTokens[i]).safeTransfer(msg.sender, _sendAmount);
            }
    ...
    function rent(
        ...
        require(_rentalStartTimestamp < (block.timestamp + reservationLimits), "Reserve.<10days");

        Lend memory _lend = lendRent[_lockId].lend;

        // Verify executor for private listing
        if (_lend.listType == ListType.Private) require(msg.sender == privateList[_lockId], "invalidRenter");

        _calcFee(_lend.vault, _lockId, _rentalDurationByDay, _amount);

        (uint256 _rentalStartTime, uint256 _rentalExpireTime) = CalcDuration.duration(
        ...
        _addLockId(renterLockIds, msg.sender, _lockId);

        // Pseudo Transfer WrappedNFT (if it starts later, only booking)
        IVault(_lend.vault).mintWNft(msg.sender, _rentalStartTime, _rentalExpireTime, _lockId);
    ...
    function activate(uint256 _lockId, uint256 _rentId) external {
        IVault(lendRent[_lockId].lend.vault).activate(_rentId, _lockId, msg.sender);
    }
    ...
    function tokenURI(uint256 _lockId) public view override returns (string memory) {
        ...
        // This must be changed by deploying chain
    ...
    function _calcFee(
        ...
        if (_paymentToken == address(0)) {
            // Native token
            require(msg.value >= _rentalFee, "NotEnoughValue");
            ...
            require(IERC20(_paymentToken).balanceOf(msg.sender) >= _rentalFee, "NotEnoughBalance");
            if (_lend.lockStartTime == _lend.lockExpireTime) {
                IERC20(_paymentToken).safeTransferFrom(msg.sender, address(_lend.lender), _rentalFee);
            } else {
    ...
    function _mintONft(uint256 _lockId) internal {
        ...
        if (_originalNftAddress.is721()) {
            ...
            require(msg.sender == _holder, "not owner address");
        }
        ...
        if (_originalNftAddress.is1155()) {
            ...
            uint256 _balance = IERC1155(_originalNftAddress).balanceOf(msg.sender, _tokenId);
            require(_balance >= _amount, "not enough tokens");
        }

        // NFT sent to vault (= locked)
        _safeTransferBundle(_originalNftAddress, msg.sender, _vaultAddress, _tokenId, _amount);
    ...
    function _createList(
        ...
        //for frontend
        _addLockId(lenderLockIds, msg.sender, _lockId);
    ...
    // 0: ERC721, 1: ERC1155, 2: ERC20
    /*function emergencyWithdraw(uint256 _lockId) external whenPaused onlyProtocolAdmin {
        _redeemNFT(_lockId);
        _claimFee(_lockId);
    }*/
}
