...
contract Cartoons is ERC721Enumerable, Ownable {
    ...
    constructor (bytes32 _root) ERC721("Cartoons", "TOONS"){
        baseURI = "";
    ...
    function whitelistMint(bytes32[] calldata _proof, uint256 _amt) public payable {
        require(claimed[msg.sender] + _amt < 3, "Requested Claim Amount Invalid");
    ...
    function getMintedAmount(bytes32[] calldata _proof, address _user) public view returns (uint256 amount) {
        bytes32 leaf = keccak256(abi.encodePacked(_user));
        require(MerkleProof.verify(_proof,root,leaf), "Invalid Proof");
        amount = claimed[_user];
    ...
    function mint(uint _numMints) public payable {
        require(isActive,  "Unknown Observer Public Sale Not Active");
        require(!mintFinalized,  "Unkown Observer Minting Permanently Disabled.");
        require(_tokenIds.current() + _numMints <= 9969 - _reserved,  "Requested Number of Mints Exceeds Total Cap");
        require(msg.value >= itemPrice * _numMints,  "Insufficient Payment for Requested Number of Mints");
        require(_numMints <= MINT_LIMIT,  "Requested Number of Mints Exceeds Single Transaction Cap");
        require(_numMints > 0,  "Requested Number of Mints Must be a Positive Integer");
    ...
    // Owner has 69 Unknown Observers Reserved to Mint for Giveaways, sends _amount of Unknown Observers to the _to address, onlyOwner Access
