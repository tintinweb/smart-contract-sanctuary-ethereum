...
contract MinerApePool is ERC721A, Ownable, ReentrancyGuard {
    ...
    string public baseURI = "https://dev-api.minerapepool.com/";
    ...
    mapping(uint8 => mapping(address => uint256)) private addressPresaleMintCount;
    mapping(uint => uint256) public nftRefundAmount;
    mapping(uint => address) public nftMinter;
    ...
    function refund(uint nftTokenId) external nonReentrant {
    ...
    function checkRefundability(address _address, uint nftTokenId) external view returns (uint256) {
    ...
    function setDrop(uint8 _drop) external onlyOwner {
        drop = _drop;
    }
