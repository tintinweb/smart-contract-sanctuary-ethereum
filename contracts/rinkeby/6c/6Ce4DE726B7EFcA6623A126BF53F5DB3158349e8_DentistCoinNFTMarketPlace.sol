...
// File contracts/DentistCoinNFTMarketPlace.sol
...
contract DentistCoinNFTMarketPlace is Ownable, ReentrancyGuard, Pausable {
    ...
    event FundsReceiverUpdated(address indexed owner, address indexed fundsReceiver);
    ...
    //create modifier for valid nft or if the person really owns nft if needed
    ...
    constructor(
        ...
        _nftCategories[1] = NFTCategory(1, 1, 4, 460 ether, 21000000 * 1e18);

        //Category => 2. Range => (5-44). Name => Earth. Price => 230 ether. AirdropAmount => 9 million DEN
        _nftCategories[2] = NFTCategory(2, 5, 44, 230 ether, 9000000 * 1e18);

        //Category => 3. Range => (45-99). Name => Moon. Price => 150 ether. AirdropAmount => 5 million DEN
        _nftCategories[3] = NFTCategory(3, 45, 99, 150 ether, 5000000 * 1e18);

        //Category => 4. Range => (100-199). Name => Whale. Price => 80 ether. AirdropAmount => 2 million DEN
        _nftCategories[4] = NFTCategory(4, 100, 199, 80 ether, 2000000 * 1e18);

        //Category => 5. Range => (200-309). Name => Dolphin. Price => 55 ether. AirdropAmount => 1 million DEN
        _nftCategories[5] = NFTCategory(5, 200, 309, 55 ether, 1000000 * 1e18);

        //Category => 6. Range => (310-449). Name => Seal. Price => 45 ether. AirdropAmount => 700,000 DEN
        _nftCategories[6] = NFTCategory(6, 310, 449, 45 ether, 700000 * 1e18);

        //Category => 7. Range => (450-499). Name => Mango/Coconut. Price => 36 ether. AirdropAmount => 540,000 DEN
        _nftCategories[7] = NFTCategory(7, 450, 499, 36 ether, 540000 * 1e18);

        //Category => 8. Range => (500-799). Name => Watermelon. Price => 32 ether. AirdropAmount => 480,000 DEN
        _nftCategories[8] = NFTCategory(8, 500, 799, 32 ether, 480000 * 1e18);

        //Category => 9. Range => (800-1099). Name => Pineapple. Price => 16 ether. AirdropAmount => 230,000 DEN
        _nftCategories[9] = NFTCategory(8, 800, 1099, 16 ether, 230000 * 1e18);

        //Category => 10. Range => (1100-1499). Name => Kiwi/Berries. Price => 8 ether. AirdropAmount => 100,000 DEN
        _nftCategories[10] = NFTCategory(9, 1100, 1499, 8 ether, 100000 * 1e18);

        //Category => 11. Range => (1500-2299). Name => Orange. Price => 4 ether. AirdropAmount => 46,000 DEN
        _nftCategories[11] = NFTCategory(11, 1500, 2299, 4 ether, 46000 * 1e18);

        //Category => 12. Range => (2300-3499). Name => Strawberries/Cherries. Price => 2 ether. AirdropAmount => 21,000 DEN
        _nftCategories[12] = NFTCategory(12, 2300, 3499, 2 ether, 21000 * 1e18);

        //Category => 13. Range => (3500-4799). Name => Peach/Apricot. Price => 0.6 ether. AirdropAmount => 5000 DEN
        _nftCategories[13] = NFTCategory(13, 3500, 4799, 0.6 ether, 5000 * 1e18);

        //Category => 14. Range => (4800-5399). Name => Pear. Price => 0.3 ether. AirdropAmount => 2000 DEN
        _nftCategories[14] = NFTCategory(14, 4800, 5399, 0.3 ether, 2000 * 1e18);

        //Category => 15. Range => (5400-5599). Name => Dental Chair Office 1. Price => 10 ether. AirdropAmount => 100,000 DEN
        _nftCategories[15] = NFTCategory(15, 5400, 5599, 10 ether, 100000 * 1e18);

        //Category => 16. Range => (5600-5799). Name => Dental Implant Office 2. Price => 8 ether. AirdropAmount => 80,000 DEN
        _nftCategories[16] = NFTCategory(14, 5600, 5799, 8 ether, 80000 * 1e18);

        //Category => 17. Range => (5800-6199). Name => Dental Handpiece Office 3. Price => 6 ether. AirdropAmount => 60,000 DEN
        _nftCategories[17] = NFTCategory(17, 5800, 6199, 6 ether, 60000 * 1e18);

        //Category => 18. Range => (6200-6497). Name => Tooth Brush Smile Office 4. Price => 4 ether. AirdropAmount => 40,000 DEN
        _nftCategories[18] = NFTCategory(18, 6200, 6497, 4 ether, 40000 * 1e18);

        //Category => 19. Range => (6498-6500). Name => VIP Combo. Price => 580 ether. AirdropAmount => 27 million DEN
        _nftCategories[19] = NFTCategory(19, 6498, 6500, 580 ether, 27000000 * 1e18);
    }

    function getTotalNFTsForCategory(uint256 _categoryId)
        public
        view
        ensureValidCategory(_categoryId)
        returns (uint256 totalNFTsForCategory)
    {
        NFTCategory memory nftCategory = _nftCategories[_categoryId];
        totalNFTsForCategory = nftCategory.lastTokenId - nftCategory.firstTokenId + 1;
    }

    function getRemainingNFTsForSale() external view returns (uint256 remainingNFTsForSale) {
        remainingNFTsForSale = dentistCoinNFT.TOKEN_LIMIT() - dentistCoinNFT.totalNFTsMinted();
    }
    ...
    function getCategoryNFTPrice(uint256 _categoryId)
        external
        view
        ensureValidCategory(_categoryId)
        returns (uint256 categoryNFTPrice)
    {
        categoryNFTPrice = _nftCategories[_categoryId].categoryPrice;
    }

    function getCategoryAirdropAmount(uint256 _categoryId)
        external
        view
        ensureValidCategory(_categoryId)
        returns (uint256 categoryAirdropAmount)
    {
        categoryAirdropAmount = _nftCategories[_categoryId].dentistCoinAirdropAmount;
    }
    ...
    function validateTokenIdWithCategory(uint256 _tokenId, uint256 _categoryId)
        ...
        returns (bool)
    {
        //validate and test this function
        NFTCategory memory nftCategory = _nftCategories[_categoryId];
        return
    ...
    function buyNFT(uint256 _tokenId, uint256 _categoryId)
        ...
        whenNotPaused
    {
        //check if there are any NFTs left to buy for that category
        require(
        ...
        emit NFTPurchased(_categoryId, _tokenId, msg.sender, nftCategory.categoryPrice);
    }
        ...
        ensureNonZeroAddress(address(_newFundsReceiver))
    {
        ...
        emit FundsReceiverUpdated(owner(), _newFundsReceiver);
    }

    //check if fallback functions is needed
}