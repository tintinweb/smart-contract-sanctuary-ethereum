...
contract Test is ERC721A, Ownable  {
        ...
        bool public isPublicSaleStopped;
        bool public isPrivateSale;
        uint256 WL_UNIT_PRICE = 0.4 ether;
        ...
        // Percentages for the lottery per each round
        uint8[5] public PERCENTAGES= [33,25,15,10,5];
        uint64 public REWARD= 0 ether;

        // Is the dutch auction started? (default: false)
        ...
        constructor(address _lotteryAddress, string memory _baseURI) ERC721A("Test", "TEST") {
        ...
        function setLotteryContractAddress(address _lotteryAddress) external onlyOwner {
            lotteryAddress = _lotteryAddress;
        }
        ...
        function setPublicSale() external onlyOwner {
            isPublicSaleStopped = !isPublicSaleStopped;
        }
     ...
     */
        function getPrice() public view returns(uint) {
            ...
            if(block.timestamp - startTime >= DUTCH_AUCTION_MAX_DURATION) {
                return DUTCH_PRICE_END;
            }
            else {
                return DUTCH_PRICE_START - (dutchRoundNumber * DUTCH_DROPPING_STEP);
            }
         }
    ...
    function presaleMint(
        ...
        require(isPrivateSale, "Presale is not opened yet");
        require(
            MerkleProof.verify(
                proof,
                _root,
                keccak256(abi.encode(msg.sender, tokenPrice, maxPerWallet))
            ),
            "!proof"
        );
        require(msg.value >= WL_UNIT_PRICE * quantity, "Not enought ETH");
        require(tokenPerWallet[msg.sender] + quantity < maxPerWallet, "!too many");
    ...
    function mint(uint _quantity) external payable {
        uint price = getPrice();
        require(!isPrivateSale, "Presale is still opened");
        require(initialized, "Sale has not started");
        require(!isPublicSaleStopped, "Sale stopped");
        ...
        if(getTime() == 0 && dutchRoundNumber < DUTCH_AUCTION_MAX_ROUND) {
            dutchRoundNumber += 1;
    ...
    function lottery() external {
        ...
        require(initialized, "Sale has not started");
        require(isPublicSaleStopped, "Sale isn't over");
        require(chanceToWinPerAddress[msg.sender][0] + chanceToWinPerAddress[msg.sender][1] + chanceToWinPerAddress[msg.sender][2] + chanceToWinPerAddress[msg.sender][3] + chanceToWinPerAddress[msg.sender][4] >0, "You have no ticket for the lottery" );
