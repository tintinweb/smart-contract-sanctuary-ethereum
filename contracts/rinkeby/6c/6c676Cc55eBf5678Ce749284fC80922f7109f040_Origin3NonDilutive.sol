...
contract Origin3NonDilutive is 
    ERC721A
    ,MimeticMetadata
    ,INonDilutive
{
    ...
    uint256 public warriorSupplyMinted = 0;
    uint256 public medicSupplyMinted = 0;
    uint256 public techSupplyMinted = 0;
    ...
    constructor(
        ...
        ,string memory _warriorBaseUnrevealedURI
        ,string memory _medicBaseUnrevealedURI
        ,string memory _techBaseUnrevealedURI
        ,uint256 _MAX_SUPPLY
    ) ERC721A(_name, _symbol) {
        MAX_SUPPLY = _MAX_SUPPLY;
        warriorBaseUnrevealedURI = _warriorBaseUnrevealedURI;
        medicBaseUnrevealedURI = _medicBaseUnrevealedURI;
        techBaseUnrevealedURI = _techBaseUnrevealedURI;
        _safeMint(msg.sender, 1);
        unrevealedURIMapping[0] = 3;
     ...
     */
    function whitelistMint(uint256 quantity, uint256 factionId) 
        public 
        virtual 
        payable 
    {
     ...
     *         supply is not exceeded and the proper $$ has been supplied.
     */
    function publicMint(uint256 quantity, uint256 factionId) 
        public 
        virtual 
        payable 
    {
    ...
    function minter(uint256 quantity, uint256 factionId) 
        private 
    {
        uint256 totalSupply = totalSupply();
        if(factionId == 1){
            if(warriorSupplyMinted + quantity > 3333) revert MintExceedsMaxWarriorSupply();
             _safeMint(msg.sender, quantity);
            for(uint256 i; i < quantity; i++) {
                unrevealedURIMapping[totalSupply + i] = 1;
            }
            warriorSupplyMinted += quantity;
        } else if(factionId == 2){
            if(medicSupplyMinted + quantity > 3333) revert MintExceedsMaxMedicSupply();
            _safeMint(msg.sender, quantity);
            for(uint256 i; i < quantity; i++) {
                unrevealedURIMapping[totalSupply + i] = 2;
            }
            medicSupplyMinted += quantity;
        } else if(factionId == 3){
            if(techSupplyMinted + quantity > 3333) revert MintExceedsMaxTechSupply();
            _safeMint(msg.sender, quantity);
            for(uint256 i; i < quantity; i++) {
                unrevealedURIMapping[totalSupply + i] = 3;
            }
            techSupplyMinted += quantity;
        } else {
            revert("Group data is invalid");
        }
    }

    function batchReserve(address[] memory targets, uint256[] memory quantities, uint256[] memory factionIds) 
        external 
        onlyOwner 
    {
        //Handle setting unrevealed uri here. 
        for (uint256 i = 0; i < targets.length; i++) {
            minter(quantities[i], factionIds[i]);
        }
    }
    ...
    ,Ownable
{
    ...
    string public warriorBaseUnrevealedURI;
    string public medicBaseUnrevealedURI;
    string public techBaseUnrevealedURI;

    mapping(uint256 => uint256) public unrevealedURIMapping;
        ...
        ) 
    {
        ...
        if(_tokenId >= activeGeneration.top) {
            ...
            if (factionId == 1) {
                return warriorBaseUnrevealedURI;
            } else if (factionId == 2) {
                return medicBaseUnrevealedURI;
            } else if (factionId == 3) {
                return techBaseUnrevealedURI;
            }
