...
contract Kimono is ERC165, IERC1155, IERC1155MetadataURI, Ownable {
    ...
    uint256 public constant MAX_KIMONOS_PER_USER = 2;

    mapping(address => uint256) private kimonos;
    ...
    function claimWithKimono(uint256 _id) public returns (uint256) {
        ...
        _onMintingWithKimono(msg.sender, _id);
    ...
    function canClaimWithKimono(uint256 _id, address _samurai)
        ...
        returns (bool)
    {
        return (mintedTo[_id] == _samurai && !mintedWithKimono[_samurai]);
    }
    ...
    function onMintingWithKimono(address _samurai, uint256 _id)
        public
        onlyArmour
    {
        _onMintingWithKimono(_samurai, _id);
    }

    function _onMintingWithKimono(address _samurai, uint256 _id) private {
        require(
            !mintedWithKimono[_samurai],
            "Samurai can claim with kimono only once"
        );
        require(mintedTo[_id] == _samurai, "Kimono: reward only for minter");
    ...
    function _safeTransferFrom(
        ...
        if (amount == 1) {
            _owners[id] = to;
            kimonos[to] = kimonos[to] + 1;
            kimonos[from] = kimonos[from] - 1;
        }
        require(
            kimonos[to] <= MAX_KIMONOS_PER_USER || isNull[to],
    ...
    function _safeBatchTransferFrom(
        ...
        kimonos[to] = kimonos[to] + transferedKimonos;
        kimonos[from] = kimonos[from] - transferedKimonos;
    ...
    function _mint(
        ...
        require(
            kimonos[to] <= MAX_KIMONOS_PER_USER,
            "Max Kimonos per Samurai reached"
        );
