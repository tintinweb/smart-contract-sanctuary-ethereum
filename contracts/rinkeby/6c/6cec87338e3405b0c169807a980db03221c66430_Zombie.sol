...
// File: contracts/aug20_zombie.sol
...
// File: contracts/Zombieverse.sol
...
contract Zombie is ERC721Enumerable, Ownable, RandomlyAssigned, PaymentSplitter  {
  using Strings for uint256;
  ...
  uint256 public maxPublicSale = maxSupply - maxPresale;
  bool public paused = false;
  bool public onlyWhitelisted = false;
  bool public revealed = false;
  mapping(address => bool) public whitelisted;
  mapping(address => bool) public presaleUsedByWhitelisted;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(!onlyWhitelisted,"Public Sale is not yet started");
    ...
    if (msg.sender != owner()) {
    require(msg.value >= publicSalecost * _mintAmount,"you are sending less ETH than required");
    ...
    require(!paused,"Minting is currently paused");
    uint256 supply = totalSupply();
    require(supply + 1 <= maxSupply,"minting amount is exceeding total supply");
    ...
    require(!presaleUsedByWhitelisted[msg.sender],"presale buy limit exceeded. Buy from public sale");
    require(msg.value >= preSalecost,"you are sending less ETH than required");
       
    }
     uint256 nextId = nextToken();
    _safeMint(msg.sender, nextId);
    presaleUsed = presaleUsed+1;
    presaleUsedByWhitelisted[msg.sender]=true;
  ...
  //only owner
  function setCost(uint256 _preSalecost, uint256 _publicSalecost) public onlyOwner() {
  ...
  function setmaxPresale(uint256 _maxPresale) public onlyOwner() {
    ...
    maxPublicSale = maxSupply - maxPresale;
