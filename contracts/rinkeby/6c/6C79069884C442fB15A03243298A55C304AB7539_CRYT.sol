...
contract CRYT is ERC721, Ownable, ReentrancyGuard {
    bytes32 whroot;
    bytes32 raroot;
    ...
    uint256 public maxminteditems = 100;
...
constructor() ERC721("Cryptomus Test II", "CRYT"){}
	...
	if ( publicmintopen == 0 )
	{
	    ...
	    if ( whitelistmintopen == 1 && cHa == whroot ) st=1;
	    if ( rafflemintopen == 1 && cHa == raroot ) st=1;
	    require( st==1, "You are not eligible for Whitelist or Raffle mint!" );
	}
        st=calculateq(msg.value);
        require(mintedq[msg.sender]+st<=maxminteditems,"This would exceed the maximum NFTs/address!");
        mintedq[msg.sender]+=st;
        for (uint256 a = 0; a < st; a++) 
	{
	    currentTokenId+=1;
    ...
    function setmaxminteditem(uint256 _p) external onlyOwner 
    {
	maxminteditems=_p;
    }
