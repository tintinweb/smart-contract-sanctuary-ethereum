...
contract SaoGou is ERC721A, Ownable {
    ...
    uint8 public constant MaxPerTransaction = 15;
    uint16 public constant MaxFreeTokens = 30;
    uint16 public constant MAX_SUPPLY = 200;
    uint256 public PublicPrice = 0.001 ether;

    string private _baseTokenURI;
    
    constructor() ERC721A("SaoGou", "SG", MaxPerTransaction, MAX_SUPPLY) {
    ...
    function mint(uint256 numTokens) external payable {
        ...
        require(totalSupply() + numTokens <= MAX_SUPPLY, "Purchase more than max supply");

        if (totalSupply() >= MaxFreeTokens)
            require(msg.value >= numTokens * PublicPrice, "Ether too low");
