 ...
 *
 *          - Chronium is minted using time x rate and timestamping is based on block.number.
 *          - Because time is non-fungible across accounts, this contract allow time to be transferred into another accounts' "sub"-account.
 *            This allows an owner to reserve a 'sub'-account's time.
 *
 */

contract Chronium is ERC20, Ownable, IERC677 {
    ...
    /*
    *  EIP677: https://github.com/ethereum/EIPs/issues/677
    */
    event LogTransferAndCall(address From, address To, uint Value, bytes Data);
    function transferAndCall(address _to, uint _value, bytes memory _data)
    external override
    returns (bool success)
    {
        address sender = _msgSender();
        require(timestamp[sender]>0, "Chronium: no time account");
        ...
        if (_to.isContract()) {
            ...
            {
                emit LogTransferAndCall(sender,_to,_value,_data);
            }
            catch(bytes memory reason) {
                if (reason.length == 0) 
                    revert("Chronium: onTokenTransfer failed");
                assembly 
                {
    ...
    */
    ...
    {
        require(timestamp[timeOwner]>0, "Chronium: no time account");
    ...
    /**
     * Time is decreased by moving the timestamp forward.
     */
    function _decreaseTime(address sender, uint256 time)
    internal
    {
        uint256 timeBalance = checkTimeBalance(sender);
        require(timeBalance >= time, "Chronium: not enough time");
    ...
    internal
    {
        require (time <= timestamp[sender],"Chronium: not enough blocks.");
        timestamp[sender] -= time;
    }
    ...
    external virtual returns (bool) {
        require(transferrableTimeBalances[_msgSender()][originator] >= amount,"Chronium: Originator not enough time.");
...
// interfaces/IERC677.sol
// SPDX-License-Identifier: MIT
pragma solidity = 0.8.8;

/*
 *  IERC677: https://github.com/ethereum/EIPs/issues/677
 */
interface IERC677 {
...
/*
 *  IERC677Receiver: https://github.com/ethereum/EIPs/issues/677
 */
