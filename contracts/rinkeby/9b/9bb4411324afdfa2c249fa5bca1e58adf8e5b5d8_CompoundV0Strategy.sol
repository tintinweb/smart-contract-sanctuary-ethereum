...
contract CompoundV0Strategy is CompoundStrategy {
    ...
    function _bonusPerBlockPerUnderlying(uint256, bool) internal pure override returns (uint256) {
        return 0;
    }
}
...
contract CompoundStrategy is BaseRhoStrategy {
    ...
    /**
     * @dev calculates the COMP bonus (in terms of USD) per block per underlying as follows:
     * NOTE: The steps are packed in one line to retain precision.
     * 1. compPerBlockPerCtoken = (unitroller.compSpeeds(address(cToken)) * 10**cToken.decimals()) / cToken.totalSupply();
     * 2. exchangeRate = (cToken.exchangeRateStored() * 10**cToken.decimals()) / 10**underlying.decimals();
     * 3. compPerBlockPerUnderlying = (compPerBlockPerCtoken * 1e18) / exchangeRate;
     * 4. usdPerBlockPerUnderlying = (compPerBlockPerUnderlying * compUsdPriceOracle.price(address(comp))) / 10**compUsdPriceOracle.decimals();
     */
    function _bonusPerBlockPerUnderlying(uint256 delta, bool isPositive)
        internal
        view
        virtual
        returns (uint256 usdPerBlockPerUnderlying)
    {
        if (address(compUsdPriceOracle) == address(0)) return 0;
        uint256 newSupply = isPositive
            ? cToken.exchangeRateStored() * cToken.totalSupply() + delta * 1e18
            : cToken.exchangeRateStored() * cToken.totalSupply() - delta * 1e18;
        return
            (comptroller.compSpeeds(address(cToken)) *
                compUsdPriceOracle.price(address(comp)) *
                10**(18 + underlying.decimals() - compUsdPriceOracle.decimals())) / newSupply;
    }

    function bonusRatePerBlock() external view override returns (uint256) {
        return _bonusPerBlockPerUnderlying(0, true);
    }

    function bonusSupplyRate() external view override returns (uint256) {
        return _bonusPerBlockPerUnderlying(0, true) * BLOCK_PER_YEAR;
    }

    function effectiveSupplyRate() external view override returns (uint256 _amount) {
        return (cToken.supplyRatePerBlock() + _bonusPerBlockPerUnderlying(0, true)) * BLOCK_PER_YEAR;
    }
    ...
    function withdrawAllCashAvailableInternal() internal override {
        ...
        if (balance > withdrawable) {
            ...
            require(
                cToken.redeemUnderlying{gas: 300000}(withdrawable) == 0,
        ...
        require(
            cToken.redeem{gas: 300000}(cToken.balanceOf(address(this))) == 0,
            "CompoundStrategy: Fail to withdraw from compound"
        );
    ...
    function collectRewardTokens(uint256 rewardCollectThreshold) external override onlyRole(VAULT_ROLE) nonReentrant {
        if (_shouldCollectReward(rewardCollectThreshold)) {
            if (address(comp) != address(0) && address(tokenExchange) != address(0)) {
                ...
                if (balance > 0) {
                    comp.safeIncreaseAllowance(address(tokenExchange), balance);
                    tokenExchange.sellExactInput(comp, underlying, _msgSender(), balance);
                }
            }
        }
    }

    function bonusTokens() external view override returns (address[] memory) {
        address[] memory _addr = new address[](1);
        _addr[0] = address(comp);
        return _addr;
    }
    ...
    function _shouldCollectReward(uint256 rewardCollectThreshold) internal view returns (bool) {
    ...
    function shouldCollectReward(uint256 rewardCollectThreshold) external view override returns (bool) {
        return _shouldCollectReward(rewardCollectThreshold);
    }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function bonusTokens() external view virtual override returns (address[] memory) {
    ...
    function bonusTokensAccrued(address) external view virtual override returns (uint256) {
        return this.bonusTokensAccrued();
    }
