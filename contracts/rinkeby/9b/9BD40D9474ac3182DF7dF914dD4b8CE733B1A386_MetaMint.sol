...
contract MetaMint is ERC721, Ownable {
  ...
  uint256 public teamClaimStartAmount = 3001;
  uint256 constant public wlAllowance = 1;
  uint256 private checkpoint = 1;

  EnumerableSet.UintSet private unclaimedTokens;
  ...
  // Payment Addresses
  address constant dao = 0x29ED72142baA17f8862039cB702eFF93b6ABA62B;
  address constant recipient = 0x29ED72142baA17f8862039cB702eFF93b6ABA62B;
  address genesisAddr;

  

  constructor(address _genesisAddr) ERC721("Meta Mint Rental", "MM") {
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable {
    ...
    uint256 mintIndex = _tokenId.current();
  ...
  function publicMint() external payable {
    ...
    uint256 mintIndex = _tokenId.current();
    _mint(address(this), mintIndex);
    IERC721 token = IERC721(address(this));
    token.setApprovalForAll(address(this), true);
    token.transferFrom(address(this), msg.sender, mintIndex);
  ...
  function revoke(uint256 _tokenId) external {
    IERC721 token = IERC721(address(this));
    token.transferFrom(token.ownerOf(_tokenId), msg.sender, _tokenId);
  } 
