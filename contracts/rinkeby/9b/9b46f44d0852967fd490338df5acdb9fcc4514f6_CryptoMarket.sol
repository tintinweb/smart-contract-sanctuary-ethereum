...
contract CryptoMarket is ReentrancyGuard {
  ...
  event create_market_items_for_erc721(uint256 ,string ,string ,string  , address ,uint );

  event create_market_items_for_erc1155(uint256 ,string ,string ,string  , address ,uint,uint);

  event create_market_sale(uint, uint256 , address,address);
  ...
  event bid(uint,uint256,address);

  event burn_token(uint);

  event cancel_sale(uint);

  event title_change(uint,string);

  event description_change(uint,string);



  function create_market_items_erc721(uint256 tokenId ,string memory URI,string memory title,string memory description, address nftContract,uint royalities) public  {

    increment();

    markettokens[n] = MarketToken(n,tokenId,URI,title,description,nftContract,payable(msg.sender),payable(msg.sender),payable(0x00),0,royalities,0,payable(0x00),false,false,true,false);

    emit create_market_items_for_erc721(tokenId ,URI,title, description, nftContract,royalities);
  ...
  function createMarketSale_erc721(uint256 itemId) public payable nonReentrant{
    ...
    uint256 price = markettokens[itemId].price * 1000000000000000000;
    ...
    if(markettokens[itemId].is_fixed==true){
      ...
      markettokens[itemId].owner.transfer(price);

      markettokens[itemId].creater.transfer(price*royality);
  ...
  function create_market_items_erc1155(uint256 tokenId ,string memory URI,string memory title,string memory description, address nftContract,uint royalities,uint copies) public  {
    ...
    for(uint i=0;copies>i;i++){
      ...
      markettokens[n] = MarketToken(n,tokenId,URI,title,description, nftContract,payable(msg.sender),payable(msg.sender),payable(0x00),0,royalities,0,payable(0x00),false,false,false,false);

    }

    emit create_market_items_for_erc1155(tokenId ,URI,title, description, nftContract,royalities,copies);
  ...
  function edit_title(uint256 itemId,string memory title) public{

      require(msg.sender == markettokens[itemId].owner,'your are not owner of token');

      markettokens[itemId].title = title;

      emit title_change(itemId,title);

  }



  function edit_description(uint256 itemId,string memory description) public{

      require(msg.sender == markettokens[itemId].owner,'your are not owner of token');

      markettokens[itemId].description = description;

      emit description_change(itemId,description);
