...
import "./structs/DeploymentConfig.sol";
import "./structs/RuntimeConfig.sol";
import "./structs/ContractInfo.sol";

import "hardhat/console.sol";

contract NFTCollection is ERC721, Pausable, AccessControl {
    event OwnershipTransferred(
        address indexed previousOwner, 
        address indexed newOwner);

    string public constant VERSION = "0.0.4";
    bytes32 public constant SECUTIRY_ROLE_ADMIN = keccak256("SECUTIRY_ROLE_ADMIN");
    bytes32 public constant SECURITY_ROLE_PAUSER = keccak256("SECURITY_ROLE_PAUSER");

    uint256 public totalSupply;

    constructor(
        DeploymentConfig memory deploymentConfig,
        RuntimeConfig memory runtimeConfig) ERC721("", "") {
        _validateDeploymentConfig(deploymentConfig);

        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(SECUTIRY_ROLE_ADMIN, msg.sender);
        _grantRole(SECURITY_ROLE_PAUSER, msg.sender);
    ...
    function updateConfig(RuntimeConfig calldata newConfig)
        external
        onlyRole(SECUTIRY_ROLE_ADMIN)
    {
        _validateRuntimeConfig(newConfig);
        _runtimeConfig = newConfig;
    }

    function withdrawFees() external onlyRole(SECUTIRY_ROLE_ADMIN) {
        // _deploymentConfig.treasuryAddress.sendValue(address(this).balance);
        Address.sendValue(
            _deploymentConfig.treasuryAddress, 
            address(this).balance);
    }

    function pause() public onlyRole(SECURITY_ROLE_PAUSER) {
        _pause();
    }

    function unpause() public onlyRole(SECURITY_ROLE_PAUSER) {
        _unpause();
    }
    ...
    function _mintTokens(address to, uint256 amount) internal {
        require(
            ...
            "Maximum amount per transaction exceeded");
        require(
            msg.value >= _deploymentConfig.mintPrice * amount,
            "Payment too small");
        ...
        returns (string memory)
    {
        ...
        return totalSupply == _deploymentConfig.maxSupply && bytes(_runtimeConfig.baseURI).length > 0 ? 
            string(abi.encodePacked(
                // _runtimeConfig.baseURI, tokenId.toString())) : 
                _runtimeConfig.baseURI, "/", Strings.toString(tokenId))) : 
