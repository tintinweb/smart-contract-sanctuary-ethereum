...
contract EthereumClockToken is IEthereumClockToken, Raffle
{
     ...
     */
    constructor(string memory _pendingURI, address _chainLinkVRFCoordinator, address _linkTokenAddress) ERC721("Ethereum Clock Token", "Eth-Clock") public Raffle(_chainLinkVRFCoordinator, _linkTokenAddress) {
    ...
    function drop() public payable returns (uint256) {
        ...
        tokenIdList[newItemId] = environmentIndex + shineIndex * _ENVIRONMENT_PROBABILITY_.length + efficiencyIndex * _ENVIRONMENT_PROBABILITY_.length * _SHINE_PROBABILITY_.length;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (!_REVEAL_ALLOWED_) {
            ...
            uint256 tokenIpfsId = frozen[tokenId] ? (tokenIdList[tokenId] + _COUNT_PER_LEVEL_) : (charred[tokenId] ? tokenIdList[tokenId] + _COUNT_PER_LEVEL_ * 2 : tokenIdList[tokenId]);
            tokenIpfsId = tokenIpfsId + (levels[tokenId] - 1) * _COUNT_PER_LEVEL_ * 3;
            return string(
                abi.encodePacked(
                    currentBaseURI,
                    tokenIdList[tokenIpfsId].toString()
                )
            );
        }
    }
...
abstract contract Raffle is Admin, VRFConsumerBase {
    ...
    function raffle() external onlyOwner returns (bytes32) {
        ...
        bytes32 requestId = requestRandomness(_CHAIN_LINK_KEY_HASH_, _CHAIN_LINK_FEE_);
