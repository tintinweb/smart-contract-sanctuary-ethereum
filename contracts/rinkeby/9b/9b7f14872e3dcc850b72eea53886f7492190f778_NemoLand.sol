...
contract NemoLand is Admin, ERC721Upgradeable {
    ...
    mapping(uint256 => uint256[]) private referralToIds;

    event Referral(uint256 indexed referral, uint256 indexed id);
    event DisallowBuy(address indexed owner, uint256 indexed id);
    event ZonePrice(uint256 indexed zone, uint256 indexed weiAmount);
    event Resale(
        uint256 indexed id,
     ...
     * @param weiAmount new Price of zone
     */
    function setSalePrice(uint256 _zoneId, uint256 weiAmount)
        external
        onlyAdmin
    {
        zoneIdToPrice[_zoneId] = weiAmount;
        emit ZonePrice(_zoneId, weiAmount);
    }
    ...
    function mintLand(
        ...
        for (counter = 0; counter < _id.length; counter++) {
            ...
            if (referral > 0) {
                ...
                emit Referral(referral, _id[counter]);
            }
        }
    }
    ...
    function allowBuy(uint256 _id, uint256 weiAmount) external {
        ...
        tokenIdToPrice[_id] = OnSaleLand(weiAmount, msg.sender);
    ...
    function setResaleCom(uint256 resaleCom_) external onlyAdmin {
        require(resaleCom_ > 0, "Invalid percent");
    ...
    function getReferredTokens(uint256 referral)
        public
        view
    ...
    function contractURI() public pure returns (string memory) {
        return "https://run.mocky.io/v3/b1742f86-0e7a-4b19-9491-c232d86d5194";
    }
