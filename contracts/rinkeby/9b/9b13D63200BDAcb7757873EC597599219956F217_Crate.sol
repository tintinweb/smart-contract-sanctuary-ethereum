...
/////////////////////////////
    ...
    function RegisterRangeOfNftIds(uint256 lower, uint256 upper, address nftContractAddress) public {
        for (uint i = lower; i <= upper; i++){
            RegisterNftInContract(i, nftContractAddress, 123123123123);
        }
    }
    ...
    function RegisterNftInContract(uint256 nftId, address nftContractAddress, uint256 buyItNowNativePrice) public {
        ...
        for (uint256 i = 0; i < addyArray.length; i++){         //for each slot in vending machine
            if (nftContractAddress == addyArray[i]){            //if supplied address matches up with a vending slot
                ...
                tempStruct.tokenId = nftId;
                tempStruct.index   = q[addyArray[i]].length+1;
    ...
    function getAllRegisteredForSlot(uint256 slotIndex) public view returns (slotInhabitant[] memory, string[] memory){
        ...
        return( q[addyArray[slotIndex]], tokenUriArray);
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override  {
        ...
        for (uint256 i = 0; i < oddsArray.length; i++){
            ...
            // bool tripped = false;
            // for (uint z = 0; z < q[addyArray[slotWinner]].length; z++){
            //     if (q[addyArray[slotWinner]][z].index == 1){
            //         tripped = true;
            //     }
            // }
            // if (tripped == false ) {
        ...
        uint lastSlot = q[addyArray[slotWinner] ].length - 1;
        
        for (uint r = lastSlot; r> 0; r--){
