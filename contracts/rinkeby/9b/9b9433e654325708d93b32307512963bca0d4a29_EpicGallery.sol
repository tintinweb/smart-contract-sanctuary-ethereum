...
contract EpicGallery is ERC721 {
    ...
    function airdrop(address[] memory airdropAddress) external onlyOwner {
        ...
        for (uint256 k = 0; k < airdropAddress.length; k++) {
            if (!_exists(currentID)) {
                _safeMint(airdropAddress[k], currentID);
                Items[currentID] = Item(currentID, msg.sender, airdropAddress[k], "", 0, PRICE);
    ...
    function setRoyalty(uint256 _tokenId, uint256 royalty) public creatorOnly(_tokenId){
        Items[_tokenId].royalty = royalty;
    }
