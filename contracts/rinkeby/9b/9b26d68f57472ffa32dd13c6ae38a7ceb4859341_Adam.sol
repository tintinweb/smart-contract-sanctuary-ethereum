...
contract Adam is Initializable, UUPSUpgradeable, ILayerZeroReceiver {
    ...
    function initialize(
        ...
        lzEndpoint = ILayerZeroEndpoint(0x79a63d6d8BBD5c6dfc774dA79bCcD948EAcb53FA);
    }
    ...
    function crossChain(bytes memory _payload) public {
        (uint256 messageFee, ) = lzEndpoint.estimateFees(10001, address(this), _payload, false, bytes(""));

        lzEndpoint.send{value: messageFee}(
            10001,               // destination LayerZero chainId
            trustedRemoteLookup[10001],  // send to this address on the destination          
    ...
    function lzReceive(uint16 _srcChainId, bytes calldata _srcAddress, uint64 _nonce, bytes calldata _payload) override external {
        ...
        require(_srcChainId == 10002, "Not desired chain");

        string memory target;
        (target) = abi.decode(_payload, (string));

        if (keccak256(abi.encodePacked(("totalDaos"))) == keccak256(abi.encodePacked((target)))) {
            this.crossChain(abi.encodePacked("setTotalDaos", daos.length));
        }
    }
}
