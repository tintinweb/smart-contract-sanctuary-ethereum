...
contract ParrotPFP is ERC721A, Ownable {

    ParrotPassContract public PASS;

    uint256 public constant MAX_PARROTS = 12000;
    uint256 public constant MAX_PAID_PARROTS = 3033;
    uint256 public constant MAX_FREE_PARROTS = 8888;
    uint256 public constant MAX_RESERVED_PARROTS = 79;
    ...
    function mint(uint256 numParrots) external payable {
        ...
        require(
            ...
            'Not enough Parrots left for minting, try reducing the amount you want.'
        );
        require(
            paidParrotsMinted + numParrots <= MAX_PAID_PARROTS,
            'Not enough paid parrots left for minting.'
        );
        require(
    ...
    function freeClaim(uint256 passId) external {
        require(tx.origin == msg.sender, 'Cant be called from another contract');
        require(!mintPaused, 'Free Parrot minting paused.');
        require(!claimedParrots[passId], 'Pass has already claimed a parrot.');
        require(passId < MAX_FREE_PARROTS, 'Invalid Pass ID');
        require(
            PASS.ownerOf(passId) == msg.sender,
            'Need to own the pass youre claiming a parrot PFP for.'
        );
        require(
        ...
        require(
            freeParrotsMinted + 1 <= MAX_FREE_PARROTS,
            'Not enough free passes left for minting.'
        );
        freeParrotsMinted++;
    ...
    function freeClaimMultiple(uint256[] calldata passIds) external {
    ...
    function isClaimed(uint256 passTokenId) external view returns (bool) {
        require(passTokenId < MAX_FREE_PARROTS, 'Invalid Pass ID');
