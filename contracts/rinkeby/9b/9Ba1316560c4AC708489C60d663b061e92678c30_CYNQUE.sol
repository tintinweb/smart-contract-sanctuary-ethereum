...
contract CYNQUE is ERC721, Ownable{
    ...
    {
        collectionSize = 1111;
        duration = 2640; //~11 hours
        startBlock = startBlock_;
        stepDuration = 60; //~15 minutes
        startPrice = 0;
        floorPrice = 0;
        priceDeltaUp = 0.11 ether;
        priceDeltaDown = 0.0275 ether;
        auctionId = 0;

        expectedStepMintRate = 2;
...
*/
    function mintCynqueWithPassports(
        ...
        if(passportIds.length > 4) revert MaxPassportMintExceeded();
    ...
    function mintCynqueWithNFTPAY(uint256 _quantity) external payable {
        require(msg.sender == nftPay, "Only NFTpay!");
        if (_quantity < 1 || _quantity > 4) revert IncorrectQuantity();
        if (quantityOfPublicMinted[msg.sender] + _quantity > 4)
            revert MaxMintedOnPublicSale();
...
*/
    ...
    function setNftpayAddress(address _nftpayAddress) external onlyOwner {
        nftPay = _nftpayAddress;
    }
    ...
    function restartAuction(
        ...
        expectedStepMintRate = mintRate_;
