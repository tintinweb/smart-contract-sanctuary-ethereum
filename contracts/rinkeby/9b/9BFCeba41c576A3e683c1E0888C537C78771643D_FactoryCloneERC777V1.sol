...
contract FactoryCloneERC777V1 {
    address immutable tokenImplementation;

    constructor() {
        tokenImplementation = address(new ERC777PresetFixedSupplyUpgradeable());
    }

    function createToken(string calldata name, string calldata symbol, address[] memory defaultOperators, uint256 initialSupply, address owner) external returns (address) {
        address clone = Clones.clone(tokenImplementation);
        ERC777PresetFixedSupplyUpgradeable(clone).initialize(name, symbol, defaultOperators, initialSupply, owner);
        return clone;
    }
}
