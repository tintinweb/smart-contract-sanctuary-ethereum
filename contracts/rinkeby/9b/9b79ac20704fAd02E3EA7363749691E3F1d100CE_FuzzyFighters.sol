...
contract FuzzyFighters is ERC721A, Ownable, ReentrancyGuard {
    ...
    // ===== Variables =====
    address public constant DISBURSEMENT_WALLET = 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266;
    ...
    uint256 public constant PRESALE_MAX_QTY = 1500;
    ...
    function presaleMint(bytes32[] memory proof,uint quantity,uint allowance) external payable onlySender nonReentrant {
        require(status == Status.PreSale, "The waitlist sale not active.");
        require(_verify(whitelistMerkleRoot,_leaf(msg.sender, allowance), proof), "Invalid merkle proof.");
    ...
    function publicMint(uint quantity) external payable onlySender nonReentrant {
        require(status == Status.PublicSale, "The public sale not active.");
        require(preSalesMintedQty + publicSalesMintedQty + quantity  <= SALES_MAX_QTY, "Minting exceed the sale allocation.");
    ...
    function reservedMint(bytes32[] memory proof) external onlySender nonReentrant {
        require(status == Status.ReservedSale, "The free mint not active.");
        require(_verify(reservedListMerkleRoot,_leafReserved(msg.sender), proof), "Invalid merkle proof.");
    ...
    function gift(address recipient,uint quantity) external payable onlyOwner nonReentrant {
        require(
    ...
    function setReservedListMerkleRoott(bytes32 _reservedListMerkleRoot) external onlyOwner{
        reservedListMerkleRoot = _reservedListMerkleRoot;
    }
