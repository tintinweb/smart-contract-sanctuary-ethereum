...
contract PrimeApe is ERC721Upgradeable, OwnableUpgradeable {
    ...
    uint64 public constant MINT_PRICE = 20000000000000000; // 0.02 ETH
    uint64 public constant MINT_PRICE_WHITELIST = 10000000000000000; // 0.01 ETH

    // always add trailing / at the end
    string private _base;
    string private _contractURI;

    bool public whitelistOngoing;
    ...
    function initialize() public initializer {
        ERC721Upgradeable.__ERC721_init("Prime Apes v164w", "PAP");
        OwnableUpgradeable.__Ownable_init();

        whitelistOngoing = false;
        mintIsActive = false;
        totalSupply = 0;
    }
    ...
    function mintApeWhitelist() public payable isWhitelisted {
        require(
            whitelistOngoing,
            "Currently not for open for whitelist minting"
        );
        require(
            totalSupply + 1 <= MAX_WHITELIST_APES,
            "Purchase would exceed max supply of whitelist apes!"
        );
        require(
            msg.value >= MINT_PRICE_WHITELIST,
    ...
    function flipWhitelistState() public onlyOwner {
        whitelistOngoing = !whitelistOngoing;
    }
