    ...
    OwnableUpgradeable
{
    ...
    /**
      @param recipients address to set approved minting status for
      @dev Sets the approved minting status of the given address.
           This requires that msg.sender is the owner of the given edition id.
           If the ZeroAddress (address(0x0)) is set as a minter,
             anyone will be allowed to mint.
           This setup is similar to setApprovalForAll in the ERC721 spec.
     */
    function setApprovedMinters(address[] memory recipients) public onlyOwner {
        // allowedMinters[minter] = true;
        for (uint i = 0; i < recipients.length; i++) {
            allowedMinters[recipients[i]] = true;
        }
    }
    ...
    function purchase() external payable returns (uint256) {
        ...
        require(balanceOf(msg.sender) < 1 || msg.sender == owner(), "Limit one badge per address");
    ...
    function mintEdition(address to) external override returns (uint256) {
        ...
        // Remove non-owners from minters list once they've made a purchase
        if (owner() != msg.sender) {
            allowedMinters[msg.sender] = false;
        }
