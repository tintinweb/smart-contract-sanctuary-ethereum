    ...
    ERC2771ContextUpgradeable
{
    ...
    function safeMint(address to, uint256 ID) public payable {
        ...
        require(IERC20Upgradeable(discountTokenAddress).transfer(address(this), AVAILABLE_NFTS[ID].token_reward), "the transfer failed");
    }
    ...
    function depositToken(uint256 amount) public {
        require(
            IERC20Upgradeable(discountTokenAddress).balanceOf(msg.sender) >=
                amount,
            "Your token amount must be greater then you are trying to deposit"
        );
        require(
            IERC20Upgradeable(discountTokenAddress).approve(
                address(this),
                amount
            )
        );
