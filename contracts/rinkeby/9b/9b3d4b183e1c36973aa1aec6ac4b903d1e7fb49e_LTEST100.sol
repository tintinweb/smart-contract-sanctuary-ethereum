...
contract LTEST100 is ERC721, Ownable {using Strings for uint256; using Counters for Counters.Counter; Counters.Counter private supply;
  ...
  uint256 public maxFreeMintAmountPerTx = 5;

  bool public paused = false; bool public revealed = false;

  constructor() ERC721("LTEST100", "LT100") {setHiddenMetadataUri("ipfs://QmWrX4cCtnQZPbDDVH1WMibnLYQNyW68WrEVHeAPF7u8VP/hidden.json");}
  modifier mintCompliance(uint256 _mintAmount) {require(_mintAmount > 0 && _mintAmount <= maxMintAmountPerTx, "Invalid mint amount!"); require(supply.current() + _mintAmount <= maxSupply, "Max supply exceeded!");_;}
  modifier mintPriceCompliance(uint256 _mintAmount) {require(msg.value >= cost * _mintAmount, 'Insufficient funds!'); _;}
  modifier freeMintsAvailable() {require(totalSupply() <= NUM_FREE_MINTS, "Not enough free mints remain");_;}
  function setNumFreeMints(uint256 _numfreemints) external onlyOwner {NUM_FREE_MINTS = _numfreemints;}
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) freeMintsAvailable(){
    require(!paused, "The contract is paused!"); require(msg.value >= cost * _mintAmount, "Insufficient funds!"); _mintLoop(msg.sender, _mintAmount);}
  ...
  function setMaxFreeMintAmountPerTx(uint256 _maxFreeMintAmountPerTx) public onlyOwner {maxFreeMintAmountPerTx = _maxFreeMintAmountPerTx;}
