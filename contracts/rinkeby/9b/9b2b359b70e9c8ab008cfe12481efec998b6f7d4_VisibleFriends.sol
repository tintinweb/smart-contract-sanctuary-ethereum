...
contract VisibleFriends is ERC721SmallFriend, EIP712, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public _closeFriendsSummoned;
    ...
    function summonCloseFriends(uint256 amount) external nonReentrant {
        ...
        require(
            _closeFriendsSummoned[msg.sender] + amount <=
                config.closeFriendSummonLimit,
            "VisibleFriends: Can't summon more than limit"
        );
        uint256 totalMinted = _totalMinted();
        require(
            totalMinted + amount <= config.fixedFreeMintSupply,
    ...
    function summonFriends(uint256 amount) external payable nonReentrant {
        ...
        uint256 totalMinted = _totalMinted();
        require(
            totalMinted >= config.fixedFreeMintSupply,
            "VisibleFriends: we are waiting close friends"
        );
        require(
