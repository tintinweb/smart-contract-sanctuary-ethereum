...
contract dailyShroomClaimer is Ownable {
    ...
    event RequestProcessed(uint256 _amount,address _user,uint256 _requestId,uint8 _v,bytes32 _r,bytes32 _s,bool _status);

    function validateSignature(uint256 _amount,address _user,uint256 _requestId,uint8 _v,bytes32 _r,bytes32 _s) public  {
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        bytes32 prefixedHashMessage = keccak256(abi.encodePacked(prefix, _amount,_user,_requestId));
    ...
    function claim(uint256 _amount,address _user,uint256 _requestId,uint8 _v,bytes32 _r,bytes32 _s) external {
        validateSignature(_amount,_user,_requestId,_v,_r,_s);
