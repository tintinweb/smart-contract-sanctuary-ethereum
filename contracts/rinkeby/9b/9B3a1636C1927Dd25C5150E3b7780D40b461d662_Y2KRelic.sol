...
contract Y2KRelic is ERC721A, CollaborativeOwnable, PaymentSplitter, ReentrancyGuard {
  ...
  uint16 public publicMintTxLimit = 3;
  bool public publicMintIsActive = false;

  uint16 public mintAllocation = 333;

  address[] payees = [
    0xe235BcF8C4CE374d527B1e3c11d79242CFbB335b,
    0x3B76466A862EF36D1cf089ae2529798198cdB4a6 // TODO
  ];
  ...
  constructor(address _proxyRegistryAddress)
    ERC721A("Y2K Relic Capsule", "Y2KRELIC")
  ...
  function publicMint(uint16 _quantity) external payable nonReentrant {
    require(publicMintIsActive, "inactive");
    require(_quantity <= tokensRemaining(), "insufficient supply");
    require(_quantity <= mintAllocation, "insufficient allocation");
    require(_quantity <= publicMintTxLimit, "exceeds transaction limit");
    require(msg.value >= publicMintPrice.mul(_quantity), "insufficient funds");
    
    _safeMint(_msgSender(), _quantity);
    
    mintAllocation = mintAllocation - _quantity;
  }
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    require(_exists(_tokenId), "invalid");
  ...
  function setBaseURI(string memory __baseURI) external onlyCollaborator {
    baseURI = __baseURI;
  }

  function reduceMaxSupply(uint16 _maxSupply) external onlyCollaborator {
    require(_maxSupply < maxSupply, "must be less than current max supply");
    require(_maxSupply >= totalSupply(), "must be gte total supply");
    maxSupply = _maxSupply;
  }

  function setProxyRegistryAddress(address _proxyRegistryAddress) external onlyCollaborator {
    proxyRegistryAddress = _proxyRegistryAddress;
  }

  function airDrop(address _to, uint16 _quantity) external onlyCollaborator {
    require(_to != address(0), "zero");
    require(_quantity > 0 && _quantity <= tokensRemaining(), "quantity");
    _safeMint(_to, _quantity);
  }

  function setMintAllocation(uint16 _mintAllocation) external onlyCollaborator {
    require(_mintAllocation <= tokensRemaining(), "must be lte tokens remaining");
    mintAllocation = _mintAllocation;
  }
  
  // Public

  function setPublicMintIsActive(bool _publicMintIsActive) external onlyCollaborator {
  ...
  function setPublicMintTxLimit(uint16 _publicMintTxLimit) external onlyCollaborator {
    publicMintTxLimit = _publicMintTxLimit;
  }

  // Whitelist

  function setWhitelistMintIsActive(bool _whitelistMintIsActive) external onlyCollaborator {
    whitelistMintIsActive = _whitelistMintIsActive;
  }

  function setWhitelistMintPrice(uint256 _whitelistMintPrice) external onlyCollaborator {
    whitelistMintPrice = _whitelistMintPrice;
  }

  function setWhitelistMintWalletLimit(uint16 _whitelistMintWalletLimit) external onlyCollaborator {
    whitelistMintWalletLimit = _whitelistMintWalletLimit;
  }

  function setWhitelistMerkleRoot(bytes32 _whitelistMerkleRoot) external onlyCollaborator {
