...
interface N2DRewards is IERC20 {
  function mint(address to, uint256 amount) external;
}

contract NFTMultiVaultStaking is Ownable, IERC721Receiver {
...
function addVault(
 ...
 function updateVault(       
        ...
        vaultid.nft=_nft;        
        vaultid.token=_token;        
        vaultid.name=_name;        
        vaultid.emission_rate=_emission_rate;
    }   
  ...
  function _claim(address account, uint256[] calldata tokenIds, uint256 _pid, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      Stake memory staked = vault[tokenId];        
        ...
        earned+=(block.timestamp - staked.timestamp) * (vaultid.emission_rate);
        vault[tokenId] = Stake({
        owner: account,
        tokenId: uint24(tokenId),
        timestamp: uint48(block.timestamp)
  ...
  function earningInfo(uint256[] calldata tokenIds,uint256 _pid) external view returns (uint256[2] memory info) {
     vaultInfo storage vaultid = VaultInfo[_pid];
     uint256 tokenId;     
     uint256 earned = 0;
// 
...
//  
return [earned, vaultid.emission_rate];
}
