...
contract SpookiesEtherealLove is Context, Ownable, AccessControlEnumerable, ERC1155Burnable, ERC1155Pausable {
    ...
    uint256 public MAX_MINTS = 222;
    uint256 public supply;
    uint256 public eventCounter;

    uint256[] public tokenIds = [1,2,3,4,5,6];
    ...
    function mint(uint256 quantity, bytes memory data) external payable{
        require(activeSale, "Sale is not active");
        require(quantity > 0, "Must mint more than zero");
        require(quantity < 6, "Cannot mint more than five");
        require((supply + quantity) <= MAX_MINTS, "The quantity of mints surpasses the max");
        require(msg.value == (quantity * PRICE), "Incorrect amount for requested quantity");
              
        _mint(msg.sender, 1, quantity, data);

        checkBonus(msg.sender, data);

        supply += quantity;
    }

    function gift(address account, bytes memory data) external {
        require(activeGift, "Gifting is not active");
        require(balanceOf(msg.sender, 1) > 0, "Insuffienct token balance");
        require(account != msg.sender, "You can not gift to yourself");

        safeTransferFrom(msg.sender, account, 1, 1, "");
    ...
    function specialburn(bytes memory data) external {
        require(activeBurn, "Burning is not active");
        require(balanceOf(msg.sender, 2) > 0, "Insuffienct token balance");

        _burn(msg.sender, 2, 1);
        _mint(msg.sender, 3, 1, data); 

        checkBonus(msg.sender, data);
    }

    function fuse(bytes memory data) external {
        require(
        ...
        _mint(msg.sender, 5, 1, data);
    }

    function fuse2(bytes memory data) external {
        require(
            balanceOf(msg.sender, 1) >= 0 
            && balanceOf(msg.sender, 2) >= 0 
            && balanceOf(msg.sender, 3) >= 0 
        ...
        _mint(msg.sender, 6, 1, data);
    }


    function togglePhase(uint256 phase) external onlyOwner {
        if (phase == 1) {
    ...
    function updateBonus(uint256[] calldata bonusEvents) external onlyOwner {
    ...
    function checkBonus(address account, bytes memory data) internal {
    ...
    function contractURI() public view returns (string memory) {
        return "ipfs://QmZfZF6AT3mhQFDfpdEem7MoJARC1hedLzWzUb2vsPv7et";
    }
