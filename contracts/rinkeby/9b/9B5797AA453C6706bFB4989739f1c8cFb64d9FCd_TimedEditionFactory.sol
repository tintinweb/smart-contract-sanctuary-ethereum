  ...
  IERC721CreatorMintPermissions
{
  ...
  function listReleases() external view returns (TimedEditionInfo[] memory) {
    uint256 len = _releasesIds.current();
    TimedEditionInfo[] memory rels = new TimedEditionInfo[](len);
    for (uint256 i = 0; i < len; i++) {
      rels[i] = TimedEditionInfo({
          name: releases[i + 1].name,
          id: i + 1
      });
    }
    return rels;
