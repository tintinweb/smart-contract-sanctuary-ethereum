...
contract MccmMeta is ERC721Enumerable, Ownable {
    ...
    bool public _isSaleActive = false; //flase：未開賣
    bool public _revealed = false;     //false：盲盒

    // Constants
    uint256 public constant MAX_SUPPLY = 6666;  //能挖到的總數量
    uint256 public mintPrice = 0.0003 ether;
    uint256 public maxMint = 369;  //一次能挖的NFT數
    ...
    function mintMccmMeta(uint256 tokenQuantity) public payable {   //給用戶搶NFT用的
        ...
        require(tokenQuantity > 0 && tokenQuantity <= maxMint, "Exceeded the maximum purchase quantity");
    ...
    function withdraw(address to) public onlyOwner {  //提領的功能，全部轉走
        uint256 balance = address(this).balance;
        payable(to).transfer(balance);
    }

    function ContractBalance() public view returns (uint256) {  //查合約餘額
        uint256 balance = address(this).balance;
        return balance;
    }
