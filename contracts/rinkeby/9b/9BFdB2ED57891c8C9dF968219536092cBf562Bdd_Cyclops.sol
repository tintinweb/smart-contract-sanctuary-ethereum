...
contract Cyclops is ERC721, Ownable {
    ...
    uint256 _presaleprice = 0.055 ether;
    ...
    constructor() ERC721("Cyclops", "Cycs") {
        setBaseURI("https://nftscreen.art/cyclops/");
    }

    function mintPresale(uint256 mintCount,uint8 v, bytes32 r,bytes32 s,uint256 mint_allowed) external payable {
        ...
        require(verifySignature(v,r,s,mint_allowed), "Invalid signature - please mint only on our website and make sure you are on the whitelist.");
 ...
 //verify if the hash creted with the signature is by owner or not
  function verifySignature(uint8 v, bytes32 r,bytes32 s,uint256 amountAllowed) public view returns (bool) {
    bytes32 messageHashed = keccak256(abi.encodePacked( msg.sender, amountAllowed));
