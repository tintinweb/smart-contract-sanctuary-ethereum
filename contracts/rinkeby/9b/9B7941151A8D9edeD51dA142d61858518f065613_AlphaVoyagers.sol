...
contract AlphaVoyagers is Ownable, ERC721A, IERC2981, ReentrancyGuard {
    ...
    uint64 public constant MAX_SUPPLY = 7777;
    ...
    // Treasury
    address payable private constant _treasury = payable(address(0));

    // Owners
    address payable private constant _owner1 = payable(address(0));
    address payable private constant _owner2 = payable(address(0));
    address payable private constant _owner3 = payable(address(0));

    // Dev
    address payable private constant _dev = payable(address(0));

    // Merkle Root
    bytes32 public constant root = 0x0;
    ...
    modifier onWhitelist(bytes32[] calldata merkleProof_) {
        ...
        require(MerkleProof.verify(merkleProof_, root, leaf), "AV: !on whitelist");
        _;
    }
    ...
    modifier maxWLMints(uint256 quantity_) {
        require(quantity_ <= MAX_WL_MINT_QUANTITY, "AV: max wl mint exceeded");
        require(balanceOf(msg.sender) + quantity_ <= MAX_WL_MINT_QUANTITY, "AV: max wl mint exceeded");
        _;
    }
    ...
    constructor() ERC721A("Alpha Voyagers", "AV") {}
    ...
    function withdrawTokens(IERC20 token) public {
        token.safeTransfer(_treasury, token.balanceOf(address(this)));
    }
    ...
    {
        return (_treasury, salePrice_.mulDivDown(5, 100));
    }
}
