...
contract legendsofasians is ERC721Enumerable, Ownable {

    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public constant collection_Supply = 5555;

    uint256 public preSaleGoatsMinted;

    uint256 public constant PRICE_PER_NFT = 100000000000000000 wei;  //0.11 ETH

    uint256 public constant PreSale_PRICE_PER_NFT = 80000000000000000 wei;  //0.08 ETH
...
myStruct[] public myStructs;
    ...
    function mint(uint16 numberOfTokens) public payable {
        ...
        require(PRICE_PER_NFT.mul(numberOfTokens) == msg.value, "Ether value sent is incorrect.Need 0.10 ETH for each NFT");
    ...
    function preSalemint(uint16 numberOfTokens) public payable {
        ...
        require(saleIsActive(), "Sale is not active");
        require(PreSale_PRICE_PER_NFT.mul(numberOfTokens) == msg.value, "Ether value sent is incorrect.Need 0.08 ETH For Presale NFT");
        require(totalSupply().add(numberOfTokens) < collection_Supply, "Insufficient supply");
...
function getMyNFTs() public view returns(myStruct[] memory) {
  return myStructs;
}
 ...
 function IsonSaleToken(uint256 tokenId) public view returns(bool status) {
    ...
    function getCurrentPriceOfToken(uint256 tokenId) public view virtual returns(uint256) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        require(_tokenListOwner[tokenId] == ownerOf(tokenId) , "Not For Sale");
    ...
    function tokenURI(uint256 tokenId) override public view returns(string memory) {
       ...
       string memory baseURI = "https://lofastag.s3.amazonaws.com/tether/";
