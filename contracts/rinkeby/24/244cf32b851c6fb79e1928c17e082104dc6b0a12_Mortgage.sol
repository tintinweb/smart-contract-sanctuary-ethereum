...
contract Mortgage is UpgradeablePausable, IMortgage {
  ...
  ) public override onlyHouseHold initializer {
  ...
  function onDrawdown(uint256 amount) external override onlyAdmin whenNotPaused {
    accounting.balance = amount;
  }

  function onRepayment(uint256 amount) external override onlyAdmin whenNotPaused {
    accounting.balance = accounting.balance.sub(amount);
    accounting.principalOwed = accounting.principalOwed.add(amount);
  }
