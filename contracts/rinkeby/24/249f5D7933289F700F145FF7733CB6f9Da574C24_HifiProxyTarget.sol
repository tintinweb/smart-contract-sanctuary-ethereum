...
contract HifiProxyTarget is IHifiProxyTarget {
    ...
    function removeLiquidityAndWithdrawUnderlying(
        ...
        //uint256 minHTokenAmount = normalize(withdrawAmount, hifiPool.underlyingPrecisionScalar());
        // if (hTokenReturned < minHTokenAmount) {
        //     revert HifiProxyTarget__withdrawUnderlyingInsufficientBalancee(minHTokenAmount, hTokenReturned);
        // }
        ...
        uint256 newHTokenBalance = hToken.balanceOf(address(this));
        unchecked {
            uint256 hTokenDelta = hTokenReturned - newHTokenBalance;
            if (hTokenDelta > 0) {
                hToken.transfer(msg.sender, hTokenDelta);
            }
        }
...
interface IHifiProxyTarget {
    ...
    /// @notice Removes liquidity from the AMM, and withdraws underlying.
    ///
    /// @dev Requirements:
    /// - The caller must have allowed the DSProxy to spend `poolTokensBurned` tokens.
    ///
