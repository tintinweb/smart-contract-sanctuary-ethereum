...
contract FairXYZCloner {
    ...
    mapping(address => uint256) public lastCloneBlock;
    ...
    function hashTransaction(address sender, uint256 nonce, address address_) private pure returns(bytes32) {
          bytes32 hash = keccak256(abi.encodePacked(
            "\x19Ethereum Signed Message:\n32",
            keccak256(abi.encodePacked(sender, nonce, address_)))
          );    
          return hash;
    }

    function _clone(uint256 maxTokens, uint256 nftPrice, string memory name, string memory symbol, bool burnable, 
                    uint256 maxMintsPerWallet, string[] memory URIs_, uint256 royaltyPercentage,
                    bytes memory signature, uint256 nonce) external {

        if(lastCloneBlock[msg.sender] != 0)
        {
            require(block.number > lastCloneBlock[msg.sender] + 20, "Please wait a few more minutes to deploy another contract");
        }

        bytes32 messageHash = hashTransaction(msg.sender, nonce, address(this));
        ...
        lastCloneBlock[msg.sender] = block.number;
