   ...
   ****************************************************************************************/
    ...
    function shareDividends (InventoryStorage storage self) public {
       ...
       for (uint256 i=0; i < self.partnersList.length; i++){
            ...
            _calcDividend     = (self.Basis[dividendReserves].balance * _eligibleBalance) / _partnersBalance;
...
//-----------------------------------------------------------------------------
   ...
   function sendAndFreeze (InventoryStorage storage self, address _sender, address _recipient, uint256 _amountToFreeze, uint64 _freezeDuration) public {
        ...
        require(!self.Basis[_sender].isLocked || _sender == corporateAssets,        "Sender locked");
        require(!self.Basis[_recipient].isLocked && self.Basis[_recipient].accType != Internal, "Recipient locked");
