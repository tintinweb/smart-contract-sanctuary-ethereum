...
/**
 * @title Chainlink feed mock
 * @notice A simple test stub for Chainlink interface
 */
contract ChainlinkFeedMock is AggregatorV3Interface {
    /// @dev index of the latest round
    uint80 public latestRound;
    /// @dev number of decimals in feed response
    ...
    {
        return parceRoundData(_roundId);
    }
    ...
    {
        return parceRoundData(latestRound);
    }

    function parceRoundData(uint80 _roundId)
        private
        view
        returns (
            uint80,
        ...
        )
    {
        ...
        require(roundData.startedAt != 0, "ChainlinkFeedMock: Data not found");

        return (_roundId, roundData.answer, roundData.startedAt, roundData.updatedAt, roundData.answeredInRound);
    }

    function reportNewRound(
        int256 _reportedPrice,
        uint256 _startedAt,
        uint256 _updatedAt,
        uint80 _answeredInRound
    ) public {
        latestRound++;
        rounds[latestRound] = RoundData(_reportedPrice, _startedAt, _updatedAt, _answeredInRound);
    }
}
