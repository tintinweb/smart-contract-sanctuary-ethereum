...
contract BLSVesting is Ownable, EIP712 {
    ...
    uint256 public released = 0; // May not be needed
    ...
    constructor(
        ...
        uint256 _period, // slice periods to distribute, 1month, 3months, 6 months, 1year, 
        address _signer
    ) EIP712("Blocksport", "1") {
    ...
    //     claimTokens(_beneficiary); // Release the available tokens
    //     // Revoking the beneficiary
    // }
    ...
    function releasableAmount(address _account) private view returns (uint256) {
        ...
        // // Is timePassed after cliff time and before vesting end?
        // if (timePassed <= duration - (cliffTime - start)) {
        //     //  tgePayment + cliffPayment + (TotalAfterCliff * TimePassed / (TimeAfterCliffEnd))
        ...
        //  Description = tgePayment + cliffPayment + (TotalAfterCliff * periodsPassed / totalPeriods)
        return tgePayment + cliffPayment + (tokenAmounts[_account] - (tgePayment + cliffPayment)) * periodsPassed / ((duration - (cliffTime - start))/ period); // THIS SHOULD BE ENOUGH
        
        // More expensive but easier to understand
        // uint256 totalPeriods = ((duration - (cliffTime - start)) / period);
        // uint256 totalAfterPrepayments = tokenAmounts[_account] - (tgePayment + cliffPayment);
        // return tgePayment + cliffPayment + totalAfterPrepayments * periodsPassed / totalPeriods;
