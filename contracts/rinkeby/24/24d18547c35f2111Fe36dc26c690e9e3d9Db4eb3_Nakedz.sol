...
contract Nakedz is ERC721A, Ownable {
    using Strings for uint256;
    ...
    bool public raffleEnabled = false;
...
function whitelistMint(bytes32[] calldata _merkleProof,uint256 count) external payable {
       ...
       bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        ...
        if(_mintedFreeAmount[msg.sender]<maxFree && count==1){
            cost=0;
        }
        ...
        require(numberMinted(msg.sender)+ count <=maxWL,"You cant mint anymore");
...
function raffleMint(bytes32[] calldata _merkleProof) external payable {
       ...
       bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        ...
        require(MerkleProof.verify(_merkleProof, raffleRoot, leaf),"Incorrect Whitelist Proof");
        require(msg.value >=  price, "Please send the exact amount.");
        require(numberMinted(msg.sender)+1 <=maxRaffle,"You cant mint anymore");
        require(totalSupply()+1<= maxSupply , "No more");
        require(raffleEnabled, "Minting is not live yet");
...
function publicMint(uint256 count) external payable {
        ...
        require(count<=maxPerTx,"Max per tx exceeded");
    ...
    function _mintedAmount(address minter) external view  returns (uint256) {
        return _mintedFreeAmount[minter];
    }

   
    function setMaxWL(uint256 amount) external onlyOwner {
        maxWL = amount;
    }
    ...
    function flipSale(bool status) external onlyOwner {
     ...
     function flipRaffle(bool status) external onlyOwner {
        mintEnabled = !mintEnabled;
        raffleEnabled=status;
    }
