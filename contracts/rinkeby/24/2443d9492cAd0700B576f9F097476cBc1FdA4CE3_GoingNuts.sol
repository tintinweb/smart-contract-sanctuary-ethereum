...
contract GoingNuts is ERC721A, Allowlist, SalesActivation{
    ...
    // For giveaway 
    uint256 public team_mint = 100;
    // Max presale 
    uint256 public og_max = 50;
    ...
    // Free Mint 
    mapping(address => bool) public free_list;
    mapping(address => bool) public freeListMinted;
    // OG claimed
    mapping(address => bool) public og_list;
    mapping(address => bool) public ogListBought;
    // Presale claimed
    address[] public addressesOfPresaleBought;
    mapping(address => uint256) public presaleListBought;
    // team addresses
    address nuts_wallet = 0x799CD72D86A6AA46876C2689A13637Cbe6ae89da;
    //Event presale
    ...
    * @dev OG sales nuts
    */
    function ogSale() public payable isOGSalesActive {
        uint256 supply = totalSupply();
        require(og_list[msg.sender], 'You are not on the OG list');
        require(og_purchased + 1 <= og_max, 'Purchase exceeds max allowed');
        require( msg.value >= og_price,             "Ether sent is not correct" );
        require( supply+1 <= max_sales_nuts - team_mint,      "Exceeds maximum nuts supply" );
        require(!ogListBought[msg.sender], 'Purchase exceeds max allowed');
        require(tx.origin == msg.sender, "Contracts not allowed to mint");
        // og spot bought
        ogListBought[msg.sender] = true;
    ...
    */
    function presale(uint256 nutsNumber, bytes memory _signature) public payable isPreSalesActive isSenderAllowlisted(nutsNumber, _signature){
        uint256 supply = totalSupply();
        require( presale_period_purchased + nutsNumber <= presale_period_phase_max, 'Exceeds presale nuts supply');
        require( supply+nutsNumber <= max_sales_nuts - team_mint,      "Exceeds maximum nuts supply" );
        require(presaleListBought[msg.sender] + nutsNumber <= presaleListMax, 'Purchase exceeds max allowed');
        require( msg.value >= wl_price * nutsNumber,             "Ether sent is not correct" );
        require(tx.origin == msg.sender, "Contracts not allowed to mint");

        for (uint256 i = 0; i < nutsNumber; i++) {
            _safeMint(msg.sender, supply + i);
            presaleListBought[msg.sender] += 1;
            // increment purchased wl numbers
            presale_period_purchased += 1;
        }
    ...
    */
    function mint(uint256 nutsNumber) public payable isPublicSalesActive {
        ...
        require( presale_period_purchased + nutsNumber <= presale_period_phase_max,      "Exceeds maximum period nuts supply" );
        require( supply+nutsNumber <= max_sales_nuts - team_mint,      "Exceeds maximum nuts supply" );
