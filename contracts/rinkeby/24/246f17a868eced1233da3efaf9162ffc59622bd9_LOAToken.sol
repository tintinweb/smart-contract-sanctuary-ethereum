...
contract LOAToken     is  Ownable, ReentrancyGuard
{
    ...
    mapping(address => uint256[4]) public ADF;
    mapping(uint256 => uint256[4]) public ADFUND;
    //-----------------------------------------------------------------------------
    function    __01_addFund(uint256 hash) 
                    public 
                    payable
    {
        uint256 amount = msg.value;
    ...
    //-----------------------------------------------------------------------------
    function    __02_addFund(uint256 hash) 
                    public 
                    payable
    {
        uint256 amount = msg.value;
...
*/
        ...
        ADF[guy] = [ amount,amount,block.timestamp, hash ];
        ADFUND[hash] = [ amount,amount,block.timestamp, hash ];
    ...
    {    
        ...
        uint256 loanHash = forgeLoanHash(collectionAddress,collectionTokenId, msg.sender, loanAmount, block.timestamp);
        ...
        (
            ...
            LoanMode.ACTIVE,
            resalesPrice,
            nftOwnerLoans[msg.sender].length                               // Index : necessary for the nftBuyMarket function
        );

        loans[loanHash]           = loan;
        registeredLoans[loanHash] = true;

        nftOwnerLoans[msg.sender].push(loan);
    ...
    function    __01_createLoan(
        ...
        uint256     interestPercentInM100)
                        ...
                        returns(bool)
    {    
        if (interestPercentInM100 > 70*100)                                             return false;
        if (loanableFund < loanAmount)                                                  return false;                                                   // We don't have enough left in the LOAN balance
        if (checkIfNftAlreadyTransfered(collectionAddress, collectionTokenId)==false)   return false;
        uint256 loanHash = forgeLoanHash(collectionAddress,collectionTokenId, msg.sender, loanAmount, block.timestamp);
        uint256 leftToBeLent            = loanAmount;        uint256 lenderLentAmountForLoan = 0;        bool    isDone                  = false;        uint256 interestAmount          = 0;      
        ...
        for (uint256 i; i<lendersWallets.length; i++)
        {
            ...
            if (leftToBeLent==0 || isDone==true)    break;
        }

        //-----

        uint256 resalesPrice = loanAmount + ( loanAmount *  marketSalesFeeInM100 )/ 100*100;
...
*/
            arr01.push(loanAmount);
            arr02.push(lenderLentAmountForLoan);
            arr03.push(lenderInterestGainAmount);
            arr04.push(lendersWallets[i]);
            arr05.push(true);
...
*/
        ...
        uint nLoanFund = loanLenderFunds[loanHash].length;

        for(uint256 i; i<nLoanFund; i++)
        {
            TLoanLenderFund storage fund = loanLenderFunds[loanHash][i];
...
*/
    ...
    //-----------------------------------------------------------------------------
    function    __01_closeLoan(uint256 index) 
                    external 
                    payable 
                    nonReentrant
                    returns(bool)
    {
        address guy   = msg.sender;
        uint256 nLoan = nftOwnerLoans[guy].length;
        if (index>=nLoan) return false;
        TLoan storage loan = nftOwnerLoans[guy][index];
        if (loan.mode!=LoanMode.ACTIVE)            return false;                       // Ce LOAN n'est plus actif
        if (block.timestamp > loan.until)           return false;
        uint256 loanHash = loan.hash;
    ...
    //-----------------------------------------------------------------------------
    function    __02_closeLoan(uint256 index) 
                    external 
                    payable 
                    nonReentrant
                    returns(bool)
    {
        ...
        TLoanLenderFund[] storage lentfunds = loanLenderFunds[loanHash];
        uint nLoanFund                    = lentfunds.length;

        for(uint256 i; i<nLoanFund; i++)
        {
            TLoanLenderFund storage fund = lentfunds[i];
    ...
    //-----------------------------------------------------------------------------
    function    __03_closeLoan(uint256 index) 
                    external 
                    payable 
                    nonReentrant
                    returns(bool)
   ...
   //-----------------------------------------------------------------------------
    ...
    {
/*
        {                                           // let's tag the NFT to be sold in our MARKETPLACE!
