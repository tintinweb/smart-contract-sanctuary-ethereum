...
contract BabyChedda is Ownable, IERC20 {
    ...
    function _executeSwap(uint256 tokens, uint256 native) private {
        ...
        if (address(devWallet) != address(0)) {
            swapTokensdev = (tokens * devFee) / totalFee;
        }
        ...
        uint256 swapTokensTotal = swapTokensdev+
            swapTokensDividends +
            swapTokensLiquidity;
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "Baby Chedda_DividendTracker";
    string private _symbol = "Baby Chedda_DividendTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "Baby Chedda_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "Baby Chedda_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "Baby Chedda_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "Baby Chedda_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("RBaby Chedda_DividendTracker: method not implemented");
    }
    ...
    function allowance(address, address)
        ...
        returns (uint256)
    {
        revert("Baby Chedda_DividendTracker: method not implemented");
    }
