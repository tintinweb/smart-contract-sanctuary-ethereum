...
// File contracts/quests_skills/AradenaCommodities.sol
...
contract AradenaCommodities is ERC1155, Ownable {
  ...
  /**
   * @dev Mapping of addresses that are authorized to mint & burn tokens.
   */
  ...
  function mint(address _to, uint _id, uint _amount) external onlyAuthorized() {
    require(bytes(uri(_id)).length > 0, 'Invalid tokenID');
    _mint(_to, _id, _amount, "");
  }

  function burn(address from, uint _id, uint _amount) external onlyAuthorized() {
    require(bytes(uri(_id)).length > 0, 'Invalid tokenID');
