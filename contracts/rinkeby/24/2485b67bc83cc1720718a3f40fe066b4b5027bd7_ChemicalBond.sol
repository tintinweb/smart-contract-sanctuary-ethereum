...
//GregTheDev.eth
contract ChemicalBond is Ownable, ReentrancyGuard, ERC721A, ERC721AQueryable {
    ...
    constructor(string memory inputURI, bytes32 alRoot) ERC721A("ChemicalBond", "BOND")
    {
        _baseTokenURI = inputURI;
        _alMerkleRoot = alRoot;
    }
    ...
    modifier mintCompliance(uint256 _quantity) {
        ...
        if (_quantity * price > msg.value) revert NSF();
        _;
    }
    ...
    //External Functions
    function alMint(uint256 _quantity, bytes32[] calldata proof)
        external
        payable
        ...
        nonReentrant
    {
        ...
        if (_numberMinted(msg.sender) + _quantity > 3) revert MintCap();
        require(
    ...
    function pubMint(uint256 _quantity)
        ...
        nonReentrant
    {
        require(pubActive, "Public inactive");
        if (_numberMinted(msg.sender) + _quantity > 3) revert MintCap();
        _mint(msg.sender, _quantity);
    }

    function alPubMint(uint256 _quantity, bytes32[] calldata proof)
        external
        payable
        mintCompliance(_quantity)
        nonReentrant
    {
        require(pubActive, "Public inactive");
        if (_numberMinted(msg.sender) + _quantity > 4) revert MintCap();
        require(
            MerkleProof.verify(proof, _alMerkleRoot, _leaf(msg.sender)),
            "Not AL"
        );
    ...
    function updateState(bool _alActive, bool _pubActive) external onlyOwner {
        alActive = _alActive;
        pubActive = _pubActive;
    }
    ...
    function withdraw(address _to) external onlyOwner nonReentrant {
        uint256 devBal = (address(this).balance * 2) / 25;
        (bool success1, ) = payable(0xC5A07c29a71db1a8AdB7FE8011608074DC64E94f)
            .call{value: devBal}("");
        require(success1, "dev transfer failed.");
        (bool success2, ) = payable(_to).call{value: address(this).balance}("");
