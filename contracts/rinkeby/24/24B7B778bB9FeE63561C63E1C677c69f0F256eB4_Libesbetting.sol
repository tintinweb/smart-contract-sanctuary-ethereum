...
contract Libesbetting is Ownable {
  ...
  function betTournament(
    ...
    require(Blacklist[_tokenId] != true, "This Token have been Betted!");
  ...
  function existTokenBet(uint256 _tournamentId, uint256 _tokenId)
    internal
    view
    returns (bool)
  {
    for (uint256 i = 0; i < betters[_tournamentId].length; i++) {
      if (betters[_tournamentId][i].TokenId == _tokenId) {
        return true;
      }
    }
    return false;
  ...
  function cancelBet(
    ...
    require(
      existTokenBet(_tournamentId, _tokenId) == true,
      "The bet is not exists"
    );
    for (uint256 i = 0; i < betters[_tournamentId].length; i++) {
