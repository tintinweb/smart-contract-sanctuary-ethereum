...
/**
 ...
 .----------------.  .----------------.  .----------------.   .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------. 
| .--------------. || .--------------. || .--------------. | | .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |  _________   | || |  ____  ____  | || |  _________   | | | |    ______    | || |     _____    | || | ____    ____ | || |   ______     | || |     _____    | || |  _________   | || |    _______   | |
| | |  _   _  |  | || | |_   ||   _| | || | |_   ___  |  | | | |  .' ___  |   | || |    |_   _|   | || ||_   \  /   _|| || |  |_   __ \   | || |    |_   _|   | || | |_   ___  |  | || |   /  ___  |  | |
| | |_/ | | \_|  | || |   | |__| |   | || |   | |_  \_|  | | | | / .'   \_|   | || |      | |     | || |  |   \/   |  | || |    | |__) |  | || |      | |     | || |   | |_  \_|  | || |  |  (__ \_|  | |
| |     | |      | || |   |  __  |   | || |   |  _|  _   | | | | | |    ____  | || |      | |     | || |  | |\  /| |  | || |    |  ___/   | || |      | |     | || |   |  _|  _   | || |   '.___`-.   | |
| |    _| |_     | || |  _| |  | |_  | || |  _| |___/ |  | | | | \ `.___]  _| | || |     _| |_    | || | _| |_\/_| |_ | || |   _| |_      | || |     _| |_    | || |  _| |___/ |  | || |  |`\____) |  | |
| |   |_____|    | || | |____||____| | || | |_________|  | | | |  `._____.'   | || |    |_____|   | || ||_____||_____|| || |  |_____|     | || |    |_____|   | || | |_________|  | || |  |_______.'  | |
| |              | || |              | || |              | | | |              | || |              | || |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' | | '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'   '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------' 
...
contract Test3 is AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage {
    ...
    mapping(uint => bool) private hasNewNft;
    ...
    IERC721 private _genesisContract;
        ...
        ERC721 (name, symbol)
    {
        ...
        _genesisContract = IERC721(theGimpiesAddress);
        _wallet = wallet;
    ...
    function revealNewNFT() external onlyAdmin {
        _revealed = true;
    }
    ...
    function setOGPrice(uint ogMintPrice) external onlyAdmin {
        ogPrice = ogMintPrice;
    }
    ...
    function ogMint(uint[] calldata genesisTokenIds) public payable {
        uint numNewNft = genesisTokenIds.length;
        ...
        require(
            tokenIdTracker.current() + numNewNft <= max_supply + 1,
            "You've tried to mint too many"
        );
        require(msg.value == ogPrice * numNewNft, "Amount sent is incorrect");

        for (uint i=0; i < numNewNft; ++i) {
            uint tokenId = tokenIdTracker.current();
            uint genesisTokenId = genesisTokenIds[i];

            require(
                _genesisContract.ownerOf(genesisTokenId) == msg.sender,
                "Caller is not owner of that Gimp"
            );
            require(
                hasNewNft[genesisTokenId] == false,
                "Your genesis NFT has already minted a New NFT"
            );
    ...
    function publicMint() public payable {
        ...
        require(_openMint == true, "Minting is not currently open.");
    ...
    function hasGenesisNftMintedNewNft(uint genesisTokenId) public view returns (bool) {
        return hasNewNft[genesisTokenId];
    }
