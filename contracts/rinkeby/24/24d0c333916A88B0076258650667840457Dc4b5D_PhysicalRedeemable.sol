...
contract PhysicalRedeemable is Ownable {
    ...
    function collect(
        address account,
        uint256[] memory accountAmounts,
        bytes32[] calldata merkleProof,
        uint256[] memory requestedPhysicalItemIds,
        uint256[] memory requestedPhysicalItemQuantities,
        uint256 parallelTransactionId
    ) external payable {
        // Verify the merkle proof.
        bytes32 leaf = keccak256(
            abi.encodePacked(account, physicalItemTokenIds, accountAmounts)
        );
        ...
        // has not requested more then avalible assigned items
        for (uint256 i = 0; i < requestedPhysicalItemIds.length; i++) {
            require(
                requestedPhysicalItemQuantities[i] +
                    PhysicalItemsClaimed[account][i] <=
                    accountAmounts[i],
        ...
        for (uint256 i = 0; i < requestedPhysicalItemIds.length; i++) {
            PhysicalItemsClaimed[account][i] += requestedPhysicalItemQuantities[
                i
            ];
        }
        ...
        parallelAuxiliaryItem.safeBatchTransferFrom(
            ...
            account,
            requestedPhysicalItemIds,
            requestedPhysicalItemQuantities,
            bytes("")
        );
        // emite collected event
        emit Collected(
            account,
            requestedPhysicalItemIds,
            requestedPhysicalItemQuantities,
