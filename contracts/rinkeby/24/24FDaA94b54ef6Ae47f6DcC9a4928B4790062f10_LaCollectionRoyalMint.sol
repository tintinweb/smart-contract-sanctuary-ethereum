...
/**
 * @title LaCollectionRoyalMint
 */
contract LaCollectionRoyalMint is ERC721URIStorage, ERC721Burnable, Pausable, LaCollectionAccess {
    using SafeMath for uint256;

    INewContract public newContract;

    constructor() ERC721("LaCollectionRoyalMint", "LCRM") {
    }
    ...
    function setNewContract(address newContractAddress) external onlyOwner {
        require(
            ...
            "LaCollectionRoyalMint: NewContract already set"
        );
    ...
    function migrateTokens(uint256[] calldata tokenIds) external onlyOwner {
        require(
            address(newContract) != address(0),
            "LaCollectionRoyalMint: New contract not set"
        );
