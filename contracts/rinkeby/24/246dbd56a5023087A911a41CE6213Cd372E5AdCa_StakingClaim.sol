...
contract StakingClaim is IStakingClaim, Ownable, ReentrancyGuard {
    ...
    mapping (address => receivedInfo) receivedInfos;
    ...
    {
        uint256 _duration = (condition.duration).mul(1 days);
        ...
        if(_paymentDt > block.timestamp + _duration) {
            _elapsedDays = (_paymentDt.sub(block.timestamp)).div(_duration) + 1;
        } else {
            return 1;
        }
    }
    ...
    {
        uint256 _amount = compensationAmount.mul(_proofLength); // 받을 수 있는 총량 [단위 ether]
        uint256 _totalReciveableToken = _amount.div(condition.unlockCnt).mul(GetElapsedToDuration(_roothashIdx));
        uint256 _amountReceived = receivedInfos[_msgSender()].amountReceived; // 받은 총량 [단위 ether]
        if(_totalReciveableToken > _amountReceived) {
            _tokensCurrentlyReciveable = _totalReciveableToken - _amountReceived; // 현재 받을 수 있는 총 토큰량 - 현재까지 받은량 = 현재 받을 수 있는 량
        } else {
        ...
        nonReentrant
    {
        ...
        uint256 _amount = compensationAmount.mul(_proof.length);
        uint256 _amountReceived = receivedInfos[_msgSender()].amountReceived;
        require(_amount > _amountReceived,"ClaimToStaking_ERR02");

        uint256 _amountReceive = GetTokensCurrentlyReciveable(_roothashIdx ,_proof.length);

        IERC20(tavaTokenAddress).transfer(_msgSender(), _amountReceive);

        receivedInfos[_msgSender()] = receivedInfo(_roothashIdx, (_amountReceived + _amountReceive));
...
interface IStakingClaim {
    ...
    // [allUser] 스테이킹 보상 수령
    function ClaimToStaking(uint256 _merkleTreeIdx,bytes32[] memory _proof, bool[] memory _proofFlags, bytes32[] memory _leaves) external;
