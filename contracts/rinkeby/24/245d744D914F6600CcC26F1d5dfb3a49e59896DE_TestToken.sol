...
contract TestToken is ERC721Core {
    /* solhint-disable no-empty-blocks */
    constructor()
        ERC721Core("TestToken", "TST", address(0), address(0), 0, 100, "", "")
    {}
    ...
    OwnerChanger
{
    ...
    modifier onlyOwnerOrSale() {
        ...
        require(owner() == caller || _saleAddress == caller, "invalid caller");
        _;
    }
    ...
    function setApproveDelegator(
        address proxyRegistryAddress,
        address allOperatorAddress
    ) external onlyOwner {
        ...
        )
    {
        ...
        bool isNext = true;
        uint256 len = _ownedTokenIds[owner].length;
        for (uint256 i = 0; i < TOKEN_IDS_BY_OWNER_ONCE_COUNT; i++) {
            uint256 index = i + offset;
            if (index < len) {
                tokenIds[i] = _ownedTokenIds[owner][index];
                tokenIdCount++;
            } else {
                tokenIds[i] = 0;
                isNext = false;
    ...
    function setSaleAddress(address newSaleAddress) external onlyOwner {
        address previousSaleAddress = _saleAddress;
        _saleAddress = newSaleAddress;
        emit SaleAddress(previousSaleAddress, newSaleAddress);
    }
        ...
        returns (uint256)
    {
        ...
        return _ownedTokenIds[tokenOwner].length;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && from != to) {
            // remove from `from`
            uint256 targetIndex = _ownedTokenIds[from].length;
            for (uint256 i = 0; i < _ownedTokenIds[from].length; i++) {
                if (_ownedTokenIds[from][i] == tokenId) {
                    targetIndex = i;
                    break;
                }
            }
            require(
                targetIndex < _ownedTokenIds[from].length,
                "`from` not owned token."
            );
            uint256 lastIndex = _ownedTokenIds[from].length - 1;
            _ownedTokenIds[from][targetIndex] = _ownedTokenIds[from][lastIndex];
...
contract ApproveDelegator {
    ...
    constructor() {
        address proxyRegistryAddress_ = address(0);
        ...
        assembly {
            ...
            case 1 {
                // mainnet
                proxyRegistryAddress_ := 0xa5409ec958c83c3f309868babaca7c86dcb077c1
            }
            case 137 {
                // polygon
                allOperatorAddress_ := 0x58807baD0B376efc12F5AD86aAc70E78ed67deaE
            }
            case 4 {
                // rinkeby
                // https://github.com/ProjectOpenSea/opensea-creatures/blob/master/migrations/2_deploy_contracts.js#L29
                proxyRegistryAddress_ := 0x1E525EEAF261cA41b809884CBDE9DD9E1619573A
            }
            case 80001 {
                // mumbai
                allOperatorAddress_ := 0xff7Ca10aF37178BdD056628eF42fD7F799fAc77c
            }
        }

        _setApproveDelegator(proxyRegistryAddress_, allOperatorAddress_);
    }
    ...
    function _setApproveDelegator(
        address newProxyRegistryAddress,
        address newAllOperatorAddress
    ) internal {
        address previousProxyRegistryAddress = _proxyRegistryAddress;
        ...
        emit ProxyRegistryAddress(
            previousProxyRegistryAddress,
            newProxyRegistryAddress
        );
        emit AllOperatorAddress(
            previousAllOperatorAddress,
            newAllOperatorAddress
        );
    }

    function _isApprovedForAllByDelegator(address owner, address operator)
        internal
        view
        returns (bool)
    {
        if (_proxyRegistryAddress != address(0)) {
            if (
                address(ProxyRegistry(_proxyRegistryAddress).proxies(owner)) ==
                operator
            ) {
                return true;
            }
...
abstract contract ERC721MetadataOnly is IERC721Metadata, ERC165 {
    ...
    function _setTokenURI(
        ...
        emit TokenURI(newTokenURIPrefix, newTokenURISuffix);
    }
}
