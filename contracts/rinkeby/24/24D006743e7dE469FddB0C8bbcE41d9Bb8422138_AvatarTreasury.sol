...
contract AvatarTreasury is AccessControl, ReentrancyGuard {
    ...
    uint256 public MAX_NUM_ITEMS_PER_TX;
    ...
    modifier saleConditions(uint256 itemCount) {
        require(
            MAX_NUM_ITEMS_PER_TX >= itemCount,
            "Minting per transaction limit reached"
        );
        require(_PUBLIC_SALE_ENABLED == true, "Sale is not started yet!");
        require(remainingItemsForSale() >= itemCount, "Max limit reached");
        _;
    }
    ...
    constructor() {
        ...
        MAX_NUM_ITEMS_PER_TX = 500;
    }

    /**
     * @dev purchase avatar minting right by public users via ETH.
     */
    function purchaseMintingRight(uint256 itemCount)
        external
        payable
        saleConditions(itemCount)
    {
        require(
            msg.value >= _ITEM_PRICE_IN_ETH.mul(itemCount),
     ...
     */
    function purchaseMintingRightByToken(
        IERC20 tokenAddress,
        uint256 amount,
        uint256 itemCount
    ) external saleConditions(itemCount) {
        require(
            tokenAddress == TOKEN_USDC ||
                tokenAddress == TOKEN_USDT ||
                tokenAddress == TOKEN_WETH,
            "Token not supported"
        );

        if (tokenAddress == TOKEN_USDC) {
            require(
                amount >= _ITEM_PRICE_IN_USDC.mul(itemCount),
                "Amount is not sufficient for purchase by USDC token"
            );
        } else if (tokenAddress == TOKEN_USDT) {
            require(
                amount >= _ITEM_PRICE_IN_USDT.mul(itemCount),
                "Amount is not sufficient for purchase by USDT token"
            );
        } else if (tokenAddress == TOKEN_WETH) {
            require(
                amount >= _ITEM_PRICE_IN_WETH.mul(itemCount),
                "Amount is not sufficient for purchase by WETH token"
            );
        }
        ...
        incrementSalesCount(tokenAddress, amount, itemCount);
    }
    ...
    /**
     * @notice Allows an admin to update mint per tx.
     */
    function adminUpdateMintTxLimits(uint256 itemCount)
        external
        onlyRole(ADMIN_ROLE)
    {
        MAX_NUM_ITEMS_PER_TX = itemCount;
        ...
        onlyRole(ADMIN_ROLE)
    {
        ...
        if (tokenAddress == TOKEN_USDC) {
            ...
            tokenAddress == IERC20(0x0000000000000000000000000000000000000000)
        ) {
     ...
     */
    function adminUpdateTokenAddress(
        IERC20 usdcAddress,
        IERC20 usdtAddress,
        IERC20 wethAddress
    ...
    function incrementSalesCount(
        ...
        _soldItemIdCounter.current().add(itemCount);
