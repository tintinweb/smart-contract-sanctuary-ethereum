...
contract MoonwalkersRoyalties is ReentrancyGuard {
    ...
    address public moonwalkersContractAddress = 0x1e3136CD0203b271753c990fADaFA7Ae0283aD79;

    mapping(address => uint256) public shares;

    receive() external payable {
        emit Received(msg.sender, msg.value);
        setOwnerShares(msg.value);
    }

    /// @dev Get the ownership data.
    function getOwnership(uint256 tokenId) public view returns (Moonwalkers.TokenOwnership memory) {
        return Moonwalkers(moonwalkersContractAddress).getOwnershipData(tokenId);
    }

    /// @dev Get the ownership data.
    function setOwnerShares(uint256 _amount) public {
        uint256 totalSupply = totalSupplyCaller();
        uint256 amountPerWallet = _amount.div(totalSupply);
        for (uint256 i = 0; i < totalSupply; i++) {
            shares[getOwnership(i).addr] += amountPerWallet;
        }
    }
