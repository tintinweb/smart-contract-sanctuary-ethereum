...
contract Vendor is Ownable {
  ...
  function sellTokens(uint256 _amount) external {
    require(_amount > 0, "Must enter amount greater than 0");
    require(_amount <= yourToken.balanceOf(msg.sender), "You do not have the specified amount of tokens");

    //Check the vendor can facilitate a trade
    uint256 ethOwed = _amount/tokensPerEth;
    require(address(this).balance >= ethOwed, "Vendor does not have the required funds");

    //Approval must be performed before this transferFrom can go through
    (bool sent) = yourToken.transferFrom(msg.sender, address(this), _amount);
    require(sent, "Transfer of tokens to vendor unsuccessful");
