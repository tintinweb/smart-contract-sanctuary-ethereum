...
contract Swap is BaseContract
{
    ...
    /**
     * Token price.
     * @param amount_ Amount of payment in.
     * @return uint[] Amount out.
     */
    function tokenPrice(uint256 amount_) external view whenNotPaused returns (uint[] memory)
    {
        ...
        return _getPrice(address(_payment_), address(_token_), amount_);
    }

    /**
     * Payment price.
     * @param amount_ Amount of token in.
     * @return uint[] Amounts out.
     */
    function paymentPrice(uint256 amount_) external view whenNotPaused returns (uint[] memory)
    {
        require(amount_ > 0, "Invalid amount");
        IERC20 _payment_ = IERC20(addressBook.get("payment"));
        require(address(_payment_) != address(0), "Payment token not set");
        IToken _token_ = IToken(addressBook.get("token"));
        require(address(_token_) != address(0), "Token not set");
        return _getPrice(address(_token_), address(_payment_), amount_);
    }

    function _getPrice(address from_, address to_, uint256 amount_) internal view returns (uint[] memory)
    {
        ...
        _path_[0] = from_;
        _path_[1] = to_;
        return _router_.getAmountsOut(amount_, _path_);
    }
    ...
    function swapPaymentForToken(uint256 amount_) external whenNotPaused returns (bool)
    {
        ...
        IUniswapV2Router02 _router_ = IUniswapV2Router02(addressBook.get("router"));
        require(address(_router_) != address(0), "Router not set");
        require(_payment_.transferFrom(address(msg.sender), address(this), amount_), "Payment transfer failed");
        address[] memory _path_ = new address[](2);
        _path_[0] = address(_payment_);
        _path_[1] = address(_token_);
        _payment_.approve(address(_router_), amount_);
        ...
        return _token_.transfer(msg.sender, _token_.balanceOf(address(this)));
    }
    ...
    function swapTokenForPayment(uint256 amount_) external whenNotPaused
    {
        ...
        IUniswapV2Router02 _router_ = IUniswapV2Router02(addressBook.get("router"));
        require(address(_router_) != address(0), "Router not set");
        require(_token_.transferFrom(address(msg.sender), address(this), amount_), "Token transfer failed");
        uint256 _balance_ = _token_.balanceOf(address(this));
        _token_.approve(address(_router_), _balance_);
        address[] memory _path_ = new address[](2);
