...
contract SpiritAnimals is Ownable, ERC721A {
	...
	uint16 public length      = 200 minutes;
	uint16 public interval    = 20 minutes;

	// Tracking
	mapping(address => uint8) public mintClaims;
	mapping(address => uint8) public dutchMints;
	...
	constructor(string memory uri, uint128 reserved_amount, address[] memory monarchs) ERC721A("Spirit Animals", "DAS") {
		baseUri      = uri;
		claimSupply  = reserved_amount + 10; //Add the monarch airdrops
	...
	function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
		...
		if(tokenId <= 10) {
			return string(abi.encodePacked(baseURI, tokenId % 2 == 0 ? 'nemesis' : 'hero'));
		}

		return string(abi.encodePacked(baseURI, 'blank'));
	}
	...
	function claim(uint8 quantity, bytes calldata signature) public callerIsUser authMint(quantity, signature) {
		require(claimAvailable, "Claiming is no longer available");
		require(mintClaims[msg.sender] < 1, "Claim has already been executed.");
		require(totalSupply() + quantity <= claimSupply, "Exceeds claim supply");

		mintClaims[msg.sender] = quantity;
		_safeMint(msg.sender, quantity);
	}

	function dutchMint(uint8 quantity) public payable callerIsUser {
		require(dutchActive, "Dutch auction is not active");
		require(dutchMints[msg.sender] + quantity <= dutchMaxMint, "Exceeds dutch max mint");
		require(totalSupply() + quantity <= getDutchSupplyMax(), "Exceeds dutch supply");
		require(block.timestamp < endTime, "Dutch auction ended.");

		uint256 price = getAuctionPrice() * quantity;
		require(msg.value >= price, "Need to send more ETH.");

		dutchMints[msg.sender] += quantity;
		_safeMint(msg.sender, quantity);
	...
	function freeMint(uint8 quantity, bytes calldata signature) public callerIsUser authMint(quantity, signature) {
		require(freeAvailable, "Free mints are not available yet.");
		require(freeClaims[msg.sender] < 1, "Free Claim has already been executed.");
		require(totalSupply() + quantity <= MAX_SUPPLY, "Exceeds max supply");

		freeClaims[msg.sender] = quantity;
		_safeMint(msg.sender, quantity);
	}
	...
	function getDutchSupplyMax() internal view returns (uint256) {
		return claimSupply + dutchSupply;
	}
	...
	function toggleFreeMinting() public onlyOwner {
		freeAvailable = !freeAvailable;
	}

	function toggleDutch() public onlyOwner {
	...
	function toggleClaimAvailable() public onlyOwner {
