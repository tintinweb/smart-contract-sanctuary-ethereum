...
contract ACCESSCARDNFT is ERC1155Supply,Ownable {
  ...
  mapping(address=>uint256) private privateSaleUserMints;
    uint256[] supplies = [47, 300, 500, 1153];
    uint256[] minted = [0, 0, 0, 0];
    uint256[] privaterates = [0.04 ether, 0.03 ether, 0.02 ether, 0.01 ether];
  ...
  constructor() ERC1155("https://gateway.pinata.cloud/ipfs/QmWFp5PRgz1KJUGE4zcEaqYgwHuY8Kr6ab3y7Sc16Ms4sR/{id}.json") 
  {
  ...
  function mint(uint256 id,uint256 numberOfTokens) 
    public 
    payable 
    {
        ...
        uint256 salePrice;
        uint256 index = id - 1;
        require(minted[index] + numberOfTokens <= supplies[index], "Not enough supply");
        //Private Sale
            salePrice=privaterates[index];
    ...
    function uri(uint256 id) override public pure returns(string memory)
    {
        return string(
            abi.encodePacked(
                "https://gateway.pinata.cloud/ipfs/QmWFp5PRgz1KJUGE4zcEaqYgwHuY8Kr6ab3y7Sc16Ms4sR/",
                Strings.toString(id),
                ".json"
            )
        );
