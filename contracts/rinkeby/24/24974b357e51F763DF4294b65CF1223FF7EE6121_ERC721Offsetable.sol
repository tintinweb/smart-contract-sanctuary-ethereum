...
contract ERC721Offsetable is ERC721AQueryableUpgradeable, OwnableUpgradeable, Offsetable {
     ...
     * @param _uri Token URI to return before reveal
    ...
    function tokenOffset() public view returns (uint256) {
        if (config.bypassOffset) revert ERC721Offsetable_TokenOffsetDisabled();
     ...
     * @param _config Struct with updated configuration values
     */
    ...
    function privateMint(uint256 amount, bytes memory signature) external payable {
        if (config.status != MintStatus.PRIVATE) revert ERC721Offsetable_InvalidMintStatus();
    ...
    function publicMint(uint256 amount) external payable {
        if (config.status != MintStatus.PUBLIC) revert ERC721Offsetable_InvalidMintStatus();
    ...
    function _mint(uint256 amount) internal {
        ...
        if (msg.value != config.price * amount) revert ERC721Offsetable_InvalidValue();
    ...
    function commitMetadata() external {
        if (!config.revealed) revert ERC721Offsetable_MetadataNotRevealed();
