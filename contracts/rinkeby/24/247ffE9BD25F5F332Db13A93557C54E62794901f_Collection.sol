 ...
 */
contract Collection is ProxyBaseStorage, ERC721A, Ownable, CollectionStorage{
 ...
 */
  ...
  function buyBox(uint256 _roundId)  payable public {
    Round storage tempRound = roundInfo[_roundId];
    userInfo storage tempUser = user[msg.sender][_roundId];
    require(tempRound.isPublic || tempUser.isWhiteList , "not authorize");
    require(tempRound.startTime <= block.timestamp, "series not started");  
    require(tempRound.endTime >= block.timestamp, "series has ended"); 
    require(tempRound.maxSupply > tempRound.supply.current(), "soldout");
    require(tempRound.userBuyLimit == 0 || tempRound.userBuyLimit > tempUser.purchases.current(), "limit reach");
    uint256 depositAmount = msg.value;
    uint256 price = tempRound.price;
    require(price <= depositAmount, "NFT 108");
        
        _safeMint(msg.sender, tempRound.limit);
        tempRound.supply.increment();
        tempUser.purchases.increment();
