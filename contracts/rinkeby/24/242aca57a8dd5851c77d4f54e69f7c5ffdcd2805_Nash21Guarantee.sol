...
contract Nash21Guarantee is INash21Guarantee, UUPSUpgradeableByRole {
    ...
    bytes32 private constant _GUARANTEE_ADMIN_ROLE = keccak256('GUARANTEE_ADMIN_ROLE');
    ...
    /// @inheritdoc INash21Guarantee
    mapping(bytes32=>address) public priceFeed;
    ...
    function claim(
        ...
        bytes32 currency = factoryInterface.getCurrency(id);
        tokenInterface.transfer(account, transformCurrency(currency, amount));
    ...
    ) public onlyRole(_GUARANTEE_ADMIN_ROLE) {
        require(currencies.length == feeds.length, 'Nash21Guarantee: arrays are not the same size');
    ...
    function transformCurrency(
        ...
        AggregatorV3Interface aggregatorInterface = AggregatorV3Interface(priceFeed[currency]);
    ...
    function released(
        ...
        INash21Factory factoryInterface = INash21Factory(INash21Manager(manager).get(keccak256('FACTORY')));
        return factoryInterface.getReleased(id);
    }
    ...
    function pay(
        ...
        INash21Factory factoryInterface = INash21Factory(INash21Manager(manager).get(keccak256('FACTORY')));
        IERC20 tokenInterface = IERC20(INash21Manager(manager).get(keccak256('USDT')));
        uint256 originId = factoryInterface.getOrigin(id);
        uint256 left = factoryInterface.getValue(originId) - paid[originId];
        bytes32 currency = factoryInterface.getCurrency(originId);
        address account = msg.sender;

        if (amount < left) {
            paid[originId] += amount;
            tokenInterface.transferFrom(account, address(this), transformCurrency(currency, amount));
            emit Pay(originId, account, amount, currency);
        } else {
            require(left > 0, "Nash21Guarantee: token already paid");
            paid[originId] += left;
            tokenInterface.transferFrom(account, address(this), transformCurrency(currency, left));
    ...
    function split(
        ...
        (uint256 id1, uint256 id2) = factoryInterface.split(account, id, timestamp);
        uint256 value1 = factoryInterface.getValue(id1);

        if(distributed[id] > value1) {
            distributed[id1] = value1;
            distributed[id2] = distributed[id] - value1;
        } else {
            distributed[id1] = distributed[id];
        }
    ...
    function _setFeed(bytes32 currency, address feed) internal {
        priceFeed[currency] = feed;
        emit NewFeed(currency, feed);
    }

    function _transformUsd(uint256 usd) internal view returns (uint256) {
        AggregatorV3Interface aggregatorInterface = AggregatorV3Interface(INash21Manager(manager).get(keccak256('FEED_USDT_USD')));
...
interface INash21Factory is IERC721EnumerableUpgradeable {
  ...
  /// @param hashId Hashed contract identifier
  ...
  /// @notice Returns the token id (0 when not mapped) of a contract identifier
  /// @param hashId Contract identifier
  /// @return Token identifier
  function hashMap ( bytes32 hashId ) external view returns ( uint256 );
  ...
  /// @notice Returns whether the creation of a token for contract identifier is unauthorized or not
  /// @param hashId Hashed contract identifier
  /// @return Whether the creation of a token for a contract identifier is unauthorized or not
...
interface INash21Guarantee {
  ...
  /// @notice Returns the pending amount to be released for a token
  /// @param id Token identifier
  /// @return Pending amount to be released
  ...
  function priceFeed ( bytes32 currency ) external view returns ( address );

  /// @notice Returns the released amount for a token
  /// @param id Token identifier
