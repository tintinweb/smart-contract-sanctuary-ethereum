...
contract NFT is ERC721, Ownable {
    ...
    // uint256 public constant price = 0.04e18; //0.04 ETH
    uint256 public price = 0.04e18; //0.04 ETH
    uint256 public constant PRESALE_TIME_START = 1650249300; // 2022/04/15 10:35
    uint256 public constant SALE_TIME_START = 1650250200; // 2022/04/18 10:50
    uint public constant maxNFTPresalePurchase = 2;
    ...
    constructor(uint256 maxNFTSupply, uint256 _presaleSupply, address payable fundReceiver) ERC721("NFT", "NFT") {
    ...
    function mint(uint numberOfTokens) external payable {
        ...
        for(uint i = 1; i <= numberOfTokens; i++) {
            _mint(msg.sender, _nowTokenId + i);
        }
    ...
    function presaleMint(uint numberOfTokens, bytes32[] memory proof) external payable {
        // Gas optimization
        uint256 _nowTokenId = nowTokenId;
        ...
        require(_nowTokenId + numberOfTokens <= presaleSupply, "Purchase would exceed max supply of presale");
        require(price * numberOfTokens == msg.value, "Ether value sent is not correct");
        require(presaleListPurchases[operator] + numberOfTokens <= maxNFTPresalePurchase, 'Each person can only mint 2 tokens');
