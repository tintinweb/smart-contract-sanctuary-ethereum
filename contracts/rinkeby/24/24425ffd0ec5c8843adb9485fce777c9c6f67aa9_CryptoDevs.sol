...
contract CryptoDevs is ERC721Enumerable, Ownable {
     ...
     * 用于计算 {tokenURI}。 如果设置，每个令牌的结果 URI 将是 `baseURI` 和 `tokenId` 的连接。
     */
    ...
    // 白名单合约实例
    MerkleTreeWhiteList merkleTreeWhiteList;

    // 用于跟踪预售何时开始的布尔值
    bool public presaleStarted;

    // timestamp for even presale would end  结束预售的时间戳
     ...
     * 设置_baseTokenURI，初始化whitelist实例
     */
    constructor (string memory baseURI, address whitelistContract) ERC721("Crypto Devs", "CD") {
        _baseTokenURI = baseURI;
        // whitelist = IWhitelist(whitelistContract);
        merkleTreeWhiteList = MerkleTreeWhiteList(whitelistContract);
    }
    ...
    * @dev 开始预售白名单地址
    ...
    function startPresale() public onlyOwner {
        presaleStarted = true;
        // 将 presaleEnded 设置为当前时间 + 5 分钟
        presaleEnded = block.timestamp + 5 minutes;
    }

    function isWhiteList(bytes32[] memory proof,bytes32 leaf) private view  returns (bool){
        return merkleTreeWhiteList.verify(proof, leaf);
    }

    /**
     * @dev presaleMint 允许用户在预售期间为每笔交易铸造一个 NFT。
     */
    function presaleMint(bytes32[] memory proof, bytes32 leaf) public payable onlyWhenNotPaused {
        require(presaleStarted && block.timestamp < presaleEnded, "Presale is not running");
        // require(whitelist.whitelistedAddresses(msg.sender), "You are not whitelisted");
        require(isWhiteList(proof, leaf), "You are not whitelisted");

        require(tokenIds < maxTokenIds, "Exceeded maximum Cypto Devs supply");
        require(msg.value >= _price, "Ether sent is not correct");
        tokenIds += 1;
        //_safeMint 是 _mint 函数的更安全版本，因为它确保
        // 如果铸造的地址是一个合约，那么它知道如何处理 ERC721 代币
        // 如果铸造的地址不是合约，它的工作方式与 _mint 相同
    ...
    */
    ...
    /**
    * @dev baseURI 覆盖 Openzeppelin ERC721 实现，默认情况下它为 baseURI 返回一个空字符串
    */
