...
contract VhilsNFT is NFTEnumerable, ERC1155Holder {
  ...
  uint16 public constant PRESALE_SUPPLY = 8000;
  uint16 public constant TEAM_SUPPLY = 250;
  ...
  function presaleMint(uint16 _maxAmount, bytes calldata _signature, uint16 _amount) external payable {
    ...
    ITEAR(tearContract).safeTransferFrom(address(this), msg.sender, 0, tearReward, ""); // transfer alpha token
  ...
  function mint(uint16 _amount) external payable {
    ...
    require(_amount <= MAX_PER_TXN, "Exceed txn"); // txn limit
    require(balanceOf(msg.sender) + _amount <= MAX_PER_WALLET, "Exceed wallet"); // wallet limit
    require(presaleClaimed + saleClaimed + _amount + TEAM_SUPPLY <= MAX_SUPPLY, "Exceed supply"); // supply limit
  ...
  // verified
  function tear(uint16 _tokenId, uint8 _tearId, string calldata _uri, bytes calldata _signature) external {
    uint8 tearUnit = 1;
    ...
    bytes32 message = keccak256(abi.encodePacked(hashKey, newLevel, _tokenId, _uri, msg.sender));
  ...
  function eligibleByWhitelist(uint16 _maxAmount, address _account, bytes memory _signature, uint16 _amount) internal view returns (bool) {
    ...
    return validSignature(message, _signature) && whitelistClaimed[_account] + _amount <= _maxAmount;
  }
  ...
  function teamClaim() external onlyOwner {
    uint16 start = MAX_SUPPLY - TEAM_SUPPLY;
    for (uint16 i = start; i < MAX_SUPPLY; i++) {
      _mint(msg.sender, i);
    }
  }
}
...
abstract contract NFTEnumerable is BaseControl, IERC721Enumerable, ERC721Royalty, ERC721Burnable {
  ...
  constructor() ERC721("Vhils NFT", "VHN") {
