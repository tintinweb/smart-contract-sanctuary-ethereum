...
contract ERC721 {
    ...
    function balanceOf(address payable owner) public view returns(uint256) {
        require(owner != address(0), "Address is zero");
        return _balances[owner];
    }

    // Finds the owner of an NFT
    function ownerOf(uint256 tokenId) public view returns(address payable) {
    ...
    function transferFrom(address from, address payable to, uint256 tokenId) public {
        address owner = ownerOf(tokenId);
        require(
            msg.sender == owner ||
    ...
    function safeTransferFrom(address from, address payable to, uint256 tokenId, bytes memory _data) public {
...
contract Anime is ERC721 {

    string public nameNFT; // ERC721Metadata 

    string public symbolNFT; // ERC721Metadata
    ...
    function mintNFT(string memory _tokenURI) public {
        ...
        _owners[tokenCount] = payable(msg.sender);
