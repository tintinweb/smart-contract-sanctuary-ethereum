...
abstract contract TTokenStorage is ITToken {
    ...
    mapping(address => mapping(uint256 => BorrowSnapshot))
...
abstract contract TErc20Interface is TErc20Storage {
    ...
    function repayBorrow(uint256 repayAmount, uint256 positionId)
        external
        virtual
        returns (uint256);
    ...
    ReentrancyGuardUpgradeable
{
    ...
    {
        return
            transferTokens(_msgSender(), _msgSender(), dst, amount) ==
            NO_ERROR;
    }
    ...
    function transferFrom(
        ...
        return transferTokens(_msgSender(), src, dst, amount) == NO_ERROR;
    }
     ...
     */
    function borrowBalanceStoredInternal(address account, uint256 positionId)
        internal
        view
        returns (uint256)
    {
    ...
    function mintInternal(uint256 mintAmount) internal nonReentrant {
        ...
        mintFresh(_msgSender(), mintAmount);
    }
    ...
    function mintFresh(address minter, uint256 mintAmount) internal {
        ...
        require(mintTokens > 0, "Mint amount is too small");
        /*
    ...
    function redeemInternal(uint256 redeemTokens) internal nonReentrant {
        ...
        redeemFresh(payable(_msgSender()), redeemTokens, 0);
    }
        ...
        nonReentrant
    {
        accrueInterest();
        // redeemFresh emits redeem-specific logs on errors, so we don't need to
        redeemFresh(payable(_msgSender()), 0, redeemAmount);
    }
     ...
     */
    function borrowInternal(uint256 borrowAmount, uint256 positionId)
        internal
        nonReentrant
    {
        accrueInterest();
        // borrowFresh emits borrow-specific logs on errors, so we don't need to
        borrowFresh(payable(_msgSender()), borrowAmount, positionId);
    }
     ...
     * @param positionId position identifier for which the debt is taken.
     */
    function repayBorrowInternal(uint256 positionId) internal nonReentrant {
        accrueInterest();
        // repayBorrowFresh emits repay-borrow-specific logs on errors, so we don't need to
        repayBorrowFresh(_msgSender(), _msgSender(), positionId);
    }
    ...
    function repayBorrowBehalfInternal(
        ...
        accrueInterest();
        // repayBorrowFresh emits repay-borrow-specific logs on errors, so we don't need to
        repayBorrowFresh(_msgSender(), borrower, positionId);
    }
     ...
     *  Its absolutely critical to use _msgSender() as the borrowed tToken and not a parameter.
    ...
    function seize(
        ...
        seizeInternal(_msgSender(), liquidator, borrower, seizeTokens);
     ...
     *  Its absolutely critical to use _msgSender() as the seizer tToken and not a parameter.
    ...
    /**
     * @notice Accrues interest and reduces reserves by transferring from _msgSender()
        ...
        returns (uint256, uint256)
    {
        ...
        actualAddAmount = doTransferIn(_msgSender(), addAmount);
        ...
        emit ReservesAdded(_msgSender(), actualAddAmount, totalReservesNew);
    ...
    PausableUpgradeable
{
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            results[i] = uint256(addToMarketInternal(tToken, _msgSender()));
        }
        ...
        returns (uint256)
    {
        ...
        (uint256 oErr, uint256 tokensHeld, uint256 amountOwed, ) = tToken
            .getAccountSnapshot(_msgSender(), 0);
        ...
        if (!marketToExit.accountMembership[_msgSender()]) {
            return uint256(Error.NO_ERROR);
        }

        /* Set tToken account membership to false */
        delete marketToExit.accountMembership[_msgSender()];

        /* Delete tToken from the account’s list of assets */
        // load into memory for faster iteration
        TToken[] memory userAssetList = accountAssets[_msgSender()];
        ...
        TToken[] storage storedList = accountAssets[_msgSender()];
        storedList[assetIndex] = storedList[storedList.length - 1];
        storedList.pop();

        emit MarketExited(tToken, _msgSender());
    ...
    function borrowAllowed(
        ...
        if (!markets[tToken].accountMembership[borrower]) {
            ...
            require(_msgSender() == tToken, "sender must be tToken");

            // attempt to add borrower to the market
            Error err = addToMarketInternal(TToken(_msgSender()), borrower);
            if (err != Error.NO_ERROR) {
                return uint256(err);
            }
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            IPositionRouter.PositionType.OPENED_LONG_COMPLITE &&
            position.positionType !=
            IPositionRouter.PositionType.OPENED_SHORT_COMPLITE
        ) {
            return 0;
        }
