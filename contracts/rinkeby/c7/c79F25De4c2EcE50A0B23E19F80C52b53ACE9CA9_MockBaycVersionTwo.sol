...
import "./IMockBaycCoin.sol";

contract MockBaycVersionTwo is
    Ownable,
    ERC2981,
    ERC721AQueryable,
    ReentrancyGuard
{
    ...
    uint256 public MAX_PER_HOLDER = 4444;
    uint256 public COIN_PER_MINT = 4000000000000000000000000000;
    ...
    IERC721 public immutable VERSION_ONE_CONTRACT;
    IMockBaycCoin public immutable MOCK_BAYC_COIN;
    ...
    constructor() ERC721A("MockBaycVersionTwo", "MBV2") {
        _setDefaultRoyalty(owner(), ROYALTY_PERCENTAGE);
        VERSION_ONE_CONTRACT = IERC721(
            0x932879Be48A0eeC73A945BDAE2B25BB5c0a49A6f
        );
        MOCK_BAYC_COIN = IMockBaycCoin(
            0xF13aC3315fdC0C6f5cF3a46ACaa0029aBd6d56cc
        );
    }
    ...
    function setIsClaimActive(bool isActive) external virtual onlyOwner {
        IS_CLAIM_ACTIVE = isActive;
    }
    ...
    function versionOneOwnerMint(uint256[] memory tokenID) external {
        require(IS_CLAIM_ACTIVE, "Claim not active");

        require(
            tokenID.length > 0 && tokenID.length % 4 == 0,
            "Invalid token to burned amount"
        );

        uint32 quantity = uint32(tokenID.length / 3);
        require(
            quantity + _holderClaimed <= MAX_PER_HOLDER,
            "Max supply reached"
        );

        MOCK_BAYC_COIN.holderClaim(msg.sender, quantity * COIN_PER_MINT);
