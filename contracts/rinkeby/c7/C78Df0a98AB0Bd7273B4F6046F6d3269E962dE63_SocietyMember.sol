    ...
    ReentrancyGuard
{
    ...
    function migrateMint(uint256 oldTokenId) external nonReentrant {
        updateMode();
        require(
            mode == Mode.SellingPreThreshold ||
                mode == Mode.SellingPostThreshold,
        ...
        require(
            memberCount() + 3 <= MAXIMUM_TOTAL_MEMBER_COUNT,
            "not enough memberships remaining"
        );
        require(
        ...
        require(
            oldMemberK.ownerOf(oldTokenId) == msg.sender,
            "sender must own the old token"
        );
        require(
            oldMemberK.getApproved(oldTokenId) == address(this),
        ...
        // It is transferred upon reaching the sales threshold. See #transferPending()
    ...
    function transferPending(uint256 tokenId) external nonReentrant {
        updateMode();
        require(
    ...
    function setArtSale(IInitialArtSale _artSale) external onlyOwner {
        artSale = _artSale;
    }
