...
contract StakingOBS is Ownable {
    ...
    constructor(address _tokenAddress, uint _stakingAndDaoTokens) {
    ...
    uint public constant cliffTime = 0 days; // TODO - temp for check

    uint public totalClaimedRewards = 0;

    EnumerableSet.AddressSet private holders;

    uint depositedTokensTotal;
    ...
    function updateAccount(address account) private {
        ...
        if (pendingDivs > 0) {
            ...
            require(Token(tokenAddress).balanceOf(address(this)) >= depositedTokensTotal, "We can't pay from deposited tokens");
    ...
    function deposit(uint amountToStake) public {
        ...
        depositedTokensTotal = depositedTokensTotal.add(amountAfterFee);

        if (!holders.contains(msg.sender)) {
            holders.add(msg.sender);
            stakingTime[msg.sender] = block.timestamp;
            lastClaimedTime[msg.sender] = block.timestamp;
        } else updateAccount(msg.sender);
    }
    ...
    function emergencyWithdraw(uint amountToWithdraw) public {
        ...
        depositedTokensTotal = depositedTokensTotal.sub(amountToWithdraw);
