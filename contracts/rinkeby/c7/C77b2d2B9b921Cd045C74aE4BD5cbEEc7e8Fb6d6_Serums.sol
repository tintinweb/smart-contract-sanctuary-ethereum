...
contract Serums is ERC1155, Ownable {
  ...
  address private wlAuthorizer;
  address public mutantContract;

  bool private ogClaimOpen;
  ...
  function batchClaim(uint[] memory serumIds, uint[] memory claimAmounts, bytes memory signature) external  {
    if(!ogClaimOpen) revert SaleNotStarted();
  ...
  function whitelistMint(uint serumId, uint amount,uint max,bytes memory signature) external payable {
    ...
    if(hash.toEthSignedMessageHash().recover(signature) != wlAuthorizer) revert NotAuthorized();
...
function setWlAuthorizer(address account) external onlyOwner{
    wlAuthorizer = account;
  }

  function setOgClaim(bool status) external onlyOwner{
    ogClaimOpen = status;
  }
