...
contract KnightMarket is KnightAttack {
    struct Bid {
        uint knightID;
        uint price;
        uint32 timeEnd;
    ...
    mapping(uint => uint) public saleInBidID;

    event SaleKnight(uint knightID, uint price, uint bidID, address owner, uint32 timeEnd);
    event BuyKnight(uint knightID, uint price, uint bidID, address newOner);
    event DestroySaleKnight(uint bidID, address owner);

    modifier checkKnightIsSale(uint _knightID, address _owner) {
         require(knightIsSalling[_owner][_knightID] == false, "Knight is Salling!");
        _;
    }
    ...
    function saleKnight(uint _knightID, uint _amount, uint32 _time) 
        ...
        checkKnightIsSale(_knightID, _msgSender())
    {
        ...
        knightIsSalling[_msgSender()][_knightID] = true; 
        saleInBidID[_knightID] = bidCount;
        emit SaleKnight(_knightID, _amount, bidCount, msg.sender, biddingEnd);
    }
    ...
    {   
        ...
        if(currentBid.timeEnd <= block.timestamp){
        ...
        _transfer(currentBid.owner, msg.sender, currentBid.knightID);
        payable(currentBid.owner).transfer(currentBid.price);
        delete biddingList[_bidID];
        delete saleInBidID[currentBid.knightID];
        delete knightIsSalling[currentBid.owner][currentBid.knightID];
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public virtual override checkKnightIsSale(tokenId, _msgSender()) {
    ...
    function approve(address to, uint256 tokenId) public virtual override checkKnightIsSale(tokenId, _msgSender()) {
...
contract KnightFamily is KnightHelper {
    ...
    event TriggerTired(uint _knightID, uint _timeOut);
        ...
        onlyOwnerOfKnight(_knightMotherID) 
    {
        ...
        if(_resoult) {
            ...
            address ownerKnightFather = ownerOf(_knightFatherID);
            address ownerKnightMother = ownerOf(_knightMotherID);
    ...
    function destroyMarry(uint _knightFatherID, uint _knightMotherID) 
        external  
    {
        require(ownerOf(_knightFatherID) == msg.sender || ownerOf(_knightMotherID) == msg.sender,"Invalid owner of knight");
        ...
        onlyOwnerOfKnight(_knightFatherID)
    {
        ...
        address from = ownerOf(_knightFatherID);
        address to = ownerOf(_knightMotherID);
    ...
    function interCourseKnight(uint _knightFatherID, uint _knightMotherID) 
        external 
        aboveLevel(1, _knightFatherID) 
        aboveLevel(1, _knightMotherID) 
        ...
        internal 
    {
        ...
        if(randWhoParent <= 5) 
        {
            _createKnight("BabyKnight", newDna, uint8(randGender), avgLevel, ownerOf(_knightFatherID));
        } else  {
            _createKnight("BabyKnight", newDna, uint8(randGender), avgLevel, ownerOf(_knightMotherID));
        }
...
contract KnightHelper is KnightFeeding {
    ...
    event ChangeDNA(uint _knightID, uint _newDna);
        ...
        onlyOwnerOfKnight(_knightID)
    {
        ...
        emit ChangeDNA(_knightID, _newDna);
    }
    ...
    {
        Knight[] memory listKnight = new Knight[](balanceOf(_owner));
        uint counter = 0;
    ...
    function getAllKnightsNotOwner(address _owner) 
        external view 
        returns(Knight[] memory)
    {
        Knight[] memory listKnight = new Knight[](listOfKnight.length - balanceOf(_owner));
        uint counter = 0;
        for (uint i = 0; i < listOfKnight.length; i++) {
            if(ownerOf(i) != _owner){
                listKnight[counter]= listOfKnight[i];
        ...
        external payable 
    {
        ...
        emit LevelUp(_knightID, listOfKnight[_knightID].level);
    }
        ...
        external payable 
    {
        ...
        emit ExcitementPointUp(_knightID,  myKnight.excitementPoint);
    }
...
contract KnightFeeding is KnightFactory {
    ...
    event TriggerCoolDown(uint _knightID, uint _timeOut);
    ...
    function _triggerCoolDown(Knight storage _knight) internal {
        ...
        emit TriggerCoolDown(_knight.id, _knight.readyTime);
    }
...
contract KnightFactory is KnightToken {
    using Strings for uint256;
    using Counters for Counters.Counter;

    Counters.Counter internal _tokenIdCounter;
    string baseURIKnight = "ipfs://QmcZFo1X7KJkaVnKAsTcDXr9cG7YMEy1G3UBhmuacNgHFA/";
    uint dnaKnight = 16;
    ...
    event NewKnight(uint knightID, string name, uint dna, string tokenURI, address owner, uint32 level,  uint32 readyTime,  uint32 sexTime, uint8 gender);
    ...
    function _createKnight(string memory _name, uint _dna, uint8 _gender, uint32 _level, address _owner) internal {
        ...
        uint32 attackTime = uint32(block.timestamp + coolDownTime);
        uint32 sexTime = uint32(block.timestamp + coolDownSex);
        listOfKnight.push(Knight(_name, _dna, knightID, _level, attackTime, sexTime, 0, 0, 0, _gender));
        _tokenIdCounter.increment();
        _safeMint(_owner, knightID);
        uint randUri = randTokenUri(amountOfpage, knightID);
        _setTokenURI(knightID, string(abi.encodePacked(baseURIKnight, Strings.toString(randUri), ".json")));
        emit NewKnight(knightID, _name, _dna, tokenURI(knightID), _owner, _level, attackTime, sexTime, _gender);
    }
    ...
    function createKnight(string memory _name, uint8 _gender) public {
        require(initKnight[_msgSender()] == false, "Create a knight only for the first time");
        uint dnaRand = _generateRandomDan(_name);
        _createKnight(_name, dnaRand, _gender, 1, msg.sender);
        initKnight[_msgSender()] = true; 
    }
    // ERC721URIStorage
...
contract KnightToken is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("KnightNFT_V", "KNV") {}
