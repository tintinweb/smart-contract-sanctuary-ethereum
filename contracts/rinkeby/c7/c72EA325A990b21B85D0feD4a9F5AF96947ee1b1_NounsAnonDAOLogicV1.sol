...
import "./NounsAnonDAOInterfaces.sol";

contract NounsAnonDAOLogicV1 is NounsAnonDAOStorageV1, NounsAnonDAOEvents {
    ...
    uint256 public constant MIN_VOTING_PERIOD = 1; // 5_760; // About 24 hours (4 * 60 * 24)
     ...
     * @param semaphore_ The address allowed of the semaphore contract that ensures votes are valid
    ...
    function initialize(
        ...
        vetoer = vetoer_;
        semaphore = semaphore_; // I added for semaphore contract
    ...
    function propose(
        ...
        // NOTE: i added to check if is valid
        if (isAnonVoting) {
            require(
                semaphore != address(0),
                "NounsDAO::propose: semaphore address has not been set"
            );
        }
        ...
        newProposal.isAnonVoting = isAnonVoting;
    ...
    {
        require(
            proposals[proposalId].isAnonVoting == true,
            "NounsDAO::getReciept: there are no reciepts in anonymous proposals."
        );
    ...
    function castAnonVote(
        ...
        emit AnonVoteCast(
            proposalId,
            support,
            castAnonVoteInternal(proposalId, support),
            reason
        );
    }
    ...
    function castVoteInternal(
        ...
        Proposal storage proposal = proposals[proposalId];
        require(
            proposal.isAnonVoting == false,
            "NounsDAO::castVoteInternal: castVoteInternal cannot be called for anonymous votes."
        );
     ...
     * @return The number of votes cast
     */
    function castAnonVoteInternal(uint256 proposalId, uint8 support)
        internal
        returns (uint96)
    {

        require(semaphore != address(0), "NounsDAO::castAnonVoteInternal: the semaphore contract is not set.");

        require(msg.sender == semaphore, "NounsDAO::castAnonVoteInternal: only the semaphore contract can this function");

        require(
            state(proposalId) == ProposalState.Active,
            "NounsDAO::castAnonVoteInternal: voting is closed"
        );
        require(support <= 2, "NounsDAO::castAnonVoteInternal: invalid vote type");
        Proposal storage proposal = proposals[proposalId];
        require(
            proposal.isAnonVoting == true,
            "NounsDAO::castAnonVoteInternal: proposal was not configured for anonymous voting."
        );

        if (support == 0) {
            proposal.againstVotes = proposal.againstVotes + 1; //assuming that vote is 1 per noun
        } else if (support == 1) {
    ...
    function _setVetoer(address newVetoer) public {
     ...
     /**
     * @notice Changes semaphore address
     * @dev Semaphore function for updating semaphore address
     */
    function _setSemaphore(address newSemaphore) public {
        require(msg.sender == semaphore, "NounsDAO::_setSemaphore: semaphore only");

        emit NewSemaphore(semaphore, newSemaphore);
...
contract NounsAnonDAOEvents {
    ...
    /// @notice Emitted when semaphore is changed (I added)
    event NewSemaphore(address oldSemaphore, address newSemaphore);
}

contract NounsAnonDAOProxyStorage {
 ...
 * NounsDAOStorageVX.
 */
contract NounsAnonDAOStorageV1 is NounsAnonDAOProxyStorage {
    /// @notice Semaphore who has the ability to check that anonymous votes are valid
    ...
    struct Proposal {
        ...
        bool isAnonVoting; // Flag that marks whether the proposal uses anonymous voting NOTE: I added
    }
