...
import "@nefty/contracts/BridgedERC721.sol";
import "@thirdweb-dev/contracts/extension/SignatureMintERC721.sol";

contract BridgedAlpacas is BridgedERC721 {
    ...
    constructor(
        ...
        uint128 _royaltyBps
    ) BridgedERC721(_name, _symbol, _royaltyRecipient, _royaltyBps) {}
...
import "../contracts/ERC721Base.sol";
...
abstract contract BridgedERC721 is ERC721Base, EIP712, IERC721Receiver {
    using ECDSA for bytes32;

    bytes32 private constant TYPEHASH =
        keccak256(
            "BridgeRequest(address to,string uri,uint128 expirationTimestamp,uint64 assetId,bytes32 uid)"
        );

    mapping(bytes32 => bool) private usedSignature;
    mapping(uint64 => uint256) private assetIdMapping;
    mapping(uint256 => string) private fullURI;
    mapping(uint64 => uint64) private assetIdOwner;
     ...
     *  @param expirationTimestamp The unix timestamp at which the payload expires.
    ...
    constructor(
        ...
        EIP712("BridgedERC721", "1")
    {}

    /// @dev Start from token id 1 to ensure 0 doesn't exist
    function _startTokenId() internal view virtual override returns (uint256) {
        return 1;
    }

    function getTokenIdByAssetId(uint64 _assetId)
        external
        view
        returns (uint256)
    {
        return assetIdMapping[_assetId];
    }

    function bridgeMatches(uint256 _tokenId, uint64 _assetId)
        external
        view
        returns (bool)
    {
        return assetIdMapping[_assetId] == _tokenId;
    }
        ...
        returns (bool success, address signer)
    {
        ...
        success = !usedSignature[_req.uid] && _canSignMintRequest(signer);
    }
    ...
    function _processRequest(
        ...
        if (block.timestamp > _req.expirationTimestamp) {
            revert("Req expired");
        }

        usedSignature[_req.uid] = true;
    }
    ...
    function _encodeRequest(BridgeRequest calldata _req)
        internal
        pure
        ...
        returns (uint256 tokenId)
    {
        ...
        delete assetIdOwner[_req.assetId];
        if (assetIdMapping[_req.assetId] == 0) {
            // Get a new token id
            tokenId = nextTokenIdToMint();

            // Set asset id and URI
            assetIdMapping[_req.assetId] = tokenId;
            fullURI[tokenId] = _req.uri;

            // Mint
            _mint(_req.to, 1);
        } else {
            tokenId = assetIdMapping[_req.assetId];
            require(
            ...
            ERC721A(address(this)).safeTransferFrom(
                address(this),
                _req.to,
                tokenId
            );
    ...
    function bridgeOut(
        ...
        require(
            assetIdMapping[_assetId] == _tokenId,
            "No asset mapped to token"
        );
        assetIdOwner[_assetId] = _owner;
        safeTransferFrom(msg.sender, address(this), _tokenId);
    }

    function getAssetIdOwner(uint64 _assetId) external view returns (uint64) {
        return assetIdOwner[_assetId];
    }
