...
/*
# python - please copy/paste and run to prove provenenance
...
contract SeasonTwoSale is ERC721A, Ownable, VRFConsumerBase{
    ...
    uint256 public StartingIndex = MAX_SUPPLY;
    ...
    function devMint(uint256 _quantity, address _addr) public onlyOwner{
        require(_quantity.add(totalSupply()) <= MAX_SUPPLY, "sale closed");
    ...
    function revealCollection(string memory _hash) public {
        require(StartingIndex == MAX_SUPPLY, "StartingIndex already set");
        BaseHash = _hash;
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory) {
        ...
        if (StartingIndex == MAX_SUPPLY) {
            return string(abi.encodePacked('ipfs://', PlaceholderHash));
        } else {
            uint256 id = tokenId.add(StartingIndex) % MAX_SUPPLY;
    ...
    function remainingToMint(bytes32[] calldata _merkleProof, address _addr) public view returns(uint256) {
        ...
        if (MerkleProof.verify(_merkleProof, MerkleRoot[CurrentPhase], leaf) && !PublicSaleOpen) {
            return MaxMintPerPhase[CurrentPhase] - WhitelistMinted[_addr][CurrentPhase];
        }
        else{
            return PublicSaleOpen ? MaxPublicMint : 0;
        }
    }
    ...
    function testRandom(uint256 randomness) public view returns (uint256) {
        return (randomness % MAX_SUPPLY);
    }
}
