    ...
    ,INonDilutive
{
    ...
    bool public whitelistMintOpen = true; //setting to true for testing
    ...
    function isMessageValid(bytes memory _signature) public virtual
    {
        // Validate signature 
     ...
     */
    function whitelistMint(bytes memory _signature, uint256 quantity) public virtual payable {
        // Validate signature 
        if (signer != keccak256(abi.encodePacked(address(this), msg.sender)
            ).toEthSignedMessageHash().recover(_signature)) revert SignatureNotValid();
        ...
        require(_numberMinted(msg.sender) + quantity < maxMintPerWallet + 1, "Max mint 3 per wallet");
    ...
    // Update max each wallet can mint
    function setMaxMint(uint256 _amount) 
        external 
        onlyOwner 
    {
    ...
    ,Ownable
{
        ...
        onlyOwner
    {
        ...
        if(generation.offset == 0) {
            // Keep the same offset per layer so we don't dilute rarity at a layer level. 
            if (_layerId > 0) {
                generation.offset = generations[0].offset;
            } else {
