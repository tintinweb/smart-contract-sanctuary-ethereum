...
Begin!!!
*/
contract StefansCollegeGraduateCertificate is ERC721, ERC721Enumerable, Ownable {
    uint256 constant public TOKEN_LIMIT = 0x5; //0x17;
    string private _baseURIextended;
    // address payable public immutable shareholderAddress;
    mapping(address => bool) public whitelist;
    uint256[TOKEN_LIMIT] public _tokenIndexMap;

    constructor() ERC721("Stefans College Graduate Certificate", "StefansCollegeGraduateCertificate") {
        // require(shareholderAddress_ != address(0));
        // shareholderAddress = shareholderAddress_;
        for (uint i=0;i<TOKEN_LIMIT; i++){
            _tokenIndexMap[i] = i+1;
        }
    ...
    /**
    1. 每个白名单只能mint一个
    2. 每次mint都会随机一个tokenId
    */
    function mint() public needWhitelisted() {
        ...
        require(totalSupply() + 1 <= TOKEN_LIMIT, "All NFTs have been minted");
        // uint256 remaining = TOKEN_LIMIT - totalSupply();
        ...
        uint256 randomIndex;
        uint256 indexValue;
        (randomIndex, indexValue) = useRandomAvailableToken();
        _safeMint(msg.sender, indexValue);

        delete _tokenIndexMap[randomIndex];
        // mint之后就不再是白名单了
        whitelist[_msgSender()] = false;
        require(totalSupply() <= TOKEN_LIMIT, "limit reached");
    }

    function useRandomAvailableToken() internal view returns (uint256, uint256)
    {
        uint256 randomNum = uint256(keccak256(abi.encode(msg.sender,tx.gasprice,block.number,block.timestamp,blockhash(block.number - 1), totalSupply())));
        uint256 remaining = TOKEN_LIMIT - totalSupply();
        uint256 randomIndex = randomNum % remaining;
        uint256 indexValue = _tokenIndexMap[randomIndex];
        return (randomIndex, indexValue);
    }
