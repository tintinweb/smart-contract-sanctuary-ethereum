...
contract TokenBridge is Ownable {
    ...
    bytes32 public constant VERIFY_TYPEHASH = keccak256("Verify(string functionName,address tokenAddress,uint32 amount,uint32 nonce)");
    ...
    function unlock(address tokenAddress, address receiver, uint32 amount, uint32 nonce, uint8 v, bytes32 r, bytes32 s) public onlyOwner returns (bool success, bytes memory result){
        ...
        require(verify("burn()", tokenAddress, amount, nonce, v, r, s) == owner());
    ...
    event Mint(bytes _tokenNativeAddress, uint32 _amount, address _receiver);
    event Burn(bytes _tokenNativeAddress, uint32 _amount, address _receiver, uint32 nonce);
    ...
    function mint(bytes memory tokenNativeAddress, address receiver, uint32 amount, uint32 nonce, WrappedTokenParams memory tokenParams, uint8 v, bytes32 r, bytes32 s) public onlyOwner returns (bool success, bytes memory result){
        require(amount > 0);
        ...
        require(verify("unlock()", receiver, amount, nonce, v, r, s) == owner());
    ...
    function burn(bytes memory tokenNativeAddress, uint32 amount) public returns (bool success, bytes memory result) {
        require(amount > 0);
    ...
    function verify(string memory functionName, address tokenAddress, uint32 amount, uint32 nonce, uint8 v, bytes32 r, bytes32 s) view public returns(address){
