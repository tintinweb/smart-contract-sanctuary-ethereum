    ...
    PausableUpgradeable
{
    ...
    function setNewMarketOrdersController(
        ...
        marketOrdersController = IMarketOrdersController(_marketOrdersController);
    }

    function setTTokenFactory(
        ITFactory _tFactory
    ) external onlyAdmin {
        tTokenFactory = ITFactory(_tFactory);
    }
    ...
    function setNewController(IController _newController) external onlyAdmin {
        ...
        returns (bytes32)
    {
        ...
        // Prepare data for borrow and mint
        ITErc20 tToken = ITErc20(
            tTokenFactory.getTToken(_params.collateralToken)
        );
        // Borrow
        tToken.borrow(
            borrowAmount,
            keccak256(abi.encode(_params.poolAddress, positionCounter))
        );
        ...
        _executionComplite(_params.poolAddress, orderId);
    }
        ...
        uint256 orderId = limitOrdersController.createOrder{value: msg.value}(
            ILimitOrdersController.OrderParams({
                ...
                amountOfToken1: orderType ==
                    ...
                    ? _params.collateralAmount + borrowAmount
                    : 0,
                recievedAmountOfToken0: orderType ==
                    ILimitOrdersController.OrderType.BUY_LIMIT
                    ...
                    : 0,
                recievedAmountOfToken1: orderType ==
                    ILimitOrdersController.OrderType.TAKE_PROFIT
    ...
    function closePositionUsingLimit(ConfirmParams memory _params)
        external
        payable
    {
        require(_params.deadline >= block.timestamp, "Deadline");
        ...
        require(
            ...
            "Position must be opened to be executed"
        );

        ILimitOrdersController.Order
            memory positionStatus = limitOrdersController.orders(
                position.poolAddress,
                position.openOrder.orderId
            );
        position.openOrder.receivedAmountOfToken0 = positionStatus
            .transferedToken0;
        position.openOrder.receivedAmountOfToken1 = positionStatus
            .transferedToken1;
        ILimitOrdersController.OrderType orderType = position
            ...
            : ILimitOrdersController.OrderType.BUY_LIMIT;
        IERC20Upgradeable(
            orderType == ILimitOrdersController.OrderType.TAKE_PROFIT
    ...
    function getPositionIdForExecutedOrder(address pool, uint256 orderId)
        internal
        view
        returns (uint256 positionId)
    {
        ...
        if (limitToPositionId[orderId] != 0) {
            ...
            position = positions[keccak256(abi.encode(pool, positionId))];
            if (
    ...
    function executionComplite(address pool, uint256 orderId) external {
        ...
        _executionComplite(pool, orderId);
    }

    function _executionComplite(address pool, uint256 orderId) internal {
        Position storage position;
        uint256 positionId = getPositionIdForExecutedOrder(pool, orderId);
        ...
        // repay:
        ITErc20 tToken = ITErc20(
            tTokenFactory.getTToken(position.collateralToken)
        );
