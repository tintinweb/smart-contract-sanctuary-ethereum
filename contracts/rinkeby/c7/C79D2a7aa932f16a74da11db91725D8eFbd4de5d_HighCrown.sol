...
import "./interfaces/IEmperorContract.sol";

contract HighCrown is ERC721, Ownable {
    ...
    address public emperorContract;
    ...
    bool public burnEnabled; //defaults to false
    ...
    // metadata URIs
    // TODO: set to private before deploy
    string public _contractURI; // initially set at deploy
    string public _notRevealedURI; // initially set at deploy
    ...
    modifier onlyAllowMintEnabledAndValidCount(uint256 _mintAmount) {
        require(
            _totalMintSupply + _mintAmount <= totalTokens,
            "Exceeds supply"
        );
    ...
    modifier onlyAllowBurnEnabledAndValidCount(uint256 _burnAmount) {
        require(
            _totalBurnSupply + _burnAmount <= totalBurnTokens,
            "Exceeds supply"
        );
     ...
     */
    function mint(
        uint256 _mintPassTokenId,
        uint256[] memory _burnTokenIds,
        uint16 _mintAmount
    ) external onlyAllowMintEnabledAndValidCount(_mintAmount) {
        // ensure they own the mint pass
        require(
            _msgSender() ==
    ...
    /**
     * @dev Burn tokens; mint emperor
     */
    function burnForEmperor(uint256[] memory _tokenIds)
        external
        onlyAllowBurnEnabledAndValidCount(_tokenIds.length)
    {
        ...
        IEmperorContract emperorTokenContract = IEmperorContract(
            emperorContract
        );
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            _burn(_tokenIds[i]);
     ...
     * @dev Setter for the Mint Pass contract address
     */
    function setEmperorContractAddress(address _emperorContract)
        external
        onlyOwner
    {
        require(_emperorContract != address(0), "Zero address");
        emperorContract = _emperorContract;
    }
     ...
     */
    function setMintEnable() public onlyOwner {
        require(
     ...
     */
    function setBurnEnable() external onlyOwner {
        burnEnabled = true;
    }
