...
contract Settings is AccessControl {
    ...
    bytes32[] public defaultSettings;

    mapping(uint256 => mapping(bytes32 => bytes32)) public settings;
    ...
    modifier checkKeys (bytes32[] calldata keys) {
    ...
    function updateDefaultSettings(bytes32[] memory _newDefaultSettings) public onlyAdmin {
        defaultSettings = _newDefaultSettings;
    }

    function getDeafaultSettings() public view returns(bytes32[] memory) {
        return defaultSettings;
    }

    function userInfo(address _user) view public returns(bytes32[] memory ) {
        bytes32[] memory setts = new bytes32[](defaultSettings.length);
