...
contract PeriFiCuration is Ownable, SignatureHelper {
    struct Vote {
        bool accept;
        bool bet;
    ...
    function castVote(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(curationEnd, msg.sender, curationHash)
        );
        ...
        require(cardLevel >= 1 && cardLevel < 5, "Invalid card level");

        require(
            PERIv2(vpcAddresses[cardLevel - 1]).revealChecked(cardId),
            "Card not releaved"
        );

        if (curationHashToEndTime[curationHash] != 0) {
            require(
                curationHashToEndTime[curationHash] == curationEnd,
        ...
        for (uint256 i = 0; i < votes[curationHash].length; i++) {
            if (votes[curationHash][i].voter == msg.sender) {
                revert("Already voted");
            }
        }

        require(
            vpcAddresses[cardLevel - 1] != address(0),
            "VPC address not set"
        );

        IERC721(vpcAddresses[cardLevel - 1]).transferFrom(
            msg.sender,
            address(this),
            cardId
        );
    ...
    function endCuration(
        ...
        require(!curationHashToEnded[curationHash], "Curation ended");

        require(
            curationHashToEndTime[curationHash] == 0 ||
                curationHashToEndTime[curationHash] >= block.timestamp,
            "Curation end time not reached"
        );

        curationHashToEnded[curationHash] = true;

        Vote[] storage _votes = votes[curationHash];

        uint256 numCardsRedeemed;
        uint256 numVotersRewarded;

        for (uint256 i = 0; i < _votes.length; i++) {
            Vote memory _vote = _votes[i];

            if (rejected || !_vote.bet || _vote.loanAmount == winningOption) {
                numCardsRedeemed += 1;
            }
        ...
        address[] memory votersCardRedeemed = new address[](numCardsRedeemed);
        address[] memory votersRewarded = new address[](numVotersRewarded);
        uint256[] memory rewards = new uint256[](numVotersRewarded);
        ...
        for (uint256 i = 0; i < _votes.length; i++) {
            ...
            if (rejected || !_vote.bet || _vote.loanAmount == winningOption) {
                _vote.vpcWithdrawn = true;
                votersCardRedeemed[redeemIdx++] = _vote.voter;
            ...
            if (!rejected && _vote.bet && _vote.loanAmount == winningOption) {
                uint256 reward = (_vote.cardLevel * totalRewards) /
                    winningPower;
                rewards[rewardIdx++] = reward;
                votersRewarded[rewardIdx] = _vote.voter;
                _vote.rewardsClaimed = true;

                Vault(payable(rewardPool)).transferERC20(
                    rewardToken,
                    _vote.voter,
                    reward
                );
