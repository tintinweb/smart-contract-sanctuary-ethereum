...
import "./OndoLinearTimelock.sol";

contract Ondo is AccessControl, OndoLinearTimelock {
  ...
  function _transferTokens(
    ...
    if (hasTimelockedOndo[src]) {
      require(
        amount >= getFreedBalance(src),
  ...
  function updateTrancheBalance(
    ...
    hasTimelockedOndo[beneficiary] = true;
    investorBalances[beneficiary] = InvestorParam(investorType, amount);
  }

  /**
   * @notice Gets the amount of unlocked Ondo for an account that participated in Coinlist/Seed Investments
   */
  function getFreedBalance(address account) public view returns (uint96) {
    if (passedAllVestingPeriods()) {
      ...
      if (passedCliff()) {
        ...
        (uint256 vestingPeriod, uint256 elapsed) =
          getTrancheInfo(investorParam.investorType);
...
contract OndoLinearTimelock {
  ...
  /// @notice the linear vestnig period for the Seed/Series A Tranche
  ...
  /// @dev mapping to check if address has timelocked tokens
  mapping(address => bool) internal hasTimelockedOndo;
  ...
  /**
    @notice View function to get the user's amount of unlocked tokens
   */
  function getVestedTokens(address account)
    external
    view
  ...
  function _proportionAvailable(
    ...
    if (investorParam.investorType == InvestorType.SeedTranche) {
