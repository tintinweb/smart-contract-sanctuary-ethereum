...
contract Karma is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public constant PUBLIC_MINT_SIZE = 10000;
    uint256 public constant MAX_MINT_PER_ALLOWLIST_SPOT = 2;
    uint256 public constant MAX_PUBLIC_MINT_PER_TXN = 20;
    uint256 public constant PUBLIC_ALLOCATION = 1000; // number to reserve for public mint

    uint256 public salePrice = 0.5 ether;
    uint256 public constant EATING_PRICE = 0.03 ether;
    uint256 public numKarmasMintedPublicMint  = 0; // Number of Minted Karmas (not from Desserts) by Public
    uint256 public numKarmasMintedOwnerMint   = 0; // Number of Minted Karmas (not from Desserts) by Owner
    uint256 public numKarmasMintedManagerMint = 0; // Number of Minted Karmas (not from Desserts) by MintManager
    // numKarmasMintedPublicMint + numKarmasMintedOwnerMint + numKarmasMintedManagerMint <= PUBLIC_MINT_SIZE

    uint256 public nextMintManagerTokenId = 9000; // next tokenId that mintManager will mint, only decrementing, above 9000 is Metagood allocation
    uint256 public minimumMintManagerTokenId = 9001; // minimum tokenId that mintManager can mint, 9001 means mintManager cannot mint
    ...
    address public mintManager; // can mint from special allocation from 9000 and decrementing
    address public missionManager; // can modify level and exp
    ...
    mapping(address => bool) private minted; // with allowlist
    ...
    //   Karma3: 30001-30015
    //
    // Mint Karma: 1-10000 (contains 5 Karma3 too)
    //
    //   AllowList:    1 to (minimumMintManagerTokenId - PUBLIC_ALLOCATION - 1) 
    //   Public:       (minimumMintManagerTokenId - PUBLIC_ALLOCATION) to (minimumMintManagerTokenId - 1)
    //   Mint Manager: minimumMintManagerTokenId to 9000
    //.  Metagood:     9001 to 10000
    //
    //   (mintManager can mint from 9000, decrementing)
    ...
    //function ownerClaim(address toAddress, uint256 karmaId) external nonReentrant onlyOwner {
    //    require(karmaId > 9000 && karmaId < 10001, "invalid claim");
    //    numKarmasMintedOwnerMint++;
    //    _safeMint(toAddress, karmaId);
    //}

    function ownerClaimQuantity(address toAddress, uint256 karmaId, uint256 quantity) external nonReentrant onlyOwner {
        require(karmaId > 9000 && (karmaId+quantity) <= 10001, "invalid claim");
        for(uint256 i=karmaId; i<karmaId+quantity; i++) { 
    ...
    function generalMint(uint256 numKarmas, bytes32[] calldata merkleProof) external payable nonReentrant notSmartContract {
        require(numKarmasMintedPublicMint + numKarmasMintedOwnerMint + numKarmasMintedManagerMint + numKarmas <= PUBLIC_MINT_SIZE, "Minting exceeds max supply");
        ...
        if (publicSaleActive) {
            ...
            require(numKarmasMintedPublicMint + numKarmas < minimumMintManagerTokenId, "reserved for mint manager");
        } else {
            require(allowListActive, "Allow list is not active");
            require(onAllowList(msg.sender, merkleProof), "Not on allow list");
            require(numKarmas <= MAX_MINT_PER_ALLOWLIST_SPOT, "Exceeds max mint");
            require(numKarmasMintedPublicMint + numKarmas < minimumMintManagerTokenId - PUBLIC_ALLOCATION, "reserved for public");
    ...
    function allowListNFTMint() external payable whenAllowListNFTActive nonReentrant notSmartContract {
        ...
        require(numKarmasMintedPublicMint + numKarmasMintedManagerMint + numKarmasMintedOwnerMint < PUBLIC_MINT_SIZE, "Minting exceeds max supply");
    ...
    function totalKarmaFromDesserts() external view returns (uint256) {
        return totalSupply() - numKarmasMintedPublicMint - numKarmasMintedManagerMint - numKarmasMintedOwnerMint;
    }
