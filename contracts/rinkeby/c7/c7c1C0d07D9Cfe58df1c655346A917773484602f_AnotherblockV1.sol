...
contract AnotherblockV1 is Ownable, ReentrancyGuard, ABErrors, ERC165Storage {
    ...
    function updateDropCounter(uint256 _dropId, uint256 _quantity) external {
        ...
        // For each new token, assign the corresponding ignoreAmounts
        for (uint256 i = 0; i < _quantity; i++) {
            ignoreAmounts[drop.firstTokenIndex + drop.sold + i] =
    ...
    /**
     NOTE 1 : Optimize looping to group the transfer on ERC20 payouts
     NOTE 2 : pass the tokenIds as parameters (remove enumerable)
    */
