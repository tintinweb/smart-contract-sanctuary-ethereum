...
contract Marketplace is Ownable, ReentrancyGuard{
    ...
    event MintEvent(address Caller, string pointerMoment, string transactionID, uint256 TimeStamp);
    ...
    function mint(address callerAddress, string memory pointerMoment, string memory transactionID, Sig memory mintRSV) public initializer{
        require(verifySigner(callerAddress, messageHash(abi.encodePacked(callerAddress, pointerMoment, transactionID)), mintRSV), "Sell rsv invalid");
        HDY721(NFTAddress).mint(callerAddress, pointerMoment);
        emit MintEvent(callerAddress, pointerMoment, transactionID, block.timestamp);
    }
    ...
    function buy(address callerAddress, Buy memory buyStruct, Sig memory buyRSV) public payable initializer {
        ...
        require(sent, "Failed to send Ether");
        ERC721(NFTAddress).safeTransferFrom(buyStruct.seller, buyStruct.buyer, buyStruct.tokenID);
...
contract HDY721 is ERC721A, Ownable {
    ...
    function mint(
        ...
        emit Minted(callerAddress, 1, pointerMoment);
    }
    ...
    function batchMint(address[] memory to, uint256 amount, string[] memory pointerMoment) external {
        ...
        for (uint256 i = 0; i < amount; i++) {
            _mint(to[i], 1);
            emit Minted(to[i], 1, pointerMoment[i]);
        }
    }
