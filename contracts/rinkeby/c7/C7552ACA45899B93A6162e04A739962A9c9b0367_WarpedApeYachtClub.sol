...
contract WarpedApeYachtClub is ERC721Enumerable, ContextMixin, NativeMetaTransaction, AuthorizedList {
    using Strings for string;
    using SafeMath for uint256;

    address public proxyRegistryAddress;
    uint256 public MAX_APE_HOLDINGS = 15;
    ...
    constructor(string memory _name, string memory _symbol, address _proxyRegistryAddress)
        ...
        factoryMintTo(_msgSender(), "ipfs://bafkreigawndlh5r4s3caea53yqvkxnozjynuit6ekqnnryjhsnv6miifs4");
    }

    function factoryMintTo(address _initialOwner, string memory _uri) public authorized returns (uint256){
        return mintTo(_initialOwner, totalSupply() + 1, _uri);
    }

    function mintTo(address _initialOwner, uint256 _id, string memory _uri) public authorized returns (uint256) {
...
//    }
    ...
    function _beforeTokenTransfer(
        ...
        require(balanceOf(to) + 1 < MAX_APE_HOLDINGS);
...
contract AuthorizedList is Ownable
{
    ...
    function authorizeCaller(address authAddress, bool shouldAuthorize) public virtual onlyOwner {
