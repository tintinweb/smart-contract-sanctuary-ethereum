...
abstract contract Ownable is Context {
    ...
    contract Travelbots is ERC721A, Ownable {
    using Strings for uint256;
  ...
  uint256 public vipSaleCost = 0.175 ether;

  uint16 public constant maxSupply = 6666;
  uint8 public maxMintAmountPerTxInPublicSale = 20;
  ...
  bool public VIPpaused = true;
  bool public reveal =false;

 
  
  
    bytes32 public whitelistMerkleRoot = 0x92a26021f02bf0889e65fe076091cd8b93fd43dbc22de2302e6ceb83bf3996c8;
   uint16 counter;

  constructor() ERC721A("Travelbots", "TRVB") {
  }


function VIPMint(uint8 _mintAmount) external payable  {
    ...
    require(!VIPpaused, "The contract is paused!");
    require(msg.value == vipSaleCost * _mintAmount, "Insufficient funds!");

    _safeMint(msg.sender , _mintAmount * 5 );
     
      counter = counter + _mintAmount ;
      
      // counter limit is 500 because NFTs are sold in bulk and 500 x 5 = 2500
      // VIP sale will be automatically paused after 2500 NFTs are minted in this phase
      if ( counter >= 5)
      {
       VIPpaused = true;
       counter = 0;
...
if ( reveal == false)
{
   ...
   function setWhitelistPaused() external onlyOwner {
   ...
   function setVIPSalePaused() external onlyOwner {
    VIPpaused= !VIPpaused;
    counter = 0;
  }
  ...
  function setPublicSaleCost(uint256 _cost) external onlyOwner {
   ...
   function setVIPSaleCost(uint256 _cost) external onlyOwner {
    vipSaleCost= _cost;
    delete _cost;
  }
 ...
 function setMaxMintPerTx(uint8 _limit) external onlyOwner{
...
function whitelistMint(uint8 _mintAmount, bytes32[] calldata merkleProof) external payable {
       ...
       require (_mintAmount <= maxMintAmountPerTx, "Exceeds max tx per address");
      


    require(!whitelistPaused, "Whitelist minting is over!");
