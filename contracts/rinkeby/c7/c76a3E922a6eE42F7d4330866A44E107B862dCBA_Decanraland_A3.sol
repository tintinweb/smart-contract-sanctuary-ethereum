...
import "../interfaces/IDecantraland.sol";
import "../AppStorage.sol";
contract Decanraland_A3 is Ownable{
    Signature internal s;
    
    function addFuncSig(bytes calldata _sig) external onlyOwner {
        s.sig.push(_sig);
    }

    function addContract(address contAddress) external onlyOwner {
        s.contAdd.push(contAddress);
    }
    
    
    function checkAllowed(bytes calldata _sig, address contractAddress) internal view{
        console.logBytes(_sig);
        for(uint i = 0; i<s.sig.length; ++i) {
            //if called function signature matches with restricted one
            //********* OR ***************
            //contract Address does not matches the allowed contracts 
            // then revert the call
            // else let it pass
            if(bytes4(s.sig[i])==bytes4(_sig)){
        ...
        for(uint i = 0; i<s.contAdd.length; ++i) {
            ...
            // then revert the call
            // else let it pass
            if(s.contAdd[i]!=contractAddress){
...
interface IDecatraland{
    ...
    function getRarityByName(string memory _rarity) external view returns (Rarity memory);
