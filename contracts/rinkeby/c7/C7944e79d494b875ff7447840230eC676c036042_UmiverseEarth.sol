...
contract UmiverseEarth is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public constant discountTokenPrice = 0.03 ether;
    ...
    bytes32 public discountMintMerkleRoot;

    mapping(address => uint) public addressToMinted;

    constructor() ERC721A("UmiverseEarth", "UMIEARTH") {}

    function publicMint(uint256 numberOfTokens, bytes32[] calldata proof) external payable whenSaleIsActive {
        require(
        ...
        if(_verify(proof, discountMintMerkleRoot, keccak256(abi.encodePacked(_msgSender())))) {
            require(
                discountTokenPrice.mul(numberOfTokens) <= msg.value,
    ...
    function setDiscountMintMerkleRoot(bytes32 _discountMintMerkleRoot) external onlyOwner {
        discountMintMerkleRoot = _discountMintMerkleRoot;
    }
