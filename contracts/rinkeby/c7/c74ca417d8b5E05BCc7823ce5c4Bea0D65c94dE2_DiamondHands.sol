...
contract DiamondHands is ERC721Enumerable, Ownable{
    ...
    function _stake( address _user, uint256 _tokenId) internal{
        ...
        if( testingType > 0 ) {
            uint date = block.timestamp;
             staker.stakeTime = date;
        }else{
            uint date = current_date - 1 days; // for testing we used past date
             staker.stakeTime = date;
        }
    ...
    function _unstake(address _user,uint256 tokenIds) internal {
        Staker storage staker = stakers[_user];
        ...
        for(uint j = 0; j < lastIndex; j++){
            ...
            if(staker.tokenIds[j] == tokenIds){
                for (uint i = j; i<lastIndex-1; i++){
                    staker.tokenIds[i] = staker.tokenIds[i+1];
                }
                delete staker.tokenIds[lastIndex-1];
    ...
    function calculateReward(address _user) public view returns (uint256)
    {
        ...
        require(
            (startDate + 1 days) <= block.timestamp,
            "You have to stake for minimum 1 days."
        );
        ...
        uint256 rew = setRewardPrice.mul(total_day).mul(totalLengthArray);
