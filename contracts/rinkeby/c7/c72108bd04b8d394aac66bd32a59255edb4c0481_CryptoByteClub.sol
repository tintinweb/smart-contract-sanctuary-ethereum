...
contract CryptoByteClub is ERC1155, Ownable, ContextMixin, NativeMetaTransaction {
  ...
  constructor() ERC1155("") {
    name = "Crypto Byte Club";
    symbol = "CBC";
  }

  function mint(address _to, uint _id, uint _amount) private onlyOwner {
    _mint(_to, _id, _amount, "");
  }

  function mintBatch(address _to, uint[] memory _ids, uint[] memory _amounts) private onlyOwner {
  ...
  function getCount() public view onlyOwner returns (uint256) {
    ...
    */
    ...
    mintBatch(msg.sender, idsnbr, idsqty);

        for (uint256 i = 1; i < _quantity; i++) {

            string memory suri = string(
	            abi.encodePacked(
		            "ipfs://QmVPX4YFdjGYY3WnDxKofUCwbWTf6kaiSx2i2cCqtJeSdq/",
                    substring(Strings.toHexString(i, 32),2,66),
    	            ".json"
	        ));
            setURI(i, suri);
