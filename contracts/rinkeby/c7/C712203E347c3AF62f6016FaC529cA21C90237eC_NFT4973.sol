...
contract NFT4973 is ERC4973, EIP712Whitelist, ReentrancyGuard {
    ...
    modifier callerIsUser() {
        require(tx.origin == msg.sender, "caller can not be contract");
        _;
    }

    modifier callerIsNFTOwner(address _addr) {
        require(nftContract != IERC721(address(0)), "Linked NFT contract is not set");
        require(nftContract.balanceOf(_addr) > 0, "caller is not linked NFT owner");
        _;
    }
    ...
    function publicMint() external callerIsUser callerIsNFTOwner(msg.sender) {
        _runMint(msg.sender);
    }
...
contract ERC4973 is Context, ERC165, IERC721Metadata {
// contract ERC4973 is Context, ERC165, IERC721, IERC721Metadata {
    /**
      * @dev Emitted when `tokenId` token is transferred from `from` to `to`.
      */
    event Transfer(address indexed from, address indexed to, uint256 indexed tokenId);
    ...
    function balanceOf(address owner) public view virtual returns (uint256) {
        require(owner != address(0), "ERC721-Identity: balance query for the zero address");
    ...
    function ownerOf(uint256 tokenId) public view virtual returns (address) {
        ...
        require(owner != address(0), "ERC721-Identity: owner query for nonexistent token");
        return owner;
    }
    ...
    //     require(to != owner, "ERC721-Identity: approval to current owner");

    //     require(
    //         _msgSender() == owner || isApprovedForAll(owner, _msgSender()),
    //         "ERC721-Identity: approve caller is not owner nor approved for all"
    //     );
    ...
    //     require(_exists(tokenId), "ERC721-Identity: approved query for nonexistent token");
    ...
    //     revert("ERC721-Identity: disabled feature");
    // }
    ...
    //     require(_exists(tokenId), "ERC721-Identity: operator query for nonexistent token");
    //     address owner = ownerOf(tokenId);
    ...
    //     //     "ERC721-Identity: transfer to non ERC721Receiver implementer"
    //     // );
    // }
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        require(to != address(0), "ERC721-Identity: mint to the zero address");
        require(!_exists(tokenId), "ERC721-Identity: token already minted");
        require(balanceOf(to) == 0, "ERC721-Identity: one token per one address");
    ...
    function burn(uint256 tokenId) public virtual {
        ...
        // require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721-Identity: caller is not owner nor approved");
        _burn(tokenId);
    }
    ...
    //     bool approved
    // ) internal virtual {
    //     require(owner != operator, "ERC721-Identity: approve to caller");
    ...
    //             if (reason.length == 0) {
    //                 revert("ERC721-Identity: transfer to non ERC721Receiver implementer");
    //             } else {
    //                 assembly {
