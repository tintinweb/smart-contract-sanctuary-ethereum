...
contract DOC721A is ERC721AQueryable, ReentrancyGuard, Ownable {
    using Strings for uint256;

    uint256 public constant maxTokensPerTransaction = 30;
    uint256 public tokenPrice = 0.02 ether;
    uint256 public constant maxNumber = 333;
    ...
    uint8[] flap_s = [10,20,40,60,80,100,110,120,130]; 
    uint8[] side_s = [10,30,50,70,90,100,120,140]; 
    uint8[] top_s = [10,30,40,50,70,90,100,120]; 

    constructor() ERC721A("DOC721A", "DOC721A") {
...
//     }
    ...
    function randomOCD(string memory seed) public view returns (OCDGenerator.OCDRands memory) {   
        
        OCDGenerator.OCDRands memory ocd;
        
        //trait
        ocd.bg1 = random(seed, "bg1") / 23 % 22;
        ocd.bg2 = random(seed, "bg2") / 17 % 22;
        ocd.flap = mapScore(flap_s,random(seed, "flip") % 130);
        ocd.side = mapScore(side_s,random(seed, "side") % 140);
        ocd.hole =  random(seed, "hole") % 2;   // 0: circle 1: rect
        ocd.eyecover = random(seed, "eyecover") % 5;
        ocd.brws = random(seed, "brws") % 5;
        ocd.top = mapScore(top_s,random(seed, "top") % 120);
        //color
        ocd.eyec = random(seed, "eyecolor") / 3 % 22;
        ocd.brwsc = random(seed, "brwc") / 5 % 22;
        ocd.shirtc = random(seed, "shirtc") / 12 % 22;
        ocd.outrc = random(seed, "outrc") /13 % 22;
        ocd.pktc = random(seed, "pktc") % 8;
        ocd.blnk =  Strings.toString(random(seed, "blnk") / 33 % 3 + 6 );
        return ocd;
    }

    function mapScore(uint8[] memory w, uint s) public pure returns (uint) {  
        uint p = 0;
        while(s>uint(w[p])){
            p++;
        }
    ...
    function tokenURI(uint256 _tokenId) public view override(ERC721A, IERC721A) returns (string memory){
        ...
        OCDGenerator.OCDRands memory ocd = randomOCD(Strings.toString(_tokenId));
        string memory svg = OCDGenerator.getOCDForSeed(ocd);
        string memory aparts = string(abi.encodePacked(
              '[{"trait_type": "Wall","value": "', Strings.toString(ocd.bg1),
              '"},{"trait_type": "Floor","value": "', Strings.toString(ocd.bg2),
              '"},{"trait_type": "Flap","value": "', Strings.toString(ocd.flap),
              '"},{"trait_type": "Side","value": "',Strings.toString(ocd.side),
              '"},{"trait_type": "Eyes","value": "',Strings.toString(ocd.hole),
              '"},{"trait_type": "Eyecover","value": "',Strings.toString(ocd.eyecover),
              '"},{"trait_type": "Brews","value": "',Strings.toString(ocd.brws),
              '"},{"trait_type": "Top","value": "',Strings.toString(ocd.top),
              '"}]'
              ));
    ...
    function mint(uint quantity) public payable {
        ...
        require(quantity <= maxTokensPerTransaction, "Max tokens per transaction number exceeded");
        require(totalSupply() + quantity <= maxNumber, "Minted Out");
