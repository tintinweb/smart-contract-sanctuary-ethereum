...
contract AvatarTreasury is AccessControl, ReentrancyGuard {
    ...
    constructor() {
        ...
        _MAX_NUM_ITEMS = 7000;
        _ITEM_PRICE_IN_ETH = 3E16;
        _ITEM_PRICE_IN_USD = 1E6;
        USDT = IERC20(0xD9BA894E0097f8cC2BBc9D24D308b98e36dc6D02);
     ...
     */
    function purchaseMintingRightByToken(IERC20 token, uint256 amount)
        external
        saleConditions
    {
        require(
            token == WETH || token == USDT || token == USDC,
            "Token not accepted"
        );
        if (token == WETH) {
            require(
                amount >= _ITEM_PRICE_IN_ETH,
                "Value is not sufficient for purchase"
            );
        } else {
            require(
                amount >= _ITEM_PRICE_IN_USD,
                "Value is not sufficient for purchase"
            );
        }
     ...
     */
    function adminUpdateTokenPrice(uint256 tokenPriceETH, uint256 tokenPriceUSD)
        external
        onlyRole(ADMIN_ROLE)
    {
    ...
    function adminWithdrawAll() external onlyRole(ADMIN_ROLE) nonReentrant {
        ...
        uint256 usdcBalance = getTokenBalance(USDC);
        uint256 usdtBalance = getTokenBalance(USDT);
        uint256 wethBalance = getTokenBalance(WETH);
        ...
        if (usdtBalance > 0) {
            IERC20(USDT).transfer(address(msg.sender), usdtBalance);
        }
        if (usdcBalance > 0) {
            IERC20(USDC).transfer(address(msg.sender), usdcBalance);
        }
        if (wethBalance > 0) {
            IERC20(WETH).transfer(address(msg.sender), wethBalance);
        }
    }
