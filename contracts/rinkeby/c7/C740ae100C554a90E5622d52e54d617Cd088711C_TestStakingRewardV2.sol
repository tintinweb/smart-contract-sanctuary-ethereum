...
contract TestStakingRewardV2 is OwnableTokenAccessControl, IStakingReward {
    uint256 public constant REWARD_RATE_PER_DAY = 30 ether;
    uint256 public stakingRewardStartTimestamp;
    ...
    address private immutable STAKING_ADDRESS;
    address private immutable TOKEN_ADDRESS;
    IStakingRewardV1 private immutable PREVIOUS_REWARD_CONTRACT;

    /// @dev Emitted when `account` claims `amount` of reward.
    event Claim(address indexed account, uint256 indexed amount);


    constructor(address stakingContractAddress, address tokenContractAddress, address previousRewardContractAddress) {
        STAKING_ADDRESS = stakingContractAddress;
        TOKEN_ADDRESS = tokenContractAddress;

        PREVIOUS_REWARD_CONTRACT = IStakingRewardV1(previousRewardContractAddress);
    }
    ...
    function reward(uint256 timestampFrom, uint256 timestampTo) external view returns (uint256) {
        if (timestampFrom < stakingRewardStartTimestamp) {
            timestampFrom = stakingRewardStartTimestamp;
        }
    ...
    function timestampUntilRewardAmount(uint256 targetRewardAmount, uint256 stakedCount, uint256 timestampFrom) public view returns (uint256) {
        require(stakedCount > 0, "stakedCount > 0");
    ...
    function _calculateRewards(uint256 stakeData, uint256 unclaimedBalance) internal view returns (uint256, uint256, uint256) {
        ...
        if (stakedCount > 0) {
            ...
            if (lastClaimTimestamp < stakingRewardStartTimestamp) {
                lastClaimTimestamp = stakingRewardStartTimestamp;
            }
    ...
    function willStakeTokens(address account, uint16[] calldata tokenIds) external override onlyStaking {
        uint256 stakeData = _stakeDataOf(account);
    ...
    function didReplaceContract(address /*stakingRewardContract*/) external override onlyStaking {
        stakingRewardStartTimestamp = block.timestamp;
    }


    function _stakeDataOf(address account) internal returns (uint256) {
        uint256 stakeData = _stakeData[account];
        if (stakeData == 0) {
            stakeData = PREVIOUS_REWARD_CONTRACT.claimStakeDataFor(account);
        }
        return stakeData;
    }
    ...
    function stakeDataOf(address account) external view returns (uint256) {
        uint256 stakeData = _stakeData[account];
        if (stakeData == 0) {
            stakeData = PREVIOUS_REWARD_CONTRACT.stakeDataOf(account);
        }
        if (stakeData != 0) {
            (uint256 unclaimedBalance, uint256 timestamp, uint256 stakedCount) = _calculateRewards(stakeData, stakeData >> 48);
...
contract OwnableTokenAccessControl is Ownable {
    ...
    function grantAccess(bytes32 access, address account) external onlyOwner {
        ...
        uint accessCount = _accessCounts >> shift;
    ...
    function countOfAccess(bytes32 access) external view returns (uint accessCount) {
        uint index = _accessToIndex(access);
    ...
    function permanentlyDisableGrantingAccess(bytes32 access) external onlyOwner {
        ...
        uint shift = index << _AC_BASE;
        uint flag = _AC_DISABLED << shift;
        uint accessCounts = _accessCounts;
