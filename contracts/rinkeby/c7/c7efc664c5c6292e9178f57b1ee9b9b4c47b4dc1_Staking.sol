...
//    using SafeMath for uint256;
   ...
   // IERC20 public rewardToken;
    ...
    uint256 public pool1 = 10000*10**18;
    uint256 public pool2 = 100000*10**18;
    uint256 public pool3 = 1000000*10**18;
    uint public Apr1 = 30;
    uint public Apr2 = 40;
    uint public Apr3 = 50;
    // uint256 amount;
   ...
   // uint accuracyFactor = 10 ** 10;
    ...
    event Staked(address indexed user,address _thisaddress, uint256 amount);
    ...
    function setApr1(uint _value) public onlyOwner {
        Apr1 = _value;
    }
    function setApr2(uint _value) public onlyOwner {
        Apr2 = _value;
    }
    function setApr3(uint _value) public onlyOwner {
        Apr3 = _value;
    }

     function setPool1(uint _pool1value) public onlyOwner {
        pool1 = _pool1value;
    }
    function setPool2(uint _pool2value) public onlyOwner {
        pool2 = _pool2value;
    }
    function setPool3(uint _pool3value) public onlyOwner {
        pool3 = _pool3value;
    }
    function getReward(address account,uint id,uint poolType) public view returns (uint256) {
        ...
        if(poolType ==1){
             ...
             amount = ((depositedAmount* Apr1)/100)/365;
           if(!hell[msg.sender][id][poolType]){
            ...
            amount = ((depositedAmount* Apr2)/100)/365;
            if(!hell[msg.sender][id][poolType]){
               totalDays = (block.timestamp - Stakers[account][id].stakeTime)/ 1 minutes;
            }else{
            ...
            uint depositedAmount = Stakers[account][id].stakeAmount;
            amount = ((depositedAmount* Apr3)/100)/365;
            if(!hell[msg.sender][id][poolType]){
             totalDays = (block.timestamp - Stakers[account][id].stakeTime)/ 1 minutes;
            }else{
    ...
    function stake(uint256 poolType) public {
        if(poolType==1){
           ...
           // Stakers[msg.sender].push(staker);
             stakeToken.transferFrom(msg.sender, address(this),pool1);
             emit Staked(msg.sender,address(this),pool1);
        }else if(poolType==2){
             stakerDetails memory staker;
             staker.stakeTime = block.timestamp;
             staker.stakeAmount = pool2;
             staker.poolType =2;
             totalStaking+=pool2;
             totalStakeRecords[msg.sender] += 1;
             staker.id = totalStakeRecords[msg.sender];
           // staker.apr = getAPR(noOfDays);    
             Stakers[msg.sender].push(staker);
            //  Stakers[msg.sender].push(staker);
             stakeToken.transferFrom(msg.sender, address(this),pool2);
             emit Staked(msg.sender, address(this),pool2);
        }else if(poolType==3){
             stakerDetails memory staker;
             staker.stakeTime = block.timestamp;   
             totalStakeRecords[msg.sender] += 1;
             staker.id = totalStakeRecords[msg.sender];
             staker.stakeAmount = pool3;
             staker.poolType =3;
             totalStaking+=pool3;
             Stakers[msg.sender].push(staker);
             stakeToken.transferFrom(msg.sender, address(this),pool3);
             emit Staked(msg.sender, address(this),pool3);
        }
    }
...
function withdrawRewards (uint id,uint256 poolType) public { 
           uint totalRewards = getReward(msg.sender,id,poolType);
            hell[msg.sender][id][poolType]=true;
