...
contract WithdrawalPool is IWithdrawalPool, Ownable, CheckContract {
    ...
    function withdrawUSDC(uint _id) external override {
        ...
        (uint USDCAmount,,,uint USDCFee,uint withdrawlTime) = stakingPool.getNFTVariables(_id);
        
        require(withdrawlTime >= block.timestamp, "WithdrawalPool: withdrawal time period didnt expire");
        require(usdc_token.balanceOf(address(this)) >= USDCAmount, "Insufficient USDC in WithdrawalPool");

        _transferUSDCtoLender(USDCAmount - USDCFee);
        _transferUSDCtoTreasury(USDCFee);
        taygenNFT.burn(_id);
        
    }

    function _transferUSDCtoLender(uint _amount) internal {
        usdc_token.transfer(tx.origin, _amount);
    }

    function _transferUSDCtoTreasury(uint _amount) internal {
        usdc_token.transfer(treasuryAddress, _amount);
    }

    function _requireCallerIsStakingPool() internal {
        require(msg.sender == stakingPoolAddress, "WithdrawalPool: Caller is not stakingPool");
    }
