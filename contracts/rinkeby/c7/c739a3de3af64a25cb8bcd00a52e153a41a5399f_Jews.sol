...
contract Jews is ERC721URIStorage, Ownable {
	...
	function mint(uint256 quantity) external {
		...
		if(allowListEnabled) {
			require(_allowList[to] >= 1, 'Not in whitelist');
		}
		require(maxSupply >= _tokenIds.current() + quantity, "Max supply exceeded!");
		require(_minted[to] < quantity, "Reached max count of free mint");
		uint256 i = 0;
		while(i < quantity) {
			_tokenIds.increment();
	...
	function canMint(address _address) public view returns(bool) {
		return _allowList[_address] > _minted[_address];
	}
