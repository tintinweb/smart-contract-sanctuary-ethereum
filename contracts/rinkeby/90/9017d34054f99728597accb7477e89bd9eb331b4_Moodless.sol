...
contract Moodless is ERC721, Ownable, ERC721Enumerable {
    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public constant tokenPrice = 19000000000000000; // 0.019 ETH
    ...
    event MoodlessMinted(uint256 tokenId, address owner);

    constructor() ERC721("Moodless", "MD") {}
    ...
    function mintMoodless(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            if (totalSupply() < MAX_TOKENS) {
                ...
                emit MoodlessMinted(id, msg.sender);
            }
        }
    }

    function presaleMoodless(uint256 numberOfTokens) external payable {
    ...
    function tokenURI(uint256 tokenId)
        ...
        returns (string memory)
        {
            ...
            if (isRevealed == false) {
                return
                "ipfs://QmSxygrDQ2od3ur3SwERxd18oCExmUPQC8HEzqMncmgmZs/abc_gif.json";
                }
