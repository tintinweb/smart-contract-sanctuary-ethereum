...
contract CGCWhitelistERC721A is ERC721A, Ownable, Pausable, ReentrancyGuard, IPhaseSalable {
    ...
    // Royalty percent
    uint16 public royaltyPercent = 1000; // 10 %
    // MAX_PERCENTAGE
    ...
    event SetRoyaltyTreasury(address treasury);
    event SetRoyaltyPercent(uint16 percent);
    ...
    constructor(
        ...
        if (_royaltyTreasury == address(0)) revert InvalidRoyaltyTreasury();
    ...
    function royaltyInfo(uint256, uint256 _salePrice) external view returns (address receiver, uint256 royaltyAmount) {
        receiver = royaltyTreasury;
        royaltyAmount = (_salePrice * royaltyPercent) / MAX_PERCENTAGE;
    }

    /// @dev update royalty treasury address
    function setRoyaltyTreasury(address _royaltyTreasury) external onlyOwner {
        if (_royaltyTreasury == address(0)) revert InvalidRoyaltyTreasury();

        royaltyTreasury = _royaltyTreasury;
        emit SetRoyaltyTreasury(_royaltyTreasury);
    }

    /// @dev update royalty fee
    function setRoyaltyFee(uint16 _royaltyPercent) external onlyOwner {
        if (_royaltyPercent >= MAX_PERCENTAGE) revert InvalidRoyaltyPercent();

        royaltyPercent = _royaltyPercent;
        emit SetRoyaltyPercent(_royaltyPercent);
    }

    /// @dev support interfaces from ERC165
