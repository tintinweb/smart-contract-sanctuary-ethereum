...
contract Survey is Ownable {
    ...
    mapping(uint256 => address) public orgMember;
    uint256[] public proposalList;
    uint rewards = 5;



    function join(address _orgMember) public onlyOwner {
        require(whitelistMembers[_orgMember] = true, 'Already whitlisted');
        whitelistMembers[_orgMember] = true; 
        tokenContract.mint(_orgMember, rewards);
    }


    function createProposal(string memory name, string memory description) public  returns(uint256){
        require(whitelistMembers[msg.sender] = true ,"Registered member can create a proposal");
    
    uint256 proposalId = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender)));
...
function vote(uint _proposalId, VotingOptions _vote) external {
    require(whitelistMembers[msg.sender] = true);
    ...
    function checkVotes() public view returns(uint256 [] memory, uint256 [] memory) {
        uint256[] memory  proposalId = new uint256[](nextProposalId);
        uint256[] memory  totalVotes = new uint256[](nextProposalId);
        for (uint i = 0; i < nextProposalId ; i++) {
        //  uint256  tempString = proposalList[i];
          Proposal storage proposal = proposals[i];
          proposalId[i] = proposal.id;
          totalVotes[i] = proposal.totalVotes;
      }
        return (proposalId, totalVotes);
    }

    function getProposals() public view returns(uint256 [] memory){
        uint256[] memory  totalProposal = new uint256[](nextProposalId);
        for (uint i = 0; i < nextProposalId ; i++) {
         // uint256  tempString = proposalList[i];
          Proposal storage proposal = proposals[i];
          totalProposal[i] = proposal.id;
      }
