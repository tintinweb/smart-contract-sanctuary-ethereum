...
// File contracts/WalletCoins.sol
...
contract WalletCoins is ERC721, ERC721Enumerable, Dna {
    ...
    constructor(uint256 _maxSupply) ERC721("WalletCoins", "PLPKS") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No WalletCoins left :(");
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "WalletCoins #',
                tokenId.toString(),
                '", "description": "Wallet Coins are randomized Avataaars stored on chain to teach DApp development on Platzi", "image": "',
                image,
                '"}'
            )
        );
