...
contract xbapc is ERC721A, Ownable {

    using Strings for uint256;

    string public uriPrefix = "ipfs://QmeCbrj382SwUYxz5ctNd5aK2UojJTdJ65yPRScxTKxYxT/";
    ...
    // FreeMint
    uint256 public FREE_SUPPLY = 100;
    uint256 public FREE_PER_WALLET = 2;

    bool public SALE_ACTIVE;

    address[] private allow_collections;

    mapping(address => uint256[]) public claimMap;

    constructor() ERC721A("TESTNFT", "TEST") {
    }
    ...
    function getFromAddress(address wallet) public view returns(uint256[] memory) {
        uint256 held = balanceOf(wallet);
        uint256 x = 0;
        uint256[] memory tokens = new uint256[](held);
    ...
    function initOS() external onlyOwner {
        require(totalSupply() < MAX_SUPPLY, "Max supply exceeded!");
        _safeMint(msg.sender, 1);
    }

    function mintForAddress(uint256 _amount, address _receiver) public payable mintCompliance(_amount) onlyOwner {
    ...
    function setCost(uint256 _price) public payable onlyOwner {
        PRICE = _price;
    }
