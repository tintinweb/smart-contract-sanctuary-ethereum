...
contract PIKASOSingle is
    ...
    
 {
    ...
    function SalewithToken(address bidtoken,address bidaddr,  uint256 amount, uint256 tokenId) internal{
        uint256 val = pERCent(amount, serviceValue).add(amount);
        ERC20 t = ERC20(bidtoken);
    ...
    function saleTokenwithToken(string memory tokenAss,address from , uint256 tokenId, uint256 amount) public{
    ...
    function burnToken(uint256 id) public{
        require( balances[id][msg.sender] == 1, "Your Not a Token Owner or insufficient Token Balance");
            burn(id);
