...
contract RealEstate{
   ...
   constructor(string memory _propertyID, string memory _propertysymbol, uint256 total) {
    ...
    function getYourMoney() public view returns(uint256){
        return address(this).balance * balances[msg.sender]/_total;
    }

    function withdraw() public payable{
        require(block.timestamp>depositTime);
        if (balances[msg.sender]>0)
        withdraw_amount=balances[msg.sender]*accumulation/_total;
        income[msg.sender]+=withdraw_amount;
        user=payable(msg.sender);
        user.transfer(getYourMoney());        
