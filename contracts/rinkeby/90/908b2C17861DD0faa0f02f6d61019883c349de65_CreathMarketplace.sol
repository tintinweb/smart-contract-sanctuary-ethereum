...
UUPSUpgradeable{
    ...
    /// @notice TokenId -> bool check if item is listed
    ...
    function setTreasuryAddress(address _treasuryAddress)
        ...
        onlyOwner
    {
        require(_treasuryAddress != address(0), "CreathMarketplace: Address 0 is not allowed");
        treasury = ITreasury(_treasuryAddress);
    ...
    function listItems(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i = i + 1) {
            require(!listed[_tokenIds[i]], "Creath Marketplace: Item listed");
            listings[_nftAddress][_tokenIds[i]] = Listing(
                owner,
                _pricePerItem,
                _startingTime
            );
            listed[_tokenIds[i]] = true;
    ...
    function cancelListing(address _nftAddress, uint256 _tokenId)
        ...
        onlyOwner
    {
        require(listed[_tokenId], "Creath Marketplace: Item Not listed");

        delete (listings[_nftAddress][_tokenId]);
        emit ItemCanceled(owner, _nftAddress, _tokenId);
    }
    ...
    {
        IERC721Upgradeable nft = IERC721Upgradeable(_nftAddress);
        require(msg.sender != nft.ownerOf(_tokenId), "Creath Marketplace: cannot buy your own asset.");
        require(listed[_tokenId], "Creath Marketplace: Item Not listed");
        
        Listing memory listedItem = listings[_nftAddress][_tokenId];
        require(_amount == listedItem.pricePerItem, "Creath Marketplace: Transfer the required price of the asset.");
