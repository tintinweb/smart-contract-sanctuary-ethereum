    ...
    ShinikiTransfer
{
    ...
    //number mint public
    mapping(address => mapping(uint64 => uint256)) public numberPublicMinted;

    //number mint pre
    mapping(address => mapping(uint64 => uint256)) public numberPreMinted;
    ...
    function initialize(
        ...
        ROUND = 1;
        currentIndex[ROUND][TYPE_1] = 0;
        currentIndex[ROUND][TYPE_2] = collectionSize_ / 2;
        startIndex[ROUND][TYPE_1] = 0;
        startIndex[ROUND][TYPE_2] = collectionSize_ / 2;
        revealed = false;
    ...
    function preSaleMint(
    ...
    ) external nonReentrant callerIsUser whenNotPaused {
        require(receiver == msg.sender, "Shiniki: caller is not receiver");
        require(round == ROUND, "Shiniki: round error");
        require(
        ...
        require(
            totalSupply(ROUND) + totalMint <= collectionSize[ROUND],
            "Shiniki: reached max supply"
        );
        require(
            numberPreMinted[msg.sender][ROUND] + totalMint <= amountAllowce,
            "Shiniki: can not mint greater than whiteList"
        );

        numberPreMinted[msg.sender][ROUND] += totalMint;
    ...
    function publicSaleMint(
        ...
        require(
            numberPublicMinted[msg.sender][ROUND] + totalMint <=
    ...
    function privateMint(uint256 quantity) public onlyOwner whenNotPaused {
        require(
            totalSupply(ROUND) + quantity <= collectionSize[ROUND],
            "Shiniki: reached max supply"
        );
    ...
    // set new round
    function setNewRound(uint256 _collectionSize) public onlyOwner {
        ROUND++;
        collectionSize[ROUND] = _collectionSize - collectionSize[ROUND - 1];
        currentIndex[ROUND][TYPE_1] = uint64(collectionSize[ROUND-1]);
        currentIndex[ROUND][TYPE_2] = uint64((collectionSize[ROUND] / 2) + collectionSize[ROUND-1]);
        startIndex[ROUND][TYPE_1] = currentIndex[ROUND][TYPE_1];
        startIndex[ROUND][TYPE_2] = currentIndex[ROUND][TYPE_2];
    }
    ...
    function withdrawAll(address receiver) external onlyOwner nonReentrant {
    ...
    IERC721MetadataUpgradeable
{
    ...
    mapping(uint64 => uint256) public collectionSize;

    // curent index by round,type
    mapping(uint64 => mapping(bytes32 => uint256)) public currentIndex;

    // start index by round,type
    mapping(uint64 => mapping(bytes32 => uint256)) public startIndex;
    ...
    function __ERC721_init_unchained(
        ...
        _name = name_;
        _symbol = symbol_;
        collectionSize[1] = collectionSize_;
    }
    ...
    function totalSupply() public view returns (uint256) {
        uint256 total = 0;
        unchecked {
            for (uint64 i = 1; i <= ROUND; i++) {
                total +=
                    (currentIndex[i][TYPE_1] - startIndex[i][TYPE_1]) +
                    (currentIndex[i][TYPE_2] - startIndex[i][TYPE_2]);
            }
            return total;
        }
    }
    ...
    /**
     * @dev See {totalSupply _ round}.
     */
    function totalSupply(uint64 round) public view returns (uint256) {
        unchecked {
            return
                (currentIndex[round][TYPE_1] - startIndex[round][TYPE_1]) +
                (currentIndex[round][TYPE_2] - startIndex[round][TYPE_2]);
        }
    }
    ...
    function typeTokenUri(uint256 tokenId) internal view returns (bytes32) {
        for (uint64 i = 1; i <= ROUND; i++) {
            if (
                tokenId < currentIndex[i][TYPE_1] &&
                tokenId >= startIndex[i][TYPE_1]
            ) {
                return TYPE_1;
            }
        }
    ...
    function _exists(uint256 tokenId) public view returns (bool) {
        for (uint64 i = 1; i <= ROUND; i++) {
            if (
                (tokenId < currentIndex[i][TYPE_1] &&
                    tokenId >= startIndex[i][TYPE_1]) ||
                (tokenId < currentIndex[i][TYPE_2] &&
                    tokenId >= startIndex[i][TYPE_2])
            ) {
                return true;
            }
        }
    ...
    function _safeMint(
        ...
        uint256 startTokenId = currentIndex[ROUND][typeMint];
        ...
        unchecked {
            ...
            currentIndex[ROUND][typeMint] = updatedIndex;
        }
