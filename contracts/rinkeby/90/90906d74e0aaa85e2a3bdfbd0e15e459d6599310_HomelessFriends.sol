...
contract HomelessFriends is Ownable, ERC721A, ReentrancyGuard {
  ...
  mapping(address => bool) private whitelistedAddressesForFreeMint;
...
function mint(uint256 quantity) external payable callerIsUser {
    if(status == 1){
    require(whitelistedAddressesForFreeMint[msg.sender], "You are not White Listed For Free Mint");
    ...
    require(totalSupply() < 8122, "Pre Sale NFTs have been minted.");
    ...
    require(totalSupply() < 9999, "Free NFTs have been minted.");
    require(quantity > 0,"Mint Quantity should be more than 0");
  ...
  function isWhitelistedForFreeMint(address _user) public view returns (bool) {
    return whitelistedAddressesForFreeMint[_user];
  }
  ...
  function addNewWhitelistUserForFreeMint(address[] calldata _users) public onlyOwner {
    ...
    for(uint i=0;i<_users.length;i++)
        whitelistedAddressesForFreeMint[_users[i]] = true;
  }
