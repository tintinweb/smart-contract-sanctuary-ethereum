...
contract USDR is Initializable, OwnableUpgradeable, ERC20PermitUpgradeable {
    ...
    function initialize() public initializer {
        ...
        __ERC20_init("Ruyi USD", "USDR");
        __ERC20Permit_init("Ruyi USD");
    }
    ...
    function mint(address to, uint256 amount) external {
        if(owner() == _msgSender()) {
            ...
            require(miners[msg.sender] >= amount, "USDR: Invalid miner");
    ...
    function _beforeTokenTransfer(
        ...
        require(!freezed[from], "USDR: SRC Account Freezed");
        require(!freezed[to], "USDR: DST Account Freezed");
    }

    function increaseMiner(address miner, uint256 amount) external onlyOwner {
        require(miner != address(0), "USDR: Zero address");
        miners[miner] += amount;
    ...
    function decreaseMiner(address miner, uint256 amount) external onlyOwner {
        ...
        if (amount >= approvedAmount) {
            miners[miner] = 0;
            emit DecreaseMiner(miner, miners[miner]);
        } else {
            miners[miner] -= amount;
    ...
    function delegateMiner(address to, uint256 amount) external {
        ...
        require(miners[user] >= amount, "USDR: Not enough approval amount");
...
abstract contract EIP712Upgradeable is Initializable {
    ...
    function _domainSeparatorV5() internal view returns (bytes32 a,bytes32 b ,bytes32 c) {
        a = _TYPE_HASH;
        b = _EIP712NameHash();
        c = _EIP712VersionHash();
    }
