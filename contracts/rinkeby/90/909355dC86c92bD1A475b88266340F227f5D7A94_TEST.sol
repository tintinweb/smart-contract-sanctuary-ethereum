...
contract TEST is ERC721A, Ownable {
    ...
    uint public constant maxMintAmountPerTx = 10;
    
    uint public constant maxPerWalletWL = 1;
    uint public constant presalePrice = 0 ether;
    ...
    function mint(bytes32[] calldata merkleProof, uint count) external payable {
        ...
        require(count <= maxMintAmountPerTx, " Number of requested tokens exceeds allowance (10)");
        if(saleStatus == SaleStatus.PRESALE) {
            require(msg.value >= count * presalePrice, " Ether value sent is not sufficient");
            require(_whitelistMintedCount[msg.sender] + count <= maxPerWalletWL, " Number of requested tokens exceeds allowance (1)");
