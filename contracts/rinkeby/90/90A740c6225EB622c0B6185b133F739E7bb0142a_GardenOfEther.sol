...
contract GardenOfEther is Ownable, ERC721A, ReentrancyGuard {
    ...
    constructor() ERC721A("Garden Of Ether", "GOE") {
        ...
        glowingOrDarkToUint["Glowing Depleted"] = 2; // remove this not used
        glowingOrDarkToUint["Dark Depleted"] = 3;  // remove this not used
        typeToUint["Shard"] = 0;
        ...
        glowingOrDark[ //Splinter Dark Depleted
            7
        ] = "https://gateway.pinata.cloud/ipfs/QmWWzPT1j67UcVE16pwPSY4JsjcSpebAggPh9q6JDcMXJN";
    }
    ...
    // changing in general
    function setShardGlowingURL(string memory _newURL) external onlyOwner {
        glowingOrDark[0] = _newURL;
    }

    function setShardDarkURL(string memory _newURL) external onlyOwner {
        glowingOrDark[1] = _newURL;
    }

    function setSplinterGlowURL(string memory _newURL) external onlyOwner {
        glowingOrDark[2] = _newURL;
    }

    function setSplinterDarkURL(string memory _newURL) external onlyOwner {
        glowingOrDark[3] = _newURL;
    }

    function setGlowingDepletedShardURL(string memory _newURL)
        external
        onlyOwner
    {
        glowingOrDark[4] = _newURL;
    }

    function setDarkDepletedShardURL(string memory _newURL) external onlyOwner {
        glowingOrDark[5] = _newURL;
    }

    function setGlowingDepletedSplinterURL(string memory _newURL)
        external
        onlyOwner
    {
        glowingOrDark[6] = _newURL;
    }

    function setDarkDepletedSplinterURL(string memory _newURL)
        external
        onlyOwner
    {
        glowingOrDark[7] = _newURL;
    }
    ...
    function manipulateReserveOfTokenBySub(
        ...
        if (mapToToken[_tokenId].massReserve != 0) {
            if (
                mapToToken[_tokenId].massReserve - 1 < _reserveToBeDecreasedBy
            ) {
                mapToToken[_tokenId].massReserve -
                    mapToToken[_tokenId].massReserve;
            } else {
...
*/
    ...
    function cappedMint(uint256 _amount) external {
        if (
            currentStateMintTracker[currentState] + _amount >
            currentStateMaxSupplyCap[currentState]
        ) revert AmountExceeded();
        _mint(msg.sender, _amount, "", true);
    }

    function testManipulateReserveOfTokenBySub(
        uint256 _tokenId,
        uint48 _reserveToBeDecreasedBy
    ) external  {
        manipulateReserveOfTokenBySub(_tokenId, _reserveToBeDecreasedBy);
    }

    function testInitiateDepletion(uint256 _tokenId) external {
    ...
    function readMassForTokenId(uint256 _tokenId) external view returns (uint48) {
        return mapToToken[_tokenId].massReserve;
    }
    function readImageForToken(uint256 _tokenId) external view returns (uint48) {
        return mapToToken[_tokenId].massReserve;
    }
