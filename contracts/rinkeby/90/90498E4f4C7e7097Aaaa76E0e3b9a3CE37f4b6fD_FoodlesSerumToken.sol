...
contract FoodlesSerumToken is ERC1155, Ownable {
    ...
    mapping(address => uint256) public serumClaimed;
     ...
     * @param available The total number of available tokens for this address.
    ...
    function mintSerum(
        ...
        require(numTokens + serumClaimed[msg.sender] <= available, "FoodlesSerumToken: Exceeds available serums");
        bytes32 leaf = keccak256(abi.encode(msg.sender, available));
        require(verify(merkleRoot, leaf, proof), "FoodlesSerumToken: Not a valid proof");

        serumClaimed[msg.sender] += numTokens;
