...
struct PresaleConfig {
    ...
    uint32 endTime;
    uint32 supplyLimit;
    uint256 mintPriceDefender;
    uint256 mintPriceConqueror;
}
...
contract NFT is ERC721, Ownable {
    ...
    uint256 public constant NUMBER_OF_CONQUER = 8888;
    uint256 public constant NUMBER_OF_RESERVED_CONQUER = 500;
    uint256 public constant MAX_SUPPLY = 23665; // need this amount
    uint256 public constant MAX_PER_TX = 6; // MAX_PER_TX is 5
    uint256 public constant MAX_PRESALE_MINTS_PER_ADDRESS = 6; // max mints during presale per address is 5
    ...
    uint256 public mintPriceDefender;  //  0.01 ether;
    uint256 public mintPriceConqueror; //  0.02 ether;
    ...
    ERC721("TokenName","TokenSymbol") {
        ...
        //     endTime: 0,
        //     supplyLimit: 10,
        //     mintPriceDefender: 0.005 ether,
        //     mintPriceConqueror: 0.01 ether
        // });
    }

    /// Return the totalSupply
    function totalSupply() external view returns (uint256) {
        return defenderMinted + conquerorMinted;
    }
    ...
    function enterPhase(SalePhase _phase) external onlyOwner {
        // require(uint8(_phase) > uint8(phase), 'can only advance phase');
        phase = _phase;
    }
    ...
    function presaleMintDefender(uint256 count) external payable {
        PresaleConfig memory _config = presaleConfig;

        require(block.timestamp >= _config.startTime && block.timestamp <= _config.endTime, 'This phase of presale is not active.');
        require((_config.mintPriceDefender * count) <= msg.value, 'Ether value sent is not correct');
        require((defenderMinted + count) <= _config.supplyLimit, 'Excedes max supply of Defender.'); // supplyLimit should be updated during all presale phases, supplyLimit <= NUMBER_OF_DEFENDER

        require(count < MAX_PER_TX, 'Exceeds max per transaction.');
        require(phase == SalePhase.PreSale, 'Presale event is not active.');
        // require(addressToMinted[_msgSender()] + count < MAX_PRESALE_MINTS_PER_ADDRESS, 'Exceeds number of presale mints allowed.'); 
        require(balanceOf(_msgSender()) + count < MAX_PRESALE_MINTS_PER_ADDRESS, 'Exceeds number of presale mints allowed.');
    ...
    /// @param count number of tokens to mint in transaction
    /// @notice mints tokens with counter token IDs to addresses eligible for presale
    function presaleMintConqueror(uint256 count) external payable {
        PresaleConfig memory _config = presaleConfig;

        require(block.timestamp >= _config.startTime && block.timestamp <= _config.endTime, 'This phase of presale is not active.');
        require((_config.mintPriceConqueror * count) <= msg.value, 'Ether value sent is not correct');
        require((conquerorMinted + count) <= _config.supplyLimit, 'Excedes max supply of Defender.'); // supplyLimit should be updated during all presale phases, supplyLimit <= NUMBER_OF_CONQUER - NUMBER_OF_RESERVED_CONQUER

        require(conquerorMinted + count <= NUMBER_OF_CONQUER - NUMBER_OF_RESERVED_CONQUER, 'Excedes max supply of Conqueror.');
        require(count < MAX_PER_TX, 'Exceeds max per transaction.');
        require(phase == SalePhase.PreSale, 'Presale event is not active.');
        // require(addressToMinted[_msgSender()] + count < MAX_PRESALE_MINTS_PER_ADDRESS, "Exceeds number of presale mints allowed"); 
    ...
    /// Public Defender minting open to all
	...
	/// @param count number of tokens to mint in transaction
    function publicMintDefender(uint256 count) external payable {
        // disallow contracts from buying
        // require((!msg.sender.isContract() && msg.sender == tx.origin), "Contract buys not allowed");

        require((mintPriceDefender * count) <= msg.value, 'Ether value sent is not correct');
        require(phase == SalePhase.PublicSale, 'Public sale in not active');
        require(defenderMinted + count <= NUMBER_OF_DEFENDER, 'Excedes max supply of Defender.');
        require(count < MAX_PER_TX, 'Exceeds max per transaction.');
    ...
    /// Public Conqueror minting open to all
	/// @dev mints tokens during public sale, limited by `MAX_MINTS_PER_ADDRESS`
	/// @param count number of tokens to mint in transaction
    function publicMintConqueror(uint256 count) external payable {
        require((mintPriceConqueror * count) <= msg.value, 'Ether value sent is not correct');
        require(phase == SalePhase.PublicSale, 'Public sale in not active');
        require(conquerorMinted + count <= NUMBER_OF_CONQUER - NUMBER_OF_RESERVED_CONQUER, 'Excedes max supply of Conqueror.');
        require(count < MAX_PER_TX, 'Exceeds max per transaction.');
    ...
    function _mintConquerors(address to, uint256 count) internal {
        for (uint256 index = 0; index < count; index++) {
            _safeMint(to, _tokenIdConqueror);
            _tokenIdConqueror++;
        }
    }
    ...
    function configurePresale(uint256 startTime, uint256 endTime, uint256 supplyLimit) external onlyOwner {
        ...
        require(0 < _supplyLimit, 'Invalid supply limit');
