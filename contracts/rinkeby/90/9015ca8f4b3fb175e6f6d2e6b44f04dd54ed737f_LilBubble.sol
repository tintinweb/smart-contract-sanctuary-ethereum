...
/**
 * @title Lil Bubble Originals
 * @author Ben Yu, Itzik Lerner, rminla - NFTDevz
 * @notice This contract handles minting Lil Bubble Originals tokens.
 */
contract LilBubble is ERC721A, Ownable, ERC2981 {
    using ECDSA for bytes32;
    using Strings for uint256;
    using SafeMath for uint256;

    uint256 public constant publicPrice = 0.09 ether;
    uint256 public constant presalePrice = 0.069 ether;
    uint256 public constant maxSupply = 3333;
    address public constant withdraw1Address = 0xc9b6321dc216D91E626E9BAA61b06B0E4d55bdb1;
    address public constant withdraw2Address = 0xc9b6321dc216D91E626E9BAA61b06B0E4d55bdb1;
    ...
    /**
     * @notice To be updated by contract owner to allow free mints from existing past collection token holders
     */
    function setFreeClaimState(bool _saleActiveState) public onlyOwner {
        require(isFreeClaimActive != _saleActiveState, "NEW_STATE_IDENTICAL_TO_OLD_STATE");
        isFreeClaimActive = _saleActiveState;
    }
     ...
     * @notice Set the maximum public mints allowed per a given address
     */
    function setPublicMintsAllowsPerAddress(uint256 _mintsAllowed) public onlyOwner {
    ...
    /**
     * @notice Allow past collection token holders to mint a single token for free
     */
    function freeClaim(
        bytes32 messageHash,
        ...
        require(totalSupply() + 1 <= maxSupply, "MAX_SUPPLY_EXCEEDED");
    ...
    function mint(
        ...
        require(_numberMinted(msg.sender) + numTokens <= publicMintsAllowedPerAddress, "MAX_MINTS_EXCEEDED");
        require(totalSupply() + numTokens <= maxSupply, "MAX_SUPPLY_EXCEEDED");
    ...
    function withdraw() public onlyOwner {
        require(withdraw1Address != address(0), "INVALID_WITHDRAW_ADDRESS");
        uint256 _balance = address(this).balance;
        require(_balance > 0, 'NO_ETH_TO_WITHDRAW');

        if (withdraw1Address == withdraw2Address) {
            require(payable(withdraw1Address).send(_balance));
        } else {
            require(withdraw2Address != address(0), "INVALID_WITHDRAW_ADDRESS");
            uint256 _split = _balance.mul(90).div(100);
            require(payable(withdraw1Address).send(_split));
            require(payable(withdraw2Address).send(_balance.sub(_split)));
        }
    }
