...
contract ERCX is IERCX, ERC721, ERC165Storage, AccessControl {
    ...
    function setTokenUri(uint256 _id, string calldata _uri) external {
        require(hasRole(URI_SETTER_ROLE, msg.sender), "ERCX: minter only");
        tokenUri[_id] = _uri;
    }
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, ERC165Storage, AccessControl) returns (bool) {
        return
   ...
   * @return owner address currently marked as the owner of the given item ID
   */
    function ownerOf(uint256 itemId) public override(IERCX, ERC721) view returns (address) {
   ...
   */
    function setApprovalForAll(address to, bool approved) public override(IERCX, ERC721) {
  ...
  */
   ...
   */
    ...
    * @param data bytes data to send along with a safe transfer check
    */
    function _safeMint(address to, uint256 itemId, bytes memory data) internal override {
        _mint(to, itemId);
    ...
    */
    function mint() external {
        require(hasRole(ADMIN_ROLE, msg.sender), "ERCX: Admin only");

        _tokenIds.increment();
        uint256 itemId = _tokenIds.current();
        _itemOwner[itemId][1] = msg.sender;
        _itemOwner[itemId][2] = msg.sender;
        _ownedItemsCount[msg.sender][1].increment();
        _ownedItemsCount[msg.sender][2].increment();
