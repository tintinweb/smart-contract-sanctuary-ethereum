...
//        limitLegendary = _limitLegendary;
    ...
    function _mintCommonPack(address packReceiver, uint256 packCount) private {
    ...
    function _mintRarePack(address packReceiver, uint256 packCount) private {
    ...
    function _mintLegendaryPack(address packReceiver, uint256 packCount) private {
    ...
    function buyCommonPacks() public payable saleIsRunningModifier {
        ...
        _mintCommonPack(msg.sender, packCount);

        emit Buy(msg.sender, COMMON_RARITY_ID, packCount);
    }
    ...
    function buyRarePacks() public payable saleIsRunningModifier {
        ...
        _mintRarePack(msg.sender, packCount);
    ...
    function buyLegendaryPacks() public payable saleIsRunningModifier {
        ...
        _mintLegendaryPack(msg.sender, packCount);
    ...
    function mintPresalePacks(address[] memory commonAddresses, address[] memory rareAddresses, address[] memory legendaryAddresses) public onlyOwner {
        for (uint256 i = 0; i < commonAddresses.length; i++) {
            _mintCommonPack(commonAddresses[i], 4);
        }
        for (uint256 i = 0; i < rareAddresses.length; i++) {
            _mintRarePack(rareAddresses[i], 4);
        }
        for (uint256 i = 0; i < legendaryAddresses.length; i++) {
            _mintLegendaryPack(legendaryAddresses[i], 4);
        }
    }
...
contract Staking {
    ...
    mapping(address => uint256) public ercStakingBalances;
    ...
    function getArtifactsBoostFractionMultE18(address account) public view returns(uint256) {
        uint256 artifactsSummaryPercent = 0;
        ...
        return artifactsSummaryPercent * 1e18 / 100;
    }

    function getArtifactsBoostPercent(address account) public view returns(uint256) {
        return (getArtifactsBoostFractionMultE18(account) * 100) / 1e18;
    }

    function getDefiBoostFractionMultE18(address account) public view returns(uint256) {
        return ercStakingBalances[account] / 100 / 100;
    }

    function getDefiBoostPercent(address account) public view returns(uint256) {
        return (getDefiBoostFractionMultE18(account) * 100) / 1e18;
    }
    ...
    function getStakingBalance(address account) public view returns(uint256) {
        ...
        uint256 boostFraction = getArtifactsBoostFractionMultE18(account) + getDefiBoostFractionMultE18(account);
        balance = (balance * (1e18 + boostFraction)) / 1e18;
        //todo check
    ...
    function getDailyRevenue(address account) public view returns(uint256) {
        ...
        uint256 boostFraction = getArtifactsBoostFractionMultE18(account) + getDefiBoostFractionMultE18(account);
        //todo check
        return (collectionEnergyPerDay * (1e18 + boostFraction)) / 1e18;
    }
    ...
    function stakeErc20(uint256 amount) public {
        address staker = msg.sender;
        _internalBalance[staker] = _resetInternalBalances(staker);

        _erc20Contract.lockTokens(staker, amount);
        ercStakingBalances[staker] += amount;
    }

    function unstakeErc20(uint256 amount) public {
        address staker = msg.sender;
        require(ercStakingBalances[staker] >= amount, "Trying to unlock too many tokens");

        _internalBalance[staker] = _resetInternalBalances(staker);

        _erc20Contract.unlockTokens(staker, amount);
        ercStakingBalances[staker] -= amount;
    }
}
...
contract Erc20Token is IERC20 {
    ...
    function lockTokens(address owner, uint256 amount) public onlyStaker {
        _transfer(owner, stakerAddress, amount);
    }

    function unlockTokens(address owner, uint256 amount) public onlyStaker {
        _transfer(stakerAddress, owner, amount);
    }
