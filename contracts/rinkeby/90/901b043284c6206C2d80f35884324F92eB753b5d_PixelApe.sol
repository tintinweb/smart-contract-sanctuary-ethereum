...
contract PixelApe is ERC721Tradable {
    ...
    function mint(uint256 numberOfTokens) public payable {
        ...
        require(canClaim(_msgSender()) && numberOfTokens <= Ape.balanceOf(_msgSender()) , "Max claim");
        require(totalPublicSupply.current() + numberOfTokens <= maxPublicSupply, "Max limit");
        for (uint256 i=0; i<numberOfTokens; i++) {
            totalPublicSupply.increment();
            _safeMint(_msgSender(), totalPublicSupply.current());
        }
