...
contract Primealphaclub is ERC721A, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "Primealphaclub - The Contract is Paused");
        require(
            totalSupply() + _amount <= maxSupply,
            "Primealphaclub - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "Primealphaclub - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "Primealphaclub - max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "Primealphaclub - insufficient ethers"
            );
        }
