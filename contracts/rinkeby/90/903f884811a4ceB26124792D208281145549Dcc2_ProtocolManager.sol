...
contract ProtocolManager is PausableUpgradeable, AccessControlUpgradeable {
    ...
    function initialize(
        ...
        USlusBurnerAddr = _USlusBurnerAddr;
    }
    ...
    function setUSPlusMinterAddress(address _USPlusMinterAddr)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        USPlusMinterAddr = _USPlusMinterAddr;
    ...
    function setUSlusBurnerAddress(address _USlusBurnerAddr)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        USlusBurnerAddr = _USlusBurnerAddr;
        ...
        returns (address)
    {
        ...
        Redeemer newRedeemer = new Redeemer();
            Redeemer(newRedeemer).initialize(
                fluentUSPlusAddress,
            USlusBurnerAddr,
            fedMemberId
            );
        ...
        IUSPlusBurner(USlusBurnerAddr).toGrantRole(address(newRedeemer));
...
/// @dev
contract Redeemer is PausableUpgradeable, AccessControlUpgradeable {
    bytes32 public constant USER_ROLE = keccak256("USER_ROLE");
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant APPROVER_ROLE = keccak256("APPROVER_ROLE");
    bytes32 public constant TRANSFER_REJECTED_AMOUNTS_OPERATOR_ROLE = keccak256("TRANSFER_REJECTED_AMOUNTS_OPERATOR_ROLE");
    bytes32 public constant TRANSFER_ALLOWLIST_TOKEN_OPERATOR_ROLE = keccak256("TRANSFER_ALLOWLIST_TOKEN_OPERATOR_ROLE");
    bytes32 public constant TRANSFER_ALLOWLIST_TOKEN_COMPLIANCE_ROLE = keccak256("TRANSFER_ALLOWLIST_TOKEN_COMPLIANCE_ROLE");
    ...
    bytes32[] public _refIds;
     ...
     function initialize(
        ...
        require(_USlusBurnerAddr != address(0x0), "ZERO Addr is not allowed");
        require(_fedMemberId != address(0x0), "ZERO Addr is not allowed");

        AccessControlUpgradeable._grantRole(DEFAULT_ADMIN_ROLE, _fedMemberId);
        AccessControlUpgradeable._grantRole(PAUSER_ROLE, _fedMemberId);
        AccessControlUpgradeable._grantRole(APPROVER_ROLE, _fedMemberId);
    ...
    {
        require(
            IERC20Upgradeable(fluentUSPlusAddress).balanceOf(msg.sender) >= amount,
            "NOT_ENOUGH_BALANCE"
        );
        require(
            IERC20Upgradeable(fluentUSPlusAddress).allowance(msg.sender, address(this)) >=
                amount,
            "NOT_ENOUGH_ALLOWANCE"
        );

        require(!burnTickets[refId].usedTicket, "ALREADY_USED_REFID");
        ...
        require(
            IERC20Upgradeable(fluentUSPlusAddress).transferFrom(
                msg.sender,
                address(this),
                amount
            ),
        ...
        returns (bool isTicketApproved)
    {
        ...
        IUSPlusBurner(USlusBurnerAddr).requestBurnUSPlus(
            ticket.refId,
            address(this),
            ticket.from,
            fedMemberId,
    ...
    function transferRejectedAmounts(bytes32 refId, address recipient)
        external
        onlyRole(TRANSFER_REJECTED_AMOUNTS_OPERATOR_ROLE)
        whenNotPaused
    {
        require(!hasRole(APPROVER_ROLE, msg.sender), "Call not allowed. Caller has also Approver Role");
        require(rejectedAmount[refId], "Not a rejected refId");

        BurnTicket storage ticket = burnTickets[refId];
        require(ticket.ticketStatus == TicketStatus.REJECTED, "Ticket not rejected");

        ticket.ticketStatus = TicketStatus.TRANSFERED;

        rejectedAmount[refId] = false;
        require(
            IERC20Upgradeable(fluentUSPlusAddress).transfer(recipient, ticket.amount),
            "FAIL_TRANSFER"
        );
    }

    function revertTicketRejection(bytes32 refId)
        external
        onlyRole(APPROVER_ROLE)
        whenNotPaused
    {
    ...
    {
        if (burnTickets[refId].usedTicket) {
            return (
                burnTickets[refId].usedTicket,
                burnTickets[refId].ticketStatus,
...
interface IUSPlusBurner {
    ...
    /// @dev
    /// @param redeemerAddr The address where the Plus was deployed
