    ...
    function resultAuction(address _nftAddress, uint256 _tokenId) external nonReentrant {
        ...
        if (winningBid >= auction.reservePrice) {
            ...
            if (royalties[_nftAddress] > uint8(0)){
                uint256 royaltyFee = aboveReservePrice.sub(platformFeeAboveReserve).mul(royalties[_nftAddress]).div(1000);
                
                (bool royalityTransferSuccess, ) = royaltyRecipient[_nftAddress].call{ value: royaltyFee }("");
                require(royalityTransferSuccess, "ArtGrailAuction.resultAuction: Failed to send platform royalty");

                (bool ownerTransferSuccess, ) = auction.owner.call{ value: winningBid.sub(platformFeeAboveReserve).sub(royaltyFee) }("");
