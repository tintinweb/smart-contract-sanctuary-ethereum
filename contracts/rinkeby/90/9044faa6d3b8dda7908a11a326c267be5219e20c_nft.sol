...
contract nft is ERC721Enumerable, Ownable {
    ...
    //amount of tokens that will be minted in presale
    uint private mintsPresale = 100;

    //declares the maximum amount of tokens that can be minted, for each type
    uint256 private mintsElite = 444;
    uint256 private mintsPublic = 1111;
    ...
    uint private mintCostElite = 111000000000000000;
    uint private mintCostPublic = 55500000000000000;

    //the maximum amount of reserved mints allowed for creator and marketing wallet
    uint private reservedMintsElite = 10;
    uint private reservedMintsPublic = 100;
    
    //number of mints an address can have maximum
    uint private maxMintPresale = 2;
    uint private maxMintPublicSale = 4;
    ...
    constructor() ERC721('WishMaco', 'WM') {
    ...
    function tokenId(uint type_) internal view returns(uint256) {
        if (type_ == 1) {
            return 1111 - mintsPublic + 1;
        }
        else{
            return 444 - mintsElite + 1;
        }
    }
    ...
    */
    ...
    //@param _r ECDSA signature parameters r.
    //@param _s ECDSA signature parameters s.
    function presaleMint(uint256 number) external payable {
        ...
        require(mintsPresale >= number, "Not enough NFTs left to mint in Presale..");
        require(mintsPerAddress[msg.sender] + number <= maxMintPresale, "Only 2 Mints are allowed per Address");
        require(msg.value >= number * mintCost(1), "Insufficient Funds to mint this number of Tokens!");
        ...
        mintsPresale = mintsPresale - number;
        mintsPublic = mintsPublic - number;
    }
    ...
    function publicSaleMint(uint256 number, uint type_) external payable {
        require(saleState() != State.NoSale, "Sale in not open yet!");
        require(saleState() == State.PublicSale, "Public Sale in not open yet!");
        require(type_ > 0 && type_ < 4, "Invalid type!");
        require(mintsLeft(type_) >= number, "Not enough NFTs left to mint of this type..");
        require(mintsPerAddress[msg.sender] + number <= maxMintPublicSale, "Only 5 Mints are allowed per Address");
        require(msg.value >= number * mintCost(type_), "Insufficient Funds to mint this number of Tokens!");
        
        
        for (uint256 i = 0; i < number; i++) {
            uint256 tid = tokenId(type_);
            _safeMint(msg.sender, tid);
        }
        mintsPerAddress[msg.sender] += number;
        if (type_ == 1) {
            ...
            mintsElite = mintsElite - number;
        }

    }

    function mintsLeft(uint type_) public view returns(uint) {
        require(type_ > 0 && type_ < 4, "Invalid type!");
        if (type_ == 1) {
            return mintsPublic - reservedMintsPublic;
        }
        else{
            return mintsElite - reservedMintsElite;
        }
    }
    ...
    function reservedMint(uint256 number, uint type_) public onlyOwner {
        require(type_ > 0 && type_ < 4, "Invalid type!");
        require(reservedMintsLeft(type_) >= number, "No NFTs left to mint of this type.."); 
        for (uint256 i = 0; i < number; i++) {
            uint256 tid = tokenId(type_);
            _safeMint(msg.sender, tid);
        }
        mintsPerAddress[msg.sender] += number;
        if (type_ == 1) {
            reservedMintsPublic = reservedMintsPublic - number;
        }
        else{
            reservedMintsElite = reservedMintsElite - number;
        }
    }
    ...
    function reservedMintsLeft(uint type_) public view returns(uint) {
        require(type_ > 0 && type_ < 4, "Invalid type!");
        if (type_ == 1) {
            return reservedMintsPublic;
        }
        else{
            return reservedMintsElite;
        }
    }
    ...
    function withdrawAll() public onlyOwner {
        ...
        withdraw(payable(0x8e763037655A59034Dc1dd5Bb535a1b31b26EB12), ((balance * 10) / 100)); 
        withdraw(payable(0x8e83da6349f0c33Aa99134476F233d4310CE6F14), ((balance * 10) / 100));
        withdraw(payable(0xf5f650Ba137bCfCa473C43d1B2538b24e6281632), accountBalance()); //to avoid dust eth
    }
    ...
    function mintCost(uint type_) public view returns(uint) {
        require(type_ > 0 && type_ < 4, "Invalid type!");
        if (type_ == 1) {
            return mintCostPublic;
        }
        else{
            return mintCostElite;
        }
    }
}