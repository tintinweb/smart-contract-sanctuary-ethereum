...
contract TownsLandCoin is ERC20, OwnableOrApproved, ITownsLandCoin {
    modifier onlyTownBuilder() {
        require(_msgSender() == _townBuilderAddress);
        _;
    }

    address private _townBuilderAddress;
    ...
    constructor(
        ...
        _townBuilderAddress = townBuilderAddress;
    }
    ...
    function mintBonus(address account, uint256 amountWei)
        ...
        onlyTownBuilder
    {
        _mint(account, amountWei);
    }

    function mint(address account, uint256 amountWei)
        external
        onlyOwnerOrApproved
    {
        _mint(account, amountWei);
    ...
    function approveAccount(address account) external onlyTownBuilder {
        _approveAccount(account);
    }

    function revokeAccount(address account) external onlyTownBuilder {
        _revokeAccount(account);
    }
}
