 ...
 */
contract Alt_Repack is PackDropBase {

    /// @dev Rinkeby Mainnet constructor.
    /// See {https://docs.chain.link/docs/vrf-contracts/v1/#rinkeby} for VRF parameters.
    constructor() PackDropBase(
        "Alt Repack I",
        2,                                                                  // Max 2 tokens per transaction OK
        10,                                                                 // Max 10 tokens per address OK
        0.001 * 10 ** 18,                                                     // 0.001 ETH on Rinkeby (subject to be updated before the drop) OK
        0xc778417E063141139Fce010982780140Aa0cD5Ab,                         // WETH on Rinkeby OK
        0xa3BddF923370ffaE55A0D412462320cb8D9e48be,                         // Alt Engineering Multisig on Rinkeby (Emile test Address) OK
        0x84B72920b6D8862bdA87ffD245045bBd57d40d12,                         // Alt Vault Proxy deployed on Rinkeby NOT OK
        0xb3dCcb4Cf7a26f6cf6B120Cf5A73875B7BBc655B,                         // VRFCoordinator on Rinkeby OK
        0x01BE23585060835E02B77ef475b0Cc51aA1e0709,                         // LINK Token on Rinkeby OK
        0x2ed0feb3e7fd2022120aa84fab1945545a9f2ffc9076fd6156fa96eaff4c1311, // Chainlink keyHash on Rinkeby OK 
        0.1 * 10 ** 18                                                   // 0.1 LINK on Rinkeby OK
    ) {}
}
...
import "./ERC20PaymentProcessor.sol";
 ...
 */
abstract contract PackDropBase is Context, ReentrancyGuard, Ownable, ERC20PaymentProcessor, PresaleList, TokenMintingPool {
     ...
     */
    function addTokens(bytes32[] memory tokenHashes, uint256 categoryNumber) public onlyOwner {
        _addTokens(tokenHashes, categoryNumber);
    }

    /**
     * @dev See {TokenMintingPool._removeTokens}.
     */
    function removeTokens(bytes32[] memory tokenHashes, uint256 categoryNumber) public onlyOwner {
        _removeTokens(tokenHashes, categoryNumber);
    }
    ...
    function saleStatus() public view returns (string memory) {
        ...
        for (uint256 i = 0; i < 6; i++) {
            supply += remainingSupplyCount(i);
        }
        bool tokenSupplyIsEmpty = supply <= 0;
     ...
     */
    function _processMintRequest(address minter, uint256 requestedAmount, uint256 categoryNumber) private {
        uint256 remainingSupply = remainingSupplyCount(categoryNumber);
        ...
        _mintTokens(minter, mintableAmount, categoryNumber);
    }
     ...
     */
    function claimTokens(uint256 amount, uint256 categoryNumber) external nonReentrant {
        address caller = _msgSender();
        require(caller == tx.origin, 'TokenDropBase: No DelegateCall authorized');
        ...
        _processMintRequest(caller, amount, categoryNumber);
    }
...
abstract contract TokenMintingPool is VRFConsumerBase {
    ...
    IVault public immutable vault;              // Reference to the registry where tokens will be minted.

    EnumerableSet.Bytes32Set[] private _availableTokenHashes;     // The available token hashes that haven't been minted yet
                                                                  // for each category.

    uint256[] public _randomSeeds;                                // A random seed number provided by Chainlink, ensuring that
     ...
     * Note: if the input is too big, the transaction will fail due to high gas limit.
     */
    function _addTokens(bytes32[] memory tokenHashes, uint256 categoryNumber) internal {
        require(!tokenSupplyLocked(), "TokenMintingPool: Token supply is locked. Cannot add new tokens.");
        require(categoryNumber < 6, 'TokenMintingPool: Only 6 categories for this repack');
        for (uint ii = 0 ; ii < tokenHashes.length ; ii++) {
            _availableTokenHashes[categoryNumber].add(tokenHashes[ii]);
        }
    }
     ...
     * Note: if the input is too big, the transaction will fail due to high gas limit.
     */
    function _removeTokens(bytes32[] memory tokenHashes, uint256 categoryNumber) internal {
        require(!tokenSupplyLocked(), "TokenMintingPool: Token supply is locked. Cannot remove tokens.");
        require(categoryNumber < 6, 'TokenMintingPool: Only 6 categories for this repack');
        for (uint ii = 0 ; ii < tokenHashes.length ; ii++) {
            _availableTokenHashes[categoryNumber].remove(tokenHashes[ii]);
        }
    }
    ...
    function fulfillRandomness(bytes32, uint256 randomness) internal override {
        if (!tokenSupplyLocked()) {
            for (uint256 i = 0; i < 6; i++) {
                _randomSeeds[i] = _expandRandomness(randomness, i);
            }
        }
    }

    /**
     * @dev From a single VRF Coordinator Random Value, we generate a seed
     */
    function _expandRandomness(uint256 randomValue, uint256 i) internal pure returns (uint256 expandedSeed) {
        expandedSeed = uint256(keccak256(abi.encode(randomValue, i)));
    }
     ...
     */
    function remainingSupplyCount(uint256 categoryNumber) public view returns (uint256) {
        return _availableTokenHashes[categoryNumber].length();
    }
     ...
     *
     */
    function _mintTokens(address to, uint256 numTokens, uint256 categoryNumber) internal {
        require(tokenSupplyLocked(), "TokenMintingPool: Token supply needs to be locked.");
        require(remainingSupplyCount(categoryNumber) >= numTokens, "TokenMintingPool: Not enough tokens left.");
        require(categoryNumber < 6, 'TokenMintingPool: Only 6 categories for this repack');
        for (uint ii = 0 ; ii < numTokens ; ii++) {
            uint256 index = _randomSeeds[ii] % remainingSupplyCount(categoryNumber);
            bytes32 tokenHash = _availableTokenHashes[ii].at(index);
            vault.mint(to, tokenHash, 1, '');
            _availableTokenHashes[ii].remove(tokenHash);
        }
    }
