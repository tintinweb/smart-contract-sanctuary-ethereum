 ...
 */
contract RocketRiderNFT is ERC721Enumerable, Ownable {
    ...
    string private revealedMetadataUri;
    string private uriSuffix = ".json";

    event Received(address, uint);

    constructor(string memory _name, string memory _symbol, string memory _hiddenMetadataUri) ERC721(_name, _symbol) {
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(_mintAmount <= maxPerWallet, "Max amount per transaction exceeded!");
        require(this.balanceOf(msg.sender) < maxPerWallet, "Max per wallet exceeded!");
    ...
    function setRevealedMetadataUri(string memory _revealedMetadataUri) public onlyOwner {
    ...
    function getFullMetadataUri(string memory _uri, uint256 _tokenId) internal view returns (string memory) {
        return bytes(_uri).length > 0 ? string(abi.encodePacked(_uri, _tokenId.toString(), uriSuffix)) : "";
    }
