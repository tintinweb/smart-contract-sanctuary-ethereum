...
contract FlameLocker is Ownable {
    ...
    constructor(address _fireFlame, address _emergencyAddress, uint256 _lockingPeriodInDays, uint256 _devLockingPeriodInDays) public {
        require(address(_fireFlame) != address(0), "_flame token is a zero address");
        require(address(_emergencyAddress) != address(0), "_emergencyAddress is a zero address");
        fireFlame = IBEP20(_fireFlame);
    ...
    function claimAllForUser(uint256 r, address user) public {
        ...
        fireFlame.transfer(user, totalTransferableAmount);
    }
    ...
    function emergencyWithdrawOwner(address _to) external onlyOwner{
        uint256 amount = fireFlame.balanceOf(address(this));
        require(fireFlame.transfer(_to, amount), 'MerkleDistributor: Transfer failed.');
    }
...
contract DevFlameDistributor is Ownable {
    using SafeMath for uint256;

    IBEP20 public flame;
    FlameLocker public flameLocker;
    ...
    constructor (
        IBEP20 _flame,
        FlameLocker _flameLocker,
        address _devWallet,
        address _marketingAndGrowthWallet
    ) public {
        require(address(_flame) != address(0), "_flame is a zero address");
        require(address(_flameLocker) != address(0), "_flameLocker is a zero address");
        flame = _flame;
        flameLocker = _flameLocker;
    ...
    function distribute(uint256 _total) external onlyOwner {
        ...
        require(flame.transfer(devWallet, devWalletShare), "transfer: devWallet failed");
        require(flame.transfer(marketingAndGrowthWallet, marketingAndGrowthWalletShare), "transfer: marketingAndGrowthWallet failed");
    }

    // funtion to claim the locked tokens for devFlameDistributor, which will transfer the locked tokens for dev to devAddr after the devLockingPeriod
    function claimLockedTokens(uint256 r) external onlyOwner {

        flameLocker.claimAll(r);
    }
    // Update flameLocker address by the owner.
    function flameLockerUpdate(address _flameLocker) public onlyOwner {
        flameLocker = FlameLocker(_flameLocker);
    }
}