...
contract JackpotApeLottery is ReentrancyGuard, IJackpotApeLottery, Ownable {
    ...
    uint256 public freeTicketTokenAmount;

    uint256 public extraFreeTickets = 1;
    address[] public extraFreeTicketTokens;
    mapping(address => uint256) public extraFreeTicketTokenAmounts;
        ...
        nonReentrant
    {
        ...
        uint256 _calcualtingTickets = _ticketNumbers.length;
        uint256 _freeTickets = getFreeTickets();
        uint256 _extraFreeTickets = getExtraFreeTickets();

        _freeTickets = _freeTickets > _calcualtingTickets ? _calcualtingTickets : _freeTickets;        
        _calcualtingTickets = _calcualtingTickets - (_freeTickets > _extraFreeTickets ? _freeTickets : _extraFreeTickets);

        if (_extraFreeTickets > _freeTickets) {
            // If user has extra free ticket, he should purchase one paid token at least.
            require(_ticketNumbers.length > _extraFreeTickets, "You have to purchase one paid ticket at least");
        }
    ...
    function setFreeTicketToken(address _freeTicketTokenAddress, uint256 _freeTicketTokenAmount) external onlyOwner {
        freeTicketToken = IERC20(_freeTicketTokenAddress);
        freeTicketTokenAmount = _freeTicketTokenAmount;
    }

    function setExtraFreeTicketTokenAmounts(address _tokenAddress, uint256 _amount) external onlyOwner {        
        if (extraFreeTicketTokenAmounts[_tokenAddress] == 0 && _amount > 0) {
            extraFreeTicketTokens.push(_tokenAddress);
        }
        extraFreeTicketTokenAmounts[_tokenAddress] = _amount;
    }

    function getFreeTickets() public view returns (uint256) {
        uint256 tickets = 0;
        if (address(freeTicketToken) != address(0) && freeTicketTokenAmount > 0) {
            tickets = freeTicketToken.balanceOf(msg.sender) / freeTicketTokenAmount;
        }

        return tickets;
    }

    function getExtraFreeTickets() public view returns (uint256) {
        uint256 extraTickets = 0;
        if (extraFreeTicketTokens.length > 0) {
            for(uint256 i = 0; i < extraFreeTicketTokens.length; i ++) {
                if (extraFreeTicketTokenAmounts[extraFreeTicketTokens[i]] > 0 && IERC20(extraFreeTicketTokens[i]).balanceOf(msg.sender) >= extraFreeTicketTokenAmounts[extraFreeTicketTokens[i]]) {
                    extraTickets = extraFreeTickets;
                    break;
                }
            }
        }
