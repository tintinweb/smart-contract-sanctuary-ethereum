...
contract MyStorage is ERC721, VRFConsumerBase, Ownable {
    ...
    event Withdraw2(uint32 indexed num);
     ...
     */
    function transferERC20Token(address recipient, uint256 value) external {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        if(isNftToken) { 
            ...
            _safeMint(dogOwner, newItemId-1);
            _setTokenURI(newItemId-1, tokenURI);
            Breed breed = Breed(randomNumber % 3); 
            tokenIdToBreed[newItemId-1] = breed;
            requestIdToTokenId[requestId] = newItemId-1;
            tokenIdTorequestId[newItemId-1] = requestId;
     ...
     */
    function getNFTtokenCount() external view returns (uint32) {
        return tokenCounter;
    }
}
...
contract EthUsPriceConvert {
    ...
    function getEntranceFee() external view returns (uint256) {
        ...
        // 50/2,000
        // 50 * 100000 / 2000
       return ((usdEntryFee * 10**18) / (uint256(price) * 10**10)); // 18 decimals
    }
}
