...
contract SVGProvider {
  ...
  function generateSVG(uint256 _tokenId, Params.SVGParams memory _params, bool _isFull) public view returns (string memory) {
...
library NFTSVG {
  function generateFullSVG(uint256 _tokenId, Params.Character[][8] memory _characters, Params.SVGParams memory _params) internal pure returns (bytes memory) {
    return
      bytes.concat(
        ...
        bytes.concat(
          ...
          generateScreen(),
          generateTitle(_tokenId, _characters, _params.character_ids)
        ),
        "</svg>"
      );
  }

  function generateCoreSVG(uint256 _tokenId, Params.Character[][8] memory _characters, Params.SVGParams memory _params) internal pure returns (bytes memory) {
    return
      bytes.concat(
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024">',
        bytes.concat(
          ...
          generateStyles(),
          generateFilterDefs(_params),
          generateFixedFilterNoise(_params),
          "</defs>"
        ),
        bytes.concat(
          generateBackground(_params),
  ...
  function generateFilterDefs(Params.SVGParams memory _params) internal pure returns (bytes memory) {
    return
      bytes.concat(
        ...
        generateFilterDSA(),
        generateFilterDSB(),
        generateFixedFilterGrey(_params),
  ...
  // updated
  function generateFixedFilterNoise(Params.SVGParams memory _params) internal pure returns (bytes memory) {
    return
      bytes.concat(
  ...
  // updated
  function generateFixedFilterGrey(Params.SVGParams memory _params) internal pure returns (bytes memory) {
    bytes memory values = Combine.getFixedMatrixByPaletteAndOffset(_params.color.palette);
    return
      bytes.concat(
        bytes.concat('<filter id="FFG">'), //
  ...
  function generateFilterDSA() internal pure returns (bytes memory) {
    return
      bytes.concat(
        '<filter id="FDSA" x="-20%" y="-20%" width="140%" height="140%">', //
        bytes.concat(
          '<feDropShadow stdDeviation="0" dx="-2" dy="-2" flood-color="hsl(0,0%,90%)"/>'
  ...
  function generateScreen() internal pure returns (bytes memory) {
    return
      bytes.concat(
        ...
        '<rect x="720" y="815" width="96" height="33"/>',
        "</g>",
        "</g>",
        "</g>"
      );
  ...
  // updated
  function generateTitle(uint256 _tokenId, Params.Character[][8] memory _characters, uint8[8] memory _charactersIds) internal pure returns (bytes memory) {
    uint8 gap = 14;
    uint16 translateX = 208;
    bytes memory truth;
    for (uint8 i = 0; i < 8; i++) {
    ...
    return
      bytes.concat(
        ...
        "</g>",
        "<g>",
        abi.encodePacked('<text class="colB" style="font-size:24px;font-weight:bold;font-family: \'Courier New\',monospace;" text-anchor="middle" transform="translate(760,840)">', Strings.toString(_tokenId),'</text>'),
        "</g>",
        "</g>",
        "</g>"
      );
...
library Combine {
  ...
  function getMatrixByPaletteAndOffset(uint8 _palette, uint8 _offset) internal pure returns (bytes memory) {
    ...
    bytes[3] memory red = [bytes("0.132"), "0.318", "0.603"];
    bytes[3] memory green = [bytes("0.318"), "0.603", "0.132"];
    bytes[3] memory blue = [bytes("0.603"), "0.132", "0.318"];
  ...
  function getFixedMatrixByPaletteAndOffset(uint8 _palette) internal pure returns (bytes memory) {
    _palette = _palette % 3;

    bytes[3] memory red = [bytes("0.132"), "0.318", "0.603"];
    bytes[3] memory green = [bytes("0.318"), "0.603", "0.132"];
    bytes[3] memory blue = [bytes("0.603"), "0.132", "0.318"];
    bytes memory row = bytes.concat(red[_palette], " ", green[_palette], " ", blue[_palette], " 0 0 ");
    return bytes.concat(row, row, row, "0 0 0 1 0");
  }
}