...
contract VRFV2Wrapper is ConfirmedOwner, TypeAndVersionInterface, VRFConsumerBaseV2, VRFV2WrapperInterface {
  ...
  function calculatePrice(uint32 callbackGasLimit) public view override onlyConfiguredNotDisabled returns (uint256) {
    return calculatePriceInternal(callbackGasLimit, tx.gasprice);
  }

  function estimatePrice(uint32 _callbackGasLimit, uint256 _requestGasPrice)
    external
    view
    onlyConfiguredNotDisabled
    returns (uint256)
  {
    return calculatePriceInternal(_callbackGasLimit, _requestGasPrice);
  }

  function calculatePriceInternal(uint32 _callbackGasLimit, uint256 _requestGasPrice) internal view returns (uint256) {
    int256 weiPerUnitLink = getFeedData();
    require(weiPerUnitLink >= 0, "Invalid LINK wei price");

    uint256 feeNoWrapperPremium = (1e18 * _requestGasPrice * (_callbackGasLimit + s_wrapperGasOverhead)) /
  ...
  function onTokenTransfer(
    ...
    s_callbacks[requestId] = _sender;
    lastRequestId = requestId;
  }

  function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
    address callback = s_callbacks[_requestId];
    require(callback != address(0), "request not found"); // This should never happen
    VRFConsumerBaseV2(callback).rawFulfillRandomWords(_requestId, _randomWords);
  }
...
interface VRFV2WrapperInterface {
  ...
  function calculatePrice(uint32 _callbackGasLimit) external view returns (uint256);
}
