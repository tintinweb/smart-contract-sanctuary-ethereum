...
error NftMarketplace__CantBeZero();
...
contract NftMarketplace is ReentrancyGuard {
    ...
    modifier isOwner(
        ...
        if (IERC721(nftAddress).ownerOf(tokenId) != spender) {
            // compare this modifier to one on github for gas cost
    ...
    function buyItem(address nftAddress, uint256 tokenId) external payable nonReentrant isListed(nftAddress, tokenId) {
        ...
        // Sending the money to the user is a NO GO
        // Have them withdraw the money is the way
    ...
    function updateListing(
        ...
        if (newPrice <= 0) {
            revert NftMarketplace__CantBeZero();
        }
