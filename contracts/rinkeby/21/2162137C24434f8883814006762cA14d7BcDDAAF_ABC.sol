...
contract ABC is ERC20, Ownable {
    ...
    /// @notice Contract ABC balance threshold before `_swap` is invoked
    ...
    /// @notice Change the minimum contract ABC balance before `_swap` gets invoked
    ...
    /// @notice Rescue ABC from the marketing amount
    /// @dev Should only be used in an emergency
    /// @param _amount The amount of ABC to rescue
    /// @param _recipient The recipient of the rescued ABC
    ...
    /// @notice Rescue ABC from the treasury amount
    /// @dev Should only be used in an emergency
    /// @param _amount The amount of ABC to rescue
    /// @param _recipient The recipient of the rescued ABC
    ...
    /// @notice Rescue ABC from the reflection amount
    /// @dev Should only be used in an emergency
    /// @param _amount The amount of ABC to rescue
    /// @param _recipient The recipient of the rescued ABC
    ...
    /// @notice Perform a Uniswap v2 swap from ABC to ETH and handle tax distribution
    /// @param amount The amount of ABC to swap in wei
    ...
    function withdrawAll() public onlyOwner() {
        ...
        uint256 split = currentBalance / 2;
        buybackWallet.transfer(split);
        treasuryWallet.transfer(currentBalance - split);
    }

    /// @notice Transfers ABC from an account to this contract for taxes
    /// @param _account The account to transfer ABC from
    ...
    function setSwapFees(bool _swapFees) public onlyOwner() {
        swapFees = _swapFees;
    }
