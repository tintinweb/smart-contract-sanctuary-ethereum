// SPDX-Lisence-Identifire: UNLISENCED
...
contract TestNft is ERC721A, Ownable {
  ...
  constructor() ERC721A('TestMint','TM'){
   ...
   function mint_onlywl(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable {
    ...
    require(wlMinted[msg.sender] < maxWlMintAmount, 'Address already claimed!');
    require(_mintAmount > 0 && wlMinted[msg.sender] + _mintAmount <= maxWlMintAmount, 'claim is over max amount');
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(tx.origin == msg.sender, 'the caller is another controler');
    require(totalSupply() + _mintAmount <= maxSupply, 'Max supply exceeded!');
     require(_mintAmount > 0 && publicMinted[msg.sender] + _mintAmount <= maxPublicMintAmount, 'claim is over max amount');
    require(msg.value >= cost * _mintAmount, "not enough eth");
  ...
  function setMaxWlMintAmount(uint256 _maxWlMintAmount) public onlyOwner {
