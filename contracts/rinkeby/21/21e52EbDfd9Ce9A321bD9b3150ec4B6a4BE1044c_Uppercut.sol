...
contract Uppercut is ERC721A, Ownable, ReentrancyGuard {
    ...
    // Mainnet winter contract address: 0xd541da4C37e268b9eC4d7D541Df19AdCf564c6A9
    // Rinkeby winter contract address: 0xe0CB05cBf3dBeb647394905848d9361Daa99dE28
    // Local winter contract address: 0x70997970C51812dc3A010C7d01b50e0d17dc79C8
    address public immutable WINTER_ADDRESS;
    ...
    uint256 public immutable numberOfHeroesForTeam;
    uint256 public immutable numberOfHeroesForAllowList;
    ...
    constructor(
        uint32 _numberOfHeroes,
        uint32 _numberOfHeroesForTeam,
        uint32 _numberOfHeroesForAllowList,
        address _winter_address
    ) ERC721A("Uppercut", "HERO") {
        require(_numberOfHeroes % NUMBER_OF_DOMOS == 0, "heroes % 3 == 0");
        require(_numberOfHeroesForTeam < _numberOfHeroes, "team < heroes");
        require(
            (_numberOfHeroes - _numberOfHeroesForTeam) % NUMBER_OF_DOMOS == 0,
            "(heroes - team) % 3 == 0"
        );
        require(
            (_numberOfHeroes -
                _numberOfHeroesForTeam -
                _numberOfHeroesForAllowList) %
                NUMBER_OF_DOMOS ==
                0,
            "(heroes - team - allowList) % 3 == 0"
        );
        numberOfHeroes = _numberOfHeroes;
        numberOfHeroesForTeam = _numberOfHeroesForTeam;
        numberOfHeroesForAllowList = _numberOfHeroesForAllowList;
        numberOfHeroesPerDomos = _numberOfHeroes / NUMBER_OF_DOMOS;
        WINTER_ADDRESS = _winter_address;
    }
    ...
    function seedAllowlist(address[] calldata _addresses) external onlyOwner {
    ...
    function mintMultiple(
        ...
        uint256 _tngo,
        uint256 _gnasis,
        uint256 _perDomosLimit,
        uint256 _heroLimit
    ) private {
        uint256 totalQuantity = _oqo + _tngo + _gnasis;

        if (_transferTo != WINTER_ADDRESS) {
            require(
                balanceOf(_transferTo) + totalQuantity <= MAX_PER_WALLET,
                "Max 5 per wallet"
            );
        }

        require(totalQuantity <= MAX_PER_MINT, "Max 5 at once");

        require(_currentIndex + totalQuantity <= _heroLimit, "Sold out");

        // [domosOqoBalance, domosTngoBalance, domosGnasisBalance]
        uint256 lastMintedHeroIdOqo = balances[0];
        uint256 lastMintedHeroIdTngo = balances[1];
        uint256 lastMintedHeroIdGnasis = balances[2];

        require(lastMintedHeroIdOqo + _oqo <= _perDomosLimit, "Oqo sold out");
        require(
            lastMintedHeroIdTngo + _tngo <= _perDomosLimit,
            "Tngo sold out"
        );
        require(
            lastMintedHeroIdGnasis + _gnasis <= _perDomosLimit,
            "Gnasis sold out"
        );

        updateDomoNFTMapping(
            _currentIndex,
            _currentIndex + _oqo,
            0,
            lastMintedHeroIdOqo
        ...
        updateDomoNFTMapping(
            _currentIndex + _oqo,
            _currentIndex + _oqo + _tngo,
            1,
            lastMintedHeroIdTngo
        );

        updateDomoNFTMapping(
            _currentIndex + _oqo + _tngo,
            _currentIndex + totalQuantity,
            2,
            lastMintedHeroIdGnasis
        );

        _safeMint(_transferTo, totalQuantity);
    ...
    function updateDomoNFTMapping(
        uint256 _startIndex,
        uint256 _endIndex,
        uint256 _domoId,
        uint256 _lastMintedHeroId
    ) private {
        for (uint256 i = _startIndex; i < _endIndex; i++) {
            mintIndexToDomosIndex[i] = _domoId;
            mintIndexToHeroInDomosIndex[i] =
                _lastMintedHeroId +
                (i - _startIndex);
        }
    }

    function allowlistMint(uint256 _domosId) external payable callerIsUser {
        require(_domosId < NUMBER_OF_DOMOS, "Invalid _domosId");
        require(
            saleConfig.allowListPrice != 0,
            "Allowlist sale has not begun yet"
        );
        require(allowlist[msg.sender] > 0, "Not eligible");
        require(_currentIndex < numberOfHeroes, "Sold out");
        require(balances[_domosId] < numberOfHeroesPerDomos, "Domos sold out");

        mintIndexToHeroInDomosIndex[_currentIndex] = balances[_domosId];
        mintIndexToDomosIndex[_currentIndex] = _domosId;
        ...
        refundIfOver(saleConfig.allowListPrice);
    }
    ...
    function publicMint(
        ...
        require(saleConfig.publicPrice != 0, "Public sale has not yet begun");
        require(_oqo + _tngo + _gnasis > 0, "Invalid quantity");
        ...
        refundIfOver(saleConfig.publicPrice * (_oqo + _tngo + _gnasis));
    }
    ...
    function teamMint() external onlyOwner {
        ...
        for (uint256 i = _currentIndex; i < numberOfHeroesForTeam; i++) {
            _domosId = i % NUMBER_OF_DOMOS;
            mintIndexToHeroInDomosIndex[i] = uint256(i / 3);
            mintIndexToDomosIndex[i] = _domosId;
        }

        uint256 perDomos = numberOfHeroesForTeam / NUMBER_OF_DOMOS;

        balances = [perDomos, perDomos, perDomos];

        _safeMint(msg.sender, numberOfHeroesForTeam);
    }
}
