...
contract AssetManager is Initializable, UUPSUpgradeable, MultiToken, Manageable, IAssetManager, ERC721HolderUpgradeable, AdamOwned {
    ...
    event TestAmount(address strategy, uint256 amountIn, uint256 amountOut, uint256 amountInLeft, uint256 amountOutLeft, uint256 totalBalance);
    ...
    function execSwapTransaction(address to, bytes memory _data, uint256 value, address[] calldata execStrategies, uint8[] calldata ratio) external {
        ...
        for(uint i = 0; i < execStrategies.length; i++) {
            ...
            emit TestAmount(execStrategies[i], strategyDetails.amountIn, strategyDetails.amountOut, strategyDetails.amountInLeft, strategyDetails.amountOutLeft, strategyDetails.totalBalanceOfTokenIn);
