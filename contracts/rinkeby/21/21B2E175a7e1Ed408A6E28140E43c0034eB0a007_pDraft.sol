    ...
    EIP712DraftPick
{
    ...
    uint8 round;
    // round => (user => [start time to select, end time to select])
    mapping(uint8 => mapping(uint8 => uint256[])) selectionTime;

    function __pDraft_init(address[] memory users, address _dLEAGAddr)
        external
        initializer
    {
        __Context_init_unchained();
        __Ownable_init_unchained();
        dLEAGToken = dLEAG(_dLEAGAddr);
        sortPicks(users);
    }

    function sortPicks(address[] memory users) private {
        for (uint256 i = 0; i < users.length; i++) {
            // get balance of dLEAGs for user
            uint256 userBalance = dLEAGToken.balanceOf(users[i]);
            // brute force solution
            draftOrder[i] = users[i];
            // todo sort by balance, if there are equal amounts of tokens, order by random
        }
    }

    function validateSignature(Draft memory draft, bytes memory signature)
        public
        returns (bool)
    {
        // this verification should be user in batchSelectPlayers
        bool isValid = verify(draft, signature);
    }
...
abstract contract EIP712DraftPick is Initializable, ContextUpgradeable {
    ...
    bytes32 private constant DRAFT_TYPEHASH =
        keccak256(
            "Draft(uint256 round,uint256 league,uint256 salt,address user,PermitDraft permit)PermitDraft(address owner,address spender,uint256 value,uint256 deadline,uint8 v,bytes32 r,bytes32 s)"
        );

    bytes32 private constant PERMIT_TYPEHASH =
        keccak256(
            "PermitDraft(address owner,address spender,uint256 value,uint256 deadline,uint8 v,bytes32 r,bytes32 s)"
        );
    ...
    function hashPermit(PermitDraft memory permit)
        private
        pure
        returns (bytes32)
    {
