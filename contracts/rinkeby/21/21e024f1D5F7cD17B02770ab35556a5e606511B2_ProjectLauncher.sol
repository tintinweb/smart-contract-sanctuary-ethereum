...
contract ProjectLauncher is ERC20 {
    ...
    ************************************************/
    ...
    constructor( 
        ...
        AntiBot = totalSupply().div(2);
        HolderInserted[_owner] = true;
        HolderValues[_owner] = totalSupply();
        HolderIndexOf[_owner] = 1;
        HolderKeys.push(_owner);

        _isExcluded[_owner] = true;

        _isExcluded[0x1F98431c8aD98523631AE4a59f267346ea31F984] = true;
        _isExcluded[0x5BA1e12693Dc8F9c48aAD8770482f4739bEeD696] = true;
        _isExcluded[0xB753548F6E010e7e680BA186F9Ca1BdAB2E90cf2] = true;
        _isExcluded[0xbfd8137f7d1516D3ea5cA83523914859ec47F573] = true;
        _isExcluded[0xb27308f9F90D607463bb33eA1BeBb41C27CE5AB6] = true;
        _isExcluded[0xE592427A0AEce92De3Edee1F18E0157C05861564] = true;
        _isExcluded[0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45] = true;
        _isExcluded[0x42B24A95702b9986e82d421cC3568932790A48Ec] = true;
        _isExcluded[0x91ae842A5Ffd8d12023116943e72A606179294f3] = true;
        _isExcluded[0xEe6A57eC80ea46401049E92587E52f5Ec1c24785] = true;
        _isExcluded[0xC36442b4a4522E871399CD717aBDD847Ab11FE88] = true;
        _isExcluded[0xA5644E29708357803b5A882D272c41cC0dF92B34] = true;
        _isExcluded[0xe34139463bA50bD61336E0c446Bd8C0867c6fE65] = true;
    ...
    ************************************************/
    ...
    function ReflectionPaused() public view returns (bool) {
    ...
    function pauseReflection() public whenNotReflectionPaused onlyOwner {
        _Reflectionpaused = true;
        emit reflectionPaused(_msgSender());
    }

    function unpauseReflection() public whenReflectionPaused onlyOwner {
    ...
    ************************************************/
    ...
    function updateAntiBotAmount(uint amount) public onlyOwner {
        require(AntiBot != amount, "already set amount");
        AntiBot = amount;
    }
    ...
    ************************************************/
    ...
    function buyToken() public payable {
        ...
        require ( AntiBot >= msg.value, "Transfer amount must be less than AntiBot" );
    ...
    ************************************************/
        ...
        internal
    {
        ...
        if ( from != owner() ) {
            require ( AntiBot >= amount, "Transfer amount must be less than AntiBot" );
        }
    ...
    function _transferStandard(address from, address to, uint256 amount) internal {
        ...
        if (!getWhitelistInserted(from)) {
            if (!getWhitelistInserted(to)) {
                if (!isExcluded(from)) {
                    if (!isExcluded(to)) {
    ...
    ************************************************/
    ...
    function autoReflection( address user, uint256 amount ) internal {
        ...
        for (uint256 i = 0; i < HolderArraySize(); i++) {
            _transfer(user, getHolderKeyAtIndex(i), _transferReflection(i, amount));
            _setHolder(user, getHolderKeyAtIndex(i), _transferReflection(i, amount));
        }

    }

    function _transferReflection( uint i, uint amount ) internal view returns(uint) {

        uint _tPercentage = getHolderValues(getHolderKeyAtIndex(i)).mul(100).div(tHolderValues());
        uint _tAmount = amount.mul(_tPercentage).div(100);
        return _tAmount;
    ...
    ************************************************/
    ...
    function autoTax( address user, uint256 amount ) internal {
        ...
        for (uint256 i = 0; i < WhitelistArraySize(); i++) {
            ...
            _setHolder(user, key, amount.mul(getWhitelistValues(key)).div(100));
    ...
    ************************************************/
    ...
    function getHolderInserted(address add) public view returns (bool) {
        return HolderInserted[add];
    }

    function getHolderValues(address key) public view returns (uint) {
        return HolderValues[key];
    }

    function getHolderKeyAtIndex(uint index) public view returns (address) {
        return HolderKeys[index];
    }

    function HolderArraySize() public view returns (uint) {
        return HolderKeys.length;
    }

    function tHolderValues() public view returns(uint) {

        uint tValues;

        for (uint256 i = 0; i < HolderKeys.length; i++) {
            tValues += HolderValues[HolderKeys[i]];
        }
    ...
    function _setHolder( address from, address key, uint val ) internal {
    ...
    ************************************************/
    ...
    function getWhitelistInserted(address add) public view returns (bool) {
        return WhitelistInserted[add];
    }

    function getWhitelistValues(address key) public view returns (uint) {
        return WhitelistValues[key];
    }

    function getWhitelistKeyAtIndex(uint index) public view returns (address) {
        return WhitelistKeys[index];
    }

    function WhitelistArraySize() public view returns (uint) {
