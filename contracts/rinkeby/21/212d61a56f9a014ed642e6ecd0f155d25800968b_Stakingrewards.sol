...
contract Stakingrewards is Ownable, Pausable {
    ...
    uint256 public rewardPerSecond = 1000e18;
    ...
    function earned(address _account) public view returns(uint256) {
        uint256 earn = ((block.timestamp).sub(Invtime[msg.sender])).mul(rewardPerSecond);
        return (earn.mul(Deposit[msg.sender]).div(1e18)).add(Pending[_account]);
    }
