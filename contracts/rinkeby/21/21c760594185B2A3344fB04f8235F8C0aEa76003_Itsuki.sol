...
contract Itsuki is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public maxMintWhitelisted = 10;
    uint256 public maxMintNormalSale = 5;
    ...
    bool public normalSaleActive = false;
    ...
    function reserve(uint256 _mintNum) external onlyOwner callerIsUser {
    ...
    function getCost(uint256 _mintNum) public view returns (uint256 cost) {
        if (msg.sender == owner()) {
            return 0;
        } else {
            if (normalSaleActive && whitelistSaleActive) {
                if (
                    _whiteListed[msg.sender] &&
                    balanceOf(msg.sender) + _mintNum <= maxMintWhitelisted
                ) {
                ...
                return whitelistCost;
            } else if (normalSaleActive) {
                return publicCost;
            }
        }
    }

    function getNFT(uint256 _mintNum) public payable callerIsUser {
        uint256 supply = totalSupply();
        uint256 cost = getCost(_mintNum);
        require(whitelistSaleActive || normalSaleActive, "not ready for sale");
        require(supply + _mintNum <= maxSupply, "Supply Limit Reached");
        ...
        if (whitelistSaleActive) {
            if (
                ...
                _safeMint(msg.sender, _mintNum);
                refundIfOver(cost * _mintNum);
            } else if (normalSaleActive) {
                if (_whiteListed[msg.sender]) {
                    require(
                        balanceOf(msg.sender) + _mintNum <=
                            maxMintNormalSale + maxMintWhitelisted,
    ...
    function setNormalSale(bool _state) external onlyOwner {
        normalSaleActive = _state;
    }
    ...
    function setmaxMintWhitelisted(uint256 _num) external onlyOwner {
        maxMintWhitelisted = _num;
    }

    function setmaxMintNormalSale(uint256 _num) external onlyOwner {
