...
contract DegePepe is ERC721A, Ownable {
    ...
    function mintPepeFree() external payable {
        uint256 currentPepe = totalSupply();
        require(publicSale, "Public sale not active");
        if (numPepes[msg.sender] >= freeLimit) {
            require(msg.value == price, "Mint price sent is not correct. You can free mint max 1 DGP per wallet, please consider minting for price");
        }
        require(currentPepe + 1 <= pepesLimit, "Pepe supply has exceeded");
        _safeMint(msg.sender, 1);
        numPepes[msg.sender] += 1;
    }

    // function mintPepe() external payable {
    //     uint256 currentPepe = totalSupply();
    //     require(publicSale, "Public sale not active");
    //     require(currentPepe > 2554, "Free pepe supply has not exceeded yet");
    //     require(msg.value == price, "Mint price sent is not correct");
    //     require(currentPepe + 1 <= pepesLimit, "Pepe supply has exceeded");
    //     _safeMint(msg.sender, 1);
    //     numPepes[msg.sender] += 1;
    // }
...
contract ERC721A is IERC721A {
     ...
     */
    function _startTokenId() public view virtual returns (uint256) {
        return 0;
    }
