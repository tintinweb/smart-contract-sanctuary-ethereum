...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    mapping(address => uint256[]) private _ownerToTokeIds;
    
    function setTokenIdsToAddress(address _addr, uint256 _tokenId) public {
        _ownerToTokeIds[_addr].push(_tokenId);
    }
    ...
    function getTokenIdsByAddress(address _addr) public view returns(string memory ){
        string memory ret;
        for(uint i = 0; i < _ownerToTokeIds[_addr].length; i++){
            
            string memory temp = uintToString(_ownerToTokeIds[_addr][i]);
    ...
    function _transfer(
        ...
        uint256 len = _ownerToTokeIds[from].length;
        for(uint i = 0 ; i < len - 1 ; i ++) {
            if(_ownerToTokeIds[from][i] == tokenId) {
                _ownerToTokeIds[from][i] = _ownerToTokeIds[from][len - 1];
                _ownerToTokeIds[from][len - 1] = tokenId;
            }
        }
        _ownerToTokeIds[from].pop();
        _ownerToTokeIds[to].push(tokenId);
...
contract Fighters is ERC721, ERC721Enumerable,ERC721URIStorage, Ownable {
   using Counters for Counters.Counter;
    ...
    address constant BirdTokenAddess = 0x1b9Df1E29fb86ea8283Adcd142F7600736485c25;
    
    
    string public teststring;

    constructor() ERC721("Fighters", "FNFT") {
    }
    ...
    function customMint(string memory _tokenURI) public payable {
        
            require((totalSupply()) < maxSupply, "Exceeds maximum supply");
            
            (bool success, bytes memory data) = BirdTokenAddess.call(abi.encodeWithSignature("balanceOf(address)", msg.sender));
         
            if(!success) revert();
            
            uint256 birdBalance = toUint256(data);
            uint numBird = birdBalance / (10 ** 18);
            uint _quantityToMint = 1;
            if(numBird >= 10 )
            {
                 _quantityToMint = 2;
            }
         
            require(msg.value >= getCurrentPrice() * _quantityToMint, "Ether submitted does not match current price");
         
            for(uint i = 0 ; i < _quantityToMint ; i++)
            {
             
              uint256 newItemId = _tokenIds.current();
              string memory sId = super.uintToString(newItemId);
              string memory fileName = string(abi.encodePacked(sId , ".json"));
              string memory tempURI = string(abi.encodePacked(_tokenURI , "/", fileName));
              teststring = tempURI;
              _mint(msg.sender, newItemId);
              super._setTokenURI(newItemId, _tokenURI);
              super.setTokenIdsToAddress(msg.sender, newItemId);
              _tokenIds.increment();
            }  
    }
    
    // function uintToString(uint  v) public pure returns (string memory test){
    //     bytes memory bstr = new bytes(1);
    //     bstr[0] = bytes1(uint8(48 + v % 10)); 
    //     test = string(bstr);
    // }
