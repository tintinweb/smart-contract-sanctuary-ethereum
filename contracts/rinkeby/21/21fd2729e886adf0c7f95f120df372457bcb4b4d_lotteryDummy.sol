...
// File: contracts/lotteryDummy.sol
...
contract lotteryDummy is Ownable, VRFConsumerBaseV2{
   ...
   //??
    ...
    struct LotteryDetails{
        ...
        // no need for lottery number, getting it from loterry Number
    ...
    uint32 constant publicMember = 1;
    uint32 constant commonMember = 2;
    uint32 constant uncommonMember = 3;
    ...
    function registerForParticipant(uint32 memberType, uint32 _lotteryNumber) external{

        require(msg.sender != address(0), "Mint to the zero address");

        //using numbers instead of string 
        ...
        require(memberType == publicMember || memberType == commonMember || memberType == uncommonMember , "Participants type not correct!");
...
//randome assigner is for users guesses
...
//getting the value fof user guesss
    ...
    //making request for random number to chainlink
    ...
    function endLottery(uint32 _lotteryNumber) public onlyOwner returns(address[] memory){
        ...
        // no need to add this as we are applying check for firsct call randm number generator that already has it 
    ...
    //new functionality 
    function updateTierCount(uint32 teirType, uint32 newLimit) public onlyOwner {

        require ( teirType == 1 || teirType == 2 || teirType == 3 , "Participants type not correct!");
    ...
    //we can handle both status with only one conditions true or start
