...
error RiggedRoll__NotSuccessful();
error RiggedRoll__WithdrawNotSuccessful();
error RiggedRoll__HigherNumber();

contract RiggedRoll is Ownable {
    uint256 public nonce = 0;
    uint256 private amountInEth = 0.002 ether;
    ...
    function withdraw(address _addr, uint256 _amount) public payable onlyOwner {
        ...
        if (!success) {
            revert RiggedRoll__WithdrawNotSuccessful();
        }
    }
    ...
    function riggedRoll() public payable {
        ...
        if (roll <= 2) {
            (bool success, ) = payable(diceGame).call{value: amountInEth}(
                abi.encodeWithSelector(DiceGame.rollTheDice.selector)
            );
            if (!success) {
                revert RiggedRoll__NotSuccessful();
            }
        } else {
            revert RiggedRoll__HigherNumber();
        }
    }
