...
*/
contract Comic2Apes is PVERC721, PVAllowlist, PaymentSplitter {

    uint256 purchaseWindowOpens;
    uint256 stageLength = 3600;

    uint256 constant MAX_PURCHASE = 750;
    uint256 constant MAX_PURCHASE_PER_TX = 1;

    bytes32[] merkleRoots;
    mapping(address => uint256) public purchased;

    uint256 mintPrice = 1e17;
    ...
    ) PVERC721(_name, _symbol, _uri) PaymentSplitter(payees, shares_){
        ...
        whenInAllowlist(index, maxAmount, merkleProof, merkleRoots[stage-1]) 
    {
        ...
        eliteApeHelper.increasePurchased(msg.sender, amount);
...
interface IEliteApeHelper {
    ...
    function increasePurchased(address account, uint256 amount) external;
 }
