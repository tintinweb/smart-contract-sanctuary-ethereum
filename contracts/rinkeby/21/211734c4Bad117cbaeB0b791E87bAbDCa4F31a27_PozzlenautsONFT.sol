...
contract UniversalONFT721 is ONFT721 {
    ...
    uint256 public mintPrice = 199;

    string public baseURI;

    mapping(address => uint) public countMinted;
    ...
    modifier isMerkleProoved(bytes32[] calldata _merkleProof) {
        require(
        MerkleProof.verify(
            _merkleProof,
            glMerkleRoot,
    ...
    modifier notClaimed() {
        require(isClaimed[msg.sender] == false, "UniversalONFT721: Greenlist already claimed!");
        _;
    }

    function _mint(uint _mintCount) private {
        require(
            nextMintId + _mintCount - 1 <= maxMintId,
            "UniversalONFT721: max mint limit reached"
        );

        usdcToken.transferFrom(msg.sender, address(this), _mintCount * mintPrice * 10 ** usdcToken.decimals());

        for(uint256 i = 0; i < _mintCount; i ++) {
            uint256 newId = nextMintId;
            _safeMint(msg.sender, newId);
            _setTokenURI(newId, Strings.toString(newId));

            nextMintId ++;
        }
    }

    function publicMint(uint _mintCount) external payable {
        require(
        ...
        countMinted[msg.sender] = countMinted[msg.sender] + _mintCount;
    }
...
contract ONFT721 is ONFT721Core, ERC721URIStorage, ReentrancyGuard, IONFT721 {
    constructor(
        string memory _name,
        string memory _symbol,
        address _lzEndpoint
