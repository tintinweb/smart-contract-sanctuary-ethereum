...
contract Etheregiftme is Ownable {
    struct Gift {
        address gifter;
        uint256 amount;
        string message;
    ...
    event Gifted(Gift gift);
    event FundsPulled(uint256 amount);

    function giftMe(string memory message) public payable {
        require(msg.value > 0, "Sent value is zero");
        require(bytes(message).length > 0, "Message is empty");
        Gift memory gift = Gift(msg.sender, msg.value, message);
        gifts.push(gift);
        emit Gifted(gift);
    }
    ...
    function pullFunds() public onlyOwner {
        ...
        emit FundsPulled(currentBalance);
    }
}
