...
contract Integration {
    ...
    event IntegrationUpdated(bool status);
    ...
    mapping(address => Integrations) integrationMap;
    ...
    //add single post
    function addIntegration(string memory _hash_integration,string memory _hash_integration_approve, address payable _account, uint256 amount_integration,uint256 amount_approve, bytes32[] memory groups,bytes32 group_for_vote)
    public
    onlyPost
    returns (uint256) {
        ...
        integration_approve.addIntegrationApprove(_hash_integration_approve,_account,amount_approve, groups,group_for_vote,block.number+1,block.number+10);
        emit IntegrationAdded(true,"Integration addded",amount_integration,_id);
        return _id;
    }
    ...
    onlyPost {
        ...
        emit IntegrationUpdated(true);
    }

    //delete single post
    function deleteSingleIntegration(uint _id)
    public
    onlyPost
    {
        //delete posts[_id];
        for(uint i = _id; i < integrations.length-1; i++){
...
contract IntegrationApprove {
    ...
    //change hash of single integration
    ...
    // get needed groups for integration
    ...
    // get status of confirm - pending/active/itd
    ...
    // get status of confirm single group
