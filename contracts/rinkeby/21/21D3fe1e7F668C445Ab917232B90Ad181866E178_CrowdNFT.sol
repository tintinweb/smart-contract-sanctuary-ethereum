...
contract CrowdNFT is ERC1155, Ownable, ERC1155Burnable, ERC1155Pausable, ERC1155Supply, ERC1155URIStorage, Lockable {
    ...
    mapping(uint256 => uint256[]) private _projectItems;
    ...
    function mint(uint256 projectId, uint256 id, uint256 amount, uint price, bytes memory data) public onlyOwner
    {
        ...
        _projectItems[projectId].push(id);

        _setURI(id, Strings.toString(id));
    }

    function mintBatch(uint256 projectId, uint256[] memory ids, uint256[] memory amounts, uint[] memory prices, bytes memory data) public onlyOwner
    {
    ...
    function buy(uint256 id, uint256 amount) public {
        ...
        TokenInfo storage tokenInfo = _tokenInfos[id];

        uint totalAmount = amount * tokenInfo.price;

        payable(owner()).transfer(tokenInfo.price);
        _safeTransferFrom(owner(), msg.sender, id, totalAmount, "");
    }
    ...
    function getProjectItems(uint projectId) public view returns (uint256[] memory)
    {
        return _projectItems[projectId];
    }
    ...
    function burn(
        ...
        if (account == owner()) {
            TokenInfo storage tokenInfo = _tokenInfos[id];
            tokenInfo.totalBurned = tokenInfo.totalBurned + value;
        }
    }
