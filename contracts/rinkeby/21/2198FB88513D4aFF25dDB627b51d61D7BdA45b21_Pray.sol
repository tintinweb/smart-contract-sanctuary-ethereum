...
contract Pray is ERC721A, ERC2981, Ownable {
  ...
  bool public mintable = true; // todo: false
  bool public teamMinted;

  address public withdrawAddress;
  address public teamWallet; // todo: hardcode this?

  string public baseTokenURI;

  // Constructor

  constructor() ERC721A("Butter", "Butta") {
    _mint(msg.sender, 1);
  }
  ...
  function mint(uint _amount) external payable {
    if (mintable == false) revert SaleNotStarted();
    if (totalSupply() + _amount > MAX_SUPPLY) revert MintedOut();
    if (_amount > maxMint) revert CantMintThatMany();
    if (_amount * price != msg.value) revert WrongPrice();
  ...
  function teamMint() external onlyOwner {
    ...
    _mint(teamWallet, 10); // todo: update with real value
    teamMinted = true;
  }
  ...
  */
  ...
  /**
  @notice Sets the max that a person can mint
  */
  function setMaxMint(uint _count) external onlyOwner {
    maxMint = _count;
  }

  /**
  @notice Turns on and off the contract
  */
  function setMintable(bool _val) external onlyOwner {
    mintable = _val;
  }
  ...
  */
  ...
  // do we want a separate contract to point to?
