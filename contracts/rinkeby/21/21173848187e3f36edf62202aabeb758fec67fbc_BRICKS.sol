...
contract BRICKS is ERC721Enum, Ownable, ReentrancyGuard {
    using Strings for uint256;
	
    uint256 public SALE_NFT = 1000;
	uint256 public MAX_MINT_RESERVE = 2;
	uint256 public MAX_MINT_SALE = SALE_NFT;
	uint256 public MAX_BY_MINT_IN_TRANSACTION_RESERVE = 2;
	uint256 public MAX_BY_MINT_IN_TRANSACTION_SALE = 10;
	
	uint256 public SALE_PRICE = 25 * 10**15;
	uint256 public SALE_MINTED;
	
    bool public reserveEnable = false;
    ...
    constructor() ERC721P('Baby Bricks', 'BRICKS') {}
	
    function _baseURI() internal view virtual returns (string memory) {
        return baseURI;
    }
	
	function mintReserveNFT(uint256 _count, bytes32[] calldata merkleProof) public payable{
		bytes32 node = keccak256(abi.encodePacked(msg.sender));
		uint256 totalSupply = totalSupply();
		require(
			reserveEnable, 
			"Reserve-Sale is not enable"
		);
        require(
		...
		require(
			users[msg.sender].reservemint + _count <= MAX_MINT_RESERVE,
			"Exceeds max mint limit per wallet"
		);
		require(
			_count <= MAX_BY_MINT_IN_TRANSACTION_RESERVE,
		...
		users[msg.sender].reservemint = users[msg.sender].reservemint + _count;
    }
	...
	function updateReserveMintLimit(uint256 newLimit) external onlyOwner {
	    require(SALE_NFT >= newLimit, "Incorrect value");
        MAX_MINT_RESERVE = newLimit;
    }
	...
	function updateMintLimitPerTransectionReserve(uint256 newLimit) external onlyOwner {
	    require(SALE_NFT >= newLimit, "Incorrect value");
        MAX_BY_MINT_IN_TRANSACTION_RESERVE = newLimit;
    }
