...
//DVDA contract 0xC084a29DD0C9436568435938B1C6c5af4F5C035f

interface IERC721 is IERC165 {
    function balanceOf(address owner) external view returns (uint256); //number of dvdas he hold
    function tokensOfOwner(address owner) external view returns (uint256); //# dvda token ids
...
contract RottenBananos is ERC1155, Ownable {
    ...
    uint256 public constant BANANA_PRICE_WHITELIST = 0 ether;
    ...
    mapping (uint256 => uint256) private AlreadyClaimedDVDA; //DVDA TOKEN CLAIMED
    ...
    function MintWhitelist(uint256 amount, uint256 DVDA_ID) public payable {
        require(!WhitelistPaused, "Whitelist sale is paused"); //whitelist paused or not
        require(amount + _minted <= MAX_SUPPLY, "Bananos: Exceed max supply"); //max supply
        address _owner = DVDA.ownerOf(DVDA_ID); //owner of DVDA
        require(_owner == msg.sender, "Must be Owner of DVDA collection to mint"); //requires to own at least 1 dvda
        require(msg.value == amount * BANANA_PRICE_WHITELIST, "Invalid funds provided");
        require(AlreadyClaimedDVDA[DVDA_ID] < 1 , "Bananos already claimed per DVDA.");
        require(AlreadyClaimedDVDA[DVDA_ID] + amount <= 1, "Cannot claim too many Bananos per DVDA Token.");
        _minted += amount;
        AlreadyClaimedDVDA[DVDA_ID] += amount; 
    ...
    function MintPublic(uint256 amount) public payable {
        ...
        require(addressPublicMintedCount + amount <= MAX_MINTS_PUBLIC, "Max NFT per address exceeded");
    ...
    function CheckHowManyClaimed(uint256 NftID) public view returns (uint256){
        uint256 claimed = AlreadyClaimedDVDA[NftID];
        return claimed;
    }
    ...
    function DVDA_TOKENS_ID(address account) public view returns (uint256){
            uint256 tokendvdas = DVDA.tokensOfOwner(account);
            return tokendvdas;
        }
