...
contract FancyTraitV2Sale is AccessControlEnumerable, ReentrancyGuard {
    ...
    function purchaseTraitsWithHoney(
        ...
        if (purchaseData.fancyBear.length > 0) {
            ...
            require(
                IFancy721(_levelAttributionArgs.collection).ownerOf(
                    _levelAttributionArgs.tokenId
                ) == msg.sender,
                "purchaseTraitsWithHoney: caller down not own token ID on collection for level attribution"
            );
    ...
    function purchaseTraitsWithETH(
        ...
        if (_levelAttributionArgs.collection != address(0)) {
            require(
                ...
                "purchaseTraitsWithETH: caller down not own token ID on collection for level attribution"
            );
    ...
    function purchaseTraitsWithERC20(
        ...
        if (_levelAttributionArgs.collection != address(0)) {
            require(
                ...
                "purchaseTraitsWithERC20: caller does not own token ID on collection for level attribution"
            );
