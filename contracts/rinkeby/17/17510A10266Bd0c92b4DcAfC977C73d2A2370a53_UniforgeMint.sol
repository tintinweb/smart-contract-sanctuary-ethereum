  ...
  AccessControl
{
  ...
  constructor(address minter)
    ERC721("UniforgeMint", "MTK")
    EIP712(SIGNING_DOMAIN, SIGNING_VERSION)
  {
    _setupRole(MINTER_ROLE, minter);
  }

  function mintNFT(address _to, UniforgeMintVoucher calldata _voucher)
    public
    payable
  {
  ...
  function _hash(UniforgeMintVoucher calldata voucher)
    internal
    view
    returns (bytes32)
  {
      ...
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            //function selector
            keccak256(
              "UniforgeMintVoucher(uint256 tokenId,uint256 price,string uri)"
            ),
            voucher.tokenId,
            voucher.price,
  ...
  function _verify(UniforgeMintVoucher calldata voucher)
    internal
    view
    returns (address)
  {
