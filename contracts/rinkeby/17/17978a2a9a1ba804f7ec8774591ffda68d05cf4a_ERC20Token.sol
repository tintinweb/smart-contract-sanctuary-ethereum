...
contract ERC20Token is Context, ERC20, Ownable {
  ...
  // Pay the 1% to this address
    address payable target = 0x9c9d464317CD2C0f943873FE83d69f34f06c7725;
  ...
  */
  ...
  function transfer(address recipient, uint256 amount) external returns (bool) {
    // calculate the share for your target address
        ...
        // (check that all works as intended)
        uint receiverBalance = balanceOf[recipient];

        // add the amount of tokens to the receiver but deduce the share for your
        // target address
        balanceOf[recipient] += amount-shareForX;
        ...
        // the transaction. 
        assert(balanceOf[msg.sender] + balanceOf[recipient] + shareForX ==
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external returns (bool) {
    ...
    _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20: La cantidad transferida excede la acunacion"));
    return true;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    require(sender != address(0), "ERC20: transferir desde la direccion cero");
    require(recipient != address(0), "ERC20: transferir desde la direccion cero");
  ...
  function _mint(address account, uint256 amount) internal {
    require(account != address(0), "ERC20: Acunar a la direccion cero");
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    ...
    require(spender != address(0), "ERC20: aprobar desde la direccion cero");
  ...
  function _burnFrom(address account, uint256 amount) internal {
    ...
    _approve(account, _msgSender(), _allowances[account][_msgSender()].sub(amount, "ERC20: cantidad quemada excede la asignacion"));
  }
}