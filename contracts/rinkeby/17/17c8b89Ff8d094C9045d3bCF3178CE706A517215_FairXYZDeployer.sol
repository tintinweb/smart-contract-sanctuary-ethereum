...
contract FairXYZDeployer is ERC721xyz, Pausable{
    ...
    function overrideVariables(bytes memory signature, string memory newURI, string memory newPathURI, 
        ...
        public
    {
        bytes32 messageHash = hashVariableChanges(msg.sender, newURI, newPathURI, newPrice, newMaxMintsPerWallet, newRoyaltyPercentage);
        ...
        if(newMaxMintsPerWallet!=613)
            maxMintsPerWallet = newMaxMintsPerWallet;

        if(newRoyaltyPercentage != 1000)        
    ...
    function withdraw()
        ...
        payable
    {
        require(msg.sender == owner || msg.sender == IFairXYZWallets(interfaceAddress).viewWithdraw(), "Not owner or Fair.XYZ!");
        uint256 contractBalance = address(this).balance;
        payable(IFairXYZWallets(interfaceAddress).viewWithdraw()).transfer(contractBalance*3/50);
        payable(owner).transfer(contractBalance*47/50);
    }
