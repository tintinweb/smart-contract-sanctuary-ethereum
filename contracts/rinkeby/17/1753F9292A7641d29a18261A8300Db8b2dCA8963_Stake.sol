...
contract Stake is Initializable, ContextUpgradeable, UUPSUpgradeable, Whitelist{
    ...
    function updateAccount(address account) private {
        ...
        if (pendingDivs > 0) {
            ...
            require(
                Token(depositToken).transfer(account, amountAfterFee.div(userDetails[account].cmnStakePrice).mul(100)),
                "Could not transfer tokens."
            );
            require(
                Token(depositToken).transfer(owner(), fee.div(userDetails[account].cmnStakePrice).mul(100)),
                "Could not transfer tokens."
            );
    ...
    function getPendingDivs(address _holder) public view returns (uint256) {
        ...
        uint256 pendingDivs = stakedAmount
            .mul(userDetails[_holder].cmnStakePrice)
    ...
    function withdraw() public onlyWhitelisted {
        ...
        userDetails[msg.sender].depositedTokens = userDetails[msg.sender].depositedTokens.sub(
            tokentoSend.div(cmnRate).mul(100)
        );
