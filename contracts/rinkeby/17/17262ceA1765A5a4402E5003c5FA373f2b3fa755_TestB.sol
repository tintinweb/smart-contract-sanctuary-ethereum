    ...
    ILayerZeroUserApplicationConfig
{
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "BAD role");
        _;
    }
    ...
    function remoteCallA(
        ...
        lzEndpoint.send{value: msg.value}(
            ...
            abi.encodePacked(VERSION, FEE)
        );
    }

    event MessageFailed(bytes payload);
    function lzReceive(
        uint16 _srcChainId,
        ...
        try this._lzReceive(_srcChainId, _srcAddress, _nonce, _payload) {

        } catch {
            emit MessageFailed(_payload);
        }
    }

    function _lzReceive(uint16, bytes memory, uint64, bytes memory _payload) public {
        require(msg.sender == address(this), "CALLER_MUST_BE_BRIDGE");
        (bool isTest, uint addVal) = abi.decode(_payload, (bool, uint));
        if (isTest) {
            _val += addVal;
        } else {
            revert("bad case");
    ...
    function setConfig(
        ...
        lzEndpoint.setConfig(lzEndpoint.getSendVersion(address(this)), _chainId, _configType, _config);
    }
