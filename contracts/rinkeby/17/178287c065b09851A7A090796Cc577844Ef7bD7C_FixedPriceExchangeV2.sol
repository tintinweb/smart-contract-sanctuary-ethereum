...
import "./IERC1155Verifiable.sol";
import "./NativeMetaTransaction.sol";
import "./IRoyaltiesManager.sol";
import "./OwnableSignable.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
contract FixedPriceExchangeV2 is Ownable, Pausable, NativeMetaTransaction {
  ...
  bytes4 public constant ERC1155_Interface = bytes4(0x80ac58cd);
  ...
  constructor (
    ...
    // EIP2612 init
    _initializeEIP2612('Katana Marketplace', '1');
    //__Ownable_init();
    //require(owner()==address(0xf7C83e092814C907f12F7730944ACD4263F62289), "Not the owner");
  ...
  */
    ...
    internal
  {
    ...
    IERC1155Verifiable nftRegistry = IERC1155Verifiable(nftAddress);
    require (nftRegistry.balanceOf(sender, assetId)>0, "Sender does not own the asset");
  ...
  function _executeOrder(
   ...
   internal returns (Order memory)
   {
    ...
    require(nftRegistry.balanceOf(order.seller, assetId)>0, "MarketplaceV2#_executeOrder: SELLER_NOT_OWNER");
    require(nftRegistry.balanceOf(order.seller, assetId)>=amount,"The amount to be transferred is not available");

    delete orderByAssetId[nftAddress][assetId];

    uint256 feesCollectorShareAmount;
    //TODO: A very basic solution to get rid of the stack full problem that might need a better approach
    royalities memory r;
    // Royalties share
    if (royaltiesCutPerMillion > 0) {
      r.royaltiesShareAmount = (price * royaltiesCutPerMillion) / 1000000;
      ...
      if (success) {
        (r.royaltiesReceiver) = abi.decode(res, (address));
        if (r.royaltiesReceiver != address(0)) {
          require(
            acceptedToken.transferFrom(sender, r.royaltiesReceiver, r.royaltiesShareAmount),
    ...
    // Fees collector share
    {
      ...
      if (r.royaltiesShareAmount > 0 && r.royaltiesReceiver == address(0)) {
        totalFeeCollectorShareAmount += r.royaltiesShareAmount;
      }
    ...
    require(
      acceptedToken.transferFrom(sender, order.seller, price - r.royaltiesShareAmount - feesCollectorShareAmount),
  ...
  function _requireERC1155(address nftAddress) internal view {
    require(nftAddress.isContract(), "MarketplaceV2#_requireERC1155: INVALID_NFT_ADDRESS");

    IERC1155 nftRegistry = IERC1155(nftAddress);
    require(
      nftRegistry.supportsInterface(ERC1155_Interface),
      "MarketplaceV2#_requireERC1155: INVALID_ERC1155_IMPLEMENTATION"
    );
  }
}
...
abstract contract NativeMetaTransaction is EIP2612Base, ERC20 {
...
interface IERC1155Verifiable is IERC1155 {
