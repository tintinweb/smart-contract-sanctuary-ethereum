...
contract DanLand is ERC721, ERC721Enumerable, EIP712, IERC2981, Ownable, AccessControlEnumerable, Pausable, ERC721Burnable {
    ...
    function validateSigner(NFTVoucher calldata voucher) external view {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Unauthorized operation.");
        address signer = _verify(voucher);
        string memory strSigner = string(abi.encodePacked("Signature invalid or unauthorized for signer: ", Strings.toHexString(uint160(signer), 20)));
    ...
    function transferByOwner(address from, address to, uint256[] memory tokenIds) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Unauthorized operation.");
        for (uint tokenIdCpt = 0; tokenIdCpt < tokenIds.length; tokenIdCpt++) {
            _transfer(from, to, tokenIds[tokenIdCpt]);
        }
    }
