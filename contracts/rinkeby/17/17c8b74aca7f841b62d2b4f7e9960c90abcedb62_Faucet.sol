...
import {IHero} from "../interfaces/IHero.sol";
import {IFighter} from "../interfaces/IFighter.sol";
import {IERC20Burnable} from "../interfaces/IERC20Burnable.sol";

import {AccessControlEnumerable} from "@openzeppelin/contracts/access/AccessControlEnumerable.sol";

contract Faucet is AccessControlEnumerable {
    IHero public immutable hero;
    IFighter public immutable fighter;
    IERC20Burnable public immutable confetti;

    mapping(address => uint256) private timeUsed;

    constructor(
        IHero _hero,
        IFighter _fighter,
        IERC20Burnable _confetti
    ) {
    ...
    function mint() external {
        ...
        require(block.chainid != 1, "Intended to run on testnets only");
        require(
            timeUsed[user] + 24 hours < block.timestamp,
            "Wait 24 hours to use faucet again"
        );

        timeUsed[user] = block.timestamp;

        hero.mint(user, 1);
        fighter.mint(user, 6);
        confetti.mint(user, 10_000 ether);
    }
}
