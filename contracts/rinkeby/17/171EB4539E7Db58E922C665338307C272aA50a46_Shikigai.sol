...
contract Shikigai is ERC721A, Ownable {
    ...
    uint8 public constant SECTION_BITMASK = 15;
    uint16 public constant DISTRICT_BITMASK = 4095;
    uint40 public constant COUNTERS_BITMASK = 68719476735;

    uint48 public ichiroSupply = 0;
    uint48 public jiroSupply = 0;
    uint48 public saburoSupply = 0;
    uint16 public maxDistrictSupply = 2730;
    uint8 public maxAllowancePublic = 3;
    uint8 public currSalesPhase = 4;
    ...
    function publicMint(bytes32 _r, bytes32 _s, uint8 _v, uint48 _inputConfig) 
        ...
        CallerIsUser
    {
        if (currSalesPhase > 0) revert WrongSalesPhase();

        uint48 inputCounters = _inputConfig & COUNTERS_BITMASK; 
        uint48 ichiroAmount = inputCounters & DISTRICT_BITMASK;
        uint48 jiroAmount = (inputCounters >> 12) & DISTRICT_BITMASK;
        uint48 saburoAmount = (inputCounters >> 24) & DISTRICT_BITMASK;
        ...
        CallerIsUser
    {
        ...
        if (!MerkleProof.verify(_proof, _merkleRoot, leaf)) revert InvalidMerkleProof();
