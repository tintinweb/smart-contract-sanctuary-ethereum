...
interface ITopMinterSetup {
    function setup_constructor(string calldata name,string calldata symbol,
        ...
        address _VRFCoordinator,string calldata _uri_prefix
    )  external ;
}
...
contract TopMinter is Ownable,ERC721Full,ReentrancyGuard,ITopMinterSetup,RadicalNFT {
    ...
    function setup_constructor(string calldata name,string calldata symbol,
        ...
        s_subscriptionId=uint64((_value1>>64)&0xffffffffffffffff);
...
;
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        require(_exists(tokenId));
        return append(uri_prefix,_name,"/",tokenId,".json");
    }
    function append(string memory a, string memory b,string memory b1, uint c,string memory end) internal pure returns (string memory) {

        return string(abi.encodePacked(a, b,b1, uintToString(c),end));
    ...
    function getRandomLength() external view returns (uint256) {
        return s_randomWords.length;
    }
    function requestRandom() external  {

        IVRFCoordinatorV2Interface COORDINATOR = IVRFCoordinatorV2Interface(vrfCoordinator);
