...
contract TestContract is  Ownable, ERC721A {

    constructor() ERC721A("TestContract123456", "TT") {
        deployer = msg.sender;
    }
    ...
    mapping(address => uint256) private _publicMintAmount;
    ...
    function changeMintPrice(uint256 newPrice) public onlyOwner returns(uint256) {
        mintPrice = newPrice;
        return mintPrice;
    }

    function changeDeployer(address newDeployer) public onlyOwner returns(address) {
        deployer = newDeployer;
        return deployer;
    }

    function changeMintLimit(uint256 newMintLimit) public onlyOwner returns(uint256) {
        mintLimit = newMintLimit;
        return mintLimit;
    }

    function checkWhitelistedAmount(address wallet) public view returns (uint256) {
        return _presaleMintAmount[wallet];
    }
    ...
    function mintPresale(uint numberOfTokens, uint maxAllocation) external payable {
        require(preSaleState, "Presale is not active");
        require(numberOfTokens <= mintLimit, "Too many tokens for one transaction");
        require(numberOfTokens + checkWhitelistedAmount(msg.sender) < maxAllocation, "You don't have enough whitelisted tokens");
        require(msg.value >= mintPrice * numberOfTokens, "Insufficient payment");


        mintInternal(msg.sender, numberOfTokens);
        _presaleMintAmount[msg.sender] += numberOfTokens;
    }
    ...
    function mint(uint numberOfTokens) external payable {
        ...
        mintInternal(msg.sender, numberOfTokens);
        _publicMintAmount[msg.sender] += numberOfTokens;
    }
