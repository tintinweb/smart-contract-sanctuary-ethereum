 ...
 * @notice It automatically sells WETH periodically to LOOKS through Uniswap V3.
...
contract AggregatorFeeSharingWithUniswapV3 is Ownable, Pausable, ReentrancyGuard {
    ...
    // Whether harvest and WETH selling is triggered automatically
    ...
    /**
     * @notice Constructor
     * @param _looksRareToken address of the LooksRare token (LOOKS)
     * @param _rewardToken address of the reward token (WETH)
    ...
    constructor(
        ...
        IERC20(_looksRareToken).approve(_feeSharingSystem, type(uint256).max);
    ...
    function deposit(uint256 amount) external nonReentrant whenNotPaused {
        ...
        if (block.number != lastHarvestBlock && canHarvest && totalShares != 0) {
            _harvest();
        }
        ...
        // Initialize the number of shares to issue
     ...
     * @dev Only callable by owner.
     */
    function harvestAndSellToLOOKS() external nonReentrant onlyOwner {
    ...
    /**
     * @notice Harvest and (sell if possible)
     */
    ...
    function _sellRewardTokenToLOOKS(uint256 _amount) internal returns (bool) {
        // Increase allowance for this transaction only
        rewardToken.safeIncreaseAllowance(address(uniswapRouter), _amount);
    ...
    function _withdraw(uint256 _shares) internal {
        if (block.number != lastHarvestBlock && canHarvest) {
            _harvest();
        }
        ...
        // Transfer the amount back to user
