...
abstract contract OrderValidator is Initializable, ContextUpgradeable, EIP712Upgradeable {
    ...
    function __OrderValidator_init_unchained() internal initializer {
        __EIP712_init_unchained("Ango Exchange", "1");
    }
...
abstract contract ExchangeV2Core is Initializable, OwnableUpgradeable, AssetMatcher, TransferExecutor, OrderValidator, ITransferManager {
    ...
    function matchOrders(
        ...
        if (price > 0){
            address token;
            uint tokenId;
            if (makeMatch.assetClass == LibAsset.ERC721_ASSET_CLASS){
                (token, tokenId) = abi.decode(makeMatch.data, (address, uint256));
                lastPrice[token][tokenId] = price;

            }
            if (takeMatch.assetClass == LibAsset.ERC721_ASSET_CLASS){
                (token, tokenId) = abi.decode(takeMatch.data, (address, uint256));
                lastPrice[token][tokenId] = price;
            }
        }

         else{
             if (makeMatch.assetClass == LibAsset.ERC721_ASSET_CLASS && takeMatch.assetClass == LibAsset.ERC20_ASSET_CLASS)  {
                (address WETH1) = abi.decode(takeMatch.data, (address));

                if (WETH1 == WETH){
                    (address token, uint tokenId) = abi.decode(makeMatch.data, (address, uint256));
                    lastPrice[token][tokenId] = orderLeft.takeAsset.value;
                }
             }

             if (takeMatch.assetClass == LibAsset.ERC721_ASSET_CLASS && makeMatch.assetClass == LibAsset.ERC20_ASSET_CLASS){
                 (address WETH1) = abi.decode(makeMatch.data, (address));
                 
                  if (WETH1 == WETH){
                    (address token, uint tokenId) = abi.decode(takeMatch.data, (address, uint256));
                    lastPrice[token][tokenId] = orderLeft.makeAsset.value;
                  }
             }
        } 
    }
