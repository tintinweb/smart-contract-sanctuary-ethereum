...
contract BridgeDistributorV2 is AbstractDistributor,
    IDistributor,
    Initializable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    function __BridgeDistributorV2_init(
                address _contractsRegistry
        ) public initializer {
        __Ownable_init();
        __ReentrancyGuard_init();
        contractsRegistry = IContractsRegistryV2(_contractsRegistry);
        policyRegistry = IPolicyRegistryV2(contractsRegistry.getPolicyRegistryContract());
        policyBookRegistry = IPolicyBookRegistryV2(contractsRegistry.getPolicyBookRegistryContract());
        claimRegistry = IClaimingRegistryV2(contractsRegistry.getClaimingRegistryContract());
        policyQuote = IPolicyQuoteV2(contractsRegistry.getPolicyQuoteContract());
    }
    ...
    function getCoverCount(address _userAddr, bool _isActive)
        ...
        PolicyInfo memory pol;
           (pol.policiesCount,
            pol.polBooksArr,
            pol.policiesInfo,
            pol.status) = policyRegistry.getPoliciesInfo(_userAddr, _isActive, 0, _ownersCoverCount);
        return pol.policiesCount;
    }

    function getCover(address _userAddr, uint256 _coverId, bool _isActive, uint256 _limitLoop) external view override
         returns(IDistributor.Cover memory) {
           ...
           (pol.policiesCount,
            ...
            require(_coverId < pol.policiesCount,"BridgeDistributor: Invalid coverId");
            ...
            for(uint coverId = 0 ;coverId < limit; coverId++){
    ...
    function getQuote(
        ...
        bool isPolicyBook = policyBookRegistry.isPolicyBook(_bridgeProductAddress);
        require(isPolicyBook == true, "BridgeDistributor: Not a policyBook");

        IPolicyBookV2 policyBook = IPolicyBookV2
        (_bridgeProductAddress);

        IDistributor.CoverQuote memory coverQuote;
       (coverQuote.prop1, // totalSeconds, totalPrice
        coverQuote.prop2,
        coverQuote.prop3)= policyBook.getPolicyPrice(_bridgeEpochs, _amountInWei, _buyerAddress);
    ...
    function buyCover (
        ...
        require(IERC20Upgradeable(stblToken).allowance(msg.sender, address(this)) >= _premium, "BridgeDistributor: Need funds approval");

        // transfer erc20 funds to this contract
        IERC20Upgradeable(stblToken).safeTransferFrom(msg.sender, address(this), _premium);
       if (IERC20Upgradeable(stblToken).allowance(address(this), address(policyBook)) == uint256(0)) {
        ...
        policyBookFacade.buyPolicyFromDistributorFor(_buyerAddress,_epochsNumber,_sumAssured, _treasuryAddress);
    }

    function listWithStats(uint8 _offset, uint256 _limitLoop) external view
        returns (PolicyCatalog[] memory) {

        uint256 count = policyBookRegistry.count();

        uint256 limit = 0;
                if(count > _limitLoop){ limit = _limitLoop;
                } else { limit = count; }

        (address[] memory _policyBooks,
         IPolicyBookRegistryV2.PolicyBookStats[] memory _stats) = policyBookRegistry.listWithStats(_offset, count);
    ...
    function list(uint256 _offset)
        ...
        returns (address[] memory _policyBooks){
            uint limit = policyBookRegistry.count();
...
interface IPolicyBookRegistryV2 {
    ...
    function countByTypeWhitelisted(IPolicyBookFabricV2.ContractType contractType)
        external
        view
        returns (uint256);
...
abstract contract AbstractDistributor {
   ...
   function _swapTokenForToken(
        ...
        uint256 _expectedOut = IPriceFeed(_priceFeed).howManyTokensAinB(_to, _from, _via, _amountIn, false);
    ...
    function _swapExactETHForTokens(address _priceFeed,address _token,uint256 _amountIn) internal  returns (uint256) {
        IUniswapV2Router02 _uniswapRouter = IUniswapV2Router02(IPriceFeed(_priceFeed).getUniswapRouter());
       ...
       address _tokenFeed = IPriceFeed(_priceFeed).chainlinkAggregators(_token);
       if( _tokenFeed != address(0) ){
             (, int256 _price, , , ) = AggregatorV3Interface(_tokenFeed).latestRoundData();
        ...
        uint256 _amountOutMin = _expectedOut.mul(99).div(100);
        return _uniswapRouter.swapETHForExactTokens{value: _amountIn}(
...
interface IPriceFeed {
    ...
    function chainlinkAggregators(address _token) external view returns(address);
