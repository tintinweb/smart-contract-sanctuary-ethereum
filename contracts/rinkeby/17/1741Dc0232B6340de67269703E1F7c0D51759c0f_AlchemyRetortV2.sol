...
import "./interface/IAlchemyRetort.sol";
import "./interface/IVerifyAttestation.sol";
import "./interface/IAlchemyRemix.sol";
import "./interface/IProxyMaster.sol";
 ...
 * @dev Storage slot with the address of the current implementation.
    ...
    function _forceSetImplementation(address impl) external {
...
contract AlchemyDataV2 {
    ...
    mapping (uint256 => WrappedERC721Token[]) _transformedTokens;
    ...
    //address constant _attestorAddress = 0x538080305560986811c3c1A2c5BCb4F37670EF7e; //Attestor key: NB Testnet contracts were deployed before we moved this to a variable value

}

contract ERC721AlchemyV2 is AlchemyDataV2, ERC721Upgradeable {
...
//        if (!_existsAndNotPaid(commitmentId)){
//            revert TokenNotExists(commitmentId);
//        }
...
contract AlchemyRetortV2 is ERC721AlchemyV2, UUPSUpgradeable, OwnableUpgradeable, IAlchemyRetortV2 {

    using AddressUpgradeable for address;

    bytes constant emptyBytes = new bytes(0x00);

    error OnlyRemixV2Allowed();
    error ContractRequired();
    error TokenNotExists(uint256 tokenId);
    error RequiresNFTtoCommit();



    //address __attestorAddress; //Attestor key

    // Original ERC721 Contract => Remixed ERC721 Contract
    mapping (address => address) private _originalToRemixContracts;
    mapping (address => address) private _remixToOriginalContracts;

    address private _remixLogicAddress;
    // When try to refresh Implementation Contract Address
    uint256 private _RemixLogicTTL;

    event TransferRemix(address indexed remix, uint256 indexed tokenId, address indexed to);
    error WrongCommision();

    modifier onlyRemixV2Sender() {
        if (!isRemixContract(msg.sender)) {
            revert OnlyRemixV2Allowed();
        }
        _;
    }

    function remixTransfer(uint256 tokenId, address to) external onlyRemixV2Sender{
        emit TransferRemix(msg.sender, tokenId, to);
    }

    function contractRequired(address addr) private view {
        if (!addr.isContract()) {
            revert ContractRequired();
        }
    }

    function getImplementationAndExpiry() external view returns (address logic, uint expiry){
        logic = _remixLogicAddress;
        expiry = block.timestamp + _RemixLogicTTL;
    }

    function _forceSetImplementation(address payable target) external onlyOwner {
        RemixProxy p = RemixProxy(target);
        p._forceSetImplementation(getRemixLogicAddress());
    }

    function setRemixV1Address(address newAddr) public onlyOwner {
        // TODO verify contract interface
        contractRequired(newAddr);
        _remixContract = newAddr;
    }

    function getRemixV1Address() internal view returns (address){
        return _remixContract;
    }
    ...
    function initialize(address verificationContract, address dvpContract) public initializer
    {
        contractRequired(verificationContract);
        contractRequired(dvpContract);
        __Ownable_init();
    ...
    function getRemixLogicAddress() internal view returns(address) {
        return _remixLogicAddress;
    }

    function setRemixLogicAddress(address newAddr, uint ttl) external onlyOwner {
        contractRequired(newAddr);
        _remixLogicAddress = newAddr;
        _RemixLogicTTL = ttl;
    }
    ...
    function getAttestor() public view returns(address)
    {
        return _attestorAddress;
    }
...
//    event RemixV2(string indexed identifier, uint256 indexed commitmentId, address erc721Addr, uint256 tokenId);
    event Remix(string indexed identifier, uint256 indexed commitmentId, uint256 newTokenId, address erc721Addr, uint256 tokenId);

    event AddedIdentifier(uint256 indexed commitmentId, address indexed erc721contract, uint256 indexed tokenId, string identifier);

    event RemovedIdentifier(address indexed erc721, uint256 indexed tokenId, string identifier, uint256 indexed commitmentId);
    event DeployedRemix(address indexed erc721, address indexed remix);
...
// upgraded for dekembee
    function commitNFTAndreturnId(ERC721Token[] memory nfts, string memory identifier) external payable returns(uint) {
        _commitNFT(nfts, identifier);
        return _commitmentId - 1;
    }

    function _commitNFT(ERC721Token[] memory nfts, string memory identifier) internal returns(uint)
    {
//        require (nfts.length > 0, "Requires NFT to commit");

        uint256 len = nfts.length;
        if (len == 0) {
            revert RequiresNFTtoCommit();
        }
        ...
        // _commitments[id].adrPayee  = payable(0); // null no need to assign ZERO value
        ...
        for (uint256 index = 0; index < len; index++)
        {
            ...
            _commitments[id].nfts.push(thisToken);
            
            if (!isRemixContract(thisToken.erc721) || thisToken.erc721 == getRemixV1Address()) {
    ...
    function isRemixContract(address erc721) internal view returns (bool){
        return _remixToOriginalContracts[erc721] != address(0);
    }

    function hasRemixedContract(address erc721) internal view returns (bool){
        return _originalToRemixContracts[erc721] != address(0);
    }

    function deployTransparentProxyForERC721(address erc721) internal {
        // its first time when we remix NFT for this contract
        // create derived Remix and set "remix" variable
        // deploy new Remix proxy contract
        RemixProxy transparentRemix = new RemixProxy(getRemixLogicAddress());
        address remix = address(transparentRemix);

        // TODO we can remove originalContract from initializer, maybe worth to do it, its can save 20k of gas
        IAlchemyRemixV2 remixContract = IAlchemyRemixV2(remix);
        remixContract.initialize(erc721);

        _originalToRemixContracts[erc721] = remix;
        _remixToOriginalContracts[remix] = erc721;

        emit DeployedRemix(erc721, remix);

    }

    function checkAttestationValidity(bytes memory nftAttestation, ERC721Token[] memory commitmentNFTs,
        string memory commitmentIdentifier, address attestorAddress, address sender) internal view returns (address payable){
        ...
        (passedVerification, subjectAddress) = verifier.checkAttestationValidity(nftAttestation, commitmentNFTs, commitmentIdentifier, attestorAddress, sender);
    ...
    // TODO maybe limit it to DvP only?
    function deliver(uint256 commitmentId, bytes memory nftAttestation) external override returns (uint256 committedAmount, address payable subjectAddress) {
        ...
        // uint256 offerNFTLength = offer.nfts.length;
        ...
        subjectAddress = checkAttestationValidity(nftAttestation, offer.nfts, offer.identifier, getAttestor(), msg.sender);
...
//        IAlchemyRemix remixTokenContract;
        ...
        {
            // address erc721 = offer.nfts[index].erc721;
            // uint tokenId = offer.nfts[index].tokenId;
            erc721 = offer.nfts[index].erc721;
            tokenId = offer.nfts[index].tokenId;

            if (erc721 == getRemixV1Address()) {
                ...
                emit Remix(offer.identifier, commitmentId, newId, erc721, tokenId);
                tokenId = newId;

            } else {
                IAlchemyRemixV2 remixContract;

                if (!isRemixContract(erc721)) {
                    // its not Remix contract,
                    // lets check if we have remix for that contract already
                    if (!hasRemixedContract(erc721)) {
                        deployTransparentProxyForERC721(erc721);
                    }

                    // remix for that erc721 already deployed
                    erc721 = _originalToRemixContracts[erc721];
                    remixContract = IAlchemyRemixV2( erc721 );

                    // create Remix contract instance to mint token
                    // mint the new token
                    remixContract.mint(owner, tokenId);

                    emit Remix(offer.identifier, commitmentId, tokenId, erc721, tokenId);

                } else {
                    // remixContractAddress = erc721;
                    // its Remix contract, so no need to mint token, just add "identifier"
                    remixContract = IAlchemyRemixV2(erc721);
                }

                remixContract.addIdentifier(commitmentId, tokenId, offer.identifier);
                // save to the erc721 remix address for current NFT
                emit AddedIdentifier(commitmentId, erc721, tokenId, offer.identifier);

            }

            // for compatability with Remix_v1
            // erc721 is remix address for current NFT

            WrappedERC721Token memory wrappedERC721Token;
            wrappedERC721Token.erc721 = erc721;
            wrappedERC721Token.tokenId = tokenId;

            _transformedTokens[commitmentId].push(wrappedERC721Token);
...
//    function getUnderlayingTokenData(RemixCommit memory commitmentEntry) external view returns (ERC721Token memory){
//        require(_remixContract == msg.sender, "Only Remix contract allowed");
//        return _commitments[commitmentEntry.commitmentId].nfts[commitmentEntry.commitmentIndex];
//    }

    function emitRemovedIdentifier(uint256 tokenId, string memory identifier, uint256 commitmentId) external override onlyRemixV2Sender{
        // lookup for original contract
        // onlyRemixV2Sender();
        emit RemovedIdentifier(msg.sender, tokenId, identifier, commitmentId);

    }

    function getRemixContract(address original) public view returns (address){
        return _originalToRemixContracts[original];
    }

    function getOriginalContract(address remix) public view returns (address){
        return _remixToOriginalContracts[remix];
    }
...
//        require(paymentSuccessful, "Payment failed");
    ...
    function unwrapToken(RemixCommit memory commitmentEntry, address tokenOwner) external override
    {
        address remixV2Original = getOriginalContract(msg.sender);
        require (msg.sender == _remixContract || remixV2Original != address(0), "Only Remix can call this");
        address erc721;
        uint tokenId;

        if (msg.sender == _remixContract) {
            //is this a valid wrapped token(s)? Do the input nfts correspond to the commitment?
            require (_commitments[commitmentEntry.commitmentId].adrPayee != address(0), "Tokens must be transformed");
            //lookup exact token
            ERC721Token memory originalToken = _commitments[commitmentEntry.commitmentId].nfts[commitmentEntry.commitmentIndex];
            erc721 = originalToken.erc721;
            tokenId = originalToken.tokenId;
        } else {
            require(commitmentEntry.commitmentId == 0);
            erc721 = remixV2Original;
            // Remix2.0 send tokenID as RemixCommit(0,tokenId)
            tokenId = commitmentEntry.commitmentIndex;
        }

        IERC721 tokenContract = IERC721(erc721);
        tokenContract.safeTransferFrom(address(this), tokenOwner, tokenId);
    }
    ...
    returns (WrappedERC721Token[] memory nfts)
    {
    ...
    function burn(uint256 commitmentId) public payable
    {
        ...
        for (uint256 index = 0; index < commitment.nfts.length; index++)
        {
            ...
            IERC721 nftContract = IERC721(thisToken.erc721);
            if (nftContract.ownerOf(thisToken.tokenId) == address(this)) {
...
import "./IAlchemyRemix.sol";
import "./IVerifyAttestation.sol";

interface IAlchemyRetortBase {
...
interface IAlchemyRetortV2 is IAlchemyRetortBase {
    function emitRemovedIdentifier(uint tokenId, string memory identifier, uint256 commitmentId) external;
    function remixTransfer(uint256 tokenId, address to) external;
}

interface IAlchemyRetort is IAlchemyRetortBase {
    function getUnderlayingTokenData(RemixCommit memory commitmentEntry) external view returns (ERC721Token memory);
}
...
interface IAlchemyRemixV2 is IERC721Upgradeable {
    function mint(address to, uint tokenId) external;
    function initialize(address originalContract) external;
    function addIdentifier(uint256 commitmentId, uint256 tokenID, string memory identifier) external;
    function removeIdentifier(uint256 tokenID, string memory identifier) external;
    function removeIdentifierByCommitmentId(uint256 tokenId, uint256 commitmentId) external;
}
