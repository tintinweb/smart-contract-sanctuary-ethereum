...
abstract contract BridgeEIP712 is EIP712 {
    ...
    uint256 public constant requestLifetime = 5 minutes;

    bytes32 public constant RECEIPT_SCHEMA_HASH =
        keccak256(
            "Receipt(uint256 amount,address token,address from,address to,string fromName,string toName,uint256 receiptId,uint256 signTimestamp,uint256 tokenId)"
        );
    ...
    function checkRequestLifetime(uint256 _signTimestamp) internal view {
        if (_signTimestamp + requestLifetime < block.timestamp)
