...
contract PreservaLandTickets is AbstractMintVoucherFactory, ReentrancyGuard  {
    ...
    address payable public preservaWallet;
    uint256 public reservePercentage;
    uint256 public preservaPercentage;
    ...
    constructor(
        ...
        address _preservaAddress
    ) ERC1155("https://bafybeibq23v65yqorpvqall72sz6el3yqcz2xoqi4zjm3ihptb2hfyhvmi.ipfs.nftstorage.link/metadata/") {
        require(address(_reserveAddress) != address(0) && address(_preservaAddress) != address(0), "Treasury address cannot be zero");

        name_ = _name;
        symbol_ = _symbol;
        reserveWallet = payable(_reserveAddress);
        preservaWallet = payable(_preservaAddress);
        reservePercentage = 30;
        preservaPercentage = 70;
    }
    ...
    function updatePayoutPercentage(uint256 _reservePercentage, uint256 _preservaPercentage) external onlyOwner
    {
        require(_reservePercentage + _preservaPercentage <= 100, "Total percentage cannot be greater than 100");
        reservePercentage = _reservePercentage;
        preservaPercentage = _preservaPercentage;
    }

    function withdrawFunds() public onlyOwner
    {
        uint256 currentBalance = address(this).balance;
        uint256 amount1 = (currentBalance * reservePercentage)/100;
        uint256 amount2 = currentBalance - amount1;

        (bool success1, ) = address(reserveWallet).call{value:amount1}("");
        require(success1, "Transfer1 failed.");

        (bool success2, ) = address(preservaWallet).call{value:amount2}("");
