...
abstract contract Generic721 is ERC721 {
    ...
    function getMintTime(uint64 _cardID) public view returns (uint128 , uint128) {
        return (CardType[_cardID].mintTime , CardType[_cardID].mintEndTime);
    }

    function canMint(uint64 _cardID) public view returns (bool) {
        return (block.timestamp > CardType[_cardID].mintTime && CardType[_cardID].mintEndTime > block.timestamp);
    }
