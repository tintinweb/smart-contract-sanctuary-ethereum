...
RolesUpgradeable {
    ...
    public
    onlyRole(ADMIN_ROLE) {
        require(artworksIds.length == maxEditions.length && artworksIds.length == metadataHashes.length,
            "artworksIds, maxEditions, metadataHashes must have the same length");
     ...
     */
    function mintEditions(address creator, uint256[] memory artworksIds, uint256[] memory amounts)
    public
    onlyRole(ADMIN_ROLE) {
        for (uint i = 0; i < artworksIds.length; i++) {
     ...
     */
    function mintEditionsAndTransfer(address creator, uint256[] calldata artworksIds, uint256[] calldata amounts, TransferDescriptor[] calldata transferDescriptors)
    external {
        mintEditions(creator, artworksIds, amounts);
        executeTransfers(creator, transferDescriptors);
    }
     ...
     */
    function mintEditionsAndTransferByGallery(address creator, uint256[] calldata artworksIds, uint256[] calldata amounts, GalleryTransferDescriptor[] calldata galleryTransferDescriptors)
    external {
        mintEditions(creator, artworksIds, amounts);
        executeTransfersViaGallery(creator, galleryTransferDescriptors);
    }
