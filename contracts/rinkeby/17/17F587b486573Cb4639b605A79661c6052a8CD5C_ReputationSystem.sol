...
contract ReputationSystem is IReputationSystem, Initializable, AbstractDependant {
    ...
    function _initTeamReputation(address[] memory team) internal {
        for (uint256 i = 0; i < team.length; i = uncheckedInc(i)) {
    ...
    function _setNewReputation(address voter, uint256 newReputation) internal {
        require(
            newReputation >= PRECISION.uncheckedDiv(10),
            "ReputationSystem: reputation too low"
        );
        require(newReputation <= PRECISION.tryMul(3), "ReputationSystem: reputation too high");
        ...
        uint256 flooredOldReputation = voterReputation.tryMul(10).uncheckedDiv(PRECISION);

        _reputation[voter] = newReputation;

        uint256 flooredNewReputation = newReputation.tryMul(10).uncheckedDiv(PRECISION);
        ...
        _roundedReputations[flooredOldReputation] = _roundedReputations[flooredOldReputation]
            .trySub(1);

        _roundedReputations[flooredNewReputation] = _roundedReputations[flooredNewReputation]
            .tryAdd(1);
    }
    ...
    function _recalculateTrustedVoterReputationThreshold() internal {
        uint256 trustedVotersAmount =
            Math.max(
                ...
                _votedOnceCount.mul(PERCENTAGE_OF_TRUSTED_VOTERS).uncheckedDiv(PERCENTAGE_100)
            );
        ...
        unchecked {
            for (uint8 i = REPUTATION_PRECISION - 1; i >= LEAST_TRUSTED_VOTER_REPUTATION; i--) {
                ...
                if (votersAmount >= trustedVotersAmount) {
                    if (
                        votersAmount >= (trustedVotersAmount * 3) / 2 &&
        ...
        returns (uint256)
    {
        ...
        require(
            voterReputation >= PRECISION.uncheckedDiv(10),
            "ReputationSystem: reputation too low"
        );
        require(voterReputation <= PRECISION.tryMul(3), "ReputationSystem: reputation too high");

        if (percentageWithPrecision >= PRECISION.tryMul(50)) {
            return
                Math.min(
                    MAXIMUM_REPUTATION,
                    voterReputation.tryAdd(
                        percentageWithPrecision.uncheckedDiv(100).uncheckedDiv(20)
                    )
                );
        } else {
            uint256 squared = PERCENTAGE_100.sub(percentageWithPrecision.tryMul(2));
            uint256 fraction =
                squared.mul(squared).uncheckedDiv(2).uncheckedDiv(PERCENTAGE_100).uncheckedDiv(
                    100
                );

            return
                fraction < voterReputation
                    ? Math.max(MINIMUM_REPUTATION, voterReputation.uncheckedSub(fraction))
                    : MINIMUM_REPUTATION;
        }
    }
    ...
    function isTrustedVoter(address user) external view override returns (bool) {
        return
            _reputation[user] >= _trustedVoterReputationThreshold.mul(PRECISION).uncheckedDiv(10);
    }
    ...
    function getTrustedVoterReputationThreshold() external view override returns (uint256) {
        return _trustedVoterReputationThreshold.mul(PRECISION).uncheckedDiv(10);
    }
