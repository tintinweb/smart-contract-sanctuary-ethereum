...
contract Ownable {
    ...
    /**
     * @dev The constructor sets the original `contractOwner` of the contract to the sender account.
     */
    constructor() public {
    ...
    Ownable
{
    ...
    mapping(uint256 => string) private _tokenHashs;
        ...
        returns (string memory)
    {
        ...
        string memory _tokenHash = _tokenHashs[tokenId];
        return string(_tokenHash);
    }
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        if (bytes(_tokenHashs[tokenId]).length != 0) {
            delete _tokenHashs[tokenId];
        }
    ...
    function _setTokenHash(uint256 tokenId, string memory hash)
        internal
        virtual
    {
        require(
...
contract ArtFungible is ERC721 {
    event PlaceNewBid(
        address indexed from,
        uint256 indexed tokenId,
    ...
    event BidApproved(
        address indexed BidUser,
        uint256 indexed tokenId,
        uint256 indexed BidAmount
    );
    event UpdateCommissionPercentage(
        uint256 indexed adminCommission,
        uint256 indexed royaltyCommission
    );


    using SafeMath for uint256;  
    uint256 public lastTokenID;
    uint256 public CommiossionFee = 10 * 10**3; // 10% fee;
    uint256 public RoyaltyFee = 5 * 10**3; // 5% fee;
    ...
    function updateCommissionPercentage(
        ...
        CommiossionFee = adminCommission;
        RoyaltyFee = royaltyCommission;
    }
    ...
    function updateSeller(
        ...
        tokenDetails[tokenId].creator = _seller;
    }
    
    function mint(uint256 tokenId,address seller, string memory ipfsHash,uint256 salePrice,SellType sellType, uint256 auctionStart, uint256 auctionExpire ) external onlyOwner returns(uint256){
        require(lastTokenID != tokenId, "token id already minted" );
        require(
        ...
        tokenDetails[tokenId] = _tokenDetails(seller,address(this),auctionStart,auctionExpire,salePrice,sellType,true,ipfsHash);
        return tokenId;
    }

    function burn(uint256 _tokenId) public returns (bool) {
        _burn(_tokenId);
        delete tokenDetails[_tokenId];
        return true;
    }

    function updateDetails(uint256 tokenId,uint256 salePrice,SellType sellType, uint256 auctionStart, uint256 auctionExpire) external returns(bool){
    ...
    function placeBid(uint256 tokenId) external payable returns(bool) {
        ...
        emit PlaceNewBid(msg.sender, tokenId, msg.value);
        return true;
    }



    function ItemPurchase(uint256 tokenId) public payable {
        ...
        this.safeTransferFrom(tokenDetails[tokenId].owner,msg.sender,tokenId);
        

        uint256 adminAmount = (msg.value.mul(CommiossionFee)).div(100000);
        uint256 royaltyAmount = (msg.value.mul(RoyaltyFee)).div(100000);
        uint256 itemOwnerAmount = msg.value.sub(adminAmount.add(royaltyAmount));


        payable(contractOwner).transfer(adminAmount);     
        payable(tokenDetails[tokenId].creator).transfer(royaltyAmount);
        payable(tokenDetails[tokenId].owner).transfer(itemOwnerAmount);   
    ...
    function acceptBid(uint256 tokenId) external returns(bool) {
    require(tokenDetails[tokenId].isActive, "Invalid token ID");
    require(bidDetail[tokenId].isActive, "Bid not available for sale");
    require(tokenDetails[tokenId].owner == msg.sender || contractOwner == msg.sender, "Caller is not item owner"); 

    this.safeTransferFrom(tokenDetails[tokenId].owner,bidDetail[tokenId].bidder,tokenId);

    uint256 adminAmount = (bidDetail[tokenId].amount.mul(CommiossionFee)).div(100000);
    uint256 royaltyAmount = (bidDetail[tokenId].amount.mul(RoyaltyFee)).div(100000);
    uint256 itemOwnerAmount = bidDetail[tokenId].amount.sub(adminAmount.add(royaltyAmount));
    ...
    emit BidApproved(
        bidDetail[tokenId].bidder,
        tokenId,
        bidDetail[tokenId].amount
    );
    return true;
  }
