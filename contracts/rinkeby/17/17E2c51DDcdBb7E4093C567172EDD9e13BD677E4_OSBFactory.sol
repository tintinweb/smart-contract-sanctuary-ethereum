...
contract OSBFactory is OwnableUpgradeable {
    ...
    event Deployed(uint256 indexed id, TokenInfo tokenInfo);
    
    function initialize(address _owner, address _library721Address, address _library1155Address) external initializer {
        __Ownable_init();
    ...
    function create(bool _isSingle, string memory _name, string memory _symbol, uint256 _maxBatch, address _royaltyReceiver, uint96 _royaltyFeeNumerator) external returns(TokenInfo memory) {
        lastId++;
        ...
        if(_isSingle) {
            ...
            _osb721.initialize(_msgSender(), _name, _symbol, _maxBatch, _royaltyReceiver, _royaltyFeeNumerator); 
            deployedContract = address(_osb721);
        } else {
            OSB1155 _osb1155 = OSB1155(Clones.cloneDeterministic(library1155Address, salt));
            _osb1155.initialize(_msgSender(), _name, _symbol, _maxBatch, _royaltyReceiver, _royaltyFeeNumerator); 
        ...
        emit Deployed(lastId, tokenInfo);
        return tokenInfo;
    }
}
