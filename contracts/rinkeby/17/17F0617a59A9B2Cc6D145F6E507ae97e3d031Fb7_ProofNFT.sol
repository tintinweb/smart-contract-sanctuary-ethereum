...
contract ProofNFT is ERC721A("Proof NFT", "PFN") {
    ...
    // external contract --mainnet only--
    ...
    function generateSVG(ProofInfo memory assetinfo) public pure returns (string memory) {
        //--mainnet only--
        //bytes memory _svgPart = bytes(assetStore.generateSVGPart(101, "asset101")); // get Material Icons
        //testnet
        string memory _svgPart = '<g id="asset101" desc="Material Icons (Apache 2.0)/UI Actions/favorite"><path d="M512 911 l-62 -56 C230 655 85 524 85 363 85 231 189 128 320 128 c74 0 145 35 192 89 C559 163 630 128 704 128 835 128 939 231 939 363 c0 161 -145 293 -365 492 L512 911 z" /></g>';
        ...
        if(assetinfo.message.length == 1){
         ...
         string memory _strsoulbound = Strings.toHexString(uint256(uint160(assetinfo.soulbound)), 20);
        _textPart =  abi.encodePacked(_textPart,TextPart_Hooter, assetinfo.blocknumber, ' : ', _strsoulbound ,TextPart_End);
    ...
    function _generateTraits(ProofInfo memory _attr) internal pure returns (bytes memory) {
        ...
        string memory _strsoulbound = Strings.toHexString(uint256(uint160(_attr.soulbound)), 20);
