...
contract LuchaMaskRandom is ERC721, Ownable, RandomlyAssigned {
   ...
   /** @dev minter tokens count */
    ...
    function mintMask() public payable callerIsUser returns (uint256) {
        require(msg.value >= mintPrice, "Ether value sent is not enough");
        
        /* If a user hold 500 tvk token allow him to mint upto 5 masks */
        if((IERC20(TVKtoken).balanceOf(msg.sender) >= 500 * 10**18 ))
        {
            mintLimit = 5;
        }
        else // no TVK
        ...
        require(tokenCounter + 1 <= totalSupply(), "Can't mint more than maximum supply" );

        if ( whiteListSale == true){
            require(whiteList[msg.sender] == true, "wallet is not whitelisted");
            require(minters[msg.sender] + 1 <= mintLimit, "Mint exceeding the allowed limit");
        } else {
            require(minters[msg.sender] + 1 <= mintLimit, "Mint exceeding the allowed limit");
        }
    ...
    function setBaseURI(string memory _baseURI) public onlyOwner {
        require(baseURIsetted == false, "Base URI is already set");
        baseURI = _baseURI;
        baseURIsetted = true;
    }
    ...
    function setRevealNft(bool _isRevealed) external {
