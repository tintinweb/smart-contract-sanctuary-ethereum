...
contract ERC721MetaSalt is ERC721Base {
    ...
    function __ERC721MetaSalt_init(string memory _name, string memory _symbol, string memory baseURI, address _erc20Token) external initializer {
        __ERC721MetaSalt_init_unchained(_name, _symbol, baseURI, _erc20Token);
        emit CreateERC721MetaSalt(_msgSender(), _name, _symbol);
    }

    function __ERC721MetaSalt_init_unchained(string memory _name, string memory _symbol, string memory baseURI, address _erc20Token) internal {
        _setBaseURI(baseURI);
        __ERC721Lazy_init_unchained(_erc20Token);        
...
abstract contract ERC721Lazy is IERC721LazyMint, ERC721Upgradeable, Mint721Validator, OwnableUpgradeable {
    ...
    uint256 public MetasaltTokenCreateRewardValue = 1000000000;

    function __ERC721Lazy_init_unchained(address _metaSaltToken) internal initializer {
