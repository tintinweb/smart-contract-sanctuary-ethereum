...
contract CryptoFamer is ERC721A, AccessControlEnumerable {
    ...
    address constant private member1 = address(0);
    address constant private member2 = address(0);
    ...
    uint256 public whitelistSaleStartTime = 0;
    ...
    constructor() ERC721A("CFtest2", "CF") {
    ...
    */
    ...
    function addAdmin(address newAdmin) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newAdmin != address(0), 'Add zero address as admin');
        grantRole(ADMIN, newAdmin);
    }

    function revokeAdmin(address toRemove) external onlyRole(DEFAULT_ADMIN_ROLE) {
        revokeRole(ADMIN, toRemove);
    }

    // Length of owners should be only 1
    function owners() external view returns(address[] memory) {
        uint256 length = getRoleMemberCount(DEFAULT_ADMIN_ROLE);

        address[] memory rets = new address[](length);

        for (uint256 i; i < length ; i++) {
            rets[i] = getRoleMember(DEFAULT_ADMIN_ROLE, i);
        }
        return rets;
    }

    function getAdmins() external view returns(address[] memory) {
        uint256 length = getRoleMemberCount(ADMIN);

        address[] memory rets = new address[](length);

        for (uint256 i; i < length ; i++) {
            rets[i] = getRoleMember(ADMIN, i);
        }
        return rets;
    }
    ...
    /*
        Variable settings
    */
    function setTierSupply(uint256 newTierSupply) public onlyRole(ADMIN) {
    ...
    function setMintPrice(uint256 newMintPrice) public onlyRole(ADMIN) {
        mintPrice = newMintPrice;
    }

    function setWhitelistMintPrice(uint256 newWLMintPrice) public onlyRole(ADMIN) {
        whitelistPrice = newWLMintPrice;
    }

    function setMaxAmountPerMint(uint256 newMaxAmountPerMint) public onlyRole(ADMIN) {
        maxAmountPerMint = newMaxAmountPerMint;
    }

    function setBaseURI(string memory newBaseUri) external onlyRole(ADMIN){ 
        _baseUri = newBaseUri;
    }
    ...
    */
    ...
    /*
        sale settings
    */
    function flipPublicSaleActive() public onlyRole(ADMIN) {
        isPublicSaleActive = !isPublicSaleActive;
    }

    function setPublicSaleTime(uint256 newStartTime) public onlyRole(ADMIN) {
        publicSaleStartTime = newStartTime;
    }

    function flipWhitelistSaleActive() public onlyRole(ADMIN) {
        isWhitelistSaleActive = !isWhitelistSaleActive;
    }

    function setWhitelistSaleTime(uint256 newStartTime) public onlyRole(ADMIN) {
    ...
    */
    ...
    function withdraw() public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (totalSupply() >= 201) {
            uint256 dev = balance * 75 / 1000;
            uint256 owner = balance - dev;
            require(payable(member1).send(owner), "Send Failed");
            require(payable(member2).send(dev), "Send Failed");
        } else {
            require(payable(member1).send(balance), "Send Failed");
        }
    }
