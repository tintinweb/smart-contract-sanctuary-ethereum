...
contract CoolCat is ERC721, IERC2981, Ownable, ReentrancyGuard {
    ...
    modifier maxCoolCatPerWallet(uint256 numberOfTokens) {
        require(
            balanceOf(msg.sender) + numberOfTokens <= MAX_CoolCat_PER_WALLET,
            "Max CoolCat to mint is three"
        );
        _;
    }

    modifier canMintCoolCat(uint256 numberOfTokens) {
        require(
            tokenCounter.current() + numberOfTokens <=
                maxCoolCat - maxGiftedCoolCat,
            "Not enough CoolCat remaining to mint"
        );
        _;
    }

    modifier canGiftCoolCat(uint256 num) {
        require(
            numGiftedCoolCat + num <= maxGiftedCoolCat,
            "Not enough CoolCat remaining to gift"
        );
        require(
            tokenCounter.current() + num <= maxCoolCat,
            "Not enough CoolCat remaining to mint"
        );
        _;
    }
    ...
    constructor(
        uint256 _maxCoolCat,
        uint256 _maxCommunitySaleCoolCat,
        uint256 _maxGiftedCoolCat
    ) ERC721("0xCoolCat", "0XCoolCat") {
        maxCoolCat = _maxCoolCat;
        maxCommunitySaleCoolCat = _maxCommunitySaleCoolCat;
        maxGiftedCoolCat = _maxGiftedCoolCat;
    }
    ...
    function mint(uint256 numberOfTokens)
        ...
        publicSaleActive
        canMintCoolCat(numberOfTokens)
        maxCoolCatPerWallet(numberOfTokens)
    {
        ...
        isValidMerkleProof(merkleProof, communitySaleMerkleRoot)
    {
        ...
        require(
            numAlreadyMinted + numberOfTokens <= MAX_CoolCat_PER_WALLET,
            "Max CoolCat to mint in community sale is three"
        );

        require(
            tokenCounter.current() + numberOfTokens <= maxCommunitySaleCoolCat,
            "Not enough CoolCat remaining to mint"
        );
    ...
    // 准备一个 bytes32[] 保存社区预售白名单，使用 merkle tree 进行 hash 加密， 使用 merkcle root 来进行验证
    ...
    function reserveForGifting(uint256 numToReserve)
        ...
        nonReentrant
        onlyOwner
        canGiftCoolCat(numToReserve)
    {
        numGiftedCoolCat += numToReserve;
    ...
    // 全部 nft = 项目方预留 + 空投名单 + 免费 mint 名单 + 预售名单 + 公售
    function giftCoolCat(address[] calldata addresses)
        external
        nonReentrant
        onlyOwner
        canGiftCoolCat(addresses.length)
    {
        uint256 numToGift = addresses.length;
        numGiftedCoolCat += numToGift;
    ...
    function rollOverCoolCat(address[] calldata addresses)
        external
        nonReentrant
        onlyOwner
    {
        require(
            ...
            "All CoolCat are already rolled over"
        );
