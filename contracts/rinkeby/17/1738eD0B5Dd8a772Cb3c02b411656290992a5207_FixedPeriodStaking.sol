...
contract FixedPeriodStaking is AccessControl {
    ...
    ITreatVesting public treatVesting;
    ...
    {
        require(amount > 10**12, "Low amount");
        ...
        uint256 receivedAmount = subsequentBalance - initialBalance;

        uint256 shares = receivedAmount / 10**9;
        ...
        newDepositsAmount += receivedAmount;

        depositId = deposits[msg.sender].length - 1;
        emit NewDeposit(msg.sender, receivedAmount, depositId);
    ...
    function withdraw(uint256 depositId) external {
        ...
        uint256 amountToTransfer = rewardAmount + depositAmount;
        //Transfer
        require(withdrawnPercent[msg.sender] == 0, "reentrancy");
        uint256 withdrawnPercentage = _getPercent(
            msg.sender,
            depositAmount
        );
        userDeposit.active = false;
        withdrawnPercent[msg.sender] = withdrawnPercentage;
    ...
    /**
     *  @notice Calculates withdrawn percentage of deposited tokens
     *  @param account User account
     *  @param amountToWithdraw Deposit amount
     */
    function _getPercent(
        ...
        for(uint i = 0; i < deposits[account].length; i++) {
            if (deposits[account][i].active == true) {
                ownedTokens += deposits[account][i].amount;
            }
        }
        if(ownedTokens == 0) return 0;
        withdrawnPercentage =  amountToWithdraw * 10000 / ownedTokens;
    }
}
