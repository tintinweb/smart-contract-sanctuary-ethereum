    ...
    PlayerOperations
{
    function __playerCore_init(
        ...
        __playerDraft_init(_dleagToken);
    }
...
contract Settings is OwnableUpgradeable {
    ...
    mapping(uint256 => mapping(uint256 => address[12])) public users; // 12 teams in each division

    /// @notice season id => division id => is draft ended
    mapping(uint256 => mapping(uint256 => bool)) public draftEnded;

    /// @notice season id => division id => is round processed
    mapping(uint256 => mapping(uint256 => mapping(uint8 => bool)))
        public roundProcessed;
    ...
    modifier onlyWhenDraftEnded(uint256 seasonId, uint256 divisionId) {
        require(
            roundProcessed[seasonId][divisionId][20],
            "Settings: Draft has not ended yet!"
        );
        _;
    }
    ...
    modifier onlyFromDivision(uint256 divisionId) {
        uint256 actualDivisionId = IFantasyLeague(fantasyLeague).UserToDivision(
            msg.sender, IFantasyLeague(fantasyLeague).seasonId()
        );
        require(
    ...
    // @notice Sets Leag Token ERC20 address
    /// @param _deagToken Leag Token contract
    function setDLeagToken(address _deagToken)
        external
        onlyOwner
        onlyValidAddress(_deagToken)
    {
        dLeagToken = _deagToken;
        emit DLeagTokenSet(_deagToken);
    }
    ...
    function assignToRoster(
        ...
        require(
            roster[seasonId][divisionId][user] < 20,
            "PlayerDraft: Cannot have more than 20 players in possession"
        );

        roster[seasonId][divisionId][user]++;
    ...
    function removeFromRoster(
        ...
        require(
            roster[seasonId][divisionId][user] > 0,
    ...
    Settings
{
    ...
    mapping(uint256 => mapping(uint256 => uint8)) public round;

    /// @notice player reservation details per season id => division id => tokenId => reservation state
    mapping(uint256 => mapping(uint256 => mapping(uint256 => ReservationState)))
        public reservedPlayers;
    ...
    function __playerDraft_init(address _dleagToken)
        internal
        initializer
    ...
    function processRound(
        ...
        //todo require isUser first

        uint256 seasonId = 1;//IFantasyLeague(fantasyLeague).seasonId();
        uint256 divisionId = 0; //IFantasyLeague(fantasyLeague).UserToDivision(
        //     drafts.length != 0 ? drafts[0].user : _reservedPlayers[0].user, seasonId
        // );

        uint8 currentRound = round[seasonId][divisionId];

        require(
            currentRound <= totalRounds &&
                !roundProcessed[seasonId][divisionId][currentRound],
        ...
        require(
            drafts.length + _reservedPlayers.length <=
                users[seasonId][divisionId].length,
        ...
        for (uint256 i = 0; i < drafts.length; i++) {
            if (
                ...
                draftPlayer(drafts[i], signatures[i], seasonId, divisionId);
            }
        }

        for (uint256 i = 0; i < _reservedPlayers.length; i++) {
            reservePlayer(_reservedPlayers[i], seasonId, divisionId);
        }

        roundProcessed[seasonId][divisionId][currentRound] = true;
        if (round[seasonId][divisionId] < totalRounds) {
            round[seasonId][divisionId]++;
        }
        emit RoundProcessed(round[seasonId][divisionId]);
    }
    ...
    function sortPicks(MerkleProof[] memory mp) public {
        require(
        ...
        uint256 seasonId = 1;//IFantasyLeague(fantasyLeague).seasonId();
        uint256 divisionId = 0;//IFantasyLeague(fantasyLeague).UserToDivision(
        //     mp[0].user,IFantasyLeague(fantasyLeague).seasonId()
        // );

        round[seasonId][divisionId] = 1;

        for (uint256 i = 0; i < mp.length; i++) {
            // uint256 userDivision = IFantasyLeague(fantasyLeague).UserToDivision(
            //     mp[i].user, seasonId
            // );
            ...
            users[seasonId][divisionId][i] = mp[i].user;
            ...
            if (userBalance == balance) {
                ...
                if (equalStartIndex != equalEndIndex) {
                    users[seasonId][divisionId] = DraftPickLib.randomizeArray(
                        users[seasonId][divisionId],
                        equalStartIndex,
                        equalEndIndex
                    );
        ...
        emit UsersSorted(users[seasonId][divisionId]);
    }
    ...
    {
        if (round[seasonId][divisionId] % 2 != 0) {
            return users[seasonId][divisionId];
        }

        address[12] memory reversedOrder;
        uint256 j = users[seasonId][divisionId].length;
        for (uint256 i = 0; i < users[seasonId][divisionId].length; i++) {
            reversedOrder[j - 1] = users[seasonId][divisionId][i];
            j--;
        }
    ...
    function draftPlayer(
        ...
        IGen2PlayerToken(playerV2).mint(
            draft.tokenId,
            draft.divisionId,
            draft.permit.owner
        );
        ...
        // uint256 season = IFantasyLeague(fantasyLeague).seasonId();
        // uint256 division = IFantasyLeague(fantasyLeague).UserToDivision(
        //     msg.sender, season
        // );
    ...
    function reservePlayer(
        ReservedPlayer memory reservedPlayer,
        uint256 seasonId,
        uint256 divisionId
    ) public onlyValidAddress(reservedPlayer.user) {
        reservedPlayers[seasonId][divisionId][reservedPlayer.tokenId]
            .user = reservedPlayer.user;
        reservedPlayers[seasonId][divisionId][reservedPlayer.tokenId]
            .startPeriod = block.timestamp;
        reservedPlayers[seasonId][divisionId][reservedPlayer.tokenId]
            .endPeriod = block.timestamp + reserveExpirationTime;
        reservedPlayers[seasonId][divisionId][reservedPlayer.tokenId]
            .redeemed = false;

        emit PlayerReserved(
            0, //todo hardcoded for subgraph will be changed later
            reservedPlayers[seasonId][divisionId][reservedPlayer.tokenId].user,
    ...
    function buyPlayers(uint256[] memory _tokenIds, PermitSig memory permit)
        public
    {
        uint256 seasonId = 0;//IFantasyLeague(fantasyLeague).seasonId();
        // require(
        //     IFantasyLeague(fantasyLeague).isUser(seasonId, msg.sender),
        //     "User is not participant in curr season"
        // );

        uint256 divisionId = 1;//IFantasyLeague(fantasyLeague).UserToDivision(
        //     permit.owner, seasonId
        // );

        require(
            roundProcessed[seasonId][divisionId][20], //todo this needs to be a setter
            "Player Draft: draft has not ended"
        );
        uint256 _totalCost = 0;

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ReservationState memory _state = reservedPlayers[seasonId][
                divisionId
            ][_tokenIds[i]];
            ...
            assignToRoster(seasonId, divisionId, msg.sender, _tokenIds[i]);
        }
    ...
    Settings
{
    ...
    // function auctionStart(uint256 playerTokenId, PermitSig calldata permitSig)
    //     external
    //     virtual
    //     onlyWhenTournamentStarted
    // {
    //     require(
    ...
    //     );
            ...
            //todo firs
    //     uint256 seasonId = IFantasyLeague(fantasyLeague).seasonId();
    //     uint256 divisionId = IFantasyLeague(fantasyLeague).UserToDivision(
    //         permitSig.owner
    //     );
    ...
    // /// @param permitSig Permit signature as struct for the owner, spender and value in order for a permit to be successful
    // function auctionBid(uint256 auctionId, PermitSig calldata permitSig)
    //     external
    //     virtual
    //     nonReentrant
    // {
    ...
    //     uint256 prevWinnerDivisionId = IFantasyLeague(fantasyLeague)
    //         .UserToDivision(aucState.winning);
    ...
    //     IGen2PlayerToken(playerV2).safeTransferFrom(
    //         address(this),
    //         aucState.winning,
    //         aucState.playerTokenId
    //     );

    //     uint256 fraction = aucState.price / users[seasonId][divisionId].length;
    //     uint256 dust = aucState.price -
    //         (fraction * users[seasonId][divisionId].length);
    ...
    Settings
{
    ...
    {
        IGen2PlayerToken(playerV2).burn(tokenId);
        ...
        removeFromRoster(seasonId, divisionId, msg.sender, tokenId);
...
interface IFantasyLeague {
    //????
    function UserToDivision(address user_address, uint256 season)
        external
        pure
        returns (uint256);
