...
contract TastyBones is Ownable, ERC721A {
    using SafeMath for uint256;
    using Address for address;
    ...
    // free mint - 500
    // 2 max free mints per whitelisted wallet
    // left overs from free mint and presale will be raffled
    ...
    uint8 public constant maxFreeBoneMint = 1;
    uint8 public constant maxFreeMint = 2;
    uint8 public constant maxPresaleMint = 2;
    uint8 public constant maxRaffleMint = 1;
    uint256 public maxFreeMintSupply = 500;
    uint256 public maxPresaleSupply = 4000;
    uint256 public maxPublicSale = 500;
    uint256 public maxTastyBones = 5050;
    bool public isRaffleActive = true;
    bool public isPresaleActive = true;
    bool public isFreeActive = true;

    // free mint
    mapping (uint256 => uint256) public mintedTBforFreeMintBone;
    ...
    // 
    mapping (address => uint256) public mintedTBforFreeExtra;
    mapping(address => uint256) addressBlockBought;

    bytes32 private freeMintBoneMerkleRoot = 0x7a02a5a664bd5f7c938792115f65cb704c187d4683b9fd89d7ad49d47044a37b;
    bytes32 private freeMintListMerkleRoot = 0x2308f67e7c832900642bcaa5d26aed32467d5155f59306a93d8442db99ca221a;

    bytes32 private presaleMerkleRoot = 0x04dafba28d78cea1c319b6659d3f4f7f790c247a3571bda4a853d8f74f0f70f4;
    bytes32 private raffleMerkleRoot = 0x04dafba28d78cea1c319b6659d3f4f7f790c247a3571bda4a853d8f74f0f70f4;

    ApprovingBone public approvingBoneContract;
    Counters.Counter public tokenSupply;
    constructor(address boneContractAddress) 
        ERC721A("Tasty Bones", "TASTYBONES", 5050)  {
        // setBaseURI for metadata
    ...
    function mintFree(
        ...
        require(!Address.isContract(msg.sender),"CONTRACTS_NOT_ALLOWED_TO_MINT");
        require(isFreeActive, "FREE_MINT_IS_NOT_YET_ACTIVE");
        require(tokenSupply.current().add(numToMint) <= maxFreeMintSupply, "EXCEEDS_FREE_MINT_SUPPLY" );
        require(numToMint <= maxMint,"CANNOT_MINT_MORE_THAN_ALLOWED");
        require(mintedTBforFreeMintAddress[msg.sender] + numToMint <= maxFreeMint,"ALREADY_MINTED_MAX_2");
        ...
        if(bonesOfOwner) {
            ...
            require(mintedTBforFreeMintBone[boneTokenId] < 1, "BONE_ALREADY_USED_FOR_MINTING");
            canMint += 1;
        }

        // For role
        if(freeMintProof.length > 0) {
            require(MerkleProof.verify(boneProof, freeMintListMerkleRoot, keccak256(abi.encodePacked(msg.sender, maxMint))), "YOU_ARE_NOT_WHITELISTED_TO_MINT_FREE");
            if(bonesOfOwner) {
                canMint += 1;
            } else {
                canMint += maxMint;
        ...
        require(canMint <= maxMint,"CANNOT_MINT_MORE_THAN_ALLOWED");
        _safeMint( msg.sender, numToMint);
        mintedTBforFreeMintAddress[msg.sender] += canMint;
        addressBlockBought[msg.sender] = block.timestamp;
        
        if(bonesOfOwner) {
            mintedTBforFreeMintBone[boneTokenId] = mintedTBforFreeMintBone[boneTokenId].add(1);
        }
    }
     ...
     */
    function mintPresale(uint256 numberOfTokens, bytes32[] calldata proof) external payable {
        require(addressBlockBought[msg.sender] < block.timestamp, "CANNOT_MINT_ON_THE_SAME_BLOCK");
        require(!Address.isContract(msg.sender),"CONTRACTS_NOT_ALLOWED_TO_MINT");
        ...
        require(numberOfTokens <= maxPresaleMint, "MAX_PRESALE_MINT_EXCEEDED");
        require(msg.value >= price * numberOfTokens, "INSUFFICIENT_PAYMENT");
        require(tokenSupply.current().add(numberOfTokens) <= maxPresaleSupply, "EXCEEDS_MAX_SUPPLY" );
        require(mintedTBforPresale[msg.sender] <= maxPresaleMint, "EXCEEDS_MAX_PRESALE_MINT" );
     ...
     */
    function mintRaffle(uint256 numberOfTokens, bytes32[] calldata proof) external payable {
        require(addressBlockBought[msg.sender] < block.timestamp, "CANNOT_MINT_ON_THE_SAME_BLOCK");
        require(!Address.isContract(msg.sender),"CONTRACTS_NOT_ALLOWED_TO_MINT");
        ...
        require(msg.value >= price * numberOfTokens, "MAX_MINT_REACHED");
        require(numberOfTokens <= maxRaffleMint, "MAX_RAFFLE_MINT_EXCEEDED");
        require(tokenSupply.current().add(numberOfTokens) <= maxTastyBones, "EXCEEDS_MAX_SUPPLY" );
        require(mintedTBforPresale[msg.sender] <= maxRaffleMint, "EXCEEDS_MAX_RAFFLE_MINT" );
        addressBlockBought[msg.sender] = block.timestamp;
     ...
     */
    function mintTBForTeam() external onlyOwner {
        require(teamTBones > 0, "NFTS_FOR_THE_TEAM_HAS_BEEN_MINTED");

        _safeMint(msg.sender, teamTBones);

        teamTBones -= teamTBones;
    }
