...
contract ChainScouts is IChainScouts, MerkleWhitelistERC721Enumerable {
    ...
    uint private _mintPriceWei = 0.067 ether;
...
abstract contract ExtensibleERC721Enumerable is IExtensibleERC721Enumerable, ERC721A {
    ...
    function adminBurn(uint tokenId) external override onlyAdmin {
        _burn(tokenId);
    }

    function adminTransfer(address from, address to, uint tokenId) external override {
...
abstract contract MerkleWhitelistERC721Enumerable is ExtensibleERC721Enumerable {
    ...
    function internalMint(uint count) internal returns (uint) {
        if (!isAdmin[msg.sender]) {
