...
contract GsdStaking is ReentrancyGuard {
    ...
    mapping(address => uint256) public nftAmountDeposited;
    mapping(address => mapping(uint256 => bool)) public nftStakers;
    ...
    constructor(address _governor, address _baseToken, address _nftcontract, address _interestToken) public {
        require(_governor != address(0), "Cannot be 0x0");
        require(_baseToken != address(0), "Cannot be 0x0");
        require(_nftcontract != address(0), "Cannot be 0x0");
        require(_interestToken != address(0), "Cannot be 0x0");
    ...
    function stake(uint256 _amount, uint256[] memory tokenIds) external nonReentrant noContractAllowed {
        ...
        if (tokenIds.length > 0) {
            nftAmountDeposited[depositor] = nftAmountDeposited[depositor].add(tokenIds.length);

            for (uint i = 0; i < tokenIds.length; i++) {
                uint256 _gsdValue = GSNFT(nftcontract).getGSValue(tokenIds[i]);
                ...
                nftStakers[depositor][tokenIds[i]] = true; 
                                                                                                                                             
                GSNFT(nftcontract).safeTransferFrom(depositor, address(this), tokenIds[i]);
            }
        }

        stakers[depositor] = Position({depositGSD: depoGSD, depositNFTValue: depoNFT, TVL: tvl, lastIndex: index, claimableAux: claims});
    ...
    function unstake(uint256 _amount, uint256[] memory tokenIds, bool _claimAndUnstake) external nonReentrant noContractAllowed {
        ...
        if (tokenIds.length > 0) {
            nftAmountDeposited[depositor] = nftAmountDeposited[depositor].sub(tokenIds.length);

            for (uint256 i = 0; i < tokenIds.length; i++) {
                require(nftStakers[depositor][tokenIds[i]] == true);
                uint256 _gsdValue = GSNFT(nftcontract).getGSValue(tokenIds[i]);
                ...
                nftStakers[depositor][tokenIds[i]] = false;
                GSNFT(nftcontract).safeTransferFrom(address(this), depositor, tokenIds[i]);
            }
        ...
        if (tvl == 0) {
            ...
            stakers[depositor] = Position({depositGSD: depoGSD, depositNFTValue: depoNFT, TVL: aggregateTVL, lastIndex: index, claimableAux: 0});           
            IDetailedERC20(interestToken).safeTransfer(depositor, claims);

            emit AuxClaimed(depositor, claims);

        } else {
           stakers[depositor] = Position({depositGSD: depoGSD, depositNFTValue: depoNFT, TVL: aggregateTVL, lastIndex: index, claimableAux: claims}); 
        }
    }

    /*
    Note: Old unstake function. Will remove once I'm 100% sure about new one.
    function _unstake(uint256 _amount, bool _claimAndUnstake) external nonReentrant noContractAllowed {
        address depositor = msg.sender;

        (uint256 depo, uint256 index, uint256 claims) = updateClaimableInterest(depositor);
        require(_amount <= depo);

        aggregateTVL.sub(_amount);
        IDetailedERC20(baseToken).safeTransfer(depositor, _amount);

        emit GsdUnstaked(depositor, _amount);

        if (_amount == depo || _claimAndUnstake == true) {
            // Claim is mandatory if the user unstakes full deposited amount.
            delete stakers[depositor];
            ...
            stakers[depositor] = Position(depo.sub(_amount), index, 0);

            IDetailedERC20(interestToken).safeTransfer(depositor, claims);
            emit AuxClaimed(depositor, claims);
            
        } else {
            stakers[depositor] = Position(depo.sub(_amount), index, claims);
        }
    }
    */
...
contract GSNFT is ERC721, Ownable {

    // Contract address that sells the NFTS.
    address public marketplace;

    // Mapping from tokenId to GSD equivalence.
    ...
    constructor(string memory _baseURI) ERC721("GS DAO Token Collection", "GSNFT") public {
        _setBaseURI(_baseURI);
    }

    function mintGSNFT(address to, uint256 tokenId, string calldata tokenURI, uint256 gsEquivalence) external onlyMarketplace {
        gsdEquivalence[tokenId] = gsEquivalence;
