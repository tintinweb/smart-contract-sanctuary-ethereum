...
contract VZOOFactory is FactoryERC721, Ownable {
    ...
    constructor() {
        ...
        nftAddress = 0xd13009A71370e85AA183Fe91A88f0a5d80254bbD;
        fireTransferEvents(address(0), owner());
    }

    function name() external pure override returns (string memory) {
        return "VZOO Item Sale";
    }
    ...
    /**
        @dev Sets a new proxy registry address (just in case).
    ...
    function canMint(uint256 _optionId) public view override returns (bool) {
        ...
        return vzooSupply < (VZOO_SUPPLY - numItemsAllocated);
    }
    ...
    function canMintAndOwn(address _toAddress) public view returns (bool) {
        ...
        require(
            mintedBalance <= vzoo.mintPerAddressLimit(),
            "mints per address limit exceeded"
        );
        require(
            addressBalance <= vzoo.amountPerAddressLimit(),
            "nft per address limit exceeded"
        );
        return true;
    }
...
/**
    @title VZOO
    VZOO - a contract for ERC721 non-fungible token.
 */
...
/**
    ...
    VZOO - ERC721 contract that whitelists a trading address, and has minting functionality.
 */
    ...
    Ownable
{
    ...
    uint256 private _maxMintPerAddress = 4;
    uint256 private _maxAmountPerAddress = 5;
    address private _receiver;
    uint96 private _feeNumerator;
    mapping(address => uint256) public addressMintedBalance;

    uint256 MAX_SUPPLY = 10;
    uint256 MAX_SUPPLY_TEAM = 200;
    ...
    function mintTo(address _to) public whenNotPaused onlyOwner {
        ...
        require(_totalSupply <= currentTokenId, "total supply limit reached");
    ...
    /**
        @dev Returns the allowed amount of token per address.
     */
    ...
    /**
        @dev Returns the allowed amount of mints per address.
     */
    ...
    /**
        @dev Sets the allowed amount of token per address.
     */
    ...
    /**
        @dev Sets the allowed amount of mints per address.
     */
    ...
    /**
        @dev Sets a new base URI extension.
        Defaults to "", but can be set to values like .json.
     */
    ...
    /**
        @dev Sets the default royalty address and fee.
        Defaults to "1000" = 10% of transaction value.
     */
    ...
    /**
        @dev Returns the base token uri of the nfts.
     */
    function baseTokenURI() public pure virtual returns (string memory);

    /**
        @dev Returns the token uri of the nfts.
     */
    ...
    function _beforeTokenTransfer(
        ...
        if (to != owner()) {
            require(
                balanceOf(to) + 1 <= _maxAmountPerAddress,
    ...
    // /**
    //     @dev Overrides withdrawPayments to make it an onlyOwner function.
    //  */
