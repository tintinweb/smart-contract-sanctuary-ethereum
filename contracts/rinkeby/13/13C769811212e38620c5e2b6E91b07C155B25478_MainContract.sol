...
contract MainContract is ERC721Psi {
    constructor() ERC721Psi("GreenerPlanetContract", "GP") {}
    ...
    function mint(uint256 quantity, string calldata path) external payable {
        //require and revert solidity methods does a check and returns some of the gas if rejected
        require(
            totalSupply() + quantity <= 30,
            "Mint total exceeds the remaining available tokens"
        );
        setBaseURI(path);
