...
contract LazyLeopardClub is ERC721Enumerable, Ownable
{
    ...
    uint public maxPerTxn = 2; // 2 - whitelist , 10 - public
    uint public maxPerWallet = 2; // 2 - whitelist , 100 - public
    ...
    uint private reserved = 113;
    
    mapping(address => bool) whitelistedAddresses;

    constructor() ERC721("Lazy Leopard Club", "LLC")  {
        setBaseURI("");
    }
    ...
    function mint(uint chosenAmount) public payable isSaleOpen{
        require( (status == 2) || ( status == 1 && whitelistedAddresses[msg.sender] ) , "Sale is not active at the moment" );
        ...
        require( chosenAmount + balanceOf( msg.sender ) <= maxPerWallet , "Chosen Amount exceeds MaxQuantity" );
    ...
    function addWhitelistAddress(address a) public onlyOwner {
        whitelistedAddresses[a] = true;
    }
    function addWhitelistAddresses(address[] memory a) public onlyOwner {
        for(uint i=0; i<a.length; i++)
            whitelistedAddresses[a[i]] = true;
    }
}