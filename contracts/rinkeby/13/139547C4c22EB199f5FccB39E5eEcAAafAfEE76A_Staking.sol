...
mapping (address => uint[]) stakedNfts;
...
modifier Update() {
debt[msg.sender] += (poolPerNFT - reclaimed[msg.sender]) * stakedCount[msg.sender];
reclaimed[msg.sender] = poolPerNFT;
    _;
}
...
function stake(uint[] memory _nftIds, uint[] memory amounts) public Update {
    ...
    stakedNfts[msg.sender].push(_nftIds[i]);
...
function withdraw(uint[] memory _nftIds, uint[] memory amounts) public Update {
...
function claimFunds() public Update {
...
function CheckOwnership(uint[] memory ids) public view returns(uint[] memory) {
    uint[] memory idsChecked;
    for(uint i; i < ids.length; i++) {
        if(tokenIdOwner[ids[i]] == msg.sender) {
          idsChecked[i] = ids[i];
        }
    }
    return idsChecked;
