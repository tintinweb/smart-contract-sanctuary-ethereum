...
interface RektRacoons {
    function balanceOf(address _user) external view returns(uint256);
    function ownerOf(uint256 _tokenId) external view returns(address);
    function totalSupply() external view returns (uint256);
}

contract DogeVille is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public _devWalletAddress1 = 0x497d5F8e974B0F115D94Bb65FccD6F0CdC4a61f0;   
    address public _devWalletAddress2 = 0xc9c314f3C662dccdae12E06b043F315f1e6FAa77;   // TODO - team wallet here
    ...
    uint256 public transferStartFrom = 1639756586;
    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    bool inSwapAndLiquify;
    bool public transferOnlyOwner = true;
    bool public swapAndLiquifyEnabled = true;
    uint256 public _maxTxAmount;
    uint256 public numTokensSellToAddToLiquidity;
    RektRacoons public rektRacoons;
    ...
    constructor (string memory _NAME,  uint256 _baseRate, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _DexFee,address routerAddress,address tokenOwner, address _rektRacoonsAddress)  {
        _name = _NAME;
        _symbol = _SYMBOL;
        _decimals = _DECIMALS;
        ...
        baseRate: _baseRate * 10 ** _decimals,
        start: 1643533896,
        end: 1795017600
        });
        ...
        address rektRacoonsContractAddress = _rektRacoonsAddress;
		rektRacoons = RektRacoons(rektRacoonsContractAddress);
    ...
    function claimReward(uint256 _tokenId) public returns (uint256) {
        ...
        require(rektRacoons.ownerOf(_tokenId) == msg.sender, "Caller does not own the token being claimed for.");
        ...
        _transfer(_owner, msg.sender, unclaimedReward);
        USDT.transfer(msg.sender,unclaimedReward);

        return unclaimedReward;
    }

    function claimRewards(uint256[] calldata _tokenIds) public returns (uint256) {
        require(contractSettings.end > block.timestamp, "Time for claiming has expired");
        ...
        address payable _owner = payable(address(uint160(owner())));
        _transfer(_owner, msg.sender, totalUnclaimedRewards);
        USDT.transfer(msg.sender,totalUnclaimedRewards);
    ...
    function getUnclaimedRewardsAmount(uint256[] calldata _tokenIds) public view returns (uint256) {
    ...
    function getTotalUnclaimedRewardsForContract() public view returns (uint256) {
        uint256 totalUnclaimedRewards = 0;
        uint256 rektRacoonsTotalSupply = rektRacoons.totalSupply();

        for(uint256 i = 0; i < rektRacoonsTotalSupply; i++) {
    ...
    function computeUnclaimedReward(uint256 _tokenId) internal view returns (uint256) {
        ...
        if (lastClaimDate != uint256(0)) {
            ...
            uint256 totalReward = computeAccumulatedReward(contractSettings.start, baseRate, block.timestamp);

            return totalReward;
        }
    }

    function updateTime(uint256 _tokenId) internal {
        require(msg.sender == address(rektRacoons));
        bytes32 lastClaimKey = keccak256(abi.encode(_tokenId));
        lastClaim[lastClaimKey] = block.timestamp;
    }
    ...
    require(transferOnlyOwner && msg.sender == _owner , "currently you are not allowed to transfer");
    _transfer(_msgSender(), recipient, amount);
    return true;
        //  if (transferOnlyOwner) {
        //     require(msg.sender == _owner , "currently you are not allowed to transfer");
        //     _transfer(_msgSender(), recipient, amount);
        //     return true;
        // } else {
        //     // require(transferStartFrom <= block.timestamp, "transfer time is not start");
        //     _transfer(_msgSender(), recipient, amount);
        //     return true;
        // }
    ...
    function setAllowTransferToAll(bool _enabled) external onlyOwner() {
        transferOnlyOwner = _enabled;
    }
    function setTokenTransferDate(uint256 _time) external onlyOwner() {
        transferStartFrom = _time;
    }

    function setRewardAmount(uint256 _Amount) external onlyOwner() {
        contractSettings.baseRate = _Amount * 10 ** _decimals ;
    }
    ...
    function setRektRacoonsAddress(address _addr) public onlyOwner {
        rektRacoons = RektRacoons(_addr);
    }
    
    function setDevWalletAddress(address _addr1,address _addr2) public payable onlyOwner {
        _devWalletAddress1 = _addr1;
        _devWalletAddress2 = _addr2;
    ...
    function _takeDev(uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
        uint256 part1 = rDev/2;
        _rOwned[_devWalletAddress1] = _rOwned[_devWalletAddress1].add(part1);
        _rOwned[_devWalletAddress2] = _rOwned[_devWalletAddress2].add(rDev.sub(part1));
        if(_isExcluded[_devWalletAddress1] || _isExcluded[_devWalletAddress2])
            _tOwned[_devWalletAddress1] = _tOwned[_devWalletAddress1].add(tDev/2);
            _tOwned[_devWalletAddress2] = _tOwned[_devWalletAddress2].add(tDev/2);
    ...
    function setNumTokensSellToAddToLiquidity(uint256 amountToUpdate) external onlyOwner {
        ...
        function BuyFromOwner(uint256 numberOftoken) external payable {
        // require(msg.value >= 10 ** 17 && msg.value <= 10 ** 18, "Min 0.1 bnb, Max 1 bnb");
        // uint256 amountPerBNB = 800 * 10**12 * 10**uint256(_decimals);
        // uint256 amountTransfer = amountPerBNB.mul(msg.value).div(10**18);
        uint256 amountTransfer = numberOftoken*10**_decimals; //for testing
        address payable _owner = payable(address(uint160(owner())));
        _transfer(_owner, msg.sender, amountTransfer);
