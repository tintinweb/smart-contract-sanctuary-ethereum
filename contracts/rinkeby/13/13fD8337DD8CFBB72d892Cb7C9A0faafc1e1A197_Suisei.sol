    ...
    CustomPaymentSplitterUpgradeable
{
    ...
    function initialize(
        ...
        setPrice(uint8(Stage.Whitelist), 1 ether);
        setPrice(uint8(Stage.Public), 1 ether);        
    }

    function isApprovedForAll(address owner_, address operator_) public view virtual override(ERC721AUpgradeable) returns (bool) {
        return super.isApprovedForAll(owner_, operator_) || OpenSeaGasFreeListing.isApprovedForAll(owner_, operator_) || msg.sender == address(staking);
    }
    ...
    function vipMint(
        ...
        external
        onlyWhitelisted(uint8(Stage.Vip), msg.sender, proof_)
    {
        ...
        onlyWhitelisted(uint8(Stage.Whitelist), msg.sender, proof_)
    {
        ...
        _handleIfStaking(amount_, stake_);
        handlePayment(amount_ * price(_stage));
    }
...
contract WhitelistUpgradable is Initializable, AdminManagerUpgradable {
    ...
    mapping(uint8 => Whitelist.Data) internal _whitelists;
...
contract PriceUpgradable is Initializable, AdminManagerUpgradable {
    ...
    function handlePayment(uint256 cost) internal {
