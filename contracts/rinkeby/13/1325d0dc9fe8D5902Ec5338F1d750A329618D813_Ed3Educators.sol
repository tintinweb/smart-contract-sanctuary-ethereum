    ...
    IEd3EducatorsMetadata
{
    ...
    function purchasePreSale(uint256 numberOfTokens, bytes32[] calldata proof)
        external
        payable
        override
    {
        ...
        bool isValidMerkleProof = verifyMerkle(
            msg.sender,
            numberOfTokens,
            proof
        );
        if (!isValidMerkleProof) revert NotInMerkle();
    ...
    function reserve(address to, uint256 amount) external override onlyOwner {
        require(totalSupply() < MAX_SUPPLY, "All tokens have been minted");
        require(
            totalReservedSupply + amount <= SUPPLY_RESERVED,
            "Reserve would exceed max reserved tokens"
        );
