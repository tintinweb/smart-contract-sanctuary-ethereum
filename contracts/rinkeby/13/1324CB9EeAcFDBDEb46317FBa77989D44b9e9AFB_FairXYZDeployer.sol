...
contract FairXYZDeployer is ERC721xyz, Pausable{
    ...
    // Mint a token    
    function mint(bytes memory signature, uint256 block_nonce, uint256 numberOfTokens)
        payable
        public
        whenNotPaused
        saleIsOpen
        returns (uint256)
    {
        string memory nonce = string(abi.encodePacked(address(this), block_nonce));
        ...
        require(block.number <= block_nonce  + 20, "Time limit has passed");
        if(Max_mints_per_wallet > 0)
