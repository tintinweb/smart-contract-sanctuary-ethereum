...
contract OddworxStaking is Pausable {
    ...
    event UnstakedNft(address indexed user, address indexed nftContract, uint256 indexed nftId);
    ...
    function getNftData(IERC721 nftContract, uint256 id) external view returns (address, bool, uint64, bool) {
        nftDataStruct memory nft = nftData[nftContract][id];
    ...
    function stakeNfts(IERC721 nftContract, uint256[] calldata nftIds) external whenNotPaused {
        ...
        for (uint256 i; i<nftIds.length; i++) {
            ...
            if (nft.legacyStaking == false) {
                totalRewards += _executeRewards(nftContract, nftId, HOLDING_REWARD, HOLDING_REWARD);
            } else {
    ...
    function unstakeNfts(IERC721 nftContract, uint256[] calldata nftIds) external whenNotPaused {
        ...
        for (uint256 i; i<nftIds.length; i++) {
            ...
            emit UnstakedNft(msg.sender, address(nftContract), nftId);
        }
        if (totalRewards > 0) mint(msg.sender, totalRewards);
    }    
    ...
    function unstakeNftEmergency(IERC721 nftContract, uint256[] calldata nftIds, address to) external onlyAdmin {
        for (uint256 i; i<nftIds.length; i++) {
            ...
            emit UnstakedNft(msg.sender, address(nftContract), nftIds[i]);
        }
    }
    ...
    function claimRewards(IERC721 nftContract, uint256[] calldata nftIds) external whenNotPaused {       
        if (!nftInterfaces[nftContract]) revert InvalidInput();
        uint256 totalRewards;
        nftDataStruct memory nft;
        ...
        for (uint256 i; i<nftIds.length; i++) {
            ...
            if (nft.staked == false) {
                if (nftContract.ownerOf(nftId) != msg.sender) revert NotOwnerOfToken();
                totalRewards += _executeRewards(nftContract, nftId, HOLDING_REWARD, HOLDING_REWARD);
            } else {
                if (nft.ownerAddress != msg.sender) revert NotOwnerOfToken();
                totalRewards += _executeRewards(nftContract, nftId, STAKING_REWARD, 0);
