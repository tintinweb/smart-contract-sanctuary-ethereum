...
contract ensdaddy14 is Ownable {
struct ensdata {
    uint256 id;
    string subdomain;
    string domain;
...
constructor(){}
    ...
    function register_subdomain(string memory _domain, string memory _subdomain, string memory _fulldomain) external payable {
        require(
                 (_saleIsActive),
                "Minting is not Live"
        );
    ...
    function update_allsubdomain(string memory _domain) public {
        for (uint i=0; i< ensdomains_count; i++) {
            if(keccak256(bytes(ensdomains[i].domain)) == keccak256(bytes(string(abi.encodePacked(_domain)))) ){
    ...
    function update_subdomain(string memory _domain, string memory _fulldomain) public {
        for (uint i=0; i< ensdomains_count; i++) {
            if(keccak256(bytes(ensdomains[i].domain)) == keccak256(bytes(string(abi.encodePacked(_domain)))) ){
