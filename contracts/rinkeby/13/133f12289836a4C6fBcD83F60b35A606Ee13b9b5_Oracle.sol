...
contract Oracle is Ownable{
    ...
    @param _escrow Contract address of Escrow.
    @param _uid Predefined unique identifier for contract/agreement.
    @param _validationStatus true if it passes all checks, false if it fails on any.
    @param _fee fee amount in given currency with decimals
    @param _isNewMint mark minting as a must
    @custom:events this emits "Complete" event.
    */
    function validate(address _escrow,uint _uid, bool _validationStatus,uint _fee, bool _isNewMint) public onlyOwner{
        Escrow escrow = Escrow(_escrow);
        escrow.validateContract(_uid, _validationStatus, _fee,_isNewMint);
    }
}
