...
contract Villain is ERC721URIStorage, Ownable, ERC721Enumerable {
    ...
    //Fires on a new mint
    event VillainClaim(uint256 nftId, address nftOwner);
    ...
    function canClaimAnything(address to)
    private
    returns(uint256)
    {
    ...
    public
    {
        require(canClaimAnything(to) > 0,"Nothing to claim :(");
        uint256 ownerBalance = IWASNFT(_wasNFTSmartContract).balanceOf(to);
        uint256 satoshiId;
    ...
    //This method does not check if the user can claim anything or not.
    //Trying to measure how much gas this will save.
    //Double usage of this methof will not result in minting additional tokens
    function claimv2(address to)
    public
    {
        uint256 ownerBalance = IWASNFT(_wasNFTSmartContract).balanceOf(to);
        uint256 satoshiId;
    ...
    //Claim v3 method 
    //This method will use master mint method withour a counter
    //This methof will inherit satoshi ids.
    function claimv3(address to)
    public
    {
        uint256 ownerBalance = IWASNFT(_wasNFTSmartContract).balanceOf(to);
        uint256 satoshiId;
        ...
        for (uint256 i = 0; i<ownerBalance; i++) {
            ...
            if(!_claimedSatoshisStorage[satoshiId]) {
                _claimedSatoshisStorage[satoshiId] = true;
                _claimedSatoshis.push(satoshiId);
                _masterMintNoCounter(to, satoshiId);
            }
        }
    }

    //Standard claim single satoshi
    function claimSingleSatoshi(address to, uint256 satoshiId)
    public
    {
        require(canClaimAnything(to) > 0,"Nothing to claim :(");
        require(IWASNFT(_wasNFTSmartContract).ownerOf(satoshiId) == to, "You must be the owner of this Satoshi.");
    ...
    private
    {
        ...
        emit VillainClaim(newItemId,to);
    }

    //This is a v2 method of token mint. Token ids are not incremented but passed from satoshi tokenid
    function _masterMintNoCounter(address to, uint256 tokenId)
    private
    {
        require(!_isPaused,"Mint is currently paused.");
        require(_tokensAvailable > 0, "All tokens have been minted already");
    ...
    //Needs to be updated depending on which claiming mechanism is chosen
    ...
    //This can be removed, no supply change since thi is a 1:1 claim
