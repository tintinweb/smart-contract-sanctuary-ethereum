...
contract BondDepository is Ownable {
    ...
    event BondPriceChanged(
        uint256 indexed beforePrice,
        uint256 indexed afterPrice,
    ...
    address public immutable MINT; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints MINT when receives principle
    ...
    struct Bond {
        uint256 payout; // MINT remaining to be paid
    ...
    function initializeBondTerms(
        ...
        // TODO: wETHBondDepository bases this off currentDebt() check
        // allowing the bcv to be initialized to something other than 0
        // research which one to use
        require(terms.controlVariable == 0, 'Bonds must be initialized from 0');
        // TODO: reactivate this for prod
        // require(_vestingTerm >= 10000, 'Vesting must be longer than 36 hours');
        require(
            _initialDebt <= _maxDebt,
            'Initial debt must not exceed max debt'
        );
        terms = Terms({
    ...
    function deposit(
        ...
        uint256 value = ITreasury(treasury).computedValueOf(principle, _amount);
        uint256 payout = payoutFor(_amount); // payout to bonder is computed

        require(payout >= 1e16, 'Bond too small'); // must be > 0.01 MINT ( underflow protection )
        require(payout <= maxPayout(), 'Bond too large'); // size protection because there is no slippage

        // TODO: Add tests to ensure that terms.fee is always <= 10000
        ...
        if (fee != 0) {
            ...
            IERC20(MINT).safeTransfer(DAO, fee);
        }
        ...
        emit BondPriceChanged(nativePrice, _bondPrice(), debtRatio());
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(MINT).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(MINT).approve(stakingHelper, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return (IERC20(MINT).totalSupply() * (terms.maxPayout)) / (100000);
    }
    ...
    function payoutFor(uint256 _value) public view returns (uint256) {
        PRBMath.UD60x18 memory xud = PRBMath.UD60x18({value: _value}); // value is in wei
        PRBMath.UD60x18 memory yud = PRBMath.UD60x18({value: bondPrice()}); // bondPrice is in ether

        return xud.div(yud).value;
    }

    /**
     *  @notice calculate current bond premium 
        bcv * ratio / 1e9 + 1000 gwei, floor is 1000 gwei per 1 ether of MINT
        should this be computed in terms of MINT wei? 
     *  @return price_ uint
     */
    function bondPrice() public view returns (uint256 price_) {
        // TODO: Double check this config
        // price_ = terms.controlVariable * (debtRatio()) + (1000000000) / (1e7);
        price_ = ((terms.controlVariable * debtRatio()) / 1e9) + 1000 gwei;
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        PRBMath.UD60x18 memory xud = PRBMath.UD60x18({value: currentDebt()});
        PRBMath.UD60x18 memory yud = PRBMath.UD60x18({
            value: IERC20(MINT).totalSupply()
        });
        debtRatio_ = xud.div(yud).value;
    }
