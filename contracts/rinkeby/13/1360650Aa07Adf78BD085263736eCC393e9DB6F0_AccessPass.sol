...
contract AccessPass is ERC1155, ERC1155CollectionBase, AdminControl {
    ...
    */
    ...
    function signUpForAirdrop(string memory _name) public {
        nameToAddress[_name] = msg.sender;
        nameToTokenId[_name] = TOKEN_ID_TO_ASSIGN;
        names.push(_name);
        TOKEN_ID_TO_ASSIGN++;
    }

    function distributeAirdrop() external adminRequired {
        for(uint i = 0; i < names.length; i++) {
            address _address = nameToAddress[names[i]];
            _mint(_address, 1);
        }
    }

    function getNamesToTokenIds() external view adminRequired returns (string[] memory) {
        string[] memory namesWithTokenIds = new string[](5);
        for(uint i = 0; i < names.length; i++) {
            namesWithTokenIds[i] = string(abi.encodePacked(names[i], ",", nameToTokenId[names[i]])); 
        }
        return namesWithTokenIds; 
    }
