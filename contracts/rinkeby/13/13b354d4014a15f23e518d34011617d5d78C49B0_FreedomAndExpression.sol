...
contract FreedomAndExpression is ERC721, RaribleRoyaltyV2, Ownable {
  ...
  uint256 private constant UNIQUE_PRICE = 0.0007 ether;
  uint256 private constant EDITION_PRICE = 0.00007 ether;

  uint256 private constant EDITION_SUPPLY = 20;
  uint256 private constant UNIQUE_COUNT = 5;

  address private constant ROYALTY_RECEIVER = 0x6A4c66e953087d8357b6c09FCbAF84b98D6Fe692;

  address private momintousWallet = 0x6A4c66e953087d8357b6c09FCbAF84b98D6Fe692;
  address private artistWallet = 0x6A4c66e953087d8357b6c09FCbAF84b98D6Fe692;

  //  EDITION_FLOWER_CHILD = 0;
  //  EDITION_MY_PLEASURE = 1;
  //  EDITION_PHANTOM_OF_THE_OPERA = 2;
  //  EDITION_POETRY_IN_MOTION = 3;
  //  EDITION_HURRICANE = 4;
  ...
  constructor() ERC721("FreedomAndExpression", "RSFE") {
    _setDefaultRoyalty(ROYALTY_RECEIVER, 1000);
    totalSupply = 0;
  }

  function rareMint(uint256 tokenId) public payable {
    require(tokenId <= UNIQUE_COUNT && tokenId > 0, "tokenId must be between 1 and 5");
    require(_owners[tokenId] == address(0), "That token was already minted");
    require(msg.value == UNIQUE_PRICE, "You must send the right amount");
  ...
  function mint(uint256 editionId) public payable {
    require(editionId < UNIQUE_COUNT, "That edition doesn't exist");
    require(editionSupply[editionId] < EDITION_SUPPLY, "That edition was minted out");
    require(msg.value == EDITION_PRICE, "You must send the right amount");
    uint256 tokenId = UNIQUE_COUNT + editionId * 20 + editionSupply[editionId] + 1;
    editionSupply[editionId]++;
  ...
  function remainingSupply(uint256 editionId) public view returns (uint256) {
    return EDITION_SUPPLY - editionSupply[editionId];
  }
  ...
  function setMomintousWallet(address _wallet) public onlyOwner {
    momintousWallet = _wallet;
  }

  function setArtistWallet(address _wallet) public onlyOwner {
  ...
  function withdraw() public {
    ...
    uint256 cut = balance * 15 / 100;
    uint256 payment = balance * 85 / 100;
    (bool cutSuccess, ) = momintousWallet.call{value: cut}("");
    require(cutSuccess, "Could not process payment");
    (bool paymentSuccess, ) = artistWallet.call{value: payment}("");
    require(paymentSuccess, "Could not process payment");
    emit Withdrawal(momintousWallet, cut, block.timestamp);
    emit Withdrawal(artistWallet, payment, block.timestamp);
  }
}
