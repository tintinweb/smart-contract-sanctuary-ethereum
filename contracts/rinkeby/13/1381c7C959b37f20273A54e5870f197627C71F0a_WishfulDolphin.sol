...
contract WishfulDolphin is ERC721Tradable, ReentrancyGuard {
    ...
    function updateMaxPreSaleMints(uint256 amount) external onlyOwner {
        maxPreSaleMints = amount;
    }
    ...
    function mintNFT(
        ...
        require( preSalePaused == false, "Pre-Sale is not active at the moment.");
        ...
        require( numTokens > 0 && numTokens <= maxPreSaleMints, "Invalid pre-sale mint amount.");
        require( totalMints[msg.sender] + numTokens <= maxPreSaleMints, "Address exceeds max pre-sale allowed mints.");
    ...
    function mintNFT(uint256 numTokens) external payable nonReentrant isSaleOpen {
        require( salePaused == false, "Public Sale is not active at the moment.");
        require( totalSupply() + numTokens <= maxSupply, "Exceeds maximum NFT supply.");
