...
contract JumpRateModelV3 is Initializable, InterestRateModel {
    ...
    function calc(uint a, uint b) public pure returns(int) {
        return int(a.div(b));
    }

    function getRateChangeLess(int utilErr, uint elapsedBlocks, uint lastInterestRate) public view returns(int interestRateChange, uint interestRate) {
        interestRateChange = utilErr * int(gainPerBlock) / 1e18;

        interestRate = interestRateChange > 0 ? lastInterestRate.add(elapsedBlocks.mul(uint(interestRateChange))) : 0;
    }
    ...
    function getBorrowRateInternal(uint cash, uint borrows, uint reserves) internal view returns (uint) { 
        ...
        uint lastCalcBorrowRateBlock = BTokenInterfaces(bLendingToken).accrualBlockNumber();
        uint elapsedBlocks = getBlockNumber().sub(lastCalcBorrowRateBlock);
        uint lastInterestRate = BTokenInterfaces(bLendingToken).borrowIndex();
    ...
    function updateJumpRateModelInternal(uint gainPerYear, uint jumGainPerYear, uint targetUtil_) internal {
        gainPerBlock = (gainPerYear.mul(1e18)).div(blocksPerYear.mul(targetUtil_)); 
