...
contract FHReward is ERC20, Ownable {

    iFootHulk public FootHulkz;
    ...
    constructor(address FootHulkAddress) ERC20("FHReward", "$FHR") {
        FootHulkz = iFootHulk(FootHulkAddress);
        START = block.timestamp;
    }

    function updateReward(address from, address to) external {
        require(msg.sender == address(FootHulkz));
        if(from != address(0)){
    ...
    function startReward(address to) external {
        lastUpdate[to] = block.timestamp;
    }
    ...
    function burn(address user, uint256 amount) external {
        require(allowedAddresses[msg.sender] || msg.sender == address(FootHulkz), "Address does not have permission to burn");
        _burn(user, amount);
    }
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return FootHulkz.balanceGenesis(user) * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 86400;
    }
