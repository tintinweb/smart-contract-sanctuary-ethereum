...
contract Donation
{
  ...
  function  donation()external payable
  {
   ...
   function updateManager(string memory _fullname, string memory _contactNo) public onlyOwner 
   {
    ...
    function NewIncharge(address[] memory whiteAddress,string memory _firstname,string memory _lastname,string memory _UC_Name,string memory _Email,string memory _contactNo,uint _CNIC) public onlyOwner
    {
    ...
    function RequestForBlood(uint _CNIC,string memory _firstname,string memory _lastname,string memory _email,string memory _bloodtype,string memory _disease,string memory _adress,string memory _contact_No, string memory _status,uint _NoOfBottles,string memory _dateOfapprove )public
    {
      require( _addressExist[msg.sender]=true);
      uint count;
      count=requests.length;
      requesterid[_CNIC]=RequestBlood(_CNIC,_firstname, _lastname, _email,_bloodtype,_disease,_adress,_contact_No, _status,_NoOfBottles,_dateOfapprove,count+1);
      Inchargeadr_requester[msg.sender][count+1]=RequestBlood(_CNIC,_firstname, _lastname, _email,_bloodtype,_disease,_adress,_contact_No, _status,_NoOfBottles,_dateOfapprove,count+1);
    ...
    //   request.dateOfrequest=_dateOfrequest;
   ...
   //Donate blood
   function Donateblood(uint _CNIC,string memory _firstname,string memory _lastname,string memory _bloodtype,string memory _adress,string memory _contact_No,uint _NoOfBottles,string memory _dateOfDonation)public
   {
     require( _addressExist[msg.sender]=true);
     uint count;
