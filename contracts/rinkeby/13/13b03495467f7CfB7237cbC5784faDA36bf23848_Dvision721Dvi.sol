...
contract Dvision721Dvi is ERC721, ERC721Holder, AccessControl, ReentrancyGuard
{
    ...
    function Sell_Item(address _marketaddress, uint _tokenId, uint256 _price, uint8 _currency) external
    {
    ...
    function Transaction_Item(address _to, uint _tokenId) nonReentrant external payable {
        ...
        {
            owner.transfer(_items[_tokenId].price * (100 - fee_percent) / 100);
    ...
    //_to : buyer
    function Transaction_Item_WithToken(address _to, uint _tokenId, uint256 _price) nonReentrant external
    {
    ...
    function TokensOfOwner(address _owner) external view returns(uint[] memory)
    {
        ...
        else
        {
            ...
            uint256 _tokenId;
            for(_tokenId = 0; _tokenId < _balance; _tokenId++)
            {
                _result[_tokenId] = tokenOfOwnerByIndex(_owner, _tokenId);
            }
    ...
    function Get_Items(uint _tokenId) external view returns(uint _id, uint256 _price, uint8 _currency, bool _forSale, address _owner)
    {
        _id = _tokenId;
    ...
    function Get_SellingItems() external view returns(uint[] memory, uint[] memory, uint[] memory)
    {
    ...
    function Set_BaseURI(string memory _new) onlyManager external returns(string memory)
    {
        emit URIChange(_new);
