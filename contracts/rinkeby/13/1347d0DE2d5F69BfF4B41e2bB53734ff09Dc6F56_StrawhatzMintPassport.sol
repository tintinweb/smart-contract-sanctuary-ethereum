...
contract StrawhatzMintPassport is ERC1155, IERC2981, Ownable {
    ...
    {
        return (_royaltyRecipient, (salePrice * 1000) / 10000);
    }
    ...
    constructor(string memory newBaseTokenURI) ERC1155("") Ownable() {
    ...
    function burn(
        ...
        require(balanceOf(who, tokenId) > 0, PASSPORT_EMPTY);
        require(balanceOf(who, tokenId) >= count, NOT_ENOUGH_PASSPORTS);

        _burn(who, tokenId, count);
    }

    function setNftAddress(address contractAddress) external onlyOwner {
        nftAddress = contractAddress;
    }

    function changePresale(bool enabled) external onlyOwner {
        presaleEnabled = enabled;
    }

    function changePublicSale(bool enabled) external onlyOwner {
    ...
    function addToWhitelist(address newAddress) external onlyOwner {
        whitelist[newAddress] = MAX_BUY_COUNT_PER_USER;
    }

    function addBatchToWhitelist(address[] memory newAddresses) external onlyOwner {
        for (uint256 i = 0; i < newAddresses.length; i++) {
            whitelist[newAddresses[i]] = MAX_BUY_COUNT_PER_USER;
        }
    }
     ...
     */
    function giveawayPresale(address to, uint256 count) external onlyOwner returns (uint256 tokenId) {
        tokenId = PRESALE_TOKEN_ID;
        _mint(to, tokenId, count);
    }

    modifier buyForEther(uint256 count, uint256 price) {
        require(count > 0, MINT_ZERO_TOKENS);
        require(msg.value == count * price, PAYMENT_AMOUNT_INCORRECT);
        _;
    }
     ...
     */
    function buyPresale(uint256 buyCount)
        external
        payable
        buyForEther(buyCount, PRESALE_TOKEN_PRICE)
        returns (uint256 _tokenId)
    {
        ...
        require(balanceOf(msg.sender, PRESALE_TOKEN_ID) + buyCount <= whitelist[msg.sender], PASS_USED_UP);

        _tokenId = PRESALE_TOKEN_ID;
        _mint(msg.sender, _tokenId, buyCount);
    }

    /**
     * @notice mint public passport for ether
     */
    function buyPublic(uint256 buyCount)
        external
        payable
        buyForEther(buyCount, PUBLIC_TOKEN_PRICE)
        returns (uint256 _tokenId)
    {
