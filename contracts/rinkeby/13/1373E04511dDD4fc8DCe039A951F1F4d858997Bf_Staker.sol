...
contract Staker {
  ...
  error InsufficientBalance(uint requested, uint available);
    ...
    modifier notCompleted() {
        ...
        require(!completed, "Staking has been completed");
        _;
    }

    modifier deadlineElapsed(bool requireElapsed) {
        uint timeRemaining = timeLeft();
        if(requireElapsed) {
          require(timeRemaining == 0, "Deadline not elapsed yet");
        } else {
          require(timeRemaining > 0, "Deadline elapsed");
        }
        _;
    }

  function stake() public payable notCompleted deadlineElapsed(false) {  
  ...
  function execute() public notCompleted deadlineElapsed(true) {
    uint256 contractBalance = address(this).balance;
    require(contractBalance >= threshold, "Balance did not reach threshold");
  ...
  function withdraw() public notCompleted deadlineElapsed(true) {
    uint userBalance = balances[msg.sender];
    uint256 contractBalance = address(this).balance;

    //check if user has enough balance
    require(userBalance > 0, "You don't have enough balance to withdraw");
    require(contractBalance < threshold, "Balance exceeded threshold");
