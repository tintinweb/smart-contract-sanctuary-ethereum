...
contract MyBoatsCollection is ERC721, Ownable {
  ...
  modifier mintCompliance(uint256 _mintAmount) {
    // require(_mintAmount > 0 && _mintAmount <= maxMintAmountPerTx, "Invalid mint amount!"); // Any amount can be minted as long as less then current max supply
    require(supply.current() + _mintAmount <= currentMaxSupply, "Current Max supply exceeded!");
    _;
  }
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    //require(!whitelistClaimed[_msgSender()], "Address already claimed!"); // One wallet can only mint once, remove if more than one allowed
  ...
  // Update the hidden metadata URI, if needing to change
