...
contract SniperNFT is ERC721Enumerable, Ownable {
    ...
    uint256 public SNIPER_SUPPLY = 0;
    uint256 public GOLD_SNIPER_SUPPLY = 0;

    uint256 public MAX_SNIPERS = 930;
    uint256 public MAX_GOLD_SNIPERS = 69;

    uint256 public SNIPER_PRICE = 0 ether;
    uint256 public GOLD_SNIPER_PRICE = .1 ether;

    uint256 public feePaid = 0;

    // bytes
    bytes32 merkleRoot;

    string private _tokenBaseURI = 'ipfs://bafybeifasux4tvdyjdtafiyfmngidhkkkjh6tuqsc74wvpfdjzcthur4ya/metadata/sniper';
    string private _tokenGoldURI = 'ipfs://bafybeifasux4tvdyjdtafiyfmngidhkkkjh6tuqsc74wvpfdjzcthur4ya/metadata/gold_sniper';

    //Mappings
    mapping(uint256 => bool) private _tokenIsGold;
    mapping(address => uint256) private _tokenClaimed;

    constructor(bytes32 _merkleRoot) ERC721("SniperTester", "SNIPERTESTER") {
        _owner = msg.sender;
    ...
    */
     ...
     * @dev Mint internal, this is to avoid code duplication.
     */
    function mint(bool isGold, bytes32[] calldata proof) payable public returns(uint256 tokenId) {
        ...
        if(isGold){
            ...
            require(SNIPER_SUPPLY < MAX_SNIPERS, "All snipers have been minted");
            require(msg.value == SNIPER_PRICE, "Wrong amount of ether");
        }

        tokenId = mintInternal(isGold);
        _tokenClaimed[msg.sender] += 1;
        return tokenId;
    }
