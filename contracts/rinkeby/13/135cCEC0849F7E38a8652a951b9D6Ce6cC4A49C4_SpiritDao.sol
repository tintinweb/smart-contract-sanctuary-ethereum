...
contract SpiritDao is Ownable, ERC721A, ReentrancyGuard {
    ...
    // mapping of amount minted per round
    mapping(address => uint256) salesRoundMinted;

    event Minted(address to, uint256 quantity);

    constructor() ERC721A("Azuki Spirit Dao", "SPIRITDAO") {}
    ...
    function getSalePrice() public view returns (uint256) {
        require(saleOpen, "Sale is not open.");
        return salesConfig.mintPrice;
    }

    function getSalesConfig() external view returns (SalesConfig memory) {
        return salesConfig;
    }
     ...
     */
    function getMintableTokensForWallet(
        address walletAddress,
        bytes32[] calldata merkleProof
    ) public view returns (uint32) {
        if (!isValidMerkleProof(merkleProof)) {
            return 0;
        }

        if (
            allowListMinted[salesRound][walletAddress] <=
            salesConfig.maxPerWallet
        ) {
            return
                uint32(
                    salesConfig.maxPerWallet -
