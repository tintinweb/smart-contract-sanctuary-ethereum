...
contract HolaSpeaker is ERC1155, Ownable {
    ...
    function mintToPerson(address _address, uint _tokenId) external onlyOwner {
      //   require(_amount > 0, "Amount must be greater than zer0");
            _mint(_address, _tokenId, 1, "");
    }

    function mintNFT(uint256 _tokenId) external onlyOwner {
     //   require(_amount > 0, "Amount must be greater than zer0");
            _mint(msg.sender, _tokenId, 1, "");
    }

    function name() public pure returns (string memory) {
        return "Hola Metaverso Speaker";
    }
