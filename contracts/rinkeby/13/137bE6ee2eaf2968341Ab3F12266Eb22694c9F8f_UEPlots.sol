...
contract UEPlots is ERC721Enumerable, MultiOwnable, ReentrancyGuard {
	...
	mapping(address => address) _prevWhitelistMember;
	uint256 listSize = 0;
	address constant GUARD = address(1);

	string public uriPrefix = 'https://metadata.unrealestates.io/';
	...
	constructor(
		...
		_prevWhitelistMember[GUARD] = GUARD;
	}
	...
	function getFromWhitelist(address add) public view returns (uint256) {
		...
		onlyOwner
	{
		...
		for (uint256 i = 0; i < adds.length; i++) {
			...
			whitelistClaimed[adds[i]] = false;

			_nextWhitelistMember[adds[i]] = _nextWhitelistMember[GUARD];
			_prevWhitelistMember[_nextWhitelistMember[GUARD]] = adds[i];

			_nextWhitelistMember[GUARD] = adds[i];
			_prevWhitelistMember[adds[i]] = GUARD;
	...
	function removeFromWhitelist(address[] memory adds) public onlyOwner {
		...
		for (uint256 i = 0; i < adds.length; i++) {
			delete whitelist[adds[i]];
			delete whitelistClaimed[adds[i]];

			address right = _nextWhitelistMember[adds[i]];
			address left = _prevWhitelistMember[adds[i]];
			_nextWhitelistMember[left] = right;
			_prevWhitelistMember[right] = left;

			listSize--;

			delete _nextWhitelistMember[adds[i]];
			delete _prevWhitelistMember[adds[i]];
		}
	}
	...
	function emptyWhitelist() public onlyOwner {
		...
		for (uint256 i = 0; currentAddress != GUARD; i++) {
			...
			delete _prevWhitelistMember[currentAddress];
			currentAddress = _nextWhitelistMember[currentAddress];
		}
	...
	function whitelistMint(address _receiver) internal {
		...
		require(whitelist[_msgSender()] > 0, 'Address not in whitelist!');
		require(whitelist[_msgSender()] > 10, 'Address does not have a plot associated with it!');
		whitelistClaimed[_msgSender()] = true;

		_safeMint(_receiver, whitelist[_msgSender()]);
	}
	...
	function mintSpecific(uint256 tokenID)
		...
		mintPriceCompliance
	{
		// make sure normal mint does not work with universcal mint
