...
contract HyewonNft is IHyewonNft, ERC721A, Ownable, ReentrancyGuard, Pausable {
    ...
    ) ERC721A("HyewonBookNft", "HyewonBook") Ownable() Pausable() {
        _receiver = receiver;
        ...
        returns (string memory)
    {
        ...
        uint16 roundNumber = round.roundNumber;
        ...
        Round memory prevRound = rounds[roundNumber - 1];

        // Round robin from randomSelection number
        uint256 diff = round.randomSelection - prevRound.lastMintedId + 1;
        uint256 returnId = tokenId + diff;

        if (returnId > round.lastMintedId) {
            returnId -= prevRound.lastMintedId;
        }
