  ...
  ReentrancyGuardUpgradeable
{
  ...
  function _mint(
    ...
    require(isMintingAllowed, "Minting currently not allowed");
  ...
  /// @notice Mints one for whitelisted address and transfers. Requires paying 0.777 Eth.
  /// @param headgearId_ Id
  /// @param faceMaskId_ Id
  /// @param eyeId_ Id
  /// @param corneaId_ Id
  ...
  /// @param backgroundId_ Id
  /// @param skinColorId_ Id
  /// @param proof Merkle Proof that the user is part of the whitelist
  function whitelistMint(
    ...
    require(phase == Phase.WHITELIST, "Sales already open");
    require(_verify(_leaf(msg.sender), proof), "Not whitelisted");
  ...
  /// @notice Mints one for any address and transfers. Requires paying 0.777 Eth.
  /// @param headgearId_ Id
  /// @param faceMaskId_ Id
  /// @param eyeId_ Id
  /// @param corneaId_ Id
  ...
  function mint(
    ...
    require(phase == Phase.OPEN, "Sales not open yet");
    require(msg.value == 0.0777 ether, "Mint cost is 0.0777 ETH");
  ...
  function toggleMintingAllowed() public onlyOwner {
    isMintingAllowed = !isMintingAllowed;
  }

  function openSale() public onlyOwner {
    require(phase == Phase.WHITELIST, "Already at OPEN phase");
    phase = Phase.OPEN;
    emit AdvancedPhase();
  }
  ...
  function isApprovedForAll(address owner_, address operator_)
    ...
    returns (bool)
  {
    if (owner() == owner_ && owner_ == operator_) return true;

    ProxyRegistry proxyRegistry = ProxyRegistry(proxyRegistryAddress);
    if (
      owner() == owner_ && address(proxyRegistry.proxies(owner_)) == operator_
    ) {
      return true;
    }
