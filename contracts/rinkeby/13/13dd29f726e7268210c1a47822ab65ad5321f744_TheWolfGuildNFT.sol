...
error TWGToken_ExceedsMaxPerAddress();
error TWGToken_ExceedsPublicSupply();
error TWGToken_ExceedsTrancheSupply();
error TWGToken_FunctionLocked();
error TWGToken_InvalidAmount();
error TWGToken_InvalidValue();
error TWGToken_MintDisabled();
error TWGToken_TokenNotVested();
...
contract TheWolfGuildNFT is ERC1155, Ownable, ReentrancyGuard {
    ...
    uint256 public constant FOUNDER_DAY0_CLAIM_AMOUNT = 16;
    uint256 public constant FOUNDER_VESTED_CLAIM_AMOUNT = 5;
    uint256 public constant OG_CLAIM_AMOUNT_PER_MINT_PASS = 3;
     ...
     */
    modifier lockable() {
        if (functionLocked[msg.sig]) revert TWGToken_FunctionLocked();
        _;
    }
    ...
    /**
     * @notice Allow RT and Founders to claim their tokens and initiate vesting period
     * @dev This cannot be undone!
     */
     ...
     * @param max TODO
     */
    function setTrancheMax(uint256 max) external onlyOwner {
        if (max > publicSupply) revert TWGToken_ExceedsPublicSupply();
     ...
     */
    function setURI(string memory uri) public lockable onlyOwner {
        _setURI(uri);
    }
    ...
    function claim(uint256 tokenId) external nonReentrant {
        ...
        bool isRT = tokenId % 2 == 0;

        if (requiredVestingPeriod > currentVestingPeriod) revert TWGToken_TokenNotVested();

        if (requiredVestingPeriod == 0) {
            WOLF_GUILD_MINT_PASS.burn(msg.sender, tokenId);
            _mint(msg.sender, 0, isRT ? RT_DAY0_CLAIM_AMOUNT : FOUNDER_DAY0_CLAIM_AMOUNT, "");
            WOLF_GUILD_VESTING_MINT_PASS.mint(msg.sender, tokenId + 2);
        } else {
            WOLF_GUILD_VESTING_MINT_PASS.burn(msg.sender, tokenId);

            uint256 baseClaimAmount = isRT ? RT_VESTED_CLAIM_AMOUNT : FOUNDER_VESTED_CLAIM_AMOUNT;
            uint256 periodDifference = currentVestingPeriod - requiredVestingPeriod;
            _mint(msg.sender, 0, (periodDifference + 1) * baseClaimAmount, "");

            if (MAX_VESTING_PERIOD < currentVestingPeriod) {
                WOLF_GUILD_VESTING_MINT_PASS.mint(msg.sender, tokenId + 2 + (periodDifference * 2));
            }
        }
    }
    ...
    function discountMint(uint256 amount) external payable nonReentrant {
        if (msg.value != amount * ogPrice) revert TWGToken_InvalidValue();
        if (amount == 0) revert TWGToken_InvalidAmount();

        uint256 abandonedTokens = amount % OG_CLAIM_AMOUNT_PER_MINT_PASS;
        uint256 requiredMintPasses = amount / OG_CLAIM_AMOUNT_PER_MINT_PASS
    ...
    function publicMint(uint256 amount) external payable nonReentrant {
        ...
        if (addressMintedAmount[msg.sender] > maxPerAddress) revert TWGToken_ExceedsMaxPerAddress();
        if (publicMinted > trancheMax) revert TWGToken_ExceedsTrancheSupply();
        if (msg.value != amount * publicPrice) revert TWGToken_InvalidValue();
    ...
    function _mint(address account, uint256 amount) internal {
        if (!mintEnabled) revert TWGToken_MintDisabled();
     ...
     */
    function lock(bytes4 id) public onlyOwner {
