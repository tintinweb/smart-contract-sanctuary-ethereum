...
contract goldenEggMarketRunMain is ERC721Upgradeable {
    ...
    function cancelTheSale(
        ...
        egg.tokenId = _tokenId;
        egg.currentOwner = msg.sender;
        egg.forSale = false;
        egg.forSalLog = block.timestamp;
        allGoldenEggRun[_tokenId] = egg;
        //Transfer token to administrator
        _transfer(tokenOwner, _royaltiesAddr, _tokenId);
    ...
    function buyToken(uint256 _tokenId) public {
        ...
        //beans to the seller
        if (payOwnerAmount > 0) be.transferFrom(msg.sender, egg.currentOwner, payOwnerAmount);
