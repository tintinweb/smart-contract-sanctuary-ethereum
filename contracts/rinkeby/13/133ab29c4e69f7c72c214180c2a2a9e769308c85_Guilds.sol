...
contract Guilds is ERC1155Supply {
    ...
    function lockSpots(uint256 _tokenId) public onlyRole(ADMIN, _tokenId) {
    ...
    function setNewGuild(uint256 _id, address _initiator, string memory _name, string memory _desc, string memory _guildType) private {
        ...
        AllGuilds[_id].FreezeMetaData = true;
    ...
    function setNewTokenUri(uint256 _id, string memory _uri) public onlyOwner {
        require(!AllGuilds[_id].FreezeMetaData, "Guild Master must approve metadata changes");
        tokenURI[_id] = _uri;
        emit URI(_uri, _id);
    }
