...
/**
 * it's the remix to guidition
 * hot and fresh out the kitchen
 */
contract Guidition is AdminControl, ICreatorExtensionTokenURI, ILazyDelivery, ILazyDeliveryMetadata {
    address private _creator;
    address private _marketplace;
    uint private _listingId1;
    uint private _listingId2;
    uint private _listingId3;
    ...
    function setListing(uint listingId1, uint listingId2, uint listingId3, address marketplace, uint tokenIdToMint) public adminRequired {
        _listingId1 = listingId1;
        _listingId2 = listingId2;
        _listingId3 = listingId3;
    ...
    function deliver(address, uint256 listingId, uint256, address to, uint256, uint256) external override returns(uint256) {
        require(msg.sender == _marketplace &&
                    (listingId == _listingId1 || listingId == _listingId2 || listingId == _listingId3),
            "Invalid call data");
        ...
        if (listingId == _listingId1) {
            require(mintCount[to] + 1 <= 3, "Max 3 per wallet");
            amounts[0] = 1;
        }

        if (listingId == _listingId2) {
            require(mintCount[to] + 2 <= 3, "Max 3 per wallet");
            amounts[0] = 2;
        }

        if (listingId == _listingId3) {
            require(mintCount[to] + 3 <= 3, "Max 3 per wallet");
            amounts[0] = 3;
        }

        mintCount[to] += amounts[0];
