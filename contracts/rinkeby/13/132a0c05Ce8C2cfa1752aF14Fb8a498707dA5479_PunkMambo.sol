...
contract PunkMambo is ERC721A, Pausable, AccessControl, EIP712 {
    ...
    function mint(
        ...
        bytes32 hash = _hashMint(msg.sender, amount, maxPerWallet, nonce);
        require(_verify(hash, signature), "INVALID_SIGNATURE");
        require(_hashes[hash] == address(0), "HASH_ALREADY_USED");
        require(_mintSignatures[msg.sender].length + amount <= maxPerWallet, "EXCEED_QUOTA");

        //Reserve a spot for holders of the token
        uint256 reserved = amount + totalSupply() + (_mintPasses - _redeemedCounter.current());
        console.log("Reserved: %d, maxItems: %d", reserved, _maxItems);
        require(reserved <= _maxItems, "TOTAL_SUPPLY");
    ...
    function _hashMint(
        ...
        _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "NFT(address account,uint256 amount,uint256 maxPerWallet,uint256 nonce)"
                    ),
                    account,
                    amount,
                    maxPerWallet,
