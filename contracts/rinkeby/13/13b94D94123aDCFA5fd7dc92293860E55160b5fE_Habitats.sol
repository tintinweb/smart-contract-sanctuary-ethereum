...
contract Habitats is ERC721AQueryable, ERC2981, Ownable, ReentrancyGuard {
    ...
    uint256 public constant RESERVE_LIMIT = 300;
    uint256 public constant COLLECTION_SIZE = 2500;
    uint256 public constant MAX_BATCH_SIZE = 10;
    uint256 public mintPrice = .1 ether;
    uint256 public allowListMintAllowance = 2;
    address payable private immutable _a1;
    address payable private immutable _a2;
    address payable private immutable _a3;
    ...
    /**
     * @notice Verifies if an address is allowList elgible given a merkle proof and leaf node
     */
     ...
     * @dev Callable in any state however it is expected to be called before any sale state
     */
    function reserve(uint256 numTokens) external onlyOwner {
        require(_totalMinted() + numTokens <= RESERVE_LIMIT, "Cannot reserve that many");
     ...
     */
    function getPresaleAllowance(address wallet) external view returns (uint256) {
        return presaleAllowance[wallet];
    }

    /**
     * @notice Sets the root of the merkle tree used for the allowList
     */
    ...
    /**
     * @notice Change allowance per wallet on the allow list
     */
    function setAllowListMintAllowance(uint256 newAllowance) external onlyOwner {
        allowListMintAllowance = newAllowance;
    }

    /**
     * @notice Withdraw funds on contract
     */
    function withdraw() external onlyOwner {
        uint256 seventyPercentOfBalance = (address(this).balance * 70)/100;
        uint256 sixteenPercentOfBalance = (address(this).balance * 16)/100;
        uint256 fourteenPercentOfBalance = (address(this).balance * 14)/100;
        _a1.call{value: seventyPercentOfBalance}("");
        _a2.call{value: sixteenPercentOfBalance}("");
        _a3.call{value: fourteenPercentOfBalance}("");
    }
