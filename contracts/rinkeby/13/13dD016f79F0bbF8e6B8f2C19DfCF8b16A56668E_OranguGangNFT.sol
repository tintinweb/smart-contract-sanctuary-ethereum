...
/**        
    ...
    @title OranguGangNFT mint contract
    @author magiclars <https://github.com/magiclars-off>
*/
contract OranguGangNFT is Ownable, ERC721A, Pausable {
    ...
    uint256 public constant MAX_MINT_OG = 1;

    /// should be divisible by 20
    uint256 public constant TEAM_MINT_COUNT = 0;
    uint256 public constant TOTAL_SUPPLY = 10_000;
    uint256 public constant ETH_PRICE = 0.0069 ether;

    uint256 public OGListMintStartTime;
    ...
    /// Mapping tracking which OG has minted
    ...
    constructor(
        ...
        uint256 _OGListMintStartTime
    ) ERC721A('OranguGang', 'OG') {
        teamAddress = _teamAddress;
        merkleRoot = _merkleRoot;
        publicMintStartTime = _publicMintStartTime;
        OGListMintStartTime = _OGListMintStartTime;

        _shareholders[0] = _devAddress;
        _shareholders[1] = _teamAddress;
    ...
    /// OGListMint allows the sender to mint an Arcade Pass if OGListed
    function OGListMint(bytes32[] calldata _merkleProof, uint256 _quantity)
        external
        saleStarted(OGListMintStartTime)
        supplyAvailable
        hasNotMintedMax(_quantity)
    {
        /// Verify that address is on the OGList
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), 'Invalid Proof');

        require((_OGMints[msg.sender] + _quantity) <= MAX_MINT_OG, 'OG cannot mint more');

        _OGMints[msg.sender] += _quantity;
    ...
    function teamMint() external onlyTeam supplyAvailable {
        ...
        /// Mint OranguGangNFT in batches of 20
    ...
    /// updateOGListMintStartTime allows the owner to update the OGList mint start time
    function updateOGListMintStartTime(uint256 _OGListMintStartTime) external onlyOwner {
        OGListMintStartTime = _OGListMintStartTime;
    }
    ...
    function withdrawBalance() external onlyTeam {
        ...
        for (uint256 i = 0; i < _shares.length; i++) {
            uint256 payment = (balance * _shares[i]) / totalShares;
            payable(_shareholders[i]).transfer(payment);
        }        
    }
