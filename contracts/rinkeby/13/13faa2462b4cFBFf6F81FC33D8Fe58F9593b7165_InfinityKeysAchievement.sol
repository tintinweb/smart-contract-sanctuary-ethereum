    ...
    CheckExternalNFT
{
    ...
    */
    function addToken(
        ...
        t.gate = _gateState;
        t.internalGateID = _internalGateID;
    ...
    */
    function addToken( bool _claimable, GateState _gateState, string memory _tokenURI ) public onlyAuthorized {
        addToken(_claimable, _gateState, 0, address(0), _tokenURI);
    }   
    ...
    */
    function gateCheck ( uint256 _tokenID, address _address ) private view returns ( bool ) {
        ...
        if (gate == GateState.noGate) {
            ...
            return (balanceOf(_address, tokens[_tokenID].internalGateID) > 0);
        } else if (gate == GateState.externalGate) {
            return checkNFT(_address, tokens[_tokenID].externalGateContract);
        }
        return false;
    }
    ...
    */
    ...
    /**
    @dev onlyOwner- release ETH to given address
    */
    ...
    /**
    @dev onlyOwner- release given ERC20 to given address
    */
...
* @notice: Check if address owns external NFT               *
*************************************************************/
...
contract CheckExternalNFT {
    /**
    @dev Checks if specified address owns NFT on specified contract 
    */
    function checkNFT ( address _address, address _contract ) internal view returns ( bool ) {
    ...
    */
    function traverseChain(uint16 _chainID, uint _tokenID) external payable {
        ...
        require(
            ...
            "TraverseChain: Destination blockhain can't be the same as source"
        );
