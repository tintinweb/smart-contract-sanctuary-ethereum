...
contract CityTour is Ownable, ERC721A {
    ...
    uint public salesStatus = 0; // 0 Sales paused, 1 WL, 2 Public

    // white list claim mapping
    mapping(address => uint) private inkedClaimed;

    // wl proof
    bytes32 private wlProof = 0x431971a89138a952323d511c2e42f4d15ffdda4bcdcd2996d4d27a1732b2330d; 

    // cashout wallets
    address private gc = 0x7811f901b82fb506399ca016EcA41539AaD6435e;
    address private yt = 0x7811f901b82fb506399ca016EcA41539AaD6435e;
    address private el = 0x7811f901b82fb506399ca016EcA41539AaD6435e;
    address private com = 0x7811f901b82fb506399ca016EcA41539AaD6435e;

    constructor() ERC721A("CityTourBestOffer", "CTBO"){
        

        // mint first to company
        _safeMint(0x546FAB59Cd271aAC95c9280a23F398e03bAc2765,1);
    }
   ...
   function buy(uint _count) public payable{
        ...
        require(salesStatus == 2, "Sale is Paused.");
        // check token count
        require(_count > 0, "Mint at least one token");
        // check transaction limit
        require(_count <= walletLimit, "Per mint limit reached!");
        // check token supply
        require(totalMinted + devReserve + _count <= collectionSize, "Sold Out");
        // check wallet limit
        require(inkedClaimed[msg.sender] < walletLimit, "Pre mint limit reached!");
        // check eth amount
    ...
    function buyPreMint(bytes32[] calldata _merkelProof, uint _count) public payable {
        // check wl mint
        require(salesStatus == 1, "Wait for wl open");
        // check wl wallet limit
        require(inkedClaimed[msg.sender] < walletLimit, "Per mint limit reached!");
        // sender hex
        ...
        require(_count > 0, "mint at least one token");
        // check transaction limit
        require(_count <= walletLimit, "max allowed token limit reached");
        // check token supply
        require(totalMinted + devReserve + _count <= collectionSize, "Sold Out");
        // check eth amount
        require(msg.value >= tokenPrice * _count, "Incorrect ether amount");
    ...
    // buy premint
    function buyMint(bytes32[] calldata _merkelProof, uint _count) public  {
        // check sales status
        require(salesStatus == 3, "Wait for wl open");
        // check wallet limit
        require(inkedClaimed[msg.sender] < walletLimit, "Per mint limit reached!");
        // sender hex
        bytes32 senderHex = keccak256(abi.encodePacked(msg.sender)); 
