...
contract DaoEvmosFactory {
  ...
  function createDAO(
      ...
      DaoEvmos dao = new DaoEvmos(name, symbol, signer, signerVerification, _commonURI, _rareURI, _legendaryURI);
...
contract DaoEvmos is ERC721{
   ...
   enum VotingStatus {
    ...
    string commonBaseURI;
    string rareBaseURI;
    string legendaryBaseURI;
    ...
    constructor(
      ...
      string memory _legendaryURI
        ...
        commonBaseURI = _commonURI;
        rareBaseURI = _rareURI;
        legendaryBaseURI = _legendaryURI;
    } 

    function mintNft(RarityNFT _tokenRarity, bytes memory signature) external {
      require(signerVerification.isMessageVerified(signer, signature, _concatParams(_tokenRarity, msg.sender)), 'Incorrect rarity');
    ...
    function vote(uint256 votingId, bool voteType) public {
      require(balanceOf(msg.sender) != 0, "You cant't participate in this DAO");
      require(votes[votingId].status == VotingStatus.PROCESSING, "Voting already ended");
      require(!votesInfo[votingId].hasVote[msg.sender], "You already vote");
      ...
      if(rarityForNft[tokenForOwner[msg.sender]] == RarityNFT.LOW){
          voteCount = 1;
      }else if(rarityForNft[tokenForOwner[msg.sender]] == RarityNFT.MEDIUM){
          voteCount = 2;
      }else{
          voteCount = 3;
      }
      ...
      if(voteType){
        voteInfo.positiveVote += voteCount;

      }else{
          voteInfo.negativeVote += voteCount;
      }


      voteInfo.hasVote[msg.sender] = true;

      emit Voted(msg.sender, address(this), votingId, voteCount, voteType);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(rarityForNft[tokenId] == RarityNFT.LOW){
          return commonBaseURI;
        }else if(rarityForNft[tokenId] == RarityNFT.MEDIUM){
          return rareBaseURI;
        }else{
          return legendaryBaseURI;
        }
	}
  ...
  event Voted(address user, address dao, uint256 votingId, uint256 voteCount, bool isPositiveVote);
