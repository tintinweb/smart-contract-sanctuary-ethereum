...
contract VOLTAMA is Context, IERC20, Ownable {
    ...
    string private _name = "VOLTAMA";
    string private _symbol = "VOLTAMA";
    ...
    uint256 public _burnFeeSell = 2;
    uint256 public _marketingFeeSell = 6;
    uint256 public totalFeeSell = 12;
    
    address public marketingAddress = payable(0x8BcA151f01602D60Dd1159c13eC4cc4C1f0360F8);
    ...
    uint256 private numTokensSellToAddToLiquidity = 20_000_000_000 * 10**9;
    ...
    function setNoOfTokensSell(uint256 newVal) external onlyOwner() {
        require(newVal >= _tTotal.div(100_000),"Swap amount can't be lower than %0.00001 of total supply");
        numTokensSellToAddToLiquidity = newVal;
    }

    // fee management
    function setBuyFees(uint256 tax, uint256 burn, uint256 liquidity, uint256 marketing) external onlyOwner {
        _taxFeeBuy = tax;
        _burnFeeBuy = burn;
        _liquidityFeeBuy = liquidity;
        _marketingFeeBuy = marketing;
        totalFeeBuy = tax + burn + liquidity + marketing;
        require(totalFeeBuy <= 25, "max 25%");
    }

    function setSellFees(uint256 tax, uint256 burn, uint256 liquidity, uint256 marketing) external onlyOwner {
        _taxFeeSell = tax;
        _burnFeeSell = burn;
        _liquidityFeeSell = liquidity;
        _marketingFeeSell = marketing;
        totalFeeSell = tax + burn + liquidity + marketing;
        require(totalFeeSell <= 25, "max 25%");
    }
