    ...
    ClearingHouseStorageV1
{
    ...
    function initialize(
        ...
        // CH_ENC: Trader is not contract
        require(traderArg.isContract(), "CH_ENC");
        // CH_IFANC: InsuranceFund address is not contract
        require(insuranceFundArg.isContract(), "CH_IFANC");

        address liquidityProviderArg = ITrader(traderArg).getLiquidityProvider();
        ...
        returns (RemoveLiquidityResponse memory)
    {
        ...
        //   baseToken: in Trader.settleFunding()
        ...
        int256 traderOpenNotional = IPositionMgmt(_positionMgmt).getTraderOpenNotional(trader, params.baseToken);
        uint256 markPrice = ITrader(_trader).getTwapMarkPrice(params.baseToken, 0);
        emit PositionChanged(
            trader,
            params.baseToken,
            response.traderBase, // base
            response.traderQuote, // quote
            0,
            traderOpenNotional, // openNotional
        ...
        returns (uint256 base, uint256 quote)
    {
        ...
        ITrader.SwapResponse memory response = _openPosition(
        ...
        returns (uint256 base, uint256 quote)
    {
        ...
        ITrader.SwapResponse memory response = _closePosition(
            InternalClosePositionParams({ trader: trader, baseToken: params.baseToken })
        );
    ...
    function liquidate(address trader, address baseToken) external override whenNotPaused nonReentrant {
        ...
        //   => accountValue / sum(abs(positionValue_pool)) <
        //        sum(mmRatio * abs(positionValue_pool)) / sum(abs(positionValue_pool))
        //   => accountValue < sum(mmRatio * abs(positionValue_pool))
        //   => accountValue < sum(abs(positionValue_pool)) * mmRatio = totalMinimumMarginRequirement
        //
        ...
        ITrader.SwapResponse memory response = _closePosition(
            InternalClosePositionParams({ trader: trader, baseToken: baseToken })
        );
    ...
    /// @inheritdoc IClearingHouse
    function cancelOpenOrder(address lp, address baseToken) external override whenNotPaused nonReentrant {
        ...
        require(
            (_getFreeCollateralByRatio(lp, IClearingHouseConfig(_clearingHouseConfig).getMmRatio()) < 0) ||
                getAccountValue(lp) < IPositionMgmt(_positionMgmt).getMarginRequirementForLiquidation(lp),
            "CH_NEXO"
        );

        // must settle funding first
        _settleFunding(lp, baseToken);

        Account.LPInfo memory order = ILiquidityProvider(_liquidityProvider).getOpenOrder(lp, baseToken);
        ILiquidityProvider.RemoveLiquidityResponse memory response = ILiquidityProvider(_liquidityProvider)
            .removeLiquidity(
                ILiquidityProvider.RemoveLiquidityParams({
                    lp: lp,
        ...
        _settleBalanceAndRealizePnl(lp, baseToken, response);
    }
    ...
    function getAccountValue(address trader) public view override returns (int256) {
        int256 fundingPayment = ITrader(_trader).getAllPendingFundingPayment(trader);
    ...
    function _settleBalanceAndRealizePnl(
        ...
        if (response.traderBase != 0) {
            pnlToBeRealized = ITrader(_trader).getPnlToBeRealized(
                ITrader.RealizePnlParams({
                    trader: lp,
                    baseToken: baseToken,
                    base: response.traderBase,
                    quote: response.traderQuote
                })
            );
        }
    ...
    function _openPosition(InternalOpenPositionParams memory params) internal returns (ITrader.SwapResponse memory) {
        ITrader.SwapResponse memory response = ITrader(_trader).swap(
            ITrader.SwapParams({
        ...
        IPositionMgmt(_positionMgmt).modifyTraderBalance(
            params.trader,
            params.baseToken,
            response.base,
            response.quote
        ...
        int256 openNotional = IPositionMgmt(_positionMgmt).getTraderOpenNotional(params.trader, params.baseToken);
        uint256 markPrice = ITrader(_trader).getTwapMarkPrice(params.baseToken, 0);
    ...
    function _closePosition(InternalClosePositionParams memory params) internal returns (ITrader.SwapResponse memory) {
        int256 positionSize = IPositionMgmt(_positionMgmt).getTraderPositionSize(params.trader, params.baseToken);
        ...
        uint256 amountLimit = ITrader(_trader).getEstimateAmountLimit(
            params.baseToken,
            positionSize.abs(),
            true,
            isExactInput
        ...
        returns (Funding.Growth memory fundingGrowthGlobal)
    {
        ...
        (fundingPayment, fundingGrowthGlobal) = ITrader(_trader).settleFunding(trader, baseToken);
