...
contract ERC721LA is IERC721LA, Initializable, AccessControl {
        ...
        returns (uint256)
    {
        ...
        // Send a transfer event for the first token of the collection
        uint256 firsTokenId = editionedTokenId(state._editionCounter, 1);
        emit Transfer(address(0), msg.sender, firsTokenId);
        state._editionCounter += 1;
     ...
     */
    function totalEditions() public view returns (uint256 total) {
        ERC721LAState storage state = _getERC721LAState();
