...
contract FoodVote {
    ...
    event NewVoting(address indexed creator, uint256 votingId, VotingTopics topic);
    event Vote(address indexed voter, uint256 votingId);
    event VoteFood(address indexed voter, uint256 votingId, uint256 foodId);
    event RevokeVote(address indexed voter, uint256 votingId);
    event VotingFinished(uint256 votingId);
    ...
    function revokeVote(uint256 votingId) external onlyValidVoting(votingId) onlyOngoingVoting(votingId) {
        ...
        emit RevokeVote(msg.sender, votingId);
    }
    ...
    function voteForFood(uint256 foodId) external onlyMember {
        ...
        emit VoteFood(msg.sender, latestVotingId, foodId);
    }
    ...
    function _createVoting(VotingTopics topic, address newMemberAddress) private {
        ...
        emit NewVoting(msg.sender, currentVotingId, topic);
    }
