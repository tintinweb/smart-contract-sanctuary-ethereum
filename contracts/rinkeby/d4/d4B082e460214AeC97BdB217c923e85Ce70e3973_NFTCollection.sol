    ...
    KeeperCompatibleInterface
{
        ...
        returns (string memory)
    {
        ...
        if (_metadataCleared) {
            parts[1] = string(
                abi.encodePacked("Randomness: ", _toString(_randomness))
            );
        } else {
    ...
    function _fullfillRandomnessForMetadata(uint256 randomness) internal {
        metadatas.push(
            Metadata({
    ...
    function fulfillRandomness(bytes32, uint256 randomness)
        ...
        override
    {
        _fullfillRandomnessForMetadata(randomness);
    }
