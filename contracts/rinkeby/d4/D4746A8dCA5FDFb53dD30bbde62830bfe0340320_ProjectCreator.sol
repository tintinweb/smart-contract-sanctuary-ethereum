...
contract ProjectCreator is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    // mapping for project address to song address
    mapping (address => address) public projectToSong;

    // ============ Events ============

    /// Emitted when an Project is created
    event CreatedProject(uint256 projectId, string name, string symbol, address indexed projectAddress, address indexed songAddress);
    ...
    function createProject(
        ...
        BeaconProxy proxyProject = new BeaconProxy(
            beaconAddress,
         ...
         // Create Song for the Project
        BeaconProxy proxySong = new BeaconProxy(
            beaconAddress,
            abi.encodeWithSelector(
                Song(address(0)).initialize.selector,
                msg.sender,
                abi.encodePacked(_name, "-song"),
                abi.encodePacked(_symbol, "-song"),
                _baseURI,
                uint256(0),
                address(proxyProject)
            )
        );

        // add to registry
        projectContracts.push(address(proxyProject));

        // map project address to song address
        projectToSong[address(proxyProject)] = address(proxySong);

        emit CreatedProject(atProjectId.current(), _name, _symbol, address(proxyProject), address(proxySong));

        atProjectId.increment();

        return address(proxyProject);
    }
...
contract Project is ERC721Upgradeable, IERC2981Upgradeable, OwnableUpgradeable {
	...
	return string(abi.encodePacked(baseURI, "stems/" , tokenToCollection[_tokenId].toString(), "/", _tokenId.toString()));
    }
...
/**
  ...
  * @author Stems Labs
 */
contract Song is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  CountersUpgradeable.Counter private atSongId;

  // mapping of whitelist user address to Song token ID
  mapping (address => uint256) private _userToSong;
  // mapping of user address to whitelist flag
  mapping (address => bool) private _whitelist;
  // Mapping of remix NFT to stemNFT
  mapping(uint256 => uint256[]) public songToStems;

  // Mint price of Song NFT
  uint256 public _tokenPrice;
  // The amount of funds that have been deposited for a given Song Token
  uint256 public depositedForSong;
  // Project Address
  ...
  event SongMinted(address _to, uint256 _tokenId, uint256[] _refStems, uint256 _tokenPrice);

  event WhitelistedUser(address _to);

  event WithdrawnFunds(address _amount);

  /**
    * @dev Initialize function stands for constructor in normal smart contract
    */
  function initialize (
    address _owner,
    string memory _name,
    ...
    // Set song id start to be 1 not 0
    atSongId.increment();

    // Set Project Address that is from Project.sol
    projectAddress = _projectAddress;
  }

  /// @notice Creates a new song token for the given token id, and assigns it to the remix creator
  /// @param _tokenId Song token id will be increased accordingly
  /// @param _refStems Reference Stem NFT TokenId list
  function mint(uint256 _tokenId, uint256[] calldata _refStems) external payable {
    // Check the msg.value is sufficient
    require(msg.value >= _tokenPrice, "Must send enough to mint the song NFT.");

    depositedForSong += msg.value;

    _mint(msg.sender, _tokenId);

    songToStems[atSongId.current()] = _refStems;
    atSongId.increment();

    emit SongMinted(msg.sender, _tokenId, _refStems, msg.value);
  }

  /// @notice returns song ids that referenced given stem NFT
  /// @param _stemTokenId reference stem NFT id
  function getSongsByStem(uint256 _stemTokenId) public view returns (uint256[] memory) {
    uint256[] memory songList = new uint256[](atSongId.current());
    uint256 counter = 0;

    for (uint256 i = 1; i <= atSongId.current(); i ++) {
      for (uint256 j = 0; j < songToStems[i].length; j ++) {
        if (songToStems[i][j] == _stemTokenId) {
          songList[counter] = i;
          counter ++;
        }
      }
  ...
  /// @notice list user to whitelist with given address
  /// @param _user address that will be on the whitelist in case of it has never been on the whitelist
  function whitelistUser(address _user) external onlyOwner {
    require(_whitelist[_user] == false, "This user is already on the whitelist");

    _whitelist[_user] = true;
  }

  /// @notice Checks if user is on the whitelist or not
  ...
  /// @notice Returns token URI (metadata URL). e.g. https://stemsdao.com/api/metadata/[projectId]/song/[collectionId]/[tokenId]
  function tokenURI(uint256 _tokenId) public view override returns (string memory) {
      require(_exists(_tokenId), "ERC721Metadata: URI query for nonexistent token");

      return string(abi.encodePacked(baseURI, "song/", _tokenId.toString()));
  }

  /// @notice set Song NFT price
  /// @param _newTokenPrice new Song NFT price that will be set
  function setTokenPrice(uint256 _newTokenPrice) external onlyOwner {
    _tokenPrice = _newTokenPrice;
  }

  /// @notice Withdraw funds that staked in the contract to recipient address
  /// @param _recipient address that fund will be sent
  function withdrawFunds(address _recipient) external onlyOwner nonReentrant {
    require(address(this).balance >= depositedForSong, "Insufficient Value");

    (bool success, ) = _recipient.call{value: depositedForSong}("");
    require(success, "Unable to send value: recipient may have reverted");
