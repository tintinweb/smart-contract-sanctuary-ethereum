...
contract MetaLeons is ERC721AQueryable, Ownable, ReentrancyGuard, ERC2981 {
    ...
    uint256 public maxPerInvestor = 33;
    ...
    /// @notice Associate wallet address to wether a free NFT has been gifted.
    mapping (address => uint256) public userGifted;
    ...
    //        Events        //
    //////////////////////////
    /// @notice Is emitted when NFTs are minted, return address of minter and minted amount.
    /// @return recipient, an address.
    /// @return quantity, a uint256.
    event Minted(address recipient, uint256 quantity);

    /// @notice Is emitted when a free NFT is minted, return address of minter.
    /// @return recipient, an address
    ...
    modifier canMint(uint256 _quantity) {
        require(
            balanceOf(msg.sender) + _quantity <= maxPerUser, 
    ...
    constructor(
        ...
        _setDefaultRoyalty(secondaryWallet, 500);
    }
    ...
    ) external payable canMint(_quantity) presaleOnly {
    ...
    function investorMint(
        ...
        uint256 _minted = balanceOf(msg.sender);
        // how many actually paid by user
        uint256 _realMinted = _minted - userGifted[msg.sender];
        // separate toMint to not have it influence the calculations of gifted
        uint256 _toMint = _quantity;
        // gift free mints if thresholds exceeded
        for(uint i = userGifted[msg.sender]; i < maxGifted; i++) {
            if(_realMinted + _quantity >= (i + 1) * giftThreshold) {
                _toMint += 1;
                userGifted[msg.sender] += 1;
                emit FreeMinted(msg.sender);
            }
        }
        // ensures _quantity does not exceed maxSupply and maxPerInvestor
        require(_minted + _toMint <= maxPerInvestor, "exceeds per investor limit");
        require(totalSupply() + _toMint <= maxSupply, "Exceeds supply");
        // mint quantity
        _mint(_toMint);
    }
    ...
    function freezeMetadata(
        ...
        bytes calldata _signature,
        uint256 _validTime
    ) external isOwnerOfToken(_tokenId) {
        require(
    ...
    function airDrop(address _recipient, uint256 _quantity) public onlyOwner {
        ...
        // emit Minted event
        emit Minted(_recipient, _quantity);
    }
