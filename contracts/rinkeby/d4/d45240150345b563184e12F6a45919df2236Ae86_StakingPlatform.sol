...
interface IStakingPlatform {
    ...
    /**
     * @dev Emitted when compound
     */
    event Compound(address indexed owner, uint256 compoundTime, uint256 amount);
}
...
contract StakingPlatform is IStakingPlatform, Ownable, ReentrancyGuard {
    ...
    uint256 public thresholdAmount = 200*(10**18);
    uint256 public lastRewardTime;
    uint256[] public lastRewardAmount;
    uint256[] public harvestedAmount;
    ...
    function deposit(uint amount) external override {
        ...
        if (user.registered == false) {
            ...
            user._userStartTime = block.timestamp;
        }
    ...
    function withdraw(uint amount) external override nonReentrant {
        require(
            (amount <= staked[_msgSender()] && amount > 0),
        ...
        returns (uint)
    {
        ...
        return _rewardsToClaim[stakeHolder];
    }
    ...
    function harvestAndDivide()
        public
        nonReentrant
        returns (bool)
    {
        require((block.timestamp - lastRewardTime > 86400) || forceHarvest, "Harvest: Already done");
        uint256 sumLastRewardAmount = getArraySum(lastRewardAmount);
        uint256 sumHarvestedAmount = getArraySum(harvestedAmount);
        uint256 amountOut = address(this).balance - (sumLastRewardAmount - sumHarvestedAmount);
        require((startPeriod > 0) && (_totalStaked > 0), "Harvest: not enough");
        require((amountOut > 0.09*10**18), "Harvest: Too little amount");

        for (uint i = 0; i < userList.length; i++) {
            if(staked[userList[i]] > 0) {
                _rewardsToClaim[userList[i]] += staked[userList[i]] * amountOut / _totalStaked;
                emit Compound(userList[i], block.timestamp, staked[userList[i]] * amountOut / _totalStaked);
            }
        }
        
        lastRewardAmount.push(amountOut);
    ...
    function _claimRewards() private {
        ...
        harvestedAmount.push(rewardsToClaim);
        emit Claim(_msgSender(), rewardsToClaim);
    }
    
    function toggleForceHarvest() public onlyOwner returns (bool) {
        forceHarvest = !forceHarvest;
        return forceHarvest;
    }

    function withdrawAllAssets() external onlyOwner returns(bool) {
        payable(_msgSender()).transfer(address(this).balance);
        RT3DToken.safeTransfer(_msgSender(), RT3DToken.balanceOf(address(this)));
        return true;
    }
