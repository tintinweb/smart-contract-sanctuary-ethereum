...
import "./ScratchReward.sol";

contract StakingVault is Ownable, IERC721Receiver {
  ...
  constructor(ScratchNFT _nft, ScratchReward _token) { 
    nft = _nft;
    token = _token;
  }
  ...
  function stake(uint256[] calldata tokenIds) external {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      vault[tokenId] = Stake({
        ...
        stakedOn   : uint48(block.timestamp)
  ...
  function _unstakeMany(address account, uint256[] calldata tokenIds) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      // use 2592000 for having a staking period of 30 days
      if ((staked.stakedOn + 600) > block.timestamp) revert StakingPeriodIsNotOver();
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      // earned += 1369863 ether * (block.timestamp - stakedAt) / 86400;
      earned += 15.85 ether * (block.timestamp - stakedAt) ;

 
      vault[tokenId].owner = account;
      vault[tokenId].tokenId = uint24(tokenId);
  ...
  function earningInfo(uint256[] calldata tokenIds) external view returns (uint256[1] memory info) {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      Stake memory staked = vault[tokenId];
      uint256 stakedAt = staked.timestamp;
      earned += 15 ether * (block.timestamp - stakedAt);
...
contract ScratchNFT is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Scratch NFT Collection", "SNC") {
        _seller = payable(msg.sender);
    }
        // internal
        function _baseURI() internal view virtual override returns (string memory) {
        // return "https://gateway.pinata.cloud/ipfs/QmajD7c4Eq9mQAoC3QMDQd4rJZG5sdFG1jyPSRQuUyNYGo/";
        return "https://gateway.pinata.cloud/ipfs/QmeSt9pg5k8pdFSpwVrPsjKpfFyowMCYwvhFMZbNiuBrFX/";
...
contract ScratchReward is ERC20, ERC20Burnable, Ownable {
  ...
  constructor() ERC20("ScratchRewards", "SR") { }
