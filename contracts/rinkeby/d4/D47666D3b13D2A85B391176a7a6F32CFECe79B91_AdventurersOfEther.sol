 ...
 */
contract AdventurersOfEther is PublicSale {
    constructor() PublicSale() {}
     ...
     */
    function transferToTreasury(address _treasury) external onlyOwner returns (bytes memory) {
        require(_treasury != address(0), "zero address");
        (bool _sent, bytes memory _data) = _treasury.call{value: address(this).balance}("primary");
 ...
 */
contract PublicSale is PreSale {
    ...
    // uint public constant TOKENS_PER_ADDRESS = 20;
    constructor() PreSale() {}
...
/**
 * @notice Whilelist stage of Adventurers Token workflow
 */
contract WhiteList is Reveal, EIP712 {
    string public constant EIP712_VERSION = "1.0.0";

    /* state */
    address public signer;
    mapping(address /* minter */ => /* minted */ uint) private whilelistMinters;
        ...
        external returns (uint from, uint to) 
    {
        ...
        uint _maxCount = _signatureCount + 1 - whilelistMinters[msg.sender];
        require(_count < _maxCount, "eip-712: invalid count");
        whilelistMinters[msg.sender] += _count;
...
abstract contract ERC721 is IERC721Enumerable, IERC721Metadata, Ownable {
    ...
    function setMaxSupply(uint256 _maxSupply) external onlyOwner {
        require(_maxSupply < MAX_SUPPLY, "max supply excceded");
