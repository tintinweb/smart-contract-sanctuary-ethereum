...
contract XCO2 is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    {
        require(msg.value > 0);
        require(amount * pricePerToken == msg.value);
    ...
    {
        require(amount > 0);
        require(totalMinted + amount <= maxSupply);
        uint discountPrice = pricePerToken - (pricePerToken * discountPercents[2] / 100);
        require(msg.value == discountPrice * amount);
        uint mintAmount = amount * 10 ** 18;
    ...
    function genCert(address addr) public view returns (string memory) {
        return string(
            abi.encodePacked(
                ...
                '<text x="15" y="57" class="b small">', getTonsOffset(retiredBalanceOf[addr]), ' Tons Offset</text>',
    ...
    function getTonsOffset(uint retiredBalance) internal pure returns (string memory){
        ...
        uint256 carbonOffsetTons = retiredBalance / 1000000000000000000;

        string memory cOffset = StringsUpgradeable.toString(retiredBalance / 1000000000);

        bytes memory stringLen = bytes(cOffset);

        cOffset = substring(cOffset, (stringLen.length - 9), stringLen.length);

        return string(abi.encodePacked(StringsUpgradeable.toString(carbonOffsetTons), ".", cOffset));
    }
