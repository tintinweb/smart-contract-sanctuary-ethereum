...
contract Stakingbtcpool is Ownable {
 ...
 struct PoolInfoF {
        ...
        uint256 apyF;                // APY
        uint256 withdrawFeeF;        // Fee percentage for withdrawing anytime
        uint256 stakedAmountF;
        uint256 compoundedAmountF;
 }
    ...
    event DepositF(address indexed user, uint256 indexed pid, uint256 amount);
    event transfer(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event WithdrawF(address indexed user, uint256 indexed pid, uint256 amount);
   ...
   function addflex(uint256 _apy, uint256 _withdrawFee) external onlyOwner {
        poolInfoF.push(PoolInfoF({
            apyF: _apy,
            withdrawFeeF: _withdrawFee,
            stakedAmountF: 0,
            compoundedAmountF: 0
        }));
    }
 ...
 function depositF(uint256 _pid, uint256 _amount) external {
        PoolInfoF storage pool = poolInfoF[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(_amount > 0, "deposit: wrong amount");
        crace.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.rewardDebt = user.amount * pool.apyF * (block.timestamp - user.timestamp) / 3153600000 + user.rewardDebt;
        user.timestamp = block.timestamp;
        user.amount = user.amount + _amount;
        pool.stakedAmountF = pool.stakedAmountF + _amount;
        emit DepositF(msg.sender, _pid, _amount);
    }
    ...
    function withdrawF(uint256 _pid, uint256 _amount) external {
        PoolInfoF storage pool = poolInfoF[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount && _amount > 0, "withdraw: wrong amount");
        user.rewardDebt = user.amount * pool.apyF * (block.timestamp - user.timestamp) / 3153600000 + user.rewardDebt;
        ...
        pool.stakedAmountF = pool.stakedAmountF - _amount;
        crace.safeTransfer(address(msg.sender), _amount);
        emit WithdrawF(msg.sender, _pid, _amount);
    }
