...
contract Tarurative is ERC721Enumerable, ERC2981, AccessControl, Ownable {
    ...
    // FIXME: 
    constructor() ERC721("AAANIIII", "AAAUUU") {
        transferOwnership(msg.sender);
    }

    address public SUPPORTER_ADDRESS;
    ...
    function _checkSupporter() internal view {
        require(msg.sender == owner() || msg.sender == SUPPORTER_ADDRESS, 'access denied');
    }
    ...
    mapping(address => uint256) private taruWhitelist; // Whitelist of Taru members
    mapping(address => uint256) private whitelist; // Whitelist
    ...
    function mint(uint256 amt) public payable {
        require(totalSupply() + amt <= MAX_SUPPLY, 'too much');

        if (publicSale) {
            require(publicPrice * amt == msg.value, '');
            for (uint256 i=0; i<amt; i++) {
                _safeMint(msg.sender, nextTokenId++);
            }
        }

        if (taruWhitelist[msg.sender] > 0) {
            require(taruWhitelist[msg.sender] - amt >= 0, 'too much');
            for (uint256 i=0; i<amt; i++) {
                _safeMint(msg.sender, nextTokenId++);
            }
            return;
        ...
        if (whitelist[msg.sender] > 0) {
            require(whitelist[msg.sender] - amt >= 0, 'too much');
            for (uint256 i=0; i<amt; i++) {
                _safeMint(msg.sender, nextTokenId++);
            }
            return;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory taruimageUri = string(abi.encodePacked(baseURI, tokenId));
        ...
        if (!revealed) {
            ...
            return string(
                abi.encodePacked(baseURI, metadata)
            );
        }
        ...
        if (bytes(collabTaruInfo[tokenId].imageUri).length > 0) {
            taruimageUri = collabTaruInfo[tokenId].imageUri;
            metadata = Base64.encode(
                bytes(
                    string(
                        abi.encodePacked(
    ...
    function setCollabTaruInfo(uint256 tokenId, string calldata collaborator, string calldata url) public {
        ...
        collabTaruInfo[tokenId].imageUri = url;
    ...
    // かわいそうな樽
    // write
    function thereTherePoorTaru(uint256 tokenId) public onlyOwner {
        collabTaruInfo[tokenId].name = '';
        collabTaruInfo[tokenId].account = 0x000000000000000000000000000000000000dEaD;
        collabTaruInfo[tokenId].imageUri = '';
    ...
    function revokeSupporterAddress(address _address) external onlySupporter {
    }
    ...
    function setBaseURI(string calldata uri) public onlySupporter {
        baseURI = uri;
    }
