...
// File: contracts/Lottery Contracts/FiftyFiftyLottery.sol
...
contract FiftyFiftyTicket is VRFConsumerBase, ReentrancyGuard, Ownable, Pausable {
    ...
    function finalizeFunds() whenPaused() public payable onlyOwner {
        payable(msg.sender).transfer(address(this).balance);
    }

    // when called the balance of the contract or percentage of balance is send to winner
    function payWinner() whenPaused() public nonReentrant onlyOwner {
        require(randomResult > 0, "Must have a source of randomness before choosing winner");
        uint index = randomResult % players.length;
        // The below will send 50% of the lottery pot to the winner
        players[index].transfer( (address(this).balance / 1000) * 500 );
