...
contract Land is ERC721, ERC2981, ReentrancyGuard, Ownable, Pausable {
    ...
    */
    ...
    /**
    * @dev ELITE_SUPPLY is the quantity of elite tokens, a fraction of max supply
    *      that can be minted only during private or open sale
    * @dev Passed on function saleMode
    */
    // **NOTE: ELITE_SUPPLY value should change to 1000 before mainnet deployment
    uint8 constant ELITE_SUPPLY = 4;
    ...
    */
    ...
    /**
    * @dev saleStatus returns unsigned integer where:
    *      0 is Sale.Open or open sale - allows minting of both elite and non-elite tokens
    *      1 is Sale.Public or public sale - allows minting of non-elite tokens only
    *      2 is Sale.Private or private sale - allows minting of elite tokens only
    * @dev Default status is Sale.Public or 1
    */

    Sale public saleStatus = Sale.Public;
    ...
    */
    event SaleStatusChanged(Sale _saleStatus);
    ...
    * @dev Throws error if _tokenId is not allowed to mint depending on the state
    ...
    */
    ...
    // **NOTE: Elite and non-elite token ranges should change before mainnet deployment
    function saleMode(uint256 _tokenId) view private {
        uint256 tokenBorder = MAX_SUPPLY - ELITE_SUPPLY;

        if (saleStatus == Sale.Public) {
            require(_tokenId <= tokenBorder,
                "Elite token mint not allowed");
        }

        if (saleStatus == Sale.Private) {
            require(_tokenId > tokenBorder,
    ...
    * @dev Sale is enum
    *
    * @notice Only values 0, 1, 2 are allowed
    *         which represents sale status open,
    *         public and private respectively
    *
    * @param _saleStatus input is uint
    */
    function setSaleStatus(Sale _saleStatus)
        external
        onlyOwner
    {
