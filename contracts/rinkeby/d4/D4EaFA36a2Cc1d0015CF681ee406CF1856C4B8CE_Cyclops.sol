...
contract Cyclops is ERC721, Ownable {
   ...
   // bytes32 public root;
    uint256 _price = 0.066 ether;
    uint256 _allowlistprice = 0.055 ether;
    ...
    address a1 = 0x7d52923Ca0135F59B15986FCADeC7107758BbeFd;
    address _signingAddress = 0xA030ed6d2752a817747a30522B4f3F1b7f039c80;
    
    mapping(address => uint256) public addressMintedBalance;

    constructor(address dependentContractAddress) ERC721("Cyclops", "Cycs") {
        setBaseURI("https://nftscreen.art/cyclops/");
        
        mntps = MNTPS(dependentContractAddress);
    }
// was called mintPresale() before
    function allowlistMint(uint256 mintCount,uint8 v, bytes32 r,bytes32 s,uint256 mint_allowed,uint256 free) external payable {
        ...
        // require((_tokenIdCounter.current() + mintCount) <= presaleSupply, "Presale supply exceeded - all the presale Cyclops have been minted!");
    ...
    //    uint256 ownerMintedCount = addressMintedBalance[msg.sender];
        require(addressMintedBalance[msg.sender] + mintCount <= mint_allowed, "Minting limit for Cyclops per whitelisted wallet allowed exceeded!");
    ...
    function mintpassMint(uint256 mintCount) external payable {
        ...
        require(balance > 0, "Must hold at least one Mintpass for this function");
        require(mintCount < 11, "Cyclops quantity must be 10 or less");
    ...
    require(verifySignature(v,r,s,mint_allowed,free), "Invalid signature - please mint only on our website and make sure you are on the whitelist.");
    require(free == 1, "you are not allowed to mint for free");
    mint(msg.sender,mintCount);
    ...
    function publicMint(uint256 mintCount) external payable {
        ...
        require(mintCount < 10, "Cyclops quantity must be less than or equal to 9");
        require((_tokenIdCounter.current() + mintCount) <= tokenSupply, "Sold out! No more Cyclops are available");
        uint256 ownerMintedCount = addressMintedBalance[msg.sender];
        require(ownerMintedCount + mintCount <= _perWalletLimit, "Minting of up to 50 Cyclops per wallet allowed!");

        mint(msg.sender,mintCount);
    }

    function mint(address addr, uint256 mintCount) private {
        require((_tokenIdCounter.current() + mintCount) <= tokenSupply, "No Cyclops available");
