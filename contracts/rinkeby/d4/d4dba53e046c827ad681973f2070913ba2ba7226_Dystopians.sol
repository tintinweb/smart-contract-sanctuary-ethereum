...
contract Dystopians is ERC721A, Ownable {
    using Strings for uint256;

    uint256 public constant FOR_SALE_TOKENS = 666;
    ...
    uint256 public tokenPricePresale = 0.00666 ether;
    uint256 public tokenPricePublicSale = 0.00000000000001 ether;

    uint256 public maxTokensPerAddressPresale = 1;
    uint256 public maxTokensPerAddress = 3;
    uint256 public maxTokensPerTransaction = maxTokensPerAddress;

    uint256 public presaleTotalLimit = 66;

    bytes32 public whitelistRootOG;
    bytes32 public whitelistRoot;

    string public tokenBaseURI = "ipfs://QmYnZWhp3e6uAm7SFJ8sucgFrqvtGRa3o6A1qteAfgdQmB/";

    uint256 public soldAmount = 0;
    mapping(address => uint256) public purchased;

    constructor() ERC721A("Dystopians", "DYST") {
    ...
    function setMaxTokensPerAddressPresale(uint256 val) external onlyOwner {
        maxTokensPerAddressPresale = val;
    }
    ...
    function gift(uint256 amount, address to) external onlyOwner {
        require(soldAmount + amount <= FOR_SALE_TOKENS, "No tokens left");
        _safeMint(to, amount);
        soldAmount += amount;
    }
    ...
    function giftToMultiple(uint256 amount, address[] calldata toAddresses) external onlyOwner {
        ...
        require(soldAmount + totalAmount <= FOR_SALE_TOKENS, "No tokens left");
