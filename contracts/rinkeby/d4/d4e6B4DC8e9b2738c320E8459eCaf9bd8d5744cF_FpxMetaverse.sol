...
contract FpxMetaverse is ERC721A,Ownable,Pausable {
    ...
    function mint() external callerIsUser payable whenNotPaused  {
        ...
        require(!publicSaleResult[msg.sender], "This address has finished public mint");
        publicSaleResult[msg.sender]=true;
        _safeMint(msg.sender, 1);
        refundIfOver(msg.value-publicSalePrice);
    }


    function allowListMint(bytes32[] calldata merkleProof) external callerIsUser payable whenNotPaused {
        require(mintStatus==FpxMintStatus.ALLOWLIST_MINT, "Allowlist sale closed");
        require(totalSupply() + 1 <= maxSupply, "Exceed supply");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(merkleProof, _merkleRoot, leaf), "Invalid merkle proof");
        require(msg.value>=allowlistSalePrice);
        require(!allowlistSaleResult[msg.sender], "This address has finished allowlisted mint");
        allowlistSaleResult[msg.sender]=true;
        _safeMint(msg.sender, 1);
        refundIfOver(msg.value-allowlistSalePrice);
    }
