...
// File: contracts/lumx_complete.sol
...
contract BLKX is Ownable, ERC721A, ReentrancyGuard, ERC721AQueryable{
    ...
    uint256 public presaleWindowOpens;
    uint256 public presaleWindowCloses;
    // Metadata uris
    ...
    uint256 _presaleWindowCloses
        ...
        cost = _cost;
        windowOpens = _windowOpens; //1655918414
        presaleWindowOpens = _presaleWindowOpens;
        presaleWindowCloses = _presaleWindowCloses; // 1719076814
    }
    ...
    function mintPreSale(uint256 quantity, uint8 _v, bytes32 _r, bytes32 _s) external payable mintConditions(quantity) onlyValidAccess(_v, _r, _s) {
        require(block.timestamp >= presaleWindowOpens && block.timestamp <= presaleWindowCloses, "Presale: purchase window closed.");
        require(presaleMintedBalance[msg.sender] + quantity <= (maxPerAddress), "You can only mint 2 (3) tokens during presale (OG role has one more).");
       
        presaleMintedBalance[msg.sender] += quantity;
        _safeMint(msg.sender, quantity);
    }

    function mint(uint256 quantity) external payable mintConditions(quantity) {
        require(block.timestamp >= windowOpens, "Purchase window closed.");
        require(mintedBalance[msg.sender] + quantity <= maxPerAddress, "You can only mint 3 tokens.");
