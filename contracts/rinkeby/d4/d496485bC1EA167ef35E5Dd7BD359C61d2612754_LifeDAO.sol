...
contract LifeDAO is ERC20, ERC20Burnable, AccessControl, Pausable, ERC20Votes {
  ...
  function transferAndLock(
    ...
    console.log("_releaseTime ", _releaseTime);
  ...
  function release(address benefit) public whenNotPaused {
    console.log("release ", block.timestamp);
    ...
    require(totalReleasable != 0, "no releasable");
    transferLocked[benefit] = transferLocked[benefit].sub(totalReleasable);
    if (transferLocked[benefit] == 0) delete lockDetail[benefit];
    _transfer(address(this), benefit, totalReleasable);
    emit ReleaseToken(benefit, totalReleasable);
  }

  function lockedBalances(address benefit) public view returns (LockInfo[] memory) {
    uint256 len = lockDetail[benefit].length;
    LockInfo[] memory items = new LockInfo[](len);
    for (uint256 i = 0; i < len; i++) {
      LockInfo storage currentItem = lockDetail[benefit][i];
      items[i] = currentItem;
    }
    return items;
  }
  ...
  constructor() ERC20("DAGEN lifeDAO", "DAGEN") ERC20Permit("DAGEN lifeDAO") {
