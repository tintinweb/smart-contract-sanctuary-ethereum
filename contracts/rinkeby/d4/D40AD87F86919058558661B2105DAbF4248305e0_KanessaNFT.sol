...
contract KanessaNFT is ERC721, Ownable {
    ...
    bool private enableSale = false;
    
    
    uint256 private MAX_SUPPLY = 1000;
    uint256 private LIMIT_PER_WALLET = 5;
    ...
    constructor() ERC721("Figura Amare - Plus Size Lady", "PSL") {
    ...
    function getEnableSale() external view returns(bool) {
        return enableSale;
    }
    ...
    function getLimitPerWallet() external view returns(uint256) {
        return LIMIT_PER_WALLET;
    }

    function setLimitPerWallet(uint256 _limit) external onlyOwner {
        LIMIT_PER_WALLET = _limit;
        emit LimitPerWalletChanged(LIMIT_PER_WALLET);
    }
    ...
    function mintAdmin(address to, uint256 number) external onlyOwner {
        require(totalSupply() + number <= MAX_SUPPLY, "Exceed max supply.");
        require(balanceOf(to) + number <= LIMIT_PER_WALLET, "You exceed limit holding amount.");
    ...
    function payToMint(address recipiant, uint256 number) public payable SaleEnabled {
        require(!_whitelistActive, "Public mint is not started yet!");
        require(msg.value >= price() * number, "Money is not enough!");
        require(totalSupply() + number <= MAX_SUPPLY, "Exceed max supply");
        require(balanceOf(recipiant) + number <= LIMIT_PER_WALLET, "You exceed limit.");
