...
contract aura_test is ERC1155Supply, Ownable  {
    ...
    //uint256[] public cous = new uint256[];
    //uint256[] private _layer = new storage address[];    
    ...
    //mapping(uint256 => address) private _layer; 
    ...
    address w2 = 0x8f56d93987659016071e3864B3EE3F3F4a8446Da;
    address wDev;
    address public wStat = 0xE098Ad7d2E174Cb343Eb2E8284ACB1eaD076D8C0;

constructor(string memory _uri, string memory _name, string memory _symbol, uint256 _StartSale, address _wDev) ERC1155(_uri) {
    name = _name;
    symbol = _symbol;
...
function autoR (uint _num, bool select) external payable { //0..9
    ...
    if (user.status[_num - 1]){
        user._auto[_num - 1] = !user._auto[_num - 1]; //on-off auto
    }
    else {
        require(user.qalification, "Error! You are not qualified!");
...
if ((level.countid > 0)&&(level.countid % 3==0)) {
    ...
    if (user._auto[_num - 1]){
        ...
        _stat = user.status;
        _stat[_num - 1] = false; // test  - not to iD
        user.status = _stat;
    }
    }
level.countpay += 1;
