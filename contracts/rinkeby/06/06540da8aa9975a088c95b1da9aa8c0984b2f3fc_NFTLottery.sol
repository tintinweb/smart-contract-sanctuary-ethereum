    ...
    IERC721Receiver
{
    ...
    mapping(uint256 => address) ticketBook;
    ...
    event BuyTickets(uint256 startIndex, uint256 count);
    event LotteryOpened(address luckyAddr, uint256 cost, uint256 value);
    ...
    modifier onlyExpired() {
        require(
            lottery.status == Status.NotOpened &&
    ...
    function initialize(
        ...
        require(info.ticketCount > 0, "Invalid ticket count");
        require(
            info.endTime == 0 || (info.endTime > info.startTime),
            "Invalid time"
        );
        require(
            info.maxBuy > 0 && info.maxBuy >= info.minBuy,
            "Invalid buy limit"
        );
        _transferOwnership(msg.sender);
    ...
    function buyTickets(uint256 count) external payable {
        ...
        require(
            count > 0 && count >= lottery.minBuy && count <= lottery.maxBuy,
            "Invalid count"
        );
        require(
        ...
        require(
            lottery.soldCount + count <= lottery.ticketCount,
            "Not enough tickets"
        );
        require(msg.value >= lottery.ticketPrice * count, "Not enough value");
        require(
            IERC721(lottery.prizeAddress).ownerOf(lottery.prizeId) ==
                address(this),
            "Prize not exist"
        );

        for (uint256 i = 0; i < count; i++) {
            ticketBook[lottery.soldCount + i] = msg.sender;
        }
        lottery.soldCount += count;
        ticketBalance[msg.sender] += count;

        emit BuyTickets(lottery.soldCount - count, count);
    ...
    function retrievePrize() external onlyExpired onlyOperator {
        transferPrizeTo(owner());
        setLotteryStatus(Status.Stopped);
        Address.sendValue(
            payable(factory.getTreasury()),
    ...
    //     require(
    //         lottery.status == Status.Stopped ||
    //             (lottery.status == Status.NotOpened &&
    //                 lottery.endTime > 0 &&
    //                 block.timestamp > lottery.endTime),
    //         "Not refundable"
    //     );
    //     require(ticketBalance[msg.sender] > 0, "No available refund");

    //     uint256 count = ticketBalance[msg.sender];
    //     ticketBalance[msg.sender] = 0;
    //     uint256 refundValue = lottery.ticketPrice.mul(count);
    //     Address.sendValue(payable(msg.sender), refundValue);
    // }

    function claimPrize() external {
        require(lottery.status == Status.Claimable, "Not claimable");
        require(msg.sender == ticketBook[lottery.luckyNumber], "Not winner");
        transferPrizeTo(msg.sender);
    }
    ...
    function receiveRandomNumber(uint256 _randomNumber) external override {
        ...
        lottery.luckyNumber = randomNumber % lottery.ticketCount;
        setLotteryStatus(Status.Claimable);

        address luckyAddr = ticketBook[lottery.luckyNumber];
        emit LotteryOpened(
            luckyAddr,
            lottery.ticketPrice.mul(ticketBalance[luckyAddr]),
            lottery.ticketPrice.mul(lottery.ticketCount)
        );
    }

    function transferPrizeTo(address user) private {
        IERC721(lottery.prizeAddress).safeTransferFrom(
            address(this),
            user,
            lottery.prizeId
        );
    ...
    function ticketBalanceOf(address a) public view returns (uint256) {
        return ticketBalance[a];
    }

    function ticketOwnerByIndex(uint256 i) public view returns (address) {
        return ticketBook[i];
    }

    function withdraw() external onlyExpired onlyOperator {
        Address.sendValue(
            payable(factory.getTreasury()),
            address(this).balance
        );
