...
// File contracts/PochiInuStaking.sol
...
contract PTest1Staking is Ownable, ReentrancyGuard, IERC721Receiver {
    using SafeERC20 for IERC20;
    
    IERC20 public token;
    address public nftContract = address (0x9e1fFAe86995bA9b58B381B55AD138f5b764ce68);
    ...
    address public teamWallet = (0x9bf8FaE6B3A5EB044A92d0d4Dd270c1C76b7DCEf);
	...
	function lock(uint256 _amount, uint256 nftId) public nonReentrant {
		...
		require(pochiContract.ownerOf(nftId) == msg.sender, 'Not owner of NFT');
	
        userLocks[msg.sender].tokenAmount = userLocks[msg.sender].tokenAmount + _amount;
        userLocks[msg.sender].startTime = block.timestamp;
		userLocks[msg.sender].nftIds.push(nftId);
        
        // move the tokens
        token.safeTransferFrom(address(msg.sender), address(this), _amount);
		
		// push to NFT contract
		uint256 tokensPerNFT = userLocks[msg.sender].tokenAmount / userLocks[msg.sender].nftIds.length;
		uint256 tokensLeft = userLocks[msg.sender].tokenAmount;
		uint i = 0;
		
		for (i = 0; i < userLocks[msg.sender].nftIds.length - 1; i++) {
			pochiContract.giveFeed(userLocks[msg.sender].nftIds[i], tokensPerNFT, msg.sender);
			tokensLeft -= tokensPerNFT;
		}
		pochiContract.giveFeed(userLocks[msg.sender].nftIds[i], tokensLeft, msg.sender);
		
        emit Locked( msg.sender, userLocks[msg.sender].tokenAmount, userLocks[msg.sender].nftIds );
    }
    ...
    function claimLock(uint256 _amount) public nonReentrant {
		...
		// push to NFT contract
		if (userLocks[msg.sender].nftIds.length > 0) {
			iPNFTNft pochiContract = iPNFTNft(nftContract);
			
			uint256 tokensPerNFT = _amount / userLocks[msg.sender].nftIds.length;
			uint256 tokensLeft = _amount;
			uint i = 0;
			
			for (i = 0; i < userLocks[msg.sender].nftIds.length - 1; i++) {
				pochiContract.takeFeed(userLocks[msg.sender].nftIds[i], tokensPerNFT);
				tokensLeft -= tokensPerNFT;
			}
			pochiContract.takeFeed(userLocks[msg.sender].nftIds[i], tokensLeft);
		}
	...
	function burnTokensForNft(uint256 _nftId) public nonReentrant {
		...
		require(userLocks[msg.sender].tokenAmount > burnAmount, 'Already Burned to Adulthood');
    ...
    function getLocked(address _addr) public view returns(uint256, uint256[] memory){
        return (userLocks[_addr].tokenAmount, userLocks[_addr].nftIds);
    }
