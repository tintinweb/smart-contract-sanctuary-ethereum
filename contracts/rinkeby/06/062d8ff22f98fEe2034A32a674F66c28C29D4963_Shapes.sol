...
contract Shapes is ERC721, ERC721Enumerable, Ownable {

  // how it ends
  uint public FINAL_SUPPLY = 2048;
  ...
  uint public defaultShapeIndex = 0;
  ...
  modifier salesIsOpen() {
    require(totalSupply() != FINAL_SUPPLY, "SALES_ENDED");
    require(totalSupply() < maxSupply && totalSupply() < FINAL_SUPPLY, "SALES_NOT_OPEN");
    _;
  }

  constructor() ERC721("SHAPES", "SHAPES") {
  ...
  function mint(address _recipient) payable public salesIsOpen returns (uint256) {
  ...
  function batchMint(address _recipient, uint256 numberOfNfts) public payable salesIsOpen {
    require(numberOfNfts > 0, "Number of NFTs cannot be 0");
    require(totalSupply() + numberOfNfts <= maxSupply, "MAX_SUPPLY");
      ...
      return string(
          abi.encodePacked(
              ...
              Base64.encode(
                  bytes(
                      abi.encodePacked(
                        '{"name": "Shapes","description": "The SHAPES by makio135.eth & clemsos.eth.","image":"',
                        svgString,
                        '", "external_link": "https://makio135.io/shapes", "seller_fee_basis_points": 500, "fee_recipient":"0x',
  ...
  function tokenURI(uint256 tokenId) 
    ...
    returns (string memory) 
    {
      uint shapeTypeIndex = shapeIndexes[tokenId];
      address svgAddress = shapesAddresses[shapeTypeIndex];

      IShape svg = IShape(svgAddress);
      string memory svgString;
      string memory attributes;
  ...
  function setMaxSupply(uint _maxSupply) public onlyOwner {
    require(_maxSupply >= totalSupply() && _maxSupply <= FINAL_SUPPLY, 'INCORRECT');
  ...
  function setShapesAddresses(address[] calldata _shapesAddresses) public onlyOwner {
    for (uint256 i = 0; i < _shapesAddresses.length; i++) {
      require(_shapesAddresses[i] != address(0), 'ZERO_ADDRESS');
    }
    shapesAddresses = _shapesAddresses;
  }

  function setDefaultShapeIndex(uint _shapeIndex) public onlyOwner {
    require(_shapeIndex <= shapesAddresses.length -1, 'OUT_OF_RANGE');
    defaultShapeIndex = _shapeIndex;
  }

  function changeSeed(uint _tokenId) public {
    require(ownerOf(_tokenId) == msg.sender, '403');
    _setNextSeed(_tokenId);
  }

  function changeShape(uint _tokenId, uint _shapeIndex) public {
    require(ownerOf(_tokenId) == msg.sender, '403');
    _setShape(_tokenId, _shapeIndex);
  }

  function _setShape(uint _tokenId, uint _shapeIndex) internal {
    require(_shapeIndex < shapesAddresses.length, 'MISSING');
    shapeIndexes[_tokenId] = _shapeIndex;
  }
  ...
  function swapSlash(uint _slashTokenId, uint _shapeIndex) public returns (uint tokenId){
    ...
    require(slashes.ownerOf(_slashTokenId) == msg.sender, '403');
    
    // burn old token by transferring to the contract itself
    slashes.transferFrom(msg.sender, address(this), _slashTokenId);
