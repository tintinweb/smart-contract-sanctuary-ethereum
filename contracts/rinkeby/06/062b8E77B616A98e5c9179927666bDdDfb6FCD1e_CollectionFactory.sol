...
contract CollectionFactory is NonblockingLzApp {
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64, bytes memory _payload) internal override {
        ...
        (bytes memory collNameBytes, bytes memory collectionURIBytes, bytes memory fileURIBytes, address creator, uint256 mintPrice, uint256 dropFrom, uint256 dropTo, string[] memory metadataURIs)
        = abi.decode(_payload, (bytes, bytes, bytes, address, uint256, uint256, uint256, string[]));
        string memory collName = string(collNameBytes);
        string memory collectionURI = string(collectionURIBytes);
        string memory fileURI = string(fileURIBytes);
    ...
    function _getPayload(string memory collName, string memory URI, string memory fileURI, uint256 price, uint256 from, uint256 to, address tokenOwner, string[] memory URIs) private pure returns(bytes memory) {
        return abi.encode(bytes(collName), bytes(URI), bytes(fileURI), tokenOwner, price, from, to, URIs);
    }
