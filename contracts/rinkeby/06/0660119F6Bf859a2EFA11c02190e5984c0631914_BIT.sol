...
contract BIT is ERC721ARoyalty, Ownable, ReentrancyGuard {
  ...
  function setPublicSaleActive(bool active) external onlyOwner {
    require(publicSaleActive != active, "PubSale: Active Same State");
    ...
    onlyEOA
  {
    ...
    require(publicSaleActive, "PubSale: Not Active");
    require(isPublicSaleValid(sig), "PubSale: Invalid Signature");
    require(qty != 0, "PubSale: 0 Quantity");
    require(totalSupply() + qty <= MAX_SUPPLY, "PubSale: Over Max Supply");
    require(
      quantityPerWallet[msg.sender] + qty <= MAX_SUPPLY_PER_WALLET,
      "PubSale: Over Max Supply Per Wallet"
    );
    ...
    onlyEOA
  {
    ...
    require(IERC20(apeAddress).balanceOf(msg.sender) >= salePriceAPE * qty, "PreSale: Insufficient APE");
