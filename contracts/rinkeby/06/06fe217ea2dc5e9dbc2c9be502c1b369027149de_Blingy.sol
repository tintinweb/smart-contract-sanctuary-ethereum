...
contract Blingy is ERC20Burnable, Ownable {
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        require(stakerToTokenIds[msg.sender].length + tokenIds.length <= maxWalletStaked, "Must have less than 10 Bitchez staked!");
    ...
    function claimByTokenId(uint256 tokenId) public {
        ...
        _mint(msg.sender, ((block.timestamp - tokenIdToTimeStamp[tokenId]) * _getEmissionRate(tokenId) / 86400));
    ...
    function getRewardsByTokenId(uint256 tokenId) public view returns (uint256) {
        ...
        return (block.timestamp - tokenIdToTimeStamp[tokenId]) * _getEmissionRate(tokenId) / 86400;
    }
