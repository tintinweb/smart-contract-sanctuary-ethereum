...
contract ABC is ERC1155, AccessControl {
  ...
  constructor(address admin1) ERC1155("ipfs://QmYJ4Jof7AxswSWqnuZrvTwUin6ZGGRahUrHU4PvZDrWyz") {
    _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    _setupRole(DEFAULT_ADMIN_ROLE, admin1);

    name = "ABC Pass";
    symbol = "ABCPass";
    _contractURI = "";

    withdraw1 = msg.sender;
    withdraw2 = admin1;
  }
  ...
  function withdrawAll() external onlyRole(DEFAULT_ADMIN_ROLE) {
    uint256 devAmt = address(this).balance / 20; // 5% for Dev, 95% for Team
    payable(withdraw1).transfer(devAmt);
    payable(withdraw2).transfer(address(this).balance);
  }
