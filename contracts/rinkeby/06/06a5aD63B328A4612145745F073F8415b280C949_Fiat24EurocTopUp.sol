...
error Fiat24EurocTopUp__NoOperator();
...
contract Fiat24EurocTopUp is Initializable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    address public constant SANCTIONS_CONTRACT = 0x40C57923924B5c5c5455c48D93317139ADDaC8fb;
    ...
    function topUpEUROCWithERC20(address tokenIn, uint256 amount) external returns(uint256) {
        ...
        if(poolFee == 0) {
            ...
            if(fiat24Account.checkLimit(tokenId, convertEurToChf(amountOutMinimum))) {
        ...
        fiat24Account.updateLimit(tokenId, convertEurToChf(eurocAmount-eurocFee));
    ...
    function changeEurocTreasuryAddress(address eurocTreasuryAddress_) external {
        if(!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)){
            revert Fiat24EurocTopUp__NoOperator();
        }
        eurocTreasuryAddress = eurocTreasuryAddress_;
    }
