...
contract FuckFiat is Ownable, ERC721A, ReentrancyGuard {
    ...
    function mint(uint256 index, uint256 amount, uint256 baseAmount, bytes32[] calldata merkleProof) external payable onlyActive nonReentrant() {
        require(amount > 0, "zero count");
        require(amount <= MAX_SUPPLY.sub(totalMinted), "not enough nfts");

        (uint256 costForMinting, uint256 freeAmount) = costForMint(index, msg.sender, amount, baseAmount, merkleProof);
        if (costForMinting > 0) {
    ...
    function costForMint(uint256 index, address account, uint256 amount, uint256 baseAmount, bytes32[] calldata merkleProof) public view returns(uint256, uint256) {
        ...
        if (freeClaimed) {
            ...
            freeAmount = freeClaimable(index, account, baseAmount, merkleProof);
