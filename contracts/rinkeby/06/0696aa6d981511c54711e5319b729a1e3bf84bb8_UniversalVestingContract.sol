...
//        setPauseStatus(true);
    ...
    //         linearVestingTimePeriod = linearVesti
    ...
    function setDates (uint startTime) external onlyOwner {
        ...
        linearStart8=startTime + 5 minutes;
        ...
        linearVestingAmountWithdrawThresholdTime= [0,startTime+10 minutes,startTime + 1 minutes,startTime + 1 minutes,startTime + 1 minutes,startTime + 5 minutes,startTime + 10 minutes,startTime + 10 minutes,startTime + 40 minutes, startTime + 15 minutes,startTime + 1 minutes];
//        setPauseStatus(false);
    ...
    function intermediateVestStatus(address _userAddress) public view returns (uint,uint) {
        ...
        {
            // if(!check[_userAddress]){
            //  timeDifference=block.timestamp
            // }
            // lastClaimTime=intermediateVestingTimePeriod[Investors[_userAddress].investorType]+intermediateArray[Investors[_userAddress].investorType];
  ...
  uint256 public linearStart8;
    ...
    function linearVestingDetails(address _userAddress) public view returns (uint) {
        uint lastClaimTime = Investors[_userAddress].lastClaimTime;
        uint timeDifference;
        uint[11] memory linearTime = [0,globalStartTime+ 10 minutes,globalStartTime + 1 minutes,globalStartTime + 1 minutes,globalStartTime + 1 minutes,globalStartTime + 5 minutes/*5*/,globalStartTime + 10 minutes,globalStartTime + 10 minutes,linearStart8 + 5 minutes, globalStartTime + 15 minutes,globalStartTime + 1 minutes];
        if (block.timestamp <= linearVestingTimePeriod[Investors[_userAddress].investorType]+linearTime[Investors[_userAddress].investorType]) {
            // lastClaimTime=linearVestingTimePeriod[Investors[_userAddress].investorType]+linearTime[Investors[_userAddress].investorType];
              timeDifference = block.timestamp - lastClaimTime;
        }
        else
        {
