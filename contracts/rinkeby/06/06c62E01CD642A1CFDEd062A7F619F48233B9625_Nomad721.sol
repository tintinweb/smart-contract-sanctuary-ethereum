...
/* 
    @title NewToken
    @notice ERC721-ready Nomad721 contract
    @author cryptoware.eth | Nomad
*/

contract Nomad721 is Ownable, ERC721, ERC721Burnable, ERC721Pausable {
    ...
    modifier notLocked(){
        require(!locked, "Nomad721: Metadata URIs are locked");
    ...
    modifier notRevealed() {
        require(!revealed, "Nomad721: Art is already revealed");
        _;
    }

    /// @notice Art is already revealed
    modifier Revealed() {
        require(revealed, "Nomad721: Art is not revealed");
        _;
    }
    ...
    receive() external payable {
        revert("Nomad721: Please use Mint or Admin calls");
    }
    ...
    **/
    ...
    function adminMint(address to) external whenNotPaused onlyOwner {
        require (to != address(0), "Nomad721: Address cannot be 0");
        ...
        whenNotPaused
    {
        ...
        require(to != address(0), "Nomad721: Address cannot be 0");
        require(
            received == _mintPrice[_phase].mul(amount), "Nomad721: Ether sent is not the right amount"
        );

        maxSupplyNotExceeded(amount);

        if(_phase == Phase.Presale){
            require (amount <= 3, "Nomad721: Allowed amount to mint exceeded");
            isAllowedToMint(_proof);
            walletLimitNotExceeded(to, amount);
        } else {
            require(amount <= 10, "Nomad721: Allowed amount to mint exceeded");
            walletLimitNotExceeded(to, amount);
        }
    ...
    function changeMintBeneficiary(address beneficiary) public onlyOwner {
        require(
            ...
            "Nomad721: Minting beneficiary cannot be address 0"
        );

        require(
            beneficiary != _mintingBeneficiary,
            "Nomad721: beneficiary cannot be the same as previous"
        );
    ...
    function setPhase(
        ...
        require(mintCost>0, "Nomad721: rate is 0");
        require(_phase != phase, "Nomad: Phase cannot be the same");
    ...
    function changeMintCost(uint256 mintCost) public onlyOwner {
        require(
            ...
            "Nomad721: mint cost cannot be same as previous"
        );
    ...
    function changeBaseURI(string memory newBaseURI)
        ...
        notLocked
    {
        require((keccak256(abi.encodePacked((_baseTokenURI))) != keccak256(abi.encodePacked((newBaseURI)))), "Nomad721: Base URI cannot be same as previous");
    ...
    **/
        ...
        onlyOwner
    {
        require(
            ...
            "Nomad721: Merkle root cannot be same as previous"
        );
        _merkleRoot = merkleRoot;
    }
    ...
    function maxSupplyNotExceeded(uint32 amount) public view returns (bool) {
        require(_tokenIdTracker.current().add(amount.sub(1)) <= _maxTokenId, "Nomad721: max NFT limit exceeded");
        return true;
    }
    ...
    function isAllowedToMint(bytes32[] memory _proof) internal view returns (bool){
        require(
            ...
            "Nomad721: Caller is not whitelisted for Presale"
        );
        return true;
    }
    ...
    function walletLimitNotExceeded(address minter, uint32 amount) internal view returns (bool) {
        require(balanceOf(minter).add(amount) <= _presaleLimit, "Nomad721: max NFT per address exceeded");
        return true;
    }
    ...
    function _forwardFunds(uint256 received) internal {
        ...
        require(success, "Nomad721: Failed to forward funds");
    }
