...
contract PLONDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("PLON_Dividend_Tracker", "PLON_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "PLON_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "PLON_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PLON contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PLON_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "PLON_Dividend_Tracker: Cannot update claimWait to same value");
...
Token Distribution & Deployment
Name: PLONA (PLON) 
First Chain : ETH (ERC-20)
*/
contract PLON is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    PLONDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PLONA", "PLON") {
        ...
        onSellmarketingFee = 3;
        onSellburnFee = 1;

        marketingWallet = payable(0x3e78777d7cD5bd7477fabD2a9e18ab3342eF9808);

        totalFees = ETHRewardFee.add(liquidityFee).add(marketingFee).add(burnFee); // total fee transfer and buy

        onSelltotalFees = onSellETHRewardFee.add(onSellliquidityFee).add(onSellmarketingFee).add(onSellburnFee);

    	dividendTracker = new PLONDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "PLON: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "PLON: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "PLON: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PLON: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PLON: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PLON: Cannot update gasForProcessing to same value");
