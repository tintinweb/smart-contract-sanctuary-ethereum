...
contract MergeMana is Ownable {
   ...
   * Mana based logic
   */
  function _getInherentAndBoostedManaWithNoPenalties(uint256 id)
    internal
    view
    returns (uint256)
  {
  ...
  function getConsumableMana(uint256 id) public view returns (uint256) {
    return getInherentAndBoostedMana(id) + getChanneledMana(id);
  }
  ...
  function getInherentMana(uint256 id) public view returns (uint256) {
    ...
    return uint256(uint8(baseManaStorage.indexToData(id)[0])) *
      baseManaMultiplier;
  }

  function getInherentAndBoostedMana(uint256 id) public view returns (uint256) {
    uint256 decay = getManaDecay();
    uint256 consumedAmount = consumedMana[id];
    if ((decay + consumedAmount) > channeledMana[id]) {
      uint256 leftOverManaPenalty = (decay +
        consumedAmount -
        channeledMana[id]);
      if (leftOverManaPenalty > _getInherentAndBoostedManaWithNoPenalties(id)) {
        return 0;
      }
      return _getInherentAndBoostedManaWithNoPenalties(id) - leftOverManaPenalty;
    }
    return _getInherentAndBoostedManaWithNoPenalties(id);
  }
  ...
  function _channelMana(uint256 from, uint256 to) internal {
    uint256 inherentManaAndBoostedMana = getInherentAndBoostedMana(from);
  ...
  function manaBoostRitual(uint256[] memory tokenIds)
    ...
    onlyIsNotShutdown
  {
    uint256 boostedManaAmount = getBoostManaAmount(msg.sender);
    ...
    onlyIsNotShutdown
  {
    uint256 boostedManaAmount = getBoostManaAmount(msg.sender);
    for (uint256 i = 0; i < tokenIds.length; ++i) {
      if (merge.ownerOf(tokenIds[i]) == msg.sender && boostedMana[tokenIds[i]] == 0) {
        _boostMana(tokenIds[i], boostedManaAmount);
      }
    }
  }
