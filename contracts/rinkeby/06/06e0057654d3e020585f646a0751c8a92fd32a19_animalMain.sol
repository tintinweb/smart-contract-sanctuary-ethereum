...
contract animalMain is  ERC721,Ownable {
  ...
  function random(uint maxNum)public view returns(uint){
      bytes32 result = keccak256(abi.encodePacked(block.coinbase,blockhash(block.number-1),now));
      return uint(result) % maxNum + 1;
   }

  function randomStar()private view returns(uint8){
  ...
  function randomDNA()private view returns(bytes32){
      bytes32 result = keccak256(abi.encodePacked(block.coinbase,blockhash(block.number-1),now));
      return result;
  }
