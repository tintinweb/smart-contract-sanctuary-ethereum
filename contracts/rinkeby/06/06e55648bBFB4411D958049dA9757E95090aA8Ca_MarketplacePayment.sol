    ...
    PausableUpgradeable
{
    ...
    address internal TREASURY_WALLET;
    ...
    function _setFeeSplit(FeeSplit[] calldata _feeSplits, bool isUpdate) internal {
        for (uint256 i; i < _feeSplits.length; i++) {
    ...
    function _chargeAndSplit(
        ...
        IERC20Upgradeable ptoken = IERC20Upgradeable(_erc20Token);

        uint256 platformFee;
        if (_erc20Token == address(0)) {
            payable(royaltyReceiver).transfer(royaltyValue);
            platformFee = _splitFee(user, _amount, _erc20Token);
            IWETH(WETHAddress).deposit{ value: (stakingSplit + lpSplit) }();
        } else {
            require(ptoken.allowance(user, address(this)) >= _amount, "charge:insufficient spent allowance");
            ptoken.transferFrom(user, royaltyReceiver, royaltyValue);
            platformFee = _splitFee(user, _amount, _erc20Token);
            ptoken.transferFrom(user, address(this), (lpSplit + stakingSplit));
        }

        if (stakingSplit > 0) {
            ptoken.approve(address(StakeAddress), stakingSplit);
            IStake stake = IStake(StakeAddress);
            stake.receiveWETHFee(stakingSplit);
        }
        if (lpSplit > 0) {
            ptoken.approve(address(LPStakeAddress), lpSplit);
        ...
        return _amount - (stakingSplit + lpSplit + platformFee);
    }
}
