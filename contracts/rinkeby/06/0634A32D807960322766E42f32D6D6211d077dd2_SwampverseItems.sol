...
contract SwampverseItems is ERC1155Supply, Pausable, Ownable {
    ...
    uint256 public constant RUSTY_METAL = 2;
    uint256 public constant SWAMP_BOAT = 3;
    uint256 public constant SWAMP_PASS = 4;

    string public beginning_uri;
    string public ending_uri;

    uint256 public swampPileCostForRustyMetal;
    uint256 public rustyMetalCostForSwampBoat;
    ...
    constructor(
      ...
      swampPileCostForRustyMetal = _swampPileCostForRustyMetal;
      rustyMetalCostForSwampBoat = _rustyMetalCostForSwampBoat;
    ...
    function mintRustyMetal(uint256 amount) external whenNotPaused {
      _burn(msg.sender, SWAMP_PILE, swampPileCostForRustyMetal*amount);
      _mint(msg.sender, RUSTY_METAL, amount, "");
    }

    function mintSwampBoat(uint256 amount) external whenNotPaused {
      require(totalSupply(SWAMP_BOAT)+amount < maxSwampBoats+1, "Swampboat limit reached");
      _burn(msg.sender, RUSTY_METAL, rustyMetalCostForSwampBoat*amount);
      _mint(msg.sender, SWAMP_BOAT, amount, "");
    }
    
    /**
      @param _mode: 
      1 - replace swamp pile cost for rusty metals
      2 - replace rusty metal cost for swamp boat
      3 - replace max swamp boat values
    ...
    */
    function setValues(uint256 _mode, uint256 _value) external onlyOwner {
      if (_mode == 1) swampPileCostForRustyMetal = _value;
      else if (_mode == 2) rustyMetalCostForSwampBoat = _value;
      else if (_mode == 3) maxSwampBoats = _value;
