...
contract GradieERC20 is Ownable, ERC20, ERC20Capped, ReentrancyGuard {
    ...
    constructor() ERC20("Gradie Token", "$GG") ERC20Capped(100000000 ether) {
        //premine
    ...
    function getTotalClaimableAmount(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            uint256 halveTimes = 2 * ((totalSupply() / 10000000 ether) - 1);
            if (halveTimes == 0) halveTimes = 1;
            totalRewards +=
                dayPassed *
                (rewardPerDay / halveTimes) +
                _verifiedBonus;
        }
        return totalRewards;
    }
    ...
    function getCurrentDailyYield() public returns (uint256) {
        uint256 halveTimes = 2 * ((totalSupply() / 10000000 ether) - 1);
        if (halveTimes == 0) halveTimes = 1;
        return rewardPerDay / halveTimes;
    }
