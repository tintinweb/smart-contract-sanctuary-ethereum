...
contract Adapter is IAdapter, Ownable {
    ...
    mapping(bytes32 => bool) private _requests;
    ...
    function configureSubscription(
        ...
        external
    {
        bytes32 dataHash = keccak256(abi.encodePacked(block.chainid, this, "configureSubscription", recipient));
        // Signature is EIP191-compatible (https://eips.ethereum.org/EIPS/eip-191)
        bytes32 messageHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", dataHash));
        ...
        require(signer == authority, "Adapter: invalid signature");
        _requests[messageHash] = true;
        tokenStreamer.register(recipient);
    }
    ...
    function subscriptionToken()
        ...
        returns (IERC20)
    {
        return IERC20(tokenStreamer.token());
    }

    function getSubscriptionPrice(address)
        override
        external
        view
        returns (uint256)
    {
        return tokenStreamer.rate();
    }

    /// @dev Returns true if subscription is active
    function isSubscriptionValid(address sender, address recipient)
        external
        view
        returns (bool)
    {
        ...
        uint256 balance = tokenStreamer.balanceOf(streamId, sender);
        return balance > 0;
    }
}
...
/// @dev Provides a standard interface for Mitra instance.
interface IAdapter {
    ...
    function configureSubscription(address recipient, uint8 v, bytes32 r, bytes32 s) external;
    ...
    function subscriptionToken() external view returns (IERC20);
...
/**
 * @dev Example of collectible token contract.
 * This contract is supposed to be controlled by Adapter contract.
 */
...
contract TokenStreamer is ISubscription, Ownable {
    ...
    uint256 public immutable rate; // units per second
    ...
    event RegisterRecipient(address indexed recipient);
    event CreateStream(
        ...
        onlyOwner
    {
        ...
        require(!recipients[recipient], "TokenStreamer: recipient already registered");
        recipients[recipient] = true;
        emit RegisterRecipient(recipient);
    }
    ...
    function create(address recipient, uint256 deposit)
        public
    {
        require(recipients[recipient], "TokenStreamer: unknown recipient");
        ...
        require(deposit % rate == 0, "TokenStreamer: deposit is not a multiple of rate");
        ...
        emit CreateStream(streamId, msg.sender, recipient, deposit);
        ...
        public
    {
        ...
        require(amount % rate == 0, "TokenStreamer: deposit is not a multiple of rate");
        if (stream.stopTime < block.timestamp) {
            ...
            stream.startTime = stream.stopTime - duration;
        }
        // Extend
        stream.stopTime = stream.stopTime + amount / rate;
        token.safeTransferFrom(msg.sender, address(this), amount);
        emit ExtendStream(streamId, amount);
