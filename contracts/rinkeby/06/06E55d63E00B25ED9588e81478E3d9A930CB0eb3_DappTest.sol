...
contract DappTest is ERC721A, ERC721AQueryable, Ownable, ReentrancyGuard {
   ...
   bytes32 private DOMAIN_VERIFICATION = keccak256(
    ...
    address public allowlistSigner =  0xbEE2c4e66A987851d423e2e6203e532e8342f17f;    
    ...
    constructor() ERC721A("DAPPTEST", "DT") {}
   ...
   function isAllowlisted(address buyerWalletAddress, bytes memory _signature)public view returns (bool)
    ...
    function devMint(uint256 _amount) external onlyOwner { 
        require(totalSupply()+_amount <= 5555,"Can't mint more than max tokens");  //max tokens = 5555
    ...
    function AllowlistMint(uint256 _amount, bytes memory _signature) public payable {     
                ...
                require(totalSupply()+_amount<=5555,"Can't mint more than max tokens available");

                require(
                    _amount > 0 && _amount <= 5,  //max mint per txn = 5
                    string(
                        abi.encodePacked(
                ...
                require(
                    msg.value >= 0.77 ether * _amount, //allowlist price = 0.77E 
                    string(
                        abi.encodePacked(
                            "Not enough ETH! At least ",
                            Strings.toString(0.77 ether * _amount),
                            " wei has to be sent!"
                        )
                    )
                );
    ...
    function mintPublicSale(uint256 _amount) public payable {                     
                ...
                require(totalSupply()+_amount<=5555,"Can't mint more than max tokens");
                ...
                require(
                    msg.value >= 1 ether * _amount, //public price = 1E
                    string(
                        abi.encodePacked(
                            "Not enough ETH! At least ",
                            Strings.toString(1 ether * _amount),
                            " wei has to be sent!"
                        )
                    )
                );
            ...
            require(
                20 >= balanceOf(msg.sender) + _amount,  //max tokens per wallet = 20
                "Max token count per wallet exceeded!"
            );            
