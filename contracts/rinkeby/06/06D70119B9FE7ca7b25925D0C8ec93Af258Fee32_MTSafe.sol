...
contract MTSafe is ERC721, Ownable {
    ...
    uint256 public renewalPrice = 0 ether;
    ...
    constructor(string memory tokenURI_, string memory contractURI_) ERC721("MTTEST", "MTTEST") {
        _tokenURI = tokenURI_;
        _contractURI = contractURI_;
    }

    modifier nonOwner(address _to) {
        if (msg.sender != owner()) {
            require(transfersEnabled, "Token transfers are currently disabled.");
            require(balanceOf(_to) == 0, "User already holds a token.");
        }
        _;
    }
    ...
    function SafeTestMint() external payable noHaxxor {
