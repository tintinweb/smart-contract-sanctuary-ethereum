...
contract Wgmis is ERC721, Ownable {
    ...
    constructor(
        ...
        require(_payoutAddresses.length > 0, "_payoutAddresses must contain at least one entry");
        require(_payoutAddresses.length == _payoutAddressBasisPoints.length, "each payout address must have a corresponding basis point share");
        uint256 totalBasisPoints;
        for(uint256 i = 0; i < _payoutAddresses.length; i++) {
            require((_payoutAddressBasisPoints[i] > 0) && (_payoutAddressBasisPoints[i] <= 10000), "_payoutAddressBasisPoints may not contain values of 0 and may not exceed 10000 (100%)");
            totalBasisPoints += _payoutAddressBasisPoints[i];
        }
        require(totalBasisPoints == 10000, "_payoutAddressBasisPoints must add up to 10000 together");
    ...
    function mint(address _recipient, uint96 _quantity) external payable {
        ...
        require(block.timestamp >= mintingStartTimeUnix, "minting period has not started");
        require(_quantity <= singleOrderLimit, "quantity exceeds max per transaction");
        require((_tokenIds.current() + _quantity) <= (supplyLimit - 100), "would cause total supply to exceed max supply");
        require((msg.value) == (price * _quantity), "incorrect ETH value provided");
    ...
    function mintFoundation(address _recipient, uint96 _quantity) external onlyOwner {
        require(_quantity > 0, "quantity can't be zero");
        require(block.timestamp >= mintingStartTimeUnix, "minting period has not started");
        require((foundationMinted + _quantity) <= 100, "FOUNDATION_MINT_EXCEEDS_ALLOWANCE");
    ...
    function mintMerkleWhitelist(bytes32[] calldata _merkleProof, uint96 _quantity) external {
        ...
        require((_tokenIds.current() + _quantity) <= (supplyLimit - 100), "EXCEEDS_MAX_SUPPLY");
    ...
    function mintMerkleWhitelistEarlyAccess(bytes32[] calldata _merkleProof, uint96 _merkleProofAmount, uint96 _mintAmount) external payable {
        ...
        require(isRandomnessRequested == false, "MINTING_OVER");
        require(block.timestamp >= (mintingStartTimeUnix - 24 hours), "EARLY_ACCESS_NOT_STARTED");
        require((_tokenIds.current() + _mintAmount) <= (supplyLimit - 100), "EXCEEDS_MAX_SUPPLY");
    ...
    function initiateRandomDistribution() external {
        require(_tokenIds.current() == supplyLimit, "minting period has not ended");
        uint256 supply = _tokenIds.current();
        require(supply > 0, "supply must be more than 0");
        require(isRandomnessRequested == false, "request for random number has already been initiated");
    ...
    function commitRandomDistribution() external {
        require(isRandomnessRequested == true, "request for random number has not been initiated");
        IRandomNumberConsumer randomNumberConsumer = IRandomNumberConsumer(vrfProvider);
        uint256 result = randomNumberConsumer.readFulfilledRandomness(randomNumberRequestId);
        require(result > 0, "randomResult has not been provided to vrfProvider");
        vrfResult = result;
    }
    ...
    function totalSupply() public view returns(uint256) {
        return _tokenIds.current() + foundationMinted;
    }
    ...
    function distributeFees() public onlyOwner {
        ...
        for(uint256 i = 0; i < payoutAddresses.length; i++) {
            ...
            require(feeCutDeliverySuccess, "Fee cut delivery unsuccessful");
        }
    }
