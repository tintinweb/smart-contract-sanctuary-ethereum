...
contract  RoyalPenguins is ERC721Enumerable, Ownable {
   ...
   string public baseTokenURI;
    ...
    uint256 public publicSalemaxSupply = 5000;
    uint256 public preSaleStartDate = 1644250349;       
    uint256 public publicStartDate = 1644509549;        

    uint256 public preSalePrice = 0.001 ether;
    uint256 public publicSalePrice = 0.002 ether;

    uint256 public maxPreMintCount = 5;
    uint256 public maxPublicMintCount = 20;
    ...
    */
    ...
    constructor() ERC721("RoyalPenguins", "RP") {
        //setBaseURI(baseURI);
    }

    function price() public view returns (uint256) {
        uint256 d_now = block.timestamp;
        if (d_now < publicStartDate) {
            return preSalePrice;
        } else {
    ...
    function preSaleMint(uint256 _mintCount) public payable onlyWhitelisted {
        ...
        require(supply + _mintCount < maxSupply,        'This transaction would exceed max supply of RoyalPenguins');
        require(block.timestamp < publicStartDate,      'Presale Minting is ended.');
        require(supply + _mintCount <= preSalemaxSupply,           string(abi.encodePacked(preSalemaxSupply.toString(), ' RoyalPenguins can be minted in PreSale')));
        require(tokenCount + _mintCount <= maxPreMintCount,           string(abi.encodePacked('You can mint only ', maxPreMintCount.toString(), ' RoyalPenguins in PreSale')));
    ...
    function mint(uint256 _mintCount) public payable {
        ...
        require(supply + _mintCount < maxSupply,        'This transaction would exceed max supply of agelessteez');
        require(supply + _mintCount <= publicSalemaxSupply,           string(abi.encodePacked(publicSalemaxSupply.toString(), ' RoyalPenguins can be minted in PreSale')));
        require(tokenCount + _mintCount <= maxPublicMintCount,           string(abi.encodePacked('You can mint only ', maxPublicMintCount.toString(), ' RoyalPenguins in PublicSale')));
        require(msg.value >= price(),                   'Ether value is too low');
    ...
    function setPrice(uint256 _preSalePrice, uint256 _publicSalePrice) public onlyOwner {
    ...
    function setSupply(uint256 _max_supply, uint256 _preSaleMaxSupply, uint256 _publicSaleMaxSupply) public onlyOwner {
        maxSupply = _max_supply;
        preSalemaxSupply = _preSaleMaxSupply;
        publicSalemaxSupply = _publicSaleMaxSupply;
    }
    ...
    function airDrop(address[] memory addresses, uint256 count) external onlyOwner {        
        require(!paused,                                'Contract is paused.');
        require(airdropSupply < maxAirdropSupply,       'This transaction would exceed airdrop max supply');
    ...
    function info() public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        return (price(), preSaleStartDate, publicStartDate, totalSupply(), maxSupply, maxPreMintCount, maxPublicMintCount);
    }
}