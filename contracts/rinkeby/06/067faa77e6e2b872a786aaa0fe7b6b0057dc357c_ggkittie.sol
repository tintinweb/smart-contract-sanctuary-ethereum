...
contract ggkittie is ERC721A, Ownable {
    ...
    mapping(address=>bool) public hasClaimedtwo;
    ...
    function internalLogic(uint quantity) private {
        ...
        if(quantity == 7 && !hasClaimedtwo[msg.sender]) {
            require(hasClaimedtwo[msg.sender] == false, "already claimed");
            hasClaimedtwo[msg.sender] = true;
            _mint(msg.sender, quantity);
        } else if (quantity == 7 && hasClaimedtwo[msg.sender]) {
            require(msg.value >= cost2, "not enough balance!");
            _mint(msg.sender, quantity);
        } else {
            if(hasClaimedtwo[msg.sender] == false) {
            hasClaimedtwo[msg.sender] = true;
