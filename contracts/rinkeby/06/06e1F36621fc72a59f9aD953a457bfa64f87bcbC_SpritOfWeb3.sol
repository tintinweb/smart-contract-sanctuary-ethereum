...
contract SpritOfWeb3 is ONFT {
    uint public nextMintId;
    uint public maxMintId;

    /// @notice Constructor for the SpritOfWeb3
    /// @param _layerZeroEndpoint handles message transmission across chains
    /// @param _startMintId the starting mint number on this chain
    /// @param _endMintId the max number of mints on this chain
    constructor(address _layerZeroEndpoint, uint _startMintId, uint _endMintId) ONFT("Sprit of web3", "SWEB3", _layerZeroEndpoint) {
    ...
    function mint() external payable {
        require(nextMintId <= maxMintId, "SpritOfWeb3: Max Mint limit reached");
    ...
    function withdraw(address payable _to) external payable onlyOwner {
        require(address(this).balance >= msg.value, "Cannot withdraw more than contract balance.");
