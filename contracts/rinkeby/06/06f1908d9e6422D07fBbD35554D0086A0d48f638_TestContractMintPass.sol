...
contract TestContractMintPass is ERC721, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;

    // Contract controls
    bool public isPaused; //defaults to false
    // TODO: set back to private before deploy
    address public redeemer;
    uint256 public expiration = 3 days; // 3 days

    // counter
    uint256 private _totalSupply = 0; // start with zero
    uint256 private _totalUsed = 0; // start with zero
    uint256 private _totalExpired = 0; // start with zero

    // metadata URIs
    // TODO: set back to private before deploy
    string public _validURI =
        "ipfs://Qmej2vdEZ3j9mAUUcbs2U6ZScXS2rLpcTV1LGvt4gJcoH2/valid.json"; // valid passes
    // TODO: set back to private before deploy
    string public _usedURI =
        "ipfs://Qmej2vdEZ3j9mAUUcbs2U6ZScXS2rLpcTV1LGvt4gJcoH2/used.json"; // used passes
    // TODO: set back to private before deploy
    string public _expiredURI =
        "ipfs://Qmej2vdEZ3j9mAUUcbs2U6ZScXS2rLpcTV1LGvt4gJcoH2/expired.json"; // expired passes

    mapping(uint256 => bool) private usedTokens;
    mapping(uint256 => bool) private expiredTokens;
    mapping(uint256 => uint256) private mintTime;

    constructor() ERC721("TestContractMintPass", "TKCMP") {}
    ...
    /**
     * @dev Returns wether a token has expired
     */
    function isExpired(uint256 tokenId) external view returns (bool) {
        require(_exists(tokenId), "Nonexistent token");
        return expiredTokens[tokenId];
    }
    ...
    function totalValid() external view returns (uint256) {
        return _totalSupply - _totalUsed - _totalExpired;
    }
     ...
     */
    function totalExpired() external view returns (uint256) {
        return _totalExpired;
    }

    function timeUntilExpired(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "Nonexistent token");
        if (block.timestamp - mintTime[tokenId] < expiration) {
        ...
        contractIsNotPaused
    {
        ...
        uint256 tmpTotalMintedTokens = _totalSupply + 1;
        _totalSupply += _addresses.length;

        for (uint256 i; i < _addresses.length; i++) {
            _mint(_addresses[i], tmpTotalMintedTokens);
            mintTime[tmpTotalMintedTokens] = block.timestamp;
            tmpTotalMintedTokens++;
        }
        delete tmpTotalMintedTokens;
    }
    ...
    /**
     * @dev Sets the base uri for valid passes
     */
    ...
    function setAsUsed(uint256 tokenId) external contractIsNotPaused {
        ...
        require(_exists(tokenId), "Nonexistent token");
        require(!usedTokens[tokenId], "Token used");
        require(!expiredTokens[tokenId], "Token expired");
        //TODO: this is untested with chai
        if (timeUntilExpired(tokenId) == 0) {
    ...
    /**
     * @dev Flags the token as expired; anyone call call this
     */
    function setAsExpired(uint256 tokenId) external contractIsNotPaused {
        require(_exists(tokenId), "Nonexistent token");
        require(!usedTokens[tokenId], "Token used");
        require(!expiredTokens[tokenId], "Token expired");
        //TODO: this is untested with chai
        //TODO: this is the most important require!
        // require(timeUntilExpired(tokenId) == 0, "Token still valid");
        _totalExpired++;
        expiredTokens[tokenId] = true;
    }

    /**
     * @dev Owner can force expire a token
     */
    function ownerSetAsExpired(uint256 tokenId) external onlyOwner {
        require(_exists(tokenId), "Nonexistent token");
        require(!usedTokens[tokenId], "Token used");
        require(!expiredTokens[tokenId], "Token expired");
        _totalExpired++;
