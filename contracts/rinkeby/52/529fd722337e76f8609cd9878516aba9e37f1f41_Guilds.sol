    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            if (_tokensLeft == amounts[i]) {
                ...
                AllGuilds[ids[i]].GuildMembers[indexOfAddress(AllGuilds[ids[i]].GuildMembers, from)] = AllGuilds[ids[i]].GuildMembers[AllGuilds[ids[i]].GuildMembers.length - 1];
                AllGuilds[ids[i]].GuildMembers.pop();
