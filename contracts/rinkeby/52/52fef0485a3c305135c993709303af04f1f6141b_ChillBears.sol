...
contract ChillBears is IERC2981, ERC721 {
    ...
    function ogMint1(bytes calldata sig) external payable mintFunc(_preMintSupply, _ogPrice) {
        _ogMint(sig);
        require(amountMinted[msg.sender] < 3, "Can't mint more than 2 tokens!");
    ...
    function ogMint2(bytes calldata sig) external payable mintFunc(_preMintSupply - 1, _ogPrice) {
        _ogMint(sig);
        require(amountMinted[msg.sender] + 2 < 3, "Can't mint more than 2 tokens!");
        
        _preMintSupply -= 2;
        amountMinted[msg.sender] += 2;
        _mint2(msg.sender);
    }
