...
contract BearXNFTStaking is OwnableUpgradeable {
    ...
    function initialize() public initializer{
        ...
        BearXNFTAddress = 0xe961Defe0eced8d723445Ec8250505DFfFAB9BBd;
        ROOTxTokenAddress = 0xD0141d7F46B4556F998f3AD73D93d7e6904D1Bb2;
        SROOTxTokenAddress = 0x8d2d214C4b7C2f3a2b17320628bF06677eAfE9fc;
        totalStakes = 0; 
    ...
    uint256 public _ids_length;
    uint256 public ownerIds_length;
    uint256 public temp_pub;
    function unStake(uint256[] memory _ids) public isOnlyStaker {
        ...
        uint256 temp = 0;
        ownerIds_length = ownerIds.length;
        _ids_length = _ids.length;
         for(uint256 i=0; i<_ids.length;i++) {
    ...
    function claimOf(address _addr) public view returns (uint256, uint256, uint256 )
    {
        ...
        if (sumofgenesisbear >= 5) {
            ...
            for (uint256 i = 0; i < stakes[_addr].length; i++) {
                ///ROOTX
                uint256 dd_root = calDay(stakes[_addr][i].claimedDate_ROOTX);
                if ((isSpecialBear(stakes[_addr][i].nft_id) || isGenesisBear(stakes[_addr][i].nft_id)) && dd_root > 0) {
                    flag = true;
                }
                if (flag) {
                    claimAmountOfROOTxToken = RootX_Store.div(10**18).add(10*dd_root*sumofgenesisbear);
                }
                // else if(!flag) {
            ...
            for (uint256 i = 0; i < stakes[_addr].length; i++) {
                if (isSpecialBear(stakes[_addr][i].nft_id)) {
                    uint256 dd_sroot = calDay(stakes[_addr][i].claimedDate_SROOT);
                    claimAmountOfSROOTxToken = claimAmountOfSROOTxToken.add(200 * (10**18) * dd_sroot);
                    uint256 dd_weth = calDay(stakes[_addr][i].claimedDate_WETH);
                    claimAmountOfWETH = claimAmountOfWETH.add( 200 * (10**18) * dd_weth );
                }
