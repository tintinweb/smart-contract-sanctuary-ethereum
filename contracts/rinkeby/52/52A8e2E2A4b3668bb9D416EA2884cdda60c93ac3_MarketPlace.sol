...
contract MarketPlace is Ownable, Pausable, ReentrancyGuard {
    ...
    function _paySellerAndRoyalties(
        ...
        if (paymentERC20 == address(0)) {
            ...
            payable(_makerWallet).transfer(createrFee);
            payable(seller).transfer(price - platformFee - createrFee);
        } else {
            IERC20(paymentERC20).transferFrom(
                buyer,
                _platformWallet,
                platformFee
            );
            IERC20(paymentERC20).transferFrom(buyer, _makerWallet, createrFee);
            IERC20(paymentERC20).transferFrom(
                buyer,
                seller,
