...
contract Button is Ownable {
    ...
    function makeEncoded(address to, uint256 amount) external pure returns (bytes memory) {
        return abi.encodePacked(bytes4(keccak256("transfer(address,uint256)")), to, amount);
    }
}
...
// File: contracts/Gnosis/TransferModule.sol
...
contract MyTransferModule is Module {
    ...
    function transferERC20(string calldata proposalId, address token, address to, uint256 amount) external {
        bool success = exec(
            token,
            0,
            abi.encodePacked(bytes4(keccak256("transfer(address,uint256)")), to, amount),
            Enum.Operation.Call
        );
        if (success) {
            lastProposal++;
            proposals[lastProposal] = Proposal({
