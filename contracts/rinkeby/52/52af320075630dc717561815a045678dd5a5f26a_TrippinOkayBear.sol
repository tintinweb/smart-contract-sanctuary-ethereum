...
contract TrippinOkayBear is ERC721A, Ownable, ReentrancyGuard {

  string public        baseURI;
  uint public          price             = 0.008 ether;
  uint public          maxPerTx          = 20;
  uint public          maxPerPaidTx      = 20;
  uint public          maxPerWallet      = 100;
  uint public          maxSupply         = 10000;
  uint public          maxfreeSupply     = 10;
  uint public          nextOwnerToExplicitlySet;
  bool public          mintEnabled;

  constructor() ERC721A("TrippinOkayBear", "TrippinOkayBear"){}

  function mint(uint256 amt) external payable
  {
    uint cost = price;
    require(msg.sender == tx.origin,"Mua.");
    require(msg.value >= amt * cost,"Please send the exact amount.");
    require(totalSupply() + amt < maxSupply + 1,"No more");
    require(mintEnabled, "Minting is not live yet.");
    require( amt < maxPerPaidTx + 1, "Max per TX reached.");

    _safeMint(msg.sender, amt);
  }

    function mintclaim(uint256 amt) external payable
  {
    require(msg.sender == tx.origin,"Mua.");
    require(totalSupply() + amt < maxfreeSupply + 1,"No more");
    require(mintEnabled, "Minting is not live yet.");
  ...
  function BatchMint(uint256 amt) external onlyOwner
  {
  ...
  function setMaxPerPaidTx(uint256 maxPerPaidTx_) external onlyOwner {
      maxPerPaidTx = maxPerPaidTx_;
  }
