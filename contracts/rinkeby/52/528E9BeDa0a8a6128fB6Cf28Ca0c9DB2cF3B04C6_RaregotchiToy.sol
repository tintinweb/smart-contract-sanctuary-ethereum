...
contract RaregotchiToy is ERC721 {
    ...
    mapping(address => uint8) private mintedTokensPerWallet;
     ...
     */
    function setDefaultBaseUri(string calldata _baseUri)
        external
        onlyOwner
        contractIsNotFrozen
    {
    ...
    function mint(
        ...
        bytes32 messageHash = generateMintHash(msg.sender, fromTimestamp);
        ...
        returns (string memory)
    {
        ...
        if (tokenOpen[tokenId]) {
            ...
            return
                string(
                    abi.encodePacked(defaultBaseUri, Strings.toString(tokenId))
                );
        }
    }
    ...
    function generateMintHash(address _address, uint256 _fromTimestamp)
        internal
        pure
        returns (bytes32)
    {
