...
contract AdvanceCollectible is ERC721, VRFConsumerBase{
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override{
        ...
        // _setTokenURI(newTokenId, tokenURI); we need to create a new tokenURI`
        tokenCounter = tokenCounter + 1;
    }

    function setTokenURI(uint256 tokenId, string memory _tokenURI) public{
        // we need 3 token uri each for one of the three dogs
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: caller is neither owner nor approoved");
