...
contract JoyDeposit is ContextUpgradeable, AuthorizableU {
    ...
    // State variables
    ////////////////////////////////////////////////////////////////////////
    // Info of each depositor
    struct DepositInfo {
        address addr;                   // deposited address
        uint256 amount;                 // deposited amount
        uint256 time;                   // deposit time
    }    
    struct DepositUser {
        uint256 totalAmount;            // Total deposited amount
        uint256 lastTime;               // Last deposited time
        DepositInfo[] deposits;         // each deposit info
    }
    ...
    // User Info
    mapping(string => DepositUser) public depositUsers;
    string[] public depositUserIds;
    ...
    ////////////////////////////////////////////////////////////////////////
    // Events.
    event TokenDeposited(string userId, address addr, uint256 totalAmount, uint256 amount, uint256 time);
    ...
    function deposit(string memory userId, uint256 tokenAmount) public {
        require(token.balanceOf(_msgSender()) >= tokenAmount, "Depositor's token amount is lack!");
        require(token.allowance(_msgSender(), address(this)) >= tokenAmount, "Depositor's allowance is low!");

        // transfer token from sender to here
        token.safeTransferFrom(_msgSender(), address(this), tokenAmount);

        // transfer token from here to treasury
        if (treasuryAddr != address(0x0)) {
            uint256 transferAmount = token.balanceOf(address(this));
            token.safeTransfer(treasuryAddr, transferAmount);
        }

        // record user deposit state
        DepositInfo memory depositInfo = DepositInfo(_msgSender(), tokenAmount, block.timestamp);
       DepositUser storage depositUser = depositUsers[userId];
       if (depositUser.totalAmount == 0) {
           depositUserIds.push(userId);
           depositUser.totalAmount = depositUser.totalAmount.add(tokenAmount);
           depositUser.lastTime = depositInfo.time;

           
           depositUser.deposits.push(depositInfo);
       }

       // emit token deposit event
       emit TokenDeposited(userId, _msgSender(), depositUser.totalAmount, tokenAmount, depositInfo.time);
    }
}
