...
contract OnCloudNine is ERC721A, Ownable {
    ...
    constructor() ERC721A('On Cloud 9', 'ON9'){
        supply = 0;
        maxMint = 3;
        maxSupply = 4;
    }
    ...
    function mint(uint256 quantity_) public {
        require(isPublicMintEnabled, 'Minting Has Not Began');
        require(supply + quantity_ <= maxSupply, 'Sold Out');
        require(_numberMinted(msg.sender) < maxMint, 'Dont Get Greedy');
        supply++;
        _mint(msg.sender, quantity_);
    }

    function setPublicMintEnabled(bool isMintEnabled_) external onlyOwner {
        isPublicMintEnabled = isMintEnabled_;
    }

    function tokenURI(uint256 tokenId_) public view override returns (string memory) {
        require(_exists(tokenId_), 'Token DNE');
    ...
    function withdraw() external onlyOwner {
        ...
        require(success, 'Sad Day: Withdraw failed');
    }
}
