...
contract LoanOrchestrator {
    ...
    function activateLoan(address _originalSender, uint256 _loanId, uint256[] memory _bids) public {
        // Get Loan
        LoanLibrary.Loan memory loan = loanBorrower.loans(_loanId);

        // The loan borrower must be the one to activate the loan
        require(
            _originalSender == loan.nftHolder,
        ...
        for (uint256 i = 0; i < _bids.length; i++) {
            ...
            LoanLibrary.LoanBid memory bid = loanProvider.loanBids(index);

            if (bid.offered <= loanRemainingBalance) {
                loanProvider.updateLoanBidAcceptedAmount(index, bid.offered);
                ...
                loanProvider.updateLoanBidAcceptedAmount(index, loanRemainingBalance);
                bid.acceptedAmount = loanRemainingBalance;
            }

            // Transfer loan unit to Borrower
            loanTreasury.transferFunds(bid.provider, loan.nftHolder, bid.acceptedAmount);

            loanProvider.updateLoanBidStatus(index, LoanLibrary.LoanBidStatus.ACCEPTED);
        ...
        uint256 tokenId = loanTreasury.mintFractionalizeToken(loanParameters.fractionalizeSupply());
        loan.fractionalizedTokenId = tokenId;

        // Calculate NET
        uint256 totalGrossEarning = 0;
        uint256 totalYearlyInterestEarning = 0;
        for (uint256 i = 0; i < _bids.length; i++) {
            uint256 index = _bids[i];
            LoanLibrary.LoanBid memory bid = loanProvider.loanBids(index);
            totalGrossEarning += LoanLibrary.calculateYearlyGross(bid.acceptedAmount, bid.apr, loanParameters.precision());
            totalYearlyInterestEarning += LoanLibrary.calculateYearlyInterestRevenue(
                bid.acceptedAmount, bid.apr, loanParameters.precision());
        }

        // Calculate % of NET and transfer tokens to provider
        for (uint256 i = 0; i < _bids.length; i++) {
            uint256 index = _bids[i];
            LoanLibrary.LoanBid memory bid = loanProvider.loanBids(index);
            uint256 val = LoanLibrary.calculateYearlyGross(bid.acceptedAmount, bid.apr, loanParameters.precision());
            uint256 res = val.mul(loanParameters.fractionalizeSupply()).div(totalGrossEarning);
            loanTreasury.safeTransferFractionalizeToken(bid.provider, tokenId, res);
        }

        loanBorrower.updateCalculatedApr(
            _loanId, totalYearlyInterestEarning
        .mul(loanParameters.precision())
        .div(loan.loanAmount)
        );
        ...
        uint256 nftTokenId = loanTreasury.mintCollateralToken();
        loanTreasury.safeTransferCollateralToken(loan.nftHolder, nftTokenId);
        loanBorrower.updateLoanNftTokenId(_loanId, nftTokenId);

        /////////////////////////////////////////
        // reference TFL-0017
        /////////////////////////////////////////
        uint256 amount = LoanLibrary.calculatePercentage(
            loan.nftEstimatedValue, loanParameters.loanProcessingFeePercent(), loanParameters.precision());

        loanTreasury.transferFunds(_originalSender, address(loanTreasury), amount);
...
contract LoanBorrower is Ownable {
    ...
    /// @dev loanPayments[loanId] => unit256[] list of payment IDs
    ...
    function createLoanCampaign(
        ...
        require (
            ...
            "The loan duration must be greater than or equal to the minimum duration in days"
        );
        ...
        /////////////////////////////////////////////
        require(_loanAmount >=
            LoanLibrary.calculatePercentage(
                _nftEstimatedValue,
                loanParameters.minLoanPercentageOfCollateral(),
                loanParameters.precision()
            ), "The loan amount must be greater than or equal to the minimum loan percentage of collateral"
        );
        ...
        require(_loanAmount <=
            LoanLibrary.calculatePercentage(
                ...
                loanParameters.precision()
            ), "The loan amount must be less than or equal to the maximum loan percentage of collateral"
        );
    ...
    function updateCalculatedApr(uint256 _loanId, uint256 _calculatedApr) public onlyOrchestratorOrOwner {
        LoanLibrary.Loan storage loan = loanMapping[_loanId];
        loan.calculatedApr = _calculatedApr;
    }
    ...
    function calculateActualInterestToDate(uint256 _loanId) public view returns (uint256) {
        ...
        uint256 currentDurationOfLoanInDays = LoanLibrary.calculateDurationInDays(loan.loanStart, block.timestamp);

        return LoanLibrary.calculateInterestRevenueForDays(loan.loanAmount,
            loan.calculatedApr, loanParameters.precision(), currentDurationOfLoanInDays);
    }

    /// @notice convenient method to get the loan payoff amount
    /// @param _loanId The loan ID
    /// @return The calculated payoff amount
    function calculatePayoffAmount(uint256 _loanId) public view returns (uint256) {
        LoanLibrary.Loan memory loan = loans(_loanId);

        uint256 interestMinLoanDuration = LoanLibrary.calculateInterestRevenueForDays(loan.loanAmount,
            loan.calculatedApr, loanParameters.precision(), loanParameters.minLoanDurationInDays());

        // ######################################
        // Reference TFL-0015
        // ######################################
        uint256 minDurationPenaltyInterest = LoanLibrary.calculateInterestRevenueForDays(loan.loanAmount,
            loanParameters.minPercentOfDurationForInterest(), loanParameters.precision(), loan.loanDuration);
    ...
    function makePayment(uint256 _loanId, uint256 _amount) public onlyOwner {
        ...
        uint256 payOffAmount = calculatePayoffAmount(_loanId);
