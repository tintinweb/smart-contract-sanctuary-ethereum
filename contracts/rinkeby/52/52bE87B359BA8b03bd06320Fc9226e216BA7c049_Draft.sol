...
contract Draft is ERC1155, Ownable, ReentrancyGuard {
  // Sale stage
  enum SaleStages {
    CLOSED,
    PRESALE,
  ...
  function ownerMint(uint256 _tokenId, uint256 _amount) external onlyOwner {
    require(_tokenId < tokenIdCounter, "Token does not exist");
    Token storage token = tokens[_tokenId];
  ...
  function createToken(
    ...
    setTokenInfo(token.id, _price, _supply, _maxPerTx, _maxPerAddr, _merkleRoot, _tokenUri, _onlyOwner);
  }
