...
contract NounSoup is ERC721A, Ownable {
    ...
    string public tokenDescription =
        "Noun Soups are 100% on-chain pop-art. They are a derivative of Nouns, a CC0 project.";
    ...
    function toggleMintingPaused() public onlyOwner {
...
contract NounSoupRenderer is INounSoupRenderer, Ownable {
        ...
        returns (SoupPalette memory)
    {
        ...
        bool isAltDuoTone = (seed % 17) == 0;
        seed = _randomizeSeed(seed);
        uint256 altSeed = isAltDuoTone ? seed : ((seed + 1) * 2);
        seed = _randomizeSeed(seed);
        bool isStandardColors = (seed % 4) == 0;
        seed = _randomizeSeed(seed);
        ...
        seed = _randomizeSeed(seed);
        string memory p2 = isAltDuoTone
            ? _DEFAULT_BADGE_COLOR
            : isStandardColors
            ? _DEFAULT_BADGE_COLOR
            : _PALETTE[(altSeed * 7) % _PALETTE.length];
        string memory s2 = isAltDuoTone ? s1 : isStandardColors
            ? _DEFAULT_BOTTOM_COLOR
        ...
        if (
            seed % 31 == 0 && // rarely
            bytes(p1)[1] != bytes(s1)[1] && // only if the colors are different
    ...
    function _fullSVGForToken(
        ...
        string memory svg3 = string(
            abi.encodePacked(
                ...
                palette.p2,
                _CAN_9,
                palette.isClassic ? _BLACK_COLOR : palette.s1,
                _CAN_10
            )
        );
