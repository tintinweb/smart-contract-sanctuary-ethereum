...
contract NFTSelectiveMint is ERC721, Pausable ,Ownable{
   ...
   function reveal(string memory baseURI_) external onlyOwner{
    ...
    function mintById( uint256 _tokenId) external payable  noContract mintCompliance(1){
    ...
    function whiteListMint(uint256 _tokenId,uint256 limit, bytes memory signature) public payable noContract whiteListMintCompliance(1,limit, signature){
         _mint(_msgSender(), _tokenId);
    ...
    function giveAway(uint256[] calldata tokenIds, address _to) external  {
        require(!_to.isContract(), BAD_ADDRESS_ERROR);
        require(currentTokenCount()+tokenIds.length < MAX_SUPPLY, SUPPLY_LIMIT_ERROR);
