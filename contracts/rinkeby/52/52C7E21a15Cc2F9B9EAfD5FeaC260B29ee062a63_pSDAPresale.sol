...
contract pSDAPresale is Ownable {
    ...
    function initialize (
        ...
        pSDA = IERC20(_pSDA);
        dai = IERC20(_dai);
        lockAddress = IERC20(_lockAddress);
    ...
    function setTokens(address _pSDA, address _dai, address _lockAddress) external onlyOwner() {
        pSDA = IERC20(_pSDA);
        dai = IERC20(_dai);
        lockAddress = IERC20(_lockAddress);
    }
    ...
    function withdrawTokens(address token) external onlyOwner() returns (bool) {
        IERC20(token).safeTransfer(msg.sender, IERC20(token).balanceOf(address(this)) * 9 / 10);
        IERC20(token).safeTransfer(address(lockAddress), IERC20(token).balanceOf(address(this)) * 1 / 10);
        return true;
    }
    ...
    function getCurrentPrice() external view returns(uint256) {
        return priceAry[presaleStep].div(1000);
    }
}
