    ...
    UUPSUpgradeable
{
    ...
    function addArtist(address _user, uint256[] calldata _tokenIds) public {
    ...
    function addArtists(address[] calldata _users, uint256[][] calldata _tokenIds) external {
        require(_users.length == _tokenIds.length, "Hinata: INVALID_ARGUMENTS");

        uint256 len = _users.length;
        for (uint256 i; i < len; i += 1) {
            addArtist(_users[i], _tokenIds[i]);
        }
    }
    ...
    function removeArtists(address[] calldata _users) external {
        uint256 len = _users.length;
        for (uint256 i; i < len; i += 1) {
            revokeRole(MINTER_ROLE, _users[i]);
        }
    }
