...
contract MyFirstNFT is ERC721Enumerable, Ownable { 
    ...
    string public baseURI = "ipfs://QmNe3a4cB2ywGnMWXXYymkiditYHL1vLdp5E9dcaTW2abC"; // 这一行是NFT 该去哪里找你的MetaData ||| 活值
    bool public paused = false; // 可以拿来暂停或者开启Mint 
    uint256 public cost = 0.0001 ether; // Mint 价格
    uint256 public maxSupply = 5; // 只有五个NFT 
    uint256 public maxMintAmount = 1; // 一次最多只能Mint 一个
    ...
    // 不用经过我们自己设定的判断直接Mint(设定为private 且只有onlyOwner 可以使用) 
    function safeMint(address to, uint256 tokenId) private onlyOwner { 
        _safeMint(to, tokenId); 
    }

    // 提供给大家Mint 的Function(另外加上payable 代表这个function 可以接收乙太币) 
    function mint(address _to, uint256 _mintAmount) public payable { 
        require(paused != true, "Sale must be active"); // 合约必须不是暂停
        require(_mintAmount > 0); // 每次必须铸造超过0 个
        require(_mintAmount <= maxMintAmount, "You can only adopt 1 BigBenFun at a time"); // 铸造的数量不可以大于每次最大铸造数量
        require(cost * _mintAmount <= msg.value, "Ether value sent is not correct"); // Mint 的价格不可以少于我们订定的价格

        for(uint256 i = 0; i < _mintAmount; i++) { 
            uint256 mintIndex = _tokenIdCounter.current(); 
            // 查看接下来要Mint 的NFT 是否有人持有了，如果已经持有就跳过
            while(_exists(mintIndex)){ 
                i++; 
            } 
            // 接下来如果Mint NFT 的编号如果小于初始提供的数量就让使用者Mint 
