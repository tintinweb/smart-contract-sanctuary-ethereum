...
contract oUSDCToken is CheckContract, IoUSDCToken, Ownable {
    using SafeMath for uint256;
    
    uint256 private _totalSupply;
    string constant internal _NAME = "ORUM USDC";
    string constant internal _SYMBOL = "oUSDC";
    string constant internal _VERSION = "1";
    uint8 constant internal _DECIMALS = 6;
    ...
    function _requireCallerIsBorrowerOps() internal view {
        require(msg.sender == borrowerOpsAddress, "oUSDC: Caller is not the BorrowerOps");
    }

    function _requireCallerIsStakingPool() internal view {
        require(msg.sender == stakingPoolAddress, "oUSDC: Caller is not the StakingPool");
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        _approve(sender, msg.sender, _allowances[sender][msg.sender].sub(amount, "oUSDCToken: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) external override returns (bool) {
        _approve(msg.sender, spender, _allowances[msg.sender][spender].sub(subtractedValue, "oUSDCToken: decreased allowance below zero"));
        return true;
    }
    ...
    (
        ...
        override 
    {            
        require(deadline >= block.timestamp, 'oUSDC: expired deadline');
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01', 
                         domainSeparator(), keccak256(abi.encode(
                         _PERMIT_TYPEHASH, owner, spender, amount, 
                         _nonces[owner]++, deadline))));
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress == owner, 'oUSDC: invalid signature');
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        _balances[sender] = _balances[sender].sub(amount, "oUSDCToken: transfer amount exceeds balance");
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        _balances[account] = _balances[account].sub(amount, "oUSDCToken: burn amount exceeds balance");
