  ...
  PausableUpgradeable
{
  ...
  ) external override nonReentrant whenNotPaused unusedNonce(nonce) returns (bool) {
    address signer = keccak256(abi.encode(receiver, amount, nonce))
      .ethSignedMessage()
      .recover(signature);
    require(signer == TRUSTED_PARTY, "Shiniki: Invalid signature");
    return true;
  }

  function setUsedNonce(bytes32 nonce) public onlyOwner {
