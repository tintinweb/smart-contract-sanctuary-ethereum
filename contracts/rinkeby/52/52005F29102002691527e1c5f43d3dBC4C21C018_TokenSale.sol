...
contract TokenSale  is Crowdsale {
    ...
    uint256 public initialSalePrice = 21505;
    uint256 public seedSalePrice = 10752;
    uint256 public finalSalePrince = 3225;
...
Stages public stage = Stages.initialSale;
 constructor(address payable wallet,IERC20 token) Crowdsale(_rate, wallet, token) public
    {

    }
 function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal  {
        super._preValidatePurchase(beneficiary,weiAmount);
        uint256 requestedToken=_getTokenAmount(weiAmount);
        require( requestedToken <= totalTokens, "Token not Available");
        totalTokens = totalTokens - requestedToken;
        if (totalTokens == 0) {
            setTokenStage();
        }
    }
    ...
    function setTokenStage() internal {
        ...
        if (stage == Stages.initialSale && totalTokens == 0) {
            ...
            _rate= finalSalePrince;
            totalTokens = RemaingSaleQty;
        }
    }

    function setFinalSalePrice(uint256 _finalSalePrince) public{
        finalSalePrince=_finalSalePrince;
    }
