...
contract CallMyName {
    function callMe(string memory _name) external returns (string memory) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();

        bytes4 selector = bytes4(keccak256('hello()'));
        address facet = ds.facetAddressAndSelectorPosition[selector].facetAddress;

        (bool success, bytes memory result) = facet.delegatecall(abi.encodeWithSelector(selector));
        require(success, "delegatecall fail");

        return string(bytes.concat(result, abi.encodePacked(_name)));
    }
}
