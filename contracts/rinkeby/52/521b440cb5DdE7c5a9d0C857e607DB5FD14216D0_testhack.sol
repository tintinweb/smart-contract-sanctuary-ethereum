...
/// @author solipsis
contract testhack is Ownable, ERC721A, ReentrancyGuard, ERC2981 {

    constructor() ERC721A("testhack", "THACK") {
        stage = Stage.INITIAL;
    ...
    */
    uint256 public immutable collectionSize = 10101;

    /// @dev baseURI for all ERC721 Metadata interactions
    string private _baseTokenURI;

    /// @dev thack foundation address, exempt from individual address minting cap
    address private treasuryAddress;

    /// @dev thack controlled allowlist signer
    address private allowlistSignerAddress;
    ...
    /// @notice withraw current contract balance to current contract owner
    ...
    /// @notice sets the thack treasury address
    ...
    /// @param _receiver recipient address of royalties
    /// @param _basisPoints basis points numerator
    /// @dev see EIP-2981. Default denominator == 10000
    function setRoyaltyInfo(address _receiver, uint96 _basisPoints) external onlyOwner {
        _setDefaultRoyalty(_receiver, _basisPoints);
    }
    ...
    /// @param signature thack provided signature over other parameters
        ...
        onlyStage(Stage.ALLOWLIST_SALE)
    {
        ...
        // TODO: memoize improve gas?

        // treasury address is exempt from minting limits
        if (numberMinted(msg.sender) + quantity > maxPerAddress && msg.sender != treasuryAddress) revert ExceededAllowlistMaxPerAddress();
        ...
        onlyStage(Stage.PUBLIC_SALE)
    {
        ...
        if (totalSupply() + quantity > collectionSize) revert ExceededMaxSupply();
        if (quantity > publicSaleMaxBatch) revert ExceededPublicSaleMaxBatch();
        if (numberMinted(msg.sender) + quantity > publicSaleMaxPerAddress && msg.sender != treasuryAddress) revert ExceededPublicSaleMaxPerAddress();
    ...
    /// @param key thack provided sale key
    ...
    function configureAllowlistSale(uint32 key, uint32 maxBatch, uint32 maxPerAddress, uint64 price) 
        external
        onlyOwner
    {
    ...
    /// @param maxPerAddress maximum cumulative mints for each unique address
    /// @param price sale price in wei
    function configurePublicSale(uint32 key, uint32 maxBatch, uint32 maxPerAddress, uint64 price) 
        external
        onlyOwner
    {
    ...
    /// @notice returns cumulative and current hacking stats
