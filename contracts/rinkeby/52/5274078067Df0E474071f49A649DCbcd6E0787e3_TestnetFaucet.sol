...
import "./frax/GodModeErc20.sol";
import "../TempleERC20Token.sol";

contract TestnetFaucet {
    mapping(address=>uint256) public lastPayout;
    TempleERC20Token public temple;
    GodModeErc20 public frax;
    FakeERC20 public fei;

    uint256 constant PAYOUT_AMOUNT = 10000000000000000000000;

    constructor(TempleERC20Token _temple, GodModeErc20 _frax, FakeERC20 _fei) {
        temple = _temple;
        frax = _frax;
        fei = _fei;
    }
    ...
    function mint() external {
        require((lastPayout[msg.sender] == 0) || (block.timestamp - lastPayout[msg.sender] > 86400), "Can only receive payout once per day");

        temple.mint(msg.sender, PAYOUT_AMOUNT);
        frax.mint(msg.sender, PAYOUT_AMOUNT);
        fei.mint(msg.sender, PAYOUT_AMOUNT);
