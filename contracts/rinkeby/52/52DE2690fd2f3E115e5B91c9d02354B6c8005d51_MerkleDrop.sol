...
contract MerkleDrop is AccessControlEnumerable {
    ...
    event NFTClaimed(uint256[] indexed tokenIds, address indexed receiver);
    ...
    function claim(
        ...
        uint256 minted = _getMintedAmount(_receiver, mintableIds, collectibles);
        uint256 willingToMint = _getWillingToMint(_amountsOverride);

        if ((minted + willingToMint) > collateralBalance) {
    ...
    function _getMintedAmount(
        address _receiver,
        uint256[] memory mintableIds,
        Collectibles collectibles
    ) private view returns (uint256) {
        address[] memory receiver = new address[](3);
        receiver[0] = _receiver;
        receiver[1] = _receiver;
        receiver[2] = _receiver;

        uint256[] memory balances = collectibles.balanceOfBatch(receiver, mintableIds);
        uint256 minted = 0;

        for (uint256 i = 0; i < balances.length; i++) {
            minted = minted + balances[i];
        }

        return minted;
    }

    function _getWillingToMint(uint256[] calldata _amounts) private pure returns (uint256) {
        uint256 willingToMint = 0;

        for (uint256 i = 0; i < _amounts.length; i++) {
            willingToMint = willingToMint + _amounts[i];
        }

        return willingToMint;
    }
