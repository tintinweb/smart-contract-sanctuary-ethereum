...
contract Land is ERC721, ERC2981, ReentrancyGuard, Ownable, Pausable {
    ...
    *
    */
    uint16 public maxMintPerAddress = 5;

    /**
    * @notice Default sale status on deployment is public or 1
    *
    */
    ...
    */
    ...
    /**
    * @dev Fired in `setSaleStatus()`
    *
    * @param _saleStatus new saleStatus
    */
    ...
    */
    ...
    /**
    * @param _tokenId token id to check if single elite token and
    *        non-elite token are premitted to mint based
    *        on sale mode
    */
    ...
    /**
    * @param _tokenIds token id to check if elite tokens (batch) and
    *        non-elite tokens (batch )are premitted to mint based
    *        on sale mode
    */
    ...
    /**
    * @dev Throws if _tokenId is not permitted to mint on the
    *      current state of saleStatus
    *
    * @dev saleStatus: 1 is public sale, salesStatus: 2 is private sale
    *      and saleStatus: 0 is open sale
    */

    // **NOTE: Elite and non-elite token ranges should be change before mainnet deployment
    function saleMode(uint256 _tokenId) view private {
        if (saleStatus == 1) { // public
            require(_tokenId >= 1 && _tokenId <= 16, "Elite token mint not allowed");
        }

        if (saleStatus == 2) { // private
            require(_tokenId >= 17 && _tokenId <= 20, "Non-elite token mint not allowed");
        }

        if (saleStatus == 0) { // open
            require(_tokenId >= 1 && _tokenId <= 20, "Invalid token id");
        }
    }
