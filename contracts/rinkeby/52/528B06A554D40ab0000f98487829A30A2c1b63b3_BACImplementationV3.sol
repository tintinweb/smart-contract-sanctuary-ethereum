...
contract BACImplementationV3 is ERC721A, AccessControl, VRFConsumerBaseV2 {
    enum State {
        InitMint,
        BulkMint,
        ComboMint,
    ...
    bool public IsRarityListHashSet = false;
    string public reorderedListHash;
    bool public IsReorderedListHashSet = false;
    bool public IsFileKeySet = false;
    ...
    constructor(address contractAddress) ERC721A("Bored Ape Comic #2", "BAC2") VRFConsumerBaseV2(vrfCoordinator) {
    ...
    function setupCoordinatorLinkKeyHash(address _coordinator, address _link, bytes32 _keyHash) external onlyRole(OPERATOR_ROLE) {
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        if (IsRarityListHashSet) {
            revert RarityListHashError();
        }
        rarityListHash = _hash;
        IsRarityListHashSet = true;
    }
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
    {
        if (IsReorderedListHashSet) {
            revert ReorderedListHashError();
        }

        reorderedListHash = _hash;
        IsReorderedListHashSet = true;
    }

    function setKey(string memory _fileKey)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
    {
        if (IsFileKeySet) {
    ...
    function fulfillRandomWords(
        ...
        emit RandomWordsGenerated(randomWords);
    }
