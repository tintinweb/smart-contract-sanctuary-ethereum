...
contract HYCollectionMintPass is Ownable, ERC721A, ReentrancyGuard {
    uint256 public constant PRICE = 100000000000000000;
    uint256 public constant MAX_PER_WALLET = 2;
    uint256 public constant MAX_PER_TRX = 2;
    uint256 public constant SUPPLY = 111;
    ...
    constructor() ERC721A("Huayu Collection Mint Pass", "HYCMP") {}
    ...
    modifier mintGuard(uint256 tokenCount) {
        ...
        require(totalSupply() + tokenCount <= SUPPLY, "Not enough supply");
        require(tokenCount <= MAX_PER_TRX, "Exceeds token txn limit ");
        _;
    }

    function reserve(uint256 amount) external onlyOwner {
        require(totalSupply() + amount <= SUPPLY, "Not enough supply");
        require(amount % MAX_PER_TRX == 0);

        uint256 chunks = amount / MAX_PER_TRX;
        for (uint256 i = 0; i < chunks; i++) {
            _safeMint(msg.sender, MAX_PER_TRX);
        }
    }
    ...
    function mint(uint256 amount) external payable mintGuard(amount) {
        ...
        require(
            publicAddressMintCount[msg.sender] + amount <= MAX_PER_WALLET,
        ...
        mintGuard(amount)
    {
        ...
        require(
            presaleAddressMintCount[msg.sender] + amount <= MAX_PER_WALLET,
            "Address already minted allocation"
        );
