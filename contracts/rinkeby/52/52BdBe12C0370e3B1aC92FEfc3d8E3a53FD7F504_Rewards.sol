...
interface IRewards {
    ...
    function oliveToken() external view returns (IERC20);
...
contract Rewards is Ownable, IRewards {
    ...
    IERC20 public immutable override oliveToken;
    address public override rewardsSigner;

    constructor(IERC20 token, address signerAddress) {
        require(address(token) != address(0), "Invalid LAAS Address");
    ...
    function claim(
        ...
        require(oliveToken.balanceOf(address(this)) >= claimableAmount, "Insufficient Funds");

        claimedAmounts[recipient.wallet] = claimedAmounts[recipient.wallet] + claimableAmount;

        oliveToken.safeTransfer(recipient.wallet, claimableAmount);
