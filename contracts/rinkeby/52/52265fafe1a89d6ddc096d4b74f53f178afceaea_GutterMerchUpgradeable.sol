...
contract GutterMerchUpgradeable is ERC1155Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	struct SaleState {
		...
		uint256 publicMaxPerWallet; // for public mint
		mapping (address => uint256) userPublicMinted; // for public mint
	...
	function publicMint(uint256 id, uint256 qty) external payable nonReentrant {
		...
		saleStates[id].totalMinted += qty;
		saleStates[id].userPublicMinted[msg.sender] += qty;
	}
	...
	function _validatePublicMint(
		...
		require(saleStates[id].userPublicMinted[sender] + qty <= saleStates[id].publicMaxPerWallet, "max per wallet exceeded");
	}
	...
	function setSaleState(
		...
		saleStates[id].publicMaxPerWallet = _publicMaxPerWallet;
	}
