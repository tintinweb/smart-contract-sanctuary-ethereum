...
contract OCTK1 is ERC1155, Ownable {
    ...
    constructor(uint256 initialAmount, uint256[] memory punderations, string memory baseUri, string[] memory links) ERC1155("") Ownable() {
        require(links.length == punderations.length, string(abi.encodePacked("Links length and punderations length should be equals to : ", links.length)));
        _baseUri = baseUri;
        _links = links;
    ...
    function _check(uint256 id) internal view{
        require(id >= 1 && id <= _links.length, string(abi.encodePacked("Non existant token: ", id)));
    }
    ...
    function setLinks(string[] calldata links) public onlyOwner {
        require(links.length == _links.length, string(abi.encodePacked("Links length should be equals to : ", _links.length)));
