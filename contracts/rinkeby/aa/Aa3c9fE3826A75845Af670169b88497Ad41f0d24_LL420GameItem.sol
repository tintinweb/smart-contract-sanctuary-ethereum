...
contract LL420GameItem is ERC1155, ReentrancyGuard, Ownable, Pausable {
    ...
    uint32[4] public supplyCaps = [420, 200, 350, 2700];
    uint32[4] public prices = [12600, 42000, 70000, 35000];
     ...
     *
     * @param _who Game item will be sent to this address
     * @param _id Game item id 0 - 1
    ...
    function buyA(
        ...
        if (_id != ROLL_PAPER_ID && _id != HOODIE_ID) revert InvalidId();
        ...
        if (_id == ROLL_PAPER_ID) {
            ...
            uint256 burnt = locker.burntAmount(_who);
            if (burnt == 0) revert NotEnoughBurnt();
        }

        ILL420Wallet wallet = ILL420Wallet(walletAddress);
        uint256 totalPrice = prices[_id];
        if (wallet.balance(_who) < totalPrice) revert NotEnoughBalance();
        wallet.withdraw(totalPrice);

        _mint(_who, _id, _amount, "");
    }
