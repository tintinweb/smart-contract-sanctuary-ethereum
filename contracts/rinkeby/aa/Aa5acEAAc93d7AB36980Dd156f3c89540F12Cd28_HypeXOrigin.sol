...
contract HypeXOrigin is ERC1155Supply, Ownable, ReentrancyGuard {
    ...
    function flipSaleState() external onlyOwnerOrOperator {
    ...
    function setWhitelistedRecipient(address recipient, bool approved)
        external
        onlyOwnerOrOperator
    {
        require(recipient != address(0), "Origin: ZERO_ADDRESS");
        whitelistedRecipients[recipient] = approved;
    }

    function mintNFTs(uint256 amount) external payable nonReentrant {
        require(saleIsActive, "Origin: SALE_INACTIVE");
