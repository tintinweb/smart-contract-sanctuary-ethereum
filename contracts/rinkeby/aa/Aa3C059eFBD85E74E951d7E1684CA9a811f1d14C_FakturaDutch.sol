...
UUPSUpgradeable
{
    ...
    mapping(uint => Sales) private salesPhase;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(_revealed) {
            return string(abi.encodePacked(_tokenBaseURI, _toString(tokenId), ".json"));
        } else {
    ...
    function getSalesPhase(uint256 n) public view returns (uint) {
        require(block.timestamp > auction.startDate, "Auction not started yet.");
        uint256 decreases = MathUpgradeable.min(
            (block.timestamp - auction.startDate) / auction.decreaseInterval,
                auction.numDecreases
    ...
    function safeMint(uint256 amount, Coupon memory coupon) public payable {
        require(_totalMinted() + amount <= expectedTokenSupply - mintReserve, "There's no token to mint.");
        require(_numberMinted(msg.sender) + amount <= maxMintPerAddress, "One address can mint 10 tickets.");
        require(msg.value >= getPrice(amount), "Mint price is not correct.");
