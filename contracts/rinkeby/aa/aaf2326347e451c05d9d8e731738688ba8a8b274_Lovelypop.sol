...
contract Lovelypop is Ownable, ERC721A {
    ...
    // metadata URI
    string private _baseTokenURI = " https://gateway.pinata.cloud/ipfs/QmfGDsRszzn2Cc6wCZG6HNEmVjFnCFnFdStuSBbxKKpEGN/";

    constructor() ERC721A("Lovelypop", "LP") {
    }
    ...
    function mint(uint256 _mintAmount) external payable callerIsUser{
        ...
        uint256 priceWei;
        if (minted > 0) {
            priceWei = _mintAmount * publicPrice;
        } else {
            priceWei = (_mintAmount - 1) * publicPrice;
        }
