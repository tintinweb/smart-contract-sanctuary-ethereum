...
// File: contracts/FusionFrames.sol
...
contract FusionFrames is ERC721URIStorage, ERC1155Holder, Ownable {
    ...
    uint256[] public befores;
    uint256[] public afters;
    ...
    //Check that the tokenIds are from the same series
    function _isFromSameSeries(uint256[] memory ids) internal pure returns (bool) {
    ...
    function _isACombo(uint256[] memory array, uint256 series) internal view returns (bool) {
        ...
        require(_isFromSameSeries(array)==true);
        bool result = false;
    ...
    function _reduce(uint256[] memory array, uint256 series) internal pure returns (uint256[] memory) {
        uint256[] memory reducedArray = array;
    ...
    function sendFramesBatch(address from, uint256[] memory ids, uint256[] memory ids2, uint256[] memory amounts, bytes memory data) public virtual {
        require(_isArrayEqual(ids, ids2) == true);
        require(_isFromSameSeries(ids)==true);
        uint256 series = _getSeries(ids[0]);
        require(_isACombo(ids,series)==true);
        afters = ids;
        framesContract.safeBatchTransferFrom(from, address(this), ids2, amounts, data);
