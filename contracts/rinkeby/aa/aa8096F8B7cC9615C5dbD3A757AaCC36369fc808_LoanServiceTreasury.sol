...
contract LoanServiceTreasury is Ownable {
    ...
    /// @param _loanId The loanId for which the nft is being returned
    /// @param _recipient The recipient of the NFTs being claimed
    function claimNft(uint256 _loanId, address _recipient) public {

        address loanServiceAddress = LoanOracle_I(getLoanOracleAddress()).getLoanServiceAddress();
        LoanCommon.Loan memory loan = LoanService_I(loanServiceAddress).loans(_loanId);
        require(loan.status == LoanCommon.LoanStatus.PAID, "The loan must be paid in full");

        address loanStorageNftCollateralAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageNftCollateralAddress();
        LoanCommon.NftCollateral memory collateral =
            LoanStorageNftCollateral_I(loanStorageNftCollateralAddress).collaterals(_loanId);

        address loanStorageTreasuryAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageTreasuryAddress();
        address collateralTokenAddress = LoanStorageTreasury_I(loanStorageTreasuryAddress).collateralToken();

        // make sure msg.sender owns collateral token
        address ownerOf = ERC721_I(collateralTokenAddress).ownerOf(loan.loanCollateralNftTokenId);
        require(ownerOf == msg.sender, "sender must own the collateral token");

        // Burn Token
        LoanStorageTreasury_I(loanStorageTreasuryAddress).burnCollateralToken(loan.loanCollateralNftTokenId);
        ...
        for (uint256 i = 0; i < collateral.nftContracts.length; i++) {
            if (collateral.standards[i] == LoanCommon.Standard.ERC721) {
                LoanStorageTreasury_I(loanStorageTreasuryAddress).erc721safeTransferFrom(
                    collateral.nftContracts[i],
                    loanStorageTreasuryAddress,
                    _recipient,
                    collateral.nftTokenIds[i]);
            } else if (collateral.standards[i] == LoanCommon.Standard.ERC1155) {
                LoanStorageTreasury_I(loanStorageTreasuryAddress).erc1155SafeTransferFrom(
                    collateral.nftContracts[i],
                    loanStorageTreasuryAddress,
                    _recipient,
                    collateral.nftTokenIds[i],
    ...
    function addFunds(address _recipient, uint256 _amount) public onlyServiceContractsOrOwner {
        require (_recipient != address(0x0), "valid recipient required");
        address loanStorageTreasuryAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageTreasuryAddress();
        LoanStorageTreasury_I(loanStorageTreasuryAddress).addFunds(_recipient, _amount);
    }

    /// @notice get account balance
    /// @param _wallet The wallet address
    function account(address _wallet) public view returns (uint256) {
        address loanStorageTreasuryAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageTreasuryAddress();
        return LoanStorageTreasury_I(loanStorageTreasuryAddress).account(_wallet);
    }
    ...
    function mintFractionalizeToken(uint256 _supply) public returns (uint256) {
        address loanStorageTreasuryAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageTreasuryAddress();
        return LoanStorageTreasury_I(loanStorageTreasuryAddress).mintFractionalizeToken(_supply);
    }

    /// @notice the address of which profits are tracked
    function profitAddress() public view returns (address) {
        address loanStorageTreasuryAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageTreasuryAddress();
        return LoanStorageTreasury_I(loanStorageTreasuryAddress).profitAddress();
    }
    ...
    function transferFunds(address _from, address _to, uint256 _amount) public onlyServiceContractsOrOwner {
        address loanStorageTreasuryAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageTreasuryAddress();
        LoanStorageTreasury_I(loanStorageTreasuryAddress).transferFunds(_from, _to, _amount);
    }
    ...
    function fractionalizeTokenSafeTransferFrom(address _to, uint256 _tokenId, uint256 _amount) public onlyServiceContractsOrOwner {
        address loanStorageTreasuryAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageTreasuryAddress();
        LoanStorageTreasury_I(loanStorageTreasuryAddress).fractionalizeTokenSafeTransferFrom(_to, _tokenId, _amount);
    }
    ...
    function mintCollateralToken() public onlyServiceContractsOrOwner returns (uint256) {
        address loanStorageTreasuryAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageTreasuryAddress();
        return LoanStorageTreasury_I(loanStorageTreasuryAddress).mintCollateralToken();
    }
    ...
    function collateralTokenSafeTransferFrom(address _to, uint256 _tokenId) public onlyServiceContractsOrOwner {
        address loanStorageTreasuryAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStorageTreasuryAddress();
        LoanStorageTreasury_I(loanStorageTreasuryAddress).collateralTokenSafeTransferFrom(_to, _tokenId);
    }
}
