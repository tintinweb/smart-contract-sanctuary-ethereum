...
contract Uppercut is ERC721A, Ownable, ReentrancyGuard {
    ...
    // Real winter contract address: 0xd541da4C37e268b9eC4d7D541Df19AdCf564c6A9
    address public constant WINTER_ADDRESS =
    ...
    function mintMultiple(
        ...
        // user address and
        require(
            (_transferTo == msg.sender) ||
                (_transferTo != msg.sender && msg.sender == WINTER_ADDRESS),
            "Invalid addresses"
        );

        if (
            (_transferTo == msg.sender && _transferTo != WINTER_ADDRESS) ||
            (_transferTo != msg.sender) // implied that msg.sender is WINTER_ADDRESS
        ) {
            require(
