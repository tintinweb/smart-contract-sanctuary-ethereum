...
contract AFTERLIF3 is ERC721A, ERC2981Base, Ownable, ReentrancyGuard {
  ...
  uint256 internal constant COST = 0.088 ether;
  ...
  mapping(address => bool) public _allowedListClaimed;
  ...
  modifier isMintPaymentCompliant(uint256 mintQuantity) {
    require(msg.value >= COST * mintQuantity, "Insufficient funds");
    _;
  }
  ...
  modifier isAllowedListCompliant(
    ...
    require(
      !_allowedListClaimed[msg.sender],
      "Mint allocation already claimed"
    );
    require(
  ...
  {
    _allowedListClaimed[msg.sender] = true;
    _safeMint(msg.sender, mintQuantity);
  }
  ...
  function _validateTransaction(bytes32[] memory proof, address _user)
    internal
    view
    returns (bool)
  {
  ...
  function _merkleVerify(bytes32[] memory proof, bytes32 hashedLeaf)
    ...
    returns (bool)
  {
    return MerkleProof.processProof(proof, hashedLeaf) == _merkleRootHash;
  }
  ...
  function setClaimedStateForAddress(bool _state, address _address)
    ...
    onlyOwner
  {
    _allowedListClaimed[_address] = _state;
  }
...
contract $AFTERLIF3 is AFTERLIF3 {
    ...
    function $_validateTransaction(bytes32[] calldata proof,address _user) external view returns (bool) {
        return super._validateTransaction(proof,_user);
    }
