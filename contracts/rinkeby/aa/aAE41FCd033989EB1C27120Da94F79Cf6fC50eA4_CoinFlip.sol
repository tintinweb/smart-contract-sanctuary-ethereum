...
contract CoinFlip is VRFConsumerBaseV2 {
    ...
    event Result(uint256 id, uint256 bet, uint256 randomSeed, uint256 amount, address player, uint256 winAmount,bool winResult, uint256 s_randomWords, uint256 time);
    ...
    function verdict(uint256 random) public payable onlyVFRC {
        ...
        emit Result(games[i].id, games[i].bet, games[i].seed, games[i].amount, games[i].player, winAmount,winResult,random, block.timestamp);
        }
        //save current gameId to lastGameId for the next betting round
        lastGameId = gameId;
    }

    function payWinner() public payable onlyVFRC{
