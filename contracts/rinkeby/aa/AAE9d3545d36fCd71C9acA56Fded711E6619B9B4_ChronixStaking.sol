...
contract ChronixStaking is Initializable, ReentrancyGuardUpgradeable, ContextUpgradeable {
    ...
    event AutoDeposit(address indexed staker,address contractAddress,uint256 tokensAmount);
    ...
    function getPotTax(address staker) public view returns(uint) {
        ...
        if(user.stakedChronix.length !=0 || ERC721A(PushersNFT).balanceOf(staker) !=0){
    ...
    function registerDeposit(address ownerOfToken, address contractAddress, uint256[] memory tokenIds) public authorised {
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        require(ERC721A(contractAddress).ownerOf(tokenIds[i]) == address(this), "!Owner");
        require(ownerOf(contractAddress, tokenIds[i]) == address(0), "Already deposited");
        _ownerOfToken[contractAddress][tokenIds[i]] = ownerOfToken;
        user.stakedKits.push(tokenIds[i]);
        newYield +=traitsForContract[contractAddress];
      }
      
      accumulate(ownerOfToken);
      user.currentYield = newYield;
      emit AutoDeposit(ownerOfToken, contractAddress, tokenIds.length);
    }
