...
contract PolicyBook is IPolicyBook, ERC20PermitUpgradeable, AbstractDependant {
    ...
    function _submitClaimAndInitializeVoting(
        ...
        uint256 virtualEndEpochNumber =
            policyHolders[_msgSender()].endEpochNumber.tryAdd(virtualEpochs);
        ...
        if (!appeal) {
            ...
            uint256 endLockEpoch =
                Math.max(
                    getEpoch(claimingRegistry.claimEndTime(claimIndex)).tryAdd(1),
        ...
        if (
            ...
            uint256 endUnlockEpoch =
                Math.max(
                    ...
                    policyHolders[claimer].endEpochNumber.tryAdd(virtualEpochs)
                );
        ...
        returns (uint256 newTotalCoverTokens, uint256 newTotalLiquidity)
    {
        ...
        for (uint256 i = lastDistributionEpoch; i < newDistributionEpoch; i = uncheckedInc(i)) {
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 _reinsurancePrice =
            _totalPrice.mul(PROTOCOL_PERCENTAGE).uncheckedDiv(PERCENTAGE_100).sub(
                _distributorFeeAmount
            );
    ...
    function endActivePolicy(address _holder) external override onlyClaimingRegistry {
        ...
        uint256 _virtualEndEpochNumberPrevious =
            policyHolders[_holder].endEpochNumber.tryAdd(virtualEpochs);
        ...
        )
    {
        ...
        _maxCapacities = _totalSTBLLiquidity.trySub(newTotalCoverTokens);

        _buyPolicyCapacity = _maxCapacities.trySub(_getPendingWithdrawalAmount());
        ...
        if (possibleCoverage >= MINUMUM_COVERAGE) {
            ...
            _annualInsuranceCost = _annualInsuranceCost
                .mul(ANNUAL_COVERAGE_TOKENS.mul(PRECISION).tryDiv(possibleCoverage))
