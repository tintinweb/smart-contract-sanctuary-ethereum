...
contract MetaMint is ERC721A, Ownable{
    ...
    address public constant MM_VAULT = address(0x79b6067682413DfA8AA99552a627E47B4B28c208);
    ERC1155 public constant OS_STOREFRONT = ERC1155(0x495f947276749Ce646f68AC8c248420045cb7b5e);
    uint256 public constant OS_METAMINT_TOKENID =
        75079589279640323933009448590078242754813563268524912197102354240542638866932;
    ...
    function claimTokens() external isRefunding {
    ...
    /*
        View Functions
    */
    function isTokenFlagged(uint256 tokenId_) public view returns (bool) {
        for (uint256 i = 0; i < flaggedTokens.length; i++) {
            if (flaggedTokens[i] == tokenId_) return true;
        }
        return false;
    }
    ...
    /*
        Team Functions
    */
    function flagToken(uint256 tokenId_) external onlyOwner {
        flaggedTokens.push(tokenId_);
        emit KeyFlagged(msg.sender, tokenId_);
    }

    function unflagToken(uint256 tokenId_) external onlyOwner {
        for (uint256 i = 0; i < flaggedTokens.length; i++) {
            if (flaggedTokens[i] == tokenId_) {
                flaggedTokens[i] = flaggedTokens[flaggedTokens.length - 1];
                flaggedTokens.pop();
                break;
            }
        }
    ...
    function _beforeTokenTransfers(address from,address to,uint256 startTokenId,uint256 quantity) internal override {
        if (owner() != msg.sender) {
            for (uint256 i = startTokenId; i < startTokenId + quantity; i++) {
                require(!isTokenFlagged(i), "Token is flagged");
            }
        }
