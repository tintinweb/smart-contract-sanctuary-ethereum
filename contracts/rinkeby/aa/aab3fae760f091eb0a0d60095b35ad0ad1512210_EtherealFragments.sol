...
contract EtherealFragments is ERC20("Ethereal Fragments", "FRAGZ") {
	...
	event FragzBurnt(address indexed user, uint256 amount, uint256 indexed concerning, string indexed reason);

	// Link the _worlds contract and mint 1500 tokens to provide liquidity
	constructor(address _worlds) {
		_worldsContract = IEtherealWorlds(_worlds);
		_mint(0xA9aA4eA2d781D640293BBF9DCD6e43B13e7d566C, 1500 ether);
	}
	...
	function burn(uint256 _amount, uint256 _concerning, string memory _reason) external {
		require(msg.sender == _worldsContract.ownerOf(_concerning), "Fragz: Not Owner");
		_burn(msg.sender, _amount);
		emit FragzBurnt(msg.sender, _amount, _concerning, _reason);
	}
