...
contract DNAChipDescriptor is Ownable {
    ...
    function tokenURI(uint256 _tokenId) public view returns (string memory) {
        ...
        RedactedLibrary.Traits memory traits = RedactedLibrary.representationToTraits(traitsRepresentation);
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    name,
                                    '", "image": "data:image/svg+xml;base64,',
                                    AnonymiceLibrary.encode(bytes(getEvolutionPodSVG(traits.base))),
                                    '","attributes":',
                                    getMetadata(traits.base, traits.level),
                                    ', "description": "DNA Chips is a collection of 3,550 DNA Chips. All the metadata and images are generated and stored 100% on-chain. No IPFS, no API. Just the Ethereum blockchain."',
                                    "}"
                                )
                            )
