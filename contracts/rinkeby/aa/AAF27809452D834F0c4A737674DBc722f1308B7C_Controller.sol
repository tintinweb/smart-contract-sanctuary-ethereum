...
contract Controller is RewardsManager, ErrorTracker {
    ...
    {
        // Initialize the ICE virtual pool.
        ...
        // Initialize the STM virtual pool.
        ...
        _dInitialH2O = D_INITIAL_H2O_SUPPLY +
            D_INITIAL_ICE_POOL_H2O_SIZE +
            D_INITIAL_STM_POOL_H2O_SIZE;
    ...
    // Previews how much ICE results from swapping H2O to ICE
    ...
    // Previews how much H2O results from swapping ICE to H2O.
    ...
    // Previews how much STM results from swapping H2O to STM
    ...
    // Previews how much H2O results from swapping STM to H2O
    ...
    /// Swap H2O for ICE using the cold virtual pools.
    function swapH2OForICE(uint256 dH2OAmount) public returns (uint256) {
        ...
        swap(_icePool.tokenA(), _icePool.tokenB(), dH2OAmount, dICEAmount);

        return dICEAmount;
    }

    /// Swap ICE for H2O using the cold virtual pools.
    function swapICEForH2O(uint256 dICEAmount) public returns (uint256) {
        ...
        swap(_icePool.tokenB(), _icePool.tokenA(), dICEAmount, dH2OAmount);

        return dH2OAmount;
    }

    /// Swap H2O for STM using the warm virtual pools.
    function swapH2OForSTM(uint256 dH2OAmount) public returns (uint256) {
        uint256 dSTMAmount = _stmPool.swapAB(dH2OAmount, msg.sender);
        swap(_stmPool.tokenA(), _stmPool.tokenB(), dH2OAmount, dSTMAmount);

        return dSTMAmount;
    }

    /// Swap STM for H2O using the warm virtual pools.
    function swapSTMForH2O(uint256 dSTMAmount) public returns (uint256) {
        uint256 dH2OAmount = _stmPool.swapBA(dSTMAmount, msg.sender);
        swap(_stmPool.tokenB(), _stmPool.tokenA(), dSTMAmount, dH2OAmount);
    ...
    function swap(
        ...
        private
    {
        tokenX.burn(msg.sender, dAmountX);
        tokenY.mint(msg.sender, dAmountY);

        emit Swap(msg.sender, tokenX.name(), dAmountX, tokenY.name(), dAmountY);
    }
    ...
    // Update pools sizes when H2O is added or removed from the system.
    ...
    function _updatePoolSize() private {
        ...
        uint256 dUpdateRatio = dTotalH2O.div(_dInitialH2O);
    ...
    // Allow the target ice price to drift towards the actual ice price
    ...
    // at equilibrium we do not manipulate the steam price
    ...
    // at equilibrium we will probably have some non-zero accumulated error
    function _updateCondensationRate(
        int256 dAccumPIDError,
        uint256 iTimeDelta
    ) 
        private
    {
        ...
        int256 dVariableCondensationRate = dAccumPIDError.mul(
    ...
    event Swap(address indexed account,
        string tokenFrom, uint256 amountFrom,
        string tokenTo, uint256 amountTo);
}
...
contract VirtualPool
{
    ...
    // todo review
    function setPriceA(uint256 dPrice) public {
    ...
    // todo review
    function setPriceB(uint256 dPrice) public {
        // Update the pool B size to achieve this price.
        poolSizeB = poolSizeA.div(dPrice);
    }

    function scalePools(uint256 dFactor) public {
    ...
    function swapAB(uint256 dAmountA, address sender) public returns (uint256) {
    ...
    function swapBA(uint256 dAmountB, address sender) public returns (uint256) {
...
// A Generic contract that holds an instance of each of the 3 Icewater tokens.
contract IceWaterTokens {
...
/// @title ICE Token Contract.
/// Extends the  {RewardToken} contract

contract IceToken is RewardToken {
    // Use Fixed Point library for decimal ints.
    using UFixedPoint for uint256;
    using SFixedPoint for int256;
    ...
    constructor() RewardToken("ICE", "ICE") {
        mint(msg.sender, D_INITIAL_ICE_SUPPLY);
    }
...
/// @title H2O Token Contract.
/// Extends the  {Token} contract

contract H2OToken is Token {

    constructor() Token("H2O", "H2O") {
        mint(msg.sender, D_INITIAL_H2O_SUPPLY);
    }
