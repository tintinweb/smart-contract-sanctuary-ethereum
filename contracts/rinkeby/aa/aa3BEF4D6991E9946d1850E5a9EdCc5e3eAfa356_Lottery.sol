...
contract Lottery is VRFConsumerBase, ColorMap  {
    ...
    function runLottery() public onlyOwner returns (bytes32 requestId) {
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness) internal override {
        ...
        uint lottPot;
        randomResult = _randomness % cells;
        x = randomResult % gridSize;
        y = (randomResult - x) / gridSize;
        ...
        if(coin == linkToken) {
            uint lottPot = IERC20(coin).balanceOf(address(this)) - 1 ether;
        } else {
...
contract ColorMap is Ownable, ReentrancyGuard, Pausable {
    ...
    uint public delay = 60 * 5; // 5 minutes

    uint public gridSize = 64;
    ...
    function addPixel(uint32 _color, uint256 _x, uint256 _y) public whenNotPaused {
        require(block.timestamp - timeout[msg.sender] > delay, "Have patience");
        require(_x < gridSize, "X Out of Bounds");
        require(_y < gridSize, "Y Out of Bounds");
    ...
    function changeGridSize(uint256 _gridSize) public onlyOwner {
        gridSize = _gridSize;
    }
