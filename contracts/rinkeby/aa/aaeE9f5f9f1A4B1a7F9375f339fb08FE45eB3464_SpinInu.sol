...
contract SpinInu is Context, IERC20Metadata, Ownable {
    ...
    function _takeFees(uint256 amount) private returns (uint256) {
        ...
        uint256 totalComputedFee = operationFee.add(developmentFee).add(treasuryFee);

        uint256 receivedBNB = swapTokensForBNB(totalComputedFee);

        uint256 bnbToOperation = receivedBNB.mul(operationFee).div(totalComputedFee);
        uint256 bnbToDevelopment = receivedBNB.mul(developmentFee).div(totalComputedFee);
        uint256 bnbToTreasury = receivedBNB.sub(bnbToOperation, "Insufficient amount").sub(bnbToDevelopment, "Insufficient amount");

        _operationWallet.call{value : bnbToOperation}("");
        _developmentWallet.call{value : bnbToDevelopment}("");
        _treasuryWallet.call{value : bnbToTreasury}("");
