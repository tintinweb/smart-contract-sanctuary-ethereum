...
contract AGOVLPStaking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event Withdraw(address indexed owner, uint256 stakeId, uint256 tokenId, uint256 reward);
    ...
    function initialize(
        ...
        nonfungiblePositionManager = INonfungiblePositionManager(_positionManagerA);
     ...
     *
     * @param amount AGOV token amount which was deposited from klaytn
    ...
    function deposit(
        ...
        if (!_verify(_msgSender(), round, amount, lock, apr, timestamp, signature)) revert FailedVerify();

        // create a new lp position wrapped in a NFT
        (uint256 tokenId, uint128 liquidity, uint256 ethAmount, uint256 agovAmount) = _mintPosition(msg.value, amount);

        // set the owner and data for position
        deposits[totalStaked++] = Deposit({
            owner: user,
            id: tokenId,
            liquidity: liquidity,
            apr: apr,
     ...
     */
    function withdraw(uint256 stakingId) external whenNotPaused nonReentrant {
        Deposit memory staking = deposits[stakingId];
        ...
        nonfungiblePositionManager.transferFrom(address(this), _msgSender(), stakingId);
        ...
        emit Withdraw(_msgSender(), stakingId, staking.id, reward);
    }
     ...
     */
    function rewardOf(uint256 tokenId) public view returns (uint256 reward) {
        Deposit memory lp = deposits[tokenId];
        ...
        )
    {
        ...
        TransferHelper.safeApprove(wethToken, address(nonfungiblePositionManager), ethAmount);
        TransferHelper.safeApprove(agovToken, address(nonfungiblePositionManager), agovAmount);
