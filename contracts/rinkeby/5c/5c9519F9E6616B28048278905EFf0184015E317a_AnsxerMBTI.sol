...
contract AnsxerMBTI is ERC721Psi, Ownable, Pausable {
  ...
  function allowlistMint(Order[] calldata orders, bool staked, uint8 v, bytes32 r, bytes32 s) external whenNotPaused callerIsUser {
    ...
    require(isAllowlist(msg.sender, v, r, s), "You are not eligible for allowlist mint");
  ...
  function isAllowlist(address addr, uint8 v, bytes32 r, bytes32 s) public view returns (bool) {
    bytes32 msgHash = keccak256(abi.encodePacked(this, addr));
    bytes32 messageDigest = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", msgHash));
    return _singer == ecrecover(messageDigest, v, r, s);
  }
