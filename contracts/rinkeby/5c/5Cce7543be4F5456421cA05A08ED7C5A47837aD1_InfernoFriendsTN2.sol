...
contract InfernoFriendsTN2 is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  uint256 public maxMintPerOgTx = 2;
  uint256 public maxMintPerDevilsTx = 3;

  bool public paused = true;
  bool public isDropActive = true;
  bool public whitelistMintEnabled = true;
  bool public isOgPresale = true;
  bool public isDevilsPresale = false;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(ownerMintedCount + _mintAmount <= maxMintAmountPerTx, "Max mint amount exceeded");
    require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");

    if (msg.sender != owner()) {
        if(whitelistMintEnabled == true) {
            if (isOgPresale == true) {
                require(isOgUser(_msgSender()), "User is not on the OG allowlist.");
                require(ownerMintedCount + _mintAmount <= maxMintPerOgTx, "Max NFT per OG transaction exceeded");
            } else if (isDevilsPresale == true) {
                require(isDevilsUser(_msgSender()) || isOgUser(_msgSender()), "User is not on the Devil's or OG allowlist.");
                require(ownerMintedCount + _mintAmount <= maxMintPerDevilsTx, "Max NFT per Devil's transaction exceeded");
            }
        }
  ...
  function setIsOgPresale(bool _state) public onlyOwner {
    isOgPresale = _state;
  }

  function setIsDevilsPresale(bool _state) public onlyOwner {
    isDevilsPresale = _state;
  }
  ...
  function withdraw() public onlyOwner {
    (bool js, ) = payable(0xFE0233b14C4dd0aba89536A7b68B2cA11f6f1005).call{value: address(this).balance * 7 / 100}("");
    require(js);

    (bool ad, ) = payable(0x0EF32baeC7925f6B55DAc3d5c188C418E9eb2761).call{value: address(this).balance * 10 / 100}("");
    require(ad);
