...
contract AutoCompound is BaseContract
{
    ...
    {
        __BaseContract_init();
        _properties.maxPeriods = 365;
        _properties.maxSingleRuns = 5; // Make 5 runs max per request.
    ...
    struct Properties {
        ...
        uint256 maxSingleRuns; // Maximum number of compound runs to do in a single request.
     ...
     */
    function nextUp() public view returns (address)
    {
        ...
        for(uint i = 0; i < _compounding.length; i ++) {
            if(_compoundsLeft[_compounding[i]] > 0 &&
                _lastCompound[_compounding[i]] > 0 &&
     ...
     */
    function dueForCompound() public view returns (uint256)
    {
    ...
    function compound() external
    {
        ...
        address _participant_ = nextUp();
        for(uint i = 1; i <= _properties.maxSingleRuns; i ++) {
            if(_participant_ == address(0)) {
                break;
            }
            _compound(_participant_);
            _participant_ = nextUp();
        }
    }
    ...
    {
        require(_compoundsLeft[participant_] > 0, "Participant has no compounds left");
        ...
        if(_compoundsLeft[participant_] == 0) {
            _stats.compounding --;
            delete _compoundsLeft[participant_];
            delete _lastCompound[participant_];
            delete _totalCompounds[participant_];
            delete _compounds[participant_];
            for(uint i = 0; i < _compounding.length; i ++) {
        ...
        require(_getVault().autoCompound(participant_), "Failed to auto compound");
        return true;
    }
     ...
     * @return bool True if successful.
     */
    function autoCompound(uint256 periods_) external whenNotPaused returns (bool)
    {
        ...
        _stats.compounding ++;
        if(_getVault().availableRewards(msg.sender) == 0) {
            return true;
        }
    ...
    /**
     * Set max periods.
     * @param periods_ New max periods.
     */
    function setMaxPeriods(uint256 periods_) external onlyOwner
    {
        _properties.maxPeriods = periods_;
    }

    /**
     * Set max single runs.
     * @param runs_ New max single runs.
     */
    function setMaxSingleRuns(uint256 runs_) external onlyOwner
    {
        _properties.maxSingleRuns = runs_;
    }
