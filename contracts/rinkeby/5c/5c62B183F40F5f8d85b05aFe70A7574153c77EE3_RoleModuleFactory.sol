...
import {IModuleFactory} from "contracts/interfaces/IModuleFactory.sol";
import {RoleModuleProxy} from "contracts/proxies/RoleModuleProxy.sol";
import {SafeRegistry} from "contracts/registry/SafeRegistry.sol";
import {FactoryVersion} from "contracts/factories/FactoryVersion.sol";
import {Address} from "@openzeppelin/contracts/utils/Address.sol";

contract RoleModuleFactory is FactoryVersion, IModuleFactory {
    ...
    constructor(address registry_, address _implementation) FactoryVersion(registry_) {
        _registry = SafeRegistry(registry_);
        implementation = _implementation;
    }

    function deployRoleModule(
        bytes32 moduleSalt,
        address channel,
        ...
        require(
            !Address.isContract(getRoleModuleAddress(moduleSalt)),
            "RoleModuleFactory: Duplicate salt"
        );

        require(roles.length == accounts.length, "RoleModuleFactory: Array length mismatch");

        RoleModuleProxy proxy = new RoleModuleProxy{salt: moduleSalt}();

        proxy.init(channel, roles, accounts);

        module = address(proxy);

        emit ModuleDeployed(module, msg.sender, bytes4(keccak256("ROLES")));
    }

    function getRoleModuleAddress(bytes32 salt) public view returns (address moduleAddress) {
        bytes memory bytecode = type(RoleModuleProxy).creationCode;
...
interface IRoleModuleFactory {
  function implementation() external view returns(address);
}

contract RoleModuleProxy is RoleModuleStorage {

    event RoleGranted(bytes32 indexed role, address indexed account, address indexed sender);

    /**
     * @notice Initialize role module 
     * @param channel       Channel of the role module
     * @param roles         List of roles to approve
     * @param accounts      List of accounts to grant roles with
     */
    function init(
        address channel,
        bytes32[] memory roles,
        ...
        require(!_initialized, "RoleModule: Already initialized");
        _initialized = true;
        _implementation = IRoleModuleFactory(msg.sender).implementation();

        for(uint256 i = 0; i < roles.length; i++) {
            _roles[roles[i]].members[accounts[i]] = true;
            emit RoleGranted(roles[i], accounts[i], msg.sender);
        }

        _roles[DEFAULT_ADMIN_ROLE].members[channel] = true;
        emit RoleGranted(DEFAULT_ADMIN_ROLE, channel, msg.sender);
    }
