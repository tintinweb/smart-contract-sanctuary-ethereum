...
contract Lottery is VRFConsumerBaseV2, Ownable {
    ...
    event ItemUpdated(uint256 project, uint8 status, uint256 price, uint32 chance, uint256 tickets);
    ...
    function addItems(InputItem[] calldata _items) external onlyOwner {
        for (uint8 i = 0; i < _items.length; i++) {
            require(
                ...
                string(abi.encodePacked("Item already listed - ", _items[i].projectId))
            );
            require(
    ...
    function removeItems(uint256[] calldata _items) external onlyOwner {
        for (uint8 i = 0; i < _items.length; i++) {
            require(
                ...
                string(abi.encodePacked("Item NOT listed - ", _items[i]))
            );
            require(
                listDetails[_items[i]].winner == address(0), 
                string(abi.encodePacked("Item already won - ", _items[i]))
            );
    ...
    * @param _items: [InputItem, InputItem, ...]
    */
    function updateItems(InputItem[] calldata _items, bool restartRound) external onlyOwner {
        for (uint8 i = 0; i < _items.length; i++) {
            require(
                listDetails[_items[i].projectId].status != 0, 
                string(abi.encodePacked("Item NOT listed - ", _items[i].projectId))
            );
            require(
                listDetails[_items[i].projectId].winner == address(0), 
                string(abi.encodePacked("Item already won - ", _items[i].projectId))
            );
            require(
                _items[i].chance >= 0 && _items[i].chance < MAX_CHANCE, 
                "Chance needs to be in range 0 - MAX_CHANCE."
            ...
            if (restartRound) {
                listDetails[_items[i].projectId].roundStart = block.timestamp;
                listDetails[_items[i].projectId].winnerBLD = address(0);
            }
    ...
    */
    function draw(uint256[] calldata _items) external onlyOwner {
        ...
        for (uint8 i = 0; i < _items.length; i++) {
            projectId = _items[i];
            listDetails[projectId].status = 1; // disabled submitting tickets

            require(
                listDetails[projectId].status != 0, 
                string(abi.encodePacked("Item NOT listed - ", projectId))
            );
            require(
                listDetails[projectId].winner == address(0), 
                string(abi.encodePacked("Item already won - ", projectId))
            );
