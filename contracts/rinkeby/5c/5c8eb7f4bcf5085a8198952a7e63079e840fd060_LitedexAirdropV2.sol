...
contract LitedexAirdropV2 is Ownable {
    ...
    event Allocate(uint256 total, uint256 time);
    event Withdraw(uint256 total, uint256 time);
    ...
    function multisendAirdrop(address[] memory _sender, uint256 _amount, string memory _note) external onlyAdmin{
        address[] memory _temp = _sender;

        require(_amount.mul(_temp.length) <= airdropAllocation.sub(tokenOut), "Litedex: sending exceed balance");
        
        for(uint j=0;j<_temp.length;j++){
            require(_transferAirdrop(_temp[j], _amount), "Failed Send Airdrop");
        }
        emit AirdropEvent(_note, _amount.mul(_temp.length), block.timestamp);
    }
    
    function sendAirdrop(address _account, uint256 _amount) external onlyAdmin{
        require(_amount <= airdropAllocation.sub(tokenOut), "Litedex: sending exceed balance");
        require(_transferAirdrop(_account, _amount), "Failed Send Airdrop");
    }

    function _transferAirdrop(address _to, uint256 _amount) private returns(bool){
        require(_to != address(0), "Litedex: Zero Address");
        require(_amount > 0, "Litedex: Zero Amount");

        emit SendAirdrop(_to, _amount);
        IBEP20(ldxToken).transfer(_to, _amount);
        tokenOut = tokenOut.add(_amount);
        return true;
    }
    ...
    function allocate(uint256 _amount) external onlyOwner{
        ...
        emit Allocate(_amount, block.timestamp);
        IBEP20(ldxToken).safeTransferFrom(msg.sender, address(this), _amount);
        airdropAllocation = airdropAllocation.add(_amount);
    }
    function withdraw(uint256 _amount) external onlyOwner{
        require(_amount > 0, "Litedex: Zero Address");
        require(_amount <= airdropAllocation.sub(tokenOut), "Litedex: Amount exceed balance");

        airdropAllocation = airdropAllocation.sub(_amount);
