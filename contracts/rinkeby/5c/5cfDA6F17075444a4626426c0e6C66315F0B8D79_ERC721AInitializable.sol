 ...
 * @title Base contract for standard ERC721A token drops using an "initializable" pattern
 * @author Augminted Labs, LLC
 * @notice Contract has been optimized for fairness and security
 */
contract ERC721AInitializable is ERC721AQueryable, Ownable, AccessControl, Initializable {
    ...
    bytes32 public constant OFFSET_MANAGER_ROLE = keccak256("OFFSET_MANAGER_ROLE");
    ...
    constructor() ERC721A("InitializableERC721A", "ERC721A") {}
     ...
     * @param provenanceHash Provenance hash of the metadata
     * @param metadata TODO
     * @param mintConfig TODO
     * @param maxMintable Max amount of tokens that can be minted
     * @param reserveAmount TODO
     * @param offsetManager Contract to set the token offset
     */
    function initialize(
        string memory name,
        ...
        public
    {
        ...
        _grantRole(OFFSET_MANAGER_ROLE, offsetManager);

        setProvenanceHash(provenanceHash);
        setMetadata(metadata);
        setMintConfig(mintConfig);

        if (reserveAmount > 0) _mint(_msgSender(), reserveAmount);
    }
     ...
     */
    function setTokenOffset(uint256 randomness) internal onlyRole(OFFSET_MANAGER_ROLE) {
        if (bytes(_provenanceHash).length == 0) revert ERC721ABase_ProvenanceHashNotSet();
        if (_tokenOffset != 0) revert ERC721ABase_TokenOffsetAlreadySet();

        _tokenOffset = randomness % _maxMintable;
    }
     ...
     */
    function setMetadata(Metadata memory _metadata) public lockable onlyOwner {
        metadata = _metadata;
    }
    ...
    function _mint(uint256 amount) internal virtual {
        if (_totalMinted() + amount > _maxMintable) revert ERC721ABase_InsufficientSupply();
