...
contract KitsoftNftAvatars is ERC721 {
    enum MintPermission {
        ALLOW_FOR_ALL,
        ALLOW_ONLY_FOR_WHITELIST,
        DENY
    }
    ...
    modifier mintAllowed() {
        require(
            _mintPermission == MintPermission.DENY ||
                (_mintPermission == MintPermission.ALLOW_ONLY_FOR_WHITELIST &&
    ...
    function createCollectible(uint256 tokenId)
        public
        mintAllowed
        returns (uint256)
    {
    ...
    function addAvatar(string memory tokenMetaUri) public onlyAdmin {
        uint256 tokenId = _nextTokenId;
        _tokenUriList[tokenId] = tokenMetaUri;
        _nextTokenId += 1;
    }
    ...
    function allowMintOnlyForWhitelist() public onlyAdmin {
        _mintPermission = MintPermission.ALLOW_ONLY_FOR_WHITELIST;
    }
