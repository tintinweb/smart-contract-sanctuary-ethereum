...
contract Quik_ComMarketplaceContract is Ownable {
    ...
    //fees
    uint256 ethTradeSellerFee = 250;
    uint256 quikTradeSellerFee = 150;
    ...
    mapping (address => uint256[]) public auctionTokenIds; //Auction token for sales nftaddress -> auction
    ...
    checkNFT(_nft)
     {
        ...
        //set nft to auction
        auctionTokenIds[_nft].push(_tokenId);
        ...
        IsTokenOwner(_nft, _tokenId)
    {
        ...
        removeNftIdFromAuction(_nft, _tokenId);
    ...
    function ClaimNFTDomain(address _nft, uint256 _tokenId) public payable{
        ...
        (address minter, uint256 royaltyFee) = IERC2981(_nft).royaltyInfo(_tokenId, act.currentBidPrice);
    ...
    function removeNftIdFromAuction(address _nft, uint256 _tokenId) internal{
        uint256[] storage auctions = auctionTokenIds[_nft];
        for(uint256 i=0; i<auctions.length; i++){
            if(auctions[i] ==_tokenId){
    ...
    checkNFT(nft)
    {
        ...
        ( minter, royaltyFee) = IERC2981(nft).royaltyInfo(tokenId, sellingPrice);
    ...
    function returnSigner(
        ...
        hashStruct = keccak256(
            abi.encode(
                keccak256("uint256 tokenAmount,uint256 nonce,address nft,uint256 tokenId,address seller,uint256 sellingPrice,bool quikTrade"),
                tokenAmount, 
                nonce, 
                nft, 
                tokenId, 
