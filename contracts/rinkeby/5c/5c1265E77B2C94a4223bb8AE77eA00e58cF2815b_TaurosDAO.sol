...
contract TaurosDAO is ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    mapping(address => uint) private _taurosList;
  ...
  function isPrivateSale(bool _private) public onlyOwner {
    ...
    function claimTauros(uint256 _count) public payable {
        ...
        if (msg.sender != owner()) {
            if(onlyWhitelisted == true) {
                ...
                require(_count <= _taurosList[msg.sender], "Exceeded max available to purchase");
                _taurosList[msg.sender] -= _count;
            }
            require(msg.value >= PRICE.mul(_count), "Not enough ether to purchase NFTs.");
        }

        for (uint256 i = 1; i <= _count; i++) {
            // PRICE increments after first 1000 tokens minted 
    ...
    function whitelistUsers(address[] calldata _users, uint256 numAllowedToMint) external onlyOwner {
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            _taurosList[_users[i]] = numAllowedToMint;
        }
