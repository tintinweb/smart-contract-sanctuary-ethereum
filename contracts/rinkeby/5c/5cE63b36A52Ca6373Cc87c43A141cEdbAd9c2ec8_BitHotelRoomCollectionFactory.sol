...
contract BitHotelRoomCollectionFactory is AccessControl {
    ...
    event Deployed(address newContract, address sender, string name, string symbol, uint256 replicas);
    ...
    function deploy(
        ...
        uint256 replicas_
    ) external onlyRole(WHITELISTED_ROLE) returns (address) {
        BitHotelCollection newCollection = new BitHotelCollection(name, symbol, msg.sender, replicas_);
        ...
        emit Deployed(newCollectionAddress, msg.sender, name, symbol, replicas_);
    ...
    ERC2981PerTokenRoyalties
{
    ...
    function bulkSafeMint(
        ...
        for (uint256 i = 0; i < tos.length; i++) {
            safeMint(uri, tos[i], royaltyRecipient, royaltyValue, data_);
        }
    }
