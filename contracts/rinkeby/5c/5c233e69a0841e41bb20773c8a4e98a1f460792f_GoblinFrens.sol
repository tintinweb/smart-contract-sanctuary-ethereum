...
contract GoblinFrens is ERC721A ,Ownable {
    ...
    uint256 public              MAKSI_GOBBO_NUMBE       = 3333;            
    uint256 public              HOWMENEE_FREE_MINNZ     = 500;
    uint256 public              FREE_GARB_PRO_WALLY     = 5;
    uint256 public              GOLBI_PER_TARNSECTIURGH = 20;  
   
    uint256 public constant     TEEM_WESERVDS           = 333;
    uint256 private             teemHfMentid            = 0;
...
/*
    hIII hoOomAnnn CheCK burGhherr soUrccceee
*/   
   function mintGoblinFren(uint256 count) external payable {        
        require(! isPauzzd , "iZz PauZZdddsss!");
        require(count <= GOLBI_PER_TARNSECTIURGH, "To mUChy ate OncE!");
        require(msg.sender == tx.origin,"nO duMp coMtaRcT siLLLi ughMEn!");   
        require(totalSupply() + count < MAKSI_GOBBO_NUMBE, "hAvE NO mOr ggolllBisss!");       

        //iz fri??
        if ( (totalSupply()-teemHfMentid) <= HOWMENEE_FREE_MINNZ)            
            require(addressToMinted[_msgSender()]+count<=FREE_GARB_PRO_WALLY,"tO muCHy forr Frrre dON beuRGH grrreeeDii!");         
            else
