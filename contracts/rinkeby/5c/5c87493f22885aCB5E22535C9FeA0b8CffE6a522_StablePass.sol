...
contract StablePass is ERC721A, Ownable, PaymentSplitter {
    using MerkleProof for bytes32[];
    using Strings for uint256;

    mapping(address => uint256) public alreadyMinted;
    uint256[] public tierByTokenId;
    uint8 public constant maxPerWallet = 25;
    ...
    uint16 public constant basicNFTs = 9500;

    uint256 public nftPrice = 0.01 ether;

    //The below addresses are for example purposes only. Please modify them.
    address[] private payees = [0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2, 0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db];

    //The below percentages are for example purposes only. Please modify them.
    uint256[] private payeesShares = [20, 80];

    function _baseURI(uint256 tier) internal view returns (string memory) {
        return baseURI[tier];
    }
    ...
    function setBaseURI(string memory _baseUri, uint256 tier) public onlyOwner {
        baseURI[tier] = _baseUri;
    }
    ...
    function mintTo(
        ...
        require(
            ...
            string(abi.encodePacked("Cannot mint more than 5 in a single transaction"))
        );
        require(msg.value == (nftPrice * amount), "The price is invalid");
        require(saleStarted == true, "The sale is paused");
        require(totalSupply() - reservedNFTsAmount + amount <= basicNFTs, "Mint limit reached");
        require(alreadyMinted[to] + amount <= maxPerWallet, "Trying to mint over address max minting amount");
        require(keccak256(abi.encodePacked(to)) == leaf, "This leaf does not belong to the sender");
    ...
    function reserveNFT(address to, uint16 amount) public onlyOwner {
        require(reservedNFTsAmount + amount <= totalNFTs - basicNFTs, "Out of stock");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(baseURI[tierByTokenId[tokenId]], tokenId.toString()));
    }
}
