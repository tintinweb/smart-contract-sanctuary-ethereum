...
contract HYFI_Presale is Initializable, AccessControlUpgradeable {
    ...
    function initialize(
        ...
        calc = HYFI_PriceCalculator(_priceCalculatorContractAddress);
        referrals = HYFI_Referrals(_referralsContractAddress);
    ...
    function _buyWithMainToken(
        ...
        require(
            calc.getTokenData(token).tokenAddress.balanceOf(msg.sender) >=
                priceTotal,
            "Buyer does not have enough funds to make this purchase"
        );
        calc.getTokenData(token).tokenAddress.transferFrom(
            msg.sender,
            collectorWallet,
            priceTotal
        );
    ...
    function _buyWithHYFIToken(
        ...
        calc.getTokenData("HYFI").tokenAddress.transferFrom(
            msg.sender,
            collectorWallet,
            HYFItokenPayment
        );
    ...
    function setNewPriceCalculatorImplementation(address newPriceCalculator)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        calc = HYFI_PriceCalculator(newPriceCalculator);
    }
    ...
    function setNewReferralImplementation(address newReferral)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        referrals = HYFI_Referrals(newReferral);
    }
...
contract HYFI_PriceCalculator is Initializable, AccessControlUpgradeable {
    ...
    function initialize(
        ...
        whitelist = HYFI_Whitelist(_whitelistCotractAddress);
        referrals = HYFI_Referrals(_referralsContractAddress);
    ...
    function setNewWhitelistImplementation(address newWhitelist)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        whitelist = HYFI_Whitelist(newWhitelist);
    }
    ...
    function setNewReferralsImplementation(address newReferrals)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        referrals = HYFI_Referrals(newReferrals);
    }
...
contract HYFI_Whitelist is Initializable, OwnableUpgradeable {
    ...
    function addMultipleToWhitelist(address[] memory _addresses)
        ...
        onlyOwner
    {
        for (uint256 addr = 0; addr < _addresses.length && gasleft() > 100000; addr++) {
