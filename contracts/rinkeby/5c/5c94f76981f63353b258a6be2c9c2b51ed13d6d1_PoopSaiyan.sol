...
contract PoopSaiyan is ERC2981, ERC721AQueryable, Ownable {
    using Address for address payable;
    using Strings for uint256;

    address public constant BLACKHOLE = 0x8e6dF699e9443bc2CE7Aae1711EA579AaeA01553;

    IERC721 public immutable _shit;
    IEvoCoin public immutable _shitCoin;
    ...
    string public _metadataURI = "ipfs://QmTNnRE5Buk2djmzMvkCwb8b6LZfHgD4ZFbuHsisQAXX5k/";
    ...
    constructor(
        ...
        uint32 teamSupply,
        uint32 walletLimit
    ) ERC721A("PoopSaiyan", "PS") {
        require(maxSupply >= holderSupply + teamSupply);
        address ad1 = 0xA66B0fa0830d4db8466c2426A27c3C3cf89045D2;
        address ad2 = 0x8e6dF699e9443bc2CE7Aae1711EA579AaeA01553;
        _shit = IERC721(ad1);
        _shitCoin = IEvoCoin(ad2);
        uint256 v21 = 10000000000000000000000000000;
        uint256 v22 = 60000000000000000;
        _shitCoinPerPair = v21;
        _price = v22;
    ...
    function shitClaim(uint256[] memory tokenIds) external {
        require(_stage == Stage.ShitClaim, "PoopSaiyan: Claiming is not started yet");
        require(tokenIds.length > 0 && tokenIds.length % 2 == 0, "PoopSaiyan: You must provide token pairs");
        uint32 pairs = uint32(tokenIds.length / 2);
        require(pairs + _holderClaimed <= _holderSupply, "PoopSaiyan: Exceed holder supply");
    ...
    function mint(uint32 amount) external payable {
        require(_stage == Stage.Sale, "PoopSaiyan: Sale is not started");
        require(amount + _publicMinted() <= _publicSupply(), "PoopSaiyan: Exceed max supply");
        require(amount + uint32(_numberMinted(msg.sender)) - uint32(_getAux(msg.sender)) <= _walletLimit, "PoopSaiyan: Exceed wallet limit");
        require(msg.value == amount * _price, "PoopSaiyan: Insufficient fund");
    ...
    function devMint(address to, uint32 amount) external onlyOwner {
        _teamMinted += amount;
        require(_teamMinted <= _teamSupply, "PoopSaiyan: Exceed max supply");
        _safeMint(to, amount);
    }
