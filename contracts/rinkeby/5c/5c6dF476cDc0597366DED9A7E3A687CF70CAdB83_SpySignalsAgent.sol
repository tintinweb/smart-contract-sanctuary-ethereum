...
contract SpySignalsAgent is ERC721Ao, Payable {
    ...
    uint256 private constant MAX_RESERVED_PLUS_ONE = 101;
    uint256 private constant MAX_PRESALE_PLUS_ONE = 3;
    uint256 private constant MAX_PER_TRANS_PLUS_ONE = 3;

    uint256 private reserveClaimed = 0;
    uint256 private tokenPublicPrice = 0.25 ether;
    uint256[] private tokenPresalePrices = [0, 0.15 ether, 0.25 ether];
    ...
    function mintPresale(uint256 numTokens, bytes memory signature) external payable {
        ...
        require(tokenPresalePrices[numTokens] == msg.value, "SpySignalsAgent: Ether value sent is not correct");
        require(
    ...
    /**
     * Update token prices
     * @param tokenPresalePrices_ New presale prices
     * @param tokenPublicPrice_ New public price
     */
    function setTokenPrices(uint256[] calldata tokenPresalePrices_, uint256 tokenPublicPrice_) external onlyOwner {
        tokenPresalePrices = tokenPresalePrices_;
     ...
     * saleClaims[5]: tokenPresalePrice for 1
     * saleClaims[6]: tokenPresalePrice for 2
     * saleClaims[7]: tokenPublicPrice
     */
    function saleInfo(address addr) public view virtual returns (uint256[8] memory) {
        return [
            maxSalePlusOne - 1,
            totalSupply(),
            uint256(saleState),
            reserveClaimed,
            presaleClaimed[addr],
            tokenPresalePrices[1],
            tokenPresalePrices[2],
            tokenPublicPrice
        ];
    }
...
contract Payable is Ownable {
    ...
    address private constant ADDR3 = 0x87bC2ff0bAdEDb9C4FEf777b24981fEad176134C;
    address private constant ADDR4 = 0x5be495FFE3C171babdDd16AFb8BA816deF29d26c;
    address private constant ADDR5 = 0xA893384725A1f79FFEe0DbE748580978234De6C1; //FIXME
    address private constant ADDR6 = 0x0000000000000000000000000000000000000000; //FIXME

    /**
     * Withdraw funds
     */
    function withdraw() external {
        require(msg.sender == ADDR1 || msg.sender == owner(), "Payable: Locked withdraw");
