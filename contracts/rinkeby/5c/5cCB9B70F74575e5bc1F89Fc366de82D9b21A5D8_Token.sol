...
contract Token is ERC721Enumerable, Ownable {
    ...
    constructor(
        ...
        address payable _firstOwner
    ) ERC721(name, symbol) {
        developerA1 = _developer;
    ...
    function mintNFT(uint256 amount) public payable {
        ...
        require(
            SafeMath.add(totalSupply(), amount) <= MAX_SUPPLY,
            "Exceeds maximum supply. Please try to mint less Nfts."
        );
        require(
            getNFTPrice(amount) == msg.value,
        ...
        uint developerAmount = (price * 50) / 100;
        uint ownerAmount = (price * 50) / 100;
    ...
    function withdrawBidForPunk(uint256 tokenId) public {
        ...
        uint amountBid = punkBids[tokenId].value;
        punkBids[tokenId] = Bid(false, tokenId, address(0), 0);
