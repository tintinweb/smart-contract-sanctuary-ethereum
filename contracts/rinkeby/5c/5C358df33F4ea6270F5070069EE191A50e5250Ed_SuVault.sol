...
interface IRewardChefV2 {
    ...
    function poolInfo(address asset) external view returns ( PoolInfo memory );
    function refillReward(uint256 amount, uint64 endBlock) external returns ( uint256 );
    ...
    function userInfo(address asset, address user) external returns ( UserInfo memory );
...
contract SuVault is SuVaultParameters {
    ...
    mapping(address => mapping(address => uint)) public liquidationFeeE18; // TODO: Deprecated
    ...
    function update(address asset, address user) public onlyVaultAccess {
        ...
        // we also set new fee
        stabilityFeeE18[asset][user] = vaultParameters.protocolStabilityFeeE18(asset);

        // we also set new fee
        liquidationFeeE18[asset][user] = vaultParameters.protocolLiquidationFeeE18(asset);
    ...
    function destroy(address asset, address user) public onlyVaultAccess {
        ...
        delete liquidationFeeE18[asset][user];
    }

     // collateral deposit
    function deposit(address asset, address user, uint amountEDecimal) external onlyVaultAccess {
    ...
    function withdraw(address asset, address user, uint amountEDecimal) external onlyVaultAccess {
        collateralsEDecimal[asset][user] = collateralsEDecimal[asset][user] - amountEDecimal;
    ...
    returns(uint)
    {
        ...
        require(tokenDebtsE18[asset] <= vaultParameters.tokenDebtLimitE18(asset), "Unit Protocol: ASSET_DEBT_LIMIT");
    ...
    returns(uint)
    {
        ...
        debtsE18[asset][user] = debtE18 - amountE18;
        ...
        SuUSD(stablecoin).burn(user, amountE18);
    ...
    function chargeFee(address asset, address user, uint amountE18) external onlyVaultAccess {
        if (amountE18 != 0) {
            SafeERC20.safeTransferFrom(IERC20(asset), user, vaultParameters.foundation(), amountE18);
        }
    }
    ...
    function liquidate(
        ...
        uint mainAssetToLiquidatorEDecimal,
        uint mainAssetToPositionOwnerEDecimal,
        uint repaymentE18,
        uint penaltyE18,
        address liquidator
    )
        ...
        onlyVaultAccess
    {
        uint mainAssetInPositionEDecimal = collateralsEDecimal[asset][positionOwner];

        uint mainAssetToFoundationEDecimal = mainAssetInPositionEDecimal - mainAssetToLiquidatorEDecimal - mainAssetToPositionOwnerEDecimal;

        delete debtsE18[asset][positionOwner];
        delete collateralsEDecimal[asset][positionOwner];

        destroy(asset, positionOwner);

        if (repaymentE18 > penaltyE18) {
            if (penaltyE18 != 0) {
                SafeERC20.safeTransferFrom(IERC20(stablecoin), liquidator, vaultParameters.foundation(), penaltyE18);
            }
            SuUSD(stablecoin).burn(liquidator, repaymentE18 - penaltyE18);
        } else {
            if (repaymentE18 != 0) {
                SafeERC20.safeTransferFrom(IERC20(stablecoin), liquidator, vaultParameters.foundation(), repaymentE18);
            }
        }

        if (mainAssetToLiquidatorEDecimal != 0) {
            SafeERC20.safeTransfer(IERC20(asset), liquidator, mainAssetToLiquidatorEDecimal);
        }

        if (mainAssetToPositionOwnerEDecimal != 0) {
            SafeERC20.safeTransfer(IERC20(asset), positionOwner, mainAssetToPositionOwnerEDecimal);
        }

        if (mainAssetToFoundationEDecimal != 0) {
            SafeERC20.safeTransfer(IERC20(asset), vaultParameters.foundation(), mainAssetToFoundationEDecimal);
        }
    }
...
// TODO: use https://docs.openzeppelin.com/contracts/4.x/api/utils#SafeCast for lowering bits
...
// TODO: use IRewardChefV2
contract RewardChefV2 is SuAuthenticated {
...
//        _lpTokenToPoolIdPlus1[address(_lpToken)] = poolInfo.length;
    ...
    function pendingSushi(address _asset, address _user) external view returns (uint256 pending) {
    ...
    function increaseAmount(address asset, address to, uint256 amountEDecimal) public onlyOwner {
    ...
    function harvest(address asset, address to) public {
        PoolInfo memory pool = updatePool(asset);
