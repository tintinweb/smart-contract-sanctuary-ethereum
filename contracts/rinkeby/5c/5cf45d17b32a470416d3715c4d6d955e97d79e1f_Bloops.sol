...
contract Bloops is ERC721 {
    ...
    function claim() public {
        ...
        require(addressToMinted[msg.sender] == false, "Address already minted");
        require(totalMinted >= 27 && totalMinted < 256, "All have been minted");
        addressToMinted[msg.sender] = true;
        _internalMint(totalMinted);
    }

    function ownerClaim(uint256 amount) public {
        for (uint256 i = totalMinted; i < amount; i++) {
            _internalMint(i);
        }
    }
    ...
    // Gets a seed
    function createSeedFromTx(uint256 tokenId) public returns (uint256) {
        randNonce++;
        uint256 o = uint256(
            keccak256(
                abi.encodePacked(
