...
contract TEST is ERC721Enumerable, Ownable {
    ...
    //Dutch auction logic
    uint256 public startingPrice = 1000000000000000; // 0.001 for testing only
    uint256 public decrease_interval = 300; // In seconds
    uint256 public decrease_per_interval = 25000000000000; // 0.000025 eth
    uint256 public ending_price = 100000000000000; // 0.0001 eth
    uint256 public starting_block;

    //Whitelist
    uint256 public whitelistPrice = 100000000000000; // 0.0001 eth
    mapping (address => uint256) private _whitelistClaimed;
    bool public _WLpaused = false;
    ...
    function getCurrentPrice() public view returns(uint256) {
        ...
        if(passed_intervals > 36){
            return ending_price;
        }
        return startingPrice.sub(passed_intervals.mul(decrease_per_interval));
    }

    function whiteListMint(bytes memory sig,uint256 _mintAmount) public payable {
        require(!_paused, ": Contract paused.");
        require(!_WLpaused, ": Whitelist is paused.");
        require(isValidUser(msg.sender, sig),"INVALID SIGNATURE");
        uint256 supply = totalSupply();
        require(supply + 1 <= maxSupply, "No more NFTs to mint, decrease the quantity or check out OpenSea.");
        require(_mintAmount > 0, ": Amount should be greater than 0.");
        require(_whitelistClaimed[msg.sender] + _mintAmount <= whitelistMaxMint, "Whitelist can be used Twice Only!");
        require(msg.value >= whitelistPrice * _mintAmount, "Insufficient funds.");
        // require(_whitelistReserved > 0 ,"Whitelist Reserve Drained!");
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(_mintAmount <= maxMintAmount, ": Maximum minting amount = 10/transaction.");
        if(msg.sender != owner()){
            require(!_paused, ": Contract paused.");
        ...
        for(uint256 i = 1; i <=_mintAmount; i++){ //Changed i=1 , cause we need to start token from 1
    ...
    function isValidUser(address _addr,bytes memory sig)public view returns (bool){
