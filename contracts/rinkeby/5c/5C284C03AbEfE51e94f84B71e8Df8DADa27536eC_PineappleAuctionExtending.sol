...
contract MemeTokenWrapper{
	...
	function bid(uint256 id, uint256 amount) public virtual{
	...
	function _end(
		...
		_totalSupply = _totalSupply.sub(amount);
		uint256 pineappleFee = (amount.mul(fee)).div(100);

		_totalDeposits[id] = _totalDeposits[id].sub(amount);
		_balances[id][highestBidder] = _balances[id][highestBidder].sub(amount);
		meme.transfer(artist, amount.sub(pineappleFee));
		meme.transfer(daoAddress, pineappleFee);
	}
}

interface Pineapples {
	function create(uint256 _amount) external returns (uint256 tokenId);
	function pineappleTransferFrom(address _from, address _to, uint256 _id, uint256 _amount, bytes calldata _data) external;
}

contract PineappleAuctionExtending is Ownable, ReentrancyGuard, MemeTokenWrapper, IERC1155Receiver, IERC721Receiver {
	using SafeMath for uint256;
	...
	function pingappleNFTTokenID(uint256 id) public view returns (uint256) {
		return auctionsById[id].tokenID;
	}
	...
	function create(
		...
		require(auction.artist == address(0), "PineappleAuction::create: auction already created");
		...
		auction.pineappleNFTAddress = nftAddress;

		auctions.push(auctionID.current());

		auctionID.increment();

		uint256 tokenId = Pineapples(nftAddress).create(1);
		require(tokenId > 0, "PineappleAuction::create: create did not succeed");
		auction.tokenID = tokenId;
	}

	function bid(uint256 id, uint256 amount) public virtual override nonReentrant {
		AuctionInfo storage auction = auctionsById[id];
		require(auction.artist != address(0), "PineappleAuction::bid: auction does not exist");
		require(block.timestamp >= auction.auctionStart, "PineappleAuction::bid: auction has not started");
		require(block.timestamp <= auction.auctionEnd, "PineappleAuction::bid: auction has ended");

		uint256 newAmount = amount.add(balanceOf(msg.sender, id));
		require(newAmount > auction.highestBid, "PineappleAuction::bid: bid is less than highest bid");

		auction.highestBidder = msg.sender;
		auction.highestBid = newAmount;

		if (auction.extension > 0 && auction.auctionEnd.sub(block.timestamp) <= auction.extension) {
			auction.auctionEnd = block.timestamp.add(auction.extension);
		}

		super.bid(id, amount);
		emit BidPlaced(msg.sender, id, newAmount);
	}

	function withdraw(uint256 id) public virtual override nonReentrant {
		AuctionInfo storage auction = auctionsById[id];
		uint256 amount = balanceOf(msg.sender, id);
		require(auction.artist != address(0), "PineappleAuction::withdraw: auction does not exist");
		require(amount > 0, "PineappleAuction::withdraw: cannot withdraw 0");

		require(
			auction.highestBidder != msg.sender,
			"PineappleAuction::withdraw: you are the highest bidder and cannot withdraw"
		);
	...
	function emergencyWithdraw(uint256 id) public onlyOwner {
		AuctionInfo storage auction = auctionsById[id];
		require(auction.artist != address(0), "PineappleAuction::create: auction does not exist");
		require(block.timestamp >= auction.auctionEnd, "PineappleAuction::emergencyWithdraw: the auction has not ended");
		require(!auction.auctionEnded, "PineappleAuction::emergencyWithdraw: auction ended and item sent");
	...
	function end(uint256 id) public nonReentrant {
		AuctionInfo storage auction = auctionsById[id];
		require(auction.artist != address(0), "PineappleAuction::end: auction does not exist");
		require(block.timestamp >= auction.auctionEnd, "PineappleAuction::end: the auction has not ended");
		require(!auction.auctionEnded, "PineappleAuction::end: auction already ended");

		auction.auctionEnded = true;
		_end(id, auction.highestBidder, auction.artist, daoAddress, auction.fee, auction.highestBid);
		Pineapples(auction.pineappleNFTAddress).pineappleTransferFrom(address(this), auction.highestBidder, auction.tokenID, 1, "");
	...
	function onERC1155Received(
		...
		require(_operator == address(this), "PineappleAuction::onERC1155BatchReceived:: operator must be auction contract");
	...
	function onERC721Received(
		...
		require(operator == address(this), "PineappleAuction::onERC721Received:: operator must be auction contract");
