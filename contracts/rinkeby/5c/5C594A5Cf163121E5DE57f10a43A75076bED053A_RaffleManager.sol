...
error RaffleManager_InsufficientWinners();

/**
 * @title KaijuMart Raffle Manager
 * @author Augminted Labs, LLC
 * @notice Event-based raffle entry system
 */
contract RaffleManager is IRaffleManager, AccessControl, VRFConsumerBaseV2 {
    ...
    function create(
        uint256 id,
        CreateRaffle calldata _raffle
    )
        public
        override
        onlyRole(KMART_CONTRACT_ROLE)
    {
        if (bytes(scripts[_raffle.scriptId]).length == 0) revert RaffleManager_InvalidScriptId();
        if (_raffle.winners == 0) revert RaffleManager_InsufficientWinners();

        raffle[id] = Raffle({
            seed: 0,
            scriptId: _raffle.scriptId,
            winners: _raffle.winners,
            drawn: false
        });
    }
        ...
        onlyRole(KMART_CONTRACT_ROLE)
    {
        ...
        if (vrf) {
            ...
            raffle[id].seed = uint256(keccak256(
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        raffle[requests[requestId]].seed = randomWords[0];
    }
}
