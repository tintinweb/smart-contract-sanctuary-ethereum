...
contract BloodShedBearsStaking is Ownable, Pausable {
    ...
    function _moveTokenInTheListStake(
        ...
        require(tokenIndex != 0, "NOT OWNER");
    ...
    function stakePasses(
        ...
        if (walletsStakedPasses[msg.sender] != 0) {
            if (withdrawCurrentYield) {
                ...
                transferPassesYield(msg.sender);
            }
        }
    ...
    function unStakePasses(bool withdraw) external whenNotPaused  {
        uint256 walletBalance = walletsStakedPasses[msg.sender];
        IMintPass(contractsAddressesMap["mintPass"]).safeTransferFrom(address(this), msg.sender, 0, walletBalance, "");
    ...
    function transferPassesYield(address wallet_) public whenNotPaused {
    ...
    function _isOwner(address wallet_, address collection_, uint256 tokenId_) internal view returns (bool) {
    ...
    function stakeTokens(StakeSelection[] calldata selections_) external whenNotPaused {
        ...
        for (uint256 i = 0; i < selections_.length; ++i) {
            require(
                ...
                .isOwnerOfBatch(selections_[i].tokens, msg.sender)
            );
            ...
            if (stack) {
                require(_isOwner(msg.sender, contractsAddressesMap["treeHouse"], selections_[i].targetHouse));
                require(
            ...
            for (uint256 j = 0; j < selections_[i].tokens.length; ++j) {
                _enterStakeStance(selections_[i].collectionAddress, selections_[i].tokens[j], selections_[i].stakeType);
                if (stack) {
                    _enterStackStance(selections_[i].collectionAddress, selections_[i].tokens[j], selections_[i].targetHouse);
                }
                ICollection721(selections_[i].collectionAddress).safeTransferFrom(msg.sender, address(this), selections_[i].tokens[j]);
            }
    ...
    function unStakeTokens(UnStakeSelection[] calldata selections_) external whenNotPaused {
        ...
        for (uint256 i = 0; i < selections_.length; ++i) {
            totalAccumulatedYield += _exitStakeStanceMany(selections_[i].collectionAddress, selections_[i].tokens, true, true);
    ...
    function _exitStakeStanceMany(
        ...
        uint256[] memory levelBoosts = IGameStats(contractsAddressesMap["gameStats"]).getLevelBoosts(collection_, tokenIds_);
        for (uint256 i = 0; i < tokenIds_.length; ++i) {
    ...
    function _exitStakeStance(
        ...
        stakedTokensByWallet[msg.sender][collection_] =
    ...
    function moveIntoDifferentStake(StakeSelection[] calldata selections_) external whenNotPaused {
        for (uint256 i = 0; i < selections_.length; ++i) {
            _exitStakeStanceMany(selections_[i].collectionAddress, selections_[i].tokens, false, false);
            for (uint256 j = 0; j < selections_[i].tokens.length; ++j) {
    ...
    function _getYield(
        ...
        if (stakedTokensData[collection_][tokenId_].stakeType == stakeTypesVariables["treeHouse"]) {
            yield +=
            (block.timestamp - treeHousesEnrolments[stakedTokensData[collection_][tokenId_].houseTokenId][collection_][tokenId_].enrolDate) *
        ...
        if (
            stakedTokensData[collection_][tokenId_].stakeType == stakeTypesVariables["battle"]
            || stakedTokensData[collection_][tokenId_].stakeType == stakeTypesVariables["treeHouse"]
            || stakedTokensData[collection_][tokenId_].stakeType == stakeTypesVariables["house"]
        ) {
            if (
                !IGameStats(contractsAddressesMap["gameStats"])
        ...
        if (updateClaimField) {
            ...
            if (stakedTokensData[collection_][tokenId_].stakeType == stakeTypesVariables["treeHouse"]) {
                treeHousesEnrolments[stakedTokensData[collection_][tokenId_].houseTokenId][collection_][tokenId_].enrolDate = block.timestamp;
            }
        }
    ...
    function claimYieldForAll() external whenNotPaused {
        uint256 totalYield;
        for (uint256 i = 0; i < collections.length; ++i) {
            totalYield += claimYield(collections[i], stakedTokensByWallet[msg.sender][collections[i]]);
        }

        IToken(contractsAddressesMap["token"]).add(msg.sender, totalYield);
    }

    function calculateYieldSafe(address collection_, uint256[] calldata tokenIds_) external view {
        uint256 totalYield;
        for (uint256 i = 0; i < tokenIds_.length; ++i) {
    ...
    function getStakedTokens(address wallet_, address collection_) external view returns (uint256[] memory) {
        uint256[] memory stakedTokens = new uint256[](stakedTokensByWallet[wallet_][collection_].length);

        for (uint256 i = 0; i < stakedTokensByWallet[wallet_][collection_].length; ++i) {
            stakedTokens[i] = stakedTokensByWallet[wallet_][collection_][i];
        }
