...
/// @author ItsCuzzo
/// @title ERC-721 for Dusktopia

contract Dusktopia is Ownable, ERC721A, PaymentSplitter {
    ...
    uint256 public constant DL_MAX_TOKENS = 2;
    ...
    /// @notice Function used to mint tokens during the dusklist sale.
    ...
    function dusklistMint(uint256 quantity, bytes calldata signature) external payable {
        ...
        if (msg.value != quantity * PRICE_PER_TOKEN) revert InvalidEtherAmount();
        if (_numberMinted(msg.sender) + quantity > DL_MAX_TOKENS) revert TokenAlreadyClaimed();

        bytes32 _hash = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                // bytes32(abi.encodePacked(uint8(quantity), uint160(msg.sender)))
                bytes32(uint256(uint160(msg.sender)))
            )
        );

        if (_signerAddress != _hash.recover(signature)) revert InvalidSignature();
    ...
    function reserveMint(bytes calldata signature) external payable {
        if (msg.sender != tx.origin) revert NonEOA();
        if (saleState != SaleStates.RESERVE) revert InvalidSaleState();
        if (_totalMinted() + 1 > DL_SUPPLY) revert DuskSupplyExceeded();
        if (msg.value != PRICE_PER_TOKEN) revert InvalidEtherAmount();
        if (_numberMinted(msg.sender) != 0) revert TokenAlreadyClaimed();
    ...
    function _teamMint(address receiver) internal {
        _safeMint(receiver, RESERVED_TOKENS);
    }
    ...
    /// @notice Function used to set the a new contract URI.
    ...
    /// @notice Function used to get the current contract URI.
    /// @return The current _contractURI value.
