...
interface PriceOracle {
    ...
    function updatePrices(uint256 potion, uint256 reward, uint256 dollar) external;
}
...
contract Oracle is PriceOracle, Ownable {
  ...
  ///remake
  uint256 private _potionPrice;
  uint256 private _rewardAmount;
  uint256 private _currentDollar;

   mapping (address => bool) public allowed;

  constructor(uint256 _seed) {
    _potionPrice = 1000000000000000000;
    _rewardAmount = 1000000000000000000;
    _currentDollar = 1000000000000000000;
    nonce = _seed;
  ...
  //Price Oracle
  function currentPotion() external override view returns (uint256) {
    return _potionPrice;
  }
  function currentReward() external override view returns (uint256) {
    return _rewardAmount;
  }
  function currentDollar() external override view returns (uint256) {
    return _currentDollar;
  }

  function showSeed() external override view returns (uint256) {
    require(allowed[msg.sender], "Contract must be alllowed to call oracle"); 
    return seed;
  }
  

  ///set these at the price of reward value in USD in wei using keeper bots
  function updatePrices(uint256 _potion, uint256 _reward, uint256 _dollar) external override {
    require(msg.sender == PRICE_UPDATER, "Missing PRICE_UPDATER role");
    require(_rewardAmount > 0, "Price must be non-zero");
    _potionPrice = _potion;
    _rewardAmount = _reward;
    _currentDollar = _dollar;
    emit RewardsUpdated(_potionPrice, _rewardAmount, _currentDollar);
  }
