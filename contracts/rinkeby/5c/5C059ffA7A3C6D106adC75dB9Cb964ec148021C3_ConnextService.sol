...
contract ConnextService is Ownable, Pausable, IMdexCrossChainSwap {
    ...
    function singleSwap(bytes calldata data) external whenNotPaused returns (bytes calldata) {
        ...
        IERC20(assetAddress[destinationDomain]).approve(address(mdexController), amount);
        mdexController.swap(assetAddress[destinationDomain], tokenDestinationAddress, amount, routeIndex);
        IERC20(tokenDestinationAddress).transfer(sender, IERC20(tokenDestinationAddress).balanceOf(address(this)));
