...
contract Dapp {
    ...
    event transferOriginAddress(address origin, uint value,uint value2);
    ...
    function downloadMusic(string calldata name)public payable returns(string memory){
        payable(musicToaddress[name]).transfer(1000 wei);
        emit transferOriginAddress(musicToaddress[name],2000,1000);
