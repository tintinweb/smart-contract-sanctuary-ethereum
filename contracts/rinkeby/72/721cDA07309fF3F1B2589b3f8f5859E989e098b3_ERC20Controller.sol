 ...
 */
contract ERC20Controller is Ownable, Controller {
     ...
     * @param _minCommitmentAge Wait period after commitment to finish registration.
     * @param _maxCommitmentAge Commitment expiration period in seconds.
     */
    constructor(IBaseNFT _base, IPriceOracle _prices, uint _minCommitmentAge, uint _maxCommitmentAge) {
    ...
    function registerWithConfig(
        ...
        bytes32 _commitment = makeCommitmentWithConfig(_name, _owner, _secret, _resolver, _addr);
        (address _paymentToken, uint _cost) = _consumeCommitment(_name, _duration, _commitment);
        ...
        if(_resolver != address(0)) {
            ...
            _expires = base.register(_tokenId, address(this), _duration);

            // The nodehash of this label
            bytes32 _nodehash = keccak256(abi.encodePacked(base.baseNode(), _label));

            // Set the resolver
            base.crs().setResolver(_nodehash, _resolver);
            ...
            _expires = base.register(_tokenId, _owner, _duration);
        }
     ...
     * @param _duration Lease duration in seconds.
     */
    function renew(string calldata _name, uint _duration) external override payable {
        ...
        bytes32 _label = keccak256(bytes(_name));
        uint _expires = base.renew(uint256(_label), _duration);
...
abstract contract Controller is Ownable {
    ...
    function rentPrice(string memory _name, uint _duration) view public returns(address, uint) {
        ...
        return prices.price(base.nameExpires(uint256(_hash)), _duration);
    }
    ...
    function commit(bytes32 _commitment) public {
        require(commitments[_commitment] + maxCommitmentAge < block.timestamp);
     ...
     * @param _duration Lease duration in seconds.
     */
    function renew(string calldata _name, uint _duration) external virtual payable;
    ...
    function withdraw(address _paymentToken) external onlyOwner {
        if (_paymentToken == address(0)) {
            ...
            IERC20(_paymentToken).safeTransferFrom(_me, msg.sender, _balance);
        }  
    }

    /**
     * @dev Change price oracle.
     * @param _prices Address for a new price oracle.
     */
    ...
    /**
     * @dev Configure commitment settings.
     * @param _minCommitmentAge Minimal commitment duration in seconds (waiting time for registration finalisation).
     * @param _maxCommitmentAge Maximum commitment duration in seconds (commitment expiration time).
     */
     ...
     * @return registration cost in wei or ERC20 token.
     */
    function _consumeCommitment(
        ...
        require(commitments[_commitment] + minCommitmentAge <= block.timestamp);

        // If the commitment is too old, or the name is registered, stop
        require(commitments[_commitment] + maxCommitmentAge > block.timestamp);
        require(available(_name));

        delete(commitments[_commitment]);

        (address _tokenAddr, uint _cost) = rentPrice(_name, _duration);
        require(_duration >= MIN_REGISTRATION_DURATION);
 ...
 */
interface IResolver is ISupportsInterface, IAddressResolver, INameResolver, ITextResolver, IContentHashResolver, IABIResolver, IInterfaceResolver, IPubkeyResolver {
