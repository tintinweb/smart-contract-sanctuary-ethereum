...
contract ADStaking is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    function _calculateReward(address for_) private view returns (uint256) {
        return (min(block.timestamp, lastWrldClaim + (distributionPeriod * 1 days)) - max(lastWrldClaim, lastClaimTime[for_])) * dailyReward * staked[for_].length / 1 days;
    }

    function _calculateWhaleReward(address for_) private view returns (uint256) {
        uint256 multiplier = staked[for_].length / 25;

        return (min(block.timestamp, lastWrldClaim + (distributionPeriod * 1 days)) - max(lastWrldClaim, lastClaimTime[for_])) * whaleDailyReward * multiplier / 1 days;
    }
