...
contract RandomOnChainSVG is ERC721URIStorage, ERC721Burnable, IERC2981, VRFConsumerBase, Ownable {
    bytes32 public keyHash;
    uint256 public fee;
    uint256 public tokenCounter;
    uint256 public totalTokensEverMinted;
    ...
    ERC721("RandomOnChainSVG", "ROSVG")
    {
        keyHash = _keyHash;
        fee = _fee;
        totalTokensEverMinted = tokenCounter = 0;
        _recipient = owner();
    }
    ...
    function create() public payable mintCompliance(msg.value, 1) returns (bytes32 requestId){
        ...
        uint256 tokenId = totalTokensEverMinted; 
        totalTokensEverMinted++;
    ...
    function createtBatch(uint256 numTokens) public payable mintCompliance(msg.value, numTokens) returns (bytes32 requestId){
        require(numTokens > 1, "Must create at least 2 tokens");
    ...
    function formatTokenURI(string memory imageURI) public view returns (string memory) {
        return string(abi.encodePacked("data:application/json;base64,",Base64.encode(bytes(
            abi.encodePacked(
                '{"name":"', '"Random Generated On-Chain SVG #"',  uint2str(tokenCounter),'", "description":"An NFT based on an SVG that was generated randomly with the help of ChainLink VRF!", '"attributes"':"',"{ \"trait_type\":\"ID\" }","{ \"value\":\"John\" }"'", "image":"',imageURI,'"}'
            ))))
        );
    }
     ...
     */
    function uint2str(uint _i) internal view returns (string memory _uintAsString) {
        if (_i == 0) {
            return "0";
        }
        uint j = _i;
    ...
    /**
        * @dev Overriden burn function, any token owner can burn their own tokens
        * @param tokenId uint256 tokenId to be burned
        * Requires:
        * - `msg.sender` is the owner of the given `tokenId`
        * - `tokenCounter` is greater than `0`
     */
    function _burn(uint256 tokenId) internal virtual override (ERC721, ERC721URIStorage){
        require(tokenCounter > 0, "ERC721: total supply must be greater than 0");
        require(tokenIdToRecipient[tokenId] == msg.sender, "ERC721: only token owner can call burn");
        tokenCounter--;
        delete(tokenIdToRecipient[tokenId]);
