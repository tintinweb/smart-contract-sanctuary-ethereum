...
// The goal of this farm is to allow a stake OphirToken to earn anything model
// In a flip of a traditional farm, this contract only accepts OphirToken as the staking token
// Each new pool added is a new reward token, each with its own start times
// end times, and rewards per second.
contract StakeDapp is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 OphirTokenStakedAmount; // # of dummyToken allocated to this pool
    ...
    IERC20 public immutable OphirToken;
    ...
    constructor(IERC20 _OphirToken) {
        OphirToken = _OphirToken;
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && pool.OphirTokenStakedAmount != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp, pool);
            uint256 reward = multiplier * pool.RewardPerSecond;
            accRewardPerShare += (reward * pool.TokenPrecision) / pool.OphirTokenStakedAmount;
        }
    ...
    function updatePool(uint256 _pid) internal {
        ...
        if (pool.OphirTokenStakedAmount == 0) {
        ...
        pool.accRewardPerShare += reward * pool.TokenPrecision / pool.OphirTokenStakedAmount;
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        safeTransfer(OphirToken, address(msg.sender), _amount);
    ...
    function emergencyWithdraw(uint256 _pid) external {
        ...
        pool.OphirTokenStakedAmount -= user.amount;
        user.amount = 0;
        user.rewardDebt = 0;

        OphirToken.safeTransfer(address(msg.sender), oldUserAmount);
    ...
    function recoverWrongTokens(address _tokenAddress) external onlyOwner {
        require(_tokenAddress != address(OphirToken), "recoverWrongTokens: Cannot be OphirToken");
