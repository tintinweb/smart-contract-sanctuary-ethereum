...
contract TestStakeOne is AccessControl, ReentrancyGuard {
    ...
    event stakeCapChanged(uint160 stakeCap);
    event individualStakeCapChanged(uint160 individualStakeCap);
    ...
    uint160 public stakeCap; //Maximum total of tokens allowed for staking
    uint160 public individualStakeCap; //Maximum total of tokens allowed for individual staking
    ...
    constructor(address _stakingToken, uint48 _lockTimePeriod, uint160 _stakeCap) {
    ...
    function individualCap() external view returns (uint160 cap) {
        return individualStakeCap;
    }
    ...
    function setLockTimePeriod(uint48 _lockTimePeriod) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(block.timestamp + _lockTimePeriod < stakeRewardEndTime, "Lock time should be < than stake reward time.");
        lockTimePeriod = _lockTimePeriod;
        emit LockTimePeriodChanged(_lockTimePeriod);
    }
    
    function setStakeCap(uint160 _stakeCap) external onlyRole(DEFAULT_ADMIN_ROLE) {
        stakeCap = _stakeCap;
        emit stakeCapChanged(_stakeCap);
    }

    function setIndividualStakeCap(uint160 _individualStakeCap) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_individualStakeCap <= stakeCap, "Individual cap must be <= than total cap.");
        individualStakeCap = _individualStakeCap;
        emit individualStakeCapChanged(_individualStakeCap);
    }
    ...
    function _stake(uint256 _amount) internal returns (uint256) {
        ...
        require(toUint160(user.stakeAmount + _amount) <= individualStakeCap, "Amount exceeds individual stake cap.");
        ...
        emit Stake(msg.sender, _amount, user.stakeTime); // = user.stakeTime
        return _amount;
    }
    ...
    function _withdraw(uint256 amount) internal returns (uint256) {
        ...
        emit Withdraw(msg.sender, amount, user.stakeTime); // = user.stakeTime
        return amount;
    }
