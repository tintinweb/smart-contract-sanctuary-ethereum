...
contract Loan {
    ...
    function createLoan(
        ...
        require(_loanCompleteTime > block.timestamp, "Cannot create loan");
        ...
        NftLoans[loanId].interestRate = _interestRate;
        NftLoans[loanId].maxLoanAmount = _maxLoanAmount;
    ...
    /**
     * Calculate annual percentage rate for a particular lender
     * @param _loanId NftLoan id
    ...
    function calculateInterestAccrued(uint256 _loanId, uint256 _future)
        ...
        returns (uint256)
    {
        NftLoan memory loan = NftLoans[_loanId];
        uint256 _secondsAsTopBid = block.timestamp + _future - loan.lastBidTime;
        uint256 _secondsSinceFirstBid = loan.loanCompleteTime -
            loan.firstBidTime;
    ...
    /**
     * Calculates required additional capital (over topbid) to outbid loan
     * @param _loanId NftLoan id
        ...
        returns (uint256)
    {
        NftLoan memory loan = NftLoans[_loanId];
        return
            loan.historicInterest + calculateInterestAccrued(_loanId, _future);
    }
    ...
    function underwriteLoan(uint256 _loanId) external payable {
        ...
        require(msg.value > 0, "Cannot underwrite with 0 Ether.");
        require(
            loan.tokenOwner != address(0),
            "Cannot underwrite a repaid loan."
        );
        require(
            loan.loanCompleteTime >= block.timestamp,
            "Cannot underwrite expired loan."
        );
        if (loan.firstBidTime != 0) {
            ...
            require(_bidPayout < msg.value, "Cannot underwrite < top lender.");
            require(
                loan.maxLoanAmount + _totalInterest >= msg.value,
                "Cannot underwrite > max loan."
            );
    ...
    function repayLoan(uint256 _loanId) external payable {
        NftLoan storage loan = NftLoans[_loanId];
        require(loan.tokenOwner != address(0), "Cannot repay paid loan.");
        require(loan.firstBidTime != 0, "Cannot repay loan with 0 bids.");
        require(
            loan.loanCompleteTime >= block.timestamp,
    ...
    function cancelLoan(uint256 _loanId) external {
        ...
        require(loan.firstBidTime == 0, "Cannot cancel loan with >0 bids.");
        IERC721(loan.tokenAddress).transferFrom(
            address(this),
            msg.sender,
            loan.tokenId
    ...
    function seizeNFT(uint256 _loanId) external {
        ...
        require(
            loan.tokenOwner != address(0),
            "Cannot seize from repaid loan."
        );
        require(
            loan.loanCompleteTime < block.timestamp,
            "Cannot seize before expiry."
        );
        IERC721(loan.tokenAddress).transferFrom(
            address(this),
            loan.lender,
