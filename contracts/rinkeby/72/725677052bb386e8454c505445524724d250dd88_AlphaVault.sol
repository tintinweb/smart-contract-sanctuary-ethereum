...
contract AlphaVault is Ownable, IERC721Receiver {
  ...
  // mapping(address => uint256[]) private _alreadyStaked; idea behind it would be collect all the IDs then use _claim on stake
...
/*
  ...
  function stakeFirst(address account, uint256[] calldata tokenIds) external { // think about expanding it to an array again
    require(_staked[account] == 0,"only for first NFT to be staked. Please use the StakeNotFirst function");
    uint256 tokenId;
...
// enter 0 if you have not ever staked
  ...
  function stake(address account, uint256[] calldata tokenIds, uint256[] calldata alreadyStakedIds) external {
    if(_staked[account] >= 1){
    _claim(account, alreadyStakedIds, false);
    }
    uint256 tokenId;
    totalStaked += tokenIds.length;
    _staked[msg.sender] += tokenIds.length;
  ...
  function _unstakeMany(address account, uint256[] calldata tokenIds) internal {
    uint256 tokenId;
    totalStaked -= tokenIds.length;
    _staked[account] -= tokenIds.length;
    for (uint i = 0; i < tokenIds.length; i++) {
      tokenId = tokenIds[i];
...
// need to add minus tokenid length from future mapping addres to uint256
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    if(_staked[account] > 1){
      if(_staked[account] == 2){
        score = 13;
      }
    
      if(_staked[account] >= 3){
        score = 20;
      }
    }
  ...
  function earningInfo(uint256[] calldata tokenIds) external view returns (uint256[2] memory info) {
     ...
     for (uint i = 0; i < tokenIds.length; i++) {
      ...
      earned += 1 ether * bonus * score * (block.timestamp - stakedAt) / 1 days;
    }
...
/*  function numberStaked(address owner) public view returns (uint256) {
        return _numberStaked(owner);
  }
*/
