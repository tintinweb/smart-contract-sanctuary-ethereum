...
contract Story is ERC721, Ownable, ReentrancyGuard  {
  ...
  //  run on inital deployment
  constructor() payable ERC721('Story', 'STORY') {
    // initalize variables
    totalSupply = 0;
    maxSupply = 10000;
    maxPerWallet = 10;
  ...
  //  CREATE A USER
  function setStory(uint256 _tokenId, string memory _tokenText) internal  {
    StoryStruct memory story = StoryStruct(_tokenId, _tokenText);
  ...
  function updateStory(uint256 _tokenId, string memory _newTokenText) public {
    ...
    setStory(_tokenId, _newTokenText);
  }

  function adminUpdateStory(uint256 _tokenId, string memory _newTokenText) external onlyOwner {
  ...
  function setIsAdminUpdateEnabled(bool _isAdminUpdateEnabled) external onlyOwner nonReentrant {
    require(isAdminUpdateEnabled, "this feature has been turned off by owner.");
  ...
  function tokenURI(uint256 _tokenId) override public view returns (string memory) {
    ...
    parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { margin: auto; position: relative; color: #e3e637; fill: "white"; top: 200px; padding: 10px; border-radius: 10px; display: inline; width: auto; font-family: "Courier New", monospace; font-size: 14px; }</style><rect width="100%" height="100%"  />';
    parts[1] = '<circle cx="0" cy="50" r="15" fill="blue" stroke="black" stroke-width="1"><animate attributeName="cx" from="0" to="500" dur="5s" repeatCount="indefinite" /></circle>';
    parts[2] = '<foreignObject x="20" y="20" width="320" height="320"><p class="base" xmlns="http://www.w3.org/1999/xhtml">';
    parts[3] = readStory(_tokenId);
    parts[4] = '</p></foreignObject></svg>';

    string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4]));
    string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Test Tile #', Strings.toString(_tokenId), '", "description": "Testing is a test with some extra  fun.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
  ...
  function mint( string memory _tokenText) public payable {
    require(isPublicMintEnabled, 'Minting is not enabled yet!');
    require(unlockTime < block.number, "Minting is not unlocked yet");
    uint price = getPrice();

    require(msg.value >= price, "The amount of ETH sent is less than the price of token");
    //    require(totalSupply + _quantity <= maxSupply, 'sold out');
    //    require(walletMints[msg.sender] + _quantity <= maxPerWallet, 'exceeded max mint for wallet!');
    uint256 newTokenId = totalSupply;
    totalSupply++;
    setStory(newTokenId, _tokenText);
    _safeMint(msg.sender, newTokenId);
    startAt = block.timestamp;
    unlockTime = block.number + lockDuration;
  }
  ...
  function getAllTokens() public view returns (StoryStruct[] memory) {
    ...
    for (uint256 i = 0; i < totalSupply; i++) {
      if (_exists(i)) {
        res[index] = StoryStruct(storyData[i].tokenId, storyData[i].tokenText);
        index++;
      }
    }
    return res;
