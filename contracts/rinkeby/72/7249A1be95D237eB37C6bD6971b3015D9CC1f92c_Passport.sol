    ...
    UUPSUpgradeable
{
    ...
    event Mint(string indexed userId, address indexed wallet);

    // If a user id has ever minted a passport this maps to a
    ...
    // TODO: pass in at init: name, symbol
    function initialize(address verifierAddress) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __ReentrancyGuard_init_unchained();
        __ERC165_init_unchained();
        __ERC721_init_unchained("Passport", "PSPT");
        __ERC721Enumerable_init_unchained();
        __Pausable_init_unchained();
        __ERC721Pausable_init_unchained();
        __EIP712_init_unchained("Passport", "1.0.0");
    ...
    /* Mint passport using voucher. Must be called from wallet specified in voucher. */
    function mintTo(SignedPassportVoucher calldata _voucher)
        external
        nonReentrant
        whenNotPaused
        checkIfUsedAndMarkAsUsed(_voucher.signature)
    {
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256("PassportVoucher(string userId,address wallet,uint256 nonce)"),
                    keccak256(bytes(_voucher.userId)),
                    _voucher.wallet,
                    _voucher.nonce
                )
            )
        ...
        //minting may be performed by any wallet
        //or for gasless minting allow the oracle server to mint on someone's behalf and then transfer
        require(
        ...
        if (userIdToTokenId[_voucher.userId] != 0) {
            revert("User ID has already minted");
        } else {
            _tokenIds.increment();
            uint256 newTokenId = _tokenIds.current();
            userIdToTokenId[_voucher.userId] = newTokenId;
            _safeMint(_voucher.wallet, newTokenId);
        }

        emit Mint(_voucher.userId, _voucher.wallet);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        //an external contract can be used for more advanced metadata.
        //otherwise just return the uri that has been set with setURI()
        if (metadataAddress != address(0)) return IExternalMetadata(metadataAddress).tokenURI(tokenId);
        else return _uri;
    }
