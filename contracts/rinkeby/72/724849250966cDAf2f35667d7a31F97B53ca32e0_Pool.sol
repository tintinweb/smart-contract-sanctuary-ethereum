...
library LiquidationLogic {
        ...
        )
    {
        ...
        if (INToken(collateralReserve.xTokenAddress).isIndividualTokenPricing()) {
            vars.collateralPrice = oracle.getAssetPriceForTokenId(
                collateralAsset,
                tokenId
            );
        } else {
...
library GenericLogic {
    ...
    function _getUserBalanceInBaseCurrency(
        ...
        if (reserve.assetType == DataTypes.AssetType.ERC20) {
            ...
            if (INToken(xTokenAddress).isIndividualTokenPricing()) {
                uint256 totalBalance = INToken(xTokenAddress).balanceOf(user);

                for (uint256 index = 0; index < totalBalance; index++) {
                    uint256 tokenId = IERC721Enumerable(xTokenAddress).tokenOfOwnerByIndex(user, index);
                    if (ICollaterizableERC721(xTokenAddress).isUsedAsCollateral(tokenId)) {
                        assetPrice = IPriceOracleGetter(oracle).getAssetPriceForTokenId(
                            currentReserveAddress,
                            tokenId
                        );
                        balance += assetPrice;
                ...
                balance =
                ICollaterizableERC721(xTokenAddress)
...
interface IPriceOracleGetter {
    ...
    /**
     * @notice Returns the asset price of tokenId in the base currency
     * @param asset The address of the asset
     * @param tokenId The address of the asset
     * @return The price of the asset
     **/
    function getAssetPriceForTokenId(address asset, uint256 tokenId)
        external
        view
        returns (uint256);
}
    ...
    IERC1155Receiver
{
    ...
    function isIndividualTokenPricing() external view returns (bool);
}
