...
contract VLTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("VLT_Dividend_Tracker", "VLT_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "VLT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "VLT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main VLT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "VLT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "VLT_Dividend_Tracker: Cannot update claimWait to same value");
...
contract VLT is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    VLTDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 100000000000 * (10**9);
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _devFee, uint256 _burnFee) public onlyOwner {
        ETHRewardFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        devFee = _devFee;
        burnFee = _burnFee;

        totalFees = ETHRewardFee.add(liquidityFee).add(devFee).add(burnFee); // total fee transfer and buy
    }

    function setSellFee(uint256 _onSellbnbRewardFee, uint256 _onSellliuidityFee, uint256 _onSelldevFee, uint256 _onSellBurnFee) public onlyOwner {
        onSelltotalFees = _onSellbnbRewardFee;
        onSellliquidityFee = _onSellliuidityFee;
        onSelldevFee = _onSelldevFee;
        onSellburnFee = _onSellBurnFee;

        onSelltotalFees = onSellETHRewardFee.add(onSellliquidityFee).add(onSelldevFee).add(onSellburnFee);
    }
    ...
    function setdevWallet(address payable _newdevWallet) public onlyOwner {
        devWallet = _newdevWallet;
    }

    constructor() ERC20("Voltoken", "VLT") {
        ETHRewardFee = 1;
        liquidityFee = 3;
        devFee = 5;
        burnFee = 1;
        ...
        devWallet = payable(0x828FF894DFEae781489Ed076F04B65Da7a37613A);

        totalFees = ETHRewardFee.add(liquidityFee).add(devFee).add(burnFee); // total fee transfer and buy

        onSelltotalFees = onSellETHRewardFee.add(onSellliquidityFee).add(onSelldevFee).add(onSellburnFee);

    	dividendTracker = new VLTDividendTracker();
        ...
        */
        _mint(owner(), 10000000000000 * (10**9));
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "VLT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "VLT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "VLT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "VLT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "VLT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "VLT: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(onSelltotalFees.sub(onSellliquidityFee)).mul(onSelldevFee);
        devWallet.transfer(marketingAmount);
    ...
    function swapAndSendBNBToMarketing(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, devWallet);
    }
