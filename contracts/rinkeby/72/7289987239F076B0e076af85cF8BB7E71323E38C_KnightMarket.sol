...
contract KnightFactory is KnightToken {
    ...
    function _createKnight(string memory _name, uint _dna, uint8 _gender, uint32 _level, address _owner) internal {
        ...
        listOfKnight.push(Knight(_name, _dna, knightID, _level, uint32(block.timestamp + coolDownTime), uint32(block.timestamp + coolDownSex), 0, 0, 0, _gender));
    ...
    function createKnight(string memory _name, uint8 _gender) public {
        ...
        initKnight[_msgSender()] = true; // fix error balance owner == 0 until create knight, function only call for first create account
    }
    // ERC721URIStorage
