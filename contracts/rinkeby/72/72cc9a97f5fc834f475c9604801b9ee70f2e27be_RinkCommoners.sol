...
interface IApeToken {
    function balanceOf(address from) external view returns(uint256 balance);
    function transfer(address to, uint256 amount) external; // only for ape coin
    function transferFrom(address from, uint256 amount) external; // only for ape coin
}
...
contract Commoners is ERC721A {
    ...
    IApeToken public apeToken; // ape token to buy commoners
    uint256 public apeMintPrice;
    uint8 public saleStatus; // 0 : nothing, 1: claim, 2: WL, 3: public sale, 4: next gen
    ...
    function initialize(address dogewood_, address treat_, address apeToken_, address vrf_) public {
        ...
        rarities[3] = [255, 255];
        aliases[3] = [1, 1];
        // clothes: TODO -- rarity will be updated later
        rarities[4] = [255, 255, 255, 255, 255, 255, 255, 255, 255, 255];
        aliases[4] = [9, 9, 9, 9, 9, 9, 9, 9, 9, 9];
        // accessory: TODO -- rarity will be updated later
        rarities[5] = [255, 255, 255, 255, 255, 255, 255, 255, 255, 255];
        aliases[5] = [9, 9, 9, 9, 9, 9, 9, 9, 9, 9];
        // background: TODO -- rarity will be updated later
        rarities[6] = [255, 255, 255, 255, 255, 255, 255, 255, 255, 255];
        aliases[6] = [9, 9, 9, 9, 9, 9, 9, 9, 9, 9];
        // smithing
    ...
    function withdraw(address to_) external payable {
        require(msg.sender == admin, "not admin");
        payable(to_).transfer(address(this).balance);
        apeToken.transfer(to_, apeToken.balanceOf(address(this)));
    }
    ...
    function claimMint(uint16[] memory doges_) external noCheaters {
        require(saleStatus == 1, "status is not claim");
        require(doges_.length > 0, "empty doges");
        for (uint16 i = 0; i < doges_.length; i++) {
            require(dogeClaimed[doges_[i]] == false, "already claimed");
            require(dogewood.validateOwnerOfDoge(doges_[i], msg.sender), "invalid owner");
        }

        treat.burn(msg.sender, doges_.length * 40 ether);
        for (uint16 i = 0; i < doges_.length; i++) dogeClaimed[doges_[i]] = true;
        _mintCommoners(msg.sender, uint16(doges_.length));
    }

    // WL with mint with ETH or $APE
    //     Supply: 3000
    //     Max mint per WL = 3
    //     Cost 0.035 ETH & TBD on $APE
    //     Implement Merkle Tree to save gas
    //     After 48hrs the unclaimed supply will go towards the public sale
    function whitelistMint(uint8 quantity_, bool isEthMint, bytes memory signature) external payable noCheaters {
        require(saleStatus == 2, "status is not whitelist");
        require(quantity_ > 0, "empty quantity");
        require(isValidSignature(msg.sender, signature), "invalid signature");
        require(whitelistMinted[msg.sender] + quantity_ <= 3, "exceeds quantity");
        require(totalSupply()+quantity_ <= MAX_SUPPLY-DOGE_SUPPLY, "sold out");

        if(isEthMint) {
            require(msg.value >= quantity_ * 0.035 ether, "insufficient eth");
        } else {
            apeToken.transferFrom(msg.sender, quantity_ * apeMintPrice);
        }
    ...
    // Initial Public Sale
    //     Supply =  5000 - (total WL roles * 3) 
    //     Max mint per Tx = 6
    //     Cost 0.035 ETH & TBD on $APE
    //     Starts at the same time as Claim with TREAT and WL mint
    function publicMint(uint8 quantity_, bool isEthMint) external payable noCheaters {
        require(saleStatus == 3, "status is not public sale");
    ...
    // function nextGenMint(uint16 quantity_) external payable {
    //     require(saleStatus == 4, "status is not next gen mint");
    //     require(quantity_ <= 20, "exceed max quantity");
    //     require(totalSupply() >= GENESIS_SUPPLY, "still in sale");
    //     require(totalSupply() + quantity_ <= MAX_SUPPLY, "still in sale");

    //     treat.burn(msg.sender, quantity_ * 100 ether);
    //     _mintCommoners(msg.sender, quantity_);
    // }
    ...
    function _airdropTopTalents() internal {
        uint256 airdropMax_ = _currentIndex > MAX_SUPPLY ? MAX_SUPPLY : _currentIndex;
        for (uint8 i = 1; i <= 4; i++) {
            ...
            // TODO -- set traits of top talents - commoners[topCommoner_]
        }
    }
...
contract RinkCommoners is Commoners {
    function takeCommoner(uint256 id) public {
    ...
    function initMint(address to, uint16 amount) external {
        _mintCommoners(to, amount);
        // _mint(to, amount, '', false);
    }

    // function init(address dogewood_, address treat_, address apeToken_, address vrf_) external {
    //     admin = msg.sender;
    //     initialize(dogewood_, treat_, apeToken_, vrf_);
    // }
}