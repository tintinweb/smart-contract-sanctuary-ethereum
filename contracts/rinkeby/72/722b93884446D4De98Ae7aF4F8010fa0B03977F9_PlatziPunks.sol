...
//to know what token a person has. To list the NFT that person has




/**
underscore at the beginning of function is just a standard style for internal functions
 */

contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
	using Counters for Counters.Counter; // using libraries
	using Strings for uint256; //using libraries
	...
	//mint: is the ability to take a phisical asset to a digital assets on the blockchain
	function mint() public {
		uint256 current = _idCounter.current();
		require(current < maxSupply,"No PlatziPunks left :(");

		tokenDNA[current] = deterministicPseudoRandomDNA(current, msg.sender);
		_safeMint(msg.sender, current); //create the NFT
		_idCounter.increment(); //create new tokenId identifier
	}

	//this function already exist, we are overriding it 
	...
	//return a metadata standard:  https://github.com/ethereum/EIPs/blob/master/EIPS/eip-721.md
	//opensea and others support these metadadata: https://docs.opensea.io/docs/metadata-standards
	function tokenURI(uint256 tokenId) public view override returns (string memory) {
		require(_exists(tokenId),"ERC721, metada URI not found");

		uint256 dna = tokenDNA[tokenId];
		string memory image = imageByDNA(dna);

		/*
		abi enconded contact 2 string and return bytes and we need to return it in base64
		*/
		string memory jsonURIBase64 = Base64.encode(
			abi.encodePacked(
					'{ "name": "PlatziPunks #',
					tokenId.toString(),
		...
		return string(abi.encodePacked("data:application/json;base64,",jsonURIBase64));
	}
