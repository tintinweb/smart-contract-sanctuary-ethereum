...
contract WhiteSandsStaking is IERC721Receiver, Ownable {
    ...
    function acceptCollection(address collection) external onlyOwner {
        ...
        external
    {
        ...
        for (uint256 i = 0; i < collections.length; i++) {
            ...
            _staked.push(
                ...
                TokenInfo(collection, id, msg.sender, block.timestamp)
            );
        ...
        external
    {
        ...
        for (uint256 i = 0; i < collections.length; i++) {
            ...
            if (_stakedByOwners[msg.sender].length > 1) {
                ...
                _indexOfTokensByOwners[last.collection][last.id][
                    msg.sender
                ] = index;
            }
            if (_staked.length > 1) {
                ...
                _indexOfTokens[last.collection][last.id][last.owner] = index;
                uint256 indexOfLast = _indexOfTokensByOwners[last.collection][
                    last.id
                ][last.owner];
        ...
        )
    {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            collections[i] = address(ti.collection);
            ids[i] = ti.id;
    ...
    {
        uint256 count = _staked.length;
        address[] memory collections = new address[](count);
        uint256[] memory ids = new uint256[](count);
        address[] memory owners = new address[](count);
        uint256[] memory timestamps = new uint256[](count);
        for (uint256 i = 0; i < count; i++) {
            collections[i] = address(_staked[i].collection);
            ids[i] = _staked[i].id;
