...
contract TheIncinerator is Ownable {
  ...
  ERC721A public _interface; // todo: remove, update all _interfaces to `jarDudeContract`
  ...
  // todo: remove _partner attr. might be able to remove constructor alltogether
  constructor(address _partner) {
    _interface = ERC721A(_partner);
  }
  ...
  function burn(uint256[] calldata _ids) external reentrancyGuard() {
    require(_interface.isApprovedForAll(msg.sender, address(this)), "Not approved to burn Jar Dudes");
    ...
    for(uint256 i; i < length;) {
      ...
      _interface.transferFrom(msg.sender, address(0x0000000000000000000000000000000000000001), id);
  ...
  function createNewPhase(uint256 _burnsPerAddress, uint256 _totalSlots) external onlyOwner {
  ...
  function currentPhaseId() view external returns (uint256) {
  ...
  function currentPhaseBurnsPerAddress() view external returns (uint256) {
  ...
  function currentPhaseSlotsRemaining() view external returns (uint256) {
