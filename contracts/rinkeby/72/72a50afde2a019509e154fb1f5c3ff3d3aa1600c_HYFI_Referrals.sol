...
contract HYFI_Referrals is Initializable, AccessControlUpgradeable {
    mapping(string => uint256) internal _referralDiscountAmount;
    mapping(uint256 => string[]) internal _referralInfo;
    mapping(string => uint256) referralCodes;
    string[] internal _referralCodeList;
    ...
    function addToReferralList(
        ...
        _referralInfo[referralDiscount].push(referralCode);
        _referralDiscountAmount[referralCode] = referralDiscount;
    }
    ...
    function addMultipleToReferralList(
        ...
        for (
            uint256 code = 0;
            code < referralCode.length;
            code++
        ) {
            addToReferralList(referralDiscount, referralCode[code]);
        }
    }

    function removeFromReferralList(string memory referralCode)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        delete _referralDiscountAmount[referralCode];
    }

    function removeReferralInfoLayer(uint256 referralDiscount)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (
            uint256 code = 0;
            code < _referralInfo[referralDiscount].length;
            code++
        ) {
            removeFromReferralList(_referralInfo[referralDiscount][code]);
        }
        delete _referralInfo[referralDiscount];
    }
    ...
    function updateAmountBoughtWithReferral(
        ...
        if (getAmountBoughtWithReferral(referralCode) == 0) {
            _referralCodeList.push(referralCode);
        }
    ...
    function getReferralInfo(uint256 amount)
        ...
        returns (string[] memory)
    {
        return (_referralInfo[amount]);
    }

    function getReferralDiscountAmount(string memory referralCode)
        external
        view
