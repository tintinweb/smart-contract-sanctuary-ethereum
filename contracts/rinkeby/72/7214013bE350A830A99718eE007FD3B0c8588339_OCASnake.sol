...
// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@#5PPPPPPPPPP5.     [email protected]@@@@#PPP[email protected]@@@@@:  ?PP&@@@@@@@@@@@@@@@//
...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    ...
    function generateAdjectives(uint256 tokenId)
        private
        view
    ...
    function findNameThatDoesNotExist(uint256 tokenId, string memory snakename)
        private
        view
    ...
    function snakeNameSave(
        uint256 tokenId,
        uint256 seed,
        uint256 index
    ) private returns (string memory) {
        uint256 snakeNameIndex = uint256(
            generateAppleSeed(tokenId, 69 + seed) % snakes.length
        );

        string memory snakeName = snakes[snakeNameIndex];
        string memory randomAdj = findNameThatDoesNotExist(tokenId, snakeName);
        //combine the two strings.
        string memory userName = string(
            abi.encodePacked(randomAdj, "_", snakeName)
        );
    ...
    function _safeMint(
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            tokenInfoList[startTokenId] = TokenInfo(
                0,
                [address(0), address(0), address(0)],
                HighScores(0, 0),
                snakeNameSave(startTokenId, 69, 0)
            );
