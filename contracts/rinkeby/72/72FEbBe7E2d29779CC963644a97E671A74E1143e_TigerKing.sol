...
contract TigerKing is ERC721Enumerable, Ownable, Pausable {
    ...
    constructor() ERC721("Tiger King NFT", "TKN") {}
        ...
        whenNotPaused
    {
        ...
        if (msg.sender != owner()) {
            require(
                walletOfOwner(_to).length + _mintAmount <= 20,
        ...
        if (supply >= 1 && supply <= 1000) {
            cost = 0.01 * 10**18;
        } else if (supply >= 1001 && supply <= 2000) {
            cost = 0.02 * 10**18;
        } else {
            cost = 0.03 * 10**18;
        }

        if (msg.sender != owner() || check) {
    ...
    function Reveal(string memory _newRevealURI) public onlyOwner {
        baseURI = _newRevealURI;
    }
