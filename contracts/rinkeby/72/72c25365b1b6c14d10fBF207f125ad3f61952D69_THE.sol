...
// File @openzeppelin/contracts/token/ERC721/extensions/IERC721Enumer[email protected]
...
// File contracts/THE.sol

//Contract based on https://docs.openzeppelin.com/contracts/3.x/erc721T
...
contract THE is ERC721Enumerable, ReentrancyGuard, Pausable, Ownable {
    ...
    uint256 public itemPrice = 0.30 ether;
    uint256 public whitelistPrice = 0.20 ether;
    uint256 public constant _TOTAL_SUPPLY = 9_999;
    uint256 private totalContractBalance = 0;

    mapping(address => bool) private whitelist;
    mapping(address => uint256) private addressIndices;
    mapping(address => uint256) private balances;

    string private nameChanged;
    ...
    modifier isWhitelisted(address _minter) {
        require(whitelist[_minter], "Not allowed to whitelist mint");
        _;
    }
    ...
    modifier incorrectWhiteListPayableAmount(uint256 _amount) {
        require(
    ...
    modifier publicListSaleStarted() {
        require(isPublicSaleActive, "Public sale not started");
        _;
    }

    modifier incorrectItemPayableAmount(uint256 _amount) {
    ...
    function publicMint(uint256 _amount)
        ...
        callerIsUser
        publicListSaleStarted
        incorrectItemPayableAmount(_amount)
        returns (uint256)
    {
    ...
    function whitelistMint(uint256 _amount)
        ...
        incorrectWhiteListPayableAmount(_amount)
    {
        address minter = msg.sender;
    ...
    function addToWhitelistSaleList(address[] calldata _whitelistMinters)
        external
        onlyOwner
    {
    ...
    function unlistWhitelistMinter(address[] calldata _minters)
        external
        onlyOwner
    {
        ...
        callerIsUser
    {
        ...
        (bool succeed, ) = destAddress.call{value: amount}("");
