...
contract NkmClb is ERC721Enumerable, Pausable, Ownable {
  ...
  constructor() ERC721('Nakama Club', 'NKMCLB') {
    admin = msg.sender;
  }
  ...
  function getPrice() public view returns (uint256) {
    uint256 _price;
    _price = 2000000000000000000; //2 MATIC
    return _price;
  }

  function manageStatus(uint256 _amount) internal {
  ...
  function mintNkmclb(uint256 _amount) external payable whenNotPaused {
    require(_amount > 0, "NKMCLB: zero amount");
    require(
      _amount <= 1,
      "NKMCLB:  Can only mint 1 token at a time and per address"
    );
    require(
      totalNFT + _amount <= maxTotalSupply,
      "NKMCLB: max supply exceeded"
    );
    uint256 _price = getPrice();
    require(
      _price * _amount <= msg.value,
      "NKMCLB: MATIC value sent is not correct"
    );
    require(balanceOf(msg.sender) < 1, 'One NKMCLB NFT per Wallet');
    uint256 _newItemId;
