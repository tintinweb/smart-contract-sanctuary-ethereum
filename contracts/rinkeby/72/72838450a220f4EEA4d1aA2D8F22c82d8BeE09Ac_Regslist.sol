...
contract Regslist is Ownable, IERC1363Receiver {
    using Counters for Counters.Counter;

    IERC1363 public immutable token;
    uint public price;
    Counters.Counter private _tokenIdCounter;
    mapping(uint => address) public posts;

    constructor(IERC1363 _token){
        token = _token;
        price = 5 * 10 ** 18;
    }

    event PaymentMade(uint id, address wallet);
    ...
    function onTransferReceived(address operator, address from, uint256 value, bytes memory data) external returns (bytes4) {
        ...
        // return overpayment
        if (value > price) {
            token.transfer(from, value - price);
        }

        posts[_tokenIdCounter.current()] = from;
        _tokenIdCounter.increment();

        emit PaymentMade(_tokenIdCounter.current(),from);
