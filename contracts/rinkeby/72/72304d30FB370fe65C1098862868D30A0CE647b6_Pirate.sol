...
contract Pirate is ERC721, IERC2981, Ownable {
   ...
   constructor() ERC721("Pirate Insults", "PI") {}

   uint public price = 0.025 ether;
   bool private _allowAll = false;
   mapping(uint => bool) private trueInsult;
   uint private _tokenCount;
   bytes32 public _validTokens;
   //address public _royalties;
   ...
   function _performMint(bytes32[] calldata proof, address destination, uint tokenId) internal  {      
      ...
      bytes32 tokenHex = keccak256(abi.encodePacked(Strings.toString(tokenId)));
      bool verified = MerkleProof.verify(proof, _validTokens, tokenHex);
      require( isValidInsult(tokenId) && (verified || _allowAll), "Token does not exist");
    ...
    function isValidInsult(uint tokenId) internal pure returns (bool)
    {
         return (((tokenId >> 5) & 0x1F) < 32 && (tokenId & 0x1F) < 16);
    }
   ...
   function tokenURI(uint tokenId) public view override returns (string memory)
   {
      ...
      if(_exists(tokenId))
      {
         string memory encodedJSON =  Flag.generateMetadata(tokenId, trueInsult[tokenId]);
         return string(abi.encodePacked("data:application/json;base64,", encodedJSON));
      }
      else 
         return "Arrgh! This token does not exist or is not minted yet!";
   ...
   function setAllowAll(bool allow) public onlyOwner {
      _allowAll = allow;
   }
...
/*
   function setRoyalties(address royaltyAddr) public onlyOwner {
      _royalties = royaltyAddr;
   }
*/
   ...
   function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (address, uint256 royaltyAmount) {
      ...
      return (msg.sender, royaltyAmount);
