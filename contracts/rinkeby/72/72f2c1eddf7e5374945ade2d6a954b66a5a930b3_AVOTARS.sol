...
contract AVOTARS is ERC721A, Ownable, ReentrancyGuard {
 ...
 constructor(string memory _hiddenMetadataUri) ERC721A("AVOTAR", "AVT") {
 ...
 modifier mintCompliance(uint256 _mintAmount) {
   require(_mintAmount > 0 && _mintAmount <= maxMintAmountPerUser, "Invalid mint amount!");
 ...
 // funzione che restituisce l'uri del token in base all'id dato in input
 function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
   require(_exists(_tokenId), "ERC721Metadata: URI query for nonexistent token");
  // se la funzione revealed è imposto su falso allora l'uri non viene rivelato
   if (revealed == false) {
     return hiddenMetadataUri;
   }
  // la variabile currentBaseURI prende il valore di baseURI è lo meorizza
   string memory currentBaseURI = _baseURI();
   return bytes(currentBaseURI).length > 0 // se la lunghezza dell'uri è > 0 allora viene composta la stringa 
       ? string(abi.encodePacked(currentBaseURI, _tokenId.toString(), uriSuffix)) // currentBaseURI + tokenId + uriSuffix
       : '';
 }
 
 // funzione che permette di impostare lo stato del reveal disponibile solo al proprietario
 ...
 // imposta il prefisso dell'uri
 function setUriPrefix(string memory _uriPrefix) public onlyOwner {
   uriPrefix = _uriPrefix;
 }
 
 // imposta il suffisso dell'uri
...
// i ricavi vengono suddivisi tra due address
...
// address n2 riceverà il 20% dei ricavi
 ...
 // baseURI sarebbe il prefisso dell'uri
