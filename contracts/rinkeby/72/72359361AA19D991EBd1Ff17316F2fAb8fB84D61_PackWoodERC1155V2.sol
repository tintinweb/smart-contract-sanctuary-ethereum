...
contract PackWoodERC1155V2 is ERC1155Upgradeable, OwnableUpgradeable {
    ...
    */
    ...
    event crossmintTokenDetails(address from, address sendTo, uint256 tokenId1, uint256 tokenId2, uint256 tokenId3, uint256 tokenId1Amt, uint256 tokenId2Amt, uint256 tokenId3Amt, uint256 price);
    ...
    */
     ...
     * @dev mints the token. 
     *
     * @param _address token uri before part.
     *
     * Requirements:
     * - only owner can mint.
    */
    function updateCrossMintAddress(address _address) external onlyOwner{
    ...
    */
    ...
    function buyToken(Order memory order, bytes memory signature) payable external returns(bool){
        ...
        payable(owner()).transfer(msg.value - tempValue);
    ...
    */
    ...
    function buyCrossMint(address to, Order memory order, bytes memory signature) payable external returns(bool){
        bool status = SignatureCheckerUpgradeable.isValidSignatureNow(owner(), order.signKey, signature);
        require(status == true, "$PackWoodERC1155: cannot purchase the token");
        require(msg.sender == CrossMintAddress, "PackWoodERC1155: Method can be only called by Cross mint address");
        ...
        for(uint256 i = 0; i < order.tokenIds.length; i++){
            if(order.random[i] > 0){
                _safeTransferFrom(owner(), to, order.tokenIds[i], order.random[i], "");
            } 
        }

        emit crossmintTokenDetails(owner(), to, order.tokenIds[0], order.tokenIds[1], order.tokenIds[2], order.random[0], order.random[1], order.random[2], msg.value);
