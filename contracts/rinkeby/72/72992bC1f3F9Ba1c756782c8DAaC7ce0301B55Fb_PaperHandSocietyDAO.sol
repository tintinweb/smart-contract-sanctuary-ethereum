...
/// @title Paper Hand Society DAO
/// @author MilkyTaste @ Ao Collaboration Ltd.
/// https://paperhandsdao.wtf

import "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";
import "./ERC721AEnumerable.sol";
import "./Payable.sol";

contract PaperHandSocietyDAO is ERC721AEnumerable, Payable {
    using Strings for uint256;

    // Note some values marked P1 are incremented for gas efficiency

    string public baseURI;
    string public staticURI;

    uint256 private maxSupplyP1 = 2501;
    ...
    // Public
    uint256 public constant PUBLIC_ALLOWANCE_P1 = 4;

    // Presale
    uint256 public constant PRESALE_ALLOWANCE_P1 = 4;
    bytes32 public merkleRoot = "";

    constructor() ERC721AEnumerable("PaperHandSocietyDAO", "PAPER") Payable(750) {}
    ...
    modifier isContractState(ContractState contractState_) {
        require(contractState == contractState_, "PaperHandSocietyDAO: Invalid state");
        _;
    }
    ...
    modifier withinMintLimit(uint256 quantity) {
        require((_totalMinted() + quantity) < maxSupplyP1, "PaperHandSocietyDAO: Exceeds available tokens");
        _;
    }
    ...
    modifier correctValue(uint256 expectedValue) {
        require(expectedValue == msg.value, "PaperHandSocietyDAO: Ether value sent is not correct");
        _;
    }
    ...
    {
        uint64 willHaveMinted = _getAux(msg.sender) + quantity;
        require(willHaveMinted < PUBLIC_ALLOWANCE_P1, "PaperHandSocietyDAO: Exceeds allowance");
        _setAux(msg.sender, willHaveMinted);
    ...
    function mintPresale(uint256 quantity, bytes32[] calldata proof)
        ...
        correctValue(tokenPrice * quantity)
    {
        require(_numberMinted(msg.sender) + quantity < PRESALE_ALLOWANCE_P1, "PaperHandSocietyDAO: Exceeds allowance");
        bytes32 leaf = keccak256(abi.encode(msg.sender));
        require(verify(merkleRoot, leaf, proof), "PaperHandSocietyDAO: Not a valid proof");
        _safeMint(msg.sender, quantity);
    }
     ...
     * @param quantity Amount of tokens to mint.
     */
    function mintTeam(address to, uint256 quantity) external onlyOwner withinMintLimit(quantity) {
