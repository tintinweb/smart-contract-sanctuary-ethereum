    ...
    ERC721LockerUpgradeable
{
    ...
    mapping(uint32 => mapping(uint256 => uint256)) public proofsUsed;
        ...
        returns (string memory)
    {
        ...
        if (bytes(coll.uri).length > 0) {
            return
                string(
                    abi.encodePacked(coll.uri, tokenId.toString(), ".json")
                );
        } else {
    ...
    function mint(MintRequest calldata mr) external payable {
        ...
        bool isCommunityMember = _isCommunityMember(msg.sender);

        ProofState proofState = (mr.proof.length > 0)
            ? (
        ...
        if (
            ...
            ((phase == Phase.Community) && !isCommunityMember)
        ) {
            if (proofState != ProofState.Valid) revert InvalidMerkleProof();
        }

        uint256 price;
        if (phase != Phase.OpenSale) {
            price = coll.communityPrice;
        } else {
            // Phase.OpenSale
            if (isCommunityMember || proofState == ProofState.Valid) {
                price = coll.communityPrice;
            } else {
                price = coll.price;
            }
        }
        if (price * mr.pieceIds.length != msg.value) revert InvalidETHAmount();

        uint256 quota = (phase != Phase.Whitelist) ? coll.quota : mr.amount;
        if (coll.mints[msg.sender] + mr.pieceIds.length > quota)
            revert MintingExceedsQuota();
        if (coll.supply + mr.pieceIds.length > coll.cap)
            revert MintingExceedsSupply(coll.cap);

        _mintN(msg.sender, mr.collectionId, coll, mr.pieceIds);

        if (mr.autostake) {
            _autostake(msg.sender, mr.pieceIds.length);
        }
    }
    ...
    function setCollectionURI(uint256 collectionId, string calldata newURI) external {
        _onlyOwner();
        _collectionExists(collectionId);
    ...
    function _validateMerkleProof(
        ...
        bytes32 merkleRoot,
        uint32 merkleIndex
    ) internal returns (ProofState) {
        if (_isClaimed(proofsUsed[merkleIndex], index))
            return ProofState.Invalid;

        bytes32 node = keccak256(abi.encodePacked(index, account, amount));

        bool proofValid = MerkleProofUpgradeable.verify(
            proof,
            merkleRoot,
            node
        );

        if (proofValid) _setClaimed(proofsUsed[merkleIndex], index);

        return proofValid ? ProofState.Valid : ProofState.Invalid;
    }
