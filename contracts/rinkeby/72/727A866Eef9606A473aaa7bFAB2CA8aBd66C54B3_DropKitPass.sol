    ...
    ERC721EnumerableUpgradeable
{
    ...
    // passes per stage
    mapping(uint256 => mapping(uint96 => Pass)) private _passes;
    ...
    function redeemPass(
        ...
        _verifySignature(stageId, expiration, hash, signature);
        _purchasePass(stageId, feeRate, recipient, quantity);

        emit PassRedeemed(stageId, feeRate, hash);
    }

    function getPass(uint256 stageId, uint96 feeRate)
        public
        view
        returns (Pass memory data)
    {
        Pass memory pass = _passes[stageId][feeRate];
    ...
    function createPass(
        ...
        require(!_passes[stageId][feeRate].isValue, "Pass already exists");
        _passes[stageId][feeRate] = Pass({price: price, isValue: true});

        emit PassCreated(stageId, feeRate);
    }
    ...
    function updatePass(
        ...
        require(_passes[stageId][feeRate].isValue, "Pass doesn't exist");
        _passes[stageId][feeRate].price = newPrice;
    }
    ...
    function setPassStatus(
        ...
        _passes[stageId][feeRate].isValue = active;
    }
    ...
    function batchAirdrop(
        ...
        for (uint256 i = 0; i < length; ) {
            require(
                _passes[stageId][feeRates[i]].isValue,
                "Pass doesn't exist"
            );
    ...
    function isRedeemed(uint256 stageId, bytes32 hash)
        public
        view
        returns (bool)
    {
        return _isVerified(stageId, hash);
    }
    ...
    function _purchasePass(
        ...
        require(_passes[stageId][feeRate].isValue, "Pass doesn't exist");
        require(
            _passes[stageId][feeRate].price.mul(quantity) <= msg.value,
            "Value incorrect"
        );
...
abstract contract SignaturePresale {
    ...
    function _verifySignature(
        ...
        require(
            !_verifiedHashes[stageId].contains(hash),
            "Hash already verified"
        );
        require(expiration > block.timestamp, "Signature expired");
        require(
            keccak256(abi.encodePacked(stageId, expiration, hash))
                .toEthSignedMessageHash()
                .recover(signature) == _signer,
            "Invalid signature"
        );

        _verifiedHashes[stageId].add(hash);
    }
    ...
    function _isVerified(uint256 stageId, bytes32 hash)
        internal
        view
        returns (bool)
    {
        return _verifiedHashes[stageId].contains(hash);
    }
}
