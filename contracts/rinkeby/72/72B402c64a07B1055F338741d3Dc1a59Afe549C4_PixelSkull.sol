...
contract PixelSkull is AdminControl, ReentrancyGuard {
    ...
    constructor(
        address creator,
        Collection memory _collectionData,
    ...
    function getPrice() private view returns (uint256 price) {
        uint256 _price = collectionData.price;
        if (contractData.isPresaleActive) {
            require(
                whitelistedAddresses[msg.sender],
                "You need to be whitelisted"
            );
            price = collectionData.presalePrice;
        }
        return _price;
    }
    ...
    function removeUsersFromWhiteList(address[] memory _addressToWhitelist)
        public
        adminRequired
    {
        for (uint256 i = 0; i < _addressToWhitelist.length; i += 1) {
    ...
    function setIsPresaleActive(bool _isActive) public adminRequired {
    ...
    function withdraw(string memory imageHash) private {
        ...
        uint256 valueTransferOwner = address(this).balance;
