...
import "./lib/SSCErrorsMessages.sol";
...
contract MSSC is SSCVault {
     ...
     *         meets some requirements, further call to {isReadyToExecute} is recommended before performing
     *         this function to ensure that the SettlementCycle meets the requirements to execute instructions.
     *
    ...
    /**
     * @notice Make deposits (Native coin or ERC20 tokens) to a existent instruction, {msg.sender} must be a
     *         sender of any of the instructions in the Settlement Cycle.
     *
    ...
    /**
     * @notice Withdraw funds from a settlement.
     *
    ...
    /**
     * @notice View function to get the instructions in a settlement cycle.
     *
    ...
    /**
     * @notice View function to get deposited funds to a instruction.
     *
        ...
        returns (bool, string memory)
    {
        ...
        if (_cycles[cycleId].executed) return (false, ERR_MSG_EXECUTED);

        // check valid balances and allowance
        (bool result, string memory errMsg) = _checkBalancesFromCycle(cycleId);
        if (!result) return (false, errMsg);
    ...
    function _checkBalancesFromCycle(bytes32 cycleId)
        private
        view
        returns (bool, string memory)
    {
...
contract SSCVault is Assertions {
     ...
     *
     * @param item Struct data to perform the deposit.
    ...
    /**
     * @notice Internal function to withdraw funds from an instruction.
     *
     * @param item Struct data to perform the withdraw.
     */
    ...
    /**
     * @notice Transfer allocated funds to another account.
     *
     * @param item Contains data of the item to deposit (token, type and instruction).
     */
    ...
    // Check if an address is a sender of the instruction.
...
// Error messages
string constant ERR_MSG_REGISTERED = "settlement cycle already registered";
string constant ERR_MSG_NOT_REGISTERED = "settlement cycle is not registered";
string constant ERR_MSG_NO_ZERO_AMOUNT = "no zero amount";
string constant ERR_MSG_NO_ZERO_ADDRESS = "no zero addresses";
string constant ERR_MSG_INVALID_ASSET = "invalid asset";
string constant ERR_MSG_EXECUTED = "settlement cycle already executed";
string constant ERR_MSG_NOT_ENOUGH_DEPOSIT = "Insufficient allocated funds";
string constant ERR_MSG_NOT_APPROVED = "not enough approvals";
string constant ERR_MSG_NOT_ENOUGH_BALANCE = "not enough token balance";
string constant ERR_MSG_NOT_ENOUGH_ALLOWANCE = "not enough allowance";
...
interface MSSCErrors {
    ...
    /**
     * @dev Revert with an error when a non-zero address being called as an assumed
     *      ERC20 contract does not have code.
     *
    ...
    /**
     * @dev Revert with an error when attempting to register an account (sender or recipient)
    ...
    /**
     * @dev Revert with an error when invalid ether or ERC20 tokens is deposited for an instruction.
     *
