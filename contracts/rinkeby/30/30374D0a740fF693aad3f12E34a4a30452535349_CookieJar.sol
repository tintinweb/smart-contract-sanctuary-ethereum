...
contract CookieJar is StakingWrapper, Pausable, AccessControl {
    ...
    uint256[4] public boosts = [
        60000000000000 * 1e18, //  10%
        150000000000000 * 1e18, // 25%
        198000000000000 * 1e18, // 33%
        300000000000000 * 1e18 //  50%
    ];
    ...
    constructor(address _tokenAddress, address _cookiesAddress)
    ...
    function boostAmount() public view returns (uint256 boost) {
        if (totalStaked() >= boosts[0]) return 100;
        if (totalStaked() >= boosts[1]) return 250;
        if (totalStaked() >= boosts[2]) return 330;
        if (totalStaked() >= boosts[3]) return 500;
        return 0;
    }
    ...
    function cardReleaseTime(uint256 cookie) public view returns (uint256) {
