...
contract TokenAuctions is TokenSale {
        ...
        returns (bool)
    {
        ...
        uint256 getCurrentBid = artworkOnXYZ[artworkID]
            .openArtworkAuction[artworkOnXYZ[artworkID].openForBid]
            .bidsOnArtwork[
                artworkOnXYZ[artworkID]
                    .openArtworkAuction[artworkOnXYZ[artworkID].openForBid]
        ...
        uint256 calculateNewBidThreshold = getCurrentBid +
            getCurrentBid.mul(minimumBidThreshold).div(100);

        require(
            msg.value >= calculateNewBidThreshold,
...
contract TokenSale is TokenCore {
        ...
        returns (bool)
    {
        ...
        _transfer(address(this), msg.sender, artworkID);
        ...
        returns (bool)
    {
        ...
        _splitPaymentsBySales(artworkID, msg.value);

        _transfer(address(this), msg.sender, artworkID);
        // safeTransferFrom(address(this), _msgSender(), artworkID);
        artworkOnXYZ[artworkID].ownershipTransferCount.increment();
    ...
    RoyaltiesV2Impl
{
    ...
    struct Artwork {
        ...
        string proofOfAuthenticityCertificateOnARweaveURL;
        uint256 price;
    ...
    uint256 public secondaryOwnerPercentage;
    uint256 public secondaryPlatformPercentage;
    uint256 public minimumBidThreshold;
    ...
    function _splitPaymentsBySales(uint256 artworkID, uint256 price) internal {
        if (artworkOnXYZ[artworkID].ownershipTransferCount.current() == 0) {
            XYZWallet.transfer(price);
        } else {
            uint256 platformFunds = price.mul(secondaryPlatformPercentage).div(
                100
            );
            uint256 ownerFunds = price.mul(secondaryOwnerPercentage).div(100);
            XYZWallet.transfer(platformFunds);
    ...
    event UpdatedAuctionMinimumBid(
        uint256 artworkID,
        uint256 minimumBid,
        address updatedBy
    );

    event UpdatedAuctionExpiryTimestamp(
        uint256 artworkID,
        uint256 expiryTimestamp,
        address updatedBy
    );

    event UpdatedAuctionReservePrice(
        uint256 artworkID,
        bytes32 reservePrice,
        address updatedBy
    );
    ...
    constructor() ERC721("Token", "TAST") {
        ...
        secondaryOwnerPercentage = 85;
        secondaryPlatformPercentage = 15;
        minimumBidThreshold = 5;

        whitelistedAddresses[msg.sender] = true;
        emit WhitelistAddressUpdated(msg.sender, true, address(this));
    }

    function addNewArtwork(
        address owner,
        string memory artworkURI,
        string memory poaCertificateARweaveURL
        ...
        _safeMint(owner, newArtworkId);
        artworkOnXYZ[newArtworkId].owner = payable(owner);
        artworkOnXYZ[newArtworkId]
            .proofOfAuthenticityCertificateOnARweaveURL = poaCertificateARweaveURL;
        ...
        artworkOnXYZ[newArtworkId].tokenMetaData = artworkURI;
        _setTokenURI(newArtworkId, artworkURI);

        emit NewArtworkAdded(
            newArtworkId,
            artworkOnXYZ[newArtworkId].owner,
            artworkOnXYZ[newArtworkId]
                .proofOfAuthenticityCertificateOnARweaveURL,
        ...
        )
    {
        return (
            artworkOnXYZ[artworkID].owner,
            artworkOnXYZ[artworkID].proofOfAuthenticityCertificateOnARweaveURL,
            artworkOnXYZ[artworkID].artworkStatus,
