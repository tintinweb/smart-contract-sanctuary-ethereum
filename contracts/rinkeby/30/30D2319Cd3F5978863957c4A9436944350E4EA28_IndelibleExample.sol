...
contract IndelibleExample is ERC721Enumerable, ReentrancyGuard, Ownable {
...
*/
    ...
    function mintKevin(uint256 _count) external whenPublicMintActive returns (uint256, uint256) {
        ...
        require(tokenIds.current() + _count <= MAX_TOKENS, "All tokens are gone");
        ...
        uint256 firstMintedId = tokenIds.current() + 1;

        for (uint256 i = 0; i < _count; i++) {
            tokenIds.increment();
            mint(tokenIds.current());
        }
...
*/
    ...
    function isPublicMintActive() public view returns (bool) {
        return tokenIds.current() < MAX_TOKENS && isMintingPaused == false;
    }
    ...
    function hashToSVG(string memory _hash)
        ...
        returns (string memory)
    {
        string memory svgString = '<svg id="example" width="100%" height="100%" version="1.1" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><style>#example{background-image:url(data:image/png;base64,';
        ...
        thisTraitIndex = HelperLib.parseInt(
            HelperLib._substring(_hash, 16, 18)
        );
        ...
        returns (string memory)
    {
        ...
        return string(abi.encodePacked(baseURI, Strings.toString(_tokenId), "?dna=", _tokenIdToHash[_tokenId]));
    }
