...
contract sFiscus is IsFISC, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgFISC public gFISC; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked FISC", "sFISC", 9) ERC20Permit("Staked FISC") {
    ...
    function setgFISC(address _gFISC) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gFISC) == address(0), "gFISC:  gFISC already set");
        require(_gFISC != address(0), "gFISC:  gFISC is not a valid contract");
        gFISC = IgFISC(_gFISC);
    }
    ...
    /**
        @notice increases rFISC supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sFISC of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sFISC
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sFISC: insufficient balance");
    }
    ...
    // toG converts an sFISC balance to gFISC terms. gFISC is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gFISC.balanceTo(amount);
    }

    // fromG converts a gFISC balance to sFISC terms. sFISC is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gFISC.balanceFrom(amount);
    }

    // Staking contract holds excess sFISC
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gFISC.balanceFrom(IERC20(address(gFISC)).totalSupply())).add(
