...
// File: Lock\interfaces\IUniswapV2Router02.sol
...
// File: Lock\interfaces\IUniswapV2Pair.sol
...
// File: Lock\interfaces\IUniswapV2Factory.sol
...
// File: Lock\interfaces\IPoolManager.sol
...
// File: Lock\interfaces\ILock.sol
...
contract SafeLock is ILock, Ownable, UUPSUpgradeable, Initializable {
  ...
  Lock[] private _locks;
  Lock[] private _normalLocks;
  Lock[] private _lpLocks;
  ...
  function _addLock(
    ...
    if (isLp) {
      _lpLocks.push(newLock);
    } else {
      _normalLocks.push(newLock);
    }
  ...
  function allNormalLocks(uint from, uint to) public view returns (Lock[] memory) {
    require (from <= _normalLocks.length, "Invalid index");

    if (to > _normalLocks.length) {
      to = _normalLocks.length;
    }

    Lock[] memory res = new Lock[](to - from);

    uint i;
    for (i = from; i < to; i++) {
      res[i - from] = _normalLocks[i];
    }

    return res;
  }

  function allLpLocks(uint from, uint to) public view returns (Lock[] memory) {
    require (from <= _lpLocks.length, "Invalid index");

    if (to > _lpLocks.length) {
      to = _lpLocks.length;
    }
