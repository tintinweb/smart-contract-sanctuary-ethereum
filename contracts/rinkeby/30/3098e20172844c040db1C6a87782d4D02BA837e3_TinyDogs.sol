...
contract TinyDogs is ERC721A, Ownable, ReentrancyGuard  {
    ...
    function whitelistMint(uint256 quantity, bytes32[] calldata _merkleProof) external nonReentrant {
    ...
    function checkAllowlist(bytes32[] calldata _merkleProof) external view returns (bool) { 
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
