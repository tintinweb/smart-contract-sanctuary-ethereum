...
import "./Guestbook__v1_2.sol";

contract GuestbookSupplyLimit is Guestbook__v1_2 {
  mapping(uint256 => uint256) private _totalSupply;
  mapping(uint256 => uint256) private _totalSupplyLimit;

  /// @custom:oz-upgrades-unsafe-allow constructor
  constructor() {}

  // We cannot retroactively determine the supply of tokens, so prior Guestbooks
  // cannot upgrade to this version without having incorrect _totalSupply counts.
  // Therefore, we intentionally breaks upgrading to this contract to avoid that risk
  function upgradeTo__v1_3() public onlyOwner upgradeVersion(3) {
    revert("GuestbookSupplyLimit: not part of the Guestbook__v1_x upgrade series.");
  }

  // Overidden to guard against which users can access
  function _authorizeUpgrade(address newImplementation) internal virtual override onlyOwner {}

  // Deploy using the init
  function init(string memory _name, string memory _symbol) public initializer {
    Guestbook__v1_2.initialize__v1_2(_name, _symbol);
  }

  function setSupplyLimit(uint256 id, uint256 supplyLimit) public onlyOwner {
    _totalSupplyLimit[id] = supplyLimit;
  }
  ...
  function totalSupplyLimit(uint256 id) public view virtual returns (uint256) {
    return _totalSupplyLimit[id];
  }

  function supplyLimitReached(uint256 id) public view virtual returns (bool) {
    return _totalSupply[id] >= _totalSupplyLimit[id];
  }
  ...
  function _beforeTokenTransfer(
    ...
    if (from == address(0)) {
      for (uint256 i = 0; i < ids.length; ++i) {
        require(
          (_totalSupply[ids[i]] += amounts[i]) <= _totalSupplyLimit[ids[i]],
          "GuestbookSupplyLimit: mint amount exceeds tokenLimit"
        );
      }
    }
