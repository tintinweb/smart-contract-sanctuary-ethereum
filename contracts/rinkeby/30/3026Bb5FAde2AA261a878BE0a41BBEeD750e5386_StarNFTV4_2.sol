 ...
 * {ERC721Enumerable}.
 */
contract StarNFTV4_2 is Ownable, ERC165, IERC721, IERC721Metadata, IStarNFT {
    ...
    struct Star {
        uint160 owner;
        uint96 cid; // max cid is 0xffff_ffff_ffff_ffff_ffff_ffff_ffff_ffff, it is enough to store uint96
    }
    ...
    modifier onlyTransferable() {
        require(_transferable, "must be transferable");
        _;
    }
    ...
    function cid(uint256 tokenId) public view returns (uint256) {
        require(tokenId > 0 && tokenId <= getNumMinted(), "tokenId is not valid");
    ...
    returns (address)
    {
        address owner = address(_tokens[tokenId].owner);
        require(
            owner != address(0),
    ...
    returns (bool)
    {
        require(id <= getNumMinted(), "ERC721: owner query for nonexistent token");
...
//    }
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        return _tokens[tokenId].owner != 0x0;
    }
...
//        _mint(to);
//        require(
//            _checkOnERC721Received(address(0), to, getNumMinted(), _data),
//            "ERC721: transfer to non ERC721Receiver implementer"
//        );
//    }
//
...
//    }
    ...
    function burn(address account, uint256 id) external override onlyMinter {
        ...
        require(isOwnerOf(account, id), "ERC721: not owner");
    ...
    onlyMinter
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            require(isOwnerOf(account, tokenId), "ERC721: not owner");
            // Clear approvals
