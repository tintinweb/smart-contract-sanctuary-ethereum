...
contract RezdRobsTest is ERC721A, Ownable {
    ...
    uint16 public maxFreeAmount = 500;

    uint16 public freeCount = 0;

    bool public isFree = true;

    uint64 public cost = 0.003 ether;

    uint8 public maxMintPerWallet = 50;

    bool public isSale = false;

    bool public isRevealed = false;

    mapping(address => uint16) public addressesMintedBalance;
    ...
    /// @notice mints a token, using the ERC721A, depending of {isSale} and {isSaleWhitelist}

    /// @dev mints the token if {isSale} or {isSaleWhitelist} is set to true, using the related cost. Is not working when both are true.
       ...
       if (msg.sender != owner()) {
            ...
            if (freeCount + quantity >= maxFreeAmount) {
...

                {
                    ...
                    uint16 difference = (freeCount + quantity) - maxFreeAmount;
    ...
    /// @dev set the maximumx amount of tokens that can be minted from a wallet.
    ...
    /// @dev set the publicSale cost

    function setCost(uint16 _newCost) public onlyOwner {
