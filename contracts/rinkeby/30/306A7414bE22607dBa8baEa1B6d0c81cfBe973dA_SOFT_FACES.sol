...
contract SOFT_FACES is Ownable, ERC721URIStorage {
    ...
    function greenList(address[] memory addresses, uint size) public returns (uint) {
    ...
    function mint(
        ...
        if (greenListMap[msg.sender] == true) {
            require(
                artBalance[msg.sender] == 0,
                "You must own zero art pieces"
            );
        }
        else {
            require(
