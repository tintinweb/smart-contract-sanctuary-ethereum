...
contract Lottery is VRFConsumerBaseV2, Ownable {
    ...
    event StartLottery(LotteryState state);
    event EnterLottery(address indexed player, uint256 value);
    event EndLottery(LotteryState state);
    ...
    uint32 private constant NUM_WORDS = 1; // 1 random value
    bytes32 private immutable keyHash;
    uint64 public s_subscriptionId;
    uint256 public entranceFee;
    //uint32 endAt = (block.timestamp + 7 days);
    ...
    function start() external onlyOwner {
        ...
        emit StartLottery(lotteryState);
    }
    ...
    function enter() external payable {
        ...
        require(
            msg.value >= entranceFee.getEntranceFee(priceFeed),
            "The entrance fee is $50.00"
        );
    ...
    function returnKeyHash() external view returns (bytes32) {
        return keyHash;
    }
...
library PriceConverter {
        ...
        returns (uint256)
    {
        ...
        return uint256(price * 10000000000); // or 1e10, 1**10
    }

    function getEntranceFee(uint256 minValue, AggregatorV3Interface priceFeed)
        internal
        view
        returns (uint256)
    {
        uint256 entranceFee = (minValue * (10**18)) / getLatestPrice(priceFeed);
        return entranceFee;
    }
}
