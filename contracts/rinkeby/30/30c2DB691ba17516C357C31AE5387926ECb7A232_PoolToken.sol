...
contract PoolToken is Ownable {
    ...
    function transferTokenSameValue(address _tokenAddress,address[] memory _listUsers ,uint256 _amount) external onlyOwner {
        for (uint256 i = 0; i < _listUsers.length; i++) {
            IERC20(_tokenAddress).transfer(_listUsers[i], _amount);
        }
    }

    function transferTokenAnyValue(address _tokenAddress, UserDetails[] memory _userDetails) external onlyOwner {
        for (uint256 i = 0; i < _userDetails.length; i++) {
            IERC20(_tokenAddress).transfer(_userDetails[i].user, _userDetails[i].amount);
        }
    }
}
