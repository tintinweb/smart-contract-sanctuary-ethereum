...
contract Token is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(
        string memory name_, string memory symbol_, string memory baseURI_, bool addTypeToURI_, uint256 limitSupply_
    ) ERC721A(name_, symbol_)  {
        _baseTokenURI = baseURI_;
        addTypeToURI = addTypeToURI_;
//        todo do we need limit?
    ...
    function animatedTokens(uint256 tokenId) external view returns (bool) {
        require(_exists(tokenId), "TT: Animated query for nonexistent token");
        return _animatedTokens[tokenId];
    }

//    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
//        require(_exists(tokenId), "TT: URI query for nonexistent token");
//
...
//        }
//        return string(abi.encodePacked(baseURI, tokenId.toString()), "/?animated=", _animatedTokens[tokenId].toString());
//    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        string memory tokenURI_ = super.tokenURI(tokenId);
        if (addTypeToURI && _animatedTokens[tokenId]) {
//            todo return string(abi.encodePacked(tokenURI_, "/?animated=true"));
            return string(abi.encodePacked(tokenURI_, "/?animated"));
        }
        return tokenURI_;
    }
    ...
    function remainingSaleTime() external view whenSaleActive returns (uint256) {
        require(saleStartTime > 0, "TT: Sale hasn't started yet");
    ...
    function _preValidatePurchase(uint256 tokensAmount) internal view {
        ...
        require(tokensAmount <= transactionLimit, "TT: Limited amount of tokens");
        require(_remainingSaleTime() > 0, "TT: Sale is over");
        // todo do we need limitSupply?
    ...
    function animatedMint(uint256 tokensAmount) external payable whenSaleActive nonReentrant {
        ...
        require(animatedPrice * tokensAmount <= msg.value, "TT: Insufficient funds. (Animated)");
    ...
    function mixedMint(bool[] memory animated) external payable whenSaleActive nonReentrant {
        _preValidatePurchase(animated.length);

        uint256 totalPrice;
        for (uint8 i; i < animated.length; i +=1) {
            totalPrice += animated[i] ? animatedPrice : price;
            _animatedTokens[_currentIndex + i] = animated[i];
        }

        require(totalPrice <= msg.value, "TT: Insufficient funds. (Mixed)");
        _safeMint(msg.sender, animated.length);
    }
