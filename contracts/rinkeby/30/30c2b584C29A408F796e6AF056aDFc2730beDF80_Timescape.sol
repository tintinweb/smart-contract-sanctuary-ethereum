...
contract Timescape is ERC721Enumerable, Ownable {

address public _systemAddress = address(0x8c209E7a039fb146C8d9e63d951b44F541448BA7);
uint256 maxTokenNum = 10000;
//0,2 ether
uint256 public constant timescaleToken = 2 * 1e17;

mapping(uint256 => uint256) idToTimescape;

string private _currentBaseURI;

constructor() ERC721("Time Scape ", "TIMESCAPE") {
        setBaseURI("https://timescape.art/metadata/");
    }

// function mint(address to, uint256 quantity) public payable{
//         require(quantity <= maxBatchSize, "cannot mint more than maxBatchSize");
//         require(timescaleToken * quantity > msg.value,
...
//     }
    ...
    function mint(address to, uint256 id, uint256 price, bytes memory signature) public payable {
        ...
        require(idToTimescape[id] == 0, "token id already minted");
        require(price < msg.value, "Ether value sent is not correct");
        // verify
        emit keccak256Event(keccak256(abi.encodePacked(id, price)));
        require(_isValidSignature(keccak256(abi.encodePacked(id, price)),
                signature), "CP: Invalid signature");
        _safeMint(to, id);
        idToTimescape[id] = 1;
    }

    event keccak256Event(bytes32 hash);
