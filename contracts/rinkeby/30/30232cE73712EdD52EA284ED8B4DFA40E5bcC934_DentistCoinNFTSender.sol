...
// File contracts/DentistCoinNFTSender.sol

pragma solidity ^0.8.4;

//Add description that this contract acts as an airdrop and contains minted nfts to save gas and directly trasnfer tokens and NFT
contract DentistCoinNFTSender is ERC721Holder, Pausable, Ownable, ReentrancyGuard {
    ...
    modifier ensureValidTokenId(uint256 _tokenId) {
        //add any other checks if needed here
        require(dentistCoinNFT.exists(_tokenId), "DentistCoinNFTSender: NFT not minted");
        require(
            dentistCoinNFT.ownerOf(_tokenId) == address(this),
            "DentistCoinNFTSender: NFT not in the contract"
        );
        require(
            dentistCoinNFT.balanceOf(address(this)) > 0,
            "DentistCoinNFTSender: NFT not in the contract"
        );
        _;
    }
        ...
        whenNotPaused
    {
        ...
        require(
            dentistCoinNFT.ownerOf(_tokenId) == address(_receiver),
            "DentistCoinNFTSender: Receiver hasn't received NFT"
        );

        emit NFTDelivered(_tokenId, msg.sender);
    }
    ...
    function NFTsInContract() external view returns (uint256) {
        return dentistCoinNFT.balanceOf(address(this));
    }

    function returnNFTsToOwner(uint256[] calldata _tokenIds) external onlyOwner nonReentrant {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (dentistCoinNFT.ownerOf(_tokenIds[i]) == address(this)) {
                dentistCoinNFT.safeTransferFrom(address(this), address(owner()), _tokenIds[i]);
            }
        }
    }
}