    ...
    ReentrancyGuard
{
    ...
    uint256 public publicId = 0;
    uint256 public whitelistPrice = 0 ether;
    mapping(uint256 => uint256) public firstPublicPrices;
    mapping(uint256 => uint256) public publicPrices;
    mapping(uint256 => uint256) public publicMaxMints;

    constructor(
        string memory _initBaseURI
    ) ERC721A("IchikaWorld", "Ichika", MAX_PER_WALLET, MAX_SUPPLY) { 
        setBaseURI(_initBaseURI);

        firstPublicPrices[0] = 0.003 ether;
        publicPrices[0] = 0.002 ether;
        publicMaxMints[0] = 3;

        firstPublicPrices[1] = 0.01 ether;
        publicPrices[1] = 0.006 ether;
        publicMaxMints[1] = 3;

        firstPublicPrices[2] = 0.05 ether;
        publicPrices[2] = 0.033 ether;
        publicMaxMints[2] = 3;
    }
    ...
    function getPrice(address _address, uint256 _quantity) public view returns (uint256) {
        if (_quantity > 1 || numberMinted(_address) > 0) {
            return (publicPrices[publicId] * _quantity);
        } else {
            return firstPublicPrices[publicId];
        }
    }

    function publicSalesMint(uint256 _quantity)
        external
        payable
        callerIsUser
        nonReentrant
    {
        ...
        require(totalSupply() + _quantity <= publicMaxMints[publicId], "reached the max supply");
        ...
        if (totalSupply() + _quantity == publicMaxMints[publicId]) {
            publicId++;
            paused = true;
        }
        
        uint256 price = getPrice(msg.sender, _quantity);
        _safeMint(msg.sender, _quantity);
        refundIfOver(price);
    }

    function whitelistSalesMint(uint256 _quantity)
        external 
        payable 
        callerIsUser
        nonReentrant 
    {
        ...
        require(whitelistSalesMintedQty + _quantity <= WHITELIST_MINT_LIMIT, "exceed whitelist mint limit");
        require(whitelistSalesMinterToTokenQty[msg.sender] + _quantity <= WHITELIST_MAX_PER_WALLET, "exceed max per wallet");
        ...
        whitelistSalesMinterToTokenQty[msg.sender] += _quantity;
        whitelistSalesMintedQty += _quantity;

        _safeMint(msg.sender, _quantity);
        refundIfOver(whitelistPrice * _quantity);
    }
    ...
    function setPrices(uint256 _maxPublicId, uint256[] calldata _firstPrices, uint256[] calldata _prices, uint256[] calldata _maxMints) external onlyOwner {
        require(_maxPublicId >= publicId,  "Out or range");
        require(_firstPrices.length == _prices.length,  "Size not same");

        for (uint256 i = 0; i < _firstPrices.length; i++) {
            firstPublicPrices[i] = _firstPrices[i];
            publicPrices[i] = _prices[i];
            publicMaxMints[i] = _maxMints[i];
        }
    }
    ...
    function setWhitelistPaused(bool _status) external onlyOwner {
