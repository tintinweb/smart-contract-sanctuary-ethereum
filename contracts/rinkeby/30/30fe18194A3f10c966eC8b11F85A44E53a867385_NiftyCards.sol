...
interface IAuctionHouse {
    struct Auction {
        // Address for the ERC721/ERC1155 contract
...
contract NiftyCards is IAuctionHouse, ReentrancyGuard {
    using Counters for Counters.Counter;

    Counters.Counter private _auctionIdTracker;
    bytes4 constant ERC_721_INTERFACE_ID = 0x80ac58cd; // 721 interface id
    ...
    function createAuction(
        ...
        require(
            IERC165(tokenContract).supportsInterface(ERC_721_INTERFACE_ID),
        ...
        auctionExists(auctionId)
    {
        require(
            ...
            "Must be token seller"
        );
        require(
            auctions[auctionId].endTime == 0,
        ...
        nonReentrant
    {
        ...
        if (_auction.endTime == 0) {
            ...
            _auction.endTime = block.timestamp + _auction.duration;
            _auction.bidder = msg.sender;
            _auction.bid = msg.value;
            emit AuctionBid(
                auctionId,
                auctions[auctionId].tokenId,
                auctions[auctionId].tokenContract,
        ...
        require(
            msg.value >= _auction.bid + ((_auction.bid * 5) / 100),
            "Must send more than last bid by 5% amount"
        );

        payable(_auction.bidder).transfer(_auction.bid);

        _auction.bidder = msg.sender;
        _auction.bid = msg.value;

        if (_auction.endTime - block.timestamp < 15 minutes) {
            _auction.endTime = _auction.endTime + 4 hours;
    ...
    {
        Auction storage _auction = auctions[auctionId];
        require(_auction.endTime != 0, "Auction hasn't begun");

        require(block.timestamp > _auction.endTime, "Auction hasn't completed");

        // Otherwise, transfer the token to the winner and pay out the participants below
        try
            IERC721(_auction.tokenContract).safeTransferFrom(
                address(this),
        ...
        payable(_auction.seller).transfer(_auction.bid);

        emit AuctionEnded(
            auctionId,
            _auction.tokenId,
            _auction.tokenContract,
            _auction.seller,
            _auction.bidder,
            _auction.bid
        );
    ...
    function _cancelAuction(uint256 auctionId) internal {
        if (auctions[auctionId].bidder != address(0)) {
            payable(auctions[auctionId].bidder).transfer(
                auctions[auctionId].bid
            );
        }
        address tokenOwner = auctions[auctionId].seller;
