...
contract DialerContract is BoringOwnable, BoringBatchable {
	...
	function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
		...
		if (block.number > pool.lastRewardBlock) {
			...
			if (lpSupply > 0) {
				// TELE_PER_BLOCK = 100000000000000000000 = 100 TELE
				// pool.allocPoint = 10
				// totalAllocPoint = 1000         - Assumed
				// multiplier = 4320 = 1 day blocks
				// teleReward = (4320 * 100000000000000000000 * 10) / 1000 = 4320000000000000000000
				// lpSupply = 1000000000000000000 - Assumed
				// pool.accTelePerShare = 0 + ((4320000000000000000000 * 1000000000000) / 1000000000000000000) = 43200000000000000
				uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
				uint256 teleReward = multiplier.mul(TELE_PER_BLOCK).mul(pool.allocPoint) /
					totalAllocPoint;
	...
	function harvest(uint256 pid, address to) public {
		...
		// accumulatedTele = (1000000000000000 * 4320000000000000000) / 1000000000000 = 432000000000000000000000 = 432000.00000000006 TELE
