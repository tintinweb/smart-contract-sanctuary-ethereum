    ...
    ReentrancyGuard
{
    ...
    struct FantasticMoment {
        ...
        // * fantasticO share wallet - from 10 to 100 maximum
        uint8 fmCharityFees;
    ...
    // * Address of the FantasticO Share wallet
    address public fmShareWallet;
    // * Address of Fantastico Concierge Wallet
    address public fmConciergeWallet;
    // * Value for calculation of % of newly minted FMS sent to
    // * fantasticO share wallet - from 0 to 10 maximum
    uint256 public fmFantasticoFees;
     ...
     * @dev Constructor
     *
     * @param shareWallet_ The address of the FantasticO Share wallet
     * @param conciergeWallet_ The address of the FantasticO Concierge wallet
     */
    constructor(address shareWallet_, address conciergeWallet_) ERC1155("") {
        require(shareWallet_ != address(0), "No address(0)");
        require(conciergeWallet_ != address(0), "No address(0)");
        ...
        fmShareWallet = shareWallet_;
        fmConciergeWallet = conciergeWallet_;
        // * 7% of newly minted FMs sent to FantasticO Share wallet
        // * if minted amount > 15
        fmFantasticoFees = 7;
        // * 5% of royalty fees
        fmRoyaltyFees = 5;
     ...
     * @param fmCharityFees_ Fees on marketplace mint
     *
    ...
    function mintByAthlete(
        ...
        uint256 fmImgHash_,
        uint256 amount_,
        uint256 fmCharityFees_
    ) external onlyRole(MINTER_ROLE) whenNotPaused {
        mint(msg.sender, fmURL_, fmImgHash_, amount_, fmCharityFees_);
    }
    ...
    function mintForAthlete(
        ...
        mint(fmCreatorAddress_, fmURL_, fmImgHash_, amount_, fmCharityFees_);
    }
    ...
    function mint(
        ...
        require(
            fmCharityFees_ > 9 && fmCharityFees_ < 101,
            "not between 10-100!"
        );
        ...
        // amountForFTO is the shared amount for FantasticO Wallet
        uint256 amountForFTO = 0;
        uint256 amountForCharity = 0;
        uint256 allocatedAmount = amount_;

        if (fmFantasticoFees > 0) {
            amountForFTO = allocatedAmount < 14 ? 0 : (allocatedAmount * fmFantasticoFees) / 100;
        }

        allocatedAmount -= amountForFTO;

        amountForCharity = allocatedAmount < 8 ? 0 : allocatedAmount == 8 ? 1 : (allocatedAmount * fmCharityFees_) / 100;
        ...
        if(msg.sender == fmCreatorAddress_){
            _mint(fmCreatorAddress_, _newTokenId, amount_, "");
        } else{
            _mint(fmConciergeWallet, _newTokenId, amount_, "");
        }
        ...
        if(msg.sender == fmCreatorAddress_){
            if (amountForFTO > 0 || amountForCharity > 0) {
            safeTransferFrom(
                fmCreatorAddress_,
                fmShareWallet,
                _newTokenId,
                amountForFTO + amountForCharity,
                ""
            );
        }
        }
        ...
        emit FmCreated(
            ...
            // Max supply
            amount_,
            // FantasticO Share amount
            amountForFTO,
            // Charity Share amount
            amountForCharity,
            fmURL_
        );
    ...
    function fmCharityFees(uint256 tokenId_)
        ...
        returns (uint256)
    {
        return _fmCollection[tokenId_].fmCharityFees;
    }
    ...
    /**
     * @dev Update the address of the Fantastico Share wallet
     *
     * @param address_ the new address of the Fantastico Share wallet.
     *
     * Requirements: ADMIN_ROLE
     */
    function setFantasticoShareWalletAddress(address address_)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(address_ != address(0), "No address(0)");

        fmShareWallet = address_;
    }

    /**
     * @dev Update the address of the Fantastico Concierge wallet
     *
     * @param address_ the new address of the Fantastico Concierge wallet.
     *
     * Requirements: ADMIN_ROLE
     */
    function setConciergeWalletAddress(address address_)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(address_ != address(0), "No address(0)");

        fmConciergeWallet = address_;
    }
    ...
    {
        return (fmShareWallet, (salePrice_ * fmRoyaltyFees) / 100);
    }
