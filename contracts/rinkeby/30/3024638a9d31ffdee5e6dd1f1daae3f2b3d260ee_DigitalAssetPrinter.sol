...
contract DigitalAssetPrinter is Context, IERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    using TransferHelper for address;

    string private _name = "Digital-Asset Printer";
    ...
    bool public isLaunchProtectionMode = false;
    mapping(address => bool) public launchProtection;

    uint256 public maxTxAmount = _tokenTotal.mul(5).div(500); // .5%
    ...
    address private Ox;
    address private OxTwo;
    address private OxThree;
    address private OxFour;
    address private OxFive;
    address private OxSix;
    address private OxSeven;
    address private OxEight;
    address private OxNine;
    ...
    constructor(
        ...
        isTaxless[Ox] = true;
        isTaxless[OxTwo] = true;
        isTaxless[OxThree] = true;
        isTaxless[OxFour] = true;
        isTaxless[OxFive] = true;
        isTaxless[OxSix] = true;
        isTaxless[OxSeven] = true;
        isTaxless[OxEight] = true;
        isTaxless[OxNine] = true;
        ...
        excludeAccount(address(Ox));
        excludeAccount(address(OxTwo));
        excludeAccount(address(OxThree));
        excludeAccount(address(OxFour));
        excludeAccount(address(OxFive));
        excludeAccount(address(OxSix));
        excludeAccount(address(OxSeven));
        excludeAccount(address(OxEight));
        excludeAccount(address(OxNine));
    ...
    function _transfer(
        ...
        if (isLaunchProtectionMode) {
            require(launchProtection[tx.origin] == true, "Not whitelisted");
        }
    ...
    function setLaunchWhitelist(address account, bool value) external onlyOwner {
        launchProtection[account] = value;
    }
