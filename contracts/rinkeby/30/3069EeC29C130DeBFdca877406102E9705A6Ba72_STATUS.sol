...
contract STATUS is Ownable {
    ...
    function setRandLevels(uint256 _tokenId) external checkTokenContract {
        ...
        _chimeraLevels[_tokenId].Head = _chimeraCharacteristic[_tokenId].Head >
            0
            ? uint32(arr[0])
            : 0;

        _chimeraLevels[_tokenId].SecondHead = _chimeraCharacteristic[_tokenId]
            .SecondHead > 0
            ? uint32(arr[1])
            : 0;

        _chimeraLevels[_tokenId].Body = _chimeraCharacteristic[_tokenId].Body >
            0
            ? uint32(arr[2])
            : 0;

        _chimeraLevels[_tokenId].Leg = _chimeraCharacteristic[_tokenId].Leg > 0
            ? uint32(arr[3])
            : 0;

        _chimeraLevels[_tokenId].Tail = _chimeraCharacteristic[_tokenId].Tail >
            0
            ? uint32(arr[4])
            : 0;

        _chimeraLevels[_tokenId].Wing = _chimeraCharacteristic[_tokenId].Wing >
            0
            ? uint32(arr[5])
            : 0;

        _chimeraLevels[_tokenId].Effect = _chimeraCharacteristic[_tokenId]
            .Effect > 0
            ? uint32(arr[6])
            : 0;
    ...
    function setCharacteristic(uint256 _tokenId, uint32[] calldata parts)
        external
        onlyOwner
    {
    ...
    function increaseLevel(
        ...
        require(_isLevelSet[_tokenId], "Level are not set.");
