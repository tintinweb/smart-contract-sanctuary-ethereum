...
|______.'  '.__.'.',__`  '.__.'[___||__]|_____|  [___].',__`  '.__.' '.__.' [___||__][\__) ) 
                ( ( __))                             ( ( __)) 
...
contract DegenPigeonsStaking is ERC20Burnable, Ownable {
    ...
    uint256 public constant LEGENDARY_EMISSION_RATE = 25; // 200 per day
    uint256 public constant PIGEONS_EMISSION_RATE = 10; // 10 per day
    address public constant PIGEONS_ADDRESS = 0x8539440DDFaE82B74653ED35144745E8A7995F20;
    ...
    function getStakedPigeon(address staker) public view returns (uint256[] memory) {
        return stakerToPigeon[staker];
    }
    
    function getStakedAmount(address staker) public view returns (uint256) {
        return stakerToPigeon[staker].length;
    }

    function getStaker(uint256 tokenId) public view returns (address) {
        return pigeonsStaker[tokenId];
    }
    ...
    function getAllRewards(address staker) public view returns (uint256) {
        ...
        uint256[] memory pigeonsTokens = stakerToPigeon[staker];
        for (uint256 i = 0; i < pigeonsTokens.length; i++) {
            totalRewards += getReward(pigeonsTokens[i]);
        }
    ...
    function stakePigeonsById(uint256[] calldata tokenIds) external stakingEnabled {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            _degenPigeonsContract.transferFrom(msg.sender, address(this), id);

            stakerToPigeon[msg.sender].push(id);
            pigeonsTimeStaked[id] = block.timestamp;
    ...
    function unstakePigeonsByIds(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 id = tokenIds[i];
            require(pigeonsStaker[id] == msg.sender, "NEEDS_TO_BE_OWNER");
    ...
    function unstakeAll() external {
        ...
        for (uint256 i = stakerToPigeon[msg.sender].length; i > 0; i--) {
            uint256 id = stakerToPigeon[msg.sender][i - 1];

            _degenPigeonsContract.transferFrom(address(this), msg.sender, id);
            totalRewards += getReward(id);

            stakerToPigeon[msg.sender].pop();
            pigeonsStaker[id] = address(0);
        }
    ...
    function claimAll() external {
        ...
        uint256[] memory pigeonsTokens = stakerToPigeon[msg.sender];
        for (uint256 i = 0; i < pigeonsTokens.length; i++) {
            uint256 id = pigeonsTokens[i];
    ...
    function burn(address from, uint256 amount) external {
        require(msg.sender == PIGEONS_ADDRESS, "NOT_AUTHORIZED");
    ...
    function getReward(uint256 tokenId) internal view returns(uint256) {
        ...
        uint256 stakedTime = block.timestamp - pigeonsTimeStaked[tokenId];
        uint256 daysBonus;
        uint256 reward;
        ...
        if (tokenId == 7 || tokenId == 44 || tokenId == 101 || tokenId == 152 || tokenId == 235 || tokenId == 279 || tokenId == 306 || tokenId == 403) {
            EMISSION_RATE = LEGENDARY_EMISSION_RATE;
        } else {
            EMISSION_RATE = PIGEONS_EMISSION_RATE;
        }
