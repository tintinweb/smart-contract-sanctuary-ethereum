	...
	function getMultiplier(
		...
		if (_from > bonusInfo[bonusInfo.length.sub(1)].endBlock) {
			...
			for (uint16 t = _multiplierIndex; t < bonusInfo.length; t++) {
				...
				if (t > 0 && bonusInfo[t].startBlock.to256().sub(bonusInfo[t - 1].endBlock) > 1) {
					blocks = blocks.add(
						bonusInfo[t].startBlock.to256().sub(bonusInfo[t - 1].endBlock)
					);
				}
				...
				if (t == bonusInfo.length.sub(1) && _to > bonusInfo[t].endBlock) {
					blocks = blocks.add(
						bonusInfo[t].endBlock.to256().sub(_from).mul(bonusInfo[t].multiplier).add(
							_to.sub(bonusInfo[t].endBlock)
						)
					);
				} else {
					blocks = blocks.add(
						_to < bonusInfo[t].endBlock
							? _to.sub(_from).mul(bonusInfo[t].multiplier)
							: bonusInfo[t].endBlock.to256().sub(_from).mul(bonusInfo[t].multiplier)
					);
				}
			}
		}
