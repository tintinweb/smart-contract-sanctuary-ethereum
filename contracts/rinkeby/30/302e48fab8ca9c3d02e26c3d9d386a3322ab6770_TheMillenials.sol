...
// ================== Variables Start =======================
  ...
  bytes32 public root = 0x3953a7f0c4806b0587e3409a3e0d3c21d1358703abbe9ee73e5796bc2f956d6c; 
...
function mint(uint256 qty, bytes32[] calldata _merkleProof)public payable {
        bool iswhitelisted= checkValidity(_merkleProof, msg.sender);
        require(iswhitelisted= true,"Not whitelisted");
        {
         uint256 bal=  addressminted[msg.sender];
          if (bal==0 || qty==1)
          {
          ...
          if(bal==0 || qty>1){
        require((qty-1)*cost <=msg.value, "Ether amount sent is incorrect");
              addressminted[msg.sender] = addressminted[msg.sender]+qty;
            _safeMint(msg.sender,qty);
          }
          if (bal>0){
        require(qty*cost <=msg.value, "Ether amount sent is incorrect");
            addressminted[msg.sender] = addressminted[msg.sender]+qty;
            _safeMint(msg.sender,qty);
...
function Cost()public view returns (uint256 costt){
    if (whitelistenable==true){
    return cost;}
    if(whitelistenable==false){
    return cost2;
    }
    }
// ================== Variables End =======================  
...
// sales toggle
  ...
  function setWhitelistMintStatus() public onlyOwner {
...
function tokensOfOwner(address owner) external view returns (uint256[] memory) {
  ...
  function _startTokenId() internal view virtual override returns (uint256) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        return string(abi.encodePacked("https://TheMillenials.com/token/unreveal.json"));
    }
    else{
