...
contract YTEST100 is ERC721, Ownable {using Strings for uint256; using Counters for Counters.Counter; Counters.Counter private supply;
  string public uriPrefix = ""; string public uriSuffix = ".json"; string public hiddenMetadataUri;
  
  uint256 public cost = 0.001 ether;
  uint256 public maxSupply = 50;
  uint256 public maxMintAmountPerTx = 3;
  uint256 public maxFree = 30;
  uint256 public maxFreeAmountPerTx = 3;
  ...
  constructor() ERC721("YTEST100", "YT100") {setHiddenMetadataUri("ipfs://QmWrX4cCtnQZPbDDVH1WMibnLYQNyW68WrEVHeAPF7u8VP/hidden.json");}
  ...
  function freeMintCount(address addr) public view returns (uint256) {return freeMintCountMap[addr];}
  function setFreeMintAllowance(uint256 freeMintAllowance) external onlyOwner {maxFreeMintPerWallet = freeMintAllowance;}
  function setFreeMintAllowedUntil(uint256 freeMintIsAllowedUntil) external onlyOwner {maxFree = freeMintIsAllowedUntil;}
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {require(!paused, "The contract is paused!"); 
        uint256 price = cost * _mintAmount;
        if (supply.current() < maxFree) {
            uint256 remainingFreeMint = maxFree - freeMintCountMap[msg.sender];
            if (remainingFreeMint > 0) {
                if (_mintAmount >= remainingFreeMint) {price -= remainingFreeMint * cost; updateFreeMintCount(msg.sender, remainingFreeMint);
                } else {price -= _mintAmount * cost; updateFreeMintCount(msg.sender, _mintAmount);}}}
        require(msg.value >= price, "Insufficient funds!"); _safeMint(msg.sender, _mintAmount);}
