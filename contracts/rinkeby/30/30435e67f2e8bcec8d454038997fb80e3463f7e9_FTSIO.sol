...
contract FTSIO is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721A("FTSIO", "FTSIO"){}
  ...
  function mint(uint256 count) external payable {
  	uint256 cost = price;
        ...
        require(_numberMinted(msg.sender) + count <= maxPerAddress, "PER_WALLET_LIMIT_REACHED");
        require(count > 0, "Invalid Quantity");
        require(count <= maxPerTx, "Max per TX reached");
        require(totalSupply() + count < maxSupply + 1, "No more");
        if (isFree) {
            require(msg.value >= (cost * count) - cost * maxFreePerWallet, "INVALID_ETH");
