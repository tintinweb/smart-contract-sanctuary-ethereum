...
contract RandomIpfsNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    /*   when we mint an nft we trigger a chainlink VRF  call to get us a random number using that number we will get a random nft cutie,shiba inu,st. Benerad 
    cuties are super rare 
    shiba inu is sort of rare 
    st. Benerad is common 
    
    users also have to pay to mint an nft
    and then the owner of the contract can withdraw the eth
   */
    //type declaration
    enum Breed {
        CUTIE,
    ...
    uint internal constant c_max_chance_value = 100;
    string[] internal s_nftTokenUri;
    uint internal immutable i_mintFee;

    //Events
    event RequestedNft(uint indexed requestId, address requester);
    event ConfirmedNft(Breed nftBreed, address minter);
        ...
        override
    {
        ...
        uint moddedRng = randomWords[0] % c_max_chance_value;
        //it will always returns a value between 0 and 99
        //so 7 means its a cutie
        //88 means its a st. Benerad
        //45 means its a st. Benerad
        //15 means its a shiba inu
        Breed NftBreed = getBreedFromModdedRng(moddedRng);
        s_tokenCounter = s_tokenCounter + 1;
        _safeMint(owner, newTokenId);
        _setTokenURI(newTokenId, s_nftTokenUri[uint(NftBreed)]);
        emit ConfirmedNft(NftBreed, owner);
    }
    ...
    function getChanceArray() public pure returns (uint[3] memory) {
        return [10, 30, c_max_chance_value];
    }
