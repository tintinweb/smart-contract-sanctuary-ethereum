...
contract Perpetual is Users, Deposits, Withdrawals, UpdateState, GlobalConfig, Dac {
    ...
    struct perpetualParams {
        ...
        I_Verifier escapeVerifierAddress;
    ...
    function escape(
        ...
        require(!escapesUsed[packedKey], "aae"); 
        require(_amount < (1 << 63), "aofr");

        bool proofCorrect = escapeVerifier.verifyEscapeProof(accountRoot, orderStateHash, _accountId, _l2Key, _amount, _proof);
        require(proofCorrect, "x");

        uint256 externalAmount = _amount * (10 ** (systemTokenDecimal - innerDecimal));
...
abstract contract Users is Storage {
    ...
    function registerUser(
	    ...
	    for (uint32 i = 0; i < l2Keys.length; ++i) {
            require(l2Keys[i] < (1 << 160), "l2KeyError");
...
abstract contract UpdateState is ForcedWithdrawals, ForcedTrade, Config {
    ...
    function createBlockCommitment(
        ...
        bytes memory h = abi.encodePacked(newBlock.blockNumber, oldAccountRoot,newBlock.accountRoot,oldOrderStateHash,newOrderStateHash, globalConfigHash,newBlock.validiumAccountRoot);
        ...
        alignSize = newBlock.blockChunkSize * Operations.FORCED_OP_PUBDATA_BYTES;
        bytes32 forced_commitment = pubdataPadCommitment(newBlock.forcedPubData, alignSize);
    ...
    function processForcedAction(
        ...
        uint256 offset = 0;
        while (offset < pubData.length) {
            Operations.ForcedAction memory op = Operations.readForcedActionPubdata(pubData, offset);
...
contract Storage is Governance, ReentrancyGuard {
    ...
    function removeForcedRequest(uint160 req, bool isForcedWithdrawal) internal {
        require(forcedActionRequests[req] != 0, "cfr0");
        if (isForcedWithdrawal) {
            delete forcedActionRequests[req];
        } else { // forcedTrade
            require(forcedActionRequests[req] != ~uint256(0), "cfr1");
            forcedActionRequests[req] = ~uint256(0);
        }
    }

    function freeze(uint160 req) public {
		require(forcedActionRequests[req] != 0 && forcedActionRequests[req] != ~uint256(0), "fare");
		require(forcedActionRequests[req] + FORCED_ACTION_EXPIRE_TIME > block.timestamp, "ftne");
		stateFrozen = true;
    }
...
abstract contract ForcedWithdrawals is Storage {
    ...
    function forcedWithdrawalRequest(
		...
		require(quantizedAmount < (1 << 63), "fwria");
...
abstract contract ForcedTrade is Storage {
    ...
    function forcedTradeRequest(
		...
		require(syntheticAssetId > 0 && syntheticAssetId <= MAX_ASSETS_COUNT, "assetId out of range");
		require(amountCollateral < (1 << 63), "amountCollateral");
		require(amountSynthetic < (1 << 63), "amountSynthetic");
		require(submissionExpirationTime >= block.timestamp, "out of date");
	...
	function verifySignature(
        ...
        bytes32 signedData = keccak256(abi.encodePacked(uint256(actionHash), submissionExpirationTime));
        address signer;
        {
        ...
        require(l2KeyAddr != address(0), "UnregisteredB");
        require(signer == l2KeyAddr, "InvalidSig");
    }
}