...
contract CryptoMoonie is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public maxPremiumMintAmount = 3;
    uint256 public maxVipMintAmount = 10;

    uint256 public remainingMembershipMints = 100;
    uint256 public remainingPromoMints = 150;
    uint256 public remainingTeamMints = 30;
    ...
    address internal coldStorage =
        payable(0x054499A5FEC4faf545610383ee52136136672e77);
    address internal team = payable(0xb3a05B0feCC927e32ab448415C7D0EFC694fD5E4);
    ...
    modifier membershipCompliance(
        ...
        require(_memberType == memberType, "Invalid membership type");
        require(
            remainingMembershipMints - _mintAmount >= 0,
            "Exceeds membership supply"
        );
        _;
    }

    /// @dev We confirm that the user has sent the appropriate amount of ether
    /// to pay for the mint.
    ...
    {
        // Require the allowlist to be acive.
        require(allowlistMintEnabled, "The allowlist is not enabled!");
        // Confirm the user has not pre-minted already.
        require(!allowlistMinted[_msgSender()], "Address already claimed!");
        // Generate a leaf hash for the user.
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        // Verify the merkle proof that was generated from the allowlist.
        require(
            MerkleProof.verify(_merkleProof, merkleRoot, leaf),
            "Invalid proof!"
        );
        // Register the users address as minted.
     ...
     * We have provided a checkMembershipToken function that will allow users to check to see if a
     * membership token has already minted. We will also have a client-side utility that will allow
     * users to check the mint status of a specific token id at
     * https//creekdrops.io/tools/check-membership.
     */

    /// @notice Premium perks include two free mints per Premium Card
    /// holder + bonus mint at 0.025 ETH.
    function premiumMint(uint256 _mintAmount, uint256 _tokenId)
        public
        payable
        notPaused
        mintCompliance(_mintAmount, maxPremiumMintAmount)
        membershipCompliance(maxMintAmount, _tokenId, MembershipType.PREMIUM)
    {
        /// @dev We are confirming that the user can only mint two free mints.
        if (_mintAmount > 2 && msg.value < (_mintAmount - 2) * 0.025 ether) {
    ...
    function remainingReservedMints() public view returns (uint256) {
        return
            remainingPromoMints + remainingTeamMints + remainingMembershipMints;
    }
        ...
        returns (bool _isMember, MembershipType _memberType)
    {
        ...
        MembershipType memberType = _tokenId <= 100
            ? MembershipType.PREMIUM
            : MembershipType.VIP;
        return (isMember, memberType);
    }
    ...
    function setCurrentMaxSupply(uint256 _supply) public onlyOwner {
        require(
            _supply <= finalMaxSupply && _supply >= currentMaxSupply,
            "Invalid supply"
        );
    ...
    function withdraw() public onlyOwner nonReentrant {
        ...
        bool success;
        (success, ) = coldStorage.call{value: ((balance * 85) / 100)}("");
        require(success, "Transaction Unsuccessful");

        (success, ) = team.call{value: ((balance * 15) / 100)}("");
        require(success, "Transaction Unsuccessful");
    }
}
