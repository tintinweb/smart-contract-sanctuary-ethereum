...
contract AncientDevice is ERC721Enumerable, OwnableWithAdmin, Pausable {
  ...
  uint256 constant SECONDS_PER_DAY = 1 minutes; //rinkeby ... TODO delete for mainnet
  uint constant YIELD_END_TIME = 1648058400; // rinkeby: March 23, 2022, 2pm ET ... TODO : delete for mainnet
  // TODO mainnet: uint constant YIELD_END_TIME = 1742839200; // March 24, 2025, 19:00 UTC: Mercury's first Inferior Conjunction of 2025
  ...
  // Emergency coin migration mechanism
  // TODO: Owner decides if necessary
  ...
  function getUpgradeCost(uint256 _tokenId, uint _targetLevel) external view returns (uint256) {
    ...
    while(level < _targetLevel){
      ...
      else if (level == 2) totalCost += 30 ether;
      else if (level == 3) totalCost += 120 ether;
      else if (level == 4) totalCost += 480 ether;
      level++;
    }
    return totalCost;
  }
...
contract rMAGNESIUM is ERC20, OwnableWithAdmin {

  uint256 constant HARD_CAP = 22000000 ether; // the actual max supply will likely be much lower when yielding ends

  // a mapping from an address to whether or not it can mint / burn. Intended for Melange Labs contracts only
  mapping(address => bool) controllers;
  
  constructor() ERC20("rMAGNESIUM", "rMAG") { } // TODO change for mainnet
  ...
  /**
   * enables an address to mint / burn (Intended for Melange Labs contracts only)
