...
/**
 * @title BiletsClubHunt Contract
 */

contract BiletsClubHunt is Ownable, ReentrancyGuard {
    string public name = "Bilets Club Hunt";
    string public symbol = "BLT-Hunt";
    ...
    function huntBLT(
        ...
        require(
            atackTime[tokenID] <= (block.timestamp + (huntTimeMinutes * 60)),
            "You need to wait for the next hunt."
        );
        require(
            contractBiletsClubCharStatus.getTokenHealth(tokenID) >= damage,
            "You do not have enough health."
        );
        // Last Atack
        atackTime[tokenID] = block.timestamp;
        // earn?, tokenID, amountExp, amountHp, amountMana
        contractBiletsClubCharStatus.changeStatus(true, tokenID, expEarn, 0, 0);
        contractBiletsClubCharStatus.changeStatus(false, tokenID, 0, damage, 0);
        // Token Earn
        contractBiletsClubToken.mint(msg.sender, tokenEarn);
    }
...
contract BiletsClub is ERC721A, Ownable {
    ...
    uint256 public constant maxMintsPerWallet = 2;
