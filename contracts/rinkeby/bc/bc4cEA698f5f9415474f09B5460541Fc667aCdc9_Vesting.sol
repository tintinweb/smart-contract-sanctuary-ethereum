    ...
    BasicMetaTransaction
{
    ...
    mapping(uint256 => mapping(address => uint256)) propertyOwnerToken;
    ...
    function startVestingForEstateID(uint256 _estateID) external onlyOwner {
        require(
    ...
    function updateVestingForEstateID(uint256 _estateID) external onlyOwner {
        require(
        ...
        returns (uint256)
    {
        ...
        IICO.ICO_SetupStruct memory icoData = ICO.getICOinfo(_estateID);
        IDEED.Agreement memory deedData = DEED.agreements(_estateID);
        require(!icoData.state, "ICO: estateID is paused");
        if (
    ...
    function _claim(
        ...
        if (musd <= cliamablePrice) {
            ...
            emit ReleasedMogulToken(claimAddress, transferAbleToken, _estateID,musd);
            musdToken.safeTransferFrom(claimAddress, propertyOwnerAddress[_estateID], musd);
        ...
        onlyPropertyOwner(_estateID, _msgSender())
    {
        ...
        uint256 transferAbleToken = uint256(
            propertyOwnerToken[_estateID][claimAddress]
        );
        tempAdminStruct.currentEstateTokenBal += transferAbleToken;
        emit ReleasedMogulToken(claimAddress, transferAbleToken, _estateID, 0);
        delete propertyOwnerToken[_estateID][claimAddress];
        estateToken.safeTransferFrom(
            address(this),
            claimAddress,
            _estateID,
    ...
    function updateAdminRoles(
        ...
        propertyOwnerToken[_estateID][_propertyOwner] = deedData
