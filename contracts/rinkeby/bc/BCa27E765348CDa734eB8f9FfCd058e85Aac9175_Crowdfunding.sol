...
contract Crowdfunding is Ownable,ReentrancyGuard{
    ...
    mapping(address => uint) public rewardClaimed;

    //Fund

    function fund() external payable{
        require(raised < goal,"Limit Reached");
        require(block.timestamp <= endDate,"Crowfunding Closed");

        if(raised.add(msg.value) > goal){
            uint value = raised.add(msg.value).sub(goal);
            userAmount[msg.sender] = userAmount[msg.sender].add(value);
    ...
    function claimReward() public{
        ...
        require(block.timestamp > endDate,"Not Now");
        require(userAmount[msg.sender] > 0);
        require(rewardClaimed[msg.sender] < userAmount[msg.sender]);

        uint reward = userAmount[msg.sender].sub(rewardClaimed[msg.sender]);
        token.mint(msg.sender, reward);
        rewardClaimed[msg.sender] = userAmount[msg.sender];
    }

    //Withdraw if goal is not reached
    function claimRefund() public nonReentrant{
        require(address(this).balance < goal && block.timestamp > endDate,"Not Now");
        require(userAmount[msg.sender] > 0,"Zero Balance");

        uint amount = userAmount[msg.sender];
        (bool result,) = payable(msg.sender).call{value: amount}("");
        require(result, "Failed To Claim");
