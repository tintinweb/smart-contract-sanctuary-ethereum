...
contract ICO is SpaceToken {
    ...
    // Manually tracking the balance is beeded to protect against `selfdestruct(address)` where someone could
    ...
    function withdraw(address to) external {
        ...
        uint256 tokensToSend = tokensBought[msg.sender];
        if (tokensToSend == 0) revert NothingToWithdraw();

        tokensBought[msg.sender] = 0;

        _transfer(address(this), to, tokensToSend);
    }
}
