...
contract BaseAuction is Ownable, Pausable {
  ...
  uint256 public constant endingWindowDuration = 3600;
  ...
  */
  ...
  /// @dev Attempt to close auction if it's within the ending window
  function _attemptToCloseAuction() private {
    if (block.timestamp >= endTime) {
      _closeAuction();
    }
    else if (block.timestamp >= (endTime - endingWindowDuration)) {
      uint256 randomNumber = uint256(keccak256(abi.encode(block.timestamp, block.number, blockhash(block.number-1))));
      if (randomNumber % (endTime - block.timestamp) == 0) {
        _closeAuction();
      }
    }
  }
