...
contract FreeBreadsticks is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
    ...
    //Required arguments for ERC721A constructor
    uint256 private constant TotalCollectionSize_ = 6969; // total number of nfts

    uint256 private freebies;
    uint16 private freebieLimit = 6549;
        ...
        )
    {
        ...
        setBaseURI(
            "ipfs://PINATA_CID/" //make sure the slash / is at the end of it
        );
    ...
    function mint(uint256 quantity) public payable {
        ...
        require(
         ...
         if(!isFree || totalSupply() + quantity > freebieLimit) {
                require(msg.value >= price * quantity, "insufficient funds");
            }
            else {
                require(freebies + quantity <= freebieLimit, "not enough freebies left");
                freebies = freebies + quantity;
            }
    ...
    function setMAX_PER_WALLET(uint256 q) public onlyOwner {
        MAX_PER_WALLET = q;
    }
    ...
    function FreebieTime(bool _state) public onlyOwner {
        isFree = _state;
    }
