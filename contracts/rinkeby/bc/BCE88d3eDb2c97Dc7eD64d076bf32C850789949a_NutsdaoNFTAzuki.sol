...
contract NutsdaoNFTAzuki is ERC721AUpgradeable, OwnableUpgradeable {
    string public baseTokenURI;
    uint256 public maxSupply;

    function initialize(string memory _baseTokenURI, uint _maxSupply)
        public
        initializerERC721A
        initializer
    {
        __ERC721A_init("DicksonTestV2", "PTS");
        __Ownable_init();
    ...
    function mint(uint8 quantity) external payable {
        require(msg.value >= quantity * 0.1 ether, "Ether is insufficient");
        require(
            balanceOf(msg.sender) + quantity <= 5,
            "You can only mint a maximum of 5 NFTs"
        );
