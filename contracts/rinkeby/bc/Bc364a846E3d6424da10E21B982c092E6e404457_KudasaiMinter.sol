...
contract KudasaiMinter is Ownable {
    ...
    function setKudasaiListMerkleRoot(bytes32 _merkleRoot, uint256 _round) external onlyOwner {
    ...
    function ban(uint256[] memory _ids) external onlyOwner {
        for (uint256 i = 0; i < _ids.length; i++) {
            claimed[_ids[i]] = true;
        }
    }
    ...
    function kantsu(uint256 _quantity, bytes32 _code) public isNotContract {
    ...
    function kudasai(uint256[] memory _ids) public onlyKudasaiHolder(_ids) isNotContract {
