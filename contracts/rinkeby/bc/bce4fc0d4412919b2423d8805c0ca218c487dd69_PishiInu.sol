...
contract PishiInu is Context, IERC20, Ownable 
{
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0x11E2363Fb2019e2b69B7a96EEa839685f934851a);
    ...
    string private _name = "Pishi Inu";
    string private _symbol = "PISHU";
    ...
    function setExtraEarlySellFee(uint256 _fee) external onlyOwner
    {
        extraEarlySellFee = _fee;
        require(_fee<=25, "Too High Fee");
    }


    function enableExtraEarlySellFee(bool _enable) external onlyOwner
    {
    ...
    function setAllBuyFeePercentages(uint256 liquidityFee, uint256 marketingFee, uint256 burnFee) 
    external onlyOwner()
    {
        ...
        require(totalFee<=25, "Too High Fee");
        emit updateBuyFee(totalFee, block.timestamp);
    }


    function setLiquidityBuyFeePercentage(uint256 liquidityFee) 
    external onlyOwner()
    {
        _liquidityFee = liquidityFee;
        _previousLiquidityFee = liquidityFee;      
    ...
    function setMarketingBuyFeePercentage(uint256 marketingFee) 
    external onlyOwner()
    {
        _marketingFee = marketingFee;
        _previousMarketingFee = marketingFee;
    ...
    function setBurnBuyFeePercentages(uint256 burnFee) 
    external onlyOwner()
    {
    ...
    function setAllSaleFeePercentages(uint256 liquidityFee, uint256 marketingFee, uint256 burnFee) 
    external onlyOwner()
    {
    ...
    function setLiquiditySellFeePercentage(uint256 liquidityFee) 
    external onlyOwner()
    {
        _sellLiquidityFee = liquidityFee;
        uint256 totalFee = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBurnFee);
    ...
    function setMarketingSellFeePercentages(uint256 marketingFee) 
    external onlyOwner()
    {
        _sellMarketingFee = marketingFee;
        uint256 totalFee = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBurnFee);
    ...
    function setBurnSellFeePercentages(uint256 burnFee) 
    external onlyOwner()
    {
        _sellBurnFee = burnFee;
        uint256 totalFee = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBurnFee);
    ...
    {
        noTxFee = false;
        _maxTxAmount = _tTotal.div(100).mul(2);
    ...
    function setNoTxFeeEnabled(bool _enable) external onlyOwner
    {
        noTxFee = _enable;
    }
    ...
    event AccountBlacklisted(address _account, uint256 timestamp);
    event AccountUnBlacklisted(address _account, uint256 timestamp);

    function blacklistAddress(address account) external onlyOwner
    {
        require(_isBlacklisted[account] == false,"Address is already blacklisted");
        _isBlacklisted[account] = true;
        emit AccountBlacklisted(account,block.timestamp);
    }
