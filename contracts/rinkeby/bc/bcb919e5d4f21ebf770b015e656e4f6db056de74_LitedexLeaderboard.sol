...
interface ILitedexInitalOffering {
    function allReferrer() external returns(address[] memory);
    function getTotalReferrer() external returns(uint);
    function getBnbPrice() external returns(uint256);
...
contract LitedexLeaderboard is TimeCrowdsale{
    ...
    function alreadyClaim(address account) public view returns(bool){
    ...
    function getCustomerData(address account) public view returns(bytes3 referralId,
        uint totalReferred,
        uint totalReferredUsd,
        uint usdPurchased,
        uint tokenPurchased,
    ...
    function getId(address account) public view returns(bytes3 id){
        (bytes3 rid, , , , , , ) = litedexico.accounts(account);
        return rid;
    }
    function getTotalReffered(address account) public view returns(uint tReffered){
        (, uint totalReferred, , , , , ) = litedexico.accounts(account);
        return totalReferred;
    }
    function getTotalRefferedUsd(address account) public view returns(uint tRefferedUsd){
        (, , uint totalReferredUsd, , , , ) = litedexico.accounts(account);
        return totalReferredUsd;
    }
    function getTokenPurchase(address account) public view returns(uint tPurchased){
        (, , , , uint tokenPurchased, , ) = litedexico.accounts(account);
        return tokenPurchased;
    }
    function getUsdToConverted(address account) public view returns(uint usdtc){
        (, , , , , uint usdToConverted, ) = litedexico.accounts(account);
        return usdToConverted;
    }
    function getPoint(address account) public view returns(uint point){
        (, , , , , , uint points) = litedexico.accounts(account);
        return points;
    }
    function updateLeaderboard(uint tDisplayedData) external onlyOwner returns(bool){
        address[] memory temp = ILitedexInitalOffering(address(litedexico)).allReferrer();
        uint n = ILitedexInitalOffering(address(litedexico)).getTotalReferrer();
        for (uint i = 0; i < n; i++){                     //Loop for descending ordering
		    for (uint j = 0; j < n; j++){            //Loop for comparing other values
			    if (getPoint(temp[j]) < getPoint(temp[i])){              //Comparing other array elements
                    address tmp = temp[i];         //Using temporary variable for storing last value
                    temp[i] = temp[j];            //replacing value
                    temp[j] = tmp;             //storing last value
			    }
		    }
	    }
        for(uint i=0;i< tDisplayedData;i++){
            rank[i] = temp[i];
        }
    }
    function leaderboard() external view returns(address[] memory){
    ...
    function claimBonus() external isClaimLocked isAlreadyClaim returns(bool){
        require(block.timestamp > end, 'Litedex: Claim after Crowdsale');
        uint256 _bonus = getUsdToConverted(msg.sender);
        uint256 _bnbPrice = ILitedexInitalOffering(address(litedexico)).getBnbPrice();
        ILitedexInitalOffering(address(litedexico)).safeTransferBNB(msg.sender, _bonus/_bnbPrice);
        return true;
    }
}