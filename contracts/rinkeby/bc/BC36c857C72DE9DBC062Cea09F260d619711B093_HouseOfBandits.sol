...
contract HouseOfBandits is ERC1155Base, EquitySplitter {
    ...
    constructor(
        ...
        string memory _uri,
        bytes32 _merkleRoot
    ) ERC1155(_uri) MintingBase(1, 0.1 ether, _merkleRoot) {
        name_ = _name;
        symbol_ = _symbol;
    }
    ...
    function ogMint(
        ...
        limitTxs
        onlyAllowList(_merkleProof)
    {
        uint8 maxAmountHouseAllowed = 2;
        uint8 maxTotalAmountAllowed = 3;
        _mintTokens(
            _amountMaster,
            _amountHouse,
            maxAmountHouseAllowed,
            maxTotalAmountAllowed
        );
    }
    ...
    function _mintTokens(
        ...
        require(
            !(_amountMaster == 0 && _amountHouse == 0),
            "Invalid amount of keys"
        );
        require(
            _amountMaster + _amountHouse <= _maxTotalAmountAllowed,
            "Invalid amount of keys"
        );
        require(
            totalSupply(HOUSE) + _amountHouse <= MAX_SUPPLY_HOUSE &&
        ...
        require(
            msg.value == (_amountMaster + _amountHouse) * mintPrice,
        ...
        emit Minted(tokenIds, msg.sender, _amountMaster + _amountHouse);
...
contract EquitySplitter is Context {
    ...
    constructor() payable {
        ...
        address spliffyAddress = 0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB;
        _addShareHolder(spliffyAddress, _getEquity(150));

        address djDabsAddress = 0x617F2E2fD72FD9D5503197092aC168c91465E7f2;
...
abstract contract MintingBase is Ownable {
    uint8 public maxTxs;
