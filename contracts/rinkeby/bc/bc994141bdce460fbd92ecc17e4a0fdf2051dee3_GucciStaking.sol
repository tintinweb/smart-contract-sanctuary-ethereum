...
contract GucciStaking is RewardsVault {
    ...
    modifier lockTheStake(){
        require(locked==false, "Stacking is locked.");
        _;
    }
    ...
    function getStakeInfoList(address _address) public view returns(Stake[] memory) {
		uint256 count = stakeCount[_address];
        Stake[] memory array = new Stake[](count);
        for(uint i = 0; i < count; i++){
            bytes16 stakeId = getStakeID(_address, i);
            array[i] = stakes[_address][stakeId];
        }
        return array;
    }
