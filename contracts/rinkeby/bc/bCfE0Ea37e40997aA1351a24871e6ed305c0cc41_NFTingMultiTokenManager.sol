...
contract NFTingMultiTokenManager is NFTingBase {
...
contract NFTingBase is Context, IERC721Receiver, IERC1155Receiver {
    ...
    modifier isApprovedMarketplace(address _nftAddress, uint256 _tokenId) {
        if (_supportsInterface(_nftAddress, INTERFACE_ID_ERC1155)) {
            if (
    ...
    PreAuthorization
{
    ...
    function publicMint(
        ...
        emit TokenMinted(curTokenId, _metadataId, _amount, _msgSender());
    }

    function additionalMint(uint256 _tokenId, uint256 _amount) external {
        if (exists(_tokenId) && tokenCreator[_tokenId] != _msgSender()) {
            revert PermissionDenied();
        }

        _mint(_msgSender(), _tokenId, _amount, "");

        emit TokenAdded(_tokenId, _amount, _msgSender());
    }
    ...
    function publicBatchMint(
        ...
        emit TokenBatchMinted(tokenIds, _metadataIds, _amounts, _msgSender());
    }
    ...
    {
        return string(abi.encodePacked(ERC1155.uri(_tokenId), "{id}.json"));
    }
...
abstract contract PreAuthorization is Ownable {
    ...
    function unauthorizeOperator(address _operator) public onlyOwner {
        if (authorizedAddresses.length == 0) {
            ...
            address lastOperator = authorizedAddresses[authorizedAddresses.length - 1];
