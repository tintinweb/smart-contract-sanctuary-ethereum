...
contract NFTStaking is Ownable, ReentrancyGuard {
  ...
  constructor(IERC20 _rewardsToken) {
    ...
    // addVault(0xeE5f115811d18a1c5D95457c83ba531Ce0c92f06, "Singularity 0 Universe ANTHRO");
    // addVault(0xa5c5198c6CE1611f1e998cf681450F8b9E599255, "Singularity 0 Universe HUMAN");
    // addVault(0xeE5f115811d18a1c5D95457c83ba531Ce0c92f06, "Singularity 0 Universe ANKOKU");
    addVault(0xe6096DeeDCAe8e15D3113a13Db8ae079ed9465ab, "Singularity 0 Universe ANTHRO");
    addVault(0x960962ED55CBb4b8fCA35324938f6D956b417049, "Singularity 0 Universe HUMAN");
    addVault(0xd82c2f4dA460eA5708Fb8b02f223c7DcFE219005, "Singularity 0 Universe ANKOKU");
  }
  ...
  function stake(StakeItem[] calldata _stakeItems) external nonReentrant {
    ...
    for (uint256 k; k < length; ++k) {
      ...
      for (uint256 i; i < len; ++i) {
        require(
          IERC721(vaultInfo[_pid].nft).ownerOf(_tokenIds[i]) == msg.sender,
          "Can't stake tokens you don't own!"
        );
        IERC721(vaultInfo[_pid].nft).transferFrom(msg.sender, address(this), _tokenIds[i]);
        vaultInfo[_pid].stakerAddress[_tokenIds[i]] = msg.sender;
      }
  ...
  function withdraw(StakeItem[] calldata _stakeItems) external nonReentrant {
    ...
    for (uint256 k; k < length; ++k) {
      ...
      uint256 len = _tokenIds.length;
      for (uint256 i; i < len; ++i) {
        require(vaultInfo[_pid].stakerAddress[_tokenIds[i]] == msg.sender);
        vaultInfo[_pid].stakerAddress[_tokenIds[i]] = address(0);
        IERC721(vaultInfo[_pid].nft).transferFrom(address(this), msg.sender, _tokenIds[i]);
      }
      stakers[msg.sender].amountStaked -= len;
      stakers[msg.sender].timeOfLastUpdate = block.timestamp;
      if (stakers[msg.sender].amountStaked == 0) {
