 ...
 * @dev Extends ERC721A
 */
contract MinterCow is ERC721ABurnable, AllowListSale, AccessControlEnumerable, ReentrancyGuard {
    using ECDSA for bytes32;

    /**
    @notice Role of administrative users allowed to expel a Cows from the pasture.
    @dev See expelFromPasture().
     */
    ...
    // default sale confgis
    uint256 private defaultPublicSaleId = 0;

    uint256 public constant COLLECTION_SIZE = 32;

    //constructor() ERC721("MinterCow", "MC") {}
    // for testing dates: 1653761671, 1656440071
    // price 10000000000000 0.00001
    ...
    constructor(
        ...
        ERC721A(name, symbol)
    {
        // _addSale(1, SaleConfig(1653761671, 1656440071, 10000000000000, 5, 0x9408F08E123A0452744ea1C089fD5694ba5aBb81, false, false, true));
    }

    // @dev Setup AllowListSale

    /**
    @dev creates allowList config to the given id
    */
    function addSale(
        uint256 id,
        uint256 startDate,
    ...
    /**
    @dev creates publicSale config to the given id
    */
    ...
    */
    ...
    /**
    @dev mints with verifying the provided key
    */
    ...
    /**
    @dev mints on public sale without verifying a key
    */
    ...
    function publicMint2(uint256 quantity) external payable nonReentrant {
        require(totalSupply() + quantity <= COLLECTION_SIZE, "purchase would exceed max supply of Cows");
        verifyPublicSale(quantity, defaultPublicSaleId);
        handlePurchase(quantity, defaultPublicSaleId, msg.sender);
        _safeMint(msg.sender, quantity);
    }

    function ownerMint(uint256 quantity, address recipient) external onlyOwner {
        require(totalSupply() + quantity <= COLLECTION_SIZE, "purchase would exceed max supply of Cows");
    ...
    // grazing (heavily inspired by moonbirds contract)
    ...
    /**
    @dev Longest continuous grazing streake., excluding the current period.
     */
    mapping(uint256 => uint256) private grazingMax;

    /**
    @notice Returns the length of time, in seconds, that the Cow has
    been grazing.
    @dev Grazing is tied to a specific Cow, not to the owner, so it doesn't
    reset upon sale.
    @return grazing Whether the Cow is currently grazing. MAY be true with
    zero current nesting if in the same block as grazing began.
    @return current Zero if not currently grazing, otherwise the length of time
    since the most recent grazing began.
    @return total Total period of time for which the Cow has been grazing across
    its life, including the current period.
    @return max Longest continous period of time for which the Cow has been
    grazing across its life, including the current period.
     */
    ...
    /**
    @dev MUST only be modified by safeTransferWhileGrazing(); if set to 2 then
    the _beforeTokenTransfer() block while grazing is disabled.
     */
    uint256 private grazingTransfer = 1;

    /**
    @notice Transfer a token between addresses while the Cow is grazing,
    thus not resetting the grazing period.
     */
    ...
    /**
    @dev Block transfers while grazing.
     */
    ...
    /**
    @dev Emitted when a Cow stops grazing; either through standard means or
    by expulsion.
     */
    ...
    */
    ...
    /**
    @dev Changes the Cow' grazing status.
     */
    ...
    /**
    @notice Admin-only ability to expel a Cow from grazing.
    @dev As most sales listings use off-chain signatures it's impossible to
    detect someone who has been grazing and then deliberately undercuts the floor
    ...
    the undercutting cow to be sold on the open market. Since OpenSea uses
    ...
    Cows.
     */
    function expelFromPasture(uint256 tokenId) external onlyRole(EXPULSION_ROLE) {
 ...
 *        - fixed price
 *        - quantity limit
 *        - allow-lists with signature verification (we choose this method
 *                since it's more cost efficient with lists larger than 127
 *                members than merkle trees)
 *        - public sales without signature verification
 *    This contracts aims to handle the needed verifications and also to
 *        provide a flexible dynamic configuration option.
 */
contract AllowListSale is Ownable {
    ...
    struct SaleConfig {
        ...
        uint256 price;
        uint256 quantityLimit; // how many tokens can be minted part of the sale
     ...
     *    every saleKey can be used only once
     */
     ...
     *      - address in it matches with the sender's address
     *      - it is signed with the stored signer address
     *      - returns true when it the sale is a public sale
     */
    ...
    function verifyQuantity(uint256 quantity, uint256 saleId) internal virtual view {
        require(quantity <= saleConfig[saleId].quantityLimit, "sale limit is exceeded for this address");
        require(quantity * saleConfig[saleId].price <= msg.value, "not enough value was sent to complete the purchases");
    }
    ...
    function handlePurchase(uint256 quantity, uint256 saleId, address sender) internal {
        uint256 cost = quantity * saleConfig[saleId].price;
        require(cost <= msg.value, "not enough value was sent to complete the purchases");

        if (msg.value > cost) {
            address payable reimburse = payable(sender);
            uint256 refund = msg.value - cost;
            Address.sendValue(reimburse, refund);
        }
        ...
        return signer == 
            keccak256(
                abi.encodePacked(
                    ...
                    bytes32(uint256(uint160(sender))) // creates the 0x000000000000000000000000<sender> expected foramt
                )
            ).recover(signature);
    }
