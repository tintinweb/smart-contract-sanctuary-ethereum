...
contract Spaceship is ERC721ALowCap, AccessControl, Ownable, ReentrancyGuard {
    ...
    IERC20 private yollar; // yollar used for minting spaceship.
    ...
    bool public publicMintPaused; // toggle

    uint256 public publicMintMaxCount; // setter
    uint256 public publicMintCurrentCount; // settor
    uint256 public publicMintYollarPrice; // setter
    ...
    function mintPublicWithYollar(uint256 redeemAmount)
        external
        whenPublicMintActive
        nonReentrant
    {
        address redeemer = msg.sender;
        if (currentMinted + redeemAmount > mintableSupply) {
            revert NotEnoughRedeemsAvailable();
        }
        if (publicMintCurrentCount + redeemAmount > publicMintMaxCount) {
            revert NotEnoughRedeemsAvailable();
        }
        publicMintCurrentCount += redeemAmount;

        _mintSpaceship(redeemer, redeemAmount);

        // This will automatically revert if it fails
        if (publicMintYollarPrice != 0) {
            yollar.safeTransferFrom(
                msg.sender,
                address(this),
                publicMintYollarPrice * redeemAmount
            );
        }
    }
    ...
    // -------------------------------- admin functions
    function withdrawYollar(uint256 _amount) external onlyOwner nonReentrant {
    ...
    function setPublicMintMaxCount(uint256 mintMaxCount_) external onlyOwner {
        if (mintMaxCount_ > mintableSupply) {
            revert InvalidMaxMintCount();
        }
        publicMintMaxCount = mintMaxCount_;
    }

    function setPublicMintPrice(uint256 yollarPrice_, address yollarAddress_)
        external
        onlyOwner
    {
        publicMintYollarPrice = yollarPrice_;
        yollar = IERC20(yollarAddress_);
    }
    ...
    function togglePublicMintPause() external adminOrWL {
        if (publicMintPaused) {
            emit PublicMintActiveEvent();
        } else {
            emit PublicMintPausedEvent();
        }
