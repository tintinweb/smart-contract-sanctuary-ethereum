...
contract YFIAGNftMarketplace is IYFIAGNftMarketplace, ERC721, IERC721Pausable{
    ...
    function burn(uint256 _tokenId) external override tokenNotFound(_tokenId){
    ...
    function setPriceAndSell(uint256 _tokenId, uint256 _price) public override tokenNotFound(_tokenId) isUnlock(_tokenId) isRootToken(_tokenId){
        require(ownerOf(_tokenId) == msg.sender, "isn`t owner of token");
    ...
    function buy(uint256 _tokenId) public payable override tokenNotFound(_tokenId) isUnlock(_tokenId) isRootToken(_tokenId){
 ...
 * {ERC721Enumerable}.
 */
contract ERC721 is Context, ERC165, IERC721, IERC721Enumerable, IERC721Metadata, ILockable, ERC721Receiver, Ownable {
    using Address for address;
    ...
    modifier isRootToken(uint256 _tokenId) {
        require(!_rootTokens[_tokenId], "Token is root");
        _;
    }
  ...
  function _setTokenURI(uint256 tokenId, string memory uri) internal {
    ...
    function lock(uint256 _tokenId) public override tokenNotFound(_tokenId) isUnlock(_tokenId) {  // onlyOwner()
        require(msg.sender == owner() || isAdmin(msg.sender), "Only admin or owner");
    ...
    function unlock(uint256 _tokenId) public override tokenNotFound(_tokenId) {  // onlyOwner()
        require(msg.sender == owner() || isAdmin(msg.sender), "Only admin or owner");
