...
contract NFTContractERC721A is ERC721A, Ownable {
    ...
    constructor(bytes32 _initRootHash,string memory _initBaseURI, string memory _initPreviewURI) ERC721A("NFT Name", "NFT Symbol"){
    ...
    function whitelistMint (bytes32[] calldata _merkleProof, uint256 _mintAmount) external payable callerIsUser {
        require(contractState, "Smart contract is currently disabled");
        ...
        require(whitelistMintCounter[msg.sender] + _mintAmount <= mintLimit, "Each whitelist is only entitled to 2 mints");
    ...
    function publicMint(uint256 _mintAmount) external payable callerIsUser {
        ...
        require(_mintAmount <= transactionLimit, "Each transactions only allowed up to 2 mints");
        require(totalSupply() + _mintAmount <= maxSupply, "Total Supply Exceeded");

        //Perform checks to see if the user (including whitelisted) has mint before (<2) [during public sale]
        require(publicMintCounter[msg.sender] + _mintAmount <= mintLimit, "Each wallet address is only entitled to 2 mints");

        require(msg.value == cost * _mintAmount, "Insufficient fund in your wallet");
        publicMintCounter[msg.sender] += _mintAmount;
        _safeMint(msg.sender, _mintAmount);
    }
