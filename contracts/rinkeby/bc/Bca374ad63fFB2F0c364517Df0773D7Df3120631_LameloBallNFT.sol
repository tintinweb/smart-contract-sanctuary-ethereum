...
contract LameloBallNFT is ERC721Enumerable, controllerPanel {
    ...
    string public baseURI = "https://client-metadata.ether.cards/api/lamelo2/";
...
*/
    ...
    function buyCardInternal(
        ...
        if (_ec_token_id > 0 && _melo_token_id == 0) {
            ...
            require(
            IERC721(ec_contract_address).ownerOf(_ec_token_id) == msg.sender,
            "!EC"
        );
        } else if (_ec_token_id == 0 && _melo_token_id > 0) {
            _discount = getMeloDiscountPercentage(_melo_token_id);
             require(
            IERC721(lameloContractAddress).ownerOf(_melo_token_id) == msg.sender,
            "!Lamelo"
        );
        }
    ...
    function setDataFolder(string memory _uri,string memory __tokenPreRevealURI) external onlyOwner {
        baseURI = _uri;
