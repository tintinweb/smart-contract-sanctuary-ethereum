...
interface IFrameNFT {
	function initialize(address creator, address _feeAddress) external;
}
...
contract NFTFactory is OwnableUpgradeable {
	...
	function createCollection() external payable returns(address collection) {
        ...
        IFrameNFT(collection).initialize(msg.sender, feeAddress);
