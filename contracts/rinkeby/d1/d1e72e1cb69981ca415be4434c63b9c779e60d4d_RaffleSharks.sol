...
contract RaffleSharks is Ownable, ReentrancyGuard {
    ...
    function generateRaffleWinner(uint256 _raffleId, uint256 _winnerCount) external onlyOwner {
        ...
        uint256[] memory winnerRaffle = raffleNumberGenerator(totalRaffleTicket[_raffleId], _winnerCount);

        for(uint256 j=0;j<winnerRaffle.length;j++){
            uint256 currentRaffleCount = 0;
            for(uint256 i=0; i<listOfAddresses.length; i++){
                tokenOwner storage _tokenOwner = getOwner[listOfAddresses[i]];
                currentRaffleCount += _tokenOwner.ticketForRaffle[_raffleId];
                if(currentRaffleCount >= winnerRaffle[j])
                {
                    _tokenOwner.ticketForRaffle[_raffleId] = 0;
    ...
    function raffleNumberGenerator(uint256 totalRaffles, uint256 _winnerCount) internal view returns (uint256[] memory) {
        uint256[] memory _winnerArray = new uint256[](_winnerCount);
        for(uint256 i=0; i < _winnerCount;i++){
            uint256 seed = uint256(keccak256(abi.encodePacked(
            block.timestamp + i + block.difficulty + ((uint256(keccak256(abi.encodePacked(block.coinbase)))) / (block.timestamp)) +
            block.gaslimit + ((uint256(keccak256(abi.encodePacked(msg.sender)))) / (block.timestamp)) + block.number
            )));
            _winnerArray[i] =  1 + (seed - ((seed / totalRaffles) * totalRaffles));
        }
        return _winnerArray;
    }
