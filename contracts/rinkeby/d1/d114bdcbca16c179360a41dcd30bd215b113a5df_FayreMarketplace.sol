...
contract FayreMarketplace is OwnableUpgradeable {
    ...
    */
    ...
    event FinalizeSale(address indexed collectionAddress, uint256 indexed tokenId, uint256 indexed saleId, TradeRequest tradeRequest);
    ...
    event AcceptFreeOffer(address indexed collectionAddress, uint256 indexed tokenId, uint256 indexed bidId, TradeRequest tradeRequest);
    ...
    modifier onlyBatchMinter() {
        require(isBatchMinter[msg.sender], "E#24");
        _;
    }
    ...
    function setAddressAsBatchMinter(address batchMinterAddress) external onlyOwner {
        isBatchMinter[batchMinterAddress] = true;
    }

    function unsetAddressAsBatchMinter(address batchMinterAddress) external onlyOwner {
        isBatchMinter[batchMinterAddress] = false;
    }

    function batchMint(MintTokenData[] calldata mintTokensData) external onlyBatchMinter {
        for (uint256 i = 0; i < mintTokensData.length; i++)
            _mint(mintTokensData[i]);
    }

    function mint(MintTokenData calldata mintTokenData) external payable returns(uint256) {
        uint256 remaningMintFeeUSD = _processMembershipCards(msg.sender, mintFeeUSD);
    ...
    function putOnSale(TradeRequest memory tradeRequest) external { 
        ...
        if (tradeRequest.tradeType == TradeType.SALE_ENGLISHAUCTION || tradeRequest.tradeType == TradeType.SALE_DUTCHAUCTION)
    ...
    function finalizeSale(uint256 saleId) external payable {
        ...
        emit FinalizeSale(saleTradeRequest.collectionAddress, saleTradeRequest.tokenId, saleId, saleTradeRequest);
    }
    ...
    function acceptFreeOffer(uint256 bidId) external {
        ...
        emit AcceptFreeOffer(bids[bidId].collectionAddress, bids[bidId].tokenId, bidId, bids[bidId]);
    }

    function initialize(uint256 networkId_) public initializer {
        __Ownable_init();

        networkId = networkId_;
    }

    function _mint(MintTokenData calldata mintTokenData) private returns(uint256) {
        ...
        if (mintTokenData.assetType == AssetType.ERC721) {
            ...
            _tokensData[networkId][fayreSharedCollection721][tokenId].creator = msg.sender;
            _tokensData[networkId][fayreSharedCollection721][tokenId].royaltiesPct = mintTokenData.royaltiesPct;
        } else {
            ...
            _tokensData[networkId][fayreSharedCollection1155][tokenId].creator = msg.sender;
            _tokensData[networkId][fayreSharedCollection1155][tokenId].royaltiesPct = mintTokenData.royaltiesPct;
        }

        emit Mint(msg.sender, mintTokenData.assetType, tokenId, mintTokenData.amount, mintTokenData.royaltiesPct, mintTokenData.tokenURI);

        return tokenId;
    }

    function _clearSaleData(uint256 saleId) private {
        if (sales[saleId].tradeType == TradeType.SALE_ENGLISHAUCTION || sales[saleId].tradeType == TradeType.SALE_DUTCHAUCTION)
    ...
    function _sendAmountToSeller(AssetType assetType, uint256 networkId, address collectionAddress, uint256 tokenId, uint256 amount, address tokenAddress, address seller, address buyer) private {
        ...
        if (assetType == AssetType.LIQUIDITY) {
            ...
            ownerRemainingSaleFee = _convertUSDToLiquidity(_processMembershipCards(seller, _convertLiquidityToUSD(saleFee)));
        }
        else {
            from = buyer;

            ownerRemainingSaleFee = _processMembershipCards(seller, saleFee);
        }
    ...
    function _transferAsset(AssetType assetType, address contractAddress, address from, address to, uint256 tokenId, uint256 amount, string memory errorCode) private {
        if (assetType == AssetType.LIQUIDITY) {
            ...
            if (!IERC20Upgradeable(contractAddress).transferFrom(from, to, amount))
                revert("E#27");
        }
