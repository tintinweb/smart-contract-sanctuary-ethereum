...
contract Inerme is ERC721Enumerable, Ownable {
 ...
 constructor(string memory _NotRevealedUri) ERC721("INE", "INM") {
...
function setRootHash(bytes32 updatedRootHash) public onlyOwner {
    ...
    function preSalemint( bytes32[] calldata _merkleProof,uint256 quantity) public payable {
        ...
        require(totalSupply() + quantity <= preSaleSupply , "Quantity is greater than remaining presaleSupply");
 
        require(_preSaleAddresses[msg.sender].counter + quantity <= maxPerWallet, "Can not mint more than maxperWallet");
    ...
    function mint(uint256 chosenAmount) public payable isSaleOpen {
        ...
        require(isPaused == false, "Sale is not active at the moment");
         ...
         require(_preSaleAddresses[msg.sender].counter + chosenAmount <= maxPerWallet, "Sorry can not mint more than maxwallet");
...
//SHOULD WE USE SEparate tokenID variable here ??
   function tokenURI(uint256 tokenId1)
    public
    view
    virtual
    override
    returns (string memory)
  {
    ...
    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI, tokenId1.toString(), ".json"))
        : "";
  }
