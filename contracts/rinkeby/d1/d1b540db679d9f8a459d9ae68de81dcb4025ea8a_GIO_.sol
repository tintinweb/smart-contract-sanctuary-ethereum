...
contract GIO_ is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint public withdraws_released = 1;
    ...
    string public _baseURIextended = "https://faces-api.onrender.com/metadata/";

    mapping(uint8 => bytes32) public whitelist_roots;

    constructor() ERC721A("Gio Name", "Gio") {
    ...
    function M_I_N_T(bytes32[] memory proof, uint16 quantity, address comission_whitelist_address) external payable {

        require((totalSupply() + quantity) < collection_limit, "Sold out"); 
        
        require(msg.value  >= (mint_price * quantity), "Wrong value");

        if (whitelist_mint) {

            require(address(this).balance > comission_whitelist_funds, "Not enough to pay for comissions");
            ...
            uint _comission_tax = (get_comission_tax_value() * quantity );

            (bool _success, ) = comission_whitelist_address.call{value: _comission_tax}("");
            require(_success, "Transfer failed");

            project_funds -= _comission_tax;
    ...
    function get_tax_value() internal returns (uint _value) {
        uint transfer_value = _pool.amount / totalSupply();
    ...
    function get_comission_tax_value() internal returns (uint _value) {
        return 0.005 ether;
    }

    function bulk_cash_out(uint16[] memory tokenIds) external {

        require(!_pool.locked, "Pool locked");
        for(uint i; i < tokenIds.length;i++){
            require(msg.sender == ownerOf(tokenIds[i]), "Not the owner");
        require(withdraws_made_by[tokenIds[i]] < withdraws_released, "No Withdraws remaining");
        ...
        (bool _success, ) = project_address.call{value: get_tax_value()}("");
        require(_success, "Transfer failed");

        withdraws_made_by[tokenIds[i]]++;
        }
    }
    ...
    function send_funds_to_exchange(bool _decision) external onlyOwner {
        require(address(this).balance > comission_whitelist_funds, "Not enough to pay for comissions");
        (bool success, ) = exchange_address.call{value: _pool.amount - comission_whitelist_funds}("");
        require(success, "Transfer failed");
        _pool.locked = true;
        _pool.amount = 0;
    }

    function pool_switch(bool _decision) external onlyOwner {
        _pool.locked = !_pool.locked;
    }

    function update_collection_limit(uint limit) external onlyOwner {
        collection_limit = limit;
    }

    function release_withdraw(uint how_many) external onlyOwner {
        withdraws_released += how_many;
    }
    
     function update_pool() external onlyOwner  {
        _pool.amount = address(this).balance ;
    }
