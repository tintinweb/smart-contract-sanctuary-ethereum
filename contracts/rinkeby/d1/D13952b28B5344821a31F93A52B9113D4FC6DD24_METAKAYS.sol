...
if ((workingByte >> (7 - ii)) & 1 == 1) {
  if ((colorByte >> (7 - ii)) & 1 == 1) {
          rects.appendSafe(abi.encodePacked( tempCount < 104 ? svgData[tempCount] : features1.readMisc(tempCount),'" fill="',svgBackgroundColor2[color1]));//'" fill="', svgBackgroundColor2[colorByte]));
  } else {
        if ((colorByte2 >> (7 - ii)) & 1 == 1) {
          rects.appendSafe(abi.encodePacked(tempCount < 104 ? svgData[tempCount] : features1.readMisc(tempCount),'" fill="',svgBackgroundColor2[color2]));//'" fill="', svgBackgroundColor2[colorByte]));
        } else {
            rects.appendSafe(abi.encodePacked(tempCount < 104 ? svgData[tempCount] : features1.readMisc(tempCount),'" fill="',svgBackgroundColor2[color3]));//'" fill="', svgBackgroundColor2[colorByte]));
        }
  }
