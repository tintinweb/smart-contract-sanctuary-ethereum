...
contract MetazoidSocialClub is ERC721, Ownable {
    ...
    uint public constant PRESALE_MINT_PRICE = 0.01 ether;
    ...
    function redeem(bytes32[] calldata merkleProof, uint count) external payable {
        ...
        require(count <= TOKENS_PER_TRAN_LIMIT, "ZeroCodeNFT: Requested token count exceeds allowance (20)");
        if(saleStatus == SaleStatus.PRESALE) {
            ...
            require(_whitelistMintedCount[msg.sender] + count <= TOKENS_PER_PERSON_WL_LIMIT, "ZeroCodeNFT: Requested token count exceeds allowance (20)");
            ...
            require(_mintedCount[msg.sender] + count <= TOKENS_PER_PERSON_PUB_LIMIT, "ZeroCodeNFT: Requested token count exceeds allowance (20)");
