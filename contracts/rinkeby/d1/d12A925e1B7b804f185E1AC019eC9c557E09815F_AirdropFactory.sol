...
contract Airdrop {
  ...
  constructor(address _owner, IERC20 _token, string memory _data) {
    owner = _owner;
    token = _token;
    data = _data;
  }
...
contract AirdropFactory is Ownable {
  ...
  function create(IERC20 _token, string memory _data) external payable {
    require (msg.value >= fee, "Insufficient fee!");

    address airdrop = address(new Airdrop(msg.sender, _token, _data));
