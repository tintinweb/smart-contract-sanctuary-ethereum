// Squishiland by Squishiverse (www.squishiverse.com)
...
contract Squishiland is ERC721, Ownable {
    ...
    mapping(LandSize => LandAttribute) private _land;

    constructor() ERC721("Squishiland", "SVLAND") {
        _land[LandSize.Mythic] = LandAttribute(0.01 ether, 2494, 1950, 0, 0);
        _land[LandSize.Legendary] = LandAttribute(0.02 ether, 1500, 450, 0, 0);
        _land[LandSize.Epic] = LandAttribute(0.03 ether, 400, 50, 0, 0);
        _land[LandSize.Rare] = LandAttribute(0.04 ether, 50, 0, 0, 0);
    }
    ...
    modifier hasFunding(LandSize _size, uint256 _quantity) {
        require(
            msg.value >= _land[_size].price * _quantity,
            "Insufficent funds"
        );
        _;
    }
    ...
    function _mintMany(
        ...
        uint256 startingId = _land[_size].minted + _land[_size].startingId;
        unchecked {
            _land[_size].minted += _quantity;
        }
    ...
    modifier withinSupplyLimit(LandSize _size, uint256 _quantity) {
        require(
            _land[_size].minted + _quantity <= _land[_size].supply,
            "Surpasses supply"
        );
        _;
    }
    ...
    /**
     * @notice Fetch overall total supply
     */
    function totalSupply() public view returns (uint256) {
        uint256 total;
        for (uint256 s; s < MAX_LAND_SIZES; s++) {
            LandSize size = LandSize(s);
            total += _land[size].minted - _land[size].burnt;
        }
        return total;
    }
    ...
    function burn(uint256 tokenId) public {
        ...
        require(
            _land[size].burnt < _land[size].minted,
            "Squishiland: invalid call to burn"
        );
        unchecked {
            _land[size].burnt++;
        }
    ...
    /**
     * @notice Fetch the overall total minted on a per size basis
     */
    function totalSupplyBySize(LandSize _size) public view returns (uint256) {
        return _land[_size].minted - _land[_size].burnt;
    }

    /**
     * @notice Returns all the attributes of a given land plot
     * @dev Untidy since structs don't look nice on etherscan and public mapping
     *      handled invalid sizes weirdly
     */
    function attributesOfLandSize(LandSize _size)
        public
        view
        returns (
            uint256 price,
    ...
    {
        LandAttribute memory attrs = _land[_size];
        return (
            attrs.price,
            attrs.supply,
            attrs.startingId,
    ...
    function getLandSize(uint256 _tokenId) public view returns (LandSize) {
        for (uint256 s; s < MAX_LAND_SIZES; s++) {
            LandSize size = LandSize(s);
            uint256 start = _land[size].startingId;
            if (_tokenId >= start && _tokenId <= start + _land[size].minted) {
                return size;
            }
        }
    ...
    address private constant address1 =
        0x0D5A20570bbD271C2B1684da0556cEa471BE7C4e;
