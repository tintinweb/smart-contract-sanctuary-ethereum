...
// File: contracts/mb-lootbox.sol

pragma solidity ^0.8.0; // 8.4
...
contract MBLOOTBOX is ERC1155, AccessControl, Pausable, ERC1155Burnable, ERC1155Supply {
    ...
    Box public BASIC = Box(100, Level.BASIC, "BASIC", 300, 0.5 ether);
    Box public BASICPLUS = Box(200, Level.BASICPLUS, "BASIC PLUS", 150, 1 ether);
    Box public BRONZE = Box(300, Level.BRONZE, "BRONZE", 50, 2.5 ether);
    Box public SILVER = Box(400, Level.SILVER, "SILVER", 25, 5 ether);
    Box public GOLD = Box(500, Level.GOLD, "GOLD", 10, 10 ether);
    Box public PLATINUM = Box(600, Level.PLATINUM, "PLATINUM", 5, 20 ether);
    Box public EMERALD = Box(700, Level.EMERALD, "EMERALD", 3, 40 ether);
    Box public DIAMOND = Box(800, Level.DIAMOND, "DIAMOND", 1, 80 ether);
...
constructor() 
  ...
  */
  function getBoxesBoughtByOwner(address wallet) external view returns(Box[] memory) {
    ...
    function decrementBoxSupplyById(uint256 boxId) external {
      if(boxes[boxId].supply > 0) {
        boxes[boxId].supply = boxes[boxId].supply - 1;
      }
    }
