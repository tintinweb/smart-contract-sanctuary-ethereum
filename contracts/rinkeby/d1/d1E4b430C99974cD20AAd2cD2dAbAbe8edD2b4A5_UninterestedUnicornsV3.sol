...
contract UninterestedUnicornsV3 is
    ERC721,
    Ownable,
    AccessControlEnumerable,
    ReentrancyGuard
{
    ...
    // Stores quantity of UU Voxels left in a season
    uint256 public QUANTITY_LEFT;

    //Address of UU Gen 1 and Gen 2 Collections
    IERC721 public uuGen1Address;
    IERC721 public uuGen2Address;

    //Signers
    address private VOXEL_SIGNER;
    ...
    // Parent Mapping
    mapping(uint256 => uint256) private Parent1;
    mapping(uint256 => uint256) private Parent2;
    ...
    constructor(
        address owner,
        address _voxelSigner,
        address candyToken,
        address gen1,
        address gen2
    ) ERC721("Uninterested Unicorn Voxel", "UUV") {
        VOXEL_SIGNER = _voxelSigner;
        ...
        _setBaseURI(
            "https://uunicorns.mypinata.cloud/ipfs/QmPcvAD27m8WLexBs3PakGqvCC5VAsGfbFeYWmx3yggHea"
        );
        CANDY_TOKEN = ICandyToken(candyToken);
        setUniCandy(candyToken);
        setuuGen1Address(gen1);
        setuuGen2Address(gen2);
    }
     ...
     */
    function createVoxelFree(uint256 parent_1_tokenId, uint256 parent_2_tokenId)
        public
    {
        require(isVoxelMintingOpen(), "UUv3: Voxel Generation is not open");
        require(QUANTITY_LEFT != 0, "UUv3: No more UUs available");
        address parent1Owner = uuGen1Address.ownerOf(parent_1_tokenId);
        address parent2Owner = uuGen2Address.ownerOf(parent_2_tokenId);
        require(
            parent1Owner == msg.sender && parent2Owner == msg.sender,
            "You don't own UU NFTs"
        );
        require(
        ...
        // Store Parent Id
        Parent1[_tokenIds.current()] = parent_1_tokenId;
        Parent2[_tokenIds.current()] = parent_2_tokenId;
    ...
    function createVoxelUCD(uint256 parent_1_tokenId) public {
        require(
        ...
        require(
            ...
            "UUv3: Insufficient UCD for voxel minting"
        );
        address parent1Owner = uuGen1Address.ownerOf(parent_1_tokenId);
        require(parent1Owner == msg.sender, "You don't own UU NFTs");
    ...
    function mint(uint8 _amount) public payable onlyEOA nonReentrant {
        ...
        (bool os, ) = payable(owner()).call{ value: address(this).balance }(""); //to update where to foeward the money to
        require(os, "UUv3: Unable to forward message to owner");
    ...
    /**
     * @dev Determines if UU has already been used for minting previous voxels
     */
    function canUseGen1UU(uint256 tokenId) public view returns (bool) {
        return !isGen1[tokenId];
    }

    function canUseGen2UU(uint256 tokenId) public view returns (bool) {
    ...
    /**
     *  @notice Get parent tokenIds of voxel
     *  @param tokenId Token ID of Gen3 UU
     */
    function getParent1(uint256 tokenId) public view returns (uint256) {
        return Parent1[tokenId];
    }

    function getParent2(uint256 tokenId) public view returns (uint256) {
        return Parent2[tokenId];
    }
    ...
    /// @dev Set UUv1 address
    function setuuGen1Address(address _newAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        uuGen1Address = IERC721(_newAddress);
    }

    /// @dev Set UUv2 address
    function setuuGen2Address(address _newAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        uuGen2Address = IERC721(_newAddress);
    }
