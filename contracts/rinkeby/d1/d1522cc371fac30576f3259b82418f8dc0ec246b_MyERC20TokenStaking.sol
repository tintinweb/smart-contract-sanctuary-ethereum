...
contract MyERC20TokenStaking is Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    IERC20 public constant MyERC20Token =
    ...
    function remainingRewards() external view returns (uint256) {
        return MyERC20Token.balanceOf(address(this)) - totalTokensInPools;
    }
    ...
    function compound(uint256 _pid) external ignoreVesting {
        PoolInfo storage pool = poolInfo[_pid];
        require(pool.stakingToken == MyERC20Token, "Can compound only MyERC20Token");
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.stakingToken) == address(MyERC20Token)) {
                totalTokensInPools += _amount;
            }

            pool.stakingToken.approve(address(this), 1000);
    ...
    function safeTokenTransfer(
        ...
        uint256 balance = MyERC20Token.balanceOf(address(this));
        if (balance > totalTokensInPools) {
            ...
            if (_amount >= rewardBalance) {
                ...
                MyERC20Token.transfer(_to, rewardBalance);
            } else if (_amount > 0) {
                failedRewardsCache[_pid][_to] = 0;
                MyERC20Token.transfer(_to, _amount);
            }
        } else {
    ...
    function emergencyRewardWithdraw() external onlyOwner {
        uint256 rewardAmount = MyERC20Token.balanceOf(address(this)) -
            totalTokensInPools;
        MyERC20Token.transfer(msg.sender, rewardAmount);
    ...
    uint256 balance = MyERC20Token.balanceOf(user);
    return balance;
    }
}