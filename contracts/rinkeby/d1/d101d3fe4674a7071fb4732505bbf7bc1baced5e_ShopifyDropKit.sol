...
contract ShopifyDropKit is AdminControl, ICreatorExtensionTokenURI {
    ...
    // Untested
    function mint(uint256 quantity, address buyer) public shopOnly {
        if (quantity > 1) {
            for (uint256 i = 0; i < quantity; i++) {
                IERC721CreatorCore(_creator).mintExtension(buyer);
            }
        }
        IERC721CreatorCore(_creator).mintExtension(buyer);
    }
