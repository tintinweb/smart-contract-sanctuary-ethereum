	...
	ReentrancyGuard
{
	...
	uint private constant MINT_PRICE = 0.03 ether;
	...
	// mapping variable to mark as claimed
	...
	constructor(
		...
		ERC721("GrittyCats", "GCAT")
		VRFConsumerBase(
			0xb3dCcb4Cf7a26f6cf6B120Cf5A73875B7BBc655B, // Mainnet: 0xf0d54349aDdcf704F77AE15b96510dEA15cb7952, // VRF Coordinator
			0x01BE23585060835E02B77ef475b0Cc51aA1e0709 // Mainnet: 0x514910771AF9Ca656af840dff83E8264EcF986CA // LINK Token
		)
	{
		royaltyAddress = _royaltyAddress;
		signerAddress = _signerAddress;

		GrittyPass = IERC1155(_grittyPass);
		...
		_theBaseURI = _baseURI;
	}

	function mintPassPurchase(address _collectionAddress, uint256 tokensToMint)
		public
		nonReentrant
	{
		require(
			_collectionAddress == address(GrittyPass),
		...
		require(
			...
			"You can mint max 9 tokens per transaction"
		);
		require(
			totalSupply().add(tokensToMint) <= MAX_SUPPLY,
			"Mint more gritty cats that allowed"
		);
	...
	function signatureClaim(
		...
		require(
			totalSupply().add(tokensToMint) <= MAX_SUPPLY,
			"Minting more Gritty Cats than allowed"
		);
	...
	function setBaseURI(string memory newBaseURI) public onlyOwner {
		...
		string memory currentURI = _theBaseURI;
		...
		returns (string memory)
	{
		...
		if (!metadataRevealed) return _theBaseURI;
		return string(abi.encodePacked(_theBaseURI, tokenId.toString()));
	}
...
/// @param msg more details.
error OutOfBounds(string msg);
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
		...
		returns (uint256)
	{
		if (index < _owners.length)
			revert OutOfBounds("ERC721Enumerable: global index out of bounds");
		...
		returns (uint256 tokenId)
	{
		if (index < balanceOf(owner))
			revert OutOfBounds("ERC721Enumerable: owner index out of bounds");
