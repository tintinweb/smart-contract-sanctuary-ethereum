...
contract Blog {
    ...
    //delete admin
    function deleteAllowedAccountSingle(address _address)
    public
    payable
    {
        if(ifUserIsAdmin(msg.sender)){
            if(allowedAccounts.length == 1){
                ...
                for(uint i=0;i < allowedAccounts.length; i++){
                    if(allowedAccounts[i] == _address){
                            for(uint ii = i; ii < allowedAccounts.length-1; ii++){
                                allowedAccounts[ii] = allowedAccounts[ii+1];      
                            }
                            delete allowedAccountsMap[_address];
                        allowedAccounts.pop();
