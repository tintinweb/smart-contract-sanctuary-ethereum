...
contract PaydeceEscrow is  ReentrancyGuard  {
   ...
   function getState(uint _orderId) public view returns (EscrowStatus){
    ...
    function getFeesAvailable() public virtual view onlyOwner returns (uint256){
        return feesAvailable;
    }

    /* This is called by the server / contract owner */
    function createEscrow(uint _orderId, address payable _buyer, address payable _seller, uint256 _value) external virtual nonReentrant {
        require(escrows[_orderId].status == EscrowStatus.Unknown, "Escrow already exists");
        
        //TODO: validar la cantidad que envia el comprador, deberia ser el valor del asset + el buyerFee

        //Obtiene el monto a transferir desde el comprador al contrato        
        uint256 _amountFeeBuyer = _value  * (feeBuyer * 10**decimals) / (100 * 10**decimals);                                

        //Valida el Allowance
        require( tokenccy.allowance( _buyer, address(this)) >= (_value+_amountFeeBuyer),"Seller approve to Escrow first");                      

        //Transfer USDT to contract
        tokenccy.safeTransferFrom(_buyer, address(this) , (_value+_amountFeeBuyer) );
                
        escrows[_orderId] = Escrow(_buyer, _seller, _value, feeSeller, feeBuyer, EscrowStatus.Funded);

        emit EscrowDeposit(_orderId, escrows[_orderId]);
    }

    function releaseEscrowOwner(uint _orderId) external onlyOwner nonReentrant{
        _releaseEscrow(_orderId);         
    }

    /* This is called by the buyer wallet */
    function releaseEscrow(uint _orderId) external onlyBuyer(_orderId) nonReentrant{
        _releaseEscrow(_orderId);         
    }

    function _releaseEscrow (uint _orderId) private onlyBuyer(_orderId) nonReentrant{
        require(escrows[_orderId].status == EscrowStatus.Funded,"USDT has not been deposited");             
        
        //Obtiene el monto a transferir desde el comprador al contrato        
        uint256 _amountFeeBuyer = escrows[_orderId].value  * (feeBuyer * 10**decimals) / (100 * 10**decimals);
        uint256 _amountFeeSeller = escrows[_orderId].value  * (feeSeller * 10**decimals) / (100 * 10**decimals);  
        ...
        tokenccy.safeTransfer( escrows[_orderId].seller, escrows[_orderId].value - _amountFeeSeller);   
    ...
    */
    ...
    /* Cancela el Escrow y devuelve las stable al COMPRADOR */
    function cancelEscrow(uint _orderId) external nonReentrant onlyOwner{
