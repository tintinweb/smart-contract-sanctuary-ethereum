...
contract PoolManager is OwnableUpgradeable, IPoolManager {
    ...
    function getTopPool() public view returns (CumulativeLockInfo[] memory) {
        uint256 length = _pools.length();
        CumulativeLockInfo[] memory poolInfo = new CumulativeLockInfo[](length);
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if(saleType[0] == 0 && info[1] >= block.timestamp){
                poolInfo[currentIndex] = CumulativeLockInfo(
                    _pools.at(i),
                    token,
                    saleType[0],
                    saleType[1],
        ...
        //         if(poolInfo[j].totalRaised > poolInfo[i].totalRaised ){
        //             (poolInfo[j], poolInfo[j+1]) = (poolInfo[j+1], poolInfo[j]);
        //         }
        //     }
        // }
