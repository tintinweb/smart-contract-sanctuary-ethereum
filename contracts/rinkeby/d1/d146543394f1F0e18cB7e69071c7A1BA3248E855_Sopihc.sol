...
contract Sopihc is ERC721A, Ownable, ReentrancyGuard {
    ...
    {
        // Verify whitelist requirements
        require(ogListMintEnabled, "The whitelist sale is not enabled!");
        require(!ogListClaimed[_msgSender()], "Address already claimed!");
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(
            MerkleProof.verify(_merkleProof, ogListmerkleRoot, leaf),
            "Invalid proof!"
        );
    ...
    function withdraw() public onlyOwner nonReentrant {
        //autogiveaddress from inital sale
