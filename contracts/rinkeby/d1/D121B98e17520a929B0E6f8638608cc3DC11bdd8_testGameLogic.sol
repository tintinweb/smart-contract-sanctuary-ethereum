...
contract testGameLogic is Pausable, Ownable, IERC721Receiver {
    ...
    function initiateGameAt (uint[] memory tokenIds) external {
        for (uint i ; i<tokenIds.length;i++) {
            uint index = RandomnessEngine.getCurrentIndex();
            randomIndex[tokenIds[i]] = initGame(index,index,index);
            initiateGame[tokenIds[i]] = true;
        }
    }
    ...
    function _claimYetiFromPalace(uint tokenId, bool unstake) public returns (uint owedFrxst) {
        ...
        if (unstake) {
            if (stake.activityId == 0) {
                ...
                uint rand = RandomnessEngine.initiateRandomness(tokenId,init.gathering);
                require (rand > 0 , "Randomness not received");
                if (rand % 100 < GATHERING_TAX_RISK_PERCENTAGE)
                {
                    init.gathering = RandomnessEngine.getCurrentIndex();
            ...
            // Check Injury
                ...
                initGame storage init = randomIndex[tokenId];
                uint rand = RandomnessEngine.initiateRandomness(tokenId,init.hunt);
                require (rand>0,"Randomness not received");

            if (rand % 100 < chanceOfInjury) {
                init.hunt = RandomnessEngine.getCurrentIndex();
    ...
    function _claimYetiFromFighting(uint tokenId, bool unstake) internal returns (uint owedFrxst) {
      ...
      //  uint c = rs.GetRarity(tokenId);
        uint hourly = rates[stake.activityId][rs.GetRarity(tokenId)] * levels[tokenId] * (levels[tokenId]) * 100;
        ...
        if (unstake) {
            initGame storage init = randomIndex[tokenId];
            uint rand = RandomnessEngine.initiateRandomness(tokenId,init.fight);
            require (rand > 0,"Randomness not received");
            yeti.safeTransferFrom(address(this), _msgSender(), tokenId, ""); // Send back Yeti
           ...
           // uint chanceOfStolen = FIGHTING_STOLEN_RISK_PERCENTAGE - levels[tokenId];
            if (rand % 100 < FIGHTING_STOLEN_RISK_PERCENTAGE - levels[tokenId]) {
                init.fight = RandomnessEngine.getCurrentIndex();
