...
contract TestRewarder is IRewarder, IContract {
    ...
    bytes32 public constant override TEMPLATE_ID = keccak256("TEST_REWARDER");
    ...
    // PW: this needs to be change to a mapping or arry for tokenID
    ...
    function addPoolContract(address _poolAddress, uint256 _poolPoints) external override {
        require(
            ...
            "Rewards.addPoolTemplate: Sender must be admin"   
        );
        require(_poolAddress != address(0));//dev: poolAddress must be a non zero address
        require(poolData[_poolAddress].lastRewardTime == 0, "Rewards.addPoolTemplate: Template already Exists");
        tokenPools.push(_poolAddress);
        setPoolPoints(_poolAddress,_poolPoints);
        emit TokenPoolAdded(_poolAddress); 
    }
    ...
    function addRewardsToPool(
        ...
        // /// PW: Need to think about the starting and stopping of rewards for different tokens at different times.
        ...
        // /// PW: Need to think about the pool points if already exists
        ...
        returns(bool)
    {
        ...
        if (m_net == 0) {
            ...
            for (uint256 i = 0; i < poolRewardTokens[_poolAddress].length; i++) {
                /// PW: pool rewards already loops, can be refactored
    ...
    function poolRewards(address _poolAddress, address _rewardToken, uint256 _from, uint256 _to) public override view returns (uint256 rewards) {
        ...
        /// PW: Check that the maths doesnt leave dust
    ...
    function totalRewards() external override view returns (address[] memory, uint256[] memory) {
        uint rewardCount = rewardTokenArray.length;
        address[] memory rTokens = new address[](rewardCount);
        uint256[] memory rewards = new uint[](rewardCount);

        for(uint8 i = 0; i < rewardCount; i++) {  
            rTokens[i] = rewardTokenArray[i];
            rewards[i] = totalRewards(rewardTokenArray[i]);
    ...
    function _updateWeights()  internal {
        // PW: This is blank for non dynamic pools
    }
        ...
        external
    {
        ...
        // PW: Maybe you should be able to recove the rewards as this is the rewarder
        require(
            accessControls.hasAdminRole(msg.sender),
    ...
    function initContract(
        ...
        address _accessControls = abi.decode(_data, (address));
        initRewarder(_accessControls);
    }
   ...
   */
    ...
    {
        return abi.encode(_accessControls);
    }
...
interface IJellyPool {
    ...
    // function stakedTokenTotal() external view returns(uint256);
    // function stakedBalance(address _tokenId) external view returns(uint256);
