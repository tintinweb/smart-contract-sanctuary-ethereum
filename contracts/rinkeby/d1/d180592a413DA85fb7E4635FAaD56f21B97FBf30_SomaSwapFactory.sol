...
contract SomaSwapFactory is ISomaSwapFactory, Accessible {
    ...
    // It becomes locked to a templateFactory for gas optimizations
    ...
    constructor(address somaAddress, bytes32 template, uint256 templateVersion)
    SomaContract(somaAddress) {
        TEMPLATE = template;
        TEMPLATE_VERSION = templateVersion;
        DEPLOYER = address(SOMA.templateFactory());

        ITemplateFactory.Version memory pair = ITemplateFactory(DEPLOYER).version(TEMPLATE, TEMPLATE_VERSION);

        require(pair.exists, 'SomaSwapFactory: INVALID_TEMPLATE_VERSION');
        require(!pair.deprecated, 'SomaSwapFactory: TEMPLATE_DEPRECATED');
    ...
    function createPair(address tokenA, address tokenB) external whenNotPaused onlyRole(CREATE_PAIR_ROLE) override returns (address pair) {
        ...
        pair = ITemplateFactory(DEPLOYER).deployTemplate(TEMPLATE, TEMPLATE_VERSION, args, salt);
...
abstract contract GuardableUpgradeable is IGuardable, AccessibleUpgradeable {
    ...
    function __Guardable__init_unchained() internal onlyInitializing {
        ...
        _updateRequiredPrivileges(GuardHelper.DEFAULT_REQUIRED);
    }
    ...
    function updateRequiredPrivileges(bytes32 newRequiredPrivileges) public virtual override returns (bool) {
        require(
