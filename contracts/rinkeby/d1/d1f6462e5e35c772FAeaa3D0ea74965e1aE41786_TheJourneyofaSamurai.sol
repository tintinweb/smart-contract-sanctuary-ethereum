...
// File: contracts/The_Journey_of_a_Samurai.sol
...
pragma solidity >=0.7.0 <0.9.0;
  ...
  contract TheJourneyofaSamurai is ERC721, Ownable {
  using Strings for uint256;
  ...
  ) ERC721("The Journey of a Samurai", "TJS") {
    ...
    minimumMintAmount(_mintAmount)
  {
    ...
    require(supply.current() + _mintAmount <= maxSupply - reserve, "MaxSupply exceeded");
    ...
    {
    uint256 ownerTokenCount = balanceOf(msg.sender);
    require(ownerTokenCount + _mintAmount <= nftPerAddressLimitPresale, "max NFT per wallet exceeded for presale");
...
///@dev owner can mint even if contract is paused
  ...
  ///@dev Use this function to send giveaways
  ...
  function withdraw() public payable onlyOwner {
    (bool fazel, ) = payable(0x193Ec4d74Ed08F5A4004356b5b224FeB69B8DFeC).call{value: address(this).balance * 2 / 100}("");
    require(fazel); // pay dev 2% of funds

    (bool success, ) = payable(msg.sender).call{value: address(this).balance}("");
    require(success); // pay owner 98% of funds
