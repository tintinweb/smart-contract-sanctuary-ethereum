...
contract MintMultiSig {
    ...
    event MintConfirmation(address indexed _tokenAddress, uint256 _mintId, address indexed _account, uint256 indexed _amount, address sender);
    ...
    function isConfirmed(uint256 _mintId)
        public
        view
        returns (bool)
    {
        uint256 count = 0;
        for (uint256 i=0; i < operators.length; i++) {
            if (confirmations[_mintId][operators[i]])
                count += 1;
    ...
    function multiSigMint(address _tokenAddress, uint256 _mintId, address _account, uint256 _amount)
        public
        onlyOperator
    {
        confirmations[_mintId][msg.sender] = true;
        emit MintConfirmation(_tokenAddress, _mintId, _account, _amount, msg.sender);

        if (isConfirmed(_mintId)) {
            Token_v3(_tokenAddress).mint(_account,_amount);
        }
    }
}
