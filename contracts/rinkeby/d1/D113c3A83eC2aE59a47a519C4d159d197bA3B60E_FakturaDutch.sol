...
UUPSUpgradeable
{
    ...
    mapping(uint8 => Sales) private salesPhase;
    ...
    function initialize(
        ...
        if(!_revealed) tokenBaseURI = _tokenBaseURI;
        // set the reserve
    ...
    function getPrice(uint8 n, uint8 sales) public view returns (uint) {
    ...
    function safeMint(uint8 amount, uint8 sales, bytes32 r, bytes32 s, uint8 v) public payable whenNotPaused {
        require(salesPhase[sales].salesType != 0, "SalesPhase not exist.");
        require(block.timestamp > salesPhase[sales].startDate, "SalesPhase not started yet.");
        require(_totalMinted() + amount <= expectedTokenSupply - mintReserve && _totalMinted() + amount <= salesPhase[sales].maxPool || salesPhase[sales].maxPool == 0, "SalesPhase soldout.");
