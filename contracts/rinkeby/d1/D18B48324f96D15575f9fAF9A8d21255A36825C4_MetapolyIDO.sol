...
contract MetapolyIDO is Ownable, ERC721A, BaseRelayRecipient {
    ...
    // Rinkeby
    IERC20 constant USDT = IERC20(0x057305Eb5B8bb21eFdb7AfaCB383CB366514A999);
    IERC20 constant USDC = IERC20(0xDf5324ebe6F6b852Ff5cBf73627eE137e9075276);
    ...
    constructor(
        ...
        uint _maxSupply, string memory baseURI_, address _signer, address _biconomy
    ) ERC721A(name, symbol) {
    ...
    function mintWithETH(bytes calldata signature) external payable {
        require(msg.value == mintPriceInETH, "Invalid price in ETH");
        _mint(signature);
    }

    function mintWithUSD(bytes calldata signature, IERC20 token) external {
        require(token == USDT || token == USDC, "Invalid token");
        token.safeTransferFrom(msg.sender, address(this), mintPriceInUSD);
        _mint(signature);
    }

    function _mint(bytes calldata signature) private {
    ...
    function getTokenId(address _owner) external view returns (uint) {
