...
contract GRInsurance is Ownable {
    ...
    function addInsurance(uint _id, string memory _currencyName, uint _oneCoinPrice, uint _buyEndDateAt, uint _endDateAt) external onlyOwner {
    ...
    function getInsuranceById(uint _id) public view returns (uint id, string memory currencyName, uint oneCoinPrice, uint buyEndDateAt, uint endDateAt) {
