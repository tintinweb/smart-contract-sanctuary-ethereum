...
contract BabyWizards is ERC721ABurnable, Ownable, ReentrancyGuard {
  ...
  modifier validateAdoption(uint256 adoptCount, uint256 tokenId) {
    require(
 ...
 function generalAdoption(uint adoptCount, uint256 tokenId) validateAdoption(adoptCount, tokenId)  nonReentrant external payable {
     require(currentMode == ReleaseMode.OPEN, "It's not go time yet.");
        ...
        try IERC721(Big_Wizards_Addr).ownerOf(tokenId) returns (
            ...
            if (wizardOwner == msg.sender) {
                     mint(msg.sender, adoptCount, tokenId, true);
            }
 ...
 function generalFreeAdoption(bytes32[] memory _merkleProof, uint adoptCount, uint256 tokenId) validateAdoption(adoptCount, tokenId) nonReentrant external payable {
     require(currentMode == ReleaseMode.OPEN, "It's not go time yet.");
...
///@dev change openSeaProxyRegistryAddress in case issues with migration.
 ...
 function mint(address to, uint256 count, uint256 tokenId, bool countTowardsWalletLimit) private {
     if(countTowardsWalletLimit){
       _mintedPerAddress[to] += count;
     }
     _safeMint(to, count, tokenId);
 }
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function _safeMint(address to, uint256 quantity, uint256 tokenId) internal {
        _safeMint(to, quantity, tokenId, '');
    }
    ...
    function _safeMint(
        ...
        _mint(to, quantity, tokenId, _data, true);
    }
