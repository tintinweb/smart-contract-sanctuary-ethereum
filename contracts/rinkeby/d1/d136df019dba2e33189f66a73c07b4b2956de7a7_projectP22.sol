...
contract projectP22 is ERC1155, Ownable {
    ...
    address public revenueSharingContractAddress = 0x81630eF592fa958aeB03f07323D9AcB62F41281d;
    ...
    function unclaimTokenShare(uint256 tokenId) external view returns(uint256) {
        return ((totalEarning * 6) / (10 * tokenRevenueSharingMembers))  - perTokenClaimedAmount[tokenId];
    }

    function unclaimOwnerShare() external view returns(uint256) {
        return ((totalEarning * 3) / 10)  - ownerClaimedAmount;
    }

    function unclaimCreatorShare() external view returns(uint256) {
        return (totalEarning / 10)  - creatorClaimedAmount;
    }

    function claimTokenShare(uint256 tokenId) external {
    ...
    function claimAllOwnsTokensShare() external {
