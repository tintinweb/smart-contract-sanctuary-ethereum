...
pragma solidity ^0.8.1;
 ...
 contract Mardy is  ERC721PresetMinterPauserAutoId, Ownable  {
    ...
    constructor() ERC721PresetMinterPauserAutoId("Mardy", "MRD","ipfs://metadata")
    {
        baseURI = "ipfs://metadata/";
        baseURIHidden = "ipfs://metadata/hidden";
    }

    function mint(uint256 _count, address to) payable public {
       require(totalSupply().add(_count) <= MaxSupply, "MRD Not enough tokens left.");
       require(SaleisActive, "MRD Public sale is not active.");
       require(_count <= MaxPurchase, "MRD 10 transactions max");
       require(mintPrice * _count <= msg.value, "MRD Ether value sent is not correct");
    ...
    function withdraw(address payable receiver, uint amount) payable external  returns(bool success) {
        require(hasRole(ADMIN_ROLE, _msgSender()), "MRD must have admin role to withdraw");
    ...
    function adminMinting(uint56 _count, address to) public {
        require(hasRole(ADMIN_ROLE, _msgSender()), "MRD must have admin role to mint");
        require(totalSupply().add(_count) <= MaxSupply, "MRD Not enough tokens left.");
        super.mint(to);
    }

    function setNewbaseURI(string memory newBaseURI) public  returns (string memory) {
        require(hasRole(ADMIN_ROLE, _msgSender()), "MRD must have admin role to update base URI");
        baseURI = newBaseURI;
        return baseURI;
    }
      function setNewbaseHiddenURI(string memory newBaseURI) public  returns (string memory) {
        require(hasRole(ADMIN_ROLE, _msgSender()), "MRD AD to update base URI");
    ...
    function flipSaleStatus() external {
        require(hasRole(ADMIN_ROLE, _msgSender()), "MRD AD to flip the status");
