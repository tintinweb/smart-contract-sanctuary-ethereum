...
contract VendingMachineFactory {
    ...
    VendingMachineToken public tokenFactory;
    ...
    require(factoryOwner==sender,"Only for Vending Owner");
    _;
    }

    modifier onlyTokenOwner (address sender){
    require(checkIfTokenHolder(sender)==true, "Caller does not own any tokens.");
    _;
    }

    modifier onlyMachineOwner(uint256 tokenId, address sender){
    require(checkTokenOwnerById(tokenId)== sender,"Only For Contract Owner.");
    _;
    }

    constructor(){
        currentNFTPrice = 10000000000000000;
        factoryOwner = payable(msg.sender); 
        tokenFactory = new VendingMachineToken(msg.sender, address(this));
        tokenCount =0;
    }

    function setNFTPrice(uint256 price) public onlyFactoryOwner(msg.sender){
        currentNFTPrice = price;
    }

    function mintAgreement () public payable {
        require(msg.value>=currentNFTPrice,"Not Enough Eth to purchase NFT.");
        (bool sent,) = factoryOwner.call{value:msg.value}("");
        require(sent, "Failed to purchase NFT. Send Eth to buy.");

        tokenFactory.mintNFT(msg.sender);
        tokenCount++;
    ...
    function addressToTokenID(address toSearch) public view onlyTokenOwner(msg.sender) returns (uint256 [] memory){
...
contract VendingMachineToken is ERC721URIStorage, Ownable {
    ...
    constructor(address owner, address fAddress) ERC721("Vending Machine", "VEND") {
    ...
    require(factoryAddress==msg.sender,"Only from Factory Address");
    _;
    }
...
contract VendingMachine{
    ...
    VendingMachineToken public factoryReference; 
    uint256 public tokenId;

    event Deposit(address payee, uint256 value, uint256 time, uint256 currentContractBalance);
    event Withdraw(uint256 time, uint256 amount, address owner);
    ...
    modifier onlyOwner(){
        require(msg.sender == factoryReference.ownerOf(tokenId), "function is only for owner");
        _;
    }
    ...
    function ownerWithdraw() public onlyOwner() {
        ...
        address owner = factoryReference.ownerOf(tokenId);
