...
import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.4.0/contracts/token/ERC1155/extensions/ERC1155Burnable.sol";
...
contract Airdrops is ERC1155Supply, ERC1155Burnable, Mintable {
    ...
    */
    function mint(uint256 token, uint[] calldata tokenIds) public{
        require(saleIsActive[token],"Sale NOT active");
        uint length = tokenIds.length;
        require(length != 0, "numberOfNfts cannot be 0");
        uint amount;
        for (uint i=0; i < length;) {
            if (!tokenUsed[token][tokenIds[i]]){ // TODO ADD OWNERSHIP CHECK
