...
contract MetaQuestHeroes is ERC721A, Ownable, PaymentSplitter {
    ...
    uint256 public startPublicSaleDate;
    uint256 public startFreeMintingDate;
    address private presaleSigner;
    uint256 public constant MAX_SUPPLY = 3333;
    uint256 public constant MAX_PAID_SUPPLY = 3000;
    uint256 public constant MAX_PURCHASE_COUNT = 2;
    uint256 public constant MAX_PURCHASES_PER_WALLET = 4;
    uint256 public constant MINT_PRICE = 0.125 ether;
    string public constant HASH_PREFIX = "METAQUESTHEROES";
    ...
    constructor(
        ...
        ERC721A("MetaQuestHeroes", "METAQUESTHEROES")
    ...
    /// @notice Reveals token metadata with a given base URI
    /// @param _newBaseURI The base URI for locating the metadata
    ...
    /// @param _startPublicSaleDate The new public Start Date
    function setStartPublicSaleDate(uint256 _startPublicSaleDate)
        external
        onlyOwner
    {
    ...
    /// @notice Owner can update the free mint sale date
    /// @param _startFreeMintingDate The new free minting Start Date
    function setStartFreeMintingDate(uint256 _startFreeMintingDate)
        external
        onlyOwner
    {
        startFreeMintingDate = _startFreeMintingDate;
    }

    // ----------------- Public -----------------

    /// @notice Helper to determine if the presale is active
    /// @return freeMintingActive true if active
    function freeMintingActive() public view returns (bool) {
        return startFreeMintingDate <= block.timestamp;
    }
    ...
    function saleActive() public view returns (bool) {
        return startPublicSaleDate <= block.timestamp;
    }

    /// @notice Return our custom baseTokenURI
    /// @return baseTokenURI the current URI
    function _baseURI() internal view virtual override returns (string memory) {
        return baseTokenURI;
    }

    /// @notice This function will claim free `numberOfTokens` NFT's
    /// @param numberOfTokens The number of tokens to claim in this transaction
    /// @param pass Signed ticket granting permission for the mint
    /// @param allotment The total number of tokens allowable on the ticket
    function freeClaim(
        uint256 numberOfTokens,
        bytes memory pass,
        uint256 allotment
    ) public {
        if (!freeMintingActive()) revert FreeClaimNotStarted();
        uint256 remainingAllotment = validateTicket(
            pass,
            allotment,
            TicketType.FREE_CLAIM
        );
        if (numberOfTokens > remainingAllotment) revert ExceedsAllotment();
        useTicket(pass, numberOfTokens);
        mintHeroes(numberOfTokens);
    }

    /// @notice This function will purchase `numberOfTokens` NFT's on presale
    /// @param numberOfTokens The number of tokens to claim in this transaction
    /// @param pass Signed ticket granting permission for the mint
    /// @param allotment The total number of tokens allowable on the ticket
    function presaleMint(
        ...
        if (!presaleActive()) revert PresaleNotStarted();
        if (_totalMinted() + numberOfTokens > MAX_PAID_SUPPLY) revert SoldOut();
        if (balanceOf(msg.sender) + numberOfTokens > MAX_PURCHASES_PER_WALLET)
    ...
    function mintHeroes(uint256 numberOfTokens) internal {
        if (numberOfTokens > MAX_PURCHASE_COUNT) revert MintingTooMany();
        if (_totalMinted() + numberOfTokens > MAX_SUPPLY) revert SoldOut();
    ...
    function getHash(uint256 allotment, TicketType ticketType)
        internal
        view
        returns (bytes32)
    {
    ...
    function validateTicket(
        ...
        bytes32 hash = getHash(allotment, ticketType);
        address signer = recover(hash, pass);
        if (signer != presaleSigner) revert InvalidPass();
