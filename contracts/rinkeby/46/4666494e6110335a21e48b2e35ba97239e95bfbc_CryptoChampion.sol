    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        if (ENABLE_REVEAL){
            for (uint256 k = 0; k < Item_Count; k++) {
                if (k == 0 && tokenId < Items[0].order_cnt)
                    return string(abi.encodePacked(Items[0].base_uri, tokenId.toString()));
                else if ((tokenId >= Items[k - 1].order_cnt) && (tokenId < Items[k].order_cnt))
                    return string(abi.encodePacked(Items[k].base_uri, (tokenId  - Items[k - 1].order_cnt).toString()));
            }            
        }
        return UNREVEAL_URI;
