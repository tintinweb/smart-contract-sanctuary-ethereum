...
contract XBeyond2 is ERC721, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 collectionId, uint256 tokenId, bool isBend, bytes32[] calldata proof) payable public nonReentrant{
        ...
        IERC721 nft;
        if (isBend) {
            nft = IERC721(collection.bendAsset);
        } else {
    ...
    function redeem(uint256 collectionId, uint256 tokenId, bytes32[] calldata proof) public {
        ...
        // erc20.transfer(address(this), _config.redeemPrice);
        if (_config.redeemPrice > 0) {
            IERC20(paymentToken).transferFrom(msg.sender, address(this), _config.redeemPrice);
        }
