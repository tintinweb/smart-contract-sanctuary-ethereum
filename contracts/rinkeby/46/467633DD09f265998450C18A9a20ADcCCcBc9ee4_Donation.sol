...
/// @title Contract for accepting donations
contract Donation is Ownable {
    ...
    uint256 internal _maximumDonation = 10 ether;
    ...
    uint256 internal _minimumDonation = 0.01 ether;
	...
	mapping(bytes32 => uint256) internal _causesToDonations;
    ...
    function donate(string calldata cause) external payable {
        if (msg.value < _minimumDonation || msg.value > _maximumDonation) revert InvalidDonationAmount();
		_causesToDonations[_keyForCause(cause)] += msg.value;
		emit DonatedToCause(msg.sender, cause, msg.value);
    }
	...
	function donationsForCause(string calldata cause) external view returns (uint256) {
		return _causesToDonations[_keyForCause(cause)];
	}
	...
	function updateDonationLimits(uint256 minimum, uint256 maximum) external onlyOwner {
		_maximumDonation = maximum;
		_minimumDonation = minimum;
	}
