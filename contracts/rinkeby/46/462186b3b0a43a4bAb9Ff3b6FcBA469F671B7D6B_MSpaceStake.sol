...
contract MSpaceStake is owned, IMStake {
    ...
    constructor(address _gameDevWallet, address _coinAddress) {
        ...
        percentage[1].dividend = 3;
        percentage[1].divisor = 20;
        percentage[2].valid = true;
        percentage[2].dividend = 27;
        percentage[2].divisor = 100;
        percentage[3].valid = true;
        percentage[3].dividend = 3;
        percentage[3].divisor = 10;
    }
    ...
    function updateEndTime(uint40 _new) public onlyOwner{
        endTime = _new;
    }
    ...
    function withdrawAmount(address _addr) view external override returns(uint256) {
        ...
        if(daysDeposited >= minumTime && daysDeposited < mediumTime){
            ...
            if(daysDeposited >= endTime){
                value += (((staker.amount) * endTime * percentage[3].dividend / percentage[3].divisor) / 100)+ staker.amount;
            }
            else{
                value += (((staker.amount) * daysDeposited * percentage[3].dividend / percentage[3].divisor) / 100)+ staker.amount;
            }
