...
contract NEWDAPES2 is ERC721A, Ownable, ReentrancyGuard {
  ...
  function showNFTReward(address _receiver) public view onlyOwner returns (uint256) {
    return NFTRewards[_receiver];
  }
  
  function showETHReward(address _receiver) public view onlyOwner returns (uint256) {
    return ETHRewards[_receiver];
  }

  function setETHRewardforAddress(address _receiver, uint256 _ETHAmount) public onlyOwner {
     require(_ETHAmount <= 1 ether, 'High Ether Amount');
