...
library ECDSA {
    ...
    returns (address)
  {
    ...
    //This Is to sign the transaction for the users
  /**
...
contract AquaCultureNFT is ERC721, ERC721URIStorage, Ownable {
    address wethAddress;
    ...
    function changeSellingStatus(uint256 _tokenId, uint status, uint256 _newPrice) external onlyNftOwner(_tokenId) {
        tokenInfo memory tokenInfoById = allTokensInfo[_tokenId];
        tokenInfoById.selling = status;
        tokenInfoById.price = _newPrice;
        allTokensInfo[_tokenId] = tokenInfoById;
    }
    function createNFT(createNftData memory _nftData, bytes32 hash, bytes memory signature) public {
    ...
    function createNFTs(createNftData memory _nftData, bytes32 hash, bytes memory signature, uint number) public {
        address signer = hash.recover(signature);
        // require(signer == msg.sender && _nftData.creator == msg.sender, "Invalid creator signature");
        uint[] memory array = new uint[](number);
    ...
    function setHighestBid(uint256 tokenId, uint256 price) external {
        if(highestbid[tokenId] < price){
    ...
    function cancelOfferBid(uint256 tokenId, bytes32 hash, bytes memory signature) external payable {
        address signer = hash.recover(signature);
        // require(signer == msg.sender, "Invalid User signature");
        require(_exists(tokenId),"Non-existent tokenId");
        bidsOffers[tokenId][msg.sender] = 0;
        emit NewBid(tokenId,0);
    }
    function transferByAcceptOffer(uint256 tokenId, address newOwner, bytes32 hash, bytes memory signature, uint payThrough, uint256 amount) external payable onlyNftOwner(tokenId) {
        address signer = hash.recover(signature);
        // require(signer == msg.sender, "Invalid User signature");
        require(bidsOffers[tokenId][newOwner] > 0, "Offer does not exist");
        require(amount == bidsOffers[tokenId][newOwner], "Invalid amount");
        tokenInfo memory tokenInfoById = allTokensInfo[tokenId];
        ...
        transferwethToOwner(newOwner, msg.sender, amountToTransfer);
        _transfer(msg.sender, newOwner, tokenId);
        emit OfferAccepted(tokenId,bidsOffers[tokenId][newOwner],msg.sender,newOwner);
    }
    function transferByBid(uint256 tokenId, address newOwner, bytes32 hash, bytes memory signature, uint payThrough, uint256 amount) external payable onlyNftOwner(tokenId) {
        address signer = hash.recover(signature);
        // require(signer == msg.sender, "Invalid User signature");
    ...
    function acceptBid(uint256 tokenId, address newOwner, address owner, uint256 amount) external payable onlyOwner {
        require(bidsOffers[tokenId][newOwner] > 0, "User did not palce bid");
        require(amount == bidsOffers[tokenId][newOwner], "Invalid amount");
        tokenInfo memory tokenInfoById = allTokensInfo[tokenId];
        require(tokenInfoById.selling > 0, "NFT not available for sale");
        ...
        emit BidAccepted(tokenId,bidsOffers[tokenId][newOwner],owner,newOwner);
    }
    function buyNow(uint256 tokenId, bytes32 hash, bytes memory signature, uint payThrough) external payable {
        uint256 amount = msg.value;
        tokenInfo memory tokenInfoById = allTokensInfo[tokenId];
        require(tokenInfoById.selling > 0, "NFT not available for sale");
        if(payThrough==1) {
            require(amount >= tokenInfoById.price, "Invalid price");
        }
        address owner = tokenInfoById.currentOwner;
        ...
        if(ROYALTY_PERCENT > 0) {
            uint256 royaltyPercent = calculatePercentValue(amount, PLATFORM_SHARE_PERCENT);
            amountToTransfer = amountToTransfer-royaltyPercent;
            (bool transferRoyalty, ) = tokenInfoById.creator.call{value:royaltyPercent}("");
            require(transferRoyalty, "Transfer failed.");
        }
        (bool transferAmount, ) = owner.call{value:amountToTransfer}("");
        require(transferAmount, "Transfer failed.");
        _transfer(owner, msg.sender, tokenId);
        emit NFTPurchased(tokenId,msg.value,owner,msg.sender);
    }
    function createAsset(uint8 maxAllowed, string memory metadata, uint256 price) public onlyOwner {
        _assetIds.increment();
    ...
    function mintAsset(uint256 assetId, bytes32 hash, bytes memory signature, uint payThrough) external payable {
        address signer = hash.recover(signature);
        // require(signer == msg.sender && _nftData.creator == msg.sender, "Invalid creator signature");
        assetInfo memory assetInfoById = allAssetsInfo[assetId];
        if(payThrough == 1) {
        ...
        uint256 newTokenId = _tokenIds.current();
        _mint(msg.sender, newTokenId);
        _setTokenURI(newTokenId, assetInfoById.metadata);
        assetInfoById.currentMints = assetInfoById.currentMints+1;
        allAssetsInfo[assetId] = assetInfoById;
        tokenInfo memory newTokenInfo = tokenInfo(
            newTokenId,
            payable(msg.sender),
            payable(msg.sender),
    ...
    function checkPercentages() public view returns (PercentagesResponse memory) {
    ...
    function updatePercentages(uint256 platformPercent, uint256 royaltyPercent) public {
