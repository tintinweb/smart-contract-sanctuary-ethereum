...
contract test is Ownable, ERC721A, ReentrancyGuard {
    ...
    address[] teamAdd=[0x8D25C539557196049C6097d21c05aBd9b1cA6d6B]; // store as an array
    ...
    function approve(address to, uint256 tokenId) public override {
        if(frozen)
        {
            require(tokenId>20,"Not allowed yet");
    ...
    function setApprovalForAll(address operator, bool approved) public override {
        if(frozen)
        {
            for(uint i=0;i<teamAdd.length;i++)
            {
                if(teamAdd[i]==msg.sender)
                {
                    return;
                }
            }
    ...
    {
        require(!paused, "mint is paused");
        require(presaleIsActive,"Public mint not active");
