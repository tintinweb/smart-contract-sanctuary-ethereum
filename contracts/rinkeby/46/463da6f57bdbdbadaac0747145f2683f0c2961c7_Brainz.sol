...
contract Brainz is ERC20Burnable, Ownable {
    ...
    */
    ...
    address public HolyHeroesAddress;
    ...
    function setHolyHeroesAddress(address _HolyHeroesAddress) public onlyOwner {
        HolyHeroesAddress = _HolyHeroesAddress;
        return;
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(HolyHeroesAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(HolyHeroesAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
