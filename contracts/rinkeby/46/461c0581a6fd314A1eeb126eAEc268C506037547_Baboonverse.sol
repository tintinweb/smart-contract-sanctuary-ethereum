...
contract Baboonverse is ERC721A, Ownable, VRFConsumerBase, ReentrancyGuard, PaymentSplitter {
    ...
    */
        ...
        PaymentSplitter(payees_, shares_)
    {
        ...
        */
        ...
        maxMintBatchSize = maxMintBatchSize_;
    ...
    function whitelistMint(uint256 quantity_, bytes32[] memory proof_) external payable isUser isWhitelistMint { 
        ...
        require(numberMinted(msg.sender) + quantity_ <= maxPerWallet, "Max per wallet reached");
        require(msg.value == mintPrice * quantity_, "Wrong value!");
        require(whitelistSupply >= totalSupply() + quantity_, "Whitelist supply exceeded");
    ...
    function submitProvenanceHashForMultipleReveal(string memory hash_) external onlyOwner {
        ...
        require(revealedOnBatch[revealBatch - 1] > 0, "Prev request random not finished");
    ...
    function ownerMintToAddress(address to_, uint256 quantity_) external onlyOwner {
        ...
        require(quantity_ % maxMintBatchSize == 0, "can only mint a multiple of the maxMintBatchSize");
        uint256 numChunks = quantity_ / maxMintBatchSize;
        for (uint256 i = 0; i < numChunks; i++) {
            _safeMint(to_, maxMintBatchSize);
        }
    }
    ...
    function reserveTokens(uint256 quantity_) external onlyOwner {
        ...
        uint256 numChunks = quantity_ / maxMintBatchSize;
        for (uint256 i = 0; i < numChunks; i++) {
            _safeMint(msg.sender, maxMintBatchSize);
        }
    }
