...
import "./erc721a/contracts/ERC721A.sol";   //import "erc721a/contracts/ERC721A.sol";
...
contract MYCONTRACTMERKLE is ERC721A, ERC2981, Ownable, Pausable{
    using Strings for uint256;

    string public baseURI = "";
    string internal revealUri = "ipfs://QmewHM4hC5hSq8WgXS7DnWKYzyz7Swqyk1q4jubPng21hW";
    ...
    bool internal isRevealed = false;  //before　-> false　, after　-> true
    bool public presale = true;

    uint64 public preMintedPhase = 1;   //カウントアップでミント済みをリセット
    uint64 public publicMaxPerTx = 10;
    uint64 public royaltyFee = 1000;    //1000 = 10%
    uint64 public maxSupply = 2000;
    address public royaltyAddress = 0x9e81fd993203113AebD886707D8E8c21dDe939c6;

    bytes32 public merkleRoot;
    mapping(uint64 => mapping(address => uint256)) public preMintedLists;
    mapping(string => bool) internal lockLists;   //2,7     trueでロックされる
    // mapping(address => uint256) public preMintListsMapping;
    ...
    function publicMint(uint256 _mintAmount) public payable whenNotPaused callerIsUser {
        ...
        if(presale) revert PresaleIsActive();
        if(_mintAmount > publicMaxPerTx) revert MintAmountOver();
    ...
    //     mintCheck(_mintAmount,  cost);
    //     if(!presale) revert PresaleIsNotActive();
    //     require(
    //         preMintListsMapping[msg.sender] >= _mintAmount,
    ...
    function preMint2(
        ...
        if(preMintedLists[preMintedPhase][msg.sender] + _mintAmount > _maxCount) revert OverMintLimit();    // 登録数は超えてないか？
        preMintedLists[preMintedPhase][msg.sender] += _mintAmount;
        _safeMint(msg.sender, _mintAmount);
    }
    ...
    function setPreMintedPhase() external onlyOwner {
        preMintedPhase = preMintedPhase + 1;
    }

    function getMintedCount(address _address) external view returns (uint256) {
        return preMintedLists[preMintedPhase][_address];
    }

    function setRoyaltyFee(uint64 _feeNumerator) external onlyOwner {
    ...
    function setMaxSupply(uint64 _maxSupplyNumerator) external onlyOwner {
        maxSupply = _maxSupplyNumerator;
    }

    function setPublicMaxPerTx(uint64 _publicMaxPerTx) external onlyOwner {
    ...
    function _beforeTokenTransfers(
        ...
        if(to != address(0)){
            for (uint256 i = startTokenId; i < startTokenId + quantity; i++) {
                if(lockLists[Strings.toString(i)] == true) revert ThisTokenIsLocked();
            }
        }
    }
