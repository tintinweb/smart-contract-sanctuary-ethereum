 ...
 */
contract CustomFutureVault is FutureVault {
...
import "contracts/protocol/futures/RewardsRecipient.sol";
...
import "contracts/interfaces/apwine/IRateOracle.sol";
 ...
 */
abstract contract FutureVault is
    Initializable,
    RoleCheckable,
    RegistryStorage,
    ...
    IFutureVault
{
    ...
    mapping(address => mapping(address => uint256)) internal delegationsByDelegator;
    ...
    IERC1820RegistryUpgradeable internal _ERC1820_REGISTRY;
    ...
    function initialize(
        ...
        require(_ERC1820_REGISTRY.getInterfaceImplementer(address(_ibt), keccak256(abi.encodePacked("ERC777Token"))) == address(0x0), "ERC777: We do not support ERC777 IBTs");
        ibt = _ibt;
     ...
     */
    function startNewPeriod() public virtual override nextPeriodAvailable periodsActive nonReentrant onlyController {
        _switchPeriod();
    }
    ...
    function _switchPeriod() internal periodsActive {
        ...
        if (yield > 0) {
            ...
            yieldOfPeriod[currentPeriodIndex] = convertIBTToUnderlying(yield.mul(IBT_UNIT).div(totalUnderlyingDeposited));
            uint256 collectedYield = yield.mul(fyts[currentPeriodIndex].totalSupply()).div(totalUnderlyingDeposited);
            ...
            if (collectedYield > 0) ibt.safeTransfer(address(futureWallet), collectedYield);
        } else {
    ...
    function _preparePTClaim(address _user, uint256 _currentPeriodIndex) internal virtual returns (uint256 claimablePT) {
     ...
     */
    function deposit(address _user, uint256 _amount) external virtual override nonReentrant periodsActive depositsEnabled onlyController {
    ...
    function _deposit(address _user, uint256 _amount) internal {
        ...
        uint256 currentPeriodIndex = getCurrentPeriodIndex();
        uint256 ptToMint = _preparePTClaim(_user, currentPeriodIndex).add(underlyingDeposited);
     ...
     */
    function withdraw(address _user, uint256 _amount) external virtual override nonReentrant withdrawalsEnabled onlyController {
        require((_amount > 0) && (_amount.add(getTotalDelegated(_user)) <= pt.balanceOf(_user)), "FutureVault: ERR_AMOUNT");
        uint256 currentPeriodIndex = getCurrentPeriodIndex();
        IFutureYieldToken futureYieldToken = fyts[currentPeriodIndex];
        require(_amount <= futureYieldToken.balanceOf(_user), "FutureVault: ERR_FYT_AMOUNT");
    ...
    function createFYTDelegationTo(
        ...
        require(_receiver != address(0), "ERR: Cannot delegate to zero address");
        ...
        delegationsByDelegator[_delegator][_receiver] = delegationsByDelegator[_delegator][_receiver].add(_amount);
        totalDelegationsMade[_delegator] = totalDelegationsMade[_delegator].add(_amount);
    ...
    function withdrawFYTDelegationFrom(
        ...
        delegationsByDelegator[_delegator][_receiver] = delegationsByDelegator[_delegator][_receiver]
        .sub(_amount, "FutureVault: ERR_AMOUNT");
        totalDelegationsMade[_delegator] = totalDelegationsMade[_delegator].sub(_amount);
     ...
     */
    function getTotalDelegated(address _delegator) public view override returns (uint256) {
     ...
     */
    function claimFYT(address _user, uint256 _amount) external virtual override nonReentrant {
        uint256 currentPeriodIndex = getCurrentPeriodIndex();
        require(msg.sender == address(fyts[currentPeriodIndex]), "FutureVault: ERR_CALLER");
        updateUserState(_user);
        _claimFYT(_user, _amount, currentPeriodIndex);
    }

    function _claimFYT(address _user, uint256 _amount, uint256 _currentPeriodIndex) internal virtual {
     ...
     */
    function exitTerminatedFuture(address _user) external override nonReentrant onlyController {
     ...
     * @dev needs corresponding permissions for sender
     */
    function setFutureWallet(IFutureWallet _futureWallet) external override onlyAdmin {
    ...
    function convertIBTToUnderlyingWithRate(uint256 _amount, uint256 _rate) public view virtual override returns (uint256) {
    ...
    function convertUnderlyingtoIBTWithRate(uint256 _amount, uint256 _rate) public view virtual override returns (uint256) {
     ...
     */
    function getClaimablePT(address _user, uint256 _totalDelegated) public view virtual override returns (uint256) {
        uint256 currentPeriodIndex = getCurrentPeriodIndex();
        ...
        if(lastPeriodClaimedUser == 0) {
            ...
            recordedBalance = recordedBalance.add(totalDelegationsReceivedUser).sub(_totalDelegated); // add delegated FYTs
            uint256 userStackingGrowthFactor = yieldOfPeriod[lastPeriodClaimedUser];
            if (userStackingGrowthFactor > 0) {
                recordedBalance = recordedBalance.add(claimableFYTByUser[_user].mul(userStackingGrowthFactor) / IBT_UNIT); // add reinvested FYTs
            }
            for (uint256 i = lastPeriodClaimedUser + 1; i < currentPeriodIndex; i++) {
                recordedBalance = recordedBalance.add(yieldOfPeriod[i].mul(recordedBalance) / IBT_UNIT);
            }
            return
                recordedBalance.add(_totalDelegated).sub(pt.recordedBalanceOf(_user)).sub(
                    totalDelegationsReceivedUser
                );
        } else {
            return 0;
        }
     ...
     */
    function getClaimableFYTForPeriod(address _user, uint256 _periodIndex) external view virtual override returns (uint256) {
        uint256 currentPeriodIndex = getCurrentPeriodIndex();
    ...
    function getUnrealisedYieldPerPT() public view override returns (uint256) {
        ...
        uint256 totalUnderlyingNow = convertIBTToUnderlyingWithRate(ibt.balanceOf(address(this)), rate);
        uint256 yieldForAllPT = convertUnderlyingtoIBTWithRate(totalUnderlyingNow.sub(totalUnderlyingAtStart), rate);
    ...
    function getPTPerAmountDeposited(uint256 _amount) public view override returns (uint256) {
        uint256 rate = getIBTRate();
        uint256 underlyingAmountOfDeposit = convertIBTToUnderlyingWithRate(_amount, rate);
        uint256 underlyingYieldPerPT = convertIBTToUnderlyingWithRate(getUnrealisedYieldPerPT(), rate);
     ...
     */
    function getYieldOfPeriod(uint256 _periodID) external view override returns (uint256) {
     ...
     */
    function getNextPeriodIndex() public view virtual override returns (uint256) {
        return fyts.length;
    }
     ...
     * @dev index starts at 1
     */
    function getCurrentPeriodIndex() public view override virtual returns (uint256) {
        if(isTerminated()){
            return fyts.length;
        }
     ...
     */
    function getTotalUnderlyingDeposited() external view override returns (uint256) {
     ...
     * @return IBT address
     */
    function getIBTAddress() public view virtual override(IFutureVault, RewardsRecipient) returns (address) {
        return address(ibt);
    }
     ...
     * @return FYT address
     */
    function getFYTofPeriod(uint256 _periodIndex) external view override returns (address) {
...
import "contracts/interfaces/apwine/IRewardsRecipient.sol";
 ...
 */
abstract contract RewardsRecipient is RoleCheckable, ReentrancyGuardUpgradeable, IRewardsRecipient {
    using SafeERC20Upgradeable for IERC20;
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;
     ...
     */
    function harvestRewards() public virtual override nonReentrant onlyController {
        _harvestRewards();
     ...
     */
    function redeemVaultRewards(address _rewardToken) external virtual override onlyController {
     ...
     * @param _rewardToken the reward token to redeem the balance of
     */
    function redeemWalletRewards(address _rewardToken) external virtual override onlyController {
        _redeemRewards(_rewardToken);
    }

    function _redeemAllRewards() internal {
        require(rewardsRecipient != address(0), "RewardsRecipient: ERR_RECIPIENT");
    ...
    function _redeemRewards(address _rewardToken) internal {
        require(rewardsRecipient != address(0), "RewardsRecipient: ERR_RECIPIENT");
        require(rewardTokens.contains(address(_rewardToken)), "RewardsRecipient: ERR_TOKEN_ADDRESS");
    ...
    function addRewardsToken(address _token) external override onlyAdmin {
        require(_token != getIBTAddress(), "RewardsRecipient: ERR_TOKEN_ADDRESS");
     ...
     */
    function setRewardRecipient(address _recipient) external override onlyAdmin {
     ...
     */
    function isRewardToken(address _token) external view override returns (bool) {
     ...
     */
    function getRewardTokenAt(uint256 _index) external view override returns (address) {
...
interface IController {
    ...
    event NextPeriodSwitchSet(uint256 indexed _periodDuration, uint256 _nextSwitchTimestamp);
     ...
     * @dev only pt are required as there  aren't any new FYTs
     */
    function exitTerminatedFuture(IFutureVault _futureVault, address _user) external;
     ...
     */
    function setPeriodDurationIndex(uint256 _periodDuration, uint256 _newPeriodIndex) external;
    ...
    function redeemVaultRewards(IFutureVault _futureVault, address _rewardToken) external;
    ...
    function redeemWalletRewards(IFutureVault _futureVault, address _rewardToken) external;
}
...
interface IFutureVault is IRewardsRecipient {
    ...
    event DelegationCreated(address indexed _delegator, address indexed _receiver, uint256 _amount);
    event DelegationRemoved(address indexed _delegator, address indexed _receiver, uint256 _amount);
     ...
     * @return the amount of pt claimable by the user
     */
    function getClaimablePT(address _user, uint256 _totalDelegated) external view returns (uint256);
     ...
     * @return the corresponding amount of underlying
     */
    function convertIBTToUnderlyingWithRate(uint256 _amount, uint256 _rate) external view returns (uint256);
     ...
     * @return the corresponding amount of IBTs
     */
    function convertUnderlyingtoIBTWithRate(uint256 _amount, uint256 _rate) external view returns (uint256);
}
