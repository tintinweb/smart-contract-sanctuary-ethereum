...
contract FairXYZDeployer is ERC721xyzUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    // Signer address for ECDSA
    address internal signAdd;

    /**
     * @dev Return list of contract variables
     */
    function viewAllVariables()
        public
        view
        returns (
            uint256,
    ...
    function _initialize(
        ...
        signAdd = 0xb403d77946B4Ac4FC7CA2EE1059e73f1b72D6e93;
    }
    ...
    function hashVariableChanges(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encodePacked(sender, newURI, newPathURI, newPrice, newMaxMintsPerWallet, newRoyaltyPercentage)
                )
            )
        );
        return hash;
     ...
     */
    function changeRoyaltyReceivers(
        address newPrimaryRoyaltyReceiver,
        ...
        if (requiresSecondaryChange) _setDefaultRoyalty(_secondaryRoyaltyReceiver, _secondaryRoyaltyValue);
    }
    ...
    function overrideVariables(
        ...
        bytes32 messageHash = hashVariableChanges(
            msg.sender,
            newURI,
            newPathURI,
            newPrice,
        ...
        if (!lockURI) {
            if (bytes(newPathURI).length != 0) pathURI = newPathURI;
    ...
    function remainingMints(address minterAddress) public view returns (uint256) {
        ...
        uint256 mintsLeft = maxMintsPerWallet - mintData[minterAddress].mintsPerWallet;
     ...
     */
    function airdrop(address[] memory address_, uint256 tokenCount) external onlyOwner returns (uint256) {
        require(
            _mintedTokens + address_.length * tokenCount <= maxTokens,
    ...
    function mint(
        ...
        require(messageHash.recover(signature) == signAdd, "Unrecognizable Hash");
        require(mintData[msg.sender].blockNumber < nonce, "Reused Hash");
        require(msg.value >= nftPrice * numberOfTokens, "You have not sent the required amount of ETH");
        require((0 < numberOfTokens) && (numberOfTokens <= 20), "Token minting limit per transaction exceeded");
        ...
        uint256 mintsPerWallet = uint256(mintData[msg.sender].mintsPerWallet);
        ...
        if (maxMintsPerWallet > 0) {
            require(mintsPerWallet < maxMintsPerWallet, "Exceeds number of mints per wallet");

            if (mintsPerWallet + numberOfTokens > maxMintsPerWallet) {
                numberOfTokens = maxMintsPerWallet - mintsPerWallet;
            }
        }
        ...
        _mint(msg.sender, numberOfTokens, nonce);
     ...
     */
    function mintNoSignature(uint256 numberOfTokens) external payable whenNotPaused saleIsOpen {
 ...
 */
contract ERC721xyzUpgradeable is
    ContextUpgradeable,
    ERC165Upgradeable,
    IERC721Upgradeable,
    ERC2981Upgradeable,
