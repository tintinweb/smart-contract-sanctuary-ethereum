    ...
    VRFConsumerBaseV2
{
    ...
    mapping(uint256 => CanaryBreed) public tokenIdToCanaryRandomBreed; //Store the Breed identified for one tokenId @todo : might be redundant with tokenIdToCanaryRandomBreedNumber
    mapping(uint256 => uint256) public tokenIdToCanaryRandomBreedNumber; // Store the random mumber identified per tokenId
    mapping(uint256 => uint256) public tokenIdToRequestId; //Store the random requestId for a tokenId
    mapping(uint256 => uint256) public requestIdToTokenId; //Store the tokenId for each random requesId
    ...
    event tokenIdRandomRequest(uint256 indexed requestId, uint256 tokenId);
    ...
    //     _safeMint(s_owner, tokenCounter.current());

    //     // a random number is requested to the VRF Randomness manager and stored against the token ID.
    //     // uint256 requestId = this.requestRandomness();

    //     s_requestId = this.COORDINATOR.requestRandomWords(
    //         vrf_keyHash,
    //         s_subscriptionId,
    ...
    //     emit randomRequest(tokenCounter.current(), s_requestId);

    //     requestIdToTokenId[s_requestId] = tokenCounter.current();
    //     emit tokenIdRandomRequest(s_requestId, tokenCounter.current());

    //     return tokenCounter.current();
    // }

    // function revealTokenId(uint256 tokenId) external onlyOwner {
    //     require(
    ...
    //     // We genreate random breed number for Canary NFTs, map it against the tokenID and emit the related event
    //     uint256 randomCanaryBreedNumber = s_randomWords[0] % _numberBreed;
    ...
    //     // We genreate random breed ENUM number for Canary NFTs, map it against the tokenID and emit the related event
        ...
        returns (uint256)
    {
        require(
            ...
            "There is no random CanaryBreed for the given Token Id"
        );
    ...
    function fulfillRandomWords(
        ...
        require(_requestId > 0, "Can't find the RequestId");
        require(
            requestIdToTokenId[_requestId] > 0,
            "There is no RequestId mapped to the TokenId"
        );
        s_randomWords = randomWords;
        uint256 tokenId = requestIdToTokenId[_requestId];
        // revealTokenId(requestIdToTokenId[_requestId]);
