...
contract GoldenEggi is ERC721Enumerable, AccessControl {
    ...
    uint256 public maxNftAllowed;

    uint256 public lastTokenId; // token counter for the last token id minted
    uint256 public maxTokenId; // maximum number of nfts 9,999

    uint256 public airdropLastTokenId; // starts from 1
    uint256 public airdropMaxTokenId; // maximum number of airdop 199
    ...
    uint256 public whitelistCurrentCount;
    uint256 public whitelistMaxCount;
    uint256 public receiverPercentage;

    address public receiverWallet1;
    address public receiverWallet2;
    address public receiverWallet3;
    ...
    mapping(address => uint256) public usersMintCount;
    address[] public allWhitelistUsers;

    event Receiver1(address receiverWallet1);
    event Receiver2(address receiverWallet1);
    event Receiver3(address receiverWallet1);
    event WhitelistMaxCount(uint256 maxWhitelistCount);
    event MaxNftAllowed(uint256 maxNFTAllowed);
    event PreSalePrice(uint256 preSalePrice);
    event PublicSalePrice(uint256 publicSalePrice);
    event WhitelistStartTime(uint256 whitelistStartTime);
    event WhitelistCloseTime(uint256 whitelistCloseTime);
    ...
    function setReceiverWallet1(address _receiver1) external onlyAdmin {
        require(_receiver1 != address(0), "invalid address");
        receiverWallet1 = _receiver1;
        emit Receiver1(_receiver1);
    }

    function setReceiverWallet2(address _receiver2) external onlyAdmin {
        require(_receiver2 != address(0), "invalid address");
        receiverWallet2 = _receiver2;
        emit Receiver2(_receiver2);
    }

    function setReceiverWallet3(address _receiver3) external onlyAdmin {
        require(_receiver3 != address(0), "invalid address");
        receiverWallet3 = _receiver3;
        emit Receiver3(_receiver3);
    }

    function setWhitelistMaxCount(uint256 _maxWhitelist) external onlyAdmin {
        whitelistMaxCount = _maxWhitelist;
        emit WhitelistMaxCount(_maxWhitelist);
    }

    function setMaxNftAllowed(uint256 _maxNfts) external onlyAdmin {
        require(_maxNfts > 0, "invalid value");
        maxNftAllowed = _maxNfts;
        emit MaxNftAllowed(_maxNfts);
    }
        ...
        onlyAdmin
    {
        ...
        emit WhitelistStartTime(_whitelistStartTime);
    }

    function setWhiteListCloseTime(uint256 _whitelistCloseTime)
        external
        onlyAdmin
    {
        whitelistCloseTime = _whitelistCloseTime;
        emit WhitelistCloseTime(_whitelistCloseTime);
    }
    ...
    function whitelistWallet() public {
        require(
            block.timestamp >= whitelistStartTime &&
                block.timestamp <= whitelistCloseTime,
            "GoldenEggi: whitelist registration times end"
        );
        require(
            whitelistCurrentCount + 1 <= whitelistMaxCount,
            "GoldenEggi: whitelist count exceeds limit"
        );
        require(
        ...
        allWhitelistUsers.push(msg.sender);
        whitelistCurrentCount++;
    }

    function whitelistWalletAdmin(address[] memory _users) external onlyAdmin {
        uint256 usersLength = _users.length;
        for (uint256 i = 0; i < usersLength; i++) {
            require(
                whitelistUsers[_users[i]] == false,
                "already registered for presale"
            );
            whitelistUsers[msg.sender] = true;
            allWhitelistUsers.push(msg.sender);
    ...
    function preSale(uint256 _num) public payable returns (bool result) {
        ...
        require(whitelistUsers[msg.sender], "GoldenEggi: not whitelisted");
        require(
            usersMintCount[msg.sender] + _num <= maxNftAllowed,
            "mint count exceed"
        );
        uint256 totalPrice = tokenPricePreSale * _num;
        uint256 distPrice = (totalPrice * receiverPercentage) / 10000;
        require(
        ...
        usersMintCount[msg.sender] += _num;
    ...
    function publicSale(uint256 _num) public payable returns (bool result) {
        ...
        for (uint256 i = 0; i < _num; i++) {
            lastTokenId++; //it should increment one less then the last Token Id, for public sale
            totalMints++;
            if(notAllowedToMint(lastTokenId)) {
                continue;
            }
    ...
    function airdropNfts(address[] memory _airdropAddresses) public onlyAdmin {
        ...
        for (uint256 i = 0; i < walletsLength; i++) {
            airdropLastTokenId++; //it should increment one less then the last Token Id, for public sale
            totalMints++;
    ...
    function minyByAdmin(address _walletAddress, uint256 _tokenId) public onlyAdmin {
