...
contract IncredibleAniballs is ERC721A, Ownable, Pausable, PaymentSplitter {
    ...
    uint8 public maxPerTransaction = 3;
    // Max supply of current step
    uint16 public maxSupply = 1086;
    // NFT price in wei
    uint256 public price = 0.0543 ether;
    // Provenance Hash calculated based on the final shuffled order of metadata files
    ...
    string private _hiddenBaseURI =
        "ipfs://bafybeid6msicjitnp433ymrrba6bwesw5xu3esh5e72mxpqb646rzq2qh4/hidden.json";
    ...
    modifier mintCompliance(uint8 quantity) {
        require(
            quantity > 0 &&
        ...
        require(
            uint8(_numberMinted(msg.sender)) + quantity <= maxPerAddress,
    ...
    function presaleMint(uint8 quantity, bytes32[] calldata proof)
        external
        payable
        whenPresale
    ...
    function mintForAddresses(uint8 quantity, address[] calldata addresses)
        external
        onlyOwner
    {
    ...
    function mintForAddress(uint8 quantity, address to)
        public
        mintCompliance(quantity)
        onlyOwner
    {
    ...
    /// @param newMerkleRoot, root of merkle tree
    function setMerkleRoot(bytes32 newMerkleRoot) external onlyOwner {
        _merkleRoot = newMerkleRoot;
    }

    /// @notice setProvenance, called only by owner
    /// @dev set the provenance proof of NFTs random order
    /// @param provenance_, hash of the proof
    ...
    function setMaxPerAddress(uint8 newMaxPerAddress) external onlyOwner {
    ...
    function setMaxPerTransaction(uint8 newMaxPerTransaction)
        external
        onlyOwner
    {
        require(
    ...
    function setMaxSupply(uint16 supply) external onlyOwner {
        require(
            supply + _totalMinted() < COLLECTION_SIZE + 1,
            "Public supply is too big"
        );
    ...
    function _verify(address account, bytes32[] memory proof)
        internal
        view
        returns (bool)
    {
