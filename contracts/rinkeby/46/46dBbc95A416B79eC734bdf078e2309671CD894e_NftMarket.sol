...
contract NftMarket is ReentrancyGuard {
    ...
    uint listingFee = 0.0001 ether;
    address payable owner;
    mapping(address => uint) public nftOwned;
    mapping(address => uint) public itemsCreated;
   ...
   mapping(uint => MarketItem) public idToMarketItem;
    ...
    function createMarketItem(
        ...
        itemsCreated[msg.sender] += 1;

        emit MarketItemCreated(currentId, _nftContract, _tokenId, payable(msg.sender), payable(address(0)), _price, false);
    }
    ...
    function sellMarketItem(
        ...
        uint price = currentItem.price;
        require(msg.value == price, "please send the required amount");
        address _nftaddress = currentItem.nftContract;
        uint _tokenId = currentItem.tokenId;

        (bool sent, ) = currentItem.seller.call{value: msg.value}("");
    ...
    function unSoldMarketItems() public view returns (MarketItem[] memory) {
        uint totalItems = itemId.current();
        uint totalSoldItems = itemsSold.current();
        uint unSoldItems = totalItems - totalSoldItems;
        MarketItem[] memory items = new MarketItem[](unSoldItems);
        uint currentId = 0;
        
        for (uint i = 1; i <= totalItems; i++) {
            if (idToMarketItem[i].sold == false) {
                uint currentItem = idToMarketItem[i].itemId;
                MarketItem memory item = idToMarketItem[currentItem];
    ...
    function fetchMyNFTs() public view returns (MarketItem[] memory) {
        uint totalItems = itemId.current();
        uint myNFTs = nftOwned[msg.sender];
        MarketItem[] memory items = new MarketItem[](myNFTs);
        uint currentId = 0;
    ...
    function fetchItemsCreated() public view returns(MarketItem[] memory) {
        uint totalItems = itemId.current();
        uint myItems = itemsCreated[msg.sender];
