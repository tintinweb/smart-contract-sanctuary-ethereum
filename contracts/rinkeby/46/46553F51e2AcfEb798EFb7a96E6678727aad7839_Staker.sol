...
contract Staker {
  ...
  uint256 timeBeforeDeadline = 72 hours;
  uint256 public deadline;
  event Stake(address,uint256);

  constructor(address exampleExternalContractAddress) public {
      exampleExternalContract = ExampleExternalContract(exampleExternalContractAddress);
      deadline=block.timestamp + timeBeforeDeadline;
  }
  ...
  function withdraw() public {
    require(block.timestamp >= deadline, "must be after deadline");
    require(address(this).balance < threshold, "contract balance must be less than threshold");
