...
╚══════╝╚═╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝     ╚═╝    ╚═╝  ╚═══╝╚═╝        ╚═╝   
   ...
   WebSite:  https://zikko.xyz        Twitter: @zikko_xyz  
   Telegram: https://t.me/zikko_xyz   Discord: https://discord.gg/K6G2mFN5      
   Contract: 0x                                                               
*/
...
contract ZIKKO is ERC721A, Ownable, ReentrancyGuard {
  ...
  address public verifyAddress = 0x00000000003ce4450631433324417B21a826Ec0E;
  bool public locked;
  ...
  uint256 private constant PRESALE_PRICE = 0.049 ether;
  uint256 private constant PUBLIC_PRICE = 0.079 ether;
  uint256 private constant LAST_PRICE = 0.099 ether;
  uint256 public preSaleStartTimestamp;
  uint256 public publicSaleStartTimestamp;
  mapping(address => uint256) public freeNonce;
  mapping(address => uint256) public presaleNonce;

  string private permanentIPFSLink = "";
  string private tokenBaseURIStates = "https://zikko.xyz/api/metadata/";

  uint256 constant Free = 1;
  uint256 constant Presale = 2;

  constructor() ERC721A("ZIKKO", "ZIKKO") {
  ...
  function mintPublicSale(uint256 _count) external payable nonReentrant whenPublicSaleActive {
    ...
    require(_count > 0, "Invalid ZIKKO characters count");
    require(totalSupply() + _count <= MAX_SUPPLY, "All ZIKKO characters have been minted");
  ...
  ) external payable nonReentrant whenPreSaleActive {
    require(!presalelocked, "presale ended");
    require(_count > 0, "Invalid ZIKKO characters count");
    require(totalSupply() + _count <= MAX_SUPPLY, "All early access ZIKKO characters have been minted");
  ...
  function freeMint(
    ...
    uint256 _nonce,
    bytes calldata signature
  ) external nonReentrant whenPreSaleActive {
  ...
  // 验证是否在白名单 merkel-tree 方式验证
  function verifyProof(bytes32 _value, bytes32[] calldata _proof) public view returns (bool) {
