...
import './FightCoin.sol';
import './MintFighterContract.sol';
...
contract NFTStaking {
    ...
    uint rewardPerHour = 10000;

    FightCoin fightCoin;
    MintFighterContract nft;
    ...
    constructor(FightCoin _fightCoin, MintFighterContract _nft) {
        fightCoin = _fightCoin;
        nft = _nft;
    }
    ...
    function Stake(uint[] calldata tokenIds) external {
        uint tokenId;
        totalStacked += tokenIds.length;

        for(uint i = 0; i < tokenIds.length ; i++) {
            tokenId = tokenIds[i];
            require(nft.ownerOf(tokenId) == msg.sender, "Not the owner");
            require(NFTsStaked[tokenId].stakingStartTime == 0, "Already Stacked");
    ...
    function _unstakeMany(address owner, uint[] calldata tokenIds) internal {
        ...
        for(uint i = 0; i < tokenIds.length ; i++){
            tokenId = tokenIds[i];
            require(NFTsStaked[tokenId].owner == owner, "Not the owner");
    ...
    function _claim(address owner, uint[] calldata tokenIds, bool _unstake) internal {
        ...
        for(uint i = 0; i < tokenIds.length ; i++) {
            ...
            require(thisStake.owner == owner, "Not the owner, you cannot clain the awards");
        ...
        if(totalEarned > 0) {
            fightCoin.mint(owner, totalEarned);
        }
        if(_unstake) {
    ...
    function getOwnerOfTokenId(uint[] calldata tokenIds) external view returns(address[] memory) {

        address[] memory addresses = new address[](tokenIds.length);
        for(uint i = 0; i < tokenIds.length; i++){
            uint tokenId = tokenIds[i];
            Staking memory thisStake = NFTsStaked[tokenId];
            addresses[i] = thisStake.owner;
        }
...
contract FightCoin is ERC20, Ownable, Authorizable {
    ...
    //mint for the stacking of nfts
     function mint(address _to, uint _amount) external onlyAuthorized {
        _mint(_to, _amount);
    }
...
import "./IFighterFactory.sol";

import "./Authorizable.sol";

contract MintFighterContract is ERC721Enumerable, Authorizable {
    ...
    IFighterFactory private gameContract;
    ...
    constructor(
        ...
        gameContract = IFighterFactory(_gameContract);
        setBaseURI(_initBaseUri);
        //gameContract.setBaseUri(_initBaseUri);
    ...
    function presale(uint _quantity) external payable whitelistIsActive {
        uint256 supply = totalSupply();

        require(supply <= presaleSupply, "Presale is sold out.");
        require(supply + _quantity <= presaleSupply, "Max quantity exceed.");
        require(_quantity <= whitelistMaxMint && _quantity >= 1, "You can mint 1 nft minimum and 5 maximum");
        require(onWhitelist[msg.sender], "You are not in whitelist");
        require(
            whitelistClaimedBy[msg.sender] + _quantity <= whitelistMaxMint,
            "Purchase exceeds max allowed"
        );
        require(msg.value >= presalePrice * _quantity, "try to send more ETH");

        gameContract._setUpRole(gameContract.MINTER_ROLE(), msg.sender);

        for(uint i = 0; i < _quantity; i++){
            uint256 itemId = _tokenIds.current();
            gameContract.setFighter(itemId);
        ...
        whitelistClaimedBy[msg.sender] += _quantity;
    ...
    function mint(uint _quantity) public payable contractIsActive {
        uint256 supply = totalSupply();

        require(supply + _quantity <= maxSupply, "max NFT limit exceeded");
        require(msg.value >= cost * _quantity, "insufficient funds");
        require(_quantity >= 1 && _quantity <= nftPerAddressLimit, "You can mint 1 nft minimum and 5 maximum");
        require(
            balanceOf(msg.sender) + _quantity < nftPerAddressLimit,
        ...
        returns (string memory)
    {
        ...
        // return gameContract.tokenURI(tokenId);
    ...
    function giftNft(address _sendTo) external onlyAuthorized {
        uint256 itemId = _tokenIds.current();
        gameContract._setUpRole(gameContract.MINTER_ROLE(), _sendTo);
    ...
    function setGameContract(address _contractAddress) public onlyAuthorized {
        gameContract = IFighterFactory(_contractAddress);
    }
...
interface IFighterFactory {
    ...
    function setFighter(uint256 fighterId) external;
