...
contract Totem is ERC1155, ERC2981, Ownable {
    ...
    mapping(uint256 => uint256) public totem;
    ...
    function mint(uint256 id, uint256 items) external payable {
        ...
        require(totem[id] > 0, "invalid token id");
        ...
        require(totemMinted[id] + items <= totem[id], "exceed max item supply");
        // check eth
        require(msg.value * items == mintPriceInWei, "not enough eth balance");
    ...
    function createCollection(uint256 id, uint256 cap) public onlyOwner {
        totem[id] = cap;
        totalCollections += 1;
        totalSupply += cap;
    }
    ...
    function setMintPriceInWei(uint256 mintPriceInWei_) external onlyOwner {
