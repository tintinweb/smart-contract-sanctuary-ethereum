...
// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@#5PPPPPPPPPP5.     [email protected]@@@@#PPPPP[email protected]@@@@@:  ?PP&@@@@@@@@@@@@@@@//
...
contract OCASnake is Ownable, ERC721A, ERC2981, ReentrancyGuard, OCASnakeArt {
    ...
    function snakeNameSave(uint16 tokenId, uint256 seed)
        private
        returns (uint16)
    {
        ...
        uint8 adjectiveIndex = uint8(
            generateAppleSeed(tokenId, 0) % wackyAdjectives.length
        ) + 2;
        uint8 supremeIndex = uint8(
            generateAppleSeed(tokenId, 0) % supremeAdjectives.length
        ) + 1;
        uint16 combineIndex = snakeNameIndex * adjectiveIndex * supremeIndex;
