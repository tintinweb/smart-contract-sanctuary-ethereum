...
contract Harlemify is ERC721Base, ReentrancyGuard {

  // ============ Constants ============
  //the sale price per token
  uint256 public constant SALE_PRICE = 0.00001 ether;

  // ============ Deploy ============

  /**
   * @dev Sets up ERC721Base.
   */
  constructor(string memory uri) ERC721Base("Harlemify", "HARLEM") {
    _setContractURI(uri);
  }
   ...
   */
  function mint(string memory uri) external onlyRole(DEFAULT_ADMIN_ROLE) {
   ...
   * on the base URI passed at construction.
   */
  function buy(string memory uri) external payable nonReentrant {
    //the value sent should be the price
    require(SALE_PRICE <= msg.value, "Amount sent is not correct");
    _safeMint(_msgSender(), 1);
    _setTokenURI(lastTokenId(), uri);
  }
  ...
  BEP721
{
  ...
  /**
   * @dev Describes linear override for `tokenURI` used in both 
   * `ERC721B`, `ERC721BURIStorage`
   */
  ...
  /**
   * @dev Describes linear override for `_burn` used in 
   * both `ERC721B` and `ERC721BURIStorage`
   */
  function _burn(uint256 tokenId) internal virtual override(ERC721B, ERC721BURIStorage) {
    super._burn(tokenId);
  }
}
...
abstract contract ERC721BPausable is Pausable, ERC721B {
  ...
  function _beforeTokenTransfers(
    ...
    super._beforeTokenTransfers(from, to, startTokenId, quantity);
    require(!paused(), "ERC721BPausable: token transfer while paused");
  }
}
