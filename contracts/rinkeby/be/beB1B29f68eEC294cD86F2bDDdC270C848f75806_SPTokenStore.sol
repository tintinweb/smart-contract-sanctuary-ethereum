...
// File: contracts/SPTokenStore.sol
...
//import './libraries/SPOperations.sol';
//import './interfaces/ISPTokenStore.sol';
//import './interfaces/ISPProjects.sol';
//import './SPToken.sol';
...
contract SPToken is ISPToken, ERC20Permit, Ownable {
  ...
  */
  function decimals() public view override(ERC20, ISPToken) returns (uint8) {
...
contract SPTokenStore is ISPTokenStore {
  //*********************************************************************//
  // --------------------------- custom errors ------------------------- //
  //*********************************************************************//
  ...
  */
  mapping(uint256 => ISPToken) public override tokenOf;
  ...
  */
  mapping(ISPToken => uint256) public override projectOf;
  ...
  */
  function totalSupplyOf(uint256 _projectId) external view override returns (uint256 totalSupply) {
    ...
    ISPToken _token = tokenOf[_projectId];

    // If the project has a current token, add it's total supply to the total.
    if (_token != ISPToken(address(0))) totalSupply = totalSupply + _token.totalSupply(_projectId);
  }
  ...
  */
    ...
    returns (uint256 balance)
  {
    ...
    if (_token != ISPToken(address(0))) balance = balance + _token.balanceOf(_holder, _projectId);
  }
  ...
  */
  function issueFor(
    ...
    string calldata _name,
    string calldata _symbol
  ) external override returns (ISPToken token) {
    ...
    if (tokenOf[_projectId] != ISPToken(address(0))) revert PROJECT_ALREADY_HAS_TOKEN();

    // Deploy the token contract.
    token = new SPToken(_name, _symbol);
  ...
  /**
    ...
    @dev
    This contract must have access to all of the token's `ISPToken` interface functions.
  ...
  */
  function changeFor(
    ...
    ISPToken _token,
    address _newOwner
  ) external override returns (ISPToken oldToken) {
    // Can't remove the project's token if the project requires claiming tokens.
    if (_token == ISPToken(address(0)) && requireClaimFor[_projectId])
    ...
    if (_token != ISPToken(address(0)) && _token.decimals() != 18)
  ...
  */
  function burnFrom(
    ...
    uint256 _claimedBalance = _token == ISPToken(address(0))
      ? 0
  ...
  */
  function claimFor(
    ...
    if (_token == ISPToken(address(0))) revert TOKEN_NOT_FOUND();
