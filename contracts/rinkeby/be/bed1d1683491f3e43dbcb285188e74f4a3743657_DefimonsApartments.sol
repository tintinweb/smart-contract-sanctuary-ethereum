 ...
 */
contract DefimonsApartments is Ownable, ONFT721 {
    ...
    error NotInSalePhase(uint256 saleId, uint256 start, uint256 finish, uint256 current);
    error UserNotWhitelistedOrWrongProof(uint256 saleId, address user, bytes32[] proof);
    error WrongValueSentForMint(uint256 saleId, uint256 value, uint256 price, uint8 quantity);
    ...
    event LogSaleCreated(uint256 indexed saleId, uint64 start, uint64 finish, uint8 limit, uint64 price, bool whitelist, bytes32 root);
    event LogSaleEdited(uint256 indexed saleId, uint64 start, uint64 finish, uint8 limit, uint64 price, bool whitelist, bytes32 root);
    ...
    function addSale(
        ...
        emit LogSaleCreated(_sales.length - 1, _start, _finish, _limit, _price, _whitelist, _root);
    }
    ...
    function editSale(
        ...
        emit LogSaleEdited(_saleId, _start, _finish, _limit, _price, _whitelist, _root);
    }
    ...
    function withdrawEther(address _to, uint256 _amount)
        external
        onlyOwner
    {
    ...
    function getMintedAmount(uint256 _saleId, address _user) external view returns (uint256) {
    ...
    function saleMint(
        ...
        if (block.timestamp < sale.start || block.timestamp > sale.finish)
            revert NotInSalePhase(_saleId, sale.start, sale.finish, block.timestamp);
        
        // validate whitelist
        if (sale.whitelist) {
            bytes32 leaf = keccak256(abi.encodePacked(_user));
            if(!_verify(sale.root, _proof, leaf)) revert UserNotWhitelistedOrWrongProof(_saleId, _user, _proof);
        }

        // validate ETH amount send to contract
        if (msg.value != _quantity * sale.price) {
            revert WrongValueSentForMint(_saleId, msg.value, sale.price, _quantity);
        }
        ...
        if (quantityToMint < _quantity) {
            ...
            payable(msg.sender).transfer(sale.price * (_quantity - quantityToMint));
        }
    }
