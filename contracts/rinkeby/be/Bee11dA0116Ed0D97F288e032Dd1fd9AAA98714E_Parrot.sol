...
contract Parrot is ERC20, Ownable {
  ...
  function _swap(uint256 _amountToSwap) private lockSwap {
    ...
    uint256 _balBefore = _usdc.balanceOf(address(this));
    ...
    uint256 balToProcess = _usdc.balanceOf(address(this)) - _balBefore;
    if (balToProcess > 0) {
      uint256 _treasuryUSDC = (balToProcess * _tokensForTreasury) /
        _finalAmountToSwap;
      uint256 _developmentUSDC = (balToProcess * _tokensForDevelopment) /
        _finalAmountToSwap;
      uint256 _liquidityUSDC = balToProcess - _treasuryUSDC - _developmentUSDC;
      _processFees(
        _developmentUSDC,
        _treasuryUSDC,
        _liquidityUSDC,
  ...
  function _addLp(uint256 tokenAmount, uint256 usdcAmount) private {
  ...
  function _processFees(
    ...
    if (_liquidityUSDC > 0 && _liquidityTokens > 0) {
      _addLp(_liquidityTokens, _liquidityUSDC);
    }
  }
