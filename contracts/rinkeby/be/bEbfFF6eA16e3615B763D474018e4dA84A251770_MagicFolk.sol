...
contract MagicFolk is ERC165, ERC721, ERC721Enumerable, Ownable, CommonConstants, IERC1155Receiver, ReentrancyGuard {
    ...
    event Equipped(uint256 indexed _tokenId, uint256 indexed _itemId);
    event Unequipped(uint256 indexed _tokenId, uint256 indexed _itemId);
    event publicSaleToggled();
    event privateSaleToggled();

    //TODO: change calculations for getters etc
    mapping(uint256 => Stats) private _tokenStats; // token id => stats
    ...
    uint256 public constant PUBLIC_PRICE = 0.15 ether;
    address _signer;
    ...
    uint8[MAX_MINT] private _basePowerLevels; // token id => base power level
    ...
    function setMagicFolkGemsContract(address magicFolkGems) public onlyOwner {
    ...
    function togglePublicSaleSigRequired() public onlyOwner {
    ...
    // Power levels will be set after mint but before reveal based on rarity
    ...
    function safeMintPublic(
        ...
        require(qty < PUBLIC_ALLOWANCE);
        require(_tokenIdCounter.current() + qty < MAX_MINT);

        require(
            _verifyMsg(to, 0, msgHash, signature) 
    ...
    function safeMintPrivate(
        ...
        require(
            _verifyMsg(to, privateSaleAllowance, msgHash, signature), 
            "INVALID_SIG"
        );
    ...
    function claimGems(uint256 tokenId) public {
        require(_isApprovedOrOwner(_msgSender(), tokenId));
        uint256 allocation = _calcAllocation(tokenId);
        require(allocation > 0, "No gems available to be claimed yet");
    ...
    function _rollStats(uint256 tokenId) internal {
        //TODO: chainlink VRF etc
        _setBasePowerLevel(tokenId, 1);
    }
    ...
    function getStats(
        uint256 tokenId
    ) public view tokenExists(tokenId) returns (Stats memory) {
    ...
    function getBasePowerLevel(
        uint256 tokenId
    ) public view tokenExists(tokenId) returns (uint8) {
    ...
    function onERC1155Received(
        ...
        require(_operator == _from, "Shady cunt trying to steal someone else's item...");
        require(
        ...
        require(
            ...
            "Sender of item does not own MagicFolkNFT"
        );
    ...
    function unequip(
        ...
        require(ownerOf(nftTokenId) == from); // dev: shady cunt trying to steal someone else's item
    ...
    function onERC1155BatchReceived(
        ...
        bytes calldata
            ...
            revert();
            // require(msg.sender == address(MAGIC_FOLK_ITEMS), "Can only receive MagicFolkItems");
    ...
    function _verifyMsg(
        ...
        return (
            _verifySigner(msgHash, signature) 
            && _hashMsg(sender, allowance) == msgHash
        );
    }
    ...
    function _hashMsg(
        ...
        return keccak256(abi.encodePacked(sender, allowance));
    }
    ...
    function _equip(Item memory _item, uint256 _nftTokenId) internal {
        ...
        emit Equipped(_nftTokenId, _item.itemId);
    }
    ...
    function _isEquipped(
        uint256 nftTokenId, 
        ItemType slot
    ) internal view tokenExists(nftTokenId) returns (bool) {
    ...
    function _getItemStats(
        uint256 tokenId
    ) internal view tokenExists(tokenId) returns (Stats memory) {
    ...
    function _setBasePowerLevel(
        uint256 tokenId, 
        uint8 _powerLevel
    ) internal tokenExists(tokenId) {
        // Stats memory stats = _getStats(tokenId);
        // stats.powerLevel = (
        //     stats.mainHand.powerLevel 
        //     + stats.offHand.powerLevel
        //     + _powerLevel
        //     );
        // _setStats(tokenId, stats);
        _basePowerLevels[tokenId] = _powerLevel;
    }

    function _getBasePowerLevel(
        uint256 tokenId
    ) internal view tokenExists(tokenId) returns (uint8) {
        // Stats memory stats = _getStats(tokenId);
        // return stats.powerLevel 
        //     - stats.mainHand.powerLevel 
        //     - stats.offHand.powerLevel;
    ...
    function _calcAllocation(
        ...
        Stats memory stats = _getStats(tokenId);
        uint256 timeDeltaDays = (
            block.timestamp - _getLastClaim(tokenId)
        ) / SECONDS_PER_DAY;
        
        return timeDeltaDays * stats.powerLevel;
    }

    function _getLastClaim(
        uint256 tokenId
    ) internal view tokenExists(tokenId) returns (uint256) {
        return _lastClaims[tokenId];
    }

    function _setLastClaim(
        uint256 tokenId
    ) internal tokenExists(tokenId) {
    ...
    // For staking/allocation calculations
    // function _calcPowerLevel(
    //     uint256 tokenId
    // ) internal view tokenExists(tokenId) returns (uint8) {
    //     Stats memory stats = _tokenStats[tokenId];
    //     return stats.powerLevel 
...
contract MagicFolkGems is ERC20, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    bytes32 public constant DAO_ROLE = keccak256("DAO_ROLE");

    IERC721 immutable MAGIC_FOLK_CONTRACT;
    IERC1155 immutable MAGIC_FOLK_MAINHAND;
    IERC1155 immutable MAGIC_FOLK_OFFHAND;
    IERC1155 immutable MAGIC_FOLK_PET;
    IGovernor DAO;
    ...
    constructor(
        ...
        // Item contracts have minter roles so users can sell items in future,
        // and burner roles so users can buy items. Will remove minter roles
        // if we don't decide to implement selling to store
        _grantRole(MINTER_ROLE, magicFolkMainhand);
        _grantRole(BURNER_ROLE, magicFolkMainhand);
        MAGIC_FOLK_MAINHAND = IERC1155(magicFolkMainhand);
        
        _grantRole(MINTER_ROLE, magicFolkOffhand);
        _grantRole(BURNER_ROLE, magicFolkOffhand);
        MAGIC_FOLK_OFFHAND = IERC1155(magicFolkOffhand);
        
        _grantRole(MINTER_ROLE, magicFolkPet);
    ...
    function addBurner(address newBurner) public onlyRole(DEFAULT_ADMIN_ROLE) {
...
contract MagicFolkItems is ERC1155, ERC1155Holder, 
                           Ownable, Pausable, ERC1155Supply, ReentrancyGuard {
    using ECDSA for bytes32;
    ...
    ItemType immutable public _itemType;
    ...
    function setMagicFolkGemsAddress(address magicFolkGems) public onlyOwner {
        _magicFolkGems = MagicFolkGems(magicFolkGems);
    }

    function setMagicFolkContractAddress(address magicFolk) public onlyOwner {
    ...
    function setPrice(uint256 itemId, uint256 price) public onlyOwner {
    ...
    function buyCollabItem(
        ...
        require(_isInitialised(itemId), "Item not initialised");
        require(
            amount + _getRedeemed(itemId, tokenId) <= _collabAllowancePerNFT[itemId],
            "TX_WILL_EXCEED_ALLOWANCE"
        );
        require(
            _verifyMsg(to, tokenId, msgHash, signature),
            "INVALID_SIG"
        );
        _safeTransferFrom(address(this), to, itemId, amount, "");
        _magicFolkGems.burn(to, totalPrice);
    ...
    function mint(
        ...
        _mint(address(this), itemId, amount, data);
    }

    function mintCollabItem(
        address nftContract,
        ...
        _addCollabItem(id, nftContract, allowancePerNFT);
    ...
    function decodeItem(
        bytes calldata encodedOwnerIdAndItem
    ) public pure returns (uint256, Item memory) {
