...
contract ParaStaking is Ownable, Mutex, IERC721Receiver {
    ...
    uint256 _totalStake;
    uint256 _totalToken;
    uint256 _baseReward = 625000000 * (10 ** 18);
    ...
    function unstake(uint256 id) external nonReentrant {
        ...
        if(!isUnlocked)
            require(block.timestamp > _stakes[id].startTime + _stakes[id].lockPeriod, "Unstake: unexpired lock");
        require(_token.transfer(_msgSender(), _stakes[id].amount), "Unstake: failed to transfer tokens");

        if(block.timestamp > _rewards[id].updateTime + 30 days)
            _updateReward(id);
    ...
    function updateReward(uint256 id) external {
        ...
        require(block.timestamp > _rewards[id].updateTime + 30 days, "Update: early time");
    ...
    function _updateReward(uint256 id) internal {
        uint monthCnt = (block.timestamp - _rewards[id].updateTime) / 30 days;
        uint256 reward = monthCnt * _calculateReward(id);
        _rewards[id].amount += reward;
        _rewards[id].updateTime += monthCnt * 30 days;
