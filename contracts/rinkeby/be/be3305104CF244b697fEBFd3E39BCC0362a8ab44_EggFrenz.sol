...
import "@openzeppelin/contracts/access/Ownable.sol";
import "./ILockRegistry.sol";

abstract contract LockRegistry is ILockRegistry, Ownable {
    mapping(address => bool) public approvedContract;
    mapping(uint256 => address[]) public locks;

    function isUnlocked(uint256 _id) override public view returns (bool) {
    ...
    function emergencyUnlock(uint256 _id, uint256 pos, address addr) override external onlyOwner { 
        require(locks[_id].length > pos, "Pos invalid");
        require(locks[_id][pos] == addr, "Address invalid");
    ...
    function lock(uint256 _id) override external {
    ...
    function unlock(uint256 _id, uint256 pos) override external {
        require(approvedContract[msg.sender], "Access denied");
    ...
    function findPos(uint256 _id, address addr) override external view returns (uint256)
    {
        for (uint256 i = 0; i < locks[_id].length; i++) {
            if (locks[_id][i] == addr) {
                return i;
