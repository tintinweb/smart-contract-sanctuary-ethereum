...
contract Colander is ReentrancyGuardUpgradeable {
    ...
    function surge(address _collateral, uint256 _auction_id) external auth nonReentrant {
        ...
        IERC20Upgradeable(_collateral).approve(address(dex), amount);
        uint256 amountOut = _swap(_collateral, amount, expectedAmount, block.timestamp + 300);
        IERC20Upgradeable(_collateral).approve(address(dex), 0);
    ...
    // address tokenIn; collateral
    // address tokenOut; stablecoin
    // uint24 fee; 300
    // address recipient; this
    // uint256 deadline; time +300
    // uint256 amountIn; 0
    // uint256 amountOutMinimum; expectedAmount
    // uint160 sqrtPriceLimitX96; 0
