...
contract Drop is ERC721, Ownable {
    ...
    function randomizeMonthlyTraits() public onlyOwner {
        ...
        for (uint i; i < metadata.dynamicChance.length;) {
            uint256 expectation = uint(uint(metadata.dynamicChance[i] * tokenIdCounter) / uint(10000));
