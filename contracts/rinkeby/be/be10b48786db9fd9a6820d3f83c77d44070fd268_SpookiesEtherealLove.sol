...
contract SpookiesEtherealLove is Context, Ownable, AccessControlEnumerable, ERC1155Burnable, ERC1155Pausable {
    ...
    function mint(uint256 amount, uint256 id, uint256 quantity, bytes memory data) external payable {
        ...
        require(amount == (quantity * PRICE), "Incorrect amount for requested quantity");
    ...
    function gift(address account, uint256 tokenId, bytes memory data) external {
    ...
    function specialburn(uint256 quantity, bytes memory data) external {
        ...
        _mint(msg.sender, 3, quantity, data); 
