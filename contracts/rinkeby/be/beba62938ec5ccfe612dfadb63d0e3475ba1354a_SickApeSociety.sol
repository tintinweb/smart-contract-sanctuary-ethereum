...
contract SickApeSociety is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	uint256 public constant SickApe_MAX_AMOUNT = 7777;
	uint256 public constant TEAM_FUND = 1000;
	uint256 public SickApe_PER_TX = 15;
	uint256 public SickApe_PRICE = 0.0000001 ether;
	uint256 public constant SickApe_TIME_FOR_GROWN = 14 days;

	uint256 public mintedPresaleCount;

	mapping (uint256 => uint256) private _SickApeBirthdays;
	...
	function mintPublic(uint256 amount) external payable nonReentrant() {
		...
		require(amount <= SickApe_PER_TX, "You can't mint that many tokens");
		require(totalSupply() + amount <= SickApe_MAX_AMOUNT, "Mint would exceed max supply of tokens");
		require(msg.value == amount * SickApe_PRICE, "You didn't send the right amount of eth");
	...
	function _mintMultiple(address owner, uint256 amount) private {
		for (uint256 i = 0; i < amount; i++) {
			...
			_SickApeBirthdays[tokenId] = block.timestamp;
	...
	function enableClaimFoundingMembers(address[] memory members, uint256[] memory amounts) external onlyOwner {
		...
		for (uint256 i = 0; i < members.length; i++) {
			...
			require(amounts[i] <= SickApe_PER_TX, "amount is more than the allowable");
	...
	function setIpfsHash(string[] memory ipfsHashs, uint256[] memory idTokens) external onlyOwner {
		...
		for (uint256 i = 0; i < idTokens.length; i++) {
			...
			require(idTokens[i] <= SickApe_MAX_AMOUNT, "id is more than the allowable");
	...
	function makePresaleMemberMint(uint256 tokenAmount, uint256 etherAmount) private {
		...
		require(totalSupply() + tokenAmount <= SickApe_MAX_AMOUNT, "Mint would exceed max supply of tokens");
		require(mintedPresaleCount + tokenAmount <= SickApe_MAX_AMOUNT, "Mint would exceed max pre-sale tokens");
		require(etherAmount == tokenAmount*SickApe_PRICE, "You didn't send the right amount of eth");
	...
	function setEntityPrice(uint256 newPrice) public onlyOwner {
		...
		emit SetNewValue("SickApe_PRICE", SickApe_PRICE, newPrice);
		SickApe_PRICE = newPrice;
	}

	function setEntityPerTx(uint256 newPerTx) public onlyOwner {
		require(newPerTx > 0, "Wrong per tx");
		emit SetNewValue("SickApe_PER_TX", SickApe_PER_TX, newPerTx);
		SickApe_PER_TX = newPerTx;
	}
	...
	function getBirthday(uint256 tokenId) public view returns (uint256) {
		require(tokenId < totalSupply(), "That SickApe has not been make yet");
		return _SickApeBirthdays[tokenId];
	}

	function getIpfsTokenUri(uint256 tokenId) external view returns (string memory) {
		require(tokenId < totalSupply(), "That SickApe has not been make yet");
		require(getBirthday(tokenId) < block.timestamp + SickApe_TIME_FOR_GROWN, "That SickApe has not grown yet");
