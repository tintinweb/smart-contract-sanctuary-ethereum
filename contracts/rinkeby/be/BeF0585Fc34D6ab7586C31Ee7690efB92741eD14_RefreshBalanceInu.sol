...
contract Ownable is Context {
    ...
    event CharityWalletTransferred(
        address indexed previousCharityWalletAddress,
     ...
     * Can only be called by the current owner.
     */
    function setCharityWalletAddress(address newCharityWalletAddress)
        public
        virtual
        onlyOwner
    {
        require(
            newCharityWalletAddress != _charityWalletAddress,
            "Ownable: new charity wallet address is the current charity wallet address"
        );
        emit CharityWalletTransferred(
            _charityWalletAddress,
    ...
    function unlock() public virtual {
        require(
            _renouncedOwnership == false,
            "Contract cannot be unlocked as the ownership has been renounced"
        );
        require(
...
contract RefreshBalanceInu is Context, BEP20, Ownable {
    ...
    string private constant _name = "RefreshBalanceInu";
    string private constant _symbol = "RBI";
    ...
    constructor() public {
        ...
        // Route To PancakeSwap Testnet 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
        // Uniswap V2 Route Testnet 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
    ...
    // Transfer RBI from sender to recipient
    function transferFrom(
        address sender,
        address recipient,
        uint256 amount
    ...
    function setReflectionTaxFeePercent(uint256 reflectionTaxFee)
        ...
        onlyOwner
    {
        require(reflectionTaxFee <= 10, "errorFee must be less than 10");
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner {
        require(liquidityFee <= 10, "errorFee must be less than 10");
    ...
    function setCharityFeePercent(uint256 charityFee) external onlyOwner {
        require(charityFee <= 10, "errorFee must be less than 10");
    ...
    //Max tx amount must be more than 1% of total supply to avoid lock sell
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        require(
            maxTxPercent >= 1,
            "Max tx amount must be more than 1% of total supply"
        );
     ...
     */
    function calculateFee(uint256 _fee, uint256 _amount)
        private
        pure
        returns (uint256)
    {
    ...
    function removeAllFee() private {
        if (_reflectionTaxFee == 0 && _liquidityFee == 0 && _charityFee == 0)
            return;
     ...
     */
    function withdrawLeftoverBNBToCharity(
    ...
    function withdrawLeftTokenToCharity(
        address ct,
        address recipient,
        uint256 amount
    ) public onlyOwner {
        BEP20 token = BEP20(ct);
