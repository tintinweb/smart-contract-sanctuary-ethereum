...
contract CarNFT is ERC721, Ownable {
    ...
    address public RECEIVER_WALLET;
    ...
    uint256 public reservedSupply = 600;

    address public openSeaProxyRegistryAddress;

    bool public isOpenSeaProxyActive = true;

    constructor(uint256 _supply, uint256 _price, address _receiver, uint256 _revealTimeStamp, address _openSeaProxyRegistryAddress) ERC721('Car', 'CARR') {    
        MAX_SUPPLY = _supply;
        SALE_PRICE = _price;
    ...
    /**
     * Reserved mint for the contract owner
     */
    function mintReserved(uint256 _numberOfTokens) external onlyOwner {
        ...
        if (reservedSupply < _numberOfTokens) {
            reservedSupply = 0;
            return;
        }

        reservedSupply -= _numberOfTokens;
    }

    /**
     * Mint
     */
    function mint(uint256 _numberOfTokens, bytes32[] memory proof) external payable onlyExternal {
        require(isPublicSaleActive || isPresaleActive, 'Sale must be active in order to mint');
        require(_numberOfTokens <= MAX_PURCHASE, 'Can only mint 20 tokens at a time');
        require(tokenCount + _numberOfTokens <= MAX_SUPPLY - reservedSupply, 'Minting would exceed total supply of tokens');
        ...
        if (isPresaleActive) {
            require(
                ...
                'Address is not on the presale list'
            );
        }

        payable(RECEIVER_WALLET).sendValue(msg.value);
        ...
        if (startingIndexBlock == 0 && (tokenCount == MAX_SUPPLY || block.timestamp >= revealTimeStamp)) {
            startingIndexBlock = block.number;
        }
    }

    function setSaleState(bool _isPresaleActive, bool _isPublicSaleActive) external onlyOwner {
