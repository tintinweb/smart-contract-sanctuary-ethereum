...
contract BluechipsBooster is IBluechipsBooster, ERC721, AdminUtility, TerminalUtility {
    ...
    function createCustomBooster(
        ...
        for (uint256 i = 0; i < _bluechips.length; i++) {
            if (_bluechips[i] == address(0)) revert ZeroAddress();
            if (multiplierOf[_bluechips[i]] > 0) revert BoosterRegisterd();
    ...
    function createProof(address _bluechip, uint256 _tokenId) external payable override {
        ...
        bytes32 _proof = keccak256(abi.encode(_bluechip, _tokenId));

        address _preOwner = proofBy[_proof];
        if (_preOwner == _from) revert RenewFirst();

        if (_preOwner != address(0)) {
            _redeem(_bluechip, _proof, _from, _preOwner);
        }
    ...
    function createProof(
        ...
        bytes32 _proof = keccak256(abi.encode(_projectId, _bluechip, _tokenId));

        address _preOwner = proofBy[_proof];
        if (_preOwner == _from) revert RenewFirst();

        if (_preOwner != address(0)) {
            _redeem(_bluechip, _proof, _from, _preOwner, _projectId);
        }
    ...
    function challengeProof(address _bluechip, uint256 _tokenId) external override {
        ...
        _redeem(_bluechip, _proof, msg.sender, _creator);
    ...
    function challengeProof(
        ...
        _redeem(_bluechip, _proof, msg.sender, _creator, _projectId);
    ...
    function redeemProof(address _bluechip, uint256 _tokenId) external override {
        ...
        _redeem(_bluechip, _proof, _from, _from);
    ...
    function redeemProof(
        ...
        _redeem(_bluechip, _proof, _from, _from, _projectId);
    ...
    function _redeem(
        ...
        uint256 _tokenId = tokenIdOf[_proof];
        _burn(_tokenId);
        delete tokenIdOf[_proof];

        delete proofBy[_proof];

        emit Remove(_from, _beneficiary, _proof, boosterWeights[_from]);
    }
