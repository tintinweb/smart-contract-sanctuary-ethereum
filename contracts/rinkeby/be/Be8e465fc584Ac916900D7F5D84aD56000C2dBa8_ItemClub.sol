...
import "../../base/ItemBase.sol";


contract ItemClub is ItemBase {
  ...
  string internal constant _NAME = "Club";
  string internal constant _SYMBOL = "CLUB";
  uint private constant _LEVEL = 1;
  IItem.ItemType private constant _TYPE = IItem.ItemType.ONE_HAND;
  string private constant _URI = "https://raw.githubusercontent.com/tetu-io/tetu-game-assets/master/items/1/club.png";
  ...
  function init(
    ...
    __ItemBase_init(
      ...
      augmentTokenAmount_,
      _LEVEL,
      IItem.ItemType.ONE_HAND
    );
  }

  function _generateAttributes()
  internal override returns (IStatController.Attributes memory, ItemRarity) {

    uint size = 7;
    uint sum;
    uint[] memory randomNumbers = new uint[](size);
    for (uint i; i < size;) {
      randomNumbers[i] = _oracle().getRandomNumber(1e18);
      sum += randomNumbers[i];
    unchecked{++i;}
    }

    IStatController.Attributes memory attributes = IStatController.Attributes({
    strength : _randomAttribute(0, 5, 5e16, randomNumbers[0]),
    dexterity : _randomAttribute(0, 5, 5e16, randomNumbers[1]),
    vitality : _randomAttribute(0, 5, 5e16, randomNumbers[2]),
    energy : _randomAttribute(0, 5, 5e16, randomNumbers[3]),
    damageMin : _randomAttribute(5, 5, 1e18, randomNumbers[4]),
    damageMax : _randomAttribute(15, 15, 1e18, randomNumbers[5]),
    attackRating : _randomAttribute(0, 300, 5e17, randomNumbers[6]),
    defense : 0,
    blockRating : 0,
    life : 0,
    mana : 0,
    ...
    return (attributes, _calculateRarity(sum / size));
  }
...
abstract contract ItemBase is NftBase, IItem {
  ...
  function onlyDungeon() internal view {
    require(IController(_controller()).validDungeons(msg.sender), "I: Not dungeon");
  }

  function onlyOwner(uint tokenId) internal view {
    require(ownerOf(tokenId) == msg.sender, "I: Not owner");
  }
  ...
  function mint() external override returns (uint tokenId) {
    ...
    ItemRarity rarity;
    (_itemAttributes[tokenId], rarity) = _generateAttributes();
  ...
  function takeOff(uint tokenId, address heroToken, uint heroTokenId, uint itemSlot) external {
    onlyOwner(tokenId);
    require(IHero(heroToken).isOwner(address(this), heroTokenId), "Not hero owner");
    require(IHero(heroToken).currentDungeon(heroTokenId) == address(0), "Hero in dungeon");
