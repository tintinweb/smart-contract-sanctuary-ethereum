...
constructor(address _priceFeed, Creators _creatorsContract, Subscriptions _subscriptionsContract, Content _contentContract) {
...
function getSubscriptionByCreatorAddress(address _creator, address _follower) public view  returns(address, address, uint, bool){
    ...
    function getContent(uint256 _contentID, bytes memory _signature, string memory _message) public view returns (string memory,
        string memory,
        uint256,
        string memory) {
    ...
    function getContractBalance(bytes memory _signature, string memory _message) public view returns(uint256){
    ...
    function getCreatorBalance(bytes memory _signature, string memory _message) public view returns(uint256){
...
contract Subscriptions {
    ...
    constructor(address _priceFeed, Creators _creatorsContract) {
        creators_ref = _creatorsContract;
        s_priceFeed = AggregatorV3Interface(_priceFeed);
        owner = msg.sender;
...
contract Content {
    ...
    constructor(Creators _creatorsContract, Subscriptions _subscriptionsContract) {
        creators_ref = _creatorsContract;
        subscriptions_ref = _subscriptionsContract;
        owner = msg.sender; 
    }
...
library Signature {
    ...
    function getSignature(string memory _phrase, bytes memory _signature) 
    internal 
    pure 
    returns (address){

        require(_signature.length == 65, "invalid signature length");
            bytes32 prefixedHashMessage = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(_phrase))));
            bytes32 r;
            bytes32 s;
            uint8 v;
            assembly {
