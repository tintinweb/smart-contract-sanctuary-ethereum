...
contract HumansFactoryPass is ERC1155Supply, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    uint256 public constant HUMANS_FACTORY_PASS_ID = 0;
    uint256 public constant HUMANS_FACTORY_OG_PASS_ID = 1;
    ...
    uint256[] private _teamShares = [10000, 19475, 24475];

    mapping(uint256 => saleParams) public sales;
    mapping(string => mapping(address => uint256)) public claimsPerWallet;
    ...
    ) ERC1155("ipfs://QmdtARLUPQeqXrVcNzQuRqr9UCFoFvn76X9cdTczt4vqfw/") PaymentSplitter(_team, _teamShares) {
        name_ = _name;
        symbol_ = _symbol;
    }
    ...
    function configureSale(
        ...
        require(_startTime > 0 && _endTime > 0, "HF: Times must be greater than zero.");
    ...
    function saleMint(
        ...
        require(_sale.startTime > 0 && _sale.endTime > 0, string(abi.encodePacked("HF: Sale '", _saleId, "'doesn't exists")));
        require(_sale.tokenId == _tokenId, "HF: Wrong token ID");
        ...
        require(claimsPerWallet[_sale.name][_msgSender()] + _nb <= alloc, "HF: Allocation exceeded.");
        require(_nb * _sale.price >= msg.value, "HF: Insufficient amount.");
        ...
        claimsPerWallet[_sale.name][_msgSender()] += _nb;
        _mint(_msgSender(), _tokenId, _nb, "");
        emit TokensMinted(_msgSender(), _nb, _tokenId, _sale.name);
    }

    // PUBLIC
    function maxSupplyOf(uint256 _tokenId) public pure returns (uint256) {
        if (_tokenId == 0) return 10000;
        else if (_tokenId == 1) return 150;
        else return 0;
    }
