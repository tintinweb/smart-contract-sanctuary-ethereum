...
contract CoinStaking is Ownable, ReentrancyGuard, IStaking {
    ...
    constructor(
        ...
        // require(_tokenPoolAllocPoint > 0 && _lpPoolAllocPoint > 0 , "WRONG ALLOCATION POINT");
        ...
        returns (uint256)
    {
        ...
        if (currentTime > pool.lastRewardTime && supply != 0) {
            ...
            console.log(
                "-s-multiplier(%s) = block.timestamp(%s) - pool.lastRewardTime(%s)",
                multiplier,
                currentTime,
                pool.lastRewardTime
            );
            console.log(
                "-s-reward = multiplier(%s)*rewardPerSecond(%s)*pool.allocPoint(%s)/totalAllocPoint",
                multiplier,
                rewardPerSecond,
                pool.allocPoint
            );
        ...
        console.log(
            "-s-user.amount(%s)*accRewardPerShare(%s)/1e12 - user.rewardDebt",
            user.amount,
            accRewardPerShare,
            user.rewardDebt
        );
        ...
        nonReentrant
    {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                rewardBalance -= pending;
                pool.token.transfer(sender, pending);
            }
        }
        // get changed amount
    ...
    function inCaseTokensGetStuck(address _token) external onlyOwner {
        require(
            address(poolInfo[0].token) != _token &&
                address(poolInfo[1].token) != _token,
