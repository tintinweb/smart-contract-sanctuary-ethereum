...
contract CyberPhunks is Ownable, ERC721A {
    ...
    function punkAttributesAsJSON(uint16 punkId) public view returns (string memory json) {
        ...
        outputBytes.appendSafe(abi.encodePacked(
            '{"trait_type":"Color Count", "value":"', getPhunkColorCount(phunk.id).toString(), '"},'
        ));
        
        outputBytes.appendSafe(abi.encodePacked(
            '{"trait_type":"Trait Count", "value":"', attrCount.toString(), '"},'
        ));
        
        outputBytes.appendSafe(abi.encodePacked(
            '{"trait_type":"Glowing Trait Count", "value":"', litAttrCount.toString(), '"},'
        ));
        
        outputBytes.appendSafe(abi.encodePacked(
            '{"trait_type":"Rarity Rank", "value":"', getPhunkRarityRank(phunk.id).toString(), '"}'
        ));
