...
import "./TODInterface.sol";

contract TheOddDystrictPortals is ERC721Enum, Ownable, PaymentSplitter, Pausable,  ReentrancyGuard {
    using Strings for uint256;
    string public baseURI;
    TODInterface private immutable todinterface;

    //sale settings
    uint256 public cost = 0.00 ether;
    uint256 public maxSupply = 8000;
    uint256 public maxClaimSupply = 5000;
    ...
    constructor(string memory name, string memory symbol, address TODAddress) ERC721P(name, symbol) PaymentSplitter(addressList, shareList){
        todinterface = TODInterface(TODAddress);
    }
    ...
    // public minting
    function multiMintWithDoor(uint256[] memory TODIds) public nonReentrant {
        uint256 s = totalSupply();
        for (uint256 i = 0; i < TODIds.length; ++i) {
            require(TODIds[i] >= 1 && TODIds[i] <= 5000, "Token ID invalid");
            require(todinterface.ownerOf(TODIds[i]) == msg.sender, "Not the owner of this TOD");
    ...
    function mint(uint256 _mintAmount) public payable nonReentrant{
        ...
        require(publicMinted + _mintAmount <= maxPublicSupply, "Cant go over public supply");
    ...
    function gift(uint[] calldata quantity, address[] calldata recipient) external onlyOwner{
        ...
        require(publicMinted + totalQuantity <= maxPublicSupply, "Cant go over supply");
        for(uint i = 0; i < recipient.length; ++i){
            for(uint j = 0; j < quantity[i]; ++j){
            _safeMint( recipient[i], s++, "");
            }
        }
        publicMinted += totalQuantity;
	...
	function setMaxClaimSupply(uint256 _newMaxSupply) public onlyOwner {
...
abstract contract TODInterface {
