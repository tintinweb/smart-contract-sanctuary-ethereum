...
contract Sale is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    function _checkWinner(uint256 saleId) private {
        require(winners[saleId][_msgSender()], "Invalid winner");
    }

    function _checkManagerWithProjectId(uint256 projectId) private {
    ...
    function _checkOnSale(uint256 _saleId) private {
    ...
    function setContracts(address _project) external {
    ...
    function setController(address _account, bool _allow) external {
        ...
        require(_allow != controllers[_account], "Duplicate setting");
        controllers[_account] = _allow;
    }

    function setWinners(uint256 _saleId, address[] memory _accounts, bool _isWinner) external {
        require(controllers[_msgSender()], "Caller is not the controller");
        require(_saleId > 0 && _saleId <= lastedId, "Invalid sale id");

        for (uint256 i; i < _accounts.length; i++) {
            require(_accounts[i] != address(0), "Invalid account");
            require(_isWinner != winners[_saleId][_accounts[i]], "Duplicate setting");
            winners[_saleId][_accounts[i]] = _isWinner;
        }
    }

    function createRaiseSingle(uint256 _projectId, uint256[] memory _tokenIds, uint256[] memory _raisePrices) external payable {
        _checkManagerWithProjectId(_projectId);
        require(_tokenIds.length > 0 && _tokenIds.length == _raisePrices.length, "Invalid param");
        ProjectInfo memory _project = project.getProject(_projectId);
        require(_project.status == ProjectStatus.INACTIVE, "Invalid Status");
        require(_project.isSingle, "Invalid token");
        require(_project.isRaise, "Invalid auction");
    ...
    function createDutchSingle(uint256 _projectId, uint256[] memory _tokenIds, uint256[] memory _maxPrices, uint256[] memory _minPrices, uint256[] memory _priceDecrementAmts) external {
        _checkManagerWithProjectId(_projectId);
    ...
    function createRaiseMulti(uint256 _projectId, uint256[] memory _tokenIds, uint256[] memory _amounts, uint256[] memory _raisePrices) external {
        _checkManagerWithProjectId(_projectId);
        require(_tokenIds.length > 0 && _tokenIds.length == _amounts.length && _tokenIds.length == _raisePrices.length, "Invalid param");
        ProjectInfo memory _project = project.getProject(_projectId); 
        require(_project.status == ProjectStatus.INACTIVE, "Invalid status");
        require(!_project.isSingle, "Invalid token");
        require(_project.isRaise, "Invalid auction");
    ...
    function createDutchMulti(uint256 _projectId, uint256[] memory _tokenIds, uint256[] memory _amounts, uint256[] memory _maxPrices, uint256[] memory _minPrices, uint256[] memory _priceDecrementAmts) external {
        _checkManagerWithProjectId(_projectId);
    ...
    function _createSale(uint256 saleId, ProjectInfo memory _project, uint256 tokenId, uint256 amount, uint256 raisePrice, uint256 maxPrice, uint256 minPrice, uint256 priceDecrementAmt) private returns(SaleInfo memory) {
        ...
        if (!_project.isRaise) {
            require(maxPrice > minPrice && minPrice > 0, "Invalid price");
            require(priceDecrementAmt > 0 && priceDecrementAmt <= maxPrice - minPrice, "Invalid price");
        }

        uint256 tokenAmount = project.getProject(_project.id).amount;
        tokenAmount += _project.isSingle ? 1 : amount;
        project.setTokenAmount(_project.id, tokenAmount);
        ...
        if (_project.isSingle) IOSB721(_project.token).safeTransferFrom(_msgSender(), address(this), tokenId);
        else IOSB1155(_project.token).safeTransferFrom(_msgSender(), address(this), tokenId, amount, "");
        return sale;
    }

    function bidSingle(uint256 _saleId) external payable {
        _checkWinner(_saleId);
        _checkOnSale(_saleId);
        ...
        uint256 _price = 0;
        if (_project.isRaise) {
            _price = sale.raisePrice;
            require(msg.value == _price, "Invalid value");
        } else {
            _price = currentDutchPrice(_saleId);
            require(msg.value >= _price, "Invalid value");
        }

        sale.isSoldOut = true;
        uint256 soldAmount = _project.sold;
        soldAmount += 1;
        ...
        _sharing(_project, sale, 0, _price);
        if (_project.minSales == 0 && _project.isInstantPayment) IOSB721(_project.token).safeTransferFrom(address(this), _msgSender(), sale.tokenId);
    }

    function bidMulti(uint256 _saleId, uint256 _amount) external payable {
        _checkWinner(_saleId);
        _checkOnSale(_saleId);
        SaleInfo storage sale = sales[_saleId];
        ProjectInfo memory _project = project.getProject(sale.projectId);
        ...
        require(!_project.isSingle, "Invalid token");
        require(_amount > 0 && _amount <= sale.amount, "Invalid amount");
        uint256 _price = 0;
        if (_project.isRaise) {
            _price = sale.raisePrice * _amount;
            require(msg.value == _price, "Invalid value");
        } else {
            _price = currentDutchPrice(_saleId) * _amount;
            require(msg.value >= _price, "Invalid value");
        }
        ...
        if (_project.isInstantPayment && _project.sold == _project.amount) project.end(_project.id);
    ...
    function _sharing(ProjectInfo memory _project, SaleInfo memory _sale, uint256 _amount, uint256 _price) private {
        ...
        if (_project.minSales == 0 && _project.isInstantPayment) {
            ...
            if (supperAdminProfit > 0) Helper.safeTransferNative(project.getSuperAdmin(), supperAdminProfit);
            ...
            billInfo.supperAdminFee += supperAdminProfit;
    ...
    function closeProject(uint256 _projectId, bool _isGive) external nonReentrant {
        _checkManagerWithProjectId(_projectId);
        ProjectInfo memory _project = project.getProject(_projectId);
        ...
        for (uint256 i; i < saleIdsOfProject[_projectId].length; i++) {
            SaleInfo memory saleInfo = sales[saleIdsOfProject[_projectId][i]];     
            address[] memory accountBuyers = buyers[saleIdsOfProject[_projectId][i]];
            if (accountBuyers.length == 0) {
                if (_project.isSingle) IOSB721(_project.token).safeTransferFrom(address(this), _project.manager, saleInfo.tokenId);
                else IOSB1155(_project.token).safeTransferFrom(address(this), _project.manager, saleInfo.tokenId, saleInfo.amount, "");
                continue;
            }
            if (saleInfo.amount > 0) IOSB1155(_project.token).safeTransferFrom(address(this), _project.manager, saleInfo.tokenId, saleInfo.amount, "");
            _close(_project, saleInfo, accountBuyers, _isGive);
        }
        project.end(_projectId);
        emit CloseProject(_projectId, _isGive);
    }

    function _close(ProjectInfo memory _project, SaleInfo memory _sale, address[] memory _accountBuyers, bool _isGive) private {
        for (uint256 i; i < _accountBuyers.length; i++) {
            if (bills[_sale.id][_accountBuyers[i]].account == _accountBuyers[i]) {
                ...
                uint256 profitShare = billInfo.royaltyFee + billInfo.supperAdminFee + billInfo.sellerFee;
                ...
                if (moneyPaid > 0) Helper.safeTransferNative(billInfo.account, moneyPaid);
                    ...
                    Helper.safeTransferNative(project.getSuperAdmin(), billInfo.supperAdminFee);
                    Helper.safeTransferNative(project.getManager(_project.id), billInfo.sellerFee);
                }

                address receiver;
                if (_isGive || !_project.isInstantPayment && _project.sold >= _project.minSales) receiver = billInfo.account;
                else if (_project.minSales > 0 && _project.sold < _project.minSales || !_project.isInstantPayment) {
                    receiver = _project.manager;
                }
