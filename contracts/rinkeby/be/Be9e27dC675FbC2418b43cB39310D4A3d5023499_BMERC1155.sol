    ...
    IERC2981
{
    ...
    function changeURI(uint256 tokenId, string memory tokenCID)
        public
        onlyOwner
    {
    ...
    function setTokenCID(uint256 tokenId, string memory cid) public isExchange {
    ...
    function mint(
        ...
        string memory cid,
        bytes memory data
    ) public isExchange nonReentrant returns (bool) {
        require(
            amount > 0,
            "BleemNFTERC1155:mint: Token amount cannot be zero!"
        );
        require(
            !exists(tokenId),
            "Err: Token Id exists and cannot mint repeatedly"
        );
    ...
    function mintBatch(
        ...
        bytes memory data
    ) public isExchange returns (bool) {
        require(
            tokenIds.length == amounts.length,
            "BleemNFTERC1155:mintBatch: tokenIds and amounts not match"
        );
        ...
        canBurnNFT
    {
        require(
            ...
            "BleemNFTERC1155:burnBatch: Amount id list and token id list are inequal!"
        );
    ...
    {
        (receiver, royaltyAmount) = BleemIERC2981(_royaltiesContract)
            .royaltyInfo(address(this), _tokenId, _salePrice);
    }

    address public _royaltiesContract;

    function setRoyaltiesContract(address royaltiesContract_)
        external
        onlyOwner
    {
...
contract BMBase is Ownable {
    ...
    function setExchange(address contractAddress, bool flag)
        external
        onlyOwner
    {
        _exchanges[contractAddress] = flag;
    }
    ...
    modifier isExchange() {
        require(
            _exchanges[_msgSender()],
            "Err: You do not have permission to behave the action."
        );
        _;
    }
    ...
    function settleCoin(address to) external onlyOwner {
    ...
    function settleToken(address tokenAddress, address to) external onlyOwner {
