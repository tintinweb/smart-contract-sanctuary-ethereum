...
contract DeepZikko is ERC721A, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  uint8 public whitelistMaxBuy = 10;
  ...
  uint256 public mintPrice = 0.059 ether;
  uint256 public lastPrice = 0.079 ether;

  // string public baseURI = 'https://zikko.xyz/api/metadata/';
  string public baseURI = "ipfs://QmSv5NA8XGWY5k3WxcqzNDeeXSwhxeazPBNP45y4NR9ieE/";
  address private contractOwner;

  mapping(address => uint8) public walletBuys;

  event Mint(address indexed _to, uint256 _amount);
  event Received(address _address, uint256 _value);

  receive() external payable {
    emit Received(msg.sender, msg.value);
  }

  constructor() ERC721A("DeepZikko", "DZKK") {
  ...
  function batchMint(uint256 _mintAmount) public onlyOwner nonReentrant {
    require(totalSupply() + _mintAmount < maxSupply + 1, "too many!");
    _safeMint(msg.sender, _mintAmount);
  }

  function giveaway(address[] calldata _addresses, uint256 quantity) public onlyOwner nonReentrant {
    for (uint256 i = 0; i < _addresses.length; i++) {
      _safeMint(_addresses[i], quantity);
    }
  }

  function publicMint(uint8 _mintAmount) public payable nonReentrant {
    require(!paused, "This contract is paused");
    require(msg.sender == tx.origin, "Be yourself");
    require(_mintAmount > 0, "Need to mint at least 1 NFT");
    require(totalSupply() + _mintAmount <= maxSupply, "You can't mint more then the total supply");
  ...
  function presaleMint(uint8 _mintAmount) public payable nonReentrant {
    require(!paused, "This contract is paused");
    require(msg.sender == tx.origin, "Be yourself");
    require(_mintAmount > 0, "Need to mint at least 1 NFT");
    require(totalSupply() + _mintAmount <= maxSupply, "You can't mint more then the total supply");
  ...
  function whitelistMint(uint8 _mintAmount, bytes32[] calldata _merkleProof) public payable nonReentrant {
    require(!paused, "This contract is paused");
    require(msg.sender == tx.origin, "Be yourself");
    require(_mintAmount > 0, "Need to mint at least 1 NFT");
    require(totalSupply() + _mintAmount <= maxSupply, "You can't mint more then the total supply");
    require(walletBuys[msg.sender] + _mintAmount <= whitelistMaxBuy, "Buy limit reached");
  ...
  function setWhitelistMaxBuy(uint8 _maxBuy) external onlyOwner {
    whitelistMaxBuy = _maxBuy;
  }

  function updateMerkleRootHash(bytes32 _rootHash) external onlyOwner {
