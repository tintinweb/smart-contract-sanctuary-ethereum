...
contract GameToken is ERC721Enumerable, Ownable {
    ...
    uint256 public priceIncrement = 0.0025 ether;
    ...
    function hashTransaction(address sender, uint256 qty, string memory nonce, string memory list, uint256 timestamp) private pure returns(bytes32) {
      bytes32 hash = keccak256(abi.encodePacked(
        "\x19Ethereum Signed Message:\n32",
        keccak256(abi.encodePacked(sender, qty, nonce, list, timestamp)))
      );
    ...
    function whitelistMint(bytes32 hash, bytes memory signature, string memory nonce, uint256 tokenQuantity, string memory list, uint256 timestamp) external payable {
        uint price = 0;
        ...
        require(hashTransaction(msg.sender, tokenQuantity, nonce, list, timestamp) == hash, "HASH_FAIL");
    ...
    function mint(bytes32 hash, bytes memory signature, string memory nonce, uint256 tokenQuantity, string memory list, uint256 timestamp) external payable {
        ...
        require(price * tokenQuantity <= msg.value, "Ether value sent is not enough");
