...
contract ConfettiRoll is AccessControlEnumerable, Ownable, Pausable {
    ...
    IConfetti immutable public confetti;
    RpSeeder immutable public seeder;
    ...
    mapping(address => EnumerableSet.Bytes32Set) pendingRewards;

    constructor(IConfetti confetti_, address seeder_, address treasury_) {
    ...
    function setTreasuryFee(uint256 treasuryFee_ ) public onlyRole(TREASURY_ROLE) {
    ...
    function setBets(uint256 min, uint256 max, uint256 default_) public onlyOwner {
        minBet = min;
        maxBet = max;
        defaultBet = default_;
    }

    function setStartingRolls(uint256 min, uint256 max, uint256 default_) public onlyOwner {
        minStartingRoll = min;
        maxStartingRoll = max;
    ...
    function getGameResults(bytes32 gameId) public view returns (GameResult memory) {
    ...
    function getPendingRewards(address player) public view returns (uint256) {
        uint256 sum = 0;
        for (uint256 i = 0; i < pendingRewards[player].length(); i++) {
            bytes32 gameId = pendingRewards[player].at(i);
    ...
    function withdrawRewards() public whenNotPaused {
        ...
        for (uint256 i = 0; i < pendingRewards[msg.sender].length();) {
            bytes32 gameId = pendingRewards[msg.sender].at(i);
            if (isGameFinished(gameId)) {
                rewards += gameResults[gameId].prizeShare;
                pendingRewards[msg.sender].remove(gameId);
    ...
    // Given a game result, return an index of a player who lost
    ...
    function calcGameId(address initializer, uint256 roundNum) public pure returns(bytes32) {
    ...
    function createGame(address initializer, uint256 poolBet, uint256 startingRoll, uint256 roundNum) public whenNotPaused returns(bytes32) {
        if (startingRoll == 0) {
        ...
        require(poolBet >= minBet && poolBet <= maxBet, "Bet outside legal range");
        require(startingRoll >= minStartingRoll && startingRoll <= maxStartingRoll, "Start roll outside legal range");
        ...
        require(
            ...
            (initializer == address(this) && poolBet == defaultBet && startingRoll == defaultStartingRoll)
        );
    ...
    function joinGame(bytes32 gameId) public whenNotPaused {
        ...
        pendingRewards[msg.sender].add(gameId);
    ...
    function joinGlobalGame() public whenNotPaused returns(bytes32) {
        ...
        if (games[globalGameId].startingRoll == 0) {
            createGame(address(this), defaultBet, defaultStartingRoll, currentRound());
        }
        joinGame(globalGameId);
        return globalGameId;
    }

    function shuffledPlayers(address[] memory players, uint256 seed) public pure returns (address[] memory) {
        ...
        for (uint i = 0; i < players.length; i++) {
            ...
            pick = uint256(keccak256(abi.encodePacked(players[i], seed))) % (players.length - i);
            temp = shuffled[i];
    ...
    function simulateGame(bytes32 gameId) internal onlyOwner whenNotPaused {
        ...
        uint roll = game.startingRoll;
        while (roll > 0) {
            roll = uint256(keccak256(abi.encodePacked(rolls.length, seed))) % roll;
            rolls.push(roll + 1);
        }
    }
    ...
    function commenceGame(bytes32 gameId) public whenNotPaused {
        ...
        pendingRewards[loser].remove(gameId);
        ...
        uint256 prizePool = payableBet - treasuryShare;

        confetti.transfer(treasury, treasuryShare);
        ...
        result.prizeShare = prizePool / (result.players.length - 1);
    }
