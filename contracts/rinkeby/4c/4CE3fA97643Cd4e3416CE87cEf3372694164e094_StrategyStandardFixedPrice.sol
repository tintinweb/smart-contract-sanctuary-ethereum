...
contract StrategyStandardFixedPrice is ISynchronousStrategy {
        ...
        )
    {
        ...
        return (
            (bidPrice == askPrice &&
                ...
                makerBid.validFrom <= block.timestamp &&
                makerBid.validUntil >= block.timestamp),
            makerBid.tokenId,
            takerAsk.amount,
    ...
    {
        uint256 askPrice = abi.decode(makerAsk.strategyParams, (uint256));
        uint256 bidPrice = abi.decode(takerBid.strategyParams, (uint256));
        return (
            (askPrice == bidPrice &&
                makerAsk.tokenId == takerBid.tokenId &&
                makerAsk.validFrom <= block.timestamp &&
                makerAsk.validUntil >= block.timestamp),
            makerAsk.tokenId,
            takerBid.amount,
            bidPrice * takerBid.amount
        );
    }
