...
// File: contracts/FoundationVest.sol
...
contract FoundationVest is Ownable {
    ...
    uint256 public constant totalAllocation = 1.5 * (10**8) * (10**18);

    uint256 private month = 1 minutes;

    uint256 public constant vestingStages = 5;
    /**
    ...
    function lock(uint256 unLockTime) internal notLocked onlyOwner {
        ...
        stages[0].period = 1;
        stages[0].periodPercentage = 2000;

        //Need to release 32%
        stages[1].startDate = unLockedAt;
        stages[1].period = 12;
        stages[1].periodPercentage = 267;

        //Need to release 24%
        stages[2].startDate = unLockedAt + month * 12;
        stages[2].period = 12;
        stages[2].periodPercentage = 200;

        //Need to release 16%
        stages[3].startDate = unLockedAt + month * 24;
        stages[3].period = 12;
        stages[3].periodPercentage = 133;

        //Need to release 8%
        stages[4].startDate = unLockedAt + month * 36;
        stages[4].period = 12;
        stages[4].periodPercentage = 66;
