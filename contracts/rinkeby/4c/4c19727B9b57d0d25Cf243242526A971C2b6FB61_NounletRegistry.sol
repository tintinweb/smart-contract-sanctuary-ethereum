...
contract NounletRegistry is INounletRegistry {
    ...
    constructor() {
        ...
        fNFTImplementation = address(new NounletToken());
    }
    ...
    function batchBurn(address _from, uint256[] memory _ids) public {
        address info = vaultToToken[msg.sender];
        if (info == address(0)) revert UnregisteredVault(msg.sender);
    ...
    function create(
        ...
        address fNFT = fNFTImplementation.clone(
            abi.encodePacked(address(this), _descriptor, _nounId)
        );
        vault = _deployVault(_merkleRoot, fNFT, _plugins, _selectors);
    }
...
contract NounletToken is Clone, ERC1155BCheckpointable, INounletToken {
        ...
        returns (INounsSeeder.Seed memory)
    {
        ...
        INounsSeeder.Seed memory nounSeed = INounsToken(
            0x27c2511eBF928DE31ED3Ba641cA17774f8AA7d11
        ).seeds(nounId);
...
abstract contract ERC1155BCheckpointable is ERC1155B {
    ...
    function safeBatchTransferFrom(
        ...
        uint96 length = ids.length.safeCastTo96();
        _moveDelegates(delegates(src), delegates(dst), length);
