...
contract ERC721ABreeding is ERC721A, Ownable {
    ...
    function getBreedable(address _owner, uint _type) external view returns (uint256[] memory, bool[] memory) {
        ...
        unchecked {
            ...
            bool[] memory hasBred = new bool[](tokenIdsLength);
            for (uint256 i = 0; tokenIdsIdx != tokenIdsLength; i++) {
                if (parentContract.ownerOf(i) == _owner) {
                    tokenIds[tokenIdsIdx] = i;
                    hasBred[tokenIdsIdx++] = _type == 1 ? isDad[i] : isMom[i];
                }
            }
            return (tokenIds, hasBred);
        }
    }
