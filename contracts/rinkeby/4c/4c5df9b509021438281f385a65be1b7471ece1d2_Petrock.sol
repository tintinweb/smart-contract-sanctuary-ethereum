...
contract Petrock is ERC721Enumerable, Ownable, IERC721Receiver {
    using Counters for Counters.Counter;
    using MerkleProof for bytes32[];
     using Strings for uint256;
    Counters.Counter private _tokenId;

    uint256 public constant pet = 5500;
    ...
    constructor(string memory baseURI) ERC721("Petrock", "PET") {
        setBaseURI(baseURI);
    }
    ...
    function airdrop(address[] calldata _recipients) external onlyOwner {
        require(
            totalSupply() + _recipients.length <= pet,
            "Airdrop will exceed maximum supply of Petrock"
        );
        ...
        payable
    {
        ...
        require(
            _count > 0 && _count <= 2,
            "Minimum 1 & Maximum 2 Petrock can be minted per transaction"
        );
        require(
            totalSupply() + _count <= pet,
            "Exceeds maximum supply of Petrock"
        );
        ...
        require(
            ...
            "Maximum 2 Petrock can be minted in presale per address"
        );
        require(
    ...
    //mint petrock
    function mintpetrock(uint256 _count) external payable {
        require(
            totalSupply() + _count <= pet,
            "Exceeds maximum supply of Petrock"
        );
