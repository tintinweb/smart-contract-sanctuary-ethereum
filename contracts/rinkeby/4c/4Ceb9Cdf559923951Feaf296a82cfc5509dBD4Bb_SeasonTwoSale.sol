...
/*
# python - please copy/pasta and run to prove provenenance
import requests
import hashlib
...
def h(path, index):
    s.update(requests.get(path).content)
    print(f'getting {index}/10000')
    return s.hexdigest()


s.update("".join([h(f'{FP}/{i}.png', i) for i in range(1, 10001)]).encode('utf-8'))
print(f'provenance hash is {s.hexdigest()}')
print(f'provenance matches - {s.hexdigest() == PH}')
*/

contract SeasonTwoSale is ERC721A, Ownable, VRFConsumerBase{
    using SafeMath for uint256;
    using Strings for uint256;

    string public constant PROVENANCE = '';
    uint256 private constant TEN_THOUSAND_AND_ONE = 10001;

    string public BaseHash = '';
    string public PlaceholderHash = ''; // << pre-reveal hash
    ...
    uint256 public StartingIndex = TEN_THOUSAND_AND_ONE;
    ...
    constructor() ERC721A("name", "sym") 
        ...
        fee = 0.1 ether; // 2 LINK (Varies by network)
    }
    ...
    function togglePublicSaleOpen() public onlyOwner {
    ...
    function publicMint(bytes32[] calldata _merkleProof, uint256 _quantity) payable public {
        ...
        if (!PublicSaleOpen) {
            ...
            require(MerkleProof.verify(_merkleProof, MerkleRoot, leaf), "not authorised");
    ...
    function revealCollection(string memory _hash) public {
        require(StartingIndex == 10001, "StartingIndex already set");
        BaseHash = _hash;
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory) {
        ...
        if (StartingIndex == TEN_THOUSAND_AND_ONE) {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        require(StartingIndex == TEN_THOUSAND_AND_ONE, "StartingIndex already set");
        require(vrfRequestId == requestId, "Chainlink VRF Request Id must match");
        StartingIndex = (randomness % (MaxSupply - 1)) + 1;
    }
}
