...
// File: TChess Collection_flat.sol
...
// File: contracts/TChess Collection.sol
...
contract TCC is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    address constant team_address = 0x346246Fb326A45BD8D0b5ee03e8676d56880282a;

    bytes32 public merkleRootPrivate;
    bytes32 public merkleRootRaffle;

    uint256 public constant MAXIMUM_SUPPLY = 9999 ;
    uint256 public constant MAXIMUM_GIFT = MAXIMUM_SUPPLY;

    uint256 public constant MAXIMUM_MINT = 10 ;
    ...
    function presaleMint(uint256 ammount, bytes32[] calldata _merkleProof) external payable nonReentrant
    {
        ...
        require(workflow == WorkflowStatus.Presale, "TCC: Presale is not started yet!");
        require(whitelistSoldOut == false, "TCC: Presale is SOLD OUT!");

        require(tokensPerWalletWhitelist[msg.sender] + ammount <= 2, "TCC: Presale mint is two token only.");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRootPrivate, leaf), "TCC: You are not whitelisted");

        require(msg.value >= price * ammount, "TCC: Not enough ETH sent");
    ...
    function publicSaleMint(uint256 ammount, bytes32[] calldata _merkleProof) public payable nonReentrant {
        ...
        require(workflow != WorkflowStatus.SoldOut, "TCC: SOLD OUT!");
        require(workflow == WorkflowStatus.Sale, "TCC: Raffle is not started yet");
        require(msg.value >= price * ammount, "TCC: Not enough ETH sent");
        require(ammount <= 5, "TCC: You can only mint up to five token at once!");
        require(tokensPerWalletRaffle[msg.sender] + ammount <= 5, "TCC: You can't mint more than 5 tokens!");
        require(supply + ammount <= MAXIMUM_SUPPLY, "TCC: Mint too large!");

        if(raffleMode == true) {
          bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
          require(MerkleProof.verify(_merkleProof, merkleRootRaffle, leaf), "TCC: You haven't been selected");
        }
    ...
    function setUpSale() external onlyOwner {
        require(workflow == WorkflowStatus.Presale, "TCC: Unauthorized Transaction");
