    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        require(factory_ != address(0), "HinataMarket: INVALID_COLLECTION_FACTORY");
    ...
    function setAcceptPayToken(address payToken, bool accept) public onlyAdmin {
    ...
    function purchaseListing(uint256 listingId) external nonReentrant {
        Listing storage listing = listings[listingId];
        require(listing.seller != msg.sender, "HinataMarket: SAME_PERSON");
        if (
    ...
    function bid(uint256 listingId, uint256 bidAmount) external nonReentrant {
        ...
        if (bidding.bidder != address(0)) {
            ...
            require(bidAmount >= listing.price, "HinataMarket: LOWER_THAN_STARTING");
        }
    ...
    function _proceedRoyalty(
        ...
        // to seller
        if (isFromContract) IERC20Upgradeable(payToken).safeTransfer(seller, price - fee);
        else IERC20Upgradeable(payToken).safeTransferFrom(buyer, seller, price - fee);

        // to collection owners
        uint256 sum;
        address[] memory owners = new address[](collections.length);
        uint256[] memory royalties = new uint256[](collections.length);
        for (uint256 i; i < collections.length; i += 1) {
            ICollectionFactory.Collection memory collection_ = factory.getCollection(
                collections[i]
            );
            owners[i] = collection_.owner;
            royalties[i] = collection_.royalty;
            sum += tokenAmounts[i] * royalties[i];
        }
        if (sum == 0) {
            if (!isFromContract)
                IERC20Upgradeable(payToken).safeTransferFrom(buyer, address(this), fee);
            return;
        }
        for (uint256 i; i < collections.length; i += 1)
            if (isFromContract)
                IERC20Upgradeable(payToken).safeTransfer(
                    owners[i],
                    (fee * tokenAmounts[i] * royalties[i]) / sum
                );
            else
