...
/**
 *Submitted for verification at DAIerscan.io on 2022-07-21
*/
...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - see https://github.com/DAIereum/solidity/issues/2691
        return msg.data;
    }
}
...
interface IDexRouter {
    ...
    function swapExactTokensForDAISupportingFeeOnTransferTokens(
        uint256 amountIn,
    ...
    function swapExactDAIForTokensSupportingFeeOnTransferTokens(
        uint256 amountOutMin,
        address[] calldata path,
        address to,
        uint256 deadline
    ...
    function removeLiquidityDAI(
        ...
        address to,
        uint256 deadline
    ) external returns (uint256 amountToken, uint256 amountDAI);
...
contract Hades is Context, IERC20, Ownable {
    ...
    event SwapTokensForDAI(uint256 amountIn, address[] path);
    ...
    //to receive DAI from dexRouter when swapping
    ...
    function transferToAddressDAI(address payable recipient, uint256 amount)
        private
    {
        bool success;
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        swapTokensForDAI(tokensForSwap);

        uint256 amountReceived = address(this).balance;

        uint256 bnbShares = totalShares - autoBurnShare - (lpShare / 2);

        uint bnbForLiquidity = ((amountReceived * lpShare) / bnbShares) / 2;
        uint bnbForMarketing = amountReceived - bnbForLiquidity;

        if (bnbForMarketing > 0) {
            transferToAddressDAI(marketingWalletAddress, bnbForMarketing);
        }
    ...
    function swapTokensForDAI(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> wDAI
        ...
        // make the swap
        dexRouter.swapExactTokensForDAISupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 DAIAmount) private {
        ...
        // add the liquidity
        dexRouter.addLiquidityDAI{value: DAIAmount}(
            address(this),
            tokenAmount,
    ...
    function launch(uint256 _snipePenaltyBlocks, uint256 _sellPenaltyBlocks) external onlyOwner {
        ...
        // create pair
        lpPair = IDexFactory(dexRouter.factory()).createPair(address(this),dexRouter.DAI());
        isMarketPair[address(lpPair)] = true;
        isWalletLimitExempt[address(lpPair)] = true;

        // add the liquidity

        require(address(this).balance > 0, "Must have DAI on contract to launch");

        require(balanceOf(address(this)) > 0, "Must have Tokens on contract to launch");

        _approve(address(this), address(dexRouter), balanceOf(address(this)));
        dexRouter.addLiquidityDAI{value: address(this).balance}(
            address(this),
    ...
    // withdraw DAI if stuck or someone sends to the address
