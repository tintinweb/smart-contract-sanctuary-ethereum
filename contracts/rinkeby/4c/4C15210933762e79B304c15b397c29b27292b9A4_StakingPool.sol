...
contract StakingPool is IStakingPool, Ownable, CheckContract{
    ...
    uint internal MAXIMUM_WITHDRAWAL_PERIOD = 86400;
    ...
    function requestWithdrawFromStakingPool(uint _amount) external override {
        ...
        withdrawRequest[tokenIndex] = RequestWithdraw(
            ...
            conversionRate,
            USDCFee,
            block.timestamp + MAXIMUM_WITHDRAWAL_PERIOD
        );
    ...
    function modifyMaximumWithdrawlPeriod(uint _time) external onlyOwner returns (uint) {
        MAXIMUM_WITHDRAWAL_PERIOD = _time;
        return MAXIMUM_WITHDRAWAL_PERIOD;
    }
