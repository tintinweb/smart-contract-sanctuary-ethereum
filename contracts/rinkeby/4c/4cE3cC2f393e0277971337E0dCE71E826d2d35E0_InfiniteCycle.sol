...
contract InfiniteCycle is ERC1155, Ownable, Pausable {
 ...
 constructor() ERC1155("https://gateway.pinata.cloud/ipfs/QmVEoaBqmyzxVWg9ZsjdkzN2wSpVxdyQLh7qsHxrv2NYbh/{id}.json") {
    ...
    function burnWater(address _address,uint _id) external onlyOwner{
         uint256 currBalance = balanceOf(_address,WATER);
         if(currBalance > 9){
          uint256 _burnAmount = currBalance.mod(9);
          require(_burnAmount > 0  , "Burn is not possible");
          _burn(_address, _id, _burnAmount);
        }
      }
