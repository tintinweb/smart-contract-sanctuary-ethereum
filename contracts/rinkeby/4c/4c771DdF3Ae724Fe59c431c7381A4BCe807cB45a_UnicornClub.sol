...
contract UnicornClub is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint public presalePrice = .0001 ether; //$50 on mainnet
    uint public publicPrice = .0001 ether; //$70 on mainnet
    uint public maxPresaleMints = 10;
   ...
   //todo: decide if OG's will only have one signature check in ogMint and redeem and inside the frontend we distinguish
    ...
    \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*/
     function redeemOne(uint max, bytes memory signature) external {
        ...
        if(isEligibleForSecondRedemption[msg.sender]){
            if(numRedemptions[msg.sender] + 1 > 2) revert MaxRedemptions();
            numRedemptions[msg.sender]++;
            //todo: go over this again
            uint numToMint = numRedemptions[msg.sender] == 2 ? 1 : 2; 
            _mint(msg.sender,numToMint);
            return;
        }

        if(numRedemptions[msg.sender] + 1 > 1) revert MaxRedemptions();
        numRedemptions[msg.sender]++;
        _mint(msg.sender,1);
        return;
