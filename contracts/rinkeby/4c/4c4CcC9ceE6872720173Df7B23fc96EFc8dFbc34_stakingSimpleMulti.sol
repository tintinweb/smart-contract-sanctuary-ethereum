...
contract stakingSimpleMulti is Ownable, ReentrancyGuard {
    IERC721 public baseNFT;
    IERC721 public secondaryNFT;
	...
	mapping(address => uint256) public _baseRate;
    ...
    constructor(address _setBaseNFTContract, address _setSingnerAddress) {
        ...
        _contractTypes[_setBaseNFTContract] = ContractTypes.BASENFT;
        _baseRate[_setBaseNFTContract] = 1500 ether;
    ...
    function deposit(bytes[] memory signature, address contractAddress, uint256[] memory tokenIds, uint256[] memory rarityWeight) public nonReentrant {
        ...
        require(contractAddress != address(0) && contractAddress == address(baseNFT) || contractAddress == address(secondaryNFT), "Unknown contract");
        ...
        if (rarityWeight.length > 0) {
            ...
            _setTokensValues(contractAddress, tokenIds, rarityWeight);
        }
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            if (user.currentYield != 0 || contractType != ContractTypes.SECONDNFT) {
                newYield += getTokenYield(contractAddress, tokenIds[i]);
            }

            //add token id to user struct
            if (contractType == ContractTypes.BASENFT) {
                user.stakedBaseNFT.push(tokenIds[i]);
            }
            if (contractType == ContractTypes.SECONDNFT) {
                user.stakedSecondaryNFT.push(tokenIds[i]);
            }
        }

        if (user.currentYield == 0 && newYield != 0 && user.stakedSecondaryNFT.length > 0) {
            for (uint256 i; i < user.stakedSecondaryNFT.length; i++) {
                uint256 tokenYield = getTokenYield(address(secondaryNFT), user.stakedSecondaryNFT[i]);
                newYield += tokenYield;
            }
        }
    ...
    function withdraw(address contractAddress, uint256[] memory tokenIds) public nonReentrant {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            if (contractType == ContractTypes.SECONDNFT) {
                user.stakedSecondaryNFT = _moveTokenInTheList(user.stakedSecondaryNFT, tokenIds[i]);
                user.stakedSecondaryNFT.pop();
            }
    ...
    function _setTokensValues(address contractAddress, uint256[] memory tokenIds, uint256[] memory rarityWeight) internal {
        for (uint256 i; i < tokenIds.length; i++) {
            if (rarityWeight[i] != 0 && rarityWeight[i] <= 3000 ether) {
                _tokensMultiplier[contractAddress][tokenIds[i]] = rarityWeight[i];
            }
        }
    }
    ...
    function getTokenYield(address contractAddress, uint256 tokenId) public view returns (uint256) {
        ...
        if (tokenYield == 0) {
            tokenYield = _baseRate[contractAddress];
        }
    ...
    function setSecondaryNFTContract(address _secondnft, uint256 _baseReward) public onlyOwner {
      secondaryNFT = IERC721(_secondnft);
      _contractTypes[_secondnft] = ContractTypes.SECONDNFT;
      _baseRate[_secondnft] = _baseReward;
    }
   ...
   // Returns token owner address (returns address(0) if token is not inside the gateway)
    ...
    function getStakerTokens(address staker) public view returns (uint256[] memory, uint256[] memory) {
        return (_stakers[staker].stakedBaseNFT, _stakers[staker].stakedSecondaryNFT);
    }
