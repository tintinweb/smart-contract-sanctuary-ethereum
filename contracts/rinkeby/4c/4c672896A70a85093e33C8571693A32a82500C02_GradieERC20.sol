...
contract GradieERC20 is Ownable, ERC20, ERC20Capped, ReentrancyGuard {
    ...
    IERC721 public parentNFT =
        IERC721(0xEB1Dd3bC084781BcDcf34333f739F46eA3987Ed9);
    // date that start calculating claimable rewards
    uint256 public startingTimestamp = 1652889600;
    ...
    function claim(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
                ...
                "you're not the NFTs holder"
            );
            if (tokenLastUpdated[_tokenIds[i]] < startingTimestamp)
    ...
    function getTotalClaimableAmount(
        ...
        require(_tokenIds.length > 0, "need to claim for atleast one token");
        require(
            _tokenIds.length == _bonuses.length &&
                _bonuses.length == _signatures.length,
            "Invalid Input"
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if (lastUpdatedTime < startingTimestamp)
                lastUpdatedTime = startingTimestamp;
