...
contract Mounts is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getGender(uint256 tokenId) public view returns (string memory) {
        return Gender[random("Gender", tokenId, 0, 5)];
    }
    ...
    function getName(uint256 tokenId) public view returns (string memory) {
        uint256 r = random("Name", tokenId, 0, 99);
    ...
    function getColorId(uint256 r) public view returns(string memory){
        if( r < 50 ) return "#fff";
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory){
        ...
        parts[1] = getColorId(random("Name", tokenId, 0, 99));
    ...
    function mint(uint256 q) public nonReentrant {
        require(totalSupply() < (TOTALSUPPLY - reserved), "Token ID invalid");
        
        for(uint i=0; i<q; i++){
            _safeMint(_msgSender(), totalSupply()+1);
            tokenTime[totalSupply()] = block.timestamp + totalSupply();
        }
    }
