...
contract ApeX4 is ERC721, Ownable {
    ...
    /// matching public key for the private key used for signing on the frontend
    ...
    constructor (
        ...
        /// experimental
        for (uint256 i = 0; i < wallets.length; i++) {
            numMintedPerWallet[wallets[i]] = 0;
        } 
    }
    ...
    function whitelistMint(uint16 amount, address _address, uint256 _numAllowed, bytes calldata _voucher) external payable {
        ...
        if (amount +  numMintedPerWallet[_address] > _numAllowed) revert TooManyMintForTier();
        ...
        numMintedPerWallet[_address] += amount;
    ...
    function generalMint(uint16 amount) external payable {
        ...
        if (msg.value < amount * mintPrice) revert NotEnoughETH();
        if (amount > maxGeneralMintAmount) revert TooManyMintAtOnce();
        numMintedPerWallet[msg.sender] += amount;
