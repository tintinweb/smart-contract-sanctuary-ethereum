    ...
    UUPSUpgradeable
{
    ...
    function swapAndAddMarketingCurrency(uint256 _marketingFree) private {
        // split the contract balance into half
        // uint256 half = contractTokenBalance / 2;
        // uint256 otherHalf = contractTokenBalance - half;
        // capture the contract's current ETH balance.
        ...
        // swap tokens for ETH
        swapTokensForEth(_marketingFree); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        ...
        _balances[to] += amount - totalFee;
         if (!marketingSwap ) {
            marketingSwap = true;
                ...
                swapAndAddMarketingCurrency(currentMarketingFee);
                if (taxDisableInLiquidity) {
                    enableFee = true;
                    taxDisableInLiquidity = false;
                }
