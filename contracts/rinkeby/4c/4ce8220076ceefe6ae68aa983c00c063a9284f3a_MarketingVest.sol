...
contract MarketingVest is Ownable {
    ...
    uint256 public constant INVERSE_BASIS_POINT = 10000;
     ...
     uint256 private month = 1 minutes;

    uint8 private constant vestStages = 4;
    ...
    function lock(uint256 unLockTime) internal {
        lockedAt = unLockTime;
        //Need to release 5%,
        //Start releasing 5%
        stages[0].startDate = block.timestamp;
        stages[0].period = 1;
        stages[0].periodPercentage = 500;

        //Need to release 38%
        stages[1].startDate = lockedAt;
        stages[1].period = 12;
        stages[1].periodPercentage = 316;

        //Need to release 32%
        stages[2].startDate = lockedAt + month * 12;
        stages[2].period = 12;
        stages[2].periodPercentage = 267;

        //Need to release 25%
        stages[3].startDate = lockedAt + month * 24;
        stages[3].period = 12;
        stages[3].periodPercentage = 208;
        emit Allocated(withdrawAddress, block.timestamp);
    }

    function allocate(uint256 unLockTime) public notLocked onlyOwner {
