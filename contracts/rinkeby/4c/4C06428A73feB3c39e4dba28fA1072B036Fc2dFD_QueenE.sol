...
import {QueenParliament} from "QueenParliament.sol";
...
contract QueenE is IQueenE, QueenParliament {
    ...
    // QueenE id that is will be used for next auction
    uint256 public override _currentAuctionQueenE;

    string private _ipfsProvider = "ipfs://";
    string private _ipfsContractURIHash = "HASH_HERE";
    ...
    modifier whenTraitStorageNotLocked() {
        require(!isTraitStorageLocked, "Queen Trait Storage is locked");
        _;
    }
    ...
    modifier whenLabNotLocked() {
        require(!isLabLocked, "QueenE Lab is locked");
        _;
    }
    ...
    function contractURI() public view override returns (string memory) {
        return
            string(abi.encodePacked(_ipfsProvider, _ipfsContractURIHash, "/"));
    }
     ...
     * QueenE. Sir reward QueenEs are minted on 10th mint whenever there is a Sir
     * with no reward. It mint's only one QueenE reward for each Sir
     * @dev Call _mintTo with the minter address.
     */
    function mint() public override onlyMinter returns (uint256) {
        if (_haveSirWithPendingReward() && _currentAuctionQueenE % 10 == 0) {
            address sirAddress = _getSirWithPendingAward(_currentAuctionQueenE);
            if (sirAddress != address(0)) {
                uint256 sirQueenE = _mintTo(
                    sirAddress,
                    _currentAuctionQueenE++
                );
                sirAward[sirAddress] = sirQueenE;
            }
        }
        return _mintTo(queenStaff.minter(), _currentAuctionQueenE++);
    }
    ...
    {
        require(_exists(tokenId), "QueenE: URI query for nonexistent token");

        return
            string(
                abi.encodePacked(
                    '{"name":"QueenE", "description":"',
                    queenes[tokenId].description,
                    '"',
                    '"image": "',
                    contractURI(),
                    queenes[tokenId].finalArt,
                    '"}'
                )
            );
    }
        ...
        onlyOwner
    {
        ...
        require(
            ...
            "Final art hash already set! Cant override"
        );
     ...
     */
    function _mintTo(address _to, uint256 _queeneId)
        internal
        returns (uint256)
    {
        RoyalLibrary.sQUEEN memory queene = queenStaff.QueenLab().GenerateQueen(
            _queeneId,
            queenStaff.QueenTraits(),
            IQueenE(address(this))
        );

        queenes[_queeneId].queeneId = queene.queeneId;
        for (uint256 idx = 0; idx < queene.blueBlood.length; idx++) {
            queenes[_queeneId].blueBlood.push(queene.blueBlood[idx]);
        }
    ...
    function _afterTokenTransfer(
        ...
        if (!ownerOfQueenEs[to].contains(tokenId))
            ownerOfQueenEs[to].add(tokenId);

        if (
            to != queenStaff.minter() &&
            (!sirs.contains(to) || sirAward[to] > 0)
        ) {
            if (
                _giveParliamentSeat(
                    to,
                    queenes[tokenId].queeneId,
                    queenStaff.QueenLab().GetQueenRarity(queenes[tokenId].dna)
                )
            ) {
                _takeParliamentSeat(from, tokenId);
            }
        }
    }
    ...
    function nominateSir(address _sir)
        ...
        returns (bool)
    {
        require(_sir != address(0), "Invalid addres(0)!");
        require(
            !houseOfBanned.contains(_sir),
            "Subject was banned from houses and can't be awarded Sir title!"
        );
        require(!sirs.contains(_sir), "Subject already has Sir title!");
    ...
    {
        if (_seatType == 0)
            return houseOfLords.length() + houseOfCommons.length();
        else if (_seatType == 1) return houseOfLords.length();
        else if (_seatType == 2) return houseOfCommons.length();
        else if (_seatType == 3) return houseOfBanned.length();
        else return 0;
    }
     ...
     *
     */
    function getHouseSeat(address addr)
        external
        view
        override
        returns (uint256)
        ...
        returns (RoyalLibrary.sSIR memory)
    {
        return
            RoyalLibrary.sSIR({
                isSir: sirs.contains(_address),
                sirAddress: _address,
                queen: sirAward[_address]
            });
    }
}
 ...
 */
abstract contract QueenParliament is ERC721Enumerable, Votes {
    ...
    event NominatedRejectionBannedFromHouses(
        address indexed common,
        uint256 queeneId
    );
    ...
    event ChangedLordVoteMultiplier(uint256 indexed _multiplier);
    ...
    EnumerableSet.AddressSet internal sirs;
    mapping(address => uint256) internal sirAward;

    uint256 internal lordMultiplier;
     ...
     *
     */
    function _haveSirWithPendingReward() internal view returns (bool reward) {
        if (sirs.length() == 0) return false;

        for (uint256 idx = 0; idx < sirs.length(); idx++) {
            if (
                sirAward[sirs.at(idx)] == 0 &&
                !houseOfBanned.contains(sirs.at(idx))
            ) {
                return true;
            }
        }
    ...
    /**
     * @dev return sir reward for given address.
     *
     */
    function _getSirReward(address _sirAddress)
        internal
        view
        returns (uint256 queeneId)
    {
        require(sirs.contains(_sirAddress), "Not a Nominated Sir");
        return sirAward[_sirAddress];
    }
        ...
        returns (address)
    {
        ...
        for (uint256 idx = 0; idx < sirs.length(); idx++) {
            if (
                ...
                sirsToBeWarded[nextIdx++] = sirs.at(idx);
            }
        }
        ...
        if (sirsToBeWarded.length == 1) {
            ...
            return sirsToBeWarded[pseudoRandomNumber % sirsToBeWarded.length];
        }
     ...
     * Emits a {BannedFromHousesRejection} event if subject was banned.
     */
    function _giveParliamentSeat(
        address to,
        uint256 queeneId,
        IQueenLab.queeneRarity rarity
    ) internal returns (bool) {
        if (houseOfBanned.contains(to)) {
            emit NominatedRejectionBannedFromHouses(to, queeneId);
            return true; //seat must be vacant anyways
        }

        if (to != address(0)) {
            if (
                houseOfLords.contains(to) ||
                (houseOfCommons.contains(to) &&
                    rarity < IQueenLab.queeneRarity.RARE)
            ) {
                //already nominated
                return true;
            }
            ...
            if (
                ...
                houseOfLords.add(to);
                if (!houseOfLordsFull)
                    houseOfLordsFull = houseOfLords.length() >= 15;
                if (houseOfCommons.contains(to)) {
                    houseOfCommons.remove(to);
                }
     ...
     *
     * Emits a {ParliamentEndOfMandate} event if succeded.
     */
    ...
    {
        if (from != address(0)) {
            if (balanceOf(from) > 0) return true; //keeps seat in Parliament

            if (houseOfLords.contains(from)) {
                houseOfLords.remove(from);

                emit ParliamentEndOfMandate(from, tokenId, "LORDS");

                return true;
            } else if (houseOfCommons.contains(from)) {
                houseOfCommons.remove(from);

                emit ParliamentEndOfMandate(from, tokenId, "COMMONS");
     ...
     *
     */
    function _banFromHouses(address _subject) internal {
        require(!houseOfBanned.contains(_subject), "Subject already banned!");
        require(
            houseOfLords.contains(_subject) ||
                houseOfCommons.contains(_subject),
            "Subject doesnt have seat in any of the houses!"
        );

        if (houseOfLords.contains(_subject)) houseOfLords.remove(_subject);
        else if (houseOfCommons.contains(_subject))
            houseOfCommons.remove(_subject);
    }
    ...
    {
        return
            (balanceOf(account) - (sirAward[account])) *
            (houseOfLords.contains(account) ? lordMultiplier : 1);
    }
}
...
//
// CHANGES:
// inherits from our own interface controller (BaseContractController) instead of the pure ERC165 standard controller.
...
contract ERC721 is Context, BaseContractController, IERC721, IERC721Metadata {
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(BaseContractController, IBaseContractController)
        returns (bool)
    {
        return
