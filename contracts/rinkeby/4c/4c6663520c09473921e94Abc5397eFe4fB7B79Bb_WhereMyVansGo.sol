...
// @author: andrewjiang.eth, built on manifold.xyz
...
contract WhereMyVansGo is ERC721Creator {
    ...
    bytes32 public merkleRoot; // Merkletree root to verify old token IDs and new token IDs are valid
    uint8[] public migrationRange;
    string public contractMetadataURI;

    mapping(uint256 => address) public originalTokenIdToAddress;
    mapping(uint256 => bool) public newTokenMinted;
    ...
    function resurrect(
        ...
        require(newTokenMinted[newTokenId] == false, "New token already minted");
        require(bytes(uri).length > 0, "URI cannot be empty");
        
        newTokenMinted[newTokenId] = true;
        _safeMint(owner, newTokenId);
        _tokenURIs[newTokenId] = uri;

        // Call post mint
        _postMintBase(owner, newTokenId);
    }

    function mint(address to, uint8 tokenId, string calldata uri) public virtual nonReentrant adminRequired returns(uint256) {
        require(newTokenMinted[tokenId] == false, "New token already minted");
