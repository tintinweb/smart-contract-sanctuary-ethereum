...
contract SpaceMilk is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function calculateRewards(uint256 _mintingAmount, uint256 _collection) internal view returns (uint256, uint256) {
        ...
        if (remainingBoost > _mintingAmount) {
            boostRewards = _mintingAmount.mul(2);
            removedBoost = _mintingAmount;
    ...
    function setRewards(address _user, uint256 _collection) internal {
        if (remainingSupplyByCollection[_collection] > 0) {
            ...
            if (userTimer > 0) {
                ...
                updateUserTimeOnMint(_user, _collection);
                return;
            }
    ...
    function getTotalClaimable(address _user, uint256 _collection) external view returns(uint256) {
        require(remainingSupplyByCollection[_collection] > 0, string(abi.encodePacked("Supply is fully minted for collection: ", collectionInfo[_collection].name)));
        uint256 userTimer = lastUpdate[_user][_collection];
        require(userTimer > 0, "Pending reward timer has to been more then 0");
    ...
    function setSpaceCowsAddress(address _nftContract) external onlyOwner {
        spaceCows = SpaceCows(_nftContract);
    }
...
import "../Token/SpaceMilk.sol";
import "../Sale/Sale.sol";

contract SpaceCows is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable, UpgradeNFT {
    using StringsUpgradeable for uint256;
    ...
    function initialize(
        ...
        baseURI = _baseUri;
        baseExtension = ".json";
        YieldToken = SpaceMilk(_tokenAddress);
    }
    ...
    function setTokenAddress(address _newTokenAddress) public onlyOwner {
        YieldToken = SpaceMilk(_newTokenAddress);
    }

    function setSalesAddress(address _newSalesAddress) public onlyOwner {
        SaleContract = Sale(_newSalesAddress);
    }
...
contract UpgradeNFT is Initializable, ERC721Upgradeable, OwnableUpgradeable {
    ...
    function changeCustomName(uint256 _tokenId, string memory _newName) public virtual {
		...
		require(_msgSender() == owner, "Caller is not the owner");
		require(_validateName(_newName) == true, "Not a valid new name");
		require(sha256(bytes(_newName)) != sha256(bytes(customName[_tokenId])), "New name is same as the current one");
...
contract Sale is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, Whitelisted {
    ...
    function setSpaceCowsAddress(address _newNftContract) external onlyOwner {
        spaceCows = SpaceCows(_newNftContract);
    }
