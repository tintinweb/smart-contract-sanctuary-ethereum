...
abstract contract RandomnessBatches {
    ...
    constructor(uint128 _maxSupply, uint128 _batchSize) {
        maxSupply = _maxSupply;
        batchSize = _batchSize;
        uint256 batches = ceilDiv(_maxSupply, _batchSize);
        randomness = new bytes32[](batches);
    ...
    function _rngTryReveal(uint256 _newSupply) internal {
        if (_newSupply == maxSupply) {
            _rngReveal(randomness.length - 1);
            return;
        }
...
contract WassieverseNFT is NFTSale, RandomnessBatches, ERC721A, ERC2981 {
    using Strings for uint256;
    ...
    constructor(
        ...
        RandomnessBatches(_supply, _revealBatchSize)
