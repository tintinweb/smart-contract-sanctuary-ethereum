...
contract GAGGLE is Context, IERC20, Ownable {
    ...
    uint256 private constant _totalSupply = 20_000_000_000 * 10**9;
    ...
    constructor(
        ...
        uint256 cmo = blackHole.mul(1).div(100);
        uint256 treasury = blackHole.mul(28).div(100);

        uint256 rate = getRate();

        _reserveTokenBalance[burnAddress] = blackHole.mul(rate);
        _reserveTokenBalance[_msgSender()] = presale.mul(rate);
        _reserveTokenBalance[_msgSender()] = lp.mul(rate);
        _reserveTokenBalance[_treasuryAddress] = team.mul(rate);
        _reserveTokenBalance[_cmoAddress] = cmo.mul(rate);
