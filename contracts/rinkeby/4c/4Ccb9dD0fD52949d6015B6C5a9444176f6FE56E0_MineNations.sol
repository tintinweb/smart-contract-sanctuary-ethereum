...
contract MineNations is ERC721A, Ownable {
    ...
    function whitelistMint(uint _count, bytes32[] memory _merkleProof) public payable {
      bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
      require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Invalid proof");
      require(total() + _count <= 10000, "Can't mint anymore");
      require(presaleEnded > 0, "Can't mint the nft");
      if(block.timestamp < presaleEnded) {
        require(balanceOf(msg.sender) + _count <= 2, "Maxium is 2 during pre-sale step");
        require(msg.value >= _count * whitelistPrice, "Not match balance");
      } else {
        require(balanceOf(msg.sender) + _count <= 5, "Maxium is 5");
    ...
    function mintItem(uint _count) public payable {
      ...
      if(msg.sender != owner()) {
        ...
        require(msg.value >= _count * publicPrice, "Not match balance");
        require(balanceOf(msg.sender) + _count <= 3, "Maxium is 3");
      }
