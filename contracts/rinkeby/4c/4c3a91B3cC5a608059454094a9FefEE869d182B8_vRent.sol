...
contract vRent is IvRent2615, ERC721Holder, ERCXHolder { 
    ...
    ERCX public _ERCX;
    uint256 private coolDuration = 5 minutes;
    ...
    function manageLease(CallData memory _cd) private {
        for (uint256 i = 0; i < _cd.tokenIds.length; i++) {
            _verifyIsLeaseable(_cd, i, msg.sender);
    ...
    function manageRent(CallData memory _cd) private {
        for (uint256 i = 0; i < _cd.tokenIds.length; i++) {
            ...
            if(block.timestamp < item.Leasing.leaseTime) {
                revert cannotRentBeforeTime();
            }
           ...
           // _ERCX.safeTransferUser(address(this), msg.sender, _cd.tokenIds[i]);
            _ERCX.safeTransferUser(item.Leasing.leaserAddress, msg.sender, _cd.tokenIds[i]);//CHANGED
    ...
    function manageReturn(CallData memory _cd) private {
        for (uint256 i = 0; i < _cd.tokenIds.length; i++) {
            ...
            _verifyIsNotNull(item.Leasing);
            if(item.renting.renterAddress == msg.sender) {
                ...
                _verifyDeadlinePassed(item.renting, msg.sender, block.timestamp);
            }
            
            item.Leasing.leaseTime = block.timestamp + coolDuration;

            emit Returned(_cd.leasingIds[i], uint32(block.timestamp));

            _ERCX.revokeTenantRight(_cd.tokenIds[i]);
            _ERCX.safeTransferUser(item.renting.renterAddress, item.Leasing.leaserAddress, _cd.tokenIds[i]);
           _ERCX.approveTenantRight(address(this), _cd.tokenIds[i]);
     ...
     */
    function _verifyIsLeaseable(CallData memory _cd, uint256 _i, address _msgSender) private view {
        require(_cd.maxLeaseDurations[_i] > 0, "vRent::duration is zero");
        require(_ERCX.ownerOf(_cd.tokenIds[_i]) == _msgSender, "");
        require(
    ...
    // changes
        function _verifyDeadlinePassed(
        Renting memory _renting,
        address _msgSender,
        uint256 _blockTimestamp
    ) private pure {
        require(!(_renting.renterAddress == _msgSender), "vRent::renter cannot call");
          require(_isPastReturnDate(_renting, _blockTimestamp), "vRent:: return date not passed yet");
...
interface IvRent2615 is IERC721Receiver {
    ...
    /**
    @dev revert the tx if user try to rent again before cool down time
    **/
    error cannotRentBeforeTime();
}
...
//     1) Leaser cannot take back the NFT after the renting-end date
// - if caller is leaser, return the NFT
// 2) leasingIds is used instead of tokenIds
// - The latest code which was updated on 18 April already solved this
// 3) Multiple issues in the implementation of the manageStopLeasing
// - Partial issues mentioned in this doc is already resolved in latest commit, remaining depends on issue 4.
// 4) User role must be transferred back to leaser when renter returning the NFT token
// - Transfer user role back to leaser
// 5) Leaser cannot stop leasing if renter keeps renting NFT immediately after returning
// - This issue is already resolved in Rental 721/1155 contracts. Will implement in Rental 2615 too.
// 6) Lien address must be revoked before token transfer
