...
contract TinyAstro is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  modifier mintGeneralCompliance( uint256 _mintAmount, uint8 _tierNumber ) {
    ...
    if( _tierNumber != mintConfig.tierNumber ){ revert IncorrectTierAttempt(); }
    ...
    if( _mintAmount > mintConfig.maxMint ){ revert InvalidMintAmount(); }
    ...
    if( aux & tierMintedMask > 0 ){ revert InvalidMintAmount(); }
  ...
  function mintTier( uint256 _mintAmount, bytes32 _r, bytes32 _s, uint256 _v, uint8 _tierNumber )
  external
