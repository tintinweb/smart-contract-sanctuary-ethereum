...
/**
 * @dev NATIVE Token functions that do not require timelock or have a timelock less than the min timelock
 */
interface INativeToken {
    function mintSideChainRewards(uint256 _chainId) external returns (address, address, uint256);
    function getChainData(uint256 _chainId) external view returns (address, uint256, uint256);
}
...
contract TimelockController is AccessControl, ReentrancyGuard {
    ...
    address payable public devWalletAddress; // deployer
    ...
    function setMultichainAddresses(address _multichainRouter, address _anyCallAddress) public {
        require(msg.sender == address(this), "TimelockController: caller must be timelock");
        require(tx.origin == devWalletAddress, "tx.origin != devWalletAddress");
        require(_multichainRouter != address(0), "_multichainRouter can not be zero address");
        require(_anyCallAddress != address(0), "_anyCallAddress can not be zero address");
        multichainRouter = _multichainRouter;
        anyCallAddress = _anyCallAddress;
    }
    ...
    function bridgeRewards(uint256 _chainId) external {
        (address anyToken, address sidechainFarm, uint256 mintAmount) = 
            INativeToken(nativeToken).mintSideChainRewards(_chainId);

        IERC20(nativeToken).safeIncreaseAllowance(multichainRouter, mintAmount);
        ...
        IMultichainRouter(multichainRouter).swapout(
            anyToken, // address token,
            mintAmount, // uint256 amount,
            sidechainFarm, // address receiver,
            _chainId, // uint256 toChainId,
            2 // uint256 flags - 2 = fee paid on source chain
        );
    }

    function syncSideChain(uint256 _chainId) external {
        // function for propagating nativePerSecond + startTime to sidechain using anycall
        (address sidechainFarm, uint256 startTime, uint256 nativePerSecond) = 
            INativeToken(nativeToken).getChainData(_chainId);

        require(sidechainFarm != address(0), "sidechainFarm not set");
        require(startTime > 0, "startTime = 0");
        
        CallProxy(anyCallAddress).anyCall(
            sidechainFarm,
            abi.encode(startTime, nativePerSecond),
