...
error ExceedsMaxPresaleSlots();
error InvalidQuantity();
error InsufficientPayment();
error WithdrawFailed();

contract MaxPersaleMints is
    ERC721A,
    Ownable,
    Signature
{
    ...
    uint public presaleSupply = 100;
    uint public presaleTokensMinted;
    uint private maxPresaleMints = 2;
    ...
    constructor(bool isPaying, uint256 deploymentPrice) ERC721A("Max Persale Mints", "mpm") payable {
        if(isPaying){
    ...
    function mint(uint256 count) external payable {
        if (saleState != SaleState.PUBLIC) revert PublicSaleNotActive();
    ...
    {
        if (saleState != SaleState.PRESALE) revert PresaleNotActive();
        if (tx.origin != msg.sender) revert CallerIsContract();
        if (presaleTokensMinted + count > presaleSupply) revert ExceedsPresaleSupply();
        if (count > MAX_MINT_COUNT_PER_TXN) revert InvalidQuantity();
        if (_nextTokenId() + (count - 1) > maxSupply) revert SoldOut();
        if (msg.value < presalePrice * count) revert InsufficientPayment();

        uint64 numPresaleMints = _getAux(msg.sender) + count;
        if (numPresaleMints > maxPresaleMints) revert ExceedsMaxPresaleSlots();
        _mint(msg.sender, count);
        _setAux(msg.sender, numPresaleMints); 
        presaleTokensMinted += count;
    }
    ...
    /** ADMIN FUNCTIONS **/
    /**
     * @dev Sets sale state to CLOSED (0), PUBLIC (1), PRESALE (2).
     */
    ...
    /**
     * @dev Set presale price of this token
     */
    function setPresalePrice(uint256 presaleNewPrice) external onlyOwner {
        presalePrice = presaleNewPrice;
    }
    ...
    /**
     * @dev Set Max supply of this token
     */
    function setMaxSupply(uint256 newSize) external onlyOwner {
        maxSupply = newSize;
    }
...
contract Signature is Ownable {
    ...
    bytes32 private immutable NAME = keccak256("Max Persale Mints");
