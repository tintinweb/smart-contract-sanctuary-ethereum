...
contract ReentalMigrator is UUPSUpgradeableByRole {

  mapping(address => address) public toOf;

  event Migrate(address from, address to, address account, uint256 amount);

  // solhint-disable-next-line private-vars-leading-underscore
  bytes32 private constant MIGRATOR_ADMIN_ROLE = keccak256("MIGRATOR_ADMIN_ROLE");

  function initialize() public initializer {
    __AccessControlProxyPausable_init(msg.sender);
  }

  function migrate(address account, address from) public {
    address to = toOf[from];
    require(to != address(0), "ReentalMigrator: token not mapped");
    IReentalToken fromInterface = IReentalToken(from);
    uint256 amount = fromInterface.balanceOf(account);
    require(amount > 0, "ReentalMigrator: balance is zero");
    IReentalToken toInterface = IReentalToken(to);

    fromInterface.transferFrom(account, address(this), amount);
    toInterface.mint(account, amount);
    
    emit Migrate(from, to, account, amount);
  }

  function mapBatch(
    address[] memory from,
    address[] memory to
  ) public onlyRole(MIGRATOR_ADMIN_ROLE) {
