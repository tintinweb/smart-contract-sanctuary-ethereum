...
contract TheDrones is ERC721A, Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public maxPerPublicTx = 3; // Actually 2, this is to avoid using <=
    ...
    function withdraw() external nonReentrant onlyOwner {
        ...
        (bool successC, ) = payable(WALLET_C).call{ value: (currentBalance / 100) * 10 }("");
        require(successC, "Failed to send to C");

        (bool successD, ) = payable(WALLET_D).call{ value: (currentBalance / 100) * 13 }("");
        require(successD, "Failed to send to D");

        (bool successS, ) = payable(WALLET_S).call{ value: (currentBalance / 100) * 25 }("");
