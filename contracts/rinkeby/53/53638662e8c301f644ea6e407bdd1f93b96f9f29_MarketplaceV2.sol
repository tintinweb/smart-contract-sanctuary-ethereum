...
contract MarketplaceV2 is Whitelist {
    ...
    function initialize(uint256 _commissionPercent, address _weth) external {
    ...
    function changeOwner(address _newOwner) external onlyWhiteList {
        require(_newOwner != address(0), 'New owner can not be null');
        ...
        amountShouldBeAskingPrice(_contractAddress, _tokenId)
    {
        ...
        // In case ERC-721 does not implement royaltyInfo
        ...
        )
    {
        ...
        // In case ERC-721 does not implement royaltyInfo
        if (royaltyReceiver != address(0)) {
            t1 = weth.transferFrom(_buyer, royaltyReceiver, royaltyAmount);
        }

        if(t1 && weth.transferFrom(_buyer, tokenOwner, netAmount) && weth.transferFrom(_buyer, owner, commissionAmount)) {
            IERC721(_contractAddress).safeTransferFrom(
                tokenOwner,
                _buyer,
                _tokenId
    ...
    function getAllSales() external view returns (saleResponseStruct[] memory) {
        ...
        for (uint256 index = 0; index < listedTokenKeys.length; index++) {
            saleResponseStruct memory saleRes = saleResponseStruct(address(0), 0, 0, address(0), false);
        ...
        )
    {
        ...
        if (IERC721(_contractAddress).supportsInterface(0x40d8d24e) && ITraded(_contractAddress).isTraded(_tokenId) == false) {
            netAmount = (_amount * 60) / 100;
            commissionAmount = _amount - netAmount;

            ITraded(_contractAddress).traded(_tokenId);
        } else {
            ...
            grossAmount = (_amount * grossPay) / 10000;
            commissionAmount = _amount - grossAmount;
            netAmount = grossAmount - royaltyAmount;
        }
