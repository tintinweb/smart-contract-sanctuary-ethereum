...
contract IggyPop is ERC1155Burnable, Ownable {
    address public rainerHoshAddress;    
    ...
    constructor(address _rainerHoshAddress) ERC1155("https://iggy.rainerhosch.com/{id}") {
        _idTracker.increment();
        rainerHoshAddress = _rainerHoshAddress;
    }

    function setMinters(address[] memory addresses, bool isMinter) public onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            _minters[addresses[i]] = isMinter;
        }
    }
    ...
    function mint() public {
        ...
        ERC1155PresetMinterPauser rainerHoshToken = ERC1155PresetMinterPauser(rainerHoshAddress);

        require(rainerHoshToken.balanceOf(msg.sender, burnTokenId) >= burnTokenAmount, "No tokens");
        require(rainerHoshToken.isApprovedForAll(msg.sender, address(this)), "Not approved");
        rainerHoshToken.burn(msg.sender, burnTokenId, burnTokenAmount);
