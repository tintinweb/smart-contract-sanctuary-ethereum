...
contract SwarmDOTCEscrow is ERC1155Holder, IEscrow, AccessControl {
    ...
    mapping(uint256 => MakerDeposit) private deposits;
    mapping(uint256 => NftDeposit) private nftDeposits;
     ...
     */
    function setMakerDeposit(uint256 _offerId) external override isEscrowFrozen isEscrowManager {
        deposits[_offerId] = MakerDeposit(
            _offerId,
            IdOTC(dOTC).getOfferOwner(_offerId),
            IdOTC(dOTC).getOffer(_offerId).amountIn,
            IdOTC(dOTC).getOffer(_offerId).amountOut,
            false
        );
    }
     ...
     *   @param _offerId uint256 the offer ID
     */
    function setNFTDeposit(uint256 _offerId) external override isEscrowFrozen isEscrowManager {
        nftDeposits[_offerId] = NftDeposit(
            _offerId,
            IdOTC(dOTC).getNftOfferOwner(_offerId),
            IdOTC(dOTC).getNftOffer(_offerId).nftIds,
            IdOTC(dOTC).getNftOffer(_offerId).nftAmounts,
            false
        );
    }
        ...
        isEscrowManager
    {
        ...
        uint256 _amount = IdOTC(dOTC).getTakerOrders(orderId).amountToReceive;
        require(deposits[offerId].amountDeposited >= _amount, "Invalid Amount");
        deposits[offerId].amountDeposited -= _amount;
    ...
    function withdrawNftDeposit(uint256 _nftOfferId, uint256 _nftOrderId)
        ...
        override
        isEscrowFrozen
        isDepositFrozen(_nftOfferId)
        isEscrowManager
    {
        address _receiver = IdOTC(dOTC).getNftOrders(_nftOrderId).takerAddress;
        address _nftAddress = IdOTC(dOTC).getNftOffer(_nftOrderId).nftAddress;
        // Set Approval for DOTC
        IERC1155(_nftAddress).setApprovalForAll(dOTC, true);
    ...
    */
    ...
    ) external override isEscrowManager returns (bool status) {
    ...
    function safeInternalTransfer(
        ...
        require(IERC20(token).transfer(_receiver, _amount), "Transfer failed and reverted.");
    }
    ...
    function freezeEscrow(address _account) external override isEscrowManager returns (bool status) {
        isFrozen = true;
     ...
     *   @return status bool
     */
    function unFreezeEscrow(address _account) external override isEscrowManager returns (bool status) {
        isFrozen = false;
     ...
     *   @return status bool
     */
    function setdOTCAddress(address _token) external override isEscrowManager returns (bool status) {
        dOTC = _token;
        return true;
    }
     ...
     *   @return status bool
     */
    function removeOffer(uint256 offerId, address _account) external override isEscrowManager returns (bool status) {
