...
contract SuperSpaceDefenders is ERC721A, Ownable {  
    ...
    bool public thirdAllowSaleActive = false;

    // Prices
    uint256 public holderPrice = 0.04 ether;
    uint256 public allowListPrice = 0.06 ether;
    uint256 public publicPrice = 0.08 ether;

    // Allowlist limits
    uint256 public allowMintCount = 0;
    uint256 public maxAllowListLimit = 4444;
    uint256 public firstAllowSaleLimit = 1;
    uint256 public secondAllowSaleLimit = 3;
    uint256 public thirdAllowSaleLimit = 5;
    ...
    mapping (address => uint256) public thirdMintedList;
    ...
    function mintPresale(uint256 _amount, bytes32[] calldata _merkleProof, uint256[] calldata _ownedACTokens) public payable {
        ...
        if(_ownedACTokens.length > 0) {
            for (uint256 i = 0; i < _ownedACTokens.length; i++) {
                uint256 acTokenLimit = 3 - acTokenIdToAmounMinted[_ownedACTokens[i]]; 
                holderMintLimit = holderMintLimit + acTokenLimit;
            }
        }

        if(isWhiteListed) {
            if(thirdAllowSaleActive) {
                allowLimit = thirdAllowSaleLimit - thirdMintedList[msg.sender];
            } else if (secondAllowSaleActive) {
        ...
        uint256 allowCount = 0;

        for (uint256 i = 1; i <= _amount; i++) {
            if(i <= holderMintLimit) {
                totalPrice = totalPrice + holderPrice;
            } else {
                ...
                totalPrice = totalPrice + allowListPrice;
            }
        }
        ...
        for (uint256 i = 1; i <= _amount; i++) {
            if(i <= holderMintLimit) {
                for (uint256 k = 0; k < _ownedACTokens.length; k++) {
                    if(acTokenIdToAmounMinted[_ownedACTokens[k]] < 3) {
                ...
                if(thirdAllowSaleActive) {
                    thirdMintedList[msg.sender] = thirdMintedList[msg.sender] + 1;
                } else if (secondAllowSaleActive) {
    ...
    // Start and stop third allowList sale window
    function setThirdAllowSaleActive(bool val) public onlyOwner {
        thirdAllowSaleActive = val;
    }
    ...
    // Set different third allowList limit, just in case
    function setThirdAllowSaleLimit(uint256 limit) public onlyOwner {
        thirdAllowSaleLimit = limit;
    }
