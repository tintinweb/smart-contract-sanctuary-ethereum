...
contract BUNI is ERC721A, Ownable,  RandomlyAssigned { 
  ...
  uint256 public cost = 100000000 gwei; 
  uint256 public maxTokenCount = 2000; // Needs to be 10,000 on mainnet
  ...
  mapping(uint256 => BuniNFT) public allBuniNFT;
  ...
  constructor(address marketplaceAddress)
    public ERC721A("Blockchain United Notation Index", "BUNI") 
    RandomlyAssigned(maxTokenCount,1) {
    collectionName = name();
  ...
  function toggleForSale(uint256 _tokenId) public {
    ...
    BuniNFT memory buninft = allBuniNFT[_tokenId];
    if(buninft.forSale) {
      buninft.forSale = false;
    } else {
      buninft.forSale = true;
    }
    allBuniNFT[_tokenId] = buninft;
  }
  ...
  function buyToken(uint256 _tokenId) public payable {
    ...
    require(tokenOwner != msg.sender);
    // get that token from all crypto boys mapping and create a memory of it defined as (struct => BuniNFT)
    BuniNFT memory buninft = allBuniNFT[_tokenId];
    // price sent in to buy should be equal to or more than the token's price
    require(msg.value >= buninft.price);
    // token should be for sale
    require(buninft.forSale);
    address payable sendTo = buninft.currentOwner;
    // send token's worth of ethers to the owner
    sendTo.transfer(msg.value);
    // update the token's previous owner
    buninft.previousOwner = buninft.currentOwner;
    // update the token's current owner
    // update the how many times this token was transfered
    buninft.numberOfTransfers += 1;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(currentSupply + _mintAmount <= maxTokenCount, "Not enough left");
