...
contract TokenVault is Ownable, Pausable {
    ...
    /// @notice Withdraw function for owner in case of emergency at staking
    ...
    /// @dev Only works with anyone except staking tokens of pools
    ...
    function withdrawShares(uint256 _shares) public notContract whenNotPaused {
        ...
        uint256 fullUserBalance = (stakingBalance * user.shares) /
            totalShares;
        ...
        if (finalUserBalance < user.deposit){
            user.earned += currentAmountToWithdraw + finalUserBalance - user.deposit;
