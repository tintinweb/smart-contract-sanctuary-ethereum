...
contract EthKun is ERC721A, Ownable {
    ...
    uint256 public tier0_maxMintsPerTransaction = 2;

    // tier 1 (paid)
    uint256 public tier1_price = 0.01 ether;
    uint256 public tier1_maxTokensOwnedInWallet = 32;
    uint256 public tier1_maxMintsPerTransaction = 32;
    ...
    function setMaxMints(uint256 _maxMintsPublic) external onlyOwner {
        tier0_maxTokensOwnedInWallet = _maxMintsPublic;
    }
        ...
        string memory json = Base64.encode(
            bytes(string(
                abi.encodePacked(
                    ...
                    '"description": "ethkun is an 100% on-chain experimental NFT project celebrating the Ethereum Proof-of-Stake merge. Developed by @eddietree and @secondbestdad.",',
                    '"attributes":[',
    ...
    function _getStatsMetadata(uint256 seed, uint256 currLevel) private pure returns (string memory) {
        ...
        string memory metadata = string(abi.encodePacked(
          ...
          '{"trait_type":"Mergeability", "display_type": "number", "value":', Strings.toString(_randStat(seed, 5, 2, 10+currLevel)), '},',
    ...
    uint256 public minSteakingLevel = 3;
    ...
    function _canSpawnEthKunBaby(uint256 tokenId) private view returns (bool) {
        ...
        return totalSteakingTime >= _getSecsSteakingRequiredToMintBaby();
    }

    function _getSecsSteakingRequiredToMintBaby() private view returns (uint256) {
        // a function of numminted(), 
        // each numminted over geneis will be +1 days 
        return 0; // TODO: I WILL FIGURE OUT THE MATH CURVE SOON
    }

     function getSecsSteakingRequiredToMintBaby() external view returns (uint256) {
        return _getSecsSteakingRequiredToMintBaby();
    }

    /// @notice Mint a baby ethkun from steaked parent ethkun!
    /// @param parentTokenId Steaked ethkun tokenId 
    function mintEthKunBaby(uint256 parentTokenId) external {
        require(steakingEnabled == true);
        require(steakingStartTimestamp[parentTokenId] != NULL_STEAKING, "Not steaking");
        require(level[parentTokenId] >= minSteakingLevel, "Not level");
        require(msg.sender == ownerOf(parentTokenId), "Not yours");
        ...
        steakingTotalTime[parentTokenId] = 0.0;
