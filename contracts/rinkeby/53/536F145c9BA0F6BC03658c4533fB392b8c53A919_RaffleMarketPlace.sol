    ...
    KeeperCompatibleInterface
{
    ...
    /* Raffle Variables */
    uint256 private immutable i_registerFee;
    ...
    // mapping(uint256 => uint8) public s_raffleState;
    ...
    function setFeeByMillion(uint256 feeByMillion) external onlyOwner {
    ...
    function registerRaffle(
        ...
        address raffleAddress = address(raffleContract);
    ...
    function buyTickets(uint256 raffleId, uint256 tickets) public payable {
        ...
        raffleContract.batchMint(msg.sender, tickets);
            ...
            for (uint256 i = 0; i < s_totalRaffleCounter; i++) {
                ...
                if (raffle.raffleState == 1) {
                    ...
                    if (raffle.soldTickets == raffle.totalTickets) {
                        ...
                        requestIdToRaffleId[requestId] = i;
                        emit RequestedRaffleWinner(i, requestId);
                        break;
                    }
                    /* check if time is over in every active raffles */
                    if (raffle.created + raffle.duration < block.timestamp) {
                        raffle.raffleState = 2; // pending
                        /** process cancel the raffle */
                        break;
                    }
                }
            }
        ...
        override
    {
        ...
        uint256 winnerTicket = randomWords[0] % raffle.totalTickets;
        address payable seller = payable(raffle.seller);
        Raffle raffleContract = Raffle(raffle.raffleAddress);
        address winner = raffleContract.ownerOf(winnerTicket);
        /** Fee management */
        // send NFT to winner;
