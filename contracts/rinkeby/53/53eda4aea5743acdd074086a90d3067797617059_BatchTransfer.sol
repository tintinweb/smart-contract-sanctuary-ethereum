...
contract BatchTransfer is Ownable {
    ...
    function transferTokens(address[] calldata coin, address[] calldata dsts, uint256[] calldata values) public onlyOwner{
        require(dsts.length == values.length);
		require(dsts.length == coin.length);
        for (uint256 i = 0; i < dsts.length; i++) {
		    Token token = Token(coin[i]);
