...
contract ONBulkEnsRegister {
  ...
  function makeCommitments(string[] calldata names, address sender, bytes32 secret) public view returns (bytes32[] memory) {
    ETHRegistrarController controller = ETHRegistrarController(_controllerAddress);
    bytes32[] memory commitments = new bytes32[](names.length);
    for ( uint i = 0; i < names.length; ++i ) {
        commitments[i] = controller.makeCommitment(names[i], sender, secret);
    }
    return commitments;
  }
