...
contract USMTemplate is IUSM, Oracle, ERC20Permit, WithOptOut, Delegable {
    ...
    event CollateralPriceChanged(uint price, uint updateTime);
    event SyntheticPriceChanged(uint price, uint updateTime);
    ...
    struct StoredState {
        ...
        uint32 priceUpdatedTimestamp;
        uint80 collateralUsdPrice;          // Stored in billionths, not WADs: so 123.456 is stored as 123,456,000,000
        uint80 syntheticUsdPrice;
    ...
    struct LoadedState {
        ...
        uint priceUpdatedTimestamp;
        uint collateralUsdPrice;            // This one is in WADs, not billionths
        uint syntheticUsdPrice;
    ...
    StoredState public storedState = StoredState({
        timeSystemWentUnderwater: 0, priceUpdatedTimestamp: 0, collateralUsdPrice: 0,
        syntheticUsdPrice: 0, bidAskAdjustmentTimestamp: 0, bidAskAdjustment: uint80(BILLION)   // Initialize adjustment to 1.0 (scaled by 1b)
    });
    ...
    function refreshPrice() public virtual override(IUSM, Oracle) returns (uint collateralPrice, uint syntheticPrice, uint updateTime) {
        LoadedState memory ls = loadState();
        bool priceChanged;
        (collateralPrice, syntheticPrice, updateTime, ls.bidAskAdjustment, priceChanged) = _refreshPrice(ls);
        if (priceChanged) {
            (ls.collateralUsdPrice, ls.syntheticUsdPrice, ls.priceUpdatedTimestamp) = (collateralPrice, syntheticPrice, updateTime);
            _storeState(ls);
        }
    }
    ...
    function _mintUsm(address to, uint minUsmOut) trialPeriod() internal returns (uint usmOut)
    {
        ...
        (ls.collateralUsdPrice, ls.syntheticUsdPrice, ls.priceUpdatedTimestamp, ls.bidAskAdjustment, ) = _refreshPrice(ls);
        ...
        ls.collateralUsdPrice = ls.collateralUsdPrice.wadMulDown(adjShrinkFactor);
    ...
    function _burnUsm(address from, address payable to, uint usmToBurn, uint minEthOut) internal returns (uint ethOut)
    {
        ...
        ls.collateralUsdPrice = ls.collateralUsdPrice.wadMulUp(adjGrowthFactor);

        // 5. Check that the burn didn't leave debt ratio > 100%:
        uint newDebtRatio = debtRatio(ls.collateralUsdPrice, ls.ethPool - ethOut, ls.usmTotalSupply - usmToBurn);
    ...
    function _fundFum(address to, uint minFumOut) trialPeriod() internal returns (uint fumOut)
    {
        ...
        (ls.timeSystemWentUnderwater, ls.usmTotalSupply) =
            checkIfUnderwater(ls.usmTotalSupply, ls.ethPool, ls.collateralUsdPrice, ls.timeSystemWentUnderwater, block.timestamp);
    ...
    function _defundFum(address from, address payable to, uint fumToBurn, uint minEthOut) internal returns (uint ethOut)
    {
        ...
        uint newDebtRatio = debtRatio(ls.collateralUsdPrice, ls.ethPool - ethOut, ls.usmTotalSupply);
    ...
    function _refreshPrice(LoadedState memory ls)
        internal returns (uint collateralPrice, uint syntheticPrice, uint updateTime, uint adjustment, bool priceChanged)
    {
        (collateralPrice, syntheticPrice, updateTime) = oracle.refreshPrice();
        console.log(collateralPrice, syntheticPrice);
        ...
        priceChanged = updateTime > ls.priceUpdatedTimestamp;

        if (!priceChanged) {                // If the price isn't fresher than our old one, scrap it and stick to the old one
            collateralPrice = ls.collateralUsdPrice;
        } else if (ls.collateralUsdPrice != 0) {   // If the old price is 0, don't try to use it to adjust the bidAskAdjustment...
            /**
            ...
            if (adjustment > WAD) {
                ...
                adjustment = WAD.wadMax(ls.collateralUsdPrice * adjustment / collateralPrice);
            } else if (adjustment < WAD) {
                // min(1, old sell price / new mid price):
                adjustment = WAD.wadMin(ls.collateralUsdPrice * adjustment / collateralPrice);
            }
        }
    }
    ...
    function _storeState(LoadedState memory ls) internal {
        ...
        require(ls.priceUpdatedTimestamp <= type(uint32).max, "priceUpdatedTimestamp overflow");

        uint collateralPriceToStore = ls.collateralUsdPrice + HALF_BILLION;
        unchecked { collateralPriceToStore /= BILLION; }
        if (collateralPriceToStore != storedState.collateralUsdPrice) {
            require(collateralPriceToStore <= type(uint80).max, "collateralPriceToStore overflow");
            unchecked { emit CollateralPriceChanged(ls.collateralUsdPrice, ls.priceUpdatedTimestamp); }
        }

        uint syntheticPriceToStore = ls.collateralUsdPrice + HALF_BILLION;
        unchecked { syntheticPriceToStore /= BILLION; }
        if (syntheticPriceToStore != storedState.syntheticUsdPrice) {
            require(syntheticPriceToStore <= type(uint80).max, "syntheticPriceToStore overflow");
            unchecked { emit SyntheticPriceChanged(ls.syntheticUsdPrice, ls.priceUpdatedTimestamp); }
        }
        ...
        (storedState.timeSystemWentUnderwater,storedState.priceUpdatedTimestamp,
        storedState.collateralUsdPrice, storedState.syntheticUsdPrice,
         storedState.bidAskAdjustmentTimestamp, storedState.bidAskAdjustment) =
            (uint32(ls.timeSystemWentUnderwater), uint32(ls.priceUpdatedTimestamp),
            uint80(collateralPriceToStore), uint80(syntheticPriceToStore),
    ...
    function latestPrice() public virtual override(IUSM, Oracle) view returns (uint collateralPrice, uint syntheticPrice, uint updateTime) {
        (collateralPrice, syntheticPrice, updateTime) = (storedState.collateralUsdPrice * BILLION, storedState.syntheticUsdPrice * BILLION, storedState.priceUpdatedTimestamp);
    }

    // ____________________ Public informational view functions ____________________

    function latestOraclePrice() public virtual override view returns (uint collateralPrice, uint syntheticPrice, uint updateTime) {
        (collateralPrice, syntheticPrice, updateTime) = oracle.latestPrice();
    } 
    ...
    function loadState() public view returns (LoadedState memory ls) {
        ...
        ls.priceUpdatedTimestamp = storedState.priceUpdatedTimestamp;
        ls.collateralUsdPrice = storedState.collateralUsdPrice * BILLION;     // Converting stored BILLION (10**9) format to WAD (10**18)
        ...
        public pure returns (uint usmOut, uint adjShrinkFactor)
    {
        ...
        uint usmBuyPrice0 = usmPrice(IUSM.Side.Buy, ls.collateralUsdPrice, ls.bidAskAdjustment);
        ...
        public pure returns (uint ethOut, uint adjGrowthFactor)
    {
        ...
        uint usmSellPrice0 = usmPrice(IUSM.Side.Sell, ls.collateralUsdPrice, ls.bidAskAdjustment);
        ...
        public pure returns (uint fumOut, uint adjGrowthFactor)
    {
        if (ls.ethPool == 0) {
            ...
            adjGrowthFactor = WAD;
            uint fumBuyPrice0 = fumPrice(IUSM.Side.Buy, ls.collateralUsdPrice, ls.ethPool, ls.usmTotalSupply, fumSupply,
                                         ls.bidAskAdjustment);
            ...
            uint effectiveDebtRatio = debtRatio(ls.collateralUsdPrice, ls.ethPool, ls.usmTotalSupply);
            ...
            uint ethUsdPrice1 = ls.collateralUsdPrice.wadMulUp(adjGrowthFactor);
        ...
        public pure returns (uint ethOut, uint adjShrinkFactor)
    {
        ...
        uint fumSellPrice0 = fumPrice(IUSM.Side.Sell, ls.collateralUsdPrice, ls.ethPool, ls.usmTotalSupply, fumSupply,
                                      ls.bidAskAdjustment);
        ...
        uint fumDelta = WAD.wadDivUp(WAD - debtRatio(ls.collateralUsdPrice, ls.ethPool, ls.usmTotalSupply));
        ...
        // can end up at:
        uint lowerBoundEthUsdPrice1 = ls.collateralUsdPrice.wadMulDown(lowerBoundAdjShrinkFactor);
...
contract DiaOracleAdapter is Oracle {
    ...
    function latestPrice() public virtual override view returns (uint collateralPrice, uint syntheticPrice, uint updateTime) {
        ...
        // TODO return time
        return (diaCollateralPrice, diaSyntheticPrice, Math.min(collateralUpdateTime, syntheticUpdateTime));
    }
