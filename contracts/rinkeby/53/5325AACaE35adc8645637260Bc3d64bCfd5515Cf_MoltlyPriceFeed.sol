...
interface IMoltlyAddressRegistry {
    function tokenRegistry() external view returns (address);
}

interface IMoltlyTokenRegistry {
...
contract MoltlyPriceFeed is Ownable {
    /// @notice keeps track of oracles for each tokens
    mapping(address => address) public oracles;

    /// @notice Moltly address registry contract
    ...
    function registerOracle(address _token, address _oracle)
        ...
        onlyOwner
    {
        IMoltlyTokenRegistry tokenRegistry = IMoltlyTokenRegistry(
            IMoltlyAddressRegistry(addressRegistry).tokenRegistry()
        );
