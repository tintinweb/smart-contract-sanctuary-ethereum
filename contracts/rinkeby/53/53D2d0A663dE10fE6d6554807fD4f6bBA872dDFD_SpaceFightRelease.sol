...
* @author loltapes.eth
*/
contract SpaceFightRelease is ERC1155, ERC2981, ERC1155Burnable, Ownable, Pausable {
  ...
  // @notice Address to withdraw funds; royalties are set up separately.
  //   Should be updated with each release to a payment splitter involving all parties
  address public mintPayoutAddress;

  // @notice Address for the payout of the core team.
  //   Can be combined with artist addresses for token based royalties.
  ...
  onlyOwner
  {
    ...
    mintInternal(msg.sender, releaseId, tracksAmount, teamReserve);
  ...
  function setMintPrice(uint256 releaseId, uint128 mintPrice) external onlyOwner whenReleaseExists(releaseId) {
    releases[releaseId].mintPrice = mintPrice;
  }

  function setMaxSupply(uint256 releaseId, uint96 maxSupply) external onlyOwner whenReleaseExists(releaseId) {
    uint256 currentSupply = totalReleaseSupply(releaseId);
    // assuming tracks have same supply for a release
    require(maxSupply >= currentSupply, "New supply below existing supply");
  ...
  function setMetadataUri(uint256 releaseId, string calldata metadataUri) external onlyOwner whenReleaseExists(releaseId) {
    ...
    for (uint256 i = 0; i < tracksAmount;) {
      // required as per ERC1155 standard
      emit URI(metadataUri, toTokenId(releaseId, ++i));
    }
  }
  ...
  function mintSale(uint256 amount, bytes32[] calldata proof) external payable whenNotPaused whenReleaseExists(releaseIdForSale) {
    ...
    if (currentMerkleRoot != 0) {
      require(
        MerkleProof.verify(proof, currentMerkleRoot, keccak256(abi.encodePacked(msg.sender, ",", amount))),
    ...
    require(newTotalSupply <= release.maxSupply, "Over supply");
    ...
    if (release.walletMintLimit > 0) {
      // presale only allows one mint
      if (currentMerkleRoot != 0) {
        require(minted[releaseIdForSale][msg.sender] == 0, "Over wallet mint limit during presale");
      } else {
    ...
    if (newTotalSupply == release.maxSupply) {
      _endSale();
    }
  }
  ...
  function setTokenRoyaltyForArtist(uint256 tokenId, address artistAddress, uint96 basisPoints) public onlyOwner {
