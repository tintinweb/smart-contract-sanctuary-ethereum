...
library VaultLifecycle {
    ...
    /**
     * @notice Sets the next vtoken the vault will be shorting, and calculates its premium for the auction
    ...
    function claimAuctionVTokens(
        ...
        GnosisAuction.claimAuctionVTokens(
            auctionSellOrder,
            gnosisEasyAuction,
        ...
        returns (uint256)
    {
        ...
        vTokenPrice = buyAmount / sellAmount;
    ...
    function cloneVToken(
        ...
        // TODO: Need unique var for salt preparation
        ...
        returns (uint256)
    {
        ...
        uint96 premium;
        (, premium,) = _decodeOrder(auctionData.clearingPriceOrder);

        return uint256(premium);
    }
...
import "./Vault.sol";
import "../interfaces/IGnosisAuction.sol";
import "../interfaces/IVolmexRealisedVolVault.sol";
...
library GnosisAuction {
        ...
        returns (uint256 auctionID)
    {
        ...
        //     "vTokenSellAmount > type(uint96) max value!"
        // );
        ...
        )
    {
        ...
        sellAmount = bidDetails
            .lockedBalance
            .mul(bidDetails.vTokenAllocation)
            .div(100 * Vault.ALLOCATION_MULTIPLIER);

        // divide the `asset` sellAmount by the target premium per vToken to
        // get the number of vTokens to buy (18 decimals)
        buyAmount = sellAmount
            .mul(10**(bidDetails.assetDecimals.add(Vault.VTOKEN_DECIMALS)))
            .div(bidDetails.vTokenPremium)
        ...
        // approve that amount
        IERC20Upgradeable(bidDetails.asset).safeApprove(
    ...
    function claimAuctionVTokens(
        ...
        IGnosisAuction(gnosisEasyAuction).claimFromParticipantOrder(
            IVolmexRealisedVolVault(counterpartyThetaVault).vTokenAuctionID(),
            orders
        );
    }
...
interface IVolmexRealisedVolVault {
