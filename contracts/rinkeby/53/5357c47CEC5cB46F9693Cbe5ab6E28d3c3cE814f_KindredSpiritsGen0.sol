...
contract KindredSpiritsGen0 is CustomERC721Metadata, Ownable {
    ...
    function addBatchMintWhitelisted(address[] memory mintWhitelist) public onlyAdmin {
        // strings.slice memory s = mintWhitelist.toSlice();                
        // strings.slice memory delim = ",".toSlice();                            
        // string[] memory parts = new string[](s.count(delim) + 1);                  
        // for (uint i = 0; i < parts.length; i++) {                              
        //     parts[i] = s.split(delim).toString();
        //     addMintWhitelisted(address(parts[i]));                       
        // }
    ...
    function mint(uint256 _projectId) external payable returns (uint256 _tokenId) {
        ...
        require(publicSale || isMintWhitelisted[currentMintWhiteListVersion][msg.sender], "Must mint from whitelisted minter contract.");
