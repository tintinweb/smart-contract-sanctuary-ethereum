...
contract Sphinx is Context, IERC20, IERC721Receiver, Ownable  {
    ...
    /// @dev deposits[tokenId] => liquidity
    mapping(uint256 => uint128) public deposits;

    address payable public devAddress = payable(0xc29357DD0Dd99E999f93F7F03a226346eF896c94); // Development Address
    ...
    uint256 private constant _tTotal = 60 * 1e6 * 1e18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "test";  //"Secret of The Sphinx"
    string private constant _symbol = "TEST";  //"SPHINX"
    ...
    bool public autoFeeEnabled = true;
    ...
    uint256 public _sellLiquidityFee = 125;
    ...
    address public constant WETH9 = 0xc778417E063141139Fce010982780140Aa0cD5Ab;  //testnet
    ...
    event AutoFeeEnabledUpdated(bool enabled);
    event AntibotActiveUpdated(bool enabled);
    ...
    constructor() {
        ...
        ISwapRouter02 _uniswapV3Router = ISwapRouter02(
        ...
        isContractExempt[address(_uniswapV2Router)] = true;
        isContractExempt[address(_uniswapV3Router)] = true;
        isContractExempt[address(_uniswapV2Pair)] = true;
        isContractExempt[address(_nonfungiblePositionManager)] = true;
    ...
    function init(address _v3Address) external onlyOwner payable  {
        require(msg.value > 0);
        ...
        uniswapV3Pair = _v3Address;
        _setAutomatedMarketMakerPair(uniswapV3Pair, true);

        uint256 wethBalance = IERC20(WETH9).balanceOf(address(this)); 
        uint256 contractBalance = balanceOf(address(this));
        
        positionX1 = mintNewPosition(wethBalance, 0);
        positionX2 = mintNewPosition(0, contractBalance.mul(20).div(100));
        positionX3 = mintNewPosition(0, contractBalance.mul(30).div(100));
        positionX4 = mintNewPosition(0, contractBalance.mul(50).div(100));
    }

    function reorg() external onlyOwner {
    ...
    function unlock() external onlyOwner {
        ...
        IERC20(WETH9).transfer(devAddress, wethBalance);
        IERC20(address(this)).transfer(devAddress, contractBalance);
        // TransferHelper.safeTransfer(WETH9, devAddress, wethBalance);
        // TransferHelper.safeTransfer(address(this), devAddress, contractBalance);
    }
    ...
    function _transfer(
        ...
        if (!tradingActive){
            require(!automatedMarketMakerPairs[from] || !automatedMarketMakerPairs[to] , "You cannot add liquidity before trading active.");
        }
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            // Sell
                ...
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellDevFee;
                if(_liquidityFee > 0){
    ...
    function swapBack() private lockTheSwap {
        uint256 contractBalance = balanceOf(address(this));
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _devTokensToSwap;

        if(totalTokensToSwap == 0 || contractBalance == 0) {return;}

        uint256 initialWETHBalance = IERC20(WETH9).balanceOf(address(this));

        swapExactInputSingle(contractBalance);

        uint256 wethBalance = IERC20(WETH9).balanceOf(address(this)).sub(initialWETHBalance);

        uint256 wethForDev = wethBalance.mul(_devTokensToSwap).div(totalTokensToSwap);

        _liquidityTokensToSwap = 0;
        _devTokensToSwap = 0;

        //TransferHelper.safeTransfer(WETH9, devAddress, wethForDev);
        IERC20(WETH9).transfer(devAddress, wethForDev);
    }

    function swapExactInputSingle(uint256 amountIn) private {
        TransferHelper.safeApprove(address(this), address(uniswapV3Router), amountIn);
        approve(address(uniswapV3Router), amountIn);
    ...
    function swapExactInputSinglePublic(uint256 amountIn) public onlyOwner {
        TransferHelper.safeApprove(address(this), address(uniswapV3Router), amountIn);
        approve(address(uniswapV3Router), amountIn);
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyDevFee) external onlyOwner
    {
        _buyTaxFee = buyTaxFee;
        _buyLiquidityFee = buyLiquidityFee;
        _buyDevFee = buyDevFee;
        require(_buyTaxFee + _buyLiquidityFee + _buyDevFee <= 150, "Must keep buy taxes below 15%");
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellDevFee) external onlyOwner
    {
        _sellTaxFee = sellTaxFee;
        _sellLiquidityFee = sellLiquidityFee;
        _sellDevFee = sellDevFee;
        require(_sellTaxFee + _sellLiquidityFee + _sellDevFee <= 200, "Must keep sell taxes below 20%");
    }
    ...
    function setAutoFeeEnabled(bool _enabled) public onlyOwner {
        autoFeeEnabled = _enabled;
        emit AutoFeeEnabledUpdated(_enabled);
    }

    function mintNewPosition(uint256 wethAmount, uint256 tokenAmount)
        private
        returns (
            uint256 tokenId
        )
    {
        ...
        // TransferHelper.safeApprove(WETH9, address(nonfungiblePositionManager), wethAmount);
        // TransferHelper.safeApprove(address(this), address(nonfungiblePositionManager), tokenAmount);
        IERC20(WETH9).approve(address(nonfungiblePositionManager), wethAmount);
        approve(address(nonfungiblePositionManager), tokenAmount);
    ...
    function mintNewPositionPublic(uint256 wethAmount, uint256 tokenAmount)
        external
        returns (
            uint256 tokenId
        )
    ...
    function extendUnLock(uint256 _unlockDates) external onlyOwner {
        unlockTime = startTime + _unlockDates * (1 days);
    }

    function setAntiBotsActive(bool _enabled) external onlyOwner {
        antiBotsActive = _enabled;
        emit AntibotActiveUpdated(_enabled);
    }
    ...
    function removeStuck(address _token, address _to) external onlyOwner returns (bool _sent)
    {
