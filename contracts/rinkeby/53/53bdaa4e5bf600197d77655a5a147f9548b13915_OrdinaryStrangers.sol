...
// File: contracts/OrdinaryStrangers.sol
...
contract OrdinaryStrangers is ERC721A, Ownable {
    using Strings for uint256;
    uint256 public mintPrice = 0.00666 ether;
    uint256 public constant supply = 4444;
    string private unRevealedURI =
        "ipfs://QmVY4tYCYhn3JGQhJBARAooFqtBZrhJ9UrLJDMfw28xHqt/";
    string private baseURI =
        "ipfs://QmccLaXffECvXzYvQp3hmWr6evVcSKWU3LxV5SZddUC7PL/";
    uint256 public maxNftPerWallet = 5;
    bool teamMinted = false;
    bool public paused = false;
    bool public isRevealed = false;

    constructor() ERC721A("Ordinary Strangers", "OSTR") {}

    function toggleReveal() external onlyOwner {isRevealed = !isRevealed;}
    ...
    function teamMint(uint _teamSupply, address _teamWallet) external onlyOwner {
        require(totalSupply() + _teamSupply <= supply, "You can't mint more than the total supply");
        require(!teamMinted, "Team has already minted");
        teamMinted = true;
        _mint(_teamWallet, _teamSupply);
    }
    ...
    function mint(uint256 quantity) external payable {
        ...
        if (msg.sender != owner()) {
            ...
            require(quantity + _numberMinted(msg.sender) <= maxNftPerWallet, string(abi.encodePacked("You can only mint 5 NFTs")));
            require(msg.value >= mintPrice * quantity, "Insufficient funds");
        }
        _mint(msg.sender, quantity);
    }

    function getBaseURI() public view onlyOwner returns (string memory) {return baseURI;}
    ...
    function setMaxNftPerWallet(uint256 max_) external onlyOwner {maxNftPerWallet = max_;}

    function getNotRevealedURL() external view onlyOwner returns (string memory) {return unRevealedURI;}

    function numberMinted(address _address) public view returns (uint256) {return _numberMinted(_address);}
