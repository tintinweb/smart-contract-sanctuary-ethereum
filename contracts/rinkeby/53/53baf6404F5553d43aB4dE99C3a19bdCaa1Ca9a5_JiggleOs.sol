...
contract JiggleOs is ERC721, ReentrancyGuard, Ownable {
    ...
    //@notice The minter, who provides the seed and name, is the progenitor
    function mint(uint32 seed, string memory name) public payable nonReentrant {
    ...
    function drawSVG(uint256 _tokenId) internal view returns(string memory) {
        ...
        string memory seed = string(abi.encodePacked(currentSeed.toString()));
    ...
    function rollMetadata(uint256 _tokenId) internal view returns(string memory) {
        string memory name = string(abi.encodePacked(idToName[_tokenId]));
        string memory description = string(abi.encodePacked("JiggleO #", _tokenId.toString(), "- Generative Ethereum Art"));
        string memory external_url = string(abi.encodePacked(base_external_url, _tokenId.toString()));
        string memory progenitor = string(abi.encodePacked(Strings.toHexString(uint256(uint160(idToSeeder[_tokenId])), 20)));
        string memory seed = string(abi.encodePacked((idToSeed[_tokenId]).toString()));
        string memory image = string(abi.encodePacked("data:image/svg+xml;base64, ", drawSVG(_tokenId) ));
