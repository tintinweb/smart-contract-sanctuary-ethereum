...
contract Powder is ERC20Burnable, Operator {
    using SafeMath for uint256;

    // TOTAL MAX SUPPLY = 50,101 POWDERs
    ...
    uint256 public constant POWDER_GENESIS_REWARD_ALLOCATION = 100 ether;
    ...
    constructor(uint256 _startTime, address _communityFund, address _devFund, address _daoFund) public ERC20("Thrive Shares", "POWDER") {
        _mint(msg.sender, 1 ether); // mint 1 POWDER for initial pools deployment
     ...
     */
    function distributeReward(address _farmingIncentiveFund, address _powderGenesisRewardPool) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_farmingIncentiveFund != address(0), "!_farmingIncentiveFund");
        require(_powderGenesisRewardPool != address(0), "!_powderGenesisPool");
        rewardPoolDistributed = true;
        _mint(_farmingIncentiveFund, FARMING_POOL_REWARD_ALLOCATION);
        _mint(_powderGenesisRewardPool, POWDER_GENESIS_REWARD_ALLOCATION);
    }
