...
contract vestor is Initializable,OwnableUpgradeable, UUPSUpgradeable{
    ...
    mapping(address => uint256[]) idsbyaddress;
    mapping(address => uint256[]) idcountbyinvestor;
   ...
   require(isWhitelisted(msg.sender,_contractid)!=false,"you are not eligible for the claim");
   ...
   emit HTLCERC20Withdraw(msg.sender,_contractid);
    ...
    function getinvestorinfo(address _address)public view returns (address[]memory tokenaddress,uint256[]memory,uint256[]memory){
          uint256 id = fetchcontractsamountbyaddress(_address).length;
        
        uint256[] memory amountofinvestor = new uint256[](id);
        address[] memory _tokenaddress = new address[](id);
        uint256[] memory _contract = fetchcontractsamountbyaddress(_address);

          for (uint i = 0; i < fetchcontractsamountbyaddress(_address).length; i++) {
             _tokenaddress[i] = vestcontracts[i]._tokencontractaddress;
            amountofinvestor[i] =  amountofinvestorsbyindex[_address][i];
