...
import "../client/node_modules/@openzeppelin/contracts/access/AccessControlEnumerable.sol";
...
import "../client/node_modules/@openzeppelin/contracts/interfaces/IERC2981.sol";
import "../client/node_modules/@openzeppelin/contracts/security/Pausable.sol";
import "../client/node_modules/@openzeppelin/contracts/token/ERC1155/extensions/ERC1155Burnable.sol";
...
contract DanLandErc1155 is ERC1155, EIP712, IERC2981, Ownable, AccessControlEnumerable, Pausable, ERC1155Burnable {
    mapping(uint256 => bool) private _mintedLands;
    
    // SIGNER role is used to sign the voucher on the server backend
    bytes32 private constant SIGNER_ROLE = keccak256("SIGNER_ROLE");
    string private constant SIGNING_DOMAIN = "DanLand-Voucher"; // Value is Temporary, will change when ready for production
     ...
     * - `wnkAddress` Address to deposit the winky
    ...
    function buyLandWnk(NFTVoucher calldata voucher) external whenNotPaused {
        ...
        //Create an arra with indexes 1
        uint256[] memory amounts = new uint256[](voucher.tokenIds.length);
        for (uint i = 0; i < voucher.tokenIds.length; i++) {
            amounts[i] = 1;
            
            // Verify if the land has already been minted
            require(_mintedLands[voucher.tokenIds[i]] == false, "Land already minted.");
            _mintedLands[voucher.tokenIds[i]] = true;
        }
        _mintBatch(msg.sender, voucher.tokenIds, amounts, "");
    }
    
    /**
     * @dev Function used to retrieve the Voucher-Signer address
     */
    function validateSigner(NFTVoucher calldata voucher) external view {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Unauthorized operation.");
        address signer = _verify(voucher);
        string memory strSigner = string(abi.encodePacked("Address used for signing: ", Strings.toHexString(uint160(signer), 20)));
        require(hasRole(SIGNER_ROLE, signer), strSigner);
    }
    ...
    function mintByOwner(WalletNfts[] calldata nftsToMint) external {
        ...
        for (uint walletCpt = 0; walletCpt < nftsToMint.length; walletCpt++) {
            ...
            uint256[] memory amounts = new uint256[](walletNfts.tokenIds.length);
            for (uint i = 0; i < walletNfts.tokenIds.length; i++) {
                amounts[i] = 1;
            }
            _mintBatch(walletNfts.walletAddress, walletNfts.tokenIds, amounts, "");
        }
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(ERC1155, IERC165, AccessControlEnumerable) returns (bool) {
