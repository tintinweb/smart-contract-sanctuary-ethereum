...
contract TastyBones is Ownable, ERC721A {
    ...
    bytes32 public presaleMerkleRoot = 0x04dafba28d78cea1c319b6659d3f4f7f790c247a3571bda4a853d8f74f0f70f4;
    ...
    function mintFree(uint256 boneTokenId, uint256 numberOfTokens) public payable {
        uint256 boneCount = approvingBoneContract.balanceOf(msg.sender);
        uint256 corgiPerBoneCount = mintedTBforFreeMint[boneTokenId];
        ...
        require(corgiPerBoneCount + numberOfTokens <= maxFreeMint,"MAX_MINT_REACHED");
    ...
    function mintPresale(uint256 numberOfTokens, bytes32[] calldata proof) public payable {
        require(addressBlockBought[msg.sender] < block.timestamp, "CANNOT_MINT_ON_THE_SAME_BLOCK");
        require(!Address.isContract(msg.sender),"CONTRACTS_NOT_ALLOWED_TO_MINT");
        require(isPresaleActive, "PRESALE_MINT_IS_NOT_YET_ACTIVE");
        require(MerkleProof.verify(proof, presaleMerkleRoot, keccak256(abi.encodePacked(msg.sender))), "INVALID_WHITELIST_SIGNATURE");
        require(msg.value >= price * numberOfTokens, "INSUFFICIENT_PAYMENT");
     ...
     */
    function mintRaffle(uint256 numberOfTokens, bytes calldata whitelistSignature) public payable {
        require(addressBlockBought[msg.sender] < block.timestamp, "CANNOT_MINT_ON_THE_SAME_BLOCK");
        require(!Address.isContract(msg.sender),"CONTRACTS_NOT_ALLOWED_TO_MINT");
        require(isPublicSaleActive, "RAFFLE_MINT_IS_NOT_YET_ACTIVE");
        require(msg.value >= price * numberOfTokens, "MAX_MINT_REACHED");
        require(numberOfTokens <= maxRaffleMint, "You can adopt a maximum of 10 Corgis");
        require(tokenSupply.current().add(numberOfTokens) <= maxTastyBones, "EXCEEDS_MAX_SUPPLY" );
        require(verifyOwnerSignature(keccak256(abi.encode(msg.sender)), whitelistSignature), "Invalid whitelist signature");
        addressBlockBought[msg.sender] = block.timestamp;
     ...
     */
    function mintTBForTeam() public onlyOwner {
     ...
     * check if bone can still be used
     */
    function checkACBoneBalanceForPresale() public view returns(uint256) {
        return mintedTBforPresale[msg.sender];
    }
     ...
     */
    function tokenIdOfOwner(address _owner) public view returns(uint256[] memory) {
        uint256 tokenCount = balanceOf(_owner);
    ...
    function setRoot(bytes32 presaleRoot) external onlyOwner {
