...
contract AuctionHouse is Ownable, ReentrancyGuard {
  ...
  function purchase(uint256 _auctionId) external payable {
    uint256 price = auctions[_auctionId].currentPrice;
    require(msg.value == price, 'Please submit the asking price in order to complete the purchase');
    require(auctions[_auctionId].status == AuctionStatus.LISTED, 'This NFT has already sold');

    uint256 fee = commissionPercentage.mul(price).div(1 ether);
    uint256 amount = price.sub(fee);
    (bool sellerSent, ) = auctions[_auctionId].seller.call{value: amount}('');
    require(sellerSent, 'Failed to send Ether to Receiver');
    address contractAddress = auctions[_auctionId].contractAddress;
