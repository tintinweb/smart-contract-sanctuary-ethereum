...
contract OSBRoyaltyERC721 is ERC721, ERC721Enumerable, ERC2981, Ownable {
    using Strings for uint256;

    uint256 private constant maxBatch = 50;
    uint256 public lastedId;
    string  public baseURI;
    address public receiverRoyaltyFeeDefault;
    uint256 public percentageRoyaltyFeeDefalt;
    ...
    constructor(address _owner, string memory _name, string memory _symbol, address _receiverRoyaltyFee, uint96 _percentageRoyaltyFee) ERC721(_name, _symbol) {
        ...
        if (_receiverRoyaltyFee != address(0)) {
            ...
            receiverRoyaltyFeeDefault  = _receiverRoyaltyFee;
            percentageRoyaltyFeeDefalt = _percentageRoyaltyFee;
    ...
    function mintBatchWithRoyalty(string memory _baseUri, address[] memory _receiverRoyaltyFees, uint96[] memory _percentageRoyaltyFees) external onlyOwnerOrController {
        ...
        for (uint256 i; i < _receiverRoyaltyFees.length; i++) {
            tokenIds[i] = id++;
            if (_receiverRoyaltyFees[i] == address(0) && _percentageRoyaltyFees[i] == 0) {
                _safeMint(_msgSender(), id);
            } else if (_receiverRoyaltyFees[i] == address(0) && _percentageRoyaltyFees[i] > 0) {
                _mintAndSetRoyalty(id, receiverRoyaltyFeeDefault, _percentageRoyaltyFees[i]);
            } else {
                _mintAndSetRoyalty(id, _receiverRoyaltyFees[i], _percentageRoyaltyFees[i]);
            }
        }
        baseURI  = _baseUri;
        lastedId = id;
    ...
    function _mintAndSetRoyalty(uint256 _tokenId, address _receiverRoyaltyFee, uint96 _percentageRoyaltyFee) private {
        require(_percentageRoyaltyFee > 0, "Requires royalty fee more significant than 0");
        _safeMint(_msgSender(), _tokenId);
        _setTokenRoyalty(_tokenId, _receiverRoyaltyFee, _percentageRoyaltyFee);
    }
...
import "../contracts/OSBRoyaltyERC721.sol";

contract $OSBRoyaltyERC721 is OSBRoyaltyERC721 {
    constructor(address _owner, string memory _name, string memory _symbol, address _receiverRoyaltyFee, uint96 _percentageRoyaltyFee) OSBRoyaltyERC721(_owner, _name, _symbol, _receiverRoyaltyFee, _percentageRoyaltyFee) {}
