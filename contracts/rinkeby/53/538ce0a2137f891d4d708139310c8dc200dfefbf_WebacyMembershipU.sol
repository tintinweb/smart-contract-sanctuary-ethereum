...
contract WebacyMembershipU is Initializable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    function setFreePassesAvailable(uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        freePassesAvailable = _amount;
    }

    // For testing only
    function unpayMembership(address _address) external onlyRole(DEFAULT_ADMIN_ROLE) {
        membershipPaid[_address] = false;
        proxyFactory.removeProxy(_address);
    }
}
