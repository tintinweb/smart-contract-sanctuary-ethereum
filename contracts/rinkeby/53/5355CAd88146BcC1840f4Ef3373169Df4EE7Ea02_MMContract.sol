    ...
    Merkle
{
        ...
        nonReentrant
    {
        ...
        require(current + count <= nft.supply(), "Not enough Mints available");
        nft.mint(count, msg.sender);
        ...
        onlyOwner
    {
        ...
        require(current + countTotal <= nft.supply(), "Not enough Mints available");
        for (uint256 j; j < count.length; j++) {
            nft.mint(count[j], recipient[j]);
        }
    }
    ...
    function setNFT(IBaseErc721 _nft) external onlyOwner {
        nft = _nft;
    }
}
...
contract BaseERC721 is ERC721A, Ownable, ReentrancyGuard, Pausable {
    ...
    function mint(uint256 count, address to) external nonReentrant whenNotPaused {
        require(admins[msg.sender], "EOA");
        _callMint(count, to);
    }
