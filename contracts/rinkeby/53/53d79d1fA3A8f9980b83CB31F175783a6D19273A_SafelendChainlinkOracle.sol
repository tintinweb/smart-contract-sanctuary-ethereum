...
contract SafelendChainlinkOracle is PriceOracle {
    ...
    function getUnderlyingPrice(SToken sToken) public view returns (uint) {
        string memory symbol = sToken.symbol();
        if (compareStrings(symbol, "vBNB")) {
            ...
            return getPrice(sToken);
        }
    }

    function getPrice(SToken sToken) internal view returns (uint price) {
    ...
    function setUnderlyingPrice(SToken sToken, uint underlyingPriceMantissa) external onlyAdmin() {
...
/**
 * @title Safelend's SErc20 Contract
...
contract SErc20 is SToken, SErc20Interface {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrow(address borrower, uint repayAmount, STokenInterface cTokenCollateral) external returns (uint) {
...
/**
 * @title Safelend's SToken Contract
...
contract SToken is STokenInterface, Exponential, TokenErrorReporter {
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, STokenInterface cTokenCollateral) internal nonReentrant returns (uint, uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, STokenInterface cTokenCollateral) internal returns (uint, uint) {
...
/**
  * @title Safelend's InterestRateModel Interface
  * @author Safelend
  */
