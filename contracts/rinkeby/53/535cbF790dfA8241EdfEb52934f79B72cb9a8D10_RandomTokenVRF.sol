...
contract RandomTokenVRF is VRFConsumerBaseV2, Ownable {
    event RandomSeedFilfullmentSuccess(uint256 requestId, uint256 randomSeed);
    event RandomSeedFilfullmentFail(uint256 requestId, uint256 randomSeed);
    ...
    // RNG token internal variables
    uint256 public totalMinted;
    uint256 private _currentRng;
    ...
    function getIndex() external view returns (uint256) {
        uint256 _remaining = MAX_SUPPLY - totalMinted;
        return 1 + (_currentRng % _remaining);
    }
    ...
    function getRandomSeed() external view onlyOwner returns (uint256) {
        return randomSeed;
    }
    // @TESTONLY MUST remove on final deployment
    ...
    function mint(uint256 _qty) external {
        ...
        require(_qty > 0, "Quantity should be more than 1");
        require(minted + _qty <= max, "Reached max supply");
    ...
    function fulfillRandomWords(uint256 _reqId, uint256[] memory _randomWords) internal override {
        ...
        if (_randomWords[0] > 0) {
            ...
            emit RandomSeedFilfullmentSuccess(_reqId, _randomWords[0]);
        } else {
            emit RandomSeedFilfullmentFail(_reqId, _randomWords[0]);
        }
    }
}
