...
contract PumaNitroPass is ERC721A, Ownable {
    ...
    address public TokenContract = 0x54fd1a39379Ce49c85F8F2a8e0fA3ce56edc0147;
    ...
    function burnNitroPassAndMintForeverPhysical(uint256 NitroPassId, address mintToAddress) external {
        require(state == Status.ENDED, "PUMA Nitropass: Minting is still live");
        require(msg.sender == ForeverContract || msg.sender == PhysicalContract, "PUMA Nitropass: Invalid contract calling Nitropass functions");
        PumaPhysical(PhysicalContract).mint(mintToAddress, 1);
        PumaForever(ForeverContract).mint(mintToAddress, 1);
        _burn(NitroPassId);
    }
    ...
    function burnNitroTokenforNitroPass(uint256[] memory tokenIds) external {
