...
import "./compound/NftPriceOracle.sol";
...
contract CNftPriceOracle is NftPriceOracle {
    ...
    /// @dev Mapping from CToken address to underlying NFTX token address.
    ...
    function addAddressMapping(
        ...
        for (uint256 i = 0; i < cNfts.length; ++i) {
            require(
                underlyingNftxTokenAddress[cNfts[i]] == address(0),
                "CNftPriceOracle: Cannot overwrite existing address mappings."
            );
            underlyingNftxTokenAddress[cNfts[i]] = nftxTokens[i];
        }
    }
    ...
    function getUnderlyingPrice(CNftInterface cNft)
        ...
        returns (uint256)
    {
        address nftxToken = underlyingNftxTokenAddress[address(cNft)];
        require(
            nftxToken != address(0),
...
abstract contract CNftInterface {
    ...
    function seize(address liquidator, address borrower, uint256[] calldata seizeIds, uint256[] calldata seizeAmounts) virtual  external;
}
