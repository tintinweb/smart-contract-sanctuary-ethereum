...
contract TAUT is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  modifier onlyAmountMltipleOfThousand(uint256 amount) {
    require(amount % (1000 * 10**decimals()) == 0, "Must be multiple of 1000 tokens");
    _;
  }
   ...
   */
  function burn(bytes16 id, uint256 amount) external onlyAmountMltipleOfThousand(amount) {
   ...
   * @param amount The amount of tokens to burn
   */
  function burnFromAccount(address account, uint256 amount)
    external
    onlyOwner
    onlyAmountMltipleOfThousand(amount)
  {
