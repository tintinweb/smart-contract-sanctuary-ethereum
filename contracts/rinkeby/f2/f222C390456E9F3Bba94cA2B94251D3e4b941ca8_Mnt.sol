...
abstract contract MntVotes is IERC20Votes, ERC20Permit, AccessControl {
    ...
    function getPastVotes(address account, uint256 blockNumber) public view returns (uint256) {
        require(blockNumber < block.number, ErrorCodes.MV_BLOCK_NOT_YET_MINED);
    ...
    function delegateBySig(
        ...
        require(block.timestamp <= expiry, ErrorCodes.MV_SIGNATURE_EXPIRED);
        ...
        require(nonce == _useNonce(signer), ErrorCodes.MV_INVALID_NONCE);
    ...
    modifier buybackOnly() {
        require(buyback != Buyback(address(0)) && address(buyback) == msg.sender, ErrorCodes.UNAUTHORIZED);
        _;
    }

    /// @dev Throws if called by any account other than the governor.
    modifier governorOnly() {
        require(governor != MntGovernor(payable(0)) && address(governor) == msg.sender, ErrorCodes.UNAUTHORIZED);
        _;
    }

    // TODO: add test for buyback setter
    ...
    function setGovernor(MntGovernor newGovernor_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(address(newGovernor_) != address(0), ErrorCodes.ZERO_ADDRESS);
        MntGovernor oldGovernor = governor;
        require(oldGovernor == MntGovernor(payable(0)), ErrorCodes.SECOND_INITIALIZATION);
    ...
    function updateVotesForAccount(
        ...
        require(account != address(0), ErrorCodes.TARGET_ADDRESS_CANNOT_BE_ZERO);
    ...
    function setMaxNonVotingPeriod(uint256 newPeriod_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newPeriod_ >= 90 days && newPeriod_ <= 2 * SECS_PER_YEAR, ErrorCodes.MNT_INVALID_NONVOTING_PERIOD);
...
contract Buyback is AccessControl {
    ...
    /// @dev Value is the Keccak-256 hash of "EMISSION_COMMISSION"
    bytes32 public constant EMISSION_COMMISSION =
        bytes32(0x28f6e1d2a2ff3901c43fd6f58e617abae9a11d1cccfc4e9c77b5e4b43648e0d2);
    ...
    /// @notice How much MNT is available for claim
    ...
    /// @notice is claim function paused
    bool public isClaimRewardPaused;
    ...
    event LeaveBuyback(address who);
    ...
    /// @notice function to change unstake enabled mode
    /// @param isPaused_ new state of _claimReward allowance
    function setClaimRewardPaused(bool isPaused_) external onlyRole(GATEKEEPER) {
        emit BuybackActionPaused("ClaimReward", isPaused_);
        isClaimRewardPaused = isPaused_;
    }
    ...
    /// Stakes the specified amount of MNT and transfers them to this contract.
    /// account's weight would increase by passed amount of staked funds.
    /// msg.sender must be the EmissionCommission Contract.
    /// @param account_ Account how stakes in buyback
    /// @param amount_ The amount of staking tokens
    function stakeEmissionCommission(address account_, uint256 amount_) external onlyRole(EMISSION_COMMISSION) {
        require(isParticipating(account_), ErrorCodes.NOT_PARTICIPATING_IN_BUYBACK);

        StakeData storage staked = stakes[account_];
        staked.amount += amount_;
        staked.discounted += amount_;

        _restakeFor(account_);
        mnt.safeTransferFrom(msg.sender, address(this), amount_);
    }

    /// @notice Unstakes the specified amount of MNT and transfers them back to sender.
    ///         Sender's weight would decrease by discounted amount of unstaked funds,
    ///         but resulting weight would not be greater than staked amount left.
    ...
    function unstake(uint256 amount_) external {
        ...
        uint256 newStaked = staked.amount - amount_;
        if (newStaked > 0) {
            staked.amount = newStaked;
            ...
            staked.discounted = newDiscounted < newStaked ? newDiscounted : newStaked;
        } else {
    ...
    function _claimReward(address who_, MemberData storage member_) internal {
        require(!isClaimRewardPaused, ErrorCodes.OPERATION_PAUSED);
    ...
    function _leave(address participant_) internal {
        ...
        uint256 stakeAmount = stakes[participant_].amount;
        weightSum -= members[participant_].weight;
        delete members[participant_];
        delete stakes[participant_];

        emit LeaveBuyback(participant_);

        if (stakeAmount > 0) {
            emit Unstake(participant_, stakeAmount);
            mnt.safeTransfer(participant_, stakeAmount);
        }
...
library ErrorCodes {
    ...
    string internal constant UNTRUSTED_LIQUIDATOR = "E105";
    string internal constant CALLER_IS_NOT_THE_SUPERVISOR = "E106";
    ...
    string internal constant RECEIVED_INCORRECT_DEBT_REDEMPTION_RATE = "E218";
    ...
    string internal constant EC_INVALID_ARRAY_LENGTH = "E265";
    ...
    string internal constant LIQUIDATE_ONLY_RELIABLE_LOANS = "E316";
    string internal constant BORROWER_LIQUIDATED_NOT_COMPLETELY = "E317";
    string internal constant LIQUIDATE_ONLY_FORGIVABLE_LOANS = "E318";
    string internal constant NOT_ENOUGH_PROTOCOL_INTEREST_TO_REPAY = "E319";
    string internal constant RATE_NOT_IN_RANGE = "E320";
    string internal constant IMPOSSIBLE_TO_BUY_REQUIRED_REPAY_AMOUNT = "E321";
    string internal constant EB_INDEX_SHOULD_BE_GREATER_THAN_INITIAL = "E322";
    string internal constant NO_VESTING_SCHEDULES = "E323";
    string internal constant INSUFFICIENT_UNRELEASED_TOKENS = "E324";
    string internal constant INSUFFICIENT_FUNDS = "E325";
    string internal constant ORACLE_PRICE_EXPIRED = "E326";
    string internal constant TOKEN_NOT_FOUND = "E327";
    string internal constant RECEIVED_PRICE_HAS_INVALID_ROUND = "E328";
    string internal constant FL_PULL_AMOUNT_IS_TOO_LOW = "E329";
    string internal constant INSUFFICIENT_TOTAL_PROTOCOL_INTEREST = "E330";
    string internal constant BB_ACCOUNT_RECENTLY_VOTED = "E331";
    string internal constant BUYBACK_DRIPS_ALREADY_HAPPENED = "E332";
    ...
    string internal constant SWAP_FEE_GREATER_THAN_100_PERCENT = "E407";
    string internal constant LIQUIDATOR_NOT_EXIST = "E408";
    string internal constant LIQUIDATOR_ALREADY_EXISTS = "E409";
    string internal constant ORACLE_ADDRESS_CANNOT_BE_ZERO = "E410";
    string internal constant UNDERLYING_TOKENS_DECIMALS_SHOULD_BE_GREATER_THAN_ZERO = "E411";
    string internal constant REPORTER_MULTIPLIER_SHOULD_BE_GREATER_THAN_ZERO = "E412";
    string internal constant CONTRACT_ALREADY_SET = "E413";
    string internal constant INVALID_TOKEN = "E414";
    string internal constant INVALID_PROTOCOL_INTEREST_FACTOR_MANTISSA = "E415";
    string internal constant INVALID_REDUCE_AMOUNT = "E416";
    string internal constant LIQUIDATION_FEE_MANTISSA_SHOULD_BE_GREATER_THAN_ZERO = "E417";
    string internal constant GATE_KEEPER_ADDRESS_CANNOT_BE_ZERO = "E418";
    string internal constant INVALID_UTILISATION_FACTOR_MANTISSA = "E419";
    string internal constant INVALID_MTOKENS_OR_BORROW_CAPS = "E420";
    string internal constant FL_PARAM_IS_TOO_LARGE = "E421";
    string internal constant BB_DISTRIBUTOR_IS_ZERO = "E422";
    string internal constant NEW_VALUE_IS_TOO_BIG = "E423"; // New value has to be less than partialLiquidationMinSum
    string internal constant NFT_MINTER_CANT_BE_ZERO_ADDRESS = "E424";
    string internal constant MNT_INVALID_NONVOTING_PERIOD = "E425";
    string internal constant INPUT_ARRAY_LENGTHS_ARE_NOT_EQUAL = "E426";
    string internal constant EC_INVALID_BOOSTS = "E427";
    string internal constant EC_ACCOUNT_IS_ALREADY_LIQUIDITY_PROVIDER = "E428";
    string internal constant EC_INVALID_PROVIDER_REPRESENTATIVE = "E429";
    string internal constant EC_PROVIDER_CANT_BE_REPRESENTATIVE = "E430";
}
    ...
    GovernorTimelockControlUpgradeable
{
    ...
    modifier onlyProposer() {
        ...
        require(ac.hasRole(PROPOSER_ROLE, msg.sender), ErrorCodes.UNAUTHORIZED);
        _;
    }
...
contract Supervisor is SupervisorV1Storage, SupervisorInterface {
    ...
    event NewEmissionCommission(EmissionCommission oldEmissionCommission, EmissionCommission newEmissionCommission);
    ...
    /**
     * @notice accrues interest for each market the borrower is in
     * @param borrower The address of the borrower
     */
    function accrueAutoLiquidation(address borrower) external override {
        MToken[] memory borrowerMarkets = accountAssets[borrower];
        // For each market the borrower is in
        for (uint256 i = 0; i < borrowerMarkets.length; i++) {
            //slither-disable-next-line calls-loop
            borrowerMarkets[i].accrueInterest();
        }
    }
    ...
    /**
     * @notice Reset account's liquidation attempts depending on supplyAmount
     * @param supplier Account whose attempts will be updated
     * @param supplyAmount Account supply amount
     */
    function resetLiquidationAttempts(address supplier, uint256 supplyAmount) external override {
        require(markets[msg.sender].isListed, ErrorCodes.MARKET_NOT_LISTED);

        if (address(liquidator) == address(0)) return;

        if (markets[address(msg.sender)].accountMembership[supplier]) {
            liquidator.resetAccountLiquidationAttempts(supplier, MToken(msg.sender), supplyAmount);
        }
    }
    ...
    /// @notice function to set EmissionCommission contract
    /// @param newEmissionCommission_ new EmissionCommission contract address
    function setEmissionCommission(EmissionCommission newEmissionCommission_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        EmissionCommission oldEmissionCommission = emissionCommission;
        emissionCommission = newEmissionCommission_;
        emit NewEmissionCommission(oldEmissionCommission, newEmissionCommission_);
    }
    ...
    /**
     * @dev Calculate MNT accrued by a supplier. The calculation takes into account NFT emission boosts
    ...
    function distributeSupplierMnt(address mToken, address supplier) internal {
        // TODO: Don't distribute supplier MNT if the user is not in the supplier market.
        // This check should be as gas efficient as possible as distributeSupplierMnt is called in many places.
        ...
        uint256 representativeCommissionRate = 0;
        uint256 deltaIndexBoost = 0;

        // Calculate change in the cumulative sum of the MNT per mToken accrued (with considering BDR boost)
        if (address(emissionCommission) != address(0)) {
            //slither-disable-next-line calls-loop
            (representative, representativeCommissionRate, deltaIndexBoost) = emissionCommission.calculateEmissionBoost(
                supplier,
                deltaIndex
            );
        }
        ...
        if (accrueDelta > 0) {
            ...
            if (representative != address(0)) {
                uint256 representativeAccruedDelta = (accrueDelta * representativeCommissionRate) / EXP_SCALE;
    ...
    /**
     * @dev Calculate MNT accrued by a borrower. The calculation takes into account NFT emission boosts
    ...
    function distributeBorrowerMnt(
        ...
        // TODO: Don't distribute borrower MNT if the user is not in the borrower market.
        // This check should be as gas efficient as possible as distributeBorrowerMnt is called in many places.
        // - We really don't want to call an external contract as that's quite expensive.
    ...
    /**
     * @notice Withdraw all the mnt accrued by holder in all markets
     * @param holder The address to withdraw MNT for
     */
    function withdrawMnt(address holder) external {
        return withdrawMnt(holder, allMarkets);
    }

    /**
     * @notice Withdraw all the mnt accrued by holder in the specified markets
     * @param holder The address to withdraw MNT for
     * @param mTokens The list of markets to withdraw MNT in
     */
    function withdrawMnt(address holder, MToken[] memory mTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        withdrawMnt(holders, mTokens, true, true);
    }

    /**
     * @notice Withdraw all mnt accrued by the holders
     * @param holders The addresses to withdraw MNT for
     * @param mTokens The list of markets to withdraw MNT in
     * @param borrowers Whether or not to withdraw MNT earned by borrowing
     * @param suppliers Whether or not to withdraw MNT earned by supplying
     */
    //slither-disable-next-line reentrancy-no-eth
    function withdrawMnt(
        address[] memory holders,
        MToken[] memory mTokens,
        bool borrowers,
        bool suppliers
        ...
        // We are distributing MNT tokens for holders from `mTokens` markets
        distributeMnt(holders, mTokens, borrowers, suppliers);
        ...
        for (uint256 j = 0; j < holders.length; j++) {
            ...
            mntAccrued[holder] = grantMntInternal(holder, mntAccrued[holder]);
    ...
    function getMntAddress() public view virtual returns (address) {
        //TODO: Replace with MNT address during deployment.
        return mntAddress;
    }
...
contract Vesting is AccessControl {
     ...
     * all tokens (49,996,790 MNT tokens) are vested according to the account's vesting schedules
     * and this value is equal to zero.
     */
    uint256 public freeAllocation = 49_996_790 ether;
    ...
    function createVestingScheduleBatch(ScheduleData[] memory schedulesData) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            uint256 mntRemaining = mnt.balanceOf(address(this));
            require(
                freeAllocation >= schedule.totalAmount && mntRemaining >= schedule.totalAmount,
...
contract MToken is MTokenInterface, MTokenStorage {
    ...
    function lendFresh(address lender, uint256 lendAmount) internal nonReentrant returns (uint256 actualLendAmount) {
        ...
        supervisor.resetLiquidationAttempts(lender, lendAmount);
    ...
    /**
     * @notice In case of forgivable liquidation and lack of account's collateral repay toxic debt out of
     *         protocol interest.
     * @dev Used in automation liquidation for Forgivable case, msg.sender must be the liquidator.
     *      The caller must ensure that the accrueInterest() has been called.
     *      We do not updateMntBorrowIndex and distributeBorrowerMnt due to the fact that we want to achieve
     *      a minimum impact of "dead" debt on the emission of MNT for other users
     * @param borrower_ The address of the borrower
     * @return The amount of protocolInterest used to repay debt
     */
    //slither-disable-next-line reentrancy-benign
    function autoLiquidationRepayDeadBorrow(address borrower_) external nonReentrant returns (uint256) {
        //slither-disable-next-line reentrancy-events
        supervisor.beforeAutoLiquidationRepay(msg.sender, borrower_, address(this), borrowIndex.toUint224());

        uint256 actualRepayAmount = 0;
        uint256 _totalProtocolInterest = totalProtocolInterest;

        if (_totalProtocolInterest > 0) {
            uint256 _accountBorrows = borrowBalanceStoredInternal(borrower_);

            actualRepayAmount = Math.min(_accountBorrows, _totalProtocolInterest);

            totalBorrows -= actualRepayAmount;
            accountBorrows[borrower_].principal = _accountBorrows - actualRepayAmount;
            accountBorrows[borrower_].interestIndex = borrowIndex;
            totalProtocolInterest -= actualRepayAmount;

            emit RepayDeadBorrow(
                borrower_,
                actualRepayAmount,
                accountBorrows[borrower_].principal,
                totalBorrows,
    ...
    function autoLiquidationSeize(
        ...
        // Infinity means ForgivableComplete liquidation has occurred - all account's collateral has to be burn.
...
interface SupervisorInterface is IERC165 {
    ...
    function resetLiquidationAttempts(address supplier, uint256 supplyAmount) external;
    ...
    function accrueAutoLiquidation(address borrower) external;
...
import "./Buyback.sol";
import "./EmissionCommission.sol";
import "./EmissionBooster.sol";
import "./Liquidation.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import "@openzeppelin/contracts/access/AccessControl.sol";

contract SupervisorV1Storage is AccessControl, ReentrancyGuard {
     ...
     */
    // TODO Remove slither's comment after we start using this variable
    ...
    /// @notice Contract to create BDR agreement and calculate rewards for representative and provider
    EmissionCommission public emissionCommission;

    //TODO: Replace with MNT address during deployment.
    address internal mntAddress;
}
...
interface MTokenInterface is IERC20, IERC3156FlashLender, IERC165 {
    ...
    /**
     * @notice Event emitted when a dead borrow is repaid
     */
    event RepayDeadBorrow(
        address borrower,
        uint256 repayAmount,
        uint256 accountBorrowsNew,
        uint256 totalBorrowsNew,
...
contract EmissionCommission is AccessControl {
    uint256 internal constant EXP_SCALE = 1e18;
    using SafeERC20 for Mnt;

    struct CommissionAgreement {
        /// Emissions Boost for liquidity providers
        uint256 liquidityProviderBoost;
        /// Percentage of the total emissions earned by the Business Development Representative
        uint256 commissionRate;
        /// The number of the block in which Commission Agreement ends.
        uint32 endBlock;
        /// Business Development Representative
        address representative;
    }
    /// Linking the provider with the agreement
    mapping(address => CommissionAgreement) public providersAgreement;
    /// Counts providers of the representative
    mapping(address => uint256) public representativesProviderCounter;

    Supervisor public supervisor;

    event CommissionAgreementAdded(
    ...
    event CommissionAgreementEnded(
        address indexed provider,
        address indexed representative,
        uint256 commissionRate,
        uint256 liquidityProviderBoost,
    ...
    /**
     * @notice Creates a new Commission Agreement between Liquidity provider and Business Development Representative
     * @dev Admin function to create a new Commission Agreement
     * @param provider_ address of the liquidity provider
     * @param representative_ address of the liquidity provider representative.
     * @param commissionRate_ percentage of the boost for representative
     ...
     */
    function createCommissionAgreement(
        address provider_,
        ...
        // (1 + providerBoost) * (1 + representativeBoost) <= 150%
        require(
            (EXP_SCALE + liquidityProviderBoost_) * (EXP_SCALE + commissionRate_) <= 1.5e36,
            ErrorCodes.EC_INVALID_BOOSTS
        );
        // one account at one time can be a provider once,
        require(!isAccountLiquidityProvider(provider_), ErrorCodes.EC_ACCOUNT_IS_ALREADY_LIQUIDITY_PROVIDER);
        // one account can't be a liquidity provider and a representative at the same time
        require(
            !isAccountRepresentative(provider_) && !isAccountLiquidityProvider(representative_),
            ErrorCodes.EC_PROVIDER_CANT_BE_REPRESENTATIVE
        );

        // we are distribution MNT tokens for liquidity provider
        // slither-disable-next-line reentrancy-no-eth,reentrancy-benign,reentrancy-events
        supervisor.distributeAllMnt(provider_);

        // we are creating Commission Agreement between Liquidity Provider and Business Development Representative
        providersAgreement[provider_] = CommissionAgreement({
    ...
    /**
     * @notice Removes a new Commission Agreement between Liquidity provider and Business Development Representative
     * @dev Admin function to remove a Commission Agreement
     * @param provider_ address of the liquidity provider
     * @param representative_ address of the liquidity provider representative.
     */
    function removeCommissionAgreement(address provider_, address representative_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        CommissionAgreement storage agreement = providersAgreement[provider_];
        require(agreement.representative == representative_, ErrorCodes.EC_INVALID_PROVIDER_REPRESENTATIVE);

        emit CommissionAgreementEnded(
            provider_,
            representative_,
        ...
        // We call emission system for liquidity provider, so provider and his representative will accrue
        // MNT tokens with their emission boosts
        // slither-disable-next-line reentrancy-no-eth,reentrancy-benign
        supervisor.distributeAllMnt(provider_);

        // We remove agreement between liquidity provider and business development representative
        delete providersAgreement[provider_];
        representativesProviderCounter[representative_]--;
    }
    ...
    /**
     * @notice Calculates emission commission boosts for liquidity provider and business representative,
     *         also removes the agreement if it is not active anymore.
     * @param provider_ address of the liquidity provider,
     * @param deltaIndex_ difference between the current MNT index and the index of the last update for
     *        the Liquidity Provider
     */
    function calculateEmissionBoost(address provider_, uint256 deltaIndex_)
        external
        returns (
            address representative,
            uint256 commissionRate,
    ...
    {
        // get a Business Representative for the account_ and his commission rate
        CommissionAgreement storage agreement = providersAgreement[provider_];
        ...
        // we are removing the commission agreement if it has expired
        if (isAgreementExpired(provider_)) {
            delete providersAgreement[provider_];
            representativesProviderCounter[representative]--;
        }
    }
    ...
    function isAccountLiquidityProvider(address account_) public view returns (bool) {
        return providersAgreement[account_].representative != address(0);
    }

    /**
     * @notice checks if `account_` is business development representative.
     * @dev account_ is business development representative if he has providers.
     * @param account_ address to check
     ...
     * @dev the calling code must be sure that the `address provider_` is a valid liquidity provider.
     * @param provider_ address of the liquidity provider
     * @return `true` if `provider_` is not expired, otherwise returns false
     */
    function isAgreementExpired(address provider_) public view returns (bool) {
        return providersAgreement[provider_].endBlock <= _getBlockNumber();
    }
...
/**
 * This contract provides the liquidation functionality. In the Minterest protocol, liquidation
 * is based on the principle of “all loans - all collaterals”.
 * The protocol provides four types of liquidation: partial, complete, forgivable, hopeless.
 * In case of the partial liquidation, the balance is withdrawn from the account's borrows and
 * collaterals in order to make the account's loan to a safe state. In case of complete liquidation,
 * the account's entire borrow is written off, and all the account's collateral is withdrawn.
 */

contract Liquidation is AccessControl, ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint256 private constant EXP_SCALE = 1e18;

    /**
     * @notice The maximum number of partial liquidations a account has. After reaching this parameter,
     * a complete liquidation occurs
     */
    uint256 public partialLiquidationMaxAttempts;

    /**
     * @notice Minimal sum for partial liquidation. Loans with amount below this parameter will be liquidate in full
     */
    uint256 public partialLiquidationMinSum;

    /**
     * @notice Step used in liquidation to protect the account from micro liquidations, scaled by 1e18
     */
    uint256 public safetyBuffer = 0.04e18; // 4.0%

    /**
     * @notice Allowable deviation of currentSupplyRatio from saveSupplyRatio, scaled by 1e18
     */
    uint256 public safeDeviationThreshold = 0.01e18; // 1.0%
    ...
    /**
     * @notice Counter of the number of partial liquidations at the account
     */
    // slither-disable-next-line uninitialized-state
    mapping(address => uint256) public accountLiquidationAttempts;
    ...
    event PartialLiquidationMaxAttemptsChanged(uint256 oldValue, uint256 newValue);
    event PartialLiquidationMinSumChanged(uint256 oldValue, uint256 newValue);
    event SafetyBufferChanged(uint256 oldValue, uint256 newValue);
    event SafeDeviationThresholdChanged(uint256 oldValue, uint256 newValue);
    event TrustedLiquidatorAdded(address newLiquidator);
    event TrustedLiquidatorRemoved(address liquidatorToRemove);
    ...
    event ForgivableLiquidation(
        ...
        uint256[] repayAmounts,
        uint256[] interestUsedAmounts
    );

    /**
     * @notice Construct a Liquidation contract
     * @param partialLiquidationMaxAttempts_ The maximum number of partial liquidations
     * @param partialLiquidationMinSum_ Minimal sum for partial liquidation
     ...
     */
    constructor(
        uint256 partialLiquidationMaxAttempts_,
        uint256 partialLiquidationMinSum_,
        ...
        partialLiquidationMaxAttempts = partialLiquidationMaxAttempts_;
        partialLiquidationMinSum = partialLiquidationMinSum_;
    ...
    /**
     * @dev Possible liquidation types that we can work with
     */
    enum LiquidationType {
        // This type of liquidation in which the borrower is returned to a safe position at the expense of the minimum
        // possible amount of debt redemption.
        // Partial liquidation can be applied a partialLiquidationMaxAttempts number of times
        Partial,
        // This type of liquidation in which the entire debt position of the borrower is repaid.
        Complete,
        // This type of liquidation applies when accountTotalSeize > accountTotalSupply. As in the case with Complete,
        // the entire debt position is repaid.
        // If the account's collateral is not enough to repay the entire debt position, the remainder of the debt
        // is repaid at the expense of the market's protocol interest.
        ForgivableComplete,
        // This type of liquidation applies when accountTotalSupply == 0 && accountTotalBorrow > 0
        // Hopeless debt is repaid at the expense of the market's protocol interest.
        Hopeless
    }
    ...
    /**
     * @notice Liquidate account with "Healthy" debt
     * @param borrower_ Account which is being liquidated
     * @param drRateMantissa_ debt redemption rate, shows what percentage of a unit of a debt asset must be repaid to
              return the account to a safe position (scaled by 1e18).
     */
    //slither-disable-next-line reentrancy-benign
    function liquidateUnsafeLoan(address borrower_, uint256 drRateMantissa_)
        external
        onlyRole(TRUSTED_LIQUIDATOR)
        nonReentrant
    {
        AccountLiquidationAmounts memory accountState;

        require(drRateMantissa_ <= EXP_SCALE, ErrorCodes.INVALID_DEBT_REDEMPTION_RATE);

        //slither-disable-next-line reentrancy-events
        supervisor.accrueAutoLiquidation(borrower_);
        accountState = calculateLiquidationAmounts(borrower_, drRateMantissa_);
        ...
        LiquidationType liquidationType = chooseLiquidationType(
            borrower_,
            accountState.accountTotalSupplyUsd,
            accountState.accountTotalBorrowUsd,
            accountState.accountPresumedTotalSeizeUsd
        );

        require(
            liquidationType == LiquidationType.Partial || liquidationType == LiquidationType.Complete,
            ErrorCodes.LIQUIDATE_ONLY_RELIABLE_LOANS
        );
        require(approveDrRate(drRateMantissa_, liquidationType), ErrorCodes.RECEIVED_INCORRECT_DEBT_REDEMPTION_RATE);

        bool isManualLiquidation = hasRole(MANUAL_LIQUIDATOR, msg.sender);

        mutateAccountLiquidationAttempts(borrower_, liquidationType);
        seize(
            borrower_,
            accountState.marketAddresses,
            accountState.seizeAmounts,
            accountState.accountTotalBorrowUsd <= insignificantLoanThreshold,
            isManualLiquidation,
            liquidationType
        );
        repay(borrower_, accountState.marketAddresses, accountState.repayAmounts, isManualLiquidation);

        if (liquidationType == LiquidationType.Partial) {
            require(
                approveBorrowerSupplyRatio(borrower_, accountState.saveSupplyRatio, accountState.marketAddresses),
                ErrorCodes.RATE_NOT_IN_RANGE
            );
        } else {
            require(
                checkLiquidatedCompletely(borrower_, accountState.marketAddresses, accountState.repayAmounts),
                ErrorCodes.BORROWER_LIQUIDATED_NOT_COMPLETELY
            );
        }
        ...
        emit ReliableLiquidation(
            ...
            accountState.marketAddresses,
            accountState.seizeAmounts,
            accountState.repayAmounts
        );
    }

    /**
     * @notice Liquidate account with "Unhealthy" debt
     * @param borrower_ Account which is being liquidated
     */
    //slither-disable-next-line reentrancy-benign
    function liquidateForgivableLoan(address borrower_) external onlyRole(TRUSTED_LIQUIDATOR) nonReentrant {
        AccountLiquidationAmounts memory accountState;

        //slither-disable-next-line reentrancy-events
        supervisor.accrueAutoLiquidation(borrower_);
        accountState = calculateLiquidationAmounts(borrower_, EXP_SCALE);
        ...
        require(
            liquidationType == LiquidationType.ForgivableComplete || liquidationType == LiquidationType.Hopeless,
            ErrorCodes.LIQUIDATE_ONLY_FORGIVABLE_LOANS
        );
        ...
        seize(
            borrower_,
            accountState.marketAddresses,
            accountState.supplyAmounts,
            accountState.accountTotalBorrowUsd <= insignificantLoanThreshold,
            isManualLiquidation,
            liquidationType
        );
        accountState.interestUsedAmounts = repayDeadBorrows(
            borrower_,
            accountState.marketAddresses,
            accountState.repayAmounts,
            isManualLiquidation
        ...
        emit ForgivableLiquidation(
            ...
            accountState.supplyAmounts,
            accountState.repayAmounts,
            accountState.interestUsedAmounts
        );
    }

    /**
     * @notice Reset account's liquidation attempts depending on supplyAmount
     * @param supplier_ Account which is being liquidated
     * @param market_ The market in which the account makes a supply
     * @param supplyAmount_ Account supply amount
     */
    function resetAccountLiquidationAttempts(
        address supplier_,
        MToken market_,
        uint256 supplyAmount_
    ) external {
        require(Supervisor(msg.sender) == supervisor, ErrorCodes.CALLER_IS_NOT_THE_SUPERVISOR);

        uint256 supplyAmountUsd = (supplyAmount_ * oracle.getUnderlyingPrice(market_)) / EXP_SCALE;

        if (supplyAmountUsd >= partialLiquidationMinSum && accountLiquidationAttempts[supplier_] != 0) {
            accountLiquidationAttempts[supplier_] = 0;
        }
    }

    /**
     * @dev Mutate account's liquidation attempts depending on liquidation type
     * @param borrower_ The address of the borrower
     * @param liquidationType_ The type of liquidation that corresponds to the state of the account
     */
    function mutateAccountLiquidationAttempts(address borrower_, LiquidationType liquidationType_) internal {
        if (liquidationType_ == LiquidationType.Partial) {
            accountLiquidationAttempts[borrower_]++;
        } else {
            accountLiquidationAttempts[borrower_] = 0;
        }
    }

    /**
     * @dev In case of forgivable liquidation and lack of account's collateral repay toxic debts out of
     *      protocol interest. Used in automation liquidation for Forgivable case
     * @param borrower_ The address of the borrower
     ...
     * @return (interestUsedAmounts)
     *         interestUsedAmounts: Array of protocolInterest amounts used to repay
     */
    function repayDeadBorrows(
        address borrower_,
        MToken[] memory marketAddresses_,
        uint256[] memory repayAmounts_,
        bool isManualLiquidation_
    ) internal returns (uint256[] memory) {
        uint256[] memory interestUsedAmounts = new uint256[](marketAddresses_.length);
        bool isAnyDebtRepaid = false;
        for (uint256 i = 0; i < marketAddresses_.length; i++) {
            ...
            if (isManualLiquidation_) {
                market.addProtocolInterestBehalf(msg.sender, repayAmount);
            }

            uint256 interestUsedAmount = market.autoLiquidationRepayDeadBorrow(borrower_);
            if (interestUsedAmount > 0) {
                (interestUsedAmounts[i], isAnyDebtRepaid) = (interestUsedAmount, true);
            }
        }
        require(isAnyDebtRepaid, ErrorCodes.NOT_ENOUGH_PROTOCOL_INTEREST_TO_REPAY);
        return interestUsedAmounts;
    }

    /**
     * @dev Checks if the borrower has been completely liquidated
     * @param borrower_ The account whose debts is being checked
     * @param marketAddresses_ Array of markets the borrower is in
     * @param repayAmounts_ Array of repay amounts in underlying assets
     * @return Whether or not the verification succeeded
     */
    function checkLiquidatedCompletely(
        address borrower_,
        MToken[] memory marketAddresses_,
        uint256[] memory repayAmounts_
        ...
        for (uint256 i = 0; i < marketAddresses_.length; i++) {
            if (repayAmounts_[i] > 0) {
                MToken market = marketAddresses_[i];
                if (market.borrowBalanceStored(borrower_) != 0) {
                    return false;
                }
            }
        }
    ...
    /**
     * @notice For each market calculates the liquidation amounts for borrower.
     * @dev Used in automation liquidation.
     * @param account_ The address of the borrower
     * @param redemptionRateMantissa_ debt redemption rate, shows what percentage of a unit of a debt asset must
     *        be redeemed to return the account to a safe position, scaled by 1e18
     ...
     */
    function calculateLiquidationAmounts(address account_, uint256 redemptionRateMantissa_)
        public
        view
        virtual
        returns (AccountLiquidationAmounts memory accountState)
    {
        accountState.marketAddresses = supervisor.getAccountAssets(account_);
        uint256 accountMarketsLen = accountState.marketAddresses.length;

        accountState.repayAmounts = new uint256[](accountMarketsLen);
        accountState.seizeAmounts = new uint256[](accountMarketsLen);
        accountState.supplyAmounts = new uint256[](accountMarketsLen);

        // For each market the borrower is in calculate liquidation amounts
        for (uint256 i = 0; i < accountMarketsLen; i++) {
            MToken market = accountState.marketAddresses[i];

            uint256 oraclePrice = oracle.getUnderlyingPrice(market);
            require(oraclePrice > 0, ErrorCodes.INVALID_PRICE);

            (uint256 supplyWrap, uint256 borrowUnderlying, uint256 exchangeRateMantissa) = market.getAccountSnapshot(
                account_
            );
            (uint256 liquidationFeeMantissa, uint256 utilisationFactorMantissa) = supervisor.getMarketData(market);

            if (borrowUnderlying > 0) {
                // accountTotalBorrowUsd += borrowUnderlying * oraclePrice
                uint256 accountBorrowUsd = (borrowUnderlying * oraclePrice) / EXP_SCALE;
                accountState.accountTotalBorrowUsd += accountBorrowUsd;

                // repayAmountUnderlying = borrowUnderlying * redemptionRate
                accountState.repayAmounts[i] = (borrowUnderlying * redemptionRateMantissa_) / EXP_SCALE;
                ...
                accountState.accountPresumedTotalSeizeUsd +=
                    (accountBorrowUsd * (liquidationFeeMantissa + EXP_SCALE)) /
                    EXP_SCALE;
            }

            if (supplyWrap > 0) {
                // supplyAmounts[i] = supplyWrap * exchangeRate
                accountState.supplyAmounts[i] = (supplyWrap * exchangeRateMantissa) / EXP_SCALE;

                // accountTotalSupplyUsd += supplyWrap * exchangeRate * oraclePrice
                uint256 accountSupplyUsd = (accountState.supplyAmounts[i] * oraclePrice) / EXP_SCALE;
                accountState.accountTotalSupplyUsd += accountSupplyUsd;

                // accountTotalCollateralUsd += accountSupplyUSD * utilisationFactor
                accountState.accountTotalCollateralUsd += (accountSupplyUsd * utilisationFactorMantissa) / EXP_SCALE;
            }
        }

        if (accountState.accountTotalCollateralUsd > 0) {
            // For each market the borrower is in calculate seize amount
            for (uint256 i = 0; i < accountMarketsLen; i++) {
                // actualTotalSeizeUsd shows actual total seize amount in USD that will be burned from user's collateral
                // actualTotalSeizeUsd = accountPresumedTotalSeizeUsd * redemptionRate
                uint256 actualTotalSeizeUsd = (accountState.accountPresumedTotalSeizeUsd * redemptionRateMantissa_) /
                    EXP_SCALE;
                // seizeAmounts[i] = (actualTotalSeizeUsd * supplyAmounts[i]) / accountTotalSupplyUsd
                accountState.seizeAmounts[i] =
                    (actualTotalSeizeUsd * accountState.supplyAmounts[i]) /
                    accountState.accountTotalSupplyUsd;
            }

            // saveSupplyRatio = (accountTotalSupplyUsd / accountTotalCollateralUsd) + safetyBuffer
            accountState.saveSupplyRatio =
                (accountState.accountTotalSupplyUsd * EXP_SCALE) /
     ...
     * @param isManualLiquidation_ Marker for manual liquidation process.
     * @param liquidationType_ The type of liquidation that corresponds to the state of the account.
     *        Used within this function to determine whether to seize all accounts collateral.
     */
    function seize(
        address borrower_,
        MToken[] memory marketAddresses_,
        ...
        for (uint256 i = 0; i < marketAddresses_.length; i++) {
            ...
            if (seizeUnderlyingAmount > 0) {
                ...
                if (liquidationType_ == LiquidationType.ForgivableComplete) {
                    seizeUnderlyingAmount = type(uint256).max;
                }
    ...
    /**
     * @dev Selects the liquidation mode for the account's loan. The choice of the liquidation mode is made based on the
     *      parameters of the current number of account's liquidation attempts and the current loan state
     * @param borrower_ Account for which the liquidation mode is chosen
     * @param accountTotalSupplyUsd_ The sum of all account loans
     * @param accountTotalBorrowUsd_ The sum of all account deposits
     * @param accountPresumedTotalSeizeUsd_ The sum of all account seizes calculated on the basis of drRateMantissa == 1
     * @return Type of liquidation
     */
    function chooseLiquidationType(
        address borrower_,
        uint256 accountTotalSupplyUsd_,
        uint256 accountTotalBorrowUsd_,
        uint256 accountPresumedTotalSeizeUsd_
    ) internal view returns (LiquidationType) {
        uint256 accountAttempts = accountLiquidationAttempts[borrower_];
        if (
            accountTotalBorrowUsd_ >= partialLiquidationMinSum &&
            accountAttempts < partialLiquidationMaxAttempts &&
            accountPresumedTotalSeizeUsd_ < accountTotalSupplyUsd_
        ) {
            return LiquidationType.Partial;
        } else if (accountTotalSupplyUsd_ == 0 && accountTotalBorrowUsd_ > 0) {
            return LiquidationType.Hopeless;
        } else if (accountPresumedTotalSeizeUsd_ > accountTotalSupplyUsd_) {
            return LiquidationType.ForgivableComplete;
        }
        return LiquidationType.Complete;
    }

    /**
     * @dev Checks that drRate value matches the type of liquidation
     * @param drRate_ Debt redemption rate, shows what percentage of a unit of a debt asset must be redeemed to
     *        return the account to a safe position (scaled by 1e18).
     * @param typeToApprove_ The type of liquidation that corresponds to the state of the account
     * @return Whether or not the approval succeeded
     */
    function approveDrRate(uint256 drRate_, LiquidationType typeToApprove_) internal pure returns (bool) {
        if (typeToApprove_ == LiquidationType.Partial) {
            return drRate_ < EXP_SCALE;
        } else {
            return drRate_ == EXP_SCALE;
        }
    }
    ...
    /**
     * @dev Approve that current supply ration value satisfies the condition:
     *      saveSupplyRatio - safeDeviationThreshold < currentSupplyRatio < saveSupplyRatio + safeDeviationThreshold
     * @param borrower_ The account with the debt being payed off
     * @param saveSupplyRatio_ ratio that corresponds to the secured state of the account loan
     * @param marketAddresses_ Array of markets the borrower is in
     * @return Whether or not the current account supply ratio correct
     */
    function approveBorrowerSupplyRatio(
        address borrower_,
        uint256 saveSupplyRatio_,
        MToken[] memory marketAddresses_
    ) internal view returns (bool) {
        ...
        for (uint256 i = 0; i < marketAddresses_.length; i++) {
            ...
            if (supplyWrap > 0) {
                accountTotalSupply += ((((supplyWrap * exchangeRateMantissa) / EXP_SCALE) * oraclePriceMantissa) /
                    EXP_SCALE);
            }
        }

        // currentSupplyRatio = accountTotalSupply / accountTotalBorrow
        uint256 currentSupplyRatio = (accountTotalSupply * EXP_SCALE) / accountTotalBorrow;

        // saveSupplyRatio - safeDeviationThreshold < currentSupplyRatio < saveSupplyRatio + safeDeviationThreshold
        return (saveSupplyRatio_ - safeDeviationThreshold < currentSupplyRatio &&
            currentSupplyRatio < saveSupplyRatio_ + safeDeviationThreshold);
    }

    /*** Admin Functions ***/

    /**
     * @notice Sets a new value for partialLiquidationMaxAttempts
     */
    function setPartialLiquidationMaxAttempts(uint256 newValue_) external onlyRole(TIMELOCK) {
        uint256 oldValue = partialLiquidationMaxAttempts;

        require(newValue_ != oldValue, ErrorCodes.IDENTICAL_VALUE);
        partialLiquidationMaxAttempts = newValue_;

        emit PartialLiquidationMaxAttemptsChanged(oldValue, newValue_);
    }

    /**
     * @notice Sets a new value for PartialLiquidationMinSum
     */
    function setPartialLiquidationMinSum(uint256 newValue_) external onlyRole(TIMELOCK) {
        uint256 oldValue = partialLiquidationMinSum;

        require(newValue_ != oldValue, ErrorCodes.IDENTICAL_VALUE);
        partialLiquidationMinSum = newValue_;

        emit PartialLiquidationMinSumChanged(oldValue, newValue_);
    }

    /**
     * @notice Sets a new value for SafetyBuffer
     */
    function setSafetyBuffer(uint256 newValue_) external onlyRole(TIMELOCK) {
        uint256 oldValue = safetyBuffer;

        require(newValue_ != oldValue, ErrorCodes.IDENTICAL_VALUE);
        safetyBuffer = newValue_;

        emit SafetyBufferChanged(oldValue, newValue_);
    }

    /**
     * @notice Sets a new value for safeDeviationThreshold
     */
    function setSafeDeviationThreshold(uint256 newValue_) external onlyRole(TIMELOCK) {
        uint256 oldValue = safeDeviationThreshold;

        require(newValue_ != oldValue, ErrorCodes.IDENTICAL_VALUE);
        safeDeviationThreshold = newValue_;

        emit SafeDeviationThresholdChanged(oldValue, newValue_);
    }
    ...
    function setInsignificantLoanThreshold(uint256 newValue_) external onlyRole(TIMELOCK) {
        require(newValue_ < partialLiquidationMinSum, ErrorCodes.NEW_VALUE_IS_TOO_BIG);
...
contract DeadDrop is AccessControl {
    ...
    function swapTokensForExactTokens(
        ...
        // TODO: check deadline parameter value
        //slither-disable-next-line unused-return
        router.swapTokensForExactTokens(amountOut, amountInMax, path, address(this), block.timestamp + 30);

        uint256 newTokenInBalance = tokenIn.balanceOf(address(this));

        emit SwapTokensForExactTokens(amountInMax, tokenInBalance - newTokenInBalance, amountOut, router, path);
    }
    ...
    function swapExactTokensForTokens(
        ...
        // TODO: check deadline parameter value
        //slither-disable-next-line unused-return
        router.swapExactTokensForTokens(amountIn, amountOutMin, path, address(this), block.timestamp + 30);

        uint256 newTokenOutBalance = tokenOut.balanceOf(address(this));
        uint256 amountOutActual = newTokenOutBalance - tokenOutBalance;

        emit SwapExactTokensForTokens(amountIn, amountOutMin, amountOutActual, router, path);
    }
