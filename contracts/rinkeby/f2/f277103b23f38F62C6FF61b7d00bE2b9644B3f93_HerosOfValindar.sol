...
contract HerosOfValindar is ERC721r, Ownable, ReentrancyGuard, ProxyRegistry {
    ...
    uint256 public manualDistCap = 50;
    uint256 public mintedAllowedCap = 9950;
    uint256 public whitelistLimit = 50;
    ...
    constructor(bytes32 merkleroot, address _address1, address _address2)
    ERC721r("Heros of Valindar", "HoV", 10000)
    ReentrancyGuard() {
        root = merkleroot;
        address1 = _address1;
        address2 = _address2;
  ...
  function whitelistMint(uint256 tokenAmount, bytes32[] calldata _merkleProof) public payable isValidMerkleProof(_merkleProof) {
    require(!paused, "Contract is Paused");
    require(whitelistEnabled, "Whitelist is NOT enabled");
    require(mintedAllowedCap >= tokenAmount + totalSupply(), "Supply Limit Reached");
    require(msg.value >= tokenAmount * whitelistPrice, "Insufficient ETH for Mint");
    require(userMintedTokens + tokenAmount <= mintedAllowedCap, "User Minting Amount Reached");
    require(walletMints[msg.sender] + tokenAmount <= whitelistLimit, "Exceeded Max Allocation per Wallet");

    userMintedTokens += tokenAmount;
    _buy(msg.sender, tokenAmount, whitelistPrice);
  }

  function publicMint(uint256 tokenAmount) public payable {
    require(!paused, "Contract is Paused");
    require(publicSale, "Public Sale is NOT enabled");
    require(mintedAllowedCap >= tokenAmount + totalSupply(), "Supply Limit Reached");
    require(msg.value >= tokenAmount * publicPrice, "insufficient ETH for Mint");
    require(userMintedTokens + tokenAmount <= mintedAllowedCap, "User Minting Amount Reached");

    userMintedTokens += tokenAmount;
    _buy(msg.sender, tokenAmount, publicPrice);
  }
  ...
  function _buy(address to, uint256 quantity, uint256 price) internal {
    ...
    function airdrop(address receiver, uint256 amount) public onlyOwner nonReentrant {
        require(10000 >= amount + totalSupply(), "Supply limit");
        require(
  ...
  function _startTokenId() internal view virtual returns (uint256) {
    ...
    function setManualDistCap(uint256 _manualDistCap) public onlyOwner {
    manualDistCap = _manualDistCap;
  }

  function setMintedAllowedCap(uint256 _mintedAllowedCap) public onlyOwner {
    mintedAllowedCap = _mintedAllowedCap;
  }
  ...
  function setSecretImageURI(string memory _secretImage) public onlyOwner {
  ...
  function togglePublicMint() public onlyOwner {
    ...
    function withdraw() public onlyOwner nonReentrant {
        ...
        //Remainder of funds to Project provided wallet address
        uint256 amt2 = address(this).balance * 95/100;
