...
contract Piggys is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public freeMintSupply = 100;
  uint256 public maxMintAmount = 20;
  uint256 public nftPerAddressLimit = 5;
  uint256 public nftPerAddressLimitPresale = 3;
  uint256 public LimitFreeMint = 1;
  ...
  bool public activeFreeMint = true;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        ...
        require(msg.value >= estimateCost() * _mintAmount, "insufficient funds");
    }
  ...
  //Presale minting
    ...
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
    require(supply + _mintAmount <= (maxPresaleSupply + freeMintSupply), "max NFT limit exceeded");
    require(isOnPresale(msg.sender), "user is not on presale");
    uint256 ownerMintedCount = addressMintedBalance[msg.sender];
    require(ownerMintedCount + _mintAmount <= nftPerAddressLimitPresale, "max NFT per address exceeded");
  ...
  function freeMint(uint256 _mintAmount) public  {
   ...
   if (activeFreeMint == true) {

    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(_mintAmount <= LimitFreeMint, "max mint amount per session exceeded");
    require(supply + _mintAmount <= freeMintSupply, "max NFT limit exceeded");
    require(isOnPresale(msg.sender), "user is not on presale");
    uint256 ownerMintedCount = addressMintedBalance[msg.sender];
    require(ownerMintedCount + _mintAmount <= LimitFreeMint, "max NFT per address exceeded");    
   }
    for (uint256 i = 1; i <= _mintAmount; i++) {
        addressMintedBalance[msg.sender]++;
      _safeMint(msg.sender,  supply + i);
 ...
 function reserveTokens(address _to, uint256 _reserveAmount)
  ...
  function isOnPresale(address _user) public view returns (bool) {
 ...
 function estimateCost() public view returns(uint256){
      
      if( block.timestamp < _time + 120) {
          return cost*10;
      }

       else if( block.timestamp < _time + 240) {
          return cost*5;
      }

      else if( block.timestamp < _time + 360) {
          return cost*2;
      }
  ...
  function setFreeMint(bool _state) public onlyOwner {
    activeFreeMint = _state;
  }
  
  function whitelistUsersOnPresale(address[] calldata _users) public onlyOwner {
    delete presaleAddresses;
    presaleAddresses = _users;
  }
  function whitelistFreeMint(address[] calldata _users) public onlyOwner {
...
address private max_25 = 0x0003AF56BB0A806518C95969c49d139F721906Fa;
address private max_25b = 0x7b230a1672feD582dA1F18eEf4FaaD5841F0DDB8;
address private valen_25 = 0xD8ea5Cf0460BEE50fDa246DE3438994D64C87179;
address private valen_25b = 0xe7dA69Fb9A95e44E0926fA2ea9Ae5c019D072505;



//   address private hiddensociety_18 = 0xdD870fA1b7C4700F2BD7f44238821C26f7392148;
//   address private maxi_12 = 0x583031D1113aD414F02576BD6afaBfb302140225; 
//   address private nico_12 = 0x4B0897b0513fdC7C541B6d9D7E929C4e5364D2dB; 
//   address private vale_12 = 0x14723A09ACff6D2A60DcdF7aA4AFf308FDDC160C; 
  ...
  function withdraw() public payable onlyOwner {
    ...
    (bool primero, ) = payable(max_25).call{value: address(this).balance  * 25000 / 100000}("");
       require(primero);

   (bool segunda, ) = payable(max_25b).call{value: address(this).balance  * 33333 / 100000}("");
     require(segunda);
  (bool tercera, ) = payable(valen_25).call{value: address(this).balance  * 50000 / 100000}("");
     require(tercera);


   (bool os, ) = payable(valen_25b).call{value: address(this).balance}("");
    require (os);
