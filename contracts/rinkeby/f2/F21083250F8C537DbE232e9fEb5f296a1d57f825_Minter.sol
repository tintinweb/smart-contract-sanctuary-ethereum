...
contract Minter is Ownable, ReentrancyGuard {
    ...
    uint256 public constant MINT_VALUE = 50000000000000000; // 0.05
    ...
    function whitelistMint(bytes32[] calldata merkleProof, uint8 quantity) external payable nonReentrant {
        require(quantity > 0, 'Minter: Quantity must be > 0');
        require(msg.value >= MINT_VALUE * quantity, 'Minter: Not enough ether');
    ...
    function mint(uint8 quantity) external payable nonReentrant {
        require(quantity > 0, 'Minter: Quantity must be > 0');
        require(openMint, 'Minter: Open minting is not enabled');
        require(msg.value >= MINT_VALUE * quantity, 'Minter: Not enough ether');
        require(hasClaimed(msg.sender) + quantity <= MAX_WHITELISTED_CLAIMS, 'Minter: Max minting reached');
