...
contract DoxxedCO is ERC721A, ERC721ABurnable, ERC721AQueryable, Ownable {
    using ECDSA for bytes32;
    /**
     * @dev Mapping of `(minter << 96) | wlRound` to number of 
     * tokens minted for the whitelist sale.
     */
    mapping(uint256 => uint256) internal _wlMinted;

    /**
     * @dev Mapping of `imageId` to the number minted.
     */
    mapping(uint24 => uint256) internal _imageIdsMinted;
    ...
    constructor() ERC721A('DoxxedCO', 'DCO') {}

    /**
     * @dev Retuns the key into `_wlMinted`.
     */
    function _wlMintedKey(
        address minter, 
        uint16 round
    ) 
        internal 
     ...
     */
    function _mintBatch(address to, uint256 quantity, uint24 imageId) internal {
        unchecked {
        ...
        payable 
    {
        unchecked {
            ...
            // Check if the `imageId`, `imageIdLimit`, `imageIdPrice` are correct.
            hash = keccak256(abi.encode(imageId, imageIdLimit, imageIdPrice, salt));
            require(
                hash.toEthSignedMessageHash().recover(imageIdSignature) == signer, 
                'Invalid image ID signature.'
            );

            // Checks and updates `_imageIdsMinted[imageId]`.
            uint256 minted = _imageIdsMinted[imageId];
            minted += quantity;
            require(minted <= imageIdLimit, 'Image ID out of stock.');
            _imageIdsMinted[imageId] = minted;
        }
     ...
     */
    function imageIdsMinted(uint24[] memory imageIds) public view returns (uint256[] memory) {
        unchecked {
     ...
     */
    function imageIdsOf(uint256[] memory tokenIds) public view returns (uint24[] memory) {
        unchecked {
    ...
    function forceMint(address[] calldata to, uint24 imageId) external onlyOwner {
        unchecked {
            uint256 n = to.length;
            for (uint256 i; i < n; ++i) {
    ...
    function setWLRound(uint16 value) external onlyOwner {
        wlRound = value;
    }
