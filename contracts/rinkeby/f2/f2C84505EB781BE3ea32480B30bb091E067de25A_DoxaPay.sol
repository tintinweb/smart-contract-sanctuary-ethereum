...
contract DoxaPay is Ownable {
    ...
    function payDoxa(uint amount, string memory id, string memory eventId, string memory payer) public {
        ...
        if(keccak256(bytes(payer)) == keccak256(bytes("host"))) {
            ...
            token.transferFrom(msg.sender, address(this), amount.sub(fee));
    ...
    function payETH(string memory id, string memory eventId, string memory payer) public payable {
        ...
        if(keccak256(bytes(payer)) == keccak256(bytes("host"))) {
            ...
            payable(address(this)).transfer(amount.sub(fee));        
        }
    ...
    function payUsingAppWallet(uint amount, string memory id, string memory eventId, string memory payer ) public {
        ...
        if(keccak256(bytes(payer)) == keccak256(bytes("host"))) {
            ...
            address host = eventHostRecords[eventId];
            require(host != address(0), "Invalid event ID or event not created!");
            token.transferFrom(walletAddress, address(this), amount.sub(fee));
    ...
    function claimEventAmount(uint txID) public {
        ...
        uint amount = info.amount.sub(info.amount.div(10));
    ...
    // function refund(address account, uint txID) public {
    //     require(PaymentRecords[account][txID - 1].amount > 0, "Invalid transaction ID");
    //     require(PaymentRecords[account][txID - 1].isRefunded == false, "The payment is already refunded!");

    //     PaymentRecords[account][txID - 1].isRefunded = true;
    //     PaymentInfo memory info = PaymentRecords[account][txID - 1];

    //     if(keccak256(bytes(info.paymentType)) == keccak256(bytes("ETH"))) {
    //         require(adminAddress.balance >= info.amount, "Insufficient balance to refund, Please contact admin!");
    //         payable(adminAddress).transfer(info.amount);
    //     }

    //     if(keccak256(bytes(info.paymentType)) == keccak256(bytes("DOXA"))) {
    //         require(token.balanceOf(adminAddress) >= info.amount, "Insufficient balance to refund, Please contact admin!");
    //         token.transferFrom(adminAddress, account, info.amount);
    //     }

    //     emit Refund(account, info);
    // }

    // function cancelEvent(address account, uint eventId) public {
    //     return (account, eventId)
    // }
    ...
    function payUSDT(uint amount, string memory id, string memory eventId, string memory payer) public {
        ...
        if(keccak256(bytes(payer)) == keccak256(bytes("host"))) {
            ...
            address host = eventHostRecords[eventId];
            require(host != address(0), "Invalid event ID or event not created!");
            usdt.transferFrom(msg.sender, address(this), amount.sub(fee));
