...
contract PurgeGameBeta is ERC721, ERC721Enumerable, Pausable, Ownable
{
    ...
    function encodeTokenId(uint16 _tokenId) private
    {
        ...
        tokenMetadata[_tokenId] = (randomHash % 84) + ((randomHash / 1000 % 84) + 100) * 1000 + ((randomHash / 1000000 % 84) + 200) * 1000000 + ((randomHash / 1000000000 % 84) + 300) * 1000000000;
    }
    ...
    function checkForWinner() public view returns(uint16)
{
    ...
    for(uint16 i=1; i<= totalSupply() + MAPtokens; i++)
    {
        ...
        {
            uint16 traitOne = uint16(tokenMetadata[i] % 1000);
            uint16 traitTwo =  uint16(tokenMetadata[i] / 1000 % 1000);
            uint16 traitThree =  uint16(tokenMetadata[i] / 1000000 % 1000);
            uint16 traitFour =  uint16(tokenMetadata[i] / 1000000000% 1000);
