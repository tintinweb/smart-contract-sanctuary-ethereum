...
import {ProposalSale} from "./ProposalSale.sol";
...
error Proposal_Sale_Cannot_List_Amount_Zero();

error Proposal_Sale_Contract_Not_Approved();

error Proposal_Sale_Only_Seller_Can_Cancel();

error Proposal_Sale_Not_Valid_Proposal();

error Proposal_Sale_Propose_Value_Must_Be_Greater_then_Minimum();

error Proposal_Sale_Propose_Must_Be_Higher_Then_Last_Propose(
    uint256 lastPropose
);

error Proposal_Sale_Only_Seller_Can_Accept();

error Proposal_Sale_No_Active_Propose_To_Accept();

abstract contract ProposalSale is Constants, Core, Payment {
    uint256 internal _proposalSaleId;

    struct ProposalSaleList {
        uint256 amount;
        address seller;
        uint256 propose;
        address proposer;
    ...
    mapping(address => mapping(uint256 => mapping(uint256 => ProposalSaleList)))
        internal _saleIdToProposal;

    event ListProposalSale(
        address nftAddress,
        uint256 tokenId,
        uint256 saleId,
        address seller,
    ...
    function listProposalSale(
        address nftAddress,
        uint256 tokenId,
        uint256 amount,
        address[] calldata royaltiesPayees,
        uint256[] calldata royaltiesCuts
    ) external {
        if (amount < 1) {
            revert Proposal_Sale_Cannot_List_Amount_Zero();
        }

        if (royaltiesPayees.length > 0) {
            _checkRoyalties(royaltiesPayees, royaltiesCuts);
        }

        if (!_saleContractAllowlist[nftAddress]) {
            revert Proposal_Sale_Contract_Not_Approved();
        }

        ProposalSaleList storage proposalSale = _saleIdToProposal[nftAddress][
            tokenId
        ][++_proposalSaleId];

        _setRoyalties(_proposalSaleId, royaltiesPayees, royaltiesCuts);

        _trasferNFT(msg.sender, address(this), nftAddress, tokenId, amount);

        proposalSale.seller = msg.sender;

        proposalSale.amount = amount;

        emit ListProposalSale(
            nftAddress,
            tokenId,
            _proposalSaleId,
            proposalSale.seller,
    ...
    function cancelProposalSale(
        ...
        address seller = proposalSale.seller;

        if (seller == address(0)) {
            revert Proposal_Sale_Not_Valid_Proposal();
        }

        if (seller != msg.sender) {
            revert Proposal_Sale_Only_Seller_Can_Cancel();
        }

        address proposer = proposalSale.proposer;
        //if proposer exist
        if (proposer != address(0)) {
            // return back  the proposer propose due to canceling of proposal
            payable(proposalSale.proposer).transfer(proposalSale.propose);
        }
        ...
        delete _saleIdToProposal[nftAddress][tokenId][saleId];

        emit CancelProposalSale(nftAddress, tokenId, saleId, seller);
    }
    ...
    function propose(
        ...
        if (msg.value < MIN_PRICE) {
            revert Proposal_Sale_Propose_Value_Must_Be_Greater_then_Minimum();
        }
        ProposalSaleList storage proposalSale = _saleIdToProposal[nftAddress][
            tokenId
        ][saleId];
        ...
        if (proposalSale.seller == address(0)) {
            revert Proposal_Sale_Not_Valid_Proposal();
        }

        if (proposalSale.propose > msg.value) {
            revert Proposal_Sale_Propose_Must_Be_Higher_Then_Last_Propose(
                proposalSale.propose
            );
        }

        if (proposalSale.propose != 0) {
            oldProposer = proposalSale.proposer;
            oldPropose = proposalSale.propose;

            payable(oldProposer).transfer(oldPropose);
        }

        proposalSale.proposer = msg.sender;

        proposalSale.propose = msg.value;
    ...
    function acceptProposal(
        ...
        ProposalSaleList memory proposalSale = _saleIdToProposal[nftAddress][
            tokenId
        ][saleId];
        ...
        if (seller != msg.sender) {
            revert Proposal_Sale_Only_Seller_Can_Accept();
        }
        if (proposer == address(0)) {
            revert Proposal_Sale_No_Active_Propose_To_Accept();
        }
        _trasferNFT(
            address(this),
            proposer,
        ...
        uint256 acceptedPropose = proposalSale.propose;
        ...
        (
            ...
            address[] memory royaltiesPayees,
            uint256[] memory royaltiesCuts
        ) = _splitPayment(seller, acceptedPropose, saleId);
