...
contract DNAChipDescriptor is Ownable {
    ...
    function getEvolutionPodMetadata(uint8[8] memory traits) internal view returns (string memory) {
        string memory metadataString;
        uint8 base = traits[0];
        ...
        metadataString = string(
            abi.encodePacked(
                metadataString,
                '{"trait_type": "',
                traitTypeNames[traits.length],
                '","value":"',
                _traitDataByCharacter[base][uint8(traits.length)][traits[traits.length]].name,
                '"}'
            )
        );
