...
contract UtopiaFactory {
    ...
    mapping(address => address[]) public userVerses;
    ...
    event LandCreated(address owner, uint256 time, address verseAddress, address collectionAddress);
    ...
    function createVerse(address _owner) public returns(
        address verse,
        address collection
    ){
        ...
        userVerses[_owner].push(verse);
        collection = address(new UtopiaNFT(
            verse,
            _owner
        ));
        userCollections[_owner].push(collection);
        collections[verse] = collection;
        emit LandCreated(_owner, block.timestamp, verse, collection);
    }

    function setVerseCreationFee(uint256 _fee) public {
    ...
    function userInfo(
        ...
        address[] memory _verses,
        address[] memory _collections
        ) {
        _verses = new address[](userVerses[user].length);
        _collections = new address[](userCollections[user].length);
        for (uint i = 0; i < _verses.length; i++) {
            _verses[i] = userVerses[user][i];
            _collections[i] = userCollections[user][i];
        }
    }
