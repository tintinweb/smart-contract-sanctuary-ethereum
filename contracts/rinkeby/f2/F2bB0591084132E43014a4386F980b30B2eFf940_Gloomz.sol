...
contract Gloomz  is ERC721A , Ownable{
  ...
  uint256 public  DEV_MINT_RESERVE = 20;
  ...
  function mintGloom(uint256 amount) external payable  {
    ...
    uint256 supplyCap = ts+reserve+ amount -2;
    int freeMintLeft = int(AMOUNT_FREE_MINTS-ts);
    require(supplyCap>= SUPPLY_CAP,"SUPPLY CAP REACHED");
    if(ts>= AMOUNT_FREE_MINTS || freeMintLeft<int(amount)){
      require(amount<=MAX_PER_TX_PUBLIC,"TO MANY MINTS ON PUBLIC");
      if(freeMintLeft<=0){
        require(msg.value >= amount* MINT_PRICE, "NOT ENUFF FUNDS SEND");
      }else {
        require(msg.value>=  uint256(amount-uint256(freeMintLeft) * MINT_PRICE),"NOT ENUFF FUNDS 2");
      }

    }else{
      require(amount<=MAX_PER_TX_FREE, "TO MANY NFTS ");
    }
