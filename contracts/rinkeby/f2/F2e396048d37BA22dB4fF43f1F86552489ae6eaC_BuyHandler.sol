...
contract BuyHandler is ReentrancyGuard, BuySignatureUtils {
    ...
    */
    function buy(
        ...
        require(addressMintAmountInPrivateTime[msg.sender] < _userMaxMint, "BuyHandler: you can only buy up to the specific limition");
    ...
    */
    function buyBatchFake(
        ...
        require(msg.sender == IAviumNFT(PROXY).owner(), "BuyHandler: only AviumNFT's owner");
        uint256 currentIndex = IAviumNFT(PROXY).getCurrentIndex();
        require(currentIndex + _quantity <= IAviumNFT(PROXY).getTotalMint(), "BuyHandler: total mint has been exeeded");
        IAviumNFT(PROXY).mint(_to, _quantity, _data);
    }
}
