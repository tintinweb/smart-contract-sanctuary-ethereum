...
contract DistributorFactory is Operators, IDistributorFactory {
    ...
    function withdrawWithProof(
        ...
        override
    {
        distributors[_poolID].withdrawWithProof(_campaignID,_totalAmount, _merkleProof);
    }
    ...
    function getWithdrawableWithTotalAmount(
        ...
        return distributors[_poolID].getWithdrawableWithTotalAmount(_campaignID, _user, _totalAmount);
    }
...
interface IDistributor {
    ...
    function getWithdrawableWithTotalAmount(string calldata _campaignID, address _user, uint256 _totalAmount)
        external
        view
        returns (uint256);
...
contract MixDistributor is ReentrancyGuard, BaseDistributor {
    ...
    function setCampaignClaimData(string calldata _campaignID, bytes32 _merkleTreeRoot, IERC20 _token) 
        external 
        override 
        onlyOwner 
    {
    ...
    function getWithdrawableWithTotalAmount(string calldata _campaignID, address _user, uint256 _totalAmount)
        ...
        returns (uint256)
    {
        return _getWithdrawableWithTotalAmount(_campaignID, _user, _totalAmount);
    }
    ...
    function withdrawWithProof(
        ...
        CampaignClaimData memory campaignClaimData =  campaignStandaloneClaimData[_campaignID];
//        require(
//            MerkleProof.verify(_merkleProof, campaignClaimData.merkleRoot, leaf),
//            "Invalid withdraw proof"
//        );
//
//        uint256 _amount = _getWithdrawableWithTotalAmount(_campaignID, _user, _totalAmount);
//        require(_amount <= _totalAmount - claimedAmount[_campaignID][_user], "Exceed max amount of withdraw.");
//        claimedAmount[_campaignID][_user] += _amount;
//
...
//        emit Withdraw(_user, _campaignID, amount);
    ...
    function _getWithdrawableWithTotalAmount(string calldata _campaignID, address _user, uint256 _totalAmount)
        internal
        view
        returns (uint256)
    {
    ...
    function _getWithdrawable(string calldata _campaignID, address _user, uint256 _totalAmount) 
        ...
        for (uint32 i = 0; i < _info.trancheStartTimestamps.length; i++) {
            if (block.timestamp >= _info.trancheStartTimestamps[i]) {
                if (_info.trancheTypes[i] == TrancheType.ONCE) {
                    totalClaimable += (_info.percentageOfTranches[i] * _totalAmount ) / MILLION;
