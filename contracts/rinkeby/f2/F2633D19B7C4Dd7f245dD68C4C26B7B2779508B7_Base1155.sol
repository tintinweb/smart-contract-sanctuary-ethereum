...
contract Base1155 is Authorized, ERC1155Supply, PaymentSplitter {
    ...
    */
    ...
    @dev Sets token uri.
     */
    function setTokenURI( uint256 _tokenID, string memory _tokenURI ) external onlyAuthorized {
    ...
    /**
    @dev Set token whether or not token sale is open.
     */
    ...
    @dev Set token price.
     */
    function setTokenPrice ( uint256 _tokenID, uint256 _price ) external onlyAuthorized {
        require(exists(_tokenID), "EditTokenURI: Token ID does not exist");
        Token storage t = tokens[_tokenID];
        t.price = _price;  
    }
    ...
    */
    function mint ( uint256 _tokenID, uint256 _quantity, address to ) external payable {
        require( exists(_tokenID), "mint: token does not exist" );
        require( isSaleOpen(_tokenID), "mint: sale is closed" );
        
        tokens[_tokenID].minted[to] += _quantity;
        //above/below changed from msg.sender => to for crossmint
        _mint(to, _tokenID, _quantity, "");
    }
