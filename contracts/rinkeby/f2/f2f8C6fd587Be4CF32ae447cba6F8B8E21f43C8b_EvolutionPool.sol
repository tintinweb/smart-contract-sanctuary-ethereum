...
contract EvolutionPool is MonsterCollectible {
    ...
    mapping(uint256 => uint256) public monsterIdToMonsterRarity;
    ...
    {
        setMonsterMapper_EvolutionPool();
    }

    // take in an array of IDs and verify 1) they are all the same and 2) user owns them
    function evolveMonster(address _owner, uint256[] memory _monsterIds) public {
        s_allSameMonsterIds = false;
        s_correctQuantityOfMonsterCards = false;
        
        uint256 monsterToEvolveId = s_tokenIdToMonster[0];
        
        // check to make sure all IDs are the same monster
        for (uint i = 1; i < _monsterIds.length; i++) {
            if (monsterToEvolveId == s_tokenIdToMonster[i]) {
                continue;
            } else {
                break;
            }
        ...
        uint256 rarityLevel = monsterIdToMonsterRarity[monsterToEvolveId];
        if (rarityLevel == 0 && _monsterIds.length == 10) {     //TODO: change back to 20!
            s_correctQuantityOfMonsterCards = true;
        } else if (rarityLevel == 1 && _monsterIds.length == 5) {
            s_correctQuantityOfMonsterCards = true;
        }

        if (s_correctQuantityOfMonsterCards != true){
            revert("Quanity of provided cards for evolution is incorrect.");
        }
    ...
    function mint_commons() public {
        uint i = 0;
        while (i < 11) {
            mintMonster(101, msg.sender);
            i++;
        }
    }
    function mint_rares() public {
        uint i = 0;
        while (i < 6) {
            mintMonster(105, msg.sender);
            i++;
        }
    }
    ...
    function setMonsterMapper_EvolutionPool() private {
        monsterIdToMonsterRarity[101]=0;
        monsterIdToMonsterRarity[102]=1;
        monsterIdToMonsterRarity[103]=2;
        monsterIdToMonsterRarity[104]=0;
        monsterIdToMonsterRarity[105]=1;
        monsterIdToMonsterRarity[106]=2;
        monsterIdToMonsterRarity[107]=0;
        monsterIdToMonsterRarity[108]=1;
        monsterIdToMonsterRarity[109]=2;
    }
}
