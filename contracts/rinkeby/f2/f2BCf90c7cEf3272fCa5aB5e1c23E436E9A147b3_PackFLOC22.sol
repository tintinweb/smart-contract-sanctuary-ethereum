    ...
    ERC2771ContextUpgradeable
{
    ...
    function setAvailableNFT(string memory metadata, uint256 price)
        public
        onlyRole(UPDATER_ROLE)
    {
        AVAILABLE_NFT memory newNFT = AVAILABLE_NFT(price, metadata);
        AVAILABLE_NFTS.push(newNFT);
    }

    function checkAvailableNFT(uint256 tokenID)
        public
        view
        returns (AVAILABLE_NFT memory nft)
    {
        if (tokenID >= AVAILABLE_NFTS.length) {
            revert("NFT not available");
        }
        nft = AVAILABLE_NFTS[tokenID];
        return nft;
    }

    function safeMint(address to, uint256 ID) public payable {
        if ((ID - 1) >= AVAILABLE_NFTS.length) {
            revert("NFT not available");
        }
    ...
    // Only transfer if the contract is not paused
