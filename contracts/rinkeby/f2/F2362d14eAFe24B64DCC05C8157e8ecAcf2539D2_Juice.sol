...
contract Juice is WizardsERC1155 {
    ...
    bool private _allowMerge = true;
    bool private _allowRandom = true;
    address private _pinaverse;

    uint256[5] private _juiceBurnAmount = [3, 3, 3, 10, 0];
    uint256[4] private _jmi = [2555, 1622, 866, 212];
        ...
        )
    {
        ...
        for (uint256 j = 0; j < _jmi.length; j++) {
            setMaxSupply(j, _jmi[j]);
        }
        setMaxSupply(4, 5);
    }
    ...
    function _payment(uint256 quantity, uint256 paymentToken) private {
        ...
        if (p.token == address(0) && msg.value != (total)) {
            revert PaymentFailed();
        } else {
            IERC20 erc20 = IERC20(p.token);
            if (erc20.transferFrom(_msgSender(), _pinaverse, total) != true) {
                revert PaymentFailed();
            }
        }
    }
    ...
    function _merge(
        ...
        uint256 tierId = id; // 0
        uint256 nextTierId = tierId + 1; // 1
        uint256 mintQty; //
        uint256 tierBurn = _juiceBurnAmount[tierId]; // 3
        ...
        unchecked {
            ...
            for (uint256 i = nextTierId; i < _juiceBurnAmount.length - 1; i++) {
