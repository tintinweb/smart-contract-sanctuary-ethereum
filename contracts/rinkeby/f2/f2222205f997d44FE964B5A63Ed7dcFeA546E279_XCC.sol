...
contract XCC is ERC721A, AuthorityControlled, ReentrancyGuard {
    ...
    function deleteBlacklist(address[] memory _blacklist) external onlyOwner {
        require(
            status == Status.Bidding,
            "XCC: Only Bidding or Pending status can execute this operation."
        );
    ...
    function syncBlacklist(address[] memory _blacklist) external onlyOwner {
        require(
            status == Status.Bidding,
            "XCC: Only Bidding or Pending status can execute this operation."
        );
    ...
    function drop(address addr_, uint8 _type) external onlyOwner {
        drop_(addr_, _type);
    }
    ...
    function payout(address _addr) public virtual onlyApprover {
        require(
            status == Status.Finished,
            "payout: Cannot payout if status is not finished"
        );
        require(_addr != address(0), "payout: addr can't be null");
        require(
            payoutAddr == address(0) || (payoutAddr == _addr),
            "Apayout: illegal addr"
        );
        bool isPayout;
        uint256 idx;
        ...
        if (checkRate(payoutApprove.length)) {
            (bool success, ) = payable(_addr).call{
    ...
    function setPrice(uint256 _floorPrice) external onlyOwner {
        floorPrice = _floorPrice;
        emit PriceChanged(_floorPrice);
    }

    function setTotalBid(uint256 _totalBid) external onlyOwner {
        totalBid = _totalBid;
        emit TotalBidChanged(_totalBid);
    }

    function setTokens(uint256 _tokens) external onlyOwner {
        tokens = _tokens;
    ...
    function setMarket(IMarket _market) public onlyOwner {
        require(
