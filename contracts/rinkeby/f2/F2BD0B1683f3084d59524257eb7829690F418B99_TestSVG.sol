...
contract TestSVG is ERC721 {
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    bytes(
                        'data:text/plain,{"name": "testsvg", "description": "A simple test", "image": "data:image/svg+xml,'
                    ),
                    bytes(
                        '<svg viewBox="0 0 2105 1600"><text x="0" y="15" fill="black" transform="rotate(30 20,40)">'
                    ),
    ...
    function testTokenURI(uint256 tokenId) public view returns (string memory) {
        // ipfs hash for image: QmbXUiGUuXEf3h1hoRy6MzVmQgXyJwCzLaDJLXs8NmVfBB
        (uint256 prevBTC, uint256 currentBTC) = getPrice(0);
        return
            string(
