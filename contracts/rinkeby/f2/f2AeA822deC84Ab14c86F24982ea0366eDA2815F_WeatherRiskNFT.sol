...
// File: @openzeppelin/contracts-upgradeable/utils/Address.sol
...
contract WeatherRiskNFT is 
    ...
    mapping(uint256 => address) private _viewers;
    mapping(address => string) private _pubKeys;
     ...
     * @param tokenId uint256 base10 representation of contract's ID (base64)
     */
    ...
    /**
     * @dev Called from frontend on each client view, acts as a complement to ownerOf
     * @notice can only be called by Viewer
     * @param tokenId uint256 token ID of contract
     */
    function getViewer(uint256 tokenId) 
        external
        view
        onlyRole(VIEWER_ROLE)
        returns (address)
    ...
    /**
     * @dev Called from frontend after 
     * @notice can only be called by Viewer
    ...
    /**
     * @notice can only be called by Evaluator
    ...
    /**
     * @dev Called from frontend before mints
     * @notice can only be called by Admin
     * @param pubKey string Metamask public encryption key for the msg sender
