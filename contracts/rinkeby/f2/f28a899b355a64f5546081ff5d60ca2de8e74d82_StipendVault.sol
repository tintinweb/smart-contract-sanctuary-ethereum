...
contract StipendVault is Ownable{
    ...
    function deposit(uint256 _poolId, uint256 _amount) external {
        ...
        //Update pool tokens staked
        pool.tokensStaked = pool.tokensStaked + _amount;

        //recalculate each Staker weight on global change to pool TVL

        updateStakerAndPoolWeight(_poolId);
    ...
    function withdrawAll(uint256 _poolId) external {
        ...
        // Update all staker and pool weight with change in pool TVL
        updateStakerAndPoolWeight(_poolId);
    ...
    **/
    ...
    /**
     * @dev Pool, Staker weight is PVP: weight changes happen with each deposit or withdrawal. 
     * Each time there is a deposit or withdrawal, we need to update all stakers, pool weight
     */ 
    function updateStakerAndPoolWeight(uint256 _poolId) private {
        Pool storage pool = pools[_poolId];
        uint256 weight = 0;
        for (uint256 i; i < pool.stakers.length; i++) {
            address stakerAddress = pool.stakers[i];
            PoolStaker storage staker = poolStakers[_poolId][stakerAddress];
            staker.weight = estimateWeight(staker.amount, stakerAddress, _poolId);
    ...
    function updateStakersRewards(uint256 _poolId) private {
        ...
        for (uint256 i; i < pool.stakers.length; i++) {
            ...
            uint256 stakerShare = (staker.weight * STAKER_SHARE_PRECISION / pool.totalWeight);
