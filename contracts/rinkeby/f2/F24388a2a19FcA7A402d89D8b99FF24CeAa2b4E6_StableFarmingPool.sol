    ...
    AccessControlUpgradeable
{
     ...
     */
    function stableFarmingUpdatePool(uint256 _poolId, uint256 _startDate, uint256 _endDate)  
        external
        onlyRole(ADMIN) {
        StableFarmingPoolInfo storage pool = stableFarmingPoolInfo[_poolId];
        
        require(pool.startJoinTime > block.timestamp, "stableFarmingStakingPool: update pool after pool started");
     ...
     */
    function farmingAddBlackList(address[] memory _blackList) external onlyRole(ADMIN) {
        require(
            _blackList.length > 0, 
     ...
     * @param _blackList whitelist
     */
    function farmingRemoveBlackList(address[] memory _blackList) external onlyRole(ADMIN) {
        require(
            _blackList.length > 0, 
            "stableFarmingStakingPool: no items in blacl list array"
        );
