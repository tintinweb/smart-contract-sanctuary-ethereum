...
contract NFT is ERC721, PullPayment, Ownable {
    ...
    function mintToBulk(address recipient, uint256 count) public payable
    {
        require(mintingOpen == true, "Minting is not open right now!");
        ...
        require(msg.value == MINT_PRICE.mul(count), "Transaction value did not equal to the total mint price");
    ...
    function mintToAdminBulk(address recipient, uint256 count) public onlyOwner {
        uint256 tokenId = currentTokenId.current();
        require(tokenId + count < TOTAL_SUPPLY, "Max supply reached");
    ...
    function currentPrice(uint256 currentId) public view returns (uint256) {
        uint256 range = currentId / 10 * 10;
        uint256 remainder = currentId.div(10);
        uint256 currRange = range = remainder;
        uint256 currPrice = BASE_PRICE.add(BASE_PRICE.mul(currRange));
        return currPrice;
    }
