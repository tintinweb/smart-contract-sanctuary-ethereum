...
import { PellarNFTEnumerable, ERC721, IERC721 } from "./PellarNFTEnumerable.sol";
import { ERC721Holder } from "@openzeppelin/contracts/token/ERC721/utils/ERC721Holder.sol";
import { ERC1155Holder } from "@openzeppelin/contracts/token/ERC1155/utils/ERC1155Holder.sol";

contract HDKNTest is Ownable, PellarNFTEnumerable {
  struct SaleInfo {
    bool buyable;
    uint256 price;
  }
  ...
  constructor() ERC721("HDKN Test", "HDKN Test") {}
  ...
  function getAuctionHistories(
    ...
    returns (
      uint256 totalPages,
      bool hasNext,
      AuctionHistory[] memory items
    )
  {
    uint256 size = auctionHistories[_tokenId].length;

    totalPages = size / _pageSize;
    if (totalPages * _pageSize < size) {
      totalPages += 1;
    }
    uint256 offset = (_pageIndex - 1) * _pageSize;
    hasNext = size > (_pageSize * _pageIndex);

    uint256 from = offset > size ? size : offset;
    uint256 to = (_pageIndex * _pageSize) > size ? size : (_pageIndex * _pageSize);

    items = new AuctionHistory[](to - from);

    for (uint256 i = from; i < to; i++) {
      items[i - from] = auctionHistories[_tokenId][i];
    }
  }
  ...
  function buyNow(uint16 _tokenId) external payable nonReentrant(_tokenId) onlyInited(_tokenId) {
    ...
    delete tokens[_tokenId].saleInfo;
    delete tokens[_tokenId].auctionInfo;
  }
  ...
  function auctionWinnerWithdraw(uint16 _tokenId) external {
    AuctionInfo storage auctionInfo = tokens[_tokenId].auctionInfo;
    ...
    IERC721(address(this)).transferFrom(address(this), winner, _tokenId);
  }
...
import { ERC721, IERC165, IERC721 } from "@openzeppelin/contracts/token/ERC721/ERC721.sol";
