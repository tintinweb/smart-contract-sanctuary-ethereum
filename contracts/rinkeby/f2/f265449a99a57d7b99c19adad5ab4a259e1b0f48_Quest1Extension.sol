...
contract Quest1Extension is Ownable {
...
// Variables
    ...
    uint256 internal _extensionTokenId;
    IERC1155CreatorCore internal _creator;

// Initialization

    constructor(address creator_, uint256 tokenMaxSupply) {
        _creator = IERC1155CreatorCore(creator_);
        TOKEN_MAX_SUPPLY = tokenMaxSupply;
    }

    function initialize(string calldata uri) external onlyOwner {
        if (_extensionTokenId != 0) revert Conflict();
        ...
        _extensionTokenId = _creator.mintExtensionNew(receivers, amounts, uris)[0];
    }
...
// Airdrop
    ...
    function airdrop(address[] calldata wallets) external onlyOwner {
        ...
        tokenIds[0] = _extensionTokenId;
        amounts[0] = 1;

        _creator.mintExtensionExisting(wallets, tokenIds, amounts);
    }
...
// Metadata
    ...
    function setTokenURI(string calldata uri) external onlyOwner {
        if (_extensionTokenId == 0) revert Conflict();
        _creator.setTokenURIExtension(_extensionTokenId, uri);
    }
...
// Supply
    ...
    function availableSupply() public view returns (uint256) {
        return TOKEN_MAX_SUPPLY - _totalSupply;
    }
}
