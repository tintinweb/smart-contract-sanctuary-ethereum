...
contract SAYC is ERC721A, Ownable {
    ...
    */
    modifier onlyDev() {
        require(msg.sender == devAddress, 'Dev only');
        _;
    }

    constructor() ERC721A("Street Ape Yolo Club", "SAYC") {
        hiddenURI = ''; //set hidden URI here
        devAddress = 0xF0Db5bDa2C0B9239AF30C8589D494481717193c8;
        devShare = 2;
    }
    ...
    */
    ...
    function mintReserve() external payable onlyOwner {
        ...
        require(tokenCount <= reserve,          'This transaction would exceed reserve supply.');
    ...
    */
    ...
    function mintForDev() external payable onlyDev {
        uint256 supply = totalSupply();
        uint256 tokenCount = balanceOf(msg.sender);

        require(tokenCount <= devReserve,         'This transaction would exceed reserve supply for dev.');
        require(supply + devReserve <= maxSupply, 'This transaction would exceed max supply.');

        _safeMint(msg.sender, devReserve);
    }
    ...
    */
    ...
    function preSale(bytes32[] calldata __proof, uint256 __count) public payable {
        uint256 supply = totalSupply();
        uint256 tokenCount = balanceOf(msg.sender);
        ...
        require(msg.value >= price * __count,  'Ether value is too low');

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(__proof, merkleRoot, leaf), "Not whitelisted.");
    ...
    */
    ...
    function publicSale(uint256 __count) public payable {
        uint256 supply = totalSupply();
        uint256 tokenCount = balanceOf(msg.sender);
    ...
    function setMaxPreSale(uint256 __number) public onlyOwner {
        maxPreSaleMint = __number;
    }

    function setMerkleRoot(bytes32 __merkleRoot) public onlyOwner {
    ...
    function setProvenance(string memory __provenance) public onlyOwner {
