...
import "./guards/StacktControllerGuard.sol";
...
contract StacktWalletFactory {
    ...
    event StacktWalletCreation(StacktProxy proxy, address singleton);

    address public immutable proxyFactoryAddress;
    address public immutable stacktWalletSingletonAddress;
    address public immutable stacktControllerGuardSingletonAddress;
    address public immutable fallbackAddress;

    constructor(address _proxyFactoryAddress, address _stacktWalletSingletonAddress, address _stacktControllerGuardSingletonAddress, address _fallbackAddress) {
        proxyFactoryAddress = _proxyFactoryAddress;
        stacktWalletSingletonAddress = _stacktWalletSingletonAddress;
        stacktControllerGuardSingletonAddress = _stacktControllerGuardSingletonAddress;
    ...
    function createWallet(Setup calldata setup) public payable returns (StacktProxy stacktWalletProxy) {
        ...
        // controller guard
        bytes memory controllerGuardSetupData = abi.encodeCall(StacktControllerGuard.setup, (
            setup.controllers
        ));

        StacktProxy controllerGuardProxy = proxyFactory.createProxy(stacktControllerGuardSingletonAddress, controllerGuardSetupData);
        address controllerGuardAddr = address(controllerGuardProxy);
        ...
        //
        StacktControllerGuard(controllerGuardAddr).setStacktWalletAddress(address(stacktWalletProxy));
        
        emit StacktWalletCreation(stacktWalletProxy, stacktWalletSingletonAddress);
