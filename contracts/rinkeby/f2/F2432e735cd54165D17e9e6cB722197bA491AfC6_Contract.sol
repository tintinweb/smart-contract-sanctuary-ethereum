...
contract Contract is ERC721A, Ownable {
	...
	uint256 public preSalePrice = 0.08 * 10**18;
	uint256 public pubSalePrice = 0.08 * 10**18;
	...
	constructor(address _proxyRegistryAddress) ERC721A("Nathan Mythical Beings 1", "BEINGS1") {
	...
	function mint(uint256 _quantity) public payable {
		...
		if (preSaleIsActive) {
			...
			require(whitelist[msg.sender].hasMinted + _quantity <= maxPerWallet, "Exceeds per wallet pre-sale limit.");
			whitelist[msg.sender].hasMinted = whitelist[msg.sender].hasMinted + _quantity;
		} else {
	...
	function withdraw() external onlyOwner {
		...
		uint256 transferBalance = totalBalance - (wallet1Balance + wallet2Balance + wallet3Balance);
