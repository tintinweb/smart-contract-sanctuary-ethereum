...
// TODO: change contract and token name
contract TokenTBDAuction is Ownable {
    uint8 internal constant PLATINUM_SUPPLY = 100;
    ...
    // Token ID => top bid
    mapping(uint8 => Bid) public currentTopBids;
    ...
    constructor(
        ...
        tokenTBDK = TokenTBD(_tokenTBDAddress);
    ...
    function _placeBid(uint8 tokenId, address recipient) private notLocked {
        Bid memory currentTopBid = currentTopBids[tokenId];
        require(
        ...
        require(!currentTopBid.cancelled, "AUCTION_CANCELLED");

        Bid memory latestBid = Bid(
            msg.sender,
            recipient,
            msg.value,
            false /* cancelled */
        ...
        currentTopBids[tokenId] = latestBid;
    ...
    function endAuction(uint8 tokenId) external notLocked {
        ...
        require(!currentTopBid.cancelled, "AUCTION_CANCELLED");
        require(
            msg.sender == currentTopBid.walletBidder || msg.sender == owner(),
            "ONLY_BIDDER_OR_OWNER_ALLOWED"
        );
        require(
            block.timestamp >= bidTimestamps[tokenId].biddingClosed,
            "BIDDING_NOT_OVER"
        );
        require(address(tokenTBDK) != address(0), "NULL_CONTRACT_ADDRESS");
        require(currentTopBid.walletRecipient != address(0), "NO_WINNERS");
        tokenTBDK.mintPlatinum(tokenId, currentTopBid.walletRecipient);
    ...
    function cancelAuction(uint8 tokenId)
        external
        payable
        onlyPaperOrOwner
        notLocked
    {
        Bid memory currentTopBid = currentTopBids[tokenId];
        require(!currentTopBid.cancelled, "ALREADY_CANCELLED");
        ...
        require(address(tokenTBDK) != address(0), "NULL_CONTRACT_ADDRESS");
        require(!tokenTBDK.mintedPlatinum(tokenId), "AUCTION_ALREADY_ENDED");

        // Pay back bid to top bidder.
        payable(currentTopBid.walletBidder).transfer(currentTopBid.bidAmount);

        // Cancel bid/auction.
        currentTopBids[tokenId].cancelled = true;
        ...
        onlyOwner
    {
        ...
        tokenTBDK = TokenTBD(contractAddress);
    }
    ...
    function migrate(
        BidMap[] calldata _currentTopBids,
        BidHistoryMap[] calldata _bidHistory
    ) external onlyOwner {
        // Update all top bids.
        for (uint8 i = 0; i < _currentTopBids.length; i++) {
            uint8 tokenId = _currentTopBids[i].tokenId;

            currentTopBids[tokenId] = Bid(
                _currentTopBids[i].bid.walletBidder,
                _currentTopBids[i].bid.walletRecipient,
                _currentTopBids[i].bid.bidAmount,
                _currentTopBids[i].bid.cancelled
            );
        }
            ...
            for (uint256 j = 0; j < _bidHistory[i].bids.length; j++) {
                bidHistory[tokenId].push(
                    Bid(
                        ...
                        _bidHistory[i].bids[j].cancelled
                    )
                );
            }
        }
...
// TODO: see contract warnings
// TODO: change contract and token name
contract TokenTBD is ERC721, Ownable {
    using ECDSA for bytes32;

    uint16 public constant PLATINUM_SUPPLY = 100;

    uint32 public constant SECONDS_IN_DAY = 86400;
    uint16 public constant DAYS_TO_REVEAL_STARTING_INDEX = 9;
    ...
    uint16 public mintedGold;
    uint16 public mintedSilver;
    mapping(uint8 => bool) public mintedPlatinum;
    ...
    constructor(
        ...
        silverPrice = _silverPrice; // 0.2 ether
        goldPrice = _goldPrice; // 0.5 ether
        silverSupply = _silverSupply; // 7900
        goldSupply = _goldSupply; // 2000
    }
    ...
    function mint(
        ...
        if (silverQuantityToMint > 0) {
            mintSilverQuantity(silverQuantityToMint);
        }
        if (goldQuantityToMint > 0) {
            mintGoldQuantity(goldQuantityToMint);
        }
    ...
    function mintSilverQuantity(uint16 quantityToMint) private {
        for (uint16 i = 0; i < quantityToMint; i++) {
            _safeMint(msg.sender, getNextSilverTokenID() + i);
        }
        mintedSilver += quantityToMint;
    }

    function mintGoldQuantity(uint16 quantityToMint) private {
        for (uint16 i = 0; i < quantityToMint; i++) {
            _safeMint(msg.sender, getNextGoldTokenID() + i);
        }
        mintedGold += quantityToMint;
    }

    function getNextSilverTokenID() internal view returns (uint16) {
        return mintedSilver + goldSupply + PLATINUM_SUPPLY + 1;
    }

    function getNextGoldTokenID() internal view returns (uint16) {
        return mintedGold + PLATINUM_SUPPLY + 1;
    }

    // TODO: determine whether we need to price verification, as well.
        ...
        auctionNotLocked
    {
        ...
        require(
            tokenID > 0 && tokenID <= PLATINUM_SUPPLY,
            "TOKEN_ID_OUT_OF_PLATINUM_RANGE"
        );
        require(buyer != address(0), "NULL_ADDRESS");
        require(!mintedPlatinum[tokenID], "TOKEN_ALREADY_MINTED");
        require(msg.sender == auctionAddress, "UNAUTHORIZED");

        _safeMint(buyer, tokenID);
        mintedPlatinum[tokenID] = true;
    }
    ...
    function giftGold(address[] calldata receivers) external onlyOwner {
        require(mintedGold + receivers.length <= goldSupply, "OUT_OF_STOCK");
