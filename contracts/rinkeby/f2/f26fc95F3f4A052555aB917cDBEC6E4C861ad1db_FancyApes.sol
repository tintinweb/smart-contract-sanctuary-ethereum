...
contract FancyApes is Fancy721 {
    constructor(string memory  _URI, IERC721Enumerable _referenceContract)
    Fancy721("Fancy Apes", "fAPE", _URI, _referenceContract) 
    {}
}
...
contract Fancy721 is ERC721Enumerable, Ownable, AccessControlEnumerable {
    ...
    function safeMint(address _to, uint256[] calldata _referenceTokenIds) public onlyRole(MINTER_ROLE) {
        for(uint256 i = 0; i < _referenceTokenIds.length; i++){
            require(referenceContract.ownerOf(_referenceTokenIds[i]) == _to, "safeMint: destination address must own bored ape token");
            require(!tokenClaimed[_referenceTokenIds[i]], "safeMint: ape token already used for claim");
