...
contract MultiVesting is Ownable {
  ...
  function getAvailableAmountAtTimestamp(
    ...
    uint256 periodsPassed = _timestamp.sub(vesting.startedAt).div(5 minutes); // We say that 3 months are always 90 days
    ...
    if (periodsPassed <= phase1Peroids) {
      ...
      uint256 rewardPhase1 = rewardPerPhase1Period.mul(phase1Peroids);
      uint256 rewardPhase2 = rewardPerPhase2Period.mul(phase2PeriodsPassed);
