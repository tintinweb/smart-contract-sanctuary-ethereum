...
contract CLAP is ERC721A, ERC2981 , Ownable, Pausable {
    ...
    string internal revealUri = "ipfs://QmSwstuQQNJKmZeNo15qvWPNkU8XQa8tBgEyFbHXUBmLHH";
    ...
    uint256 public needBurnCount = 2;

    function setNextContract(address _contract) external onlyOwner {
        nextContract = _contract;
    }
    function getNextContract() external view returns (address) {
        return nextContract;
    }

    function burnToMint(uint256[] memory burnTokenIds) external {
        NextInterface NextContract = NextInterface(nextContract);

        require(msg.sender == tx.origin, "Cannot be called by contract");
        require((burnTokenIds.length % needBurnCount) == 0, "Too many arguments");
        uint256 newCount = 0;
        ...
        NextContract.mintFromBurn(newCount, msg.sender);
    }
