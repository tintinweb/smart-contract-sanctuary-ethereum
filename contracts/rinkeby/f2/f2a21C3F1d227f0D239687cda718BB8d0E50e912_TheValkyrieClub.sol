...
//Define our contract
contract TheValkyrieClub is ERC721, VRFConsumerBase, Ownable {
    ...
    event GameHasStarted(uint startTime, uint gameDuration);
    ...
    function startTheGame(uint gameDuration) external onlyOwner {
        _start = block.timestamp;
        _end = _start + gameDuration;
        toggleIsMintEnabled();
        emit GameHasStarted(_start, gameDuration);
    }
    ...
    function claimNFT() external  {
        ...
        emit ClaimedNFTSuccessful(msg.sender, tokenId);
    }
