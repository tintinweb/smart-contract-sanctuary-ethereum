...
contract EthAndPrimeRewards is PrimeRewards {
        ...
        onlyOwner
    {
        ...
        ethPoolInfo.push(
            EthPoolInfo({
                ...
                lastRewardBlock: uint64(Math.max(block.number, ethStartBlock))
            })
        );
    ...
    function setEthPerBlock(
        ...
        require(
            ethEndBlock < block.number,
            "Only updates after initial ethEndBlock"
        );
        require(
    ...
    function updateEthPool(uint256 pid) public {
        ...
        uint256 updateToBlock = Math.min(block.number, ethEndBlock);
        uint256 blocks = updateToBlock - ethPool.lastRewardBlock;
        uint256 ethReward = (blocks * ethAmountPerBlock * ethPool.allocPoint) /
            ethTotalAllocPoint;
        ethAmount -= ethReward;
        if (totalSupply > 0) {
    ...
    function claimPools(uint256[] calldata pids) public virtual {
