...
Dapp - Stake
- Stake NFT earn Token

* Requirements *
max 10 NFT  mint per wallet
part prices may change
...
* STEPS *
- Round1 -
1- Whitelist - 0.06 ETH - Adet(?)
2- Public sale - 0.07 ETH - 2500 (max)

- Round2 -
1- whitelist - 0.07 ETH 
2- Public sale 2 - 0.07 ETH

- Round3 -
1- whitelist - 0.07 ETH 
2- Public sale 3 - 0.07 ETH



Deploy contract with not revealed uri
setWhitelist
setWhitelistPrices for each round
setPrices for each round
set maxMintPerRound
set onlyWhitelisted = on/off
...
contract NFTContract is ERC721Enumerable, ERC721Royalty, Pausable, Ownable, ReentrancyGuard {
    ...
    function mint(uint _mintAmount) external payable whenNotPaused nonReentrant {
        ...
        uint addressMintedCount = addressMintedBalance[msg.sender];
        require(addressMintedCount + _mintAmount <= maxMintAmountPerAddress, "max NFT per address exceeded");
    ...
    function _getTokenRound(uint _tokenId) internal view returns (uint) {
        for (uint _i = 1; _i <= totalRound; _i++) {
            uint _roundLimit = maxMintPerRound[_i];

            if (_tokenId <= _roundLimit) return _i;
        }
    ...
    function setmaxMintAmountPerAddress(uint _newmaxMintAmount) public onlyOwner {
