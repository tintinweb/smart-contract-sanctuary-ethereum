...
contract BruFactory is IBruFactory {
    ...
    mapping(string => address) public override poolAddresses;
    ...
    function deployPool(string memory poolName) external override onlyAdmin  {
        ...
        BruPool(poolAddress).initialize(admin,poolName);
        poolAddresses[poolName] = poolAddress;
...
contract BruPool is IBruPool, ERC1155 {
    ...
    //Rate variables
    uint256 STABLE_RATE = uint256(65).div(1000);
    ...
    mapping(string => BorrowDetails) borrowedNft;
    ...
    function initialize(address adminAddress, string memory poolName)
        external
        override
    {
    ...
    function allowTokens(address tokenAddress) external override onlyAdmin {
        require(
    ...
    function mintNft(
        ...
        nft[nftId] = NFT(tokenId, nftValue, false, data);
        unchecked {
            nftIdCounter += 1;
        }
    }
    ...
    function repay(address userAddress, string memory nftId, uint tokenAmount) external {
         require(
            balanceOf(userAddress, nft[nftId].tokenId) == 1,
            "NFT is not owned by this address"
        );
        require(nft[nftId].borrowed == true, "This NFT cannot be repaid for");
        require(tokenAmount <=borrowedNft[nftId].borrowedAmount, "Amount given greate than borrowed amount");
        if ( borrowedNft[nftId].borrowedAmount == tokenAmount ) {
