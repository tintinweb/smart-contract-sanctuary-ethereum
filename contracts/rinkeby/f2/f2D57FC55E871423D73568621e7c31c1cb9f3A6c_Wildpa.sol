...
contract Wildpa is ERC721A, Ownable, ReentrancyGuard {
    ...
    // uint256 public GENERAL_TOKEN = 50; // reserve token id 1 - 50 for 1/1 token

    //Configuration
    uint256 public constant MAX_SUPPLY = 5555;
    uint256 public constant MAX_SUPPLY_COMMON = 4500;
    ...
    uint256 public constant PRICE_COMMON_TOKEN = 0.01 ether;
    uint256 public constant PRICE_RARE_TOKEN = 0.02 ether;
    uint256 public constant PRICE_LEGENDARY_TOKEN = 0.03 ether;    
    mapping(address => uint256) public publicSalesMinterToTokenQty;

    //free mint for founder reserve
    ...
    // }
    // 1/1 mint
    // uint256 public constant PRICE_PER_TOKEN_ONE = 0.015 ether;
    // uint256 public constant MAX_SUPPLY_ONE = 50;
    ...
    modifier mintCompliance(uint256 quantity, string memory Type) {
        // require(quantity > 0 && quantity <= PUBLIC_MINT_LIMIT, 'Invalid mint amount!');
        require(keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("common")) || keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("rare")) || keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("legendary")), "Invalid Type");
        if (keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("common"))){ 
            require(totalMintedCommon() + quantity <= MAX_SUPPLY_COMMON, 'Max supply for common token exceeded!');
        } else if (keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("rare"))){ 
            require(totalMintedRare() + quantity <= MAX_SUPPLY_RARE, 'Max supply for rare token exceeded!');
        } else if (keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("legendary"))){ 
            require(totalMintedRare() + quantity <= MAX_SUPPLY_LEGENDARY, 'Max supply for rare token exceeded!');
        }
    ...
    function airdropTeam(uint256 quantity, address address_to, string memory Type ) 
        external 
        nonReentrant
        onlyOwner
        mintCompliance(quantity, Type)
        {
            if (keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("common"))){ 
            _safeMint(address_to, quantity, Type);    
            for (uint256 i = 0; i < quantity;) {
                uint256 _tokenID = _startTokenIdCommon() + i;
            ...
            _safeMint(address_to, quantity, Type);    
            for (uint256 i = 0; i < quantity;) {
                uint256 _tokenID = _startTokenIdRare() + i;
                _mintedDate[_tokenID] = block.timestamp;
                unchecked{i++;}
            }
            emit Minted(
            ...
            _safeMint(address_to, quantity, Type);    
            for (uint256 i = 0; i < quantity;) {
                uint256 _tokenID = _startTokenIdLegendary() + i;
                _mintedDate[_tokenID] = block.timestamp;
                unchecked{i++;}
            }
            emit Minted(
    ...
    function reserve(uint256 quantity, string memory Type) 
        external 
        nonReentrant
        onlyOwner
        mintCompliance(quantity, Type)
        {       
        if (keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("common"))){ 
            _safeMint(_msgSender(), quantity, Type);    
            for (uint256 i = 0; i < quantity;) {
                uint256 _tokenID = _startTokenIdCommon() + i;
                _mintedDate[_tokenID] = block.timestamp;
                unchecked{i++;}
        ...
        mintCompliance(quantity, Type)
    {
        ...
        if (keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("common"))){ 
            require(msg.value >= PRICE_COMMON_TOKEN * quantity, "Ether value sent is not correct");
            _safeMint(_msgSender(), quantity, Type);    
            for (uint256 i = 0; i < quantity;) {
                uint256 _tokenID = _startTokenIdCommon() + i;
                _mintedDate[_tokenID] = block.timestamp;
            ...
            require(msg.value >= PRICE_RARE_TOKEN * quantity, "Ether value sent is not correct");
            _safeMint(_msgSender(), quantity, Type);    
            for (uint256 i = 0; i < quantity;) {
                uint256 _tokenID = _startTokenIdRare() + i;
                _mintedDate[_tokenID] = block.timestamp;
            ...
            require(msg.value >= PRICE_LEGENDARY_TOKEN * quantity, "Ether value sent is not correct");
            _safeMint(_msgSender(), quantity, Type);    
            for (uint256 i = 0; i < quantity;) {
                uint256 _tokenID = _startTokenIdLegendary() + i;
                _mintedDate[_tokenID] = block.timestamp;
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721A) returns (string memory) {
        ...
        uint256 phase1_evolve_time = mintedDates + 1 minutes;
        uint256 phase2_evolve_time = mintedDates + 2 minutes;
    ...
    // function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override(ERC721A) {
...
contract ERC721A is IERC721A {
    ...
    function totalMintedCommon() internal view returns (uint256) {
        return _currentIndexCommon - _startTokenIdCommon();
    }
    function totalMintedRare() internal view returns (uint256) {
        return _currentIndexRare - _startTokenIdRare();
    }
    function totalMintedLegendary() internal view returns (uint256) {
        return _currentIndexLegendary - _startTokenIdLegendary();
    }
    /**
