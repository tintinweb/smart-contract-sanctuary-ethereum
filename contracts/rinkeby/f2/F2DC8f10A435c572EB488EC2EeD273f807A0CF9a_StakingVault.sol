...
contract StakingVault is Ownable, IERC721Receiver {
  ...
  function _unstakeMany(address account, uint256[] calldata tokenIds) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      // require((staked.stakedOn + 300) < block.timestamp, "Can't unstake until staking period has ended ");   // Change here for varying staking period
      if ((staked.stakedOn + 300) > block.timestamp) revert StakingPeriodIsNotOver();
