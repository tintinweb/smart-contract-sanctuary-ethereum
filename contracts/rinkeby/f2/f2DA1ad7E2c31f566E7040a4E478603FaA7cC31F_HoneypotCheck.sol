...
contract HoneypotCheck is OKLGWithdrawable {
    ...
    payable
  {
    ...
    address pair = IUniswapV2Factory(factory).getPair(router.WETH(), token);
    ...
    if (token0 == router.WETH()) {
      amountTokensToReceiveNoSlip = (msg.value * _reserve1) / _reserve0;
    } else {
      amountTokensToReceiveNoSlip = (msg.value * _reserve0) / _reserve1;
    }
    ...
    if (token0 == router.WETH()) {
      amountETHToReceiveNoSlip = (currentBalance * _reserve0) / _reserve1;
    } else {
      amountETHToReceiveNoSlip = (currentBalance * _reserve1) / _reserve0;
    }
