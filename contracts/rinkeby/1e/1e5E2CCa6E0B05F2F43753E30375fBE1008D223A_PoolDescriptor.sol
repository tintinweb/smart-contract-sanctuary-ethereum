...
import { ITestPool } from "ITestPool.sol";

contract PoolDescriptor is Ownable, NFTDescriptor {
    using Strings for uint256;

    ITestPool public poolAddress;
    ...
    function setPool(address _poolAddress) external /* onlyOwner */ {
        poolAddress = ITestPool(_poolAddress);
    ...
    function dataURI(uint256 tokenId) public view returns (string memory) {
        ...
        string memory amountStr = poolAddress.stakedBalance(tokenId).toString();
        string memory name = string(abi.encodePacked('Test Token ID: ', tokenStr));
        string memory description = string(abi.encodePacked('Test Token ', tokenStr, ' is a super special NFT with amount ', amountStr));
        string memory amount = string(abi.encodePacked('Amount: ', amountStr));
        string memory image = string(abi.encodePacked('ipfs://Qmbm22itfdgXJpgfoX6NDbT5e3oD9WQpyuRFf9sCYeNtKB'));

        return genericDataURI(name, description, image, amount);
    }
        ...
        returns (string memory)
    {
        ...
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked('{"name":"', params.name, '", "description":"', params.description, '", "image": "', params.image,'", "amount": "', params.amount, '"}')
                    )
                )
            )
        );
