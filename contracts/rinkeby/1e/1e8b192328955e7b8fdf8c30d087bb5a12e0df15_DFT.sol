...
contract DFT is Context, IERC20, Ownable {
    ...
    address payable public treasuryWallet = payable(0x13d50FFfd5AB94462135F7A589C539FF7C1BeC59);
    ...
    uint256 public _maxTxAmount = 500000000000000 * 10**9; //0.5% Max buy transaction amount on buy and sell.
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityTokenShare = contractTokenBalance.mul(_liquidityFee).div(_liquidityFee.add(_marketingFee).add(_treasuryFee));
        uint256 marketingTokenShare = contractTokenBalance.mul(_marketingFee).div(_liquidityFee.add(_marketingFee).add(_treasuryFee));
        ...
        swapTokensForWallets(teamTokenShare, treasuryWallet);
    ...
    returns (uint256, uint256)
    {
        ...
        uint256 tTeam = tAmount.div(100).mul(_treasuryFee);
    ...
    function setFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 treasuryFee, uint256 marketingFee) external onlyOwner() {
        _taxFee = taxFee;
