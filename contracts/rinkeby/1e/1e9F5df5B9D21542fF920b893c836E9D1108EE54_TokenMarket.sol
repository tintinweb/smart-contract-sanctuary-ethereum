...
import "contracts/admin/tierLevel/interfaces/IUserTier.sol";
...
contract TokenMarket is TokenMarketBase, PausableImplementation {
        ...
        for (uint256 i = 0; i < loanIds.length; i++) {
            ...
            require(
                (apyFee +
                    (loanDetails.loanAmountInBorrowed -
                        (apyFee + platformFee)) +
                    platformFee) == loanDetails.loanAmountInBorrowed,
                "GLM, invalid amount"
            );
            {
    ...
    function updatePaybackAmount(uint256 _loanId, uint256 _paybackAmount)
        ...
        override
    {
        require(msg.sender == Liquidator, "GTM: Caller not liquidattor");
    ...
    /// @dev this function can return the stable coin apy fee in the contract
    /// @param _stableCoin address of the stable coin address
    /// @return uint256 returns the amount of stable coin in the contract
    function getStableCoinAPYFeeinContract(address _stableCoin)
        ...
        returns (uint256)
    {
        return platformFeeStable[address(this)][_stableCoin];
    }
}
...
abstract contract TokenMarketBase is ITokenMarket {
    ...
    /// @dev mapping to store unearned APY Fee in stable
    /// @dev taking first address as key which represents the token market contract address
    mapping(address => mapping(address => uint256)) public platformFeeStable;
}
