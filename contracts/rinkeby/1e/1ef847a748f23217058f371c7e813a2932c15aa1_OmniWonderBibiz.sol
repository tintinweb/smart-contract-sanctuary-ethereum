...
contract OmniWonderBibiz is ERC721Enumerable, NonblockingReceiver, ILayerZeroUserApplicationConfig {
    using Strings for uint256;

    string public baseTokenURI = "ipfs://QmSy18FYfEFqLak6a9d9o8qkcJNKPmJzrwPMLaY3cF5wkK/";
    uint256 nextTokenId = 0;
    uint256 public immutable maxMint = 666;
    uint256 public immutable maxPerTx = 5;
    uint256 price = 0.005 ether;
    bool public revealed = false;

    constructor(address _layerZeroEndpoint) 
    ERC721("Layer WonderBibiz v2", "LZWBz") {
        endpoint = ILayerZeroEndpoint(_layerZeroEndpoint);
    }

    /// @notice Mint your OmnichainNonFungibleToken
    function mint(uint256 quantity) external payable {
        require(quantity <= maxPerTx && balanceOf(msg.sender) < maxPerTx, "ONLY_5_PER_WALLET");
