 ...
 *
 * TheWhitelistSignature is a ERC721 Contract that supports Burnable.
 * sale.
 */
contract TheWhitelistSignature is ERC721Burnable, ERC2981, Ownable {
    ...
    Counters.Counter private _tokensBurnedCounter;

    // _abseTokenURI serving nft metadata per token
    string private _baseTokenURI = "https://api.thewhitelist.io/signature_tokens/";
     ...
     */
    function mintMultiple(uint256 quantity) public onlyOwner {
    ...
    function setTokenURI(uint256 tokenId, string memory permanentTokenURI)
        ...
        onlyOwner
    {
        require(_exists(tokenId), "TheWhitelistSignature: URI set of nonexistent token");
        _tokenURIs[tokenId] = permanentTokenURI;
        emit TokenUriChanged(msg.sender, tokenId, permanentTokenURI);
    }

    function totalSupply() public view returns (uint256) {
        return _tokenIdTracker.current() - _tokensBurnedCounter.current();
    }
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "TheWhitelistSignature: URI query for nonexistent token"
        );
    ...
    function _burn(uint256 tokenId) internal virtual override {
        super._burn(tokenId);
        _tokensBurnedCounter.increment();
