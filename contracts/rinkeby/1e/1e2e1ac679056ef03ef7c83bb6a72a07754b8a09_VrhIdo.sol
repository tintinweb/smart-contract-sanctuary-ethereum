...
contract VrhIdo is Ownable {
    ...
    event Purchase(address indexed buyer,uint256 indexed round, uint256 vrhAmount, uint256 lockedVrhAmount, uint256 lockedEnd, uint256 ratio);
    ...
    constructor(address _vrhTokenAddress, address _quoteTokenAddress, address _votingEscrowAddress, address _fundAddress, uint256 _lockedVrhRatio) {
        ...
        if(_quoteTokenAddress == address(0)){
            ...
            quoteTokenDecimals = IERC20(quoteTokenAddress).decimals();
        }
    ...
    function purchase(uint256 amount, uint256 yearCount) external payable {
        ...
        if(quoteTokenAddress == address(0)){
            ...
            require(IERC20(quoteTokenAddress).transferFrom(msg.sender, fundAddress, amount), "token insufficient");
        ...
        emit Purchase(msg.sender, index, vrhAmount, lockedVrhAmount, end, idoRound.ratio);
    }
