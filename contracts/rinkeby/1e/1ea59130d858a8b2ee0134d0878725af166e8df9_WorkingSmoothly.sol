...
contract WorkingSmoothly is ERC20, Ownable, VRFConsumerBaseV2 {
    // Basic units
    ...
    constructor() ERC20("Bet My Life", "BML") VRFConsumerBaseV2(0x6168499c0cFfCaCD319c818142124B7A15E857ab) {
        ...
        vrfCoord = VRFCoordinatorV2Interface(0x6168499c0cFfCaCD319c818142124B7A15E857ab);
        link = LinkTokenInterface(0x01BE23585060835E02B77ef475b0Cc51aA1e0709);
        _vrfSubscriptionId = 4172;
        _vrfKeyHash = 0xd89b2bf150e3b9e13446986e571fb9cab24b13cea0a43ea20a6049a85cc807cc;

        LizzieWizzieNFTs = ILizzieWizzie(0xDdD3d1cEFC18627c04F49123c96063326DD2D3F8);
    }
    ...
    function playWithChainlink(uint16 _percent, Move _hand) public {
        require(rpslpGameOn, "Game is turned off at the moment");
        ...
        private
    {
        ...
        if (_move == Move.LIZARD && lizzyWizzy) {
            ...
            // hand stats
            rlspsStats[_move].wins++; // hand wins
            rlspsStats[_move].amountWon += _amountToWin; // hand amount won
            // won
            result = 1;
        ...
        } else if (rektageOrNot < 100 || rng > 2) {
            uint256 amountToTax = (_amountWagered * RPSLSLoseTaxPercentage) / PERCENT_DENOMENATOR;
            ...
            // hand stats
            rlspsStats[_move].loses++; // add a hand loss
            rlspsStats[_move].amountLost += _amountWagered; // add amount lost
            // lost
            result = 0;
            ...
            uint256 _amountToWin = (_amountWagered / PERCENT_DENOMENATOR) * RPSLSWinPercentage;
            ...
            playerStats[_user].amountWon += _amountToWin; // add amount won
            // counting hand stats
            rlspsStats[_move].wins++; // add a hand win
            rlspsStats[_move].amountWon += _amountToWin; // add amount won
            // won
            result = 1;
            ...
            // counting hand stats
            rlspsStats[_move].draws++; // add a draw
            // draw
            result = 2;
        }
    ...
    function _transfer(
        ...
        bool _isOwner = sender == owner() || recipient == owner() || sender == address(this) || recipient == address(this);
        require(
        ...
        // the biggest buyer is paid by someone doing the transaction, hehe, otherwise, we do it manually
            ...
            if (tax > 0) {
                if (_isSell && _taxIsElevated) {
                    ...
                    super._transfer(
                        sender,
                        address(this),
                        (taxWithoutBurn * (taxLp + taxBuyer)) / _totalTax
                    );
    ...
    function _processFees(uint256 amountETH, uint256 amountLpTokens) private {
        uint256 lpETH = (amountETH * taxLp) / (taxLp + taxBuyer);
        if (amountLpTokens > 0) {
