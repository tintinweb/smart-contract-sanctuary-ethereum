...
/***************************\
* BLOCK BANG SMART CONTRACT *
\***************************/

contract BlockBangScTest is ERC721, Ownable, Pausable {
	...
	uint256 private 						_price_whitelist = 0.09 ether;
    uint256 private 						_price_public = 0.15 ether;
	uint256 private 						_mint_supply = 1000;
	uint256 private 						_mint_limit = 10;
	uint256 private 						_mint_limit_whitelist = 2;
	string private 							_base_url = 'ipfs://QmQpdZtzqGSU8HoNnizyhburLSLJYn7z17u3mVVzkt7dPR/hidden.json';
	bool private							_is_whitelist = true;

	// Construct
	constructor() ERC721('BlockBang', 'BLCKBNGTEST') {}
	...
	// Get
	function get_price_whitelist() external view returns (uint256) {
        return _price_whitelist;
	}
	function get_price_public() external view returns (uint256) {
       return _price_public;
	}    
	...
	function mint(uint256 mint_num) public payable {
        // Set public price
        uint256 _price = _price_public;

        // If whitelist is on, set whitelist price
        if (_is_whitelist) {
            _price = _price_whitelist;
        }

		// Entire mint supply has been minted
		require(_total.current() + mint_num <= _mint_supply, 'exceeded remaining');

		// Must mint at least one
		require(mint_num > 0, 'must mint at least one');
