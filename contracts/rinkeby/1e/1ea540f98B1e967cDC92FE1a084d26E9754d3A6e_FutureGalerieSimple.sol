...
/*
	NOTES:
		- Owners want ability to release more later (i say we airdrop 203, supply 1000,
		  pause the possible mint function after airdrop)
...
contract FutureGalerieSimple is Satoshigoat {
	...
	mapping(uint256 => string) internal _tidToHash;//token id -> ipfs hash

	//_levelHashes[0] is Level 1 ... etc
	string [NUM_LEVELS] _levelHashes = ["QmV9tumN7tkC25rKcF6rGxmDts1iarKAMgYRcQXWFtk1GM",
						  	  		    "QmXd7JUxxeAHZUVavMRCNHRnP8hvjpKXvGYS4eaeQt2bJP",
						  	   		    "QmdgF9nkfKwY3gn5ygGuEdHtztzx56qUccMCAxusiT6aFd",
						  	   			"QmVFRFgmkqA7D1dy3prwT8Kef6Bc48X62abjjojf2ed4DR"];
	// -----------
	// RESTRICTORS
	// -----------
	...
	{
		_contractURI = "ipfs://QmdjH9mUmEutXGvNvfhSyKxnGLp9JiR2Jf6DTNskS138go";
	...
	{	
		...
		return string(abi.encodePacked(_baseURI(), _tidToHash[tid]));
	}
	...
	function _beforeTokenTransfers(
    	...
    	// If `to` already has an "orb" (and isn't the owner), 
    	// don't let the transfer occur
    	if (balanceOf(to) > 1 || !isInSquad(to)) return;
    	...
    	unchecked {
    		...
    		uint i;
    		string memory member = _levelHashes[0];
    		for (i = startTokenId; i < endingId; i++) {
				...
				setStorageTokenIDHash(i, member);
		    }
		}
    }
	...
	//@dev Allows owners to airdrop to address `to` at any level
	function singleAddressAirdrop(address to, uint8 level) 
		external
		isSquad 
		nonReentrant
		notContract(to)
		...
		enoughSupply(1) 
	{
		...
		_safeMint(to, 1);
		setStorageTokenIDHash(currentIdx, _levelHashes[level-1]);
	}
	...
	function bulkAddressAirdrop
	(
		address[] memory airdropAddressesArray, 
		uint startTokenID,
		uint endingTokenID
	) 
		external 
		isSquad
		nonReentrant
		enoughSupply(endingTokenID - startTokenID - 1)
	{
		unchecked {
			uint i;
			for (i = 0; i < airdropAddressesArray.length; i++) {
				...
				setStorageTokenIDHash(currentIdx, _levelHashes[0]);
			}
		}
	}
	...
	// ------------------------------------------------
	// FUNCTIONS TO MANUALLY UPDATE METADATA PER TOKEKN
	// ------------------------------------------------
	...
	(
		...
		levelInBounds(level)
	{
		if (tidsLength != tids.length)
			revert DataError("length(`tids)` != `tidsLength`");
		if (level < 1 || level > NUM_LEVELS)
			revert DataError("level must be between 1 and 4");

		unchecked {
			uint i;
			string memory lvl = _levelHashes[level-1];
			for (i = 0; i < tidsLength; i++) {
				if (!_exists(tids[i]))
					revert OwnerQueryForNonexistentToken();
				setStorageTokenIDHash(tids[i], lvl);
			}
		}
	}
	...
	//@dev [PRIVATE METHOD] Change the ipfs hash associated with a token id `tid` 
	function setStorageTokenIDHash(uint256 tid, string memory newHash)
		private onlyValidTokenID(tid) notEqual(_tidToHash[tid], newHash) { _tidToHash[tid] = newHash; }
}
