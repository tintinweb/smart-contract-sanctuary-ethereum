...
contract CandyTest2 is ERC1155Supply, Ownable, ReentrancyGuard {
    ...
    //paid accessors
    function updateCandyPrice(uint256 _price) external onlyOwner {
        price = _price;
    }
    ...
    function updateMaxSupply(uint256 _id, uint256 _maxSupply)
        external
        onlyOwner
    {
    ...
    function updatePrice(uint256 _id, uint256 _newPrice) external onlyOwner {
        require(ShopItems[_id].maxSupply > 0, "Shop item does not exist");
        ShopItems[_id].candyPrice = _newPrice;
    }

    //update max per address
    function updateMaxPerAddr(uint256 _id, uint256 _newMaxPerAddr)
        external
        onlyOwner
    {
        require(ShopItems[_id].maxSupply > 0, "Shop item does not exist");
    ...
    function toggleSale(uint256 _id) external onlyOwner {
        require(ShopItems[_id].maxSupply > 0, "Shop item does not exist");
