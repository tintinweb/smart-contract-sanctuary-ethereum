...
contract AirdropContract is Ownable{
    ...
    function setErc20Address(address add)public onlyOwner{
       _ierc20 = ierc20(add);
    } 
    

    function depositErc20(uint256 amount)public onlyOwner{
       require(_ierc20.balanceOf(msg.sender)>=amount*10**18,"not enoughn balance");
      _ierc20.transferFrom(msg.sender,address(this),amount*10**18);  
    }
    ...
    function getReward(address add,bytes32[] memory proof)internal view returns(uint){
        bool check;
   ...
   function claimaAirDrop(bytes32[] memory proof)external {
       uint airDropAmount =getReward(msg.sender,proof);
       require(_ierc20.balanceOf(address(this))>=airDropAmount*10**18,"not enough balance");
       require(checkClaimed[msg.sender]==false,"already claimed airdrop");
       checkClaimed[msg.sender]=true;
       _ierc20.transfer(msg.sender, airDropAmount*10**18);
      
    } 
   
   function setRewardTierAndRoot(uint[] memory settier, bytes32[] memory _root )public onlyOwner{
   ...
   function withdrawErc20(address erc20addd,address _to)public onlyOwner{
      ierc20  _erc20interface=ierc20(erc20addd);
