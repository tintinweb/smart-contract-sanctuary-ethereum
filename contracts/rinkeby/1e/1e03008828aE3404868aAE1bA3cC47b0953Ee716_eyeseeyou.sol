...
contract eyeseeyou is ERC721, ERC721Enumerable , ERC721URIStorage, Ownable {
    ...
    uint256 private maxTokensForUser = 2;
    uint256 private _presaleMintPrice = 0.01 ether;
    uint256 private _publicsaleMintPrice = 0.02 ether;
    bool    private _isLive;
    string  private baseURL = "ipfs://QmfM6RGPN79CxURx3Bmboh7f2J9r18Mga7ZctkzvtCJmnw/";
    ...
    function internalSale() internal onlyOwner {
        _isLive = !_isLive;
    }
    ...
    function isWhitelisted(address addressToCheck) public view returns(bool) {
        return whitelistedAddresses[addressToCheck];
    }
    function addUsers(address[] calldata _addressToWhitelist) external onlyOwner {
        for(uint256 i = 0; i < _addressToWhitelist.length; i++) {
           whitelistedAddresses[_addressToWhitelist[i]] = true;
          }
        }
    function safeMint(address to) payable public whenSaleStarted {
        if(owner() != to){
        ...
        if(tokenId <= reserved) { 
        require(whitelistedAddresses[msg.sender], "This is a presale mint, You are not on the list.");
    ...
    function contractURI() public pure returns (string memory) {
        return "ipfs://Qmd7UK6476DD41supMsdyFgfQCFj3LumnMhuJMJ2VTaDd1";
   }
}
