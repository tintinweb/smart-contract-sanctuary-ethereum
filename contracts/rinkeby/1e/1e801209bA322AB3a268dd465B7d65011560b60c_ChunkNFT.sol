...
contract ChunkNFT is ERC721, Ownable, ReentrancyGuard, VRFConsumerBase {
    ...
    // event MintCost(uint256 mintCost, uint256 mintSent);
    ...
    function updateMetadataIpfsHash(
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(_msgSender(), metadataIpfsCid, _tokenId)
        );
    ...
    function mint(uint16 _numberOfTokens, bytes memory signature)
        external
        payable
        callerIsUser
    {
        ...
        if (!isWhiteListSaleEnded) {
            ...
            whitelistMintCount += _numberOfTokens;
        }

        uint256 cost = PRICE_PER_TOKEN * _numberOfTokens;

        //emit MintCost(cost, msg.value);
        ...
        if (_msgSender() != owner()) {
            ...
            ownerMintCount += _numberOfTokens;
        }

        // increate addr mint count & total mint count
        uint16 _addressMintedCount = addressMintCount[_msgSender()];
        addressMintCount[_msgSender()] = _addressMintedCount + _numberOfTokens;
        _totalMinted += _numberOfTokens;
