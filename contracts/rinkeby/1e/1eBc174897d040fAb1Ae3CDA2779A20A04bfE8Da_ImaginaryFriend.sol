    ...
    ERC2981SinglePercentual
{
    ...
    /// @dev Reverts if the token has not yet been revealed (no answer or
    /// auto-generation).
        ...
        returns (Features memory)
    {
        ...
        (Features memory features, bool isRevealed) = renderer.tokenFeatures(
            tokenId,
            data,
            _computeEntropy(all)
        );

        if (!isRevealed) revert TokenNotYetRevealed();
