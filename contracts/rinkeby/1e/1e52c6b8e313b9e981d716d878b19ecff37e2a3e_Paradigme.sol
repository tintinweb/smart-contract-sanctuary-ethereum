...
contract Paradigme is ERC721Enumerable, Ownable {
  using Strings for uint256;
 ...
 mapping(uint256 => _bidDetail) public bidDetail;
  ...
  function pause(bool _state) public onlyOwner {
    ...
    function _mint(uint256 tokenId,uint256 salePrice, uint256 royalty,SellType sellType, uint256 auctionStart, uint256 auctionExpire, address[] memory _collaborator) public payable returns(uint256){
        ...
        tokenDetails[tokenId] = _tokenDetails(msg.sender,_collaborator,msg.sender,royalty,auctionStart,auctionExpire,salePrice,sellType,true,true,false);
        return tokenId;
    }
    ...
    function setCollaboratorOfNft(uint256 tokenId,address[] memory _collaborator) public {
        require(msg.sender == tokenDetails[tokenId].creator, "Caller is not token creator");
        tokenDetails[tokenId].collaborator = _collaborator;        
    }
    ...
    function buy(uint256 tokenId) payable public {
        ...
        uint256 royaltyAmt;
        uint256 feeAmt;
        if(tokenDetails[tokenId].royalty > 0 && tokenDetails[tokenId].firstSale){
                royaltyAmt = tokenDetails[tokenId].salePrice.mul(tokenDetails[tokenId].royalty).div(10000);
                royaltyDistribution(tokenId,royaltyAmt);            
        }
  ...
  function royaltyDistribution(uint256 tokenId, uint256 amount) internal {
            uint256 payAmount = amount.div(tokenDetails[tokenId].collaborator.length.add(1));
            payable(tokenDetails[tokenId].creator).transfer(payAmount);
            for(uint i = 0; i < tokenDetails[tokenId].collaborator.length; i++){
                payable(tokenDetails[tokenId].collaborator[i]).transfer(payAmount);
            }
  }
