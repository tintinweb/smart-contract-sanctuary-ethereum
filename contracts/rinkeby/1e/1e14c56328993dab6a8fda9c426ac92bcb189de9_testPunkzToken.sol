...
contract testPunkzToken is ERC20, Ownable{
    ...
    mapping (address => uint[]) public addressToTotalToken;
    mapping (uint => uint) public lockClaim;
    ...
    uint public punkXMintingTime = 1644086191;
    ...
    function stake (uint[] memory tokenIds) external {
        ...
        for (uint i;i<tokenIds.length;i++) {
            ...
            addressToTotalToken[msg.sender].push(tokenIds[i]);
            Punkz.safeTransferFrom(msg.sender,address(this),tokenIds[i]);
        }
    }

    //bonus claim award to the people who claims within 3 days of the staking protocol launch and have tokenId < 2
    ...
    function dailyReward(uint[] memory tokenIds) external {
        ...
        for(uint i=0;i<tokenIds.length;i++){
            require(tokenOwner[tokenIds[i]]==msg.sender,"Not token owner");
            uint timeInDays = (block.timestamp - lastClaim[tokenIds[i]])/ 10 ;
            if(timeInDays >= 1) {
                combo += 1;
            }
            totalAmount += timeInDays*rewards[(tokenIds[i]-1)/2];
    ...
    function claimLockedValue() external {
        ...
        for(uint i=0;i<addressToTotalToken[msg.sender].length;i++){
            ...
            uint timeInDays = (block.timestamp - lockClaim[addressToTotalToken[msg.sender][i]])/ 10 ;
            if(timeInDays >= 1) {
                combo += 1;
            }
            totalAmount += timeInDays*rewards[(addressToTotalToken[msg.sender][i]-1)/2];
