...
contract Demo is MaxAccess
  ...
  modifier presaleChecks() {
    if (balanceOf(msg.sender) >= 4) {
      revert MaxSplaining({
        reason: "Token: Tap the hate brakes Thanos, 4 is the maximum at this time."
      });
    }
    _;
  }
  ...
  function presaleMint(
    ...
    if (quant > 2) {
      revert MaxSplaining({
        reason: "Token: Maximum of 2, not 3, can mint 1, but not 3 or more at a time."
      });
    }
    for (uint x = 0; x < quant;) {
      ...
      if (balanceOf(msg.sender) >= 4) {
        revert MaxSplaining({
          reason: "Token: Ok, trying to mint 5? Just mint on another chain!"
        });
      }
      // mint it
  ...
  , uint tokenId
    ...
    if (trustedRemoteLookup[_chainId].length == 0) {
      revert MaxSplaining({
        reason: "Token: Ok this.theDev() didn't set this paramater, contact MaxFlowO2.eth."
      });
    }
    ...
    if (messageFee > msg.value) {
      revert MaxSplaining({
        reason: "Token: The fees outweigh the amount sent."
      });
    }
...
abstract contract MaxAccess is MaxErrors
  ...
  constructor() {
    ...
    contractRoles.add(ADMIN, TheDev);
  ...
  function transferDeveloper(
    ...
    virtual
    override {
    this.grantRole(DEVS, newDeveloper);
    this.renounceRole(DEVS);
  }
  ...
  function acceptDeveloper()
    ...
    virtual
    override {
    this.renounceRole(PENDING_DEVS);
    _grantRole(DEVS, msg.sender);
  }
  ...
  function transferOwnership(
    ...
    virtual
    override {
    this.grantRole(OWNERS, newOwner);
    this.renounceRole(OWNERS);
  }
  ...
  function acceptOwnership()
    ...
    virtual
    override {
    this.renounceRole(PENDING_OWNERS);
  ...
  function showMaxAccessERCCodes()
    public
    view
    returns (bytes4, bytes4 ,bytes4 ,bytes4 ,bytes4) {
    return (type(IRole).interfaceId, type(IDeveloper).interfaceId, type(IDeveloperV2).interfaceId, type(IOwner).interfaceId, type(IOwnerV2).interfaceId);
  }
}
