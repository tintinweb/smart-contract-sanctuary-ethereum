...
contract AIKaijuKinz is ERC721Optimized, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256 public constant maxSupply = 3333;
    uint256 public maxPurchase = 5;
    uint256 public cost = 0.02 ether;
    string public _baseKKAIURI;
    
    event Minted(address indexed mintAddress, uint256 indexed tokenId);
    event PermanentURI(string _value, uint256 indexed _id);

    constructor(string memory baseURI) ERC721Optimized("AIKaijuKinz", "KKAI") {
        _baseKKAIURI = baseURI;
    }
    ...
    function setMintPrice(uint256 newPrice) public onlyOwner {
        require(newPrice >= 0, "KKAI price must be greater than zero");
        cost = newPrice;
    }
    ...
    function mint(uint256 numberOfTokens) public payable nonReentrant {
        ...
        require(numberOfTokens <= maxPurchase, "You can mint max 10 tokens per transaction");
