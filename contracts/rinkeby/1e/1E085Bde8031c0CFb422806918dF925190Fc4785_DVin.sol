...
// Access control used to delegate minting permissions to admin or sale contracts or others

interface ITieredNFT {
    function mintTier(uint256, address) external;
}

contract DVin is ERC721, AccessControl, ITieredNFT {
    using Strings for uint256;

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant OWNER_ROLE = keccak256("OWNER_ROLE");
    uint256 constant PREFIX_MUL = 10000000000; /*Prefix for tiered token IDs*/
    ...
    mapping(uint256 => Counters.Counter) private _tierCounters;

    constructor(string memory _contractURI, string memory baseURI_)
        ERC721("DVin", "DV")
    {
    ...
    *****************/
    ...
    function mintTier(uint256 _tier, address _dst) external override {
        ...
        if (_tierCounters[_tier].current() == 0) {
            _tierCounters[_tier] = Counters.Counter({
                _value: PREFIX_MUL * _tier
            }); /*Initialize token IDs for new tier*/
        }
        _tierCounters[_tier].increment();

        uint256 _id = _tierCounters[_tier].current();
    ...
    function setTradingState(bool _enabled) external {
        require(hasRole(OWNER_ROLE, msg.sender), "!owner");
    ...
    *****************/
    ...
    function _beforeTokenTransfer(
        ...
        require(
            from == address(0) || to == address(0) || tradingEnabled,
            "!enabled"
        );
    }
}
