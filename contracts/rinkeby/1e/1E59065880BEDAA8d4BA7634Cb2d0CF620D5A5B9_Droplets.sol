...
contract Droplets is ERC721A, Ownable {
    ...
    constructor(string memory baseURI_) ERC721A("Droplets", "DRIP") {
        baseURI = baseURI_;
    }
    ...
    modifier onlyExternallyOwnedAccount() {
        require(
            ...
            "Droplets: not externally owned account"
        );
        _;
    }
    ...
    function tokensRemainingForAddress(address who)
        public
        view
        returns (uint256)
    {
    ...
    function buy(uint256 quantity) external payable onlyExternallyOwnedAccount {
        require(
            ...
            "Droplets: mint exceeds max supply"
        );
        require(saleState == SaleState.Open, "Droplets: sale state mismatch");
        require(
            msg.value >= TOKEN_PRICE * quantity,
            "Droplets: insufficient value"
        );
        require(
            tokensRemainingForAddress(msg.sender) >= quantity,
            "Droplets: mint limit for user reached"
        );
        ...
        onlyValidProof(proof)
    {
        ...
        require(
            saleState == SaleState.Restricted,
            "Droplets: sale state mismatch"
        );
        require(
            msg.value >= TOKEN_PRICE * quantity,
            "Droplets: insufficient value"
    ...
    function batchMint(
        ...
        require(
            ...
            "Droplets: arguments length mismatch"
        );
        ...
        for (uint256 i; i < recipients.length; i++) {
            ...
            require(
                supply <= MAX_SUPPLY,
                "Droplets: batch mint exceeds max supply"
            );
