...
contract TwizzyNft is ERC721Enumerable, Ownable {
    ...
    modifier onlyActiveSale() {
        require(saleState == true, "Sale must be active");
        _;
    }
    ...
    constructor(string memory name, 
                ...
                bool saleState,
                IERC20 paymentToken) ERC721(name, symbol) {
                    setSaleState(saleState);
                    setPaymentToken(paymentToken);
                }

    function mint(uint _numberOfTokens) public payable onlyActiveSale {
        require(_numberOfTokens <= maxMint, "Cant mint so much");
        require(_numberOfTokens + totalSupply() <= maxElements, "Exceeds maximum mint supply");
        require(msg.value == _numberOfTokens * mintPrice, "Wrong amount");
    ...
    function publicMint() external  {
        require(saleState, "Blacksmith sleeping");
        require(maxElements > totalSupply(), "Armory empty");
        require(!minter[msg.sender], "You have already minted");
        minter[msg.sender] = true;
        _mintOne(msg.sender);
    }

    function mintERC20(uint _numberOfTokens) public {
        uint256 amountToPay = mintPrice * _numberOfTokens;
