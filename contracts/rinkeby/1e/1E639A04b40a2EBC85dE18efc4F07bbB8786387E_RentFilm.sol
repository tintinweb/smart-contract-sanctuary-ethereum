...
contract RentFilm is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard {
    ...
    function _isPaidFee() private returns(bool) { 
        ...
        if(expectedVABAmount > 0 && expectedVABAmount >= PAYOUT_TOKEN.balanceOf(msg.sender)) {
            ...
            PAYOUT_TOKEN.transfer(STAKING_POOL, expectedVABAmount);
            return true;
        } else {
            return false;
        }
    ...
    /// @notice Approve/Deny all of pending-withdraw by Auditor
    function releasePendingWithdraw() external onlyAuditor nonReentrant {
        require(withdrawRequesters.length > 0, "releasePendingWithdraw: No requester");

        // Transfer withdrawable amount to all requesters
        for(uint256 i; i < withdrawRequesters.length; i++) {
            if(userInfo[withdrawRequesters[i]].withdrawAmount > 0) {
                if(userInfo[withdrawRequesters[i]].withdrawAmount <= userInfo[withdrawRequesters[i]].amount) {
                    transferWithdraw(withdrawRequesters[i], i);
                }            
            }
        }
    }
    ...
    function transferWithdraw(address _to, uint256 registIndex) private returns(bool flag_) {
...
interface IUniHelper {
    ...
    function expectedAmount(uint256 _depositAmount, address _incomingAsset) external returns (uint256 amount_);
