...
contract SmartContract is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmountini = 500000000000001 * 10**_decimals; //500T
    uint256 public swapTokensAtAmountfin = 1000000000000001 * 10**_decimals; //1Q
    uint256 public maxTxAmount = 500000000000001 * 10**_decimals; //500T
    uint256 public maxWalletSize = 6000000000000001 * 10**9; //6Q
    ...
    string private constant _name = "FTS3";
    string private constant _symbol = "FTS3";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xDC41032b5CaA4D20bc45C3d79225834D94586b11),
   ...
   function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (lpPairs[to]) { 
            ...
            if(!swapping && swapEnabled && from != lpPair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
                if (contractTokenBalance >= swapTokensAtAmountini) {
                    if(contractTokenBalance >= swapTokensAtAmountfin) { 
                        contractTokenBalance = swapTokensAtAmountfin; }
    ...
    function updateSwapTokensAtAmount(uint256 amount) external onlyOwner{
        swapTokensAtAmountini = amount * 10 **_decimals;
        swapTokensAtAmountfin = amount * 10 **_decimals;
    }
