    ...
    PaymentSplitter
{
    ...
    uint256 public constant PRICE = 22220000000000000; // 0.02222 ETH
    uint256 public constant MAX_SUPPLY = 22224; // real max supply is 22222 - saving gas for the minters
    uint256 public constant MAX_PER_WALLET = 23;
    uint256 public constant MAX_PER_TRX = 23;
    uint256 public TMP_MAX = 2224; // Temporary max, for gen0, gen1 etc.

    bytes32 public allowlistMerkleRoot;

    bool public isPublic = false;
    bool public isPresale = false;
    bool public isNextGenPublic = false;
    bool public burningAllowed = false;

    mapping(address => uint) public addressToMinted;

    constructor(address[] memory _payees, uint256[] memory _shares)
        ERC721("Meta Punk Mint Pass", "MPMP")
        PaymentSplitter(_payees, _shares)
    {}

    // public mint is capped to 22 per wallet 
    // this function is only used for gen0 public mint only
    function mint(uint amount) external payable nonReentrant {
        require(isPublic, "Wait for public sale");
        require(addressToMinted[msg.sender] + amount < MAX_PER_WALLET, "Max per wallet 22");
        uint256 supply = _owners.length;
        require(supply + amount < TMP_MAX, "Max supply reached");
      ...
      function nextGenMint(uint amount) external payable nonReentrant {
        require(isNextGenPublic, "Wait for next gen public sale");
        require(amount < MAX_PER_TRX, "Max per transaction");
        uint256 supply = _owners.length;
        require(supply + amount < TMP_MAX, "Max supply reached");
        require(PRICE <= msg.value, "Invalid funds provided");
        for(uint i = 0; i < amount; ++i) {
    ...
    function allowlistMint(uint256 amount, uint256 allowance, bytes32[] calldata proof)
        external
        nonReentrant
    {
        ...
        require(
            addressToMinted[msg.sender] + amount <= allowance,
    ...
    function toggleNextGenPublicSale() external onlyOwner {
        isNextGenPublic = !isNextGenPublic;
    }
    ...
    function setTemporaryMax(uint supply) external onlyOwner {
        require(supply <= MAX_SUPPLY, "Cannot be more than max supply");
        TMP_MAX = supply;
    }
