...
contract NFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        mintTeamReserve(_teamAddress, _teamReserve);
    }

    function mintTeamReserve(address _teamAddress, uint256 _teamReserve)
        internal
    {
        uint256 supply = totalSupply();
        for (uint256 i = 1; i <= _teamReserve; i++) {
            _safeMint(_teamAddress, supply + i);
        }
    }
    ...
    function mint(uint256 _mintAmount) external payable nonReentrant {
        ...
        uint256 supply = totalSupply();
        require(supply + teamReserve + _mintAmount <= maxSupply);
        // TODO: check price of allow mint and public mint
        require(mintCost * _mintAmount == msg.value, "Invalid price");
        ...
        {
            uint256 mintedNum = mintedNums[msg.sender];
            require(
                mintedNum + _mintAmount <= mintLimit,
                "Excess the mint limit"
            );
            mintedNums[msg.sender] = mintedNum + _mintAmount;
        }
    ...
    function isAllowListed(address _user) public view returns (bool) {
        address[] memory tempAllowListAddresses = allowlistAddresses;
        for (uint256 i = 0; i < tempAllowListAddresses.length; i++) {
            if (tempAllowListAddresses[i] == _user) {
                return true;
            }
        }
        return false;
