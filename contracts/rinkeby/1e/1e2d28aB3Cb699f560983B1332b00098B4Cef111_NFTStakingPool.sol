...
contract NFTStakingPool is AccessControl, ERC1155Holder {
  ...
  uint256 constant public REWARD_PER_STAKED_DECIMALS = 1000000000000;

  uint256 constant public STAMINA_DECIMALS = 1000000000;
  ...
  mapping(address => mapping(uint256 => uint32[])) public stakedNFTs;
  ...
  function setStakedNft(address _account, uint256 _level, uint32 _index, uint32 _nftId)
    external onlyEditor {
  ...
  function addStakedNft(address _account, uint256 _level, uint32 _nftId)
    external onlyEditor {
      uint32[] memory nfts = stakedNFTs[_account][_level];
      bool wasAdded = false;
      for (uint j = 0; j < nfts.length; j++) {
        if (nfts[j] == 0) {
          stakedNFTs[_account][_level][j] = uint32(_nftId);
          wasAdded = true;
          break;
        }
      }
      if (!wasAdded) {
        stakedNFTs[_account][_level].push(uint32(_nftId));
      }
  }

  function removeStakedNft(address _account, uint256 _level, uint32 _nftId)
  ...
  function _initNft(address _nftContractAddress, uint256 _tokenId, uint32 _stamina, bool _isStaked, uint24 _staminaEventId) internal returns (uint32 maxStamina) {
    ...
    rainiNfts[rainiNftCount] = RainiNft({
      lastStamina: _stamina == 0 ? _cl.maxStamina : _stamina,
  ...
  function removeStaminaEvent(uint24 _removeId, uint24 _prevId) external onlyEditor {
    if (staminaEvents[_removeId].timeStamp > block.timestamp) {
      require(staminaEvents[_prevId].next == _removeId, 'bad prevId');
  ...
  function getTokenStaminaTotal (uint256 _tokenId, address _nftContractAddress) public view returns (uint256 stamina) {
    ...
    if (_rainiNft.isStaked) {
      uint256 degraded = (_rainiNft.maxStamina * (block.timestamp - _rainiNft.lastUpdated)) / staminaDuration;
  ...
  function mergeTokens(uint256 _newTokenId, uint256[] memory _tokenIds, address _nftContractAddress) external onlyEditor {
    ...
    IRainiNft1155 tokenContract = IRainiNft1155(_nftContractAddress);
    IRainiNft1155.TokenVars memory _tv = tokenContract.tokenVars(_newTokenId);
    require(_tv.cardId != 0, "Invalid token");
    IRainiNft1155.CardLevel memory _cl1 = tokenContract.cardLevels(_tv.cardId, 1);
    IRainiNft1155.CardLevel memory _cl2 = tokenContract.cardLevels(_tv.cardId, 2);

    uint256 _stamina = 0;
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      _stamina += getTokenStaminaTotal(_tokenIds[i], _nftContractAddress);
    }
    _stamina = (_stamina * _cl2.maxStamina) / (_cl1.maxStamina * _tokenIds.length);
  ...
  function recoverNfts(uint256[] memory _tokenId, address[] memory _contractAddress, address[] memory owner) external onlyOwner {
    for (uint256 i; i < _tokenId.length; i++) {
      ...
      bool ownsNFT = false;
      uint32[] memory nfts = stakedNFTs[owner[i]][_nft.level];
      for (uint j = 0; j < nfts.length; j++) {
        if (nfts[j] == _nftId) {
          ownsNFT = true;
  ...
  function getBalanceUpdateEventIds(address _account, uint256 _level, uint256[] memory _startIds, uint32[] memory nftOrder, uint256 _maxSteps) 
    public view returns (int256[] memory eventIds) {
      ...
      uint32[] memory _stakedNFTs = stakedNFTs[_account][_level];
      uint256 lastEventId = 0;
      int256[] memory _eventIds = new int256[](nftOrder.length);
      ...
      for (uint256 n = 0; n <= nftOrder.length; n++) {
        ...
        if (n < nftOrder.length) {
          ...
          nft = RainiNft(0,0,0,0,0,false,false);
