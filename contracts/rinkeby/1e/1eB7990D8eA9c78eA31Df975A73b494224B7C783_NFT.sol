...
contract NFT is ERC721Enumerable, Ownable {
  ...
  uint256 public tier1WhitelistPrice = 0.17 ether;
  uint256 public tier2WhitelistPrice = 0.17 ether;

  // Supply 
  uint public tier1WhitelistSupply = 188;
  uint public tier2WhitelistSupply = 187;
  uint public tier1Supply = 750 - tier1WhitelistSupply;
  uint public tier2Supply = 750 - tier2WhitelistSupply;
  ...
  uint public tier1WhitelistMintedSupply = 0;
  uint public tier2WhitelistMintedSupply = 0;
  ...
  function mintOG(address _to, uint _mintCount) public payable {
    ...
    if(tier1WhitelistMintedSupply > tier2WhitelistMintedSupply) {
      // Tier 2 Whitelist Mint
      require(tier2WhitelistMintedSupply + _mintCount <= tier2WhitelistSupply, "Tier 2 Whitelist Supply limited.");
      if (msg.sender != owner()) {
        require(msg.value >= tier2WhitelistPrice * _mintCount, 'Not enough fees to mint.');
      }
      // Mint NFT
      for (uint256 i = 1; i <= _mintCount; i++) {
          tier2WhitelistMintedSupply ++;
          _safeMint(_to, 750 + tier2Supply + tier2WhitelistMintedSupply);
      }
    }
    else {
      // Tier 1 Whitelist Mint
      require(tier1WhitelistMintedSupply + _mintCount <= tier1WhitelistSupply, "Tier 1 Whitelist Supply limited.");
      if (msg.sender != owner()) {
        require(msg.value >= tier1WhitelistPrice * _mintCount, 'Not enough fees to mint.');
      }
      // Mint NFT
      for (uint256 i = 1; i <= _mintCount; i++) {
          tier1WhitelistMintedSupply ++;
          _safeMint(_to, tier1Supply + tier1WhitelistMintedSupply);
      }
    }
  }
