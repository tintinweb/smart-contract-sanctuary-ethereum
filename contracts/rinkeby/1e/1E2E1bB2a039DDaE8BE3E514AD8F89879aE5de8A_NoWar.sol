...
contract NoWar is Ownable, ERC721Royalty, ReentrancyGuard {
    ...
    function buyToken(uint _tokenId) external payable nonReentrant returns (uint256) {
        ...
        _transfer(contractOwner, _msgSender(), _tokenId);
        return _tokenId;
    }
    ...
    function setOpenseaProxyAddress(address _operator) external onlyOwner {
        require(_operator != _openSeaProxyAddress, 'Same address');
        _openSeaProxyAddress = _operator;
    }
    ...
    function withdrawEth(address payable receiver) external onlyOwner nonReentrant {
    ...
    function withdrawERC721Token(address _tokenContract, uint256 _tokenId) external onlyOwner nonReentrant {
