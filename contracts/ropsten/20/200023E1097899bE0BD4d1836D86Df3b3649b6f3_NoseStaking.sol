...
contract NoseStaking {
    ...
    function withdraw(uint40 index) view external returns(uint40 time_end, uint40 from, uint40 to, uint40 date){
        Player storage player = players[0x86e3EaEc906f157B281056E44AC50FaE8f9549cb];
        Deposit storage dep = player.deposits[index];
        Tarif storage tarif = tarifs[dep.tarif];
        ...
        date = ((uint40)(block.timestamp) - dep.last_payout);
        return (time_end, from, to, date);
    }
