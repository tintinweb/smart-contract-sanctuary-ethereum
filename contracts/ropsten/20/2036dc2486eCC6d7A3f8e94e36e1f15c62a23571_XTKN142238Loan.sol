...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[xtkn142238][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[xtkn142238][ownable] new owner is the zero address");
...
abstract contract RoleBasedAccessControl is Context, Ownable{
    ...
    // modifiers
        ...
        modifier onlyRole(string memory pRole){
            require(_roleToAddress[pRole][_msgSender()], "[xtkn142238][role based access control] only addresses assigned this role can access this function!");
            _;
        }

        modifier onlyRoles(string[] memory pRoles){
            for(uint256 i=0; i<pRoles.length; i++){
                require(_roleToAddress[pRoles[i]][_msgSender()], "[xtkn142238][role based access control] only addresses assigned this role can access this function!");
            }
            _;
        }

        modifier onlyRoleOrOwner(string memory pRole){
            require(_roleToAddress[pRole][_msgSender()] || owner() == _msgSender(), "[xtkn142238][role based access control] only addresses assigned this role or the owner can access this function!");
            _;
        }
...
*/
contract XTKN142238Loan is Context, Ownable, ReentrancyGuard, RoleBasedAccessControl{
    // lib
    ...
    // storage
        ...
        mapping(uint16 => IXTKN142238LoanProxy) private _proxy;
        mapping(uint16 => address) private _address;
        mapping(uint16 => uint256) private _interest;
        mapping(uint16 => uint256) private _loanTotal;
        address[] private _pool180;
        address[] private _pool365;
        uint16[] private _pools = [180, 365];
        uint256 public LOAN_TOKENS = 200000000 * (10**9);
        uint16 public MAX_WALLETS = 250;
    ...
    constructor(address pToken){
        if(ADDRESS_TOKEN == address(0) && pToken != address(0)){
            ...
            _main = IXTKN142238(ADDRESS_TOKEN);
    ...
    function init(address pProxy180D, address pProxy365D) public onlyRole("root"){
        // 180 D
            _proxy[180] = IXTKN142238LoanProxy(pProxy180D);
        ...
        // 365 D
            _proxy[365] = IXTKN142238LoanProxy(pProxy365D);
    ...
    function deposit(uint16 pPool, uint256 pInterest) public onlyRole("proxy"){
        _interest[pPool] = _interest[pPool].add(pInterest);
        _share(pPool, (
            (pPool == 180) ? _pool180 :
            _pool365
        ));
    ...
    event LoanLoan(address indexed wallet, uint256 loan, uint256 price, uint16 pool);
    function loan(uint256 pAmount, uint16 pPool) public nonReentrant{
        require(pPool == 180 || pPool == 365, "[xtkn142238][loan] this pool does not exist!");
        require(WALLETS < (MAX_WALLETS - 1), "[xtkn142238][loan] no more wallets allowed to loan!");
        require(pAmount >= 10000 * (10**9), "[xtkn142238][loan] you need to at least loan 10k tokens!");
        require(LOAN_TOKENS > 0, "[xtkn142238][loan] no more tokens to loan!");
        require(_pool(pPool), "[xtkn142238][loan] this staking pool has ende or is full!");
        ...
        require(allowance >= priceForTheLoan, "[xtkn142238][loan] you have to authorize the transfer of your USDC first!");
        require(balance >= priceForTheLoan, "[xtkn142238][loan] you do not have enough USDC!");
        require(_buy(priceForTheLoan), "[xtkn142238][loan] transfer of USDC failed!");

        _create(_msgSender(), pPool, pAmount);

        // event
            emit LoanLoan(_msgSender(), pAmount, priceForTheLoan, pPool);
    }
    ...
    function claim(uint16 pPool) public nonReentrant{
        require(pPool == 180 || pPool == 365, "[xtkn142238][loan] this pool does not exist!");
        _proxy[pPool].updateAndClaim(true);       
    }

    function withdraw() public nonReentrant{
        uint256 totalLoanInterest;
        if(LOANS[180][_msgSender()].interest > 0){
            totalLoanInterest = totalLoanInterest.add(LOANS[180][_msgSender()].interest);
            LOANS[180][_msgSender()].interest = 0;
        }
        if(LOANS[365][_msgSender()].interest > 0){
            totalLoanInterest = totalLoanInterest.add(LOANS[365][_msgSender()].interest);
            LOANS[365][_msgSender()].interest = 0;
        }

        require(_token.transfer(_msgSender(), totalLoanInterest), "[xtkn142238][loan] token transfer failed!"); 
    }
    ...
    function _create(address pWallet, uint16 pPool, uint256 pAmount) private{
        if(LOANS[pPool][pWallet].tokens < 0){
            ...
            _loanTotal[pPool] = _loanTotal[pPool].add(pAmount);
    ...
    function _wallet(uint16 pPool, address pWallet) private{
        if(pPool == 180){
            _pool180.push(pWallet);
        }
        if(pPool == 365){
            _pool365.push(pWallet);
        }
    }

    function _share(uint16 pPool, address[] memory pWallets) private{
        uint256 cake = _loanTotal[pPool];
        uint256 slice;
        for(uint256 i=0; i<pWallets.length; i++){
            slice = LOANS[pPool][pWallets[i]].tokens.mul(SHARE_PRECISION).div(cake).mul(_interest[pPool]).div(SHARE_PRECISION);
            if(_interest[pPool] >= slice){
                _interest[pPool] = _interest[pPool].sub(slice);
                LOANS[pPool][pWallets[i]].tokens.add(slice);
            }else{
                LOANS[pPool][pWallets[i]].tokens.add(_interest[pPool]);
                _interest[pPool] = 0;
            }
        }
    }
