...
*/
contract Feng is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 5 days genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 42_000 ether;
    // Initial distribution for the day 0 FENG-WFTM LP -> FENG pool
    uint256 public constant INITIAL_FENG_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the Feng ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("FENG", "FENG") {
        // Mints 1 FENG to contract creator for initial pool setup
    ...
    function _getFengPrice() internal view returns (uint256 _fengPrice) {
        try IOracle(fengOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Feng: failed to fetch FENG price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _fengPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_fengPrice >= taxTiersTwaps[tierId]) {
    ...
    function setFengOracle(address _fengOracle) public onlyOperatorOrTaxOffice {
        require(_fengOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints FENG to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of FENG to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentFengPrice = _getFengPrice();
            currentTaxRate = _updateTaxRate(currentFengPrice);
            if (currentFengPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_fengPool != address(0), "!_fengPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_fengPool, INITIAL_FENG_POOL_DISTRIBUTION);
