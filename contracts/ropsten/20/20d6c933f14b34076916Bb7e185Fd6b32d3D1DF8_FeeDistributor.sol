...
import "../dex/Neuron.sol";
import "../interfaces/IUniswapV2Router02.sol";
import "../interfaces/IgNR.sol";
...
contract FeeDistributor is Ownable {
    ...
    /// @notice Address of NR contract.
    address public nr;
    /// @notice Address of gNR contract.
    address public gnr;
    /// @notice Dev address.
    address public dev;
    ...
    function tidAddress(address feeTokenAddress) public view returns (uint256 tid) {
        tid = _tIdOf[feeTokenAddress];
    }
    ...
    {
        pending = int256(
            // IERC20(gnr).balanceOf(user).mul(poolInfo[tId].accFeePerShare) /
            (poolInfo[tId].accFeePerShare).mul(100) /
                ACC_FEE_PRECISION
        ...
        for (uint256 i = 0; i < len; ++i) {
            ...
            if(amount > 0) {
                if (feeLp[address(feeLps[i])] == true) {
                    ...
                    if(token0 == WKLAY) {
                        IWKLAY(WKLAY).deposit{value: amountA}();
                    } else if (token1 == WKLAY) {
                        IWKLAY(WKLAY).deposit{value: amountB}();
                    }
    ...
    function updateFeeToken(address feeToken_, uint256 amount) public {
        ...
        // uint256 totalSupply = IERC20(gnr).totalSupply();
    ...
    /// @notice Deposit gNR tokens for rewards allocation.
    /// @param tId The index of the pool. See `poolInfo`.
    /// @param amount gNR token amount to deposit.
    ...
    /// @notice Withdraw gNR tokens.
    /// @param tId The index of the pool. See `poolInfo`.
    /// @param amount gNR token amount to withdraw.
    /// @param to Receiver of the gNR tokens.
    function _withdraw(
        uint256 tId,
        uint256 amount,
        address to
    ...
    function withdraw(address user, uint256 amount) public {
        // require(msg.sender == gnr || msg.sender ==  owner());
        uint256 len = poolInfo.length;
        for (uint256 i = 0; i < len; ++i) {
    ...
    function harvest(
        ...
        int256 accumulatedFee = int256(
            IERC20(gnr).balanceOf(msg.sender).mul(pool.accFeePerShare) /
                ACC_FEE_PRECISION
        );
...
contract Neuron is ERC20("Neuron", "NR"), Ownable {
    ...
    event MinterChanged(address minter, address minter_);
     ...
     */
    function setMinter(address minter_) onlyOwner() external {
    ...
    function mint(address dst, uint rawAmount) onlyMinter() external {
        _mint(dst, rawAmount);
        _moveDelegates(address(0), _delegates[dst], rawAmount);
    }
...
interface IUniswapV2Router01 {
    ...
    function swapExactKLAYForTokens(uint amountOutMin, address[] calldata path, address payable to, uint deadline)
        external
        payable
        returns (uint[] memory amounts);
    function swapTokensForExactKLAY(uint amountOut, uint amountInMax, address[] calldata path, address payable to, uint deadline)
        external
        returns (uint[] memory amounts);
    function swapExactTokensForKLAY(uint amountIn, uint amountOutMin, address[] calldata path, address payable to, uint deadline)
        external
        returns (uint[] memory amounts);
    function swapKLAYForExactTokens(uint amountOut, address[] calldata path, address payable to, uint deadline)
        external
        payable
        returns (uint[] memory amounts);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, bool fee) external pure returns (uint amountIn);
