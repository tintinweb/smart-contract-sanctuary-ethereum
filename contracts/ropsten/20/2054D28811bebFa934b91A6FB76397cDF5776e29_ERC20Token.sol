...
contract ERC20Token {
    ...
    //Events
    event TokensApproved(address  tokenOwner, address  sender, uint tokens);
    event Transferred(address  from, address  to, uint tokens);
    event TokenMinted(address to, uint amount);
    event TokenBurnt(address user, uint amount);
   ...
   constructor(uint256 _countSupply, string memory _name, string memory _symbol) {  
	    countSupply = _countSupply;
        name = _name;
        symbol = _symbol;
        Owner = msg.sender;

        balance[msg.sender] = _countSupply;
        emit Transferred(address(0), msg.sender, _countSupply);
    }
