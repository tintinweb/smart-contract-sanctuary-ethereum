...
abstract contract HasSale is Marketplace, HasAuthorization {
    using Address for address payable;

    struct Sale {
        address collection;
        uint tokenId;   // 0 means ALL tokens
        uint amount;    // ceiling amount for sale. 0 means ALL owned by seller
        address seller;
        uint price;     // per unit
    }
...
abstract contract Marketplace is ERC1155Holder, HasCosts {
    ...
    event Sold(uint id, address collection, uint tokenId, uint amount, address buyer, uint price);
