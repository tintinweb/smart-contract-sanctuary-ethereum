...
contract TestCham is IERC721, Ownable, Functional {
    ...
    address public callerAdd;
    
    // Specific Functionality
    bool public mintActive;
    bool public whitelistActive;
    bool public discountMintActive;
    ...
    uint256 public whiteStartTime;
    uint256 public timehourdiiff;

    address[] public mintAddress;
    ERC721[] private _contractList;
     ...
     */
    constructor() {
        _name = "Test Reflect Cham";
        _symbol = "TRCNFT";
        _baseURI = "https://reflectionnft.io/metadata/";
        _hideTokens = true;
        
        totalTokens = 3333; // 0-3333
        price = 10 * (10 ** 16); // Replace leading value with price in finney
        whitelistPrice = 40 * (10 ** 15);
        maxPerWallet = 2;
        maxPerWalletWhiteList = 1;
        whitelistMax = 3100;
        reservedTokens = 233; // reserved for giveaways and such
        maxPerTxn = 1;
        callerAdd = msg.sender;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == TestCham.onERC721Received.selector;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        address spotone        = payable(0xA8c604F8475236dfD43CD96245a53F4dF7391Fb0);
        address spottwo        = payable(0x104EF0c5B8811980D72A22520a774C934ef4665B);
        address spotthree      = payable(0x585e43dB33732e98438D0D28fE66A07355D79935);
        
        bool success;
        (success, ) = spotone.call{value: ((sendAmount * 15)/100)}("");
        require(success, "Transaction Unsuccessful");
        
        (success, ) = spottwo.call{value: ((sendAmount * 30)/100)}("");
        require(success, "Transaction Unsuccessful");
        
        (success, ) = spotthree.call{value: ((sendAmount * 55)/100)}("");
        require(success, "Transaction Unsuccessful");
    ...
    function ownerMint(address _to, uint256 qty) external onlyOwner {
        ...
        mintAddress.push(msg.sender);
    }

    
    function checkForDiscount(address toValidate) public view returns(bool) {
        for (uint256 i; i < _contractList.length; i++){
            if (_contractList[i].balanceOf(toValidate) > 0){
                return true;
            }
        }
        return false;
    }
    function mintWhitelist(uint256 qty) external payable reentryLock {
        ...
        require((_tokensMintedby[_msgSender()] + qty) <= maxPerWalletWhiteList, "Whilelist Mint: Max tokens per wallet exceeded");
        require(qty <= maxPerTxn, "max 1 per txn");
        require(whitelistActive, "Mint: Whitelist Mint not Open");
    ...
    function mintPublic(uint256 qty) external payable reentryLock {
        require(totalTokens >= qty + numberMinted, "sold out");
        require((qty + reservedTokens + numberMinted) < totalTokens, "Mint: Not enough avaialability");
        require((_tokensMintedby[_msgSender()] + qty) <= maxPerWallet, "Mint: Max tokens per wallet exceeded");
        require(qty <= maxPerTxn, "max 1 per txn");
        require(msg.value >= qty * price, "Mint: Insufficient Funds");
        require(mintActive, "Mint: Public Mint not Open");
    ...
    */
    ...
    function addDiscountContract(address newContract) external onlyOwner {
        _contractList.push(ERC721(newContract));
    }
    ...
    function setWhitelistMax(uint256 maxWhitelist) external onlyOwner {
        whitelistMax = maxWhitelist;
    }
    ...
    function activateWhitelist() public onlyOwner {
        ...
        mintActive = false;
        whiteStartTime = block.timestamp;
    }

    function activateDiscountMint() public onlyOwner {
        discountMintActive = true;
    }
    
    function updateReservedToken() public  onlyOwner returns(uint256)  {
        require(whiteStartTime - block.timestamp == 3600, "You cannot update before 12 hours");
        timehourdiiff =  (whiteStartTime - block.timestamp);
        reservedTokens = 333;
    }
    ...
    function whiteListMany(address[] memory accounts) external onlyOwner {
        require(accounts.length <=50,"Cannot add more than 50 accounts");
