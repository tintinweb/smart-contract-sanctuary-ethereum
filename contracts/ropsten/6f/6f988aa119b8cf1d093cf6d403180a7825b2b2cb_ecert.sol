...
contract ecert
{
    ...
    function requestDiploma(uint256 _hash) external view returns (Status, string memory, string memory, uint, uint, uint, uint)
    {
        ...
        return (status, statusString[uint24(status)], string(abi.encodePacked('Diploma found with status ', statusString[uint24(status)])), diplomas[diplomaNr-1].hash1, diplomas[diplomaNr-1].hash2, diplomaNr, diplomas[diplomaNr-1].numberEntries);
    }
