...
*/
contract EthshToken is IEthshToken, Owned, ERC20Token, TokenHolder {
    // triggered when the total supply is increased
    event Issuance(uint256 _amount);
    // triggered when the total supply is decreased
    event Destruction(uint256 _amount);
    // triggered when deposit is called
    ...
    */
    function EthshToken()
        ERC20Token('Ethsh Token', 'ETHSH', 18) {
    }

    /**
        @dev deposit ether in the account
    */
    function deposit(address _from) ownerOnly payable {
        balanceOf[_from] = safeAdd(balanceOf[_from], msg.value); // add the value to the account balance
        totalSupply = safeAdd(totalSupply, msg.value); // increase the total supply
        Deposit(_from, msg.value);
    }
    ...
    */
    ...
    function withdrawFrom(address _from, address _to, uint256 _amount)
        ...
        notThis(_from)
    {
        balanceOf[_from] = safeSub(balanceOf[_from], _amount); // deduct the amount from the account balance
        totalSupply = safeSub(totalSupply, _amount); // decrease the total supply
        _to.transfer(_amount); // send the amount to the target account
