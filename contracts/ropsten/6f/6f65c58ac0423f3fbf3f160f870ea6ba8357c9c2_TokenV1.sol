...
contract TokenV1 is AbstractTokenV1, Ownable, Pausable, Blacklistable {
    ...
    mapping(address => uint256) internal minterAllowance; //Allowance set by approval
    ...
    function initialize(
        ...
        require(!initialized, "FiatToken: contract already initialized");
        require(
            newMasterMinter != address(0),
            "FiatToken: new masterMinter is zero address"
        );
        require(
            newPauser != address(0),
            "FiatToken: new pauser is zero address"
        );
        require(
            newBlacklister != address(0),
            "FiatToken: new blacklister is zero address"
        );
    ...
    function mint(address _to, uint256 _amount)
        ...
        returns (bool)
    {
        require(_to != address(0), "TokenV1: mint to the zero address");
        require(_amount > 0, "TokenV1: mint amount not greater than 0");
        require(isMinter[_to], "TokenV1: minter not configured");
        require(
            _amount <= maxAmountPerMint[_to],
            "TokenV1: mint amount exceeds minterAllowance"
        );
        require((minterAllowance[_to] + _amount) <= minterCumulative[_to], "TokenV1: minter cap exceeded");
...
contract MultiSigAdmin is Ownable{
    ...
    address internal multiSigAdmin;
    ...
    modifier onlyMultiSigAdmin() {
        require(
            ...
            "PermissionAdmin: caller not the permission admin"
        );
        _;
    }
...
contract Blacklistable is Ownable, PermissionAdmin {
    ...
    function updateBlacklister(address _newBlacklister) external onlyPermissionAdmin {
        require(
