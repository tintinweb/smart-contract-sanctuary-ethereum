...
contract TestFeature is IFeature, ITestFeature, FixinCommon {
    ...
    function map(uint256 key) external view returns (MapContents memory) {
        return LibTestStorage.getStorage().testMap[key];
    }

    function setMap(uint256 key, MapContents calldata value) external {
        LibTestStorage.getStorage().testMap[key] = value;
    }
}
...
import "../features/interfaces/ITestFeature.sol";

/// @dev Storage helpers for the `Test` feature.
library LibTestStorage {
    /// @dev Storage bucket for this feature.
    struct Storage {
        // The owner of this contract.
        string name;
        address lastAddress;
        mapping(uint256 => ITestFeature.MapContents) testMap;
    }

    /// @dev Get the storage bucket for this contract.
    function getStorage() internal pure returns (Storage storage stor) {
        uint256 storageSlot = LibStorage.getStorageSlot(LibStorage.StorageId.Test);
...
interface ITestFeature {
    ...
    /// @dev Set your name in this contract.
    /// @param name_ Xour name.
    function setHello(string calldata name_) external;

    /// @dev Get hello from this contract.
    /// @return hey Greeting.
    function hello() external view returns (string memory hey);

    /// @dev See who changed your name for the last time.
    /// @return setter Time in unix seconds.
    function lastAddress() external view returns (address setter);
