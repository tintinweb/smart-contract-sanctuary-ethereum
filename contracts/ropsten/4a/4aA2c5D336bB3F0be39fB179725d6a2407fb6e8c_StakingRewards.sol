...
contract StakingRewards is Ownable, ReentrancyGuard, Pausable {
    ...
    function deposit(uint256 _pid, uint256 _amount, uint256 _days) external payable nonReentrant whenNotPaused {
        ...
        require(_days == 0 || _days == 15 || _days == 30 || _days == 90, "Lock-in period does not meet requirements");
        uint256 _lockEndTime;
        if (_days == 0) {
            _lockEndTime = 0;
        } else {
            _lockEndTime = block.timestamp + _days * 60;
        }
        if (user.amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant whenNotPaused {
        ...
        if (user.period > 0){
            user.lockEndTime = block.timestamp + user.period * 60;
        }
    }
    ...
    function getRewardPerSeconds(IERC20 _token, uint256 _amount, uint256 _price, uint256 _period) public view returns(uint256) {
        ...
        uint256 amount = _amount * _price / decimals; // MMT decimal == _price decimal
        uint256 reward;
        if (_period == 0) {
            reward = (amount * 100) / (MAG * 86400); // persec reward 1%
        }
        if (_period == 15) {
            reward = (amount * 2000) / (MAG * 86400 * 15); // persec reward 20%
        }
        if (_period == 30) {
            reward = (amount * 4500) / (MAG * 86400 * 30); // persec reward 45%
        }
        if (_period == 90) {
            reward = (amount * 15000) / (MAG * 86400 * 90); // persec reward 150%
        }
        return reward;
    }
      ...
      /**
        * @dev User get TotalRewards
        * @param _pid The index of the pool.
        * @param _amount The admount of the user.
     */
    function getRewardTotal(uint256 _pid, uint256 _amount,uint256 _period) public view returns(uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        uint256 multiplier = _period * 86400;
        uint256 rewardPerSec = getRewardPerSeconds(pool.token, _amount, pool.tokenPrice, _period);
    ...
    function _getRewards(uint256 _pid, address _user) internal {
        ...
        if (block.timestamp > user.lockEndTime && user.lockEndTime > 0) {
