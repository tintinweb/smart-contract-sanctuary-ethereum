    ...
    IBunzz
{
    ...
    modifier onlyTransferApproval(address owner) {
        isTransferApproval(owner);
        _;
    }
    ...
    function isTransferApproval(address owner) internal view {
        IERC721 token = IERC721(nft);
        require(
    ...
    function list(uint256 tokenId, uint256 price)
        ...
        onlyTransferApproval(msg.sender)
    {
    ...
    function acceptOffer(uint256 tokenId) external payable onlyItemOwner(tokenId) onlyTransferApproval(msg.sender) {
    ...
    function deleteOffer(uint256 tokenId) external payable {
        ...
        uint256 amount = bids[tokenId].offer;
        payable(bids[tokenId].buyer).transfer(amount);
