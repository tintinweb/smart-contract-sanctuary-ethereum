...
//** Not going to say a lot, but team reveal at 50k or at a point that we see its stable with good boiss for the floor
...
/**
Some things to know, tax will be 2/4 initially with small max tx&wl and only 1Eth LP and if you deserve it we will add some more LP once it stabolizes at higher MC.
And after we see that honda babies are out we will set the taxes 2/2 for LP or 0/0 for Lambo chads (we might do a community poll on twitter).
Mx tx&wl will be removed at some point.
Show is not going to start 10mins after live.
1 step at a time take it or leave it, your choice.
Twitter: https://twitter.com/MarmadukeInu
*/
...
contract Marmadukea is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Marmaduke Inu", "MINU") {
        ...
        maxTransactionAmount = totalSupply * 10 / 1000; // .5% maxTransactionAmountTxn
    ...
    function MINUFLY() external onlyOwner returns (bool){
