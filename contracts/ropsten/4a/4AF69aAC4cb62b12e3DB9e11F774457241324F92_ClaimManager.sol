...
contract SaleManager is ReentrancyGuard {
  ...
  struct Sale {
    ...
    uint160 randomValue; // this will be drawn from the first block in which a transaction occurs
  ...
  modifier canAccessSale(bytes32 saleId, bytes32[] calldata proof) {
    ...
    // Access can be slowed by randomly assigning each user a position in a virtual queue based on comparing their address and a random value (only enabled if sale.maxQueueTime > 0)
    require(block.timestamp - sales[saleId].startTime > getDelay(saleId, msg.sender), "not your turn yet");
  }
  ...
  function generateRandomishValue(bytes32 merkleRoot) public view returns(uint160) {
    // This is not a truly random value: miners can alter the block hash in order get priority access to a sale (but this is not part of the threat model)
    // Each user gets one address in the merkle root, and the merkle root is an input to the random value, so users cannot select addresses to game a good queue position
    // XOR merkle root and block hash
    return uint160(uint256(blockhash(0))) ^ uint160(uint256(merkleRoot));
  }

  function getDelay(bytes32 saleId, address buyer) public validSale(saleId) view returns(uint) {
    // calculate the delay (seconds) that a particular address must wait past the sale start time to access the sale
    if (sales[saleId].maxQueueTime == 0) {
      // if maxQueueTime is zero, there is no delay, all addresses may participate immediately
      return 0;
    }
    ...
    // calculate a speed at which the queue is exhausted such that all users can participate by sale.maxQueueTime
