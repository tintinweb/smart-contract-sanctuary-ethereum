...
contract NFTBiarritzTicket is ERC721, Ownable {
    ...
    string public constant DATA_URI =
    ...
    string public constant ARWEAVE_HASH =
        "AxnqV14aVIzmTHj2IqXMZlS9dlSuSGS8Vb9-VSYpFtw";
    string public constant SHA256_HASH_PROVENANCE =
        "42a17a7874b0629bbc035e5f802d7516286e45a6e1cc89087e0f55990a77a718";
    ...
    constructor() ERC721("NFT Biarritz 2022 Ticket", "NFTB22") {
    ...
    function contractURI() public view returns (string memory) {
        return
            "https://arweave.net/y1dPNJQUXZSo7ZRBdgVgD1xk62S0rXu1IR5oCmC91d8";
    }
    ...
    function appendCoupon(address nft, uint256 discount) public {
        contractList.push(nft);
    ...
    function getDiscount(address buyer) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < contractList.length; i++) {
            IERC721 NftContract = IERC721(contractList[i]);
            if (NftContract.balanceOf(buyer) >= 1) {
                if (discount < couponStructs[contractList[i]].discount) {
                    discount = couponStructs[contractList[i]].discount;
                }
            }
        }
        return discount;
    ...
    function mint(string memory name) public payable {
        require(quantityMinted() < MAX_SUPPLY + 1, "Exceeds maximum supply");
        require(
            (msg.value >= getFinalPrice(msg.sender)),
            "Ether value sent is not correct"
        );

        // tokenToMetadata[tokenIdCounter.current()] = Metadata(name);

        _safeMint(msg.sender, tokenIdCounter.current());
        emit Mint(msg.sender, tokenIdCounter.current());
        emit PermanentURI(DATA_URI, tokenIdCounter.current());
