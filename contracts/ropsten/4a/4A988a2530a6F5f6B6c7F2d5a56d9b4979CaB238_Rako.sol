/**
 *Submitted for verification at EtherScan.com on 2022-09-05
 */
...
Ticker: $REIKI
    ...
    - 1,000,000,000 $REIKI Supply with progressive 0.5% burn on every transaction
 ...
 */
// To do:
// - Change the router addresses from PancakeSwap to UniSwap contracts
// - Change wallet addresses for the Devs & DevMarketing
...
contract Rako is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public sellDelayActive = true;

    event SwapAndLiquifyEnabled(bool enabled);

    bool public _feesLaunched = false;
    ...
    function enableFeesOnLaunch() external onlyOwner {
        require(!_feesLaunched);
        feeUnits = 10000;
    ...
    // function swapAndInjectEthToLiquidity (uint256 tokenAmount) private {
    //     uint256 initialBalance = address(this).balance;
    //     swapTokensForEth(tokenAmount);
    //     uint256 newBalance = address(this).balance.sub(initialBalance);
    //     // inject it directly to the liquidity pool
    //     super._transfer(address(this), uniswapV2Pair, newBalance);
    // }
