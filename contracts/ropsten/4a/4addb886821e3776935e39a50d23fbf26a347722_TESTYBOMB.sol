...
contract MyDataConsumerForOnad is ConsumerBase, Ownable {
    ...
    // RinkeBy 
    // address private ROUTER_ADDRESS = address(0xC1eeDAAae25E52E62F2dA1a18468AFC5dF94e0CB);

    // RinkeBy
    // address private MockToken_ADDRESS = address(0xC1eeDAAae25E52E62F2dA1a18468AFC5dF94e0CB);
...
contract TESTYBOMB is ERC721, Ownable {
    using SafeMath for uint256;
    IERC20Burnable public immutable ONAD;
    MyDataConsumerForOnad public immutable OnadPrice;
    ...
    uint256 public constant MAX_NFT_SUPPLY = 1125;
    uint256 public CAPPED_NFT_LIMIT_PER_USER = 10;
    uint256 public ONAD_PRICE;
    uint256 public startingIndexBlock;
    uint256 public startingIndex;
    
    address public moderator;

    uint256 private tier1Onad = 10;
    uint256 private tier2Onad = 20;
    uint256 private tier3Onad = 30;
    ...
    bool public pausedOnadOracle = false;
    ...
    constructor(string memory name, string memory symbol,  address _onadToken, address  _onadpriceToken, address _moderator,uint256 _ONAD_PRICE) ERC721(name, symbol) {
        ONAD = IERC20Burnable(_onadToken);
        OnadPrice = MyDataConsumerForOnad(_onadpriceToken);
        moderator = _moderator;
        ONAD_PRICE = _ONAD_PRICE;
    }
    ...
    function flipOnadOracle() public onlyOwner {
        pausedOnadOracle = !pausedOnadOracle;
    }

    /**
     * @dev Gets current NFT Price without oracle
     */
    function getNFTPriceForONAD(uint256 numberOfNfts) public view returns (uint256) {
        require(block.timestamp >= SALE_START_TIMESTAMP, "Sale has not started");
        require(totalSupply() < MAX_NFT_SUPPLY, "Sale has already ended");
        
        uint currentSupply = totalSupply();

        (uint256 _la, uint256 _lb, uint256 _lc) = getONADPerETHRate();

        if (currentSupply >= 1125) {
            return (getOnadEthPrice().mul(_lc).div(100)).mul(numberOfNfts); // 1125 # of LEASH for 0.3 ETH
        } else if (currentSupply >= 600) {
            return (getOnadEthPrice().mul(_lb).div(100)).mul(numberOfNfts); // 600 - 1124 # of LEASH for 0.2 ETH
        } else {
            return (getOnadEthPrice().mul(_la).div(100)).mul(numberOfNfts); // 0 - 599 # of LEASH for 0.1 ETH
        }
    }
    ...
    function getNFTPriceForETH(uint256 numberOfNfts) public view returns (uint256) {
        ...
        if (currentSupply >= 1125) {
            return numberOfNfts.mul(_ec); // 1125 0.3 ETH
        } else if (currentSupply >= 600) {
            return numberOfNfts.mul(_eb); // 600 - 1124 0.2 ETH
        } else {
            return numberOfNfts.mul(_ea); // 0 - 599 0.1 ETH
        }
    }

    function setOnadEthPrice(uint256 _ONAD_PRICE) public onlyOwner {
        ONAD_PRICE = _ONAD_PRICE;
    }
    ...
    function setONADPerETHRate(uint256 _tier1Onad, uint256 _tier2Onad, uint256 _tier3Onad) public onlyOwner returns(uint256, uint256, uint256){
        tier1Onad = _tier1Onad;
        tier2Onad = _tier2Onad;
        tier3Onad = _tier3Onad;
        return (tier1Onad, tier2Onad, tier3Onad);
    }
    ...
    function getONADPerETHRate() public view returns(uint256, uint256, uint256){
        return (tier1Onad, tier2Onad, tier3Onad);    
    }

    function getOnadEthPrice() internal view returns(uint256){
        if(!pausedOnadOracle){
            return OnadPrice.price();
        }
        else{
            return ONAD_PRICE;
        }
    ...
    function withdrawONAD() public onlyOwner {
        uint balance = ONAD.balanceOf(address(this));
        require(balance > 0, "ONAD balance is 0");
        ONAD.transfer(msg.sender, balance);
    }
    ...
    */
    function mintNFT(uint numberOfNfts, uint256 _value, bool _isETH) public payable {
        ...
        {
            require(getNFTPriceForONAD(numberOfNfts) == _value, "LEASH value sent is not correct");
            ONAD.transferFrom(msg.sender, address(this), _value);
        }
