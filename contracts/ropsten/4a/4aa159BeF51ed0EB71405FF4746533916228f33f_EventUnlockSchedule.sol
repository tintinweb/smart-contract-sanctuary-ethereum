...
contract EventUnlockSchedule is BaseGovernanceWithUserUpgradable {
    ...
    function initialize(ILock lock, bytes calldata data) public initializer {
        ...
        __EventUnlockSchedule_init(lock, _conditions, _offsetPrices, _amounts, /*_tknAddrBase, _tknAddrQuote,*/ _priceFeed, governanceAddress);
    }

    function __EventUnlockSchedule_init(ILock _lock, uint256[] memory _conditions, int256[] memory _offsetPrices, uint256[] memory _amounts, /*address _tknAddrBase, address _tknAddrQuote,*/ address _priceFeed, address governanceAddress) internal onlyInitializing {
        __BaseGovernanceWithUser_init(governanceAddress);
        __EventUnlockSchedule_init_unchained(_lock, _conditions, _offsetPrices, _amounts, /*_tknAddrBase, _tknAddrQuote,*/ _priceFeed);
    }

    function __EventUnlockSchedule_init_unchained(ILock _lock, uint256[] memory _conditions, int256[] memory _offsetPrices, uint256[] memory _amounts, /*address _tknAddrBase, address _tknAddrQuote,*/ address _priceFeed) internal onlyInitializing {
    ...
    function checkUpKeep(bytes calldata /*checkData*/) external view returns (bool upkeepNeeded, bytes memory performData) {
        ...
        if(offsetPrices.length > 0){
            int price = _getPrice();
            for(uint256 i = 0; i < offsetPrices.length; i++) {
                upkeepNeeded = false;
                if(conditions[i] == 0){
    ...
    function performUpKeep(bytes calldata performData) external {
        ...
        upKeepCountTest = upKeepCountTest+1;
    }
    function _updateAmountsUnlocked(uint256 amount) internal {
        amountsUnlocked.push(amount);
        upKeepCountTest = upKeepCountTest+100;
    }
    function _removeEventSchedule(uint idx) internal {
        delete offsetPrices[idx];
        delete conditions[idx];
        delete amounts[idx];
        upKeepCountTest = upKeepCountTest+500;
    }


    function checkKPTest() public view returns (uint) {
        return offsetPrices.length;
    }
    function checkAmntsTest() public view returns (uint) {
        return amounts.length;
    }
    function checkCndtsTest() public view returns (uint) {
        return conditions.length;
    }
    function whatUpkeepCount() public view returns (uint) {
        return upKeepCountTest;
    }
