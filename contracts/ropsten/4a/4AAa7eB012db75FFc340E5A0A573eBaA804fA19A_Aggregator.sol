...
contract Aggregator is Ownable {
    ...
    function resolveBets(
        ...
        IBetsPool.PlayInBet[] memory bets = _betsPool.allActiveBets(
            _sportId,
            _event
        );
        require(bets.length > 0, "No bets found");
        uint256 i = lastUpdated[_sportId];
        for (; i < bets.length; i++) {
            if (
                bets[i].betStatus == IBetsPool.status.unresolved &&
                keccak256(abi.encodePacked(bets[i].betSlug)) ==
                keccak256(abi.encodePacked(_betSlug))
            ) {
                _betsPool.updateBet(_sportId, i, IBetsPool.status.won, _event);
            }
        }
        lastUpdated[_sportId] = i;
    }
}
