...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    event UpdateEmissionRate(
        address indexed user,
        uint256 indexed _rewardsPerSec
    );
    ...
    function add(
        ...
        uint256 lastRewardTime = currentblockTime > startTime
            ? currentblockTime
            : startTime;
        ...
        returns (uint256)
    {
        ...
        if (currentBlockTime > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 reward = nbSeconds
                .mul(rewardsPerSec)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accRewardPerShare = accRewardPerShare.add(
                reward.mul(PRECISION_FACTOR).div(lpSupply)
            );
        }
        return
            user.amount.mul(accRewardPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        _harvest(_pid, pool, user, msg.sender);
    ...
    function enterStaking(uint256 _amount) public {
        ...
        _updatePool(0);
        _harvest(0, pool, user, msg.sender);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(
                address(msg.sender),
                address(this),
