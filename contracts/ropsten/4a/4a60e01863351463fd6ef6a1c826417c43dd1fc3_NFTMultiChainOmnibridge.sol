...
contract MediatorBalanceStorage is TokensTypeRegistry {
    ...
    function _updateMediatorBalances(
        ...
        for (uint256 i = 0; i < _tokenIDs.length; i++) {
            ...
            if (!_isERC721) {
                _currentAmount = uintStorage[keccak256(abi.encodePacked("mediatorBalance", _token, "#", _tokenIDs[i]))];
            }
...
// File: contracts/upgradeable_contracts/NFTMultiChainOmnibridge.sol

pragma solidity 0.7.5;


/**
 * @title NFTMultiChainOmnibridge
 * @dev implementation for multi-chain multi-token mediator intended to work on top of AMB bridge.
 * It is designed to be used as an implementation contract of EternalStorageProxy contract.
 */
contract NFTMultiChainOmnibridge is BasicOmnibridge, SelectorTokenGasLimitConnector {
    using SafeMath for uint256;
     ...
     * @param _maxPerTxMinPerTxArray array with limit values for the assets to be bridged to the other network.
     *   [ 0 = maxPerTx, 1 = minPerTx ]
     * @param _executionMaxPerTx execution max per tx limit
    ...
    function initialize(
        ...
        uint256[2] calldata _maxPerTxMinPerTxArray, // [ 0 = _maxPerTx, 1 = _minPerTx ]
        ...
        _setLimits(address(0), _maxPerTxMinPerTxArray);
        _setExecutionMaxPerTx(address(0), _executionMaxPerTx);
     ...
     */
    function upgradeToReverseMode(address _gasLimitManager) external {
    ...
    function _handleTokens(
        ...
        require(withinExecutionLimit(_token, _tokenIDs.length));

        bytes32 _messageId = messageId(_chainID);

        _releaseTokens(_token, _recipient, _tokenIDs, _amounts);

        emit TokensBridged(_chainID, _token, _recipient, _tokenIDs, _amounts, _messageId);
    }
    ...
    function bridgeSpecificActionsOnTokenTransfer(
        ...
        require(withinLimit(_token, _tokenIDs.length));

        bytes memory data = _prepareMessage(_token, _receiver, _tokenIDs, _amounts);
        bytes32 _messageId = _passMessage(_chainID, data);

        _recordBridgeOperation(_chainID, _messageId, _token, _from, _tokenIDs, _amounts);
    }
