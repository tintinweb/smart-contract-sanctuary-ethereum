...
contract stakeHorse is Initializable {
    ...
    function claimAble(address _user) public view returns(uint256, uint256){
        ...
        if(stakee.stake_time > updatedTime){
            claimableDays = block.timestamp.sub(stakee.stake_time).div(1 days);
        }else{
    ...
    function redeem() public{
        ...
        require(block.timestamp.sub(stakes[msg.sender].stake_time).div(1 days) > maturityDays,'Rewards not matured');
