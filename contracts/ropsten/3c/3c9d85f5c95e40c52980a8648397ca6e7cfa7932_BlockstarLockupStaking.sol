    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < len; i++) {
            if( block.timestamp > userdata[i].endtime && userdata[i].amount > 0 ){
                total += ((userdata[i].amount * userdata[i].returnPer / 100) / 365) * userdata[i].lockupDuration;
            }
        }
        return total;
    }
    ...
    function withdraw() external {
        ...
        for (uint256 i = 0; i < len; i++) {
            if( block.timestamp > userdata[i].endtime && userdata[i].amount > 0){
                ...
                uint256 reward = ((amount * userdata[i].returnPer / 100) / 365) * userdata[i].lockupDuration;
