...
abstract contract Mintable is Ownable, IMintable {
    ...
    uint public dropcounter = 0;
    mapping(uint256 => bytes) public blueprints;
    mapping(uint8 => uint) private drops;
    uint8 private round = 0;
    ...
    function setrounds(uint8 _round) public  {
        require(msg.sender == imx,"only owner can set");
        round = _round;
        dropcounter = 0;
    }


    function setNewDrop(uint8 _round, uint8 _drop) public  {
        require(msg.sender == imx,"only owner can set");
        drops[_round] = _drop;
    }
