...
import {Default} from "./templates/Default.sol";
import {ERC721Royalty} from "./ERC721Royalty.sol";

contract ContractMinter is Default {

    constructor() Default(10000, 'Contract Minter', 'minter') ERC721Royalty(_msgSenderERC721A(), 1000) {
        setBaseURI('https://niftyvs.com/metadata/contract-minter/json/');
        setConfig(0, 0, 3, 1000, 0.04 ether, 0);
        setConfig(1, 0, 5, 2500, 0.05 ether, 0);
    }
...
abstract contract Default is ERC721, ERC721Burnable, ERC721Metadata, ERC721Pausable, ERC721Royalty, Withdrawable {
    ...
    //  giveaways etc. alongside the main mint sale.
    // - With the ability to modify any setting after deployment.
    struct SaleConfig {
        uint64 endsAt;
        uint32 maxMint;
        uint32 maxSupply;
    ...
    constructor(uint256 maxSupply, string memory name, string memory symbol) ERC721(name, symbol) {
    ...
    function mint(uint256 key, bytes32[] memory proof, uint16 quantity) payable public whenNotPaused {
        ...
        // `SaleConfig` has reached max supply or key doesn't exist
        if (config.maxSupply == 0) {
            revert MintHasEnded();
        }

        // Allowlist gate sale
        if (_allowlists[key] != 0) {
            MintGate.allowed(buyer, proof, _allowlists[key]);
        }

        // Max supply within `SaleConfig` cannot exceed max collection supply
        ...
        // Minted counters
        uint16[4] memory minted = _unpackAux(_getAux(buyer));

        MintGate.open(config.endsAt, config.startsAt);
        MintGate.price(buyer, config.price, quantity, msg.value);
        MintGate.supply(available, config.maxMint, minted[key], quantity);

        unchecked {
            // Update max supply associated with sale key
            _config[key].maxSupply -= quantity;

            // Update counter associated with mint key
            minted[key] += quantity;

            _setAux(buyer, _packAux(minted));
        }
...
library MintGate {
    ...
    function supply(uint256 available, uint256 maxMint, uint256 minted, uint256 quantity) internal pure {
        supply(available, quantity);

        unchecked {
            if (maxMint > 0 && (minted + quantity) > maxMint) {
                revert CannotMintMoreThan({ amount: maxMint });
            }
        }
    }
}
