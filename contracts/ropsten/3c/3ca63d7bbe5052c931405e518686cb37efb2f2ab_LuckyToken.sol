...
contract LuckyToken is MintableToken {
    ...
    function () public payable{
        ...
        emit Buy(price,msg.value,amount);
    ...
    function burn(uint256 amount) public {
        ...
        uint256 refundedSum = this.balance*amount/(totalSupply_-balanceOf(houseContract));
        msg.sender.transfer(refundedSum);
        uint256 price = refundedSum*(1 ether)/amount;
        emit Refund(refundedSum,price,amount);
    }
    ...
    event Buy(uint256 price,uint256 ethAmount,uint256 luckyAmount);
    event Refund(uint256 price,uint256 ethAmount,uint256 luckyAmount);
}