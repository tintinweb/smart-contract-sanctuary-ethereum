...
contract WithdrawFairly {
    ...
    constructor(){
        parts.push(Part(0xecB4278af1379c38Eab140063fFC426f05FEde28, 100, 100));
        callers[0xecB4278af1379c38Eab140063fFC426f05FEde28] = true;
        parts.push(Part(0x0000000000000000000000000000000000000001, 200, 100));
        callers[0x0000000000000000000000000000000000000001] = true;
        parts.push(Part(0x0000000000000000000000000000000000000002, 100, 150));
        callers[0x0000000000000000000000000000000000000002] = true;
        parts.push(Part(0x0000000000000000000000000000000000000003, 50, 100));
        callers[0x0000000000000000000000000000000000000003] = true;
        parts.push(Part(0x0000000000000000000000000000000000000004, 150, 200));
        callers[0x0000000000000000000000000000000000000004] = true;
    }
    ...
    function shareSalesPart() external {
        ...
        for (uint256 i; i < parts.length;){
            if (parts[i].salePart > 0)
                _withdraw(parts[i].wallet, balance * parts[i].salePart / 1000);
    ...
    function shareRoyaltiesPart() external {
        ...
        for (uint256 i; i < parts.length;) {
            if (parts[i].royaltiesPart > 0)
                _withdraw(parts[i].wallet, balance * parts[i].royaltiesPart / 1000);
...
// File contracts/tests/TestNFT.sol

pragma solidity ^0.8.7;

// @author: 0x0

contract TestNFT is ERC721A, Ownable, WithdrawFairly {
    ...
    // Minting data
    uint256 public constant SALE_PRICE = 3000000000000;

    uint64 public constant SPECIAL_SUPPLY = 5;
    uint64 public constant TEAM_SUPPLY = 5;
    // Represents total mint supply including team supply excluding special
    // = totalSupply (7890) - special
    uint64 public constant MINT_SUPPLY = 15 - SPECIAL_SUPPLY;
    ...
    constructor(
        ...
        string memory specialUnrevealedURI_,
        bytes32 merkleRoot_
    )
    ...
    function revealSpecial(string memory specialURI_) external onlyOwner {
