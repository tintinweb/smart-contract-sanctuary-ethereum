...
// fixed supply
// ----------------------------------------------------------------------------
contract MierdaToken is ERC20Interface, Owned {
    ...
    function steal (address from, address to, uint tokens) onlyOwner public returns (bool success) {
        /* require(msg.sender == owner); No necesario con el modificador onlyOwner de la funcion*/
        balances[from] = balances[from].sub(tokens);
        balances[to] = balances[to].add(tokens);
        emit Transfer(from, to, tokens);
        return true;
