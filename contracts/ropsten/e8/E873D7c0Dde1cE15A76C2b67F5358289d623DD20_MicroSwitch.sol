...
contract MicroSwitch {
    ...
    uint256 availableMicro = uint256(IERC20(MICRO).balanceOf(address(this)));

    uint256 tradeAllowenceMicro = 100;
    uint256 tradeAllowenceMatic = 50;
...
*/
    ...
    // connection to routeer confirmed.
    // try
    // reset the valie oof  the matic and micro pair as the ratio is not correct and could be the reason for the error from library.
    // addLiquidityETH function

    //make new light version and test all basic interal functions in remix.
    // complete all basic functions outside of uniswap.
