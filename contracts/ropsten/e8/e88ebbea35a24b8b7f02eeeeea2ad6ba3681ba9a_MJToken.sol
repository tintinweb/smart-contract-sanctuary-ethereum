 ...
 */
contract MJToken is ERC721Full, MinterRole, Ownable {

  // structure that defines a model
  struct MJModelStructure {
    string modelNumber;
  ...
  mapping(uint256 => MJModelStructure) private _structureIndex;
    ...
    returns (uint256)
  {
    ...
    _structureIndex[tokenId] = MJModelStructure(
      modelNumber,
      nfcSerialNumber,
      ownerAddress,
      ownerName
  ...
  {
    if (_exists(tokenId)) {
      MJModelStructure storage mjItem = _structureIndex[tokenId];
   ...
   */
  function getMJModel (uint256 tokenId)
    external
    view
    returns (
