...
contract CoinStaking is Ownable, ReentrancyGuard, IStaking {
    ...
    /// @dev amount [BN] - staked, rewardDebt [BN] - system, earned[BN] - saves earnings amount
    struct UserInfo {
        uint256 amount;
        uint256 rewardDebt;
        uint256 earned;
        ...
        correctPool(_pid)
    {
        ...
        updatePool(_pid);
        user.earned = _payReward(sender, user, pool.accRewardPerShare);
    ...
    function _payReward(
        ...
        if (_userInfo.amount > 0) {
            ...
            if (pending > 0) {
                ...
                return _userInfo.earned + pending;
            }
        }
        return _userInfo.earned;
    }
}
