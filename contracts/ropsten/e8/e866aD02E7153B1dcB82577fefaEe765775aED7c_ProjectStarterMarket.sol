...
// offer created
  ...
  function makeOffer(uint _nftId,address _nftAddress,uint _nftPrice) public payable {
      ...
      userBalance[msg.sender]+=userBalance[msg.sender];
...
contract AuctionHouse is IERC721Receiver {
    ...
    struct SaleOrAucitonItem {
        uint256 index; // SaleOrAucitonItem Index
        address addressNFTCollection; // Address of the ERC721 NFT Collection contract
        uint256 nftId; // NFT Id
        address creator; // Creator of the SaleOrAucitonItem
        uint price;
    ...
    SaleOrAucitonItem[] public allSalesOrAuctions;
    ...
    modifier isAuction(uint _index){
        require(allSalesOrAuctions[_index].isAuction, "Invalid Auction");
        _;
    }
    ...
    function createSellOrAuction(
        ...
        // Create new SaleOrAucitonItem object
        SaleOrAucitonItem memory newMarketItem = SaleOrAucitonItem({
            index: index,
        ...
        //update list
        allSalesOrAuctions.push(newMarketItem);
    ...
    function isOpen(uint256 _index) public view returns(bool) {
        SaleOrAucitonItem storage auction = allSalesOrAuctions[_index];
    ...
    * @param _index Index of the auction
    */
    function getMarketItem(uint _index) public view returns(SaleOrAucitonItem memory) {

        require(_index < allSalesOrAuctions.length, "Invalid auction index");
        return allSalesOrAuctions[_index];
    ...
    returns(address)
    {
        ...
        return allSalesOrAuctions[_index].currentBidOwner;
    }
    ...
    {
        require(_index < allSalesOrAuctions.length, "Invalid auction index");
        return allSalesOrAuctions[_index].currentBidPrice;
    }
    ...
    returns(bool)
    {
        ...
        require(isOpen(_index), "SaleOrAucitonItem is not open");
    ...
    function buy(uint _index) external payable {
        ...
        require(!allSalesOrAuctions[_index].isAuction, "Item not for sale"); 

        SaleOrAucitonItem memory item = allSalesOrAuctions[_index];
    ...
    function claimNFT(uint256 _index) external payable isAuction(_index) {
        ...
        require(!isOpen(_index), "SaleOrAucitonItem is still open");
    ...
    function claimFee(uint256 _index) external {
        require(_index < allSalesOrAuctions.length, "Invalid auction index"); // XXX Optimize
