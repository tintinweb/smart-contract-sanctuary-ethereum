...
contract RPS {
  ...
  function compareHands (uint256 roomNum) private {
    ...
    require(rooms[roomNum].originator.count == 1 && rooms[roomNum].taker.count ==1);
    uint256 originator = uint8(rooms[roomNum].originator.result);
    uint256 taker = uint8(rooms[roomNum].taker.result);
