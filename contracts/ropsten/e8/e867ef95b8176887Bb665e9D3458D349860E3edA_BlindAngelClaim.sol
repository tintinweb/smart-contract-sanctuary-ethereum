...
contract BlindAngelClaim {
    ...
    event BatchClaimed(address indexed dealer);
    ...
    function batchClaim() external onlySigners {
        require(last_creator != msg.sender, "This account changed claim list. Can't claim");
        require(claimList.length > 0, "Claim list is empty");
        for (uint i; i < claimList.length; i ++) {
            if (reward[claimList[i]].balance > 0) {
                token.transfer(claimList[i], reward[claimList[i]].balance);
            }
        }
        delete claimList;
