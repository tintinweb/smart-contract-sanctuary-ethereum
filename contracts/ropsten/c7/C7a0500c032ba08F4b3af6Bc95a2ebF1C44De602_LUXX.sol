...
contract LUXX is IERC20, Auth {
    ...
    //USD(C) MAINNET 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48
    //USD(T) TESTNET 0x3B00Ef435fA4FcFF5C209a37d1f3dcff37c705aD



    IERC20 USDC = IERC20(0x6EE856Ae55B6E1A249f04cd3b947141bc146273c);
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "LUXX";
    string constant _symbol = "LUXX";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 10_000_000 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply.div(200); // 0.5%
    uint256 public _maxWalletToken = _totalSupply.div(100); //1%
    ...
    uint256 reflectionFee = 500; //4%
    uint256 marketingFee = 300;  //2%
    ...
    // /!\ DELETE THIS FOR MANUAL SWAPBACK
    ...
    constructor () Auth(payable(msg.sender)) {
        ...
        //IUniswapV2Router02 same for MAINNET && TESTNET
        ...
        //next 2 lines change msg.sender by the addy you want to sends fees to
        autoLiquidityReceiver = payable(0x52f1083157Fef956D188Dea728b6838f26b62F28);
        marketingFeeReceiver = payable(0xa55B8A60956A6a84217a1DC546ffB47915470860);

        antiBotEnabled = true;

        isLimitExempt[msg.sender] = true;
        isLimitExempt[address(this)] = true;
        isLimitExempt[address(pair)] = true;
        isLimitExempt[address(router)] = true;
        //isLimitExempt[address(reflections_pair)] = true;
        isLimitExempt[address(distributorAddress)] = true;
        isLimitExempt[address(autoLiquidityReceiver)] = true;
        isLimitExempt[address(marketingFeeReceiver)] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && !isLimitExempt[recipient] && recipient != pair) {
            require((balanceOf(recipient) + amount) <= _maxWalletToken,"max wallet limit reached");
        }

        if (recipient == pair){
          checkAmountTx(sender, amount);
        }else{
          checkAmountTx(recipient, amount);
        }

        // /!\ DELETE THIS FOR MANUAL SWAPBACK
        if(shouldSwapBack(address(sender))){ swapBack(swapThreshold); }

        _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");

        uint256 amountReceived = (!shouldTakeFee(sender) || !shouldTakeFee(recipient))  ? takeFee(payable(sender), amount) : amount;

        _balances[recipient] = _balances[recipient].add(amountReceived);

        if(!isLimitExempt[sender]){ try distributor.setShare(payable(sender), _balances[sender]) {} catch {} }
        else if(!isLimitExempt[recipient]){ try distributor.setShare(payable(recipient), _balances[recipient]) {} catch {} }
    ...
    function rescueStuckTokens() public payable onlyOwner {
        uint256 contractTokenBalance = USDC.balanceOf(address(this));
        // rescue stuck tokens
        IERC20(USDC).transfer(payable(owner), contractTokenBalance);
    }

    function rescueStuckNative() public payable onlyOwner {
        // get the amount of Ether stored in this contract
        uint contractETHBalance = address(this).balance;
        // rescue Ether to recipient
        (bool success, ) = payable(owner).call{value: contractETHBalance}("");
    ...
    function isBlacklisted(address sender, address recipient) internal view {
        require(!bots[sender] && !bots[recipient], "TOKEN: Your account is blacklisted!");
    }
    ...
    function takeFee(address payable sender, uint256 amount) internal returns (uint256) {
    ...
    // /!\ MANUAL SWAPBACK FUNCTION
    function sweepContingency(uint256 amnt) external onlyOwner(){
      require(amnt <= balanceOf(address(this)), "Request exceeds contract token balance.");
      swapBack(amnt);
      try distributor.process(distributorGas) {} catch {}

    }

    function swapBack(uint256 amnt) internal swapping {
        uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? liquidityFee.div(2) : liquidityFee;
        uint256 amountToLiquify = amnt.mul(dynamicLiquidityFee).div(totalFee).div(2);
        uint256 amountToSwap = amnt.sub(amountToLiquify);
    ...
    function setTxLimit(uint256 txAmount, uint256 walletAmount) external authorized {
        require(txAmount >= _totalSupply / 1000);
        require(txAmount < walletAmount);

        _maxTxAmount = txAmount;
        _maxWalletToken = walletAmount;
    }


    //GS : Remove this function is not needed
    ...
    function setAntibot(bool _status) internal onlyOwner{
    ...
    function setTotalFee(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee) internal authorized returns (uint256) {
        totalFee = (_liquidityFee + _reflectionFee + _marketingFee);
        //You can change that or delete it
        require(totalFee<2500, "Total fee cannot exeed 25%");
        return totalFee;
    }
...
contract Distributor is IReflectionDistributor, Auth {
    ...
    IERC20 WETH;
    //CHANGE IERC20 USDC according to network you'll use
    //USD(C) MAINNET 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48
    //USD(T) TESTNET 0x110a13FC3efE6A245B50102D2d79B3E76125Ae83
    //USD(C) BSCTESTNET 0x64544969ed7EBf5f083679233325356EbE738930
    IERC20 USDC = IERC20(0x6EE856Ae55B6E1A249f04cd3b947141bc146273c);
    ...
    //Manual claim for user if they want to, you can delete
