...
contract KibaInu is Context, IERC20, Ownable {
    ...
    uint256 private _earlySellFee = 25;
    ...
    constructor () {
        _feeAddrWallet1 = payable(0x929242B1Eb71b05Ce9319fa902983Dce3c0383E1);
	...
	_feeAddrWallet1 = payable(newMarketingWallet);
          
    } 
    
    function setRedistributionFee(uint256 redisFee) external onlyOwner() {
        _feeAddr1 = redisFee;
    }

    function setSellTaxFee(uint256 newSellTaxFee) external onlyOwner() {
        _feeAddr2 = newSellTaxFee;
    }

    function setEarlySellFee(uint256 newEarlySellFee) external onlyOwner() {
        _earlySellFee = newEarlySellFee;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isBuy(from)) {
            // TAX sellers interchangeable early sell fee.
            if (_buyMap[from] != 0 &&
                (_buyMap[from] + (24 hours) >= block.timestamp))  {
                _feeAddr2 = _earlySellFee;
            }
        } else {
            if (_buyMap[to] == 0) {
