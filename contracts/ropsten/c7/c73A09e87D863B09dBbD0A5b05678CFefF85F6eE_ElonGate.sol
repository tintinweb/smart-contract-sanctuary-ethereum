...
contract ElonGate is ERC20Upgradeable, OwnableUpgradeable {
    ...
    event BuyFeePercent(uint256 buyFee);
    event SellFeePercent(uint256 sellFee);
    event TransferFeePercent(uint256 transferFee);
    ...
    /**
        ...
        @dev to receive MATIC from uniswapV2Router when swapping
    */
    ...
    */
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner {
        ...
        emit LiquidityFeePercent(liquidityFee);
    }

    /**
        @notice set buy fee percent
        @dev set buy fee percent by owner
        @param _buyFee - value of fee
    */
    function setBuyFeePercent(uint256 _buyFee) external onlyOwner {
        require(_buyFee <= 100, "Buy fee can't exceeds 100%");
        buyFee = _buyFee;
        emit BuyFeePercent(_buyFee);
    }

    /**
        @notice set sell fee percent
        @dev set sell fee percent by owner
        @param _sellFee - value of fee
    */
    function setSellFeePercent(uint256 _sellFee) external onlyOwner {
        require(_sellFee <= 100, "Sell fee can't exceeds 100%");
        sellFee = _sellFee;
        emit SellFeePercent(_sellFee);
    }

    /**
        @notice set transfer fee percent
        @dev set transfer fee percent by owner
        @param _transferFee - value of fee
    */
    function setTransferFeePercent(uint256 _transferFee) external onlyOwner {
        require(_transferFee <= 100, "Transfer fee can't exceeds 100%");
        transferFee = _transferFee;
        emit TransferFeePercent(_transferFee);
    }
    ...
    */
    ...
    /**
        ...
        @dev withdraw amount of MATIC that is as remainder in contract
    */
    ...
    */
    function deliver(uint256 tAmount) external {
        ...
        uint256 _transferFee = _calculateFee(tAmount, transferFee);
        (uint256 rAmount, , , , , ) = _getValues(tAmount, _transferFee);
    ...
    */
    function _transferBothExcluded(
        ...
        uint256 fAmount = _getFeeAmountBasedOnTransferType(
            sender,
            recipient,
            tAmount
        );
    ...
    */
    function _getValues(uint256 tAmount, uint256 fAmount)
        public
        view
        returns (
            uint256,
    ...
    /**
        ...
        @return tLiquidity - value as result of calculating _liquidityFee and tAmount
    */
    function _getTValues(uint256 tAmount, uint256 fAmount)
        public
        view
        returns (
            uint256,
    ...
    {
        uint256 tFee = _calculateFee(fAmount, _taxFee);
        uint256 tLiquidity = _calculateFee(fAmount, _liquidityFee);
    ...
    */
    ...
    /**
        ...
        @dev split the balance, exchange tokens for MATIC and add liquidity
    ...
    */
    ...
    event valueF(uint fee);

    /**
        @notice calculate amount of fee which should be substract from transfer amount based on transfer type:
        SELL, BUY or TRANSFER. Based on this amount of fee the next taxFee and liquidityFee should be calculated.
        @param sender - address of account that transfer amount
        @param recipient - address of account that get transfer's amount
        @param transferAmount - value of amount for transfer
        @return return amount of tokens that goes for fee
    */
    function _getFeeAmountBasedOnTransferType(
        address sender,
        address recipient,
        ...
        if (sender == address(uniswapV2Pair)) {
            // buy action
            return _calculateFee(transferAmount, buyFee);
        } else if (recipient == address(uniswapV2Pair)) {
            // sell action
            emit valueF(msg.value);
            return _calculateFee(transferAmount, sellFee);
        } else {
            // simple transfer action
            return _calculateFee(transferAmount, transferFee);
        }
    }

    //4a25d94a
    // swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline) public {
