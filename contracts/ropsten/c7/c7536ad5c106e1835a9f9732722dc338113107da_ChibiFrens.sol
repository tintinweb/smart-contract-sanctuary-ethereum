...
contract ChibiFrens is ERC721A, Ownable, ReentrancyGuard {
    ...
    function claim(claims calldata _claim) external onlySender {
        require(!claimPaused, "No claim.");
        uint256 amount = 0;

        uint256 numOfGen1 = _claim.gen1TokenIds.length;
        uint256 numOfGen2 = _claim.gen2TokenIds.length;
        uint256 numOfGen3 = _claim.gen3TokenIds.length;

        // for gen 1
        for (uint256 i = 0; i < numOfGen1; i++) {
            uint256 id =  _claim.gen1TokenIds[i];
            _claimCheck(Gen1Contract, gen1Addr, id);
            amount++;
        }

        // for gen 2
        for (uint256 i = 0; i < numOfGen2; i++) {
            uint256 id =  _claim.gen2TokenIds[i];
            _claimCheck(Gen2Contract, gen2Addr, id);
            amount++;
        }

        // for gen 3
        for (uint256 i = 0; i < numOfGen3; i++) {
            uint256 id =  _claim.gen3TokenIds[i];
            _claimCheck(Gen3Contract, gen3Addr, id);
            amount++;
        }
    ...
    function setPaused(bool _claimPaused, bool _publicMintPaused) external onlyOwner {
