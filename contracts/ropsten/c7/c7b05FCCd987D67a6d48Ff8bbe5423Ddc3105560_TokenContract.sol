...
// Contest
    ...
    function contest(address buyer, uint256 tokenAmount) internal {
        ...
        uint256 bought = getTokenPrice(tokenAmount);
        biggestBuy[buyer] = bought;
        
        if(biggestBuy[biggestBuyer]< bought){
            biggestBuyer = buyer;
            }

        if (time_since_start >= 5 minutes){
            ...
            contestWinner = biggestBuyer;
            biggestBuyer == address(0); 
        }
    }

    function startContest() public payable onlyOwner{
