...
contract AlfinAdminNft is Ownable, Pausable, ERC721, ERC721Enumerable {
    using SafeERC20 for IERC20;
    using Strings for uint256;

    bool public _freeMintEnabled = false;

    uint256 public _nftPrice = 0.1 ether;
    uint256 public _maxSupply = 1111;
    uint256 public _maxPerWallet = 0;
    uint256 public _maxMintPerTx = 0;
    uint256 public _mintActiveAt = 1658000000; // July 16
     ...
     * @dev User mint nfts
     */
    function userMint(uint256 mintAmount_) external payable whenNotPaused {
        uint256 callerBalance = balanceOf(_msgSender());
        require(_freeMintEnabled, "User mint disabled");
        ...
        require(
            _maxPerWallet == 0 || callerBalance + mintAmount_ <= _maxPerWallet,
            "Exceed wallet limit"
        );
        require(
            msg.value >= _nftPrice * mintAmount_,
            "Insufficient funds to mint"
        );

        _mintLoop(address(0), _msgSender(), mintAmount_);
    }

    /**
     * @notice Admin mint nfts
     */
    function adminMint(address[] memory accounts_, uint256 mintAmount_)
        external
    {
        require(_admins[_msgSender()], "Only admin");
        require(mintAmount_ > 0, "Invalid amount");
        for (uint256 i = 0; i < accounts_.length; i++) {
            address to = accounts_[i];
            _mintLoop(_msgSender(), to, mintAmount_);
        }
    }
    ...
    function _mintLoop(
        ...
        for (uint256 i = 0; i < mintAmount_; i++) {
            ...
            if (admin_ != address(0)) {
                emit AdminMint(admin_, receiver_, mintIndex);
            }
        }
    }

    /**
     * @notice Set admin or not the account
     */
    function setAdmin(address account_, bool flag_) external onlyOwner {
        _admins[account_] = flag_;
    }

    /**
     * @notice Enable free mint from users
     */
    function enableFreeMint(bool flag_) external onlyOwner {
        _freeMintEnabled = flag_;
    }
     ...
     */
    function setMaxMintPerTx(uint16 amount_) external onlyOwner {
     ...
     */
    function setMaxPerWallet(uint16 amount_) external onlyOwner {
