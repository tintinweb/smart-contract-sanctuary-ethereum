...
contract CurrencyMinter is AccessControl, Ownable, Signer {
  event LogWithdrawApproved(
    address indexed _to,
    uint256 _amount,
    uint256 _nonce
  );
  ...
  /**
   * @dev withdraw a user to withdraw his rewards
  ...
  function withdrawApproved(
    ...
    bytes32 msgHash = keccak256(abi.encodePacked(_to, _amount, _nonce));
    ...
    ICurrency(currencyAddress).mint(_to, _amount);

    emit LogWithdrawApproved(_to, _amount, _nonce);
  }

  function setCurrencyAddress(address _currencyAddress) external onlyOwner {
