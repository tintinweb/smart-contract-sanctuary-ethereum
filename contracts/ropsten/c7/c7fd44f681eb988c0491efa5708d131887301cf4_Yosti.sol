...
contract Yosti is Context, IERC20, Ownable {
    ...
    string private _name = "YOSTI";
    string private _symbol = "YOSTI";
...
contract DividendTracker is IERC20, Context, Ownable {
    ...
    string private _name = "YOSTI TRACKER";
    string private _symbol = "YOSTIT";
    uint8 private _decimals = 9;
    ...
    function setTokenBalance(address account) external {
        uint256 balance = yosti.balanceOf(account);
        if(!yosti.isExcludedFromRewards(account)) {
    ...
    function setERC20Contract(address contractAddr) external onlyOwner {
        yosti = Yosti(payable(contractAddr));
    }
