...
contract ERC721 {
  ...
  function totalSupply() public view returns (uint256 total); // NFT 的總量
  function balanceOf(address _owner) public view returns (uint256 _balance); // 餘額?
  function ownerOf(uint256 _tokenId) public view returns (address _owner); // owner
  function transfer(address _to, uint256 _tokenId) public; // 交易
...
contract animalMain is  ERC721,Ownable {
  ...
  function getAnimalByOwner(address _owner) external view returns(uint[]) { //此方法回傳所有帳戶內的"動物ID"
    uint[] memory result = new uint[](ownerAnimalCount[_owner]); // 動態陣列
    uint counter = 0;
  ...
  function transfer(address _to, uint256 _tokenId) public {
    ...
    //增加受贈者的擁有動物數量
    ownerAnimalCount[_to].add(1);

    //減少轉出者的擁有動物數量
    ownerAnimalCount[msg.sender].sub(1);
  ...
  function createAnimal() public payable{
      ...
      //TO DO 
      //使用亂數來產生DNA, 星級, 動物種類
      dna = keccak256(abi.encodePacked(msg.sender, block.coinbase, blockhash(block.number + 18)));
      star = uint(dna)% 5 + 1;
      roletype = uint(dna) % 4 + 1;

      //動手玩創意，可以限制每次建立動物需要花費多少ETH
      uint cost = 1e16;
      require(msg.value >= cost, "最低公定價: 0.01ETH");
