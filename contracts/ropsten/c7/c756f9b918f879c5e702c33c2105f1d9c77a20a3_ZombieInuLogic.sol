...
// File: Tokens/ZINU/ZombieInuLogic.sol
...
contract ZombieInuLogic is Initializable, UUPSUpgradeable, OwnableUpgradeable, IERC20Upgradeable {
    ...
    function initialize() public initializer {
        ...
        _tTotal = 1000000000000 * 1e18; //Total Supply
        _maxTxAmount = _tTotal.mul(30).div(10000); //0.30%
        _maxWalletAmount = _tTotal.mul(100).div(10000); //1%
        swapAmount = _tTotal.mul(10).div(10000); //0.10%
        _buybackThreshold = 10;//10**18; //1 ether
        ...
        _taxWallet1 = payable(0xd3c3A23402Eaf9A932f7b9Dafef44b246632faB8);
        _taxWallet2 = payable(0xd3c3A23402Eaf9A932f7b9Dafef44b246632faB8);
        _buybackTokenReceiver = 0xd3c3A23402Eaf9A932f7b9Dafef44b246632faB8;
        _lpTokensReceiver = 0xd3c3A23402Eaf9A932f7b9Dafef44b246632faB8;
    ...
    function removeAllFee() private {
        if (rBurnFee == 0 && rLPFee == 0 && rMarketingFee == 0) return;
        pBurnFee = rBurnFee;
        pLPFee = rLPFee;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        fees.tAmount = amount.sub(fees.tBurn).sub(fees.tLiq).sub(fees.tBuyback).sub(fees.tMarket);
        ...
        _balances[address(this)] = _balances[address(this)].add(fees.tBurn).add(fees.tLiq).add(fees.tBuyback).add(fees.tMarket);
    ...
    function setSellFee(uint256 _sMarketingFee, uint256 _sLPFee, uint256 _sBuybackFee, uint _sBurnFee) public onlyOwner {
    ...
    function readFees() external view returns (uint _totalBuyFee, uint _totalSellFee, uint _burnFeeBuy, uint _burnFeeSell, uint _marketingFeeBuy, uint _marketingFeeSell, uint _liquidityFeeBuy, uint _liquidityFeeSell, uint _buybackFeeBuy, uint _buybackFeeSell) {

        return (
            bBurnFee+bMarketingFee+bLPFee+bBuybackFee,
            sBurnFee+sMarketingFee+sLPFee+sBuybackFee,
            bBurnFee,
            sBurnFee,
            bMarketingFee,
            sMarketingFee,
