...
contract TicTacToeGame {
    ...
    modifier atStatus(GameStatus _status) {
    ...
    function move(uint256 _cell, TicTac _tictac) 
        external 
        atStatus(GameStatus.started) 
    {
        if (_cell >= 9 || games[id].fields[_cell] == TicTac.no || _tictac == TicTac.no) {
            revert invalidCellOrTicTac();
        }

        if ((msg.sender != games[id].player1 && _tictac == TicTac.cross) ||
                (msg.sender != games[id].player2 && _tictac == TicTac.zero)) 
        {
