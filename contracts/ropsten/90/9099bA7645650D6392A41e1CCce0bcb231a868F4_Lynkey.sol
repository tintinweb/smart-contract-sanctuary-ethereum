...
contract Lynkey is ERC20Burnable {
    ...
    function lockInvestorWallet(address _wallet, uint256 _amount) private{
        ...
        StageItem memory last = stageList[stageList.length - 1];
        if(currentTime >= last.endTime){
            return;
        }
        for(uint256 i = 0; i< stageList.length; i++){
            StageItem memory item = stageList[i];
            if (block.timestamp >= item.startTime && block.timestamp < item.endTime){
                uint idx =1;
                uint256 totalAmount = 0;
                uint256 amount = 0;
                while (idx <= item.totalVestingMonth) {
                    uint256 releaseTime = listingDate + (idx - 1)*2628000;
                    if(idx <= item.firstVestingMonth){
                        amount = _amount*item.firstVestingPercent/10000;
                        totalAmount += amount;
                    }else if(idx < item.totalVestingMonth){
                        amount = _amount*item.lastVestingPercent/10000;
                        totalAmount += amount;
                    }else{
