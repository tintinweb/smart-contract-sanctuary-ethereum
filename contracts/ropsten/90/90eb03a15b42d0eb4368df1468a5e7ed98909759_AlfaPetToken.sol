 ...
 */
    ...
    function _transfer(address _to, uint _value, bytes _data, string _custom_fallback) private returns (bool success) {
    ...
    // Function that is called when a user or another contract wants to transfer funds.
    function _transfer(address _to, uint _value, bytes _data) private returns (bool success) {
    ...
    * @param _value The amount to be transferred.
    */
    function _transfer(address _to, uint _value) private returns (bool success) {            
        //standard function transfer similar to ERC20 transfer with no _data
        //added due to backwards compatibility reasons
        bytes memory empty;
        if(isContract(_to)) {
