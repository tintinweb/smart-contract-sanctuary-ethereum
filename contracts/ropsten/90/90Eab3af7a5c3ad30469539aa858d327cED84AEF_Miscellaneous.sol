...
contract Miscellaneous is ERC721, Ownable, Ticketed {
    ...
    // this is only here for frontend purposes,
    // the above unclaimedDays map is for checking!
    ...
    {
        uint256 _nextTokenId = nextTokenId;
        if (!allowlistSaleActive) revert SaleInactive();
    ...
    function setAllowlistSaleState(bool active) external onlyOwner {
        allowlistSaleActive = active;
    }
    ...
    function claim(uint256 tokenId) external {
        ...
        if (currentDay == 0) revert NotRevealed();
        claims.push(tokenId);
    }
