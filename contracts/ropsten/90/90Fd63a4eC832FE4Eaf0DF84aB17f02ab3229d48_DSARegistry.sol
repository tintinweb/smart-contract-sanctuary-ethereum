...
contract DSARegistry is IRegistry, AccessControl {
    ...
    function newDatadownload(
        ...
        if (paymentToken == address(0)) {
            ...
            downloadIdData[downloadId] = USERDOWNLOADDATA({buyer: msg.sender, enterprise: enterprise, datasetId: datasetId, symbol: 'ETH', paymentToken: datasets[datasetId].paymentToken, amount: datasets[datasetId].amount, ipfsHash: ipfsHash});
            userDownloadData[msg.sender].push(USERDOWNLOADDATA({buyer: msg.sender, enterprise: enterprise, datasetId: datasetId, symbol: 'ETH', paymentToken: datasets[datasetId].paymentToken, amount: datasets[datasetId].amount, ipfsHash: ipfsHash}));
            ...
            emit Newdownload(downloadId, 'ETH', datasets[datasetId].paymentToken, datasets[datasetId].amount);  
    ...
    function uploadDataset(
        ...
        string memory symbol = paymentToken == address(0)? 'ETH': ERC20(paymentToken).symbol();

        // add to array index to include in getAllDatasets()
        datasets[datasetId].arrayIndex = dataArrayIndex++;
        datasets[datasetId].datasetId = datasetId;
        datasets[datasetId].symbol = symbol;
        ...
        metadata[datasetId] = METADATA({datasetId: datasetId, symbol: symbol, paymentToken: paymentToken, amount: amount, access: DATA_ACCESS(dataAccess)});
        ...
        if (!c.enterpriseFeeWithdrawn && amount > 0) {
            if (c.paymentToken == address(0)) {
                if (to == address(0)) {
                    ...
                    emit PaymentWithdrawn('ETH', c.paymentToken, amount, downloadId, msg.sender);
                } else {
                    payable(to).transfer(amount);
                    emit PaymentWithdrawn('ETH', c.paymentToken, amount, downloadId, to);
                }
        ...
        override
    {
        ...
        if (!c.adminFeeWithdrawn && amount > 0) {
            if (c.paymentToken == address(0)) {
                ...
                emit FeeWithdrawn('ETH', c.paymentToken, amount, SpaceChain);
...
interface IRegistry {
    ...
    function getAllMetadata() external view returns (METADATA[] memory);
