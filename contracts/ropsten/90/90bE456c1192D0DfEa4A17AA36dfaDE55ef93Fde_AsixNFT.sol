...
contract AsixNFT is ERC1155, Ownable{
    ...
    function mintItem(string memory _name, uint256 _price, TYPE itemType, string memory _url, string memory _others) external {
		...
		IdToItem[currentId].url = _url;
        IdToItem[currentId].onSell = false;
		IdToItem[currentId].name = _name;
		IdToItem[currentId].price = pricePlusFees;
		IdToItem[currentId].others = _others;
    ...
    function getItemPrice(uint256 _itemId) external
        view
        returns (uint256)
    {
