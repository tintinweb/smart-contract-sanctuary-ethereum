...
abstract contract ERC721URIStorage is ERC721 {
    ...
    function drawPoly(uint256 x1, uint256 y1, uint256 x2, uint256 y2, uint256 x3, uint256 y3, uint256 x4, uint256 y4, string memory color) public view returns (string memory) {
    ...
    function drawRect(uint256 x, uint256 y, uint256 w, uint256 h, string memory color) public view returns (string memory) {
    ...
    function test(uint256[2][] memory coordies) public view virtual returns (string memory){
        SlotInfo memory slot;
        slot.polys = '<svg width="500" height="500" xmlns="http://www.w3.org/2000/svg"><polygon fill="black" points="0,0 500,0 500,500 0,500" />';
        slot.poly = drawRect(coordies[0][0], coordies[0][1], coordies[1][0], coordies[1][1], "blue");
        slot.polys = string(abi.encodePacked(slot.polys, slot.poly));
        slot.poly = drawRect(coordies[2][0], coordies[2][1], coordies[3][0], coordies[3][1], "pink");
        slot.polys = string(abi.encodePacked(slot.polys, slot.poly));
        
        if (coordies[0][1] < coordies[2][1]) {
            slot.poly = drawPoly(coordies[0][0], coordies[0][1], coordies[0][0] + coordies[1][0], coordies[0][1], coordies[2][0] + coordies[3][0], coordies[2][1], coordies[2][0], coordies[2][1], "red");
            slot.polys = string(abi.encodePacked(slot.polys, slot.poly));
        }
        
        if (coordies[0][0] + coordies[1][0] > coordies[2][0] + coordies[3][1]) {
            slot.poly = drawPoly(coordies[0][0] + coordies[1][0], coordies[0][1], coordies[2][0] + coordies[3][0], coordies[2][1], coordies[2][0] + coordies[3][0], coordies[2][1] + coordies[3][1], coordies[0][0] + coordies[1][0], coordies[0][1] + coordies[1][1], "yellow");
            slot.polys = string(abi.encodePacked(slot.polys, slot.poly));
        }
        
        if (coordies[0][1] + coordies[1][1] > coordies[2][1] + coordies[3][1]) {
            slot.poly = drawPoly(coordies[0][0], coordies[0][1] + coordies[1][1], coordies[0][0] + coordies[1][0], coordies[0][1] + coordies[1][1], coordies[2][0] + coordies[3][0], coordies[2][1] + coordies[3][1], coordies[2][0], coordies[2][1] + coordies[3][1], "red");
            slot.polys = string(abi.encodePacked(slot.polys, slot.poly));
        }
        if (coordies[0][0] < coordies[2][0]) {
            slot.poly = drawPoly(coordies[0][0], coordies[0][1], coordies[2][0], coordies[2][1], coordies[2][0], coordies[2][1] + coordies[3][1], coordies[0][0], coordies[0][1] + coordies[1][1],  "orange");
            slot.polys = string(abi.encodePacked(slot.polys, slot.poly));
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        for (uint256 i = 0; i < _noObjects[tokenId]-1; i++) {
            for (step = 0; step < limit; step++){ 
                ...
                //slot.newX =  _coords[tokenId][i*2][0] + (_coords[tokenId][i * 2 + 2][0] - _coords[tokenId][i*2][0]) * (step / limit);
                slot.newX =  20 + (20 - 20);
                //slot.newX =  40 + (80 - 40) * (step / limit);
                ...
                slot.polys = string(abi.encodePacked(slot.polys, "test"));
            }
        }
