...
contract ProfileWithTeams is AccessControl, ERC721Holder {
    ...
    uint256 public activeProfilesCount;
    uint256 public reactivationFee;
    uint256 public registrationFee;
    uint256 public updateProfileFee;
    ...
    Counters.Counter private _usersCounter;

    // Event to notify a new team is created
    event TeamNew(uint256 teamId, string teamName);
    ...
    constructor(
        IERC20 _feeToken,
        uint256 _reactivationFee,
        uint256 _registrationFee,
        uint256 _updateProfileFee
    ) {
        feeToken = _feeToken;
        reactivationFee = _reactivationFee;
        registrationFee = _registrationFee;
        updateProfileFee = _updateProfileFee;
    ...
    function createProfile(
        ...
        require((_teamId <= teamsCount) && (_teamId > 0), "Invalid teamId");
        ...
        // Increment the _usersCounter counter and get userId
        _usersCounter.increment();
        uint256 newUserId = _usersCounter.current();
        ...
        activeProfilesCount = activeProfilesCount.add(1);
    ...
    function pauseProfile() external {
        ...
        activeProfilesCount = activeProfilesCount.sub(1);
    ...
    function addTeam(
        ...
        // Increment the _teamsCounter counter and get teamId
        _teamsCounter.increment();
        uint256 newTeamId = _teamsCounter.current();
        ...
        teamsCount = newTeamId;
        emit TeamNew(newTeamId, _teamName);
    }
        ...
        onlySpecial
    {
        ...
        require(
            (_newTeamId <= teamsCount) && (_newTeamId > 0),
    ...
    function claimFee(uint256 _amount) external onlyOwner {
        feeToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    function makeTeamJoinable(uint256 _teamId) external onlyOwner {
        require((_teamId <= teamsCount) && (_teamId > 0), "teamId invalid");
    ...
    /**
     * @dev Updates registration, reactivation, and update fees.
     * Callable only by owner admins.
     */
    function updateFees(
        uint256 _newreactivationFee,
        uint256 _newregistrationFee,
        uint256 _newupdateProfileFee
    ) external onlyOwner {
        reactivationFee = _newreactivationFee;
        registrationFee = _newregistrationFee;
        updateProfileFee = _newupdateProfileFee;
    }
