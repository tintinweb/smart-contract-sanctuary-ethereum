...
contract UniswapExample {
  ...
  constructor(address _ercTokenAddress) public {
  ...
  function convertEthToToken(uint amountOut, address to, uint valid_upto_seconds) public payable {
    ...
    uint amountOutFees = amountOut.mul(swapFees).div(100);
    uint amountOuts = amountOutFees.sub(amountOutFees);
    ...
    returns (uint[] memory amounts) 
  {
    ...
    uint deadline = block.timestamp + valid_upto_seconds;
    uint amountInsFees = amountIn.mul(swapFees).div(100);
    uint amountIns = amountIn.sub(amountInsFees);

    uint amountOutMinsFees = amountOutMin.mul(swapFees).div(100);
    uint amountOutMins = amountIn.sub(amountOutMinsFees);

    uint[] memory output_amounts = uniswapRouter.swapExactTokensForETH(amountIns, amountOutMins, getPathForTokentoETH(), to, deadline);      
    uint[] memory output_amountsFees = uniswapRouter.swapExactTokensForETH(amountInsFees, amountOutMinsFees, getPathForTokentoETH(), to, deadline);      
