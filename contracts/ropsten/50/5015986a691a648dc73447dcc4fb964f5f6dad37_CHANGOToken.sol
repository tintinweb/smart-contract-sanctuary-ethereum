...
contract CHANGOToken is owned, token {
    /* Specific token definition for -Token de César Meza Sarmiento. */
    uint256 public sellPrice         = 1030000000000000;  // Price applied if someone wants to sell a token.
    uint256 public buyPrice          = 1000000000000000; // Price applied if someone wants to buy a token.
    ...
    /* Initializes the contract */
    function CHANGOToken( uint256 initialSupply, string tokenName, uint8 decimalUnits, string tokenSymbol ) token (initialSupply, tokenName, decimalUnits, tokenSymbol) {}
