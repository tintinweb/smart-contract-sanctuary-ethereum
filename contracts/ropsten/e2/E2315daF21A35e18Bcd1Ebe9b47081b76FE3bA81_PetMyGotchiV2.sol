...
contract PetMyGotchiV2 is Initializable, OwnableUpgradeable, UUPSUpgradeable {

    using MathUpgradeable for uint256;
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    EnumerableSetUpgradeable.AddressSet private parents;
    EnumerableSetUpgradeable.AddressSet private banned_parents;
    ...
    function addParent() external payable{
        require(! EnumerableSetUpgradeable.contains(parents, _msgSender()), "Already exists!");
        require(! EnumerableSetUpgradeable.contains(banned_parents, _msgSender()), "Banned!");
        ...
        EnumerableSetUpgradeable.add(parents, _msgSender());
        parent_balance[_msgSender()] += msg.value;
    }

    function addCustomParent(address _parent) public onlyOwner payable{
        require(! EnumerableSetUpgradeable.contains(parents, _parent), "Already exists!");
        require(! EnumerableSetUpgradeable.contains(banned_parents, _parent), "Banned!");

        EnumerableSetUpgradeable.add(parents, _parent);
        parent_balance[_parent] += msg.value;
    }

    function removeParent() public {
        require(EnumerableSetUpgradeable.contains(parents, _msgSender()), "Does not exists!");

        EnumerableSetUpgradeable.remove(parents, _msgSender());
        withdrawBalanceFor(_msgSender());
    }

    function removeCustomParent(address _parent) public onlyOwner{
        require(EnumerableSetUpgradeable.contains(parents, _parent), "Does not exists!");

        EnumerableSetUpgradeable.remove(parents, _parent);
        withdrawBalanceFor(_parent);
    }

    function banCustomParent(address _parent) external onlyOwner{
        require(! EnumerableSetUpgradeable.contains(banned_parents, _parent), "Already Banned!");

        EnumerableSetUpgradeable.add(banned_parents, _parent);

        if(EnumerableSetUpgradeable.contains(parents, _parent)){
            EnumerableSetUpgradeable.remove(parents, _parent);
            withdrawBalanceFor(_parent);
        }
    }
    ...
    function unbanCustomParent(address _parent) external onlyOwner{
        require(EnumerableSetUpgradeable.contains(banned_parents, _parent), "Not Banned!");

        EnumerableSetUpgradeable.remove(banned_parents, _parent);
    }
    ...
    function parentExists(address _parent) public view returns(bool) {
        return EnumerableSetUpgradeable.contains(parents, _parent);
    }

    function getParentAt(uint256 _index) public view returns(address) {
        return EnumerableSetUpgradeable.at(parents, _index);
    }

    function countParents() public view returns(uint256) {
        return EnumerableSetUpgradeable.length(parents);
    }

    function getBannedParentAt(uint256 _index) public view returns(address) {
        return EnumerableSetUpgradeable.at(banned_parents, _index);
    }

    function countBannedParents() public view returns(uint256) {
        return EnumerableSetUpgradeable.length(banned_parents);
    }
    ...
    function whenNextDepositIsRequiredFor(address _parent) public view returns (uint256){
        ...
        if(_feePerDay > 0){
            _enoughForDays = MathUpgradeable.ceilDiv( parent_balance[_parent], _feePerDay ) * 1 days;
    ...
    function takingCareOf() public view returns(uint256[] memory){
        ...
        for (uint256 i = 0; i < EnumerableSetUpgradeable.length(parents); i++) {

            address _parent = EnumerableSetUpgradeable.at(parents, i);
