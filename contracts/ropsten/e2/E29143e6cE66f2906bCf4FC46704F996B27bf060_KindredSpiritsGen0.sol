...
contract KindredSpiritsGen0 is CustomERC721Metadata {
    ...
    function getRandomness(uint256 _projectId, uint256 currentValue, uint256 maxValue) internal view returns (uint256) {
        uint256[] memory enabledIds = new uint256[](TEN_THOUSAND);
        uint256 count = 0;
        ...
        uint256 randomNumber = uint256(keccak256(abi.encodePacked(block.number, block.timestamp, msg.sender, currentValue, randomizerContract.returnValue()))) % count;
        return enabledIds[randomNumber];
    }

    function randomValue() public view returns (uint256) {
        return uint256(randomizerContract.returnValue());
    }
}