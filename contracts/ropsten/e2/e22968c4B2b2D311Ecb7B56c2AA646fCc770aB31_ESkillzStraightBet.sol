...
contract ESkillzStraightBet is Ownable {
    ...
    //   sport.mintMore(amount);
      ...
      //gamebetting[gameIds].push(Bet(feeReceiver, amount, 0));
    ...
    function SetMPGameResult(uint256 GameID) external {
        ...
        //require(winnerAddress == msg.sender, "Winner Address must includes in this game");
    ...
    function getAmountsSPGameToDistribute(uint256 game, uint256 result) private view returns (uint256, uint256) {
        uint256 amountToWinner = (gamebetting[game][0].amount+gamebetting[game][0].amount.mul(result).div(100)).mul(100-eskillz_fee).div(100);
        uint256 amountToESkillz = (gamebetting[game][0].amount+gamebetting[game][0].amount.mul(result).div(100)).mul(eskillz_fee).div(100);
