...
contract InvestLogic is Ownable, Pausable {  
    ...
    mapping(uint256 => uint) public investmentPool;     // to manage the staking of usdtToken and distibue the profit as usdtToken 
    mapping(uint256 => uint) public depositedAmount;     // to track the deplosited amount 
    ...
    mapping(uint256 =>uint256) public stakedMiami; // Staked Miami amount of the Pool
    ...
    uint256 private miamiPercent = 10;
    uint256 private restakingTaxPercent = 5;
 
    // mapping(address => mapping(address => uint256)) userAmount;
 
    constructor(IERC20 _token, IERC20 _miamiToken, address _usdtAddress, uint256 _usdConversionFactor) {
        usdtToken = _token;
        miamiToken = _miamiToken;
    ...
    function getMiamiToTransfer(uint256 _amount) external view whenNotPaused returns(uint256){
        return _amount * usdConversionFactor * miamiPercent/(100);
    }
    ...
    function staketoken(uint _amount, terms time, address _stableCoin) public whenNotPaused {  
        ...
        uint256 miamiAmount = _amount * usdConversionFactor * miamiPercent/(100);
 
        require(miamiToken.balanceOf(msg.sender) >= miamiAmount, "Not enough balance of Miami");
        ...
        // Take 10% Miami
        miamiToken.safeTransferFrom(msg.sender, address(this), miamiAmount);
        // UPDATES
        if(time == terms.one){
        ...
        totalDepositedToStakedToken[_stableCoin] += _amount;
        stakingStartTime[currentPoolID] = block.timestamp;
        coinsTotalPools[_stableCoin] ++;
        totalInvestmentPoolBalance += _amount;
        investmentPool[currentPoolID] = _amount;
        depositedAmount[currentPoolID] = _amount;
        stakedMiami[currentPoolID] = miamiAmount;
        isStaking[msg.sender][currentPoolID] = true; // error
    ...
    function calculateReward(uint256 investmetPoolBalance, uint256 id) internal virtual returns(uint256){
        uint256 startTime = stakingStartTime[id];
        uint256 termMuliplier = (block.timestamp - startTime)/userTerms[id];
        uint256 reward = investmetPoolBalance * termMuliplier * userRates[id] / (100);
        return reward;
    }
 
    function redeemInterest(uint256 poolID) public whenNotPaused {
       
        require(isStaking[msg.sender][poolID], "User have no staked tokens to get the reward");
        ...
        require(block.timestamp - redeemedAt[poolID] >= userTerms[poolID], "wait after last redeem");
    ...
    function restake(uint256 poolID) public whenNotPaused {
        ...
        uint256 rewardAmount = calculateReward(balance, poolID);
        uint256 restakingTax = rewardAmount * restakingTaxPercent /100;
        uint256 restakingAmount = rewardAmount - restakingTax;
    ...
    function withdraw(uint256 poolID) public whenNotPaused {
        ...
        uint256 depositedBalance = depositedAmount[poolID];
        uint256 investmentPoolBalance = investmentPool[poolID];
        require(depositedBalance > 0, "staking balance cannot be 0");
        uint256 startTime = stakingStartTime[poolID];

        uint256 usdtReward = calculateReward(investmentPoolBalance, poolID);
        uint256 stableCoinBalance = depositedBalance;
        uint256 stakedMiamiBalance = stakedMiami[poolID];
        ...
        require(miamiToken.balanceOf(address(this)) >= stakedMiamiBalance, "Not Enough Miami in the smart contract");
        ...
        if(block.timestamp - startTime >= userTerms[poolID]){
            if((block.timestamp - redeemedAt[poolID] >= userTerms[poolID])){
                ...
                miamiToken.safeTransfer(msg.sender, stakedMiamiBalance);
        ...
        // UPDATES
        isStaking[msg.sender][poolID] = false;
    ...
    function calculateInterest(uint256 poolID, address staker) public view whenNotPaused returns(uint256){
 
        require(isStaking[staker][poolID], "User have no staked tokens to get the reward");
        require(poolOwner[poolID] == staker || msg.sender == owner(), "Caller is not the owner of the Pool or admin");
        uint256 balance = investmentPool[poolID];
        ...
        require(block.timestamp - startTime >= userTerms[poolID], "No Interest to redeem");
        uint256 termMuliplier = (block.timestamp - startTime)/userTerms[poolID];
        uint256 reward = balance * termMuliplier * userRates[poolID] / (100);
    ...
    function restakeByAdmin(uint256 poolID, address Owner) external whenNotPaused onlyOwner{
        ...
        require(isStaking[Owner][poolID], "User have no staked tokens to get the reward");
    ...
    function setMiamiPercent(uint256 percent) external onlyOwner whenNotPaused {
        miamiPercent = percent;
    }

    function setRestakingTaxPercent(uint256 percent) external onlyOwner whenNotPaused {
        restakingTaxPercent = percent;
    }

    function setMiamiToken(IERC20 _token) external onlyOwner whenNotPaused {
        miamiToken = _token;
    }
    ...
    function getMiamiToken() external view whenNotPaused returns(IERC20){
        return miamiToken;
    }
    ...
    function getMiamiPercent() external view whenNotPaused returns(uint256){
        return miamiPercent;
    }

    function getRestakingTaxPercent() external view whenNotPaused returns(uint256){
        return restakingTaxPercent;
    }
