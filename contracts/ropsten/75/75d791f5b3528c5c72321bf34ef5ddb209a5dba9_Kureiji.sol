...
contract Kureiji is ERC721Enum, Ownable, PaymentSplitter {
    ...
    function allowlistMint(uint256 _mintAmount, bytes32[] calldata _proof) external payable mintCheck(_mintAmount) {
        require(!presaleOnlyActive && !paused && !publicSaleActive, "Allowlist sale is not active");
    ...
    function publicMint(uint256 _mintAmount) external payable mintCheck(_mintAmount) {
        require(publicSaleActive && !paused, "Public sale is not active");
    ...
    function publicSaleOnlyActive(bool _state) public onlyOwner {
