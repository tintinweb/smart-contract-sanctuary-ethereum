...
contract Marketplace is IERC721Receiver, Context, Ownable {
    ...
    uint96 private _minCommission = 0;
    uint96 private _maxCommission = 5;
    uint96 public _commission = 1;

    uint256 private totalCommission = 0;

    uint96 public minRoyalty = 0;
    ...
    mapping(address => RoyaltyInfo) private _customRoyaltyInfo;
    ...
    event NewBidOnAuction(uint256 auctionIndex, uint256 newBid, uint256 bidCount, address bidBy);

    event Claimed(uint256 auctionIndex, uint256 nftId, address claimedBy);

    event ContractCreated(address NFTContract, address recipient);
    event Mint(address NFTContract, uint256 tokenId, address recipient, string id);
    ...
    function _createAuction(
        ...
        //Casting from address to address payabletransferAndCreateAuction
    ...
    function transferAndCreateAuction(
        address _addressNFTCollection,
        uint256 _nftId,
        uint256 _initialBid,
        uint256 _buyNowPrice,
        ...
        _createAuction(_addressNFTCollection, _nftId, _initialBid, _buyNowPrice, _endAuction);
    ...
    function mintAndCreateAuction(
        ...
        _mintAndCreateAuction(_initialRoyalty, _addressNFTCollection, _initialBid, _buyNowPrice, _endAuction, _id);
        return index;
    ...
    function _mintAndCreateAuction(
        ...
        NFTCollection _NFTCollection = NFTCollection(_addressNFTCollection);

        uint256 _nftId = _NFTCollection.safeMintExt(address(this));
        emit Mint(_addressNFTCollection, _nftId, _msgSender(), _id);

        _NFTCollection.setTokenRoyalty(_nftId, _msgSender(), _initialRoyalty);
    ...
    function collectionAndCreateAuction(
        string memory _name,
        ...
        NFTCollection _NFTCollection = new NFTCollection(
            _msgSender(),
            _msgSender(),
            _initialRoyalty,
            _name,
        ...
        address _addressNFTCollection = address(_NFTCollection);
        emit ContractCreated(_addressNFTCollection, _msgSender());
    ...
    function isOpen(uint256 _auctionIndex) public view returns (bool) {
        ...
        if (auction.buyNowPrice > 0 && auction.currentBidPrice >= auction.buyNowPrice) return false;
        return true;
    }
    ...
    returns (bool)
    {
        ...
        if (auction.bidCount > 0) {
            (bool success,) = payable(auction.currentBidOwner).call{
        ...
        emit NewBidOnAuction(_auctionIndex, _newBid, auction.bidCount - 1, _msgSender());
    ...
    function _claim(uint256 _auctionIndex) internal {
        require(_auctionIndex < allAuctions.length, "Invalid auction index");
        ...
        uint256 total = auction.currentBidPrice;
        uint256 commission = total.mul(_commission).div(_feeDenominator());
        ...
        RoyaltyInfo memory _royalty = _customRoyaltyInfo[auction.addressNFTCollection];

        if (_royalty.receiver == address(0)) {
            if( NFTCollection(auction.addressNFTCollection).supportsInterface(type(IERC2981).interfaceId) ){
                (royaltyReceiver, royalty) = NFTCollection(auction.addressNFTCollection).royaltyInfo(auction.nftId, total);
            }
        } else{
            royalty = total.mul(_royalty.royaltyFraction).div(_feeDenominator());
            royaltyReceiver = _royalty.receiver;
        }
        ...
        (success,) = payable(auction.creator).call{
            value : auction.currentBidPrice - commission - royalty
        }("");
        require(
            success,
            "Transfer error"
        ...
        emit Claimed(_auctionIndex, auction.nftId, _msgSender());
    }
    ...
    function refund(uint256 _auctionIndex) external {
        ...
        emit NFTRefunded(_auctionIndex, auction.nftId, _msgSender());
    }
    ...
    function setCustomRoyalty(
        ...
        require(feeNumerator <= maxRoyalty, "Royalty fee will exceed maxRoyalty");
        require(collection != address(0), "Invalid parameters");
        require(NFTCollection(collection).owner() == _msgSender(), "Invalid collection owner");

        _customRoyaltyInfo[collection] = RoyaltyInfo(_msgSender(), feeNumerator);
    }


    function resetCustomRoyalty(address collection) external {

        require(collection != address(0), "Invalid parameters");
        require(NFTCollection(collection).owner() == _msgSender(), "Invalid collection owner");

        delete _customRoyaltyInfo[collection];
    }
    ...
    function setCommission(uint96 commission1) public onlyOwner {
        // require(commission1 >= _minCommission, "Invalid commission");
        // require(commission1 <= _maxCommission, "Invalid commission");
        // _commission = commission1;
    }

    function claimCommission(uint256 amount) public onlyOwner {
        require(amount <= totalCommission, "Invalid amount");
    ...
    function setDefaultRoyalty(address contractAddress, address receiver, uint96 feeNumerator) public virtual {
        require(NFTCollection(contractAddress)._creator() == _msgSender(), "Creator error");
        NFTCollection(contractAddress).setDefaultRoyalty(receiver, feeNumerator);
    }

    function deleteDefaultRoyalty(address contractAddress) public virtual {
        require(NFTCollection(contractAddress)._creator() == _msgSender(), "Creator error");
        NFTCollection(contractAddress).deleteDefaultRoyalty();
    }
    ...
    function setTokenRoyalty(
        ...
        require(NFTCollection(contractAddress)._creator() == _msgSender(), "Creator error");
        NFTCollection(contractAddress).setTokenRoyalty(tokenId, receiver, feeNumerator);
    }

    function resetTokenRoyalty(address contractAddress, uint256 tokenId) public virtual {
        require(NFTCollection(contractAddress)._creator() == _msgSender(), "Creator error");
        NFTCollection(contractAddress).resetTokenRoyalty(tokenId);
    }
