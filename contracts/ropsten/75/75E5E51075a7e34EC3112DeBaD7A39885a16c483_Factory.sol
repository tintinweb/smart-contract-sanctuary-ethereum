...
import "./interfaces/IRythmPriceOracle.sol";
...
contract Factory is IFactory {
    ...
    IRythmPriceOracle public rythmPriceOracle;
    ...
    constructor(
        ...
        IRythmPriceOracle _rythmPriceOracle
    ) public {
        admin = _admin;
        reservesAdmin = _reservesAdmin;
        bDeployer = _bDeployer;
        cDeployer = _cDeployer;
        rythmPriceOracle = _rythmPriceOracle;
    ...
    function initializeLendingPool(address uniswapV2Pair) external {
        ...
        require(!lPool.initialized, "Rythm: ALREADY_INITIALIZED");

        require(
            lPool.collateral != address(0),
            "Rythm: COLLATERALIZABLE_NOT_CREATED"
        );
        require(
            lPool.borrowable0 != address(0),
            "Rythm: BORROWABLE0_NOT_CREATED"
        );
        require(
            lPool.borrowable1 != address(0),
            "Rythm: BORROWABLE1_NOT_CREATED"
        );

        (, , , , , bool oracleInitialized) =
            rythmPriceOracle.getPair(uniswapV2Pair);
        if (!oracleInitialized) rythmPriceOracle.initialize(uniswapV2Pair);
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        require(msg.sender == admin, "Rythm: UNAUTHORIZED");
    ...
    function _acceptAdmin() external {
        require(msg.sender == pendingAdmin, "Rythm: UNAUTHORIZED");
    ...
    function _setReservesPendingAdmin(address newReservesPendingAdmin)
        external
    {
        require(msg.sender == reservesAdmin, "Rythm: UNAUTHORIZED");
    ...
    function _acceptReservesAdmin() external {
        require(msg.sender == reservesPendingAdmin, "Rythm: UNAUTHORIZED");
...
interface IFactory {
	...
	function rythmPriceOracle() external view returns (address);
