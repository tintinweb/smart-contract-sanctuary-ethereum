...
contract BrandingOrder is Adminer {
  ...
  function verifyOrder(Order calldata order) public pure {
    ...
    require(signatureOwner == info.owner);
  }
  ...
  function fulfillOrder(Order calldata order, uint256 amount) external payable {
    ...
    OrderInfo calldata info = order.info;
    require(info.owner != msg.sender, "Sender should not be the owner");
    require(!checkHasExpired(info), "OrderInfo has expired");
    require(!checkHasCanceled(info), "OrderInfo has canceled");
    require(getRemainAmount(info) >= amount, "Insufficient number of remaining");

    if (info.orderType == 0) {
      ERC1155(info.contractAddress).safeTransferFrom(info.owner, msg.sender, info.tokenId, amount, "Branding NFT trade");

      uint256 totalPrice = amount * info.price;
      require(msg.value >= totalPrice, "Underpayment");

      (bool ownerReceiveSuccess, ) = payable(info.owner).call{ value: (totalPrice / 100) * (100 - _platformFee) }("");
      ...
      ERC1155(info.contractAddress).safeTransferFrom(msg.sender, info.owner, info.tokenId, amount, "Branding NFT trade");

      uint256 totalPrice = amount * info.price;
      uint256 platformReceiveBalance = (totalPrice / 100) * _platformFee;

      weth.transferFrom(info.owner, address(this), platformReceiveBalance);
      weth.transferFrom(info.owner, msg.sender, totalPrice - platformReceiveBalance);
    }

    OrderFulfill memory orderFulfill;
    orderFulfill.id = info.id;
    orderFulfill.owner = msg.sender;
    orderFulfill.time = block.timestamp;
    orderFulfill.amount = amount;
    _fulfillMap[info.id].push(orderFulfill);
  }
