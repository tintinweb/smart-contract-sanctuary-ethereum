...
contract ERC721Factory is Administrated {
     ...
     */
    constructor(address _splitter, address _weth) {
    ...
    function createRedKiteERC721(
        ...
        deployedContracts.push(address(_new));
        emit CreatedERC721(address(_new), _name, _symbol);
    }

    /**
     * @notice Count deployed NFT contracts
     * @return count - Length of the deployedContracts array
     */
    function countDeployedNFT() external view returns (uint256 count) {
    ...
    Administrated
{
    ...
    event SaleStatus(bool started, uint256 price);

    /// @dev When signer changed
    event SignerChanged(address indexed oldSigner, address indexed newSigner);

    /// @dev When user bought NFT on presale
    event PresaleBuy(uint256 indexed tokenId, address indexed buyer);
    ...
    /**
     * @notice Buy with whitelist for ETH
    ...
    function presaleBuy(
        ...
        _tokenId = _buy(_signature, _uuid, _tokenURI, presalePrice);

        emit PresaleBuy(_tokenId, msg.sender);
    }

    /**
     * @notice Buy many NFTs with whitelist for ETH
     * @param _signatures Signature hashes
     * @param _uuids Universal Unique Identifiers
     * @param _tokenURIs Token URI Strings
     */
    function presaleBuyBatches(
        bytes[] calldata _signatures,
        bytes[] calldata _uuids,
        string[] memory _tokenURIs
    ) external payable returns (uint256 _tokenId) {
        require(presaleStarted, "presale is not started");
        require(_signatures.length == _uuids.length, "arrays have no equal length");
        require(_signatures.length == _tokenURIs.length, "arrays have no equal length");
        require(_signatures.length > 0, "empty arrays");

        require(_signatures.length * presalePrice == msg.value, "incorrect price");
        
        for (uint256 _i; _i < _signatures.length; _i++) {
            _tokenId = _buy(_signatures[_i], _uuids[_i], _tokenURIs[_i], presalePrice);
            emit PresaleBuy(_tokenId, msg.sender);
        }
    }

    /**
     * @notice Public buy with ETH
     * @param _signature Signature hash
     * @param _uuid Universal Unique Identifier
     * @param _tokenURI Token URI String
     */
    function publicBuy(
        ...
        _tokenId = _buy(_signature, _uuid, _tokenURI, price);

        emit Buy(_tokenId, msg.sender);
    }

    /**
     * @notice Public buy many NFTs with whitelist for ETH
     * @param _signatures Signature hashes
     * @param _uuids Universal Unique Identifiers
     * @param _tokenURIs Token URI Strings
     */
    function publicBuyBatches(
        bytes[] calldata _signatures,
        bytes[] calldata _uuids,
        string[] memory _tokenURIs
    ) external payable returns (uint256 _tokenId) {
        ...
        require(_signatures.length * price == msg.value, "incorrect price");
        
        for (uint256 _i; _i < _signatures.length; _i++) {
            _tokenId = _buy(_signatures[_i], _uuids[_i], _tokenURIs[_i], price);
            emit Buy(_tokenId, msg.sender);
        }
    }
    
    /**
     * @notice Buy with whitelist for ETH (internal)
     * @param _signature Signature hash
     * @param _uuid Universal Unique Identifier
     * @param _tokenURI Token URI String
    ...
    function presaleBuyWETH(
        ...
        _tokenId = _buyWETH(_signature, _uuid, _tokenURI, presalePrice);
    ...
    function publicBuyWETH(
        ...
        _tokenId = _buyWETH(_signature, _uuid, _tokenURI, price);

        emit Buy(_tokenId, msg.sender);
    }

    /**
     * @notice Presale buy many tokens with WETH
     * @param _signatures Signature hashes
     * @param _uuids Universal Unique Identifiers
     * @param _tokenURIs Token URI Strings
     */
    function presaleBuyBatchesWETH(
        bytes[] calldata _signatures,
        bytes[] calldata _uuids,
        string[] memory _tokenURIs
        ...
        for (uint256 _i = 0; _i < _signatures.length; _i++) {
            _tokenIds[_i] = _buyWETH(_signatures[_i], _uuids[_i], _tokenURIs[_i], presalePrice);
            emit PresaleBuy(_tokenIds[_i], msg.sender);            
        }
    }

    /**
     * @notice Public buy many tokens with WETH
     * @param _signatures Signature hashes
     * @param _uuids Universal Unique Identifiers
     * @param _tokenURIs Token URI Strings
     */
    function publicBuyBatchesWETH(
        bytes[] calldata _signatures,
        bytes[] calldata _uuids,
        string[] memory _tokenURIs
    ) external returns (uint256[] memory _tokenIds) {
        ...
        for (uint256 _i = 0; _i < _signatures.length; _i++) {
            _tokenIds[_i] = _buyWETH(_signatures[_i], _uuids[_i], _tokenURIs[_i], price);
            emit Buy(_tokenIds[_i], msg.sender);            
        }
    }
    
    /**
     * @notice Buy with whitelist for WETH (internal)
     * @param _signature Signature hash
     * @param _uuid Universal Unique Identifier
     * @param _tokenURI Token URI String
     * @param _price Token price
    ...
    function _buyWETH(
        ...
        // Splitting
        if (_price > 0) {
            IERC20(weth).safeTransferFrom(msg.sender, splitter, _price);
            ISplitterContract(splitter).primaryDistribution(artist, _price);
        }
    ...
    /**
     * @dev Burn token to reduce total supply
    ...
    function setSaleStatus(bool _started, uint256 _price) external onlyAdmin {
        ...
        price = _price;
        emit SaleStatus(_started, _price / 1e9);
    }
        ...
        onlyAdmin
    {
        ...
        emit PresaleStatus(_started, _presalePrice / 1e9);
    }
