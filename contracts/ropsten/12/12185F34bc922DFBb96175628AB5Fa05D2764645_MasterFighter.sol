...
contract MasterFighter is Ownable, Statable {
    ...
    event ReceivedMoney(uint256 _stake);
    event Debug(uint256 _stake, bool withdrawn);

    function deposit() public payable requireState(1) {
        emit ReceivedMoney(msg.value);
    }
    ...
    function refund(address _betterAddress) external requireState(3) {
        uint256 stake = 0;
        for (uint8 i = 0; i < fighterAddressess.length; i++) {
            emit Debug(bets[fighterAddressess[i]][_betterAddress].stake, bets[fighterAddressess[i]][_betterAddress].withdrawn);
