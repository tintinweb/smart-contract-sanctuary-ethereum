...
contract DOGPool is IDOGPool, DepositNFTEnumerable, DeposiNFTTimeEnumerable,DeposiDaiTimeEnumerable,Ownable{
    ...
    function findOwnerNFTTokens(address owner) 
        ...
        IDOGERC721 erc721 = IDOGERC721(nftAddress);
        for(uint i=0;i<tokenIndex;i++){
            _tokens[i] = _depositTokenOfOwnerByIndex(owner,i);
            _URIs[i] = erc721.tokenURI(_tokens[i]);
            starTime = _depositTokenTime(owner, _tokens[i]);
            _amounts[i] = CalProfitMath.calStepProfitAmount(blockStartTime,starTime,endTime,DAY_PROFIT); 
        }
