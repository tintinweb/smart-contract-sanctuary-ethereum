...
contract NFTMarket is AccessRole{
    ...
    event BUY(address user, uint256 ids);
    ...
    function buy(uint256 _quantity) external {
        ...
        require( (userLimit[user] + _quantity) <= maxBuyLimit, "More than max limit");

        MEGA.safeTransferFrom(user, DEAD, (_quantity*price));
        burnedMega = burnedMega + (_quantity*price);
        uint256 id;
        for(uint256 i=1; i<=_quantity; i++){
            id = IHeroNFT(HERONFT).mint(user);
            emit BUY(user, id);
        }
    }
