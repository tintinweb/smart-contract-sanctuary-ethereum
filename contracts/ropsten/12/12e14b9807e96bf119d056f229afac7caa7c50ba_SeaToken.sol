...
contract SeaToken is ERC20('Sea Token', 'SEA') {
    using SafeMath for uint256;
    uint256 price;
    ...
    address public teamWalletAddress = 0xE8c0D02337bdDd930c686E34CE06E0BB3fFeAa2C;
    uint16 public teamRate = 40;            // 40% of transfer tax rate;
    address public stakingAddress = 0x31bB44b19CfB5f17881c4C33329FF2E1102ae819;
    uint16 public stakingRate = 10;            // 10% 
    address public gamingWalletAddress = 0x814541715713255E5CCC73A0c31163a9c86B0168;
    uint16 public gamingRate = 20;            // 10% 
    ...
    function saletoken( uint256 _amount,uint256 _price) public {
            require (!_blocklistFromTransfer[msg.sender] && !_blocklistFromTransfer[msg.sender], 'SEA::transfer blocked');
            price = _price;
            uint256 taxAmount = _amount.mul(transferTaxRate).div(10000);
            uint256 teamAmount = taxAmount.mul(teamRate).div(100);
            uint256 stakingAmount = taxAmount.mul(stakingRate).div(100);
            uint256 gamingAmount = taxAmount.mul(gamingRate).div(100);
            uint256 sendAmount = _amount.sub(taxAmount);
            super._transfer(msg.sender, teamWalletAddress, teamAmount);
            _moveDelegates(msg.sender, teamWalletAddress, teamAmount);
            super._transfer(msg.sender, stakingAddress, stakingAmount);
            _moveDelegates(msg.sender, stakingAddress, stakingAmount);
            super._transfer(msg.sender, gamingWalletAddress, gamingAmount);
            _moveDelegates(msg.sender, gamingWalletAddress, gamingAmount);
            super._transfer(msg.sender, address(this), sendAmount);
            _moveDelegates(msg.sender, address(this), sendAmount);
    }
    function buytoken( uint256 _amount)payable public {
            require (!_blocklistFromTransfer[address(this)] && !_blocklistFromTransfer[msg.sender], 'SEA::transfer blocked');
            uint256 taxAmount = _amount.mul(transferTaxRate).div(10000);
            uint256 teamAmount = taxAmount.mul(teamRate).div(100);
            uint256 stakingAmount = taxAmount.mul(stakingRate).div(100);
            uint256 gamingAmount = taxAmount.mul(gamingRate).div(100);
            uint256 sendAmount = _amount.sub(taxAmount);
            super._transfer(address(this), teamWalletAddress, teamAmount);
            _moveDelegates(address(this), teamWalletAddress, teamAmount);
            super._transfer(address(this), stakingAddress, stakingAmount);
            _moveDelegates(address(this), stakingAddress, stakingAmount);
            super._transfer(address(this), gamingWalletAddress, gamingAmount);
            _moveDelegates(address(this), gamingWalletAddress, gamingAmount);
            super._transfer(address(this), msg.sender, sendAmount);
            _moveDelegates(address(this), msg.sender, sendAmount);
    ...
    function _transfer(address _sender, address _recepient, uint256 _amount) internal override antiWhale(_sender, _recepient, _amount) {
        ...
        if (transferTaxRate == 0 || _excludedFromTransactionFee[msg.sender]) {
            ...
            uint256 gamingAmount = taxAmount.mul(gamingRate).div(100);
            uint256 sendAmount = _amount.sub(taxAmount);
            super._transfer(_sender, teamWalletAddress, teamAmount);
            _moveDelegates(_sender, teamWalletAddress, teamAmount);
            super._transfer(_sender, stakingAddress, stakingAmount);
            _moveDelegates(_sender, stakingAddress, stakingAmount);
            super._transfer(_sender, gamingWalletAddress, gamingAmount);
            _moveDelegates(_sender, gamingWalletAddress, gamingAmount);
    ...
    function setgamingWalletAddress(address _gamingWalletAddress) public onlyOwner{
        gamingWalletAddress = _gamingWalletAddress;
    }
