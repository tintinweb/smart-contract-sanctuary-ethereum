...
contract Todo {    
    ...
    mapping (address => uint) internal doneTasks;
    ...
    function completeTask (string memory task_name) public onlyOwner(task_name) isNotTimeOut(task_name) isNotCompleted(task_name) {
        AllTasks[indexOfTask[task_name]].completed = true;
        doneTasks[msg.sender] += 1;
    }

    function showProductivity (address user_address) public view returns (uint productivity) {
        productivity = ((doneTasks[user_address] * 100) / undoneTasks[user_address]);
    }

    function showTask (string memory task_name) public view returns (uint hours_todo, string memory description, address owner, bool completed) {
