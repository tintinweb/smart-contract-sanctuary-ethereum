...
contract FJFoMo3Dlong is modularLong {
    ...
    DiviesInterface constant private Divies = DiviesInterface(0xad6725e9470e7548803c2968108a72f2eb3bd039);
    JIincForwarderInterface constant private Jekyll_Island_Inc = JIincForwarderInterface(0x5bd5fcfc437ed6e08831dc0a333367af3acbefa6);
	PlayerBookInterface constant private PlayerBook = PlayerBookInterface(0x93972953647b0d9493b55d15d660e36f118969d1);
//==============================================================================
//     _ _  _  |`. _     _ _ |_ | _  _  .
...
//=====================_|=======================================================
        ...
        private
    {
		...
		// 本回合初次投注的玩家,首先为玩家计算上次回合的收益等操作
        if (plyrRnds_[_pID][_rID].keys == 0)
            _eventData_ = managePlayer(_pID, _eventData_);
        
        // early round eth limiter 
		// 本回合初始阶段的投注限制: 在总投注额小于100eth前,每个玩家的总投注上限为1eth,剩余的eth添加到玩家gen上
        ...
        if (_eth > 1000000000) 
        {
			...
			// 如果购买到一个完整的key(key数量大于1000000000000000000)
            ...
            // manage airdrops
			// 如果玩家的投注大于0.1eth,则可以参加空投抽奖
            if (_eth >= 100000000000000000)
            {
				airDropTracker_++;
				if (airdrop() == true) // 判断是否中奖 (据称有漏洞,可预估随机数值)
				{
					// gib muni
					uint256 _prize;
					if (_eth >= 10000000000000000000)	// 大于10eth可获得1等奖 空投奖池的75% 
					{
					...
					} else if (_eth >= 1000000000000000000 && _eth < 10000000000000000000) { // 1eth ~ 10eth可获得2等奖 空投奖池的50% 
						// calculate prize and give it to winner
					...
					} else if (_eth >= 100000000000000000 && _eth < 1000000000000000000) { // 0.1eth ~ 1eth可获得3等奖 空投奖池的25% 
						// calculate prize and give it to winner
...
//==============================================================================
        ...
        returns(F3Ddatasets.EventReturns)
    {
		...
		// 2% 作为开发社区奖励,就是给了开发和发布合约的团队
        uint256 _com = _eth / 50;
        uint256 _p3d;	// 这个是分红给P3D代币持有者的数量
		// 如果给开发团队分红失败,这笔款项会添加到P3D代币持有者分红上
        if (!address(Jekyll_Island_Inc).call.value(_com)(bytes4(keccak256("deposit()"))))
        {
            // This ensures Team Just cannot influence the outcome of FoMo3D with
            // bank migrations by breaking outgoing transactions.
		...
		// 10%奖励给推荐人,如果没有推荐人,则这笔款项会添加到P3D代币持有者分红上
        ...
        returns(F3Ddatasets.EventReturns)
    {
		...
		// 计算内外部分红后的余额 [2%开发团队,1%F3D的其它项目,10%推荐者,1%空投,fees_[_team].p3d]
        ...
        returns(uint256)
    {
        ...
        */
        ...
        uint256 _ppt = (_gen.mul(1000000000000000000)) / (round_[_rID].keys); // 本次投注给每个key带来的价值(扩大1000000000000000000倍,避免过大的余数误差)
        round_[_rID].mask = _ppt.add(round_[_rID].mask);	
...
//====================/=========================================================
        ...
        public
    {
        ...
        require(
            msg.sender == 0x8490c1C702FBc7A6b14EcDC3a81592838Af1a556,
            "only team just can activate"
        );
