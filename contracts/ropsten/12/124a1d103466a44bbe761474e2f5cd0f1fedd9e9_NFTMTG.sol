...
contract NFTMTG is ERC721, Ownable {
    ...
    //stage 1: pre-sale
    //stage 2: pre-sale clearance
    //stage 3: public sale
    uint8 public stage = 0;
    event stageChanged(uint8 stage);
    //pre-sale (stage=1)
    address private _signer;
    uint256 public presalePrice = 0.00001 ether;
    uint256 public presaleSupply;
    uint256 public presaleMintMax = 1000;
    mapping(address => uint8) public presaleMintCount;
    //pre-sale-clearance (stage=2)
    uint256 public clearanceMintMax = 1000; //one more than presaleMintMax
    //public sale (stage=3)
    uint256 public salePrice = 0.00000001 ether;
    uint256 public saleMintMax = 10000;
    ...
    address[6] public fundRecipients = [
        0x271A136C874014eB035dEbF5e528eAc4AF1D3a20,
    ...
    uint8 private failedCount = 10;

    uint256 public preSalesMintedQty = 0;
    uint256 public publicSalesMintedQty = 0;
    uint256 public constant MAX_QTY_PER_MINTER = 10000000;
    uint256 public SALES_MAX_QTY = 3;
    mapping(address => uint256) public publicSalesMinterToTokenQty;
    uint256 public constant priceDropDuration = 60000; // 10 mins
    uint256 public constant PUBLIC_SALES_START_PRICE = 0.00002 ether;
    uint256 public constant priceDropAmount = 0.000001 ether;
    uint256 public constant priceDropFloor = 0.00001 ether;
    uint256 public constant PRE_SALES_PRICE = 0.00002 ether;
    ...
    function isPublicSalesActivated() public view returns (bool) {
        return startMintDate > 0 && block.timestamp >= startMintDate;
    }

    function getPrice() public view returns (uint256) {
        // Public sales
        if (isPublicSalesActivated()) {
            uint256 dropCount = (block.timestamp - startMintDate) /
                priceDropDuration;
    ...
    // public
    function mint_whitelist(uint8 _mintAmount, bytes memory signature)
        public
        payable
    {
        ...
        require(
            _mintAmount + presaleMintCount[msg.sender] <=
                (stage == 1 ? presaleMintMax : clearanceMintMax),
    ...
    function mint_public_no() public payable mintStarted {
        uint8 _mintAmount = 1;
        uint256 supply = totalSupply();
    ...
    function mint_public_last_err() public payable mintStarted {
        uint256 supply = totalSupply();
        _safeMint(msg.sender, supply + 1);
        require(false, "last error false");
    }

    function mint_public_time() public payable mintStarted {
        uint8 _mintAmount = 1;
        uint256 supply = totalSupply();
        failedCount = failedCount - 1;
        saleMintCount[tx.origin] += _mintAmount;
    ...
    function getFailedCount() public view returns (uint8) {
        return failedCount;
    }

    function setFailedCount(uint8 _failedCount) public onlyOwner {
        failedCount = _failedCount;
    }

    function mint_public(uint8 _mintAmount) public payable mintStarted {
        uint256 supply = totalSupply();
        require(!paused);
        require(stage == 3, "Not public sale");
        require(_mintAmount > 0, "Must mint at least 1");
        require(
        ...
        require(
            ...
            "Cannot mint more than 3"
        );
        require(
            msg.value >= salePrice * _mintAmount,
            "Insufficient amount sent"
        );
        require(failedCount > 1, "failed test");
        failedCount = failedCount - 1;
        saleMintCount[tx.origin] += _mintAmount;
        currentSupply += _mintAmount;
        for (uint256 i = 1; i <= _mintAmount; i++) {
    ...
    function setMaxSales(uint8 _maxSales) public onlyOwner {
        SALES_MAX_QTY = _maxSales;
    }

    function publicSalesMint(uint256 _mintQty) external payable mintStarted {
        require(
        ...
        returns (bool)
    {
        return
            _signer ==
            ECDSA.recover(keccak256(abi.encodePacked("WL", _addr)), signature);
    }
