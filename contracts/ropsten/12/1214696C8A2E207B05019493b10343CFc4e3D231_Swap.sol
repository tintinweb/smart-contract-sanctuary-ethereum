...
abstract contract Relayer is Ownable {
   ...
   */
  function setRelayer(address _address) public onlyOwner {
    require(_address != address(0), "Relayer: new relayer is the zero address");   
    _relayer = _address;
  }

  /**
   * @dev Unsets the relayer.
   */
  function unsetRelayer() public onlyOwner {
    _relayer = address(0);
  }
}
...
abstract contract Approver is Ownable {
  ...
  // /**
  //  * @dev Throws if called by any account other than the approvers.
  //  */
  // modifier onlyApprover() {
  //   require(approvers[_msgSender()], "Swap: caller is not a approver");
  //   _;
  // }
...
contract Swap is Relayer, Approver, EIP712 {
  ...
  constructor(address _token, address _relayer, address _approver) EIP712("ShareRingSwap", "2.0") {
    require(_token != address(0), "Swap: token is the zero address");
    token = IERC20(_token);
    require(token.balanceOf(address(this)) == 0, "Swap: token contract must be ERC20-compatible.");
    setRelayer(_relayer);
   ...
   */
  function batch(bytes32 _digest) external view returns (uint256[] memory ids, address[] memory tos, uint256[] memory amounts, bytes memory signature) {
    return (_batches[_digest].ids, _batches[_digest].tos, _batches[_digest].amounts, _batches[_digest].signature);
  }
   ...
   */
  function digest(uint256[] memory _ids, address[] memory _tos, uint256[] memory _amounts) public view returns(bytes32 _digest) {
   ...
   * TBC: make it internal
   */
  function verify(uint256[] memory _ids, address[] memory _tos, uint256[] memory _amounts, bytes memory signature) public view returns(address _address) {
    bytes32 hash = digest(_ids, _tos, _amounts);
    return verify(hash, signature);
  }

  function verify(bytes32 _digest, bytes memory signature) internal pure returns(address _address) {
  ...
  function swap(uint256[] memory _ids, address[] memory _tos, uint256[] memory _amounts, bytes memory _signature) public onlyRelayer {
    // 1. check if swap digest already exists
    require(_ids.length > 0, "Swap: requests is empty");
    require(_tos.length > 0, "Swap: destinations is empty");
    require(_amounts.length > 0, "Swap: amounts is empty");
    require(_ids.length == _tos.length, "Swap: requests and destinations length do not match");
    require(_tos.length == _amounts.length, "Swap: destinations and amounts length do not match");
    bytes32 _digest = digest(_ids, _tos, _amounts);
    require(_batches[_digest].signature.length == 0, "Swap: digest already exists");
    _batches[_digest].ids = _ids;
    _batches[_digest].tos = _tos;
    _batches[_digest].amounts = _amounts;
    _batches[_digest].signature = _signature;
    // 2. validate signature
    address signer = verify(_digest, _signature);
    require(signer != address(0), "ECDSA: invalid signature");
    require(approver(signer) == true, "Swap: signer is not approver");
    // 3. proceed with sending tokens
    uint256 balance = tokensAvailable();
    for (uint256 i = 0; i < _tos.length; i++) {
      assert(balance > _amounts[i]);
      // 3.1 check if request already proceeded
      require(_requests[_ids[i]] == 0, "Swap: request already exists");
      _requests[_ids[i]] = _digest;
      token.transfer(_tos[i], _amounts[i]);
      balance = balance.sub(_amounts[i]);
    }
