...
// File contracts/DKYCNFTStaking.sol
...
contract DKYCNFTStaking is ERC721Holder, Ownable  {
    ...
    function add(uint256 _allocPoint, IERC20 _rewardsToken, uint256 _nftLevel, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint256 _nftLevel, bool _withUpdate) external onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function joinPool(uint256 _pid, uint256 _tokenId) external {
        ...
        require(DKYCNFTToken.nftLevel(_tokenId) == pool.nftLevel, "level mismatch");
        ...
        DKYCNFTToken.safeTransferFrom(msg.sender, address(this), _tokenId);
    ...
    function joinPoolBatch(uint256 _pid,  uint256[] memory _tokenIds) external {
        require (_tokenIds.length > 0, "length mismatch");
        ...
        for (uint i = 0 ; i < _tokenIds.length; i++) {
            ...
            require(DKYCNFTToken.nftLevel(_id) == pool.nftLevel, "nftLevel mismatch");
            DKYCNFTToken.safeTransferFrom(msg.sender, address(this), _id);
    ...
    // Leave NFT items.
    function leavePool(uint256 _pid, uint256 _id) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        DKYCNFTToken.safeTransferFrom(address(this), msg.sender, _id);
