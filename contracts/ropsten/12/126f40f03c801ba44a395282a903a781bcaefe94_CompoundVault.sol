...
// File: contracts/CompoundVault2.sol
...
contract CompoundVault is Ownable {
        ...
        if (block.timestamp > lastRewardTime) {
            ...
            if (stakedEXPO > 0) {
                ...
                if (address(this).balance + ethToCompound >= threshold) {
                    ...
                    uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: threshold}(
                        0,
                        path,
                        address(this),
                        block.timestamp
                    );
                    uint256 expoReward = expo.balanceOf(address(this)) - before;
    ...
    function withdraw(uint256 _amount) public {
        ...
        _update();
        uint256 pending = (user.amount * accExpoPerShare / 1e18) - user.rewardDebt;
        user.amount -= _amount;
        totalDeposited -= _amount;
        user.rewardDebt = user.amount * accExpoPerShare / 1e18;
        safeExpoTransfer(msg.sender, _amount + pending);
    ...
    function emergencyWithdraw() public {
        ...
        user.rewardDebt = 0;
        safeExpoTransfer(address(msg.sender), amount);
    ...
    function update() public {
        ...
        if (stakedEXPO > 0) {
            ...
            uint256 expoBefore = expo.balanceOf(address(this));
            ...
            uint256 expoReward = expo.balanceOf(address(this)) - expoBefore;
            totalETHCompounded += ethBefore;
            totalCompoundedExpo += expoReward;
            accExpoPerShare += (expoReward * 1e18) / stakedEXPO;
        }
    }
    ...
    function getPlatformAPY() public view returns (uint256) {
        return (((totalCompoundedExpo * 365 days * 10000) / (totalDeposited - totalCompoundedExpo)) / (lastRewardTime - startTime));
    }

    function getUserAPY(address user) public view returns (uint256) {
        UserInfo memory _user = userInfo[user];
        return (((_user.compoundedExpo * 365 days * 10000) / (_user.amount - _user.compoundedExpo)) / (lastRewardTime - _user.startTime));
    }
