...
// File: IWhitelistAtomicOracle.sol
...
/**
 * @title A Basic Whitelist Atomic Oracle
 * @author Ville Sundell <[email protected]>
 * @dev An Atomic Oracle curating access to Status Storage by whitelisting
 * Ethereum addresses (Client Smart Contracts) for a certain amount of time.
 *
 * Access control is implemented by OpenZeppelin's {AccessControl} contract.
 * It allows us to specify different accounts for different administrative
 * actions (if desired). By default, `admin` is set for every role as a Role
 * Admin.
 *
 * There is only one administrative action:
 *  - (un)set whitelisting for an Ethereum address, including expiration
 *    (`WHITELIST_ROLE`).
 *
 * Target address is intentionally implemented as `string calldata`:
 *  - `target` is not meant to be manipulated, only relayed from Oracle
 *    Contracts to the Status Storage, and
 *  - `calldata` saves gas when handling arrays such as `string`.
 */
interface IWhitelistAtomicOracle is IBaseAtomicOracle {
    /**
     * @dev Set whitelisting status, and expiration for an Ethereum account.
     *
     * The Ethereum account in question is whitelisted if the call is made
     * before the `expiration`.
     *
     * @param addr The Ethereum account to whitelist
     * @param expiration Unix epoch timestamp when whitelisting will expire, 0
     * to disable whitelisting for this account
     */
    function setWhitelist(address addr, uint256 expiration) external;

    /**
     * @dev Get whitelisting status for an Ethereum account.
     *
     * @param addr The Ethereum account whose whitelisting will be queried
     * @return expiration The Unix epoch timestamp when whitelisting expires,
     * 0 to disable whitelisting for this account
     */
    function getWhitelist(address addr) external view returns (uint256 expiration);

    /**
     * @dev Caller's whitelisting status will be verified here, and then
     * the `target` is relayed to {BaseAtomicOracle-_getStatus}, and its
     * return value will be returned to the caller.
     */
    function getStatus(string calldata target) external view returns (bytes32 status);

    /**
     * @dev Emitted when an account's whitelisting status changes
     *
     * @param addr Whose whitelisting status changed
     * @param expiration When whitelisting expires, can be in the past, such
     * as 0
     */
    event Whitelisted(address addr, uint256 expiration);
}
...
// File: WhitelistAtomicOracle.sol
 ...
 *  - `calldata` saves gas when handling arrays such as `string`.
 */
contract WhitelistAtomicOracle is AccessControl, BaseAtomicOracle, IWhitelistAtomicOracle {
     ...
     * later.
     */
    constructor(address admin, IStatusStorage statusStorage) BaseAtomicOracle(admin, statusStorage) {
        _setupRole(DEFAULT_ADMIN_ROLE, admin);
        _setupRole(WHITELIST_ROLE, admin);
    }

    /**
     * @dev See {IWhitelistAtomicOracle-setWhitelist}.
     */
    function setWhitelist(address addr, uint256 expiration) external override {
        require(hasRole(WHITELIST_ROLE, msg.sender), "WhitelistAtomicOracle: the caller is not allowed to whitelist addresses");

        _whitelist[addr] = expiration;

        emit Whitelisted(addr, expiration);
    }

    /**
     * @dev See {IWhitelistAtomicOracle-getWhitelist}.
     */
    function getWhitelist(address addr) external view override returns (uint256 expiration) {
        return _whitelist[addr];
    }

    /**
     * @dev See {IWhitelistAtomicOracle-getStatus}.
     */
    function getStatus(string calldata target) external view override returns (bytes32 status) {
        require(_whitelist[msg.sender] > block.timestamp, "WhitelistAtomicOracle: you are not whitelisted");
