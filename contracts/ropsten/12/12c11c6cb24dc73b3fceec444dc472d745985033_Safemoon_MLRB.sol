...
contract Safemoon_MLRB is ISafemoon, Context, Auth {
    ...
    modifier preventBlocklisted(address _account, string memory errorMsg) {
        require(!_isBlocklisted[_account], errorMsg);
        _;
    }
    ...
    constructor() Auth(payable(_msgSender())) {
        __Safemoon_v2_init_unchained();
    }

    function __Safemoon_v2_init_unchained() internal {
        _name = "SafeMoon";
        _symbol = "SFM";
        _decimals = 9;
    ...
    function __Safemoon_tiers_init() internal {
        _defaultFees = _addTier(0, 500, 500, 0, address(0));
        _addTier(50, 50, 100, 0, address(0));
        _addTier(50, 50, 100, 100, address(0));
        _addTier(125, 125, 125, 125, address(0));
    }
    ...
    preventBlocklisted(_account, "Safemoon: Selected account is in blocklist")
    {
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        uint256 _fees = _tier.marketingFee.add(_tier.liquidityFee).add(_tier.taxFee).add(_tier.burnFee);
    ...
    function checkFeesChanged(FeeTier memory _tier, uint256 _oldFee, uint256 _newFee) internal view {
        uint256 _fees = _tier.marketingFee
    ...
    function setMarketingFeeAddress(uint256 _tierIndex, address _marketing) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_marketing != address(0), "Safemoon: Address Zero is not allowed");
    ...
    function blocklistAddress(address account) public onlyOwner() {
    ...
    function _getValues(uint256 tAmount, uint256 _tierIndex) private view returns (FeeValues memory) {
        ...
        uint256 tTransferFee = tValues.tLiquidity.add(tValues.tMarketing).add(tValues.tBurn);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tValues.tFee, tTransferFee, _getRate());
        return FeeValues(rAmount, rTransferAmount, rFee, tValues.tTransferAmount, tValues.tMarketing, tValues.tLiquidity, tValues.tFee, tValues.tBurn);
    }
    ...
    function _getTValues(uint256 tAmount, uint256 _tierIndex) private view returns (tFeeValues memory) {
        ...
        tValues.tTransferAmount = tAmount.sub(tValues.tMarketing).sub(tValues.tFee).sub(tValues.tLiquidity).sub(tValues.tBurn);
        return tValues;
    }
    ...
    private
    preventBlocklisted(owner, "Safemoon: Owner address is blocklisted")
    preventBlocklisted(spender, "Safemoon: Spender address is blocklisted")
    {
    ...
    private
    preventBlocklisted(_msgSender(), "Safemoon: Address is blocklisted")
    preventBlocklisted(from, "Safemoon: From address is blocklisted")
    preventBlocklisted(to, "Safemoon: To address is blocklisted")
    isRouter(_msgSender())
    {
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        // split the contract balance into halves
            ...
            ISafemoon(address(this)).transferFrom(msg.sender,address(this),swappedToken);
    ...
    function swapBnbForTokens(uint256 bnbAmount) private {
        ...
        path[1] = address(this);
        _approve(_owner(), address(uniswapV2Router), bnbAmount);
        // make the swap
    ...
    function _takeFees(address sender, FeeValues memory values, uint256 tierIndex) private {
        ...
        _takeFee(sender, values.tMarketing, feeTiers[tierIndex].marketing);
