...
contract RaffleContract is Ownable {
    ...
    function createRaffle(
        ...
        require(IERC20(USDC).allowance(owner(), address(this)) >= rafflePrize_);
    ...
    function purchaseTicket(uint256 id, uint256 quantity) public {
        require(id <= _raffleId.current() && id > 0);
        Raffle storage raffle = raffles[id];
        require(quantity > 0 && quantity <= raffle.totalTickets);
        require(totalTicketSold[id] + quantity <= raffle.totalTickets);
        uint256 totalPrice = quantity * raffle.price;
        require(
            IERC20(USDC).allowance(msg.sender, address(this)) >= totalPrice
        );
        require(IERC20(USDC).balanceOf(msg.sender) >= totalPrice);
