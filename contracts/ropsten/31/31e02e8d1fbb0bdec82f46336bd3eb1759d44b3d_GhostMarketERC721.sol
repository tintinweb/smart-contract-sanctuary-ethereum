    ...
    Mint721Validator
{
     ...
     */
    bytes4 constant _INTERFACE_ID_ERC721_GHOSTMARKET =
        bytes4(keccak256("_INTERFACE_ID_ERC721_GHOSTMARKET"));

    /**
     * bytes4(keccak256(_GHOSTMARKET_NFT_ROYALTIES)) == 0xe42093a6
     */
    bytes4 constant _GHOSTMARKET_NFT_ROYALTIES =
        bytes4(keccak256("_GHOSTMARKET_NFT_ROYALTIES"));
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < royalties.length; i++) {
            require(
                royalties[i].recipient != address(0x0),
                "Recipient should be present"
            );
            require(
                royalties[i].value > 0,
                "Royalties value should be positive"
            );
            require(
                royalties[i].value <= 5000,
    ...
    function _checkMintFees() internal {
        if (_ghostmarketMintFees > 0) {
            require(
                msg.value == _ghostmarketMintFees,
                "Wrong fees value sent to GhostMarket for mint fees"
            );
        }
        if (msg.value > 0) {
    ...
    function mintGhostWithSignature(        
        LibERC721LazyMint.Mint721Data memory lazyMintData,
        address to,
        Royalty[] memory royalties,
        string memory externalURI,
        string memory metadata,
        string memory lockedcontent) external payable {
        if (lazyMintData.minter != _msgSender()) {
            validate(lazyMintData.minter, LibERC721LazyMint.hash(lazyMintData), lazyMintData.signature);
        }
        this.mintGhost(to, royalties, externalURI, metadata, lockedcontent);
    }
    ...
    function mintGhost(
        ...
        require(
            keccak256(abi.encodePacked(externalURI)) !=
    ...
    function setGhostmarketMintFee(uint256 gmmf) external {
        require(
            ...
            "Caller must have admin role to set mint fees"
        );
...
library LibERC721LazyMint {
    ...
    bytes32 public constant MINT_AND_TRANSFER_TYPEHASH = keccak256("Mint721(uint256 tokenId,string tokenURI,address minter)");
