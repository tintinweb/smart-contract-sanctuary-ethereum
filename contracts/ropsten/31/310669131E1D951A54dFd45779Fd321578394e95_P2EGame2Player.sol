...
/**
 * 2 Players can play game and admin will operate game
 */
contract P2EGame2Player is Ownable {
    // admin address
    ...
    uint256 public maxSupply = 10**20; // <-- temporaily set manually for flexibility while in pre-alpha development
    uint256 public unit = 10**18; // <-- temporaily set manually for flexibility while in pre-alpha development
    uint256 public gameId;
    uint8 public fee = 5; // 5%
    ...
    event NewGame(uint256 id, address indexed player1, address indexed player2);
    event EndGame(uint256 id, address indexed winner);

    // only admin account can operate game(maybe bot)
    modifier onlyAdmin() {
    ...
    function setAdmin(address _admin) external onlyOwner returns (bool) {
        require(_admin != address(0), "P2EGame2Player: admin address cannot be zero address");
        admin = _admin;
        return true;
    }
        ...
        )
    {
        return (
            ...
            balances[_gameId].treasury,
            balances[_gameId].player1,
            balances[_gameId].player2
        );
    }
    ...
    function createGame(
        ...
        require(_p >= unit, "P2EGame2Player: must insert 1 whole token");
        // transfer from player to the contract's address to be locked in escrow
        token.transferFrom(_player1, address(this), _p);
        token.transferFrom(_player2, address(this), _p);

        // full escrow balance
        contractBalance += (_p + _p);

        // iterate game identifier
        gameId++;

        // init game data
        balances[gameId].balance = (_p + _p);
        balances[gameId].treasury = address(this);
    ...
    function playerWon(uint256 _gameId, address _winner)
        external
        onlyAdmin
        returns (bool)
    {
        ...
        require(
            ...
            "P2EGame2Player: winnings exceed balance in escrow"
        );
        // final winnings = balance locked in escrow + in-game winnings
        // transfer to player the final winnings
        require(
            _winner == balances[_gameId].player1 || _winner == balances[_gameId].player2,
            "P2EGame2Player: It seems winnner is invalid address"
        );
        token.transfer(_winner, balances[_gameId].balance * (100 - fee) / 100);
        token.transfer(admin, balances[_gameId].balance * (fee / 2) / 100);
        // TODO: add post-transfer funcs to `_afterTokenTransfer` to validate transfer

        // amend escrow balance
        contractBalance -= balances[_gameId].balance;
        // set game balance to spent
        balances[_gameId].spent = true;

        emit EndGame(gameId, _winner);

        return true;
    }

    // Admin is able to withdraw
    function withdrawByAdmin() external onlyOwner returns (bool) {
        GameToken token = GameToken(tokenAddress);
