...
contract Dvd is ERC20, DvdShareholderPoint, Ownable {
    ...
    function increaseShareholderPoint(address account, uint256 amount)
        external
        onlyController
    {
    ...
    function decreaseShareholderPoint(address account, uint256 amount)
        external
        onlyController
    {
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            ...
            _decreaseShareholderPoint(
                from,
                Math.min(amount, shareholderPointOf(from))
            );
        }
