...
contract envoContract is ERC721Enumerable, Ownable {
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        if(currentTime > vipDate && currentTime < startDate && _to != contractCreator && msg.sender != contractCreator) {
            require(supply + _mintAmount <= MAX_VIPWA);
            require(balanceOf(msg.sender) <= MAX_VIPWA, "You have reached your maximum allowance of Envos.");
            require(_mintAmount <= MAX_VIPWA, "Maximum of mints per wallet exceeded.");
        }

        if (_to != contractCreator && msg.sender != contractCreator) {
    ...
    // - VIP section -----------------------------------------------------------------------
    // Formerly known as "whitelisted", we feel it is politically correct to call it VIP
