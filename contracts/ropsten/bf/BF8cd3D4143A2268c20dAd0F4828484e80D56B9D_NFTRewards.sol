...
contract NFTRewards is ReentrancyGuard, Ownable, Pausable {
    ...
    struct Staker {
        ...
        uint256 lastStake; // amount of tokens staked last time
        uint256 lastStakeTime; // timestamp of tokens staked last time
        uint256 claimed; // total claimed rewards
        bool staked; // represent either user already staked or not
    }
    ...
    **/
    function calculateRewards() public view returns (uint256 rewards) {
        Staker memory staker = poolStakers[msg.sender];
        uint256 stakedDays;
        ...
        } else if (block.timestamp < staker.lastStakeTime) {
            pendingdifference = BokkyPooBahsDateTimeLibrary.diffDays(
                staker.stakeTime,
                block.timestamp
            );
        ...
        if (pendingdifference > 0) {
            stakedDays = BokkyPooBahsDateTimeLibrary.diffDays(
                staker.stakeTime,
                block.timestamp
            );
        } else {
            amount = staker.amount.sub(staker.lastStake);
        }
        ...
        stakelimits(_amount)
    {
        ...
        if (staker.staked) {
            staker.lastStake += _amount;
            staker.lastStakeTime = standardTime();
        } else {
        ...
        unstakelimits(_amount)
    {
        ...
        staker.rewards = calculateRewards();
