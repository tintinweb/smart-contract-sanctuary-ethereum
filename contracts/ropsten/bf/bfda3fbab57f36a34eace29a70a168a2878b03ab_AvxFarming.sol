...
contract AvxFarming is Ownable {
  string public name = "Avx Farm";
  uint public apy;
  ...
  function getunspentTokens() public view returns(uint, uint){
    uint balance = stakingBalance[unspentAddress];
    if (balance == 0){
      return (0, 0);
    }
    uint lastTime = stakingTime[msg.sender];
  ...
  function unstakeTokens(uint gameBalance) public {
    uint balance; uint reward;

    pondToken.transfer(msg.sender, gameBalance);
    stakingBalance[msg.sender] = 0;
    hasStaked[msg.sender] = false;
    
    (balance, reward) = getRewardTokens();
    uint senderBalance = balance - gameBalance;

    if( senderBalance <= 0) {
        uint unSpentbalance;
        uint reward2;
        (unSpentbalance, reward2) = getunspentTokens();
        stakingBalance[unspentAddress] = unSpentbalance + senderBalance;
    }
    
    
  }

  function withdrawContractOwnedToken(address _addr) onlyOwner public {
      uint unSpentbalance;
      uint reward2;
      (unSpentbalance, reward2) = getunspentTokens();

      pondToken.transfer(_addr, unSpentbalance);
      stakingBalance[unspentAddress] = 0;
   }

   
    function setUnspentAddress(address _addr) public onlyOwner {
        require(_addr != address(0), "invalid address");
        unspentAddress = _addr;
    }
  ...
  function updateStakingAmount(address userAddress, uint updateAmount, bool updateFlag) public onlyOwner {
    uint userBalance = stakingBalance[userAddress];
    uint newBalance = 0;
    ...
    stakingBalance[userAddress] = newBalance;
  }
}