...
contract modularDG is DGEvents {}

contract DogGame is modularDG {
    using SafeMath for *;
    using NameFilter for string;
    using RSKeysCalc for uint256;
    
    // TODO: check address
    DGInterfaceForForwarder constant private DGCorp = DGInterfaceForForwarder(0x9aa917ff8484415eda0f98c54968a6503a50ccb7);
    DGBookInterface constant private DGBook = DGBookInterface(0x90256CC9618aB190FB5172D7813c4fdB79319d85);

    string constant public name = "DogGame Round #1";
...
//****************
    ...
    mapping (uint256 => DGdatasets.Player) public plyr_;   // (pID => data) player data
    mapping (uint256 => DGdatasets.PlayerRounds) public plyrRnds_;    // (pID => rID => data) player round data by player id & round id
    mapping (uint256 => mapping (bytes32 => bool)) public plyrNames_; // (pID => name => bool) list of names a player owns.  (used so you can change your display name amongst any name you own)
//****************
// ROUND DATA 
//****************
    DGdatasets.Round public round_;   // round data
//****************
// TEAM FEE DATA 
//****************
...
//==============================================================================
    ...
    /**
     * @dev prevents contracts from interacting with DogGame 
     */
    modifier isHuman() {
...
//====|=========================================================================
        ...
        payable
    {
        ...
        DGdatasets.EventReturns memory _eventData_ = determinePID(_eventData_);
        ...
        public
    {
        ...
        DGdatasets.EventReturns memory _eventData_;
    ...
    {        
        ...
        if (_now > round_.end && round_.ended == false && round_.plyr != 0)
        {
            ...
            emit DGEvents.onWithdrawAndDistribute
            (
                msg.sender, 
                plyr_[_pID].name, 
                _eth, 
        ...
        // in any other situation
            ...
            // fire withdraw event
            emit DGEvents.onWithdraw(_pID, msg.sender, plyr_[_pID].name, _eth, _now);
        }
    }
        ...
        payable
    {
        ...
        (bool _isNewPlayer, uint256 _affID) = DGBook.registerNameXIDFromDapp.value(_paid)(_addr, _name, _affCode, _all);

        uint256 _pID = pIDxAddr_[_addr];
        
        // fire event
        emit DGEvents.onNewName(_pID, _addr, _name, _isNewPlayer, _affID, plyr_[_affID].addr, plyr_[_affID].name, _paid, now);
    }
        ...
        payable
    {
        ...
        address _addr = msg.sender;
        uint256 _paid = msg.value;
        (bool _isNewPlayer, uint256 _affID) = DGBook.registerNameXaddrFromDapp.value(msg.value)(msg.sender, _name, _affCode, _all);
        ...
        payable
    {
        ...
        address _addr = msg.sender;
        uint256 _paid = msg.value;
        (bool _isNewPlayer, uint256 _affID) = DGBook.registerNameXnameFromDapp.value(msg.value)(msg.sender, _name, _affCode, _all);
...
//=====================_|=======================================================
     ...
     */
    function buyCore(uint256 _pID, uint256 _affID, DGdatasets.EventReturns memory _eventData_)
        private
    {
        // grab time
        uint256 _now = now;
        ...
        // if round is not active     
            ...
            if (_now > round_.end && round_.ended == false) 
            {
                ...
                emit DGEvents.onBuyAndDistribute
                (
                    msg.sender, 
                    plyr_[_pID].name, 
                    msg.value, 
     ...
     * incoming eth depending on if we are in an active round or not 
     */
    function reLoadCore(uint256 _pID, uint256 _affID, uint256 _eth, DGdatasets.EventReturns memory _eventData_)
        private
    {
        // grab time
        uint256 _now = now;
        ...
        // if round is not active and end round needs to be ran   
            ...
            // fire buy and distribute event 
            emit DGEvents.onReLoadAndDistribute
            (
                msg.sender, 
                plyr_[_pID].name, 
                _eventData_.compressedData, 
     ...
     * is live.
     */
    function core(uint256 _pID, uint256 _eth, uint256 _affID, DGdatasets.EventReturns memory _eventData_)
        private
    {
...
//==============================================================================
    ...
    function receivePlayerInfo(uint256 _pID, address _addr, bytes32 _name, uint256 _laff)
        external
    {
        require (msg.sender == address(DGBook), "only DGBook can call this function");
     ...
     * @return pID 
     */
    function determinePID(DGdatasets.EventReturns memory _eventData_)
        private
        returns (DGdatasets.EventReturns)
    {
        uint256 _pID = pIDxAddr_[msg.sender];
        // if player is new to this version of DogGame
        if (_pID == 0)
        {
            // grab their player ID, name and last aff ID, from player names contract 
            _pID = DGBook.getPlayerID(msg.sender);
            bytes32 _name = DGBook.getPlayerName(_pID);
            uint256 _laff = DGBook.getPlayerLAff(_pID);
     ...
     */
    function managePlayer(uint256 _pID, DGdatasets.EventReturns memory _eventData_)
        private
        returns (DGdatasets.EventReturns)
    {            
     ...
     */
    function endRound(DGdatasets.EventReturns memory _eventData_)
        private
        returns (DGdatasets.EventReturns)
    {        
        ...
        // community rewards
        if (!address(DGCorp).call.value(_com)(bytes4(keccak256("deposit()"))))
        {
            _gen = _gen.add(_com);
            _com = 0;
        }
     ...
     */
    function distributeExternal(uint256 _pID, uint256 _eth, uint256 _affID, DGdatasets.EventReturns memory _eventData_)
        private
        returns(DGdatasets.EventReturns)
    {
        ...
        if (_affID != _pID && plyr_[_affID].name != '') {
            ...
            emit DGEvents.onAffiliatePayout(_affID, plyr_[_affID].addr, plyr_[_affID].name, _pID, _aff, now);
        } else {
     ...
     */
    function distributeInternal(uint256 _pID, uint256 _eth, uint256 _keys, DGdatasets.EventReturns memory _eventData_)
        private
        returns(DGdatasets.EventReturns)
    {
        // calculate gen share
     ...
     */
    function endTx(uint256 _pID, uint256 _eth, uint256 _keys, DGdatasets.EventReturns memory _eventData_)
        private
    {
        _eventData_.compressedData = _eventData_.compressedData + (now * 1000000000000000000);
        _eventData_.compressedIDs = _eventData_.compressedIDs + _pID;
        
        emit DGEvents.onEndTx
        (
            _eventData_.compressedData,
            _eventData_.compressedIDs,
            plyr_[_pID].name,
        ...
        public
    {
        ...
        // TODO: set owner
        require(
            msg.sender == 0x12120B3E8e7eB23eC6040bfbeb6dfB3439383829,
            "only owner can activate"
        );
        
        // can only be ran once
        require(activated_ == false, "DogGame already activated");
