...
// File: contracts/lib/contracts/CommonConstants.sol
...
// File: contracts/lib/interface/ERC1155TokenReceiver.sol
...
// File: contracts/lib/utils/StringLibrary.sol
...
// File: contracts/lib/contracts/HasTokenURI.sol
...
// File: contracts/lib/interface/IERC1155MetadataURI.sol
...
// File: contracts/lib/contracts/ERC1155MetadataURI.sol
...
// File: contracts/lib/interface/IERC165.sol
...
// File: contracts/lib/interface/IERC1155.sol
...
// File: contracts/lib/contracts/ERC165.sol
...
// File: contracts/lib/contracts/ERC1155.sol
...
// File: contracts/lib/contracts/HasContractURI.sol
...
// File: contracts/lib/contracts/PermittedTransfer.sol
...
// File: contracts/lib/contracts/MinterRole.sol
...
// File: contracts/lib/contracts/ERC1155Base.sol
...
// File: contracts/token/CarbonToken.sol
...
contract CarbonToken is Ownable, ERC1155Base, MinterRole, PermittedTransfer {
...
// File: contracts/lib/contracts/Seller.sol
...
/**
 * @title Seller
 * Seller - a sales contract for CryptoArte non-fungible tokens 
 * corresponding to paintings from the www.cryptoarte.io collection
 */
contract Seller is Ownable, Pausable, EIP712 {

  event Sent(address indexed payee, uint256 amount, uint256 balance);
  event Received(address indexed payer, uint tokenId, uint256 amount, uint256 balance);

  string private constant SIGNING_DOMAIN = "CarbonClubNFT-Voucher";
  string private constant SIGNATURE_VERSION = "1";
  CarbonToken private _token;
  mapping(string => bool) private _usedVouchers;
  ...
  * @param _nftAddress address for Crypto e-Art non-fungible token contract CarbonToken
  */
  constructor(address _nftAddress) EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) { 
    require(_nftAddress != address(0) && _nftAddress != address(this));
    _token = CarbonToken(_nftAddress);
  }

  /// @notice Redeems an NFTVoucher for an actual NFT.
  /// @param voucher A signed NFTVoucher that describes the NFT to be purchaised.
  function redeemNFTVoucher(
  ...
  whenNotPaused {
    ...
    // check if voucher has been expired
    require(voucher.expiredAt >= block.timestamp, "Voucher has been expired");

    // check if voucher already used
    require(_usedVouchers[voucher.voucherId] == false, "Voucher already used");

    // make sure that the signer is contract's owner
    require(_token.owner() == signer, "Signature invalid or unauthorized");
    ...
    _usedVouchers[voucher.voucherId] = true;
    
    emit Received(voucher.buyer, voucher.tokenId, msg.value, address(this).balance);
  }

  /**
  * @dev withdraw / withdraw _amount to _payee
  */
  ...
  function _hash(NFTVoucher calldata voucher) internal view returns (bytes32) {
    return _hashTypedDataV4(keccak256(abi.encode(
      keccak256("NFTVoucher(string voucherId,uint256 tokenId,uint256 count,uint256 minPrice,address buyer,uint256 expiredAt)"),
      keccak256(abi.encodePacked(voucher.voucherId)),
      voucher.tokenId,
      voucher.count,
      voucher.minPrice,
      voucher.buyer,
   ...
   * @dev Will revert if the signature is invalid. Does not verify that the signer is authorized to transfer NFTs.
   */
  /// 
  ...
  /// Mint token if permission is correct
  // function permittedMint(
  //     address to,
  //     uint256 id, 
  //     uint256 supply, 
  //     string memory uri,
  //     MintPermission calldata permission
  // )
  // onlyMinter
  // public {
  //     require(id != 0, "required id and not 0");
  //     require(supply != 0, "required supply and not 0");
  //     require(bytes(uri).length > 0, "required uri token");
  //     require(
  //         to == permission.to &&
  //         id == permission.id &&
  //         supply == permission.supply &&
  //         keccak256(bytes(uri)) == keccak256(bytes(permission.uri)),
  //         "signed data is not equal to passed parameters"
  //     );

  //     address signer = _verifyMintPermission(permission);
  //     require(owner() == signer, "signature invalid or unauthorized");

  //     _mint(id, supply, uri);
  // }

  /// @notice Verifies the signature for a given MintPermission,
  /// returning the address of the signer.
  /// @dev Will revert if the signature is invalid.
  /// Does not verify that the signer is authorized to mint NFTs.
  // function _verifyMintPermission(
  //     MintPermission calldata permission
  // ) 
  // internal 
  // view 
  // returns (address) {
  //     bytes32 digest = _hashMintPermission(permission);
  //     return ECDSA.recover(digest, permission.signature);
  // }

  /// @notice Returns a hash of the given MintPermission, 
  /// prepared using EIP712 typed data hashing rules.
  // function _hashMintPermission(
  //     MintPermission calldata permission
  // ) 
  // internal 
  // view 
  // returns (bytes32) {
  //     return _hashTypedDataV4(keccak256(abi.encode(
  //         keccak256("MintPermission(address to,uint256 id,uint256 supply,string uri)"),
  //         permission.to,
  //         permission.id,
  //         permission.supply,
  //         permission.uri
