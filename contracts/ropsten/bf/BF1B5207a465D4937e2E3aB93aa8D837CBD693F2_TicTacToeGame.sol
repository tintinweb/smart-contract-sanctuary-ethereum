...
contract TicTacToeGame {
        ...
        atStatus(_id, GameStatus.started) 
    {
        ...
        if ((msg.sender != games[_id].player1 && _tictac == TicTac.cross) ||
                (msg.sender != games[_id].player2 && _tictac == TicTac.zero)) 
        {
            revert invalidAddress();
        } 
    ...
    function win(uint256 _id) 
        ...
        atStatus(_id, GameStatus.started) 
    {
        if (msg.sender != games[_id].player1 || msg.sender != games[_id].player2) { 
            revert invalidAddress();
        }
