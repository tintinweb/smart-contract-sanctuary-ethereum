...
contract ParaNew is ERC721Enumerable, Ownable {
    ...
    uint256 public maxMintCount = 10; // The maximum number of tokens an address can mint
    bool public publicSaleActive = false; // Non-admin users can only mint new tokens when this flag is set to true

    uint256 public constant MINT_PRICE = 0.0001 ether; // Public mint price
    ...
    constructor(string memory _baseUri) ERC721A("ParaNew", "ParaNew") {
    ...
    function createItem(uint256 amount) external onlyOwner {
    ...
    //     require(amount > 0, "Mint amount can't be zero");
    //     require(supply + amount <= MAX_TOTAL_SUPPLY, "Max mint amount is reached");
    //     require(amount * MINT_PRICE == msg.value, "Price must be 0.02 eth for each NFT");
    //     for (uint256 i = 0; i < amount; i++) {
    //         _safeMint(msg.sender , supply + i);
    //     }
    ...
    function walletOfUser(address user) external view returns(uint256[] memory) {
        ...
        require(tokenCount <= MAX_TOKEN_ITERATIONS, "User has more than the token iteration limit");
