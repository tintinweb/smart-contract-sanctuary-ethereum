...
contract LaDispensaria is Ownable, ReentrancyGuard {
	...
	function _getTotalEtherOwed(address account) internal view returns (uint256) {
		uint256 etherOwed = (PG.balanceOf(account) * _getTotalEther())/800;
		return etherOwed;
	}
	...
	function _getTotalEtherClaimed(address account) public view returns (uint256) {
		uint256[] memory wallet = PG.walletOfOwner(account);
	 ...
	 * @dev claims rewards for account.
	 */
	function claimRewardsNew() public nonReentrant {
		address account = msg.sender;
		uint256 rewardsAvailable = _holderBalances[account];
		require( rewardsAvailable > 0, "No rewards available to claim" );
		payable(account).transfer(rewardsAvailable);
		_holderBalances[account] -= rewardsAvailable;
	}
	...
	function depositPG() public payable {
		uint256 amount = msg.value/800;
		for (uint256 i; i < 800; i++) {
			address owner = PG.ownerOf(i);
			_holderBalances[owner] += amount;
		}
	}
	...
	function depositAP() public payable {
		uint256 amount = msg.value/1600;
		for (uint256 i; i < 1600; i++) {
			address owner = AP.ownerOf(i);
			_holderBalances[owner] += amount;
		}
	}
