...
*/
    ...
    function acceptOffer(uint256 _orderId, uint8 _offerId) external nonReentrant {
        ...
        uint256 totalPayableAmount = orders[_orderId].principal + orders[_orderId].totalInterestAmount;
        uint256 installmentCount = orders[_orderId].NoOfInstallment;
        uint256 loanDuration = orders[_orderId].repaymentTimestamp - block.timestamp;

        uint256 emiAmount = totalPayableAmount / installmentCount;
        uint256 emiDuration = loanDuration / installmentCount;

        //@TODO add a validation to fix the precision error...
        // require((emiAmount *installmentCount) == totalPayableAmount,"EMI_AMOUNT_PRECISION_ERROR" );
        // require((emiDuration *installmentCount) == loanDuration,"EMI_DURATION_PRECISION_ERROR" );
    ...
    function withdrawActiveOffer(uint256 _orderId, uint256 _offerId) public nonReentrant {
        require(isExistOffer(_orderId, _offerId), "NONEXIST_OFFER");
        require(orders[_orderId].loanState == LoanState.Active, "INVALID_LOAN_STATE");
        require(orders[_orderId].offers[_offerId].userOffered == msg.sender, "OFFER_LENDER_ALLLOWED");
        require(orders[_orderId].offers[_offerId].status == OfferStatus.Initial, "NON_INITAL_OFFER_STATE");

        // require(orders[_orderId].activeOffer == 0, "OFFER_ALREADY_ACTIVE");
        // change the LoanState to initial
    ...
    function withdrawOrder(uint256 _orderId) public {
        ...
        require(
            orders[_orderId].loanState == LoanState.Active || orders[_orderId].loanState == LoanState.Created,
            "INVALID_LOAN_STATE"
        );
    ...
    function payEMI(uint256 _orderId, bool _oneTimePayment) external payable {
        require(isExistOrder(_orderId), "NONEXIST_ORDER");
        require(orders[_orderId].borrower == msg.sender, "UNAUTHORIZED_USER");
        ...
        require(MAX_EMI_BOUNCE_COUNT >= emiInfo[_orderId].EMIBounceCount, "MAXM_NO_BOUNCE_REACHED");
    ...
    function withDrawOfferedAmount(uint256 _orderId, uint256 _offerId) external nonReentrant {
        require(isExistOrder(_orderId), "NONEXIST_ORDER");
        require(
        ...
        //@TODO withdraw the funds from the escrow smart contract and transfer back too the
        // user .How to make that withdrawal acess safe and secure ?
        // How to keep that access decentralized ?
        // What are other escrow service used for this purpose ?
    }
