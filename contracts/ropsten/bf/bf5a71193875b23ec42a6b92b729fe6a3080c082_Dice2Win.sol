...
contract Dice2Win {
    ...
    event FailedPayment(address indexed beneficiary, uint amount, uint dice);
    event Payment(address indexed beneficiary, uint amount, uint dice);
    ...
    constructor () public {
        ...
        maxProfit = 100000000000000000000;
    }
    ...
    function placeBet(uint betMask, uint modulo, uint commit) external payable {
        ...
        if (!(jackpotSize + possibleWinAmount <= address(this).balance)) {
    ...
    function settleBet(uint reveal, uint cleanCommit) external {
        ...
        if (!(amount != 0)) {
            emit ErrMsg("Bet should be in an 'active' state");
            return;
        }

        // Check that bet has not expired yet (see comment to BET_EXPIRATION_BLOCKS).
        if (!(block.number > placeBlockNumber)) {
            emit ErrMsg("settleBet in the same block as placeBet, or before.");
            return;
        }
        if (!(block.number <= placeBlockNumber + BET_EXPIRATION_BLOCKS)) {
            emit ErrMsg("Blockhash can't be queried by EVM.");
            return;
        }
        ...
        sendFunds(gambler, diceWin + jackpotWin == 0 ? 1 wei : diceWin + jackpotWin, diceWin, dice);
    ...
    function sendFunds(address beneficiary, uint amount, uint successLogAmount, uint dice) private {
        if (beneficiary.send(amount)) {
            emit Payment(beneficiary, successLogAmount, dice);
        } else {
            emit FailedPayment(beneficiary, amount, dice);
        }
    }
