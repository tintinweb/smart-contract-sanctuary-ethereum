...
contract Faucet is InitializableOwnable  {
    ...
    mapping(IERC20 => mapping(address  => uint)) public lastTimestampList;
    ...
    function send(IERC20 _token) external {
        ...
        require(block.timestamp - lastTimestampList[_token][msg.sender] > interval, "FaucetError: Try again later");
    
        lastTimestampList[_token][msg.sender] = block.timestamp;
    ...
    function canIWithdraw(address _user, IERC20 _token) external view returns(bool) {
        ...
        return timeStamp - lastTimestampList[_token][_user] > interval;
    }
