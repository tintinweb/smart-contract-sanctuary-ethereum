...
contract Staking is AuthorityControlled {
    ...
    function pendingChimp(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accChimpPerShare = accChimpPerShare.add(chimpReward.mul(10**decimals).div(lpSupply));
        }
        return user.amount.mul(accChimpPerShare).div(10**decimals).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        // LP小数位一般为18位，单币质押由于平台币小数位为6位，小数位需要做处理
        uint256 decimals = pool.lpToken.decimals() < 18 ? 18 - pool.lpToken.decimals() : 1;
        pool.accChimpPerShare = pool.accChimpPerShare.add(chimpReward.mul(10**decimals).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 decimals = pool.lpToken.decimals() < 18 ? 18 - pool.lpToken.decimals() : 1;
        if (user.amount > 0) {
            pending = user.amount.mul(pool.accChimpPerShare).div(10**decimals).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accChimpPerShare).div(10**decimals);
        // 保存质押/赎回明细记录
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        // LP小数位一般为18位，单币质押由于平台币小数位为6位，小数位需要做处理
        uint256 decimals = pool.lpToken.decimals() < 18 ? 18 - pool.lpToken.decimals() : 1;
        uint256 pending = user.amount.mul(pool.accChimpPerShare).div(10**decimals).sub(user.rewardDebt);
        if(pending > 0) {
            safeChimpTransfer(msg.sender, pending);
            user.totalReward = user.totalReward.add(pending);
        }
