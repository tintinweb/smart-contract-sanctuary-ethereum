...
interface IUniswapV2Pair {
    ...
    function Prize0CumulativeLast() external view returns (uint256);

    function Prize1CumulativeLast() external view returns (uint256);
...
contract Archa is Context, IBEP20, Ownable {
    ...
    string private constant _name = "Archa";
    string private constant _symbol = "ARCHA";
    ...
    uint256 public _maxTxAmount = 10 * 10**9 * 10**18;
    uint256 public _maxWalletHoldingLimit = 100 * 10**9 * 10**18;
    ...
    function calculateLiquidityFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_liquidityFee).div(feeDenominator);
    }
    ...
    function IsAutomaticMarketMaker(address AMM) external view returns (bool) {
        return _isAutomaticMarketMaker[AMM];
    }
    ...
    function UpdateBuyTaxFeePercentage(
        ...
        uint256 TotalFee = taxFee
            .add(liquidityFee)
            .add(burnFee);
        emit BuyTaxFeeUpdated(TotalFee);
    }
    ...
    function UpdateSellTaxFeePercentage(
        ...
        uint256 TotalFee = taxFee
            .add(liquidityFee)
            .add(burnFee);
        emit SellTaxFeeUpdated(TotalFee);
    }
    ...
    function ExcludeorIncludeFromWalletHoldingLimit(address account, bool status)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (_isAutomaticMarketMaker[from] && !_isExcludedFromFee[to]) {
            _taxFee = _buyTaxFee;
            _liquidityFee = _buyLiquidityFee;
            _burnFee = _buyBurnFee;

        }
        if (_isAutomaticMarketMaker[to] && !_isExcludedFromFee[from]) {
            _taxFee = _sellTaxFee;
    ...
    /* Airdrop Begins Fixed Amount of tokens to all holders*/
    function multiTransfer_fixed_AirDrop(
        ...
        uint256 SCCC = tokens.mul(addresses.length);

        require(balanceOf(from) >= SCCC, "Not enough tokens in wallet");

        for (uint256 i = 0; i < addresses.length; i++) {
            _transfer(from, addresses[i], tokens);
        }
    }

    /* Airdrop Begins For Mixed Amount of tokens to all holders*/
