...
contract CDeployer is ICDeployer {
	...
	function deployCollateral(address rythmPair) external returns (address collateral) {
		...
		bytes32 salt = keccak256(abi.encodePacked(msg.sender, rythmPair));
		assembly {
...
contract Collateral is ICollateral, PoolToken, CStorage, CSetter {
    ...
    function getPrices() public returns (uint256 price0, uint256 price1) {
        (uint224 twapPrice112x112, ) =
            IRythmPriceOracle(rythmPriceOracle).getResult(underlying);
        (uint112 reserve0, uint112 reserve1, ) =
            IRythmPair(underlying).getReserves();
        uint256 collateralTotalSupply =
            IRythmPair(underlying).totalSupply();
         ...
         */
        require(price0 > 100, "Rythm: PRICE_CALCULATION_ERROR");
        require(price1 > 100, "Rythm: PRICE_CALCULATION_ERROR");
    }
    ...
    function _transfer(
        ...
        require(tokensUnlocked(from, value), "Rythm: INSUFFICIENT_LIQUIDITY");
    ...
    function seize(
        ...
        require(shortfall > 0, "Rythm: INSUFFICIENT_SHORTFALL");
    ...
    function flashRedeem(
        ...
        if (data.length > 0)
            IRythmCallee(redeemer).rythmRedeem(msg.sender, redeemAmount, data);
        ...
        require(
            ...
            "Rythm: INSUFFICIENT_REDEEM_TOKENS"
        );
...
contract CStorage {
	...
	address public rythmPriceOracle;
...
contract CSetter is PoolToken, CStorage {
    ...
    function _initialize(
        ...
        rythmPriceOracle = IFactory(factory).rythmPriceOracle();
    }
