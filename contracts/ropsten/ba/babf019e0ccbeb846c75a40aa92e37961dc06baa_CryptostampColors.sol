...
// File: /mnt/dev/capacity/cryptostamp/truffle-colors/node_modules/openzeppelin-solidity/contracts/introspection/IERC165.sol
...
// File: /mnt/dev/capacity/cryptostamp/truffle-colors/node_modules/openzeppelin-solidity/contracts/token/ERC721/IERC721.sol
...
// File: /mnt/dev/capacity/cryptostamp/truffle-colors/node_modules/openzeppelin-solidity/contracts/token/ERC721/IERC721Enumerable.sol
...
// File: /mnt/dev/capacity/cryptostamp/truffle-colors/node_modules/openzeppelin-solidity/contracts/token/ERC721/IERC721Metadata.sol
...
contract CryptostampColors {
    ...
    function setColorMulti(uint256 _tokenIdStart, Colors[] memory _values)
    public
    onlyCreateControl
    requireCryptostamp
    {
        require(_tokenIdStart == allColors.length, "Values can can only be appended at the end.");
        uint256 valcount = _values.length;
        require(_tokenIdStart + valcount <= cryptostamp.totalSupply(), "Cannot set colors for non-existing stamps.");
        for (uint256 i = 0; i < valcount; i++) {
            allColors.push(_values[i]);
        }
        emit SavedColors(_tokenIdStart, allColors.length - 1);
    }
    ...
    returns (Colors)
    {
        require(tokenId < allColors.length, "The token ID is not valid.");
        return allColors[tokenId];
    }
