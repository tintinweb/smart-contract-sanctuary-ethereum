    ...
    ReentrancyGuardUpgradeable 
{
    ...
    bytes32 constant REQUEST_FROM_ROUTER_METHOD_NAME = 0x7265717565737446726f6d526f75746572000000000000000000000000000000;
    ...
    event ValsetUpdatedEvent(uint64 indexed _newValsetNonce, address[] _validators, uint64[] _powers);
        ...
        returns (bytes32)
    {
        bytes32 checkpoint = keccak256(
            abi.encode(_chainId, CHECK_POINT_METHOD_NAME, _valsetArgs.valsetNonce, _valsetArgs.validators, _valsetArgs.powers)
        );
    ...
    function initialize(
        ...
        _valset = Utils.ValsetArgs(_validators, _powers, 0);
        ...
        emit ValsetUpdatedEvent(valsetNonce, _validators, _powers);
        // Constructor Fx
    }

    function _authorizeUpgrade(address newImplementation) internal virtual override onlyRole(DEFAULT_ADMIN_ROLE) {}

    function requestToRouter(bytes memory payload, string memory routerBridgeContract) external nonReentrant whenNotPaused {
        eventNonce++;
        emit SendToRouterEvent(msg.sender, eventNonce, chainType, routerBridgeContract, chainId, payload);
    }
    ...
    function requestFromRouter(
        ...
        if (block.timestamp > requestPayload.expTimestamp) {
            ...
            emit EventOutboundAck(requestPayload.sender, chainType, chainId, requestPayload.nonce, emptyData, 3, false);
            return;
        }

        (bool success, bytes memory data) = address(this).call(
            abi.encodeWithSelector(
                IGateway.executeHandlerCalls.selector,
                requestPayload.sender,
                requestPayload.handlers,
                requestPayload.payloads,
                requestPayload.isAtomic
        ...
        emit EventOutboundAck(requestPayload.sender, chainType, chainId, requestPayload.nonce, data, 0, success);
    }
        ...
        for (uint8 i = 0; i < handlers.length; i++) {
            ...
            if (success) {
                (success, ) = handler.call(
                    abi.encodeWithSelector(IUser.handleRequestFromRouter.selector, sender, payloads[i])
                );
            }
