...
interface IERC721 {
    ...
    function walletOfOwner(address owner) external returns (uint256[] memory);
}

contract DOODv3 is ERC20, ERC721Holder, Ownable {
    IERC721 public DCats;

    bool public paused = false;
    uint256 public maxSupply = 4000000000 ether;

    uint256 public tokensStaked;
    uint256 public tokensBurned; // inthe variables section
    struct StakeInfo {
        address owner;
        uint256 tokenId;
        uint256 start;
    ...
    constructor() ERC20("DoodCatsToken", "DOODv3") {
        DCats = IERC721(0xfC0FabbEc6E7F0b6958dd7615f0DCa34DC7bF4e6); // Deploy the NFT contract first
    }
    ...
    function mintTokensToOwner(uint256 amount) public onlyOwner {
        // please enter the amount in wei
    ...
    function pendingRewards(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 rewardTier = reward(tokenId);
        uint256 result = (1 * rewardTier * timePassed) / 86400;
        if (result < 0) {
            return 0;
        }
        return result;
    ...
    function harvest(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        require(
            DCats.ownerOf(tokenId) == address(this),
            "You need to stake the token first!"
        );
        StakeInfo storage info = stakes[msg.sender][tokenId]; // returns default values if the caller doesn't have any stakes
        require(info.status, "Stake info for this ID not found!"); // I implemented this, this is the fix
    ...
    function harvestBatch(address user) external payable {
        require(!paused, "The contract is paused");
    ...
    function upgradeTokenRewardTier(uint256 tokenId) public {
        ...
        if (tier == 0) {
            _burn(msg.sender, 900 ether);
            tokensBurned += 900 ether;
            info.tier = 1;
        }
        if (tier == 1) {
            _burn(msg.sender, 1350 ether);
            tokensBurned += 1350 ether;
            info.tier = 2;
        }
        if (tier == 2) {
            _burn(msg.sender, 1800 ether);
            tokensBurned += 1800 ether;
            info.tier = 3;
        }
        if (tier >= 3) {
            _burn(msg.sender, 2250 ether);
            tokensBurned += 2250 ether;
            info.tier = 4;
        }
    }

    function upgradeMax(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        require(
            DCats.ownerOf(tokenId) == address(this),
            "You need to stake the token first!"
        ...
        StakeInfo storage info = stakes[msg.sender][tokenId];
        _burn(msg.sender, 2250 ether);
        tokensBurned += 2350 ether;
        info.tier = 4;
    }

    function stake(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        DCats.safeTransferFrom(msg.sender, address(this), tokenId);
        require(
            DCats.ownerOf(tokenId) == address(this),
    ...
    function stakeBatch() external {
        require(!paused, "The contract is paused");
        uint256[] memory tokenIds = DCats.walletOfOwner(msg.sender);
        for (uint256 currentId = 0; currentId < tokenIds.length; currentId++) {
            if (tokenIds[currentId] == 0) {
                continue;
            }
    ...
    function unstake(uint256 tokenId) public {
        ...
        if (pendingRewards(tokenId) > 0) {
            harvest(tokenId); // this will revert this call because we fixed the harvest, but pendingRewards might be zero aswell
        }
        StakeInfo storage info = stakes[msg.sender][tokenId];
        require(info.status, "Stake info for this ID not found!"); // the fix
        require(info.owner != msg.sender, "You dont own this token!");
        info.owner = address(0);
        info.start = 0;
        info.tier = 0;
        info.status = false;
        DCats.safeTransferFrom(address(this), msg.sender, tokenId); // can steal literally anyones tokens
        require(
            DCats.ownerOf(tokenId) == msg.sender,
            "Error while transferring!"
        );
        tokensStakedByUser[msg.sender] -= 1;
        tokensStaked -= 1;
