...
contract ArcadeLand is ERC721Enumerable, Ownable {
    ...
    function setSupply(Size size, uint256 _newSupply) external onlyOwner {
        require(
            _newSupply < landSpecs[size].maxSupply,
            "supply cannot be greater"
        );
    ...
    modifier mintCompliance(Size size, uint256 quantity) {
        ...
        require(
            addressList[msg.sender] + quantity <= maxMintPerAddress,
            "over address limit"
        );
    ...
    modifier mintComplianceWithWL(Size size, uint256 quantity) {
        ...
        require(
            whitelist[msg.sender] + quantity <= maxMintPerWL,
            "over WL limit"
        );
        LandSpec memory land = landSpecs[size];
        require(
    ...
    function mintStandardLandsWhitelist(
    ...
    function mintLargeLandsWhitelist(
        bytes32[] calldata merkleProof,
        uint256 quantity
    ) external payable {
    ...
    function mintXLargeLandsWhitelist(
        bytes32[] calldata merkleProof,
        uint256 quantity
    ) external payable {
    ...
    function mintXLargeLandsForAddress(uint256 quantity, address receiver)
        external
        onlyOwner
    {
    ...
    function mintMegaLandsWhitelist(
        bytes32[] calldata merkleProof,
        uint256 quantity
    ) external payable {
    ...
    function mintMegaLandsForAddress(uint256 quantity, address receiver)
        external
        onlyOwner
    {
    ...
    function priceBySize(Size size) external view returns (uint256) {
