...
contract Seduction is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_SEDUCTIONS_WHITELIST_CAP = 10;
    uint256 public constant SEDUCTIONS_FOR_WAIFU = 3;

    mapping(uint256 => uint256) private chickTraitofTokenId;
    mapping(address => uint256) private whitelistAddressPresaleCount;
    ...
    function preSale(
        ...
        require(ownerOf(tokenId) == owner(), "Invalid token for presale.");
        require(whitelistAddressPresaleCount[_msgSender()].add(1) <= MAX_SEDUCTIONS_WHITELIST_CAP, "You can only mint a maximum of 10 for presale");
        safeTransferFrom(owner(), _msgSender(), tokenId);
        whitelistAddressPresaleCount[_msgSender()] += 1;
    }
