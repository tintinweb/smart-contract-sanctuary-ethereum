...
contract ZetaToken is Context, IERC20, Ownable {
    ...
    string private _name = "ZetaNetwork Token";
    string private _symbol = "ZETA";
    ...
    uint256 public _MKTshare = 1;


    uint256 public _maxTxAmount = 1000000;
    ...
    function totalBurn() public view returns (uint256) {
        return _tOwned[BURN_ADDRESS];
    }
    
    function calculatedSupply() public view returns (uint256) {
        return _tTotal.sub(_tOwned[BURN_ADDRESS]);
    }
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_taxFee).div(94);
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            // 1% of every transfer mkt
            uint256 mktAmount = amount.mul(_MKTshare).div(100);
            // 5% of every transfer burn
            uint256 burnAmount = amount.mul(_burnFee).div(100);
            // 94% of transfer sent to recipient
            uint256 sendAmount = amount.sub(burnAmount).sub(mktAmount);
            require(
                amount == sendAmount + mktAmount + burnAmount,
                "ZETA::transfer: mkt and Burn value invalid"
            );

            //transfer amount, it will take tax, burn
            _tokenTransfer(from, DEV_WALLET, mktAmount, false);                        
            _tokenTransfer(from, BURN_ADDRESS, burnAmount, false);
