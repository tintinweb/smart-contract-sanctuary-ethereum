...
contract CelebrityGame is Ownable {
	 ...
	 * @param _cardIds must be returned when creating CARD
	 * @param _playerIds must be returned when registering player
	 * (this might cost a lot of gas)
	 */
    function likeCelebrity(uint256[] _cardIds, uint256[] _playerIds) external isStartEnable {
	 ...
	 * @param _cardIds must be returned when creating CARD
	 * @param _playerIds must be created when registering player
	 * (this might cost a lot of gas)
	 */
    function dislikeCelebrity(uint256[] _cardIds, uint256[] _playerIds) external isStartEnable {
		for (uint i = 0; i <  _cardIds.length; i++){
		    uint256 _cardId = _cardIds[i];
		    uint256  _playerId  =  _playerIds[i];
            Card storage queryCard = cards[cardIdXSeq[_cardId]];
