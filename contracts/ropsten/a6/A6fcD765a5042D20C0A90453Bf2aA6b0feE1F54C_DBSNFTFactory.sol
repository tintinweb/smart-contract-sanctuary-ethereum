...
contract DBSNFTFactory is FactoryERC721, Ownable, ReentrancyGuard {
    ...
    event Minted(address indexed minter, address indexed account, uint256[] tokens, uint256 tableID);
    ...
    function _canMint(
        ...
        (_amount > 0 && balanceOf(owner(), _tokenId) >= _amount && isApprovedForAll(owner(), _fromAddress));
    }
    ...
    function _canMintBatch(
        ...
        return (_tokenId.length == _amount.length)&&(_fromAddress == owner())||(isApprovedForAll(owner(), _fromAddress));
    }
    ...
    function canMint(uint256 _tokenId, uint256 _amount)
        override
        external
        view
        returns (bool)
    {
        return _canMint(_msgSender(), _tokenId, _amount);
    }
    ...
    function mint(
        ...
        require( _canMint(_msgSender(), _tokenId, _amount),"DBSNFTFactory#_mint: CANNOT_MINT_MORE");
        uint256[] memory tokens = _mint(_tokenId, _toAddress, _amount);

        emit Minted(_msgSender(), _toAddress, tokens, _tokenId);
    }
    ...
    function _mint(
        ...
        uint256[] memory ids = tradable.mint(_toAddress, _Id, _amount);
    ...
    function mintBatch(
        ...
        require(_canMintBatch(_msgSender(), _ids, _amounts),"DBSNFTFactory#_mint: CANNOT_MINTBATCH_MORE");
        (uint256[] memory tokens, uint256[] memory tableids) = _batchMint(_ids, _toAddress, _amounts);
    ...
    function _batchMint(
        ...
        DBSNFTTokens tradable = DBSNFTTokens(nftAddress);
        (uint256[] memory tokens, uint256[] memory tableids) = tradable.batchMint(_toAddress, _ids, _amounts);
    ...
    /**
     * Get the factory's owner balances.
    ...
    function balanceOf(address _owner, uint256 _Id)
        ...
        returns (uint256)
    {
        require(_owner == owner(), "owner failed");

        uint256 currentSupply = tokenSupply[_Id];
...
interface FactoryERC721 {
     ...
     * @param _toAddress address of the future owner of the asset(s)
     * @param _amounts amount of the option to mint
     */
    function mintBatch( 
        uint256[] memory _ids,
        address _toAddress,
