...
contract TwitterAnalyticsSubscription is Ownable {
    ...
    constructor() {
        subscriptionPlans.push(Subscription(true, 15000000000000000,7));
    ...
    function addPlan(bool enabled, uint _price, uint _days) external onlyOwner {
        Subscription memory newPlan = Subscription(enabled, _price, _days);
        subscriptionPlans.push(newPlan);
    }

    function enablePlan(uint _id) external onlyOwner {
        require(subscriptionPlans[_id]._enabled == false, "Plan already enabled");
        subscriptionPlans[_id]._enabled = true;
    }

    function disablePlan(uint _id) external onlyOwner {
        require(subscriptionPlans[_id]._enabled == true, "Plan already disabled");
        subscriptionPlans[_id]._enabled = false;
    }
