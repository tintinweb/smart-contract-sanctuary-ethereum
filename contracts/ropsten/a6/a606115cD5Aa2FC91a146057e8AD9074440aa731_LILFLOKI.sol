...
contract LILFLOKI is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x910979430a4c74dEF9265037cB3b50e0F9c53381); // Marketing Address
    ...
    uint256 private _tTotal = 6000000000000000 * (10**18);
    ...
    uint256 public autoLpDivisor = 30;

    uint256 public _totalFee = 60;  // marketingDivisor + autoLpDivisor
    uint256 private _previousTotalFee = _totalFee;
    
    uint256 public maxSellTransactionAmount = 600000000000000 * (10**18);
    uint256 public swapTokensAtAmount = 100000000 * (10**18);
    uint256 public _maxWalletToken = 600000000000000 * (10**18);
    ...
    event FeesUpdated(uint256 autoLpDivisor, uint256 marketingDivisor);
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && !automatedMarketMakerPairs[from]) {
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalBNBFee = _totalFee.sub(autoLpDivisor.div(2));

        // adding liquidity
        if(amountToLiquify > 0) // enabling to set autoLP tax to zero
            addLiquidity(amountToLiquify, transferredBalance.mul(autoLpDivisor).div(totalBNBFee).div(2));

        //Send to Marketing address
        transferToAddressETH(marketingAddress, transferredBalance.mul(marketingDivisor).div(totalBNBFee));
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rTax, uint256 tTransferAmount, uint256 tTax, uint256 tFee) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tTax, uint256 tFee) = _getTValues(tAmount);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tFee = calculateTotalFee(tAmount);
    ...
    function updateFeeDivisor(uint256 newAutoLpDivisor, uint256 newMarketingDivisor) external onlyOwner {
        uint256 newTotalFee = newAutoLpDivisor.add(newMarketingDivisor);
        ...
        emit FeesUpdated(newAutoLpDivisor, newMarketingDivisor);
    }
    
    
    function setMaxSellTransactionAmount(uint256 _maxSellTransactionAmount) external onlyOwner() {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LilFloki: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LilFloki: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "LilFloki: The liquidity wallet is already this address");
    ...
    function setMaxWalletTokend(uint256 _maxToken) external onlyOwner {
  	    _maxWalletToken = _maxToken * (10**18);
  	}
