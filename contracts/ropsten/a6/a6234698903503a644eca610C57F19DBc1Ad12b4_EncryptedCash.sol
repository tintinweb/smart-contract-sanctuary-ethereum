...
contract EncryptedCash is ERC20Detailed, ERC20Pausable, MinterRole, BlacklistedRole {
  ...
  //   uint256 private _totalSupply = 1000000000 * (10 ** uint256(decimals));
        ...
        public
    {
        ...
        // _total_supply = 100 * 10**7 * 10**uint256(decimals)
        _presale_amount = 20 * 10**7 * 10**uint256(decimals);
        _liquidity_amount = 10 * 10**7 * 10**uint256(decimals);
        _public_amount = 15 * 10**7 * 10**uint256(decimals);
        _utility_amount = 35 * 10**7 * 10**uint256(decimals);
        _team_amount = 20 * 10**7 * 10**uint256(decimals);
