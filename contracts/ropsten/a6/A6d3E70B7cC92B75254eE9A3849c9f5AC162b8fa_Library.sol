...
contract Library is Ownable {
    ...
    // representation of keccak256("Wish(address owner, uint256 bookId, uint256 value)")
    bytes32 public WISH_TYPEHASH;
    ...
    mapping(address => uint256) public wishNonces;
    ...
    constructor(address libTokenAddress, address payable libWrapperAddress) {
        ...
        WISH_TYPEHASH = keccak256(
            "Wish(address receiver,uint256 bookId,uint256 value,uint256 nonce,uint256 deadline)"
        );
    ...
    function fullfillWish(
        ...
        uint256 value,
        uint256 deadline,
        Signature calldata wishSignature,
        ...
        markBookAsBorrowed(bookId, receiver);

        // pay
        uint256[] memory bookIds = new uint256[](1);
        bookIds[0] = bookId;
        payTheBooks(bookIds, deadline, permitSignature);
    }
    ...
    function markBookAsBorrowed(uint256 bookId, address buyer) internal {
        ...
        require(
            !userToBorrowedBookId[buyer][bookId].isCurrentlyBorrowed,
            "User already borrowed that book !"
        );

        userToBorrowedBookId[buyer][bookId].isCurrentlyBorrowed = true;
        book.copies--;

        if (!userToBorrowedBookId[buyer][bookId].isEverBorrowed) {
            bookIdToBorrowerHistory[bookId].push(buyer);
            userToBorrowedBookId[buyer][bookId].isEverBorrowed = true;
        }
    }

    function payTheBooks(
        uint256[] memory bookIds,
        uint256 deadline,
        Signature calldata signature
    ) internal {
        uint256 value = bookIds.length * bookPriceLibWei;
    ...
    function borrowBooks(
        ...
        for (uint256 i = 0; i < bookIds.length; i++) {
            markBookAsBorrowed(bookIds[i], msg.sender);
        }
        payTheBooks(bookIds, deadline, signature);
    }
    ...
    function withdrawLibTokens() external onlyOwner {
        ...
        // Marto, is this the way to do the below task (i.e. for the contract to approve the tokens spending) ?
...
contract LibraryToken is ERC20PresetMinterPauser {
    ...
    function permit(
        ...
        require(value > 0, "approve value should be > 0");
        ...
        require(
            ...
            "Recovered address not equals the owner's one !"
        );
