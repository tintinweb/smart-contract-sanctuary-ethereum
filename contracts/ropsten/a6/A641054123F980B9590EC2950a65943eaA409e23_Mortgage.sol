...
contract Mortgage is AccessControl, Pausable, Utils {
    ...
    uint256 totalMortgageNFTNumLimit = 20;
    uint256 singleMortgageNFTNumLimit = 1;
    uint256 mortgageNum;
    ...
    function mortgageNFT(uint256[] memory mortgageTokenIds, bytes32 hash, bytes memory signature, string memory nonce) public {
        ...
        //验证抵押的数量是否合法
        require(mortgageTokenIds.length >= singleMortgageNFTNumLimit, "You must mortgage NFT at least singleMortgageNFTNumLimit!");
        require(mortgageTokenIds.length.add(mortgageNum) <= totalMortgageNFTNumLimit, "You must mortgage must less than totalMortgageNFTNumLimit!");
        //验证hash
        ...
        if (mortgageMap[msg.sender].length == 0){
            mortgagedAddress.push(msg.sender);
        }
        //记录Map
        for (uint256 i = 0; i < mortgageTokenIds.length; i++) {
            mortgageMap[msg.sender].push(mortgageTokenIds[i]);
        }
        emit Mortgage(msg.sender, mortgageTokenIds, nonce);
        mortgageNum += mortgageTokenIds.length;
    }
    ...
    function recaptureNFT(bytes32 hash, bytes memory signature, string memory nonce) public {
        ...
        mortgageNum -= recaptureTokenIds.length;
