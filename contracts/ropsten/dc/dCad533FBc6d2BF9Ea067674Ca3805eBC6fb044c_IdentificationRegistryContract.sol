...
abstract contract ERC721VotesUpgradeable is Initializable, ERC721Upgradeable, VotesUpgradeable {
    ...
    function _getVotingUnits(uint256 tokenId) internal view virtual override returns (uint256) {
        require(ownerOf(tokenId) == msg.sender, "ERC721Upgradeable: Address not owner of the token");
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts (last updated v4.6.0) (governance/utils/VotesUpgradeable.sol)
pragma solidity ^0.8.0;
...
abstract contract VotesUpgradeable is Initializable, IVotesUpgradeable, ContextUpgradeable, EIP712Upgradeable {
    ...
    mapping(uint256 => uint256[]) private _delegated;
    ...
    function _getTotalSupply() public view virtual returns (uint256) {
        return _totalCheckpoints.latest();
    }

    // Returns all the tokenId that have delegated to 'tokenId'
    function getDelegatedIdsForTokenId(uint256 tokenId) public view returns (uint256[] memory){
        return _delegated[tokenId];
    }
    ...
    function delegate(uint256 tokenId, uint256 delegatee) public virtual {
        ...
        //return all delegated votes
        uint256[] memory delegated = _delegated[tokenId];
    ...
    // delegate from 'tokenId' to delegatee
    function _delegate(uint256 tokenId, uint256 delegatee) internal virtual {
        uint256 oldDelegate = delegates(tokenId);
        _delegation[tokenId] = delegatee;

        //Delete delegated from 'tokenId' & add delegated to 'delegatee'
        delete _delegated[tokenId];
        _delegated[delegatee].push(tokenId);

        emit DelegateChanged(tokenId, oldDelegate, delegatee);
        _moveDelegateVotes(oldDelegate, delegatee, 1);
    }
    ...
    ) internal virtual
    {
        ...
        _delegated[tokenId].push(tokenId);
    ...
    function _moveDelegateVotes(
        ...
        if (from != to && amount > 0) {
            ...
            (uint256 oldValueTo, uint256 newValueTo) = _delegateCheckpoints[to].push(_add, amount);
...
interface IVotesUpgradeable {
    ...
    /**
    * @dev Emitted when a token is burned, so the delegated votes are lost
    */
    event DelegateRemoved(uint indexed tokenId);
    /**
    * @dev Emitted when a token is minted, so the delegated votes are created
    */
    event DelegateCreated(uint indexed tokenId);
    /**
