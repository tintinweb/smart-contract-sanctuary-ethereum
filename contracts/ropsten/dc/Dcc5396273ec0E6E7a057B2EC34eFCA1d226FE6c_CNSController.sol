...
contract CNSController is Ownable, ENSController {
    ...
    */
        ...
        token.setTokenURI(
            ...
            string(
                abi.encodePacked(
                    ...
                    string(abi.encodePacked(token.currentId()))
                )
            )
        );
    }
    ...
    function removeSubdomain(bytes32 _subnode) internal onlyController {
        token.burn(subdomains[getSubdomainId(_subnode)].cnsTokenId);
        delete subdomains[getSubdomainId(_subnode)];
    }

    function removeSubdomainWithoutBurn(bytes32 _subnode)
        internal
        onlyController
    {
        delete subdomains[getSubdomainId(_subnode)];
...
contract CNSToken is ERC721, AccessControl, Ownable {
    ...
    function _burn(uint256 tokenId) internal override onlyRole(Controller) {
