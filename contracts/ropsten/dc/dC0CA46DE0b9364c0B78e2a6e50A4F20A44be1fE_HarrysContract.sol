...
contract HarrysContract is ERC721A, Ownable, Pausable {
    ...
    uint public constant PRICE = 1e13;//1e15; //0.005
    uint public constant MAX_PER_MINT = 2;
    uint public constant MAX_PER_WALLET = 3;//15;

    string public _contractBaseURI= "https://gateway.pinata.cloud/ipfs/QmYNQVgVbH9HM8WyooMqCBfp9frLyKrwWtbdvq8PEMmqEi/";
    ...
    function lockMetadata(uint256 quantity) internal {
        string memory strEnd = ".json";
        for (uint256 i = quantity; i > 0; i--) {
            uint256 tid = totalSupply() - i;
            emit PermanentURI(string(abi.encodePacked(tokenURI(tid),strEnd)), tid);
        }
    }
