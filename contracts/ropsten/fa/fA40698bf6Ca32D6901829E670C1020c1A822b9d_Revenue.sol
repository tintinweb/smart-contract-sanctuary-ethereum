...
contract Revenue is Ownable{
    ...
    event RechageTokenEvent(address token,address from,uint256 amount);
    event WithdrawTokenEvent(address token,address from,uint256 amount);

    function rechargeToken(address token,uint256 amount) public payable{
        if(token==address(0)){
            require(msg.value>0,"amount must be > 0");
            emit RechageTokenEvent(address(0), msg.sender, msg.value);
        }else{
            require(amount>0,"amount must be > 0");
            safeTransferFrom(token,msg.sender,address(this),amount);
            emit RechageTokenEvent(token, msg.sender, amount);
        }
    }

    function withdraw(address token) public onlyController {
        ...
        for(uint256 i=0;i<length;i++){
            ...
            uint256 ratio = roles[i].ratio;
            uint256 amount = balance * ratio /10000;
            if(token==address(0)){
                payable(account).transfer(amount);
            }else{
                safeTransfer(token,account,amount);
            }
            emit WithdrawTokenEvent(token, account, amount);
        }

    }

    function setRoles(Role[] memory _roles) public onlyController {
        delete roles;
        uint256 length = _roles.length;
        uint256 total;
        for(uint256 i=0;i<length;i++){
            total+=_roles[i].ratio;
            roles.push(_roles[i]);
        }
        require(total == 10000,"data is worng");
    }

    function getRolesLength() public view returns(uint256) {
        return roles.length;
    }

    function getRoleByIndex(uint256 index) public view returns(Role memory) {
        return roles[index];
    }
