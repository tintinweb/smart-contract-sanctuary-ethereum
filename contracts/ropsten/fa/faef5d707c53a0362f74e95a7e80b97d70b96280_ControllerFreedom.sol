...
contract ControllerFreedom is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    */
     ...
     */
    function createCommunityGroup(uint256 daoId, Group[] memory group) internal { //uint256[] memory id
	...
	function communityCreate(string memory name, string memory purpose, string memory legalStatus, string memory legalDocuments, string[] memory links, Group[] memory group, string memory logoImage, string memory coverImage) external onlyProxy returns (bool) {
        ...
        if(group.length >= 0){
            // groupIdsArr = 
            createCommunityGroup(commId, group);
        }
     ...
     */
    function transferBounty(string memory amount, string memory targetAcc, uint256 commId) external returns (bool){
        require(assertVotingContractAddr() != false, "You are unauthorized user.");
     ...
     */
    function changeName(string memory proposedDaoName, uint256 commId) external returns (bool){
        require(assertVotingContractAddr() != false, "You are unauthorized user.");
        Community storage comm = fetchAssertCommunity(commId, "Can't change community data, bcz it doesn't exist.");
     ...
     */
    function changePurpose(string memory proposedPurpose, uint256 commId) external returns (bool){
        require(assertVotingContractAddr() != false, "You are unauthorized user.");
        Community storage comm = fetchAssertCommunity(commId, "Can't change community data, bcz it doesn't exist.");
    ...
    */
    function createGroup(Group memory newGroup, uint256 commId) external returns (bool) {
        require(assertVotingContractAddr() != false, "You are unauthorized user.");
        fetchAssertCommunity(commId, "Can't change community data, bcz it doesn't exist.");
    ...
    */
    function proposalCreationRight(uint256 _type, uint256 commId, address senderAddr) external view returns (bool) {        
        ...
        MemberOfCommRes memory res = isMemberOfCommunity(commId, senderAddr);
        if(res.isMember) {
            uint256[] memory proposalCreationRights = res.proposalCreation; 
            for (uint256 index = 0; index < proposalCreationRights.length; index++) {
                if(_type == proposalCreationRights[index]){
                    return true;
                }
            }
        }
    ...
    */
    function votingRight(uint256 _type, uint256 commId, address senderAddr) external view returns (bool) {        
        ...
        MemberOfCommRes memory res = isMemberOfCommunity(commId, senderAddr);
        if(res.isMember) {
            uint256[] memory voting = res.voting; 
            for (uint256 index = 0; index < voting.length; index++) {
                if(_type == voting[index]){
                    return true;
                }
            }
        }
