...
/*     function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (!swapEnabled && sender != owner() && recipient != address(this) && 
            recipient != address(DEAD) && recipient != uniswapV2Pair) 
        {
            require(_isExcludedFromMaxWallet[recipient] 
                || (balanceOf(recipient) + amount <= (totalSupply() * maxWalletBPS) / 10000), "MAX_WALLET");
        }

        if (swapEnabled && // true
            balanceOf(address(this)) > swapTokensAtAmountPlusOne && // true
            ...
            swapping = true;
            _executeSwap(!swapAllToken ? swapTokensAtAmount : balanceOf(address(this)), address(this).balance);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (
            ...
            swapping = true;
            contractTokenBalance = swapAllToken ? swapTokensAtAmount : contractTokenBalance;
        ...
        if (takeFee) {
            ...
            amount -= fees;
            _executeTransfer(sender, address(this), (sender == address(uniswapV2Pair) ? totalBuyTax : totalSellTax));
        }
