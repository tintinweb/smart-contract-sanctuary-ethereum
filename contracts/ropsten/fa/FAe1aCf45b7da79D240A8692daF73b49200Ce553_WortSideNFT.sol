...
contract WortSideNFT is ERC721,Ownable{
    using Strings for uint256;


    uint256 public mintPublicPrice;
    uint256 public mintWhiteListPrice;
    ...
    constructor(
    ) payable ERC721("Wort Side","WS"){
        setBaseURI("ipfs://QmZCJRwkPap1Az6my4iPt8w7GTm8ja9f1LJntp2oJNcfAu");
        setNotRevealedURI("ipfs://QmeSjSinHpPnmXmspMjwiXyN6zS4E9zccariGR3jxcaWtq/1");
        mintApePrice = 0.009 ether;
        mintWhiteListPrice = 0.015 ether;
    ...
    function mintforOwner() internal onlyOwner{
    ...
    function setIsApeMintEnabled() public onlyOwner{
        ...
        if(isApeMintEnable==true){
            isPrivateMintEnable = false;
            isPublicMintEnable = false;
        }
    }
    function setIsPrivateMintEnabled() public onlyOwner{
        isPrivateMintEnable = true;
        if(isPrivateMintEnable==true){
    ...
    function whitelistUsers(address[] calldata _users) public onlyOwner {
        require(_users.length <= maxWhitelistedAddresses,"that over Limit for whitelistUsers");
        whitelistedAddresses = _users;
    }
    function apelistUsers(address[] calldata _users) public onlyOwner {
        require(_users.length <= maxApelistedAddresses,"that over Limit for whitelistUsers");
    ...
    function privateMint(uint256 quantity_) public payable{
        require(isPrivateMintEnable,"private minting not enable");
        require(isWhitelisted(msg.sender) || isApelisted(msg.sender), "user is not whitelisted");
        require(msg.value == quantity_ * mintWhiteListPrice,"wrong mint value");
        require(totalSupply + quantity_ <= maxSupply,"supply not enough");
        require(quantity_ <= 5,"max unit mint per transection");
        for(uint256 i=0;i<quantity_; i ++){
            uint256 newTokenId = totalSupply + 1;
