...
contract PandolfiniEscrow is ReentrancyGuard, RoleControl {
  ...
  Counters.Counter private _itemsTransferred;
  error CannotReceive();
  ...
  */
  ...
  function transferItem(address _nftAddress, uint256 _tokenId, address _toAddress) public onlyOperator nonReentrant {
    ...
    uint256 id = _itemsTransferred.current();
  ...
  function releaseApproved(address _nftAddress, uint256 _tokenId) public onlyOperator nonReentrant {
    require(IERC721(_nftAddress).getApproved(_tokenId) == address(this), "NFT must be approved to Pandolfini Escrow");
    IERC721(_nftAddress).approve(address(0), _tokenId);
  }

  function getItemsTransferred() public view returns (Item[] memory) {
    uint256 index = 0;
    uint256 itemCount = _itemsTransferred.current();
    Item[] memory outputs = new Item[](itemCount);
    for (uint256 i = 1; i <= itemCount; i++) {
      outputs[index] = items[i];
      index ++;
    }
    return outputs;
  }
