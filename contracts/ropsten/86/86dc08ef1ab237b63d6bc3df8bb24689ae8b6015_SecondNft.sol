...
contract SecondNft is ERC721, ERC2981, Ownable, MerkleProof {
    ...
    modifier mintCompliance(uint256 firstTokenId, address recipient, string memory uri, uint96 feeNumerator) {
        ...
        if (msg.sender != owner()) {
            ...
            require(msg.value >= minCost, "insufficient funds");
        }
        _;
    }

    function whitelistMint(uint256 firstTokenId, address recipient, string memory uri, uint96 feeNumerator, bytes32[] memory proof) public payable mintCompliance(firstTokenId, recipient, uri, feeNumerator) returns (uint256) {
        uint256 supply = _tokenIds.current();
        if (msg.sender != owner()) {
    ...
    function mintNFT(uint256 firstTokenId, address recipient, string memory uri, uint96 feeNumerator) public payable mintCompliance(firstTokenId, recipient, uri, feeNumerator) returns (uint256) {
        uint256 supply = _tokenIds.current();
        _mint(recipient, supply);
        _tokenIds.increment();
        _setTokenRoyalty(supply, recipient, feeNumerator);
