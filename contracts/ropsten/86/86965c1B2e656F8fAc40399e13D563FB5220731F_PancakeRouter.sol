            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"61be98b9384231059798683b970bdc1cd04c46f43555af35c9c2cc5bd1d97b06" // Change to INIT_CODE_PAIR_HASH of Pancake Factory
                    )
                )
            )
        );
...
contract PancakeRouter is IPancakeRouter02, OwnableUpgradeable {
    ...
    function initialize(
        ...
        uint256 _resetDuration,
        uint256 _backdraftVolumePercent,
        address _recoveryWallet,
        address _opsWallet,
        address _reflectionsWallet,
        address _advisoryWallet,
        address _liquidityWallet,
        uint256 _normalFees,
        uint256 _backdraftFees,
        uint256 _antiFees

    ) external initializer{
        WETH = _weth; // 0xA5ba1FD868178BCE83367a0C7c5548DBDa93c7e3
        factory = _factory; // 0xc778417E063141139Fce010982780140Aa0cD5Ab
        tokenAddess = _tokenAddess; // 0x4e5A902De3c97103De7F2b8aED15F46a8e747DD9
        ...
        normalFees = _normalFees;
        backdraftFees = _backdraftFees;
        antiFees = _antiFees;
    }
        ...
        returns (uint256[] memory amounts)
    {
        ...
        require(path[1] == tokenAddess, "PancakeRouter: INVALID_PATH");
        ...
        uint256 fees = amount.mul(tFees).div(100);
        ...
        returns (uint256[] memory amounts)
    {
        ...
        require(path[0] == tokenAddess, "PancakeRouter: INVALID_PATH");
        require(
            !_isBlackListed[msg.sender] && !_isBlackListed[to],
            "Account is blacklisted"
        );
        ...
        uint256 fees = amount.mul(tFees).div(100);
        uint256 afterFees = amount.sub(fees);
        emit totalFeesA(tFees);
        emit totalFeesA(fees);
        emit totalFeesA(afterFees);
        _takeFees(fees);
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            uint256 amountOutput;
            {
                ...
                (uint256 reserveInput, uint256 reserveOutput) = input == token0
                    ? (reserve0, reserve1)
                    : (reserve1, reserve0);
                amountInput = IERC20Upgradeable(input).balanceOf(address(pair)).sub(
                    reserveInput
                );
    ...
    function setFees(
        ...
        require(
            ...
            "Fees Distrbution must be greater than 100%"
        );
    }
    ...
    function setAntiBotFees(
        ...
        AntiBotFee.recoveryFee = recovery;
        AntiBotFee.opsFee = ops;
        AntiBotFee.reflectionsFee = reflections;
        AntiBotFee.advisoryFee = advisory;
        AntiBotFee.liquidityFee = liquidity;
        require(
            recovery + ops + reflections + advisory + liquidity < 100,
            "Fees Distrbution must be greater than 100%"
        );
    ...
    function setAntiBotFee() private {
       _advisoryFee = AntiBotFee.advisoryFee;
        _recoveryFee = AntiBotFee.recoveryFee;
        _reflectionsFee = AntiBotFee.reflectionsFee;
        _opsFee = AntiBotFee.opsFee;
        _liquidityFee = AntiBotFee.liquidityFee;
    }
    ...
    function setAntiFees(uint256 _fees) public onlyOwner {
        antiFees = _fees;
    }
    ...
    function setAntiBotBlocks(uint256 _antiBotBlocks) public onlyOwner {
