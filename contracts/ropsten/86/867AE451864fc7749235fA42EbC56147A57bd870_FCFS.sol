...
contract FCFS is ERC1155Holder, Ownable {
    ...
    function placeALot(
        ...
        _createALot(tokenId, amount, pricePerOne, period, allowedTiers);
    }
    ...
    function stake(uint256 lotId, uint256 amount) external {
        ...
        stakes[stakeCounter] = Stake({lotId: lotId, amount: amount, claimed: false, better: msg.sender});
    ...
    {
        ids = lots[lotId_].stakeIds;
        stakes_ = new Stake[]((lots[lotId_].stakeIds).length);

        for (uint256 i; i < (lots[lotId_].stakeIds).length; i++) {
            stakes_[i] = stakes[i];
        }
    }
