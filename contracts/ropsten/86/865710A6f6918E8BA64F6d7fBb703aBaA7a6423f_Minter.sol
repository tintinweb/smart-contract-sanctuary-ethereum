...
contract Minter is SafeOwnable {
    ...
    constructor(address _WETH, IMintableERC721 _nft, uint _startAt, uint _finishAt, uint _maxNum, IERC20[] memory _tokens, uint[] memory _prices, address payable _receiver, address _verifier) {
    ...
    modifier NotFinish(uint num) {
        require(block.timestamp <= finishAt && sellNum + num <= maxNum && buyNum[msg.sender] < num, "already finish");
        _;
    }
    ...
    function mint(IERC20 _payToken, uint _num, uint8 _v, bytes32 _r, bytes32 _s) external payable AlreadyBegin TokenSupport(_payToken) NotFinish(_num) {
        if (verifier != address(0)) {
            //we need signature to verify the request
            require(
                ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(address(this), msg.sender, _num)))), _v, _r, _s) == verifier,
                "verify failed"
            );
        }
        unchecked {
            uint remain = _num - buyNum[msg.sender];
            uint cost = remain * tokensPrice[_payToken];
            if (cost > 0) {
