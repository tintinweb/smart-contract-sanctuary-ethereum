...
contract CTZN is ERC20, ReentrancyGuard
{
    bytes32 private OWNER = keccak256('OWNER');
    uint Curve = 1.1 ether;
    uint ActiveBRTContracts;
    uint ActiveCitizenCities;
    ...
    event RewardRatesChanged(uint[] ProjectIDs, uint[][] RewardRates);
    ...
    mapping(uint => BRTContract) BRTContracts;
    mapping(uint => CitizenContract) CitizenContracts;
    ...
    constructor() ERC20("CTZN", "CTZN") 
    {
        ...
        ActiveBRTContracts = 4;
        CitizenContracts[0] = CitizenContract('CryptoGalacticans', 0xE231EB9d9Bb1676B95e0FDf30229E62E11Cd03c3, 0, 100);
        CitizenContracts[1] = CitizenContract('CryptoVenetians', 0xE231EB9d9Bb1676B95e0FDf30229E62E11Cd03c3, 0, 100);
        CitizenContracts[2] = CitizenContract('CryptoNewYorkers', 0xfcE8A5DA534fB7829a0880C76c9feDa48Abee02c, 0, 100);
        CitizenContracts[3] = CitizenContract('CryptoBerliners', 0xfcE8A5DA534fB7829a0880C76c9feDa48Abee02c, 0, 100);
        CitizenContracts[4] = CitizenContract('CryptoCitizen City #5', 0xfcE8A5DA534fB7829a0880C76c9feDa48Abee02c, 0, 0);
        CitizenContracts[5] = CitizenContract('CryptoCitizen City #6', 0xfcE8A5DA534fB7829a0880C76c9feDa48Abee02c, 0, 0);
        CitizenContracts[6] = CitizenContract('CryptoCitizen City #7', 0xfcE8A5DA534fB7829a0880C76c9feDa48Abee02c, 0, 0);
        CitizenContracts[7] = CitizenContract('CryptoCitizen City #8', 0xfcE8A5DA534fB7829a0880C76c9feDa48Abee02c, 0, 0);
        CitizenContracts[8] = CitizenContract('CryptoCitizen City #9', 0xfcE8A5DA534fB7829a0880C76c9feDa48Abee02c, 0, 0);
        CitizenContracts[9] = CitizenContract('CryptoCitizen City #10', 0xfcE8A5DA534fB7829a0880C76c9feDa48Abee02c, 0, 0);
        ActiveCitizenCities = 4;
    }
    ...
    function DelegateCitizens(address Recipient, uint[][] calldata TokenIDs) public nonReentrant
    {
        ...
        {
            if(TokenIDs[x].length > 0)
            {
                for(uint y; y < TokenIDs[x].length; y++)
                {
                    require(
                        TokenIDs[x][y] >= CitizenContracts[x].StartingIndex 
                        && 
                        TokenIDs[x][y] <= CitizenContracts[x].EndingIndex, 
                        "CTZN: Invalid TokenID"
                    );
                    require(
                        IERC721(CitizenContracts[x].ContractAddress).ownerOf(TokenIDs[x][y])
                        == 
                        msg.sender,
                        "CTZN: User Does Not Own TokenID"
                    );
                    if(CitizenDelegation[TokenIDs[x][y]] != address(0))
                    {
                        address OldOwner = CitizenDelegation[TokenIDs[x][y]];
                        AddressDelegatedCitizens[OldOwner]--;
                    }
                    AddressDelegatedCitizens[Recipient]++;
                    CitizenDelegation[TokenIDs[x][y]] = Recipient;
                }
            }
        }
     ...
     */
    function IncrementRewards(address Recipient, uint Amount) external onlyBRTContract
    { 
        if(AddressDelegatedCitizens[Recipient] > 0)
        {
            uint NewAmount = (Amount * (AddressDelegatedCitizens[Recipient] * Curve));
    ...
    /**
     * @dev Changes Reward Rates For Projects
     */
    function __ChangeRewardRates(uint[] calldata ProjectIDs, uint[][] calldata RewardRates) external onlyOwner
    {
        require(ProjectIDs.length == RewardRates.length, "Arrays Must Be Of Equal Length");
        for(uint x; x < ProjectIDs.length; x++)
        {
            IBRT(BRTContracts[ProjectIDs[x]].ContractAddress).ModifyRewardRates(RewardRates[x]);
        }
        emit RewardRatesChanged(ProjectIDs, RewardRates);
    }

    /**
     * @dev Transfers Ownership Of Smart Contract
     */
    function __TransferOwnership(address NewOwner) external onlyOwner
    {
    ...
    modifier onlyOwner
    {
        require(Role[msg.sender] == OWNER, "CTZN: Sender Is Not Owner");
        _;
    }
}
...
interface IBRT 
{ 
    function ModifyRewardRates(uint[] calldata RewardRates) external;
}
