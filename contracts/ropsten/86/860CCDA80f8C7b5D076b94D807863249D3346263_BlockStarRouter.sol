...
interface IBlockStarRouter02 is IBlockStarRouter01 {
...
library BlockStarLibrary {
    ...
    {
        require(tokenA != tokenB, "BlockStarLibrary: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "BlockStarLibrary: ZERO_ADDRESS");
    }
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"ee7df0824118e49142dae6d286ed9d7c8ea44ea5537248059a7c42d9a9b72ff9" // Change to INIT_CODE_PAIR_HASH of BlockStar Factory
                    )
                )
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IBlockStarPair(
            pairFor(factory, tokenA, tokenB)
        ).getReserves();
    ...
    function quote(
        ...
        require(amountA > 0, "BlockStarLibrary: INSUFFICIENT_AMOUNT");
        require(
            reserveA > 0 && reserveB > 0,
            "BlockStarLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "BlockStarLibrary: INSUFFICIENT_INPUT_AMOUNT");
        require(
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "BlockStarLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
        require(
            reserveIn > 0 && reserveOut > 0,
            "BlockStarLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, "BlockStarLibrary: INVALID_PATH");
...
contract BlockStarRouter is IBlockStarRouter02, OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    event feesName(string name);
    event totalFeesA(uint256 fees);

    function initialize() external initializer {
         __Ownable_init();
        WETH = 0xc778417E063141139Fce010982780140Aa0cD5Ab; 
        factory = 0x1B79Ca3d17CD506cA2C413A8b6AaD92b4430941C; 
        tokenAddess = 0x88BBf2522E2bC2cc1738EDe4C18Bb6BbAD5380B3; 
        resetDuration = 86400;
        backdraftVolumePercent = 2;
        recoveryWallet = payable(0xAd5292D3D35F57CC0D7876cfD7B583DC99637b0d);
        opsWallet = payable(0xbBf23259FB8588d24D0C2db58AFE8C4A7409898d);
        reflectionsWallet = payable(0xe0D01085BDb2aFFa0755941B554B36aA6871a05c);
        advisoryWallet = payable(0xA81a8544AE780842F53308720D23A4314E331138);
        liquidityWallet = payable(0x05196fD35E94ebe90a5f2b705c0daaF838BC163d);
        normalFees = 1000;
        backdraftFees = 3000;
    ...
    function _addLiquidity(
        ...
        if (IBlockStarFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IBlockStarFactory(factory).createPair(tokenA, tokenB);
        }
        (uint256 reserveA, uint256 reserveB) = BlockStarLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = BlockStarLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "BlockStarRouter: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = BlockStarLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "BlockStarRouter: INSUFFICIENT_A_AMOUNT"
                );
        ...
        )
    {
        ...
        address pair = BlockStarLibrary.pairFor(factory, tokenA, tokenB);
        TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountB);
        liquidity = IBlockStarPair(pair).mint(to);
    }
        ...
        )
    {
        ...
        address pair = BlockStarLibrary.pairFor(factory, token, WETH);
    ...
    {
        address pair = BlockStarLibrary.pairFor(factory, tokenA, tokenB);
        IBlockStarPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint256 amount0, uint256 amount1) = IBlockStarPair(pair).burn(to);
        (address token0, ) = BlockStarLibrary.sortTokens(tokenA, tokenB);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (address token0, ) = BlockStarLibrary.sortTokens(input, output);
            uint256 amountOut = amounts[i + 1];
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2
                ? BlockStarLibrary.pairFor(factory, output, path[i + 2])
                : _to;
            IBlockStarPair(BlockStarLibrary.pairFor(factory, input, output))
        ...
        returns (uint256[] memory amounts)
    {
        ...
        amounts = BlockStarLibrary.getAmountsOut(factory, afterFees, path);
        require(
        ...
        assert(
            IWETH(WETH).transfer(
                BlockStarLibrary.pairFor(factory, WETH, tokenAddess),
                amounts[0]
            )
        );
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (takeFee) {
            ...
            if (
                ...
                setBackDraftFee();
                tFees = backdraftFees;
                emit feesName("setBackDraftFee");
            }
        }

        amounts = BlockStarLibrary.getAmountsOut(factory, amountIn, path);
        require(
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            BlockStarLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = BlockStarLibrary.sortTokens(input, output);
            IBlockStarPair pair = IBlockStarPair(
                BlockStarLibrary.pairFor(factory, input, output)
            );
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountInput = IERC20Upgradeable(input)
                    ...
                    .sub(reserveInput);
                amountOutput = BlockStarLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
        ...
        returns (uint256[] memory amounts)
    {
        ...
        amounts = BlockStarLibrary.getAmountsOut(factory, msg.value, path);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
            "BlockStarRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    ...
    function quote(
        ...
        return BlockStarLibrary.quote(amountA, reserveA, reserveB);
    }
    ...
    function getAmountOut(
        ...
        return BlockStarLibrary.getAmountOut(amountIn, reserveIn, reserveOut);
    }
    ...
    function getAmountIn(
        ...
        return BlockStarLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
    ...
    function getAmountsOut(uint256 amountIn, address[] memory path)
        ...
        returns (uint256[] memory amounts)
    {
        return BlockStarLibrary.getAmountsOut(factory, amountIn, path);
    }
    ...
    function getAmountsIn(uint256 amountOut, address[] memory path)
        ...
        returns (uint256[] memory amounts)
    {
        return BlockStarLibrary.getAmountsIn(factory, amountOut, path);
    }
