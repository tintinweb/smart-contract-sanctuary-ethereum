...
contract Casino {
    ...
    function withdrowFunds(uint256 amount) external {
        require(msg.sender == owner, "Lottery: Only the owner can withdrow funds" );
        require(amount <= address(this).balance, "Lottery: Can not withdrow more funds then there is in the Casino");

        (bool success,) = msg.sender.call{value: amount}("");
        require(success, "Lottery: ETH withdrow failed");

        emit Fundswithdrawan(msg.sender, amount);
    }

    function getCasionoBalance() external view returns (uint256) {
