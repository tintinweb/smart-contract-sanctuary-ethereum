...
contract Staking is Ownable {
    ...
    function claim ( address _recipient ) public {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 time = block.timestamp - info.stakedTime;
            uint256 amount = ISWORD( SWORD ).balanceForGons( info.gons );
            if(time < 1 hours)
                IWarmup( warmupContract ).retrieve( _recipient, amount * 80 / 100 );
            else if(time >= 1 hours && time < 24 hours)
                IWarmup( warmupContract ).retrieve( _recipient, amount );
            else
            IWarmup( warmupContract ).retrieve( _recipient, amount * 120 / 100 );
        }
    }
