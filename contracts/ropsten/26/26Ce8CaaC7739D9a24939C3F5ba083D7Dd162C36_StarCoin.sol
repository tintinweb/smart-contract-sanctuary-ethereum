...
contract StarCoin is Context, IERC20, Ownable {
    ...
    uint256 public _maxwalletamount = 10 ** 10 * 10 ** _decimals;
    ...
    event SetCoolDown(uint256 coolDown);
    event SetMaxTxAmount(uint256 maxTxAmount);
    event SetMaxWalletAmount(uint256 maxWalletAmount);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) internal {
       ...
       if(_jackpotTax > 0) {
           _jackpotBalance = _jackpotBalance.add(amount.mul(_jackpotTax).div(100));
    ...
    function setCoolDown(uint256 coolDown) public onlyOwner() {
        _coolDown = coolDown;

        emit SetCoolDown(coolDown);
    }
