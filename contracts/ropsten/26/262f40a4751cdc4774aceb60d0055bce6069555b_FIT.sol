...
contract FIT is ERC20, Pausable, AccessControl, Blacklist {
    ...
    function _adminAddBlacklist(address account, uint8 cause) public onlyRole(ADMIN_ROLE) {
        _addBlacklist(account, cause);
    }
    function _adminRevokeBlacklist(address account) public onlyRole(ADMIN_ROLE) {
        _revokeBlacklist(account);
    }
    function _adminAirdrop(address sender, address recipient, uint256 amount) public onlyRole(ADMIN_ROLE) {
