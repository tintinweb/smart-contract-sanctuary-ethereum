...
contract RopstenSwap is ZetaInteractor, ZetaReceiver, RopstenSwapErrors, Pausable {
        ...
        whenNotPaused()
    {
        ...
        // TODO - the reciever will be different when doing xChain SWAP_MESSAGE
        // for single chain swaps sender==receiver
        (
            bytes32 messageType,
        ...
        ) = abi.decode(zetaMessage.message, (bytes32, address, address, address, bytes, bytes));
        ...
        whenNotPaused()
    {
        ...
        (
            ...
            bytes memory secondLegSwapCallData,
            bytes memory revertLegSwapCallData
        ) = abi.decode(zetaRevert.message, (bytes32, address, address, address, bytes, bytes));
        ...
        IERC20 zetaToken = IERC20(_zetaToken);
        if (!zetaToken.approve(_zeroxExchangeProxy, zetaRevert.zetaAmount))
            revert ErrorApprovingTokens(_zetaToken);
        ...
        if (sellTokenAddress == _ethAddress) {
            repurchasedAmount = address(this).balance;
            bool ethTransfer = receiverAddress.send(repurchasedAmount);
            if (!ethTransfer) revert ErrorTransferringEther();
        } else {
            IERC20 repurchasedToken = IERC20(sellTokenAddress);
            repurchasedAmount = repurchasedToken.balanceOf(address(this));
            bool tokenTransfer = repurchasedToken.transfer(receiverAddress, repurchasedAmount);
            if (!tokenTransfer) revert ErrorTransferringTokens(sellTokenAddress);
        }

        // Emit revert event
        emit FirstLegReverted(receiverAddress, sellTokenAddress, zetaRevert.zetaAmount);
    }
}
