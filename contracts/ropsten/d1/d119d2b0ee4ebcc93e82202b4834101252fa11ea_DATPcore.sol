...
contract DATPcore{
    ...
    function setFeeTokenAddress(address newFeeToken) onlyOwner public {
    ...
    function postListed(address _contractAddress, string _names, string _symbols, uint _decimals, string _introduction, string _website, string _logoURL) onlyOwner payable public{
    ...
    function getTXwithCode(address _contractAddress) view public returns(string, string, uint, string, string, string){
        var l = listeddata[_contractAddress];
        return(l.name,
        l.symbol,
        l.decimal,
    ...
    function postTX(address _contractAddress, string _symbols, uint256 _decimals, uint256 _amountToken, uint256 _rateTokenWei) payable public{
        require(verifStatus[msg.sender] = true && _amountToken > 0 && _rateTokenWei > 0);
    ...
    function cancelOngoingTxByAdmin(bytes32 _codeTXid) onlyOwner public returns (bool){
        var u = datakeeper[_codeTXid];
