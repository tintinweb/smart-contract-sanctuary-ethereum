...
contract CopygameAxelarBridge is IAxelarExecutable, ICopygameBridge, Ownable {
  using SafeERC20 for IERC20;

  IAxelarGasReceiver public immutable gasReceiver;

  constructor(address _controller, address _gateway, address _gasReceiver) IAxelarExecutable(_gateway) ICopygameBridge(_controller) {
  ...
  mapping(uint256 => SiblingData) public siblings;
  mapping(bytes32 => uint256) public chainNameLookup;

  event AddSibling(uint256 indexed chainId, string chainName, string bridgeAddress);
  function addSibling(uint256 chainId, string memory chainName, string memory bridgeAddress) public onlyOwner {
  ...
  function _bridgeAxelarWithToken(string memory destinationChain, string memory destinationAddress, bytes calldata payload, string calldata symbol, uint256 amount) internal {
    {
      IERC20 token = IERC20(gateway.tokenAddresses(symbol));

      token.safeTransferFrom(msg.sender, address(this), amount);
      token.safeApproveNew(address(gateway), amount);
    }
  ...
  function _bridge(uint256 chainId, bytes calldata payload) override internal {
  ...
  function _bridgeWithToken(uint256 chainId, bytes calldata payload, string calldata symbol, uint256 amount) override internal {
    require(msg.sender == address(controller), "F");
    _bridgeAxelarWithToken(siblings[chainId].chainName, siblings[chainId].bridgeAddress, payload, symbol, amount);
  }
  ...
  function _executeWithToken(
    ...
    (uint256 chainId, address originalLeader) = abi.decode(payload, (uint256, address));
    ...
    token.safeTransfer(address(controller), amount);

    controller.creditMaster(chainId, originalLeader, token, amount);
  }
}
