...
contract Crowdsale is Finalizable{
  ...
  function buyTokens(address _beneficiary) public payable {
    ...
    uint256 tokens =msg.value.mul(tokensPerEther).mul(bonusStages[currentStage].add(10000)).div(tokenConversionFactor);
    require(tokens > 0);
    ...
    if (maxContribution > 0) {
      ...
      if (tokens > maxTokens) {
        ...
        // Calculate the actual weiAmount for that new amount of tokens.
        weiAmount = tokens.mul(tokenConversionFactor).div(tokensPerEther.mul(bonusStages[currentStage].add(10000)));
    ...
    _updatePurchasingState(_beneficiary, tokens, contribution);
   ...
   * @param _contribution Value in wei involved in the purchase
   */
    ...
    internal
  {
    ...
    weiRaised        = weiRaised.add(_contribution);
  }
