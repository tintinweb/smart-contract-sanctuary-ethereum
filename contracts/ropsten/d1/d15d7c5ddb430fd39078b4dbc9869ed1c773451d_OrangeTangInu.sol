...
contract OrangeTangInu is IERC20, Ownable {
    ...
    uint8 private _liquidityFee = 0;
    uint8 private _marketingFee = 0;
    uint8 private _devFee = 0;
    uint8 private _buyBackFee = 0;
    uint8 private _holdersFee = 0;
    uint8 private _totalFee = 0;
    ...
    function activateTrading() external onlyOwner {
        ...
        address _uniswapV2Pair = IFactory(_uniswapV2Router.factory()).getPair(
                address(this),
    ...
    function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotal, "OrangeTang Inu: Amount must be less than total reflections");
        uint256 currentRate =  _getRate();
        return rAmount / currentRate;
    }
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) external view returns (uint256) {
        require(tAmount <= _tTotal, "OrangeTang Inu: Amount must be less than supply");
        uint256 currentRate = _getRate();
    ...
    function _transfer(
        ...
        if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
            require(isTradingEnabled, "OrangeTang Inu: Trading is currently disabled.");
            require(!_isBlocked[to], "OrangeTang Inu: Account is blocked");
            require(!_isBlocked[from], "OrangeTang Inu: Account is blocked");
