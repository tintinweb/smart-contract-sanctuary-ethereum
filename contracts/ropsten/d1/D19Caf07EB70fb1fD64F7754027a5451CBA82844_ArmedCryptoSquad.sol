...
{  
    ...
    function mintReserved(uint256 amount) public payable onlyOwner {
        _commonMint(msg.sender, amount, 0);       
    }
    ...
    function airDrop(address[] memory addresses) public onlyOwner {
        ...
        require((_tokenCounter + addresses.length) <= (_maxTotal - _reservedMints), "You would exceed the limit");
        require(_tokenCounter < ((_currentBatchID+1)*_batchSize), "current batch supply is exceeded"); 

        for (uint256 i = 0; i < addresses.length; i++) {
            _safeMint(addresses[i], _tokenCounter);
            emit Mint(msg.sender, _tokenCounter);
            _tokenCounter++;
            _reservedMints--;   
        }
    }
    ...
    function _commonMint(address to, uint256 amount, uint256 cost) internal {
        ...
        // require((_tokenCounter + amount) < (_maxTotal - _reservedMints), "total supply is exceeded");
