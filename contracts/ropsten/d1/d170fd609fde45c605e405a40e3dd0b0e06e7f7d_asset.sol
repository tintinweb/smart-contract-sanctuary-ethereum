...
contract asset {
    ...
    struct assetreg {
        ...
        string fileType;//assetAddress
        uint assetPrice;
        string description;//allLatitudeLongitude
        string creatorSignature;//propertyPID
        string category;//physicalSurveyNumber
        string document;
        bool isforSell;
    ...
    function registerUser(string memory _name, uint _age, string memory _city,string memory _aadharNumber, string memory _phoneNumber, string memory _document, string memory _email
    ) public {
        ...
        UserMapping[msg.sender] = User(msg.sender, _name, _age, _city,_aadharNumber,_phoneNumber, _document,_email,false);
    ...
    //-----------------------------------------------asset-----------------------------------------------
    function addasset(string memory _fileName, string memory _fileType, uint _assetPrice,string memory _description, string memory _creatorSignature,string memory _category, string memory _document) public {
        require(isUserVerified(msg.sender));
        assetsCount++;
        assets[assetsCount] = assetreg(assetsCount, _fileName, _fileType, _assetPrice,_description,_creatorSignature, _category , _document,false,payable(msg.sender),false);
