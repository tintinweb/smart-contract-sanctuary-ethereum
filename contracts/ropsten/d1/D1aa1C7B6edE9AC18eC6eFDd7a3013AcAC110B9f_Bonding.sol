...
/* ========== VIEWS ========== */
    ...
    function getBrainPrice(uint256 amount) public view returns (uint256){
        ...
        return((amount*Res1*10**18)/Res0); // return amount of wETH needed to buy BRAIN, multiply by decimals
    }
    ...
    function bond(uint256 amount) external nonReentrant whenNotPaused {
        ...
        uint256 currentExchangeRate = getBrainPrice(amount);
    ...
    function withdraw(uint256 id) public nonReentrant {
        ...
        require(
            block.timestamp > _balances[msg.sender][id].bondedAt.add(vestingPeriod),
            "5 day vesting period has not passed yet"
        );
        require(
        ...
        uint256 brainReceived = amount.div(oldExchangeRate).mul(oldDenominator).div(oldNumerator);
