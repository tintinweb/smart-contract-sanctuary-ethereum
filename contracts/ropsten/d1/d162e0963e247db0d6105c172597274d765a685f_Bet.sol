...
{    
	 ...
	 function GetLuckyResults() public view returns(bool[])
	 {
         bool[] memory ret = new bool[](luckyLandIds.length);
		 for (uint i = 0; i < luckyLandIds.length; i++) 
         {
			address[] memory luckyGambs = GetLuckyGamblers(i);
            ret[i] = (luckyGambs.length > 0);
         }   
		 return ret;
	 }
