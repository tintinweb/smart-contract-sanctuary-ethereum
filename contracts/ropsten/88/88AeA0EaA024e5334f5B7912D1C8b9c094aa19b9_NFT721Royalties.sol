...
abstract contract AbstractRoyalties is Initializable {
    
    mapping (uint256 => LibPart.Part[]) public royalties;

    function AbstractRoyalties_init() public initializer {
        AbstractRoyalties_init_unchained();
    }
    function AbstractRoyalties_init_unchained() internal {}
/**function that called for saving all the data about royalties for given tokenID */
    function _saveRoyalties (
        uint256 _id,
        LibPart.Part[] memory _royalties
    ) internal {
        for(uint8 i = 0; i < _royalties.length; i++){
            
            require(
                _royalties[i].account != address(0),
                "AbstractRoyalties: Address Should be provided (address(0) Present)"
            );

            require(
                _royalties[i].value != 0,
                "AbstractRoyalties: Royalty value should be provided"
            );

            royalties[i].push(_royalties[i]);
        }
        _onRoyaltiesSet(_id,_royalties);
    }

    function _getV2Royalties(uint256 _id) internal view returns(LibPart.Part[] memory){
        return royalties[_id];
    }
/**if you want to update information about royalties holder */
    function _updateAccount(
        ...
        for(uint8 i = 0; i < length; i++){
            if(royalties[_id][i].account == _from) {
                ...
                /** payable(address(uint160(address))) this statement
                changes a normal address to payable address and allows it to store */
                royalties[_id][i].account = payable(address(uint160(_to)));
            }
        }
    }

/* function to be implemented by contract inheriting this contract */
...
contract RoyaltiesV2Impl is Initializable,
AbstractRoyalties {

    function __RoyaltiesV2Impl_init() public initializer{
        __RoyaltiesV2Impl_init_unchained();
    }
    function __RoyaltiesV2Impl_init_unchained () internal{}
/**function that emits when new royalties are set */
    event RoyaltiesSet(
        uint256 tokenId, 
        LibPart.Part[] royalties
    );

    /**function to return Royalties information */
    function getRaribleV2Royalties(uint256 id) 
    external view returns(LibPart.Part[] memory){
        return _getV2Royalties(id);
    }

/* called at last of the function _saveRoyalties in
