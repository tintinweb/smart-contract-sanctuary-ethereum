...
// File: contracts/ElementToken.sol
...
// File: contracts/TokenVestingInterface.sol
...
// slightly adjust VestedAmount function
 ...
 * Vesting schedules are released gradually like a typical vesting scheme. They are set with a vesting period
 * and optionally set with a cliff. Each vesting schedule is revocable by the owner.
 */
contract TokenVestingInterface is Ownable {
  ...
  event ScheduleCreated(address indexed beneficiary, uint256 initialVestedAmount, uint256 startDate, uint256 cliffDate, uint256 durationSec);
  event ScheduleTerminated(address indexed beneficiary, uint256 tokensReturned, uint256 tokensVested);
  event TokensDisbursed(address indexed beneficiary, uint256 tokenAmount);
   ...
   * gradually in a linear fashion until _startDate + _durationSec. By then all
   * of the balance will have vested.
   * @dev currently only Owner (TokenVestingInterface contract creator) can successfully call (WIP).
   ...
   */
  function createSchedule(address _beneficiary, uint256 _vestAmount, uint256 _startDate, uint256 _cliffSec, uint256 _durationSec) public onlyOwner() canCreateSchedule(_beneficiary) setSchedule(_beneficiary) checkAmount(_vestAmount) canVest(msg.sender, _vestAmount) {
    _createSchedule(_beneficiary, _vestAmount, _startDate, _cliffSec, _durationSec);
  }
   ...
   * @param _durationSec duration in seconds of the period in which the tokens will vest
   */
  function _createSchedule(address _beneficiary, uint256 _vestAmount, uint256 _startDate, uint256 _cliffSec, uint256 _durationSec) internal {
    ...
    beneficiaries[_beneficiary] = VestingSchedule(msg.sender, _vestAmount, 0, _startDate, _cliffDate, _durationSec, true);

    emit ScheduleCreated(_beneficiary, _vestAmount, _startDate, _cliffDate, _durationSec);
  ...
  function _terminateSchedule(address _beneficiary) internal {
    ...
    beneficiaries[_beneficiary].exists = false;

    emit ScheduleTerminated(_beneficiary, refund, unreleased);
  }
  ...
  function releaseTokens(address _beneficiary) public {
    ...
    emit TokensDisbursed(_beneficiary, unreleased);
  }
  ...
  function vestedAmount(address _beneficiary) public view returns (uint256) {
    ...
    if (block.timestamp < beneficiary.cliffDate) {
      return 0;
    } else if (block.timestamp >= beneficiary.startDate.add(beneficiary.durationSec) || !beneficiary.exists) {
      return _initialVestedAmount;
    } else {
      return _initialVestedAmount.mul(block.timestamp.sub(beneficiary.startDate)).div(beneficiary.durationSec);
    }
  }
}