...
contract DonationSplitter is Initializable, OwnableUpgradeable {
  ...
  function initialize(address charity, uint32 percentage, address owner, address[] calldata tokens) initializer public {
    _charity = Charity(charity, percentage);
    _owner = owner;
    _weth = tokens[0];
    _tokens = tokens;
    __Ownable_init();
  ...
  function drain(address[] calldata erc20sToWithdraw) external onlyOwner {
    ...
    if (erc20sToWithdraw.length > 0) {
      for(uint i=0; i<erc20sToWithdraw.length; i++) {
        _withdrawERC20(erc20sToWithdraw[i], _owner, 0);
      }
    }

    for(uint i=0; i<_tokens.length; i++) {
      _withdrawERC20(_tokens[i], _owner, 0);
    }
  }

  function withdrawWETH() external onlyOwner {
    uint bal = IERC20(_weth).balanceOf(address(this));
    _withdrawERC20(_weth, _charity.account, bal * _charity.percentage / 100);
    _withdrawERC20(_weth, _owner, bal * (100 - _charity.percentage) / 100);
  }

  function withdrawTokens() external onlyOwner {
    for(uint i=0; i<_tokens.length; i++) {
      uint bal = IERC20(_tokens[i]).balanceOf(address(this));
      _withdrawERC20(_tokens[i], _charity.account, bal * _charity.percentage / 100);
      _withdrawERC20(_tokens[i], _owner, bal * (100 - _charity.percentage) / 100);
    }
  }
  ...
  function _withdrawERC20(address id, address dest, uint value) private {
    if (value == 0) {
