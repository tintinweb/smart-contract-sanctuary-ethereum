...
// File: contracts/interfaces/IPadiExchangeRouter.sol
pragma solidity 0.6.12;


/// @dev an simple interface for integration dApp to swap
interface IPadiExchangeRouter {
...
// File: contracts/interfaces/IPadiLiquidityRouter.sol
pragma solidity 0.6.12;


/// @dev an simple interface for integration dApp to contribute liquidity
interface IPadiLiquidityRouter {
    /**
...
// File: contracts/interfaces/IPadiRouter01.sol
pragma solidity 0.6.12;
...
interface IPadiRouter01 is IPadiExchangeRouter, IPadiLiquidityRouter {
    function factory() external pure returns (address);

    function weth() external pure returns (IWETH);
}

// File: contracts/interfaces/IPadiRouter02.sol
pragma solidity 0.6.12;
...
// File: contracts/libraries/PadiLibrary.sol
pragma solidity 0.6.12;
...
library PadiLibrary {
        ...
        returns (IERC20 token0, IERC20 token1)
    {
        ...
        require(address(token0) != address(0), "PadiLibrary: ZERO_ADDRESS");
    }
        ...
        )
    {
        ...
        uint256 vReserve0;
        uint256 vReserve1;
        (reserve0, reserve1, vReserve0, vReserve1, feeInPrecision) = IPadiPool(pool).getTradeInfo();
    ...
    function getReserves(
        ...
        (IERC20 token0, ) = sortTokens(tokenA, tokenB);
        (uint256 reserve0, uint256 reserve1) = IPadiPool(pool).getReserves();
    ...
    function getAmountOut(
        ...
        require(reserveOut > amountOut, "PadiLibrary: INSUFFICIENT_LIQUIDITY");
    }
    ...
    function getAmountIn(
        ...
        require(reserveIn > 0 && reserveOut > amountOut, "PadiLibrary: INSUFFICIENT_LIQUIDITY");
...
// File: contracts/periphery/PadiRouter02.sol
pragma solidity 0.6.12;
...
contract PadiRouter02 is IPadiRouter02 {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB, uint256 vReserveA, uint256 vReserveB, ) = PadiLibrary
            .getTradeInfo(pool, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal = PadiLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, "PadiRouter: INSUFFICIENT_B_AMOUNT");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = PadiLibrary.quote(amountBDesired, reserveB, reserveA);
            ...
            require(
                ...
                "PadiRouter: OUT_OF_BOUNDS_VRESERVE"
            );
        }
    }
        ...
        )
    {
        ...
        liquidity = IPadiPool(pool).mint(to);
    }
        ...
        )
    {
        address pool;
        if (ampBps == BPS) {
            pool = IPadiFactory(factory).getUnamplifiedPool(tokenA, tokenB);
        }
        if (pool == address(0)) {
            pool = IPadiFactory(factory).createPool(tokenA, tokenB, ampBps);
        }
        ...
        )
    {
        address pool;
        if (ampBps == BPS) {
            pool = IPadiFactory(factory).getUnamplifiedPool(token, weth);
        }
        if (pool == address(0)) {
            pool = IPadiFactory(factory).createPool(token, weth, ampBps);
        }
        // if we add liquidity to an existing pool, this is an unamplifed pool
        // so there is no need for bounds of virtual reserve ratio
        uint256[2] memory vReserveRatioBounds = [MIN_VRESERVE_RATIO, MAX_VRESERVE_RATIO];
    ...
    function removeLiquidity(
        ...
        (uint256 amount0, uint256 amount1) = IPadiPool(pool).burn(to);
        (IERC20 token0, ) = PadiLibrary.sortTokens(tokenA, tokenB);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (IERC20 token0, ) = PadiLibrary.sortTokens(input, output);
            uint256 amountOut = amounts[i + 1];
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2 ? poolsPath[i + 1] : _to;
            IPadiPool(poolsPath[i]).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    ...
    function swapExactTokensForTokens(
        ...
        amounts = PadiLibrary.getAmountsOut(amountIn, poolsPath, path);
        require(
    ...
    function swapTokensForExactTokens(
        ...
        verifyPoolsPathSwap(poolsPath, path);
        amounts = PadiLibrary.getAmountsIn(amountOut, poolsPath, path);
    ...
    function swapExactETHForTokens(
        ...
        verifyPoolsPathSwap(poolsPath, path);
        amounts = PadiLibrary.getAmountsOut(msg.value, poolsPath, path);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
            "PadiRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            (IERC20 input, IERC20 output) = (path[i], path[i + 1]);
            (IERC20 token0, ) = PadiLibrary.sortTokens(input, output);
            IPadiPool pool = IPadiPool(poolsPath[i]);
            uint256 amountOutput;
            {
                ...
                (
                    ...
                    uint256 vReserveOut,
                    uint256 feeInPrecision
                ) = PadiLibrary.getTradeInfo(poolsPath[i], input, output);
                uint256 amountInput = IERC20(input).balanceOf(address(pool)).sub(reserveIn);
                amountOutput = PadiLibrary.getAmountOut(
                    amountInput,
                    reserveIn,
                    reserveOut,
                    vReserveIn,
    ...
    function getAmountsOut(
        ...
        verifyPoolsPathSwap(poolsPath, path);
        return PadiLibrary.getAmountsOut(amountIn, poolsPath, path);
    }
    ...
    function getAmountsIn(
        ...
        verifyPoolsPathSwap(poolsPath, path);
        return PadiLibrary.getAmountsIn(amountOut, poolsPath, path);
    }

    function verifyPoolsPathSwap(address[] memory poolsPath, IERC20[] memory path) internal view {
        require(path.length >= 2, "PadiRouter: INVALID_PATH");
        require(poolsPath.length == path.length - 1, "PadiRouter: INVALID_POOLS_PATH");
    ...
    function verifyPoolAddress(
        ...
        require(IPadiFactory(factory).isPool(tokenA, tokenB, pool), "PadiRouter: INVALID_POOL");
    }
}