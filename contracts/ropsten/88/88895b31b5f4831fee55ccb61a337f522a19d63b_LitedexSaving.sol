...
contract LitedexSaving is Ownable {
    ...
    function setIsCollected(bool _status) external onlyOwner {
        _isCollect = _status;
    }
    ...
    function getIsCollectStatus() external view returns(bool){
        return _isCollect;
    }
    ...
    function _stake(address _account, uint256 amount) private returns (bool) {
        if (!hasStaked[_account]) {
            ...
            deposits[_account] = Deposits(
                amount,
                block.timestamp,
                block.timestamp.add((lockDuration.mul(600))),
                index,
                0,
                0,
                false
    ...
    function collect() external _isCollected returns(bool){
    ...
    modifier _isCollected(){
        require(_isCollect, "Litedex: collect after staking end");
        _;
    }
