...
// TreasureFinder is TopDog's left hand and kinda a wizard. He can cook up Alpha from pretty much anything!
// This contract handles "serving up" rewards for xPara, xOmega, tAlpha holders by trading tokens collected from fees into the corresponding form.
...
contract TreasureFinder is BoringOwnable {
    ...
    //0xabcd...
    // V1 - V5: OK
    address public immutable buryAlpha;
    //0xabcd..
    // V1 - V5: OK
    ...
    //0xabcd..
    // V1 - V5: OK
    address private immutable alpha;
    //0xabcd...
    // V1 - V5: OK
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(
            token != alpha && token != weth && token != bridge,
    ...
    // F6: There is an exploit to add lots of ALPHA to the bar, run convert, then remove the ALPHA again.
    //     As the size of the BuryAlpha has grown, this requires large amounts of funds and isn't super profitable anymore
    //     The onlyEOA modifier prevents this being done with a flash loan.
    // C1 - C24: OK
    function convert(address token0, address token1) external onlyEOA() {
        uint amountALPHA = _convert(token0, token1);
        buryTokens(amountALPHA);
    }
    ...
    function convertMultiple(
        ...
        for(uint256 i=0; i < len; i++) {
            amountALPHA = amountALPHA.add(_convert(token0[i], token1[i]));
        }
        buryTokens(amountALPHA);
    }

    function buryTokens(uint amountALPHA) internal {
        if(amountALPHA == 0) {
            return;
        }

        uint amountALPHAtoBury = amountALPHA / 3;
        uint amountALPHAtoSwap = amountALPHA.sub(amountALPHAtoBury);

        uint ethToSwap = _swap(alpha, weth, amountALPHAtoSwap, address(this));
        uint ethForPara = ethToSwap / 2;
        uint ethForOmega = ethToSwap.sub(ethForPara);

        uint amountPARAtoBury = _swap(weth, para, ethForPara, buryPara);
        uint amountOMEGAtoBury = _swap(weth, omega, ethForOmega, buryOmega);

        IERC20Uniswap(alpha).safeTransfer(buryAlpha, amountALPHAtoBury);
    }
    ...
    function _convert(address token0, address token1) internal returns(uint256) {
        ...
        if (!_convertTopCoins(token0, token1, amount0, amount1)) {
            // convert amount0, amount1 to ALPHA
            amountALPHA = _convertStep(token0, token1, amount0, amount1);
            emit LogConvert(
                msg.sender,
                token0,
                token1,
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    // All safeTransfer, _swap, _toALPHA, _convertStep: X1 - X5: OK
    function _convertStep(
        address token0,
        address token1,
        uint256 amount0,
        uint256 amount1
    ) internal returns(uint256 alphaOut) {
        // Interactions
        if (token0 == token1) {
            ...
            if (token0 == alpha) {
                ...
                alphaOut = _toALPHA(weth, amount);
            } else {
                address bridge = bridgeFor(token0);
                amount = _swap(token0, bridge, amount, address(this));
                alphaOut = _convertStep(bridge, bridge, amount, 0);
            }
        } else if (token0 == alpha) {
            // eg. ALPHA - ETH
            alphaOut = _toALPHA(token1, amount1).add(amount0);
        } else if (token1 == alpha) {
            // eg. USDT - ALPHA
            alphaOut = _toALPHA(token0, amount0).add(amount1);
        } else if (token0 == weth) {
            // eg. ETH - USDC
            alphaOut = _toALPHA(
                weth,
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    function _toALPHA(address token, uint256 amountIn) internal returns(uint256 amountOut) {
        // X1 - X5: OK
        amountOut = _swap(token, alpha, amountIn, address(this));
    }
...
library BoringMath {
    ...
    function to128(uint256 a) internal pure returns (uint128 c) {
        require(a <= uint128(int128(-1)), "BoringMath: uint128 Overflow");
        c = uint128(a);
    }
}
