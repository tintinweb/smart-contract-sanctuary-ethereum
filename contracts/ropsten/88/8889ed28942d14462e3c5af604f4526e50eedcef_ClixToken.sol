...
contract ClixToken is ERC20, Ownable {
    ...
    modifier onlyFirstTime() {
        require(!hasReceived[msg.sender]);
        _;
    }
    ...
    function() external payable {
        ...
        if (hasReceived[msg.sender]) {
            // Function for returning buyers
            buyClixToken(); 

        } else {
            // Function for first time buyers
            getClixToken();
        }
     }
    
    function getClixToken() payable distributionAllowed onlyWhitelist 
     onlyFirstTime public {
    ...
    function buyClixToken() payable distributionAllowed onlyWhitelist public {
        
        require(amountToSend <= totalRemaining);
        require(hasReceived[msg.sender]);

        uint256 tokenReceived = amountToSend.mul(msg.value);
