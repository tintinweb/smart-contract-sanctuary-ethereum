...
contract ERC20 {
    ...
    /*function transfer(address from,address _to, uint256 _value) public returns (bool success) {//the function transfer token from owner account to other account , if success ,return true.
...
contract GameStatus{
   ...
   function getToken(address sender,uint256 score)public
    ...
    {
        erc20.approveContract(sender,1);
        erc20.transferFrom(sender,init_owner,entryPrice);//燒掉滑雪板，確認他有nft
        if(skiboardId!=0)
        {
            erc1155.setApprovalForAllContract(sender,true);
