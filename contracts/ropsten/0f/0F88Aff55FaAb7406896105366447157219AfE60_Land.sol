...
contract Land is ERC721Enumerable, Ownable, ReentrancyGuard, VRFConsumerBase {
    ...
    modifier whenClaimableActive() {
        require(
            claimableActive && !adminClaimStarted,
            "Claimable state is not active"
        );
        _;
    }
    modifier checkMetadataRange(Metadata memory _landMetadata) {
        require(
            _landMetadata.endIndex < MAX_LANDS_WITH_FUTURE,
            "Range upper bound cannot exceed MAX_LANDS_WITH_FUTURE - 1"
        );
        _;
    }
    ...
    modifier onlyContributors(address _contributor) {
        require(
            ...
            "Only contributors can call this method"
        );
        _;
    }
    ...
    modifier onlyFutureMinter() {
        require(
            futureMinter == msg.sender,
            "Only futureMinter can call this method"
        );
        _;
    }

    modifier checkFirstMetadataRange(
        uint256 index,
        uint256 startIndex,
        uint256 endIndex
    ) {
        if (index == 0) {
            require(
                startIndex == 0,
                "For first metadata range lower bound should be 0"
            );
            require(
                endIndex == MAX_LANDS - 1,
                "For first metadata range upper bound should be MAX_LANDS - 1"
            );
        }
        _;
    }
    ...
    constructor(
        ...
        for (uint256 i; i < _contributors.length; ++i) {
            contributors[_contributors[i].contributor] = _contributors[i]
                .amount;
    ...
    function setKycCheckRequired(bool _isKycCheckRequired)
        external
        onlyOperator
    {
    ...
    function getMintPrice() public view whenPublicSaleActive returns (uint256) {
        ...
        if (elapsed < publicSalePriceLoweringDuration) {
            ...
            price =
                ...
                ((publicSaleStartPrice - publicSaleEndingPrice) * elapsed) /
                publicSalePriceLoweringDuration;
        } else {
        ...
        nonReentrant
    {
        ...
        require(
            numLands <= maxMintPerTx,
            "numLands should not exceed maxMintPerTx"
        );
        require(
            numLands + mintedPerAddress[msg.sender] <= maxMintPerAddress,
            "sender address cannot mint more than maxMintPerAddress lands"
        );
        if (isKycCheckRequired) {
            require(
                ...
                "Sender address is not in KYC allowlist"
            );
        } else {
            require(
    ...
    ) external whenClaimableActive {
        require(
            alphaTokenIds.length > 0 || betaTokenIds.length > 0,
            "Should claim at least one land"
        );
        require(
            alphaTokenIds.length + betaTokenIds.length <= MAX_MINT_PER_BLOCK,
            "Input length should be <= MAX_MINT_PER_BLOCK"
        );
    ...
    function startContributorsClaimPeriod() external onlyOperator {
        require(
            !contributorsClaimActive,
            "Contributors claim is already active"
        );
        contributorsClaimActive = true;
        emit ContributorsClaimStart(block.timestamp);
    }

    function stopContributorsClaimPeriod()
        external
        onlyOperator
        whenContributorsClaimActive
    {
        contributorsClaimActive = false;
        emit ContributorsClaimStop(block.timestamp);
    }

    function contributorsClaimLand(uint256 amount, address recipient)
        external
        onlyContributors(msg.sender)
        whenContributorsClaimActive
    {
        require(amount > 0, "Must mint at least one land");
        require(
            amount <= MAX_MINT_PER_BLOCK,
            "amount should not exceed MAX_MINT_PER_BLOCK"
        );
        require(
            amount <= contributors[msg.sender],
            "Contributor cannot claim other lands"
        );
    ...
    function claimUnclaimedAndUnsoldLands(address recipient)
        external
        onlyOwner
    {
    ...
    function claimUnclaimedAndUnsoldLandsWithAmount(
        ...
        require(
            maxAmount <= MAX_MINT_PER_BLOCK,
            "maxAmount cannot exceed MAX_MINT_PER_BLOCK"
        );
        require(
            alphaClaimedAmount < MAX_ALPHA_NFT_AMOUNT ||
                betaClaimedAmount < MAX_BETA_NFT_AMOUNT ||
                mintIndexPublicSaleAndContributors < MAX_LANDS,
            "Max NFT amount already claimed or sold"
        );
        ...
        if (betaClaimedAmount < MAX_BETA_NFT_AMOUNT) {
            ...
            uint256 toMint = leftToBeMinted < maxAmount
                ? leftToBeMinted
        ...
        if (mintIndexPublicSaleAndContributors < MAX_LANDS) {
            uint256 leftToBeMinted = MAX_LANDS -
        ...
        onlyFutureMinter
    {
        ...
        require(
            futureLandsNftIdCurrent < MAX_LANDS_WITH_FUTURE,
            "All future lands were already minted"
        );
        for (
            uint256 claimed;
            claimed < maxAmount &&
    ...
    function loadLandMetadata(Metadata memory _landMetadata)
        external
        onlyOperator
        checkMetadataRange(_landMetadata)
        checkFirstMetadataRange(
            metadataHashes.length,
            _landMetadata.startIndex,
            _landMetadata.endIndex
        )
    {
        ...
        returns (bytes32 requestId)
    {
        require(
            ...
            "Public Sale And Contributors Offset already requested"
        );
        ...
        returns (bytes32 requestId)
    {
        require(
            ...
            "Owner Claim Offset already requested"
        );
        ...
        override
    {
        if (isRandomRequestForPublicSaleAndContributors[requestId]) {
            publicSaleAndContributorsOffset = (randomness %
                (MAX_PUBLIC_SALE_AMOUNT + RESERVED_CONTRIBUTORS_AMOUNT));
