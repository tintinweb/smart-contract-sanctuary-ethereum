...
contract Ruggies is ERC721, Ownable,ReentrancyGuard {
    using Counters for Counters.Counter;
    using SafeMath for uint256;
    using ECDSA for bytes32;
    using Strings for uint256;

    uint256 public MAX_RUGGIES_RUGS = 10000;
    uint256 public MAX_RUGGIES_RUGS_PER_PURCHASE = 1;
    uint256 public MAX_RUGGIES_RUGS_WHITELIST_CAP = 1;
    uint256 public constant RUGGIES_PRICE = 1 wei;
    uint256 public RESERVED_RUGGIES = 0;
    ...
    constructor() ERC721("Ruggies", "RUGGIES") {}
    ...
    function setWhitelistCap(uint256 _whitelist_cap) external onlyOwner {
        require(_whitelist_cap > RESERVED_RUGGIES, "New reserved count must be higher than old");
        RESERVED_RUGGIES = _whitelist_cap;
    }
    ...
    function publicMint(uint256 _quantity) external payable {
        ...
        require(
            _quantity <= MAX_RUGGIES_RUGS_PER_PURCHASE,
    ...
    function _safeMintHeroes(uint256 _quantity) internal {
        ...
        require(
            tokenSupply.current().add(_quantity) <= MAX_RUGGIES_RUGS,
            "This purchase would exceed max supply of Hero Galaxy Heroes"
        );
        require(
            msg.value >= RUGGIES_PRICE.mul(_quantity),
    ...
    function mintReservedHeroes() external onlyOwner {
        ...
        require(
            tokenSupply.current().add(RESERVED_RUGGIES) <= MAX_RUGGIES_RUGS,
            "This mint would exceed max supply of Heroes"
        );

        for (uint256 i = 0; i < RESERVED_RUGGIES; i++) {
            uint256 mintIndex = tokenSupply.current();
