...
contract EnvoyStaking is Pausable, AccessControl {
    ...
    uint256 public dailyBonusRate_1 = 10003271876792519; //1,0003271876792519
    uint256 public dailyBonusRate_3 = 11003271876792519; //1,1003271876792519
    uint256 public dailyBonusRate_6 = 12003271876792519; //1,2003271876792519
    uint256 public dailyBonusRate_12 = 13003271876792519; //1,3003271876792519
    ...
    mapping(uint256 => DBR[]) public dailyBonusRates;
    ...
    function calculateDailyBonusRate(uint256 _lockupPeriod) public view returns(uint256) {
        ...
        for (uint i = 0; i < dailyBonusRates[_lockupPeriod].length; i++) {
            if (dailyBonusRates[_lockupPeriod][i].from <= totalActiveStaked && totalActiveStaked <= dailyBonusRates[_lockupPeriod][i].to && dailyBonusRate < dailyBonusRates[_lockupPeriod][i].dailyBonusRate) {
                dailyBonusRate = dailyBonusRates[_lockupPeriod][i].dailyBonusRate;
            } 
        }

        return dailyBonusRate;
    }
    ...
    function _addStake(address _beneficiary, uint256 _totalStake, uint256 _lockupPeriod, bool _isEmbargo) internal whenNotPaused {
        ...
        Stake memory stake = Stake({exists:true,
                                    ...
                                    dailyBonusRate: calculateDailyBonusRate(_lockupPeriod),
                                    claimed:false,
    ...
    function calculateRewards(address _account, uint256 _date) public view returns (uint256) {
        ...
        uint256 dailyBonusRate = stakes[_account].dailyBonusRate;
    ...
    function _setDailyBonusRate(uint256 _lockupPeriod, uint256 _from, uint256 _to, uint256 _dailyBonusRate) public {
        require(hasRole(RATES_ROLE, msg.sender));
        for (uint i = 0; i < dailyBonusRates[_lockupPeriod].length; i++) {
            if (dailyBonusRates[_lockupPeriod][i].from == _from && dailyBonusRates[_lockupPeriod][i].to == _to) {
                dailyBonusRates[_lockupPeriod][i].dailyBonusRate = _dailyBonusRate;
                dailyBonusRates[_lockupPeriod][i].enabled = true;
                return;
            }
        }

        DBR memory dbr = DBR({from:_from, to:_to, dailyBonusRate:_dailyBonusRate, enabled:true});
        dailyBonusRates[_lockupPeriod].push(dbr);
    }

    function _removeDailyBonusRate(uint256 _lockupPeriod, uint256 _from, uint256 _to) public {
        require(hasRole(RATES_ROLE, msg.sender));

        for (uint i = 0; i < dailyBonusRates[_lockupPeriod].length; i++) {
            if (dailyBonusRates[_lockupPeriod][i].from == _from && dailyBonusRates[_lockupPeriod][i].to == _to) {
                dailyBonusRates[_lockupPeriod][i].enabled = false;
                return;
            }
        }
    ...
    function getStake(address _account) external view returns (bool _exists, uint256 _createdOn, uint256 _initialAmount, uint256 _lockupPeriod, uint256 _dailyBonusRate, bool _claimed, bool _isEmbargo, uint256 _finishesOn, uint256 _rewardsSoFar, uint256 _totalRewards) {
        Stake memory stake = stakes[_account];
        if (!stake.exists) {
            return (false, 0, 0, 0, 0, false, false, 0, 0, 0);
        }
        ...
        return (stake.exists, stake.createdOn, stake.initialAmount, stake.lockupPeriod, stake.dailyBonusRate, stake.claimed, stake.isEmbargo, finishesOn, rewardsSoFar, totalRewards);
    }
