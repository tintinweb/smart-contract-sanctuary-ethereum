...
import {FeeAndBonusPayoutSupportV1} from "../../../common/BonusPayoutSupport/BonusPayoutSupportV1.sol";
...
contract AsksV1_2 is ReentrancyGuard, UniversalExchangeEventV1, IncomingTransferSupportV1, FeeAndBonusPayoutSupportV1, ModuleNamingSupportV1 {
    ...
    constructor(
        ...
        address _marketGovernance,
        address _protocolFeeSettings,
        address _wethAddress
    )
        IncomingTransferSupportV1(_erc20TransferHelper)
        FeeAndBonusPayoutSupportV1(_royaltyEngine, _protocolFeeSettings, _marketGovernance, _wethAddress, ERC721TransferHelper(_erc721TransferHelper).ZMM().registrar())
    ...
    function fillAsk(
        ...
        _handleBonusPayout(_tokenContract, _tokenId, msg.sender, ask.askPrice, ask.askCurrency, USE_ALL_GAS_FLAG);
...
import {IMarketGovernance} from "./IMarketGovernance.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
contract FeeAndBonusPayoutSupportV1 is OutgoingTransferSupportV1 {
    /// @notice The ZORA Module Registrar
    address public immutable registrar;

    IMarketGovernance marketGovernance;

    event BonusPayout(address indexed tokenContract, uint256 indexed tokenId, address recipient, uint256 amount);
    ...
    /// @param _daoGovernance The Market DAO address
    ...
    constructor(
        ...
        marketGovernance = IMarketGovernance(_daoGovernance);
        royaltyEngine = IRoyaltyEngineV1(_royaltyEngine);
        protocolFeeSettings = ZoraProtocolFeeSettings(_protocolFeeSettings);
        registrar = _registrarAddress;
    }

    function setGovernanceAddress(address _daoGovernance) public {
        require(msg.sender == registrar, "setGovernanceAddress only registrar");
        require(
            ERC165Checker.supportsInterface(_daoGovernance, type(IMarketGovernance).interfaceId),
            "setGovernanceAddress must match IMarketGovernance interface"
        );
        marketGovernance = IMarketGovernance(_daoGovernance);
    }

    function _handleBonusPayout(
        address _tokenContract,
        uint256 _tokenId,
        address _buyer,
        uint256 _amount,
        ...
        try this._handleMarketGovernancePayout{gas: gas}(_tokenContract, _tokenId, _buyer, _amount, _payoutCurrency) returns (uint256 remainingFunds) {
    ...
    function _handleMarketGovernancePayout(
        address _tokenContract,
        uint256 _tokenId,
        address _buyer,
        uint256 _amount,
        ...
        require(msg.sender == address(this), "_handleBonusPayout only self callable");

        // Get the royalty recipients and their associated amounts
        (address payable[] memory recipients, uint256[] memory amounts) = marketGovernance.getBoostingReward(_tokenContract, _tokenId, _buyer, _amount);
        ...
        for (uint256 i = 0; i < numRecipients; ) {
            ...
            emit BonusPayout(_tokenContract, _tokenId, recipient, amount);
...
interface IMarketGovernance is IERC165 {
    ...
    /**
     * View only version of getBoostingReward
     * 
     * @param tokenAddress - The address of the token
     * @param tokenId      - The id of the token
     * @param buyer        - The address of the token's buyer
     ...
     * returns Two arrays of equal length, reward recipients and the corresponding amount each recipient should get
     */
    // function getBoostingView(address tokenAddress, uint256 tokenId, address buyer, uint256 value) external view returns(address payable[] memory recipients, uint256[] memory amounts);
}
