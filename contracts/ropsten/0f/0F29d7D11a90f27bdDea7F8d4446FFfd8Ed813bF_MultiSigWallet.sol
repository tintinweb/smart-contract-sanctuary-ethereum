...
contract MultiSigWallet {
    ...
    string constant public login = "xjudaj00";
    ...
    mapping (uint => bool) public executed; // Mapping of Txn IDs to whether they were executed
    ...
    modifier checkValidSettings(uint ownerCount, uint _requiredSigs) {
        ...
        if (ownerCount <= 0){
            revert("Minimum of required owners is 1.");
        }
        _;
    }
    ...
    //     _;
    // }
    modifier replayProtect(uint transactionId) {
        if (executed[transactionId]){
            revert("This transaction was already executed.");
        }
        _;
    } 
        ...
        replayProtect(transactionId)                
    {
        ...
        if (isTxConfirmed(transactionId)) {
            ...
            executed[transactionId] = true; // second part of replay protection        
    ...
    function getOwnersWhoSignedTx(uint transactionId) public view returns (address[] memory)
    {
        uint count = 0;
        address[] memory temp = new address[](MAX_OWNER_COUNT);
        for (uint i = 0; i < owners.length; i++){
            if (signatures[transactionId][owners[i]]){
                temp[count] = owners[i];
                count += 1;
