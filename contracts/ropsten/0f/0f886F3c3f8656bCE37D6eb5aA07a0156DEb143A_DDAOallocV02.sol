...
contract DDAOallocV02 is AccessControl, Ownable
{
	...
	event DDAOAllocate(uint256 number,address payer,address addr, uint256 sale,uint256 level,uint256 amount);
	...
	function Allocate(uint256 sale, uint8 level, address addr, uint256 amount, uint8 token)public
	{
		...
		emit DDAOAllocate(AllocCount,_msgSender(), addr, sale,level, amount);
	}
