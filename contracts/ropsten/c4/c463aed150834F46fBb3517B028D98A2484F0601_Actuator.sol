...
contract Actuator is SynchronV1{
    ...
    */
    event Bidding(address indexed bidder,uint256 offerId,uint256 price,uint256 indexed biddingTime);
    /**
     * @notice Emitted when canceling existing bidding information
    */
    event CancelBidding(address indexed operator,uint256 offerId,uint256 indexed biddingTime);
    /**
    ...
    */
    ...
    /**
      * @notice The option number is preset to verify the validity of order parameters
      ...
      */
    function createOption(OptionParam calldata optionParam) external returns(uint256 optionId){
    ...
    function bidding(uint256 optionId,uint256 price) external returns(uint256 offerId){
        ...
        (uint256 amount,bool authorization) = whetherSufficientNFTForSale(option.optionParam.token, option.optionParam.identifier, option.optionParam.creator);
        require(authorization != false && amount >= option.optionParam.amount,
        ...
        require(hold >= price && allowed >= hold,"Actuator:Insufficient amount");
        ...
        initOfferNumber++;
        emit Bidding(msg.sender, offerId, price, block.timestamp);
    } 
    /**
    ...
    function cancelBidding(uint256 offerId) external {
        ...
        emit CancelBidding(msg.sender, offerId, block.timestamp);
    }
    ...
    function whetherSufficientNFTForSale(
        ...
        (
            ...
            if (IERC165(collection).supportsInterface(INTERFACE_ID_ERC1155)){
                ...
                authorization = IERC1155(collection).isApprovedForAll(spender, address(this));
            }           
    }
