...
contract presale is ReentrancyGuard, Context, Ownable {
    ...
    bool public timeMode = false;
    
    modifier presaleActive() {
        if (timeMode){
            require(block.timestamp >= presaleStartTime && block.timestamp <= presaleStartTime + presaleDuration);
            }
        else{
    ...
    function timeLeft() public view returns(uint256 timeleft){
        if(block.timestamp < presaleStartTime){
            timeleft = presaleDuration;
        }
        uint256 pre = presaleStartTime + presaleDuration;
        timeleft = pre - block.timestamp;
    }
    ...
    function presaleSettings(string memory _name,uint256 hardcap, uint256 _maxPurchaseAmount, uint256 _starttime, uint256 _duration, bool _overflow, bool _whitelist, bool _timedpre) external onlyOwner  {
        presaleName = _name;
        guarenteedRateHardCap = hardcap * 1 ether;
        softCap = guarenteedRateHardCap / 2;
        maxPurchaseAmount = _maxPurchaseAmount * 1 ether;
        overflow = _overflow;
        whitelistMode = _whitelist;
	...
	function publicSale() presaleActive external onlyOwner  {
        whitelistMode = false;
    }
    ...
    function updateTimes(uint256 _start, uint256 _duration) external onlyOwner {
        presaleStartTime = _start;
        presaleDuration = _duration;
    }

    function updatePaymentToken(address _payment) external onlyOwner {
        paymentToken = _payment;
    }
