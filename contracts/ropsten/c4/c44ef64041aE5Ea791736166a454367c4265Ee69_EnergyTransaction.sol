...
contract EnergyTransaction{
	...
	function removeEnergy(uint index) private { // Delete a energy at a certain location
		for(uint i=index;i<Energy.length-1;i++) {
			...
			Energy[i].amount=Energy[i+1].amount;
	...
	event energyAdded(uint _price, uint amount, address _user);
	event energyRemoved(uint _price, uint amount, address _byUser, uint _energyId);

	function addEnergy(uint _price, uint _amount) public { // For string, specifying data location is mandatory. Here specify "calldata"
		energy memory newEnergy; // For struct type, specifying data location is mandatory. Here specify "memory"
		newEnergy.price = _price;
		newEnergy.amount = _amount;
		newEnergy.seller = msg.sender;
		Energy.push(newEnergy);
		emit energyAdded(_price, _amount, msg.sender);
	}
	...
	function buyEnergy(uint _energyId) public payable {
		...
		emit energyRemoved(Energy[_energyId].price, Energy[_energyId].amount, msg.sender, _energyId);
