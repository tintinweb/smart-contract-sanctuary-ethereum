...
contract Loot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getSurprise(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "WEAPON", weapons);
    }
    
    function getName(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "NAME", chestArmor);
    }
    
    function getStyle(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "STYLE", headArmor);
    }
    ...
    function getUtility(uint256 tokenId) public view returns (string memory) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[9] = getStyle(tokenId);

        parts[10] = '</text><text x="10" y="120" class="base">';

        parts[11] = getMaterial(tokenId);
