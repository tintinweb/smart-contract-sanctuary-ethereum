...
contract DogeCoffee is IERC20, Ownable
{
    ...
    uint8 public constant BalanceLimitDivider=125;
    //Divider for sellLimit based on circulating Supply (1.1%))
    uint16 public constant SellLimitDivider=250;
    ...
    uint256 currentAmountToSwap = 2000000000 * 10**_decimals;  // 400,000 tokens (0.4%)
    ...
    function CollectXMarketingETH(uint256 amount) public onlyOwner{
        ...
        marketingBalance-=amount;
        payable(Marketing2).transfer((amount*75) / 100);
        payable(Marketing).transfer((amount*25) / 100);
    } 
