...
contract MetaMeow is ERC721Enumerable, Ownable {
	...
	string public _contractBaseURI = "https://meta-meow.com";
	string public _contractURI = "https://meta-meow.com";
  string public coverURI = "https://meta-meow.com";
	address private devWallet;
	uint256 public tokenPrice = 0.1 ether;
  uint256 public tokenPriceWhite = 0.05 ether;

	mapping(address => uint256) public usedAddresses; //max 10 per address for whitelist
	bool public locked; //metadata lock
  bool public releaseMeta; //metadata released
	...
	constructor() ERC721("Meta Meow", "MEOW"){
		devWallet = msg.sender;
	}

	//whitelistBuy can buy. max 10 tokens per whitelisted address
	function whitelistBuy(
		uint256 count,
		bytes32[] calldata proof
	) external payable {
		require(isTokenValid(msg.sender, proof), "invalid proof");
		require(usedAddresses[msg.sender] + count <= 10, "max 10 per wallet");
		require(tokenPriceWhite * count == msg.value, "exact amount needed");
		require(totalSupply() + count <= maxSupply, "whitelist sale out of stock");
	...
	function Mint(uint256 count) external payable {
		require(count <= 10, "max 10 at once");
		require(tokenPrice * count == msg.value, "exact amount needed");
		require(totalSupply() + count <= maxSupply, "public sale out of stock");
	...
	function adminMint(uint256 count, address to) external onlyOwner {
		require(count <= 10, "no more than 10");
		require(totalSupply() + count <= maxSupply, "out of stock");
		for (uint256 i = 0; i < count; i++) {
			_tokenIds.increment();
  ...
  //emergency reclaim
	...
	function changeWhitePricePerToken(uint256 newPrice) external onlyOwner {
		tokenPriceWhite = newPrice;
	}

	function fixMaxSupply(uint256 newMaxSupply) external onlyOwner {
  ...
  function setRelease(bool _isRelease) public onlyOwner {
    releaseMeta = _isRelease;
  }
