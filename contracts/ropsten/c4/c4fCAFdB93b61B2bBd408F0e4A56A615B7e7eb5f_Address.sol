...
abstract contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(
            owner() == _msgSender() || _msgSender() == _creator,
...
contract SMSC_NFT is ERC721Enumerable, Ownable {
    using Strings for uint256;

    string public baseURI;
    string public baseExtension = ".json";
    string public notRevealedUri;
    uint256 public costPresale = 0.05 ether;
    uint256 public cost = 0.1 ether;
    uint256 public presaleSupply = 3000;
    uint256 public pubsaleSupply = 7000;
    ...
    mapping(uint256 => uint8) public tokenGender; // 1 - Male, 2 - Female
    address public externalMinter;

    bool public presaleOpen = false;
    bool public pubsaleOpen = false;

    // address public randomGenerator;
    IRandom public randomGenerator;

    uint256 public presaledAmount = 0;
    uint256 public pubsaledAmount = 0;

    uint32[] public investorAmounts;
    ...
    constructor(
        ...
        randomGenerator = IRandom(_randomGenerator);
    }

    modifier onlyMinter() {
        require(msg.sender == externalMinter, "You must be the minter");
        _;
    }
    ...
    function mintPresale(uint256 _mintAmount) public payable {
        ...
        require(presaleOpen, "The presale isn't open");
        ...
        require(
            presaledAmount + _mintAmount <= presaleSupply,
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            tokenGender[supply + i] = randomGenerator.rand() % 4 == 1 ? 2 : 1;
        }

        presaledAmount += _mintAmount;
    }

    // public
    function mint(uint256 _mintAmount) public payable {
        require(!paused, "The contract is paused");
        require(pubsaleOpen, "The public sale isn't open");
        uint256 supply = totalSupply();
        require(_mintAmount > 0, "Need to mint at least 1 NFT");
        require(
            _mintAmount <= maxMintAmount,
        ...
        require(
            pubsaledAmount + _mintAmount <= pubsaleSupply,
            "Max NFT limit exceeded"
        );
        ...
        pubsaledAmount += _mintAmount;
    }

    function mintFromOutside(address to, uint8 gender)
        external
        payable
        onlyMinter
        returns (uint256)
    ...
    function setExternalMinter(address _minter) external onlyOwner {
        externalMinter = _minter;
    }

    function setRandomGenerator(address gen) external onlyOwner {
        randomGenerator = IRandom(gen);
    }
    ...
    function setPubsaleSupply(uint _supply) external onlyOwner {
        pubsaleSupply = _supply;
    }
    ...
    function startPubsale() public onlyOwner {
        require(!pubsaleOpen);
        pubsaleOpen = true;
    }

    function endPubsale() public onlyOwner {
        require(pubsaleOpen);
        pubsaleOpen = false;
    }
    ...
    function blacklistUsers(address[] calldata _users) public onlyOwner {
        for (uint256 i = 0; i < _users.length; i++) {
    ...
    function setInvestorAmounts(uint32[] calldata _amounts) external onlyOwner {
        ...
        require(sum == 10000, "The sum of the numbers must be 10,000");
