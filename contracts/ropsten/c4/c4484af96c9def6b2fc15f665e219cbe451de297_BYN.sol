...
contract manageAddress is Variable, Modifiers, Event {
    ...
    function add_timeAddress(address _address, uint8 total_month)
        public
        isOwner
    {
    ...
    function add_timeAddressApply(address _address, uint256 lock_balance)
        public
        isOwner
    {
        if (balanceOf[_address] >= lock_balance && lock_balance > 0) {
            ...
            lockPermitBalance[_address] = 0;
            for (uint8 i = 0; i < lockCountMonth[_address]; i++) {
                lockBalance[_address][i] = (
                    lock_balance.mul(lockPercent[_address][i])
                ).div(100);
                if (i > 0) {
    ...
    function refresh_lockPermitBalance(address _address) public {
        ...
        for (uint8 i = 0; i < lockCountMonth[msg.sender]; i++) {
            if (
                now >= lockTime[_address][i] && lockCheck[_address][i] == false
            ) {
...
contract Admin is Variable, Modifiers, Event {
    ...
    function admin_tokenBurn(uint256 _value)
        public
        isOwner
...
contract Set is Variable, Modifiers, Event {
    function setTransferLock(bool _transferLock)
        public
        isOwner
        returns (bool success)
    {
...
contract BYN is Variable, Event, Get, Set, Admin, manageAddress {
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        ...
        if (lockTimeAddress[msg.sender]) {
            lockPermitBalance[msg.sender] = lockPermitBalance[msg.sender].sub(
                _value
            );
        }
