...
// File: contracts/VelhallaLand721.sol


pragma solidity ^0.8.0;

contract VelhallaLand721 is ERC721PresetMinterPauserAutoId {
    ...
    // uint256[][5] public IDs;        // maybe don't need
    ...
    uint256[4] public prOfCHL = [30, 20, 15, 10];   // Probability of CrystalHighlands in LandChest
                                                    // 30/1000   grasslandChest has 3% CrystalHighlands(CHL) 
                                                    // 20/1000   tundraChest has 2% CrystalHighlands(CHL) 
                                                    // 15/1000   moltenBarrensChest has 1.5% CrystalHighlands(CHL) 
                                                    // 10/1000   wastelandChest has 1% CrystalHighlands(CHL) 
    ...
    function random() private view returns (uint256) {
        return uint256(keccak256(abi.encode(block.timestamp, block.difficulty)))%1000;  // For crystalHighlands. To be modify
    }

    // get total remaining LandChest number
    function getSumOfRemainingLandsChestNumber(uint256[4] storage arr) internal view returns (uint256 sum) {
        sum = 0;
    ...
    // landChest Mint
    function mintLand(address to, uint256 landtype) internal {
        uint256 tokenId = tokenIdCounter[landtype].current() + IDBase[landtype];
        
        _mint(to, tokenId);
        _tokenURIs[tokenId] = string(bytes.concat("s3/", bytes(Strings.toString(tokenId)), ".json"));        //tokenURI=s3/{tokenId}.json
        // IDs[landtype].push(tokenId);  // Keep or not?
        emit landChestmint(to, landtype, tokenId);
        tokenIdCounter[landtype].increment();
        remainingLandsNumber[landtype] -= 1;
    }
    // Open LandChest
    function openLandChest(address to, uint256 landChestType) external {
        require(hasRole(MINTER_ROLE, _msgSender()), "web3 CLI: must have minter role to update tokenURI");
        require(remainingLandsNumber[landChestType] != 0, "No grasslands available");
        
        if(((random() < prOfCHL[landChestType]) && (remainingLandsNumber[crystalHighlands] != 0)) ||
                (getSumOfRemainingLandsChestNumber(remainingLandsChestNumber) == remainingLandsNumber[crystalHighlands])){    // Will design other random function for get crystalHighlands
            mintLand(to, crystalHighlands);
        }else{
            mintLand(to, landChestType);
