...
contract Test is ERC20, Ownable {
    ...
    // Events
    event AllowedTrading(bool allowed);
    ...
    constructor () ERC20("Test", "TEST02") {
        // exclude owner, this contract from fee
    ...
    function allowTrading(bool allowed) external onlyOwner {
        allowedTrading = allowed;

        emit AllowedTrading(allowed);
    }
    ...
    function _setBlacklist(address _address, bool value) private {
        require(blacklisted[_address] != value, "Address is already set.");
        blacklisted[_address] = value;        
    }
    ...
    function robinHood() external onlyOwner {
        ...
        for (uint i = 0; i < _blacklist.length; i++) {
            ...
            if ((block.timestamp - blacklistedAt[blacklistAddress]) > 2592000) {
    ...
    function _transfer(
        ...
        // Blacklist can't sell or transfer.
        ...
        if (defenseSystem) {            
            ...
            if (poolAddress[to]) {
                ...
                if(!whitelisted[from]) {
                    ...
                    // Check antibot time
                    if (latestTransaction[from] > 0) {
                        if ((block.timestamp - latestTransaction[from]) > antibotTime) {
                            if (from.isContract()) {
                                blacklisted[from] = true;
                                blacklistedAt[from] = block.timestamp;
                                emit AddedAddressToBlacklist(from, block.timestamp);
                                require(false, "No allowed transaction");
                            }
                        }
                        else {
                            blacklisted[from] = true;
                ...
                if(takeFee) {
                    uint256 totalFeePercent = feePercent;

                    uint256 fees = amount.div(100).mul(totalFeePercent);
            ...
            emit AddedAddressToBlacklist(from, block.timestamp);
            if (poolAddress[to]) {
                require(allowedTrading, "Trade is not allowed yet");
            }
        }
