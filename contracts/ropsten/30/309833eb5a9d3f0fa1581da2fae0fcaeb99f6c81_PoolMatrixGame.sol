...
contract PoolMatrixGame {
    ...
    */
    ...
    constructor() {
        ...
        creationTimestamp = block.timestamp; // 1662814800 = GMT: Saturday, 10 September 2022, 13:00:00
    ...
    function buyLevel(uint8 level, uint256 investAmount) public payable {
        ...
        require(block.timestamp < creationTimestamp + levelIntervals[level], "Slot is not available yet");
    ...
    function getSlots(address userAddr) public view returns(int16[] memory) {
        ...
        for (uint8 level = 0; level < levelsCount; level++) {
            if (block.timestamp < creationTimestamp + levelIntervals[level]) {
                slots[level] = -1; // Not available yet
                continue;
            }

            if (!users[userAddr].levels[level].opened) {
                if (!users[userAddr].levels[level].openedOnce)
                    slots[level] = -2; // Available for opening
                else
                    slots[level] = -3; // Available for reopening
            ...
            for (uint i = headIndex[level]; i < levelQueue[level].length; i++) {
                ...
                if (levelQueue[level][i] == userAddr) {
                    ...
                    slots[level] = int16((n - place) * 1000 / n); // Slot is opened
                    break;
                }
            }
        }
