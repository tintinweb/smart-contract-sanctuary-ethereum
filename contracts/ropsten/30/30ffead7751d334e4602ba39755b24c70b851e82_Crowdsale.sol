...
contract TokenVesting is Ownable {
    ...
    uint256 public presaleBonus;    // bonus in presale in %
    bool crowdsaleClosed;
    
    // @notice Specify address of token contract for case with white list required
    // @param _tokenAddress {address} address of token contract
    // @param _start {uint256} date of the vesting start
    // @param _cliff {uint256} vesting clif, time when vesting should start counting down
    // @param _duration {uint256} duration in hours in which vesting should take place
    // @param _presaleBonus {uint256} presale bonus in %
    ...
    // @notice Show amount of token still available for vesting 
    // @return amount {uint} tokens still available for vesting
    // @return decimals {uint} amount of decimals for token
...
contract Crowdsale is Pausable, TokenVesting {
    ...
    uint public tokensSentMain;         // Tokens sent during pubic sale
    ...
    uint public refundCount;            // How many refunds were processed
    uint public totalRefunded;          // Amount of total refunded ETH
    uint public campaignDurationDays;   // campaign duration in days 
    uint public firstPeriod;            // Length of first bonus period in blocks
    uint public secondPeriod;           // Length of second bonus period in blocks 
    uint public thirdPeriod;            // Lenght of third bonus period in blocks 
    ...
    bool public codeExportEnabled;            // True if user paid to view the code
    uint public accessCodePrice;        // Cost of code expoert
    ...
    address[] public devIndex;          // to be able to iterate through devs when distributing the tokens
    ...
    function Crowdsale(uint _decimalPoints,
        ...
        accessCodePrice = 10 ether;

        if (msg.value == accessCodePrice) {
    ...
    function enableCodeExport() external payable {
    ...
    function finalize() external onlyOwner() {
        ...
        // transfer remainng funds to the campaign wallet
    ...
    // and contract switched to mode refunding. Exact amount of ether needs to be sent
    // to refund everybody 
