...
contract MetaTopeAuction is Ownable, MetaTopeIERC1155Receiver {
     ...
     */
    function start(uint256 _totalTokenAmount, uint256 _minPricePerToken)
        external
        onlyOwner
    {
        ...
        endAt = startAt + 2 days; // todo discuss auction duration
     ...
     */
    function cancel() external onlyStarted onlyWhitelisted {
    ...
    function claim(address[] memory _bidders) external onlyOwner onlyEnded {
        for (uint256 i = 0; i < _bidders.length; ++i) {
            ...
            if (_approvedAmount >= bidders[_bidder].totalPrice) {
