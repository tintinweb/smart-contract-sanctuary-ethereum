...
import {UniLib} from "./libs/UniLib.sol";

// contracts
import {FlashSwap} from "./FlashSwap.sol";

contract SqueethSaver is FlashSwap, IERC721Receiver {
    ...
    /// @dev params for flashMakeMoneyPGA()
    struct FlashMakeMoneyPGAParams {
        uint256 vaultId;
    ...
    /// @dev params for flashMakeMoneyFB()
    struct FlashMakeMoneyFBParams {
        uint256 vaultId;
        uint256 collateralToDeposit;
        uint256 wPowerPerpAmount;
        uint256 minToReceive;
        uint256 rewardToCoinbase;
    }

    constructor(address _controller, address _uniswapFactory)
        FlashSwap(_uniswapFactory)
    {
        ...
        quoteCurrency = IController(_controller).quoteCurrency();
        ethQuoteCurrencyPool = IController(_controller).ethQuoteCurrencyPool();
    ...
    function flashMakeMoneyPGA(FlashMakeMoneyPGAParams calldata _params)
        external
        payable
    {
        require(_isThereFreeMoney(), "failed");

        // flashswap _params.wPowerPerpAmount for _params.minToReceive WETH, calling FLASH_MAKE_MONEY_PGA callback and passing encoded _params
        exactInFlashSwapInternal(
            wPowerPerp,
            weth,
            IUniswapV3Pool(wPowerPerpPool).fee(),
            _params.wPowerPerpAmount,
            _params.minToReceive,
            uint8(CALLBACK_SOURCE.FLASH_MAKE_MONEY),
            abi.encode(_params)
        );

        // send back all ETH in this contract to msg.sender
        payable(msg.sender).sendValue(address(this).balance);
    }

    function flashMakeMoneyFB(FlashMakeMoneyFBParams calldata _params)
        external
        payable
    {
        // flashswap _params.wPowerPerpAmount for _params.minToReceive WETH, calling FLASH_MAKE_MONEY_PGA callback and passing encoded _params
        ...
        // send _params.rewardToCoinbase ETH to block miner
        block.coinbase.transfer(_params.rewardToCoinbase);

        // send back all ETH in this contract to msg.sender
        payable(msg.sender).sendValue(address(this).balance);
    }

    function isThereFreeMoney() external view returns (bool) {
        return _isThereFreeMoney();
    }
    ...
    function swapCallbackInternal(
        ...
        if (CALLBACK_SOURCE(_callSource) == CALLBACK_SOURCE.FLASH_MAKE_MONEY) {
            FlashMakeMoneyPGAParams memory data = abi.decode(
                _callData,
                (FlashMakeMoneyPGAParams)
            );
    ...
    function _getCollateralRequired(uint256 _currentEthPrice)
        private
        view
        returns (uint256)
    {
        uint256 normFactor = IController(controller)
    ...
    function _isThereFreeMoney() private view returns (bool) {
        uint256 oSqthEthPrice = UniLib.getTokenPriceInternal(
            wPowerPerpPool,
            wPowerPerp,
            weth
        );
        uint256 ethQuoteCurrencyPrice = IOracle(oracle).getTwap(
        ...
        uint256 collatRequired = _getCollateralRequired(ethQuoteCurrencyPrice);

        return oSqthEthPrice > collatRequired;
    }
}
...
library UniLib {
    ...
    /**
     * @notice get the token price of the given pool converted with base & quote token decimals
     ...
     */
    function getTokenPriceInternal(address _pool, address _baseToken, address _quoteToken) internal view returns (uint256) {
        uint256 quoteTokenPrice = _getRawTokenPrice(_pool, _baseToken, _quoteToken);

        uint8 baseDecimals = IERC20(_baseToken).decimals();
        uint8 quoteDecimals = IERC20(_quoteToken).decimals();

        if (baseDecimals == quoteDecimals) return quoteTokenPrice;

        // if quote token has less decimals, the returned quoteAmountOut will be lower, need to scale up by decimal difference
        if (baseDecimals > quoteDecimals) return quoteTokenPrice.mul(10**(baseDecimals - quoteDecimals));

        // if quote token has more decimals, the returned quoteAmountOut will be higher, need to scale down by decimal difference
        return quoteTokenPrice.div(10**(quoteDecimals - baseDecimals));
    }

    /**
     * @notice get the raw token price of the given pool
     * @param _pool address of the uniswap pool
     * @param baseToken address of an ERC20 token contract used as the baseAmount denomination
     * @param quoteToken address of an ERC20 token contract used as the quoteAmount denomination
     */
    function _getRawTokenPrice(address _pool, address baseToken, address quoteToken) private view returns (uint256 tokenPrice) {
        IUniswapV3Pool pool = IUniswapV3Pool(_pool);
        (uint160 sqrtPriceX96, , , , , , ) = pool.slot0();

        // Calculate tokenprice with better precision if it doesn't overflow when multiplied by itself
        if (sqrtPriceX96 <= type(uint128).max) {
            uint256 ratioX192 = uint256(sqrtPriceX96) * sqrtPriceX96;
            tokenPrice = baseToken < quoteToken
                ? FullMath.mulDiv(ratioX192, 1e18, 1 << 192)
                : FullMath.mulDiv(1 << 192, 1e18, ratioX192);
        } else {
            // need to double check if this is right way to do. Kinda copied from OracleLibrary.sol
...
contract FlashSwap is IUniswapV3SwapCallback {
    ...
    function uniswapV3SwapCallback(
        ...
        swapCallbackInternal(data.caller, tokenIn, tokenOut, fee, amountToPay, data.callData, data.callSource);
    }
     ...
     */
    function exactInFlashSwapInternal(
        address _tokenIn,
        address _tokenOut,
        uint24 _fee,
        uint256 _amountIn,
        ...
        require(amountOut >= _amountOutMinimum, "amount out greater than amount out min");
     ...
     * @param _data arbitrary data assigned with the call
     */
    function exactOutFlashSwapInternal(
        address _tokenIn,
        address _tokenOut,
        uint24 _fee,
        uint256 _amountOut,
