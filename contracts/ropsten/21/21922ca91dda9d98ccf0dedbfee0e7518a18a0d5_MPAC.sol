...
contract MPAC is ERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;

    uint256 public feeDevelopment = 50;
    uint256 public feeDiscretionary
     = 0;
    uint256 public feeChurch = 50;
    ...
    string private _name = "MPAC";
    string private _symbol = "MPAC";
    ...
    address public walletDiscretionary
    ;
    address public walletDevelopment;
    address public walletChurch;
    ...
    constructor (address _walletDiscretionary
    , address _walletDevelopment, address _walletChurch, address _DAOcontrol) ERC20(_name, _symbol) {
        ...
        walletDiscretionary
         = _walletDiscretionary
        ;
        walletDevelopment = _walletDevelopment;
        walletChurch = _walletChurch;
    ...
    function updateWalletDiscretionary
    (address _walletDiscretionary
    ) external {
    ...
    function updateWalletChurch(address _walletChurch) external {
        require(_msgSender() == DAOcontrol);
        walletChurch = _walletChurch;
    }
    ...
    function setFeesPercent(uint256 devFee, uint256 DiscretionaryFee, uint256 ChurchFee) public {
        require(_msgSender() == DAOcontrol);
        feeDevelopment = devFee;
        feeDiscretionary = DiscretionaryFee;
        feeChurch = ChurchFee;
        require(devFee + DiscretionaryFee + ChurchFee <= 490, "Check fee limit");
    }
    ...
    function manualsend() external {
        ...
        uint256 ethDevelopment = amount.mul(feeDevelopment).div(feeDevelopment.add(feeDiscretionary
        ).add(feeChurch));
        uint256 ethDiscretionary
         = amount.mul(feeDiscretionary
        ).div(feeDevelopment.add(feeDiscretionary
        ).add(feeChurch));
        uint256 ethChurch = amount.sub(ethDevelopment).sub(ethDiscretionary
        );
        ...
        if(ethDiscretionary
         > 0)
            payable(walletDiscretionary
            ).transfer(ethDiscretionary
            );
        if(ethChurch > 0)
            payable(walletChurch).transfer(ethChurch);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            uint256 totalFeesPercent = feeDevelopment.add(feeDiscretionary
            ).add(feeChurch);
