...
contract TEST is ERC20, Ownable {
    ...
    // _maxTransfer set to 25 means the max transfer is 0.25% of the total supply
    uint256 internal _maxTransfer = 25;
    ...
    /// @notice Contract TEST balance threshold before `_swap` is invoked
    ...
    /// @notice Change the minimum contract TEST balance before `_swap` gets invoked
    ...
    /// @notice Rescue TEST from the treasury amount
    /// @dev Should only be used in an emergency
    /// @param _amount The amount of TEST to rescue
    /// @param _recipient The recipient of the rescued TEST
    ...
    /// @notice Rescue TEST from the reflection amount
    /// @dev Should only be used in an emergency
    /// @param _amount The amount of TEST to rescue
    /// @param _recipient The recipient of the rescued TEST
    ...
    function getUserTaxRelief(address addr) public view returns (uint8) {
        ...
        if(userTaxReliefStartDate > 0) {
            ...
            uint256 taxReliefPeriod = (block.timestamp - userTaxReliefStartDate) / 120;
            if(taxReliefPeriod > 3) {
                taxReliefInterval = 3;
            } else {
                taxReliefInterval = uint8(taxReliefPeriod);
            }
        }
    ...
    /// @notice Perform a Uniswap v2 swap from TEST to ETH and handle tax distribution
    /// @param amount The amount of TEST to swap in wei
    ...
    /// @notice Transfers TEST from an account to this contract for taxes
    /// @param _account The account to transfer TEST from
    ...
    )
    {
        ...
        reflect = amount * (reflectRate / 100) * (taxRelief / 1000);
        treasury = amount * (treasuryRate / 100) * (taxRelief / 1000);
