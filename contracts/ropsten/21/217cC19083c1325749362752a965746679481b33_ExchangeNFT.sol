...
contract ExchangeNFT is Ownable, TokenRecipient{
        ...
        returns (bool)
    {
        ...
        if(keccak256(bytes(str)) == keccak256(bytes("BUY"))) {
            if (addrs[0] == address(0) || addrs[1] == address(0) || addrs[2] == address(0) || addrs[3] == address(0)) {
                return false;
            }
    ...
    function buy(
        ...
        require(validateTransaction(contracts, addrs, strs[0]));

        uint256 feeValue = uints[0]*10**uints[3];
        uint256 sellValue = uints[1]*10**uints[3];
        uint256 royaltyValue = uints[2]*10**uints[3];
    ...
    function exchange(
        ...
        adminOnly()
    {
        require(validateTransaction(contracts, addrs, ""));
        uint256 feeValue = uints[0]*10**uints[3];
        uint256 sellValue = uints[1]*10**uints[3];
        uint256 royaltyValue = uints[2]*10**uints[3];
        uint256 totalValue =  (feeValue + sellValue + royaltyValue);
        ...
        if(keccak256(bytes(strs[0])) == keccak256(bytes("BUY"))) {
            ...
            exchangeToken = ERC20(contracts[1]);
            if(addrs[1] != address(0)){
                require(exchangeToken.transferFrom(addrs[0], addrs[1], feeValue));
            }
            if(addrs[3] != address(0)){
                require(exchangeToken.transferFrom(addrs[2], addrs[3], sellValue));
            }
            if(addrs[5]!= address(0)){
                require(exchangeToken.transferFrom(addrs[4], addrs[5], royaltyValue));
            }
        }
