...
contract CyberAssassins is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, PaymentSplitter, ReentrancyGuard {
  ...
  uint256 public constant MAX_REGULAR_TOKENS = 12000;
  uint256 public constant MAX_SUPER_TOKENS = 6000;
  uint256 public constant MAX_ULTIMATE_TOKENS = 4000;
  uint256 private constant REGULAR_START_AT = 55;
  uint256 private constant SUPER_START_AT = 12025;
  uint256 private constant ULTIMATE_START_AT = 18012;
   ...
   */
  modifier isValidMerkleProof(bytes32[] memory merkleProof) {
    require(
        MerkleProof.verify(
            merkleProof,
            whitelistMerkleRoot,
   ...
   */
  function setWhitelistMerklyRoot(bytes32 newMerkleRoot) external onlyOwner {
    whitelistMerkleRoot = newMerkleRoot;
  }

  /**
   * @dev Get Merkle Root for Whitelist
   */
  function getWhitelistMerkleRoot() internal view returns (bytes32) {
  ...
  */
   ...
   */
  function mintRegularTokensWhiteList(bytes32[] memory merkleProof, uint256 _count) external payable saleIsOpen nonReentrant whitelistSaleIsOpen isValidMerkleProof(merkleProof) {
    address wallet = _msgSender();
    uint256 total = _tokenIdRegularTracker.current() + REGULAR_START_AT;
    ...
    // Check the balance
    require(msg.value >= whitelistprice(_count), "Not enough ETH for transaction");
