...
abstract contract Mintable is Ownable, IMintable {
    ...
    mapping(uint8 => uint) private drops;
    bool public isPreSale = true; 
    uint8 public round = 0;
    uint MAX_PRESALE = 500;
    ...
    function addAddressToWhitelist(address[] calldata _beneficiaries) external onlyOwner {
    ...
    function mintFor(
        ...
        if(isPreSale) {
            require(whitelistedAddresses[user], "Only whitelisted addresses can mint NFTs in presale");
            if(isFirstPhase) {
                require(!hasClaimedNFT[user], "You have already minted NFT for presale");
