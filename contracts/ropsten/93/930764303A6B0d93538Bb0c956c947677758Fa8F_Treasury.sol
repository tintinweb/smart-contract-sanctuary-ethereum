...
contract Treasury is mortal {
    ...
    mapping(address => uint256) public payInfo;
    ...
    function withdrawAll(
        ...
        payInfo[payable_addresses[0]] = withdraw_amounts[0];
        addresses[0] = payable_addresses[0];
        uint256 j = 0;

        for (uint256 i = 1; i < withdraw_amounts.length; i++) {
            if (payInfo[payable_addresses[i]] == 0) {
                j++;
                infoLength++;
                payInfo[payable_addresses[i]] += withdraw_amounts[i];
                addresses[j] = payable_addresses[i];
            } else {
                payInfo[payable_addresses[i]] += withdraw_amounts[i];
            }
        }
        ...
        for (uint256 i = 0; i < infoLength; i++) {
            daitoken.transfer(addresses[i], payInfo[addresses[i]]);
        }
