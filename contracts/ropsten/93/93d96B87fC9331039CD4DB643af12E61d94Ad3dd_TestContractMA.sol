...
contract TestContractMA is ERC721Enumerable, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    function privateSaleMint(bytes32[] calldata proofWhitelistMerkle, uint32 amount) external payable nonReentrant {
        ...
        if (amount <= 3) {
            require(msg.value >= PRIVATE_SALE_PRICE * amount, "TestContractMA: Insufficient Funds !"); 
        } else if (amount == 4) {
            require(msg.value >= MAX_MINT_PRICE, "TestContractMA: Insufficient Funds !"); 
        }
    ...
    function raffleSaleMint (bytes32[] calldata proofRaffleMerkle, uint32 amount) external payable nonReentrant {
        ...
        if (amount <= 3) {
        require(msg.value >= RAFFLE_SALE_PRICE * amount, "TestContractMA: Insufficient Funds !");
        } else if (amount == 4) {
        require(msg.value >= MAX_MINT_PRICE, "TestContractMA: Insufficient Funds !");
        }
    ...
    function publicSaleMint (uint32 amount) public payable nonReentrant {
        ...
        if (amount <= 3) {
            require(msg.value >= PUBLIC_SALE_PRICE * amount, "TestContractMA: Insufficient Funds !");
        } else if (amount == 4) {
            require(msg.value >= MAX_MINT_PER_PUBLIC_SALE_WALLET, "TestContractMA: Insufficient Funds !");
        }
