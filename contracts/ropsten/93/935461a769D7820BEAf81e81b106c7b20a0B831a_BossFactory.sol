...
import "IBossFactory.sol";


contract BossFactory is ICommonStructs, IBossFactory {
	function newBoss(uint id, address brain, BossConfig calldata config, address owner) external override returns (IBoss) {
...
contract Boss is IBoss, IERC721Receiver, Ownable {
	...
	address public immutable override brain;
	uint public immutable override id;
	...
	constructor(uint id_, address brain_, BossConfig memory config_) Ownable() {
		id = id_;
		brain = brain_;
		_config = config_;
	}
...
interface IBossFactory is ICommonStructs {
	function newBoss(uint id, address brain, BossConfig calldata config, address owner) external returns (IBoss);
}