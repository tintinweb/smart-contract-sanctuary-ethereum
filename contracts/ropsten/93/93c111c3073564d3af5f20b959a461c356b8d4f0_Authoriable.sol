...
contract Authoriable is Ownable {
    ...
    function assignAuthorizeRole(address account) public onlyOwner {
        require(account != address(0), "Authoriable: approved from the zero address");
        authorizeRole = account;
        emit AuthorizeRoleAssigned(account);
    }
}

contract EternalHero is ERC721Enumerable, Ownable, Authoriable {
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        return string(abi.encodePacked(_baseTokenURI, Strings.toString(_tokenId), '.gif'));
    }

    /**
     * @dev Set token URI
     */
    function updateBaseURI(string calldata baseTokenURI) public onlyAdmin {
        _baseTokenURI = baseTokenURI;
    }

    /**
     * See {ERC721-_mint}.
     * Requirements: the caller must have the `onlyAdmin`.
     */
