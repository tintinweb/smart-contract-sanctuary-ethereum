...
interface IMarket {
    function add(address _receive, uint256 _amount) external returns (bool);
}
...
contract XCC is ERC721A, AuthorityControlled, ReentrancyGuard {
    ...
    // event DropParams(uint256 tokens1, uint256 tokens2, uint256 tokens3);
        ...
        for (uint256 i = 0; i < bidRecord.length; i++) {
            ...
            if (have) {
                ...
                if (bidRecord[recordIndex].status == BidStatus.Minted) {
                    // drop token
                    // market.add(bidRecord[recordIndex].addr,tokens_.mul(bidRecord[recordIndex].amount).div(totalBid_));
                    dropToken[bidRecord[recordIndex].addr] = tokens_
                        .mul(bidRecord[recordIndex].amount)
                        .div(totalBid_);
                    //drop interest
                    dropInterest[bidRecord[recordIndex].addr] = bidRecord[
                        recordIndex
                    ].amount.mul(10**10).div(totalBid_);
                    /* emit DropParams(
                        tokens_.mul(bidRecord[recordIndex].amount),
                        tokens_.mul(bidRecord[recordIndex].amount).div(
                            totalBid_
                        ),
    ...
    function setMarket(IMarket _market) public onlyManager {
        require(
            address(0) == address(_market),
            "setMarket: market can't be null address"
        );
        market = _market;
    }
