...
contract DonationSplitter is Initializable, Ownable {
  ...
  function withdrawERC20(address id, address dest, uint value) internal {
    if (value == 0) {
  ...
  function panic(address[] calldata erc20sToWithdraw) external onlyOwner {
    (bool success,) = _owner.call{value: address(this).balance}("");
    withdrawERC20(_weth, _owner, 0);
    for(uint i=0; i<erc20sToWithdraw.length; i++) {
      withdrawERC20(erc20sToWithdraw[i], _owner, 0);
    }
  }

  function withdrawWETH() external {
    uint bal = IERC20(_weth).balanceOf(address(this));
    withdrawERC20(_weth, _charity.account, bal * _charity.percentage / 100);
    withdrawERC20(_weth, _owner, bal * (100 - _charity.percentage) / 100);
  }
