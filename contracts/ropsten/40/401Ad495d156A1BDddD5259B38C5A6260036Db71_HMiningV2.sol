...
contract HMiningV2 is TrustListTools{
  ...
  constructor(address _pool, uint256 _start_round, address _dispatcher) public{
  ...
  event SetCurrentRound(uint256 current_round);
  function set_current_round(uint256 _current_round) public onlyOwner{
    current_round = _current_round;
    emit SetCurrentRound(current_round);
  }
  ...
  function get_total_reward(uint256 ratio, address addr) public view returns(uint256){
    //handle cache
    state_info memory user = user_state[ratio][addr];
  ...
  function get_claimable_reward(uint256 ratio, address addr) public view returns(uint256){
    state_info memory user = user_state[ratio][addr];
    return get_total_reward(ratio, addr).safeSub(user.claimed);
  }
  function get_multi_claimable_reward(uint256[] memory ratio, address addr) public view returns(uint256){
    uint amount = 0;
    for (uint i = 0; i < ratio.length; i++){
      amount = amount.safeAdd(get_claimable_reward(ratio[i], addr));
    }
    return amount;
  }
  ...
  function claim_reward_for(address payable rec, uint256 ratio) public{
    _user_checkpoint(ratio, rec);
    state_info storage user = user_state[ratio][rec];
    uint amount = user.rewards.safeSub(user.claimed);
    user.claimed = user.rewards;
    bank.issue(rec, amount);
    emit ClaimedReward(rec, amount);
  }
  function claim_reward(uint256 ratio) public{
    claim_reward_for(msg.sender, ratio);
  }
  function claim_multi_reward_for(address payable rec, uint256[] memory ratio) public{
    uint256 amount = 0;
    for (uint i = 0; i < ratio.length; i++){
      _user_checkpoint(ratio[i], rec);
      state_info storage user = user_state[ratio[i]][rec];
      amount = amount.safeAdd(user.rewards).safeSub(user.claimed);
      user.claimed = user.rewards;
    }
    bank.issue(rec, amount);
    emit ClaimedReward(rec, amount);
  }
  function claim_multi_reward(uint256[] memory ratio) public{
    claim_multi_reward_for(msg.sender, ratio);
  }
...
contract HMiningV2Factory {
  ...
  function createHMiningV2(address _pool, uint256 _start_round, address _dispatcher) public returns(address){
    HMiningV2 h = new HMiningV2(_pool, _start_round, _dispatcher);
