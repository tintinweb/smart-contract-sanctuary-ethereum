...
//		amount = temp.part_time;
    ...
    function PartTime(uint48 interval)public view returns(uint256 out)
    {
	uint256 sum = TimeEnd - TimeStart;
	uint256 i = uint256(interval) * 10**18;
//	out = i / sum / 10**10;
	out = i / sum;
//	out = uint256(interval);
//	out = out.mul(10**8);
//	out = sum;
//	out = TimeEnd - TimeStart;
    } 
    function PartAmount(uint8 val, uint8 sum)public view returns(uint256 out)
    {
	uint256 i = uint256(val) * 10**18;
//	out = i / uint256(sum) / 10**10;
	out = i / uint256(sum);
    }
}
