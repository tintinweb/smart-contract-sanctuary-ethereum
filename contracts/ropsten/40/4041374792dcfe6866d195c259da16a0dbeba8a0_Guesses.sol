...
contract Guesses is Ownable{
    ...
    uint internal maxId = 4*(10**9);//最大id 4亿
    uint private currentId=0;//当前id
    uint internal winPercent=100;//返奖比例
    ...
    struct Guess {
        address drawer;//发起人
        uint id; //竞猜id
        uint price;//竞猜每张票的价格（单位finny）
        uint startTimestamp;//竞猜开始时间戳(秒)
        uint endTimestamp; //竞猜结束时间戳(秒)
        uint totOptionNum;//选项总额
        bool stoped;//是否被中止了
        bool reward;//是否已发奖 
        mapping(uint=>GuessOption) options;//竞猜选项
    }
    
    //玩家竞猜
    struct GuessOption{
        uint optionIndex;//选项序号
        uint totNum;//总投注数量
        address[] players;//投注用户 
        mapping(address=>uint) bets;//竞猜（地址=>倍率）
        mapping(address=>uint) wins;//奖金 
    }
	 ...
	 * @dev 创建竞猜
	 * @param _id 竞猜id
	 * @param _price 竞猜每张票的价格（单位finney）
     * @param _startTimestamp 竞猜开始时间戳(秒)
     * @param _endTimestamp 竞猜结束时间戳(秒)
     * @param _totOptionNum 竞猜选项总数 
	 * @return 创建的合约id
	 * 
	 */
	function createGuess(uint _id,uint _price, uint _startTimestamp, uint _endTimestamp, uint _totOptionNum) public returns (uint){
	    require(now<_startTimestamp && _endTimestamp>_startTimestamp);
	    require(_price>0&&_id>0);
	    require(_totOptionNum>0&&_totOptionNum<1000);
	    require(allGuesses[_id].id==0);
	    //uint _id=getNextGuessId();
	    allGuesses[_id].id=_id;
	    allGuesses[_id].drawer=msg.sender;
	    allGuesses[_id].price=_price *(10**18);
    ...
    /**
     * @dev 开始竞猜
     * @param  bet 格式：竞猜id_选项index_数量;竞猜id_选项index_数量;竞猜id_选项index_数量... 注意选项index是从1开始
     * @return  成功
     */
    function guess(string bet) public payable returns(bool){
        ...
        for(uint i=0;i<splits.length;i++){
            ...
            //innerGuess(singleBet[0]singleBet[1],singleBet[2],msg.sender);
        }
    ...
    /**
     * @dev 把玩家的竞猜内容放入storage
     * @param _id   竞猜id
     * @param optionIndex  选项index
     * @param num   倍率 
     * @param player 玩家 
     * 
     */ 
    function innerGuess(uint _id,uint optionIndex,uint num,address player) internal{
        ...
        //未停止并且在竞猜时间内 
        require(!allGuesses[_id].stoped &&allGuesses[_id].startTimestamp<now&&allGuesses[_id].endTimestamp>now);
    ...
    /**
     * @dev 设置竞猜答案
     * @param  answers 格式：竞猜id_正确选项index;竞猜id_正确选项index;竞猜id_正确选项index
     * @return  成功
     */
    function setGuessAnswer(string answers) public noReentrancy returns(bool){
        ...
        for(uint i=0;i<splits.length;i++){
            ...
            //竞猜创建者或者合约持有人才能设置答案 
            require(guess.drawer==msg.sender||msg.sender==owner);
            //竞猜确实存在 
    ...
    /**
     * @dev 获取竞猜信息
     * @param  ids 格式：竞猜id_竞猜id_竞猜id_竞猜id_竞猜id
