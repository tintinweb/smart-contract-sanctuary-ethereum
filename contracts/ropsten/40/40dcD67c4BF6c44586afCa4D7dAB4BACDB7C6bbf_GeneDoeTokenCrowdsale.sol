...
// ISSUES TO RESOLVE
/*
 - Pausing the token prevents everything! need to way to prevent investors from dumping tokens during crowdsale that doesn't prevent minting, etc.
 - No "finishMinting" function because MintableToken is no longer a thing unlike the tutorials
 - Make sure admin is only one that can finalize
 - Make sure admin is only one that can pause
...
import "./GeneDoeToken.sol";
import "openzeppelin-solidity/contracts/token/ERC20/utils/TokenTimelock.sol";


contract GeneDoeTokenCrowdsale is MintedCrowdsale, CappedCrowdsale, TimedCrowdsale, AccessControlEnumerable, RefundableCrowdsale, Ownable {

    //Minimum investor contribution - 0.002 Ether
    //Maximum investor contrubiont - 50 Ether
    // Track investor contributions
    uint256 public investorMinCap =  2000000000000000;   // 0.002 Ether (need to make sure)
    uint256 public investorHardCap = 50000000000000000000; //50 Ether (need to make sure)
    ...
    GeneDoeToken public geneDoeToken;
    ...
    //testing some stuff not permanent
    ...
    constructor(
        ...
        RefundableCrowdsale(_goal)
        {
            ...
            testString = "changed in the constructor";
    ...
    function _depositsOf(address investor) public override view returns (uint256) {

        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        return super._depositsOf(investor);
    ...
    */
    ...
    /**
    * @dev Extend parent behavior requiring purchase to repect investor min/max funding cap.
    ...
    */
    function _finalization() internal override  {
        if (goalReached()) {
            ...
            testString = "it changed!";

            uint256 _alreadyMinted = geneDoeToken.totalSupply();

            uint256 _finalTotalSupply = _alreadyMinted.div(tokenSalePercentage).mul(100);

            foundersTimelock   = new TokenTimelock(IERC20(geneDoeToken), foundersFund, releaseTime);
            foundationTimelock = new TokenTimelock(IERC20(geneDoeToken), foundationFund, releaseTime);
            partnersTimelock   = new TokenTimelock(IERC20(geneDoeToken), partnersFund, releaseTime);

            
            geneDoeToken.mint(address(foundersTimelock), _finalTotalSupply.div(foundersPercentage));
            geneDoeToken.mint(address(foundationTimelock), _finalTotalSupply.div(foundationPercentage));
            geneDoeToken.mint(address(partnersTimelock), _finalTotalSupply.div(partnersPercentage));

            //geneDoeToken.unpause();
...
import "openzeppelin-solidity/contracts/utils/escrow/RefundEscrow.sol";
import "openzeppelin-solidity/contracts/access/AccessControlEnumerable.sol";
...
abstract contract RefundableCrowdsale is Context,FinalizableCrowdsale {
    ...
    function _depositsOf(address investor)  public virtual view returns (uint256) {
        return _escrow.depositsOf(investor);
...
contract GeneDoeToken is ERC20PresetMinterPauser, Ownable{
