...
contract BITKOINDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BITKOIN _Dividend_Tracker", "BITKOIN _Dividend_Tracker") {
        claimWait = 432000;
        minimumTokenBalanceForDividends = 250000 * (10**18); //must hold 250k+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BITKOIN _Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BITKOIN _Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BITKOIN  contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 432000, "BITKOIN _Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BITKOIN _Dividend_Tracker: Cannot update claimWait to same value");
...
contract BITKOIN is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    BITKOINDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 42000000 * (10**18); //0.2%
    uint256 public swapTokensAtAmount = 21000000 * (10**18); //0.1%
    ...
    function setFee(uint256 _ethRewardFee, uint256 _liquidityFee) public onlyOwner {
        ...
        totalFees = ETHRewardFee.add(liquidityFee); // total fee transfer and buy
        require(totalFees <= 15, "Must Keep 15% or less");
    }

    function setExtraFeeOnSell(uint256 _extraFeeOnSell) public onlyOwner {
        extraFeeOnSell = _extraFeeOnSell; // extra fee on sell
        require(extraFeeOnSell <= 5, "Must Keep 5% or less");
    }
    ...
    constructor() ERC20("Bitkoin", "BITKOIN") {
        ETHRewardFee = 2;
        liquidityFee = 5;
    	...
    	dividendTracker = new BITKOINDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BITKOIN : The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BITKOIN : Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BITKOIN : The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BITKOIN : Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BITKOIN : gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BITKOIN : Cannot update gasForProcessing to same value");
