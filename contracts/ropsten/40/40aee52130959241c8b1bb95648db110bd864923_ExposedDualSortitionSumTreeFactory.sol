...
// TODO QUERY
    ...
    function drawAndUpdate(DualSortitionSumTrees storage self, bytes32 _key, uint _drawnNumber, bool requestUpdate) public returns(bytes32 ID) {
        ...
        uint setTreeSize = self.sortitionSumSetTrees[_key].nodes.length;

        if (requestUpdate || 2*self.lastThreshholdUpdate < setTreeSize){
            self.lastThreshholdUpdate = setTreeSize;
