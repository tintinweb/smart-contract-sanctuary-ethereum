...
contract SupportTransaction is Ownable
{
    event InSwapp(address user , uint256 amount , bytes32 hash  );
    event OutSwapp(address user , uint256 amount , bytes32 hash  );
   ...
   RealmsOfEthernity tokenaddess ;
    ...
    function swappIN (uint256 amount , bytes calldata signature, bytes32 txhash) public returns (bool)
    {
       ...
       emit InSwapp(msg.sender,amount,txhash);
        return true ;
