...
contract LostBoy is ERC721Enumerable, Ownable {
    ...
    uint256 public constant PRICE_PER_Boy = 1000000000000000000 ;  //1 PrabhToken 

    bool private _saleIsActive;

    address public txFeeToken;

    string private _metaBaseUri = "https://lostboyassets.s3.amazonaws.com/";

    uint [] mintednfts;
  
struct Cassets {
        uint256 tokenid; 
        address cassetownerAddress; 
    }
...
Cassets[] private allCassets;

Vinyl[] private allVinyl;


    constructor(address _txFeeToken) ERC721("LostBoy", "BOY") {
        txFeeToken = _txFeeToken;
        _saleIsActive = true;
    }

    //Send 1 for vinyl and 2 for cassets in TokenType 
    function mint(uint TokenId,uint numberoferc20Tokens , uint TokenType) public  {
        require(!_exists(TokenId) , "Token already minted");
        require(saleIsActive(), "Sale is not active");
        require(PRICE_PER_Boy >= numberoferc20Tokens, "LostBoy Token value sent is incorrect.Need 1 LostBoy Token For Presale for each BOY");
        ...
        require(allCassets.length <=50 ,"Casettes sold out.");
        require(_checkCassets(msg.sender) , "Cannot mint more than 1 casset");
        address owner = owner();
        _payTxFee(msg.sender,owner , numberoferc20Tokens);
        _mintTokens(TokenId,msg.sender);
        mintednfts.push(TokenId);
        Cassets memory newCasset = Cassets({
            tokenid : TokenId,
            cassetownerAddress : msg.sender
        });
        allCassets.push(newCasset);
        }
        else{
...
function _checkVinyl(address owner) internal returns (bool){
...
function _checkCassets(address owner) internal returns (bool){
   if(allCassets.length > 0) {
    for(uint i= 0 ; i< allCassets.length ; i++) {
        if(allCassets[i].cassetownerAddress == owner) {
            return false;
        }
       }
    }
    ...
    function getAllCassets() public view returns (Cassets[] memory) { 
