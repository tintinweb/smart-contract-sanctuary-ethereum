...
/**
 * @dev Stixex main smart contract for receiving investments (only in USDC)
 */
contract Stixex_USDC is BlackList, Pausable {
    using ECDSA for bytes32;
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IERC20 private _usdc;                               // interface of usdc token
    uint256 private _minAllowedAmount = 2 ether;        // 2 USDC default min deposit amount
    ...
    constructor (IERC20 usdc) public {
        require(address(usdc) != address(0), "constructor: invalid usdc token address");
        _usdc = usdc;
    }
    ...
    function deposit(address sender, uint256 amount) public isNotBlocked whenNotPaused amountAllowed(amount) {
        ...
        _usdc.safeTransferFrom(sender, address(this), amount);
    ...
    function _withdrawUsdt(address receiver, uint256 amount) private {
        _usdc.safeTransfer(receiver, amount);
