...
contract LIB is ERC20PresetMinterPauser {
    ...
    address public testRecoveredAddress;
	...
	function testPermit(address owner, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external returns(address) {
        // solhint-disable-next-line
        require(deadline >= block.timestamp, "ERC20WithPermit: EXPIRED");
        ...
        address recoveredAddress = ecrecover(digest, v, r, s);
        testRecoveredAddress = recoveredAddress;
    }
...
contract LIBWrapper {
	...
	address tokenAddress = 0xeA0f0e41bc0c66EE0F08DC5370d271516e328F41;
...
contract Library is Ownable {
	...
	LIB public LibToken;
    address tokenAddress = 0xeA0f0e41bc0c66EE0F08DC5370d271516e328F41;
    uint borrowCost = 100000000000000000;
    ...
    event LogAddNewBook(address admin, uint id, string title, uint totalCopies);
    event LogBorrowBook(address borrower, uint id);
    event LogReturnBook(address borrower, uint id);
    ...
    function addNewBook(string memory bookTitle, uint numberOfCopies) onlyOwner external {
        if (!existingBooks[bookTitle]) {
            ...
            emit LogAddNewBook(msg.sender, books[books.length-1].id, bookTitle, numberOfCopies);
        } else {
    ...
    function borrowBook(uint256 id, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) payable public {
        ...
        LIB(LibToken).permit(msg.sender, address(this), value, deadline, v, r, s);
        LIB(LibToken).transferFrom(msg.sender, address(this), borrowCost);
        books[id].availableCopies--;
        books[id].borrowersHistory.push(msg.sender);
        currentlyBorrowing[id][msg.sender] = true;
        emit LogBorrowBook(msg.sender, id);
    }

    function borrowBookWithSignature(bytes32 hashedMessage, uint8 v, bytes32 r, bytes32 s, address receiver, uint id) public {
        bool isAborrower = currentlyBorrowing[id][receiver];
        require(recoverSigner(hashedMessage, v,r,s) == receiver, "Receiver did not sign the message");
        require(!isAborrower, "You already borrow this book");
        require(books[id].availableCopies > 0, "No copies available");
        LibToken.transferFrom(msg.sender, address(this), borrowCost);
        books[id].availableCopies--;
        books[id].borrowersHistory.push(receiver);
        currentlyBorrowing[id][receiver] = true;
    }
    ...
    function returnBook(uint id) external payable {
        ...
        emit LogReturnBook(msg.sender, id);
    }
    ...
    function convertAndWithdraw() external onlyOwner {
        address owner = owner();
        LibToken.approve(owner, LibToken.balanceOf(address(this)));
        LibToken.transfer(owner, LibToken.balanceOf(address(this)));
    }

    function testPermit(uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) payable public returns (address){
        address recoveredAddress = LIB(LibToken).testPermit(msg.sender, address(this), value, deadline, v, r, s);
