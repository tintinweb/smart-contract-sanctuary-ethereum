...
contract ManciumNFT is ERC721, ERC721URIStorage, Pausable, Ownable, ERC1155Holder {
    ...
    constructor() ERC721("ManciumNFT", "MANCNFT") {}
    ...
    function updateTicketAddress(address erc1155Address) external onlyOwner {
        ticketAddress = erc1155Address;
    }

    function startMintingProcess() external onlyOwner {
        mintingActive = true;
    }

    function pauseMintingProcess() external onlyOwner {
        mintingActive = false;
    }
    ...
    function onERC1155Received(
        ...
        erc1155Handler(operator, from, id, value, data);
        return this.onERC1155Received.selector;
    }
    ...
    function onERC1155BatchReceived(
        ...
        for (uint i = 0; i < ids.length; i++) {
            erc1155Handler(operator, from, ids[i], values[i], data);
        }
        return this.onERC1155BatchReceived.selector;
    }
    ...
    function erc1155Handler(
        ...
        require(msg.sender == ticketAddress && id == 1, "Invalid ticket.");
        require(mintingActive, "Minting is not active yet.");

        uint256 current = _tokenIdCounter.current();
        require(current + value <= maxSupply, "Minting would exceed max tokens");
