...
// File: contracts/ReservationStorage.sol

pragma solidity 0.8.14;

contract ReservationStorage {
    IERC20 public USDT;
    IERC20 public USDC;
    IERC20 public DAI;

    // Reservation amount receiver
    address public owner;
    // Referral Fee in %
    uint256 public referralFee = 5;
    // Referee Fee in %
    uint256 public refereeFee = 5;
    ...
    // Reservation amount $50 in USDT/USDC/DAI
    mapping(address => uint256) public paymentAmount;

    event Reserved(address user, address referral);
    ...
    function transferOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0x00), "Zero newOwner address");
        owner = newOwner;
...
contract Reservation is ReservationStorage {
    ...
    constructor(
        ...
        require(_owner != address(0x00), "Zero address");
        ...
        paymentAmount[_USDT] = 50e6;
        paymentAmount[_USDC] = 50e6;
        paymentAmount[_dai] = 50e18;
    }

    function reserve(address _token, address _referral)
        external
        ifNotPaused
        ifNotContract(_referral)
    {
        address _user = msg.sender;
        require(!userInfo[_user].allow, "Already reserved");

        require(_token != address(0x00), "Zero token address");
        ...
        uint256 _amount = paymentAmount[_token];
        ...
        if (_referral != address(0x00) && (referralFee > 0)) {
            ...
            // transfer tokens
            IERC20(_token).safeTransferFrom(_user, owner, _amount);
            IERC20(_token).safeTransferFrom(_user, _referral, _referralFee);
            // store the referral address
            userInfo[_user].referral = _referral;
        } else {
            // if the  referral is zero address transfer all token to the owner
            IERC20(_token).safeTransferFrom(_user, owner, _amount);
        }
