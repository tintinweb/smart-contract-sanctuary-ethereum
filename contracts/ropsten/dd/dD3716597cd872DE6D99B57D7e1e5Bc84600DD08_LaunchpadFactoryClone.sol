...
contract LaunchpadFactoryClone is ReentrancyGuard, Ownable, Pausable {
    ...
    event CreateLaunch(address indexed token, LaunchpadPresale.Parameters par, LaunchpadPresale.WhitelistParameters wpar, 
        LaunchpadPresale.VestingParameters vpar, uint256 tokens);
    ...
    memory wpar, LaunchpadPresale.VestingParameters memory vpar) public payable nonReentrant whenNotPaused {
        ...
        emit CreateLaunch(token, par, wpar, vpar, tokens);
    }
...
contract LaunchpadPresale is Initializable, ReentrancyGuard, Ownable, Pausable {
    ...
    event Success(address token, uint256 amountToLiquidity, uint256 tokensToLiquidity, uint256 unsold, bool refundType);
    ...
    WhitelistParameters memory _wpar, VestingParameters memory _vpar) public initializer {
     ...
     * Unsold tokens are either burned or returned to owner
     * If fails then return all tokens to owner
     * @param status: 0-fail, 1-success 
     * @param refundType: 0-burn, 1-refund to owner
     */
    function finalize(bool status, bool refundType) external onlyOwner {
        ...
        if (status==true) {
            ...
            if (totalBought < par.hardcap) {
                if (refundType==true) {
                    ...
                    token.safeTransfer(burnAddress, unsold);
                }
            }

            emit Success(address(token), amountToLiquidity, tokensToLiquidity, unsold, refundType);
