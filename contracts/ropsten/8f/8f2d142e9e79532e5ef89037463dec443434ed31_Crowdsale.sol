...
contract Crowdsale is Whitelist {
    ...
    uint256 private _earlyInvestors    =  300000000 * 1 ether;
    ...
    function transferTokensManually(address to, uint256 amount, uint256 id) public onlyManyOwners returns (bool){
        ...
        if(id == 0) {
            ...
            require(_earlyInvestors >= amount, "transferTokensManually: not enough tokens in early investors balance");
            _earlyInvestors = _earlyInvestors.sub(amount);
            _processPurchase(to, amount);
        } else if(id == 5) {
     ...
     * 5 - Equity
     */
    function transferTokensManuallyToMany(address[] addresses, uint256[] amounts, uint256 id) public onlyManyOwners returns (uint256) {
     ...
     */
    function getTokensReserve() external view returns(uint256 teamTokens, uint256 advisorsTokens, uint256 foundationTokens, uint256 syndicateTokens, uint256 earlyTokens, uint256 equityTokens) {
        teamTokens = _teamTokens;
        advisorsTokens = _advisorsTokens;
        foundationTokens = _foundationTokens;
        syndicateTokens = _syndicateTokens;
        earlyTokens = _earlyInvestors;
    ...
    function _detectInvestorGroup(uint256 usdAmount) internal view returns (SaleStage storage) {
        if(usdAmount >= _minLimitPrivateSale && !privateSale.finalized)
            return privateSale;
        else if(usdAmount >= _minLimitPublicFirstSale && !publicFirst.finalized)
            return publicFirst;
        else
            return publicSecond;
    }
