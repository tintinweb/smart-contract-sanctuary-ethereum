...
contract ERC721RExample is ERC721A, Ownable {
    ...
    uint256 public constant OWNER_MAX_MINT = 200;
    uint256 public ownerMinted;
    uint256 public refundFee = 10;
    address public developerWallet = payable(0xfcD8036577262aDC7B8af417fAdD988a744d7546);

    string public HiddenMetadata = 'ipfs://QmbYgXaezsZsGiquXzftdt3TKMLUiVHrsxKjGfsqH5LzMx/';
    ...
    function ownerMint(uint256 quantity) external onlyOwner {
        ...
        require(ownerMinted + quantity <= OWNER_MAX_MINT,
            "Owner Max Mint Limit Reached!"
        );
    ...
    function refund(uint256[] calldata tokenIds) external {
        ...
        uint deduction = ( refundAmount * refundFee ) / 100;   //10% deduction
        uint transferable = refundAmount - deduction;
        Address.sendValue(payable(developerWallet), deduction);
        Address.sendValue(payable(msg.sender), transferable);
    }
    ...
    function setFeeReciever(address _reciever) public onlyOwner {
        developerWallet = _reciever;
    }

    //put 'true' to show the nft and 'false' to show hide
