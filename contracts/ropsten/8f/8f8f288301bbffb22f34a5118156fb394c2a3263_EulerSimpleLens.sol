...
contract EulerSimpleLens is Constants {
    ...
    function interestRate(address underlying) external view returns (int96) {
        return markets.interestRate(underlying);
    }
    ...
    function getAccountStatus(address account) external view returns (uint collateralValue, uint liabilityValue, uint healthScore) {
        IRiskManager r = IRiskManager(euler.moduleIdToProxy(MODULEID__RISK_MANAGER));
        (bool success, bytes memory result) = address(r).staticcall(abi.encodeWithSelector(IRiskManager.computeLiquidity.selector, account));
        if (!success || result.length < 32) return (0, 0, 0);
    ...
    function getAccountBalancesAndDebts(address underlying, address account) external view returns (uint eTokenBalance, uint dTokenBalance, uint pTokenBalance) {
        ...
        if (account != address(0)) {
            eTokenBalance = IERC20(eTokenAddr).balanceOf(account);
            dTokenBalance = IERC20(dTokenAddr).balanceOf(account);
            pTokenBalance = IERC20(pTokenAddr).balanceOf(account);
        } else {
            eTokenBalance = IERC20(eTokenAddr).balanceOf(msg.sender);
    ...
    function interestRatesAPY(address underlying) external view returns (ResponseIRM memory r) {
        ...
        uint32 reserveFee = markets.reserveFee(underlying);
