...
contract EscrowTrade is Initializable, OwnableUpgradeable {
    string public name;
    ...
    function initialize() public initializer {
        ...
        name   = "ESCROW-TRADE";
        symbol = "EP";
        fundWallet = 0xbDed733B4129C5e38221e154C7da6F31f704E886;
        USDT = IERC20(0x185BAB30466bc30b34D73829A19E04124DCA7bd0);
    ...
    // USDT
    function setUSDT (IERC20 _TokenContract) public onlyOwner{
        USDT = _TokenContract;
    }
    ...
    function startTradeUSDT (address _seller, uint _amount) public payable {
        require(pause == false, "contract is paused.");
        require( IERC20(USDT).balanceOf(msg.sender) > 0,  "you don't have enough balance");
        require( IERC20(USDT).allowance(msg.sender, address(this)) >= _amount,  "approve or increase allowance");
        require(_seller != address(0), "invalid seller address");
        require(!id_exist[IDcounter], "trade id already exist");
        ...
        IERC20(USDT).transferFrom(msg.sender, fundWallet, __owner);
        IERC20(USDT).transferFrom(msg.sender, address(this), __seller);
    }
    ...
    function payUSDTtoWinner(uint _tradeId,address winner) internal {
        uint256 __amount = (trades[_tradeId].amount * (1000 - trades[_tradeId].fees)) / 1000;
        IERC20(USDT).transfer(winner, __amount);
        trades[ _tradeId ].state  = 3;
    ...
    function payUSDTtoSeller(uint _tradeId) internal {
        uint256 __seller = (trades[_tradeId].amount * (1000 - trades[_tradeId].fees)) / 1000;
        IERC20(USDT).transfer(trades[_tradeId].seller, __seller);
        trades[ _tradeId ].state  = 3;
        emit CompleteTrade(trades[_tradeId].buyer, trades[_tradeId].seller, __seller, trades[_tradeId].coin);
    }
