...
// Additional method available for reward token
...
// Additional methods available for staked NFT
interface IStakedNFT is IERC721 {
...
contract NFTStaking is Ownable {
    ...
    struct UserInfo {
        uint256 shares;     
        uint256 rewardDebt; 
		mapping(uint256 => bool) stakedIds;
    }

	// NFT
	IERC721 public mainNFT = IERC721(0x3201cfC05d1a3b33663B138Cfc45A2e6090DA2fc);

	// Reward Token
	IRewardToken public rewardToken = IRewardToken(0xf3384A119400815c509744B4A8008eB4fE0DBf64);

	// Staked NFT
	IStakedNFT public stakedNFT = IStakedNFT(0x5d0FeeDf92c3c18594bB0627e0D728cD8Ee5fbC3);

	address public devAddress = 0xE95ba9547735666cfbf284d12EF039Dfc35d74C1;

	uint256 public rewardsPerBlock = 10;
	
    // The block number when staking rewards starts.
    uint256 public startBlock = 12509990;
	...
	function updatePool() public {
		...
		uint256 totalShares = mainNFT.balanceOf(address(this));
		...
		rewardToken.mint(devAddress, accRewards * 3 / 20);
		rewardToken.mint(address(this), accRewards);
	...
	function deposit(uint256[] calldata _tokenIds) public {
		...
		for(uint256 i = 0; i < length; i++) {
			mainNFT.safeTransferFrom(msg.sender, address(this), _tokenIds[i]);
			user.stakedIds[_tokenIds[i]] = true;	
			stakedNFT.mint(msg.sender, _tokenIds[i]);	// Mint StakedNFT upon deposit
		}
	...
	function withdraw(uint256[] calldata _tokenIds) public {
		...
		for (uint256 i; i < length; i++){
			...
			mainNFT.safeTransferFrom(address(this), msg.sender, _tokenIds[i]);

			if (stakedNFT.ownerOf(_tokenIds[i]) != address(0)) {
				stakedNFT.burn(stakedNFT.ownerOf(_tokenIds[i]), _tokenIds[i]);
			}
		}
	...
	// Deprecated, remove before mainnet
	function mintStakedNFT(uint256[] memory _tokenIds) public {
		UserInfo storage user = userInfo[msg.sender];
		...
		for (uint256 i; i < length; i++){
			require(user.stakedIds[_tokenIds[i]], "token not staked");
			stakedNFT.mint(msg.sender, _tokenIds[i]);
		}
	}
	...
	function setStartBlock(uint256 _startBlock) public onlyOwner {
		require(!isStakingActive, "Staking is active");
	...
	function setStakingActive() public onlyOwner {
		require(block.number < startBlock, "Staking already started");
