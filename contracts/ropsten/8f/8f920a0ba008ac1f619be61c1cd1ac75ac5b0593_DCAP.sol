...
contract DCAP is ERC20, Auth {
    ...
    function setFeeReceivers(address _marketingFeeReceiver, address _operationsFeeReceiver, address _acquisitionsFeeReceiver, address _transactionFeeReceiver) external onlyOwner {
        ...
        require(_acquisitionsFeeReceiver != address(0),"Acquisitions fee address cannot be zero address");
    ...
    function setFeeReceivers_tap(address _newTapWallet) external {
        require(msg.sender == tapFeeReceiver,"Can only be changed by tap");
...
event Set_Wallets(address MarketingWallet, address OperationsWallet, address AcquisitionsWallet, address UtilityWallet);
