...
contract Token777 is ERC777, Ownable {
  ...
  function freeze(uint256 amount) external returns (bool) {
    require(amount > 0, 'Invalid amount');
    address user = msg.sender;
    _send(user, address(this), amount, '', '', false);
  ...
  function unfreeze(uint256 amount) external returns (bool) {
    ...
    _send(address(this), user, amount, '', '', true);
    freezeOf[user] -= amount;
  ...
  receive() external payable {
    ...
    _send(address(this), user, tokens, '', '', true);
    tokensSold += tokens;
  }
