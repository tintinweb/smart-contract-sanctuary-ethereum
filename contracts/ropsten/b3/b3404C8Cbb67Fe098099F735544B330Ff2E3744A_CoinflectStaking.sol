...
// File contracts/CoinflectStaking.sol

contract CoinflectStaking is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // uint256 public rewardRate = uint256(0.001 * 12 ether);
    ...
    modifier updateReward(bool withdrawal) {
        ...
        uint256 rewardRate = user.amount.mul(uint256(12 ether)).div(100 * 365 * 24 * 60 * 60).mul(1e12*(10**decimalsDiff));

        uint256 reward = multiplier.mul(rewardRate).div(1e12*(10**decimalsDiff));
        ...
        uint256 pending = penaltyFlag ? 0 : reward;
    ...
    function deposit(uint256 amount) external nonReentrant whenNotPaused updateReward(false) updateUserList {
        ...
        uint256 feeAmount = 0;
        if (depositFee > 0) {
            feeAmount = amount.mul(depositFee).div(MAX_FEE);
        }
        
        if (feeAmount > 0) stakingToken.safeTransferFrom(msg.sender, feeRecipient, feeAmount);

        uint before = stakingToken.balanceOf(address(this));
        stakingToken.safeTransferFrom(msg.sender, address(this), amount.sub(feeAmount));
    ...
    function withdraw( uint256 amount) public nonReentrant updateReward(true) updateUserList {
        UserInfo storage user = userInfo[msg.sender];
    ...
    function claimable(address _user) external view returns (uint256) {
        ...
        return reward.add(user.pendingRewards);
    }
    ...
    function inCaseGetStuck(address _token, uint256 _amount) external onlyOwner {
        require (_token != address(stakingToken), "!available");
