...
// File: contracts/helpers/interfaces/IPAXGERC20.sol



pragma solidity ^0.8.4;

interface IPAXGERC20 {
...
// File: contracts/regular/PAXGTreasury.sol
...
contract PAXGTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SPAXG }
    ...
    address public sPAXG;
    uint public sPAXGQueue; // Delays change to sPAXG address
    ...
    /**
        @notice allow approved address to deposit an asset for PAXG
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint PAXG needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( PAXG ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn PAXG for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IPAXGERC20( PAXG ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sPAXG ).balanceOf( msg.sender ); // Can only borrow against sPAXG held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with PAXG
        @param _amount uint
     */
    function repayDebtWithPAXG( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved as a Debtor" );

        IPAXGERC20( PAXG ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, PAXG, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( PAXG ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( PAXG ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns PAXG valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match PAXG decimals
            value_ = _amount.mul( 10 ** IERC20( PAXG ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SPAXG ) { // 9
            sPAXGQueue = block.number.add( blocksNeededForQueue );
        } else return false;
