...
contract JesusChristo is Ownable, ERC721A {
    using Strings for uint256;

    uint256 public maxSupply = 10000;
    uint256 public maxPerTransaction = 5;
    uint256 public privatePrice = 0.02 ether;
    uint256 public whitelistPrice = 0.009 ether;
    uint256 public publicPrice = 0.033 ether;
    uint256 public airdropReserve = 10;
    ...
    constructor() ERC721A("Jesus Christo", "JC") {
        baseURI = "ipfs://QmZaDkSx56YfWKKFof3kBWgrsRbwC4JVaE5DePkRemR2kc/";
        hiddenURI = "ipfs://";
    }
    ...
    modifier isPrivatelistActive() {
        require(privateMintActive, "Private mint not active!");
        _;
    }
    ...
    function whitelistMint(uint256 _amount) external payable isWhitelistActive {
        uint256 supply = totalSupply();
        uint256 quota = whitelist[msg.sender];

		require(!isContract(msg.sender), "Caller cannot be contract!");
        require(quota > 0, "address not whitelisted!");
        require(_amount <= quota, "no quota left!");
        require(_amount > 0 && _amount <= maxPerTransaction, "Invalid mint amount!");
        require(supply + _amount <= maxSupply - airdropReserve, "Max Supply Reached");
        require(msg.value == _amount * whitelistPrice, "Send proper amount!");

        whitelist[msg.sender] = quota - _amount;
        _safeMint(msg.sender, _amount);
    }

    function privateMint(uint256 _amount) external payable isPrivatelistActive {
        uint256 supply = totalSupply();
        uint256 quota = privateList[msg.sender];

		require(!isContract(msg.sender), "Caller cannot be contract!");
        require(quota > 0, "address not private listed!");
        require(_amount <= quota, "no quota left!");
        require(_amount > 0 && _amount <= maxPerTransaction, "Invalid mint amount!");
        require(supply + _amount <= maxSupply - airdropReserve, "Max Supply Reached");
        require(msg.value == _amount * privatePrice, "Send proper amount!");

        privateList[msg.sender] = quota - _amount;
        _safeMint(msg.sender, _amount);
    }
    ...
    function updatePrivatePrice(uint256 _newPrice) external onlyOwner {
    ...
    // 0 == pause all, 1 == private, 2 == whitelist, 3 == private
    ...
    function manageWhitelist(address[] calldata addresses, uint256 _quota) external onlyOwner {
        for(uint256 i; i < addresses.length; i++){
            whitelist[addresses[i]] = _quota;
        }
    }

    function managePrivateList(address[] calldata addresses, uint256 _quota) external onlyOwner {
        for(uint256 i; i < addresses.length; i++){
            privateList[addresses[i]] = _quota;
        }
    }
    ...
    function withdraw() external onlyOwner {
        ...
        (bool shareholder1, ) = payable(0x0c2F86222082c2cD5ac9CD25d46E9F2bC4e49A71).call{value: share1}("");
        require(shareholder1);
        (bool shareholder2, ) = payable(0xd77180454a9e514AB4579F232dAd3F3fC4CEA78B).call{value: share2}("");
