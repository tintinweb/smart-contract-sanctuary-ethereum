...
contract AngelTokenCrowdsale is Pausable, Crowdsale, Ownable {
    ...
    {
        require(
            _tokensSold + _getTokenAmount(weiAmount) <= _crowdSaleAmount,
        ...
        returns (uint256)
    {
        ...
        if (discountRates.length > 0) {
            ...
            tokens =
                rate().mul(weiAmount).div(10**18) +
                (rate().mul(weiAmount)).mul(discount).div(10000).div(10**18);
        } else {
            tokens = rate().mul(weiAmount).div(10**18);
        }
...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    function _getTokenAmount(uint256 weiAmount)
        ...
        returns (uint256)
    {
        return weiAmount.mul(_rate).div(10**18).div(10**18);
    }
}
