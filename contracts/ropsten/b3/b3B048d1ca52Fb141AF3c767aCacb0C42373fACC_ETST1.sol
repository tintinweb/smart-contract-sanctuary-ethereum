...
contract ETST1 is IERC20, Auth {
	string _name = "El 1";
	string _symbol = "ETST1";
	...
	address private constant _dexRouterAddress = address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // UniswapV2 for ETH
	...
	constructor (uint32 smd, uint32 smr) Auth(msg.sender) {      
		...
		liquidityPool = IUniswapV2Factory(_dexRouter.factory()).createPair(address(this), _dexRouter.WETH()); 
		isLiquidityPool[liquidityPool] = true;
		_nonSniper[liquidityPool] = true;
	...
	function approve(address spender, uint256 amount) public override returns (bool) {
		if ( _humanBlock > block.number && !_nonSniper[msg.sender] ) {
			_addBlacklist(msg.sender, block.number);
		}
