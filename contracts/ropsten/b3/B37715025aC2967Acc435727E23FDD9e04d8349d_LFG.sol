...
contract LFG is ERC721, Ownable, Pausable {
    ...
    constructor() ERC721("Let's Fucking Go", "LFG") { }
    ...
    function mint() public whenNotPaused payable {
        ...
        require(newItemId + 1 <= maxSupply, "Mint amount exceeds the max supply");
    ...
    function tokensOfOwner(address _owner) public view returns (uint256[] memory) {
        ...
        uint256[] memory allTokenIds = new uint256[](balanceOfOwner);
        uint256 currentId = 1;
        uint256 ownedTokenIndex = 0;

        while (ownedTokenIndex < balanceOfOwner && currentId <= maxSupply) {
            address currentTokenOwner = ownerOf(currentId);

            if (currentTokenOwner == _owner) {
                allTokenIds[ownedTokenIndex] = currentId;
