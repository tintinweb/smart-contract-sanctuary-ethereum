...
// import "@unification-com/dona-router/contracts/lib/ConsumerBase.sol";
...
abstract contract ConsumerBase is RequestIdBase {
    ...
    IERC20_Ex internal immutable DONA;
     ...
     * @param _dona address of the deployed DONA smart contract
     */
    constructor(address _router, address _dona) {
        require(_router != address(0), "router cannot be the zero address");
        require(_dona != address(0), "dona cannot be the zero address");
        router = IRouter(_router);
        DONA = IERC20_Ex(_dona);
    }
     ...
     * the DONA Router
     * Allows this contract to increase the DONA allowance for the Router contract
    ...
    function _increaseRouterAllowance(uint256 _amount) internal returns (bool) {
        ...
        require(DONA.increaseAllowance(address(router), _amount), "failed to increase allowance");
        return true;
    }
...
contract MyDataConsumerForDona is ConsumerBase, Ownable {
    ...
    // Ropsten 
    // address private ROUTER_ADDRESS = address(0x1581929AAC3d7BBc0A02bf398C679c582e12b56E);

    // // Ropsten
    // // address private DONA_ADDRESS = address(0x730db079E062cB824f84bE4B3Aa7268B69Bd45aA);

    // RinkeBy 
    // address private ROUTER_ADDRESS = address();

    // // RinkeBy
    // // address private DONA_ADDRESS = address();

    // Mainnet 
    // address private constant ROUTER_ADDRESS = address();

    // // Mainnet
    // // address private constant DONA_ADDRESS = address(0xA6bF49Ab16c2B42A6bc9C6Eb531873A53f5e43D7);
    ...
    constructor(address router, address dona) ConsumerBase(router, dona) {
        price = 0;
    }
