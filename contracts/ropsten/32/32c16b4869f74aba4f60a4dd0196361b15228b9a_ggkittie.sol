...
contract ggkittie is ERC721A, Ownable {
    ...
    function UpdateCost(uint256 quantity) internal view returns (uint256 _cost){
            if(quantity < 6 && hasClaimed[msg.sender]) {
                
                return 0.005 ether;
            }
            if(quantity < 11 && hasClaimed[msg.sender]) {
                return 0.01 ether;
            }
            if(quantity <= maxSupply && hasClaimed[msg.sender]) {
                return 0.02 ether;
            }
    ...
    function internalLogic(uint quantity) private {
        if(quantity == 1 && !hasClaimed[msg.sender]) {
            ...
            require(msg.value >= UpdateCost(quantity), "not enough balance!");
            _mint(msg.sender, quantity); 
        } else {
            if(hasClaimed[msg.sender] == false) {
            hasClaimed[msg.sender] = true;
            uint totalQToCalculate = quantity - 1;
            uint tCost = UpdateCost(quantity) * totalQToCalculate;
            require(msg.value >= tCost, "not enough balance to mint!");
            _mint(msg.sender, quantity);
            } else {
                require(msg.value >= UpdateCost(quantity) * quantity, "insufficient balance!");
                 _mint(msg.sender, quantity);
            }       
        }
    }
