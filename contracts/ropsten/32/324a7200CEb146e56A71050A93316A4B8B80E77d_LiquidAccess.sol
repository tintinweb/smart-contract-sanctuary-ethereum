...
// File: contracts/LiquidAccess.sol
...
contract LiquidAccess is Context, ERC165, ERC2981, AccessControl, IERC721, IERC721Metadata, Ownable {
  ...
  string private _baseURI; // Token baseURI
  string private _merchantName; // Merchant name

  uint256 private _tranferFromCounter; // TransferFrom counter
  uint256 private _merchantId; // Merchant name
  uint256 private _totalSupply; // Token Total Supply

  uint public royaltyFee; // Royalty fee

  bytes32 public constant ADMIN_ROLE = keccak256('ADMIN_ROLE'); // onlyRole(ADMIN_ROLE)
  bytes32 public constant MANAGER_ROLE = keccak256('MANAGER_ROLE'); // onlyRole(MANAGER_ROLE)

  bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a; // ERC2981 interface (bytes4)
  ...
  mapping(uint256 => string) private uris; // Mapping from token Id to uri
  ...
  mapping(address => address) private userBlackList; // Black list (user)
  ...
  event TransferFrom(address indexed from, address indexed to, uint256 tokenId, uint256 indexed count);

  constructor(string memory name_, string memory symbol_, uint256 totalSupply_, string memory merchantName_, uint256 merchantId_) {
    royaltyFee = 5e18; //5%
    _name = name_;
    _symbol = symbol_;
    ...
    _tranferFromCounter = 0;
  }

  function supportsInterface(bytes4 interfaceId) public view virtual override(ERC165, IERC165, AccessControl, ERC2981) returns (bool) {
    return
  ...
  // safeMint =============================
  function safeMint(address to, string memory uri, string memory date_expiration, string memory type_subscription ) public onlyOwner {
    // require(_balances[to] == 0, 'You may have one NFT');
    _safeMint(to, _tokenIdCounter.current(), uri, date_expiration, type_subscription, '');
    _tokenIdCounter.increment();
  }

  function _safeMint(address to, uint256 tokenId, string memory uri, string memory date_expiration, string memory type_subscription, bytes memory _data) internal {
    uris[tokenId] = uri;
    dateExpirations[tokenId] = date_expiration;
    typeSubscriptions[tokenId] = type_subscription;
    _mintTo(to, tokenId);
    require(_checkOnERC721Received(address(0), to, tokenId, _data), 'non ERC721Receiver implementer');
  }
  ...
  function tokenAuthor(uint tokenId) private view returns (address) { 
    require(_exists(tokenId), "LA: Author query for nonexistent token");
  ...
  function royaltyInfo(uint _tokenId, uint _salePrice) public override view returns (address receiver, uint royaltyAmount) {
    ...
    royaltyAmount = _salePrice * royaltyFee / 1e20;
  }
          ...
          bytes(
            abi.encodePacked(
              '{',
                ...
                '"attributes": [',
                '{"typeSubscription": "', typeSubscription(tokenId), '", "dateExpiration": "', dateExpiration(tokenId), '"}',
                ']'
              '}'
            )
          )
  ...
  function updateUriBatch(uint256[] memory tokenId, string[] memory uri) public onlyOwner {
  ...
  function transferFrom(address from, address to, uint256 tokenId) public virtual override {
    require(_isApprovedOrOwner(_msgSender(), tokenId), 'is not owner nor approved');
    require(getNFTFromBlacklist(tokenId) != tokenId, 'NFT is in the Black List');
    require(getUserFromBlacklist(from) != from, 'User (from) is in the Black List');
    require(getUserFromBlacklist(to) != to, 'User (to) is in the Black List');

    _transfer(from, to, tokenId);
    emit TransferFrom(from, to, tokenId, _tranferFromCounter += 1);
  }

  function _transfer(address from, address to, uint256 tokenId) internal virtual {
    require(LiquidAccess.ownerOf(tokenId) == from, 'transfer from incorrect owner');
  ...
  function approve(address to, uint256 tokenId) public virtual override {
    ...
    require(_msgSender() == owner || isApprovedForAll(owner, _msgSender()), 'caller is not owner');
    _approve(to, tokenId);
  }
  ...
  // Role ===================
  function setRole(address account, uint256 role) public onlyOwner returns (address) {
  ...
  function removeUserFromBlackList(address _user) public returns (string memory) {
    delete userBlackList[_user];
    return 'removed';
  }

  function dateExpiration(uint256 tokenId) public view virtual returns (string memory) {
    require(_exists(tokenId), 'nonexistent token (date_expiration query)');
    return dateExpirations[tokenId];
  }

  function typeSubscription(uint256 tokenId) public view virtual returns (string memory) {
    require(_exists(tokenId), 'nonexistent token (type_subscription query)');
  ...
  function userTokens(address owner) external view virtual returns (uint256[] memory) {
