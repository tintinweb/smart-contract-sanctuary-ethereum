...
contract UNDERDOGZ is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function claimDividends(address payable _tokenHolder) nonReentrant() public {
        ...
        require(_tokenHolder != owner(), "Dividends Are Only For The UDZ DAO Members!"); 
        
        uint256 validClaimAmount = 0;
        uint256 walletClaimAmount = 0;

        for(uint256 i = 0; i < walletOfOwner(_tokenHolder).length; i++) {

            uint256 currentTokenId = tokenOfOwnerByIndex(_tokenHolder, i);
            ...
            if(tokenIdToToken[currentTokenId] == false && walletClaimAmount <= maxDividendWhaleValue) {
                ...
                emit tokenDividends (currentTokenId, tokenIdToToken[currentTokenId], _tokenHolder, dividendAmount, dividendRound, block.timestamp); 
        ...
        emit claimerDividends (_tokenHolder, validClaimAmount, amountRecieved, dividendRound, block.timestamp);
    }
}