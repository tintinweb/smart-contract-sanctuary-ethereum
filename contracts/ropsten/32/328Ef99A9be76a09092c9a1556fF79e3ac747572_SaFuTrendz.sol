...
contract SaFuTrendz is Context, IERC20, Ownable {
    ...
    uint256 public _swapFeeEnableLimit = 1 * 10**5 * 10 **9; // 0.01% of total supply
    uint256 public _maxTxAmount = 1 * 10**7 * 10**9; // 1% of total supply
    uint256 public _maxWalletAmount = 5 * 10**7 * 10**9; // 5% of total supply
    ...
    function setUniswapRouterAddress(address routerAddress) public onlyOwner{
        ...
        _uniswapV2RouterAddress = routerAddress;
        ...
        excludeFromMaxTX(_uniswapV2Pair);
        excludeFromMaxTX(routerAddress);
    }
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner {
        //  0.01% total suppy <= maxTxAmount <= 1% total supply
        require(maxTxAmount >= 10**5 * 10**9 && maxTxAmount <= 10**7 * 10**9, "Invalid setting");
        _maxTxAmount = maxTxAmount;
    }
    
    function setMaxWalletAmount(uint256 maxWalletAmount) external onlyOwner {
        //  0.1% total suppy <= maxWalletAmount <= 10% total supply
        require(maxWalletAmount >= 10**6 * 10**9 && maxWalletAmount <= 10**8 * 10**9, "Invalid setting");
    ...
    function _transfer(
        ...
        if( from != _uniswapV2RouterAddress &&
    ...
    function feeWithdrawExecute() internal
    {
        ...
        if(_buyFeeAmount - _buyFeeWithdrawed >= _swapFeeEnableLimit)
        {
            swapAndLiquify(1);
        }
