...
/**
____________ ________  ___  ___   _   _ _____  _____ 
| ___ \ ___ \  _  |  \/  | / _ \ | \ | /  __ \|  ___|
| |_/ / |_/ / | | | .  . |/ /_\ \|  \| | /  \/| |__  
| ___ \    /| | | | |\/| ||  _  || . ` | |    |  __| 
| |_/ / |\ \\ \_/ / |  | || | | || |\  | \__/\| |___ 
\____/\_| \_|\___/\_|  |_/\_| |_/\_| \_/\____/\____/ 
...
contract FireBattleLite is Context, IERC20, Ownable {
    ...
    string private _name = "Fire Battle Lite";
    string private _symbol = "FBL";
    ...
    uint256 internal _tokenTotal = 10 *10**13; // 100k total supply
    uint256 public maxWallet = 3 *10**13;
    uint256 public maxTxAmount = 3 *10**13;
    ...
    // fees
    uint256 public _taxFee = 300; // 100 = 1%
    uint256 public _BurnFee = 300; // 100 = 1%
    uint256 public _liquidityFee = 100; // 100 = 1%
    ...
    //addresses
    address public liquidityAddress = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // Should be changed to LP contract using function
 ...
 /**
    ...
    */
    ...
    function excludeAccount(address account) external onlyOwner() {
        ...
        require(account != address(this), 'FBL: The contract it self cannot be excluded');
        require(!_isExcluded[account], "FBL: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "FBL: Account is already included");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        //@dev maxTx
        if(sender != owner() && recipient != owner() && sender != liquidityAddress && recipient != liquidityAddress) {
                require(amount <= maxTxAmount, "Transfer amount exceeds maximum transaction");
       }
        //@dev maxWallet
        if(sender != owner() && recipient != owner() && sender != liquidityAddress && recipient != liquidityAddress) {
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(0), "FBL::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FBL::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "FBL::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "FBL::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "BROMANCE::_writeCheckpoint: block number exceeds 32 bits");
