...
contract NFTStaking is Ownable, ReentrancyGuard {
  ...
  address constant ANTHROCollection = 0xe6096DeeDCAe8e15D3113a13Db8ae079ed9465ab;
  address constant HUMANCollection = 0x960962ED55CBb4b8fCA35324938f6D956b417049;
  address constant ANKOKUCollection = 0xd82c2f4dA460eA5708Fb8b02f223c7DcFE219005;
  ...
  // uint256 private rewardsPerHour 0.001ETH;
  ...
  constructor(IERC20 _rewardsToken) {
    ...
    addVault(address(ANKOKUCollection), "Singularity 0 Universe ANKOKU");
  }
  ...
  function userStakeVault(address _user)
    public
    view
    returns (uint256 _tokensStaked, uint256 _availableRewards, StakeItem[] memory _stakedItems)
  {
    uint256 len = vaultInfo.length;
    StakeItem[] memory stakedItems = new StakeItem[](len);
    ...
    for (uint256 i; i < len; ++i) {
      ...
      for (uint256 k; k < length; ++k) {
        if(vault.stakerAddress[k] == _msgSender()) {
          stakedIds[index] = k;
          index +=1;
        }
      }

      stakedItems[i] = StakeItem(
        i, stakedIds
      );
    }

    return (stakers[_user].amountStaked, availableRewards(_user), stakedItems);
  }
