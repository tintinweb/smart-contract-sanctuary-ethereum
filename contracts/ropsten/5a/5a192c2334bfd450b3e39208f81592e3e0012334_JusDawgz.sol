...
contract JusDawgz is Ownable, ERC721A, ReentrancyGuard {
    uint256 public perWalletLimitPublic = 10;
    uint256 public perWalletLimitPresale = 5;

    uint256 public perTxLimitPublic = 5;
    uint256 public perTxLimitPresale = 5;
    ...
    uint256 public WL_Price = 0.033 ether;
    ...
    function mintNFT(uint256 _quantity) public payable {
        require(_quantity > 0 && _quantity <= perTxLimitPublic, "Wrong Quantity.");
        require(totalSupply() + _quantity < maxSupply, "Reaching max supply");
        require(msg.value == price * _quantity, "Needs to send more eth");
        require(getMintedCount(msg.sender) + _quantity <= perWalletLimitPublic, "Exceed max adoption amount");
    ...
    function mintPresale(uint256 _quantity) public payable {
        require(_quantity > 0 && _quantity <= perTxLimitPresale, "Wrong Quantity.");
        require(totalSupply() + _quantity < maxSupply, "Reaching max supply");
        require(msg.value == WL_Price * _quantity, "Needs to send more eth");
        require(getMintedCount(msg.sender) + _quantity <= perWalletLimitPresale, "Exceed max adoption amount");
    ...
    function setperWalletLimitPublic(uint256 _temp) public onlyOwner() {
        perWalletLimitPublic = _temp;
    }
    function setperWalletLimitPresale(uint256 _temp) public onlyOwner() {
        perWalletLimitPresale = _temp;
    }
    function setTxLimitPublic(uint256 _temp) public onlyOwner() {
        perTxLimitPublic = _temp;
    }
    function setTxLimitPresale(uint256 _temp) public onlyOwner() {
        perTxLimitPresale = _temp;
    }
