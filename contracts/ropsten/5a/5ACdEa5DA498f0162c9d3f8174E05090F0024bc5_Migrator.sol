...
library HodlLibrary {
    ...
    {
        require(tokenA != tokenB, "HodlLibrary: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "HodlLibrary: ZERO_ADDRESS");
    }
    ...
    function pairFor(
        ...
        bytes32 initCodeHash = IFactory(factory).pairCodeHash();
        pair = address(
            uint256(
                keccak256(
                    abi.encodePacked(
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = ICore(
    ...
    function quote(
        ...
        require(amountA > 0, "HodlLibrary: INSUFFICIENT_AMOUNT");
        require(
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "HodlLibrary: INSUFFICIENT_INPUT_AMOUNT");
        require(
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "HodlLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
        require(
            reserveIn > 0 && reserveOut > 0,
            "HodlLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, "HodlLibrary: INVALID_PATH");
