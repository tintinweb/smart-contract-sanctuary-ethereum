...
contract ggkittie is ERC721A, Ownable {
    ...
    function internalLogic(uint quantity) private {
        ...
        function internalLogic2(uint quantity) private {
        if(quantity < 6 && !hasClaimed2[msg.sender]) {
            require(hasClaimed2[msg.sender] == false, "already claimed");
            hasClaimed2[msg.sender] = true;
            _mint(msg.sender, quantity);
        } else if (quantity < 6 && hasClaimed2[msg.sender]) {
            require(msg.value >= cost2, "not enough balance!");
            _mint(msg.sender, quantity); 
        } else {
            if(hasClaimed2[msg.sender] == false) {
            hasClaimed2[msg.sender] = true;
            uint totalQToCalculate = quantity;
            uint tCost = cost2 * totalQToCalculate;
            require(msg.value >= tCost, "not enough balance to mint!");
            _mint(msg.sender, quantity);
            } else {
