...
contract SFDC15 is IERC20, Auth {
	string constant _name = "Safdic 15";
	string constant _symbol = "SFDC15";
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		if ( _humanBlock > block.number ) {
			...
			if ( block.number < _humanBlock + 10 && tx.gasprice >= block.basefee + 300 * 10**_decimals ) { revert("Excessive gas"); }
		}
