  ...
  UUPSUpgradeable
{
  ...
  // * creator 변경
  function setCreator(uint256 id, address newCreator) virtual public onlyCreator(id) {
  ...
  // * uri 조회
  function uri(uint256 id) override virtual public view returns (string memory) {
  ...
  function _beforeTokenTransfer(
    ...
    bytes memory data
  ) override(ERC1155Upgradeable, ERC1155SupplyUpgradeable) virtual internal whenNotPaused {
...
contract Royalties is Initializable, IRoyalties {
  ...
  // * 로열티 조회
  function royalties(uint256 tokenId) override external view returns (Royalty[] memory) {
    return _royalties[tokenId];
  }

  // * 로열티 최대치 변경
  function setMaxRoyalty(uint96 maxRoyalty_) override external {
    maxRoyalty = maxRoyalty_;
  }
