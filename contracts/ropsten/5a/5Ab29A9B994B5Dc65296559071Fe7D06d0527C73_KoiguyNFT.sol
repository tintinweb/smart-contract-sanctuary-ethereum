...
contract CoolNFT is Ownable, ERC721A, PaymentSplitter {
    ...
    function Mint(address _account, uint _quantity, bytes32[] calldata _proof) external callerIsUser{
        if(isWhiteListed(msg.sender, _proof)) {
            whitelistMint(_account, _quantity, _proof);
        } else {
            publicSaleMint(_account, _quantity);
        }
    }

    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) public payable callerIsUser {
    ...
    function publicSaleMint(address _account, uint _quantity) public payable callerIsUser {
