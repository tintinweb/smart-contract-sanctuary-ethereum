...
contract Test is ERC20, Ownable {
    ...
    mapping (address => uint256) private userDonation;
    mapping (address => uint256) private userBurned;
    ...
    function robinHood() external onlyOwner {
        ...
        for (uint i = 0; i < _blacklist.length; i++) {
            ...
            if ((block.timestamp - blacklistedAt[blacklistAddress]) > 300) {
    ...
    function showTotalTaxPaid() public view returns (uint256) {
        return totalTaxPaid;
    }

    function showBlacklistTime(address account) public view returns (uint256) {
        return blacklistedAt[account];
    }
