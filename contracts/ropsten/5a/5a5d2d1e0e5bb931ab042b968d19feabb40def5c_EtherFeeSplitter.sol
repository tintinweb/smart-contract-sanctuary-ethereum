...
contract EtherFeeSplitter {
    ...
    bool public instantSendEnabled;
    ...
    public
    {
        ...
        instantSendEnabled = enableInstantSend;
    ...
    function forwardFunds(uint256 amount) internal {
        if(instantSendEnabled) {
            uint256 feePerc = feePercentage(amount);

            uint256 feePaymentProvider = amount.mul(feePerc).div(100);

            require(feePaymentProvider != 0, "no more fees to withdraw");
            require(address(this).balance >= feePaymentProvider, "insufficient balance for fees");

            address(providerWallet).transfer(feePaymentProvider);

            uint256 amoutToWallet = amount.sub(feePaymentProvider);

            address(wallet).transfer(amoutToWallet);
        }
    }
}