...
contract OracleFacet {
    ...
    /// @notice Function that calls capture for tracking the Delta B on both the Uniswap and Curve LP pools
    ...
    function initializeOracle() internal {
        uint256 priceCumulative = s.index == 0 ? IUniswapV2Pair(s.c.pair).price0CumulativeLast() : IUniswapV2Pair(s.c.pair).price1CumulativeLast();
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IUniswapV2Pair(s.c.pair).getReserves();
        ...
        if (reserve0 != 0 && reserve1 != 0 && blockTimestampLast != 0) {
            ...
            (uint256 peg_priceCumulative, , uint32 peg_blockTimestamp) = UniswapV2OracleLibrary.currentCumulativePrices(s.c.pegPair);
    ...
    function updatePrice() private returns (Decimal.D256 memory, Decimal.D256 memory) {
        (uint256 price0Cumulative, uint256 price1Cumulative, uint32 blockTimestamp) = UniswapV2OracleLibrary.currentCumulativePrices(s.c.pair);
        (uint256 peg_priceCumulative, , uint32 peg_blockTimestamp) = UniswapV2OracleLibrary.currentCumulativePrices(s.c.pegPair);
