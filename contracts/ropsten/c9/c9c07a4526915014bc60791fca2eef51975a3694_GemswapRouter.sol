...
contract GemswapRouter {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = GemswapLibrary.getReserves(_factory, implementation, tokenA, tokenB);

        uint256 amountBOptimal = GemswapLibrary.quote(amountADesired, reserveA, reserveB);
        if (amountBOptimal <= amountBDesired) {
            require(amountBOptimal >= amountBMin, "INSUFFICIENT_B_AMOUNT");
            (amountA, amountB) = (amountADesired, amountBOptimal);
        } else {
            uint256 amountAOptimal = GemswapLibrary.quote(amountBDesired, reserveB, reserveA);
