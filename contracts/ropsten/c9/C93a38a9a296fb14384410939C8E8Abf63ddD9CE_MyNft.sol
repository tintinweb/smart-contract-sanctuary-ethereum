...
contract MyNft is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _mintRandom(address to, uint _numToMint) public payable {
        require(isPublicMintEnabled, 'minting not enabled!');
        require(msg.value == _numToMint * mintPrice, 'wrong mint value');
    ...
    function getRandomAvailableTokenId(address to, uint updatedNumAvailableTokens)
        ...
        returns (uint256)
    {
        uint256 randomNum = uint256(keccak256(abi.encode(to,tx.gasprice,block.number,block.timestamp,block.difficulty,blockhash(block.number - 1),address(this),updatedNumAvailableTokens)));
    ...
    function _transfer(
        ...
        require(MyNft.ownerOf(tokenId) == from, "ERC721: transfer from incorrect owner");
