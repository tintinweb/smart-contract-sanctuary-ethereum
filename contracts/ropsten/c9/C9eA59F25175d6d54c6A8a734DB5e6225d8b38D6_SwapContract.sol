...
contract SwapContract is AccessControlEnumerable, Pausable, ECDSAOffsetRecovery
{
    ...
    mapping(uint128 => uint256) public feePctOfBlockchain;
        ...
        whenNotPaused
    {
        ...
        uint256 feepct = feePctOfBlockchain[numOfThisBlockchain];
        uint256 fee = SafeMath.div(SafeMath.mul(amountWithFee, feepct), 10 ** precision);
    ...
    /** 
      * @dev Changes fee percentage for blockchains in feePctOfBlockchain variables
      * @param blockchainNum Existing number of blockchain
      * @param feePct Fee percent to calculate from transfer amount
      */
    function setFeePctOfBlockchain(uint128 blockchainNum, uint256 feePct) external onlyOwnerAndManager
    {   
        require(feePct >= 0);
        feePctOfBlockchain[blockchainNum] = feePct;
    }


    /** 
      * @dev Changes fee percentage precision decimals
      * @param precisiondec Fee percent to calculate from transfer amount
      */
    function setFeePctOfBlockchain(uint8 precisiondec) external onlyOwnerAndManager
    {   
        require(precisiondec >= 0);
