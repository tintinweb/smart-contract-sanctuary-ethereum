...
contract SxTCommunity is ISxTCommunity, SxTCommunityStorage, ERC1155, Ownable, ReentrancyGuard, Pausable {
    ...
    function retrieveNFTs(uint256 pageNumber, uint256 pageSize, bool isAvailable) public view returns(string memory, uint256) {
        ...
        uint256 allAcceptableTokensCount = 0;
        ...
        for ( uint256 i = currentTokenIndex.current(); i > 0; i--){
            if(isAvailable != checkMaxSupplyReached(i)){
                ...
                allAcceptableTokens[allAcceptableTokensCount] = tokenTemp;
                allAcceptableTokensCount++;
            }
        }
        uint256 startIndex = ((pageNumber - 1) * pageSize);
        uint256 endIndex = startIndex + pageSize;
        if(startIndex >= currentTokenIndex.current()){
            return (nftsResponseJSON, allAcceptableTokensCount);
        }
