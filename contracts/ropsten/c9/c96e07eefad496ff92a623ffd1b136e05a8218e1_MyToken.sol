...
contract MyToken is ERC20Interface, Owned, SafeMath {
    ...
    function () public payable{
        ...
        require(now < endTime && startTime < endTime);
        require(buyRate > 0);
    ...
    function setConfig(uint _buyRate,uint _sellRate,string _symbol,string _name,uint _startTime,uint _endTime) public onlyOwner {
        require(_buyRate < _sellRate && _buyRate > 0 && _sellRate > 0);
        require(_buyRate < sellRate && _buyRate > 0 && _sellRate == 0);
        require(buyRate < _sellRate && _buyRate == 0 && _sellRate > 0);
