...
contract CastableNFT is ERC721, Ownable {
     ...
     */
    function createCollectible(string memory _uri) public payable returns (uint256) {
     ...
     */
    function createMultipleCollectibles(string memory _uri, uint256 count) public payable returns (uint256[] memory) {
        require(msg.value >= minMintValue, "Less than the minimum casting value");
     ...
     *
     */
    function createMultipleCollectiblesCustom(string[] memory _uris, uint256[] memory _values) public payable returns (uint256[] memory) {
        require(msg.value >= minMintValue, "Less than the minimum casting value");
    ...
    * @dev create a collection minted using erc20 tokens.
     ...
     *
     */
    function createCollectibleByERC20(address _addr, uint256 _value, string memory _uri) public returns (uint256) {
    ...
    * @dev create multiple collectibles minted using erc20 tokens.
     ...
     *
     */
    function createMultipleCollectiblesByERC20(address _addr, string memory _uri, uint256 _value, uint256 count) public returns (uint256[] memory) {
        require(_value > 0, "Value must be greater than 0");
        IERC20 erc20Token = IERC20(_addr);
        require(erc20Token.allowance(msg.sender, address(this)) >= _value, "Insufficient allowance!");
        require(erc20Token.transferFrom(msg.sender, address(this), _value), "Transfer failed!");
     ...
     *
     */
    function createMultipleCollectiblesByERC20Custom(address _addr, string[] memory _uris, uint256[] memory _values) public returns (uint256[] memory) {
        require(_uris.length == _values.length, "Inconsistent length");
        uint256 totalValue;
        for (uint256 i = 0; i < _values.length; i++) {
            totalValue += _values[i];
    ...
    * @dev create a collection minted using erc20 tokens.
     *
     */
    function createCollectibleByERC721(address _addr, uint256 _id, string memory _uri) public returns (uint256) {
        ...
        erc721Token.safeTransferFrom(msg.sender, address(this), _id);
    ...
    * @dev create multiple collectibles minted using erc721 tokens.
     *
     */
    function createMultipleCollectiblesByERC721(address _addr, string[] memory _uris, uint256[] memory _ids) public returns (uint256[] memory) {
        // check if it has been cast before
        if (_isERC721TokenHasBeenCast(_addr) == false) {
            _hasBeenCastERC721TokenAddrs.push(_addr);
        }
        ...
        for (uint256 i = 0; i < _uris.length; i++) {
            ...
            erc721Token.safeTransferFrom(msg.sender, address(this), _ids[i]);
    ...
    /**
     * @dev burn collectibles returns the casting value(Ethereum ,erc20,erc721 token value).
     */
