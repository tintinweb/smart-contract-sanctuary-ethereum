...
contract Digipolis is Context, IERC20, Ownable {
    ...
    string private _name = "Digipolis Token";
    ...
    uint256 public totalFee = _liquidityFee.add(_treasuryFee);
    ...
    function removeAllFee() private {
        if(_liquidityFee == 0 && _treasuryFee==0 ) return;        
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 _treasuryFee = _treasuryFee;
        uint256 _liquidityfee = _liquidityFee;

        if (recipient == uniswapV2Pair) {
            _totalFee = totalFee.add(sellLiquidityFee).add(sellTreasuryFee);
            _treasuryFee = _treasuryFee.add(sellTreasuryFee);
            _liquidityfee = _liquidityFee.add(sellLiquidityFee);
        }
        
        uint256 TreasuryAmt = amount.mul(_treasuryFee).div(100);

        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(TreasuryAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(TreasuryAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(TreasuryAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(TreasuryAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(TreasuryAmt)));
        }
    ...
    function setSellTreasuryFeePercent(uint256 sellTreasuryFee) external onlyOwner() {
        sellTreasuryFee = sellTreasuryFee;
    }  
