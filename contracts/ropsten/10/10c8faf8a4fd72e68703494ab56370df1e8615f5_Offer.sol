...
contract Offer {
    ...
    constructor(address tokenContractAddress, string _offer_id, address _cpaOwner) public {
    ...
    function writeConversion(string _conversion_id, address _affiliate, uint256 _amount, uint256 _toAffiliate)
        ...
        if (getBalance(address(this)) >= _amount) {
            ...
            conversions[keccak256(abi.encodePacked(_conversion_id))] = conversion(_conversion_id, now, _affiliate, _amount, _toAffiliate);
    ...
    function getConversionInfo(string _conversion_id)
        ...
        conversion storage _c = conversions[keccak256(abi.encodePacked(_conversion_id))];
