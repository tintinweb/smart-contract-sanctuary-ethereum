...
contract SummerClub is ERC721G, Ownable, ReentrancyGuard {
    ...
    uint256 public WHITE_PRICE = 0.0 ether;
    ...
    constructor(string memory initURI, address signer1, address signer2) ERC721G("Summer Club", "SUMMER") {
        _uri = initURI;
        maxTotalSupply = 5556;
        PreMaxMint = 2;
        PublicMaxMint = 1;
    ...
    function _baseURI() internal view  override(ERC721G) returns (string memory) {
        return _uri;
    }
    ...
    function mint(uint256 num, string calldata salt, bytes calldata token) external payable {
        ...
        uint256 numberResult = _verify(_hash(salt, msg.sender), token);

        require(numberResult >= num, "SUM001");
        require(publicNumberMinted(msg.sender) + num <= PublicMaxMint, "SUM008");

        _verified_public(num);
        _verified(num);
    ...
    function preSaleMint(
        ...
        require(preMaxMint >= amount, "SUM001");
        require(numberMinted(msg.sender) + amount <= preMaxMint, "SUM008");
        _verified_white(amount);
        _verified(amount);
    ...
    function _verified_white(uint256 num) private {
        require(num > 0, 'SUM011');
        require(msg.value >= WHITE_PRICE * num, 'SUM002');
        if (msg.value > WHITE_PRICE * num) {
            payable(msg.sender).transfer(msg.value - WHITE_PRICE * num);
        }
    }

    function _verified_public(uint256 num) private {
        require(num > 0, 'SUM011');
        require(msg.value >= PRICE * num, 'SUM002');
        if (msg.value > PRICE * num) {
            payable(msg.sender).transfer(msg.value - PRICE * num);
        }
    }

    function _verified(uint256 num) private {
    ...
    function publicNumberMinted(address owner) public view returns (uint256) {
        require(
            owner != address(0),
            "ERC721G: number minted query for the zero address"
        );
    ...
    function verify(string calldata salt, bytes calldata token) public view returns(address){
//        return _verify(_hash(salt, msg.sender), token);
        return _recover(_hash(salt, msg.sender), token);
    }
