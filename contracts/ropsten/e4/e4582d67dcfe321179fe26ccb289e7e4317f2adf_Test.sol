...
contract Test is ERC223, Ownable {
  using SafeMath for uint256;

  string public name = "TestCoin";
  string public symbol = "TEST";
  uint8 public decimals = 8;
  uint256 public initialSupply = 500e8 * 1e8;
  ...
  /**
   * @dev Function to distribute different amount of tokens to the list of addresses by the provided amount
   */
  function distributeDifferentAmountOfTokens(address[] addresses, uint256[] amounts) public returns (bool) {
    ...
    for (uint i = 0; i < addresses.length; i++) {
      ...
      uint256 totalAmount = SafeMath.add(totalAmount, SafeMath.mul(amounts[i], 1e8));
    }

    require(balances[msg.sender] >= totalAmount);

    for (uint o = 0; o < addresses.length; o++) {
      require(addresses[o] != 0x0
              && frozenAccount[addresses[o]] == false
              && now > unlockUnixTime[addresses[o]]);

      balances[addresses[o]] = SafeMath.add(balances[addresses[o]], amounts[o]);
      Transfer(msg.sender, addresses[o], amounts[o]);
    }
