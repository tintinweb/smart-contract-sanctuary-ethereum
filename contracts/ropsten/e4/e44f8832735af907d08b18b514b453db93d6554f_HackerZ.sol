...
/**
 * HackerZ Nft Collection.
 */
contract HackerZ is ERC721, ERC721Enumerable, Ownable {

    using SafeMath for uint256;

    uint256 public constant MAX_SUPPLY = 10000;
    uint256 public MAX_NFT_PER_WALLET = 20;
    uint256 public MAX_NFT_PER_TX = 5;
    uint256 private _price = 0.01 ether;
    uint256 private _reserved = 0;
    // Start at 1
    uint256 private _startOffset = 1;
    ...
    // Use the mint pass ?
    bool private _use_secret = false;
    ...
    constructor() ERC721("HackerZ", "HKZ") {
        _saleStarted = false;
    ...
    function _mint(uint256 _nbTokens) private {
        ...
        require(supply + _nbTokens + _startOffset <= MAX_SUPPLY - _reserved, "Not enough Tokens left.");
        require(_nbTokens * _price <= msg.value, "Inconsistent amount sent!");

        for (uint256 i; i < _nbTokens; i++) {
            _safeMint(msg.sender, supply + i + _startOffset);
        }
    }
    ...
    function setMaxWallet(uint256 n) external onlyOwner {
        MAX_NFT_PER_WALLET = n;
    }

    function setMaxNFTPerTx(uint256 n) external onlyOwner {
        MAX_NFT_PER_TX = n;
    }
...
contract fuzz {
    ...
    constructor() {
        _seed = keccak256(abi.encodePacked(block.timestamp, block.difficulty, blockhash(block.number)));
    }
}
