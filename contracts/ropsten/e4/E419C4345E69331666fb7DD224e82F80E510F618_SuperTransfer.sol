...
contract SuperTransfer is Ownable  {
    ...
    function setAccountsMerkleRoot(bytes32  _merkleRoot,uint256 _index)  external onlyOwner {
        ...
        require(mindex!=_index,"index must be different");
        mindex=_index;
    }
    ...
    function isWithdrawed(address account) public view  returns (bool) {
        ...
        if(WithdrawedBitMap[account]==mindex){
            return true;
        }else{
            return false;
        }
    ...
    function _setWithdrawed(address _account) private {
        ...
        WithdrawedBitMap[_account] = mindex;
    }

    function withdraw(uint256 index,address token, address account, uint256 amount, bytes32[] calldata merkleProof) external  {
