...
abstract contract Swap is Storage, UserWallet, EIP712Upgradeable {
  ...
  bytes32 public constant _SWAP_TYPEHASH =
   keccak256("Swap(address user,bytes32 swapGateway,address tokenFrom,address tokenTo,uint256 amountFrom,uint256 minAmountTo,uint256 nonce,uint256 deadline)");
  ...
  event SwapPerformed(bytes32 swapId, bytes32 swapGateway, address indexed user,
   address indexed tokenFrom, uint256 amountFrom, address indexed tokenTo, uint256 amountTo);

  event BridgedFunds(address indexed user, address indexed token, uint256 amount);
  ...
  function executeSwapWithSignature(
    ...
    (,uint256 tokenToAmount) = performSwap(swapConstraints.user, swapConstraints.tokenFrom, swapConstraints.tokenTo, swapConstraints.amountFrom, swapConstraints.minAmountTo, data);
    ...
    postSwap(swapConstraints.swapGateway, tokenToAmount, swapConstraints.tokenTo, swapConstraints.user);

    emit SwapPerformed(bytes32(0), swapConstraints.swapGateway, swapConstraints.user, swapConstraints.tokenFrom, swapConstraints.amountFrom, swapConstraints.tokenTo, tokenToAmount);
  }
  ...
  function executeSwapWithSelfLiquidity(
    ...
    transfer(address(this), tokenFrom, user, amountFrom);
    (,uint256 tokenToAmount) = performSwap(user, tokenFrom, tokenTo, amountFrom, minAmountTo, data);

    emit SwapPerformed(bytes32(0), bytes32(0), user, tokenFrom, amountFrom, tokenTo, tokenToAmount);
  }
  ...
  function postSwap(
    ...
    if (swapGateway == DEVERSIFI_GATEWAY) {
      ...
      emit BridgedFunds(user, receivedToken, receivedtokenAmount);
    }
  }
  ...
  function performSwap(
    ...
    uint256 minAmountTo,
    bytes memory data
  ) private returns(uint256 tokenFromAmount, uint256 tokenToAmount) {
    tokenFromAmount = IERC20Upgradeable(tokenFrom).balanceOf(address(this));
    tokenToAmount = IERC20Upgradeable(tokenTo).balanceOf(address(this));
    ...
    tokenToAmount = IERC20Upgradeable(tokenTo).balanceOf(address(this)) - tokenToAmount;

    require(tokenFromAmount <= amountFrom, "HIGHER_THAN_AMOUNT_FROM");
    require(tokenToAmount >= minAmountTo, "LOWER_THAN_MIN_AMOUNT_TO");

    _decreaseBalance(tokenFrom, user, tokenFromAmount);
    _increaseBalance(tokenTo, user, tokenToAmount);
  ...
  function verifySwapSignature(
    ...
    require(block.timestamp <= swapConstraints.deadline, "DEADLINE_EXPIRED");
    ...
    bytes32 structHash = _hashTypedDataV4(keccak256(
      abi.encode(
        ...
        // TODO deprecate and get from request otherwise users
        // must know the current value before signing requests
