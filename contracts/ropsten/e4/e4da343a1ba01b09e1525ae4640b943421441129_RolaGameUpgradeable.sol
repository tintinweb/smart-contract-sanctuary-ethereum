...
contract RolaGameUpgradeable is RolaGameInternalUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyAdminOrOwner() {
        require(msg.sender == adminAddress || msg.sender == owner(), "Not admin or owner");
        _;
    }
    ...
    function betBear(uint256 roundId, uint256 amount) external override whenNotPaused nonReentrant notContract {
        ...
        require(amount <= rolaCoaster.balanceOf(_msgSender()), "Insufficient ROLA tokens");
    ...
    function claim(uint256[] calldata epochs) external override nonReentrant notContract {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            require(roundData[epochs[i]].startTimestamp != 0, "Round has not started");
            uint256 closeTimestamp = roundData[epochs[i]].startTimestamp + (2 * roundData[epochs[i]].roundExecutionTime);
            require(block.timestamp > closeTimestamp, "Round has not ended");
    ...
    function lockRound(uint256 roundId) external override whenNotPaused onlyOperator {
        require(genesisRound[roundId].genesisStartOnce, "Can only run after genesisStartRound is triggered");
        require(!genesisRound[roundId].genesisLockOnce, "Can only run genesisLockRound once");
    ...
    function setBufferTime(uint256 _bufferSeconds) external override whenPaused onlyAdmin {
        require(_bufferSeconds != 0, "bufferSeconds must be superior to 0");
        bufferSeconds = _bufferSeconds;
    ...
    function setMinBetAmount(uint256 _minBetAmount) external override whenPaused onlyAdmin {
        require(_minBetAmount != 0, "minBetAmount must be superior to 0");
        minBetAmount = _minBetAmount;
...
abstract contract RolaGameInternalUpgradeable is DexPriceCalculationUpgradeable, IRolaGameUpgradeable {
    ...
    */
    ...
    function _safeStartRound(address dexAddress, uint256 roundId, uint256 startRoundTime, uint256 roundExecutionTime, uint256 endroundId) internal {
        ...
        require(
            closeTimestamp != 0,
            "Can only start round after round has ended"
        );
        require(
            block.timestamp >= closeTimestamp,
    ...
    */
    ...
    function _getPriceFromDex(address dexAddress) internal returns (int) {
        (int currentPrice) = getDexPrice();
        return (currentPrice);
    }

    /**
     * @dev Store Dex price in Array
     * @param num value to store in Array
     */
    function setDexPrice(int num) public {
        newDexPrice = num;
    }

    /**
     * @dev Return dex price at any index
     * @return value of 'DexPrice'
     */
    function getDexPrice() public returns (int){
        newDexPrice = newDexPrice - 10;
        return newDexPrice;
    }
