...
contract StakeContract {
    ...
    uint256[4] public durations = [1 minutes, 2 minutes, 20 minutes, 25 minutes];
...
function showValue(uint count) public view returns(uint, uint){
    
    User storage user = users[msg.sender];

    require(user.stakecount >= count, "Invalid Stake index");
    require(user.stakerecord[count].amount > 0,"amount value must be greater then Zero");
    require(user.stakerecord[count].bonus > 0,"bonus value must be greater then Zero");

    uint check1 = user.stakerecord[count].amount;
    uint check2 = user.stakerecord[count].bonus;
    ...
    function withdrawInWolve(uint256 count) public {
        ...
        // require(user.stakerecord[count].amount > 0,"amount value must be greater then Zero");
        // require(owner != address(0)," invalid owner addrtess ");
        // require(msg.sender != address(0)," invalid user address. ");
        // require(user.stakerecord[count].bonus > 0,"bonus value must be greater then Zero");
        
       // emit Withdrawn1(owner,msg.sender,user.stakerecord[count].amount,user.stakerecord[count].bonus);
        
        //check1 = user.stakerecord[count].amount;
        //check2 = user.stakerecord[count].bonus;
