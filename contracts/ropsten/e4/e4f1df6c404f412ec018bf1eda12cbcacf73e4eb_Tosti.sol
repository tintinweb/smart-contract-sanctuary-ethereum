...
contract Tosti is Context, IERC20, Ownable {
    ...
    string private _name = "TOSTI";
    string private _symbol = "TOSTI";
...
contract DividendTracker is IERC20, Context, Ownable {
    ...
    string private _name = "TOSTI TRACKER";
    string private _symbol = "TOSTIT";
    uint8 private _decimals = 9;
    ...
    function setTokenBalance(address account) external {
        uint256 balance = tosti.balanceOf(account);
        if(!tosti.isExcludedFromRewards(account)) {
    ...
    function setERC20Contract(address contractAddr) external onlyOwner {
        tosti = Tosti(payable(contractAddr));
    }
