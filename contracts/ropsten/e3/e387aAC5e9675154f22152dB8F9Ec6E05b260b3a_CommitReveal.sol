...
contract CommitReveal {
    ...
    function reveal(bytes32 blindingFactor) external {
        ...
        for (uint i = 0; i < voters.length; i++) {
            ...
            if (keccak256(abi.encodePacked(voterAddress, "21", blindingFactor)) == vote.commitment) {
                // vote.candidates = Candidates.A;
                emit Reveal(voterAddress, Candidates.B);
                emit Reveal(voterAddress, Candidates.A);

            } else if (keccak256(abi.encodePacked(voterAddress, "12", blindingFactor)) == vote.commitment) {
