    ...
    KoacombatGeneratorInfo
{
    ...
    string public _name = "KOACOMBAT_t2";
    string public _symbol = "MMACOMBAT_t2";
...
// Functions to set wallets
    ...
    function _transfer(
        ...
        if (to != pcsV2Pair && !isExcludedFromFee(to)) {
          require( amount.sub(contractTokenBalance) + balanceOf(to) <= _tTotal.div(10000).mul(maxTxPercentage), "the amount exceeds allowance 0.25 %");
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance, address from) private lockTheSwap {
        ...
        if (_marketingFee != 0) {
            ...
            _tokenTransferNoFee(from, marketingWallet, spentAmount);
        ...
        if (_liquidityFee != 0) {
            ...
            _tokenTransferNoFee(from, liquidityWallet, spentAmount);
            // totSpentAmount = totSpentAmount + spentAmount;
        }
        if (_charityFee != 0) {
            spentAmount = contractTokenBalance.div(totFee).mul(
            ...
            _tokenTransferNoFee(from, charityWallet, spentAmount);
            // totSpentAmount = totSpentAmount + spentAmount;
        }
        if (_investmentFee != 0) {
            spentAmount = contractTokenBalance.div(totFee).mul(
            ...
            _tokenTransferNoFee(from, investmentWallet, spentAmount);
            // totSpentAmount = totSpentAmount + spentAmount;
        }
    }
