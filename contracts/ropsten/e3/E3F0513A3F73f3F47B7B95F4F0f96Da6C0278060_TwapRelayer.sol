...
contract TwapRelayer is ITwapRelayer {
    ...
    mapping(address => uint256) public override tokenLimitMax;
    ...
    function setSwapFee(address pair, uint256 fee) external override {
        ...
        swapFee[pair] = fee;
        emit SetSwapFee(pair, fee);
    }

    function setTwapInterval(address pair, uint32 _interval) external override {
        require(msg.sender == owner, 'TR00');
        require(_interval != twapInterval[pair], 'TR01');
        require(_interval > 0, 'TR56');
        twapInterval[pair] = _interval;
        emit TwapIntervalSet(pair, _interval);
    }

    function setPairEnabled(address pair, bool enabled) external override {
        require(msg.sender == owner, 'TR00');
        require(isPairEnabled[pair] != enabled, 'TR01');
        isPairEnabled[pair] = enabled;
        emit PairEnabled(pair, enabled);
    }
    ...
    function setTokenLimitMax(address token, uint256 limit) external override {
        require(msg.sender == owner, 'TR00');
        require(limit != tokenLimitMax[token], 'TR01');
        tokenLimitMax[token] = limit;
        emit TokenLimitMaxSet(token, limit);
    }
    ...
    function sell(Orders.SellParams calldata sellParams) external payable override lock returns (uint256 orderId) {
        ...
        if (sellParams.wrapUnwrap == false) {
        ...
        require(isPairEnabled[pair], 'TR54');

        (uint256 _amountIn, uint256 _amountOut, uint256 fee) = swapExactIn(
            pair,
            inverted,
            sellParams.tokenIn,
            sellParams.tokenOut,
        ...
        require(_amountOut >= sellParams.amountOutMin, 'TR37');
    ...
    function buy(Orders.BuyParams calldata buyParams) external payable override lock returns (uint256 orderId) {
        ...
        (uint256 _amountIn, uint256 _amountOut, uint256 fee) = swapExactOut(
            pair,
            inverted,
            buyParams.tokenIn,
            buyParams.tokenOut,
        ...
        require(_amountIn <= buyParams.amountInMax, 'TR08');
        ...
        if (buyParams.wrapUnwrap == true) {
            uint256 balanceAfter = address(this).balance;
            if (balanceAfter > balanceBefore) {
                TransferHelper.safeTransferETH(msg.sender, balanceAfter.sub(balanceBefore), ETHER_TRANSFER_COST);
            }
        }
    }
    ...
    function getPair(address tokenA, address tokenB) internal view returns (address pair, bool inverted) {
        ...
        pair = ITwapFactory(factory).getPair(token0, token1);
        ...
        )
    {
        ...
        require(_amountOut <= calculatedAmountOut, 'TR2E'); // TODO error code
    }
        ...
        )
    {
        ...
        // Do we need to account for the .add(1) in this function?
        ...
        fee = calculatedAmountIn.mul(swapFee[pair]).div(PRECISION);
        uint256 amountInPlusFee = calculatedAmountIn.add(fee);
        _amountIn = transferIn(tokenIn, amountInPlusFee, wrapUnwrap);

        require(_amountIn >= amountInPlusFee, 'TR2E'); // TODO error code
    }
    ...
    function calculateAmountIn(
        ...
        amountIn = amountOut.mul(10**18).div(price);

        uint8 decimals = inverted ? yDecimals : xDecimals;
        if (decimals > 18) {
            amountIn = amountIn.mul(10**(decimals - 18));
        } else if (decimals < 18) {
    ...
    function calculateAmountOut(
        ...
        (uint8 xDecimals, uint8 yDecimals, uint256 price) = getPriceByPairAddress(pair, inverted);
        amountOut = amountIn.mul(price).div(10**18);

        uint8 decimals = inverted ? xDecimals : yDecimals;
        if (decimals > 18) {
            amountOut = amountOut.mul(10**(decimals - 18));
        } else if (decimals < 18) {
            amountOut = amountOut.div(10**(18 - decimals));
        }
    }
    ...
    function getPricesVolume(
        ...
        require(amountsLength > 0, 'TR57');

        (address pair, bool inverted) = getPair(tokenIn, tokenOut);

        prices = new uint256[](amountsLength);
        for (uint256 i; i < amountsLength; ++i) {
            if (isSellSide) {
                uint256 fee = amounts[i].mul(swapFee[pair]).div(PRECISION);
                uint256 amountInMinusFee = amounts[i].sub(fee);
                prices[i] = calculateAmountOut(pair, inverted, amountInMinusFee);
                require(prices[i] >= tokenLimitMin[tokenOut], 'TR03');
                require(
                    prices[i] <= IERC20(tokenOut).balanceOf(address(this)).mul(tokenLimitMax[tokenOut]).div(PRECISION),
                    'TR3A'
                );
            } else {
                require(amounts[i] >= tokenLimitMin[tokenOut], 'TR03');
                require(
                    amounts[i] <= IERC20(tokenOut).balanceOf(address(this)).mul(tokenLimitMax[tokenOut]).div(PRECISION),
                    'TR3A'
                );
                uint256 calculatedAmountIn = calculateAmountIn(pair, inverted, amounts[i]);
                uint256 fee = calculatedAmountIn.mul(swapFee[pair]).div(PRECISION);
                prices[i] = calculatedAmountIn.add(fee);
            }
        }
    }
        ...
        )
    {
        ...
        int256 decimalsConverter = (10**(18 + xDecimals - yDecimals)).toInt256();
        averagePrice = getAveragePrice(pair, uniswapPair, decimalsConverter);
    }

    function getAveragePrice(
        address pair,
        address uniswapPair,
        int256 decimalsConverter
        ...
        if (sqrtRatioX96 <= type(uint128).max) {
            ...
            return FullMath.mulDiv(ratioX192, uint256(decimalsConverter), 1 << 192);
        } else {
            uint256 ratioX128 = FullMath.mulDiv(sqrtRatioX96, sqrtRatioX96, 1 << 64);
            return FullMath.mulDiv(ratioX128, uint256(decimalsConverter), 1 << 128);
        }
    }
    ...
    function transferOut(
        ...
        require(amount <= IERC20(token).balanceOf(address(this)).mul(tokenLimitMax[token]).div(PRECISION), 'TR3A');
        if (token == weth) {
            if (unwrap) {
                IWETH(token).withdraw(amount);
                TransferHelper.safeTransferETH(to, amount, ETHER_TRANSFER_COST);
            } else {
            ...
            TransferHelper.safeTransfer(token, to, amount);
            uint256 balanceAfter = IERC20(token).balanceOf(address(this));
            require(balanceBefore > balanceAfter, 'TR2C'); // TODO error code
...
interface ITwapRelayer {
    ...
    event PairEnabled(address pair, bool enabled);
    event SetSwapFee(address pair, uint256 fee);
    ...
    event TokenLimitMaxSet(address token, uint256 limit);
    event Approval(address token, address to, uint256 amount);
    ...
    function isPairEnabled(address pair) external returns (bool);

    function setPairEnabled(address pair, bool enabled) external;

    function executionGasLimit() external returns (uint256);

    function setExecutionGasLimit(uint256 limit) external;

    function gasPriceMultiplier() external returns (uint256);

    function setGasPriceMultiplier(uint256 multiplier) external;

    function tokenLimitMin(address token) external returns (uint256);

    function setTokenLimitMin(address token, uint256 limit) external;

    function tokenLimitMax(address token) external returns (uint256);

    function setTokenLimitMax(address token, uint256 limit) external;
    ...
    // TODO
    // getPrice(address tokenIn, address tokenOut) returns (uint256 price); // Reverts if invalid tokens
    // quoteSell(address tokenIn, address tokenOut, uint256 amountIn) returns (uint256 amountOut); // Reverts if invalid tokens or not enough liquidity
    // quoteBuy(address tokenIn, address tokenOut, uint256 amountOut) returns (uint256 amountIn); // Reverts if invalid tokens or not enough liquidity
}
