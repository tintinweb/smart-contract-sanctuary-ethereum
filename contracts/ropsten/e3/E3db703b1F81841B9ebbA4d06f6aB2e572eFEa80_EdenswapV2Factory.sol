...
contract EdenswapV2Factory is IUniswapV2Factory {
...
contract UniswapV2Pair is UniswapV2ERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'KishiswapV2: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'KishiswapV2: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1, uint _totalFee, uint _alpha, uint _beta) external {
        require(msg.sender == factory, 'KishiswapV2: FORBIDDEN'); // sufficient check
    ...
    function updateFee(uint _totalFee, uint _alpha, uint _beta) external {
        require(msg.sender == factory, 'KishiswapV2: FORBIDDEN');
        totalFee = _totalFee;
        alpha = _alpha;
        beta = _beta;
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'KishiswapV2: OVERFLOW');
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'KishiswapV2: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'KishiswapV2: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'KishiswapV2: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'KishiswapV2: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'KishiswapV2: INVALID_TO');
        ...
        require(amount0In > 0 || amount1In > 0, 'KishiswapV2: INSUFFICIENT_INPUT_AMOUNT');
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'KishiswapV2: K');
        }
...
contract UniswapV2ERC20 {
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'KishiswapV2: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'KishiswapV2: INVALID_SIGNATURE');
