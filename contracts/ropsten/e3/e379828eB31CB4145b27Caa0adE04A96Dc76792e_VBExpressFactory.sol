...
contract VBExpressFactory is IVBExpressFactory {
    mapping(address => mapping(address => mapping(uint24 => mapping(uint24 => address)))) public override getPair;
    address[] public override allPairs;

    event PairCreated(address indexed token0, address indexed token1, uint24 k0, uint24 k1, address indexed pair, uint);

    function allPairsLength() external override view returns (uint) {
        return allPairs.length;
    }

    function createPair(address tokenA, address tokenB, uint24 kA, uint24 kB) external override returns (address pair) {
        require(tokenA != tokenB, "VBchain Express:: IDENTICAL_ADDRESSES");
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        (uint24 k0, uint24 k1) = tokenA < tokenB ? (kA, kB) : (kB, kA);
        require(token0 != address(0), "VBchain Express: ZERO_ADDRESS");
        require(getPair[token0][token1][k0][k1] == address(0), "VBchain Express: PAIR_EXISTS"); // single check is sufficient
        bytes memory bytecode = type(VBExpressPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1, k0, k1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IVBExpressPair(pair).initialize(tokenA, tokenB, kA, kB);
        getPair[token0][token1][k0][k1] = pair;
        getPair[token1][token0][k1][k0] = pair; // populate mapping in the reverse direction
        allPairs.push(pair);
        emit PairCreated(token0, token1, k0, k1, pair, allPairs.length);
    }
}