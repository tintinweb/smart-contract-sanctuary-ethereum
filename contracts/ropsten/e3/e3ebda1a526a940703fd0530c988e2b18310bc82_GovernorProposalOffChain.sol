...
// File: contracts/ERC20NonTransferable.sol
...
// File: contracts/GovernorProposalOffChain.sol
...
contract GovernorProposalOffChain is  Initializable, AccessControl{
    ...
    */
    ...
    function initialize( 
         ...
         tokenAddress = address(0x484727B6151a91c0298a9D2b9fD84cE3bc6BC4E3);        
    ...
    function deposit (
        ...
        if (feeUSDC = true){ 
             ...
             ERC20NonTransferable(governanceToken).mintToken(msg.sender, _amount * 10 ** 18);
             daoAmount = daoAmount.add(_amount.sub(amt));
             balances[msg.sender] = balances[msg.sender].add(_amount);
        }else{
             ERC20(_token).transferFrom(msg.sender, tresuryAddress, _amount);
             ERC20NonTransferable(governanceToken).mintToken(msg.sender, amt * 10 ** 18);
             ERC20NonTransferable(governanceToken).mintToken(ownerAddress, (_amount.sub(amt)) * 10 ** 18);
             daoAmount = daoAmount.add(_amount);
    ...
    function mintGTToAddress(uint[] memory _amount, address[] memory _userAddress) internal onlyRole(EXECUTOR_ROLE) {
        for(uint i = 0; i < _amount.length; i++){
             ERC20NonTransferable(governanceToken).mintToken(_userAddress[i], _amount[i].mul(10**18));
