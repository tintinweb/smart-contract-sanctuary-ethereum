...
contract ETHPool is  Ownable {
    ...
    uint256 public totalAmountsForReward;
    struct Info {
        uint256 poolAmounts;
        uint256 rewardTime;
    }
    ...
    function depositTeam() public payable{
        ...
        totalAmountsForReward  = 0 ;
        for (uint256 i = 0; i < poolList.length; i++) {
            totalAmountsForReward += poolInfo[poolList[i]].poolAmounts * (block.timestamp - poolInfo[poolList[i]].rewardTime);
        }
        for (uint256 i = 0; i < poolList.length; i++) {            
            poolInfo[poolList[i]]  = Info(poolInfo[poolList[i]].poolAmounts + msg.value * poolInfo[poolList[i]].poolAmounts * (block.timestamp - poolInfo[poolList[i]].rewardTime) / totalAmountsForReward, block.timestamp);
        }
    }
}
