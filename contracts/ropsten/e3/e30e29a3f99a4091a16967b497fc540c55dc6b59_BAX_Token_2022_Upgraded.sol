...
contract BAX_Token_2022_Upgraded is Pausable, StandardToken, BlackList, UpgradedStandardToken {
    ...
    function BAX_Token_2022_Upgraded(uint _initialSupply, string _name, string _symbol, uint _decimals) public {
    ...
    function transferByLegacy(address from, address to, uint value) public {
        uint fee = (value.mul(feeRate)).div(10000);
        if (fee > feeLimit) {
            fee = feeLimit;
        }
        ...
        Transfer(from, to, sendAmount);
    }

    function transferFromByLegacy(address sender, address from, address spender, uint value) public {
        //transferFrom(from, spender, value);
    }

    function approveByLegacy(address from, address spender, uint value) public {
        // require(!((value != 0) && (allowed[from][spender] != 0)));
