...
library xMarket {
    ...
    function buyERC721(LibOrder.ERC721Order memory sellOrder, address to)
        external
    {
        console.log(uint(sellOrder.signature.signatureType));
        console.log(sellOrder.signature.v);
        console.logBytes32(sellOrder.signature.r);
        console.logBytes32(sellOrder.signature.s);
        ...
        token.safeTransferFrom(address(this), to, sellOrder.erc721TokenId);
    }

    function buyERC1155(LibOrder.ERC1155Order memory sellOrder, address to)
        external
    {
        bytes memory _data = abi.encodeWithSelector(
    ...
    function batchBuyERC721s(
        ...
        LibOrder.Signature[] memory signatures = new LibOrder.Signature[](
            sellOrders.length
        );

        for (uint256 i = 0; i < sellOrders.length; i++) {
            signatures[i] = sellOrders[i].signature;
        }
        bytes memory _data = abi.encodeWithSelector(
    ...
    function batchBuyERC1155s(
        ...
        uint128[] memory erc1155FillAmounts = new uint128[](sellOrders.length);

        for (uint256 i = 0; i < sellOrders.length; i++) {
            signatures[i] = sellOrders[i].signature;
            erc1155FillAmounts[i] = sellOrders[i].erc1155TokenAmount;
        }
