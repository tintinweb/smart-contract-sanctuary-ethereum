...
contract MuxxoCat is Ownable, ERC721AQueryable, ERC2981 {
    ...
    uint16 public constant MAX_PRESALE_SUPPLY = 4499;
    uint8 public constant MAX_MINT_PRESALE_NORMAL = 1;
    uint8 public constant MAX_MINT_PRESALE_SPECIAL = 1;
    uint8 public constant MAX_MINT_PUBLIC = 20;

    string private baseTokenUri;

    uint16 public TOTAL_PUBLIC_MINTED;
    uint16 public TOTAL_PRESALE_MINTED;
    ...
    mapping(address => uint16) public specialMinted;
    ...
    function mintPublic(uint16 _quantity) external payable callerIsUser {
        ...
        require(
            (publicSaleMinted[msg.sender] + _quantity) <= MAX_MINT_PUBLIC,
            'Muxxo Cat :: Already minted max allowed amount!'
        );
        TOTAL_PUBLIC_MINTED += _quantity;
    ...
    function mintPrivate(bytes32[] memory _merkleProof, uint16 _quantity) external payable callerIsUser {
        ...
        require((totalSupply() + _quantity) <= MAX_SUPPLY, 'Muxxo Cat :: Cannot mint beyond max supply');
        require(
            (TOTAL_PRESALE_MINTED + _quantity) <= MAX_PRESALE_SUPPLY,
            'Muxxo Cat :: Beyond Max Supply for Minting Genesis'
        );
        require(_quantity >= 1, 'Muxxo Cat :: Invalid quantity');
        require(
            _quantity <= MAX_MINT_PRESALE_NORMAL + MAX_MINT_PRESALE_SPECIAL,
            'Muxxo Cat :: Cannot mint beyond max supply'
        );

        require(
            (presaleMinted[msg.sender] + _quantity) <= MAX_MINT_PRESALE_NORMAL + MAX_MINT_PRESALE_SPECIAL,
            'Muxxo Cat :: Already minted max allowed amount!'
        );
        ...
        if (presaleMinted[msg.sender] == 0) {
            _privateMint(1);
            if (_quantity - MAX_MINT_PRESALE_NORMAL <= MAX_MINT_PRESALE_SPECIAL) {
                _specialMint(1);
            }
        } else {
            _specialMint(1);
    ...
    function _privateMint(uint16 _quantity) internal callerIsUser {
        TOTAL_MINTED+= _quantity;
        TOTAL_PRESALE_MINTED += _quantity;
        presaleMinted[msg.sender] += _quantity;
        _safeMint(msg.sender, _quantity);
    }

    function _specialMint(uint16 _quantity) internal callerIsUser {
        require(msg.value >= (SPECIAL_MINT_PRICE * _quantity), 'Muxxo Cat :: Payment is below the price');
        TOTAL_MINTED+= _quantity;
        TOTAL_SPECIAL_MINTED += _quantity;
        TOTAL_PRESALE_MINTED += _quantity;
        presaleMinted[msg.sender] += _quantity;
        specialMinted[msg.sender] += _quantity;
        _safeMint(msg.sender, _quantity);
