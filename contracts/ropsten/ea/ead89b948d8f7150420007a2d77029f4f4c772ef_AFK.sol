...
contract ERC20 is Context, IERC20, Ownable {
  ...
  address payable public developmentAddress =
    payable(0xda4670DAa206A70Af130681F006aECAe085c0834);
  ...
  function setdevelopmentAddress(address _addy) external onlyOwner {
    developmentAddress = payable(_addy);
  }
  ...
  function setdevelopmentFee(uint256 percentage) external onlyOwner {
    require(percentage <= 100, 'Rate should be less than or equal to 100%');
    developmentFee = percentage;
  }

  function setRewardsFee(uint256 percentage) external onlyOwner {
    require(percentage <= 100, 'Rate should be less than or equal to 100%');
    rewardsFee = percentage;
  }
  ...
  function _transfer(
    ...
    _rewardsLastClaim[recipient] = block.timestamp;
    _rewardsLastClaim[sender] = block.timestamp;
  ...
  function _takeFeeOnSwap(
    ...
    if (developmentFee == 0 && rewardsFee == 0) return _amount;
    uint256 _amountToContract = _amount.mul(developmentFee.add(rewardsFee)).div(100);
  ...
  function _swapTokens(uint256 _contractTokenBalance) private lockTheSwap {
    ...
    uint256 amountDueDev = (ethBalanceUpdate.mul(developmentFee)).div(developmentFee.add(rewardsFee));
    uint256 amountDueRewards = ethBalanceUpdate.sub(amountDueDev);

    ethRewardsBalance = ethRewardsBalance.add(amountDueRewards);

    // send to development address
    if (amountDueDev > 0) {
      _sendETHTodevelopment(amountDueDev);
    }
  }
  ...
  function _sendETHTodevelopment(uint256 _amount) private {
    // Ignore the boolean return value. If it gets stuck, then retrieve via `emergencyWithdraw`.
    developmentAddress.call{ value: _amount }('');
  }

  function calculateRewardsShare(address wallet) public view returns(uint256) {
    uint256 rewardsShare = ethRewardsBalance.mul(balanceOf(wallet)).div(_totalSupply.sub(balanceOf(deadAddress)));
    return rewardsShare;
  }
