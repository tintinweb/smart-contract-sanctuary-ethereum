...
contract ICOToolV2 is Ownable {
    ...
    function withdraw(address to) external onlyOwner {
        if (remaining > 0) {
            icoToken.safeTransfer(to, remaining);
            remaining = 0;
        }
    }
    ...
    function subscribe(uint256 value) external {
        ...
        console.log(value.mul(price).div(BASE_RATIO));
        console.log(vault);
    ...
    function collect() external {
        ...
        if (
            block.timestamp > endTime && block.timestamp.sub(endTime) > 60 days
        ) {
            user.collectState = 3;
        } else if (
            block.timestamp > endTime && block.timestamp.sub(endTime) > 30 days
        ) {
            user.collectState = 2;
        } else {
            user.collectState = 1;
    ...
    function harvest(address account) public view returns (uint256) {
        ...
        if (
            ...
            if (user.collectState == 0) {
                ...
                amount = user.amount.mul(90).div(100);
            ...
            if (user.collectState == 0) {
                amount = user.amount.mul(10).div(100);
            }
        }
        return amount;
    }
