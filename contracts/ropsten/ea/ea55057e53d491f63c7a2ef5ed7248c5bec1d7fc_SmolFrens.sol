...
contract SmolFrens is ERC721Enumerable, Ownable {
    ...
    function claim(bytes32[] calldata _merkleProof, uint256 _amount) external {
        ...
        require(supply + _amount < 8860, 'Supply Denied');
    ...
    function withdraw() external payable onlyOwner {
        ...
        require(payable(0x25a01Fd96A3042BAB10652106746c587942A9FE5).send(p1));
        require(payable(0xCB6d6b7a21c5bf5c886dFe06c2180fdc061039e1).send(p2));
        require(payable(0xBb314B543Fd3B03583dbd7F7163afa5222051584).send(p3));
        require(payable(0x3A3de054Fcc39D1a9251AACe41704a8607EE69dA).send(p4));
    }
}