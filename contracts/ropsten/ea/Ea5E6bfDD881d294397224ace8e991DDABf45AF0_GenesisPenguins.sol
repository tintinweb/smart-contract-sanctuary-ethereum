...
// File contracts/GenesisPenguins.sol
...
contract GenesisPenguins is ERC721A, Ownable, ReentrancyGuard {
    //minting params
    uint256 public constant PUBLIC_COST = 0.05 ether;
    uint256 public constant WHITELIST_COST = 0.04 ether;
    uint256 public constant MAX_SUPPLY = 1000;
    uint256 public constant MAX_PER_TRANSACTION = 10;
    //useless counter to test ownerOf() cost when used in a transaction
    uint256 internal uselessCounter;
    //management
    bytes32 public merkleRoot = 0x0;
    uint256 public mintingStage;
    string public baseURI =
        "ipfs://QmREERborddMBHN2fV4854uNqtJwAbiivbqui4CjU9oPrm";
    ...
    //Helper Modifiers:
    /// @notice Checks if user exceeds current mint allocation
    modifier allocationCheck(uint256 newTotal, uint256 limit) {
        require(newTotal <= limit, "Mint limit exceeded for user");
        _;
    }

    /// @notice Checks if user payment is sufficient
    modifier paymentCheck(uint256 cost, uint256 mintAmount) {
        require(cost * mintAmount == msg.value, "Inusfficient Eth");
        _;
    }

    /// @notice Checks for valid merkle proof
    modifier merkleCheck(uint256 allocation, bytes32[] calldata merkleProof) {
    ...
    //bad practice, but currently works until tx.origin is patched out
    /// @notice Checks EOA
    modifier botCheck() {
    ...
    /// todo: emit any events that are useful for team use
    ...
    function whitelistMint(
        ...
        payable
        allocationCheck(
            minted[mintingStage][msg.sender] + _mintAmount,
            allocation
        )
        paymentCheck(WHITELIST_COST, _mintAmount)
        merkleCheck(allocation, merkleProof)
        nonReentrant
    {
    ...
    function publicMint(uint256 _mintAmount)
        external
        payable
        paymentCheck(PUBLIC_COST, _mintAmount)
        botCheck
        nonReentrant
    {
        require(isPublicSale, "Public sale closed.");
        require(_mintAmount <= MAX_PER_TRANSACTION, "Exceeded max per mint");
        require(totalSupply() + _mintAmount <= MAX_SUPPLY, "Out of stock.");
        _safeMint(msg.sender, _mintAmount);
    }
    ...
    /// todo: maybe have seperate function to increment/decrement minting stage seperately?
    function setMerkleRoot(bytes32 _root) external onlyOwner {
        merkleRoot = _root;
        mintingStage = mintingStage + 1;
    }
