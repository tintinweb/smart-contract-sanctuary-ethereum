...
contract NFT is ERC721, Ownable {
    ...
    uint256 defaultPrice = 0.1 ether;
    ...
    constructor() public ERC721("FreedomK ", "FDK") {
        _contractOwner = msg.sender;
    }

    function adminMint(string calldata _tokenURI, address _toAddress, uint256 _price) external onlyOwner returns (uint) {
    ...
    function mint(string calldata _tokenURI, address _toAddress) external payable returns (uint) {
        require(msg.value >= defaultPrice, "error: price is not enough");

        uint _tokenId = totalSupply() + 1;
        price[_tokenId] = msg.value;
        listedMap[_tokenId] = true;
        ...
        emit Minted(_toAddress, msg.value, _tokenId, _tokenURI);
    ...
    function _trade(uint _id) internal {
        ...
        uint _commissionValue = price[_id].div(royaltyFee).mul(100) ;
    ...
    function updateContractOwner(address payable _owner) external onlyOwner {
