...
contract WaifuPotions is ERC1155, ERC1155Supply, ERC1155Burnable, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    uint256 public maxTokensPerTransactionPublic = 8;
    uint256[] public maxTokensPublic = [500,350,250,100];
    uint256[] public numberMintedPublic = [0,0,0,0];
    uint256[] public tokenPricePublic = [0.02 ether,0.025 ether,0.03 ether,0.035 ether];
     ...
     */
    function mint(uint256 tokenId, uint256 numberOfTokens) external payable nonReentrant {
        ...
        require(
	        msg.value >= tokenPricePublic[tokenId] * numberOfTokens,
            "You sent the incorrect amount of ETH"
        );
        require(
            numberMintedPublic[tokenId] + numberOfTokens <= maxTokensPublic[tokenId], 
            "Not enough Public Mint tokens left to mint that many"
        );

        numberMintedPublic[tokenId] += numberOfTokens;
    ...
    */
    function initFreeWalletList(address[] memory walletList, uint256[][] memory quantities) external onlyOwner {
        require(walletList.length == quantities.length, "length of arrays do not match");
	    for (uint256 i = 0; i < walletList.length; i++) {
		    freeWalletList[walletList[i]] = quantities[i];
	    }
    }
     ...
     */
    function checkAddressOnFreeWalletList(address wallet, uint256 tokenId) public view returns (uint256) {
	    return freeWalletList[wallet][tokenId];
    }
    ...
    function mintFreeWalletList() external nonReentrant {
        ...
        for (uint256 i = 0; i < freeWalletList[msg.sender].length; i++) {
            uint256 amount = freeWalletList[msg.sender][i];
            freeWalletList[msg.sender][i] = 0;
            if (amount > 0) {
    ...
    */
    function setTokenPricePublic(uint256 tokenId, uint256 tokenPrice) external onlyOwner {
        require(tokenPrice >= 0, "Must be greater or equal then zer0");
        tokenPricePublic[tokenId] = tokenPrice;
    }

    /**
    *  @notice Set max tokens allowed minted in public sale - maxTokensPublic
    */
    function setMaxTokensPublic (uint256 tokenId, uint256 amount) external onlyOwner {
        require(amount >= 0, "Must be greater or equal than zer0");
        maxTokensPublic[tokenId] = amount;
    }

    /**
    *  @notice Set total number of tokens minted in public sale - numberMintedPublic
    */
    function setNumberMintedPublic(uint256 tokenId, uint256 amount) external onlyOwner {
        require(amount >= 0, "Must be greater or equal than zer0");
        numberMintedPublic[tokenId] = amount;
    }
