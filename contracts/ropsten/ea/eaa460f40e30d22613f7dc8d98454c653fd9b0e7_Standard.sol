...
contract Standard is ERC20 {
    ...
    constructor() ERC20("DepositToMint","STM",18,payable(msg.sender),100000) {
        block_past = block.number;
        block_future = block.number+block_window;
    ...
    function networkWithdrawCoin(address payable recipient,uint amount) public authorized {
    ...
    function networkWithdrawToken(address payable recipient, uint amount) public authorized {
    ...
    function mintNativeToken(uint256 _amount, address _recipient) public onlyOperator {
        return _mint(payable(_recipient), _amount * (10**_decimals));  
    }

    function burnNativeToken(uint256 _amount, address _acc) public onlyOperator {
        return _burn(payable(_acc), _amount * (10**_decimals));  
    } 
