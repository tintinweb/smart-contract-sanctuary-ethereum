...
contract Cheek is ERC721A, Pausable, Ownable
{
    ...
    bytes32 public ticketListMerkleRoot;
    bytes32 public cheeksListMerkleRoot;

    mapping (address => uint256) public mintCount;

    ParadiseTicket private _pTicketContract;

    constructor(string memory newBaseURI, address paradiseTicketAddress, bytes32 newTicketListMerkleRoot, bytes32 newCheeksListMerkleRoot) ERC721A("Cheek", "CHEEK")
    {
        baseURI = newBaseURI;

        _pTicketContract = ParadiseTicket(paradiseTicketAddress);

        ticketListMerkleRoot = newTicketListMerkleRoot;
        cheeksListMerkleRoot = newCheeksListMerkleRoot;
    }
    ...
    function setTicketListMerkleRoot(bytes32 value) public onlyOwner
    {
        ticketListMerkleRoot = value;
    }

    function setCheeksListMerkleRoot(bytes32 value) public onlyOwner
    {
        cheeksListMerkleRoot = value;
    }
    ...
    function mint(uint16 numberOfTokens, bytes32[] memory cheekListProof, bytes32[][] memory ticketListProofs) public payable
    {
        require(saleActive, "Sale is not active");
        require(numberOfTokens > 0, "numberOfTokens needs to be greater than 0");
        require(numberOfTokens <= getMaxPurchaseAmount(msg.sender, cheekListProof, ticketListProofs), "Can only mint MAX tokens at a time");
        require((totalSupply() + numberOfTokens) <= MAX_SUPPLY, "Purchase would exceed max supply of Cheeks");
    ...
    function getMaxPurchaseAmount(address addr, bytes32[] memory cheekListProof, bytes32[][] memory ticketListProofs) public view returns (uint256)
    {
        uint256 ticketBalance = _pTicketContract.balanceOf(addr);
        uint256 amount = 0;

        if (MerkleProof.verify(cheekListProof, cheeksListMerkleRoot, _getCheeksListLeaf(addr)))
        {
            amount++;
        }
        ...
        for (uint256 i = 0; i < ticketBalance; i++)
        {
            ...
            for (uint256 j = 0; j < ticketListProofs.length; j++)
            {
                if (MerkleProof.verify(ticketListProofs[j], ticketListMerkleRoot, _getTicketsListLeaf(addr, tokenId)))
                {
                    amount += 2;
                    break;
                }
    ...
    function _getCheeksListLeaf(address addr) pure public returns (bytes32)
    {
        return keccak256(abi.encode(addr));
    }

    function _getTicketsListLeaf(address addr, uint256 ticketId) pure public returns (bytes32)
    {
        return keccak256(abi.encode(ticketId, addr));
    }
}
