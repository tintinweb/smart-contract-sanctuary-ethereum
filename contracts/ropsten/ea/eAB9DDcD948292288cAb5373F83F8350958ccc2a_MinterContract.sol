...
contract MinterContract {
    ...
    function _mint (uint256 charityId, uint256 amount ) public {
        _charity.addCharity(charityId, amount);
        _starify.safeMint(msg.sender, amount);
    }
...
contract Starify is ERC721A, Ownable {
    constructor(address minterAddress, uint256 maxBatchSize_, uint256 collectionSize_) ERC721A("Starify", "STF", maxBatchSize_, collectionSize_) {
    ...
    bool public isLimitSet = false;

    mapping (uint256 => Star) public tokenDetails;


    event tokenCreated(uint256 tokenId, address tokenOwner);
