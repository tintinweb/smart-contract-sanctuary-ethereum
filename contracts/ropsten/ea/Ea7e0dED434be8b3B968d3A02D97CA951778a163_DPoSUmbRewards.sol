...
contract DPoSUmbRewards is DPoSCore {
    RewardsSettings public tokenSettings;
    ...
    function setLimit(uint96 _limit) external onlyOwner {
        uint96 totalPaid = tokenSettings.totalPaid;

        if (_limit < totalPaid) {
            _limit = totalPaid;
        }

        tokenSettings.totalLimit = _limit;
    ...
    function transferReward(IERC20 _token, address _recipient, uint256 _totalAmount, bytes memory _signature) external {
        require(tokenSettings.transferringActive, "transferring NOT active");
        ...
        _token.transfer(_recipient, rewardToClaim);
    }
    ...
    function mintReward(Token _token, address _recipient, uint256 _totalAmount, bytes memory _signature) external {
        require(tokenSettings.mintingActive, "minting NOT active");
...
abstract contract DPoSCore is Ownable {
    ...
    function _executeClaim(
        RewardsSettings storage _tokenSettings,
        ...
        returns (uint256 rewardToClaim)
    {
        ...
        unchecked {
            ...
            RewardsSettings memory settings = _tokenSettings;
            require(settings.totalPaid + rewardToClaim <= settings.totalLimit, "reward limit");

            // overflow not possible because of require(r.totalPaid + rewardToClaim <= r.totalLimit)
            _tokenSettings.totalPaid = uint96(settings.totalPaid + rewardToClaim);
