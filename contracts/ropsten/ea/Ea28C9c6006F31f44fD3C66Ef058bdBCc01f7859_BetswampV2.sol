...
contract BetswampV2 {
    ...
    // maps user address to a specific category
    mapping(address=>mapping(string=>bool)) private isInchargeOfSubcategory;

    // maps subcategory to whether it is assigned a specific user
    mapping(string=>bool) private isSubcategoryAssigned;

    // maps an address to the subcategory it was assigned.
    mapping(address=>string) private assignedSubcategory;
    ...
    // maps whether a user paused betting
    mapping(address=>bool) private paused;

    // maps user to their pause duration
    mapping(address=>uint256) private pause_duration;
    ...
    */
    ...
    /**
     * @dev modifier checks if user paused betting
    */
    modifier isPaused(address _address){
        require(paused[_address] == false || pause_duration[_address] <= currentTime(), "BP");
        if(paused[_address] == true){
            pause_duration[_address] = 0;
            paused[_address] = false;
        }
        _;
    }
    ...
    */
    ...
    function addSubCategory(Category _category, string memory _sub_category)
        external
        returns (bool)
    {
    ...
    /**
     *@dev function assigns and unsigns addr explicit right to
     * create markets on restricted categories
    */
    function assignSubcategory(address _address, string memory _subcategory, bool _outcome) 
        external
        returns (bool)
    {
        context_address.onlyOwner(msg.sender);
        isInchargeOfSubcategory[_address][_subcategory] = _outcome;
        isSubcategoryAssigned[_subcategory] = _outcome;
        // check val of outcome
        if(_outcome == false){
            delete assignedSubcategory[_address];
        }else{
            assignedSubcategory[_address] = _subcategory;
        }
        return true;
    }

    function isAssignedSubcategory(string memory _subcategory, address _address) 
        internal view
        returns (bool)
    {
        // check if subcategory is restricted
        if(isSubcategoryAssigned[_subcategory] == true){
            // check if address has perm to create market on subcategory
            if(isInchargeOfSubcategory[_address][_subcategory] == true){
                return true;
            }else{
                revert("SR");
            }
    ...
    function _createEvent(
        ...
        string memory _event2,
        address _creator
    ) private isPaused(_creator)   {
        context_address.isMarketCreationPaused();
        context_address.isPlatformActive();
        isAssignedSubcategory(_sub_category, _creator);
    ...
    function _placeBet(
        ...
        address _bettor
    ) private isPaused(_bettor) returns (bool) {
        context_address.isPlatformActive();
        context_address.isBettingPaused();
        // check bal
        require(
        ...
        require(events[event_id].isCreatorBetted == true || _bettor == events[event_id].betCreator, "ONW");
        ...
        if(_bettor == events[event_id].betCreator && events[event_id].isCreatorBetted == false){
        ...
        if (bets[event_id][_bettor] == true) {
            ...
            require(userBets[event_id][_bettor].occurence == _occurred, "CAONA");
    ...
    */
    ...
    function _distributionFormular(uint256 event_id) internal {
        ...
        setDivs.bettorsDiv = setDivs.bettorsDiv.add((div_amount * 14) / 100);  // bettors reward
        setDivs.systemDiv = setDivs.systemDiv.add((div_amount * 5) / 100);  // system reward
        ...
        hasClaimedReward(event_id)
    {
        ...
        if(bets[event_id][user_address] == true){
            ...
            if (
                ...
                user_reward = divs[event_id].bettorsDiv /
    ...
    */
    function _reclaimBettingWager(uint256 event_id, address user_address) internal {
        ...
        BUSD_token.transfer(user_address, userBets[event_id][user_address].amount);  // refund user wager
    ...
    */
    function _rewardSystem(uint256 event_id) internal {
        ...
        eventDetails.amountClaimed = eventDetails.amountClaimed.add(
                   divs[event_id].systemDiv
                ); // increment event winnings claimed
        BUSD_token.transfer(context_address.getSystemRewardAddress(), divs[event_id].systemDiv);  // transfer funds to team wallet
    }
    ...
    */
    ...
    /**
     * @dev function pauses a user ability to wager and create markets
    */
    function pauseBet(uint256 _duration) external {
        require(paused[msg.sender] == false, "AP");
        require(_duration > currentTime(), "IT3");
        pause_duration[msg.sender] = _duration;  // set pause duration
        paused[msg.sender] = true;  // mark as paused
    }
    ...
    */
    function _boostEvent(uint256 _boostAmount, uint256 event_id) private{
         uint256 boostCost = 5000000000000000000;
        // check event endtime
        require(events[event_id].endTime > currentTime(), "EAE");
        require(events[event_id].validationElapseTime > currentTime(), "EAE");
        // check bal
    ...
    /**
     * @dev function unpauses a user paused betting ability
    */
    function unpauseBet() external {
        require(paused[msg.sender] == true, "NP");
        pause_duration[msg.sender] = 0;
        paused[msg.sender] = false;
    }

    /**
     * @dev function returns [true] if user bet is paused, and [false] otherwise.
    */
    function isBetPaused(address _address) external  view returns (bool){
        return paused[_address];
    }
