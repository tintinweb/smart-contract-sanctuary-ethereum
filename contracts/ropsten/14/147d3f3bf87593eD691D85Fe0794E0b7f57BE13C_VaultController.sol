...
// File: contracts/oracle/IOracleMaster.sol
...
// File: contracts/oracle/OracleMaster.sol
...
// File: contracts/lending/VaultController.sol
...
/// @title Controller of all vaults in the USDx borrow/lend system
  ...
  OwnableUpgradeable
{
  ...
  ///@notice any function with this modifier can be paused by USDX._pauser() in the case of an emergency
  modifier onlyPauser() {
    require(_msgSender() == _usdx.pauser(), "only pauser");
    _;
  }
  ...
  /// @notice register the USDx contract
  /// @param usdx_address address to register as USDx
  function registerUSDx(address usdx_address) external override onlyOwner {
    _usdx = IUSDX(usdx_address);
  }
  ...
  function checkVault(uint96 id) public view override returns (bool) {
    ...
    uint256 usdx_liability = truncate((vault.baseLiability() * _interest.factor));
    // if the LTV >= liability, the vault is solvent
    return (total_liquidity_value >= usdx_liability);
  }

  /// @notice borrow USDx from a vault. only vault minter may borrow from their vault
  /// @param id vault to borrow from
  /// @param amount amount of USDx to borrow
  /// @dev pays interest
  function borrowUsdx(uint96 id, uint192 amount) external override paysInterest whenNotPaused {
    // grab the vault by id if part of our system. revert if not
    IVault vault = getVault(id);
    // only the minter of the vault may borrow from their vault
    require(_msgSender() == vault.minter(), "sender not minter");
    // the base amount is the amount of USDx they wish to borrow divided by the interest factor
    ...
    uint256 usdx_liability = truncate(uint256(_interest.factor) * base_liability);
    // now get the LTV of the vault, aka their borrowing power, in usdx
    uint256 total_liquidity_value = get_vault_borrowing_power(vault);
    // the LTV must be above the newly calculated usdx_liability, else revert
    require(total_liquidity_value >= usdx_liability, "vault insolvent");
    // now send usdx to the minter, equal to the amount they are owed
    _usdx.vaultControllerMint(_msgSender(), amount);
    // emit the event
    emit BorrowUSDx(id, address(vault), amount);
  }

  /// @notice repay a vault's USDx loan. anyone may repay
  /// @param id vault to repay
  /// @param amount amount of USDx to repay
  /// @dev pays interest
  function repayUSDx(uint96 id, uint192 amount) external override paysInterest whenNotPaused {
    // grab the vault by id if part of our system. revert if not
    IVault vault = getVault(id);
    // the base amount is the amount of USDx entered divided by the interest factor
    ...
    // burn the amount of USDx submitted from the senders vault
    _usdx.vaultControllerBurn(_msgSender(), amount);
    // emit the event
    emit RepayUSDx(id, address(vault), amount);
  }

  /// @notice repay all of a vaults USDx. anyone may repay a vaults liabilities
  /// @param id the vault to repay
  /// @dev pays interest
  function repayAllUSDx(uint96 id) external override paysInterest whenNotPaused {
    // grab the vault by id if part of our system. revert if not
    IVault vault = getVault(id);
    // get the total USDx liability, equal to the interest factor * vault's base liabilty
    //uint256 usdx_liability = truncate(safeu192(_interest.factor * vault.baseLiability()));
    uint256 usdx_liability = uint256(safeu192(truncate(_interest.factor * vault.baseLiability())));
    // decrease the vaults liability by the vauls base liability
    vault.modifyLiability(false, vault.baseLiability());
    // burn the amount of USDx paid back from the vault
    _usdx.vaultControllerBurn(_msgSender(), usdx_liability);

    emit RepayUSDx(id, address(vault), usdx_liability);
  }
  ...
  function liquidateVault(
    ...
    // the USDx to repurchase is equal to the bad fill price multiplied by the amount of tokens to liquidate
    uint256 usdx_to_repurchase = truncate(badFillPrice * tokens_to_liquidate);
    // get the vault that the liquidator wishes to liquidate
    IVault vault = getVault(id);

    //decrease the vault's liability
    vault.modifyLiability(false, (usdx_to_repurchase * 1e18) / _interest.factor);

    //decrease liquidator's USDx balance
    _usdx.vaultControllerBurn(_msgSender(), usdx_to_repurchase);
    ...
    // emit the event
    emit Liquidate(id, asset_address, usdx_to_repurchase, tokens_to_liquidate);
  ...
  ///@notice amount of USDx needed to reach even solvency
  ...
  /// @param id id of vault
  /// @return amount of USDx the vault owes
  ...
  /// @notice accrue interest to borrowers and distribute it to USDx holders.
  ...
  function pay_interest() private returns (uint256) {
    ...
    uint256 ui18 = uint256(_usdx.reserveRatio());
    // cast the reserve ratio now to an int in order to get a curve value
    int256 reserve_ratio = int256(ui18);

    // calculate the value at the curve. this vault controller is a USDx vault and will refernce
    ...
    // this donation is what pays out interest to USDx holders
    _usdx.vaultControllerDonate(valueAfter - valueBefore - protocolAmount);
    // send the protocol's fee to the owner of this contract.
    _usdx.vaultControllerMint(owner(), protocolAmount);
    // emit the event
