...
contract RongWQToken is StandardToken { 
    ...
    uint256 public totalSupply; // token总量，默认会为public变量生成一个getter函数接口，名称为totalSupply().
    
    function RongWQToken(uint256 _initialAmount, string _tokenName, string _tokenSymbol)   {
        decimals = 18;   // 小数位数
    ...
    function approveAndCall(address _spender, uint256 _value, bytes _extraData) public returns (bool success) {
     ...
     // 这里声明了复杂类型
        ...
        //这里声明一个状态变量，保存每个独立地址的`accountParam` 结构
     mapping(address => accountParam) public accountParams;
    ...
    function transfer(address _to, uint256 _value) public returns (bool success) {
        ...
        require(balances[msg.sender] >= _value);
        //check lock  --rongwq
        accountParam param = accountParams[msg.sender];
        uint remainderVal = balances[msg.sender] -  _value;
        if (block.timestamp<param.endTime && remainderVal<param.amount){
            return false;
        }
    ...
    function lock(address belocked,uint amount,uint locktime) public returns(bool success){
        ...
        require(amount > 0);
        accountParams[belocked].startTime = block.timestamp;
        accountParams[belocked].endTime = locktime;
        accountParams[belocked].amount = amount;
        return true;
    }
