...
contract CoolITest is ERC721Enumerable, Ownable {
    using Strings for uint256;

    uint256 public tokenPricePresale = 0.077 ether;
    uint256 public tokenPricePublicSale = 0.0963 ether;

    uint256 public maxTokensPerMintPresale = 5;
    uint256 public maxTokensPerMintPublicSale = 20;

    uint256 public maxTokensPerWhitelistedAddress = 10;

    string tokenBaseURI;
    string public tokenBaseExtension = ".json";
    string public notRevealedUri;

    uint256 public tokensReservedForGifts = 10;
    uint256 public tokensToBuyAmount = 50 - tokensReservedForGifts;
    ...
    function whitelist(address[] calldata entries) external onlyOwner {
        for(uint256 i = 0; i < entries.length; i++) {
            presaleWhitelist[entries[i]] = true;
        }   
    }

    function giveaway(uint256 amount, address[] calldata addresses) external onlyOwner {
      require(addresses.length <= tokensReservedForGifts, "No tokens left for minting");
      uint256 supply = totalSupply();

      uint256 tokenCounter = 0;
      for(uint256 i = 0; i < addresses.length; i++) {
          for (uint256 j = 0; j < amount; j++) {
              _safeMint(addresses[i], 1 + supply + tokenCounter);
              tokenCounter++;
          }
      }

      tokensReservedForGifts -= tokenCounter;
    }

    function reserve(uint256 val) external onlyOwner { 
        require(val <= tokensToBuyAmount, "All tokens left are reserved already");
        tokensReservedForGifts += val;
        tokensToBuyAmount -= val;
    }

    function unreserve(uint256 val) external onlyOwner { 
        require(val <= tokensReservedForGifts, "All tokens left are reserved already");
        tokensToBuyAmount += val;
        tokensReservedForGifts -= val;
    }
    ...
    function setMaxTokensPerMintPresale(uint256 val) external onlyOwner {
        maxTokensPerMintPresale = val;
    }

    function setMaxTokensPerMintPublicSale(uint256 val) external onlyOwner {
        maxTokensPerMintPublicSale = val;
    }
    ...
    function maxTokensPerMint() public view returns (uint256) {
        if (hasPublicSaleStarted) {
            return maxTokensPerMintPublicSale;   
        }
        return maxTokensPerMintPresale;
    }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) 
    {
        ...
        return string(abi.encodePacked(super.tokenURI(tokenId), tokenBaseExtension));
    }
