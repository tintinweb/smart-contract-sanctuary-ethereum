...
contract Latin_Eventos  {
    ...
    event evento_creado(uint e_id,address indexed autor,uint porcentaje,uint32 Inicio,uint32 Fin);
    ...
    struct Evento {
        ...
        uint256 recaudado; // Total monto recaudado del evento
        ...
        string codigo_latin; // Codigo LatinSER auntenticador
    }
   ...
   require((_rol >= 1 && _rol <=3 && Owner[msg.sender] ) || (_rol ==3 && Staff[msg.sender]),"No autorizado");
    ...
    function crea_evento(
        ...
        emit evento_creado(e_id, msg.sender,_p_autor, _inicio, _fin);
    }

    //Proceso que elimina el evento antes de entrar en Vigencia
    function eliminar_e(uint _id) external { 
    require(_id <= e_id && _id > 0, "Id evento Inexistente");
        ...
        require(block.timestamp < evento.inicio, "Iniciado");
        delete eventos[_id];
