...
contract SlashCore is ISlashCore, Ownable {
    ...
    mapping(address => bool) merchantListBool;
    ...
    function handleFee(
        ...
        if (feeAmount < merchant.viewMinAmountToProcessFee()) {
            _donationFeePerMerchant[merchant_] = _donationFeePerMerchant[merchant_].add(donationFeeAmount_);
            _taxFeePerMerchant[merchant_] = _taxFeePerMerchant[merchant_].add(taxFeeAmount_);
            return;
        }
    ...
    function handlingEmergencyFees(address ownerMarketingWallet) external onlyOwner{
        for(uint16 i=0; i < merchantList.length; i++){
            IMerchantProperty merchant = IMerchantProperty(merchantList[i]);
            if(merchant.viewMarketingWallet() == ownerMarketingWallet){
                uint256 taxFeeAmount = _taxFeePerMerchant[merchantList[i]];
                uint256 donationFeeAmount = _donationFeePerMerchant[merchantList[i]];
