...
contract MusicReward {
    ...
    uint public _rewardAmount; // Reward Token amount(AsixPoint)
    ...
    function buyGold() external returns (bool) { // Buy Platinum ticket.
        ...
        require( (_totalUser * 10) / 100 < _curSlotNumber, "There is not seat.");
    ...
    function setRewardAmount(uint _amount) external { // Set RewardAmount for listening per hour.
        require(msg.sender == _ownerAddr, "Can't set Amount");
        require(_amount > 0, "Can't set that Amount");
    ...
    function claimReward(uint _hour) external checkUserValidate(msg.sender) { // claim tokens base on listening time.
        ...
        uint reward = _hour * _rewardAmount; // Calculation Reward Amount.
