...
contract TinyDucks is ERC721, NonblockingReceiver, ILayerZeroUserApplicationConfig {
    ...
    uint256 public immutable maxPerWallet = 6;
    ...
    function mint(uint8 _amountMint) external {
        console.log(nextTokenId + _amountMint, maxSupply);
        require(nextTokenId + _amountMint <= maxSupply, "OMNI_DRAGONS: MAX_SUPPLY_FOR_CHAIN");
        require(!paused, "OMNI_DRAGONS: IS PAUSED");
        require(_amountMint > 0 && _amountMint <= maxMintPerTx, "OMNI_DRAGONS: CAN ONLY MINT 5 PER TX");
        require(_amountMint <= maxMintPerTx && balanceOf(msg.sender) <= maxPerWallet, "OMNI_DRAGONS: Only max 6 for address");
        unchecked {
