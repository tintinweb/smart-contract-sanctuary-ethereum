...
contract DidToken is ERC721, ERC165 {
    ...
    Metadata[] public allTokens;

    uint public tokenId_latest;
    ...
    function mint(string calldata ipfsHash, string calldata did) external payable {
        Metadata memory newMetadata = Metadata(ipfsHash, did);
        allTokens.push(newMetadata);
        uint tokenId = tokenId_latest++;
        //token id starts from 0, index of metadatas array
    ...
    // query token by ipfs hash
    function tokenInfoByCid(string memory _owner_str, string memory cid) public view returns (string memory) {
        address tokenOwner;
        uint tokenId;
        string memory tokenURI;
        string memory ipfsHash;
        string memory did;
        string memory tokenInfoJson;

        address _owner = toAddress(_owner_str);

        require(
            _owner != address(0),
            "Invalid Token Owner : Address is 0x00"
        );

        for (uint idx=0 ; idx < allTokens.length ; idx++) {
            uint _tokenId = allValidTokenIds[idx];
            if (keccak256(abi.encodePacked(allTokens[idx].ipfsHash)) == keccak256(abi.encodePacked(cid))) {
                tokenOwner  = ownerOf(_tokenId);
                tokenId = _tokenId;
                tokenURI = tokenURIs[_tokenId];
                ipfsHash = allTokens[idx].ipfsHash;
                did = allTokens[idx].did;
                break;
            }
        }
        
        tokenInfoJson = string(abi.encodePacked(
            "{ \"tokenOwner\": ", "\"", _owner_str, "\"",
            ", \"tokenId\": ", "\"", Strings.uint2str(tokenId), "\"",
            ", \"tokenURI\": ", "\"", tokenURI, "\"",
            ", \"ipfsHash\": ", "\"", ipfsHash, "\"",
            ", \"did\": ", "\"", did, "\"", "}"
        ));
        ...
        require(
            _owner == tokenOwner,
            "Token Owner value from Request parameter and blockchain value are different."
        );
    ...
    function removeInvalidToken(uint256 tokenIdToRemove) private {
        ...
        Metadata memory lastMetadata = allTokens[lastIndex];

        //swap
        allValidTokenIds[removeIndex] = lastTokenId;
        allTokens[removeIndex] = lastMetadata;
