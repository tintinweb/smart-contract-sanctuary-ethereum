...
contract DBSMarketplace is ERC721Holder {
    using Counters for Counters.Counter;
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    
    DBSNFTTokens internal dbsnft;
    IERC20 internal token;

    Counters.Counter internal counter;
    uint256 internal ordercount;
    mapping(uint256 => Order) public sales;
    uint256[] keys;
    Counters.Counter version;

    event orderCreated(uint256 _orderId, uint256 _tokenId, uint256 _tableId, address indexed _orderCreator, uint256 _price, address indexed _Owner);
    event NFTSold(uint256 _orderId, address indexed _newOwner, uint256 _tokenId, uint256 _tableId);
    event unshelve(uint256 _orderId, uint256 _tokenId, uint256 _tableId);

    /// @notice Constructor
    /// @param _dbsnft dbsnft token address
    constructor(address _dbsnft, IERC20 _token) {
        dbsnft = DBSNFTTokens(_dbsnft);
        token = _token;
    }
    ...
    function buy(uint256 _saleId) public payable {
        require(ordercount > 0, "order is not engough.");
        Order memory s = sales[_saleId];
        require(token.allowance(msg.sender, address(this)) >= s.price, "token is not engough.");

        // Transfer the token
        token.safeTransferFrom(msg.sender, s.owner, s.price);
        // transfer nft
        dbsnft.safeTransferFrom(address(this), msg.sender, s.tokenId);

        emit NFTSold(s.orderId, msg.sender, s.tokenId, s.tableId);

        ordercount = ordercount.sub(1);
        keys[sales[_saleId].index] = keys[ordercount];
        sales[keys[ordercount]].index = sales[_saleId].index;
        // Delete sale
    ...
    function forSale(uint256 _tokenId, uint256 _price) public{
        require(_price > 0 && _tokenId > 0, "_price or _tokenId failed");
        require(dbsnft.isApprovedForAll(msg.sender, address(this)), "permission failed");
        uint256 _tableId = dbsnft.getPropertyID(_tokenId);
        require(_tableId > 0 , "_tokenId failed");
        dbsnft.safeTransferFrom(msg.sender, address(this), _tokenId);
        uint256 _orderId = counter.current();
        keys.push(_orderId);
        Order memory s = Order({
            orderId: _orderId,
            tokenId: _tokenId,
            tableId: _tableId,
            price: _price,
        ...
        version.increment();
        counter.increment();
        emit orderCreated(_orderId, _tokenId, _tableId, msg.sender, _price, msg.sender);
    }

    /// @notice Remove listing
    /// @param _orderId order Id
    function delist(uint256 _orderId) public{
        require(sales[_orderId].owner == msg.sender, "invalid order");

        dbsnft.safeTransferFrom(address(this), msg.sender, sales[_orderId].tokenId);

        ordercount = ordercount.sub(1);
        keys[sales[_orderId].index] = keys[ordercount];
        sales[keys[ordercount]].index = sales[_orderId].index;
        keys.pop();
        emit unshelve(_orderId, sales[_orderId].tokenId, sales[_orderId].tableId);
        delete sales[_orderId];

        version.increment();
    }

    /// @notice order list
    function orderlist() public view returns(Order[] memory){
