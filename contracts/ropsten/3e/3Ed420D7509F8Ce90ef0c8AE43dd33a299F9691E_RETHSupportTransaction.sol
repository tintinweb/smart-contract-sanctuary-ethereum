...
contract RETHSupportTransaction is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        tokenAddress=RealmsOfEthernity(0x53F27Af8F3e9bF25FEd74e8869a39C6F11436649);
        swapOn = true;
    }
    ...
    function swappIN (uint256 amount, bytes calldata signature, bytes32 txhash) public whenSwapIsOn nonReentrant returns (bool) {
        ...
        require(_signerAddress == keccak256(
            abi.encodePacked(
                ...
                keccak256(abi.encodePacked(userMinted[msg.sender] + (amount)))     
            )
    ...
    function testSignerRecovery3(uint256 amount,bytes calldata signature ) public view returns (address) {
     return keccak256(
            abi.encodePacked(
               "\x19Ethereum Signed Message:\n32",
                keccak256(abi.encodePacked(userMinted[msg.sender] + (amount)))    
