...
contract SmartBeast is ERC721Enumerable, Pausable, PaymentSplitter {
    using Counters for Counters.Counter;

    uint256 public maxTotalSupply = 5757;
    uint256 public maxGiftSupply = 75;
    ...
    address[] private _team = [
        0x32Aa5E20586ad84FE741Bc1Fbe3E7e13eaDA2091,   //dev
        0x255c94b6D060a001FED4a2F9bE3C30A6c9DA2232,   //advisor
        0xA56058e9eE1217f1ea623A89Dc32367bABcB6B53    //founder
    ];
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "Smart Beast: Can't add a zero address"
            );
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "Smart Beast: Can't remove a zero address"
            );
    ...
    function setUpPresale() external onlyOwner {
        require(
            ...
            "Smart Beast: Unauthorized Transaction"
        );
        ...
        whenNotPaused
    {
        require(
            ...
            "Smart Beast: max total supply exceeded"
        );

        require(
            giftCount + _addresses.length <= maxGiftSupply,
            "Smart Beast: max gift supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "Smart Beast: recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function presaleMint(uint256 _amount) internal {
        ...
        require(
            ...
            "Smart Beast: Can only mint 2 tokens per adress"
        );
        require(
            totalNFT + _amount <= maxTotalSupply,
            "Smart Beast: max supply exceeded"
        );
        uint256 _price = getPrice();
        require(
            _price * _amount <= msg.value,
            "Smart Beast: Ether value sent is not correct"
        );
        uint256 _newItemId;
        for (uint256 ind = 0; ind < _amount; ind++) {
            _tokenIds.increment();
    ...
    function saleMint(uint256 _amount) internal {
        require(_amount > 0, "Smart Beast: zero amount");
        require(workflow == WorkflowStatus.Sale, "Smart Beast: sale is not active");
        uint256 _maxCount = getMaxCount();
        require(
            _saleClaimed[msg.sender] + _amount <=  _maxCount,
            "Smart Beast:  Can only mint 5 tokens per adress"
        );
        require(
            totalNFT + _amount <= maxTotalSupply,
            "Smart Beast: max supply exceeded"
    ...
    function mainMint(uint256 _amount) external payable whenNotPaused {
        require(
            ...
            "Smart Beast: sold out"
        );
        require(
            workflow != WorkflowStatus.CheckOnPresale,
            "Smart Beast: presale not started"
        );
        if (
            workflow == WorkflowStatus.Presale
        ) {
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Smart Beast: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Smart Beast: burn caller is not owner nor approved"
        );
        _burn(tokenId);
        totalNFT = totalNFT - 1;
    }
