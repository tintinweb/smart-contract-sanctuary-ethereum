...
contract CoreResolver is KVStorage {
    ...
    function getReverse(address main_address) public view returns (bytes32, string memory) {
        bytes32 node = coreDB.reverseRecord(main_address);
    ...
    function getContribution(address main_address_or_owner) external view returns (uint256) {
        return abiBytesToUint256(coreDB.getOwnerItem(main_address_or_owner, encodeItemKey(bytes32(0), KEY_CONTRIBUTION)));
    }
...
contract CoreRegistrar is KVStorage {
    ...
    function setWeb2(bytes32 node, uint256 web2_type, bytes memory web2_info) external onlyTopLevelRegistrar {
        ...
        address owner = getNodeOwner(node);
        coreDB.setOwnerItem(owner, encodeItemKey(node, web2_type), web2_info);
        emit NodeOwnerItemChangedWithValue(node, owner, bytes32(web2_type), web2_info);
    }

    function setWeb3(address main_address_or_owner, uint256 web3_type, bytes memory we3_info) external onlyTopLevelRegistrar {
        require(web3_type >= KEY_CONTRIBUTION && web3_type <= KEY_ENS_INFO, "this web3 type is not supported");
        coreDB.setOwnerItem(main_address_or_owner, encodeItemKey(bytes32(0), web3_type), we3_info);
    ...
    function setReverse(bytes32 node) external {
        ...
        emit ReverseRecordSet(msg.sender, node);
    }

    function deleteReverse() external {
        bytes32 node = coreDB.reverseRecord(msg.sender);
        if (node != bytes32(0)) {
            coreDB.deleteReverse(msg.sender);
            emit ReverseRecordSet(msg.sender, bytes32(0));
        }
    }

    function setReverseByNFT(bytes32 node, address owner) external onlyCoreNFT {
        if (coreDB.reverseRecord(owner) == bytes32(0) && node != bytes32(0)) {
            coreDB.setReverse(owner, node);
            emit ReverseRecordSet(owner, node);
        }
    }

    function deleteReverseByNFT(bytes32 node) external onlyCoreNFT {
        address main_address = getNodeOwner(node);
        if (coreDB.reverseRecord(main_address) == node && node != bytes32(0)) {
            coreDB.deleteReverse(main_address);
...
contract CoreDB is ICoreDB, APP {
    ...
    function setNodeOwnerItem(bytes32 node, bytes32 item_key, bytes memory item_value) external onlyExisted(node) onlyOperator {
        ownerDB[nodeRecord[node].owner][item_key] = item_value;
    }
    ...
    function deleteNodeOwnerItem(bytes32 node, bytes32 item_key) external onlyExisted(node) onlyOperator {
        delete ownerDB[nodeRecord[node].owner][item_key];
    }
    ...
    function clearNode(bytes32 node) external onlyCoreNFT {
        ...
        balanceOf[nodeRecord[node].owner] -= 1;
        totalSupply -= 1;
        delete nodeRecord[node];
    }
...
interface IReverse {
    function setReverseByNFT(bytes32 node, address owner) external;
    function deleteReverseByNFT(bytes32 node) external;
}
...
contract CoreNFT is IERC721Metadata, APP {
    ...
    function _transfer(address from, address to, uint256 tokenId) internal {
        ...
        IReverse(core_registrar).deleteReverseByNFT(node);
        coreDB.transferNodeOwner(node, to);
        IReverse(core_registrar).setReverseByNFT(node, to);
    ...
    function afterMint(address to, uint256 tokenId, bytes memory _data) external onlyCoreRegistrar {
        _approve(address(0), tokenId); // Clear approvals from the previous owner
        IReverse(_msgSender()).setReverseByNFT(bytes32(tokenId), to);
