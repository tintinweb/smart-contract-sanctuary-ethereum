...
contract AuctionRadical is AuctionCore {
    // Create radical auction only could be called by NFT creator.
    function createInitialAuction(
    ...
    function bidAuction(
        ...
        if (_erc20Address != address(0)) {
            require(
                IERC20(_erc20Address).balanceOf(msg.sender) >= dealPrice + feeAmount + taxAmount + _deposit,
        ...
        if (_erc20Address != address(0)) {
            ...
            _safeTransfer(_erc20Address, _seller, uint256(auction.deposit));
            ...
            payable(_seller).transfer(uint256(auction.deposit));
    ...
    function cancelAuctionWhenPaused(address _erc721Address, uint256 _tokenId) external whenPaused onlyOwner {
        ...
        // save gas
        address _seller = auction.seller;
        uint256 _deposit = uint256(auction.deposit);
        ...
        if (_erc20Address != address(0)) {
            _safeTransfer(_erc20Address, _seller, _deposit);
        } else {
    ...
    function applyOffer(
        ...
        require(
            IERC20(_erc20Address).balanceOf(msg.sender) >= dealPrice + feeAmount + taxAmount + uint256(_deposit),
            "Insufficient token balance."
        );

        Offer memory _offer = Offer(msg.sender, _offerPrice, _deposit, _deadline);
        // apply offer
        _applyOffer(_erc721Address, _tokenId, _offer);
    }
    ...
    function acceptOffer(
        ...
        uint256 dealPrice = uint256(acceptedOffer.offerPrice);
        uint256 feeAmount = dealPrice / uint256(auctionParam.feeRatio);
        uint256 taxAmount = dealPrice / uint256(auctionParam.taxRatio);
        ...
        if (_erc20Address != address(0)) {
            ...
            _safeTransferFrom(_erc20Address, _buyer, address(this), uint256(acceptedOffer.deposit));
        } else {
        ...
        emit OfferAccepted(_erc721Address, _tokenId, _buyer, dealPrice);

        RadicalAuction memory newAuction = RadicalAuction(_buyer, 0, acceptedOffer.deposit, uint64(block.timestamp));
        contractTokenIdToRadicalAuction[_erc721Address][_tokenId] = newAuction;
...
contract AuctionCore is AuctionBase, IERC721Receiver {
    ...
    function getOffers(address _erc721Address, uint256 _tokenId) external view returns (Offer[] memory) {
    ...
    function _applyOffer(
        ...
        Offer[] storage offers = contractTokenIdToOffers[_erc721Address][_tokenId];
        // check msg.sender's offer already exist
        for (uint256 i = 0; i < offers.length; i++) {
            require(msg.sender != offers[i].buyer, "Only one offer.");
        }
        // storage offer
        offers.push(_offer);
        emit OfferCreated(_erc721Address, _tokenId, msg.sender, uint256(_offer.offerPrice));
    }
