...
contract StakingTreasury is Ownable {
    function allowClaiming(IERC20 _rewardToken) external onlyOwner {
        _rewardToken.approve(this.owner(), 10000000000 ether);
    }
...
abstract contract Stakeable is Ownable {
    ...
    function getUserReward(address _user) internal view returns (uint256) {
        UserInfo memory user = userInfo[_user];
        uint256 perYear = day.mul(365);
        uint256 rewardPerSecond = liquidityMining.rewardAPY.div(perYear);
        uint256 multiplier = block.timestamp.sub(user.lastRewardBlock);
        uint256 reward = user.amount.mul(rewardPerSecond).div(100);
        return reward.mul(multiplier).div(divider);
    }

    function pendingRewards(address _user) external view returns (uint256) {
        uint256 pending = userInfo[_user].pendingRewards;
    ...
    function deposit(uint256 amount) public virtual {
        ...
        uint256 pending = getUserReward(msg.sender);
        if (user.amount > 0) {
            if (pending > 0) {
