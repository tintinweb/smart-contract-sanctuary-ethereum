...
contract Chessgame is IERC20 {
    ...
    string constant private _name = "Chessgame";
    string constant private _symbol = "CHESS";
    ...
    address payable private rewardsPool = payable(0x66076AA28Ffd1907734059E5702f4d66c3Da8f35);
    address payable private treasuryWallet = payable(0x32239690806a2caf30CeaF30a7d9dB641c72cA17);
    ...
    IERC20 pawn = IERC20(0x66076AA28Ffd1907734059E5702f4d66c3Da8f35);
...
//===============================================================================================================
    ...
    function updatePawnAddress(IERC20 _pawn) external onlyOwner {
        require(_pawn != IERC20(address(this)), "Pwan address is not this address");
        pawn = IERC20(_pawn);
    ...
    function getWhaleFee(address from) public view returns (uint256) {
        if(pawn.balanceOf(from) >= 1 &&
            pawn.balanceOf(from) < 20 ){return vBuy1;}
        if(pawn.balanceOf(from) >= 20 &&
            pawn.balanceOf(from) < 50 ){return vBuy2;}
        if(pawn.balanceOf(from) >= 50 &&
            pawn.balanceOf(from) < 100 ){return vBuy3;}
        if(pawn.balanceOf(from) >= 100) {return vBuy4;}
        else{
            return 0;
        }
