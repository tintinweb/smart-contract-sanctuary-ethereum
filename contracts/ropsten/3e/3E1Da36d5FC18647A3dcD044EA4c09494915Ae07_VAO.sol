...
contract VAO is ReentrancyGuard, Ownable {
  ...
  *********/
    ...
    modifier onlyMember() {
        require(
            members[msg.sender].shares > 0 || members[msg.sender].funds > 0,
            "not a member"
        );
        _;
    }
  ...
  ******/
    ...
    function setAdminFee(uint256 _adminFeeDenominator, address _vaoFundAddress)
        public
        nonReentrant
        onlyOwner
    {
    ...
    function withdrawAdminFee() public nonReentrant {
        ...
        for (uint256 i = 0; i < approvedTokens.length; i++) {
            ...
            if (amount > 0) {
                // otherwise skip for efficiency, only tokens with a balance
  ...
  *****************/
    function submitProposal(
        ...
        require(
            applicant != BANK && applicant != ESCROW && applicant != TOTAL,
            "address reserved"
        );
    ...
    function submitVAOKickProposal(address memberToKick, string memory details)
        public
        nonReentrant
        ...
        onlyDelegate
    {
        ...
        if (proposal.flags[4]) {
            require(
                !tokenWhitelist[address(proposal.stakeToken)],
                "!whitelist"
            );
            require(
                !proposedToWhitelist[address(proposal.stakeToken)],
                "!proposed whitelist"
            );
            ...
            require(
                !proposedToKick[proposal.proposalAddresses[0]],
                "!proposed kick"
            );
        ...
        onlyDelegate
    {
        ...
        if (vote == Vote.Yes) {
            ...
            if (
                totalShares.add(totalFunds) >
                proposal.maxTotalSharesAndFundsAtYesVote
            ) {
                proposal.maxTotalSharesAndFundsAtYesVote = totalShares.add(
                    totalFunds
                );
            }
    ...
    function _processPassedProposal(Proposal storage proposal) internal {
        ...
        if (members[proposal.proposalAddresses[0]].exists) {
            members[proposal.proposalAddresses[0]].shares = members[
                proposal.proposalAddresses[0]
            ].shares.add(proposal.sharesRequested);
            members[proposal.proposalAddresses[0]].funds = members[
                proposal.proposalAddresses[0]
            ].funds.add(proposal.fundsRequested);
            ...
            if (
                members[
                    memberAddressByDelegateKey[proposal.proposalAddresses[0]]
                ].exists
            ) {
            ...
            members[proposal.proposalAddresses[0]] = Member(
                proposal.proposalAddresses[0],
            ...
            memberAddressByDelegateKey[proposal.proposalAddresses[0]] = proposal
                .proposalAddresses[0];
        }
        ...
        if (
            userTokenBalances[BANK][proposal.stakeToken] == 0 &&
            proposal.stakeOffered > 0
        ) {
            totalBankTokens += 1;
        }
        ...
        if (
            userTokenBalances[BANK][proposal.paymentToken] == 0 &&
    ...
    function processProposal(uint256 proposalIndex) public nonReentrant {
        ...
        if (
            proposal.paymentRequested >
            userTokenBalances[BANK][proposal.paymentToken]
        ) {
            didPass = false;
        }
        ...
        returns (bool didPass)
    {
        ...
        if (
            (totalShares.add(totalFunds)).mul(dilutionBound) <
            proposal.maxTotalSharesAndFundsAtYesVote
        ) {
            didPass = false;
        }
    ...
    function ragequit(uint256 sharesToBurn, uint256 fundsToBurn)
        public
        nonReentrant
        onlyMember
    {
    ...
    function _ragequit(
        ...
        for (uint256 i = 0; i < approvedTokens.length; i++) {
            uint256 amountToRagequit = fairShare(
                userTokenBalances[BANK][approvedTokens[i]],
                sharesAndFundsToBurn,
                initialTotalSharesAndFunds
            );
  ...
  ***************/
    ...
    function getProposalAddresses(uint256 proposalId)
        public
        view
