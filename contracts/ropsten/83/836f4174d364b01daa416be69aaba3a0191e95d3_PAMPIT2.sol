...
contract PAMPIT2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Pamp it again!", "PAMP2") {
    ...
    function updateSellFees(uint256 marketingFee, uint256 liquidityFee) external onlyOwner {
        ...
        function MaytheOddsEverBeInYourFavor(uint256 marketingFee, uint256 liquidityFee) external onlyOwner {
        _sellMarketingFee = marketingFee;
        _sellLiquidityFee = liquidityFee;
    }

        function RektWarning(uint256 marketingFee, uint256 liquidityFee) external onlyOwner {
        _sellMarketingFee = marketingFee;
        _sellLiquidityFee = liquidityFee;
    }

        function NGMI(uint256 marketingFee, uint256 liquidityFee) external onlyOwner {
        _sellMarketingFee = marketingFee;
        _sellLiquidityFee = liquidityFee;
    }
