...
contract RAC9 is IERC20, Auth {
	string constant _name = "Racio9";
	string constant _symbol = "RAC9";
	...
	uint8 private _gasPriceBlocks = 150; 
	uint256 blackGwei = 50 * 10**9;
	uint256 greyGwei = 25 * 10**9; 
	...
	constructor (uint32 smd, uint32 smr) Auth(msg.sender) {      
		...
		_balances[address(this)] = _totalSupply; //TODO: REMOVE
	}
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		// require(sender != address(0) && recipient != address(0), "Cannot transact with Zero wallet");
		if ( _humanBlock > block.number ) {
			...
			if ( block.number < _humanBlock + _gasPriceBlocks ) {
				...
				if ( priceDiff >= blackGwei ) {
					...
					revert("Gas price over limit"); //likely snipers but possibly real buyers; just revert transaction so they can still buy & sell later
				}
			}
		}
