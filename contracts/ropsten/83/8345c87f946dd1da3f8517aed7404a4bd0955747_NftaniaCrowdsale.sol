...
contract NftaniaCrowdsale is Pausable, Ownable, ReentrancyGuard {
   ...
   //////////////////////// Main Variables /////////////////////////////////// 
        ...
        uint256 private totalLiquidityPair;
        ...
        address payable public poolWallet =  payable(address(0xc5674346FcC6Deb279e63238fa7da45264ae64eA));
   ...
   //////////////////////// Events /////////////////////////////////////////// 
    ...
    // Liquidity Added
        ...
        event LiquidityAdded (uint amountToken, uint amountETH, uint amountliquidityPair, uint totalLiquidityPair, address pairAddress);
   ...
   //////////////////////// Fallback //////////////////////////////////////   
    fallback() external payable {freeBalance = freeBalance + msg.value;} 
    receive()  external payable {freeBalance = freeBalance + msg.value;} 
    
   ////////////////////////  get remaining tokens //////////////////////////////// 
    function getRemainingTokens() public view returns (uint256 _remainingTokens){
        _remainingTokens = remainingTokens;
        // require (_remainingTokens >= uint (1e0 ether),"remaining tokens are fractions");
   ...
   //////////////////////// Function update Remaining Tokens /////////////  
    function updateStatus() internal {
        ...
        capReached = remainingTokens < minPurchase * 10**18;  // Checks if there is remaining tokens supply
        require(block.timestamp >= startDate,"Nftania Crowdsale: Sale has not started yet");
        require(!capReached,"Nftania Crowdsale: All tokens are sold out");  
   ...
   //////////////////////// Update Dates //////////////////////////////////////// 
    ...
    function setTest2 (bool _G, bool _H, bool _I,bool _J, bool _K, bool _L, bool _M, bool _N) public {
        G=_G;
        H=_H;
        I=_I;
        J=_J;
   ...
   //////////////////////// Purchase Tokens  /////////////  
    ...
    uint256 _walletPurchases, uint256 walletTokenBalance, uint256 _purchaseOrderID, uint256 date){  
        ...
        walletTokenBalance = IERC20(tokenAddress).balanceOf(msg.sender); //get wallet total token balance after pruchase
   ...
   //////////////////////// function Forward Funds /////////////  
    function forwardFunds(uint256 _revenueShare, uint256 _poolShare) internal {  
        // (bool success, ) = revenueWallet.call{value: _revenueShare}("");
        // require(success, "Error");
            require(A,"A");
        revenueWallet.transfer(_revenueShare);
        payable(address(this)).transfer(_poolShare);
   ...
   ////////////////// calculate shares for revenue and liquidity pool /////////////
    function getShares(uint256 paymentAmount) internal view returns (uint256 poolShare, uint256 revenueShare ) {
        ...
        // require(poolShare > 1000,"small transaction amount");
   ...
   ////////////////////Automatic Add Liquidity /////////////  
    ...
    function getPoolInfo() public view returns (uint _poolRaised,uint _targetPool,uint _poolBalance,address _LiquidityPoolLocker, uint _totalLiquidityPair) {
        return (poolRaised,targetPool,poolBalance,LiquidityPoolLocker,totalLiquidityPair);
    }
   ...
   //////////////////// Add Liquidity /////////////  
    function callNftaniaAddLiquidity (uint tokenAmount, uint EthAmount) internal
    returns (uint amountToken, uint amountETH,uint amountliquidityPair, uint _totalLiquidityPair, address pairAddress){
        IERC20(tokenAddress).safeTransferFrom(owner(), address(this), tokenAmount); 
        IERC20(tokenAddress).safeApprove(NftaniaAddLiquidity, tokenAmount);  
            require(M,"M");  

        (amountToken, amountETH, amountliquidityPair, totalLiquidityPair, pairAddress) = 
        ...
        emit LiquidityAdded (amountToken, amountETH, amountliquidityPair, totalLiquidityPair, pairAddress);
        return (amountToken, amountETH, amountliquidityPair, totalLiquidityPair, pairAddress);
    }   
