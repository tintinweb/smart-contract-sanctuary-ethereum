...
contract MyToken is Stakeable, ERC20 {
    ...
    function hasStake(address _staker) public view returns(StakingSummary memory){
        require(_isStaker() == true, "Not a staker!");
    ...
    function stake(uint256 _amount) public 
    {
        ...
        require(!_isStaker(), "Cannot stake while having open stake!");
    ...
    * @notice withdrawStake is used to withdraw stakes from the account holder
    ...
    {
        IERC20 LPOXLP = IERC20(LPOXLP_ADDRESS);
        //uint256 original_LP_staked = _getOriginalLPStaked(stake_index);
        uint256 amount_to_mint = _withdrawStake(amount);

        require(amount <= LPOXLP.balanceOf(address(this)), "LPOX LP must be higher, this shouldn't happen ever!");

        // Return staked tokens 
        LPOXLP.approve(address(this), amount);
        LPOXLP.transferFrom(address(this), msg.sender, amount);
