...
// File contracts/ApolloSafePlayMultiCurrency/ApolloSafePlayRLM.sol
...
/**
 * Apollo safe play token with all functionalities except burning
...
contract ApolloSafePlayRLM is Context, IERC20Metadata, Ownable {
    // Token parameters
    string private NAME;
    ...
    // Reflection will happen in every transactions while other 3 will happen after thresold
    ...
    function _transfer(
        ...
        // if pendingToPay.liquidity is more than the limit(i.e. swapTokensAtAmount) then only we will swap
    ...
    function _tokenTransfer(
        ...
        emit Transfer(
            sender,
            address(this),
            values.tLiquidity + values.tMarketing + values.tBuyBack
        );
    }
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        uint256 totalFeePercent = buyFeeRates.liquidity +
            buyFeeRates.marketing +
            buyFeeRates.buyBack;
        pendingToPay.liquidity +=
            (reflection * buyFeeRates.liquidity) /
            totalFeePercent;
        pendingToPay.marketing +=
