...
error DayAlreadyClaimed();

contract Miscellaneous is ERC721, Ownable, Ticketed {
    ...
    uint public startTime = 1735711200;

    mapping(address => uint) public freeMints;

    mapping(uint256 => uint) public lockedMetadata;

    mapping(uint256 => bool) public unclaimedDays;
    uint256[] public unclaimedDayList;
    ...
    // dutch auction tokenIds will be 1-50
    uint256 public constant DUTCH_AUCTION_SUPPLY = 50;
    ...
    function getPrice() public view returns (uint) {
        if (block.timestamp < startTime) return 100;
        ...
        payable
    {
        ...
        if (_nextTokenId > DUTCH_AUCTION_SUPPLY) revert SoldOut();
        ...
        if (price == 0) {
           ...
           freeMints[msg.sender] = currentAccountMints + 1; 
        }
    ...
    function mintOneAllowlist(bytes calldata _signature, uint256 spotId)
        external
        payable
    {
        uint256 _nextTokenId = nextTokenId;
    ...
    function fcfsClaim(uint256 tokenId, uint256 day) external {
        if (currentDay != 101) revert TooEarlyToClaim();
        if (ownerOf(tokenId) != msg.sender) revert NotOwner();
        if (lockedMetadata[tokenId] != 0) revert MetadataAlreadyLocked();
        if (unclaimedDays[day] != true) revert DayAlreadyClaimed();

        lockedMetadata[tokenId] = day;
    }

    function forceClaim(uint256 tokenId, uint256 day) external onlyOwner {
        if (currentDay != 101) revert TooEarlyToClaim();
        if (lockedMetadata[tokenId] != 0) revert MetadataAlreadyLocked();
        if (unclaimedDays[day] != true) revert DayAlreadyClaimed();
    ...
    function progressDay() external onlyOwner {
        ...
        if (claims.length != 0) {
            ...
            unclaimedDays[currentDay] = true;
            unclaimedDayList.push(currentDay);
        }
