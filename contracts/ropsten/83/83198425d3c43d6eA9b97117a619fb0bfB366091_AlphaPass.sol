...
contract AlphaPass is ERC721, Ownable, Pausable, RoyaltiesV2Impl {
    ...
    Counters.Counter private _wlTokenIdCounter;
    ...
    constructor(IParams memory params) ERC721(params.name, params.symbol) {
        setBaseURI(params.initBaseURI);
        setNotRevealedURI(params.initNotRevealedUri);

        require(params.saleEndTime > params.saleStartTime);
        ...
        whenSaleOpen
    {
        ...
        if (isWhitelisted(msg.sender)) {
            uint256 supply = _wlTokenIdCounter.current();
            require(supply + _mintAmount <= maxWlSupply);

            for (uint256 i = 1; i <= _mintAmount; i++) {
                uint256 tokenId = _wlTokenIdCounter.current();
                _wlTokenIdCounter.increment();
        ...
        onlyOwner
    {
        ...
        totalSupply += _mintAmount;
        emit MintPass(_address, _mintAmount);
    }
