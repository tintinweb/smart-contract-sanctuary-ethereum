...
contract StakingPool is OwnableUpgradeable, ReentrancyGuard 
{
    ...
    event progressOfStaking(uint256 poolId,address buyer,
        uint256 amount, uint period, uint256 totalamount);
    event registerpool(uint256 poolId, uint256 stakper, uint256 amount, uint256 stak, uint256 base, uint256 claimable1);
   ...
   // uint256 public claimable1;
    ...
    mapping(address => uint256) public claimable;
     mapping(uint256 =>uint256) public claimable1;
    ...
    function claimAmount(uint256 poolId) external {
        ...
        require(poolInfo.claimable1 > 0 ,"nothing to claim");

        uint256 amount = poolInfo.claimable1;

        poolInfo.claimable1 = 0;

        IERC20(ideaToken).safeTransfer(msg.sender, amount);
    }

    function register( uint256 _stakingPeriod, uint256 _weiAmount,uint256 _staking, uint256 _base, uint256 _claimable1) public{

        require(_weiAmount > 10000, "weiamount should be grater then 10000 can't be zero!");
       ...
       // poolsCount = poolsCount + 1;
        ...
        emit registerpool(createdPoolId,stakingPeriod,weiAmount,staking,base,_claimable1);
    ...
    function applyForStaking(uint256 poolId ) external {
        ...
        emit progressOfStaking(createdPoolId,msg.sender, poolInfo.weiAmount, poolInfo.stakingPeriod, poolInfo.claimable1);
    ...
    function stakingClaim(uint256 poolId ) public {
       ...
       //  _released[address(token)] = _released[address(token)].add(stakingInstallment);
        ...
        poolInfo.claimable1-=totalAmount;
