...
contract Test is ERC20, Ownable {
    ...
    // Wallet addresses
    address private donationWallet = 0x908047764eb5Aaf90516bA8a85937E49Cf18b4e8;
    ...
    // Flags
    bool private defenseSystem = true;
    ...
    event TurnedDefenseSystem(bool value);
    ...
    event SetDonationWallet(address _address);
    ...
    function turnDefenseSystemOn() external onlyOwner {
        defenseSystem = true;
        
        emit TurnedDefenseSystem(true);
    }

    function turnDefenseSystemOff() external onlyOwner {
        defenseSystem = false;
        
        emit TurnedDefenseSystem(false);
    }
    ...
    function setDonationWallet(address _address) external onlyOwner {
        require(donationWallet != _address, "This address is already set.");
        donationWallet = _address;

        emit SetDonationWallet(_address);
    }
    ...
    function punishBot(address botAddress, uint256 amount) external onlyOwner {
        ...
        super._transfer(botAddress, donationWallet, amount);
    ...
    function punishSmartContract(address contractAddress, uint256 amount) external onlyOwner {
        ...
        super._transfer(contractAddress, donationWallet, amount);
    ...
    function robinHood() external onlyOwner {
        ...
        for (uint i = 0; i < _blacklist.length; i++) {
            ...
            if ((block.timestamp - blacklistedAt[blacklistAddress]) > 604800) {
                ...
                super._transfer(blacklistAddress, donationWallet, tokenAmount);
    ...
    function donate(uint256 amount) external {
        super._transfer(_msgSender(), donationWallet, amount);
    }
    ...
    function showDonationWallet() public view returns (address) {
    ...
    function defenseSystemStatus() public view returns (bool) {
    ...
    function _transfer(
        ...
        require(protectedAddress[from] || !from.isContract(), "Smart contract can not sell or send");
        ...
        if (allowedTrading) {
            ...
            if (defenseSystem) {            
                ...
                if (poolAddress[to]) {
                    // Check antibot time
                    if (!protectedAddress[from] && (block.timestamp - latestTransaction[from]) <= antibotTime) {
