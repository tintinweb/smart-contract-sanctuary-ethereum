...
contract Market is ReentrancyGuard, IERC721Receiver, Ownable {
    /*
		Enums
	*/
    enum ListingStatus {
	...
	*/
    ...
    event NewOffer(address offerer, uint256 price);

    event OfferAccepted(address offerer, uint256 price);

    event OfferCancelled(address offerer, uint256 price);
    ...
    /*
		State Variables
	*/
    // Listing fee is BasisPoints (10,000 = 100%)
	...
	*/
	...
	modifier isOfferPending(uint256 listingId, address offerer) {
		require(
            _offers[listingId][offerer].status == OfferStatus.Pending,
            "!Pending"
        );
        _;
	}
	...
	*/
	...
	nonReentrant 
	onlyTokenOwner(token, tokenId) 
	{
        _transferFrom(msg.sender, address(this), token, tokenId);
	...
	isActive(listingId) 
	{
        ...
        uint256 finalSettlementAmount = listing.price;

        // 1. Listing fee

        uint256 feePerListing = calculateListingFee(listing.price);
        unchecked {
        ...
        (address receiver , uint256 royaltyFeePerListing) = IRoyaltyFeeRegistry(royaltyFeeManager).royaltyInfo(listing.token, listing.price);

        if(receiver == address(0) || royaltyFeePerListing == 0) {
            (receiver, royaltyFeePerListing) = IRoyaltyFeeManager(royaltyFeeManager).calculateRoyaltyFeeAndGetRecipient(listing.token, listing.tokenId, listing.price);
        }
        ...
        _transfer(listing.seller, finalSettlementAmount);
	...
	isActive(listingId)
	{
        ...
        require(_offers[listingId][msg.sender].price == 0, "Offer already exists");
        require(price > 0, "Offer price cannot be zero");

        Offer memory offer = Offer(msg.sender, price, OfferStatus.Pending);

        _offers[listingId][msg.sender] = offer;

        // _transfer(address(this), price);

        emit NewOffer(msg.sender, price);
    }
	...
	onlySeller(listingId)
	isActive(listingId)
	isOfferPending(listingId, offerer)
	{
        Listing storage listing = _listings[listingId];
        Offer storage offer = _offers[listingId][offerer];
        ...
        uint256 finalSettlementAmount = offer.price;

        // 1. Listing fee

        uint256 feePerListing = calculateListingFee(offer.price);
        unchecked {
            finalSettlementAmount -= feePerListing;
        }
        ...
        emit OfferAccepted(offer.offerer, offer.price);
    }

    function cancelOffer(uint256 listingId) external 
	nonReentrant 
	isOfferPending(listingId, msg.sender)
	{
        Offer storage offer = _offers[listingId][msg.sender];
		
        require(msg.sender == offer.offerer, "Only offerer can cancel offer");

        offer.status = OfferStatus.Cancelled;

        _transfer(msg.sender, offer.price);

        emit OfferCancelled(offer.offerer, offer.price);
    }
	...
	*/
	...
	// function getOffers(uint256 listingId)
    //     public
    //     view
    //     returns (Offer[] memory offers)
    // {
    //     offers = _offers[listingId];
    // }
    ...
    function updateListingFee(uint _listingFeePercent) public onlyOwner{
        listingFee = _listingFeePercent;
        emit UpdatedListingFee(_listingFeePercent);
    }
