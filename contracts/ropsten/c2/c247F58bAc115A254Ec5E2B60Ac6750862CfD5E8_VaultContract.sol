...
contract VaultContract is IERC721Receiver {
    ...
    ILendingPool aaveV2PoolContract =
    ...
    function openPosition(
        ...
        address depositOnBehalfOf,
        uint16 depositReferralCode,
        address borrowAsset,
        uint256 borrowAmount,
        uint256 borrowInterestRateMode,
        uint16 borrowReferralCode,
        address borrowOnBehalfOf
    ) public {
        ...
        depositToAave(
            depositAsset,
            depositAmount,
            depositOnBehalfOf, // address(this)
            depositReferralCode // 0
        );
        // Borrow From Aave
        borrowFromAave(
            borrowAsset,
            ...
            borrowReferralCode, // 0
            borrowOnBehalfOf // address(this)
        );
    }
    ...
    function closePosition(
        ...
        console.log("Contract Balance : ", wethBalance);
        console.log("Total Debt : ", totalDebt);

        if (wethBalance > totalDebt) {
            // Repay the borrowed amount
            repayToAave(weth, totalDebt, repayRateMode, address(this));
            console.log("1. Repayed to Aave");
            // Withdraw the deposited amount
            withdrawFromAave(usdc, type(uint).max, address(this));
            console.log("1. Withdrawn from Aave");
            // Swap Back to USDc
            uint256 wethLeft = token.balanceOf(address(this));
            swapExactInputSingle(weth, usdc, swapPool_Fee, wethLeft, 0);
            console.log("1. Swapped back to USDC");
        } else {
            // Calculate the AMount Needed for Repayment
            uint256 amountNeeded = (totalDebt - wethBalance) + 1000000000000000;
            swapExactInputSingle(usdc, weth, swapPool_Fee, amountNeeded, 0);
            console.log(
    ...
    function repayToAave(
        ...
        aaveV2PoolContract.repay(asset, amount, rateMode, onBehalfOf);
    }
