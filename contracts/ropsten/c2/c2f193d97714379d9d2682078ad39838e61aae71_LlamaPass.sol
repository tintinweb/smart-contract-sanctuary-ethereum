...
contract LlamaPass is ERC1155, Ownable {
    ...
    address private passwordSigner = 0xe0CA10854e7bABC98DcbaF409189067Ff4bbd52C;

    uint private constant GOLD_PASS_ID = 1; //代币名称序号
    uint private constant SILVER_PASS_ID = 2;//代币名称序号

    string public name = "Llama Pass";//合约名称
    string public symbol = "LLP";//合约简称

    uint16 private maxGoldPasses = 500;//发行总量
    uint16 private maxSilverPasses = 3500;//发行总量

    uint16 private currentGoldPassCount = 0;//私人 当前计数
    uint16 private currentSilverPassCount = 0;//私人 当前计数

    uint private silverPassPrice = 0.75 ether;//价格
    uint private goldPassPrice = 5 ether;//价格

    bool public sale = false;//公开发售

    string private baseUri = "ipfs://QmZEMXDzfHDtEiYbHcUtyQ7gVmnvMT6VoNfDX7Ztsdc7b2/";//IPFS地址

    mapping (address => bool) public alreadyMinted;

    constructor() ERC1155("") {}
//以太币功能，应付地址，金额单位（withdrawEther和上面价格单位对应）
...
//购买功能
    function buy(uint16 tokenId) public payable {
        require(sale, "Sale not open");////////销售未开放
        //要求当前tokenID加1小于等于发行总量
        require(tokenId == GOLD_PASS_ID ? currentGoldPassCount + 1 <= maxGoldPasses : currentSilverPassCount + 1 <= maxSilverPasses, "Max supply");
        require(!alreadyMinted[msg.sender], "Double mint");//已经铸造 双薄荷
        require(tokenId == GOLD_PASS_ID || tokenId == SILVER_PASS_ID, "Bad ID");//错误的ID
        require(msg.value >= getPassPrice(tokenId), "Wrong amt");//错误的时间

        tokenId == GOLD_PASS_ID ? currentGoldPassCount++ : currentSilverPassCount++;         
        alreadyMinted[msg.sender] = true;//铸造确认

        _mint(msg.sender, tokenId, 1, "");//mint数量为1或者其他
    }
...
/////mint功能
    function mint(uint16 tokenId, bytes memory signature) public {
        require(tokenId == 1 || tokenId == 2, "Bad ID");//要求tokenID等于对应值,否则发出错误的ID
        //要求ID+1小于最大供应量
        require(tokenId == GOLD_PASS_ID ? currentGoldPassCount + 1 <= maxGoldPasses : currentSilverPassCount + 1 <= maxSilverPasses, "Max supply");
        //要求被列入白名单，否则发出错误警告
...
///////////////////////////////////////////////////////////////////////////////////////////
    ...
    function claimToAdmin(uint16 tokenId, uint16 tokenAmt) public onlyOwner {//定义管理员xxxxxxx
...
////////////////////////////
   //列入白名单功能
    function isWhitelisted(address user, uint16 tokenId, bytes memory signature) public view returns (bool) {
        ...
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);//获取 Eth 签名消息哈希

        return recoverSigner(ethSignedMessageHash, signature) == passwordSigner;//获取签名消息返回
    }
//////////////////////////////
    //获取ETH签名
    function getEthSignedMessageHash(bytes32 _messageHash) private pure returns (bytes32) {
        /*
        签名是通过使用以下格式对 keccak256 哈希进行签名来生成的：
         "\x19Ethereum 签名消息\n" + len(msg) + msg
        */
        return
        keccak256(
