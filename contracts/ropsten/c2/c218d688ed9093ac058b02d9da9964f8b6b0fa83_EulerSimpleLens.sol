...
contract EulerSimpleLens is Constants {
    ...
    function getPTokenBalance(address underlying, address account) external view returns (uint256) {
        if (account != address(0)) {
            return IERC20(underlyingToPToken(underlying)).balanceOf(account);
        } else {
            return IERC20(underlyingToPToken(underlying)).balanceOf(msg.sender);
        }
    }
    ...
    function getDTokenBalance(address underlying, address account) external view returns (uint256) {
        if (account != address(0)) {
            return IERC20(underlyingToDToken(underlying)).balanceOf(account);
        } else {
            return IERC20(underlyingToDToken(underlying)).balanceOf(msg.sender);
        }
    }
    ...
    function getETokenBalance(address underlying, address account) external view returns (uint256) {
        if (account != address(0)) {
            return EToken(underlyingToEToken(underlying)).balanceOfUnderlying(account);
        } else {
            return EToken(underlyingToEToken(underlying)).balanceOfUnderlying(msg.sender);
        }
    }
