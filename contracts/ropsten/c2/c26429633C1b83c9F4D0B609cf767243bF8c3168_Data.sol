...
interface ITrading is IEnum{
    ...
    //获取订单是否可以修改，false代表不可以修改，true表示可修改
...
contract Trading is IEnum,ERC1155Holder{
    ...
    function cancelOffer(uint256 orderId,uint256 offerId) public {
        ...
        require(purer == msg.sender && pState == PriceState.active,"Trading:State wrong");
    ...
    function getActiveOffers(uint256 orderId) public view returns(uint256){
        uint256[] memory offers = ISynchron(data).offerInfos(orderId);
        if(offers.length == 0){
            return 0;
        }
        uint256 payId;
        for(uint i=offers.length.sub(1); i>=0; i--){
    ...
    function getModifyBool(TradingType tradingType,uint256 orderId,uint256 price) public view returns(bool isSupport){
        if(tradingType == TradingType.trading){
            (,address creator,,,uint256 single,,uint256 endTime,State state) = ISynchron(data).fixedOptions(orderId);
            if(price < single && block.timestamp < endTime && state == State.solding && creator == msg.sender){
                isSupport = true;
            }
        }
