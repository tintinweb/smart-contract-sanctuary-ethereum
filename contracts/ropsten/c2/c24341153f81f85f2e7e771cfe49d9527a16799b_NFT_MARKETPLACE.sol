...
contract NFT_MARKETPLACE is IERC721Receiver {
...
/////
    constructor(address contractJAGO) {
        tkoContract = IERC20(contractJAGO);
    }
    ...
    external {
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            uint256 numAsk = _tokenIdCounter.current();
    ...
    function buyNFT(uint256 numAsk_) external {
        address sender = msg.sender;
        AskEntry memory NFTSeller = _NFTSellers[numAsk_];

        //PERHITUNGAN FEE

        tkoContract.transferFrom(sender, NFTSeller._seller, NFTSeller._price);
