...
contract Library is Ownable {
    ...
    event LogBookAdded(bytes32 id, string name, uint8 copies);
    event LogBookUpdated(bytes32 id, uint8 newQuantity);
    event LogBookTaken(bytes32 bookId, address userAddress);
    event LogBookReturned(bytes32 bookId, address userAddress);

    struct Book {
        string name;
        uint8 copies;
        mapping(address => bool) currentBookOwners;
        mapping(address => bool) hasEverBorrowBook;
        address[] buyers;
    }
    ...
    modifier bookIsNotTaken(bytes32 id) {
        require(
            !books[id].currentBookOwners[msg.sender],
        ...
        bookIsNotTaken(id)
    {
        ...
        book.copies -= 1;
        book.currentBookOwners[msg.sender] = true;
        if (!book.hasEverBorrowBook[msg.sender]) {
            book.hasEverBorrowBook[msg.sender] = true;
            book.buyers.push(msg.sender);
        }
        emit LogBookTaken(id, msg.sender);
    }

    function returnBook(bytes32 id) public bookExists(id) bookIsTaken(id) {
        books[id].copies += 1;
        books[id].currentBookOwners[msg.sender] = false;
        emit LogBookReturned(id, msg.sender);
    }
