...
contract AGLET_SNEAKERS_1 is ERC721URIStorage, Ownable, IMintable, ERC2981 {
    ...
    function safeMintWithRoyaltyDetails(
        address to,
        uint256 tokenId,
        string memory tokenUri,
    ...
    /// @param mintingBlob Tokens data, format {'tokenId'}:{'royaltyNumerator';'royaltyReceiver';'tokenURI'}
    function mintFor(
        address user,
        uint256 quantity,
    ...
    public override onlyMinters {
        ...
        tokenDetails = Minting.getNextDetails(mintingBlob, index);
        safeMintWithRoyaltyDetails(user, tokenId, tokenURI, royaltyReceiver, royaltyNumerator, tokenDetails);
    ...
    internal {
        require(_exists(tokenId), "Attributes set of nonexistent token");
        sneakerDetails[tokenId] = details;
    }
...
library Minting {
    ...
    returns (
        ...
        return (royaltyNumerator, startIndex + 6);
    }
    ...
    returns (
        ...
        return (royaltyRecipient, startIndex + 43);
    }
    ...
    returns (
        string memory,
        uint256) {
        require(blob[startIndex - 1] == ";", "Bad format of mintingBlob");
        int256 index = indexOf(blob, ";", startIndex, blob.length);
        require(uint256(index) < blob.length, "Bad format of mintingBlob");
        bytes calldata uriBytes = blob[startIndex : uint256(index)];
        string calldata uri = string(
            uriBytes
        );
        return (uri, uint256(index) + 1);
    }

    function getNextDetails(bytes calldata blob, uint256 startIndex)
    internal
    pure
    returns (
        string memory) {
        require(blob[startIndex - 1] == ";", "Bad format of mintingBlob");
        bytes calldata detailsBytes = blob[startIndex : blob.length - 1];
