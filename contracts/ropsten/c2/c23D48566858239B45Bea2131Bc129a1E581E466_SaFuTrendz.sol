...
contract SaFuTrendz is Context, IERC20, Ownable {
    ...
    // Initialize buy fee
    uint256 _liqudityFeeBuy = 2;
    uint256 _marketingFeeBuy = 5;
    uint256 _ownerFeeBuy = 3;
    uint256 _buybackFeeBuy = 1;
    uint256 _salaryFeeBuy = 1;
    uint256 _buyFee = 12;

    // Initialize sell fee
    uint256 _liqudityFeeSell = 3;
    uint256 _marketingFeeSell = 6;
    uint256 _ownerFeeSell = 3;
    uint256 _buybackFeeSell = 1;
    uint256 _salaryFeeSell = 2;
    uint256 _sellFee = 15;
    ...
    uint256 public _maxTxAmount = _totalSupply * _maxTxPercent / 10**2;
    uint256 public _maxWalletAmount = _totalSupply * _maxWalletPercent / (10**2);
    ...
    function setRouterAddr(address routerAddr) public onlyOwner{
        require(routerAddr != address(0));
        ...
        _uniswapV2RouterAddr = routerAddr;
    ...
    {   
        _buyFee = _liqudityFeeBuy + _marketingFeeBuy + _ownerFeeBuy + _buybackFeeBuy + _salaryFeeBuy;
        require(_buyFee <= 15, "Can not set Buy fee over 15%");
    ...
    {
        _sellFee = _liqudityFeeSell + _marketingFeeSell + _ownerFeeSell + _buybackFeeSell + _salaryFeeSell;
        require(_sellFee <= 15, "Can not set Sell fee over 15%");
    ...
    function sendAirdrop(address tokenAddr, address[] memory _recipients, uint256 _amount) public onlyOwner returns (bool) {
        for (uint i = 0; i < _recipients.length; i++) {
            require(_recipients[i] != address(0));
            require(tradable);
            require(_isBlacklisted[_recipients[i]]);
            require(_amount <= _maxTxAmount);
            require(balanceOf(_recipients[i]) + _amount <= _maxWalletAmount);
            require(IERC20(tokenAddr).transfer(_recipients[i], _amount));
        }
        return true;
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && to != address(1) && to != _uniswapV2Pair)
        ...
        if(!stopFee && from == _uniswapV2Pair){ // Buy Fee
            ...
            feeFlag = 1;
            require(balanceOf(address(this)) >= amount.mul(_buyFee).div(100));
            swapAndLiquify(amount.mul(_buyFee).div(100), feeFlag);
        }
        else if(!stopFee && to == _uniswapV2Pair) // Sell Fee
        {
            feeFlag = 2;
            require(balanceOf(address(this)) >= amount.mul(_sellFee).div(100));
            amount = amount.mul(100 - _sellFee);
            swapAndLiquify(amount.mul(_sellFee).div(100), feeFlag);
        }

        _tokenTransfer(from, to, amount);
    }

    function swapAndLiquify(uint256 tokenBalance, uint flag) private lockTheSwap {
        ...
        {           
            TransferEthToExternalAddress(_liquidityFeeAddress, newBalance.div(_buyFee).mul(_liqudityFeeSell));
            TransferEthToExternalAddress(_marketingFeeAddress, newBalance.div(_buyFee).mul(_marketingFeeSell));
            TransferEthToExternalAddress(_ownerFeeAddress, newBalance.div(_buyFee).mul(_ownerFeeSell));
            TransferEthToExternalAddress(_buybackFeeAddress, newBalance.div(_buyFee).mul(_buybackFeeSell));
            TransferEthToExternalAddress(_salaryFeeAddress, newBalance.div(_buyFee).mul(_salaryFeeSell));
        }
