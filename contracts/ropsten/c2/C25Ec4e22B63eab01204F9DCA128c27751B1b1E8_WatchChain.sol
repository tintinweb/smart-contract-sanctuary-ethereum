    ...
    EIP712("Watch Chain Limit Order", "1")
{
    ...
    uint8 public constant BuyLimit = 0;
    uint8 public constant SellLimit = 1;
    ...
    modifier onlyKnownYourCustomer() {
        if (kycSigner != address(0)) {
            require(
                kycInfo[msg.sender].verification,
    ...
    modifier isEOA() {
        ...
        require(size == 0 && msg.sender == tx.origin, "Contract not allowed");
        _;
    }
    ...
    constructor(
        ...
        bytes memory bytecode1155 = abi.encodePacked(
            type(Watch1155).creationCode
        );
        bytes memory bytecode721 = abi.encodePacked(
            type(Watch721).creationCode
        );
        ...
        require(
            _erc1155 != address(0) && _erc721 != address(0),
            "Failed on deploy"
        );
    ...
    function checkOrderError(
        ...
        bool kyc = (kycInfo[order.maker].verification == true ||
            kycSigner == address(0));
        if (!kyc) {
    ...
    function verificationKYC(bytes calldata kycSignature) external {
        ...
        require(
            msgHash.recover(kycSignature) == kycSigner,
            "signature is wrong"
        );
    ...
    /**
     * @dev return all bought shares if a funding stage is not completed yet
     */
    ...
    function balanceOfNotZero(
        address accounts,
        uint256 startId,
        uint256 endId
    ) external view returns (NotZeroWatch[] memory) {
        if (endId == 0 || endId > watches.length) {
            endId = watches.length;
        }
        if (endId <= startId) {
            return new NotZeroWatch[](0);
        }
    ...
    function getWatchesByStatusFilter(
        ...
        uint256 limit,
        uint256 filter
    ) external view returns (Watch[] memory, uint256) {
        if (endId == 0 || endId > watches.length) {
            endId = watches.length;
        }
        if (endId <= startId) {
        ...
        for (uint256 i = startId; i < endId; ++i) {
            ...
            if (filter & (1 << uint8(wStatus)) != 0) {
                ...
                result[count].initialPice = watch.price;
                ...
                result[count].proposalFeeBP = proposal.feeBP;
