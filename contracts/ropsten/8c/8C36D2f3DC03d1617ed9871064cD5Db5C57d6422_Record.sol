...
contract Record is BaseDutchAuctionERC721A {
    struct RECORD {
        uint256 startAt;
        uint256 stopAt;
        string padNumbers;       
    ...
    mapping(address => uint256[]) stopAts;
    constructor(
    ...
    ) BaseDutchAuctionERC721A(payees, shares, name, symbol, _whitelistMaxMint, _publicListMaxMint, _nonReservedMax, _reservedMax, _discountedPrice)
    {
    }

    function setRecord(uint256 startAt, uint256 stopAt, string memory padNumbers) public {
        require(records[msg.sender][stopAt].startAt == 0, "already set");

        RECORD memory record;
        record.startAt = startAt;
        record.stopAt = stopAt;
        record.padNumbers = padNumbers;
        records[msg.sender][stopAt] = record;

        stopAts[msg.sender].push(stopAt);
    }

    function getRecords() external view returns(RECORD[] memory){
        uint256[] memory stops = stopAts[msg.sender];
        uint256 len = stops.length;
        RECORD[] memory userRecords = new RECORD[](len);
        for (uint256 i = 0; i < len; i ++) {
            userRecords[i] = records[msg.sender][stops[i]];
        }
