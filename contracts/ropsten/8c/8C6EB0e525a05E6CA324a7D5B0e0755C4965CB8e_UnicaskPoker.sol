...
contract UnicaskPoker {
    ...
    // return 1~52
    function draw() public blockNoNotUsed {
        ...
        if (_gameNo > _personCards[msg.sender].gameNo){
            ...
            require(_personCards[msg.sender].drawTimes < _drawCount, string(abi.encodePacked("you have draw ", _drawCount," times")));
        }

        uint8 randomNo = uint8(uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, block.coinbase))) % 52) + 1;
