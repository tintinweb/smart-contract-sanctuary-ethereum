...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, sBETS }

    address public immutable BETS;
    ...
    address public sBETS;
    uint public sBETSQueue; // Delays change to sBETS address
    ...
    /**
        @notice allow approved address to deposit an asset for BETS
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint BETS needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( BETS ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BETS for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITEMERC20( BETS ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sBETS ).balanceOf( msg.sender ); // Can only borrow against sBETS held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BETS
        @param _amount uint
     */
    function repayDebtWithBETS( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITEMERC20( BETS ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, BETS, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( BETS ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( BETS ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BETS valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BETS decimals
            value_ = _amount.mul( 10 ** IERC20( BETS ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.sBETS ) { // 9
            sBETSQueue = block.number.add( blocksNeededForQueue );
        } else return false;
