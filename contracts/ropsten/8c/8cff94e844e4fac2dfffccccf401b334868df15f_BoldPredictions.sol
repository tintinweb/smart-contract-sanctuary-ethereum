...
contract BoldPredictions {
    ...
    string public deploymentNote; // A short text about this deployment  
    uint16 public objectionPeriodDays; // how many days need to be waited from last outcomeSubmission until the reward can be claimed
    uint8 public facilitationFeePercentage; // how much in percentage is kept by this contract
    bool public newPredictionsAllowed; // Wheter it is possible to create new predictions
    ...
    Prediction[] public predictions; // array of all created predictions
    ...
    modifier isCreator(uint16 _predictionId) {
        require(msg.sender == predictions[_predictionId].creator, "Caller is not creator of this prediction");
        _;
    }
    ...
    /**
     * @dev Calculates the count of bets for and against the prediction
     */
    function getTotalBetCounts(uint16 _predictionId) public view  validPredictionId(_predictionId) returns (uint forCount, uint againstCount){
        forCount = 0;
        againstCount = 0;
        for(uint i = 0; i < predictions[_predictionId].bets.length; i++){
            if(predictions[_predictionId].bets[i].predictedOutcome)
    ...
    function getStatusText(PredictionStatus _enum) private pure returns (string memory returnText) {
            if(_enum == PredictionStatus.open){
                return "open";
            }
            else if(_enum == PredictionStatus.succeeded){
                return "succeeded";
            }
            else if(_enum == PredictionStatus.failed){
                return"failed";
            }
            else return "inconclusive";
    }
    // -------------------------------------------------------------------------------------
    //  Functions used by UI to get data
    // -------------------------------------------------------------------------------------
    /**
     * @dev Returns list of all predictions
     */
    function getPredictionsList() public view returns (uint16[] memory pIds, address[] memory creatorAddresses, string[] memory statusTexts, uint[] memory createdTimestamps, uint[] memory noBetsAfterTimestamps, string[] memory predictionTexts, uint[] memory betForCounts, uint[] memory betAgainstCounts, uint[] memory betForSums, uint[] memory betAgainstSums){
        // Initialize arrays
        pIds = new uint16[](predictions.length);
        creatorAddresses = new address[](predictions.length);
        statusTexts = new string[](predictions.length);
        createdTimestamps = new uint[](predictions.length);
        noBetsAfterTimestamps = new uint[](predictions.length);
        predictionTexts = new string[](predictions.length);
        betForCounts = new uint[](predictions.length);
        betAgainstCounts = new uint[](predictions.length);
        betForSums = new uint[](predictions.length);
        betAgainstSums = new uint[](predictions.length);

        for(uint i = 0; i < predictions.length; i++){
            pIds[i] = predictions[i].predictionId;
            creatorAddresses[i] = predictions[i].creator;
            statusTexts[i] = getStatusText(predictions[i].predictionOutcome);
            createdTimestamps[i] = predictions[i].timestamp;
            noBetsAfterTimestamps[i] = predictions[i].noBetsAfter;
            predictionTexts[i] = predictions[i].predictionText;

            (betForCounts[i], betAgainstCounts[i]) = getTotalBetCounts(predictions[i].predictionId);
            (betForSums[i], betAgainstSums[i]) = getTotalBetAmounts(predictions[i].predictionId);

        }


        return (pIds, creatorAddresses, statusTexts, createdTimestamps, noBetsAfterTimestamps, predictionTexts, betForCounts, betAgainstCounts, betForSums, betAgainstSums);
    }    
    /**
     * @dev Returns list of all bets for a specific prediction
     */
    function getBetsList(uint16 _predictionId) public view returns (address[] memory senderAddresses, uint[] memory createdTimestamps,  uint[] memory amounts, bool[] memory predictedOutcomes){
        // Initialize arrays

        senderAddresses = new address[](predictions[_predictionId].bets.length);
        createdTimestamps = new uint[](predictions[_predictionId].bets.length);
        amounts = new uint[](predictions[_predictionId].bets.length);
        predictedOutcomes = new bool[](predictions[_predictionId].bets.length);

        for(uint i = 0; i < predictions[_predictionId].bets.length; i++){
            senderAddresses[i] = predictions[_predictionId].bets[i].sender;
            createdTimestamps[i] = predictions[_predictionId].bets[i].timestamp;
            amounts[i] = predictions[_predictionId].bets[i].amount;
            predictedOutcomes[i] = predictions[_predictionId].bets[i].predictedOutcome;

        }

        return (senderAddresses, createdTimestamps, amounts, predictedOutcomes);
    }   
    /**
     * @dev Returns list of all submitted outcomes for a specific predictions
     */
    function getOutcomesList(uint16 _predictionId) public view returns (address[] memory senderAddresses, uint[] memory createdTimestamps,  bool[] memory predictedOutcomes, bool hadDisputeResolution){
        hadDisputeResolution = predictions[_predictionId].hadDisputeResolution;
        // Initialize arrays
        senderAddresses = new address[](predictions[_predictionId].outcomeSubmissions.length);
        createdTimestamps = new uint[](predictions[_predictionId].outcomeSubmissions.length);
        predictedOutcomes = new bool[](predictions[_predictionId].outcomeSubmissions.length);

        return (senderAddresses, createdTimestamps, predictedOutcomes, hadDisputeResolution);
    }   
        /**
     * @dev Returns list of all bets that a specific address has done across all predictions
     */
    function getBetsOfAddress(address _address, uint startListFromId) public view returns (uint16[] memory pIds, address[] memory creatorAddresses, string[] memory statusTexts, uint[] memory createdTimestamps, string[] memory predictionTexts, uint[] memory betTimestamps,  uint[] memory amounts, bool[] memory predictedOutcomes, uint totalBetCount){
        // Initialize arrays - max list of 10 at a time

        pIds = new uint16[](10);
        creatorAddresses = new address[](10);
        statusTexts = new string[](10);
        createdTimestamps = new uint[](10);
        predictionTexts = new string[](10);
        betTimestamps = new uint[](10);
        amounts = new uint[](10);
        predictedOutcomes = new bool[](10);
        ...
        for(uint i = 0; i < predictions.length; i++){
            ...
            // Now go through all bets to find those that are from _address
            for(uint x = 0; x < predictions[i].bets.length; x++){
                if(predictions[i].bets[x].sender == _address){
                    totalBetCount++;
                    if(arrayCount < 10 && totalBetCount >= startListFromId){
                        arrayCount++;
                        // General stuff from the prediction
                        pIds[arrayCount-1] = predictions[i].predictionId;
                        creatorAddresses[arrayCount-1] = predictions[i].creator;
                        statusTexts[arrayCount-1] = getStatusText(predictions[i].predictionOutcome);
                        createdTimestamps[arrayCount-1] = predictions[i].timestamp;
                        predictionTexts[arrayCount-1] = predictions[i].predictionText;
                        // Bet-specifics
                        betTimestamps[arrayCount-1] = predictions[i].bets[x].timestamp;
                        amounts[arrayCount-1] = predictions[i].bets[x].amount;
                        predictedOutcomes[arrayCount-1] = predictions[i].bets[x].predictedOutcome;
                    }
                }
            }
        ...
        return (pIds, creatorAddresses, statusTexts, createdTimestamps, predictionTexts, betTimestamps, amounts, predictedOutcomes, totalBetCount);
    }    
}