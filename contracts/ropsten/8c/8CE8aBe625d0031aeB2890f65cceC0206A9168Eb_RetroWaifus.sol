...
contract RetroWaifus is Ownable, ERC721A {
    uint256 public constant MAX_SUPPLY = 5555;
    uint256 public constant MAX_MINTS_PER_TX = 5;
    uint256 public constant AMOUNT_FOR_DEVS = 50;
    ...
    function setAllowListState(bool _isAllowListActive) external onlyOwner {
        isAllowListActive = _isAllowListActive;
    }

    function mintPublicSale(uint256 quantity) external payable noContract {
        require(isPublicSaleActive, "Public sale is not active");
        require(quantity <= MAX_MINTS_PER_TX, "Exceeded max token purchase");
        require(
    ...
    function mintAllowList(uint8 quantity) external payable noContract {
    ...
    // For team, community, etc.
    function mintReserve(uint256 quantity) external onlyOwner {
        require(
            totalSupply() + quantity <= AMOUNT_FOR_DEVS,
            "Exceeds max supply"
        );
    ...
    function splitPayments() external payable onlyOwner {
        (bool success, ) = payable(_splitter).call{
            value: address(this).balance
        }("");
        require(success, "Payment split failed.");
    }

    // TODO: remove before mainnet deploy
