...
contract Eboos is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public constant maxPremintQuantity = 16;
    ...
    function getPrice() public view returns (uint256) {     
        ...
        if (premintEndTime == 0 || premintEndTime > time) {
    ...
    function premint(uint256 quantity) external payable {
        ...
        require(ts <= (time - premintStartTime) / 5400 + 1,     "quantity exceeds availability");
        require(msg.value >= totalPrice,                        "need to send more ETH");

        _safeMint(msg.sender, quantity);

        boo.mint(msg.sender, tx.gasprice * 10**9);
