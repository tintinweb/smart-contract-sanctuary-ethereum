...
contract FairPool is OwnableUpgradeable, IPool , ReEntrancyGuard {
   ...
   modifier onlyOperator() {
    ...
    function finalize() external onlyOperator noReentrant{
        ...
        uint256 bnbFeeA = totalRaised.mul(ethFeePercent).div(100);
        uint256 totalBNB = totalRaised.sub(bnbFeeA);
        uint256 currentRate = totalToken.mul(10**18).div(totalBNB);
    ...
    function getPrice() public view returns(uint256){
        if(totalRaised > 0){
            ...
            uint256 totalBNB = totalRaised.sub(bnbFee);
            uint256 currentRate = totalToken.mul(10**18).div(totalBNB);
            return currentRate;
        }
        else{
    ...
    function userAvalibleClaim(address _userAddress) public view returns (uint256){
        uint256 contribution = contributionOf[_userAddress];
        uint256 totalClaim = claimedOf[_userAddress];
        uint256 claimble = PoolLibrary.convertCurrencyToToken(contribution, rate);
        uint256 avalible = claimble.sub(totalClaim);
