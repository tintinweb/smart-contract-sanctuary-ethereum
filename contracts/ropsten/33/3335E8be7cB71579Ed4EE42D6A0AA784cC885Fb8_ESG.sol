...
interface ITaxFeeHandler {
    function swapBack() external;
}
contract TaxFeeHandler is ITaxFeeHandler {
    address _tokenAddr;
    IERC20 esg;
    IDEXRouter router;
    address public MATIC = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
    modifier onlyToken() {
        require(msg.sender == address(_tokenAddr)); _;
    }
    constructor(){
        router = IDEXRouter(address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D));
        _tokenAddr = msg.sender;
        esg = IERC20(_tokenAddr);
    }
    function swapBack() external override onlyToken {
        uint256 amount = esg.balanceOf(address(this));
        if(amount>0) {
            ...
            router.swapExactTokensForETHSupportingFeeOnTransferTokens(
                ...
                0,
                path,
                address(0xa300915690Ac000E05bd5fD91f6C14EF0838727c),
                block.timestamp
            );
        }
    }
...
contract ESG is IERC20, Auth {
    ...
    address public pair;
    ITaxFeeHandler public feehandler;
    bool inSwap;
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if(swapInToken) {
            ...
            _balances[address(feehandler)] = _balances[address(feehandler)].add(feeAmount);
            emit Transfer(sender, address(feehandler), feeAmount);
        }
