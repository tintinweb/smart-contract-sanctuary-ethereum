...
abstract contract MigratablePool is Ownable {
...
///         When locking user choose period and based on period multiplier is apply to the amount (boost).
///         If pool is set for rUMB1->rUMB2, (rUmbPool) then rUMB2 can be locked as well
contract StakingLockable is LockSettings, IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    struct Times {
        uint32 periodFinish;
    ...
    struct Balance {
        ...
        // TODO write test for max possible values, to see if this holds with uint160
    ...
    /// @notice when `true`, that means this pool accepts rUMB1 (and rUMB2) as input tokens
    ///         at the same time, we only have locking option available for rUMB tokens, we can not stake
    bool public immutable rUmbPool;

    uint256 public rewardRate = 0;
    uint256 public rewardPerTokenStored;

    Supply private _totalSupply;
    ...
    event LockedTokens(address indexed user, uint256 lockId, uint256 amount, uint256 period, uint256 multiplier);
    event UnlockedTokens(address indexed user, uint256 lockId, uint256 amount);
    ...
    modifier onlyAcceptedToken(address _token) {
        require(_token == address(stakingToken) || (rUmbPool && _token == address(rewardsToken)), "invalid token");
    ...
    modifier updateReward(address _account) virtual {
        ...
        times.lastUpdateTime = uint32(lastTimeRewardApplicable());
    ...
    constructor(
        ...
        require(
            MintableToken(_stakingToken).maxAllowedTotalSupply() * MAX_MULTIPLIER / RATE_DECIMALS <= type(uint96).max,
            "stakingToken overflow"
        );

        require(
            MintableToken(_rewardsToken).maxAllowedTotalSupply() * MAX_MULTIPLIER / RATE_DECIMALS <= type(uint96).max,
            "rewardsToken overflow"
        );
        ...
        times.rewardsDuration = 2592000; // 30 days
    }
    ...
    function finishFarming() external whenActive onlyOwner {
        ...
        if (_totalSupply.totalBalance != 0) {
            uint32 remaining = uint32(t.periodFinish - block.timestamp);
            times.rewardsDuration = t.rewardsDuration - remaining;
        }

        times.periodFinish = uint32(block.timestamp);
    ...
    function stake(uint256 _amount) override external {
        if (rUmbPool) revert("only locking is possible for rUmbPool");

        _stake(address(stakingToken), msg.sender, _amount, 0, false);
    }

    function periodFinish() external view returns (uint256) {
        return times.periodFinish;
    }

    function rewardsDuration() external view returns (uint256) {
        return times.rewardsDuration;
    }

    function lastUpdateTime() external view returns (uint256) {
        return times.lastUpdateTime;
    }

    function balanceOf(address _account) override external view returns (uint256) {
        Balance memory b = balances[_account];
        return b.balance + b.lockedWithBonus;
    }

    function getRewardForDuration() override external view returns (uint256) {
        return rewardRate * times.rewardsDuration;
    }

    function rewards(address _user) external view returns (uint256) {
        return balances[_user].rewards;
    }

    function swapStakingForReward() public {
        if (!rUmbPool) return;

        uint256 myBalance = stakingToken.balanceOf(address(this));
        if (myBalance == 0) return;

        IBurnableToken(address(stakingToken)).burn(myBalance);
        OnDemandToken(address(rewardsToken)).mint(address(this), myBalance);
    }
    ...
    function notifyRewardAmount(
        uint256 _reward
    ) override public whenActive onlyRewardsDistribution updateReward(address(0)) {
        // this method can be executed on its own as well, I'm including here to not need to remember about it
        swapStakingForReward();

        Times memory t = times;

        if (block.timestamp >= t.periodFinish) {
            rewardRate = _reward / t.rewardsDuration;
        } else {
            uint256 remaining = t.periodFinish - block.timestamp;
            uint256 leftover = remaining * rewardRate;
            rewardRate = (_reward + leftover) / t.rewardsDuration;
        }

        require(rewardRate != 0, "invalid rewardRate");

        times.lastUpdateTime = uint32(block.timestamp);
        times.periodFinish = uint32(block.timestamp + t.rewardsDuration);

        emit RewardAdded(_reward);
    }

    function setRewardsDuration(uint256 _rewardsDuration) public whenActive onlyRewardsDistribution {
        require(_rewardsDuration != 0, "empty _rewardsDuration");

        require(
            block.timestamp > times.periodFinish,
            "Previous period must be complete before changing the duration"
        );

        times.rewardsDuration = uint32(_rewardsDuration);
    ...
    function totalSupply() override public view returns (uint256) {
        return _totalSupply.totalBalance;
    }

    function lastTimeRewardApplicable() override public view returns (uint256) {
        return Math.min(block.timestamp, times.periodFinish);
    }

    function rewardPerToken() override public view returns (uint256 perToken) {
        Supply memory s = _totalSupply;
        ...
        perToken = rewardPerTokenStored + (
            (lastTimeRewardApplicable() - times.lastUpdateTime) * rewardRate * 1e18 / (s.totalBalance + s.totalBonus)
        );
    }

    function earned(address _account) override virtual public view returns (uint256) {
        Balance memory b = balances[_account];
        uint256 totalBalance = b.balance + b.lockedWithBonus;
    ...
    function calculateBonus(uint256 _amount, uint256 _multiplier) public pure returns (uint256 bonus) {
        bonus = _amount * (_multiplier - RATE_DECIMALS) / RATE_DECIMALS;
    }

    function _stake(address _token, address _user, uint256 _amount, uint256 _bonus, bool _migration)
        internal
        nonReentrant
        notPaused
        ...
        updateReward(_user)
    {
        ...
        require(times.periodFinish != 0, "Stake period not started yet");
        require(amountWithBonus != 0, "Cannot stake 0");

        _totalSupply.totalBalance += uint96(_amount);
        _totalSupply.totalBonus += uint128(_bonus);

        if (_bonus == 0) {
            balances[_user].balance += uint96(_amount);
        } else {
    ...
    function _lockTokens(address _user, address _token, uint256 _amount, uint256 _period) internal {
        uint256 multiplier = multipliers[_period];
        require(multiplier != 0, "invalid period");

        uint256 stakeBonus = calculateBonus(_amount, multiplier);

        _stake(_token, _user, _amount, stakeBonus, false);
        _addLock(_user, _amount, _period, multiplier);
    }

    function _addLock(address _user, uint256 _amount, uint256 _period, uint256 _multiplier) internal {
        ...
        lock.amount = uint128(_amount);
        lock.multiplier = uint32(_multiplier);
        lock.lockDate = uint32(block.timestamp);
        lock.unlockDate = uint32(block.timestamp + _period);

        emit LockedTokens(_user, newIndex, _amount, _period, _multiplier);
    }

    function _unlockTokensFor(address _user, uint256[] calldata _indexes, address _recipient) internal {
        uint256 totalAmount;
        uint256 totalBonus;

        for (uint256 i; i < _indexes.length; i++) {
            (uint256 amount, uint256 bonus) = _unlockTokenFor(_user, _indexes[i]);
            totalAmount += amount;
            totalBonus += bonus;
        }

        if (totalAmount + totalBonus != 0) {
            balances[_user].lockedWithBonus -= uint96(totalAmount + totalBonus);
            _totalSupply.totalBalance -= uint96(totalAmount);
            _totalSupply.totalBonus -= uint128(totalBonus);
        }

        if (_recipient != address(0) && totalAmount != 0) {
            IERC20 token = rUmbPool ? rewardsToken : stakingToken;
            // note, there is one case when this transfer can fail:
            // on rUmbPool when we did not swap stakingToken -> rewardsToken,
            // in that case we have to execute `swapStakingForReward`
            // to save gas I'm not including it here, because it is unlikely case
            token.transfer(_recipient, totalAmount);
        }
    }

    function _unlockTokenFor(address _user, uint256 _index) internal returns (uint256 amount, uint256 bonus) {
        ...
        if (lock.amount == 0) return (0, 0);

        locks[_user][_index].withdrawnAt = uint32(block.timestamp);
        emit UnlockedTokens(msg.sender, _index, lock.amount);

        return (lock.amount, calculateBonus(lock.amount, lock.multiplier));
    }
    ...
    function _withdraw(uint256 _amount, address _user, address _recipient) internal nonReentrant updateReward(_user) {
        ...
        if (_amount == type(uint256).max) _amount = balance.balance;
        else require(balance.balance >= _amount, "withdraw amount to high");

        if (_amount == 0) return;

        // not using safe math, because there is no way to overflow because of above check
        _totalSupply.totalBalance -= uint128(_amount);
        balances[_user].balance = uint96(balance.balance - _amount);

        // not using safe transfer, because we working with trusted tokens
        require(stakingToken.transfer(_recipient, _amount), "token transfer failed");
...
abstract contract LockSettings is Ownable {
    ...
    mapping(uint256 => uint256) public periodKeys;
    // TODO add getters for FE that will return periods + miltipliers
    uint256[] public periods;

    event TokenSettings(uint256 period, uint256 multiplier);

    function removePeriod(uint256[] calldata _periods) external onlyOwner {
        for (uint256 i; i < _periods.length; i++) {
            if (_periods[i] == 0) revert("InvalidSettings");

            multipliers[_periods[i]] = 0;
            _removePeriod(_periods[i]);

            emit TokenSettings(_periods[i], 0);
        }
    }

    function setStakingTokenSettings(uint256[] calldata _periods, uint256[] calldata _multipliers)
        external
        onlyOwner
    {
        ...
        for (uint256 i; i < _periods.length; i++) {
            ...
            multipliers[_periods[i]] = _multipliers[i];
            emit TokenSettings(_periods[i], _multipliers[i]);

            if (_multipliers[i] == 0) _removePeriod(_periods[i]);
            else _addPeriod(_periods[i]);
        }
    }
    ...
    function _addPeriod(uint256 _period) internal {
        uint256 key = periodKeys[_period];
        if (key != 0) return;

        periods.push(_period);
        // periodKeys must be != 0
        periodKeys[_period] = periods.length;
    }

    function _removePeriod(uint256 _period) internal {
        uint256 key = periodKeys[_period];
        if (key == 0) return;

        periods[key - 1] = periods[periods.length - 1];
        periodKeys[_period] = 0;
        periods.pop();
    }
}
...
abstract contract SwappableTokenV2 is Owned, ERC20 {
    ...
    // TODO mayby struct with swapStartsOn?
    bool public immutable swappableSupported;

    event LogStartEarlySwapNow(uint time);
    event LogSwap(address indexed swappedTo, uint amount);
    event LogDailyCup(uint newCup);

    modifier onlyWhenSwappableSupported() {
        if (!swappableSupported) revert("swap not supported");
        _;
    }

    constructor(address _umb, uint32 _swapStartsOn, bool _swappableSupported) {
        require(!_swappableSupported || _swapStartsOn > block.timestamp, "invalid swapStartsOn");
        require(ERC20(_umb).decimals() == 18, "invalid UMB token");

        swapStartsOn = _swapStartsOn;
        umb = ISwapReceiver(_umb);
        swappableSupported = _swappableSupported;
    }

    function swapForUMB() external onlyWhenSwappableSupported {
        ...
        require(amountToSwap <= limit, "daily CUP limit");

        swapData.usedLimit = uint32(freshLimit ? amountWoDecimals : data.usedLimit + amountWoDecimals);
        ...
        umb.swapMint(_msgSender(), amountToSwap);

        emit LogSwap(_msgSender(), amountToSwap);
    }

    function startEarlySwap() external onlyWhenSwappableSupported onlyOwner {
    ...
    function setDailyCup(uint32 _cup) external onlyWhenSwappableSupported onlyOwner {
        swapData.dailyCup = _cup;
        emit LogDailyCup(_cup);
    }

    function isSwapStarted() external view returns (bool) {
        return swappableSupported && (block.timestamp >= swapStartsOn || swapData.swapEnabledAt != 0);
    }

    function canSwapTokens(address _address) external view returns (bool) {
        if (!swappableSupported) return false;

        (uint256 limit,) = _currentLimit(swapData);
        return balanceOf(_address) / ONE <= limit;
    }
