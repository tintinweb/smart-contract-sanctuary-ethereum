...
contract NFTMarket is IERC721Receiver, ReentrancyGuard, Ownable{
 ...
 address payable _minter;
  MintNft public unicusNft;
  address unicusNftAddress;
  uint256 public treasuryRoyalty = 2;
  
  constructor(address _nftContract,address _treasury) {
      
  unicusNftAddress = _nftContract;    //nft contract for unicus platform
  unicusNft = MintNft(_nftContract);
...
//struct for each market item
  ...
  function createSale(
    ...
    if( nftContract == unicusNftAddress){
    minter = unicusNft.minterOfToken(tokenId);
    royalty = unicusNft.royaltyForToken(tokenId);
    }
