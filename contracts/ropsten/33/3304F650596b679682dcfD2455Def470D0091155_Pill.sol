...
contract Pill is IBEP20, Auth, IERC721Receiver {
    ...
    string constant _name = "NPILL";
    string constant _symbol = "NPILL";
    ...
    IBEP20 public myWETH;
    INhatManager public nhatmanager;
    ...
    uint256 public swapThreshold = 1000000000000000000; // 0.05%
    bool inSwap;
    ...
    constructor (
        ...
        address _nhatmanager
    ) Auth(msg.sender) {
        uniswapRouter = IUniswapRouter(_uniswapRouter);
        myWETH = IBEP20(uniswapRouter.WETH9());
        nhatmanager = INhatManager(_nhatmanager);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNB = myWETH.balanceOf(address(this)).sub(balanceBefore);
        ...
        TransferHelper.safeApprove(address(this), address(nhatmanager), amountToLiquify);
        TransferHelper.safeApprove(WBNB, address(nhatmanager), amountBNBLiquidity);   

        if(amountToLiquify > 0){
            emit AutoLiquify(amountBNBLiquidity, amountToLiquify,balanceBefore ,dynamicLiquidityFee,amountBNBMarketing);
            nhatmanager.increaseLiquidityCurrentRange2(
                tokenId,
                amountToLiquify,
                address(this),
                amountBNBLiquidity,
    ...
    event AutoLiquify(uint256 amountBNB, uint256 amountBOG,uint256 amountBOG2,uint256 amountBOG3,uint256 amountBOG4);
