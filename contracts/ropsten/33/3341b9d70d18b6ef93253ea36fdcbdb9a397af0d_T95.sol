...
contract T95 is IERC20, Auth {
	string constant _name = "Tel95";
	string constant _symbol = "T95";
	...
	mapping (address => uint256) public _lastSwapBlock; //TODO consider making this private
	...
	//TODO - finalize addresses and values before deploying!
	function _distributeInitialBalances() internal {
		// TXN settlement address + 263B L9 -
		uint256 txnSettlementAmount = 263_000_000_000 * 10**_decimals;
		_balances[address(0xCe3A0aB7C4c82A53D0FB7FdC4A9067A0AfEFd769)] = txnSettlementAmount;
		emit Transfer(address(0), address(0xCe3A0aB7C4c82A53D0FB7FdC4A9067A0AfEFd769), txnSettlementAmount );

		// Rewards pool allocation address 25% 2.5T: 
		uint256 rewardsPoolAmount = 2_500_000_000_000 * 10**_decimals;
		_balances[address(0xCe3A0aB7C4c82A53D0FB7FdC4A9067A0AfEFd769)] = rewardsPoolAmount;
		emit Transfer(address(0), address(0xCe3A0aB7C4c82A53D0FB7FdC4A9067A0AfEFd769), rewardsPoolAmount );

		// exchange/burn 25%  2.5T:
		uint256 exchangeBurnAmount = 2_500_000_000_000 * 10**_decimals;
		_balances[address(0xDB8690098bE8BAaEffF1D4A463b210cc8F0863A4)] = exchangeBurnAmount;
		emit Transfer(address(0), address(0xDB8690098bE8BAaEffF1D4A463b210cc8F0863A4), exchangeBurnAmount );
		 
		// Dev Fund 15% 1.5T: 
		uint256 devFundAmount = 1_500_000_000_000 * 10**_decimals;
		_balances[address(0xDB8690098bE8BAaEffF1D4A463b210cc8F0863A4)] = devFundAmount;
		emit Transfer(address(0), address(0xDB8690098bE8BAaEffF1D4A463b210cc8F0863A4), devFundAmount );

		// Team wallet 5% 500B:
		uint256 teamAmount = 500_000_000_000 * 10**_decimals;
		_balances[address(0xc1bE78Bd439D6c83801724bBbAd0207155e58C9a)] = teamAmount;
		emit Transfer(address(0), address(0xc1bE78Bd439D6c83801724bBbAd0207155e58C9a), teamAmount );
		 
		// Marketing 1.7% 170B: 
		uint256 marketingAmount = 170_000_000_000 * 10**_decimals;
		_balances[address(0xC5290Fa5Ac8286c231618389C4d5f0bD3d8dc702)] = marketingAmount;
		emit Transfer(address(0), address(0xC5290Fa5Ac8286c231618389C4d5f0bD3d8dc702), marketingAmount );
		...
		// amount in uniswap LP at 16/02/2022 00:04 UTC...
	...
	function _openTrading() internal {
		maxTxAmount     = 200 * _totalSupply / 1000 + 10**_decimals;  //TODO 20% but set final limits
		maxWalletAmount = 200 * _totalSupply / 1000 + 10**_decimals;  //TODO 20% but set final limits
		taxRateBuy = 3;
		taxRateSell = 3;
		tradingOpen = true;
	}
	...
	function excludeFromAntiBot(address wallet, bool excluded) external onlyOwner {
		...
		excludedFromAntiBot[wallet] = excluded;
	}

	function excludeFromFees(address wallet, bool toggle) external onlyOwner {
		if (!toggle) { require(wallet != address(this) && wallet != owner, "Cannot enforce fees for this address"); }
		excludedFromFees[ wallet ] = toggle;
	}

	function excludeFromLimits(address wallet, bool toggle) external onlyOwner {
