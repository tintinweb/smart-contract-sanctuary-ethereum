...
contract EventsDatabase is Ownable, EventStructContract {
    ...
    event createEventLog(string _description, uint _eventDate,  uint64 indexed _eventID );
    event setEventResultLog(string _description,  uint64 indexed _eventID, uint8 _result );
    event editEventResultLog(string _description,  uint64 indexed _eventID, uint8 _result );
...
contract BetContract is Ownable, EventStructContract {
    ...
    // события
    event createBetLog(uint64 indexed _eventID, uint _rate, uint _value, uint indexed betID, address indexed _bettor  );
    event createOfferLog(uint64 indexed _eventID, uint _rate, uint _value, uint indexed betID, address indexed _bookmaker  );

    event acceptBetLog(uint64 indexed _eventID, uint indexed _betID, address indexed _bookmaker  );
    event acceptOfferLog(uint64 indexed _eventID, uint indexed _betID, address indexed _bettor  );
  ...
  constructor(address initAddr) public {
    ...
    eventsDatabase = EventsDatabase(0x9e7AC8B21196941DeC70c53aA0E6275db6157019);//initAddr);
