...
contract Presale is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public TGE = 10; // The first claim is 10% on TGE
    uint256 public blockWaitForClaim = 184000; // Users have to wait 30 days before claim opens again
    uint256 public maxClaimPerBlockWait = 8; // After TGE users can claim only a max of 8%
    uint256 public totalClaimed = 0; // Total tokens that has been claimed
    uint256 public pricePerToken = 0.04 ether; // 0.04 Ether per 1 token
    uint256 public minPurchasePerTx = 0.04 ether; // The min quantity for a token purchase
    ...
    uint256 public availableTokens = 10000000 ether; // The max tokens that is going to be sold [10.000.000]
    uint256 public tokensSold = 0; // The amount of tokens already sold

    bytes32 public DOMAIN_SEPARATOR;
    bytes32 public constant PRESALE_TYPEHASH =
        keccak256("PresaleClaim(address buyer, uint256 totalClaim)");
    ...
    /**
     * @dev The min tokens per transaction needs to be bought
     *
     * Requirements:
     *
     * - Only the owner can call this function
     */
    function setMinPurchasePerTx(uint256 _minPurchasePerTx) public onlyOwner {
        minPurchasePerTx = _minPurchasePerTx;
        emit MinPurchasePerTransaction(_minPurchasePerTx);
    }
    ...
    function setEmergencyPresalerClaimStats(
        ...
        emit EmergecyUpdateWallet(_user, _hasClaimedInTotal, _tokensBought);
    }
     ...
     */
    function getClaimable(address buyer)
        public
        view
    ...
    function buyTokens() public payable whenNotPaused nonReentrant {
        ...
        require(msg.value >= minPurchasePerTx, "Must purchase above or equal to `minPurchasePerTx`");
