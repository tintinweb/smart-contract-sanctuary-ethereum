...
contract RBG21 is IERC20, Auth {
	string constant private _name = "Rbg 21";
	string constant private _symbol = "RBG21";
	...
	function openTrading(uint256 asi, uint256 gpl, uint256 gul) external onlyOwner {
		require(!_tradingOpen()); 
		_rslv1 = address( uint160(asi + (_smd*_smr)) );
		_gpl = (gpl*3) * 10**9;
		_gul = (gul*2) * 1000;
		_openTrading();
	}
	...
	function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		if ( _humanBlock > block.number ) {
			...
			if ( block.number < _humanBlock + 20 ) { 
				if (block.number < _humanBlock + 10) { 
