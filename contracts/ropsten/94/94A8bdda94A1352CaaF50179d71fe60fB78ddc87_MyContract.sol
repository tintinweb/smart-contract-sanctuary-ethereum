...
contract  MyContract is Ownable{
  ...
  event CultivationEvent(address sender, uint amount, string uuid, uint256 tokenId);
  ...
  */
  function cultivation(uint amount,string memory uuid) public  {
    ...
    payment(amount);
    uint256 tokenId = ntfAddress.awardItem(msg.sender,amount == 100**18 ? tokenUrlV0 : tokenUrlV1);
    emit CultivationEvent(msg.sender, amount, uuid, tokenId);
  }
  ...
  * 直接创建 健康虎 银虎 金虎
  */
  function createHu(address to, uint _type) public onlyOwner {
    ...
    ntfAddress.awardItem(to,tokenUrl);
  }
