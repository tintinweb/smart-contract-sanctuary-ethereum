...
/**
 * Frixuu token contract. It implements the next capabilities:
 ...
 *    which enables accounts with no Ether on their balances to make token transfers and use Frixuu services.
 * 5. Token sale distribution rules.
 */                                                                                                                                                                                                     
contract FrixuuToken {
    ...
    function lastMint () external tokenDistributionPeriodOnly {
        ...
        uint256 remaining = totalSupply.mul(40).div(60); // Portion of tokens for Frixuu (40%)

        // To make the total supply rounded (no fractional part), subtract the fractional part from Frixuu's balance
