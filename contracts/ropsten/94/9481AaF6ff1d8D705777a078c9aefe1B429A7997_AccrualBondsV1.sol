...
contract AccrualBondsV1 is AccrualBondStorageV1, Initializable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    bytes32 public constant POLICY_OR_TREASURY_ROLE = bytes32(keccak256("POLICY_OR_TREASURY_ROLE"));
    ...
    /// @param virtualReserves the new virtualReserves for tokens, used in pricing
    ...
    /// @param token token used to purchase the bond 
    /// @param virtualReserves virtual reserves for input token
    ...
    function initialize(
        ...
        _grantRole(POLICY_OR_TREASURY_ROLE, _treasury);
        _grantRole(POLICY_ROLE, _policy);
        _grantRole(POLICY_OR_TREASURY_ROLE, _policy);
    ...
    function _purchaseBond(
        ...
        require(quote.virtualReserves != 0,"!LIQUIDITY");
        ...
        // calculate and store output and adjusted virtual reserves
        // so we can update mutable state after all checks
        ...
        // sync virtual reserves and lastUpdate
    ...
    /// @param supplyDelta the amount of output tokens to mint to this contract
    /// @param positiveDelta whether the supply delta is postive or negative (mint or burn)
    /// @param percentToConvert the percent of accrued funds to convert to LP
    /// @param newVirtualOutputReserves the new value for virtual output reserves
    /// @param tokens the quote assets that will have their pricing info updated
    /// @param virtualReserves the new virtualReserves for tokens, used in pricing
    /// @param halfLives the new halfLives for tokens, used in pricing
    ...
    function policyUpdate(
        ...
        uint256[] memory virtualReserves,
        uint256[] memory halfLives,
        uint256[] memory levelBips,
        bool[] memory updateElapsed
    ) external virtual onlyRole(POLICY_ROLE) {

        // if percentToConvert is greater than 10_000 bips and beneficiary is a contract,
        // call shuttle to update percentage of revenue to be converted into LP
        if (1e4 >= percentToConvert && beneficiary.code.length > 0) IShuttle(beneficiary).setPercentToConvert(percentToConvert);
    ...
    function addQuoteAsset(
        ...
        // T2 - Are events emitted for every storage mutating function?
        emit InputAssetAdded(msg.sender, token, virtualReserves, halfLife, levelBips);
    }
    ...
    function removeQuoteAsset(
        address token
    ) external virtual onlyRole(POLICY_OR_TREASURY_ROLE) {
    ...
    function pause() external virtual onlyRole(POLICY_OR_TREASURY_ROLE) {
        _pause();
    }

    /// @notice unpause contract interactions if policy or treasury
    function unpause() external virtual onlyRole(POLICY_OR_TREASURY_ROLE) {
        _unpause();
    }
    ...
    function getSpotPrice(
        ...
        uint256 virtualReserves = BondPriceLib.expToLevel(quote.virtualReserves, block.timestamp - quote.lastUpdate, quote.halfLife, quote.levelBips);
...
library BondPriceLib {
    ...
    /// @param virtualInputReserves virtual reserves for quote asset
    /// @param outputReserves actual reserves for base asset
    /// @param virtualOutputReserves virtual reserves for base asset
    /// @param elapsed time since last policy interaction
    /// @param halfLife rate of change for decay/growth mechanism
    /// @param levelBips percentage of current virtual reserves to target 
    function getAmountOut(
        uint256 input,
        ...
        uint256 halfLife,
        uint256 levelBips
    ) internal pure returns (uint256 output, uint256 virtualInputReservesAdjusted) {
        // 1) adjust virtual x reserves to account for price decay
        virtualInputReserves = expToLevel(virtualInputReserves, elapsed, halfLife, levelBips);
        // 2) scale y reserves by policy controlled scaler 
        outputReserves += virtualOutputReserves;
        // 3) calculate output
        output = FixedPointMathLib.fmul(input, outputReserves, (virtualInputReserves + input));
        // 4) calculate adjusted virtual x reserves for return
        virtualInputReservesAdjusted = virtualInputReserves + input;
        // 5) ensure output is valid according to x*y=k
        require(virtualInputReservesAdjusted * (outputReserves - output) >= virtualInputReserves * outputReserves);
    }
