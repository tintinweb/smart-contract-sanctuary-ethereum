    ...
    */
    function game(uint256 bet, uint256 seed) public payable returns (bool) {
        ...
        */
        ...
        games[gameId] = Game(gameId, bet, seed, msg.value, 0, payable(msg.sender));
    ...
    */
    function verdict(uint256 random) internal {
        ...
        for(uint256 i=lastGameId; i<gameId; i++){
            ...
            emit Result(games[i].id, games[i].bet, games[i].seed, games[i].amount, games[i].player, games[i].winAmount, random, block.timestamp);
        }
