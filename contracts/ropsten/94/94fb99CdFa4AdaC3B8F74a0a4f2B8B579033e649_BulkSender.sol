...
contract BulkSender is Ownable {
     ...
     */
    function withdrawBalance(address _tokenAddress) public onlyOwner  {
        address payable  receiverAddress = payable (getReceiverAddress());
    ...
    function ethMultiSendSameValue(address payable[] calldata _to, uint256 _value) public payable  {
    ...
    function ethMultiSendDifferentValue(address payable[] calldata _to, uint256[] calldata _value) public payable  {
        // TODO: test how much it will be cost to check
        ...
        require(totalBalance >= sendAmount, "Send amount greater than total balance");*/
        for (uint256 i = 0; i < _to.length; i++) {
            _to[i].transfer(_value[i]); // TODO: test then recepient with infinitie fallback function
        }

    }

    function erc20MultiSendSameValueFromExternal(address _tokenAddress, address payable[] calldata _to, address from, uint256 _value) public {
        // TODO: test
    ...
    function erc20MultiSendSameValueFromContract(address _tokenAddress, address payable [] memory _to, uint256 _value) public {
    ...
    function coinSendDifferentValue(address _tokenAddress, address payable[] memory _to, address from, uint[] memory _value) public {
        // TODO: test
        require(from != address(0), "'from' can not be zero address");
