...
library LibLazyBidERC721 {

    bytes32 constant BID_TYPEHASH = keccak256(
        "Bid(address maker,address taker,address token,address auctioneer,uint256 tokenId,uint256 value,uint256 start,uint256 end,bytes tokenData)"
    );
    struct Bid {
        address maker;
        address taker;
...
library LibLazyAuctionERC721 {

    bytes32 constant AUCTION_TYPEHASH = keccak256(
        "Auction(address maker,address taker,address token,uint256 tokenId,uint256 min,uint256 start,uint256 end)"
    );
...
interface IAuctioneerUpgradeable {
    function getFee(address _token, address _minter, address _signer) external view returns (uint96);
}
