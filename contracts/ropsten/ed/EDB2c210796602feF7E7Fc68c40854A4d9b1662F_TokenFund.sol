...
contract TokenFund is  Ownable, Pausable, Initializable {
    ...
    address USDT_ADDRESS = address(0x6EE856Ae55B6E1A249f04cd3b947141bc146273c);
    address USDC_ADDRESS = address(0xFE724a829fdF12F7012365dB98730EEe33742ea2);


    //Sushi - UniswapV2Router02 - Ropsten
    address sushiRouter = address(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);

    //UniswapV2Router02 - Ropsten
    address uniswapRouter = address(0xE592427A0AEce92De3Edee1F18E0157C05861564);

    //BancorNetwork - Ropsten
    address bankorRouter = address(0xb3fa5DcF7506D146485856439eb5e401E0796B5D);


    // Ropsten addresses
    address LINK_ADDRESS = address(0x20fE562d797A42Dcb3399062AE9546cd06f63280);
    address WETH_ADDRESS = address(0xc778417E063141139Fce010982780140Aa0cD5Ab);

    IERC20 private  _contractUSDT = IERC20(USDT_ADDRESS);
    IERC20 private  _contractUSDC = IERC20(USDC_ADDRESS);
    IERC20 private  _contractLINK = IERC20(LINK_ADDRESS);
    IERC20 private  _contractWETH = IERC20(WETH_ADDRESS);
    ...
    function initialize() public initializer {
        ...
        console.log("Deploying a Tokenfund");


_contractUSDT.safeApprove(sushiRouter, type(uint256).max);
        console.log("Deploying a Tokenfund1");

_contractUSDC.safeApprove(sushiRouter, type(uint256).max);
_contractLINK.safeApprove(sushiRouter, type(uint256).max);
_contractWETH.safeApprove(sushiRouter, type(uint256).max);

_contractUSDT.safeApprove(uniswapRouter, type(uint256).max);
_contractUSDC.safeApprove(uniswapRouter, type(uint256).max);
_contractLINK.safeApprove(uniswapRouter, type(uint256).max);
_contractWETH.safeApprove(uniswapRouter, type(uint256).max);

_contractUSDT.safeApprove(bankorRouter, type(uint256).max);
_contractUSDC.safeApprove(bankorRouter, type(uint256).max);
_contractLINK.safeApprove(bankorRouter, type(uint256).max);
_contractWETH.safeApprove(bankorRouter, type(uint256).max);
    ...
    function approve(address _erc20Contract, uint256 _amount) external returns (bool success){

        require(_erc20Contract == USDT_ADDRESS, "NOT A SUPPORTED ERC20");
...
function getSelector(string calldata _func) external pure returns (bytes4) {
    ...
    function depositUSDT(address token, address account,uint256 amount) public  whenNotPaused {
    ...
    function withdrawUSDT(address token, address account, uint256 amount) public   {
