...
contract ERC20 {
    ...
    //the owner address of this contract. The owner can mint more tokens and add global hold operators
    ...
    // the hold operators of each address. Address(0) holds the global operators.
   ...
   modifier onlyOwner {
     ...
     * i.e. what address can mint new tokens and what address can create global hold operators.
   ...
   /**
    ...
    /**
     * @dev Changes the contract owner account. The contract owner can mint new tokens and change the global hold operators
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
     * Requirements:
    ...
    /**
     * @dev authorises `holdOperator' to receive holds from any account.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
     ...
     * Requirements
     *
     * - `holdOperator` cannot be the empty address.
     * - the caller must be the contract owner.
     */
    function authorizeGlobalHoldOperator(address holdOperator) onlyOwner() public returns (bool){
        
        require(holdOperator != address(0), "Authorising address(0)");
        
        _holdOperators[address(0)][holdOperator] = true;

        emit AuthorizedHoldOperator(holdOperator, address(0));

        return true;
    }

    /**
     * @dev revokes `holdOperator' from receiving holds from any account.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
     * Emits a {AuthorizedHoldOperator} event
     ...
     * - the caller must be the contract owner.
     */
    function revokeGlobalHoldOperator(address holdOperator) onlyOwner() public returns (bool){

        delete _holdOperators[address(0)][holdOperator];

        emit RevokedHoldOperator(holdOperator, address(0));
    ...
    function authorizeHoldOperator(address account, address holdOperator) public returns (bool){
        ...
        require(account != address(0), "Authorising for address(0)");
    ...
    function isHoldOperatorFor(address holdOperator, address account) public view returns (bool){
        return
            _holdOperators[address(0)][holdOperator] || //checks for global hold operator
            _holdOperators[account][holdOperator];      //checks for specific account hold operator
    }
