...
contract D4AProtocol is Initializable, ReentrancyGuardUpgradeable{
  ...
  mapping (uint256 => bytes32) public tokenid_2_canvas;
  ...
  //event from library
  event NewProject(bytes32 project_id, string uri, address fee_pool, address erc20_token, address erc721_token);
  ...
  function createProject(uint256 _start_prb,
    ...
    return all_projects.createProject(settings, _start_prb, _mintable_rounds, _floor_price_rank, _project_uri);
  }
  ...
  function getProjectInfo(bytes32 _project_id) public view returns(uint256 start_prb, uint256 mintable_rounds, uint256 floor_price_rank,
                                                                   address fee_pool, string memory uri, uint256 erc20_total_supply){
  ...
  function createCanvas(bytes32 _project_id, string memory _canvas_uri) public payable nonReentrant returns(bytes32 canvas_id){
    ...
    canvas_id = all_canvases.createCanvas(settings, all_projects[_project_id].fee_pool, _project_id, all_projects[_project_id].start_prb, _canvas_uri);
    all_projects[_project_id].canvases.push(canvas_id);
    //We consider create canvas as an activity which may affect price
    all_prices.updateCanvasPrice(settings, _project_id, canvas_id, 1);
  }
  ...
  function getCanvasNextPrice(bytes32 _canvas_id) public view returns(uint256){
    ...
    return all_prices.getCanvasNextPrice(settings, pi.floor_prices, pi.floor_price_rank, project_id, _canvas_id);
  }
  ...
  function mintNFT(bytes32 _canvas_id, string memory _token_uri) public payable nonReentrant returns(uint256 token_id){
    ...
    uint256 price = 0;
    {
      price = all_prices.getCanvasNextPrice(settings, all_projects[proj_id].floor_prices, all_projects[proj_id].floor_price_rank, proj_id, _canvas_id);
      ...
      bool succ;
      if(m != 0){
        (succ, ) = settings.owner_proxy().ownerOf(proj_id).call{value:m}("");
      ...
      all_rewards.updateRewardForCanvas(settings, proj_id, _canvas_id, all_projects[proj_id].start_prb, all_projects[proj_id].mintable_rounds);
    }
    ...
    tokenid_2_canvas[token_id] = _canvas_id;
    emit D4AMintNFT(proj_id, _canvas_id, token_id, _token_uri, price);
  }

  function getNFTTokenCanvas(uint256 token_id) public view returns(bytes32){
    return tokenid_2_canvas[token_id];
  }
  ...
  function claimProjectERC20Reward(bytes32 _project_id) public returns(uint256){
    ...
    return all_rewards.claimProjectReward(settings, _project_id, pi.erc20_token, pi.start_prb, pi.mintable_rounds);
  }
  ...
  function claimCanvasReward(bytes32 _canvas_id) public returns(uint256){
    ...
    return all_rewards.claimCanvasReward(settings, project_id, _canvas_id, pi.erc20_token, pi.start_prb, pi.mintable_rounds);
  }
}
...
library D4AReward{
  ...
  function issueTokenToCurrentRound(mapping(bytes32=>reward_info) storage all_rewards,
                                   ...
                                   bytes32 _project_id, address erc20_token, uint256 _start_round, uint256 total_rounds) internal returns(uint256){
  ...
  function claimCanvasReward(mapping(bytes32=>reward_info) storage all_rewards,
                             ID4ASetting _settings,
                             bytes32 _project_id, bytes32 _canvas_id, address _erc20_token, uint256 _start_round, uint256 _total_rounds) internal returns(uint256){
    uint256 cur_round;
    {
      ID4APRB prb = _settings.PRB();
    ...
    uint256 total_amount = 0;
    {
      ...
      for(uint256 i = ri.canvas_2_to_claim_round_index[_canvas_id]; i <= ri.final_issued_round_index; i++){
        ...
        total_amount += tk * ri.canvas_2_block_2_amount[_canvas_id][ri.active_rounds[i]]/ri.round_2_total_amount[ri.active_rounds[i]] ;
  ...
  function claimProjectReward(mapping(bytes32=>reward_info) storage all_rewards,
                             ID4ASetting _settings,
                             bytes32 _project_id, address erc20_token, uint256 _start_round, uint256 _total_rounds) internal returns(uint256){
    reward_info storage ri = all_rewards[_project_id];
    if(ri.active_rounds.length == 0){
      return 0;
    }
...
library D4AProject{
  ...
  function createProject(mapping(bytes32=>project_info) storage all_projects,
    ...
    if(all_projects[project_id].exist) revert D4AProjectAlreadyExist(project_id);
    {
      ...
      pi.erc20_token = _createERC20Token(_settings, project_id);
      address pool= _settings.feepool_factory().createD4AFeePool(string(abi.encodePacked("Asset Pool for DAO4Art Project ", toHex(project_id))));
      ...
      pi.erc721_token = _createERC721Token(_settings, project_id);
      D4AERC721(pi.erc721_token).grantRole(keccak256("ROYALTY"), address(this));
      D4AERC721(pi.erc721_token).setRoyaltyInfo(pi.fee_pool, 500);
      ...
      pi.exist = true;
      emit NewProject(project_id, _project_uri, pool, pi.erc20_token, pi.erc721_token);
    }
  }
  ...
  function _createERC20Token(ID4ASetting _settings, bytes32 _project_id) internal returns(address){
    string memory project_hex = toHex(_project_id);
    string memory name = string(abi.encodePacked("D4A Token for ", project_hex));
    string memory sym = string(abi.encodePacked("[email protected]", subString(project_hex, 0, 8)));
    return _settings.erc20_factory().createD4AERC20(name, sym, address(this));
  }

  function _createERC721Token(ID4ASetting _settings, bytes32 _project_id) internal returns(address){
    string memory project_hex = toHex(_project_id);
    string memory name = string(abi.encodePacked("D4A NFT for ", project_hex));
    string memory sym = string(abi.encodePacked("D4A [email protected]", subString(project_hex, 0, 8)));
...
library D4APrice {
  ...
  function getCanvasLastPrice(mapping(bytes32=>project_price_info) storage all_prices,
                              bytes32 _project_id, bytes32 _canvas_id) internal view returns(uint256 round, uint256 value){
  ...
  function getCanvasNextPrice(mapping(bytes32=>project_price_info) storage all_prices,
                              ID4ASetting _settings,
                              uint256[] memory price_slots, uint256 price_rank,
    ...
    if(ppi.max_price.round == cur_round){
      return floor_price;
    }
    if(price_rank > 0){
      return price_slots[price_rank - 1];
    }
    return price_slots[price_rank];
  }
...
contract D4AERC721 is Initializable, ERC721URIStorageUpgradeable, AccessControlUpgradeable, ERC721RoyaltyUpgradeable{
  ...
  function supportsInterface(bytes4 interfaceId) public view override(ERC721Upgradeable, AccessControlUpgradeable, ERC721RoyaltyUpgradeable) returns(bool){
