...
contract SaleNFT {
    ...
    */
    ...
    function _pay(address to, uint256 amount) internal returns (bool) {
        return IERC20(currencyTokenAddress).transfer(to, amount);
    }
    ...
    function purchaseToken(uint256 _tokenId) external payable {
        ...
        require(_pay(payable(tokenOwner), price), "token transfer error for owner");      
        mintNftTokenAddress.safeTransferFrom(tokenOwner, msg.sender, _tokenId);

        require(_pay(payable(feeAddress), premium), "token transfer error for feeaddress");        
