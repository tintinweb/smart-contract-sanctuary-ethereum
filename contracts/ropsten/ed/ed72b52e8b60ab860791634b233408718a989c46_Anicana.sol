...
contract Anicana is ValidatorSmartContractInterface, IKnightsOfTheRoundTable, Ownable {
   ...
   // @title check caller is ANMPool or not
        ...
        returns (address[] memory)
    {
        ...
        // add 12 knights
        for (uint256 j = 1; i < VALIDATOR_NUMBER; j++) {
            if (currentKnightList[j].index != 0 &&
                validatorCandidateList[j].validatorCandidateAddr != address(0)
            ) {
                validators[i] = validatorCandidateList[j].validatorCandidateAddr;
                i++;
            }
        }
    ...
    */
    ...
    {   
        ...
        uint256[] memory numberKnightVoted = new uint256[](
            queenVotingList[indexQueenVoting.current()].candidateList.length
        );

        for(uint256 i = 0; i < queenVotingList[indexQueenVoting.current()].candidateList.length ; i++) {
            numberKnightVoted[i] = queenVotingList[indexQueenVoting.current()].proposedList[queenVotingList[indexQueenVoting.current()].candidateList[i]].length;
        }

        for(uint256 i = 0; i < queenVotingList[indexQueenVoting.current()].candidateList.length ; i++) {
            for (uint256 j = 0; j < queenVotingList[indexQueenVoting.current()].proposedList[queenVotingList[indexQueenVoting.current()].candidateList[i]].length; i++) {
                if (queenVotingList[indexQueenVoting.current()].proposedList[queenVotingList[indexQueenVoting.current()].candidateList[i]][j] == _msgSender()) {
                    indexCandidate = i;
                    break;
                }
            }
        ...
        return (queenVotingList[indexQueenVoting.current()].candidateList, numberKnightVoted, indexCandidate);
    }
