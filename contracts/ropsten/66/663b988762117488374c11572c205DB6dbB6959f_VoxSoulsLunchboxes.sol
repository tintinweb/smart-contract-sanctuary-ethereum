    ...
    Signing
{
    ...
    uint256 public maxSaleLunchBoxes;
    ...
    function setMaxLunchboxes(uint256 _maxMint) public onlyOwner {
        maxLunchboxes = _maxMint;
    }

    function setMaxSaleLunchBoxes(uint256 _numSale) public onlyOwner {
        maxSaleLunchBoxes = _numSale;
    }
    ...
    function getCurrCost(uint256 count) public view returns (uint256) {
        ...
        if (!allowListEvent) {
            for (uint256 i = 0; i < count; i++) {
                if (currentSaleCount + i > maxLunchboxes) {
                    //5000
                    total += .1 ether;
                }
            }
