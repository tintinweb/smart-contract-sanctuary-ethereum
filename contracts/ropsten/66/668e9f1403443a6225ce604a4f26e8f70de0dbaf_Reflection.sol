...
contract DividendDistributor is IDividendDistributor {
    ...
    address public immutable APE = address(0x4d224452801ACEd8B2F0aebE155379bb5D594381); //APE
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);
            IERC20(APE).transfer(shareholder, amount);
        }
    }
    ...
    function getDividendsClaimedOf (address shareholder) external view returns (uint256) {
        require (shares[shareholder].amount > 0, "You're not a CultEarn shareholder!");
        return shares[shareholder].totalClaimed;
    }

    }

contract Reflection is IERC20, Auth {
    using SafeMath for uint256;
    ...
    string private constant  _name = "CultEarn";
    string private constant _symbol = "CultEarn";
    ...
    address payable public marketingWallet = payable(0xE41Aa9778CeC1bAB2CdF193C21bE20B21884B16c);
    ...
    function swapBack() internal swapping {
        ...
        swapTokensForAPE(amountToSwap.div(2));

        uint256 dividends = IERC20(APE).balanceOf(address(this));

        bool success = IERC20(APE).transfer(address(distributor), dividends);
