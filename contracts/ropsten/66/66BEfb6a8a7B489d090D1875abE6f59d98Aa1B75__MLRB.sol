...
contract _MLRB is IERC20, Ownable {
    ...
    uint public tradingEnabledAtBlock = 0; 
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if(!isTradingEnabled || tradingEnabledAtBlock == 0){
            if(_msgSender() != owner() || _msgSender() == address(uniswapV2Pair) || _msgSender() == address(uniswapV2Router) || address(from) == address(uniswapV2Router) || address(from) == address(uniswapV2Pair)){
                // allow pair to receive
