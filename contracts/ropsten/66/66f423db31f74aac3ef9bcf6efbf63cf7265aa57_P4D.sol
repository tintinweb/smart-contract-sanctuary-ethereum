    ...
    */
    ...
    {   
        ...
        if (contractBalance > lastContractBalance_ && totalSupply() != 0) {
            ...
            uint256 devFee = SafeMath.div(additionalDivsFromP3D, 1000); // 0.1% of the divs are for the dev
            divsMap_[_dev].balance = SafeMath.add(divsMap_[_dev].balance, devFee);

            // the remaining amount is spread proportionally over all token P4D token holders
        ...
        returns(uint256)
    {
        ...
        // set up the referral for the P3D purchased by this transaction
        // it will default to the dev but if _referredBy has the required P3D balance to be a referrer, it will use that address
        address buyRef = _dev;
        if (
            ...
            // does the referrer have at least X whole P3D tokens?
