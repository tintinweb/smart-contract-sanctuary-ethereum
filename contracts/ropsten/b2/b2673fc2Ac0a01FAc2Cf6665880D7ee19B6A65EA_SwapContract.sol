...
import {ECDSAOffsetRecovery} from "./ECDSAOffsetRecovery.sol";
    ...
    ECDSAOffsetRecovery
{
    ...
    // TODO CHECK uint32 that can be cheaper
    uint128 public minConfirmationBlocks;
    uint128 public numOfThisBlockchain;
    uint256 public maxGasPrice;
    uint256 public minConfirmationSignatures;
    // TODO check that immutable should be cheaper
    ...
    modifier onlyValidToken(uint128 _tokenNum) {
        require(numsTokens.contains(_tokenNum), "swapContract: invalid token");
        _;
    }

    /**
     * @dev Constructor of contract
     * @param _tokenAddresses Address of token contract (initial one)
     * @param _tokenNums index of a token contract (initial one)
     * @param _minTokenAmounts Minimal amount of tokens required for token swap
    ...
    constructor(
        ...
        require(
            _tokenAddresses.length == _tokenNums.length,
            "swapContract: Wrong array length"
        );
        uint128 length = uint128(_tokenAddresses.length);
        // TODO check performance
        for (uint256 i = 0; i < length; i++) {
            uint128 currentTokenNum = _tokenNums[i];
            address currentTokenAddress = _tokenAddresses[i];
            uint256 currentMinTokenAmount = _minTokenAmounts[i];

            require(
                currentTokenAddress != address(0) &&
                    !numsTokens.contains(currentTokenNum) &&
                    currentMinTokenAmount > 0,
                "swapContract: Wrong token data"
            );

            minTokenAmount[currentTokenNum] = currentMinTokenAmount;
            numsTokens.set(currentTokenNum, currentTokenAddress);
        }
    ...
    /**
     * @dev Returns length of the token data enumarable map
     */
    function getNumsTokensLength() public view returns (uint256) {
        return numsTokens.length();
    }

    /**
     * @dev Returns address of token by number
     * @param tokenNum Number of token
     * @return Address Token address
     */
    function getTokenByNum(uint128 tokenNum) public view returns (address) {
        require(
            tokenNum < numsTokens.length(),
            "swapContract: invalid tokenNum"
        );
        return numsTokens.get(tokenNum);
    }
    ...
    function transferToOtherBlockchain(
    ...
    ) external payable virtual whenNotPaused onlyValidToken(token) {
        address tokenAddress = numsTokens.get(token);
        _checkTransferFrom(blockchain, token, amount, newAddress);
        // TODO check gas for next string
        ...
        if (tokenAddress == 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE) {
            require(
                amount == msg.value,
                "swapContract: Wrong amount or msg.value"
            );
        } else {
    ...
    //  * User calls this function when he wants to transfer tokens to another blockchain.
    //  * @param token Token to transfer, address should be 0xEeeee...eE
    //  * @param blockchain Number of blockchain
    //  * @param newAddress Address in the blockchain to which the user wants to transfer
    //  */
    // function transferNativeToOtherBlockchain(
    //     uint128 token,
    //     uint128 blockchain,
    //     string memory newAddress
    // ) external payable whenNotPaused onlyValidToken(token) {
    //     address tokenAddress = numsTokens.get(token);
    //     require(
    //         tokenAddress == 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE,
    //         "swapContract: Token isn't native"
    //     );
    //     // TODO check gas for next string
    ...
    function _checkTransferFrom(
        ...
        require(
            amount >= minTokenAmount[tokenNum],
        ...
        require(
            amount >= feeAmountOfBlockchain[blockchain][tokenNum],
    ...
    function transferToUserWithFee(
    ...
    ) external virtual onlyRelayer whenNotPaused onlyValidToken(token) {
        _checkTransferTo(
            token,
            user,
            amountWithFee,
        ...
        if (tokenAddress == 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE) {
            ...
            TransferHelper.safeTransfer(tokenAddress, user, amountWithoutFee);
            TransferHelper.safeTransfer(tokenAddress, feeAddress, fee);
        }
    ...
    function _checkTransferTo(
        ...
        // TODO check constant call gas
        uint256 signatureLength = SIGNATURE_LENGTH;
        // TODO check this performance too
        uint256 concatLength = concatSignatures.length;
        require(
            concatLength % signatureLength == 0,
            "swapContract: Signatures lengths must be divisible by 65"
        );
        uint256 signaturesCount; /* = concatLength / signatureLength;*/
        assembly {
            signaturesCount := div(concatLength, signatureLength)
        }
        require(
            signaturesCount >= minConfirmationSignatures,
     ...
     * @param token for token address, or 0x0000000000000000 for native token
     * @param _minTokenAmount Amount of tokens
     */
    function setMinTokenAmount(uint128 token, uint256 _minTokenAmount)
        external
        onlyOwnerAndManager
    {
    ...
    function setMinConfirmationBlocks(uint128 _minConfirmationBlocks)
        external
        onlyOwnerAndManager
    {
        minConfirmationBlocks = _minConfirmationBlocks;
