...
contract DBSNFTTokens is ERC721Tradable{
    ...
    function initProperty(uint[] memory _ids, string[] memory _images, string[] memory _des) public onlyOwner {
...
abstract contract ERC721Tradable is ERC721Burnable, AccessControl, Ownable {
    ...
    */
    ...
    modifier onlyMinter() {
        require(hasRole(DEFAULT_ADMIN_ROLE,_msgSender()), "MinterRole: caller does not have the Minter role");
        _;
    }
    ...
    function mint(
        ...
        uint256 _id,
        uint256 _amount
    ) virtual public onlyRole(MINTER_ROLE) returns(uint256[] memory){
    ...
    function batchMint(
        ...
        uint256 counts = 0;
        for (uint256 i = 0; i < _ids.length; i++) {
            counts = counts.add(_amounts[i]);
        }
        uint256[] memory alltokens = new uint256[](counts);
        uint256[] memory allids = new uint256[](counts);
        counts = 0;
        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 _id = _ids[i];
            uint256 amount = _amounts[i];
            uint256[] memory got = mint(_to, _id, amount);
            for (uint256 j = 0; j < got.length; j++) {
                alltokens[counts] = got[j];
                allids[counts] = _id;
                counts++;
            }
        }
    ...
    /**
        ...
        * @param _ids         Array of ids to mint
        */
    function singleBatchMint(
        address _to,
        uint256[] memory _ids
    ) public onlyRole(MINTER_ROLE) returns(uint256[] memory){
        uint256[] memory all = new uint256[](_ids.length);
        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 currentTokenId = _nextTokenId.current();
            _nextTokenId.increment();
            _mint(_to, currentTokenId);
            tokens[currentTokenId] = _ids[i];
            all[i] = currentTokenId;
        }
        return all;
    }
