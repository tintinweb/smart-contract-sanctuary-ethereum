 ...
 */
interface IBalance {
    function balanceERC721( address holderWallet ) external view returns (uint);
    function balanceERC1155( address holderWallet, uint id ) external view returns (uint);
}
...
contract DankbotsArtDrops is ERC1155, Ownable {
	...
	function setDiscount( uint _id, uint256 _discount ) 
	external 
	onlyOwner {
		discount[ _id ] = _discount;
		emit discountEvent( _id, _discount );
	}
	...
	payable {
		...
		if ( msg.sender != owner() ) {
			require( balanceOf( msg.sender, _id ) == 0, "Address has already claimed!" );
	...
	payable {
		...
		if ( msg.sender != owner() ) {
			require( balanceOf( msg.sender, _id ) == 0, "Address has already claimed!" );
			require( IBalance( dankbots_address ).balanceERC721( msg.sender ) > 0, "Must have a DANKBOT!" );
			require( IBalance( founders_pass_address ).balanceERC1155( msg.sender, _foundersPassId ) > 0, "Must have DANKBOTS FOUNDERS PASS!" );
