...
abstract contract NFTCore is ERC721, SafeOwnableInterface {
...
contract GenesisNFT is SafeOwnable, NFTCore, Mintable {
    ...
    uint public constant MAX_MINT_NUM = 2000;
    uint public constant MAX_RESERVE_NUM = 500;
    ...
    function draw(uint _luckyNftId, uint8 _v, bytes32 _r, bytes32 _s) external onlyMinterOrMinterSignature(keccak256(abi.encodePacked(address(this), msg.sender, _luckyNftId)), _v, _r, _s) {
        ticketNFT.burn(msg.sender, _luckyNftId);
        unchecked {
            require(mintedNum < MAX_MINT_NUM, "mint already full");
