...
contract KungfuFighting is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    constructor() ERC721A("KungfuFighting", "KFFG") {}

    function _baseURI() internal view virtual override returns (string memory) {
        return baseURI;
    }

    function mintGiveawayNFT(uint256 _count) public onlyOwner {
        require(GIVEAWAY_MINTED + SALE_MINTED <= MAX_NFT, "Max limit");
        ...
        returns (string memory)
    {
        ...
        if (bytes(currentBaseURI).length > 0) {
            ...
            return
                "https://ipfs.io/ipfs/QmdBjX7u1H8TT5QmZvKQFgBijsGxQ566UiRVjUUXPTNaw2";
        }
    }
    ...
    function updateMaxSupply(uint256 newSupply) external onlyOwner {
        require(newSupply >= GIVEAWAY_MINTED + SALE_MINTED, "Incorrect value");
        MAX_NFT = newSupply;
    }

    function updateMintLimitPerTransaction(uint256 newLimit)
        external
        onlyOwner
    {
