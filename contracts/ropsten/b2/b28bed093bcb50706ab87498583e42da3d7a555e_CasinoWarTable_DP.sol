...
contract CasinoWarTable_DP{
    ...
    event CardDealtEvent(address _player, uint _cardNumber);
    ...
    function dealCards() internal{
        ...
        for (uint i=0; i<players.length; i++){
            playerCards[players[i]] = pickRandomCard();
            // emit the event
            emit CardDealtEvent(players[i], playerCards[players[i]]);
        }

        dealerCard = pickRandomCard();
        // emit the event
        emit CardDealtEvent(dealer, dealerCard);
    }
    ...
    function pickRandomCard() internal returns(uint){
        uint randomNumber;
        randomNumber = uint(keccak256(abi.encodePacked(initialNumberSeed++)));
        return (randomNumber % 13) + 1 ; //Get a random number between 1 and 13 (both incl.) - similar to 13 cards in a deck
    }
}