...
contract CryptoMillionsCrowdsale {
    ...
    function getCountTransactions(address _to) view public returns (uint256) {
        ...
        if( countTransactions[_to] > 0 ){
            xTransaction = countTransactions[_to];
        }
        return xTransaction;
    }
	...
	function getTransaction(address _to , uint256 _id) constant public returns(address sender, uint256 date, uint256 eth, uint256 tokens, uint256 bonus, uint256 total, string coin, string hash){
