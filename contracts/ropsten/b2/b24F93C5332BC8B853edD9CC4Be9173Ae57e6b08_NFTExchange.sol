...
contract NFTExchange is Initializable, OwnableUpgradeable, EIP712Upgradeable {
  ...
  function buy(Ask calldata ask)
    ...
    returns (address)
  {
    require(ask.tokenType == 721 || ask.tokenType == 1155, "NFTExchange: invalid nft type");
  ...
  // * Bid 해시 값
  function getBidHash(Bid calldata bid) virtual external returns(bytes32) {
    return _bidHash(bid);
  }

  // * Bid 조회
  function getBid(bytes32 bidHash) virtual external returns(Bid memory) {
  ...
  function deleteBid(bytes32 bidHash) virtual external {
    ...
    require(bid.bidder != address(0), "NFTExchange: not founded");
