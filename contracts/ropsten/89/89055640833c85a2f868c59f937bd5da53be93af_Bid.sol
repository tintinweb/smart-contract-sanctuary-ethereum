...
contract Bid is ERC2771Context, Ownable {
    struct BidOrder {
        bytes cid;
        address owner;
        uint256 amount;
    ...
    constructor(address _dpdRepository, address _forwarder) {
    ...
    function withdraw() external onlyOwner {
        (bool sent, ) = address(_msgSender()).call{value: address(this).balance}(
            ""
        );
