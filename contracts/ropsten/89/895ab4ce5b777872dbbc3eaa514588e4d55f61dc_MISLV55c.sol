...
contract MISLV55c is Context, IERC20, Ownable {
    ...
    struct RValuesStruct {
        ...
        uint256 rBurnFee;
        uint256 rMarketingAndTeamTokenFee;
    ...
    struct TValuesStruct {
        ...
        uint256 tBurnFee;
        uint256 tMarketingAndTeamTokenFee;
    ...
    string private _name = "MISSL55CNOSL";
    string private _symbol = "MISSL55CNOSL";
    ...
    uint256 public _marketingAndTeamTokenFee = 1;
    uint256 private _previousMarketingAndTeamTokenFee = _marketingAndTeamTokenFee;

    uint256 public _marketingETHFee = 1;
    uint256 private _previousMarketingETHFee = _marketingETHFee;

    address public marketingAndTeamFeeWallet = 0x641Daa2e3B73cD217FF02a5EEbC004b7FC456D41;
    ...
    bool public _marketingTransferEnabled = true;
    ...
    function marketingTransferEnabled() public view returns (bool) {
        return _marketingTransferEnabled;
    }
    ...
    function _distributeFee(uint256 rReflectionFee, uint256 rBurnFee, uint256 rMarketingAndTeamTokenFee, uint256 tReflectionFee, uint256 tBurnFee, uint256 tMarketingAndTeamTokenFee) private {
        ...
        _rOwned[marketingAndTeamFeeWallet] = _rOwned[marketingAndTeamFeeWallet].add(rMarketingAndTeamTokenFee);
        if (_isExcluded[marketingAndTeamFeeWallet]) {
            _tOwned[marketingAndTeamFeeWallet] = _tOwned[marketingAndTeamFeeWallet].add(tMarketingAndTeamTokenFee);
        }
    }

    function _getValues(uint256 tAmount) private view returns (ValuesStruct memory) {
        TValuesStruct memory tvs = _getTValues(tAmount);
        RValuesStruct memory rvs = _getRValues(tAmount, tvs.tReflectionFee, tvs.tBurnFee, tvs.tMarketingAndTeamTokenFee, tvs.tMarketingETHFee, _getRate());
        ...
        return ValuesStruct(
            ...
            rvs.rReflectionFee,
            rvs.rBurnFee,
            rvs.rMarketingAndTeamTokenFee,
            rvs.rMarketingETHFee,
            tvs.tTransferAmount,
            tvs.tReflectionFee,
            tvs.tBurnFee,
            tvs.tMarketingAndTeamTokenFee,
    ...
    function _getTValues(uint256 tAmount) private view returns (TValuesStruct memory) {
        ...
        uint256 tMarketingAndTeamTokenFee = calculateMarketingAndTeamTokenFee(tAmount);
        uint256 tMarketingETHFee = calculateMarketingETHFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tReflectionFee).sub(tBurnFee).sub(tMarketingAndTeamTokenFee).sub(tMarketingETHFee);
        return TValuesStruct(tTransferAmount, tReflectionFee, tBurnFee, tMarketingAndTeamTokenFee, tMarketingETHFee);
    }

    function _getRValues(uint256 tAmount, uint256 tReflectionFee, uint256 tBurnFee, uint256 tMarketingAndTeamTokenFee, uint256 tMarketingETHFee, uint256 currentRate) private pure returns (RValuesStruct memory) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rReflectionFee = tReflectionFee.mul(currentRate);
        uint256 rBurnFee = tBurnFee.mul(currentRate);
        uint256 rMarketingAndTeamTokenFee = tMarketingAndTeamTokenFee.mul(currentRate);
        uint256 rMarketingETHFee = tMarketingETHFee.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rReflectionFee).sub(rMarketingETHFee).sub(rBurnFee).sub(rMarketingAndTeamTokenFee);
        return RValuesStruct(rAmount, rTransferAmount, rReflectionFee, rBurnFee, rMarketingAndTeamTokenFee, rMarketingETHFee);
    }
    ...
    function calculateMarketingAndTeamTokenFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_marketingAndTeamTokenFee).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        ...
        _marketingETHFee = 0;
        _burnFee      = 0;
        _marketingAndTeamTokenFee = 0;
    }

    function restoreAllFee() private {
        _reflectionFee       = 1;
        _marketingETHFee = 1;
        _marketingAndTeamTokenFee = 1;
        _burnFee      = 1;
    ...
    function _transfer(
        ...
        if (
            ...
            swapMarketingTokentoETH(contractTokenBalance);
        }
    ...
    function swapMarketingTokentoETH(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if(_marketingTransferEnabled) {
            payable(marketingAndTeamFeeWallet).transfer(address(this).balance.sub(initialBalance));
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        _distributeFee(vs.rReflectionFee, vs.rBurnFee, vs.rMarketingAndTeamTokenFee, vs.tReflectionFee, vs.tBurnFee, vs.tMarketingAndTeamTokenFee);
    ...
    function enableAllFees() external onlyOwner() {
        ...
        _marketingAndTeamTokenFee = 1;
        _previousMarketingAndTeamTokenFee = _marketingAndTeamTokenFee;
        _marketingETHFee = 1;
    ...
    function setMarketingTransfer(bool _enabled) public onlyOwner {
        _marketingTransferEnabled = _enabled;
    }
