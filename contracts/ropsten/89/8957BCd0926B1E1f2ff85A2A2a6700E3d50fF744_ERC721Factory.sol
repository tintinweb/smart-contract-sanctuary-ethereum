...
contract ERC721Factory is Administrated {
    ...
    function createRedKiteERC721(
        ...
        returns (address)
    {
        RedKiteERC721 _new = new RedKiteERC721(
            _name,
            _symbol,
            owner(),
            splitter,
        ...
        emit CreatedERC721(address(_new), _name, _symbol);
        return address(_new);
    }
}
    ...
    Administrated
{
    ...
    function presaleBuy(
        ...
        require(msg.value == presalePrice, "incorrect price");

        // Signature verification
        bytes32 _msgHash = keccak256(abi.encode(msg.sender, _UUID, _tokenURI));
    ...
    function publicBuy(
        ...
        _mint(msg.sender, _tokenId);
        _setTokenURI(_tokenId, _tokenURI);
        emit Buy(1, msg.sender);
    }

    /**
     * @notice Public buy with WETH
     */
    function publicBuyWETH(
        bytes calldata _signature,
        bytes calldata _UUID,
        string memory _tokenURI
    ) external returns (uint256 _tokenId){
        require(saleStarted, "presale is not started");
        IERC20(WETH).safeTransferFrom(
            msg.sender,
            splitter,
            price
