...
contract Ownable {
   ...
   */
  function operatorManager(address operator,uint8 flag) 
    public 
    onlyOwner 
    returns(bool){
      if(flag == uint8(0)){
...
/**
 *  预测事件合约对象 
 *  @author linq <<a href=/cdn-cgi/l/email-protection class=__cf_email__ data-cfemail=764746474e4643454740403607075815191b>[email&#160;protected]</a>>
 */
contract GuessBaseBiz is Pausable {
    
  // MOS合约地址 
  address public MOSContrctAddress = 0xbC668E5c79992Cc26C9B979dC10F397C3C816067;
  // 平台地址
  ...
  constructor() public {
      MOS = ERC20Token(MOSContrctAddress);
  }
  ...
  function disabled(uint256 id) internal view returns(bool) {
 ...
 /**
  ...
  function deposit(uint256 id, uint256 optionId, uint256 bean) 
    ...
    returns (bool) {
      ...
      // 某用户订单该选项总投注数
      userOptionTotalBean[optionId][msg.sender] += bean * 10 ** 18;
      // 存储事件总投注
      guessTotalBean[id] += bean * 10 ** 18;
      MOS.transferFrom(msg.sender, address(this), bean * 10 ** 18);
    
      emit Deposit(id, msg.sender, optionId, bean * 10 ** 18);
      return true;
  }
