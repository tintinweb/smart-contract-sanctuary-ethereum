...
contract FakeNewsApp{
   ...
   //mapping(address=>uint256) public voteStore;
    ...
    function calculateResult(uint256 id)public {
        ...
        articleList[id].status = ArticleStatus.ResultCalucalated;

     
    }
    function distribute(uint256 _id)public payable{
        require(articleList[_id].status == ArticleStatus.ResultCalucalated,"Sorry Result not calculated");
...
//VIEW FUNCTIONS
    ...
    function getParticipantsStake(uint256 id,address _add) public view returns(uint256){
        return(Participants[id][_add].staked);
    }
        function getParticipantsVote (uint256 id,address _add) public view returns(bool){
        require(articleList[id].status==ArticleStatus.VotingCompleted,"Session not Completed");
        return (Participants[id][_add].vote);
    }
        function getVerdict(uint256 id) public view returns (bool){
             require( articleList[id].status == ArticleStatus.ResultCalucalated,"Session not Completed");
             return(articleList[id].verdict);
    }
        function getCurrentStatus(uint256 id) public view returns (ArticleStatus){
            return(articleList[id].status);
    }
