...
contract Crowdsale is Whitelist {
    ...
    filterWhitelist(beneficiary)
  {
    ...
    if (weiAmount < weiRemain) {
      //weiRemain = weiRemain.sub(weiAmount);
    }
    else {
      //weiRemain = 0;
      //isStop = true;
  ...
  function setRaiseLimit(uint256 weiLimit) onlyOwner public returns (bool) {
    require(weiLimit > 0);
    weiRemain = weiLimit;
    return true;
  }
