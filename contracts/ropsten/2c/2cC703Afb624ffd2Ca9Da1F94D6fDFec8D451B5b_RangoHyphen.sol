...
contract RangoHyphen is IRangoHyphen, BaseContract {
    ...
    function hyphenBridge(
        ...
        if (_token == address(0)) {
            IHyphenBridge(hyphenAddress).depositNative{ value: _amount }(_receiver, _dstChainId, "RANGOEX");
            emit HyphenNativeDeposit(_dstChainId, _receiver, _amount);
        } else{
            IHyphenBridge(hyphenAddress).depositErc20(_dstChainId, _token, _receiver, _amount, "RANGOEX");
