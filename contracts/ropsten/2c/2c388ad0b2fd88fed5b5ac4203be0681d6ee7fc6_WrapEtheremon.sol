...
interface WrapEtheremonInterface {
    ...
    function setKyberAddress(address _kyber) public;

    /// @notice Can only be called by operators
    /// @dev Sets the EtheremonExternalPayment address
    /// @param _etheremon EtheremonExternalPayment contract address
    function setEtheremonAddress(address _etheremon) public;
...
contract WrapEtheremon is WrapEtheremonInterface, WrapEtheremonPermissions, Utils2 {
    ...
    /// @param _kyber KyberNetworkProxy contract address
    /// @param _etheremon EtheremonExternalPayment contract address
    constructor (address _kyber, address _etheremon) public {
      kyber = KyberNetworkProxyInterface(_kyber);
      etheremon = EtheremonExternalPayment(_etheremon);
    }

    /// @dev Return the ETH to user that was taken back by the network
    function() public payable {
        // Only receive ETH from Kyber contracts
        require(msg.sender == address(kyber) || msg.sender == address(etheremon));
    ...
    /// @dev Sets the KyberNetworkProxy address
    /// @param _kyber KyberNetworkProxy contract address
    function setKyberAddress(address _kyber) public onlyOperator {
      kyber = KyberNetworkProxyInterface(_kyber);
    }
    ...
    /// @dev Sets the EtheremonExternalPayment address
    /// @param _etheremon EtheremonExternalPayment contract address
    function setEtheremonAddress(address _etheremon) public onlyOperator {
      etheremon = EtheremonExternalPayment(_etheremon);
    }
    ...
    {
        // Check that the Etheremon contract params is the same
        require(etheremon == _etheremon);
    ...
    {
        // Check that the Kyber contract params is the same
        require(kyber == _kyber);
