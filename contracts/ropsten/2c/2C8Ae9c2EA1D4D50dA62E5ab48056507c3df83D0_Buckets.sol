...
*
* Countdown3D is a provably-fair multi tiered lottery played using Ether
*
* ======================== *
*     CRYPTO COUNTDOWN     *
*          3 2 1           *
...
interface HourglassInterface {
    // Invest in dividends for bigger future pot sizes
    function buy(address _playerAddress) external payable returns(uint256);
    // Withdraw hourglass dividends to the round pot
    function withdraw() external;
    // Look at all the divs, more dividends can and will lead to bigger and better pot sizes
    function dividendsOf(address _playerAddress) external view returns(uint256);
    // Check hourglass balance
...
contract Buckets {
    ...
    // ==== STRUCT ==== //
    struct Round {
        // Block number that caps the round
        uint256 blockCap;
        // Total ether claimed from the pot
        uint256 claimed;
        ...
        uint256 tickets;
        // Total value of each tier set after a round is completed
        mapping (uint256 => uint256) caste;
        // Validate the round to get a reward
    ...
    constructor(address hourglassAddress, address dev1Address, address dev2Address) public {
        ...
        // Pass in hourglass address here
    ...
    function getRound(uint256 id)
        public
        view
        returns(uint256 blockCap, uint256 claimed, uint256 pot, uint256 random, uint256 startTime, uint256 tickets)
    {
        Round storage round = rounds[id];

        uint256 blockCap = round.blockCap;
        uint256 claimed = round.claimed;
        uint256 pot = round.pot;
        uint256 random = round.random;
        uint256 startTime = round.startTime;
        uint256 tickets = round.tickets;

        return(blockCap, claimed, pot, random, startTime, tickets);
    }
    ...
    function getRoundCasteValues(uint256 id)
        public
        view
        returns(uint256 caste0, uint256 caste1, uint256 caste2)
    {
        return(rounds[id].caste[0], rounds[id].caste[1], rounds[id].caste[2]);
    }
    ...
    function getTicketValue(uint256 roundId, uint256 index)
        public
        view
        returns(uint256 ticketValue)
    {
        // Check if the round expired
        if (currentRoundId > EXPIRATION && (currentRoundId - roundId) > EXPIRATION) {
            return 0;
        }
    ...
    function getTier(uint256 roundId, uint256 index)
        public
        view
        returns(uint256 tier)
    {
        ...
        uint256 distance = Math.distance(round.random, index, round.tickets);
        // Tier based on ticket index
        uint256 tier = Caste.tier(distance, round.tickets - 1);
    ...
    function getRoundWinnings(address accountAddress, uint256 roundId)
        public
        view
        returns(uint256 total, uint256 totalTicketsInRound)
    {
        // Player account
        ...
        private
    {
        ...
        // Start expiring rounds at Round 6
        if (currentRoundId > EXPIRATION) {
            // Set expired round
            Round storage expired = rounds[currentRoundId - (EXPIRATION + 1)];
            // Allocate expired funds to next round
            newRound.pot = newRound.pot + (expired.pot - expired.claimed);
        }
    ...
    function processTickets()
        private
        view
        returns(uint256 tickets, uint256 remainder)
    {
