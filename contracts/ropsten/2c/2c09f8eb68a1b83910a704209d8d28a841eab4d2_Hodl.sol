...
contract Hodl {
    ...
    function withdrawTokens(address tokenContract) public returns (uint256) {
        ...
        require(
            bank[msg.sender].unlockDate <= block.timestamp,
            "HODL: Still lock"
        );
    ...
    function getUnlockDate(address _owner) public view returns (uint256) {
        require(bank[_owner].owner != address(0), "HODL: Bank is not exist");

        Bank storage myBank = bank[_owner];

        return myBank.unlockDate;
    }

    function getBalance(address _owner, address tokenContract)
        public
        view
        returns (uint256)
    {
