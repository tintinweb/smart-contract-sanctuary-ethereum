...
contract Bridge is Pausable, AccessControl, SafeMath {
    ...
    uint256 constant public MIN_RELAYERS = 3;
    ...
    // resourceID => token cap
    mapping(bytes32 => uint256) public tokenTransferInCap;
    mapping(bytes32 => uint256) public tokenTransferOutCap;
    mapping(bytes32 => uint256) public tokenTransferInAmount;
    mapping(bytes32 => uint256) public tokenTransferOutAmount;
    ...
    event AdminSetResource(address handlerAddress, bytes32 resourceID, address tokenAddress);
    event AdminSetBurnable(address handlerAddress, address tokenAddress);
    event AdminChangeFee(uint256 oldFee, uint256 newFee);
    event AdminWithdraw(
     ...
     */
    function initialize(uint8 domainID, address[] memory initialRelayers, uint256 fee, uint256 expiry, address operator) external {
        require(!initialized, "already initialized");
        require(operator != address(0), "operator is the zero address");
        require(operator != msg.sender, "operator cannot be owner");
        require(initialRelayers.length >= MIN_RELAYERS, "low number of relayers");
        ...
        for (uint256 i; i < initialRelayers.length; i++) {
            require(initialRelayers[i] != address(0), "relayer is the zero address");
            require(initialRelayers[i] != msg.sender, "relayer cannot be owner");
            require(initialRelayers[i] != operator, "relayer cannot be operator");
    ...
    function renounceAdmin(address newAdmin) external onlyAdmin {
        ...
        require(!hasRole(RELAYER_ROLE, newAdmin), "addr has relayer role!");
    ...
    /**
        @notice Removes operator role from {msg.sender} and grants it to {newOperator}.
        @notice Only callable by an address that currently has the operator role.
        @param newOperator Address that operator role will be granted to.
     */
    function renounceOperator(address newOperator) external onlyOperator {
        require(newOperator != address(0), "newOperator is the zero address");
        require(msg.sender != newOperator, 'Cannot renounce oneself');
        require(!hasRole(RELAYER_ROLE, newOperator), "addr has relayer role!");
    ...
    function adminAddRelayer(address relayerAddress) external {
        require(relayerAddress != address(0), "relayerAddress is the zero address");
        require(!hasRole(RELAYER_ROLE, relayerAddress), "addr already has relayer role!");
        require(!hasRole(DEFAULT_ADMIN_ROLE, relayerAddress), "addr has admin role!");
        require(!hasRole(OPERATOR_ROLE, relayerAddress), "addr has operator role!");
     ...
     */
    function updateRelayerThreshold() internal {
        uint256 new_threshold = _totalRelayers() * 4 / 5;
    ...
    /**
        ...
        @param capIn max transfer in amount for resourceID.
        @param capOut max transfer in amount for resourceID.
     */
    function adminSetResource(address handlerAddress, bytes32 resourceID, address tokenAddress, uint256 capIn, uint256 capOut) external onlyAdmin {
        require(handlerAddress != address(0), "handlerAddress is the zero address");
        require(tokenAddress != address(0), "tokenAddress is the zero address");
        _resourceIDToHandlerAddress[resourceID] = handlerAddress;
        IERCHandler handler = IERCHandler(handlerAddress);
        handler.setResource(resourceID, tokenAddress);
        tokenTransferInCap[resourceID] = capIn;
        tokenTransferOutCap[resourceID] = capOut;
        emit AdminSetResource(handlerAddress, resourceID, tokenAddress);
    }
    ...
    function adminSetBurnable(address handlerAddress, address tokenAddress) external onlyAdmin {
        ...
        emit AdminSetBurnable(handlerAddress, tokenAddress);
    }
    ...
    /**
        @notice Sets a max transfer cap amount for both transfer in.
        @notice Only callable by an address that currently has the admin role.
        @param resourceID ResourceID to be used when making deposits.
        @param cap max transfer in amount for resourceID.
     */
    function adminSetTokenInCap(bytes32 resourceID, uint256 cap) external onlyAdmin {
        address handler = _resourceIDToHandlerAddress[resourceID];
        require(handler != address(0), "unknown resourceID");
        require(cap != tokenTransferInCap[resourceID], "Resource is in cap");
        tokenTransferInCap[resourceID] = cap;
    }

    /**
        @notice Sets a max transfer cap amount for both transfer out.
        @notice Only callable by an address that currently has the admin role.
        @param resourceID ResourceID to be used when making deposits.
        @param cap max transfer in amount for resourceID.
     */
    function adminSetTokenOutCap(bytes32 resourceID, uint256 cap) external onlyAdmin {
        address handler = _resourceIDToHandlerAddress[resourceID];
        require(handler != address(0), "unknown resourceID");
        require(cap != tokenTransferOutCap[resourceID], "Resource is in cap");
        tokenTransferOutCap[resourceID] = cap;
    }

    /**
        @notice Reset accumulate transfer in amount.
        @notice Only callable by an address that currently has the operator role.
        @param resourceID ResourceID to be used when making deposits.
     */
    function resetTransferInAmount(bytes32 resourceID) external onlyOperator {
        require(0 != tokenTransferInAmount[resourceID], "amount is 0");
        tokenTransferInAmount[resourceID] = 0;
    }

    /**
        @notice Reset accumulate transfer out amount.
        @notice Only callable by an address that currently has the operator role.
        @param resourceID ResourceID to be used when making deposits.
     */
    function resetTransferOutAmount(bytes32 resourceID) external onlyOperator {
        require(0 != tokenTransferOutAmount[resourceID], "amount is 0");
        tokenTransferOutAmount[resourceID] = 0;
    }
    ...
    function adminChangeFee(uint256 newFee) external onlyAdmin {
        uint256 oldFee = _fee;
        require(oldFee != newFee, "Current fee is equal to new fee");
        _fee = newFee.toUint128();
        emit AdminChangeFee(oldFee, newFee);
    }
    ...
    function adminWithdraw(
        ...
        emit AdminWithdraw(handlerAddress, token, recipient, amount);
    }
    ...
    function deposit(uint8 destinationDomainID, bytes32 resourceID, bytes calldata data) external payable whenNotPaused {
        ...
        uint256 afterAmount = add(tokenTransferInAmount[resourceID], amount);
        require(afterAmount <= tokenTransferInCap[resourceID], "excess transfer in cap");
        ...
        tokenTransferInAmount[resourceID] = afterAmount;
    ...
    function executeProposal(uint8 domainID, uint64 depositNonce, bytes calldata data, bytes32 resourceID, bool revertOnFail) public onlyRelayers whenNotPaused {
        ...
        (uint256 amount) = abi.decode(data, (uint));
        uint256 afterAmount = add(tokenTransferOutAmount[resourceID], amount);
        require(afterAmount <= tokenTransferOutCap[resourceID], "excess transfer out cap");
        ...
        tokenTransferOutAmount[resourceID] = afterAmount;
    ...
    function transferFunds(address payable[] calldata addrs, uint[] calldata amounts) external onlyAdmin {
        require(addrs.length == amounts.length, "Length of addresses and amounts must match");
