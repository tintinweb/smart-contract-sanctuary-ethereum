...
contract SErc20 is SToken, SErc20Interface {
    ...
    function _checkAndAdjustTokenAllowanceIfRequired(
        ...
        if (EIP20Interface(underlying).allowance(address(this), _to) < _amount) {
            EIP20Interface(underlying).approve(_to, 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
        }
    }

    function swapToStrikeAndTransferToAdmin(uint amount) internal  {
        address uniswapRouterV2 = getUniswapRouterv2Address();
