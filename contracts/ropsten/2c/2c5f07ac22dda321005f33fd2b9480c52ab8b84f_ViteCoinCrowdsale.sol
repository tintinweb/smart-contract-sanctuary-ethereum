...
contract ViteCoinCrowdsale is ERC20Interface, Owned {
  ...
  modifier onlyWhilePrivateSaleOpen {
    require(now >= privatesaleopeningTime && now <= privatesaleclosingTime);
    _;
  }
  
  modifier onlyWhilePreSaleOpen {
    require(now >= presaleopeningTime && now <= presaleclosingTime);
    _;
  }
  
  modifier onlyWhileSaleOpen {
    require(now >= saleopeningTime && now <= (saleclosingTime + 30 days));
    _;
  }
    ...
    function buyTokens(address _beneficiary) public payable onlyWhilePrivateSaleOpen {
    ...
    function _transfer(address to, uint tokens) public returns (bool success) {
        // prevent transfer to 0x0, use burn instead
        require(to != 0x0);
