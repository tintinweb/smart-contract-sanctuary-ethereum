...
contract AirdropDefirmTo777{
    ...
    function claim() public {
        ...
        uint256 availableAmount = user.debt.add(user.perSecond.mul(period));
    ...
    function setWhiteList(address[] memory _accounts, uint256[] memory _amounts, uint256 ratio, bool _value) public onlyOwner {
      for(uint256 i = 0; i < _accounts.length; i++) {
            UserInfo storage user = userinfo[_accounts[i]];
            user.isWL = _value;
            user.amount = _amounts[i].mul(10 ** 18).div(ratio);
