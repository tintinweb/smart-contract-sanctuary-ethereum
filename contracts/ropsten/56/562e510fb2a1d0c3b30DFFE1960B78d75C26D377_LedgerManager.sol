...
contract LedgerManager is AccessControl, ReentrancyGuard {
    ...
    string constant public ERROR_NATIVE_NOT_SUPPORTED = "Native asset not supported";
    ...
    UniswapExchangeAdapter public adapters;
    mapping(bytes => bool) public usedSignatures;

    constructor (SlidingWindowLedger ledger_, address priceEngine, UniswapExchangeAdapter adapter_, PaypoolV1ERC20 rewardToken_)
    {
        ledger = ledger_;
    ...
    */
    function _createMessageHash(LedgerTypes.OrderInfo memory orderInfo, uint256 endsInSec, uint256 deadline, uint256 rewardBaseAmount) internal pure returns (bytes32) {
        ...
        return keccak256(abi.encodePacked(orderInfo.askAsset, orderInfo.askAmount, orderInfo.offerAsset, orderInfo.offerAmount, orderInfo.owner, orderInfo.isPut, endsInSec, deadline, rewardBaseAmount));
    }
    ...
    */
    function addOrder(LedgerTypes.OrderInfo memory orderInfo, uint256 endsInSec, uint256 deadline, uint256 rewardBaseAmount, bytes memory signature) external payable returns (uint256) {
        ...
        require(usedSignatures[signature] == false, "Signature already used");
    ...
    function _swapAssets(FundsInfo[] calldata fundsInfo) internal {
        for (uint256 i = 0; i < fundsInfo.length; i++) {
            ...
            if (fundInfo.from == address(0)) { // from native to token
                ...
                bytes memory call = adapters.exchangeNativeForToken(nativeAsset, fundInfo.fromAmount, assets[fundInfo.to], fundInfo.toAmount);

                nativeAsset.execute(address(adapters.router()), call, fundInfo.fromAmount);

            } else if (fundInfo.to == address(0)) { // from token to native
                require(address(nativeAsset) != address(0), ERROR_NATIVE_NOT_SUPPORTED);
                require(address(assets[fundInfo.from]) != address(0), string(abi.encodePacked("Asset ", Strings.toHexString(uint160(fundInfo.from), 20), " not supported")));

                Erc20Asset from = assets[fundInfo.from];
                bytes memory call = adapters.exchangeTokenForNative(from, fundInfo.fromAmount, nativeAsset, fundInfo.toAmount);

                from.execute(address(adapters.router()), call, fundInfo.fromAmount);
            } else { // tokens
                require(address(assets[fundInfo.from]) != address(0), string(abi.encodePacked("Asset ", Strings.toHexString(uint160(fundInfo.from), 20), " not supported")));
                require(address(assets[fundInfo.to]) != address(0), string(abi.encodePacked("Asset ", Strings.toHexString(uint160(fundInfo.to), 20), " not supported")));

                Erc20Asset from = assets[fundInfo.from];
                bytes memory call = adapters.exchangeTokens(from, fundInfo.fromAmount, assets[fundInfo.to], fundInfo.toAmount);
...
contract SlidingWindowLedger is AccessControl {
    ...
    * @dev Adds a new order into the order pool.
    ...
    function addOrder(LedgerTypes.OrderInfo memory orderInfo, uint256 endsInSec) external onlyManager returns(uint256) {
        ...
        emit OrderAdded(orderId, orderInfo.askAsset, orderInfo.askAmount, orderInfo.offerAsset, orderInfo.offerAmount, orderInfo.owner, orderInfo.isPut);
        return orderId;
    }
