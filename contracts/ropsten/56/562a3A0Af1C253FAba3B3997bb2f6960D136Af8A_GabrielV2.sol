...
contract GabrielV2 is Ownable {
    ...
    function reusePool(uint _pid, ExtraArgs memory extraArgs, uint _NORT, address[] memory _rewardTokens, uint[] memory _totalRewards) external onlyOwner {
        ...
        pool.totalStaked = pool.totalStaked.sub(pool.emergencyUnstaked);
    ...
    function harvest(uint _pid) external {
        ...
        for (uint i = 0; i < NORT; i++) {
            uint reward = pool.lockDuration * user.amount * pool.totalRewards[i];
            uint lpSupply = pool.lockDuration * pool.totalStaked;
            uint pending = reward.div(lpSupply);
            if (pending > 0) {
                uint forfeitedRwds = (pool.lockDuration *
                    pool.emergencyUnstaked *
                    pool.totalRewards[i]) / (pool.lockDuration * pool.totalStaked);
    ...
    function withdraw(uint _pid) external onlyOwner {
        ...
        for (uint i = 0; i < NORT; i++) {
            ...
            uint notWithdrawn = (pool.totalStaked).sub(pool.emergencyUnstaked);
            uint min = i == 0 ? notWithdrawn.add(forfeitedRwds) : forfeitedRwds;
            // Calculate LOR.
            if (poolBal > min) {
