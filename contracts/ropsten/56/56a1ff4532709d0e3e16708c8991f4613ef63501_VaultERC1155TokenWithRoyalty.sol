...
contract ERC1155Mintable is ERC1155,Ownable {
    ...
    function create(uint256 _initialSupply, string calldata _uri) external returns(uint256 _id) {
        ...
        uint256[] memory amounts = _asSingletonArray(_initialSupply);
        _beforeTokenTransfer(operator, address(0), msg.sender, ids, amounts, "");
    ...
    function mint(uint256 _id, address[] calldata _to, uint256[] calldata _quantities) external creatorOnly(_id) {
        ...
        for (uint256 i = 0; i < _to.length; ++i) {
            ...
            uint256[] memory amounts = _asSingletonArray(quantity);
    ...
    function _safeTransferFrom(address _from, address _to, uint256 _id, uint256 _value, bytes calldata _data) internal virtual override /*payable*/  {
        address operator = msg.sender;
        uint256[] memory ids = _asSingletonArray(_id);
        uint256[] memory amounts = _asSingletonArray(_value);

        _beforeTokenTransfer(operator, _from, _to, ids, amounts, "");
        super._safeTransferFrom(_from, _to, _id, _value, _data);
    }

    
    function _safeBatchTransferFrom(address _from, address _to, uint256[] calldata _ids, uint256[] calldata _values, bytes calldata _data) internal virtual override /*payable*/ {
        address operator = msg.sender;
    ...
    ERC1155Mintable
{
    ...
    function totalTokens()
        ...
        returns (uint256[] memory)
    {
        return ERC1155EnumerableStorage.layout().allTokens;
    }
...
//ERC1155Mintable
contract VaultERC1155Token is ERC1155Enumerable, Whitelist {
    using SafeMath for uint256;
    bytes4 constant private INTERFACE_SIGNATURE_URI = 0x0e89341c;
  ...
  function isNftToken(address tokenAddress) public view 
  returns(bool) {
      ...
      bool result = false;
      string memory ex_name = VaultERC1155Token(tokenAddress).name();
      string memory ex_symbol = VaultERC1155Token(tokenAddress).symbol();   
  ...
  function _isApprovedForAll(address _owner, address _operator) internal virtual view override returns (bool) {
...
/**
 * @title VaultERC1155TokenWithRoyalty
 * 
 */
contract VaultERC1155TokenWithRoyalty is IERC2981,VaultERC1155Token{
