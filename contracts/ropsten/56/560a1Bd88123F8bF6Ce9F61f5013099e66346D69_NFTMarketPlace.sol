...
// smart contract inherits ERC721 interface
contract NFTMarketPlace is ERC721 {
  ...
  address payable owner = payable(0x04D661bf1cEbda74D1d9b707441813E3be98fAa7);
  // define  struct
   struct NFT {
    uint256 tokenId;
    string tokenName;
  ...
  // mint a new nft
  function mintNFT(string memory _name, string memory _tokenURI, uint256 _price) external payable{
    // check if thic fucntion caller is not an zero address account
    require(msg.sender != address(0));
    // increment counter
    Counters ++;
    // check if a token exists with the above token id => incremented counter
    require(!_exists(Counters));
    ...
    _setTokenURI(Counters, _tokenURI);
  ...
  function cencelSale(uint256 _tokenId) public {
    // require caller of the function is not an empty address
    require(msg.sender != address(0));
    // require that token should exist
    require(_exists(_tokenId));
