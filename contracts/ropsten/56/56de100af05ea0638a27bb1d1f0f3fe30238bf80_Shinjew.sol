...
contract Shinjew is Context, IERC20, Ownable {
    ...
    string private _name = "SHINJEW";
    string private _symbol = "SJEW";
    ...
    address payable public marketingWallet = payable(0x726EC9F8aCf4a7D48C5266664308164aA67FC661); // marketing wallet address
    ...
    uint256 public maxBuyTxAmount = 1000000000 * 10**18;
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            require(amount <= maxBuyTxAmount, "Transfer amount exceeds the maxTxAmount.");
    ...
    function setFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner() {
        require(taxFee.add(liquidityFee).add(marketingFee) <= 20, "Fees too high");
        _taxFee = taxFee;
    ...
    function setMaxBuyTxAmount(uint256 _maxTxAmount) external onlyOwner() {
        maxBuyTxAmount = _maxTxAmount;
        require(maxBuyTxAmount > totalSupply().div(1000), "transaction amount must be greater than 0.1% of total supply");
    }
    ...
    function setMaxWalletToken(uint256 _maxToken) external onlyOwner {
          ...
          require(maxWalletToken > totalSupply().div(1000), "transaction amount must be greater than 0.1% of total supply");
  	}
