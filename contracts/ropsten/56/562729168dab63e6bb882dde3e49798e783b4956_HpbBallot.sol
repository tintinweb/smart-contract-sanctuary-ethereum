...
contract HpbBallot {
    ...
    */
    ...
    // 增加候选者
    // add candidate
    event CandidateAdded(address indexed candidateAddr,string indexed facilityId,uint serialNumber,string name);
    
    // 更新候选者
    // update candidate
    event CandidateUpdated(address indexed candidateAddr,string indexed facilityId,uint serialNumber,string name);
        ...
        uint[] nums
         ...
         for (uint i = 1;i<candidateArray.length;i++){
             if(i<=capacity){
                 ...
                 _facilityIds[i-1]=stringToBytes32(candidateArray[i].facilityId);
               ...
               if(candidateArray[i].numberOfVotes==min){
                   ...
                   if(candidateArray[i].candidateAddr.balance>_addrs[minIndex].balance){
		               ...
		               _facilityIds[minIndex]=stringToBytes32(candidateArray[i].facilityId);
