...
contract GFTShoppe is ERC721Enumerable, Ownable {
    ...
    uint256 public maxMintCount = 12; // The maximum number of tokens an address can mint
    bool public publicSaleActive = true; // Non-admin users can only mint new tokens when this flag is set to true
    bool public whitelistSaleActive = true; // If set to true, whitelisted users will be allowed to mint
    ...
    constructor(string memory _baseUri) ERC721("Atari GFT A", "GFTA") {
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        if (redeemedStatus[_tokenId]) {
            require(bytes(redeemedBaseTokenURI).length > 0, "The redeemed base URI must be set first");
    ...
    function createItem(uint256 amount) external payable {
        ...
        require(
            addressMintCount[msg.sender] + amount <= maxMintCount,
    ...
    function createWhitelistItem(uint256 amount) external payable {
        uint256 supply = totalSupply();
        require(whitelistSaleActive == true, "Whitelist sale is not active");
    ...
    // An onwer-only function which toggles the whitelist sale on/off
    ...
    // Fuction called by second minting contract which sets the redeemed status
