  ...
  //StakeWithAmd
   ...
   function extendStake(uint256 count,uint256 newplan) public {
      ...
      // require(user.stakerecord[count].stakeTime != ,"This stake is already withdrawn.");
       ...
       user.stakerecord[user.stakecount].withdrawTime = user.stakerecord[count].stakeTime.add(durations[newplan]);
       user.stakerecord[user.stakecount].bonus = user.stakerecord[count].amount.mul(percentages[newplan]).div(percentDivider);
       
       emit ExtenderStake(msg.sender,user.stakerecord[count].amount,block.timestamp);
   }
   
   function migrateV1(address userList) external onlyOwner returns (bool){
   
       // for (uint i=0; i< userList.length; i++){

           // require(userList[i] != address(0),"This is not a valid address");
            
            User storage user = users[userList];
            (uint256 _totalstakeduser, uint256 _stakecount, uint256 _claimedstakeTokens) = stakeInstance.users(userList);
            ...
            for(uint256 j = 0; j < _stakecount; j++){
                ...
                (uint256 _withdrawTime/*,uint256 _stakeTime*/,uint256 _amount,uint256 _bonus,uint256 _plan,
                    bool _withdrawan) = stakeInstance.stakedetails(userList,j);
