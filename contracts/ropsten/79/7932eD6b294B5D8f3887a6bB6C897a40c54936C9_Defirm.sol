...
contract Defirm is Ownable, ERC20DetailedUpgradeable{
    ...
    function __USDCReceiver_init() external onlyOwner {
        require(usdcReceiver == address(0), 'initialized');
        USDCReceiver receiver = new USDCReceiver();
        usdcReceiver = address(receiver);
        IUSDCReceiver(usdcReceiver).setUSDC(USDC);
    ...
    function _transferFrom(
        ...
        if(sender != address(liquidityManager) && recipient != address(liquidityManager)){
    ...
    function _swapTokensForUsdc(uint256 tokenAmount, address receiver) internal {
        if(swappingOnlyFromContract){
            _allowedFragments[address(this)][address(liquidityManager)] = tokenAmount;
...
contract USDCReceiver is Ownable {
    ...
    function setUSDC(address _usdc) public {
