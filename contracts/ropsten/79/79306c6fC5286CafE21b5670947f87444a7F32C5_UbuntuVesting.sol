...
import "../access/UbuntuAuth.sol";
import "../../interfaces/token/ERC20.sol";

/**
 * @title AfricaRare Vesting Contract
 * @author Beau Williams (@beauwilliams)
 * @dev Smart contract for Africarare to vest claimable tokens to our investors
 */
contract UbuntuVesting is Auth {
    using SafeMath for uint256;

    address ubuntuTokenAddress;
    ...
    function getLockedShares(address holder) public view returns (uint256) {
        if(vestStart == 0) return 0;

        uint256 locked = shares[holder] - (getClaimableShares(holder) + claimed[holder]);
        return locked;
    }
...
/**
 * @title AfricaRare Ubuntu Auth Contract
 * @author Beau Williams (@beauwilliams)
 * @dev Smart contract for Africarare's Auth controls
 */
