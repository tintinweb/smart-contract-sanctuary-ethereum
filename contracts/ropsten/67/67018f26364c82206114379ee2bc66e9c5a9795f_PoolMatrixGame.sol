...
contract PoolMatrixGame {
    ...
    function importCleanUsers() public {
        uint i;
        require(msg.sender == adminWallet);
    ...
    function importCleanQueues(uint start, uint end) public {
        require(msg.sender == adminWallet);
        for (uint level = start; level <= end; level++) {
    ...
    function setParams(uint totalTransactions, uint totalTurnover, uint marketing, uint32 users24h1, uint32 users24h2, uint32 transactions24h1, uint32 transactions24h2, uint turnover24h1, uint turnover24h2) public {
        require(msg.sender == adminWallet);
