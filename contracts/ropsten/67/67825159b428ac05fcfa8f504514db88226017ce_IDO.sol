...
contract Pool is IPool, Ownable {
 ...
 // transactions from IDO account 
    // only owner can do this deposit
        ...
        returns (uint256 _tokensAmount)
    {
        ...
        _tokensAmount = collaborations[_participant];
    }
...
contract Whitelist is Ownable {
   struct user{
     ...
     mapping(address=>user) private WhitelistedAddresses;
    
    IERC20 private IDOToken;
    uint private stakedamt;
   // mapping(address => bool) private whitelistedAddressesMap;
    //mapping(address => uint) private whitelistedAddressestier;
    mapping(address => uint) private IDObalance;
    address[] private whitelistedAddressesArray;
    enum tiers{Unilisted, Tier1, Tier2, Tier3 }

    event AddedToWhitelist(address indexed account, uint Tierno, uint);
    event RemovedFromWhitelist(address indexed account, uint tierno);


    function getTier(uint Amount) internal pure returns (uint8 number){
        if(Amount>5000) return 1;
        else if (Amount>3000) return 2;
        else if(Amount>0) return 3;
        else return 0;
    }

    function addToWaitlist(address _address, uint amount)
        internal
        onlyOwner
        returns (bool success)
    {
            ...
            Validations.revertOnZeroAddress(userAddress);
                ...
                WhitelistedAddresses[msg.sender].time = block.timestamp + 432000;
                WhitelistedAddresses[msg.sender].tier = getTier(amount);
                IDOToken.transfer(address (this), amount); 
                WhitelistedAddresses[msg.sender].balance = amount;
                emit AddedToWhitelist(userAddress, getTier(amount), WhitelistedAddresses[msg.sender].time);
        
        success = true;
    }
    function removeFromWhitlist(address _address)
    internal 
    returns(bool success){
        require (WhitelistedAddresses[_address].balance >0, "no amount staked");
        IDOToken.transfer(_address, WhitelistedAddresses[_address].balance);
        success = true;
    }
    ...
    function isAddressWhitelisted(address _address)
        ...
        returns (bool isIt)
    {
       require(WhitelistedAddresses[_address].balance>0);
       if(WhitelistedAddresses[_address].time > block.timestamp){
            WhitelistedAddresses[_address].listed = true;
            isIt = true;
       }
       else {
       revert("Wait for 5 days");
       }
    }
}
...
contract IDO is Pausable, AccessControl, Ownable, Whitelist {
    ...
    function addAddressesToWhitelist(address whitelistedAddresses, uint _amount)
        public
    {
        addToWaitlist(whitelistedAddresses,_amount);
    }
    ...
    modifier _onlyWhitelisted(address _address) {
        require( isAddressWhitelisted(_address), "Not Whitelisted!");
        _;
    }
