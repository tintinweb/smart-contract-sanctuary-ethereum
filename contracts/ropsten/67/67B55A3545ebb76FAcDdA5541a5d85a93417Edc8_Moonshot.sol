...
contract Moonshot is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "MSHOTv3";
    ...
    event SetFeeForAddress(address account, uint256 newFee, bool enabled);
   ...
   function setFees(uint256 newRewardFee, uint256 newLiquidityFee, uint256 newMarketingFee) external onlyOwner() {
    ...
    function setFeeForAddress(address account, uint256 newFee, bool enabled) external onlyOwner {
        require( newFee <= 1000, "Total fee must be <= 1000" );

        _specialFees[ account ] = newFee;
        _hasSpecialFee[ account ] = enabled;
        emit SetFeeForAddress(account, newFee, enabled);
    }
    ...
    function _transfer(
        ...
        if( takeFee && !inSwapAndLiquify && from != uniswapV2Pair) 
