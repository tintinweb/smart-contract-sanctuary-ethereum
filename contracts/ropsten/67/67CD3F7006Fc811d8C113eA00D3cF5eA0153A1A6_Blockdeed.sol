...
contract Blockdeed is ERC721, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    // uint256 public stakingDuration = 7776000; // 90 days
    uint256 public stakingDuration = 300; // 5 minutes

    address public TokenContract;
    Counters.Counter private _tokenIdCounter;

    constructor(address tokenContract) ERC721("Blockdeed", "BKDNFT") {
        _owner = msg.sender;
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual override {
        ...
        require(stake.unstaked!=0,"Your NFT is in staking.");
    ...
    function whitelis(address _user) public onlyOwner {
    ...
    function getStakeData(uint256 tokenId) public view returns (address, uint256, uint256, uint256, uint256, uint256){
    ...
    function getStakeData(address staker) public view returns(Stake[] memory){
        // for(uint256 i;i<stake_holders.length;i++){
        //     Stake memory _stake = stakes[i];
        //     if(_stake.user==staker){
        //         stakingSummarys.push(StakingSummary(staker,stakes[i]));
        //     }
        // }
