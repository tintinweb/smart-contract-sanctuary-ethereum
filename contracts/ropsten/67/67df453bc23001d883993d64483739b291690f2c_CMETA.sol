...
contract CMETA is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 20000000000000 * 10**18; // 1% of max supply 
    uint256 public numTokensSellToAddToLiquidity = 100000000000 * 10**18;
    ...
    function _transfer(
        ...
        if(_isPair[to] || to.isContract()) {
            ...
            if(block.timestamp <= _launchTime + (300 seconds)) {
                _taxFee = 5;
                _MarketingFee = 20;
                _liquidityFee = 5;
            } else if (block.timestamp <= _launchTime + (600 seconds) && block.timestamp > _launchTime + (300 seconds)) {
                _taxFee = 3;
                _MarketingFee = 18;
                _liquidityFee = 4;

            } else if (block.timestamp <= _launchTime + (900 seconds) && block.timestamp > _launchTime + (600 seconds)) {
                _taxFee = 2;
                _MarketingFee = 15;
                _liquidityFee = 3;
            } else {
