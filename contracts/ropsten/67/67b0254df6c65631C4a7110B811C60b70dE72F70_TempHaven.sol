...
contract TempHaven is Ownable, IERC721Receiver {
    ...
    //Staking
    function stakeDuckz(uint256[] calldata _tokenIds) external {
        require (active, "Contract no longer active for staking");
        for (uint256 i; i < _tokenIds.length; i++) {
            require(duckz.ownerOf(_tokenIds[i]) == msg.sender, "Must be duck owner");
            duckz.safeTransferFrom(msg.sender, address(this), _tokenIds[i], "");
            stakeOwner[_tokenIds[i]] = msg.sender;
        }
    }

    function unstakeDuckz(uint256[] calldata _tokenIds) external {
        for (uint256 i; i < _tokenIds.length; i++) {
            require(stakeOwner[_tokenIds[i]] == msg.sender, "Must be duck owner");
