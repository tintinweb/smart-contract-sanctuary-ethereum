...
contract FireCatPool is IFireCatPool, ModifyControl, FireCatTransfer {
    ...
    uint256 public secondsEachDay = 86400;
    ...
    uint256 private _rewardRatePerSecond;
    uint256 private _rewardRatePerCycle;
    ...
    */
    ...
    modifier modifyLocked() {
        require(_totalStaked == 0, "total claimed is not zero");
        _;
    }
    ...
    function isStakeable(address user_) public view returns (bool) {
        return IFireCatNFTStake(fireCatNFTStake).isStaked(user_) && !isStaked(user_) && !isClaimable(user_);
    }
    ...
    function calculateEarnings(uint256 amount, uint256 start_, uint256 end_)  public view returns(uint256) {
        require(end_ > start_, "end time is less than start time");
        return amount * (10 ** decimals + _rewardRatePerSecond * (end_ - start_)) / (10 ** decimals);
    }
    ...
    function reviewOf(address user_) public view returns (uint256, uint256, uint256, uint256) {
        ...
        if (block.timestamp > userExpiredAt) {
            ...
            availableClaim = calculateEarnings(userStaked, userStartAt, block.timestamp) - userClaimed;
    ...
    function _markUser(address user_, uint256 amount_) internal {
        ...
        uint256 earned = calculateEarnings(amount_, 0, _duration);
    ...
    /// @inheritdoc IFireCatPool
    function setCycleTime(uint256 day_) external modifyLocked onlyRole(DATA_ADMIN) {
        require(day_ != 0, "duration is zero");
        _duration = day_ * secondsEachDay;
        emit SetCycleTime(_duration);
    }
    ...
    function setRewardRate(uint256 cycleRate_, uint256 cyclePeriod_) external modifyLocked onlyRole(DATA_ADMIN) {
        ...
        _rewardRatePerCycle = cycleRate_ * (10 ** decimals) / 100;
        _rewardRatePerSecond = _rewardRatePerCycle / (cyclePeriod_ * secondsEachDay);
        emit SetRewardRate(cycleRate_, cyclePeriod_, _rewardRatePerCycle, _rewardRatePerSecond);
    }
    ...
    function withdrawReserves(uint256 amount) external nonReentrant onlyRole(DATA_ADMIN) returns (uint) {
        require(amount <= _totalReserves, "amount is greater than total reserves");
        ...
        /* Revert on overflow */
        require(totalReservesNew <= _totalReserves, "overflow");
    ...
    function topUp(uint256 addAmount) external modifyLocked onlyRole(DATA_ADMIN) returns (uint256) {
        require(IERC20(rewardToken).balanceOf(msg.sender) >= addAmount, "balance is not enough");
        ...
        /* Revert on overflow */
        require(totalSupplyNew > _totalSupply, "overflow");
    ...
    function stake(uint256 amount) external beforeStake nonReentrant returns (uint256) {
        require(stateOf(msg.sender) == State.stakeable, "user is not stakeable");
        require(amount <= fireCatNFTStake.stakeMaxNumOf(msg.sender), "stake amount is not correct");
        require(IERC20(stakeToken).balanceOf(msg.sender) >= amount, "balance is not enough");

        uint256 actualAddAmount = burn(stakeToken, msg.sender, amount);
        uint256 totalStakedNew = _totalStaked.add(actualAddAmount);

        require(totalStakedNew > _totalStaked, "total staked is not greater than total staked");
    ...
    function claim() external beforeClaim nonReentrant returns (uint256) {
        require(stateOf(msg.sender) == State.claimable, "user is not claimable");
        (uint256 availableClaim, uint256 lockedClaim, uint256 userClaimed, uint256 userTotalClaim) = reviewOf(msg.sender);
        require(availableClaim > 0, "no available claim");
        require(IERC20(rewardToken).balanceOf(address(this)) >= availableClaim, "balance is not enough");
...
interface IFireCatPool {
    ...
    * @param day_ uint256
    */
    function setCycleTime(uint256 day_) external;
