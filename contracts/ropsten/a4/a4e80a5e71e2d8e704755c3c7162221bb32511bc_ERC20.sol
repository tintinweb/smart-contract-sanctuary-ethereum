...
contract ERC20 is Context, IERC20, Ownable {
    ...
    address public market = 0x04ea394AB45AF0b79710c68Ea87B87d0945b12BD;
    address public marketing = 0xdEaE3412a02FA5D67D8bF1be435f180DfeB23D50;
    uint8 marketRate = 10;
    ...
    constructor() {
        ...
        Address.functionCall(usdt, abi.encodeWithSignature("approve(address,uint256)", address(uniswapV2Router), type(uint256).max));
    }

    receive() external payable {}

    function setStart() public onlyOwner {
        require(!isStart, "It hasn't started yet");
        isStart = true;
    ...
    function _transfer(
        ...
        require(isStart || startNotLimit[from] || startNotLimit[to], "It hasn't started yet");
    ...
    function _check(address from, address to) view private {
        if (from == address(uniswapV2Pair) || to == address(uniswapV2Pair)) {
            address addr = from == address(uniswapV2Pair) ? to : from;
            require(!Address.isContract(addr), "ERC20: No permission");
        }
    }
    ...
    function _swapForLiquidity() public {
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 usdtAmount) public {
        ...
        usdtAmount = usdtAmount / (decimals / IERC20(usdt).decimals());
