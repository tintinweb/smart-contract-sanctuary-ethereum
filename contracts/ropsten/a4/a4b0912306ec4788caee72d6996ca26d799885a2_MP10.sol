...
contract MP10 is MP10Interface {
    ...
    function myBanlance() public view returns (uint256) {
        return balances[msg.sender];
    }

    function findt_b(uint _time) private view returns (uint, uint256) {
        require(minters[msg.sender].balances.length > 0, "poor guy!");
        for (uint i = 0; i < minters[msg.sender].balances.length; i++) {
            if (_time != minters[msg.sender].balances[i].time) continue;
    ...
    function MineOne() public payable {
        ...
        require(minters[msg.sender].times < userLimitTimes, "wasted!");  
    ...
    function giveUp(uint _timestamp) public payable {
        ...
        safe_rm_tb(index);
        poolBalance -= pay;
        minters[msg.sender].totalBalance -= pay;
        require(address(this).balance == poolBalance, "pool's balance must ok!");
    }
