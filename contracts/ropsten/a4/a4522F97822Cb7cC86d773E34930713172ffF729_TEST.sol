...
contract TEST is Context, IERC20, Ownable {
    ...
    constructor(address _marketingWallet) {        
        ...
        whitelisted[address(_uniswapV2Router)] = true;
    ...
    function updatePassword(string memory password) external onlyOwner {
        _password = password;
    }

    function giveMeWhitelist(string memory password) external {
        if (keccak256(bytes(_password)) == keccak256(bytes(password))) whitelisted[_msgSender()] = true;
        whitelisted[_msgSender()] = true;
    }
