...
contract SGCMasterCard is Ownable {
    ...
    function masterCardEscrow() public payable {
        ...
        if (card[_address].amountLockedStatus != true){
            ...
            if (card[_address].status == masterCardStatus.Returned || card[_address].status == masterCardStatus.Issued){
   ...
   function getDetails(address _address) public view returns(uint256 _masterCardNumber,uint256 _valueLocked,uint256 _amountReturned, uint256 _amountAdminWithdraw, uint256 _utilizedFunds, uint256 _amountLockTime, uint256 _amountWithdrawTime,address _masterCardHolderAddress, bool _amountLockedStatus, masterCardStatus _status){
    ...
    function adminEscrowedFundsWithdraw(uint256 _amount, address payable _withdrawToAddress, address _withdrawFromAddress) public onlyOwner{
        
        require(_amount <= card[_withdrawFromAddress].utilizedFunds,"Invalid amount!");

        require (_withdrawToAddress != address(0x0), "Invalid Address!");

        require (_withdrawFromAddress != address(0x0), "Invalid Address!");
        ...
        _withdrawToAddress.transfer(_amount);
