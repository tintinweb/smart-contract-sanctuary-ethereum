...
import "./interface/IWrapReNFT.sol";
import "./BaseReNFT.sol";

contract WrapReNFT is BaseReNFT,IWrapReNFT{
    ...
    function mintXNft(uint256 oid) public nonReentrant override returns(uint256 tid){
        ...
        //按照上述信息mintReNFT
        tid = mintReNft(lastOwner,oid,uint64(block.timestamp),type(uint64).max);
        oid2xid[oid] = tid;
    }
    ...
    function couldRedeem(uint256 tokenId,uint256[] calldata durationIds) public view returns(bool) {
        ...
        ReNftInfo storage info = reNftMapping[tokenId];
...
interface IBaseReNFT is IERC721Receiver {
    struct Duration{
        uint64 start; 
        uint64 end;
    }
    ...
    function getReNftInfo(uint256 tokenId) external view returns(uint256 oid, uint256[] memory durationIds,uint64[] memory starts,uint64[] memory ends,uint64 nonce);
...
import "./OwnableContract.sol";
import "./interface/IBaseReNFT.sol";

abstract contract BaseReNFT is OwnableContract,ReentrancyGuard,ERC721Enumerable,IBaseReNFT {
    using EnumerableSet for EnumerableSet.UintSet;
    address internal oNftAddress;
    uint256 public curReid;
    uint256 public curDurationId;
    uint64 private maxDuration = 31536000;
    mapping(uint256 => ReNftInfo) internal reNftMapping;
    mapping(uint256 => Duration) internal durationMapping;
    mapping(uint256 => Distribution)internal distributionMapping;
    mapping(int256 => uint256)internal MinProfitMapping;
    mapping(uint256 => uint256) internal oid2xid;
    mapping(uint256 => address) internal checkInUserMapping;

    bool private isOnlyNow = true;
    string private _reName;
    string private _reSymbol;
    string private _rclURI;
    ...
    function contains(uint256 tokenId,uint256 durationId) public view returns(bool){
        return reNftMapping[tokenId].durationList.contains(durationId);
    }
    ...
    function getDurationListLength(uint256 tokenId) external view returns(uint256){
        return reNftMapping[tokenId].durationList.length();
    }

    function getReNftInfo(uint256 tokenId) public view returns(uint256 oid, uint256[] memory durationIds, uint64[] memory starts,uint64[] memory ends,uint64 nonce){
        ReNftInfo storage info = reNftMapping[tokenId];
        oid = info.oid;
        nonce = info.nonce;
        uint256 length = info.durationList.length();
    ...
    function getNonce(uint256 tokenId) external view returns(uint64){
       return reNftMapping[tokenId].nonce;
    }
    ...
    function mint(
        ...
        if (start == duration.start && end == duration.end) {
            tid = mintReNft(to,info.oid,start,end);
    ...
    function newReNft(uint256 oid_,uint64 start,uint64 end) internal returns (uint256)
    {
        curReid++;
        ReNftInfo storage info = reNftMapping[curReid];
        info.oid = oid_;
        info.nonce = 0;
        newDuration(curReid,start,end);
        return curReid;
    }
    ...
    function newDuration(uint256 tokenId,uint64 start,uint64 end) private{
        ...
        reNftMapping[tokenId].durationList.add(curDurationId);
        emit DurationUpdate(curDurationId,tokenId,start,end);
    }

    function mintReNft(address to, uint256 oid_,uint64 start,uint64 end)
        internal
        returns (uint256)
    {
        newReNft(oid_,start,end);
        _safeMint(to, curReid);
        return curReid;
    }
    ...
    function concat(uint256 tokenId,uint256 durationId,uint256 targetTokenId,uint256 targetDurationId) public {
        ...
        require(reNftMapping[tokenId].oid == reNftMapping[targetTokenId].oid , "diff oid");
        ...
        if(reNftMapping[tokenId].durationList.length() == 0){
            _burn(tokenId);
        }
    ...
    function _burnDuration(uint256 tokenId,uint256 durationId) private{
        ...
        reNftMapping[tokenId].durationList.remove(durationId);
    ...
    function _burnXNft(uint256 wid) internal {
        ReNftInfo storage info = reNftMapping[wid];
        uint256 length = info.durationList.length();
        for (uint256 index = 0; index < length; index++) {
    ...
    function getFingerprint(uint256 tokenId) public view returns(bytes32 print){
        (uint256 oid, uint256[] memory durationIds,uint64[] memory starts,uint64[] memory ends,uint64 nonce) = getReNftInfo(tokenId);
    ...
    function isXNft(uint256 tokenId) public view returns(bool) {
        ...
        return oid2xid[reNftMapping[tokenId].oid] == tokenId ;
    }
    ...
    function _beforeTokenTransfer(
        ...
        reNftMapping[tokenId].nonce++;
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return
            interfaceId == type(IBaseReNFT).interfaceId ||
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory){
        return string(abi.encodePacked(_rclURI, Strings.toString(tokenId)));
    }
