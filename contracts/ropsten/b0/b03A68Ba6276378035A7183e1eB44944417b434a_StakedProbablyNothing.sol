...
contract StakedProbablyNothing is IStakedProbablyNothing, Ownable, ERC20Burnable {
    ...
    function setupStakingPeriod(uint256 rewardsPerEpoch_, uint numEpochs_) public override onlyOwner {
        ...
        require(block.timestamp > rewardsPeriodEndTime, "Can only update rewards after last rewards period ends");
        ...
        rewardsPeriodEndTime = block.timestamp + numEpochs_ * epochLength;
        require(vestedRewards() == 0, "Invariant failed: vested rewards should be 0. None should have vested yet.");
    }

    function setEpochSeconds(uint256 numSeconds) external onlyOwner {
        require(numSeconds < 86401, "Epochs cannot be longer than 86400 seconds");
        epochLength = numSeconds;
    }
    ...
    function getElapsedEpochs() public view returns (uint256) {
        return (block.timestamp - rewardsPeriodStartTime) / epochLength;
    }

    function vestedRewards() public view returns (uint256) {
        if (block.timestamp < rewardsPeriodStartTime) {
            return 0;
        }

        uint elapsedEpochs = (block.timestamp - rewardsPeriodStartTime) / epochLength;
    ...
    //     uint elapsedEpochs = (block.timestamp - rewardsPeriodStartTime) / epochLength;
    //     if (elapsedEpochs >= rewardsFundedEpochs) {
    //         return rewardsPeriodStartTime + rewardsFundedEpochs * epochLength;
    //     }

    //     return rewardsPeriodStartTime + (elapsedEpochs + 1) * epochLength;
    // }
    ...
    /**
     * Convert staking token to base token based on current conversion ratio.
     */
    ...
    /**
     * Stake base token and received staked token.
     */
    function addStake(uint256 baseTokenAmount) external override {
        ...
        uint256 stakedTokensAmount = toStakedToken(baseTokenAmount);
    ...
    /**
     * Remove staking token and withdraw base token.
     */
    function removeStake(uint256 stakedTokenAmount) external override {
