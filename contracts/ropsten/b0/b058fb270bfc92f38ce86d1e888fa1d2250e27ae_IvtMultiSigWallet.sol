...
contract IvtMultiSigWallet is RLPEncode{
    ...
    bool private safeMode; // When active, wallet may only send to signer addresses
    ...
    function verifySignatures(bytes32 _msgHash, uint8[] _v, bytes32[] _r,bytes32[] _s) view private{
        ...
        for (uint8 i = 0; i < _v.length; i++){
            require(signers[ecrecover(_msgHash, _v[i], _r[i], _s[i])]);
            hasConfirmed++;
        }
