...
contract UtilToken is ERC721AUpgradeable, BlackListUpgradeable,  PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IUtilToken{
    ...
    uint256 private previousBatchLength;
    ...
    function mintSanityCheck(address callerAddress, bytes32[] calldata _merkleProof) internal {
        ...
        if(block.timestamp < currentBatch.endTime && block.timestamp > currentBatch.startTime){
            if(!currentBatch.publicMint){
                ...
                require(!whitelistClaimed[previousBatchLength][msg.sender], "Whitelisted user already minted for current batch");
                whitelistClaimed[previousBatchLength][msg.sender] = true;
            }
        }
    }
    ...
    function addBatch(uint256 _startTime, uint256 _endTime, bytes32 _merkleHash, bool _publicMint) public onlyOwner whenNotPaused {
        if(currentBatch.endTime < block.timestamp){
            updateCurrentBatch(_startTime, _endTime, _merkleHash);
        }
        else{
            if(currentBatch.endTime > 0){
                ...
                previousBatchLength = previousBatchLength.add(1);
            }
            currentBatch = Batch(_startTime, _endTime, _merkleHash, _publicMint);
        }
    }

    function updateCurrentBatch(uint256 _startTime, uint256 _endTime, bytes32 _merkleHash) public onlyOwner whenNotPaused {
        currentBatch.startTime = _startTime;
        currentBatch.endTime = _endTime;
        currentBatch.merkleHash = _merkleHash;
    }

    function totalBatchCount() external view returns(uint256){
        return previousBatchLength.add(1);
    }
...
interface IUtilToken {
    ...
    /**
     * @dev Alter the Sale active status
     */
    function alterSaleActiveStatus(bool isActive) external;

    /**
     * @dev Set the Mint amount 
     */
    function setMintPrice (uint256 newAmount) external;

    /**
     * @dev Get the Mint amount 
     */
