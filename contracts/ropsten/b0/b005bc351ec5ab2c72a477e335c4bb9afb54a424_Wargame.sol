...
contract Wargame {
		...
		returns(bool)
    {
		...
		if (now >= roundEnd_ && roundEnd_ > 0) {
			roundRunning_ = false;
			return false;	// card has not been flipped
		}
        ...
        public
    {
		...
		if (contractActivated_) {
			...
			if (!roundRunning_) {
				...
				// bcs we are nice, this ensures your transaction does not use any gas
				revert();
			}
    ...
    {		
		...
		// if card has not been flipped, the sender has ended the round instead and gets his eth back
		bool _cardFlipped = _buyNationInternal(_index, _value);
        if (!_cardFlipped) {
    ...
    {		
		...
		if (!_cardFlipped) {
			...
			_excess = msg.value.sub(_ethToOwner).sub(_ethToFund).sub(_ethToPot);
		}
