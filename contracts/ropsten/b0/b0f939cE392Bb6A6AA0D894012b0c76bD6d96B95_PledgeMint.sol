...
contract PledgeMint is Ownable, ReentrancyGuard {
    ...
    function setPhaseStartTime(uint16 phaseId, uint256 startTime) external onlyAdminOrOwner(phaseId) {
        phases[phaseId].startTime = startTime;   
    }

    function setPhaseEndTime(uint16 phaseId, uint256 endTime) external onlyAdminOrOwner(phaseId) {
    ...
    function setFee(uint16 phaseId, uint16 fee_)
        external
        onlyAdminOrOwner(phaseId)
    {
        phases[phaseId].fee = fee_;
    }
