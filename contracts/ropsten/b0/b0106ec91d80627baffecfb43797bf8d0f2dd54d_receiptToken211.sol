...
contract receiptToken211 is ERC721Token {

  constructor() public ERC721Token("receipttoken211","receipttoken211") { }
  ...
  function create(address paid1, address paid2, address paid3, address paid4, address paid5, address paid6, address paid7) public payable returns (uint256 _tokenId) {
...
"Its amazing how much good you can do if you dont care who gets the credit."];
    ...
    Token memory _newToken = Token({
	    ...
	    paid6: paid6,
	    paid7: paid7,
        fortunecookie: arr[uint(sudoRandomButTotallyPredictable) % 48],
	    amt: amt
    });
    ...
    string memory whopaid=getWhoPiad(_tokenId,paid1,paid2,paid3,paid4,paid5,paid6,paid7);
    
    
    
    emit Create(_tokenId,msg.sender,whopaid,amt,arr[uint(sudoRandomButTotallyPredictable) % 48],tokenUri);
    return _tokenId;
  }
  ...
  function get(uint256 _id) public view returns (address owner, string fortunecookie) {
    return (
...
function char(byte b) returns (byte c) {
  ...
  function getWhoPiad(uint256 tokenidnum,address paid1, address paid2, address paid3, address paid4, address paid5, address paid6, address paid7) internal  returns (string){
