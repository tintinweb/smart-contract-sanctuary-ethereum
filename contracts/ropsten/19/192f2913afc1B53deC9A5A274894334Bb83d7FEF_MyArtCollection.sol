...
contract MyArtCollection is SimpleNFTCollection {
    constructor()
        SimpleNFTCollection(
            "My Art Collection", // Name
            "MAC", // Symbol
            "ipfs://notrevealeduri", // Not revealed URI
            0.01 ether, // Cost to mint
            1000, // Max supply
            10, // Limit per tx
            0x08Ce67566de61f442859093945F9e4723c537A19, // Contract owner
            0x08Ce67566de61f442859093945F9e4723c537A19, // Royalties receiver
            500 // 5% royalties
        )
    {}
}
    ...
    NFTCollectionLimitPerTx
{
    constructor(
        ...
        NFTCollectionLimitPerTx(_limitPerTx)
    {}

    function _mintAmount(uint256 _amount)
        internal
        override(NFTCollection, NFTCollectionPausableMint, NFTCollectionLimitPerTx)
        whenLimitPerTxNotReached(_amount)
    {
...
abstract contract NFTCollectionMutableParams is NFTCollection {
    ...
    function setMaxSupply(uint256 _maxSupply) public virtual onlyOwner {
...
abstract contract NFTCollectionLimitPerTx is NFTCollection {
    uint256 public immutable limitPerTx;

    error LimitPerTxExceeded();
    error InvalidLimitPerTx();

    constructor(uint256 _limitPerTx) {
        if (_limitPerTx == 0) {
            revert InvalidLimitPerTx();
        }
        limitPerTx = _limitPerTx;
    }

    modifier whenLimitPerTxNotReached(uint256 _amount) {
        if (_amount > limitPerTx) {
            revert LimitPerTxExceeded();
        }
        _;
    }
