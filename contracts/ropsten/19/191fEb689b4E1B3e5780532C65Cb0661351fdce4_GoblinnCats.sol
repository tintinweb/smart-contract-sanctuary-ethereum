...
contract GoblinnCats is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    function canWhitelistMint() public view returns(bool) {
        if ( totalWlMintedNumber + 3 <= MAX_WL_SUPPLY  && totalSupply() + 3 <= MAX_SUPPLY ) return true;
        else return false;
    }

    function canFreeMint() public view returns(bool) {
        if ( totalSupply() < MAX_SUPPLY ) return true;
        else return false;
    }

    function canPublicMint() public view returns(bool) {
        if ( block.timestamp <= wlMintEndTimeStamp ) {
            if ( MAX_WL_SUPPLY - totalWlMintedNumber + totalSupply() < MAX_SUPPLY ) return true;
            else return false;
        } else {
            if ( totalSupply() < MAX_SUPPLY) return true;
            else return false;
