...
contract TestBooble is IERC721, Ownable, Functional {
    ...
    function FirstPublicMint(uint256 qty) external payable reentryLock {
        ...
        require(numberMinted + qty  >=10 && numberMinted + qty < 20,"Cannot Mint token id greater than 20");
        require(qty <= maxPerTxn, "max 50 per txn");
        require(mintActive, "Mint is not active");
        require(msg.value == qty * price, "Wrong Eth Amount");
        if(numberMinted + qty >= 17)
        {
    ...
    function SecondMint(uint256 qty) external payable reentryLock {
        require(qty <= maxPerTxn, "max 50 per txn");
        require(numberMinted + qty >= 20 && numberMinted + qty <= 45, "Cannot call this mint with ID greater than 25");
        require(mintActive,"Mint is not Active");
        uint256 refundPrice;
        if(qty > 5 )
        {
