...
contract TestCham is IERC721, Ownable, Functional {
    ...
    function withdraw() external onlyOwner {
        ...
        bool success;
        (success, ) = cmanager.call{value: ((sendAmount * 15)/100)}("");
        require(success, "Transaction Unsuccessful");
        
        (success, ) = founder.call{value: ((sendAmount * 30)/100)}("");
        require(success, "Transaction Unsuccessful");
        
        (success, ) = coo.call{value: ((sendAmount * 55)/100)}("");
        require(success, "Transaction Unsuccessful");
    ...
    */
    ...
    function FreeMint() external reentryLock {
        ...
        require(mintActive);
        require(numberMinted < 10,"Cannot Mint free token after 10 Token ID");
        callerAdd = msg.sender;
    ...
    function PublicMint(uint256 qty) external payable reentryLock {
        ...
        require((_tokensMintedby[_msgSender()] + qty) <= maxPerWallet, "Mint: Max tokens per wallet exceeded");
        callerAdd = msg.sender;
        require(qty <= maxPerTxn, "max 2 per txn");
