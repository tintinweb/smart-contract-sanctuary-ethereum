...
contract NyxDAO is ReentrancyGuard, AccessControl, Ownable {
    ...
    mapping(address => uint256[]) private stakeholderInvestmentVotes;
    mapping(address => uint256[]) private stakeholderRevenueVotes;
    mapping(address => uint256[]) private stakeholderGovernanceVotes;
    mapping(address => uint256[]) private stakeholderAllocationVotes;
    mapping(address => uint256[]) private stakeholderFreeVotes;
    ...
    function setProposalConf(ProposalType proposalType, uint256 proposalId, bytes memory encodedProposal)
        public
    {
        if (proposalType == ProposalType.Investment)
        {
        ...
        onlyStakeholder("Only stakeholders are allowed to vote")
    {
        ...
        votable(proposalType, conf);
        stakeholderVotes[proposalType][msg.sender].push(conf.id);
        ...
        setProposalConf(proposalType, proposalId, abi.encode(conf));
    }

    function votable(ProposalType proposalType, ProposalConf memory proposalConf)
        private view
    {
        ...
        uint256[] memory tempVotes = stakeholderVotes[proposalType][msg.sender];

        for (uint256 iVote = 0; iVote < tempVotes.length; iVote++)
        {
            if (proposalConf.id == tempVotes[iVote])
            {
        ...
        onlyStakeholder("Only stakeholders are allowed to settle proposals")
    {
        ...
        setProposalConf(proposalType, proposalId, abi.encode(proposalConf));
    }
