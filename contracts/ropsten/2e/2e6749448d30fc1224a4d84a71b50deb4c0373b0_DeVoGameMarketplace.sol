...
// File: contracts/DeVoMarketplace.sol
 ...
 */
contract DeVoGameMarketplace is AccessControl, Pausable, ERC1155Holder, KeeperCompatibleInterface {
    ...
    /** Address of the 1155 contract. */
    IERC1155 public devo1155;
    ...
    uint16 public buyFixedFee = 100;
    uint16 public buyAuctionFee = 100;
    uint16 public sellFixedFee = 100;
    uint16 public sellAuctionFee = 100;
    ...
    constructor(
        ...
        devo1155 = IERC1155(_nftContract);

        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);

        feeReceiver1 = msg.sender;
        feeReceiver2 = msg.sender;
    }
  ...
  */
    ...
    function setFees(
        ...
        address newFeeReceiver2,
        uint16 _buyFixedFee,
        uint16 _buyAuctionFee,
        uint16 _sellFixedFee,
        uint16 _sellAuctionfee
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {

        feeReceiver1 = newFeeReceiver1;
        feeReceiver2 = newFeeReceiver2;
        buyAuctionFee = _buyAuctionFee;
        buyFixedFee = _buyFixedFee;
        sellAuctionFee = _sellAuctionfee;
        sellFixedFee = _sellFixedFee;
    }


    function listFixedPriceBatchperItem(
        uint256 itemId,
        uint256 amount,
        uint256 basePrice
    ) external whenNotPaused notZero(basePrice) {
        uint256[] memory Id = new uint256[](amount);
        uint256[] memory Amount = new uint256[](amount);
        for (uint8 i = 0; i < amount; i++) {
            Id[i] = itemId;
            Amount[i] = 1;
        }
        listFixedPriceBatch(Id, Amount, basePrice);
    }


    function listAuctionBatchperItem(
        uint256 itemId,
        uint256 amount,
        uint256 basePrice,
        uint256 bidStep
        ...
        listAuctionBatch(Id, Amount, basePrice, bidStep);
    }
   ...
   /** @notice Lists user item with a fixed price on the marketplace.
     ...
     *
     * @param itemId Item ID array.
     * @param basePrice Price of the item.
     */
    function listFixedPriceBatch(
        uint256[] memory itemId,
        uint256[] memory amount,
        uint256 basePrice
    ) internal whenNotPaused notZero(basePrice) {
        uint256 len = itemId.length;
        for (uint8 i = 0; i < len; i++) {
            _addOrder(itemId[i], amount[i], basePrice, 0, OrderType.FixedPrice);
        }
    }
   ...
   /** @notice Lists user auction item on the marketplace.
     ...
     */
    function listAuctionBatch(
        uint256[] memory itemId,
        uint256[] memory amount,
        uint256 basePrice,
        uint256 bidStep
    ) internal whenNotPaused notZero(basePrice) notZero(bidStep) {
        uint256 len = itemId.length;
        for (uint8 i = 0; i < len; i++) {
            _addOrder(
                itemId[i],
    ...
    function listFixedPrice(
        ...
        _addOrder(itemId, amount, basePrice * amount, 0, OrderType.FixedPrice);
    }
    ...
    function _addOrder(
        ...
        _transferItem(msg.sender, address(this), itemId, itemAmount);
     ...
     */
    function buyOrder(uint256 orderId, bool _tokenUsed)
        external
        payable
        whenNotPaused
    {
        ...
        // Transfer NFT to `msg.sender` and BNB to order maker
        _exchange(
            orderId,
            order.itemId,
            order.itemAmount,
        ...
        if (_tokenUsed) {
            uint256 amount = getPriceInToken(order.basePrice, _token);
            uint256 sellFees = (amount * sellFixedFee) / 1000;
            IERC20(_token).transferFrom(
                msg.sender,
                order.maker,
                amount - sellFees
            );
            uint256 buyFees = (amount * buyFixedFee) / 1000;
            ...
            uint256 buyFees = (order.basePrice * buyFixedFee) / 1000;
            uint256 sellFees = (order.basePrice * sellFixedFee) / 1000;
            require(
                order.basePrice + buyFees == msg.value,
                "Not enough amont paid"
            );
            uint256 totalFees = buyFees + sellFees;
            payable(order.maker).transfer(msg.value - buyFees);

            payable(feeReceiver1).transfer((totalFees * 30) / 100);
            payable(feeReceiver2).transfer((totalFees * 70) / 100);
        }
     ...
     *
     * @param orderId Order ID.
     * @param bidAmount Amount in BNB tokens.
     */
    function makeBid(
        uint256 orderId,
        uint256 bidAmount,
        ...
        address lastBidder = lastBid.bidder;
        order.numBids++;

        if (_tokenUsed) {
            uint256 amount = getPriceInToken(bidAmount, _token);
            uint256 fees = (amount * buyAuctionFee) / 1000;
            IERC20(_token).transferFrom(
                msg.sender,
                address(this),
                amount + fees
            ...
            uint256 fees = (bidAmount * buyAuctionFee) / 1000;
            require(msg.value == bidAmount + fees, "Not enough amont paid");
            bids[orderId][order.numBids] = Bid({
                amount: bidAmount,
                bidder: msg.sender,
                isToken: false
        ...
        if (numBids > 0) {
            if (lastBid.isToken) {
                uint256 refund = getPriceInToken(lastBid.amount, _token);
                uint256 fees = (refund * buyAuctionFee) / 1000;
                uint256 amount = refund + fees >
                    IERC20(_token).balanceOf(address(this))
                    ? IERC20(_token).balanceOf(address(this))
                    : refund + fees;
                IERC20(_token).transfer(lastBidder, amount);
            } else {
                uint256 fees = (lastBid.amount * buyAuctionFee) / 1000;
                payable(lastBidder).transfer(lastBid.amount + fees);
            }
        }
    ...
    function finishAuction(uint256 orderId) external whenNotPaused {
        ...
        if (numBids > 2) {
            ...
            if (lastBid.isToken) {
                uint256 tAmount = getPriceInToken(lastBid.amount, _token);
                uint256 buyFees = (tAmount * buyAuctionFee) / 1000;
                uint256 sellFees = (tAmount * sellAuctionFee) / 1000;
                uint256 amount = tAmount + buyFees + sellFees >
                    IERC20(_token).balanceOf(address(this))
                    ? IERC20(_token).balanceOf(address(this))
                    : tAmount;
                IERC20(_token).transfer(order.maker, amount - sellFees);
                uint256 totalFees = buyFees + sellFees;
                IERC20(_token).transfer(feeReceiver1, (totalFees * 30) / 100);
                IERC20(_token).transfer(feeReceiver2, (totalFees * 70) / 100);
            } else {
                uint256 buyFees = (lastBid.amount * buyAuctionFee) / 1000;
                uint256 sellFees = (lastBid.amount * sellAuctionFee) / 1000;
                uint256 totalFees = buyFees + sellFees;

                payable(order.maker).transfer(lastBid.amount - totalFees);
                payable(feeReceiver1).transfer((totalFees * 30) / 100);
                payable(feeReceiver2).transfer((totalFees * 70) / 100);
            }
            _cancelOrder(orderId, true);
        } else {
            // Return Devo to the last bidder
            if (numBids > 0) {
                if (lastBid.isToken) {
                    uint256 tAmount = getPriceInToken(lastBid.amount, _token);
                    uint256 buyFees = (tAmount * buyAuctionFee) / 1000;
                    ...
                    uint256 amount = tAmount + buyFees + sellFees >
                        ...
                        ? IERC20(_token).balanceOf(address(this))
                        : tAmount;
                    IERC20(_token).transfer(lastBid.bidder, amount - sellFees);
                    uint256 totalFees = buyFees + sellFees;
                    ...
                    payable(lastBid.bidder).transfer(
                        lastBid.amount - totalFees
                    );
                    payable(feeReceiver1).transfer((totalFees * 30) / 100);
                    payable(feeReceiver2).transfer((totalFees * 70) / 100);
                }
    ...
    /** @notice Exchanges DEVO tokens and Items between users.
     * @dev `payer` here is either `itemRecipient` or `address(0)`
     * which means that we should transfer DEVO from the contract.
     *
     * @param orderId Order ID.
     * @param itemId Item ID.
     * @param price Item price in DEVO tokens.
    ...
    function _exchange(
        ...
        _transferItem(address(this), itemRecipient, itemId, itemAmount);
    ...
    function _transferItem(
        ...
        devo1155.safeTransferFrom(from, to, itemId, itemAmount, "");
    }
   ...
   /** @notice Checks if item is currently listed on the marketplace.
    ...
    function getPriceInToken(uint256 amount, address token)
        internal
        view
    ...
    {
        (Order[] memory openOrders, ) = this.getOpenOrders();

        upkeepNeeded = openOrders.length > 0;
    }

    function performUpkeep(
        bytes calldata /* performData */
    ) external override {
        (, uint256[] memory openIds) = this.getOpenOrders();

        if (openIds.length > 0) {
            for (uint256 i = 1; i <= openIds.length; i++) {
//                this.finishAuction(openIds[i]);
                emit AuctionFinished(openIds[i], openIds.length);
            }
        }
    }
