...
contract RNFTGeneric is ERC1155 {
    ...
    function addValidator(address user) external OnlyOwner{
        validator[user] = true;
    }
    ...
    function setURI(string memory newuri, uint256 tokenId) external OnlyOwner {
        RNFTProjectDetails[tokenId].uri = newuri; 
    }
    ...
    function getRNFTHolders(uint256 RNFTId)
        public
        view
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        return (RNFTProjectDetails[tokenId].uri);
    }

    function createRNFTRequest(
        uint256 id,
        string memory uri,
        uint256 price
    ) public {
    ...
    function buyRNFT(uint256 amount, uint256 tokenId) external payable {
        require( RNFTProjectDetails[tokenId].status == RNFTStatus.CREATED);
