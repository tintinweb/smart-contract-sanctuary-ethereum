...
contract SampleToken is ERC721, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721("SampleToken", "STK") {
        ...
        uint64 private premintMinted  = 0;
    uint64 private premitLimitPerWallet = 3;
    uint64 private premintTotalSupplyLimit = 5;
    uint64 public premintStagePrice = 0.1 ether;

    uint128 private publicMintStageLimitPerWallet = 4;
    uint128 public publicMintStagePrice = 0.2 ether;

    uint public TotalSupply = 9; //tokenID should be <= TotalSupply
        ...
        whenNotPaused
    {
        ...
        require(isPremintStageActive, "Premint not yet active");
        require(
    ...
    function ownerMintToken(uint256 _tokenID)
        public
        payable
        onlyOwner
    {
...
onlyOwner
{
  ...
  function publicMintToken() payable public {
      require(msg.value == 0.002 ether,"Incorrect Price");
        uint currentPlusPlus = _tokenIdCounter.current()+1;
        require(currentPlusPlus <= TotalSupply, "Total Supply Exhausted");
        tokenID2Owner[_tokenIdCounter.current()] = msg.sender;
        _safeMint(msg.sender, _tokenIdCounter.current());
        _tokenIdCounter.increment();
    }


    function mintToken(uint256 _tokenID) private {
