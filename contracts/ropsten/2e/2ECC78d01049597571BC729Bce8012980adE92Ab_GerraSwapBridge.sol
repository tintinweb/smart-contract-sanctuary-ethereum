...
contract GerraSwapBridge is Context {
    ...
    uint256 public feePercentageInGerra;
    ...
    event SwapFilled(
        address indexed bep20Addr,
        bytes32 indexed otherChainTxHash,
    ...
    constructor(
        uint256 fee_Native,
        uint256 fee_PerGerra,
        address payable fee_Receiver,
        address super_Admin
    ) {
        swapFee = fee_Native;
        feePercentageInGerra = fee_PerGerra;
    ...
    /**
     * Returns set minimum swap fee in TRR from BEP20 to other chains
     */
    function setSwapFeePercentageOfTRR(uint256 _feePerGerra)
        external
        onlyOwner
    {
        require(
            _feePerGerra < 100000000000000000000,
            "feePercentageInGerra: Greater than 100 %"
        );
        feePercentageInGerra = _feePerGerra;
    }

    /**
     * Register swap pair for chain
     */
    function registerSwapPair(address bep20Addr, string calldata chain)
        external
        onlyOwner
        returns (bool)
    {
        require(
            !registeredBEP20[string(abi.encode(bep20Addr, chain))],
            "already registered"
        );
        ...
        registeredBEP20[string(abi.encode(bep20Addr, chain))] = true;
    ...
    function fillSwap(
        ...
        IERC20(bep20Addr).safeTransfer(toAddress, amount);
        filledOtherChainsTx[crossChainTxHash] = true;

        emit SwapFilled(bep20Addr, crossChainTxHash, toAddress, amount);
        return true;
    }
    ...
    function swapToken(
        ...
        require(amount > 0, "Amount should be greater than 0");
        require(
            feePercentageInGerra < 100000000000000000000,
            "feePercentageInGerra: Greater than 100 %"
        );

        uint256 feeAmountInTrr = 0;
        if (feePercentageInGerra > 0) {
            feeAmountInTrr = amount.mul(feePercentageInGerra);
            feeAmountInTrr = feeAmountInTrr.div(100000000000000000000);
            amount = amount.sub(feeAmountInTrr);
            IERC20(bep20Addr).safeTransferFrom(
                msg.sender,
                feeReceiver,
                feeAmountInTrr
            );
     ...
     */
    function getRegisteredPairs(address bep20Addr, string calldata chain)
        external
        view
        returns (bool)
    {
        return registeredBEP20[string(abi.encode(bep20Addr, chain))];
    }
}
