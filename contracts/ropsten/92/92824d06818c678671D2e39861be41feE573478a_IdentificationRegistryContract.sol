...
abstract contract VotesUpgradeable is Initializable, IVotesUpgradeable, ContextUpgradeable, EIP712Upgradeable {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) private _delegatedIndex;
    ...
    function foo(uint256 delegatee, uint256 tokenId) public view returns (uint256) {
        return _delegatedIndex[delegatee][tokenId];
    }
    ...
    function delegate(uint256 tokenId, uint256 delegatee) public virtual {
        ...
        //return all delegated votes if it have votes delegated to him
    ...
    function _delegate(uint256 tokenId, uint256 delegatee, uint256 votingUnits) internal virtual {
        ...
        // update delegatee delegated and its index map
        _delegatedIndex[delegatee][tokenId] = _delegated[delegatee].length;
    ...
    function _deleteTokenIdFromDelegatedArray(uint256 tokenId, uint256 tokenIdToRemove) internal {
        if(tokenId != tokenIdToRemove) {
            console.log("HOLAAAA");
            uint256 lastTokenIndex = _delegated[tokenId].length - 1;
            uint256 tokenIndex = _delegatedIndex[tokenId][tokenIdToRemove];
            // When the token to delete is the last token, the swap operation is unnecessary
            if (tokenIndex != lastTokenIndex) {
                uint256 lastTokenId = _delegated[tokenId][lastTokenIndex];
                _delegated[tokenId][tokenIndex] = lastTokenId; // Move the last token to the slot of the to-delete token
                _delegatedIndex[tokenId][lastTokenId] = tokenIndex; // Update the moved token's index
            }
            delete _delegatedIndex[tokenId][tokenIdToRemove];
            _delegated[tokenId].pop();
        }
    }
