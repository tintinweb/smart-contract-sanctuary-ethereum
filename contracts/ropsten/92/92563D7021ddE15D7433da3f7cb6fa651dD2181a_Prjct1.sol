...
contract Prjct1 is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    uint256 public constant MAXSUPPLY = 777;
    mapping(address => uint256) public addressCheck;
    address private signerAddress = 0x2C8d36360E9f288246ebfF75133d5a836114Ad59;
    address public mainAddress = 0xaE8f989645Af51BBaA762E1F4d09Ad63E3b3A129;
   ...
   function splitSignature(bytes memory sig)
    ...
    nonReentrant
    {
        ...
        require(preSaleControl == true, "Prjct1: Presale is not started yet!");
        require(hashMessage(msg.sender) == messageHash, "MESSAGE_INVALID");
        require(
            isValidData(messageHash, signature),
            "SIGNATURE_VALIDATION_FAILED"
        );
        require(msg.value >= price * ammount, "Prjct1: Insuficient funds");
    ...
    nonReentrant
    {
        ...
        require(ogMintControl == true, "Prjct1: Ogmint is not started yet!");
        require(ammount <= 1, "Prjct1: Ogmint mint is one token only.");
        require(addressCheck[msg.sender] < 1, "Prjct1: Ogmint mint is one token only.");
        require(hashMessage(msg.sender) == messageHash, "MESSAGE_INVALID");
        require(
            isValidData(messageHash, signature),
            "SIGNATURE_VALIDATION_FAILED"
    ...
    function publicSaleMint(uint256 ammount) public payable nonReentrant {
        ...
        require(!saleFinishControl, "Prjct1: SOLD OUT!");
        require(publicSaleControl, "Prjct1: public sale is not started yet");
        require(msg.value >= price * ammount, "Prjct1: Insuficient funds");
        require(ammount <= 20, "Prjct1: You can only mint up to 20 token at once!");
        require(supply + ammount <= publicSaleLimit(), "Prjct1: Mint too large!");
