...
contract BankiFi {
    ...
    function matchBorrowerLender(uint256 id, address _NFTContract, string memory uri, uint256 _timeMonths) public{
        ...
        uint256 lockedAsset = this.swapExactInputSingle(amount.mul(2));
        ...
        // Update the states. // Interest Rate is kept at 10%.
        uint256 temp3 = (_borrowerList[id].amountDeposit.div(10)).mul((11**_timeMonths));
        uint256 temp5 = (11**_timeMonths).sub((10**_timeMonths));
        ...
        _borrowerList[id].lockedWETH = lockedAsset;
        ...
        _lenderList[i].lockedWETH = lockedAsset;
    }    
    ...
    function CorrectInstallment(uint256 id) public{
        ...
        if(_lenderList[id].lastBlockTimeStamp >= 2592000 && _lenderList[id].lastBlockTimeStamp < 5184000){
            // Revise installment Amount.
            uint256 temp1 = _borrowerList[_borrowerId].installmentAmount;
            uint256 temp2 = temp1.mul(3);
            uint256 penalInstallment = temp2.div(2);
            _borrowerList[_borrowerId].installmentAmount = penalInstallment ; // Replace with a formula afterwards.
            _lenderList[id].installmentAmount = penalInstallment;
        }

        if(_lenderList[id].lastBlockTimeStamp >= 5184000){
            // Do the closure and transfer the weth - installment Paid to the lender.
            uint256 WETHbalance = IERC20(WETH).balanceOf(address(this));

            uint256 closureAmount = (_lenderList[id].lockedWETH).mul(_lenderList[id].installmentAmount.mul(_lenderList[id].installmentsLeft)).div(_lenderList[id].amountDeposit); // Replace with formula AmountDeposit - installmentAmount*(TimeMonths - InstallmentsLeft)
