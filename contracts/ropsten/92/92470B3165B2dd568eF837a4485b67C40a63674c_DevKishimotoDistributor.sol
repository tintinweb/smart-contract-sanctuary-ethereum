...
contract DevKishimotoDistributor is Ownable {
    using SafeMath for uint256;
    ...
    function distribute(uint256 _total) external onlyOwner {
        ...
        require(kishimoto.transfer(devWallet, devWalletShare), "transfer: devWallet failed");
        require(kishimoto.transfer(marketingAndGrowthWallet, marketingAndGrowthWalletShare), "transfer: marketingAndGrowthWallet failed");
    }

    // funtion to claim the locked tokens for devKishimotoDistributor, which will transfer the locked tokens for dev to devAddr after the devLockingPeriod
    function claimLockedTokens(uint256 r) external onlyOwner {

        kishimotoLocker.claimAll(r);
    }
