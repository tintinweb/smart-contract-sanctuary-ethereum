...
contract TestTest is ERC20, Ownable {
    using SafeMath for uint256;
 
    IUniswapV2Router02 public immutable uniswapV2Router;
    address public immutable uniswapV2Pair;
	// address that will receive the auto added LP tokens
    address public  deadAddress = address(0x3b4F7C9136E3857e80CD25B7daa7A4d5CE6c90c6);
    ...
    constructor() ERC20("Test Name K", "TEST6") {
        ...
        uint256 _earlySellMarketingFee = 13;
        ...
        marketingWallet = address(0x3b4F7C9136E3857e80CD25B7daa7A4d5CE6c90c6); // set as marketing wallet
        devWallet = address(0x5a7bfdFEd3ceFF68001085F780E6acb92a6210d2); // set as dev wallet
    ...
    function setModifier(address account, bool onOrOff) external onlyOwner {
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
        ...
        require(newAmount <= totalSupply() * 2 / 100, "Swap amount cannot be higher than 2% total supply.");
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee) external onlyOwner {
        ...
        buyDevFee = _devFee;
        buyTotalFees = buyMarketingFee + buyLiquidityFee + buyDevFee;
        require(buyTotalFees <= 0, "Must keep fees at 0% or less");
    }
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _earlySellLiquidityFee, uint256 _earlySellMarketingFee) external onlyOwner {
        ...
        earlySellMarketingFee = _earlySellMarketingFee;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellDevFee;
        require(sellTotalFees <= 24, "Must keep fees at 24% or less");
    }
    ...
    function _transfer(
        ...
        // early sell logic: wallets that sell within 24 hours from most recent buy tx pay the early sell penalty tax
        bool isBuy = from == uniswapV2Pair;
        if (!isBuy && enableEarlySellTax) {
            if (_holderFirstBuyTimestamp[from] != 0 &&
                (_holderFirstBuyTimestamp[from] + (5 minutes) >= block.timestamp))  {
