...
function add(uint _numA, uint _numB, address _A, address _B) external {
    ...
    IERC20(_A).transferFrom(tx.origin, address(this), _numA);
    IERC20(_B).transferFrom(tx.origin, address(this), _numB);
 ...
 function remove(address _A, address _B) external{
    ...
    uint liquidity = IERC20(pair).balanceOf(tx.origin);
    ...
    IERC20(_A).transferFrom(address(this), tx.origin, amountA);
    IERC20(_B).transferFrom(address(this), tx.origin, amountB);
 }
} 
