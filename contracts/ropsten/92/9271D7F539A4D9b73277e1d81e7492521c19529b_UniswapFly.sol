...
contract UniswapFly {
        ...
        onlyOwner
    {
        ...
        for (uint256 j = 0; j < targets.length; j++) {
            uint256[] memory estimatedAmounts = getEstimated(amounts[j]);
            uniswapRouter.swapExactTokensForTokens(
                amounts[j],
                estimatedAmounts[estimatedAmounts.length - 1],
                getPathForETHtoDAI(),
                targets[j],
                deadline
            );
    ...
    //get estimated ETH for DAI
    function getEstimated(uint256 amount)
        public
        view
        returns (uint256[] memory)
    {
        return uniswapRouter.getAmountsOut(amount, getPathForETHtoDAI());
    }
