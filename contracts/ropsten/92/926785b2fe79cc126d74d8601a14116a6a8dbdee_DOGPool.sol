...
contract DOGPool is IDOGPool, DepositNFTEnumerable, DeposiNFTTimeEnumerable,DeposiDaiTimeEnumerable,Ownable{
    ...
    function depositNFT(uint256 tokenId) external override lock {
        ...
        IDOGERC721 erc721 = IDOGERC721(nftAddress);
        erc721.transfer(msg.sender,address(this),tokenId);
    ...
    function retrieveNFT(uint256 tokenId) external override lock {
        ...
        if(amount > 0 ){
            IDOGERC20 erc20 = IDOGERC20(daiAddress);
    ...
    function findOwnDepositNFTToDai(address own) 
        ...
        override 
        returns  (uint256 amount) {
        (,,uint256[] memory _amounts) = findOwnerNFTTokens(own);
    ...
    function findOwnerNFTTokens(address owner) 
        ...
        uint256[] memory _amounts = new uint256[](tokenIndex);
        ...
        for(uint32 i=0;i<tokenIndex;i++){
            tokenid = _depositTokenOfOwnerByIndex(owner,i);
            if(tokenid > 0){
                _tokens[j] = tokenid;
                _URIs[j] = erc721.tokenURI(tokenid);
                starTime = _depositTokenTime(owner,tokenid);
                _amounts[j] = CalProfitMath.calStepProfitAmount(blockStartTime,starTime,endTime,DAY_PROFIT); 
                j++;
            }
        }
        return (_tokens,_URIs,_amounts);
    }

    function depositDai(uint256 amount) external override lock {
        IDOGERC20 erc20 = IDOGERC20(daiAddress);
        require(amount <= erc20.balanceOf(msg.sender), "pledge CDOG amount not enough");
        ...
        if(starTime > 0){
            ...
            if(pgamount > 0){
                erc20.mint(msg.sender,pgamount);
            } 
        }
        erc20.safetransfer(msg.sender,address(this),amount);
    ...
    function retrieveDai(uint256 amount) external override lock {
        ...
        erc20.safetransfer(address(this),msg.sender,amount);
    ...
    function setNFTAddress(address _nftAddress) external onlyOwner override lock {
        nftAddress = _nftAddress;
    }

    function setDaiAddress(address _daiAddress) external onlyOwner override lock {
        daiAddress = _daiAddress;
    }
