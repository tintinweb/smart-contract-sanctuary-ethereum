...
contract DJPToken is owned, TokenERC20  {
	...
	string _tokenName="DJtest";
	string _tokenSymbol="DJT";
	address public lockedWallet = 0x6896E8bCfB0163a81B58bE5f639BA5A2E3499E9a;
	...
	function checkLockedBalance(address wallet, uint256 _value) internal returns (bool){
		if(wallet==lockedWallet){
			...
			if(now<startTime + 365 * 1 seconds){ //15% tokens locked first year
				return balanceOf[lockedWallet].sub(_value)>=totalSupply.mul(15).div(100)? true : false;
			}else if(now>=startTime + 365 * 1 seconds && now<startTime + 730 * 1 seconds){ //13% tokens locked second year
				return balanceOf[lockedWallet].sub(_value)>=totalSupply.mul(13).div(100)? true : false;
			}else if(now>=startTime + 730 * 1 seconds && now<startTime + 1095 * 1 seconds){ //10% tokens locked third year
				return balanceOf[lockedWallet].sub(_value)>=totalSupply.mul(10).div(100)? true : false;	
			}else if(now>=startTime + 1095 * 1 seconds && now<startTime + 1460 * 1 seconds){ //6% tokens locked fourth year
