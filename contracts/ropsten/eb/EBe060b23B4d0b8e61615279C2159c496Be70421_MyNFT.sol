...
//  _______ ___ ___ ___ ___      ______ _______ ______ _______ _______ _______ _____        ______ _____   _______ ______
// |    |  |   |   |   |   |    |      |   _   |   __ \_     _|_     _|   _   |     |_     |      |     |_|   |   |   __ \
// |       |\     /|-     -|    |   ---|       |    __/_|   |_  |   | |       |       |    |   ---|       |   |   |   __ <
// |__|____| |___| |___|___|    |______|___|___|___|  |_______| |___| |___|___|_______|    |______|_______|_______|______/
...
contract MyNFT is  ERC1155Supply, Ownable {
    uint256 NYX_NFT_ID = 0;
    string NFT_METADATA_URL = "https://ipfs.io/ipfs/QmRYmbDR6kGKaYBHQLPthKdFmuVZeXFDKwpU8XT1pAoQ2P?filename=nyx.json";
    ...
    uint256 public constant _MAX_HOLDING = 2;
    uint256 public constant _MAX_HOLDING_WHITELIST = 5;
    uint256 public constant _MAX_HOLDING_SEC_MKT = 20;
    uint256 public constant _MAX_WHITELISTED = 5555;
    ...
    string temporaryMetadataURI = "";       // When minted, all NFTs will have this URI setted as their token URI for seconds. When mint is successfull, we will cal updateTokenURI on our web backend to set a randomly selected NFT URI.

    constructor() ERC1155(NFT_METADATA_URL)
    {
        for (uint256 iAddress = 0; iAddress < _TEAM_ADDRESSES.length; iAddress++)
        {
            address teamAddress = _TEAM_ADDRESSES[iAddress];
            giveaway(1, teamAddress);
        ...
        returns (uint256)
    {
        if (_isMinting)
        {
            return super.balanceOf(owner, NYX_NFT_ID);
        }
        else
        {
            return presaleAmount[owner];
        }
    }
        ...
        public onlyOwner
    {
        if (!_isMinting)
        {
            ...
            {
                address whiteListedAddress = whitelist[i];
                uint amount = presaleAmount[whiteListedAddress];
                _mint(whiteListedAddress, NYX_NFT_ID, amount, "");
            }
    ...
    function _mintNFT(uint256 nMint, address recipient)
        private
    {
        require(totalSupply(NYX_NFT_ID) + nMint <= _MAX_SUPPLY, "No more NFT to mint");
        require(balanceOf(recipient) + nMint <= _MAX_HOLDING ||
                _TEAM_ADDRESSES_DICT[recipient] ||
                isWhitelist(recipient) && balanceOf(recipient) + nMint <= _MAX_HOLDING_WHITELIST,
                "Max holded NFTs is reached");
        _mint(recipient, NYX_NFT_ID, nMint, "");
        addHolder(recipient);
    }
    ...
    function mintNFT(uint256 nMint, address recipient)
        public payable
    {
        require(_isMinting || isWhitelist(recipient), "Mint period have not started yet and you are note Whitelisted");
        require(msg.value >= _PRICE * nMint, "Not enough ETH to mint");

        if (isWhitelist(recipient))
        {
            return mintNFTpresaleNoDelivery(nMint, recipient);
        }
    ...
    // Presale Mint
    function mintNFTpresale(uint256 nMint, address recipient)
        public payable
    {
        require(!_isMinting, "Minting have started, ad we are not in Presale period anymore. Please use the regular mintNFT function");
        require(isWhitelist(recipient), "Have to be whitelisted to mint");
        require(msg.value >= _PRICE * nMint, "Not enough ETH to mint");
        return _mintNFT(nMint, recipient);
    }

    // Presale Mint without delivery
    // function mintNFTpresaleNoDelivery(address recipient, string memory tokenURI)
    function mintNFTpresaleNoDelivery(uint256 nMint, address recipient)
        public payable
    {
        require(totalSupply(NYX_NFT_ID) + nMint <= _MAX_SUPPLY, "No more NFT to mint");
        require(!_isMinting, "Minting have started, ad we are not in Presale period anymore. Please use the regular mintNFT function");
        require(isWhitelist(recipient), "Have to be whitelisted to mint");
        require(msg.value >= _PRICE * nMint, "Not enough ETH to mint");
        require(_TEAM_ADDRESSES_DICT[recipient] ||
                isWhitelist(recipient) && balanceOf(recipient) + nMint <= _MAX_HOLDING_WHITELIST,
                "Max holded NFTs is reached");
        presaleAmount[recipient] += nMint;
        addHolder(recipient);
    }

    function burnNFT(address fromAddress, uint256 amount)
        public onlyOwner
    {
        _burn(fromAddress, NYX_NFT_ID, amount);
        if (balanceOf(fromAddress, NYX_NFT_ID) <= 0)
        {
            removeHolder(fromAddress);
        }
    }
    ...
    function addWhitelist(address addr)
        public onlyOwner
    {
        require(whitelist.length < _MAX_WHITELISTED, "Too much whitelisted address");
        require(getETHBalance(addr) > _PRICE, "Have to get enough ETH to be whitelisted");

        bool add_to_whitelist = true;
        ...
        private
    {
        ...
        delete whitelist[idx_to_delete];
    }

    function isWhitelist(address addr)
        public view
        returns (bool)
    {
        for (uint256 i = 0; i < whitelist.length; i++)
