...
//        uint b6 = IERC721(catAddress[Cat.Spock]).balanceOf(address(msg.sender));
        ...
        uint[7] memory counts = [b1,0,0,0,0,0,b7];

        return counts;
    }

    function getUserCatTokenIds() internal view
        returns(uint[] memory, uint[] memory, uint[] memory, uint[] memory, uint[] memory, uint[] memory, uint[] memory) {
        uint[] memory b1 = ICat(catAddress[Cat.SnoopDogg]).getUserTokens(address(msg.sender));
        uint[] memory b2;
        uint[] memory b3;
        uint[] memory b4;
        uint[] memory b5;
        uint[] memory b6;
        uint[] memory b7 = ICat(catAddress[Cat.ChanKwongYan]).getUserTokens(address(msg.sender));
