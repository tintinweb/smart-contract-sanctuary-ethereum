...
contract subsetSum {
    ...
    modifier restrictedAccess {
        require(now<expiryTime && authorisedEntrants[msg.sender]);
        _;
    }
    ...
    modifier winnerOnly {
        require(now>expiryTime && (msg.sender==leader.id || address(0)==leader.id || (now>expiryTime+2629746 && msg.sender==admin)));
        _;
    }
    ...
    function getPrizePot() public view returns (uint256 balance) {
    ...
    function authoriseEntrants(address[] addressesToAuthorise) public adminOnly {
        for (uint256 i = 0; i<addressesToAuthorise.length; i++) {
            authorisedEntrants[addressesToAuthorise[i]]=true;
        }
    }
