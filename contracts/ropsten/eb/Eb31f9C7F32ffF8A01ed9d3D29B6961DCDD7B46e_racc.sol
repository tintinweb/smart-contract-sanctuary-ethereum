...
contract racc is ERC721A, Ownable, ReentrancyGuard {
    ...
    mapping (address => uint8) addressPublicSaleCountMap;    // Up to 4
    ...
    function mintPublic(uint8 _mintCount) external payable nonReentrant returns (uint256) {
        ...
        require(mintStep == 2 && _mintCount > 0 && _mintCount <= LIMIT4);
        require(msg.sender != address(0));
        require(msg.value == (mintPrice * _mintCount));
        require(addressPublicSaleCountMap[msg.sender] + _mintCount <= LIMIT4);
    ...
    function setMintStep(uint8 _mintStep) external onlyOwner returns (uint8) {
        ...
        mintStep = _mintStep;
        emit Setting(mintStep, mintPrice, mintPriceDiscount, totalLimit, LIMIT4);
        return mintStep;
    }
