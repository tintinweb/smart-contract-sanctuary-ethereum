...
contract SpozzBridge is Ownable, Pausable, ReentrancyGuard {
    ...
    function swapStart(
        ...
        address to,
        uint amount
        ) public onlyActivatedChains(toChainID) whenNotPaused notContract nonReentrant onlyOwner {
        require(amount >= MIN_AMOUNT && amount <= maxAmount, "Wrong amount");
        //require(to == msg.sender, "Swap accepted only to same account");
        require(IERC20(rootToken).allowance(to, address(this)) >= amount, "Not enough allowance");
        _depositToken(to, amount); // transfer from.
        uint commission;
        ...
        //BVG07 fixed
        bytes32 eventHash = keccak256(abi.encode(_depositCount, _chainID, toChainID, to, to, amount));
        ...
        userEventMap[to] = eventHash;
        emit SwapStart(eventHash, _depositCount, toChainID, to, to, amount);
    }
    //BVG10 fixed
    function swapEnd(
        bytes32 eventHash,
    ...
    function _depositToken(address from, uint amount) private {
        IERC20(rootToken).safeTransferFrom(from, address(this), amount);
    }
