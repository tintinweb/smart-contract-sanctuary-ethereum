 ...
 */
abstract contract BlacklistFeature is ERC20Base {
    ...
    function addAddressesToBlacklist(address[] calldata addresses) external onlyRole(BLACKLIST_MANAGER_ROLE) {
        for(uint256 i; i < addresses.length; i++) {
            grantRole(BLACKLISTED_ROLE, addresses[i]);
        }
    }

    function removeAddressesFromBlacklist(address[] calldata addresses) external onlyRole(BLACKLIST_MANAGER_ROLE) {
        for(uint256 i; i < addresses.length; i++) {
 ...
 * @dev ERC20 token with a burn feature
 */
abstract contract ERC20BurnFeature is ERC20Base {
 ...
 */
abstract contract ERC20MintFeature is ERC20Base {
     ...
     */
    function bulkMint(address[] calldata accounts, uint256[] calldata amounts) public onlyRole(MINTER_ROLE) {
 ...
 * @dev ERC20 token with a mint control feature
 */
abstract contract PausableWithWhitelistFeature is ERC20Base, PausableUpgradeableRewrited {
    ...
    function addAddressesToWhitelistFrom(address[] calldata addresses) external onlyRole(PAUSE_MANAGER_ROLE) {
        for(uint256 i; i < addresses.length; i++) {
            grantRole(WHITELISTED_FROM_ROLE, addresses[i]);
        }
    }

    function removeAddressesFromWhitelistFrom(address[] calldata addresses) external onlyRole(PAUSE_MANAGER_ROLE) {
        for(uint256 i; i < addresses.length; i++) {
            revokeRole(WHITELISTED_FROM_ROLE, addresses[i]);
        }
    }

    function addAddressesToWhitelistSender(address[] calldata addresses) external onlyRole(PAUSE_MANAGER_ROLE) {
        for(uint256 i; i < addresses.length; i++) {
            grantRole(WHITELISTED_SENDER_ROLE, addresses[i]);
        }
    }

    function removeAddressesFromWhitelistSender(address[] calldata addresses) external onlyRole(PAUSE_MANAGER_ROLE) {
        for(uint256 i; i < addresses.length; i++) {
...
abstract contract SnapshotFeature is ERC20SnapshotRewrited {
...
// File contracts/templates/BurnTxFeeTemplate.sol
...
contract BurnTxFeeTemplate is ERC20Base, ERC20BurnFeature, TxFeeFeature {
    ...
    function _beforeTokenTransfer_hook(
        ...
        address to,
        uint256 amount
    )  internal virtual override(TxFeeFeature) {
        TxFeeFeature._beforeTokenTransfer_hook(from, to, amount);
    }
