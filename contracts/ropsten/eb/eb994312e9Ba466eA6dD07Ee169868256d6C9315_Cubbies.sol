...
contract Cubbies is Ownable, ERC721A, ReentrancyGuard {
    ...
    string public baseURI = "https://cubhouse.io/api/";
    string private _contractURI = "https://cubhouse.io/api/contract/";

    uint256 public presalePrice = 0.03 ether;
    uint256 public publicPrice = 0.05 ether;

    uint256 constant MAX_PUBLIC_MINT = 5;
    uint256 constant MAX_NFTS = 7777;
    ...
    function _internalMint(address destination, uint256 count) private {
        ...
        require(currentSupply + count <= MAX_NFTS, "Maximum Cubbies NFTs supply exceeded");
    ...
    function availableForPresale(address addr) public view returns (uint256) {
        return presaleList[addr];
    }

    function presaleMint(uint256 count) public payable {
        require(saleState == SaleState.Presale, "Sale is closed");

        uint256 reservedAmount = presaleList[msg.sender];
        require(reservedAmount > 0, "No reserved cubbies for address");
        ...
        require(msg.value == presalePrice * count, "Incorrect ETH to mint");
    ...
    function publicMint(uint256 count) public payable {
        ...
        require(count <= MAX_PUBLIC_MINT, string(abi.encodePacked("You can only mint ", MAX_PUBLIC_MINT.toString(), " or less at once")));
        require(msg.value == publicPrice * count, "Incorrect ETH to mint");
