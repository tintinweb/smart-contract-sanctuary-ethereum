...
contract Wrapper is Utils {
    ...
    function test_fourth(KyberNetwork network, ERC20 token) public view returns(address[]) {
        address[] memory result = new address[](2);
        result[0] = network.reservesPerTokenSrc(token, 0);
        result[1] = network.reservesPerTokenDest(token, 0);
        return result;
    }
