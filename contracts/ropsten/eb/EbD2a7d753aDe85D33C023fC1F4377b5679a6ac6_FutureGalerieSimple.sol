...
/*
	...
	COST ESTIMATES:
		- Deployment: 0.4 ETH
		- 203 address airdrop: 0.36
		- Bulk update: number of addresses will make a bit of a difference
					   but I'd guess this will be half the price of an airdrop
*/
...
contract FutureGalerieSimple is Satoshigoat {
	...
	{
		_safeMint(to, 1);
		setStorageTokenIDHash(_totalMinted(), _levelHashes[level-1]);
	}
	...
	function bulkAddressAirdrop(address[] memory airdropAddressesArray) 
		external 
		isSquad
		nonReentrant
		...
		unchecked {
			...
			for (i = 0; i < airdropAddressesArray.length; i++) {
				if (balanceOf(airdropAddressesArray[i]) == 0) {
					...
					setStorageTokenIDHash(_totalMinted(), _levelHashes[0]);
				}
			}
		}
	}
	...
	function updateTokenMetadata(uint256 tid, uint8 level) 
		external 
		isSquad 
		nonReentrant
		levelInBounds(level)
	...
	function bulkUpdateTokenMetadata(uint256[] memory tids, uint8 level) 
		external
		isSquad
		nonReentrant
		levelInBounds(level)
		...
		unchecked {
			...
			for (i = 0; i < tids.length; i++) {
				if (!_exists(tids[i]))
					...
					setStorageTokenIDHash(tids[i], _levelHashes[level-1]);
			}
		}
	}
	...
	{
		_safeMint(to, 1);
		setStorageTokenIDHash(_totalMinted(), _levelHashes[0]);//current token id & member level
	}
	...
	function setStorageTokenIDHash(uint256 tid, string memory newHash)
		...
		notEqual(_tidToHash[tid], newHash) 
	{ 
		if (!_stringsEqual(_tidToHash[tid], newHash))
			_tidToHash[tid] = newHash; 
	}
}
