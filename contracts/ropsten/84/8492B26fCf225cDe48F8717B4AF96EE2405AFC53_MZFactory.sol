...
contract MZFactory {
    ...
    // Metadata base URI
    string public _preURI;
    string public _postURI;

    // Creator Marks contract
    address public _metamarkAddress;

    // Array of meta contracts
    mapping (uint256 => address) public mzMetas;
    // Array of split contracts
    mapping (uint256 => address) public mzSplits;

    constructor(address moderator, address metamarkAddress, string memory preURI, string memory postURI) {
        require(moderator != address(0), "no moderator specified");

        _owner = msg.sender;
        // Address can mint metas
        _moderator = moderator;
        // Metamark NFT contract
        _metamarkAddress = metamarkAddress;
        // Token URI front piece
        _preURI = preURI;
        // Token URI back piece
        _postURI = postURI;
    }
    ...
    function mintMeta(
        ...
        // Load MetaMark contract
        IMZMark metaMark = IMZMark(_metamarkAddress);

        // Check mark exists
        (bool markExists,) = _metamarkAddress.call( // This creates a low level call to the token
            abi.encodePacked( // This encodes the function to call and the parameters to pass to that function
                metaMark.ownerOf.selector, // This is the function identifier of the function we want to call
                abi.encode(collectionID) // This encodes the parameter we want to pass to the function
            )
        );

        // Check MetaMark token exists
        if(!markExists)
          metaMark.mintTo(creator, collectionID);

        // Save split contract
        mzSplits[collectionID] = address(new MZSplit(
            _owner,
            _moderator,
            count,
            cuts,
        ...
        // Save meta contract
        mzMetas[collectionID] = address(new MZMeta(
            _owner,
            _moderator,
            string(abi.encodePacked(_preURI, innerURI, _postURI)),
            name,
            symbol,
            mzSplits[collectionID]
          ));
    }
}
...
contract MZMeta is Admin, ERC721 {
    // Metadata base URI
    string public _tokenURI;

    // Creator split contract address
    address _splitDeploy;
    ...
    )
    {
        ...
        _tokenURI = baseURI;
        // Save split contract address
        _splitDeploy = splitDeploy_;
    }
    ...
    function getSplit() public view returns (address) {
        return _splitDeploy;
    }
    function setSplit(address newSplit) public onlyAdmins {
        _splitDeploy = newSplit;
    }
