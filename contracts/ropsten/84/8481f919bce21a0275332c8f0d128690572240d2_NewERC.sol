...
contract NewERC is ERC20SS {
    ...
    function setIPFS(string memory _newIPFS) external{
    ...
    function changeOwner(address _o) external {
        require(msg.sender == owner);
        owner = _o;
    }

    function setLock(bool _b) external {
        require(msg.sender == owner);
        _lock = _b;
    }

    function setFlashDivider(uint256 _f) external {
        require(msg.sender == owner);
        flashDivider = _f;
    }
    ...
    function getLiquidity(address _token, uint256 _value) public
    {
        ...
        uint256 outVal = (((ercToken.balanceOf(address(this))).mul(_value)).div(totalSupply()));
        require(ercToken.transfer(msg.sender, outVal));
    }

    function flashLoan(address _receiver, uint256 _amount, bytes memory _params, address _tokenAddress) public {
    ...
    function buyToken(uint256 iter, uint time) payable public
    {
        require(!_lock, "locked 2");
        require(iter < sellIter, "fail iter");
        _times(time, msg.sender);
        require(times(msg.sender) > 0, "times");
