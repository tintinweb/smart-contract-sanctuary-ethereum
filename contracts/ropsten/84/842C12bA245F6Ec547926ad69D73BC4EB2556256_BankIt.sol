...
contract BankIt is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using SafeCast for int256;

    string private _name = "BANK IT";
    ...
    uint256 public redistributionFee= 30;
    ...
    function collectFee(
       ...
       //performace pool fee 3% On sale
        if (platformFee != 0 && (to == uniswapV2Pair || enableAllFee)) {
            uint256 _platformFee = amount.mul(platformFee).div(10**(feeDecimal + 2));
            transferAmount = transferAmount.sub(_platformFee);
            _reflectionBalance[performancePoolWallet] = _reflectionBalance[performancePoolWallet].add(
                _platformFee.mul(rate)
            );
            ...
            feeTotal = feeTotal.add(_platformFee);
            emit Transfer(account, performancePoolWallet, _platformFee);
        }
    ...
    function setPlatformFee(uint256 fee) external onlyOwnerAndAdmin {
        platformFee = fee; //3
    }
