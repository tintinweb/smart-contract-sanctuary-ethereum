...
/// @title veChai
/// @notice The staking contract for Chai, as well as the token used for governance.
/// Note Allows depositing/withdraw of Chai.
/// Here are the rules of the game:
/// - If you stake Chai, you generate veChai at the current `generationRate` until you reach `maxCap`
/// - If you unstake any amount of Chai, you loose all of your veChai.
/// Note that it's ownable and the owner wields tremendous power. The ownership
/// will be transferred to a governance smart contract once Chai is sufficiently
    ...
    IVeChai
{
    ...
    struct UserInfo {
        uint256 amount; // Chai staked by user
        uint256 lastRelease; // time of last veChai claim or first deposit if user has not claimed yet
    }

    /// @notice the Chai token
    IERC20 public chai;

    /// @notice the chaiMaster contract
    IChaiMaster public chaiMaster;

    /// @notice max veChai to staked Chai ratio
    /// Note if user has 10 Chai staked, they can only have a max of 10 * maxCap veChai in balance
    uint256 public maxCap;

    /// @notice the rate of veChai generated per second, per Chai staked
    ...
    /// @dev contract addresses are by default unable to stake Chai,
    /// they must be previously whitelisted to stake Chai
    ...
    // ============== CONSTRUCTOR ==============
    function initialize(IERC20 _chai, IChaiMaster _chaiMaster) public initializer {
        require(address(_chaiMaster) != address(0), "zero address");
        require(address(_chai) != address(0), "zero address");

        // Initialize veChai
        __ERC20_init("Voting Escrow Chai", "veChai");
        __Ownable_init();
        __ReentrancyGuard_init_unchained();
        __Pausable_init_unchained();

        // set generationRate (veChai per sec per Chai staked)
    ...
    /// @notice checks wether user _addr has Chai staked
    /// @param _addr the user address to check
    /// @return true if the user has Chai in stake, false otherwise
    function isUser(address _addr) public view override returns (bool) {
        return users[_addr].amount > 0;
    }

    /// @notice returns staked amount of Chai for user
    /// @param _addr the user address to check
    /// @return staked amount of Chai
    function getStakedChai(address _addr) external view override returns (uint256) {
    ...
    /// @notice Calculate the amount of veChai that can be claimed by user
    /// @param _addr the address to check
    /// @return amount of veChai that can be claimed by user
    ...
    function _claimable(address _addr) private view returns (uint256) {
        ...
        // get user's veChai balance
        uint256 userVeChaiBalance = balanceOf(_addr);

        // user veChai balance cannot go above user.amount * maxCap
        uint256 maxVeChaiCap = user.amount * maxCap;

        // first, check that user hasn't reached the max limit yet
        if (userVeChaiBalance < maxVeChaiCap) {
            // then, check if pending amount will make user balance overpass maximum amount
            if ((userVeChaiBalance + pending) > maxVeChaiCap) {
                return maxVeChaiCap - userVeChaiBalance;
            } else {
                return pending;
            }
        }
    ...
    function getVotes(address _account) external view virtual override returns (uint256) {
        uint256 veChaiBalance = balanceOf(_account);

        // check that user has more than voting treshold of maxCap and has Chai in stake
        if (
            veChaiBalance * invVoteThreshold > users[_account].amount * maxCap && isUser(_account)
        ) {
            return veChaiBalance;
        } else {
            return 0;
        }
    }
    ...
    /// @notice deposits Chai into contract
    /// @param _amount the amount of Chai to deposit
    function deposit(uint256 _amount) external override nonReentrant whenNotPaused {
        ...
        if (isUser(msg.sender)) {
            // if user exists, first, claim their veChai
            _claim(msg.sender);
        ...
        // Request Chai from user
        chai.safeTransferFrom(msg.sender, address(this), _amount);
        emit Staked(msg.sender, _amount);
    }

    /// @notice claims accumulated veChai
    ...
    /// @notice withdraws staked Chai
    /// @param _amount the amount of Chai to unstake
    /// Note Beware! you will loose all of your veChai if you unstake any amount of Chai!
    function withdraw(uint256 _amount) external override nonReentrant whenNotPaused {
        ...
        // update user balance before burning or sending back Chai
        users[msg.sender].amount -= _amount;

        // get user veChai balance that must be burned
        uint256 userVeChaiBalance = balanceOf(msg.sender);

        _burn(msg.sender, userVeChaiBalance);

        // send back the staked chai
        chai.safeTransfer(msg.sender, _amount);

        emit Unstaked(msg.sender, _amount);
    }

    /// @notice hook called after token operation mint/burn
    /// @dev updates chaiMaster
    /// @param _account the account being affected
    /// @param _newBalance the newVeChaiBalance of the user
    function _afterTokenOperation(address _account, uint256 _newBalance) internal override {
        chaiMaster.updateFactor(_account, _newBalance);
    }
    ...
    /// @notice sets chaiMaster address
    /// @param _chaiMaster the new chaiMaster address
    function setChaiMaster(IChaiMaster _chaiMaster) external onlyOwner {
        require(address(_chaiMaster) != address(0), "zero address");
        chaiMaster = _chaiMaster;
    }
