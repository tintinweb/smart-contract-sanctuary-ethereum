...
contract ControllerFreedom is Initializable, UUPSUpgradeable, OwnableUpgradeable {
	...
	*/
	function isMemberOfCommunity(uint256 commId) internal view returns (MemberOfCommRes memory){
    ...
    */
    function proposalCreationRight(uint256 _type, uint256 commId) external view returns (bool) {        
        bool haveRights = false;
        MemberOfCommRes memory res = isMemberOfCommunity(commId);
        if(res.isMember) {
    ...
    */
    function votingRight(uint256 _type, uint256 commId) external view returns (bool) {        
        bool haveRights = false;
        MemberOfCommRes memory res = isMemberOfCommunity(commId);
        if(res.isMember) {
...
contract Voting {
	...
	*/
	function communityVoteCast(uint256 commId, uint256 pollId, bool vote) external {
		...
		require(community.votingRight(poll._type, commId) != false, "You already voted.");
	...
	*/
	function communityPollCreate(uint256 daoId, uint256 pollType, ProposeChanges memory proposedChanges, uint256 startTime, uint256 endTime
		...
		require(community.proposalCreationRight(pollType, daoId) != false, "You dont have permission to create proposal"); //check wheather a member have a proposal creation rights
		_PollIds.increment();
	...
	function _doProposedChanges(uint256 commId, uint256 pollId) external view returns (bool) { 
		if(_computePollResult(commId, pollId)){
			...
			if(commPoll._type == 1){
				...
				res = community.addMemberToGroup(proposechanges.memberToRemove, proposechanges.targetGroupId, commId);
