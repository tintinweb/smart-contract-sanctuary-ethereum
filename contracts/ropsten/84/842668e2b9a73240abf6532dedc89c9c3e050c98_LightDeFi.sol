...
Light DeFi is a token created with the proposal to unify ESG (Environmental, social and corporate governance) and tokenization, encouraging the development of new technologies capable of promoting the construction of Solar Power Plants and gathering concepts of Sustainability and Decentralized Finance (DeFi). For more information, access the link below:
...
BRILLACOM S.A.
RUT nº 218780380016
Calle Rincón, 610. 4th floor, office 410,
Montevideo, Uruguay.
*/
...
contract LightDeFi is Context, IERC20, Ownable {
    ...
    event SetChartityFeePercent(uint256 charityFee);
    event SetRouterAddress(address newRouter);
    ...
    event SetcharityWallet(address newWallet);
    ...
    function swapTokensForEth(uint256 tokenAmount) private returns (bool){
        ...
        // make the swap
        // Due to the characteristics of the project,
        // it is not possible to define a minimum amount receivable.
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private returns (bool) {
        ...
        // add the liquidity
        // Due to the characteristics of the project,
        // it is not possible to define a minimum quantity of output.
    ...
    function setcharityWallet(address newWallet) external onlyOwner() {
        ...
        emit SetcharityWallet(newWallet);
    }
    ...
    function setChartityFeePercent(uint256 charityFee) external onlyOwner() {
        ...
        emit SetChartityFeePercent(charityFee);
    }
