...
contract TheLordOfFlies is ERC721A, Ownable, Pausable {
	...
	function mint(uint256 _quantity) external callerIsUser payable whenNotPaused {
		...
		uint256 _remainFreeQuantity = mintInfo.freeQuantity - _hasMinted > 0 ? mintInfo.freeQuantity - _hasMinted : 0;
		uint256 _needPayPrice = _remainFreeQuantity >= _quantity ? 0 : (_quantity - _remainFreeQuantity) * mintInfo.mintPrice;

		require(_quantity > 0, "Invalid quantity");
		require(_hasMinted + _quantity <= mintInfo.maxSupply, "Exceed supply");
		require(_currentMintedCount + _quantity <= mintInfo.singleAddressLimitQuantity || (mintInfo.ownerMintUnrestricted && msg.sender == owner()), "Exceed mint limit");
