...
contract Stagefi is ERC721, ERC721Burnable, ERC721URIStorage, Mintable {
    string private _baseUrl;

    modifier onlyOwnerOrStagefi(uint256 tokenId) {
        require(_isApprovedOrOwner(_msgSender(), tokenId) || msg.sender == owner(), "Function can only be called by owner or Stagefi");
        _;
    }
    ...
    function setBaseAddress(string memory baseUrl) public onlyOwner returns (string memory) {
        require(
    ...
    function burn(uint256 tokenId) override onlyOwnerOrStagefi(tokenId) public {
        _burn(tokenId);
    }
