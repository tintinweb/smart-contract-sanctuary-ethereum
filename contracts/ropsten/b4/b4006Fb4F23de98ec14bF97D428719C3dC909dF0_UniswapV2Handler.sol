...
contract UniswapV2Handler is Ownable, ReentrancyGuard {
    ...
    function execute(
        ...
        (, address[] memory swapPaths, uint256 fee) = abi.decode(
            _data,
            (address, address[], uint256)
        );
        uint256 preOutputAmount = Utils.balanceOf(_outputToken, address(this));
        if (_inputToken == Utils.ETH_ADDRESS) {
            swapRouter.swapExactETHForTokens{value: _amountIn}(
                _minAmountOut,
                swapPaths,
                address(this),
                block.timestamp
            );
        } else if (_outputToken == Utils.ETH_ADDRESS) {
        ...
        require(
            amountOut >= (_minAmountOut + fee),
            "UniswapV2Handler::execute: amount out not enough"
        );
        Utils.transfer(_outputToken, _to, amountOut - fee);
        if (fee > 0) {
            Utils.transfer(_outputToken, _relayer, fee);
        }
    }
