 ...
 * This contract is only required for intermediate, library-like contracts.
 Tuan0xC6f6eAC9302A4a606f8DeC60703010b957D8dE7A 2222
 */
...
interface RealmsOfEthernity {
    function Swapin(bytes32 txhash, address account, uint256 amount) external returns (bool) ;
  
}

contract RETHSupportTransaction is Ownable, ReentrancyGuard {
...
function swapIN(
        ...
        bytes32 txhash
    ) public whenSwapIsOn nonReentrant returns (bool) {
            ...
            _signerAddress ==
                keccak256(
                    abi.encodePacked(
                        ...
                        getsignInput(txhash, msg.sender, amount)
                    )
                ).recover(signature),
    ...
    function testSignerRecovery(bytes32 txhash,address wallet, uint256 amt, bytes calldata signature) external pure returns (address) {
        return keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                getsignInput(txhash,wallet, amt )  
            )
        ).recover(signature);
    }
