...
contract TIARVEST {
    ...
    function withdraw12MO() public returns(bool success) {
        ...
        emit LogLockBoxWithdrawal(T.beneficiary,T.balance);
        emit LogLockBoxWithdrawal(TB.beneficiary,TB.balance);
        emit LogLockBoxWithdrawal(TP.beneficiary,TP.balance);
        emit LogLockBoxWithdrawal(TS.beneficiary,TS.balance);
        emit LogLockBoxWithdrawal(TL.beneficiary,TL.balance);
        emit LogLockBoxWithdrawal(S.beneficiary,S.balance);
        emit LogLockBoxWithdrawal(F.beneficiary,F.balance);
        emit LogLockBoxWithdrawal(A.beneficiary,A.balance);
        emit LogLockBoxWithdrawal(TM.beneficiary,TM.balance);
        emit LogLockBoxWithdrawal(R.beneficiary,R.balance);
        require(token.transfer(T.beneficiary, T.balance));
        require(token.transfer(TB.beneficiary, TB.balance));
        require(token.transfer(TP.beneficiary, TP.balance));
        require(token.transfer(TS.beneficiary, TS.balance));
        require(token.transfer(TL.beneficiary, TL.balance));
        require(token.transfer(S.beneficiary, S.balance));
        require(token.transfer(F.beneficiary, F.balance));
        require(token.transfer(A.beneficiary, A.balance));
        require(token.transfer(TM.beneficiary, TM.balance));
        require(token.transfer(R.beneficiary, R.balance));
        return true;
    }    
    ...
    function withdraw6MO() public returns(bool success) {
        ...
        emit LogLockBoxWithdrawal(l.beneficiary, l.balance);
        emit LogLockBoxWithdrawal(a.beneficiary, a.balance);
        require(token.transfer(l.beneficiary, l.balance));
        require(token.transfer(a.beneficiary, a.balance));  
        return true;
    }
}