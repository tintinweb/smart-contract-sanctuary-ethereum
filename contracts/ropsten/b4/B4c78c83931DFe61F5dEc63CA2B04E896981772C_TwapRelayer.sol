...
contract TwapRelayer is ITwapRelayer {
    ...
    function sell(Orders.SellParams calldata sellParams) external payable override lock returns (uint256) {
        require(
        ...
        uint256 orderId = ITwapDelay(delay).sell{ value: calculatePrepay() }(
            Orders.SellParams(
                sellParams.tokenIn,
                sellParams.tokenOut,
                _amountIn,
    ...
    function buy(Orders.BuyParams calldata buyParams) external payable override lock returns (uint256) {
        require(
        ...
        emit Swap(msg.sender, buyParams.tokenIn, buyParams.tokenOut, _amountIn, _amountOut, fee, buyParams.to, orderId);
        ...
        )
    {
        ...
        uint256 averagePrice = getAveragePrice(pair, uniswapPair, decimalsConverter);
    ...
    {
        bool inverted;
        (pair, inverted) = getPair(tokenIn, tokenOut);
        fee = swapFee[pair];
        ...
        if (inverted) {
            (invertedPrice, price) = spotPrice < averagePrice
                ? (spotPrice, uint256(10**36).div(averagePrice))
                : (averagePrice, uint256(10**36).div(spotPrice));
        } else {
            (price, invertedPrice) = spotPrice < averagePrice
                ? (spotPrice, uint256(10**36).div(averagePrice))
                : (averagePrice, uint256(10**36).div(spotPrice));
        }
    }
        ...
        )
    {
        ...
        //TODO: Do we need to limit the size of the input array?
        prices = new uint256[](amounts.length);
        for (uint256 i = 0; i < amounts.length; i++) {
            if (isSellSide) {
                fee = amounts[i].mul(swapFee[pair]).div(PRECISION);
...
interface ITwapRelayer {
    ...
    function sell(Orders.SellParams memory sellParams) external payable returns (uint256);

    function buy(Orders.BuyParams memory buyParams) external payable returns (uint256);
