...
contract QueenTraits is BaseContractController, IQueenTraits {
        ...
        returns (RoyalLibrary.sRARITY[] memory raritiesList)
    {
        if (_onlyWithArt && _traitId <= 0)
            /*revert RoyalLibrary.InvalidParametersError({
                _caller: "QueenTraits.getRarities",
                _msg: "Invalid Parameters",
                _arg1: _onlyWithArt ? "true" : "false",
                _arg2: _traitId.toString(),
                _arg3: ""
            });*/
            require(
        ...
        whenNotPaused
    {
        ...
        for (uint256 index = 0; index < _artUri.length; index++) {
            ...
            require(_traitId > 0, "Trait id invalid!");
            require(_rarityId > 0, "Rarity id invalid!");
            require(getTrait(_traitId).enabled == 1, "Trait disabled!");
