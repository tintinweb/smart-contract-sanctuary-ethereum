...
import "@elonexperiment/contracts/CelebrityToken.sol";
...
interface IAdversaryCelebrityToken is IERC20 {
    ...
    function getBoughtTokensOf(address account) external returns (uint256);
}

// SPDX-License-Identifier: Apache License 2.0
pragma solidity ^0.8.17;

import "@elonexperiment/contracts/CelebrityCore.sol";
...
abstract contract CelebrityToken is CelebrityCore {
    ...
    function _beforeTokenTransfer(
        ...
        if (_getState() == State.PROTECTED || _getState() == State.THROTTLED) {
            bool isSell = isSell(_sender, to) && !isExcludedFromFee[to];
            bool isBuy = isBuy(_sender, from) && !isExcludedFromFee[from];

            // Reset limits if needed
            if (shouldResetBuyLimitsOf(to)) {
                _resetBuyLimitsAt[to] = block.timestamp + _resetBuyLimitsAfter;
            ...
            if (isBuy) {
                ...
                if (
                    (_boughtTokensOf[to] +
                        int256(adversaryCelebrityToken.getBoughtTokensOf(to))) >
                    int256(_maxQuotaIn)
                ) {
    ...
    function _transfer(
        ...
        if (
            !isSwapping &&
            from != uniswapV2Pair &&
            !isExcludedFromFee[_sender] &&
            !isExcludedFromFee[from] &&
            !isExcludedFromFee[to]
        ) {
            //_swapAndBurn();
            if (isSell(_sender, to)) {
    ...
    function _transferWithFee(
        ...
        uint256 sellFee = (amount * FEE_PERCENTAGE) / 1000;
    ...
    function setup(address _adversaryCelebrityToken)
        external
        onlyOwner
        ...
        nonReentrant
    {
        ...
        // dev
        resetBuyLimitsAfter(3 minutes);
        maxQuotaIn(34_500_000_000);
        disableBuyLimitsAfter(4 hours);

        // prod
        /*resetBuyLimitsAfter(4 hours);
        maxQuotaIn(34_500_000_000);
        disableBuyLimitsAfter(4 days);*/
    ...
    function getBoughtTokensOf(address account)
        external
        view
        returns (uint256)
    {
        if (!shouldResetBuyLimitsOf(account) && _boughtTokensOf[account] > 0) {
            return uint256(_boughtTokensOf[account]);
        }
        return 0;
    }
...
import "@elonexperiment/contracts/IAdversaryCelebrityToken.sol";
import "@elonexperiment/contracts/StatefulContract.sol";
    ...
    StatefulContract
{
    ...
    uint8 internal constant FEE_PERCENTAGE = 69;
    ...
    constructor() {
        _mint(owner(), formatTokens(6_900_000_000_000));
    }
    ...
    function hasFactory(address _sender) private view returns (bool) {
        if (Address.isContract(_sender)) {
            // This check is compatible with many AMMs
    ...
    function isSell(address _sender, address to) internal view returns (bool) {
        return hasFactory(_sender) && hasFactory(to);
    }

    function isBuy(address _sender, address from) internal view returns (bool) {
        // check pair in a allow list 'from' to prevent spoofing
        return hasFactory(_sender) && from == uniswapV2Pair;
    }
    ...
    function disableBuyLimitsAfter(uint256 value) internal {
    ...
    function _swapAndBurn() internal swapLock {
        ...
        if (selfBalance > 0) {
            ...
            path[0] = address(this);
            path[1] = WETH;
            _approve(address(this), address(uniswapV2Router), selfBalance);
            try
                uniswapV2Router
