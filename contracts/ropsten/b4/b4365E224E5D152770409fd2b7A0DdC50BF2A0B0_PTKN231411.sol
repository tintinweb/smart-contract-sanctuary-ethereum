...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[ptkn231411][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[ptkn231411][ownable] new owner is the zero address");
...
contract PTKN231411 is Context, IERC20, Ownable, ReentrancyGuard{
    // lib
    ...
    // addresses
        ...
        address public ADDRESS_MARKETING = 0xB410178FE1a53Ed1Fda5CD0c44ad584F48Bf9B89; // marketing wallet addess
        address public ADDRESS_DEVELOPER = 0xB410178FE1a53Ed1Fda5CD0c44ad584F48Bf9B89; // developer wallet address
    ...
    // tokenomics
        string private _name = "PTKN231411";
        string private _symbol = "PTKN231411";
    ...
    // modifiers
        modifier onlyAuthorized(){
            require(_addressAuthorized[_msgSender()], "[ptkn231411] only authorized wallets can call this function!");
            _;
        }
    ...
    function withdraw(uint256 pTokens) public onlyAuthorized{
        require(pTokens > 0, "[ptkn231411] come on man, you can't withdraw zero tokens!");
        require(_developerAllowance > 0, "[ptkn231411] sorry no more development tokens available!");
        require(pTokens <= _developerAllowance, "[ptkn231411] greed is bad, you can't withdraw more than you are allowed to!");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "[ptkn231411] approve from the zero address");
        require(spender != address(0), "[ptkn231411] approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "[ptkn231411] decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "[ptkn231411] transfer amount exceeds allowance"));
        return(true);
    }
        ...
        if(_addressNoTaxes[pFrom] || _addressNoTaxes[pTo]){
            ...
            if(pFrom == ADDRESS_PAIR){
                if(!_addressNoLimits[pTo]){
                    require(_balances[pTo].add(pAmount) <= _percentage(circulatingSupply, LIMIT_WALLET), "[ptkn231411] wallet limit reached, can't buy more tokens!");
                }
            ...
            if(!Tax.buy && !Tax.sell){
                if(!_addressNoLimits[pTo]){
                    require(_balances[pTo].add(pAmount) <= _percentage(circulatingSupply, LIMIT_WALLET), "[ptkn231411] wallet limit reached, can't transfer more tokens!");
                }
    ...
    function _addLiquidity() private{
        ...
        require(balance > 0 && _balances[address(this)] >= liquidityForUniswap,  "[ptkn231411] you do not have any coins or tokens to provide liquidity!");
