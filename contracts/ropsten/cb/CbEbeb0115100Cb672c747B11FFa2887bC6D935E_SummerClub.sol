...
contract SummerClub is ERC721G, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 amount, string calldata salt, bytes calldata token) external payable {
        require(status == Status.PublicSale, "SUM006");
        uint256 numberResult = _verify(_hash(salt, msg.sender), token);
        require(numberResult >= amount, "SUM001");
        require(numberMinted(msg.sender) + amount <= numberResult, "SUM008");

        _verified_public(amount);
        _verified(amount);
    ...
    function setStatus(Status _status, address signer, uint256 amount, uint256 price) external onlyOwner {
        status = _status;

        if (status == Status.PreSale) {
            _signerOfNum[signer] = amount;
            WHITE_PRICE = price;
        }
    ...
    function _verified(uint256 num) private view {
