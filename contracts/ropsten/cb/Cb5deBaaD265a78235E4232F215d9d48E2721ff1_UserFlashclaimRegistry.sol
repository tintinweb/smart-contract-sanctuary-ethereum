...
contract UserFlashclaimRegistry {
    address public nTokenRegistry;
    mapping(address => address) public userReceivers;

    constructor(address nTokenRegistry_) {
        nTokenRegistry = nTokenRegistry_;
    }

    function createReceiver() public {
        AirdropFlashClaimReceiver receiver = new AirdropFlashClaimReceiver(msg.sender, nTokenRegistry, 1);
...
contract AirdropFlashClaimReceiver is IFlashClaimReceiver, ReentrancyGuard, Ownable, ERC721Holder, ERC1155Holder {
    address public immutable nTokenRegistry;
    mapping(bytes32 => bool) public airdropClaimRecords;
    uint256 public deployType;

    constructor(
        address owner_,
        address nTokenRegistry_,
        uint256 deployType_) {
        require(owner_ != address(0), "zero owner address");
        require(nTokenRegistry_ != address(0), "zero registry address");
    ...
    function executeOperation(
        ...
        address nTokenAddress = INTokenRegistry(nTokenRegistry).getNTokenAddresses(nftAsset);
        require(nTokenAddress == msg.sender, "caller not ntoken");
