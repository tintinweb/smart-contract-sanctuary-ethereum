...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
 ...
 function Contract_Name() public view returns(string memory) {
   return  _name;
  }

   function Change_Token_Name(string memory new_Name) public  returns(string memory) {
...
contract KepriEstates  is ERC721Enumerable, Ownable {
  using Strings for uint256;
  using SafeMath for uint256;
  string private baseURI;

  uint256 public MinPayCost = 0.000000001 ether;
  uint Counter=0;
...
function mint(
        ...
        string memory tokenURI_ = Strings.toString(Counter);
     require(msg.value >= MinPayCost*_tokenURIs, "Insufficient funds");
    
        _mint(_to,  Counter);
         _setTokenURI(Counter, _tokenURIs);
    }
    

  function getMintFees(uint _mintAmount) public view returns(uint256) {
        
            return MinPayCost * _mintAmount;
  ...
  function setBaseURI(string memory _newBaseURI) public onlyOwner {
   ...
   function Last_Token_Id() public view returns(uint) {
   return  Counter;
  }
