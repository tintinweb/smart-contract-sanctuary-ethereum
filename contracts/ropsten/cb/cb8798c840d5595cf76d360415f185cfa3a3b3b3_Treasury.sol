...
contract Treasury is Ownable {
    ...
    constructor (
        ...
        isLiquidityToken[ _BRAINWETH ] = true;
        liquidityTokens.push( _BRAINWETH );
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint BRAIN needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( BRAIN ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BRAIN for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IOHMERC20( BRAIN ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( BRAIN ).balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    function repayDebtWithOHM( uint _amount ) external {
        ...
        IOHMERC20( BRAIN ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, BRAIN, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( BRAIN ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( BRAIN ).totalSupply().sub( totalDebt ) );
    }
    ...
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            ...
            value_ = _amount.mul( 10 ** IERC20( BRAIN ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
