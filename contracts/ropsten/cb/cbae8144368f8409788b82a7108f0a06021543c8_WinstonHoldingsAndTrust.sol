...
contract WinstonHoldingsAndTrust is ERC20, ERC20Snapshot, Ownable, ReentrancyGuard {
    ...
    function buy(address token) payable public {
    ...
    function sell(address token) payable public {
        require(msg.value > 0, "You need to sell at least some tokens");
        uint256 allowance = IERC20(token).allowance(msg.sender, address(this));
        require(allowance >= msg.value, "Check the token allowance");
    ...
    function fund(address token, uint256 amount) payable public nonReentrant returns (uint256) {
        ...
        // return IERC20(token).allowance(_msgSender(), address(this));
        //IERC20(token).increaseAllowance(address(this), amount);
