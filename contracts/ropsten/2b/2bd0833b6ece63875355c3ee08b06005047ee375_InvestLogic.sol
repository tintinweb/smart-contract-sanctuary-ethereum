...
contract InvestLogic is Ownable, Pausable {  
   
    using SafeERC20 for IERC20;
    string public name = "Invest Pools";
 
    IERC20 private usdtToken;
    IERC20 private omegaToken;
    uint256 private usdConversionFactor; //USD Per stable coin 
    ...
    mapping(uint256 => uint) public investmentPool;     // to manage the staking of usdtToken and distibue the profit as usdtToken B
    ...
    constructor(IERC20 _token, IERC20 _omegaToken, address _usdtAddress, uint256 _usdConversionFactor) {
        usdtToken = _token;
        omegaToken = _omegaToken;
        usdtAddress = _usdtAddress; // Address which transfers the reward usdt
        usdConversionFactor = _usdConversionFactor;
    }

    function setUSDConversionFactor(uint256 _usdConversionFactor) external onlyOwner whenNotPaused {
        usdConversionFactor = _usdConversionFactor;
    }
   
    function getUSDConversionFactor() external view whenNotPaused returns(uint256){
        return usdConversionFactor;
    }
    
    function getOmegaToTransfer(uint256 _amount) external view whenNotPaused returns(uint256){
        return _amount * usdConversionFactor * omegaPercent/(100);
    }
    ...
    function staketoken(uint _amount, terms time, address _stableCoin) public whenNotPaused {  
 
        require(_amount > 0, "staking balance cannot be 0");
 
        require(IERC20(_stableCoin).balanceOf(msg.sender) > _amount, "Not enough balance");
 
        uint256 omegaAmount = _amount * usdConversionFactor * omegaPercent/(100);
        ...
        userToken[currentPoolID] = _stableCoin;
    ...
    function calculateReward(uint256 stakeBalance, uint256 id) internal virtual returns(uint256){
        uint256 reward = stakeBalance * userRates[id] / (100);
        return reward;
    }
    ...
    function calculateInterest(uint256 poolID) public view whenNotPaused returns(uint256){
        ...
        uint256 reward = balance * userRates[poolID] * (100);
    ...
    function getUsdtAddress() external view whenNotPaused returns(address){
        return usdtAddress;
    }

    function getOmegaToken() external view whenNotPaused returns(IERC20){
        return omegaToken;
    }

    function getUSDTToken() external view whenNotPaused returns(IERC20){
        return usdtToken;
    }
