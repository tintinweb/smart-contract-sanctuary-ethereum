...
contract Token is ERC20, Ownable {
    ...
    uint256 public BushPoolFee = 7;

    // Penalty Fee for Early Sells
    uint256 public earlySellReflectionFee = 1;
    uint256 public earlySellPoolFee = 49;
    ...
    constructor() ERC20("Bushido", "$BUSHIDO") {
    ...
    function updateFees(uint256 _bushPoolFee, uint256 _reflectionFee) external onlyOwner {
        ReflectionFee = _reflectionFee;
        BushPoolFee = _bushPoolFee;
        require(BushPoolFee + ReflectionFee <= 16, "Must keep fees at 16% or less");
    }
    
    function updatePenaltyFees(uint256 _bushPoolFee, uint256 _reflectionFee) external onlyManager {
        earlySellReflectionFee = _reflectionFee;
        earlySellPoolFee = _bushPoolFee;
        require(BushPoolFee + ReflectionFee <= 50, "Must keep fees at 50% or less");
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from] && to != address(uniswapV2Router)) {
                fees = amount.mul(BushPoolFee).div(100);
