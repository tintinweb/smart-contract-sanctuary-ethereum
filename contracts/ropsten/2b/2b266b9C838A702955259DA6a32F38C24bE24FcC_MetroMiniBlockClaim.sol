...
contract MetroMiniBlockClaim is Ownable {
    ...
    function _claim(address account, uint256[] calldata tokenIds) internal {
        ...
        uint256 prevTokenId;
        MetroMiniBlock.TokenOwnership[] memory tokenInfos = miniBlock.explicitOwnershipsOf(tokenIds);
        ...
        for (uint256 i; i < tokenIds.length; ++i) {
            ...
            MetroMiniBlock.TokenOwnership memory tokenInfo = tokenInfos[i];
            require(tokenInfo.addr == account, "not an owner");
            require(tokenInfo.burned == false, "token is burned");

            console.log("Start timestamp: %d, block: %d, diff: %d", tokenInfo.startTimestamp, block.timestamp, block.timestamp - tokenInfo.startTimestamp);
            earned += 1 ether * score * (block.timestamp - tokenInfo.startTimestamp) / 1 days;
            console.log("Earned: %d", earned);
        }

        miniBlock.setTimestampsOf(account, tokenIds, uint64(block.timestamp));

        if (earned > 0) {
            uint256 boost = blockInfoContract.getHoodBoost(tokenIds);
            console.log("Boost: %s", boost);
