...
contract MotherContract {
  ...
  function createPoolETH(uint256 amount, string memory name) external payable {
    LiquidityPool pool;
    //require(amount>=1000000000000000);
    pool=LiquidityPool(Clones.clone(lpTemplate));
    pool.init{value:amount}(msg.sender,name,address(0x0),lpTokenTemplate, lpErc1155Template);
