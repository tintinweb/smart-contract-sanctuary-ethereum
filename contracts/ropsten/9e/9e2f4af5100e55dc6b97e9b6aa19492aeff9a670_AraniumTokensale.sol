...
contract AraniumTokensale {
  ...
  mapping(address => uint256) public overAlloc; // -- wei
  ...
  constructor() public {
    ...
    tokenAddr = 0xe65a09f22a777af9c03457f31cd700dfbe2153bd;
  ...
  function buyTokens(address _beneficiary, uint _weiAmount) internal whenNotPaused isWhitelisted(_beneficiary) {
    ...
    if (changeAmt > 0) {
      ...
      overAlloc[_beneficiary] = changeAmt;
    //   _beneficiary.transfer(changeAmt);
    }
  }

    function refund(address _beneficiary, uint _weiAmount) payable onlyOwner public {
        require(overAlloc[_beneficiary] >= _weiAmount);
        _beneficiary.transfer(_weiAmount);
        overAlloc[_beneficiary] = overAlloc[_beneficiary].sub(_weiAmount);
    }
