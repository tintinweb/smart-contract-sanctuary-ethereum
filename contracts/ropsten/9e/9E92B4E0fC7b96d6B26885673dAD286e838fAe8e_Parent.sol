...
interface TRC20 {
    ...
    function setBudget(uint256 mBudget) external returns (bool);
    function getBudget() external view returns (uint256);
    function getOwnerAddress() external view returns (address);
    function balanceOf(address _add) external view returns (uint256 balance);
    function transfer(address _to, uint256 _value) external returns (bool success);
    function transferTo(address _to, uint256 _amount) external returns (bool);
    function buyToken(address _from, address _to, uint256 _value) external returns(bool);
    function getUnlockMarket(address _from) external view returns(uint256);
    function addUnlockMarket(address _from, uint256 _value) external returns(bool);
    function subUnlockMarket(address _from, uint256 _value) external returns(bool);
...
contract Parent is IERC20 {
    ...
    function transferFrom(address _from, address _to, uint256 _value) public override returns (bool success) {
        require(balances[_from] >= _value, "ERC20: Balance not available");
        require(allowed[_from][msg.sender] >= _value, "ERC20: Allowed not available");
        require(_value > 0, "ERC20: Amount not available");
        balances[_to] += _value;
    ...
    function mint(uint256 _value) public override returns (uint256){
        _mint(msg.sender, _value);
        return _totalSupply;
    }

    function burn(uint256 _value) public override returns(uint256){
    ...
    function safeTransfer(address token, address _seller, address _buyer, uint256 amount) public returns (bool){
        require(token != address(0), "Parent: Contract not found");
        require(balances[_buyer] >= amount, "Parent: Please buy coin");
        uint256 budget = TRC20(token).getBudget();
        uint256 tmpValue = amount * 1000 / budget;
        uint256 unlockMarket = TRC20(token).getUnlockMarket(_seller);
        require(unlockMarket >= tmpValue, "So luong token con lai khong du cung cap cho ban");
        _transfer(_buyer, _seller, amount);
        emit SafeTransfer(token, _seller, _buyer, amount);
        return TRC20(token).buyToken(_seller, _buyer, tmpValue);
    }
}