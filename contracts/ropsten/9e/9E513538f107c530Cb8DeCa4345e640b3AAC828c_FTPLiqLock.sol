...
contract FTPLiqLock is Ownable {
    ...
    function checkFactor(address _pair) external view returns(uint256,uint256,uint256,uint256){
        uint256 _pairBalance = ERC20(_pair).balanceOf(address(this));
        uint256 _wethBalance = ERC20(0xc778417E063141139Fce010982780140Aa0cD5Ab).balanceOf(_pair).mul(100000);
        uint256 _starting = m_StartingBalance[_pair];
        uint256 _factor = _wethBalance.div(_starting);
        uint256 _share = _pairBalance.mul(100000).div(_factor);
        return (_wethBalance, _starting, _factor, _share);
    }
}