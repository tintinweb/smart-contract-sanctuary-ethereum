...
contract NFTMarketPlace is ERC721 {
    ...
    uint256 public highest_payable_bid;
    uint256 public bid_inc;
    ...
    modifier notOwner() {
        require(msg.sender != nftOwner, "owner cannot bid");
        _;
    }
    modifier onlyOwner() {
        require(msg.sender == nftOwner, "only nftOwner end");
        _;
    }
    modifier onlyBidder() {
        require(msg.sender == highestBidder, "bidder can with draw");
        _;
    }
    modifier started() {
        require(
            block.timestamp > startblock,
            "starttime is greater than startblock"
        );
        _;
    }
    modifier beforeending() {
        require(
            ...
            "endtime is greater than startblock"
        );
        _;
    }
    ...
    function auctionStartwithcoin(
        ...
        endblock = _endblock;
        highest_payable_bid = payable_bid;
        emit auctionstartcoin(
            nft_address,
            _tokenId,
            _startblock,
            _endblock,
    ...
    function finallizeAuc() public {
        require(
            auctionState == Auc_State.Cencelled ||
                auctionState == Auc_State.Ended ||
                block.number > endblock
        );
        require(msg.sender == nftOwner || bids[msg.sender] > 0);
