...
import "../ERC20/utils/ERC20Helper.sol";
 ...
 */
contract SomaERC777Wrapper is SomaERC777 {
    using ERC20Helper for IERC20;

    IERC20 public underlying;

    bytes32 public constant GLOBAL_RECOVER_ROLE = keccak256('SomaERC777Wrapper:global-recover-role');
    bytes32 public LOCAL_RECOVER_ROLE;

    constructor(address networkAddress)
    SomaERC777(networkAddress) {}

    function initialize(IERC20 underlyingToken, address[] memory defaultOperators) external virtual {
        LOCAL_RECOVER_ROLE = keccak256(abi.encodePacked(address(this), 'SomaERC777Wrapper:local-recover-role'));
        underlying = underlyingToken;
        super.initialize(
            string(abi.encodePacked("SOMA Wrapped: ", underlyingToken.tryName("SomaERC777"))),
            string(abi.encodePacked("SW", underlyingToken.trySymbol("C777"))),
            defaultOperators,
            0,
            address(0)
        );
    ...
    function initialize(
        ...
        revert("Please use the other initialization method");
    }
    ...
    function recover(address account) public virtual returns (uint256) {
        require(hasRole(LOCAL_RECOVER_ROLE, _msgSender()) || hasRole(GLOBAL_RECOVER_ROLE, _msgSender()), 'SomaERC777Wrapper: invalid role requirements');
...
contract SomaERC777 is ERC777Upgradeable, SecurityToken {
    ...
    bytes32 public constant GLOBAL_ADMIN_OPERATOR_ROLE = keccak256('SomaERC777:global-admin-operator');
    bytes32 public LOCAL_ADMIN_OPERATOR_ROLE;

    constructor(address networkAddress) GuardableUpgradeable(networkAddress) {}
    ...
    function initialize(
        ...
        LOCAL_ADMIN_OPERATOR_ROLE = keccak256(abi.encodePacked(address(this), 'SomaERC777:local-admin-operator'));
...
interface ISomaGuard {
    ...
    function switchOn(uint256[] memory ids, bytes32 base) external view returns (bytes32);

    function switchOff(uint256[] memory ids, bytes32 base) external view returns (bytes32);

    function merge(bytes32 access1, bytes32 access2) external view returns (bytes32);

    function check(address account, bytes32 query) external view returns (bool);

    function has(address account, uint256[] memory ids) external view returns (bool);
...
import "../../SomaNetwork/utils/NetworkLocations.sol";

abstract contract GuardableUpgradeable is IGuardable, NetworkAccessUpgradeable {
    using NetworkLocations for ISomaNetwork;
    ...
    function __Guardable__init_unchained() internal onlyInitializing {
        LOCAL_UPDATE_ACCESS_ROLE = keccak256(abi.encodePacked(address(this), "Guardable:update-access-role"));
    ...
    bytes32 public constant GLOBAL_UPDATE_ACCESS_ROLE = keccak256('Guardable:default-update-access');

    constructor(address networkAddress) NetworkAccessUpgradeable(networkAddress) {
        DEFAULT_ACCESS = _guard().DEFAULT_ACCESS();
        // Do we want a setter to change DEFAULT_ACCESS after contract deploy?
    }
    ...
    function canAccess(address account) public view virtual override returns (bool) {
        return _guard().check(account, requiredAccess());
        // Any difference in gas for requiredAccess() vs _requiredAccess ?
    }
    ...
    function _guard() internal view returns (ISomaGuard) {
...
library NetworkLocations {
    bytes32 internal constant SomaGuard = bytes32('SomaGuard');
    bytes32 internal constant SomaSwapFactory = bytes32('SomaSwapFactory');
    bytes32 internal constant TemplateFactory = bytes32('TemplateFactory');
    bytes32 internal constant SomaSwapRouter = bytes32('SomaSwapRouter');

    function guard(ISomaNetwork network) internal view returns (ISomaGuard) {
        return ISomaGuard(network.get(SomaGuard));
    }

    function factory(ISomaNetwork network) internal view returns (ISomaSwapFactory) {
        return ISomaSwapFactory(network.get(SomaSwapFactory));
    }

    function router(ISomaNetwork network) internal view returns (ISomaSwapRouter) {
        return ISomaSwapRouter(routerAddress(network));
    }

    function templateFactory(ISomaNetwork network) internal view returns (ITemplateFactory) {
        return ITemplateFactory(network.get(TemplateFactory));
    }

    function routerAddress(ISomaNetwork network) internal view returns (address) {
        return network.get(SomaSwapRouter);
    }
}
...
interface ITemplateFactory {
    ...
    function predictDeployedAddress(bytes32 templateId, uint256 version, bytes memory args) external view returns (address);

    function predictDeployedAddress(bytes32 templateId, uint256 version, bytes memory args, bytes32 salt) external view returns (address);
