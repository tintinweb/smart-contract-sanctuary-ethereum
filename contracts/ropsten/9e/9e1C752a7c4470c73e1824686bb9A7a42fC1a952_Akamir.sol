...
contract Akamir is Ownable, ERC721A, ReentrancyGuard {
  ...
  function getAuctionPrice(uint256 _saleStartTime) public view returns (uint256) {
    ...
    if (block.timestamp - _saleStartTime >= saleConfig.auctionSaleDropCurve) {
      ...
      return saleConfig.auctionSaleStartPrice - (steps * saleConfig.auctionSaleDropCurve);
    }
  }
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
   ...
   *TODO lockup 기능 추가
   */
  function lock(uint256 tokenId) public {} //검증 코드들

  /**
   *TODO lockup 기능 추가
   */
  function _lock(uint256 tokenId) internal {} //실제 동작로직
  ...
  function _transfer(
    ...
    if (prevOwnership.lockUntil != 0 && block.timestamp <= prevOwnership.lockUntil) revert TrasferLockedToken();
