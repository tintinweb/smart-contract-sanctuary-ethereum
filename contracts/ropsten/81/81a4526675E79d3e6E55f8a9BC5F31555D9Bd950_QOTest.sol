...
contract QOTest is Ownable, ERC721A, ReentrancyGuard {
  ...
  function publicSaleMint(uint64 quantity) external payable {
    ...
    if (_getAux(msg.sender) + quantity > maxTokensInPublicSale) revert ExceededPublicSaleLimit();
    if (totalSupply() + quantity > maxSupply) revert ExceededMaxSupply();
    if (saleConfig.price * quantity > msg.value) revert ValueBelowPrice();
    _mint(msg.sender, quantity, '', false);
