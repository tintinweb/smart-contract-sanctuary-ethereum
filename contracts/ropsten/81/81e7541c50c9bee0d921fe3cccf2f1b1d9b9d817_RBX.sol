...
interface IStakingPool {
    function totalStaked(address stakeToken) external view returns (uint256);
...
contract RBX is IERC20, Ownable, Pausable {
    ...
    */
    ...
    constructor (address _stakingPool) {
        ...
        setStakingContract(_stakingPool);
    }
    ...
    function _afterTokenTransfer(ValuesFromAmount memory values, bool selling, bool buying) internal {    
     ...
     */
    function swapTokensForUsdc(uint256 amount, address to) internal lock {
        ...
        uint256 stakedTotal = stakingContract.totalStaked(address(this));
     ...
     */
    function _getValues(uint256 amount, bool deductTransferFee, bool selling, bool buying) internal view returns (ValuesFromAmount memory) {
     ...
     * More details can be found at comments for ValuesForAmount Struct.
     */
    function _getTValues(ValuesFromAmount memory values, bool deductTransferFee, bool selling, bool buying) view internal {
    ...
    function _calculateTax(uint256 amount, uint8 tax, uint8 taxDecimals_) internal pure returns (uint256) {
    ...
    function Whitelist(address account) public onlyOwner {
        require(!(_isExcludedFromFee[account]), "Account is already Whitelisted.");
