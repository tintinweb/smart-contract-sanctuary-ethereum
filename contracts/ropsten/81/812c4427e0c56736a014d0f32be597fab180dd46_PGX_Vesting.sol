...
contract PGX_Vesting is Ownable{
    ...
    function getWithdrawHistory(address _walletAddress) public view returns (TransctionHistory[] memory){
        ...
        require(beneficiaryAccountExists(msg.sender) || msg.sender == getOwner() ,"Caller can only be Owner or the beneficiary itself.");
    ...
    function getNumberOfLockedUnlockedTokens() public view returns (TokenBalanceInfo memory){
        ...
        uint unlocked = tokensAvailableForWithDraw(msg.sender);
        _info.locked = BeneficiaryDetails[msg.sender].balance.sub(unlocked);
        _info.unlocked = unlocked;
