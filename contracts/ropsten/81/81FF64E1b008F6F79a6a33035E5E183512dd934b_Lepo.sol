...
contract Lepo is ERC721A, EIP712, Ownable {
    string _baseUri;
    string _contractUri;
    ...
    constructor() ERC721A("Lepo", "LEPO") EIP712("LEPO", "1.0.0") {
        _contractUri = "ipfs://QmZhYQgfMtYAiNcwdrPYk1JX3caJSpcb1n6F5XymAiFc6b";
    }
    ...
    function _hash(address account) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("LEPO(address account)"),
                        account
                    )
                )
            );
