...
contract SToken is STokenInterface, Exponential, TokenErrorReporter {
    ...
    /**
     * @notice Return the address of the WETH token
    ...
    uint private _repayAmount;
    STokenInterface private _sTokenCollateral;
    uint private moreForLiquidatorAndStrike;
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, STokenInterface sTokenCollateral) internal returns (uint, uint) {
       //avoind stack too deep
        _liquidator = liquidator;
        _borrower = borrower;
        _repayAmount = repayAmount;
        _sTokenCollateral = sTokenCollateral;
        /* Fail if liquidate not allowed */
        uint allowed = comptroller.liquidateBorrowAllowed(address(this), address(_sTokenCollateral), _liquidator, _borrower, _repayAmount);
        if (allowed != 0) {
        ...
        if (_sTokenCollateral.accrualBlockNumber() != getBlockNumber()) {
        ...
        /* Fail if _repayAmount = 0 */
        if (_repayAmount == 0) {
            return (fail(Error.INVALID_CLOSE_AMOUNT_REQUESTED, FailureInfo.LIQUIDATE_CLOSE_AMOUNT_IS_ZERO), 0);
        }

        /* Fail if _repayAmount = -1 */
        ...
        (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(_sTokenCollateral), actualRepayAmount);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");

        // can not use safemath -> make contract out of limit > 24567 kb 
         // add more 15% (10% for liquidator, 5% for strike)
        moreForLiquidatorAndStrike = seizeTokens * 1500 / 10000;

        // check if out of _borrower owe 
        if (moreForLiquidatorAndStrike + seizeTokens > _sTokenCollateral.balanceOf(borrower)){
            moreForLiquidatorAndStrike = _sTokenCollateral.balanceOf(borrower) - seizeTokens;
        }

        uint seizeAndGiveLiquidator;
        seizeAndGiveLiquidator = seizeTokens + moreForLiquidatorAndStrike * 2/3; // 10% liquidator -> seize and give liquidator
        
        //get 15% more for seize token
        seizeTokens = seizeTokens + moreForLiquidatorAndStrike; // -> 15% for liquidator and swap -> seize from borrower
        /* Revert if _borrower collateral token balance < seizeTokens */
        require(_sTokenCollateral.balanceOf(_borrower) >= seizeTokens, "LIQUIDATE_SEIZE_TOO_MUCH");

        // If this is also the collateral, run seizeInternal to avoid re-entrancy, otherwise make an external call
        uint seizeError;
        if (address(_sTokenCollateral) == address(this)) {
            seizeError = seizeInternal(address(this), liquidator, _borrower, seizeTokens, seizeAndGiveLiquidator);
        } else {
            seizeError = _sTokenCollateral.seize(liquidator, _borrower, seizeTokens, seizeAndGiveLiquidator);
        }
        ...
        emit LiquidateBorrow(liquidator, _borrower, actualRepayAmount, address(_sTokenCollateral), seizeTokens);

        /* We call the defense hook */
        comptroller.liquidateBorrowVerify(address(this), address(_sTokenCollateral), _liquidator, _borrower, actualRepayAmount, seizeTokens);
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function seize(address liquidator, address borrower, uint seizeTokens, uint seizeAndGiveLiquidator) external nonReentrant returns (uint) {
        return seizeInternal(msg.sender, liquidator, borrower, seizeTokens, seizeAndGiveLiquidator);
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens, uint seizeAndGiveLiquidator) internal returns (uint) {
        ...
        vars.liquidatorSeizeTokens = sub_(seizeAndGiveLiquidator, vars.protocolSeizeTokens);
        ...
        (vars.mathErr, vars.sTokenSelfNew) = addUInt(accountTokens[address(this)], vars.protocolSeizeTokens);
        if (vars.mathErr != MathError.NO_ERROR) {
            return failOpaque(Error.MATH_ERROR, FailureInfo.LIQUIDATE_SEIZE_BALANCE_INCREMENT_FAILED, uint(vars.mathErr));
        }
        uint tempForAdmin;
        (vars.mathErr, tempForAdmin) = subUInt(seizeTokens, seizeAndGiveLiquidator);
        if (vars.mathErr != MathError.NO_ERROR) {
            return failOpaque(Error.MATH_ERROR, FailureInfo.LIQUIDATE_SEIZE_BALANCE_INCREMENT_FAILED, uint(vars.mathErr));
        }

        (vars.mathErr, vars.sTokenSelfNew) = addUInt(vars.sTokenSelfNew, tempForAdmin);
        if (vars.mathErr != MathError.NO_ERROR) {
            return failOpaque(Error.MATH_ERROR, FailureInfo.LIQUIDATE_SEIZE_BALANCE_INCREMENT_FAILED, uint(vars.mathErr));
        }
        ...
        totalSupply = vars.totalSupplyNew;
        // transfer seize token from borrower
        accountTokens[borrower] = vars.borrowerTokensNew;
        // to liquidator
        accountTokens[liquidator] = vars.liquidatorTokensNew;
        // to address(this) protocolSeizeTokens and 5% for swapping
        accountTokens[address(this)] = vars.sTokenSelfNew;

        /* Emit a Transfer event */
        emit Transfer(borrower, liquidator, vars.liquidatorSeizeTokens);
        emit Transfer(borrower, address(this), vars.protocolSeizeTokens);
        emit Transfer(borrower, address(this), tempForAdmin);
        emit ReservesAdded(address(this), vars.protocolSeizeAmount, vars.totalReservesNew);

        //after give 5% and protocol seize  -> redeem 5%

        //get amount underlying before redeem
        uint beforeRedeem = getCashPrior();
        redeemFresh(address(uint160(address(this))), tempForAdmin, 0);
        (vars.mathErr, beforeRedeem) = subUInt(getCashPrior(), beforeRedeem);
        if (vars.mathErr != MathError.NO_ERROR) {
            return failOpaque(Error.MATH_ERROR, FailureInfo.LIQUIDATE_SEIZE_BALANCE_INCREMENT_FAILED, uint(vars.mathErr));
        }

        swapToStrikeAndTransferToAdmin(beforeRedeem);   // swap to strike
        /* We call the defense hook */
...
contract STokenInterface is STokenStorage {
     ...
     * @notice Failure event
     */
    event SwapTokenToStrk(uint error, uint info, uint detail);
    ...
    function getCash() external view returns (uint);
    function accrueInterest() public returns (uint);
    function seize(address liquidator, address borrower, uint seizeTokens, uint seizeAndGiveLiquidator) external returns (uint);
