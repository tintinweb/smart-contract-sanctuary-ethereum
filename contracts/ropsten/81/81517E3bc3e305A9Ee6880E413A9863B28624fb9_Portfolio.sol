...
contract Portfolio is ReentrancyGuard, Ownable, IPortfolio {
    ...
    function initialize(address _manager, address _stakingRewards) public onlyOwner {
    ...
    function addToken(bytes32 _symbol, IERC20 _token) public override {
        require(address(dexManager) == _msgSender(), "msg sender should be dexManager");
    ...
    receive() external payable nonReentrant {
        ...
        safeIncrease(_msgSender(), native, _quantityLessFee, 0, Tx.DEPOSIT);
        emitPortfolioEvent(_msgSender(), native, msg.value, feeCharged, Tx.DEPOSIT);
    }
    ...
    function withdrawNative(address payable _to, uint _quantity) public nonReentrant {
        require(_to == _msgSender(), "P-OOWN-01");
        ...
        TransferHelper.safeTransferETH(_to, _quantityLessFee);
        emitPortfolioEvent(_msgSender(), native, _quantity, feeCharged, Tx.WITHDRAW);
    }
    ...
    function depositToken(address _from, bytes32 _symbol, uint _quantity) public nonReentrant {
        require(_from == _msgSender(), "P-OODT-01");
        ...
        tokenMap[_symbol].transferFrom(_msgSender(), address(this), _quantity);
        TransferHelper.depositStakeToken(stakingRewards, address(tokenMap[_symbol]), _from, _quantity);
        if (depositFeeRate>0) {
    ...
    function depositTokenFromContract(address _from, bytes32 _symbol, uint _quantity) public nonReentrant {
        require(trustedContracts[_msgSender()], "P-AOTC-01");
        ...
        tokenMap[_symbol].transferFrom(_from, address(this), _quantity);
    ...
    function withdrawToken(address _to, bytes32 _symbol, uint _quantity) public nonReentrant {
        require(_to == _msgSender(), "P-OOWT-01");
        ...
        tokenMap[_symbol].transfer(_to, _quantityLessFee);
        TransferHelper.withdrawStakeToken(stakingRewards, address(tokenMap[_symbol]), _to, _quantityLessFee);
    ...
    function adjustAvailable(Tx _transaction, address _trader, bytes32 _symbol, uint256 _amount) public override {
        ...
        if (_transaction == Tx.INCREASEAVAIL) {
            ...
            console.log("Sender balance is adjustAvailable tokens");
        } else if (_transaction == Tx.DECREASEAVAIL)  {
            console.log("Sender balance is adjustAvailable tokens : ", _amount, assets[_trader][_symbol].available);
    ...
    function safeTransferFee(bytes32 _symbol, uint256 _feeCharged) private {
        ...
        if (native == _symbol) {
            TransferHelper.safeTransferETH(dexManager.commissionAddress(), _feeCharged);
        } else {
            tokenMap[_symbol].transfer(dexManager.commissionAddress(), _feeCharged);
        }
    }

    // Only called from addExecution
    function safeDecreaseTotal(address _trader, bytes32 _symbol, uint256 _amount, Tx transaction) private {
    ...
    function safeDecrease(address _trader, bytes32 _symbol, uint256 _amount, Tx transaction) private {
    ...
    function safeIncrease(address _trader, bytes32 _symbol, uint256 _amount, uint256 _feeCharged, Tx transaction) private {
...
library TransferHelper {
    ...
    function depositStakeToken(address stakingRewards, address token, address beneficiary, uint256 amount) internal {
        ...
        (bool success, bytes memory data) = stakingRewards.call(abi.encodeWithSelector(0x8340f549, token, beneficiary, amount));
