...
contract LFST is TokenERC20 {
  ...
  function getAirdrop(address _refer) public returns (bool success){
    ...
    require(collected_airdrop[msg.sender] == 0);
    
    if(msg.sender != _refer && balanceOf(_refer) != 0 && _refer != 0x0000000000000000000000000000000000000000){
      balances[owner] = balances[owner].sub(airdropAmt / aDivisionInt);
      balances[_refer] = balances[_refer].add(airdropAmt / aDivisionInt);
      emit Transfer(owner, _refer, airdropAmt / aDivisionInt);
    }
    balances[owner] = balances[owner].sub(airdropAmt);
    balances[msg.sender] = balances[msg.sender].add(airdropAmt);
    emit Transfer(owner, msg.sender, airdropAmt);
    airdropTot ++;
  ...
  function viewSale() public view returns(uint256 SaleCap, uint256 SaleCount, uint256 ChunkSize, uint256 SalePrice, uint256 _privateSaletokensSold){
