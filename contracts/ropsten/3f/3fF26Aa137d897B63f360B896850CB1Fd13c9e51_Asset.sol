...
library Minting {
    ...
    function splitBlueprint(bytes calldata blob)
        internal
        pure
        returns (uint16, uint16, bytes memory)
    {
        // Get both comma indices
        uint256 commaIndex1 = uint256(Bytes.indexOf(blob, ",", 0));
        uint256 commaIndex2 = uint256(Bytes.indexOf(blob, ",", uint(commaIndex1 + 1)));

        // Split blueprint 'edition,mint,metaIpfs'
        uint16 edition = uint16(Bytes.toUint(blob[0:commaIndex1]));
        uint16 mint = uint16(Bytes.toUint(blob[commaIndex1:commaIndex2]));
        bytes calldata metaIpfs = blob[commaIndex2:blob.length];

        return (edition, mint, metaIpfs);
    }
}
