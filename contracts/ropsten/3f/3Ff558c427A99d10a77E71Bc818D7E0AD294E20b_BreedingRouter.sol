...
contract BreedingRouter is Ownable, Pausable, ERC721Holder, ReentrancyGuard {
    ...
    uint256 public constant secondsPerBlock = 15;
    ...
    uint256 public gatchaBunnyCost;
    ...
    uint256 public constant itemPrice = 200 * 1e18;
    uint256 public constant dripCost = 200 * 1e18;
    ...
    event ApplyItem(address indexed user, uint256 indexed nftId);
    ...
    ////10000 is 100%
    function setDripRate(uint256 _rate) external onlyOwner {
        dripRate = _rate;
    }

    function setGatchaBunnyCost(uint256 _cost) external onlyOwner {
        gatchaBunnyCost = _cost;
    }
    ...
    function resetFerdility(uint256 tokenId) external {
        ...
        ERC20(address(rewardToken)).safeTransferFrom(msg.sender, rentFeeRecient, resetFerdilityCost);
    ...
    function changeGen1Ferdility(uint256 tokenId) external {
        ...
        ERC20(address(rewardToken)).safeTransferFrom(msg.sender, rentFeeRecient, turnOnBreedGen1Cost);
        idToBreedCounts[tokenId] = 0;
    }
    ...
    function setIntToBreedBoots(uint16[] calldata breedBoots) external onlyOwner {
    ...
    function _getBreedingTime(
        ...
        (bytes32 gens1, , , , ) = nftAddress.bunnies(bunnyId1);
        (bytes32 gens2, , , , ) = nftAddress.bunnies(bunnyId2);
        uint256 breedBoots1 = intToBreedBoots[uint8(_parseBytes32(gens1, 1))];
        uint256 breedBoots2 = intToBreedBoots[uint8(_parseBytes32(gens2, 1))];
    ...
    function _breedingWithRentedBunny(
        ...
        require(getFertility(rentedBunnyId) > 0, "Not enough Ferdility");
        if (useNyanKee) {
            require(
            ...
            ERC20(address(rewardToken)).safeTransferFrom(msg.sender, rentFeeRecient, nyanKeeCost);
        }
        ...
        ERC20(address(rewardToken)).safeTransferFrom(msg.sender, rentFeeRecient, pillCost);
    ...
    function gatchaBunny(uint256 tokenId, uint256 itemId) external {
        require(nftAddress.ownerOf(tokenId) == msg.sender, "you are not owner of bunny");
        require(rewardToken.balanceOf(msg.sender) >= gatchaBunnyCost, "Not enougn token for this function");
        ERC20(address(rewardToken)).safeTransferFrom(msg.sender, rentFeeRecient, gatchaBunnyCost);
        gatchaItem.safeTransferFrom(msg.sender, burnAddress, itemId, 1, "0x");
    ...
    function buyGatchaItem(uint256 quantity) external {
        ...
        ERC20(address(rewardToken)).safeTransferFrom(msg.sender, rentFeeRecient, itemPrice * quantity);
        for (uint256 i = 0; i < quantity; i++) {
            uint256 itemId = _randomItem(i);
            gatchaItem.mint(msg.sender, itemId, 1);
            emit UserBuyGatchaItem(msg.sender, itemId);
        }
    }

    function _randomItem(uint256 i) internal view returns (uint256) {
        uint256 rd = _rand(i);
        uint256 tokenId;
        if (rd <= 500) {
            tokenId = 1;
        } else if (500 < rd && rd <= 1000) {
            tokenId = 2;
        } else if (1000 < rd && rd <= 1500) {
            tokenId = 3;
        } else if (1500 < rd && rd <= 2000) {
            tokenId = 4;
        } else if (2000 < rd && rd <= 2500) {
            tokenId = 5;
        } else if (2500 < rd && rd <= 3000) {
            tokenId = 6;
        } else if (3000 < rd && rd <= 3500) {
            tokenId = 7;
        } else if (3500 < rd && rd <= 4000) {
            tokenId = 8;
        } else if (4000 < rd && rd <= 4500) {
            tokenId = 9;
        } else if (4500 < rd && rd <= 5000) {
            tokenId = 10;
        } else if (5000 < rd && rd <= 5500) {
            tokenId = 11;
        } else if (5500 < rd && rd <= 5500) {
            tokenId = 12;
        } else if (6000 < rd && rd <= 6500) {
            tokenId = 13;
        } else if (6500 < rd && rd <= 7000) {
            tokenId = 14;
        } else if (7000 < rd && rd <= 7500) {
            tokenId = 15;
        } else if (7500 < rd && rd <= 8000) {
            tokenId = 16;
        } else if (8000 < rd && rd <= 8500) {
            tokenId = 17;
        } else if (8500 < rd && rd <= 9000) {
            tokenId = 18;
        } else if (9000 < rd && rd <= 9500) {
            tokenId = 19;
        } else if (9500 < rd && rd <= 9999) {
            tokenId = 20;
        }
        return tokenId;
    }
    ...
    function gatchaBunnyWithRandomItem(uint256 tokenId) external {
        ...
        require(rewardToken.balanceOf(msg.sender) >= gatchaBunnyCost, "Not enougn token for this function");
        ERC20(address(rewardToken)).safeTransferFrom(msg.sender, rentFeeRecient, gatchaBunnyCost);
        ERC20(address(rewardToken)).safeTransferFrom(msg.sender, rentFeeRecient, itemPrice);
        uint256 itemId = _randomItem(tokenId);
        emit UserBuyGatchaItem(msg.sender, itemId);
        emit UserGatchaBunny(msg.sender, tokenId, itemId);
    }
    ...
    function giveBirth(uint256 tokenId) external returns (uint256) {
        ...
        require(!paused(), "contract paused");
        require(isPregnant(tokenId), "bunny don't breeding yet");
        ...
        // random 0->9999
        uint256 random = _rand(breed.bunnyId1);
    ...
    function isPregnant(uint256 tokenId) public view returns (bool) {
    ...
    function viewNftRate(uint256 _nftId) public view returns (uint16) {
        ...
        uint8 earnRateInt = uint8(_parseBytes32(genes, 2));
    ...
    function viewReward(address _user, uint256 _nftId) public view returns (uint256) {
        ...
        if (user.item[_nftId].endBlock != 0 && user.lastRewardBlock[_nftId] <= user.item[_nftId].endBlock) {
    ...
    function applyItem(uint256 _nftId) public nonReentrant {
        ...
        ERC20(address(rewardToken)).safeTransferFrom(msg.sender, rentFeeRecient, dripCost);
        UserInfo storage user = userInfo[msg.sender];
        ...
        emit ApplyItem(msg.sender, _nftId);
    }
}
