...
contract JCFv3 is StandardToken {
    ...
    // timestamp to index
    mapping (uint256 => uint256) public indexOfPortfolio;

    // cryptoTypes to names
    mapping (uint256 => string) public cryptoNames;
    ...
    constructor (address _dividendWallet, address _teamAndDonationWallet) public {
        ...
        storeCryptoNames('BTC');
        storeCryptoNames('ETH');
    ...
    function storeCryptoNames(string _crypto) public only_if_controlWallet {
    ...
    function storePortfolio(uint256 _timestamp, uint256[] _cryptoTypes, uint256[] _amounts, uint256[] _prices) external only_if_controlWallet {
        ...
        while (i < _cryptoTypes.length && i < _amounts.length && i < _prices.length) {
            ...
            portfolios[currentPortfolio].cryptoTypes.push(cryptoNames[_cryptoTypes[i]]);
            portfolios[currentPortfolio].amounts.push(_amounts[i]);
            portfolios[currentPortfolio].prices.push(_prices[i]);
            i = safeAdd(i, 1);
        }
