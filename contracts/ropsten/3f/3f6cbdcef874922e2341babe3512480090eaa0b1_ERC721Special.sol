...
contract FeesDistribution {
    ...
    uint256 internal totalPercentage = 0;
    uint256 internal totalFees;
    mapping (address => uint256) _comissionPercentage;
    mapping (address => uint256) _lastKnownAmount;

    function _getFeeAmount(address user, uint256 currentAmount) internal view returns(uint256) {
        return (currentAmount - _lastKnownAmount[user]) * _comissionPercentage[user] / totalPercentage / PRECISION;
    }
...
abstract contract ERC721 is Initializable, ContextUpgradeable, IERC721MetadataUpgradeable {
    ...
    address[MINTING_LIMIT] _allowances;
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory) {
        return string(abi.encode(_uri, tokenId.toString()));
    }
    ...
    function balanceOf(address _user) public notZeroAddress(msg.sender) view override returns (uint256) {
        uint256 counter = 0;
        for (uint256 i = 0; i < MINTING_LIMIT; i++) {
            if (_user == _owners[i]) counter++;
        }
        return counter;
    }
...
contract ContractRoles is AccessControlUpgradeable {
    ...
    bytes32 public constant AIRDROP_ROLE = "AIRDROP_ROLE";
...
contract ERC721Special is Initializable, ERC721, ContractRoles, Whitelist, FeesDistribution {
    ...
    uint256 public _defaultMintingPrice = 2e17;
    uint256 public _whitelistMintingPrice = 1e17;
    ...
    function _withdrawFees(address user) internal reentrancyGuard {
        require(
            _comissionPercentage[user] > 0,
            "User cannot withdraw rewards or withdrawed rewards recently"
        );
