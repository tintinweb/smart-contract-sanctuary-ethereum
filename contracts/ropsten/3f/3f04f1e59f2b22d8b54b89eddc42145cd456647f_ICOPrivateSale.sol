...
contract ICOPrivateSale is Initializable, OwnableUpgradeable, UUPSUpgradeable{

    event Print(string msg, uint256 stime, uint256 _etime);
 

    uint256  presaleRate; 
    address  MTCtokenContractAddr;
    address  USDTContractAddr; 
    uint8 icoTokenDecimal;
    uint8 busdDecimal;
    uint256  RaisedFund;   
    uint256  startTime; 
    uint256  endTime; 
    ...
    bool public isVestingDone;
    bool public isRefundDone;
    uint8 vestingRound;
    ...
    function initialize(address _MTCtokenContractAddr, address _USDTContractAddress, uint256 _presaleRate, uint256 _startTime, uint256 _endTime, 
                            uint256 _softCap, uint256 _hardCap, uint256 _totalTokenSupply) initializer public {
        __Ownable_init();
        __UUPSUpgradeable_init();

        USDTContractAddr = _USDTContractAddress;
        MTCtokenContractAddr = _MTCtokenContractAddr;
        presaleRate = _presaleRate;
        startTime = _startTime/10**icoTokenDecimals;
        endTime = _endTime/10**icoTokenDecimals;
        softCap = _softCap/10**icoTokenDecimals;
        hardCap = _hardCap/10**icoTokenDecimals;
        totalTokenSupply = _totalTokenSupply;
        icoTokenDecimals = Token(MTCtokenContractAddr).decimals();
        busdTokenDecimals = Token(USDTContractAddr).decimals();
        icoTokenDecimal = Token(MTCtokenContractAddr).decimals();
        busdDecimal = Token(USDTContractAddr).decimals();
    }
 ...
 // SaleICOToken function that will use to sell the SoldOut Token
    function SaleICOToken(uint256 _busdAmount) public returns (bool) {
        ...
        require(soldOutTokens + (_busdAmount*10**icoTokenDecimals)/presaleRate <= totalTokenSupply,"This Transaction may exceed ICO Maximux Allocation Limit for public sale, Please try with lower bnbAmount.");
        // Collect BUSD to ICO Contract Address
        Token(USDTContractAddr).transferFrom(msg.sender, address(this), _busdAmount); 
    ...
    function getTokenomics() public view returns(uint8, uint8, uint256, uint256, uint256, uint256, uint256, uint256){
        return(icoTokenDecimals, busdTokenDecimals, presaleRate, startTime, endTime, soldOutTokens, RaisedFund, totalTokenSupply);
    }
    ...
    function setSellTrack(uint256 _busdAmount) private {
        ...
        soldOutTokens +=  (_busdAmount*10**icoTokenDecimals)/presaleRate;
    ...
    function isIcoOver() public view returns (bool) {
        if (
            (block.timestamp >= endTime || (soldOutTokens >= totalTokenSupply))
        ) {
            return true;
        } else {
            return false;
    ...
    function Vesting() public onlyOwner returns (bool) {
        ...
        require(!isRefundDone, "Refund Already Done");
        require(!isVestingDone, "Vesting Already Done");
        require(soldOutTokens <= Token(MTCtokenContractAddr).balanceOf(address(this)), "IDO does not have supply to provid the tokens");
        for(uint8 i=0; i<SellTrackAddr.length; i++)
        {
            Token(MTCtokenContractAddr).transfer(SellTrackAddr[i],  (SellTrackAddrAmount[i]*10**icoTokenDecimals)/(9*presaleRate)); // (1/9) tokens will transfr
        }
        vestingRound += 1;
        if(vestingRound == 8) {
            isVestingDone = true;
        ...
        /*1. After Sale End*/
        require(isIcoOver(), "Sale is not Ended. Wait for it");
        require(vestingRound == 0, "Vesting Already Started");
        require(RaisedFund <= Token(USDTContractAddr).balanceOf(address(this)), "IDO does not have supply to provid the USDT tokens");
        for(uint8 i=0; i<SellTrackAddr.length; i++)
        {
            Token(USDTContractAddr).transfer(SellTrackAddr[i],  (SellTrackAddrAmount[i])); 
        }
    ...
    function setTokenContract(address _tokenContract) public onlyOwner {
        MTCtokenContractAddr = _tokenContract;
    }
}