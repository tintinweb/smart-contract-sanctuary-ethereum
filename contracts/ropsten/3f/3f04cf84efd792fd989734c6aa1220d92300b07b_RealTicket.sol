  ...
  ERC721Status
{
  ...
  event OnSale(uint256 tokenId, uint256 price);
  event Bought(uint256 tokenId, uint256 price);
  ...
  function use(uint256 tokenId) public virtual {
      require(hasRole(SECURITY_ROLE, _msgSender()), "RealTicket: must have security role");
      _useTicket(tokenId);
  }

  function block(uint256 tokenId) public virtual {
      require(hasRole(SECURITY_ROLE, _msgSender()), "RealTicket: must have security role");
  ...
  function firstBuy() public payable {
      ...
      emit Bought(_tokenIdTracker.current(), _price);
      _tokenIdTracker.increment();
  }
  ...
  function secondBuy(uint256 tokenId, address payable from) public payable {
      ...
      emit Bought(tokenId, _sale[tokenId]);
      _sale[tokenId] = 0;
  }
