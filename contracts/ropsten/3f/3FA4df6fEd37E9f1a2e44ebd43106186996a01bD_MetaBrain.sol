...
import "IMetaBrain.sol";
import "IBrainOracle.sol";
import "IMetaDungeon.sol";
...
contract MetaBrain is IMetaBrain, IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    IBossFactory public immutable bossFactory;
    ...
    function getTeamIdByAnftId(uint teamSize, uint anftTokenId, uint playerId) external view returns (uint) {
        return uint(keccak256(abi.encodePacked(epochRandNums[anftTokenId], playerId))) % (IERC721Enumerable(address(players)).totalSupply() / teamSize);
    }
    ...
    function newBoss(
        ...
        boss = bossFactory.newBoss(anftTokenId, address(this), config, owner());
