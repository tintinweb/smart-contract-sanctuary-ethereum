...
contract TestContract is ERC721, Ownable {
    ...
    uint256 public SALE_PRICE = 0.065 ether; //0.175 ether;
    ...
    function _setSaleState(uint16 _sale_state) public ownerOnly {
        if (_sale_state == 0) saleState = SaleState.Closed;
        else if (_sale_state == 1) saleState = SaleState.Restricted;
        else if (_sale_state == 2) saleState = SaleState.Open;
    ...
    function buy(uint256 quantity) external payable {
        ...
        require(msg.value == SALE_PRICE * quantity, "Droplets: insufficient value");
        require(minted[msg.sender] + quantity <= quantity, "Droplets: mint limit for user reached");
