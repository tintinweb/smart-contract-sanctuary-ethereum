    ...
    ReentrancyGuardUpgradeable
{
   ...
   /**
    ...
    /**
     * @notice Return the rewards tier scheme in four arrays that 
     * correspond to the columns of the rewards scheme.
     * @return referralsUpTo[] Tiers max value
     * @return rewardAmountERC20[] ERC20 reward amount by tier
     * @return rewardAmountERC721[] ERC721 reward amount by tier
     * @return rewardERC721Step[] ERC721 reward step
     * @dev Returning dynamic arrays is not supported, so I have to create fixed size memory arrays.
     */
    function getRewardsTiers()
        external
        view 
        onlyAdmins
        returns (
    ...
    {
        uint itemsCount = tiersInfo.length;
        uint[] memory referralsUpTo = new uint[](itemsCount);
        uint[] memory rewardAmountERC20 = new uint[](itemsCount);
        uint[] memory rewardAmountERC721 = new uint[](itemsCount);
        uint[] memory rewardERC721Step = new uint[](itemsCount);

        for (uint i = 0; i < itemsCount; i++) {
            referralsUpTo[i] = tiersInfo[i].referralsUpTo;
            rewardAmountERC20[i] = tiersInfo[i].rewardAmountERC20;
            rewardAmountERC721[i] = tiersInfo[i].rewardAmountERC721;
            rewardERC721Step[i] = tiersInfo[i].rewardERC721Step;
        }
    ...
    /**
     * @notice Return all program parameters in two arrays where 
     * the first corresponds to addresses and the second to numeric parameters
     * @return addresses[] 
     * @return params[]
     */
    function getLatestParameters()
        external
        view
        onlyAdmins
        returns (
    ...
    {
        address[] memory adressess = new address[](3);
        uint[] memory params = new uint[](6);

        adressess[0] = publisher;
        adressess[1] = rewardTokenERC20;
        adressess[2] = rewardTokenERC721;

        params[0] = gameId;
        params[1] = programEndTime;
        params[2] = maxRewardAmount;
        params[3] = minClaimAmount;
        params[4] = refereeRewAmountERC20;
        params[5] = refereeRewAmountERC721;

        return (adressess, params);
    }
