...
contract TokenSLP is Context, IERC20, IERC20Metadata, Ownable {
    ...
    string private _name = "Token SLP";
    string private _symbol = "&#9671;NEO";
    uint256 private _initSupply = 1000 * 10**18;

    uint256 public sellFee = 6; //take fee while sell token to dex
    ...
    constructor() {
        ...
        //IPancakeRouter _router = IPancakeRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); //for test
    ...
    function setFeeAddress(address _taxAddress) public onlyOwner {
    ...
    function _transfer(
        ...
        if (recipient == pairAddress && takeFee) {
            uint256 taxFee = amount.mul(sellFee).div(100);
            ...
            uint256 taxFee = amount.mul(buyFee).div(100);
            _balances[taxAddress] = _balances[taxAddress].add(taxFee);
            emit Transfer(sender, taxAddress, taxFee);
            amount = amount.sub(taxFee);
        }
