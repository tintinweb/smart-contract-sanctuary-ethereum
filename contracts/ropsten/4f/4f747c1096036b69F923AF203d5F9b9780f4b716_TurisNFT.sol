...
contract TurisNFT is ERC721URIStorage {
  ...
  mapping(uint256 => destinyNFT) public destinyMap;

    constructor() ERC721("TurisNFT", "TNFT") {
    ...
    function createToken(string memory tokenURI, string memory _destiny) payable public returns (uint) {
        //Increment the tokenId counter, which is keeping track of the number of minted NFTs
        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();

        require(msg.value == listPrice, "Please submit the asking price in order to complete the purchase");
        ...
        destinyMap[newTokenId].destiny = _destiny;
