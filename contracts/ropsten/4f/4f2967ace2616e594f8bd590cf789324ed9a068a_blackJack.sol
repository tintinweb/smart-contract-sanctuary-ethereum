...
contract blackJack is Ownable {
    ...
    event handEndEvent(uint indexed playerPower, uint indexed housePower, bool indexed playerWin); //will only log non-split wins/losses, not pushes
    ...
    function isBlackjack() internal returns (bool aceHole){ //fill this function later to check both player and dealer for a blackjack after _play is called, then process
