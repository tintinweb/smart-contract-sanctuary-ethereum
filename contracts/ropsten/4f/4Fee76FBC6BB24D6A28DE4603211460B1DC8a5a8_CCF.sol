...
contract CCF {
    ...
    uint16[30] CoinTypePlaced;
    bool[30] GameStarted;
    uint private randNonce = 0;
    uint private Tax =6;
    bool public VsBotsActive=false;

    address constant public ContractOwner = 0xa4087A999288B7866f31E8b5537721c92584dE97;
    ...
    function SetRandomNonce() public {
        randNonce =uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,  randNonce))) % 100;
    }
    ...
    require(balance[ContractOwner]>PriceOfTable[GameID]);
            ...
            //Pick random number
            randNonce++;
            uint Rng = uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,  randNonce))) % 2;
    ...
    function CancelGame(uint GameID) public{
        if(msg.sender == PlayerOneAddress[GameID])
        {
    ...
    function CreateGame(uint amount, uint16 CoinType) public {
