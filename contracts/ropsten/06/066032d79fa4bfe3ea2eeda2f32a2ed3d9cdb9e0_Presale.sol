...
contract Presale {
    ...
    function Config(IERC20 token_addr, IERC20 wdf_addr, uint256 pricetoken_amount, uint256 priceeth_amount, address team_addr) public isOwner returns (bool success) {
		require(pricetoken_amount > 0, "Config: missing by token price");
		require(priceeth_amount > 0, "Config: missing by eth price");
		require(team_addr != address(0), "Config: missing team address");
		_token = token_addr;
		_wdf = wdf_addr;
		_team = payable(team_addr);
	...
	function BuyWDF(uint256 amount) public returns (bool success) {
        ...
        require(_token.transferFrom(msg.sender, _team, paid) == true, "BuyWDF: Couldn't transfer tokens to WDF Team");
