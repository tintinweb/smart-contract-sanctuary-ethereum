 ...
 */
contract MetaTopeAuction is Ownable, MetaTopeIERC1155Receiver {
    ...
    event EndAuction();
    event Claimed();
    event ClaimFailed(address indexed user);
    ...
    modifier onlyWhitelisted() {
        if (isWhitelistMode) {
            require(
                whitelistedBidders[msg.sender] == true,
                "Bidder should be whitelisted"
            );
        }
        _;
    }
     ...
     */
    function setWhitelist(address _bidder) external onlyOwner {
        whitelistedBidders[_bidder] = true;
    }
        ...
        onlyStarted
    {
        ...
        if (bidders[msg.sender].tokenAmount == 0) {
            bidderAddresses.push(msg.sender); // track bidder addresses
            emit MakeBid(
                msg.sender,
                _tokenAmount,
                _pricePerToken,
    ...
    function claim(address[] memory _bidders) external onlyOwner onlyEnded {
        for (uint256 i = 0; i < _bidders.length; ++i) {
            ...
            // It will transfer to those that are whitelisted and for
            // those not whitelisted nothing will happen.
            // Using if checks here because if we do require then the whole
            // transaction will fail even for bidders that are whitelisted
            if (isWhitelistMode) {
                if (whitelistedBidders[_bidder]) {
                    transferTokens(_bidder);
                    emit Claimed();
                } else {
                    emit ClaimFailed(_bidder);
                }
            } else {
                transferTokens(_bidder);
                emit Claimed();
    ...
    /**
     * @dev Function to transfer tokens between contract and bidder
     * @param _bidder address to claim
     */
    function transferTokens(address _bidder) internal {
        uint256 _approvedAmount = depositToken.allowance(
            _bidder,
            address(this)
        );
        if (_approvedAmount >= bidders[_bidder].totalPrice && !bidders[_bidder].claimed) {
            depositToken.transferFrom(
                _bidder,
                address(this),
                bidders[_bidder].totalPrice
            );
            rewardToken.safeTransferFrom(
                address(this),
                _bidder,
     ...
     * @param _to withdraw
     */
    function withdraw(address _to) onlyOwner onlyEnded external {
        ...
        depositToken.transfer(_to, _depositTokenAmount);
