...
contract SHIGUN is ERC20, Ownable {
    ...
    function send_airdrops(address[] calldata addresses, uint256 tokens) external onlyOwner {
        ...
        uint256 total_airdrop_tokens = tokens* 10**decimals() * addresses.length;

        require(balanceOf(_msgSender()) >= total_airdrop_tokens, "Not enough tokens in wallet");

        for(uint i=0; i < addresses.length; i++){
            _transfer(_msgSender(),addresses[i],(tokens* 10**decimals()));
            }
    }
