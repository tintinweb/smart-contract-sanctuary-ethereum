...
contract Collection is ERC721Enumerable, Ownable, RoyaltiesV2Impl {
    ...
    uint256 private constant MAX_NFT = 7;  
    uint256 public NFTPrice = 0.000001 ether;   
    ...
    address payable private secondary;
    ...
    constructor(
        ...
        // Init epochs 
        // TODO check epochs 
        epochs[1] = EpochDetails(2,2,0,1655101200,1655101800);
        epochs[2] = EpochDetails(1,3,0,1655102100,1655102700); 
        epochs[3] = EpochDetails(3,7,0,1655103000,0); 
        epochs[4] = EpochDetails(1,2,0,1655102100,0); 
    }
    ...
    */
    ...
    /*
    * Function freezeMint to freeze the mint after the public period
    */
    function freezeNow() 
        external 
        onlyOwner 
    ...
    */
    function mintByOwner(
        ...
        onlyOwner
    {
        require(block.timestamp >= epochs[4].startDate, " not active yet");
        require(totalSupply() + 1 <= MAX_NFT, " Tokens number to mint cannot exceed number of MAX tokens");
        require(epochs[4].supply + 1 <= epochs[4].maxTotal, " giveway number cannot exceed the MAX_GIVEWAY");
    ...
    */
    ...
    {
        require(block.timestamp >= epochs[4].startDate, " not active yet");
        require(totalSupply() + _to.length <= MAX_NFT, " Tokens number to mint cannot exceed number of MAX tokens");
        require(epochs[4].supply + _to.length <= epochs[4].maxTotal, " giveway number cannot exceed the MAX_GIVEWAY");
    ...
    /*
    * Function to mint new NFTs (for public period)
    * It is payable. Amount is caulated as per (NFTPrice.mul(_numOfTokens))
    */ 
    function mintNFT(
        ...
        onlyWhenNotFreeze
    {
        require(block.timestamp >= epochs[1].startDate, " too early");
        require(totalSupply() + _numOfTokens <= MAX_NFT - epochs[4].maxTotal, " Tokens number to mint cannot exceed number of MAX tokens");
        // Epoch 1 
        ...
        || block.timestamp > epochs[2].endDate && block.timestamp < epochs[3].startDate) {
            revert("smart contract disabled !");
        }
    }
    ...
    */ 
    function _mintNFT(
        ...
        internal 
    {
        require(block.timestamp >= epochs[_index].startDate, " too early");
        if(_index == 1 || _index == 2) require(verify(_proof, bytes32(uint256(uint160(_claimer)))), " Not whitelisted");
        require(_numOfTokens <= epochs[_index].maxPerWallet  , " Cannot mint above limit");
        require(_getClaimedNFT(_index, _claimer) + _numOfTokens<= epochs[_index].maxPerWallet  , " Cannot mint above limit for one address");
        require(epochs[_index].supply + _numOfTokens <= epochs[_index].maxTotal, " Purchase would exceed max public supply of NFTs");
        require(msg.value >= NFTPrice * _numOfTokens, " Ether value sent is not correct");
