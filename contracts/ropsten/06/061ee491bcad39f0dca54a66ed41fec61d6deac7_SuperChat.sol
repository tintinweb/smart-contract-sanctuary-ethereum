...
contract SuperChat is ReentrancyGuard{
    ...
    function sendMsg(uint _amount,string memory _msgHash, uint _deadline, address _receipient, uint _version ) public nonReentrant {
        
        require(_amount > 0, 'Please provide a minimum amount to send msg');
    ...
    function readMsg(uint _id) public nonReentrant returns(string memory) {
        ...
        SafeERC20.safeTransfer(BUSD, msg.sender, response.amount);
    ...
    function revertMsg(uint _id) public nonReentrant {
        ...
        require(response.sender != address(0), 'Message at this id does not exist');
        require(response.sender == msg.sender,'You can only read your message');
    ...
    function removeElementfromReceiver(address _receipient, uint _index) internal {
     ...
     function msgMetaData(uint _id) public view returns(address,address,uint,uint,uint){
         MsgInfo memory response = createdMsgs[_id];
         return(response.sender, response.receiver, response.amount, response.deadline, response.createdAt);
