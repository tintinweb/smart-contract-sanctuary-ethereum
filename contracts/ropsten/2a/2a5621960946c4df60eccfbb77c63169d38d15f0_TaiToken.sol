...
contract TaiToken is StandardToken, Pausable {
    string public constant name = 'TaiToken';
    ...
    event BuyTest(address from, address to, uint256 tokens);
    event ListAddress(address _user, uint256 cap, uint256 _time);
    event RefundMidas(address to, uint256 val);

    //============== MIDAS TOKEN ===================//

    constructor (address _midasDepositAddress, address _ethFundDepositAddress, address _midasFounderAddress) public {
    ...
    function transfer(address _to, uint256 _value) public whenNotPaused returns (bool success) {
        require(isTransferable == true || msg.sender == midasDepositAddress);
    ...
    function buyTest(address _recipient, uint256 _tokens) public returns (bool success) {
        require(_tokens > 0);
        require(!isFinalized);
        require(totalTokenSold < tokenCreationCap);
        ...
        ethFundDepositAddress.transfer(address(this).balance);
        //        // lock this account balance
        emit BuyTest(midasDepositAddress, _recipient, _tokens);
        return true;
    }
