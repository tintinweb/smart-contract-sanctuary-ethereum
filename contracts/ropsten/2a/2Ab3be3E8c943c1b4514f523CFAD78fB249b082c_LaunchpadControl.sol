...
contract LaunchpadControl is 
  ...
  function buyNFT(uint256 _id, uint256 _amount) external payable {
    ...
    uint256 discountRate = getDiscountRate(totalStakedAlu);

    uint256 plutusAmount = items[_id].price.mul(_amount).mul(100 - discountRate).div(100);

    require(
      items[_id].currency != address(0x0) || plutusAmount == msg.value,
      "Invalid amount"
    );
    ...
    emit BuyNFT(msg.sender, item.owner, _id, item.collection, item.token_id, _amount, item.currency, item.price, plutusAmount);
  }
...
contract Membership {
  ...
  function __Membership_init() public {
  ...
  function getDiscountRate(uint256 totalStakedAlu) public view returns (uint256 discountRate) {
