...
import {SquidSwapUser} from "./SquidSwapUser.sol";
...
contract SquidSwapExecutable is AxelarForecallable, SquidSwapUser, Upgradable {
    ...
    constructor(address _gateway, address _gasReceiver, address _swapper)
        SquidSwapUser(_swapper)
    {
    ...
    function contractId() external pure override returns (bytes32) {
        return keccak256('squid-swap-executable');
    }
    ...
    function _sendTrade(
        ...
        gasReceiver.payNativeGasForContractCallWithToken{value: address(this).balance}(
            address(this),
            destinationChain,
            stringAddress,
            payload,
    ...
    function _procureToken(
        address tokenIn,
        uint256 amountIn
    ) internal {
        if (tokenIn == address(0)) {
            if (msg.value < amountIn) revert("INSUFFICIENT_VALUE");
        } else {
            SafeERC20.safeTransferFrom(
                IERC20(tokenIn),
                msg.sender,
    ...
    function tradeSend(
        ...
        _procureToken(tokenIn, amountIn);
        require(_swapSourceWithDelegate(tokenIn, amountIn, tradeData), 'TRADE_FAILED');
    ...
    function tradeSendTrade(
        ...
        _procureToken(tokenIn, amountIn);
        require(_swapSourceWithDelegate(tokenIn, amountIn, tradeData1), 'TRADE_FAILED');
    ...
    function _receiveTrade(
        ...
        if (!_swapDestinationWithDelegate(tokenAddress, amount, tradeData))
    ...
    function _executeWithToken(
        ...
        (
            ...
            bytes32 traceId,
            address fallbackRecipient
        ) = abi.decode(payload, (bytes, bytes32, address));
        _receiveTrade(
            symbol,
            amount,
            data,
...
import {SquidSwapSwapper} from "./SquidSwapSwapper.sol";
import {SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract SquidSwapUser {
    ...
    function _swapSourceWithDelegate(address tokenIn, uint256 amountIn, bytes memory swapSourceCallData) internal returns (bool success){
        (success,) = swapper.delegatecall(abi.encodeWithSelector(
            SquidSwapSwapper.swapSource.selector, 
            tokenIn, 
            amountIn, 
            swapSourceCallData
        ));
    ...
    function _swapSourceWithoutDelegate(address tokenIn, uint256 amountIn, bytes memory swapSourceCallData) internal returns (bool success){
        uint256 value = 0;
        if(tokenIn == address(0)) {
            value = amountIn;
        } else if (IERC20(tokenIn).allowance(address(this), swapper) == 0) {
            SafeERC20.safeApprove(IERC20(tokenIn), swapper, type(uint256).max);
        }
        (success,) = swapper.call{value: value}(abi.encodeWithSelector(
            SquidSwapSwapper.swapSource.selector, 
            tokenIn, 
            amountIn, 
            swapSourceCallData
    ...
    function _swapDestinationWithDelegate(address tokenIn, uint256 amountIn, bytes memory swapDestinationCallData) internal returns (bool success){
        (success,) = swapper.delegatecall(abi.encodeWithSelector(
            SquidSwapSwapper.swapDestination.selector, 
            tokenIn, 
            amountIn, 
            swapDestinationCallData
        ));
    }

    function _swapDestinationWithoutDelegate(address tokenIn, uint256 amountIn, bytes memory swapDestinationCallData) internal returns (bool success){
        uint256 value = 0;
        if(tokenIn == address(0)) {
            value = amountIn;
        } else if (IERC20(tokenIn).allowance(address(this), swapper) == 0) {
...
contract SquidSwapSwapper {
    ...
    function _fixAmountIn(SwapData memory swapData, uint256 amountIn) internal pure {
        //This hack puts the amount in the correct position.
        uint256 inputPos = swapData.inputPos;
        assembly {
            mstore(add(mload(add(swapData, 32)), inputPos), amountIn)
        }
    }

    function _ensureAllowance(address router, IERC20 token) internal {
        if(address(token) == address(0)) return;
        if(token.allowance(address(this), router) != type(uint256).max) {
            token.approve(router, type(uint256).max);
        }
    }

    function _iterate(IERC20 tokenIn, uint256 amountIn, SwapData[] memory swapDataArray) internal{
        SwapData memory swapData = swapDataArray[0];
        _ensureAllowance(swapData.router, tokenIn);
        uint256 value = address(tokenIn) == address(0) ? amountIn : 0;
        (bool success,) = swapData.router.call{value: value}(swapData.callData);
        if(!success) revert("swap failed.");
        for(uint i = 1; i < swapDataArray.length; i++) {
            tokenIn = IERC20(swapData.tokenOut);
            uint256 amountOut = tokenIn.balanceOf(address(this));
            swapData = swapDataArray[i];
            _ensureAllowance(swapData.router, tokenIn);
            _fixAmountIn(swapData, amountOut);
            (success,) = swapData.router.call(swapData.callData);
            if(!success) revert("swap failed.");
        }
    }

    function _procureFunds(IERC20 tokenIn, uint256 amountIn) internal {
        if(address(tokenIn) == address(0)) {
            require(msg.value >= amountIn, "insufficient value");
            return;
        }
        uint256 balance = tokenIn.balanceOf(address(this));
        if(balance >= amountIn) return;
        SafeERC20.safeTransferFrom(tokenIn, msg.sender, address(this), amountIn - balance);
    }

    function swapSource(IERC20 tokenIn, uint256 amountIn, bytes calldata swapSourceCallData) external payable returns (SwapData[] memory swapDataArray) {
        _procureFunds(tokenIn, amountIn);
        swapDataArray = abi.decode(swapSourceCallData, (SwapData[]));
        _iterate(tokenIn, amountIn, swapDataArray);
    }

    function swapDestination(IERC20 tokenIn, uint256 amountIn, bytes calldata swapSourceCallData) external payable returns (SwapData[] memory swapDataArray) {
        _procureFunds(tokenIn, amountIn);
        swapDataArray = abi.decode(swapSourceCallData, (SwapData[]));
        _fixAmountIn(swapDataArray[0], amountIn);
        _iterate(tokenIn, amountIn, swapDataArray);
    }
}
