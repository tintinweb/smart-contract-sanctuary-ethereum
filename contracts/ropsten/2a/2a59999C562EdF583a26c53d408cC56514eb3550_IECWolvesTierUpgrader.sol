...
import "./IECWolves.sol";
import "./IECWolvesTierUpgrades.sol";

error DowngradingNotPossible();
error NotOwnerOfNft();
error NotEnoughEtherSent();
error UpgradeNotActive();

/**
 * @title IECWolves
 * @dev ERC721 for the IEC Wolves NFT project.
 * @custom:version v1.0
 * @custom:date 24 May 2022
 */
contract IECWolvesTierUpgrader is AccessControl, PaymentSplitter {
    bytes32 public constant STAFF_ROLE = keccak256("STAFF_ROLE");

    bool public upgradeActive = false;

    // 0 = Unrevealed, 1 = Bronze, Silver, 1 = Gold, 2 = Exec, 3 = vip
    uint256[] public upgradeFee = [0, 0, 0.05 * 10**18, 0.05 * 10**18, 0.05 * 10**18, 0.05 * 10**18];
    
    // 0 = Unrevealed, 1 = Bronze, Silver, 1 = Gold, 2 = Exec, 3 = vip
    uint256[] public maxUpgrades = [0, 0, 100, 100, 100, 100];

    IECWolves private immutable _iecwolves = IECWolves(address(0xde59bc2bdD9DD4a66e33497BF9b54A9c09bC1B3d)); // Ropsten
    IECWolvesTierUpgrades private immutable _upgrades =
        IECWolvesTierUpgrades(address(0x70B7f4f187c22f41fd8eA00e76361010E0CAfeeD)); // Ropsten

    event TierUpgraded(uint256 tokenId, uint8 tier);

    address[] private _payees = [
        0x442ba05B53FEA8e658EAf77655Df6caE9B93e780,
        0x068b82D7A3EdB51836E6d31dE8C076Af52f06E33,
        0xF4675863670A005aeAdF8fD53C254F2Dd018799F,
        0x5F3fd013493e5644F4379B9D1C2CeA2e657139eA,
        0x67C00cb4C90e324A84474E81A2aE5C5b9118c10c
    ];
    uint256[] private _shares = [30, 10, 20, 20, 20];
    ...
    /**
        @dev tier 2: Silver, 3: Gold, 4: Executive, 5: VIP
     */
    function upgradeTier(uint256 tokenId, uint8 tier) external payable {
        if (!upgradeActive) revert UpgradeNotActive();

        // Must own NFT
        if (_iecwolves.ownerOf(tokenId) != msg.sender) revert NotOwnerOfNft();

        // Get current tier
        uint8 currentTier = _upgrades.getTier(tokenId);

        // Must be higher
        if (tier < currentTier) revert DowngradingNotPossible();

        // Get payment
        if (msg.value < upgradeFee[tier]) revert NotEnoughEtherSent();
        payable(msg.sender).transfer(upgradeFee[tier]);

        // Upgrade
        _upgrades.upgrade(tokenId, tier);

        // Fire upgrade event with token id and tier
        emit TierUpgraded(tokenId, tier);
    }

    function flipUpgradeActive() external onlyRole(STAFF_ROLE) {
        upgradeActive = !upgradeActive;
    }

    function setUpgradeFee(uint8 tier, uint256 _upgradeFee) external onlyRole(STAFF_ROLE) {
        upgradeFee[tier] = _upgradeFee;
    }
}
 ...
 * @dev ERC721 for the IEC Wolves NFT project.
 * @custom:version v1.0
 * @custom:date 31 May 2022
 */
contract IECWolves is ERC721Preset {
    bytes32 public constant MINT_ROLE = keccak256("MINT_ROLE");

    constructor() ERC721Preset("IEC Wolves", "WOLF") {
...
/**
 * @title IEC Wolves Upgrades
 * @dev Upgrade register for the IEC Wolves NFT collection
 * @custom:version v1.0
 * @custom:date 24 May 2022
 */
contract IECWolvesTierUpgrades is AccessControl {
    bytes32 public constant STAFF_ROLE = keccak256("STAFF_ROLE");
    bytes32 public constant UPGRADE_ROLE = keccak256("UPGRADE_ROLE");

    // Mapping from token ID to tier
    // 0: Unrevealed, 1: Bronze, 2: Silver, 3: Gold, 4: Executive, 5: VIP
    ...
    function reveal(uint8[] memory tiers) external onlyRole(STAFF_ROLE) {
        for (uint256 i = 1; i < tiers.length; i++) {
            _tiers[i] = tiers[i];
        }
    }

    function upgrade(uint256 tokenId, uint8 tier) external onlyRole(UPGRADE_ROLE) {
        _tiers[tokenId] = tier;
    }
     ...
     */
    function setProvenance(string memory _provenance) external onlyRole(STAFF_ROLE) {
